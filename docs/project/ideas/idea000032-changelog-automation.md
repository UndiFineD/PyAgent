...
Planned project mapping: prj0000087 (n8n-workflow-bridge, lane=Released), prj0000036 (standards-release, lane=Released)

## Idea summary
This idea focuses on changelog automation in area 6 – Docs. The current signal indicates priority P3, impact M, and urgency L. The SWOT tag is O (Opportunity to improve capability).

## Problem statement
`CHANGELOG.md` is edited manually. Configuring `release-please` or `conventional-changelog` tied to the `feat`/`fix` commit convention would automate release notes.
