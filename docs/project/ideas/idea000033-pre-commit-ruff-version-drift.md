# idea-033 - pre-commit-ruff-version-drift

Planned project mapping: prj0000031 (standards-commit-style, lane=Released)

## Idea summary
This idea focuses on pre commit ruff version drift in area 7 – CI. The current signal indicates priority P3, impact M, and urgency M. The SWOT tag is W (Weakness in current implementation).

## Problem statement
`.pre-commit-config.yaml` pins ruff at `v0.15.5` while `requirements-ci.txt` pins `ruff==0.15.6`. The local pre-commit hook uses a different lint binary than CI.
