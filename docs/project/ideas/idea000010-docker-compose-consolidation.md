# idea-010 - docker-compose-consolidation

Planned project mapping: prj0000012 (deployment-operations, lane=Released)

## Idea summary
This idea focuses on docker compose consolidation in area 8 – Data/Deploy. The current signal indicates priority P2, impact M, and urgency H. The SWOT tag is W (Weakness in current implementation).

## Problem statement
Two independent Docker Compose files exist (`deploy/compose.yaml` and `deploy/docker-compose.yaml`) with different service topologies. Consolidate into one parameterised file.
