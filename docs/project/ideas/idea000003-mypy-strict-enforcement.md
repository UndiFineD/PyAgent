...
## Problem statement
`mypy.ini` sets `strict = False` and `ignore_errors = True`, rendering all type annotations decorative. Progressive mypy strictness should be enabled starting with the `src/core/` package.
