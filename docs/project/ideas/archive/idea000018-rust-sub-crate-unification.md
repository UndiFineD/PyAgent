...
## Problem statement
`rust_core/crdt/`, `rust_core/p2p/`, and `rust_core/security/` are standalone Cargo crates with separate `Cargo.lock` files. Consider a Cargo workspace to unify the dependency graph.
