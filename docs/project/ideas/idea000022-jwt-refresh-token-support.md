...
## Problem statement
`backend/auth.py` supports API key + single JWT but no refresh token or session rotation. Long-lived sessions with non-expiring JWTs are a security risk for a multi-agent system.
