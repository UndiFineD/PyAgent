# idea-038 - chromadb-optional-extra

Planned project mapping: None yet

## Idea summary
This idea focuses on chromadb optional extra in area 9 – Dependencies. The current signal indicates priority P4, impact M, and urgency L. The SWOT tag is W (Weakness in current implementation).

## Problem statement
`chromadb>=0.5.0` is listed in `pyproject.toml` main dependencies but not in `requirements.txt`. ChromaDB pulls in heavy ML dependencies and should be an optional `[vector]` extra.
