# idea-027 - windows-ci-matrix

Planned project mapping: prj0000069 (ci-test-parallelization, lane=Released), prj0000035 (standards-ci, lane=Released)

## Idea summary
This idea focuses on windows ci matrix in area 5 – Tests. The current signal indicates priority P3, impact H, and urgency M. The SWOT tag is W (Weakness in current implementation).

## Problem statement
All 5 CI workflows run exclusively on `ubuntu-latest`. Since the primary developer OS is Windows and the app distributes a `.pyd` Rust extension, a Windows runner is needed in CI.
