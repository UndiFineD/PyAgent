# idea180384 - run-static-checks API Consistency

Planned project mapping: none yet

## Idea summary
Generate a focused improvement initiative for `src-old/tools/run_static_checks.py` with objective: Normalize interfaces and reduce behavioral drift.

## Problem statement
Legacy file `src-old/tools/run_static_checks.py` from v3.7.0 may contain latent quality risks, drift, or missing modernization opportunities.
...
## Research findings
Static signals: size=3056 bytes, function_like_tokens=1, todo_like_tokens=0, has_test_signal=False.
...
## Source references
- `src-old/tools/run_static_checks.py`
