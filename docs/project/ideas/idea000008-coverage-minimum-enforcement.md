...
## Problem statement
`quality.yml` passes `--cov-fail-under=1`, which is effectively no threshold. Setting a meaningful baseline (≥70%) and iteratively raising it would prevent coverage regressions.
