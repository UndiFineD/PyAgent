...
## Idea summary
This idea focuses on torch optional dependency split in area 9 – Dependencies. The current signal indicates priority P3, impact M, and urgency M. The SWOT tag is W (Weakness in current implementation).

## Problem statement
`torch>=2.5.0` is listed in `pyproject.toml` main dependencies but absent from `requirements.txt`. Torch is ~2GB and should be an optional `[ai]` extra to avoid forcing the download.
