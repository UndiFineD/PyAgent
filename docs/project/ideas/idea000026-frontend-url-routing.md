...
## Idea summary
This idea focuses on frontend url routing in area 4 – Frontend. The current signal indicates priority P3, impact M, and urgency L. The SWOT tag is O (Opportunity to improve capability).

## Problem statement
Navigation between app windows is purely DOM/event driven with no URL routing. Adding React Router would enable deep-linking, browser history, and shareable window states.
