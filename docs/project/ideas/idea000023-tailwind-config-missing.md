...
## Idea summary
This idea focuses on tailwind config missing in area 4 – Frontend. The current signal indicates priority P3, impact M, and urgency H. The SWOT tag is W (Weakness in current implementation).

## Problem statement
The web app uses `tailwind-merge` and `clsx` but has no `tailwind.config.ts`. Without a config, JIT purge paths are undefined and design tokens cannot be centrally managed.
