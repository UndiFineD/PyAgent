...
Planned project mapping: prj0000073 (api-documentation, lane=Released), prj0000028 (api-reference, lane=Released)

## Idea summary
This idea focuses on automated api docs ci in area 6 – Docs. The current signal indicates priority P3, impact M, and urgency M. The SWOT tag is W (Weakness in current implementation).

## Problem statement
`docs/api/index.md` contains only a mkdocstrings placeholder. MkDocs is in `requirements-ci.txt` but no CI job runs `mkdocs build`, so the published API reference is never built.
