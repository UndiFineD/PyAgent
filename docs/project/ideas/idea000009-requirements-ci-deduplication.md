# idea-009 - requirements-ci-deduplication

Planned project mapping: prj0000077 (ci-backend-deps, lane=Released), prj0000034 (standards-docs, lane=Released)

## Idea summary
This idea focuses on requirements ci deduplication in area 9 – Dependencies. The current signal indicates priority P2, impact M, and urgency H. The SWOT tag is W (Weakness in current implementation).

## Problem statement
`pytest-xdist>=3.6` appears twice in `requirements-ci.txt`. Duplicate lines indicate the file is not mechanically validated; add a structure test to catch this.
