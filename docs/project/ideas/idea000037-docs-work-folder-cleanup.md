...
## Idea summary
This idea focuses on docs work folder cleanup in area 6 – Docs. The current signal indicates priority P4, impact L, and urgency L. The SWOT tag is W (Weakness in current implementation).

## Problem statement
`docs/work/` contains 10+ stale working documents that are no longer current. These should be archived to `docs/archive/` or deleted.
