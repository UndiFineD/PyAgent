...
Planned project mapping: prj0000026 (architecture-adr-template, lane=Released)

## Idea summary
This idea focuses on adr backfill in area 6 – Docs. The current signal indicates priority P3, impact M, and urgency M. The SWOT tag is O (Opportunity to improve capability).

## Problem statement
Only an ADR template exists. Major architectural decisions (transaction manager, Rust FFI, P2P CRDT, E2E encryption, maturin build) are undocumented in ADR format.
