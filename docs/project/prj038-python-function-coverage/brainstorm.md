...
- Motivation is not to prove correctness, but to catch runtime issues 
    such as missing imports, invalid defaults, or other immediate exceptions.
...
- For each function definition (excluding test helpers and private helpers), 
    attempt to call it with default/dummy args.
