...
- `.github/agents/0master.memory.md` — master-level plan & decisions
- `.github/agents/1project.memory.md` — project boundary, scope, and branch-plan coordination
- `.github/agents/2think.memory.md` — deep analysis, reasoning, and alternatives
- `.github/agents/3design.memory.md` — selected design direction and interface decisions
- `.github/agents/4plan.memory.md` — implementation plans and task breakdowns
- `.github/agents/5test.memory.md` — testing strategy and test plan status
- `.github/agents/6code.memory.md` — code-workflow tracking and code health notes
- `.github/agents/7exec.memory.md` — execution-focused notes (deploy, infra, runtime)
- `.github/agents/8ql.memory.md` — query and security-analysis note tracking
- `.github/agents/9git.memory.md` — git process, branch strategy, PRs, branch hygiene failures
...
- If a number must be skipped, reserved, or retired, record the reason in `.github/agents/0master.memory.md` so later coordinators can trace the sequence.
...
- If a branch mismatch, inherited branch, or mixed-project file set is discovered, stop git handoff, record the failure in `.github/agents/0master.memory.md`, and send the task back to the agent that owns the project overview correction.
...
3. Compare: if observed branch != expected branch, stop delegation immediately → **BLOCKED**:
   ...
   - Mark the task blocked in `.github/agents/0master.memory.md`.
   ...
   - If the correct branch cannot be determined, escalate to `@1project` and record `BLOCKED` in `.github/agents/0master.memory.md`.
...
- Write / append to `.github/agents/0master.memory.md` with decisions and next steps.
- Use `.github/agents/2think.memory.md` for deeper analysis and alternatives.

**How to keep the master agent lean:**
- Push detailed technical discussion into the appropriate specialized memory file.
- Keep `.github/agents/0master.memory.md` focused on decisions, outcomes, and next actions.
...
6. Update `.github/agents/0master.memory.md` (or relevant memory file) to explain why this agent exists and how it should be used.
