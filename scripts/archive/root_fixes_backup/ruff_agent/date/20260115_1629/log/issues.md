## Issues found by ruffAgent at 20260115-1629
...
F841 Local variable `total_latency` is assigned to but never used
   --> src\observability\stats\exporters\OTelManager.py:140:13
    |
138 |                 raw_span.attributes.update(attributes)
139 |
140 |             total_latency = raw_span.end_time - raw_span.start_time
    |             ^^^^^^^^^^^^^
141 |             # ... existing logic for completed_spans could go here if needed for export_spans()
142 |             self.completed_spans.append(raw_span)
    |
help: Remove assignment to unused variable `total_latency`

F401 `.analysis.MODEL_COSTS` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> src\observability\stats\metrics_engine.py:50:27
   |
49 | try:
50 |     from .analysis import MODEL_COSTS, HAS_PSUTIL
   |                           ^^^^^^^^^^^
51 |     import psutil
...
F811 Redefinition of unused `DerivedMetricCalculator` from line 45
   --> src\observability\stats\metrics_engine.py:298:7
    |
298 | class DerivedMetricCalculator:
    |       ^^^^^^^^^^^^^^^^^^^^^^^ `DerivedMetricCalculator` redefined here
299 |     """Calculate derived metrics from dependencies using safe AST evaluation."""
    |
   ::: src\observability\stats\metrics_engine.py:45:5
    |
 43 |     TokenCostCore,
 44 |     ModelFallbackCore,
 45 |     DerivedMetricCalculator,
...
F841 Local variable `content` is assigned to but never used
  --> temp\better_fix_pytest.py:89:5
   |
88 |     # Read current file
89 |     content = conftest_path.read_text("utf-8")
   |     ^^^^^^^
90 |
91 |     # Simple replace of the function
   |
help: Remove assignment to unused variable `content`

F841 Local variable `pattern` is assigned to but never used
  --> temp\better_fix_pytest.py:93:5
   |
91 |     # Simple replace of the function
92 |     # We look for the decorator and the function def
93 |     pattern = r'@pytest\.fixture\(name="agent_backend_module"\)\s*\ndef agent_backend_module\(\):[\s\S]*?return [^\n]*'
   |     ^^^^^^^
94 |     # Need to match strictly to avoid eating too much if there's code after
95 |     # But usually it is the end or followed by skipped lines.
   |
help: Remove assignment to unused variable `pattern`

F841 Local variable `pattern` is assigned to but never used
   --> temp\better_fix_pytest.py:220:5
    |
218 |     # Match the fixture function
219 |     # We match @pytest.fixture and the function definition
220 |     pattern = r"(@pytest\.fixture(?:.*\n)*def base_agent_module\(\).*:(?:\n\s+.*)+)"
    |     ^^^^^^^
221 |     # This greedy match might eat too much.
    |
help: Remove assignment to unused variable `pattern`

F841 Local variable `in_target` is assigned to but never used
   --> temp\better_fix_pytest.py:230:5
    |
228 |     new_lines = []
229 |
230 |     in_target = False
...
E741 Ambiguous variable name: `l`
  --> temp\debug_fix.py:48:13
   |
46 |     ):
47 |         while other_lines:
48 |             l = other_lines.pop(0)
   |             ^
49 |             final_output.append(l)
50 |             if '"""' in l or "'''" in l:
   |

E722 Do not use bare `except`
   --> temp\find_stubs_v3.py:108:5
    |
106 |                     return False
107 |             return has_definition
108 |     except:
    |     ^^^^^^
109 |         return False
    |
...
F841 Local variable `docstring_end_idx` is assigned to but never used
  --> temp\fix_e402_and_headers_v4.py:49:9
   |
47 |     ):
48 |         # It's a docstring
49 |         docstring_end_idx = (
   |         ^^^^^^^^^^^^^^^^^
50 |             tree.body[0].end_lineno - 1
51 |         )  # 0-indexed index in original lines
   |
help: Remove assignment to unused variable `docstring_end_idx`

E741 Ambiguous variable name: `l`
  --> temp\fix_e402_and_headers_v4.py:86:13
   |
84 |     ):
85 |         while other_lines:
86 |             l = other_lines.pop(0)
   |             ^
87 |             final_output.append(l)
88 |             if '"""' in l or "'''" in l:
   |

F841 Local variable `docstring_node` is assigned to but never used
  --> temp\fix_e402_docstrings.py:26:13
   |
24 |         ):
25 |             # It's already at the top
26 |             docstring_node = tree.body[0]
   |             ^^^^^^^^^^^^^^
27 |
28 |     # Look for a string literal that is NOT at the top
   |
help: Remove assignment to unused variable `docstring_node`

F841 Local variable `new_lines` is assigned to but never used
  --> temp\fix_e402_versions_v2.py:13:5
   |
11 |         return False
12 |
13 |     new_lines = []
...
F841 Local variable `import_lines` is assigned to but never used
  --> temp\fix_e402_versions_v2.py:14:5
   |
13 |     new_lines = []
14 |     import_lines = []
   |     ^^^^^^^^^^^^
...
15 |     other_lines = []
   |     ^^^^^^^^^^^
16 |
17 |     # We want to find all import statements and move them to the top,
   |
help: Remove assignment to unused variable `other_lines`

F841 Local variable `footer` is assigned to but never used
  --> temp\fix_e402_versions_v2.py:22:5
   |
20 |     # First, separate docstrings and __future__ imports.
21 |     header = []
22 |     footer = []
   |     ^^^^^^
23 |     middle = []
...
F841 Local variable `middle` is assigned to but never used
  --> temp\fix_e402_versions_v2.py:23:5
   |
21 |     header = []
22 |     footer = []
23 |     middle = []
...
Found 531 errors.
No fixes available (66 hidden fixes can be enabled with the `--unsafe-fixes` option).
