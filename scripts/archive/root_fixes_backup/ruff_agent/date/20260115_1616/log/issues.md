## Issues found by ruffAgent at 20260115-1616

```
E402 Module level import not at top of file
  --> debug_registry.py:17:1
   |
17 | from src.infrastructure.fleet.OrchestratorRegistry import LazyOrchestratorMap
   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   |

E402 Module level import not at top of file
  --> debug_registry_v2.py:17:1
   |
17 | from src.infrastructure.fleet.AgentRegistry import LazyAgentMap
   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
18 | from src.infrastructure.fleet.OrchestratorRegistry import LazyOrchestratorMap
   |

E402 Module level import not at top of file
  --> debug_registry_v2.py:18:1
   |
17 | from src.infrastructure.fleet.AgentRegistry import LazyAgentMap
18 | from src.infrastructure.fleet.OrchestratorRegistry import LazyOrchestratorMap
   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
E402 Module level import not at top of file
  --> debug_registry_v3.py:18:1
   |
18 | from src.infrastructure.fleet.AgentRegistry import LazyAgentMap
   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   |

F841 Local variable `processed_pairs` is assigned to but never used
   --> src\core\base\NeuralPruningEngine.py:185:9
    |
184 |         # Basic similarity check (heuristic: symbol overlap)
185 |         processed_pairs: set[Any] = set()
    |         ^^^^^^^^^^^^^^^
186 |         definitions = self._discover_definitions(search_root)
    |
help: Remove assignment to unused variable `processed_pairs`

F811 Redefinition of unused `right_to_be_forgotten` from line 107
   --> src\core\base\ShardedKnowledgeCore.py:161:15
    |
159 |         await self.create_index_snapshot()
160 |
161 |     async def right_to_be_forgotten(self, entity_name: str) -> bool:
    |               ^^^^^^^^^^^^^^^^^^^^^ `right_to_be_forgotten` redefined here
162 |         """
163 |         Removes an entity from the knowledge store across all shards
    |
   ::: src\core\base\ShardedKnowledgeCore.py:107:15
    |
105 |         return stable
106 |
107 |     async def right_to_be_forgotten(self, entity_name: str) -> bool:
    |               --------------------- previous definition of `right_to_be_forgotten` here
108 |         """
109 |         Phase 238: Prunes all knowledge associated with a specific entity (user/project)
    |
...
F401 `.AgentPluginBase.AgentPluginBase` imported but unused; consider removing, adding to `__all__`, or using a redundant alias
  --> src\core\base\__init__.py:37:30
   |
35 |     OrchestratorInterface as OrchestratorInterface,
36 | )
37 | from .AgentPluginBase import AgentPluginBase
   |                              ^^^^^^^^^^^^^^^
38 | from .models.enums import HealthStatus
   |
...
F401 `.models.enums.HealthStatus` imported but unused; consider removing, adding to `__all__`, or using a redundant alias
  --> src\core\base\__init__.py:38:27
   |
36 | )
37 | from .AgentPluginBase import AgentPluginBase
38 | from .models.enums import HealthStatus
...
F841 Local variable `registry` is assigned to but never used
  --> src\core\base\delegation.py:60:9
   |
59 |         # Check registry for existing instance of this type to avoid redundant spawns
60 |         registry = AgentRegistry()
   |         ^^^^^^^^
61 |         type_clean = agent_type.replace("Agent", "").lower()
   |
help: Remove assignment to unused variable `registry`

F841 Local variable `type_clean` is assigned to but never used
  --> src\core\base\delegation.py:61:9
   |
59 |         # Check registry for existing instance of this type to avoid redundant spawns
60 |         registry = AgentRegistry()
61 |         type_clean = agent_type.replace("Agent", "").lower()
...
F821 Undefined name `sys`
   --> src\core\base\utils\DiffGenerator.py:150:17
    |
148 |         for line in diff_result.diff_lines:
149 |             if line.startswith("+") and not line.startswith("+++"):
150 |                 sys.stdout.write(f"\033[92m{line}\033[0m")  # Green
    |                 ^^^
151 |             elif line.startswith("-") and not line.startswith("---"):
152 |                 sys.stdout.write(f"\033[91m{line}\033[0m")  # Red
    |

F821 Undefined name `sys`
   --> src\core\base\utils\DiffGenerator.py:152:17
    |
150 |                 sys.stdout.write(f"\033[92m{line}\033[0m")  # Green
151 |             elif line.startswith("-") and not line.startswith("---"):
152 |                 sys.stdout.write(f"\033[91m{line}\033[0m")  # Red
...
154 |                 sys.stdout.write(f"\033[96m{line}\033[0m")  # Cyan
    |

F821 Undefined name `sys`
   --> src\core\base\utils\DiffGenerator.py:154:17
    |
152 |                 sys.stdout.write(f"\033[91m{line}\033[0m")  # Red
153 |             elif line.startswith("@@"):
154 |                 sys.stdout.write(f"\033[96m{line}\033[0m")  # Cyan
...
F821 Undefined name `sys`
   --> src\core\base\utils\DiffGenerator.py:156:17
    |
154 |                 sys.stdout.write(f"\033[96m{line}\033[0m")  # Cyan
155 |             else:
156 |                 sys.stdout.write(line)
...
F821 Undefined name `sys`
   --> src\core\base\utils\DiffGenerator.py:157:9
    |
155 |             else:
156 |                 sys.stdout.write(line)
157 |         sys.stdout.flush()
...
36 |
37 | from src.core.base.version import VERSION
   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
38 | import json
39 | import time
   |
...
44 | from pathlib import Path
   | ^^^^^^^^^^^^^^^^^^^^^^^^
45 | from typing import Any
46 | from src.infrastructure.fleet.FleetManager import FleetManager
   |
...
45 | from typing import Any
   | ^^^^^^^^^^^^^^^^^^^^^^
46 | from src.infrastructure.fleet.FleetManager import FleetManager
47 | from src.observability.StructuredLogger import StructuredLogger
   |

E402 Module level import not at top of file
  --> src\infrastructure\dev\scripts\run_fleet_self_improvement.py:46:1
   |
44 | from pathlib import Path
45 | from typing import Any
46 | from src.infrastructure.fleet.FleetManager import FleetManager
...
E402 Module level import not at top of file
  --> src\infrastructure\dev\scripts\run_fleet_self_improvement.py:47:1
   |
45 | from typing import Any
46 | from src.infrastructure.fleet.FleetManager import FleetManager
47 | from src.observability.StructuredLogger import StructuredLogger
   | ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
48 |
49 | # Phase 120: Load environment variables if available
   |

F821 Undefined name `loop`
   --> src\infrastructure\orchestration\ConsensusOrchestrator.py:137:24
    |
135 |                         f"{p['agent']}.refine", context=context
136 |                     )
137 |                     if loop.is_running():
    |                        ^^^^
138 |                         coro.close()
139 |                         refined = f"[DEFERRED] {p['agent']} refine"
    |

F821 Undefined name `loop`
   --> src\infrastructure\orchestration\ConsensusOrchestrator.py:141:35
    |
139 |                         refined = f"[DEFERRED] {p['agent']} refine"
140 |                     else:
141 |                         refined = loop.run_until_complete(coro)
    |                                   ^^^^
142 |
143 |                     new_proposals.append(
    |

F821 Undefined name `AsyncGenerator`
  --> src\infrastructure\orchestration\LockManager.py:68:10
   |
66 |     async def acquire_async(
67 |         self, resource_id: str, lock_type: str = "memory", timeout: float = 10.0
68 |     ) -> AsyncGenerator[None, None]:
   |          ^^^^^^^^^^^^^^
69 |         """Phase 152: Async-native lock acquisition."""
70 |         if lock_type == "file":
   |

F821 Undefined name `Generator`
   --> src\infrastructure\orchestration\LockManager.py:147:10
    |
145 |     def acquire(
146 |         self, resource_id: str, lock_type: str = "memory", timeout: float = 10.0
147 |     ) -> Generator[None, None, None]:
    |          ^^^^^^^^^
148 |         """Generic lock acquisition helper."""
149 |         if lock_type == "file":
    |

F841 Local variable `new_content` is assigned to but never used
   --> src\infrastructure\orchestration\core\SelfImprovementCore.py:175:9
    |
173 |         Applies non-AI assisted simple fixes.
174 |         """
175 |         new_content = content
    |         ^^^^^^^^^^^
176 |
177 |         if issue_type == "Robustness Issue":
    |
...
F401 `src.logic.agents.compliance.core.ComplianceCore.ComplianceIssue` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> src\logic\agents\compliance\ComplianceAgent.py:11:9
   |
 9 |     from src.logic.agents.compliance.core.ComplianceCore import (
10 |         ComplianceCore,
11 |         ComplianceIssue,
   |         ^^^^^^^^^^^^^^^
12 |     )
...
47 |             "issue_count": len(all_issues),
   |                                ^^^^^^^^^^
48 |             "critical_violations": [
49 |                 i.message for i in all_issues if i.severity == "CRITICAL"
   |

F821 Undefined name `all_issues`
  --> src\logic\agents\compliance\ComplianceAgent.py:49:36
   |
47 |             "issue_count": len(all_issues),
48 |             "critical_violations": [
49 |                 i.message for i in all_issues if i.severity == "CRITICAL"
   |                                    ^^^^^^^^^^
50 |             ],
51 |             "status": "PASS" if score > 0.8 else "FAIL",
   |

F821 Undefined name `time`
   --> src\logic\agents\swarm\FleetDeployerAgent.py:121:26
    |
119 |             "type": agent_type,
120 |             "status": "provisioning",
121 |             "timestamp": time.time() if "time" in globals() else 0,
    |                          ^^^^
122 |         }
    |
...
F821 Undefined name `total`
   --> src\observability\improvements\ImprovementManager.py:263:16
    |
261 |             created_at=datetime.now().isoformat(),
262 |         )
263 |         return total
    |                ^^^^^
264 |
265 |     def export_markdown(self) -> str:
    |

F811 Redefinition of unused `calculate` from line 280
   --> src\observability\stats\MetricsCore.py:334:9
    |
332 |         return metric
333 |
334 |     def calculate(self, name: str, context: dict[str, float]) -> float:
    |         ^^^^^^^^^ `calculate` redefined here
335 |         if name not in self.derived_metrics:
336 |             return 0.0
    |
   ::: src\observability\stats\MetricsCore.py:280:9
    |
278 |             raise TypeError(f"Unsupported operation: {type(node)}")
279 |
280 |     def calculate(self, metric_name: str, context: dict[str, float]) -> float:
    |         --------- previous definition of `calculate` here
281 |         """Calculate derived metric value."""
282 |         if metric_name not in self.derived_metrics:
    |
help: Remove definition: `calculate`

F821 Undefined name `version`
  --> src\observability\stats\ab_engine.py:91:12
   |
89 |             "metrics_count": len(comp.metrics_a) + len(comp.metrics_b),
90 |         }
91 |         if version.lower() == "a":
   |            ^^^^^^^
92 |             comp.metrics_a[metric_name] = value
93 |         elif version.lower() == "b":
   |

F821 Undefined name `metric_name`
  --> src\observability\stats\ab_engine.py:92:28
   |
90 |         }
91 |         if version.lower() == "a":
92 |             comp.metrics_a[metric_name] = value
   |                            ^^^^^^^^^^^
93 |         elif version.lower() == "b":
94 |             comp.metrics_b[metric_name] = value
   |

F821 Undefined name `value`
  --> src\observability\stats\ab_engine.py:92:43
   |
90 |         }
91 |         if version.lower() == "a":
92 |             comp.metrics_a[metric_name] = value
...
F821 Undefined name `version`
  --> src\observability\stats\ab_engine.py:93:14
   |
91 |         if version.lower() == "a":
92 |             comp.metrics_a[metric_name] = value
93 |         elif version.lower() == "b":
...
F821 Undefined name `metric_name`
  --> src\observability\stats\ab_engine.py:94:28
   |
92 |             comp.metrics_a[metric_name] = value
93 |         elif version.lower() == "b":
94 |             comp.metrics_b[metric_name] = value
...
F821 Undefined name `value`
  --> src\observability\stats\ab_engine.py:9
... [TRUNCATED] ...
^^^^^^^^^^^^^^^^^^^^^
...
F401 `tests.utils.agent_test_utils.load_module_from_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_COMPREHENSIVE_UNIT.py:20:9
   |
18 |         AGENT_DIR,
19 |         agent_sys_path,
20 |         load_module_from_path,
   |         ^^^^^^^^^^^^^^^^^^^^^
21 |         agent_dir_on_path,
22 |     )
   |
help: Remove unused import
...
F401 `tests.utils.agent_test_utils.agent_dir_on_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_COMPREHENSIVE_UNIT.py:21:9
   |
19 |         agent_sys_path,
20 |         load_module_from_path,
21 |         agent_dir_on_path,
...
F401 `tests.utils.agent_test_utils.load_module_from_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_CORE_UNIT.py:18:9
   |
16 |         AGENT_DIR,
17 |         agent_sys_path,
18 |         load_module_from_path,
   |         ^^^^^^^^^^^^^^^^^^^^^
19 |         agent_dir_on_path,
20 |     )
   |
help: Remove unused import

F401 `tests.utils.agent_test_utils.agent_dir_on_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_CORE_UNIT.py:19:9
   |
17 |         agent_sys_path,
18 |         load_module_from_path,
19 |         agent_dir_on_path,
...
F401 `tests.utils.agent_test_utils.load_module_from_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_INTEGRATION.py:18:9
   |
16 |         AGENT_DIR,
17 |         agent_sys_path,
18 |         load_module_from_path,
...
F401 `tests.utils.agent_test_utils.agent_dir_on_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_INTEGRATION.py:19:9
   |
17 |         agent_sys_path,
18 |         load_module_from_path,
19 |         agent_dir_on_path,
...
F821 Undefined name `Self`
  --> tests\unit\test_utils\test_test_utils_INTEGRATION.py:26:32
   |
25 |     class agent_sys_path:
...
F403 `from tests.utils.agent_test_utils import *` used; unable to detect undefined names
 --> tests\unit\test_utils\test_test_utils_LEGACY.py:4:5
  |
3 | try:
...
F405 `agent_dir_on_path` may be undefined, or defined from star imports
  --> tests\unit\test_utils\test_test_utils_LEGACY.py:12:10
   |
10 |     """Test that agent_dir_on_path adds AGENT_DIR to sys.path."""
11 |     original_path = list(sys.path)
12 |     with agent_dir_on_path():
   |          ^^^^^^^^^^^^^^^^^
13 |         assert str(AGENT_DIR) in sys.path
   |

F405 `AGENT_DIR` may be undefined, or defined from star imports
  --> tests\unit\test_utils\test_test_utils_LEGACY.py:13:20
   |
11 |     original_path = list(sys.path)
12 |     with agent_dir_on_path():
13 |         assert str(AGENT_DIR) in sys.path
...
F401 `tests.utils.agent_test_utils.load_module_from_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_PERFORMANCE.py:15:9
   |
13 |         AGENT_DIR,
14 |         agent_sys_path,
15 |         load_module_from_path,
   |         ^^^^^^^^^^^^^^^^^^^^^
16 |         agent_dir_on_path,
17 |     )
   |
help: Remove unused import

F401 `tests.utils.agent_test_utils.agent_dir_on_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_PERFORMANCE.py:16:9
   |
14 |         agent_sys_path,
15 |         load_module_from_path,
16 |         agent_dir_on_path,
...
F821 Undefined name `Self`
  --> tests\unit\test_utils\test_test_utils_PERFORMANCE.py:23:32
   |
...
F401 `tests.utils.agent_test_utils.load_module_from_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_UNIT.py:16:9
   |
14 |         AGENT_DIR,
15 |         agent_sys_path,
16 |         load_module_from_path,
   |         ^^^^^^^^^^^^^^^^^^^^^
17 |         agent_dir_on_path,
18 |     )
   |
help: Remove unused import

F401 `tests.utils.agent_test_utils.agent_dir_on_path` imported but unused; consider using `importlib.util.find_spec` to test for availability
  --> tests\unit\test_utils\test_test_utils_UNIT.py:17:9
   |
15 |         agent_sys_path,
16 |         load_module_from_path,
17 |         agent_dir_on_path,
...
F841 Local variable `MockAIBackend` is assigned to but never used
   --> tests\unit\test_utils\test_test_utils_UNIT.py:502:9
    |
500 |     def test_mock_response_with_custom_content(self, utils_module: Any) -> None:
501 |         """Test mock response with custom content."""
502 |         MockAIBackend = utils_module.MockAIBackend
    |         ^^^^^^^^^^^^^
503 |         MockResponse = utils_module.MockResponse
    |
help: Remove assignment to unused variable `MockAIBackend`

F841 Local variable `MockResponse` is assigned to but never used
   --> tests\unit\test_utils\test_test_utils_UNIT.py:503:9
    |
501 |         """Test mock response with custom content."""
502 |         MockAIBackend = utils_module.MockAIBackend
503 |         MockResponse = utils_module.MockResponse
...
F841 Local variable `original_content` is assigned to but never used
   --> tests\utils\legacy_support.py:107:17
    |
105 |             original_content = ""
106 |             if file_path.exists():
107 |                 original_content = file_path.read_text()
    |                 ^^^^^^^^^^^^^^^^
108 |             return None
    |
...
E741 Ambiguous variable name: `l`
   --> tests\utils\legacy_support.py:203:48
    |
201 |             if ignore_file.exists():
202 |                 lines = ignore_file.read_text().splitlines()
203 |                 patterns.extend([l.strip() for l in lines if l.strip()])
    |                                                ^
204 |             # Check parent just in case (though test seems to test load_cascading_codeignore_loads_subdirectory_patterns)
205 |             if path and path != self.repo_root:
    |

E741 Ambiguous variable name: `l`
   --> tests\utils\legacy_support.py:209:52
    |
207 |                 if root_ignore.exists():
208 |                     lines = root_ignore.read_text().splitlines()
209 |                     patterns.extend([l.strip() for l in lines if l.strip()])
    |                                                    ^
210 |             return patterns
    |

Found 553 errors.
No fixes available (69 hidden fixes can be enabled with the `--unsafe-fixes` option).
