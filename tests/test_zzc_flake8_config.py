...
def test_flake8_repo_config_has_no_repo_issues() -> None:
    """Ensure the checked-in flake8 config passes for the repository."""
    ...
    res = subprocess.run(
        [sys.executable, "-m", "flake8", "--config", str(repo_config), str(repo_root)],
        cwd=str(repo_root),
        capture_output=True,
        text=True,
        check=False,
    ...
    assert res.returncode == 0, output or "flake8 reported issues in the repository"
