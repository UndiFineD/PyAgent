...
class TestRAGCore:
    ...
    @pytest.fixture
    def core(self):
        """Create a fresh RAG core instance for each test."""
        return RAGCore()

    @pytest.mark.asyncio
    async def test_register_vector_store(self, core):
        """Test vector store registration."""
        store_id = await core.register_vector_store(
            store_id="test_store",
            store_type=VectorStoreType.QDRANT,
            config={"host": "localhost", "port": 6333}
        )

        assert store_id == "test_store"
        assert store_id in core.vector_stores
    ...
    async def test_create_rag_tool(self, core):
        ...
        tool = await core.create_rag_tool(
            tool_id="test_tool",
            name="Test RAG Tool",
            description="A test RAG tool",
            vector_store_id="test_store",
            collection_name="test_collection"
        )

        assert tool.tool_id == "test_tool"
        assert tool.name == "Test RAG Tool"
        assert tool.collection_name == "test_collection"
        assert tool.tool_id in core.rag_tools

    @pytest.mark.asyncio
    async def test_create_rag_tool_invalid_store(self, core):
        """Test creating RAG tool with invalid vector store."""
        with pytest.raises(ValueError, match="Vector store nonexistent not found"):
    ...
    async def test_create_rag_tool_duplicate(self, core):
        """Test creating duplicate RAG tool."""
        await core.register_vector_store("test_store", VectorStoreType.QDRANT, {})
        ...
        with pytest.raises(ValueError, match="RAG tool test_tool already exists"):
            await core.create_rag_tool(
                tool_id="test_tool",
                name="Duplicate Tool",
                description="Test",
    ...
    async def test_add_documents(self, core):
        """Test adding documents to RAG tool."""
        await core.register_vector_store("test_store", VectorStoreType.QDRANT, {})
        await core.create_rag_tool(
            tool_id="test_tool",
            name="Test Tool",
        ...
        documents = [
            Document(
                ...
                metadata={"author": "test", "topic": "programming"}
            ),
            Document(
                doc_id="doc2",
        ...
        assert "doc1" in core.documents
        assert "doc2" in core.documents

    @pytest.mark.asyncio
    async def test_add_documents_with_chunking(self, core):
        """Test adding documents with chunking."""
        await core.register_vector_store("test_store", VectorStoreType.QDRANT, {})
        await core.create_rag_tool(
            tool_id="test_tool",
            name="Test Tool",
    ...
    async def test_retrieve_similarity(self, core):
        ...
        # Add test documents
        documents = [
            Document(doc_id="doc1", content="Python programming tutorial"),
            Document(doc_id="doc2", content="Machine learning with Python"),
            Document(doc_id="doc3", content="Cooking recipes and ingredients")
        ]
    ...
    async def test_retrieve_mmr(self, core):
        """Test MMR (Maximal Marginal Relevance) retrieval."""
        await core.register_vector_store("test_store", VectorStoreType.QDRANT, {})
        await core.create_rag_tool(
            tool_id="test_tool",
            name="Test Tool",
        ...
        # Add test documents
        documents = [
            Document(doc_id="doc1", content="Python programming tutorial basics"),
            Document(doc_id="doc2", content="Advanced Python programming techniques"),
            Document(doc_id="doc3", content="Python web development with Django"),
            Document(doc_id="doc4", content="Cooking with Python ingredients")
        ]
        await core.add_documents("test_tool", documents)
    ...
    async def test_retrieve_multi_query(self, core):
        ...
        documents = [
            Document(doc_id="doc1", content="Python programming tutorial"),
            Document(doc_id="doc2", content="Machine learning algorithms"),
            Document(doc_id="doc3", content="Data science with Python")
        ]
        await core.add_documents("test_tool", documents)
    ...
    async def test_retrieve_with_filters(self, core):
        ...
        documents = [
            Document(doc_id="doc1", content="Python tutorial", metadata={"topic": "programming"}),
            Document(doc_id="doc2", content="Cooking recipes", metadata={"topic": "cooking"}),
            Document(doc_id="doc3", content="Python data science", metadata={"topic": "programming"})
        ]
        await core.add_documents("test_tool", documents)

        # Retrieve with filter
        filters = {"topic": "programming"}
        result = await core.retrieve("test_tool", "Python", filters=filters)
    ...
    async def test_rag_query(self, core):
        """Test complete RAG query with response generation."""
        await core.register_vector_store("test_store", VectorStoreType.QDRANT, {})
        await core.create_rag_tool(
            tool_id="test_tool",
            name="Test Tool",
        ...
        # Add test document
        document = Document(doc_id="doc1", content="Python is a programming language.")
        await core.add_documents("test_tool", [document])

        # Perform RAG query
        rag_query = RAGQuery(query="What is Python?")
        result = await core.rag_query("test_tool", rag_query, generate_response=True)

        assert result["query"] == "What is Python?"
        assert isinstance(result["retrieval_result"], RetrievalResult)
        assert "generated_response" in result
        assert "Python is a programming language" in result["generated_response"]
    ...
    async def test_update_document(self, core):
        ...
        # Add document
        document = Document(doc_id="doc1", content="Original content")
        await core.add_documents("test_tool", [document])
        ...
        assert success
        assert core.documents["doc1"].content == "Updated content"
        assert core.documents["doc1"].metadata["version"] == "2"
    ...
    async def test_delete_documents(self, core):
        ...
        # Add documents
        documents = [
            Document(doc_id="doc1", content="Content 1"),
            Document(doc_id="doc2", content="Content 2")
        ]
        await core.add_documents("test_tool", documents)
        ...
        assert success
        assert "doc1" not in core.documents
        assert "doc2" not in core.documents
    ...
    async def test_get_tool_stats(self, core):
        ...
        assert stats["name"] == "Test RAG Tool"
        assert stats["document_count"] == 0  # No documents added yet
        assert stats["vector_store_type"] == "qdrant"
        assert stats["collection_name"] == "test_store"

    @pytest.mark.asyncio
    async def test_retrieve_invalid_tool(self, core):
        """Test retrieval with invalid tool ID."""
        with pytest.raises(ValueError, match="RAG tool nonexistent not found"):
            await core.retrieve("nonexistent", "test query")

    @pytest.mark.asyncio
    async def test_pre_processors(self, core):
        """Test pre-processor functionality."""
        await core.register_vector_store("test_store", VectorStoreType.QDRANT, {})
        await core.create_rag_tool(
            tool_id="test_tool",
            name="Test Tool",
            ...
            vector_store_id="test_store",
            collection_name="test_store",
            pre_processors=[lambda q: f"processed: {q}"]
        )

        # Add document
        document = Document(doc_id="doc1", content="processed: test content")
        await core.add_documents("test_tool", [document])
    ...
    async def test_chunk_document(self, core):
        ...
        assert len(chunks) > 1
        assert all(chunk.doc_id.startswith("test_doc_chunk_") for chunk in chunks)
        assert all(chunk.metadata["parent_doc_id"] == "test_doc" for chunk in chunks)
    ...
    async def test_cleanup(self, core):
        ...
        # Add some data
        document = Document(doc_id="doc1", content="test content")
        await core.add_documents("test_tool", [document])
