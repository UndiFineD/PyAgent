...
class TestTaskQueueMixin:
    ...
    async def test_submit_task(self):
        ...
        assert agent.task_results[job_id]['status'] == 'queued'
        assert 'submit_time' in agent.task_results[job_id]
    ...
    async def test_task_processing(self):
        ...
        assert status['result'] == f"processed_{job_id}"
    ...
    async def test_queue_full_error(self):
        ...
        with pytest.raises(ValueError, match="Task queue is full"):
            await agent.submit_task({'test': 'data2'})
