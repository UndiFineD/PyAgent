...
class TestDataProcessingMixin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mixin = DataProcessingMixin()
    ...
    def test_convert_account_expires_never(self):
        """Test account expires conversion for never expires."""
        result = self.mixin.convert_account_expires(0)
    ...
    def test_process_change_record_uac(self):
        """Test processing UAC change record."""
        record = {
            'attribute_name': 'userAccountControl',
            'attribute_value': '512'  # NORMAL_ACCOUNT
        }
        result = self.mixin.process_change_record(record)

        self.assertIn('explanation', result)
        self.assertIn('NORMAL_ACCOUNT', result['explanation'])
        ...
        changes = [
            {
                ...
                'attribute_name': 'userAccountControl',
                'attribute_value': '512',
                'last_orig_change_time': '2023-01-01 12:00:00',
                'explanation': 'NORMAL_ACCOUNT'
            }
        ]

        result = self.mixin.format_change_output(changes, 'table')
    ...
    def test_format_change_output_json(self):
        ...
        result = self.mixin.format_change_output(changes, 'json')
