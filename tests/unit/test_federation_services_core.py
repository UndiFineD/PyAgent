...
class TestFederationServicesCore:
    ...
    def setup_method(self):
        ...
        self.core = FederationServicesCore()

    def test_initialization(self):
        """Test core initialization."""
        assert self.core is not None
        assert hasattr(self.core, 'generate_saml_token')
        assert hasattr(self.core, '_generate_office365_token')
        assert hasattr(self.core, '_generate_dropbox_token')
    ...
    async def test_generate_office365_token(self):
        ...
        assert result.audience == "urn:federation:MicrosoftOnline"
    ...
    async def test_generate_dropbox_token(self):
        ...
        assert result is not None
        assert hasattr(result, 'token_id')
        assert "dropbox.com" in result.audience
    ...
    async def test_get_federation_statistics(self):
        """Test federation statistics retrieval."""
        stats = await self.core.get_federation_statistics()
