...
class TestAgentCapability:
    ...
    def test_agent_creation(self):
        ...
        assert TaskType.CODE_GENERATION in agent.skills
    ...
    def test_can_handle_task(self):
        ...
        agent = AgentCapability(
            agent_id="agent-001",
            name="Code Agent",
            skills=[TaskType.CODE_GENERATION]
        )
...
class TestTaskManager:
    ...
    def test_update_task(self, manager):
        ...
        task = create_task("Original", "Original desc", TaskType.CODE_GENERATION)
        task_id = manager.add_task(task)
        ...
        assert updated.priority == PriorityLevel.P0

    def test_remove_task(self, manager):
        """Test removing a task."""
        task = create_task("Test", "Test", TaskType.CODE_GENERATION)
        task_id = manager.add_task(task)
    ...
    def test_assign_task(self, manager):
        ...
        # Create task
        task = create_task("Test Task", "Test", TaskType.CODE_GENERATION)
        task_id = manager.add_task(task)
        ...
        # Assign task
        success = manager.assign_task(task_id, "agent-001")
        assert success is True

        # Check assignment
        assert manager.tasks[task_id].assigned_to == "agent-001"
        assert manager.tasks[task_id].status == TaskStatus.ASSIGNED
        assert agent.current_workload == 1

    def test_assign_incompatible_task(self, manager):
        """Test assigning incompatible task."""
        # Create research task
        task = create_task("Research", "Research task", TaskType.RESEARCH)
        task_id = manager.add_task(task)

        # Create code-only agent
        agent = create_agent_capability(
            "agent-001", "Code Agent",
            skills=[TaskType.CODE_GENERATION]
        )
        manager.register_agent(agent)
    ...
    def test_auto_assign_tasks(self, manager):
        ...
        # Create tasks
        task1 = create_task("Code Task", "Generate code", TaskType.CODE_GENERATION)
        task2 = create_task("Research Task", "Do research", TaskType.RESEARCH)
        ...
        assert ("code-agent", task1.id) in [(a, t) for t, a in assignments]
        assert ("research-agent", task2.id) in [(a, t) for t, a in assignments]
    ...
    def test_complete_task(self, manager):
        ...
        agent = create_agent_capability("agent-001", "Agent", skills=[TaskType.CODE_GENERATION])
        manager.register_agent(agent)
    ...
    def test_get_overdue_tasks(self, manager):
        ...
        # Create normal task
        normal_task = create_task("Normal", "Normal task", TaskType.CODE_GENERATION)
        manager.add_task(normal_task)
    ...
    def test_get_task_queue(self, manager):
        ...
        high_priority = create_task("High", "High priority", TaskType.CODE_GENERATION, PriorityLevel.P0)
        low_priority = create_task("Low", "Low priority", TaskType.CODE_GENERATION, PriorityLevel.P2)
    ...
    def test_agent_workload_info(self, manager):
        ...
        assert "agent-001" in workload
        assert workload["agent-001"]["current_workload"] == 0
        assert workload["agent-001"]["max_concurrent_tasks"] == 3
...
class TestConvenienceFunctions:
    ...
    def test_create_task(self):
        ...
        assert "dep-1" in task.dependencies
