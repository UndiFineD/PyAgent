...
>>>>>>> 6b596bef0 (Refactor: Massive test suite migration and reorganization. Legacy tests verified and moved to tests/unit/phases and tests/unit/features. Deleted tests-old.):tests/unit/logic/agents/analysis/core/test_benchmark_core.py
...
class TestBenchmarkCoreBasics:
    ...
    def test_check_regression_zero_baseline(self) -> None:
        """Test regression check with zero baseline."""
        core = BenchmarkCore()
...
class TestBenchmarkCorePropertyBased:
    ...
    def test_baseline_mean_is_within_range(self, latencies: list[float]) -> None:
        ...
        assert min(latencies) <= baseline <= max(latencies)
...
class TestBenchmarkCoreEdgeCases:
    ...
    def test_regression_with_improvement(self) -> None:
        """Test regression detection with performance improvement."""
        core = BenchmarkCore()
        result = core.check_regression(50.0, 100.0, threshold=0.10)
        assert not result["regression"]
        assert result["delta_percentage"] == -50.0

    def test_regression_boundary_condition(self) -> None:
        """Test regression at exact threshold boundary."""
        core = BenchmarkCore()
...
class TestBenchmarkCoreConsistency:
    ...
    def test_efficiency_deterministic(self) -> None:
        """Test that efficiency scoring is deterministic."""
        core = BenchmarkCore()
    ...
    def test_regression_check_deterministic(self) -> None:
        """Test that regression check is deterministic."""
        core = BenchmarkCore()
