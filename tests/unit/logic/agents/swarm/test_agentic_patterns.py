...
class TestSequentialAgentPattern:
    ...
    def sequential_pattern(self, mock_orchestrator):
        """Create sequential agent pattern."""
        return SequentialAgentPattern(mock_orchestrator)

    @pytest.fixture
    def sample_context(self):
        """Create sample cascade context."""
    ...
    def sample_config(self):
        """Create sample sequential agent config."""
    ...
    async def test_sequential_execution_success(
        ...
        mock_orchestrator.execute_with_pattern.side_effect = [
            {"result": "step1_success", "completed": True},
            {"result": "step2_success", "completed": True}
        ]

        initial_input = {"task": "test_task"}
        result = await sequential_pattern.execute_sequential(
    ...
    async def test_sequential_execution_with_failure_continue(
        ...
        config = SequentialAgentConfig(
            ...
            sub_agents=[
                {"name": "agent_1", "type": "pattern", "pattern": "peer"},
                {"name": "agent_2", "type": "pattern", "pattern": "debate"}
            ]
        )
    ...
    async def test_sequential_execution_with_failure_stop(self, sequential_pattern, sample_context, mock_orchestrator):
        """Test sequential execution with failure and continue_on_failure=False."""
        config = SequentialAgentConfig(
            name="test_stop",
    ...
    def test_prepare_next_input(self, sequential_pattern):
        """Test input preparation for next agent."""
        current_input = {"original": "data"}
        agent_result = {"processed": "result", "final_state": {"key": "value"}}
        agent_config = {"output_key": "step_output"}
...
class TestParallelAgentPattern:
    ...
    def parallel_pattern(self, mock_orchestrator):
        """Create parallel agent pattern."""
        return ParallelAgentPattern(mock_orchestrator)
    ...
    async def test_parallel_execution_success(self, parallel_pattern, sample_context, sample_agent_configs, mock_orchestrator):
        ...
        # Mock successful execution results
        mock_orchestrator.execute_with_pattern.side_effect = [
            {"weather": "sunny", "completed": True},
            {"news": "tech_update", "completed": True}
        ]

        input_data = {"location": "Mountain View", "topic": "technology"}
        result = await parallel_pattern.execute_parallel(
            sample_context, sample_agent_configs, input_data
        )

        # Verify execution
        assert result["pattern"] == "parallel"
        assert result["execution"]["total_agents"] == 2
        assert result["execution"]["successful"] == 2
        assert result["execution"]["failed"] == 0
        assert len(result["results"]) == 2
    ...
    async def test_parallel_execution_with_partial_failure(
        ...
        mock_orchestrator.execute_with_pattern.side_effect = [
            {"weather": "sunny", "completed": True},
            Exception("News API unavailable")
        ]
    ...
    async def test_parallel_execution_all_failure(
        ...
        assert result["execution"]["total_agents"] == 2
        assert result["execution"]["successful"] == 0
        assert result["execution"]["failed"] == 2

        # Verify combined output indicates no successful results
        combined = result["combined_output"]
        assert "error" in combined
        assert "No successful results" in combined["error"]

    def test_combine_parallel_results(self, parallel_pattern):
        """Test combining parallel execution results."""
        results = [
            {
                "agent": "agent_1",
                "success": True,
        ...
        assert combined["agent_1"]["output"] == "result1"
        assert combined["agent_2"]["output"] == "result2"
        assert "agent_3" not in combined

        assert combined["_summary"]["total_agents"] == 3
        assert combined["_summary"]["successful_agents"] == 2
        assert "agent_1" in combined["_summary"]["agent_names"]
        assert "agent_2" in combined["_summary"]["agent_names"]
