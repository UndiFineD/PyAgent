...
class TestOrchestratorWorkPatternMixin:
    ...
    def test_register_work_pattern(self):
        ...
        assert "TestPattern" in mixin._work_patterns
        assert mixin._work_patterns["TestPattern"] == pattern
        assert mixin._default_work_pattern == "TestPattern"

    def test_register_peer_pattern_as_default(self):
        """Test that PEER pattern becomes default."""
        mixin = OrchestratorWorkPatternMixin()

        # Register a regular pattern first
        pattern1 = MagicMock()
        pattern1.name = "OtherPattern"
        ...
        peer_pattern.name = "PEER"
        mixin.register_work_pattern(peer_pattern)
        assert mixin._default_work_pattern == "PEER"

    def test_get_work_pattern(self):
        """Test getting a work pattern."""
        mixin = OrchestratorWorkPatternMixin()
        ...
        retrieved = mixin.get_work_pattern("TestPattern")
    ...
    async def test_execute_with_pattern(self):
        """Test executing with a work pattern."""
        mixin = OrchestratorWorkPatternMixin()

        # Create mock pattern
        pattern = MagicMock()
        pattern.name = "TestPattern"
        pattern.execute = AsyncMock(return_value={"result": "success"})
        mixin.register_work_pattern(pattern)
        ...
        pattern.execute.assert_called_once_with(context)

    @pytest.mark.asyncio
    async def test_execute_with_default_pattern(self):
        """Test executing with default pattern."""
        mixin = OrchestratorWorkPatternMixin()
    ...
    async def test_execute_with_pattern_not_found(self):
        ...
        with pytest.raises(ValueError, match="Work pattern 'NonExistent' not found"):
            await mixin.execute_with_pattern(context, "NonExistent")

    @pytest.mark.asyncio
    async def test_execute_without_default_pattern(self):
        """Test executing without default pattern set."""
        mixin = OrchestratorWorkPatternMixin()

        context = CascadeContext(task_id="test_task")

        with pytest.raises(ValueError, match="No work pattern specified and no default pattern set"):
            await mixin.execute_with_pattern(context)

    def test_validate_work_pattern_setup_valid(self):
        """Test validating a valid work pattern setup."""
        mixin = OrchestratorWorkPatternMixin()

        pattern = MagicMock()
        pattern.name = "TestPattern"
        pattern.validate_agents.return_value = True
        mixin.register_work_pattern(pattern)

        assert mixin.validate_work_pattern_setup("TestPattern") is True

    def test_validate_work_pattern_setup_invalid_agents(self):
        """Test validating a pattern with invalid agents."""
        mixin = OrchestratorWorkPatternMixin()

        pattern = MagicMock()
        pattern.name = "TestPattern"
        pattern.validate_agents.return_value = False
        mixin.register_work_pattern(pattern)
