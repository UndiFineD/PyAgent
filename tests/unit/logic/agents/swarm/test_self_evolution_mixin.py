...
class TestEvolutionHistory:
    ...
    def test_default_values(self):
        ...
        history = EvolutionHistory(original_workflow=original)
...
class TestSelfEvolutionMixin:
    ...
    def mock_orchestrator(self):
        """Create mock orchestrator with mixin."""
    ...
    def successful_result(self):
        ...
        return {
            "completed": True,
            "final_score": 0.9,
            "execution_time": 25.0,  # Fast execution (< 30.0)
            "results": [
                {"completed": True, "score": 0.95},
                {"completed": True, "score": 0.85},
                {"completed": True, "score": 0.90}
            ]
        }
    ...
    def failed_result(self):
        ...
        return {
            ...
            "results": [
                {"completed": True, "score": 0.8},
                {"completed": False, "error": "timeout"},
                {"completed": False, "error": "validation_failed"}
            ]
        }
    ...
    def test_enable_evolution(self, mock_orchestrator):
        """Test enabling/disabling evolution."""
        mock_orchestrator.enable_evolution(False)
    ...
    def test_set_evolution_params(self, mock_orchestrator):
        ...
        mock_orchestrator.set_evolution_params(max_iterations=5, improvement_threshold=0.2)
    ...
    async def test_execute_without_evolution(self, mock_orchestrator, sample_context, successful_result):
        """Test execution when evolution is disabled."""
        mock_orchestrator.enable_evolution(False)
    ...
    async def test_execute_with_evolution_no_improvement_needed(self, mock_orchestrator, sample_context, successful_result):
        ...
        history = mock_orchestrator.get_evolution_history("test_task_123")
    ...
    async def test_execute_with_evolution_and_improvement(self, mock_orchestrator, sample_context, failed_result):
        """Test execution with evolution that improves the workflow."""
        ...
        improved_result = {
            ...
            "execution_time": 60.0,
            "results": [
                {"completed": True, "score": 0.9},
                {"completed": True, "score": 0.8}
            ],
            "evolved": True,
            "evolution_config": {"retry_enabled": True}
        }
    ...
    def test_calculate_metrics_successful(self, mock_orchestrator, successful_result):
        """Test metric calculation for successful execution."""
        metrics = mock_orchestrator._calculate_metrics(successful_result)
    ...
    def test_calculate_metrics_failed(self, mock_orchestrator, failed_result):
        """Test metric calculation for failed execution."""
        metrics = mock_orchestrator._calculate_metrics(failed_result)
    ...
    def test_should_evolve(self, mock_orchestrator):
        """Test evolution decision logic."""
        # Should evolve: low success rate
    ...
    def test_analyze_workflow_issues(self, mock_orchestrator, failed_result):
        ...
        expected_suggestions = {
            "improve_agent_selection", "add_retry_logic", "enhance_prompt_quality",
            "add_validation_steps", "add_error_handling", "improve_context_passing",
            "optimize_parallel_execution", "reduce_agent_count"
        }
        assert set(suggestions) == expected_suggestions

    def test_apply_evolution_suggestions(self, mock_orchestrator, failed_result):
        """Test applying evolution suggestions."""
        suggestions = ["add_retry_logic", "enhance_prompt_quality", "add_validation_steps"]
        evolved = mock_orchestrator._apply_evolution_suggestions(failed_result, suggestions, 0)

        assert evolved["retry_enabled"] is True
        assert evolved["max_retries"] == 2
        assert "add_context_examples" in evolved["prompt_enhancements"]
        assert evolved["validation_enabled"] is True
    ...
    def test_record_evolution_step(self, mock_orchestrator, successful_result):
        """Test recording evolution steps."""
    ...
    def test_extract_lessons(self, mock_orchestrator, successful_result):
        """Test lesson extraction from results."""
        metrics = mock_orchestrator._calculate_metrics(successful_result)
        lessons = mock_orchestrator._extract_lessons(successful_result, metrics)

        # Should extract positive lessons for good performance
        assert "High success rate indicates good agent selection" in lessons
        assert "Strong quality score suggests effective collaboration patterns" in lessons
        assert "Zero errors indicate robust error handling" in lessons
        assert "Fast execution suggests efficient workflow design" in lessons

    def test_get_evolution_insights(self, mock_orchestrator, successful_result):
        """Test getting evolution insights."""
        workflow_id = "test_workflow"
        metrics = mock_orchestrator._calculate_metrics(successful_result)
        ...
        assert insights["total_evolutions"] == 3
        assert insights["best_performance"] is not None
        assert len(insights["lessons_learned"]) > 0
        assert insights["improvement_trend"] in ["insufficient_data", "improving", "declining", "stable"]
