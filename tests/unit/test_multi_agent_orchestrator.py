...
class TestMultiAgentOrchestrator:
    ...
    def test_agent_type_registration(self, temp_dir):
        """Test registering agent type handlers."""
        orchestrator = MultiAgentOrchestratorCore(base_working_dir=temp_dir)
        ...
        assert "test_agent" in orchestrator.agent_handlers
    ...
    def test_agent_creation(self, temp_dir):
        ...
        def dummy_handler(action, name, metadata):
            if action == "create":
                return {"ok": True, "message": f"Created {name}"}
            return {"ok": False, "error": "Unknown action"}

        orchestrator.register_agent_type("test_agent", dummy_handler)

        result = orchestrator.create_agent("test_agent", "test_agent_1")
        assert result["ok"] is True
        assert result["agent_name"] == "test_agent_1"
        assert "test_agent_1" in orchestrator.agent_registry
    ...
    def test_agent_listing(self, temp_dir):
        ...
        # Create some agents
        orchestrator.create_agent("test_agent", "agent1")
        orchestrator.create_agent("test_agent", "agent2")
    ...
    def test_task_dispatch(self, temp_dir):
        ...
        def dummy_handler(action, name, metadata):
            if action == "create":
                ...
                return {"ok": True, "result": "Task completed"}
            return {"ok": False}

        orchestrator.register_agent_type("test_agent", dummy_handler)

        # Create agent
        result = orchestrator.create_agent("test_agent", "test_agent")
        assert result["ok"] is True

        # Dispatch task
        task_result = orchestrator.dispatch_task("test_agent", "Test task")
...
class TestVoiceAgentOrchestrator:
    ...
    def test_voice_session_management(self, temp_dir):
        ...
        assert end_result["session_id"] == result["session_id"]
    ...
    def test_voice_input_processing(self, temp_dir):
        ...
        result = voice_orchestrator.process_voice_input(audio_data)
    ...
    def test_agent_creation_via_voice(self, temp_dir):
        """Test creating agents through voice commands."""
        orchestrator = MultiAgentOrchestratorCore(base_working_dir=temp_dir)
        voice_orchestrator = VoiceAgentOrchestrator(orchestrator)
        ...
        transcription = "create a coding agent"
        result = voice_orchestrator._process_transcription(transcription)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("✓ Agent type registration test passed")
    except Exception as e:
    ...
    try:
        ...
        print("✓ Task dispatch test passed")
    except Exception as e:
