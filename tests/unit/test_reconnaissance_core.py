...
class TestReconnaissanceCore:
    ...
    @pytest.fixture
    def recon_core(self):
        """Create ReconnaissanceCore instance"""
        return ReconnaissanceCore()

    def test_generate_wordlist_basic(self, recon_core):
        """Test basic wordlist generation"""
        patterns = ["{{word}}", "{{word}}{{number}}"]
        payloads = {
            "word": ["api", "dev"],
            "number": ["1", "2"]
        }
        ...
        assert "dev" in wordlist
        assert "api1" in wordlist
        assert "dev2" in wordlist
        assert len(wordlist) == 6  # 2 words + 4 combinations

    def test_generate_wordlist_empty_payloads(self, recon_core):
        """Test wordlist generation with default payloads"""
        patterns = ["{{word}}"]
        payloads = {}
    ...
    async def test_certificate_transparency_source(self, recon_core):
        """Test Certificate Transparency source"""
        ...
        with patch('aiohttp.ClientSession.get') as mock_get:
            ...
            assert results[1].subdomain == "api.example.com"  # Wildcard removed
            assert results[0].source == "crtsh"

    @pytest.mark.asyncio
    async def test_threatcrowd_source(self, recon_core):
        """Test ThreatCrowd source"""
        tc_source = ThreatCrowdSource()
        config = ReconConfig(domain="example.com")
        ...
        with patch('aiohttp.ClientSession.get') as mock_get:
            ...
            assert results[0].source == "threatcrowd"

    @pytest.mark.asyncio
    async def test_enumerate_subdomains_integration(self, recon_core):
        """Test full subdomain enumeration integration"""
        config = ReconConfig(
            domain="example.com",
            wordlist=["www", "api"],
            sources=["crtsh"],
