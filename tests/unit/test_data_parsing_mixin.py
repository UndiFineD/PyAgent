...
class TestDataParsingMixin:
    ...
    def test_html_unescape(self):
        ...
        mixin = DataParsingMixin()
        result = mixin.html_unescape("&lt;test&gt;&amp;&quot;")
        assert result == '<test>&"'
    ...
    def test_find_pattern(self):
        ...
        mixin = DataParsingMixin()
        haystack = "some text with pattern in it"
        result = mixin.find_pattern(haystack, "pattern")
        assert result == "pattern in it"

    def test_find_pattern_not_found(self):
        """Test pattern finding when not found."""
        mixin = DataParsingMixin()
        result = mixin.find_pattern("haystack", "needle")
        assert result is None
