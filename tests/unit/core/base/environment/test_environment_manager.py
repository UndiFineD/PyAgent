...
class TestEnvironmentManager:
    ...
    async def test_register_environment(self, temp_dir):
        ...
        retrieved = await manager.get_environment("test-env", "1.0.0")
    ...
    async def test_unregister_environment(self, temp_dir):
        ...
        config = EnvironmentConfig(name="test-env", version="1.0.0")
        await manager.register_environment(config)
    ...
    async def test_list_environments(self, temp_dir):
        ...
        config1 = EnvironmentConfig(name="env1", version="1.0.0")
        config2 = EnvironmentConfig(name="env2", version="1.0.0")
    ...
    async def test_create_instance_context_manager(self, temp_dir):
        """Test creating an environment instance with context manager."""
        manager = EnvironmentManager(base_dir=temp_dir)
        await manager.initialize()
        ...
        async with manager.create_instance("test-env", "1.0.0") as instance:
            instance_created = instance
            assert instance.environment_name == "test-env@1.0.0"
            assert instance.status == EnvironmentStatus.RUNNING
    ...
    async def test_create_instance_not_found(self, temp_dir):
        """Test creating instance with non-existent environment."""
        manager = EnvironmentManager(base_dir=temp_dir)
        await manager.initialize()

        with pytest.raises(ValueError, match="Environment nonexistent@1.0.0 not found"):
            async with manager.create_instance("nonexistent", "1.0.0"):
                pass
    ...
    async def test_instance_expiration(self, temp_dir):
        """Test instance expiration and cleanup."""
        manager = EnvironmentManager(base_dir=temp_dir)
        await manager.initialize()
    ...
    async def test_environment_variables(self, temp_dir):
        ...
        async with manager.create_instance("test-env", "1.0.0", {"environment_variables": custom_vars}) as instance:
            # Check that environment variables are set
            expected_vars = {"TEST_VAR": "test_value", "CUSTOM_VAR": "custom_value"}
            assert instance.environment_variables == expected_vars
    ...
    async def test_persistence(self, temp_dir):
        ...
        config = EnvironmentConfig(name="persistent-env", version="1.0.0")
        await manager1.register_environment(config)

        async with manager1.create_instance("persistent-env", "1.0.0") as instance:
            instance_id = instance.id
        ...
        retrieved_config = await manager2.get_environment("persistent-env", "1.0.0")
    ...
    async def test_list_instances(self, temp_dir):
        ...
        config = EnvironmentConfig(name="test-env", version="1.0.0", isolation=EnvironmentIsolation.NONE)
        await manager.register_environment(config)

        instances_before = await manager.list_instances()
        initial_count = len(instances_before)

        async with manager.create_instance("test-env", "1.0.0"):
            instances_during = await manager.list_instances()
