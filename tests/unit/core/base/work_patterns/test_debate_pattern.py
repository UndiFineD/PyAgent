...
class MockDebateAgent:
    ...
    def __init__(self, agent_id: str, role: str, incentives: str = "Balanced reasoning"):
...
class TestDebateWorkPattern:
    ...
    def test_initialization(self):
        ...
        assert pattern.description == "Multi-agent debate pattern"
        assert pattern.config.max_rounds == 3
    ...
    def test_validate_agents_valid(self):
        ...
        agents = [
            MockDebateAgent("agent1", "Advocate"),
            MockDebateAgent("agent2", "Critic")
        ]
    ...
    def test_validate_agents_duplicate_roles(self):
        ...
        agents = [
            MockDebateAgent("agent1", "Advocate"),
            MockDebateAgent("agent2", "Advocate")  # Duplicate role
        ]
    ...
    async def test_execute_debate_success(self):
        ...
        advocate = MockDebateAgent("advocate", "Advocate", "Maximize user benefit")
        auditor = MockDebateAgent("auditor", "Auditor", "Minimize risk")
    ...
    async def test_execute_debate_insufficient_agents(self):
        ...
        with pytest.raises(ValueError, match="Debate pattern requires at least 2 agents"):
    ...
    async def test_auto_synthesis(self):
        ...
        debate_history = [{
            "arguments": [
                {
                    "role": "Advocate",
                    "position": {"confidence": 0.8, "decision": "Implement"}
                },
                {
                    "role": "Auditor",
                    "position": {"confidence": 0.9, "decision": "Review carefully"}
                }
            ]
        }]
    ...
    async def test_weighted_vote_synthesis(self):
        ...
        debate_history = [{
            "arguments": [
                {"role": "Advocate", "position": {"decision": "Implement"}},
                {"role": "Auditor", "position": {"decision": "Review"}}
            ]
        }]
        ...
        assert result["votes"]["Auditor"] == 1.5  # Higher weight for auditor
        assert result["votes"]["Advocate"] == 1.2  # Higher weight for advocate