...
class TestPeerWorkPattern:
    ...
    def test_initialization(self):
        ...
        assert pattern.name == "PEER"
        assert "Planning, Executing, Expressing, Reviewing" in pattern.description
    ...
    def test_validate_agents_with_agents(self):
        ...
        executing = MockAgent("executing")
        expressing = MockAgent("expressing")
        reviewing = MockAgent("reviewing")
    ...
    async def test_execute_without_agents(self):
        ...
        with pytest.raises(ValueError, match="PEER pattern requires all four agent types"):
    ...
    async def test_execute_with_mock_agents(self):
        ...
        planning = MockAgent("planning", {"plan": "Test plan"})
        executing = MockAgent("executing", {"execution": "Test execution"})
        expressing = MockAgent("expressing", {"expression": "Test expression"})
        reviewing = MockAgent("reviewing", {"review": "Test review", "score": 0.9})
        ...
        assert result["pattern"] == "PEER"
        assert len(result["results"]) == 1  # Should complete in one attempt
        assert result["final_score"] == 0.9
        assert result["completed"] is True

        # Check that all phases were executed
        round_result = result["results"][0]
        assert "planning" in round_result
        assert "executing" in round_result
        assert "expressing" in round_result
        assert "reviewing" in round_result
    ...
    async def test_execute_with_low_score_retry(self):
        ...
        async def reviewing_response(context):
            ...
            call_count += 1
            if call_count == 1:
                return {"review": "Poor quality", "score": 0.5}
            else:
        ...
        expressing = MockAgent("expressing", {"expression": "Test expression"})
        reviewing = MagicMock()
        reviewing.agent_id = "reviewing"
        reviewing.execute_task = reviewing_response
        ...
        assert result["pattern"] == "PEER"
        assert len(result["results"]) == 2  # Should take two attempts
        assert result["final_score"] == 0.9
        assert result["completed"] is True
