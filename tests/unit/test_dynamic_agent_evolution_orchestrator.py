...
class TestDynamicAgentEvolutionOrchestrator:
    ...
    def test_coverage_calculation(self, temp_dir):
        """Test coverage calculation between tasks and agents."""
        orchestrator = DynamicAgentEvolutionOrchestrator(temp_dir)
        ...
        agent_sheet = AgentSkillSheet(
            name="web_specialist",
            domain="web_development",
            capabilities=["coding", "api_design", "testing"],
    ...
    def test_create_specialized_agent(self, temp_dir):
        ...
        assert agent_sheet.name.startswith("web_development_specialist")
        assert agent_sheet.tier == AgentTier.SPECIALIZED
        assert agent_sheet.domain == "web_development"
        assert "coding" in agent_sheet.capabilities
        assert "documentation" in agent_sheet.capabilities
        assert len(agent_sheet.constraints) > 0

        # Check skill sheet was saved
        assert agent_sheet.name in orchestrator.skill_sheets

        # Check agent definition file was created
        agent_file = orchestrator.specialized_dir / f"{agent_sheet.name}.md"
        assert agent_file.exists()

    def test_create_integrated_agent(self, temp_dir):
        """Test creating an integrated agent from multiple parents."""
        orchestrator = DynamicAgentEvolutionOrchestrator(temp_dir)
        ...
        assert agent_sheet.name.startswith("integrated_")
        assert agent_sheet.tier == AgentTier.INTEGRATED
        assert agent_sheet.domain == "web_development"
        assert "coding" in agent_sheet.capabilities
        assert "api_design" in agent_sheet.capabilities
        assert "testing" in agent_sheet.capabilities
        assert parent1.name in agent_sheet.parent_agents
        assert parent2.name in agent_sheet.parent_agents

        # Check agent definition file was created
        agent_file = orchestrator.integrated_dir / f"{agent_sheet.name}.md"
        assert agent_file.exists()

    def test_select_or_create_agent_new(self, temp_dir):
        """Test selecting/creating agent when no existing agents."""
        orchestrator = DynamicAgentEvolutionOrchestrator(temp_dir)
    ...
    def test_select_or_create_agent_existing(self, temp_dir):
        """Test selecting existing agent with good coverage."""
        orchestrator = DynamicAgentEvolutionOrchestrator(temp_dir)
    ...
    def test_update_agent_metrics(self, temp_dir):
        ...
        updated_sheet = orchestrator.skill_sheets["test_agent"]
        assert updated_sheet.usage_count == 1
    ...
    def test_promote_to_elite(self, temp_dir):
        ...
        agent_file = orchestrator.specialized_dir / f"{sheet.name}.md"
        ...
        assert success
        updated_sheet = orchestrator.skill_sheets["good_agent"]
        assert updated_sheet.tier == AgentTier.ELITE
        assert not updated_sheet.promotion_candidate

        # Check file was moved
        old_file = orchestrator.specialized_dir / f"{sheet.name}.md"
        new_file = orchestrator.elite_dir / f"{sheet.name}.md"
        assert not old_file.exists()
        assert new_file.exists()

    def test_get_agent_pool_stats(self, temp_dir):
        """Test getting agent pool statistics."""
        orchestrator = DynamicAgentEvolutionOrchestrator(temp_dir)

        # Create some test agents
        agents_data = [
            ("specialized_agent", AgentTier.SPECIALIZED, "web", 0.8),
            ("integrated_agent", AgentTier.INTEGRATED, "data", 0.9),
            ("elite_agent", AgentTier.ELITE, "ai", 0.95),
        ]
        ...
        assert stats["by_tier"]["specialized"] == 1
        assert stats["by_tier"]["integrated"] == 1
        assert stats["by_tier"]["elite"] == 1
        assert stats["by_domain"]["web"] == 1
        assert stats["by_domain"]["data"] == 1
        assert stats["by_domain"]["ai"] == 1
        assert abs(stats["avg_success_rate"] - 0.883) < 0.01  # (0.8 + 0.9 + 0.95) / 3
