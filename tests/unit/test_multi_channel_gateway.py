...
class TestChannelMessage:
    ...
    def test_channel_message_creation(self):
        ...
        assert message.channel_type == ChannelType.TELEGRAM
        assert message.channel_id == "chat_123"
...
class TestGatewayPresence:
    ...
    def test_gateway_presence_creation(self):
        """Test creating gateway presence."""
        ...
        assert presence.client_id == "client_123"
...
class TestGatewaySession:
    ...
    def test_gateway_session_creation(self):
        """Test creating a gateway session."""
        session = GatewaySession(
            session_id="session_123",
            agent_id="agent_456",
            channel_type=ChannelType.DISCORD,
            channel_id="channel_789",
            activation_mode=SessionActivationMode.ALWAYS,
            metadata={"workspace": "test"}
        )

        assert session.session_id == "session_123"
        assert session.agent_id == "agent_456"
        assert session.channel_type == ChannelType.DISCORD
        assert session.channel_id == "channel_789"
        assert session.activation_mode == SessionActivationMode.ALWAYS
...
class TestGatewayProtocol:
    ...
    async def test_handle_session_create(self, protocol):
        ...
        assert session.agent_id == "agent_456"
        assert session.channel_type == ChannelType.TELEGRAM
        assert session.channel_id == "chat_789"
        assert session.activation_mode == SessionActivationMode.ALWAYS
        assert session.metadata == {"workspace": "test"}
...
class TestMultiChannelGatewayCore:
    ...
    def test_get_sessions_by_channel(self, gateway):
        ...
        sessions = gateway.get_sessions_by_channel(ChannelType.TELEGRAM, "chat1")
    ...
    async def test_send_channel_message_no_session(self, gateway):
        ...
        message_id = await gateway.send_channel_message("invalid_session", "Hello")
    ...
    async def test_send_channel_message_no_provider(self, gateway):
        ...
        session = GatewaySession(
            agent_id="agent1",
            channel_type=ChannelType.WHATSAPP,  # No provider registered
            channel_id="chat1"
        )
        gateway.protocol.sessions[session.session_id] = session

        message_id = await gateway.send_channel_message(session.session_id, "Hello")
        assert message_id is None
