...
# Adjust import path if needed, but it seems correct based on file listing.
...
class TestSamplingEngine:
    ...
    def test_sample_with_temperature(self):
        ...
        # If temperature is low enough, 4 is very likely
        # With 0.5 temp, exp(5/0.5) = exp(10) which is huge compared to others
        assert 0 <= token_id < 5
        assert 0 < prob <= 1
