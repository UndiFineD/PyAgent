...
class MockNode(FleetBackupMixin):
    ...
    def __init__(self, node_id):
        ...
        self.config = {"resilience": {"shard_parts": 4, "mirror_factor": 2}}
        self.voyager_transport = AsyncMock()
        ...
        self.voyager_discovery.get_active_peers.return_value = [
            {"node_id": "node-b", "host": "127.0.0.1", "port": 5556},
            {"node_id": "node-c", "host": "127.0.0.1", "port": 5557}
        ]
...
async def test_raid10_sharding_and_recovery():
    ...
    """
    # 1. Setup Nodes
    node_a = MockNode("node-a")
    node_b = MockNode("node-b")
    node_c = MockNode("node-c")
    ...
    async def route_message(target_host, target_port, message, timeout=5000):
        ...
        if target_node:
            ...
            if msg_type == "shard_store":
                await target_node._on_shard_stored("node-a", message)
                return {"status": "success"}
            elif msg_type == "shard_request":
                return {"status": "success", "shards": list(target_node._shard_store.values())}
            elif msg_type == "shard_response":
                return {"status": "success"}
        print(f"FAILED TO ROUTE to {target_port}")
        return {"status": "error"}
    ...
    print(f"Node-B stored {len(node_b._shard_store)} shards")
    print(f"Node-C stored {len(node_c._shard_store)} shards")
    ...
    # 4 parts, 2 mirror = 8 shards total.
    # Distributed across Node-B and Node-C.
    assert len(all_shards) == 8
    
    print("Multi-node RAID-10 Resilience Test PASSED")

if __name__ == "__main__":
    asyncio.run(test_raid10_sharding_and_recovery())
