...
class TestArtifactCleanupCore:
    ...
    def cleanup_core(self, temp_dir):
        """Create a cleanup core instance with test configuration."""
        return ArtifactCleanupCore(
    ...
    def test_initialization(self, temp_dir):
        """Test core initialization with default and custom parameters."""
    ...
    def test_get_ttl_for_file(self, cleanup_core):
        """Test TTL determination for different file types."""
        cleanup_core.max_age_overrides = {".log": 1800, ".tmp": 300}

        # Test default TTL
        file_path = Path("test.txt")
        assert cleanup_core._get_ttl_for_file(file_path) == cleanup_core.default_ttl

        # Test override TTL
        log_path = Path("test.log")
        assert cleanup_core._get_ttl_for_file(log_path) == 1800

        tmp_path = Path("test.tmp")
        assert cleanup_core._get_ttl_for_file(tmp_path) == 300

    def test_should_cleanup_file(self, cleanup_core, temp_dir):
        """Test file cleanup decision logic."""
        # Create a test file
    ...
    async def test_cleanup_directory(self, cleanup_core, temp_dir):
        ...
        sub_old_file.write_text("sub old")
    ...
    async def test_force_cleanup_now(self, cleanup_core, temp_dir):
        ...
        old_file = Path(temp_dir) / "old.txt"
        old_file.write_text("old")
        old_time = time.time() - cleanup_core.default_ttl - 1
        os.utime(old_file, (old_time, old_time))
    ...
    def test_get_stats(self, cleanup_core):
        ...
        stats = cleanup_core.get_stats()
        expected_keys = ["running", "cleanup_interval", "default_ttl", "cleanup_dirs", "total_cleaned", "dry_run"]
    ...
    def test_dry_run_mode(self, temp_dir):
        ...
        old_file = Path(temp_dir) / "old.txt"
        old_file.write_text("old")
        old_time = time.time() - 10  # Very old
        os.utime(old_file, (old_time, old_time))
