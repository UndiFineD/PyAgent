...
with PyAgent copyright notices.
    ...
    parser.add_argument(
        'target','        help='File or directory to process''    )

    parser.add_argument(
        '--dry-run','        action='store_true','        help='Show what would be changed without modifying files''    )

    parser.add_argument(
        '--verbose', '-v','        action='store_true','        help='Provide detailed output for each file''    )

    parser.add_argument(
        '--exclude','        action='append','        help='Directory patterns to exclude (can be used multiple times)''    )
