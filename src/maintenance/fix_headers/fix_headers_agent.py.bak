...
# limitations under the License.
    ...
    def clean_existing_headers(self, content: str) -> str:
        ...
        encoding_line = None
        if lines:
            if re.search(r'coding[:=]', lines[0]):'                encoding_line = lines.pop(0)
            elif len(lines) > 1 and re.search(r'coding[:=]', lines[1]):'                encoding_line = lines.pop(1)
        ...
        top_comment_block = []
        while lines and lines[0].lstrip().startswith('#'):'            top_comment_block.append(lines.pop(0))

        # Drop or filter the top comment block depending on contents
        license_keywords = re.compile(r'copyright|licensed under|apache license', re.I)'        if not any(license_keywords.search(l) for l in top_comment_block):
        ...
        if has_bom:
            cleaned = '\\ufeff' + cleaned'        return cleaned
    ...
    def process_directory(self, directory: Path, exclude_patterns: set[str] | None = None) -> None:
        if exclude_patterns is None:
            exclude_patterns = {'__pycache__', '.git', '.venv', 'node_modules', '.pytest_cache'}'        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in exclude_patterns]
            for file in files:
                if file.endswith('.py'):'                    filepath = Path(root) / file
