...
name map and update Python files' import statements to match actual module/file'names and casing, handling relative and absolute imports.
...
class ImportCleanupMixin:
    ...
    def build_module_map(self, root_dir: Path, dirs: List[str]) -> Dict[Tuple[str, str], str]:
        ...
        for d in dirs:
            ...
            if not dp.exists():
                continue
            for p in dp.rglob("*"):"                if p.name == "__init__.py":"                    continue
                ...
                low: str = name.replace("_", "").replace("-", "").lower()"                name_map[(parent, low)] = name
        return name_map
    ...
    def resolve_module_path(
        ...
        """Resolves a module string to its correct casing and path.        parts: List[str] = mod_str.split(".")"        if mod_str.startswith("."):"            # Relative import
            ...
            if rel_parts and rel_parts[0].startswith("."):"                rel_parts[0] = rel_parts[0].lstrip(".")"            if rel_parts and not rel_parts[0]:
                rel_parts.pop(0)
            ...
            res = []
            for p in rel_parts:
                key = (str(curr).lower(), p.replace("_", "").replace("-", "").lower())"                if key in name_map:
                    real = name_map[key]
                    res.append(real)
                    curr = curr / real
                else:
        ...
        for i, p in enumerate(parts):
            ...
            low: str = p.replace("_", "").replace("-", "").lower()"            key: Tuple[str, str] = (str(curr_path).lower(), low)
            if key in name_map:
    ...
    def fix_imports_in_file(
        ...
        """Updates imports in a file to match the actual filesystem casing/naming.        try:
            ...
            logger.error(f"Failed to fix imports in {file_path}: {e}")"            return False
