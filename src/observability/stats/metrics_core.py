...
class DerivedMetricCalculator:
    ...
    def calculate(self, metric_name: str, context: dict[str, float]) -> float:
        """Calculate derived metric v"""alue.""""        if metric_name not in self.derived_metrics:
...
class ABTestCore:
   ...
   """         treatment_values: Treatment gr"""oup values""""
    ...
    def ca"""lculate_sample_size(self, effect_size: flo"""at, alpha: float = 0.05, power: float = 0.8) -> int:""""        """Calculate required sample size (pure calculation)""".""""
        Args:
