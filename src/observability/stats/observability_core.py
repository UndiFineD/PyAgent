...
AggregationType, RetentionPolicy, MetricNamespace, and other domain types.
Use the Metric alias that proxies to
src.core.base.common.telemetry_core.Metric for integration with the core
telemetry plumbing. The module may use rust_core for accelerated processing
when available and exposes structures for alerting, retention, federation,
rollups, streaming, and subscriptions.

WHAT IT DOES:
Provides typed dataclasses and enums representing:
- alerts, thresholds, snapshots, and retention policies
- namespaces, annotations, correlations, and subscriptions
- export destinations and federation configuration
- rollup and streaming configs and agent-captured metrics
Acts as a thin facade tying these models to the existing telemetry core (Metric)
and conditionally enabling rust_core acceleration. It centralizes observability
domain types so other components can share a single schema.

WHAT IT SHOULD DO BETTER:
Add robust validation for timestamps, numeric bounds, and operator semantics.
Prefer stricter typing (for example, use TypedDict/Protocol where appropriate)
and more explicit Optional usage. Expose async-friendly helpers for federation
polling and streaming, add unit tests for serialization/deserialization, and
provide a stable adapter layer for switching between Python and rust
implementations with feature-flagged fallbacks. Provide clear conversion helpers
to/from telemetry_core.Metric and documented examples for exporters such as
Prometheus, CloudWatch, and Datadog.
...
Standardi""
zes""
a metric entry.        self.metrics_history.append(metric)

    def summarize_performance(self) -> dict[str, Any]:
"""
Calculates ""
agg""
regate stats from history.        if not self.metrics_history:
...
"""
Helper to break down""
me""
trics by agent.        by_agent: dict[str, dict[str, float]] = {}
    ...
    def filter_by_time(self, start_iso: str, end_iso: str) -> list[AgentMetri""
c]:        ""
Filters metrics within a time range.        results = []
        for m in self.metrics_history:
...
Core logic for statistics processing, separated from the Agent shell.
    ...
    def detect_anomaly(history: list[Metric], value: float, threshold_std: float = 2.0) -> tuple[bool, float]:
        Dete""
ct if a value is anomalous using standard deviation.        if len(history) < 2:
            return False, 0.0
        ...
        for key, metrics in list(metrics_dict.items()):
            ...
            if policy.max_age_days > 0:
                ...
                def _parse_ts(ts: Any) -> datetime:
                    # Accept numeric Unix timestamps (int/float) or ISO strings; coerce otherwise.
                ...
                metrics_dict[key] = [m for m in metrics if _parse_ts(m.timestamp) > cutoff]
...
"""
A single threshold alert emitted by ThresholdAlertM""
anager.
    metric: str
    value: float
    sever""
