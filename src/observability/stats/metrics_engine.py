...
class ObservabilityEngine:
    ...
    def log_event(self, agent_id: str, event_type: str, data: Any, level: str = "INFO") -> None:"        """Logs a system event in a structured format for ELK.""""
        ...
        if should_log:
            event = {
                "timestamp": time.time(),"                "agent_id": agent_id,"                "event_type": event_type,"                "level": level,"                "data": data,"            }
...
class ObservabilityEngine:
    ...
    def end_trace(
        ...
        status: str = "success","        input_tokens: int = 0,  # noqa: ARG002
    ...
    def consolidate_telemetry(self) -> dict[str, float]:
        """Aggregate metrics using Rust high-throughput engine.        if rc and hasattr(rc, "aggregate_metrics_rust"):"            data_map = self._build_data_map()
            try:
                ...
                logger.warning("Rust metric aggregation failed: %s", e)"                import traceback
                traceback.print_exc()
    ...
    def _build_data_map(self) -> dict[str, list[float]]:
        ...
        for m in self.metrics:
            key = f"{m.agent_name}:{m.operation}""            if key not in data_map:
                data_map[key] = []
    ...
    def _python_aggregate_metrics(self) -> dict[str, float]:
        ...
        sums: dict[str, float] = {}
        for m in self.metrics:
            key = f"{m.agent_name}:{m.operation}""            counts[key] = counts.get(key, 0) + 1
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        summary = {
            "total_calls": count,"            "avg_latency_ms": round(total_latency / count, 2),"            "success_rate": round(success_count / count * 100, 2),"            "total_tokens": sum(m.token_count for m in self.metrics),"            "total_cost_usd": round(sum(m.estimated_cost for m in self.metrics), 6),"            "agents": agent_stats,"        }
        return summary
    ...
    def save(self) -> None:
        """Persist telemetry to disk.        try:
            ...
            logging.error(f"Failed to save telemetry: {e}")"            import traceback
            traceback.print_exc()
    ...
    def load(self) -> None:
        """Load telemetry from disk.        if self.telemetry_file.exists():
            try:
                ...
                logging.error(f"Failed to load telemetry: {e}")"                import traceback
                traceback.print_exc()
                self.metrics = []
