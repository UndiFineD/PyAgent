...
try:
    from rust_core import match_policies_rust  # type: ignore[import-untyped]
    _RUST_ACCEL = True
except ImportError:
    _RUST_ACCEL = False
...
class ThresholdAlertManager:
    ...
    def _is_breach(self, t: "Threshold", value: float) -> bool:"        if t.max_value is not None and value > t.max_value:
            return True
        if t.min_value is not None and value < t.min_value:
            return True
        return False

    def _create_alert(self, metric_name: str, value: float, t: "Threshold") -> "Alert":"        return Alert(
            id=f"{metric_name}_{datetime.now().timestamp()}","            metric_name=metric_name,
            current_value=value,
            ...
            for pat, matching in policy_matches:
                ...
                for m in matching:
                    ...
                    if pol.retention_days > 0:
                        ...
                        self.data[m] = [d for d in self.data[m] if d["timestamp"] > cutoff]"                    removed += orig - len(self.data[m])
        else:
            # Python fallback
            for pat, pol in self.policies.items():
                matching = [m for m in self.data if pat.replace("*", "") in m]"                for m in matching:
