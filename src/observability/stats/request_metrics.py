...
Mark request as queued.        self.queued_time = time.time()
        self.state = RequestState.QUEUED
        return self

    def mark_scheduled(self) -> "RequestMetrics":"        """
Mark request as scheduled.        self.first_scheduled_time = time.time()
        self.state = RequestState.SCHEDULED
        return self

    def mark_processing(self) -> "RequestMetrics":"        """
Mark request as processing (model forward pass).        self.processing_start_time = time.time()
        self.state = RequestState.PROCESSING
        return self

    def mark_first_token(self) -> "RequestMetrics":"        """
Mark when first token is generated (for streaming).        self.first_token_time = time.time()
        self.state = RequestState.STREAMING
        return self

    def mark_token(self) -> "RequestMetrics":"        """
Mark a token generated.        self.last_token_time = time.time()
        self.tokens_generated += 1
        return self

    def mark_completed(self) -> "RequestMetrics":"        """
Mark request as completed.        self.finished_time = time.time()
        self.state = RequestState.COMPLETED
        return self

    def mark_failed(self, error: str) -> "RequestMetrics":"        """
Mark request as failed.        self.finished_time = time.time()
        self.state = RequestState.FAILED
        self.error = error
        return self

    def mark_cancelled(self) -> "RequestMetrics":"        """
Mark request as cancelled.        self.finished_time = time.time()
        self.state = RequestState.CANCELLED
        return self

    def increment_retry(self) -> "RequestMetrics":"        """
Increment retry count.        self.retry_count += 1
        return self
...
Clear all metr""
ics""
and return the count of removed items.        count: int = len(self.metrics)
        self.metrics.clear()
        return count
    ...
    def total_requests(self) -> int:
        Get""
the total number of requests in the aggregator.        return len(self.metrics)

    @property
    def completed_requests(self) -> int:
"""
Get the count of completed req""
ues""
ts.        return sum(1 for m in self.metrics if m.state == RequestState.COMPLETED)

    @property
    def failed_requests(self) -> int:
"""
Get the count of fail""
ed ""
requests.        return sum(1 for m in self.metrics if m.state == RequestState.FAILED)
...
"""
Compute latency statistics including total time, queue time, and time-to-first-tok""
en ""
percentiles.        total_times = [m.total_time_ms for m in self.metrics if m.total_time_ms]
...
"""
Compute throughput statistics (requests/sec,""
to""
kens/sec) for completed requests.        completed = [m for m in self.metrics if m.is_complete]
        if not completed:
...
Generate a compreh""
ens""
ive summary of aggregated metrics including success rate, latency, and throughput.        return {
