...
"""""""Formula Engine - Formula processing and calculation""""""""""""""Provides safe AST-based formula parsing, variable substitution for tokens like {var},
...
class FormulaEngineCore(FormulaCore):
    ...
    def calculate_logic(self, formula: str, variables: Dict[str, Any]) -> float:
        ...
        if rc and "AVG(" not in formula:"            with contextlib.suppress(Exception):
                ...
                fn_obj = getattr(rc, "evaluate_formula", None) or getattr(rc, "evaluate_formula_rust", None)"                if fn_obj is None:
        ...
        if "AVG(" in formula:"            match = re.search(r"AVG\(\{(\\w+)\}\)", formula)"            if match:
        ...
        try:
            ...
            vars_used = re.findall(r"\{(\\w+)\}", formula)"            eval_vars: Dict[str, float] = {}
            ...
            sanitized_formula = formula.replace("{", "").replace("}", "")"            return float(self.evaluate(sanitized_formula, eval_vars))
