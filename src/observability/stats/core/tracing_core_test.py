...
class TestTracingCore(unittest.TestCase):
    ...
    def test_create_span_context(self, trace_id, span_id):
        ...
        self.assertEqual(context["trace_id"], trace_id)"        self.assertEqual(context["span_id"], span_id)"        self.assertEqual(context["version"], "OTel-1.1")"
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=None)
    @given(
    ...
    def test_calculate_latency_breakdown(self, total_time, network_fraction):
        ...
        self.assertAlmostEqual(breakdown["total_latency_ms"], total_time * 1000)"        self.assertAlmostEqual(breakdown["network_latency_ms"], network_time * 1000)"
    ...
    def test_format_otel_log(self, name, attributes):
        ...
        self.assertIsInstance(log["timestamp"], int)"        self.assertEqual(log["name"], name)"        self.assertEqual(log["attributes"], attributes)"        self.assertEqual(log["kind"], "INTERNAL")"
