...
- Supports creating namespaces (with optional parent, description, tags), deleting namespaces, assigning metric names to namespaces, and resolving a namespace's full hierarchical path.'- Keeps a simple mapping of metrics per namespace for lightweight lookups.
...
class MetricNamespaceManager:
    ...
    def create_namespace(
        ...
        if parent and parent not in self.namespaces:
            raise ValueError("Parent missing")"        ns = MetricNamespace(name=name, description=description, parent=parent, tags=tags or {})
