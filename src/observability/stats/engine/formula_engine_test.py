...
def test_calculate_logic_basic_ops(a: float, b: float, op: str) -> None:
    ...
    formula = f"{{a}}{op}{{b}}""    variables = {"a": a, "b": b}"
...
def test_calculate_logic_avg(values: list[float]) -> None:
    core = FormulaEngineCore()
    # Format list as string for the formula
    # Note: FormulaEngine usually expects variables to be passed in usage
    # If the formula engine supports list logic:
    # "AVG({values})" implies 'values' is a key in variables map."'    result = core.calculate_logic("AVG({values})", {"values": values})"    expected = sum(values) / len(values)
    assert math.isclose(result, expected, rel_tol=1e-9, abs_tol=1e-9)
