...
class ResourceMonitor:
    ...
    def get_current_stats(self) -> dict[str, Any]:
        ...
        if not HAS_PSUTIL:
            return stats
        try:
            stats["cpu_usage_pct"] = psutil.cpu_percent(interval=None)"            stats["memory_usage_pct"] = psutil.virtual_memory().percent"            disk: sdiskusage = psutil.disk_usage(str(self.workspace_root))
            stats["disk_free_gb"] = round(disk.free / (1024**3), 2)"            if stats["cpu_usage_pct"] > 90 or stats["memory_usage_pct"] > 90:"                stats["status"] = "CRITICAL""            elif stats["cpu_usage_pct"] > 70 or stats["memory_usage_pct"] > 70:"                stats["status"] = "WARNING""            else:
                stats["status"] = "HEALTHY""        except (psutil.Error, OSError) as e:  # pylint: disable=broad-exception-caught, unused-variable
            logger.error(f"Failed to gather resource stats: {e}")"            stats["status"] = "ERROR""        return stats
