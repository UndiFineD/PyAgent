...
"""
ReportAPI - Programmatic report access

[Brief Summary]
DATE: 2026-02-12
AUTHOR: Keimpe de Jong
USAGE:
Instantiate ReportAPI(reports_dir=Path(...)) and call list_reports(file_pattern), get_report(file_stem, ReportType), or create_report(file_stem, ReportType, content).

WHAT IT DOES:
Provides a tiny programmatic/REST-style interface to list, read, and write simple markdown report files in a reports directory; maps ReportType to filename suffixes and performs basic file I/O.

WHAT IT SHOULD DO BETTER:
Validate and sanitize file_stem input, surface detailed error logging, raise/carry exceptions instead of swallowing IO/encoding errors, support atomic writes/locking, and optionally return richer metadata (paths, modified times, sizes) and use async I/O for large workloads.
