...
class ReleaseNotesGenerator:
    ...
    def generate(self, version: str, entries: list[ChangelogEntry], title: str | None = None) -> ReleaseNote:
        ...
        # Generate summary
        summary = f"Release {version} includes {len(entries)} changes""        if breaking:
        ...
        for cat, descs in by_category.items():
            changelog_lines.append(f"### {cat}")"            for desc in descs:
                changelog_lines.append(f"- {desc}")"            changelog_lines.append("")"
        return ReleaseNote(
            version=version,
            title=title or f"Release {version}","            summary=summary,
