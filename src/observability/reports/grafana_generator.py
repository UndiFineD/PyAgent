...
class GrafanaDashboardGenerator:
    ...
    def generate_fleet_summary(self) -> str:
        """Generates a summary dashboard for the entire fleet.        dashboard = {
            "title": "PyAgent Fleet Summary","            "panels": ["                {
                ...
                {
                    "title": "Fleet Latency","                    "type": "timeseries","                    "targets": [{"expr": "rate(fleet_request_duration_seconds_sum[5m])"}],"                },
            ],
            "schemaVersion": 36,"            "uid": "pyagent-fleet-summary","        }

        output_path = self.output_dir / "fleet_summary.json""        output_path.write_text(json.dumps(dashboard, indent=2))
    ...
    def generate_shard_obs(self, shard_name: str) -> str:
        """Generates a dashboard for a specific swarm shard.        dashboard = {
            ...
            "schemaVersion": 36,"            "uid": f"shard-{shard_name}","        }

        output_path = self.output_dir / f"shard_{shard_name}.json""        output_path.write_text(json.dumps(dashboard, indent=2))
        return str(output_path)