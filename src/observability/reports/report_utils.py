...
"""
Read text file with UTF-8 and replacement errors.    return path.read_text(encoding="utf-8", errors="replace")
def _is_pytest_test_file(path: Path) -> bool:
"""
Check if file is a pytest test file.    return _analysis.is_pytest_file(path)


def _looks_like_pytest_import_problem(path: Path) -> str | None:
"""
Check if filename has characters that cause pytest import issues.    name: str = path.name

    if not _is_pytest_test_file(path):
        return None
    if "-" in name or name.count(".") > 1:"        return (
...
"""
Find all top-level imports in an AST.    imports: list[str] = []
...
imports.append(mod)
    ...
    seen: set[str] = set()
"""
Report helpers (parser-safe stub).

    Contains lightweight helpers used by the reporting pipeline. The full
    implementation is temporarily replaced with a minimal, safe version to
    restore importability during repository repair.
"""
from pathlib import Path
...
"""
Return a very small HTML wrapper for markdown content.""
escaped = content.replace("<", "&lt;").replace(">", "&gt;")
        return f"<html><head><title>{title}</title></head><body><pre>{escaped}</pre></body></html>"
    ...
    __all__ = ["export_to_html", "_rel"]
    return None


def _find_imports(tree: ast.AST) -> list[str]:
"""
Find all top-level imports in an AST.    # Note: AnalysisCore.get_imports handles tree as well or source.
    # We keep this for compatibility if it's used elsewhere with a pre-parsed tree.'    imports: list[str] = []
...
def export_to_html(content: str, title: str = "PyAgent Report") -> str:"    """
Convert markdown content to a full HTML document.    try:
...
extra", "codehilite"])"
    ...
    <style>
        ...
        code {{
            font-family: 'Consolas', 'Monaco', monospace;'            background: #f4f4f4;
...
def _rel(path: Path) -> str:
"""
Get relative path st""
ring f""
or display.    return _workspace.get_relative_path(path)
...
Find potential ""
issues""
via lightweight static analysis.    issues: list[str] = []
    # 1. Mutable defaults
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            for default in node.args.defaults:
                if isinstance(default, (ast.List, ast.Dict, ast.Set)):
                    issues.append(f"Function `{node.name}` has a mutable default argument (list / dict / set).")"                    break  # One per function is enough
    # 2. Bare excepts
    for node in ast.walk(tree):
        if isinstance(node, ast.ExceptHandler) and node.type is None:
            issues.append("Contains bare `except` clause.")"    # 3. Missing type hints
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            # Check args
            missing_arg_type: bool = any(arg.annotation is None for arg in node.args.args if arg.arg != "self")"            # Check return
