...
class ReportUtils:
    """Compatibility wrapper providing the ReportUtils symbol expected by
    package consumers and tests. It exposes commonly used helpers as
    staticmethods to avoid import-time side effects.
    """

    read_text = staticmethod(_read_text)
    looks_like_pytest_import_problem = staticmethod(_looks_like_pytest_import_problem)
    find_imports = staticmethod(_find_imports)
    detect_argparse = staticmethod(_detect_argparse)
    placeholder_test_note = staticmethod(_placeholder_test_note)
    export_to_html = staticmethod(export_to_html)
    rel = staticmethod(_rel)
    find_issues = staticmethod(_find_issues)
