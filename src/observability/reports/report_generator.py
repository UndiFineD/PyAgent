...
class ReportGenerator:
    ...
    def export_jsonl_report(self, items: list[dict[str, Any]], filename: str = "audit_log.jsonl") -> bool:"        """Exports report items to JSONL format (Phase 183)."""""""        output_path: Path = self.output_dir / filename
        ...
        self.logger.info(
            "Exported deduplicated items","            count=len(unique_items),
    ...
    def generate_full_report(self) -> str:
        ...
        lines: list[str] = [
            ...
            "","            "## 📈 Processing Summary","            f"- Files Processed: {processed.get('count', 0)}","'            f"- Files Skipped: {processed.get('skipped', 0)}","'            f"- Errors Encountered: {processed.get('errors', 0)}","'            "","            "## 📂 Module Reports","            (
    ...
    def process_file(self, py_path: Path) -> bool:
        ...
        if existing_sha == current_sha:
            logging.debug(f"Skipping unchanged file: {py_path.name}")"            return False
        logging.info(f"Processing {py_path.name}...")"        return self._process_and_write_reports(py_path, source, stem)
    ...
    def render_errors(self, py_path: Path, source: str, compile_result: CompileResult | str | None) -> str:
        ...
        if error_msg:
            lines.append("- `py_compile` equivalent: FAILED")"            lines.append("```")"            lines.append(error_msg.strip())
            lines.append("```")"        else:
        ...
        known: list[str] = []
        if 'subprocess.run(["git"' in source or "subprocess.run(['git'" in source:"'            known.append("Runs `git` via `subprocess`; will fail if git is not installed or repo has no remote.")"        if 'subprocess.run(["gh"' in source or "subprocess.run(['gh'" in source:"'            known.append("Runs GitHub CLI via `subprocess`; requires `gh` to be authenticated.")"        if "copilot" in source and "subprocess.run" in source:"            known.append("Invokes `copilot` CLI; will be a no-op / fallback if Copilot CLI is not installed.")"
        # Detected by AST
        try:
    ...
    def render_improvements(self, py_path: Path, source: str, tree: ast.AST) -> str:
        ...
        if not ast.get_docstring(cast(ast.Module, tree)):
            suggestions.append("Add a concise module docstring describing purpose / usage.")"        if classes and "__init__" not in source:"            suggestions.append("Consider documenting class construction / expected invariants.")"        if "print(" in source and "logging" not in source:"            suggestions.append("Consider using `logging` instead of `print` for controllable verbosity.")"
    ...
    def _find_issues(self, tree: ast.AST, source: str, py_path: Path) -> list[str]:
        ...
        if self._is_pytest_test_file(py_path) and re.search(r"def\\\\s+test_placeholder\\\\s*\(", source):"            issues.append("Replace placeholder tests with real assertions; target the most important behaviors first.")"        if self._looks_like_pytest_import_problem(py_path):
