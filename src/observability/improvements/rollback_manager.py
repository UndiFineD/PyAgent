...
- Typical import and use:
  ...
  point = mgr.create_rollback_point("improvement-123", {"files": {"a.py": "old content"}})
  # later restore the latest snapshot for that improvement
  state = mgr.rollback("improvement-123")
- Intended for ephemeral in-memory snapshots of agent improvements during multi-step changes.
