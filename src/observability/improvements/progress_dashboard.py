...
class ProgressDashboard:
    ...
    def generate_bmad_strategic_grid(self, root_path: Path) -> str:
        ...
        # Mapping to Grid
        p_prd = "Γ£à" if has_prd else "Γ¥î"
        p_arch = "Γ£à" if has_arch else "Γ¥î"
        p_backlog = "Γ£à" if has_backlog else "Γ¥î"

        d_code = "Γ£à" if has_readme else "ΓÅ│"
        d_git = "Γ£à" if has_git else "Γ¥î"
        d_stories = "ΓÅ│"  # Placeholder for story-level tracking

        q_tests = "Γ£à" if has_tests else "Γ¥î"
        q_results = "Γ£à" if has_results else "ΓÅ│"
        q_health = "Γ¥î" if has_errors else "Γ£à"

        grid = [
            "## ≡ƒù║∩╕Å Strategic Development Grid (BMAD Pattern)",
