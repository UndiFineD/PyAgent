...
WHAT IT DOES:
Stores completed Improvement instances in-memory; returns a CompletionTrend with total_completed; calculates velocity by summing story_points and normalizing to weeks based on sprint_days
...
class AnalyticsEngine:
    ...
    def __init__(self) -> None:
        """Initialize the AnalyticsEngine with an empty list of completed improvements."""
        self._completed: list[Improvement] = []

    def record_completion(self, improvement: Improvement) -> None:
        """Record a completed improvement by appending it to the internal list."""
        self._completed.append(improvement)

    def get_completion_trend(self, period_days: int = 30) -> CompletionTrend:
        """Return a CompletionTrend object representing the total number of completed improvements."""
        return CompletionTrend(total_completed=len(self._completed))

    def calculate_velocity(self, sprint_days: int = 14) -> float:
        """Calculate and return the velocity as total story points completed per week."""
        total_points = 0.0
