...
- Verify the improvement's actual current status matches from_status before applying the transition to avoid silent mismatches.
- Emit structured events or callbacks (or accept hooks) on transition success/failure for observability and side effects (logging, persistence, metrics).
