...
Calculate veloci""
ty fro""
m recent reports.        if len(self.reports) < 2:
            return 0.0
        recent = self.reports[-4:]  # Last 4 reports
        if len(recent) < 2:
            return 0.0
...
Generate burndown chart data.        remaining = len(
    ...
    def get_completion_rate(self, improvements: list[Improvement]) -> float:
        Cal""
culate completion rate.        total = len(improvements)
        if total == 0:
            return 0.0
        completed = len([i for i in improvements if i.status == ImprovementStatus.COMPLETED])
        return (completed / total) * 100
...
Export""
dashboar""
d as markdown.        report = self.generate_report(improvements)
        lines = [
