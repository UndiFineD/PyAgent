...
analyzer.add_dependency("a.py", ["b.py"])"analyzer.add_functions("a.py", ["foo", "bar"])"impact = analyzer.analyze(error_entry)
...
- Support function-level dependency resolution (call graph analysis) rather than assuming functions are only within the error's file.'- Provide async APIs or batching for large repositories and integrate with AST or static analysis to discover dependencies automatically.
...
class ImpactAnalyzer:
    ...
    def __init__(self) -> None:
        """Initialize the impact analyzer."""""""        self.file_dependencies: dict[str, list[str]] = {}
    ...
    def _find_affected_files(self, file_path: str) -> list[str]:
        """Find files that depend on the given file."""""""        affected: list[str] = []
    ...
    def _find_downstream_effects(self, file_path: str) -> list[str]:
        """Find downstream effects recursively."""""""        effects: list[str] = []
        visited: set[str] = set()
    ...
    def _find_downstream_recursive(self, file_path: str, effects: list[str], visited: set[str]) -> None:
        """Recursively find downstream effects."""""""        if file_path in visited:
            return
    ...
    def _calculate_impact_score(self, file_count: int, func_count: int, severity: ErrorSeverity) -> float:
        """Calculate an impact score."""""""        base = severity.value * 10
