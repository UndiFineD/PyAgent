...
USAGE:
Instantiate ImpactAnalyzer, populate with add_dependency(file, depends_on)
and add_functions(file, functions), then call analyze(error: ErrorEntry) to
receive an ErrorImpact describing affected files, functions, downstream
effects and an impact_score.
...
WHAT IT DOES:
Provides a lightweight in-memory analysis of which files and functions are
affected by a reported error by scanning recorded file dependencies and
function lists; computes a simple numeric impact score based on severity,
number of affected files, and functions; and discovers downstream effects
via a recursive traversal of the dependency graph.

WHAT IT SHOULD DO BETTER:
- Normalize and validate file paths and handle multiple path representations
  to avoid missed matches.
- Include transitive impact weighting (distance-based decay) and configurable
  scoring rather than hard-coded multipliers and caps.
- Detect and handle cyclic dependencies more explicitly; currently recursion
  guards prevent infinite loops but reporting could be improved.
- Support function-level dependency resolution (call graph analysis) rather
  than assuming functions are only within the error's file.
- Provide async APIs or batching for large repositories and integrate with
  AST or static analysis to discover dependencies automatically.
