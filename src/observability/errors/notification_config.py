...
- Import the dataclass and instantiate with desired channel, endpoint and optional settings:
  ...
  cfg = NotificationConfig(channel=NotificationChannel.WEBHOOK, endpoint="https://hooks.example.com/abc", min_severity=ErrorSeverity.MEDIUM, enabled=True)"- Pass the NotificationConfig instance to whatever error-dispatching component or notifier the agent uses.
