...
and get_fixed_errors(base_branch, feature_branch).

WHAT IT DOES:
Provides a lightweight in-memory facility to record sets of error IDs per branch 
and compute differences: errors only in A, only in B, and common errors. 
Returns a BranchComparison dataclass-like object with those lists.

WHAT IT SHOULD DO BETTER:
- Validate inputs and raise clear exceptions for unknown branches 
instead of silently treating them as empty sets.
- Preserve deterministic ordering (e.g., sort returned lists) 
and maintain original error metadata rather than just IDs.
