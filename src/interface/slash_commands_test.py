...
"""
Test parsing commands with text between them.        from src.interface.slash_commands import parse_commands

        result = parse_commands("Hello /datetime and /stats please")"        assert len(result) == 2
        assert result[0].command == "datetime""        assert result[1].command == "stats""
...
"""
Test that command positions are recorded.        from src.interface.slash_commands import parse_commands

        prompt = "Hello /datetime world""        result = parse_commands(prompt)
...
Test creating OK result.        from src.interface.slash_commands import CommandResult
        ...
        assert result.output == "Hello""        assert result.data == {"key": "value"}"        assert result.error is None
...
"""
Test arg_string property.        from src.interface.slash_commands import CommandContext

        ctx = CommandContext(command="test", args=["hello", "world"])"        assert ctx.arg_string == "hello world""
    def test_first_arg(self):
"""
Test first_arg property.        from src.interface.slash_commands import CommandContext

        ctx = CommandContext(command="test", args=["first", "second"])"        assert ctx.first_arg == "first""
...
Test registering a command.        from src.interface.slash_commands import CommandRegistry, CommandContext, CommandResult
        ...
        assert defn.name == "test""        assert defn.description == "Test command""
...
"""
Test @command decorator.        from src.interface.slash_commands import CommandRegistry, CommandContext, CommandResult

        registry = CommandRegistry()

        @registry.command("greet", description="Greet someone")"        def cmd_greet(ctx: CommandContext) -> CommandResult:
            return CommandResult.ok(f"Hello, {ctx.first_arg or 'world'}!")
        defn = registry.get("greet")"        assert defn is not None
        assert defn.description == "Greet someone"
    def test_list_commands(self):
"""
Test listing commands.        from src.interface.slash_commands import CommandRegistry, CommandContext, CommandResult

        registry = CommandRegistry()

        def handler(ctx: CommandContext) -> CommandResult:
            return CommandResult.ok("test")
        registry.register("visible", handler)"        registry.register("hidden", handler, hidden=True)"
...
Test executing /datetime command.        from src.interface.slash_commands import SlashCommands
        ...
        assert "UTC" in result.output"        assert "utc" in result.data"        assert "timestamp" in result.data
...
"""
Test executing /time command.        from src.interface.slash_commands import SlashCommands
...
Test executing /version command.        from src.interface.slash_commands import SlashCommands
        ...
        assert result.success is True
        assert "Python" in result.output"        assert "python_version" in result.data"
...
Test executing /uptime command.        from src.interface.slash_commands import SlashCommands
        ...
        assert result.success is True
        assert "Uptime" in result.output"        assert "uptime_seconds" in result.data"
...
Test executing /tokens command.        from src.interface.slash_commands import SlashCommands
        ...
        assert result.success is True
        assert "tokens" in result.output.lower()"        assert result.data["words"] == 2"
...
"""
Test /env with non-existent variable.        from src.interface.slash_commands import SlashCommands
...
"""
Test executing /help command.        from src.interface.slash_commands import SlashCommands
...
"""
Test executing /help for specific command.        from src.interface.slash_commands import SlashCommands
...
Test executing /random command.        from src.interface.slash_commands import SlashCommands
        ...
        assert result.success is True
        assert result.data["max"] == 10"        assert 1 <= result.data["value"] <= 10"
    def test_execute_cwd(self):
"""
Test executing /cwd command.        from src.interface.slash_commands import SlashCommands
...
"""
Test executing /python command.        from src.interface.slash_commands import SlashCommands
        import sys
...
Test processing prompt with multiple commands.        from src.interface.slash_commands import SlashCommands
        ...
        assert "UTC" in result.processed"        assert "Python" in result.processed"
    def test_process_inline_replacement(self):
"""
Test that commands are replaced inline.        from src.interface.slash_commands import SlashCommands

        slash = SlashCommands()
        result = slash.process("Hello /version and more")
        # Command should be replaced with output
        assert "/version" not in result.processed"        assert "Python" in result.processed"        assert "Hello" in result.processed"        # Note: text after command until next / or newline becomes args

    def test_process_remove_commands(self):
"""
Test removing commands from prompt.        from src.interface.slash_commands import SlashCommands

        slash = SlashCommands()
        result = slash.process("Hello /datetime world", remove_commands=True)
        # Command text should be removed
        assert "/datetime" not in result.processed"        # Results should still be captured
        assert result.has_commands is True
...
"""
Test accessing command data from result.        from src.interface.slash_commands import SlashCommands

        slash = SlashCommands()
        result = slash.process("/version")
        data = result.command_data
        assert "version" in data"        assert "python_version" in data["version"]"
    def test_process_command_outputs(self):
"""
Test accessing command outputs from result.        from src.interface.slash_commands import SlashCommands
...
"""
Test register_command function.        from src.interface.slash_commands import (
            register_command,
            execute_command,
            CommandContext,
            CommandResult,
...
Test /stats command.        from src.interface.slash_commands import SlashCommands
        ...
        assert result.success is True
        assert "CPU" in result.output"        assert "RAM" in result.output"        assert "cpu_percent" in result.data
...
Test /memory command.        from src.interface.slash_commands import SlashCommands
        ...
        assert result.success is True
        assert "System" in result.output"        assert "Process" in result.output"        assert "system_used_mb" in result.data
...
"""
Test processing empty prompt.        from src.interface.slash_commands import SlashCommands
...
"""
Test prompt that is just a command.        from src.interface.slash_commands import SlashCommands
...
"""
Test commands at start of lines.        from src.interface.slash_commands import SlashCommands
    ...
    def test_get_help_method(self):
        ""
Test get_help method.        from src.interface.slash_commands import SlashCommands
        ...
        help_dt = slash.get_help("datetime")"        assert "datetime" in help_dt"
        # Unknown command
        help_unknown = slash.get_help("unknownxyz")"        assert "Unknown" in help_unknown"