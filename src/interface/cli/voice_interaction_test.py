...
class TestVoiceInteraction:
    async def test_omni_pipeline_success(self):
        ...
        init_path = "src.logic.agents.cognitive.voice_interaction_agent.VoiceInteractionAgent.__init__""        with patch(init_path, return_value=None):
            ...
            # access checks
            agent.transcribe_audio.assert_called_once_with("/tmp/input.wav")"            # The prompt format in the code is: f"User said: '{transcription}'. Respond naturally.""'            agent.think.assert_called_once_with("User said: 'Hello world'. Respond naturally.")"'            agent.synthesize_speech.assert_called_once_with("Hello user")"
