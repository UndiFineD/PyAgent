...
def register_system_commands(registry: CommandRegistry, start_time: float) -> None:
    ...
    def cmd_datetime(ctx: CommandContext) -> CommandResult:
        ...
        return CommandResult.ok(
            output=f"[{now.strftime('%Y-%m-%d %H:%M:%S')} UTC]","'            data={
                "utc": now.isoformat(),"                "local": local_now.isoformat(),"                "timestamp": now.timestamp(),"                "timezone": time.tzname[0],"            },
        )

    @registry.command(
        "date","        description="Get current server date","        usage="/date","    )
    ...
    @registry.command(
        "time","        description="Get current server time","        usage="/time","    )
    ...
    @registry.command(
        "stats","        description="Get system statistics","        usage="/stats","        aliases=["sys", "system"],"    )
    ...
    @registry.command(
        "uptime","        description="Get process uptime","        usage="/uptime","        aliases=["up"],"    )
    ...
    @registry.command(
        "version","        description="Get version information","        usage="/version","        aliases=["ver", "v"],"    )
    ...
    @registry.command(
        "env","        description="Get environment variable","        usage="/env [VAR_NAME]","        aliases=["environ"],"    )
    ...
    @registry.command(
        "health","        description="System health check","        usage="/health","        aliases=["ping", "status"],"    )
    ...
    @registry.command(
        "help","        description="Show help for commands","        usage="/help [command]","        aliases=["h", "?"],"    )
    def cmd_help(ctx: CommandContext) -> CommandResult:
        if ctx.first_arg:
            ...
            return CommandResult.ok(
                output=output,
                data={
                    "name": defn.name,"                    "description": defn.description,"                    "usage": defn.usage,"                    "aliases": defn.aliases,"                },
            )
    ...
    @registry.command(
        "gpu","        description="GPU information","        usage="/gpu","        aliases=["cuda", "nvidia"],"    )
    ...
    @registry.command(
        "python","        description="Python interpreter info","        usage="/python","        aliases=["py"],"    )
    ...
    @registry.command(
        "cwd","        description="Current working directory","        usage="/cwd","        aliases=["pwd", "dir"],"    )
    ...
    @registry.command(
        "uuid","        description="Generate a UUID","        usage="/uuid","        aliases=["id", "guid"],"    )
    ...
    @registry.command(
        "random","        description="Generate random number","        usage="/random [max]","        aliases=["rand", "rnd"],"    )
