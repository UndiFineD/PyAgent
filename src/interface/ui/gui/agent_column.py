...
class AgentColumn:
    ...
    def setup_ui(self) -> None:
        ...
        # Actions in Header
        close_btn = ttk.Button(header_frame, text="✕", width=3, command=lambda: self.remove_callback(self.frame, self.agent_name))
        close_btn.pack(side=tk.RIGHT)
        
        dup_btn = ttk.Button(header_frame, text="📑", width=3, command=lambda: self.callbacks.get("duplicate")(self.get_data()))
        ...
        self.run_btn = ttk.Button(action_bar, text="▶️", width=5, command=lambda: self.execute_callback(self.agent_name))
        self.run_btn.pack(side=tk.RIGHT, padx=1)
        
        self.stop_btn = ttk.Button(action_bar, text="⏹️", width=3, state=tk.DISABLED, command=lambda: self.stop_callback(self.agent_name))
