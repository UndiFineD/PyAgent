...
class BmadManager:
    ...
    def setup_ui(self) -> None:
        ...
        ttk.Radiobutton(scope_opts_frame, text="Project", variable=self.target_mode, value="project").pack(
            side=tk.LEFT, padx=5
        )
        ttk.Radiobutton(scope_opts_frame, text="Git Changes", variable=self.target_mode, value="git").pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Button(
            btn_frame,
            text="≡ƒÜÇ DEPLOY BULK AGENTS",
            style="Accent.TButton",
            command=self.deploy_bulk,
        ).pack(fill=tk.X, pady=2)
        ttk.Button(btn_frame, text="≡ƒöä START BMAD WORKFLOW", command=self.start_workflow_action).pack(fill=tk.X, pady=2)
        ttk.Button(btn_frame, text="ΓÜí Workflow-Init", command=self.workflow_init).pack(fill=tk.X, pady=2)
