...
class DialogManager:
    ...
    def show_custom_agent_dialog(self, callback: Callable[[str], Any]) -> None:
        ...
        ttk.Label(dialog, text="Agent Name:").pack(pady=10)
        name_var = tk.StringVar(value="Experimental")
    ...
    def show_memory_dialog(self, agent_name: str, history: List[Dict[str, Any]], save_callback: Callable[[List[Dict[str, Any]]], Any]) -> None:
        ...
        ttk.Label(dialog, text=f"🧠 Context Memory: {agent_name}", font=("Segoe UI", 12, "bold")).pack(pady=10)
        ...
        def redraw() -> None:
            ...
            for i, msg in enumerate(temp_history):
                ...
                txt.insert("1.0", msg["content"])
                txt.pack(fill=tk.X, padx=5, pady=2)
