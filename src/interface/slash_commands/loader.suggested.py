...
def discover_command_modules() -> list[str]:
    ...
    for file in commands_dir.iterdir():
        if file.is_file() and file.suffix == ".py" and not file.name.startswith("_"):"            modules.append(file.stem)
...
def load_module(module_name: str) -> bool:
    ...
    try:
        ...
        print(f"Warning: Failed to load command module '{module_name}': {e}")"'        return False


def unload_module(module_name: str) -> bool:
        Unload a command module (removes from loaded set).

    Note: This doesn't actually unload from sys.modules,'    but prevents it from being reloaded.
...
def load_commands(registry: "CommandRegistry | None" = None) -> int:"        Load all command modules from the commands directory.
