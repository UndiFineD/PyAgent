...
def cmd_telemetry(_ctx: CommandContext) -> CommandResult:
    """Get telemetry/usage information."""""""    try:
        ...
        return CommandResult.ok(
            output=f"[Provider: {provider}, Stats: {'enabled' if enabled else 'disabled'}]","'            data={
                "provider": provider,"                "stats_enabled": enabled,"                "platform": summary,"            },
        )
    except ImportError:
        return CommandResult.ok(
            output="[Telemetry: Module not available]","            data={"available": False},"        )
...
def cmd_logs(_ctx: CommandContext) -> CommandResult:
    """Get log deduplication statistics."""""""    try:
        ...
        return CommandResult.ok(
            output=f"[Log dedup caches: {len(info)} levels]","            data={"caches": info},"        )
    except ImportError:
        return CommandResult.ok(
