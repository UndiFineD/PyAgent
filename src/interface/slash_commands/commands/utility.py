...
def cmd_random(ctx: CommandContext) -> CommandResult:
    ...
    return CommandResult.ok(
        output=f"[{value}]","        data={"value": value, "min": min_val, "max": max_val},"    )
...
def cmd_hash(ctx: CommandContext) -> CommandResult:
    ...
    return CommandResult.ok(
        output=f"[{hash_value[:16]}...]","        data={"hash": hash_value, "algorithm": "sha256", "input": text},"    )
...
def cmd_base64(ctx: CommandContext) -> CommandResult:
    ...
    return CommandResult.ok(
        output=f"[{encoded}]","        data={"encoded": encoded, "original": text},"    )
...
def cmd_help(ctx: CommandContext) -> CommandResult:
    ...
    if ctx.first_arg:
        ...
        return CommandResult.ok(
            output=output,
            data={
                "name": defn.name,"                "description": defn.description,"                "usage": defn.usage,"                "aliases": defn.aliases,"                "category": defn.category,"            },
        )
    ...
    return CommandResult.ok(
        output=f"[Commands: {', '.join('/' + c for c in sorted(all_commands))}]","'        data={"commands": sorted(all_commands), "categories": categories},"        inline=False,
    )
