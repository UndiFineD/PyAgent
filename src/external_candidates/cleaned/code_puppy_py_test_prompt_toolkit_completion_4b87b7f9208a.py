# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\test_prompt_toolkit_completion_4b87b7f9208a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_prompt_toolkit_completion.py
...
def test_completion_directory_listing(tmp_path):
    ...
    try:
        ...
        filenames = {c.display[0][1] if hasattr(c.display, "__getitem__") else str(c.display) for c in completions}
...
def test_set_completer_on_set_trigger(monkeypatch):
    ...
    completion_metas = sorted([c.display_meta for c in completions])  # Corrected display_meta access
...
def test_set_completer_excludes_model_key(monkeypatch):
    ...
    assert completions == [], "SetCompleter should not complete for 'model' key directly"
    ...
    assert completions == [], "SetCompleter should not complete for 'model' key even partially"
...
def test_set_completer_excludes_puppy_token(monkeypatch):
    ...
    assert completions == [], "SetCompleter should not complete for 'puppy_token' key directly"
    ...
    assert completions == [], "SetCompleter should not complete for 'puppy_token' key even partially"
...
def test_cd_completer_initial_trigger(setup_cd_test_dirs, monkeypatch):
    ...
    displays = sorted(
        [
            ("".join(item[1] for item in c.display) if isinstance(c.display, list) else str(c.display))
    ...
    assert displays == sorted(["another_dir/", "dir1/", "dir2_long_name/", "mock_home/"])
...
def test_cd_completer_home_directory_expansion(setup_cd_test_dirs, monkeypatch):
    ...
    monkeypatch.setattr(os.path, "expanduser", lambda p: p.replace("~", str(mock_home_path)))
...
def test_cd_completer_non_existent_base(setup_cd_test_dirs, monkeypatch):
    ...
    doc = Document(text="/cd non_existent_dir/", cursor_position=len("/cd non_existent_dir/"))
...
async def test_get_input_with_combined_completion_defaults(
    ...
    assert mock_prompt_session_cls.call_args[1]["completer"] == mock_merge_completers.return_value
    ...
    assert mock_session_instance.prompt_async.call_args[0][0] == FormattedText([(None, ">>> ")])
...
async def test_get_input_with_combined_completion_with_history(mock_safe_file_history, mock_prompt_session_cls):
    mock_session_instance = MagicMock()
...
async def test_get_input_with_combined_completion_custom_prompt(
    ...
    assert mock_session_instance.prompt_async.call_args[0][0] == FormattedText([(None, custom_prompt_str)])
...
async def test_get_input_key_binding_alt_m(mock_prompt_session_cls):
    ...
    for binding in bindings.bindings:
        if len(binding.keys) == 2 and binding.keys[0] == Keys.Escape and binding.keys[1] == "m":
