# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_owl.py\owl.py\camel.py\terminators.py\response_terminator_4c340511fb62.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\terminators\response_terminator.py
...
class ResponseWordsTerminator(ResponseTerminator):
    ...
    def _validate(self):
        ...
        for word in self.words_dict:
            ...
            if threshold <= 0:
                raise ValueError(f"Threshold for word `{word}` should be larger than 0, got `{threshold}`")
