# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\agents.py\test_base_agent_configuration_2d9d0782453f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\agents\test_base_agent_configuration.py
...
class TestBaseAgentConfiguration:
    ...
    def test_load_model_with_fallback_success(self, agent):
        ...
        with patch("code_puppy.model_factory.ModelFactory.get_model", return_value=test_model):
            model, model_name = agent._load_model_with_fallback("gpt-4", models_config, "test_group")
    ...
    def test_load_model_with_fallback_success_different_model(self, agent):
        ...
        with (
            ...
            model, model_name = agent._load_model_with_fallback("nonexistent", models_config, "test_group")
    ...
    def test_load_model_with_fallback_empty_config(self, agent):
        ...
        with patch(
            ...
            with pytest.raises(ValueError):
                agent._load_model_with_fallback("nonexistent", models_config, "test_group")
    ...
    def test_load_mcp_servers_with_config(self, agent):
        ...
        with (
            patch("code_puppy.config.get_value", return_value="false"),
            patch("code_puppy.config.load_mcp_server_configs", return_value=test_configs),
            patch("code_puppy.mcp_.get_mcp_manager", return_value=mock_manager),
        ):
            result = agent.load_mcp_servers()
