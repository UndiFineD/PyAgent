# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentkit.py\backend.py\app.py\tests.py\tools.py\test_sql_tool_f8e67902ac84.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\tests\tools\test_sql_tool.py
...
def mock_llm_call(llm: BaseLanguageModel):  # pylint: disable=redefined-outer-name
    with patch(
        "app.services.chat_agent.tools.library.sql_tool.sql_tool.get_llm",
        return_value=llm,
    ):
        yield
...
def patch_check_init():  # pylint: disable=redefined-outer-name
    ...
    with patch(
        "app.services.chat_agent.tools.library.sql_tool.sql_tool.SQLTool.check_init",
        new=dummy,
    ):
        yield
...
def sql_tool_db():
    ...
    with patch(
        "app.services.chat_agent.tools.library.sql_tool.sql_tool.sql_tool_db",
        new=fake_db,
    ):
        yield
...
async def test_sql_tool_run(sql_tool: SQLTool, tool_input: str):
    with (
        ...
        patch(
            "app.services.chat_agent.tools.library.sql_tool.sql_tool.SQLTool._parse_query",
            return_value="query",
        ),
        patch(
            "app.services.chat_agent.tools.library.sql_tool.sql_tool.is_sql_query_safe",
            return_value=True,
        ),
    ):
