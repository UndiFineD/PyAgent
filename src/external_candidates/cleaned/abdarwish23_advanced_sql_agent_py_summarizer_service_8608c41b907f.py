# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\abdarwish23_advanced_sql_agent.py\app.py\services.py\summarizer_service_8608c41b907f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\summarizer_service.py
...
def summarizer_node(state: AgentState) -> AgentState:
    ...
    if not state["analyzed_query"].is_query_relevant:
        state["summary"] = (
            f"I'm sorry, but your query '{state['user_query']}' is not relevant to the available database information. {state['analyzed_query'].explanation}"
        )
