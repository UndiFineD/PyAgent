# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2023.py\numberphile.py\prime_race_330e2c5dafc7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\numberphile\prime_race.py
...
class PrimeRace(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            frame.animate.set_height(8, about_point=blocks.get_right() + 2 * LEFT),
            run_time=3,
        )
    ...
    def construct(self):
        ...
        def set_x_squish(x1, x2, **kwargs):
            ...
            self.play(
                ...
                UpdateFromAlphaFunc(
                    prime_label,
                    lambda m, a: m.set_value(primes[min(int(interpolate(x1, x2, a)), len(primes) - 1)]),
                ),
                **kwargs,
            )
        ...
        self.play(
            VGroup(axes, graph).animate.set_width(len(primes) / 10, about_point=axes.c2p(len(primes), 0), stretch=True),
            FadeOut(rect),
            run_time=6,
        )
