# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\triangle_of_power.py\intro_3e85b770ce34.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\triangle_of_power\intro.py
...
class TrigAnimation(Animation):
    ...
    def __init__(self, **kwargs):
        ...
        self.trig_lines = [
            Line(ORIGIN, RIGHT, color=color) for color in (self.sin_color, self.cos_color, self.tan_color)
        ]
    ...
    def interpolate_mobject(self, alpha):
        ...
        points = [
            ...
            (np.sign(np.cos(theta)) * np.sqrt(np.tan(theta) ** 2 - np.sin(theta) ** 2) + np.cos(theta)) * RIGHT
            + self.center,
        ]
...
class Notation(Scene):
    ...
    def shift_to_good_and_back(self):
        ...
        new_sin, new_cos, new_tan = [
            VMobject().set_points_as_corners(corners).replace(trig_part.split()[0])
        ...
        x1, x2, x3 = [trig_part.split()[1] for trig_part in self.trigs1.split()]
    ...
    def shift_to_visuals(self):
        ...
        sin, cos, tan = [trig_part.split()[0] for trig_part in new_trig.split()]
    ...
    def get_harmonic_sum_lines(self):
        ...
        for n in range(1, 8):
            ...
            little_line = Line(
                ...
                big_line.number_to_point(1.0 / n),
                color=RED,
            )
...
class TwoThreeEightExample(Scene):
    def construct(self):
        ...
        self.play(Transform(rogue_two, two2), Write(dot1), run_time=0.5)

        self.add(two2)

        self.play(Transform(rogue_two, two3), Write(dot2), run_time=0.5)

        self.add(two3)

        self.remove(rogue_two)

        self.play(Write(eq), Write(eight), GrowFromCenter(brace), Write(three), run_time=1)
        ...
        rad_three, rad1, rad2, rad_eight, rad_eq, rad_two = OldTex("\\sqrt[3]{8} = 2").split()
        ...
        l, o, g, log_two, p1, log_eight, p2, log_eq, log_three = OldTex("\\log_2(8) = 3").split()
        ...
        self.play(ApplyMethod(VMobject(l, o, g).set_color, RED, rate_func=there_and_back, run_time=2))
...
class WhatTheHell(Scene):
    def construct(self):
        ...
        q_marks = VMobject(*[OldTex("?!").next_to(arrow, RIGHT) for arrow in (arrow1, arrow2)])
        ...
        phrases = [
            OldTexText(["Communicate with", words]).next_to(mob, LEFT, buff=1)
            for words, mob in [
                ("position", exp),
                ("a new symbol", rad),
                ("a word", log),
        ...
        self.play(ShowCreationPerSubmobject(arrow1), ShowCreationPerSubmobject(arrow2))
...
class PascalsCollision(Scene):
    def construct(self):
        ...
        equation = OldTex(["{n \\choose k}", " = \\dfrac{n!}{(n-k)!k!}"])
...
class HaveToShare(Scene):
    def construct(self):
        ...
        left_dot, top_dot, bottom_dot = [
            Dot(point, radius=0.1) for point in (ORIGIN, RIGHT + 0.5 * UP, RIGHT + 0.5 * DOWN)
        ]

        line1, line2 = [Line(left_dot.get_center(), dot.get_center(), buff=0) for dot in (top_dot, bottom_dot)]
