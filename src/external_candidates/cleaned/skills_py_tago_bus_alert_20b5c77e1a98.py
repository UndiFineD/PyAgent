# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\hsooooo.py\korea_metropolitan_bus_alerts.py\scripts.py\tago_bus_alert_20b5c77e1a98.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hsooooo\korea-metropolitan-bus-alerts\scripts\tago_bus_alert.py
...
def cmd_arrivals(args: argparse.Namespace) -> int:
    ...
    print(
        json.dumps(
            {"cityCode": args.city, "nodeId": args.node, "arrivals": out},
            ensure_ascii=False,
            indent=2,
        )
    )
...
def main() -> int:
    ...
    p2 = sub.add_parser(
        "arrivals",
        help="List arrivals for a stop (optionally filtered by route numbers)",
    )
