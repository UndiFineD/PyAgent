# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\agentic_layer.py\vectorize_service_5d5cdc33c178.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\agentic_layer\vectorize_service.py
...
class VectorizeConfig:
    ...
    def __post_init__(self):
        ...
        if not self.api_key:
            self.api_key = os.getenv("VECTORIZE_API_KEY") or os.getenv("VECTORIZE_API_KEY", "")
        ...
        if self.batch_size == 10:
            self.batch_size = int(os.getenv("VECTORIZE_BATCH_SIZE") or os.getenv("VECTORIZE_BATCH_SIZE", "10"))

        if self.max_concurrent_requests == 5:
            self.max_concurrent_requests = int(os.getenv("VECTORIZE_MAX_CONCURRENT", "5"))
...
class VectorizeServiceInterface(ABC):
    ...
    @abstractmethod
    async def get_embedding(self, text: str, instruction: Optional[str] = None) -> np.ndarray:
        pass
    ...
    @abstractmethod
    async def get_embeddings(self, texts: List[str], instruction: Optional[str] = None) -> List[np.ndarray]:
        pass
...
class VectorizeService(VectorizeServiceInterface):
    ...
    async def get_embedding(self, text: str, instruction: Optional[str] = None, is_query: bool = False) -> np.ndarray:
