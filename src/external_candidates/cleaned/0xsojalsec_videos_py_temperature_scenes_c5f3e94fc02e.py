# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\diffyq.py\part4.py\temperature_scenes_c5f3e94fc02e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part4\temperature_scenes.py
...
class StepFunctionExample(BringTwoRodsTogether, FourierSeriesIllustraiton):
    ...
    def bring_rods_together(self):
        ...
        black_rects[0].add_updater(lambda m: m.align_to(rods[0].get_right(), LEFT))

        black_rects[1].add_updater(lambda m: m.align_to(rods[1].get_left(), RIGHT))
        ...
        self.play(Restore(rods, **kw), Restore(words, **kw), *map(ShowCreation, arrows))
    ...
    def add_labels(self):
        ...
        y_numbers = y_axis.get_number_mobjects(*np.arange(-1, 1.5, 0.5), unit="^\\circ", num_decimal_places=1)
    ...
    def compare_to_sine_wave(self):
        ...
        self.play(k_tracker.set_value, PI, A_tracker.set_value, 4 / PI, run_time=3)
...
class BreakDownStepFunction(StepFunctionExample):
    ...
    def init_updaters(self):
        ...
        for mg in self.mini_graphs:
            mg.add_updater(lambda m, dt: self.update_graph(m, dt, alpha=self.scale_factor * self.alpha))
