# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\conversation_data_raw_repository_c20cf6e456a7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\repository\conversation_data_raw_repository.py
...
class ConversationDataRepository(ABC):
    ...
    @abstractmethod
    async def save_conversation_data(self, raw_data_list: List[RawData], group_id: str) -> bool:
...
class ConversationDataRepositoryImpl(ConversationDataRepository):
        ...
        try:
            ...
            for raw_data in raw_data_list:
                try:
                    ...
                    if raw_data.content:
                        timestamp = raw_data.content.get("timestamp") or raw_data.content.get("createTime")
    ...
    async def get_conversation_data(
        ...
        try:
            ...
            start_dt = _normalize_datetime_for_storage(start_time) if start_time else None
            ...
            try:
                ...
                for cache_data in cache_data_list:
                    try:
                        ...
                        if json_data is None:
                            logger.warning("Missing 'data' field in cache data: %s", cache_data)
                ...
                logger.debug("Successfully deserialized %d RawData objects", len(raw_data_list))
    ...
    async def delete_conversation_data(self, group_id: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.info("Successfully deleted conversation data: group_id=%s", group_id)
            ...
            logger.error("Failed to delete conversation data: group_id=%s, error=%s", group_id, e)
