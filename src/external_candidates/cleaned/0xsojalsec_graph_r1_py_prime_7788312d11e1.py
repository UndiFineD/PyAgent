# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graph_r1.py\verl.py\workers.py\reward_manager.py\prime_7788312d11e1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\workers\reward_manager\prime.py
...
async def single_compute_score(evaluation_func, completion, reference, task, executor, timeout=300.0):
...
async def parallel_compute_score_async(evaluation_func, completions, references, tasks, num_processes=64):
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        tasks_async = [
            single_compute_score(evaluation_func, completion, reference, task, executor, timeout=300.0)
