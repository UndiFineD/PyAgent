# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\entity_raw_repository_2b76b8509f16.py
...
class EntityRawRepository(BaseRepository[Entity]):
    ...
    async def get_by_entity_id(self, entity_id: str, session: Optional[AsyncClientSession] = None) -> Optional[Entity]:
        """Get entity by entity ID"""

        try:
            result = await self.model.find_one({"entity_id": entity_id}, session=session)

            if result:
                logger.debug("✅ Successfully retrieved entity by entity ID: %s", entity_id)
    ...
    async def get_by_alias(self, alias: str, session: Optional[AsyncClientSession] = None) -> List[Entity]:
        """Get list of entities by alias"""

        try:
            results = await self.model.find({"aliases": {"$in": [alias]}}, session=session).to_list()
    ...
    async def update_by_entity_id(
        ...
        try:
            existing_doc = await self.model.find_one({"entity_id": entity_id}, session=session)
    ...
    async def delete_by_entity_id(self, entity_id: str, session: Optional[AsyncClientSession] = None) -> bool:
    ...
    async def get_all_entities(self, limit: int = 100, session: Optional[AsyncClientSession] = None) -> List[Entity]:
        """Get all entities"""

        try:
            results = await self.model.find({}, session=session).limit(limit).to_list()

            logger.debug("✅ Successfully retrieved all entities: count=%d", len(results))
    ...
    async def get_entities_by_ids(
        ...
        try:
            results = await self.model.find({"entity_id": {"$in": entity_ids}}, session=session).to_list()
    ...
    async def count_by_type(self, entity_type: str, session: Optional[AsyncClientSession] = None) -> int:
        """Count the number of entities of specified type"""

        try:
            count = await self.model.find({"type": entity_type}, session=session).count()
