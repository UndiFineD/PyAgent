# Extracted from: C:\DEV\PyAgent\.external\abetlen-llama-cpp-python\llama_cpp\llama_chat_format.py
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        if self.stop_token_ids is not None:
            ...
            def stop_on_last_token(tokens: npt.NDArray[np.intc], logits: npt.NDArray[np.single]) -> bool:
        ...
        **kwargs,  # type: ignore
        ...
        if tool_choice is not None and isinstance(tool_choice, dict) and tools is not None:
            ...
            try:
                ...
                grammar = llama_grammar.LlamaGrammar.from_json_schema(json.dumps(schema), verbose=llama.verbose)
...
def register_chat_format(name: str):
    def decorator(f: ChatFormatter):
        ...
        LlamaChatCompletionHandlerRegistry().register_chat_completion_handler(name, chat_completion_handler)
        return f
...
def functionary_chat_handler(
    ...
    **kwargs,  # type: ignore
    ...
    def prepare_messages_for_inference(
        ...
        all_messages.append(llama_types.ChatCompletionRequestSystemMessage(role="system", content=SYSTEM_MESSAGE))
        ...
        all_messages.append(llama_types.ChatCompletionRequestAssistantMessage(role="assistant", content=None))
...
def functionary_v1_v2_chat_handler(
    ...
    **kwargs,  # type: ignore
    ...
    if function_call == "none" or functions is None or len(functions) == 0:
        ...
        if stream is False:
            completion_or_completion_chunks["choices"][0]["text"] = completion_or_completion_chunks["choices"][0][
                "text"
            ].lstrip()
        return _convert_completion_to_chat(completion_or_completion_chunks, stream=stream)  # type: ignore
        ...
        # If "auto" or no tool_choice/function_call
            ...
            while True:
                ...
                if function_name == "all":
                    ...
                    if (
                        "<|from|> assistant" in completion_text or "<|from|>assistant" in completion_text
    ...
    if stream is not False:
        return generate_streaming(tools=tools, functions=functions, function_call=function_call, prompt=prompt)
    else:
        if version == "v1":
    ...
    def __call__(
        ...
        **kwargs,  # type: ignore
        ...
        if system_prompt == "" and self.DEFAULT_SYSTEM_MESSAGE is not None:
            messages = [
                llama_types.ChatCompletionRequestSystemMessage(role="system", content=self.DEFAULT_SYSTEM_MESSAGE)
            ] + messages
    ...
    if not stream:
        ...
        return {
            ...
            "usage": {
                ...
                "prompt_tokens": sum(
                    completion["usage"]["prompt_tokens"] if "usage" in completion else 0 for completion in completions
                ),
                "total_tokens": sum(
                    completion["usage"]["total_tokens"] if "usage" in completion else 0 for completion in completions
                ),
            },
        }
