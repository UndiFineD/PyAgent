# Extracted from: C:\DEV\PyAgent\.external\abetlen-llama-cpp-python\llama_cpp\_internals.py
...
class LlamaModel:
    ...
    def __init__(
        ...
        *,
        ...
        with suppress_stdout_stderr(disable=verbose):
            model = llama_cpp.llama_model_load_from_file(self.path_model.encode("utf-8"), self.params)
    ...
    def tokenize(self, text: bytes, add_bos: bool, special: bool):
        ...
        n_tokens = llama_cpp.llama_tokenize(self.vocab, text, len(text), tokens, n_ctx, add_bos, special)
        if n_tokens < 0:
            n_tokens = abs(n_tokens)
            tokens = (llama_cpp.llama_token * n_tokens)()
            n_tokens = llama_cpp.llama_tokenize(self.vocab, text, len(text), tokens, n_tokens, add_bos, special)
            if n_tokens < 0:
...
class LlamaContext:
    ...
    def __init__(
        ...
        *,
        ...
        self.sampler = (
            None  # LlamaContext doesn't manage samplers directly, but some cleanup code expects this attribute
        )
...
class LlamaSampler:
    ...
    def add_logit_bias(self, n_vocab: int, logit_bias: Dict[int, float]):
        ...
        sampler = llama_cpp.llama_sampler_init_logit_bias(n_vocab, len(logit_bias), bias_array)
        llama_cpp.llama_sampler_chain_add(self.sampler, sampler)
