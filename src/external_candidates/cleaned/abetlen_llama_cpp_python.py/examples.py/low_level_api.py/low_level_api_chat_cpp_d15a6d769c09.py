# Extracted from: C:\DEV\PyAgent\.external\abetlen-llama-cpp-python\examples\low_level_api\low_level_api_chat_cpp.py
"""
...
specified) expect poor results""",
        ...
        self.model = llama_cpp.llama_load_model_from_file(self.params.model.encode("utf8"), self.lparams)
...
| {llama_cpp.llama_print_system_info().decode('utf8')}",
        ...
        self.need_to_save_session = len(self.params.path_session) > 0 and self.n_matching_session_tokens < (
            len(self.embd_inp) * 3 / 4
        )
...
""",
        ...
        while self.remaining_tokens > 0 or self.params.interactive or self.params.n_predict == -1:
            ...
            if len(self.embd) > 0:
                ...
                if self.n_past + len(self.embd) > self.n_ctx:
                    ...
                    _insert = self.last_n_tokens[self.n_ctx - int(n_left / 2) - len(self.embd) : -len(self.embd)]
    ...
    def token_to_str(self, token_id: int) -> bytes:
        ...
        n = llama_cpp.llama_token_to_piece(self.model, llama_cpp.llama_token(token_id), buffer, size)
        assert n <= size
