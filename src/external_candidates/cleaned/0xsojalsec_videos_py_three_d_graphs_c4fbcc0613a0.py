# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\diffyq.py\part4.py\three_d_graphs_c4fbcc0613a0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part4\three_d_graphs.py
...
class ShowLinearity(WriteHeatEquationTemplate, TemperatureGraphScene):
    ...
    def add_function_labels(self):
        ...
        T_sum_with_scalars = OldTex("a", "T_1", "+", "b", "T_2", **kw)
        ...
        s1_decimal.add_updater(lambda m: m.set_value(self.scalar_trackers[0].get_value()))
        ...
        s2_decimal.add_updater(lambda m: m.set_value(self.scalar_trackers[1].get_value()))
    ...
    def get_axes(self, **kwargs):
        ...
        axes.x_axis.label.next_to(axes.x_axis.get_end(), DOWN, buff=2 * SMALL_BUFF)
...
class CombineSeveralSolutions(ShowLinearity):
    ...
    def show_time_passing(self):
        ...
        anims = [ApplyMethod(time_tracker.set_value, 1, run_time=1, rate_func=linear)]
        ...
        for axes, graph_group in zip(all_axes, all_graphs):
            ...
            gslice.add_updater(lambda m: m.become(self.get_time_slice_graph(m.axes, m.func, t=get_t())))
    ...
    def get_decimals(self, axes_group, scalar_trackers):
        ...
        for axes, st in zip(axes_group, scalar_trackers):
            ...
            decimal.add_updater(lambda d: d.set_value(d.scalar_tracker.get_value()))
        ...
        for x in range(self.n_cycles):
            self.play(
                LaggedStart(
                    *[
                        ApplyMethod(st.set_value, value)
                        for st, value in zip(scalar_trackers, 3 * np.random.random(nst) - 1.5)
                    ]
                ),
                run_time=3,
            )
