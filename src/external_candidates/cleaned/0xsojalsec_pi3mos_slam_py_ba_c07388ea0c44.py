# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_pi3mos_slam.py\dpvo.py\ba_c07388ea0c44.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\dpvo\ba.py

import torch

from torch_scatter import scatter_sum

from . import fastba, lietorch
...
def BA(
    ...
    E = safe_scatter_add_mat(Eik, ii, kk, n, m).view(b, n, m, 6, 1) + safe_scatter_add_mat(Ejk, jj, kk, n, m).view(
        b, n, m, 6, 1
    )

    C = safe_scatter_add_vec(torch.matmul(wJzT, Jz), kk, m)

    v = safe_scatter_add_vec(vi, ii, n).view(b, n, 1, 6, 1) + safe_scatter_add_vec(vj, jj, n).view(b, n, 1, 6, 1)
