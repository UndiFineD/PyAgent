# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\aenvironment.py\aenv.py\src.py\cli.py\cmds.py\init_40e9850d3fb2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\cmds\init.py
...
@click.option("--work-dir", "-w", help="Working directory for initialization", default=os.getcwd())
...
def init(cfg: Config, name, version, template, work_dir, force):
    ...
    next_steps_table.add_row("Navigate to project", f"cd {os.path.basename(project_dir)}")
