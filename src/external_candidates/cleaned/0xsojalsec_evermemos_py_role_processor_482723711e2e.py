# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\group_profile.py\role_processor_482723711e2e.py
    ...
    def process_roles_with_evidences(
        ...
        for role_name, assignments in role_data.items():
            ...
            for assignment in assignments:
                ...
                user_name = self.data_processor.get_user_name(speaker_id, speaker_mapping)
                ...
                if key in historical_role_map:
                    ...
                    merged_evidences = self.data_processor.validate_and_filter_memcell_ids(
                        llm_evidences,
                        valid_memcell_ids,
                        user_id=speaker_id,
                        memcell_list=memcell_list,
            ...
            processed_assignments.sort(key=lambda x: (x["confidence"] != "strong", x["user_name"]))
