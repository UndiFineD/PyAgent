# Extracted from: C:\DEV\PyAgent\.external\action\2_engineering_team\output\app.py
import gradio as gr
...
def create_account(user_id, initial_deposit):
    ...
    return (
        f"Account created for {user_id} with initial deposit of ${initial_deposit:.2f}",
        get_account_info(),
    )
...
def buy_shares(symbol, quantity):
    ...
    if account.buy_shares(symbol, quantity):
        return (
            f"Successfully bought {quantity} shares of {symbol} at ${price:.2f} each.",
            get_account_info(),
        )
    else:
...
def get_transactions():
    ...
    for i, tx in enumerate(transactions, 1):
        if tx["type"] == "deposit":
            ...
            result += (
                f"{i}. Buy: {tx['quantity']} shares of {tx['symbol']} at ${tx['price']:.2f} = ${tx['total']:.2f}\n"
            )
        elif tx["type"] == "sell":
            result += (
                f"{i}. Sell: {tx['quantity']} shares of {tx['symbol']} at ${tx['price']:.2f} = ${tx['total']:.2f}\n"
            )
...
with gr.Blocks(title="Trading Simulation Platform") as demo:
    ...
    deposit_btn.click(fn=deposit, inputs=[deposit_input], outputs=[result_output, account_info])

    withdraw_btn.click(fn=withdraw, inputs=[withdraw_input], outputs=[result_output, account_info])
    ...
    portfolio_btn.click(fn=get_portfolio_value, inputs=[], outputs=[portfolio_output])

    profit_btn.click(fn=get_profit_loss, inputs=[], outputs=[profit_output])

    holdings_btn.click(fn=get_holdings, inputs=[], outputs=[holdings_output])

    transactions_btn.click(fn=get_transactions, inputs=[], outputs=[transactions_output])
