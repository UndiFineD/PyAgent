# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\examples\mini-program\agent\agent_server.py
# Copyright 2025.
#
...
def get_openai_client() -> Any:
    ...
    if _openai_client is None:
        ...
        logger.info(f"Initialized OpenAI client with base_url={base_url or 'default'}, timeout=300s")
    ...
    try:
        ...
        if isinstance(tools, list):
            ...
            for tool_info in tools:
                ...
                tool_base_name = tool_full_name.split("/")[-1] if "/" in tool_full_name else tool_full_name
            ...
            for tool_key, tool_info in tools.items():
                ...
                tool_base_name = tool_key.split("/")[-1] if "/" in tool_key else tool_key
                if tool_base_name == tool_name:
                    tool_found = tool_key
                    break
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if is_rate_limit and attempt < max_retries:
                ...
                if is_rate_limit:
                    logger.error(f"Rate limit error after {max_retries + 1} attempts. Giving up.")
                raise
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if isinstance(tools_result, list):
            ...
            logger.warning(f"Unexpected tools_result type: {type(tools_result)}, value: {tools_result}")

        logger.info(f"Prepared {len(tools_list)} tools for formatting")
        openai_tools = format_tools_for_openai(tools_list)
        logger.info(f"Formatted {len(tools_list)} tools into {len(openai_tools)} OpenAI tools")

        # Prepare messages
        # List available tool names for the system prompt
        available_tool_names = [tool["function"]["name"] for tool in openai_tools] if openai_tools else []
        system_content = (
        ...
        while message.tool_calls and iteration < max_iterations:
            ...
            for tool_call in message.tool_calls:
                ...
                try:
                    ...
                    logger.warning(f"Failed to serialize tool result as JSON: {e}, using string representation")
            ...
            logger.info(f"Calling OpenAI API for next iteration (iteration {iteration})")
            try:
                response = await call_openai_with_retry(
                    client=client,
                    model=model,
        ...
        if message.tool_calls:
            ...
            logger.warning(f"Reached max iterations ({max_iterations}), stopping tool calls")
