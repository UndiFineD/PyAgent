# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\aenv\core\environment.py
# Copyright 2025.
#
...
class Environment:
    ...
    def __init__(
        ...
        if not aenv_url:
            aenv_url = self.dummy_instance_ip or os.getenv("AENV_SYSTEM_URL", "http://localhost")
    ...
    def _log_prefix(self) -> str:
        ...
        instance_id = getattr(self._instance, "id", "None") if self._instance else "None"
    ...
    async def initialize(self) -> bool:
        ...
        if self._initialized:
            logger.info(f"{self._log_prefix()} Environment '{self.env_name}' already initialized")
            return True
        ...
        try:
            ...
            self._initialized = True
            logger.info(f"{self._log_prefix()} Environment '{self.env_name}' initialized successfully")
            return True
            ...
            await self.release()
            raise EnvironmentError(f"Failed to initialize environment '{self.env_name}': {str(e)}") from e

    async def release(self):
        """Release environment resources."""
        logger.info(f"{self._log_prefix()} Releasing environment resources: {self.env_name}")
        ...
        if self._mcp_client:
            try:
                ...
                logger.warning(f"{self._log_prefix()} Failed to close MCP client: {str(e)}")
            finally:
        ...
        if self._client:
            if self._instance and not self.dummy_instance_ip:
                try:
                    logger.debug(f"{self._log_prefix()} Deleting environment instance: {self._instance.id}")
                    await self._client.delete_env_instance(self._instance.id)
                    logger.debug(f"{self._log_prefix()} Environment instance deleted")
                except Exception as e:
                    logger.warning(f"{self._log_prefix()} Failed to delete environment instance: {str(e)}")
            ...
            try:
                ...
                logger.warning(f"{self._log_prefix()} Failed to close scheduler client: {str(e)}")
            finally:
                self._client = None

        self._instance = None
        self._initialized = False
        logger.info(f"{self._log_prefix()} Environment resources released: {self.env_name}")
    ...
    async def list_tools(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with client:
                ...
                logger.info(f"{self._log_prefix()} Found {len(tools)} tools in environment {self.env_name}")
            ...
            raise EnvironmentError(f"Failed to list tools for environment '{self.env_name}': {str(e)}")
    ...
    async def list_functions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                logger.info(f"{self._log_prefix()} Found {total} functions in environment {self.env_name}")
                return function_data

            # Fallback: return as-is
            logger.warning(f"{self._log_prefix()} Unexpected response format from list_functions: {result}")
            return result
        except Exception as e:
            logger.error(
            ...
            raise EnvironmentError(f"Failed to list functions for environment '{self.env_name}': {str(e)}")
    ...
    async def call_reward(
        ...
        """
        return await self._call_function(self.aenv_reward_url, arguments=arguments, timeout=timeout)
    ...
    async def _call_function(
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout or self.timeout) as client:
                if method == "GET":
                    params = {k: (json.dumps(v) if isinstance(v, (dict, list)) else v) for k, v in arguments.items()}
                    ...
                    raise ValueError(f"{self._log_prefix()} Unsupported HTTP method: {method}")
            ...
            raise EnvironmentError(f"Function '{function_url}' execution failed: {error_msg}")
        except Exception as e:
            logger.error(
            ...
            raise EnvironmentError(f"Function '{function_url}' execution failed: {str(e)}")
    ...
    async def check_health(
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout or self.timeout) as client:
                response = await client.get(self.aenv_health_url, params=arguments, headers=self.proxy_headers)

                response.raise_for_status()
                result = response.json()
                logger.info(f"{self._log_prefix()} Health function executed successfully with result: {result}")
                return result
    ...
    async def call_tool(
        ...
        if "/" in tool_name:
            ...
            if env_name != self.env_name:
                raise ToolError(f"Tool '{tool_name}' not found in environment '{self.env_name}'")
        else:
            actual_tool_name = tool_name

        logger.info(f"{self._log_prefix()} Executing tool: {actual_tool_name} in environment {self.env_name}")

        try:
            client = await self._get_mcp_client()
            async with client:
                result = await client.call_tool_mcp(name=actual_tool_name, arguments=arguments, timeout=timeout)
    ...
    async def _wait_for_healthy(self, timeout: float = 300.0) -> None:
        ...
        logger.info(f"{self._log_prefix()} Waiting for environment {self.env_name} to be healthy...")
        try:
            self.proxy_headers = {
            ...
            while True:
                ...
                try:
                    ...
                    logger.debug(f"{self._log_prefix()} check {self.env_name} health result={result}")

                    if result.get("status") == "success" or result.get("status") == "healthy":
                        logger.info(f"{self._log_prefix()} Environment {self.env_name} is healthy")
                        return

                except Exception as e:
                    logger.debug(f"{self._log_prefix()} Health check failed: {str(e)}, retrying...")

                if asyncio.get_event_loop().time() - start_time > timeout:
                    raise EnvironmentError(f"Timeout waiting for status healthy: {self._instance.id}")
            ...
            raise EnvironmentError(f"Failed to wait for environment '{self.env_name}' to be healthy: {str(e)}")
    ...
    async def wait_for_ready(self, timeout: float = 300.0) -> None:
        ...
        try:
            instance = await self._client.wait_for_status(self._instance.id, EnvStatus.RUNNING, timeout=timeout)
            ...
            raise EnvironmentError(f"Failed to wait for environment '{self.env_name}' to be ready: {str(e)}")
    ...
    async def _create_env_instance(self):
        ...
        logger.info(f"{self._log_prefix()} Creating environment instance: {self.env_name}")
        try:
            ...
            logger.info(f"{self._log_prefix()} Environment instance created with ID: {self._instance.id}")
    ...
    async def call_function(
        ...
        return await self._call_function(function_url, arguments=arguments, timeout=timeout)
    ...
    async def _get_mcp_client(self) -> Client:
        ...
        try:
            ...
            self._mcp_client = Client(
                transport=StreamableHttpTransport(self.aenv_data_url, headers=self.proxy_headers),
                timeout=self.timeout,
            )
