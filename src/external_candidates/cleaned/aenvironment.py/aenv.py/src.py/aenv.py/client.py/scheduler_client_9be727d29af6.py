# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\aenv\client\scheduler_client.py
# Copyright 2025.
#
            ...
            try:
                ...
                try:
                    ...
                    if api_response.success and api_response.data:
                        ...
                        error_msg = getattr(api_response, "error_message", "Unknown error")
                        raise EnvironmentError(f"Failed to create instance: {error_msg}, rsp: {api_response}")
                ...
                if attempt < self.max_retries:
                    ...
                    logger.warning(f"Network error, retrying in {wait_time:.2f}s: {str(e)}")
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                try:
                    ...
                    if api_response.success and api_response.data:
                        instance = EnvInstance(**api_response.data)
                        logger.debug(f"Instance status: {instance.id} -> {instance.status}")
                        return instance
                    else:
                        error_msg = getattr(api_response, "error_message", "Unknown error")
    ...
    async def wait_for_status(
        ...
        while True:
            ...
            if asyncio.get_event_loop().time() - start_time > timeout:
                raise EnvironmentError(f"Timeout waiting for status {target_status.value}: {instance_id}")
