# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\client\aenv_hub_client.py
# Copyright 2025.
#
...
class AEnvHubClient:
    ...
    # ===== Advanced Features =====
    def clone_environment(self, source_name: str, target_name: str, description: str = "") -> Dict[str, Any]:
        """Clone environment"""
        data = {
    ...
    def search_environments(self, query: str, tags: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """Search environments"""
        params = {"q": query}
        if tags:
            params["tags"] = ",".join(tags)
        response = self._make_request("GET", "/api/v1/environments/search", params=params)
    ...
    def load_client(cls, base_url: str = None, api_key: str = None):
        ...
        if cls._singleton_lock:
            raise RuntimeError("Recursive call detected during AEnvHubClient singleton creation")
