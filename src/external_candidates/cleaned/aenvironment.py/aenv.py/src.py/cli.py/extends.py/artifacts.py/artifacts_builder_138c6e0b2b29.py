# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\extends\artifacts\artifacts_builder.py
# Copyright 2025.
#
                    ...
                    if "stream" in log_line:
                        ...
                        if stream_text:
                            ...
                            if stream_text.startswith("Step"):
                                ...
                                container_id = stream_text.split("Running in ")[1].strip()
                                print(f"   🏃 Running in container: {container_id[:12]}")
                            elif "Removing intermediate container" in stream_text:
                                container_id = stream_text.split("Removing intermediate container ")[1].split()[0]
                                print(f"   🗑️  Removing intermediate container: {container_id[:12]}")
                            elif "Successfully built" in stream_text:
                                image_id = stream_text.split("Successfully built ")[1].strip()
                                print(f"   ✅ Successfully built: {image_id[:12]}")
                            elif "Successfully tagged" in stream_text:
                                tagged_image = stream_text.split("Successfully tagged ")[1].strip()
                        ...
                        if "id" in log_line:
                            ...
                            if progress:
                                ...
                                current = log_line.get("progressDetail", {}).get("current", 0)
                                total = log_line.get("progressDetail", {}).get("total", 0)
                                if total > 0:
