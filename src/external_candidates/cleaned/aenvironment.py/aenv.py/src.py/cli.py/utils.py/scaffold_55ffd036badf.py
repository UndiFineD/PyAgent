# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\utils\scaffold.py
# Copyright 2025.
#
...
class LocalScaffold(AEnvScaffold):
    def init(self, params: ScaffoldParams):
        ...
        click.echo(f"✓ Successfully initialized project from template '{params.template}'")
        ...
        try:
            ...
            try:
                ...
                if hasattr(importlib.resources, "files"):
                    ...
                    try:
                        with importlib.resources.path("cli.templates", template_name) as template_path:
                            if template_path.exists() and template_path.is_dir():
                                return template_path
    ...
    def _list_available_templates(self) -> list[str]:
        ...
        try:
            ...
            if templates_dir.exists():
                return [
                    item.name for item in templates_dir.iterdir() if item.is_dir() and not item.name.startswith("__")
                ]
    ...
    def _copy_template(self, template_dir: Path, target_path: Path, force: bool = False):
...
def load_aenv_scaffold() -> AEnvScaffold:
    ...
    if mode == "local":
        ...
        raise click.ClickException(f"Unsupported mode: {mode}. Only 'local' mode is supported for scaffolding.")
