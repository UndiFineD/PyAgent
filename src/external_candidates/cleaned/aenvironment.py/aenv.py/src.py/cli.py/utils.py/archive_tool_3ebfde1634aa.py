# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\utils\archive_tool.py
# Copyright 2025.
#
...
class ArchiveTool:
    ...
    def pack_directory(
        ...
        with tarfile.open(output_path, mode) as tar:
            for root, dirs, files in os.walk(source_dir):
                ...
                dirs[:] = [d for d in dirs if not any(exclude in d for exclude in exclude_set)]
                files = [f for f in files if not any(exclude in f for exclude in exclude_set)]
                for file in files:
                    file_path = Path(root) / file
                    arc_path = file_path.relative_to(source_path.parent)
                    tar.add(file_path, arcname=arc_path)
        logger.info(f"✅ Packaging completed: {output_path} ({os.path.getsize(output_path)} bytes)")
        return output_path

    @classmethod
    def pack_and_cleanup(cls, source_dir: str, remove_source: bool = False, **pack_kwargs) -> str:
    ...
    def get_archive_info(cls, archive_path: str) -> dict:
        ...
        return {
            ...
            "compression": (archive_path.split(".")[-1] if "." in archive_path else "none"),
