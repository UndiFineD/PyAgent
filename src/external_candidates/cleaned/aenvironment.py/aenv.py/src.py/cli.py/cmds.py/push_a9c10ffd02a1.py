...
def push(work_dir, dry_run, force, version):
    ...
    if exist:
        click.echo(f"aenv:{env_name}:{version} already exists in remote aenv_hub", err=False)
    ...
    click.echo("✅Push successfully! Now you can get environment status with `aenv get`")
