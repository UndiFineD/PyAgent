# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\cmds\common.py
# Copyright 2025.
#
...
class ErrorHandler:
    ...
    def _handle_validation_error(cls, error: ValidationError) -> None:
        ...
        error_info = cls.ERROR_SUGGESTIONS.get(error.error_code, cls.ERROR_SUGGESTIONS["VALIDATION_ERROR"])
        ...
        if details:
            ...
            content.renderables.append(Text("Detailed Information:", style="bold yellow"))
        ...
        content.renderables.extend([Text(f"  {i + 1}. {s}") for i, s in enumerate(formatted_suggestions)])
    ...
    def _handle_unexpected_error(cls, error: Exception) -> None:
        ...
        panel = Panel(content, title="[bold red]❌ System Error", border_style="red", expand=False)
        ...
        CliConsole.console().print("  2. Ensure all dependencies are correctly installed")
...
def _handle_click_error(e: click.ClickException):
    ...
    if isinstance(e, click.UsageError):
        CliConsole.console().print(Panel(f"{e}", title="CLI error", border_style="yellow"))
    else:
        # Other Click Error
        CliConsole.console().print(Panel(f"[red]Error:[/red] {e}", title="Error", border_style="red"))
