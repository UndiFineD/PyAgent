# Extracted from: C:\DEV\PyAgent\.external\AEnvironment\aenv\src\cli\cmds\build.py
# Copyright 2025.
#
...
class BuildProgressTracker:
    ...
    def update_stage(self, stage: str, status: str, progress: float = 0.0) -> None:
        ...
        self.console.print(f"[dim]{stage}[/dim]: [bold]{status}[/bold] ({progress:.1f}%)")
...
@click.option("--push/--no-push", default=False, help="Push image to registry after build")
@click.option("--platform", "-p", type=str, help="Platform for the Docker image")
@pass_config
def build(cfg: Config, work_dir, image_name, image_tag, registry, namespace, platform, push):
    """
        ...
        try:
            ...
            with console.status("[bold green]Building Docker image..."):
                try:
                    builder.trigger(ctx)
                    if cfg.verbose:
                        tracker.log_message("Docker image built successfully", "success")
