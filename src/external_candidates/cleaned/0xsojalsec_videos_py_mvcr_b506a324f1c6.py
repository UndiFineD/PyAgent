# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2018.py\mvcr_b506a324f1c6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\mvcr.py
...
class HoldUpMultivariableChainRule(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_all_student_changes("confused", look_at=screen)
...
class ComputationalNetwork(MovingCameraScene):
    ...
    def walk_through_parts(self):
        ...
        for formula in f_formula, g_formula:
            ...
            self.play(ReplacementTransform(x.copy(), formula.get_parts_by_tex("x"), path_arc=PI / 3))

            self.wait()

        self.play(indicate(h_formula[0]))

        self.play(ReplacementTransform(f_formula[0].copy(), h_formula.get_part_by_tex("f"), path_arc=PI / 3))

        self.play(ReplacementTransform(g_formula[0].copy(), h_formula.get_part_by_tex("g"), path_arc=PI / 3))
    ...
    def feed_forward_input(self):
        ...
        self.revert_to_formula_animations = [
            ApplyMethod(term.set_fill, {"opacity": 1}) for term in (x_in_f, x_in_g, f_in_h, g_in_h)
    ...
    def expand_out_h_as_function_of_x(self):
        ...
        expanded_formula = OldTex("h = x^4 \\cos(\\pi x)", tex_to_color_map=self.tex_to_color_map)
    ...
    def show_four_derivatives(self):
        ...
        partial_term_rects = VGroup(*[SurroundingRectangle(pt, buff=0.05) for pt in partial_terms])
        ...
        self.play(LaggedStartMap(FadeIn, partial_term_rects), Animation(derivatives))

        self.wait()

        self.play(LaggedStartMap(FadeOut, partial_term_rects), Animation(derivatives))
    ...
    def plug_in_values(self):
        ...
        self.play(ReplacementTransform(full_derivative.copy(), rhs, lag_ratio=0.5, run_time=2))
