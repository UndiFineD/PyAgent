# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_online.py\src.py\second_brain_online.py\application.py\rag.py\retrievers_6e97bda7f820.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-online\src\second_brain_online\application\rag\retrievers.py
...
def get_retriever(
    ...
    embedding_model = get_embedding_model(embedding_model_id, embedding_model_type, device)
...
def get_hybrid_search_retriever(embedding_model: EmbeddingsModel, k: int) -> MongoDBAtlasHybridSearchRetriever:
...
def get_parent_document_retriever(embedding_model: EmbeddingsModel, k: int = 3) -> MongoDBAtlasParentDocumentRetriever:
