# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\llmtornado.py\src.py\llmtornado.py\llmtornado.py\fskb.py\indexing.py\recovery_6879a7e1c412.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\indexing\recovery.py
...
class RecoveryManager:
    ...
    async def save_state(self, roots: Dict[Path, Dict]):
        ...
        try:
            ...
            await asyncio.to_thread(lambda: temp_file.write_text(json.dumps(state, indent=2)))

            await asyncio.to_thread(lambda: temp_file.replace(self.state_file))
    ...
    async def load_state(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            content = await asyncio.to_thread(lambda: self.state_file.read_text())
