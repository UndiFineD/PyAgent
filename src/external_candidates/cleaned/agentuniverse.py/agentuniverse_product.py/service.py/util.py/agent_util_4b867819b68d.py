# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\util\agent_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def assemble_agent_config_data(agent_dto: AgentDTO) -> Dict:
    ...
    """
    agent_config_data = {
        "info": {"name": agent_dto.id, "description": agent_dto.description},
        "profile": {},
        "action": {},
    }
...
def get_planner_dto(agent: Agent) -> PlannerDTO | None:
    ...
    return PlannerDTO(nickname="", id=planner_name, members=members, workflow_id=workflow_id)
...
def get_tool_dto_list(agent_model: AgentModel) -> List[ToolDTO]:
    ...
    for tool_name in tool_name_list:
        ...
        tool_dto = ToolDTO(
            nickname=product.nickname if product is not None else "",
...
def get_prompt_dto(agent_model: AgentModel) -> PromptDTO | None:
    ...
    if not profile_prompt_model:
        return None
    return PromptDTO(
        introduction=profile_prompt_model.introduction,
        target=profile_prompt_model.target,
