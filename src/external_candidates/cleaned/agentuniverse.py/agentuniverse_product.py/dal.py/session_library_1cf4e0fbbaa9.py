...
class SessionLibrary:
    ...
    def update_session(self, session_do: SessionDO) -> str:
        ...
        try:
            with self.get_db_session() as db_session:
                session_orm = (
                    db_session.query(SessionORM).filter(SessionORM.session_id == session_do.session_id).first()
                )
                if session_orm:
    ...
    def delete_session(self, session_id: str) -> str | None:
        ...
        with self.get_db_session() as db_session:
            session_orm = db_session.query(SessionORM).filter(SessionORM.session_id == session_id).one_or_none()
            if session_orm:
    ...
    def get_session_detail(self, session_id: str) -> SessionDO | None:
        ...
        with self.get_db_session() as db_session:
            session_orm = db_session.query(SessionORM).filter(SessionORM.session_id == session_id).first()
            if session_orm:
