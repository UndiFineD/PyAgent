# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\agentuniverse_product.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class AgentUniverseProduct(object):
    ...
    def __scan_and_register_product(self, app_configer: AppConfiger):
        ...
        """
        core_product_package_list = (
            app_configer.core_product_package_list or app_configer.core_default_package_list
        ) + self.__system_default_product_package
        if core_product_package_list is None:
            return
        product_configer_list = AgentUniverse().scan(
            core_product_package_list, ConfigTypeEnum.YAML, ComponentEnum.PRODUCT
        )
