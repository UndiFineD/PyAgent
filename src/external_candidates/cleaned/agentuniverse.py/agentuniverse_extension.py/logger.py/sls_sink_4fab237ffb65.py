# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_extension\logger\sls_sink.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class SlsSender:
    ...
    def _send_put_logs_request(
        self, log_item_list: List[LogItem], topic: str = "", source: str = ""
        ...
        """
        try:
            put_request = PutLogsRequest(self.project, self.log_store, topic, source, log_item_list)
    ...
    def send_single_log(self, message: str, topic: str = "", source: str = "") -> Optional[PutLogsResponse]:
    ...
    def batch_send(self, topic: str = "", source: str = "") -> Optional[PutLogsResponse]:
        ...
        if self.log_queue is not None and size > 0:
            for i in range(size):
                try:
                    ...
                    self._logger.error("sls log queue shorter than expected, all logs have been sent")
                    break
                log_item_list.append(log_item)
    ...
    def start_batch_send_thread(self):
        ...
        if self.send_thread is None or not self.send_thread.is_alive():
            ...
            self.send_thread = threading.Thread(
                target=self._schedule_send_log, name="loop_send_log_thread", daemon=True
            )
...
class AsyncSlsSink:
    ...
    def __call__(self, message):
        ...
        item = LogItem(contents=[("content", message)], timestamp=int(record["time"].timestamp()))
