# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_toolkit_app\prompt\prompt_optimizer.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PromptOptimizer:
    ...
    def optimize_prompt(
        ...
        optimized_intro = self._optimize_section(prompt.introduction, strategies, custom_rules)
        optimized_target = self._optimize_section(prompt.target, strategies, custom_rules)
        optimized_instruction = self._optimize_section(prompt.instruction, strategies, custom_rules)

        # Generate improvements list
        improvements = self._generate_improvements(prompt, optimized_intro, optimized_target, optimized_instruction)

        # Generate suggestions
        suggestions = self._generate_suggestions(quality_scores, strategies)

        # Calculate confidence score
        confidence_score = self._calculate_confidence_score(quality_scores, improvements)

        # Format results
        original_prompt = self._format_prompt(prompt)
        optimized_prompt = self._format_optimized_prompt(optimized_intro, optimized_target, optimized_instruction)
    ...
    def _initialize_quality_patterns(self) -> Dict[PromptQualityMetric, Dict[str, Any]]:
        ...
        return {
            PromptQualityMetric.CLARITY: {
                ...
                "negative_patterns": [r"可能|也许|大概|或许", r"不清楚|模糊|不明确"],
                "weight": 0.3,
            },
            PromptQualityMetric.SPECIFICITY: {
                ...
                "negative_patterns": [r"一般|通常|大概|可能", r"简单|基础|基本"],
                "weight": 0.25,
            },
            ...
            PromptQualityMetric.STRUCTURE: {
                ...
                "negative_patterns": [r"混乱|无序|随意"],
                "weight": 0.15,
            },
            PromptQualityMetric.TONE: {
                "positive_patterns": [r"专业|友好|礼貌", r"请|谢谢|感谢"],
                "negative_patterns": [r"命令|强制|必须", r"粗鲁|不礼貌"],
                "weight": 0.1,
            },
        }
    ...
    def _generate_suggestions_for_metric(self, metric: PromptQualityMetric, score: float) -> List[str]:
        ...
        if metric == PromptQualityMetric.CLARITY and score < 0.7:
            suggestions.extend(["使用更清晰、具体的语言", "避免模糊或歧义的表达", "提供明确的指导步骤"])

        if metric == PromptQualityMetric.SPECIFICITY and score < 0.7:
            suggestions.extend(["添加具体的示例和说明", "提供详细的执行步骤", "明确约束条件和要求"])
        ...
        if metric == PromptQualityMetric.STRUCTURE and score < 0.7:
            suggestions.extend(["使用有序的步骤和流程", "添加编号或列表格式", "确保逻辑结构清晰"])

        if metric == PromptQualityMetric.TONE and score < 0.7:
            suggestions.extend(["使用更专业、友好的语调", "避免命令式语言", "添加礼貌用语"])
    ...
    def _rule_applies_to_strategies(self, rule: OptimizationRule, strategies: List[OptimizationStrategy]) -> bool:
    ...
    def _generate_suggestions(
        self, quality_scores: List[QualityScore], strategies: List[OptimizationStrategy]
    ) -> List[str]:
    ...
    def _calculate_confidence_score(self, quality_scores: List[QualityScore], improvements: List[str]) -> float:
    ...
    def _format_optimized_prompt(self, introduction: str, target: str, instruction: str) -> str:
