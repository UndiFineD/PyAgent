# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_toolkit_app\test\test_prompt_optimizer.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class TestPromptOptimizer(unittest.TestCase):
    ...
    def test_optimize_prompt_with_strategies(self):
        ...
        result = self.optimizer.optimize_prompt(self.sample_prompt, strategies=strategies)
    ...
    def test_optimize_prompt_with_custom_rules(self):
        ...
        result = self.optimizer.optimize_prompt(self.sample_prompt, custom_rules=[custom_rule])
    ...
    def test_get_applicable_rules_with_custom(self):
        ...
        strategies = [OptimizationStrategy.CLARITY]
        rules = self.optimizer._get_applicable_rules(strategies, [custom_rule])
    ...
    def test_calculate_metric_score(self):
        ...
        score = self.optimizer._calculate_metric_score("你是一个助手，帮助用户", patterns)
        self.assertIsInstance(score, float)
        self.assertGreater(score, 0.5)  # Should be positive

        # Test with negative content
        score = self.optimizer._calculate_metric_score("这是一个不好的错误", patterns)
        self.assertIsInstance(score, float)
    ...
    def test_generate_feedback(self):
        ...
        # Test high score
        feedback = self.optimizer._generate_feedback(PromptQualityMetric.CLARITY, 0.9)
        self.assertIsInstance(feedback, str)
        self.assertIn("优秀", feedback)

        # Test low score
        feedback = self.optimizer._generate_feedback(PromptQualityMetric.CLARITY, 0.3)
        self.assertIsInstance(feedback, str)
    ...
    def test_generate_suggestions_for_metric(self):
        ...
        suggestions = self.optimizer._generate_suggestions_for_metric(PromptQualityMetric.CLARITY, 0.5)
        self.assertIsInstance(suggestions, list)
        self.assertGreater(len(suggestions), 0)

        # Test high score (should have fewer suggestions)
        suggestions_high = self.optimizer._generate_suggestions_for_metric(PromptQualityMetric.CLARITY, 0.9)
    ...
    def test_generate_improvements(self):
        ...
        improvements = self.optimizer._generate_improvements(self.sample_prompt, "优化介绍", "优化目标", "优化指令")
    ...
    def test_generate_suggestions(self):
        ...
        suggestions = self.optimizer._generate_suggestions(quality_scores, [OptimizationStrategy.CLARITY])
    ...
    def test_calculate_confidence_score(self):
        ...
        score = self.optimizer._calculate_confidence_score(quality_scores, ["改进1", "改进2"])
    ...
    def test_format_optimized_prompt(self):
        ...
        formatted = self.optimizer._format_optimized_prompt("优化介绍", "优化目标", "优化指令")
