# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_generator_app\app\intelligence\test\prompt_generator.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
    ...
    try:
        ...
        if isinstance(config, str):
            ...
            if "introduction" in config:
                intro_preview = (
                    config["introduction"][:100] + "..."
                    if len(config["introduction"]) > 100
                    else config["introduction"]
                )
    ...
    try:
        ...
        if isinstance(config, str):
            ...
            if "instruction" in config:
                instruction_preview = (
                    config["instruction"][:150] + "..." if len(config["instruction"]) > 150 else config["instruction"]
                )
