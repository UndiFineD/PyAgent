# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\document_classifier_tool\test_classifier.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class TestDocumentClassifier(unittest.TestCase):
    ...
    def test_keyword_classification(self):
        ...
        # 检查第一个文档（技术文档）
        self.assertEqual(classified_docs[0].metadata["classification"], "技术文档")
        self.assertGreater(classified_docs[0].metadata["classification_confidence"], 0.5)

        # 检查第二个文档（学术论文）
        self.assertEqual(classified_docs[1].metadata["classification"], "学术论文")

        # 检查第三个文档（新闻资讯）
        self.assertEqual(classified_docs[2].metadata["classification"], "新闻资讯")

        # 检查第四个文档（其他）
        self.assertEqual(classified_docs[3].metadata["classification"], "其他")
    ...
    def test_empty_document(self):
        ...
        self.assertEqual(classified_doc.metadata["classification"], "其他")
        self.assertEqual(classified_doc.metadata["classification_confidence"], 0.0)
...
class TestChineseDocumentClassifier(unittest.TestCase):
    ...
    def test_content_type_classification(self):
        ...
        self.assertEqual(self.classifier.classify_by_content_type(tech_text), "技术文档")

        # 学术论文
        academic_text = "本研究通过实验分析了机器学习算法的性能。"
        self.assertEqual(self.classifier.classify_by_content_type(academic_text), "学术论文")

        # 新闻资讯
        news_text = "据最新消息，某公司今日宣布重大事件。"
        self.assertEqual(self.classifier.classify_by_content_type(news_text), "新闻资讯")
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_no_categories(self):
        ...
        self.assertEqual(classified_doc.metadata["classification"], "未分类")
