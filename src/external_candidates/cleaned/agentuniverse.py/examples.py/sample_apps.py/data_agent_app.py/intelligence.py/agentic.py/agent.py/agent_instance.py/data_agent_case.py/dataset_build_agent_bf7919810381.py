# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\data_agent_app\intelligence\agentic\agent\agent_instance\data_agent_case\dataset_build_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from data_agent_app.intelligence.utils.common.jsonl_file_util import (
    JsonFileReader,
    JsonFileWriter,
)
...
class DatasetBuildAgent(Agent):
    ...
    def build_dataset(self, agent_input: dict) -> List[List[Tuple[str, str]]]:
        ...
        candidate_agent_name = (
            agent_input.get("candidate")
            if agent_input.get("candidate")
            else self.agent_model.profile.get("candidate", "")
        )
