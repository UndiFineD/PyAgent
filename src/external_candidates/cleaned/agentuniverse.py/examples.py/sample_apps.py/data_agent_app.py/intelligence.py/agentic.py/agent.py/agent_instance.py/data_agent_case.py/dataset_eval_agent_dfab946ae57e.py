# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\data_agent_app\intelligence\agentic\agent\agent_instance\data_agent_case\dataset_eval_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class DatasetEvalAgent(Agent):
    ...
    def execute(self, input_object: InputObject, agent_input: dict):
        ...
        LOGGER.info("-------------------------------------------")
        return {
            "eval_report_json_list": eval_report_json_list,
            "eval_dims_json_list": eval_dims_json_list,
        }
    ...
    @staticmethod
    def generate_eval_results_excel(
        query_answer_list: List[List[Tuple[str, str]]],
            ...
            for i in range(len(eval_dims_json_list)):
                ...
                for j in range(len(one_turn_eval_results)):
                    ...
                    row = [
                        line_number,
                        overall_score,
                        one_row_query_answer[0],
                        one_row_query_answer[1],
                    ]
                    for dim in dims:
                ...
                df.to_excel(
                    f"./data/eval_result_turn_{i + 1}_{date}.xlsx",
                    index=False,
                    engine="openpyxl",
                )
