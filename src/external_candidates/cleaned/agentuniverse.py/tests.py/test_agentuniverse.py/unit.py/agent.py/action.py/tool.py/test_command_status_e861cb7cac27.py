# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\tool\test_command_status.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.agent.action.tool.common_tool.command_status_tool import (
    CommandStatusTool,
)
...
class CommandStatusToolTest(unittest.TestCase):
    ...
    def test_command_status_check(self):
        tool_input = ToolInput({"command": 'echo "Hello World"', "cwd": os.getcwd(), "blocking": True})
        result_json = self.run_tool.execute(tool_input)
        result = json.loads(result_json)
        thread_id = result["thread_id"]

        status_input = ToolInput({"thread_id": thread_id})
