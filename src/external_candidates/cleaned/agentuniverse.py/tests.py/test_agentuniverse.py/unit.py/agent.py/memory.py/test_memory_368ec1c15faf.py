# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\memory\test_memory.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class MemoryTest(unittest.TestCase):
    ...
    def test_summarize_memory_1(self) -> None:
        ...
        llm_chain = ConversationChain(llm=langchain_memory.llm, memory=langchain_memory)
    ...
    def test_summarize_memory_2(self) -> None:
        ...
        llm_chain = LLMChain(llm=langchain_memory.llm, prompt=prompt, memory=langchain_memory)
    ...
    def test_truncate_memory_2(self) -> None:
        ...
        langchain_memory = self.chat_memory.as_langchain()
        prompt = PromptTemplate(input_variables=["chat_history", "human_input"], template=template)
        llm_chain = LLMChain(llm=langchain_memory.llm, memory=langchain_memory, prompt=prompt)
