# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\tool\test_arxiv_tool.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ArxivToolTest(unittest.TestCase):
    ...
    def test_search_papers(self) -> None:
        tool_input = ToolInput({"input": "machine learning", "mode": SearchMode.SEARCH.value})
        result = self.tool.execute(tool_input)
        self.assertTrue(result != "")

    def test_get_paper_detail(self) -> None:
        tool_input = ToolInput({"input": "1605.08386v1", "mode": SearchMode.DETAIL.value})
        result = self.tool.execute(tool_input)
        self.assertTrue(result != "")

    def test_invalid_mode(self) -> None:
        tool_input = ToolInput({"input": "test", "mode": "invalid_mode"})
