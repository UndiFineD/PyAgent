# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent_serve\test_service_manager.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
import unittest
...
@patch(
    "agentuniverse.base.component.component_manager_base.ApplicationConfigManager",
...
def test_service_manager():
    ...
    mock_service.service_code = SimpleService.generate_service_code(mock_service.name)
    ...
    unittest.TestCase().assertRaises(ValueError, service_manager.register, mock_service.service_code, mock_service)

    assert len(service_manager.get_instance_name_list()) == 1
    service_manager.unregister(mock_service.service_code)
