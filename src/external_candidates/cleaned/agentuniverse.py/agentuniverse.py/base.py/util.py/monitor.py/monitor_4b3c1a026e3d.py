...
class Monitor(BaseModel):
    ...
    def get_invocation_chain():
        ...
        if isinstance(current_chain, list):
            ...
            current_chain = []
            FrameworkContextManager().set_context(trace_id + "_invocation_chain", current_chain)
            return current_chain
    ...
    def get_invocation_chain_bak():
        ...
        trace_id = AuTraceManager().get_trace_id()
        current_chain = FrameworkContextManager().get_context(trace_id + "_invocation_chain_bak")
        if isinstance(current_chain, list):
            return current_chain
        else:
            current_chain = []
            FrameworkContextManager().set_context(trace_id + "_invocation_chain_bak", current_chain)
            return current_chain
