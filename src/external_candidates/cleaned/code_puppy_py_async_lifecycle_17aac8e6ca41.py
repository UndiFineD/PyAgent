# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\mcp.py\async_lifecycle_17aac8e6ca41.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\mcp_\async_lifecycle.py
...
class AsyncServerLifecycleManager:
    ...
    async def _server_lifecycle_task(
        ...
        try:
            ...
            logger.info(f"Server {server_id} _running_count before enter: {getattr(server, '_running_count', 'N/A')}")

            # Enter the server's context

            await exit_stack.enter_async_context(server)

            logger.info(f"Server {server_id} _running_count after enter: {getattr(server, '_running_count', 'N/A')}")
            ...
            logger.info(f"Server {server_id} started successfully and stored in _servers")
            ...
            while True:
                ...
                if not is_running:
                    logger.warning(f"Server {server_id} stopped unexpectedly! _running_count={running_count}")
            ...
            logger.info(f"Server {server_id} lifecycle ending, _running_count={running_count}")
            ...
            logger.info(f"Server {server_id} context closed, _running_count={running_count_after}")
