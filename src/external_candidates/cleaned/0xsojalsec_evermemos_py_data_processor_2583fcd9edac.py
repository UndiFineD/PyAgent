# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\group_profile.py\data_processor_2583fcd9edac.py
...
class GroupProfileDataProcessor:
    ...
    def validate_and_filter_memcell_ids(
        ...
        if user_id is not None:
            if memcell_list is None:
                logger.error("[validate_and_filter_memcell_ids] user_id provided but memcell_list is None")
    ...
    def merge_memcell_ids(
        ...
        valid_new = self.validate_and_filter_memcell_ids(new, valid_ids, user_id=user_id, memcell_list=memcell_list)
        ...
        valid_new_sorted = sorted(
            valid_new,
            key=lambda mid: memcell_id_to_timestamp.get(mid, get_now_with_timezone().replace(year=1900)),
        )
        ...
        if existing_roles:
            for role, users in existing_roles.items():
                for user_info in users:
                    ...
                    if user_id and user_name and user_id not in ["not_found", "unknown"]:
        ...
        for memcell in memcell_list:
            ...
            if self.conversation_source == "original":
                ...
                if episode_text:
                    ...
                    logger.warning(f"No episode found for memcell {memcell_id}, using original_data as fallback")
                ...
                raise ValueError(f"Unsupported conversation_source: {self.conversation_source}")

        return "\n\n".join(all_conversation_text)

    def extract_existing_group_profile(self, old_memory_list: Optional[List]) -> Optional[Dict]:
        """
                    ...
                    for topic in existing_topics:
                        if hasattr(topic, "__dict__"):
                            ...
                            if isinstance(topic_dict.get("last_active_at"), datetime):
                                topic_dict["last_active_at"] = topic_dict["last_active_at"].isoformat()
    ...
    def get_user_name(self, user_id: str, speaker_mapping: Optional[Dict[str, Dict[str, str]]] = None) -> str:
