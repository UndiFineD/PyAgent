# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\plugins.py\init_f8b01946d184.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\__init__.py
        ...
        if item.is_dir() and not item.name.startswith("_"):
            ...
            if callbacks_file.exists():
                ...
                try:
                    ...
                    logger.warning(f"Failed to import callbacks from built-in plugin {plugin_name}: {e}")

                except Exception as e:
                    logger.error(f"Unexpected error loading built-in plugin {plugin_name}: {e}")
        ...
        if item.is_dir() and not item.name.startswith("_") and not item.name.startswith("."):
            ...
            if callbacks_file.exists():
                try:
                    ...
                    logger.warning(f"Failed to import callbacks from user plugin {plugin_name}: {e}")
