# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-robofinder\robofinder.py
import argparse
import datetime
import re
import signal
...
def get_all_links(args) -> list:
    logger(
        args.debug,
        "Sending an HTTP request to the archive to obtain all paths for robots.txt files.",
    )
    try:
        obj = requests.get(
            "https://web.archive.org/cdx/search/cdx?url={}/robots.txt&output=json&fl=timestamp,original&filter=statuscode:200&collapse=digest".format(
                args.url
            )
        ).json()
    except:
