# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\mongo.py\tenant_aware_mongo_client_7a0fb923d856.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\mongo\tenant_aware_mongo_client.py
...
class TenantAwareMongoClient(AsyncMongoClient):
    ...
    def _create_client_from_config(self, config: Dict[str, Any]) -> AsyncMongoClient:
        ...
        if uri:
            ...
            extra_kwargs = {k: v for k, v in config.items() if k not in ("uri", "database")}
        ...
        extra_kwargs = {
            k: v for k, v in config.items() if k not in ("host", "port", "username", "password", "database")
        }
    ...
    async def close(self):
        ...
        for cache_key, client in self._client_cache.items():
            try:
                ...
                logger.error("❌ Failed to close client [cache_key=%s]: %s", cache_key, e)
...
class TenantAwareDatabase(AsyncDatabase):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        logger.debug("🔍 Getting real MongoDB database object attribute or method: %s", name)
    ...
    def __repr__(self) -> str:
        ...
        return f"TenantAwareDatabase(client={self._tenant_aware_client}, name={self.name})"
