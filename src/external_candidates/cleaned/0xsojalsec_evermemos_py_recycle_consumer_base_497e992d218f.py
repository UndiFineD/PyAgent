# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\longjob.py\recycle_consumer_base_497e992d218f.py
...
class RecycleConsumerBase(LongJobInterface, ABC):
    ...
    def __init__(
        ...
        self._error_handler = self.consumer_config.error_handler or DefaultErrorHandler(self.logger)
    ...
    async def start(self) -> None:
        ...
        if self.status in [LongJobStatus.RUNNING, LongJobStatus.STARTING]:
            self.logger.warning("Consumer %s is already running or starting", self.job_id)
        ...
        try:
            ...
            self.logger.error("Failed to start consumer %s: %s", self.job_id, str(e), exc_info=True)

            raise

    async def shutdown(self, timeout: float = 30.0, wait_for_current_task: bool = True) -> None:
        """
        ...
        if self.status in [LongJobStatus.STOPPED, LongJobStatus.STOPPING]:
            self.logger.warning("Consumer %s is already stopped or stopping", self.job_id)
        ...
        if self._task and not self._task.done():
            if wait_for_current_task:
                try:
                    ...
                    self.logger.info("Current task completed gracefully in consumer %s", self.job_id)
                ...
                self.logger.info("Immediately cancelling task in consumer %s", self.job_id)
        ...
        try:
            ...
            self.logger.error("Error during cleanup: %s", str(cleanup_error), exc_info=True)
            ...
            try:
                ...
                try:
                    ...
                    if not should_continue:
                        self.logger.error("Error handler requested stop for consumer %s", self.job_id)
    ...
    async def _consume_messages(self) -> None:
        ...
        try:
            ...
            try:
                should_continue = await self._error_handler.handle_error(timeout_error, context)
    ...
    async def _process_single_message(self) -> None:
        ...
        for attempt in range(retry_config.max_retries + 1):
            try:
                ...
                if self._error_handler.is_fatal_error(e) and not retry_config.retry_on_fatal:
                    self.logger.error(
                ...
                if not self._error_handler.is_retryable_error(e) and not retry_config.retry_on_fatal:
                    self.logger.error("Non-retryable error in consumer %s: %s", self.job_id, str(e))
