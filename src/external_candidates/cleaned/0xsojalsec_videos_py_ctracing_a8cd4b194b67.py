# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2020.py\ctracing_a8cd4b194b67.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\ctracing.py
...
class ExplainTracing(Scene):
    def construct(self):
        ...
        self.play(words[1].to_edge, UP, FadeOut(words[0], 6 * UP))
        ...
        self.play(Write(implies), FadeIn(loc_tracking, UP))
...
class ContactTracingMisnomer(Scene):
    def construct(self):
        ...
        rects = VGroup(*[SurroundingRectangle(word, buff=0.2) for word in words])
            ...
            for p1 in sim.people:
                for p2 in sim.people:
                    ...
                    if dist < max_dist:
                        ...
                        line.set_stroke(interpolate_color(WHITE, RED, alpha), width=4 * alpha)
...
class WhatAboutPeopleWithoutPhones(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.change, "pondering", words) for pi in self.pi_creatures],
        )
...
class MoreLinks(Scene):
    def construct(self):
        ...
        arrows = VGroup(*[Vector(1.5 * DOWN, stroke_width=10) for x in range(4)])
