# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graph_r1.py\verl.py\workers.py\rollout.py\hf_rollout_6c73b7f17cb9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\workers\rollout\hf_rollout.py
...
class HFRollout(BaseRollout):
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        if hasattr(self.config, "custom_stop_words") and self.config.custom_stop_words and self.tokenizer:
            custom_criteria = CustomStopWordsCriteria(self.tokenizer, self.config.custom_stop_words, idx.device)
