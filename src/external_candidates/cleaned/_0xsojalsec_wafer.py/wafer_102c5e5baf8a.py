...
class WAFBypass:
    ...
    def __init__(self, opts) -> None:
        ...
        self.driver.execute_cdp_cmd("Page.addScriptToEvaluateOnNewDocument", {"source": self.code})
            ...
            for tag in self.unfiltered_tags:
                ...
                try:
                    for attr in TagSpecificAttributes[tag.name]:
                        ...
                        is403, _ = self.navigate(self.driver, url)
                        if not is403:
                            self.unfiltered_attributes["tag_specific"][tag.name].append(attr)
                except KeyError:
    ...
    def fuzz_thread(self, driver: webdriver.Chrome, started):
        driver.execute_cdp_cmd("Page.addScriptToEvaluateOnNewDocument", {"source": self.code})
...
def main():
    ...
    args.add_argument("--headless", help="Run in headless mode", action="store_true", default=False)
