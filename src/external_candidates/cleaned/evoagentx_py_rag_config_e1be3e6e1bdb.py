# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\rag.py\rag_config_e1be3e6e1bdb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\rag_config.py
...
class ReaderConfig(BaseModule):
    ...
    custom_metadata_function: Optional[Callable] = Field(
        default=None, description="Custom function to extract metadata from files."
    )

    extern_file_extractor: Optional[Dict[str, Any]] = Field(
        default=None, description="External file extractors for specific file types."
    )
...
class ChunkerConfig(BaseModule):
    ...
    strategy: str = Field(
        ...
        description="Chunking strategy (SIMPLE, SEMANTIC, HIERARCHICAL).",
    )
...
class EmbeddingConfig(BaseModule):
    ...
    provider: str = Field(
        ...
        description="Embedding provider (OPENAI, HUGGINGFACE, AZURE_OPENAI).",
    )
    ...
    normalize: Optional[bool] = Field(
        default=True,
        description="Whether to normalize the embedding model(huggingface).",
    )
...
class IndexConfig(BaseModule):
    ...
    index_type: str = Field(
        default=IndexType.VECTOR,
        description="Index type (VECTOR, GRAPH, SUMMARY, TREE).",
    )
...
class RetrievalConfig(BaseModule):
    ...
    postprocessor_type: str = Field(
        default=RerankerType.SIMPLE,
        description="The type of postprocessor for retrieve.",
    )

    top_k: int = Field(default=5, description="Number of top results to retrieve.")

    similarity_cutoff: Optional[float] = Field(
        default=0.7, description="Minimum similarity score for retrieved chunks."
    )
...
class RAGConfig(BaseConfig):
    ...
    modality: str = Field(
        default="text",
        description="RAG modality: 'text' for text-only, 'multimodal' for images.",
    )

    num_workers: Optional[int] = Field(
        default=None,
        description="Number of workers for parallel processing (e.g., reading, retrieval).",
    )
