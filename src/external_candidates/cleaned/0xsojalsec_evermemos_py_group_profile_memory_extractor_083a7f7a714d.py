# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\group_profile_memory_extractor_083a7f7a714d.py
...
def convert_to_datetime(timestamp, fallback_timestamp=None) -> datetime:
    ...
    if isinstance(timestamp, datetime):
        ...
        try:
            ...
            logger.exception(f"Failed to parse timestamp: {timestamp}, error: {e}, using fallback")
...
class TopicInfo:
    ...
    id: Optional[str] = None  # Unique topic ID (system-generated, LLM does not need to provide)

    update_type: Optional[str] = None  # "new" | "update" (only used during incremental updates)

    old_topic_id: Optional[str] = None  # Points to old topic during update (only used during incremental updates)

    evidences: Optional[List[str]] = field(default_factory=list)  # memcell_ids as evidence
...
class GroupProfileMemoryExtractor(MemoryExtractor):
    ...
    def llm_handler(self):
        ...
        if self._llm_handler is None:
            ...
            self._llm_handler = GroupProfileLLMHandler(self.llm_provider, self.max_topics)

        return self._llm_handler

    # ========== Business logic methods ==========

    def _filter_group(self, group_name: Optional[str], user_id_list: Optional[List[str]]) -> bool:
        """
    ...
    async def extract_memory(self, request: GroupProfileMemoryExtractRequest) -> Optional[List[GroupProfileMemory]]:
        """
        ...
        if self._filter_group(group_name, request.user_id_list):
            logger.info(f"[GroupProfileMemoryExtractor] Skipping group '{group_name}' - filtered out")

            return None

        # ===== 2. Extract historical profile and build conversation text =====

        existing_profile = self.data_processor.extract_existing_group_profile(request.old_memory_list)

        conversation_text = self.data_processor.combine_conversation_text_with_ids(memcell_list)
        ...
        try:
            ...
            logger.info(f"[GroupProfileMemoryExtractor] Executing parallel analysis for group: {group_name}")
            ...
            valid_memcell_ids = set(str(mc.event_id) for mc in memcell_list if hasattr(mc, "event_id") and mc.event_id)

            logger.debug(f"[extract_memory] Valid memcell IDs count: {len(valid_memcell_ids)}")
            ...
            logger.info(f"[extract_memory] Processed {len(all_topics)} topics (strong + weak)")
            ...
            comprehensive_mapping = self.data_processor.get_comprehensive_speaker_mapping(memcell_list, existing_roles)
            ...
            group_profile = GroupProfileMemory(
                ...
                ori_event_id_list=[str(mc.event_id) for mc in memcell_list if hasattr(mc, "event_id")],
                group_id=group_id,
                group_name=group_name,
            ...
            logger.error(f"[GroupProfileMemoryExtractor] Extraction error: {e}", exc_info=True)
