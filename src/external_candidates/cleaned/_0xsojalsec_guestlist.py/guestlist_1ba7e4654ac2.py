# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-guestlist\guestlist.py
#!/usr/bin/env python3
#
# GUESTLIST
...
class UrlChecker:
    ...
    def control_fireprox_endpoint(self, method, command):
        ...
        if command == "create":
            ...
            if method == "silent":
                ...
                self.current_endpoint = (
                    f"https://{extracted_id}.execute-api.{fireprox_region}.amazonaws.com/fireprox/common/oauth2/token"
                )
            ...
            print(
                "Renewing endpoints. This will take some time (60s). Pauses built in to handle rate limiting max of 2 api calls/minute"
            )
            result = fp.list_api()
            if debug:
                print(f"{result}")
    ...
    def nestoriLookup(self, username):
        ...
        body = {"username": f"{guest_username}", "isOtherIdpSupported": "True"}
        ...
        try:
            ...
            response = self.requests_retry_session().post(
                f"{self.current_endpoint}", headers=headers, json=body, timeout=10.0
            )
            if debug:
    ...
    def graphLookup(self, username):
        ...
        try:
            ...
            response = self.requests_retry_session().post(
                f"{self.current_endpoint}", headers=headers, data=body, timeout=10.0
            )
...
def lookup_tenant(domain):
    ...
    try:
        ...
        tenant_extract = [
            i for i, x in enumerate(domain_extract) if ".onmicrosoft.com" in x and ".mail.onmicrosoft.com" not in x
        ]  # this line gets the matching list item numbers only
        ...
        mail_extract = [
            i for i, x in enumerate(domain_extract) if ".mail.onmicrosoft.com" in x
        ]  # this line gets the matching list item numbers only
        ...
        if len(onedrive_list) > 0:
            ...
            if len(onedrive_list) == 1:
                ...
                if matching_mail:
                    if verbose:
                        print(
                            "INFO: Found matching mail record shared with onedrive URL. This is probably it. If you do not get results, re-run and manually choose a different tenant"
                        )
                    # print(matching_mail)
                    tenantname = matching_mail[0]
                else:
                    print(
                        "Could not reliably determine the primary domain. Try specifying different ones using the '-t' flag until you find it."
                    )
...
def main():
    ...
    parser.add_argument(
        "-m",
        "--method",
        help="silent or interactive (default:silent)",
        required=False,
        metavar="",
    )
    parser.add_argument(
        "-d",
        "--domain",
        help="target domain name (required)",
        required=False,
        metavar="",
    )
    parser.add_argument(
        "-t",
        "--tenant",
        help="tenant name if known, otherwise specify domain and will lookup",
        metavar="",
    )
    ...
    parser.add_argument(
        "-U",
        "--userfile",
        help="file containing usernames (wordlists) -- will also take a directory",
        metavar="",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="file to write output to (default: output.log)",
        default="output.log",
        metavar="",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-f",
        "--credfile",
        help="fireprox - file containing aws_secret and aws_key values",
        metavar="",
    )
