# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\seanrea.py\ridb_search.py\scripts.py\search_5ad12216432e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\seanrea\ridb-search\scripts\search.py
...
def geocode(location: str) -> tuple[float, float]:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "ridb-search/1.0 (camping availability checker)"})
...
def main():
    parser = argparse.ArgumentParser(description="Search RIDB for campgrounds near a location")

    parser.add_argument("--location", "-l", help="Location to search near (e.g., 'Bend, OR')")
    ...
    parser.add_argument("--camping-only", action="store_true", help="Filter to camping facilities only")
