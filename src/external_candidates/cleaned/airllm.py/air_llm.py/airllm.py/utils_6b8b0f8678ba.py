# Extracted from: C:\DEV\PyAgent\.external\airllm\air_llm\airllm\utils.py
import ctypes
import gc
import json
import os
...
def uncompress_layer_state_dict(layer_state_dict):
    ...
    if any(["4bit" in k for k in layer_state_dict.keys()]):
        ...
        for k, v in layer_state_dict.items():
            if "4bit" not in k:
                quant_state_dict = {
                    kk[len(k) :]: kv for kk, kv in layer_state_dict.items() if kk.startswith(k) and k != kk
                }
...
def split_and_save_layers(
    ...
    for layer in tqdm(layers):
        ...
        shards = [
            int(v.split("-")[1]) for k, v in index.items() if k.startswith(layer) and "-" in v and len(v.split("-")) > 1
        ]
        if len(shards) > 0:
            if max(shards) > shard:
...
def find_or_create_local_splitted_path(
    ...
    if os.path.exists(model_local_path_or_repo_id):
        if os.path.exists(Path(model_local_path_or_repo_id) / "pytorch_model.bin.index.json") or os.path.exists(
