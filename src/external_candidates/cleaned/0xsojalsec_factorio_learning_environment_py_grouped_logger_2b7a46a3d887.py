# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\mcts.py\grouped_logger_2b7a46a3d887.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\mcts\grouped_logger.py
...
class GroupedFactorioLogger:
    ...
    def __init__(self, n_groups: int, instances_per_group: int, base_port=27000, resume_version=0):
        ...
        for group_id in range(n_groups):
            ...
            for i in range(instances_per_group):
                ...
                group_instances[current_port] = InstanceMetrics(tcp_port=current_port, is_holdout=False)

                self.port_to_group[current_port] = group_id

                current_port += 1

            self.groups[group_id] = InstanceGroupMetrics(group_id=group_id, instances=group_instances)
    ...
    def _generate_instance_panel(self, instance: InstanceMetrics, group_id: int) -> Panel:
        ...
        right_table.add_row(
            "Inventory:",
            Text(f"{instance.start_inventory_count} → {instance.final_inventory_count}"),
        )
    ...
    def _generate_layout(self) -> Layout:
        ...
        if self.progress_task is not None:
            ...
            final_layout.split_column(Layout(main_layout, ratio=4), Layout(self.progress, ratio=1))
