# Extracted from: C:\DEV\PyAgent\.external\llama-cpp-python\llama_cpp\llama_grammar.py
...
class LlamaGrammar:
    ...
    def from_file(cls, file: Union[str, Path], verbose: bool = True) -> "LlamaGrammar":
        try:
            ...
            raise Exception(f"{cls.from_file.__name__}: error reading grammar file: {err}")

        if grammar:
            return cls.from_string(grammar, verbose=verbose)

        raise ValueError(f"{cls.from_file.__name__}: error parsing grammar file: params_grammer is empty")
...
class SchemaConverter:
    ...
    def _format_literal(self, literal):
        escaped = GRAMMAR_LITERAL_ESCAPE_RE.sub(lambda m: GRAMMAR_LITERAL_ESCAPES.get(m.group(0)), literal)
        return f'"{escaped}"'
    ...
    def visit(self, schema, name):
        ...
        if (ref := schema.get("$ref")) is not None:
            ...
            return self._add_rule(
                rule_name,
                self._build_object_rule(properties, required, hybrid_name, additional_properties=[]),
            )
