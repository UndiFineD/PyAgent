# Extracted from: C:\DEV\PyAgent\.external\llama-cpp-python\llama_cpp\server\types.py
...
max_tokens_field = Field(default=16, ge=1, description="The maximum number of tokens to generate.")
...
mirostat_eta_field = Field(default=0.1, ge=0.001, le=1.0, description="Mirostat learning rate")
...
class CreateCompletionRequest(BaseModel):
    prompt: Union[str, List[str]] = Field(default="", description="The prompt to generate completions for.")
    suffix: Optional[str] = Field(
        default=None,
        description="A suffix to append to the generated text. If None, no suffix is appended. Useful for chatbots.",
    )
    max_tokens: Optional[int] = Field(default=16, ge=0, description="The maximum number of tokens to generate.")
...
class TokenizeInputRequest(BaseModel):
    ...
    model_config = {"json_schema_extra": {"examples": [{"input": "How many tokens in this query?"}]}}
...
class DetokenizeInputResponse(BaseModel):
    ...
    model_config = {"json_schema_extra": {"example": {"text": "How many tokens in this query?"}}}
