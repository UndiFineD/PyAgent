# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\devops_scripts.py\data_fix.py\milvus_rebuild_collection_e6206de60085.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\devops_scripts\data_fix\milvus_rebuild_collection.py
...
def migrate_data_callback(old_collection: Collection, new_collection: Collection, batch_size: int = 3000) -> None:
    """
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.warning("Current pymilvus version does not support order_by parameter, using unordered query")

                logger.warning(
                    "It is recommended to upgrade pymilvus to version 2.4+, or create an STL_SORT index on the id field"
                )

                query_result = old_collection.query(expr="", output_fields=["*"], limit=batch_size, offset=offset)
...
def run(alias: str, drop_old: bool, migrate_data: bool, batch_size: int) -> None:
    ...
    try:
        ...
        if migrate_data:
            ...
            populate_fn = lambda old_col, new_col: migrate_data_callback(old_col, new_col, batch_size)

        else:
            populate_fn = None

        result = rebuild_collection(alias=alias, drop_old=drop_old, populate_fn=populate_fn)
...
Example usage:
  ...
  python milvus_rebuild_collection.py -a episodic_memory --drop-old
    ...
    parser.add_argument("--alias", "-a", required=True, help="Collection alias, e.g.: episodic_memory")
