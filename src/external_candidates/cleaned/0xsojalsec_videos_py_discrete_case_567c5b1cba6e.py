# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\diffyq.py\part3.py\discrete_case_567c5b1cba6e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part3\discrete_case.py
...
class ShowNewRuleAtDiscreteBoundary(DiscreteSetup):
    ...
    def show_boundary_point_influenced_by_neighbor(self):
        ...
        q_marks = VGroup(*[OldTex("?").scale(1.5).next_to(arrow, arrow.get_vector()) for arrow in arrows])

        arrows.set_color(YELLOW)

        q_marks.set_color(YELLOW)

        blocking_rects = VGroup(*[BackgroundRectangle(VGroup(*dots[i:-i], *self.rod_pieces[i:-i])) for i in [1, 2]])
        ...
        self.play(
            ...
            LaggedStart(*[FadeIn(q_mark, -arrow.get_vector()) for q_mark, arrow in zip(q_marks, arrows)]),
            run_time=1.5,
        )
    ...
    def update_dots(self, dots, dt):
        for ds in zip(dots, dots[1:], dots[2:]):
            ...
            self.update_dot(dot=ds[1], dt=dt, mean_diff=0.5 * (y2 - 2 * y1 + y0) / dx)
