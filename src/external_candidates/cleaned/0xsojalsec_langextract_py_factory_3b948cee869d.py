# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\factory_3b948cee869d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\factory.py
...
def _kwargs_with_environment_defaults(model_id: str, kwargs: dict[str, typing.Any]) -> dict[str, typing.Any]:
...
def create_model(
    ...
    kwargs = _kwargs_with_environment_defaults(model_id or config.provider or "", config.provider_kwargs)
    ...
    try:
        ...
        raise exceptions.InferenceConfigError(f"Failed to create provider {provider_class.__name__}: {e}") from e
...
def _create_model_with_schema(
    ...
    kwargs = _kwargs_with_environment_defaults(model_id or config.provider or "", kwargs)
