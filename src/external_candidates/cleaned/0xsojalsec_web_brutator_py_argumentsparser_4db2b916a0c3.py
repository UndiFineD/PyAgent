# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_web_brutator.py\lib.py\core.py\argumentsparser_4db2b916a0c3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-web-brutator\lib\core\ArgumentsParser.py
...
from lib.core.Utils import LineWrapRawTextHelpFormatter, Utils
...
class ArgumentsParser:
    ...
    def __init__(self):
        ...
        self.parser.add_argument("--target", type=self.check_arg_type, dest="type", help="Target type")

        self.parser.add_argument("-u", "--username", type=str, dest="username", help="Single username")

        self.parser.add_argument(
            "-U",
            "--userlist",
            type=self.check_arg_file,
            dest="userlist",
            help="Usernames list",
        )

        self.parser.add_argument("-p", "--password", type=str, dest="password", help="Single password")
        ...
        self.parser.add_argument(
            ...
            type=self.check_arg_file,
            dest="combolist",
            help="Combos username:password list",
        )

        self.parser.add_argument(
            "-t",
            "--threads",
            type=self.check_arg_threads,
            dest="threads",
    ...
    def check_arg_threads(self, threads):
        ...
        if threads < 1 or threads > MAX_THREADS:
            raise argparse.ArgumentTypeError("Number of threads must be <= {}".format(MAX_THREADS))
    ...
    def check_args(self):
        ...
        if self.args.list is False:
            ...
            if self.args.combolist is not None:
                if (
                    self.args.username is not None
                    or self.args.userlist is not None
                    or self.args.password is not None
                    or self.args.passlist is not None
                ):
                    self.parser.error("When combolist is used, -u, -U, -p and -P are not supported")

            if (
                self.args.username is None
                ...
                self.parser.error("Username/userlist + password/passlist or combolist required")
