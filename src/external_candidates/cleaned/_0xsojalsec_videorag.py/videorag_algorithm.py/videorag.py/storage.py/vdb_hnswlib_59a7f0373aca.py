# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoRAG\VideoRAG-algorithm\videorag\_storage\vdb_hnswlib.py
import asyncio
import os
import pickle
...
class HNSWVectorStorage(BaseVectorStorage):
    ...
    def __post_init__(self):
        self._index_file_name = os.path.join(self.global_config["working_dir"], f"{self.namespace}_hnsw.index")
    ...
    async def upsert(self, data: dict[str, dict]) -> np.ndarray:
        ...
        embeddings = np.concatenate(
            await asyncio.gather(
                *[self.embedding_func(contents[i : i + batch_size]) for i in range(0, len(contents), batch_size)]
            )
        )
