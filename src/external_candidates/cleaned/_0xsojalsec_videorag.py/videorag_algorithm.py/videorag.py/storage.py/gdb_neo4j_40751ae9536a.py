# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoRAG\VideoRAG-algorithm\videorag\_storage\gdb_neo4j.py
import asyncio
import json
...
class Neo4jStorage(BaseGraphStorage):
    ...
    async def upsert_node(self, node_id: str, node_data: dict[str, str]):
        ...
        async with self.async_driver.session() as session:
            await session.run(
                f"MERGE (n:{self.namespace}:{node_type} {{id: $node_id}}) SET n += $node_data",
                node_id=node_id,
                node_data=node_data,
            )
    ...
    async def clustering(self, algorithm: str):
        if algorithm != "leiden":
            raise ValueError(f"Clustering algorithm {algorithm} not supported in Neo4j implementation")
    ...
    async def _debug_delete_all_node_edges(self):
        async with self.async_driver.session() as session:
            try:
                ...
                logger.info(f"All nodes and edges in namespace '{self.namespace}' have been deleted.")
            except Exception as e:
