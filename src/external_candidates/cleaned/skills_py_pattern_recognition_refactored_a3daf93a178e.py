...
class PatternRecognition:
    ...
    def __init__(self, min_pattern_length: int = 10):
        ...
        self.chart_detector = ChartPatternDetector(min_pattern_length=min_pattern_length, similarity_threshold=0.02)

        self.candlestick_detector = CandlestickPatternDetector(doji_threshold=0.1, wick_body_ratio=2.0)

        self.sr_analyzer = SupportResistanceAnalyzer(clustering_threshold=0.02, min_distance=3, default_num_levels=3)

        self.trend_analyzer = TrendAnalyzer(short_term_period=10, medium_term_period=20, strength_period=14)

        self.volume_analyzer = VolumeAnalyzer(recent_period=10, high_volume_threshold=1.5, low_volume_threshold=0.7)
    ...
    def analyze_comprehensive(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if not trend.get("error"):
                logger.info(f"Trend: {trend['short_term']['direction']}, Strength: {trend['trend_strength']}")
            ...
            if not volume.get("error"):
                logger.info(
                    f"Volume: {volume['volume_status']}, OBV: {volume['obv_trend']}, VPT: {volume['vpt_trend']}"
                )
            ...
            if not regime.get("error"):
                logger.info(f"Regime: {regime['market_regime']}, Volatility: {regime['volatility_regime']}")
            ...
            logger.info(f"Analysis complete: {analysis['overall_bias']} with {analysis['confidence']}% confidence")
    ...
    def detect_all_patterns(self, df: pd.DataFrame) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(
                f"Detected {len(chart_patterns)} chart patterns, {len(candlestick_patterns)} candlestick patterns"
            )
