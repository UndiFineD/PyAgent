# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\brachistochrone.py\cycloid_9ed59c46c731.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\cycloid.py
...
class CycloidScene(Scene):
    ...
    def roll_back(self, run_time=3, *anims, **kwargs):
        ...
        self.play(
            ...
            ShowCreation(self.cycloid, rate_func=lambda t: smooth(1 - t), **kwargs),
            *anims,
        )
...
class IntroduceCycloid(CycloidScene):
    ...
    def get_q_marks_and_arrows(self, mob, n_marks=10):
        ...
        for x in range(n_marks):
            ...
            arrow = Arrow(start_point, q_point, color=BLUE_E)
...
class LeviSolution(CycloidScene):
    ...
    def show_pendulum(self, arc_angle=np.pi, arc_color=GREEN):
        ...
        right_angle_symbol = Mobject(Line(UP, UP + RIGHT), Line(UP + RIGHT, RIGHT))
        ...
        for start, angle in pairs:
            arc = Arc(angle=angle, radius=line_length, start_angle=start, color=GREEN)

            arc.shift(self.c_point)

            self.play(
                ShowCreation(arc),
                ApplyMethod(line.rotate, angle, path_func=path_along_arc(angle)),
                run_time=2,
            )
    ...
    def show_theta(self, radius=1):
        ...
        self.play(ShowCreation(arc), ShimmerIn(theta))
    ...
    def show_similar_triangles(self):
        ...
        triangle1 = Polygon(self.p_point, self.c_point, self.bottom_point, color=MAROON, **kwargs)

        triangle2 = Polygon(y_point, self.p_point, self.c_point, color=WHITE, **kwargs)
    ...
    def show_sin_thetas(self):
        ...
        for line, tex, scale in triplets:
            ...
            trig_mob.set_width(scale * line.get_length())
    ...
    def show_y(self):
        ...
        y_expression = OldTex(["D ", "\\sin", "^2", "(\\theta)"])
    ...
    def rearrange(self):
        ...
        one_over.shift((sin_over.get_bottom()[1] - one_over.get_bottom()[1]) * UP)

        self.play(
            Transform(equals, new_eq),
            ShimmerIn(sin_over),
            ShimmerIn(one_over),
            ApplyMethod(d_sqrt.next_to, one_over, DOWN, path_func=path_along_arc(-np.pi)),
            ApplyMethod(y_sqrt.next_to, sin_over, DOWN, path_func=path_along_arc(-np.pi)),
            run_time=2,
        )
        ...
        self.play(GrowFromCenter(brace), ShimmerIn(constant))
...
class SlidingObject(CycloidScene, PathSlidingScene):
    CONFIG = {"show_time": False, "wait_and_add": False}
    ...
    def construct(self, with_words):
        ...
        start_randy = self.adjust_mobject_to_index(randy.copy(), 1, self.cycloid.points)
...
class RotateWheel(CycloidScene):
    def construct(self):
        ...
        self.play(Rotating(self.circle, axis=OUT, run_time=5, rate_func=smooth))
