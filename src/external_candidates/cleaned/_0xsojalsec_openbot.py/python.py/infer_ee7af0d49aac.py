# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OpenBot\python\infer.py
import os
...
class InferNetwork:
    ...
    def predict_model(self, img_input, cmd_input):
        ...
        if self.inference_backend == "tf":
            ...
            self.interpreter.set_tensor(self.input_details[0]["index"], np.expand_dims(img_input, axis=0))
            self.interpreter.set_tensor(self.input_details[1]["index"], cmd_input)
            self.interpreter.invoke()

            output = self.interpreter.get_tensor(self.output_details[0]["index"])
        elif self.inference_backend == "openvino":
            output = self.interpreter([cmd_input, img_input[None, ...]])[self.output_layer]
    ...
    def run_debug(self):
        ...
        while True:
            ...
            cmd_input = np.array([self.cmd_dataset.iloc[self.counter]], dtype=np.float32)
