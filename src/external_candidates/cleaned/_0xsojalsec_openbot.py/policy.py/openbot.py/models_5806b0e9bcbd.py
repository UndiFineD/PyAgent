# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OpenBot\policy\openbot\models.py
...
def create_mlp(in_dim, hidden_dim, out_dim, activation="relu", dropout=0.2, name="cmd"):
    ...
    model.add(tf.keras.layers.Dense(hidden_dim, input_dim=in_dim, activation=activation, name=name))
    if dropout > 0:
