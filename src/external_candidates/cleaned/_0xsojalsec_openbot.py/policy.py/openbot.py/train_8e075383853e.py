# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OpenBot\policy\openbot\train.py
import argparse
import os
import re
import threading
...
class Hyperparameters:
    ...
    def __str__(self):
        model_name = "{0}_lr{1}_bz{2}".format(self.MODEL, self.LEARNING_RATE, self.TRAIN_BATCH_SIZE)
...
def load_tfrecord(tr: Training, verbose=0):
    ...
    tr.test_ds = test_dataset.batch(tr.hyperparameters.TEST_BATCH_SIZE).prefetch(AUTOTUNE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb", action="store_true", help="training logs with weights & biases")
    parser.add_argument(
        "--policy",
        type=str,
        default="autopilot",
