# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OpenBot\policy\openbot\server\api.py
import asyncio
import os
import shutil
import threading
...
async def handle_static(request: web.Request) -> web.StreamResponse:
    ...
    if os.path.isfile(real):
        return web.FileResponse(real, headers={"Cache-Control": "public, max-age=2592000"})
