# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OpenBot\policy\openbot\losses.py
...
def sq_weighted_mse_angle(y_true, y_pred):
    ...
    return tf.math.square(weight) * (
        tf.keras.losses.mean_squared_error(angle_true, angle_pred) + tf.keras.losses.mean_squared_error(y_true, y_pred)
    )
