# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OpenBot\policy\openbot\dataloader.py
    ...
    def __init__(self, data_dir: str, datasets: List[str], policy: str):
        ...
        if self.policy == "autopilot":
            ...
            self.label_values = tf.constant(
                [(float(label[0]) / 255.0, float(label[1]) / 255.0) for label in self.labels.values()]
            )
            self.cmd_values = tf.constant([(float(label[2])) for label in self.labels.values()])
        elif self.policy == "point_goal_nav":
            self.processed_frames_file_name = "matched_frame_ctrl_goal_processed.txt"
            self.labels = self.load_labels()
            self.index_table = self.lookup_table()
            self.label_values = tf.constant([(float(label[0]), float(label[1])) for label in self.labels.values()])
            self.cmd_values = tf.constant([(float(l[2]), float(l[3]), float(l[4])) for l in self.labels.values()])
        else:
            ...
            for folder in [f for f in os.listdir(os.path.join(self.data_dir, dataset)) if not f.startswith(".")]:
                ...
                if os.path.isfile(labels_file):
                    with open(labels_file) as f_input:
                        ...
                        data = f_input.read()
                        lines = (
                            data.replace(",", " ").replace("\\", "/").replace("\r", "").replace("\t", " ").split("\n")
                        )
                        data = [
