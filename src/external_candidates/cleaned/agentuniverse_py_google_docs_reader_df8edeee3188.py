# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentuniverse.py\agentuniverse.py\agent.py\action.py\knowledge.py\reader.py\cloud.py\google_docs_reader_df8edeee3188.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\cloud\google_docs_reader.py
...
class GoogleDocsReader(Reader):
    ...
    def _build_drive_service(self, ext_info: Optional[Dict]):
        try:
            ...
            raise ImportError(
                "Install Google API deps: `pip install google-api-python-client google-auth google-auth-oauthlib`"
            )
