# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-weblogicScanner\ws.py
#!/usr/bin/env python3
...
if __name__ == "__main__":
    ...
    for filename in os.listdir("./stars"):
        ...
        try:
            module = importlib.import_module(".{}".format(script_name), "stars")
    ...
    if args.output:
        with open(
            os.path.join(
                args.output,
                f"result_{time.strftime('%m%d_%H%M%S', time.localtime(time.time()))}.json",
            ),
            "w",
        ) as _f:
