# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-weblogicScanner\utils\process.py
import os
import random
import threading
import time
...
class AutoProcess:
    ...
    def __active_tasks_waiting(self):
        ...
        while True:
            ...
            if (
                self.auto_end
                and time.time() - (self.auto_end_last_time + self.auto_end_wait_time) > 0
                and len(self.tasks_waiting) == 0
                and len(self.tasks_running) == 0
    ...
    def put_task(
        ...
        args: List = None,
        kwargs: Dict = None,
        queue: Union[bool, SyncManager.Queue] = False,
    ) -> str:
        """
        提交待执行的任务，返回任务id
        func: 要多进程运行的函数
        ...
        if queue and isinstance(queue, bool):
            ...
            args.insert(0, queue)
        self.tasks_waiting.append(Process(target=func, args=args, kwargs=kwargs))
    ...
    def run(self):
        ...
        scan_interval: 扫描间隔
        """
        self.__thread_active_tasks_waiting = threading.Thread(target=self.__active_tasks_waiting)
        self.__thread_clear_tasks_complate = threading.Thread(target=self.__clear_tasks_complate)
...
if __name__ == "__main__":
    ...
    # 返回值获取测试
    [autopro.put_task(test_return_func, [100, 999], queue=True) for i in range(8)]
    autopro.wait()
