# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\agents.py\test_base_agent_run_mcp_5b28e6037f58.py
...
class TestBaseAgentRunMCP:
    ...
    async def test_run_with_mcp_with_link_attachments(self, agent):
        ...
        with patch.object(agent, "_code_generation_agent") as mock_agent:
            ...
            await agent.run_with_mcp("Review these links", link_attachments=[image_url, doc_url])
    ...
    async def test_run_with_mcp_with_dbos(self, mock_set_workflow_id, mock_use_dbos, agent):
    ...
    @patch("code_puppy.agents.base_agent.get_use_dbos", return_value=True)
    @patch("code_puppy.agents.base_agent.SetWorkflowID")
    async def test_run_with_mcp_with_dbos_and_mcp_servers(self, mock_set_workflow_id, mock_use_dbos, agent):
    ...
    @pytest.mark.asyncio
    @patch.object(CodePuppyAgent, "should_attempt_delayed_compaction", return_value=False)
    async def test_run_with_mcp_skips_compaction_when_not_needed(self, mock_should_compact, agent):
    ...
    @pytest.mark.asyncio
    @patch.object(CodePuppyAgent, "should_attempt_delayed_compaction", return_value=False)
    async def test_run_with_mcp_without_delayed_compaction(self, mock_should_compact, agent):
        """Test run_with_mcp skips compaction when not needed."""
    ...
    async def test_run_with_mcp_empty_attachments_list(self, agent):
        ...
        with patch.object(agent, "_code_generation_agent") as mock_agent:
            ...
            await agent.run_with_mcp("Empty attachments", attachments=[], link_attachments=[])
