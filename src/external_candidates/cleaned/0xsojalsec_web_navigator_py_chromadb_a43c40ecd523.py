# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_web_navigator.py\src.py\vectorstore.py\chromadb_a43c40ecd523.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Web-Navigator\src\vectorstore\chromadb.py
...
class ChromaDBVectorStore(BaseVectorStore):
    def __init__(
        ...
        path: Path = Path.cwd() / ".chroma",
    ):
        self.settings = Settings(
            anonymized_telemetry=False,
            is_persistent=True,
            persist_directory=path.as_posix(),
        )
