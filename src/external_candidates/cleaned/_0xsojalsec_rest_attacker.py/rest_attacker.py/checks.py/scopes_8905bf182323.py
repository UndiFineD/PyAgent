...
class CheckScopesEndpoint(TestCase):
    ...
    def __init__(self, check_id: int, request_info: RequestInfo, auth_info: AuthRequestInfo) -> None:
        """
            ...
            for path_id, path_item in descr["paths"]:
                ...
                for op_id, op in path_item.items():
                    ...
                    for server in descr["servers"]:
                        ...
                        test_cases.append(CheckScopesEndpoint(cur_check_id, auth_request, auth_info))
                        cur_check_id += 1
                ...
                for requirement in requirements:
                    ...
                    for scopename in requirement[requirement_name]:
                        if scopename not in scopemap.keys():
                            ...
                            logging.debug(
                                f"Scope {scopename} required for {op_id} / {path_id} not declared in security schemes."
                            )
                            scopemap[scopename] = []
...
class CompareTokenScopesToClientScopes(TestCase):
    ...
    def __init__(self, check_id: int, token: OAuth2Token, client_info: ClientInfo) -> None:
        """
    ...
    def generate(cls, config, check_id_start=0) -> list:
        ...
        for cred in config.credentials.values():
            if isinstance(cred, OAuth2TokenGenerator):
                ...
                test_cases.append(CompareTokenScopesToClientScopes(cur_check_id, token, cred.client_info))
                cur_check_id += 1
...
class TestTokenRequestScopeOmit(TestCase):
    ...
    def propose(self, config, check_id_start) -> list:
        ...
        if self.result.value["received_scopes"] is not None:
            test_cases.append(CompareTokenScopesToClientScopes(cur_check_id, self.token, self.token_gen.client_info))
            cur_check_id += 1
...
class TestTokenRequestScopeInvalid(TestCase):
    ...
    def run(self) -> None:
        ...
        try:
            self.token = self.token_gen.request_new_token(scopes=[scope], grant_type=self.grant_type)
...
class TestRefreshTokenRequestScopeOmit(TestCase):
    ...
    def propose(self, config, check_id_start) -> list:
        ...
        if self.result.value["received_scopes"] is not None:
            test_cases.append(
                CompareTokenScopesToClientScopes(cur_check_id, self.first_token, self.token_gen.client_info)
            )
            cur_check_id += 1
