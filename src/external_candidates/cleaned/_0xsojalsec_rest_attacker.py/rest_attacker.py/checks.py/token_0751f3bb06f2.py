...
class TestRefreshTokenRevocation(TestCase):
    ...
    def run(self) -> None:
        ...
        if not self.token:
            self.token = self.token_gen.request_new_token(grant_type="code", policy=AccessLevelPolicy.MAX)
...
class TestRefreshTokenClientBinding(TestCase):
    ...
    def run(self) -> None:
        ...
        if not self.token:
            self.token = self.token_gen0.request_new_token(grant_type="code", policy=AccessLevelPolicy.MAX)
        ...
        if len(oauth2_clients) > 1 and len(refreshable_clients) > 0:
            for refr_client in refreshable_clients:
                for other_client in oauth2_clients:
                    ...
                    test_cases.append(TestRefreshTokenClientBinding(cur_check_id, refr_client, other_client))
                    cur_check_id += 1
