            ...
            for server in descr["servers"]:
                for path_id, path in descr.endpoints.items():
                    for op_id, op in path.items():
                        ...
                        test_cases.append(TestObjectIDInvalidUserAccess(cur_check_id, request_info))
                        cur_check_id += 1

                        if config.auth:
                            auth_info = AuthRequestInfo(config.auth)
                            test_cases.append(TestObjectIDInvalidUserAccess(cur_check_id, request_info, auth_info))
                            cur_check_id += 1
...
class CountParameterRequiredRefs(TestCase):
    ...
    def propose(self, config, check_id_start):
        ...
        new_checks.append(FindParameterReturns(check_id_start, self.description, list(self.result.value.keys())))
...
class FindSecurityParameters(TestCase):
    ...
    def propose(self, config, check_id_start):
        ...
        for path_name, path_item in self.result.value["endpoints"].items():
            ...
            new_checks.append(GetParameters(check_id_start, new_request, new_auth_info, parameters=params))
