# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\body.py
                    ...
                    for op_id, op in path.items():
                        ...
                        for status_code, response in op["responses"].items():
                            ...
                            for schema in schemas:
                                ...
                                test_cases.append(
                                    CompareHTTPBodyToSchema(cur_check_id, request_info, schema, auth_info)
                                )
                                cur_check_id += 1
...
class CompareHTTPBodyAuthNonauth(TestCase):
    ...
    def run(self):
        ...
        common_values, unique_values_left, unique_values_right = _recursive_diff(response1_json, response2_json)
            ...
            for server in descr.servers:
                for path_id, path in descr.endpoints.items():
                    for op_id, op in path.items():
                        ...
                        test_cases.append(CompareHTTPBodyAuthNonauth(cur_check_id, request_info, auth_info))
                        cur_check_id += 1
            ...
            if key_left in right.keys():
                ...
                if value_left == value_right:
                    ...
                    if isinstance(value_left, dict):
                        # Dict comparison recurse
                        common, un_left, un_right = _recursive_diff_dict(value_left, value_right)
                        ...
                        # List comparison recurse
                        common, un_left, un_right = _recursive_diff_list(value_left, value_right)
                        common_values.update({key_left: common})
                        unique_values_left.update({key_left: un_left})
                        unique_values_right.update({key_left: un_right})
