# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\https.py
...
class TestHTTPSAvailable(TestCase):
    ...
    def run(self):
        ...
        if urlparse(self.request_info.url)[0] != "https":
            logging.info(f"Scheme of provided URL {self.request_info.url} is not HTTPS.")

            # Construct HTTPS URL if none was given
            self.request_info.url = ("https", *self.request_info._url[1:])
            logging.info(f"Using constructed URL {self.request_info.url} with HTTPS scheme.")
        ...
        if 300 <= response.status_code < 400:
            self.result.value.update({"redirect_url": response.headers["location"]})
            ...
            for server in descr["servers"]:
                for path_id, path in descr.endpoints.items():
                    for op_id, op in path.items():
                        nonauth_request = RequestInfo(server["url"], path_id, op_id, allow_redirects=False)
                        test_cases.append(TestHTTPSAvailable(cur_check_id, nonauth_request))
                        cur_check_id += 1

                        if config.auth:
                            auth_request = RequestInfo(server["url"], path_id, op_id, allow_redirects=False)
                            auth_info = AuthRequestInfo(config.auth, policy=AccessLevelPolicy.MAX)
                            test_cases.append(TestHTTPSAvailable(cur_check_id, auth_request, auth_info))
                            cur_check_id += 1
...
class TestHTTPAvailable(TestCase):
    ...
    def run(self):
        ...
        if urlparse(self.request_info.url)[0] != "http":
            logging.info(f"Scheme of provided URL {self.request_info.url} is not HTTP.")

            # Construct HTTPS URL if none was given
            self.request_info.url = ("http", *self.request_info._url[1:])
            logging.info(f"Using constructed URL {self.request_info.url} with HTTP scheme.")
                ...
                for path_id, path in descr.endpoints.items():
                    for op_id, op in path.items():
                        ...
                        if config.auth:
                            ...
                            test_cases.append(TestHTTPAvailable(cur_check_id, auth_request, auth_info))
                            cur_check_id += 1
...
class TestDescriptionURLs(TestCase):
    ...
    def propose(self, config, check_id_start):
        ...
        for http_url in self.result.value["http_urls"]:
            new_request = RequestInfo(http_url["url"], "/", "get")
            if config.auth:
                auth_info = AuthRequestInfo(config.auth, policy=AccessLevelPolicy.MAX)
            ...
            new_checks.append(TestHTTPAvailable(check_id_start, new_request, auth_info))

            check_id_start += 1

        for https_url in self.result.value["https_urls"]:
            new_request = RequestInfo(https_url["url"], "/", "get")
            if config.auth:
                auth_info = AuthRequestInfo(config.auth, policy=AccessLevelPolicy.MAX)
