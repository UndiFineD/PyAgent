# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\undocumented.py
...
class TestOptionsHTTPMethod(TestCase):
    ...
    def run(self):
        ...
        if "allow" in response.headers.keys():
            ...
            self.result.status = CheckStatus.ERROR
            self.result.error = Exception(
                f"Could not retrieve allowed methods for path {self.request_info.path} using OPTIONS method"
            )
            return
        ...
        for descr in config.descriptions.values():
            ...
            for path_id, path_item in descr.endpoints.items():
                ...
                for server_url in server_urls:
                    auth_request = RequestInfo(server_url, path_id, "options")
                    ...
                    test_cases.append(TestOptionsHTTPMethod(cur_check_id, auth_request, auth_info, claims=claims))
                    cur_check_id += 1
...
class MetaTestOptionsHTTPMethod(TestCase):
    ...
    def run(self):
        ...
        if any(check.result.status == CheckStatus.QUEUED for check in self.checks):
            self.result.status = CheckStatus.ERROR
            self.result.error = Exception(f"Cannot run meta check {self}. Some checks are not finished.")
            return
        ...
        for descr in config.descriptions.values():
            ...
            for path_id, path in descr.endpoints.items():
                ...
                for op in HTTP_REQUEST_METHODS:
                    ...
                    for server_url in server_urls:
                        auth_request = RequestInfo(server_url, path_id, op)
                        ...
                        test_cases.append(TestAllowedHTTPMethod(cur_check_id, auth_request, auth_info))
                        cur_check_id += 1
        ...
        for check in self.checks:
            if check.result.issue_type == IssueType.CANDIDATE:
                ...
                if path not in self.result.value["found_methods"]:
                    self.result.value["affected_paths"] += 1
                    self.result.value["found_methods"][path] = {"undocumented_methods": []}
