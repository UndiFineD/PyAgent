# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\auth\auth_generator.py
    ...
    def get_auth(
        ...
        if scheme_ids:
            ...
            for scheme_id in scheme_ids:
                auth_infos.append(self.get_auth_scheme(scheme_id=scheme_id, scopes=scopes, policy=policy))
    ...
    def get_auth_scheme(
        ...
        if scheme_id:
            ...
            if auth_type and not scheme.auth_type is auth_type:
                ...
                raise Exception(f"scheme '{scheme.scheme_id}' does not match auth type '{auth_type}'")

            if credentials_map and not scheme.supports_credentials(credentials_map.keys()):
                # Check if the scheme supports the credentials
                raise Exception(f"scheme '{scheme.scheme_id}' does not support credentials '{credentials_map.keys()}'")
            ...
            for sch in self.supported_schemes.values():
                ...
                raise Exception(f"Could not find scheme with auth type '{auth_type}'")
            ...
            for sch in self.supported_schemes.values():
                ...
                raise Exception(f"Could not find scheme that uses credentials '{credentials_map.keys()}'")

        else:
            raise Exception(
                "Generator cannot select scheme. Specify at least one of: scheme ID, auth type or credentials ID."
            )

        logging.debug(f"Generating auth info for scheme '{scheme.scheme_id}'")
        auth_type, auth_info = scheme.get_auth(credentials_map=credentials_map, scopes=scopes, access_policy=policy)
