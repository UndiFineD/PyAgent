# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\auth\token_generator.py
...
from rest_attacker.util.auth.session import (
    ROBrowserSession,
    ROCookieSession,
    ROWebSession,
)
...
class OAuth2TokenGenerator:
    ...
    def refresh_token(self, token: OAuth2Token, scopes: list[str] = None) -> OAuth2Token:
        ...
        session = OAuth2Session(client_id=self.client_info.client_id, scope=scopes, token=token)
        ...
        if user_session:
            ...
            if isinstance(user_session, ROBrowserSession):
                ...
                if RedirectHandler.called:
                    ...
                    logging.warning(
                        f"No redirect received after {server.timeout} seconds. Token could not be generated"
                    )
            ...
            cli_manual_request = f"Authorization required for OAuth2 Authorization Grant:\n{auth_url}"
