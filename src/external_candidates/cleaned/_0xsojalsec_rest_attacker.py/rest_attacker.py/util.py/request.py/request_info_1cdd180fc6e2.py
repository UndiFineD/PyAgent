...
class RequestInfo:
    ...
    def get_curl_command(self, auth_data: list[tuple[AuthType, dict]] = None) -> str:
        ...
        if len(self.headers) > 0:
            output += " ".join(
                f'-H "{header_id}: {header_payload}"' for header_id, header_payload in self.headers.items()
            )
            output += " "
        ...
        if len(self.cookies) > 0:
            output += " ".join(
                f'-b "{cookie_id}={cookie_payload}"' for cookie_id, cookie_payload in self.cookies.items()
            )
            output += " "
        ...
        # Query params
        if self.params:
            output += "?" + "&".join(f"{param_id}={param_payload}" for param_id, param_payload in self.params.items())
