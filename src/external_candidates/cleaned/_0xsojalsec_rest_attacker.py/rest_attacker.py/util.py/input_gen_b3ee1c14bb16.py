# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\input_gen.py
...
def replace_params(path: str, user_info: UserInfo, param_defs: dict[str, dict]) -> tuple[str, dict, dict, dict] | None:
    """
    ...
    for param_def in params:
        ...
        if required_schemas and param_id in required_schemas.keys():
            ...
            try:
                ...
                logging.info(f"Parameter '{param_id}' does not conform to requested schema.")

            except jsonschema.SchemaError:
                # Continue if schema is not correct but log error
                logging.info(f"Requested schema for parameter '{param_id}' is invalid.")
...
def replace_http_params(defined_params: dict[str, list[str]], param_defs: dict[str, dict]) -> tuple[dict, dict, dict]:
    """
