# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\parsers\config_run.py
...
def load_config(
    ...
    for check_def in check_defs:
        ...
        if check:
            checks.append(check)
            logging.debug(f"Configured check for test case '{check_def['test_case']}' loaded.")

        else:
            logging.info(f"Check '{check_def['check_id']}' coild not be loaded. No deserialization avaliable")
