...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser("REST-Attacker", description=("Pentesting tool for analyzing REST APIs"))

    parser.add_argument(
        "config",
        default=None,
        help=("Path to the service configuration. Can be a directory or OpenAPI file."),
    )

    parser.add_argument("--version", nargs=0, action=GetVersion, help="Print version number.")
    parser.add_argument(
        "--list",
        "-l",
        nargs=0,
    ...
    parser.add_argument("--output-dir", default=None, help="Export path for logs and reports.")
    ...
    parser.add_argument(
        ...
        default=3,
        choices={1, 2, 3, 4, 5},
        help=("Set the loglevel for the CLI. Choices map to Python loglevels. Logging to file is always level 5."),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Generate fake input parameter values when generating checks. WARNING: May result in destructive behavior."
        ),
    )

    run_cfg = parser.add_mutually_exclusive_group()
    run_cfg.add_argument("--run", default=None, help="Start test run from a run configuration file.")
    ...
    proxy.add_argument("--cacert", default=None, type=str, help="Path to a custom CA certificate.")
...
def setup_outputs(args: argparse.Namespace) -> None:
    ...
    if args.output_dir:
        ...
        out_folder = datetime.utcfromtimestamp(time.time()).strftime("%Y-%m-%dT%H-%M-%SZ")
...
def setup_config(args: argparse.Namespace) -> EngineConfig:
    ...
    if not args.config_path.exists():
        ...
        args.config_path = pathlib.Path().resolve() / "rest_attacker" / "cfg" / args.config
...
def setup_run(cfg: EngineConfig, args: argparse.Namespace) -> list[TestCase]:
    ...
    if args.test_type:
        test_filters.update({"test_type": [TestCaseType[test_type] for test_type in args.test_type]})
    if args.auth_type:
        test_filters.update({"auth_type": [AuthType[auth_type] for auth_type in args.auth_type]})
    if args.live_type:
        test_filters.update({"live_type": [LiveType[live_type] for live_type in args.live_type]})
    ...
    if args.run:
        ...
        logging.warning("Use --run to specify run configuration or --generate to automatically generate checks.")
    ...
    if args.handle_limits:
        ...
        if cfg.current_user_id:
            ...
            if default_user.userinfo_endpoint:
                ...
                handlers.append(AccessLimitHandler(request_info, auth_info, interval=20))
...
def main():
    ...
    try:
        ...
        logging.exception("Exporting results failed with the following error:", exc_info=e)
