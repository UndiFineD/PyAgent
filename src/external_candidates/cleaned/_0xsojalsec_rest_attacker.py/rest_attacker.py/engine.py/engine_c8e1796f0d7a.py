...
class Engine:
    ...
    def current_check(self) -> None:
        ...
        """
        logging.debug(
            f"Starting: Check {self.checks[self.index].check_id} ({self.checks[self.index].get_test_case_id()})."
        )
        ...
        try:
            ...
            logging.warning((f"Check {current_check.check_id} did not execute: API operation is restricted"))
        ...
        if current_check.live_type is LiveType.ONLINE:
            if self.state.rate_limit:
                ...
                if rl_limit_reached:
                    ...
                    logging.warning(f"Rate limit reached: Next check possible in {reset_time} seconds.")
            ...
            if self.state.access_limit:
                ...
                if self.state.access_limit.current_pos >= self.state.access_limit.interval:
                    ...
                    if acc_limit_reached:
                        ...
                        while (
                            self.checks[self.index].check_id != self.state.access_limit.last_check_id
                            and self.index > -1
                        ):
                            # Clear test result
