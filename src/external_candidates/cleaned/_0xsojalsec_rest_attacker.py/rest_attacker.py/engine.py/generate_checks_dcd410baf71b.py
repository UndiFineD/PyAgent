# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\engine\generate_checks.py
...
def generate_checks(config: EngineConfig, test_cases: dict[str, TestCase], filters: dict = None) -> list[TestCase]:
    """
    ...
    if filters:
        ...
        if "test_cases" in filters.keys():
            ...
            for allowed_type in filters["test_cases"]:
                if allowed_type not in test_cases.keys():
                    raise Exception(f"Could not find test case with ID '{allowed_type}' in available test cases.")
        ...
        for test_case_id, test_case in test_cases.items():
            ...
            if allowed:
                ...
                filtered_test_cases.update({test_case_id: test_case})
