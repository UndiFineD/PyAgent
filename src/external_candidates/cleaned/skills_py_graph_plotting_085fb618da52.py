# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\inclinedadarsh.py\manimce_best_practices.py\examples.py\graph_plotting_085fb618da52.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\inclinedadarsh\manimce-best-practices\examples\graph_plotting.py
...
class AnimatedGraph(Scene):
    ...
    def construct(self):
        ...
        graph = always_redraw(
            lambda: axes.plot(lambda x: amplitude.get_value() * np.sin(x), color=BLUE, x_range=[-3, 3])
        )
...
class PolarPlot(Scene):
    ...
    def construct(self):
        ...
        cardioid = polar_plane.plot_polar_graph(lambda theta: 1 + np.sin(theta), theta_range=[0, 2 * PI], color=BLUE)

        # Rose curve: r = 2*cos(3*theta)

        rose = polar_plane.plot_polar_graph(lambda theta: 2 * np.cos(3 * theta), theta_range=[0, PI], color=RED)
