# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alirezarezvani.py\app_store_optimization.py\scripts.py\aso_scorer_4726ada7435d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\app-store-optimization\scripts\aso_scorer.py
...
class ASOScorer:
    ...
    def calculate_overall_score(
        ...
        recommendations = self.generate_recommendations(metadata_score, ratings_score, keyword_score, conversion_score)
    ...
    def _identify_weaknesses(self, score_breakdown: Dict[str, Any]) -> List[str]:
        ...
        for category, data in score_breakdown.items():
            if data["score"] < 60:
                weaknesses.append(f"{category.replace('_', ' ').title()}: {data['score']}/100 - needs improvement")
