# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\clacks.py\solution2.py\pi_creature_scenes_464b0c94cf20.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\solution2\pi_creature_scenes.py
...
class OnAnsweringTwice(TeacherStudentsScene):
    def construct(self):
        ...
        for oq in other_questions:
            self.play(
                shown_questions.shift,
                0.85 * UP,
                FadeInFromDown(oq),
                self.change_students(*["pondering"] * 3, look_at=oq),
            )
...
class NowForTheGoodPart(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            target_mode="hooray",
            added_anims=[self.change_students("hooray", "surprised", "happy")],
        )
