# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ldapper\test\test_ldapnode.py
# -*- coding: utf-8 -*-
...
conn = connection.connect(logindn="cn=admin,dc=acme,dc=org", password="JonSn0w")
...
class TestLDAPNode:
    ...
    def test_non_system_fields(self):
        assert sorted(["addresses", "fullname", "lastname", "uid"]) == sorted(Person._non_system_fields.keys())
        assert sorted(["date_created", "date_modified", "user_created", "user_modified"]) == sorted(
            Person._system_fields.keys()
        )
