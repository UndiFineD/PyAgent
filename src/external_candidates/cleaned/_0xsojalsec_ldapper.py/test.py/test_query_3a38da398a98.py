# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ldapper\test\test_query.py
# -*- coding: utf-8 -*-
...
class TestQ:
    ...
    def test_complex_conditions2(self):
        qset = (Q(firstname="Alice") | Q(firstname="Bob")) & (Q(uid="liam") | Q(uid="derek"))
        assert qset.compile(Person) == "(&(|(givenName=Alice)(givenName=Bob))(|(uid=liam)(uid=derek)))"

    def test_complex_conditions3(self):
        # Test that And() & And() doesn't nest more than it needs to.
        # Same goes for Or() | Or().
        qset = (Q(lastname="Lennon") & Q(lastname="McCartney")) & (Q(lastname="Harrison") & Q(lastname="Starr"))
        assert qset.compile(Person) == "(&(sn=Lennon)(sn=McCartney)(sn=Harrison)(sn=Starr))"

        qset = (Q(lastname="Lennon") | Q(lastname="McCartney")) | (Q(lastname="Harrison") | Q(lastname="Starr"))
        assert qset.compile(Person) == "(|(sn=Lennon)(sn=McCartney)(sn=Harrison)(sn=Starr))"

    def test_complex_conditions4(self):
        qset = (Q(lastname="Lennon") & Q(lastname="McCartney")) & Q(firstname="Ringo", lastname="Starr")
        assert qset.compile(Person) == "(&(sn=Lennon)(sn=McCartney)(givenName=Ringo)(sn=Starr))"

        qset = (Q(lastname="Lennon") | Q(lastname="McCartney")) | Q(firstname="Ringo", lastname="Starr")
        assert qset.compile(Person) == "(|(sn=Lennon)(sn=McCartney)(&(givenName=Ringo)(sn=Starr)))"

    def test_deeply_nested(self):
        qset = Q(firstname="Liam") & (Q(lastname="Smith") | (Q(uid="liam") & Q(lastname="Monahan")))
        assert qset.compile(Person) == "(&(givenName=Liam)(|(sn=Smith)(&(uid=liam)(sn=Monahan))))"
    ...
    def test_deeply_nested2(self):
        ...
        assert qset.compile(Person) == "(|(givenName=Liam)(sn=Smith)(uid=liam))"

    def test_deeply_nested3(self):
        qset = Q(uid="liam") & Q(lastname="Monahan") & Q(firstname="Liam")
        assert qset.compile(Person) == "(&(uid=liam)(sn=Monahan)(givenName=Liam))"

        qset = Q(uid="liam") | Q(lastname="Monahan") | Q(firstname="Liam")
        assert qset.compile(Person) == "(|(uid=liam)(sn=Monahan)(givenName=Liam))"

        qset = Q(firstname="Liam") | (
            Q(lastname="Smith") & (Q(uid="liam") & Q(lastname="Monahan") & Q(firstname="Liam"))
        )
        assert qset.compile(Person) == "(|(givenName=Liam)(&(sn=Smith)(uid=liam)(sn=Monahan)(givenName=Liam)))"
