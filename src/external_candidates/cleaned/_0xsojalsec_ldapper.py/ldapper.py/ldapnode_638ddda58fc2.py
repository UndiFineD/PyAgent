# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ldapper\ldapper\ldapnode.py
import datetime
import logging
...
from ldapper.fields import BinaryField, Field
...
class LDAPNode(object, metaclass=LDAPNodeBase):
    ...
    def _fetch_entry(cls, primary=None, dnprefix=None, **kwargs):
        ...
        if dnprefix is None:
            if kwargs:
                try:
                    ...
                    logging.warning(
                        "Unable to fetch %s %s. Missing keyword" % (cls.hrn, primary)
        ...
        filter = "(&(%s=%s)%s)" % (
            cls._primary_field.ldap,
            primary,
            cls.objectclass_filter(),
        )
        ...
        try:
            ...
            logging.warning("Unable to fetch %s %s.  Bad LDAP search filter." % (cls.hrn, primary))
            return None
        ...
        if result is None:
            ...
            logging.warning("Found %d results.  Unable to fetch %s %s." % (len(result), cls.hrn, primary))
            return None
    ...
    def fetch(cls, primary=None, dnprefix=None, **kwargs):
        ...
        if result is not None:
            ...
            logging.debug("Loaded %s %s Successfully" % (obj.__class__.__name__, obj.dnattr()))
            return obj
        else:
            return None
    ...
    def fetch_by_dn(cls, dn, **kwargs):
        ...
        try:
            ...
            return None
        objects = cls.list(filter=filter, dnprefix=dnprefix, max_results=1, **kwargs)
        try:
            return objects[0]
        except IndexError:
            return None
    ...
    def list(
        ...
        **kwargs,
        ...
        if search_prefix:
            filter = "(&%s%s)" % (
                filter,
                cls.searchable_fields_search_prefix_filter(search_prefix=search_prefix),
            )

        if search_string:
            filter = "(&%s%s)" % (
                filter,
                cls.searchable_fields_search_string_filter(search_string=search_string),
            )
        ...
        if max_results:
            ...
            for dn, entry in results:
                if num_results_processed < max_results:
                    ...
                    logging.debug("Loaded %s %s Successfully" % (o.__class__.__name__, o.dnattr()))
                    objs.append(o)
    ...
    @classmethod
    def _searchable_fields_filter(cls, search_term, template, searchable_fields=None):
        """
    ...
    @classmethod
    def searchable_fields_search_prefix_filter(cls, search_prefix, searchable_fields=None):
    ...
    @classmethod
    def searchable_fields_search_string_filter(cls, search_string, searchable_fields=None):
    ...
    def log__did_not_validate(self):
        ...
        self.logger.error("%s %s did not pass validation." % (self.hrn, self.dnattr()))

    def log__add_success(self):
        """Log a successful add of the current LDAPNode"""
        self.logger.info("%s %s was added." % (self.hrn, self.dnattr()))

    def log__add_failure(self):
        """Log an unsuccessful add of the current LDAPNode"""
        self.logger.error("Could not add %s %s." % (self.hrn, self.dnattr()))
    ...
    def log__modify_attr_success(self, attr_name, old_val, new_val):
        ...
        if isinstance(old_val, list) and isinstance(new_val, list):
            ...
            if added:
                ...
                self.logger.info("Added %s %s to %s %s" % (name, added, self.hrn, self.dnattr()))
            if removed:
                name = inflect_given_cardinality(attr_name, len(removed))
                removed = list_items_to_sentence(removed)
                self.logger.info("Removed %s %s from %s %s" % (name, removed, self.hrn, self.dnattr()))
        else:
            self.logger.info(
    ...
    def log__delete_success(self):
        ...
        self.logger.info("%s %s was removed successfully." % (self.hrn, self.dnattr()))

    def log__delete_failure(self):
        """Log an unsuccessful removal of an LDAPNode from the LDAP"""
        self.logger.error("%s %s was not removed successfully." % (self.hrn, self.dnattr()))

    def log__delete_attr_success(self, attr_name):
        """Log a successful deletion of a field on the current LDAPNode"""
        self.logger.info("Successfully removed %s attribute from %s %s" % (attr_name, self.hrn, self.dnattr()))

    def log__delete_attr_failure(self, attr_name):
        """Log an unsuccessful deletion of a field on the current LDAPNode"""
        self.logger.error("Failed to remove %s attribute from %s %s" % (attr_name, self.hrn, self.dnattr()))

    @classmethod
    def objectclass_filter(cls):
        """Return a string LDAP filter from the objectClass attributes"""
        f = build_ldap_filter(op="&", attrname="objectClass", items=cls._meta.objectclasses)
        if cls._meta.excluded_objectclasses:
            n = build_ldap_filter(op="!", attrname="objectClass", items=cls._meta.excluded_objectclasses)
            f = "(&%s%s)" % (f, n)
        return f
