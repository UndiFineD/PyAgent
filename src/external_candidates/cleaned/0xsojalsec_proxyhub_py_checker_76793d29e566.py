# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_proxyhub.py\proxyhub.py\checker_76793d29e566.py
...
class Checker:
    ...
    def __init__(
        ...
        self._req_http_proto = not types or bool(("HTTP", "CONNECT:80", "SOCKS4", "SOCKS5") & types.keys())
    ...
    async def check_judges(self):
        ...
        await asyncio.gather(*[j.check(real_ext_ip=self._real_ext_ip) for j in self._judges])

        self._judges = [j for j in self._judges if j.is_working]

        log.debug("%d judges added. Runtime: %.4f;" % (len(self._judges), time.time() - stime))
...
def _get_anonymity_lvl(real_ext_ip, proxy, judge, content):
    ...
    via = (content.count("via") > judge.marks["via"]) or (content.count("proxy") > judge.marks["proxy"])
