# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\src.py\datasets.py\text_causal_language_modeling_ds_fd84705ac7e1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\datasets\text_causal_language_modeling_ds.py
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        input_text_dict["systems"] = [self.parse_system(self.cfg, system) for system in input_text_dict["systems"]]

        input_text_dict["prompts"] = [self.parse_prompt(self.cfg, prompt) for prompt in input_text_dict["prompts"]]

        input_text_dict["answers"] = [self.parse_answer(self.cfg, answer) for answer in input_text_dict["answers"]]

        sample = dict()

        system_encoding, prompt_encodings, answer_encodings = self.get_encodings(input_text_dict=input_text_dict)

        input_ids = torch.cat(
            [
                torch.cat([prompt_encoding, answer_encoding])
                for prompt_encoding, answer_encoding in zip(prompt_encodings, answer_encodings, strict=False)
            ]
        )
    ...
    def parse_prompt(cfg: Any, prompt: str):
        prompt = f"{codecs.decode(cfg.dataset.text_prompt_start, 'unicode_escape')}{prompt}"
    ...
    def parse_system(cfg: Any, system: str):
        ...
        system = f"{codecs.decode(cfg.dataset.text_system_start, 'unicode_escape')}{system}"
    ...
    @staticmethod
    def batch_to_device(batch: dict | list | torch.Tensor, device: str) -> dict | list | torch.Tensor | str:
        ...
        if isinstance(batch, torch.Tensor):
            ...
            return {key: CustomDataset.batch_to_device(value, device) for key, value in batch.items()}
    ...
    def preprocess_dataframe(df: pd.DataFrame, cfg: Any) -> pd.DataFrame:
        ...
        if cfg.dataset.personalize:
            ...
            df[cfg.dataset.answer_column] = df[cfg.dataset.answer_column].apply(personalize)
    ...
    def postprocess_batch_predictions(self, output: dict) -> dict:
        if "predicted_answer_ids" in output.keys():
            predicted_text = [
                self.tokenizer.decode(ids, skip_special_tokens=True).strip() for ids in output["predicted_answer_ids"]
            ]
    ...
    def format_output(self, cfg, df: pd.DataFrame, output: dict) -> tuple[dict, pd.DataFrame]:
        output = {key: value for key, value in output.items() if key not in ["loss", "target", "losses"]}

        output.pop("target_text", None)

        # in case limit_chained_samples is True, only last answer is predicted

        end_conversation_ids = self.conversation_chain_handler.get_conversation_end_ids()
        ...
        if isinstance(cfg.dataset.prompt_column, tuple):
            ...
            output[cfg.dataset.prompt_column] = df.loc[end_conversation_ids, cfg.dataset.prompt_column].values
        ...
        if "predicted_text" in output.keys():
            ...
            df[f"pred_{col_name}"] = "NO ANSWER GENERATED. ONLY LAST ANSWER OF A CONVERSATION IS PREDICTED."
    ...
    def sanity_check(cls, df: pd.DataFrame, cfg: Any, mode: str = "train"):
        ...
        if (
            ...
            assert (df[cfg.dataset.parent_id_column] != df[cfg.dataset.id_column]).all(), (
                f"Parent id column:{cfg.dataset.parent_id_column} is the same as id column for some rows"
            )
        ...
        assert cfg.dataset.answer_column in df.columns, (
            f"Answer column {cfg.dataset.answer_column} not found in the {mode} DataFrame."
        )

        assert df.shape[0] == df[[cfg.dataset.answer_column]].dropna().shape[0], (
            f"The {mode} DataFrame column {cfg.dataset.answer_column} contains missing values."
        )
        ...
        if cfg.dataset.parent_id_column != "None" and df[cfg.dataset.parent_id_column].notna().any():
            ...
            if not (None in parent_id_list or pd.isna(parent_id_list).any()):
                raise AssertionError(
                    "There must be at least one root element (with no parent). Currently, all records have a parent."
                )
            ...
            def find_ancestor(node, parent_map):
                ...
                while node in parent_map:
                    if node in seen:
                        raise AssertionError(f"Circular reference detected for ID '{node}'.")
            ...
            parent_map = {id_list[i]: parent_id_list[i] for i in range(len(id_list)) if parent_id_list[i] is not None}
        ...
        if np.random.random() < self.cfg.augmentation.random_parent_probability:
            ...
            parent_encodings = [
                self._get_sample_encoding(
                    self.parse_system(self.cfg, self.conversation_chain_handler.systems[idx]),
                    self.parse_prompt(self.cfg, self.conversation_chain_handler.prompts[idx]),
    ...
    def _get_sample_encoding(self, system: str, prompt: str, answer: str) -> list:
        if len(system) > 0:
            system_encoding = self.encode(self.tokenizer, system, self.cfg.tokenizer.max_length, "right")["input_ids"]

        else:
            system_encoding = torch.empty(0)

        prompt_encoding = self.encode(self.tokenizer, prompt, self.cfg.tokenizer.max_length, "left")["input_ids"]

        answer_encoding = self.encode(self.tokenizer, answer, self.cfg.tokenizer.max_length, "right")["input_ids"]
