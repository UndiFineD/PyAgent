# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\oxm.py\milvus.py\migration.py\utils_2005d4162052.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\milvus\migration\utils.py
...
def find_collection_manager_by_alias(alias: str) -> Type[MilvusCollectionBase]:
    ...
    for doc_class in all_doc_classes:
        ...
        if not hasattr(doc_class, "_COLLECTION_NAME") or doc_class._COLLECTION_NAME is None:
            continue
...
def rebuild_collection(
    ...
    logger.info("Starting to rebuild Collection: alias=%s, drop_old=%s", alias, drop_old)
    ...
    if issubclass(collection_class, MilvusCollectionWithSuffix):
        ...
        raise NotImplementedError("Unsupported collection type: %s", collection_class.__name__)
