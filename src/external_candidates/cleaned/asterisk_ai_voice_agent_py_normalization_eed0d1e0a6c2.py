# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\config.py\normalization_eed0d1e0a6c2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\config\normalization.py
...
def normalize_pipelines(config_data: Dict[str, Any]) -> None:
    ...
    for pipeline_name, raw_entry in pipelines_cfg.items():
        ...
        raise TypeError(
            ...
            f"Unsupported pipeline definition for '{pipeline_name}': {type(raw_entry).__name__}"
...
def normalize_profiles(config_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        profiles_block = (
            ...
            (config_data.get("profiles") or {})

            if isinstance(config_data.get("profiles"), dict)
        ...
        if isinstance(local_block, dict):
            ...
            try:
                ...
                if isinstance(local_block.get("connect_timeout_sec"), str):
                    ...
                    local_block["connect_timeout_sec"] = float(

                        local_block["connect_timeout_sec"]
            ...
            try:
                ...
                if isinstance(local_block.get("response_timeout_sec"), str):
                    ...
                    local_block["response_timeout_sec"] = float(

                        local_block["response_timeout_sec"]
