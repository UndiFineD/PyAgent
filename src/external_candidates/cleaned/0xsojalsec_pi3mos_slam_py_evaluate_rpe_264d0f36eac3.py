# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_pi3mos_slam.py\evaluation.py\evaluate_rpe_264d0f36eac3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\evaluation\evaluate_rpe.py
    ...
    if not param_fixed_delta:
        if param_max_pairs == 0 or len(traj_est) < np.sqrt(param_max_pairs):
            pairs = [
                ...
                for j in range(len(traj_est))  # pylint: disable=g-complex-comprehension
            ]
