# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_offline.py\src.py\second_brain_offline.py\application.py\agents.py\quality_753be78eee6a.py
...
"""
    ...
    async def __get_quality_score_batch(self, documents: list[Document]) -> list[Document]:
        ...
        documents_with_scores = [doc for doc in scored_documents if doc.content_quality_score is not None]

        documents_without_scores = [doc for doc in scored_documents if doc.content_quality_score is None]

        # Retry failed documents with increased await time, as most failures are due to rate limiting.

        if documents_without_scores:
            logger.info(f"Retrying {len(documents_without_scores)} failed documents with increased await time...")

            retry_results = await self.__process_batch(documents_without_scores, await_time_seconds=20)
        ...
        success_count = len([doc for doc in scored_documents if hasattr(doc, "quality_score")])
    ...
    async def __process_batch(self, documents: list[Document], await_time_seconds: int) -> list[Document]:
        semaphore = asyncio.Semaphore(self.max_concurrent_requests)

        tasks = [
            self.__get_quality_score(document, semaphore, await_time_seconds=await_time_seconds)
    ...
    async def __get_quality_score(
        ...
        async def process_document() -> Document:
            ...
            try:
                input_user_prompt = utils.clip_tokens(input_user_prompt, max_tokens=8192, model_id=self.model_id)

            except Exception as e:
                logger.warning(f"Failed to clip tokens for document {document.id}: {str(e)}")
