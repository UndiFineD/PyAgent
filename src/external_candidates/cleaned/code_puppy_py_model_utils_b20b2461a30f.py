# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\model_utils_b20b2461a30f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\model_utils.py
...
_ANTIGRAVITY_PROMPT_PATH = pathlib.Path(__file__).parent / "prompts" / "antigravity_system_prompt.md"
...
def _load_antigravity_prompt() -> str:
    ...
    if _antigravity_prompt_cache is None:
        if _ANTIGRAVITY_PROMPT_PATH.exists():
            _antigravity_prompt_cache = _ANTIGRAVITY_PROMPT_PATH.read_text(encoding="utf-8")
...
def prepare_prompt_for_model(
    ...
    results = callbacks.on_get_model_system_prompt(model_name, system_prompt, user_prompt)
