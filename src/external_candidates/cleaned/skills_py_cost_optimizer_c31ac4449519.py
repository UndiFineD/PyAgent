# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alirezarezvani.py\aws_solution_architect.py\scripts.py\cost_optimizer_c31ac4449519.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\aws-solution-architect\scripts\cost_optimizer.py
...
class CostOptimizer:
    ...
    def analyze_and_optimize(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "savings_percentage": (
                round((potential_savings / self.monthly_spend) * 100, 2) if self.monthly_spend > 0 else 0
            ),
    ...
    def _analyze_compute(self) -> float:
        ...
        if lambda_functions:
            oversized = sum(
                1 for fn in lambda_functions if fn.get("memory_mb", 128) > 512 and fn.get("avg_memory_used_mb", 0) < 256
            )
