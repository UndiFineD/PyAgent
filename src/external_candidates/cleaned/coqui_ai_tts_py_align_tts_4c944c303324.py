# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\coqui_ai_tts.py\tts.py\tts.py\models.py\align_tts_4c944c303324.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\align_tts.py
...
class AlignTTS(BaseTTS):
    ...
    def compute_log_probs(mu, log_sigma, y):
        ...
        exponential = -0.5 * torch.mean(
            torch._C._nn.mse_loss(expanded_y, expanded_mu, 0) / torch.pow(log_sigma.exp(), 2),
            dim=-1,
        )  # B, L, T
    ...
    def get_criterion(self):
        from TTS.tts.layers.losses import (
            AlignTTSLoss,  # pylint: disable=import-outside-toplevel
        )
