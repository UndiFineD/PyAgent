...
class SherlockBaseTest(unittest.TestCase):
    ...
    def site_data_filter(self, site_list):
        ...
        for site in site_list:
            with self.subTest(f"Checking test vector Site '{site}' exists in total site data."):
    ...
    def username_check(self, username_list, site_list, exist_check=True):
        ...
        for username in username_list:
            ...
            for site, result in results.items():
                with self.subTest(f"Checking Username '{username}' {check_type_text} on Site '{site}'"):
                    if (self.skip_error_sites == True) and (result["status"].status == QueryStatus.UNKNOWN):
