# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentuniverse.py\agentuniverse.py\agent.py\action.py\knowledge.py\embedding.py\ollama_embedding_96b459f4bba7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\embedding\ollama_embedding.py
...
class OllamaEmbedding(Embedding):
    ...
    ollama_api_key: Optional[str] = Field(default_factory=lambda: get_from_env("OLLAMA_API_KEY"))
    ...
    def as_langchain(self) -> Any:
        ...
        try:
            ...
            return OllamaEmbeddings(base_url=self.ollama_base_url, model=self.embedding_model_name)
    ...
    def _initialize_clients(self) -> None:
        ...
        if self.client is None:
            self.client = httpx.Client(base_url=self.ollama_base_url, headers=headers, timeout=self.timeout)

        if self.async_client is None:
            self.async_client = httpx.AsyncClient(base_url=self.ollama_base_url, headers=headers, timeout=self.timeout)
