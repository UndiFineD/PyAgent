# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agents_generic.py\livekit_agents.py\livekit.py\agents.py\tts.py\stream_pacer_803d48845e80.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\tts\stream_pacer.py
...
class StreamPacerWrapper(SentenceStream):
    ...
    async def _send_task(self) -> None:
        ...
        while not self._closing:
            ...
            remaining_audio = audio_start_time + audio_duration - curr_time if audio_start_time > 0.0 else 0.0

            if first_sentence or (generation_stopped and remaining_audio <= self._options.min_remaining_audio):
                batch: list[str] = []
            ...
            self._wakeup_timer = asyncio.get_event_loop().call_later(wait_time, self._wakeup_event.set)
