# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\images\gunicorn\server.py
...
def app(environ, start_response) -> list[bytes]:
    response_body: bytes = (
        ...
        + b'","uri":"'
        + b64encode(
            (environ["PATH_INFO"] + (("?" + environ["QUERY_STRING"]) if environ["QUERY_STRING"] else "")).encode(
                "latin1"
            )
        )
        + b'","method":"'
