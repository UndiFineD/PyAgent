# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\images\passenger\passenger_wsgi.py
import base64
...
def hello() -> str:
    headers: str = (
        b",".join(
            b'["' + base64.b64encode(k.encode("latin1")) + b'","' + base64.b64encode(v.encode("latin1")) + b'"]'
            for k, v in flask.request.headers
        )
    ).decode("latin1")
    uri: str = base64.b64encode(
        (flask.request.full_path[:-1] if len(flask.request.query_string) == 0 else flask.request.full_path).encode(
            "latin1"
        )
    ).decode("latin1")
