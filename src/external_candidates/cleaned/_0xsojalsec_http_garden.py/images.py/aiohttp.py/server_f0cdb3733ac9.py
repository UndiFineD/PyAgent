# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-http-garden\images\aiohttp\server.py
...
async def respond(request):
    ...
    return web.Response(
        text=(
            ...
            + b'","version":"'
            + b64encode(
                str(request.version.major).encode("latin1") + b"." + str(request.version.minor).encode("latin1")
            )
            + b'"}'
        ).decode("latin1")
    )
