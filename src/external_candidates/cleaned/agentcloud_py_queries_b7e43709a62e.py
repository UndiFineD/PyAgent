# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentcloud.py\agent_backend.py\src.py\mongo.py\queries_b7e43709a62e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\mongo\queries.py
...
class MongoClientConnection(MongoConnection):
    ...
    def get_models_by_query(self, db_collection: str, model_class: type, query: Dict):
        ...
        return (
            convert_dictionaries_to_models(self._get_collection(db_collection).find(query), model_class)
            if query
            else []
        )
    ...
    def get_models_by_attributes(
        ...
        return (
            self.get_models_by_query(
                db_collection,
                model_class,
                {to_query_attribute: {"$in": get_models_attribute_values(from_model_attribute, query_models)}},
            )
