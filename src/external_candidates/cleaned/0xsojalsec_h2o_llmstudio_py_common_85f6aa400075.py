# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\app_utils.py\sections.py\common_85f6aa400075.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\sections\common.py
...
async def meta(q: Q) -> None:
    ...
    q.page["meta"] = ui.meta_card(
        ...
        scripts=[ui.script(source, asynchronous=True) for source in q.app["script_sources"]],
...
def heap_analytics(userid, user_properties=None, event_properties=None) -> ui.InlineScript:
    script = (
    ...
    if q.client["init_interface"] is None:
        ...
        if q.app.heap_mode:
            ...
            q.page["meta"].script = heap_analytics(
                ...
                event_properties=(f"{{version: '{q.app.version}'" + f", product: '{q.app.name}'}}"),
            )
        ...
        q.page["nav_bar"] = ui.nav_card(
            ...
            items=[
                ui.nav_group(
                    "Navigation",
                    items=[ui.nav_item(page.lower(), page) for page in navigation_pages],
                ),
                ui.nav_group(
                    "Datasets",
                    items=[
            ...
            value=(default_cfg.start_page if q.client["nav/active"] is None else q.client["nav/active"]),
        )

    else:
        # Only update menu properties to prevent from flickering

        q.page["nav_bar"].value = default_cfg.start_page if q.client["nav/active"] is None else q.client["nav/active"]
