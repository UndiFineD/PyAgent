# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\prompt_e3e190c786ce.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\prompt.py
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--device", type=str, required=False, default="cuda:0", help="Device")
    ...
    with torch.device(DEVICE):
        ...
        cfg.architecture.pretrained_weights = os.path.join(args.experiment, "checkpoint.pth")
    ...
    while True:
        ...
        try:
            ...
            inputs = cfg.dataset.dataset_class.encode(tokenizer, prompt, cfg.tokenizer.max_length, "left")

            inputs["prompt_input_ids"] = inputs.pop("input_ids").unsqueeze(0).to(DEVICE)

            inputs["prompt_attention_mask"] = inputs.pop("attention_mask").unsqueeze(0).to(DEVICE)
            ...
            predicted_text = [tokenizer.decode(ids, skip_special_tokens=True) for ids in output["predicted_answer_ids"]]
