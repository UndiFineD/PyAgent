# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\coqui_ai_tts.py\tts.py\tts.py\layers.py\tacotron.py\tacotron_d91aba821d2b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\tacotron\tacotron.py
        ...
        if self.use_memory_queue:
            if self.memory_size > self.r:
                ...
                self.memory_input = torch.cat(
                    [
                        new_memory,
                        self.memory_input[:, : (self.memory_size - self.r) * self.frame_channels].clone(),
                    ],
                    dim=-1,
                )
    ...
    def inference(self, inputs):
        ...
        while True:
            ...
            if t > self.max_decoder_steps:
                logger.info(
                    "Decoder stopped with `max_decoder_steps` %d",
