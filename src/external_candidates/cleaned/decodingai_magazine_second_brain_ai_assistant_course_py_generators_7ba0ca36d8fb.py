# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_offline.py\src.py\second_brain_offline.py\application.py\dataset.py\generators_7ba0ca36d8fb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\application\dataset\generators.py
...
class SummarizationDatasetGenerator:
    ...
    def __init__(
        ...
        self.postgeneration_filters: list[Callable[[Document], bool]] = [
            ...
            and len(document.summary) < int(self.summarization_max_characters * self.max_summary_length_factor),
        ]
    ...
    def __summarize_documents(self, documents: list[Document]) -> list[Document]:
        ...
        filtered_documents = self.filter_documents(self.pregeneration_filters, documents)

        logger.info(f"Num documents after pregeneration filtering: {len(filtered_documents)}")

        summarized_documents: list[Document] = self.__augmented_summarization_loop(
            filtered_documents, loops=self.augmentation_loops
        )

        logger.info(f"Num documents before postgeneration filtering: {len(summarized_documents)}")

        filtered_summarized_documents = self.filter_documents(self.postgeneration_filters, summarized_documents)

        logger.info(f"Num documents after postgeneration filtering: {len(filtered_summarized_documents)}")
    ...
    def __augmented_summarization_loop(self, documents: list[Document], loops: int = 3) -> list[Document]:
        ...
        for i in range(loops):
            ...
            logger.info(f"Loop {i + 1} of {loops} - Summarizing documents with temperature {temperature}")
            ...
            valid_summarized_documents = [doc for doc in summarized_documents if doc.summary is not None]

            augmented_documents.extend(valid_summarized_documents)

        return augmented_documents

    def filter_documents(self, filters: list[Callable[[Document], bool]], documents: list[Document]) -> list[Document]:
        ...
        for document_filter in filters:
            documents = [document for document in documents if document_filter(document)]
