# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\episodic_memory_raw_repository_8fff895266af.py
...
class EpisodicMemoryRawRepository(BaseRepository[EpisodicMemory]):
    ...
    async def get_by_event_id(
        ...
        try:
            ...
            result = await self.model.find_one({"_id": object_id, "user_id": user_id}, session=session)
            ...
            logger.error("❌ Failed to retrieve episodic memory by event ID and user ID: %s", e)
    ...
    async def append_episodic_memory(
        ...
        if episodic_memory.episode and not episodic_memory.vector:
            try:
                vector = await self.vectorize_service.get_embedding(episodic_memory.episode)
    ...
    async def delete_by_event_id(
        ...
        try:
            ...
            result = await self.model.find({"_id": object_id, "user_id": user_id}, session=session).delete()
            ...
            logger.error("❌ Failed to delete episodic memory by event ID and user ID: %s", e)
