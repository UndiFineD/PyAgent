# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alphafactor.py\x_publisher.py\scripts.py\x_publisher_1cc455ce114f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alphafactor\x-publisher\scripts\x_publisher.py
...
def get_client():
    ...
    try:
        ...
        return {"client": client, "api": api, "auth": auth}
...
def upload_media(api, media_path: str) -> Optional[str]:
    ...
    try:
        ...
        if file_ext in video_exts:
            media = api.media_upload(media_path, chunked=True, media_category="tweet_video")
...
def publish_tweet(client, text: str, media_ids: List[str] = None) -> Optional[Dict]:
    ...
    try:
        ...
        return {"success": False, "error": "发布失败", "message": str(e)}
...
示例:
  ...
  %(prog)s verify
    ...
    tweet_parser.add_argument("--media", "-m", action="append", help="媒体文件路径（可多次使用，最多4个）")
