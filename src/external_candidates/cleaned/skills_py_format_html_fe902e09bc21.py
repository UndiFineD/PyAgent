# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\visionik.py\ouracli.py\src.py\ouracli.py\format_html_fe902e09bc21.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\visionik\ouracli\src\ouracli\format_html.py
...
def format_html(data: Any, title: str | None = None) -> str:
        ...
        if isinstance(item_data, dict):
            ...
            for key in sorted_keys:
                ...
                if isinstance(value, dict):
                    if key == "met" and "items" in value:
                        ...
                        for subkey, subvalue in value.items():
                            if subkey not in EXCLUDE_FIELDS:
                                ...
                                table_rows.append(f"<tr><td>{sub_human_key}</td><td>{subvalue}</td></tr>")
                    ...
                    if value and isinstance(value[0], dict) and "bpm" in value[0] and "timestamp" in value[0]:
    ...
    html_parts.append('<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js"></script>')
        ...
        if data and isinstance(data[0], dict) and "bpm" in data[0] and "timestamp" in data[0]:
            ...
            for day in sorted(by_day.keys()):
                ...
                if chart_config:
                    ...
                    chart_div = f'<div style="height: 400px; margin: 20px 0;"><canvas id="{chart_id}"></canvas></div>'
