...
parser = argparse.ArgumentParser(description="Deploy the cosmosdb databases that are needed in Azure")
parser.add_argument("file", nargs="?", help="The path of the json file", default="challenges.json")
parser.add_argument(
    "--env",
...
async def create_database_if_not_exists(client: CosmosClient, database_name):
    ...
    for container in CONTAINERS:
        pending_tasks.append(
            database.create_container_if_not_exists(id=container, partition_key=PartitionKey(CONTAINERS[container]))
        )
    ...
    if "Challenges" in data:
        async with CosmosClient(COSMOSDB_URI, credential=credential) as client:
            ...
            for challenge in data["Challenges"]:
                ...
                if args.drop:
                    pending_tasks.append(delete_database_if_exists(client, database_name))
                else:
                    pending_tasks.append(create_database_if_not_exists(client, database_name))
