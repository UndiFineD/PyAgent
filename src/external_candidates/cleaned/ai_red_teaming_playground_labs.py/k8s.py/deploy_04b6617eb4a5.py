# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\k8s\deploy.py
...
def write_appsettings(challenge, data) -> str:
    ...
    if "Ctfd" in challenge:
        ...
        if "Id" in challenge["Ctfd"]:
            appsettings["Challenge"]["Ctfd"]["ChallengeId"] = str(challenge["Ctfd"]["Id"])

    if "PromptOptions" in challenge:
        appsettings["Prompts"]["ResponseTemperature"] = challenge["PromptOptions"]["Temperature"]
        appsettings["Prompts"]["ResponseTopP"] = challenge["PromptOptions"]["TopP"]
        appsettings["Prompts"]["ResponsePresencePenalty"] = challenge["PromptOptions"]["PresencePenalty"]
        appsettings["Prompts"]["ResponseFrequencyPenalty"] = challenge["PromptOptions"]["FrequencyPenalty"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("file", help="The path of the json file", default="challenges.json")
    parser.add_argument("--namespace", help="The namespace to deploy the challenges", default="dev")
    parser.add_argument(
        "--env",
    ...
    if "Challenges" in data:
        ...
        for challenge in data["Challenges"]:
            ...
            ingress_path = copy.deepcopy(ingress["spec"]["rules"][0]["http"]["paths"][0])
            ...
            if not picture:
                ...
                config_map = client.V1ConfigMap(
                    api_version="v1",
                    kind="ConfigMap",
                    metadata=client.V1ObjectMeta(name=config_map_name, namespace=args.namespace),
                    data={
            ...
            if check_config_map_exists(v1, config_map_name, args.namespace):
                v1.replace_namespaced_config_map(config_map_name, args.namespace, config_map)
            ...
            deployment["spec"]["template"]["spec"]["volumes"][0]["configMap"]["name"] = config_map_name
            ...
            for container in deployment["spec"]["template"]["spec"]["containers"]:
                ...
                for env in container["env"]:
                    ...
                    if env["name"] == "Challenge__HumanScorer__ApiKey" and (
                        "HumanScorer" not in challenge or not challenge["HumanScorer"]
                    ):
            ...
            if not ctfd:
                ...
                if check_config_map_exists(v1, config_map_name, args.namespace):
                    v1.replace_namespaced_config_map(config_map_name, args.namespace, config_home)
                    print("Config map updated: " + config_map_name)
                else:
                ...
                if check_deployment_exists(v1apps, "challenge-home", args.namespace):
                    annotations = {"kubectl.kubernetes.io/restartedAt": datetime.datetime.now().isoformat()}
                    challenge_home_deployment["spec"]["template"]["metadata"]["annotations"] = annotations
                    v1apps.replace_namespaced_deployment("challenge-home", args.namespace, challenge_home_deployment)
                    print("Deployment updated: challenge-home")
                else:
                    v1apps.create_namespaced_deployment(args.namespace, challenge_home_deployment)
                ...
                if "disclaimer" in challenge and challenge["disclaimer"]:
                    description = (
                        "**Disclaimer:** This challenge contains content that might be offensive for some people. If you don't feel comfortable with this material, please don't continue and attempt the challenge.\n\n"
                        + description
                    )
        ...
        if ctfd:
            ...
            config_ctfd = client.V1ConfigMap(
                ...
                kind="ConfigMap",
                metadata=client.V1ObjectMeta(name=config_map_name, namespace=args.namespace),
                data={"challenges.json": json.dumps({"challenges": challenges_settings}, indent=2)},
            )

            # Create the config map for the ctfd
            if check_config_map_exists(v1, config_map_name, args.namespace):
                v1.replace_namespaced_config_map(config_map_name, args.namespace, config_ctfd)
                print("Config map updated: " + config_map_name)
            else:
            ...
            pods = v1.list_namespaced_pod(args.namespace, label_selector=f"job-name={job_name}")
        ...
        if check_if_ingress_exists(v1networking, ingress_name, args.namespace):
            v1networking.replace_namespaced_ingress(ingress_name, args.namespace, ingress)
        ...
        if ingress_chat_scoring:
            ...
            if check_if_ingress_exists(v1networking, ingress_name, args.namespace):
                v1networking.replace_namespaced_ingress(ingress_name, args.namespace, ingress_chat_scoring)
        ...
        if deployment_chat_scoring:
            ...
            if check_deployment_exists(v1apps, deployment_name, args.namespace):
                annotations = {"kubectl.kubernetes.io/restartedAt": datetime.datetime.now().isoformat()}
                deployment_chat_scoring["spec"]["template"]["metadata"]["annotations"] = annotations
                v1apps.replace_namespaced_deployment(deployment_name, args.namespace, deployment_chat_scoring)
        ...
        if service_chat_scoring_yml:
            ...
            if check_service_exists(v1, service_name, args.namespace):
                v1.replace_namespaced_service(service_name, args.namespace, service_chat_scoring_yml)
                print("Service updated: " + service_name)
            else:
