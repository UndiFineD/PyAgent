# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\src\picture-submission\webapi\server\controller\scoring.py
...
from server.settings import (
    CONFIG_CHALLENGE_FLAG,
    CONFIG_CHALLENGE_GOAL,
    CONFIG_CHALLENGE_ID,
    CONFIG_CHALLENGE_NAME,
    CONFIG_CHALLENGE_SETTINGS,
    CONFIG_SCORING_KEY,
    CONFIG_SCORING_SCORING_ENDPOINT,
    CONFIG_SCORING_SETTINGS,
    CONFIG_SCORING_SUBMISSION_ENDPOINT,
)
...
class ScoringController:
    ...
    def send_scoring_request(self, id: str, picture: str) -> bool:
        ...
        if response.status_code != 200:
            self.__logger.error(
                f"Failed to send scoring request for submission {id} with status code: {response.status_code}"
            )
            return False
        return True
