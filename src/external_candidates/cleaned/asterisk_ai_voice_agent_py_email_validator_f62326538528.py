# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\utils.py\email_validator_f62326538528.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\utils\email_validator.py
...
class EmailValidator:
    ...
    def parse_from_speech(cls, speech_text: str) -> Optional[str]:
        ...
        if not cls.EMAIL_PATTERN.match(text):
            logger.warning("Invalid email format after parsing", original=speech_text, parsed=text)

            return None

        logger.info("Parsed email from speech", original=speech_text, parsed=text)
    ...
    async def validate_domain(cls, email: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                if a_records:
                    logger.info("Domain validation successful via A record", domain=domain)
                ...
                logger.warning("Domain validation failed", domain=domain, error=str(e))
            ...
            logger.error("Domain validation error", domain=domain, error=str(e))
