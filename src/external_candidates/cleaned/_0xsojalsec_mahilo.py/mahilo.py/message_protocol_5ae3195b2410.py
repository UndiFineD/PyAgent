# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\message_protocol.py
import json
import time
import uuid
...
class MessageEnvelope:
    ...
    def verify(self, secret_key: str) -> bool:
        ...
        try:
            ...
            return decoded["message_id"] == self.message_id and decoded["payload"] == self.payload
        except jwt.InvalidTokenError:
            return False

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary"""
        return {k: str(v) if isinstance(v, MessageType) else v for k, v in asdict(self).items()}
...
class MessageBroker:
    ...
    def _get_message_context(self, message: MessageEnvelope) -> Dict:
        ...
        if self.store:
            try:
                ...
                history = self.store.get_conversation_history(agent1=message.sender, agent2=message.recipient, limit=10)
    ...
    def _create_error_response(
        self, original_message: MessageEnvelope, violations: List["PolicyViolation"]
    ) -> MessageEnvelope:
    ...
    async def send_message(self, message: MessageEnvelope) -> None:
        ...
        if message.message_type != MessageType.ERROR and self.validator:
            # Validate message
            valid, violations = await self.validator.validate(message, self._get_message_context(message))
            ...
            if not valid:
                ...
                if self.telemetry:
                    violation_details = [{"policy": v.policy_name, "reason": v.reason} for v in violations]
