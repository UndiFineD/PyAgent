# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\integrations\langgraph\agent.py
...
class LangGraphAgent(BaseAgent):
    ...
    async def process_queue_message(self, message: str = None, websockets: List[WebSocket] = []) -> None:
        ...
        messages.append(
            (
                "system",
                "You should only contact other agents if the need be. If you already have info from them, don't call any tools and just return your answer based on their response.",
            )
        )
