...
class BaseAgent:
    ...
    def get_contactable_agents_with_description(self) -> Dict[str, str]:
        ...
        all_available_agents_with_description = {
            agent.name: agent.short_description for agent in self._agent_manager.get_all_agents()
        }
        return {
            name: desc
    ...
    async def _process_chat_message_internal(
            ...
            for tool_call in tool_calls:
                ...
                try:
                    ...
                    if isinstance(function_response, dict):
                        ...
                        function_response = [
                            json.dumps(item) if isinstance(item, dict) else str(item) for item in function_response
                        ]
                except Exception as e:
        ...
        activated_agents = [
            agent for agent in self._agent_manager.get_all_agents() if agent.is_active() and agent.name != self.name
        ]
    ...
    async def process_queue_message(self, websockets: List[WebSocket] = []) -> None:
        ...
        if self._telemetry:
            with self._telemetry.start_processing_span(message_id=str(id(websockets)), agent_id=self.name) as span:
                try:
    ...
    async def _process_queue_message_internal(self, websockets: List[WebSocket] = []) -> None:
        ...
        for envelope in pending_messages:
            try:
                ...
                self._agent_manager.message_broker.acknowledge_message(envelope.message_id, self.name)

            except Exception as e:
                print(f"Error processing message {envelope.message_id}: {e}")
                # Handle failure and retry if needed
                should_retry = self._agent_manager.message_broker.handle_failure(envelope.message_id, self.name)
                if not should_retry:
    ...
    def deactivate(self) -> None:
        ...
        if self._telemetry:
            self._telemetry.record_event(event_type=EventType.AGENT_DEACTIVATED, agent_id=self.name)
        self._session = None
    ...
    def _validate_tool_function(self, func: Callable, tool_name: str) -> None:
        ...
        if return_type not in valid_return_types and not (
            ...
            and (
                (return_type.__origin__ is list and return_type.__args__[0] in (str, Dict, dict))
                or (return_type.__origin__ is dict)
            )
        ):
            raise ToolFunctionError(
                f"Tool function '{tool_name}' must return str, List[str], Dict, or List[Dict], got {return_type}"
            )
    ...
    def _validate_tool_config(self, tool_config: Dict[str, Any]) -> None:
        ...
        if "tool" not in tool_config or "function" not in tool_config:
            raise ToolFunctionError("Tool config must contain both 'tool' and 'function' keys")

        tool = tool_config["tool"]
        func = tool_config["function"]

        if "function" not in tool or "name" not in tool["function"]:
            raise ToolFunctionError("Tool configuration must have a 'function' property with a 'name' field")

        tool_name = tool["function"]["name"]
        if tool_name in ["chat_with_agent", "contact_human"]:
            raise ToolFunctionError(f"Tool name '{tool_name}' is reserved for base tools")
