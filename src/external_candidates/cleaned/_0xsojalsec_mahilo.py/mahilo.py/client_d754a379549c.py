# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\client.py
import asyncio
import base64
import json
import threading
...
class Client:
    def __init__(self, url: str, agent_name: Optional[str] = None, voice: bool = False):
        ...
        if self.voice:
            if not PYAUDIO_AVAILABLE:
                raise ImportError(
                    "PyAudio is required for voice features. Learn how to install it for your OS here: https://pypi.org/project/PyAudio/."
                )
            ...
            while True:
                ...
                if self.voice:
                    ...
                    if data["event"] == "media":
                        ...
                        rich.print(
                            f"[bold green]🎵  Received audio data[/bold green] ([italic]{len(audio_data)} bytes[/italic])"
                        )
    ...
    async def _record_and_send_audio(self):
        if not self.voice:
            raise RuntimeError(
                "Voice features are not enabled. Initialize the client with voice=True to use voice features."
            )
        ...
        try:
            while self.is_recording:
                ...
                await self.websocket.send(json.dumps({"event": "media", "media": {"payload": audio_payload}}))
