# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\examples\policy_validation\policy_example.py
"""
...
async def test_message(agent_manager, sender, recipient, content, test_name, previous_violations):
    ...
    await agent_manager.send_message_to_agent(sender=sender, recipient=recipient, message=content)
...
async def main():
    ...
    test_messages = [
        ...
        (
            ...
            "My password is 12345 and my API key is abcdef.",
        ),
    ]
    ...
    for test_name, content in test_messages:
        result, all_violations = await test_message(
            agent_manager, "agent1", "agent2", content, test_name, all_violations
        )
