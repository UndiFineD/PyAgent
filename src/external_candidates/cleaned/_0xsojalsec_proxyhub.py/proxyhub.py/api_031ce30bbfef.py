# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-proxyhub\proxyhub\api.py
import asyncio
import io
import signal
import warnings
...
class Broker:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._judges = judges
        self._providers = [p if isinstance(p, Provider) else Provider(p) for p in (providers or PROVIDERS)]
    ...
    async def _grab(self, types=None, check=False):
        def _get_tasks(by=MAX_CONCURRENT_PROVIDERS):
            providers = [pr for pr in self._providers if not types or not pr.proto or bool(pr.proto & types.keys())]
            while providers:
    ...
    async def _push_to_check(self, proxy):
        ...
        if self._server and not self._proxies.empty() and self._limit <= 0:
            log.debug("pause. proxies: %s; limit: %s" % (self._proxies.qsize(), self._limit))
