...
GeoData = namedtuple("GeoData", ["code", "name", "region_code", "region_name", "city_name"])
...
class Resolver:
    ...
    async def _resolve(self, host, qtype):
        try:
            resp = await asyncio.wait_for(self._resolver.query(host, qtype), timeout=self._timeout)
