# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-proxyhub\proxyhub\proxy.py
import asyncio
import ssl as _ssl
import time
import warnings
...
class Proxy:
    ...
    def __init__(self, host=None, port=None, types=(), timeout=8, verify_ssl=False):
        ...
        if not Resolver.host_is_ip(self.host):
            raise ValueError("The host of proxy should be the IP address. Try Proxy.create() if the host is a domain")
    ...
    def log(self, msg, stime=0, err=None):
        ...
        log.debug(
            "{h}:{p} [{n}]: {msg}; Runtime: {rt:.2f}".format(h=self.host, p=self.port, n=ngtr, msg=msg, rt=runtime)
        )
