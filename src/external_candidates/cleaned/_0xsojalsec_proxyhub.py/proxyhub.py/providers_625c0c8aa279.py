# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-proxyhub\proxyhub\providers.py
import asyncio
import re
import warnings
...
class Checkerproxy_net(Provider):
    ...
    async def _pipe(self):
        ...
        urls = ["https://checkerproxy.net/api%s" % path for path in re.findall(exp, page)]
        await self._find_on_pages(urls)
...
class Proxy_list_org(Provider):
    ...
    async def _pipe(self):
        ...
        page = await self.get(url)
        urls = ["http://proxy-list.org/english/%s" % path for path in re.findall(exp, page)]
        urls.append(url)
        await self._find_on_pages(urls)
...
class Gatherproxy_com(Provider):
    ...
    def find_proxies(self, page):
        ...
        return [(host, str(int(port, 16))) for host, port in self._pattern_h.findall(page) if port]
    ...
    async def _pipe(self):
        ...
        for t in ["anonymous", "elite"]:
            ...
            urls = [
                {"url": url, "data": {"Type": t, "PageIdx": pid}, "method": method} for pid in range(1, lastPageId + 1)
            ]
...
class Spys_ru(Provider):
    ...
    async def _pipe(self):
        ...
        method = "POST"
        urls = [{"url": url, "data": {**data, "xf1": lvl}, "method": method} for lvl in [3, 4]]
        await self._find_on_pages(urls)
        # expCountries = r'>([A-Z]{2})<'
...
class Free_proxy_cz(Provider):
    ...
    def find_proxies(self, page):
        return [(b64decode(h).decode(), b64decode(p).decode()) for h, p in self._find_proxies(page)]
...
class Proxyb_net(Provider):
    ...
    def find_proxies(self, page):
        ...
        for host in _hosts.split("</tr><tr>"):  # noqa: W605
            ...
            hosts.append(host[0])
        ports = [
            self._port_pattern.findall(b64decode(port).decode())[0] for port in self._port_pattern_b64.findall(_ports)
        ]
...
class Proxylistplus_com(Provider):
    ...
    async def _pipe(self):
        ...
        urls = ["http://list.proxylistplus.com/%s-List-%d" % (i, n) for i in names for n in range(1, 7)]
        await self._find_on_pages(urls)
