# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-proxyhub\proxyhub\server.py
import asyncio
import heapq
import time
...
class ProxyPool:
    ...
    def put(self, proxy):
        is_exceed_time = (proxy.error_rate > self._max_error_rate) or (proxy.avg_resp_time > self._max_resp_time)
...
class Server:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._server = None
        self._connections = {}
        self._proxy_pool = ProxyPool(proxies, min_req_proxy, max_error_rate, max_resp_time, min_queue)
    ...
    def start(self):
        ...
        log.info("Listening established on {0}".format(self._server.sockets[0].getsockname()))
    ...
    async def _handle(self, client_reader, client_writer):
        log.debug("Accepted connection from %s" % (client_writer.get_extra_info("peername"),))

        request, headers = await self._parse_request(client_reader)
        scheme = self._identify_scheme(headers)
        client = id(client_reader)
        log.debug("client: %d; request: %s; headers: %s; scheme: %s" % (client, request, headers, scheme))
        ...
        if headers["Host"] == "proxycontrol":
            ...
            if _api == "api":
                if _operation == "remove":
                    ...
                    if query_type == "url":
                        previous_proxy = history.get(f"{client_reader._transport.get_extra_info('peername')[0]}-{url}")
        ...
        for attempt in range(self._max_tries):
            ...
            log.debug("client: %d; attempt: %d; proxy: %s; proto: %s" % (client, attempt, proxy, proto))
            ...
            try:
                ...
                history[f"{client_reader._transport.get_extra_info('peername')[0]}-{headers['Path']}"] = (
                    proxy.host + ":" + str(proxy.port)
                )
                inject_resp_header = {"headers": {"X-Proxy-Info": proxy.host + ":" + str(proxy.port)}}

                stime = time.time()
                stream = [
                    asyncio.ensure_future(self._stream(reader=client_reader, writer=proxy.writer)),
                ...
                log.debug("client: %d; error: %r; EOF: %s" % (client, e, client_reader.at_eof()))
                for task in stream:
                    if not task.done():
                        task.cancel()
