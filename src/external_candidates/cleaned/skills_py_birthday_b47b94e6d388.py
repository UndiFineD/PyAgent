# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\manantra.py\birthday_reminder.py\scripts.py\birthday_b47b94e6d388.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\manantra\birthday-reminder\scripts\birthday.py
...
def add_birthday(name, date_str, year_born=None):
    ...
    birthdays[name] = {"day": date.day, "month": date.month, "year": year_born}

    save_birthdays(birthdays)

    print(
        f"✅ Geburtstag gespeichert: {name} - {date.day:02d}.{date.month:02d}." + (f"{year_born}" if year_born else "")
    )
...
def next_birthday():
    ...
    if closest:
        ...
        if days_left == 0:
            ...
            print(
                f"📅 **{name}** hat in {days_left} Tagen Geburtstag{age_str} ({data['day']:02d}.{data['month']:02d}.)!"
            )
...
def check_reminders(days_ahead=7):
    ...
    for name, data in birthdays.items():
        ...
        if days_left <= days_ahead:
            age = calculate_age(
                data.get("year"),
                today.replace(year=today.year + (1 if days_left < 365 else 0)),
            )
