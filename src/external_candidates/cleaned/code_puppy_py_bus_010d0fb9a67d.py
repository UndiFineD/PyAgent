# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\messaging.py\bus_010d0fb9a67d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\messaging\bus.py
...
class MessageBus:
    ...
    def provide_response(self, command: AnyCommand) -> None:
        ...
        if isinstance(command, UserInputResponse):
            ...
            self._complete_request(command.prompt_id, (command.confirmed, command.feedback))

        elif isinstance(command, SelectionResponse):
            self._complete_request(command.prompt_id, (command.selected_index, command.selected_value))
    ...
    def _complete_request(self, prompt_id: str, result: object) -> None:
        ...
        if future is not None and not future.done():
            ...
            if self._event_loop is not None:
                try:
                    self._event_loop.call_soon_threadsafe(self._set_future_result, future, result)
