# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\plugins_bf057cb162d9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\plugins.py
...
def available_providers(allow_override: bool = False, include_optional: bool = True) -> Dict[str, str]:
...
def _load_class(spec: str) -> Type[base_model.BaseLanguageModel]:
    ...
    try:
        ...
        raise ImportError(f"Provider class '{class_name}' not found in module '{module_path}'") from e
    ...
    if not isinstance(cls, type) or not issubclass(cls, base_model.BaseLanguageModel):
        ...
        if missing:
            raise TypeError(f"{cls} is not a BaseLanguageModel and missing required methods: {missing}")
