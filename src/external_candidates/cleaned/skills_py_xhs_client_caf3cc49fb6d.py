# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\borye.py\xiaohongshu_mcp.py\scripts.py\xhs_client_caf3cc49fb6d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\borye\xiaohongshu-mcp\scripts\xhs_client.py
...
def search_notes(keyword, sort_by="综合", note_type="不限", publish_time="不限"):
    ...
    try:
        ...
        resp = requests.post(f"{BASE_URL}/api/v1/feeds/search", json=payload, timeout=TIMEOUT)
        ...
        if data.get("success"):
            ...
            for i, feed in enumerate(feeds, 1):
                ...
                print(
                    f"    Likes: {interact.get('likedCount', '0')} | Collects: {interact.get('collectedCount', '0')} | Comments: {interact.get('commentCount', '0')}"
                )
...
def get_note_detail(feed_id, xsec_token, load_comments=False):
    ...
    try:
        ...
        resp = requests.post(f"{BASE_URL}/api/v1/feeds/detail", json=payload, timeout=TIMEOUT)
...
def main():
    ...
    search_parser.add_argument("--type", default="不限", choices=["不限", "视频", "图文"], help="Note type")
