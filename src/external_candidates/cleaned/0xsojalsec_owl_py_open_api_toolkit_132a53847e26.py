# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_owl.py\owl.py\camel.py\toolkits.py\open_api_toolkit_132a53847e26.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\toolkits\open_api_toolkit.py
...
class OpenAPIToolkit:
    ...
    def parse_openapi_file(self, openapi_spec_path: str) -> Optional[Dict[str, Any]]:
        ...
        if not version:
            raise ValueError("OpenAPI version not specified in the spec. Only OPENAPI 3.0.x and 3.1.x are supported.")

        if not (version.startswith("3.0") or version.startswith("3.1")):
            raise ValueError(f"Unsupported OpenAPI version: {version}. Only OPENAPI 3.0.x and 3.1.x are supported.")
    ...
    def openapi_spec_to_openai_schemas(self, api_name: str, openapi_spec: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for path, path_item in openapi_spec.get("paths", {}).items():
            for method, op in path_item.items():
                ...
                if not description:
                    raise ValueError(
                        f"{method} {path} Operation from {api_name} does not have a description or summary."
                    )
