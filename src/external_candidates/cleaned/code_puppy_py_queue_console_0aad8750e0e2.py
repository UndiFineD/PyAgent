# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\messaging.py\queue_console_0aad8750e0e2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\messaging\queue_console.py
...
class QueueConsole:
    ...
    def print(
        ...
        **kwargs,
        ...
        if len(values) == 1 and hasattr(values[0], "__rich_console__"):
            ...
            for v in values:
                if hasattr(v, "__rich_console__"):
                    ...
                    temp_console = Console(file=string_io, width=80, legacy_windows=False, markup=True)
        ...
        self.queue.emit_simple(message_type, content, style=style, highlight=highlight, **kwargs)
    ...
    def log(
        ...
        *values: Any,
        ...
        self.queue.emit_simple(message_type, content, log=True, style=style, log_locals=log_locals)
    ...
    def status(self, status: str, *, spinner: str = "dots"):
        ...
        self.queue.emit_simple(MessageType.INFO, f"⏳ {status}", status=True, spinner=spinner)
    ...
    def input(self, prompt: str = "") -> str:
        ...
        try:
            ...
            if user_response:
                self.queue.emit_simple(MessageType.INFO, f"User response: {user_response}")
