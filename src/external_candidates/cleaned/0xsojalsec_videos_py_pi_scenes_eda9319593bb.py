# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\diffyq.py\part1.py\pi_scenes_eda9319593bb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part1\pi_scenes.py
...
class FormulasAreLies(PiCreatureScene):
    def construct(self):
        ...
        period_eq = OldTex("\\text{Period} = 2\\pi \\sqrt{\\,{L} / {g}}", **kwargs)

        theta_eq = OldTex(
            "\\theta(t) = \\theta_0 \\cos\\left(\\sqrt{\\,{L} / {g}} \\cdot t\\right)",
            **kwargs,
        )
...
class SoWhatIsThetaThen(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            "First, you must appreciate\\\\a deep truth...",
            added_anims=[self.change_students(*3 * ["confused"])],
        )
...
class ProveTeacherWrong(TeacherStudentsScene):
    def construct(self):
        ...
        q_marks = VGroup(
            *[OldTex("?").scale(1.5).next_to(arrow.point_from_proportion(a), UP) for a in np.linspace(0.2, 0.8, 5)]
        )
        ...
        self.play_student_changes(*3 * ["pondering"], self.teacher.change, "maybe")
...
class AskAboutActuallySolving(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            "Yeah yeah, but how do\\\\you actually \\emph{solve} it?",
            index=1,
            target_mode="sassy",
        ...
        self.teacher_says(
            ...
            added_anims=[self.change_students(*3 * ["erm"])],
        )
...
class HungerForExactness(TeacherStudentsScene):
    def construct(self):
        ...
        proposed_solution = OldTex("\\theta_0\\cos((\\sqrt{g/L})t)e^{-\\mu t}")
        ...
        solution_p2 = OldTex("c_1, c_2 = \\text{Constants depending on initial conditions}")
        ...
        solution_p3.name = OldTexText("(Jacobi amplitude function)")
        ...
        solution_p4.name = OldTexText("(Jacobi elliptic function)")
        ...
        self.student_says(
            "Right, but like,\\\\what \\emph{is} $\\theta(t)$?",
            target_mode="sassy",
        ...
        self.student_says(
            "Let $\\text{P}(\\mu, g, L; t)$ be a\\\\function satisfying this ODE.",
            index=0,
            target_mode="speaking",
            added_anims=[
                FadeOut(mystery),
