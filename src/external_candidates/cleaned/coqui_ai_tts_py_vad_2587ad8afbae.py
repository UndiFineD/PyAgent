# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\coqui_ai_tts.py\tts.py\utils.py\vad_2587ad8afbae.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\utils\vad.py
...
def map_timestamps_to_new_sr(vad_sr, new_sr, timestamps, just_begging_end=False):
    ...
    if just_begging_end and timestamps:
        ...
        new_dict = {
            "start": int(timestamps[0]["start"] * factor),
            "end": int(timestamps[-1]["end"] * factor),
        }
