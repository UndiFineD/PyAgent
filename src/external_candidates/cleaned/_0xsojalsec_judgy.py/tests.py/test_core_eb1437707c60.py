# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-judgy\tests\test_core.py
"""
...
class TestEstimatePassRate:
    ...
    def test_input_validation(self):
        ...
        with pytest.raises(ValueError, match="confidence_level must be between 0 and 1"):
            estimate_success_rate(valid_labels, valid_preds, valid_unlabeled, confidence_level=1.5)

        with pytest.raises(ValueError, match="confidence_level must be between 0 and 1"):
            estimate_success_rate(valid_labels, valid_preds, valid_unlabeled, confidence_level=0)

        # Test invalid bootstrap iterations
        with pytest.raises(ValueError, match="bootstrap_iterations must be positive"):
            estimate_success_rate(valid_labels, valid_preds, valid_unlabeled, bootstrap_iterations=0)
    ...
    def test_reproducibility(self):
        ...
        np.random.seed(42)
        result1 = estimate_success_rate(test_labels, test_preds, unlabeled_preds, bootstrap_iterations=100)

        np.random.seed(42)
        result2 = estimate_success_rate(test_labels, test_preds, unlabeled_preds, bootstrap_iterations=100)
