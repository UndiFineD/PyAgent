...
class TestGenerateTestData:
    ...
    def test_input_validation(self):
        ...
        # Invalid rates
        with pytest.raises(ValueError, match="true_positive_rate must be between 0 and 1"):
            generate_test_data(5, 5, -0.1, 0.8)

        with pytest.raises(ValueError, match="true_positive_rate must be between 0 and 1"):
            generate_test_data(5, 5, 1.1, 0.8)

        with pytest.raises(ValueError, match="true_negative_rate must be between 0 and 1"):
        ...
        # Invalid counts
        with pytest.raises(ValueError, match="n_positive and n_negative must be positive"):
...
class TestCreateExampleDataset:
    ...
    def test_all_scenarios(self):
        ...
        for scenario in scenarios:
            test_labels, test_preds, unlabeled_preds = create_example_dataset(scenario=scenario, random_seed=42)
    ...
    def test_scenario_accuracy_properties(self):
        ...
        test_labels, test_preds, _ = create_example_dataset(scenario="good_judge", random_seed=42)
        ...
        test_labels, test_preds, _ = create_example_dataset(scenario="poor_judge", random_seed=42)
