# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-judgy\examples\example.py
#!/usr/bin/env python3
"""
...
try:
    from judgy.plotting import (
        plot_fnr_fpr_sensitivity,
        plot_label_size_sensitivity,
        run_fnr_fpr_experiment,
        run_label_size_experiment,
    )

    PLOTTING_AVAILABLE = True
except ImportError:
    print("Plotting functionality not available. Install with: pip install judgy[plotting]")
...
def main():
    ...
    print(f"TPR range tested: {(1 - fnr_values[-1]) * 100:.0f}% to {(1 - fnr_values[0]) * 100:.0f}%")
    print(f"TNR range tested: {(1 - fpr_values[-1]) * 100:.0f}% to {(1 - fpr_values[0]) * 100:.0f}%")
