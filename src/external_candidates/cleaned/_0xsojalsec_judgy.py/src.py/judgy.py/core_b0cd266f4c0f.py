# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-judgy\src\judgy\core.py
"""
...
def estimate_success_rate(
    ...
    if len(test_labels) != len(test_preds):
        raise ValueError(
            f"test_labels and test_preds must have the same length. Got {len(test_labels)} and {len(test_preds)}"
        )
    ...
    for _ in range(bootstrap_iterations):
        ...
        tpr_bootstrap = ((bootstrap_labels == 1) & (bootstrap_preds == 1)).sum() / pos_count_boot
        tnr_bootstrap = ((bootstrap_labels == 0) & (bootstrap_preds == 0)).sum() / neg_count_boot
