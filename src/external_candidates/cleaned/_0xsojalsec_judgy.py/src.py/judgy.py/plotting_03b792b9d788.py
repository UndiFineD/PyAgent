...
def _check_matplotlib() -> None:
    ...
    if not MATPLOTLIB_AVAILABLE:
        raise ImportError(
            "matplotlib is required for plotting functionality. Install it with: pip install judgy[plotting]"
        )
...
def plot_fnr_fpr_sensitivity(
    ...
    # Left plot: CI vs. TPR
    axes[0].fill_between(tpr_values, tpr_lower, tpr_upper, alpha=0.3, color="lightblue", label="95% CI")
    axes[0].plot(
        tpr_values,
        tpr_estimates,
        "x-",
    ...
    axes[0].plot(tpr_values, tpr_raw_rates, "o", color="red", markersize=6, label="Raw k/m")
    axes[0].axhline(
        true_rate_pct,
        linestyle="--",
        color="black",
    ...
    axes[1].fill_between(tnr_values, tnr_lower, tnr_upper, alpha=0.3, color="lightblue", label="95% CI")
    axes[1].plot(
        tnr_values,
        tnr_estimates,
        "x-",
    ...
    axes[1].plot(tnr_values, tnr_raw_rates, "o", color="red", markersize=6, label="Raw k/m")
    axes[1].axhline(
        true_rate_pct,
        linestyle="--",
        color="black",
    ...
    if save_plots:
        ...
        ax_left.plot(tpr_values, tpr_raw_rates, "o", color="red", markersize=6, label="Raw k/m")
        ax_left.axhline(
            true_rate_pct,
            linestyle="--",
            color="black",
        ...
        ax_right.plot(tnr_values, tnr_raw_rates, "o", color="red", markersize=6, label="Raw k/m")
        ax_right.axhline(
            true_rate_pct,
            linestyle="--",
            color="black",
        ...
        ax_right.set_title(f"CI vs. TNR (baseline TPR={baseline_tpr:.0f}%)", fontsize=12)
        ...
        print("Individual plots saved to plots/tpr_sensitivity.png and plots/tnr_sensitivity.png")
...
def run_fnr_fpr_experiment(
    ...
    for fnr in fnr_values:
        ...
        test_labels, test_preds = generate_test_data(n_test_positive, n_test_negative, tpr, tnr, random_seed=42)

        # Generate unlabeled data with fixed seed for reproducibility
        unlabeled_preds = generate_unlabeled_data(n_unlabeled, true_success_rate, tpr, tnr, random_seed=42)
...
def run_label_size_experiment(
    ...
    for total_labels in label_sizes:
        ...
        test_labels, test_preds = generate_test_data(n_positive, n_negative, fixed_tpr, fixed_tnr, random_seed=42)

        # Generate unlabeled data (same for all experiments)
        unlabeled_preds = generate_unlabeled_data(n_unlabeled, true_success_rate, fixed_tpr, fixed_tnr, random_seed=42)
...
def plot_label_size_sensitivity(
    ...
    axes[0].plot(label_sizes, raw_rates, "o", color="red", markersize=6, label="Raw k/m")
    axes[0].axhline(
        true_rate_pct,
        linestyle="--",
        color="black",
    ...
    if save_plots:
        ...
        ax_left.plot(label_sizes, raw_rates, "o", color="red", markersize=6, label="Raw k/m")
        ax_left.axhline(
            true_rate_pct,
            linestyle="--",
            color="black",
        ...
        print("Individual plots saved to plots/label_size_estimates.png and plots/label_size_ci_width.png")
