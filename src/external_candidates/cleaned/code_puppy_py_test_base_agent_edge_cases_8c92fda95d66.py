# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\agents.py\test_base_agent_edge_cases_8c92fda95d66.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\agents\test_base_agent_edge_cases.py
...
class TestBaseAgentEdgeCases:
    ...
    def test_load_model_with_fallback_all_fail(
        ...
        with pytest.raises(ValueError, match="No valid model could be loaded"):
            agent._load_model_with_fallback("bad-model", {"model1": {}, "model2": {}, "model3": {}}, "test-group")
    ...
    @patch("code_puppy.model_factory.ModelFactory.get_model")
    @patch("code_puppy.model_factory.ModelFactory.load_config")
    def test_load_model_with_fallback_empty_config(self, mock_load_config, mock_get_model, agent):
    ...
    def test_filter_huge_messages_with_corrupted_messages(self, agent):
        ...
        corrupted_msg.parts = None  # None parts will cause crashes in estimate_tokens_for_message
    ...
    @patch("code_puppy.model_factory.ModelFactory.load_config")
    def test_get_model_context_length_invalid_context_length(self, mock_load_config, agent):
    ...
    @patch("code_puppy.model_factory.ModelFactory.load_config")
    def test_get_model_context_length_negative_context_length(self, mock_load_config, agent):
    ...
    def test_load_puppy_rules_file_permission_error(self, mock_exists, mock_read_text, agent):
    ...
    def test_load_puppy_rules_empty_file(self, mock_exists, mock_read_text, agent):
        ...
        assert result is None  # Should return None for empty file (empty string is falsy)
    ...
    def test_compaction_edge_cases_with_extreme_tokens(self, agent):
        ...
        huge_msg = ModelRequest(parts=[TextPart(content="x" * 100000)])  # Very long content
    ...
    @patch("code_puppy.model_factory.ModelFactory.get_model")
    @patch("code_puppy.model_factory.ModelFactory.load_config")
    def test_load_model_with_fallback_unexpected_exception(self, mock_load_config, mock_get_model, agent):
