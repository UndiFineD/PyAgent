# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\llm.py\openai_provider_7ec2d8999d66.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\llm\openai_provider.py
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.post(
                        ...
                        if response.status != 200:
                            ...
                            logger.error(f"❌ [OpenAI-{self.model}] HTTP error {response.status}:")
                        ...
                        logger.debug(f"[OpenAI-{self.model}] Duration: {end_time - start_time:.2f}s")

                        # If the duration is too long

                        if end_time - start_time > 30:
                            logger.warning(f"[OpenAI-{self.model}] Duration too long: {end_time - start_time:.2f}s")
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"❌ [OpenAI-{self.model}] API connection test failed: Empty response")
