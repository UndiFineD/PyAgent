# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\src.py\losses.py\text_dpo_modeling_losses_32aae38d58e1.py
...
class DPOLoss(nn.Module):
    ...
    def forward(
        ...
        chosen_rewards = self.cfg.training.beta * (policy_chosen_logps - reference_chosen_logps).detach()

        rejected_rewards = self.cfg.training.beta * (policy_rejected_logps - reference_rejected_logps).detach()
...
class KTOPairLoss(nn.Module):
    ...
    def forward(
        ...
        chosen_rewards = (self.cfg.training.beta * (policy_chosen_logps - reference_chosen_logps).detach()).float()

        rejected_rewards = (
            self.cfg.training.beta * (policy_rejected_logps - reference_rejected_logps).detach()
        ).float()
