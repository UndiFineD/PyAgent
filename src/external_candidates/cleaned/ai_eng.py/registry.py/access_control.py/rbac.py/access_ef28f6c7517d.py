# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\access_control\rbac\access.py
import json
...
def _project_access(project: str, user: User, access: str) -> UserAccess:
    ...
    if rbac.validate_project_access_users(project, user.username, access):
        ...
        raise ForbiddenAccess(f"{access} access for project {project} is required for user {user.username}")
...
def get_api_header(username: str):
    return {"x-registry-requestor": username}
...
def _get_project_name(id_or_name: Union[str, UUID]):
    try:
        ...
        raise BadRequest(
            f"Project Id {id_or_name} not found in Registry {config.RBAC_REGISTRY_URL}. Please check if the project exists or retry later."
        )
    except ValueError:
        # It is a name
        pass
