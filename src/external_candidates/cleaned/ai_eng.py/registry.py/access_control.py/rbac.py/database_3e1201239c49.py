# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\access_control\rbac\database.py
import logging
import os
import threading
...
def parse_conn_str(s: str) -> dict:
    ...
    """
    parts = dict([p.strip().split("=", 1) for p in s.split(";") if len(p.strip()) > 0])
...
class MssqlConnection(DbConnection):
    ...
    def connect(autocommit=True):
        ...
        if "Server=" not in conn_str:
            logging.debug("`RBAC_CONNECTION_STR` is not in ADO connection string format")
            return None
