...
class DbRegistry(Registry):
    ...
    def get_projects_ids(self) -> Dict:
        projects = {}
        if os.environ.get("FEATHR_SANDBOX"):
            query = db.select(self.entities_table.c.entity_id, self.entities_table.c.qualified_name).where(
                (self.entities_table.c.entity_type == str(EntityType.Project))
            )
            ret = self._fetch_helper(query)
        else:
    ...
    def create_project(self, definition: ProjectDef) -> UUID:
        ...
        with self.conn.transaction() as c:
            ...
            if os.environ.get("FEATHR_SANDBOX"):
                ...
                ).where((self.entities_table.c.qualified_name == definition.qualified_name))
            ...
            if r:
                ...
                if _to_type(r[0]["entity_type"], EntityType) != EntityType.Project:
                    raise ConflictError("Entity %s already exists" % definition.qualified_name)
        ...
        with self.conn.transaction() as c:
            ...
            if r:
                ...
                if _to_type(r[0]["entity_type"], EntityType) != EntityType.Source:
                    raise ConflictError("Entity %s already exists" % definition.qualified_name)
                attr: SourceAttributes = _to_type(json.loads(r[0]["attributes"]), SourceAttributes)
                if (
        ...
        with self.conn.transaction() as c:
            ...
            if r:
                ...
                if _to_type(r[0]["entity_type"], EntityType) != EntityType.Anchor:
                    raise ConflictError("Entity %s already exists" % definition.qualified_name)
                attr: AnchorAttributes = _to_type(json.loads(r[0]["attributes"]), AnchorAttributes)
            ...
            ref = EntityRef(r[0]["entity_id"], EntityType.Source, r[0]["qualified_name"])
            id = uuid4()
            if os.environ.get("FEATHR_SANDBOX"):
                query = db.insert(self.entities_table).values(
                    entity_id=str(id),
        ...
        with self.conn.transaction() as c:
            ...
            if r:
                ...
                if _to_type(r[0]["entity_type"], EntityType) != EntityType.AnchorFeature:
                    raise ConflictError("Entity %s already exists" % definition.qualified_name)
                attr: AnchorFeatureAttributes = _to_type(json.loads(r[0]["attributes"]), AnchorFeatureAttributes)
                if (
                    attr.name == definition.name
        ...
        with self.conn.transaction() as c:
            ...
            if r:
                ...
                if _to_type(r[0]["entity_type"], EntityType) != EntityType.DerivedFeature:
                    raise ConflictError("Entity %s already exists" % definition.qualified_name)
                attr: DerivedFeatureAttributes = _to_type(json.loads(r[0]["attributes"]), DerivedFeatureAttributes)
                if (
                    attr.name == definition.name
                    and attr.type == definition.feature_type
                    and attr.transformation == definition.transformation
    ...
    def _get_edges(self, ids: List[UUID], types: List[RelationshipType] = []) -> List[Edge]:
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            if len(types) > 0:
                query = self.sql_session.query(Edges.edge_id, Edges.from_id, Edges.to_id, Edges.conn_type).filter(
    ...
    def _get_entity(self, id_or_name: Union[str, UUID]) -> Entity:
        if os.environ.get("FEATHR_SANDBOX"):
            query = db.select(
                ...
                self.entities_table.c.entity_type,
                self.entities_table.c.attributes,
            ).where((self.entities_table.c.entity_id == str(self.get_entity_id(id_or_name))))
