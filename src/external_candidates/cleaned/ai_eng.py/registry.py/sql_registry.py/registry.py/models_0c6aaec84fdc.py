# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\sql-registry\registry\models.py
import json
import re
...
def to_snake(d, level: int = 0):
    ...
    return {
        to_snake(a, level + 1): (to_snake(b, level + 1) if isinstance(b, (dict, list)) else b) for a, b in d.items()
    }
...
class ExpressionTransformation(Transformation):
    ...
    def to_dict(self) -> Dict:
        return {"transformExpr": self.transform_expr}
...
class AnchorDef:
    ...
    def to_attr(self, source: EntityRef) -> AnchorAttributes:
        attr = AnchorAttributes(qualified_name=self.qualified_name, name=self.name, tags=self.tags)
        attr.source = source
        return attr
