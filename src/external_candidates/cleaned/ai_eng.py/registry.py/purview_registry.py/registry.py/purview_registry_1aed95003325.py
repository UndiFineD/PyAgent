...
class PurviewRegistry(Registry):
    ...
    def get_entity(self, id_or_name: Union[str, UUID], recursive=False) -> Entity:
        ...
        if recursive:
            ...
            if base_entity.entity_type == EntityType.Anchor:
                ...
                source_id = self.get_neighbors(base_entity.id, RelationshipType.Consumes)[0].to_id
    ...
    def get_lineage(self, id_or_name: Union[str, UUID]) -> EntitiesAndRelations:
        ...
        return EntitiesAndRelations(upstream_entities + downstream_entities, upstream_edges + downstream_edges)
    ...
    def get_project_features(self, project: str, keywords: Optional[str] = None) -> List[Entity]:
        ...
        if not project_id:
            return None
        guidAtlasEntityMap = self.purview_client.get_entity_lineage(project_id, depth=1, width=1, direction="OUTPUT")[
            "guidEntityMap"
        ]
        atlasEntities = []
    ...
    def create_project_datasource(self, project_id: UUID, definition: SourceDef) -> UUID:
        ...
        project_contains_source_relation = self._generate_relation_pairs(project_entity, source_entity, Label_Contains)
    ...
    def create_project_anchor(self, project_id: UUID, definition: AnchorDef) -> UUID:
        ...
        project_contains_anchor_relation = self._generate_relation_pairs(project_entity, anchor_entity, Label_Contains)
        anchor_consumes_source_relation = self._generate_relation_pairs(anchor_entity, source_entity, Label_Consumes)
        self._upload_entity_batch(project_contains_anchor_relation + anchor_consumes_source_relation)
    ...
    def create_project_anchor_feature(self, project_id: UUID, anchor_id: UUID, definition: AnchorFeatureDef) -> UUID:
        ...
        self._upload_entity_batch(
            project_contains_feature_relation + anchor_contains_feature_relation + feature_consumes_source_relation
        )
    ...
    def create_project_derived_feature(self, project_id: UUID, definition: DerivedFeatureDef) -> UUID:
        ...
        self._upload_entity_batch(feature_project_contain_belong_pairs + consume_produce_pairs)
    ...
    def _register_feathr_feature_types(self):
        ...
        type_feathr_sources = EntityTypeDef(
            ...
            attributeDefs=[
                ...
                AtlasAttributeDef(name="schemaStr", typeName="string", cardinality=Cardinality.SINGLE),
                AtlasAttributeDef(
        ...
        existing_entitydefs_names = (
            [entity["name"] for entity in existing_defs["entityDefs"]] if "entityDefs" in existing_defs else []
        )
        if existing_entitydefs_names and any(name in new_entitydefs_names for name in existing_entitydefs_names):
        ...
        def_result = self.purview_client.upload_typedefs(entityDefs=new_entitydefs, force_update=True)
        logger.info("Feathr Feature Type System Initialized.")
    ...
    def _upload_single_entity(self, entity: AtlasEntity):
        try:
            ...
            """
            response = self.purview_client.get_entity(qualifiedName=entity.qualifiedName, typeName=entity.typeName)[
                "entities"
            ][0]
            j = entity.to_json()
    ...
    def _generate_relation_pairs(self, from_entity: Entity, to_entity: Entity, relation_type):
        ...
        forward_relation = AtlasProcess(
            ...
            typeName="Process",
            qualified_name=self._generate_fully_qualified_name([relation_type, str(from_entity.id), str(to_entity.id)]),
