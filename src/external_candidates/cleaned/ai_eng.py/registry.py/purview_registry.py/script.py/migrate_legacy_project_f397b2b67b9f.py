...
def _generate_relation_pairs(from_entity, to_entity, relation_type):
    ...
    forward_relation = AtlasProcess(
        ...
        typeName="Process",
        qualified_name=registry_delimeter.join([relation_type, str(from_entity["guid"]), str(to_entity["guid"])]),
        inputs=[from_entity],
        outputs=[to_entity],
        guid=guid.get_guid(),
    )
...
def upload_single_entity_to_purview(purview_client, entity: Union[AtlasEntity, AtlasProcess]):
    ...
    try:
        ...
        if "PreConditionCheckFailed" in e.args[0]:
            entity.guid = purview_client.get_entity(qualifiedName=entity.qualifiedName, typeName=entity.typeName)[
                "entities"
            ][0]["guid"]
...
for entity in project_entities:
    ...
    for source in sources:
        ...
        relations = _generate_relation_pairs(project_entity, source_entity, RELATION_CONTAINS)
    ...
    for anchor in anchors:
        ...
        source_entity = None
        relations = []
        relations += _generate_relation_pairs(project_entity, anchor_entity, RELATION_CONTAINS)
        ...
        if anchor_entity["attributes"]["source"]:
            ...
            relations += _generate_relation_pairs(anchor_entity, source_entity, RELATION_CONSUMES)
        ...
        containing_anchor_features = [
            ...
            if x["qualifiedName"].startswith(project_entity["attributes"]["qualifiedName"] + "__" + anchor["name"])
        ]
        for containing_anchor_feature in containing_anchor_features:
            ...
            relations += _generate_relation_pairs(project_entity, anchor_feature_entity, RELATION_CONTAINS)
            if source_entity:
                relations += _generate_relation_pairs(anchor_feature_entity, source_entity, RELATION_CONSUMES)
        [upload_single_entity_to_purview(purview_client, x) for x in relations]
    ...
    for derived_feature in derived_features:
        ...
        relations = []
        relations += _generate_relation_pairs(project_entity, derived_feature_entity, RELATION_CONTAINS)
        ...
        for input_Feature_entity in input_feature_entities:
            relations += _generate_relation_pairs(derived_feature_entity, input_Feature_entity, RELATION_CONSUMES)
        [upload_single_entity_to_purview(purview_client, x) for x in relations]
