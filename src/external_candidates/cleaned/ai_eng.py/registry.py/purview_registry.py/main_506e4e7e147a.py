# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\purview-registry\main.py
import os
import traceback
from re import sub
...
from registry.purview_registry import ConflictError, PreconditionError, PurviewRegistry
    ...
    if isinstance(s, str):
        ...
        return dict(
            [
                (
                    to_camel(k),
                    (to_camel(s[k]) if isinstance(s[k], dict) or isinstance(s[k], list) else s[k]),
                )
                for k in s
            ]
        )
