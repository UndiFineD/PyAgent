# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\purview-registry\test\test_creation.py
...
source_id = registry.create_project_datasource(
    proj_id,
    SourceDef(
        name="source1",
        qualified_name="yihui_test_registry__source1",
        path="hdfs://somewhere",
        type="hdfs",
    ),
)

anchor1_id = registry.create_project_anchor(
    proj_id,
    AnchorDef(
        qualified_name="yihui_test_registry__anchor1",
        name="anchor1",
        source_id=source_id,
    ),
)
...
feature1 = registry.create_project_anchor_feature(
    proj_id,
    anchor1_id,
    AnchorFeatureDef(
        qualified_name="yihui_test_registry__anchor1__af1",
        name="af1",
        feature_type=ft1,
        transformation=t1,
        key=[k],
...
derived = registry.create_project_derived_feature(
    proj_id,
    DerivedFeatureDef(
        qualified_name="yihui_test_registry__df1",
        name="df1",
        feature_type=ft1,
        transformation=t1,
        key=[k],
        input_anchor_features=[feature1],
