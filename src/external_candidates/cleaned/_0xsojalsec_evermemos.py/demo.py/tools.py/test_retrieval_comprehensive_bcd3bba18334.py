...
class RetrievalTester:
    ...
    def print_summary(self):
        ...
        for data_source in self.data_sources:
            ...
            avg_latency = (
                sum(r.get("latency_ms", 0) for r in source_results if r.get("latency_ms")) / total if total > 0 else 0
            )
            print(
        ...
        for mode in self.retrieval_modes:
            ...
            avg_latency = (
                sum(r.get("latency_ms", 0) for r in mode_results if r.get("latency_ms")) / total if total > 0 else 0
            )
    ...
    def export_results(self, output_file: str = "demo/results/retrieval_test_results.json"):
        ...
        export_data = {
            ...
            "summary": {
                ...
                "success_rate": (
                    f"{(self.successful_tests / self.total_tests * 100):.1f}%" if self.total_tests > 0 else "0%"
                ),
                "total_elapsed_seconds": (
                    round(self.end_time - self.start_time, 2) if self.start_time and self.end_time else 0
                ),
                "total_request_time_ms": round(self.total_request_time, 2),
                "avg_request_time_ms": (
                    round(self.total_request_time / self.total_tests, 2) if self.total_tests > 0 else 0
                ),
...
async def main():
    ...
    print(f"     Current ({get_now_with_timezone().strftime('%Y-%m-%d')}): {result_current.get('count', 0)} items")
...
async def demo_foresight_evidence():
    ...
    try:
        ...
        print("   Please start service first: uv run python src/bootstrap.py src/run.py")
    except Exception as e:
        print(f"\n❌ Exception: {e}")
