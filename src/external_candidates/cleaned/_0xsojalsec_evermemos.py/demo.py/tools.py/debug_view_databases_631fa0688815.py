# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\demo\tools\debug_view_databases.py
#!/usr/bin/env python3
"""
                        ...
                        if resp.status == 200:
                            ...
                            if buckets:
                                for bucket in buckets:
                                    print(f"  - type={bucket['key']}: {bucket['doc_count']} items")
                            else:
...
async def main():
    ...
    parser.add_argument("--detail", action="store_true", help="Show detailed data (including samples)")
