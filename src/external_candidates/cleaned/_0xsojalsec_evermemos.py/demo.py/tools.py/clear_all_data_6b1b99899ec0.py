# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\demo\tools\clear_all_data.py
...
def _clear_milvus(verbose: bool = True, drop_collections: bool = False) -> Dict[str, Any]:
    ...
    for cls in collection_classes:
        ...
        try:
            ...
            for real_name in related_collections:
                ...
                stats["cleared"].append({"collection": real_name, "deleted": before_count, "dropped": True})
                if verbose:
                    print(f"      ✅ Milvus dropped collection {real_name} ({before_count} vectors)")
...
async def _clear_elasticsearch(verbose: bool = True, rebuild_index: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if rebuild_index:
                ...
                try:
                    ...
                    if isinstance(existing, dict):
                        for index_name in existing.keys():
                            ...
                            stats["cleared"].append({"alias": alias, "deleted_index": index_name})
                            if verbose:
            ...
            await initializer.initialize_indices([EpisodicMemoryDoc, ForesightDoc, EventLogDoc])
...
async def clear_all_memories(verbose: bool = True, rebuild_es: bool = False, drop_milvus: bool = False):
    ...
    try:
        ...
        if verbose:
            ...
            print(f"   - MongoDB deleted docs: {total_mongo_deleted} (Database: {mongo_stats.get('database')})")
            total_milvus_deleted = sum(item["deleted"] for item in milvus_stats.get("cleared", []))
            print(f"   - Milvus deleted vectors: {total_milvus_deleted}")
            if es_stats.get("recreated"):
                print("   - Elasticsearch: Indices and aliases recreated")
            else:
                total_es_deleted = sum(item["deleted"] for item in es_stats.get("cleared", []))
