...
async def test_basic_operations():
    ...
    cache = await factory.create_cache_manager(max_length=5, expire_minutes=5)  # Max 5 items, 5 minutes expiration
...
async def test_length_cleanup():
    ...
    cache = await factory.create_cache_manager(max_length=3, expire_minutes=10, cleanup_probability=0.0)
    ...
    assert stats["oldest_timestamp"] == expected_oldest, (
        f"Oldest timestamp mismatch: expected {expected_oldest}, actual {stats['oldest_timestamp']}"
    )
    assert stats["newest_timestamp"] == expected_newest, (
    ...
    assert stats_after["oldest_timestamp"] == expected_oldest_after, (
        f"After cleanup, oldest timestamp should be {expected_oldest_after}, actual {stats_after['oldest_timestamp']}"
    )
    assert stats_after["newest_timestamp"] == expected_newest_after, (
    ...
    assert additional_cleaned == 0, f"Second cleanup should delete 0 items, actually deleted {additional_cleaned}"
...
async def test_timestamp_compatibility():
    ...
    logger.info("Timestamp range: %s to %s", stats["oldest_datetime"], stats["newest_datetime"])
...
async def test_timestamp_range_query():
    ...
    factory = get_bean("redis_length_cache_factory")
    cache = await factory.create_cache_manager(max_length=20, expire_minutes=10, cleanup_probability=0.0)
    ...
    filtered_data = await cache.get_by_timestamp_range(test_key, start_timestamp=start_time)
    assert len(filtered_data) == 6, (
        f"Start time filtering failed, expected 6, actual {len(filtered_data)}"
    )  # data_4 to data_9
    ...
    assert len(filtered_data) == 5, (
        f"End time filtering failed, expected 5, actual {len(filtered_data)}"
    )  # data_0 to data_4
    ...
    range_data = await cache.get_by_timestamp_range(test_key, start_timestamp=start_time, end_timestamp=end_time)
    assert len(range_data) == 5, (
        f"Time range filtering failed, expected 5, actual {len(range_data)}"
    )  # data_2 to data_6
    ...
    assert len(dt_data) == 5, (
        f"Filtering with datetime objects failed, expected 5, actual {len(dt_data)}"
    )  # data_3 to data_7
    logger.info("Filtering with datetime objects test passed")

    # 9. Test empty result
    future_time = base_timestamp + 20 * 10000  # Future time
    empty_data = await cache.get_by_timestamp_range(test_key, start_timestamp=future_time)
    assert len(empty_data) == 0, f"Empty result test failed, should return 0 items, actual {len(empty_data)}"
...
async def test_json_pickle_mixed_data():
    ...
    for item in all_data:
        ...
        # Check JSON data
        if any(str(retrieved_data) == str(original["data"]) for original in test_data if original["type"] == "json"):
            json_count += 1
...
async def test_pickle_error_handling():
    ...
    all_data = await cache.get_by_timestamp_range(test_key)
    assert len(all_data) == 5, f"Mixed data total count error: expected 5, actual {len(all_data)}"  # 1 existing + 4 new

    # Count various data types
    json_count = sum(
        1
        for item in all_data
        if isinstance(item["data"], (dict, str)) and not isinstance(item["data"], NonSerializableTestClass)
    )
    pickle_count = sum(1 for item in all_data if isinstance(item["data"], NonSerializableTestClass))

    assert json_count >= 2, f"JSON data count abnormal: {json_count}"  # At least dict and string
    assert pickle_count >= 3, f"Pickle data count abnormal: {pickle_count}"  # 3 NonSerializableTestClass objects
...
async def test_stress_operations():
    ...
    factory = get_bean("redis_length_cache_factory")
    cache = await factory.create_cache_manager(max_length=100, expire_minutes=10, cleanup_probability=0.1)
    ...
    if final_size > 120:  # Allow some excess range
        ...
        assert size_after_cleanup <= 100, f"Queue size still exceeds limit after manual cleanup: {size_after_cleanup}"
...
async def test_expiry_mechanism():
    ...
    factory = get_bean("redis_length_cache_factory")
    cache = await factory.create_cache_manager(max_length=10, expire_minutes=1)  # 1 minute expiration
...
async def test_compatibility_layer():
    ...
    assert len(range_data) == 5, (
        f"Timestamp range query in backward compatibility layer failed, expected 5, actual {len(range_data)}"
    )  # 2 existing + 3 new
