# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_conversation_status_raw_repository.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
async def test_statistics():
    ...
    try:
        ...
        count = await repo.count_by_group_id(f"{base_group_id}_0")  # Test count for the first group
...
async def test_timezone_handling():
    ...
    try:
        ...
        assert queried.old_msg_start_time.astimezone(ZoneInfo("UTC")).replace(microsecond=0) == utc_time.replace(
            microsecond=0
        )
        assert queried.new_msg_start_time.astimezone(ZoneInfo("Asia/Tokyo")).replace(
            microsecond=0
        ) == tokyo_time.replace(microsecond=0)
        assert queried.last_memcell_time.replace(tzinfo=None).replace(microsecond=0) == shanghai_time.replace(
            microsecond=0
        )
