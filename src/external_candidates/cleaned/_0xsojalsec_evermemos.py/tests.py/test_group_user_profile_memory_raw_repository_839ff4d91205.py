# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_group_user_profile_memory_raw_repository.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
async def test_basic_crud_operations():
    ...
    try:
        ...
        profile_data = {
            "version": "v1",
            "user_name": "Zhang San",
            "hard_skills": [{"value": "Python", "level": "Advanced", "evidences": ["conv_001"]}],
...
async def test_version_management():
    ...
    try:
        ...
        v2_by_range = await repo.get_by_user_group(user_id, group_id, version_range=("202502", "202502"))
        ...
        updated_v2 = await repo.update_by_user_group(user_id, group_id, update_v2, version="202502")
...
async def test_ensure_latest():
    ...
    try:
        ...
        for old_version in ["202501", "202502", "202503"]:
            old_doc = await repo.get_by_user_group(user_id, group_id, version_range=(old_version, old_version))
...
async def test_batch_query_with_only_latest():
    ...
    try:
        ...
        latest_results = await repo.get_by_user_ids(user_ids, group_id=group_id, only_latest=True)
        ...
        all_results = await repo.get_by_user_ids(user_ids, group_id=group_id, only_latest=False)
...
async def test_batch_get_by_user_groups():
    ...
    try:
        ...
        for user_id, group_id, user_name in test_data:
            ...
            new_data = {
                ...
                "group_importance_evidence": {
                    "evidence_list": [{"speak_count": 10, "refer_count": 5, "conversation_count": 20}]
                },
            }
        ...
        for (user_id, group_id), profile in results.items():
            assert profile is not None, f"Profile for user {user_id} in group {group_id} should not be None"
            assert profile.version == "v2", f"Should return latest version v2, actually returned {profile.version}"
