# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_migrate_user_goal_to_work_responsibility.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
async def get_group_user_profile_memory_repository():
    ...
    try:
        ...
        logger.info(f"✅ Successfully obtained GroupUserProfileMemoryRawRepository: {type(group_repo)}")
        return group_repo
    except Exception as e:
...
async def migrate_user_goal_to_work_responsibility(repo: CoreMemoryRawRepository, records: List[CoreMemory]):
    ...
    for record in records:
        try:
            ...
            record.work_responsibility = record.user_goal  # Copy user_goal to work_responsibility
...
async def verify_migration_results(repo: CoreMemoryRawRepository):
    ...
    try:
        ...
        remaining_user_goals = await CoreMemory.find({"user_goal": {"$ne": None}}).to_list()
        ...
        print(f"   Records with work_responsibility data: {len(records_with_work_responsibility)}")
...
async def preview_group_migration_data():
    ...
    try:
        ...
        while True:
            ...
            # Batch query, use sorting to ensure data consistency, use skip and limit
            batch_records = (
                await GroupUserProfileMemory.find({"user_goal": {"$ne": None, "$exists": True}})
                .sort([("_id", 1)])
                .skip(total_count)
                .limit(batch_size)
                .to_list()
        ...
        if total_count > 0:
            print("\n📋 Preview of first 5 records:")
            for i, record in enumerate(all_records[:5]):
                print(f"  {i + 1}. User ID: {record.user_id}, Group ID: {record.group_id}")
                print(f"     user_goal: {record.user_goal}")
                print(f"     work_responsibility: {record.work_responsibility}")
                print()
...
async def migrate_group_user_goal_to_work_responsibility(
    ...
    for record in records:
        try:
            # Check if there is user_goal data
            if not record.user_goal:
                logger.warning(f"⚠️  Group record {record.user_id}-{record.group_id} has empty user_goal, skipping")
                continue
            ...
            error_count += 1
            logger.error(f"❌ Error migrating group record {record.user_id}-{record.group_id}: {e}")
...
async def verify_group_migration_results():
    ...
    try:
        # Check if there are still records where user_goal is not null
        remaining_user_goals = await GroupUserProfileMemory.find({"user_goal": {"$ne": None}}).to_list()
    ...
    for record in records:
        try:
                ...
                if isinstance(record.soft_skills, list):
                    for skill_item in record.soft_skills:
                        if isinstance(skill_item, dict) and "skill" in skill_item:
                            ...
                            updated = True
                elif isinstance(record.soft_skills, dict) and "skill" in record.soft_skills:
                ...
                if isinstance(record.hard_skills, list):
                    for skill_item in record.hard_skills:
                        if isinstance(skill_item, dict) and "skill" in skill_item:
                            skill_item["value"] = skill_item.pop("skill")
                            updated = True
                elif isinstance(record.hard_skills, dict) and "skill" in record.hard_skills:
            ...
            if updated:
                ...
                await record.save()
                success_count += 1
                logger.info(f"✅ Successfully renamed skill fields in CoreMemory record {record.user_id}")
                print(f"✅ Rename successful: {record.user_id}")
            else:
                logger.warning(f"⚠️  CoreMemory record {record.user_id} has no skill fields to rename")
...
async def verify_skills_rename_results():
    ...
    try:
        ...
        print(f"   GroupUserProfileMemory records with remaining skill fields: {len(group_remaining_skill)}")
        print(f"   CoreMemory records with value fields: {len(core_records_with_value)}")
        print(f"   GroupUserProfileMemory records with value fields: {len(group_records_with_value)}")
...
async def execute_user_goal_migration():
    ...
    try:
        ...
        if core_records_to_migrate:
            ...
            core_success_count, core_error_count = await migrate_user_goal_to_work_responsibility(
        ...
        if group_records_to_migrate:
            ...
            group_success_count, group_error_count = await migrate_group_user_goal_to_work_responsibility(
        ...
        print(f"   📈 Total: {len(core_records_to_migrate) if core_records_to_migrate else 0} records")

        print(f"\n👥 Group User Profile Memory table:")
        print(f"   ✅ Success: {group_success_count} records")
        print(f"   ❌ Failure: {group_error_count} records")
        print(f"   📈 Total: {len(group_records_to_migrate) if group_records_to_migrate else 0} records")
...
async def execute_skills_rename():
    ...
    try:
        ...
        if core_skill_records:
            ...
            core_skill_success_count, core_skill_error_count = await rename_skill_to_value_in_core_memory(
                core_skill_records
            )
        else:
        ...
        if group_skill_records:
            ...
            group_skill_success_count, group_skill_error_count = await rename_skill_to_value_in_group_memory(
                group_skill_records
            )
        else:
        ...
        print(f"   📈 Total: {len(group_skill_records) if group_skill_records else 0} records")
