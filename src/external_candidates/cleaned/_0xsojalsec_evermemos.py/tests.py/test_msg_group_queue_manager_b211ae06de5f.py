...
async def test_time_window_stats():
    ...
    consumed_count = 0
    for i in range(3):
        target_queue = manager._hash_route(f"time_user_{i}")  # pylint: disable=protected-access
        message = await manager.get_by_queue(target_queue, wait=False)
        if message:
            consumed_count += 1
...
async def test_basic_concurrent_operations():
    ...
    manager = MsgGroupQueueManager("concurrent_test", num_queues=5, max_total_messages=50)

    async def producer(producer_id: int):
        for i in range(10):
            await manager.deliver_message(f"producer_{producer_id}_msg_{i}", f"data_{i}")
    ...
    stats = await manager.get_manager_stats()
    assert stats["total_delivered_messages"] == 30, (
...
async def test_basic_shutdown_modes():
    ...
    manager = MsgGroupQueueManager("basic_shutdown_test", num_queues=3, max_total_messages=10)
    ...
    assert 0.8 <= total_elapsed <= 1.5, f"Soft shutdown total time abnormal: {total_elapsed:.2f}s, expected ~1.0s"
...
async def test_edge_cases():
    ...
    complex_queue = manager._hash_route("complex_test")  # pylint: disable=protected-access
...
async def test_timeout_mechanism():
    ...
    try:
        ...
        try:
            ...
            assert 0.4 <= end_time - start_time <= 0.6, f"Timeout time inaccurate: {end_time - start_time}"
        ...
        target_queue = manager._hash_route("timeout_user")  # pylint: disable=protected-access
        ...
        assert end_time - start_time < 0.1, f"Should not wait when message exists: {end_time - start_time}"
...
async def test_routing_uniformity():
    ...
    manager = MsgGroupQueueManager("routing_test", num_queues=10, max_total_messages=2000)
    ...
    try:
        ...
        for i, group_key in enumerate(uuid_keys):
            ...
            target_queue = manager._hash_route(group_key)  # pylint: disable=protected-access
        ...
        variance = sum((queue_counts[i] - avg_count) ** 2 for i in range(manager.num_queues)) / manager.num_queues
        ...
        assert coefficient_of_variation < 15.0, (
        ...
        assert max_min_ratio < 2.0, f"Queue load difference too large, max/min ratio: {max_min_ratio:.2f}"
        ...
        for test_key in consistency_test_keys:
            ...
            routes = [manager._hash_route(test_key) for _ in range(10)]  # pylint: disable=protected-access
            assert len(set(routes)) == 1, f"Routing inconsistent: key={test_key}, routes={set(routes)}"
...
async def test_concurrent_operations():
    ...
    manager = MsgGroupQueueManager("concurrent_test", num_queues=5, max_total_messages=100)
    ...
    async def consumer(consumer_id: int, target_queues: List[int]):
        ...
        for queue_id in target_queues:
            while True:
                try:
                    message = await manager.get_by_queue(queue_id, wait=True, timeout=0.1)
                    if message is None:
                        break
                    consumed += 1
    ...
    assert stats["total_delivered_messages"] == 15, f"Delivery count error: {stats['total_delivered_messages']}"
...
async def test_real_world_scenario():
    ...
    manager = MsgGroupQueueManager("kafka_simulator", num_queues=10, max_total_messages=200)
    ...
    async def consumer_worker(queue_ids: List[int]):
        ...
        for queue_id in queue_ids:
            while True:
                try:
                    message = await manager.get_by_queue(queue_id, wait=True, timeout=0.05)
                    if message is None:
                        break
                    consumed += 1
...
async def test_queue_overflow_and_recovery():
    ...
    for i in range(5):
        success = await manager.deliver_message(f"recovery_user_{i}", f"recovery_msg_{i}")
        if success:
...
async def test_shutdown_modes_integration():
    ...
    try:
        ...
        results = await asyncio.gather(shutdown_task(), consumer_task(), return_exceptions=True)
        ...
        assert isinstance(shutdown_result, (bool, dict)), f"Shutdown result type error: {type(shutdown_result)}"
        assert isinstance(consumed_count, int), f"Consumption count type error: {type(consumed_count)}"
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        ...
        if passed_count == total_count:
            ...
            print(f"⚠️  {total_count - passed_count} tests failed, please check the error messages above")
