# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_memory_controller.py
"""
...
class MemoryControllerTester:
    ...
    def init_database(self) -> bool:
        ...
        print(f"📤 Tenant Info: organization_id={self.organization_id}, space_id={self.space_id}")
    ...
    def call_patch_api(self, endpoint: str, data: dict):
        ...
        try:
            response = requests.patch(url, json=data, headers=headers, params=params, timeout=self.timeout)
            print(f"\n📥 Response Status Code: {response.status_code}")
            print("📥 Response Data:")
            response_json = response.json()
            print(json.dumps(response_json, indent=2, ensure_ascii=False))
    ...
    def test_memorize_single_message(self):
        ...
        messages = [
            ...
            },
            {
                ...
                "group_name": "Test Group",
                "message_id": "msg_004",
                "create_time": (base_time + timedelta(minutes=1, seconds=30)).isoformat(),
                "sender": "assistant_001",
                "sender_name": "AI Assistant",
            ...
            },
            {
                ...
                "group_name": "Test Group",
                "message_id": "msg_006",
                "create_time": (base_time + timedelta(hours=24, seconds=30)).isoformat(),
                "sender": "assistant_001",
                "sender_name": "AI Assistant",
        ...
        for i, msg in enumerate(messages, 1):
            ...
            assert status_code == 200, f"Message {i} status code should be 200, actual: {status_code}"
        ...
        if result["count"] > 0:
            ...
            for idx, memory in enumerate(result["saved_memories"], 1):
                ...
                summary = memory.get("summary", memory.get("content", "no summary"))[:50]
    ...
    def test_fetch_episodic(self):
        ...
        assert response.get("status") == "ok", f"Status should be ok, actual: {response.get('status')}"
        ...
        if result["total_count"] > 0 and len(result["memories"]) > 0:
            for idx, memory in enumerate(result["memories"]):
                ...
                assert memory.get("user_id") == self.user_id, f"Memory {idx} user_id should match"
    ...
    def test_fetch_foresight(self):
        ...
        if result["total_count"] > 0 and len(result["memories"]) > 0:
            for idx, memory in enumerate(result["memories"]):
                ...
                assert "parent_episode_id" in memory, f"Memory {idx} should contain parent_episode_id"
            ...
            print(f"✅ Fetch Personal Foresight successful, returned {result['total_count']} personal foresights")
    ...
    def test_search_memories_keyword(self):
        ...
        if result["total_count"] > 0 and len(result["memories"]) > 0:
            ...
            assert len(result["memories"]) == len(result["scores"]), "memories and scores should have same length"

            # Iterate through each group's memories
            for group_idx, memory_group in enumerate(result["memories"]):
                assert isinstance(memory_group, dict), f"Memory group {group_idx} should be a dictionary"
                ...
                for group_id, memory_list in memory_group.items():
                    ...
                    assert isinstance(memory_list, list), f"Memory list for group {group_id} should be a list"
                    ...
                    for mem_idx, mem in enumerate(memory_list):
                        ...
                        assert "memory_type" in mem, f"Memory {mem_idx} should contain memory_type"
                        assert "user_id" in mem, f"Memory {mem_idx} should contain user_id"
                        assert "timestamp" in mem, f"Memory {mem_idx} should contain timestamp"
    ...
    def test_search_memories_vector(self):
        ...
        if result["total_count"] > 0:
            assert "importance_scores" in result, "Vector search result should contain importance_scores field"
            assert isinstance(result["importance_scores"], list), "importance_scores should be a list"
    ...
    def test_search_memories_hybrid(self):
        ...
        if result["total_count"] > 0:
            assert "importance_scores" in result, "Hybrid search result should contain importance_scores field"
            assert isinstance(result["importance_scores"], list), "importance_scores should be a list"

            # Validate source in metadata
            metadata = result["metadata"]
            assert metadata.get("source") == "hybrid", "Hybrid search source should be hybrid"
    ...
    def test_save_conversation_meta(self):
        ...
        self.print_section("Test 8: POST /api/v1/memories/conversation-meta - Save Conversation Metadata")
    ...
    def test_patch_conversation_meta(self):
        ...
        self.print_section("Test 9: PATCH /api/v1/memories/conversation-meta - Partially Update Conversation Metadata")
        ...
        if status_code == 200:
            ...
            assert result["group_id"] == self.group_id, "Returned group_id should match"
            assert isinstance(result["updated_fields"], list), "updated_fields should be a list"
            ...
            print(f"⚠️  Patch Conversation Meta failed: {response.get('message', 'Unknown error')}")
    ...
    def run_all_tests(self, test_method: str = "all", except_test_methods: str = None):
        ...
        try:
            if except_test_methods:
                ...
                methods_to_run = [
                    (name, method) for name, method in test_methods.items() if name not in excluded_methods
                ]
...
def main():
    ...
    if not args.organization_id:
        print(f"⚠️  --organization-id not provided, using default: {MemoryControllerTester.DEFAULT_ORGANIZATION_ID}")
    if not args.space_id:
        print(f"⚠️  --space-id not provided, using default: {MemoryControllerTester.DEFAULT_SPACE_ID}")
    if not args.hash_key:
        print(f"⚠️  --hash-key not provided, using default: {MemoryControllerTester.DEFAULT_HASH_KEY}")
    ...
    tester.run_all_tests(test_method=args.test_method, except_test_methods=args.except_test_method)
