# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\core\pipeline.py
"""
...
from evaluation.src.utils.logger import get_console, setup_logger
...
class Pipeline:
    ...
    async def run(
        ...
        if self.filter_categories:
            ...
            dataset.qa_pairs = [qa for qa in dataset.qa_pairs if qa.category not in filter_set]
        ...
        if add_just_completed:
            ...
            if wait_seconds > 0 and "search" in stages:
                ...
                with Progress(
                    ...
                    task = progress.add_task(f"⏰ Backend indexing in progress...", total=wait_seconds)
        ...
        if "search" in stages and "search" not in self.completed_stages:
            ...
            if self.checkpoint:
                ...
                self.checkpoint.save_checkpoint(self.completed_stages, search_results=search_results_data)
        ...
        if "evaluate" in stages and "evaluate" not in self.completed_stages:
            ...
            self.saver.save_json(self._eval_result_to_dict(eval_result), "eval_results.json")
            ...
            if self.checkpoint:
                self.checkpoint.save_checkpoint(
                    ...
                    answer_results=(
                        answer_results_data
                        if answer_results_data
                        else [self._answer_result_to_dict(ar) for ar in answer_results]
                    ),
    ...
    def _apply_smoke_test(self, dataset: Dataset, num_messages: int, num_questions: int) -> Dataset:
        ...
        for conv in dataset.conversations:
            ...
            conv_qa_pairs = [qa for qa in dataset.qa_pairs if qa.metadata.get("conversation_id") == conv_id]
        ...
        msg_desc = (
            f"{total_messages_after}/{total_messages_before}" if num_messages > 0 else f"{total_messages_after} (all)"
        )
        qa_desc = (
        ...
        self.logger.info(f"Smoke test: {conv_desc} - {msg_desc} messages, {qa_desc} questions")
    ...
    def _apply_conversation_range(self, dataset: Dataset, from_conv: int, to_conv: Optional[int]) -> Dataset:
        ...
        selected_qa_pairs = [qa for qa in dataset.qa_pairs if qa.metadata.get("conversation_id") in selected_conv_ids]
