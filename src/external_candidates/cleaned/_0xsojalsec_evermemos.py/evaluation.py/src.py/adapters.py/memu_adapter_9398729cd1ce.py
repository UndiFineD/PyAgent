# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\adapters\memu_adapter.py
"""
...
class MemuAdapter(OnlineAPIAdapter):
    ...
    def __init__(self, config: dict, output_dir: Path = None):
        ...
        if self.valid_users:
            self.console.print(
                f"   Valid Users Filter: {len(self.valid_users)} user(s)",
                style="yellow",
            )
        if self.mock_mode:
            self.console.print(
                f"   🧪 Mock Mode: ENABLED (API calls will be simulated)",
                style="bold yellow",
            )

        # Force sequential processing (override num_workers)
        self.console.print(
            f"   🔄 Sequential Mode: ENABLED (all operations are serial)",
            style="bold cyan",
        )
    ...
    async def add(self, conversations: List[Conversation], **kwargs) -> Dict[str, Any]:
        ...
        for conv in conversations:
            ...
            if conv_info["need_dual_perspective"]:
                ...
                result_a = await self._add_user_messages(conv, speaker_a_messages, speaker="speaker_a", **kwargs)

                # Wait for speaker_a's task to complete
                await self._wait_for_conversation_tasks([result_a], conversation_id=conv_id, **kwargs)

                # Add messages for speaker_b second (sequential)
                result_b = await self._add_user_messages(conv, speaker_b_messages, speaker="speaker_b", **kwargs)

                # Wait for speaker_b's task to complete
                await self._wait_for_conversation_tasks([result_b], conversation_id=conv_id, **kwargs)
                ...
                messages = self._conversation_to_messages(conv, format_type=format_type, perspective=None)

                # Add messages for single user
                result = await self._add_user_messages(conv, messages, speaker="speaker_a", **kwargs)

                # Wait for task to complete
                await self._wait_for_conversation_tasks([result], conversation_id=conv_id, **kwargs)
    ...
    async def _add_user_messages(
        ...
        if self.valid_users is not None and user_id not in self.valid_users:
            self.console.print(
                f"   ⏭️  Skipping {user_name} ({user_id}): not in valid_users",
                style="dim yellow",
            )
            return None
        ...
        if conv.messages and conv.messages[0].timestamp:
            ...
            session_date = get_now_with_timezone().strftime("%Y-%m-%d")
        ...
        self.console.print(
            f"   📤 Adding for {user_name} ({user_id}): {len(messages)} messages",
            style="dim",
        )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                response = await loop.run_in_executor(
                    None, lambda: requests.post(url, headers=self.headers, json=payload)
                )
    ...
    async def _wait_for_conversation_tasks(self, task_results: List[Any], **kwargs) -> None:
        """
    ...
    def _build_add_result(self, conversation_ids: List[str], add_results: List[Any], **kwargs) -> Dict[str, Any]:
        """
    ...
    async def _wait_for_all_tasks(self, task_ids: List[str], conversation_id: str = "unknown") -> bool:
        """
        ...
        while time.time() - start_time < self.task_timeout:
            ...
            for task_id in list(pending_tasks):
                try:
                    ...
                    # Use run_in_executor to avoid blocking event loop
                    loop = asyncio.get_event_loop()
                    response = await loop.run_in_executor(None, lambda: requests.get(url, headers=self.headers))
                    response.raise_for_status()
                    result = response.json()
                    status = result.get("status")
