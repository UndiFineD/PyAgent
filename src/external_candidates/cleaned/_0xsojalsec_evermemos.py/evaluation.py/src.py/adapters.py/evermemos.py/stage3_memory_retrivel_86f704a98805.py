...
def multi_rrf_fusion(results_list: List[List[Tuple[dict, float]]], k: int = 60) -> List[Tuple[dict, float]]:
    """
...
async def lightweight_retrieval(
    ...
    if search_mode == "bm25_only":
        ...
        bm25_results = await asyncio.to_thread(search_with_bm25_index, query, bm25, docs, config.lightweight_bm25_top_n)
        ...
        emb_results = await search_with_emb_index(query, emb_index, top_n=config.lightweight_emb_top_n)
        ...
        emb_task = search_with_emb_index(query, emb_index, top_n=config.lightweight_emb_top_n)
        bm25_task = asyncio.to_thread(search_with_bm25_index, query, bm25, docs, config.lightweight_bm25_top_n)
...
async def hybrid_search_with_rrf(
    ...
    emb_task = search_with_emb_index(query, emb_index, top_n=emb_candidates, query_embedding=query_embedding)
...
async def agentic_retrieval(
    ...
    round2_unique = [(doc, score) for doc, score in round2_results if doc.get("event_id", id(doc)) not in round1_ids]
...
async def reranker_search(
        ...
        for attempt in range(max_retries):
            try:
                ...
                batch_results = await asyncio.wait_for(
                    reranker._make_rerank_request(query, batch_texts, instruction=reranker_instruction),
                    timeout=timeout,
                )
    ...
    for group_start in range(0, len(batches), max_concurrent):
        ...
        tasks = [process_batch_with_retry(start_idx, batch) for start_idx, batch in group_batches]
    ...
    sorted_results = sorted(all_rerank_results, key=lambda x: x["score"], reverse=True)[:top_n]

    # Map back to original documents
    final_results = [(results[original_indices[item["global_index"]]][0], item["score"]) for item in sorted_results]
    ...
    for i, conversation_data in enumerate(dataset):
        ...
        if config.use_hybrid_search:
            ...
            if not emb_index_path.exists():
                print(f"Error: Embedding index not found at {emb_index_path}. Skipping conversation.")
                continue
            ...
            if not bm25_index_path.exists():
                print(f"Error: BM25 index not found at {bm25_index_path}. Skipping conversation.")
                continue
                    ...
                    if config.retrieval_mode == "agentic":
                        ...
                        if config.use_reranker:
                            ...
                            if config.use_hybrid_search:
                                ...
                                top_results = await search_with_emb_index(query=question, emb_index=emb_index, top_n=20)
                            else:
                                top_results = await asyncio.to_thread(search_with_bm25_index, question, bm25, docs, 20)
