# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\adapters\evermemos\stage1_memcells_extraction.py
import asyncio
import json
import os
import sys
...
async def _extract_all_memories_for_memcell(
    ...
    if episode_memory and episode_memory.episode:
        ...
        if foresight_extractor:
            foresight_memories = await foresight_extractor.generate_foresight_memories_for_episode(episode_memory)
...
async def process_single_conversation(
    ...
    if event_log_extractor:
        memcells_with_episode = [
            ...
            if hasattr(memcell, "episode") and memcell.episode and memcell.episode != "Episode extraction failed"
        ]
        ...
        event_log_tasks = [extract_with_semaphore(idx, memcell) for idx, memcell in memcells_with_episode]
    ...
    if cluster_mgr and cluster_state:
        ...
        for memcell in memcell_list:
            ...
            cluster_id, cluster_state = await cluster_mgr.cluster_memcell(memcell_dict, cluster_state)
...
async def main():
    ...
    os.makedirs(os.path.join(CURRENT_DIR, config.experiment_name, "memcells"), exist_ok=True)
    ...
    pending_raw_data_dict = {
        conv_id: conv_data for conv_id, conv_data in raw_data_dict.items() if conv_id not in completed_convs
    }
    ...
    summary = {
        ...
        "conversation_results": {conv_id: len(memcell_list) for conv_id, memcell_list in results},
        "clustering_summary": {
            "total_clusters": total_clusters,
