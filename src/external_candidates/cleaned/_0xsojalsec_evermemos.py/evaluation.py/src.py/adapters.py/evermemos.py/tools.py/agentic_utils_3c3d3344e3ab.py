...
def format_documents_for_llm(results: List[Tuple[dict, float]], max_docs: int = 10, use_episode: bool = True) -> str:
    """
...
async def check_sufficiency(
    ...
    try:
        ...
        retrieved_docs = format_documents_for_llm(results, max_docs=max_docs, use_episode=True)

        # Use prompt template
        prompt = SUFFICIENCY_CHECK_PROMPT.format(query=query, retrieved_docs=retrieved_docs)
