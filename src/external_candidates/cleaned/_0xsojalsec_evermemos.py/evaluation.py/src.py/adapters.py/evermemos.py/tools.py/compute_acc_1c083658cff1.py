...
def compute_accuracy_from_judgments(file_path):
    ...
    for user_id, results in data.items():
        if results:
            ...
            if "llm_judgments" in first_item and isinstance(first_item["llm_judgments"], dict):
    ...
    accuracies = [(correct / total_questions) * 100 if total_questions > 0 else 0 for correct in judgment_correct]
