# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\adapters\evermemos\stage5_eval.py
import asyncio
import json
import logging
import time
...
async def process_group_responses(group_id, group_responses, oai_client, num_runs: int, disable_progress: bool = False):
    graded_responses = []
    ...
    for response in tqdm(
        ...
        grading_tasks = [locomo_grader(oai_client, question, ground_truth, answer) for _ in range(num_runs)]
...
async def process_single_group(group_id, group_responses, oai_client, num_runs, disable_progress: bool = False):
    try:
        start_time = time.time()
        result = await process_group_responses(group_id, group_responses, oai_client, num_runs, disable_progress)
        end_time = time.time()
...
async def main():
    ...
    oai_client = AsyncOpenAI(api_key=llm_config["api_key"], base_url=llm_config["base_url"])
    ...
    total_responses_count = sum(len(locomo_responses.get(f"locomo_exp_user_{i}", [])) for i in range(num_users))
    ...
    for group_idx in range(num_users):
        ...
        tasks.append(
            process_single_group(
                ...
                oai_client,
                num_runs,
                disable_progress=True,  # Disable individual group progress bar
            )
        )
