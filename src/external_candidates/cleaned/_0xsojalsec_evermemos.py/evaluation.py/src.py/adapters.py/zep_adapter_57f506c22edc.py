# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\adapters\zep_adapter.py
"""
...
class ZepAdapter(OnlineAPIAdapter):
    ...
    def __init__(self, config: dict, output_dir: Path = None):
        ...
        try:
            ...
            raise ImportError("Zep client not installed. Please install: pip install zep-cloud")
    ...
    def _build_single_search_result(
        ...
        **kwargs,
        ...
        facts_text = "\n".join(
            [f"  - {r['metadata']['fact']} (event_time: {r['metadata']['valid_at']})" for r in facts]
        )
        entities_text = "\n".join([f"  - {r['metadata']['name']}: {r['metadata']['summary']}" for r in entities])
