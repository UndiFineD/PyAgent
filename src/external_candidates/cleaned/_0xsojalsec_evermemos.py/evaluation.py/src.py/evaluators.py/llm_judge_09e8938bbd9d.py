# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\evaluators\llm_judge.py
"""
...
from evaluation.src.utils.prompts import format_prompt, get_prompt
...
class LLMJudge(BaseEvaluator):
    ...
    async def _evaluate_single_answer(self, answer_result: AnswerResult) -> dict:
        ...
        for _ in range(self.num_runs):
            is_correct = await self._judge_answer(question, golden_answer, generated_answer)
            judgments.append(is_correct)

        # Use judgment_1, judgment_2, ... format
        llm_judgments = {f"judgment_{i + 1}": judgment for i, judgment in enumerate(judgments)}
    ...
    async def _judge_answer(self, question: str, golden_answer: str, generated_answer: str) -> bool:
        """
