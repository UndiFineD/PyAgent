# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\evaluators\registry.py
"""
...
def _ensure_evaluator_loaded(name: str):
    ...
    if name not in _EVALUATOR_MODULES:
        raise ValueError(f"Unknown evaluator: {name}. Available evaluators: {list(_EVALUATOR_MODULES.keys())}")
