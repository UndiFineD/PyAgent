# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\biz_layer\mem_db_operations.py
"""
...
def _normalize_datetime_for_storage(timestamp: Any, current_time: Optional[datetime] = None) -> datetime:
    """
...
def _convert_timestamp_to_time(timestamp: Any, current_time: Optional[datetime] = None) -> str:
    """
...
def _convert_group_importance_evidence_to_document(
    ...
    return {
        ...
        "evidence_list": _convert_importance_evidence_to_document(group_importance_evidence.evidence_list),
    }
...
def _convert_document_to_group_importance_evidence(
    ...
    return GroupImportanceEvidence(
        group_id=group_importance_evidence["group_id"],
        is_important=group_importance_evidence["is_important"],
        evidence_list=_convert_document_to_importance_evidence(group_importance_evidence["evidence_list"]),
    )


def _convert_episode_memory_to_doc(episode_memory: Any, current_time: Optional[datetime] = None) -> EpisodicMemory:
    """
    ...
    return EpisodicMemory(
        ...
        episode=(episode_memory.episode if hasattr(episode_memory, "episode") else episode_memory.summary or ""),
...
def _convert_foresight_to_doc(
    ...
    return ForesightRecord(
        ...
        user_name=getattr(foresight, "user_name", getattr(parent_doc, "user_name", None)),
...
def _convert_group_profile_data_to_profile_format(
    ...
    topics = []
    if hasattr(group_profile_memory, "topics") and group_profile_memory.topics is not None:
    ...
    roles = {}
    if hasattr(group_profile_memory, "roles") and group_profile_memory.roles is not None:
    ...
    subject = getattr(group_profile_memory, "theme", None) or getattr(group_profile_memory, "subject", None)
...
def _convert_document_to_project_info(project_info: Dict[str, str]) -> ProjectInfo:
    ...
    def _process_field_with_evidences(value):
        ...
        if isinstance(value, list):
            if value and isinstance(value[0], dict) and ("value" in value[0] or "evidences" in value[0]):
                return value
            ...
            try:
                ...
                if isinstance(parsed_value, list):
                    ...
                    if (
                        ...
                        and ("value" in parsed_value[0] or "evidences" in parsed_value[0])
                    ):
                        return parsed_value
                    # Otherwise convert to canonical format
                    return [{"value": str(item), "evidences": []} for item in parsed_value if item]
    ...
    return ProjectInfo(
        ...
        user_objective=_process_field_with_evidences(project_info.get("user_objective")),
...
def _convert_profile_data_to_core_format(profile_memory: ProfileMemory) -> CoreMemory:
    ...
    if hasattr(profile_memory, "motivation_system") and profile_memory.motivation_system:
    ...
    if hasattr(profile_memory, "way_of_decision_making") and profile_memory.way_of_decision_making:
    ...
    if hasattr(profile_memory, "projects_participated") and profile_memory.projects_participated:
        if isinstance(profile_memory.projects_participated, list):
            ...
            for project in profile_memory.projects_participated:
                if hasattr(project, "project_id"):  # ProjectInfo object
                    ...
                    project_dict = {
                        "project_id": (str(project.project_id) if project.project_id else ""),
                        "project_name": (str(project.project_name) if project.project_name else ""),
                        "entry_date": (str(project.entry_date) if project.entry_date else ""),
    ...
    group_importance_evidence = getattr(profile_memory, "group_importance_evidence", None)
    ...
    return {
        ...
        "group_importance_evidence": _convert_group_importance_evidence_to_document(group_importance_evidence),
    }


def _convert_memcell_to_document(memcell: MemCell, current_time: Optional[datetime] = None) -> DocMemCell:
    """
    ...
    try:
        ...
        if memcell.type == RawDataType.CONVERSATION:
            for raw_data_dict in memcell.original_data:
                ...
                message = {
                    "content": raw_data_dict.get("content") or "",  # Handle None content explicitly
                    "extend": {
                        "speaker_id": to_string(raw_data_dict.get("speaker_id", "")),
                        "speaker_name": to_string(raw_data_dict.get("speaker_name", "")),
                        "timestamp": to_string(_convert_timestamp_to_time(raw_data_dict.get("timestamp", ""))),
        ...
        if memcell.timestamp:
            try:
                ...
                if isinstance(memcell.timestamp, datetime):
                    ...
                    timestamp_dt = _normalize_datetime_for_storage(memcell.timestamp * 1000)
        ...
        if hasattr(memcell, "foresight_memories") and memcell.foresight_memories:
            ...
            foresight_memories_list = [sm for sm in foresight_memories_list if sm is not None]
        ...
        doc_memcell = DocMemCell(
            ...
            extend=(extend_dict if extend_dict else None),  # ✅ Add extend (contains embedding)
        )
...
async def _save_memcell_to_database(memcell: MemCell, current_time: datetime) -> MemCell:
    """
    ...
    try:
        ...
        if doc_memcell is None:
            logger.warning(f"MemCell conversion skipped, cannot save: {memcell.event_id}")
            return
        ...
        if result:
            ...
            logger.info(f"[mem_db_operations] MemCell saved successfully: {memcell.event_id}")
            ...
            try:
                ...
                logger.debug(f"[mem_db_operations] MemCellCreatedEvent published: {memcell.event_id}")
            except Exception as e:
                logger.warning(f"[mem_db_operations] Failed to publish MemCellCreatedEvent: {e}")
        else:
...
async def _save_group_profile_memory(
    ...
    try:
        # Convert data format
        converted_data = _convert_group_profile_data_to_profile_format(group_profile_memory)
        ...
        await group_profile_raw_repo.upsert_by_group_id(group_profile_memory.group_id, save_data, timestamp=timestamp)
...
async def _update_status_for_continuing_conversation(
    ...
    try:
        ...
        if not existing_status:
            logger.info(f"Existing status not found, creating new status record: group_id={request.group_id}")
            ...
            if result:
                logger.info(f"New status created successfully: group_id={request.group_id}")
                return True
            else:
                logger.warning(f"Failed to create new status: group_id={request.group_id}")
                return False
...
async def _update_status_after_memcell_extraction(
    ...
    try:
        ...
        if request.history_raw_data_list and request.history_raw_data_list[-1]:
            ...
            if hasattr(last_history_data, "content") and isinstance(last_history_data.content, dict):
        ...
        if request.new_raw_data_list and request.new_raw_data_list[0]:
            ...
            if hasattr(first_new_data, "content") and isinstance(first_new_data.content, dict):
        ...
        if request.new_raw_data_list and request.new_raw_data_list[-1]:
            ...
            if hasattr(last_new_data, "content") and isinstance(last_new_data.content, dict):
