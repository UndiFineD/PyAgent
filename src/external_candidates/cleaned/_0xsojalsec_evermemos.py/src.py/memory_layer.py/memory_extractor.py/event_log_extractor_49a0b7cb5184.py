# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\event_log_extractor.py
"""
...
class EventLogExtractor:
    ...
    async def _extract_event_log(
        ...
        if not isinstance(event_log_data["atomic_fact"], list):
            raise ValueError(f"atomic_fact is not a list: {type(event_log_data['atomic_fact'])}")
        ...
        fact_embeddings_batch = await vectorize_service.get_embeddings(event_log_data["atomic_fact"])

        # Convert to list format
        fact_embeddings = [emb.tolist() if hasattr(emb, "tolist") else emb for emb in fact_embeddings_batch]
    ...
    async def extract_event_logs_batch(self, episodes: List[Dict[str, Any]]) -> List[Optional[EventLog]]:
        """
        ...
        tasks = [
            self.extract_event_log(episode_text=ep.get("episode", ""), timestamp=ep.get("timestamp")) for ep in episodes
        ]
