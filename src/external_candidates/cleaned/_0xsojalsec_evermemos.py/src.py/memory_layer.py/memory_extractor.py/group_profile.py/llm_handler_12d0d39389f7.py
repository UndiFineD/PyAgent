# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\group_profile\llm_handler.py
...
class GroupProfileLLMHandler:
    ...
    async def execute_parallel_analysis(
        ...
        existing_summary = existing_profile.get("summary", "") if existing_profile else ""
        existing_subject = existing_profile.get("subject", "") if existing_profile else ""
        ...
        content_response, behavior_response = await asyncio.gather(content_task, behavior_task, return_exceptions=True)
                ...
                [
                    {
                        ...
                        "summary": (t.summary if hasattr(t, "summary") else t.get("summary")),
                        "status": t.status if hasattr(t, "status") else t.get("status"),
                        "confidence": (t.confidence if hasattr(t, "confidence") else t.get("confidence", "strong")),
    ...
    def build_behavior_analysis_prompt(
        ...
        if user_organizations:
            for org in user_organizations:
                org_mapping[org.user_id] = {
                    ...
                    "skip_level_manager": org.skip_level_manager or "Unknown Skip Manager",
                }

        # Build speaker info string
        has_org_info = bool(user_organizations and org_mapping)
        speaker_info_title = (
            "**Available Speakers with Organization Context:**" if has_org_info else "**Available Speakers:**"
        )
        speaker_info = f"\n{speaker_info_title}\n"
        ...
        for speaker_id in sorted(all_available_speakers):
            ...
            if has_org_info and speaker_id in org_mapping:
                ...
                org_info = (
                    f" | Role: {org_data['role']} | Team: {org_data['team']} | Manager: {org_data['direct_manager']}"
                )
    ...
    async def _execute_with_retry(self, task_name: str, task_func, max_retries: int = 1) -> Optional[str]:
        """Execute LLM task with retry mechanism."""
        for attempt in range(max_retries + 1):
            try:
                response = await task_func()
                logger.debug(f"[{task_name}] Success on attempt {attempt + 1}, response length: {len(response)} chars")
                return response
            except Exception as e:
                if attempt < max_retries:
                    logger.exception(f"[{task_name}] Attempt {attempt + 1} failed: {e}, retrying...")
                else:
                    logger.exception(f"[{task_name}] All {max_retries + 1} attempts failed: {e}")
                    return None
        return None

    def _parse_content_response(self, response: str, existing_profile: Optional[Dict], memcell_list: List) -> Dict:
    ...
    def _parse_behavior_response(self, response: str, memcell_list: List, existing_profile: Optional[Dict]) -> Dict:
        ...
        try:
            ...
            logger.error(f"[BehaviorParse] Error parsing behavior response: {e}", exc_info=True)
    ...
    def _merge_parallel_analysis_results(
        ...
        try:
            ...
            content_data = {}
            if content_response:
                content_data = self._parse_content_response(content_response, existing_profile, memcell_list)
            else:
                logger.debug("[Merge] Content analysis failed, using fallback from existing profile")
                content_data = self._get_content_fallback(existing_profile)

            # Parse behavior analysis results
            behavior_data = {}
            if behavior_response:
                behavior_data = self._parse_behavior_response(behavior_response, memcell_list, existing_profile)
            else:
                logger.debug("[Merge] Behavior analysis failed, using fallback from existing profile")
            ...
            logger.error(f"[Merge] Error merging parallel analysis results: {e}", exc_info=True)
            return None
