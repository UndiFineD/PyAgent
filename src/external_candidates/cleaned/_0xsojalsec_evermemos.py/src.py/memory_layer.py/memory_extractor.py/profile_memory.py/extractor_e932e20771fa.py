# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\profile_memory\extractor.py
...
class ProfileMemoryExtractor(MemoryExtractor):
    ...
    async def extract_memory(self, request: ProfileMemoryExtractRequest) -> Optional[List[ProfileMemory]]:
            ...
            for mem in request.old_memory_list:
                if mem.memory_type == MemoryType.PROFILE:
                    ...
                    if participants_profile_list:
                        profile_obj_no_evidences = remove_evidences_from_profile(participants_profile_list[-1])
        ...
        async def invoke_llm(prompt: str, part_label: str) -> Optional[List[Dict[str, Any]]]:
            ...
            for attempt in range(extraction_attempts):
                try:
                    ...
                    parsed_profiles = self._extract_user_profiles_from_response(annotated_response, part_label)
                    break
                    ...
                    try:
                        ...
                        logger.error("%s repair prompt failed: %s", part_label, repair_exc)
                        if response:
                            logger.error(
        ...
        for profile in merged_profiles:
            ...
            new_evidence = merge_group_importance_evidence(old_evidence, new_evidence_list, user_id=profile.user_id)
            if new_evidence:
    ...
    def _extract_user_profiles_from_response(self, response: str, part_label: str) -> Optional[List[Dict[str, Any]]]:
    ...
    async def extract_profile_companion(self, request: ProfileMemoryExtractRequest) -> Optional[List[ProfileMemory]]:
        ...
        print(f"[ProfileExtractor] user_id_to_name (extracted from original_data): {user_id_to_name}")

        # 🔧 If user_id_to_name is empty, extract from participants field
        if not user_id_to_name:
            print(f"[ProfileExtractor] user_id_to_name is empty, attempting to extract from participants")
            for memcell in request.memcell_list:
                participants = getattr(memcell, "participants", None)
                if participants and isinstance(participants, list):
                    for user_id in participants:
                        if user_id and user_id not in user_id_to_name:
                            user_id_to_name[user_id] = user_id  # Use user_id as default name
                    print(f"[ProfileExtractor] Extracted from participants: {list(participants)}")
        ...
        for memcell in request.memcell_list:
            ...
            if episode_text:
                ...
                print(f"[ProfileExtractor] ⚠️  episode is empty, trying conversation_text fallback")
                ...
                if conversation_text and conversation_text.strip():
                    ...
                    print(f"[ProfileExtractor] Using conversation_text: {conversation_text[:200]}...")
                else:
                    print(f"[ProfileExtractor] ❌ Both episode and conversation_text are empty!")
        ...
        print(f"[ProfileExtractor] conversation_text length: {len(conversation_text)} characters")
        ...
        async def extract_single_user_companion_profile(user_id: str, user_info: Dict[str, Any]) -> List[ProfileMemory]:
            ...
            try:
                print(f"[ProfileExtractor] Calling LLM to extract Profile for {user_info['user_name']}...")
                ...
                try:
                    ...
                    structured_profiles = self._extract_user_profiles_from_response(annotated, "companion profile")
                except Exception:
                ...
                if structured_profiles:
                    print(f"[ProfileExtractor] Parsed {len(structured_profiles)} structured Profiles")
                    ...
                    batch_event_ids: List[str] = [
                        str(mc.event_id) for mc in request.memcell_list if hasattr(mc, "event_id") and mc.event_id
                    ]
                    ...
                    for p in structured_profiles:
                                    ...
                                    if isinstance(raw_evidences, list):
                                        for ev in raw_evidences:
                                            ...
                                            if "conversation_id:" in s:
                                                conv_id = s.split("conversation_id:")[-1].strip("[]() ,.\t\n")
                                            else:
                                                conv_id = s
                                            if conv_id and conv_id in valid_conversation_ids:
                                                ev_date = conversation_date_map.get(conv_id) or default_date
                                                normalized_evs.append(f"{ev_date}|{conv_id}" if ev_date else conv_id)
                        ...
                        if mem:
                            ...
                            print(f"[ProfileExtractor] ✅ Successfully converted Profile: user_id={mem.user_id}")
                        else:
                            print(f"[ProfileExtractor] ⚠️  profile_payload_to_memory returned None")
                else:
                    print(f"[ProfileExtractor] Failed to parse structured Profile, using fallback")
                    ...
                    profile_memory = ProfileMemory(
                        ...
                        user_id=user_id,
                        timestamp=_dt.now(),
                        ori_event_id_list=[mc.event_id for mc in request.memcell_list if hasattr(mc, "event_id")],
                        group_id=request.group_id or "",
                        personality=[
                            {
                    ...
                    print(f"[ProfileExtractor] ✅ Using fallback Profile: user_id={user_id}")

                print(f"[ProfileExtractor] Final result returns {len(user_profiles_result)} Profiles")
        ...
        tasks = [
            extract_single_user_companion_profile(user_id, user_info) for user_id, user_info in user_profiles.items()
        ]
        ...
        print(f"[ProfileExtractor] Parallel extraction completed, received {len(results)} results")
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, list):
                ...
                print(f"[ProfileExtractor] Task {i + 1} succeeded, returned {len(result)} Profiles")
        ...
        print(f"[ProfileExtractor] ✅ Finally successfully extracted {len(companion_profiles)} companion profiles")
