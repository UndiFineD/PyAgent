# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\profile_memory\evidence_utils.py
...
def _strip_evidences_for_identifier(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {key: _strip_evidences_for_identifier(val) for key, val in value.items() if key != "evidences"}
    if isinstance(value, list):
...
def _format_and_validate_evidences(
    ...
    *,
    ...
    for evidence in ensure_str_list(evidences):
        ...
        if valid_conversation_ids is not None and conversation_id and conversation_id not in valid_conversation_ids:
            logger.warning(
...
def remove_entries_without_evidence(payload: Any, *, path: str = "user_profile") -> Any:
    ...
    if isinstance(payload, dict):
        for key in list(payload.keys()):
            if key == "evidences":
                continue
            cleaned = remove_entries_without_evidence(payload[key], path=f"{path}.{key}")
            if cleaned is None:
