# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\cluster_manager\manager.py
...
class ClusterState:
    ...
    def _update_cluster_centroid(self, cluster_id: str, vector: np.ndarray, timestamp: Optional[float]) -> None:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cluster_centroids": {cid: centroid.tolist() for cid, centroid in self.cluster_centroids.items()},
    ...
    def from_dict(data: Dict[str, Any]) -> "ClusterState":
        ...
        state.cluster_centroids = {k: np.array(v, dtype=np.float32) for k, v in centroids.items()}
        state.cluster_counts = {k: int(v) for k, v in (data.get("cluster_counts", {}) or {}).items()}
        state.cluster_last_ts = {k: float(v) for k, v in (data.get("cluster_last_ts", {}) or {}).items()}
...
class ClusterManager:
    ...
    def _find_best_cluster(self, state: ClusterState, vector: np.ndarray, timestamp: Optional[float]) -> Optional[str]:
    ...
    async def _notify_callbacks(self, group_id: str, memcell: Dict[str, Any], cluster_id: str) -> None:
