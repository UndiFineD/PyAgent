# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\cluster_manager\config.py
...
class ClusterManagerConfig:
    ...
    def __post_init__(self):
        ...
        if self.max_time_gap_days < 0:
            raise ValueError(f"max_time_gap_days must be >= 0, got {self.max_time_gap_days}")
        ...
        if self.clustering_algorithm not in ("centroid", "nearest"):
            raise ValueError(f"clustering_algorithm must be 'centroid' or 'nearest', got {self.clustering_algorithm}")
