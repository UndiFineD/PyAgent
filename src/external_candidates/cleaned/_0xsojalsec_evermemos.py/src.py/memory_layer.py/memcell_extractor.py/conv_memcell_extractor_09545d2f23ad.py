# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memcell_extractor\conv_memcell_extractor.py
"""
...
class ConvMemCellExtractor(MemCellExtractor):
    ...
    def __init__(
        ...
        self.conv_boundary_detection_prompt = boundary_detection_prompt or get_prompt_by(
    ...
    async def _detect_boundary(
        ...
        for i in range(5):
            try:
                ...
                logger.warning(f"[ConversationEpisodeBuilder] Boundary detection error (attempt {i + 1}/5): {e}")
                continue
    ...
    async def extract_memcell(
        ...
        if request.new_raw_data_list and self._data_process(request.new_raw_data_list[-1]) is None:
            logger.warning(f"[ConvMemCellExtractor] The last new_raw_data is None, skipping processing")
        ...
        if not new_message_dict_list:
            logger.warning(f"[ConvMemCellExtractor] No valid new messages to process (possibly all filtered out)")
            status_control_result = StatusResult(should_wait=True)
            return (None, status_control_result)
    ...
    def _data_process(self, raw_data: RawData) -> Dict[str, Any]:
        ...
        if isinstance(content, dict) and msg_type is not None:
            ...
            if msg_type not in SUPPORTED_MSG_TYPES:
                ...
                logger.warning(f"[ConvMemCellExtractor] Skipping unsupported message type: {msg_type}")
                return None
            ...
            if placeholder is not None:
                ...
                logger.debug(f"[ConvMemCellExtractor] Message type {msg_type} converted to placeholder: {placeholder}")
