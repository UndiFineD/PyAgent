...
async def sync_episodic_memory_docs(batch_size: int, limit: Optional[int], days: Optional[int]) -> None:
        ...
        while True:
            ...
            for mongo_doc in mongo_docs:
                ...
                if limit and total_processed >= limit:
                    logger.info("Reached processing limit %s, stop generating actions", limit)
                    return
    ...
    try:
        ...
        async for ok, info in async_streaming_bulk(async_client, generate_actions(), chunk_size=batch_size):
            if ok:
                success_count += 1
            else:
                error_count += 1
