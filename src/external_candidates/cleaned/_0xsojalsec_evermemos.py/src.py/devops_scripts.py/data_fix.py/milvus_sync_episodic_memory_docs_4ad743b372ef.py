# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\devops_scripts\data_fix\milvus_sync_episodic_memory_docs.py
"""
...
async def sync_episodic_memory_docs(batch_size: int, limit: Optional[int], days: Optional[int]) -> None:
    ...
    try:
        ...
        while True:
            ...
            first_doc_time = mongo_docs[0].created_at if hasattr(mongo_docs[0], "created_at") else "unknown"
            last_doc_time = mongo_docs[-1].created_at if hasattr(mongo_docs[-1], "created_at") else "unknown"
            logger.info(
            ...
            for mongo_doc in mongo_docs:
                try:
                    ...
                    if not milvus_entity.get("id"):
                        logger.warning("Document missing id field, skipping: %s", mongo_doc.id)
                        batch_errors += 1
                        continue
