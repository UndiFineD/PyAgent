# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\devops_scripts\data_fix\es_sync_docs.py
import argparse
import asyncio
import traceback
...
async def run(index_name: str, batch_size: int, limit_: int | None, days: int | None) -> None:
    """Synchronize MongoDB data to the specified Elasticsearch index."""
    try:
        document_class: type[AsyncDocument] = find_document_class_by_index_name(index_name)
        logger.info(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Synchronize MongoDB data to Elasticsearch")
    parser.add_argument("--index-name", "-i", required=True, help="Index alias, e.g.: episodic-memory")
