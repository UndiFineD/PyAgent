# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\devops_scripts\data_fix\mongo_add_timestamp_shard.py
#!/usr/bin/env python3
"""
Add Timestamp Shard
    ...
    try:
        ...
        try:
            ...
            if not shard_status.get("shards"):
                logger.warning("⚠️  Current environment is not a sharded cluster, skipping sharding configuration")
                return
            logger.info(f"✅ Sharded cluster detected, total {len(shard_status['shards'])} shards")
        except OperationFailure as e:
            logger.warning(f"⚠️  Unable to check sharding status: {e}, may not be a sharded environment")
            return
        ...
        try:
            await admin_db.command("shardCollection", collection_name, key={"timestamp": 1})
        ...
        try:
            ...
            base_date = get_now_with_timezone().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            split_points = []
        ...
        try:
            ...
            if shard_info.get("sharded"):
                logger.info("✅ MemCell collection sharding configuration verified successfully")
...
async def disable_timestamp_sharding(session=None):
    ...
    """
    logger.warning("⚠️  Disabling sharding is a dangerous operation, requires manual handling by administrator")
    logger.info("📝 Please manually execute the following MongoDB commands to disable sharding:")
    logger.info("   1. Stop balancer: sh.stopBalancer()")
    logger.info("   2. Wait for balancer to complete: sh.waitForBalancer()")
    logger.info("   3. Removing sharding configuration requires recreating the collection")
