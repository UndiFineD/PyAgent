# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\run_memorize.py
#!/usr/bin/env python3
"""
...
class GroupChatMemorizer:
    ...
    def validate_input_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if is_valid:
                ...
                logger.error("Please ensure the input file conforms to the GroupChatFormat specification")
                return False
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                conversation_meta_request = {
                    ...
                    "default_timezone": meta.get("default_timezone", get_timezone().key),
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"  ⚠ Failed to save conversation metadata: {response.status_code}")
                ...
                for i, message in enumerate(messages):
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            success_count += 1
                            if memory_count > 0:
                                logger.info(f"  ✓ Successfully saved {memory_count} memories")
                            else:
            ...
            logger.info("=" * 70)
            logger.info(f"✓ Successfully processed: {success_count}/{len(messages)} messages")
    ...
    async def process_file(self, file_path: str) -> bool:
        ...
        # Check API address
        if not self.api_url:
            logger.error("✗ API address not provided, please specify using --api-url parameter")
            return False
        ...
        try:
            ...
            logger.info("Using simple direct single message format, processing one by one")
...
Input file format:
  Input file must conform to GroupChatFormat specification, refer to data_format/group_chat/group_chat_format.py
    ...
    if args.validate_only:
        ...
        memorizer = GroupChatMemorizer(api_url="", scene=args.scene)  # Pass empty string as placeholder
