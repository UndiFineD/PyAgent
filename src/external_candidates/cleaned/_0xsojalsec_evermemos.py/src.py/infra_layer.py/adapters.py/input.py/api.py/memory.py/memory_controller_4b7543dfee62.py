# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\input\api\memory\memory_controller.py
"""
...
class MemoryController(BaseController):
    ...
    def __init__(self, conversation_meta_repository: ConversationMetaRawRepository):
        ...
        logger.info("MemoryController initialized with MemoryManager and ConversationMetaRepository")
    ...
    async def memorize_single_message(self, fastapi_request: FastAPIRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Starting conversion from simple message format to internal format")
            ...
            logger.info("Conversion completed: group_id=%s, group_name=%s", group_id, group_name)
            ...
            raise HTTPException(status_code=500, detail="Failed to store memory, please try again later") from e
    ...
    async def save_conversation_meta(self, fastapi_request: FastAPIRequest) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not saved_meta:
                raise HTTPException(status_code=500, detail="Failed to save conversation metadata")
            ...
            raise HTTPException(status_code=400, detail=f"Missing required field: {str(e)}") from e
        except ValueError as e:
            ...
            logger.error("conversation-meta request processing failed: %s", e, exc_info=True)
            raise HTTPException(
                status_code=500,
    ...
    async def patch_conversation_meta(self, fastapi_request: FastAPIRequest) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not group_id:
                raise HTTPException(status_code=400, detail="Missing required field group_id")
            ...
            existing_meta = await self.conversation_meta_repository.get_by_group_id(group_id)
            ...
            return {
                ...
                "result": {
                    ...
                    "updated_at": (updated_meta.updated_at.isoformat() if updated_meta.updated_at else None),
                },
            }
            ...
            logger.error("conversation-meta partial update request missing required field: %s", e)
            raise HTTPException(status_code=400, detail=f"Missing required field: {str(e)}") from e
        except ValueError as e:
            logger.error("conversation-meta partial update request parameter error: %s", e)
            raise HTTPException(status_code=400, detail=str(e)) from e
        except Exception as e:
            logger.error(
