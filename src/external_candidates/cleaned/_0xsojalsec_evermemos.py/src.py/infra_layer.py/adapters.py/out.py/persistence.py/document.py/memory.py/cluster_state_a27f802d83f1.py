...
class ClusterState(DocumentBase, AuditBase):
    ...
    event_ids: List[str] = Field(default_factory=list, description="List of all event_ids")
    timestamps: List[float] = Field(default_factory=list, description="List of timestamps")
    cluster_ids: List[str] = Field(default_factory=list, description="List of cluster IDs")

    # event_id -> cluster_id mapping
    eventid_to_cluster: Dict[str, str] = Field(default_factory=dict, description="Mapping from event to cluster")
    ...
    cluster_counts: Dict[str, int] = Field(default_factory=dict, description="Cluster member count {cluster_id: count}")
