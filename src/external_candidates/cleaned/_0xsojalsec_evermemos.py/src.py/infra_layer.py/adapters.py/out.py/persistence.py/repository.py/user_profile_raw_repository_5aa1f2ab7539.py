# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\repository\user_profile_raw_repository.py
"""
...
class UserProfileRawRepository(BaseRepository[UserProfile]):
    ...
    async def get_by_user_and_group(self, user_id: str, group_id: str) -> Optional[UserProfile]:
        try:
            return await self.model.find_one(UserProfile.user_id == user_id, UserProfile.group_id == group_id)
        except Exception as e:
            logger.error(f"Failed to retrieve user profile: user_id={user_id}, group_id={group_id}, error={e}")
            return None
    ...
    async def get_all_by_group(self, group_id: str) -> List[UserProfile]:
        try:
            ...
            logger.error(f"Failed to retrieve group user profiles: group_id={group_id}, error={e}")
            return []

    async def get_all_by_user(self, user_id: str, limit: int = 40) -> List[UserProfile]:
        try:
            return await self.model.find(UserProfile.user_id == user_id).sort([("version", -1)]).limit(limit).to_list()
        except Exception as e:
    ...
    async def upsert(
        ...
        try:
            ...
            if existing:
                ...
                logger.info(f"Created user profile: user_id={user_id}, group_id={group_id}")
                return user_profile
        except Exception as e:
            logger.error(f"Failed to save user profile: user_id={user_id}, group_id={group_id}, error={e}")
            return None
    ...
    async def delete_by_group(self, group_id: str) -> int:
        try:
            ...
            logger.info(f"Deleted group user profiles: group_id={group_id}, count={count}")
            return count
        except Exception as e:
            logger.error(f"Failed to delete group user profiles: group_id={group_id}, error={e}")
            return 0
