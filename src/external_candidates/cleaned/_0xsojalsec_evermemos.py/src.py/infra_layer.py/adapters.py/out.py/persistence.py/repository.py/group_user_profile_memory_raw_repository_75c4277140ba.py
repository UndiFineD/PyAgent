# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\repository\group_user_profile_memory_raw_repository.py
"""
...
class GroupUserProfileMemoryRawRepository(BaseRepository[GroupUserProfileMemory]):
    ...
    async def ensure_latest(self, user_id: str, group_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        """
    ...
    async def get_by_user_group(
        ...
        try:
            ...
            result = await self.model.find_one(query_filter, sort=[("version", -1)], session=session)
    ...
    async def batch_get_by_user_groups(
        ...
        try:
            ...
            pipeline = [
                {"$match": {"$or": [{"user_id": user_id, "group_id": group_id} for user_id, group_id in unique_pairs]}},
    ...
    async def update_by_user_group(
        ...
        try:
            ...
            logger.error("❌ Failed to update group user profile by user ID and group ID: %s", e)
            return None
    ...
    async def delete_by_user_group(
        ...
        try:
            ...
            logger.error("❌ Failed to delete group user profile by user ID and group ID: %s", e)
            return False
    ...
    async def upsert_by_user_group(
        ...
        try:
            ...
            if existing_doc:
                ...
                # Create new record
                new_doc = GroupUserProfileMemory(user_id=user_id, group_id=group_id, **update_data)
    ...
    async def get_by_user_ids(
        ...
        try:
            ...
            logger.error("❌ Failed to retrieve group user profiles by user ID list: %s", e)
            return []
    ...
    async def get_by_group_ids(
        ...
        try:
            ...
            logger.error("❌ Failed to retrieve group user profiles by group ID list: %s", e)
            return []
    ...
    async def delete_by_user_id(self, user_id: str, session: Optional[AsyncClientSession] = None) -> int:
        ...
        """
        try:
            result = await self.model.find({"user_id": user_id}, session=session).delete()
            deleted_count = result.deleted_count if hasattr(result, "deleted_count") else 0
            logger.debug(
    ...
    async def delete_by_group_id(self, group_id: str, session: Optional[AsyncClientSession] = None) -> int:
        """
        ...
        """
        try:
            result = await self.model.find({"group_id": group_id}, session=session).delete()
            deleted_count = result.deleted_count if hasattr(result, "deleted_count") else 0
            logger.debug(
