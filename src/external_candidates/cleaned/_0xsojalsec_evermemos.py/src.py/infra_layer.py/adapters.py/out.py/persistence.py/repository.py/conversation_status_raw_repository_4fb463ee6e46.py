...
class ConversationStatusRawRepository(BaseRepository[ConversationStatus]):
    ...
    async def delete_by_group_id(self, group_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        ...
        try:
            result = await self.model.find_one({"group_id": group_id}, session=session)
            if not result:
                logger.warning("⚠️  Conversation status to delete not found: group_id=%s", group_id)
                return False

            await result.delete(session=session)
            logger.info("✅ Successfully deleted conversation status by group ID: %s", group_id)
            return True
        except Exception as e:
    ...
    async def upsert_by_group_id(
        ...
        try:
            ...
            try:
                ...
                print(f"[ConversationStatusRawRepository] Successfully created new conversation status: {new_doc}")
                return new_doc
                ...
                if "E11000" in error_str and "duplicate key" in error_str:
                    ...
                    retry_doc = await self.model.find_one({"group_id": group_id}, session=session)
    ...
    async def count_by_group_id(self, group_id: str, session: Optional[AsyncClientSession] = None) -> int:
        """Count the number of conversation statuses for a specified group"""
        try:
            count = await self.model.find({"group_id": group_id}, session=session).count()
            logger.debug(
