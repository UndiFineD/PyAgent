...
class ConversationStatus(DocumentBase, AuditBase):
    ...
    old_msg_start_time: Optional[datetime] = Field(default=None, description="Conversation window read start time")
    new_msg_start_time: Optional[datetime] = Field(
        default=None, description="Accumulated new conversation read start time"
    )
    last_memcell_time: Optional[datetime] = Field(default=None, description="Accumulated memCell read start time")
    ...
    class Settings:
        ...
        indexes = [
            ...
            IndexModel([("group_id", ASCENDING)], name="idx_group_id", unique=True),  # group_id must be unique
