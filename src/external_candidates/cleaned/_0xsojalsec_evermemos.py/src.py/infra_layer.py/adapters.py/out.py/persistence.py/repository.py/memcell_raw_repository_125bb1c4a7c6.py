# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\repository\memcell_raw_repository.py
"""
...
class MemCellRawRepository(BaseRepository[MemCell]):
    ...
    async def get_by_event_id(self, event_id: str) -> Optional[MemCell]:
        ...
        try:
            ...
            if result:
                logger.debug("✅ Successfully retrieved MemCell by event_id: %s", event_id)
            else:
    ...
    async def append_memcell(self, memcell: MemCell, session: Optional[AsyncClientSession] = None) -> Optional[MemCell]:
        """
        Append MemCell
        """
        try:
    ...
    async def update_by_event_id(
        ...
        try:
            ...
            if memcell:
                ...
                logger.debug("✅ Successfully updated MemCell by event_id: %s", event_id)
                return memcell
            return None
        except Exception as e:
    ...
    async def delete_by_event_id(self, event_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        """
        ...
        try:
            ...
            if memcell:
                ...
                logger.debug("✅ Successfully deleted MemCell by event_id: %s", event_id)
                return True
            return False
        except Exception as e:
            ...
            query = self.model.find(
                And(
                    Or(
                        ...
                        Eq(MemCell.participants, user_id),  # MongoDB checks if array contains the value
                    ),
    ...
    async def find_by_time_range(
        ...
        """
        try:
            query = self.model.find({"timestamp": {"$gte": start_time, "$lt": end_time}})
    ...
    async def delete_by_user_id(self, user_id: str, session: Optional[AsyncClientSession] = None) -> int:
        """
    ...
    async def count_by_time_range(self, start_time: datetime, end_time: datetime, user_id: Optional[str] = None) -> int:
        """
    ...
    async def get_latest_by_user(self, user_id: str, limit: int = 10) -> List[MemCell]:
        ...
        """
        try:
            results = await self.model.find({"user_id": user_id}).sort("-timestamp").limit(limit).to_list()
            logger.debug(
    ...
    async def get_user_activity_summary(self, user_id: str, start_time: datetime, end_time: datetime) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            latest = await self.model.find(base_query).sort("-timestamp").limit(1).to_list()
            earliest = await self.model.find(base_query).sort("timestamp").limit(1).to_list()
            ...
            summary = {
                ...
                "earliest_activity": (earliest[0].timestamp.isoformat() if earliest else None),
            }
