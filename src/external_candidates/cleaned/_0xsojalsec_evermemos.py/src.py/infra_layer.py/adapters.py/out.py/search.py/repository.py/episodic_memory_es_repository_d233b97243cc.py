# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\search\repository\episodic_memory_es_repository.py
"""
...
class EpisodicMemoryEsRepository(BaseRepository[EpisodicMemoryDoc]):
    ...
    async def delete_by_filters(
        ...
        try:
            # Build filter conditions
            filter_queries = []
            if user_id is not None:  # Use is not None instead of truthy check, supports empty string
            ...
            if not filter_queries:
                raise ValueError("At least one filter condition (user_id, group_id or date_range) must be provided")
            ...
            response = await client.delete_by_query(index=index_name, body={"query": delete_query}, refresh=refresh)
