# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\search\repository\event_log_milvus_repository.py
"""
...
class EventLogMilvusRepository(BaseMilvusRepository[EventLogCollection]):
    ...
    async def create_and_save_event_log(
        ...
        try:
            ...
            logger.error("❌ Failed to create event log document: id=%s, error=%s", id, e)
            raise
    ...
    async def delete_by_id(self, log_id: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.debug("✅ Successfully deleted event log by log_id: log_id=%s", log_id)
            return success
        except Exception as e:
            logger.error(
