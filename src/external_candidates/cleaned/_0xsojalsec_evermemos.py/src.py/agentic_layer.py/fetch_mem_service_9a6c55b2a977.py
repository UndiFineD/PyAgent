# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\agentic_layer\fetch_mem_service.py
"""
...
class FetchMemoryServiceInterface(ABC):
    ...
    @abstractmethod
    async def find_by_id(self, memory_id: str, memory_type: MemoryType) -> Optional[MemoryModel]:
        """
    ...
    @abstractmethod
    async def find_episodic_by_event_id(self, event_id: str, user_id: str) -> Optional[EpisodicMemoryModel]:
        """
...
class FetchMemoryServiceImpl(FetchMemoryServiceInterface):
    ...
    def _get_repositories(self):
        ...
        if self._conversation_meta_repo is None:
            self._conversation_meta_repo = get_bean_by_type(ConversationMetaRawRepository)
    ...
    async def _get_employee_metadata(
        ...
        try:
            ...
            if group_id:
                ...
                conversation_meta = await self._conversation_meta_repo.get_by_group_id(group_id)
                ...
                if conversation_meta and conversation_meta.user_details:
                    ...
                    if user_detail:
                        ...
                        return Metadata(
                            ...
                            email=(user_detail.extra.get("email") if user_detail.extra else None),
                            phone=(user_detail.extra.get("phone") if user_detail.extra else None),
                        )

            # If no group_id or user information not found, return basic information
            return Metadata(source=source, user_id=user_id, memory_type=memory_type, limit=limit)

        except Exception as e:
            logger.warning(f"Failed to retrieve user information: {e}, creating Metadata with basic info")
            return Metadata(source=source, user_id=user_id, memory_type=memory_type, limit=limit)
    ...
    def _convert_episodic_memory(self, episodic_memory) -> EpisodicMemoryModel:
        ...
        return EpisodicMemoryModel(
            ...
            location=(episodic_memory.extend.get("location", "") if episodic_memory.extend else ""),
    ...
    def _convert_relationship(self, relationship) -> RelationModel:
        ...
        return RelationModel(
            ...
            relation_type=(
                relationship.relationship[0]["type"] if relationship.relationship else "Unknown relationship"
            ),
            relation_description=(relationship.relationship[0]["content"] if relationship.relationship else ""),
    ...
    def _convert_behavior_history(self, behavior) -> BehaviorHistoryModel:
        ...
        return BehaviorHistoryModel(
            ...
            action_type=(behavior.behavior_type[0] if behavior.behavior_type else "Unknown behavior"),
    ...
    def _convert_event_log(self, event_log: Union[EventLogRecord, EventLogRecordProjection]) -> EventLogModel:
        ...
        return EventLogModel(
            ...
            vector=getattr(event_log, "vector", None),  # EventLogRecordShort does not have vector field
    ...
    def _convert_foresight_record(
        ...
        return ForesightRecordModel(
            ...
            vector=getattr(foresight_record, "vector", None),  # ForesightRecordProjection does not have vector field
    ...
    async def find_by_user_id(
        ...
        Returns:
            Memory query response
        """
        logger.debug(f"Fetching {memory_type} memories for user {user_id}, limit: {limit}")
        ...
        try:
            ...
            match memory_type:
                case MemoryType.MULTIPLE:
                    ...
                    core_memory_result = await self._core_repo.get_by_user_id(user_id, version_range=version_range)
                    if core_memory_result:
                        ...
                        if isinstance(core_memory_result, list):
                            memories = [
                                await self._convert_core_memory(core_memory) for core_memory in core_memory_result
                            ]
                        else:
                            memories = [await self._convert_core_memory(core_memory_result)]
                    else:
                        memories = []
                case MemoryType.FORESIGHT:
                    ...
                    memories = [self._convert_foresight_record(record) for record in foresight_records]

                case MemoryType.EPISODIC_MEMORY:
                    # Episodic memory: list of events sorted by time
                    episodic_memories = await self._episodic_repo.get_by_user_id(user_id, limit=limit)
                    memories = [self._convert_episodic_memory(mem) for mem in episodic_memories]
                ...
                case MemoryType.ENTITY:
                    ...
                    entities = await self._entity_repo.get_by_type("Person", limit=limit)
                ...
                case MemoryType.RELATION:
                    ...
                    relationships = await self._relationship_repo.get_by_relationship_type(
                        "Interpersonal relationship", limit=limit
                    )
                    memories = [self._convert_relationship(rel) for rel in relationships]

                case MemoryType.BEHAVIOR_HISTORY:
                    # Behavior history: user behaviors sorted by time
                    behaviors = await self._behavior_repo.get_by_user_id(user_id, limit=limit)
                    memories = [self._convert_behavior_history(behavior) for behavior in behaviors]
                ...
                case MemoryType.EVENT_LOG:
                    ...
                    memories = [self._convert_event_log(event_log) for event_log in event_logs]
            ...
            return FetchMemResponse(memories=[], total_count=0, has_more=False, metadata=error_metadata)
        ...
        try:
            ...
            match memory_type:
                case MemoryType.FORESIGHT:
                    ...
                    foresight = await self._foresight_record_repo.get_by_user_id(memory_id)
                    if foresight:
                ...
                case MemoryType.BEHAVIOR_HISTORY:
                    ...
                    behaviors = await self._behavior_repo.get_by_user_id(memory_id, limit=1)
                    if behaviors:
                ...
                case MemoryType.EVENT_LOG:
                    ...
                    event_log = await self._event_log_repo.get_by_id(memory_id, model=EventLogRecordProjection)
                    if event_log:
    ...
    async def find_episodic_by_event_id(self, event_id: str, user_id: str) -> Optional[EpisodicMemoryModel]:
        ...
        Returns:
            Episodic memory model, or None if not found
        """
        logger.debug(f"Fetching episodic memory by event_id: {event_id}, user_id: {user_id}")

        try:
            self._get_repositories()
            episodic_memory = await self._episodic_repo.get_by_event_id(event_id, user_id)
            if episodic_memory:
    ...
    async def find_relationship_by_entity_ids(
        ...
        Returns:
            Relation model, or None if not found
        """
        logger.debug(f"Fetching relationship by entity_ids: {source_entity_id} -> {target_entity_id}")

        try:
            self._get_repositories()
            relationship = await self._relationship_repo.get_by_entity_ids(source_entity_id, target_entity_id)
            if relationship:
                return self._convert_relationship(relationship)
            return None

        except Exception as e:
            logger.error(f"Error fetching relationship by entity_ids {source_entity_id} -> {target_entity_id}: {e}")
            return None
