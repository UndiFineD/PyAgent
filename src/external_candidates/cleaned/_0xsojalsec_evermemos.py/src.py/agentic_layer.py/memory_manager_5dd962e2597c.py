...
class MemoryManager:
    ...
    def __init__(self) -> None:
        ...
        logger.info("MemoryManager initialized with fetch_mem_service and retrieve_mem_service")
    ...
    async def fetch_mem(self, request: FetchMemRequest) -> FetchMemResponse:
        ...
        """
        logger.debug(f"fetch_mem called with request: user_id={request.user_id}, memory_type={request.memory_type}")
        ...
        logger.debug(f"fetch_mem returned {len(response.memories)} memories for user {request.user_id}")
        return response
    ...
    async def retrieve_mem(self, retrieve_mem_request: "RetrieveMemRequest") -> RetrieveMemResponse:
        ...
        try:
            ...
            return RetrieveMemResponse(
                ...
                query_metadata=Metadata(
                    ...
                    user_id=(retrieve_mem_request.user_id if retrieve_mem_request else ""),
    ...
    async def retrieve_mem_keyword(self, retrieve_mem_request: "RetrieveMemRequest") -> RetrieveMemResponse:
    ...
    async def get_keyword_search_results(self, retrieve_mem_request: "RetrieveMemRequest") -> List[Dict[str, Any]]:
        try:
    ...
    async def retrieve_mem_vector(self, retrieve_mem_request: "RetrieveMemRequest") -> RetrieveMemResponse:
        """Vector-based memory retrieval"""
        try:
    ...
    async def get_vector_search_results(self, retrieve_mem_request: "RetrieveMemRequest") -> List[Dict[str, Any]]:
        try:
            # Get parameters from Request
            logger.debug(f"get_vector_search_results called with retrieve_mem_request: {retrieve_mem_request}")
            if not retrieve_mem_request:
                raise ValueError("retrieve_mem_request is required for get_vector_search_results")
            query = retrieve_mem_request.query
            if not query:
            ...
            logger.debug(f"Query text vectorization completed, vector dimension: {len(query_vector_list)}")
    ...
    async def retrieve_mem_hybrid(self, retrieve_mem_request: "RetrieveMemRequest") -> RetrieveMemResponse:
    ...
    async def _to_response(self, hits: List[Dict], req: "RetrieveMemRequest") -> RetrieveMemResponse:
        ...
        memories, scores, importance_scores, original_data, total_count = await self.group_by_groupid_stratagy(
    ...
    async def retrieve_mem_rrf(self, retrieve_mem_request: "RetrieveMemRequest") -> RetrieveMemResponse:
    ...
    async def retrieve_mem_agentic(self, retrieve_mem_request: "RetrieveMemRequest") -> RetrieveMemResponse:
    ...
    def _calculate_importance_score(self, importance_evidence: Optional[Dict[str, Any]]) -> float:
    ...
    async def _batch_get_memcells(self, event_ids: List[str], batch_size: int = 100) -> Dict[str, Any]:
        ...
        logger.debug(f"Batch get MemCells: Total {len(unique_event_ids)} (before deduplication: {len(event_ids)})")
        ...
        for i in range(0, len(unique_event_ids), batch_size):
            ...
            logger.debug(f"Getting batch {i // batch_size + 1} MemCells: {len(batch_event_ids)} items")

            batch_memcells = await memcell_repo.get_by_event_ids(batch_event_ids)
            all_memcells.update(batch_memcells)

        logger.debug(f"Batch get MemCells completed: Successfully retrieved {len(all_memcells)} items")
        return all_memcells

    async def _batch_get_group_profiles(self, user_group_pairs: List[Tuple[str, str]]) -> Dict[Tuple[str, str], Any]:
    ...
    async def group_by_groupid_stratagy(
        ...
        memcells_task = asyncio.create_task(self._batch_get_memcells(all_memcell_event_ids))
        profiles_task = asyncio.create_task(self._batch_get_group_profiles(all_user_group_pairs))

        # Wait for all tasks to complete
        memcells_cache, profiles_cache = await asyncio.gather(memcells_task, profiles_task)
        ...
        for hit in search_results:
            ...
            if user_id and group_id:
                ...
                if (
                    ...
                    group_importance_evidence = group_user_profile.group_importance_evidence
                    # Add group_importance_evidence to memory's extend field
                    if not hasattr(memory, "extend") or memory.extend is None:
                        memory.extend = {}
                    memory.extend["group_importance_evidence"] = group_importance_evidence
                    logger.debug(f"Added group_importance_evidence to memory: user_id={user_id}, group_id={group_id}")
            ...
            if group_importance_evidence:
                memories_by_group[group_id]["importance_evidence"] = group_importance_evidence
        ...
        for group_id, group_data in memories_by_group.items():
            ...
            group_data["memories"].sort(key=lambda m: m.timestamp if m.timestamp else "")

            # Calculate importance score
            importance_score = self._calculate_importance_score(group_data["importance_evidence"])
        ...
        total_count = sum(len(group_data["memories"]) for group_data in memories_by_group.values())
