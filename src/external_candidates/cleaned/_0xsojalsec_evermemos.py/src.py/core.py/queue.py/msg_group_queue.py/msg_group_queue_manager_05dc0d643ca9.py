...
class MsgGroupQueueManager:
    ...
    def __init__(
        ...
        # Queue statistics
        self._queue_stats: List[QueueStats] = [QueueStats(queue_id=i, current_size=0) for i in range(num_queues)]

        # Manager statistics
        self._manager_stats = ManagerStats(total_queues=num_queues, total_current_messages=0)
        ...
        self._delivery_events: List[deque] = [deque() for _ in range(num_queues)]  # Delivery events for each queue
        self._consume_events: List[deque] = [deque() for _ in range(num_queues)]  # Consumption events for each queue
    ...
    async def deliver_message(self, group_key: str, message_data: Any) -> bool:
        ...
        try:
            ...
            async with self._stats_lock:
                ...
                self._manager_stats.total_current_messages = self._get_total_current_messages()
    ...
    async def get_by_queue(
        ...
        if queue_id < 0 or queue_id >= self.num_queues:
            raise ValueError(f"Queue ID out of range: {queue_id}, valid range: 0-{self.num_queues - 1}")
        ...
        try:
            if wait:
                ...
                if timeout is not None:
                    message_tuple = await asyncio.wait_for(target_queue.get(), timeout=timeout)
                else:
    ...
    async def get_queue_info(self, queue_id: Optional[int] = None) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        """
    ...
    def _update_time_window_stats(self):
        ...
        for i in range(self.num_queues):
            ...
            self._queue_stats[i].time_window_stats.delivered_1min = self._count_events_in_window(
                self._delivery_events[i], 60.0
            )
            self._queue_stats[i].time_window_stats.consumed_1min = self._count_events_in_window(
                self._consume_events[i], 60.0
            )
            self._queue_stats[i].time_window_stats.delivered_1hour = self._count_events_in_window(
                self._delivery_events[i], 3600.0
            )
            self._queue_stats[i].time_window_stats.consumed_1hour = self._count_events_in_window(
                self._consume_events[i], 3600.0
            )

        # Update manager time window statistics
        self._manager_stats.time_window_stats.delivered_1min = self._count_events_in_window(
            self._manager_delivery_events, 60.0
        )
        self._manager_stats.time_window_stats.consumed_1min = self._count_events_in_window(
            self._manager_consume_events, 60.0
        )
        self._manager_stats.time_window_stats.delivered_1hour = self._count_events_in_window(
            self._manager_delivery_events, 3600.0
        )
        self._manager_stats.time_window_stats.consumed_1hour = self._count_events_in_window(
            self._manager_consume_events, 3600.0
        )
    ...
    async def start_periodic_logging(self):
        ...
        logger.info("📊 MsgGroupQueueManager[%s] Periodic logging task started", self.name)
    ...
    async def stop_periodic_logging(self):
        ...
        logger.info("📊 MsgGroupQueueManager[%s] Periodic logging task stopped", self.name)
    ...
    async def _periodic_log_worker(self):
        ...
        try:
            ...
            logger.debug("📊 MsgGroupQueueManager[%s] Periodic logging task cancelled", self.name)
        except (OSError, ValueError, RuntimeError) as e:
            logger.error(
    ...
    async def _log_queue_details(self):
        ...
        try:
            ...
            for queue_info in queue_infos:
                ...
                is_active = has_recent_activity or has_messages or has_historical_activity

                if is_active:
                    # Calculate delivery and consumption ratios
                    delivery_rate = total_delivered / max(1, total_delivered + total_consumed) * 100
                    consume_rate = total_consumed / max(1, total_delivered + total_consumed) * 100

                    # Format time display
                    last_deliver_display = last_deliver_time[-8:] if last_deliver_time else "none"
                    last_consume_display = last_consume_time[-8:] if last_consume_time else "none"
            ...
            if active_queues:
                ...
                busiest_queue = max(active_queues, key=lambda q: q["delivered"] + q["consumed"])
            ...
            high_backlog_queues = [q for q in active_queues if q["current"] > self.max_total_messages * 0.3]
            if high_backlog_queues:
                logger.warning(
                    "⚠️ Queue backlog warning: %s",
                    ", ".join([f"Queue[{q['id']}](%d messages)" for q in high_backlog_queues]),
                )
