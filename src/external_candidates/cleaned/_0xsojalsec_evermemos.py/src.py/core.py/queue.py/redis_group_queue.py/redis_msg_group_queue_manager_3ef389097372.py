# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\queue\redis_group_queue\redis_msg_group_queue_manager.py
"""
...
class RedisManagerStats:
    ...
    start_time: str = field(default_factory=lambda: to_iso_format(get_now_with_timezone()))
...
class RedisGroupQueueManager:
    ...
    def __init__(
        ...
        self.queue_prefix = f"{key_prefix}:queue:"  # Queue key prefix, used in Lua scripts
        self.queue_key_pattern = f"{key_prefix}:queue:{{partition}}"  # partition is 001-050
        self.owner_activate_time_zset_key = f"{key_prefix}:owner_activate_time_zset"  # owner active time zset
        self.queue_list_prefix = f"{key_prefix}:queue_list:"  # owner's queue_list prefix
        self.counter_key = f"{key_prefix}:counter"

        # Process-level owner ID (generated at startup, globally unique)
        self.owner_id = f"{self.key_prefix}_{int(time.time())}_{random.randint(10000, 99999)}"
        ...
        self.partition_names = [f"{i:03d}" for i in range(1, self.FIXED_PARTITION_COUNT + 1)]

        # Manager statistics
        self._manager_stats = RedisManagerStats(total_queues=0, total_current_messages=0)
    ...
    def _parse_rebalance_result(self, result: Any, expected_count: int) -> Tuple[bool, Tuple]:
        """
    ...
    def _convert_flat_to_dict(self, assigned_partitions_flat: Any) -> Dict[str, List[str]]:
        """
        ...
        """
        assigned_partitions = {}
        if isinstance(assigned_partitions_flat, list) and len(assigned_partitions_flat) > 0:
            for i in range(0, len(assigned_partitions_flat), 2):
                if i + 1 < len(assigned_partitions_flat):
                    owner_id = self._safe_decode_redis_value(assigned_partitions_flat[i])
                    ...
                    if isinstance(partitions_raw, list):
                        partitions = [self._safe_decode_redis_value(p) for p in partitions_raw]
                    else:
    ...
    async def _ensure_scripts_loaded(self):
        ...
        if self._enqueue_script is None:
            ...
            self._get_stats_script = self.redis_client.register_script(GET_QUEUE_STATS_SCRIPT)
            self._get_all_partitions_stats_script = self.redis_client.register_script(GET_ALL_PARTITIONS_STATS_SCRIPT)
            self._rebalance_partitions_script = self.redis_client.register_script(REBALANCE_PARTITIONS_SCRIPT)
            self._join_consumer_script = self.redis_client.register_script(JOIN_CONSUMER_SCRIPT)
            self._exit_consumer_script = self.redis_client.register_script(EXIT_CONSUMER_SCRIPT)
            self._keepalive_consumer_script = self.redis_client.register_script(KEEPALIVE_CONSUMER_SCRIPT)
            self._cleanup_inactive_owners_script = self.redis_client.register_script(CLEANUP_INACTIVE_OWNERS_SCRIPT)
            self._force_cleanup_script = self.redis_client.register_script(FORCE_CLEANUP_SCRIPT)
            self._get_messages_script = self.redis_client.register_script(GET_MESSAGES_SCRIPT)
        ...
        try:
            ...
            result = await self._enqueue_script(
                ...
                args=[
                    ...
                    (max_total_messages if max_total_messages is not None else self.max_total_messages),
                ],
            )
        ...
        try:
            ...
            result = await self._get_messages_script(
                ...
                args=[
                    ...
                    score_threshold,
                    (current_score if current_score is not None else self._default_sort_key(None)),
                ],
            )
            ...
            if status_str == "JOIN_REQUIRED":
                ...
                if _retry_depth <= 0:
                    ...
                    raise RuntimeError(f"JOIN_REQUIRED retry attempts exhausted: owner_id={owner_id}")
                ...
                return await self.get_messages(score_threshold, current_score, owner_id, _retry_depth - 1)
    ...
    async def rebalance_partitions(self) -> Tuple[int, Dict[str, List[str]]]:
        ...
        try:
            ...
            # Parse return result
            success, (owner_count, assigned_partitions) = self._parse_rebalance_result(result, 2)
            if not success:
                return 0, {}
    ...
    @rate_limit(max_rate=1, time_period=1, key_func=lambda owner_id: f"join_consumer_{owner_id}")
    async def join_consumer(self, owner_id: Optional[str] = None) -> Tuple[int, Dict[str, List[str]]]:
        """
        Join consumer
    ...
    @rate_limit(max_rate=1, time_period=1, key_func=lambda owner_id: f"exit_consumer_{owner_id}")
    async def exit_consumer(self, owner_id: Optional[str] = None) -> Tuple[int, Dict[str, List[str]]]:
        """
        Consumer exit
    ...
    async def cleanup_inactive_owners(self) -> Tuple[int, int, Dict[str, List[str]]]:
        ...
        try:
            ...
            current_time = int(time.time() * 1000)  # Millisecond timestamp
            inactive_threshold = current_time - (self.inactive_threshold_seconds * 1000)  # Convert to milliseconds
            ...
            # Parse return result
            success, (cleaned_count, owner_count, assigned_partitions) = self._parse_rebalance_result(result, 3)
            if not success:
                return 0, 0, {}
    ...
    async def get_stats(
        ...
        try:
            ...
            if group_key is not None and not include_all_partitions:
                ...
                result = await self._get_stats_script(keys=[queue_key, self.counter_key], args=[])
            ...
            stats.update(
                {
                    "type": ("manager_stats" if group_key is None else "all_partitions_stats"),
            ...
            if include_consumer_info:
                try:
                    active_owners_raw = await self.redis_client.zrange(self.owner_activate_time_zset_key, 0, -1)
                    # Safely decode owner list
                    active_owners = [self._safe_decode_redis_value(owner) for owner in active_owners_raw]
                    ...
                    for owner in active_owners:
                        ...
                        assigned_partitions_raw = await self.redis_client.lrange(queue_list_key, 0, -1)
                        # Safely decode partition list
                        assigned_partitions = [self._safe_decode_redis_value(p) for p in assigned_partitions_raw]
            ...
            if include_partition_details:
                ...
                for i in range(0, len(partition_stats_raw), 4):
                    if i + 3 < len(partition_stats_raw):
                        ...
                        partitions.append(
                            {
                                "partition": self._safe_decode_redis_value(partition_stats_raw[i]),
                ...
                stats["max_partition_size"] = max_partition_size if max_partition_size != 0 else 0
                stats["min_partition_size"] = min_partition_size if min_partition_size != float("inf") else 0

            return stats

        except (redis.RedisError, ValueError, TypeError) as e:
            logger.error("Failed to get statistics: group_key=%s, error=%s", group_key, e)
    ...
    async def start(self):
        ...
        if self._state == ManagerState.SHUTDOWN:
            raise RuntimeError(f"RedisGroupQueueManager[{self.key_prefix}] has been shut down, cannot restart")

        # State must be CREATED
        if self._state != ManagerState.CREATED:
            raise RuntimeError(f"RedisGroupQueueManager[{self.key_prefix}] state abnormal: {self._state}")
    ...
    async def start_periodic_tasks(self):
        ...
        logger.info("📊 RedisGroupQueueManager[%s] Periodic tasks started", self.key_prefix)
    ...
    async def stop_periodic_tasks(self):
        ...
        logger.info("📊 RedisGroupQueueManager[%s] Periodic tasks stopped", self.key_prefix)
            ...
            for partition in partitions:
                try:
                    ...
                    if queue_size > 0:
                        ...
                        min_result = await self.redis_client.zrange(queue_key, 0, 0, withscores=True)
                        max_result = await self.redis_client.zrange(queue_key, -1, -1, withscores=True)
                    ...
                    details_lines.append(f"   Partition[{partition}]: Failed to get status: {e}")
    ...
    async def shutdown(self, mode: ShutdownMode = ShutdownMode.HARD) -> bool:
        ...
        logger.info("🔌 RedisGroupQueueManager[%s] Starting shutdown...", self.key_prefix)
    ...
    def __repr__(self) -> str:
        return f"RedisGroupQueueManager(key_prefix={self.key_prefix}, max_total_messages={self.max_total_messages})"
