# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\longjob\longjob_runner.py
"""
...
async def run_longjob_mode(longjob_name: str):
    ...
    try:
        ...
        try:
            ...
            logger.error("❌ Unable to find long-running job '%s': %s", longjob_name, str(e))
            logger.info("💡 Please ensure the long-running job is correctly registered in the DI container")
            return

        # Check if it is an implementation of LongJobInterface
        if not isinstance(longjob_instance, LongJobInterface):
            logger.error("❌ '%s' is not an implementation of LongJobInterface", longjob_name)
            logger.info("💡 Long-running jobs must inherit from LongJobInterface or its subclasses")
            return
        ...
        logger.info("✅ Long-running job '%s' has started and is running...", longjob_name)
