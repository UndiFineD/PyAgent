...
class DocBase(AsyncDocument):
    ...
    def get_index_name(cls) -> str:
        ...
        raise ValueError(f"Document class {cls.__name__} does not have correct index configuration")
...
class AliasSupportDoc(DocBase):
    ...
    def __init__(self, meta: Dict[str, Any] = None, **kwargs: Any):
        ...
        for field_name, field_value in raw_kwargs.items():
            processed_kwargs[field_name] = self._process_date_field(field_name, field_value)
        ...
        id_source_field = getattr(custom_meta_class, "id_source_field", None) if custom_meta_class else None
        ...
        if given_meta_id is not None:
            ...
            if id_source_field and id_source_field in processed_kwargs:
                ...
                if source_value not in (None, "") and source_value != given_meta_id:
                    raise ValueError("meta.id conflicts with value from ID_SOURCE_FIELD")
            ...
            if id_source_field not in processed_kwargs or processed_kwargs[id_source_field] in (None, ""):
                raise ValueError(f"{self.__class__.__name__} requires non-empty '{id_source_field}' to set meta.id")
            source_value = processed_kwargs[id_source_field]
