# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\milvus\base_repository.py
"""
...
class BaseMilvusRepository(ABC, Generic[T]):
    ...
    async def insert(self, entity: T, flush: bool = False) -> str:
        ...
        try:
            ...
            logger.debug("✅ Insert entity successful [%s]: %s", self.model_name, entity_id)
            return entity_id
    ...
    async def upsert(self, entity: T, flush: bool = False) -> str:
        ...
        try:
            ...
            if flush:
                await self.collection.flush()
            logger.debug("✅ Upsert entity successful [%s]: %s", self.model_name, entity_id)
            return entity_id
        except Exception as e:
    ...
    async def delete_by_id(self, entity_id: str, flush: bool = False) -> bool:
        ...
        try:
            ...
            if flush and success:
                await self.collection.flush()
            if success:
                logger.debug("✅ Delete entity successful [%s]: %s", self.model_name, entity_id)
            return success
        except Exception as e:
