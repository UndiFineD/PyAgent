# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\milvus\milvus_collection_base.py
...
class MilvusCollectionBase:
    ...
    def __init__(self):
        ...
        if not self._COLLECTION_NAME:
            raise NotImplementedError(f"{self.__class__.__name__} must define '_COLLECTION_NAME' class attribute")
    ...
    def collection(cls) -> Collection:
        ...
        if cls._collection_instance is None:
            raise ValueError(f"{cls.__name__} Collection instance not created, please call ensure_loaded() first")
    ...
    def ensure_loaded(self) -> None:
        ...
        try:
            ...
            if load_state == LoadState.NotLoad:
                ...
                logger.info("Collection '%s' is loading, waiting for completion...", name)
                coll.load()
            else:
    ...
    def ensure_indexes(self) -> None:
        ...
        if not self._INDEX_CONFIGS:
            logger.info("Collection '%s' has no index configuration, skipping", self.name)
            return
    ...
    def _create_indexes_for_collection(self, coll: Collection) -> None:
        ...
        try:
            ...
            logger.info("Index check and creation completed for Collection '%s'", coll.name)
        except Exception as e:
            logger.error(
...
class MilvusCollectionWithSuffix(MilvusCollectionBase):
    ...
    def load_collection(self) -> Collection:
        ...
        if not utility.has_collection(name, using=self._using):
            ...
            utility.create_alias(collection_name=_collection_name, alias=name, using=self._using)
    ...
    def create_new_collection(self) -> Collection:
        ...
        """
        if not self._SCHEMA:
            raise NotImplementedError(f"{self.__class__.__name__} must define '_SCHEMA' to support collection creation")
    ...
    def switch_alias(self, new_collection: Collection, drop_old: bool = False) -> None:
        ...
        try:
            ...
            try:
                ...
                pass
            utility.create_alias(collection_name=new_real_name, alias=alias_name, using=self._using)
        ...
        if drop_old and old_real_name:
            try:
                ...
                logger.warning("Failed to delete old collection (can be handled manually): %s", e)

        # Refresh class-level cache to alias collection
        try:
            self.__class__._collection_instance = Collection(name=alias_name, using=self._using)
        except Exception:
            pass
    ...
    def drop(self) -> None:
        ...
        try:
            if not self._collection_instance:
                self._collection_instance = Collection(name=self.name, using=self._using)
            ...
            logger.warning("Collection '%s' does not exist or deletion failed: %s", self.name, e)
    ...
    class TestCollection(MilvusCollectionWithSuffix):
        ...
        _SCHEMA = CollectionSchema(
            fields=[
                ...
                FieldSchema(name="deleted_reason", dtype=DataType.VARCHAR, max_length=255),
            ]
        )
        _INDEX_CONFIGS = [
            IndexConfig(
