# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\addons\addonize\tests\test_addon_bean_order_strategy.py
# -*- coding: utf-8 -*-
"""
...
class TestCalculateOrderKey:
    ...
    def test_order_key_includes_addon_priority(self):
        ...
        key_core = AddonBeanOrderStrategy.calculate_order_key(bean_core, is_direct_match=True, mock_mode=False)
    ...
    def test_addon_priority_overrides_other_priorities(self):
        ...
        # Calculate sort key
        key1 = AddonBeanOrderStrategy.calculate_order_key(bean1, is_direct_match=True, mock_mode=False)
        key2 = AddonBeanOrderStrategy.calculate_order_key(bean2, is_direct_match=True, mock_mode=False)
    ...
    def test_order_key_with_mock_mode(self):
        ...
        mock_bean = BeanDefinition(ServiceA, is_mock=True, metadata={"addon_tag": "enterprise"})
        normal_bean = BeanDefinition(ServiceB, is_mock=False, metadata={"addon_tag": "enterprise"})

        # Calculate sort key (Mock mode)
        mock_key = AddonBeanOrderStrategy.calculate_order_key(mock_bean, is_direct_match=True, mock_mode=True)
        normal_key = AddonBeanOrderStrategy.calculate_order_key(normal_bean, is_direct_match=True, mock_mode=True)
    ...
    def test_order_key_backward_compatible(self):
        ...
        # Calculate sort key
        primary_key = AddonBeanOrderStrategy.calculate_order_key(primary_bean, is_direct_match=True, mock_mode=False)
        normal_key = AddonBeanOrderStrategy.calculate_order_key(normal_bean, is_direct_match=True, mock_mode=False)
...
class TestSortBeansWithContext:
    ...
    def test_sort_by_addon_priority(self):
        ...
        bean_defs = [
            BeanDefinition(ServiceA, bean_name="core_bean", metadata={"addon_tag": "core"}),
            BeanDefinition(
                ServiceB,
                bean_name="enterprise_bean",
                metadata={"addon_tag": "enterprise"},
            ),
            BeanDefinition(ServiceC, bean_name="custom_bean", metadata={"addon_tag": "custom"}),
...
class TestComplexScenarios:
    ...
    def test_multi_addon_multi_implementation(self):
        ...
        bean_defs = [
            ...
            BeanDefinition(ServiceC, bean_name="plugin2_impl", metadata={"addon_tag": "plugin2"}),
