# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\constants\errors.py
"""
...
def get_error_message_by_key(message_key: str, language: str = "zh") -> str:
    ...
    """
    if language == "zh":
        return ERROR_MESSAGES_ZH.get(message_key, ERROR_MESSAGES_ZH[ErrorMessage.UNKNOWN_ERROR.value])
    elif language == "en":
        return ERROR_MESSAGES_EN.get(message_key, ERROR_MESSAGES_EN[ErrorMessage.UNKNOWN_ERROR.value])
    else:
        # Default to Chinese
        return ERROR_MESSAGES_ZH.get(message_key, ERROR_MESSAGES_ZH[ErrorMessage.UNKNOWN_ERROR.value])
...
def _validate_translations_completeness():
    ...
    for code in ErrorCode:
        ...
        if code_value not in ERROR_CODE_TRANSLATIONS_ZH:
            missing_translations.append(f"ErrorCode {code_value} missing Chinese translation")

        # Check English translation
        if code_value not in ERROR_CODE_TRANSLATIONS_EN:
            missing_translations.append(f"ErrorCode {code_value} missing English translation")
    ...
    for message in ErrorMessage:
        ...
        if message_value not in ERROR_MESSAGES_ZH:
            missing_translations.append(f"ErrorMessage {message_value} missing Chinese translation")

        # Check English translation
        if message_value not in ERROR_MESSAGES_EN:
            missing_translations.append(f"ErrorMessage {message_value} missing English translation")
    ...
    if missing_translations:
        error_message = (
            ...
            + "\n".join(f"  - {msg}" for msg in missing_translations[:10])  # Show only first 10
        )
