# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\component\kafka_producer_factory.py
"""
...
def get_default_producer_config(
    ...
    kafka_servers = [server.strip() for server in kafka_servers_str.split(",") if server.strip()]
...
class KafkaProducerFactory:
    ...
    async def get_producer(self, kafka_servers: List[str], force_new: bool = False, **kwargs) -> AIOKafkaProducer:
        """
        ...
        async with self._lock:
            ...
            if force_new or cache_key not in self._producers:
                ...
                producer = await self.create_producer(kafka_servers=kafka_servers, **kwargs)
    ...
    async def send_batch(self, producer: AIOKafkaProducer, topic: str, messages: List[Dict[str, Any]]) -> List[Any]:
        """
