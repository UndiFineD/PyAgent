...
def get_cache_key(kafka_servers: List[str], kafka_topic: str, kafka_group_id: str) -> str:
    """
    Generate cache key
...
class KafkaConsumerFactory:
    ...
    async def get_default_consumer(self, force_new: bool = False, env_prefix: str = "") -> AIOKafkaConsumer:
        """
    ...
    async def remove_consumer(self, kafka_servers: List[str], kafka_topic: str, kafka_group_id: str) -> bool:
        """
    ...
    async def seek_to_datetime(self, offset_datetime: str, consumer: AIOKafkaConsumer) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("Consumer must be started before seeking to timestamp") from e
            ...
            if seek_count > 0:
                ...
                logger.warning("No partitions were seeked for datetime %s", offset_datetime)
                return False
            ...
            logger.error("Failed to seek consumer to datetime %s: %s", offset_datetime, e)
            raise RuntimeError(f"Failed to seek consumer to datetime {offset_datetime}") from e
