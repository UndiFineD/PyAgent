# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\component\elasticsearch_client_factory.py
"""
...
def get_default_es_config() -> Dict[str, Any]:
    ...
    logger.info("  Auth: %s", "API Key" if es_api_key else ("Basic" if es_username else "None"))
...
class ElasticsearchClientWrapper:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error("❌ Elasticsearch connection test failed: %s, error: %s", self.hosts, e)
            return False
...
class ElasticsearchClientFactory:
    ...
    async def register_default_client(self) -> ElasticsearchClientWrapper:
        ...
        # Register a default client
        async_connections.add_connection(alias="default", conn=default_client.async_client)
        ...
        async with self._lock:
            if cache_key in self._clients:
                ...
                try:
                    ...
                    logger.error("Error closing Elasticsearch client during removal: %s", e)
        ...
        async with self._lock:
            for cache_key, client_wrapper in self._clients.items():
                try:
                    ...
                    logger.error("Error closing Elasticsearch client %s: %s", cache_key, e)
