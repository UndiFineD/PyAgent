# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\nlp\stopwords_utils.py
"""
...
class StopwordsManager:
    ...
    def __init__(self, stopwords_file_path: Optional[str] = None):
        ...
        """
        self.stopwords_file_path = stopwords_file_path or self._get_default_stopwords_path()
    ...
    def load_stopwords(self) -> Set[str]:
        ...
        try:
            ...
            logger.info(f"Successfully loaded stopwords, total {len(stopwords)} stopwords")
            self._stopwords = stopwords
            return stopwords
    ...
    def filter_stopwords(self, words: list, min_length: int = 1) -> list:
        ...
        filtered_words = []
        for word in words:
            if word not in self._stopwords and len(word) >= min_length and word.strip():  # Filter whitespace characters
