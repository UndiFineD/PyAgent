...
class UserContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            user_data = await self.auth_provider.get_optional_user_data_from_request(request)
            ...
            if e.status_code == 401:
                logger.debug("Authentication failed, returning 401 error directly: %s", e.detail)
                raise e
            else:
                logger.error(
