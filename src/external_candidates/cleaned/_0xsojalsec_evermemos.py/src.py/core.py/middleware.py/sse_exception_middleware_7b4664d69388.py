# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\middleware\sse_exception_middleware.py
"""
...
def sse_exception_handler(
    ...
    async def wrapper(*args, **kwargs) -> AsyncGenerator[str, None]:
        try:
            ...
            logger.error(f"HTTP exception occurred in SSE stream: {e.status_code} - {e.detail}")
            ...
            logger.error(f"Unknown exception occurred in SSE stream: {e}", exc_info=True)
            yield yield_sse_data(error_data)
