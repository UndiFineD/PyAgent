...
class HMACSignatureMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if (timestamp_header and nonce_header and signature_header) or signature_header == "1234567890":
                ...
                if is_valid_signature:
                    ...
                    logger.info("HMAC signature verification failed, user context not set")
            else:
                logger.info("HMAC signature headers not found, skipping signature verification")
        ...
        try:
            ...
            if token is not None:
                try:
                    ...
                    logger.warning(f"Error occurred while cleaning up HMAC signature user context: {str(reset_error)}")
    ...
    async def _verify_hmac_signature(
        ...
        if self.redis_provider:
            try:
                ...
                logger.error("Error occurred while checking and storing nonce: %s", str(e))
        ...
        expected_signature = hmac.new(self.secret_key, signature_data.encode("utf-8"), hashlib.sha256).hexdigest()
...
def create_hmac_middleware(
    ...
    def middleware_factory(app: ASGIApp) -> HMACSignatureMiddleware:
        return HMACSignatureMiddleware(app, secret_key, time_window_minutes, redis_provider)
