# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\middleware\database_session_middleware.py
...
class DatabaseSessionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if isinstance(response, StreamingResponse) or isinstance(response, _StreamingResponse):
                ...
                wrapped_generator = self._wrap_streaming_generator(response.body_iterator, session)
            ...
            if not is_streaming:
                ...
                try:
                    clear_current_session(token)
                    logger.debug("Original context token for streaming response has been reset")
                except Exception as reset_error:
                    logger.warning(f"Failed to reset original context token for streaming response: {str(reset_error)}")
    ...
    async def _handle_failed_request(self, session: AsyncSession, original_exception: Exception) -> None:
        """
        ...
        try:
            ...
            await self._rollback_safely(session)
            logger.info(f"Request failed, transaction rollback executed: {str(original_exception)}")
    ...
    async def _wrap_streaming_generator(
        ...
        try:
            ...
            await self._handle_successful_request(session)
            logger.debug("Streaming response transmission completed, database session handled")

        except Exception as e:
            # Exception during streaming transmission, rollback session
            await self._handle_failed_request(session, e)
            logger.error(f"Streaming response transmission failed, database session rolled back: {str(e)}")
            ...
            try:
                ...
                logger.error(f"Error while cleaning up streaming response session resources: {str(cleanup_error)}")
