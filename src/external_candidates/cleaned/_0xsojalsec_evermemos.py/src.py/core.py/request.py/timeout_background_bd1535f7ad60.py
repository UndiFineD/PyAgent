# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\request\timeout_background.py
"""
    ...
    def decorator(
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> Union[T, JSONResponse]:
            ...
            if not background_enabled:
                ...
                logger.debug("[TimeoutBackground] Task '%s' executing in sync mode", task_name)
...
async def _run_background_task(
    ...
    try:
        ...
        logger.warning("[TimeoutBackground] Background task '%s' was cancelled", task_name)
...
async def _call_on_request_complete(provider: Any, http_code: int, error_message: Optional[str]) -> None:
    """
    Call provider's on_request_complete
    ...
    try:
        ...
        if request is None:
            logger.warning("[TimeoutBackground] Unable to get request, skipping on_request_complete")
            return

        await provider.on_request_complete(request=request, http_code=http_code, error_message=error_message)
    except Exception as e:
        logger.warning("[TimeoutBackground] on_request_complete callback execution failed: %s", e)
