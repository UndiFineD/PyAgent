# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\milvus\tenant_aware_collection.py
"""
...
class TenantAwareCollection(Collection):
    ...
    def _ensure_connection_registered(using: str) -> None:
        ...
        try:
            ...
            if not tenant_info:
                raise RuntimeError("Cannot register tenant connection: tenant context not set")
            ...
            logger.error("Failed to register tenant connection [using=%s]: %s", using, e)
            raise
    ...
    def _register_connection(config: dict, using: str) -> None:
        ...
        try:
            ...
            logger.error("Failed to register Milvus connection [using=%s]: %s", using, e)
            raise
