# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\lifespan\longjob_lifespan.py
"""
    ...
    async def startup(self, app: FastAPI) -> Any:
        ...
        try:
            ...
            if not self._longjob_name:
                logger.warning("⚠️ LONGJOB_NAME environment variable not set, skipping LongJob startup")
                return None
            # Core logic: create an async task to run the long-running job
            self._longjob_task = asyncio.create_task(run_longjob_mode(self._longjob_name))
