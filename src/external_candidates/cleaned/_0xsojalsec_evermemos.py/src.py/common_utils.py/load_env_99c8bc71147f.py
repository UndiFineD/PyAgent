# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\common_utils\load_env.py
#!/usr/bin/env python3
"""
...
def load_env_file(env_file_name: str = ".env", check_env_var: Optional[str] = None) -> bool:
    ...
    if check_env_var and os.getenv(check_env_var):
        ...
        if check_env_var:
            logger.error("Please ensure that the %s environment variable is set", check_env_var)
        return False
