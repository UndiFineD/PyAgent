# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\adlai88.py\simmer_copytrading.py\copytrading_trader_2a6fd47571f0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\adlai88\simmer-copytrading\copytrading_trader.py
...
def set_risk_monitor(
    ...
    try:
        return api_request(
            "POST",
            f"/api/sdk/positions/{market_id}/monitor",
            {
                "side": side,
...
def execute_copytrading(
    wallets: list,
    top_n: int = None,
    max_usd: float = 50.0,
    dry_run: bool = False,
    buy_only: bool = True,
    detect_whale_exits: bool = False,
    max_trades: int = None,
) -> dict:
    """
    ...
    if not result.get("success"):
        ...
        for t in trades:
            if t.get("success") and t.get("action") == "buy":
                ...
                if market_id:
                    risk_result = set_risk_monitor(market_id, side, stop_loss_pct=0.25, take_profit_pct=0.50)
...
def main():
    parser = argparse.ArgumentParser(description="Simmer Copytrading - Mirror positions from Polymarket whales")

    parser.add_argument("--dry-run", action="store_true", help="Show what would trade without executing")
    ...
    parser.add_argument("--max-usd", type=float, help="Max USD per position (overrides env var)")
