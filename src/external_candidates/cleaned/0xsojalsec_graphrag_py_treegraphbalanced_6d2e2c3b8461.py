# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\graph.py\treegraphbalanced_6d2e2c3b8461.py
...
class TreeGraphBalanced(BaseGraph):
    ...
    async def _create_node_without_embedding(self, layer: int, text: str, children_indices: Set[int] = None):
        ...
        logger.info("Create node_id = unassigned, children = {children}".format(node_id=0, children=children_indices))
        ...
        for layer in range(self.config.num_layers):  # build a new layer
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                for i in range(0, self.max_workers):
                    cluster_tasks = [
                        pool.submit(
                            self._create_task_for(self._extract_cluster_relationship_without_embedding),
                            layer=layer + 1,
                            cluster=cluster,
                        )
