# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\Mobile-Agent\Mobile-Agent-qwen\host.py
import argparse
import json
import os
import random
        ...
        while True:
            if error_flag == 0:
                operation_history = add_response(
                    "user",
                    f"The user's instruction is {instruction}. " + tutorial + thought_prompt,
                    operation_history,
                    image,
                )
            ...
            if "stop" in action:
                ...
                if len(in_coordinate) >= 1:
                    ...
                    data = {
                        "operation": action,
                        "action": "tap",
                        "parameter": json.dumps([tap_coordinate[0] * x, (tap_coordinate[1] - 0.03) * y]),
                        "session_id": session_id,
                    }
                ...
                if len(out_coordinate) == 0:
                    ...
                    data = {
                        "operation": action,
                        "action": "tap",
                        "parameter": json.dumps([tap_coordinate[0] * x, tap_coordinate[1] * y]),
                        "session_id": session_id,
                    }
                ...
                data = {
                    "operation": action,
                    "action": "slide",
                    "parameter": json.dumps([[int(x / 2), int(y / 2)], [int(x / 2), int(y / 4)]]),
                    "session_id": session_id,
                }
                ...
                data = {
                    "operation": action,
                    "action": "slide",
                    "parameter": json.dumps([[int(x / 2), int(y / 2)], [int(x / 2), int(3 * y / 4)]]),
                    "session_id": session_id,
                }
