# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MobileAgent\PC-Agent\pywin.py
import base64
import os
...
class WindowsACI(ACI):
    ...
    def filter_ocr_elements(
        ...
        if True:
            if ocr_bboxes:
                ...
                for idx, (box, max_iou) in enumerate(zip(ocr_bboxes, max_ious)):
                    # if max_iou < 0.1:
                    if max_iou < 0.2:
            ...
            if ocr_bboxes:
                ...
                for idx, ((_, content, box), max_iou) in enumerate(zip(ocr_bboxes["results"], max_ious)):
                    if max_iou < 0.1:
                        ...
                        linearized_accessibility_tree.append(f"{preserved_nodes_index}\tButton\t\t{content}\t\t")
    ...
    def linearize_and_annotate_tree(self, obs: Dict, show_all_elements: bool = False) -> str:
        ...
        for idx, node in enumerate(preserved_nodes):
            tree_elements.append(f"{idx}\t{node['role']}\t{node['title']}\t{node['text']}")

        if self.ocr:
            screenshot = obs.get("screenshot", None)
            if screenshot is not None:
                # return tree_elements, preserved_nodes
                tree_elements, preserved_nodes = self.add_ocr_elements(screenshot, tree_elements, preserved_nodes)
