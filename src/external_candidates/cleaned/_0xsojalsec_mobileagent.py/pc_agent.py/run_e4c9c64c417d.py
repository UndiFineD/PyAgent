...
def select(content, screenshot_file):
    ...
    text_sys = OpenOCR(mode="mobile", drop_score=0.5, det_box_type="quad")  # det_box_type: 'quad' or 'poly'
...
def get_perception_infos(screenshot_file, screenshot_som_file, font_path):
    ...
    if args.use_a11y == 0:
        ...
        elements_filtered = [
            ele for ele in elements if len(ele["text"]) < args.text_len_thre and ele["text"] not in exclude_words
        ...
        merged_icon_coordinates = [
            [ele[0] * ratio, ele[1] * ratio, ele[2] * ratio, ele[3] * ratio] for ele in merged_icon_coordinates
        ]
        ...
        if args.use_som == 1:
            ...
            if args.use_a11y == 0:
                ...
                if elements[i]["text"] != "None":
                    perception_info = {
                        "text": "mark number: " + str(mark_number) + " icon: " + elements[i]["text"],
                        "coordinates": merged_icon_coordinates[i],
                    }
                elif elements[i]["title"] != "None":
                    perception_info = {
                        "text": "mark number: " + str(mark_number) + " icon: " + elements[i]["title"],
                        "coordinates": merged_icon_coordinates[i],
                    }
                else:
                    perception_info = {
                        "text": "mark number: " + str(mark_number) + " icon: " + elements[i]["role"],
                        "coordinates": merged_icon_coordinates[i],
                    }
...
def analyze_string(s):
    ...
    #  {'key': 'value'}
    dict_pattern = re.compile(r"\{(?:\s*[\'\"]\w+[\'\"]\s*:\s*[\'\"][^{}\'\"]+[\'\"]\s*,?)*\}")
    ...
    for i in range(num_try_subtask):
        try:
            ...
            subtask_dict = json.loads(
                output_subtask.replace("```python", "```json").split("```json")[-1].split("```")[0]
            )
...
for i in range(num_subtask):
    ...
    if string_res["type"] in [2, 4]:
        ...
        try:
            ...
            sub_instruction = (
                sub_instruction.replace("{'", "{{'").replace("'}", "'}}").format_map(value_dict)
            )  # .format(**value_dict)
        except:
    ...
    while True:
        ...
        if reflection_switch:
            ...
            reflection_thought = (
                output_reflect.split("### Thought ###")[-1].split("### Answer ###")[0].replace("\n", " ").strip()
            )
