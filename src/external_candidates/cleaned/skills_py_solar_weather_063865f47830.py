# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\andrewdmwalker.py\solar_weather.py\solar_weather_063865f47830.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\andrewdmwalker\solar-weather\solar-weather.py
...
def cmd_forecast(args):
    ...
    if args.json:
        print(json.dumps({"today": today, "tomorrow": tomorrow, "day_after": day_after}, indent=2))
        ...
        print(
            f"   R: {tomorrow.get('R', {}).get('Text', 'N/A')} (Minor: {tomorrow.get('R', {}).get('MinorProb', 'N/A')}%, Major: {tomorrow.get('R', {}).get('MajorProb', 'N/A')}%)"
        )
        ...
        print(
            f"   R: {day_after.get('R', {}).get('Text', 'N/A')} (Minor: {day_after.get('R', {}).get('MinorProb', 'N/A')}%, Major: {day_after.get('R', {}).get('MajorProb', 'N/A')}%)"
        )
