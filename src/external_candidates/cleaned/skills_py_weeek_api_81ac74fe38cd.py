# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alex_indi.py\weeek_tasks.py\scripts.py\weeek_api_81ac74fe38cd.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alex-indi\weeek-tasks\scripts\weeek_api.py
...
def cmd_update_task(args):
    ...
    print(
        json.dumps(
            request("PUT", f"/tm/tasks/{args.id}", body=body),
            ensure_ascii=False,
            indent=2,
        )
    )
...
def cmd_complete(args):
    print(
        json.dumps(
            request("POST", f"/tm/tasks/{args.id}/complete"),
            ensure_ascii=False,
            indent=2,
        )
    )
...
def cmd_uncomplete(args):
    print(
        json.dumps(
            request("POST", f"/tm/tasks/{args.id}/uncomplete"),
            ensure_ascii=False,
            indent=2,
        )
    )
...
def cmd_move_board(args):
    ...
    print(
        json.dumps(
            request("POST", f"/tm/tasks/{args.id}/board", body=body),
            ensure_ascii=False,
            indent=2,
        )
    )
...
def cmd_move_board_column(args):
    ...
    print(
        json.dumps(
            request("POST", f"/tm/tasks/{args.id}/board-column", body=body),
            ensure_ascii=False,
            indent=2,
        )
    )
...
def cmd_list_board_columns(args):
    ...
    print(
        json.dumps(
            request("GET", "/tm/board-columns", params=params),
            ensure_ascii=False,
            indent=2,
        )
    )
