# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_withoutbg.py\tests.py\conftest_95ec179c0a60.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-withoutbg\tests\conftest.py
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "performance: Performance and benchmark tests (very slow)")

    config.addinivalue_line("markers", "api: Tests requiring API access (may be skipped)")

    config.addinivalue_line("markers", "slow: Slow running tests")

    config.addinivalue_line("markers", "real_processing: Tests using actual ONNX models (very slow)")
...
def pytest_runtest_setup(item):
    ...
    if "real_processing" in item.keywords:
        ...
        if not item.config.getoption("--run-real-processing", default=False):
            pytest.skip("Real processing tests skipped (use --run-real-processing to run)")
...
def assert_image_properties(image, expected_mode=None, expected_size=None, expected_format=None):
    ...
    if expected_format:
        assert image.format == expected_format, f"Expected format {expected_format}, got {image.format}"
...
def assert_processing_result(result, original_image):
    ...
    assert alpha_channel.min() >= 0 and alpha_channel.max() <= 255, "Alpha values should be in valid range"
...
class PerformanceTracker:
    ...
    def assert_performance(self, name, max_value, message=None):
        ...
        assert actual_value <= max_value, (
            message or f"Performance regression: {name} = {actual_value} > {max_value} {metric['unit']}"
        )
