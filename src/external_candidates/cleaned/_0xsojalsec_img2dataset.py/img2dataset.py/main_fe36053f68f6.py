# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-img2dataset\img2dataset\main.py
"""Img2dataset"""
...
def arguments_validator(params):
    ...
    if params["verify_hash"] is not None:
        ...
        if verify_hash_type != params["compute_hash"]:
            raise ValueError(
                f"verify_hash and compute_hash must be the same but got {verify_hash_type} and {params['compute_hash']}"
            )
