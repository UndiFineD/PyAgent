# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\command_routing\tcp_command_router.py
import base64
import logging
import queue
import re
...
class TcpCommandRouter:
    ...
    def __init__(self, session):
        ...
        logger.debug(
            "[%s] TcpCommandRouter initialized for session %r",
    ...
    def execute(
        ...
        with self.session.exec_lock:
            ...
            logger.debug(
                "[%s] execute() completed in %.4fs, result=%r",
                time.strftime("%Y-%m-%d %H:%M:%S"),
                elapsed,
                result,
            )
    ...
    def send(
        ...
        if defender.is_active and not defender_bypass and os_type in ("windows", "linux"):
            if not defender.inspect_command(os_type, cmd):
                logger.debug(
                    "[%s] Command blocked by Session-Defender",
                    time.strftime("%H:%M:%S"),
                )
        ...
        logger.debug(
            "[%s] Wrapped command for op_id=%r: %r",
            time.strftime("%H:%M:%S"),
            op_id,
            wrapped,
        )
    ...
    def receive(
        ...
        logger.debug(
            "[%s] receive() called: op_id=%r, timeout=%s, portscan_active=%r, retries=%d",
            time.strftime("%H:%M:%S"),
            op_id,
            timeout,
            portscan_active,
        ...
        for m in pattern.finditer(resp_bytes):
            ...
            logger.debug(
                "[%s] Demuxed output for other op %r into queue (size=%d)",
                time.strftime("%H:%M:%S"),
                o,
                q.qsize(),
            )
        ...
        match = re.search(
            rf"{re.escape(start.decode())}\s*(.*?)\s*{re.escape(end.decode())}",
        ...
        if match:
            ...
            try:
                ...
                logger.debug(
                    "[%s] No queued output for op_id=%r",
                    time.strftime("%H:%M:%S"),
                    op_id,
                )
                my_out = ""
        ...
        logger.debug(
            "[%s] Raw extracted output for op_id=%r: %r",
            time.strftime("%H:%M:%S"),
            op_id,
            my_out,
        )
        ...
        logger.debug(
            "[%s] Normalized output for op_id=%r: %r",
            time.strftime("%H:%M:%S"),
            op_id,
            clean,
        )
    ...
    def flush_response(self, op_id: str = "console"):
        ...
        logger.debug(
            "[%s] flush_response cleared %d for op_id=%r",
            time.strftime("%H:%M:%S"),
            count,
            op_id,
        )
    ...
    def flush_commands(self, op_id: str = "console"):
        ...
        while not q.empty():
            ...
            count += 1
        logger.debug(
            "[%s] flush_commands cleared %d for op_id=%r",
            time.strftime("%H:%M:%S"),
            count,
            op_id,
        )
