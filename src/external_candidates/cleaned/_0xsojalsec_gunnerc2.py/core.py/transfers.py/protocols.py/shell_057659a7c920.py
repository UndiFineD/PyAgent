...
class ShellProtocol(TransferProtocol):
    ...
    def _run_cmd(
        ...
        try:
            out = (
                http_exec.run_command_http(sid, cmd, op_id=op_id, transfer_use=True, timeout=_eff_timeout)
                if tr in ("http", "https")
    ...
    def _linux_read_chunk(self, st: TransferState, index: int) -> bytes:
        ...
        bs = st.chunk_size
        cmd = (
            f"dd if={_linux_shq(st.remote_path)} bs={bs} skip={index} count=1 status=none iflag=fullblock | base64 -w 0"
        )
