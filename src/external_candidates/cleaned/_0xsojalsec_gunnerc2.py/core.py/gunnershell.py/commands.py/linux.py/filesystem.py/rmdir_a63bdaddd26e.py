# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\linux\filesystem\rmdir.py
import shlex
...
class RmdirCommand(Command):
    ...
    @property
    def help(self):
        return "rmdir <path>    Remove a directory (recursive, force)"
    ...
    def logic(self, sid, argv, op_id="console"):
        p = QuietParser(prog="rmdir", add_help=False)
        p.add_argument("path")
        try:
            ns = p.parse_args(argv)
        except SystemExit:
            return ""
        # Use rm -rf for convenience (rmdir requires empty dir)
        return _run(sid, f"rm -rf {shlex.quote(ns.path)} && echo 'OK'", op_id=op_id)
