...
class KerbruteBruteforceCommand(Command):
    ...
    @property
    def help(self):
        return "kerbrute bruteforce -u <user|userfile> -p <pass|passfile> -d <domain> --dc-ip <ip> [-C <credfile>]"
...
"""
        ...
        if out:
            ...
            hits = [line for line in out.splitlines() if not line.startswith("Nothing Found")]
            if hits:
                return "\n".join(hits)
