# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\listeners\tcp.py
import logging
...
from core.listeners.base import (
    Listener,
    _reg_lock,
    register_listener,
    socket_to_listener,
...
def _generate_tls_context(listen_ip: str) -> ssl.SSLContext:
    ...
    subject = issuer = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "GunnerC2")])
    builder = (
    ...
    san = x509.SubjectAlternativeName([x509.DNSName("GunnerC2"), x509.IPAddress(ipaddress.IPv4Address(listen_ip))])
...
class TcpListener(Listener):
    ...
    def start(
        ...
        # spawn accept loop
        self._thread = threading.Thread(target=self.run_loop, args=(self._stop_event, context), daemon=True)
        self._thread.start()
