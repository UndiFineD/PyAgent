    ...
    def accept_loop():
        ...
        while not shutdown_event.is_set():
            try:
                ...
                enc_cli.sendall(f"[+] Authenticated as {username} with role {role} (operator ID {op_id})\n".encode())
...
def resolve_operator(raw: str) -> str | None:
    ...
    if any(ch in raw for ch in "*?"):
        ...
        matches += [op.op_id for op in operators.values() if op.alias and fnmatch.fnmatch(op.alias, raw)]
        ...
        if len(matches) == 1:
            ...
            print(brightred + f"[!] Ambiguous operator pattern '{raw}' → matches {matches!r}" + reset)
            return None
