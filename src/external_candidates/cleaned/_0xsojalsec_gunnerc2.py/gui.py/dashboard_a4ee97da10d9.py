...
class Dashboard(QWidget):
            ...
            for i in range(1, self.split.count()):
                ...
                if h:
                    ...
                    h.setCursor(
                        Qt.ArrowCursor
                        if locked
                        else (Qt.SplitVCursor if self.split.orientation() == Qt.Vertical else Qt.SplitHCursor)
                    )
    ...
    def _open_console_tab(self, sid: str, hostname: str):
        ...
        username = s.get("user") or s.get("username") or (s.get("metadata") or {}).get("user") or ""
        ...
        if not username and ws:
            ...
            def _cb(msg):
                ...
                u2 = s2.get("user") or s2.get("username") or (s2.get("metadata") or {}).get("user") or ""
                if not u2:
                    return
