...
class C2Item(QGraphicsItem):
    ...
    def boundingRect(self) -> QRectF:
        ...
        return self._rect.adjusted(-8, -8, 8, 8).united(self.title.mapRectToParent(self.title.boundingRect()))
...
class SessionGraph(QWidget):
    ...
    def _apply_filters_to_items(self):
        ...
        for it in self.agent_items.values():
            ...
            os_ok = (is_win and self._show_windows) or (is_lin and self._show_linux) or (not is_win and not is_lin)
        ...
        def _find_spawn_pos(new_item: AgentItem) -> QPointF:
            ...
            for ring in range(max_rings):
                ...
                for s in range(slots):
                    ...
                    cand = QPointF(c2c.x() + math.cos(ang) * r, c2c.y() + math.sin(ang) * r)
