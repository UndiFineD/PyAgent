...
class PayloadsTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.out.setStyleSheet("QPlainTextEdit { background:#141820; color:#e6e6e6; border:1px solid #2a2f3a; }")
        # micro-optimizations
    ...
    def _choose_profile(self):
        ...
        path, _ = QFileDialog.getOpenFileName(self, "Select Profile", "", "Profile files (*.profile);;All files (*)")
        if path:
            self.profile.setText(path)

    def _choose_output_file(self):
        """Pick a local path to write text payloads (ps1/bash)."""
        path, _ = QFileDialog.getSaveFileName(self, "Select Output File", "payload.txt", "All Files (*);;Text (*.txt)")
        if path:
