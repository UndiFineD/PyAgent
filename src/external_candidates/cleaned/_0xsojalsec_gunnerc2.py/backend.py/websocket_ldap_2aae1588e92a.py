# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\websocket_ldap.py
...
from . import config
from .logutil import bind, get_logger, redacts
...
BASE = "BASE"
LEVEL = "LEVEL"
SUBTREE = "SUBTREE"  # dummies for type hints only
...
def _ps_read(dn: str) -> str:
    ...
    return (
        r"""$ErrorActionPreference='SilentlyContinue';$dn=%s;try{$e=[ADSI]('LDAP://'+$dn);$a=@{};foreach($p in $e.Properties.PropertyNames){$a[$p]=@($e.Properties[$p])};$a|ConvertTo-Json -Compress -Depth 8}catch{'{"__err__":"read_failed"}'}"""
        % (_psq(dn),)
    )
...
def _run_remote_ps(sid: str, ps: str, timeout: float, op_id: str, log) -> str:
    ...
    try:
        ...
        logger.exception(
            "ps.exec.error",
            extra={"sid": redacts(sid, show=4), "op": op_id, "err": repr(e)},
        )
        out = ""
...
async def ldap_ws(ws: WebSocket):
    ...
    async def _handle_open(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not sid:
            await _ws_send(
                ws,
                _err("ldap.opened", "missing sid (agent-only mode)", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        _log_json(
            "ldap.open_current.info",
            info,
            extra={"wsid": wsid, "sid": redacts(sid, show=4)},
        )

        if not info or not (info.get("host") or info.get("defaultNamingContext")):
            await _ws_send(
                ws,
                _err("ldap.opened", "discovery returned no DC", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        shaped = {
            ...
            "use_ssl": False,
            "start_tls": False,
            "namingContexts": info.get("namingContexts")
            or ([] if not info.get("defaultNamingContext") else [info.get("defaultNamingContext")]),
    ...
    async def _handle_discover(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not sid:
            await _ws_send(
                ws,
                _err("ldap.discovered", "missing sid", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        await _ws_send(
            ws,
            _ok({"discover": data}, "ldap.discovered", req_id=req_id),
            _log_wsid=wsid,
        )
    ...
    async def _handle_open_current(req: Dict[str, Any], req_id: Optional[str]):
        ...
        sid = _resolve_sid(req.get("sid") or default_sid or "")
        if not sid:
            await _ws_send(ws, _err("ldap.opened", "missing sid", req_id=req_id), _log_wsid=wsid)
            return
        out = _run_remote_ps(
            sid,
            _PS_DISCOVER,
        ...
        if not info or not (info.get("host") or info.get("defaultNamingContext")):
            await _ws_send(ws, _err("ldap.opened", "discovery returned no DC", req_id=req_id), log)
            return
    ...
    async def _handle_rootdse(req: Dict[str, Any], req_id: Optional[str]):
        if use_agent:
            ...
            await _ws_send(
                ws,
                _ok({"rootdse": data}, "ldap.rootdse", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        # server mode
        if conn is None:
            await _ws_send(ws, _err("ldap.rootdse", "not connected", req_id=req_id), log)
            return
    ...
    async def _handle_read(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not dn:
            await _ws_send(ws, _err("ldap.read", "missing dn", req_id=req_id), log)
            return
            ...
            if (out or "").strip():
                with suppress(Exception):
                    ...
                    if isinstance(data, dict) and data.get("__err__"):
                        await _ws_send(
                            ws,
                            _err("ldap.read", data["__err__"], req_id=req_id, dn=dn),
                            log,
                        )
                        return
                    ...
                    await _ws_send(
                        ws,
                        _ok({"dn": dn, "attrs": data or {}}, "ldap.read", req_id=req_id),
                        _log_wsid=wsid,
                    )
                    return
            await _ws_send(
                ws,
                _ok({"dn": dn, "attrs": {}}, "ldap.read", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        if conn is None:
            await _ws_send(
                ws,
                _err("ldap.children", "not connected", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        await _ws_send(
            ws,
            _ok({"dn": dn, "attrs": vals}, "ldap.read", req_id=req_id),
            _log_wsid=wsid,
        )
    ...
    async def _handle_children(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not dn:
            await _ws_send(ws, _err("ldap.children", "missing dn", req_id=req_id), log)
            return

        if use_agent:
            out = _run_remote_ps(
                agent_sid or "",
                _ps_children(dn, size),
            ...
            await _ws_send(
                ws,
                _ok({"dn": dn, "children": rows}, "ldap.children", req_id=req_id),
                _log_wsid=wsid,
            )
            return

        if conn is None:
            await _ws_send(ws, _err("ldap.children", "not connected", req_id=req_id), log)
            return
        ...
        def _list_children() -> List[Dict[str, Any]]:
            ...
            gen = conn.extend.standard.paged_search(
                ...
                search_scope=LEVEL,
                attributes=attrs,
                paged_size=max(1, min(size, 1000)),
                generator=True,
            )
            rows: List[Dict[str, Any]] = []
            count = 0
    ...
    async def _handle_search(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not base:
            await _ws_send(
                ws,
                _err("ldap.search.page", "missing base", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        if use_agent:
            out = _run_remote_ps(
                agent_sid or "",
                _ps_search(base, scope, filt, size),
                timeout=float(req.get("timeout", 12.0)),
                op_id="ldap",
                log=log,
            )
            ...
            await _ws_send(
                ws,
                _ok(
                    {"base": base, "rows": rows, "cookie": next_cookie},
                    "ldap.search.page",
                    req_id=req_id,
                ),
                _log_wsid=wsid,
        ...
        if conn is None:
            await _ws_send(
                ws,
                _err("ldap.search.page", "not connected", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        _log_json(
            "ldap.search.server.rows",
            rows,
            extra={"wsid": wsid, "base": base, "cookie": next_cookie},
        )
        await _ws_send(
            ws,
            _ok(
    ...
    async def _handle_quick(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not base:
            await _ws_send(
                ws,
                _err("ldap.quick", "missing base", req_id=req_id, kind=kind),
                _log_wsid=wsid,
            )
            return
        ...
        if not filt:
            await _ws_send(
                ws,
                _err("ldap.quick", f"bad kind: {kind}", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        if use_agent:
            ...
            _log_json(
                "ldap.quick.agent.rows",
                shaped,
                extra={"wsid": wsid, "kind": kind, "base": base},
            )
            await _ws_send(
                ws,
                _ok({"kind": kind, "rows": shaped}, "ldap.quick", req_id=req_id),
                _log_wsid=wsid,
            )
            return
        ...
        if conn is None:
            await _ws_send(
                ws,
                _err("ldap.quick", "not connected", req_id=req_id, kind=kind),
                _log_wsid=wsid,
            )
            return
        ...
        _log_json(
            "ldap.quick.server.rows",
            rows,
            extra={"wsid": wsid, "kind": kind, "base": base},
        )
        await _ws_send(
            ws,
            _ok({"kind": kind, "rows": rows}, "ldap.quick", req_id=req_id),
            _log_wsid=wsid,
        )
    ...
    try:
        while True:
            ...
            try:
                ...
                await _ws_send(ws, _err("ldap.error", "bad json"), _log_wsid=wsid)
                continue
            ...
            if act in ("ping", "__ping__", "health"):
                await _ws_send(ws, _ok({"ts": time.time()}, "pong", req_id=req_id), _log_wsid=wsid)
                continue
            ...
            if act == "ldap.open":
                ...
                await _close()
                await _ws_send(ws, _ok({}, "ldap.closed", req_id=req_id), _log_wsid=wsid)
                ...
                await _ws_send(
                    ws,
                    _err("ldap.error", f"unknown action: {act}", req_id=req_id),
                    _log_wsid=wsid,
                )
        ...
        logger.exception(
            "ws.loop.crash",
            extra={"wsid": wsid, "err": repr(e), "trace": traceback.format_exc()},
        )
