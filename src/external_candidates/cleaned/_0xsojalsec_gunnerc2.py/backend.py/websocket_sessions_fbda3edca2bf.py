# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\websocket_sessions.py
    ...
    try:
        ...
        return await _ws_send(
            ws,
            {
                "type": "error",
                "req_id": req.get("req_id"),
                "error": f"Failed to close TCP session: {e}",
            },
        )

    await _ws_send(
        ws,
        {"type": "error", "req_id": req.get("req_id"), "error": "Unknown session type"},
    )
...
async def sessions_ws(ws: WebSocket):
    ...
    async def reader():
        actions = {
            "list": lambda w, r: _ws_send(
                w,
                {
                    "type": "snapshot",
                    "req_id": r.get("req_id"),
                    "sessions": _serialize_sessions(),
                },
            ),
            "get": _cmd_get,
            "kill": _cmd_kill,
