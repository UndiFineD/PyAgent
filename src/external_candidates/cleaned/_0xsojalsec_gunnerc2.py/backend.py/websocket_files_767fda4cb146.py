...
async def files_ws(ws: WebSocket):
    ...
    async def _do_download(req: Dict[str, Any]):
        ...
        fname = (
            ntpath.basename(path) if os_type == "windows" or "\\" in path else os.path.basename(path)
        ) or "file.bin"
    ...
    async def _do_upload_begin(req: Dict[str, Any]):
        nonlocal \
            active_upload_tmp, \
            active_upload_expect, \
            active_upload_sid, \
            active_upload_remote_archive, \
            active_upload_remote_dir, \
            active_upload_is_folder
    ...
    async def _do_upload_finish():
        ...
        try:
            meta = {
                ...
                "size": (
                    os.path.getsize(active_upload_tmp) if active_upload_tmp and os.path.exists(active_upload_tmp) else 0
                ),
                "magic": file_magic(active_upload_tmp) if active_upload_tmp else "",
                "sha256": (
                    sha256_path(active_upload_tmp) if active_upload_tmp and os.path.exists(active_upload_tmp) else ""
                ),
            }
