# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2015.py\ka_playgrounds.py\transform_article_449455c3c54d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\ka_playgrounds\transform_article.py
...
def half_plane():
    ...
    plane.add_coordinates(x_vals=list(range(-6, 7, 2)), y_vals=list(range(-6, 7, 2)))
...
class SingleVariableFunction(Scene):
    ...
    def construct(self, func, name, separate_lines):
        ...
        transform_config = {"run_time": 3, "path_func": path_along_arc(np.pi / 4)}
        ...
        if separate_lines:
            self.play(*[ApplyMethod(mob.shift, 0.4 * UP) for mob in negative_numbers])
...
class LineToPlaneFunction(Scene):
    args_list = [
        ...
        (
            lambda x: (np.cos(x), 0.5 * x * np.sin(x)),
            "Swirl",
            [
                ("0", "(1, 0)", 0),
    ...
    def construct(self, func, name, numbers_to_follow):
        ...
        plane.filter_out(lambda x_y_z2: abs(x_y_z2[0]) > 0.1 and abs(x_y_z2[1]) > 0.1)
        ...
        for input_tex, output_tex, number in numbers_to_follow:
            ...
            anims.append(ApplyMethod(dot.shift, point_function(center) - center, **anim_config))
...
class PlaneToPlaneFunctionSeparatePlanes(Scene):
    args_list = [
        (
            lambda x_y3: (x_y3[0] ** 2 + x_y3[1] ** 2, x_y3[0] ** 2 - x_y3[1] ** 2),
            "Quadratic",
        )
    ]
...
class PlaneToPlaneFunction(Scene):
    args_list = [
        (
            lambda x_y4: (x_y4[0] ** 2 + x_y4[1] ** 2, x_y4[0] ** 2 - x_y4[1] ** 2),
            "Quadratic",
        )
    ]
...
class PlaneToSpaceFunction(Scene):
    args_list = [
        (
            lambda x_y1: (x_y1[0] * x_y1[0], x_y1[0] * x_y1[1], x_y1[1] * x_y1[1]),
            "Quadratic",
        ),
    ]
