# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\graph.py\treegraph_ab784e970410.py
...
class TreeGraph(BaseGraph):
    ...
    async def _process_cluster(self, i, global_clusters, embeddings, dim, threshold):
        ...
        if len(global_cluster_embeddings_) <= dim + 1:
            ...
            local_clusters, n_local_clusters = await self._GMM_cluster(reduced_embeddings_local, threshold)
    ...
    async def _perform_clustering(
        ...
        global_clusters, n_global_clusters = await self._GMM_cluster(reduced_embeddings_global, threshold)
    ...
    async def _clustering(
        ...
        clusters = await self._perform_clustering(embeddings, dim=reduction_dimension, threshold=threshold)
