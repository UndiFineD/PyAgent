# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\once_useful_constructs.py\graph_theory_e5896dded7ca.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\graph_theory.py
...
class DiscreteGraphScene(Scene):
    ...
    def construct(self):
        ...
        self.edges = self.lines = [Line(self._points[i], self._points[j]) for i, j in self.graph.edges]
    ...
    def accent_vertices(self, **kwargs):
        ...
        end = Mobject(*[Dot(point, radius=3 * Dot.DEFAULT_RADIUS, color="lightgreen") for point in self._points])
    ...
    def replace_vertices_with(self, mobject):
        ...
        self.play(
            *[CounterclockwiseTransform(vertex, mobject.copy().shift(vertex.get_center())) for vertex in self.vertices]
            + [ApplyMethod(edge.scale, (edge.get_length() - diameter) / edge.get_length()) for edge in self.edges]
        )
    ...
    def annotate_edges(self, mobject, fade_in=True, **kwargs):
        ...
        self.edge_annotations = [
            mobject.copy().rotate(angle).move_to(edge.get_center()) for angle, edge in zip(angles, self.edges)
        ]
    ...
    def trace_cycle(self, cycle=None, color="yellow", run_time=2.0):
        ...
        self.traced_cycle = Mobject(
            *[Line(self._points[i], self._points[j]).set_color(color) for i, j in zip(cycle, next_in_cycle)]
        )
    ...
    def generate_treeified_spanning_tree(self):
        ...
        parent_of = dict([tuple(reversed(pair)) for pair in self.spanning_tree_index_pairs])
        ...
        for index in indices:
            ...
            while next_generation != []:
                next_generation = reduce(op.add, [children_of[node] for node in next_generation])
        ...
        new_points = [np.array([x_coord_of[index], y_coord_of[index], 0]) for index in indices]

        self.treeified_spanning_tree = Mobject(
            *[Line(new_points[i], new_points[j]).set_color(color) for i, j in self.spanning_tree_index_pairs]
        )
    ...
    def generate_dual_graph(self):
        ...
        self.dual_points = [center_of_mass([self._points[index] for index in cycle]) for cycle in cycles]

        self.dual_vertices = [Dot(point).set_color("green") for point in self.dual_points]
        ...
        for pair in self.graph.edges:
            ...
            for i in 0, 1:
                if all(dual_point_pair[i] == point_at_infinity):
                    ...
                    vect = center_of_mass([self._points[pair[0]], self._points[pair[1]]]) - new_point
