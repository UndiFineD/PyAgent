# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_offline.py\src.py\second_brain_offline.py\infrastructure.py\notion.py\document_cbd75112d1b0.py
...
class NotionDocumentClient:
    ...
    def __parse_blocks(self, blocks: list[dict], depth: int = 0) -> tuple[str, list[str]]:
        ...
        for block in blocks:
            ...
            if block_type != "child_page" and "has_children" in block and block["has_children"]:
