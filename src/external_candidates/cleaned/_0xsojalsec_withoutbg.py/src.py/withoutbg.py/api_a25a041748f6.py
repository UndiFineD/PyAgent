# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-withoutbg\src\withoutbg\api.py
...
class StudioAPI:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: str = "https://api.withoutbg.com"):
    ...
    def _resize_for_api(self, image: Image.Image, max_size: int = 1024) -> tuple[Image.Image, tuple[int, int]]:
    ...
    def _apply_alpha_channel(self, original_image: Image.Image, alpha_image: Image.Image) -> Image.Image:
        ...
        if alpha_image.size != original_image.size:
            alpha_image = alpha_image.resize(original_image.size, Image.Resampling.LANCZOS)
    ...
    def remove_background(self, input_image: Union[str, Path, Image.Image, bytes], **kwargs: Any) -> Image.Image:
        ...
        try:
            ...
            response = self.session.post(f"{self.base_url}/v1.0/alpha-channel-base64", json=payload, timeout=30)
            ...
            if response.status_code == 401:
                ...
                raise APIError("Insufficient credits. Get more credits at https://withoutbg.com/login")
            elif response.status_code == 403:
                raise APIError(
                    "Credits expired. Top up your account to reactivate frozen credits at https://withoutbg.com/login"
                )
            ...
            if "alpha_base64" not in result_data:
                raise APIError(f"Invalid API response format\n\nMore info: sample response: {result_data}")
