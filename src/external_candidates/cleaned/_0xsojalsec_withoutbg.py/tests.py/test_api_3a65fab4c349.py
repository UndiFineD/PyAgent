# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-withoutbg\tests\test_api.py
...
class TestStudioAPI:
    ...
    def mock_alpha_path(self):
        ...
        return Path(__file__).parent / "fixtures" / "images" / "test-ice-cream-api-service-mock-alpha.png"
    ...
    def test_remove_background_success(self, mock_post, api_client, test_image, mock_alpha_image):
        """Test successful background removal with real alpha channel."""
        # Setup mock response
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.ok = True
        mock_response.json.return_value = self._create_mock_alpha_response(mock_alpha_image)
    ...
    @patch("src.withoutbg.api.requests.Session.post")
    def test_remove_background_with_file_path(self, mock_post, api_client, test_image_path, mock_alpha_image):
    ...
    @patch("src.withoutbg.api.requests.Session.post")
    def test_remove_background_with_bytes(self, mock_post, api_client, test_image_path, mock_alpha_image):
    ...
    @patch("src.withoutbg.api.requests.Session.post")
    def test_image_resizing_for_large_images(self, mock_post, api_client, mock_alpha_image):
    ...
    @patch("src.withoutbg.api.requests.Session.post")
    def test_alpha_channel_application(self, mock_post, api_client, test_image, mock_alpha_image):
    ...
    @patch("src.withoutbg.api.requests.Session.post")
    def test_api_error_402_insufficient_credits(self, mock_post, api_client, test_image):
