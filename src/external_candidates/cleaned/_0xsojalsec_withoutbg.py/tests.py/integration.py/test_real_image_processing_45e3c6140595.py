...
def assert_alpha_channel_valid(result_image: Image.Image) -> None:
    ...
    assert alpha_min < alpha_max, f"Alpha channel lacks variation: min={alpha_min}, max={alpha_max}"
...
def calculate_alpha_iou(result: Image.Image, expected: Image.Image) -> float:
    ...
    """
    assert result.size == expected.size, f"Size mismatch: {result.size} vs {expected.size}"
...
def assert_alpha_iou(result: Image.Image, expected: Image.Image, min_iou: float = 0.97) -> None:
...
def assert_images_similar(result: Image.Image, expected: Image.Image, tolerance: float = 0.15) -> None:
    """Compare two images with tolerance for minor differences."""
    assert result.size == expected.size, f"Size mismatch: {result.size} vs {expected.size}"
    assert result.mode == expected.mode, f"Mode mismatch: {result.mode} vs {expected.mode}"
...
class TestRealImageProcessing:
    ...
    def test_ice_cream_processing(self, real_snap_model, test_images_dir, expected_outputs_dir):
    ...
    def test_different_input_formats(self, real_snap_model, test_images_dir):
        ...
        assert result_pil.size == result_path.size == result_pathobj.size == result_bytes.size
    ...
    def test_different_image_formats(self, real_snap_model, sample_test_image, format_name, pil_format):
    ...
    def test_alpha_iou_metric(self, real_snap_model, test_images_dir, expected_outputs_dir):
...
class TestRealProcessingPerformance:
    ...
    def test_processing_speed_benchmark(self, real_snap_model, test_images_dir, performance_tracker):
