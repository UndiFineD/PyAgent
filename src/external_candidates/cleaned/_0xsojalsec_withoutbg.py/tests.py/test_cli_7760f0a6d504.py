...
class TestCLIUnit:
    ...
    def test_quality_validation(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".jpg") as temp_file:
            ...
            # Invalid type
            result = self.runner.invoke(main, [temp_file.name, "--quality", "not_a_number"])
            assert result.exit_code != 0
            assert "Invalid value" in result.output
    ...
    def test_api_key_option_override(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".jpg") as temp_file:
            ...
            result = self.runner.invoke(main, [temp_file.name, "--api-key", "option_key"], env=env)
            # Should not complain about missing API key
            assert "API key required" not in result.output
...
class TestCLIIntegration:
    ...
    def test_single_image_processing_snap_model(self, mock_remove_bg, test_image_file):
        ...
        assert result.exit_code == 0
        mock_remove_bg.assert_called_once_with(test_image_file, api_key=None, model_name="snap")

    @patch("src.withoutbg.cli.remove_background")
    def test_single_image_processing_with_output_path(self, mock_remove_bg, test_image_file, temp_dir):
        """Test processing with custom output path."""
        result_image = Image.new("RGBA", (256, 256), color=(255, 0, 0, 128))
        mock_remove_bg.return_value = result_image

        output_path = temp_dir / "custom_output.png"
        result = self.runner.invoke(main, [str(test_image_file), "--output", str(output_path)])
    ...
    def test_single_image_processing_studio_api(self, mock_remove_bg, test_image_file):
        ...
        result = self.runner.invoke(main, [str(test_image_file), "--model", "studio", "--api-key", "test_key"])

        assert result.exit_code == 0
        mock_remove_bg.assert_called_once_with(test_image_file, api_key="test_key", model_name="studio")
    ...
    def test_use_api_flag_forces_studio_model(self, mock_remove_bg, test_image_file):
        ...
        result = self.runner.invoke(main, [str(test_image_file), "--use-api", "--api-key", "test_key"])

        assert result.exit_code == 0
        mock_remove_bg.assert_called_once_with(test_image_file, api_key="test_key", model_name="studio")

    @patch("src.withoutbg.cli.remove_background")
    def test_batch_processing_directory(self, mock_remove_bg, test_images_directory, temp_dir):
    ...
    @patch("src.withoutbg.cli.remove_background")
    def test_batch_processing_creates_default_output_dir(self, mock_remove_bg, test_images_directory):
    ...
    def test_output_format_options(self, mock_remove_bg, test_image_file, temp_dir):
        ...
        for fmt in formats:
            ...
            assert result.exit_code == 0, f"Format {fmt} failed with output: {result.output}"
            assert output_path.exists()
            ...
            assert saved_image.format == expected_format, f"Expected {expected_format}, got {saved_image.format}"
    ...
    def test_jpeg_quality_setting(self, mock_remove_bg, test_image_file, temp_dir):
        ...
        assert result.exit_code == 0, f"JPEG quality test failed with output: {result.output}"
        assert output_path.exists()
    ...
    def test_output_filename_generation(self, test_image_file, temp_dir):
        ...
        with patch("src.withoutbg.cli.remove_background") as mock_remove_bg:
            ...
            expected_output = test_image_file.parent / f"{test_image_file.stem}-withoutbg.png"
...
class TestCLIEdgeCases:
    ...
    def test_rgba_to_jpg_conversion(self, mock_remove_bg, temp_dir):
        ...
        mock_remove_bg.return_value = Image.new("RGBA", (100, 100), color=(255, 0, 0, 128))

        output_path = temp_dir / "output.jpg"
        result = self.runner.invoke(main, [str(image_path), "--output", str(output_path), "--format", "jpg"])

        assert result.exit_code == 0, f"RGBA to JPG conversion failed with output: {result.output}"
        assert output_path.exists()
...
class TestCLITestUtilities:
    ...
    def test_verify_output_file_properties(self, temp_dir):
        ...
        def verify_output_properties(output_path, expected_format=None, expected_mode=None, expected_size=None):
