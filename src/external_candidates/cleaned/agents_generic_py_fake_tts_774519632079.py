# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agents_generic.py\tests.py\fake_tts_774519632079.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\fake_tts.py
...
class FakeTTS(TTS):
    ...
    def stream(self, *, conn_options: APIConnectOptions = DEFAULT_API_CONNECT_OPTIONS) -> FakeSynthesizeStream:
...
class FakeChunkedStream(ChunkedStream):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter):
        ...
        if resp.audio_duration > 0.0:
            ...
            max_samples = int(self._tts.sample_rate * resp.audio_duration + 0.5) * self._tts.num_channels
