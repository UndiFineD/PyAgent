# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\query.py\togquery_eef1e1d6aae5.py
...
class ToGQuery(BaseQuery):
    ...
    async def _extend_reasoning_paths_per_depth(self, query: str) -> tuple[bool, list[list[tuple]], list, list, list]:
        ...
        for index, entity in enumerate(self.topic_entity_candidates):
            if entity != "[FINISH]":
                relations, relations_dict = await self._retriever.retrieve_relevant_content(
