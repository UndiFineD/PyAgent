# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\chatgpt_codex_client_16ecb5ccc870.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\chatgpt_codex_client.py
...
class ChatGPTCodexAsyncClient(httpx.AsyncClient):
    ...
    async def send(self, request: httpx.Request, *args: Any, **kwargs: Any) -> httpx.Response:
        ...
        try:
            ...
            if request.method == "POST":
                ...
                if body_bytes:
                    updated, force_stream_conversion = self._inject_codex_fields(body_bytes)
    ...
    def _inject_codex_fields(body: bytes) -> tuple[bytes | None, bool]:
        ...
        if isinstance(input_items, list):
            for item in input_items:
                ...
                if isinstance(item_id, str) and item_id and "reasoning" in item_id and not item_id.startswith("rs_"):
    ...
    async def _convert_stream_to_response(self, response: httpx.Response) -> httpx.Response:
        ...
        logger.debug(f"Collected {len(collected_text)} text chunks, {len(collected_tool_calls)} tool calls")

        if final_response_data:
            logger.debug(f"Got final response data with keys: {list(final_response_data.keys())}")
