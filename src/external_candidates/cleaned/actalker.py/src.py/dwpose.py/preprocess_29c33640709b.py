# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\dwpose\preprocess.py
import decord
import numpy as np
...
def get_video_pose(video_path: str, ref_image: np.ndarray, sample_stride: int = 1, max_frame: int = 120):
    ...
    frames = frames[0 : min(len(frames), max_frame)]
    ...
    return np.stack(output_pose), frames
