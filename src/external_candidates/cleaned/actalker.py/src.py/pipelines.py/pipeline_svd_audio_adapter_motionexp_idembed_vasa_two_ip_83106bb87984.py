# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\pipelines\pipeline_svd_audio_adapter_motionexp_idembed_vasa_two_ip.py
import inspect
import pdb
...
class Pose2VideoLongSVDPipeline(DiffusionPipeline):
    ...
    def _clip_encode_image(
        ...
        if do_classifier_free_guidance:
            ...
            vasa_prompts = torch.stack(vasa_prompts, dim=0).to(device=device, dtype=dtype)
            audio_prompts = audio_prompts.unsqueeze(0)
            vasa_prompts = vasa_prompts.unsqueeze(0).unsqueeze(2)
            ...
            uncond_vasa_prompts = torch.stack(uncond_vasa_prompts, dim=0).to(device=device, dtype=dtype)
            uncond_vasa_prompts = uncond_vasa_prompts.unsqueeze(0).unsqueeze(2)

            pad_uncond_audio_prompt = uncond_audio_prompts[:, :1].repeat(1, frames_per_batch, 1, 1)
            pad_uncond_vasa_prompt = uncond_vasa_prompts[:, :1].repeat(1, frames_per_batch, 1, 1)
            ...
            vasa_prompts = torch.cat([vasa_prompts, pad_uncond_vasa_prompt], dim=1)
            uncond_vasa_prompts = torch.cat([uncond_vasa_prompts, pad_uncond_vasa_prompt], dim=1)
        ...
        return image_embeddings, audio_prompts, vasa_prompts
    ...
    def _encode_vae_image(
        ...
        if do_classifier_free_guidance:
            ...
            image_latents = torch.cat([negative_image_latents, image_latents, image_latents, image_latents])
    ...
    def _get_add_time_ids(
        ...
        if do_classifier_free_guidance:
            add_time_ids = torch.cat([add_time_ids, add_time_ids, add_time_ids, add_time_ids])
    ...
    @property
    def guidance_scale3(self):
        return self._guidance_scale3
    ...
    def __call__(
        ...
        exp_mask_images: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        mouth_mask_images: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        ...
        vasa_prompts: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        uncond_vasa_prompts: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        height: int = 576,
        width: int = 1024,
        ...
        min_guidance_scale3=1.0,  # 1.0,
        max_guidance_scale3=3.0,
        fps: int = 7,
        ...
        image_embeddings, audio_prompts, vasa_prompts = self._clip_encode_image(
            clip_image,
            audio_prompts,
            uncond_audio_prompts,
            vasa_prompts,
        ...
        print("min_guidance_scale3", min_guidance_scale3)
        print("max_guidance_scale3", max_guidance_scale3)
        ...
        for exp_mask_image in exp_mask_images:
            exp_mask_tensor = exp_mask_image.unsqueeze(0)
            exp_mask_tensor = exp_mask_tensor.unsqueeze(2)  # (bs, c, 1, h, w)
            exp_mask_tensor_list.append(exp_mask_tensor)
        for mouth_mask_image in mouth_mask_images:
            mouth_mask_tensor = mouth_mask_image.unsqueeze(0)
            mouth_mask_tensor = mouth_mask_tensor.unsqueeze(2)  # (bs, c, 1, h, w)
            mouth_mask_tensor_list.append(mouth_mask_tensor)
        ...
        exp_mask_tensor = torch.cat(exp_mask_tensor_list, dim=2)  # (bs, c, t, h, w)
        mouth_mask_tensor = torch.cat(mouth_mask_tensor_list, dim=2)  # (bs, c, t, h, w)
        exp_mask_tensor = exp_mask_tensor.to(device=device, dtype=self.pose_guider.dtype)
        mouth_mask_tensor = mouth_mask_tensor.to(device=device, dtype=self.pose_guider.dtype)
        exp_mask = exp_mask_tensor[0, :1, :1]
        mouth_mask = mouth_mask_tensor[0, :1, :1]
        # print(exp_mask_tensor.shape)
        # print(mouth_mask_tensor.shape)
        pose_fea = self.pose_guider(pose_cond_tensor).transpose(1, 2)  # (bs, f, c, H, W)
        ...
        guidance_scale = torch.linspace(min_guidance_scale2, max_guidance_scale2, num_inference_steps)
        guidance_scale2 = guidance_scale.to(device, latents.dtype)

        guidance_scale = torch.linspace(min_guidance_scale3, max_guidance_scale3, num_inference_steps)
        guidance_scale3 = guidance_scale.to(device, latents.dtype)

        # print(guidance_scale)

        self._guidance_scale1 = guidance_scale1
        self._guidance_scale2 = guidance_scale2
        self._guidance_scale3 = guidance_scale3
        # 8. Denoising loop
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for batch, index_start in enumerate(
                    range(0, num_frames + frames_per_batch, frames_per_batch - overlap)
                ):
                    ...
                    batch_vasa_prompts = indice_slice(vasa_prompts, idx_list)

                    mask_list = []
                    if gate[0] == 1 and gate[1] == 1:
                        mask_list = [mouth_mask, exp_mask]
                    elif gate[0] == 1 and gate[1] == 0:
                        mask_list = [face_mask, torch.zeros_like(face_mask).to(device)]
                    elif gate[0] == 0 and gate[1] == 1:
                        mask_list = [torch.zeros_like(face_mask).to(device), face_mask]

                    cross_attention_kwargs = {"ip_adapter_masks": mask_list}
                    ...
                    noise_pred = self.unet(
                        ...
                        encoder_hidden_states=(
                            ...
                            [
                                batch_audio_prompts.flatten(0, 1) * gate[0],
                                batch_vasa_prompts.flatten(0, 1) * gate[1],
                            ],
                        ),
                    ...
                    if do_classifier_free_guidance:
                        (
                            noise_pred_uncond,
                            noise_pred_drop_audio_vasa,
                            noise_pred_drop_vasa,
                            noise_pred_cond,
                        ) = noise_pred.chunk(4)
                        noise_pred = (
                            noise_pred_uncond
                            + self.guidance_scale1[i] * (noise_pred_drop_audio_vasa - noise_pred_uncond)
                            + self.guidance_scale2[i] * (noise_pred_drop_vasa - noise_pred_drop_audio_vasa)
                            + self.guidance_scale3[i] * (noise_pred_cond - noise_pred_drop_vasa)
                        )
                        # noise_pred = noise_pred_uncond + self.guidance_scale1[i] * (noise_pred_drop_audio_vasa - noise_pred_uncond) + self.guidance_scale2[i] * (noise_pred_drop_vasa - noise_pred_drop_audio_vasa)
