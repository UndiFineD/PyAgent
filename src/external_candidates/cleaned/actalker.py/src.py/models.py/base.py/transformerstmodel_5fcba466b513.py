...
class TransformerSpatioTemporalModel(nn.Module):
    ...
    def forward(
        ...
        def spatial2time(time_context):
            ...
            time_context = time_context.reshape(batch_size, num_frames, time_context.shape[-2], time_context.shape[-1])
...
class TransformerSpatioTemporalModel_mambaID_v4_wo_audio(nn.Module):
    ...
    def forward(
        ...
        for block, mamba_block, temporal_block in zip(
            ...
            if self.training and self.gradient_checkpointing:
                hidden_states = torch.utils.checkpoint.checkpoint(mamba_block, hidden_states, encoder_hidden_states[0])
            else:
                hidden_states = mamba_block(hidden_states, encoder_hidden_states[0])
            hidden_states_mix = hidden_states
            hidden_states_mix = hidden_states_mix + emb
