# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\utils\enhance\model_enhance.py
import math
import os
import random
import sys
...
class Generator(nn.Module):
    ...
    def forward_LatentAndNoise(self, latent, noise):
        """
        Input:
            latent: B*self.n_latent*512
            noise: list, [B*C*H1*W1, B*C*H2*W2, ...]
        Return:
            img: B*3*H*W
        """
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], getIntermFeat=False):
        super().__init__()
...
class FullGenerator_Makeup(nn.Module):
    def __init__(
        self,
        size,
        style_dim,
    ...
    def forward(
        ...
        for i in range(self.log_size - 1):
            ...
            noise.append(inputs)
            # print(inputs.shape)
        inputs = inputs.view(inputs.shape[0], -1)  # [B, 512*4*4]
        outs = self.final_linear(inputs)
        # print('z shape: ',outs.shape)
        outs = self.generator(
            [outs],
            return_latents,
            inject_index,
        ...
        # print('predict im shape: ', outs[0].shape)
        return outs


class FullGenerator_paper(nn.Module):
    def __init__(
        self,
        size,
        style_dim,
    ...
    def get_LatentAndNoise_FromImage(self, im):
        """
        input:
            im: B*3*H*W
        return:
            latent: B*n_latent*512
            noise
        """
        noise = []
        inputs = im
        for i in range(self.log_size - 1):
            ...
            # print(inputs.shape)
        inputs = inputs.view(inputs.shape[0], -1)  # [B, 512*4*4]
        z = self.final_linear(inputs)  # z, [B, 512]
        latent = self.generator.style(z)
        latent = latent.unsqueeze(1).repeat(1, self.generator.n_latent, 1)
        noise = noise[::-1]
        return latent, noise
    ...
    def forward_LatentAndNoise(self, latent, noise):
        ...
        Return:
            img: B*3*H*W
        """
        im = self.generator.forward_LatentAndNoise(latent, noise)
        return im
...
def get_enhance_model(pth_path="pretrained_weights/enhance-512.pth", is_training=True):
    net = FullGenerator_paper(size=512, style_dim=512, n_mlp=8, channel_multiplier=2)

    state_dict = torch.load(pth_path, map_location=lambda storage, loc: storage)
    info = net.load_state_dict(state_dict, strict=False)
    print(f"get_enhance_model {pth_path}", info)
