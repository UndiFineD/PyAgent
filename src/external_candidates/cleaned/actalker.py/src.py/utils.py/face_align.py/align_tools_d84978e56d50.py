# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\utils\face_align\align_tools.py
import math
...
def points_117_158_256(points158: List[float], points117: List[float]) -> List[float]:
    ...
    def Merge(densePoints, sparsePoints):
        ...
        # 从 sparse 中取轮廓的点位
        spase_profile_start = sparse_nose_start + NOSE_POINTS_NUM + SPARSE_MOUTH_POINTS_NUM
    ...
    def ConvertPupilSparseToDense(points):
        ...
        lradius = math.sqrt(pow((lcenter.x - lboundary.x), 2) + pow((lcenter.y - lboundary.y), 2))
        rradius = math.sqrt(pow((rcenter.x - rboundary.x), 2) + pow((rcenter.y - rboundary.y), 2))
