# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\utils\face_align\utils.py
import math
...
def draw_pts_color(img, pts, mode="pts", shift=4, color=(255, 255, 0), radius=1):
    ...
    for cnt, p in enumerate(pts):
        ...
        cv2.circle(
            img,
            (int(p[0] * (1 << shift)), int(p[1] * (1 << shift))),
            radius << shift,
            color,
            2,
            cv2.LINE_AA,
...
def landmark_warpAffine(land, M):
    ...
    for i in range(len(land)):
        pts = []
        pts.append(
            np.squeeze(np.array(M[0]))[0] * land[i][0]
            + np.squeeze(np.array(M[0]))[1] * land[i][1]
            + np.squeeze(np.array(M[0]))[2]
        )
        pts.append(
            np.squeeze(np.array(M[1]))[0] * land[i][0]
            + np.squeeze(np.array(M[1]))[1] * land[i][1]
...
def align_N_picasso(img_im, orgi_landmarks, tar_landmarks, align_size, rnd_scale=1.0):
    ...
    left_part, left_M = crop_part(
        img_im,
        np.concatenate((pts_phase1[0:8, :], pts_phase1[16:24, :]), axis=0),
        M,
        align_size,
        rnd_scale,
    )
    right_part, right_M = crop_part(
        img_im,
        np.concatenate((pts_phase1[8:16, :], pts_phase1[24:32, :]), axis=0),
        M,
        align_size,
        rnd_scale,
        flip=True,
...
def align_N_picasso_aug2(img_im, orgi_landmarks, tar_landmarks, align_size, rnd_scale=1.0):
    ...
    left_part, left_M = crop_part_aug2(
        img_im,
        np.concatenate((pts_phase1[0:8, :], pts_phase1[16:24, :]), axis=0),
        M,
        align_size,
    ...
    right_part, right_M = crop_part_aug2(
        img_im,
        np.concatenate((pts_phase1[8:16, :], pts_phase1[24:32, :]), axis=0),
        M,
        align_size,
...
def align_N_picasso_aug3(img_im, orgi_landmarks, tar_landmarks, align_size, rnd_scale=1.0):
    ...
    left_part, left_M, _ = crop_part_aug3(
        img_im,
        np.concatenate((pts_phase1[0:8, :], pts_phase1[16:24, :]), axis=0),
        M,
        align_size,
    ...
    right_part, right_M, _ = crop_part_aug3(
        img_im,
        np.concatenate((pts_phase1[8:16, :], pts_phase1[24:32, :]), axis=0),
        M,
        align_size,
...
def align_N_picasso_aug4(img_im, orgi_landmarks, tar_landmarks, net_size, rnd_scale=1.0):
    ...
    align_size = [112, 112]
    mean_landmarks_eye = read_pts(meanf.replace("phase2", "eye_brow")) * [
        align_size[0] / 112.0,
        align_size[1] / 112.0,
    ]
    mean_landmarks_reye = read_pts(meanf.replace("phase2", "reye_brow")) * [
        align_size[0] / 112.0,
        align_size[1] / 112.0,
    ]
    mean_landmarks_mouth = read_pts(meanf.replace("phase2", "mouth")) * [
        align_size[0] / 112.0,
        align_size[1] / 112.0,
    ]
...
def align_N_picasso_aug5(img_im, orgi_landmarks, tar_landmarks, align_size, rnd_scale=1.0):
    ...
    left_part, left_M = crop_part_aug5(
        img_im,
        np.concatenate((pts_phase1[0:8, :], pts_phase1[16:24, :]), axis=0),
        M,
        align_size,
    ...
    right_part, right_M = crop_part_aug5(
        img_im,
        np.concatenate((pts_phase1[8:16, :], pts_phase1[24:32, :]), axis=0),
        M,
        align_size,
