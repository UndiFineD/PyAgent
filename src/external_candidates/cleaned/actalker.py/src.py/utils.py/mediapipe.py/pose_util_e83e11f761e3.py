# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\utils\mediapipe\pose_util.py
import math
import pdb
...
def invert_projection(projected_points, transformation_matrix, pose_vectors, image_shape):
    ...
    for i in range(L):
        ...
        transformation_inv = np.linalg.inv(
            transformation_matrix @ euler_and_translation_to_matrix(pose_vectors[i][:3], pose_vectors[i][3:])
        )
