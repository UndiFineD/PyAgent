# Extracted from: C:\DEV\PyAgent\.external\ACTalker\utils\video_level_evaluation.py
import io
import os
import pdb
import time
        ...
        if verbose:
            for i, (gt_patch, res_patch) in enumerate(zip(gt_list, res_list)):
                gt_path = os.path.join(
                    output_dir,
                    f"gt_{os.path.basename(video_path).split('.')[0]}_{i}.jpg",
                )
                res_path = os.path.join(
                    output_dir,
                    f"res_{os.path.basename(video_path).split('.')[0]}_{i}.jpg",
                )
