# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\crud\crud_data.py
        ...
        *,
        ...
        if not_questionnaire_id is not None:
            if isinstance(not_questionnaire_id, list):
                query = query.find(NotIn(self.model.questionnaire_id, not_questionnaire_id))
            else:
        ...
        if invalid is not None:
            if invalid:
                query = query.find({"evaluation.questionnaire_evaluation.is_invalid_questionnaire": True})
            else:
                query = query.find({"evaluation.questionnaire_evaluation.is_invalid_questionnaire": False})
