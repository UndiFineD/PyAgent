# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\operator\label_task_stat.py
import json
import tempfile
...
async def stats_label_task(
    ...
    for v in extract_choice_config(tool_config, req.scope):
        ...
        for res in (
            await crud.data.query(task_id=task.task_id, status=schemas.data.DataStatus.COMPLETED)
            .aggregate(
    ...
    if req.kind == schemas.operator.stats.ANSWER_FLITER_KIND.WITHOUT_DUPLICATE:
        ...
        async def export_stream_data() -> AsyncGenerator:
            ...
            for v in res:
                ...
                dedup_h[str(v.data.data_id)] = 1
                index += 1
                datas += schemas.data.DoData.model_validate(v.data, from_attributes=True).model_dump_json() + "\n"
                if index % 100 == 0:
                    yield datas
                    datas = ""
            if datas:
        ...
        async def export_stream_data() -> AsyncGenerator:
            ...
            for r in filter_questionnaire(req.operator, req.filters, res2):
                for i in range(len(r.datas)):
                    ...
                    index += 1
                    datas += (
                        schemas.data.DoData.model_validate(r.datas[i], from_attributes=True).model_dump_json() + "\n"
                    )
...
async def list_label_id(
    ...
    if req.kind == schemas.operator.stats.ANSWER_FLITER_KIND.WITHOUT_DUPLICATE:
        ...
        for v in res:
            ...
            dedup_h[str(v.data.data_id)] = 1
            data_ids.append(
                schemas.operator.stats.DoDataRecord(data_id=v.data.data_id, questionnaire_id=v.data.questionnaire_id)
            )
        return schemas.operator.stats.RespFilterAnswerDataID(_id=req.task_id, data=data_ids)
        ...
        return schemas.operator.stats.RespFilterAnswerQuestionnaireID(_id=req.task_id, data=questionnaire_ids)
