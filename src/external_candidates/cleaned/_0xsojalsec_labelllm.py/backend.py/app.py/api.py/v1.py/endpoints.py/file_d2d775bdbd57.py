# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\file.py
import time
...
async def file_upload(
    ...
    await redis_session.zremrangebyscore(f"file:upload:limit:{user.user_id}", 0, int(time.time()) - 60)
...
async def file_preview(path: str):
    url = minio.client.presigned_get_object(settings.MINIO_BUCKET, path, expires=timedelta(days=1))
