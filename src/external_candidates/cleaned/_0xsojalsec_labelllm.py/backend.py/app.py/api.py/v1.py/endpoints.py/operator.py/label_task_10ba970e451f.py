# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\operator\label_task.py
import io
import random
import tempfile
import urllib.parse
...
async def derive_label_task(
    ...
    for data in req.data:
        if data.data_ids:
            ...
            if data.user_id:
                records = await crud.record.query(task_id=data.task_id, user_id=data.user_id).to_list()
        ...
        if len(datas) == 0:
            ...
            res.append(schemas.operator.task.RespLabelTaskCreateWithDataBase(is_ok=True))
        tasks.append(datas)
    ...
    for item, datas in zip(req.data, tasks):
        ...
        new_datas = [
            models.data.DataCreate(
                ...
                prompt=(data.prompt if item.data_format == schemas.data.DataFormat.RAW_LABEL_AUDIT else data.prompt),
                conversation_id=data.conversation_id,
                conversation=data.conversation,
                reference_evaluation=(
                    data.reference_evaluation
...
async def update_label_task(
    ...
    # 进行中的任务不允许修改任务配置
    if task.status == schemas.task.TaskStatus.OPEN and (req.tool_config or req.expire_time):
        raise exceptions.TASK_STATUS_NOT_ALLOW

    # 任务状态不允许越级修改
    if (req.status == schemas.task.TaskStatus.DONE and task.status != schemas.task.TaskStatus.OPEN) or (
        req.status == schemas.task.TaskStatus.OPEN and task.status != schemas.task.TaskStatus.CREATED
    ):
...
async def get_label_task(
    ...
    async for record in crud.record.query(task_id=task.task_id, status=schemas.record.RecordStatus.PROCESSING):
            ...
            [
                {
                    "$group": {
                        "_id": "$task_id",
                        "time": {"$sum": {"$subtract": ["$submit_time", "$create_time"]}},
                    }
                }
            ],
    ...
    resp = schemas.operator.task.RespGetLabelTask(
        ...
        progress=schemas.operator.task.LabelTaskProgress(
            completed=task_progress.total - task_progress.pending - task_progress.labeling,
        ...
        users=schemas.operator.task.LabelTaskUsers(
            labeling=[schemas.user.DoUserWithUsername(user_id=user.user_id, username=user.name) for user in users or []]
        ),
    )
...
async def reject_data(
    ...
    try:
        async with redis_session.lock(f"audit_task_scheduler_job_{req.task_id}", blocking=False):
            # 获取任务
            task = await crud.label_task.query(task_id=req.task_id).first_or_none()
            if not task:
                raise exceptions.TASK_NOT_EXIST
            ...
            if len(new_datas) > 0:
                ...
                await crud.record.query(task_id=req.task_id, data_id=new_data_ids, user_id=req.user_id).set(
...
async def export_data(
    ...
    async def export_stream_data(task_id) -> AsyncGenerator:
        datas = ""
        index = 0
        async for data in crud.data.query(task_id=task_id, status=status, invalid=invalid):
            index += 1
            datas += schemas.data.DoData.model_validate(data, from_attributes=True).model_dump_json() + "\n"
...
async def list_by_questionnaire_id(
    ...
    datas: list[models.data.Data] = await crud.data.query(
        ...
        invalid=(True if req.record_status == schemas.operator.task.RecordFullStatus.INVALID else None),
    ).to_list()

    return schemas.operator.task.RespQuestionnaireDataIDs(data=[v.data_id for v in datas])
...
async def export_record(
    ...
    async def export_stream_data(task_id) -> AsyncGenerator:
        ...
        async for record in crud.record.query(task_id=task_id, is_submit=True):
            index += 1
            do_record = schemas.record.DoRecord.model_validate(record, from_attributes=True)
...
async def export_workload(
    ...
    async for record in crud.record.query(task_id=task_id, is_submit=True):
        ...
        task_user_data_duration[key] += (record.submit_time - record.create_time) if record.submit_time else 0
...
async def preview_data_ids(
    ...
    if req.kind == schemas.operator.stats.ANSWER_FLITER_KIND.WITHOUT_DUPLICATE:
        records = await crud.data.query(
            ...
            invalid=(True if req.record_status == schemas.record.RecordFullStatus.INVALID else None),
        ).to_list()
    else:
...
async def preview_data(
    ...
    label_user = await crud.user.query(user_id=record.creator_id).first_or_none() if record else None
    ...
    resp = schemas.operator.task.RespPreviewData(
        ...
        evaluation=schemas.evaluation.SingleEvaluation.model_validate(data.evaluation, from_attributes=True),
        reference_evaluation=data.reference_evaluation,
        label_user=(
        ...
        .aggregate(
            [
                ...
                (
                    {
                        "$sort": {
                            "discarded_data_count": (1 if req.sort == "discarded_asc" else -1),
                        }
                    }
                    if req.sort
                    else {"$sort": {"_id": 1}}
        ...
        list=[
            schemas.operator.task.GroupDataByUser(
                label_user=schemas.user.DoUserWithUsername(
                    ...
                    username=(user_name_map[item.user_id] if item.user_id in user_name_map else ""),
                ),
