# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\user.py
...
async def create_user(req: schemas.user.UserLoginRequest, response: Response):
    ...
    default_team_users = (team.users or []) + [
        schemas.team.TeamMember(user_id=user.user_id, name=user.name, role=team_role)
    ]
...
async def get_me(user: User = Depends(deps.get_current_user)):
    ...
    return schemas.user.RespMe(user_id=user.user_id, teams=team_roles, role=user.role, name=user.name)
...
async def list_users(
    ...
    query = crud.user.query(skip=skip, limit=limit, name=name, role=roles, sort="-update_time")
    ...
    return schemas.user.ListUserTeamInfoResp(
        list=[
            schemas.user.UserTeamInfo(
                ...
                role=(
                    [v.role for v in team.users if v.user_id == user.user_id and v.role is not None]
