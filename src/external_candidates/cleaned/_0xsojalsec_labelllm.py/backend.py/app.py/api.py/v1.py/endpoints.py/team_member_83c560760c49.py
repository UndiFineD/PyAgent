# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\team_member.py
...
async def list_members(
    ...
    query = crud.user.query(skip=skip, limit=limit, name=user_name, user_id=user_ids)
...
async def create_invidation_link(
    team_id: str = Body("team_id", description="团队 id", example="test-team-id", embed=True),
    ...
    if (
        ...
        and me_role == schemas.team.TeamMemberRole.USER
    ):
...
async def remove_member(
    ...
    if (
        (user.role == schemas.user.UserType.ADMIN or user.role == schemas.user.UserType.SUPER_ADMIN)
        or (
            me_role == schemas.team.TeamMemberRole.SUPER_ADMIN
            and kicked_user_role != schemas.team.TeamMemberRole.SUPER_ADMIN
        )
        or (me_role == schemas.team.TeamMemberRole.ADMIN and kicked_user_role == schemas.team.TeamMemberRole.USER)
    ):
        pass
    else:
        raise exceptions.USER_PERMISSION_DENIED
    ...
    if team.users:
        for t_user in team.users:
            if user_id == t_user.user_id:
                if t_user.role == schemas.user.UserType.SUPER_ADMIN:
...
async def edit_team_member(
    ...
    if (user.role == schemas.user.UserType.ADMIN or user.role == schemas.user.UserType.SUPER_ADMIN) or (
        me_role == schemas.team.TeamMemberRole.SUPER_ADMIN
        and user_info.role != schemas.team.TeamMemberRole.SUPER_ADMIN
        and edited_user_role != schemas.team.TeamMemberRole.SUPER_ADMIN
    ):
        pass
    else:
        raise exceptions.USER_PERMISSION_DENIED
