# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\team.py
...
async def create_team(
    ...
    # 检查用户是否有权限创建任务
    if user.role != schemas.user.UserType.ADMIN and user.role != schemas.user.UserType.SUPER_ADMIN:
...
async def delete_team(
    ...
    # 将当前团队的用户加入到默认团队
    default_team = await crud.team.query(team_id=schemas.team.DEFAULT_TEAM_ID).first_or_none()
    ...
    for t_user in team.users:
        ...
        default_users.append(
            schemas.team.TeamMember(user_id=t_user.user_id, name="", role=schemas.team.TeamMemberRole.USER)
        )
