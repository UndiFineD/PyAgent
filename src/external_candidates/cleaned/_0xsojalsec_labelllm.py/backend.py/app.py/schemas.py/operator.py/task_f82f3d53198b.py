# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\schemas\operator\task.py
from enum import StrEnum
from typing import Literal
from uuid import UUID

from app.schemas.data import DataFormat, DataRange, DataStatus, DoDataForUser
from app.schemas.operator.stats import ANSWER_FLITER_KIND
from app.schemas.record import RecordFullStatus, RecordStatus
...
class ReqPreviewDataID(DoTaskBase):
    data_id: UUID | None = Field(description="数据id", default=None)
    kind: ANSWER_FLITER_KIND = Field(description="是否是源题模式", default=ANSWER_FLITER_KIND.WITHOUT_DUPLICATE)
    record_status: RecordFullStatus | None = Field(description="状态", default=None)
    pos_locate: RecordPosLocateKind = Field(description="当前问卷或者是下一份问卷", default=RecordPosLocateKind.CURRENT)
...
class ReqGroupRecordByUser(DoTaskBase):
    ...
    page: int = Field(description="跳过数量", default=1, gt=0)
    page_size: int = Field(description="获取数量", default=10, gt=0)
    sort: Literal["discarded_asc", "discarded_desc"] | None = Field(description="排序字段", default=None)
