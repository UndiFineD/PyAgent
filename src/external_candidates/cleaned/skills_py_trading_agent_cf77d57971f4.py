# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\zbruceli.py\unifai_trading_suite.py\src.py\agents.py\trading_agent_cf77d57971f4.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\zbruceli\unifai-trading-suite\src\agents\trading_agent.py
...
class TradingAgent:
    ...
    async def _call_polymarket_tool(self, tool_name: str, params: dict) -> str:
        ...
        try:
            ...
            messages = [
                {
                    "role": "user",
                    "content": f"Call {tool_name} with params: {json.dumps(params)}",
                },
            ]
    ...
    async def get_polymarket_trending(self) -> str:
        ...
        try:
            result = await self._call_polymarket_tool("polymarket--127--getEventsByCategory", {"category": "trending"})
...
**Alternative: Use Kalshi** for regulated prediction markets."""
    ...
    async def get_polymarket_crypto(self) -> str:
        ...
        try:
            result = await self._call_polymarket_tool("polymarket--127--getEventsByCategory", {"category": "crypto"})
...
**Alternative: Use Kalshi** for regulated prediction markets."""
    ...
    async def search_polymarket(self, query: str) -> str:
        ...
        try:
            result = await self._call_polymarket_tool("polymarket--127--search", {"q": query})
...
async def run_demo():
    ...
    try:
        ...
        try:
            response = await agent.chat("Get the top 5 trending events on Polymarket with their prices and volumes")
        ...
        try:
            ...
            response = await agent.chat("Get the top 3 crypto prediction markets on Polymarket")
