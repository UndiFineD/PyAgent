# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\logging_config.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def configure_logging(verbose: int, log: int, log_file: str = "./logs/%Y%m%d_%H%M.log"):
    ...
    custom_logger_config = {
        ...
        "formatters": {"customFormatter": {"format": "%(asctime)s - %(name)s - %(levelname)s - %(message)s"}},
        "handlers": {
            "fileHandler": {
        ...
        "loggers": {"": {"handlers": ["fileHandler"], "level": log_level, "propagate": False}},
    }
