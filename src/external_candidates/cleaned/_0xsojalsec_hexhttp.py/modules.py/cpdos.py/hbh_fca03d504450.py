# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\cpdos\hbh.py
#!/usr/bin/python3
# -*- coding: utf-8 -*-
...
from modules.utils import configure_logger, generate_cache_buster, human_time, requests
...
def cache_poisoning(url, s, parameters, response_1, response_2, authentication, headers):
    ...
    if (
        ...
        reason = f"DIFFERENT STATUS-CODE {response_1.status_code} > {response_3.status_code}"
    if (
        response_3
...
def HBH(
    ...
    for header in header_list:
        ...
        try:
            ...
            if len(response_2.content) != response_2_previous_size and len(response_2.content) != 0:
            ...
            if behavior:
                ...
                cache_poisoning(url, s, parameters, response_1, response_2, authent, headers)
            human_time(human)
