# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\cpdos\hhcn.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
from modules.utils import configure_logger, get_domain_from_url, random, requests
...
def HHCN(url, s, main_response, authent, content_delta_range=CONTENT_DELTA_RANGE):
    ...
    try:
        ...
        behavior = ""
        if not (main_response_size - content_delta_range < probe_size < main_response_size + content_delta_range) or (
            main_response.status_code != probe.status_code
        ):
            ...
            if not (main_response_size - content_delta_range < probe_size < main_response_size + content_delta_range):
                behavior = f"DIFFERENT RESPONSE LENGTH | {main_response_size}b > {probe_size}b"
                print(
            ...
            if main_response.status_code != probe.status_code:
                behavior = f"DIFFERENT STATUS-CODE | {main_response_size}b > {probe_size}b"
                print(
                    f" \033[33m└── [INTERESTING BEHAVIOR]\033[0m | HHCN | \033[34m{url}\033[0m | {behavior} | {payload}"
                )
