# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\cpdos\hho.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
from modules.utils import configure_logger, human_time, requests
...
def HHO(url, s, main_response, authent, human):
    ...
    while iteration < max_iterations and not error_detected:
        ...
        try:
            probe = s.get(url, headers=h, auth=authent, allow_redirects=False, timeout=10)
            ...
            if probe.status_code in [400, 413, 500, 502] and probe.status_code != main_status_code:
                logger.debug(
    ...
    if error_detected:
        try:
            ...
            if verify.status_code in [400, 413, 500, 502] and verify.status_code != main_status_code:
            ...
            print(
                f" {status} | HHO DOS | \033[34m{url}\033[0m | {reason} | PAYLOAD: X-Oversized-Header-x: Big-Value-0*{len(big_value) - len('Big-Value-0')}"
            )
