# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\cpdos\hmo.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
from modules.utils import configure_logger, human_time, random, requests
...
def HMO(url, s, initial_response, authent, human):
    ...
    for header, method in ((header, method) for header in hmo_headers for method in methods):
        ...
        try:
            ...
            elif len(probe.content) != main_len and len(probe.content) not in range(
                main_len - CONTENT_DELTA_RANGE, main_len + CONTENT_DELTA_RANGE
            ):
                reason = f"DIFFERENT RESPONSE LENGTH {main_len}b > {len(probe.content)}b"
                # print(probe.content)
                status = "\033[33m└── [INTERESTING BEHAVIOR]\033[0m"
            ...
            if control.status_code == probe.status_code and control.status_code not in [main_status_code, 429, 403]:
                reason = f"DIFFERENT STATUS-CODE {main_status_code} > {control.status_code}"
                status = "\033[31m└── [VULNERABILITY CONFIRMED]\033[0m"

            if len(control.content) == len(probe.content) and len(probe.content) not in range(
                main_len - CONTENT_DELTA_RANGE, main_len + CONTENT_DELTA_RANGE
            ):
                reason = f"DIFFERENT RESPONSE LENGTH {main_len}b > {len(control.content)}b"
                # print(control.content)
                status = "\033[31m└── [VULNERABILITY CONFIRMED]\033[0m"

            if reason:
                print(f" {status} | HMO DOS | \033[34m{uri}\033[0m | {reason} | PAYLOAD: {probe_headers}")
