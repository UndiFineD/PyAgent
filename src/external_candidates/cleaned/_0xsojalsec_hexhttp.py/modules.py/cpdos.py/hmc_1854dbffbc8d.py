# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\cpdos\hmc.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def check_meta_character(url, s, main_status_code, authent, meta_character, human):
    ...
    if probe.status_code in [400, 413, 500] and probe.status_code != main_status_code:
        ...
        if control.status_code == probe.status_code and control.status_code != main_status_code:
            reason = f"\033[34m{main_status_code} > {control.status_code}\033[0m"

    if reason:
        payload = f"PAYLOAD: {headers}"
        print(f"\033[31m └── [VULNERABILITY CONFIRMED]\033[0m | HMC | {url} | {reason} | {payload}")
    human_time(human)
