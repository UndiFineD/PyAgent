# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\didelco.py\pvpc_spain.py\scripts.py\find_cheap_ranges_827d460213a5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\didelco\pvpc-spain\scripts\find_cheap_ranges.py
...
def group_consecutive_hours(hours, tolerance=0.01):
    ...
    for i in range(1, len(sorted_hours)):
        ...
        if curr["hour"] == prev["hour"] + 1 and abs(curr["price"] - prev["price"]) <= tolerance:
