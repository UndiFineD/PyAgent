# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\exchanges_tests\abstract_authenticated_future_exchange_tester.py
...
class AbstractAuthenticatedFutureExchangeTester(
    ...
    async def inner_test_get_and_set_margin_type(
        self, allow_empty_position=False, symbol=None, has_open_position=False
    ):
        ...
        new_margin_type = (
            trading_enums.MarginType.CROSS
            if origin_margin_type is trading_enums.MarginType.ISOLATED
            else trading_enums.MarginType.ISOLATED
        )
        if not self.exchange_manager.exchange.SUPPORTS_SET_MARGIN_TYPE:
            assert origin_margin_type in (
                trading_enums.MarginType.ISOLATED,
                trading_enums.MarginType.CROSS,
            )
            with pytest.raises(trading_errors.NotSupported):
    ...
    async def set_margin_type(self, margin_type, symbol=None):
        await self.exchange_manager.exchange.set_symbol_margin_type(
            symbol or self.SYMBOL, margin_type is trading_enums.MarginType.ISOLATED
        )
    ...
    async def inner_test_create_and_cancel_limit_orders(self, symbol=None, settlement_currency=None):
        if self.exchange_manager.exchange.SUPPORTS_SET_MARGIN_TYPE:
            ...
            await self._inner_test_create_and_cancel_limit_orders_for_margin_type(
                symbol=symbol,
                settlement_currency=settlement_currency,
                margin_type=trading_enums.MarginType.CROSS,
            )
        else:
            await self._inner_test_create_and_cancel_limit_orders_for_margin_type(
    ...
    async def _inner_test_create_and_cancel_limit_orders_for_margin_type(
        ...
        await super().inner_test_create_and_cancel_limit_orders(
            ...
            settlement_currency=self.ORDER_CURRENCY,
    ...
    async def create_market_stop_loss_order(
        ...
        await self.enable_partial_take_profits_and_stop_loss(
            trading_enums.TakeProfitStopLossMode.PARTIAL, symbol=symbol
        )
    ...
    def check_position_size(self, previous_position, updated_position):
        assert (
            previous_position[trading_enums.ExchangeConstantsPositionColumns.SIZE.value]
    ...
    def get_other_position_symbol(self, positions_blacklist, contract_type):
        ignored_symbols = set(
            position[trading_enums.ExchangeConstantsPositionColumns.SYMBOL.value] for position in positions_blacklist
        )
        for symbol in self.exchange_manager.exchange.connector.client.markets:
            if symbol in ignored_symbols or self.exchange_manager.exchange.is_expirable_symbol(symbol):
                continue
            if (
                contract_type is trading_enums.FutureContractType.INVERSE_PERPETUAL
                and self.exchange_manager.exchange.is_inverse_symbol(symbol)
            ):
                return symbol
            elif (
                contract_type is trading_enums.FutureContractType.LINEAR_PERPETUAL
    ...
    def is_position_empty(self, position):
        if position is None:
            raise AssertionError(f"Fetched empty position should never be None as a symbol parameter is given")
    ...
    def _get_all_symbols(self):
        return [symbol for symbol in (self.SYMBOL, self.INVERSE_SYMBOL) if symbol]
