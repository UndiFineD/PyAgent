...
class AbstractAuthenticatedExchangeTester:
    ...
    CONVERTS_MARKET_INTO_LIMIT_ORDERS = (
        False  # when market orders are always converted into limit order by the exchange
    )
    ...
    async def inner_test_untradable_symbols(self):
        ...
        async with self.local_exchange_manager():
            ...
            disabled = [symbol for symbol in all_symbols_including_disabled if symbol not in all_symbols]
            ...
            for untradable_symbol in untradable:
                ...
                to_test_symbols = [tradable_symbols[i % (len(tradable_symbols) - 1)] for i in tradable_indexes] + [
    ...
    async def test_is_authenticated_request(self):
        ...
        def _http_proxy_callback_factory(_exchange_manager):
            ...
            def proxy_callback(url: str, method: str, headers: dict, body) -> typing.Optional[str]:
                ...
                rest_exchange_data["calls"].append(((url, method, headers, body), exchange_return_value))
    ...
    async def inner_is_authenticated_request(self, rest_exchange_data):
        if self.IS_AUTHENTICATED_REQUEST_CHECK_AVAILABLE:
            ...
            def assert_has_at_least_one_authenticated_call(calls):
                ...
                assert has_authenticated_call, (
                    f"{calls} should contain at last 1 authenticated call"
    ...
    async def check_require_order_fees_from_trades(self, filled_exchange_order_id, symbol=None):
        ...
        order_maybe_without_fees = await self.exchange_manager.exchange.connector.get_order(
    ...
    async def inner_test_create_and_cancel_stop_orders(self):
        ...
        assert self.exchange_manager.exchange.is_supported_order_type(trading_enums.TraderOrderType.STOP_LOSS) is True
    ...
    def check_raw_cancelled_orders(self, cancelled_orders):
        ...
        for cancelled_order in cancelled_orders:
            assert (
                cancelled_order[trading_enums.ExchangeConstantsOrderColumns.STATUS.value]
    ...
    def check_parsed_closed_order(
        ...
        assert order.status
        if order.status == trading_enums.OrderStatus.CANCELED and self.EXPECT_MISSING_FEE_IN_CANCELLED_ORDERS:
            ...
            try:
                ...
                if (
                    ...
                    and order.taker_or_maker == trading_enums.ExchangeConstantsOrderColumns.MAKER.value
                ):
    ...
    def check_raw_trades(self, trades):
        ...
        for trade in trades:
            self.check_parsed_trade(personal_data.create_trade_instance_from_raw(self.exchange_manager.trader, trade))
    ...
    def check_parsed_trade(self, trade: personal_data.Trade):
        ...
        if trade.status is not trading_enums.OrderStatus.CANCELED:
            ...
            if "USD" in trade.market:
                assert (
                    self.MIN_TRADE_USD_VALUE * decimal.Decimal("0.9") < trade.total_cost < self.MAX_TRADE_USD_VALUE
                ), (
    ...
    async def get_order(self, exchange_order_id, symbol=None):
        assert (
            self.exchange_manager.exchange.connector.client.has["fetchOrder"] is self.EXPECT_FETCH_ORDER_TO_BE_AVAILABLE
        )
    ...
    async def create_limit_order(self, price, size, side, symbol=None, push_on_exchange=True):
        order_type = (
    ...
    async def create_market_order(self, current_price, size, side, symbol=None, push_on_exchange=True):
        order_type = (
    ...
    def check_order_size_and_price(self, size, price, symbol=None, allow_empty_size=False):
        ...
        precision_amount = market_status[trading_enums.ExchangeConstantsMarketStatusColumns.PRECISION.value].get(
        ...
        precision_price = market_status[trading_enums.ExchangeConstantsMarketStatusColumns.PRECISION.value].get(
        ...
        adapted_details = personal_data.decimal_check_and_adapt_order_details_if_necessary(size, price, market_status)
        if size == trading_constants.ZERO:
            if allow_empty_size:
    ...
    def check_created_limit_order(self, order, price, size, side):
        ...
        expected_type = (
            personal_data.BuyLimitOrder if side is trading_enums.TradeOrderSide.BUY else personal_data.SellLimitOrder
        )
    ...
    def _check_order(self, order, size, side):
        if self.CONVERTS_ORDER_SIZE_BEFORE_PUSHING_TO_EXCHANGES:
            ...
            assert round(order.origin_quantity, 8) == round(size, 8), f"FALSE: {order.origin_quantity} == {size}"
    ...
    async def wait_for_order_exchange_id_in_trades(self, order_exchange_id):
        ...
        def check_order_exchange_id_in_trades(trades):
            trades_exchange_ids = [
                trade[trading_enums.ExchangeConstantsOrderColumns.EXCHANGE_ID.value] for trade in trades
            ]
    ...
    async def wait_for_edit(self, order, edited_quantity):
        def is_edited(row_order):
            return round(
                decimal.Decimal(str(row_order[trading_enums.ExchangeConstantsOrderColumns.AMOUNT.value])),
                8,
    ...
    def get_config(self):
        return {
            constants.CONFIG_EXCHANGES: {self.EXCHANGE_NAME: {constants.CONFIG_EXCHANGE_TYPE: self.EXCHANGE_TYPE}},
    ...
    def _get_market_filter(self):
        def market_filter(market):
            return market[trading_enums.ExchangeConstantsMarketStatusColumns.SYMBOL.value] in (self.SYMBOL,)
