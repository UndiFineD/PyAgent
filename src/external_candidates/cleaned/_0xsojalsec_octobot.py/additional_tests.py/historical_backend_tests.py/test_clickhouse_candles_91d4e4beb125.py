# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\historical_backend_tests\test_clickhouse_candles.py
...
async def test_fetch_candles_history(clickhouse_client):
    ...
    end_time = 1721377495
    candles_count = math.floor(
        (end_time - start_time) / (commons_enums.TimeFramesMinutes[commons_enums.TimeFrames.FIFTEEN_MINUTES] * 60)
    )
