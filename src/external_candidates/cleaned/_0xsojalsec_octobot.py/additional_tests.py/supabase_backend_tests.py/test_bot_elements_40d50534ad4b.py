# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\supabase_backend_tests\test_bot_elements.py
...
import postgrest
import pytest
from additional_tests.supabase_backend_tests import (
    admin_client,
...
async def test_create_update_and_fetch_bot(authenticated_client_1, authenticated_client_2):
    ...
    try:
        ...
        assert all(attribute.value in bot for attribute in supabase_backend_enums.BotKeys)
        ...
        if created_bot_id:
            assert (await delete_bot(authenticated_client_1, created_bot_id))[
                supabase_backend_enums.BotKeys.ID.value
            ] == created_bot_id
...
async def test_fetch_deployment_and_deployment_data(authenticated_client_1, authenticated_client_2, admin_client):
    ...
    try:
        ...
        assert all(attribute.value in deployment for attribute in supabase_backend_enums.BotDeploymentKeys)
        ...
        async def _create_deployment_url(client):
            ...
            await client.update_deployment(
                deployment_id,
                {
                    supabase_backend_enums.BotDeploymentKeys.CURRENT_URL_ID.value: created_deployment_url[
        ...
        try:
            ...
            assert all(attribute.value in deployment_url for attribute in supabase_backend_enums.BotDeploymentURLKeys)
            ...
            async def _fetch_deployment(client, to_fetch_deployment_id):
                return (
                    await client.table("bot_deployments")
                    .select("*")
                    .eq(
                        supabase_backend_enums.BotDeploymentKeys.ID.value,
                        to_fetch_deployment_id,
                    )
                    .execute()
                ).data[0]
            ...
            async def _delete_deployment_url(client):
                ...
                res = (
                    await client.table("bot_deployment_urls")
                    .delete()
                    .eq(
                        supabase_backend_enums.BotDeploymentURLKeys.ID.value,
                        deployment_url_id,
                ...
                assert res.data == []
                res2 = (
                    await admin_client.table("bot_deployment_urls")
                    .delete()
                    .eq(
                        supabase_backend_enums.BotDeploymentURLKeys.ID.value,
                        deployment_url_id,
...
async def test_fetch_startup_info(authenticated_client_1_with_temp_bot):
    ...
    assert all(isinstance(val, str) for val in parsed_info.subscribed_products_urls)
...
async def test_upsert_and_reset_trades(authenticated_client_1_with_temp_bot, authenticated_client_2):  # todo
    ...
    assert len(set([trade[supabase_backend_enums.TradeKeys.TRADE_ID.value] for trade in upserted_trades])) == len(
        updated_trades
    )
    assert all(attribute.value in trade for trade in upserted_trades for attribute in supabase_backend_enums.TradeKeys)
...
async def test_switch_and_update_portfolio(authenticated_client_1_with_temp_bot, authenticated_client_2):
    ...
    assert (
        updated_bot[supabase_backend_enums.BotKeys.CURRENT_PORTFOLIO_ID.value]
    ...
    assert all(attribute.value in created_portfolio for attribute in supabase_backend_enums.PortfolioKeys)
    ...
    portfolio_2[supabase_backend_enums.PortfolioKeys.ID.value] = created_portfolios[1][
        supabase_backend_enums.PortfolioKeys.ID.value
    ]
...
def _equal(d_1, d_2, key):
    if isinstance(d_1[key], dict):
        return all(_equal(d_1[key], d_2[key], sub_key) for sub_key in d_1[key])
    if isinstance(d_1[key], list):
        return all(_equal(d_1[key], d_2[key], sub_key) for sub_key in range(len(d_1[key])))
    if key == "time":
    ...
    trades = [
        {
            ...
            octobot_trading.enums.ExchangeConstantsOrderColumns.TYPE.value: (
                octobot_trading.enums.TraderOrderType.SELL_LIMIT.value
                if int(seed + i) % 2 == 0
