# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\supabase_backend_tests\__init__.py
...
async def delete_bot(client, bot_id):
    async def _delete_portfolio_histories(portfolio_id):
        await (
            client.table("bot_portfolio_histories")
            .delete()
            .eq(supabase_backend_enums.PortfolioHistoryKeys.PORTFOLIO_ID.value, portfolio_id)
            .execute()
        )

    async def _delete_portfolio(portfolio_id):
        await (
            client.table("bot_portfolios")
            .delete()
            .eq(supabase_backend_enums.PortfolioKeys.ID.value, portfolio_id)
            .execute()
        )

    async def _delete_config(config_id):
        await (
            client.table("bot_configs").delete().eq(supabase_backend_enums.BotConfigKeys.ID.value, config_id).execute()
        )

    async def _delete_deployment(deployment_id):
        await (
            client.table("bot_deployments")
            .delete()
            .eq(supabase_backend_enums.BotDeploymentKeys.ID.value, deployment_id)
            .execute()
        )
    ...
    if deployment_id := bot[supabase_backend_enums.BotKeys.CURRENT_DEPLOYMENT_ID.value]:
        ...
        await client.update_bot(bot_id, {supabase_backend_enums.BotKeys.CURRENT_DEPLOYMENT_ID.value: None})
    ...
    if portfolios:
        ...
        await client.update_bot(bot_id, {supabase_backend_enums.BotKeys.CURRENT_PORTFOLIO_ID.value: None})
    ...
    if configs:
        ...
        await client.update_bot(bot_id, {supabase_backend_enums.BotKeys.CURRENT_CONFIG_ID.value: None})
    ...
    async with _authenticated_client(None, None, _get_backend_service_key()) as admin_client:
        ...
        try:
            ...
            for table, rows in to_delete.items():
                await admin_client.table(table).delete().in_("id", [row["id"] for row in rows]).execute()
...
def _load_backend_creds_env_variables_if_necessary():
    ...
    if not LOADED_BACKEND_CREDS_ENV_VARIABLES:
        ...
        dotenv_path = os.getenv(
            "SUPABASE_BACKEND_TESTS_DOTENV_PATH",
...
def get_backend_client_creds(identifier):
    return os.getenv(f"SUPABASE_BACKEND_CLIENT_{identifier}_EMAIL"), os.getenv(
        f"SUPABASE_BACKEND_CLIENT_{identifier}_PASSWORD"
    )
