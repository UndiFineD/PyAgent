# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\logger.py
...
def _load_logger_config():
    try:
        ...
        if not os.path.isfile(configuration_manager.get_user_local_config_file()):
            ...
            shutil.copyfile(
                constants.LOGGING_CONFIG_FILE,
                configuration_manager.get_user_local_config_file(),
            )
...
async def init_exchange_chan_logger(exchange_id):
    await exchanges_channel.get_chan(
        channels_name.OctoBotTradingChannelsName.OHLCV_CHANNEL.value, exchange_id
    ).new_consumer(ohlcv_callback, priority_level=LOGGER_PRIORITY_LEVEL)
    await exchanges_channel.get_chan(
    ...
    await exchanges_channel.get_chan(
        channels_name.OctoBotTradingChannelsName.TRADES_CHANNEL.value, exchange_id
    ).new_consumer(
        trades_callback,
        priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value,
    )
    await exchanges_channel.get_chan(
        channels_name.OctoBotTradingChannelsName.LIQUIDATIONS_CHANNEL.value, exchange_id
    ).new_consumer(
        liquidations_callback,
        priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value,
    )
    await exchanges_channel.get_chan(
        channels_name.OctoBotTradingChannelsName.POSITIONS_CHANNEL.value, exchange_id
    ).new_consumer(
        positions_callback,
        priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value,
    )
    await exchanges_channel.get_chan(
        channels_name.OctoBotTradingChannelsName.ORDERS_CHANNEL.value, exchange_id
    ).new_consumer(
        orders_callback,
        priority_level=channel_enums.ChannelConsumerPriorityLevels.MEDIUM.value,
    )
    ...
    if constants.ENV_TRADING_ENABLE_DEBUG_LOGS:
        await exchanges_channel.get_chan(
            ...
            exchange_id,
        ).new_consumer(recent_trades_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.FUNDING_CHANNEL.value, exchange_id
        ).new_consumer(funding_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.TICKER_CHANNEL.value, exchange_id
        ).new_consumer(ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.MINI_TICKER_CHANNEL.value,
            exchange_id,
        ).new_consumer(mini_ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.ORDER_BOOK_CHANNEL.value,
            exchange_id,
        ).new_consumer(order_book_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.ORDER_BOOK_TICKER_CHANNEL.value,
            exchange_id,
        ).new_consumer(order_book_ticker_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.KLINE_CHANNEL.value, exchange_id
        ).new_consumer(kline_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
            channels_name.OctoBotTradingChannelsName.MARK_PRICE_CHANNEL.value,
            exchange_id,
        ).new_consumer(mark_price_callback, priority_level=LOGGER_PRIORITY_LEVEL)
        await exchanges_channel.get_chan(
...
async def init_evaluator_chan_logger(matrix_id: str):
    await evaluator_channels.get_chan(
        channels_name.OctoBotEvaluatorsChannelsName.MATRIX_CHANNEL.value, matrix_id
    ).new_consumer(matrix_callback, priority_level=LOGGER_PRIORITY_LEVEL)
    await evaluator_channels.get_chan(
        channels_name.OctoBotEvaluatorsChannelsName.EVALUATORS_CHANNEL.value, matrix_id
    ).new_consumer(evaluators_callback, priority_level=LOGGER_PRIORITY_LEVEL)
...
async def init_octobot_chan_logger(bot_id: str):
    await channel_instances.get_chan_at_id(constants.OCTOBOT_CHANNEL, bot_id).new_consumer(
        ...
        bot_id=bot_id,
        subject=[
            enums.OctoBotChannelSubjects.NOTIFICATION.value,
            enums.OctoBotChannelSubjects.ERROR.value,
        ],
    )
...
async def ticker_callback(exchange: str, exchange_id: str, cryptocurrency: str, symbol: str, ticker):
    BOT_CHANNEL_LOGGER.debug(
        f"TICKER : EXCHANGE = {exchange} || CRYPTOCURRENCY = {cryptocurrency} || SYMBOL = {symbol} || TICKER = {ticker}"
    )


async def mini_ticker_callback(exchange: str, exchange_id: str, cryptocurrency: str, symbol: str, mini_ticker):
    BOT_CHANNEL_LOGGER.debug(
...
async def liquidations_callback(exchange: str, exchange_id: str, cryptocurrency: str, symbol: str, liquidations):
    BOT_CHANNEL_LOGGER.debug(
...
async def mark_price_callback(exchange: str, exchange_id: str, cryptocurrency: str, symbol: str, mark_price):
    BOT_CHANNEL_LOGGER.debug(
...
def _filter_balance(balance: dict):
    ...
    if isinstance(first_value, dict):
        filtered_balance = {
            key: values for key, values in balance.items() if values.get(commons_constants.PORTFOLIO_TOTAL)
        }
        removed_count = len(balance) - len(filtered_balance)
        return (
            trading_api.parse_decimal_portfolio(filtered_balance, False),
            removed_count,
        )
    return balance, 0
...
async def orders_callback(
    ...
    order_string = (
        f"ORDERS : EXCHANGE = {exchange} || SYMBOL = {symbol} || "
...
async def octobot_channel_callback(bot_id: str, subject: str, action: str, data: dict):
    BOT_CHANNEL_LOGGER.debug(f"OCTOBOT_CHANNEL : SUBJECT = {subject} || ACTION = {action} || DATA = {data} ")
