# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\storage\trading_metadata.py
...
async def store_run_metadata(bot_id, exchange_managers, start_time, user_inputs=None, flush=False) -> dict:
    ...
    await run_db.log(common_enums.DBTables.METADATA.value, metadata)
    if flush:
        await run_db.flush()
    return metadata
...
async def store_backtesting_run_metadata(exchange_managers, start_time, user_inputs, run_dbs_identifier, name) -> dict:
    run_metadata = await _get_trading_metadata(
        exchange_managers, start_time, user_inputs, run_dbs_identifier, True, name
    )
    # use local database as a lock is required
    async with commons_databases.DBWriter.database(
        run_dbs_identifier.get_backtesting_metadata_identifier(), with_lock=True
    ) as writer:
        await writer.log(common_enums.DBTables.METADATA.value, run_metadata)
    return run_metadata


async def _get_trading_metadata(
    exchange_managers,
    run_start_time,
    user_inputs,
    run_dbs_identifier,
...
async def _get_multi_exchange_data(exchange_managers, is_backtesting):
    ...
    profitability = numpy.average(
        tuple(float(trading_api.get_profitability_stats(exchange_manager)[0]) for exchange_manager in exchange_managers)
    )
    profitability_percent = numpy.average(
        tuple(float(trading_api.get_profitability_stats(exchange_manager)[1]) for exchange_manager in exchange_managers)
    )
    markets_profitability = _get_markets_profitability(exchange_managers, time_frames) if is_backtesting else {}
    exchange_names = [trading_api.get_exchange_name(exchange_manager) for exchange_manager in exchange_managers]
    ...
    win_rate = round(
        numpy.average(
            tuple(float(trading_api.get_win_rate(exchange_manager) * 100) for exchange_manager in exchange_managers)
        ),
        3,
    )
    ...
    draw_down = round(
        numpy.average(
            tuple(float(trading_api.get_draw_down(exchange_manager)) for exchange_manager in exchange_managers)
        ),
        3,
    )
    ...
    for exchange_manager in exchange_managers:
        for symbol in trading_api.get_trading_pairs(exchange_manager):
            if symbol is markets_profitability:
                continue
            try:
                markets_profitability[symbol] = backtesting.IndependentBacktesting.get_market_delta(
                    symbol, exchange_manager, min_tf
                )
            except Exception:
                pass
...
def _get_portfolio(exchange_managers):
    ...
    for exchange_manager in exchange_managers:
        ...
        for exchange_portfolio, portfolio in zip(
            (exchange_origin_portfolio, exchange_end_portfolio),
...
def _get_future_contracts_by_exchange(exchange_managers):
    return {
        trading_api.get_exchange_name(exchange_manager): {
            ...
            if symbol in trading_api.get_trading_pairs(exchange_manager) and trading_api.is_handled_contract(contract)
        }
        for exchange_manager in exchange_managers
...
async def _get_single_exchange_data(
    exchange_manager,
    trading_mode,
    run_start_time,
    user_inputs,
