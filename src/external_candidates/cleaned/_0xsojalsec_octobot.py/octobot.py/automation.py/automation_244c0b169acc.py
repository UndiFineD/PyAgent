...
class Automation(tentacles_management.AbstractTentacle):
    ...
    async def get_raw_config_and_user_inputs(cls, config, tentacles_setup_config, bot_id):
        ...
        local_instance = cls.create_local_instance(config, tentacles_setup_config, tentacle_config)
        user_inputs = {}
    ...
    async def restart(self):
        ...
        await self.stop()
        self.automations_config = tentacles_manager_api.get_tentacle_config(self.tentacles_setup_config, self.__class__)
    ...
    def get_all_steps(self):
        all_events = {
            event.get_name(): event for event in self._all_possible_steps(abstract_trigger_event.AbstractTriggerEvent)
        }
        all_conditions = {
            condition.get_name(): condition
            for condition in self._all_possible_steps(abstract_condition.AbstractCondition)
        }
        all_actions = {action.get_name(): action for action in self._all_possible_steps(abstract_action.AbstractAction)}
    ...
    def _create_automation_details(self):
        ...
        for automation_id, automation_config in self.automations_config.get(self.AUTOMATIONS, {}).items():
            ...
            actions = [
                self._create_step(automation_config, action, all_actions) for action in automation_config[self.ACTIONS]
            ]
    ...
    async def _check_conditions(self, automation_detail):
        for condition in automation_detail.conditions:
            if not await condition.evaluate():
                ...
                self.logger.debug(
                    f"{condition.get_name()} is not valid: skipping {automation_detail.trigger_event.get_name()} event"
                )
                return False
            self.logger.debug(
                f"All conditions are valid for {automation_detail.trigger_event.get_name()} event trigger"
            )
        return True
    ...
    async def _process_actions(self, automation_detail):
        for action in automation_detail.actions:
            try:
                self.logger.debug(
                    f"Running {action.get_name()} after {automation_detail.trigger_event.get_name()} event"
                )
