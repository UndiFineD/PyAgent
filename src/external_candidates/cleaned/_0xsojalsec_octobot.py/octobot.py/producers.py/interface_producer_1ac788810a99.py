# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\producers\interface_producer.py
...
class InterfaceProducer(octobot_channels.OctoBotChannelProducer):
    ...
    async def start_interfaces(self):
        ...
        if len(started_interfaces) != len(to_start_interfaces):
            missing_interfaces = [
                interface.get_name() for interface in to_start_interfaces if interface not in started_interfaces
            ]
            self.logger.error(
    ...
    async def _create_interfaces(self, in_backtesting):
        ...
        if service_interfaces.get_bot_api() is None:
            service_api.initialize_global_project_data(
                self.octobot.octobot_api, constants.PROJECT_NAME, constants.LONG_VERSION
            )
    ...
    async def _create_interface_if_relevant(
        ...
        if self._is_interface_relevant(interface_class, backtesting_enabled):
            await self.send(
                ...
                data={
                    service_channel_consumer.OctoBotChannelServiceDataKeys.EDITED_CONFIG.value: edited_config,
                    service_channel_consumer.OctoBotChannelServiceDataKeys.BACKTESTING_ENABLED.value: backtesting_enabled,
    ...
    async def _create_notifier_class_if_relevant(
        self, notifier_factory, notifier_class, backtesting_enabled, edited_config
    ):
        if self._is_notifier_relevant(notifier_class, backtesting_enabled):
            await self.send(
                ...
                data={
                    ...
                    service_channel_consumer.OctoBotChannelServiceDataKeys.EXECUTORS.value: self.octobot.task_manager.executors,
                },
            )
    ...
    def _is_interface_relevant(self, interface_class, backtesting_enabled):
        return (
            service_api.is_enabled(interface_class)
            and tentacles_manager_api.is_tentacle_activated_in_tentacles_setup_config(
                self.octobot.tentacles_setup_config, interface_class.get_name()
            )
            and all(service.get_is_enabled(self.octobot.config) for service in interface_class.REQUIRED_SERVICES)
            and (
                not backtesting_enabled
                or (backtesting_enabled and service_api.is_enabled_in_backtesting(interface_class))
            )
        )

    def _is_notifier_relevant(self, notifier_class, backtesting_enabled):
        return (
            service_api.is_enabled_in_config(notifier_class, self.octobot.config)
            and tentacles_manager_api.is_tentacle_activated_in_tentacles_setup_config(
                self.octobot.tentacles_setup_config, notifier_class.get_name()
            )
            and all(service.get_is_enabled(self.octobot.config) for service in notifier_class.REQUIRED_SERVICES)
