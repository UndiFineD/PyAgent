# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\strategy_design_optimizer.py
...
class StrategyDesignOptimizer:
    ...
    async def _create_run_queues(self, optimizer_id, run_data):
        run_data_by_hash = (
            self._get_optimizer_runs_details_and_hashes(run_data)
            if run_data
        ...
        return {self.START_QUEUE_KEY: start_queue, self.DONE_QUEUE_KEY: done_queue}
    ...
    async def multi_processed_optimize(self, optimizer_settings, run_data_by_optimizer_id=None):
        ...
        try:
            async for selected_optimizer_ids in self._all_optimizer_ids(
                optimizer_ids, optimizer_settings.empty_the_queue
            ):
                run_queues_by_optimizer_id = {
                    optimizer_id: await self._create_run_queues(
                        optimizer_id, run_data_by_optimizer_id.get(optimizer_id)
                    )
    ...
    async def _run_multi_processed_optimizer(
        self,
        optimizer_settings,
        lock,
        shared_keep_running,
        ...
        with (
            multiprocessing_util.registered_lock_and_shared_elements(
            ...
            coros = []
            self.logger.info(
                f"Dispatching optimizer backtesting runs into {self.active_processes_count} "
                f"parallel processes (based on the current computer physical processors)."
            )
            if self.active_processes_count < 1:
                idle_cores_message = (
                    f"Requiring to leave {optimizer_settings.required_idle_cores} idle core. "
                    if optimizer_settings.required_idle_cores
                    else ""
                )
                raise RuntimeError(
                    f"{idle_cores_message}At lease one core is required to "
    ...
    def find_optimal_configuration_wrapper(
        self,
        data_files,
    ...
    async def find_optimal_configuration(
        ...
        try:
            for optimizer_id, run_queues in run_queues_by_optimizer_id.items():
                await self._execute_optimizer_run(
                    optimizer_id,
                    run_queues,
                    update_database,
                    data_files,
    ...
    async def _read_optimizer_runs_details_and_hashes(self, optimizer_id):
        async with databases.DBReader.database(
    ...
    def _get_optimizer_runs_details_and_hashes(self, run_data):
        if run_data:
            return {self.get_run_hash(run_details): run_details for run_details in run_data.values()}
        raise NoMoreRunError

    async def _execute_optimizer_run(
        self,
        optimizer_id,
        run_queues,
        update_database,
    ...
    async def _update_runs_from_done_queue(self, run_queues, optimizer_id):
        ...
        async with databases.DBWriterReader.database(
            ...
            updated_queue[self.CONFIG_RUNS] = {index: run for index, run in enumerate(runs)}
            query = await writer_reader.search()
            if runs and runs[0]:
                await writer_reader.update(
                    self.RUN_SCHEDULE_TABLE,
                    updated_queue,
    ...
    async def _get_remaining_runs_count_from_multi_process_queue(self, optimizer_ids):
        try:
            ...
            run_queues_by_optimizer_id = multiprocessing_util.get_shared_element(self.SHARED_RUNS_QUEUES_KEY)
            return sum(
                run_queues[self.START_QUEUE_KEY].qsize()
                for optimizer_id, run_queues in run_queues_by_optimizer_id.items()
    ...
    async def get_overall_progress(self):
        ...
        remaining_time = (
            remaining_runs * self.get_average_run_time() / self.active_processes_count
            if self.active_processes_count
            else 0
        )
        ...
        if all_run_results:
            ...
            for run_data_from_runs in reversed(all_run_results):
                for to_find_run_data in run_data_elements.values():
                    if self._is_using_these_user_inputs(run_data_from_runs, to_find_run_data):
    ...
    def _is_using_these_user_inputs(self, full_run_data, to_find_run_data):
        ...
        for user_input_data in to_find_run_data:
            try:
                ...
                if (
                    full_run_data_user_inputs[user_input_data[self.CONFIG_TENTACLE][0]][
                        user_input_data[self.CONFIG_USER_INPUT]
                    ]
                    != user_input_data[self.CONFIG_VALUE]
                ):
                    return False
            except KeyError:
                return False
    ...
    async def _send_optimizer_finished_notification(self):
        await services_api.send_notification(
            services_api.create_notification(
                f"Your strategy optimizer just finished.",
    ...
    async def generate_and_save_run(self):
        ...
        self.optimizer_settings.optimizer_id = (
            await self.run_dbs_identifier.generate_new_optimizer_id(taken_ids)
            if self.optimizer_settings.optimizer_id is None
            else self.optimizer_settings.optimizer_id
        )
    ...
    async def get_run_queue(cls, trading_mode, campaign_name=None):
        ...
        try:
            async with databases.DBReader.database(
                run_dbs_identifier.get_optimizer_runs_schedule_identifier(),
                with_lock=True,
            ) as reader:
                ...
                [
                    {
                        ...
                        cls.CONFIG_VALUE: (
                            float(run_data_item[cls.CONFIG_VALUE])
                            if isinstance(run_data_item[cls.CONFIG_VALUE], int)
                            else run_data_item[cls.CONFIG_VALUE]
                        ),
                    }
    ...
    async def update_run_queue(cls, trading_mode, updated_queue):
        ...
        async with databases.DBWriterReader.database(
            run_dbs_identifier.get_optimizer_runs_schedule_identifier(), with_lock=True
        ) as writer_reader:
            query = await writer_reader.search()
            try:
                ...
                try:
                    db_optimizer_run = (
                        await writer_reader.select(
                            cls.RUN_SCHEDULE_TABLE,
                            query.id == updated_queue[cls.CONFIG_ID],
                        )
                    )[0]
                except json.JSONDecodeError:
                    pass
                existing_runs = set(
                    cls.get_run_hash(run_data) for run_data in db_optimizer_run.get(cls.CONFIG_RUNS).values()
                )
                to_remove_indexes = []
                ...
                updated_queue[cls.CONFIG_RUNS] = {index: run for index, run in enumerate(runs)}
    ...
    async def run_single_iteration(
        ...
        start_run = False
        selected_run_hash, run_details = await self._synchronized_pick_run(
            run_queues[self.START_QUEUE_KEY], run_data_by_hash
        )
        if run_details:
            ...
            run_dbs_identifier = databases.RunDatabasesIdentifier(
                ...
                backtesting_id=backtesting_run_id,
            )
        ...
        if start_run:
            try:
                run_result = await self._run_with_config(
                    optimizer_id,
                    data_files,
                    backtesting_run_id,
                    run_details,
    ...
    async def _run_with_config(
        ...
        self.logger.debug(f"Running optimizer with id {optimizer_id} on backtesting {run_id} with config {run_config}")
    ...
    def _updated_nested_tentacle_config(self, nested_tentacles, user_input, config_value, local_tentacle_config):
        ...
        if len(nested_tentacles) == 1:
            ...
            self._updated_nested_tentacle_config(
                nested_tentacles[1:],
                user_input,
                config_value,
                local_tentacle_config[cleaned_tentacle_name],
    ...
    def _get_custom_tentacles_setup_config(self, optimizer_id, run_id, run_config):
        ...
        tentacles_manager_api.set_tentacles_setup_configuration_path(
            local_tentacles_setup_config, tentacles_setup_config_path
        )
        ...
        for input_config in run_config:
            self._updated_nested_tentacle_config(
                input_config[self.CONFIG_TENTACLE],
        ...
        for tentacle, updated_values in tentacles_updates.items():
            ...
            current_config = tentacles_manager_api.get_tentacle_config(self.base_tentacles_setup_config, tentacle_class)
    ...
    def _ensure_local_config_folders(run_folder):
        ...
        tentacles_specific_config_folder = os.path.join(
            run_folder, tentacles_manager_constants.TENTACLES_SPECIFIC_CONFIG_FOLDER
        )
    ...
    def _generate_runs(self):
        ...
        runs = {index: run for index, run in enumerate(itertools.product(*iterations)) if self._is_run_allowed(run)}
        if runs:
    ...
    def _parse_filter_entry(self, run, run_filter_config):
        ...
        return optimizer_filter.OptimizerFilter(
            user_input_left_operand_key,
            user_input_right_operand_key,
            left_operand,
            right_operand,
            operator,
        )
    ...
    def _get_config_possible_iterations(self):
        return [self._generate_possible_values(config_element) for config_element in self._get_config_elements()]
        ...
        try:
            if config_element[self.CONFIG_ENABLED]:
                ...
                if config_element[self.CONFIG_TYPE] is ConfigTypes.NUMBER:
                    ...
                    values = [
                        v
                        for v in self._get_all_possible_values(
            ...
            return [
                {
                    ...
                    self.CONFIG_TENTACLE: config_element[self.CONFIG_VALUE][self.CONFIG_TENTACLE].split(
                        self.CONFIG_NESTED_TENTACLE_SEPARATOR
                    ),
    ...
    def _get_config_type(self, value):
        ...
        if isinstance(config_values, dict):
            try:
                if all(
                    isinstance(config_values[key], (float, int))
    ...
    async def _create_run_schedule_and_config_snapshot(self):
        ...
        try:
            ...
            with open(
                os.path.join(optimizer_target_identifier, self.LAST_CREATED_QUEUE_CONFIG),
                "w",
            ) as fw:
            ...
            try:
                ...
                if existing_runs:
                    ...
                    self.runs_schedule[self.CONFIG_RUNS] = {
                        f"{index}": details for index, details in enumerate(merged_runs.values())
                    }
                    await writer_reader.delete(
                        self.RUN_SCHEDULE_TABLE,
                        (await writer_reader.search()).id == self.optimizer_settings.optimizer_id,
                    )
