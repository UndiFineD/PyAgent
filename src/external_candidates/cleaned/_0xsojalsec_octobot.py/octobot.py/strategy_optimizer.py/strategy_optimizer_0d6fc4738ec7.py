...
class StrategyOptimizer:
    ...
    def find_optimal_configuration(self, TAs=None, time_frames=None, risks=None):
        if not self.is_computing:
            ...
            try:
                ...
                self.all_time_frames = (
                    self.strategy_class.get_required_time_frames(self.config, self.tentacles_setup_config)
            ...
            raise RuntimeError(
                f"{self.get_name()} is already computing: processed {self.run_id}/{self.total_nb_runs} processed"
            )
    ...
    def print_report(self):
        ...
        self.logger.info(
            f"{best_result[CONFIG].get_result_string()} average trades count: {best_result[TRADES_IN_RESULT]:f}"
        )
    ...
    def _get_activated_element(
        ...
        if nb_elements_to_consider > 1:
            i = 0
            while i < len(all_elements) and len(eval_conf) < nb_elements_to_consider + additional_elements_count:
    ...
    def _get_all_TA(self):
        return [
            evaluator
            for evaluator, activated in tentacles_manager_api.get_tentacles_activation(self.tentacles_setup_config)[
