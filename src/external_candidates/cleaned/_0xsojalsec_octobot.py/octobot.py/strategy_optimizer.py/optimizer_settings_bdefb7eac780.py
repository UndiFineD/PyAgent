# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\optimizer_settings.py
...
class OptimizerSettings:
    def __init__(self, settings_dict=None):
        ...
        self.optimizer_mode = settings_dict.get(enums.OptimizerConfig.MODE.value, enums.OptimizerModes.NORMAL.value)
        ...
        self.db_update_period = int(
            settings_dict.get(
                enums.OptimizerConfig.DB_UPDATE_PERIOD.value,
        ...
        self.stay_within_boundaries = settings_dict.get(enums.OptimizerConfig.STAY_WITHIN_BOUNDARIES.value, False)
    ...
    def parse_fitness_parameters(self, parameters):
        return [fitness_parameter.FitnessParameter.from_dict(param) for param in parameters]
    ...
    def parse_optimizer_filter(self, filters):
        return [optimizer_filter.OptimizerFilter.from_dict(element) for element in filters]
    ...
    def parse_optimizer_constraint(self, constaints):
        return {
            ...
            for element in (optimizer_constraint.OptimizerConstraint.from_dict(element) for element in constaints)
        }
