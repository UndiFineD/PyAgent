...
def config_health_check(config: configuration.Configuration, in_backtesting: bool) -> configuration.Configuration:
    ...
    try:
        ...
        if trader_enabled:
            ...
            if simulator_enabled:
                logger.error(
                    f"Impossible to activate a trader simulator additionally to a real trader, simulator deactivated."
                )
    ...
    if should_replace_config:
        try:
            ...
            logger.error(f"Save of the health checked config failed : {e}, will use the initial config")
...
def get_default_tentacles_url(version=None):
    ...
    tentacles_repository = (
        ...
        else os.getenv(constants.ENV_TENTACLES_REPOSITORY, constants.TENTACLES_REPOSITORY)
    )
    if version is None:
        version = (
            constants.TENTACLES_REQUIRED_VERSION if constants.TENTACLES_REQUIRED_VERSION else constants.LONG_VERSION
        )
    return os.getenv(
...
def migrate_from_previous_config(config):
    ...
    if common_constants.CONFIG_CRYPTO_CURRENCIES in config.config:
        ...
        for exchange_config in config.config[common_constants.CONFIG_EXCHANGES].values():
            exchange_config[common_constants.CONFIG_ENABLED_OPTION] = exchange_config.get(
