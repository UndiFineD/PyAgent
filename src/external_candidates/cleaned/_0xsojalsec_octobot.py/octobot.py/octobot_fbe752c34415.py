...
class OctoBot:
    ...
    async def _post_initialize(self):
        ...
        if self.startup_messages:
            for limit_message in self.startup_messages:
                ...
                await service_api.send_notification(service_api.create_notification(limit_message))
        ...
        for exchange_manager in exchange_managers:
            for topic in constants.REQUIRED_TOPIC_FOR_DATA_INIT:
                await commons_tree.EventProvider.instance().wait_for_event(
                    self.bot_id,
                    commons_tree.get_exchange_path(trading_api.get_exchange_name(exchange_manager), topic.value),
                    timeout,
                )
    ...
    async def _on_profile_update(self, profile_name: str):
        ...
        asyncio.create_task(
            self.delayed_restart(constants.PROFILE_UPDATE_RESTART_MIN * commons_constants.MINUTE_TO_SECONDS)
        )
