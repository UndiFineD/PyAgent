...
def start_octobot(args, default_config_file=None):
    ...
    try:
        ...
        # load configuration
        config, is_first_startup = _create_startup_config(logger, default_config_file or constants.DEFAULT_CONFIG_FILE)
        ...
        community_auth = (
            None if args.backtesting else asyncio.run(_get_authenticated_community_if_possible(config, logger))
        )
...
def octobot_parser(parser, default_config_file=None):
    parser.add_argument("-v", "--version", help="Show OctoBot current version.", action="store_true")
    parser.add_argument(
        "-s",
        "--simulate",
    ...
    parser.add_argument("-u", "--update", help="Update OctoBot to latest version.", action="store_true")
    parser.add_argument(
        "-rts",
    ...
    parser.add_argument(
        "-b",
        "--backtesting",
        help="Start OctoBot in backesting mode using the backtesting config stored in config.json.",
        action="store_true",
    )
    parser.add_argument(
        "-bf",
    ...
    tentacles_parser = subparsers.add_parser(
        "tentacles",
        help='Calls OctoBot tentacles manager.\nUse "tentacles --help" to get the tentacles manager help.',
    )
