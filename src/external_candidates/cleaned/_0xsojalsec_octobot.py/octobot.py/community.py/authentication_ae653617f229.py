        ...
        try:
            ...
            try:
                ...
                if await self.auto_reauthenticate():
                    self.logger.error(f"Impossible to use default refresh token, using saved auth details instead.")
                    return await func(*args, **kwargs)
...
class CommunityAuthentication(authentication.Authenticator):
    ...
    async def register_filled_form(self, form_id):
        ...
        await self._update_account_metadata({self.user_account.FILLED_FORMS: updated_filled_forms})
    ...
    async def update_supports(self):
        def _supports_mock():
            return {"data": {"attributes": {"support_role": self.user_account.get_support_role()}}}
    ...
    async def update_is_hosting_enabled(self, enabled: bool):
        await self._update_account_metadata({self.user_account.HOSTING_ENABLED: enabled})

    def _create_client(self):
        return supabase_backend.CommunitySupabaseClient(self.backend_url, self.backend_key, self.configuration_storage)
    ...
    def is_using_the_current_loop(self):
        return self.supabase_client.event_loop is None or self.supabase_client.event_loop is asyncio.get_event_loop()
    ...
    async def _create_community_feed_if_necessary(self) -> bool:
        if self._community_feed is None:
            ...
            self._community_feed = community_feeds.community_feed_factory(self, constants.COMMUNITY_FEED_DEFAULT_TYPE)
            return True
        return False
    ...
    async def auto_reauthenticate(self) -> bool:
        if constants.IS_CLOUD_ENV and constants.USER_ACCOUNT_EMAIL and constants.USER_AUTH_KEY:
            ...
            await self.login(constants.USER_ACCOUNT_EMAIL, None, auth_key=constants.USER_AUTH_KEY)
    ...
    async def _load_bot_if_selected(self):
        ...
        if len(self.user_account.get_all_user_bots_raw_data()) == 0:
            await self.select_bot(self.user_account.get_bot_id(await self.create_new_bot()))
        # more than one possible bot, can't auto-select one

    async def create_new_bot(self):
        deployment_type = (
            backend_enums.DeploymentTypes.CLOUD if constants.IS_CLOUD_ENV else backend_enums.DeploymentTypes.SELF_HOSTED
        )
    ...
    async def refresh_selected_bot(self):
        self.user_account.set_selected_bot_raw_data(await self.supabase_client.fetch_bot(self.user_account.bot_id))
    ...
    def _get_self_hosted_bots(self, bots):
        return [bot for bot in bots if self.user_account.is_self_hosted(bot) and not self.user_account.is_archived(bot)]
    ...
    def _update_supports(self, resp_status, json_data):
        if resp_status == 200:
            ...
            self.logger.error(f"Error when fetching community support, error code: {resp_status}")
    ...
    async def _initialize_account(self, minimal=False, fetch_private_data=True):
        try:
            ...
            self.logger.exception(
                e,
                True,
                f"Error when fetching community data, please check your internet connection.",
            )
    ...
    async def fetch_bot_tentacles_data_based_config(
        ...
        return await self.supabase_client.fetch_bot_tentacles_data_based_config(bot_id, self, auth_key)
    ...
    async def fetch_private_data(self, reset=False):
        try:
            ...
            if constants.DISABLE_COMMUNITY_EXTENSIONS_CHECK:
                ...
                self.successfully_fetched_tentacles_package_urls = False
                packages, package_urls, fetched_mqtt_uuid, tradingview_email = await self._fetch_extensions_details(
                    mqtt_uuid
                )
    ...
    async def _fetch_extensions_details(self, mqtt_uuid: typing.Optional[str]) -> (list[str], list[str], str, str):
        ...
        packages = [package for package in extensions_details["paid_package_slugs"] if package]
    ...
    async def _update_deployment_activity(self):
        try:
            ...
            self.logger.debug(f"Skipping activity update: current bot {self.user_account.bot_id} has no deployment")
