# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\tentacles_packages.py
...
def get_to_install_and_remove_tentacles(community_auth, selected_profile_tentacles_setup_config, bot_version: str):
    installed_community_package_urls = [
        adapt_url_to_bot_version(package_url, bot_version)
        for package_url in tentacles_manager_api.get_all_installed_package_urls(selected_profile_tentacles_setup_config)
        if is_community_tentacle_url(package_url)
    ]
    additional_tentacles_package_urls = (
        [adapt_url_to_bot_version(package_url, bot_version) for package_url in community_auth.get_saved_package_urls()]
        if community_auth.is_logged_in()
        else []
    )
    to_keep_tentacles = set(
        additional_tentacles_package_urls
        + [adapt_url_to_bot_version(package_url, bot_version) for package_url in get_env_variable_tentacles_urls()]
    ...
    if not community_auth.successfully_fetched_tentacles_package_urls:
        ...
        can_remove_tentacles = False
    to_remove_urls = (
        [package_url for package_url in installed_community_package_urls if package_url not in to_keep_tentacles]
        if can_remove_tentacles
        else []
    )
    if to_remove_urls:
    ...
    for to_remove_url in to_remove_urls:
        installed_packages = tentacles_manager_api.get_installed_packages_from_url(
            selected_profile_tentacles_setup_config, to_remove_url
        )
    ...
    return (
        list(set(to_install_urls)),
        list(set(to_remove_tentacles)),
        force_refresh_tentacles_setup_config,
    )
...
def get_env_variable_tentacles_urls() -> list[str]:
    return (
        ...
        if constants.ADDITIONAL_TENTACLES_PACKAGE_URL
        else []
    )
