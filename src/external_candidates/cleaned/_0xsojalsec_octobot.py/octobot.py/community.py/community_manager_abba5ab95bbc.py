# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\community_manager.py
...
class CommunityManager:
        ...
        if self.enabled:
            try:
                ...
                await asyncio.sleep(
                    ...
                    else common_constants.TIMER_BEFORE_METRICS_REGISTRATION_SECONDS
                )
    ...
    def background_get_id_and_register_bot(octobot_api):
        ...
        threading.Thread(
            target=community_manager._blocking_get_id_and_register,
            name="CommunityManagerGetId",
        ).start()

    def _blocking_get_id_and_register(self):
        try:
            resp = requests.get(
                f"{common_constants.METRICS_URL}{common_constants.METRICS_ROUTE_GEN_BOT_ID}",
            ...
            if resp.status_code != 200:
                ...
                requests.post(
                    f"{common_constants.METRICS_URL}{common_constants.METRICS_ROUTE_REGISTER}",
                    json=community,
                    headers=self._headers,
                )
        except Exception as e:
    ...
    async def _update_session(self, retry_on_error=True):
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
            community_fields.CommunityFields.UP_TIME.value
        ] = int(time.time() - self.octobot_api.get_start_time())
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
            community_fields.CommunityFields.PROFITABILITY.value
        ] = self._get_profitability()
        self.current_config[community_fields.CommunityFields.CURRENT_SESSION.value][
            community_fields.CommunityFields.TRADED_VOLUMES.value
        ] = self._get_traded_volumes()
    ...
    async def _update_authenticated_bot(self):
        try:
            if authentication.Authenticator.instance().is_logged_in():
                await authentication.Authenticator.instance().update_bot_config_and_stats(self._get_profitability())
    ...
    def _get_bot_community(self):
        return {
            ...
            community_fields.CommunityFields.CURRENT_SESSION.value: {
                ...
                community_fields.CommunityFields.SIGNAL_EMITTER.value: authentication.Authenticator.instance().get_is_signal_emitter(),
                community_fields.CommunityFields.SIGNAL_RECEIVER.value: authentication.Authenticator.instance().get_is_signal_receiver(),
    ...
    def _get_exchange_types(self):
        return [
            trading_api.get_exchange_type(exchange_manager).value for exchange_manager in self._get_exchange_managers()
        ]
    ...
    def _get_notification_types(self):
        has_notifications = (
            service_constants.CONFIG_CATEGORY_NOTIFICATION in self.edited_config.config
            and service_constants.CONFIG_NOTIFICATION_TYPE
            in self.edited_config.config[service_constants.CONFIG_CATEGORY_NOTIFICATION]
        )
        return (
            self.edited_config.config[service_constants.CONFIG_CATEGORY_NOTIFICATION][
                service_constants.CONFIG_NOTIFICATION_TYPE
            ]
    ...
    def _get_eval_config(self):
        ...
        for strategy in evaluator_api.get_evaluator_classes_from_type(
            evaluator_enums.EvaluatorMatrixTypes.STRATEGIES.value, tentacle_setup_config
        ):
            config_eval.append(strategy.get_name())

        # evaluators
        evaluators = evaluator_api.get_evaluator_classes_from_type(
            evaluator_enums.EvaluatorMatrixTypes.TA.value, tentacle_setup_config
        )
        evaluators += evaluator_api.get_evaluator_classes_from_type(
            evaluator_enums.EvaluatorMatrixTypes.SOCIAL.value, tentacle_setup_config
        )
        evaluators += evaluator_api.get_evaluator_classes_from_type(
            evaluator_enums.EvaluatorMatrixTypes.REAL_TIME.value, tentacle_setup_config
        )
    ...
    def _save_bot_id(self):
        if (
            common_constants.CONFIG_METRICS not in self.edited_config.config
    ...
    async def _post_community_data(self, route, bot, retry_on_error):
        try:
            async with self.session.post(
                f"{common_constants.METRICS_URL}{route}",
                json=bot,
                headers=self._headers,
            ) as resp:
    ...
    async def _handle_post_error(self, resp, retry_on_error):
        if resp.status != 200:
            if resp.status == 404:
                ...
                self.logger.debug(
                    f"Impossible to send community data : status code: {resp.status}, text: {await resp.text()}"
                )

    def _get_exchange_managers(self):
        return trading_api.get_exchange_managers_from_exchange_ids(self.octobot_api.get_exchange_manager_ids())
