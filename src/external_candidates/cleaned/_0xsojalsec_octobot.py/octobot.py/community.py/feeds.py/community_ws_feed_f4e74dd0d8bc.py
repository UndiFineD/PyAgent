...
class CommunityWSFeed(abstract_feed.AbstractFeed):
    ...
    async def connection_watcher(self):
        while not self.should_stop:
            ...
            if self._last_ping_time is not None and time.time() - self._last_ping_time > self.STALE_CONNECTION_TIMEOUT:
    ...
    def _get_callbacks(self, parsed_message):
        ...
        try:
            ...
            self.logger.debug(
                f"Unknown feed identifier: {parsed_message[commons_enums.CommunityFeedAttrs.STREAM_ID.value]}"
            )
            return
        if identifier is None:
    ...
    async def _connect(self):
        ...
        if not self.authenticator.is_logged_in():
            raise authentication.AuthenticationRequired(
                "OctoBot Community authentication is required to use community trading signals"
            )
