# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\feeds\community_mqtt_feed.py
...
class CommunityMQTTFeed(abstract_feed.AbstractFeed):
    ...
    async def _on_message(self, client, topic, payload, qos, properties):
        try:
            self.logger.debug(
                f"Received message, client_id: {self._get_username(client)}, topic: {topic}, "
    ...
    def _should_process(self, parsed_message):
        ...
        if len(self._processed_messages) > self.MAX_MESSAGE_ID_CACHE_SIZE:
            self._processed_messages = [
                message_id for message_id in self._processed_messages[self.MAX_MESSAGE_ID_CACHE_SIZE // 2 :]
            ]
        return True
    ...
    def _on_subscribe(self, client, mid, qos, properties):
        ...
        for subscription, granted_qos in zip(subscriptions, qos):
            ...
            if granted_qos >= gmqtt.constants.SubAckReasonCode.UNSPECIFIED_ERROR.value:
                self.logger.warning(
                    f"Retrying subscribe to {[s.topic for s in subscriptions]}, "
                ...
                if self._subscription_attempts < self.MAX_SUBSCRIPTION_ATTEMPTS * len(subscriptions):
                    ...
                    self.logger.error(
                        f"Max subscription attempts reached, stopping subscription "
    ...
    async def _connect(self):
        ...
        try:
            ...
            self.logger.error(
                f"Can't connect to server, your device uuid might be invalid. Current mqtt uuid is: {device_uuid}"
            )
            self._valid_auth = False
        except asyncio.TimeoutError as err:
    ...
    def _subscribe(self, topics):
        if not topics:
            ...
            return
        subscriptions = [gmqtt.Subscription(topic, qos=self.default_QOS) for topic in topics]
