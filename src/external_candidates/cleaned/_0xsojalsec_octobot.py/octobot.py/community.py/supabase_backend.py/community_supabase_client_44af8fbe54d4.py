...
class CommunitySupabaseClient(supabase_client.AuthenticatedAsyncSupabaseClient):
    ...
    def get_in_saved_session(self) -> typing.Union[gotrue.types.Session, None]:
        return self.auth._get_valid_session(self.auth._storage.sync_storage.get_item(self.auth._storage_key))
    ...
    async def update_metadata(self, metadata_update) -> dict:
        return (await self.auth.update_user({"data": metadata_update})).user.model_dump()
        ...
        with jwt_expired_auth_raiser():
            try:
                ...
                return (
                    ...
                    .select("*,bot_deployment:bot_deployments!bots_current_deployment_id_fkey(*)")
    ...
    async def fetch_bots(self) -> list:
        with jwt_expired_auth_raiser():
            return (
                await self.table("bots")
                .select("*,bot_deployment:bot_deployments!bots_current_deployment_id_fkey!inner(*)")
                .execute()
            ).data

    async def create_bot(self, deployment_type: enums.DeploymentTypes) -> dict:
        created_bot = (
            await self.table("bots").insert({enums.BotKeys.USER_ID.value: (await self._get_user()).id}).execute()
        ).data[0]
        bot_id = created_bot[enums.BotKeys.ID.value]
        created_deployment = await self._create_deployment(deployment_type, bot_id, constants.VERSION)
        await (
            self.table("bots")
            .update(
                {
            ...
            .eq(enums.BotDeploymentKeys.ID.value, bot_id)
            .execute()
        )
    ...
    def get_deployment_activity_update(self, last_activity: float, next_activity: float) -> dict:
        return {enums.BotDeploymentKeys.ACTIVITIES.value: self._get_activities_content(last_activity, next_activity)}
    ...
    async def fetch_bot_products_subscription(self, bot_deployment_id: str) -> dict:
        return (
            await self.table("bot_deployments")
            .select("products_subscription:products_subscriptions!product_subscription_id(id, status, desired_status)")
    ...
    async def fetch_trades(self, bot_id) -> list:
        ...
        return (await self.table("bot_trades").select("*").eq(enums.TradeKeys.BOT_ID.value, bot_id).execute()).data

    async def reset_trades(self, bot_id):
        return (await self.table("bot_trades").delete().eq(enums.TradeKeys.BOT_ID.value, bot_id).execute()).data
    ...
    async def fetch_bot_tentacles_data_based_config(
        ...
        try:
            bot_config = (
                await self.table("bots")
                .select("id,name, bot_config:bot_configs!current_config_id!inner(id, options, exchanges, is_simulated)")
                .eq(enums.BotKeys.ID.value, bot_id)
                .execute()
            ).data[0]
        except IndexError:
    ...
    def _include_tentacles_config(
        ...
        tentacle_config_by_tentacle = {
            tentacle_config.name: tentacle_config.config for tentacle_config in profile_data.tentacles
        }
    ...
    async def fetch_bot_profile_data(
        ...
        profile_data.profile_details.name = formatters.create_profile_name(
            master_product_details.get(enums.ProductKeys.SLUG.value, "") or profile_data.profile_details.name,
        ...
        profile_data.trading.minimal_funds = (
            [
                ...
                for minimal_fund in executed_product_config["product"][enums.ProductKeys.ATTRIBUTES.value].get(
                    "minimal_funds", []
                )
            ]
        ...
        master_product_exchanges_configs = master_product_config[enums.ProfileConfigKeys.CONFIG.value].get("exchanges")
        ...
        if portfolio:
            ...
            try:
                formatted_portfolio = formatters.get_adapted_portfolio(usd_like_asset, portfolio)
    ...
    async def _fetch_full_exchange_configs(
        ...
        return [
            commons_profiles.ExchangeData.from_dict(exchange_data) for exchange_data in exchanges_configs
        ], exchange_tentacles_data
    ...
    async def fetch_configs(self, bot_id: str) -> list:
        ...
        return (await self.table("bot_configs").select("*").eq(enums.BotConfigKeys.BOT_ID.value, bot_id).execute()).data

    async def fetch_portfolios(self, bot_id: str) -> list:
        return (
            await self.table("bot_portfolios").select("*").eq(enums.PortfolioKeys.BOT_ID.value, bot_id).execute()
        ).data

    async def fetch_portfolio_from_id(self, portfolio_id: str) -> list:
        return (
            await self.table("bot_portfolios").select("*").eq(enums.PortfolioKeys.ID.value, portfolio_id).execute()
        ).data
    ...
    async def _paginated_fetch_historical_data(
        ...
        def request_factory(table: postgrest.AsyncRequestBuilder, select_count, last_fetched_row):
            ...
            if last_fetched_row is None:
                request = request.match(matcher).gte("timestamp", self.get_formatted_time(first_open_time))
            else:
                request = request.match(matcher).gt("timestamp", last_fetched_row["timestamp"])
            return request.lte("timestamp", self.get_formatted_time(last_open_time)).order("timestamp", desc=False)
    ...
    async def paginated_fetch(
        ...
        while request_count < self.MAX_PAGINATED_REQUESTS_COUNT:
            ...
            if (
                ...
                or len(fetched_elements) < max_size_per_fetch  # fetched the last elements
    ...
    def _format_gpt_signals(self, signals: list):
        return {
            self.get_parsed_time(signal["timestamp"]).timestamp(): signal["signal"]["content"] for signal in signals
        }
