# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\models\formatters.py
...
def _format_trade(trade: dict, exchange_name: str, bot_id: str):
    metadata = {
        trading_enums.ExchangeConstantsOrderColumns.ENTRIES.value: trade[
    ...
    return {
        ...
        backend_enums.TradeKeys.QUANTITY.value: float(trade[trading_enums.ExchangeConstantsOrderColumns.AMOUNT.value]),
    ...
    return [
        {
            ...
            backend_enums.PositionKeys.TIME.value: position[
                trading_enums.ExchangeConstantsPositionColumns.TIMESTAMP.value
            ],
            backend_enums.PositionKeys.POSITION_ID.value: position[
                trading_enums.ExchangeConstantsPositionColumns.ID.value
            ],
            # from trading positions
            backend_enums.PositionKeys.LOCAL_ID.value: position[
                trading_enums.ExchangeConstantsPositionColumns.LOCAL_ID.value
            ],
            backend_enums.PositionKeys.SYMBOL.value: position[
                trading_enums.ExchangeConstantsPositionColumns.SYMBOL.value
            ],
            backend_enums.PositionKeys.STATUS.value: position[
            ...
            backend_enums.PositionKeys.QUANTITY.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.QUANTITY.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.QUANTITY.value]
                else 0
            ),
            backend_enums.PositionKeys.SIZE.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.SIZE.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.SIZE.value]
                else 0
            ),
            backend_enums.PositionKeys.NOTIONAL.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.NOTIONAL.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.NOTIONAL.value]
                else 0
            ),
            backend_enums.PositionKeys.INITIAL_MARGIN.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.INITIAL_MARGIN.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.INITIAL_MARGIN.value]
                else 0
            ),
            backend_enums.PositionKeys.AUTO_DEPOSIT_MARGIN.value: position[
                trading_enums.ExchangeConstantsPositionColumns.AUTO_DEPOSIT_MARGIN.value
            ],
            backend_enums.PositionKeys.COLLATERAL.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.COLLATERAL.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.COLLATERAL.value]
                else 0
            ),
            backend_enums.PositionKeys.LEVERAGE.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.LEVERAGE.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.LEVERAGE.value]
                else 0
            ),
            backend_enums.PositionKeys.MARGIN_TYPE.value: position[
                trading_enums.ExchangeConstantsPositionColumns.MARGIN_TYPE.value
            ],
            backend_enums.PositionKeys.POSITION_MODE.value: position[
                trading_enums.ExchangeConstantsPositionColumns.POSITION_MODE.value
            ],
            backend_enums.PositionKeys.ENTRY_PRICE.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.ENTRY_PRICE.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.ENTRY_PRICE.value]
                else 0
            ),
            backend_enums.PositionKeys.MARK_PRICE.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.MARK_PRICE.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.MARK_PRICE.value]
                else 0
            ),
            backend_enums.PositionKeys.LIQUIDATION_PRICE.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.LIQUIDATION_PRICE.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.LIQUIDATION_PRICE.value]
                else 0
            ),
            backend_enums.PositionKeys.UNREALIZED_PNL.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.UNREALIZED_PNL.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.UNREALIZED_PNL.value]
                else 0
            ),
            backend_enums.PositionKeys.REALISED_PNL.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.REALISED_PNL.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.REALISED_PNL.value]
                else 0
            ),
            backend_enums.PositionKeys.MAINTENANCE_MARGIN_RATE.value: (
                float(position[trading_enums.ExchangeConstantsPositionColumns.MAINTENANCE_MARGIN_RATE.value])
                if position[trading_enums.ExchangeConstantsPositionColumns.MAINTENANCE_MARGIN_RATE.value]
                else 0
            ),
        }
    ...
    return [
        {
            ...
            backend_enums.OrderKeys.TYPE.value: _get_order_type(storage_order[trading_constants.STORAGE_ORIGIN_VALUE]),
            ...
            backend_enums.OrderKeys.CHAINED.value: (
                format_orders(
                    storage_order.get(trading_enums.StoredOrdersAttr.CHAINED_ORDERS.value, []),
                    exchange_name,
                )
                if storage_order.get(trading_enums.StoredOrdersAttr.CHAINED_ORDERS.value, [])
                else []
            ),
        }
...
def get_master_and_nested_product_slug_from_profile_name(
    profile_name: str,
...
def ensure_profile_data_exchanges_internal_name_and_type(
...
def format_portfolio(
    ...
    return {
        ...
        backend_enums.PortfolioKeys.PORTFOLIO_TYPE.value: (
            backend_enums.PortfolioTypes.SUB_PORTFOLIO
            if is_sub_portfolio
    ...
    return [
        {
            ...
            backend_enums.PortfolioAssetKeys.VALUE.value: float(quantity[commons_constants.PORTFOLIO_TOTAL])
            * float(price_by_asset.get(key, 0)),
        }
...
def format_portfolio_with_profitability(profitability) -> dict:
    return {backend_enums.PortfolioKeys.PROFITABILITY.value: float(profitability)}
...
def format_portfolio_history(history: dict, unit: str, portfolio_id: str) -> list:
    try:
        return [
            {
                backend_enums.PortfolioHistoryKeys.TIME.value: supabase_backend.CommunitySupabaseClient.get_formatted_time(
                    timestamp
                ),
            ...
            (
                {
                    tentacles_exchanges.HollaexAutofilled.AUTO_FILLED_KEY: {
                        exchange_internal_name: {tentacles_exchanges.HollaexAutofilled.URL_KEY: exchange_url}
                    }
                }
                if config is None
                else config
