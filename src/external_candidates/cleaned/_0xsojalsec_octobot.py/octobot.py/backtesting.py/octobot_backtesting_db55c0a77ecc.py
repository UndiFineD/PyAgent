...
class OctoBotBacktesting:
    def __init__(
        ...
        if self.backtesting_data is not None:
            self.backtesting_files = [backtesting_file for backtesting_file in self.backtesting_data.data_files]
    ...
    async def initialize_and_run(self):
        ...
        symbols_by_exchange = {
            exchange: [str(s) for s in symbols] for exchange, symbols in self.symbols_to_create_exchange_classes.items()
        }
    ...
    def check_remaining_objects(self):
        ...
        for obj, max_ref in expected_max_objects_references.items():
            if objects_references[obj][0] > max_ref:
                objects_leak_errors.append(_get_remaining_object_error(obj, max_ref, objects_references[obj]))
    ...
    async def _create_service_feeds(self):
        for feed in self.service_feeds:
            if not await service_api.start_service_feed(feed, False, {}):
                self.logger.error(
                    f"Failed to start {feed.get_name()}. Evaluators requiring this service feed might not work properly"
                )
    ...
    async def _fetch_backtesting_extra_data_if_any(self, min_timestamp: float, max_timestamp: float):
    ...
    async def _init_exchanges(self):
        for exchange_class_string in self.symbols_to_create_exchange_classes.keys():
            is_future = (
                self.exchange_type_by_exchange[exchange_class_string] == commons_constants.CONFIG_EXCHANGE_FUTURE
            )
            exchange_builder = (
                trading_api.create_exchange_builder(self.backtesting_config, exchange_class_string)
