...
class IndependentBacktesting:
    ...
    def get_market_delta(symbol, exchange_manager, min_timeframe):
        ...
        if market_begin and market_end and market_begin > 0:
            market_delta = (
                market_end / market_begin - 1 if market_end >= market_begin else -1 * (1 - market_end / market_begin)
            )
        else:
            market_delta = 0
    ...
    def _init_default_config_values(self):
        self.risk = copy.deepcopy(
            self.octobot_origin_config[common_constants.CONFIG_TRADING][common_constants.CONFIG_TRADER_RISK]
        )
        ...
        self.fees_config = copy.deepcopy(
            self.octobot_origin_config[common_constants.CONFIG_SIMULATOR][common_constants.CONFIG_SIMULATOR_FEES]
        )
                    ...
                    report[CHART_IDENTIFIERS].append(
                        {
                            ...
                            "time_frames": list(
                                reversed([tf.value for tf in trading_api.get_watched_timeframes(exchange_manager)])
                            ),
                        }
                    )
                except KeyError:
    ...
    def _adapt_config(self):
        ...
        self.backtesting_config[common_constants.CONFIG_TRADING][common_constants.CONFIG_TRADER_REFERENCE_MARKET] = (
            self._find_reference_market_and_update_contract_type()
        )
        self.backtesting_config[common_constants.CONFIG_SIMULATOR][common_constants.CONFIG_STARTING_PORTFOLIO] = (
            self.starting_portfolio
        )
        self.backtesting_config[common_constants.CONFIG_SIMULATOR][common_constants.CONFIG_SIMULATOR_FEES] = (
            self.fees_config
        )
    ...
    def _find_reference_market_and_update_contract_type(self):
        ...
        for symbols in self.symbols_to_create_exchange_classes.values():
            ...
            for symbol in symbols:
                ...
                if (
                    ref_market_candidate != quote
                    and ref_market_candidates[ref_market_candidate] < ref_market_candidates[quote]
                ):
                    ref_market_candidate = quote
