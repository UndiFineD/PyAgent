...
class DataBank(Initializable):
    def __init__(self, data_file=None):
        super().__init__()
        self.data_file = (
            data_file if data_file else "tests/static/AbstractExchangeHistoryCollector_1586017993.616272.data"
        )
    ...
    def _set_candle_manager(self, time_frame, initial_candles_max_index, use_current_data=False):
        self.candles_managers_by_time_frame[time_frame] = create_new_candles_manager(
            ...
            if use_current_data
            else self.origin_ohlcv_by_symbol[self.default_symbol][time_frame][:initial_candles_max_index]
        )
