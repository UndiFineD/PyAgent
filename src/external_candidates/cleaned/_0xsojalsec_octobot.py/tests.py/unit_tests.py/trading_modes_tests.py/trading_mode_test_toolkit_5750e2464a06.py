# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\trading_modes_tests\trading_mode_test_toolkit.py
...
def check_oco_order_group(order, order_type, order_price, market_status):
    orders_from_group = [
        found_order for found_order in order.order_group.get_group_open_orders() if found_order is not order
    ]
...
def check_orders(orders, evaluation, state, nb_orders, market_status):
    if state == trading_enum.EvaluatorStates.NEUTRAL.value or state is None:
        ...
        if math.isnan(evaluation):
            ...
            state is None
            or isinstance(state, (int, float, dict))
            or state not in [s.value for s in trading_enum.EvaluatorStates]
        ):
            assert orders == []
        else:
            assert (
            ...
            if orders:
                ...
                if order.order_type in (
                    trading_enum.TraderOrderType.SELL_MARKET,
                    trading_enum.TraderOrderType.BUY_MARKET,
                ):
            ...
            for symbol in [market, order_symbol]:
                ...
                if not only_positivity:
                    if order.order_type in (
                        trading_enum.TraderOrderType.SELL_MARKET,
                        trading_enum.TraderOrderType.BUY_MARKET,
                    ) and symbol in [order.market, order.currency]:
                        # order is filled
                        assert initial_portfolio.get_currency_portfolio(symbol) != portfolio.get_currency_portfolio(
                            symbol
                        )
                    else:
                        if order_symbol == symbol:
                            assert (
                                initial_portfolio.get_currency_portfolio(symbol).total
                            ...
                            assert (
                                initial_portfolio.get_currency_portfolio(market).total
