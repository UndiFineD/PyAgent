# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\community\test_community_supabase_feed.py
...
async def authenticated_feed():
    ...
    try:
        ...
        realtime_mock = mock.Mock(channel=mock.AsyncMock(return_value=channel_mock))
        ...
        with mock.patch.object(
            ...
            mock.Mock(side_effect=_community_feed_factory),
...
async def test_start_and_connect(authenticated_feed):
    ...
    with (
        mock.patch.object(
            authenticated_feed.authenticator,
            "is_logged_in",
            mock.Mock(return_value=True),
        ) as is_logged_in_mock,
        mock.patch.object(authenticated_feed, "_subscribe_to_table", mock.AsyncMock()) as _subscribe_to_table_mock,
    ):
        # without feed_callbacks
        with mock.patch.object(
            authenticated_feed.authenticator.supabase_client,
            "get_subscribed_channel_tables",
            mock.Mock(return_value=[]),
        ) as get_subscribed_channel_tables_mock:
        ...
        with mock.patch.object(
            authenticated_feed.authenticator.supabase_client,
            "get_subscribed_channel_tables",
            mock.Mock(return_value=["signals"]),
        ) as get_subscribed_channel_tables_mock:
...
async def test_connection(authenticated_feed):
    with mock.patch.object(
        authenticated_feed.authenticator, "is_logged_in", mock.Mock(return_value=True)
    ) as is_logged_in_mock:
        with mock.patch.object(
            authenticated_feed.authenticator.supabase_client,
            "is_realtime_connected",
            mock.Mock(return_value=True),
        ) as is_realtime_connected_mock:
            ...
            is_logged_in_mock.assert_called_once()
    with mock.patch.object(
        authenticated_feed.authenticator, "is_logged_in", mock.Mock(return_value=False)
    ) as is_logged_in_mock:
        with mock.patch.object(
            authenticated_feed.authenticator.supabase_client,
            "is_realtime_connected",
...
async def test_subscribe_to_table(authenticated_feed):
    ...
    authenticated_feed._realtime_client.channel.assert_called_once_with(authenticated_feed.SCHEMA, "plop_table")
    channel_mock.on.assert_called_once_with(authenticated_feed.INSERT_EVENT, authenticated_feed._message_callback)
    channel_mock.subscribe.assert_called_once_with(authenticated_feed._subscribed_callback)
...
async def test_register_feed_callback(authenticated_feed):
    ...
    with mock.patch.object(
        authenticated_feed, "_subscribe_to_table_if_necessary", mock.AsyncMock()
        ...
        assert authenticated_feed.feed_callbacks == {target_table: {"identifier": ["plop_cb"]}}
        ...
        assert authenticated_feed.feed_callbacks == {
            target_table: {"identifier": ["plop_cb", "plop_cb"]}  # added callback
        }
        _subscribe_to_table_if_necessary_mock.assert_called_once_with(target_table)
        _subscribe_to_table_if_necessary_mock.reset_mock()

        await authenticated_feed.register_feed_callback(commons_enums.CommunityChannelTypes.SIGNAL, "plop_cb2")
        assert authenticated_feed.feed_callbacks == {
            target_table: {"identifier": ["plop_cb", "plop_cb"], None: ["plop_cb2"]}
        }
        _subscribe_to_table_if_necessary_mock.assert_called_once_with(target_table)
        _subscribe_to_table_if_necessary_mock.reset_mock()
        ...
        with pytest.raises(NotImplementedError):
            await authenticated_feed.register_feed_callback("wrong type", "plop_cb3")
        assert authenticated_feed.feed_callbacks == {
            target_table: {"identifier": ["plop_cb", "plop_cb"], None: ["plop_cb2"]}
        }
