...
async def test_fetch_bot_nested_config_profile_data_if_any_invalid_inputs(
    ...
    assert await mock_supabase_client.fetch_bot_nested_config_profile_data_if_any(nested_config_id="") is None
    mock_supabase_client.table.assert_not_called()

    with pytest.raises(TypeError):
        await mock_supabase_client.fetch_bot_nested_config_profile_data_if_any(nested_config_id="plop")
    mock_supabase_client.table.assert_called_once()
    mock_supabase_client.table.reset_mock()
...
async def test_fetch_bot_nested_config_profile_data_if_any_valid_inputs(
    ...
    config = await mock_supabase_client.fetch_bot_nested_config_profile_data_if_any(
        master_profile_config={
            enums.NestedProductConfigKeys.NESTED_CONFIG_ROOT.value: {enums.NestedProductConfigKeys.SLUG.value: "plop"}
        }
    )
    assert config == {
        "config": "product_config.config-xyz",
    ...
    config = await mock_supabase_client.fetch_bot_nested_config_profile_data_if_any(nested_config_id="plop")
    assert config == {
        "config": "config-xyz",
        "id": "id-xyz",
        "product_config": {
