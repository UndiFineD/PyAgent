...
async def test_login(auth):
    resp_mock = mock.Mock()
    with (
        mock.patch.object(community.CommunityAuthentication, "_reset_tokens", mock.Mock()) as reset_mock,
        mock.patch.object(
            community.CommunityAuthentication, "_ensure_community_url", mock.Mock()
        ) as _ensure_community_url_mock,
        mock.patch.object(community.CommunityAuthentication, "_ensure_email", mock.Mock()) as _ensure_email_mock,
        mock.patch.object(
            community.CommunityAuthentication, "_on_account_updated", mock.AsyncMock()
        ) as _on_account_updated_mock,
        mock.patch.object(community.CommunityAuthentication, "is_logged_in", mock.Mock()) as is_logged_in_mock,
        mock.patch.object(community.CommunityAuthentication, "on_signed_in", mock.AsyncMock()) as on_signed_in_mock,
    ):
...
async def test_fetch_bot_profile_data_without_tentacles_options(auth):
    FETCHED_PROFILE_USD_LIKE = {
        ...
        "exchanges": [{"exchange_credential_id": "30ee7b12-3415-4ce4-b050-80d8bf4548be"}],
        "is_simulated": True,
        ...
        parsed_data = octobot_commons.profiles.profile_data.ProfileData.from_dict(
            {
                ...
                "trading": {
                    "minimal_funds": [{"asset": "USD-like", "available": 50, "total": 50}],
...
def test_remove_login_detail(auth):
    with (
        mock.patch.object(auth, "_reset_login_token", mock.Mock()) as _reset_login_token_mock,
...
def test_reset_login_token(auth):
    with mock.patch.object(octobot_commons.configuration.Configuration, "save", mock.Mock()) as save_mock:
        auth.configuration_storage.set_configuration(octobot_commons.configuration.Configuration("", ""))
        ...
        assert (
            auth.configuration_storage.sync_storage._configuration.config[constants.CONFIG_COMMUNITY]["_storage_key"]
            == ""
        )
...
def test_init_account(auth):
    with (
        mock.patch.object(asyncio, "create_task", mock.Mock(return_value="task")) as create_task_mock,
        mock.patch.object(auth, "_initialize_account", mock.Mock(return_value="coro")) as _auth_and_fetch_account_mock,
    ):
