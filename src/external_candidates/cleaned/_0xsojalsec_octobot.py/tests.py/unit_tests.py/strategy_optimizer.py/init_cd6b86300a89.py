# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\strategy_optimizer\__init__.py
...
def _get_mocked_ui_config():
    return {
        ...
        for element in [
            ...
            _get_optimizer_user_input_config(
                Strategies.SimpleStrategyEvaluator,
                Strategies.SimpleStrategyEvaluator.RE_EVAL_TA_ON_RT_OR_SOCIAL,
                True,
                [False],
            ),
        ]
...
def _get_filter_setting(left_value, right_value, flat_value, operator):
    return {
        "user_input_left_operand": _get_filter_setting_content("user_input_left_operand", left_value),
        "user_input_right_operand": _get_filter_setting_content("user_input_right_operand", right_value),
        "text_right_operand": _get_filter_setting_content("text_right_operand", flat_value),
        "operator": _get_filter_setting_content(strategy_optimizer.OptimizerFilter.OPERATOR_KEY, operator.value),
    }
...
def _get_mocked_filters_settings():
    return [
        _get_filter_setting(
            get_ui_identifier(Evaluator.RSIMomentumEvaluator, "period_length"),
            "null",
            5,
