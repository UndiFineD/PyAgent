# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\strategy_optimizer\test_strategy_design_optimizer.py
...
from tests.unit_tests.strategy_optimizer import (
...
async def test_generate_and_save_strategy_optimizer_runs(optimizer_inputs):
    ...
    with (
        mock.patch.object(databases.TinyDBAdaptor, "create_identifier", mock.AsyncMock()) as create_identifier_mock,
        mock.patch.object(
            strategy_optimizer.StrategyDesignOptimizer,
            "_save_run_schedule",
            mock.AsyncMock(),
        ) as _save_run_schedule_mock,
        mock.patch.object(
            strategy_optimizer.StrategyDesignOptimizer,
            "shuffle_and_select_runs",
            mock.Mock(side_effect=lambda *args, **_: args[0]),
        ) as shuffle_and_select_runs_mock,
    ):
        assert (
            await bot_module_api.generate_and_save_strategy_optimizer_runs(
...
async def test_resume_unknown_mode(optimizer_inputs):
    ...
    with mock.patch.object(optimizer, "_get_total_nb_runs", mock.AsyncMock(return_value=5)) as _get_total_nb_runs_mock:
...
async def test_resume_normal_mode(optimizer_inputs):
    ...
    with (
        mock.patch.object(optimizer, "_get_total_nb_runs", mock.AsyncMock(return_value=5)) as _get_total_nb_runs_mock,
        mock.patch.object(
            optimizer, "multi_processed_optimize", mock.AsyncMock(return_value=True)
        ) as multi_processed_optimize_mock,
    ):
