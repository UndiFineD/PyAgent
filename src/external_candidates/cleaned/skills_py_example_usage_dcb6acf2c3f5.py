# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\example_usage_dcb6acf2c3f5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\example_usage.py
...
def example_pattern_recognition():
    ...
    if df is not None:
        ...
        for p in patterns[:5]:  # Show first 5
            print(f"   - {p.get('pattern')}: {p.get('bias')} ({p.get('confidence', 0)}% confidence)")
        ...
        print(f"\nOverall Bias: {analysis.get('overall_bias')} ({analysis.get('confidence', 0)}% confidence)")
...
def example_market_scan():
    ...
    if hasattr(agent, "market_scanner"):
        ...
        opportunities = agent.market_scanner.scan_market(categories=["Major Coins", "AI Tokens"], top_n=3)
...
def example_component_usage():
    ...
    from patterns import MarketRegimeDetector, TrendAnalyzer, VolumeAnalyzer
