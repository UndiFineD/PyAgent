# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\lucaspdude.py\persistent_private_agent_memory.py\assets.py\service.py\main_7d4c8e91a96e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\lucaspdude\persistent-private-agent-memory\assets\service\main.py
...
async def recover_agent(request: AgentRecoveryRequest):
    ...
    try:
        ...
        try:
            ...
            cur.execute("SELECT public_key FROM agents WHERE agent_id = ?", (agent_id,))
            ...
            if stored_public != public_bytes:
                raise HTTPException(status_code=400, detail="Key mismatch. Invalid recovery phrase.")
...
async def store_memory(request: MemoryStoreRequest):
    ...
    try:
        ...
        try:
            ...
            cur.execute("SELECT public_key FROM agents WHERE agent_id = ?", (request.agent_id,))
        ...
        try:
            ...
            cur.execute(
                "SELECT COALESCE(MAX(version), 0) + 1 as next_version FROM memory_snapshots WHERE agent_id = ?",
                (request.agent_id,),
            )
...
async def retrieve_memory(request: MemoryRetrieveRequest):
    ...
    try:
        ...
        try:
            ...
            if not result:
                raise HTTPException(status_code=404, detail="No memory found for this agent.")
...
async def list_memory_history(request: MemoryRetrieveRequest):
    ...
    try:
        ...
        return MemoryListResponse(agent_id=request.agent_id, memories=memories, count=len(memories))
...
async def clear_memory(request: MemoryRetrieveRequest):
    ...
    try:
        ...
        try:
            ...
            cur.execute("DELETE FROM memory_snapshots WHERE agent_id = ?", (request.agent_id,))
