# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\outside_videos.py\analysis_70e32658186a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\analysis.py
...
class ExpPlay(Scene):
    ...
    def show_sum_up_to(self, n, shown_n=None, include_dots=False):
        ...
        formula = OldTex(
            "e^{it} \\approx",
            *["{(ti)^{%d} \\over %d!} +" % (k, k) for k in range(shown_n + 1)],
...
class ZetaSum(Scene):
    def construct(self):
        ...
        self.add(
            ...
            Dot(center_of_mass([line.get_end() for line in lines]), color=YELLOW),
        )
...
class ZetaSpiral(Scene):
    def construct(self):
        ...
        spiral = VGroup(
            *[
                ParametricCurve(
                    lambda t: complex_to_R3(zeta(complex(0.5, t))),
                    t_min=t1,
                    t_max=t2 + 0.1,
                )
