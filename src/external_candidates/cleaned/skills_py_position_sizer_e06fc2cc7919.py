# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\scripts.py\risk.py\position_sizer_e06fc2cc7919.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\risk\position_sizer.py
...
class PositionSizer:
    ...
    def calculate_position_size(
        ...
        try:
            ...
            standard_sizing = self._calculate_standard_sizing(entry_price, stop_loss, balance)

            # Kelly Criterion sizing (if metrics available)

            kelly_sizing = self._calculate_kelly_sizing(balance, risk_metrics or {})
            ...
            logger.info(
                f"Position size: {result['standard_size_coin']:.6f} coins (${result['standard_value_usd']:.2f})"
            )
    ...
    def _calculate_standard_sizing(self, entry_price: float, stop_loss: float, balance: float) -> Dict:
        """
    ...
    def _calculate_kelly_sizing(self, balance: float, risk_metrics: Dict) -> Dict:
        """
    ...
    def validate_position_size(self, position_value_usd: float, balance: float) -> tuple[bool, str]:
        """
        ...
        if position_value_usd > max_allowed:
            return (
                False,
                f"Position ${position_value_usd:.2f} exceeds maximum ${max_allowed:.2f}",
            )
