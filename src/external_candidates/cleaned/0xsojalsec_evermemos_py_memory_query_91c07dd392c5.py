...
class RetrieveMemRequest:
    ...
    radius: Optional[float] = None  # COSINE similarity threshold (use default 0.6 if None)
...
class RetrieveMemResponse:
    ...
    importance_scores: List[float] = field(default_factory=list)  # New: group importance scores

    original_data: List[Dict[str, List[Dict[str, Any]]]] = field(default_factory=list)  # New: original data
...
class ConversationMetaRequest:
    ...
    scene_desc: Dict[str, Any]  # Scene description, usually contains fields like bot_ids
