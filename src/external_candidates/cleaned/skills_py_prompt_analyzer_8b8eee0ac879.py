# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\dgriffin831.py\skill_scan.py\skill_scan.py\prompt_analyzer_8b8eee0ac879.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dgriffin831\skill-scan\skill_scan\prompt_analyzer.py
...
class PromptAnalyzer:
    ...
    def _detect_explicit_injection(self):
        patterns = [
            ...
            (
                r"\[INST\]|\[/INST\]",
                "Instruction delimiters — prompt injection",
                "critical",
                30,
            ),
            (
                r"\[SYSTEM\]|\[/SYSTEM\]",
                "System delimiters — prompt injection",
                "critical",
                30,
            ),
    ...
    def _detect_invisible_characters(self):
        ...
        if count > 10:
            char_class = "".join(f"\\u{ord(c):04x}" for c in INVISIBLE_CHARS)
    ...
    def _detect_mixed_scripts(self):
        ...
        if len(detected_scripts) > 3:
            ...
            self._add_finding(
                0,
                "MIXED_SCRIPTS",
                f"{len(detected_scripts)} different Unicode scripts detected: {scripts} — unusual for a skill file",
                "medium",
                10,
            )
    ...
    def _detect_manipulative_language(self):
        patterns = [
            ...
            (
                ...
                "Emotional pressure / savior framing",
                "medium",
                10,
            ),
            (
    ...
    def _detect_exfil_instructions(self):
        ...
        for m in exfil_prose.finditer(self._content):
            ...
            if re.search(
                r"(?:config|credential|key|token|secret|password|api|auth|env)",
                context,
                re.IGNORECASE,
            ):
                self._add_finding(
