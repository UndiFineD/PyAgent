# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentkit_prompting.py\examples.py\crafter.py\post_processing_25b622652b99.py
...
class ActionAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        if type(parsed_answer[-1]["repeats"]) == str and not (parsed_answer[-1]["repeats"]).isnumeric():
            raise ex.AfterQueryError(
                "Invalid answer",
                "Invalid repeats: '{}'. Expecting an integer.".format(parsed_answer[-1]["repeats"]),
            )
        ...
        self.node.result = json.dumps({"action": action_name, "repeats": self.node.db["action_repeats"]}, indent=2)
