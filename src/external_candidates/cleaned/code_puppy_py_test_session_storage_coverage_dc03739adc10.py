# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\test_session_storage_coverage_dc03739adc10.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_session_storage_coverage.py
...
class TestRestoreAutosaveMetadataReading:
    ...
    async def test_handles_corrupted_metadata_json(self, tmp_path):
        ...
        (tmp_path / "corrupted_meta.json").write_text("not valid json{{{", encoding="utf-8")
    ...
    async def test_handles_metadata_missing_fields(self, tmp_path):
        ...
        (tmp_path / "partial_meta.json").write_text(json.dumps({"session_name": "partial"}), encoding="utf-8")
...
class TestRestoreAutosaveTimestampSorting:
    ...
    async def test_sorts_sessions_by_timestamp_descending(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_return="", capture_system=displayed_sessions):
...
class TestRestoreAutosavePagination:
    ...
    async def test_pagination_with_more_than_five_sessions(self, tmp_path):
        ...
        for i in range(8):
            ...
            (tmp_path / f"session_{i}_meta.json").write_text(
                json.dumps({"timestamp": f"2024-01-0{i + 1}T00:00:00", "message_count": i}),
                encoding="utf-8",
            )

        displayed_messages = []

        async with mock_interactive_imports(mock_input_return="", capture_system=displayed_messages):
            await restore_autosave_interactively(tmp_path)
    ...
    async def test_page_navigation_cycles_through_pages(self, tmp_path):
        ...
        for i in range(8):
            ...
            (tmp_path / f"session_{i}_meta.json").write_text(
                json.dumps({"timestamp": f"2024-01-{i + 1:02d}T00:00:00", "message_count": i}),
                encoding="utf-8",
            )
...
class TestRestoreAutosaveUserSelection:
    ...
    async def test_numeric_selection_loads_session(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_return="1", mock_agent=mock_agent):
            await restore_autosave_interactively(tmp_path)
    ...
    async def test_direct_name_selection(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_return="my_specific_session", mock_agent=mock_agent):
            await restore_autosave_interactively(tmp_path)
    ...
    async def test_invalid_numeric_selection_warns_user(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_side_effect=input_sequence, capture_warning=warnings):
            await restore_autosave_interactively(tmp_path)
...
class TestRestoreAutosaveErrorHandling:
    ...
    async def test_keyboard_interrupt_cancels(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_side_effect=KeyboardInterrupt(), capture_warning=warnings):
            result = await restore_autosave_interactively(tmp_path)
    ...
    async def test_eof_error_cancels(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_side_effect=EOFError(), capture_warning=warnings):
            result = await restore_autosave_interactively(tmp_path)
    ...
    async def test_file_not_found_on_load(self, tmp_path):
        ...
        async with mock_interactive_imports(
            ...
            mock_load_session=MagicMock(side_effect=FileNotFoundError("Session file deleted")),
        ):
            result = await restore_autosave_interactively(tmp_path)
...
class TestRestoreAutosaveLastPageBehavior:
    ...
    async def test_last_page_shows_return_to_first(self, tmp_path):
        ...
        async with mock_interactive_imports(mock_input_side_effect=input_sequence, capture_system=displayed):
            await restore_autosave_interactively(tmp_path)
