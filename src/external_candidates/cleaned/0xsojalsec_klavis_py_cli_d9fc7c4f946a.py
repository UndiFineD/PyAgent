# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\open_strata.py\src.py\strata.py\cli_d9fc7c4f946a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\src\strata\cli.py
    ...
    if args.env:
        for env_var in args.env:
            if "=" not in env_var:
                ...
                print("Environment variables must be in KEY=VALUE format", file=sys.stderr)
...
def run_command(args):
    ...
    if args.port is not None:
        ...
        return run_server(args.port, json_response=False)
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(prog="strata", description="Strata MCP Router - Manage and run MCP servers")
    ...
    add_parser.add_argument("--env", action="append", help="Environment variables in KEY=VALUE format")
    ...
    add_parser.add_argument("url_or_command", help="URL for HTTP/SSE or command for stdio")
    ...
    parser.set_defaults(command="run", func=run_command, port=None, log_level="INFO", no_banner=False)
