# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\actalker.py\src.py\utils.py\mediapipe.py\torch_pose_util_7d8fc7ec4a37.py
...
def project_points(points_3d, transformation_matrix, pose_vectors, image_shape):
    ...
    for i in range(L):
        ...
        transformation = torch.matmul(
            ...
            euler_and_translation_to_matrix(pose_vectors[i][:3], pose_vectors[i][3:]).to(transformation_matrix.dtype),
        )
...
def invert_projection(projected_points, transformation_matrix, pose_vectors, image_shape):
    ...
    for i in range(L):
        ...
        transformation_inv = torch.inverse(
            torch.matmul(
                transformation_matrix,
                euler_and_translation_to_matrix(pose_vectors[i][:3], pose_vectors[i][3:]).to(torch.float32),
            )
        )
...
def euler_angles_to_matrix(euler_angles, convention):
    ...
    R_x = torch.tensor([[1, 0, 0], [0, cos_x, -sin_x], [0, sin_x, cos_x]], dtype=dtype, device=device)

    R_y = torch.tensor([[cos_y, 0, sin_y], [0, 1, 0], [-sin_y, 0, cos_y]], dtype=dtype, device=device)

    R_z = torch.tensor([[cos_z, -sin_z, 0], [sin_z, cos_z, 0], [0, 0, 1]], dtype=dtype, device=device)
