# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\env.py\tools.py\agent.py\insert_item.py\client_b344a626cab0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\agent\insert_item\client.py

from time import sleep

from typing import Union

from fle.env.entities import BeltGroup, Entity, EntityGroup, PipeGroup, Position
...
class InsertItem(Tool):
    ...
    def __call__(self, entity: Prototype, target: Union[Entity, EntityGroup], quantity=5) -> Entity:
        ...
        assert isinstance(target, Entity) or isinstance(target, EntityGroup), (
            "The second argument must be an Entity or EntityGroup, you passed in a {0}".format(type(target))
        )
        ...
        if isinstance(target, BeltGroup):
            ...
            pos = (
                ...
                else (target.outputs[0].position if len(target.outputs) > 0 else (None, None))
            )
