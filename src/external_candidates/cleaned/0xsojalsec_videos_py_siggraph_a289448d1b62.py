# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2021.py\siggraph_a289448d1b62.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\siggraph.py
...
class ThreePis(Scene):
    def construct(self):
        ...
        for pi, word, mode in zip(pis, words, modes):
            bubble = pi.get_bubble(word, bubble_type=SpeechBubble, height=3, width=3, direction=RIGHT)
...
class InterpolatingOrientations(ThreeDScene):
    ...
    def construct(self):
        ...
        logo = always_redraw(lambda: logo_template.copy().apply_matrix(get_matrix()).sort(lambda p: -p[1]))
        ...
        if self.matrix:
            ...
            bases.add_updater(lambda m: m.set_points(rotation_matrix_from_quaternion(quat_tracker.get_value()).T))
        ...
        self.play(ShowCreation(axes), frame.animate.reorient(-20, 60), run_time=3)
