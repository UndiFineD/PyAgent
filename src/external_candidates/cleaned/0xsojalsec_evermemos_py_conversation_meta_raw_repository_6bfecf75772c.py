# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\conversation_meta_raw_repository_6bfecf75772c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\repository\conversation_meta_raw_repository.py
...
class ConversationMetaRawRepository(BaseRepository[ConversationMeta]):
    ...
    def _validate_scene(self, scene: str) -> bool:
        ...
        if scene not in ALLOWED_SCENES:
            logger.warning("❌ Invalid scene value: %s, allowed values: %s", scene, ALLOWED_SCENES)
    ...
    async def get_by_group_id(
        ...
        try:
            conversation_meta = await self.model.find_one({"group_id": group_id}, session=session)
            ...
            logger.error("❌ Failed to retrieve conversation metadata by group_id: %s", e)
    ...
    async def update_by_group_id(
        ...
        try:
            ...
            if "scene" in update_data and not self._validate_scene(update_data["scene"]):
                logger.error(
    ...
    async def upsert_by_group_id(
        ...
        try:
            ...
            if "scene" in conversation_data and not self._validate_scene(conversation_data["scene"]):
                logger.error(
            ...
            existing_doc = await self.model.find_one({"group_id": group_id}, session=session)
    ...
    async def delete_by_group_id(self, group_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        """
        ...
        try:
            result = await self.model.find_one({"group_id": group_id}, session=session).delete()
