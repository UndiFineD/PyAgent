# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\agents.py\agent_50ce8934e203.py
...
class Agent(BaseModule):
    ...
    def get_agent_profile(self, action_names: List[str] = None) -> str:
        ...
        if action_names is None:
            ...
            action_descriptions = "\n".join(
                [f"  - {action.name}: {action.description}" for action in all_actions if action.name in action_names]
            )
