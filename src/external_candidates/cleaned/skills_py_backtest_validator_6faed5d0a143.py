# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\scripts.py\backtest_validator_6faed5d0a143.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\backtest_validator.py
...
class BacktestValidator:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized BacktestValidator (lookback: {lookback_days}d, "
            f"min_trades: {min_trades}, required_wr: {required_win_rate:.0%})"
        )

    def validate_strategy(self, historical_data: pd.DataFrame, signal_generator, current_signal: Dict) -> Dict:
        """
    ...
    def _run_backtest(self, data: pd.DataFrame, signal_generator) -> Optional[BacktestMetrics]:
        """
        ...
        try:
            ...
            for i in range(window_size, len(data)):
                ...
                if action in ["LONG", "SHORT"] and current_position is None:
                    ...
                    current_position["highest_price"] = max(current_position["highest_price"], data.iloc[i]["high"])

                    current_position["lowest_price"] = min(current_position["lowest_price"], data.iloc[i]["low"])

                    # Check exit conditions

                    should_exit, exit_price = self._check_exit_conditions(current_position, data.iloc[i])

                    if should_exit:
                        # Close position

                        trade_result = self._create_trade_result(current_position, exit_price, current_time)
            ...
            if current_position is not None:
                trade_result = self._create_trade_result(
                    current_position, data.iloc[-1]["close"], data.iloc[-1]["timestamp"]
                )
    ...
    def _check_exit_conditions(self, position: Dict, current_candle: pd.Series) -> Tuple[bool, float]:
        """
    ...
    def _create_trade_result(self, position: Dict, exit_price: float, exit_time: datetime) -> TradeResult:
        """
    ...
    def _validate_metrics(self, metrics: BacktestMetrics, current_signal: Dict) -> Dict:
        """
        ...
        confidence_adjustment = self._calculate_confidence_adjustment(metrics, validation_checks, current_signal)
    ...
    def _calculate_confidence_adjustment(
        self, metrics: BacktestMetrics, checks: List[Dict], current_signal: Dict
    ) -> float:
        """
    ...
    def _generate_recommendation(self, metrics: BacktestMetrics, validated: bool) -> str:
        """
