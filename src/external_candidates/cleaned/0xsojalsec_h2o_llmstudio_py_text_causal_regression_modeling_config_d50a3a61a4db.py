# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\python_configs.py\text_causal_regression_modeling_config_d50a3a61a4db.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\python_configs\text_causal_regression_modeling_config.py
...
class ConfigNLPCausalRegressionEnvironment(ConfigNLPCausalLMEnvironment):
    ...
    _summary_card_template: str = "text_causal_regression_experiment_summary_card_template.md"
...
class ConfigProblemBase(DefaultConfigProblemBase):
    ...
    dataset: ConfigNLPCausalRegressionDataset = field(default_factory=ConfigNLPCausalRegressionDataset)

    tokenizer: ConfigNLPCausalRegressionTokenizer = field(default_factory=ConfigNLPCausalRegressionTokenizer)

    architecture: ConfigNLPCausalRegressionArchitecture = field(default_factory=ConfigNLPCausalRegressionArchitecture)

    training: ConfigNLPCausalRegressionTraining = field(default_factory=ConfigNLPCausalRegressionTraining)

    augmentation: ConfigNLPCausalRegressionAugmentation = field(default_factory=ConfigNLPCausalRegressionAugmentation)

    prediction: ConfigNLPCausalRegressionPrediction = field(default_factory=ConfigNLPCausalRegressionPrediction)

    environment: ConfigNLPCausalRegressionEnvironment = field(default_factory=ConfigNLPCausalRegressionEnvironment)

    logging: ConfigNLPCausalRegressionLogging = field(default_factory=ConfigNLPCausalRegressionLogging)
    ...
    def check(self) -> dict[str, list]:
        ...
        if self.dataset.parent_id_column not in ["None", None]:
            ...
            errors["message"] += ["Parent ID column is not supported for regression datasets."]
