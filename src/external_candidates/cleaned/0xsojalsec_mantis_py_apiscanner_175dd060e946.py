# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_mantis.py\mantis.py\tool_base_classes.py\apiscanner_175dd060e946.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\tool_base_classes\apiScanner.py
    ...
    async def execute(self, tool_tuple):
        ...
        try:
            with futures.ThreadPoolExecutor(max_workers=1) as executor:
                responses = [
                    executor.submit(BaseRequestExecutor.sendRequest, method, api_call_tuple)
                    for api_call_tuple in self.asset_api_list
                ]
