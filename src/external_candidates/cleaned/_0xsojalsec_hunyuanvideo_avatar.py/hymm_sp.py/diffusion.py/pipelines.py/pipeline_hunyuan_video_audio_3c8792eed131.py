# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\diffusion\pipelines\pipeline_hunyuan_video_audio.py
...
class HunyuanVideoAudioPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
            ...
            if pixel_value_llava is not None:
                ...
                text_inputs["attention_mask"] = torch.cat(
                    [
                        ...
                        torch.ones((1, 575 * len(pixel_value_llava))).to(text_inputs["attention_mask"]),
                    ],
                    dim=1,
                )
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if uncond_pixel_value_llava is not None:
                ...
                uncond_input["attention_mask"] = torch.cat(
                    [
                        uncond_input["attention_mask"],
                        torch.ones((1, 575 * len(uncond_pixel_value_llava))).to(uncond_input["attention_mask"]),
                    ],
                    dim=1,
                )
    ...
    def encode_prompt_audio_text_base(
        ...
        if prompt_embeds is None:
            ...
            )  # data_type: video, text_inputs: {'input_ids', 'attention_mask'}
            ...
            if pixel_value_llava is not None:
                ...
                text_inputs["attention_mask"] = torch.cat(
                    [
                        text_inputs["attention_mask"],
                        torch.ones((1, 575)).to(text_inputs["attention_mask"]),
                    ],
                    dim=1,
                )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        prompt_embeds, negative_prompt_embeds, prompt_mask, negative_prompt_mask = self.encode_prompt_audio_text_base(
            prompt=prompt,
            ...
            for i, t in enumerate(timesteps):
                ...
                for index_start in range(0, infer_length, frames_per_batch):
                    ...
                    idx_list_audio = [
                        ...
                        for ii in range(index_start * 4, (index_start + frames_per_batch) * 4 - 3)
                    ]
                    ...
                    with torch.autocast(device_type="cuda", dtype=target_dtype, enabled=autocast_enabled):
                        ...
                        if i in no_cache_steps:
                            ...
                            if latent_model_input.shape[-1] * latent_model_input.shape[-2] > 64 * 112 and cpu_offload:
                                if i == 0:
                                    print(
                                        f"cpu_offload={cpu_offload} and {latent_model_input.shape[-2:]} is large, split infer noise-pred"
                                    )
                                ...
                                noise_pred_uncond = self.transformer(
                                    ...
                                    text_states=prompt_embeds_input[:1],
                                    text_mask=prompt_mask_input[:1],
                                    text_states_2=prompt_embeds_2_input[:1],
                                ...
                                self.transformer.cache_out = torch.cat(
                                    [uncond_cache_tensor, self.transformer.cache_out],
                                    dim=0,
                                )
                                ...
                                noise_pred = self.transformer(
                                    ...
                                    text_states=prompt_embeds_input,
                                    text_mask=prompt_mask_input,
                                    text_states_2=prompt_embeds_2_input,
                                    freqs_cos=freqs_cis[0],
                                    freqs_sin=freqs_cis[1],
                                    guidance=guidance_expand,
                                    return_dict=True,
                                ...
                                self.cache_tensor = {
                                    "ref": torch.zeros(
                                        [
                                            ...
                                            latents_all.shape[-3],
                                            (latent_model_input.shape[-1] // 2) * (latent_model_input.shape[-2] // 2),
                                            3072,
                                        ]
                                    )
                                    .to(self.transformer.cache_out.dtype)
                            ...
                            self.cache_tensor["ref"][:, idx_list] = (
                                self.transformer.cache_out[:, : img_ref_len - img_len]
                                .reshape(latent_model_input.shape[0], 1, -1, 3072)
                                .repeat(1, len(idx_list), 1, 1)
                            )
                            self.cache_tensor["img"][:, idx_list] = self.transformer.cache_out[
                                :, img_ref_len - img_len : img_ref_len
                            ].reshape(latent_model_input.shape[0], len(idx_list), -1, 3072)
                            self.cache_tensor["txt"][:, idx_list] = (
                                self.transformer.cache_out[:, img_ref_len:].unsqueeze(1).repeat(1, len(idx_list), 1, 1)
                            )

                        else:
                            is_cache = True
                            # self.transformer.cache_out[:, :img_ref_len-img_len] = self.cache_tensor["ref"][:, idx_list].mean(1)
                            self.transformer.cache_out[:, : img_ref_len - img_len] = self.cache_tensor["ref"][
                                :, idx_list
                            ][:, 0].clone()
                            self.transformer.cache_out[:, img_ref_len - img_len : img_ref_len] = (
                                self.cache_tensor["img"][:, idx_list].reshape(-1, img_len, 3072).clone()
                            )
                            self.transformer.cache_out[:, img_ref_len:] = self.cache_tensor["txt"][:, idx_list][
                                :, 0
                            ].clone()
