# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\sample_inference_audio.py
import math
import random
import time
...
class HunyuanVideoSampler(Inference):
    ...
    @torch.no_grad()
    def predict(self, args, batch, wav2vec, feature_extractor, align_instance, **kwargs):
        """
        ...
        audio_prompts = [
            encode_audio(
                wav2vec,
                audio_feat.to(dtype=wav2vec.dtype),
                fps.item(),
                num_frames=batch["audio_len"][0],
            )
        ...
        if audio_prompts.shape[1] <= 129:
            audio_prompts = torch.cat(
                [
                    audio_prompts,
                    torch.zeros_like(audio_prompts[:, :1]).repeat(1, 129 - audio_prompts.shape[1], 1, 1, 1),
                ],
                dim=1,
            )
        else:
        ...
        # concat_dict = {}
        freqs_cos, freqs_sin = self.get_rotary_pos_embed(target_length, target_height, target_width, concat_dict)
        ...
        pipeline_kwargs = {"cpu_offload": args.cpu_offload}
