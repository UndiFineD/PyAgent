# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\modules\models_audio.py
import os
...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # -------------------- motion-embeder --------------------
        self.motion_exp = TimestepEmbedder(self.hidden_size // 4, get_activation_layer("silu"), **factory_kwargs)
        self.motion_pose = TimestepEmbedder(self.hidden_size // 4, get_activation_layer("silu"), **factory_kwargs)

        self.fps_proj = TimestepEmbedder(self.hidden_size, get_activation_layer("silu"), **factory_kwargs)
        ...
        self.single_stream_map = {
            str(i): j + len(self.double_stream_list) for j, i in enumerate(self.single_stream_list)
        }

        self.audio_adapter_blocks = nn.ModuleList(
            [
                PerceiverAttentionCA(dim=3072, dim_head=1024, heads=33)
                for _ in range(len(self.double_stream_list) + len(self.single_stream_list))
            ]
        )
    ...
    def forward(
        ...
        **additional_kwargs,
        ...
        face_mask = torch.nn.functional.interpolate(
            face_mask, size=[ot, shape_mask[-2], shape_mask[-1]], mode="nearest"
        )
        ...
        if not is_cache:
            for layer_num, block in enumerate(self.double_blocks):
                ...
                if layer_num in self.double_stream_list:
                    ...
                    audio_feature_all_insert = torch.cat([audio_feature_pad, audio_feature_all], dim=1).view(
                        bsz, ot, 16, 3072
                    )

                    double_idx = self.double_stream_map[str(layer_num)]
                    real_img = self.audio_adapter_blocks[double_idx](audio_feature_all_insert, real_img).view(
                        bsz, -1, 3072
                    )
                    ...
                    if get_sequence_parallel_state():
                        sp_size = nccl_info.sp_size
                        sp_rank = nccl_info.rank_within_group
                        assert img.shape[1] % sp_size == 0, (
                            f"Cannot split video sequence into ulysses SP ({sp_size}) parts evenly"
                        )
                        img = torch.chunk(img, sp_size, dim=1)[sp_rank]
