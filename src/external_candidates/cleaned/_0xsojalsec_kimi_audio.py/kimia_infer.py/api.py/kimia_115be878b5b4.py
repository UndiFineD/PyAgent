# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Kimi-Audio\kimia_infer\api\kimia.py
import os
...
class KimiAudio(object):
    def __init__(self, model_path: str, load_detokenizer: bool = True):
        ...
        self.alm = AutoModelForCausalLM.from_pretrained(cache_path, torch_dtype=torch.bfloat16, trust_remote_code=True)
    ...
    def _generate_loop(
        ...
        for i in tqdm.tqdm(range(max_new_tokens), desc="Generating tokens", disable=False):
            ...
            if output_type == "text" and text_stream_is_finished or output_type == "both" and audio_stream_is_finished:
                ...
                decoder_position_ids = (
                    torch.zeros(1, 1, device=torch.cuda.current_device()).fill_(last_position_id + 1).long().view(1, 1)
                )
                last_position_id += 1
