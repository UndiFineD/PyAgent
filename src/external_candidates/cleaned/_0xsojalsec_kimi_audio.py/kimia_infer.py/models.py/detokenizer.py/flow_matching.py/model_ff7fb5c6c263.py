# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Kimi-Audio\kimia_infer\models\detokenizer\flow_matching\model.py
import math
...
class TimestepEmbedder(nn.Module):
    ...
    def timestep_embedding(t, dim, max_period=10000):
        ...
        half = dim // 2
        freqs = (
            torch.exp(-math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32) / half)
            .float()
            .to(device=t.device)
        )
