# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Kimi-Audio\finetune_codes\datasets.py
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        ) = tokenized_conversation.to_tensor()

        audio_features = tokenized_conversation.continuous_feature

        audio_labels = torch.cat(
            (audio_input_ids[:, 1:], audio_input_ids.new_full((1, 1), self.pad_token)),
            dim=1,
        )
        text_labels = torch.cat(
            (text_input_ids[:, 1:], text_input_ids.new_full((1, 1), self.pad_token)),
            dim=1,
        )
        audio_loss_mask = torch.cat(
            (
                audio_token_loss_mask[:, 1:],
                audio_token_loss_mask.new_full((1, 1), False),
            ),
            dim=1,
        )
        text_loss_mask = torch.cat(
            (text_token_loss_mask[:, 1:], text_token_loss_mask.new_full((1, 1), False)),
            dim=1,
        )
