# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Kimi-Audio\finetune_codes\model.py
import argparse
import os
import shutil
...
class KimiAudioModel(MoonshotKimiaForCausalLM):
    ...
    def export_model(input_dir, output_dir):
        ...
        shutil.copyfile(
            "finetune_codes/configuration_moonshot_kimia.py",
            os.path.join(output_dir, "configuration_moonshot_kimia.py"),
        )
        shutil.copyfile(
            "finetune_codes/modeling_kimia.py",
    ...
    def forward(
        ...
        whisper_input_feats = (
            torch.from_numpy(whisper_input_feature[0]).unsqueeze(0)[:, :].to(torch.cuda.current_device())
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action",
        type=str,
        choices=["init_from_pretrained", "export_model"],
