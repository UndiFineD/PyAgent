# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\seanrea.py\recgov_availability.py\scripts.py\check_5ad40f0f36a1.py
...
HEADERS = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) recgov-checker/1.0"}
        ...
        try:
            ...
            for site_id, site_data in data.get("campsites", {}).items():
                if site_id not in all_campsites:
                    ...
                    all_campsites[site_id]["availabilities"].update(site_data.get("availabilities", {}))
...
def check_consecutive_availability(
    ...
    while check_date + timedelta(days=nights) <= end_date + timedelta(days=30):  # Look ahead
        dates_to_check = [(check_date + timedelta(days=i)).strftime("%Y-%m-%dT00:00:00Z") for i in range(nights)]

        all_available = all(availabilities.get(d) == STATUS_AVAILABLE for d in dates_to_check)
...
def analyze_campground_status(campsites: dict, start_date: datetime, nights: int) -> dict:
    ...
    dates_to_check = [(start_date + timedelta(days=i)).strftime("%Y-%m-%dT00:00:00Z") for i in range(nights)]
...
def main():
    parser = argparse.ArgumentParser(description="Check campsite availability on recreation.gov")

    parser.add_argument("--campground", "-c", nargs="+", required=True, help="Campground ID(s) to check")
    ...
    parser.add_argument("--electric", action="store_true", help="Filter to electric sites only")

    parser.add_argument("--nonelectric", action="store_true", help="Filter to non-electric sites only")
    ...
    parser.add_argument("--show-sites", action="store_true", help="Show individual site details")
    ...
    for campground_id in args.campground:
        ...
        if not args.include_group:
            campsites = {site_id: site_data for site_id, site_data in campsites.items() if not is_group_site(site_data)}
