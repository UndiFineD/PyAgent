# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\integration\test_integration.py
import json
import os
import sys
...
def run_oasst(tmp_path, config_name, metric):
    ...
    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), f"{config_name}.yaml"),
    ) as fp:
    ...
    llm_studio_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../llm_studio/"))
    cmd = [
        f"{sys.executable}",
    ...
    assert os.path.exists(os.path.join(cfg["output_directory"], "validation_predictions.csv"))
