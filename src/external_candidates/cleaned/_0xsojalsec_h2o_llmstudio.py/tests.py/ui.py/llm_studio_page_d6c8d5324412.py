# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\ui\llm_studio_page.py
import os
...
class LLMStudioPage(BasePage):
    ...
    def import_dataset_from_aws(self, bucket: str, access_key: str, secret_key: str, dataset_name: str):
    ...
    def import_dataset_from_azure(self, connection: str, container: str, dataset_name: str):
    ...
    def import_dataset_from_kaggle(self, kaggle_command: str, username: str, secret: str):
    ...
    def mixed_precision(self, value: bool):
        old_toggle_value = self.get_by_test_id(self.MIXED_PRECISION).get_attribute("aria-checked")
    ...
    def view_experiment(self, experiment_name: str):
        ...
        status = self.page.locator(f"{self.EXPERIMENT_STATUS_SELECTOR} >> nth={i}").inner_text()
        while True:
    ...
    def find_experiment_index(self, experiment_name):
        ...
        while index < 100:  # number of experiments
            ...
            inner_text = self.page.locator(f"{self.EXPERIMENT_INDEX_SELECTOR} >> nth={index}").inner_text()
