# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\src\test_possible_values.py
import os
...
def test_strip_common_prefix():
    ...
    stripped_with_ignore = strip_common_prefix(paths_with_ignore, ignore_set={"ignore_this"})
...
def test_string_tuple_of_strings():
    s = String(values=("a", "b", "c"), allow_custom=True, placeholder="Select an option")
...
class TestDatasetValue:
    ...
    def test_compute_current_values_basic(self, current_values, possible_values, prefer_with, expected):
        result = DatasetValue._compute_current_values(current_values, possible_values, prefer_with)
    ...
    def test_compute_current_values_prefer_function_no_match(self):
        ...
        result = DatasetValue._compute_current_values(current_values, possible_values, prefer_with)
        assert result == ["a"]  # Should return first possible value when no preference matches
    ...
    def test_compute_current_values_all_filtered_out(self):
        ...
        assert result == ["a"]  # Should return first possible value when all current values are filtered out
...
def mock_dataset(temp_dir_structure):
    return {
        ...
        "dataframe": pd.DataFrame({"col1": [1, 2, 3], "col2": ["a", "b", "c"], "col3": [True, False, True]}),
    }


@pytest.mark.parametrize("filename", ["file1.csv", "file2.json", "subdir1/file3.parquet", "non-existant"])
