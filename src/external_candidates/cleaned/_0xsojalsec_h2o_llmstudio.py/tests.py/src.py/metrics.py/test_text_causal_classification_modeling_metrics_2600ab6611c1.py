# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\src\metrics\test_text_causal_classification_modeling_metrics.py
...
def test_auc_score_multiclass_classification(mock_val_df):
    ...
    expected_score = roc_auc_score(np.eye(3)[[1, 0, 2, 2]], np.array(results["logits"]), multi_class="ovr")
...
def test_auc_score_different_number_of_classes(mock_val_df):
    ...
    expected_score = roc_auc_score(np.eye(4)[[1, 0, 2, 3]], np.array(results["logits"]), multi_class="ovr")
    assert np.allclose(score, expected_score)
...
def test_logloss_score_binary_classification(mock_val_df):
    ...
    results = {
        "probabilities": softmax([[0.1, 0.9], [0.8, 0.2], [0.3, 0.7], [0.9, 0.1]], axis=1),
        "target_text": ["1", "0", "1", "0"],
    }
...
def test_logloss_score_multiclass_classification(mock_val_df):
    ...
    cfg.dataset.answer_column = ["label"]
    results = {
        "probabilities": softmax([[0.1, 0.8, 0.1], [0.7, 0.2, 0.1], [0.1, 0.1, 0.8], [0.3, 0.3, 0.4]], axis=1),
        "target_text": ["1", "0", "2", "2"],
    }
