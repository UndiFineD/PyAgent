# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\src\datasets\test_text_causal_language_modeling_ds.py
import re
...
def test_prepare_default_dataset(tmp_path):
    ...
    assert set(df.keys()) == set(["instruction", "output", "id", "parent_id", "lang", "rank"])
...
def test_clean_output():
    ...
    predicted_text_clean = CustomDataset.clean_output(output=output, cfg=cfg)["predicted_text"]
...
def test_getitem():
    ...
    assert (
        dataset.tokenizer.decode(result["input_ids"], skip_special_tokens=True) == "System:system 1"
        "Prompt:prompt 1"
        "Answer:answer 1"
        "Prompt:prompt 2"
        "Answer:answer 2"
    ...
    assert (
        dataset.tokenizer.decode(result["prompt_input_ids"], skip_special_tokens=True) == "System:system 1"
        "Prompt:prompt 1"
        "Answer:answer 1"
        "Prompt:prompt 2"
        "Answer:answer 2"
...
def test_getitem_no_chaining():
    ...
    for i in range(3):
        ...
        assert (
            dataset.tokenizer.decode(result["input_ids"], skip_special_tokens=True) == f"System:system {i + 1}"
            f"Prompt:prompt {i + 1}"
            f"Answer:answer {i + 1}"
        )

        assert (
            dataset.tokenizer.decode(result["prompt_input_ids"], skip_special_tokens=True) == f"System:system {i + 1}"
            f"Prompt:prompt {i + 1}"
            "Answer:"
        )
