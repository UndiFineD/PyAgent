# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\src\datasets\test_conversation_chain_handler.py
import time
...
def test_get_conversation_ids():
    ...
    conv_ids = ConversationChainHandler.get_conversation_ids({"id2": "id1", "id3": "id2", "id4": "id3"}, "id4")
    assert conv_ids == ["id1", "id2", "id3", "id4"]

    # test the get_conversation_ids method - circular case, should raise ValueError
    with pytest.raises(ValueError):
        ConversationChainHandler.get_conversation_ids({"id1": "id4", "id2": "id1", "id3": "id2", "id4": "id3"}, "id4")
...
def test_oasst_conversation_chain_handler(tmp_path):
    ...
    systems = ["", "", ""]
    sample = conversation_chain_handler[10719]  # 10719 == sample with 3 round conversation
...
def test_oasst_conversation_chain_handler_is_fast(tmp_path):
    ...
    for i in range(50):
        df = df_oasst.copy()
        df["parent_id"] = df["parent_id"].apply(lambda x: x + str(i) if x is not None else x)
