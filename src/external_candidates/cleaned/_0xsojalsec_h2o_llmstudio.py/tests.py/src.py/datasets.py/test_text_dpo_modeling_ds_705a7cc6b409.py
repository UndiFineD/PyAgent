# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\src\datasets\test_text_dpo_modeling_ds.py
import numpy as np
import pandas as pd
import pytest
import torch
...
def df_with_conversation_chain_ids():
    ...
    chosen_responses = [f"chosen_response {idx}" if int(idx) % 5 == 0 else f"response {idx}" for idx in ids]
    rejected_responses = [f"rejected_response {idx}" if int(idx) % 5 == 0 else f"response {idx}" for idx in ids]
    return pd.DataFrame(
        {
    ...
    for idx in range(200 // 5):
        for name, conversation_chain_handler in zip(
            ...
            expected = {
                ...
                "answers": [f"response {i + 1}" for i in range(idx * 5, (idx + 1) * 5 - 1)]
...
def test_dataset_prompt_ids_are_the_same_as_for_causal_language_modeling(
    ...
    for key in ["prompt_input_ids", "prompt_attention_mask"]:
        assert torch.all(generated_text_causal_lm[key] == generated_text[key]), f"{key} is not the same"
