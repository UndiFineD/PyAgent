# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\src\models\test_dpo_modeling_model.py
import random
...
def df2():
    ...
    rejected_answers = ["".join(random.choice(alphabet) for _ in range(10)) for _ in range(10)]
...
def test_dpo_perplexity_metric(tmp_path, df2):
    ...
    assert log_dict["chosen_cross_entropy_loss"] == log_dict["rejected_cross_entropy_loss"]
