# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\tests\app_utils\sections\test_histogram_card.py
import random
...
def test_quantiles_are_computed_correctly() -> None:
    for _ in range(5):
        ...
        first = df_quantile[df_quantile["data_type"] == f"first {int(a * 100)}% quantile"]
        last = df_quantile[df_quantile["data_type"] == f"last {100 - int(b * 100)}% quantile"]
        sorted_data = sorted(data)
        # use -1 and +1 to account for rounding errors
        expected_first_quantile_range = sorted_data[int(len(sorted_data) * a) - 1 : int(len(sorted_data) * a) + 1]
