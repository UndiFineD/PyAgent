# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\python_configs\cfg_checks.py
import logging
import os
...
def check_for_common_errors(cfg: DefaultConfigProblemBase) -> dict:
    ...
    if not len(cfg.environment.gpus) > 0:
        ...
        errors["message"] += ["Please select at least one GPU to start the experiment! "]
    ...
    if cfg.architecture.backbone_dtype in ["int4", "int8"] and not cfg.architecture.pretrained:
    ...
    if cfg.environment.use_deepspeed and len(cfg.environment.gpus) < 2:
        ...
        errors["message"] += [
            "Deepspeed does not support single GPU training. Please select more than one GPU or disable deepspeed."
        ]
        errors["type"].append("error")
    return errors
