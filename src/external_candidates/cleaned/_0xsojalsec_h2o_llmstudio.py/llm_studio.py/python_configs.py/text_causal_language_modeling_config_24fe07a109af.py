# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\python_configs\text_causal_language_modeling_config.py
import logging
...
class ConfigNLPCausalLMDataset(DefaultConfig):
    dataset_class: Any = llm_studio.src.datasets.text_causal_language_modeling_ds.CustomDataset
    ...
    def __post_init__(self):
        ...
        self._possible_values["train_dataframe"] = possible_values.Files(prefer_with=lambda path: "train" in path)
        ...
        self._possible_values["prompt_column"] = possible_values.Columns(
            prefer_with=lambda column: column in ("instruction", "prompt", "question", "input", "user")
        )
        self._possible_values["answer_column"] = possible_values.Columns(
            prefer_with=lambda column: column in ("answer", "output", "response", "assistant", "chosen")
        )
...
class ConfigNLPCausalLMTraining(DefaultConfig):
    ...
    def __post_init__(self):
        ...
        self._possible_values["learning_rate"] = possible_values.Number(step=1e-9, min=1e-9)
        self._possible_values["differential_learning_rate_layers"] = possible_values.String(
            values=("backbone", "embed", "head"),
            allow_custom=True,
            placeholder="Select optional layers...",
        )
        self._possible_values["differential_learning_rate"] = self._possible_values["learning_rate"]
        ...
        self._grid_search_values["loss_function"] = self._possible_values["loss_function"]
        ...
        self._nesting.add(
            ...
            [Dependency(key="differential_learning_rate_layers", value=None, is_set=False)],
        )
        self._nesting.add(
            ["freeze_layers"],
...
class ConfigNLPCausalLMEnvironment(DefaultConfig):
    ...
    _summary_card_template: str = "text_causal_language_modeling_experiment_summary_card_template.md"

    def __post_init__(self):
        super().__post_init__()
        self._possible_values["gpus"] = possible_values.String(
            values=tuple([(str(x), f"GPU #{x + 1}") for x in range(torch.cuda.device_count())]),
            allow_custom=False,
        )
        ...
        self._possible_values["deepspeed_allgather_bucket_size"] = possible_values.Number(step=1, min=1e6)
        self._possible_values["deepspeed_reduce_bucket_size"] = possible_values.Number(step=1, min=1e6)
        self._possible_values["deepspeed_stage3_prefetch_bucket_size"] = possible_values.Number(step=1, min=1e6)
        self._possible_values["deepspeed_stage3_param_persistence_threshold"] = possible_values.Number(step=1, min=1e6)
        self._possible_values["deepspeed_stage3_max_live_parameters"] = possible_values.Number(step=1, min=1e6)
        self._possible_values["deepspeed_stage3_max_reuse_distance"] = possible_values.Number(step=1, min=1e6)
...
class ConfigProblemBase(DefaultConfigProblemBase):
    ...
    tokenizer: ConfigNLPCausalLMTokenizer = field(default_factory=ConfigNLPCausalLMTokenizer)
    architecture: ConfigNLPCausalLMArchitecture = field(default_factory=ConfigNLPCausalLMArchitecture)
    training: ConfigNLPCausalLMTraining = field(default_factory=ConfigNLPCausalLMTraining)
    augmentation: ConfigNLPAugmentation = field(default_factory=ConfigNLPAugmentation)
    prediction: ConfigNLPCausalLMPrediction = field(default_factory=ConfigNLPCausalLMPrediction)
    environment: ConfigNLPCausalLMEnvironment = field(default_factory=ConfigNLPCausalLMEnvironment)
    ...
    def check(self) -> dict[str, list]:
        ...
        if self.prediction.temperature > 0 and not self.prediction.do_sample:
            ...
            errors["message"] += ["Please enable do sample if you want to use temperature > 0."]
