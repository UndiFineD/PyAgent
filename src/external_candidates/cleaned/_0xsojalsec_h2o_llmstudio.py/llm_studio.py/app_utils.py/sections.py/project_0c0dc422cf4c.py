# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\sections\project.py
import logging
import os
...
async def experiment_rename_action_workflow(q: Q):
    ...
    if new_name and new_name.strip():
        ...
        experiment_df["status"], experiment_df["info"] = get_experiments_status(experiment_df)
...
async def current_experiment_list_delete(q: Q) -> None:
    ...
    q.page["dataset/display/footer"].items = [
        ui.inline(
            items=[
                ui.button(name="experiment/delete/dialog", label=delete_label, primary=True),
                ui.button(name="experiment/list/current/noreset", label="Abort"),
            ]
        )
    ]
