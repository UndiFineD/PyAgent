# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\sections\home.py
import psutil
import torch
...
async def home(q: Q) -> None:
    ...
    if len(experiments) > 0:
        ...
        num_failed_stopped = len(experiments[experiments["status"].isin(["failed", "stopped"])])
        ...
        q.page["home/experiments_stats"] = ui.form_card(
            ...
            items=[
                ui.visualization(
                    plot=ui.plot([ui.mark(type="interval", x="=status", y="=count", y_min=0)]),
                    data=data(
                        fields="status count",
                        rows=[
        ...
        q.page["home/gpu_stats"] = ui.form_card(
            ...
            items=[
                ui.expander(
                    ...
                    items=get_single_gpu_usage(highlight=wave_theme.get_primary_color(q)),
                    expanded=True,
                )
            ],
        )
    ...
    q.page["dataset/list"] = ui.form_card(
        ...
        items=[
            ui.inline(
                [
                    ui.button(name="dataset/list", icon="Database", label="", primary=True),
    ...
    df_viz = df_viz[df_viz.columns.intersection(["name", "dataset", "problem type", "metric", "val metric"])]
