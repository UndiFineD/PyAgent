    ...
    if step == 1:  # select import data source
        ...
        items = [
            ...
            ui.dropdown(
                ...
                label="Source",
                value=("Upload" if q.client["dataset/import/source"] is None else q.client["dataset/import/source"]),
        ...
        if q.client["dataset/import/source"] == "S3":
            ...
            if q.client["dataset/import/huggingface_api_token"] is None:
                q.client["dataset/import/huggingface_api_token"] = q.client["default_huggingface_api_token"]
        ...
        if q.client["dataset/import/source"] != "Upload":
            buttons.insert(0, ui.button(name="dataset/import/2", label="Continue", primary=True))

        q.page["dataset/import/footer"] = ui.form_card(box="footer", items=[ui.inline(items=buttons, justify="start")])
        ...
        try:
            if not q.args["dataset/import/cfg_file"] and not edit:
                if q.client["dataset/import/source"] == "S3":
                    ...
                    (
                        q.client["dataset/import/path"],
                        q.client["dataset/import/name"],
                    ) = await h2o_drive_download(q, q.client["dataset/import/h2o_drive_filename"])
                elif q.client["dataset/import/source"] == "Kaggle":
                    (
                        q.client["dataset/import/path"],
                        q.client["dataset/import/name"],
            ...
            error = "Dataset import failed. Please make sure all required fields are filled correctly."
            await clean_dashboard(q, mode="full")
        ...
        try:
            if not q.args["dataset/import/cfg_file"] and not edit:
                q.client["dataset/import/name"] = get_unique_dataset_name(q, q.client["dataset/import/name"])
            q.page["dataset/import"] = ui.form_card(box="content", items=[])
            q.client.delete_cards.add("dataset/import")
            ...
            choices_problem_types = [ui.choice(name, label) for name, label in get_problem_types()]
                ...
                for c in choices_problem_types:
                    ...
                    if problem_type_name in q.client["dataset/import/name"]:
                        if len(problem_type_name) > max_substring_len:
                            ...
                            q.client["dataset/import/cfg_category"] = c.name.split("_")[0]
            ...
            if q.client["dataset/import/cfg_category"] not in q.client["dataset/import/cfg_file"]:
            ...
            if not edit:
                q.client["dataset/import/cfg"] = load_config_py(
                    config_path=(f"llm_studio/python_configs/{q.client['dataset/import/cfg_file']}"),
            ...
            if file_extension_is_compatible(q):
                ...
                problem_type = make_label(re.sub("_config.*", "", q.client["dataset/import/cfg_file"]))
                items += [ui.text(f"<b> The chosen file extensions is not compatible with {problem_type}.</b> ")]
                buttons = [
                    ui.button(name="dataset/list", label="Abort"),
                ]
        ...
        logger.info("Step 31")
        cfg = q.client["dataset/import/cfg"]
        cfg = parse_ui_elements(cfg=cfg, q=q, limit=default_cfg.dataset_keys, pre="dataset/import/cfg/")
        ...
        try:
            ...
            text = "Data Validity Check. Click <strong>Continue</strong> if the input data and labels appear correctly."
            ...
            logger.error(f"Error while plotting data preview: {exception}", exc_info=True)
            text = (
        ...
        if q.client["dataset/import/name"] == "":
            ...
            try:
                ...
                if os.path.exists(cfg.dataset.train_dataframe):
                    train_rows = read_dataframe_drop_missing_labels(cfg.dataset.train_dataframe, cfg).shape[0]
                validation_rows = None
                if os.path.exists(cfg.dataset.validation_dataframe):
                    validation_rows = read_dataframe_drop_missing_labels(cfg.dataset.validation_dataframe, cfg).shape[0]
...
async def dataset_merge(q: Q, step, error=""):
    if step == 1:  # Select which dataset to merge
        ...
        datasets_df = q.client.app_db.get_datasets_df()
        import_choices = [ui.choice(x["path"], x["name"]) for idx, x in datasets_df.iterrows()]
        ...
        if len(overlapping_files):
            warning = (
                f"Renamed {', '.join(rename_map.keys())} to {', '.join(rename_map.values())} due to duplicated entries."
            )
        else:
            warning = ""
...
async def dataset_list(q: Q, reset: bool = True) -> None:
    ...
    q.page["dataset/display/footer"] = ui.form_card(
        ...
        items=[
            ui.inline(
                items=[
                    ui.button(name="dataset/import", label="Import dataset", primary=True),
                    ui.button(
...
async def dataset_newexperiment(q: Q, dataset_id: int):
    ...
    q.client["experiment/start/cfg_file"] = dataset.config_file.split("/")[-1].replace(".yaml", "")
    q.client["experiment/start/cfg_category"] = q.client["experiment/start/cfg_file"].split("_")[0]
    q.client["experiment/start/dataset"] = str(dataset_id)

    await experiment_start(q)


async def dataset_edit(q: Q, dataset_id: int, error: str = "", warning: str = "", allow_merge: bool = True):
    ...
    q.client["dataset/import/cfg_file"] = dataset.config_file.split("/")[-1].replace(".yaml", "")
    q.client["dataset/import/cfg_category"] = q.client["dataset/import/cfg_file"].split("_")[0]
    ...
    await dataset_import(q=q, step=2, edit=True, error=error, warning=warning, allow_merge=allow_merge)
...
async def dataset_list_delete(q: Q):
    ...
    q.page["dataset/display/footer"].items = [
        ui.inline(
            items=[
                ui.button(name="dataset/delete/dialog", label="Delete datasets", primary=True),
...
async def dataset_display(q: Q) -> None:
    ...
    dataset_id = q.client["dataset/list/df_datasets"]["id"].iloc[q.client["dataset/display/id"]]
    ...
    if q.client["dataset/display/tab"] is None or q.args["dataset/display/data"] is not None:
    ...
    items: list[Tab] = [
        ...
        ui.tab(name="dataset/display/visualization", label="Sample Train Visualization"),
    ...
    if q.client["dataset/display/tab"] == "dataset/display/data":
        ...
        await show_statistics_tab(q, dataset_filename=dataset_filename, config_filename=config_filename)
...
async def show_summary_tab(q: Q, dataset_id):
    ...
    q.page["dataset/display/summary"] = ui.stat_list_card(box="first", items=stat_list_items, title="")
...
async def show_statistics_tab(q: Q, dataset_filename, config_filename):
    ...
    q.page["dataset/display/statistics/full_conversation_histogram"] = histogram_card(
        ...
        title="Text Length Distribution for complete conversations (split by whitespace)",
    ...
    if len(set(stats_dict["number_of_prompts"])) > 1:
        q.page["dataset/display/statistics/parent_id_length_histogram"] = histogram_card(
        ...
        q.client.delete_cards.add("dataset/display/statistics/parent_id_length_histogram")

    df_stats = stats_dict["df_stats"]
    if df_stats is None:
        component_items = [ui.text("Dataset does not contain numerical or text features. No statistics available.")]
    else:
...
def compute_dataset_statistics(dataset_path: str, cfg_path: str, cfg_hash: str) -> dict:
    ...
    stats_dict = {}
    for chat_type in ["prompts", "answers"]:
        text_lengths = [[len(text.split(" ")) for text in conversation[chat_type]] for conversation in conversations]
    ...
    stats_dict["complete_conversations"] = [len(text.split(" ")) for text in input_texts]
    stats_dict["number_of_prompts"] = [len(conversation["prompts"]) for conversation in conversations]
...
async def dataset_delete_current_datasets(q: Q) -> None:
    dataset_ids = list(q.client["dataset/list/df_datasets"]["id"].iloc[list(map(int, q.client["dataset/list/table"]))])
