# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\sections\experiment.py
import asyncio
import glob
import itertools
import logging
...
async def experiment_start(q: Q) -> None:
    ...
    items = [
        ...
        ui.dropdown(
            ...
            choices=[ui.choice(str(row["id"]), str(row["name"])) for _, row in df_datasets.iterrows()],
            trigger=True,
            tooltip=tooltips["experiments_dataset"],
        ),
    ]

    if show_update_warnings and q.client["experiment/start/dataset_prev"] != q.client["experiment/start/dataset"]:
        items += [ui.message_bar(type="warning", text=warning_message.format("Dataset"))]
        show_update_warnings = False

    if (
        q.client["experiment/start/cfg_file"] is None
        or q.client["experiment/start/dataset_prev"] != q.client["experiment/start/dataset"]
        ...
        if dataset is not None:
            problem_type = dataset.config_file.replace(dataset.path + "/", "").replace(".yaml", "")
        else:
    ...
    choices_problem_categories = [ui.choice(name, label) for name, label in get_problem_categories()]
    ...
    if q.client["experiment/start/cfg_category"] not in q.client["experiment/start/cfg_file"]:
        if q.client["experiment/start/cfg_category"] != "experiment":
    ...
    choices_problem_types = [
        ui.choice(name, label) for name, label in get_problem_types(category=q.client["experiment/start/cfg_category"])
    ]

    # remove model type if present in cfg file name here
    q.client["experiment/start/cfg_file"] = remove_model_type(q.client["experiment/start/cfg_file"])

    if len(df_experiments["id"]) > 0:
        if q.client["experiment/start/cfg_experiment"] is None:
            q.client["experiment/start/cfg_experiment"] = str(df_experiments["id"].iloc[0])
        # Default pretrained from the previous experiment to False
        if q.client["experiment/start/cfg_experiment_pretrained"] is None or is_create_experiment:
    ...
    if (
        ...
        and q.client["experiment/start/cfg_file_prev"] != q.client["experiment/start/cfg_file"]
        and q.client["experiment/start/cfg_category"] != "experiment"
    ):
        items += [ui.message_bar(type="warning", text=warning_message.format("Problem Type"))]
        show_update_warnings = False

    if q.client["experiment/start/cfg_category"] == "experiment":
        if (
            show_update_warnings
            and q.client["experiment/start/cfg_experiment_prev"] != q.client["experiment/start/cfg_experiment"]
        ):
            items += [ui.message_bar(type="warning", text=warning_message.format("previous Experiment"))]
    ...
    if q.client["experiment/start/cfg_category"] == "experiment":
        ...
        if q.client["experiment/start/cfg_experiment_pretrained"]:
            ...
            if os.path.exists(prev_weights):
                q.client["experiment/start/cfg"].architecture.pretrained_weights = prev_weights
                q.client["experiment/start/cfg"].architecture._visibility["pretrained_weights"] = -1

        experiments_df = q.client.app_db.get_experiments_df()
        output_dir = os.path.abspath(os.path.join(q.client["experiment/start/cfg"].output_directory, ".."))
        ...
        if q.client["experiment/start/cfg_experiment_prev"] != q.client["experiment/start/cfg_experiment"]:
            ...
            q.client["experiment/start/dataset"] = str(q.client["experiment/start/experiment"].dataset)

            items[1].dropdown.value = q.client["experiment/start/dataset"]
        # pick default values from config or dataset
        elif q.client["experiment/start/dataset_prev"] != q.client["experiment/start/dataset"]:
        ...
        q.client["experiment/start/cfg_experiment_prev"] = q.client["experiment/start/cfg_experiment"]
        ...
        if (q.client["experiment/start/cfg_file_prev"] != q.client["experiment/start/cfg_file"]) or (
        ...
        q.client["experiment/start/cfg_file_prev"] = q.client["experiment/start/cfg_file"]

        config_path = f"llm_studio/python_configs/{q.client['experiment/start/cfg_file']}"

        q.client["experiment/start/cfg"] = load_config_py(config_path=config_path, config_name="ConfigProblemBase")
            ...
            ui.inline(
                items=[
                    ui.button(
                        ...
                        label=("Run grid search" if q.client["experiment/start/grid_search"] else "Run experiment"),
                        primary=True,
                    )
                ],
                justify="start",
...
def experiment_input_type_error(q: Q, pre: str = "experiment/start") -> bool | GridCheckError:
...
async def experiment_run(q: Q):
    ...
    if len(grid_search) == 0:
        ...
        combinations = [dict(zip(all_grid_hyperparams, x, strict=False)) for x in list(combinations)]
                ...
                ui.buttons(
                    [
                        ui.button(
                            ...
                            label=(f"Start grid search of {len(combinations)} experiments"),
                            primary=True,
                        ),
                        ui.button(
...
async def experiment_list(
    ...
    buttons = [
        ...
        ui.button(name="experiment/list/delete", label="Delete experiments", primary=False),
    ]

    q.page["dataset/display/footer"] = ui.form_card(box="footer", items=[ui.inline(items=buttons, justify="start")])
...
async def experiment_compare(q: Q, selected_rows: list):
    ...
    experiment_ids = [q.client["experiment/list/df_experiments"]["id"].iloc[int(idx)] for idx in selected_rows]
    ...
    q.page["experiment/compare/footer"] = ui.form_card(box="footer", items=[ui.inline(items=buttons, justify="start")])
            ...
            for k1 in PLOT_ENCODINGS:
                if k1 == "df":
                    ...
                    for k2, v2 in df.items():
                        logger.info(f"Renaming charts {v2} to {v2.replace(old_name, new_name)}")
...
async def experiment_display(q: Q) -> None:
    ...
    experiment_id = q.client["experiment/list/df_experiments"]["id"].iloc[q.client["experiment/display/id"]]
    ...
    checkpoints_exists = os.path.exists(os.path.join(q.client["experiment/display/experiment_path"], "checkpoint.pth"))
    adapter_exists = os.path.exists(os.path.join(q.client["experiment/display/experiment_path"], "adapter_model"))
    ...
    if q.args["experiment/display/validation_prediction_insights"] is not None:
        q.client["experiment/display/tab"] = "experiment/display/validation_prediction_insights"
    ...
    has_train_data_insights = any(
        [charts.get(plot_encoding, dict()).get("train_data") is not None for plot_encoding in PLOT_ENCODINGS]
    )
    ...
    buttons = [ui.button(name="experiment/display/refresh", label="Refresh", primary=True)]
...
async def insights_tab(charts, q):
    if q.client["experiment/display/tab"] == "experiment/display/train_data_insights":
        key = "train_data"
    elif q.client["experiment/display/tab"] == "experiment/display/validation_prediction_insights":
    ...
    for k1 in PLOT_ENCODINGS:
        ...
        for k2, v2 in charts[k1].items():
            if k2 != key:
                continue
            if k1 == "html":
                q.page[f"experiment/display/charts/{k1}_{k2}"] = ui.markup_card(box="first", title="", content=v2)
                ...
                min_widths = {col: "350" for col in df.columns if "text" in str(col).lower()}
                #
                if key == "train_data":
                ...
                q.page[f"experiment/display/charts/{k1}_{k2}"] = ui.form_card(
                    ...
                    items=[
                        ui_table_from_df(
                            ...
                            sortables=[col for col in df.columns if col.startswith("Metric")],
                            markdown_cells=[col for col in df.columns if not col.startswith("Metric")],
...
async def summary_tab(experiment_id, q):
    ...
    cfg = load_config_yaml(os.path.join(q.client["experiment/display/experiment_path"], "cfg.yaml"))
    _ = get_tokenizer(cfg)
        ...
        items=[
            ...
            ui.stats(
                [
                    ui.stat(
                        value=(
                            "-" if input_dict["val metric"] in ["", "None", None] else str(input_dict["val metric"])
                        ),
...
async def configs_tab(q):
    experiment_cfg = load_config_yaml(os.path.join(q.client["experiment/display/experiment_path"], "cfg.yaml"))
    items = get_cfg_list_items(experiment_cfg)
    q.page["experiment/display/config"] = ui.stat_list_card(box="first", items=items, title="")
...
def subsample(key1, key2, value, max_plot_points=1000):
    ...
    if act_plot_points > max_plot_points:
        ...
        logger.info(
            f"{key1} {key2} sampled from size {act_plot_points} to size {len(value['steps'])} using stride {stride}."
        )
    return value
        ...
        for k2 in list_all_second_keys:
            ...
            for charts, label in zip(charts_list, legend_labels, strict=False):
                ...
                if k2 == "lr" and "lr_diff" in charts["meta"]:
                    ...
                    rows.extend(
                        [(v2["steps"][i], f"learning rate{label}", v2["values"][i]) for i in range(len(v2["values"]))]
                        + [
                            (
                                v3["steps"][i],
                    ...
                    rows.extend([(v2["steps"][i], label.strip(), v2["values"][i]) for i in range(len(v2["values"]))])
                    color = "=type"
                    fields = ["step", "type", "value"]
                else:
                    rows.extend(
            ...
            q.page[f"experiment/display/charts/{k1}_{k2}"] = ui.form_card(box=box[cnt], items=items)
            q.client.delete_cards.add(f"experiment/display/charts/{k1}_{k2}")
...
async def experiment_artifact_build_error_dialog(q: Q, error: str):
    q.page["meta"].dialog = ui.dialog("Failed to build artifact", items=[ui.text(error)], closable=True)
    q.client["keep_meta"] = True
...
async def experiment_download_artifact(
    ...
    q.page["meta"].script = ui.inline_script(f'window.open("{get_download_link(q, zip_path)}", "_blank");')
    await q.page.save()


async def experiment_download_predictions(q: Q):
    """Download experiment predictions."""
    await experiment_download_artifact(q, get_predictions_path, save_prediction_outputs, "Predictions path", None)
...
async def experiment_download_logs(q: Q):
    ...
    q.page["meta"].script = ui.inline_script(f'window.open("{download_url}", "_blank");')
    await q.page.save()
...
async def experiment_download_model(q: Q):
    ...
    if not os.path.exists(zip_path):
        ...
        if num_running_queued > 0 or (cfg.training.lora and cfg.architecture.backbone_dtype in ("int4", "int8")):
        ...
        with set_env(HF_TOKEN=q.client["default_huggingface_api_token"]):
            cfg, model, tokenizer = load_cfg_model_tokenizer(experiment_path, merge=True, device=device)
        ...
        FILES_TO_PUSH = set(FILES_TO_PUSH + [os.path.split(tokenizer_file)[-1] for tokenizer_file in tokenizer_files])
        ...
        for file in os.listdir(checkpoint_path):
            ...
            if os.path.getmtime(file_path) > model_save_time and file_path not in paths_added and file_path != zip_path:
                add_file_to_zip(zf=zf, path=file_path)
                paths_added.append(file_path)
                logger.info(f"Added {file_path} to zip file as it was created when saving the model state.")

        # Add all files from subdirectories, which include the intermediate checkpoints
        subdirectories = [d for d in os.listdir(checkpoint_path) if os.path.isdir(os.path.join(checkpoint_path, d))]
...
async def experiment_push_to_huggingface_dialog(q: Q, error: str = ""):
    if q.args["experiment/display/push_to_huggingface"] or error:
        devices = ["cpu", "cpu_shard"] + [f"cuda:{idx}" for idx in range(torch.cuda.device_count())]
        ...
        dialog_items = [
            ...
            ui.textbox(
                ...
                label="Model Name",
                value=hf_repo_friendly_name(q.client["experiment/display/experiment"].name),
                width="500px",
                required=True,
        ...
        model_name = q.client["experiment/display/push_to_huggingface/model_name"].replace(".", "-")
...
def get_experiment_summary_code_card(cfg) -> str:
    ...
    text = text.replace(
        "{{end_of_sentence}}",
        (str(cfg.tokenizer._tokenizer_eos_token) if cfg.dataset.add_eos_token_to_prompt else ""),
    )
    ...
    if cfg.problem_type not in NON_GENERATION_PROBLEM_TYPES:
        ...
        text = text.replace("{{repetition_penalty}}", str(cfg.prediction.repetition_penalty))
