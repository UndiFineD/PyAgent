# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\sections\chat.py
import gc
import logging
import os
...
async def chat_tab(q: Q, load_model=True):
    ...
    if cfg.dataset.parent_id_column != "None":
        ...
        sample_ids = (
            validation_dataframe["id"].astype(validation_dataframe[cfg.dataset.parent_id_column].dtype).tolist()
        )
        parent_ids = validation_dataframe[cfg.dataset.parent_id_column].tolist()

        sample_ids_set = set(sample_ids)
        is_seed_prompt = [False if idx in sample_ids_set else True for idx in parent_ids]
        validation_dataframe["is_seed_prompt"] = is_seed_prompt

        validation_dataframe = validation_dataframe.loc[validation_dataframe["is_seed_prompt"]]
...
async def should_start_chat(q: Q):
    ...
    if cfg.problem_type in NON_GENERATION_PROBLEM_TYPES:
        q.page["experiment/display/chat"] = ui.form_card(
            box="first",
            items=[ui.text("Chatbot is not available for this problem type. Please select a text generation problem.")],
            title="",
        )
        q.client.delete_cards.add("experiment/display/chat")
        return False
...
def gpu_is_blocked(q, gpu_id):
    ...
    gpu_blocked = any(
        [
            ...
            for gpu_list in running_experiments["gpu_list"].apply(lambda x: x.split(",")).to_list()
        ]
    )
    return gpu_blocked


def load_cfg_model_tokenizer(experiment_path: str, merge: bool = False, device: str = "cuda:0"):
    ...
    if merge and cfg.training.lora and cfg.architecture.backbone_dtype in ("int4", "int8"):
    ...
    with torch.device(cfg.environment._device):
        ...
        cfg.architecture.pretrained_weights = os.path.join(experiment_path, "checkpoint.pth")
