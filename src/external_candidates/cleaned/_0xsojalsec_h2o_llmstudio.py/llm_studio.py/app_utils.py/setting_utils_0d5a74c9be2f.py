...
SECRET_KEYS = [
    key for key in default_cfg.user_settings if any(password in key for password in ["api", "secret", "key"])
]
...
class Secrets:
    ...
    try:
        ...
        logger.warning("Error loading keyring due to timeout. Disabling keyring save option.")
    except Exception as e:
    ...
    for key in SECRET_KEYS:
        try:
            ...
            q.page["meta"].dialog = ui.dialog(
                ...
                name="secrets_error",
                items=[
                    ui.text(f"The following error occurred when using {secret_name}: {exception}."),
                    ui.button(name="settings/close_error_dialog", label="Close", primary=True),
                ],
                closable=True,
            )
...
def _get_secrets_handler(q: Q):
    ...
    secrets_handler = Secrets.get(secret_name)(username=get_user_id(q), root_dir=get_database_dir(q))
...
def _maybe_migrate_to_yaml(q: Q):
    ...
    old_usersettings_path = os.path.join(get_database_dir(q), f"{get_user_id(q)}.settings")
    ...
    try:
        ...
        with open(_get_usersettings_path(q), "w") as f:
            yaml.dump(
                {key: value for key, value in user_settings.items() if key in USER_SETTING_KEYS},
                f,
            )
