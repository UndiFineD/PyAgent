...
async def handle(q: Q) -> None:
    ...
    try:
        if q.args.__wave_submission_name__ == "home":
            ...
            names = list(
                q.client["dataset/list/df_datasets"]["name"].iloc[list(map(int, q.client["dataset/list/table"]))]
            )
            ...
            if error:
                ...
                q.client["notification_bar"] = f"Input type mismatch found in parameter **{error}**."
            else:
            ...
            if q.client["experiment/list/df_experiments"] is not None:
                ...
                experiment_id = q.client["experiment/list/df_experiments"]["id"].iloc[selected_idx]
            ...
            if q.args["experiment/list/stop/table"]:
                ...
                experiment_ids = list(
                    q.client["experiment/list/df_experiments"]["id"].iloc[list(map(int, selected_idxs))]
                )
            ...
            if q.client["delete_dialogs"]:
                await delete_dialog(q, names, "experiment/list/delete/table", "experiment")
            else:
            ...
            q.client["experiment/compare/diff_toggle"] = q.args["experiment/compare/diff_toggle"]
            await current_experiment_compare(q)
        ...
        elif q.client["nav/active"] == "experiment/start" or q.client["nav/active"] == "experiment/start/grid_search":
            await experiment_start(q)
