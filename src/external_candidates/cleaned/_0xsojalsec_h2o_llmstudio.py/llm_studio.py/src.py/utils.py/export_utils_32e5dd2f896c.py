# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\utils\export_utils.py
import json
import logging
import os
import zipfile
...
def get_artifact_path_path(experiment_name: str, experiment_path: str, artifact_type: str):
...
def save_logs(experiment_name: str, experiment_path: str, logs: dict):
    ...
    with open(charts_path, "w") as fp:
        json.dump({k: v for k, v in logs.items() if k in ["meta", "train", "validation"]}, fp)
...
def get_size_str(x, sig_figs=2, input_unit="B", output_unit="dynamic", show_unit=True) -> str:
    """
