...
def _load_cls(module_path: str, cls_name: str) -> DefaultConfigProblemBase:
    ...
    assert hasattr(module, cls_name), f"{module_path} file should contain {cls_name} class"
...
def load_config_py(config_path: str, config_name: str = "Config") -> DefaultConfigProblemBase:
...
def _get_type_annotation_error(v: Any, type_annotation: type) -> ValueError:
    return ValueError(f"Cannot show {v}: not a dataclass and {type_annotation} is not a known type annotation.")
...
def convert_cfg_base_to_nested_dictionary(cfg: DefaultConfigProblemBase) -> dict:
    ...
    for k, v in cfg_dict.items():
        ...
        if any([x in k for x in ["api", "secret", "key"]]):
            raise AssertionError("Config item must not contain the word 'api', 'secret', or 'key'")
        ...
        if type_annotation in KNOWN_TYPE_ANNOTATIONS:
            ...
            group_items = {k: list(v) if isinstance(v, tuple) else v for d in group_items for k, v in d.items()}
