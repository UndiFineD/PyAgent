# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\utils\modeling_utils.py
import gc
import logging
import os
import re
...
def save_checkpoint(
    ...
    if cfg.environment.use_deepspeed:
        ...
        if status:
            if cfg.environment._local_rank == 0:
                checkpoint = {"model": torch.load(os.path.join(path, "checkpoint.pth"), map_location="cpu")}
        else:
            logger.warning(
            ...
            if cfg.environment._local_rank == 0:
                # load to cpu
                state_dict = get_fp32_state_dict_from_zero_checkpoint(os.path.join(path, "ds_checkpoint"))
    ...
    if cfg.environment._local_rank == 0 and "classification_head.weight" in checkpoint["model"]:
        torch.save(
            checkpoint["model"]["classification_head.weight"],
            os.path.join(path, "classification_head.pth"),
        )
    if cfg.environment._local_rank == 0 and "regression_head.weight" in checkpoint["model"]:
        torch.save(
...
def _load_model_weights(
    ...
    if cfg.architecture.backbone_dtype == "int8":
        model_weights = {
            k: v.to(cfg.environment._device) if "weight_format" not in k else v for k, v in model_weights.items()
        }
...
def get_optimizer(model: torch.nn.Module, cfg: DefaultConfigProblemBase) -> torch.optim.Optimizer:
    """Prepares Optimizer.
    ...
    return (
        sum(
            [
                ...
                if isinstance(val, torch.Tensor) and torch.isnan(val.detach().cpu()).sum() > 0
            ]
        )
        > 0
    )
...
def run_inference(
    ...
    for itr in range(len(dataloader)):
        ...
        if cfg.environment.use_deepspeed:
            if cfg.prediction.metric != "Perplexity" and cfg.problem_type not in NON_GENERATION_PROBLEM_TYPES:
                output = {}
            ...
            with autocast(
                ...
                if cfg.prediction.metric != "Perplexity" and cfg.problem_type not in NON_GENERATION_PROBLEM_TYPES:
                    output = {}
                    output["predicted_answer_ids"] = unwrap_model(model).generate(batch, cfg).detach().cpu()
                else:
                    output = model.forward(batch)
...
def save_predictions(cfg: DefaultConfigProblemBase, val_data, val_dataloader, val_df, mode):
    val_data, val_df = val_dataloader.dataset.format_output(cfg=cfg, df=val_df, output=val_data)
...
def set_generation_config(backbone: "transformers.PreTrainedModel", cfg_prediction: Any):
    ...
    backbone.generation_config.max_time = cfg_prediction.max_time if cfg_prediction.max_time > 0 else None
...
def create_nlp_backbone(cfg: DefaultConfigProblemBase, model_class=AutoModel) -> Any:
    ...
    kwargs: dict[str, Any] = {}
    if hasattr(cfg.training, "attention_implementation") and cfg.training.attention_implementation != "auto":
    ...
    if cfg.architecture.backbone_dtype == "int8" and len(cfg.environment.gpus):
        ...
        logger.warning("Quantization is not supported on CPU. Please run on GPU or disable quantization.")
    ...
    logger.info(f"Attention implementation: {backbone.config._attn_implementation_internal}")

    if cfg.training.lora:
        # if used, gradient checkpointing will be enabled below
        loaded_in_kbit = getattr(backbone, "is_loaded_in_8bit", False) or getattr(backbone, "is_loaded_in_4bit", False)
        ...
        if cfg.architecture.backbone_dtype != "float32":
            ...
            if cfg.architecture.backbone_dtype != "bfloat16":
                logger.warning("Pure float16 or int8 training will likely lead to unstable training without adapters.")
    ...
    if backbone.generation_config.eos_token_id != config.eos_token_id:
        logger.warning(
            "EOS token id not matching between generation config and tokenizer. Overwriting with tokenizer id."
        )
    ...
    if backbone.generation_config.pad_token_id != config.pad_token_id:
        logger.warning(
            "PAD token id not matching between generation config and tokenizer. Overwriting with tokenizer id."
        )
...
class TokenStoppingCriteria(StoppingCriteria):
    ...
    def should_stop(
        ...
        if len(stop_word_id.shape) == 0:
            return (torch.mean(((generated_ids == stop_word_id).sum(1) > 0).float()) == 1).item()
        else:
        ...
        for stop_word_id in self.stop_word_ids:
            if self.should_stop(generated_ids, stop_word_id.to(generated_ids.device)):
                if generated_ids.shape[1] == 1:
                    logger.warning(f"Stopping criteria triggered for {stop_word_id} at first generated token.")
                return True
        return False
...
def prepare_lora(cfg: DefaultConfigProblemBase, backbone):
    ...
    if target_modules is None:
        ...
        for name, module in backbone.named_modules():
            if isinstance(module, (torch.nn.Linear, torch.nn.Conv1d, Conv1DTransformer)) and "head" not in name:
                name = name.split(".")[-1]
    ...
    for name, param in backbone.named_parameters():
        ...
        if any(unfreeze_layer in name for unfreeze_layer in cfg.training.lora_unfreeze_layers):
...
def generate(backbone, batch, cfg: DefaultConfigProblemBase, streamer, remove_prompt=True):
