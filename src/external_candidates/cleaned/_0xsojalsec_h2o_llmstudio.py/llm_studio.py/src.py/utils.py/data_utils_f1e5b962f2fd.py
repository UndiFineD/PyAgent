# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\utils\data_utils.py
import logging
import math
import os
...
def read_dataframe(
    ...
    if path.endswith(".csv"):
        ...
        raise ValueError(
            f"Could not determine type of file {path}: CSV (`.csv`) and Parquet (`.pq` and `.parquet`) are supported."
        )
...
def read_dataframe_drop_missing_labels(path: str, cfg: DefaultConfigProblemBase) -> pd.DataFrame:
    ...
    if (
        ...
        df[cfg.dataset.answer_column] = df[cfg.dataset.answer_column].fillna("").astype(str)
    return df
...
def sample_data(cfg: DefaultConfigProblemBase, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if cfg.dataset.parent_id_column != "None" and "id" in df.columns:
        ...
        np.random.seed(7331)
        sampled_root_ids = np.random.choice(root_ids, size=n_sampled_root_ids, replace=False)
...
def load_mt_bench_data(cfg: DefaultConfigProblemBase) -> pd.DataFrame:
    ...
    df[answer_column] = df[answer_column].fillna("").apply(lambda x: x[0] if x != "" else x)

    if hasattr(cfg.dataset, "rejected_prompt_column") and cfg.dataset.rejected_prompt_column != "None":
...
def get_data(cfg: DefaultConfigProblemBase) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if hasattr(cfg.prediction, "metric_gpt_template") and cfg.prediction.metric_gpt_template == "mt-bench":
        logger.info(
...
def load_train_valid_data(cfg) -> tuple[pd.DataFrame, pd.DataFrame]:
    if cfg.dataset.validation_strategy == "custom":
        ...
        val_df = read_dataframe_drop_missing_labels(cfg.dataset.validation_dataframe, cfg)
        ...
        if cfg.dataset.parent_id_column != "None":
            ...
            with PatchedAttribute(cfg.dataset, "limit_chained_samples", True):
                conversation_chain_ids = ConversationChainHandler(df=df, cfg=cfg).conversation_chain_ids
            ...
            conversation_chain_labels = [
                i for i, conversation_chain_id in enumerate(conversation_chain_ids) for _ in conversation_chain_id
            ]
            group_shuffle_split = GroupShuffleSplit(
                test_size=cfg.dataset.validation_size, n_splits=1, random_state=1337
            )
            train_idx, val_idx = next(group_shuffle_split.split(df, groups=conversation_chain_labels))
            # flatten conversation_chain_ids
            flattened_conversation_chain_ids = np.array(
                [idx for conversation_chain_id in conversation_chain_ids for idx in conversation_chain_id]
            )
            train_df = df.iloc[flattened_conversation_chain_ids[train_idx]].reset_index(drop=True)
            val_df = df.iloc[flattened_conversation_chain_ids[val_idx]].reset_index(drop=True)
        else:
            train_df, val_df = train_test_split(df, test_size=cfg.dataset.validation_size, random_state=1337)
    else:
...
def get_train_dataset(train_df: pd.DataFrame, cfg: DefaultConfigProblemBase) -> Dataset:
    ...
    train_dataset: Dataset = cfg.dataset.dataset_class(df=train_df, cfg=cfg, mode="train")
...
def sample_indices(length: int, n_indices: int = 10, seed: int = 1337) -> np.ndarray:
    ...
    np.random.seed(seed)
    idx = np.random.choice(np.arange(length), size=min(length, n_indices), replace=False)
...
def batch_padding(
    ...
    if cfg.environment.compile_model:
        ...
        if padding_side == "left":
            lengths = torch.stack(
                [torch.where(batch[mask_key][i] == 1)[0].min() for i in range(batch[mask_key].size(0))]
            ).float()
            quantile = 1 - cfg.tokenizer.padding_quantile
        else:
            lengths = torch.stack(
                [torch.where(batch[mask_key][i] == 1)[0].max() for i in range(batch[mask_key].size(0))]
            ).float()
