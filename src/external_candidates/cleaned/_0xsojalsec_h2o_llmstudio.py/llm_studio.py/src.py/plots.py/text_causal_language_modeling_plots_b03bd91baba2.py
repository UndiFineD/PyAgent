...
class Plots:
    ...
    def plot_data(cls, cfg) -> PlotData:
        ...
        max_conversation_length = min(max([len(conversation["prompts"]) for conversation in conversations]), 15)

        conversations_to_display = []
        for conversation_length in range(1, max_conversation_length + 1):
            conversations_to_display += [
                conversation for conversation in conversations if len(conversation["prompts"]) == conversation_length
            ][:5]
        ...
        for sample_number, conversation in enumerate(conversations_to_display):
            if conversation["systems"][0] != "":
                ...
                i += 1
            for prompt, answer in zip(conversation["prompts"], conversation["answers"], strict=False):
                df_transposed.loc[i] = [
                    sample_number,
                    "Prompt",
                    prompt,
...
def plot_validation_predictions(val_outputs: dict, cfg: Any, val_df: pd.DataFrame, mode: str) -> PlotData:
    conversations = get_conversation_chains(val_df, cfg, limit_chained_samples=cfg.dataset.limit_chained_samples)
    prompt_column_name = (
        cfg.dataset.prompt_column if len(cfg.dataset.prompt_column) > 1 else cfg.dataset.prompt_column[0]
    )
    ...
    for conversation in conversations:
        ...
        for prompt, answer in zip(prompts, answers, strict=False):
            input_text += f" **{prompt_column_name}:** {prompt}\n\n**{cfg.dataset.answer_column}:** {answer}\n\n"
        input_texts += [input_text]

    if "predicted_text" in val_outputs.keys():
        predicted_texts = val_outputs["predicted_text"]
    else:
        predicted_texts = ["No predictions are generated for the selected metric"] * len(target_texts)

    input_text_column_name = (
        "Input Text (tokenization max length setting may truncate the input text during training/inference)"
    )
    df = pd.DataFrame(
        {
    ...
    df[input_text_column_name] = df[input_text_column_name].apply(format_for_markdown_visualization)
...
def create_batch_prediction_df(batch, tokenizer, ids_for_tokenized_text="input_ids", labels_column="labels"):
    df = pd.DataFrame(
        {
            "Prompt Text": [
    ...
    if labels_column in batch.keys():
        df["Answer Text"] = [
            ...
            for labels in batch.get(labels_column, batch[ids_for_tokenized_text]).detach().cpu().numpy()
        ]
    tokens_list = [
        tokenizer.convert_ids_to_tokens(input_ids) for input_ids in batch[ids_for_tokenized_text].detach().cpu().numpy()
    ]
    masks_list = [
    ...
    df["Tokenized Text"] = [
        list_to_markdown_representation(tokens, masks, pad_token=tokenizer.pad_token, num_chars=100)
