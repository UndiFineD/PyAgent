# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\plots\text_dpo_modeling_plots.py
import hashlib
import os
...
class Plots:
    ...
    def plot_data(cls, cfg) -> PlotData:
        ...
        conversations_chosen = get_conversation_chains(df, cfg, limit_chained_samples=True)
        with PatchedAttribute(cfg.dataset, "answer_column", cfg.dataset.rejected_answer_column):
            conversations_rejected = get_conversation_chains(df, cfg, limit_chained_samples=True)

        # Limit to max 15 prompt-conversation-answer rounds
        max_conversation_length = min(
            max([len(conversation["prompts"]) for conversation in conversations_chosen]),
            15,
        )
        ...
        i = 0
        for sample_number, (conversation_chosen, conversations_rejected) in enumerate(conversations_to_display):
        ...
        df_transposed["Content"] = df_transposed["Content"].apply(format_for_markdown_visualization)
        df_transposed.to_parquet(path)
        return PlotData(path, encoding="df")

    @classmethod
    def plot_validation_predictions(cls, val_outputs: dict, cfg: Any, val_df: pd.DataFrame, mode: str) -> PlotData:
