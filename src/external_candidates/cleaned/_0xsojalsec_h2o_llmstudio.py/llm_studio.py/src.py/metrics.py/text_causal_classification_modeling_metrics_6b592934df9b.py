# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\metrics\text_causal_classification_modeling_metrics.py
...
def accuracy_score(
    ...
    if len(target) != len(predictions):
        raise ValueError(f"Length of target ({len(target)}) and predicted ({len(predictions)}) should be the same.")
    if len(target) == 0:
...
def auc_score(
    ...
    if len(target) != len(logits):
        raise ValueError(f"Length of target ({len(target)}) and logits ({len(logits)}) should be the same.")
    if len(target) == 0:
