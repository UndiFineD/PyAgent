# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\metrics\text_causal_regression_modeling_metrics.py
import logging
...
def mse_score(
    ...
    target = np.array([[float(t) for t in text.split(",")] for text in results["target_text"]])
    predictions = np.array(results["predictions"])

    if len(target) != len(predictions):
        raise ValueError(f"Length of target ({len(target)}) and predictions ({len(predictions)}) should be the same.")
    if len(target) == 0:
