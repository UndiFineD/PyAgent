# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\metrics\text_causal_language_modeling_metrics.py
import logging
import os
...
def sacrebleu_score(cfg: DefaultConfigProblemBase, results: dict, val_df: pd.DataFrame) -> NDArray:
    """
    ...
    scores = []
    for predicted_text, target_text in zip(results["predicted_text"], results["target_text"], strict=False):
        if target_text == "":
            score = 0.0
        else:
            score = BLEU(effective_order=True).sentence_score(predicted_text, [target_text]).score
        scores.append(score)
...
def gpt_score(
    ...
    if template_name == "mt-bench":
        ...
        vdf.loc[vdf.category.isin(["math", "reasoning", "coding"]), "filled_eval_template"] = eval_template

    vdf["filled_eval_template"] = vdf.apply(lambda row: row["filled_eval_template"].format(**row), axis=1)
    ...
    if template_name == "mt-bench":
        ...
        logger.info("MT-Bench scores by category:\n" + score_by_category.to_string(index=False))
