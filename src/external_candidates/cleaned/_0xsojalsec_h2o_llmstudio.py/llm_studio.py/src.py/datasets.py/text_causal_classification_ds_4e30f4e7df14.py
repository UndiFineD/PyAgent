# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\datasets\text_causal_classification_ds.py
import logging
...
class CustomDataset(TextCausalLanguageModelingCustomDataset):
    def __init__(self, df: pd.DataFrame, cfg: Any, mode: str = "train"):
        ...
        if 1 < cfg.dataset.num_classes <= max_value:
            raise LLMDataException(
                f"Number of classes is smaller than max label {max_value}. Please increase the setting accordingly."
            )
        elif cfg.dataset.num_classes == 1 and max_value > 1:
            raise LLMDataException(f"For binary classification, max label should be 1 but is {max_value}.")
        if min_value < 0:
            raise LLMDataException(f"Labels should be non-negative but min label is {min_value}.")
        if min_value != 0 or max_value != np.unique(self.answers_int).size - 1:
            logger.warning(f"Labels should start at 0 and be continuous but are {sorted(np.unique(self.answers_int))}.")

        if cfg.dataset.parent_id_column != "None":
            raise LLMDataException("Parent ID column is not supported for classification datasets.")
    ...
    def postprocess_output(self, cfg, df: pd.DataFrame, output: dict) -> dict:
        ...
        if len(cfg.dataset.answer_column) == 1:
            if cfg.dataset.num_classes == 1:
                ...
                output["predictions"] = output["probabilities"].argmax(dim=-1, keepdim=True)
        else:
            output["predictions"] = (output["probabilities"] > 0.5).long()

        preds = []
        for col in np.arange(output["probabilities"].shape[1]):
            preds.append(np.round(output["probabilities"][:, col].cpu().numpy(), 3).astype(str))
    ...
    def sanity_check(cls, df: pd.DataFrame, cfg: Any, mode: str = "train"):
        for answer_col in cfg.dataset.answer_column:
            assert answer_col in df.columns, f"Answer column {answer_col} not found in the {mode} DataFrame."
...
def check_for_non_int_answers(cfg, df):
    ...
    for column in cfg.dataset.answer_column:
        answers_non_int.extend(x for x in df[column].values if not is_castable_to_int(x))
    if len(answers_non_int) > 0:
        raise LLMDataException(
            f"Column {cfg.dataset.answer_column} contains non int items. Sample values: {answers_non_int[:5]}."
        )
