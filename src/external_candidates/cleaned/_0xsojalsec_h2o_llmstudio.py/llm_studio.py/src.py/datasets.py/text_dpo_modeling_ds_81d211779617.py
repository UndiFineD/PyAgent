# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\datasets\text_dpo_modeling_ds.py
import logging
...
class CustomDataset(text_causal_language_modeling_ds.CustomDataset):
    ...
    def __init__(self, df: pd.DataFrame, cfg: Any, mode: str = "train"):
        ...
        with PatchedAttribute(cfg.dataset, "answer_column", cfg.dataset.rejected_answer_column):
            if cfg.dataset.rejected_prompt_column != "None":
                with PatchedAttribute(cfg.dataset, "prompt_column", cfg.dataset.rejected_prompt_column):
                    self.conversation_chain_handler_rejected = ConversationChainHandler(self.df, cfg)
            else:
                self.conversation_chain_handler_rejected = ConversationChainHandler(self.df, cfg)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        chosen_sample = {f"chosen_{k}": v for k, v in chosen_sample.items() if k in keys}

        with PatchedAttribute(self, "conversation_chain_handler", self.conversation_chain_handler_rejected):
            rejected_sample = {f"rejected_{k}": v for k, v in super().__getitem__(idx).items() if k in keys}
    ...
    def sanity_check(cls, df: pd.DataFrame, cfg: Any, mode: str = "train"):
        ...
        assert cfg.dataset.rejected_answer_column in df.columns, (
            f"Answer column {cfg.dataset.rejected_answer_column} not found in the {mode} DataFrame."
        )
