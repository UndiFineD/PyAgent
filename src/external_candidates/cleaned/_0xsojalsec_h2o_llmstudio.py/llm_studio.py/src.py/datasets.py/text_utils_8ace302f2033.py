# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\datasets\text_utils.py
import codecs
import json
import logging
import os
...
def get_tokenizer(cfg: DefaultConfigProblemBase):
    ...
    tokenizer_class = AutoTokenizer.from_pretrained(cfg.llm_backbone, **kwargs).__class__
    ...
    tokenizer.padding_side = getattr(cfg.tokenizer, "_padding_side", tokenizer.padding_side)
...
def set_stop_token_ids(cfg, tokenizer):
    ...
    cfg.tokenizer._stop_words = [stop_word for stop_word in cfg.tokenizer._stop_words if stop_word != ""]
    cfg.tokenizer._stop_words_ids = []
    for stop_word in set(cfg.tokenizer._stop_words):
        cfg.tokenizer._stop_words_ids.append(
            tokenizer(stop_word, return_tensors="pt", add_special_tokens=False)["input_ids"][0]
        )
