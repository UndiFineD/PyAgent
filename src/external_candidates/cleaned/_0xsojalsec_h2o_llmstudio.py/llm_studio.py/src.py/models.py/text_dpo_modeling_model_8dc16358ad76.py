# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\models\text_dpo_modeling_model.py
import logging
...
class Model(nn.Module):
    ...
    def __init__(self, cfg: Any):
        ...
        self.loss_fn = self.cfg.training.loss_class.get(self.cfg.training.loss_function)(self.cfg)
        ...
        if self.loss_fn.requires_reference_model:
            if cfg.training.lora and not cfg.training.lora_unfreeze_layers:
                ...
                self.backbone_reference, _ = create_nlp_backbone(cfg, model_class=AutoModelForCausalLM)
    ...
    def forward(
        ...
        outputs["additional_log_chosen_cross_entropy_loss"] = SampleAveragedCrossEntropyLoss(self.cfg)(
            logits_dict["chosen"], labels_dict["chosen"]
        ).detach()
        outputs["additional_log_rejected_cross_entropy_loss"] = SampleAveragedCrossEntropyLoss(self.cfg)(
            logits_dict["rejected"], labels_dict["rejected"]
        ).detach()

        if not self.training and self.cfg.prediction.metric == "Perplexity":
            outputs["perplexity"] = self.perplexity(logits_dict["chosen"], labels_dict["chosen"])
