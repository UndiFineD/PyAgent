# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_owl.py\owl.py\camel.py\messages.py\base_d0fc55b7c3a1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\messages\base.py
        ...
        if self.image_list and len(self.image_list) > 0:
            for image in self.image_list:
                ...
                if image_type not in OpenAIImageType:
                    raise ValueError(f"Image type {image.format} is not supported by OpenAI vision model")
