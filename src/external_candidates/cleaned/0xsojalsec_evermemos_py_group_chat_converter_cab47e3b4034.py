...
def convert_group_chat_format_to_memorize_input(
    ...
    group_name = conversation_meta.get("name")  # Extract group name from conversation_meta
    ...
    if messages:
        ...
        if create_time_str:
            current_time = _parse_datetime_with_timezone(create_time_str, default_timezone)
...
def _convert_message_to_internal_format(
    ...
    create_time_iso = parsed_create_time.isoformat() if parsed_create_time else create_time
...
def _parse_datetime_with_timezone(datetime_str: Optional[str], default_timezone: str = "UTC") -> Optional[datetime]:
    """
