# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\bootstrap_523294ab6854.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\bootstrap.py
...
async def setup_project_context(env_file=".env", mock_mode=False):
    ...
    if mock_mode or (os.getenv("MOCK_MODE") and os.getenv("MOCK_MODE").lower() == "true"):
        enable_mock_mode()
...
Environment variables:
  ...
  MOCK_MODE=true    Enable Mock mode (for testing)
    ...
    if not script_path.exists():
        print(f"❌ Error: Script file does not exist: {args.script_path}", file=sys.stderr)
    ...
    try:
        ...
        if "attempted relative import with no known parent package" in str(e):
            ...
            try:
                ...
                print(f"📦 Interpreting path '{script_path}' as module '{module_name}', retrying...")
