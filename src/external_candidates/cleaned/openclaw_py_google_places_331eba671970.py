# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\openclaw.py\skills.py\local_places.py\src.py\local_places.py\google_places_331eba671970.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\openclaw\skills\local-places\src\local_places\google_places.py
...
def _request(method: str, url: str, payload: dict[str, Any] | None, field_mask: str) -> _GoogleResponse:
    try:
...
def _build_search_body(request: SearchRequest) -> dict[str, Any]:
    ...
    if request.filters:
        ...
        if filters.price_levels:
            body["priceLevels"] = [_PRICE_LEVEL_TO_ENUM[level] for level in filters.price_levels]
