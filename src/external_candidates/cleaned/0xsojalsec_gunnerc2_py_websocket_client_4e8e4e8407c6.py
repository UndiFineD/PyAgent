# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\gui.py\websocket_client_4e8e4e8407c6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\websocket_client.py
...
class SessionsWSClient(QObject):
    ...
    def _send(self, payload: Dict[str, Any], cb: Optional[Callable[[dict], None]] = None) -> str:
...
class OperatorsWSClient(QObject):
    ...
    def list_now(self, cb=None):
        return self._send({"action": "list"}, cb)

    def add(self, username, password, role, cb=None):
        return self._send(
            {"action": "add", "username": username, "password": password, "role": role},
            cb,
        )
    ...
    def update(self, ident, username_new=None, password_new=None, role_new=None, cb=None):
        ...
        if username_new is not None:
            payload["username_new"] = username_new

        if password_new is not None:
            payload["password_new"] = password_new

        if role_new is not None:
            payload["role_new"] = role_new
