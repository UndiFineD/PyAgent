# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\search.py\repository.py\foresight_es_repository_de1ae0924a7c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\search\repository\foresight_es_repository.py
...
class ForesightEsRepository(BaseRepository[ForesightDoc]):
    ...
    def _log_explanation_details(self, explanation: Dict[str, Any], indent: int = 0) -> None:
        """
    ...
    async def multi_search(
        ...
        try:
            ...
            if query:
                ...
                query_with_scores = [(word, self._calculate_text_score(word)) for word in query]

                sorted_query_with_scores = sorted(query_with_scores, key=lambda x: x[1], reverse=True)[:10]
                ...
                for word, word_score in sorted_query_with_scores:
                    should_queries.append(Q("match", search_content={"query": word, "boost": word_score}))
            ...
            if explain and query:
                ...
                search_response = await client.search(index=index_name, body=search_body, explain=True)
