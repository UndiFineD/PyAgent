# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\storage.py\networkxstorage_fbc680dc7cdf.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Storage\NetworkXStorage.py
...
class NetworkXStorage(BaseGraphStorage):
    ...
    async def get_edge_weight(self, source_node_id: str, target_node_id: str) -> Union[float, None]:
    ...
    async def get_community_schema(self):
        ...
        for node_id, node_data in self._graph.nodes(data=True):
            ...
            for cluster in clusters:
                ...
                _schemas[cluster_key].edges.update([tuple(sorted(e)) for e in this_node_edges])

                _schemas[cluster_key].chunk_ids.update(node_data["source_id"].split(GRAPH_FIELD_SEP))
        ...
        for i, curr_level in enumerate(ordered_levels[:-1]):
            ...
            for comm in this_level_comms:
                _schemas[comm].sub_communities = [
                    c for c in next_level_comms if _schemas[c].nodes.issubset(_schemas[comm].nodes)
                ]
    ...
    async def find_k_hop_neighbors_batch(self, start_nodes: list[str], k: int) -> set:
        nodes_set_list = await asyncio.gather(*[self.find_k_hop_neighbors(node, k) for node in start_nodes])
    ...
    async def get_edge_relation_name_batch(self, edges: list[tuple[str, str]]):
        relations = await asyncio.gather(*[self.get_edge_relation_name(edge[0], edge[1]) for edge in edges])
    ...
    async def get_one_path(self, start: str, cand: list[str], cutoff: int = 5):
        ...
        for node, dis in dist.items():
            if (node in cand) and (end is None or dis < dist[end]):
                end = node
        ...
        for u in start_nodes:
            ...
            while neis != []:
                ...
                if len(inter) != 0:
                    for v in inter:
                        new_neis.extend(
                            [(await self.get_edge(e[0], e[1]))["tgt_id"] for e in await self.get_node_edges(v)]
                        )

                        neighbor_list_cand.extend(
                            [(await self.get_edge(e[0], e[1])) for e in await self.get_node_edges(v)]
                        )
