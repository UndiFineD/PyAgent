# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\workers\megatron_workers.py
...
class ActorRolloutRefWorker(MegatronWorker):
    ...
    def _build_rollout(self):
        if self.config.rollout.name == "vllm":
            ...
            layer_name_mapping = {
                "qkv_layer_name": self.config.rollout.layer_name_map.get("qkv_layer_name", "qkv"),
            ...
            assert vllm_mode == "customized", (
                "Support for vllm>=0.7 for Megatron-LM backend has not been implemented yet."
            )
            rollout = vLLMRollout(
                actor_module=params,
