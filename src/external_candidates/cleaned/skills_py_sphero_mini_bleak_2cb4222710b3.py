# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\joneschi.py\sphero_mini.py\scripts.py\sphero_mini_bleak_2cb4222710b3.py
...
class SpheroMini:
    ...
    async def getAcknowledgement(self, ack):
        ...
        while 1:
            ...
            if time.time() > start + 10:
                print(
                    "Timeout waiting for acknowledgement: {}/{}".format(ack, self.sequence),
                    file=sys.stderr,
                )
    ...
    def handleNotification(self, sender, data):
        ...
        # parse on EndOfPacket received structure similar to send packets (see docstring in sphero_mini._send())
            ...
            esc_index = [
                i
                for i in range(len(self.notificationPacket) - 2, 0, -1)  # descending index between EOP and SOP
