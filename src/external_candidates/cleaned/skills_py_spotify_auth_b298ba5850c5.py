# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\braydoncoyer.py\spotify_history.py\scripts.py\spotify_auth_b298ba5850c5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\braydoncoyer\spotify-history\scripts\spotify-auth.py
...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if "code" in params:
            ...
            self.wfile.write(
                f"<html><body><h1>Error</h1><p>{params.get('error', ['Unknown'])}</p></body></html>".encode()
            )
