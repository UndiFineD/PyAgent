...
llm_functions = {
    ...
    "query_plan_accurate": {
        "query_model": partial(query_plan_accurate, max_gen=2048, temp=args.temperature),
...
while eps < args.Eps:
    ...
    while not done:
        ...
        if "unknowns_json" in database["kb"].keys():
            ...
            qprint(
                Style.DIM + json.dumps(list(database["kb"]["unknowns_json"].values())[0], indent=2) + Style.RESET_ALL
            )
        qprint("\n" + Fore.BLACK + Back.GREEN + "--" * 10 + " Skills " + "--" * 10 + Style.RESET_ALL)
        qprint(
            Style.DIM
            + json.dumps(
                {k: v["skill_desc"] for k, v in database["skills"]["skill_library"].items()},
                indent=2,
            )
            + Style.RESET_ALL
        )
        ...
        if skill is not None:
            ...
            if (
                len(
                    ...
                    & set(attention_rounds["mistake"]).union(set(attention_rounds["confusion"]))
                )
                + 1
                ...
                messages, shrink_idx = compose_feedback_prompt(
                    CTXT_dict,
                    qa_history,
                    "{}:{}".format(skill, database["skills"]["skill_library"][skill]["skill_guide"]),
...
""".strip(),
        ...
        if step % args.wandb_log_interval == 0 or done:
                ...
                feedback_table.add_data(
                    *(
                        ...
                        if skill not in database["feedback"]["skill_feedback"].keys()
                            ...
                            skill,
                            database["feedback"]["skill_feedback"][skill],
                            len(set(skill_history[skill]) & set(attention_rounds["mistake"])),
                        ]
                    )
                )
            ...
            wandb.log(
                {
                    "eps-{}-rollout/rollout {}~{}".format(eps, last_log, step - 1): rollouts,
