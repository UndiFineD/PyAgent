...
def describe_env(info):
    ...
    for idx in np.unique(semantic):
        ...
        smallest_indices = [
            (smallest_indices[0][i], smallest_indices[1][i]) for i in range(min(2, np.count_nonzero(semantic == idx)))
        ]
    ...
    if len(obj_info_list) > 0:
        status_str = "* Near-by objects (7x9 grid):\n{}".format(
            "\n".join(
                [
                    "  - {} {} steps to {}".format(name.replace("arrow", "flying-arrow"), dist, loc)
...
def match_act(string):
    ...
    if matches:
        ...
        return (
            None,
            None,
            "'{}' does not seem to match any elemet in the list: {}".format(string, action_list),
        )
