# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\examples\crafter\baseline.py
import os
...
import gym
import numpy as np
from crafter_description import action_list, describe_frame, match_act
from tqdm import tqdm
from utils import describe_achievements, get_ctxt
...
def compose_ingame_prompt(info, question, past_qa=[]):
    messages = [{"role": "system", "content": "You’re a player trying to play the game."}]
        ...
        while step < env_steps:
            ...
            text_obs = "\n\n".join(
                [
                    "== Gamestep {}{} ==\n\n".format(
                        i,
                        "" if i != trajectories[-1][0] else " (current)",
                    )
                    + "{}{}".format(d, "\n\nAction:\n{}".format(a) if a is not None else "")
                    for i, a, d in trajectories[-2:]
                ]
            )
            info["obs"] = text_obs
