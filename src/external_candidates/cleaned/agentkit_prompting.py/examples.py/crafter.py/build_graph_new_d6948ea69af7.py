# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\examples\crafter\build_graph_new.py
...
delayed_gamestep_questions = {prompts["s-action"]["prompt"]}
...
adaptive_actor_questions = [
    prompts["actor-reflect"]["prompt"],
    prompts["actor-plan-sketch"]["prompt"],
    prompts["actor-actions"]["prompt"],
]
...
gameplay_questions = {
    ...
    "Base your answers on the gameplay, create a list of up to 5 most important aspects the player should pay more attention to next time. For each aspect, write a 1-line specification on how the aspected may be addressed/quantified.": [
        "What did the player accomplish?",
        "How did the player die at the final step?",
        "What did the player fail to accomplish?",
        "What mistakes did the player make?",
    ],
}
gameplay_shorthands = {
...
def build_graph(llm_functions, database={}):
    ...
    for _, node_info in prompts.items():
        ...
        node = SimpleDBNode(
            ...
            node_prompt,
            graph,
            llm_functions[node_info["query"]]["query_model"],
            node_info["compose"],
            database,
            after_query=(node_info["after_query"] if "after_query" in node_info.keys() else None),
            verbose=True,
            token_counter=llm_functions[node_info["query"]]["token_counter"],
        )
        graph.add_node(node)
