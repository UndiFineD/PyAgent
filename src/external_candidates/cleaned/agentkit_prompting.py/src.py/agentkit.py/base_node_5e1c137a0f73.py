# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\src\agentkit\base_node.py
import copy
import datetime
...
class BaseNode:
    ...
    def __init__(
        ...
        graph: Graph,
        query_llm: Callable,
        compose_prompt: BaseComposePrompt,
        after_query: BaseAfterQuery = None,
        error_msg_fn: Callable[[list, str, AfterQueryError], list] = error_msg_default,
        verbose: bool = False,
    ...
    def commit_trace(self):
        ...
        self.chain_span.add_inputs_and_outputs(
            inputs={"dependencies": [(node.key, node.result) for node in self.adjacent_from]},
            outputs={"response": self.result},
        )
            ...
            for i in range(3):
                try:
                    ...
                    if usage is not None:
                        ...
                        self.counts.append(
                            {
                                "prompt": self.token_counter(temp_prompt),
                                "completion": self.token_counter(self.result),
                            }
                        )
