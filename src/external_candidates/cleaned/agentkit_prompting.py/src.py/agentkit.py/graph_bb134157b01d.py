# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\src\agentkit\graph.py
import datetime
...
class Graph:
    ...
    def add_temporary_node(self, node):
        ...
        assert node.key not in self.temporary_nodes.keys(), "Node ({}) already exists in temporary graph".format(
            node.key
        )
    ...
    def add_edge(self, from_key, to_key, prepend=False):
        ...
        if from_key in self.nodes.keys() and to_key in self.nodes.keys():
            assert not self.has_edge_with_temporary(from_key, to_key), "Edge ({}) already exists".format(
                (from_key, to_key)
            )
    ...
    def add_order(self, from_key, to_key):
        ...
        assert not self.has_edge_with_temporary(from_key, to_key), (
            "Edge {} already exists. No need to specify order".format((from_key, to_key))
        )
    ...
    def add_edge_temporary(self, from_key, to_key, prepend=False):
        ...
        assert to_key not in self.history.keys(), (
            "Cannot add edge to a node ({}) that has already been evaluated".format(to_key)
        )
    ...
    def remove_edge_temporary(self, from_key, to_key):
        ...
        assert to_key not in self.history.keys(), (
            "Cannot remove edge to a node ({}) that has already been evaluated".format(to_key)
        )
        assert from_key not in self.history.keys(), (
            "Cannot remove edge from a node ({}) that has already been evaluated".format(from_key)
        )
    ...
    def commit_trace(self):
        ...
        self.chain_span.add_inputs_and_outputs(inputs={}, outputs={"response": self.history})
