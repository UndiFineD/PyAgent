# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\tools.py\browser.py\test_browser_locators_4852239b915f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\tools\browser\test_browser_locators.py
...
class TestFindByRole(BrowserLocatorsBaseTest):
    ...
    async def test_find_by_role_success(self, mock_browser_manager, mock_locator):
        ...
        with patch(
            ...
            page.get_by_role.assert_called_once_with("button", name="Submit", exact=False)

            locator.first.wait_for.assert_called_once_with(state="visible", timeout=5000)
    ...
    @pytest.mark.asyncio
    async def test_find_by_role_multiple_elements(self, mock_browser_manager, mock_locator):
...
class TestFindByLabel(BrowserLocatorsBaseTest):
    ...
    @pytest.mark.asyncio
    async def test_find_by_label_input_element(self, mock_browser_manager, mock_locator):
    ...
    @pytest.mark.asyncio
    async def test_find_by_label_textarea_element(self, mock_browser_manager, mock_locator):
...
class TestFindByPlaceholder(BrowserLocatorsBaseTest):
    ...
    @pytest.mark.asyncio
    async def test_find_by_placeholder_success(self, mock_browser_manager, mock_locator):
...
class TestFindByTestId(BrowserLocatorsBaseTest):
    ...
    @pytest.mark.asyncio
    async def test_find_by_test_id_long_text_truncation(self, mock_browser_manager, mock_locator):
...
class TestFindButtons(BrowserLocatorsBaseTest):
    ...
    @pytest.mark.asyncio
    async def test_find_buttons_case_insensitive_filter(self, mock_browser_manager, mock_locator):
    ...
    @pytest.mark.asyncio
    async def test_find_buttons_no_visible_buttons(self, mock_browser_manager, mock_locator):
...
class TestFindLinks(BrowserLocatorsBaseTest):
    ...
    async def test_find_links_with_filter(self, mock_browser_manager, mock_locator):
        ...
        element.text_content = AsyncMock(side_effect=["Documentation", "API Docs", "Examples"])
        ...
        with patch(
            ...
            with patch("code_puppy.tools.browser.browser_locators.emit_info"):
                ...
                assert result["filtered_count"] == 1  # Only "Documentation" contains "docs" (case-sensitive)
...
class TestIntegrationScenarios(BrowserLocatorsBaseTest):
    ...
    @pytest.mark.asyncio
    async def test_multiple_locator_functions_same_page(self, mock_browser_manager, mock_locator):
