# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-xpander.ai\Getting-Started\hello-world\my_agent.py
"""
...
from xpander_sdk import Agent, LLMProvider, LLMTokens, MemoryStrategy, Tokens
    ...
    async def _agent_loop(self):
        ...
        while not self.agent.is_finished():
            ...
            cloud_tool_call_results[:] = [
                c for c in cloud_tool_call_results if c.tool_call_id not in {t.tool_call_id for t in local_tool_calls}
            ]
        ...
        logger.info(f"✨ Execution duration: {time.perf_counter() - execution_start_time:.2f} s")
        logger.info(
