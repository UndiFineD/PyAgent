# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\event_log_record_raw_repository_b3c068b2e1dc.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\repository\event_log_record_raw_repository.py
...
class EventLogRecordRawRepository(BaseRepository[EventLogRecord]):
    ...
    async def get_by_id(
        ...
        try:
            ...
            if target_model == self.model:
                ...
                result = await self.model.find_one({"_id": object_id}, projection_model=target_model, session=session)
    ...
    async def get_by_parent_episode_id(
        ...
        try:
            ...
            if target_model == self.model:
                query = self.model.find({"parent_episode_id": parent_episode_id}, session=session)
            ...
            logger.error("❌ Failed to retrieve event logs by parent episodic memory ID: %s", e)
    ...
    async def get_by_user_id(
        ...
        try:
            ...
            if target_model == self.model:
                ...
                query = self.model.find({"user_id": user_id}, projection_model=target_model, session=session)
    ...
    async def find_by_time_range(
        ...
        try:
            ...
            if target_model == self.model:
                ...
                query = self.model.find(filter_dict, projection_model=target_model, session=session)
    ...
    async def delete_by_id(self, log_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        """
    ...
    async def delete_by_parent_episode_id(
        ...
        try:
            result = await self.model.find({"parent_episode_id": parent_episode_id}, session=session).delete()
            ...
            logger.error("❌ Failed to delete event logs by parent episodic memory ID: %s", e)
