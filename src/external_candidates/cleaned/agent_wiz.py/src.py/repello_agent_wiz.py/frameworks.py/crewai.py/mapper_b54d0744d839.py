# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\src\repello_agent_wiz\frameworks\crewai\mapper.py
import argparse
import ast
import json
import os
...
def _stringify_ast_node(node: ast.AST) -> str:
    if isinstance(node, ast.Constant):
        ...
        return get_potential_fqn(node) or f"<Attribute {node.attr}>"
...
class CrewAIStructureExtractor(ast.NodeVisitor):
    def __init__(self):
        ...
        self.tool_instance_map: Dict[str, Dict[str, ast.AST]] = defaultdict(
            dict
        )  # class_name -> {attr_name: value_node}
    ...
    def _find_return_call(self, body_nodes: List[ast.AST], target_class_fqn: str) -> Optional[ast.Call]:
        for stmt in body_nodes:
            if isinstance(stmt, ast.Return) and isinstance(stmt.value, ast.Call):
                ...
                if (
                    func_fqn == target_class_fqn
                    or func_name_simple == target_class_fqn
                    or (isinstance(call_node.func, ast.Name) and target_class_fqn.endswith(f".{call_node.func.id}"))
                ):
                    return call_node
                ...
                for stmt in init_method.body:
                    if isinstance(stmt, ast.Assign) and len(stmt.targets) == 1:
                        ...
                        if (
                            ...
                            if tool_class_name and (
                                tool_class_name in self.tool_imports or tool_class_name in self.tools_info
                            ):
        ...
        for decorator in node.decorator_list:
            ...
            if self.current_crew_base_info and (decorator_fqn == agent_decorator or decorator_name == "agent"):
                ...
                if agent_call:
                    ...
                    config_kw_node = next(
                        (kw.value for kw in agent_call.keywords if kw.arg == "config"),
                        None,
                    )
                    if config_kw_node:
                ...
                if task_call:
                    ...
                    config_ref, config_data = (None, None)
                    config_kw_node = next(
                        (kw.value for kw in task_call.keywords if kw.arg == "config"),
                        None,
                    )
                    if config_kw_node:
                        config_ref, config_data = self._parse_config_arg(config_kw_node, base_dir)
                    ...
                    agent_kw_node = next(
                        (kw.value for kw in task_call.keywords if kw.arg == "agent"),
                        None,
                    )
                    ...
                    self.tasks_info[task_id] = {
                        ...
                        "description": kwargs.get(
                            "description",
                            (config_data.get("description", task_id) if config_data else task_id),
                        ),
                ...
                decorator_fqn == tool_decorator
                or decorator_name == "tool"
                or (isinstance(decorator, ast.Call) and get_potential_fqn(decorator.func) == "tool")
            ):
                is_crewai_decorated = True
                tool_name = func_name
            ...
            # --- End self.attribute handling ---
                ...
                if isinstance(value_node, ast.List):
                    ...
                    if func_fqn == agent_fqn or func_name_simple == "Agent":
                        config_ref, config_data = (None, None)
                        config_kw_node = next(
                            (kw.value for kw in value_node.keywords if kw.arg == "config"),
                            None,
                        )
                        if config_kw_node:
                            config_ref, config_data = self._parse_config_arg(config_kw_node, base_dir)
                        ...
                        output_ref = (
                            kwargs.get("output_json") or kwargs.get("output_pydantic") or kwargs.get("output_file")
                        )
                        context_vars = kwargs.get("context", [])
    ...
    def _parse_process_type(self, process_ref: Any) -> CrewProcess:
        ...
        if (
            process_ref_str == proc_seq_fqn
            or process_ref_str.endswith(".sequential")
            or process_ref_str == "sequential"
        ):
            return CrewProcess.SEQUENTIAL
        elif (
            process_ref_str == proc_hier_fqn
            or process_ref_str.endswith(".hierarchical")
            or process_ref_str == "hierarchical"
        ):
    ...
    def finalize_graph(self):
        ...
        for tool_ref in all_tool_refs_found:
            if tool_ref in tool_node_map:
                continue  # Already handled

            resolved_node_name = None
            # Try resolving self.attribute
            if isinstance(tool_ref, str) and tool_ref.startswith("self."):
                class_context = agent_data.get(
                    "class_context"
                )  # Assumes agent_data is in scope (needs adjustment if run outside agent loop)
        ...
        for task_id, task_data in self.tasks_info.items():
            ...
            if task_data.get("is_decorator") and not agent_node_name:
                ...
                if (
                    class_context
                    and task_name in self.agents_info
                    and self.agents_info[task_name].get("class_context") == class_context
                ):
        ...
        for crew_id, crew_data in self.crews_info.items():
            ...
            if isinstance(task_sequence_refs, list):
                ...
                if task_sequence_refs in self.task_lists:
                    ...
                    if not crew_def_location:
                        crew_def_location = self.task_lists[task_sequence_refs].get("location")

            if not actual_task_sequence:
                print(
                    f"Warning: No valid task sequence found for crew '{crew_id}'.",
                    file=sys.stderr,
                )
                continue
            ...
            crew_metadata = {
                ...
                "definition_location": crew_def_location,
            }
            ...
            if process_type == CrewProcess.SEQUENTIAL:
                ...
                for i in range(len(valid_task_node_names) - 1):
                    self._add_output_edge(
                        valid_task_node_names[i],
                        valid_task_node_names[i + 1],
                        seq_condition,
                        crew_metadata,
                    )
...
def extract_crewai_graph(directory_path: str, output_file: str):
    ...
    for root, _, files in os.walk(directory_path):
        ...
        if any(
            part.startswith(".") or part in ["venv", "env", "__pycache__", "node_modules", ".git"]
    ...
    print(
        f"Extraction finished. Found {len(graph_structure['nodes'])} nodes and {len(graph_structure['edges'])} edges."
    )
    ...
    if graph_structure["nodes"] or graph_structure["edges"]:
        try:
            ...
            print(
                f"An unexpected error occurred during JSON serialization or path creation: {e}",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=".",
        help="Directory containing the Python code (default: current directory).",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="crewai_graph_output.json",
        help="Output JSON filename (default: crewai_graph_output.json).",
    )
