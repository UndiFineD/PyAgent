# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\src\repello_agent_wiz\frameworks\agent_chat\mapper.py
import argparse
import ast
import json
import os
        ...
        if isinstance(node.value, ast.Call) and hasattr(node.value.func, "id"):
            ...
            # RoundRobinGroupChat
                ...
                if args and isinstance(args[0], ast.List):
                    ...
                    agent_ids = [
                        self.agent_names_to_ids.get(var) for var in agent_vars if var in self.agent_names_to_ids
                    ]
...
def build_graph_data(visitor):
    ...
    for source, target, edge_type in getattr(visitor, "agent_edges", []):
        edge = {
            "source": source,
            "target": target,
            "condition": {"type": ("group_sequence" if edge_type == "RoundRobinGroupChat" else "member_of_team")},
            "metadata": {"chat": edge_type},
        }
    ...
    for visitor in visitors:
        ...
        for tool_var, func_list in visitor.tools.items():
            if tool_var not in merged.tools:
                ...
                merged.tools[tool_var].extend(f for f in func_list if f not in merged.tools[tool_var])
        ...
        merged.agent_edges.extend([edge for edge in visitor.agent_edges if edge not in merged.agent_edges])
