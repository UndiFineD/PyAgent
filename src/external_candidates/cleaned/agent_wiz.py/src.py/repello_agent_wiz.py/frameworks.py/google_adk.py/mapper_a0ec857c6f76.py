...
def node_to_string(node: Optional[ast.AST]) -> str:
    ...
    if isinstance(node, ast.Dict):
        return (
            "{" + ", ".join(f"{node_to_string(k)}: {node_to_string(v)}" for k, v in zip(node.keys, node.values)) + "}"
        )
    if isinstance(node, ast.JoinedStr):
        return (
            'f"'
            + "".join(
                (node_to_string(v) if isinstance(v, ast.FormattedValue) else str(getattr(v, "value", "")))
...
class AdkStructureExtractor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        full_func_key = (
            f"{file_rel_path}::{current_class}.{func_name}" if current_class else f"{file_rel_path}::{func_name}"
        )
            ...
            if isinstance(value_node, ast.Call):
                ...
                if agent_type_key and agent_type_key != "AgentTool":
                    ...
                    self.agent_instances[instance_var_name] = {
                        ...
                        "global_instruction_param_repr": kwargs.get("global_instruction"),
                    ...
                    if wrapped_agent_var_repr and isinstance(wrapped_agent_var_repr, str):
                        ...
                        logger.warning(f"AgentTool instance {tool_instance_name} needs 'agent' kwarg.")
        self.generic_visit(node)
    ...
    def _add_output_edge(
        ...
        if not (source_id in self.output_node_ids and target_id in self.output_node_ids):
            ...
            return
        if any(
            e["source"] == source_id and e["target"] == target_id and e["label"] == label for e in self.output_edges
        ):
            return
    ...
    def _find_function_def(
        ...
        potential_match = next(
            (data for data in self.all_functions.values() if data["simple_name"] == func_name_repr),
            None,
        )
        if potential_match:
            ...
            if resolved_fqn and resolved_fqn != func_name_repr:
                potential_match = next(
                    (
                        ...
                        if data["simple_name"] == resolved_fqn or data["key"] == resolved_fqn
                    ),
                    None,
                )
                if potential_match:
                    return potential_match
                potential_match = next(
                    (data for data in self.all_functions.values() if data["key"].endswith(f"::{resolved_fqn}")),
                    None,
                )
                if potential_match:
                    return potential_match
        ...
        for agent_var, data in self.agent_instances.items():
            ...
            node_type = (
                ...
                if agent_type_key in ["LlmAgent", "Agent"]
                    ...
                    AdkNodeType[agent_type_key.upper()] if agent_type_key != "CustomAgent" else AdkNodeType.CUSTOM_AGENT
                )
            )
            metadata = {
            ...
            for tool_var_repr in data.get("tool_vars_repr", []):
                ...
                if tool_var_repr in self._agent_tool_instance_map:
                    ...
                    if target_node_id:
                        ...
                        logger.warning(f"AgentTool '{tool_var_repr}' wraps unknown agent var '{wrapped_agent_var}'.")
                else:
        ...
        for agent_var, data in self.agent_instances.items():
            ...
            for sub_agent_var_repr in data.get("sub_agent_vars_repr", []):
                ...
                if target_node_id:
                    ...
                    logger.warning(f"Sub-agent variable '{sub_agent_var_repr}' for agent '{agent_var}' not found.")
...
def extract_adk_graph(directory_path: str, output_filename: str):
    ...
    for root, dirs, files in os.walk(directory_path):
        dirs[:] = [
            d for d in dirs if not d.startswith(".") and d not in ["venv", "env", "__pycache__", "node_modules", ".git"]
        ]
