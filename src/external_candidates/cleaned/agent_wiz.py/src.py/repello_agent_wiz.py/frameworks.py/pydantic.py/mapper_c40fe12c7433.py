...
class PydanticAIStructureExtractor(ast.NodeVisitor):
    ...
    def _add_output_node(
        ...
        if name not in self.output_node_ids:
            node_data = {
                "name": name,
                "function_name": (
                    name if node_type in [PydanticAINodeType.TOOL, PydanticAINodeType.ORCHESTRATOR] else None
                ),
                "docstring": docstring,
    ...
    def _add_output_edge(
        ...
        if source in self.output_node_ids and target in self.output_node_ids:
            ...
            if not any(
                e["source"] == source and e["target"] == target and e["condition"] == condition
    ...
    def _get_call_kwargs(self, node: ast.Call) -> Dict[str, Any]:
        ...
        for keyword in node.keywords:
            ...
            if arg_name:
                ...
                elif isinstance(value_node, (ast.Name, ast.Attribute, ast.BinOp, ast.Subscript)):
                    kwargs[arg_name] = node_to_string(value_node)
                elif isinstance(value_node, ast.List):
                    kwargs[arg_name] = self._get_list_of_identifiers(value_node) or "<complex_list>"
                ...
                for sub_node in ast.walk(node):
                    if isinstance(sub_node, ast.Call):
                        if (
                            ...
                            and get_potential_fqn(sub_node.func.value) in ["asyncio", self.imports.get("asyncio")]
                        ):
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        is_pydantic_model = any(
            self._resolve_fqn_from_node(b) in ["pydantic.main.BaseModel", "pydantic.BaseModel"]
        ...
        if is_pydantic_model or is_dataclass:
            ...
            for body_node in node.body:
                if isinstance(body_node, ast.AnnAssign) and isinstance(body_node.target, ast.Name):
                    ...
                    field_type = node_to_string(body_node.annotation) if body_node.annotation else "Any"
            ...
            if isinstance(value_node, ast.Call):
                ...
                if base_func_node:
                    ...
                    is_agent_instantiation = (
                        (resolved_func_fqn and agent_fqn and resolved_func_fqn == agent_fqn)
                    ...
                    if is_agent_instantiation:
                        ...
                        if (
                            ...
                            and isinstance(value_node.args[0], (ast.Constant, ast.JoinedStr))
                        ):
                            model_name = (
                        ...
                        for kw in value_node.keywords:
                            if kw.arg == "system_prompt":
                                if isinstance(kw.value, ast.JoinedStr):
                                    system_prompt = self._get_joined_string_value(kw.value)
                                else:
                                    system_prompt = self._get_string_value(kw.value)
                                if system_prompt is None:
                                    system_prompt = f"<complex_prompt:{type(kw.value).__name__}>"
                                break
                        if (
                            system_prompt is None
                            and len(value_node.args) > 1
                            and isinstance(value_node.args[1], (ast.Constant, ast.JoinedStr))
                        ):
                            if isinstance(value_node.args[1], ast.JoinedStr):
                                system_prompt = self._get_joined_string_value(value_node.args[1])
                            else:
                                system_prompt = self._get_string_value(value_node.args[1])
        ...
        for tool_id, tool_data in self.tools_info.items():
            ...
            for called_agent_id_in_tool, call_node_in_tool in finder.called_agents:
                if called_agent_id_in_tool not in processed_agents_in_tool:
                    tool_filepath = (
                        self.tools_info[tool_id].get("definition_location", "unknown").split(":")[0]
