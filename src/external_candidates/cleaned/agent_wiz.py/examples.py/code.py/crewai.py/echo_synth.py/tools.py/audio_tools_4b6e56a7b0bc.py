# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\crewai\echo_synth\tools\audio_tools.py
import os
...
class WhisperTranscriptionInput(BaseModel):
    ...
    language: Optional[str] = Field(
        None,
        description="Optional language code (e.g., 'en', 'es'). If not provided, language will be auto-detected.",
    )
    prompt: Optional[str] = Field(
        None,
        description="Optional prompt to guide the transcription for better accuracy.",
    )
...
class AudioAnalysisInput(BaseModel):
    ...
    analysis_type: str = Field(
        ...,
        description="Type of analysis to perform. Options: 'sentiment', 'speaker_count', 'background_noise'",
    )
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an audio analysis assistant that specializes in extracting insights from transcribed audio.",
                    },
