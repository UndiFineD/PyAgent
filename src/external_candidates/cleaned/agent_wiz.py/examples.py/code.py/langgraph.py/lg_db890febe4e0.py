            ...
            if time_until_seconds < (3 * 3600):
                ...
                if not current_flight:
                    ...
                    if not current_ticket:
                        result_message = (
                            f"Current signed-in passenger with ID {passenger_id} not the owner of ticket {ticket_no}"
                        )
                    else:
                        # Proceed with update
                        cursor.execute(
...
for i, question in enumerate(tutorial_questions):
    ...
    events = part_1_graph.stream({"messages": [("user", question)]}, config_part1, stream_mode="values")
    for event in events:
...
for i, question in enumerate(tutorial_questions):
    print(f"\n--- Part 2 - Turn {i + 1} ---")
    print(f"User: {question}")
    events = part_2_graph.stream({"messages": [("user", question)]}, config_part2, stream_mode="values")
    for event in events:
...
part_3_assistant_runnable = assistant_prompt_part3 | llm.bind_tools(part_3_safe_tools + part_3_sensitive_tools)
...
builder_part3.add_node("sensitive_tools", create_tool_node_with_fallback(part_3_sensitive_tools))
...
builder_part3.add_conditional_edges("assistant", route_tools_part3, ["safe_tools", "sensitive_tools", END])
...
for i, question in enumerate(tutorial_questions):
    print(f"\n--- Part 3 - Turn {i + 1} ---")
    print(f"User: {question}")
    events = part_3_graph.stream({"messages": [("user", question)]}, config_part3, stream_mode="values")
    for event in events:
...
def route_update_flight(
    state: StatePart4,
) -> Literal["update_flight_sensitive_tools", "update_flight_safe_tools", "leave_skill", END]:
...
def route_book_car_rental(
    state: StatePart4,
) -> Literal["book_car_rental_sensitive_tools", "book_car_rental_safe_tools", "leave_skill", END]:
    route = tools_condition(state)
    if route == END:
        return END
    tool_calls = state["messages"][-1].tool_calls
...
builder_part4.add_node("enter_book_hotel", create_entry_node("Hotel Booking Assistant", "book_hotel"))
...
def route_book_excursion(
    state: StatePart4,
) -> Literal["book_excursion_sensitive_tools", "book_excursion_safe_tools", "leave_skill", END]:
    route = tools_condition(state)
    if route == END:
        return END
    tool_calls = state["messages"][-1].tool_calls
...
if __name__ == "__main__":
    ...
    for i, question in enumerate(tutorial_questions):
        print(f"\n--- Part 4 - Turn {i + 1} ---")
        print(f"User: {question}")
        events = part_4_graph.stream({"messages": [("user", question)]}, config_part4, stream_mode="values")
        for event in events:
