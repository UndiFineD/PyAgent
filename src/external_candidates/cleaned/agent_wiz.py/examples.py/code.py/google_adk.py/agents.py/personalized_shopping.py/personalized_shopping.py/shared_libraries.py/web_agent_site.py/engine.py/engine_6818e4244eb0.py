# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\google_adk\agents\personalized-shopping\personalized_shopping\shared_libraries\web_agent_site\engine\engine.py
...
def load_products(filepath, num_products=None, human_goals=True):
    ...
    for i, p in tqdm(enumerate(products), total=len(products)):
        ...
        products[i]["BulletPoints"] = (
            p["small_description"] if isinstance(p["small_description"], list) else [p["small_description"]]
        )
