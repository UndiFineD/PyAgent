...
def process_aces(data) -> None:
    ...
    for node in data:
        ...
        for ace in node.get("Aces", []):
            grouped_by_type[ace["RightName"]].append({"PrincipalSID": ace["PrincipalSID"], "TargetID": target_id})
...
def process_primary_memberships(data):
    ...
    for node in data:
        ...
        if primary_group_sid:
            relationships.append({"MemberSID": member_id, "GroupSID": primary_group_sid})
...
def process_remote_accounts(data, relationship_key, relationship_type, source_node_id="ObjectIdentifier"):
    ...
    for node in data:
        ...
        for source_id in node.get(relationship_key, []).get("Results", []):
            relationships.append({"source_SID": source_id[source_node_id], "target_SID": target_id})
    ...
    for node in data:
        ...
        for nodes in node.get(relationship_key, []):
            if relationship_type == "Contains":
                relationships.append({"source_SID": target_id, "target_SID": nodes[node_id]})
            else:
                relationships.append({"source_SID": nodes[node_id], "target_SID": target_id})
...
def process_delegation(data, relationship_key, relationship_type, target_node_id="ObjectIdentifier"):
    ...
    for node in data:
        ...
        for target_id in node.get(relationship_key, []):
            relationships.append({"source_SID": source_id, "target_SID": target_id[target_node_id]})
