# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\didelco.py\pvpc_spain.py\scripts.py\tarifa_periodos_2b2129057cf8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\didelco\pvpc-spain\scripts\tarifa_periodos.py
...
def get_period_info(hour=None, weekday=None):
    ...
    if 0 <= hour < 8:
        ...
        return {"period": "LLANO", "emoji": "⚡", "description": "Periodo llano"}

    else:  # 10-14 o 18-22
        return {"period": "PUNTA", "emoji": "🔴", "description": "Periodo punta"}
...
def get_next_valle():
    ...
    if weekday in [5, 6]:
        return {"starts_in_hours": 0, "description": "Ahora (fin de semana)"}
    ...
    if current_hour >= 8:
        ...
        return {"starts_in_hours": hours_until, "description": "00:00-08:00 (mañana)"}

    else:
        # Estamos en valle ahora

        return {"starts_in_hours": 0, "description": "Ahora (00:00-08:00)"}
...
def main():
    ...
    parser.add_argument("--now", action="store_true", help="Muestra periodo actual")

    parser.add_argument("--hour", type=int, help="Hora específica (0-23)")

    parser.add_argument("--weekday", type=int, help="Día de la semana (0=lun, 6=dom)")

    parser.add_argument("--all", action="store_true", help="Muestra todos los periodos del día")
