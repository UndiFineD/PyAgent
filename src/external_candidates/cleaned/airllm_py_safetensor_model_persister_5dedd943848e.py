# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\airllm.py\air_llm.py\airllm.py\persist.py\safetensor_model_persister_5dedd943848e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\airllm\air_llm\airllm\persist\safetensor_model_persister.py
...
class SafetensorModelPersister(ModelPersister):
    ...
    def model_persist_exist(self, layer_name, saving_path):
        ...
        safetensor_exists = os.path.exists(

            str(saving_path / (layer_name + "safetensors"))

        )

        done_marker_exists = os.path.exists(

            str(saving_path / (layer_name + "safetensors.done"))
    ...
    def load_model(self, layer_name, path):
        ...
        layer_state_dict = load_file(

            Path(path) / (layer_name + ".safetensors"), device="cpu"
