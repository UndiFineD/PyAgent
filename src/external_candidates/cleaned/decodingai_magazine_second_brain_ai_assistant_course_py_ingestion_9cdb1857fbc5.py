# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\workshops.py\rag.py\solution.py\src.py\rag_workshop.py\ingestion_9cdb1857fbc5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\workshops\rag\solution\src\rag_workshop\ingestion.py
...
def filter_documents(documents: list[Document]) -> list[Document]:
    ...
    valid_documents = [
        doc for doc in documents if "quality_score" not in doc.metadata or doc.metadata.get("quality_score", 0) >= 0.65
    ]

    logger.info(f"Documents passed after heuristic filter: {len(valid_documents)} / {len(documents)}")

    quality_judge = QualityScoreJudge(model_id=settings.OPENAI_MODEL_ID)

    valid_documents = quality_judge(valid_documents)

    valid_documents = [doc for doc in valid_documents if doc.metadata.get("quality_score", 0) > 0.2]

    logger.info(f"Documents passed after LLM filter: {len(valid_documents)} / {len(documents)}")
