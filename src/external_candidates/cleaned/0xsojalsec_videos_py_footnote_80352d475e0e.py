# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\eola.py\footnote_80352d475e0e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\footnote.py
...
class PutTogetherRotationMatrix(PutTogether3x3Matrix):
    CONFIG = {"col1": [0, 0, -1], "col2": [0, 1, 0], "col3": [1, 0, 0]}
...
class ShowVCoordinateMeaning(Scene):
    ...
    def construct(self):
        ...
        if self.post_transform:
            ...
            parens.stretch_to_fit_height(coords.get_height())
        ...
        lin_comb.arrange(RIGHT, buff=0.1, aligned_edge=ORIGIN if self.post_transform else DOWN)
...
class ShowMatrixVectorMultiplication(Scene):
    def construct(self):
        ...
        col1, col2, col3 = columns = [Matrix(col) for col in matrix.copy().get_mob_matrix().transpose()]

        coords = x, y, z = [m.copy() for m in vect.get_entries().split()]

        eq, plus1, plus2 = list(map(Tex, list("=++")))

        everything = VMobject(matrix, vect, eq, x, col1, plus1, y, col2, plus2, z, col3)
        ...
        for o_col, col, start_coord, coord, sym, color in sexts:
            ...
            self.play(start_coord.set_color, YELLOW, o_col.set_color, color)
