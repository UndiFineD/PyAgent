# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_voicecraft.py\config_82187db0540a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\config.py
...
def MyParser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="this is the effective batch size, no matter whether using gradient_accumulation_steps, not used if we specified max_num_tokens",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="max number of encodec tokens per gpu, this is only used when using dynamic batching, will ignore batch size. Note this is the final effective batch size per GPU, i.e. gradient accumulated batch size per gpu",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="used for dynamic batching, bucketing the samples based on the number of tokens",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="the value for torch.nn.utils.clip_grad_norm_(), not used if we use ScaledAdam optimizer",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3000,
        help="only use for Eden scheduler.",
    )
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        help="e.g. 'libritts', 'gigaspeech', they are folder name in the data dir also",
    )
    ...
    parser.add_argument(
        "--encodec_folder_name",
        type=str,
        default="encodec_16khz_4codebooks",
    ...
    parser.add_argument(
        ...
        type=float,
        default=20,
        help="in second, crop or drop the audio is length is longer than this",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="for my encodec that takes 16kHz audio with a downsample rate of 320, the codec sample rate is 50Hz, i.e. 50 codes (x n_codebooks) per second",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="if this is true, will drop example whose encodec sequence or phone sequence is too long, rather than cropping, to reduce hellucination",
    )
