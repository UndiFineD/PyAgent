# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenant_config_2a95dd4a18d3.py
...
class TenantConfig:
    ...
    def non_tenant_mode(self) -> bool:
        ...
        if self._non_tenant_mode is None:
            ...
            if self._non_tenant_mode:
                logger.info("🔧 Tenant mode disabled (NON_TENANT_MODE=true), traditional mode will be used")
    ...
    def mark_app_ready(self) -> None:
        ...
        if self._app_ready:
            logger.warning("⚠️ Application is already ready, mark_app_ready() called repeatedly")

            return

        self._app_ready = True

        logger.info("✅ Application startup completed, tenant strict check mode enabled")
