# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kulotzkih.py\lex.py\scripts.py\init_agent_6cbcca6bb1df.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kulotzkih\lex\scripts\init-agent.py
...
def create_agent(agent_name: str, description: str, template: str, output_dir: str):
    ...
    for filename, content in template_data["files"].items():
        ...
        formatted_content = content.format(agent_name=agent_name, description=description)
...
def main():
    parser = argparse.ArgumentParser(description="Initialize a new Warden Protocol agent")

    parser.add_argument("name", help="Agent name (e.g., 'my-warden-agent')")
