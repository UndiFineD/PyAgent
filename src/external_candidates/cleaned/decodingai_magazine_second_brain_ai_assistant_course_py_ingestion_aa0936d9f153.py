# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\workshops.py\rag.py\template.py\src.py\rag_workshop.py\ingestion_aa0936d9f153.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\workshops\rag\template\src\rag_workshop\ingestion.py
...
async def ingest_documents(
    ...
    await chunk_embed_load_documents(filtered_documents, embedding_model_id, embedding_model_dim, device)
...
def extract_documents(documents_dir: Path) -> list[Document]:
    ...
    for json_file in json_files:
        with json_file.open("r", encoding="utf-8") as f:
            ...
            documents.append(Document(page_content=document["content"], metadata=metadata))
...
async def chunk_embed_load_documents(
    ...
    with MongoDBService(model=Document, collection_name=settings.MONGODB_RAG_COLLECTION_NAME) as mongodb_client:
...
async def process_docs(
    ...
    tasks = [process_batch(retriever, batch, semaphore) for batch in get_batches(docs, batch_size)]
...
async def process_batch(
    ...
    async with semaphore:
        try:
            ...
            logger.warning(f"Error processing batch of {len(documents_batch)} documents: {str(e)}")
