...
async def test_db():
    ...
    async with db_client.get_session_context() as session:
        ...
        p_result = await session.execute(select(Project).where(Project.secret_key_hmac == FAKE_KEY))
    ...
    async with db_client.get_session_context() as session:
        ...
        se_query = await session.execute(select(Session).options(selectinload(Session.space)).where(Session.id == seid))
        ...
        text_query = await session.execute(
            select(Block).options(selectinload(Block.parent)).where(Block.id == text_block.id)
        )
        ...
        children_query = await session.execute(select(Block).where(Block.parent_id == page_block.id))
