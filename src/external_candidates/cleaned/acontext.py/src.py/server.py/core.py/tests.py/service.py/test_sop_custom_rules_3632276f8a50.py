# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\tests\service\test_sop_custom_rules.py
"""
...
class TestSOPCustomRules:
    ...
    async def test_custom_rules_storage_and_loading(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            project_config = ProjectConfig(sop_agent_custom_scoring_rules=custom_rules)
            project.configs = {"project_config": json.loads(project_config.model_dump_json())}
            ...
            assert (
                loaded_config.sop_agent_custom_scoring_rules[0].description
                == "If the task involves database operations"
            )
            assert loaded_config.sop_agent_custom_scoring_rules[0].level == "normal"
            assert (
                loaded_config.sop_agent_custom_scoring_rules[1].description == "If the task requires external API calls"
            )
    ...
    async def test_custom_rules_prompt_generation(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            customization = SOPPromptCustomization(custom_scoring_rules=loaded_config.sop_agent_custom_scoring_rules)
