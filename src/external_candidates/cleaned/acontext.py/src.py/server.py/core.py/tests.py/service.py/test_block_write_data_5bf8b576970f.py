...
class TestPageBlock:
    ...
    async def test_create_new_page_with_props(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Page_with_Props", props=props)
            assert r.ok()
            page_id = r.data.id
    ...
    async def test_create_new_page_with_parent(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create parent page
            r = await create_new_path_block(session, space.id, "Parent_Page", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            parent_id = r.data.id
            ...
            child_ids = []
            for i in range(2):
                r = await create_new_path_block(session, space.id, f"Child_Page_{i}", par_block_id=parent_id)
                assert r.ok()
    ...
    async def test_create_new_page_invalid_parent(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Test_Page", par_block_id=fake_parent_id)
            assert not r.ok()
...
class TestSOPBlock:
    ...
    async def test_write_sop_reuses_existing_tool_reference(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            sop_data = SOPData(
                ...
                preferences="",
                tool_sops=[SOPStep(tool_name="existing_tool", action="run with param=value")],
            )
            ...
            query = select(func.count(ToolReference.id)).where(ToolReference.project_id == project.id)
            result = await session.execute(query)
            count = result.scalar()
            assert count == 1  # Still only one tool reference

            # Verify ToolSOP uses existing reference
            query = select(ToolSOP).join(ToolReference).where(ToolReference.name == "existing_tool")
            result = await session.execute(query)
    ...
    async def test_write_sop_with_after_block_index(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await write_sop_block_to_parent(session, space.id, parent_id, new_sop_data, after_block_index=2)
            assert r.ok()
            new_sop_id = r.data
...
class TestFindBlockSort:
    ...
    async def test_find_block_sort_no_parent(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await _find_block_sort(session, space.id, None, block_type=BLOCK_TYPE_PAGE)
            assert r.ok()
            assert r.unpack()[0] == 0
    ...
    async def test_find_block_sort_with_parent(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create parent
            r = await create_new_path_block(session, space.id, "Parent", type=BLOCK_TYPE_FOLDER)
            parent_id = r.data.id
    ...
    async def test_find_block_sort_invalid_parent(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            fake_parent_id = uuid.uuid4()
            r = await _find_block_sort(session, space.id, fake_parent_id, BLOCK_TYPE_PAGE)
            assert not r.ok()
            assert "not found" in r.error.errmsg.lower()
...
class TestFolderBlock:
    ...
    async def test_create_new_folder_success(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            folder_ids = []
            for i in range(3):
                r = await create_new_path_block(session, space.id, f"Test_Folder_{i}", type=BLOCK_TYPE_FOLDER)
    ...
    async def test_create_nested_folders(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Parent_Folder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            parent_id = r.data.id
    ...
    async def test_create_page_in_folder(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create folder
            r = await create_new_path_block(session, space.id, "Documents", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder_id = r.data.id
    ...
    async def test_create_folder_with_props(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Special_Folder", props=props, type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder_id = r.data.id
...
class TestBlockParentChildRelationships:
    ...
    async def test_page_with_page_parent_fails(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create parent page
            r = await create_new_path_block(session, space.id, "Parent_Page", type=BLOCK_TYPE_PAGE)
            assert r.ok()
            page_id = r.data.id
    ...
    async def test_text_block_with_root_parent_fails(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Root_Text_Block", type=BLOCK_TYPE_TEXT)
            assert not r.ok()
...
class TestMovePathBlock:
    ...
    async def test_move_page_to_folder_success(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Folder2", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            ...
            r = await move_path_block_to_new_parent(session, space.id, page_id, folder2_id)
            assert r.ok()
    ...
    async def test_move_folder_to_folder_success(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "ParentFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            parent_folder_id = r.data.id

            r = await create_new_path_block(session, space.id, "ChildFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            ...
            r = await move_path_block_to_new_parent(session, space.id, child_folder_id, parent_folder_id)
            assert r.ok()
    ...
    async def test_move_page_block_not_found(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await move_path_block_to_new_parent(session, space.id, fake_page_id, folder_id)
            assert not r.ok()
            assert "not found" in r.error.errmsg.lower()
    ...
    async def test_move_invalid_block_type(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "TargetFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder_id = r.data.id
            ...
            r = await move_path_block_to_new_parent(session, space.id, sop_id, folder_id)
            assert not r.ok()
    ...
    async def test_move_page_to_non_folder_fails(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create two pages
            r = await create_new_path_block(session, space.id, "Page1", type=BLOCK_TYPE_PAGE)
            assert r.ok()
            page1_id = r.data.id

            r = await create_new_path_block(session, space.id, "Page2", type=BLOCK_TYPE_PAGE)
            assert r.ok()
            page2_id = r.data.id

            # Try to move Page2 into Page1 (should fail - pages can't be parents)
            r = await move_path_block_to_new_parent(session, space.id, page2_id, page1_id)
            assert not r.ok()
    ...
    async def test_move_page_to_nonexistent_parent_fails(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            fake_parent_id = uuid.uuid4()
            r = await move_path_block_to_new_parent(session, space.id, page_id, fake_parent_id)
            assert not r.ok()
    ...
    async def test_move_folder_to_its_own_child_fails(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await move_path_block_to_new_parent(session, space.id, parent_id, child_id)
            assert not r.ok()
            assert "cycle" in r.error.errmsg.lower()

            # Try to move Parent into Grandchild (would create cycle)
            r = await move_path_block_to_new_parent(session, space.id, parent_id, grandchild_id)
            assert not r.ok()
            assert "cycle" in r.error.errmsg.lower()
    ...
    async def test_move_folder_to_itself_fails(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create a folder
            r = await create_new_path_block(session, space.id, "Folder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder_id = r.data.id

            # Try to move folder into itself (cycle detection)
            r = await move_path_block_to_new_parent(session, space.id, folder_id, folder_id)
            assert not r.ok()
            assert "cycle" in r.error.errmsg.lower()
    ...
    async def test_move_page_with_children(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await move_path_block_to_new_parent(session, space.id, folder1_id, folder2_id)
            assert r.ok()
    ...
    async def test_move_multiple_pages_to_same_folder(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            page_ids = []
            for i in range(3):
                r = await create_new_path_block(session, space.id, f"Page{i}", type=BLOCK_TYPE_PAGE)
                assert r.ok()
                page_ids.append(r.data.id)

            # Move all pages to TargetFolder
            for page_id in page_ids[::-1]:
                r = await move_path_block_to_new_parent(session, space.id, page_id, target_folder_id)
                assert r.ok()
    ...
    async def test_move_pages_to_same_folder_with_sort(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Page 0", type=BLOCK_TYPE_PAGE)
            assert r.ok()
    ...
    async def test_move_page_updates_original_parent_children_sort(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "SourceFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            ...
            r = await move_path_block_to_new_parent(session, space.id, page_ids[1], target_folder_id)
            assert r.ok()
            ...
            assert moved_page.sort == 0, "Moved page should be first child in target folder"
...
class TestDeleteBlock:
    ...
    async def test_delete_folder_with_children(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            child_ids = []
            for i in range(3):
                r = await create_new_path_block(session, space.id, f"ChildPage_{i}", par_block_id=folder_id)
                assert r.ok()
                child_ids.append(r.data.id)
            ...
            for block_id in [folder_id] + child_ids:
                query = select(BlockEmbedding).where(BlockEmbedding.block_id == block_id)
                result = await session.execute(query)
                embeddings = result.scalars().all()
                assert len(embeddings) == 0
    ...
    async def test_delete_nested_folder_structure(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create root folder
            r = await create_new_path_block(session, space.id, "RootFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            ...
            r = await create_new_path_block(session, space.id, "RootPage", par_block_id=root_folder_id)
            assert r.ok()
            root_page_id = r.data.id

            r = await create_new_path_block(session, space.id, "SubPage1", par_block_id=sub_folder1_id)
            assert r.ok()
            sub_page1_id = r.data.id

            r = await create_new_path_block(session, space.id, "SubPage2", par_block_id=sub_folder2_id)
            assert r.ok()
    ...
    async def test_delete_block_sort_order_adjustment(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            page_ids = []
            for i in range(5):
                r = await create_new_path_block(session, space.id, f"Page_{i}", par_block_id=folder_id)
                assert r.ok()
                page_ids.append(r.data.id)
