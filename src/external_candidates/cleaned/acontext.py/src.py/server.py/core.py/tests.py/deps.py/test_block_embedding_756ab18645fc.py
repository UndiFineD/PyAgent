...
async def test_block_embedding_create_and_basic_queries():
    ...
    async with db_client.get_session_context() as session:
        ...
        try:
            ...
            result = await session.execute(select(BlockEmbedding).where(BlockEmbedding.block_id == page_block.id))
            ...
            result = await session.execute(select(BlockEmbedding).where(BlockEmbedding.space_id == space.id))
...
async def test_block_embedding_relationships():
    ...
    async with db_client.get_session_context() as session:
        # Enable pgvector extension
        await session.execute(text("CREATE EXTENSION IF NOT EXISTS vector;"))
        project = Project(secret_key_hmac=FAKE_KEY + "1", secret_key_hash_phc=FAKE_KEY + "1")
        session.add(project)
        await session.flush()
    ...
    async with db_client.get_session_context() as session:
        ...
        try:
            ...
            result = await session.execute(
                select(Block).options(selectinload(Block.embeddings)).where(Block.id == block.id)
            )
            ...
            result = await session.execute(
                select(Space).options(selectinload(Space.block_embeddings)).where(Space.id == space.id)
            )
...
async def test_block_embedding_vector_similarity_search():
    ...
    async with db_client.get_session_context() as session:
        ...
        # Create test data
        project = Project(secret_key_hmac=FAKE_KEY + "2", secret_key_hash_phc=FAKE_KEY + "2")
        session.add(project)
        await session.flush()
...
async def test_block_embedding_cascade_delete():
    ...
    async with db_client.get_session_context() as session:
        ...
        # Create test data
        project = Project(secret_key_hmac=FAKE_KEY + "3", secret_key_hash_phc=FAKE_KEY + "3")
        session.add(project)
        await session.flush()
        ...
        result = await session.execute(select(BlockEmbedding).where(BlockEmbedding.id == embedding_id))
        ...
        # Verify embedding was deleted
        result = await session.execute(select(BlockEmbedding).where(BlockEmbedding.id == embedding2_id))
        assert result.scalar_one_or_none() is None
...
async def test_block_embedding_multiple_embeddings_per_block():
    ...
    async with db_client.get_session_context() as session:
        ...
        # Create test data
        project = Project(secret_key_hmac=FAKE_KEY + "4", secret_key_hash_phc=FAKE_KEY + "4")
        session.add(project)
        await session.flush()
    ...
    async with db_client.get_session_context() as session:
        ...
        try:
            ...
            print(f"✓ Multiple embeddings per block test passed: {len(loaded_block.embeddings)} embeddings")
