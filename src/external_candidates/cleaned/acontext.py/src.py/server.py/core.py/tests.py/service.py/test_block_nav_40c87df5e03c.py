# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\tests\service\test_block_nav.py
import pytest
...
class TestBlockNav:
    ...
    async def test_list_paths_under_block_basic(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create Folder1
            r = await create_new_path_block(session, space.id, "Folder1", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder1_id = r.data.id

            # Create Page1 and Page2 under Folder1
            r = await create_new_path_block(session, space.id, "Page1", par_block_id=folder1_id)
            assert r.ok()
            page1_id = r.data.id

            r = await create_new_path_block(session, space.id, "Page2", par_block_id=folder1_id)
            assert r.ok()
            page2_id = r.data.id
            ...
            r = await create_new_path_block(session, space.id, "Page3", par_block_id=subfolder_id)
            assert r.ok()
            page3_id = r.data.id
            ...
            r = await list_paths_under_block(session, space.id, folder1_id, "Folder1/", depth=0)
            assert r.ok()
            paths, sub_page_num, sub_folder_num = r.data
            ...
            r = await list_paths_under_block(session, space.id, folder1_id, "Folder1/", depth=1)
            assert r.ok()
            paths, sub_page_num, sub_folder_num = r.data
            ...
            assert r.ok()
            dir5_id = r.data.id
            r = await create_new_path_block(session, space.id, "Page5", par_block_id=dir5_id)
            assert r.ok()
            r = await create_new_path_block(session, space.id, "Dir6", par_block_id=dir5_id, type=BLOCK_TYPE_FOLDER)
            assert r.ok()
    ...
    async def test_get_path_info_by_id_basic(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            # Create TestFolder
            r = await create_new_path_block(session, space.id, "TestFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder_id = r.data.id

            # Create TestPage under TestFolder
            r = await create_new_path_block(session, space.id, "TestPage", par_block_id=folder_id)
            assert r.ok()
            page_id = r.data.id
    ...
    async def test_read_blocks_from_par_id(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "ParentPage", type=BLOCK_TYPE_PAGE)
            assert r.ok()
            parent_id = r.data.id
            ...
            r = await read_blocks_from_par_id(session, space.id, parent_id, allowed_types={BLOCK_TYPE_TEXT})
            assert r.ok()
            blocks = r.data
            ...
            r = await read_blocks_from_par_id(session, space.id, parent_id, allowed_types={BLOCK_TYPE_SOP})
            assert r.ok()
            blocks = r.data
            ...
            r = await read_blocks_from_par_id(session, space.id, parent_id, allowed_types={BLOCK_TYPE_REFERENCE})
            assert r.ok()
            blocks = r.data
            ...
            r = await read_blocks_from_par_id(session, space.id, parent_id, allowed_types=set())
            assert r.ok()
            blocks = r.data
