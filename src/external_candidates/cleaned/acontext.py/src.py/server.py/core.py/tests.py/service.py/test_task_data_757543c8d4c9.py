# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\tests\service\test_task_data.py
import uuid
...
class TestFetchCurrentTasks:
    ...
    async def test_fetch_all_tasks_success(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac", secret_key_hash_phc="test_key_hash")
            session.add(project)
            await session.flush()
    ...
    async def test_fetch_tasks_with_status_filter(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            existing = await session.execute(select(Project).where(Project.secret_key_hmac == "test_key_hmac2"))
            ...
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac2", secret_key_hash_phc="test_key_hash2")
            session.add(project)
            await session.flush()
            ...
            result = await fetch_current_tasks(session, test_session.id, status="pending")
...
class TestUpdateTask:
    ...
    async def test_update_status_success(self):
        ...
        async with db_client.get_session_context() as session:
            # Clean up any existing project with this key
            existing = await session.execute(select(Project).where(Project.secret_key_hmac == "test_key_hmac3"))
            existing_project = existing.scalars().first()
            if existing_project:
                await session.delete(existing_project)
                await session.flush()
            ...
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac3", secret_key_hash_phc="test_key_hash3")
            session.add(project)
            await session.flush()
    ...
    async def test_update_order_success(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac4", secret_key_hash_phc="test_key_hash4")
            session.add(project)
            await session.flush()
    ...
    async def test_update_data_success(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac5", secret_key_hash_phc="test_key_hash5")
            session.add(project)
            await session.flush()
    ...
    async def test_update_multiple_fields(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac6", secret_key_hash_phc="test_key_hash6")
            session.add(project)
            await session.flush()
            ...
            result = await update_task(session, task.id, status=new_status, order=new_order, data=new_data)
    ...
    async def test_update_task_with_none_values(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac7", secret_key_hash_phc="test_key_hash7")
            session.add(project)
            await session.flush()
            ...
            result = await update_task(session, task.id, status=None, order=None, data=None)
    ...
    async def test_update_task_patch_data_success(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            result = await update_task(session, task.id, data=complete_new_data, patch_data=patch_data)
    ...
    async def test_update_task_patch_data_with_status_and_order(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            result = await update_task(session, task.id, status="running", order=5, patch_data=patch_data)
...
class TestInsertTask:
    ...
    async def test_insert_task_success(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac8", secret_key_hash_phc="test_key_hash8")
            session.add(project)
            await session.flush()
            ...
            result = await insert_task(session, project.id, test_session.id, after_order, data)
    ...
    async def test_insert_task_with_custom_status(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac9", secret_key_hash_phc="test_key_hash9")
            session.add(project)
            await session.flush()
    ...
    async def test_insert_task_default_status(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac10", secret_key_hash_phc="test_key_hash10")
            session.add(project)
            await session.flush()
    ...
    async def test_insert_task_complex_data(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac11", secret_key_hash_phc="test_key_hash11")
            session.add(project)
            await session.flush()
            ...
            result = await insert_task(session, project.id, test_session.id, 0, complex_data)
    ...
    async def test_insert_order_increment(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            result = await insert_task(session, project.id, test_session.id, 1, new_data)
...
class TestDeleteTask:
    ...
    async def test_delete_task_success(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac12", secret_key_hash_phc="test_key_hash12")
            session.add(project)
            await session.flush()
    ...
    async def test_delete_task_cascade_behavior(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac13", secret_key_hash_phc="test_key_hash13")
            session.add(project)
            await session.flush()
            ...
            count_query = select(func.count(Task.id)).where(Task.session_id == task_to_delete.session_id)
...
class TestIntegrationScenarios:
    ...
    async def test_full_task_lifecycle(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac14", secret_key_hash_phc="test_key_hash14")
            session.add(project)
            await session.flush()
            ...
            create_result = await insert_task(session, project.id, test_session.id, 0, initial_data)
            ...
            update_result = await update_task(session, task_id, data=updated_data, status="running")
    ...
    async def test_multiple_sessions_isolation(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            project = Project(secret_key_hmac="test_key_hmac15", secret_key_hash_phc="test_key_hash15")
            session.add(project)
            await session.flush()
    ...
    async def test_ordering_after_updates(self):
        ...
        async with db_client.get_session_context() as session:
            # Create test data
            project = Project(secret_key_hmac="test_key_hmac16", secret_key_hash_phc="test_key_hash16")
            session.add(project)
            await session.flush()
...
class TestAppendProgressToTask:
    ...
    async def test_append_progress_task_not_found(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            result = await append_progress_to_task(session, fake_task_id, "Some progress")
...
class TestAppendSopThinkingToTask:
    ...
    async def test_append_sop_thinking_task_not_found(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            result = await append_sop_thinking_to_task(session, fake_task_id, "Some thinking")
