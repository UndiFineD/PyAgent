# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\tests\service\test_ctx_find_block.py
import pytest
...
class TestSpaceCtxFindBlock:
    ...
    async def test_find_nested_folders(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Level1", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            level1_id = r.data.id
    ...
    async def test_find_page_in_nested_folders(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "Projects", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            ...
            r = await create_new_path_block(session, space.id, "Q4", par_block_id=year_id, type=BLOCK_TYPE_FOLDER)
            assert r.ok()
    ...
    async def test_find_partial_path_not_exists(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "ExistingFolder", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
    ...
    async def test_find_block_with_spaces_in_name(self):
        ...
        async with db_client.get_session_context() as session:
            ...
            r = await create_new_path_block(session, space.id, "My Documents", type=BLOCK_TYPE_FOLDER)
            assert r.ok()
            folder_id = r.data.id
