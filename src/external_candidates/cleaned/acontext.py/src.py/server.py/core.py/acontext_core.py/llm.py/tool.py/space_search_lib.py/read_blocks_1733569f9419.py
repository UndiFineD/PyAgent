# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\tool\space_search_lib\read_blocks.py
import json
...
async def _read_blocks_handler(
    ...
    if page_block.type != BLOCK_TYPE_PAGE:
        return Result.resolve(f"Path {page_path} is not a page (type: {page_block.type})")
    ...
    if not rendered_blocks:
        return Result.resolve(
            f"Page '{page_path}' has {total_blocks} blocks. No blocks found in range [{offset}, {offset + limit})."
        )

    blocks_display = "\n".join([json.dumps(block, ensure_ascii=False) for block in rendered_blocks])
