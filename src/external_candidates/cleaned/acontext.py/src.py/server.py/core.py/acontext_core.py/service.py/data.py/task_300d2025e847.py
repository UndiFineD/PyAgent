...
async def fetch_planning_task(db_session: AsyncSession, session_id: asUUID) -> Result[TaskSchema | None]:
    query = (
        select(Task)
    ...
    return Result.resolve(
        TaskSchema(
            ...
            raw_message_ids=[msg.id for msg in sorted(planning.messages, key=lambda m: m.created_at)],
        )
    )
...
async def fetch_task(db_session: AsyncSession, task_id: asUUID) -> Result[TaskSchema]:
    ...
    return Result.resolve(
        TaskSchema(
            ...
            raw_message_ids=[msg.id for msg in sorted(task.messages, key=lambda m: m.created_at)],
        )
    )
...
async def fetch_current_tasks(
    ...
    tasks_d = [
        TaskSchema(
            ...
            raw_message_ids=[msg.id for msg in sorted(t.messages, key=lambda m: m.created_at)],
        )
        for t in tasks
    ]
...
async def insert_task(
    ...
    lock_query = (
        select(Task.id).where(Task.session_id == session_id).with_for_update()  # This locks the rows
    )
    ...
    temp_update_stmt = (
        update(Task).where(Task.session_id == session_id).where(Task.order > after_order).values(order=-Task.order)
    )
    ...
    final_update_stmt = (
        update(Task).where(Task.session_id == session_id).where(Task.order < 0).values(order=-Task.order + 1)
    )
...
async def append_messages_to_task(
    ...
    await db_session.execute(update(Message).where(Message.id.in_(message_ids)).values(task_id=task_id))
    await db_session.flush()
    return Result.resolve(None)
...
async def append_messages_to_planning_section(
    ...
    # set those messages' task_id to task_id
    query = (
        select(Task).where(Task.session_id == session_id).where(Task.is_planning == True)  # noqa: E712
    )
    result = await db_session.execute(query)
...
async def append_sop_thinking_to_task(
    ...
    stmt = update(Task).where(Task.id == task_id).values(data=Task.data.op("||")({"sop_thinking": thinking}))
