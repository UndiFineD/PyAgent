# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\infra\redis.py
...
class RedisClient:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        if not self.redis_url:
            raise ValueError("Redis URL could not be constructed from environment variables")
...
async def init_redis() -> None:
    ...
    if await REDIS_CLIENT.health_check():
        logger.info(f"Redis connection initialized successfully {REDIS_CLIENT.get_pool_status()}")
    else:
