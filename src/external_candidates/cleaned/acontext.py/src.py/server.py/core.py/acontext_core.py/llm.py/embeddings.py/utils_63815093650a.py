# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\embeddings\utils.py
...
def get_openai_async_client_instance() -> AsyncOpenAI:
    ...
    if _global_openai_async_client is None:
        _global_openai_async_client = AsyncOpenAI(
            base_url=DEFAULT_CORE_CONFIG.block_embedding_base_url or DEFAULT_CORE_CONFIG.llm_base_url,
            api_key=DEFAULT_CORE_CONFIG.block_embedding_api_key or DEFAULT_CORE_CONFIG.llm_api_key,
        )
...
def get_jina_async_client_instance() -> AsyncClient:
    ...
    assert DEFAULT_CORE_CONFIG.block_embedding_base_url is not None, "Jina base URL is not set"
    assert DEFAULT_CORE_CONFIG.block_embedding_api_key is not None, "Jina API key is not set"
    if _global_jina_async_client is None:
        _global_jina_async_client = AsyncClient(
            ...
            headers={"Authorization": f"Bearer {DEFAULT_CORE_CONFIG.block_embedding_api_key}"},
        )
...
def get_lmstudio_async_client_instance() -> AsyncClient:
    ...
    assert DEFAULT_CORE_CONFIG.block_embedding_base_url is not None, "LMStudio base URL is not set"
    assert DEFAULT_CORE_CONFIG.block_embedding_api_key is not None, "LMStudio API key is not set"
