# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\agent\task.py
...
from ..prompt.task import TASK_TOOLS, TaskPrompt
...
def pack_previous_progress_section(
    ...
    for task in tasks[::-1]:
        ...
        if task.data.progresses is not None:
            progresses.extend([f"Task {task.order}: {p}" for p in task.data.progresses[-max_taken:]][::-1])
...
def pack_previous_messages_section(
    ...
    for ti in task_ids:
        if ti is None:
            ...
            task_descs.append("(no task linked)")
    return "\n---\n".join(
        [f"{td}\n{m.to_string(tool_mappings, truncate_chars=256)}" for td, m in zip(task_descs, messages)]
    )
...
async def build_task_ctx(
    ...
    if eil:
        return r
    LOG.debug(f"Built task context {[(t.order, t.status.value, t.data.task_description) for t in current_tasks]}")
    use_ctx = TaskCtx(
...
async def task_agent_curd(
    ...
    previous_progress_section = pack_previous_progress_section(tasks, previous_progress_num)
    ...
    _messages = [
        {
            "role": "user",
            "content": TaskPrompt.pack_task_input(previous_progress_section, current_messages_section, task_section),
        }
    ]
