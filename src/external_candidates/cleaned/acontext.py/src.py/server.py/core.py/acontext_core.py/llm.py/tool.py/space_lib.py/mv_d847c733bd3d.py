# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\tool\space_lib\mv.py
...
async def _move_handler(
    ...
    if not r.ok():
        return Result.resolve(f"Destination folder {to_folder} not found, with error {r.error}")
    ...
    r = await BK.move_path_block_to_new_parent(ctx.db_session, ctx.space_id, from_path_block.id, to_path_block.id)
    if not r.ok():
        return Result.resolve(f"Unable to move '{from_path}' to '{to_folder}', with error {r.error}")
