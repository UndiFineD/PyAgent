# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\session_message.py
import asyncio
...
async def waiting_for_message_notify(wait_for_seconds: int, body: InsertNewMessage):
    LOG.info(f"Session message buffer is not full, wait {wait_for_seconds} seconds for next turn/idle notify")
...
async def insert_new_message(body: InsertNewMessage, message: Message):
    ...
    async with DB_CLIENT.get_session_context() as read_session:
        ...
        if body.message_id != latest_pending_message_id:
            LOG.debug(f"Message {body.message_id} is not the latest pending message, ignore")
            return
        ...
        if eil:
            return
        if pending_message_length < project_config.project_session_message_buffer_max_turns:
            asyncio.create_task(
                waiting_for_message_notify(project_config.project_session_message_buffer_ttl_seconds, body)
            )
            return

    _l = await check_redis_lock_or_set(body.project_id, f"session.message.insert.{body.session_id}")
    if not _l:
        LOG.debug(
    ...
    try:
        LOG.info(f"Session message buffer is full (size: {pending_message_length}), start process")
        ...
        await MC.process_session_pending_message(project_config, body.project_id, body.session_id)
    finally:
        await release_redis_lock(body.project_id, f"session.message.insert.{body.session_id}")
...
async def buffer_new_message(body: InsertNewMessage, message: Message):
    ...
    _l = await check_redis_lock_or_set(body.project_id, f"session.message.insert.{body.session_id}")
    if not _l:
        LOG.info(f"Current Session is locked, resend Message {body.message_id} to insert queue.")
        await MQ_CLIENT.publish(
            exchange_name=EX.session_message,
            routing_key=RK.session_message_insert_retry,
            body=body.model_dump_json(),
...
async def flush_session_message_blocking(project_id: asUUID, session_id: asUUID) -> Result[None]:
    while True:
        _l = await check_redis_lock_or_set(project_id, f"session.message.insert.{session_id}")
        if _l:
            break
        LOG.debug(
            f"Current Session is locked. "
        ...
        await asyncio.sleep(DEFAULT_CORE_CONFIG.session_message_session_lock_wait_seconds)
    ...
    try:
        async with DB_CLIENT.get_session_context() as read_session:
            ...
            if eil:
                return r
        r = await MC.process_session_pending_message(project_config, project_id, session_id)
        return r
    finally:
