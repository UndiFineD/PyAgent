...
class ConsumerConfig(ConsumerConfigData):
    ...
    handler: Optional[Callable[[BaseModel, Message], Awaitable[Any]] | SpecialHandler] = field(default=None)
    ...
    def __post_init__(self):
        ...
        if eil:
            raise ValueError(f"Handler function {self.handler} does not meet the sanity requirements:\n{eil}")
...
class AsyncSingleThreadMQConsumer:
    ...
    async def connect(self) -> None:
        ...
        try:
            self.connection = await connect_robust(
                ...
                client_properties={"connection_name": self.connection_config.connection_name},
            ...
            LOG.info(f"Connected to MQ (connection: {self.connection_config.connection_name})")
    ...
    def register_consumer(self, consumer_config: ConsumerConfig) -> None:
        ...
        if self.running:
            raise RuntimeError("Cannot register consumers while the consumer is running")
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    try:
                        ...
                        validated_body = config.body_pydantic_type.model_validate(payload)
                        _logging_vars = {k: payload.get(k, None) for k in LOGGING_FIELDS}
                        with bound_logging_vars(queue_name=config.queue_name, **_logging_vars):
                            ...
                            _end_s = perf_counter()
                            LOG.debug(f"Queue: {config.queue_name} processed in {_end_s - _start_s:.4f}s")
                    except ValidationError as e:
                        LOG.error(f"Message validation failed - queue: {config.queue_name}, error: {str(e)}")
                        await message.reject(requeue=False)
                        return
                    except asyncio.TimeoutError:
                        raise TimeoutError(f"Handler timeout after {config.timeout}s - queue: {config.queue_name}")
                    ...
                    if retry_count <= max_retries:
                        ...
                        LOG.error(
                            f"Message processing failed permanently - queue: {config.queue_name}, error: {str(e)}",
                            extra={"traceback": traceback.format_exc()},
                        )
                        # goto DLX if any
                        await message.reject(requeue=False)
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                if not self.connection or self.connection.is_closed:
                    LOG.warning(f"Connection lost for queue {config.queue_name}, reconnecting...")
                    await self.connect()
                ...
                if consumer_channel and not consumer_channel.is_closed:
                    try:
                        ...
                        LOG.debug(f"Closed consumer channel - queue: {config.queue_name}")
                    except Exception as e:
                        LOG.warning(f"Error closing channel - queue: {config.queue_name}: {e}")
    ...
    async def _setup_consumer_on_channel(
        ...
        # Declare exchange
        exchange = await channel.declare_exchange(config.exchange_name, config.exchange_type, durable=config.durable)
        ...
        if config.need_dlx_queue and config.use_dlx_ex_rk is None:
            ...
            dlx = await channel.declare_exchange(dlx_exchange_name, ExchangeType.DIRECT, durable=True)
    ...
    async def publish(self, exchange_name: str, routing_key: str, body: str) -> None:
        ...
        LOG.debug(f"Published message to exchange: {exchange_name}, routing_key: {routing_key}")
        ...
        try:
            ...
            while not self._shutdown_event.is_set():
                # special handlers maybe return earlier
                    ...
                    self._consumer_loop_tasks + [asyncio.create_task(self._shutdown_event.wait())],
                ...
                for task in done:
                    try:
                        r = task.result()  # This will raise the exception if task failed
                        if task in self._consumer_loop_tasks:
                            self._consumer_loop_tasks.remove(task)
                            LOG.info(f"Consumer task completed. {r}. Remaining tasks: {len(self._consumer_loop_tasks)}")
                    except Exception as e:
                        LOG.error(f"Consumer task failed: {e}. Remaining tasks: {len(self._consumer_loop_tasks)}")
                        return
...
def register_consumer(mq_client: AsyncSingleThreadMQConsumer, config: ConsumerConfigData):
