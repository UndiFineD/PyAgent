# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\data\block_search.py
...
from ...schema.orm.block import CONTENT_BLOCK, PATH_BLOCK
...
async def search_blocks(
    ...
    distance = BlockEmbedding.embedding.cosine_distance(query_embedding).label("distance")
    ...
    try:
        ...
        for row in rows:
            block, distance = cast(Tuple[Block, float], row)  # Unpack tuple: (Block, distance)
            if block.id in seen:
                continue
...
async def search_path_blocks(
    ...
    return await search_blocks(db_session, space_id, query_text, list(PATH_BLOCK), topk, threshold, fetch_ratio)
