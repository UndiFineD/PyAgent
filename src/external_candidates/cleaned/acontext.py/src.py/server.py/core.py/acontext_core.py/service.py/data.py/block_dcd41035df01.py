# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\data\block.py
...
from .block_nav import _normalize_path_block_title, assert_block_type
...
async def find_all_parent_ids(
    ...
    while block_id is not None:
        query = select(Block.parent_id).where(Block.space_id == space_id, Block.id == block_id)
        result = await db_session.execute(query)
...
async def move_path_block_to_new_parent(
    ...
    r = await assert_block_type(db_session, space_id, new_par_block_id, BLOCK_TYPE_FOLDER)
    if not r.ok():
        return r
    ...
    if path_block_id in maybe_cycle_all_parent_ids:
        return Result.reject("Cycle detected, can't move a parent block to its child block")

    r = await _find_block_sort(db_session, space_id, new_par_block_id, block_type=BLOCK_TYPE_FOLDER)
    if not r.ok():
        return r
    ...
    r = await update_block_children_sort_by_delta(db_session, space_id, before_par_block_id, before_sort - 1, delta=-1)
    if not r.ok():
        return r
    await db_session.flush()
...
async def delete_block_recursively(
    ...
    r = await update_block_children_sort_by_delta(db_session, space_id, parent_id, block_sort - 1, delta=-1)
    if not r.ok():
        return r
