# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\schema\orm\block_embedding.py
...
class BlockEmbedding(CommonMixin):
    ...
    embedding: np.ndarray = field(
        metadata={
            "db": Column(
                Vector(DEFAULT_CORE_CONFIG.block_embedding_dim),  # Change dimension as needed
                nullable=False,
            )
        }
    )
...
async def check_legal_embedding_dim(cls: Type[BlockEmbedding], session: AsyncSession, embedding_dim):
    try:
        ...
        if result is None:
            raise ValueError("`embedding` column does not exist in the table, please check the table schema")
        ...
        LOG.info(f"Configuration embedding dimension ({embedding_dim}) matches database dimension ({actual_dim}). ")
        return actual_dim
