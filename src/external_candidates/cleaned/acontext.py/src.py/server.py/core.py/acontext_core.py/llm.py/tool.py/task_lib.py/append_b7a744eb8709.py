# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\tool\task_lib\append.py
...
async def _append_messages_to_task_handler(
    ...
    if task_order > len(ctx.task_ids_index) or task_order < 1:
        return Result.resolve(f"Task order {task_order} is out of range, appending failed.")
    ...
    if progress_note is not None:
        r = await TD.append_progress_to_task(ctx.db_session, actually_task_id, progress_note, user_preference or None)
        if not r.ok():
            return r
    ...
    return Result.resolve(f"Messages {message_order_indexes} and progress are appended to task {task_order}")
