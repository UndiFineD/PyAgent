# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\agent\task_sop.py
...
from ..prompt.task_sop import SOP_TOOLS, TaskSOPPrompt
from ..tool.sop_lib.ctx import SOPCtx


def pack_task_data(task: TaskSchema, message_blobs: list[MessageBlob]) -> tuple[str, str, str]:
    tool_mappings = {}
    return (
        task.data.task_description,
        "\n".join([f"- {p}" for p in (task.data.user_preferences or [])]),
        "\n".join([m.to_string(tool_mappings, truncate_chars=1024) for m in message_blobs]),
    )
...
def pack_previous_task_context(previous_tasks: List[TaskSchema], current_task: TaskSchema) -> str:
    prev_tasks = "\n".join([pack_one_task_progress_context(task) for task in previous_tasks])
    return f"""{prev_tasks}
...
async def sop_agent_curd(
    ...
    task_desc, user_perferences, raw_messages = pack_task_data(current_task, message_blobs)
    ...
    if project_config and project_config.sop_agent_custom_scoring_rules:
        customization = SOPPromptCustomization(custom_scoring_rules=project_config.sop_agent_custom_scoring_rules)
    ...
    _messages = [
        {
            "role": "user",
            "content": TaskSOPPrompt.pack_task_input(previous_task_context, task_desc, user_perferences, raw_messages),
        }
    ]
