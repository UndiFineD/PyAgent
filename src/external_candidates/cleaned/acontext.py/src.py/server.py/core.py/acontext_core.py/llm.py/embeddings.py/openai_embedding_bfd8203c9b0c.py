# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\embeddings\openai_embedding.py
import time
from typing import Literal

import numpy as np

from ...env import LOG
from ...schema.embedding import EmbeddingReturn
from .utils import get_openai_async_client_instance


async def openai_embedding(
    model: str, texts: list[str], phase: Literal["query", "document"] = "document"
) -> EmbeddingReturn:
    openai_async_client = get_openai_async_client_instance()

    _start_s = time.perf_counter()
    response = await openai_async_client.embeddings.create(model=model, input=texts, encoding_format="float")
    _end_s = time.perf_counter()

    prompt_tokens = getattr(response.usage, "prompt_tokens", None)
    total_tokens = getattr(response.usage, "total_tokens", None)
    LOG.info(f"OpenAI embedding, {model}, {phase}, {prompt_tokens}/{total_tokens}, time {_end_s - _start_s:.4f}s")
    return EmbeddingReturn(
        embedding=np.array([dp.embedding for dp in response.data]),
        prompt_tokens=prompt_tokens,
        total_tokens=total_tokens,
    )
