# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\controller\space_task.py
from ...env import LOG
...
async def process_space_task(
    ...
    async with DB_CLIENT.get_session_context() as db_session:
        ...
        if not r.ok():
            return
        messages, _ = r.unpack()
        messages_data = [MessageBlob(message_id=m.id, role=m.role, parts=m.parts, task_id=m.task_id) for m in messages]
    async with DB_CLIENT.get_session_context() as db_session:
