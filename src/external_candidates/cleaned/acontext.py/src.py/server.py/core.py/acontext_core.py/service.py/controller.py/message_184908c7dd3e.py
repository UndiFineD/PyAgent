# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\controller\message.py
from ...env import LOG
...
async def process_session_pending_message(
    ...
    try:
        async with DB_CLIENT.get_session_context() as session:
            ...
            await MD.update_message_status_to(session, pending_message_ids, TaskStatus.RUNNING)
        ...
        async with DB_CLIENT.get_session_context() as session:
            ...
            messages_data = [
                MessageBlob(message_id=m.id, role=m.role, parts=m.parts, task_id=m.task_id) for m in messages
            ]
        ...
        async with DB_CLIENT.get_session_context() as session:
            await MD.update_message_status_to(session, pending_message_ids, after_status)
        return r
        ...
        async with DB_CLIENT.get_session_context() as session:
            await MD.update_message_status_to(session, pending_message_ids, TaskStatus.FAILED)
        raise e
