# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\schema\orm\message.py
import json
...
class Message(CommonMixin):
    ...
    # Relationships
    session: "Session" = field(init=False, metadata={"db": relationship("Session", back_populates="messages")})

    parent: Optional["Message"] = field(
        default=None,
        init=False,
        metadata={"db": relationship("Message", remote_side="Message.id", back_populates="children")},
    )

    children: List["Message"] = field(
        default_factory=list,
        init=False,
        metadata={"db": relationship("Message", back_populates="parent", cascade="all, delete-orphan")},
    )
