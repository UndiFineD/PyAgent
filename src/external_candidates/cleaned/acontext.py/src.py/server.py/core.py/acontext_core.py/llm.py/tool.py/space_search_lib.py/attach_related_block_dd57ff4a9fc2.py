# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\tool\space_search_lib\attach_related_block.py
...
async def _attach_related_block_handler(
    ...
    if page_block.type != BLOCK_TYPE_PAGE:
        return Result.resolve(f"Page {page_path} is not a page (type: {page_block.type})")
    r = await BN.get_block_by_sort(ctx.db_session, ctx.space_id, page_block.id, block_index - 1)
    if not r.ok():
    ...
    return Result.resolve(f"Attached the block, you have attached {len(ctx.located_content_blocks)} blocks now")
