...
async def write_sop_block_to_parent(
    ...
    r = await _find_block_sort(db_session, space_id, par_block_id, block_type=BLOCK_TYPE_SOP)
    if not r.ok():
        return r
    next_sort = r.unpack()[0]
    if after_block_index is not None:
        if after_block_index < 0 or after_block_index > next_sort:
            return Result.reject(f"after_block_index out of range, it should be in [0, {next_sort}]")
    ...
    for i, sop_step in enumerate(sop_data.tool_sops):
        ...
        tool_ref_query = (
            select(ToolReference).where(ToolReference.project_id == project_id).where(ToolReference.name == tool_name)
        )
