# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\schema\session\message.py
import json
...
def pack_part_line(
    ...
    if part.type not in STRING_TYPES:
        ...
        if isinstance(tool_call_meta.arguments, str) and tool_call_meta.arguments.strip() != "":
...
class MessageBlob(BaseModel):
    ...
    def to_string(
        ...
        **kwargs,
        ...
        lines = [
            pack_part_line(self.role, p, tool_mapping, truncate_chars=truncate_chars, **kwargs) for p in self.parts
        ]
        return "\n".join(lines)
