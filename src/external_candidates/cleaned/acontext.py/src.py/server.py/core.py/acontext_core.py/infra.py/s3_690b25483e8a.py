# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\infra\s3.py
import asyncio
...
def _handle_s3_client_error(e: ClientError, bucket_name: str, key: str, ignore_not_found: bool = False) -> None:
    """
    ...
    if error_code in ("NoSuchKey", "404") and ignore_not_found:
        ...
        logger.error(f"S3 client error - bucket: {bucket_name}, key: {key}, error: {error_code}")
    raise e
...
def _handle_unexpected_error(e: Exception, bucket_name: str, key: str) -> None:
    ...
    Args:
        ...
        key: S3 object key
    """
    logger.error(f"Unexpected S3 error - bucket: {bucket_name}, key: {key}, error: {str(e)}")
    raise e
...
class S3Client:
    ...
    def __init__(self, s3_config: Dict[str, Any] = {}):
        ...
        self.use_path_style = s3_config.get("use_path_style", DEFAULT_CORE_CONFIG.s3_use_path_style)
        self.max_pool_connections = s3_config.get("max_pool_connections", DEFAULT_CORE_CONFIG.s3_max_pool_connections)
        self.connection_timeout = s3_config.get("connection_timeout", DEFAULT_CORE_CONFIG.s3_connection_timeout)
        self.read_timeout = s3_config.get("read_timeout", DEFAULT_CORE_CONFIG.s3_read_timeout)

        if not self.bucket:
            raise ValueError("S3 bucket name is required")

        logger.info(f"S3 Client Config - Region: {self.region}, Bucket: {self.bucket}, Endpoint: {self.endpoint}")
    ...
    async def _get_client(self) -> AioBaseClient:
        ...
        async with self.client_lock:
            ...
            self._client = await self._session.create_client(**client_kwargs).__aenter__()
    ...
    async def download_object(self, key: str, bucket: Optional[str] = None) -> bytes:
        ...
        try:
            async with self.get_client() as client:
                ...
                logger.debug(f"Downloaded object - bucket: {bucket_name}, key: {key}, size: {len(content)} bytes")
                return content
    ...
    async def upload_object(
        ...
        try:
            ...
            async with self.get_client() as client:
                ...
                logger.debug(f"Uploaded object - bucket: {bucket_name}, key: {key}, size: {len(data)} bytes")
                return result
    ...
    async def delete_object(self, key: str, bucket: Optional[str] = None) -> Dict[str, Any]:
        """
        Delete an S3 object.
    ...
    async def get_object_metadata(self, key: str, bucket: Optional[str] = None) -> Dict[str, Any] | None:
        """
        ...
        try:
            async with self.get_client() as client:
                ...
                # Remove ResponseMetadata as it's not useful for application logic
                metadata = {k: v for k, v in response.items() if k != "ResponseMetadata"}
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.error(f"S3 health check failed - bucket: {self.bucket}, error: {str(e)}")
            return False
...
async def init_s3() -> None:
    ...
    if await S3_CLIENT.health_check():
        logger.info(f"S3 client initialized successfully {S3_CLIENT.get_connection_status()}")
    else:
