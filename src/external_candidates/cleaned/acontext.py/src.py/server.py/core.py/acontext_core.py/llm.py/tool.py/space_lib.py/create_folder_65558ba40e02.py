# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\tool\space_lib\create_folder.py
...
async def _create_folder_handler(
    ...
    if path_block is not None and path_block.type != BLOCK_TYPE_FOLDER:
        return Result.resolve(f"Path {folder_path} is not a folder, can't have sub-folder")
