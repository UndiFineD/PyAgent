# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\data\block_render.py
...
async def render_sop_block(db_session: AsyncSession, space_id: asUUID, block: Block) -> Result[LLMRenderBlock]:
    ...
    for step in tool_sops:
        if step.tool_reference is None:
            ...
            LOG.warning(f"Tool reference {step.tool_reference_id} not found for step {step.id}")
            props = None
            break
        step_data = {
            "order": step.order,
...
async def render_text_block(db_session: AsyncSession, space_id: asUUID, block: Block) -> Result[LLMRenderBlock]:
    props = {
        "use_when": block.title,
...
async def render_content_block(db_session: AsyncSession, space_id: asUUID, block: Block) -> Result[LLMRenderBlock]:
