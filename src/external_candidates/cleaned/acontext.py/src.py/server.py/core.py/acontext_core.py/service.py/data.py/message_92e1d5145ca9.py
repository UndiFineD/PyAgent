...
async def session_message_length(db_session: AsyncSession, session_id: asUUID, status: str = "pending") -> Result[int]:
    """
...
async def fetch_messages_data_by_ids(db_session: AsyncSession, message_ids: List[asUUID]) -> Result[List[Message]]:
    """
    ...
    try:
        ...
        try:
            ...
            return Result.reject(f"Some messages({message_ids}) not found in database: {e}")

        if not ordered_messages:
            return Result.resolve([])

        # Fetch parts concurrently for all messages
        parts_tasks = [_fetch_message_parts(message.parts_asset_meta) for message in ordered_messages]
...
async def check_session_message_status(db_session: AsyncSession, message_id: asUUID) -> Result[str]:
    query = (
...
async def update_message_status_to(
    ...
    stmt = update(Message).where(Message.id.in_(message_ids)).values(session_task_process_status=status.value)
