# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\embeddings\__init__.py
...
async def embedding_sanity_check():
    ...
    if not r.ok():
        raise ValueError("Embedding API check failed! Make sure the embedding API key is valid.")
    d = r.data
...
async def get_embedding(
    ...
    try:
        results = await FACTORIES[DEFAULT_CORE_CONFIG.block_embedding_provider](model, texts, phase)
