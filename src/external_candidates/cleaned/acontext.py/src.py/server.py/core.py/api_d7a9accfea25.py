# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\api.py
import asyncio
from contextlib import asynccontextmanager
from typing import List, Optional

from acontext_core.di import DB_CLIENT, LOG, MQ_CLIENT, cleanup, setup
...
async def semantic_grep_search_func(
    ...
    search_threshold = (
        threshold if threshold is not None else DEFAULT_CORE_CONFIG.block_embedding_search_cosine_distance_threshold
    )
...
async def search_space(
    ...
    if mode == "fast":
        cited_blocks = await semantic_grep_search_func(semantic_threshold, space_id, query, limit)
        ...
        result = SpaceSearchResult(cited_blocks=cited_blocks, final_answer=r.data.final_answer)
        return result
    else:
...
async def insert_new_block(
    ...
    if request.type == BLOCK_TYPE_SOP:
        ...
        async with DB_CLIENT.get_session_context() as db_session:
            r = await BW.write_sop_block_to_parent(db_session, space_id, request.parent_id, sop_data)
            if not r.ok():
                raise HTTPException(status_code=500, detail=str(r.error))
        ...
        raise HTTPException(status_code=500, detail=f"Invalid block type: {request.type}")
    ...
    async with DB_CLIENT.get_session_context() as db_session:
        ...
        query = (
            select(
                ...
                func.sum(1 - cast(Task.space_digested, Integer)).label("not_digested_count"),
            )
