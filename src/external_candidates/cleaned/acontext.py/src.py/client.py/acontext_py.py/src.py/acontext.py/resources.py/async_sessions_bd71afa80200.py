# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\client\acontext-py\src\acontext\resources\async_sessions.py
...
class AsyncSessionsAPI:
    ...
    async def update_configs(
        ...
        *,
        ...
        """
        payload = {"configs": configs}
        await self._requester.request("PUT", f"/session/{session_id}/configs", json_data=payload)
    ...
    async def connect_to_space(self, session_id: str, *, space_id: str) -> None:
        ...
        """
        payload = {"space_id": space_id}
        await self._requester.request("POST", f"/session/{session_id}/connect_to_space", json_data=payload)
    ...
    async def send_message(
        ...
        *,
        ...
        if format != "acontext" and (file is not None or file_field is not None):
            raise ValueError("file and file_field parameters are only supported when format is 'acontext'")
    ...
    async def get_messages(
        ...
        *,
        ...
        data = await self._requester.request("GET", f"/session/{session_id}/messages", params=params or None)
    ...
    async def get_learning_status(self, session_id: str) -> LearningStatus:
        ...
        """
        data = await self._requester.request("GET", f"/session/{session_id}/get_learning_status")
    ...
    async def get_token_counts(self, session_id: str) -> TokenCounts:
        ...
        """
        data = await self._requester.request("GET", f"/session/{session_id}/token_counts")
