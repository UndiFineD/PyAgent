# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\client\acontext-py\examples\search_usage.py
"""
...
def example_semantic_glob(client: AcontextClient, space_id: str) -> None:
    ...
    for block in result:
        print(f"  - {block.title}")
        print(f"    ID: {block.block_id}, Type: {block.type}, Distance: {block.distance}")
...
async def example_async_search() -> None:
    ...
    async with AsyncAcontextClient(api_key=api_key, base_url=base_url) as client:
        ...
        exp_result, glob_result, grep_result = await asyncio.gather(exp_task, glob_task, grep_task)
