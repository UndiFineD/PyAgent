# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\graph.py\basegraph_d01b39768bdb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Graph\BaseGraph.py
...
class BaseGraph(ABC):
    ...
    async def _merge_nodes_then_upsert(self, entity_name: str, nodes_data: List[Entity]):
        ...
        merge_description = (
            MergeEntity.merge_descriptions(existing_data["description"], upsert_nodes_data["description"])
            if self.config.enable_entity_description
            else None
        )

        description = (
            await self._handle_entity_relation_summary(entity_name, merge_description) if merge_description else ""
        )

        source_id = MergeEntity.merge_source_ids(existing_data["source_id"], upsert_nodes_data["source_id"])

        new_entity_type = (
            MergeEntity.merge_types(existing_data["entity_type"], upsert_nodes_data["entity_type"])
    ...
    async def _merge_edges_then_upsert(self, src_id: str, tgt_id: str, edges_data: List[Relationship]) -> None:
        ...
        existing_edge = (
            await self._graph.get_edge(src_id, tgt_id) if await self._graph.has_edge(src_id, tgt_id) else None
        )
        ...
        source_id = MergeRelationship.merge_source_ids(existing_edge_data["source_id"], upsert_edge_data["source_id"])

        total_weight = MergeRelationship.merge_weight(existing_edge_data["weight"], upsert_edge_data["weight"])

        merge_description = (
            MergeRelationship.merge_descriptions(existing_edge_data["description"], upsert_edge_data["description"])
        ...
        keywords = (
            MergeRelationship.merge_keywords(existing_edge_data["keywords"], upsert_edge_data["keywords"])
            if self.config.enable_edge_keywords
            else ""
        )
    ...
    async def get_node_by_indices(self, node_idxs):
        return await asyncio.gather(*[self.get_node_by_index(node_idx) for node_idx in node_idxs])

    async def get_edge_by_indices(self, edge_idxs):
        return await asyncio.gather(*[self.get_edge_by_index(edge_idx) for edge_idx in edge_idxs])
    ...
    async def get_node_indices(self, node_keys):
        return await asyncio.gather(*[self.get_node_index(node_key) for node_key in node_keys])
