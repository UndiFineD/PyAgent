# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\summarization_agent_69ea97ec1941.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\summarization_agent.py
...
def _ensure_thread_pool():
    ...
    if _thread_pool is None:
        _thread_pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix="summarizer-loop")
...
def run_summarization_sync(prompt: str, message_history: List) -> List:
    ...
    prepared = prepare_prompt_for_model(model_name, _get_summarization_instructions(), prompt)
    ...
    try:
        ...
        def _worker(prompt_: str):
            return asyncio.run(_run_agent_async(agent, prompt_, message_history=message_history))
        ...
        result = asyncio.run(_run_agent_async(agent, prompt, message_history=message_history))
