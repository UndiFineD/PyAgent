# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\engine_external_media_3cd2df4b03c3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\engine_external_media.py
    ...
    async def _on_stasis_start(self, event: Dict[str, Any]):
        ...
        try:
            ...
            if channel_id in self.external_media_channels.values():
                logger.debug(
                    "Skipping StasisStart for External Media channel",
                    channel_id=channel_id,
                )
            ...
            if not caller_success or not external_success:
                logger.error(
                    "Failed to add channels to bridge",
                    channel_id=channel_id,
                    bridge_id=bridge_id,
