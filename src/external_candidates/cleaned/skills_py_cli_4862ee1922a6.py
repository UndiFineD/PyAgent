# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\hughpyle.py\keep.py\keep.py\cli_4862ee1922a6.py
...
def process_pending(
    ...
    daemon: Annotated[
        bool,
        typer.Option("--daemon", hidden=True, help="Run as background daemon (used internally)"),
    ] = False,
    output_json: JsonOption = False,
):
    """
    ...
    if all_items:
        ...
        if output_json:
            typer.echo(json.dumps({"processed": total_processed, "remaining": remaining}))
