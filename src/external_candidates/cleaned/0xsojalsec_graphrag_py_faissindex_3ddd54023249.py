# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\index.py\faissindex_3ddd54023249.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Index\FaissIndex.py
...
class FaissIndex(BaseIndex):
    ...
    async def _update_index(self, datas: list[dict[str:Any]], meta_data: list):
        ...
        self._index = VectorStoreIndex([], storage_context=storage_context, embed_model=self.config.embed_model)
    ...
    async def _load_index(self) -> bool:
        try:
            ...
            storage_context = StorageContext.from_defaults(
                vector_store=vector_store, persist_dir=self.config.persist_path
            )
