# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\search.py\elasticsearch.py\converter.py\episodic_memory_converter_c38f662b8377.py
    ...
    def from_mongo(cls, source_doc: MongoEpisodicMemory) -> EpisodicMemoryDoc:
        ...
        try:
            ...
            es_doc = EpisodicMemoryDoc(
                ...
                event_id=(str(source_doc.id) if hasattr(source_doc, "id") and source_doc.id else ""),
                user_id=source_doc.user_id,
                user_name=getattr(source_doc, "user_name", None),
                # Timestamp fields
                timestamp=source_doc.timestamp,
                # Core content fields
                title=getattr(source_doc, "subject", None),  # Map MongoDB's subject to ES's title
