# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\cache.py\redis_cache_queue.py\redis_data_processor_c2778350eb81.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\cache\redis_cache_queue\redis_data_processor.py
...
class RedisDataProcessor:
    ...
    def serialize_data(data: Union[str, Dict, List, Any]) -> Union[str, bytes]:
        ...
        try:
            ...
            logger.debug("JSON serialization failed, trying Pickle: %s", str(json_error))
            ...
            try:
                ...
                logger.debug("Pickle serialization succeeded, data length: %d", len(binary_data))
    ...
    def process_data_from_storage(member: Union[str, bytes]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                ...
                (str(member)[:100] if isinstance(member, str) else f"bytes({len(member)})"),
                str(e),
            )
