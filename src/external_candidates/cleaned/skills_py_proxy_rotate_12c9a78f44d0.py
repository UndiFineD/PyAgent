# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\mayuqi_crypto.py\stealth_browser.py\scripts.py\proxy_rotate_12c9a78f44d0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mayuqi-crypto\stealth-browser\scripts\proxy_rotate.py
    ...
    def get_proxy(
        ...
        if exclude_failed:
            ...
            candidates = [
                p
                for p in candidates
                if p.success_count + p.fail_count < 3 or p.fail_count / (p.success_count + p.fail_count) < 0.5
            ]
...
def test_proxy(proxy_url: str, test_url: str = "https://httpbin.org/ip", timeout: int = 10) -> Dict:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {"success": True, "ip": data.get("origin"), "latency_ms": latency}
...
if __name__ == "__main__":
    ...
    get_parser.add_argument(
        "--type",
        "-t",
        choices=["residential", "datacenter", "rotating"],
        help="Proxy type",
    )
