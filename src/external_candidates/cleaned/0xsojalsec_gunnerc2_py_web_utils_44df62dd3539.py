# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\payload_generator.py\common.py\web_utils_44df62dd3539.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\common\web_utils.py
...
def build_ps_http_context(
    ...
    *,
    ...
    if profile:
        ...
        headers_block = payutils.build_powershell_headers(cfg.client_headers, nostart=True, first=True)
        ...
        headers_block = payutils.build_powershell_headers(headers or {}, nostart=True, first=True)

        accept_hdr = f"$req.Accept = '{accept}';" if accept else ""

        host_hdr = f"$req.Host   = '{headers.get('Host')}';" if headers and "Host" in headers else ""
