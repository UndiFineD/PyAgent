...
class GUI(tk.Tk):
    ...
    def create_gui(self):
        ...
        self.merged_faces_canvas.bind(
            "<MouseWheel>",
            lambda event: self.merged_faces_canvas.xview_scroll(-int(event.delta / 120.0), "units"),
        )
                    ...
                    if img is not None:
                        ...
                        try:
                            ...
                            self.source_faces[-1]["TKButton"].bind(
                                "<ButtonRelease-1>",
                                lambda event, arg=len(self.source_faces) - 1: self.select_input_faces(event, arg),
                            )
        ...
        if self.widget["PreviewModeTextSel"].get() == "Image":  # images
            for i in range(len(images)):
                ...
                self.target_media_buttons[i].config(
                    ...
                    command=lambda i=i: self.load_target(i, images[i][1], self.widget["PreviewModeTextSel"].get()),
                )
            ...
            for i in range(len(videos)):
                ...
                self.target_media_buttons[i].config(
                    ...
                    compound="top",
                    anchor="n",
                    command=lambda i=i: self.load_target(i, videos[i][1], self.widget["PreviewModeTextSel"].get()),
                )
                self.target_media_canvas.create_window(
                    (i % 2) * delx,
                    (i // 2) * dely,
