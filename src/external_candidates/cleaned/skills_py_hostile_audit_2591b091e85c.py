...
def audit_repo(root: Path, level: str) -> Dict[str, Any]:
    ...
    tree_hash = hashlib.sha256("\n".join(f"{rel} {h}" for rel, h in fingerprint_items).encode("utf-8")).hexdigest()
    ...
    return {
        ...
        "level": level,
        "manifest": (
            {"present": bool(manifest), "path": manifest_path, "data": manifest} if manifest else {"present": False}
        ),
