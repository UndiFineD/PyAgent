# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2022.py\galois.py\art_supplements_4d0efefde7d1.py
...
class NightSkyOutpaintingTransition(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(MoveToTarget(image), run_time=12, rate_func=bezier([0, 0, 0, 1, 1, 1]))
...
class CauchyFourierPoisson(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(Write(name, rate_func=squish_rate_func(smooth, 0, 0.85)) for name in names),
                **kw,
            ),
        )
