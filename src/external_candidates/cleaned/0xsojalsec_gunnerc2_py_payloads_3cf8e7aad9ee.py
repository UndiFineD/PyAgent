# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\backend.py\payloads_3cf8e7aad9ee.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\payloads.py
...
from core.payload_generator.windows.http.ps1 import (
    powershell_reverse_http as win_http_ps1,
)

from core.payload_generator.windows.https.exe import exe_reverse_https as win_https_exe

from core.payload_generator.windows.https.gunnerplant import (
    gunnerplant_reverse_https as win_https_gp,
)

from core.payload_generator.windows.https.ps1 import (
    powershell_reverse_https as win_https_ps1,
)
...
def build_windows(cfg: WindowsPayload) -> str:
    ...
    if t == "http":
        if f == "ps1":
            return _as_text(
                win_http_ps1.generate_windows_powershell_http(
                    ip,
                    port,
                    cfg.obs,
                    cfg.beacon,
        ...
        if f == "exe":
            return _as_text(
                win_http_exe.generate_exe_reverse_http(
                    ip,
                    port,
                    cfg.obs,
                    cfg.beacon,
    ...
    if t == "https":
        if f == "ps1":
            return _as_text(
                win_https_ps1.generate_windows_powershell_https(
                    ip,
                    port,
                    cfg.obs,
                    cfg.beacon,
        ...
        if f == "exe":
            return _as_text(
                win_https_exe.generate_exe_reverse_https(
                    ip,
                    port,
                    cfg.obs,
                    cfg.beacon,
        ...
        if f == "gunnerplant":
            return _as_text(
                win_https_gp.generate_gunnerplant_reverse_https(
                    ip,
                    port,
                    cfg.obs,
                    cfg.beacon,
