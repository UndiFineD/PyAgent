# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\api.py\pty_manager_804be71e8052.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\api\pty_manager.py
...
class PTYManager:
    ...
    async def create_session(
        ...
        async with self._lock:
            ...
            if IS_WINDOWS:
                session = await self._create_windows_session(session_id, cols, rows, on_output, shell)

            else:
                session = await self._create_unix_session(session_id, cols, rows, on_output, shell)
    ...
    async def _create_windows_session(
        ...
        if not HAS_WINPTY:
            raise RuntimeError(
                "pywinpty is required for Windows terminal support. Install it with: pip install pywinpty"
            )
    ...
    async def _unix_reader_loop(self, session: PTYSession) -> None:
        ...
        try:
            while session._running and session.master_fd is not None:
                try:
                    data = await loop.run_in_executor(None, self._read_unix_pty, session.master_fd)
    ...
    async def _windows_reader_loop(self, session: PTYSession) -> None:
        ...
        try:
            while session._running and session.winpty_process is not None and session.winpty_process.isalive():
                try:
                    data = await loop.run_in_executor(None, session.winpty_process.read, 4096)
        ...
        try:
            if IS_WINDOWS:
                if session.winpty_process:
                    session.winpty_process.write(data.decode() if isinstance(data, bytes) else data)
