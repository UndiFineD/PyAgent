# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\api_specs.py\dtos.py\memory_command_cf5913030b31.py
...
class RawData:  # Memcell this is actually more oriented towards input, this is at a higher level of input; the one in the memcell table is the storage structure, which is more low-level
    ...
    def from_json_str(cls, json_str: str) -> "RawData":
        ...
        instance.metadata = (
            instance._deserialize_value(data.get("metadata"), "metadata") if data.get("metadata") else None
        )
