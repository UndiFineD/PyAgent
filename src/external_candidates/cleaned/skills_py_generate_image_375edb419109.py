# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\cgnl.py\nano_banana_antigravity.py\scripts.py\generate_image_375edb419109.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cgnl\nano-banana-antigravity\scripts\generate_image.py
    ...
    for path in auth_profiles_paths:
        if path.exists():
            try:
                ...
                if all_creds:
                    print(
                        f"Found {len(all_creds)} Antigravity accounts for rotation",
                        file=sys.stderr,
                    )
                ...
                if "accounts" in data:
                    for account in data["accounts"]:
                        if account.get("refreshToken"):
                            print(
                                f"Found credentials for: {account.get('email', 'unknown')}",
                                file=sys.stderr,
                            )
...
def main():
    parser = argparse.ArgumentParser(description="Generate images using Nano Banana Pro via Antigravity OAuth")
    ...
    for i, creds in enumerate(all_creds):
        try:
            print(
                f"Trying account {i + 1}/{len(all_creds)}: {creds['email']}",
                file=sys.stderr,
            )
            ...
            if not project_id:
                print(
                    f"  Could not determine project ID, trying next account...",
                    file=sys.stderr,
                )
    ...
    print(
        f"Error: All {len(all_creds)} accounts failed. Last error: {last_error}",
        file=sys.stderr,
    )
