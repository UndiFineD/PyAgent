# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphSearch\deepsearch\components.py
import asyncio
import json
...
async def query_completer(sub_query, context_data):
    try:
        completer_prompt = PROMPTS["query_completer"].format(sub_query=sub_query, context_data=context_data)
...
async def kg_query_completer(sub_query, context_data):
    try:
        completer_prompt = PROMPTS["kg_query_completer"].format(sub_query=sub_query, context_data=context_data)
        completed_query = await openai_complete(prompt=completer_prompt)
        return completed_query.strip()
    except:
        return ""
...
async def text_summary(query, context_data):
    try:
        summary_prompt = PROMPTS["retrieval_text_summarization"].format(query=query, context_data=context_data)
...
async def kg_summary(query, context_data):
    try:
        kg_summary_prompt = PROMPTS["knowledge_graph_summarization"].format(query=query, context_data=context_data)
...
async def answer_generation(query, context_data):
    try:
        answer_prompt = PROMPTS["answer_generation"].format(query=query, context_data=context_data)
...
async def answer_generation_deep(query, context_data):
    try:
        answer_prompt = PROMPTS["answer_generation_deep"].format(query=query, context_data=context_data)
        final_answer = await openai_complete(prompt=answer_prompt)
        return final_answer.strip()
    except:
        return ""
