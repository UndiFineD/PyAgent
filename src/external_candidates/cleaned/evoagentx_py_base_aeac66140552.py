# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\storages.py\base_aeac66140552.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\storages\base.py
...
class StorageHandler(BaseModule):
    ...
    def init_module(self):
        ...
        if (
            (self.storageConfig.path is not None)
            or (self.storageConfig.path != ":memory:")
    ...
    def _init_vector_store(self):
        ...
        if vector_config is not None:
            ...
            self.vector_store = VectorStoreFactory().create(
                store_type=vector_config.vector_name, store_config=vector_config_dict
            )
    ...
    def save_workflow(
        ...
        **kwargs,
        ...
        if existing:
            self.storageDB.update(
                workflow_id,
                new_metadata=workflow_data,
                store_type="workflow",
                table=table,
            )
