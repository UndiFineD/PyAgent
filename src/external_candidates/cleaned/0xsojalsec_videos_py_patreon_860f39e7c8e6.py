# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\patreon_860f39e7c8e6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\patreon.py
...
class SideGigToFullTime(Scene):
    ...
    def side_project(self, morty):
        ...
        cross = VGroup(*[Line(vect, -vect, color=RED) for vect in (UP + RIGHT, UP + LEFT)])
    ...
    def income(self, morty):
        ...
        for x in range(5):
            ...
            self.play(FadeOut(last_sign), dollar_signs.shift, LEFT)

        random.shuffle(dollar_signs.submobjects)

        self.play(
            ApplyMethod(dollar_signs.shift, (FRAME_Y_RADIUS + 1) * DOWN, lag_ratio=0.5),
            morty.change_mode,
            "guilty",
            morty.look,
            DOWN + RIGHT,
    ...
    def full_time(self, morty):
        ...
        self.play(Transform(self.cross, q_mark), morty.change_mode, "confused")

        self.play(Blink(morty))

        self.wait()

        self.play(morty.change_mode, "happy", morty.look, UP + RIGHT)
...
class GrowingToDoList(Scene):
    def construct(self):
        ...
        lines.next_to(title, DOWN, buff=LARGE_BUFF / 2.0)

        lines.to_edge(RIGHT)

        self.play(Write(title), morty.look_at, title)

        self.play(Write(lines[0]), morty.change_mode, "erm", run_time=1)

        for line in lines[1:3]:
            self.play(Write(line), morty.look_at, line, run_time=1)
...
class TwoTypesOfVideos(Scene):
    def construct(self):
        ...
        for words, vect in (fridays, LEFT), (when_done, RIGHT):
            ...
            words.next_to(morty, vect, buff=MED_SMALL_BUFF, aligned_edge=UP)
        ...
        self.play(morty.change_mode, "raise_right_hand", morty.look_at, box, ShowCreation(box))
        ...
        self.play(morty.change_mode, "raise_left_hand", morty.look_at, series)
        ...
        for words, mode in pairs:
            self.play(Write(words), morty.change_mode, mode, morty.look_at, words)
...
class RandolphWatching(Scene):
    def construct(self):
        ...
        self.play(randy.change_mode, "pondering", randy.look, RIGHT)
...
class PythagoreanTransformation(Scene):
    def construct(self):
        ...
        tri2, tri3, tri4 = copies = [tri1.copy().rotate(-i * np.pi / 2) for i in range(1, 4)]
        ...
        b_square.move_to(a_square.get_corner(DOWN + RIGHT), aligned_edge=UP + LEFT)
        ...
        self.play(tri3.shift, tri1.get_corner(UP + LEFT) - tri3.get_corner(UP + LEFT))
...
class KindWordsOnEoLA(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            Write(title),
            ShowCreation(rect),
            *[ApplyMethod(pi.look_at, rect) for pi in self.get_pi_creatures()],
            run_time=2,
        )
...
class MakeALotOfPiCreaturesHappy(Scene):
    def construct(self):
        ...
        pis = VGroup(
            *[VGroup(*[Randolph() for x in range(7)]).arrange(RIGHT, buff=MED_LARGE_BUFF) for x in range(4)]
        ).arrange(DOWN, buff=MED_LARGE_BUFF)

        pi_list = list(it.chain(*[layer.submobjects for layer in pis.submobjects]))
...
class IntegrationByParts(Scene):
    def construct(self):
        ...
        equation = OldTex("\\int_0^1 g\\,df", "+\\int_0^1 f\\,dg", "= \\big(fg \\big)_0^1")

        equation.to_edge(UP)

        equation.set_color_by_tex("\\int_0^1 g\\,df", upper_right.get_color())

        equation.set_color_by_tex("+\\int_0^1 f\\,dg", lower_left.get_color())
        ...
        self.play(ShowCreation(v_lines, run_time=2), Animation(curve), Animation(rect))
        ...
        self.play(ShowCreation(h_lines, run_time=2), Animation(curve), Animation(rect))
    ...
    def draw_curve(self, curve):
        ...
        self.play(
            ShowCreation(curve),
            UpdateFromFunc(dot, lambda d: d.move_to(curve.get_points()[-1])),
