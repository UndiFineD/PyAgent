...
class SupportResistanceAnalyzer:
    ...
    def __init__(
        ...
        if not isinstance(clustering_threshold, (int, float)) or not (0 < clustering_threshold < 1):
            raise ValueError(f"clustering_threshold must be between 0 and 1, got {clustering_threshold}")
    ...
    def detect_levels(self, df: pd.DataFrame, num_levels: int = None) -> Dict[str, List[float]]:
        """
        ...
        try:
            ...
            logger.info(f"Detected {len(result['support'])} support and {len(result['resistance'])} resistance levels")
    ...
    def calculate_level_strength(self, df: pd.DataFrame, level: float, level_type: str = "resistance") -> Dict:
        """
        ...
        try:
            ...
            strength = min(
                100,
                (num_tests * 10) + (20 if is_recent else 0) + (10 if avg_volume > overall_avg_volume else 0),
            )
            ...
            logger.debug(
                f"{level_type.capitalize()} at {level:.2f}: strength={strength}, tests={num_tests}, recent={is_recent}"
            )
    ...
    def find_nearest_levels(self, df: pd.DataFrame, current_price: float, num_levels: int = None) -> Dict:
        """
        ...
        try:
            ...
            result = {
                ...
                "resistance_distance_pct": (
                    round((nearest_resistance - current_price) / current_price * 100, 2) if nearest_resistance else None
                ),
                "support_distance_pct": (
                    round((current_price - nearest_support) / current_price * 100, 2) if nearest_support else None
                ),
            }
