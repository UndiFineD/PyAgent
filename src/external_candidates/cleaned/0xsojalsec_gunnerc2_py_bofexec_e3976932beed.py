# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\gunnershell.py\commands.py\windows.py\specialcommands.py\bofexec_e3976932beed.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\windows\specialcommands\bofexec.py
...
from core.gunnershell.bofs.base import BOFS, _resolve_bof_bytes

# Gunnershell Special Imports

from core.gunnershell.commands.base import Command, QuietParser, register

from core.session_handlers import session_manager


class ArgparseCrash(Exception):
    pass
...
class BofExecCommand(Command):
    ...
    @property
    def help(self):
        return "bofexec <bof_name_or_path> [-z STR ...] [-Z WSTR ...]   resolve a BOF and execute it on agent."
    ...
    def _parse_args(self, args: List[str]):
        ...
        p.add_argument(
            ...
            action="append",
            default=[],
            help="ASCII string argument (repeatable)",
        )
        ...
        p.add_argument(
            ...
            action="append",
            default=[],
            help="WIDE string argument (repeatable)",
        )
        ...
        p.add_argument(
            ...
            action="append",
            default=[],
            help="16 bit Intger argument (repeatable)",
        )
        ...
        p.add_argument(
            ...
            action="append",
            default=[],
            help="32 bit Intger argument (repeatable)",
        )
        ...
        p.add_argument(
            ...
            action="store_true",
            default=False,
            help="Display help menu for BOF",
        )
    ...
    def logic(
        ...
        if not res:
            ...
            if not self.gs.bofs_enabled:
                hint = (
                    f"\n{brightyellow}Note:{brightgreen} BOF registry not loaded (gunnerplant={self.gs.gunnerplant})."
                )
        ...
        return brightgreen + f"[+] BOF Output:\n" + f"{out}"
