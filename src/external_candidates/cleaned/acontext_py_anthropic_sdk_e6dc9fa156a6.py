# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\acontext.py\src.py\server.py\core.py\acontext_core.py\llm.py\complete.py\anthropic_sdk_e6dc9fa156a6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\llm\complete\anthropic_sdk.py
...
def process_messages(messages: list[dict]) -> list[dict]:
    ...
    for m in messages:
        if isinstance(m, dict) and m["role"] == "tool" and "tool_call_id" in m:
            if isinstance(new_messages[-1], dict) and new_messages[-1]["role"] == "user":
...
async def anthropic_complete(
    ...
    **kwargs,
    ...
    try:
        ...
        response: Message = await anthropic_async_client.messages.create(**request_params)
