# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\profile_memory.py\profile_helpers_5d638d754417.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\profile_memory\profile_helpers.py
...
def remove_evidences_from_profile(profile_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def strip_content(content: Any) -> Any:
        if isinstance(content, dict):
            return {key: strip_content(value) for key, value in content.items() if key != "evidences"}
...
def accumulate_old_memory_entry(memory: BaseMemory, participants_profile_list: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        if projects:
            project_payload = [project_to_dict(project) for project in projects if project is not None]
...
def profile_payload_to_memory(
    ...
    *,
    ...
    if not extracted_user_id:
        logger.debug("LLM generated user %s has no user_id, skipping", extracted_user_name)
...
def merge_single_profile(existing: ProfileMemory, new: ProfileMemory, *, group_id: str) -> ProfileMemory:
    ...
    *,
    ...
    for new_profile in profile_memories:
        ...
        if user_id in merged_dict:
            ...
            merged_dict[user_id] = merge_single_profile(existing_profile, new_profile, group_id=group_id)
...
def _merge_value_fields(
    ...
    for field in field_names:
        merged[field] = merge_value_with_evidences_lists(getattr(existing, field, None), getattr(new, field, None))
