# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GPT_Vuln-analyzer\components\packet_analysis.py
import json
import re
...
from scapy.all import ARP, DNSQR, EAPOL, IP, TCP, UDP, Raw, rdpcap
...
class PacketAnalysis:
    ...
    def detect_dns_requests(self):
        dns_requests = [
            pkt[DNSQR].qname.decode() for pkt in self.packets if DNSQR in pkt and UDP in pkt and pkt[UDP].dport == 53
        ]
    ...
    def detect_credentials(self):
        ...
        credential_packets = [
            ...
            if TCP in pkt
            and Raw in pkt
            and any(re.search(pattern, str(pkt[Raw].load), re.IGNORECASE) for pattern in credential_patterns)
        ]
