# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GPT_Vuln-analyzer\components\passbeaker.py
import hashlib
import itertools
...
class PasswordCracker:
    ...
    def print_statistics(self):
        msg = (
            f"Password Cracked! Password: {self.matched_password}"
            if self.matched_password
            else "Password Cracking Failed"
        )
        message_panel = Panel(
            Align.center(
                Group("\n", Align.center(msg)),
