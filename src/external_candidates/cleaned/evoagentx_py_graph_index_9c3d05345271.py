# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\rag.py\indexings.py\graph_index_9c3d05345271.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\indexings\graph_index.py
...
class GraphIndexing(BaseIndexWrapper):
    ...
    def __init__(
        ...
        try:
            ...
            vector_store = (
                self.storage_handler.vector_store.get_vector_store()
            ...
            self.index = PropertyGraphIndex(
                ...
                kg_extractors=[
                    kg_extractor,
                    ImplicitPathExtractor(),
                ],  # 'ImplicitPathExtractor' for node basic relation(tree structure and so on).
                embed_model=self._embed_model,
                vector_store=(
                    vector_store if (not self.storage_handler.graph_store.supports_vector_queries) else None
                ),  # if graph database support vector query, disable the vector database.,
    ...
    def _create_storage_context(self):
        ...
        assert self.storage_handler.graph_store is not None, (
            "GraphIndexing must init a graph backend in 'storageHandler'"
        )
        ...
        try:
            if node_ids:
                ...
                if nodes_to_delete:
                    ...
                    logger.info(
                        f"Deleted {len(nodes_to_delete)} nodes matching metadata filters from PropertyGraphIndex"
                    )
    ...
    async def aload(
        self,
        nodes: List[Union[Chunk, BaseNode, LabelledNode, Relation, EntityNode, ChunkNode]],
    ) -> Sequence[str]:
        """
