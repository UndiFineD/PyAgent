# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\cache.py\redis_cache_queue.py\redis_windows_cache_manager_906722317a95.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\cache\redis_cache_queue\redis_windows_cache_manager.py
...
DEFAULT_CLEANUP_MULTIPLIER = 2  # Cleanup threshold multiplier (clean data older than 2x expiration time)
...
class RedisWindowsCacheFactory:
    ...
    async def _ensure_timestamp_range_script_registered(self):
        ...
        if self._timestamp_range_script is None:
            ...
            binary_client = await self.redis_provider.get_named_client("binary_cache", decode_responses=False)

            self._timestamp_range_script = binary_client.register_script(FETCH_BY_DATE_TIMESTAMP_RANGE_LUA_SCRIPT)
...
class RedisWindowsCacheManager:
    ...
    async def _cleanup_expired_if_needed(self, key: str, client) -> int:
        ...
        if random.random() < self.cleanup_probability:
            ...
            cleaned_count = await client.zremrangebyscore(key, "-inf", cleanup_threshold)
    ...
    async def get_by_timestamp_range(
        ...
        try:
            ...
            messages = await self._fetch_by_timestamp_range_script(keys=[key], args=[min_score, max_score, limit])
            ...
            for i in range(0, len(messages), 2):
                try:
                    ...
                    try:
                        ...
                        timestamp = int(time.time() * 1000)  # Use current time as fallback

                    # Use data processor to parse data

                    processed_data = RedisDataProcessor.process_data_from_storage(message)
                    ...
                    result.append(
                        {
                            ...
                            "timestamp": timestamp,
                            "datetime": datetime.fromtimestamp(timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
                        }
                    )
    ...
    async def get_queue_stats(self, key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "oldest_datetime": (
                    datetime.fromtimestamp(oldest_timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S")
                    if oldest_timestamp
                    else None
                ),
                "newest_datetime": (
                    datetime.fromtimestamp(newest_timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S")
                    if newest_timestamp
                    else None
                ),
                "ttl_seconds": ttl,
            ...
            logger.error("Failed to get queue statistics: key=%s, error=%s", key, str(e))
...
class DefaultRedisWindowsCacheManager:
    ...
    async def get_by_timestamp_range(
        ...
        return await manager.get_by_timestamp_range(key, start_timestamp, end_timestamp, limit)
