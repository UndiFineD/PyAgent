# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\outside_videos.py\grover_bd3ac4c0b78d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\grover.py
...
class QBitDiagram(InteractiveScene):
    def construct(self):
        ...
        self.play(Rotate(vect, -1, about_point=axes.c2p(0, 0)), run_time=5)
        ...
        for line in lines:
            ...
            vect.target.flip(axis=line.get_vector(), about_point=axes.c2p(0, 0))

            angle = vect.target.get_angle() - vect.get_angle()

            self.play(MoveToTarget(vect, path_arc=angle))
...
class BlocksToQuantum(InteractiveScene):
    def construct(self):
        ...
        state_tex = VGroup(
            coefs[0],
            Tex(Rf"|\text{{{names[0]}}}\rangle"),
            Tex("+"),
            coefs[1],
            Tex(Rf"|\text{{{names[1]}}}\rangle"),
            Tex("+"),
            coefs[2],
            Tex(Rf"|\text{{{names[2]}}}\rangle"),
            Tex("+"),
            Tex(R"\cdots"),
            Tex("+"),
            coefs[-1],
