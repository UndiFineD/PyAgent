# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\chocomintx.py\xiaohongshutools.py\scripts.py\request.py\web.py\xhs_session_87e494d14233.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\chocomintx\xiaohongshutools\scripts\request\web\xhs_session.py
...
class XHS_Session:
    ...
    async def __init_cookies(self) -> None:
        ...
        self.__fp = self.__xhs_encrypt.get_fingerprint(cookies_dict, self._session.headers.get("User-Agent"))
    ...
    async def __set_websectiga_and_secPoisonId(self) -> None:
        ...
        res = await self.request(method="post", url=url, data=data, need_encrypt=False)
    ...
    async def __set_gid_and_acw_tc(self) -> None:
        ...
        res = await self.request(method="post", url=url, data=data)
    ...
    async def __set_web_session(self) -> None:
        ...
        await self.request(method="post", url=url, data=data)
    ...
    def __request_encrypt(self, url, params, data) -> None:
        ...
        xs = self.__xhs_encrypt.encrypt_headers_xs(a1, int(loadts), url_path, params, data)
