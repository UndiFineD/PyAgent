# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\araa47.py\ez_google.py\scripts.py\chat_53754096ab95.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\araa47\ez-google\scripts\chat.py
...
def messages(space_id: str, limit: int):
    ...
    try:
        results = service.spaces().messages().list(parent=space_id, pageSize=limit).execute()
...
def send(space_id: str, text: str):
    ...
    try:
        result = service.spaces().messages().create(parent=space_id, body={"text": text}).execute()
