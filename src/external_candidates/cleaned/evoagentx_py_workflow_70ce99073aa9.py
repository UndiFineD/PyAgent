# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\workflow.py\workflow_70ce99073aa9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\workflow\workflow.py
...
class WorkFlow(BaseModule):
    ...
    workflow_manager: WorkFlowManager = Field(
        default=None,
        description="Responsible for task and action scheduling for workflow execution",
    )
    ...
    max_execution_steps: int = Field(
        default=5,
        description="The maximum number of steps to complete a subtask (node) in the workflow",
    )
    ...
    async def _async_execute_task_by_action_graph(self, task: WorkFlowNode, next_action: NextAction):
        ...
        message = Message(
            content=action_graph_output,
            action=action_graph.name,
        ...
        if action_inputs_format:
            ...
            if not all(inp in action_input_data for inp in action_required_input_names):
                ...
                predecessors_messages = [
                    message
                    for message in predecessors_messages
    ...
    def _prepare_single_hitl_agent(self, agent: Agent, node: WorkFlowNode):
        ...
        for predecessor in predecessors:
            ...
            for param in predecessor_node.outputs:
                if param.required:
                    hitl_inputs_data_fields[param.name] = (
                        str,
        ...
        inputs_format = create_model(
            agent._get_unique_class_name(generate_dynamic_class_name(hitl_action.class_name + " action_input")),
        ...
        for successor in successors:
            ...
            for param in successor_node.inputs:
                if param.required:
                    hitl_outputs_data_fields[param.name] = (
                        str,
                        Field(description=param.description),
                    )
        ...
        outputs_format = create_model(
            agent._get_unique_class_name(generate_dynamic_class_name(hitl_action.class_name + " action_output")),
    ...
    def _prepare_hitl(self):
        ...
        if len(hitl_agents) != len(node_with_hitl_agents):
            raise ValueError(
                "Incorrect WorkFlowNode definition: A HITL Agent must be the only agent in its WorkFlowNode instance"
            )
