# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\adamkristopher.py\seo_dataforseo.py\scripts.py\main_0455db3ffc6b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\adamkristopher\seo-dataforseo\scripts\main.py
...
def keyword_research(
    ...
    results["overview"] = get_keyword_overview(keywords=[keyword], location_name=location_name)

    if include_suggestions:
        print("  → Getting keyword suggestions...")

        results["suggestions"] = get_keyword_suggestions(keyword=keyword, location_name=location_name, limit=50)

    if include_related:
        print("  → Getting related keywords...")

        results["related"] = get_related_keywords(keyword=keyword, location_name=location_name, depth=2, limit=50)

    if include_difficulty:
        print("  → Getting keyword difficulty...")

        results["difficulty"] = get_bulk_keyword_difficulty(keywords=[keyword], location_name=location_name)
...
def youtube_keyword_research(
    ...
    results["overview"] = get_keyword_overview(keywords=[keyword], location_name=location_name, include_serp_info=True)
    ...
    if include_serp:
        ...
        results["youtube_serp"] = get_youtube_serp(keyword=keyword, location_name=location_name, depth=20)

    if include_trends:
        print("  → Getting YouTube trends...")

        results["youtube_trends"] = get_youtube_trends(keywords=[keyword], location_name=location_name)
...
def landing_page_keyword_research(
    keywords: List[str], competitor_domain: str = None, location_name: str = None
) -> Dict[str, Any]:
    """
    ...
    results["overview"] = get_keyword_overview(keywords=keywords, location_name=location_name, include_serp_info=True)

    # Search intent

    print("  → Getting search intent...")

    results["search_intent"] = get_search_intent(keywords=keywords, location_name=location_name)

    # Difficulty scores

    print("  → Getting keyword difficulty...")

    results["difficulty"] = get_bulk_keyword_difficulty(keywords=keywords, location_name=location_name)

    # SERP analysis for primary keyword

    print("  → Getting SERP analysis...")

    results["serp"] = get_google_serp(keyword=keywords[0], location_name=location_name)
    ...
    if competitor_domain:
        ...
        results["competitor_keywords"] = get_keywords_for_site(
            target_domain=competitor_domain, location_name=location_name
        )
...
def full_keyword_analysis(
    ...
    results["keyword_ideas"] = get_keyword_ideas(keywords=keywords, location_name=location_name, limit=100)

    if include_historical:
        print("  → Getting historical search volume...")

        results["historical"] = get_historical_search_volume(keywords=keywords, location_name=location_name)

    if include_trends:
        print("  → Getting Google Trends data...")

        results["trends"] = get_trends_explore(keywords=keywords[:5], location_name=location_name)
...
def competitor_analysis(domain: str, keywords: List[str] = None, location_name: str = None) -> Dict[str, Any]:
    """
    ...
    results["domain_keywords"] = get_domain_keywords(target_domain=domain, location_name=location_name, limit=100)

    print("  → Getting keywords from Google Ads data...")

    results["ads_keywords"] = get_keywords_for_site(target_domain=domain, location_name=location_name)

    if keywords:
        print("  → Finding other competitors...")

        results["other_competitors"] = get_competitors(keywords=keywords, location_name=location_name)
...
def trending_topics(location_name: str = None) -> Dict[str, Any]:
    """
