# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\foresight_extractor_fe4f50121826.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\foresight_extractor.py
...
class ForesightExtractor(MemoryExtractor):
    ...
    async def generate_foresights_for_memcell(self, memcell: MemCell) -> List[Foresight]:
        """
        ...
        for retry in range(5):
            try:
                if retry == 0:
                    logger.info(f"🎯 Generating foresight associations for MemCell: {memcell.subject}")

                else:
                    logger.info(f"🎯 Generating foresight associations for MemCell: {memcell.subject}, retry {retry}/5")
                ...
                logger.debug(f"📝 Starting LLM call to generate foresight associations, prompt length: {len(prompt)}")

                response = await self.llm_provider.generate(prompt=prompt, temperature=0.3)

                logger.debug(f"✅ LLM call completed, response length: {len(response) if response else 0}")
                ...
                if len(foresights) > 10:
                    ...
                    logger.warning(f"Generated foresight associations less than 10, actual count: {len(foresights)}")

                logger.info(f"✅ Successfully generated {len(foresights)} foresight associations")
    ...
    async def generate_foresights_for_episode(self, episode: EpisodeMemory) -> List[Foresight]:
        """
    ...
    async def _parse_foresights_response(
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                for item in data:
                    ...
                    if item_start_time:
                        ...
                        if item_duration_days and not item_end_time:
                            item_end_time = self._calculate_end_time_from_duration(item_start_time, item_duration_days)

                        # If LLM provides end_time but no duration_days, calculate duration_days

                        elif item_end_time and not item_duration_days:
                            item_duration_days = self._calculate_duration_days(item_start_time, item_end_time)
                ...
                vectors_batch = await vs.get_embeddings(contents)  # Use get_embeddings (List[str])
    ...
    def _calculate_end_time_from_duration(self, start_time: str, duration_days: int) -> Optional[str]:
        """
