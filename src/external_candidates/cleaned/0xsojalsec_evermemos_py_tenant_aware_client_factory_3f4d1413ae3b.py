# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\mongo.py\tenant_aware_client_factory_3f4d1413ae3b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\mongo\tenant_aware_client_factory.py
...
class TenantAwareMongoDBClientFactory(MongoDBClientFactory):
    ...
    async def get_client(self, config: Optional[MongoDBConfig] = None, **connection_kwargs) -> MongoDBClientWrapper:
        """
        ...
        if self._client_wrapper is None:
            async with self._lock:
                ...
                if self._client_wrapper is None:
                    ...
                    self._client_wrapper = TenantAwareClientWrapper(tenant_aware_client, dummy_config)
...
class TenantAwareClientWrapper(MongoDBClientWrapper):
    ...
    def __init__(self, tenant_aware_client: TenantAwareMongoClient, config: MongoDBConfig):
        """
