# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\core_memory_raw_repository_3829099d667a.py
...
class CoreMemoryRawRepository(BaseRepository[CoreMemory]):
    ...
    async def ensure_latest(self, user_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        """
        ...
        try:
            ...
            latest_version = await self.model.find_one({"user_id": user_id}, sort=[("version", -1)], session=session)
        ...
        try:
            ...
            if version_range is None:
                result = await self.model.find_one(
                    query_filter,
                    sort=[("version", -1)],  # Sort by version descending to get the latest
                    session=session,
                )
    ...
    async def update_by_user_id(
        ...
        try:
            ...
            if version is not None:
                ...
                existing_doc = await self.model.find_one({"user_id": user_id, "version": version}, session=session)

            else:
                # Update latest version

                existing_doc = await self.model.find_one({"user_id": user_id}, sort=[("version", -1)], session=session)
        ...
        try:
            ...
            if version is not None:
                ...
                if success:
                    ...
                    logger.warning("⚠️  Core memory not found for deletion: user_id=%s", user_id)
        ...
        try:
            ...
            if existing_doc:
                ...
                if version is None:
                    ...
                    raise ValueError(f"Version field must be provided when creating new core memory: user_id={user_id}")
