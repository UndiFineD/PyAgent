# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\beam.py\beam_search_milestones_e50eb9d75142.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\beam\beam_search_milestones.py
...
class MilestonesBeamSearchExecutor(SupervisedTaskExecutorABC):
    ...
    async def generate_plans(self, task: ThroughputTask, nr_of_beams: int, instances) -> List[InitialPlanOutput]:
        plan_outputs = {}
    ...
    async def generate_next_step(self, group, task) -> List[PlanOutput]:
        ...
        generation_params = GenerationParameters(model=self.model_to_evaluate, max_tokens=4096, temperature=0.5)
        ...
        for instance_id, plan_output in plan_outputs.items():
            ...
            for step in steps:
                assistant_str = (
                    step.program.meta["text_response"] if step.program.meta.get("text_response", None) else ""
                )
        ...
        try:
            ...
            for program in step.sampled_programs:
                ...
                (
                    ...
                    achievements,
                    ticks,
                ) = await group.evaluator._evaluate_single(instance_id, program, instance)
    ...
    async def _process_last_step(
        ...
        try:
            ...
            step_to_process, entity_list = await self._evaluate_step(step_to_process, group, instance_id, parent_id)
            ...
            throughput_str = (
                f"Here is the current througphut of your factory: {achievements['dynamic']} created per 60 seconds"
            )
    ...
    def evaluate_task(self, plan: PlanOutput, group: PlanningGroupV2, task: ThroughputTask) -> bool:
