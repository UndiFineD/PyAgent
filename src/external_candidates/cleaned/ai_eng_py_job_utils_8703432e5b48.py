# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\ai_eng.py\feathr_project.py\feathr.py\utils.py\job_utils_8703432e5b48.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\utils\job_utils.py
...
def get_result_df(
    ...
    if data_format is None:
        ...
        if client.get_job_tags() and client.get_job_tags().get(OUTPUT_FORMAT):
            ...
            raise ValueError(
                ...
                "Cannot determine the data format. Please provide the data_format argument."
    ...
    try:
        ...
        logger.error(
            ...
            f"Failed to load result files from {local_cache_path} with format {data_format}."
...
def copy_cloud_dir(client: FeathrClient, source_url: str, target_url: str = None):
    ...
    source_url: str = source_url or client.get_job_result_uri(

        block=True, timeout_sec=1200
    ...
    if target_url is None:
        ...
        raise RuntimeError(
            ...
            "target_url None. Please make sure you provide a target_url."
        ...
        if Path(dir_path).is_file():
            ...
            try:
                ...
                return pd.concat(
                    ...
                    [pdx.read_avro(f) for f in Path(dir_path).glob("*.avro")]
        ...
        if Path(dir_path).is_file():
            ...
            try:
                ...
                return pd.concat(
                    ...
                    [pd.read_csv(f) for f in Path(dir_path).glob("*.csv")]
...
def get_cloud_file_column_names(

    client: FeathrClient, path: str, format: str = "csv", is_file_path=True
    ...
    try:
        ...
        df = get_result_df(
            ...
            client=client, data_format=format, res_url=path, is_file_path=is_file_path
