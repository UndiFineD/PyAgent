# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\gui.py\login_dialog_39892c65ef28.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\login_dialog.py
...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title = QLabel("GUNNERC2")

        subtitle = QLabel("Sign in to your operator console")
        ...
        self._eye = QToolButton(self.pass_edit)
        ...
        self._eye.setStyleSheet("QToolButton { border: 0; padding: 0 6px; }")

        self._eye.setFixedSize(18, 18)

        QTimer.singleShot(0, self._position_eye_button)
    ...
    def resizeEvent(self, e):
        ...
        self._position_eye_button()

    def _position_eye_button(self):
        m = 6

        r = self.pass_edit.rect()

        x = r.right() - self._eye.width() - m

        y = r.center().y() - self._eye.height() // 2

        self._eye.move(x, y)

        self.pass_edit.setStyleSheet("QLineEdit { padding-right: 28px; }")
