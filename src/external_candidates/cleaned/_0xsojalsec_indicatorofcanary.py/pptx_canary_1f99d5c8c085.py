# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IndicatorOfCanary\pptx_canary.py
#!/usr/bin/python3
import argparse
import hashlib
import json
...
def extract_pptx_meta(pptx_path):
    ...
    meta_data = {
        prop: value.isoformat() if isinstance(value, datetime) else value for prop, value in meta_data.items() if value
    }
    return meta_data
