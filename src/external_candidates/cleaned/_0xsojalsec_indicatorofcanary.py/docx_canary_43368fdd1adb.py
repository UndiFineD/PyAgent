...
def extract_docx_meta(docx_path):
    ...
    meta_data = {
        prop: value.isoformat() if isinstance(value, datetime) else value for prop, value in properties if value
    }
    return meta_data
...
def main():
    ...
    with zipfile.ZipFile(args.input) as doc:
        urls = [
            extract_urls_from_file(doc.read(file.filename), file.filename) for file in doc.filelist if not file.is_dir()
        ]
        urls = sum(urls, [])
