# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_manager_16b59022445a.py
...
class MemoryManager:
    ...
    async def extract_memcell(
        ...
        logger.debug(f"[MemoryManager] Starting boundary detection and creating MemCell")
        ...
        if not memcell:
            logger.debug(f"[MemoryManager] Boundary detection: no boundary reached, waiting for more messages")
    ...
    async def extract_memory(
        ...
        user_id: Optional[str] = None,  # None means group memory, with value means personal memory
        group_id: Optional[str] = None,
        group_name: Optional[str] = None,
        old_memory_list: Optional[List[BaseMemory]] = None,
        user_organization: Optional[List] = None,
        episode_memory: Optional[EpisodeMemory] = None,  # Used for Foresight and EventLog extraction
    ):
        """
        ...
        match memory_type:
            ...
            case MemoryType.PROFILE:
                return await self._extract_profile(memcell, user_id, group_id, old_memory_list)
    ...
    async def _extract_foresight(self, episode_memory: Optional[EpisodeMemory]) -> List[Foresight]:
        """Extract Foresight"""

        if not episode_memory:
            logger.warning("[MemoryManager] Missing episode_memory, cannot extract Foresight")

            return []

        logger.debug(f"[MemoryManager] Extracting Foresight for Episode: user_id={episode_memory.user_id}")
    ...
    async def _extract_event_log(self, episode_memory: Optional[EpisodeMemory]):
        ...
        if not episode_memory:
            logger.warning("[MemoryManager] Missing episode_memory, cannot extract EventLog")

            return None

        logger.debug(f"[MemoryManager] Extracting EventLog for Episode: user_id={episode_memory.user_id}")
