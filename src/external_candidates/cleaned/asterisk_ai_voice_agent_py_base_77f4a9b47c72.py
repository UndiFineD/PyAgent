# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\pipelines.py\base_77f4a9b47c72.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\pipelines\base.py
...
class Component(ABC):
    ...
    async def _test_websocket_connection(
        self, url: str, api_key: Optional[str] = None, timeout: float = 5.0
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "healthy": True,
                "error": None,
                "details": {"endpoint": url, "protocol": "websocket"},
            }
            ...
            return {
                "healthy": False,
                "error": detail,
                "details": {"endpoint": url, "exception": error_str},
            }
    ...
    async def _test_http_connection(
        ...
        try:
                ...
                async with session.get(
                    ...
                    if resp.status == 401:
                        return {
                            ...
                            "details": {"status": 401, "endpoint": test_endpoint},
                        }
                        ...
                        return {
                            ...
                            "details": {"status": 403, "endpoint": test_endpoint},
                        }
                        ...
                        return {
                            ...
                            "details": {
                                ...
                                "note": "Endpoint exists (requires POST)",
                            },
                        }
                        ...
                        return {
                            ...
                            "details": {
                                "status": 429,
                                "endpoint": test_endpoint,
                                "note": "Rate limited but API key valid",
                            },
                        }
            ...
            return {
                "healthy": False,
                "error": f"Connection timeout after {timeout}s",
                "details": {"endpoint": test_endpoint},
            }
            ...
            return {
                ...
                "details": {"endpoint": test_endpoint, "exception": str(exc)},
            }
    ...
    async def validate_connectivity(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not base_url:
            return {
                ...
                "details": {"checked_keys": ["base_url", "ws_url", "url", "endpoint"]},
            }
        ...
        if not component_key.startswith("local_") and not api_key:
            ...
            return {
                "healthy": False,
                "error": f"No API credentials found (checked {provider_prefix}_API_KEY env var)",
                "details": {"component": component_key},
            }
        ...
        if not is_primary:
            ...
            if base_url.startswith(("http://", "https://", "ws://", "wss://")):
                ...
                return {
                    "healthy": False,
                    "error": f"Invalid URL format: {base_url}",
        ...
        if base_url.startswith(("ws://", "wss://")):
            ...
            return {
                "healthy": False,
                "error": f"Unknown protocol in URL: {base_url}",
                "details": {"url": base_url},
            }
