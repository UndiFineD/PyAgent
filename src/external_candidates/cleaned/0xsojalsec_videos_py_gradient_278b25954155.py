# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2017.py\gradient_278b25954155.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\gradient.py
...
class ShowSimpleMultivariableFunction(Scene):
    def construct(self):
        ...
        many_inputs = OldTex(*["x_{%d}, " % d for d in range(1, 25)])
        ...
        decimal_change = ContinualChangingDecimal(decimal, lambda a: 1 + np.sin(value_tracker.get_value()))
        ...
        self.play(
            ReplacementTransform(func_tex[0], alt_func_tex[0]),
...
class ShowGradient(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(FadeIn, background_rects, rate_func=squish_rate_func(smooth, 0.5, 1)),
        )
...
class TakePartialDerivatives(Scene):
    def construct(self):
        ...
        for terms in xs, ys:
            terms.rects = VGroup(*[SurroundingRectangle(term, buff=0.5 * SMALL_BUFF) for term in terms])

            terms.arrows = VGroup(*[Vector(0.5 * DOWN).next_to(rect, UP, SMALL_BUFF) for rect in terms.rects])
        ...
        self.play(ReplacementTransform(partial_y[-5].copy(), partial_y[-2], path_arc=-PI))
...
class ShowDerivativeAtExamplePoint(Scene):
    def construct(self):
        ...
        gradient = VGroup(gradient_tex, partial_vect, OldTex("="), result_vector)
