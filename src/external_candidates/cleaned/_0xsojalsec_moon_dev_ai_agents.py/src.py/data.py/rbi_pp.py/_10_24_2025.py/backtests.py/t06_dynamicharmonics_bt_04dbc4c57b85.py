# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_24_2025\backtests\T06_DynamicHarmonics_BT.py
import numpy as np
import pandas as pd
import talib
...
class DynamicHarmonics(Strategy):
    ...
    def next(self):
        ...
        if (
            recent_swing_high_idx is not None
            and recent_swing_low_idx is not None
            and recent_swing_high_idx > recent_swing_low_idx
        ):
        ...
        if (
            recent_swing_low_idx is not None
            and recent_swing_high_idx is not None
            and recent_swing_low_idx > recent_swing_high_idx
        ):
        ...
        if all(long_condition):
            ...
            sl = (
                min(
                    self.bb_lower[-1],
                    (self.data.Low[recent_swing_low_idx] if recent_swing_low_idx else self.bb_lower[-1]),
                )
                - self.atr[-1]
            )
            ...
            self.log(
                f"🌙 Moon Dev: 🚀 Long Entry at {entry_price:.2f}! Size: {position_size}, SL: {sl:.2f}, TP: {self.tp:.2f} ✨"
            )
        ...
        if all(short_condition):
            ...
            sl = (
                max(
                    self.bb_upper[-1],
                    (self.data.High[recent_swing_high_idx] if recent_swing_high_idx else self.bb_upper[-1]),
                )
                + self.atr[-1]
            )
            ...
            self.log(
                f"🌙 Moon Dev: 🚀 Short Entry at {entry_price:.2f}! Size: {position_size}, SL: {sl:.2f}, TP: {self.tp:.2f} ✨"
            )
        ...
        # Debug prints
        self.log(
            f"🌙 Moon Dev Debug: ADX={self.adx[-1]:.2f} (Rising: {adx_rising}), Squeeze: {is_squeeze}, PDI>{self.pdi[-1]:.2f}>{self.mdi[-1]:.2f}, Close: {close:.2f} vs BB Lower: {self.bb_lower[-1]:.2f} Upper: {self.bb_upper[-1]:.2f} 🚀"
        )
