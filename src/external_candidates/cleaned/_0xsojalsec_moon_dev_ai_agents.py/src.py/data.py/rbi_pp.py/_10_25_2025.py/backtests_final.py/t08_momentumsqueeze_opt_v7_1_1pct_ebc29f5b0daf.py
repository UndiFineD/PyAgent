# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_final\T08_MomentumSqueeze_OPT_v7_1.1pct.py
import numpy as np
import pandas as pd
import talib
...
class MomentumSqueeze(Strategy):
    ...
    def next(self):
        ...
        # 🌙 Moon Dev: Tightened volume filter to 1.5x for stronger confirmation and higher quality entries (Entry Optimization)
        ...
        # 🌙 Moon Dev: Tightened RSI filters to >55/<45 for stronger momentum confirmation (Indicator Optimization)
        ...
        if (
            ...
            risk = entry - sl
            if risk > 0:
                # 🌙 Moon Dev: Increased risk to 3% for higher returns while maintaining controlled exposure (Risk Management)
                risk_amount = self.equity * 0.03
                pos_size = risk_amount / risk
                pos_size = int(round(pos_size))
                # 🌙 Moon Dev: Improved RR to 4:1 for capturing larger moves and boosting profitability (Exit Optimization)
                tp = entry + 4 * risk
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if self.data.Low[-1] <= self.tp:
                    ...
                    self._reset_pos()
                    return
            # 🌙 Moon Dev: Further relaxed bandwidth exit to 0.3 to hold winners longer in expanding volatility (Exit Optimization)
            if bandwidth > 0.3:
                self.position.close()
                print(f"🌙 Bandwidth expanded to {bandwidth:.4f}, exiting position 💥")
                self._reset_pos()
                return
            # 🌙 Moon Dev: Added scaling out at 2R to lock in partial profits while letting the rest run (Exit Optimization)
            if self.position.is_long:
                ...
                if favorable_move >= self.initial_risk:
                    # 🌙 Moon Dev: Loosened trailing to EMA20 - 1 ATR for more room to run while protecting gains (Exit Optimization)
                    new_sl = max(self.sl, self.ema20[-1] - self.atr[-1])
                    if new_sl > self.sl:
                        self.sl = new_sl
                ...
                if favorable_move >= self.initial_risk:
                    # 🌙 Moon Dev: Loosened trailing to EMA20 + 1 ATR for more room to run while protecting gains (Exit Optimization)
                    new_sl = min(self.sl, self.ema20[-1] + self.atr[-1])
                    if new_sl < self.sl:
                        self.sl = new_sl
