# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T10_SwiftStrength_OPT_v3.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Long entry signal at {current_price:.2f}, size {position_size:.4f}, SL {sl:.2f}, TP {tp:.2f} 🚀"
                )
            ...
            # Short entry
                ...
                self.entry_set = False  # Will be set next bar
                print(
                    f"🌙 Moon Dev: Short entry signal at {current_price:.2f}, size {position_size:.4f}, SL {sl:.2f}, TP {tp:.2f} 📉"
                )
