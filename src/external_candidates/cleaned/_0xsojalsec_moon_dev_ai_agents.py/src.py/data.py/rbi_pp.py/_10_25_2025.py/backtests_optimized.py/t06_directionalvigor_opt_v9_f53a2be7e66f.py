# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v9.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    adx_weak = (
        20  # 🌙 Moon Dev: Adjusted weakening threshold to match new entry level for consistent trend monitoring 🚀
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Retained 200 EMA for long-term trend filter ✨
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=14
        )  # 🌙 Moon Dev: Added RSI(14) to filter overbought/oversold conditions, improving entry timing and win rate 📊
        self.vol_ma = self.I(
            talib.SMA, self.data.Volume, timeperiod=20
        )  # 🌙 Moon Dev: Added 20-period Volume SMA for confirmation of strong moves, avoiding low-volume traps 🔊
        print(
            "🌙 Moon Dev: Indicators initialized for optimized DirectionalVigor with RSI, Volume MA, lowered ADX threshold, increased risk/TP for 50% target push! ✨"
        )
    ...
    def next(self):
        ...
        if long_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                )  # 🌙 Moon Dev: Removed int(round()) to allow fractional sizing for precise risk control and better capital efficiency 💰
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                position_size = (
                    risk_amount / risk_per_unit
                )  # 🌙 Moon Dev: Fractional sizing applied to shorts for optimized exposure ✨
                sl_price = current_close + risk_per_unit
                tp_price = current_close - (self.tp_multiplier * current_atr)
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if reversal or weakening:
                    ...
                    print(
                        f"🌙 Moon Dev: Exiting LONG at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 📉💫"
                    )
                ...
                if reversal or weakening:
                    ...
                    exit_reason = "Reversal" if reversal else "Weakening Trend"
                    print(
                        f"🌙 Moon Dev: Exiting SHORT at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 📉💫"
                    )
