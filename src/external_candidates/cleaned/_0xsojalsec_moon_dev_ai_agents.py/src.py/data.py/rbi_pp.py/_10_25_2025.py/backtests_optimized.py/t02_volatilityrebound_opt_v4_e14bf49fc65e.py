# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v4.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.rsi[-1] < 30
                and self.data.Volume[-1] > 1.2 * self.sma_vol[-1]
                and self.adx[-1] < 25
            ):
                ...
                print(
                    f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {self.sl:.2f}, TP {self.tp:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, Vol {self.data.Volume[-1]:.0f} > {1.2 * self.sma_vol[-1]:.0f}, ADX {self.adx[-1]:.2f} ✨"
                )
            ...
            # Short entry with tightened filters (symmetric)
                ...
                print(
                    f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {self.sl:.2f}, TP {self.tp:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, Vol {self.data.Volume[-1]:.0f} > {1.2 * self.sma_vol[-1]:.0f}, ADX {self.adx[-1]:.2f} ✨"
                )
