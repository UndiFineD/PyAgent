# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T13_VolatilityReversion_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityReversion(Strategy):
    ...
    adx_max = (
        20  # 🌙 Moon Dev: Reduced from 25 to 20 to focus on stronger ranging (low trend) regimes, avoiding weak trends
    )
    ...
    def next(self):
        ...
        if self.position:
            if self.position.is_long and current_close > self.bb_middle[-1]:
                self.position.close()
                print(
                    f"🌙 Moon Dev: Exiting long position at {current_close:.2f} - Reversion to middle band achieved! ✨"
                )
            elif self.position.is_short and current_close < self.bb_middle[-1]:
                self.position.close()
                print(
                    f"🌙 Moon Dev: Exiting short position at {current_close:.2f} - Reversion to middle band achieved! ✨"
                )
        ...
        if not self.position:
            ...
            if (
                ...
                and self.rsi[-1] < self.rsi_low
                and self.adx[-1] < self.adx_max
                and vol_confirm
                and trend_long
            ):
                stop_price = (
                    current_close - 2 * atr
                )  # 🌙 Moon Dev: Volatility-based SL (2x ATR below entry) for tighter, adaptive risk control vs. fixed band extension
                ...
                if stop_distance > 0:
                    ...
                    if size > 0:
                        self.buy(
                            size=size, sl=stop_price
                        )  # 🌙 Moon Dev: Removed int(round()) for precise fractional sizing in BTC
                        print(
                            f"🚀 Moon Dev: Long entry at {entry_price:.2f}, size {size:.6f} BTC, stop {stop_price:.2f}, risk {risk_amount:.2f} USD 🌙"
                        )

            # Short entry - Added volume and trend filters for higher-quality setups
            elif (
                current_close > self.bb_upper[-1]
                and self.rsi[-1] > self.rsi_high
                and self.adx[-1] < self.adx_max
                and vol_confirm
                and trend_short
            ):
                ...
                if stop_distance > 0:
                    ...
                    if size > 0:
                        self.sell(
                            size=size, sl=stop_price
                        )  # 🌙 Moon Dev: Removed int(round()) for precise fractional sizing
                        print(
                            f"🚀 Moon Dev: Short entry at {entry_price:.2f}, size {size:.6f} BTC, stop {stop_price:.2f}, risk {risk_amount:.2f} USD 🌙"
                        )
