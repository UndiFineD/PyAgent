# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v7.py
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                size = (
                    risk / sl_dist
                )  # 🌙 Moon Dev: Allow fractional sizing for precise risk management (no int rounding)

                tp_dist = (
                    self.rr_ratio * sl_dist
                )  # 🌙 Moon Dev: Dynamic TP based on RR ratio instead of fixed middle BB for consistent reward
                tp = close + tp_dist
                self.buy(size=size, sl=sl, tp=tp)
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol>MA: {vol > vol_ma} 🚀"
                )
            ...
            # Short entry: Improved with High touch on BB upper, tighter RSI, trend filter, volume, and ADX
                ...
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol>MA: {vol > vol_ma} ✨"
                )
