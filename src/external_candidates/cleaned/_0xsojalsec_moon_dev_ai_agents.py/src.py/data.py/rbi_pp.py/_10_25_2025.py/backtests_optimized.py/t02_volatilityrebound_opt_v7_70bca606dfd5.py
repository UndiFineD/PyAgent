    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.rsi[-1] < 30  # Kept original RSI threshold; could tighten to 25 if needed
                and self.data.Close[-1] > self.ema200[-1]  # Trend filter: only long in uptrend
            ...
            # Short entry with tightened filters
                ...
                and self.rsi[-1] > 70  # Kept original RSI threshold; could tighten to 75 if needed
                and self.data.Close[-1] < self.ema200[-1]  # Trend filter: only short in downtrend
                and self.data.Volume[-1] > 1.2 * self.avg_vol[-1]
            ):  # Volume confirmation
