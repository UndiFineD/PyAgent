# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T00_PulsarFlow_DEBUG_v5.py
import numpy as np
import pandas as pd
import talib
...
class PulsarFlow(Strategy):
    ...
    def next(self):
        ...
        # Adjust SL/TP if position just opened
        if self.position and self.stop_loss is None:
            actual_entry = self.trades[-1].entry_price
            self.entry_price = actual_entry
            if self.pending_direction == "long":
                sl = max(actual_entry * (1 - self.sl_percent), self.pending_swing)
                risk = actual_entry - sl
                tp = actual_entry + self.rr_ratio * risk
                self.stop_loss = sl
                self.take_profit = tp
                self.pending_direction = None
                self.pending_swing = None
                print(f"🌙 Moon Dev: LONG position adjusted at {actual_entry:.2f}, SL {sl:.2f}, TP {tp:.2f} ✅")
            elif self.pending_direction == "short":
                sl = min(actual_entry * (1 + self.sl_percent), self.pending_swing)
                ...
                self.pending_direction = None
                self.pending_swing = None
                print(f"🌙 Moon Dev: SHORT position adjusted at {actual_entry:.2f}, SL {sl:.2f}, TP {tp:.2f} ✅")
            ...
            if close > self.ema[current_i] and self.mfi[current_i] <= self.oversold:
                ...
                if div and swing_low is not None and close > swing_low:
                    ...
                    if risk > 0:
                        ...
                        if size > 0:
                            self.pending_swing = swing_low
                            self.pending_direction = "long"
                            self.buy(size=size)
                            intended_tp = entry_price + self.rr_ratio * risk
                            print(
                                f"🌙 Moon Dev: LONG entry signal at {entry_price:.2f}, size {size}, intended SL {sl:.2f}, TP {intended_tp:.2f} 🚀"
                            )
                            return
                else:
            ...
            if close < self.ema[current_i] and self.mfi[current_i] >= self.overbought:
                ...
                if div and swing_high is not None and close < swing_high:
                    ...
                    if risk > 0:
                        ...
                        size = int(round(risk_amount / risk))
                        if size > 0:
                            self.pending_swing = swing_high
                            self.pending_direction = "short"
                            self.sell(size=size)
                            intended_tp = entry_price - self.rr_ratio * risk
                            print(
                                f"🌙 Moon Dev: SHORT entry signal at {entry_price:.2f}, size {size}, intended SL {sl:.2f}, TP {intended_tp:.2f} 📉"
                            )
                            return
                else:
