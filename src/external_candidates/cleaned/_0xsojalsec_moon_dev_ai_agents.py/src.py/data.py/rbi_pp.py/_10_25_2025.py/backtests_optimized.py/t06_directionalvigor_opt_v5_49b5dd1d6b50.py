# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v5.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    trail_start_mult = (
        2.0  # 🌙 Moon Dev: Start trailing after 2x ATR profit to let winners develop before protecting gains ✨
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Updated to shorter EMA for faster trend alignment and more opportunities 💫
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=self.rsi_period
        )  # 🌙 Moon Dev: Added RSI for momentum confirmation to filter out weak signals and improve entry quality 🚀
        self.vol_ma = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.vol_ma_period
        )  # 🌙 Moon Dev: Added volume moving average for liquidity filter to target high-conviction moves ✨
        self.entry_price = 0
        self.is_long_pos = False
        print(
            "🌙 Moon Dev: Indicators initialized with RSI, Volume filter, shorter EMA/ADX, and trailing stop setup for optimized returns! 🚀✨"
        )

    def next(self):
        if (
            len(self.data) < 101
        ):  # 🌙 Moon Dev: Adjusted buffer for shorter periods (ADX10 + EMA50 + others) to ensure stable initialization 💫
            return
        ...
        long_signal = (
            ...
            and (current_rsi > self.rsi_long_threshold)
            and (current_vol > current_vol_ma * self.vol_multiplier)
        )
        ...
        if long_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                position_size = int(
                    round(position_size)
                )  # 🌙 Moon Dev: Retained integer sizing with increased risk for balanced scaling ✨
                sl_price = current_close - risk_per_unit
                self.buy(
                    size=position_size, sl=sl_price
                )  # 🌙 Moon Dev: Removed fixed TP to enable dynamic trailing for letting winners run longer 🚀
                self.entry_price = current_close
                self.is_long_pos = True
                print(
                    f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {position_size} | SL: {sl_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        short_signal = (
            ...
            and (current_adx > prev_adx)
            and (current_close < current_ema)
            and (current_rsi < self.rsi_short_threshold)
            and (current_vol > current_vol_ma * self.vol_multiplier)
        )
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                self.sell(
                    size=position_size, sl=sl_price
                )  # 🌙 Moon Dev: Removed fixed TP for trailing on shorts to maximize downside capture 📈
                self.entry_price = current_close
                self.is_long_pos = False
                print(
                    f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {position_size} | SL: {sl_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        if self.position and self.entry_price > 0:
            ...
            if self.is_long_pos:
                ...
                if reversal or weakening:
                    ...
                    print(
                        f"🌙 Moon Dev: Exiting LONG at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} | Profit: {profit:.2f} 📉💫"
                    )
                    self.entry_price = 0
                    self.is_long_pos = False
            else:  # Short position
                ...
                if profit > self.trail_start_mult * current_atr:
                    ...
                    if self.position.sl is None or trail_sl < self.position.sl:
                        self.position.sl = (
                            trail_sl  # 🌙 Moon Dev: Symmetric trailing for shorts to protect profits in downtrends 🚀
                        )
                ...
                if reversal or weakening:
                    ...
                    exit_reason = "Reversal" if reversal else "Weakening Trend"
                    print(
                        f"🌙 Moon Dev: Exiting SHORT at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} | Profit: {profit:.2f} 📉💫"
                    )
                    self.entry_price = 0
                    self.is_long_pos = False
