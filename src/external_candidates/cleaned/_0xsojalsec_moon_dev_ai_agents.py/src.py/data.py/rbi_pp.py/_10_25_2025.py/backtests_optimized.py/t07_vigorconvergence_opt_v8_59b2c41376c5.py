# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v8.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                and current_vol > current_vol_sma
                and current_rsi > 50
            ):  # 🌙 Moon Dev: Added multi-filter confirmation for long entries
                # Long entry
                ...
                )  # 🌙 Moon Dev: Simplified to direct size calc (fractional allowed for BTC), removes int rounding for precision
                ...
                and current_vol > current_vol_sma
                and current_rsi < 50
            ):  # 🌙 Moon Dev: Added multi-filter confirmation for short entries
                # Short entry
                sl_price = current_close + self.atr_multiplier_sl * current_atr
