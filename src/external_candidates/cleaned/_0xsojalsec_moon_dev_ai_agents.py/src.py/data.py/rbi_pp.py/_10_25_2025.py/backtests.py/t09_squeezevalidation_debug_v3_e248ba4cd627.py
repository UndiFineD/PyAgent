# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T09_SqueezeValidation_DEBUG_v3.py
import warnings
    ...
    def next(self):
        ...
        if not self.position and in_squeeze:
            if (
                ...
                print(
                    f"🌙 Moon Dev: Long Entry 🚀 Price: {actual_entry:.2f}, Size: {size:.4f}, SL: {sl_price:.2f}, TP: {tp_price:.2f} ✨"
                )
        ...
        if not self.position and in_squeeze:
            if (
                ...
                self.entry_bar = self.bar_count
                print(
                    f"🌙 Moon Dev: Short Entry 📉 Price: {actual_entry:.2f}, Size: {size:.4f}, SL: {sl_price:.2f}, TP: {tp_price:.2f} ✨"
                )
