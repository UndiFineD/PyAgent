# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T12_AdaptiveKaufman_BT.py
import numpy as np
import pandas as pd
import talib
        ...
        if not self.position:
            ...
            if self.position.is_long:
                ...
                if profit > self.trail_start * atr:
                    new_sl = max(self.position.sl, self.kama[-1] - self.trail_offset * atr)
                ...
                if profit > self.trail_start * atr:
                    new_sl = min(self.position.sl, self.kama[-1] + self.trail_offset * atr)
                    self.position.sl = new_sl
