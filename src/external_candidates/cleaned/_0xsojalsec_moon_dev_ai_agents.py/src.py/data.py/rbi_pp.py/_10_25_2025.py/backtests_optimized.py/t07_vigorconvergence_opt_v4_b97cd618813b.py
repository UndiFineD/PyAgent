# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v4.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                and current_close > current_ema200  # 🌙 Moon Dev: Trend filter - only long in bullish regime
                and current_volume > current_vol_sma  # 🌙 Moon Dev: Volume filter for confirmation
                and current_rsi < 60
            ):  # 🌙 Moon Dev: RSI filter to avoid overbought longs
                # Long entry
                ...
                )  # 🌙 Moon Dev: Simplified and allow fractional size for better precision in crypto trading
                ...
                and current_close < current_ema200  # 🌙 Moon Dev: Trend filter - only short in bearish regime
                and current_volume > current_vol_sma  # 🌙 Moon Dev: Volume filter for confirmation
                and current_rsi > 40
            ):  # 🌙 Moon Dev: RSI filter to avoid oversold shorts
                # Short entry
                sl_price = current_close + self.atr_multiplier_sl * current_atr
