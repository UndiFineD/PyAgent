# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v5.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙📊 Bar {len(self.data)}: Close={current_price:.2f}, EMA50={self.ema50[-1]:.2f}, EMA200={self.ema200[-1]:.2f}, ATR={atr:.2f}, RSI={rsi:.2f}"
            )
        ...
        if not self.position:
            ...
            if long_cross and atr_ok and vol_ok and rsi_long_ok and trend_long_ok:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, Vol OK: {vol_ok}, RSI: {rsi:.2f} ✨"
                    )
                else:
                ...
                if size > 0:
                    ...
                    self.entry_step = len(self.data) - 1
                    print(
                        f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, Vol OK: {vol_ok}, RSI: {rsi:.2f} ✨"
                    )
                else:
