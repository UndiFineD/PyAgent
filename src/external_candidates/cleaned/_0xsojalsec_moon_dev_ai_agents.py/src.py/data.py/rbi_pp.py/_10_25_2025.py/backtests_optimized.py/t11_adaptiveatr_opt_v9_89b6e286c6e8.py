# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙📊 Bar {len(self.data)}: Close={current_price:.2f}, SMA20={self.sma20[-1]:.2f}, SMA100={self.sma100[-1]:.2f}, ATR={atr:.2f}"
            )
        ...
        if not self.position:
            ...
            if long_cross and not self.low_vol_regime and atr_ok and vol_ok and momentum_long and trend_strength:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size:.4f}, ATR: {atr:.2f}, RSI: {self.rsi[-1]:.1f}, ADX: {self.adx[-1]:.1f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                ...
                if size > 0:
                    ...
                    self.entry_step = len(self.data) - 1
                    print(
                        f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size:.4f}, ATR: {atr:.2f}, RSI: {self.rsi[-1]:.1f}, ADX: {self.adx[-1]:.1f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                    print("🌙❌ Short signal but size=0, skipped.")
            elif long_cross or short_cross:
                print(
                    f"🌙⚠️ Crossover detected but filters failed: LowVol={self.low_vol_regime}, ATR_OK={atr_ok}, Vol_OK={vol_ok}, RSI={self.rsi[-1]:.1f}, ADX={self.adx[-1]:.1f}"
                )
            else:
                pass  # No signal
