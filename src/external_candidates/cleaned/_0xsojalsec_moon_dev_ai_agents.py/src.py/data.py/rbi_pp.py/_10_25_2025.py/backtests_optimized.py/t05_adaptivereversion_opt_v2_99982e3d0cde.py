# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v2.py
...
class AdaptiveReversion(Strategy):
    ...
    atr_period = 14
    atr_mult = (
        2.0  # 🌙 Moon Dev: Increased ATR multiplier for wider stops, allowing more room in volatile crypto markets
    )
    tp_rr = 2.0  # 🌙 Moon Dev: Added risk-reward ratio for dynamic TP (1:2), aiming for higher returns per trade
    risk_percent = (
        0.02  # 🌙 Moon Dev: Increased risk per trade to 2% to amplify returns while maintaining sizing discipline
    )
    max_bars_in_trade = (
        20  # 🌙 Moon Dev: Extended time limit to 20 bars, giving trades more time to revert without early exits
    )
    adx_threshold = (
        25  # 🌙 Moon Dev: Added ADX filter for ranging markets (low trend strength), ideal for mean reversion
    )
    vol_mult = (
        1.0  # 🌙 Moon Dev: Volume confirmation multiplier, ensuring entries on above-average volume for better setups
    )
        ...
        if not self.position:
            ...
            if adx < self.adx_threshold and volume > self.vol_ma[-1] * self.vol_mult:
                ...
                if close <= lower and rsi < self.rsi_oversold:
                    ...
                    print(
                        f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f} 🚀"
                    )
                ...
                # Short entry: Mean reversion in overbought ranging conditions
                    ...
                    self.position.entry_time = current_time
                    print(
                        f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f} ✨"
                    )
