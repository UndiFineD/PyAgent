# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v6.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    risk_per_trade = (
        0.015  # 🌙 Moon Dev: Increased to 1.5% risk per trade to amplify returns while still managing drawdown 📈
    )
    atr_multiplier = 2.0
    ...
    def init(self):
        ...
        self.ema200 = self.I(
            talib.EMA, self.data.Close, timeperiod=200
        )  # 🌙 Moon Dev: Retained 200 EMA for robust trend filtering ✨
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=14
        )  # 🌙 Moon Dev: Added RSI for momentum confirmation to filter high-quality entries and avoid overextended moves 📊
        self.avg_vol = self.I(
            talib.SMA, self.data.Volume, timeperiod=20
        )  # 🌙 Moon Dev: Added volume SMA for liquidity filter to ensure entries on supported volume spikes 🚀
        self.entry_price = 0
        self.breakeven = False
        self.initial_sl = 0
        print(
            "🌙 Moon Dev: Enhanced DirectionalVigor with RSI, volume filters, faster ADX, higher RR, and trailing stops for optimized returns! ✨"
        )
    ...
    def next(self):
        ...
        if long_signal and not self.position:
            risk_amount = (
                self.risk_per_trade * self._broker.starting_cash
                if self._broker.starting_cash
                else self.risk_per_trade * self.equity
            )  # 🌙 Moon Dev: Use starting cash for consistent risk calc in backtest 📊
            risk_per_unit = self.atr_multiplier * current_atr
            if risk_per_unit > 0:
                position_size = risk_amount / risk_per_unit
                max_size = (
                    (self.equity / current_close) * 0.95
                )  # 🌙 Moon Dev: Cap size to 95% of available equity to prevent over-leveraging in spot trading 💰
                position_size = min(position_size, max_size)
                position_size = max(
                    0.01, round(position_size, 2)
                )  # 🌙 Moon Dev: Allow fractional sizes for precision in crypto trading, min 0.01 to avoid zero trades ✨
                ...
                print(
                    f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                self.initial_sl = sl_price
                print(
                    f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if profit_distance > activation_level:
                    if not self.breakeven:
                        ...
                        self.breakeven = True
                        print(
                            f"🌙 Moon Dev: LONG breakeven activated at {self.entry_price:.2f} | Profit: {profit_distance:.2f} 💫"
                        )
                    else:
                        # Trail stop
                        ...
                        if new_sl > self.position.sl:
                            ...
                            print(
                                f"🌙 Moon Dev: LONG trailing SL updated to {new_sl:.2f} | Current: {current_close:.2f} 📈✨"
                            )
                ...
                if profit_distance > activation_level:
                    if not self.breakeven:
                        ...
                        self.breakeven = True
                        print(
                            f"🌙 Moon Dev: SHORT breakeven activated at {self.entry_price:.2f} | Profit: {profit_distance:.2f} 💫"
                        )
                    else:
                        # Trail stop
                        new_sl = current_close + trail_level
                        if new_sl < self.position.sl:
                            self.position.sl = new_sl
                            print(
                                f"🌙 Moon Dev: SHORT trailing SL updated to {new_sl:.2f} | Current: {current_close:.2f} 📉✨"
                            )
            ...
            if self.position.is_long:
                ...
                if reversal or weakening:
                    ...
                    print(
                        f"🌙 Moon Dev: Exiting LONG at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} | Trailing integrated 📊💫"
                    )
                ...
                if reversal or weakening:
                    ...
                    exit_reason = "Reversal" if reversal else "Weakening Trend"
                    print(
                        f"🌙 Moon Dev: Exiting SHORT at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} | Trailing integrated 📊💫"
                    )
                    self.breakeven = False  # Reset for next trade
