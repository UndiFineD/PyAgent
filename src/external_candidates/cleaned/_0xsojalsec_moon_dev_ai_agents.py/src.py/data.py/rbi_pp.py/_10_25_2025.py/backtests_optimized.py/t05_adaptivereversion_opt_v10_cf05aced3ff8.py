# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v10.py
...
class AdaptiveReversion(Strategy):
    ...
    rsi_period = 14
    rsi_overbought = (
        75  # 🌙 Moon Dev: Tightened RSI thresholds for stronger mean-reversion signals to reduce false entries
    )
    rsi_oversold = 25  # Improves entry quality by waiting for more extreme oversold/overbought conditions
    sma_period = (
        50  # 🌙 Moon Dev: Shortened SMA period from 200 to 50 for better adaptation to 15m timeframe volatility
    )
    atr_period = 14
    ...
    max_bars_in_trade = (
        20  # 🌙 Moon Dev: Extended time-based exit from 10 to 20 bars (5 hours on 15m) to allow more time for reversion
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Added volume SMA for entry filter to avoid low-volume traps
    ...
    def next(self):
        ...
        if self.position:
            ...
            if bars_in_trade > self.max_bars_in_trade:
                ...
                print(
                    f"🌙 Moon Dev: Time-Based Exit {'Long' if is_long else 'Short'} at {close} after {bars_in_trade} bars ✨"
                )
                return
        ...
        if not self.position:
            vol_confirm = (
                vol > self.vol_mult * vol_avg
            )  # 🌙 Moon Dev: Volume filter to ensure conviction in setups, avoiding choppy low-volume entries
            ...
            if close <= lower and rsi < self.rsi_oversold and close > sma50 and vol_confirm:
                ...
                size = (
                    risk / sl_dist
                )  # 🌙 Moon Dev: Use float size for precise position sizing (no int rounding) to optimize capital use
                ...
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol Confirm: {vol_confirm} 🚀"
                )
            ...
            # Short entry
                ...
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol Confirm: {vol_confirm} ✨"
                )
