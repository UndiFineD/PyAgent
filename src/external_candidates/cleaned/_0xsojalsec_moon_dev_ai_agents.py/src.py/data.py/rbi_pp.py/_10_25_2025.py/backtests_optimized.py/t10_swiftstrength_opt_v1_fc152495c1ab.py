# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T10_SwiftStrength_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class SwiftStrength(Strategy):
    ...
    def next(self):
        ...
        if len(self.trades) == 0 and not self.position:
            ...
            position_size = (
                position_size if position_size > 0 else 0
            )  # 🌙 Allow fractional sizes for finer risk control in crypto
            ...
            if (
                ...
                and (self.macd[-2] < self.macdsignal[-2] and self.macd[-1] > self.macdsignal[-1])
                and
                # 🌙 Removed redundant hist > 0 check as MACD cross implies positive histogram
                current_price > current_ema
                and current_vol > current_vol_sma  # 🌙 Volume filter for higher-quality setups
                and current_rsi > 50
            ):  # 🌙 RSI filter for bullish momentum confirmation
            ...
            # Short entry with tightened filters: volume surge + RSI momentum + EMA trend
                ...
                and (self.macdsignal[-2] < self.macd[-2] and self.macdsignal[-1] > self.macd[-1])
                and
                # 🌙 Removed redundant hist < 0 check as MACD cross implies negative histogram
                current_price < current_ema
                and current_vol > current_vol_sma  # 🌙 Volume filter for higher-quality setups
                and current_rsi < 50
            ):  # 🌙 RSI filter for bearish momentum confirmation
                sl = current_price + sl_distance
                tp = current_price - (self.rr_ratio * sl_distance)
                if sl <= current_price or tp >= current_price:
