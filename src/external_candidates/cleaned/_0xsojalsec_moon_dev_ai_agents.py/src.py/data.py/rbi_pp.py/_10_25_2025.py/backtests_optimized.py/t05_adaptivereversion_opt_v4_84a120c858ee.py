# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v4.py
...
class AdaptiveReversion(Strategy):
    ...
    risk_percent = (
        0.02  # 🌙 Moon Dev: Increased risk per trade to 2% for higher potential returns while maintaining management
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if close <= lower and rsi < self.rsi_oversold and close > sma200 and vol > vol_avg:
                ...
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol: {vol:.0f} > {vol_avg:.0f} 🚀"
                )
            ...
            # Short entry with tightened conditions and volume filter
                ...
                self.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol: {vol:.0f} > {vol_avg:.0f} ✨"
                )
