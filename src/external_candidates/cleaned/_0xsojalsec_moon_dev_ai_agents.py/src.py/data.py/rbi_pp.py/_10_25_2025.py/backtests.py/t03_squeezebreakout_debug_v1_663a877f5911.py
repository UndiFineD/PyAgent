# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T03_SqueezeBreakout_DEBUG_v1.py
import numpy as np
import pandas as pd
import talib
...
class SqueezeBreakout(Strategy):
    ...
    def init(self):
        ...
        # Keltner Channels
        self.kc_upper = self.I(lambda e, a: e + self.kc_mult * a, ema, atr)
        self.kc_lower = self.I(lambda e, a: e - self.kc_mult * a, ema, atr)
    ...
    def next(self):
        ...
        is_lowest_width = not pd.isna(current_bb_width) and not pd.isna(min_width) and current_bb_width == min_width
