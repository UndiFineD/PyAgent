# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v7.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    def init(self):
        ...
        self.entry_bar = None
        print(
            "🌙 Moon Dev Backtest Initialized: Super-Optimized DonchianAscent with ATR Trailing & Loosened Filters Loaded! Target 50% Locked In! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            )  # 🌙 New: Require bullish candle on breakout for conviction, filtering flat/doji breaks
            ...
            if (
                ...
                sl_price = current_close - (
                    self.atr_sl_mult * current_atr
                )  # ✨ Switched to ATR-based SL (1.5x) for tighter, vol-adjusted protection vs. distant channel low
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🌙 Moon Dev Long Entry: Optimized ATR Breakout at {current_close}, size={position_size:.4f} BTC, SL={sl_price:.2f} (RSI={current_rsi:.1f}, ADX={current_adx:.1f}, Trend=Up, Ascending=Yes, Strong Candle=Yes) ✨🚀"
                    )
