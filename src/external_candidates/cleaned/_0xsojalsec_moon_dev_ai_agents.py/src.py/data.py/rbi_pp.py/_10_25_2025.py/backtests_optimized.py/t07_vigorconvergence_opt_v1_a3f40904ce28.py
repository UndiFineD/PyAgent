# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class VigorConvergence(Strategy):
    ...
    atr_multiplier_sl = (
        1.2  # 🌙 Moon Dev: Tightened from 1.5 to 1.2 for closer stop losses to reduce risk exposure per trade
    )
    rr_ratio = (
        2.5  # 🌙 Moon Dev: Increased from 2.0 to 2.5 for better reward-to-risk, capturing more upside in trending moves
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                size = (
                    position_value / current_close
                )  # 🌙 Moon Dev: Allow float for fractional BTC units, better for crypto and precise risk
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Backtest Progress: {self.data.index[-1]}, ADX: {current_adx:.2f}, MACD: {current_macd:.2f}, RSI: {current_rsi:.2f}, Above EMA200: {current_close > current_ema200} ✨"
            )
