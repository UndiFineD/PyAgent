# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v2.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if long_entry and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                )  # Optimized: Use float for fractional sizes (more precise, no int rounding)
                sl_price = current_close - risk_per_unit
                tp_price = (
                    current_close + self.tp_atr_multiplier * current_atr
                )  # Added: Take profit at 4 ATR for improved reward:risk
        ...
        if self.position:
            ...
            if reversal or weakening:
                ...
                print(
                    f"🌙 Moon Dev: Exiting {'LONG' if self.position.is_long else 'SHORT'} at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} 📊💫"
                )
