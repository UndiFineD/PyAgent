...
class VigorConvergence(Strategy):
    adx_threshold = (
        28  # 🌙 Optimization: Increased ADX threshold to 28 for stronger trend confirmation, reducing false signals
    )
    risk_per_trade = 0.01  # 🌙 Optimization: Kept at 1% but with better filters, this allows consistent risk
    atr_multiplier_sl = (
        2.0  # 🌙 Optimization: Widened SL to 2x ATR to give trades more breathing room and reduce whipsaws
    )
    rr_ratio = 3.0  # 🌙 Optimization: Increased RR to 3:1 for higher reward potential on winning trades
    adx_exit_threshold = (
        15  # 🌙 Optimization: Lowered emergency exit threshold to 15 to avoid closing winners prematurely
    )
