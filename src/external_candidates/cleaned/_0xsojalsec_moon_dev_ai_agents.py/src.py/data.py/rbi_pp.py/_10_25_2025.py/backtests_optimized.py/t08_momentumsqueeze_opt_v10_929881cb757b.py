# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T08_MomentumSqueeze_OPT_v10.py
import numpy as np
import pandas as pd
import talib
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if favorable_move >= 4 * self.initial_risk and not self.partial2_closed and self.partial_closed:
                    ...
                    print(
                        f"🌙 Moon Dev: Scaled out additional 25% of original long at 4R, remaining size: {self.position.size} 💰"
                    )
                ...
                if favorable_move >= 4 * self.initial_risk and not self.partial2_closed and self.partial_closed:
                    ...
                    self.partial2_closed = True
                    print(
                        f"🌙 Moon Dev: Scaled out additional 25% of original short at 4R, remaining size: {self.position.size} 💰"
                    )
                # Trailing stop after 1:1
                if favorable_move >= self.initial_risk:
                    # 🌙 Moon Dev: Adjusted trailing to EMA50 ± ATR for smoother trailing with less noise (Exit Optimization)
