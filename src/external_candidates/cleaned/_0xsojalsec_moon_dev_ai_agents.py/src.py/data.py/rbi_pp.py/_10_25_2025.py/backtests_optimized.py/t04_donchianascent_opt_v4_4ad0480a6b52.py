# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v4.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    period = 20
    risk_percent = (
        0.02  # 🌙 Increased risk per trade to 2% to allow for higher potential returns while scaling towards 50% target
    )
    ...
    min_channel_width_pct = (
        0.015  # 🌙 Tightened minimum channel width to 1.5% to filter out more low-volatility whipsaw setups
    )
    extended_multiplier = (
        1.5  # 🌙 Reduced extension multiplier to allow slightly more aggressive entries without overextension
    )
    atr_period = 14
    ...
    def init(self):
        ...
        )  # 🌙 Added ATR for volatility-based risk management
        self.sma200 = self.I(
            talib.SMA, self.data.Close, timeperiod=200
        )  # 🌙 Added long-term SMA trend filter to only trade in uptrends
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=self.rsi_period
        )  # 🌙 Added RSI for momentum confirmation and overbought avoidance
        self.entry_bar = None
    ...
    def next(self):
        ...
        if self.position:
            ...
            vol_confirm = current_vol > (
                self.vol_multiplier * prev_avg_vol
            )  # 🌙 Strengthened volume filter for higher quality breakouts
            channel_too_narrow = (
                prev_width / current_close
            ) < self.min_channel_width_pct  # Tighter filter to avoid chop
            extended = (current_close - prev_middle) > (
                self.extended_multiplier * prev_width
            )  # Adjusted to filter climactic moves
            uptrend = (
                current_close > current_sma200
            )  # 🌙 Trend filter: only long in overall uptrend to avoid counter-trend trades
            rsi_confirm = (
                current_rsi > self.rsi_lower and current_rsi < self.rsi_upper
            )  # 🌙 RSI filter for momentum without overbought
            ...
            if breakout and vol_confirm and not channel_too_narrow and not extended and uptrend and rsi_confirm:
                ...
                if risk_per_share > 0:
                    position_size = int(
                        round((self.equity * self.risk_percent) / risk_per_share)
                    ...
                    print(
                        f"🌙 Moon Dev Long Entry: Optimized Breakout at {current_close}, size={position_size}, SL={sl_price}, TP={tp_price} ✨🚀"
                    )


# Run backtest
bt = Backtest(data, DonchianAscent, cash=1000000, commission=0.002)
stats = bt.run()
print(stats)
