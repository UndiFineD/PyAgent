...
class DonchianAscent(Strategy):
    ...
    risk_percent = (
        0.02  # 🌙 Optimized: Increased risk per trade from 1% to 2% to amplify returns while monitoring drawdown
    )
    ...
    min_channel_width_pct = (
        0.015  # 🌙 Optimized: Increased min width from 0.01 to 0.015 to filter out more choppy, low-volatility setups
    )
    extended_multiplier = (
        1.5  # 🌙 Optimized: Reduced from 2 to 1.5 to allow slightly more extended breakouts but avoid over-chasing
    )
