# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v5.py
import numpy as np
import pandas as pd
import talib
...
class VigorConvergence(Strategy):
    ...
    risk_per_trade = (
        0.015  # 🌙 Moon Dev: Slightly increased from 0.01 to 0.015 to amplify returns while keeping risk managed
    )
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                and current_close > current_ema50  # 🌙 Moon Dev: Trend filter - only long above EMA50
                and current_rsi < 70  # 🌙 Moon Dev: Avoid overbought longs
                and current_volume > current_vol_sma
            ):  # 🌙 Moon Dev: Volume filter for momentum confirmation
                # Long entry
                ...
                )  # 🌙 Moon Dev: Simplified to float size for precise risk (no int rounding)
                ...
                and current_close < current_ema50  # 🌙 Moon Dev: Trend filter - only short below EMA50
                and current_rsi > 30  # 🌙 Moon Dev: Avoid oversold shorts
                and current_volume > current_vol_sma
            ):  # 🌙 Moon Dev: Volume filter for momentum confirmation
                # Short entry
                sl_price = current_close + self.atr_multiplier_sl * current_atr
