...
class QuadrupleHarmonic(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and current_close > self.ema_short[-1]  # 🌙 NEW: Above short EMA for pullback end confirmation
            and self.adx[-1]
            > 22  # 🌙 ADJUSTED: Slightly raised from 20 to 22 for stronger trends, balancing signal quality
            and self.rsi[-1] < 35  # 🌙 TIGHTENED: Lowered from 40 to 35 for deeper, higher-quality oversold pullbacks
            and self.data.Volume[-1]
            > 2.0 * self.vol_sma[-1]  # 🌙 TIGHTENED: Increased volume multiplier to 2x for higher conviction reversals
            and valid_depth
