...
class FibonacciCrossover(Strategy):
    ...
    volume_ma_period = 20
    risk_per_trade = (
        0.005  # 🌙 Moon Dev: Reduced from 0.01 to 0.005 for tighter risk control and better capital preservation
    )
    rr_ratio = 3  # 🌙 Moon Dev: Increased from 2 to 3 for higher reward potential on winning trades
    atr_multiplier_sl = (
        2.0  # 🌙 Moon Dev: Increased from 1.5 to 2.0 for wider stops to reduce whipsaws in volatile crypto
    )
    confluence_threshold = (
        0.002  # 🌙 Moon Dev: Tightened from 0.005 to 0.002 for stricter harmonic approximation near EMA21
    )
    ...
    def next(self):
        ...
        if (
            ...
            and current_rsi > 50  # 🌙 Moon Dev: Tightened from >40 to >50 for stronger bullish momentum
            and current_close > self.ema200[-1]  # 🌙 Moon Dev: Using EMA200 instead of SMA200
            and current_volume > self.volume_ma[-1] * self.volume_multiplier  # 🌙 Moon Dev: Stronger volume requirement
            and harmonic_long
        ...
        # Short Entry - added ADX filter, tightened RSI to <50, volume *1.5, EMA200
            ...
            and self.ema5[-1] < self.ema13[-1]
            and self.ema21[-1] < self.ema55[-1]
            and current_rsi < 50  # 🌙 Moon Dev: Tightened from <60 to <50 for stronger bearish momentum
            and current_close < self.ema200[-1]  # 🌙 Moon Dev: Using EMA200 instead of SMA200
            and current_volume > self.volume_ma[-1] * self.volume_multiplier  # 🌙 Moon Dev: Stronger volume requirement
            and harmonic_short
            and current_adx > self.adx_threshold
