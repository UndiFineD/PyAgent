...
class FibonacciCrossover(Strategy):
    ...
    atr_period = 14
    adx_period = (
        14  # 🌙 Moon Dev: Added ADX for trend strength filtering to avoid choppy markets and improve signal quality
    )
    adx_threshold = 20  # 🌙 Moon Dev: ADX > 20 ensures sufficient trend strength for entries
    sma200_period = 200
    volume_ma_period = 20
    risk_per_trade = (
        0.01  # 🌙 Moon Dev: Increased risk per trade from 0.005 to 0.01 to amplify returns while keeping risk managed
    )
    ...
    confluence_threshold = (
        0.01  # 🌙 Moon Dev: Kept confluence threshold at 0.01 but made directional for better alignment with trend
    )
    ...
    def next(self):
        ...
        if (
            ...
            and current_rsi > 45  # 🌙 Moon Dev: Loosened RSI from >50 to >45 to capture more bullish momentum setups
            ...
            * self.volume_ma[
            ...
            and self.plus_di[-1]
            > self.minus_di[-1]  # 🌙 Moon Dev: Added ADX >20 and +DI > -DI for strong uptrend confirmation
            and harmonic_long
        ):
        ...
        # Short Entry - Enhanced with ADX/DI trend confirmation, loosened RSI (<55), volume (1.2x MA), directional confluence
            ...
            and self.ema21[-1] < self.ema55[-1]
            and self.ema13[-1] < self.ema21[-1]
            and current_rsi < 55  # 🌙 Moon Dev: Loosened RSI from <50 to <55 to capture more bearish momentum setups
            and current_close < self.sma200[-1]
            and current_volume
            > 1.2
            * self.volume_ma[
            ...
            and self.adx[-1] > self.adx_threshold
            and self.minus_di[-1]
            > self.plus_di[-1]  # 🌙 Moon Dev: Added ADX >20 and -DI > +DI for strong downtrend confirmation
            and harmonic_short
        ):
            if np.isnan(current_atr) or current_atr <= 0:
