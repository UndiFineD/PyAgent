...
class ConfluentHarmonics(Strategy):
    ...
    adx_period = 14
    adx_threshold = (
        20  # 🌙 OPTIMIZATION: Lowered from 25 to 20 to capture more trending opportunities without excessive noise
    )
    ...
    rr_ratio = (
        3  # 🌙 OPTIMIZATION: Increased from 2 to 3 for higher reward potential per trade, targeting better returns
    )
    risk_per_trade = (
        0.015  # 🌙 OPTIMIZATION: Increased from 0.01 to 0.015 (1.5%) to amplify returns while keeping risk controlled
    )
    rsi_period = 14
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and pdi_val > mdi_val
                and squeeze
                and rsi_val < self.rsi_oversold  # 🌙 NEW: RSI filter for oversold confirmation
            ...
            # Bearish Reversal Proxy: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze, RSI overbought, volume confirm, trend filter
                ...
                and mdi_val > pdi_val
                and squeeze
                and rsi_val > self.rsi_overbought  # 🌙 NEW: RSI filter for overbought confirmation
                and vol_confirm  # 🌙 NEW: Volume filter
                and trend_short
            ):  # 🌙 NEW: Trend alignment filter
                entry = price
