# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v3.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev's Optimized ConfluentHarmonics Strategy Initialized: Added trend, RSI, volume filters & trailing stops ✨"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and pdi_val > mdi_val
                and squeeze
                and price > ema50_val  # 🌙 Trend filter: Only long in uptrend
                and rsi_val < self.rsi_oversold  # 🌙 Oversold confirmation
                and vol_filter
            ):  # 🌙 Volume momentum
                entry = price
                ...
                print(
                    f"🌙 Bullish ConfluentHarmonics Entry at {self.data.index[-1]} 🚀 Size Frac: {size_frac}, Entry: {entry}, SL: {sl}, TP: {tp}, RSI: {rsi_val}, Vol Filter: {vol_filter}"
                )
            ...
            # Bearish Reversal: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze, trend down, RSI overbought, volume spike
                ...
                and mdi_val > pdi_val
                and squeeze
                and price < ema50_val  # 🌙 Trend filter: Only short in downtrend
                and rsi_val > self.rsi_overbought  # 🌙 Overbought confirmation
                and vol_filter
            ):  # 🌙 Volume momentum
                entry = price
                risk_dist = self.atr_sl_mult * atr_val
                ...
                print(
                    f"🌙 Bearish ConfluentHarmonics Entry at {self.data.index[-1]} ✨ Size Frac: {size_frac}, Entry: {entry}, SL: {sl}, TP: {tp}, RSI: {rsi_val}, Vol Filter: {vol_filter}"
                )
