# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v1.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    risk_per_trade = (
        0.015  # 🌙 Optimized: Increased from 0.01 to 0.015 for slightly more aggressive sizing to boost returns
    )
    rsi_period = 14
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and squeeze
                and vol_confirm
                and rsi_val < self.rsi_long_threshold
            ):
                entry = price
            ...
            # Bearish Reversal Proxy: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze, volume confirm, RSI overbought
                ...
                and squeeze
                and vol_confirm
                and rsi_val > self.rsi_short_threshold
            ):
                entry = price
                risk_dist = self.atr_sl_mult * atr_val
                sl = entry + risk_dist
