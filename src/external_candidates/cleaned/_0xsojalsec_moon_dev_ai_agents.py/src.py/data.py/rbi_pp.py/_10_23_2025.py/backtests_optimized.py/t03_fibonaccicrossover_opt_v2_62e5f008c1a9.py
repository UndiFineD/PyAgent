# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v2.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    volume_ma_period = 20
    risk_per_trade = (
        0.005  # 🌙 Moon Dev: Reduced risk per trade from 0.01 to 0.005 to lower drawdown and improve risk management
    )
    ...
    def next(self):
        ...
        if (
            ...
            > self.ema21[
                -1
            ]  # 🌙 Moon Dev: Added EMA13 > EMA21 filter to ensure momentum aligns with medium-term trend, improving entry quality
            and current_rsi > 50  # 🌙 Moon Dev: Tightened RSI from >40 to >50 to avoid weaker bullish signals
            and current_close > self.sma200[-1]
            and current_volume
            > 1.5
            * self.volume_ma[
                -1
            ]  # 🌙 Moon Dev: Increased volume filter from >MA to >1.5*MA for stronger confirmation of interest
            and harmonic_long
        ):
            ...
            self.tp = tp_price
            print(
                f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} 🚀 Optimized Harmonic Confluence Confirmed ✨"
            )
        ...
        # Short Entry - Optimized with tighter RSI (<50), EMA13 < EMA21 filter for stronger downtrend, and higher volume threshold (1.5x MA)
            ...
            and current_rsi < 50  # 🌙 Moon Dev: Tightened RSI from <60 to <50 to avoid weaker bearish signals
            and current_close < self.sma200[-1]
            and current_volume
            > 1.5
            * self.volume_ma[
            ...
            self.tp = tp_price
            print(
                f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} 📉 Optimized Harmonic Confluence Confirmed ✨"
            )
