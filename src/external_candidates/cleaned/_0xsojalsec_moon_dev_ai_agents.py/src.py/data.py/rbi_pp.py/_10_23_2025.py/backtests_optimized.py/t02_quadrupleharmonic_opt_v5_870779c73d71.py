...
class QuadrupleHarmonic(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and self.adx[-1] > 25  # 🌙 Raised ADX from 20 to 25 for stronger trend filter (reduces choppy entries)
            and self.rsi[-1] < 35  # 🌙 Tightened RSI from <40 to <35 for deeper oversold pullbacks (better timing)
            and self.data.Volume[-1]
            > 2.0 * self.vol_sma[-1]  # 🌙 Increased volume filter from 1.5x to 2x for higher conviction signals
            and self.macd[-1] > self.macdsignal[-1]  # 🌙 New: MACD bullish crossover for momentum confirmation
