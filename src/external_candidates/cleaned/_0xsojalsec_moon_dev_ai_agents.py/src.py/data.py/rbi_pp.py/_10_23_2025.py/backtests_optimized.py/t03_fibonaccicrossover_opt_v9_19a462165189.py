...
class FibonacciCrossover(Strategy):
    ...
    adx_threshold = (
        25  # 🌙 Moon Dev: Kept ADX >25 for strong trends, but added rising ADX filter below for better entry timing
    )
    ...
    def next(self):
        ...
        if (
            ...
            and self.ema55[-1] > self.sma200[-1]  # 🌙 Moon Dev: Retained EMA55 > SMA200 for uptrend confirmation
            and current_rsi > 50  # 🌙 Moon Dev: Loosened from >55 to >50 for more bullish momentum opportunities
            and current_close > self.sma200[-1]
            and current_volume
            > self.volume_multiplier * self.volume_ma[-1]  # 🌙 Moon Dev: Loosened to 1.5x for increased trade frequency
        ...
        # Short Entry - Symmetric enhancements: Loosened RSI to <50, volume to 1.5x, added rising ADX
            ...
            and self.ema21[-1] < self.ema55[-1]
            and self.ema13[-1] < self.ema21[-1]
            and self.ema55[-1] < self.sma200[-1]  # 🌙 Moon Dev: Retained EMA55 < SMA200 for downtrend confirmation
            and current_rsi < 50  # 🌙 Moon Dev: Loosened from <45 to <50 for more bearish momentum opportunities
            and current_close < self.sma200[-1]
            and current_volume
            > self.volume_multiplier * self.volume_ma[-1]  # 🌙 Moon Dev: Loosened to 1.5x for increased trade frequency
            and current_adx > self.adx_threshold  # 🌙 Moon Dev: ADX trend filter
