...
def fib_support(high, low, period=20):  # Shortened period to 20 for more responsive Fib levels in 15m timeframe 🌙
...
class KalmanResonance(Strategy):
    ...
    max_bars_in_trade = 8  # Further reduced to exit non-performers quicker, improving turnover 🚀
    ...
    def next(self):
        ...
        if is_ranging and self.adx[-1] < 20:  # Stricter ADX for purer ranging to avoid whipsaws ✨
            vel_cross_up = self.velocity[-2] < 0 and self.velocity[-1] > 0  # Strict cross to positive velocity
            stoch_oversold_cross = (
                self.stoch_k[-2] <= self.stoch_d[-2] and self.stoch_k[-1] > self.stoch_d[-1] and self.stoch_k[-1] < 30
            ...
            vol_confirm = self.data.Volume[-1] > self.avg_vol[-1]  # Keep volume > avg for momentum
        ...
        if is_ranging and self.adx[-1] < 20:
            ...
            stoch_overbought_cross = (
                self.stoch_k[-2] >= self.stoch_d[-2] and self.stoch_k[-1] < self.stoch_d[-1] and self.stoch_k[-1] > 70
            ...
            if vel_cross_down and stoch_overbought_cross and near_resistance and vol_confirm:
                ...
                if stop_distance > 0:
                    ...
                    size = 0.01 / stop_pct
                    size = min(size, 0.95)
                    tp = self.data.Close[-1] - 3 * stop_distance  # Improved to 1:3 RR 🌙
