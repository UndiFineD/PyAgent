# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_final\T04_KalmanStochastic_BTFinal_v1.py
import numpy as np
import pandas as pd
import talib
...
class KalmanStochastic(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            entry_price = self.trades[-1].entry_price if self.trades else self.data.Close[-1]
            if self.position.is_long:
                ...
                if self.stoch_k[-1] > 80:
                    self.position.close()
                    self.debug_print(
                        f"🌙 Moon Dev: Long exit on overbought Stochastic 🚀💥 Close: {self.data.Close[-1]}"
                    )
            else:  # short
                # Breakeven after 1:1
                ...
                if self.stoch_k[-1] < 20:
                    self.position.close()
                    self.debug_print(
                        f"🌙 Moon Dev: Short exit on oversold Stochastic 🌙💥 Close: {self.data.Close[-1]}"
                    )
            # Time-based exit (10 bars)
            if self.position and hasattr(self.position, "_age") and self.position._age >= 10:
                self.position.close()
