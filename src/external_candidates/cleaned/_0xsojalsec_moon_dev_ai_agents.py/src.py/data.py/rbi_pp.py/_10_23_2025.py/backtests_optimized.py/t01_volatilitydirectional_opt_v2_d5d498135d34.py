# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_VolatilityDirectional_OPT_v2.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityDirectional(Strategy):
    ...
    def next(self):
        ...
        if (
            not self.position
            and (pullback_long or squeeze_long)
            and adx_strong
            and plus_cross
            and trend_up
            and vol_confirm
            ...
            if risk_dist > 0:
                ...
                max_size = (
                    self.equity * 0.95 / entry_price
                )  # New: Cap position to 95% of equity to prevent overleverage
                ...
                print(
                    f"🌙 Moon Dev: Long entry at {entry_price:.2f}, size {pos_size:.4f}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀"
                )
        ...
        if (
            not self.position
            and (pullback_short or squeeze_short)
            and adx_strong
            and minus_cross
            and trend_down
            and vol_confirm
            ...
            if risk_dist > 0:
                ...
                self.tp_price = tp_price
                print(
                    f"🌙 Moon Dev: Short entry at {entry_price:.2f}, size {pos_size:.4f}, SL {sl_price:.2f}, TP {tp_price:.2f} ✨"
                )
        ...
        if self.position:
            ...
            # TP check (manual)
            if (self.position.is_long and close >= self.tp_price) or (
                self.position.is_short and close <= self.tp_price
            ):
                self.position.close()
                self.entry_price = None
                self.current_sl = None
            ...
            if (self.position.is_long and close <= self.current_sl) or (
                self.position.is_short and close >= self.current_sl
            ):
                self.position.close()
                self.entry_price = None
                self.current_sl = None
