# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v7.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    risk_per_trade = (
        0.015  # 🌙 Optimized: Slightly higher risk per trade (1.5%) to accelerate returns without excessive drawdown
    )
    ...
    def next(self):
        ...
        )  # 🌙 Optimized: Use only current bb_width for squeeze to focus on active contractions
        ...
        if not self.position:
            ...
            if (
                ...
                and self.ema_fast[-1] > self.ema_slow[-1]  # Trend filter
                and self.rsi[-1] > self.rsi_long_threshold  # Momentum filter
            ...
            # 🌙 Optimized Bearish Entry: Added trend filter (price < EMA50 < EMA200), RSI < 70 (momentum), and volume > 1.5x SMA for higher-quality setups
                ...
                and price < self.ema_fast[-1]
                and self.ema_fast[-1] < self.ema_slow[-1]  # Trend filter
                and self.rsi[-1] < self.rsi_short_threshold  # Momentum filter
                and self.data.Volume[-1] > self.vol_mult * self.vol_sma[-1]
            ):  # Volume filter
                entry = price
                risk_dist = self.atr_sl_mult * atr_val
