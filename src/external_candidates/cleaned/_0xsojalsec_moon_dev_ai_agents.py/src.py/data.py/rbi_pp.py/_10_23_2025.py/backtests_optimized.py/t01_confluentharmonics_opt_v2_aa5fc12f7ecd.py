# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v2.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    bb_squeeze_threshold = (
        0.03  # 🌙 Optimized: Tightened from 0.04 to 0.03 for higher-quality squeeze setups, reducing false signals
    )
    atr_period = 14
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and pdi_val > mdi_val
                and squeeze
                and self.rsi[-2] < self.rsi_oversold  # 🌙 New: Confirm oversold at BB touch
                and price > self.sma_trend[-1]  # 🌙 New: Trend filter - only long in uptrend
                and self.data.Volume[-1] > self.vol_sma[-1]
            ):  # 🌙 New: Volume surge for momentum
                entry = price
            ...
            # 🌙 Optimized: Added RSI overbought filter, trend filter (below SMA200), and volume confirmation for better entry quality
                ...
                and mdi_val > pdi_val
                and squeeze
                and self.rsi[-2] > self.rsi_overbought  # 🌙 New: Confirm overbought at BB touch
                and price < self.sma_trend[-1]  # 🌙 New: Trend filter - only short in downtrend
                and self.data.Volume[-1] > self.vol_sma[-1]
            ):  # 🌙 New: Volume surge for momentum
                entry = price
                risk_dist = self.atr_sl_mult * atr_val
