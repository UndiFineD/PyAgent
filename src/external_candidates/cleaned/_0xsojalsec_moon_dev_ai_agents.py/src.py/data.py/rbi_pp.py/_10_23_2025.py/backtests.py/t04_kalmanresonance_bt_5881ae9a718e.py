...
def kalman_velocity_position(close, q=0.1, r=1.0):
    ...
    n = len(close)
    if n == 0:
        return pd.Series(np.nan, index=pd.Index([])), pd.Series(np.nan, index=pd.Index([]))
    ...
    for i in range(n):
        ...
        filtered[i] = x[0]
        velocity[i] = x[1]
    return pd.Series(filtered, index=close.index), pd.Series(velocity, index=close.index)
