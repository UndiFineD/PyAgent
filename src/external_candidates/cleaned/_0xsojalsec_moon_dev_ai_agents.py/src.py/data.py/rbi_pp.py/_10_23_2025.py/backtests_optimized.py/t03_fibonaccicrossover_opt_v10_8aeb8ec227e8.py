# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v10.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    rsi_long_threshold = (
        52  # 🌙 Moon Dev: Loosened RSI long threshold from 55 to 52 for earlier bullish momentum capture
    )
    rsi_short_threshold = (
        48  # 🌙 Moon Dev: Loosened RSI short threshold from 45 to 48 for earlier bearish momentum capture
    )
    trail_trigger_rr = (
        2.0  # 🌙 Moon Dev: New: Trigger trailing stop after reaching 2R profit to lock in gains during extended trends
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Retained ADX to filter for trending regimes only

        print(
            "🌙 Moon Dev: Initialized Enhanced FibonacciCrossover Strategy with Loosened Filters, Adjusted Risk, and Trailing Stops for Higher Returns ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            if (
                not hasattr(self, "sl")
                or not hasattr(self, "tp")
                or not hasattr(self, "entry_price")
                or not hasattr(self, "risk_distance")
            ):
            ...
            if self.position.is_long:
                ...
                if profit_rr >= self.trail_trigger_rr:
                    ...
                    print(
                        f"🌙 Moon Dev: LONG Trailing SL Updated to {self.sl:.2f} (Profit RR: {profit_rr:.2f}, ATR: {current_atr:.2f}) 🔒 Locking Profits"
                    )
                ...
                if current_low <= self.sl:
                    ...
                    if hasattr(self, "risk_distance"):
                        del self.risk_distance
                    return
                ...
                if profit_rr >= self.trail_trigger_rr:
                    ...
                    print(
                        f"🌙 Moon Dev: SHORT Trailing SL Updated to {self.sl:.2f} (Profit RR: {profit_rr:.2f}, ATR: {current_atr:.2f}) 🔒 Locking Profits"
                    )
        ...
        if (
            ...
            > self.sma200[
                -1
            ]  # 🌙 Moon Dev: Retained EMA55 > SMA200 filter to confirm long-term uptrend alignment, reducing counter-trend trades
            and current_rsi > self.rsi_long_threshold  # 🌙 Moon Dev: Loosened RSI to >52 for more bullish setups
            and current_close > self.sma200[-1]
            and current_volume
            > self.volume_multiplier
            * self.volume_ma[
                -1
            ]  # 🌙 Moon Dev: Loosened volume filter to 1.5x MA for increased entry frequency with solid participation
            and current_adx > self.adx_threshold  # 🌙 Moon Dev: Loosened ADX to 22 to include more trending conditions
            and harmonic_long
        ):
            ...
            print(
                f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀 Optimized Entry with Trailing Potential Confirmed ✨"
            )
        ...
        # Short Entry - Enhanced with loosened ADX (22), RSI (<48), volume (1.5x), confluence (0.007), and retained trend filters
            ...
            < self.sma200[
                -1
            ]  # 🌙 Moon Dev: Retained EMA55 < SMA200 filter to confirm long-term downtrend alignment, reducing counter-trend trades
            and current_rsi < self.rsi_short_threshold  # 🌙 Moon Dev: Loosened RSI to <48 for more bearish setups
            and current_close < self.sma200[-1]
            and current_volume
            > self.volume_multiplier
            * self.volume_ma[
            ...
            self.risk_distance = sl_distance
            print(
                f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 📉 Optimized Entry with Trailing Potential Confirmed ✨"
            )
