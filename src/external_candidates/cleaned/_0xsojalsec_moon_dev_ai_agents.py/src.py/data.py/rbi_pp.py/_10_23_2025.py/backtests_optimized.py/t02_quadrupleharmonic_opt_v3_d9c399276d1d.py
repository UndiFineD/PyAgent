...
class QuadrupleHarmonic(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and self.rsi[-1] < 45  # 🌙 Loosened RSI to <45 for capturing shallower pullbacks with potential
            and self.data.Volume[-1]
            > 1.3 * self.vol_sma[-1]  # 🌙 Adjusted volume to 1.3x for more signals while keeping conviction
