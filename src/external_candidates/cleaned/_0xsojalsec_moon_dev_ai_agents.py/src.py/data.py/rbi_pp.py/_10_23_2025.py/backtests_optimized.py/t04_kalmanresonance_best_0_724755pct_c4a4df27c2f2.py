...
class KalmanResonance(Strategy):
    ...
    ranging_threshold = 0.05  # Tightened to 5% around EMA for stricter ranging detection ✨
    ...
    def next(self):
        ...
        if is_ranging and self.adx[-1] < 25:
            ...
            near_resistance = (
                abs(self.data.Close[-1] - self.fib_resistance[-1]) / self.data.Close[-1] < self.prz_tolerance
            )
