# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_GENERATED_STRATEGIES\MomentumVolatilityFusion_BT.py
# -*- coding: utf-8 -*-
...
class MomentumVolatilityFusion(Strategy):
    ...
    def init(self):
        ...
        self.volatility = self.I(
            lambda x: talib.STDDEV(x, timeperiod=self.volatility_period)
            / talib.SMA(x, timeperiod=self.volatility_period),
            self.data.Close,
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and current_volatility > self.volatility_threshold
            ...
            if self.position.is_long:
                if self.rsi[-1] > 75 or self.ema_fast[-1] < self.ema_slow[-1] or self.momentum[-1] < -50:
                    self.position.close()

            elif self.position.is_short:
                if self.rsi[-1] < 25 or self.ema_fast[-1] > self.ema_slow[-1] or self.momentum[-1] > 50:
                    self.position.close()
