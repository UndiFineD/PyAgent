# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_21_2025\backtests_final\VolatilityBandDivergence_BTFinal.py
import pandas as pd
import talib
...
class VolatilityBandDivergence(Strategy):
    ...
    def init(self):
        ...
        # VWAP StdDev
        self.vwap_std = self.I(talib.STDDEV, self.vwap, timeperiod=20, name="VWAP_STD")

        # BB Width
        self.bb_width = self.I(lambda u, l: u - l, self.upper, self.lower, name="BB_WIDTH")
        self.bb_width_ma = self.I(talib.SMA, self.bb_width, timeperiod=20, name="BB_WIDTH_MA")
    ...
    def next(self):
        ...
        if not self.position:
            # Long entry logic
            if self.bb_width[-1] > self.bb_width_ma[-1] and price < vwap_val - 2 * std_val:
            ...
            # Short entry logic
            elif self.bb_width[-1] > self.bb_width_ma[-1] and price > vwap_val + 2 * std_val:
                sl = vwap_val + 3 * std_val
                risk_amount = self.risk_per_trade * self.equity
            ...
            if self.position.is_long:
                if self.bb_width[-1] < self.bb_width_ma[-1] or price >= vwap_val - std_val:
                    self.position.close()
