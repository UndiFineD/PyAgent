# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_21_2025\backtests_final\VolumetricBreakout_BTFinal.py
import pandas as pd
import talib
    ...
    def next(self):
        if not self.position:
            if (
                ...
                and self.data.Volume[-1] >= 1.5 * self.volume_sma[-1]
            ):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌙 MOON ENTRY! Price: {entry_price:.2f}, Size: {position_size}, Cosmic SL: {sl_price:.2f}"
                    )
            ...
            for condition, reason in exit_conditions:
                if condition:
                    self.position.close()
                    print(f"🌙🌠 {reason} at {current_close:.2f}. Returning to lunar base!")
