...
class MomentumSqueeze(Strategy):
    ...
    def init(self):
        ...
        self.signal = self.I(
            lambda: fisher_result[f"FISHERTs_{self.signal_period}_{self.signal_period}"],
            name="Signal",
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                (self.fisher[-2] < self.signal[-2] and self.fisher[-1] > self.signal[-1])
        ...
        # Exit Logic 🚀
            ...
            if (self.signal[-2] < self.fisher[-2] and self.signal[-1] > self.fisher[-1]) or self.bb_width[
                -1
            ] > self.bb_width[-2] * 1.5:
                self.position.close()
