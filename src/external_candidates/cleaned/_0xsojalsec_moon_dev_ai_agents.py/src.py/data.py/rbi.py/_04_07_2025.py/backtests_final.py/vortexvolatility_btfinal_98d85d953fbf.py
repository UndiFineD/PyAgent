# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_07_2025\backtests_final\VortexVolatility_BTFinal.py
...
class VortexVolatility(Strategy):
    ...
    def init(self):
        ...
        vi_plus, vi_minus = talib.VORTEX(
            self.data.High,
            self.data.Low,
            self.data.Close,
            timeperiod=self.vortex_period,
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.data.Close[-1] < self.vix_sma[-1] and crossover(self.vi_plus, self.vi_minus):
                self.position.close()
                print(f"🌑 MOON DEV STRATEGY EXIT | Price: {self.data.Close[-1]:.2f}")

        # 🚀 Entry conditions
        elif not self.position:
            if self.data.Close[-1] > self.vix_sma[-1] and crossover(self.vi_minus, self.vi_plus):
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀 MOON DEV SHORT ENTRY | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_loss_price:.2f}"
                        )


# 🌖 Run backtest
bt = Backtest(data, VortexVolatility, cash=1_000_000, commission=0.002, exclusive_orders=True)
stats = bt.run()
