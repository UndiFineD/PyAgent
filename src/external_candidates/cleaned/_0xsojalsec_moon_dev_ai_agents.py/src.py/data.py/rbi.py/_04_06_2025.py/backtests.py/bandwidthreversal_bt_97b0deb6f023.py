# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_06_2025\backtests\BandwidthReversal_BT.py
import numpy as np
import pandas as pd
import talib
...
class BandwidthReversal(Strategy):
    ...
    def init(self):
        ...
        # 10-day SMA of BBW
        self.bbw_sma10 = self.I(talib.SMA, self.bbw, 10)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # Check RSI crosses above 30
            rsi_cross = crossover(self.rsi[-1:], [30])[0]
            ...
            if bbw_at_low and rsi_cross:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙🚀 MOONSHOT ENTRY! 🚀 | Size: {position_size} | Price: {price:.2f} | RSI: {self.rsi[-1]:.2f}"
                    )
...
# Launch backtest
bt = Backtest(data, BandwidthReversal, cash=1_000_000, commission=0.002)
stats = bt.run()
