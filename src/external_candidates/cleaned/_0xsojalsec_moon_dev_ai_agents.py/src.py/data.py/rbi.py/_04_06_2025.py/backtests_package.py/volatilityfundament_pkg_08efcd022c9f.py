# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_06_2025\backtests_package\VolatilityFundament_PKG.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        if self.position:
            ...
            if current_funding >= 0 or self.data.Close[-1] < current_vsma:
                self.position.close()
                print(
                    f"🌙✨ Closing Moon Position: Funding {current_funding:.4f} | Price {self.data.Close[-1]:.2f} vs VSMA {current_vsma:.2f}"
                )
        else:
            # 🌑 Entry Logic
...
bt = Backtest(data, VolatilityFundament, cash=1_000_000, commission=0.002)
stats = bt.run()
