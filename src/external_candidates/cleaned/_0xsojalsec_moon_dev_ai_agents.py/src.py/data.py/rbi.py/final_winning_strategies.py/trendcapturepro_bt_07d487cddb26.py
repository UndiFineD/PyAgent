...
class TrendCapturePro(Strategy):
    ...
    def assess_trend_quality(self):
        ...
        uptrend_aligned = ema_fast_val > ema_slow_val > ema_filter_val and current_close > ema_fast_val
        downtrend_aligned = ema_fast_val < ema_slow_val < ema_filter_val and current_close < ema_fast_val

        # ADX trend strength analysis
        current_adx = self.adx[-1] if not np.isnan(self.adx[-1]) else 0
        adx_strong = current_adx > self.adx_threshold
        adx_increasing = len(self.adx) > 2 and self.adx[-1] > self.adx[-2] > self.adx[-3]
    ...
    def analyze_entry_timing(self):
        ...
        macd_bullish = self.macd_line[-1] > self.macd_signal[-1] and self.macd_line[-1] > self.macd_line[-2]
        macd_bearish = self.macd_line[-1] < self.macd_signal[-1] and self.macd_line[-1] < self.macd_line[-2]
    ...
    def get_trend_confirmations(self):
        ...
        if current_volume > self.volume_sma[-1] * self.volume_threshold or current_volume > self.volume_ema[-1] * 1.6:
        ...
        if self.di_plus[-1] > self.di_minus[-1] * 1.2 or self.di_minus[-1] > self.di_plus[-1] * 1.2:
    ...
    def detect_trend_reversal(self):
        ...
        ema_reversal = False
        if self.position.is_long:
            ema_reversal = self.ema_fast[-1] < self.ema_slow[-1] and self.data.Close[-1] < self.ema_fast[-1]
        else:
            ema_reversal = self.ema_fast[-1] > self.ema_slow[-1] and self.data.Close[-1] > self.ema_fast[-1]
        ...
        if len(self.macd_line) > 3:
            if self.position.is_long:
                macd_reversal = self.macd_line[-1] < self.macd_signal[-1] and self.macd_line[-1] < self.macd_line[-2]
            else:
                macd_reversal = self.macd_line[-1] > self.macd_signal[-1] and self.macd_line[-1] > self.macd_line[-2]
