...
class SelectiveMomentumSwing(Strategy):
    ...
    def analyze_trend_structure(self):
        ...
        # Trend direction
        trend_up = ema_fast_val > ema_slow_val > ema_filter_val and current_close > ema_filter_val
        trend_down = ema_fast_val < ema_slow_val < ema_filter_val and current_close < ema_filter_val
        ...
        macd_trend_up = self.macd_line[-1] > self.macd_signal[-1] and self.macd_line[-1] > self.macd_threshold
        macd_trend_down = self.macd_line[-1] < self.macd_signal[-1] and self.macd_line[-1] < -self.macd_threshold

        strong_trend = ((trend_up and macd_trend_up) or (trend_down and macd_trend_down)) and strength >= 1.0
    ...
    def analyze_pullback_opportunity(self):
        ...
        pullback_quality_long = pullback_from_high >= self.pullback_threshold and current_close > pullback_ema_val

        # Pullback analysis for short setups
        pullback_from_low = ((current_close - recent_low) / recent_low) * 100
        pullback_quality_short = pullback_from_low >= self.pullback_threshold and current_close < pullback_ema_val
    ...
    def get_swing_confirmations(self):
        ...
        if current_volume > self.volume_sma[-1] * self.volume_multiplier or current_volume > self.volume_ema[-1] * 1.3:
        ...
        if len(self.macd_histogram) > 2 and self.macd_histogram[-1] > self.macd_histogram[-2]:
    ...
    def manage_swing_position(self):
        ...
        if pnl_pct > 1.0:  # Only check if profitable
            ...
            if self.position.is_long and self.ema_fast[-1] < self.ema_slow[-1] and current_price < self.ema_fast[-1]:
                trend_broken = True

            elif self.position.is_short and self.ema_fast[-1] > self.ema_slow[-1] and current_price > self.ema_fast[-1]:
                trend_broken = True
