# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\DivergenceVolatilityEnhanced_BT.py
...
class DivergenceVolatilityEnhanced(Strategy):
    ...
    def check_divergence_signals(self):
        ...
        if (
            ...
            and self.is_significant_swing(current_low)
        ):
    ...
    def check_bullish_divergence(self):
        ...
        for i in range(len(self.swing_history) - 1):
            ...
            if current_index - prev_idx >= self.min_separation:
                ...
                if (
                    current_low < prev_low * 0.998  # Significant lower low
                    and current_macd > prev_macd * 1.05
                ):  # Significant higher MACD
                    print(f"🌙 QUALITY DIVERGENCE DETECTED:")
                    print(
                        f"   Price: {prev_low:.2f} -> {current_low:.2f} ({((current_low / prev_low - 1) * 100):.2f}%)"
                    )
                    print(
                        f"   MACD: {prev_macd:.4f} -> {current_macd:.4f} ({((current_macd / prev_macd - 1) * 100):.2f}%)"
                    )
    ...
    def get_entry_confirmations(self):
        ...
        if (
            current_volume > self.volume_sma[-1] * self.volume_spike_threshold
            or current_volume > self.volume_ema[-1] * 1.5
        ):
        ...
        if (
            self.macd_hist[-1] > self.macd_hist[-2]  # MACD improving
            and self.price_ema_fast[-1] > self.price_ema_fast[-2]
        ...
        if (
            current_close > self.bb_lower[-1]  # Above BB lower
            and current_close < self.bb_middle[-1]
        ):  # Still room to move up
            confirmations["price_action"] = True
            confirmations["total"] += 1

        # 5. Market structure confirmation
        if (
            self.price_ema_fast[-1] > self.price_ema_slow[-1]
            or current_close
            > np.nanmean(  # Uptrend structure
                self.data.Close[-5:]
            )
    ...
    def manage_position(self):
        ...
        if (
            profit_pct > 0.5  # Only if at least 0.5% profit
            and len(self.macd_hist) > 3
...
bt = Backtest(data, DivergenceVolatilityEnhanced, cash=1000000, commission=0.002)
stats = bt.run()
