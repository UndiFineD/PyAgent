# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_17_2025\backtests_package\ContrarianMomentum_PKG.py
...
class ContrarianMomentum(Strategy):
    ...
    def init(self):
        ...
        # Bollinger Bands
        upper, _, _ = talib.BBANDS(self.data.Close, timeperiod=self.bb_period, nbdevup=2, nbdevdn=2, matype=0)
        ...
        # 3-Day Return System
        self.daily_ret = self.I(
            lambda close: (close / close.shift(96) - 1),  # 96 periods = 1 day in 15m data
            self.data.Close,
            name="Daily Return",
        )

        self.three_day_avg = self.I(
            lambda x: x.rolling(3).mean(),  # 3-day average
            self.daily_ret,
            name="3-Day Avg",
        )
    ...
    def next(self):
        ...
        if not self.position:
            if self.data.Close[-1] > self.upper_bb[-1] and self.rsi[-1] < self.rsi_oversold:
...
# 🚀 Launch Backtest
bt = Backtest(data, ContrarianMomentum, cash=1_000_000, commission=0.002)
stats = bt.run()
