...
class VelocityDivergence(Strategy):
    ...
    def next(self):
        ...
        # Exit Logic 💫
            ...
            if (self.position.is_long and current_z < 1) or (self.position.is_short and current_z > -1):
                self.position.close()
                print(f"🌙 Position Closed | Z-Score Stabilized at {current_z:.1f} σ")

            if (self.position.is_long and current_z > 3) or (self.position.is_short and current_z < -3):
                self.position.close()
