# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests\VolClusterFlip_BT.py
...
class VolClusterFlip(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            if price >= entry_price + 1.5 * self.atr[-1] or price <= entry_price - 1 * self.atr[-1]:
                self.position.close()
