# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_05_2025\backtests\ChoppyBreakout_BT.py
...
class ChoppyBreakout(Strategy):
    ...
    def init(self):
        # Choppiness Index (CI) 🌊
        self.ci = self.I(ta.chop, self.data.High, self.data.Low, self.data.Close, length=14)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.ci[-2] < 35 and self.data.Close[-2] > self.donchian_upper[-2]:
                ...
                if risk_per_share > 0:
                    ...
                    print(f"🚀 MOON DEV ALERT: Long Entry @ {entry_price:.2f} | Size: {position_size} Units 🌕")
        ...
        # Exit Logic 🌑
            ...
            if self.data.Low[-1] < trail_stop:
                self.position.close()
                print(f"🌑 MOON DEV EXIT: Trailing Stop @ {trail_stop:.2f} | Profit: {self.position.pl_pct:.2f}% 🌙")


# Run Backtest 📊
bt = Backtest(data, ChoppyBreakout, cash=1_000_000, commission=0.002)
stats = bt.run()
