# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_18_2025\backtests_package\VolatilitySpikeReversal_PKG.py
import pandas as pd
import talib
...
class VolatilitySpikeReversal(Strategy):
    ...
    def next(self):
        ...
        if (
            not self.position
            and self.data.Low[-1] <= self.lower_band[-1]
            and self.adx[-1] < 20
