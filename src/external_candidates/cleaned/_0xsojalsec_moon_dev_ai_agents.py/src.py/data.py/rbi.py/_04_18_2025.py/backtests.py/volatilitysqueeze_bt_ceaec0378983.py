# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_18_2025\backtests\VolatilitySqueeze_BT.py
...
class VolatilitySqueeze(Strategy):
    ...
    def init(self):
        ...
        self.bb_upper = self.I(lambda c: talib.BBANDS(c, 20, 2, 2, 0)[0], self.data.Close, name="bb_upper")
        self.bb_lower = self.I(lambda c: talib.BBANDS(c, 20, 2, 2, 0)[2], self.data.Close, name="bb_lower")
        self.bb_width = self.I(lambda u, l: u - l, self.bb_upper, self.bb_lower, name="bb_width")
        self.bb_width_sma = self.I(talib.SMA, self.bb_width, 20, name="bb_width_sma_20")
    ...
    def next(self):
        ...
        historical_percentile = (
            sum(1 for x in self.bb_width_history if x <= current_width) / len(self.bb_width_history)
        ) * 100
        ...
        if not self.position and historical_percentile <= 10:
            ...
            if risk_per_unit > 0:
                ...
                if position_size > 0:
                    ...
                    print(f"🌙✨ MOON DEV ALERT: Squeeze Detected! Size: {position_size:,} @ {entry_price:.2f} 🚀")
        ...
        if self.position:
            ...
            if current_width > width_sma:
                self.position.close()
                print(
                    f"🌙💥 COSMIC EXIT: Width {current_width:.2f} > SMA {width_sma:.2f} "
                    f"Profit: {self.position.pl_usd:.2f} 🌠"
                )


# Launch backtest rocket 🚀🌙
bt = Backtest(data, VolatilitySqueeze, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
