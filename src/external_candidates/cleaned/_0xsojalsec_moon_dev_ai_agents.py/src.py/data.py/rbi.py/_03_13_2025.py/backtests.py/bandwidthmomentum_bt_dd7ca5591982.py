# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\BandwidthMomentum_BT.py
...
class BandwidthMomentum(Strategy):
    ...
    def init(self):
        ...
        self.middle_band = self.I(talib.SMA, self.data.Close, 50, name="MIDDLE")
        ...
        self.rsi = self.I(talib.RSI, self.data.Close, 20, name="RSI")
        self.rsi_upper_ratio = self.I(lambda r, u: r / u, self.rsi, self.upper_band, name="RSI_UPPER_RATIO")

        # 🌙 Swing calculations for volatility contraction detection
        self.bb_width_min = self.I(talib.MIN, self.bb_width, 20, name="BB_WIDTH_MIN")
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.bb_width[-1] > self.bb_width[-2] and self.bb_width[-2] == self.bb_width_min[-2]:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙✨ MOON DEV ALERT: Long Entry {price:.2f} | "
        ...
        # 🚪 Exit Logic: Momentum threshold
            ...
            if self.rsi_upper_ratio[-1] > 1.5:
                self.position.close()
                print(f"🌙💸 MOON DEV EXIT: Closing {price:.2f} | Profit: {self.position.pl:.2f} ✨")
