# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_final\BearishBreakout_BTFinal.py
...
class BearishBreakout(Strategy):
    ...
    def next(self):
        ...
        if self.engulfing[-1] == -100:  # Fixed: Corrected engulfing pattern value
            ...
            print(
                f"🌑 Bearish Engulfing Pattern Detected! | Low: {self.current_engulfing_low:.2f} | High: {self.current_engulfing_high:.2f}"
            )

        # 🌓 Entry Logic
        if self.current_engulfing_low and not self.position:
            if self.data.Low[-1] < self.current_engulfing_low and self.data.Volume[-1] > self.volume_sma[-1]:
                # 🌔 Risk Management Calculations
                risk_amount = self.equity * self.risk_percent
                entry_price = self.data.Close[-1]
                stop_loss = self.current_engulfing_high
                risk_per_unit = abs(stop_loss - entry_price)  # Fixed: Added abs() for proper risk calculation

                if risk_per_unit > 0:
                    position_size = risk_amount / risk_per_unit
                    position_size = int(round(position_size))  # Fixed: Ensured whole number position size

                    if position_size > 0:
                        take_profit = entry_price - 2 * risk_per_unit
                        self.sell(size=position_size, sl=stop_loss, tp=take_profit)
                        print(
                            f"🚀🌒 SHORT ENTRY | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_loss:.2f} | TP: {take_profit:.2f}"
                        )
