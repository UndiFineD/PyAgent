# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\CorrelationReversal_BT.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
...
class CorrelationReversal(Strategy):
    ...
    def init(self):
        ...
        self.correlation = self.I(talib.CORREL, self.vix, self.data.Close, 20, name="CORREL")

        # 🌕 Lunar high detection
        self.daily_high = self.I(talib.MAX, self.data.High, 20, name="Daily High")
        ...
        self.next_day_low = self.I(next_day_extreme, self.data.Low, 20, talib.MIN, name="Next Day Low")

        self.next_day_high = self.I(next_day_extreme, self.data.High, 20, talib.MAX, name="Next Day High")
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if abs(self.correlation[-1]) <= 0.05 and self.data.High[-1] >= self.daily_high[-1]:
...
bt = Backtest(data, CorrelationReversal, cash=1_000_000, commission=0.002, margin=1.0)
