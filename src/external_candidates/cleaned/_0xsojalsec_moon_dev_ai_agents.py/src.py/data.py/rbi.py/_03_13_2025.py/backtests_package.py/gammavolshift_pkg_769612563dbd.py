# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\GammaVolShift_PKG.py
...
class GammaVolShift(Strategy):
    ...
    def next(self):
        ...
        if not self.position and position_size > 0:
            if current_gamma > self.gamma_entry and current_vix < self.vix_entry_threshold:
                self.buy(
                    size=position_size,
