    ...
    def next(self):
        ...
        if is_uptrend and volume_confirmation:
            if not self.position:
                ...
                self.buy(
                    size=position_size,
                    sl=stop_loss,
                    tp=current_close + (current_close - stop_loss) * self.risk_reward_ratio,
                )
                print(
        ...
        # Entry logic for short trades
            ...
            if not self.position:
                ...
                self.sell(
                    size=position_size,
                    sl=stop_loss,
                    tp=current_close - (stop_loss - current_close) * self.risk_reward_ratio,
                )
                print(
        ...
        if self.position:
            ...
            # Bearish crossover detection (replacing crossover)
                ...
                self.data.Close[-2] < self.green_ema[-2] and self.data.Close[-1] > self.green_ema[-1]
            ):
                self.position.close()
