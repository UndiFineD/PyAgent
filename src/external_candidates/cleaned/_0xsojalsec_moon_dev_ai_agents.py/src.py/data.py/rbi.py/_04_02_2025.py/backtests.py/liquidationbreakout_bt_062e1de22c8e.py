...
class LiquidationBreakout(Strategy):
    ...
    def next(self):
        ...
        moon_debug = (
            f"🌙 Close: {current_close:.2f} | LH: {self.liquidation_high[-1]:.2f} | LL: {self.liquidation_low[-1]:.2f}"
        )
        print(moon_debug)

        if not self.position:
            # 🚀 Long Entry Logic
            if current_close > self.liquidation_high[-1] and self.std_dev[-1] < self.volatility_sma[-1]:
            ...
            # 📉 Short Entry Logic
            elif current_close < self.liquidation_low[-1] and self.std_dev[-1] < self.volatility_sma[-1]:
                risk_amount = self.equity * 0.01  # 1% risk
                atr_value = self.atr[-1]
