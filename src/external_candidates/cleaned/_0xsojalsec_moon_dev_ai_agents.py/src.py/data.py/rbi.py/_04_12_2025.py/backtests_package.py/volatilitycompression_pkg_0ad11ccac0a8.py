# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_12_2025\backtests_package\VolatilityCompression_PKG.py
...
class VolatilityCompression(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                self.atr[-2] < self.atr_sma[-2]  # Previous ATR < SMA(ATR)
                and self.data.Close[-2] > self.trend_filter[-2]
            ):  # Price above SMA200
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀 Position Size: {position_size} units ({position_size / self.equity * 100:.2f}% of equity)"
                    )
