# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_31_2025\backtests_final\VolatilityRebound_BTFinal.py
...
class VolatilityRebound(Strategy):
    ...
    def init(self):
        ...
        self.lower_bb = self.I(
            lambda close: talib.BBANDS(close, self.timeperiod_bb, self.bb_dev, self.bb_dev, matype=0)[2],
            self.data.Close,
            name="LowerBB",
        )
        ...
        upper_bb = self.I(
            lambda close: talib.BBANDS(close, self.timeperiod_bb, self.bb_dev, self.bb_dev, matype=0)[0],
            self.data.Close,
        )
        self.bb_width = self.I(lambda u, l: u - l, upper_bb, self.lower_bb, name="BBWidth")
        self.bb_width_avg = self.I(lambda x: talib.SMA(x, self.exit_period), self.bb_width, name="BBWidthAvg")
    ...
    def next(self):
        if not self.position:
            ...
            if (
                self.data.fundingrate[-1] < 0
                and self.data.Volume[-1] > self.vol_pct[-1]
                and self.data.Low[-1] <= self.lower_bb[-1]
            ):
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀 MOON DEV LONG ENTRY 🚀 | Price: {entry_price:.2f} | Size: {position_size} | SL: {stop_loss:.2f}"
                        )
        else:
...
# 🚀 Launch Backtest
bt = Backtest(data, VolatilityRebound, cash=1_000_000, commission=0.002)
stats = bt.run()
