# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DeltaSentiment_AI5_working.py
...
class DeltaSentiment(Strategy):
    ...
    def init(self):
        # Calculate indicators with proper integration
        self.sma20 = self.I(sma, self.data.Close, 20)
        self.sma50 = self.I(sma, self.data.Close, 50)
        self.spread = self.I(sub, self.sma20, self.sma50)
    ...
    def next(self):
        ...
        if np.isnan(spread_now) or np.isnan(spread_prev) or np.isnan(volume_ma):
            return
        ...
        if not self.position:
            ...
            if self.position.is_long and (spread_widening or not high_liquidity):
                print(
                    f"🌕 Closing LONG position: Sentiment changed - spread widening or low liquidity at {current_close:.2f}"
                )
                self.position.close()
            elif self.position.is_short and (spread_narrowing or not high_liquidity):
                print(
                    f"🌑 Closing SHORT position: Sentiment changed - spread narrowing or low liquidity at {current_close:.2f}"
                )
                self.position.close()
