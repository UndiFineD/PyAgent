# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\research\GPT-5\ConfluencePattern_BT.py
import pandas as pd
import talib
...
class ConfluencePattern(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and self.rsi[-1] < 30
            and (self.macd[-2] < self.macd_signal[-2] and self.macd[-1] > self.macd_signal[-1])
            and volume > self.data.Volume[-2]
        ):
        ...
        # 🌙 Exit logic with manual crossover detection
            ...
            (self.macd_signal[-2] < self.macd[-2] and self.macd_signal[-1] > self.macd[-1]) or self.rsi[-1] > 70
        ):
            self.position.close()
