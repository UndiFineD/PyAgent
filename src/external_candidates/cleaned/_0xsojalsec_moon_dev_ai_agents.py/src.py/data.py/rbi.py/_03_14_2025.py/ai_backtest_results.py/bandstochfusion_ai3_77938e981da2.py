...
class BandStochFusion(Strategy):
    ...
    def next(self):
        ...
        if self.position.is_long:
            if self.data.High[-1] >= self.middle[-1] or (self.stoch_k[-2] > 80 and self.stoch_k[-1] <= 80):
                print(f"🌗 Closing LONG at {price:.2f} (Middle Band/Stoch Exit)")
                self.position.close()

        # Short Exit Conditions
        elif self.position.is_short:
            if self.data.Low[-1] <= self.middle[-1] or (self.stoch_k[-2] < 20 and self.stoch_k[-1] >= 20):
        ...
        if not self.position:
            ...
            long_cond = (
                ...
                and (self.stoch_k[-2] < self.stoch_d[-2] and self.stoch_k[-1] > self.stoch_d[-1])
            )
