# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\research\GPT-5\BandStrengthMomentum_BT.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if (self.data.close[-2] < self.lower_band[-2] and self.data.close[-1] > self.lower_band[-1]) and rs_value > 50:
            if self.data.open[-1] < self.data.close[-1]:  # Bullish candle
                ...
                print(
                    f"🌙 ✨ Moon Dev Debug: LONG ENTRY | Price: {price:.2f} | Size: {position_size} units | SL: {stop_loss:.2f} | TP: {take_profit:.2f}"
                )
        ...
        # Short Entry
            ...
            if self.data.open[-1] > self.data.close[-1]:  # Bearish candle
                ...
                print(
                    f"🌙 ✨ Moon Dev Debug: SHORT ENTRY | Price: {price:.2f} | Size: {position_size} units | SL: {stop_loss:.2f} | TP: {take_profit:.2f}"
                )
...
# Run backtest
try:
    bt = Backtest(data, BandStrengthMomentum, cash=1_000_000, commission=0.002)
    stats = bt.run()
