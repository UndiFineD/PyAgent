# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\beat_buy_and_hold\FinalWinner_AI5.py
...
class FinalWinner(Strategy):
    ...
    def next(self):
        ...
        # Add positions during major dips (when price is still low)
            ...
            self.position
            and self.trade_count < 6
            and self.bars_passed < 15000  # First half of data
            and current_close < self.first_price * 1.8
        ):  # Less than 80% up from start
            if self.equity > current_close * 5:  # If we have enough cash
                ...
                if add_size > 0:
                    ...
                    self.trade_count += 1
                    print(
                        f"🚀🌙 ADD #{self.trade_count}! Bar {self.bars_passed}, Entry: {current_close:.2f}, Size: {add_size}"
                    )

        # Take profits near the end when we've made good gains
        elif (
            self.position
            and self.bars_passed > 28000  # Last 3000 bars
            and current_close > self.first_price * 2.2  # More than doubled
            and self.trade_count < 9
        ):
