# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DivergentMomentum_AI8.py
#!/usr/bin/env python3
...
class DivergentMomentum(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            if (
                self.rsi[-1] < 50  # Much more lenient RSI
                and self.data.Close[-1] > self.ma50[-1]
            ):  # Removed strict divergence requirement
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"   🔍 RSI: {self.rsi[-1]:.2f}, Price Low: {price_low_current:.2f} -> {price_low_prev:.2f}"
                        )
        ...
        # Exit conditions with cosmic wisdom 🌌 - More aggressive exits
            ...
            if (
                ...
                or current_price < entry_price * 0.98  # 2% loss
                or self.rsi[-1] > 70
            ):  # RSI overbought
