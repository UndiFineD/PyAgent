# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MoonDev_AI8_Supreme.py
#!/usr/bin/env python3
...
class MoonDevSupreme(Strategy):
    ...
    def next(self):
        ...
        if not self.position and self.trade_count < 8:  # Allow 8 strategic trades
            ...
            pullback_to_ema = current_price > self.ema12[-1] * 0.995 and self.data.Close[-2] < self.ema12[-2]
                ...
                if big_profit or trend_reversal or take_profit_on_weakness or stop_big_loss:
                    ...
                    exit_reason = (
                        ...
                        if big_profit
                            ...
                            "TREND REVERSAL"
                            if trend_reversal
                            else ("PROFIT ON WEAKNESS" if take_profit_on_weakness else "STOP LOSS")
                        )
                    )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, MoonDevSupreme, cash=1_000_000, commission=0.002)
    stats = bt.run()
