# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\BandDivergence_AI2.py
import numpy as np
import pandas as pd
    ...
    def next(self):
        ...
        if not self.position and atr_val > 0:
            ...
            if len(self.data) >= 22:  # Ensure we have enough data for swing calculations
                ...
                if bb_mid_current > bb_mid_prev and macd_val > 0 and uptrend:
                    # 🌙 Risk Management
                    ...
                    if risk_per_share > 0:
                        ...
                        if position_size > 0:
                            ...
                            print(
                                f"🚀🌙 MOON DEV LONG: {self.data.index[-1]} | Size: {position_size} | SL: {stop_loss:.2f}"
                            )
        ...
        if not self.position and atr_val > 0:
            ...
            bb_width = (
                ...
                if not np.isnan(self.bb_upper[-1]) and not np.isnan(self.bb_lower[-1])
                else 0
            )
            bb_width_prev = (
                self.bb_upper[-2] - self.bb_lower[-2]
                if len(self.data) > 1 and not np.isnan(self.bb_upper[-2]) and not np.isnan(self.bb_lower[-2])
                else bb_width
            )
            ...
            if len(self.data) >= 22:  # Ensure we have enough data for swing calculations
                ...
                if bb_width < bb_width_prev and macd_val < 0 and downtrend:
                    # 🌙 Risk Management
                    stop_loss = price + 1.5 * atr_val
                    risk_amount = self.equity * self.risk_pct
                    ...
                    if risk_per_share > 0:
                        ...
                        if position_size > 0:
                            ...
                            print(
                                f"🌑🌙 MOON DEV SHORT: {self.data.index[-1]} | Size: {position_size} | SL: {stop_loss:.2f}"
                            )

        # Exit Logic
        if self.position:
            # Take profit conditions
            if self.position.is_long and not np.isnan(self.bb_upper[-1]) and price >= self.bb_upper[-1]:
                self.position.close()
                print(f"💰🌙 MOON DEV PROFIT - Long Exit: {price:.2f}")
            elif self.position.is_short and not np.isnan(self.bb_lower[-1]) and price <= self.bb_lower[-1]:
                self.position.close()
