# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MoonTrendRider_AI2.py
import numpy as np
import pandas as pd
...
class MoonTrendRider(Strategy):
    ...
    def next(self):
        ...
        if not self.position and atr_val > 0:
            ...
            if (
                sma_short > sma_medium > sma_long and price > sma_short and sma_short > sma_medium * 1.002
            ):  # Minimum 0.2% separation
                ...
                if price > stop_loss:
                    ...
                    if position_size > 0:
                        ...
                        print(f"🌙🚀 TREND RIDE! Long {position_size} @ {price:.2f} | Trailing SL: {stop_loss:.2f}")

        # 📉 Exit: Trend breakdown
        elif self.position and self.position.is_long:
            # Exit if price breaks below long MA or trend reverses
            if price < sma_long or sma_short < sma_medium or sma_medium < sma_long:
                self.position.close()
                profit_pct = (
                    ((price - self.position.entry_price) / self.position.entry_price) * 100
