# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DivergentVolume_AI8.py
#!/usr/bin/env python3
...
class DivergentVolume(Strategy):
    ...
    def next(self):
        ...
        if (
            not self.position
            and self.trade_count < 50  # Allow many more trades
            and current_rsi < 60  # More lenient RSI condition
            and current_volume < volume_sma
        ):  # Simple volume condition
        ...
        if self.position:
            ...
            if (
                current_close > entry_price * 1.03  # 3% profit
                or current_close < entry_price * 0.98  # 2% loss
                or current_rsi > 70
            ):  # RSI overbought
                self.position.close()
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, DivergentVolume, cash=1_000_000, commission=0.002)
    stats = bt.run()
