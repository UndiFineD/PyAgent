# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\VolatilityMomentum_BTFinal.py
...
class VolatilityMomentum(Strategy):
    ...
    def next(self):
        ...
        if self.position and (rsi_value < 50 and prev_rsi >= 50):
            self.position.close()
            print(f"\n🌑 MOON DEV EXIT SIGNAL 🌑 | Price: {current_close:.2f} | RSI: {rsi_value:.2f}")
