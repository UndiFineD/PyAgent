# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\EarlyBuyLateHold_AI5.py
...
class EarlyBuyLateHold(Strategy):
    ...
    def next(self):
        ...
        # Add more positions during specific dips
            ...
            self.position and self.trade_count < 6 and current_close < self.data.Close[0] * 1.5
        ):  # If still less than 50% up from start
        ...
        # Take some profit near the end (last 10% of data)
            ...
            self.position
            and current_bar > self.total_bars * 0.9
            and current_close > self.data.Close[0] * 2.0  # Doubled from start
            and self.trade_count < 8
        ):
