# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MoonDev_AI8_FINAL_VICTORY.py
#!/usr/bin/env python3
    ...
    def next(self):
        ...
        # Hold for the long run - minimal exits 🌊
            ...
            if self.position:
                ...
                major_reversal = self.ema20[-1] < self.ema50[-1] and self.ema20[-2] >= self.ema50[-2]  # Fresh cross

                # 2. MACD very bearish
                macd_very_bearish = self.macd[-1] < -50 and self.macd[-1] < self.macd_signal[-1]
                ...
                if major_reversal or macd_very_bearish or massive_profit:
                    ...
                    exit_reason = (
                        "MAJOR REVERSAL"
                        if major_reversal
                        else ("MACD VERY BEARISH" if macd_very_bearish else "MASSIVE PROFIT")
                    )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, MoonDevFinalVictory, cash=1_000_000, commission=0.002)
    stats = bt.run()
