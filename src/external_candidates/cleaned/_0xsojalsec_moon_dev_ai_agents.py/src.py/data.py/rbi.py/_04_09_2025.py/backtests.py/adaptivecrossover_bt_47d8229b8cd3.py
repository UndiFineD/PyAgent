# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_09_2025\backtests\AdaptiveCrossover_BT.py
            ...
            if crossover(self.ema50, self.ema200) and self.adx[-1] < 20:
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙✨ GOLDEN CROSS ALERT! Entry: {price:.2f} | Size: {position_size} | Risk: {risk_per_unit:.2f} 🚀"
                        )

        # Exit conditions 🌙
        else:
            if crossunder(self.ema50, self.ema200) or self.adx[-1] > 40:
                self.position.close()
