# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_package\BandSurgeADX_PKG.py
import pandas as pd
import talib
...
class BandSurgeADX(Strategy):
    def init(self):
        ...
        self.middle_band = self.I(talib.BBANDS, self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)[1]
            ...
            if (
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌕🚀 MOON SHOT LONG! Entry: {current_close:.2f} | Size: {position_size} | SL: {stop_loss:.2f} | TP: {tp_price:.2f}"
                        )
            ...
            # Short entry logic
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        tp_price = current_close - 2 * risk_per_share
                        print(
                            f"🌑🌌 DARK MOON SHORT! Entry: {current_close:.2f} | Size: {position_size} | SL: {stop_loss:.2f} | TP: {tp_price:.2f}"
                        )
                        self.sell(size=position_size, sl=stop_loss, tp=tp_price)


bt = Backtest(data, BandSurgeADX, cash=1_000_000, commission=0.002, exclusive_orders=True)
stats = bt.run()
print(stats)
