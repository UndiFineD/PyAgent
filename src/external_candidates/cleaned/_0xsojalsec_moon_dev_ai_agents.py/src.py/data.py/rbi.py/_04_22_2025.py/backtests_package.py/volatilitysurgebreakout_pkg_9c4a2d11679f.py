# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_package\VolatilitySurgeBreakout_PKG.py
import numpy as np
import pandas as pd
...
class VolatilitySurgeBreakout(Strategy):
    ...
    def next(self):
        if self.position:
            ...
            if self.bbwp[-2] < 90 and self.bbwp[-1] >= 90 and self.data.Volume[-1] >= 1.5 * self.volume_ma[-1]:
