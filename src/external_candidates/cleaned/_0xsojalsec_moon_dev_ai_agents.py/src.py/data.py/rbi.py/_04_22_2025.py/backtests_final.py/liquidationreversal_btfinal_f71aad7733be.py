# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_final\LiquidationReversal_BTFinal.py
import pandas as pd
import talib
...
class LiquidationReversal(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            # Long entry logic
            if self.data.Low[-1] < swing_low and price > swing_low and bb_width < bb_percentile:
            ...
            # Short entry logic
            elif self.data.High[-1] > swing_high and price < swing_high and bb_width < bb_percentile:
                risk_amount = self.equity * 0.01
