# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_final\VolatilitySurge_BTFinal.py
import pandas as pd
import talib
...
class VolatilitySurge(Strategy):
    ...
    def next(self):
        if not self.position:
            if len(self.data.Close) < 20 or len(self.upper_band) < 20:
                return

            close_cross = self.data.Close[-2] <= self.upper_band[-2] and self.data.Close[-1] > self.upper_band[-1]
            ...
            if close_cross and self.adx[-1] > 25:
                ...
                if vol_ratio >= 1.2:
                    ...
                    if position_size > 0:
                        print(
                            f"🌙🚀 MOON ENTRY: Size {position_size} @ {self.data.Close[-1]:.2f} | SL: {sl:.2f} | TP: {tp:.2f} ✨"
                        )
                        self.buy(size=position_size, sl=sl, tp=tp)


bt = Backtest(data, VolatilitySurge, cash=1_000_000, commission=0.002, exclusive_orders=True)
stats = bt.run()
