...
class BollingerBandsStrategy(Strategy):
    ...
    def next(self):
        ...
        if np.isnan(self.upper[-1]) or np.isnan(self.lower[-1]) or np.isnan(self.rsi[-1]) or np.isnan(self.atr[-1]):
            return
        ...
        if price < lower and rsi < 40 and price > prev_price and prev_price > prev_price2:  # Confirmed upward momentum
            entry = price
        ...
        # For short positions - price above upper band + RSI overbought + momentum confirmation
            ...
            price > upper and rsi > 60 and price < prev_price and prev_price < prev_price2
