# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\ValidatedBreakthrough_BT.py
#!/usr/bin/env python3
"""
...
class ValidatedBreakthrough(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙 [Moon Dev] Indicators initialized: SMA period = {}, Lookback = {}".format(
                self.sma_period, self.lookback
            )
        )
        ...
        if not self.position:
            ...
            if current_price > self.sma[-1]:
                ...
                if current_price <= self.recent_low[-1] * (1 + self.zone_buffer):
                    ...
                    print(
                        "     • Recent Low: {:.2f} | Recent High: {:.2f}".format(
                            self.recent_low[-1], self.recent_high[-1]
                        )
                    )
                    print(
                        "     • Entry: {:.2f} | Stop Loss: {:.2f} | TP: {:.2f}".format(
                    ...
                    if risk <= 0:
                        ...
                        print(
                            "🚀 [Moon Dev] RR Ratio {:.2f} below minimum {:.2f}. Skipping LONG trade!".format(
            ...
            # -------------------
                ...
                print(
                    "✨ [Moon Dev] Downtrend detected – Close {:.2f} < SMA {:.2f}".format(current_price, self.sma[-1])
                )
                ...
                if current_price >= self.recent_high[-1] * (1 - self.zone_buffer):
                    ...
                    print(
                        "     • Recent High: {:.2f} | Recent Low: {:.2f}".format(
                            self.recent_high[-1], self.recent_low[-1]
                        )
                    )
                    print(
                        "     • Entry: {:.2f} | Stop Loss: {:.2f} | TP: {:.2f}".format(
                    ...
                    if risk <= 0:
                        ...
                        print(
                            "🚀 [Moon Dev] RR Ratio {:.2f} below minimum {:.2f}. Skipping SHORT trade!".format(
                                reward / risk, self.min_rr
                            )
                        )
                    else:
