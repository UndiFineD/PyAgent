# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_18_2025\backtests_final\VolumeSurgeReversal_BTFinal.py
...
class VolumeSurgeReversal(Strategy):
    ...
    def init(self):
        ...
        self.atr5 = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, timeperiod=5)
        self.atr20_avg = self.I(talib.SMA, self.atr5, timeperiod=20)
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Metrics | Close: {current_close:.2f} | Vol: {current_volume:.2f} vs {self.volume_avg[-1]:.2f} | ATR5: {atr5:.2f} vs {self.atr20_avg[-1]:.2f}"
        )
        ...
        if not self.position:
            if (current_volume > self.volume_avg[-1]) and (atr5 < self.atr20_avg[-1]):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌕 LONG ENTRY | Size: {position_size} | Entry: {current_close:.2f} | Stop: {self.trailing_stop:.2f}"
                    )
