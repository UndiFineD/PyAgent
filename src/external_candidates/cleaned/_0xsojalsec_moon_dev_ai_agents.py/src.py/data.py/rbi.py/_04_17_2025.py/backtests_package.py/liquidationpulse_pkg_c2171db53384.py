# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_17_2025\backtests_package\LiquidationPulse_PKG.py
import numpy as np
import pandas as pd
import talib
...
class LiquidationPulse(Strategy):
    ...
    def init(self):
        ...
        self.consecutive_oi_increases = self.I(
            talib.SUM,
    ...
    def next(self):
        ...
        if self.position:
            ...
            if price >= self.position.entry_price + 1.5 * self.initial_risk:
                self.position.close()
                print(f"🌕 MOON DEV TRAILING STOP ACTIVATED: Reached 1.5R at {price}!")
                return
            ...
            if all(entry_conditions):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌙 MOON DEV LIQUIDATION PULSE TRIGGERED! "
