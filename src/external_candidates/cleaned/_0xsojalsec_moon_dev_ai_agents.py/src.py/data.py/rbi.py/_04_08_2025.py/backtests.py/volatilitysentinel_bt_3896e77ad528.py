...
class VolatilitySentinel(Strategy):
    ...
    def init(self):
        ...
        self.median_putcall = self.I(lambda s: s.rolling(self.median_period).median(), put_call, name="MED21_PCR")
        self.vix_spread = self.I(
            lambda: self.data.df["vix_near"] - self.data.df["vix_next"],
            name="VIX_SPREAD",
        )
        self.realized_vol = self.I(talib.STDDEV, self.data.Close, self.vol_lookback, name="REALIZED_VOL")
...
# Backtest Execution 🔁
bt = Backtest(data, VolatilitySentinel, cash=1_000_000, commission=0.002, exclusive_orders=True)
