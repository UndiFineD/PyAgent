# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests\DynamicTrendSync_BT.py
import pandas as pd
import talib
...
class DynamicTrendSync(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            # Entry Logic 🌟
            if crossover(self.ema50, self.ema200) and self.adx[-1] > 25 and self.adx[-1] > self.adx[-2]:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌙 MOON DEV LONG ENTRY ✨ | Size: {position_size} | Est.Entry: {est_entry:.2f} | SL: {sl_price:.2f} | RSI: {self.rsi[-1]:.1f}"
                    )

        else:
            # Exit Logic 🛑
            exit_rsi = crossover(70, self.rsi)
            exit_ema = crossover(self.ema200, self.ema50)

            if exit_rsi or exit_ema:
                self.position.close()
                reason = "RSI <70 🔻" if exit_rsi else "EMA Cross 🔀"
                print(
                    f"🌙✨ PROFIT CAPTURED | {reason} | Exit Price: {price:.2f} | P/L: {self.position.pl_pct:.1f}% 🌟"
                )
