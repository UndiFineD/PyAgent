# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests_package\VolumetricBandwidth_PKG.py
import numpy as np
import pandas as pd
import talib
...
class VolumetricBandwidth(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                (self.vw_signal[-2] < self.vw_macd[-2] and self.vw_signal[-1] > self.vw_macd[-1])
                or bandwidth > 3
                or time_in_trade >= 5
            ):
                self.position.close()
...
# Launch backtest 🚀
bt = Backtest(data, VolumetricBandwidth, cash=1_000_000, commission=0.002)
stats = bt.run()
