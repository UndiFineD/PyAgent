# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests_final\VolatilitySurge_BTFinal.py
import pandas as pd
import talib
    ...
    def next(self):
        if not self.position:
            ...
            if self.position.is_long and (
                self.lower_band[-2] > self.data.Close[-2] and self.lower_band[-1] < self.data.Close[-1]
            ):
                self.position.close()
                print("🚀 LONG TP HIT!")
