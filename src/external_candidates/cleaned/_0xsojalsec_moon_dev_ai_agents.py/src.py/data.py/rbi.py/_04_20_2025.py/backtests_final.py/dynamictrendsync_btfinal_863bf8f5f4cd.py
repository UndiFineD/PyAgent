# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests_final\DynamicTrendSync_BTFinal.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        if not self.position:
            ...
            if ema_cross and adx_strong:
                ...
                if risk_per_share <= 0:
                    print("🌙🚨 MOON DEV ALERT: Invalid stop loss price!")
                    return
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ MOON DEV ENTRY ✨🚀\nLong {position_size} units @ {entry_price:.2f}\nSL: {sl_price:.2f} | Risk: {risk_per_share:.2f}"
                    )
                else:
                    print("🌙🚨 MOON DEV ALERT: Position size too small!")
...
bt = Backtest(data, DynamicTrendSync, cash=1_000_000, commission=0.002, exclusive_orders=True)
stats = bt.run()
print(stats)
