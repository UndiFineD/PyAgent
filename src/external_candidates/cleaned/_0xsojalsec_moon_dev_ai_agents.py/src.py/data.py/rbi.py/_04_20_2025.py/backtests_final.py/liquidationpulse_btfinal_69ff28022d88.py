# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests_final\LiquidationPulse_BTFinal.py
import pandas as pd
    ...
    def next(self):
        if self.position:
            if self.volatility[-1] < self.median_vol[-1]:
                self.position.close()
                print(f"🌙 Volatility contraction! Closing Moon Position at {self.data.Close[-1]:.2f} ✨")
        else:
            ...
            if (
                ...
                if z_score > 2.0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀 MOON SHORT! Entry: {self.data.Close[-1]:.2f}, Stop: {stop_loss:.2f}, Size: {position_size} 🌙"
                        )
