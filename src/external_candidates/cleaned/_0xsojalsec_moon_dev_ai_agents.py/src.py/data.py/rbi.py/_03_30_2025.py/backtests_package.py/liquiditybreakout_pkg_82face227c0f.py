# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_30_2025\backtests_package\LiquidityBreakout_PKG.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
...
class LiquidityBreakout(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            # Long Entry Logic 🌙🚀
            if current_close > self.swing_high[-1] and current_adx > 30 and current_volume > self.volume_sma[-1]:
                sl = self.swing_low[-1]
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙🚀 BULLISH BREAKOUT! 🚀🌙 Entry: {current_close:.2f}, Size: {position_size}, SL: {sl:.2f}, TP: {tp:.2f}"
                        )

            # Short Entry Logic 🌙🔻
            elif current_close < self.swing_low[-1] and current_adx > 30 and current_volume > self.volume_sma[-1]:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙🔻 BEARISH BREAKDOWN! 🔻🌙 Entry: {current_close:.2f}, Size: {position_size}, SL: {sl:.2f}, TP: {tp:.2f}"
                        )
