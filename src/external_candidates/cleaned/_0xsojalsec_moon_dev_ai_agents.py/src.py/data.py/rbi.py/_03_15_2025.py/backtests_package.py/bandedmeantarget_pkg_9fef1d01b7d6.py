# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_package\BandedMeanTarget_PKG.py
...
class BandedMeanTarget(Strategy):
    ...
    def init(self):
        ...
        bb_upper, _, bb_lower = talib.BBANDS(self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2)
    ...
    def next(self):
        ...
        if not self.position:
            entry_condition = current_close <= 0.75 * hist_high and current_close > bb_lower
            ...
            if entry_condition:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(f"🌙✨ MOON DEV ENTRY! Buying {position_size} @ {entry_price:.2f} 🚀")
        ...
        # 🌕 EXIT STRATEGY: DYNAMIC TARGETS & STOP MANAGEMENT
            ...
            if self.data.Low[-1] <= new_stop:
                self.position.close()
                print(f"🌑🛡️ STOP LOSS HIT @ {new_stop:.2f}! PROTECTING MOON BASE! 🛡️")
