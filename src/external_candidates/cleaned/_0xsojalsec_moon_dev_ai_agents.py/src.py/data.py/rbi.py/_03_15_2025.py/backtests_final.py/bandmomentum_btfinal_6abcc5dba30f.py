# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_final\BandMomentum_BTFinal.py
...
class BandMomentum(Strategy):
    ...
    def init(self):
        ...
        self.bb_upper = self.I(
            lambda close: talib.BBANDS(close, timeperiod=50, nbdevup=2, nbdevdn=2, matype=0)[0],
            self.data.Close,
        )
        self.bb_middle = self.I(
            lambda close: talib.BBANDS(close, timeperiod=50, nbdevup=2, nbdevdn=2, matype=0)[1],
            self.data.Close,
        )
        self.bb_lower = self.I(
            lambda close: talib.BBANDS(close, timeperiod=50, nbdevup=2, nbdevdn=2, matype=0)[2],
            self.data.Close,
        )
