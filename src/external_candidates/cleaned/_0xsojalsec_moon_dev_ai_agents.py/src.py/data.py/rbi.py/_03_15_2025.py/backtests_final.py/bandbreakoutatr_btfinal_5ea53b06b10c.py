# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_final\BandBreakoutATR_BTFinal.py
import numpy as np
import pandas as pd
import talib
...
class BandBreakoutATR(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.data.BB_UPPER[-2] < self.data.BB_MIDDLE[-2] and self.data.BB_UPPER[-1] > self.data.BB_MIDDLE[-1]:
            ...
            if self.data.BB_MIDDLE[-2] > self.data.BB_LOWER[-2] and self.data.BB_MIDDLE[-1] < self.data.BB_LOWER[-1]:
                self.position.close()
