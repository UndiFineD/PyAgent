# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests\T00038_GapQuintile_BT.py
...
class GapQuintile(Strategy):
    ...
    def next(self):
        ...
        if no_signal and self.position:
            ...
            print(
                f"🌙 Moon Dev: Exiting {direction} position - signal reversal or no momentum 📊 at {self.data.index[-1]} ✨"
            )
        ...
        if long_signal and not self.position.is_long:
            ...
            print(
                f"🌙 Moon Dev: Long entry on gap up momentum! 🚀 Norm Gap: {norm_gap:.3f} > {upper_th:.3f}, Volume Confirm: ✅, Size: {size}, Entry: {entry_price:.2f}, SL: {sl:.2f}, TP: {tp:.2f} at {self.data.index[-1]} 🌙"
            )
        ...
        if short_signal and not self.position.is_short:
            ...
            print(
                f"🌙 Moon Dev: Short entry on gap down momentum! 📉 Norm Gap: {norm_gap:.3f} < {lower_th:.3f}, Volume Confirm: ✅, Size: {size}, Entry: {entry_price:.2f}, SL: {sl:.2f}, TP: {tp:.2f} at {self.data.index[-1]} 🌙"
            )
