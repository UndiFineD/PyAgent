# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class FractalCascade(Strategy):
    ...
    def next(self):
        if (
            np.isnan(self.adx[-1]) or self.adx[-1] < 20
        ):  # 🌙 Moon Dev: Lowered ADX threshold to 20 for more trading opportunities in moderate trends
            return

        risk_per_trade = (
            0.02  # 🌙 Moon Dev: Increased risk per trade to 2% to amplify returns while keeping risk managed
        )
        ...
        if not self.position:
            ...
            if (
                ...
                > 1.2
                * self.volume_ma[-1]  # 🌙 Moon Dev: Tightened volume filter to 1.2x MA for higher quality breakouts
