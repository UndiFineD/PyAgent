# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, EMA50 {ema50:.2f}, EMA200 {ema200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f}, MACD {macd:.4f} ✨"
        )
        ...
        ):  # 🌙 Moon Dev Optimization: Reduced minimum lookback to 10 for more flexible Fib calculations without sacrificing accuracy
            return
        ...
        volume_confirm = (
            volume > 1.2 * avg_vol
        )  # Reduced to 1.2x for increased entry opportunities while maintaining confirmation
        momentum_rsi = (
            rsi > 45 and rsi < 75
        )  # Broadened RSI range to capture more pullback momentum without extreme overbought filters
        ...
        if (
            ...
            risk_amount = (
                capital * 0.015
            )  # 🌙 Moon Dev Optimization: Increased risk per trade to 1.5% for higher capital utilization towards target returns
        ...
        if self.position:
            ...
            if not self.partial_taken and unrealized_pnl >= 2 * risk:
                ...
                print(
                    f"🌙 Moon Dev: Partial exit at 2:1 RR, sold {half_size} units! Remaining position open for more upside ✨"
                )
            ...
            if high > self.last_peak_price and rsi < self.last_peak_rsi and rsi > 65:
                print(
                    f"🌙 Moon Dev: Bearish RSI Divergence: Higher high {high:.2f} > {self.last_peak_price:.2f}, RSI {rsi:.2f} < {self.last_peak_rsi:.2f}, EXITING! 🚀"
                )
                self.position.close()
                return
