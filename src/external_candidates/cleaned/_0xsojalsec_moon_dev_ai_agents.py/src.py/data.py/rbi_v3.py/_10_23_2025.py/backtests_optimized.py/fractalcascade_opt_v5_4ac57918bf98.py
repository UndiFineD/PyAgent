# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v5.py
import numpy as np
import pandas as pd
import talib
...
def compute_up_fractals(high):
    ...
    for i in range(2, n - 2):
        h2 = high[i]
        if h2 > high[i - 1] and h2 > high[i - 2] and h2 > high[i + 1] and h2 > high[i + 2]:
            up[i] = h2
    return up
...
def compute_down_fractals(low):
    ...
    for i in range(2, n - 2):
        l2 = low[i]
        if l2 < low[i - 1] and l2 < low[i - 2] and l2 < low[i + 1] and l2 < low[i + 2]:
            down[i] = l2
    return down
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                > 1.5 * self.volume_ma[-1]  # 🌙 Moon Dev: Tightened volume filter for better setups
                and self.data.Close[-1] > self.ffill_up[-1]
                and self.data.Close[-2] <= self.ffill_up[-2]
                and self.data.Close[-1] > self.sma200[-1]  # 🌙 Moon Dev: Trend filter - only long in uptrend
                and self.rsi[-1] > 50
            ):  # 🌙 Moon Dev: RSI momentum confirmation
                ...
                if sl < entry_price:
                    ...
                    self.current_sl = sl
                    print(
                        f"🌙 Moon Dev: Long entry at {entry_price:.2f}, Size: {pos_size:.4f} (fraction), SL: {sl:.2f}, TP: {tp:.2f} 🚀"
                    )
            ...
            # Short entry - tightened with new filters
                ...
                and self.ao[-1] < 0
                and self.ao[-1] < self.ao[-2]
                and self.data.Volume[-1] > 1.5 * self.volume_ma[-1]  # 🌙 Moon Dev: Tightened volume filter
                and self.data.Close[-1] < self.ffill_down[-1]
                and self.data.Close[-2] >= self.ffill_down[-2]
                and self.data.Close[-1] < self.sma200[-1]  # 🌙 Moon Dev: Trend filter - only short in downtrend
                and self.rsi[-1] < 50
            ):  # 🌙 Moon Dev: RSI momentum confirmation
                ...
                if sl > entry_price:
                    ...
                    self.current_sl = sl
                    print(
                        f"🌙 Moon Dev: Short entry at {entry_price:.2f}, Size: {pos_size:.4f} (fraction), SL: {sl:.2f}, TP: {tp:.2f} 📉"
                    )
                ...
                if not np.isnan(self.down_fractal[-1]):
                    ...
                    if new_sl > current_sl and new_sl < entry_price:
                        ...
                        if new_risk_dist > 0:
                            ...
                            print(
                                f"🌙 Moon Dev: Trailing SL long to {new_sl:.2f}, Size: {new_pos_size:.4f}, TP: {new_tp:.2f} ✨"
                            )
                ...
                if not np.isnan(self.up_fractal[-1]):
                    ...
                    if new_sl < current_sl and new_sl > entry_price:
                        ...
                        if new_risk_dist > 0:
                            ...
                            self.current_sl = new_sl
                            print(
                                f"🌙 Moon Dev: Trailing SL short to {new_sl:.2f}, Size: {new_pos_size:.4f}, TP: {new_tp:.2f} ✨"
                            )
