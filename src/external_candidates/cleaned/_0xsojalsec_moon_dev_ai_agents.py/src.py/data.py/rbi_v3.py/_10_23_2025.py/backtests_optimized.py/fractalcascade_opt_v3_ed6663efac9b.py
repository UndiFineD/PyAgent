# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v3.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.ao[-2] > self.ao[-3]
                and self.data.Volume[-1]
                > 1.5 * self.volume_ma[-1]  # 🌙 Moon Dev: Increased volume filter to 1.5x for stronger conviction
                and self.data.Close[-1] > self.ffill_up[-1]
                and self.data.Close[-2] <= self.ffill_up[-2]
                and self.data.Close[-1] > self.ema200[-1]
            ):  # 🌙 Moon Dev: Long only in uptrend above EMA200
                ...
                if sl < entry_price:
                    ...
                    tp = (
                        entry_price + 3 * atr_buffer
                    )  # 🌙 Moon Dev: Added fixed TP at 3x ATR for improved risk-reward (target ~3:1 RR)
            ...
            # Short entry - tightened with 3-bar AO momentum confirmation and EMA200 trend filter
                ...
                and self.data.Close[-1] < self.ema200[-1]
            ):  # 🌙 Moon Dev: Short only in downtrend below EMA200
