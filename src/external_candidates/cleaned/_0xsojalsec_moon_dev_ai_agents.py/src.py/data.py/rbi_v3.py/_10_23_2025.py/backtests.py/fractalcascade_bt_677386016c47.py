# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests\FractalCascade_BT.py
import numpy as np
import pandas as pd
import talib
...
def compute_up_fractals(high):
    ...
    for i in range(2, n - 2):
        h2 = high.iloc[i]
        if h2 > high.iloc[i - 1] and h2 > high.iloc[i - 2] and h2 > high.iloc[i + 1] and h2 > high.iloc[i + 2]:
            up.iloc[i] = h2
    return up
...
def compute_down_fractals(low):
    ...
    for i in range(2, n - 2):
        l2 = low.iloc[i]
        if l2 < low.iloc[i - 1] and l2 < low.iloc[i - 2] and l2 < low.iloc[i + 1] and l2 < low.iloc[i + 2]:
            down.iloc[i] = l2
    return down
