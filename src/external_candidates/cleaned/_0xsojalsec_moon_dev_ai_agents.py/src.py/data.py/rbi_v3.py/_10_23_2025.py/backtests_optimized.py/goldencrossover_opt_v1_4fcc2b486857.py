# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, SMA50 {sma50:.2f}, SMA200 {sma200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f} ✨"
        )
        ...
        ):  # 🌙 Moon Dev Optimization: Require longer lookback minimum for better Fib accuracy
            return
        ...
        touch_fib = low <= fib618 + (
            0.005 * close
        )  # 🌙 Moon Dev Optimization: Tighter tolerance (0.5%) for precise Fib touch
        volume_confirm = (
            volume > 1.5 * avg_vol
        )  # 🌙 Moon Dev Optimization: Increased to 1.5x for higher quality volume spikes
        momentum_rsi = (
            rsi > 50 and rsi < 70
        )  # 🌙 Moon Dev Optimization: RSI filter for pullback momentum without overbought entries
        trend_strength = adx > 25
        uptrend = (
            close > sma200 and sma50 > sma200
        )  # 🌙 Moon Dev Optimization: Added SMA50 > SMA200 for golden cross confirmation
