# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests\T00037_ClusterLiquidity_BT.py
import numpy as np
import pandas as pd
import talib
...
class ClusterLiquidity(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # Long entry
            if prev_close <= ch and curr_close > ch + buffer and vol_surge and atr_expand and bb_expand and r > 50:
                ...
                print(
                    f"🌙🚀 Long entry at {curr_close:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, size {size}, cluster {clu:.2f}-{ch:.2f}"
                )
            # Short entry
            elif prev_close >= clu and curr_close < clu - buffer and vol_surge and atr_expand and bb_expand and r < 50:
                ...
                self.entry_bar = len(self.data) - 1
                print(
                    f"🌙🔻 Short entry at {curr_close:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, size {size}, cluster {clu:.2f}-{ch:.2f}"
                )
