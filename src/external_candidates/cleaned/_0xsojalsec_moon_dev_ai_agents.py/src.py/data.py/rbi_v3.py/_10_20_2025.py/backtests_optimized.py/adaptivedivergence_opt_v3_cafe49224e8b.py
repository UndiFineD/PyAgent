# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\AdaptiveDivergence_OPT_v3.py
...
class AdaptiveDivergence(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Changed to 50-period EMA for faster trend detection in crypto volatility
        self.adx = self.I(
            talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14
        )  # 🌙 Added ADX for trend strength filter (>25 only)
        self.vol_sma = self.I(
    ...
    def next(self):
        ...
        # Entry logic if no position
            ...
            if (
                ...
                )  # 🌙 Kept volatility adjustment for risk control
