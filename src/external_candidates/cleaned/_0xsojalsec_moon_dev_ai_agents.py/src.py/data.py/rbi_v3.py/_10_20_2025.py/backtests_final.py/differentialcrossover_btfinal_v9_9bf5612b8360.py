# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_final\DifferentialCrossover_BTFinal_v9.py
import pandas as pd
import talib
        ...
        if not self.position:
            if (
                ...
                if risk <= 0 or tp <= entry_price:
                    print(
                        f"⚠️ Moon Dev: Invalid trade parameters - risk: {risk:.4f}, tp: {tp:.4f}, entry: {entry_price:.4f}, skipping 🌙"
                    )
                    return
