# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\AdaptiveDivergence_BEST_-2.015156pct.py
...
class AdaptiveDivergence(Strategy):
    ...
    risk_per_trade = (
        0.02  # 🌙 Increased to 2% risk per trade for more aggressive capital deployment to chase higher returns
    )
    ...
    profit_trail = (
        2.0  # 🌙 Raised activation to 2x ATR to allow more room before trailing, avoiding early exits in trends
    )
    ...
    def init(self):
        ...
        )  # 🌙 Adjusted to 21-period EMA, a Fibonacci number for smoother trend filter in crypto
        self.adx = self.I(
            talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=14
        )  # 🌙 Kept ADX but will use lower threshold in entries for more opportunities
        self.vol_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)  # 🌙 Kept volume SMA

        self.confirmed_swing_lows = deque(
            maxlen=3
        )  # 🌙 Reduced to 3 for focus on most recent swings, fresher divergences
    ...
    def next(self):
        ...
        # Entry logic if no position
            ...
            if (
                ...
                vol_mult = (
                    0.3 if self.atr[-1] > 1.5 * self.atr_avg[-1] else 1.0
                )  # 🌙 Tightened vol_mult to 0.3 in high vol for stricter sizing, better risk control
