# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentReversion_OPT_v17.py
import numpy as np
import pandas as pd
import talib
...
class DivergentReversion(Strategy):
    adx_threshold = (
        20  # 🌙 Tightened ADX threshold to 20 for stronger ranging market filter, reducing false signals in weak trends
    )
    ...
    atr_multiplier_sl = 2
    atr_multiplier_tp = (
        5  # 🌙 Extended TP to 5x ATR for improved 1:2.5 RR, capturing larger reversions in volatile BTC 15m setups
    )
        ...
        if (not self.position) and (len(self.rsi) > 30):
            ...
            if (self.adx[-1] < self.adx_threshold) and (self.data.Volume[-1] > self.volume_sma[-1]):
                ...
                if (self.rsi[-1] < self.rsi_sma[-1]) and (self.stoch_k[-1] > self.stoch_sma[-1]):
                    ...
                    position_size = max(
                        0.01, min(1000, position_size)
                    )  # 🌙 Added bounds to position size for realism and to prevent extreme allocations
