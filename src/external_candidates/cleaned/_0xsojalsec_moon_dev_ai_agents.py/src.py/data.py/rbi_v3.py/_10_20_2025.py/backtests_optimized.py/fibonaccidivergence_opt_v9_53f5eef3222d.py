# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\FibonacciDivergence_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciDivergence(Strategy):
    ...
    key_fib_levels = [
        ...
        0.618,
        0.786,
    ]  # 🌙 Moon Dev: Focused on key retracement levels only, removed extremes to avoid false signals at 0/1
    fib_tolerance = (
        0.02  # 🌙 Moon Dev: Tightened from 0.05 to 0.02 for more precise Fib level hits, improving entry quality
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Switched to EMA for better trend sensitivity
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.vol_period
        )  # 🌙 Moon Dev: Added volume SMA for momentum filter
        self.low_pivots = []
        self.high_pivots = []
                                    ...
                                    if high_start < high_end and high_end <= current_bar + 1:
                                        ...
                                        if fib_range > 0:
                                            ...
                                            if is_at_fib and self.data.Volume[current_bar] > self.vol_sma[current_bar]:
                                                print(
                                                    f"🌙 Moon Dev: Bullish divergence at Fib with volume confirm! Retrace PCT={retrace_pct:.3f}, Entry signal! 🚀"
                                                )
                                                ...
                                                if atr_val > 0:
                                                    ...
                                                    )  # 🌙 Moon Dev: Improved RR from 1:2 to 1:3 by using 3*ATR for higher profit potential
                                                    stop_distance = entry_price - sl_price
                                                    position_size = (
                                                        risk_amount / stop_distance
                                                    )  # 🌙 Moon Dev: Use float for fractional sizing, more precise for crypto
                                ...
                                if curr_high > prev_high and curr_rsi < prev_rsi:
                                    ...
                                    if low_start < low_end and low_end <= current_bar + 1:
                                        ...
                                        if fib_range > 0:
                                            ...
                                            if is_at_fib and self.data.Volume[current_bar] > self.vol_sma[current_bar]:
                                                print(
                                                    f"🌙 Moon Dev: Bearish divergence at Fib with volume confirm! Retrace PCT={retrace_pct:.3f}, Short signal! 🚀"
                                                )
                                                entry_price = self.data.Close[current_bar]
                                                atr_val = self.atr[current_bar]
                                                if atr_val > 0:
