# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v25.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def next(self):
        ...
        )  # 🌙 Moon Dev Optimization: Loosened RSI threshold to <50 for more oversold opportunities and distance to <=5 for slightly fresher but more signals
        if bull_div:
            self.div_low = low2
        ...
        )  # 🌙 Moon Dev Optimization: Relaxed %K threshold to <30 to capture more convergence setups in BTC's volatile oversold regions
        if stoch_converge:
        ...
        if (
            ...
            and self.adx[-1] > 25
        ):  # 🌙 Moon Dev Optimization: Added EMA50 trend filter and tightened volume to >1.2x SMA, raised ADX to >25 for higher-quality trending entries
            ...
            if size > 0:
                ...
                )  # 🌙 Moon Dev Optimization: Retained TP for risk management but with higher RR; manual trailing for flexibility
        ...
        if self.position:
            ...
            if (
                self.risk_per_unit is not None
                and self.current_sl < self.entry_price
                and current_price >= self.entry_price + self.risk_per_unit
            ):
