# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\FibonacciDivergence_OPT_v7.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciDivergence(Strategy):
    ...
    atr_period = 14
    adx_period = (
        14  # 🌙 Moon Dev: Added ADX indicator for trend strength filter to avoid entries in weak/choppy markets
    )
    ...
    strength = (
        7  # 🌙 Moon Dev: Increased pivot strength from 5 to 7 for more reliable pivot detection, reducing false signals
    )
    ...
    key_fib_levels = [
        ...
        0.786,
        1.0,
    ]  # 🌙 Moon Dev: Focused on core retracement levels, removed extensions for more precise pullback entries
    fib_tolerance = (
        0.02  # 🌙 Moon Dev: Tightened Fibonacci tolerance from 0.05 to 0.02 for higher-quality setups at key levels
    )
    min_fib_range = (
        0.005  # 🌙 Moon Dev: Added minimum Fibonacci range (0.5% of price) to filter out insignificant swings
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Initialized ADX for market regime filtering
        self.vol_sma = self.I(
                                    ...
                                    if high_start < high_end and high_end <= current_bar + 1:
                                        ...
                                        if fib_range > self.data.Close[current_bar] * self.min_fib_range:
                                            ...
                                            if (
                                                ...
                                                > self.vol_multiplier * self.vol_sma[current_bar]
                                            ):
                                                print(
                                                ...
                                                if atr_val > 0:
                                                    ...
                                                    if position_size > 0 and sl_price < entry_price < tp_price:
                                                        ...
                                                        print(
                                                            f"🌙 Entering LONG at {entry_price:.2f}, size={position_size}, SL={sl_price:.2f}, TP={tp_price:.2f} ✨"
                                                        )
                            # Limit pivots
                                            ...
                                            if (
                                                ...
                                                if atr_val > 0:
                                                    ...
                                                    if position_size > 0 and tp_price < entry_price < sl_price:
                                                        ...
                                                        print(
                                                            f"🌙 Entering SHORT at {entry_price:.2f}, size={position_size}, SL={sl_price:.2f}, TP={tp_price:.2f} ✨"
                                                        )
                            # Limit pivots
