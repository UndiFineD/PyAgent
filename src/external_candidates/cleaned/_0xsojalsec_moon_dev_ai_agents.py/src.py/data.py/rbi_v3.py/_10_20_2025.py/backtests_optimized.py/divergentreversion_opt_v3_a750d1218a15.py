# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentReversion_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class DivergentReversion(Strategy):
    ...
    def next(self):
        ...
        if close < lower and bullish_div and volume > self.vol_mult * v_sma and rsi_long_ok and trend_long_ok:
            entry_price = close
            ...
            if risk_dist > 0:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙 LONG ENTRY SIGNAL: Price {entry_price:.2f}, Size {size}, SL {sl_price:.2f}, TP {self.tp_price:.2f}, Risk {self.risk_pct * 100}% 🚀"
                    )
                    return
        ...
        if close > upper and bearish_div and volume > self.vol_mult * v_sma and rsi_short_ok and trend_short_ok:
            entry_price = close
            ...
            if risk_dist > 0:
                ...
                if size > 0:
                    ...
                    self.tp_price = entry_price - self.rr_ratio * risk_dist  # 🌙 Set TP at 2:1 RR
                    print(
                        f"🌙 SHORT ENTRY SIGNAL: Price {entry_price:.2f}, Size {size}, SL {sl_price:.2f}, TP {self.tp_price:.2f}, Risk {self.risk_pct * 100}% 🚀"
                    )
                    return
