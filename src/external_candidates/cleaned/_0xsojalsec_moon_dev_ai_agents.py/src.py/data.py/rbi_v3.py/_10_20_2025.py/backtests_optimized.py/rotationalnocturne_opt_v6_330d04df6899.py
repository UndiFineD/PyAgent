...
class RotationalNocturne(Strategy):
    ...
    def next(self):
        ...
        macd_ok = (
            (self.macd_line[-1] > self.macd_signal[-1] and self.macd_hist[-1] > self.macd_threshold)
            if not (np.isnan(self.macd_line[-1]) or np.isnan(self.macd_signal[-1]) or np.isnan(self.macd_hist[-1]))
            else True
        )
        adx_ok = (
            self.adx[-1] > self.adx_threshold and self.plus_di[-1] > self.minus_di[-1]
            if not (np.isnan(self.adx[-1]) or np.isnan(self.plus_di[-1]) or np.isnan(self.minus_di[-1]))
            else False
        )
