# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v24.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev Optimization: Added SMA50 for intermediate trend confirmation to ensure entries align with short-term uptrends
    ...
    def next(self):
        ...
        )  # 🌙 Moon Dev Optimization: Tightened RSI oversold to <40 for stronger signals and extended distance to <=5 bars for timely yet reliable divergences
        if bull_div:
            self.div_low = low2
        ...
        if (
            ...
            and self.data.Volume[-1] > (self.vol_mult * self.vol_sma[-1])
            and self.adx[-1] > 25
        ):  # 🌙 Moon Dev Optimization: Added SMA50 uptrend filter and raised ADX to >25 for stronger trend confirmation; volume now requires 1.2x average for higher quality setups
            ...
            if size > 0:
                ...
                )  # 🌙 Moon Dev Optimization: Retained fixed TP for disciplined profit taking, complemented by trailing SL for trend extension
