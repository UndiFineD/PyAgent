...
class DivergentConvergence(Strategy):
    ...
    def init(self):
        ...
        self.slowk = self.I(
            lambda h, l, c: talib.STOCH(h, l, c, fastk_period=14, slowk_period=3, slowd_period=3)[0],
            self.data.High,
            self.data.Low,
            self.data.Close,
        )
        self.slowd = self.I(
            lambda h, l, c: talib.STOCH(h, l, c, fastk_period=14, slowk_period=3, slowd_period=3)[1],
            self.data.High,
            self.data.Low,
            self.data.Close,
        )
    ...
    def next(self):
        ...
        stoch_converge = (
            (k_now < 30) and (k_now > k_prev) and (gap_now < gap_prev) and (k_now < d_now)
        ...
        if (
            ...
            if size > 0:
                ...
                self.highest_since_entry = (
                    entry_price  # 🌙 Moon Dev Optimization: Initialize highest for trailing from entry price
                )
                self.div_bar = None
                self.div_low = None
                print(
