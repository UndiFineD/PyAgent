# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_package\VoltaicDivergence_PKG.py
import pandas as pd
import talib
...
class VoltaicDivergence(Strategy):
    ...
    def next(self):
        ...
        if self.prev_price_high is not None and price_high > self.prev_price_high and current_atr < self.prev_atr_high:
            if current_close > upper_bb and current_rsi > 50 and not self.position.is_long:
        ...
        # Short setup: bearish divergence (lower price low but higher volatility)
            ...
            if current_close < lower_bb and current_rsi < 50 and not self.position.is_short:
                # Calculate position size
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, VoltaicDivergence, commission=0.002, margin=1.0, trade_on_close=True)
    stats = bt.run()
    print(stats)
