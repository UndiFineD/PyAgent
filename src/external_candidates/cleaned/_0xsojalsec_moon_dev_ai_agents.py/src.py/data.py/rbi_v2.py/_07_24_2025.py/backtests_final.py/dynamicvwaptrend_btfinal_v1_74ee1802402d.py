# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\DynamicVWAPTrend_BTFinal_v1.py
import pandas as pd
import talib
...
class DynamicVWAPTrend(Strategy):
    ...
    def init(self):
        ...
        self.vwap = self.I(
            talib.SMA,
            (self.data.Close * self.data.Volume).cumsum() / self.data.Volume.cumsum(),
        )
...
bt = Backtest(data, DynamicVWAPTrend, commission=0.002, margin=1.0, cash=10000000)
stats = bt.run()
print(stats)
