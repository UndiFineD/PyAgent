        ...
        # Exit conditions
            ...
            if not self.position.is_short and self.data.Close[-1] >= entry_price + 2 * atr_value:
                if self.position.size > 1:  # Partial exit
                    exit_size = int(round(self.position.size * 0.5))  # Fixed to use fraction
                    print(f"🌕 Partial profit taken at 2x ATR ({entry_price + 2 * atr_value:.2f}) | Size: {exit_size}")
                    self.position.close(exit_size)
                else:
                    print(f"🌕 Full position closed at 2x ATR ({entry_price + 2 * atr_value:.2f})")
                    self.position.close()

            # Second profit target (3x ATR)
            elif not self.position.is_short and self.data.Close[-1] >= entry_price + 3 * atr_value:
...
bt = Backtest(data, VolatilityDivergence, commission=0.002, exclusive_orders=True, cash=100000)
stats = bt.run()
print(stats)
