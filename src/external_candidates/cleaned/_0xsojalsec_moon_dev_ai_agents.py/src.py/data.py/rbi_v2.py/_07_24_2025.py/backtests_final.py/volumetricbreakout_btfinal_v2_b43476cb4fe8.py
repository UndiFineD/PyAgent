# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\VolumetricBreakout_BTFinal_v2.py
import pandas as pd
import talib
...
class VolumetricBreakout(Strategy):
    ...
    def next(self):
        ...
        position_size = min(risk_amount / stop_distance, 1.0)  # Cap at 100% of equity

        # Long entry conditions
        if not self.position and volume_spike and long_breakout and ma_alignment_long and rsi_filter and volatility_ok:
        ...
        # Trailing stop for open positions
            ...
            if self.position.is_long:
                ...
                self.orders[0].sl = max(new_sl, self.orders[0].sl or 0)
            else:
                new_sl = self.data.Close[-1] + (self.atr[-1] * 1.5)
                self.orders[0].sl = min(new_sl, self.orders[0].sl or float("inf"))
