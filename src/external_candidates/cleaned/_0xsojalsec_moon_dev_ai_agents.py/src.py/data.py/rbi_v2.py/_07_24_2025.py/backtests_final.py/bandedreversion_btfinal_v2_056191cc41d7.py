# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\BandedReversion_BTFinal_v2.py
import pandas as pd
import talib
...
class BandedReversion(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            if current_price <= (self.trades[-1].entry_price - (self.atr_multiplier * atr)):
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, BandedReversion, commission=0.002, exclusive_orders=True, cash=100000)
    stats = bt.run()
    print(stats)
