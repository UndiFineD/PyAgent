# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\10_20_2025\backtests_package\ShadowSpread_PKG.py
import numpy as np
import pandas as pd
import talib
...
class ShadowSpread(Strategy):
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Backtest Update: Bar {len(self.data)}, Close: {self.data.Close[-1]:.2f}, Volume: {self.data.Volume[-1]:.2f}, RSI: {self.rsi[-1]:.2f} ✨"
            )
