# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\10_20_2025\backtests_final\VolatilityDivergence_BTFinal_v6.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position and bull_div and vol_ok and above_sma and price > self.bb_upper[-1] and price > ema_max:
            ...
            if sl_distance > 0:
                ...
                size = int(round(size))
                if size > 0:
                    max_affordable = int(self.cash // entry_price) if entry_price > 0 else 0
                    ...
                    if size > 0:
                        ...
                        print(f"🌙 Moon Dev: Size {size} after affordability cap <=0, no LONG trade 😔")
                else:
            ...
            if sl_distance > 0:
                ...
                if size > 0:
                    ...
                    if size > 0:
                        ...
                        print(f"🌙 Moon Dev: Size {size} after affordability cap <=0, no SHORT trade 😔")
                else:
