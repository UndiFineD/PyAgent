# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\video_agent.py
"""
...
def create_video_job(
    ...
    try:
        ...
        # Create job object
        job = VideoJob(job_id=video.id, prompt=prompt, status="queued", created_at=datetime.now())
            ...
            while attempts < max_attempts:
                ...
                if completed:
                    if job.status == "completed":
                        cprint(
                            f"✅ Worker {worker_id} completed: {job.job_id[:16]}...",
                            "green",
                        )
...
def display_status():
    ...
    with jobs_lock:
        ...
        for job in sorted(jobs.values(), key=lambda j: j.created_at, reverse=True):
            ...
            # Display job
            cprint(
                f"\n{status_emoji} {job.job_id[:16]}... [{job.status.upper()}] {time_info}",
                status_color,
                attrs=["bold"],
            )
...
""",
    ...
    cprint(
        f"   Model: {MODEL} | Resolution: {DEFAULT_RESOLUTION} | Duration: {DEFAULT_DURATION}s",
        "white",
    )
    ...
    try:
        ...
        while True:
            ...
            # Create video job
            cprint(
                f"\n🎬 Creating video: '{user_input[:60]}{'...' if len(user_input) > 60 else ''}'",
                "cyan",
            )
            ...
            if job:
                ...
                cprint(
                    f"   Status: {queued} queued | {generating} generating | {completed} completed",
                    "cyan",
                )
