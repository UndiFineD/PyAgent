# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\chat_agent_og.py
"""
    ...
    def get_chat_messages(self):
        ...
        try:
            ...
            if not self.last_messages:  # If this is first run
                messages_to_process = (
                    all_messages[-DEFAULT_INITIAL_CHATS:] if len(all_messages) > DEFAULT_INITIAL_CHATS else all_messages
                )
    ...
    def get_live_chat_id(self, channel_id):
        ...
        try:
            ...
            if self.using_fallback:
                ...
                if url:
                    cprint(
                        f"✨ Successfully connected to live stream via Selenium!",
                        "green",
                    )
...
"""
    ...
    def process_question(self, user, question):
        try:
            ...
            formatted_prompt = CHAT_PROMPT.format(knowledge_base=knowledge_base)

            # For simple questions, use a minimal prompt
            if len(question.split()) < 5:
                formatted_prompt = (
                    """You are Moon Dev's Live Stream Chat AI Agent. Keep responses short and friendly with emojis."""
                )
    ...
    def run(self):
        ...
        if USE_RESTREAM:
            ...
            if not self.restream_handler:
                cprint(
                    "❌ Restream handler not initialized - check your credentials!",
                    "red",
                )
                return
