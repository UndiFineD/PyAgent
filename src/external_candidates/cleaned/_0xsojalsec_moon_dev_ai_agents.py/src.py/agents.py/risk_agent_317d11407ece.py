# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\risk_agent.py
"""
...
class RiskAgent(BaseAgent):
    def __init__(self):
        ...
        if deepseek_key and MODEL_OVERRIDE.lower() == "deepseek-chat":
            self.deepseek_client = openai.OpenAI(api_key=deepseek_key, base_url=DEEPSEEK_BASE_URL)
    ...
    def should_override_limit(self, limit_type):
        ...
        try:
            ...
            if self.last_override_check and datetime.now() - self.last_override_check < timedelta(minutes=15):
            ...
            positions = positions[
                positions["Mint Address"].isin(MONITORED_TOKENS) & ~positions["Mint Address"].isin(EXCLUDED_TOKENS)
            ]
            ...
            if self.deepseek_client and MODEL_OVERRIDE.lower() == "deepseek-chat":
                ...
                response = self.deepseek_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are Moon Dev's Risk Management AI. Analyze positions and respond with OVERRIDE or RESPECT_LIMIT.",
                        },
            ...
            cprint(
                f"Using model: {'DeepSeek' if self.deepseek_client else 'Claude'}",
                "white",
                "on_blue",
            )
            print(response_text)
            ...
            if self.deepseek_client and MODEL_OVERRIDE.lower() == "deepseek-chat":
                ...
                response = self.deepseek_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are Moon Dev's Risk Management AI. Analyze the breach and decide whether to close positions.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    max_tokens=self.ai_max_tokens,
