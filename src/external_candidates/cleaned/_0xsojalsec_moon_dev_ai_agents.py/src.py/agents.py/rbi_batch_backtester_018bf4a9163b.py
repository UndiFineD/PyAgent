# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\rbi_batch_backtester.py
"""
...
DEFAULT_RESEARCH_DIR = PROJECT_ROOT / "src" / "data" / "rbi" / "03_14_2025" / "research"
...
def run_one_strategy(research_dir: Path, file_path: Path, conda_env: str) -> None:
    ...
    print(
        json.dumps(
            {k: v for k, v in result.items() if k != "stdout" and k != "stderr"},
            indent=2,
        )
    )
