# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\rbi_agent.py
"""
...
def init_deepseek_client():
    ...
    try:
        ...
        if not deepseek_key:
            cprint(
                "⚠️ DEEPSEEK_KEY not found - DeepSeek models will not be available",
                "yellow",
            )
            return None
...
def chat_with_model(system_prompt, user_content, model_config):
    ...
    try:
        ...
        if hasattr(e, "response"):
            cprint(
                f"🔍 Response error: {getattr(e, 'response', 'No response details')}",
                "yellow",
            )
...
def main():
    ...
    cprint(
        f"🔍 Status: {already_processed} already processed, {new_ideas} new ideas",
        "cyan",
    )
    ...
    for i, idea in enumerate(ideas, 1):
        ...
        if is_idea_processed(idea):
            cprint(f"\n{'=' * 50}", "red")
            cprint(
                f"⏭️  SKIPPING idea {i}/{total_ideas} - ALREADY PROCESSED",
                "red",
                attrs=["reverse"],
            )
            idea_snippet = idea[:100] + ("..." if len(idea) > 100 else "")
        ...
        try:
            ...
            if max_ideas and processed_count >= max_ideas:
                cprint(
                    "🛑 Reached RBI_MAX_IDEAS limit, exiting after quick debug run.",
                    "yellow",
                )
                break
...
if __name__ == "__main__":
    try:
        ...
        cprint(
            f"📅 Today's Date: {TODAY_DATE} - All outputs will be saved in this folder",
            "magenta",
        )
        cprint(
            f"🧠 DeepSeek-R1 thinking tags will be automatically removed from outputs",
            "magenta",
        )
        cprint(f"📋 Processed ideas log: {PROCESSED_IDEAS_LOG}", "magenta")
        cprint("\n🤖 Model Configurations:", "cyan")
        cprint(
            f"📚 Research: {RESEARCH_CONFIG['type']} - {RESEARCH_CONFIG['name']}",
            "cyan",
        )
        cprint(
            f"📊 Backtest: {BACKTEST_CONFIG['type']} - {BACKTEST_CONFIG['name']}",
            "cyan",
        )
