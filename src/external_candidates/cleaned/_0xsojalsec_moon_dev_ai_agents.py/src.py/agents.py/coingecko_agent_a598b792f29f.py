# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\coingecko_agent.py
...
def print_banner():
    ...
    cprint("\n" + "=" * 70, "white", "on_blue")
    cprint(
        "🌙 🎮 Moon Dev's Crypto Trading Game! 🎮 🌙",
        "white",
        "on_magenta",
        attrs=["bold"],
    )
...
"""
            ...
            response = (
                response_text.replace("TextBlock(text='", "")
                .replace("')", "")
                .replace("\\n", "\n")
                .replace("*", "")
                .replace("```", "")
...
""",
    ...
    def run_conversation_cycle(self):
        ...
        try:
            ...
            self.token_extractor.extract_tokens(len(self.round_history) + 1, agent_one_response, agent_two_response)
