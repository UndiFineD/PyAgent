# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\prompt_agent.py
"""
...
def animate_loading(duration=2, message="Processing", emoji="🌙"):
    ...
    while time.time() < end_time:
        ...
        clear_line()
        print(
            f"\r{colored(f' {frame} {message} {emoji} ', color, 'on_blue')}",
            end="",
            flush=True,
        )
...
def enhance_prompt_with_glm(user_prompt: str) -> Optional[str]:
    ...
    try:
        ...
        payload = {
            "model": GLM_MODEL,
            "messages": [
                {"role": "system", "content": PROMPT_DESIGN_GUIDELINES},
                {
                    "role": "user",
    ...
    try:
        while True:
            ...
            if enhanced_prompt:
                ...
                cprint(
                    f"\n✅ Prompt #{prompt_count} enhanced successfully!",
                    "white",
                    "on_green",
                )
        ...
        cprint(
            "\n🌙 Thanks for using Moon Dev's Prompt Enhancement Agent!",
            "white",
            "on_magenta",
        )
