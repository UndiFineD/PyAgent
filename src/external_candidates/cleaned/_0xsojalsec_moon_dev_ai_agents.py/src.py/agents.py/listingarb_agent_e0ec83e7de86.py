# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\listingarb_agent.py
"""
...
class AIAgent:
    ...
    def __init__(self, name: str, model: str):
        ...
        if "deepseek" in self.model.lower():
            ...
            if deepseek_key:
                self.client = openai.OpenAI(api_key=deepseek_key, base_url=DEEPSEEK_BASE_URL)
...
class ListingArbSystem:
    ...
    def __init__(self):
        ...
        cprint(
            "🔍 Moon Dev's Listing Arb System Ready!",
            "white",
            "on_green",
            attrs=["bold"],
        )
    ...
    def run_analysis_cycle(self):
        ...
        try:
            ...
            batches = [
                discovered_tokens_df[i : i + batch_size] for i in range(0, len(discovered_tokens_df), batch_size)
            ]
            ...
            if AI_ANALYSIS_FILE.exists():
                ...
                if "coingecko_url" not in full_analysis.columns:
                    ...
                    if missing_urls.any():
                        ...
                        full_analysis.loc[missing_urls, "coingecko_url"] = full_analysis.loc[
                            missing_urls, "token_id"
                        ].apply(lambda x: f"https://www.coingecko.com/en/coins/{x}")
