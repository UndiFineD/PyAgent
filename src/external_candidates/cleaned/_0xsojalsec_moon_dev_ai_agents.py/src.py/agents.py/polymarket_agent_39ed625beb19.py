# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\polymarket_agent.py
"""
...
class PolymarketAgent:
    ...
    def __init__(self):
        ...
        cprint("\n" + "=" * 80, "cyan")
        cprint(
            "🌙 Polymarket Prediction Market Agent - Initializing",
            "cyan",
            attrs=["bold"],
        )
        cprint("=" * 80, "cyan")
        ...
        cprint(
            f"🔮 Loaded {len(self.predictions_df)} existing predictions from CSV",
            "cyan",
        )
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if data.get("type") == "subscribed":
                    cprint(
                        "✅ Moon Dev WebSocket subscribed successfully to live trades!",
                        "green",
                    )
    ...
    def on_ws_close(self, ws, close_status_code, close_msg):
        ...
        self.ws_connected = False
        cprint(
            f"\n🔌 Moon Dev WebSocket connection closed: {close_status_code} - {close_msg}",
            "yellow",
        )
    ...
    def fetch_historical_trades(self, hours_back=None):
        ...
        try:
            cprint(
                f"\n📡 Moon Dev fetching historical trades (last {hours_back}h)...",
                "yellow",
            )
            ...
            cprint(
                f"💰 Found {len(filtered_trades)} trades over ${MIN_TRADE_SIZE_USD} (after filters)",
                "cyan",
            )
    ...
    def process_trades(self, trades):
        ...
        if new_markets > 0 or updated_markets > 0:
            ...
            if updated_markets > 0:
                cprint(
                    f"🔄 Updated {updated_markets} existing markets with fresh trade data",
                    "cyan",
                )
    ...
    def display_recent_markets(self):
        ...
        cprint("\n" + "=" * 80, "cyan")
        cprint(
            f"📊 Most Recent {min(MARKETS_TO_DISPLAY, len(self.markets_df))} Markets",
            "cyan",
            attrs=["bold"],
        )
        cprint("=" * 80, "cyan")
    ...
    def get_ai_predictions(self):
        ...
        cprint("\n" + "=" * 80, "magenta")
        cprint(
            f"🤖 AI Analysis - Analyzing {len(markets_to_analyze)} markets",
            "magenta",
            attrs=["bold"],
        )
        cprint(f"📊 Analysis Run ID: {analysis_run_id}", "magenta")
        cprint(
            f"💰 Price info to AI: {'✅ ENABLED' if SEND_PRICE_INFO_TO_AI else '❌ DISABLED'}",
            "green" if SEND_PRICE_INFO_TO_AI else "yellow",
        )
        cprint("=" * 80, "magenta")
...
Provide predictions for each market in the specified format."""
        ...
        if USE_SWARM_MODE and self.swarm:
            ...
            successful_responses = [
                name for name, data in swarm_result.get("responses", {}).items() if data.get("success")
            ]
            ...
            for model_name, model_data in swarm_result.get("responses", {}).items():
                if model_data.get("success"):
                    ...
                    cprint(f"\n{'=' * 80}", "cyan")
                    cprint(
                        f"✅ {model_name.upper()} ({response_time:.1f}s)",
                        "cyan",
                        attrs=["bold"],
                    )
                    cprint(f"{'=' * 80}", "cyan")
            ...
            try:
                ...
                self.predictions_df = pd.concat(
                    [self.predictions_df, pd.DataFrame([prediction_record])],
                    ignore_index=True,
                )
                self._save_predictions()
                cprint(
                    f"✅ Saved analysis run {analysis_run_id} to predictions database",
                    "green",
                )
    ...
    def _mark_markets_analyzed(self, markets, analysis_timestamp):
        ...
        try:
            ...
            cprint(
                f"✅ Marked {len(analyzed_market_ids)} markets with analysis timestamp",
                "green",
            )
    ...
    def _save_swarm_predictions(self, analysis_run_id, analysis_timestamp, markets, swarm_result):
        ...
        try:
            ...
            if new_records:
                # Add all new records
                self.predictions_df = pd.concat([self.predictions_df, pd.DataFrame(new_records)], ignore_index=True)

                # Save to CSV
                self._save_predictions()

                cprint(
                    f"✅ Saved {len(new_records)} market predictions (run {analysis_run_id})",
                    "green",
                )
            else:
    ...
    def _calculate_polymarket_consensus(self, swarm_result, markets_df):
        ...
        try:
            ...
            if market_votes:
                ...
                for market_num in sorted(market_votes.keys()):
                    ...
                    if 1 <= market_num <= len(markets_list):
                        ...
                        consensus_text += (
                            f"  Votes: YES: {votes['YES']} | NO: {votes['NO']} | NO_TRADE: {votes['NO_TRADE']}\n\n"
                        )
                    else:
                        consensus_text += f"Market {market_num}: {majority} ({confidence}% consensus)\n"
                        consensus_text += (
                            f"  YES: {votes['YES']} | NO: {votes['NO']} | NO_TRADE: {votes['NO_TRADE']}\n\n"
                        )
            else:
            ...
            # Show failed models
            failed_models = [provider for provider, data in swarm_result["responses"].items() if not data["success"]]
            if failed_models:
    ...
    def _get_top_consensus_picks(self, swarm_result, markets_df):
        ...
        try:
            cprint("\n" + "=" * 80, "yellow")
            cprint(
                "🧠 Running Consensus AI to identify top 3 picks...",
                "yellow",
                attrs=["bold"],
            )
            cprint("=" * 80 + "\n", "yellow")
            ...
            cprint(
                f"🏆 TOP {TOP_MARKETS_COUNT} CONSENSUS PICKS - MOON DEV AI RECOMMENDATION",
                "white",
                "on_blue",
                attrs=["bold"],
            )
    ...
    def status_display_loop(self):
        ...
        while True:
            try:
                ...
                cprint(f"\n{'=' * 60}", "cyan")
                cprint(
                    f"📊 Moon Dev Status @ {datetime.now().strftime('%H:%M:%S')}",
                    "cyan",
                    attrs=["bold"],
                )
                cprint(f"{'=' * 60}", "cyan")
                cprint(
                    f"   WebSocket Connected: {'✅ YES' if self.ws_connected else '❌ NO'}",
                    "green" if self.ws_connected else "red",
                )
                ...
                cprint(
                    f"   Filtered trades (>=${MIN_TRADE_SIZE_USD}): {self.filtered_trades_count}",
                    "yellow",
                )
                cprint(f"   Total markets in database: {total_markets}", "white")
                cprint(
                    f"   Fresh eligible markets: {fresh_eligible_count}",
                    ("yellow" if fresh_eligible_count < NEW_MARKETS_FOR_ANALYSIS else "green"),
                    attrs=["bold"],
                )
                ...
                if fresh_eligible_count >= NEW_MARKETS_FOR_ANALYSIS:
                    cprint(
                        f"   ✅ Ready for analysis! (Have {fresh_eligible_count}, need {NEW_MARKETS_FOR_ANALYSIS})",
                        "green",
                        attrs=["bold"],
                    )
                else:
                    cprint(
                        f"   ⏳ Collecting... (Have {fresh_eligible_count}, need {NEW_MARKETS_FOR_ANALYSIS})",
                        "yellow",
                    )
    ...
    def analysis_cycle(self):
        ...
        if is_first_run:
            ...
            cprint(
                f"   Will analyze whatever markets we have collected (minimum 1)",
                "yellow",
            )
            cprint(
                f"   Future runs will require {NEW_MARKETS_FOR_ANALYSIS} fresh eligible markets\n",
                "yellow",
            )
        else:
            ...
            if fresh_eligible_count >= NEW_MARKETS_FOR_ANALYSIS:
                cprint(
                    f"   ✅ REQUIREMENT MET - Running analysis!",
                    "green",
                    attrs=["bold"],
                )
            else:
                cprint(
                    f"   ❌ Need {NEW_MARKETS_FOR_ANALYSIS - fresh_eligible_count} more fresh eligible markets",
                    "yellow",
                    attrs=["bold"],
                )
            cprint("", "white")
        ...
        if should_analyze:
            ...
            cprint(
                f"\n💾 Updated analysis tracker: {self.last_analyzed_count} markets in database",
                "green",
            )
            cprint(
                f"⏰ Next run will only count markets with fresh trades after {datetime.now().strftime('%H:%M:%S')}",
                "cyan",
            )
        else:
            needed = NEW_MARKETS_FOR_ANALYSIS - fresh_eligible_count
            cprint(
                f"\n⏳ Need {needed} more fresh eligible markets before next analysis",
                "yellow",
            )
            cprint(
                f"   Waiting for trades on eligible markets (never analyzed OR >{REANALYSIS_HOURS}h old)",
                "yellow",
            )
    ...
    def analysis_loop(self):
        ...
        cprint(
            f"🧠 Running first analysis NOW, then checking every {ANALYSIS_CHECK_INTERVAL_SECONDS} seconds\n",
            "magenta",
        )

        # 🌙 Moon Dev - Run first analysis IMMEDIATELY (no waiting!)
        cprint(
            "🚀 Moon Dev running first analysis immediately...\n",
            "yellow",
            attrs=["bold"],
        )
        ...
        while True:
            try:
                ...
                cprint(
                    f"⏰ Next analysis check at: {next_check.strftime('%H:%M:%S')}\n",
                    "magenta",
                )
...
def main():
    ...
    cprint(
        f"🚫 Filtering out crypto/Bitcoin markets ({len(IGNORE_CRYPTO_KEYWORDS)} keywords)",
        "red",
    )
    cprint(
        f"🚫 Filtering out sports markets ({len(IGNORE_SPORTS_KEYWORDS)} keywords)",
        "red",
    )
    cprint(
        f"📜 Lookback period: {LOOKBACK_HOURS} hours (fetches historical data on startup)",
        "yellow",
    )
    cprint("", "yellow")
    ...
    cprint(
        f"   🤖 Analysis Thread: Every {ANALYSIS_CHECK_INTERVAL_SECONDS}s - Checks for new markets",
        "magenta",
    )
    cprint(
        f"   🎯 AI Analysis triggers when {NEW_MARKETS_FOR_ANALYSIS} new markets collected",
        "yellow",
    )
    cprint("", "yellow")
    cprint(
        f"🤖 AI Mode: {'SWARM (6 models)' if USE_SWARM_MODE else 'Single Model'}",
        "yellow",
    )
    cprint(
        f"💰 Price Info to AI: {'ENABLED' if SEND_PRICE_INFO_TO_AI else 'DISABLED'}",
    ...
    cprint("\n" + "=" * 80, "yellow")
    cprint(
        f"📜 Moon Dev fetching historical data from last {LOOKBACK_HOURS} hours...",
        "yellow",
        attrs=["bold"],
    )
    cprint("=" * 80, "yellow")
    ...
    try:
        ...
        cprint(
            "✨ Moon Dev WebSocket + AI running! Press Ctrl+C to stop.\n",
            "green",
            attrs=["bold"],
        )
        while True:
