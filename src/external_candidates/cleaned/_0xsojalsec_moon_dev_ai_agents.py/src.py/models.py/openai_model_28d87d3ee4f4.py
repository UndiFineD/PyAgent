# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\models\openai_model.py
"""
...
class OpenAIModel(BaseModel):
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            cprint(
                f"✨ Moon Dev's magic initialized OpenAI model: {self.model_name} 🌟",
                "green",
            )
    ...
    def _prepare_model_kwargs(self, **kwargs):
        ...
        if self._supports_reasoning_effort():
            cprint(
                "🚀 Moon Dev's O3 model powering up with reasoning capabilities! 🌙",
                "cyan",
            )
    ...
    def generate_response(self, system_prompt, user_content, **kwargs):
        ...
        try:
            ...
            if self.model_name.startswith("o3"):
                cprint(
                    "🧠 Using Moon Dev's O3 model with reasoning capabilities...",
                    "cyan",
                )
            ...
            response = self.client.chat.completions.create(model=self.model_name, messages=messages, **model_kwargs)
            ...
            if not content_text:
                cprint(
                    "🔁 Retrying once with simplified prompt format (Moon Dev fallback)",
                    "yellow",
                )
                retry_messages = [
                    {
            ...
            if not content_text and hasattr(self.client, "responses"):
                try:
                    cprint(
                        "🛠️ Moon Dev fallback: trying Responses API for text output",
                        "yellow",
                    )
                    content_str = f"Instructions: {system_prompt}\n\nInput: {user_content}"
                    ...
                    cprint(
                        f"❌ Moon Dev Responses API fallback error: {repr(fallback_e)}",
                        "red",
                    )
            ...
            if (not content_text) and self.model_name == "gpt-5":
                try:
                    cprint(
                        "🛟 Moon Dev fallback: retrying with gpt-4o to avoid empty content",
                        "yellow",
                    )
                    fallback_messages = [
                        {"role": "system", "content": system_prompt},
