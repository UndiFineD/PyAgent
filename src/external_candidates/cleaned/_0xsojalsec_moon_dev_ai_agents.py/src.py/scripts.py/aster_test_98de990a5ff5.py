# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\aster_test.py
"""
    ...
    try:
        ...
        for sym_info in exchange_info.get("symbols", []):
            if sym_info["symbol"] == symbol:
                ...
                cprint(
                    f"📏 Precision for {symbol}: Price={price_precision} decimals, Qty={quantity_precision} decimals",
                    "cyan",
                )
...
def place_limit_orders(symbol, midpoint):
    ...
    cprint(
        f"\n📝 STEP 3: Placing Limit Orders ${ORDER_OFFSET} from Midpoint...",
        "cyan",
        attrs=["bold"],
    )
...
def place_market_order(symbol):
    ...
    try:
        ...
        order = api.place_order(symbol=symbol, side="BUY", order_type="MARKET", quantity=quantity)
...
def close_position(symbol):
    ...
    try:
        ...
        cprint(
            f"📊 Current position: {position_amt} ({'LONG' if is_long else 'SHORT'})",
            "yellow",
        )
        ...
        if not new_position:
            ...
            cprint(
                f"⚠️  Position still exists: {new_position['position_amount']}",
                "yellow",
            )
...
def main():
    ...
    try:
        ...
        cprint(
            ...
            "green" if balance_change >= 0 else "red",
            attrs=["bold"],
        )
