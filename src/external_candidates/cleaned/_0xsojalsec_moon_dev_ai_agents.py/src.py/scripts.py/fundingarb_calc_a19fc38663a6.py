# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\fundingarb_calc.py
#!/usr/bin/env python3
"""
...
def print_calculator_results(results):
    ...
    print(
        f"Slippage (decimal)           : {results['slippage_decimal']:.4f} ({results['slippage_decimal'] * 100:.2f}%)"
    )
    print(f"Fees (decimal)               : {results['fees_decimal']:.4f} ({results['fees_decimal'] * 100:.2f}%)")
    print(
        f"Total one-time cost (decimal): {results['total_cost_decimal']:.4f} "
        f"({results['total_cost_decimal'] * 100:.2f}%)"
    )
    print(
        f"Annual funding rate (decimal): {results['annual_funding_rate_decimal']:.4f} "
        f"({results['annual_funding_rate_decimal'] * 100:.2f}%)"
    )
    print(
        f"Hourly funding rate (decimal): {results['hourly_funding_rate_decimal']:.8f} "
        f"({results['hourly_funding_rate_decimal'] * 100:.6f}% per hour)"
    )
    print(f"Hold duration (hours)        : {results['hold_hours']} h")
    print(
        f"Desired profit (decimal)     : {results['desired_profit_decimal']:.4f} "
        f"({results['desired_profit_decimal'] * 100:.2f}%)"
    )

    print("\n--- KEY CALCULATIONS ---")
    print(
        f"Total Funding Earned (decimal): {results['total_funding_earned_decimal']:.6f} "
        f"({results['total_funding_earned_decimal'] * 100:.4f}%)"
    )
    print(
        f"Net Result (decimal)         : {results['net_result_decimal']:.6f} "
    ...
    print(
        f"Annual Rate for Break Even in {results['hold_hours']}h : "
