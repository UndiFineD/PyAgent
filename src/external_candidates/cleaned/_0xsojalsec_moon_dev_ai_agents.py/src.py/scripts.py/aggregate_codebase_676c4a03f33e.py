# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\aggregate_codebase.py
#!/usr/bin/env python3
"""
...
class CodebaseAggregator:
    def __init__(self):
        ...
        if SPECIFIC_FOLDER:
            cprint(
                f"\n📁 Scanning specific folder: {SPECIFIC_FOLDER}",
                "yellow",
                attrs=["bold"],
            )
    ...
    def load_gitignore(self):
        ...
        if gitignore_path.exists():
            ...
            cprint(
                f"📋 Loaded {len(self.gitignore_patterns)} patterns from .gitignore",
                "green",
            )
    ...
    def aggregate_code(self):
        ...
        for root, dirs, files in os.walk(SCAN_PATH):
            ...
            dirs[:] = [
                d
                for d in dirs
                if not d.startswith(".") and d not in ["venv", "env", "__pycache__", "node_modules", "dist", "build"]
            ]
            ...
            for file in files:
                ...
                if should_exclude:
                    if file_path.suffix == ".py":
                        ...
                        cprint(
                            f"⏭️  Skipping: {file_path.relative_to(PROJECT_ROOT)}",
                            "yellow",
                        )
...
""")
            ...
            cprint(
                "\n🚀 You can now share this file with other AIs for code analysis!",
                "cyan",
                attrs=["bold"],
            )
