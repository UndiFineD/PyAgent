# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Web-Navigator\src\tool\registry\__init__.py
        ...
        try:
            ...
            if tool.params:
                ...
                if not isinstance(input, dict):
                    raise TypeError(
                        f"Action Input for tool '{name}' must be a dictionary, but got {type(input)}: {input}"
                    )
