# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Web-Navigator\src\memory\episodic\__init__.py
import json
...
from src.memory.episodic.views import Memories, Memory
from src.message import BaseMessage, HumanMessage, SystemMessage
...
class EpisodicMemory(BaseMemory):
    def __init__(
        self,
        knowledge_base: str = "knowledge_base.json",
    ...
    def idle_memory(self, conversation: list[BaseMessage]):
        if self.verbose:
            print(
                f"{colored(f'Idle memory:', color='yellow', attrs=['bold'])}\n{json.dumps(self.memories.all(), indent=2)}"
            )
        return None
    ...
    def add_memory(self, conversation: list[BaseMessage]):
        ...
        if self.verbose:
            print(
                f"{colored(f'Adding memory to Knowledge Base:', color='yellow', attrs=['bold'])}\n{json.dumps(memory.to_dict(), indent=2)}"
            )
    ...
    def update_memory(self, conversation: list[BaseMessage]):
        ...
        if self.verbose:
            print(
                f"{colored(f'Updated memory from Knowledge Base:', color='yellow', attrs=['bold'])}\n{json.dumps(memory.to_dict(), indent=2)}"
            )
        with open(f"./memory_data/{self.knowledge_base}", "r+") as f:
    ...
    def replace_memory(self, conversation: list[BaseMessage]):
        ...
        if self.verbose:
            print(
                f"{colored(f'Replacing memory from Knowledge Base:', color='yellow', attrs=['bold'])}\n{json.dumps(memory.to_dict(), indent=2)}"
            )
        with open(f"./memory_data/{self.knowledge_base}", "r+") as f:
            knowledge_base = [Memory.model_validate(memory) for memory in json.loads(f)]
    ...
    def retrieve(self, query: str) -> list[dict]:
        ...
        messages = [
            SystemMessage(system_prompt.format(memories=memories)),
        ...
        if self.verbose:
            print(
                f"{colored(f'Retrieved memories from Knowledge Base:', color='yellow', attrs=['bold'])}\n{json.dumps(self.memories.all(), indent=2)}"
            )
