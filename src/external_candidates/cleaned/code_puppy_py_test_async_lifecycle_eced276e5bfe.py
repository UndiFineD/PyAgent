# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\mcp.py\test_async_lifecycle_eced276e5bfe.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_async_lifecycle.py
...
class TestAsyncServerLifecycleTaskLifecycle:
    ...
    async def test_lifecycle_task_context_enter_exit(self):
        ...
        with patch.object(AsyncExitStack, "enter_async_context", mock_enter_async_context):
            with patch.object(AsyncExitStack, "aclose", mock_aclose):
                # Create a task that will be immediately cancelled

                task = asyncio.create_task(manager._server_lifecycle_task("test-server", server))
...
class TestAsyncServerLifecycleManagerStopServer:
    ...
    async def test_stop_server_internal_called_with_lock(self):
        ...
        with patch.object(manager, "_stop_server_internal", new_callable=AsyncMock) as mock_internal:
...
class TestAsyncServerLifecycleManagerListServers:
    ...
    def test_list_servers_multiple_servers(self):
        ...
        assert result["server-0"]["uptime_seconds"] <= result["server-1"]["uptime_seconds"]

        assert result["server-1"]["uptime_seconds"] <= result["server-2"]["uptime_seconds"]
