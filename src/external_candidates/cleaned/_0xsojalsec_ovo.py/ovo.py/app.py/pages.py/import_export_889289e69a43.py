# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\import_export.py
import os
import shutil
import tempfile
import zipfile
...
from ovo.core.logic.import_export_logic import (
...
def export_tab():
    ...
    project_ids = sorted(
        projects_by_id.keys(),
        key=lambda p: projects_by_id[p].created_date_utc,
        reverse=True,
    )
    project_id = st.selectbox(
        ...
        options=project_ids,
        index=(
            project_ids.index(session_state.project.id)
