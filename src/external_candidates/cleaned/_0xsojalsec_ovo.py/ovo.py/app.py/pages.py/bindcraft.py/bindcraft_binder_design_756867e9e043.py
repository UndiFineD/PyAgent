# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\bindcraft\bindcraft_binder_design.py
import json
...
def input_step():
    ...
    molstar_custom_component(
        structures=[
            StructureVisualization(
                pdb=storage.read_file_str(workflow.bindcraft_params.input_pdb_path),
                color="chain-id",
            )
        ],
        key="input_structure",
...
def settings_step():
    ...
    with st.columns([1, 2])[0]:
        ...
        workflow.bindcraft_params.hotspots = st.text_input(
            ...
            value=workflow.bindcraft_params.hotspots,
            key="hotspots",
        )
