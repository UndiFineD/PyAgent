# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_whisper_flow.py\whisperflow.py\streaming_9bd3d1292835.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-whisper-flow\whisperflow\streaming.py
...
def should_close_segment(result: dict, prev_result: dict, cycles, max_cycles=1):
    ...
    return cycles >= max_cycles and result["data"]["text"] == prev_result.get("data", {}).get("text", "")
...
class TranscribeSession:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, transcribe_async, send_back_async) -> None:
        ...
        self.task = asyncio.create_task(transcribe(self.should_stop, self.queue, transcribe_async, send_back_async))
