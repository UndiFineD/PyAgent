# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\steipete.py\local_places.py\src.py\local_places.py\schemas_eceaa65d92d3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\steipete\local-places\src\local_places\schemas.py
...
class Filters(BaseModel):
    ...
    def validate_types(cls, value: list[str] | None) -> list[str] | None:
        ...
        if len(value) > 1:
            raise ValueError("Only one type is supported. Use query/keyword for additional filtering.")
