# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\memory.py\memory_manager_b1add105878a.py
...
class MemoryManager(BaseModule):
    ...
    async def _prompt_llm_for_memory_operation(
        ...
        relevant_data_str = "\n".join(
            [json.dumps({"message": msg.to_dict(), "memory_id": mid}) for msg, mid in (relevant_data or [])]
        )

        prompt = MANAGER_PROMPT.replace("<<INPUT_DATA>>", json.dumps(input_data, ensure_ascii=False)).replace(
            "<<RELEVANT_DATA>>", relevant_data_str
        )
    ...
    async def handle_memory(
        ...
        if action == "add":
            ...
            input_data = [{"action": "add", "memory_id": str(uuid4()), "message": msg.to_dict()} for msg in messages]
            ...
            input_data = [{"action": "update", "memory_id": mid, "message": msg.to_dict()} for mid, msg in updates]
            ...
            if self.use_llm_management and self.llm:
                ...
                valid_memory_ids = [
                    decision.get("memory_id") for decision in llm_decisions if decision.get("action") == "delete"
                ]
