# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\providers.py\gemini_668a4d3b640a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\providers\gemini.py
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.vertexai and (not self.project or not self.location):
            raise exceptions.InferenceConfigError("Vertex AI mode requires both project and location parameters")

        if self.api_key and self.vertexai:
            logging.warning(
                "Both API key and Vertex AI configuration provided. API key will take precedence for authentication."
            )
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_index):
                    ...
                    try:
                        ...
                        raise exceptions.InferenceRuntimeError(f"Parallel inference error: {str(e)}", original=e) from e

                for result in results:
                    if result is None:
                        raise exceptions.InferenceRuntimeError("Failed to process one or more prompts")
