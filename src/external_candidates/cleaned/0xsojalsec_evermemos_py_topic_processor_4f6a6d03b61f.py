# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\group_profile.py\topic_processor_4f6a6d03b61f.py
...
class TopicProcessor:
    ...
    def find_topic_to_replace(self, topics: List, reference_time: Optional[datetime] = None) -> object:
        """
        ...
        return sorted(
            ...
            key=lambda t: (
                ...
                not (t.status == "implemented" and (t.last_active_at or now) < threshold),
                # Second priority: sort from oldest to newest
                t.last_active_at or get_now_with_timezone().replace(year=1900),
            ),
        )[0]

    def get_latest_memcell_timestamp(self, memcell_list: List, memcell_ids: Optional[List[str]] = None) -> datetime:
        """
    ...
    def apply_topic_incremental_updates(
        ...
        for topic_data in existing_topics_with_evidences:
            if isinstance(topic_data, dict):
                ...
                try:
                    ...
                    logger.warning(f"Failed to parse last_active_at: {last_active_str}, error: {e}")

                    last_active_at = get_now_with_timezone()

                topic_id = topic_data.get("id", f"topic_{str(uuid.uuid4()).replace('-', '')[:8]}")
        ...
        for llm_topic in llm_topics:
            ...
            if update_type == "update" and old_topic_id and old_topic_id in topic_dict:
                ...
                last_active_at = self.get_latest_memcell_timestamp(memcell_list, merged_evidences)
                ...
                valid_llm_evidences = self.data_processor.validate_and_filter_memcell_ids(
                    llm_evidences, valid_memcell_ids
                )

                # Calculate last_active_at based on evidences

                last_active_at = self.get_latest_memcell_timestamp(memcell_list, valid_llm_evidences)
                ...
                logger.debug(f"[TopicIncremental] Added new topic: {new_id} -> {new_topic.name}")
