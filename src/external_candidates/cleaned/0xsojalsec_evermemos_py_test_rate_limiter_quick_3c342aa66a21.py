# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\tests.py\test_rate_limiter_quick_3c342aa66a21.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_rate_limiter_quick.py
...
class TestRateLimiterQuick:
    ...
    async def test_basic_functionality(self):
        ...
        results = await asyncio.gather(quick_test_func(), quick_test_func(), quick_test_func())
    ...
    async def test_multiple_limiters_isolation(self):
        ...
        @rate_limit(max_rate=5, time_period=1, key_func=lambda service: f"service_{service}")
        ...
        assert fast_total_time >= 0.15, f"Fast service rate limiting wait time too short: {fast_total_time:.3f} seconds"
