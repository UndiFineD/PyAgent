# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agents_generic.py\tests.py\test_tts_fallback_1563fc435979.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\test_tts_fallback.py
...
async def test_tts_recover() -> None:
    ...
    assert (await asyncio.wait_for(fallback_adapter.availability_changed_ch(fake2).recv(), 1.0)).available, (
        "fake2 should have recovered"
    )
...
async def test_timeout():
    ...
    with pytest.raises(APIConnectionError):
        async with fallback_adapter.stream(conn_options=APIConnectOptions(timeout=0.1, max_retry=0)) as stream:
            stream.push_text("hello test")
