# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\robbyczgw_cla.py\sports_ticker.py\scripts.py\auto_setup_crons_4a581f59803b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\sports-ticker\scripts\auto_setup_crons.py
...
from config import get_team_by_name, get_teams

# Import schedule functions

from schedule import get_all_fixtures_json, get_team_schedule


def fixture_to_crons(
    fixture: dict,
    reminder_mins: int = 30,
    ticker_before_mins: int = 5,
    ticker_duration_mins: int = 180,
) -> list:
...
def format_cron_summary(crons: list) -> str:
    ...
    for cron in crons:
        ...
        if fixture_id not in fixtures:
            fixtures[fixture_id] = {"desc": cron.get("description", ""), "crons": []}
...
Examples:
  ...
  %(prog)s --commands                # Output as CLI commands
    ...
    parser.add_argument("--days", "-d", type=int, default=7, help="Days to look ahead (default: 7)")

    parser.add_argument("--team", "-t", type=str, help="Filter by team name")
    ...
    parser.add_argument("--commands", "-c", action="store_true", help="Output as OpenClaw CLI commands")
