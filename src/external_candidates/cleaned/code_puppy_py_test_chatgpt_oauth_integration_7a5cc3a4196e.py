# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\plugins.py\test_chatgpt_oauth_integration_7a5cc3a4196e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_chatgpt_oauth_integration.py
...
class TestModelManagement:
    ...
    def test_add_models_with_context_settings(self, mock_path, tmp_path):
        ...
        assert model_config["context_length"] == CHATGPT_OAUTH_CONFIG["default_context_length"]
...
class TestCustomCommands:
    ...
    def test_handle_chatgpt_status_authenticated(self, mock_info, mock_success, mock_load):
        ...
        with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.load_chatgpt_models") as mock_models:
    ...
    @patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.emit_success")
    @patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.emit_info")
    def test_handle_chatgpt_logout(self, mock_info, mock_success, mock_path, mock_remove):
    ...
    @patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.set_model_and_reload_agent")
    ...
    def test_handle_custom_command_status(self, mock_load):
        ...
        with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.load_chatgpt_models"):
            with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.emit_success"):
    ...
    def test_handle_custom_command_logout(self):
        ...
        with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.get_token_storage_path") as mock_path:
            mock_path.return_value = Mock(exists=Mock(return_value=False))

            with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.remove_chatgpt_models"):
                with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.emit_success"):
