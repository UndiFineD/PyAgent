# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2015.py\ka_playgrounds.py\circuits_f345c4fb6067.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\ka_playgrounds\circuits.py
...
class CircuitReduction(Scene):
    def construct(self):
        ...
        source = Mobject(Line(0.5 * UP, 2 * UP), Source().scale(0.5), Line(2 * DOWN, 0.5 * DOWN))
        ...
        ohms = dict(
            [(n, OldTex("%d\\Omega" % int(n)).scale(0.75)) for n in (1, 2, 3, 4, 6, 12, 1.1, 5, 10.1, 10, 8, 2.1)]
        )
