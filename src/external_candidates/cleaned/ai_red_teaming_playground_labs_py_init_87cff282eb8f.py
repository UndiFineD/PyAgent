# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\ai_red_teaming_playground_labs.py\docker.py\ctfd.py\utils.py\challenges.py\init_87cff282eb8f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\docker\ctfd\utils\challenges\__init__.py
...
Challenge = namedtuple("Challenge", ["id", "type", "name", "value", "category", "tags", "requirements"])
...
def get_all_challenges(admin=False, field=None, q=None, **query_args):
    ...
    chal_q = chal_q.filter_by(**query_args).filter(*filters).order_by(Challenges.name)
...
def get_solve_ids_for_user_id(user_id):
    ...
    solve_ids = Solves.query.with_entities(Solves.challenge_id).filter(Solves.account_id == user.account_id).all()
