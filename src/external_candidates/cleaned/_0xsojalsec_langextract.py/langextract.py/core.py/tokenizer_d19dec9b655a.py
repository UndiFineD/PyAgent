# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\core\tokenizer.py
...
_TOKEN_PATTERN = re.compile(rf"{_SLASH_ABBREV_PATTERN}|{_LETTERS_PATTERN}|{_DIGITS_PATTERN}|{_SYMBOLS_PATTERN}")
...
def tokens_text(
    ...
    return tokenized_text.text[start_token.char_interval.start_pos : end_token.char_interval.end_pos]
...
def _is_end_of_sentence_token(
    ...
    """
    current_token_text = text[tokens[current_idx].char_interval.start_pos : tokens[current_idx].char_interval.end_pos]
...
def _is_sentence_break_after_newline(
    ...
    gap_text = text[tokens[current_idx].char_interval.end_pos : tokens[current_idx + 1].char_interval.start_pos]
    if "\n" not in gap_text:
        return False
...
def find_sentence_range(
    ...
    if start_token_index < 0 or start_token_index >= len(tokens):
        raise SentenceRangeError(f"start_token_index={start_token_index} out of range. Total tokens: {len(tokens)}.")
