# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\core\debug_utils.py
...
def debug_log_calls(fn: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.exception("[%s] EXCEPTION: %s (%.1f ms)", mod, fn_qual, dur_ms, stacklevel=2)
            raise
...
def configure_debug_logging() -> None:
    ...
    our_handler_exists = any(
        isinstance(h, logging.StreamHandler) and getattr(h, "langextract_debug", False) for h in logger.handlers
    )
