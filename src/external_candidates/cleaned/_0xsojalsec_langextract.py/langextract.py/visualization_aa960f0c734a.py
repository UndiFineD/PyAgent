# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\visualization.py
...
def _build_legend_html(color_map: dict[str, str]) -> str:
    ...
    for extraction_class, colour in color_map.items():
        legend_items.append(
            f'<span class="lx-label" style="background-color:{colour};">{html.escape(extraction_class)}</span>'
        )
...
def _format_attributes(attributes: dict | None) -> str:
    ...
    valid_attrs = {key: value for key, value in attributes.items() if value not in (None, "", "null")}
...
def _prepare_extraction_data(
    ...
    for i, extraction in enumerate(extractions):
        ...
        assert extraction.char_interval is not None, "char_interval must be non-None for valid extractions"
        assert extraction.char_interval.start_pos is not None, "start_pos must be non-None for valid extractions"
        assert extraction.char_interval.end_pos is not None, "end_pos must be non-None for valid extractions"
        ...
        attributes_html = f"<div><strong>class:</strong> {html.escape(extraction.extraction_class)}</div>"
        attributes_html += f"<div><strong>attributes:</strong> {_format_attributes(extraction.attributes)}</div>"
...
def visualize(
    ...
    *,
    ...
    if not valid_extractions:
        empty_html = '<div class="lx-animated-wrapper"><p>No valid extractions to animate.</p></div>'
