# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\providers\__init__.py
...
from langextract.providers.router import register, register_lazy
...
def load_plugins_once() -> None:
    ...
    try:
        ...
        if hasattr(eps, "select"):
            ...
            provider_eps = [ep for ep in eps if getattr(ep, "group", None) == "langextract.providers"]
        ...
        for entry_point in provider_eps:
            try:
                ...
                if hasattr(provider_class, "get_model_patterns"):
                    ...
                    logging.info("Registered %d patterns for %s", len(patterns), entry_point.name)
