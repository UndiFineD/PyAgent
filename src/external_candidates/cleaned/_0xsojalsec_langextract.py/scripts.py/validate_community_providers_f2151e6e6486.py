# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\scripts\validate_community_providers.py
...
def validate(filepath: Path) -> bool:
    ...
    for i in range(start + 2, end):
        ...
        if not re.fullmatch(PYPI_NORMALIZED, pypi):
            ...
            errors.append(f"Line {i + 1}: PyPI package should start with `langextract-` for discoverability.")
        ...
        if not re.fullmatch(GH_REPO_LINK, repo):
            errors.append(f"Line {i + 1}: GitHub Repo must be a Markdown link to a GitHub repository.")
    ...
    if len(rows) == 1 and rows[0]["plugin"].lower().startswith("example"):
        warnings.append("The registry currently contains only the example row. Add real providers above the marker.")
