# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\tests\chunking_test.py
...
class SentenceIterTest(absltest.TestCase):
    def test_basic(self):
        ...
        self.assertEqual(tokenizer.TokenInterval(start_index=0, end_index=5), sentence_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, sentence_interval),
            "This is a sentence.",
        )
        sentence_interval = next(sentence_iter)
        self.assertEqual(tokenizer.TokenInterval(start_index=5, end_index=11), sentence_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, sentence_interval),
            "This is a longer sentence.",
        )
        sentence_interval = next(sentence_iter)
        self.assertEqual(tokenizer.TokenInterval(start_index=11, end_index=17), sentence_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, sentence_interval),
            "Mr. Bond\nasks\nwhy?",
        )
...
class ChunkIteratorTest(absltest.TestCase):
    def test_multi_sentence_chunk(self):
        ...
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=11, end_index=17), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "Mr. Bond\nasks\nwhy?",
        )
    ...
    def test_sentence_with_multiple_newlines_and_right_interval(self):
        ...
        tokenized_text = tokenizer.tokenize(text)
        chunk_interval = tokenizer.TokenInterval(start_index=0, end_index=len(tokenized_text.tokens))
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            text,
        )
    ...
    def test_long_token_gets_own_chunk(self):
        ...
        self.assertEqual(chunking.get_token_interval_text(tokenized_text, chunk_interval), "a")
        chunk_interval = next(chunk_iter).token_interval
        ...
        self.assertEqual(chunking.get_token_interval_text(tokenized_text, chunk_interval), ".")
        for _ in range(9):
            next(chunk_iter)
        with self.assertRaises(StopIteration):
            next(chunk_iter)
    ...
    def test_newline_at_chunk_boundary_does_not_create_empty_interval(self):
        ...
        actual_intervals = [(chunk.token_interval.start_index, chunk.token_interval.end_index) for chunk in chunks]
    ...
    77 y o woman in NAD with a h/o CAD, DM2, asthma and HTN on altace.""")
        ...
        self.assertEqual(chunking.get_token_interval_text(tokenized_text, chunk_interval), text)
        ...
        (
            ...
            [
                [
                    ...
                    chunking.TextChunk(
                        token_interval=tokenizer.TokenInterval(start_index=8, end_index=12),
                        document=_SAMPLE_DOCUMENT,
                    ),
                    chunking.TextChunk(
                        token_interval=tokenizer.TokenInterval(start_index=12, end_index=17),
                        document=_SAMPLE_DOCUMENT,
                    ),
                    chunking.TextChunk(
                        token_interval=tokenizer.TokenInterval(start_index=17, end_index=20),
                        document=_SAMPLE_DOCUMENT,
                    ),
                ]
            ],
...
class TextAdditionalContextTest(absltest.TestCase):
    ...
    def test_text_chunk_additional_context(self):
        ...
        chunk_iter = chunking.ChunkIterator(text=document.tokenized_text, max_char_buffer=100, document=document)
        text_chunk = next(chunk_iter)
