# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadtx\roadtools\roadtx\main.py
import argparse
import base64
import binascii
import codecs
...
def main():
    ...
    clienthelptext = (
        "Client ID (application ID) to use when authenticating. Accepts aliases (list with roadtx listaliases)"
    )
    ...
    codeauth_parser.add_argument("-t", "--tenant", action="store", help="Tenant ID or domain to auth to")
    codeauth_parser.add_argument(
        "--tokenfile",
        action="store",
        help="File to store the credentials (default: .roadtools_auth)",
    ...
    codeauth_parser.add_argument("code", action="store", help="Code to auth with that you got from Azure AD")
    ...
    intauth_parser.add_argument("-t", "--tenant", action="store", help="Tenant ID or domain to auth to")
    intauth_parser.add_argument(
        "--tokenfile",
        action="store",
        help="File to store the credentials (default: .roadtools_auth)",
    ...
    kdbauth_parser.add_argument("-t", "--tenant", action="store", help="Tenant ID or domain to auth to")
    kdbauth_parser.add_argument(
        "--tokenfile",
        action="store",
        help="File to store the credentials (default: .roadtools_auth)",
    ...
    browserprtauth_parser.add_argument("--prt", action="store", help="Primary Refresh Token")
    browserprtauth_parser.add_argument(
        "--prt-sessionkey",
        action="store",
        help="Primary Refresh Token session key (as hex key)",
    ...
    injauth_parser.add_argument("-t", "--tenant", action="store", help="Tenant ID or domain to auth to")
    injauth_parser.add_argument(
        "-d",
        "--driver-path",
        action="store",
    ...
    if args.command in ("auth", "gettokens", "gettoken"):
        ...
        if (
            args.keepass
            and args.username
            and (args.keepass_password or "KPPASS" in os.environ or args.keepass.endswith(".xml"))
        ):
            password, otpseed = selauth.get_keepass_cred(args.username, args.keepass, args.keepass_password)
        else:
