# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadtx\roadtools\roadtx\selenium.py
import os
import sys
...
class SeleniumAuthentication:
    ...
    def get_service(self, driverpath):
        ...
        if driverpath != "geckodriver" and not os.path.exists(driverpath):
            print(
                "geckodriver not found! Required for selenium operation. Please download from https://github.com/mozilla/geckodriver/releases"
            )
            return False
    ...
    def selenium_login(self, url, identity=None, password=None, otpseed=None, keep=False):
        ...
        if otpseed:
            try:
                els = WebDriverWait(driver, 2).until(
                    lambda d: d.find_element(By.CSS_SELECTOR, '[data-value="PhoneAppOTP"]')
                )
                els.click()
            except TimeoutException:
                pass
    ...
    def selenium_login_with_prt(self, url, identity=None, password=None, otpseed=None, keep=False):
        ...
        def interceptor(request):
            ...
            if request.url.startswith("https://login.microsoftonline.com"):
                if (
                    "/authorize" in request.url
                    or "/login" in request.url
                    or "/kmsi" in request.url
                    or "/reprocess" in request.url
                    or "/resume" in request.url
                ):
                    if "sso_nonce" in request.url:
                        res = urlparse(request.url)
                        params = parse_qs(res.query)
                        cookie = self.auth.create_prt_cookie_kdf_ver_2(
                            self.deviceauth.prt,
                            self.deviceauth.session_key,
                            params["sso_nonce"][0],
                        )
                    else:
                        cookie = self.auth.create_prt_cookie_kdf_ver_2(self.deviceauth.prt, self.deviceauth.session_key)
    ...
    def selenium_enrich_prt(self, url, otpseed=None):
        ...
        if otpseed:
            ...
            try:
                ...
                # No MFA?
                pass
        el = WebDriverWait(driver, 6000).until(
            lambda d: d.find_element(by=By.CSS_SELECTOR, value='form[name="hiddenform"] input[name="code"]')
        )
