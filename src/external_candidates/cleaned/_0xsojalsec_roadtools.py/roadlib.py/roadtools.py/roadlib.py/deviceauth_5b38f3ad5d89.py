# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadlib\roadtools\roadlib\deviceauth.py
import base64
import binascii
import codecs
import datetime
...
class DeviceAuthentication:
    ...
    def register_hybrid_device(
        ...
        signedblob = base64.b64encode(self.privkey.sign(signdata.encode("utf-8"), apadding.PKCS1v15(), hashes.SHA256()))
        ...
        print("Registering device")
        res = requests.put(
            f"https://enterpriseregistration.windows.net/EnrollmentServer/device/{deviceid}?api-version=2.0",
            json=data,
            headers=headers,
            proxies=self.proxies,
            verify=self.verify,
    ...
    def delete_device(self, certpath, keypath):
        ...
        if not deviceid:
            return
        res = requests.delete(
            f"https://enterpriseregistration.windows.net/EnrollmentServer/device/{deviceid}",
    ...
    def request_token_with_devicecert_signed_payload(self, payload):
        ...
        headers = {"x5c": certbytes.decode("utf-8"), "kdf_ver": 2}
