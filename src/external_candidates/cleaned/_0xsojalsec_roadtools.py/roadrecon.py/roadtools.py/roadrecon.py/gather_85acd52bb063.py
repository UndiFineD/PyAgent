...
class DataDumper(object):
    ...
    async def dump_l_to_db(self, url, method, mapping, linkname, childtbl, parent):
        ...
        if str(parent.__table__) == "Groups":
            groupcounter += 1
            print(
                "Done processing {0}/{1} groups".format(int(groupcounter / 2), totalgroups),
                end="\r",
            )
    ...
    async def dump_mfa(self, objecttype, parenttbl, method=None):
        ...
        i = 0
        for (parentid,) in parents:
            url = "https://graph.windows.net/%s/%s/%s?api-version=%s&$select=strongAuthenticationDetail,objectId" % (
                self.tenantid,
                objecttype,
                parentid,
                self.api_version,
    ...
    async def dump_keycredentials(self, objecttype, dbtype, method=None):
        if method is None:
            method = self.ahsession.get
        cache = []
        url = "https://graph.windows.net/%s/%s?api-version=1.61-internal&$select=keyCredentials,objectId" % (
            self.tenantid,
            objecttype,
        )
        async for obj in dumphelper(url, method=method):
    ...
    async def dump_custom_role_members(self, dbtype):
        ...
        jobs = []
        for parent in parents:
            url = "https://graph.windows.net/%s/roleAssignments?api-version=%s&$filter=roleDefinitionId eq '%s'" % (
                self.tenantid,
                self.api_version,
                parent.objectId,
            )
...
async def run(args):
    ...
    if totalgroups > MAX_GROUPS:
        ...
        async with aiohttp.ClientSession() as ahsession:
            ...
            tasks.append(
                dumper.dump_links_with_queue(queue, "groups", "owners", Group, mapping=group_owner_link_mapping)
            )
