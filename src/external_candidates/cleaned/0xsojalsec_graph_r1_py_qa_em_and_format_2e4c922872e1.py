# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graph_r1.py\verl.py\utils.py\reward_score.py\qa_em_and_format_2e4c922872e1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\utils\reward_score\qa_em_and_format.py
...
def compute_score_format(solution_str):
    ...
    try:
        ...
        for i, assistant_block in enumerate(assistant_blocks[:-1]):
            if (
                assistant_block.count("<think>") == 1
                and assistant_block.count("</think>") == 1
                and assistant_block.count("<query>") == 1
                and assistant_block.count("</query>") == 1
            ):
