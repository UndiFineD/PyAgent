# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\agents.py\test_base_agent_complex_methods_9161f0375c45.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\agents\test_base_agent_complex_methods.py
...
class TestBaseAgentComplexMethods:
    ...
    def test_split_messages_for_protected_summarization_basic(self, agent):
        ...
        to_summarize_short, protected_short = agent.split_messages_for_protected_summarization(short_messages)
    ...
    def test_summarize_messages_with_mock(self, agent):
        ...
        with patch("code_puppy.agents.base_agent.run_summarization_sync") as mock_summarize:
            mock_summarize.return_value = [ModelResponse(parts=[TextPart(content="Mock summary")])]

            compacted, summarized = agent.summarize_messages(messages, with_protection=True)
    ...
    def test_summarize_messages_without_protection(self, agent):
        ...
        with patch("code_puppy.agents.base_agent.run_summarization_sync") as mock_summarize:
            ...
            compacted, summarized = agent.summarize_messages(messages, with_protection=False)
    ...
    def test_message_history_processor_with_many_messages(self, agent, mock_run_context):
