# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\tools.py\base_655669bbe112.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\base.py
...
class ToolDefinition:
    ...
    def to_deepgram_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameters": {
                "type": "object",
                "properties": {p.name: p.to_dict(include_default=False) for p in self.parameters},
        ...
        for param in self.definition.parameters:
            ...
            if param.enum and param.name in parameters:
                if parameters[param.name] not in param.enum:
                    raise ValueError(f"Invalid value for {param.name}. Must be one of: {', '.join(param.enum)}")
