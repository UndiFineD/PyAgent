# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\tests.py\test_episodic_memory_milvus_repository_1f320ca8efe8.py
...
async def test_vector_search():
    ...
    try:
        ...
        test_data = [
            {
                ...
                "vector": [x + 0.1 * np.random.randn() for x in base_vector],  # Similar vector
            ...
            },
            {
                ...
                "vector": [x + 0.2 * np.random.randn() for x in base_vector],  # Similar vector
                "title": "Team Activity",
                "group_id": test_group_id,
                "event_type": "Activity",
        ...
        results = await repo.vector_search(query_vector=base_vector, user_id=test_user_id, limit=10)
        ...
        user_results = await repo.vector_search(query_vector=base_vector, user_id=test_user_id, limit=10)

        assert len(user_results) >= 2, f"Should find at least 2 user records, actually found {len(user_results)}"
        ...
        assert len(group_results) >= 1, f"Should find at least 1 group record, actually found {len(group_results)}"
...
async def test_delete_operations():
    ...
    try:
        ...
        deleted_count = await repo.delete_by_filters(user_id=test_user_id, group_id=test_group_id)

        assert deleted_count >= 2, f"Should delete at least 2 records with group_id, actually deleted {deleted_count}"
...
async def test_performance():
    ...
    try:
        ...
        for i in range(num_searches):
            ...
            results = await repo.vector_search(query_vector=query_vector, user_id=test_user_id, limit=10)
