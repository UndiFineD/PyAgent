# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\DocOwl2\evaluation\dude_eval.py
import argparse
import json
import logging
import os
        ...
        if q_id in question_ids_to_exclude:
            ...
            if "list" in gtObject["answer_type"]:
                ...
                question_result = get_NLS(
                    ...
                    gtObject["answers"],
                    detObject["answers"],
                    evaluationParams["anls_threshold"],
                )
    ...
    if show_answer_page_position:
        ...
        for answer_type in answer_types.values():
            print(
                "{:10s}\t{:1.4f}".format(
                    answer_type,
                    results["scores_by_types"]["anls_per_answer_type"][answer_type],
                )
            )
...
if __name__ == "__main__":
    postprocess_llm_vqa(
        dataset_name="DUDE",
        split="val",
        llm_pred_path="/nas-alinlp/anwenhu/DocSFTv2/DUDE/val.jsonl",
        save_results=True,
    )
