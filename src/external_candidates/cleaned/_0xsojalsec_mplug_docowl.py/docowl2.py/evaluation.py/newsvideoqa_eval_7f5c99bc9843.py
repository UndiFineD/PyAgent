...
def evaluate_method(gtJson, submJson, anls_threshold=0.5):
    ...
    for gtObject in gtJson["data"]:
        ...
        if q_id in question_ids_to_exclude:
            ...
            question_result = get_NLS([gtObject["answer"]], detObject["answers"], anls_threshold)
    ...
    methodMetrics = {
        "anls": (0 if len(gtJson["data"]) == 0 else totalANLS / (len(gtJson["data"]) - len(question_ids_to_exclude)))
    }
