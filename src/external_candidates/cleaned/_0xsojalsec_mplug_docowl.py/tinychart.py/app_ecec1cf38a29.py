# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\app.py
import argparse
import base64
import hashlib
import json
...
from tinychart.eval.eval_metric import evaluate_cmds, parse_model_output
...
def get_response(params):
        ...
        if len(images) > 0:
            ...
            if hasattr(model.get_vision_tower().config, "tome_r"):
                num_image_tokens = (
                    ...
                    - 26 * model.get_vision_tower().config.tome_r
                )
            else:
    ...
    if (
        "<step>" in generated_text
        and "</step>" in generated_text
        and "<comment>" in generated_text
        and "</comment>" in generated_text
    ):
...
def build_demo():
    ...
    with gr.Blocks(title="TinyChart", theme=gr.themes.Default(), css=block_css) as demo:
        ...
        regenerate_btn.click(
            ...
            queue=False,
        ).then(http_bot, [state, temperature, top_p, max_output_tokens], [state, chatbot])

        clear_btn.click(clear_history, None, [state, chatbot, textbox, imagebox], queue=False)
