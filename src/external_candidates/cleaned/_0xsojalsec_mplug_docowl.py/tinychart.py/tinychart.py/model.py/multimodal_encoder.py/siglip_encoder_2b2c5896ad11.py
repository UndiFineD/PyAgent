# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\tinychart\model\multimodal_encoder\siglip_encoder.py
"""
...
from tinychart.model.multimodal_encoder.merge import (
...
class SigLipEncoderLayerToMe(nn.Module):
    ...
    def forward(
        ...
        if self.r > 0:
            merge, unmerge = bipartite_soft_matching(metric, r=self.r, class_token=False, distill_token=False)
            if trace_source:
...
class SigLipEncoderToMe(nn.Module):
    ...
    def __init__(self, config: SigLipVisionConfig):
        ...
        self.layers = nn.ModuleList(
            [SigLipEncoderLayerToMe(config, layer_id=layer_id) for layer_id in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
