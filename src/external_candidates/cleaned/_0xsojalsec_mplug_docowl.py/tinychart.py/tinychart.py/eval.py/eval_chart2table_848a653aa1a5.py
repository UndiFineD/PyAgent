# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\tinychart\eval\eval_chart2table.py
# coding=utf-8
...
def _get_datapoint_metric(
    ...
    if pred_float is not None and target_float:
        return key_metric * (1 - _get_relative_distance(target_float, pred_float, number_theta))
...
def _table_datapoints_precision_recall_f1(
    ...
    for t, _ in target_datapoints:
        distance.append([1 - anls_metric(t, p, text_theta) for p, _ in prediction_datapoints])
    cost_matrix = np.array(distance)
    row_ind, col_ind = optimize.linear_sum_assignment(cost_matrix)
    score = 0
    for r, c in zip(row_ind, col_ind):
        score += _get_datapoint_metric(target_datapoints[r], prediction_datapoints[c], text_theta, number_theta)
    if score == 0:
        return 0, 0, 0
...
def table_datapoints_precision_recall(
    ...
    """
    score_dict = table_datapoints_precision_recall_per_point(targets, predictions, text_theta, number_theta)
    return {
        "table_datapoints_precision": (100.0 * sum(score_dict["precision"]) / len(targets)),
...
def _get_row_metric(
    ...
    for target, prediction in zip(target_parts, prediction_parts):
        ...
        if target == prediction:
            ...
            result.append(1 - _get_relative_distance(target_float, pred_float, number_theta))
        elif target_float is not None:
            result.append(0.0)
        else:
            result.append(anls_metric(target, prediction, text_theta))
...
def _row_datapoints_precision_recall_f1(
    ...
    aligned_prediction, aligned_score = prediction.aligned(target.headers, text_theta)
    ...
    for t in target_datapoints:
        metrics.append([aligned_score * _get_row_metric(t, p, text_theta, number_theta) for p in prediction_datapoints])
...
def row_datapoints_precision_recall(
    ...
    if len(targets) != len(predictions):
        raise ValueError(f"Targets has length {len(targets)} and predictions has length {len(predictions)}.")
    precision, recall, f1 = 0, 0, 0
    for pred, target in zip(predictions, targets):
        all_metrics = []
        prediction_tables = [_parse_table(pred, transposed=transposed) for transposed in [True, False]]
        for t in target:
