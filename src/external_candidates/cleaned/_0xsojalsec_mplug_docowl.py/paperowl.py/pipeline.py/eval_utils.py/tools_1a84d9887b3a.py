# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\PaperOwl\pipeline\eval_utils\tools.py
import collections
import copy
import itertools
import json
...
def eval_due(dataset_name, pred_path, gt_path):
    ...
    for metric in metrics:
        evaluator = DueEvaluator(reference=gts, answers=preds, ignore_case=True, metric=metric)
...
def cider(targets: Sequence[Sequence[str]], predictions: Sequence[str]) -> float:
    ...
    score, scores = scorer.compute_score(
        gts=coco_tokenizer.tokenize({str(i): [{"caption": t} for t in target] for i, target in enumerate(targets)}),
        res=coco_tokenizer.tokenize({str(i): [{"caption": prediction}] for i, prediction in enumerate(predictions)}),
    )
...
def bleu(ngram: int, targets: Sequence[Sequence[str]], predictions: Sequence[str]) -> float:
    """Compute Bleu score."""
    assert ngram <= 4
    coco_tokenizer = PTBTokenizer()
...
def doc_evaluate(metric: str, targets: Sequence[Sequence[str]], predictions: Sequence[str]):
    ...
    if save_each_eval:
        ...
        for i in range(len(imgs)):
            ...
            eval_result.append(
                {
                    "metric2score": [
                        {"metric": metric, "score": scores[i]} for metric, scores in metric2scores.items()
                    ],
                    "image": imgs[i],
                    "question": ques[i],
                    "gt": gts[i][0],
