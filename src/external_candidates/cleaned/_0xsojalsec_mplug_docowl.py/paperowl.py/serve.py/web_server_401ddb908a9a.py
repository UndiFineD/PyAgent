# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\PaperOwl\serve\web_server.py
import argparse
import datetime
import json
import os
    ...
    with gr.Blocks(title="mPLUG-DocOwl", css=css) as demo:
        ...
        if local_example:
            ...
            examples = [
                ...
                [
                    f"examples/table.jpg",
                    "What programming languages does the tokenizers supports?",
                ],
                ...
                [
                    f"examples/docowl.jpg",
                    "Give me an detail introduction about this paper.",
                ],
            ]
        ...
        upvote_btn.click(upvote_last_response, [state], [textbox, upvote_btn, downvote_btn, flag_btn])
        downvote_btn.click(
            downvote_last_response,
            [state],
        ...
        regenerate_btn.click(
            partial(regenerate_http_bot, model=model),
        ...
        textbox.submit(
            partial(partial(add_text_http_bot, model=model), model=model),
            [state, textbox, imagebox] + parameter_list,
            [state, chatbot, textbox, imagebox] + btn_list,
        )

        submit_btn.click(
            partial(add_text_http_bot, model=model),
            [state, textbox, imagebox] + parameter_list,
            [state, chatbot, textbox, imagebox] + btn_list,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mm_config",
        type=str,
        default="configs/mmsft/data_v2.4.model_v0_new_anchor_full.yaml",
    )
