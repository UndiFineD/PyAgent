# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\cache.py\work.py\copilot_tmp.py\chunk_0_colorpp_ff8c3602900b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\cache\work\copilot_tmp\chunk_0_colorpp.py

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\colorpp\colorpp.py
...
def pretty_convert(obj):
    if isinstance(obj, (float)):
        ...
        return "{1}{2}{0}".format(color.ENDC, color.CYAN, obj)

    elif isinstance(obj, (int, complex)):
        return "{1}{2}{0}".format(color.ENDC, color.BLUE, obj)

    elif isinstance(obj, (str)):
        return '{1}"{0}{2}{3}{0}{1}"{0}'.format(color.ENDC, color.REDL, color.RED, obj.encode())

    elif isinstance(obj, (dict)):
        return dict(("{1}{2}:{0}".format(color.ENDC, color.YELLOW, k), pretty_convert(v)) for k, v in obj.items())
        ...
        return dict(
            ("{1}{2}:{0}".format(color.ENDC, color.YELLOW, k), pretty_convert(v)) for k, v in obj.__dict__.items()
        )
...
def walk_obj(buf, obj, indent_num=2, depth=0, eol="", wrap_len=60, wrap_total=100, lf="\n"):
    ...
    if isinstance(obj, (str, int, float, complex)):
        buf.write("{0}".format(obj).replace(lf, "{0}{1} ".format(lf, indent)))
        ...
        for key in sorted(obj.keys()):
            buf.write("{0}{1}{2}{3} ".format(eol, indent, " " * indent_num, key))
