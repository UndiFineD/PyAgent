...
def get_pe_info(filepath):
    ...
    try:
        ...
        info = {
            "file_type": (
                "PE32+ executable" if pe.PE_TYPE == pefile.OPTIONAL_HEADER_MAGIC_PE_PLUS else "PE32 executable"
            ),
...
def register_routes(app):
    ...
    def cleanup():
        try:
            ...
            return (
                jsonify({"status": "error", "message": "Cleanup failed", "error": str(e)}),
                500,
            )
        ...
        try:
            ...
            health_status = {
                ...
                "upload_folder_accessible": (
                    os.path.isdir(upload_folder) and os.access(upload_folder, os.W_OK) if upload_folder else False
                ),
                "issues": issues,
                "configuration": {
                    "static_analysis": {
                        tool: static_config.get(tool, {}).get("enabled", False) for tool in ["yara", "threatcheck"]
                    },
                    "dynamic_analysis": {
