# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\retriever.py\relationshipretriever_f98f982b5ca1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Retriever\RelationshipRetriever.py
...
class RelationshipRetriever(BaseRetriever):
    ...
    async def _find_relevant_relationships_from_entities(self, seed: list[dict]):
        all_related_edges = await asyncio.gather(*[self.graph.get_node_edges(node["entity_name"]) for node in seed])
        ...
        all_edges_pack = await asyncio.gather(*[self.graph.get_edge(e[0], e[1]) for e in all_edges])

        all_edges_degree = await asyncio.gather(*[self.graph.edge_degree(e[0], e[1]) for e in all_edges])
    ...
    async def _find_relevant_relations_by_entity_agent(
        self, query: str, entity: str, pre_relations_name=None, pre_head=None, width=3
    ):
        """
        ...
        try:
            ...
            prompt = (
                ...
                + "\nTopic Entity: "
                + entity
                + "\nRelations: There are %s relations provided in total, seperated by ;." % str(len(total_relations))
