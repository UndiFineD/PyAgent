# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\hughpyle.py\keep.py\later.py\tagging_deb51bedf253.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hughpyle\keep\later\tagging.py
...
LANGUAGE_PATTERNS = {
    ...
    "rust": [
        r"^\s*(fn|let|mut|impl|struct|enum)\s",
        r"->\s*\w+",
        r"&mut\s",
        r"unwrap\(\)",
    ],
...
class KeywordTagger:
    ...
    def _compile_patterns(self):
        ...
        self._language_patterns = {
            lang: [re.compile(p, re.MULTILINE) for p in patterns] for lang, patterns in LANGUAGE_PATTERNS.items()
        }
    ...
    def tag(self, content: str, *, content_type_hint: str | None = None) -> dict[str, str]:
        ...
        if content_type == "code" or (
            content_type_hint
            and any(content_type_hint.endswith(ext) for ext in CONTENT_TYPES.get("code", {}).get("extensions", []))
        ):
