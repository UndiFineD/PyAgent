# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_malwoverview.py\malwoverview.py\modules.py\polyswarm_9f04115f47d1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-malwoverview\malwoverview\modules\polyswarm.py
    ...
    def requestPOLYAPI(self):
        ...
        if self.POLYAPI == "":
            print(
                ...
                + "\nTo be able to get/submit information from/to Polyswarm, you must create the .malwapi.conf file under your user home directory (on Linux is $HOME\\.malwapi.conf and on Windows is in C:\\Users\\[username]\\.malwapi.conf) and insert the Polyswarm API according to the format shown on the Github website."
                + mycolors.reset
                + "\n"
            )
        ...
        try:
            if metainfo == 4:
                ...
                for x in metaresults:
                    if cv.bkg == 1:
                        if x.sha256:
                            print(
                                mycolors.reset + "\nSHA256: " + mycolors.foreground.lightred + "%s" % x.sha256,
                                end=" ",
                            )

                        else:
                            print(
                                mycolors.reset + "\nSHA256: " + mycolors.foreground.lightred + "%s" + "None",
                                end=" ",
                            )

                        if x.md5:
                            print(
                                mycolors.reset + "MD5: " + mycolors.foreground.lightcyan + "%s" % x.md5,
                                end=" ",
                            )

                        else:
                            print(
                                mycolors.reset + "MD5: " + mycolors.foreground.lightcyan + "%s" + "None",
                                end=" ",
                            )
                        ...
                        if x.sha256:
                            print(
                                mycolors.reset + "\nSHA256: " + mycolors.foreground.red + "%s" % x.sha256,
                                end=" ",
                            )

                        else:
                            print(
                                mycolors.reset + "\nSHA256: " + mycolors.foreground.red + "%s" + "None",
                                end=" ",
                            )

                        if x.md5:
                            print(
                                mycolors.reset + "MD5: " + mycolors.foreground.green + "%s" % x.md5,
                                end=" ",
                            )

                        else:
                            print(
                                mycolors.reset + "MD5: " + mycolors.foreground.green + "%s" + "None",
                                end=" ",
                            )
            ...
            for y in metaresults:
                if cv.bkg == 1:
                    if y.sha256:
                        print(
                            mycolors.reset + "\nSHA256: " + mycolors.foreground.lightcyan + "%s" % y.sha256,
                            end=" ",
                        )

                    else:
                        print(
                            mycolors.reset + "Result: " + mycolors.foreground.yellow + "Sample not found!",
                            end=" ",
                        )
                    ...
                    print(
                        mycolors.reset + "Polyscore: " + mycolors.foreground.yellow + "%20s" % score.polyscore,
                        end=" ",
                    )
                    ...
                    if (str(y.scan.get("detections", {}).get("malicious"))) != "None":
                        print(
                            ...
                            + "%s" % y.scan.get("detections", {}).get("malicious"),
                            end=" ",
                        )

                        print(
                            "/ " + "%2s malicious" % y.scan.get("detections", {}).get("total"),
                            end=" ",
                        )

                    else:
                        print(
                            mycolors.reset + "scan: " + mycolors.foreground.pink + "not scanned yet",
                            end=" ",
                        )
                    ...
                    if y.sha256:
                        print(
                            mycolors.reset + "\nSHA256: " + mycolors.foreground.green + "%s" % y.sha256,
                            end=" ",
                        )

                    else:
                        print(
                            mycolors.reset + "scan: " + mycolors.foreground.purple + "Sample not found!",
                            end=" ",
                        )
                    ...
                    print(
                        mycolors.reset + "Polyscore: " + mycolors.foreground.red + "%20s" % score.polyscore,
                        end=" ",
                    )
                    ...
                    if (str(y.scan.get("detections", {}).get("malicious"))) != "None":
                        ...
                        print(
                            mycolors.reset + "Result: " + mycolors.foreground.purple + "not scanned yet",
                            end=" ",
                        )
    ...
    def polyfile(self, poly):
        ...
        try:
            ...
            for assertion in result.assertions:
                if cv.bkg == 1:
                    print(
                        mycolors.reset + "Engine: " + mycolors.foreground.lightcyan + "%-25s" % assertion.author_name,
                        end="",
                    )

                    print(
                        mycolors.reset + "\tVerdict:" + mycolors.foreground.lightred + " ",
                        "Malicious" if assertion.verdict else "Clean",
                    )

                else:
                    print(
                        mycolors.reset + "Engine: " + mycolors.foreground.green + "%-25s" % assertion.author_name,
                        end="",
                    )

                    print(
                        mycolors.reset + "\tVerdict:" + mycolors.foreground.red + " ",
                        "Malicious" if assertion.verdict else "Clean",
                    )
            ...
            for myhashes in results:
                if not myhashes.assertions:
                    if cv.bkg == 1:
                        print(
                            mycolors.foreground.lightred
                            + "This sample has not been scanned on Polyswarm yet!\n"
                            + mycolors.reset
                        )
                        ...
                        print(
                            mycolors.foreground.red
                            + "This sample has not been scanned on Polyswarmi yet!\n"
                            + mycolors.reset
                        )
