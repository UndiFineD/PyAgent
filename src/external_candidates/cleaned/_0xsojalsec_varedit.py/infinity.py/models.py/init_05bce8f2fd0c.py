# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\models\__init__.py
import torch
...
def _ex_repr(self):
    return ", ".join(
        ...
        if not k.startswith("_") and k != "training" and not isinstance(v, (torch.nn.Module, torch.Tensor))
    )
