...
class Infinity(nn.Module):
    def __init__(
        ...
        if word_patch_size > 1:
            assert all(raw_pn % word_patch_size == 0 for raw_pn in raw_scale_schedule), (
        ...
        self.shared_ada_lin = (
            nn.Sequential(nn.SiLU(inplace=False), SharedAdaLin(self.D, 6 * self.C)) if shared_aln else nn.Identity()
        )
    ...
    def add_lvl_embeding(self, feature, scale_ind, scale_schedule, need_to_pad=0):
        ...
        feature[:, :t_mul_h_mul_w] += self.lvl_embed(
            scale_ind * torch.ones((bs, t_mul_h_mul_w), dtype=torch.int).to(feature.device)
        )
        return feature
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            sos = cond_BD = (
                self.text_proj_for_sos((kv_compact, cu_seqlens_k, max_seqlen_k)).float().contiguous()
            ...
            if self.customized_flash_attn:
                ...
                attn_bias_for_masking = torch.where(d >= dT, 0.0, -torch.inf).reshape(1, 1, l_end, l_end)
...
def sample_with_top_k_top_p_also_inplace_modifying_logits_(
    ...
    if top_k > 0:
        top_k = min(top_k, V)
        idx_to_remove = logits_BlV < logits_BlV.topk(top_k, largest=True, sorted=False, dim=-1)[0].amin(
            dim=-1, keepdim=True
        )
...
def sampling_with_top_k_top_p_also_inplace_modifying_probs_(
    ...
    if top_k > 0:
        top_k = min(top_k, V)
        idx_to_remove = probs_BlV < probs_BlV.topk(top_k, largest=True, sorted=False, dim=-1)[0].amin(
            dim=-1, keepdim=True
        )
