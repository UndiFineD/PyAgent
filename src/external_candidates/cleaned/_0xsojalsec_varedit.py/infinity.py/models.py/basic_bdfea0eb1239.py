# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\models\basic.py
"""
...
def precompute_rope2d_freqs_grid(
    ...
    half_dim = dim // 2
    inv_freq = 1.0 / (
        base ** (torch.arange(0, half_dim, 2, dtype=torch.int64).float().to(device) / half_dim)
    )  # namely theta, 1 / (10000^(i/half_dim)), i=0,2,..., half_dim-2
    ...
    freqs_height = torch.outer(
        t_height, inv_freq
    )  # (max_height, dim / (1 for 1d, 2 for 2d, 3 for 3d) / 2), namely y*theta
    t_width = t_width / scaling_factor
    freqs_width = torch.outer(
        t_width, inv_freq
    )  # (max_width, dim / (1 for 1d, 2 for 2d, 3 for 3d) / 2), namely x*theta
    freqs_grid_map = torch.concat(
        [
            freqs_height[:, None, :].expand(
                -1, max_width * 2, -1
            ),  # (max_height, max_width, dim / (1 for 1d, 2 for 2d, 3 for 3d) / 2)
            freqs_width[None, :, :].expand(
                max_height * 2, -1, -1
            ),  # (max_height, max_width, dim / (1 for 1d, 2 for 2d, 3 for 3d) / 2)
        ],
        dim=-1,
    )  # (max_height, max_width, dim / (1 for 1d, 2 for 2d, 3 for 3d))
...
def apply_rotary_emb(
    ...
    with torch.autocast(device_type=device_type, enabled=False):
        ...
        qk = torch.stack(
            ...
            dim=-1,
        )  # (2, batch_size, heads, seq_len, half_head_dim, 2), here stack + reshape should not be concate
...
class FFNSwiGLU(nn.Module):
    def __init__(self, in_features, hidden_features, out_features=None, drop=0.0, fused_mlp=False):
        super().__init__()
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.caching:  # kv caching: only used during inference
            if start_layer:
                if self.cached_init_k is None:
                    self.cached_init_k = k
                    self.cached_init_v = v
                else:
                    k_src = down_func(
                        self.cached_init_k,
                        scale_schedule[-1],
                        scale_schedule[scale_ind],
                    )
                    v_src = down_func(
                        self.cached_init_v,
                        scale_schedule[-1],
                    ...
                    k = torch.cat([k_src, k], dim=L_dim)
                    v = torch.cat([v_src, v], dim=L_dim)
            else:
                if self.cached_k is None:
                    self.cached_k = k
                    self.cached_v = v
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        ).view(N, 2, self.num_heads, self.head_dim)  # NC => N2Hc
...
class SelfAttnBlock(nn.Module):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(
        self, x, cond_BD, ca_kv, attn_bias_or_two_vector
    ):  # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_ada_norm is None:
            ...
            x = x + self.drop_path(
                self.ffn(self.ln_wo_grad(x.float()).mul(scale2.add(1)).add_(shift2)).mul(gamma2)
            )  # this mul(gamma2) cannot be in-placed cuz we possibly use FusedMLP
        else:
            x = x + self.drop_path(
                self.attn(
                    self.fused_ada_norm(C=self.C, eps=self.norm_eps, x=x, scale=scale1, shift=shift1),
                    attn_bias_or_two_vector=attn_bias_or_two_vector,
                ).mul_(gamma1)
            )
            x = x + self.drop_path(
                self.ffn(self.fused_ada_norm(C=self.C, eps=self.norm_eps, x=x, scale=scale2, shift=shift2)).mul(gamma2)
            )  # this mul(gamma2) cannot be in-placed cuz we possibly use FusedMLP
        return x
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(
        ...
        if self.fused_norm_func is None:
            ...
            x = x + self.ca(self.ca_norm(x), ca_kv).float().mul_(self.ca_gamma)
            x = x + self.drop_path(
                self.ffn(self.fused_norm_func(C=self.C, eps=self.norm_eps, x=x, scale=scale2, shift=shift2)).mul(gamma2)
            )  # this mul(gamma2) cannot be in-placed cuz we possibly use FusedMLP
        return x
