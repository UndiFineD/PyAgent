# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\storages.py\db_stores.py\sqlite_e433718d1cd5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\storages\db_stores\sqlite.py
...
class SQLite(DBStoreBase):
    ...
    def insert_memory(
        ...
        store_type: Optional[Literal["memory", "agent", "workflow", "history", "indexing"]],
        ...
        with self._lock:
            with self.connection:
                ...
                self.connection.execute(
                    ...
                    tuple(
                        [
                            json.dumps(meta) if not isinstance(meta, str) else meta
    ...
    def update(
        ...
        store_type: Optional[Literal["memory", "agent", "workflow", "history", "indexing"]] = None,
        table: Optional[str] = None,
        *args,
        **kwargs,
    ):
        ...
        with self._lock:
            with self.connection:
                ...
                values = list(
                    [json.dumps(v) if not isinstance(v, str) else v for v in new_metadata.model_dump().values()]
                )[1:] + [metadata_id]
