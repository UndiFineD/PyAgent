# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_lumina_dimoo.py\generators.py\text_understanding_generator_cd6cfb9bae62.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Lumina-DiMOO\generators\text_understanding_generator.py
...
def generate_text_understanding(
    ...
    for num_block in range(num_blocks):
        block_mask_index = (
            x[
                :,
                code_start + num_block * block_length : code_start + (num_block + 1) * block_length :,
            ]
            == mask_id
        )
        ...
        if (x == 126081).sum().item() > 0 and num_blocks > 0:
            return x[
                :,
                : input_p_len + prompt.shape[0] + 3 + rows + (num_block + 1) * block_length,
            ]
