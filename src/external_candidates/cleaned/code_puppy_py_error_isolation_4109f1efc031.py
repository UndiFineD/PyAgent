# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\mcp.py\error_isolation_4109f1efc031.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\mcp_\error_isolation.py
...
class MCPErrorIsolator:
    ...
    async def isolated_call(self, server_id: str, func: Callable, *args, **kwargs) -> Any:
        ...
        async with self._lock:
            ...
            if self.is_quarantined(server_id):
                ...
                raise QuarantinedServerError(f"Server {server_id} is quarantined until {quarantine_until}")
    ...
    async def quarantine_server(self, server_id: str, duration: int) -> None:
        ...
        async with self._lock:
            ...
            logger.warning(f"Server {server_id} quarantined for {duration}s (count: {stats.quarantine_count})")
    ...
    def _categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(keyword in error_type for keyword in ["connection", "timeout", "network", "socket", "dns", "ssl"]):
        ...
        if any(keyword in error_message for keyword in ["json", "decode", "parse", "invalid", "malformed", "schema"]):
            return ErrorCategory.PROTOCOL
        ...
        if any(keyword in error_message for keyword in ["429", "rate limit", "too many requests", "throttle"]):
            return ErrorCategory.RATE_LIMIT
    ...
    def _calculate_quarantine_duration(self, quarantine_count: int) -> int:
        ...
        logger.debug(f"Calculated quarantine duration: {duration}s (count: {quarantine_count}, max: {max_seconds}s)")
