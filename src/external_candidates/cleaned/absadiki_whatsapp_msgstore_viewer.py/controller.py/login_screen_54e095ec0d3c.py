# Extracted from: C:\DEV\PyAgent\.external\absadiki-whatsapp-msgstore-viewer\Controller\login_screen.py
import importlib
import os.path
import traceback
...
class LoginScreenController:
    ...
    def check_files(self):
        if self.view.app.msgstore_file is None:
            self.view.show_dialog("No msgstore provided!!", title="Error", auto_dismiss=True)
            return False
...
<https://github.com/ElDavoo/WhatsApp-Crypt14-Crypt15-Decrypter>
    ...
    def on_tap_button_login(self) -> NoReturn:
        ...
        self.view.app.wa_file = (
            None if self.view.ids["wa_file_path"].text == "" else strip_quotes(self.view.ids["wa_file_path"].text)
        )
        self.view.app.wp_dir = (
            None if self.view.ids["wp_dir"].text == "" else strip_quotes(self.view.ids["wp_dir"].text)
        )
