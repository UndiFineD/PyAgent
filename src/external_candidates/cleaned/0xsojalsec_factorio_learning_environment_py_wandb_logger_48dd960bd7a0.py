# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\analysis.py\wandb_logger_48dd960bd7a0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\analysis\wandb_logger.py
...
class WandBLogger:
    ...
    def __init__(
        ...
        self.enabled = WANDB_AVAILABLE and os.getenv("DISABLE_WANDB", "").lower() not in ["true", "1"]
    ...
    def _setup_wandb_auth(self):
        ...
        if wandb_api_key:
            ...
            print("WANDB_API_KEY not found in environment - you may need to authenticate manually")
    ...
    def log_performance_metrics(
        ...
        if "ci_95_lower" in metrics_dict:
            formatted_metrics["stats/success_rate_ci_95_lower"] = metrics_dict["ci_95_lower"]

            formatted_metrics["stats/success_rate_ci_95_upper"] = metrics_dict["ci_95_upper"]
    ...
    def log_sweep_summary(
        ...
        if results_by_model:
            best_model = max(results_by_model.keys(), key=lambda m: results_by_model[m].success_rate)

            sweep_metrics["sweep/best_model"] = best_model

            sweep_metrics["sweep/best_model_success_rate"] = results_by_model[best_model].success_rate

        # Log hardest task (lowest success rate)

        if results_by_task:
            hardest_task = min(results_by_task.keys(), key=lambda t: results_by_task[t].success_rate)

            sweep_metrics["sweep/hardest_task"] = hardest_task

            sweep_metrics["sweep/hardest_task_success_rate"] = results_by_task[hardest_task].success_rate
    ...
    def log_table(self, table_name: str, data: List[Dict[str, Any]], step: Optional[int] = None):
    ...
    def log_model_comparison_table(self, model_metrics: Dict[str, PerformanceMetrics], step: Optional[int] = None):
...
class WandBSweepLogger:
    ...
    def create_run_logger(self, run_id: str, model: str, task: str, version: int, **kwargs) -> WandBLogger:
