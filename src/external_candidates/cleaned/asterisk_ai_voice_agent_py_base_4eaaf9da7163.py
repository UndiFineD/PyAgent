# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\providers.py\base_4eaaf9da7163.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\providers\base.py
...
class ProviderCapabilities:
    ...
    is_full_agent: bool = (
        ...
        False  # True for providers like OpenAI Realtime, Google Live, Deepgram Voice Agent

    )

    has_native_vad: bool = (

        False  # True if provider has built-in Voice Activity Detection

    )

    has_native_barge_in: bool = (

        False  # True if provider handles interruption/barge-in internally

    )

    requires_continuous_audio: bool = (

        False  # True if provider needs continuous audio stream (not VAD-gated)
