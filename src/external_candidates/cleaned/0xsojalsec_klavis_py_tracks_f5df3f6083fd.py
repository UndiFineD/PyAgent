# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\mcp_servers.py\spotify.py\tools.py\tracks_f5df3f6083fd.py
...
def get_tracks_info(track_ids: List[str], sp: Optional[Spotify] = None) -> Union[List[Dict], Dict]:
    """
    ...
    try:
        ...
        return [t for t in (process_track_info(track) for track in tracks if track is not None) if t is not None]
...
def get_user_saved_tracks(sp: Optional[Spotify] = None, limit: int = 20, offset: int = 0) -> Union[List[Dict], Dict]:
    """
...
def save_tracks_for_current_user(track_ids: List[str], sp: Optional[Spotify] = None) -> str:
    """
...
def check_user_saved_tracks(track_ids: List[str], sp: Optional[Spotify] = None) -> Union[List[bool], Dict]:
    """
