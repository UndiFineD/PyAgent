# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videorag.py\vimo_desktop.py\python_backend.py\videorag.py\videorag_efca1d4a5bdd.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoRAG\Vimo-desktop\python_backend\videorag\videorag.py
...
@dataclass
class VideoRAG:
    working_dir: str = field(default_factory=lambda: f"./videorag_cache_{datetime.now().strftime('%Y-%m-%d-%H:%M:%S')}")
    ...
    def __post_init__(self):
        ...
        self.llm_response_cache = (
            self.key_string_value_json_storage_cls(namespace="llm_response_cache", global_config=self.safe_config)
    ...
    def insert_video(self, video_path_list=None, progress_callback=None):
        ...
        for video_path in video_path_list:
            ...
            if progress_callback:
                progress_callback(
                    "Audio Processing",
                    f"Performing speech recognition for {video_name}...",
                )
            ...
            loop.run_until_complete(self.video_segments.upsert({video_name: segments_information}))
            ...
            if progress_callback:
                progress_callback(
                    ...
                    f"Saving video information for {video_name}...",
                )
        ...
        if progress_callback:
            progress_callback(
                ...
                f"Creating knowledge graph for all videos...",
            )
