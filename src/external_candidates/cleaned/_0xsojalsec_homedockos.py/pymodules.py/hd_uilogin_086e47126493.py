...
def count_limited_ips_within_timeframe(timeframe_minutes):
    return len(
        set(ip for ip, ts in limited_ips_added_to_shield if datetime.now() - ts < timedelta(minutes=timeframe_minutes))
    )
...
def login_page():
    ...
    failed_attempts_within_hour = [
        attempt for attempt in failed_attempts[ip_address] if attempt > datetime.now() - timedelta(hours=1)
    ]
...
def api_login():
    ...
    if not token_data or "password" not in token_data:
        return (
            jsonify({"status": "bad_request", "message": "There's no password available."}),
            400,
        )
    ...
    if sanitized_username.lower() == user_name.lower() and bcrypt.checkpw(
        ...
        if remaining_attempts <= 0:
            ...
            limited_ips_added_to_shield = [
                (ip, ts) for ip, ts in limited_ips_added_to_shield if datetime.now() - ts < timedelta(days=1)
            ]
