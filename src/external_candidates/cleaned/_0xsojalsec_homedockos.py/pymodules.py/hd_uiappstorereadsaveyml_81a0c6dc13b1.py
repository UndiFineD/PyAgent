        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "hd_group": hd_group,
                    "hd_role": hd_role,
                    "user_name": (devhook_values["user_name"] if user_placeholder_present else None),
                    "password": (devhook_values["password"] if password_placeholder_present else None),
                    "random_string": (devhook_values["random_string"] if random_string_placeholder_present else None),
                    "ssl_enabled": use_ssl,
                },
            }
        )
...
def process_config():
    ...
    if configType == "advanced":
        ...
        return (
            jsonify({"success": False, "message": "Invalid configuration type provided"}),
            400,
        )
