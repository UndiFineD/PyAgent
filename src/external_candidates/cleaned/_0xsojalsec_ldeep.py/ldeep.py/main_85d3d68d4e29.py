...
class Ldeep(Command):
        ...
        if verbose:
            ...
            for record in records:
                ...
                if "objectClass" not in record:
                    ...
                    "organizationalUnit" in record["objectClass"] or "domain" in record["objectClass"] and extra_records
                ):
                    print(record["dn"])
                    if record["gPLink"]:
                        guids = re_compile("{[^}]+}")
    ...
    def list_users(self, kwargs):
        ...
        if filter_ == "all":
            ...
            results = self.engine.query(
                self.engine.USER_ACCOUNT_CONTROL_FILTER_NEG(USER_ACCOUNT_CONTROL["ACCOUNTDISABLE"]),
                attributes,
            )
        elif filter_ == "disabled":
            results = self.engine.query(
                self.engine.USER_ACCOUNT_CONTROL_FILTER(USER_ACCOUNT_CONTROL["ACCOUNTDISABLE"]),
                attributes,
            )
        elif filter_ == "locked":
            ...
            results = self.engine.query(
                self.engine.USER_ACCOUNT_CONTROL_FILTER(USER_ACCOUNT_CONTROL["DONT_EXPIRE_PASSWORD"]),
                attributes,
            )
        elif filter_ == "passwordexpired":
            results = self.engine.query(
                self.engine.USER_ACCOUNT_CONTROL_FILTER(USER_ACCOUNT_CONTROL["PASSWORD_EXPIRED"]),
                attributes,
            )
        elif filter_ == "nokrbpreauth":
            results = self.engine.query(
                self.engine.USER_ACCOUNT_CONTROL_FILTER(USER_ACCOUNT_CONTROL["DONT_REQ_PREAUTH"]),
                attributes,
            )
        elif filter_ == "reversible":
            results = self.engine.query(
                self.engine.USER_ACCOUNT_CONTROL_FILTER(USER_ACCOUNT_CONTROL["ENCRYPTED_TEXT_PWD_ALLOWED"]),
                attributes,
            )
        else:
            return None
    ...
    def list_zones(self, kwargs):
        ...
        self.display(
            self.engine.query(
                ...
                attributes,
                base=",".join(["CN=MicrosoftDNS,DC=DomainDNSZones", self.engine.base_dn]),
            ),
            verbose,
        )
    ...
    def list_fsmo(self, kwargs):
        ...
        try:
            ...
            self.display(self.engine.query(self.engine.FSMO_DOMAIN_FILTER(), attributes), verbose)
    ...
    def misc_all(self, kwargs):
        ...
        for command, method in self.get_commands(prefix="list_"):
            ...
            if self.has_option(method, "filter"):
                ...
                for f in filter_:
                    sys.stdout = Logger(
                        "{output}_{command}_{filter}.lst".format(output=output, command=command, filter=f),
                        quiet=True,
                    )
                    kwargs["filter"] = f
                    ...
                    if self.has_option(method, "verbose"):
                        ...
                        sys.stdout = Logger(
                            "{output}_{command}_{filter}.json".format(output=output, command=command, filter=f),
                            quiet=True,
                        )
                        kwargs["verbose"] = True
                        getattr(self, method)(kwargs)
                ...
                if self.has_option(method, "verbose"):
                    info("Retrieving {command} verbose output".format(command=command))
                    sys.stdout = Logger(
                        "{output}_{command}.json".format(output=output, command=command),
                        quiet=True,
                    )
                    kwargs["verbose"] = True
                    getattr(self, method)(kwargs)
