# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\tests\test_integration.py
...
async def test_search_strategy(mock_dspy_env):
    ...
    rlm = RLM(model="test-model")
    result = await rlm.acompletion("Find all years", "The years 2020, 2021, and 2022 were important.")
...
async def test_chunk_strategy(mock_dspy_env):
    ...
    rlm = RLM(model="test-model")
    result = await rlm.acompletion("Chunk the context", "A" * 50)  # 50 chars
...
async def test_recursive_processing(mock_dspy_env):
    ...
    rlm = RLM(model="test-model", max_depth=3, _current_depth=0)
...
async def test_two_model_strategy(mock_dspy_env):
    ...
    rlm = RLM(model="expensive-model", recursive_model="cheap-model", max_depth=3)
