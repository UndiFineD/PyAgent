...
def example_basic_tracking():
    ...
    # Simple query
    result = rlm.completion(query="What are the main topics discussed?", context=LONG_DOCUMENT[:2000])
...
def example_with_different_output():
    ...
    result = rlm.completion(query="Extract key insights from the document", context=LONG_DOCUMENT)
...
def example_without_tracking():
    ...
    result = rlm.completion(query="What are the types of machine learning?", context=LONG_DOCUMENT)
...
def example_graph_analysis():
    ...
    result = rlm.completion(query="Analyze the document structure and content", context=LONG_DOCUMENT)
