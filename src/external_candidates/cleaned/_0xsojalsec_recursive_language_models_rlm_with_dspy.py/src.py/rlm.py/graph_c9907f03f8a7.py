...
class RLMGraphTracker:
    ...
    def save_html(
        ...
        for node_id, node_data in self.nodes.items():
            ...
            if node_data.node_type == "llm_call":
                ...
                status_emoji = {"success": "✅", "error": "❌", "pending": "⏳"}.get(node_data.status, "⏳")
    ...
    def get_cumulative_stats(self, node_id: str) -> Dict[str, Any]:
        ...
        # Sum up metrics
        cumulative_iterations = sum(self.nodes[nid].iterations for nid in all_nodes if nid in self.nodes)
        cumulative_llm_calls = sum(self.nodes[nid].llm_calls for nid in all_nodes if nid in self.nodes)
