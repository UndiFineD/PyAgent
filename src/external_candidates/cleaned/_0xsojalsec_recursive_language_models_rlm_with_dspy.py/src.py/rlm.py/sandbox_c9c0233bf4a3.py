# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\src\rlm\sandbox.py
...
class E2BSandboxExecutor(SandboxExecutor):
    ...
    async def aexecute(self, code: str, env: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Execute user code
            result = await asyncio.wait_for(asyncio.to_thread(sandbox.run_code, code), timeout=self.timeout)
...
class SandboxFactory:
    ...
    def create(
        ...
        if sandbox_type == "restricted":
            ...
            return E2BSandboxExecutor(timeout=timeout, max_output_chars=max_output_chars, api_key=e2b_api_key)
