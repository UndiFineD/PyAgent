# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\src\rlm\core_dspy.py
...
from .modules import MaxIterationsError, RLMChainOfThought, RLMError, RLMModule
...
class RLMDSPy:
    ...
    def _setup_dspy_lm(self):
        ...
        if model.startswith("openrouter/"):
            ...
            if not api_key:
                raise ValueError("OpenRouter requires OPENROUTER_API_KEY. Get your key at https://openrouter.ai/keys")

            # Configure OpenRouter via OpenAI-compatible API
            lm = dspy.LM(model=model, api_key=api_key, **lm_kwargs)
    ...
    def completion(self, context: Any, query: Optional[str] = None, **kwargs: Any) -> str:
        """
    ...
    async def acompletion(self, context: Any, query: Optional[str] = None, **kwargs: Any) -> str:
        """
        ...
        if self.logger:
            self.logger.run_start(depth=self._current_depth, query=query, context_size=context_size)
        ...
        try:
            ...
            if self.graph_tracker is not None and self._node_id is not None:
                self.graph_tracker.update_node(node_id=self._node_id, error=str(e))
            if self.logger:
