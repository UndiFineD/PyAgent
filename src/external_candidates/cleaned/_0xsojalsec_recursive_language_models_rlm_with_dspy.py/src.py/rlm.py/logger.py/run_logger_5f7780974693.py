# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\src\rlm\logger\run_logger.py
...
class RLMRunLogger:
    ...
    def __init__(
        ...
        if self.enabled:
            if _IMPORT_ERROR is not None:
                raise RuntimeError("rich is required for logging. Install with `pip install rich`.") from _IMPORT_ERROR
    ...
    def after_llm_call(self, iteration: int, depth: int, code: str, reasoning: Optional[str]) -> None:
        if not self.enabled:
            return
    ...
    def before_execution(self, iteration: int, depth: int, code: str, blocked_reason: Optional[str]) -> None:
        if not self.enabled:
            return
        if blocked_reason:
            ...
            self.console.print(
                Panel(
                    reason_text,
                    border_style="red",
                    title=f"Execution blocked (iter {iteration})",
                )
            )
        else:
    ...
    def execution_exception(self, iteration: int, depth: int, error: str) -> None:
        ...
        self.console.print(
            Panel(
                text,
                border_style="red",
                title=f"Execution exception (iter {iteration})",
            )
        )

    def after_execution(self, iteration: int, depth: int, output: str, error: Optional[str]) -> None:
        if not self.enabled:
            return
