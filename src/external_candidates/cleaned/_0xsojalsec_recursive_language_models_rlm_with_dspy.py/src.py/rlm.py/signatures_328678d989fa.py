# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\src\rlm\signatures.py
...
class RLMSignature(dspy.Signature):
    ...
    # Inputs
    query = dspy.InputField(desc="The question or task to answer using the context")
    context_size = dspy.InputField(
        desc="Size of the context in characters (context is stored as a variable, not in prompt)"
    )
    depth = dspy.InputField(desc="Current recursion depth (0 for root call)")
...
class RecursiveCallSignature(dspy.Signature):
    ...
    # Inputs
    sub_query = dspy.InputField(desc="The specific question for this sub-context")
    sub_context = dspy.InputField(desc="The sub-portion of the context to analyze")
    depth = dspy.InputField(desc="Current recursion depth")

    # Output
    answer = dspy.OutputField(desc="The answer extracted from the sub-context")
...
class RLMInstructionSignature(dspy.Signature):
    ...
    depth = dspy.InputField(desc="Current recursion depth")
    previous_output = dspy.InputField(desc="Output from previous code execution (if any)", default="")

    # Output
    reasoning = dspy.OutputField(desc="Brief explanation of your next steps for exploring the context")
    code = dspy.OutputField(
        desc=(
