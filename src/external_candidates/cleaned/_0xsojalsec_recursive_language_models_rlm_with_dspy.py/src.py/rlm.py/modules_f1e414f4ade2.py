...
class RLMModule(dspy.Module):
    ...
    def forward(
        ...
        *,
        ...
        repl_env = self._build_repl_env(query=query, context=context_text, recursive_llm_fn=recursive_llm_fn)
        ...
        for iteration in range(self.max_iterations):
            ...
            if last_code is not None and sanitized_code.strip() == last_code.strip():
                if self.logger:
                    self.logger.on_duplicate_code(iteration=iteration_index, depth=depth, code=sanitized_code)
            ...
            try:
                ...
                if self.logger:
                    self.logger.execution_exception(iteration=iteration_index, depth=depth, error=str(e))
            ...
            if repl_env.get("_final_result", {}).get("called", False):
                ...
                if answer is not None:
                    ...
                    if self.logger:
                        self.logger.on_final(iteration=iteration_index, depth=depth, answer=answer)
        ...
        # Max iterations exceeded
        if self.logger:
            self.logger.on_max_iterations(max_iterations=self.max_iterations, depth=depth)
        raise MaxIterationsError(f"Max iterations ({self.max_iterations}) exceeded without FINAL()")

    def _build_repl_env(self, query: str, context: str, recursive_llm_fn: Optional[Callable]) -> Dict[str, Any]:
        """
        Build REPL environment.
