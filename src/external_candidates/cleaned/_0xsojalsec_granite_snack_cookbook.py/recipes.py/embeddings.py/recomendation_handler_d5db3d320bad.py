# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-granite-snack-cookbook\recipes\Embeddings\recomendation_handler.py
#!/usr/bin/env python
# coding: utf-8
...
def get_thresholds(
    ...
    thresholds = {}
    thresholds["add_lower_threshold"] = round(add_similarities_df.describe([0.1]).loc["10%", "similarity"], 1)
    thresholds["add_higher_threshold"] = round(add_similarities_df.describe([0.9]).loc["90%", "similarity"], 1)
    thresholds["remove_lower_threshold"] = round(remove_similarities_df.describe([0.1]).loc["10%", "similarity"], 1)
    thresholds["remove_higher_threshold"] = round(remove_similarities_df.describe([0.9]).loc["90%", "similarity"], 1)
