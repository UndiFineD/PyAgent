# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\zbruceli.py\unifai_trading_suite.py\src.py\markets.py\polymarket_9f61857b6922.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\zbruceli\unifai-trading-suite\src\markets\polymarket.py
...
class PolymarketClient:
    ...
    async def _call_tool(self, tool_name: str, params: dict) -> Any:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant. Call the requested tool and return the raw result.",
            },
            {
                "role": "user",
                "content": f"Call the tool {tool_name} with parameters: {json.dumps(params)}. Return only the tool result.",
            },
        ]
    ...
    async def get_events_by_category(
        ...
        result = await self._call_tool("polymarket--127--getEventsByCategory", {"category": category})
    ...
    async def search_markets(
        ...
        result = await self._call_tool("polymarket--127--search", {"q": query})
    ...
    async def get_markets(
        ...
        result = await self._call_tool("polymarket--127--searchPolymarketMarkets", params)
    ...
    async def get_prices(
        ...
        result = await self._call_tool("polymarket--127--getPrices", {"tokenIds": token_ids, "sides": sides})
    ...
    async def get_orderbooks(
        ...
        result = await self._call_tool("polymarket--127--getOrderBooks", {"tokenIds": token_ids, "sides": sides})
