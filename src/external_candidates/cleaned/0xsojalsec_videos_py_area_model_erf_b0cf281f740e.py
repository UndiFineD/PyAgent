# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2018.py\eop.py\chapter1.py\area_model_erf_b0cf281f740e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\chapter1\area_model_erf.py
...
class IllustrateAreaModelErf(GraphScene):
    ...
    def construct(self):
        ...
        self.add_T_label(x_min_1, label="-a", side=LEFT, color=YELLOW, animated=False)

        self.add_T_label(x_max_1, label="a", side=RIGHT, color=YELLOW, animated=False)
        ...
        self.play(FadeIn(equals_sign), FadeIn(cdf_value))
        ...
        cdf_value.add_updater(lambda m: m.set_value(integral_update_func()))

        anim = self.get_animation_integral_bounds_change(graph, x_min_2, x_max_2, run_time=3)
