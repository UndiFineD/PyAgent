# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-XtremeNmapParser\app\modules\NmapParser.py
import ipaddress
...
class NmapParser:
    ...
    def get_simple_df(self):
        try:
            ...
            logger.warning(f" |?| Warning | The file may not have any information or may not exist.")
            logger.warning(
                "UnboundLocalError: 'dataframe' variable was referenced before assignment. Ensure 'dataframe' is defined before attempting to call 'to_dataframe' on it."
            )
            logger.warning(ULE)
    ...
    def merge_df(xml_file_list):
        ...
        df = df.sort_values(
            by=["IP", "Port", "State", "RelevantDuplicate"],
