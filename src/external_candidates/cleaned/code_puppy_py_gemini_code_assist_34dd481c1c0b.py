# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\gemini_code_assist_34dd481c1c0b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\gemini_code_assist.py
...
class GeminiCodeAssistModel(Model):
    ...
    async def request(
        ...
        request_body = self._build_request(messages, model_settings, model_request_parameters)
        ...
        async with httpx.AsyncClient(timeout=180) as client:
            ...
            if response.status_code != 200:
                ...
                raise RuntimeError(f"Code Assist API error {response.status_code}: {error_text}")
        ...
        async with httpx.AsyncClient(timeout=180) as client:
            async with client.stream("POST", url, json=request_body, headers=headers) as response:
                if response.status_code != 200:
                    ...
                    raise RuntimeError(f"Code Assist API error {response.status_code}: {error_text.decode()}")
    ...
    def _build_request(
        ...
        if model_request_parameters.function_tools:
            inner_request["tools"] = [self._build_tools(model_request_parameters.function_tools)]
    ...
    def _build_generation_config(self, model_settings: ModelSettings | None) -> Optional[Dict[str, Any]]:
    ...
    def _parse_response(self, data: Dict[str, Any]) -> ModelResponse:
        ...
        return ModelResponse(parts=response_parts, model_name=self._model_name, usage=usage)
