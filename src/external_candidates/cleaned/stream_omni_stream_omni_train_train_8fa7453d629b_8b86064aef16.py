...
# Extracted from: C:\DEV\PyAgent\.external\Stream-Omni\stream_omni\train\train.py
...
def save_param_info(model, output_dir):
    ...
    with open(output_file, "w") as f:
        for name, size, requires_grad in parameters_info:
            f.write(
                f"{name}: Size={size}, {'Trainable' if requires_grad else 'Frozen'}\n"
            )
...
def preprocess(
    ...
    if (
        conversation_lib.default_conversation.sep_style
        == conversation_lib.SeparatorStyle.LLAMA_3_1
    ):
...
class LazySupervisedDataset(Dataset):
    def __init__(
        ...
        if len(data_paths) != len(data_args.image_folders):
            raise ValueError(
                "The number of data paths must match the number of image folders."
            )
            ...
            if "{" in data_path and "}" in data_path:
                ...
                for file_name in file_names:
                    ...
                    with open(full_path, "r") as file:
                        ...
                        rank0_print(
                            f"Loaded {len(cur_data_dict)} samples from {full_path}"
                        )
                ...
                with open(data_path, "r") as file:
                    ...
                    for dataset in datasets:
                        ...
                        if ":" in sampling_strategy:
                            sampling_strategy, sampling_number = (
                        ...
                        if sampling_strategy == "first" and sampling_number is not None:
                            ...
                            and sampling_number is not None
                        ):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if len(image_file) > 1:
                    image = [
                        self.process_image(f, "pad", folder=image_folder)
                        for f in image_file
                    ]
...
def make_supervised_data_module(
    ...
    train_dataset = LazySupervisedDataset(
        tokenizer=tokenizer, data_paths=data_args.data_paths, data_args=data_args
    )
