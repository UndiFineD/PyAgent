# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\general\human_approval_example.py
#!/usr/bin/env python3
"""
...
from praisonaiagents.tools import file_tools, python_tools, shell_tools
...
def custom_approval_callback(function_name: str, arguments: dict, risk_level: str) -> ApprovalDecision:
    ...
    if function_name == "execute_command" and any(
        dangerous in str(arguments.get("command", "")) for dangerous in ["rm -rf", "del /f", "format", "shutdown"]
    ):
...
def main():
    ...
    process = PraisonAIAgents(agents=[agent], tasks=tasks, verbose=True)
...
async def async_demo():
    ...
    process = PraisonAIAgents(agents=[agent], tasks=[task], verbose=True)
