# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\state_management_example.py
"""
...
from praisonaiagents import Agent, PraisonAIAgents, Session, Task
...
def add_feature_to_project(feature_name: str, estimated_cost: int) -> Dict[str, Any]:
    ...
    feature = {"name": feature_name, "cost": estimated_cost, "status": "planned"}
...
def check_project_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "has_more_features": workflow.has_state("features")
        and any(f["status"] == "planned" for f in workflow.get_state("features", [])),
