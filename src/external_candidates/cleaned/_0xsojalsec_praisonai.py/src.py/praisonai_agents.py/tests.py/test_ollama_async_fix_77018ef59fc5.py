# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\test_ollama_async_fix.py
#!/usr/bin/env python3
"""
...
async def test_model_async(model_name: str):
    ...
    workflow = PraisonAIAgents(agents=[agent], tasks=[task], verbose=True)
    ...
    try:
        ...
        if not result or (
            isinstance(result, dict) and not result.get("task_results", {}).get("search_task", {}).get("output")
        ):
