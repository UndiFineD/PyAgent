# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\test_handoff_compatibility.py
"""
...
def test_agent_with_existing_tools():
    ...
    agent = Agent(name="Tool Agent", tools=[test_tool])
...
def test_agent_with_handoffs():
    ...
    agent = Agent(name="Source Agent", handoffs=[target_agent])
...
def test_handoff_with_tools():
    ...
    agent = Agent(name="Source", tools=[my_tool], handoffs=[target_agent])
...
def test_handoff_object():
    ...
    agent = Agent(name="Source", handoffs=[handoff_obj])
...
def test_handoff_filters():
    ...
    assert all(
        msg.get("role") not in ["tool"] and "tool_calls" not in msg
