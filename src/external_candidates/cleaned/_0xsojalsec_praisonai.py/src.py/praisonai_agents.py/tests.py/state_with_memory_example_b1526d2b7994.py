# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\state_with_memory_example.py
"""
...
memory_config = {"config": {"collection_name": "state_memory_demo", "path": "./state_memory_db"}}
...
def search_conversation_memory(query: str, speaker: str = None) -> Dict[str, Any]:
    ...
    # Search memory
    results = shared_memory.search(query=query, n=5, metadata_filter=metadata_filter)
...
def analyze_conversation_patterns() -> Dict[str, Any]:
    ...
    related_convos = shared_memory.search(query=topic, n=10, metadata_filter={"type": "conversation_turn"})
...
def retrieve_user_history(user_name: str) -> Dict[str, Any]:
    ...
    user_history = shared_memory.search(query=user_name, n=20, metadata_filter={"user": user_name})
...
test_workflow = PraisonAIAgents(agents=[test_agent], tasks=[test_task], memory=memory_config, verbose=0)
