# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_async_sequential.py
#!/usr/bin/env python3
...
async def test_sequential_async_execution():
    ...
    try:
        ...
        if async_batch1:
            ...
            print(
                f"  - Parallel execution: {'YES' if batch1_duration < 4 else 'NO'} (expected ~2s for parallel, 6s for sequential)"
            )
        ...
        if async_batch2:
            ...
            print(
                f"  - Parallel execution: {'YES' if batch2_duration < 1.5 else 'NO'} (expected ~1s for parallel, 2s for sequential)"
            )
