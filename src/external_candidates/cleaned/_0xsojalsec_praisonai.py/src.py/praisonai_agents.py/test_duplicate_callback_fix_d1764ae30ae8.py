                ...
                "choices": [
                    {
                        "message": {
                            ...
                            "provider_specific_fields": {"reasoning_content": "Let me think about this..."},
                        }
                    }
                ]
            }
            ...
            with patch("litellm.completion") as mock_completion:
                ...
                def mock_streaming(*args, **kwargs):
                    ...
                    if call_count == 1:
                        ...
                        return [MagicMock(choices=[MagicMock(delta=MagicMock(content=json.dumps(reflection)))])]
                    else:
                        # Final response
                        return [
...
def test_async_single_display():
    ...
    async def run_test():
        with patch("litellm.acompletion") as mock_acompletion:
            ...
            llm = LLM(model="gpt-5-nano", verbose=False)
            response = await llm.get_response_async(prompt="Test async", verbose=True, stream=True)
