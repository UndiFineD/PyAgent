# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\memory\memory.py
import json
import logging
import os
import shutil
...
class Memory:
    ...
    def _get_embedding(self, text: str) -> List[float]:
        ...
        try:
            if LITELLM_AVAILABLE:
                ...
                self._log_verbose(
                    "Neither litellm nor openai available for embeddings",
                    logging.WARNING,
                )
                return None
        except Exception as e:
        ...
        if self.use_mem0 and hasattr(self, "mem0_client"):
            ...
            try:
                if LITELLM_AVAILABLE:
                    ...
                    response = litellm.embedding(model=self.embedding_model, input=query)
                    ...
                    response = client.embeddings.create(input=query, model=self.embedding_model)
                    query_embedding = response.data[0].embedding
                else:
                    self._log_verbose(
                        "Neither litellm nor openai available for embeddings",
                ...
                resp = self.chroma_col.query(query_embeddings=[query_embedding], n_results=limit)
            ...
            for row in rows:
                ...
                if quality >= min_quality:
                    results.append({"id": row[0], "text": row[1], "metadata": meta})
            return results
        ...
        **kwargs,
        ...
        for row in rows:
            ...
            if not any(f["id"] == row[0] for f in found):
                found.append({"id": row[0], "text": text, "metadata": meta, "created_at": row[3]})
    ...
    def search_user_memory(
        self, user_id: str, query: str, limit: int = 5, rerank: bool = False, **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        if self.use_mem0 and hasattr(self, "mem0_client"):
            ...
            try:
                results = []
                search_filter = {"user_id": user_id, "$text": {"$search": query}}
    ...
    def finalize_task_output(
        ...
        try:
            ...
            self.store_short_term(text=content, metadata=metadata)
        ...
        if quality_score >= threshold:
            try:
                ...
                self.store_long_term(text=content, metadata=metadata)
            ...
            metadata.update(
                {
                    ...
                    "clarity": clarity,
                    "accuracy": accuracy,
                    "quality": self.compute_quality_score(completeness, relevance, clarity, accuracy, weights),
                }
            )
        ...
        try:
            if LITELLM_AVAILABLE:
                ...
                response = litellm.completion(
                    model=model_name,
                    messages=[{"role": "user", "content": custom_prompt or default_prompt}],
    ...
    def store_quality(
        ...
        if metrics:
            metadata.update({k: v for k, v in metrics.items()})  # Remove metric_ prefix
    ...
    def search_with_quality(
        ...
        search_func = self.search_short_term if memory_type == "short" else self.search_long_term
