...
Your Goal: {self.goal}
    ...
    def _openai_client(self):
        ...
        if self.__openai_client is None:
            try:
                self.__openai_client = get_openai_client(api_key=self._openai_api_key, base_url=self._openai_base_url)
        ...
        for handoff_item in self.handoffs:
            try:
                if isinstance(handoff_item, Handoff):
                    ...
                    logging.warning(
                        f"Invalid handoff item type: {type(handoff_item)}. Expected Agent or Handoff instance."
                    )
            except Exception as e:
...
Your Goal: {self.goal}"""
            ...
            for tool in tools_to_use:
                try:
                    if callable(tool) and hasattr(tool, "__name__"):
                        ...
                        isinstance(tool, dict) and isinstance(tool.get("function"), dict) and "name" in tool["function"]
                    ):
                        tool_names.append(tool["function"]["name"])
                    elif isinstance(tool, str):
                        tool_names.append(tool)
            ...
            try:
                ...
                if inspect.isclass(func) and hasattr(func, "run") and not hasattr(func, "_run"):
                    ...
                    run_params = {
                        ...
                        if k in inspect.signature(instance.run).parameters and k != "self"
                    }
                ...
                # CrewAI: If it's a class with an _run method, instantiate and call _run
                    ...
                    run_params = {
                        k: v
                        for k, v in arguments.items()
                        if k in inspect.signature(instance._run).parameters and k != "self"
                    }
    ...
    def chat(
        ...
        if logging.getLogger().getEffectiveLevel() == logging.DEBUG:
            param_info = {
                "prompt": (
                    str(prompt)[:100] + "..." if isinstance(prompt, str) and len(str(prompt)) > 100 else str(prompt)
                ),
                "temperature": temperature,
        ...
        if self._using_custom_llm:
            ...
            if isinstance(original_prompt, list):
                # Extract text from multimodal prompts
                normalized_content = next(
                    (item["text"] for item in original_prompt if item.get("type") == "text"),
                    "",
                )
                    ...
                    try:
                        ...
                        if not self.self_reflect:
                            ...
                            if reasoning_steps and hasattr(response.choices[0].message, "reasoning_content"):
                                ...
                                try:
                                    validated_reasoning = self._apply_guardrail_with_retry(
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
                                ...
                                try:
                                    ...
                                    reflection_response = (
                                        await self._openai_client.async_client.beta.chat.completions.parse(
                                            model=(self.reflect_llm if self.reflect_llm else self.llm),
        ...
        try:
            ...
            if self._using_custom_llm:
                ...
                if isinstance(actual_prompt, list):
                    normalized_content = next(
                        (item["text"] for item in actual_prompt if item.get("type") == "text"),
                        "",
                    )
                        ...
                        if hasattr(delta, "tool_calls") and delta.tool_calls:
                            for tool_call_delta in delta.tool_calls:
                                ...
                                if tool_call_delta.function.name:
                                    tool_calls_data[tool_call_delta.index]["function"]["name"] = (
                                        tool_call_delta.function.name
                                    )
                                if tool_call_delta.function.arguments:
                                    tool_calls_data[tool_call_delta.index]["function"]["arguments"] += (
                        ...
                        for tool_call in tool_calls_data:
                            if tool_call["id"] and tool_call["function"]["name"]:
                                try:
                                    ...
                                    tool_result = self.execute_tool(tool_call["function"]["name"], parsed_args)
    ...
    def launch(
        ...
        if protocol == "http":
            ...
            actual_mcp_tool_name = (
                f"execute_{self.name.lower().replace(' ', '_').replace('-', '_')}_task" if self.name else "execute_task"
            )
