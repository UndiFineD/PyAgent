# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\agents\agents.py
import asyncio
import json
import logging
import os
...
from ..main import TaskOutput, display_error, error_logs
from ..process.process import LoopItems, Process
    ...
    def __init__(
        ...
        if memory:
            try:
                ...
                if not mem_cfg:
                    mem_cfg = next(
                        (t.config.get("memory_config") for t in tasks if hasattr(t, "config") and t.config),
                        None,
                    )
                ...
                if not mem_cfg:
                    mem_cfg = {
                        "provider": "rag",
                        "use_embedding": True,
                        "storage": {"type": "sqlite", "path": "./.praison/memory.db"},
                    ...
                    if os.path.exists(img):
                        ...
                        if ext == ".mp4":
                            ...
                            content.append(
                                {
                                    "type": "text",
                                    "text": "These are frames from the video.",
                                }
                            )
                            for f in frames:
                                content.append(
                            ...
                            content.append(
                                {
                                    "type": "image_url",
                                    "image_url": {"url": f"data:image/{ext.lstrip('.')};base64,{encoded}"},
                                }
                            )
                    else:
...
"""
    ...
    def restore_session_state(self, session_id: str) -> bool:
        ...
        for result in results:
            ...
            if metadata.get("type") == "session_state" and metadata.get("session_id") == session_id:
    ...
    def launch(
        ...
        if protocol == "http":
                    ...
                    for agent_instance in self.agents:  # Corrected variable name to agent_instance
                        try:
                            ...
                            if asyncio.iscoroutinefunction(agent_instance.chat):
                                response = await agent_instance.achat(
                                    current_input,
                                    task_name=None,
                                ...
                                response = await loop.run_in_executor(
                                    None,
                                    lambda ci=current_input: agent_instance.chat(ci),
                                )

                            # Store this agent's result
                            results.append({"agent": agent_instance.name, "response": response})

                            # Use this response as input to the next agent
                            current_input = response
                        except Exception as e:
                            logging.error(
                                f"Error with agent {agent_instance.name}: {str(e)}",
                                exc_info=True,
                            )
                            results.append({"agent": agent_instance.name, "error": str(e)})
            ...
            if not _agents_server_started.get(port, False):
                ...
                def run_server():
                    try:
                        ...
                        print(
                            f"🔌 Registered HTTP endpoints on port {port}: {', '.join(list(_agents_registered_endpoints[port].keys()))}"
                        )
                        uvicorn.run(
                            _agents_shared_apps[port],
                            host=host,
                            port=port,
            ...
            actual_mcp_tool_name = (
                f"execute_{self.name.lower().replace(' ', '_').replace('-', '_')}_workflow"
                if self.name
                ...
                for agent_instance in self.agents:
                    try:
                        ...
                        if hasattr(agent_instance, "achat") and asyncio.iscoroutinefunction(agent_instance.achat):
                            ...
                            response = await loop.run_in_executor(
                                None,
                                lambda ci=current_input: agent_instance.chat(ci, tools=agent_instance.tools),
                            )
                        else:
                        ...
                        logging.debug(
                            f"Agent {agent_instance.name} responded. Current intermediate output: {current_input}"
                        )

                    except Exception as e:
                        logging.error(
                            f"Error during agent {agent_instance.name} execution in MCP workflow: {str(e)}",
                            exc_info=True,
                        )
            ...
            starlette_mcp_app = Starlette(
                ...
                routes=[
                    ...
                    Mount(
                        messages_mcp_path_prefix,
                        app=sse_transport_mcp.handle_post_message,
                    ),
                ],
            )
            ...
            def run_praison_mcp_server():
                try:
                    ...
                    logging.error(
                        f"Error starting PraisonAIAgents MCP server: {str(e)}",
                        exc_info=True,
                    )
