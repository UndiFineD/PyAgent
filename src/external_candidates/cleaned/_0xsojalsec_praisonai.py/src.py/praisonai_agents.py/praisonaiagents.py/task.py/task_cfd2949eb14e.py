...
class Task:
    ...
    def _setup_guardrail(self):
        ...
        if callable(self.guardrail):
            ...
            positional_args = [param for param in sig.parameters.values() if param.default is inspect.Parameter.empty]
            ...
            if return_annotation != inspect.Signature.empty:
                ...
                if not (is_guardrail_result or is_tuple):
                    raise ValueError("If return type is annotated, it must be GuardrailResult or Tuple[bool, Any]")
    ...
    async def execute_callback(self, task_output: TaskOutput) -> None:
        ...
        if self.quality_check and self.memory:
            try:
                ...
                metrics = self.memory.calculate_quality_metrics(task_output.raw, self.expected_output, llm=llm_model)
                ...
                if self.next_tasks:
                    ...
                    context = self.memory.build_context_for_task(task_descr=task_output.raw, max_items=5)
