# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\json_tools.py
...
class JSONTools:
    ...
    def validate_json(self, data: Union[Dict[str, Any], str], schema: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    def analyze_json(self, data: Union[Dict[str, Any], str], max_depth: int = 10) -> Dict[str, Any]:
            ...
            def analyze_value(value: Any, depth: int = 0) -> Dict[str, Any]:
                ...
                if isinstance(value, dict):
                    ...
                    if depth < max_depth - 1:
                        result["children"] = {k: analyze_value(v, depth + 1) for k, v in value.items()}
                    ...
                    if value:
                        ...
                        if depth < max_depth - 1:
                            result["sample_elements"] = [analyze_value(x, depth + 1) for x in value[:5]]

                elif isinstance(value, (int, float)):
                    result.update({"value": value, "is_integer": isinstance(value, int)})
    ...
    def transform_json(self, data: Union[Dict[str, Any], str], transformations: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _rename_key(self, data: Dict[str, Any], old_path: List[str], new_path: List[str]):
    ...
    def _move_value(self, data: Dict[str, Any], old_path: List[str], new_path: List[str]):
        """Move a value from one path to another."""
        self._rename_key(data, old_path, new_path)

    def _get_value(self, data: Dict[str, Any], path: List[str]) -> Optional[Any]:
...
if __name__ == "__main__":
    ...
    print("------------------------------")
    transformations = [
        {"operation": "set", "path": "details.status", "value": "active"},
        {"operation": "rename", "path": "details.city", "value": "details.location"},
    ]
