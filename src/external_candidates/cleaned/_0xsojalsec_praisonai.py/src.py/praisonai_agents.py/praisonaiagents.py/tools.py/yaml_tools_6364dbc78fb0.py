# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\yaml_tools.py
...
class YAMLTools:
    ...
    def read_yaml(
        self, filepath: str, safe_load: bool = True, encoding: str = "utf-8"
    ...
    def validate_yaml(self, data: Union[Dict[str, Any], List[Any], str], schema: Dict[str, Any]) -> bool:
    ...
    def analyze_yaml(self, data: Union[Dict[str, Any], List[Any], str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            def analyze_value(value: Any) -> Dict[str, Any]:
                ...
                if isinstance(value, dict):
                    ...
                    result["nested_types"] = {k: type(v).__name__ for k, v in value.items()}
                elif isinstance(value, list):
                    result["element_types"] = list(set(type(x).__name__ for x in value))
            ...
            result = {
                ...
                "stats": {
                    "total_keys": (sum(1 for _ in self._walk_dict(data)) if isinstance(data, dict) else len(data))
                },
            }
...
if __name__ == "__main__":
    ...
    data1 = {
        ...
        "database": {"url": "postgresql://localhost:5432/db", "pool_size": 5},
    }

    data2 = {
        "server": {"workers": 4, "timeout": 30},
    ...
    print("------------------------------")
    operations = [
        {"type": "set", "path": "server/host", "value": "0.0.0.0"},
        {"type": "delete", "path": "server/debug"},
        {"type": "set", "path": "logging/handlers", "value": ["console", "file"]},
    ]
