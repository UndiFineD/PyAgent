# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\mongodb_tools.py
...
class MongoDBTools:
    ...
    def _get_pymongo(self) -> Optional["pymongo"]:
        ...
        if util.find_spec("pymongo") is None:
            error_msg = (
                "pymongo package is not available. Please install it using: pip install 'praisonaiagents[mongodb]'"
            )
    ...
    def _get_motor(self) -> Optional[Any]:
        ...
        if util.find_spec("motor") is None:
            error_msg = (
                "motor package is not available. Please install it using: pip install 'praisonaiagents[mongodb]'"
            )
            logging.error(error_msg)
            return None
...
if __name__ == "__main__":
    ...
    result = insert_document(
        "test_collection",
        {"name": "Test Document", "value": 42, "tags": ["test", "mongodb"]},
    )
