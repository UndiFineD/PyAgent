# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\train\data\generatecot.py
import json
import logging
import os
...
class GenerateCOT:
    ...
    def cot_generate_dict(self, question: str, context: str = "") -> dict:
        ...
        return {"thought_process": thought_process, "final_answer": final_answer}
    ...
    def cot_export_csv_with_qa_pairs(
        self, filepath: str = "dataset.csv", qa_pairs: Optional[Dict[str, str]] = None
    ) -> Optional[str]:
    ...
    def cot_save(self, question: str, answer: str, filepath: str = "dataset.csv") -> Optional[str]:
...
if __name__ == "__main__":
    ...
    print("\n=== Direct QA Pairs Export Example ===")
    direct_qa_data = {"Number of r's in the word strawberry": "3"}

    direct_generator = GenerateCOT()

    # Export with qa_pairs passed directly to functions
    direct_generator.cot_export_csv_with_qa_pairs(filepath="direct_solutions.csv", qa_pairs=direct_qa_data)
