...
class PerformanceMonitor:
    ...
    def _record_function_performance(
        ...
        with self._lock:
            ...
            if self._telemetry:
                self._telemetry.track_tool_usage(tool_name=func_name, success=success, execution_time=execution_time)
                ...
                for name, stats in self._function_stats.items():
                    ...
                    if processed_stats["call_count"] > 0:
                        ...
                        processed_stats["success_rate"] = (
                            processed_stats["call_count"] - processed_stats["error_count"]
                        ) / processed_stats["call_count"]

                        if processed_stats["recent_times"]:
                            processed_stats["recent_average"] = sum(processed_stats["recent_times"]) / len(
            ...
            if api_name:
                ...
                for name, stats in self._api_calls.items():
                    ...
                    if processed_stats["call_count"] > 0:
                        processed_stats["average_time"] = processed_stats["total_time"] / processed_stats["call_count"]
                        processed_stats["success_rate"] = (
                            processed_stats["success_count"] / processed_stats["call_count"]
                        )
