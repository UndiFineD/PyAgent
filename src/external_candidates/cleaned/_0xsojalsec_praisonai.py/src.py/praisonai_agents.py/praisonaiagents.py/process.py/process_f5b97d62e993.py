# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\process\process.py
import asyncio
import csv
import json
import logging
...
class Process:
    ...
    VALIDATION_FAILURE_DECISIONS = [
        ...
        "rejected",
        "incomplete",
    ]  # Decision strings that trigger validation feedback
        ...
        while fallback_attempts < Process.DEFAULT_RETRY_LIMIT and not temp_current_task:
            ...
            for task_candidate in self.tasks.values():
                if task_candidate.status == "not started":
                    ...
                    leads_to_task = any(
                        task_value
                        for task_value in current_conditions.values()
                    ...
                    if self.task_retry_counter.get(task_candidate.id, 0) < Process.DEFAULT_RETRY_LIMIT:
                        self.task_retry_counter[task_candidate.id] = (
                            self.task_retry_counter.get(task_candidate.id, 0) + 1
                        )
                        temp_current_task = task_candidate
                        logging.debug(
                            f"Fallback attempt {fallback_attempts}: Found 'not started' task: {temp_current_task.name}, retry count: {self.task_retry_counter[temp_current_task.id]}"
                        )
                        return temp_current_task  # Return the found task immediately
                    else:
                        logging.debug(
                            f"Max retries reached for task {task_candidate.name} in fallback mode, marking as failed."
                        )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    props_desc = ", ".join(
                        [f'"{k}": <{v.get("type", "any")}>' for k, v in schema.get("properties", {}).items()]
                    )
                    ...
                    logging.warning(
                        f"Could not generate schema for ManagerInstructions: {schema_error}. Using hardcoded prompt."
                    )
        ...
        for task in self.tasks.values():
            if task.next_tasks:
                for next_task_name in task.next_tasks:
                    next_task = next(
                        (t for t in self.tasks.values() if t.name == next_task_name),
                        None,
                    )
                    if next_task:
...
Input file: {getattr(current_task, "input_file", None)}
                ...
                if getattr(current_task, "_subtasks_created", False):
                    subtasks = [t for t in self.tasks.values() if t.name.startswith(current_task.name + "_")]
                    logging.debug(f"""
...
- Condition: {st.condition}
                    ...
                    if subtasks and all(st.status == "completed" for st in subtasks):
                        ...
                        if not current_task.result:
                            ...
                            last_subtask = next(
                                (t for t in reversed(subtasks) if t.status == "completed"),
                                None,
                            )
                        ...
                        if current_task.condition:
                            ...
                            next_task = next(
                                (t for t in self.tasks.values() if t.name == task_value),
                                None,
                            )
                            if next_task:
                ...
                if (
                    not current_task.next_tasks
                    and not current_task.condition
                    and not any(
            ...
            if self.tasks[task_id].status == "completed":
                ...
                if (
                    getattr(
                        task_to_check, "rerun", True
                    )  # Corrected condition - reset only if rerun is True (or default True)
                    and task_to_check.task_type != "loop"  # Removed "decision" from exclusion
                    and not any(
                        t.task_type == "loop" and subtask_name.startswith(t.name + "_") for t in self.tasks.values()
                    )
                    and not task_to_check.async_execution
                ):  # Don't reset async parallel tasks
                    logging.debug(
                        f"=== Resetting non-loop, non-decision, non-parallel task {subtask_name} to 'not started' ==="
                    )
                    self.tasks[task_id].status = "not started"
                    logging.debug(f"Task status after reset: {self.tasks[task_id].status}")
                else:
                    logging.debug(
                        f"=== Skipping reset for loop/decision/subtask/parallel or rerun=False: {subtask_name} ==="
                    )
            ...
            if current_task and current_task.result:
                if current_task.task_type in ["decision", "loop"]:
                    ...
                    if current_task.condition:
                        ...
                        if (
                            not target_tasks
                            or target_tasks == "exit"
                            or (isinstance(target_tasks, list) and (not target_tasks or target_tasks[0] == "exit"))
                        ):
                                ...
                                if decision_str in Process.VALIDATION_FAILURE_DECISIONS:
                                    if current_task and current_task.result:
                                        ...
                                        if current_task.previous_tasks:
                                            ...
                                            validated_task = next(
                                                (t for t in self.tasks.values() if t.name == prev_task_name),
                                                None,
                                            )
                                        ...
                                        logging.debug(
                                            f"Added validation feedback to {next_task.name}: {feedback['validation_response']} (validated task: {feedback.get('validated_task', 'None')})"
                                        )
            ...
            if not next_task and current_task and current_task.next_tasks:
                ...
                if next_task:
                    ...
                    if (
                        next_task.previous_tasks
                        and current_task.name in next_task.previous_tasks
                        and next_task.next_tasks
                        and next_task.next_tasks[0] in self.tasks
                        and next_task.name in self.tasks[next_task.next_tasks[0]].previous_tasks
                    ):
                        self.workflow_finished = False
...
"""
            ...
            for a in self.agents:
                if a.name == selected_agent_name:
                    ...
                    logging.info(
                        f"Changed agent for task {selected_task_id} from {original_agent} to {selected_agent_name}"
                    )
                    break
            ...
            if self.tasks[selected_task_id].status != "completed":
                ...
                logging.info(
                    f"Finished execution of task {selected_task_id}, status: {self.tasks[selected_task_id].status}"
                )
                    ...
                    with open(start_task.input_file, "r", encoding="utf-8") as f:
                        ...
                        for i, row in enumerate(reader):
                            ...
                            row_task = Task(
                                ...
                                condition={
                                    "done": (
                                        inherited_next_tasks if inherited_next_tasks else ["next"]
                                    ),  # Use full inherited_next_tasks
                                    "retry": ["current"],
                    ...
                    with open(start_task.input_file, "r", encoding="utf-8") as f:
                        ...
                        for i, line in enumerate(lines):
                            row_task = Task(
                                ...
                                is_start=(i == 0),
                                task_type="task",
                                condition={"complete": ["next"], "retry": ["current"]},
                            )
                            self.tasks[row_task.id] = row_task
                            new_tasks.append(row_task)
...
- Regular tasks: {sum(1 for t in self.tasks.values() if t.task_type not in ["loop", "decision"])}
            ...
            if (
                current_task.task_type == "loop"
                and current_task is not start_task
                and getattr(current_task, "_subtasks_created", False) is not True
            ):
                ...
                if getattr(current_task, "input_file", None):
                    try:
                        ...
                        if new_tasks:
                            ...
                            current_task._subtasks_created = True
                            logging.info(
                                f"Created {len(new_tasks)} tasks from: {current_task.input_file} for loop task {current_task.name}"
                            )
                    except Exception as e:
