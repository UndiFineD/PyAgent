# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\llm\llm.py
import inspect
import json
import logging
import os
...
class LLM:
    ...
    OLLAMA_FINAL_ANSWER_PROMPT = (
        "Based on the tool results above, please provide the final answer to the original question."
    )
    ...
    def _log_llm_config(self, method_name: str, **config):
        ...
        should_log = logging.getLogger().getEffectiveLevel() == logging.DEBUG or (
            not isinstance(verbose, bool) and verbose >= 10
        )
        ...
        if should_log:
            ...
            if "extra_settings" in safe_config and isinstance(safe_config["extra_settings"], dict):
                safe_config["extra_settings"] = {
                    k: v for k, v in safe_config["extra_settings"].items() if k not in ["api_key"]
                }
    ...
    def __init__(
        ...
        **extra_settings,
        ...
        try:
            ...
            raise ImportError(
                "LiteLLM is required but not installed. Please install with: pip install 'praisonaiagents[llm]'"
            )
    ...
    def _is_ollama_provider(self) -> bool:
        ...
        if any(
            url and ("ollama" in url.lower() or ":11434" in url) for url in [base_url, api_base, self.base_url or ""]
        ):
            return True
    ...
    def _validate_and_filter_ollama_arguments(
                    ...
                    if param.annotation != inspect.Parameter.empty:
                        try:
                            if param.annotation == int and isinstance(param_value, str):
                                ...
                                filtered_args[param_name] = param_value.lower() in (
                                    "true",
                                    "1",
                                    "yes",
                                )
    ...
    def _handle_ollama_sequential_logic(
        self,
        iteration_count: int,
    ...
    def _build_messages(
        ...
        if output_json or output_pydantic:
            ...
            is_gemini_with_structured_output = self._is_gemini_model() and supports_structured_outputs(self.model)
            ...
            if isinstance(tool, dict) and "type" in tool and tool["type"] == "function":
                ...
                if "function" in tool and isinstance(tool["function"], dict) and "name" in tool["function"]:
                    ...
                    if "parameters" in fixed_tool["function"]:
                        fixed_tool["function"]["parameters"] = self._fix_array_schemas(
            ...
            while iteration_count < max_iterations:
                try:
                    ...
                    if reasoning_steps:
                        ...
                        reasoning_content = (
                            resp["choices"][0]["message"].get("provider_specific_fields", {}).get("reasoning_content")
                        )
                        ...
                        if use_streaming:
                            ...
                            try:
                                if verbose:
                                    ...
                                    with Live(
                                        display_generating("", current_time),
                                        console=self.console,
                    ...
                    if not tool_calls and response_text and formatted_tools:
                        ...
                        should_try_xml = (
                            self._supports_xml_tool_format()
                            or
                    ...
                    if (
                        self._is_ollama_provider()
                        and (not response_text or response_text.strip() == "")
                        and formatted_tools
                        and iteration_count == 0
                    ):
                        messages.append({"role": "user", "content": self.OLLAMA_TOOL_USAGE_PROMPT})
                        iteration_count += 1
                        continue
                    ...
                    if tool_calls and execute_tool_fn:
                        ...
                        for tool_call in tool_calls:
                                ...
                                if is_ollama and tool_result_mapping:
                                    ...
                                    for arg_name, arg_value in list(arguments.items()):
                                        if isinstance(arg_value, str) and arg_value in tool_result_mapping:
                                            ...
                                            logging.debug(
                                                f"[OLLAMA_FIX] Replaced {arg_value} with {tool_result_mapping[arg_value]} in {function_name} arguments"
                                            )

                                arguments = self._validate_and_filter_ollama_arguments(function_name, arguments, tools)

                            logging.debug(
                                f"[TOOL_EXEC_DEBUG] About to execute tool {function_name} with args: {arguments}"
                            )
                            ...
                            if verbose:
                                ...
                                logging.debug(
                                    f"[TOOL_EXEC_DEBUG] About to display tool call with message: {display_message}"
                                )
                                ...
                                messages.append(
                                    {
                                        ...
                                        "content": (
                                            ...
                                            else "Function returned an empty output"
                                        ),
                                    }
                                )
                        ...
                        if (
                            ...
                            and "final answer" in response_text.lower()
                        ):
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
                ...
                if reasoning_steps:
                    ...
                    reasoning_content = (
                        reflection_resp["choices"][0]["message"]
            ...
            if use_streaming:
                ...
                try:
                    ...
                    try:
                        for chunk in stream_iterator:
                            try:
                                ...
                                if verbose:
                                    logging.warning(
                                        f"Chunk processing error ({consecutive_errors}/{max_consecutive_errors}): {chunk_error}"
                                    )
                                ...
                                if self._is_streaming_error_recoverable(chunk_error):
                                    if verbose:
                                        logging.warning(
                                            "Recoverable streaming error detected, skipping malformed chunk and continuing"
                                        )
                                    ...
                                    if consecutive_errors < max_consecutive_errors:
                                        ...
                                        logging.warning(
                                            f"Too many consecutive streaming errors ({consecutive_errors}), falling back to non-streaming mode"
                                        )
                                        raise Exception(
                                            f"Streaming failed with {consecutive_errors} consecutive errors"
                                        ) from chunk_error
                                else:
                                    # For non-recoverable errors, re-raise immediately
                        ...
                        if self._is_streaming_error_recoverable(iterator_error):
                            ...
                            raise Exception(
                                "Stream iteration failed with recoverable error, falling back to non-streaming"
                            ) from iterator_error
                        else:
                            # For non-recoverable errors, re-raise immediately
                    ...
                    if any(
                        ...
                        logging.warning(
                            f"Streaming failed due to JSON parsing errors (likely malformed chunks from provider): {e}"
                        )
            ...
            while iteration_count < max_iterations:
                                ...
                                **self._build_completion_params(
                                ...
                                if chunk and chunk.choices and chunk.choices[0].delta:
                                    ...
                                    if delta.content:
                                        print("\033[K", end="\r")
                                        print(
                                            f"Generating... {time.time() - start_time:.1f}s",
                                            end="\r",
                                        )
                ...
                if tools and execute_tool_fn and tool_calls:
                    ...
                    for tool_call in tool_calls:
                        ...
                        if verbose:
                            display_message = (
                                f"Agent {agent_name} called function '{function_name}' with arguments: {arguments}\n"
                            )
                            if tool_result:
                                display_message += f"Function returned: {tool_result}"
                            else:
                    ...
                    if self._is_ollama_provider() and iteration_count > 0:
                        ...
                        messages.append({"role": "user", "content": self.OLLAMA_FINAL_ANSWER_PROMPT})
                            ...
                            async for chunk in await litellm.acompletion(
                                **self._build_completion_params(
                                ...
                                if chunk and chunk.choices and chunk.choices[0].delta.content:
                                    ...
                                    print("\033[K", end="\r")
                                    print(
                                        f"Reflecting... {time.time() - start_time:.1f}s",
                                        end="\r",
                                    )
                        else:
                            response_text = ""
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
    ...
    def can_use_tools(self) -> bool:
        ...
        try:
            ...
            raise ImportError(
                "LiteLLM is required but not installed. Please install it with: pip install 'praisonaiagents[llm]'"
            )
        except:
            return False
    ...
    def _track_token_usage(self, response: Dict[str, Any], model: str) -> Optional[TokenMetrics]:
        ...
        try:
            ...
            _token_collector.track_tokens(
                ...
                metrics=metrics,
                metadata={"provider": self.provider, "stream": False},
            )
    ...
    def _build_completion_params(self, **override_params) -> Dict[str, Any]:
        ...
        if output_json or output_pydantic:
            ...
            if self._is_gemini_model():
                ...
                if (
                    schema_model
                    and hasattr(schema_model, "model_json_schema")
                    and supports_structured_outputs(self.model)
                ):
    ...
    def _prepare_response_logging(
        self, temperature: float, stream: bool, verbose: bool, markdown: bool, **kwargs
    ...
    def response(
        ...
        **kwargs,
        ...
        try:
            ...
            messages, _ = self._build_messages(prompt=prompt, system_prompt=system_prompt)

            # Get response from LiteLLM
            response_text = ""
            completion_params = self._build_completion_params(
                messages=messages, temperature=temperature, stream=stream, **kwargs
            )
