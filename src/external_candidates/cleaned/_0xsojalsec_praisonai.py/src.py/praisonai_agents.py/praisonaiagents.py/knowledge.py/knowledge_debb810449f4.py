...
class CustomMemory:
    ...
    def _add_to_vector_store(self, messages, metadata=None, filters=None, infer=None):
        ...
        if isinstance(messages, list):
            parsed_messages = "\n".join(
                [msg.get("content", str(msg)) if isinstance(msg, dict) else str(msg) for msg in messages]
            )
        else:
...
class Knowledge:
    ...
    def _deps(self):
        try:
            ...
            raise ImportError(
                'Required packages not installed. Please install using: pip install "praisonaiagents[knowledge]"'
            )
                ...
                if "provider" in self._config["vector_store"]:
                    ...
                    if self._config["vector_store"]["provider"] == "mongodb":
                        base_config["vector_store"] = {
                            "provider": "mongodb",
                            "config": {
                                "connection_string": self._config["vector_store"]["config"].get(
                                    "connection_string", "mongodb://localhost:27017/"
                                ),
                                "database": self._config["vector_store"]["config"].get("database", "praisonai"),
                                "collection": self._config["vector_store"]["config"].get(
                                    "collection", "knowledge_base"
                                ),
                                "use_vector_search": self._config["vector_store"]["config"].get(
