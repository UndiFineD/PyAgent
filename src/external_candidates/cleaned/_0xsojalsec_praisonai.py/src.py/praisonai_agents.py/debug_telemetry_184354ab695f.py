# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\debug_telemetry.py
#!/usr/bin/env python3
"""
...
if hasattr(agent, "execute"):
    ...
    print(
        f"Is wrapped: {'instrumented' in str(agent.execute.__name__) if hasattr(agent.execute, '__name__') else 'Unknown'}"
    )
...
if hasattr(workflow, "start"):
    ...
    print(
        f"Is wrapped: {'instrumented' in str(workflow.start.__name__) if hasattr(workflow.start, '__name__') else 'Unknown'}"
    )
