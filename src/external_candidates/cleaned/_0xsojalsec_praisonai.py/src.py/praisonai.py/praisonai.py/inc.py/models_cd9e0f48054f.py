# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\inc\models.py
...
class PraisonAIModel:
    def __init__(self, model=None, api_key_var=None, base_url=None, api_key=None):
        ...
        logger.debug(
            f"Initialized PraisonAIModel with model {self.model_name}, api_key_var {self.api_key_var}, and base_url {self.base_url}"
        )
        ...
        if not self.api_key and self.base_url:
            ...
            is_local = parsed_url.hostname in ["localhost", "127.0.0.1"] or "api.openai.com" not in self.base_url
            if is_local:
