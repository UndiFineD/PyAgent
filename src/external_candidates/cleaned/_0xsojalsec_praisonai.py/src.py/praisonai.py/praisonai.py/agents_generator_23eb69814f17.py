# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\agents_generator.py
...
class AgentsGenerator:
    def __init__(
        ...
        if framework == "crewai" and not CREWAI_AVAILABLE:
            ...
            raise ImportError(
                "AutoGen is not installed. Please install it with 'pip install praisonai[autogen]' for v0.2 or 'pip install praisonai[autogen-v4]' for v0.4"
            )
        ...
        try:
            ...
            return {
                ...
                for name, obj in inspect.getmembers(
                    ...
                    and (
                        x.__module__.startswith("langchain_community.tools")
                        or (PRAISONAI_TOOLS_AVAILABLE and issubclass(x, BaseTool))
                    )
    ...
    def _run_autogen(self, config, topic, tools_dict):
        ...
        user_proxy = autogen.UserProxyAgent(
            ...
            is_termination_msg=lambda x: (x.get("content") or "").rstrip().rstrip(".").lower().endswith("terminate")
            ...
            for tool in details.get("tools", []):
                if tool in tools_dict:
                    try:
                        ...
                        self.logger.warning(
                            f"Warning: autogen_{type(tools_dict[tool]).__name__} function not found. Skipping this tool."
                        )
                        continue
        ...
        for role, details in config["roles"].items():
            ...
            if function_calling_llm_model:
                function_calling_llm = PraisonAIModel(
                    model=function_calling_llm_model.get("model")
    ...
    def _run_praisonai(self, config, topic, tools_dict):
        ...
        for role, details in config["roles"].items():
            ...
            agent = PraisonAgent(
                ...
                function_calling_llm=details.get("function_calling_llm", {}).get("model")
                or os.environ.get("MODEL_NAME")
                or "openai/gpt-5-nano",
                ...
                reflect_llm=details.get("reflect_llm", {}).get("model")
                or os.environ.get("MODEL_NAME")
                or "openai/gpt-5-nano",
        ...
        if config.get("process") == "hierarchical":
            ...
            agents = PraisonAIAgents(agents=list(agents.values()), tasks=tasks, verbose=True, memory=memory)
