# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\ui\sql_alchemy.py
import json
import os
import ssl
import uuid
...
class SQLAlchemyDataLayer(BaseDataLayer):
    def __init__(
        ...
        if storage_provider:
            ...
            logger.warning("SQLAlchemyDataLayer storage client is not initialized and elements will not be persisted!")
        ...
        async with self.async_session() as session:
            try:
                ...
                if result.returns_rows:
                    ...
                    assert isinstance(clean_json_result, list) or isinstance(clean_json_result, int)
    ...
    async def list_threads(self, pagination: Pagination, filters: ThreadFilter) -> PaginatedResponse:
        ...
        all_user_threads: Optional[List[ThreadDict]] = await self.get_all_user_threads(user_id=filters.userId) or []
