...
def scan_page(url, data=None):
    ...
    url, data = (
        re.sub(r"=(&|\Z)", "=1\g<1>", url) if url else url,
    ...
    try:
        for phase in (GET, POST):
            ...
            for match in re.finditer(r"((\A|[?&])(?P<parameter>[\w\[\]]+)=)(?P<value>[^&#]*)", current):
                ...
                prefix, suffix = (
                    "".join(random.sample(string.ascii_lowercase, PREFIX_SUFFIX_LENGTH)) for i in range(2)
                )
                for pool in (LARGER_CHAR_POOL, SMALLER_CHAR_POOL):
                    if not found:
                        ...
                        content = (
                            _retrieve_content(tampered, data) if phase is GET else _retrieve_content(url, tampered)
                        ).replace(
                                                    ...
                                                    (
                                                        (
                                                            ...
                                                            (
                                                                "no"
                                                                if all(
                                                                    char in sample.group(1) for char in LARGER_CHAR_POOL
                                                                )
                                                                else "some"
                                                            ),
                                                        ),
