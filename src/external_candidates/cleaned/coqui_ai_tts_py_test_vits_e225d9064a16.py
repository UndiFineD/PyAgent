# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\coqui_ai_tts.py\tests.py\tts_tests.py\test_vits_e225d9064a16.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\tts_tests\test_vits.py
...
from TTS.utils.audio.torch_transforms import (
    amp_to_db,
    db_to_amp,
    spec_to_mel,
    wav_to_mel,
...
class TestVits(unittest.TestCase):
    ...
    def _check_forward_outputs(self, config, output_dict, encoder_config=None, batch_size=2):
        self.assertEqual(
            output_dict["model_outputs"].shape[2],
            config.model_args.spec_segment_size * config.audio["hop_length"],
        )
        ...
        self.assertEqual(
            output_dict["z_p"].shape,
            (batch_size, config.model_args.hidden_channels, 30),
        )

        self.assertEqual(
            output_dict["m_p"].shape,
            (batch_size, config.model_args.hidden_channels, 30),
        )

        self.assertEqual(
            output_dict["logs_p"].shape,
            (batch_size, config.model_args.hidden_channels, 30),
        )

        self.assertEqual(
            output_dict["m_q"].shape,
            (batch_size, config.model_args.hidden_channels, 30),
        )

        self.assertEqual(
            output_dict["logs_q"].shape,
            (batch_size, config.model_args.hidden_channels, 30),
        )

        self.assertEqual(
            output_dict["waveform_seg"].shape[2],
            config.model_args.spec_segment_size * config.audio["hop_length"],
        )

        if encoder_config:
            self.assertEqual(
                output_dict["gt_spk_emb"].shape,
                (batch_size, encoder_config.model_params["proj_dim"]),
            )

            self.assertEqual(
                output_dict["syn_spk_emb"].shape,
                (batch_size, encoder_config.model_params["proj_dim"]),
            )
    ...
    def _check_inference_outputs(self, config, outputs, input_dummy, batch_size=1):
        ...
        self.assertEqual(
            outputs["z"].shape,
            (batch_size, config.model_args.hidden_channels, feat_len),
        )
    ...
    def test_train_eval_log(self):
        ...
        logger = TensorboardLogger(
            log_dir=os.path.join(get_tests_output_path(), "dummy_vits_logs"),
