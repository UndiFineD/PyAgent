# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2018.py\eop.py\chapter1.py\entire_brick_wall_313ed3317650.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\chapter1\entire_brick_wall.py
...
class EntireBrickWall(BrickRowScene, MovingCameraScene):
    ...
    def construct(self):
        ...
        for i in range(1, nb_rows + 1):
            ...
            anims = [rows[-1].shift, row_height * DOWN, Animation(rows[-2])]
        ...
        self.play(LaggedStartMap(FadeIn, tails_counters), FadeIn(nb_tails_text))
        ...
        highlighted_bricks = [highlighted_brick(row=20, nb_tails=i) for i in range(20)]

        self.wait()

        self.play(FadeIn(highlighted_bricks[10]))
        ...
        self.play(Transform(self.camera_frame, new_frame, run_time=9, rate_func=exponential_decay))
