# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\custom.py\opening_quote_9f5f980a6092.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\custom\opening_quote.py
    ...
    def get_quote(self, max_width=FRAME_WIDTH - 1):
        ...
        if isinstance(self.quote, str):
            if self.use_quotation_marks:
                quote = OldTexText("``%s''" % self.quote.strip(), **text_mobject_kwargs)
