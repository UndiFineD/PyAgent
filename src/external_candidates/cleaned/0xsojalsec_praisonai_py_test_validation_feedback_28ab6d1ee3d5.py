# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_praisonai.py\src.py\praisonai_agents.py\tests.py\test_validation_feedback_28ab6d1ee3d5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\test_validation_feedback.py
...
class TestValidationFeedback:
    ...
    def test_task_has_validation_feedback_field(self):
        ...
        task = Task(name="test_task", description="Test task", expected_output="Test output")
    ...
    def test_validation_feedback_captured_on_invalid_decision(self):
        ...
        collect_task.result = TaskOutput(raw="Collected only 3 items", agent="agent1")

        collect_task.status = "completed"

        validate_task.result = TaskOutput(raw="Not enough results, need at least 10", agent="agent2")
            ...
            if decision_str in Process.VALIDATION_FAILURE_DECISIONS:
                if current_task and current_task.result:
                    ...
                    feedback = {
                        ...
                        "rejected_output": (
                            validated_task.result.raw if validated_task and validated_task.result else None
                        ),
    ...
    def test_validation_feedback_included_in_context(self):
        ...
        process = Process(agents={}, tasks={"retry_task": task}, verbose=0)
    ...
    def test_validation_feedback_backward_compatibility(self):
        ...
        process = Process(agents={}, tasks={"normal_task": task}, verbose=0)
    ...
    def test_multiple_retry_decisions_supported(self):
        ...
        for decision in failure_decisions:
            task = Task(name="test_task", description="Test", expected_output="Test")
    ...
    def test_validation_feedback_with_context_tasks(self):
        ...
        validate_task = Task(
            ...
            condition={"valid": [], "invalid": ["data_task"]},
        )
        ...
        data_task.result = TaskOutput(raw="Generated data with errors", agent="agent1")

        data_task.status = "completed"

        validate_task.result = TaskOutput(raw="Data has errors, please fix", agent="agent2")
