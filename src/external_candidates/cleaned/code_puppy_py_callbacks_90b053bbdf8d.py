# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\callbacks_90b053bbdf8d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\callbacks.py
...
def register_callback(phase: PhaseType, func: CallbackFunc) -> None:
    if phase not in _callbacks:
        raise ValueError(f"Unsupported phase: {phase}. Supported phases: {list(_callbacks.keys())}")
    ...
    if func in _callbacks[phase]:
        logger.debug(f"Callback {func.__name__} already registered for phase '{phase}', skipping")
...
def unregister_callback(phase: PhaseType, func: CallbackFunc) -> bool:
    ...
    try:
        ...
        logger.debug(f"Unregistered async callback {func.__name__} from phase '{phase}'")
        ...
        try:
            ...
            if asyncio.iscoroutine(result):
                ...
                try:
                    ...
                    logger.warning(f"Async callback {callback.__name__} called from async context in sync trigger")
            ...
            logger.error(f"Callback {callback.__name__} failed in phase '{phase}': {e}\n{traceback.format_exc()}")
...
async def _trigger_callbacks(phase: PhaseType, *args, **kwargs) -> List[Any]:
    ...
    for callback in callbacks:
        try:
            ...
            logger.error(f"Async callback {callback.__name__} failed in phase '{phase}': {e}\n{traceback.format_exc()}")
...
async def on_pre_tool_call(tool_name: str, tool_args: dict, context: Any = None) -> List[Any]:
...
async def on_post_tool_call(
    ...
    return await _trigger_callbacks("post_tool_call", tool_name, tool_args, result, duration_ms, context)


async def on_stream_event(event_type: str, event_data: Any, agent_session_id: str | None = None) -> List[Any]:
    ...
    return await _trigger_callbacks("stream_event", event_type, event_data, agent_session_id)
...
def on_get_model_system_prompt(model_name: str, default_system_prompt: str, user_prompt: str) -> List[Dict[str, Any]]:
    ...
    return _trigger_callbacks_sync("get_model_system_prompt", model_name, default_system_prompt, user_prompt)
...
async def on_agent_response_complete(
    ...
    return await _trigger_callbacks("agent_response_complete", agent_name, response_text, session_id, metadata)
