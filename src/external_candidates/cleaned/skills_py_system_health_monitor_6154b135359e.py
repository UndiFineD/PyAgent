# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\scripts.py\system_health_monitor_6154b135359e.py
...
class SystemHealthMonitor:
    ...
    def register_component(self, component_name: str, health_check_func: Optional[Callable] = None) -> None:
        """
    ...
    def _determine_status(self, success_rate: float, response_time_ms: float) -> HealthStatus:
        """
        ...
        if error_rate >= self.ERROR_THRESHOLDS["degraded"] or response_time_ms > self.RESPONSE_TIME_THRESHOLDS["slow"]:
    ...
    def get_degradation_strategy(self) -> Dict:
        ...
        for name, comp in self.components.items():
            ...
            if comp_status == HealthStatus.FAILED:
                strategy["recommendations"].append(f"Component {name} has failed - disable dependent features")
                ...
                if fallback:
                    strategy["fallback_options"].append({"failed_component": name, "fallback": fallback})

            elif comp_status == HealthStatus.FAILING:
                strategy["recommendations"].append(f"Component {name} is failing - consider using fallback")

            elif comp_status == HealthStatus.DEGRADED:
                strategy["recommendations"].append(f"Component {name} is degraded - monitor closely")
        ...
        if status == HealthStatus.FAILED:
            ...
            strategy["recommendations"].insert(0, "WARNING: System health failing - enable fallback modes")

        elif status == HealthStatus.DEGRADED:
            strategy["recommendations"].insert(0, "CAUTION: System health degraded - increased monitoring recommended")
    ...
    def generate_health_report(self) -> str:
        ...
        if strategy["fallback_options"]:
            ...
            for option in strategy["fallback_options"]:
                report_lines.append(f"• {option['failed_component']} → {option['fallback']}")
