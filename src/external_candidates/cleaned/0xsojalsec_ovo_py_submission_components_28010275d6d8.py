# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_ovo.py\ovo.py\app.py\components.py\submission_components_28010275d6d8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\submission_components.py
...
from ovo.app.utils.bindcraft_utils import (
    get_dict_diff,
    load_json_from_file,
    merge_dictionaries,
)
...
def show_rfdiffusion_advanced_settings(workflow: RFdiffusionWorkflow):
    ...
    with st.expander(label="Show advanced settings"):
        ...
        workflow.refolding_params.primary_test = st.radio(
            ...
            key="refolding_test",
            index=(
                test_options.index(workflow.refolding_params.primary_test)
...
def show_bindcraft_advanced_settings(workflow: "BindCraftBinderDesignWorkflow"):
    ...
    with st.expander(label="Advanced settings and filter settings"):
        ...
        custom_filter, filter_edit_mode = customize_json_settings_component(
            diff=workflow.bindcraft_params.custom_filter_settings,
            default=filter_default_dict,
            key_suffix=filter_key,
        )
