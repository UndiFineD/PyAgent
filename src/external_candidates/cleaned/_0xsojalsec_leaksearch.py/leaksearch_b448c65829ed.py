# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LeakSearch\LeakSearch.py
#!/usr/bin/python3
# ============================#
...
def find_leaks_proxynova(email, proxy, number):
    ...
    if response.status_code == 200:
        ...
        print(
            colored(f"[*] Found {total_results} different records in database", "magenta"),
            end="",
        )
        ...
        print(
            colored(
                f"[!] Failed to fetch results from ProxyNova. Status code: {response.status_code}\n",
                "red",
            )
        )
        return []
        ...
        try:
            with open(database, "r") as file:
                while True:
                    ...
                    print(
                        colored(f"\r[*] Reading {line_count} lines in database..", "magenta"),
                        end="",
                        flush=True,
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-k",
        "--keyword",
        help="Keyword (user/domain/pass) to search for leaks in the DB",
    )
    parser.add_argument(
        "-n",
        "--number",
