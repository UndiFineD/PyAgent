# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graph_r1.py\verl.py\utils.py\dataset.py\rl_dataset_b0631225ffa6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\utils\dataset\rl_dataset.py
...
class ToolRLDataset(RLHFDataset):
    ...
    def __getitem__(self, item):
        ...
        if self.use_custom_tool_format_func:
            ...
            prompt_with_chat_template = self.tokenizer.apply_chat_template(
                chat, tools=self.tools, add_generation_prompt=True, tokenize=False
            )
    ...
    def _read_files_and_tokenize(self):
        ...
        self.dataframe = self.dataframe[
            self.dataframe.apply(
                lambda doc: len(
                    tokenizer.apply_chat_template(doc[prompt_key], tools=self.tools, add_generation_prompt=True)
                )
                <= self.max_prompt_length,
                axis=1,
            )
