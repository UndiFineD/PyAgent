# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2018.py\eop.py\chapter1.py\show_proportion_7051b2294d8a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\chapter1\show_proportion.py
...
class ShowProbAsProportion(Scene):
    def construct(self):
        ...
        prob_mob = ProbabilityRect(p0, unit_width=4, unit_height=2, use_percent=False, num_decimal_places=2)

        self.add(prob_mob)

        self.wait()

        self.play(ChangeProbability(prob_mob, p1, run_time=3))

        self.wait(0.5)

        self.play(ChangeProbability(prob_mob, p2, run_time=3))

        self.wait(0.5)

        self.play(ChangeProbability(prob_mob, p3, run_time=3))
