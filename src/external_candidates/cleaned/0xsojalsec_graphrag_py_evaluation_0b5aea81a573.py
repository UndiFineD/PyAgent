# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\utils.py\evaluation_0b5aea81a573.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Utils\Evaluation.py
...
def bleu_4_smooth(p, g):
    return sentence_bleu(g, p, weights=(0, 0, 0, 1), smoothing_function=SmoothingFunction().method1)
...
def metric_max_over_ground_truths(metric_fn, prediction, ground_truths, tokenize=False):
    ...
    if isinstance(score, dict) and "rougeL" in score:
        ...
        rouge_l_score["rouge_l f1"] = max([score["rougeL"].fmeasure for score in scores_for_ground_truths])

        rouge_l_score["rouge_l precision"] = max([score["rougeL"].precision for score in scores_for_ground_truths])

        rouge_l_score["rouge_l recall"] = max([score["rougeL"].recall for score in scores_for_ground_truths])
...
def get_metric_score(prediction, ground_truths):
    ...
    modify_bleu_4_score = metric_max_over_ground_truths(bleu_4_modify, prediction, ground_truths, tokenize=True)

    bleu_1_smooth_score = metric_max_over_ground_truths(bleu_1_smooth, prediction, ground_truths, tokenize=True)

    bleu_4_smooth_score = metric_max_over_ground_truths(bleu_4_smooth, prediction, ground_truths, tokenize=True)
...
class Evaluator:
    ...
    async def close_eval(self, df: pd.DataFrame):
        ...
        for index, row in df.iterrows():
            prompt = CLOSE_EXTRACT_OPTION_PORMPT.format(question=row["question"], model_output=row["output"])
        ...
        label_list, pred_list = self.get_label_pred_list(df, "extract_output", "answer_idx")
    ...
    def compute_mauve(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            human_data.append(
                " ".join((row["question"] + " " + row["answer"].strip()).split()[:100]).rstrip(string.punctuation)
            )

            model_data.append(
                " ".join((row["question"] + " " + row["output"].strip()).split()[:100]).rstrip(string.punctuation)
            )
