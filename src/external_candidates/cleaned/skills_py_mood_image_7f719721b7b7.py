# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\cpojer.py\hn_digest.py\scripts.py\mood_image_7f719721b7b7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cpojer\hn-digest\scripts\mood_image.py
...
def request_image(api_key: str, prompt: str, model: str, size: str, quality: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Images API failed ({e.code} {e.reason})" + (f": {payload}" if payload else "")) from e
