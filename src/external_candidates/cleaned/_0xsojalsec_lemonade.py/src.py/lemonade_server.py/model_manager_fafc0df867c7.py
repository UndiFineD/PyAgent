# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade_server\model_manager.py
import json
import os
import shutil
...
from lemonade.tools.llamacpp.utils import download_gguf, parse_checkpoint
...
class ModelManager:
    ...
    def supported_models(self) -> dict:
        ...
        server_models_file = os.path.join(os.path.dirname(__file__), "server_models.json")
        ...
        if os.path.exists(USER_MODELS_FILE):
            ...
            user_models = {f"user.{model_name}": model_info for model_name, model_info in user_models.items()}
            ...
            for _, model_info in user_models.items():
                if "reasoning" in model_info:
                    model_info["labels"] = (
                        ["reasoning"] if not model_info.get("labels", None) else model_info["labels"] + ["reasoning"]
                    )
            ...
            if base_checkpoint in downloaded_checkpoints:
                ...
                if variant and model_info.get("recipe") == "llamacpp":
                    try:
                        ...
                        snapshot_path = custom_snapshot_download(base_checkpoint, local_files_only=True)
    ...
    def download_models(
        ...
        for model in models:
            ...
            if "gguf" in checkpoint_to_download.lower():
                ...
                custom_snapshot_download(checkpoint_to_download, do_not_upgrade=do_not_upgrade)
    ...
    def filter_models_by_backend(self, models: dict) -> dict:
        ...
        hybrid_installed = (
            "onnxruntime-vitisai" in installed_packages and "onnxruntime-genai-directml-ryzenai" in installed_packages
        )
        filtered = {}
