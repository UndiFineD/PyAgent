# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade_server\cli.py
import argparse
import os
import sys
...
def serve(
    ...
    llamacpp_backend = llamacpp_backend if llamacpp_backend is not None else DEFAULT_LLAMACPP_BACKEND
...
def delete(model_names: List[str]):
    ...
    if server_running:
        ...
        for model_name in model_names:
            # Delete the model
            delete_response = requests.post(f"{base_url}/delete", json={"model_name": model_name})

            if delete_response.status_code != 200:
                raise DeleteError(f"Failed to delete {model_name}. Check the Lemonade Server log for more information.")
    else:
        from lemonade_server.model_manager import ModelManager
...
def load(model_name: str, port: int):
    ...
    if load_response.status_code != 200:
        raise ModelLoadError(f"Failed to load {model_name}. Check the Lemonade Server log for more information.")
...
def main():
    ...
    subparsers = parser.add_subparsers(title="Available Commands", dest="command", metavar="")
    ...
    # List command
    list_parser = subparsers.add_parser("list", help="List recommended models and their download status")

    # Pull command
    pull_parser = subparsers.add_parser(
        "pull",
        help="Install an LLM",
        epilog=("More information: https://github.com/lemonade-sdk/lemonade/blob/main/docs/server/server_models.md"),
    )
