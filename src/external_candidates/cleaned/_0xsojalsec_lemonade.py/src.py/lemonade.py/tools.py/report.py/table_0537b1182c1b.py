# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\report\table.py
import re
import textwrap
...
class SimpleStat(TableColumn):
    ...
    def get_str(self, build_stats, lean=False):
        ...
        cell_str = "\n".join([_wrap(f"{x:{self.format_str}}", self.wrap) for x in _to_list(data)])
        return cell_str
...
class AdditionalStat(TableColumn):
    ...
    def get_str(self, build_stats, lean=False):
        ...
        for stat in stats:
            ...
            value = f"{build_stats[stat]:{self.format_str}}" + (" " + units if not units is None else "")
...
class DictListStat(TableColumn):
    ...
    def get_str(self, build_stats, lean=False):
        ...
        for stat_dict in stat:
            line = [format_str.format(stat_dict[key]) for key, format_str in self.key_format_list]
...
class Table(ABC):
    ...
    def find_builds(self, cache_dirs: List[str], model: str = None):
        ...
        for cache_dir in cache_dirs:
            ...
            if model is not None:
                builds = [(cache_dir, build_name) for build_name in builds if model.lower() in build_name.lower()]
            else:
    ...
    def post_process_stats(self, model_stats: Dict) -> Dict:
        ...
        for key, value in model_stats.items():
            ...
            if (key == fs.Keys.BUILD_STATUS or fs.Keys.TOOL_STATUS in key) and value == build.FunctionStatus.INCOMPLETE:
    ...
    def __str__(self) -> str:
        ...
        stat_columns = [column for column, include in zip(stat_columns, stat_columns_include) if include]
    ...
    def sort_key(build_stats: dict) -> Tuple:
        ...
        return build_stats[fs.Keys.TIMESTAMP].strftime("%Y%m%d%H:%M:%S") if fs.Keys.TIMESTAMP in build_stats else "-"
...
class LemonadePerfTable(Table):
    table_descriptor = {
        ...
        "tool_columns": {
            OgaBench: [
                ...
                SimpleStat(_wrap("Memory Used (GB)", 8), Keys.MAX_MEMORY_USED_GBYTE, ".3f"),
            ],
    ...
    def include_stats(self, model_stats) -> bool:
        ...
        if not model_stats.get(fs.Keys.BUILD_STATUS, None) == build.FunctionStatus.SUCCESSFUL:
            return False
    ...
    def post_process_stats(self, model_stats) -> bool:
        ...
        for tool in tool_columns.keys():
            ...
            if model_stats.get(tool_status_key, None) == build.FunctionStatus.SUCCESSFUL:
                # Successful build of this tool, so remember this tool
                self.tools_found_set.add(tool)

                # Extract the declared tool stats
                data = data | {stat: model_stats.get(stat, None) for stat in tool().status_stats}

                # Find if there are any additional stats for this tool
                # First see if this tool refers to another tool
                if not isinstance(tool_columns[tool], list):
                    tool = tool_columns[tool]
                regexp_list = [stat.regexp for stat in tool_columns[tool] if isinstance(stat, AdditionalStat)]
                match_expr = "(?:% s)" % "|".join(regexp_list)
                additional_stats = [stat for stat in model_stats.keys() if re.match(match_expr, stat)]
        ...
        if "Driver Versions" in data[fs.Keys.SYSTEM_INFO]:
            sw_versions += [key + ": " + value for key, value in data[fs.Keys.SYSTEM_INFO]["Driver Versions"].items()]
