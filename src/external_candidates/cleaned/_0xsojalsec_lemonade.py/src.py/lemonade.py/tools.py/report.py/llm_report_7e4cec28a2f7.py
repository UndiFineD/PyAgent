# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\report\llm_report.py
import argparse
import csv
import os
import re
...
from lemonade.tools.report.table import LemonadePerfTable, LemonadeTable
...
class LemonadeReport(ManagementTool):
    ...
    def parser(add_help: bool = True) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "-o",
            "--output-dir",
            help="Path to folder where reports will be saved (defaults to current working directory)",
            required=False,
            default=os.getcwd(),
        )
        ...
        parser.add_argument(
            "--device",
            default=None,
            help="In the --perf table, only include output for the specified device (e.g., cpu, igpu, npu, hybrid)",
        )

        parser.add_argument(
            "--dtype",
            default=None,
            help="In the --perf table, only include output for the specified datatype (e.g., float32, int4)",
        )
