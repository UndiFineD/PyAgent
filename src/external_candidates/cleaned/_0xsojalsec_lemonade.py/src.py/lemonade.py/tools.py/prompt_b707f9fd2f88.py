# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\prompt.py
import argparse
import os
...
class LLMPrompt(Tool):
    ...
    def parser(add_help: bool = True) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            ...
            type=int,
            help=f"Maximum number of new tokens in the response (default is {DEFAULT_MAX_NEW_TOKENS})",
        )
    ...
    def run(
        ...
        if template:
            ...
            if not hasattr(tokenizer, "prompt_template"):
                printing.log_warning("Templates for this model type are not yet implemented.")
        ...
        for trial in range(n_trials):
            ...
            while counter < len_input_ids and input_ids_array[counter] == response_array[counter]:
                counter += 1

            # Only decode the actual response (not the prompt)
            response_text = tokenizer.decode(response_array[counter:], skip_special_tokens=True).strip()
