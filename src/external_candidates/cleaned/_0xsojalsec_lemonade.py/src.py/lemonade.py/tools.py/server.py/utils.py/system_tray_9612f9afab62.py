# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\server\utils\system_tray.py
import os
import sys
...
class SystemTray:
    ...
    def add_tray_icon(self):
        ...
        hicon = win32gui.LoadImage(self.hinst, str(self.icon_path), win32con.IMAGE_ICON, 0, 0, icon_flags)
    ...
    def create_menu_item(self, menu_handle, item, pos):
        ...
        if item.submenu:
            ...
            for submenu_item in item.submenu.items:
                submenu_pos = self.create_menu_item(submenu_handle, submenu_item, submenu_pos)

            # Add submenu to parent menu
            win32gui.AppendMenu(menu_handle, win32con.MF_POPUP | flags, submenu_handle, item.text)
        else:
    ...
    def setup_console_control_handler(self, logger=None):
        ...
        def console_ctrl_handler(ctrl_type):
            if ctrl_type in (0, 2):  # CTRL_C_EVENT or CTRL_CLOSE_EVENT
                if logger:
                    logger.info("Received console control event, shutting down gracefully")
