# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\server\tool_calls.py
import json
import logging
import re
...
def get_tool_call_pattern(added_tokens_decoder: List[str]) -> Optional[Pattern]:
    ...
    # Sample model that uses this pattern: Mistral-7B-Instruct-v0.3
        ...
        logging.warning("Tool calling identifiers were not found for the current model.")
        return None


def extract_tool_calls(text: str, tool_call_pattern: Optional[Pattern] = None) -> tuple[List[Dict], str]:
    """
