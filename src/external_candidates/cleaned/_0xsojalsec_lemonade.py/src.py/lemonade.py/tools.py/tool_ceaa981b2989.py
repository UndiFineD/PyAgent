# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\tool.py
import abc
import argparse
import os
import re
...
def _name_is_file_safe(name: str):
    ...
    allowed_in_unique_name = set("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-")
    ...
    def _split_lines(self, text, width):
        ...
        for idx, line in enumerate(textRows):
            ...
            if line.strip() == "":
                ...
                lines = [
                    self.__add_whitespace(i, whitespace_needed, x) for i, x in enumerate(_textwrap.wrap(line, width))
                ]
                textRows[idx] = lines
...
class ToolParser(argparse.ArgumentParser):
    ...
    def __init__(self, short_description: str, description: str, prog: str, epilog: str, **kwargs):
        super().__init__(
...
class Tool(abc.ABC):
    ...
    def set_percent_progress(self, percent_progress: float):
        ...
        if percent_progress is not None and not isinstance(percent_progress, float):
            raise ValueError(f"Input argument must be a float or None, got {percent_progress}")
    ...
    def run_helper(self, state: State, monitor: bool = False, **kwargs) -> Tuple[State, int]:
        """
        ...
        if monitor:
            ...
            self.progress = Process(target=_spinner, args=(self.monitor_message, self.progress_queue))
            self.progress.start()
