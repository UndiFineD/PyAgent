# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\perplexity.py
import argparse
import os
...
class AccuracyPerplexity(Tool):
    ...
    def run(
        ...
        model_results_dir = os.path.join(build.output_dir(state.cache_dir, state.build_name), "perplexity")
        ...
        for begin_location in range(0, seq_len, stride):
            ...
            with torch.no_grad():
                try:
                    ...
                    printing.log_error(f"Error during model forward pass execution: {e}")

            # Compute loss manually for visualization
            shift_logits = logits[..., :-1, :].contiguous()
            shift_labels = target_ids[..., 1:].contiguous()
            effective_token_count = (target_ids != -100).sum().item()
            negative_log_likelihoods.append((outputs.loss.item(), effective_token_count))
            ...
            predicted_words = tokenizer.batch_decode(predicted_tokens, skip_special_tokens=True)
            actual_words = tokenizer.batch_decode(actual_tokens, skip_special_tokens=True)
        ...
        summary_df.to_csv(os.path.join(model_results_dir, "summary_results.csv"), index=False)
        return state
