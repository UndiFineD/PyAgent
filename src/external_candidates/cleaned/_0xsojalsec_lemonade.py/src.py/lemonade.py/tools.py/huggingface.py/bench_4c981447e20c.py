# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\huggingface\bench.py
import argparse
import statistics
...
class HuggingfaceBench(Bench):
    ...
    def get_prompt_str(self, state, token_length):
        ...
        input_ids = tokenizer(test_prompt, return_tensors="pt").to(device=model.device).input_ids
    ...
    def run_prompt(
        ...
        if self.first_run_prompt:
            if vars(state).get(Keys.MODEL) is None:
                raise ValueError(f"{self.__class__.__name__} requires that a model be passed from another tool")
        ...
        input_ids = tokenizer(prompt, return_tensors="pt").to(device=model.device).input_ids
        ...
        time_to_first_token_per_iteration = [latency for latency, _ in prefill_per_iteration_result]
        ...
        self.prefill_tokens_per_second_list.append(input_ids.shape[1] / mean_time_to_first_token)
        try:
            self.std_dev_time_to_first_token_list.append(statistics.stdev(time_to_first_token_per_iteration))
        ...
        execution_latency_per_iteration = [latency for latency, _ in decode_per_iteration_result]
        token_len_per_iteration = [token_len for _, token_len in decode_per_iteration_result]
        ...
        self.token_generation_tokens_per_second_list.append((mean_token_len - 1) / mean_decode_latency)
