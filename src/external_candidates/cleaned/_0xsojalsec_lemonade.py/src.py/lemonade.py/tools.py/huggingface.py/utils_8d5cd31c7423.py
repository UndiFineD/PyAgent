# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\huggingface\utils.py
import time
...
class HuggingfaceAdapter(ModelAdapter):
    ...
    def _select_cont_toks(self, logits, context_len, cont_toks):
        ...
        token_log_probs = torch.gather(log_probs, 1, cont_toks.unsqueeze(-1)).squeeze(-1)
...
def benchmark_huggingface_llm(
    ...
    with torch.cpu.amp.autocast(enabled=amp_enabled, dtype=dtype) if amp_enabled else nullcontext():
        ...
        with torch.no_grad(), torch.inference_mode():
            ...
            for count in range(iterations):
                ...
                report_progress_fn((warmup_iterations + count + 1) / (warmup_iterations + iterations))
