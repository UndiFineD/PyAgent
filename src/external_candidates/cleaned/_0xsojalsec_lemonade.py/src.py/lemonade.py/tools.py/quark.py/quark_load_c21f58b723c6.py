...
class QuarkLoad(Tool):
    ...
    def parser(add_help: bool = True) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--torch-compile", action="store_true", help="Model torch compile")
        ...
        parser.add_argument("--params-load", action="store_true", help="Model parameters load")
    ...
    def run(
        ...
        try:
            ...
            if not params_load and not safetensors_model_reload:
                raise ValueError(" Specify load format: 'params_load' or 'safetensors_model_reload'.")

            # Reload quantized model if specified
            from quark.torch import import_model_info, load_params

            if params_load:
                printing.log_info("Restoring quantized model from JSON/safetensors files")
                model = load_params(
                    model,
                    json_path=json_path,
                ...
                printing.log_info("Restoring quantized model from quark_safetensors files")
