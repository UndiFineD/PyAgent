# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\quark\quark_quantize.py
import argparse
import os
import sys
...
class QuarkQuantize(Tool):
    ...
    def parser(add_help: bool = True) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--seq-len", type=int, default=512, help="Sequence length of data")
        ...
        parser.add_argument("--output-dir", default=None, help="Output directory for exported model")
        parser.add_argument(
        ...
        parser.add_argument("--params-save", action="store_true", help="Save model params")
        parser.add_argument(
            "--save-dir",
            help="Directory to save model parameters as \
            safetensors or pth, in the case when --params_save is used.",
        )
        parser.add_argument("--log-severity-level", type=int, default=3, help="DEBUG=1, INFO=2, ERROR=3")
    ...
    def run(self, state: State, **kwargs) -> State:
        ...
        try:
            if os.path.isdir(DEFAULT_QUARK_DIR):
                quark_llm_path = os.path.join(DEFAULT_QUARK_DIR, "examples", "torch", "language_modeling")
            ...
            model_build_path = os.path.join(build.output_dir(state.cache_dir, state.build_name))
    ...
    def _quantize(self, state: State, **kwargs) -> None:
        ...
        from llm_utils.model_preparation import get_model_type
        from quark.torch import ModelExporter, ModelQuantizer, save_params
        ...
        if not kwargs.get("skip_quantization"):
            ...
            if kwargs.get("model_export") is not None or kwargs.get("params_save") or kwargs.get("torch_compile"):
        ...
        if kwargs.get("model_export") is not None:
            ...
            if "gguf" in kwargs.get("model_export"):
                ...
                with torch.inference_mode():
                    exporter.export_gguf_model(model, kwargs.get("model_dir"), model_type)
