# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\management_tools.py
import abc
import argparse
import json
...
class Cache(ManagementTool):
    ...
    def parser(add_help: bool = True) -> argparse.ArgumentParser:
        ...
        parser = __class__.helpful_parser(
            short_description=f"Manage the build cache ",
            add_help=add_help,
        )
    ...
    def run(
        ...
        if all_builds and build_names:
            raise ValueError("all_builds and build_names are mutually exclusive, but both are used in this call.")
        elif all_builds:
            builds = fs.get_available_builds(cache_dir)
        elif build_names:
            builds = build_names
        else:
            raise ValueError("Either all_builds or build_names must be set, but this call sets neither.")
...
class SystemInfo(ManagementTool):
    ...
    def run(self, _, format="table", verbose=False):
        ...
        if not verbose:
            ...
            system_info_dict = {k: v for k, v in system_info_dict.items() if k in essential_keys}
        else:
