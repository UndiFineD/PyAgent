# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\oga\utils.py
import json
import logging
import os
import time
...
class OrtGenaiModel(ModelAdapter):
        ...
        try:
            ...
            if os.path.exists(rai_config_path):
                with open(rai_config_path, "r", encoding="utf-8") as f:
                    ...
                    if "max_prompt_length" in rai_config and detected_version in rai_config["max_prompt_length"]:
                        max_prompt_length = rai_config["max_prompt_length"][detected_version]
    ...
    def generate(
        ...
        use_oga_post_6_api = Version(og.__version__) >= Version("0.6.0") or "0.6.0" in og.__version__
        ...
        top_k_to_use = top_k if top_k is not None else search_config.get("top_k", default_top_k)
        top_p_to_use = top_p if top_p is not None else search_config.get("top_p", default_top_p)
        temperature_to_use = (
            temperature if temperature is not None else search_config.get("temperature", default_temperature)
        )
    ...
    def _select_cont_toks(self, logits, context_len, continuation_tokens):
        ...
        cont_logits = logits[context_len - 1 : context_len - 1 + len(continuation_tokens)]
        ...
        token_log_probs = torch.gather(log_probs, 1, continuation_tokens.unsqueeze(-1)).squeeze(-1)

        return token_log_probs

    def compute_logprobs(self, text, tokenizer, prompt_length=None, logprobs=None, echo=False):
        """
        ...
        for i in range(len(tokens)):
            ...
            if i > 0:  # First token has no preceding context
                ...
                if logprobs is not None:
                    ...
                    topk_values, topk_indices = torch.topk(prev_logits, min(num_alternatives, prev_logits.size(-1)))
