# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\llamacpp\load.py
import argparse
import os
...
class LoadLlamaCpp(FirstTool):
    ...
    def run(
        ...
        if os.path.isdir(input):
            ...
            if len(gguf_files) == 0:
                raise ValueError(f"The folder {input} does not contain a GGUF model file.")
            ...
            offline = is_offline()
            if offline:
                printing.log_warning("Network connectivity to huggingface.co not detected. Running in offline mode.")
                full_model_path, model_to_use = get_local_checkpoint_path(base_checkpoint, variant)
                if not full_model_path:
                    raise ValueError(f"Model {checkpoint} is not available locally.Cannot download in offline mode.")
        ...
        state.save_stat(Keys.LLAMA_CLI_VERSION_INFO, get_llama_installed_version(backend))
