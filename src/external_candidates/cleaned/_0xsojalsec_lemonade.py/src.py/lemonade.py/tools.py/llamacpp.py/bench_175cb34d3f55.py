# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\tools\llamacpp\bench.py
import argparse
import statistics
...
class LlamaCppBench(Bench):
    ...
    def run_prompt(
        ...
        for iteration in range(iterations + warmup_iterations):
            try:
                ...
                raw_output, stderr = model.generate(prompt, max_new_tokens=output_tokens, return_raw=True)
        ...
        self.prefill_tokens_per_second_list.append(model.prompt_tokens / mean_time_to_first_token)
        self.token_generation_tokens_per_second_list.append(statistics.mean(per_iteration_tokens_per_second))
        try:
    ...
    def save_stats(self, state):
        ...
        if not all(element is None for element in self.std_dev_token_generation_tokens_per_second_list):
            state.save_stat(
                Keys.STD_DEV_TOKENS_PER_SECOND,
                self.get_item_or_list(self.std_dev_token_generation_tokens_per_second_list),
            )
