# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\profilers\memory_tracker.py
import os
import textwrap
import time
...
class MemoryTracker(Profiler):
    ...
    def generate_results(self, state, timestamp, _):
        ...
        try:
            ...
            printing.log_info(f"Memory tracker file not found: {e.filename}.  No memory usage plot generated")
        ...
        if not memory_tracks or not isinstance(memory_tracks, list):
            printing.log_info(f"Memory tracker file contains no data or is improperly formatted: {self.yaml_path}")
            state.save_stat(fs.Keys.MEMORY_USAGE_PLOT, None)
            return
        ...
        plot_path = os.path.join(self.build_dir, f"{timestamp}_{MEMORY_USAGE_PNG_FILENAME}")
        plt.savefig(plot_path)
        plot_path = os.path.join(os.getcwd(), f"{timestamp}_{MEMORY_USAGE_PNG_FILENAME}")
        plt.savefig(plot_path)
    ...
    def _memory_tracker_(
        ...
        try:
            ...
            while not tracker_exit and tracked_process.status() == psutil.STATUS_RUNNING:
                    ...
                    try:
                        ...
                        if message is None or isinstance(message, str):
                            ...
                            if track_name is not None:
                                ...
                                raise TypeError("Track name must be passed to memory tracker prior to sending data")
                        else:
                            raise TypeError(f"Unrecognized message type in memory_tracker input queue: {message}")
                ...
                if not tracker_exit and track_name is not None:
                    ...
                    current_track.append(MemoryTracker.get_time_mem_list(tracked_process))
