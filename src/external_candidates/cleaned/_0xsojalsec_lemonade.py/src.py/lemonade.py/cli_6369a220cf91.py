# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\cli.py
import os
...
from lemonade.tools.management_tools import Cache, SystemInfo, Version
...
https://github.com/lemonade-sdk/lemonade/blob/main/docs/README.md""",
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="The input that will be evaluated by the starting tool (e.g., huggingface checkpoint)",
    )

    parser.add_argument(
        "-d",
        "--cache-dir",
        help=f"Cache directory where tool results are stored (default: {cache.DEFAULT_CACHE_DIR})",
        required=False,
    ...
    global_args, tool_instances, evaluation_tools = cli.parse_tools(parser, tools, cli_name="lemonade")
