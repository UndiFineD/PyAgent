# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\api.py
...
from lemonade.common.system_info import get_system_info as get_system_info_obj
from lemonade.common.system_info import (
...
def _raise_not_supported(recipe, checkpoint):
    raise NotSupported(f"Recipe {recipe} does not have support for checkpoint {checkpoint}")
...
def from_pretrained(
    ...
    if recipe == "hf-cpu":
        ...
        if recipe not in supported_recipes:
            raise NotSupported(
                f"Selected OGA recipe is not supported. The supported OGA recipes are: {supported_recipes}"
            )
