# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\common\cli_helpers.py
import argparse
import sys
from typing import Any, Dict, List, Tuple

import lemonade.common.printing as printing
from lemonade.tools import FirstTool, Tool
...
def _tool_list_help(tools: List[Tool], subclass, exclude=None) -> str:
    ...
    for tool_class in tools:
        ...
        if issubclass(tool_class, subclass):
            help = help + f" * {tool_class.unique_name}: {tool_class.parser().short_description}\n"
...
{mgmt_tool_choices}""",
    ...
    if len(management_tools) == 0 and len(evaluation_tools) == 0:
        parser.error("Calls to lemonade are required to call at least one tool or management tool.")
