# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\common\filesystem.py
import glob
import os
import pathlib
import shutil
...
def get_all(path, exclude_path=False, file_type=build.state_file_name, recursive=True):
    if recursive:
        files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(path) for f in filenames if file_type in f]
    else:
        files = []
...
def clean_file_name(script_path: str) -> str:
    ...
    if script_path.endswith("_" + build.state_file_name):
        return pathlib.Path(script_path).stem.replace("_" + os.path.splitext(build.state_file_name)[0], "")
    else:
...
def get_available_builds(cache_dir):
    ...
    builds = [
        ...
        if os.path.isdir(build.output_dir(cache_dir, build_name)) and is_build_dir(cache_dir, build_name)
    ]
    builds.sort()
...
def expand_inputs(input_paths: List[str]) -> List[str]:
    ...
    """
    input_paths_expanded = sum([glob.glob(f) for f in input_paths if "::" not in f], []) + [
