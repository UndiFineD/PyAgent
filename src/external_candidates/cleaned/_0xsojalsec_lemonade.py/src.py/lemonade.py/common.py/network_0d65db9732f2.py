# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\src\lemonade\common\network.py
import os
import socket
...
def custom_snapshot_download(repo_id, do_not_upgrade=False, **kwargs):
    ...
    if do_not_upgrade:
        try:
            ...
            return _symlink_safe_snapshot_download(repo_id, local_files_only=True, **kwargs)
        except LocalEntryNotFoundError:
            # LocalEntryNotFoundError means there was no local model, at this point
            # we'll accept a remote model
            return _symlink_safe_snapshot_download(repo_id, local_files_only=False, **kwargs)
    else:
