# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\test\server_llamacpp.py
"""
...
class LlamaCppTesting(ServerTestingBase):
    ...
    def test_000_get_hip_devices_returns_zero(self):
        ...
        expected_devices = (
            [[0, "AMD Radeon(TM) 8060S Graphics"]] if sys.platform.startswith("win") else [[0, "AMD Radeon Graphics"]]
        )
    ...
    def test_003_test_embeddings_with_gguf(self):
        ...
        assert sim_12 > sim_13, f"Semantic similarity test failed: {sim_12:.4f} <= {sim_13:.4f}"
    ...
    def test_007_test_generation_parameters_with_llamacpp(self):
        ...
        def make_request(endpoint_type, **params):
            ...
            extra_body = {
                "repeat_penalty": params.get("repeat_penalty", BASE_PARAMS["repeat_penalty"]),
        ...
        for endpoint in endpoints:
            ...
            for param_name, variant_value in PARAM_VARIANTS.items():
                ...
                assert response_modified != response1, (
