# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-lemonade\test\oga_cpu_api.py
import os
import shutil
import sys
import unittest
...
class Testing(unittest.TestCase):
    ...
    def test_001_ogaload(self):
        ...
        state = OgaLoad().run(state, input=checkpoint, device=device, dtype=dtype, force=force)
    ...
    def test_004_accuracy_lmeval(self):
        ...
        for metric_name in lm_eval_stats:
            if metric_name.endswith("_units"):
                ...
                assert isinstance(stats[metric_name], str), f"{metric_name} should be a string (units)"
            else:
                # All other lm_eval stats should be numeric
                assert isinstance(stats[metric_name], (int, float)), f"{metric_name} should be numeric"
...
class TestOgaLogprobs(unittest.TestCase):
    ...
    def test_001_compute_logprobs_completion(self):
        ...
        result = state.model.compute_logprobs(text=text, tokenizer=state.tokenizer, logprobs=5)
    ...
    def test_002_compute_logprobs_echo_parameter(self):
        ...
        text_offset_no_echo, token_logprobs_no_echo, tokens_no_echo, _ = state.model.compute_logprobs(
            text=full_text,
        ...
        # Test with echo=True
        text_offset_with_echo, token_logprobs_with_echo, tokens_with_echo, _ = state.model.compute_logprobs(
            text=full_text,
            tokenizer=state.tokenizer,
            prompt_length=prompt_length,
            logprobs=5,
...
if __name__ == "__main__":
    cache_dir, _ = common.create_test_dir("lemonade_oga_cpu_api", base_dir=os.path.abspath("."))
