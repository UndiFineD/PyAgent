...
class Testing(ServerTestingBase):
    ...
    def test_000_endpoints_available(self):
        ...
        for endpoint in valid_endpoints:
            for version in ["v0", "v1"]:
                ...
                response = requests.get(url)
                assert response.status_code != 404, f"Endpoint {endpoint} is not registered on {version}"
    ...
    def test_004_test_models(self):
        ...
        assert any(
            model.id == MODEL_NAME and model.checkpoint == MODEL_CHECKPOINT and model.recipe == "oga-cpu"
            for model in l.data
        )
    ...
    async def test_001_test_simultaneous_load_requests(self):
        async with httpx.AsyncClient(base_url=self.base_url, timeout=120.0) as client:
            ...
            assert health_data["model_loaded"] == second_model, health_data["model_loaded"]
    ...
    def test_015_test_responses_streaming(self):
        ...
        for event in stream:
            ...
            # Ensure the complete event matches the streamed response
                ...
                assert event.response.output[0].content[0].text == complete_response, (
    ...
    async def test_019_test_delete_model(self):
        ...
        async with httpx.AsyncClient(base_url=self.base_url, timeout=120.0) as client:
            ...
            assert MODEL_NAME in model_ids, f"Model {MODEL_NAME} not found in models list"

            # Test deleting the model
            delete_response = await client.post("/delete", json={"model_name": MODEL_NAME})
            ...
            models_data = models_response.json()
            model_ids = [model["id"] for model in models_data["data"]]
            assert MODEL_NAME not in model_ids, f"Model {MODEL_NAME} still found in models list after deletion"

            # Test deleting a non-existent model (should return error)
            delete_response = await client.post("/delete", json={"model_name": "NonExistentModel"})
    ...
    def test_020_test_system_info_endpoint(self):
        ...
        response = requests.get(url)
        assert response.status_code == 200, f"System info endpoint failed with status {response.status_code}"
        ...
        assert verbose_response.status_code == 200, (
