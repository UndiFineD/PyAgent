...
# Define the paths
json_file_path = os.path.join(os.path.dirname(__file__), "..", "src", "lemonade_server", "server_models.json")
markdown_file_path = os.path.join(os.path.dirname(__file__), "server", "server_models.md")
    ...
    for model_name, details in models:
        ...
        for key, value in details.items():
            if key == "checkpoint":
                ...
                section += f"<tr><td>{key.capitalize()}</td><td>{labels_str}</td></tr>\n"
