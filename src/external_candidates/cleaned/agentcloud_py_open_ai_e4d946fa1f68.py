# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentcloud.py\agent_backend.py\src.py\chat.py\agents.py\open_ai_e4d946fa1f68.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\chat\agents\open_ai.py
...
class OpenAIChatAgent(BaseChatAgent):
    ...
    async def call_model(self, state, config):
        ...
        if (isinstance(messages[-1], ToolMessage) and messages[-1].name == "human_input") and (
            isinstance(response, AIMessage) and response.tool_calls and response.tool_calls[0]["name"] == "human_input"
        ):
