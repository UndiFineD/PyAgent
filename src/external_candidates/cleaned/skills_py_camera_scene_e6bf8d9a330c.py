# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\inclinedadarsh.py\manimce_best_practices.py\templates.py\camera_scene_e6bf8d9a330c.py
...
class YourCameraScene(MovingCameraScene):
    ...
    def construct(self):
        ...
        shapes = VGroup(
            *[Circle(radius=0.3, color=color, fill_opacity=0.5) for color in [RED, BLUE, GREEN, YELLOW, PURPLE]]
        ).arrange(RIGHT, buff=1)

        # Add labels

        labels = VGroup(*[Text(str(i + 1), font_size=24).move_to(shape) for i, shape in enumerate(shapes)])
