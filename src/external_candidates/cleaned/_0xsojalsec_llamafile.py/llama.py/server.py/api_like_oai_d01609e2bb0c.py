# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-llamafile\llama.cpp\server\api_like_OAI.py
#!/usr/bin/env python3
import argparse
import json
import time
...
parser.add_argument(
    "--user-name",
    type=str,
    help="USER name in chat completions(default: '\\nUSER: ')",
    default="\\nUSER: ",
)
parser.add_argument(
    "--ai-name",
    type=str,
    help="ASSISTANT name in chat completions(default: '\\nASSISTANT: ')",
    default="\\nASSISTANT: ",
)
parser.add_argument(
    "--system-name",
    type=str,
    help="SYSTEM name in chat completions(default: '\\nASSISTANT's RULE: ')",
    default="\\nASSISTANT's RULE: ",
)
parser.add_argument(
    "--stop",
...
def make_postData(body, chat=False, stream=False):
    ...
    if is_present(body, "temperature"):
        postData["temperature"] = body["temperature"]
    if is_present(body, "top_k"):
        postData["top_k"] = body["top_k"]
    if is_present(body, "top_p"):
        postData["top_p"] = body["top_p"]
    if is_present(body, "max_tokens"):
        postData["n_predict"] = body["max_tokens"]
    if is_present(body, "presence_penalty"):
        postData["presence_penalty"] = body["presence_penalty"]
    if is_present(body, "frequency_penalty"):
        postData["frequency_penalty"] = body["frequency_penalty"]
    if is_present(body, "repeat_penalty"):
        postData["repeat_penalty"] = body["repeat_penalty"]
    if is_present(body, "mirostat"):
        postData["mirostat"] = body["mirostat"]
    if is_present(body, "mirostat_tau"):
        postData["mirostat_tau"] = body["mirostat_tau"]
    if is_present(body, "mirostat_eta"):
        postData["mirostat_eta"] = body["mirostat_eta"]
    if is_present(body, "seed"):
        postData["seed"] = body["seed"]
    if is_present(body, "logit_bias"):
        postData["logit_bias"] = [[int(token), body["logit_bias"][token]] for token in body["logit_bias"].keys()]
    if args.stop != "":
...
def make_resData_stream(data, chat=False, time_now=0, start=False):
    ...
    if chat:
        if start:
            ...
            resData["choices"][0]["delta"] = {"content": data["content"]}
            if data["stop"]:
                resData["choices"][0]["finish_reason"] = (
...
def chat_completions():
    ...
    if is_present(body, "stream"):
        stream = body["stream"]
    if is_present(body, "tokenize"):
        tokenize = body["tokenize"]
    postData = make_postData(body, chat=True, stream=stream)

    promptToken = []
    if tokenize:
        tokenData = requests.request(
            "POST",
            urllib.parse.urljoin(args.llama_api, "/tokenize"),
            data=json.dumps({"content": postData["prompt"]}),
        ).json()
    ...
    if not stream:
        data = requests.request(
            "POST",
            urllib.parse.urljoin(args.llama_api, "/completion"),
