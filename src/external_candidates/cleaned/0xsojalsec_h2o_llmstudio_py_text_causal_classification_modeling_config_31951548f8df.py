# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\python_configs.py\text_causal_classification_modeling_config_31951548f8df.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\python_configs\text_causal_classification_modeling_config.py
...
class ConfigNLPCausalClassificationDataset(ConfigNLPCausalLMDataset):
    dataset_class: Any = llm_studio.src.datasets.text_causal_classification_ds.CustomDataset
...
class ConfigNLPCausalClassificationEnvironment(ConfigNLPCausalLMEnvironment):
    ...
    _summary_card_template: str = "text_causal_classification_experiment_summary_card_template.md"
...
class ConfigNLPCausalClassificationLogging(ConfigNLPCausalLMLogging):
    plots_class: Any = llm_studio.src.plots.text_causal_classification_modeling_plots.Plots
...
class ConfigProblemBase(DefaultConfigProblemBase):
    ...
    dataset: ConfigNLPCausalClassificationDataset = field(default_factory=ConfigNLPCausalClassificationDataset)

    tokenizer: ConfigNLPCausalClassificationTokenizer = field(default_factory=ConfigNLPCausalClassificationTokenizer)

    architecture: ConfigNLPCausalClassificationArchitecture = field(
        default_factory=ConfigNLPCausalClassificationArchitecture
    )

    training: ConfigNLPCausalClassificationTraining = field(default_factory=ConfigNLPCausalClassificationTraining)

    augmentation: ConfigNLPCausalClassificationAugmentation = field(
        default_factory=ConfigNLPCausalClassificationAugmentation
    )

    prediction: ConfigNLPCausalClassificationPrediction = field(default_factory=ConfigNLPCausalClassificationPrediction)

    environment: ConfigNLPCausalClassificationEnvironment = field(
        default_factory=ConfigNLPCausalClassificationEnvironment
    )

    logging: ConfigNLPCausalClassificationLogging = field(default_factory=ConfigNLPCausalClassificationLogging)
    ...
    def check(self) -> dict[str, list]:
        ...
        if isinstance(self.dataset.answer_column, str):
            ...
            errors["message"].append(
                "Providing the answer_column as a string is deprecated. Please provide the answer_column as a list."
            )
        ...
        if len(self.dataset.answer_column) > 1:
            if self.training.loss_function == "CrossEntropyLoss":
                errors["title"] += ["CrossEntropyLoss not supported for multilabel classification"]

                errors["message"] += [
                    "CrossEntropyLoss requires a single multi-class answer column, but multiple answer columns are set."
                ]
            ...
            if self.training.loss_function == "CrossEntropyLoss":
                if self.dataset.num_classes == 1:
                    ...
                    errors["message"] += ["CrossEntropyLoss requires num_classes > 1, but num_classes is set to 1."]
        ...
        if self.dataset.parent_id_column not in ["None", None]:
            ...
            errors["message"] += ["Parent ID column is not supported for classification datasets."]
