# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WindTerm\src\Onigmo\testpy.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
def xx(pattern, target, s_from, s_to, mem, not_match):
    ...
    if r != 0:
        ...
        nerror += 1
        print_result("ERROR", "%s (/%s/ '%s')" % (msg.value, pattern, target), file=sys.stderr)
        return
    ...
    if r == onig.ONIG_MISMATCH:
        ...
        if not_match:
            ...
            if (start == s_from) and (end == s_to):
                ...
                print_result(
                    "FAIL",
                    "/%s/ '%s' %d-%d : %d-%d" % (pattern, target, s_from, s_to, start, end),
                )
    onig.onig_free(reg)
...
def main():
    ...
    if len(sys.argv) > 1:
        encs = {
            "EUC-JP": onig.ONIG_ENCODING_EUC_JP,
    ...
    def get_text_writer(fileno, **kwargs):
        ...
        writer.write = lambda s: (write(s.decode(enc)) if isinstance(s, bytes) else write(s))  # convert to unistr
        return writer
    ...
    x2("(?<愚か>変|\\(\\g<愚か>\\))", "((((((変))))))", 0, 13)
    x2(
        "\\A(?:\\g<阿_1>|\\g<云_2>|\\z終了  (?<阿_1>観|自\\g<云_2>自)(?<云_2>在|菩薩\\g<阿_1>菩薩))$",
        "菩薩自菩薩自在自菩薩自菩薩",
        0,
        13,
    )
    ...
    print(
        "\nRESULT   SUCC: %d,  FAIL: %d,  ERROR: %d      (by Onigmo %s)" % (nsucc, nfail, nerror, onig.onig_version())
    )
