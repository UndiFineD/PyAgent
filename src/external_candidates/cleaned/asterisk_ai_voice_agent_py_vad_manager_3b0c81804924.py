# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\core.py\vad_manager_3b0c81804924.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\vad_manager.py
...
class EnhancedVADManager:
        ...
        *,
        ...
        if WEBRTC_VAD_AVAILABLE:
            try:
                ...
                logger.info(
                    "Enhanced VAD - WebRTC initialized",
    ...
    async def process_frame(self, call_id: str, audio_frame_pcm16: bytes, sample_rate: int = 8000) -> VADResult:
        ...
        if self.webrtc_vad and sample_rate in WEBRTC_SUPPORTED_RATES:
            try:
                ...
                logger.debug(
                    "Enhanced VAD - WebRTC processing error",
                    exc_info=True,
        ...
        threshold = (
            call_state["adaptive_threshold"].get_threshold()
            if self.adaptive_threshold_enabled
            else self.base_energy_threshold
        )
    ...
    def _smooth_frames_per_call(self, call_id: str, raw_speech: bool) -> bool:
        ...
        if raw_speech:
            ...
            if not call_state["is_speaking"] and call_state["speech_frames"] >= self.min_speech_frames:
                ...
                logger.debug(
                    "Enhanced VAD - Speech started",
                    call_id=call_id,
                    frames=call_state["speech_frames"],
                )
            ...
            if call_state["is_speaking"] and call_state["silence_frames"] >= self.max_silence_frames:
                ...
                logger.debug(
                    "Enhanced VAD - Speech ended",
                    call_id=call_id,
                    silence_frames=call_state["silence_frames"],
                )
    ...
    def _update_call_stats(self, call_id: str, result: VADResult) -> None:
        ...
        if not result.is_speech and result.energy_level > 0:
            stats["noise_level"] = (
                stats["noise_level"] * 0.9 + min(result.energy_level / self.energy_threshold, 1.0) * 0.1
            )
