# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\outside_videos.py\misc_thumbnails_a115c1d7420e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\misc_thumbnails.py
...
class LinalgThumbnail(ThreeDScene):
    ...
    def construct(self):
        ...
        cube2 = cube.copy().apply_matrix([[1, 0, 1], [0, 1, 0], [0, 0, 1]])
...
class PrimeSpirals(InteractiveScene):
    def construct(self):
        ...
        points = np.array([primes * np.cos(primes), primes * np.sin(primes), np.zeros(len(primes))]).T
