# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\mcp.py\install_menu_7a01862076ab.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\mcp\install_menu.py
...
class MCPInstallMenu:
    ...
    def _render_category_list(self) -> List:
        ...
        for i in range(start_idx, end_idx):
            ...
            if category == CUSTOM_SERVER_CATEGORY:
                ...
                server_count = len(self.catalog.get_by_category(category)) if self.catalog else 0
        ...
        if self.view_mode == "categories":
            ...
            if cmd_args:
                ...
                for arg in cmd_args:
                    ...
                    lines.append(("fg:ansibrightblack", f"    {marker} {name}{default_str}"))
    ...
    def run(self) -> bool:
        ...
        menu_window = Window(content=self.menu_control, wrap_lines=True, width=Dimension(weight=35))

        preview_window = Window(content=self.preview_control, wrap_lines=True, width=Dimension(weight=65))

        menu_frame = Frame(menu_window, width=Dimension(weight=35), title="Browse")

        preview_frame = Frame(preview_window, width=Dimension(weight=65), title="Details")
        ...
        if self.result == "pending_install" and self.pending_server:
            ...
            if config:
                success = install_catalog_server(self.manager, self.pending_server, config)
