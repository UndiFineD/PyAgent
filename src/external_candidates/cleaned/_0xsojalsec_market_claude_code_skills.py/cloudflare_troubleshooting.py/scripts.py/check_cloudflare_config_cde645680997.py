# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\cloudflare-troubleshooting\scripts\check_cloudflare_config.py
#!/usr/bin/env python3
"""
...
def check_ssl_configuration(zone_id: str, email: str, api_key: str) -> Tuple[bool, List[str]]:
    """
    ...
    try:
        ...
        if ssl_mode == "flexible":
            ...
            issues.append(
                "   Recommendation: Change SSL mode to 'full' or 'full (strict)' if your origin supports HTTPS"
            )
...
def check_dns_records(zone_id: str, domain: str, email: str, api_key: str) -> Tuple[bool, List[str]]:
    """
    ...
    try:
        ...
        unproxied_important = [r for r in records if not r.get("proxied") and r["type"] in ["A", "AAAA", "CNAME"]]
...
def check_page_rules(zone_id: str, email: str, api_key: str) -> Tuple[bool, List[str]]:
    """
    ...
    try:
        ...
        if redirect_rules:
            ...
            issues.append("   Note: Conflicting redirect rules can cause redirect loops")
        else:
