...
class SessionContentRecovery:
    ...
    def save_recovered_files(
        ...
        if keywords:
            write_calls = [
                call for call in write_calls if any(kw.lower() in call["file_path"].lower() for kw in keywords)
            ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Recover content from Claude Code session history files")
    parser.add_argument(
        "session_file",
        type=Path,
