                        ...
                        for item in content:
                            ...
                            if item.get("type") == "tool_use":
                                ...
                                if tool_name == "Write":
                                    ...
                                    if file_path:
                                        stats["file_operations"].append(("write", file_path))
                                elif tool_name == "Edit":
                                    stats["edit_calls"] += 1
                                    file_path = item.get("input", {}).get("file_path", "")
                                    if file_path:
                                        stats["file_operations"].append(("edit", file_path))
                                elif tool_name == "Read":
                                    stats["read_calls"] += 1
                                elif tool_name == "Bash":
                                    stats["bash_calls"] += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze Claude Code session history files")
    ...
    list_parser.add_argument("--limit", type=int, default=10, help="Max sessions to show (default: 10)")
    ...
    stats_parser.add_argument("--show-files", action="store_true", help="Show file operations")
    ...
    if args.command == "list":
        ...
        sorted_matches = sorted(matches.items(), key=lambda x: x[1]["total_mentions"], reverse=True)
