    ...
    def _check_dangerous_patterns(self, path_str: str) -> None:
        ...
        for pattern in DANGEROUS_PATTERNS:
            if pattern in path_str:
                raise PathValidationError(f"Dangerous pattern '{pattern}' detected in path: {path_str}")
        ...
        if not self.allow_symlinks:
            ...
            while current != current.parent:  # Until root
                ...
                if current.is_symlink():
                    raise PathValidationError(f"Symlink in path hierarchy detected: {current}")
                current = current.parent

    def _validate_extension(self, path: Path, allowed_extensions: Set[str], operation: str) -> None:
        """
        ...
        if extension not in allowed_extensions:
            raise PathValidationError(
                f"File extension '{extension}' not allowed for {operation}. Allowed: {sorted(allowed_extensions)}"
            )
        ...
        if not parent.exists():
            if create_parent:
                ...
                if not self._is_under_allowed_directory(parent):
                    raise PathValidationError(f"Parent directory not under allowed directories: {parent}")
                try:
                    parent.mkdir(parents=True, exist_ok=True)
                    logger.info(f"Created parent directory: {parent}")
                except Exception as e:
                    raise PathValidationError(f"Failed to create parent directory: {parent}") from e
            else:
