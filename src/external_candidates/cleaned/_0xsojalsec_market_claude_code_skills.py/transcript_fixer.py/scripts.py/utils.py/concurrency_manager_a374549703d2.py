# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\concurrency_manager.py
#!/usr/bin/env python3
"""
...
class ConcurrencyManager:
        ...
        with self._circuit_lock:
            ...
            if self._circuit_state == CircuitState.CLOSED:
                if self._circuit_failures >= self.config.circuit_failure_threshold:
                    logger.warning(f"Circuit breaker: CLOSED -> OPEN ({self._circuit_failures} failures)")
    ...
    def _adjust_concurrency(self) -> None:
        ...
        with self._response_times_lock:
            ...
            if avg_time > target_time * 1.5:
                ...
                new_concurrency = max(self.config.min_concurrent, current_concurrency - 1)
                ...
                new_concurrency = min(20, current_concurrency + 1)  # Hard cap
                if new_concurrency != current_concurrency:
                    logger.info(
    ...
    async def acquire(self, timeout: Optional[float] = None):
            ...
            with self._metrics_lock:
                if self._metrics.queued_operations >= self.config.max_queue_size:
                    ...
                    raise BackpressureError(
                        f"Queue full ({self.config.max_queue_size} operations pending). Try again later."
                    )
        ...
        try:
            ...
            raise asyncio.TimeoutError(f"Operation timed out after {elapsed:.1f}s (timeout: {timeout}s)")
    ...
    def acquire_sync(self, timeout: Optional[float] = None):
        ...
        if self.config.enable_backpressure:
            with self._metrics_lock:
                if self._metrics.queued_operations >= self.config.max_queue_size:
                    self._metrics.rejected_requests += 1
                    raise BackpressureError(f"Queue full ({self.config.max_queue_size} operations pending)")
