# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\logging_config.py
#!/usr/bin/env python3
"""
...
class TimedLogger:
    ...
    def timed(self, operation_name: str, **context: Any):
        ...
        try:
            ...
            self.logger.error(f"{operation_name} failed in {duration_ms:.1f}ms{context_str}: {e}")
            raise
        else:
            duration_ms = (time.time() - start_time) * 1000
            self.logger.info(f"{operation_name} completed in {duration_ms:.1f}ms{context_str}")
