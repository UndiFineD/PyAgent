# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\database_migration.py
#!/usr/bin/env python3
"""
...
class DatabaseMigrationManager:
    ...
    def _run_migration(self, migration: Migration, direction: MigrationDirection, dry_run: bool = False) -> None:
        """
        ...
        with self._transaction() as cursor:
            ...
            try:
                ...
                logger.info(
                    f"Successfully applied {direction.value} migration {migration.version} in {execution_time_ms}ms"
                )
    ...
    def migrate_to_version(self, target_version: str, dry_run: bool = False, force: bool = False) -> None:
        """
        ...
        for version in all_versions:
            if version > from_version and version <= to_version:
                ...
                if migration.is_breaking and not force:
                    raise RuntimeError(f"Migration {migration.version} is a breaking change. Use --force to apply.")

                # Check dependencies
                for dep in migration.dependencies:
                    if dep > from_version:
                        raise RuntimeError(
                            f"Migration {migration.version} requires dependency {dep} which is not yet applied"
                        )
        ...
        for version in all_versions:
            if version <= from_version and version > to_version:
                ...
                dependent_migrations = [
                    v for v, m in self.migrations.items() if version in m.dependencies and v <= from_version
                ]
                if dependent_migrations and not force:
                    raise RuntimeError(
                        f"Cannot rollback {version} because it has dependencies: {', '.join(dependent_migrations)}"
                    )

                self._run_migration(migration, MigrationDirection.BACKWARD, dry_run)

    def rollback_migration(self, version: str, dry_run: bool = False, force: bool = False) -> None:
        """
        ...
        dependent_migrations = [
            v for v, m in self.migrations.items() if version in m.dependencies and v in applied_versions
        ]
        if dependent_migrations and not force:
            raise RuntimeError(
                f"Cannot rollback {version} because it has dependencies: {', '.join(dependent_migrations)}"
