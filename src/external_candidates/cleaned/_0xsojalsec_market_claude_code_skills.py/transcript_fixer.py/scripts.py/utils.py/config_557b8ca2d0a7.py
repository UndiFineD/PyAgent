# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\config.py
#!/usr/bin/env python3
"""
...
class Config:
    ...
    database: DatabaseConfig = field(
        default_factory=lambda: DatabaseConfig(path=Path.home() / ".transcript-fixer" / "corrections.db")
    )
    ...
    def from_env(cls) -> Config:
        ...
        # Database config
        db_path = Path(os.getenv("TRANSCRIPT_FIXER_DB_PATH", str(config_dir / "corrections.db")))
