# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\rate_limiter.py
#!/usr/bin/env python3
"""
...
class TokenBucketLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        logger.debug(f"TokenBucket initialized: capacity={self.capacity}, refill_rate={self.refill_rate:.2f}/s")
...
class SlidingWindowLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        logger.debug(f"SlidingWindow initialized: max_requests={self.max_requests}, window={self.window_seconds}s")
...
class RateLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        logger.info(f"RateLimiter created: {config.strategy.value}, {config.max_requests}/{config.window_seconds}s")
...
class RateLimitPresets:
    ...
    # API rate limits
    API_CONSERVATIVE = RateLimitConfig(max_requests=10, window_seconds=60.0, strategy=RateLimitStrategy.TOKEN_BUCKET)

    API_MODERATE = RateLimitConfig(max_requests=60, window_seconds=60.0, strategy=RateLimitStrategy.TOKEN_BUCKET)

    API_AGGRESSIVE = RateLimitConfig(max_requests=100, window_seconds=60.0, strategy=RateLimitStrategy.TOKEN_BUCKET)
    ...
    STRICT_LIMIT = RateLimitConfig(max_requests=100, window_seconds=60.0, strategy=RateLimitStrategy.SLIDING_WINDOW)
