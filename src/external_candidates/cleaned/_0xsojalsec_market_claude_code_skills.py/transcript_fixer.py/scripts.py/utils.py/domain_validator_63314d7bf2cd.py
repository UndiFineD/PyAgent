# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\domain_validator.py
#!/usr/bin/env python3
"""
...
def validate_domain(domain: str) -> str:
    ...
    if len(domain) > MAX_DOMAIN_LENGTH:
        raise ValidationError(f"Domain name too long: {len(domain)} chars (max: {MAX_DOMAIN_LENGTH})")
...
def validate_source(source: str) -> str:
    ...
    if source not in VALID_SOURCES:
        raise ValidationError(f"Invalid source: '{source}'. Valid sources: {sorted(VALID_SOURCES)}")
...
def sanitize_text_field(text: str, max_length: int, field_name: str = "field") -> str:
    ...
    if len(text) > max_length:
        raise ValidationError(f"{field_name} too long: {len(text)} chars (max: {max_length})")
