# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\utils\metrics.py
#!/usr/bin/env python3
"""
...
class Histogram:
    ...
    def snapshot(self) -> MetricSnapshot:
        ...
        percentiles = {f"p{int(p * 100)}": self.get_percentile(p) for p in PERCENTILES}
...
class MetricsCollector:
    ...
    def to_json(self) -> str:
        ...
        data = {"timestamp": time.time(), "metrics": [s.to_dict() for s in snapshots]}
    ...
    def to_prometheus(self) -> str:
        ...
        for snapshot in snapshots:
            ...
            if snapshot.type == MetricType.HISTOGRAM and snapshot.percentiles:
                ...
                lines.append(f"{snapshot.name}_count{labels_str} {snapshot.samples} {int(snapshot.timestamp * 1000)}")
                lines.append(f"{snapshot.name}_sum{labels_str} {snapshot.sum} {int(snapshot.timestamp * 1000)}")
            else:
                lines.append(f"{snapshot.name}{labels_str} {snapshot.value} {int(snapshot.timestamp * 1000)}")
