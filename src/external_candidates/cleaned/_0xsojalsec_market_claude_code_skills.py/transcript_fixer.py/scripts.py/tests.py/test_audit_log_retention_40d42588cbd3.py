# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\tests\test_audit_log_retention.py
#!/usr/bin/env python3
"""
...
def test_retention_policy_validation():
    ...
    # Valid policy
    policy = RetentionPolicy(entity_type="test", retention_days=30, strategy=CleanupStrategy.ARCHIVE)
    assert policy.retention_days == 30

    # Invalid: negative days (except -1)
    with pytest.raises(ValueError, match="retention_days must be -1"):
        RetentionPolicy(entity_type="test", retention_days=-5, strategy=CleanupStrategy.DELETE)
