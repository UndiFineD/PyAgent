...
try:
    ...
    raise ImportError("filelock library required for thread-safe operations. Install with: uv add filelock")
...
class LearningEngine:
    ...
    def _file_lock(self, lock_path: Path, operation: str = "file operation"):
        ...
        try:
            ...
            logger.error(f"Failed to acquire lock for {operation} after {self.lock_timeout}s: {lock_path}")
            raise RuntimeError(
                f"File lock timeout for {operation}. Another process may be holding the lock. Lock file: {lock_path}"
            ) from e
        finally:
    ...
    def reject_suggestion(self, from_text: str, to_text: str) -> None:
        ...
        with self._file_lock(self.pending_lock, "reject suggestion (pending)"):
            ...
            pending = [s for s in pending if not (s["from_text"] == from_text and s["to_text"] == to_text)]
            self._save_suggestions_unlocked(pending, self.pending_file)
    ...
    def _calculate_confidence(self, occurrences: List[Dict]) -> float:
        ...
        # Weighted average
        confidence = 0.5 * frequency_score + 0.3 * consistency_score + 0.2 * recency_score
    ...
    def _save_rejected_unlocked(self, rejected: set) -> None:
        ...
        data = {"rejected": [{"from": from_text, "to": to_text} for from_text, to_text in rejected]}
    ...
    def analyze_and_auto_approve(self, changes: List, domain: str = "general") -> Dict:
        ...
        for (from_text, to_text), occurrences in patterns.items():
            ...
            if frequency >= self.AUTO_APPROVE_FREQUENCY and avg_confidence >= self.AUTO_APPROVE_CONFIDENCE:
            ...
            elif frequency >= self.MIN_FREQUENCY and avg_confidence >= self.MIN_CONFIDENCE:
