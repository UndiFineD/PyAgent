...
class AIProcessorAsync:
    ...
    async def _process_async(self, text: str, context: str) -> Tuple[str, List[AIChange]]:
        ...
        if len(chunks) > MEMORY_WARNING_THRESHOLD:
            logger.warning(f"Large file detected: {len(chunks)} chunks. Will sample changes to limit memory usage.")
        ...
        tasks = [
            self._process_chunk_with_semaphore(i, chunk, context, semaphore, len(chunks))
