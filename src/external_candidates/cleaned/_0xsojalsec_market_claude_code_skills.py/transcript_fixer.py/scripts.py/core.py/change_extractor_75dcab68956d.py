# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\core\change_extractor.py
#!/usr/bin/env python3
"""
...
class ExtractedChange:
    ...
    def __eq__(self, other):
        ...
        return self.from_text == other.from_text and self.to_text == other.to_text
...
class ChangeExtractor:
    ...
    def __init__(self, min_change_length: int = 1, max_change_length: int = 50):
        ...
        if not isinstance(min_change_length, int):
            raise InputValidationError(f"min_change_length must be int, got {type(min_change_length).__name__}")

        if not isinstance(max_change_length, int):
            raise InputValidationError(f"max_change_length must be int, got {type(max_change_length).__name__}")

        # CRITICAL FIX: Validate parameter ranges
        if min_change_length < 1:
            raise InputValidationError(f"min_change_length must be >= 1, got {min_change_length}")

        if max_change_length < 1:
            raise InputValidationError(f"max_change_length must be >= 1, got {max_change_length}")

        # CRITICAL FIX: Validate logical consistency
        if min_change_length > max_change_length:
            raise InputValidationError(
                f"min_change_length ({min_change_length}) must be <= max_change_length ({max_change_length})"
            )

        # CRITICAL FIX: Validate reasonable upper bounds (DoS prevention)
        if max_change_length > 1000:
            logger.warning(f"Large max_change_length ({max_change_length}) may impact performance")

        self.min_change_length = min_change_length
        self.max_change_length = max_change_length

        logger.debug(f"ChangeExtractor initialized: min={min_change_length}, max={max_change_length}")
    ...
    def extract_changes(self, original: str, corrected: str) -> List[ExtractedChange]:
        ...
        if not isinstance(original, str):
            raise InputValidationError(f"original must be str, got {type(original).__name__}")

        if not isinstance(corrected, str):
            raise InputValidationError(f"corrected must be str, got {type(corrected).__name__}")

        # CRITICAL FIX: Validate input length (DoS prevention)
        if len(original) > MAX_TEXT_LENGTH:
            raise InputValidationError(
                f"original text too long ({len(original)} chars). Maximum allowed: {MAX_TEXT_LENGTH}"
            )

        if len(corrected) > MAX_TEXT_LENGTH:
            raise InputValidationError(
                f"corrected text too long ({len(corrected)} chars). Maximum allowed: {MAX_TEXT_LENGTH}"
            )
        ...
        logger.debug(f"Extracting changes: original={len(original)} chars, corrected={len(corrected)} chars")
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "replace":  # Actual replacement (from→to)
                ...
                confidence = self._calculate_confidence(from_text, to_text, context_before, context_after)
                ...
                if len(changes) >= MAX_CHANGES:
                    logger.warning(f"Reached maximum changes limit ({MAX_CHANGES}), stopping extraction")
                    break
    ...
    def group_by_pattern(self, changes: List[ExtractedChange]) -> dict[Tuple[str, str], List[ExtractedChange]]:
        ...
        if not isinstance(changes, list):
            raise InputValidationError(f"changes must be list, got {type(changes).__name__}")

        # CRITICAL FIX: Validate list elements
        grouped = {}
        for i, change in enumerate(changes):
            if not isinstance(change, ExtractedChange):
                raise InputValidationError(f"changes[{i}] must be ExtractedChange, got {type(change).__name__}")
    ...
    def calculate_pattern_confidence(self, occurrences: List[ExtractedChange]) -> float:
        ...
        for i, occurrence in enumerate(occurrences):
            if not isinstance(occurrence, ExtractedChange):
                raise InputValidationError(f"occurrences[{i}] must be ExtractedChange, got {type(occurrence).__name__}")
        ...
        unique_contexts = len(set((c.context_before, c.context_after) for c in occurrences))
        diversity_factor = min(1.0, unique_contexts / len(occurrences))

        # Combined confidence (weighted average)
        final_confidence = 0.5 * avg_confidence + 0.3 * frequency_factor + 0.2 * diversity_factor
    ...
    def _calculate_confidence(self, from_text: str, to_text: str, context_before: str, context_after: str) -> float:
        """
        ...
        confidence = 0.4 * length_score + 0.3 * context_score + 0.3 * chinese_score
