# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\cache.py\work.py\copilot_tmp.py\chunk_0_bloodhound_178db9984ccc.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\cache\work\copilot_tmp\chunk_0_bloodhound.py

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-NetExec\nxc\helpers\bloodhound.py
            ...
            with driver.session().begin_transaction() as tx:
                ...
                for user_info in users_owned:
                    ...
                    distinguished_name = "".join(
                        ...
                        [f"DC={dc}," for dc in user_info["domain"].split(".")]

                    ).rstrip(",")

                    domain_query = tx.run(

                        f"MATCH (d:Domain) WHERE d.distinguishedname STARTS WITH '{distinguished_name}' RETURN d"
                    ...
                    if not domain_query:
                        ...
                        logger.debug(
                            ...
                            f"Domain {user_info['domain']} not found in BloodHound. Falling back to domainless query."
...
def _add_with_domain(user_info, domain, tx, logger):
    ...
    if result[0]["c"].get("owned") in (False, None):
        ...
        logger.highlight(
            ...
            f"Node {result['name']} successfully set as owned in BloodHound"
...
def _add_without_domain(user_info, tx, logger):
    ...
    result = tx.run(
        ...
        f"MATCH (c:{account_type}) WHERE c.name STARTS WITH '{user_owned}' RETURN c"
    ...
    if len(result) == 0:
        ...
        logger.fail(
            ...
            f"Multiple accounts found with the name '{user_info['username']}' in the BloodHound database. Please specify the FQDN ex:domain.local"
        ...
        logger.debug(
            ...
            f"MATCH (c:{account_type} {{name:'{result[0]['c']['name']}'}}) SET c.owned=True RETURN c.name AS name"
