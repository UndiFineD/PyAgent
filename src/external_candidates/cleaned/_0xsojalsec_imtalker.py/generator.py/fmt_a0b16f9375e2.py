# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\generator\FMT.py
import math
import os
...
class FMTBlock(nn.Module):
    ...
    def forward(self, x, c, rotary_pos_emb=None) -> torch.Tensor:
        ...
        x = x + gate_msa * self.attn(
            self.framewise_modulate(self.norm1(x), shift_msa, scale_msa),
...
class FlowMatchingTransformer(nn.Module):
    ...
    def sequence_embedder(self, sequence: torch.Tensor, dropout_prob: float, train: bool = False) -> torch.Tensor:
        if train:
