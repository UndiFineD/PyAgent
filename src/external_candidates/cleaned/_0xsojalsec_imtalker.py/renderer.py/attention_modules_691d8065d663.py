...
class SwinUnifiedAttention(nn.Module):
    def __init__(self, dim, num_heads, window_size, qkv_bias=True, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim**-0.5
...
class UnifiedSwinBlock(nn.Module):
    def __init__(
        ...
        self.norm_q = norm_layer(dim)
        self.norm_kv = norm_layer(dim)
        self.attn = SwinUnifiedAttention(dim, num_heads, self.window_size, qkv_bias, attn_drop, drop)
        ...
        if self.shift_size > 0:
            ...
            for h in h_slices:
                for w in w_slices:
                    img_mask[:, h, w, :] = cnt
                    cnt += 1
            mask_windows = window_partition(img_mask.view(1, H, W, 1), self.window_size).view(
    ...
    def forward(self, query, key=None, value=None):
        ...
        if self.shift_size > 0:
            shifted_q, shifted_k, shifted_v = [
                torch.roll(t, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2)) for t in (q, k, v)
            ]
        else:
