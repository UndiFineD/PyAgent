# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\renderer\dataset.py
import bisect
import itertools
import os
...
def create_eye_mouth_mask(
    ...
    def to_px_coords(idx_list):
        return [(int(landmarks_68[i, 0] * image_size), int(landmarks_68[i, 1] * image_size)) for i in idx_list]
...
class TFDataset(Dataset):
    ...
    def _load_metadata(self):
        ...
        for clip_path in tqdm(clip_dirs, desc=f"Processing {self.root_path.name}"):
            ...
            meta_list.append({"dir": str(clip_path), "frames": frame_files, "lmd": str(lmd_file)})
