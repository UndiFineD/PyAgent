# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMTalker\renderer\train.py
import argparse
import os
...
class IMFSystem(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.trainer.global_rank == 0 and batch_idx == 0:
            ...
            for name, img in zip(name_list, img_list):
                self.logger.experiment.add_images(tag=name, img_tensor=img.clamp(0, 1), global_step=self.global_step)
    ...
    def configure_optimizers(self):
        ...
        scheduler_g = optim.lr_scheduler.CosineAnnealingLR(opt_g, T_max=self.args.iter, eta_min=1e-4)
        ...
        scheduler_d = optim.lr_scheduler.CosineAnnealingLR(opt_d, T_max=self.args.iter, eta_min=1e-4)
    ...
    def _safe_load(self, model, state_dict, prefix):
        ...
        print(
            f"[INFO] Loaded {prefix.strip('.')} weights. Missing: {len(msg.missing_keys)}, Unexpected: {len(msg.unexpected_keys)}"
        )
