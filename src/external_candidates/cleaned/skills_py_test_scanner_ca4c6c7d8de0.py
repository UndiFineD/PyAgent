# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\dgriffin831.py\skill_scan.py\tests.py\test_scanner_ca4c6c7d8de0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dgriffin831\skill-scan\tests\test_scanner.py
...
class TestScanDirectory:
    ...
    def test_evasive_skills_detected(self, rules, fixtures_dir):
        ...
        for d in sorted(fixtures_dir.iterdir()):
            ...
            assert report["score"] < 50, f"{d.name} should score < 50, got {report['score']}"
...
class TestContextScoring:
    ...
    def test_declared_env_vars_downweighted(self, rules, fixtures_dir):
        ...
        cred_findings = [
            f for f in report["findings"] if f.get("ruleId") == "CRED_ACCESS" and f.get("severity") == "info"
        ]
