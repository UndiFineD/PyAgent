...
def main() -> int:
    ...
    ap.add_argument(
        "--duration",
        default="5s",
        help="Video duration (model-dependent, use --list-models to see options per model)",
    )
    ...
    ap.add_argument(
        "--skip-audio-param",
        action="store_true",
        help="Don't send audio param (for models that don't support it)",
    )
    ...
    ap.add_argument(
        "--complete",
        metavar="QUEUE_ID",
        help="Clean up a previously downloaded video (use with --model)",
    )
