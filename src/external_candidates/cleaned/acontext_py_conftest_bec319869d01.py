# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\acontext.py\src.py\server.py\core.py\tests.py\service.py\conftest_bec319869d01.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\tests\service\conftest.py
...
def mock_block_get_embedding():
    ...
    with patch("acontext_core.service.data.block.get_embedding") as mock:
        ...
        mock_embedding_return = EmbeddingReturn(
            embedding=np.random.rand(1, DEFAULT_CORE_CONFIG.block_embedding_dim).astype(np.float32),
            prompt_tokens=10,
            total_tokens=10,
        )
...
def mock_block_search_get_embedding():
    ...
    with patch("acontext_core.service.data.block_search.get_embedding") as mock:
        ...
        async def get_mock_embedding(texts, phase="document"):
            ...
            base_vector = np.zeros(DEFAULT_CORE_CONFIG.block_embedding_dim, dtype=np.float32)
            ...
            if "machine" in text or "learning" in text or "neural" in text:
                ...
                base_vector = np.random.rand(DEFAULT_CORE_CONFIG.block_embedding_dim).astype(np.float32)
