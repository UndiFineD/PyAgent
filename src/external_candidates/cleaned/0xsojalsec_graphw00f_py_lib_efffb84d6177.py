# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphw00f.py\graphw00f.py\lib_efffb84d6177.py
...
class GRAPHW00F:
    ...
    """
        ...
        if response.get("data"):
            ...
            any("locations" in i for i in response["errors"]) or (any("extensions" in i for i in response))
        ):
            return True
    ...
    """
        ...
        if error_contains(response, "asyncExecutionResult[Symbol.asyncIterator] is not a function") or error_contains(
