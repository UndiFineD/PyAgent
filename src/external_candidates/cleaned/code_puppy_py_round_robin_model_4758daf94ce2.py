# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\round_robin_model_4758daf94ce2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\round_robin_model.py
...
class RoundRobinModel(Model):
    ...
    async def request(
        ...
        merged_settings, prepared_params = current_model.prepare_request(model_settings, model_request_parameters)

        try:
            response = await current_model.request(messages, merged_settings, prepared_params)
    ...
    async def request_stream(
        ...
        async with current_model.request_stream(messages, merged_settings, prepared_params, run_context) as response:
            self._set_span_attributes(current_model)
