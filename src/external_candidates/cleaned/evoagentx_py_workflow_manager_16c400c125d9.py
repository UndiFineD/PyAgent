# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\workflow.py\workflow_manager_16c400c125d9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\workflow\workflow_manager.py
...
class TaskSchedulerOutput(LLMOutputParser):
    decision: str = Field(
        description="The decision made by the scheduler, whether to re-execute, iterate or forward a certain task."
    )

    task_name: str = Field(description="The name of the scheduled task.")

    reason: str = Field(
        description="The rationale behind the scheduling decision, explaining why the task was scheduled."
    )
...
class TaskScheduler(Action):
    ...
    def __init__(self, **kwargs):
        ...
        description = (
            kwargs.pop("description", None) if "description" in kwargs else DEFAULT_TASK_SCHEDULER["description"]
        )
        ...
        super().__init__(
            ...
            prompt=prompt,
            outputs_format=TaskSchedulerOutput,
            **kwargs,
        )
    ...
    def _prepare_execution(
        ...
        graph: WorkFlowGraph,
        env: Environment,
        candidate_tasks: List[WorkFlowNode],
    ) -> Tuple[dict, str]:
        """
    ...
    def execute(
        ...
        **kwargs,
    ) -> Union[TaskSchedulerOutput, Tuple[TaskSchedulerOutput, str]]:
        """
...
class NextAction(LLMOutputParser):
    agent: Optional[str] = Field(
        default=None,
        description="The name of the selected agent responsible for executing the next action in the workflow.",
    )

    action: Optional[str] = Field(
        default=None,
        description="The name of the action that the selected agent will execute to continue progressing the subtask.",
    )

    reason: Optional[str] = Field(
        default=None,
        description="The justification for selecting this agent and action, explaining how it contributes to subtask execution based on workflow requirements and execution history.",
    )
...
class ActionScheduler(Action):
    ...
    def __init__(self, **kwargs):
        ...
        description = (
            kwargs.pop("description", None) if "description" in kwargs else DEFAULT_ACTION_SCHEDULER["description"]
        )
    ...
    def check_candidate_action(
        self,
        task_name: str,
        actions: List[str],
        agent_actions_map: Dict[str, List[str]],
    ):
        unknown_actions = []
        ...
        if unknown_actions:
            raise ValueError(
                f"Unknown actions: {unknown_actions} specified in the `next_actions`. All available actions defined for the task ({task_name}) are {merged_actions}."
            )
    ...
    def _prepare_action_execution(
        self, task: WorkFlowNode, agent_manager: AgentManager, env: Environment
        ...
        task_agent_actions_map = {
            agent.name: [action.name for action in agent.get_all_actions()] for agent in task_agents
        }
        ...
        agent_actions_info = "\n\n".join(
            [
                ...
                if agent.name in candidate_agent_actions
            ]
        )
...
class WorkFlowManager(BaseModule):
    ...
    async def schedule_next_task(self, graph: WorkFlowGraph, env: Environment = None, **kwargs) -> WorkFlowNode:
        ...
        execution_results = await self.task_scheduler.async_execute(
            llm=self.llm, graph=graph, env=env, return_prompt=True, **kwargs
        )
        ...
        message = Message(
            content=scheduled_task,
            agent=type(self).__name__,
            action=self.task_scheduler.name,
            prompt=prompt,
    ...
    async def schedule_next_action(
        ...
        **kwargs,
        ...
        execution_results = await self.action_scheduler.async_execute(
            llm=self.llm,
            task=task,
        ...
        message = Message(
            content=next_action,
            agent=type(self).__name__,
            action=self.action_scheduler.name,
            prompt=prompt,
            msg_type=MessageType.COMMAND,
            wf_goal=goal,
            wf_task=task.name,
