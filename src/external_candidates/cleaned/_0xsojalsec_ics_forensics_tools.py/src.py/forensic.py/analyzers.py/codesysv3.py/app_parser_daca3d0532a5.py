# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\app_parser.py
import csv
import json
import os
...
from forensic.analyzers.CodeSysV3.memory_dynamic_format import (
    CodesysV3MemoryDynamicFormat,
)
...
def dump_all(output_dir, g_entry_point, symbols, all_code_blocks, taskinfos, project_info):
    ...
    for taskinfo in taskinfos:
        nx.write_gexf(
            taskinfo["xrefs"],
            os.path.join(call_graph_dir, (f"{taskinfo['name']}.gexf")),
        )
        del taskinfo["xrefs"]
