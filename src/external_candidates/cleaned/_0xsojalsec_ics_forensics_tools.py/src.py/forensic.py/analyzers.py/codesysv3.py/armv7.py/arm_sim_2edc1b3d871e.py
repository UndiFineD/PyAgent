            ...
            for inst in self.iterate_no_emulate(curr_ea):
                ...
                if inst.mnemonic == "ldr":
                    if (
                        ...
                        address = (
                            inst.operands[1].mem.disp + inst.address + 8 if not self.is_thumb else 4
                        ) & 0xFFFFFFFF
            ...
            if inst.mnemonic.startswith("ldr"):
                ...
                if inst.mnemonic == "and" or (inst.mnemonic == "andvs" and self.get_reg("v_flag") == 1):
                    if (
                        ...
                        self.set_reg(
                            target_reg,
                            (self.get_reg(middle_reg) & self.get_reg(source_reg)) & 0xFFFFFFFF,
                        )
                    else:
                        mybreakpoint()
                ...
                if (
                    ...
                    self.set_reg(
                        ...
                        (
                            1
                            if left_reg_val > 0x80000000 and ((left_reg_val - right_reg_val) & 0xFFFFFFFF) <= 0x7FFFFFFF
                            else 0
                        ),
                    )
