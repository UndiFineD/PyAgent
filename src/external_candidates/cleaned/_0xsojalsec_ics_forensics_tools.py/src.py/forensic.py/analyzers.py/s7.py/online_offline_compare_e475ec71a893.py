# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\s7\online_offline_compare.py
import glob
import json
import os
import struct
...
class S7OnlineOfflineCompare(AnalyzerInterface):
    ...
    def analyze(self):
        ...
        if not os.path.exists(project_dir_path):
            self.logger.debug(
                f"Projects directory: {project_dir_path} does not exist. use --offline_project_dir to change directory path"
            )
            return
        if not ip:
            ...
            return
        self.logger.info(
            f"Start offline/online project comparison: online blocks from: {ip}, projects directory path: {project_dir_path}"
        )
    ...
    def get_device_raw_data(self, device_ip):
        parsed_data = []
        device_dir = self.output_dir.parent.joinpath("RawFileParser")
    ...
    def compare_project(
        ...
        blocks,
        project_path,
        offline_project_files_path,
        block_comparison_directory,
    ):
        ...
        for block in blocks:
            ...
            if block["type"] in ("FB", "FC", "OB"):
                ...
                if "interface_length" in block.keys():  # TODO: change only to interface_len
                    block_size = block["interface_length"] + block["segment_length"] + block["mc7_length"]
                else:
                    block_size = block["body_length"] + block["segment_length"] + block["data_length"]
                if block["data"] not in list(
                    map(
                        lambda offline_block: offline_block[block_size:],
                        all_offline_blocks_data,
                    )
                ):
