# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\s7\block_logic.py
import datetime
import ipaddress
import json
import os
...
import pandas as pd
from forensic.interfaces.analyzer import AnalyzerCLI, AnalyzerInterface
...
class S7BlockLogicCLI(AnalyzerCLI):
    def flags(self, parser):
        parser.add_argument(
            "--logic_all",
            help="Execute all logics for S7BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_author",
            help="Execute author logic for S7BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_dates",
            help="Execute date logic for S7BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_network",
            help="Execute network logics for S7BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_ob",
            help="Execute organizational blocks logic for BlockLogic analyzer",
            action="store_true",
        )
...
class S7BlockLogic(AnalyzerInterface):
    ...
    def call_tree(self, df, ip_addresses, export_dpath):
        ...
        for ip in ip_addresses:
            ...
            df_ip["called_blocks_edges"] = df_ip.apply(
                lambda row: self.get_called_blocks_edges(row["block_id"], row["used_block"]),
                axis=1,
            )
            ...
            # plt.show()
            plt.savefig(
                os.path.join(export_ip_dpath, "call_tree_{}.svg".format(ip_fmt)),
                dpi=1000,
            )
            nx.write_gexf(
                nx_graph,
                os.path.join(export_ip_dpath, "call_tree_{}.gexf".format(ip_fmt)),
            )
        ...
        for ip in ip_addresses:
            ...
            if not ob_blocks.empty:
                ...
                for i, row in ob_blocks.iterrows():
                    result_msg += "\n\t\tOB {} - start event: {}, used blocks: {}".format(
                        row.block_num, row.ob_role, row.used_block
                    )
    ...
    def ob_roles_check(self, df, ip_addresses):
        ...
        df.loc[df["type"] == "OB", "ob_role"] = df["block_num"].apply(
            lambda block_num: (
                ob_mapping[str(block_num)]["start_event"] if str(block_num) in ob_mapping.keys() else "Unknown"
            )
        )
            ...
            if not tcon_params.empty:
                ...
                for i, row in tcon_params.iterrows():
                    ...
                    tcon_params_msg += "\n\t\t\t{} {} - connection type: {}, active connection: {}, local port: {}, remote ip: {}, is remote ip external: {}, remote port: {}".format(
                        row.type,
                        row.block_num,
                        row.db_ext_header_tcon_params_connection_type,
                        True if row.db_ext_header_tcon_params_active_est else False,
                        row.db_ext_header_tcon_params_local_tsap_id,
                        row.db_ext_header_tcon_params_rem_staddr,
                        is_remote_addr_external,
    ...
    def network_check(self, df, ip_addresses):
        ...
        if "db_ext_header_tcon_params_block_length" in df.columns:
            ...
            df.loc[df.index.isin(conn_params_df.index), "conn_remote_address"] = df[
                "db_ext_header_tcon_params_rem_staddr"
            ].apply(lambda tcon_params: self.address_check(tcon_params))

            df.loc[
                (df["used_block"].isnull() == False) & (df["used_block"].str.len() > 0),
                "uses_communication_block",
            ] = df.apply(
                lambda row: self.is_use_communication_blocks(df, row["ip"], row["used_block"]),
                axis=1,
            )
    ...
    def process_dates_results(self, df, ip_addresses):
        ...
        for ip in ip_addresses:
            ...
            interface_delta_anomaly = df.loc[
                (df["ip"] == ip)
                & (
                    df["delta_current_time_vs_last_interface_change"]
                    < datetime.timedelta(days=lower_bound_interface_delta)
                )
            ]
            if not modified_delta_anomaly.empty:
                ip_anomaly_found = True
                block_ids = modified_delta_anomaly.block_id.unique().tolist()
                modified_msg += "\n\t\t{} blocks modified in that last {} days: {}".format(
                    len(block_ids), lower_bound_modified_delta, block_ids
                )
            if not interface_delta_anomaly.empty:
                ip_anomaly_found = True
                block_ids = interface_delta_anomaly.block_id.unique().tolist()
                interface_msg += "\n\t\t{} blocks interfaces modified in that last {} days: {}".format(
                    len(block_ids), lower_bound_interface_delta, block_ids
                )
    ...
    def dates_check(self, df, ip_addresses):
        ...
        df["delta_last_modified_vs_interface_change"] = df.apply(
            lambda row: (
                datetime.datetime.fromisoformat(row["last_modified"])
                - datetime.datetime.fromisoformat(row["last_interface_change"])
            ),
            axis=1,
        )
        ...
        for ip in ip_addresses:
            ...
            df.loc[df["ip"] == ip, "delta_to_min_last_interface_change"] = df["last_interface_change"].apply(
                lambda last_interface_change: datetime.datetime.fromisoformat(last_interface_change)
                - min_last_interface_change
            )
            df.loc[df["ip"] == ip, "delta_to_max_last_interface_change"] = df["last_interface_change"].apply(
                lambda last_interface_change: max_last_interface_change
                - datetime.datetime.fromisoformat(last_interface_change)
            )
    ...
    def store_df(self, df, fpath):
        df.drop(
            ["body", "data", "interface", "seg", "local_data", "body_parse"],
            axis=1,
            errors="ignore",
            inplace=True,
        )
