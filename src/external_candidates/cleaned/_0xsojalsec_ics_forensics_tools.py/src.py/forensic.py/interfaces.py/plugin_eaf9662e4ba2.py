# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\interfaces\plugin.py
...
class PluginInterface(ABC):
    def __init__(self, config: PluginConfig, output_dir: Path, verbose: bool):
        ...
        if verbose:
            self.logger_handler.create_log_file(
                os.path.join(os.getcwd(), constants.LOGS_DIR, f"{self.config.name}Plugin.log")
            )
...
class PluginCLI(ABC):
    ...
    def get_analyzer_choices(self):
        analyzers = modules.list_subclasses(AnalyzerInterface, f"{constants.APPLICATION}.analyzers.{self.folder_name}.")
...
class PluginConfig(object):
    def __init__(
        ...
        cb_event: Callable[[str, Any], None] = None,
    ):
        self.name = name
    ...
    def to_json(self):
        return {
            "name": self.name,
            "port": self.port,
            "transport": Transport(self.transport).value,
