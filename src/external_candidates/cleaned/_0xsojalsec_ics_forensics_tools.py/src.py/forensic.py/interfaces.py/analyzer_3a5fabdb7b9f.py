# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\interfaces\analyzer.py
    ...
    def __init__(self, config: AnalyzerConfig, output_dir: Path, verbose: bool):
        ...
        if verbose:
            self.logger_handler.create_log_file(
                os.path.join(os.getcwd(), constants.LOGS_DIR, f"{self.config.name}Analyzer.log")
            )
...
class AnalyzerConfig(object):
    ...
    def from_json(json: Dict) -> AnalyzerConfig:
        return AnalyzerConfig(name=json["name"], parameters=json["parameters"])
