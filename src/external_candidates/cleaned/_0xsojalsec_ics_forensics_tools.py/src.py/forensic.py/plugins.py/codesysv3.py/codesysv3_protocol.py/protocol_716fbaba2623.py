# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\CodeSysV3\codesysv3_protocol\protocol.py
import struct
...
class CodeSysV3Protocol:
    ...
    @staticmethod
    def _build_DatagramLayer(
        header_size: int,
        src_address: bytes,
        dst_address,
        dg_service: DatagramLayerServices,
        payload: bytes = b"",
    ) -> bytes:
        dg_layer = bytes(DatagramLayer(dg_service, len(dst_address), len(src_address))) + dst_address + src_address
    ...
    @staticmethod
    def build_DatagramLayerRequestOverTCP(
        src_ip: str,
        src_port: int,
        dst_ip: str,
        dst_port: int,
        ...
        dg_layer = CodeSysV3Protocol._build_DatagramLayer(
            ctypes.sizeof(BlockDriverLayerTcp),
            src_address,
            dst_address,
            dg_service,
            payload,
    ...
    def _get_codesys_address_format(src_ip: str, port: int, netmask: str) -> int:
        ...
        address = (~netmask_n) & (int.from_bytes(socket.inet_aton(src_ip), "big"))
    ...
    def build_NSServerDeviceInfo() -> bytes:
        pkt = bytes(
            NsHeader(
                subcmd=NSSubCmd.NameResolve.value,
                version=0x400,
                msg_id=CodeSysV3Protocol.PKT_COUNTER,
            )
        )
    ...
    @staticmethod
    def build_DatagramLayerRequestOverUDP(
        src_ip: str,
        src_port: int,
        dg_service: DatagramLayerServices,
        payload: bytes = b"",
        ...
        return CodeSysV3Protocol._build_DatagramLayer(0, src_address, dst_address, dg_service, payload)
    ...
    @staticmethod
    def build_CodeSysChannelLayerAppBlk(
        channel_id: int, blk_id: int, ack_id: int, payload: bytes, header_size: int = 12
    ) -> typing.List[bytes]:
        ...
        first_pkt = (
            bytes(ApplicationBlockFirst(payload, blk_id=blk_id, ack_id=ack_id, channel_id=channel_id))
        ...
        while payload_offset < len(payload):
            blk_id += 1
            pkt = (
                bytes(ApplicationBlock(blk_id=blk_id, ack_id=ack_id, channel_id=channel_id))
    ...
    @staticmethod
    def build_CodeSysServicesLayer(cmd_group: CmdGroup, subcmd: int, session_id: int, tags: typing.List[Tag]) -> bytes:
        tags_layer = b""
        for t in tags:
            tags_layer += t.to_stream()
        service_layer = ServiceLayer(
            cmd_group=cmd_group.value,
            subcmd=subcmd,
    ...
    def parse_CodeSysTCPBlockDriverLayer(pkt: bytes) -> list:
        ...
        if len(pkt) >= ctypes.sizeof(BlockDriverLayerTcp):
            ...
            if block_layer.tcp_magic != TCP_MAGIC:
                ...
                raise CodeSysProtocolV3Exception(
                    f"Missing pkt bytes, total size:{block_layer.tcp_length}, received: {len(pkt)}"
                )
            ...
            if nsclient_layer.subcmd == NSSubCmd.DeviceInfo.value and nsclient_layer.version in (0x103, 0x400):
                if len(pkt) >= ctypes.sizeof(NsClientDeviceInfo) + offset:
                    ...
                    total_strings_name = (
                        2
                        * (
                            nsclient_device_info_layer.node_name_length
                            + nsclient_device_info_layer.device_name_length
                            + nsclient_device_info_layer.vendor_name_length
                            + 3
                        )
                        + nsclient_device_info_layer.serial_length
                    )
                    if len(pkt) >= offset + total_strings_name:
                        nsclient_device_info_layer.node_name = pkt[
                            offset : offset + 2 * (nsclient_device_info_layer.node_name_length)
                        ].decode("UTF-16-LE")
                        offset += 2 * (nsclient_device_info_layer.node_name_length + 1)
                        nsclient_device_info_layer.device_name = pkt[
                            offset : offset + 2 * (nsclient_device_info_layer.device_name_length)
                        ].decode("UTF-16-LE")
                        offset += 2 * (nsclient_device_info_layer.device_name_length + 1)
                        nsclient_device_info_layer.vendor_name = pkt[
                            offset : offset + 2 * (nsclient_device_info_layer.vendor_name_length)
                        ].decode("UTF-16-LE")
                        offset += 2 * (nsclient_device_info_layer.vendor_name_length + 1)
                        nsclient_device_info_layer.serial = pkt[
                            offset : offset + nsclient_device_info_layer.serial_length
                        ].decode("ascii")
    ...
    @staticmethod
    def parse_CodeSysServiceLayer(
        pkt: bytes,
    ) -> typing.Tuple[ServiceLayer, typing.Dict[int, Tag]]:
