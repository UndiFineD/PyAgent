# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\CodeSysV3\codesysv3_protocol\structures.py
import ctypes
import enum
import socket
import struct
...
class BlockDriverLayerTcp(BaseLittleEndianStructure):
    _fields_ = [("tcp_magic", ctypes.c_uint32), ("tcp_length", ctypes.c_uint32)]
    _defaults_ = {"tcp_magic": TCP_MAGIC}

    def __init__(self, payload_len: int):
        super(BlockDriverLayerTcp, self).__init__(tcp_length=ctypes.sizeof(BlockDriverLayerTcp) + payload_len)
...
class ApplicationAck(BaseLittleEndianStructure):
    ...
    _defaults_ = {"flags": 0x80, "type": ChannelLayerType.ApplicationAck.value}
...
class KeepLive(BaseLittleEndianStructure):
    ...
    _defaults_ = {"flags": 0x00, "type": ChannelLayerType.KeepAlive.value}
