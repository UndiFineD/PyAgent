...
class CodeSysV3(PluginInterface):
    ...
    @retry(
        retry=(retry_if_exception_type(TimeoutError) | retry_if_exception_type(Exception)),
    ...
    def _conn(self, files, address):
        ...
        try:
            ...
            self.logger.info(
                f"Timeout error for IP: {address}, Port: {self.config.port}, Protocol_type: {self.config.transport}"
            )
            raise TimeoutError
        except Exception as e:
            self.logger.exception(
                f"{e} for IP: {address}, Port: {self.config.port}, Protocol_type: {self.config.transport}"
            )
            raise
