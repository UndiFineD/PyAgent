# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\s7\s7.py
import json
import socket
...
from forensic.plugins.s7.s7_client import InvalidS7RackSlot, S7Conn
...
class S7(PluginInterface):
    ...
    @retry(
        retry=(retry_if_exception_type(TimeoutError) | retry_if_exception_type(S7Error)),
        ...
        try:
            ...
            with S7Conn(self.logger, address, self.config.port, rack, slot) as s7:
                ...
                if data:
                    ...
                    self.logger.info(
                        f"Successfully connected to IP: {address}, Port: {self.config.port}, Rack: {rack}, Slot: {slot}"
                    )
        except InvalidS7RackSlot:
            self.logger.info(
                f"Invalid S7 rack slot for IP: {address}, Port: {self.config.port}, Rack: {rack}, Slot: {slot}"
            )
        except socket.timeout:
            self.logger.info(f"Timeout error for IP: {address}, Port: {self.config.port}, Rack: {rack}, Slot: {slot}")
            raise TimeoutError
        except ConnectionResetError:
            self.logger.info(
                f"Connection reset error for IP: {address}, Port: {self.config.port}, Rack: {rack}, Slot: {slot}"
            )
        except Exception as e:
            self.logger.exception(f"{e} for IP: {address}, Port: {self.config.port}, Rack: {rack}, Slot: {slot}")
            raise S7Error
