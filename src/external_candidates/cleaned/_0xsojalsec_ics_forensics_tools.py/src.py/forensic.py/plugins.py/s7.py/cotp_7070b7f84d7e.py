# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\s7\cotp.py
...
class COTP:
    ...
    def write_cr(
        ...
        bw_internal.write_struct(COTP_CR, {"dstref": dstref, "srcref": srcref, "flags": 0})
        ...
        bw.write_struct(
            COTPLayer,
            {"size": len(bw_internal) + 1, "tpdu": TPDU_TYPE_CONNECTION_REQUEST},
        )
    ...
    def write_dt(self) -> BinaryWriter:
        bw = BinaryWriter()
        bw.write_struct(COTPLayer, {"size": 2, "tpdu": TPDU_TYPE_DATA})
        bw.write(ubyte, 0x80)
