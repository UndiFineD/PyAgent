# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\s7\s7_parser.py
...
class S7Parser:
        ...
        if layer.param_len > 0:
            if layer.rosctr == ROSCTR_ACK_DATA:
                ...
                if param.error_code == 0x8104:
                    raise S7ErrorNotImplemented(
                        f"function {hex(param.function)}," + f"subfunction {hex(param.sub_function)}"
                    )
                    ...
                    raise S7Error(
                        f"Error {hex(param.error_code)} in function "
    ...
    def parse_user_data(self, layer: S7LayerResponse, br: BinaryReader) -> Any:
        ...
        if layer.param is not None and layer.data_len > 0:
            ...
            if not (layer.param.last_data_unit == NOT_LAST_DATA_UNIT and layer.param.sequence_num != 0):
                func_parsers = {
                    (FUNCTION_CPU, SUBFUNCTION_READ_SZL): self.parse_szl_data,
                    (
                        FUNCTION_BLOCK,
                        SUBFUNCTION_LIST_BLOCK,
                    ): self.parse_list_blocks_data,
                    (
                        FUNCTION_BLOCK,
                        SUBFUNCTION_LIST_BLOCKS_OF_TYPE,
                    ): self.parse_list_blocks_of_type_data,
                }
    ...
    def write_header(
        self,
        rosctr: int,
        param: BinaryWriter = BinaryWriter(),
        data: BinaryWriter = BinaryWriter(),
    ) -> BinaryWriter:
        bw = BinaryWriter()
        bw.write_struct(
            S7Request,
    ...
    def write_setup_communication(self) -> BinaryWriter:
        param = BinaryWriter()
        param.write_struct(
            S7ParamSetupCommunication,
            {
                "function": PARAM_FUNCTION_SETUP_COMM,
                "reserved": 0,
                "max_sessions": 1,
                "curr_sessions": 1,
                "pdu_length": 480,
    ...
    def build_user_data(self, function: int, sub_function: int) -> BinaryWriter:
        bw = BinaryWriter()
        bw.write_struct(
            S7ParamUserDataRequest,
            {
                "param_head": b"\x00\x01\x12",
    ...
    def build_user_data_fragmented(
        self,
        function: int,
        sub_function: int,
        sequence_num: int,
        data_unit_ref_num: int = 0,
    ) -> BinaryWriter:
        bw = BinaryWriter()
        bw.write_struct(
            S7ParamUserDataRequestFragmented,
            {
                "param_head": b"\x00\x01\x12",
    ...
    def build_user_data_packet(
        self,
        function: int,
        sub_function: int,
        data: BinaryWriter = None,
        sequence_num: int = 0,
    ) -> BinaryWriter:
        if sequence_num == 0:
            ...
            data = BinaryWriter()
            data.write_struct(S7DataFragment, {"return_code": 0x0A, "transport_size": 0, "length": 0})
    ...
    def write_list_blocks(self, sequence_num: int = 0) -> BinaryWriter:
        data = BinaryWriter()
        data.write_struct(S7EmptyRequest, {"return_code": 0x0A, "transport_size": 0, "length": 0})
    ...
    def write_start_upload(self, block_type: str, block_num: int) -> BinaryWriter:
        ...
        param = BinaryWriter()
        param.write_struct(
            S7ParamStartUploadRequest,
            {
                "function": PARAM_FUNCTION_START_UPLOAD,
                "status": 0,
                "unknown": 0,
                "upload_id": 0,
    ...
    def write_upload(self, upload_id: int) -> BinaryWriter:
        param = BinaryWriter()
        param.write_struct(
            S7ParamUploadRequest,
            {
                "function": PARAM_FUNCTION_UPLOAD,
                "status": 0,
                "unknown": 0,
                "upload_id": upload_id,
            },
    ...
    def write_end_upload(self, upload_id: int) -> BinaryWriter:
        param = BinaryWriter()
        param.write_struct(
            S7ParamEndUploadRequest,
            {
                "function": PARAM_FUNCTION_END_UPLOAD,
                "status": 0,
                "errorcode": 0,
                "upload_id": upload_id,
            },
