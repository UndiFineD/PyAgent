# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\utils.py\aflow_utils.py\evaluation_utils_692cf4e9be7c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\utils\aflow_utils\evaluation_utils.py
...
class EvaluationUtils:
    ...
    async def evaluate_graph_async(self, optimizer, validation_n, data, initial=False):
        ...
        for _ in range(validation_n):
            with suppress_logger_info():
                score, avg_cost, total_cost, all_failed = await evaluator.graph_evaluate_async(
                    optimizer.benchmark, optimizer.graph, is_test=False
                )
    ...
    async def evaluate_graph_test_async(self, optimizer):
        ...
        with suppress_logger_info():
            score, avg_cost, total_cost, all_failed = await evaluator.graph_evaluate_async(
                optimizer.benchmark, optimizer.graph, is_test=True
            )
