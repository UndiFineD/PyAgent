# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\tools.py\adapters.py\deepgram_b8749d5c6a10.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\adapters\deepgram.py
...
class DeepgramToolAdapter:
    ...
    async def handle_tool_call_event(self, event: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def send_tool_result(self, result: Dict[str, Any], context: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            logger.info(
                f"✅ Sent tool result to Deepgram: {result.get('status')}",
                function_call_id=function_call_id,
            )
