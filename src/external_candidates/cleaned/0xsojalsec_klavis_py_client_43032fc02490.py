# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\open_strata.py\src.py\strata.py\mcp_proxy.py\client_43032fc02490.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\src\strata\mcp_proxy\client.py
...
class MCPClient:
    ...
    async def connect(self) -> None:
        ...
        logger.info(f"Connected to MCP server using {self.transport.__class__.__name__}")
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> List[types.ContentBlock]:
        ...
        if result.isError:
            logger.error(f"Tool '{tool_name}' returned error: {result.structuredContent}")
