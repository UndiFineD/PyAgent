...
class PBXSocket(NBUSocket):
    ...
    async def handshake(self, extension):
        ...
        if not await self.recv():
            raise RuntimeError(f"{extension} not available through pbx_exchange for {self.host}:{self.port}")
...
class VNETDSocket(NBUSocket):
    ...
    async def connect(self):
        ...
        if self.use_pbx:
            ...
            self.sock = NBUSocket(self.host, self.port, timeout=self.timeout, quiet=True)
            await self.sock.connect()
