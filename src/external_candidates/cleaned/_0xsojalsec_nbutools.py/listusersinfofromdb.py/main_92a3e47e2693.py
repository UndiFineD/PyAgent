# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\ListUsersInfoFromDB\main.py
#!/usr/bin/python3
import argparse
import codecs
import logging
...
def parse_args():
    parser = argparse.ArgumentParser(description="Purpose: Retrieve DBA pwd of NBDB.db and get User Infos")
    parser.add_argument(
        "-k",
...
def main():
    ...
    nbdbcntr = NBDBSybaseConnector(dbapwd.decode("ascii"), args.host, args.port, args.jconn4_file_path)
    if nbdbcntr.con:
        curs = nbdbcntr.get_username_and_hashedpwd_from_ndbd()
        if curs != None:
            for u, p in curs:
                if p != None:
                    print("Username: " + str(u).replace(" ", "") + "\tHash: " + codecs.encode(p, "hex").decode())
        nbdbcntr.con.close()
