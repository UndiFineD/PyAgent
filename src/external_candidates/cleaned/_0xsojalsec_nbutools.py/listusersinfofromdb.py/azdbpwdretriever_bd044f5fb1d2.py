# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\ListUsersInfoFromDB\AZDBpwdRetriever.py
#!/usr/bin/python3
import argparse
import base64
import codecs
...
class AZDBpwdRetriever(object):
    ...
    def decrypt_azdb_dba_password(self, pwd):
        ...
        if dec:
            print("AZ_DB_PASSWORD: " + str(pwd) + " corresponds to the NBAZDB.db dba password: " + dec.decode("ascii"))
...
def parse_args():
    parser = argparse.ArgumentParser(description="De-obfuscate the dba password of NBAZDB.db")
    parser.add_argument(
        "--pwd",
        "-p",
        metavar="PASSWORD",
