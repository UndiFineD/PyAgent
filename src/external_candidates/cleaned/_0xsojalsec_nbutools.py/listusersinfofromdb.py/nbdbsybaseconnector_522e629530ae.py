# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\ListUsersInfoFromDB\NBDBSybaseConnector.py
#!/usr/bin/python3
import logging
...
class NBDBSybaseConnector(object):
    ...
    def __init__(self, password, host, port, jconn4_file_path):
        self.con = None
        try:
            logging.debug("Attempting to start JVM with classes from %s", jconn4_file_path)
        ...
        try:
            ...
            logging.debug("Connection to %s with DBA password %s successful", host, password)
        except Exception as e:
            logging.error(
    ...
    def get_username_and_hashedpwd_from_ndbd(self):
        ...
        logging.debug("Executing SQL query '%s'", query.replace("\n", " ").replace("  ", ""))
