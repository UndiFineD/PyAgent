# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\GetConfigInfos\main.py
#!/usr/bin/env python3
import argparse
import asyncio
import enum
...
async def scan(target: str, nb_threads: int, timeout: int, verbose: bool, out_format: OutputFormat):
    out = {}
    tasks = {}
    scanners = [cls(host=target, nb_threads=nb_threads, timeout=timeout, verbose=verbose) for cls in NBU_SCANNERS]
...
def main():
    ...
    parser.add_argument("-t", "--timeout", type=float, default=10, help="Timeout for TCP connections")
    parser.add_argument(
        "-f",
        "--format",
        help="Output format",
    ...
    for i, target in enumerate(targets):
        ...
        out_dict[target] = asyncio.run(scan(target, args.jobs, args.timeout, verbose, out_format))
