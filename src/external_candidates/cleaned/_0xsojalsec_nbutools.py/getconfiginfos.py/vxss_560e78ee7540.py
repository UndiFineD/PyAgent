# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\GetConfigInfos\vxss.py
import logging
...
class TestVxssConfig(ScanBaseClass):
    ...
    async def _run(self):
        ...
        if self.vxss_info:
            ...
            self._prettify(self.vxss_info, "USE_AUTHENTICATION", self.AUTHENTICATION_MAP)
    ...
    def _text(self) -> str:
        ...
        out_lines.append("USE_AUTHENTICATION = {}".format(self.vxss_info["USE_AUTHENTICATION"]))
        for domain in self.vxss_info["AUTHENTICATION_DOMAIN"]:
            out_lines.append(
