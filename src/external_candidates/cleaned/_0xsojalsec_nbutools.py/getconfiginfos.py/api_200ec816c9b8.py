# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\GetConfigInfos\api.py
import asyncio
import json
import logging
...
class GetApiInfo(ScanBaseClass):
    ...
    async def _fetch_data(self, client, url) -> bytes:
        async with self._semaphore:
            try:
                logging.debug("[GetApiInfo] Attempting to fetch information from API at %s", url)
                async with client.get(url, verify_ssl=False, timeout=self.timeout) as res:
                    res.raise_for_status()
                    data = await res.text()
                    logging.debug("[GetApiInfo] Got response from %s: %s", url, data)
                    return data
            except TimeoutError:
                logging.warning("[GetApiInfo] Timed out while connecting API from %s", url)
                return b""
            except aiohttp.ClientError as e:
                logging.warning("[GetApiInfo] Failed to get API info from %s: %s", url, e)
                return b""
    ...
    def _text(self) -> str:
        ...
        if server_info:
            ...
            out_lines.append(f"NetBackup version: {server_info.get('nbuVersion', 'Unknown')}")
