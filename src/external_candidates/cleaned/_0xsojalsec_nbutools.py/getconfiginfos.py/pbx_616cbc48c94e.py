# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\GetConfigInfos\pbx.py
import asyncio
import logging

from utils.network import PBXSocket

from .common import NbuComponentType, ScanState
...
class ListPbxExtensions(ScanBaseClass):
    ...
    PBX_EXTENSIONS = sorted(
        list(OPS_PBX_EXTENSIONS | PRIMARY_PBX_EXTENSIONS | MEDIA_PBX_EXTENSIONS | CLIENT_PBX_EXTENSIONS),
        key=str.casefold,
    )
    ...
    def _guess_role(self) -> NbuComponentType:
        ...
        # each component
        extensions = set([ext for ext in self.extension_states.keys() if self.extension_states[ext]])

        # First, check if it's an OpsCenter since it has very unique extensions
        ops_unique_exts = self.OPS_PBX_EXTENSIONS - (
            self.PRIMARY_PBX_EXTENSIONS | self.MEDIA_PBX_EXTENSIONS | self.CLIENT_PBX_EXTENSIONS
        )
        ...
        primary_unique_exts = self.PRIMARY_PBX_EXTENSIONS - (self.MEDIA_PBX_EXTENSIONS | self.CLIENT_PBX_EXTENSIONS)
    ...
    async def _run(self):
        ...
        self.extension_states = {self.PBX_EXTENSIONS[i]: results[i] for i in range(len(self.PBX_EXTENSIONS))}
