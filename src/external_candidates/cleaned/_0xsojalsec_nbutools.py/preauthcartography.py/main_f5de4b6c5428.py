# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\PreAuthCartography\main.py
#!/usr/bin/python3
import argparse
import asyncio
import logging
...
class NetbackupServer(object):
    ...
    def get_ops_version(self):
        ...
        tree = etree.parse(StringIO(page.content.decode("utf-8")), parser=etree.HTMLParser())
                    ...
                    if ("class" in j.keys()) and (j.attrib["class"] == "wrapper"):
                        for t in j:
                            if t.attrib["class"] == "productname":
                                version = t[0].text.split(" ")[1].replace(".", "") + "0000"
                                break
