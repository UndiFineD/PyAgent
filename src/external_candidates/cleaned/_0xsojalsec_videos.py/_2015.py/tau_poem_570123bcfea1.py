# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\tau_poem.py
#!/usr/bin/env python
...
class TauPoem(Scene):
    ...
    def add_line_and_number(self):
        self.first_digits, new_digit, last_digits = (
            OldTex(
                [
        ...
        first_word, rest_of_line = (
            OldTexText([line_str[:index], line_str[index:]]).to_edge(UP).shift(BUFF * DOWN).split()
        )
    ...
    def first_word_to_last_digit(self):
        if self.line_num == 19:
            ...
            self.play(ApplyMethod(self.first_digits.shift, shift_val, run_time=2.0))
            self.wait(2)
        self.play_over_time_range(
            0,
            2,
            Transform(
                deepcopy(self.first_word),
                self.number_word,
                rate_func=squish_rate_func(smooth),
            ),
        )
        self.play_over_time_range(
            2,
            4,
            Transform(self.number_word, self.new_digit, rate_func=squish_rate_func(smooth)),
        )
    ...
    def line1(self):
        ...
        tau.to_symbol()
        sphere = Mobject()
        sphere.interpolate(two_pi, Sphere().set_color("yellow"), 0.8)
        self.add(two_pi)
        self.wait()
        self.play(CounterclockwiseTransform(two_pi, sphere, rate_func=lambda t: 2 * smooth(t / 2)))
        self.remove(two_pi)
        self.play(CounterclockwiseTransform(sphere, tau, rate_func=lambda t: 2 * (smooth(t / 2 + 0.5) - 0.5)))
        self.remove(sphere)
        self.add(tau)
        self.wait()
    ...
    def line2(self):
        tau = TauCreature()
        tau.make_sad()
        tau.mouth.points = np.array(sorted(tau.mouth.points, lambda p0, p1: cmp(p0[0], p1[0])))
        blinked = deepcopy(tau).blink()
        for eye in blinked.eyes:
            eye.set_color("black")
        self.add(*set(tau.parts).difference(tau.white_parts))
        self.play(*[Transform(*eyes) for eyes in zip(blinked.eyes, tau.eyes)])
    ...
    def line3(self):
        ...
        self.play(*[Transform(*parts) for parts in zip(tau.white_parts, pi.white_parts)])
    ...
    def line4(self):
        ...
        self.add(pi)
        self.wait()
        self.play(Transform(Point(bubble.tip).set_color("black"), bubble))
    ...
    def line8(self):
        ...
        self.play(
            ...
            Transform(pi.right_leg, Point(pi.right_leg.get_points()[0, :]).set_color("black")),
            Transform(pi.mouth, tau.mouth),
            CounterclockwiseTransform(two, Dot(two.get_center()).set_color("black")),
        )
    ...
    def line11(self):
        ...
        self.play(*[Transform(*mobs, run_time=2.0) for mobs in zip(formulae, f_copy)])
    ...
    def line12(self):
        ...
        axes = Axes(radius=radius, interval_size=interval_size)
        ...
        sine_period = Line(axes_center, axes_center + 2 * np.pi * interval_size * RIGHT)
    ...
    def line13(self):
        formula = form_start, two_pi, form_end = (
            OldTex(["\\hat{f^{(n)}}(\\xi) = (", "2\\pi", "i\\xi)^n \\hat{f}(\\xi)"]).shift(DOWN).split()
        )
    ...
    def line14(self):
        pi, bubble = self.pi_speaking("Wait! Area \\\\ of circles")
        self.add(pi)
        self.play(Transform(Point(bubble.tip).set_color("black"), bubble))

    def line15(self):
        pi, bubble = self.pi_speaking("sticks oddly \\\\ to one half when \\\\ tau's preferred.")
        formula = form_start, half, form_end = OldTex(["A = ", "\\frac{1}{2}", "\\tau r^2"]).split()
    ...
    def line16(self):
        self.add(OldTex("\\frac{1}{2}\\tau r^2").scale(2).shift(DOWN))

    def line17(self):
        circle = Dot(radius=1, density=4 * DEFAULT_POINT_DENSITY_1D)
        blue_rgb = np.array(Color("blue").get_rgb())
        white_rgb = np.ones(3)
        circle.rgbas = np.array(
            [alpha * blue_rgb + (1 - alpha) * white_rgb for alpha in np.arange(0, 1, 1.0 / len(circle.rgbas))]
        )
        ...
        def trianglify(xxx_todo_changeme):
            ...
            return (norm * np.log(comp).imag, -norm, 0)

        tau_r = OldTex("\\tau r").shift(1.3 * DOWN)
        r = OldTex("r").shift(0.2 * RIGHT + 0.7 * DOWN)
        lines = [Line(DOWN + np.pi * LEFT, DOWN + np.pi * RIGHT), Line(ORIGIN, DOWN)]
        for line in lines:
            line.set_color("red")

        self.play(ApplyFunction(trianglify, circle, run_time=2.0))
        self.add(tau_r, r)
        self.play(*[ShowCreation(line, run_time=1.0) for line in lines])
        self.wait()
    ...
    def line18(self):
        ...
        self.play(WalkPiCreature(sad_pi, DOWN + 4 * LEFT), run_time=1.0)
        self.wait()
