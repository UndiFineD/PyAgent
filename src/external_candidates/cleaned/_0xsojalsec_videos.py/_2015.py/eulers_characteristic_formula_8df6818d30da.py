# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\eulers_characteristic_formula.py
#!/usr/bin/env python
...
class IllustrateDuality(GraphScene):
    def construct(self):
        ...
        self.play(
            *[Transform(*edge_pair, **kwargs) for edge_pair in zip(self.edges, self.dual_edges)]
            + [
                Transform(Mobject(*[self.vertices[index] for index in cycle]), dv, **kwargs)
...
class IntroduceGraph(GraphScene):
    def construct(self):
        ...
        edges_to_remove = [
            self.edges[self.graph.edges.index(pair)] for pair in [(4, 5), (0, 5), (1, 5), (7, 1), (8, 3)]
        ]

        connected, planar, graph = OldTexText(["Connected ", "Planar ", "Graph"]).to_edge(UP).split()
        ...
        self.play(ShowCreation(Mobject(*edges_to_remove), rate_func=lambda t: 1 - t, run_time=1.0))
        self.wait(2)
        self.remove(not_okay)
...
class PlanarGraphDefinition(Scene):
    def construct(self):
        ...
        graphs = [Mobject(*GraphScene(g).mobjects) for g in [CubeGraph(), CompleteGraph(5), OctohedronGraph()]]
...
class ThreePiecesOfTerminology(GraphScene):
    def construct(self):
        ...
        for mob in self.mobjects:
            mob.fade
        self.play(*[ShowCreation(mob, run_time=1.0) for mob in self.dual_vertices + self.dual_edges])
        self.wait()
...
class PathExamples(GraphScene):
    ...
    def construct(self):
        ...
        for path, non_path in zip(paths, non_paths):
            path_lines = Mobject(
                *[
                    Line(self.get_points()[path[i]], self.get_points()[path[i + 1]]).set_color("yellow")
...
class DefineSpanningTree(GraphScene):
    def construct(self):
        ...
        dollar_signs = Mobject(*[deepcopy(dollar_signs).shift(edge.get_center()) for edge in self.edges])
        ...
        for pair in self.spanning_tree_index_pairs:
            self.play(
                ShowCreation(
                    Line(self.get_points()[pair[0]], self.get_points()[pair[1]]).set_color("yellow"),
                    run_time=run_time_per_branch,
                )
            )
            self.play(ShowCreation(green_dot_at_index(pair[1]), run_time=run_time_per_branch))
        self.wait(2)
...
class FacebookGraph(GraphScene):
    def construct(self):
        ...
        friends = OldTex("\\leftarrow \\text{friends} \\rightarrow").scale(0.5 * EDGE_ANNOTATION_SCALE_FACTOR)
        ...
        self.wait()
        self.play(*[CounterclockwiseTransform(account, vertex) for account, vertex in zip(accounts, self.vertices)])
        self.wait()
        self.play(*[Transform(ann, edge) for ann, edge in zip(self.edge_annotations, self.edges)])
        self.wait()
...
class FacebookGraphAsAbstractSet(Scene):
    def construct(self):
        ...
        friends_string = "\\\\".join(
            ["\\text{%s}&\\leftrightarrow\\text{%s}" % (names[i], names[j]) for i, j in friend_pairs] + ["\\vdots"]
        )
        names_mob = OldTexText(names_string).shift(3 * LEFT)
        friends_mob = OldTex(friends_string, size="\\Large").shift(3 * RIGHT)
...
class ExamplesOfGraphs(GraphScene):
    def construct(self):
        ...
        vertical_line = Line((vert_line_x_val, FRAME_Y_RADIUS, 0), (vert_line_x_val, -FRAME_Y_RADIUS, 0))
        objects_and_notions = [
    ...
    def handle_dual_graph(self, words1, words2):
        ...
        self.play(*[ShowCreation(mob, run_time=1.0) for mob in self.edges + self.vertices])
        self.wait()
        for region in self.regions:
            self.set_color_region(region)
        self.add(words1)
...
class CyclesCorrespondWithConnectedComponents(GraphScene):
    ...
    def construct(self):
        ...
        self.reset_background()
        lines = Mobject(
            *[Line(self.dual_points[last], self.dual_points[next]) for last, next in zip(dual_cycle, dual_cycle[1:])]
        ).set_color("red")
        ...
        self.play(
            *[
                Transform(
                    v,
                    Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS).set_color("green"),
                )
...
class IntroduceMortimer(GraphScene):
    ...
    def construct(self):
        ...
        morty_path = (-2, -3, -4)
        morty_crossed_lines = [
            Line(self.get_points()[i], self.get_points()[j]).set_color("red") for i, j in [(7, 1), (1, 5)]
        ]
        kwargs = {"run_time": 1.0}
        ...
        self.wait()
        self.play(
            *[ApplyMethod(line.rotate, np.pi / 10, rate_func=wiggle) for line in morty_crossed_lines],
            **kwargs,
        )
...
class MortimerCannotTraverseCycle(GraphScene):
    ...
    def construct(self):
        ...
        self.play(
            *[
                Transform(v, Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS, color="green"))
...
class TwoPropertiesOfSpanningTree(Scene):
    def construct(self):
        spanning, tree = OldTexText(["Spanning ", "Tree"], size="\\Huge").split()
...
class DualSpanningTree(GraphScene):
    def construct(self):
        ...
        self.play(ShowCreation(Mobject(*np.array(self.edges)[dual_edges]).set_color("red")))
        self.add(words)
        self.wait()
...
class FinalSum(Scene):
    def construct(self):
        ...
        symbols = V, minus, E, plus, F, equals, two = OldTex("V - E + F = 2".split(" "))
        plus = OldTex("+")
        anims = []
