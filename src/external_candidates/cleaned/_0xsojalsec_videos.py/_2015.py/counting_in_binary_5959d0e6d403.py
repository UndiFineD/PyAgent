# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\counting_in_binary.py
#!/usr/bin/env python
...
class Hand(ImageMobject):
    STARTING_BOTTOM_RIGHT = [4.61111111e00, -3.98888889e00, 9.80454690e-16]

    def __init__(self, num, small=False, **kwargs):
        Mobject2D.__init__(self, **kwargs)
        path = os.path.join(VIDEO_DIR, MOVIE_PREFIX, "images", "Hand%d.png" % num)
        invert = False
...
def read_reversed_binary(string):
    return sum([2**count if char == "1" else 0 for count, char in zip(it.count(), string)])


class LeftHand(Hand):
    def __init__(self, num, **kwargs):
        Hand.__init__(self, read_reversed_binary(five_char_binary(num)), **kwargs)
...
def get_hand_map(which_hand="right"):
    if which_hand == "right":
        ...
        print("Bad arg, bro")
        return
    return dict([(num, Class(num, small=True)) for num in range(32)])
...
class SaveEachNumber(OverHand):
    def construct(self):
        ...
        for count in COUNT_TO_FRAME_NUM:
            path = os.path.join(VIDEO_DIR, MOVIE_PREFIX, "images", "Hand%d.png" % count)
...
class ShowCounting(OverHand):
    def construct(self):
        ...
        self.frames = [
            disp.paint_mobject(self.get_counting_mob(32 * count // num_frames), frame)
...
class ShowFrameNum(OverHand):
    def construct(self):
        ...
        for frame, count in zip(self.frames, it.count()):
            ...
            mob = Mobject(*[OldTex(char).shift(0.3 * x * RIGHT) for char, x in zip(str(count), it.count())])
            self.frames[count] = disp.paint_mobject(mob.to_corner(UP + LEFT), frame)
...
class CountTo1023(Scene):
    def construct(self):
        ...
        def get_num(count):
            return (
                Mobject(*[OldTex(char).shift(0.35 * x * RIGHT) for char, x in zip(str(count), it.count())])
                .center()
                .to_edge(UP)
            )

        self.frames = [
            disp.paint_mobject(Mobject(rh_map[count % 32], lh_map[count // 32], get_num(count)))
...
class ShowReadingRule(Scene):
    ...
    def read_sample(self, num):
        ...
        if num in [6, 17]:
            hand.shift(0.8 * LEFT)
        sum_mobs = (
            OldTex(" + ".join([str(2**c) for c in counts]).split(" ") + ["=%d" % num]).to_corner(UP + RIGHT).split()
        )
...
class ShowIncrementRule(Scene):
    ...
    def get_arrow_set(self, num):
        ...
        return Mobject(*[deepcopy(arrow).shift(tip) for tip in tips])
...
class MindFindsShortcuts(Scene):
    def construct(self):
        ...
        compound = Mobject(Arrow(hand, sum421), sum421, Arrow(sum421, seven))
        self.add(words1, hand, compound, seven)
        self.wait()
        self.play(
        ...
        self.clear()
        hands = dict([(num, Hand(num).center().scale(0.5).shift(DOWN)) for num in [23, 16, 7]])
...
class TwoHandStatement(Scene):
    def construct(self):
        self.add(OldTexText("With 10 fingers, you can count up to $2^{10} - 1 = 1023$..."))
