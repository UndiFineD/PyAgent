# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\playground_counting_in_binary.py
#!/usr/bin/env python
...
class BufferedCounting(SceneFromVideo):
    ...
    def buffer_pixels(self, spreads=(2, 2)):
        ...
        matrices = [thick_diagonal(dim, spread) for dim, spread in zip(self.shape, spreads)]
    ...
    def make_all_black_or_white(self):
        self.frames = [255 * (frame != 0).astype("uint8") for frame in self.frames]
...
class ClearLeftSide(SceneFromVideo):
    ...
    def construct(self, scenename):
        ...
        SceneFromVideo.construct(self, path)
        self.set_color_region_over_time_range(Region(lambda x, y: x < -1, shape=self.shape))
    ...
    def drag_pixels(self, num_frames_to_drag_over=5):
        for index in range(len(self.frames) - 1, 0, -1):
            self.frames[index] = np.max(
                [self.frames[k] for k in range(max(index - num_frames_to_drag_over, 0), index)],
                axis=0,
            )
    ...
    def construct(self, filename):
        ...
        for count in range(32):
            ...
            for index in index_range:
                self.frames[index] = disp.paint_mobject(mob, self.frames[index])
