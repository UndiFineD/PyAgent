# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\moser_intro.py
#!/usr/bin/env python
...
def logo_to_circle():
    ...
    sc = Scene()
    small_circle = (
        Circle(density=CIRCLE_DENSITY, color="skyblue")
        .scale(LOGO_RADIUS)
        .set_color(DARK_BROWN, lambda x_y_z: x_y_z[0] < 0 and x_y_z[1] > 0)
    )
...
def interesting_problems():
    ...
    fermat_copies, face_copies, tb_copies, sb_copies = (
        Mobject(*[deepcopy(mob).scale(0.5).shift(locale) for locale in locales]) for mob in [fermat, face, tb, sb]
    )
    ...
    sc.wait()
    sc.animate(
        Transform(deepcopy(fermat).repeat(len(locales)), fermat_copies),
...
def different_points(radians1, radians2):
    ...
    sc.add(circle)
    points1, points2 = (
        [(RADIUS * np.cos(angle), RADIUS * np.sin(angle), 0) for angle in radians] for radians in (radians1, radians2)
    )
    dots1, dots2 = (Mobject(*[Dot(point) for point in points]) for points in (points1, points2))
    lines1, lines2 = (
        [Line(point1, point2) for point1, point2 in it.combinations(points, 2)] for points in (points1, points2)
    )
    sc.add(dots1, *lines1)
    sc.animate(
        Transform(dots1, dots2, run_time=3),
        *[Transform(line1, line2, run_time=3) for line1, line2 in zip(lines1, lines2)],
    )
    sc.wait()
    return sc
...
def next_few_videos(*radians):
    ...
    sc.animate(
        ...
        Transform(thumbnail, deepcopy(thumbnail).scale(0.15).shift((-5, 0, 0))),
    )
