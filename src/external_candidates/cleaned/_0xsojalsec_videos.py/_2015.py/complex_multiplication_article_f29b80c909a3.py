# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\complex_multiplication_article.py
#!/usr/bin/env python
...
DEFAULT_PLANE_CONFIG = {"stroke_width": 2 * DEFAULT_STROKE_WIDTH}
...
class ShowComplexPower(SuccessiveComplexMultiplications):
    ...
    def construct(self, multiplier, num_repeats):
        SuccessiveComplexMultiplications.construct([multiplier] * num_repeats)
...
class ComplexDivision(ComplexMultiplication):
    ...
    def construct(self, num):
        ComplexMultiplication.construct(self, 1.0 / num)
...
class DrawSolutionsToZToTheNEqualsW(Scene):
    ...
    def construct(self, n, w):
        ...
        circle = Circle(radius=radius * zoom_value, stroke_width=plane.stroke_width)
        solutions = [radius * np.exp(complex(0, 1) * (2 * np.pi * k + theta) / n) for k in range(n)]
        points = list(map(plane.number_to_point, solutions))
        dots = [Dot(point, color=BLUE_B, radius=0.1) for point in points]
...
class DrawComplexAngleAndMagnitude(Scene):
    args_list = [
        (
            ...
            ("\\frac{\\sqrt{3}}{2} - \\frac{1}{2}i", complex(np.sqrt(3) / 2, -1.0 / 2)),
        ),
        (("1+i", complex(1, 1)),),
    ]
    ...
    def add_angle_label(self, number):
        arc = Arc(np.log(number).imag, radius=0.2)
    ...
    def add_lines(self, tex_representation, number):
        ...
        x_line, y_line, num_line = [
            Line(start, end, color=color, stroke_width=self.plane.stroke_width)
