# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\music_and_measure.py
#!/usr/bin/env python
...
class Piano(ImageMobject):
    CONFIG = {"stroke_width": 1, "invert": False, "scale_factorue": 0.5}
    ...
    def split(self):
        ...
        for count in range(14):
            key = Mobject(color="white", stroke_width=1)
            x0 = left + count * self.ivory_jump
            x1 = x0 + self.ivory_jump
            key.add_points(self.get_points()[(self.get_points()[:, 0] > x0) * (self.get_points()[:, 0] < x1)])
            keys.append(key)
        return keys
...
class IntervalScene(NumberLineScene):
    ...
    def show_all_fractions(self, num_fractions=27, pause_time=1.0, remove_as_you_go=True):
    ...
    def cover_fractions(self, epsilon=0.3, num_fractions=10, run_time_per_interval=0.5):
        intervals = []
        lines = []
        num_intervals = 0
        ...
        while True:
            ...
            interval, line = self.add_open_interval(
                fraction, epsilon / min(2**count, 2**30), run_time=run_time_per_interval
            )
...
class ChallengeOne(Scene):
    def construct(self):
        ...
        self.bottom_vibration = Vibrate(num_periods=1, run_time=3.0, center=DOWN, color=start_color)
        top_vibrations = [
            Vibrate(num_periods=freq, run_time=3.0, center=2 * UP, color=next(colors))
    ...
    def continuously_vary_frequency(self, top_vib_1, top_vib_2):
        ...
        one, two = (
            2 * number_line.interval_size * RIGHT,
            4 * number_line.interval_size * RIGHT,
        )
        arrow1 = Arrow(one + UP, one)
        arrow2 = Arrow(two + UP, two)
        r1 = OldTex("r").next_to(arrow1, UP)
        r2 = OldTex("r").next_to(arrow2, UP)
        kwargs = {"run_time": 5.0, "rate_func": there_and_back}
        run_time = 3.0
...
class QuestionAndAnswer(Scene):
    def construct(self):
        ...
        answer = OldTexText(
            ["When $r$ is", "sufficiently close to", "a rational number"],
            size="\\small",
        ).scale(1.5)
        answer.next_to(A)
        correction1 = OldTexText("with sufficiently low denominator", size="\\small").scale(1.5)
...
class PlaySimpleRatio(Scene):
    ...
    def construct(self, fraction, color):
        string1 = Vibrate(num_periods=1, run_time=5.0, center=DOWN, color="blue")
        string2 = Vibrate(num_periods=fraction, run_time=5.0, center=2 * UP, color=color)
...
class LongSine(Mobject1D):
    def init_points(self):
        self.add_points([(x, np.sin(2 * np.pi * x), 0) for x in np.arange(0, 100, self.epsilon / 10)])
...
class DecomposeMusicalNote(Scene):
    def construct(self):
        ...
        sine = LongSine()
        kwargs = {"run_time": 4.0, "rate_func": None}
...
class DecomposeTwoFrequencies(Scene):
    def construct(self):
        ...
        self.add(line)
        self.play(ApplyMethod(comp.shift, 15 * LEFT, run_time=7.5, rate_func=linear))
...
class PatternInFrequencies(Scene):
    ...
    def construct(self, num1, num2, color):
        ...
        top_lines = Mobject(
            *[deepcopy(line_template).shift(k * (float(num1) / num2) * RIGHT) for k in range(num_top_lines)]
        )
        line_template.shift(4 * DOWN)
        bottom_lines = Mobject(*[deepcopy(line_template).shift(k * RIGHT) for k in range(num_bot_lines)])
        bottom_lines.set_color("blue")
        top_lines.set_color(color)
        kwargs = {"run_time": 10, "rate_func": None}

        self.add(big_line)
        self.add(OldTex("%d:%d" % (num1, num2)))
        fracs = (1.0 / (num_top_lines - 1), 1.0 / (num_bot_lines - 1))
        anims = [ApplyMethod(mob.shift, setup_width * LEFT, **kwargs) for mob in (top_lines, bottom_lines)]
        anim_mobs = [anim.mobject for anim in anims]
        self.play(FlashOnXProximity(big_line, -2, *anim_mobs, **kwargs), *anims)
...
class CompareFractionComplexity(Scene):
    def construct(self):
        ...
        self.play(*[ShowCreation(arrow) for arrow in (arrow1, arrow2)])
        self.wait()
        self.play(*[DelayByOrder(ApplyMethod(frac[1].set_color, "yellow")) for frac in (frac0, frac1)])
        self.play(FadeIn(indicates), ShowCreation(arrow3), ShowCreation(arrow4))
        self.wait()
...
class PianoTuning(Scene):
    def construct(self):
        ...
        semicircles = Mobject(*[deepcopy(semicircle).shift(jump * k * RIGHT) for k in range(23)])
        ...
        self.play(ShowCreation(line), FadeIn(twelfth_root))
        self.wait()
...
class PowersOfTwelfthRoot(Scene):
    def construct(self):
        ...
        for i in range(1, num_terms + 1):
            ...
            error_string = OldTexText(["with", str(percent_error)[:4] + "\\%", "error"])
            ...
            mob_list.append(Mobject(*[term, approx_copy, approx_form, words, frac_mob, error_string]))
...
class SupposeThereIsASavant(Scene):
    def construct(self):
        words = (
            "Suppose there is a musical savant "
...
class AllValuesBetween1And2(NumberLineScene):
    def construct(self):
        ...
        one, two, irr = list(map(self.number_line.number_to_point, [1, 2, irrational]))
        ...
        approximations = [continued_fraction(cont_frac[:k]) for k in range(1, len(cont_frac))[:8]]

        kwargs = {"run_time": 3.0, "rate_func": there_and_back}
        self.play(*[ApplyMethod(mob.shift, RIGHT, **kwargs) for mob in (r, top_arrow)])
        self.wait()
        self.remove(r, top_arrow)
        self.play(ShimmerIn(irr_mob), ShowCreation(bot_arrow))
        self.wait()
        ...
        for approx in approximations:
            ...
            self.play(Transform(top_arrow, new_arrow), Transform(frac_mob, mob), run_time=0.5)
            self.wait(0.5)
...
class CoveringSetsWithOpenIntervals(IntervalScene):
    def construct(self):
        IntervalScene.construct(self)
        dots = Mobject(
            *[Dot().shift(self.number_line.number_to_point(num) + UP) for num in set([0.2, 0.25, 0.45, 0.6, 0.65])]
        )
        theorems = [
        ...
        for x in range(2 * len(theorems)):
            self.play(*[ApplyMethod(th.shift, UP, rate_func=linear) for th in theorems[: x + 1]])
...
class DefineOpenInterval(IntervalScene):
    def construct(self):
        ...
        a, less_than1, x, less_than2, b = OldTex(["a", "<", "x", "<", "b"]).shift(UP).split()
...
class ShowAllFractions(IntervalScene):
    def construct(self):
        IntervalScene.construct(self)
        self.show_all_fractions(num_fractions=100, remove_as_you_go=False, pause_time=0.3)
        self.wait()
        self.play(
            *[
...
class NaiveFractionCover(IntervalScene):
    def construct(self):
        ...
        last_interval = None
        centers = np.arange(0, 1.1, 0.1)
        random.shuffle(centers)
        for num, count in zip(centers, it.count()):
            if count == 0:
                kwargs = {"run_time": 1.0, "rate_func": rush_into}
            elif count == 10:
                kwargs = {"run_time": 1.0, "rate_func": rush_from}
            else:
                kwargs = {"run_time": 0.25, "rate_func": None}
...
class SumOfIntervalsMustBeLessThan1(IntervalScene):
    def construct(self):
        ...
        for num in np.arange(0, 1.1, 0.1):
            open_interval, line = self.add_open_interval(num, 0.1)
            self.remove(line)
...
class HowCanYouNotCoverEntireInterval(IntervalScene):
    def construct(self):
        ...
        self.wait()
        self.remove(ticks)
        self.play(*[Transform(tick, full_line) for tick in ticks.split()])
...
class StepsToSolution(IntervalScene):
    ...
    def assign_intervals_to_rationals(self):
        ...
        self.play(*[ApplyMethod(mob.shift, FRAME_WIDTH * RIGHT) for mob in to_remove])
        self.remove(*to_remove)
        self.wait()

        self.intervals = [a.mobject for a in anims]
        kwargs = {"run_time": 2.0, "rate_func": there_and_back}
        self.play(*[ApplyMethod(mob.scale, 0.5 * random.random(), **kwargs) for mob in self.intervals])
        self.wait()
    ...
    def multiply_by_epsilon(self):
        self.play(*[CounterclockwiseTransform(one, OldTex("\\epsilon").replace(one)) for one in self.ones])
        self.wait()

    def stretch_intervals(self):
        for interval, count in zip(self.intervals, it.count(1)):
            self.play(ApplyMethod(interval.scale, 1.0 / (count**2)), run_time=1.0 / count)
        self.wait()
...
class TroubleDrawingSmallInterval(IntervalScene):
    def construct(self):
        ...
        for target in shrunk, small:
            mob = deepcopy(big)
            self.play(Transform(mob, target, run_time=2.0))
            self.wait()
        ...
        self.play(
            Transform(
                line,
                deepcopy(line).scale(10).shift(DOWN),
                run_time=2.0,
...
class WhatDoesItLookLikeToBeOutside(Scene):
    def construct(self):
        self.add(OldTexText("What does it look like for a number to be outside a dense set of intervals?"))
...
class ShiftSetupByOne(IntervalScene):
    def construct(self):
        ...
        for mobs, shift_val in mobs_shifts:
            self.play(*[ApplyMethod(mob.shift, shift_val) for mob in mobs])
        self.number_line = new_interval
        self.wait()
        words = OldTexText("Almost all the covered numbers are harmonious!", size="\\small").shift(2 * UP)
        self.play(ShimmerIn(words))
        self.wait()
        for num in [7, 5]:
            point = self.number_line.number_to_point(2 ** (num / 12.0))
            arrow = Arrow(point + DOWN, point)
            mob = OldTex("2^{\\left(\\frac{%d}{12}\\right)}" % num).next_to(arrow, DOWN)
            self.play(ShimmerIn(mob), ShowCreation(arrow))
            self.wait()
            self.remove(mob, arrow)
        self.remove(words)
        words = OldTexText("Cacophonous covered numbers:", size="\\small")
        words.shift(2 * UP)
        answer1 = OldTexText("Complicated rationals,", size="\\small")
        answer2 = OldTexText("real numbers \\emph{very very very} close to them", size="\\small")
        ...
        words = OldTexText(
            [
                "To a",
                "savant,",
                "harmonious numbers could be ",
                "\\emph{precisely}",
                "those 1\\% covered by the intervals",
            ]
        ).shift(2 * UP)
        words = words.split()
        words[1].set_color()
        words[3].set_color()
        self.add(*words)
        self.play(ShowCreation(Mobject(*intervals), run_time=5.0))
        self.wait()
...
class FinalEquivalence(IntervalScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(ticks),
            Transform(deepcopy(intervals).stretch_to_fit_height(0), intervals),
        )
        everything = Mobject(*self.mobjects)
        self.clear()
        self.play(Transform(everything, deepcopy(everything).scale(0.5).to_edge(LEFT)))
        self.add(arrow)
        self.play(ShimmerIn(words))
        self.wait()
