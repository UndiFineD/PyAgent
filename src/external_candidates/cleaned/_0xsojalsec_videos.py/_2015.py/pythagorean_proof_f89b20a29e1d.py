# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\pythagorean_proof.py
...
from mobject.region import region_from_line_boundary, region_from_polygon_vertices
...
class AddParallelLines(DrawAllThreeSquares):
    ...
    def construct(self, num, trim):
        ...
        shift_pairs = [(4 * RIGHT, 3 * UP), (ORIGIN, DOWN), (3 * LEFT, 2 * DOWN)]
...
class HighlightEmergentTriangles(AddParallelLines):
    ...
    def construct(self, *args):
        ...
        for triplet in triplets:
            self.set_color_region(region_from_polygon_vertices(*triplet), color="BLUE_D")
    ...
    def construct(self, num, fill):
        ...
        for n in range(num):
            ...
            if n not in to_flip:
                vertices.reverse()
            if fill:
                self.set_color_region(region_from_polygon_vertices(*vertices), color=BLUE_D)
...
class FillInAreaOfBigRectangle(DrawAllThreeSquaresWithMoreTriangles):
    ...
    def construct(self, *args):
        ...
        self.set_color_region(
            region_from_polygon_vertices((-3, 3), (-3, -2), (4, -2), (4, 3)),
            color=color,
        )
...
class DrawTriangleWithAngles(Scene):
    def construct(self):
        ...
        angle1_arc = (
            ...
            .filter_out(lambda x_y_z: not (x_y_z[0] > 0 and x_y_z[1] < 0 and x_y_z[1] < -3 * x_y_z[0]))
            .shift(vertices[1])
        )
        angle2_arc = (
            Circle(radius=0.2, **kwargs)
            .filter_out(lambda x_y_z1: not (x_y_z1[0] < 0 and x_y_z1[1] > 0 and x_y_z1[1] < -3 * x_y_z1[0]))
            .shift(vertices[2])
        )
        alpha = OldTex("\\alpha")
...
class FillComponentsOfLargeSquare(LabelLargeSquare):
    def construct(self):
        ...
        self.set_color_region(
            region_from_line_boundary(*[[p1, p2] for p1, p2 in zip(vertices, vertices[1:])]),
            color=YELLOW,
        )
...
class ShowRearrangementInBigSquare(DrawCSquareWithAllTraingles):
    ...
    def construct(self):
        ...
        for i, j in [(0, 2), (3, 1)]:
            self.triangles[i].place_hypotenuse_on(*self.triangles[j].get_vertices()[[2, 1]])
...
class ShowRearrangementInBigSquareWithRegions(ShowRearrangementInBigSquare):
    def construct(self):
        ...
        self.set_color_region(
            region_from_polygon_vertices(2 * (LEFT + UP), 2 * LEFT + DOWN, RIGHT + DOWN, RIGHT + 2 * UP),
            color=B_COLOR,
        )
        self.set_color_region(
