# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\moser_main.py
#!/usr/bin/env python
    ...
    def __init__(self, *args, **kwargs):
        ...
        fermat = dict(
            [
                (
                    sym,
                    Mobject(*Texs(["x", "^" + sym, "+", "y", "^" + sym, "=", "z", "^" + sym])),
                )
                for sym in ["n", "2", "3"]
            ]
        )
        ...
        circle_grid = Mobject(Circle(), Grid(radius=2), OldTex(r"\mathds{R}^2").shift((2, -2, 0)))
        ...
        other_grid = Mobject(Grid(radius=2), OldTex(r"\mathds{C}").shift((2, -2, 0)))
        omega = np.array((0.5, 0.5 * np.sqrt(3), 0))
        dots = Mobject(
            *[Dot(t * np.array((1, 0, 0)) + s * omega) for t, s in it.product(list(range(-2, 3)), list(range(-2, 3)))]
        )
        ...
        self.play(
            ...
            Transform(Point((-FRAME_X_RADIUS, 0, 0)), Mobject(*CircleScene(RADIANS).mobjects)),
        )
...
class CountLines(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        n = len(radians)
        formula, answer = OldTex([r"{%d \choose 2} = \frac{%d(%d - 1)}{2} = " % (n, n, n), str(choose(n, 2))])
...
class LineCorrespondsWithPair(CircleScene):
    ...
    def __init__(self, radians, dot0_index, dot1_index, *args, **kwargs):
        ...
        self.play(
            *[Transform(dot, Dot(dot.get_center(), 3 * dot.radius), rate_func=there_and_back) for dot in (dot0, dot1)]
        )
...
class IllustrateNChooseK(Scene):
    ...
    def __init__(self, n, k, *args, **kwargs):
        ...
        tuple_mobs = Texs(
            [(r"\\&" if c % (20 // k) == 0 else r"\;\;") + str(p) for p, c in zip(tuples, it.count())],
            size=r"\small",
        )  # TODO, scale these up!
        tuple_terms = {
            2: "pairs",
        ...
        form1, count, form2 = OldTex([str1, "%d" % choose(n, k), r" \text{ total %s}" % tuple_term])
...
class EulersFormula(GraphScene):
    def __init__(self, *args, **kwargs):
        ...
        new_form = dict(
            [(key, deepcopy(mob).shift((0, -0.7, 0))) for key, mob in zip(list(form.keys()), list(form.values()))]
        )
        self.add(formula)
...
class ShowMoserGraphLines(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        n_choose_2, plus_2_n_choose_4, plus_n = OldTex([r"{n \choose 2}", r"&+2{n \choose 4}\\", r"&+n"]).split()
...
class ApplyEulerToMoser(CircleScene):
    def __init__(self, *args, **kwargs):
        ...
        F[3], equals[3], E[3], minus[3], n[3], minus1[3], nc4[3], plus[3], two[3] = OldTex(
        ...
        F[6], equals[6], nc2[6], plus1[6], nc4[6], plus[6], two[6] = OldTex(
            ["F", "=", r"{n \choose 2}", "+", r"{n \choose 4}", "+", "2"]
        ).split()
        F[7], equals[7], two[7], plus[7], nc2[7], plus1[7], nc4[7] = OldTex(
        ...
        self.play(
            *[
            ...
            + [Transform(deepcopy(line), deepcopy(line).fade(0.4)) for line in self.lines + self.circle_pieces]
        )
        self.wait()
        ...
        self.remove(*self.mobjects)
        self.play(
            Transform(E[3], Mobject(nc2[4], plus1[4], two1[4], nc41[4], plus2[4], n1[4])),
            *[Transform(d[3], d[4]) for d in [F, equals, minus, n, minus1, nc4, plus, two]]
            + [
                Transform(
                    deepcopy(line),
                    deepcopy(line).scale(0.5),
            ...
            + [Transform(deepcopy(cp), scp) for cp, scp in zip(self.circle_pieces, self.smaller_circle_pieces)],
            run_time=2.0,
        )
        self.wait()
        self.remove(*self.mobjects)
        self.play(
            ...
            *[Transform(d[4], d[5]) for d in [F, equals, nc2, plus1, two1, nc41, minus1, nc4, plus, two]],
        )
        self.wait()
        self.remove(*self.mobjects)
        self.play(
...
class FormulaRelatesToPowersOfTwo(Scene):
    def __init__(self, *args, **kwargs):
        ...
        powers_of_two = [
            OldTex("2^{%d}" % (i - 1)).scale(scale_factor).shift(result.get_center()).set_color()
...
class PascalsTriangleWithNChooseK(PascalsTriangleScene):
    def __init__(self, *args, **kwargs):
        ...
        for i in [0, 1]:
            ...
            self.play(
                *[
                    CounterclockwiseTransform(deepcopy(mob_dicts[i][n][k]), mob_dicts[1 - i][n][k])
...
class PascalsTriangleNChooseKExample(PascalsTriangleScene):
    ...
    def __init__(self, nrows, n, k, *args, **kwargs):
        ...
        formula_terms = left, n_mob, k_mob, right = OldTex([r"\left(", str(n), r"\atop %d" % k, r"\right)"])
    ...
    def __init__(self, nrows, n, *args, **kwargs):
        ...
        self.play(
            *[
            ...
            + [Transform(term, target_term) for term, target_term in zip(terms, target_terms)]
        )
        self.wait()
        ...
        self.play(
            *[
                CounterclockwiseTransform(self.coords_to_n_choose_k[n0][k0], self.coords_to_mobs[n0][k0])
                for n0, k0 in self.coords
...
class ExplainNChoose2Formula(Scene):
    ...
    def __init__(self, n, a, b, *args, **kwargs):
        ...
        n_mob, n_minus_1, over_2 = OldTex([str(n), "(%d-1)" % n, r"\over{2}"]).split()
...
class ExplainNChoose4Formula(Scene):
    ...
    def __init__(self, n, *args, **kwargs):
        ...
        for i in range(4):
            ...
            curr_num = quad[i]
        self.remove(*self.mobjects)
        num_perms_explain = OldTexText(r"There are $(4 \cdot 3 \cdot 2 \cdot 1)$ total permutations").shift((0, -2, 0))
        self.add(parens, num_perms_explain, *form_mobs)
        perms = list(it.permutations(list(range(4))))
        for count in range(6):
            perm = perms[randint(0, 23)]
            new_quad_mobs = [
                deepcopy(quad_mobs[i]).shift(quad_mobs[perm[i]].get_center() - quad_mobs[i].get_center())
                for i in range(4)
            ]
