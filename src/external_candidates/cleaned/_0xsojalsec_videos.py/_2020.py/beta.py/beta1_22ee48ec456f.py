...
class ShowThreeCases(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *[LaggedStartMap(FadeInFromLarge, review, lag_ratio=0.1) for review in reviews],
                lag_ratio=0.1,
            )
        )
        self.add(reviews)
...
class PreviewThreeVideos(Scene):
    def construct(self):
        ...
        # Show videos
        images = [
            ImageMobject(os.path.join(consts.VIDEO_DIR, OUTPUT_DIRECTORY, "images", name))
...
class LaplacesRuleOfSuccession(Scene):
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(review_label[0], lambda m, a: m.set_value(int(interpolate(10, 11, a)))),
            UpdateFromAlphaFunc(review_label[2], lambda m, a: m.set_value(int(interpolate(10, 12, a)))),
        ...
        self.play(
            ...
            data.get_center()[0] + 3,
            MaintainPositionRelativeTo(review_label, review_center),
            UpdateFromAlphaFunc(review_label[0], lambda m, a: m.set_value(int(interpolate(48, 49, a)))),
            UpdateFromAlphaFunc(review_label[2], lambda m, a: m.set_value(int(interpolate(50, 52, a)))),
            old_review_label.scale,
            0.5,
            old_review_label.to_edge,
            LEFT,
...
class WhatsTheModel(Scene):
    ...
    def introduce_buyer_and_seller(self):
        ...
        self.play(
            LaggedStartMap(FadeInFromDown, VGroup(seller, buyer, *labels), lag_ratio=0.2),
            *added_anims,
        )
        self.wait()
...
class IsSellerOne100(Scene):
    ...
    def add_review(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                tens[0],
                lambda m, a: m.set_color(interpolate_color(RED, GREEN, a)).set_value(len(m.to_count)),
            ),
            run_time=2,
    ...
    def show_random_numbers(self):
        ...
        rows = VGroup(
            *[
                VGroup(*[Integer(random.randint(0, 99)).move_to(0.85 * x * RIGHT) for x in range(10)])
                for y in range(10 * 2)
            ]
        )
