# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\beta\beta2.py
import scipy.stats
...
class WeightedCoin(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(grid.set_height, 5, grid.to_edge, DOWN, FadeInFromDown(p_label))
        ...
        for value in [0.2, 0.9, 0.0, 0.31]:
            ...
            self.wait()
            self.play(
                FadeOut(old_grid, 0.1 * DOWN, lag_ratio=0.01, run_time=1.5),
        ...
        random.shuffle(bools)
        coins = VGroup(*[get_coin("H" if heads else "T") for heads in bools])
        coins.arrange(RIGHT)
        ...
        numbers = VGroup(*[Integer(i + 1).next_to(arrow, DOWN, SMALL_BUFF) for i, arrow in enumerate(heads_arrows)])
        ...
        self.play(GrowFromPoint(eq_label[1], v_line.get_center()))
        self.wait()
        ...
        title = OldTexText("Probabilities", "of", "Probabilities")
...
class ShowInfiniteContinuum(Scene):
    def construct(self):
        ...
        self.play(
            ShowCreation(x_line),
            UpdateFromFunc(h_part, lambda m: m.next_to(m.tracked.get_end(), UR, SMALL_BUFF)),
            UpdateFromFunc(value, lambda m: m.set_value(m.x_axis.p2n(m.tracked.get_end()))),
            run_time=3,
        )
        self.wait()
        self.play(
        ...
        def get_arrow(s, denom, arrow_template=arrow_template, axes=axes):
            ...
            arrow.set_color(interpolate_color(GREY_A, GREY_C, random.random()))
            return arrow
...
class TryAssigningProbabilitiesToSpecificValues(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(zero, DOWN), FadeOut(infty, UP), morty.change, "sad", zero)
        self.play(Blink(morty))
        self.wait()
...
class WanderingArrow(Scene):
    def construct(self):
        ...
        for u in [1, -1, 1, -1, 1]:
            self.play(arrow.shift, u * 8 * RIGHT, run_time=3)
...
class CarFactoryNumbers(Scene):
    def construct(self):
        ...
        numer_words = OldTexText("2 defects found", tex_to_color_map={"2": RED})
...
class ShowLimitToPdf(Scene):
    def construct(self):
        ...
        brace = Brace(Line(arrows[0].get_start(), arrows[1].get_start()), UP, buff=SMALL_BUFF)
        ...
        self.play(ShowCreation(height_cross), FadeOut(axis_prob_label, LEFT))
        self.wait()
        self.play(
        ...
        self.play(randy.change, "confused", FadeIn(bubble.content, lag_ratio=0.1))
        self.play(Blink(randy))
        ...
        self.play(ShowCreation(graph_curve), Write(limit_words, run_time=1))
        ...
        prob_label.target.next_to(all_ineq_labels, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        ...
        step = 0.05
        new_y_numbers = VGroup(*[DecimalNumber(x) for x in np.arange(step, 5 * step, step)])
        ...
        side_label = side_brace.get_tex("{\\text{Prob.} \\over \\Delta x}")
        ...
        self.play(
            bars.arrange,
            RIGHT,
            {"aligned_edge": DOWN, "buff": SMALL_BUFF},
            bars.move_to,
            bars.get_bottom() + 0.5 * UP,
            DOWN,
        )
        ...
        for step_size in step_sizes[1:]:
            ...
            if step_size <= 0.05:
                new_bars.set_stroke(width=0)
            self.play(
                ReplacementTransform(bars, new_bars, lag_ratio=step_size),
                run_time=3,
            )
            self.wait()
            bars = new_bars
        self.add(graph, total_label)
        self.play(FadeIn(graph), FadeOut(bars), total_label.move_to, axes.c2p(0.7, 0.8))
        self.wait()
        ...
        self.play(
            ...
            initials,
            UP,
            *[ReplacementTransform(np[0], ip[0]) for np, ip in zip(func_name_copy, initials)],
            *[FadeOut(np[1:]) for np in func_name_copy],
        )
        self.add(initials)
        self.wait()
        self.play(
            ...
            FadeOut(graph_curve),
            initials.next_to,
            axes.input_to_graph_point(0.95, graph_curve),
            UR,
        )
        ...
        rhs = VGroup(OldTex("="), area_label.copy())
        rhs.arrange(RIGHT)
        ...
        self.play(
            UpdateFromAlphaFunc(
                region,
                lambda m, a: m.become(m.func(m.axes, m.graph, m.min_x, interpolate(m.min_x, m.max_x, a))),
            ),
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                area_label,
                lambda m, a: m.set_width(interpolate(m.original_width, 1e-6, a)),
            ),
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                area_label,
                lambda m, a: m.set_width(interpolate(1e-6, m.original_width, clip(5 * a, 0, 1))),
            ),
    ...
    def get_axes(self):
        ...
        axes.x_axis.add_numbers(*np.arange(0.2, 1.2, 0.2), num_decimal_places=1)
...
class ComplainAboutRuleChange(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_all_student_changes("pondering", look_at=self.teacher.bubble)
        self.wait(8)
...
class SumToIntegral(Scene):
    def construct(self):
        ...
        morty.to_corner(DR)
        morty.look_at(i_sym)
        self.play(*map(FadeOut, [axes, bars, addition_formula]), FadeIn(morty))
...
class TiePossibleToDensity(Scene):
    def construct(self):
        ...
        self.add(poss)
        self.play(FadeIn(prob, LEFT), Write(implies, run_time=1))
        self.wait()
        self.play(ShowCreation(cross))
        self.wait()

        self.play(
            VGroup(implies, cross, prob).shift,
            UP,
            FadeIn(implies2),
            FadeIn(dens),
        )
