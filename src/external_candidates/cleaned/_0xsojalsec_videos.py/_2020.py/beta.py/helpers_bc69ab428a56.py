# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\beta\helpers.py
import scipy.stats
...
def get_prob_positive_experience_label(include_equals=False, include_decimal=False, include_q_mark=False):
    label = OldTex(
        "P",
        "(",
        "00000",
    ...
    if include_q_mark:
        ...
        q_mark.add_updater(lambda m: m.next_to(m.relative_mob, RIGHT, SMALL_BUFF))
        label.add(q_mark)
...
def get_beta_dist_axes(y_max=20, y_unit=2, label_y=False, **kwargs):
    ...
    result.x_axis.add_numbers(*x_vals, num_decimal_places=1)

    if label_y:
        result.y_axis.add_numbers(*np.arange(y_unit, y_max, y_unit))
...
def scaled_pdf_axes(scale_factor=3.5):
    ...
    axes.y_axis.numbers.set_submobjects([*axes.y_axis.numbers[:5], *axes.y_axis.numbers[4::5]])
    sf = scale_factor
...
def get_random_checks_and_crosses(n=50, s=0.95, width=12):
    return get_checks_and_crosses(bools=(np.random.random(n) < s), width=width)
