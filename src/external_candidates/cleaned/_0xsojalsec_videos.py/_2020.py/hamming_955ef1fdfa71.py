# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\hamming.py
...
def get_bit_grid(n_rows, n_cols, bits=None, buff=MED_SMALL_BUFF, height=4):
    ...
    bit_mobs = VGroup(*[bit_pair.copy() for x in range(n_rows * n_cols)])
...
def get_sound_wave():
    sound = VGroup(
        *[
            Line(DOWN, UP).set_height((0.3 + 0.8 * random.random()) * abs(np.sin(x)))
...
def get_bit_n_sublist(input_list, n, bit_value=1):
    return [elem for i, elem in enumerate(input_list) if bool(i & (1 << n)) ^ bool(1 - bit_value)]
...
def image_reveal_animation(image, bit_height=0.1):
    ...
    return AnimationGroup(
        ...
        LaggedStartMap(VFadeInThenOut, bits, run_time=1, lag_ratio=3 / len(bits)),
    )
...
def focus_scan_anim_lines(scanim, point, final_stroke_width=1):
    ...
    return AnimationGroup(*[MoveToTarget(line) for line in lines])
...
class TripleRedundancy(Scene):
    def construct(self):
        ...
        self.play(FadeOut(image), FadeIn(bits, lag_ratio=1 / len(bits)))
        self.play(
            MoveToTarget(bits),
        ...
        self.play(
            LaggedStart(
                *[TransformFromCopy(bits, bits_copy) for bits_copy in copies],
                lag_ratio=0.5,
            )
        )
        self.play(
        ...
        scan_rect = SurroundingRectangle(VGroup(*[group[0] for group in bit_groups]))
        scan_rect.set_stroke(GREEN, 5)

        self.add(scan_rect)
        for i in range(flipper_index + 1):
            self.play(scan_rect.match_x, bits[i], run_time=0.25)
            self.wait(0.25)
        ...
        self.play(ReplacementTransform(bangs, flipper_rect))
        self.play(
            toggle_bit_anim(flipper),
        ...
        frac_labels = VGroup(*[DecimalNumber(100 * frac, unit="\\%", num_decimal_places=1) for frac in [1 / 3, 2 / 3]])
        ...
        self.play(
            ...
            UP,
            SMALL_BUFF,
            redundancy_word.next_to,
            braces.target[1],
            UP,
            SMALL_BUFF,
            redundancy_word.to_edge,
            RIGHT,
            run_time=3,
        )
        self.wait()
        ...
        self.wait()
        self.play(
            ReplacementTransform(SurroundingRectangle(bits, color=BLUE, stroke_opacity=0), ecc_rect),
            Write(ecc_label),
        )
        self.wait()
        self.play(
...
class TimeLine(Scene):
    def construct(self):
        ...
        timeline = NumberLine((decades[0], decades[-1], 2), big_tick_numbers=decades, width=13)
        ...
        self.play(
            ...
            2,
            {"about_edge": DL},
            hamming_word.set_fill,
            WHITE,
            LaggedStart(*[ApplyMethod(mob.set_opacity, 0.5) for mob in events[1:]]),
        ...
        self.play(
            ...
            hamming_arrow.put_start_and_end_on,
            hamming_word.target.get_bottom(),
            hamming_arrow.get_end(),
            hamming_arrow.set_opacity,
            0.5,
            rs_word.scale,
            1.5,
            {"about_edge": DOWN},
...
class WhatCDsActuallyUse(Scene):
    def construct(self):
        ...
        self.play(GrowFromPoint(words, arrow.get_start()), GrowArrow(arrow))
        self.wait()
...
class DataGettingZapped(Scene):
    ...
    def construct(self):
        ...
        for count in range(20):
            self.play(
                LaggedStart(
                    *[strike_anim(random.choice(bits)) for x in range(int(random.expovariate(0.5)) + 1)],
                    lag_ratio=0.25,
                )
            )
    ...
    def construct(self):
        ...
        for x in range(10):
            ...
            scanim = scan_anim(
                point,
                bits,
                final_stroke_width=0.2,
                run_time=3,
                lag_factor=1,
                show_robot=(x == 0),
            )
            self.play(FadeIn(block, 6 * RIGHT), FadeOut(last_block, 6 * LEFT), run_time=2)
...
class HammingAtBell(Scene):
    def construct(self):
        ...
        self.play(FadeIn(punchcard[0]), Write(punchcard[1:], lag_ratio=0.5, run_time=4))
        self.add(punchcard)
        self.wait()
        ...
        for bit in bits:
            ...
            self.play(MoveToTarget(bit), zap_anim(bit))
        self.wait()
...
class MultiplePerspectives(Scene):
    def construct(self):
        # Background
        background = VGroup(*[Rectangle().set_fill(color, 1) for color in [GREY_E, BLACK, GREY_E]])
        background.set_stroke(width=0)
...
class SetupSixteenBitExample(Scene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(bits_word),
            UpdateFromAlphaFunc(counter, lambda m, a: m.set_fill(opacity=a)),
            UpdateFromFunc(counter, lambda c: c.set_value(len(boxes))),
        ...
        self.play(FadeIn(r_label, 0.2 * RIGHT), LaggedStartMap(FadeIn, r_boxes, run_time=2))
        self.wait()
        ...
        self.play(
            UpdateFromAlphaFunc(
                d_bit_copies,
                lambda m, a: m.shift(0.03 * np.sin(7 * TAU * a) * RIGHT).fade(a),
                remover=True,
            )
        )
        ...
        for n in range(4):
            ...
            self.play(FadeIn(h_group, lag_ratio=0.2), FadeIn(bit))
        ...
        self.add(*r_boxes)
        self.play(
            *[MoveToTarget(mover, lag_ratio=0.1, run_time=3, path_arc=20 * DEGREES) for mover in movers],
        )
        self.wait()

        self.play(*[Restore(mover, lag_ratio=0.1, run_time=3, path_arc=20 * DEGREES) for mover in movers])
        r_boxes.clear_updaters()
        self.wait()

        power_of_2_rects = VGroup(*[SurroundingRectangle(p_labels[2**n]) for n in range(4)])
        ...
        self.play(VFadeIn(randy), randy.change, "confused", q_mark)
        self.play(Blink(randy))
        self.wait(2)
        self.play(Blink(randy))
        self.wait()
    ...
    def old_functions(self):
        ...
        scanim = scan_anim(
            block.get_corner(DR) + 2 * RIGHT,
            block,
            final_stroke_width=0.5,
            lag_factor=1,
        )

        self.play(zap_anim(flipper), toggle_bit_anim(flipper, target_color=RED))
        self.wait()
        self.play(scanim)
        self.play(focus_scan_anim_lines(scanim, flipper.get_center()))
        self.play(toggle_bit_anim(flipper, target_color=WHITE), FadeOut(scanim.mobject))
...
class SenderReceiverDynamic(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.change, "thinking", pi) for pi in pis],
        )
        self.wait()
        self.play(
            *it.chain(*[[pi.change, "horrified", pi.shift, 2 * UP, pi.set_opacity, 0] for pi in pis]),
        ...
        self.play(
            LaggedStart(
                *[TransformFromCopy(b1, b2) for b1, b2 in zip(block, bits_copy)],
                run_time=3,
            ),
        )
        ...
        self.play(ShowCreation(wire[2]), FadeIn(small_words[1], 0.5 * DOWN))
        ...
        self.play(LaggedStartMap(FadeIn, time_words, lambda m: (m, UP), lag_ratio=0.3))
        self.wait(2)
...
class ParityChecks(Scene):
    def construct(self):
        ...
        self.add(block)
        self.play(scanim)
        self.play(GrowFromPoint(detection_words, robot.get_top()))
        self.wait()
        self.play(
            detection_words.shift,
            0.75 * UP,
        ...
        want_even_arrow = Arrow(want_even_label.get_left(), one_counter.get_right())
        ...
        label_pair = VGroup(*[OldTexText("\\#", " 1's:").next_to(b, UP, buff=MED_LARGE_BUFF) for b in blocks])
        one_counter_pair = VGroup(
            *[get_ones_counter(label, rect, buff=MED_SMALL_BUFF) for label, rect in zip(label_pair, one_rects_pair)]
        )

        self.add(label_pair, one_counter_pair)
        self.play(*[ShowIncreasingSubsets(group) for group in one_rects_pair])
        self.wait()
        ...
        parity_labels = VGroup(
            *[
                OldTexText("Parity: ", word, tex_to_color_map={word: color}).next_to(rect, UP)
        ...
        self.play(
            LaggedStart(
                *[ShowCreationThenFadeOut(Underline(label[0], color=YELLOW)) for label in parity_labels],
                lag_ratio=0.3,
            )
        )
        self.wait()
        ...
        new_one_counter_pair = VGroup(
            *[get_ones_counter(label, rect, buff=MED_SMALL_BUFF) for label, rect in zip(label_pair, new_one_rects_pair)]
        )
        for bit in r_flippers:
            toggle_bit(bit)
        ...
        self.play(ShowIncreasingSubsets(one_rects_pair[1]))
        ...
        one_rects_pair[1].set_submobjects(get_one_rects(r_block, buff=0.05).submobjects)
        self.play(ShowIncreasingSubsets(one_rects_pair[1]))
...
class OddNumberCountTo101(Scene):
    def construct(self):
        ...
        for mob in group[:2]:
            ...
            self.remove(mob)
        self.play(
            ShowSubmobjectsOneByOne(group[2:]),
            run_time=6,
            rate_func=bezier([0, 0, 1, 1]),
        )
        self.wait()
...
class ArrayOfValidMessages(Scene):
    def construct(self):
        ...
        self.play(FadeIn(fade_rect), FadeIn(example_groups[0]))
        self.wait()
        self.play(
...
class TwentyQuestions(Scene):
    def construct(self):
        ...
        back_rects = VGroup(
            *[SurroundingRectangle(bit, buff=MED_SMALL_BUFF / 2).scale(0) for bit in get_bit_n_subgroup(block, 0)]
        )
        ...
        for n, vect in zip(it.count(), [UP, UP, RIGHT, RIGHT]):
            ...
            rects = VGroup(*[SurroundingRectangle(bit, buff=MED_SMALL_BUFF / 2) for bit in color_group])
        ...
        # Set up questions
        q_labels = VGroup(*[OldTexText(f"Q{n}:") for n in range(1, 5)])
        ...
        h_groups = VGroup(*[get_bit_n_subgroup(h_rects, n) for n in range(4)])
        ...
        self.play(
            ...
            get_bit_n_subgroup(block, 0, 0).fade,
            0.9,
        ...
        self.wait()
        self.play(get_sub_scanim(0, show_robot=False), FadeOut(bangs))
        ...
        ecc_rects = VGroup(*[h_group[0] for h_group in h_groups])
        pb_label = OldTexText("Parity bit")
        pb_label.next_to(ecc_rects[0], UP, MED_LARGE_BUFF)
        pb_arrow = Arrow(pb_label.get_bottom() + MED_SMALL_BUFF * LEFT, block[1].get_top(), buff=0.1)
        ...
        one_rects.set_submobjects(get_one_rects(get_bit_n_subgroup(block, 0)).submobjects)
        self.wait()
        ...
        self.play(
            LaggedStart(
                *map(
                    FadeOut,
                    [one_rects, counter, pb_arrow, pb_label, pc_words, h_groups[0]],
                )
            ),
            frame.restore,
        )
        ...
        self.play(
            ...
            questions[2:].fade,
            0.9,
            get_bit_n_subgroup(block, 1, 0).fade,
            0.9,
            ecc_rects[0].fade,
            0.5,
        )
        ...
        robot = scanim.mobject[-1]
        self.play(zap_anim(block[6]), toggle_bit_anim(block[6], target_color=RED))
        self.play(scanim)
        ...
        q2_rect.move_to(questions[1][3])
        q1_highlight, q2_highlight = [get_bit_n_subgroup(h_rects, n).copy().set_fill(BLUE, 0.5) for n in [0, 1]]
        ...
        self.play(
            ...
            get_bit_n_subgroup(block, 1, 0)[1::2].fade,
            0.9,
        )
        self.wait(2)
        self.play(
            q1_rect.move_to,
            questions[0][1],
            q1_highlight.move_to,
            boxes,
            LEFT,
            q1_highlight.set_fill,
            GREY_BROWN,
            block.restore,
        ...
        self.play(
            block[::4].fade,
            0.9,
            get_bit_n_subgroup(block, 1).fade,
            0.9,
        )
        self.wait(2)
        self.play(
        ...
        self.play(VFadeIn(morty), morty.change, "shruggie", questions[0], FadeIn(words, DOWN))
        ...
        self.play(
            LaggedStart(*map(FadeOut, [column_highlight, q1_rect, q2_rect])),
        ...
        self.play(
            get_bit_n_subgroup(block, 2, 0).fade,
            0.9,
        ...
        self.play(
            ...
            get_bit_n_subgroup(block, 3, 0).fade,
            0.9,
        ...
        for brace in braces:
            self.play(GrowFromCenter(brace), FadeIn(brace.words, 0.25 * RIGHT))
            self.wait()
        ...
        self.wait()
        self.play(toggle_bit_anim(block[3], target_color=WHITE))
        self.wait()
        ...
        self.play(
            ...
            questions,
            FadeOut(braces),
            *[FadeOut(brace.words) for brace in braces],
        )
        ...
        self.play(
            ...
            LaggedStart(*[toggle_bit_anim(bit) for i, bit in enumerate(block) if i not in [0, 1, 2, 4, 8]]),
        )
        self.play(LaggedStart(*[toggle_bit_anim(bit) for i, bit in enumerate(block) if i not in [0, 1, 2, 4, 8]]))
        self.wait()
        self.play(
            FadeIn(stat_words[2], 0.5 * LEFT),
            LaggedStart(*map(ShowCreationThenFadeAround, [block[2**n] for n in range(4)])),
        )
        self.play(Blink(randy))
        ...
        self.play(
            ...
            LaggedStart(*map(FadeIn, [code_name[i] for i in [0, 2, 4, 5]])),
        )
        self.wait()
        ...
        self.play(FadeOut(old_zero_rect), Restore(zero_rect), Write(zero_words))
        self.wait()
...
class BlockSize256(Scene):
    def construct(self):
        ...
        self.play(frame.move_to, 0.5 * UP, Write(title))
        self.wait()
        ...
        for k, bit, group in zip(it.count(), bits, parity_groups):
            ...
            self.play(FadeIn(word), MoveToTarget(boxes))
            self.wait()
        ...
        self.play(FadeIn(parity_bits_label, DOWN), title.to_edge, RIGHT)
        ...
        self.play(boxes[pos].set_fill, BLACK, 0, FadeOut(yes_no_group))

        # Message bits
        message_bits = VGroup(*[bit for i, bit in enumerate(block) if i not in [2**k for k in range(N)]])
        message_bits.shuffle()

        self.play(
            LaggedStart(
                *[toggle_bit_anim(bit) for bit in message_bits],
                lag_ratio=0.01,
                run_time=3,
            )
        )
...
class ChecksSpellOutPositionInBinary(Scene):
    def construct(self):
        ...
        binary_answers = VGroup(
            *[Integer(bit).move_to(word).match_color(word) for bit, word in zip(bits, yes_no_words)]
        )
        ...
        self.play(MoveToTarget(binary_pos_label), frame.move_to, 0.5 * UP)
        self.play(
            Write(equation[:-1]),
            Write(trans_words),
            GrowArrow(arrow),
        ...
        bit_parts = list(it.chain(*[bin_equation.get_parts_by_tex(f"{{{d}}}") for d in [0, 1]]))
        ...
        self.play(
            FadeIn(
                VGroup(*[part for part in bin_equation if part.get_tex() not in ["{0}", "{1}", "{7}"]]),
                lag_ratio=0.1,
            )
        )
        ...
        for n in ns:
            ...
            self.play(*[Animation(mob, remover=True, run_time=1) for mob in [new_bit_mobs, nc1, nc2]])
        ...
        # Remove 7 stuff
        self.play(
            FadeOut(VGroup(*bin_group, *bin_equation, *binary_answers), lag_ratio=0.1),
            block[pos].set_color,
            WHITE,
        )
        ...
        self.play(
            LaggedStart(
                *[ShowCreationThenFadeOut(SurroundingRectangle(b, color=GREEN)) for b in block],
                lag_ratio=0,
            ),
            randy.look_at,
            boxes.get_bottom(),
        ...
        for n in range(1, N):
            ...
            self.play(Transform(bit_arrows, bit_arrow_groups[n]), FadeOut(questions[n - 1]))
            self.wait()
            self.play(Transform(boxes, highlight_groups[n]), FadeIn(questions[n]))
            self.wait()
        ...
        for n in range(N):
            ...
            self.play(FadeIn(highlight_groups[n]), FadeIn(rect), *fade_anims)
            fade_anims = [
...
class PowerOfTwoPositions(Scene):
    def construct(self):
        ...
        numbers = VGroup(*[Integer(n).move_to(box) for n, box in enumerate(boxes)])
...
class LetsWalkThroughAnExample(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(5)
        self.teacher_says("But of\\\\course!", target_mode="tease")
...
class FullExampleWithNewEnd(Scene):
    ...
    def construct(self):
        ...
        message_boxes = VGroup(*[box for box in boxes[1:] if box not in ecc_boxes])
        ...
        self.play(
            LaggedStart(
                *[ApplyMethod(bit.move_to, box) for bit, box in zip(bits, message_boxes)],
                run_time=4,
                lag_ratio=0.3,
            )
        )
        ...
        self.play(GrowArrow(morty_arrow), morty.change, "pondering", morty_arrow)
        ...
        VGroup(*[block_group.target[2][i] for i in [0, 1, 2, 4, 8]]).set_color(GREY_C)
        self.play(MoveToTarget(block_group))

        message_bits = VGroup(*[bit for i, bit in enumerate(bits) if i not in [0, 1, 2, 4, 8]])
    ...
    def old_parity_checks(self):
        ...
        self.wait()
        self.play(FadeOut(VGroup(counter, words, one_rects), lag_ratio=0.2))

        # Read result
        final_result = results.copy()
        final_result.arrange(LEFT, buff=SMALL_BUFF)

        equation = VGroup(final_result, OldTex("\\rightarrow"), Integer(10))
        equation.arrange(RIGHT)
        equation.to_edge(UP)
        ...
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(*equation, *questions, *results),
                lambda m: (m, DOWN),
            ),
            Restore(boxes),
        )
...
class ByHandVsSoftwareVsHardware(Scene):
    def construct(self):
        ...
        self.play(MoveToTarget(randy, path_arc=-45 * DEGREES))
        self.play(Blink(randy))
...
class ScaleUp(Scene):
    def construct(self):
        ...
        for N in range(4, 13):
            ...
            grid.arrange_in_grid(2 ** int(math.floor(N / 2)), 2 ** int(math.ceil(N / 2)), buff=0)
            grid.set_width(5.5)
            if N > 8:
...
class MillionRatio(Scene):
    def construct(self):
        ...
        k = 17
        positions = VGroup(*[OldTex(int_to_bit_string(n, n_bits=20)) for n in [*range(k), *range(2**N - k // 2, 2**N)]])
...
class BurstErrors(Scene):
    def construct(self):
        ...
        block_words = VGroup(*[OldTexText(f"Block {n}", fill_color=color) for n, color in zip(range(nb), colors)])
        ...
        self.play(ShowCreation(strike), FadeIn(ruined_words, 0.5 * DOWN))
        self.wait()
        self.play(
...
class BinaryCounting(Scene):
    def construct(self):
        ...
        for n in range(16):
            ...
            anims = [ChangeDecimalToValue(counter, n), MoveToTarget(boxes)]
...
class ReviewOfXOR(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, arrows), FadeIn(parity_words))
        self.wait()
        ...
        self.play(FadeIn(mod2_words, DOWN), FadeOut(parity_words, UP), FadeOut(arrows))
        self.wait()

        # xor of two bit strings
        row_len = 8
        bit_strings = VGroup(*[Integer(random.choice([0, 1]), edge_to_fix=ORIGIN) for x in range(row_len * 3)])
        bit_strings.arrange_in_grid(3, row_len, h_buff=SMALL_BUFF, v_buff=MED_LARGE_BUFF, fill_rows_first=False)
        ...
        for row, n in zip(rows, [3, 5, 6]):
            ...
            self.play(GrowArrow(arrow), FadeIn(integer, LEFT))
...
class WhyPointToError(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(rect.become, SurroundingRectangle(bit).match_style(rect), FadeOut(arrow))
        self.wait()
...
class HammingCodesWithXOR(Scene):
    def construct(self):
        ...
        for n, box, bit in zip(it.count(), boxes, bits):
            bin_label = VGroup(*[Integer(int(c)) for c in int_to_bit_string(n, n_bits=4)])
            bin_label.arrange(RIGHT, buff=SMALL_BUFF, aligned_edge=DOWN)
        ...
        self.play(*[LaggedStartMap(MoveToTarget, mob, lag_ratio=0.02) for mob in [boxes, bits, bin_pos_labels]])
        self.wait()
        ...
        self.play(
            ...
            nt_copy.fade,
            1,
            *[toggle_bit_anim(b1, path_arc=0) for b1, b2 in zip(result, new_term) if b2.get_value() == 1],
        )
        self.remove(nt_copy)
        self.wait()
        self.play(
        ...
        arrow = Arrow(
            result.get_left(),
            bin_pos_labels[pos].get_corner(DR),
            path_arc=-30 * DEGREES,
            buff=0.1,
        )
        self.play(DrawBorderThenFill(arrow))
...
class WhatAboutTwoBitDetection(TeacherStudentsScene):
    def construct(self):
        self.student_says("What about\\\\detecting\\\\two bit errors?")
        self.play(
...
class MatrixProduct(Scene):
    def construct(self):
        ...
        encoder, message, result = [
            IntegerMatrix(matrix, **kw) for matrix in [encoder_matrix, message_matrix, result_matrix]
        ]
        equation = VGroup(encoder, message, OldTex("="), result)
...
class TooManyErrorsTripUpHamming(Scene):
    def construct(self):
        title = OldTexText("$>2$ Errors", " $\\Rightarrow$ ", "Invalid decoding")
        title.set_height(0.7)
        ...
        kw = {"lag_ratio": 0.5}
        self.play(
            LaggedStart(*[zap_anim(block[pos]) for pos in errors], **kw),
            LaggedStart(*[toggle_bit_anim(block[pos], target_color=RED) for pos in errors], **kw),
        )
...
class ReedSolomonPreview(Scene):
    def construct(self):
        ...
        cubic = axes.get_graph(lambda x: -0.05 * x * (x - 2) * (x - 4) * (x - 8) + 2)
        ...
        # Data
        dots = VGroup(*[Dot(axes.input_to_graph_point(x, cubic)) for x in range(0, 8)])
        ...
        input_arrows = VGroup(*[Arrow(input_words.get_bottom(), dot.get_center()) for dot in dots[:4]])
        ...
        redundant_arrows = VGroup(*[Arrow(redundant_words.get_corner(UL), dot.get_center()) for dot in dots[4:]])
        ...
        self.play(ShowCreation(cubic), Write(poly_words))
        self.wait()
        self.play(
...
class HammingThinking(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureBubbleIntroduction(
                randy,
                "What's the most efficient\\\\I could conceivably be?",
                bubble_type=ThoughtBubble,
            )
        )
        self.wait()
...
class TwoErrorGrids(Scene):
    def construct(self):
        ...
        vects = [np.array(tup) for tup in it.product(*[[0, 1]] * 4)]
