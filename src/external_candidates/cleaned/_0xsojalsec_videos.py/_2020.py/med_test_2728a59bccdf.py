...
class MathAsDesign(Scene):
    def construct(self):
        ...
        alt_l_formula = VGroup(
            VGroup(OldTex("\\pm").set_height(0.8), OldTex("k \\text{ odd}", font_size=36)).arrange(DOWN, SMALL_BUFF),
            OldTex("\\frac{1}{k}"),
            OldTex("="),
            VGroup(
                Sector(
...
class NewBayesRuleAndMedicalTests(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(ShowCreationThenFadeOut(SurroundingRectangle(mob, color=BLUE)) for mob in (formula, odds_formula)),
                lag_ratio=0.4,
                run_time=3,
            ),
            VFadeIn(randy),
        ...
        self.play(
            LaggedStart(
                *(ShowCreationThenFadeOut(Underline(mob, color=GREY_BROWN)) for mob in formulas),
                lag_ratio=0.3,
            ),
            morty.change,
            "raise_right_hand",
        ...
        self.play(
            LaggedStart(*(FadeOut(mob, RIGHT) for mob in [arrow2, arrow, title, formula, odds_formula, *bubbles])),
            morty.change,
            "raise_right_hand",
            paradox_group.next_to,
            morty.get_corner(UR),
...
class SamplePopulationBreastCancer(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(Indicate, fp_cases, color=GREEN_A, lag_ratio=0.05, run_time=3),
        )
        self.wait()
...
class AskWhatTheParadoxIs(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(*(FadeIn(word, shift=word.get_center() - top_words.get_center()) for word in sens_spec)),
            run_time=1,
        )
        self.wait(4)
...
class GigerenzerSession(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(ShowCreationThenFadeOut(SurroundingRectangle(stat[1], color=stat[0].get_color())) for stat in stats)
            )
        )
...
class SamplePopulation10PercentPrevalence(Scene):
    def construct(self):
        ...
        accuracy_figures = VGroup(
            OldTexText("90\\% Sensitivity,", " 10\\% False negative rate", font_size=36),
        ...
        self.play(
            ...
            healthy_cases,
            FadeOut(VGroup(*(case for case in healthy_cases if case not in false_positives))),
...
class SamplePopulationOneInThousandPrevalence(Scene):
    def construct(self):
        ...
        self.play(
            FadeOut(nc_count),
            FadeIn(fp_label),
            LaggedStart(*(ShowCreation(case.box) for case in fp_cases), lag_ratio=0.001),
...
class BayesFactor(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(ShowCreationThenFadeOut(Underline(word)) for word in [sens_word, fnr_word]),
                lag_ratio=0.4,
            )
        )
        self.play(
        ...
        self.play(
            LaggedStart(
                *(ShowCreationThenFadeOut(Underline(word)) for word in [spec_word, fpr_word]),
                lag_ratio=0.4,
            )
        )
        self.wait()
...
class RuleOfThumb(Scene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(equation.get_parts_by_tex(")")),
            TransformFromCopy(condition.get_part_by_tex("Prior"), equation.get_part_by_tex("Prior")),
            TransformFromCopy(
                bf_label[0],
...
class NewSnazzyBayesRuleSteps(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                ShowCreationThenFadeOut,
                VGroup(*(SurroundingRectangle(person, buff=0.05) for person in population[:10])),
            ),
        )
        self.wait()
...
class WhyTheBayesFactorTrickWorks(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                ShowCreationThenFadeOut,
                VGroup(*(SurroundingRectangle(icon, stroke_width=1, buff=0.05) for icon in population[:10])),
            ),
        )
        self.play(
            ...
            LaggedStartMap(
                ShowCreationThenFadeOut,
                VGroup(
                    *(SurroundingRectangle(icon, color=GREY, stroke_width=1, buff=0.05) for icon in population[10:])
                ),
                lag_ratio=0.01,
            ),
        )
        ...
        self.play(
            *(
                TransformFromCopy(frac.get_part_by_tex(tex), final_frac.get_part_by_tex(tex))
...
class ReframeWhatTestsDo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.change, "pondering", question.target) for pi in self.pi_creatures],
        )
        self.wait(2)
...
class WhatAboutANegativeResult(Scene):
    def construct(self):
        ...
        rects = VGroup(
            *(SurroundingRectangle(part, buff=0.1, stroke_color=TEAL, stroke_width=3) for part in frac_parts)
        )
...
class BayesTheorem(Scene):
    def construct(self):
        ...
        braces = VGroup(
            ...
            *(Brace(rect, u * RIGHT, buff=SMALL_BUFF) for rect, u in zip(rects, [-1, -1, 1, 1])),
        )
...
class ContrastTwoFormulas(Scene):
    def construct(self):
        ...
        odds_formula = OldTex(
            "O(\\text{D} | {+}) =O(\\text{D}){P({+} | \\text{D}) \\over P({+} | \\text{$\\neg$D})}",
            **kw,
        )
        ...
        clipboards = VGroup(
            *(get_covid_clipboard("Disease").set_height(1.5).next_to(rect, DOWN) for rect in new_bf_rects)
        )
...
class AskAboutHowItsSoLow(TeacherStudentsScene):
    def construct(self):
        question = OldTexText(
            "How can it be 1 in 11\\\\if the test is accurate more\\\\than 90\\% of the time?",
            tex_to_color_map={
                "1 in 11": BLUE,
                "90\\%": YELLOW,
            },
...
class TotalPopulationVsSymptomatic(Scene):
    def construct(self):
        ...
        # Bayes factors
        bf_labels = VGroup(
            OldTexText("Bayes factor for symptoms", tex_to_color_map={"for symptoms": BLUE}),
            OldTexText("Bayes factor positive test", tex_to_color_map={"positive test": GREEN}),
        )
