# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\monster.py
...
def get_monster_size_label():
    ...
    size_parts = VGroup(*[size_label[i : i + 12] for i in range(0, len(size_label), 12)])
...
def get_glassy_cube(frame):
    ...
    edges = [(v1, v2) for v1, v2 in it.combinations(verts, 2) if sum(v1 == v2) == 2]
    corner_dots = Group(
        *[
            Sphere(
                resolution=(51, 26),
    ...
    edge_rods = Group(*[Line3D(v1, v2) for v1, v2 in edges])
...
def get_rot_icon(degrees, mobject, mini_mob_height=1.25):
    ...
    arc1 = Arrow(RIGHT, rotate_vector(RIGHT, angle), path_arc=angle, buff=0)
...
def permutation_animation(mobs, perm=None, arc=PI / 2, lag_factor=3, run_time=None):
    ...
    return LaggedStart(
        *[ApplyMethod(m1.move_to, m2, path_arc=arc) for m1, m2 in zip(mobs, targets)],
        **kw,
    )
...
class IntroduceMonsterSize(Scene):
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                size_label,
                lambda m, a: m.set_submobjects(partials[int(a * n_syms)].submobjects),
                run_time=6,
        ...
        self.wait(2)
        self.play(LaggedStart(*[FadeOut(mob, DOWN) for mob in self.mobjects]))
...
class IntroduceGroupTheory(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(title.scale, 2.0 / 2.5, title.to_edge, UP, FadeOut(over_title, UP))

        self.play(FadeIn(title, DOWN))
        self.play(LaggedStartMap(GrowArrow, arrows, run_time=1), Write(sym_amb, run_time=2))
        ...
        face_citation = OldTexText(
            "``Average face'' from the Face Research Lab\\\\DeBruine, Lisa \\& Jones, Benedict (2017)"
        )
        ...
        self.play(FadeIn(rot_icons[0]), Rotate(snowflake, 60 * DEGREES))
        self.wait()
        self.play(FadeIn(rot_icons[1]), Rotate(snowflake, 120 * DEGREES))
        self.wait()
        ...
        self.play(FadeIn(footnote, rate_func=there_and_back_with_pause, run_time=3, remover=True))
        self.wait()
        ...
        self.play(ShowCreation(id_rect))
        self.play(
        ...
        self.play(
            ...
            *[FadeIn(f, -0.5 * f.get_center()) for f in face_group],
        )
...
class ZooOfGroups(ThreeDScene):
    def construct(self):
        ...
        sphere = Sphere()
        sphere = Group(sphere, SurfaceMesh(sphere, resolution=(21, 11)))
        ...
        self.play(LaggedStart(*[FadeIn(group[0], -0.5 * group.get_center()) for group in groups]))
        self.play(LaggedStart(*[FadeInFromLarge(group[1]) for group in groups]))
        self.play(
            LaggedStart(
                Rotate(dot_pair, PI),
        ...
        self.play(
            ...
            3,
            groups[-1].center,
            groups[-1].space_out_submobjects,
            1.5,
            *[FadeOut(mob, mob.get_center() - groups[-1].get_center()) for mob in groups[:-1]],
        )
...
class SymmetriesOfACube(ThreeDScene):
    def construct(self):
        ...
        self.wait()
        self.play(Rotate(reflection_plane, PI / 2, axis=UP))
        self.play(
        ...
        def explostion_transform(self=self, cube=cube):
            ...
            self.play(
                LaggedStart(
                    *[Transform(face, cube[perm[i]]) for i, face in enumerate(cube)],
                    lag_ratio=0.1,
                )
            )
...
class WeirdCubeSymmetryUnderbrace(Scene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(tex, 0.25 * UP))
        self.wait()
...
class PermutationGroups(Scene):
    def construct(self):
        ...
        permutations = Succession(*[get_permutation() for x in range(20)])
        ...
        self.play(
            ChangeDecimalToValue(perm_label[2], new_perm_label[2].get_value(), run_time=3),
            UpdateFromAlphaFunc(
                perm_label[2],
                lambda m, a: m.move_to(interpolate(old_full_count_center, new_perm_label[2].get_center(), a)).set_color(
                    interpolate_color(BLUE, YELLOW, a)
                ),
            ),
            ShowIncreasingSubsets(new_dots),
            *[ReplacementTransform(perm_label[i], new_perm_label[i]) for i in [0, 1, 3]],
        )
        ...
        for x in range(5):
            perm = list(range(12))
            random.shuffle(perm)
            self.play(LaggedStart(*[Transform(dots[i], dots[perm[i]], path_arc=PI / 2) for i in range(12)]))
            self.wait()
        ...
        self.play(
            perm_label[0].replace,
            new_perm_label[0],
            {"stretch": True},
            perm_label[0].set_opacity,
            0,
...
class SolutionsToPolynomials(Scene):
    def construct(self):
        ...
        quintic = OldTex(
            ...
            "(x - r_3)",
            "(x - r_4)",
            tex_to_color_map={f"r_{i}": colors[i] for i in range(5)},
        )
        ...
        self.play(
            LaggedStart(
                *[ReplacementTransform(rs.copy(), rd) for rs, rd in zip(root_syms, root_dots)],
                run_time=3,
                lag_ratio=0.3,
            )
        )
        ...
        for x in range(5):
            ...
            random.shuffle(perm)
            self.play(
                *[permutation_animation(mob, perm, arc=30 * DEGREES, lag_factor=0.5) for mob in [root_syms, root_dots]]
            )
            self.wait(0.5)
        self.play(
        ...
        quad_root_dots = get_root_dots([(1 + u * np.sqrt(5)) / 2 for u in [-1, 1]])
        ...
        self.play(Write(quadratic_formula), FadeIn(quad_form_name, DOWN))
        self.wait()
        ...
        self.wait()
        self.play(mathy.change, "pondering", root_syms[3])
        self.play(Blink(mathy))
        self.wait()
        ...
        # Arithmetic symbols
        symbols = VGroup(*[OldTex(s) for s in ["+", "-", "\\times", "/", "\\sqrt[n]{\\qquad}"]])
...
class MentionGroupsInPhysics(TeacherStudentsScene):
    def construct(self):
        ...
        self.look_at(nt_label.target, added_anims=[MoveToTarget(nt_label)])
        self.play(
            self.teacher.change,
            "happy",
            rule,
            *[FadeIn(part, rule.get_center() - part.get_center()) for part in rule],
        )
        self.wait(2)
...
class ElementsAsAbstractions(TeacherStudentsScene):
    def construct(self):
        ...
        self.look_at(r_sym, added_anims=[Write(r_sym)])
        ...
        self.remove(numeral)
        self.play(
            LaggedStart(*[TransformFromCopy(numeral[0], form[0]) for form in ops]),
            *[ApplyMethod(pi.look_at, ops) for pi in self.pi_creatures],
        )
        self.play(LaggedStart(*[Write(form[1:]) for form in ops]))
        self.wait(3)
        ...
        self.wait(4)
        self.play(
            LaggedStart(*[ApplyMethod(mob.scale, 0, remover=True) for mob in [*sum_dots, *mult_dots]]),
            LaggedStart(*[FadeIn(mob) for mob in [*ops[0][0::2], *ops[1][:3], ops[1][4]]]),
        ...
        self.play(TransformFromCopy(rot_icon[0], snowflake))

        def get_numbers(flake):
            vect = 1.2 * (flake.get_top() - flake.get_center())
            points = VGroup(*[VectorizedPoint(rotate_vector(vect, angle)) for angle in np.arange(0, TAU, TAU / 6)])
        ...
        self.look_at(alt_flake, added_anims=[TransformFromCopy(rhs_icon[0], alt_flake)])
        ...
        self.play(Rotate(alt_flake, PI, axis=line.get_vector()))
        self.play(FadeOut(line), self.change_students(*3 * ["thinking"]))
        self.wait(3)
...
class MultiplicationTable(Scene):
    def construct(self):
        ...
        icons = VGroup(
            *[get_rot_icon(deg, square) for deg in [0, 90, 180, -90]]
            + [get_flip_icon(angle, square, opacity=1) for angle in np.arange(0, PI, PI / 4)]
        )
        ...
        for icon_group, grid_group, vect in [
            (left_icons, grid[0::8], LEFT),
            (top_icons, grid[:8], UP),
        ]:
        ...
        for symbol_group, icon_group in [
            (left_symbols, left_icons),
            (top_symbols, top_icons),
        ]:
        ...
        self.play(
            LaggedStart(*[ApplyMethod(mob.scale, 0, remover=True) for mob in [*left_icons, *top_icons, *table_icons]]),
            LaggedStart(*[GrowFromCenter(mob) for mob in [*left_symbols, *top_symbols, *table_symbols]]),
        )
        self.wait()
...
class UsualMultiplicationTable(Scene):
    def construct(self):
        # Setup grid
        grid = VGroup(*[VGroup(*[Square() for x in range(4)]).arrange(RIGHT, buff=0) for y in range(4)]).arrange(
            DOWN, buff=0
        )
        grid.set_height(6)
        grid.to_edge(DOWN, buff=0.5)
        ...
        diagrams = VGroup(
            *[
                ...
                for mob1, mob2 in [
                    ...
                    (
                        OldTex("9").scale(1.5),
                        VGroup(*[Dot() for x in range(9)]).arrange_in_grid(buff=SMALL_BUFF),
                    ),
                ]
            ]
        )
...
class CubeRotations(ThreeDScene):
    ...
    def get_quaternions(self, n_rotations=30):
        ijk = [quaternion_from_angle_axis(90 * DEGREES, axis) for axis in [RIGHT, UP, OUT]]
        result = []
...
class QuadrupletShufflings(CubeRotations):
    ...
    def quaternion_to_perm(self, quat):
        ...
        rot_vects = [rotate_vector(v + OUT, angle, axis) for v in base_vects]
        perm = []
...
class EightShufflingsOfOrderThree(Scene):
    def construct(self):
        ...
        self.wait()
        for x in range(3):
            self.play(*[permutation_animation(dots, dots.perm, lag_factor=0) for dots in all_dots])
        self.wait()
...
class Isomorphism(Scene):
    def construct(self):
        ...
        icon_labels = VGroup(*[OldTexText(f"$180^\\circ$ about\\\\{axis} axis") for axis in "xyz"])
        ...
        perm_anims = [permutation_animation(perm_term[0], perm_term.perm, lag_factor=0.1) for perm_term in perm_terms]
...
class AskAboutCubeDiagonals(QuadrupletShufflings):
    def construct(self):
        ...
        diagonals = Group(
            *[Line3D(vect + OUT, -vect - OUT, color=color) for color, vect in zip(colors, [UL, UR, DR, DL])]
        )
        diagonals.match_height(cube.edge_rods)

        diag_markers = Group(*[Line3D(ORIGIN, UP, color=color) for color in colors])
        ...
        for diag in diagonals:
            globals()["diag"] = diag
            Group(
                *[cds[np.argmin([get_norm(cd.get_center() - diag.get_points()[i]) for cd in cds])] for i in [0, -1]]
            ).match_color(diag)
...
class S4WithMultipleChildren(Scene):
    def construct(self):
        ...
        lines = VGroup(*[Line(rect.get_top(), s4_label.get_bottom(), buff=0.2) for rect in s_rects])
...
class GroupsBeyondActions(Scene):
    def construct(self):
        ...
        VGroup(groups, line, sym_acts).arrange(DOWN, buff=MED_LARGE_BUFF)
    ...
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[
                ...
                self.change_students("erm", "pondering", "pondering", look_at=question.target),
            ],
        )
        self.wait(2)
        self.play(
...
class ThisQuestionIsHard(Scene):
    def construct(self):
        ...
        self.play(DrawBorderThenFill(tri), FadeIn(question, DOWN))
        question.add(tri)
...
class IntroduceSimpleGroups(Scene):
    def construct(self):
        ...
        child_lines = VGroup(*[Line(groups.get_bottom(), child.get_top(), buff=0) for child in children])
        child_lines.set_stroke(WHITE, 2)
        s_rects = VGroup(*[ScreenRectangle(height=3).move_to(child) for child in children])
        ...
        self.add(groups)
        self.wait()
        self.play(ShowCreation(child_lines[0]), FadeIn(inf_groups, 2 * UR), FadeIn(s_rects[0]))
        self.wait(2)
        self.play(
        ...
        sub_titles = VGroup(*[OldTexText("break down into\\\\", word) for word in ("primes", "atoms", "simple groups")])
        ...
        H_atom = Group(H_sphere, OldTexText("H").scale(0.5).move_to(H_sphere))
        O_sphere = Sphere()
        O_sphere.set_height(1)
        O_sphere.set_color(BLUE)
        O_atom = Group(O_sphere, OldTexText("O").scale(0.75).move_to(O_sphere))
        H2O = Group(
            O_atom.copy(),
        ...
        trees = Group(
            ...
            Group(H2O, Group(H_atom.copy(), H_atom.copy(), O_atom.copy())),
            VGroup(
                OldTex("S_4"),
                VGroup(*[OldTex("C_" + str(n), fill_color=(RED_B if n == 2 else BLUE)) for n in [2, 2, 2, 3]]),
            ),
        )
        ...
        self.play(MoveToTarget(fin_groups), LaggedStartMap(Restore, bg_rects, lag_ratio=0.3))
        ...
        for title, sub_title, tree in zip(titles, sub_titles, trees):
            ...
            self.play(
                ShowCreation(lines),
                LaggedStart(*[GrowFromPoint(child, root) for child in children]),
                FadeIn(sub_title),
            )
            self.wait()
...
class SymmetriesOfCirleAndLine(Scene):
    def construct(self):
        ...
        circle_ticks = VGroup(*[Line(0.95 * vect, 1.05 * vect) for vect in compass_directions(12)])
...
class QuinticImpliesCyclicDecomposition(Scene):
    def construct(self):
        ...
        for children, buff in (
            (prime_children, MED_LARGE_BUFF),
            (
                real_children,
                LARGE_BUFF,
            ),
            ...
            for child in children:
                children.lines.add(Line(s5.get_right(), child.get_left(), buff=0.1))
        prime_children[2].shift(SMALL_BUFF * RIGHT)

        VGroup(
            s5,
            prime_children.lines,
            prime_children,
            real_children.lines,
            real_children,
...
class TwoStepsToAllFiniteGroups(Scene):
    def construct(self):
        ...
        # Periodic table
        table = VGroup(
            *[VGroup(*[Square() for x in range(n)]).arrange(DOWN, buff=0) for n in [7, 6, *[4] * 10, *[6] * 5, 7]]
        )
        ...
        chem_arrow = Arrow(steps[1].get_right(), chem_words.get_left(), buff=0.5)
        self.play(
...
class ClassificationOfSimpleGroups(Scene):
    def construct(self):
        ...
        families_grid = VGroup(*[VGroup(*[square_template.copy() for x in range(8)]) for y in range(18)])
        ...
        # C5
        c_names = VGroup(*[OldTex(f"C_{{{p}}}") for p in [2, 3, 5, 7, 11, 13, 17, 19]])
        ...
        for n in [1, -2, 2]:
            ...
            self.wait(0.5)
        self.play(Restore(c_names), FadeOut(pentagon))
        ...
        self.play(Restore(families_grid), MoveToTarget(c_names, remover=True))
        ...
        a_names = VGroup(*[OldTex(f"A_{{{n}}}") for n in range(5, 5 + len(column) - 1)])
        ...
        self.play(
            sporadics_title.scale,
            1.25,
            sporadics_title.center,
            sporadics_title.to_edge,
            UP,
        ...
        self.play(sporadics[0].set_fill, {"opacity": 0}, FadeIn(monster))
        self.play(
            Write(monster_name),
        ...
        self.play(
            ...
            ApplyMethod(
                baby_arrow.scale,
                0,
                {"about_point": baby_arrow.get_start()},
                remover=True,
            ),
            FadeOut(baby_size_label, UP),
            FadeOut(baby_name, UP),
            full_monster_label.set_fill,
            WHITE,
            1,
        )
...
class AskWhatTheMonsterActsOn(Scene):
    def construct(self):
        ...
        # Dimension counts
        dim_words = VGroup(
            *[OldTexText("Something in\\\\", "{:,}".format(n), " dimensions", "?") for n in [2, 3, 4, 5, 196883]]
        )
...
class MonstrousMoonshine(Scene):
    def construct(self):
        ...
        self.play(
            theories_group.scale,
            0.5,
            {"about_edge": UL},
            theories_group.to_edge,
            UP,
            Write(j_func),
        )
        self.wait()
