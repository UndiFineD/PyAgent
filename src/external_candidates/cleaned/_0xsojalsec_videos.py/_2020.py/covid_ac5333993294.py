...
class ViralSpreadModel(Scene):
    ...
    def spread_infection(self, randys):
        ...
        for randy in randys:
            if randy.infected:
                distances = [get_norm(r2.get_center() - randy.get_center()) for r2 in randys]
...
class RescaleToLogarithmic(IntroducePlot):
    def construct(self):
        ...
        for i in range(2, 5):
            self.play(brace.next_to, log_y_axis.tick_marks[i : i + 2], {"buff": SMALL_BUFF})
            self.wait(0.5)
...
class ShowLogisticCurve(Scene):
    def construct(self):
        ...
        dot.add_updater(lambda m: m.move_to(axes.input_to_graph_point(x_tracker.get_value(), curve)))
