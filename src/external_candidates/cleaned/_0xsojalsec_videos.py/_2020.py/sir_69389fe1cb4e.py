...
class SIRSimulation(VGroup):
    ...
    def add_people(self):
        ...
        for box in self.boxes:
            ...
            for x in range(self.city_population):
                person = self.person_type(dl_bound=dl_bound, ur_bound=ur_bound, **self.person_config)
                person.move_to([interpolate(lower, upper, random.random()) for lower, upper in zip(dl_bound, ur_bound)])
                person.box = box
    ...
    def update_statusses(self, dt):
        for box in self.boxes:
            s_group, i_group = [list(filter(lambda m: m.status == status, box.people)) for status in ["S", "I"]]
        ...
        if self.limit_social_distancing_to_infectious:
            repelled_centers = np.array([person.get_center() for person in self.people if person.symptomatic])
        else:
    ...
    def get_status_counts(self):
        return np.array([len(list(filter(lambda m: m.status == status, self.people))) for status in "SIR"])
...
class GraphBraces(VGroup):
    ...
    def __init__(self, graph, simulation, **kwargs):
        ...
        self.lines = VGroup(*[Line(axes.c2p(1, y1), axes.c2p(1, y2)) for y1, y2 in zip(ys, ys[1:])])
...
class RunSimpleSimulation(Scene):
    ...
    def add_total_cases_label(self):
        ...
        label.add_updater(lambda m: m[1].set_value(self.simulation.get_status_counts()[1]))
    ...
    def add_graph(self):
        ...
        if self.include_graph_braces:
            self.graph_braces = GraphBraces(graph, self.simulation, update_frequency=graph.update_frequency)
...
class IntroduceSocialDistancing(Scene):
    def construct(self):
        ...
        self.play(
            FadeIn(slider),
            *[ApplyMethod(pi1.body.look_at, pi2.body.eyes) for pi1, pi2 in zip(pis, [*pis[1:], pis[0]])],
        )
        self.add(*pis)
        self.wait()
...
class ThatsNotHowIBehave(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.teacher.change, "guilty", self.change_students("erm", "erm", "sassy"))
...
class VirusSpreading(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, viruses, run_time=8, lag_ratio=3 / len(viruses)))
