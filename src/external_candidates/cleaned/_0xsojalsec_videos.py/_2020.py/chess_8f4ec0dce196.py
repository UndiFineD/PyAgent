...
class TwoSquareCase(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(FadeOut, to_remove, lambda m: (m, DOWN), run_time=2, lag_ratio=0.01),
            small_group.center,
        ...
        states = VGroup(
            *[OldTexText(letters, tex_to_color_map={"H": GOLD, "T": RED_D}) for letters in ["TT", "HT", "TH", "HH"]]
        )
        ...
        bin_states = VGroup(
            *[OldTexText(letters, tex_to_color_map={"1": GOLD, "0": RED_D}) for letters in ["00", "10", "01", "11"]]
        )
...
class CubeEdgeDescription(Scene):
    ...
    def construct(self):
        bits = VGroup(
            *[
                VGroup(*[Integer(int(b)) for b in string_to_bools(char)]).arrange(RIGHT, buff=SMALL_BUFF)
                for char in "hi"
            ]
        )
        ...
        for ndots in [1, 3, 6, 7, 9, 12]:
            row = VGroup(
                *[
                    ...
                    VGroup(*[OldTex("\\cdot") for x in range(ndots)]).arrange(RIGHT, buff=0.2),
                    OldTex("\\uparrow\\uparrow"),
                    OldTex("3"),
                ]
            )
...
class FourDCubeColoringFromTrees(ThreeDScene):
    ...
    def get_hypercube_points(self, dim=4, width=4):
        ...
        vertex_holder.set_points(
            [sum([c * v for c, v in zip(reversed(coords), [RIGHT, UP, OUT])]) for coords in all_coords]
        )
...
class IntroduceHypercube(FourDCubeColoringFromTrees):
    def construct(self):
        ...
        self.play(
            ...
            ApplyMethod(frame.set_euler_angles, -20 * DEGREES, 75 * DEGREES, run_time=3),
        )
...
class ExampleSquareAsBinaryNumber(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*[AnimationGroup(MoveToTarget(num), Restore(bin_num)) for num, bin_num in zip(nums, bin_nums)]),
            lag_ratio=0.1,
        )
        self.remove(nums)
        nums = original_nums
