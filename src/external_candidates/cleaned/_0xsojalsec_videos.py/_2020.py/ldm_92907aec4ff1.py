# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\ldm.py
import math
...
class StreamIntro(Scene):
    def construct(self):
        # Add logo
        logo = Logo()
        spikes = VGroup(*[spike for layer in logo.spike_layers for spike in layer])
...
class QuadraticFormula(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(5)
        self.play_student_changes("pondering", "thinking", "erm", look_at=formula)
        self.wait(12)
...
class SineWave(Scene):
    def construct(self):
        ...
        def get_h_line(axes=w_axes, tracker=theta_tracker):
            theta = tracker.get_value()
            return Line(axes.c2p(0, 0), axes.c2p(theta, 0), stroke_color=RED)
        ...
        sin_label = OldTex("\\sin\\left(\\theta\\right)", tex_to_color_map={"\\theta": RED})
    ...
    def get_wave_axes(self):
        ...
        wave_axes.y_axis.add_numbers(-1, 1, num_decimal_places=1)
...
class CosWave(SineWave):
    ...
    def construct(self):
        ...
        def get_square_graph(axes=w_axes, tracker=theta_tracker):
            theta = tracker.get_value()
            graph = axes.get_graph(lambda x: np.cos(x) ** 2, x_min=0, x_max=theta, step_size=0.025)
        ...
        cos_label = OldTex("\\cos\\left(\\theta\\right)", tex_to_color_map={"\\theta": RED})
...
class ExpMeaning(Scene):
    ...
    def construct(self):
        ...
        # Label
        theta_tracker = ValueTracker(0)
        theta_label = VGroup(OldTex("\\theta = "), DecimalNumber(0, num_decimal_places=4))
        theta_decimal = theta_label[1]
        theta_decimal.add_updater(lambda m, tt=theta_tracker: m.set_value(tt.get_value()))
        theta_label.arrange(RIGHT, buff=SMALL_BUFF)
        theta_label.set_color(YELLOW)
        theta_label.add_to_back(
            BackgroundRectangle(
                theta_label,
                buff=MED_SMALL_BUFF,
                fill_opacity=1,
...
class ImaginaryInterestRates(Scene):
    def construct(self):
        ...
        line = Line()
        line.add_updater(
            lambda m, v=vects: m.put_start_and_end_on(
                ORIGIN,
                v[-1].get_start() if len(v) > 0 else RIGHT,
            )
        )
        ...
        def get_arc(tracker=theta_tracker):
            theta = tracker.get_value()
            arc = Arc(radius=1, stroke_width=3, stroke_color=RED, start_angle=0, angle=theta)
            return arc
        ...
        label = VGroup(DecimalNumber(0, num_decimal_places=3), OldTexText("Years"))
        ...
        for n in range(1, 5):
            target = n * PI / 2
            self.play(theta_tracker.set_value, target, run_time=3)
            self.wait(2)
...
class ComplexExponentialPlay(Scene):
    ...
    def construct(self):
        ...
        special_dots = VGroup(*[list(filter(lambda d: abs(d.z - x) < 0.01, sample_dots))[0] for x in special_values])
        ...
        # Titles
        title = OldTex("f(x) =", "\\text{exp}(r\\cdot x)", tex_to_color_map={"r": YELLOW})
        title.to_corner(UL)
        ...
        r_eq = VGroup(OldTex("r=", tex_to_color_map={"r": YELLOW}), DecimalNumber(1))
