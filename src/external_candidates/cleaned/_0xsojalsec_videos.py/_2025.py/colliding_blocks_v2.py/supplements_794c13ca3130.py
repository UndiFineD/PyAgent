# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2025\colliding_blocks_v2\supplements.py
...
class ShowPastVideos(InteractiveScene):
    def construct(self):
        ...
        self.play(
            Write(title),
            VShowPassingFlash(
                screen.copy().set_fill(opacity=0).set_stroke(BLUE, 4).insert_n_curves(20),
                time_width=1.5,
                run_time=3,
            ),
        )
...
class GroversAlgorithmLabel(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(labels[0].animate.shift(UP), FadeIn(labels[1], 0.5 * UP))
        self.wait()
...
class LoadSolutionIntoHead(InteractiveScene):
    def construct(self):
        ...
        self.play(ShowCreation(bubble), randy.change("concentrating", 3 * UR))
        for _ in range(2):
            self.play(Blink(randy))
            self.wait(2)
...
class ShowMassRatioToCountChart(InteractiveScene):
    def construct(self):
        ...
        mass_ratios = VGroup(Integer(int(10**n), unit=":1", font_size=42) for n in range(0, 2 * n_terms, 2))
        counts = VGroup(Integer(int(PI * 10**n), font_size=42) for n in range(0, n_terms))
...
class StateSpaceLabel(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(title), GrowArrow(arrow))
        self.wait()
...
class ReferenceSmallAngleApproximations(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            self.change_students("erm", "sassy", "confused"),
        )
        self.look_at(3 * UL)
...
class ExplainSmallAngleApprox(InteractiveScene):
    def construct(self):
        ...
        x_line.add_updater(lambda m: m.put_start_and_end_on(axes.get_origin(), axes.c2p(math.cos(get_theta()), 0)))
        y_line.add_updater(
...
class DigitsOfPi(InteractiveScene):
    def construct(self):
        ...
        big_nines = Text("\n".join(["9" * digits_per_line for line in pi_text_lines]))
...
class WriteExactSolution(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(title, lag_ratio=0.1), ShowCreation(underline))
        ...
        solution = VGroup(
            ...
            Tex(
                R"\text{Where } \; \theta = \text{arctan}\left(\sqrt{m_2 / m_1}\right)",
                **kw,
            ),
        )
...
class ExactSolutionForMatt(InteractiveScene):
    def construct(self):
        # Answer
        kw = dict(t2c={R"\theta": YELLOW, R"m_1": BLUE, R"m_2": BLUE, R"\pi": WHITE})
        equations = VGroup(
            Tex(
                R"\#\text{Collisions} = {\pi \over \text{arctan}\left(\sqrt{m_2 / m_1}\right)}",
                **kw,
            ),
        ...
        self.play(FadeOut(pi), FadeIn(randy))
...
class WhoCares(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(LaggedStart(stds[1].animate.look_at(stds[0].eyes), stds[0].change("maybe")))
...
class HiddenConnections(InteractiveScene):
    def construct(self):
        ...
        q_marks = VGroup(Tex(R"???", font_size=72).move_to(screen) for screen in screens)
...
class GrowingWhiteDot(InteractiveScene):
    def construct(self):
        ...
        dot.set_color(WHITE)
        self.add(dot)
        self.play(dot.animate.set_radius(FRAME_WIDTH).set_glow_factor(0.5), run_time=35)
