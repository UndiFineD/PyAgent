...
class Blocks(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            FadeOut(ke_rect),
            FadeOut(question),
            VGroup(ke_equation, xy_equation).animate.arrange(DOWN, buff=0.5).next_to(plane, LEFT, buff=LARGE_BUFF),
        )
        underlines = VGroup(
        ...
        marked_velocity.add_updater(
            lambda m: m.set_value(np.array(plane.p2c(state_point.get_center())) / state_tracker.sqrt_mass_vect)
        )
        arc = Arc(
            190 * DEG,
            50 * DEG,
        ...
        VGroup(endzone_line_label, slope_label).rotate(state_tracker.theta, about_point=ORIGIN).shift(
            endzone_line.pfp(0.45)
        )
...
class CirclePuzzle(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (line.animate.set_stroke(PINK, 5).set_anim_args(rate_func=there_and_back) for line in lines),
                lag_ratio=0.5,
                run_time=6,
            ),
        ...
        slope_rect.add_updater(
            lambda m: m.set_width(slope_label.get_width() + 2 * SMALL_BUFF, stretch=True, about_edge=LEFT)
        )
