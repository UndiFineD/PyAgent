...
class Quiz(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (cover.animate.stretch(0, 0, about_edge=RIGHT).set_opacity(0) for cover in covers),
                lag_ratio=0.25,
            ),
        )
        self.wait(16)
...
class NeedleInAHaystackProblem(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                LaggedStart(
                    (FadeOutToPoint(glow.copy(), machine.get_left() + 0.5 * RIGHT) for glow in superposition.glows),
                    lag_ratio=0.1,
                ),
                LaggedStart(
                    (
...
class SuperpositionAsParallelization(InteractiveScene):
    def construct(self):
        ...
        mini_classical = VGroup(
            classical.copy().set_height(0.25).move_to(point).match_x(quantum) for point in superposition.center_points
        )
        lines = VGroup(
            VGroup(
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(piece, output) for piece, output in zip(superposition.pieces, outputs)),
                lag_ratio=0.01,
                run_time=3,
            )
        )
