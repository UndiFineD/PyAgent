...
class WriteQuantumComputingTitle(InteractiveScene):
    def construct(self):
        ...
        self.play(LaggedStart(Flash(letter, color=TEAL, flash_radius=1.0, line_stroke_width=2) for letter in title))
...
class StudentsCommentOnQuiz(TeacherStudentsScene):
    def construct(self):
        ...
        q_marks = VGroup(Text("?!?", font_size=72).next_to(std, UP) for std in self.students)
...
class DotsAndArrow(InteractiveScene):
    def construct(self):
        ...
        self.play(LaggedStart(FadeIn(dots, lag_ratio=0.5), GrowArrow(arrow), lag_ratio=0.7))
        self.wait()
...
class QuestionsOnTheStateVector(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.add(Point())
        self.play(stds[1].says("Why square things?", mode="raise_right_hand", look_at=morty.eyes))
        self.wait(3)
...
class ShowQubitThroughTwoHadamardGates(InteractiveScene):
    def construct(self):
        ...
        mid_sym_lines = VGroup(
            Line(line.pfp(0.5 + 0.01 * u), mid_sym_rect.get_corner(DOWN + u * RIGHT)) for u in [-1, 1]
        )
...
class AskAreYouSure(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeOutToPoint(number, machine.get_center(), path_arc=-45 * DEG, lag_ratio=0.01))
        self.play(
...
class WrapUpList(InteractiveScene):
    def construct(self):
        ...
        items = BulletedList("The Lie", R"Why $\sqrt{\quad}$", "A suprising analogy", buff=0.75)
...
class PythagoreanIntuition(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            *(Write(new_sqrts[0][tex], time_span=(1, 2)) for tex in [R"\sqrt", "+", "2"]),
            run_time=2,
        )
        self.wait()
        self.play(
        ...
        basis_vectors = VGroup(
            Vector(2 * v, thickness=4, fill_color=color) for v, color in zip(np.identity(3), [BLUE_E, BLUE_D, BLUE_C])
        )
        ...
        obs_labels = VGroup(
            KetGroup(Text(f"Obs {n}", font_size=30), height_scale_factor=1.5, buff=0.05) for n in range(1, 4)
        )
        ...
        diag_vects = VGroup(
            Vector(2 * normalize(np.array(tup))) for tup in it.product(*3 * [[-1, 0, 1]]) if get_norm(tup) > 0
        )
...
class ConstructQRCode2(InteractiveScene):
    def construct(self):
        ...
        union_pieces = VGroup(VMobject().set_points(path) for path in union.get_subpaths())
