...
class ContrstClassicalAndQuantum(InteractiveScene):
    def construct(self):
        ...
        self.play(
            *(
                LaggedStart(
                    (bit.animate.set_stroke(YELLOW, 3).set_anim_args(rate_func=there_and_back) for bit in group[1]),
                    lag_ratio=0.25,
                    run_time=4,
                )
...
class DisectAQuantumComputer(InteractiveScene):
    def construct(self):
        ...
        wire_glows = Group(GlowDot(wire.get_start(), color=TEAL, radius=0.25) for wire in wires)
        ...
        for glow in gate_glows:
            glow.add_updater(lambda m: m.set_stroke(width=4 * np.exp(-((0.5 * (m.get_x() - wire_glows.get_x())) ** 2))))
        ...
        for n in range(n_repetitions):
            ...
            self.play(wire_glows.animate.match_x(mes_gates), rate_func=linear, run_time=1)
        ...
        all_qubit_rects = VGroup(
            SurroundingRectangle(qubits, buff=0.05).set_stroke(YELLOW, 1, 0.5) for qubits in all_qubits
        )
        ...
        all_dist_rects = VGroup(self.get_dist_bars(dist, all_qubits, width_ratio=6) for dist in dists)
        ...
        self.play(FadeOut(machine_rect, DOWN), FadeOut(circuit, DOWN), MoveToTarget(qc_label))
        self.play(
        ...
        if False:
            ...
            for _ in range(2):
                self.play(
                    glass.animate.shift(qc_label.get_center() - glass[0].get_center()).set_anim_args(
                        path_arc=-45 * DEG
                    ),
        ...
        for k in [5, 6, "k"]:
            ...
            multi_bitstrings = VGroup(BitString(n, length=n_bits) for n in range(2**n_bits))
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                VGroup(brace, brace_label, longer_example, down_brace, down_brace_label),
            ),
            FadeOut(last_qubits),
            FadeOut(last_dist_rects),
            FadeIn(all_qubits),
        ...
        sample_rects = VGroup(SurroundingRectangle(qb, buff=0.05) for qb, bar in zip(all_qubits, dist_rects))
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(pre_bits, bits, path_arc=30 * DEG) for pre_bits, bits in zip(pre_indices, indices)),
                lag_ratio=0.25,
                run_time=6,
            ),
        )
        ...
        highlight_rect = SurroundingRectangle(VGroup(indices[point5_index], bar_values[point5_index]))
        ...
        self.play(
            LaggedStart(
                (FadeTransform(vector_entries[n].copy(), sum_expr[Rf"x_{{{n}}}^2"]) for n in [0, 1, 2, 3, 14, 15]),
                lag_ratio=0.05,
                run_time=2,
            ),
            FadeTransformPieces(
        ...
        for n in range(3):
            self.play(
                LaggedStart(
                    *(Transform(sign, random.choice(choice), path_arc=PI) for sign, choice in zip(signs, sign_choices)),
                    lag_ratio=0.15,
                    run_time=2,
                )
            )
        ...
        for n, state in enumerate(new_states):
            ...
            if n == 1:
                ...
                qubit_rects = VGroup(
                    SurroundingRectangle(VGroup(*pair), buff=0.05) for pair in zip(all_qubits, dist_rects)
                )
    ...
    def get_dist_bars(
        ...
        rects = VGroup(Rectangle(width_ratio * p, height).next_to(obj) for p, obj in zip(normalized_dist, objs))
...
class Qubit(DisectAQuantumComputer):
    ...
    def get_qubit_labels(self, plane):
        ...
        zero_label, one_label = qubit_labels = VGroup(VGroup(Ket(bit), bit) for bit in bits)
...
class ExponentiallyGrowingState(InteractiveScene):
    ...
    def get_state_vector(self, n):
        ...
        array = DecimalMatrix(values.reshape((2**n, 1)), decimal_config=dict(include_sign=True))
...
class ThreeDSample(InteractiveScene):
    def construct(self):
        ...
        coord_array = DecimalMatrix(np.zeros((3, 1)), decimal_config=dict(include_sign=True))
        ...
        for symbol, direction, trg_coords in zip(symbols, directions, np.identity(3)):
            ...
            self.play(self.set_vect_anim(vector, trg_coords), FadeIn(symbol, 0.25 * OUT))
        ...
        coord_rects = VGroup(SurroundingRectangle(elem) for elem in coord_array.elements)
        ...
        self.wait()
        self.play(FadeTransformPieces(balance_name, balance_ket[1]), Write(balance_ket[0]))
        ...
        self.play(frame.animate.reorient(67, 77, 0, (-0.57, 0.12, 0.86), 6.62), run_time=2)
        self.wait(4)
        self.play(
...
class GroversAlgorithm(InteractiveScene):
    def construct(self):
        ...
        dec_elements = VGroup(elem for n, elem in enumerate(vect_coords.elements) if n not in dot_indices)
        ...
        arcs = VGroup(Arc(n * theta, 2 * theta, radius=circle.get_radius()) for n in range(1, len(vect_ghosts), 2))
        for arc, color in zip(arcs, it.cycle([BLUE, RED])):
            arc.set_stroke(color, 3)

        arc_labels = VGroup(
            Tex(R"2\theta", font_size=24).next_to(arc.pfp(0.5), normalize(arc.pfp(0.5)), SMALL_BUFF) for arc in arcs
        )
...
class ComplexComponents(InteractiveScene):
    def construct(self):
        ...
        x_vect, z_vect = var_vects = [
            TexMatrix(np.array([f"{char}_{n}" for n in range(4)]).reshape(-1, 1)) for char in "xz"
        ]
        ...
        complex_planes = VGroup(ComplexPlane((-1, 1), (-1, 1)).replace(number_line, 0) for number_line in number_lines)
        ...
        R_labels, C_labels = [
            VGroup(Tex(Rf"\mathds{{{char}}}", font_size=36).next_to(mob.get_right(), RIGHT) for mob in group)
        ...
        def update_dot_lines(lines):
            for line, x, dot in zip(lines, x_vect.elements, dots):
                line.put_start_and_end_on(x.get_right() + 0.05 * RIGHT, dot.get_center())
        ...
        self.play(
            ...
            LaggedStart(
                ...
                (FadeTransform(elem, dot) for elem, dot in zip(dec_vect.elements, dots)),
                lag_ratio=0.05,
                group_type=Group,
            ),
            LaggedStart(
                (FadeTransform(elem.copy(), x) for elem, x in zip(dec_vect.elements, x_vect.elements)),
                lag_ratio=0.05,
            ),
        ...
        phi_label.add_updater(lambda m: m.move_to(arc.pfp(0.5)).shift(0.5 * (arc.pfp(0.5) - plane.c2p(0))))
