...
class BeamSplitter(InteractiveScene):
    def construct(self):
        ...
        self.play(frame.animate.reorient(-57, 74, 0, (-3.29, -0.18, -0.18), 4.80), run_time=8)

        # Add sample vector
        amplitude = wave.amplitude
        sample_point = pointer.get_right() + 3 * RIGHT

        corner_plane, h_plane, v_plane, plane_in_situ = planes = VGroup(NumberPlane((-1, 1), (-1, 1)) for _ in range(4))
        for plane in planes:
