...
class Sudoku(VGroup):
    def __init__(
        ...
        self.little_grids = VGroup(
            self.get_square_grid(height / 3, little_square_stroke_width).move_to(square) for square in self.big_grid
        )
        self.numbers = VGroup(
            VGroup(Integer(num).replace(square, 1).scale(num_to_square_height_ratio) for square, num in zip(grid, arr))
...
class TwoByTwoGrid(InteractiveScene):
    ...
    def construct(self):
        ...
        good_sudoku, bad_sudoku = sudokus = VGroup(
            Sudoku(big_square_stroke_width=2, little_square_stroke_width=0.25) for x in range(2)
        )
...
class ShowSuperposition(InteractiveScene):
    def construct(self):
        ...
        rects = VGroup(
            SurroundingRectangle(mob.copy().rotate(90 * DEG, LEFT)).rotate(90 * DEG, RIGHT) for mob in [col, vect_label]
        )
...
class PassSuperpositionIntoVerifier(InteractiveScene):
    def construct(self):
        ...
        out_Vs = VGroup(Tex(R"V").set_color(TEAL).next_to(ket, LEFT, SMALL_BUFF) for ket in out_kets)
        ...
        out_col = TexMatrix(
            np.array(["x_0", "x_1", R"\vdots", "-x_k", R"\vdots", R"x_{n-2}", R"x_{n-1}"]).reshape(-1, 1)
        )
