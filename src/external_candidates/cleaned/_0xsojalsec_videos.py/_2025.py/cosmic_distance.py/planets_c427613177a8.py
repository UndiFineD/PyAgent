...
class PerspectivesOnEarth(InteractiveScene):
    def construct(self):
        ...
        frame.add_updater(
            lambda m: m.move_to(interpolate(m.get_center(), moon.get_center(), interp_factor.get_value()))
        )
        self.play(
...
class SizeOfEarthRenewed(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            frame.animate.reorient(-91, -36, 0, (4.91, -1.44, 0.39), 11.01).set_anim_args(
...
class LuarEclipse(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            time_label.animate.set_height(large_text_height, about_point=brace_copy.get_right()).set_anim_args(
                run_time=4
            ),
...
class LineOfSight(InteractiveScene):
    def construct(self):
        ...
        # Zoom in then out
        self.play(frame.animate.reorient(80, 82, 0, moon.get_center(), 0.80), run_time=5)
        self.wait()
        self.play(
...
class DistanceToSun(InteractiveScene):
    def construct(self):
        ...
        sun[0].add_updater(
            lambda m: m.move_to(sun_point).set_height(size_ratio * get_dist(sun_point.get_center(), earth.get_center()))
        )
        ...
        VGroup(words1, words2, sub_words, arrow1, arrow2).scale(0.6 / FRAME_HEIGHT, about_point=ORIGIN).shift(
            moon.get_center()
        )
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(brace, brace2, path_arc=45 * DEG) for brace2 in brace_copies),
                lag_ratio=0.1,
                time_span=(1, 5),
            ),
...
class KeplersMethod(InteractiveScene):
    def construct(self):
        ...
        earth_orbit.rotate(
            angle_of_vector(earth.get_center()) - angle_of_vector(earth_orbit.get_start()),
            about_point=ORIGIN,
        )
        ...
        for dot, mars_line in zip(earth_copies, mars_lines):
            dot.add_updater(
                lambda m: m.move_to(
                    earth_orbit.pfp(dens_func((earth_location_tracker.get_value() + m.orbit_offset) % 1))
                )
            )
