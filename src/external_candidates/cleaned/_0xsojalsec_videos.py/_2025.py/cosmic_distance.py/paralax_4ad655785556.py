...
class ParalxInSolarSystem(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                sun,
                lambda m, a: frame.move_to(interpolate(frame.get_center(), earth.get_center(), min(2 * a, 1))),
            ),
            run_time=5,
        )
...
class ParalaxMeasurmentFromEarth(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromFunc(
                line_between,
                lambda m: m.put_start_and_end_on(obs_dots[0].get_center(), obs_dots[1].get_center()),
            ),
            run_time=3,
        )
        self.wait()
...
class NearbyStars(InteractiveScene):
    def construct(self):
        ...
        obs_points = Group(
            TrueDot(point, radius=0.1).set_color(GREEN).make_3d() for point in [orbit.get_top(), orbit.get_bottom()]
        )
        ...
        self.play(
            LaggedStart(
                ...
                *(TransformFromCopy(eq2[tex][0], rhs[tex][0]) for tex in [R"2 \cdot \tan^{-1}\left(", R"\right)"]),
                FadeIn(rhs[R"1 \over"]),
                FadeIn(rhs[R"="]),
