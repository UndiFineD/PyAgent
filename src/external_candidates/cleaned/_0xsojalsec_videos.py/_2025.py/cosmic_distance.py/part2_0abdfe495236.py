...
class SunInTheSky(InteractiveScene):
    def construct(self):
        ...
        deg_labels = VGroup(
            Tex(Rf"{n}^\circ", font_size=2).next_to(protractor.n2p(60 * n), RIGHT, buff=0.01) for n in range(5)
        )
...
class VenusTransitTimeline(InteractiveScene):
    def construct(self):
        ...
        numbers = timeline.add_numbers(range(1500, 2050, 50), group_with_commas=False, font_size=20, buff=0.15)
        ...
        next_labels = VGroup(
            Text(f"+{n} years", font_size=16).next_to(na, UP, SMALL_BUFF) for na, n in zip(next_arrows, [8, 105])
        )
...
class DemonstrateAnArcSecond(InteractiveScene):
    def construct(self):
        ...
        deg_ticks = VGroup(self.get_tick_mark(circle, n * DEG, 1, max_aparent_length=1) for n in range(360))
        deg10_ticks = VGroup(self.get_tick_mark(circle, n * DEG, 2.5, max_aparent_length=1) for n in range(0, 360, 10))
        deg30_labels = VGroup(
            self.get_tick_label(tick, number) for tick, number in zip(deg10_ticks[0::3], range(0, 360, 30))
        )
        deg10_labels = VGroup(
            self.get_tick_label(tick, number, visibility_range=(60, 30))
            for tick, number in zip([*deg10_ticks[-2:], *deg10_ticks[1:3]], [340, 350, 10, 20])
        )
        deg_labels = VGroup(
        ...
        arc_minute_ticks = VGroup(
            self.get_tick_mark(circle, n * DEG / 60, 2.5e-2, max_aparent_length=0.75) for n in range(0, 60)
        )
        ...
        arc_second_ticks = VGroup(
            self.get_tick_mark(circle, n * DEG / 60 / 60, 3.5e-4, max_aparent_length=0.5) for n in range(0, 60)
        )
    ...
    def get_tick_mark(
        ...
        tick.add_updater(lambda m: m.set_length(min(frame_prop * self.frame.get_width(), tick_length)))
...
class WriteInverseSquareLaw(InteractiveScene):
    def construct(self):
        ...
        for text, color in zip(words, colors):
            part = eq[text][0]
            self.play(FlashAround(part, color=color, buff=0.1), part.animate.set_color(color))
            self.wait()
...
class SolarSpectrum(InteractiveScene):
    def construct(self):
        ...
        x_coords = VGroup(
            Integer(n, font_size=24).next_to(axes.x_axis.n2p(n), DOWN, MED_SMALL_BUFF) for n in range(0, 2800, 400)
        )
        ...
        self.play(
            ...
            *(coord.animate.stretch(2, 0, about_point=origin).stretch(0.5, 0) for coord in x_coords),
            run_time=3,
        )
        ...
        arrows = VGroup(Vector(0.5 * DOWN, thickness=2).set_color(WHITE).move_to(point, DOWN) for point in gap_points)
...
class ShrinkADime(InteractiveScene):
    def construct(self):
        ...
        self.add(plane)
        self.play(frame.animate.reorient(-90, 84, 0, (-0.04, 0.01, -0.01), 8.00), run_time=2)
