# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\convolutions\discrete.py
...
def get_aligned_pairs(group1, group2, n):
    return VGroup(*(VGroup(m1, m2) for m1 in group1 for m2 in group2 if m1.index + m2.index == n))


def get_pair_rects(pairs, together=True, buff=SMALL_BUFF, corner_radius=0.1):
    if together:
        return VGroup(*(SurroundingRectangle(pair, buff=buff).round_corners(corner_radius) for pair in pairs))
    else:
        return VGroup(
            *(
                VGroup(*(SurroundingRectangle(m, buff=buff).round_corners(corner_radius) for m in pair))
                for pair in pairs
            )
        )
...
def get_lagrange_polynomial(data):
    def poly(x):
        return sum(
            y0 * prod((x - x1) for x1, y1 in data if x1 != x0) / prod((x0 - x1) for x1, y1 in data if x1 != x0)
            for x0, y0 in data
        )
...
class WaysToCombine(InteractiveScene):
    def construct(self):
        ...
        axes1, axes2, axes3 = all_axes = VGroup(
            *(Axes((-3, 3), (-1, 1), height=2.0, width=FRAME_WIDTH * 0.5 - 1) for x in range(3))
        )
        ...
        box_arrows = VGroup(*(Arrow(box.get_right(), mystery_box.get_corner(UR), path_arc=-PI / 3) for box in boxes))
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(*boxes, *box_arrows, mystery_box, q_marks, comb_graph)))
        ...
        for seq, label, axes, graph, op_label in tuples:
            self.play(
                LaggedStart(
                    TransformMatchingShapes(VGroup(*seq1_tex[:2], *seq2_tex[:2]).copy(), seq[:4]),
            ...
            self.add(axes)
            # Go point by point
            value_rects = VGroup(*(VGroup(*map(SurroundingRectangle, s[-8::2])) for s in [seq1_tex, seq2_tex, seq]))
            dots = Group(*(GlowDot(color=WHITE) for x in range(3)))
            self.play(
                *map(FadeIn, [seq[4], seq[-1]]),
                *(VFadeInThenOut(rects, lag_ratio=0.5) for rects in value_rects),
                LaggedStart(
                    *(FadeIn(seq[n : n + 2 if n < 11 else n + 1]) for n in range(5, 12, 2)),
                    lag_ratio=0.5,
                ),
                run_time=2,
            )
            self.play(
                ...
                *(UpdateFromAlphaFunc(dot, lambda d, a: d.set_opacity(min(10 * a * (1 - a), 1))) for dot in dots),
                *(MoveAlongPath(dot, graph) for dot, graph in zip(dots, [f_graph, g_graph])),
                run_time=4,
            )
            self.wait()
...
class DiceExample(InteractiveScene):
    def construct(self):
        ...
        combinations_label = VGroup(OldTex("6^2 = "), Integer(36), Text("Combinations"))
        ...
        for n in range(2, 13):
            ...
            if n <= 4:
                self.play(
                    LaggedStart(
                        *(FlashAround(pair.copy(), remover=True, time_width=1, run_time=1.5) for pair in pairs),
                        lag_ratio=0.2,
                    ),
                )
                self.wait()
        ...
        sixths = VGroup(*(OldTex("1 / 6", font_size=36).next_to(die, UP, SMALL_BUFF) for die in all_dice))
        sixths.set_stroke(WHITE, 0)
        blue_probs, red_probs = [np.random.uniform(0, 1, 6) for x in range(2)]
        ...
        while n < 13:
            ...
            self.play(
                TransformMatchingShapes(
                    VGroup(*(VGroup(blue.prob_label, red.prob_label) for blue, red in pairs)).copy(),
                    rhs,
                )
            )
            self.wait()
        ...
        self.play(LaggedStartMap(ShowCreationThenFadeOut, rhs_rects, lag_ratio=0.1, run_time=5))
        self.wait()
        ...
        self.wait()
        self.play(Restore(blue_dice), Restore(red_dice), diagonals.animate.set_opacity(1))
    ...
    def get_p_sum_expr(self, n, rhs=" "):
        raw_expr = Tex(Rf"P\big(O + O = {n}\big) = \, {rhs}", isolate=("O", str(n), "=", rhs))
...
class SimpleExample(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(conv, 0.5 * DOWN, time_span=(0.5, 1.5)),
        )
        self.wait()
        ...
        for n in range(len(conv_result)):
            ...
            rects = VGroup(
                *(SurroundingRectangle(lp, buff=0.2).set_stroke(YELLOW, 1).round_corners() for lp in label_pairs)
            )
            self.play(FadeIn(rects, lag_ratio=0.5))
            self.play(
                LaggedStart(
                    *(TransformFromCopy(lp, nlp) for lp, nlp in zip(label_pairs, new_label_pairs)),
                    lag_ratio=0.5,
                ),
                Write(symbols),
            )
        ...
        self.play(
            ...
            LaggedStart(
                *(value.animate.next_to(square, UP, buff=0.2) for value, square in zip(values1, grid[:3])),
                *(value.animate.next_to(square, LEFT, buff=0.2) for value, square in zip(values2, grid[0::3])),
                run_time=2,
            ),
            *(
        ...
        self.play(
            LaggedStart(
                *(ReplacementTransform(factor.copy(), product) for product in products for factor in product.factors),
                lag_ratio=0.1,
            )
        )
        self.wait()
        ...
        arrows = VGroup(*(Vector(0.5 * UP).next_to(part, DOWN) for part in rhs[2::2]))
        ...
        polynomial_eq = Tex(
            ...
            tex_to_color_map=dict((f"{{{n}}}", YELLOW) for n in conv_result),
        )
        polynomial_eq.next_to(equation, DOWN, MED_LARGE_BUFF)

        self.play(FadeIn(polynomial_eq, DOWN), grid_group.animate.center().to_edge(DOWN))
        self.wait()
        ...
        for n, oval, arrow in zip(it.count(), ovals, arrows):
            ...
            self.wait(0.5)
        self.play(FadeOut(curr_arrow), ovals.animate.set_stroke(opacity=0.5))
...
class MovingAverageExample(InteractiveScene):
    ...
    def construct(self):
        ...
        for n, conv_bar in enumerate(conv_bars):
            ...
            label_pairs = VGroup(*(VGroup(m1.value_label, m2.value_label) for m1, m2 in pairs))
            rects = VGroup(
                *(SurroundingRectangle(lp, buff=0.05).set_stroke(YELLOW, 2).round_corners() for lp in label_pairs)
            )
            ...
            self.play(
                *(FadeTransform(label.copy(), value) for lp in label_pairs for label in lp),
                Restore(rect),
                run_time=self.march_anim_run_time,
            )
...
class CompareSizes(InteractiveScene):
    def construct(self):
        ...
        brace_labels = VGroup(
            *(brace.get_tex(tex, buff=SMALL_BUFF) for brace, tex in zip(braces, ["n", "m", "n + m - 1"]))
        )
        ...
        for n in range(len(conv_arr)):
            self.play(get_row_shift(top_arr, low_arr, n), run_time=0.5)
            pairs = get_aligned_pairs(top_arr, low_arr, n)
            rects = VGroup(*(SurroundingRectangle(pair, buff=-0.05) for pair in pairs))
            self.add(rects)
...
class ImageConvolution(InteractiveScene):
    ...
    def show_pixel_sum(self, tex=None, convert_to_vect=True, row_len=9):
        ...
        rows = VGroup(*(expr[n : n + 3 * row_len] for n in range(0, len(expr), 3 * row_len)))
        for row in rows:
        ...
        if convert_to_vect:
            self.play(
                LaggedStart(*(Transform(lil_pixel, rgb_vect) for lil_pixel, rgb_vect in zip(lil_pixels, rgb_vects)))
            )
        self.wait()
...
class SobelFilter1(ImageConvolution):
    ...
    def construct(self):
        ...
        for index in self.stops:
            ...
            self.zoom_to_kernel()
            self.play(
                *(square.animate.set_fill(opacity=0, recurse=False) for square in kernel),
...
class ContrastConvolutionToMultiplication(InteractiveScene):
    def construct(self):
        ...
        left_arrays, right_arrays = (
            VGroup(*(self.get_array_mobject(arr, color) for arr, color in [(arr1, BLUE), (arr2, YELLOW)]))
            .arrange(DOWN, buff=1.0)
            .move_to(vect * quart + UP)
        ...
        lines = VGroup(*(Line(e1, e2, buff=0.1) for e1, e2 in zip(top_arr.elements, low_arr.elements)))
        lines.set_stroke(TEAL, 1)
...
class BigPolynomials(InteractiveScene):
    def construct(self):
        ...
        a_terms, b_terms = all_terms = [
            VGroup(*(Tex(Rf"{letter}_{{{n}}} x^{{{n}}}", font_size=24) for n in (*range(N - 1), 99)))
            for letter in ["a", "b"]
        ]
        ...
        self.play(FlashAround(a_terms, run_time=2, time_width=2), FadeIn(a_label))
        self.play(
...
class FunctionToCoefficientCommutativeDiagram(InteractiveScene):
    def construct(self):
        # Axes
        axes1, axes2, axes3 = all_axes = VGroup(*(Axes((-3, 3), (-2, 4), width=6, height=4) for x in range(3)))
        ...
        all_circles = VGroup(
            *(VGroup(*(Circle(radius=0.1).set_stroke(YELLOW, 2).move_to(dot) for dot in dots)) for dots in all_dots)
        )
        self.play(
...
class DataPointsToPolynomial(InteractiveScene):
    def construct(self):
        ...
        dots = Group(*(GlowDot(axes.c2p(x, y), glow_factor=0.8, radius=0.1) for x, y in data))
        dots.set_color(TEAL)
        circles = VGroup(*(Circle(radius=0.075).set_stroke(YELLOW, 2).move_to(dot) for dot in dots))

        graphs = VGroup(*(axes.get_graph(get_lagrange_polynomial(data[:n])) for n in range(1, len(data) + 1)))
        ...
        poly_tex = OldTex(
            ...
            "a_1 x",
            "+",
            *it.chain(*((f"a_{n} x^{n}", "+") for n in range(2, len(data) - 1))),
        )
        ...
        self.play(ShowCreation(graph), FadeIn(poly_tex[:3]))
        self.wait()
...
class PolynomialSystem(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(FlashAround(coef, time_width=1) for coef in poly_x.coefs),
                lag_ratio=0.1,
                run_time=3,
            ),
        )
        self.wait()
        self.play(Write(eq), FadeIn(fg, 0.25 * UP))
        self.wait()
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(top_eq.copy(), eq) for eq in equations),
                lag_ratio=0.02,
                run_time=3,
            ),
        )
        ...
        self.play(
            Write(words2),
            LaggedStart(
                *(FlashAround(lhs[:4], time_width=1.5) for lhs in lhss),
                run_time=5,
                lag_ratio=0.1,
            ),
        )
        ...
        inputs.save_state()
        consts = VGroup(*(power for eq in rhss for power in eq.powers))
        ...
        all_omega_powers = [VGroup(*(Tex(Rf"\omega^{{{(k * n) % N}}}", **kw) for n in range(N))) for k in range(0, N)]
        new_rhss = VGroup(*(self.get_polynomial(coefs.copy(), omega_powers) for omega_powers in all_omega_powers))

        new_lhss = VGroup(*(Tex(Rf"h(\omega^{n}) = ") for n in range(N)))
        ...
        for arrow in (top_arrow, low_arrow):
            ...
            self.play(GrowArrow(arrow), FadeIn(fft_label, arrow.get_vector()))
...
class RootsOfUnity(InteractiveScene):
    def construct(self):
        ...
        max_power = 3 * N
        powers = VGroup(*(OldTex(Rf"\omega^{{{k}}}", **kw) for k in range(max_power)))
        ...
        self.play(FadeOut(moving_power), powers[:N].animate.set_opacity(1))
        self.wait()
...
class AlgorithmOutline(InteractiveScene):
    def construct(self):
        ...
        # Arrays
        t2c = {tex: BLUE for tex in [R"\textbf{a}", "a_0", "a_1", "a_2", "a_{n - 1}"]}
        t2c.update({tex: TEAL for tex in [R"\textbf{b}", "b_0", "b_1", "b_2", "b_{m - 1}"]})
        ...
        lists = VGroup(
            ...
            Tex(
                R"""\textbf{a} * \textbf{b} = \left[\begin{array}{l}
                    a_0 b_0, \\
                    a_0 b_1 + a_1 b_0, \\
        ...
        axes = VGroup(*(Axes((-3, 3), (-2, 2), width=5, height=1.5) for x in range(3)))
        axes.to_edge(RIGHT)
        ...
        self.play(
            LaggedStart(*(TransformFromCopy(listy.copy(), poly) for listy, poly in zip(lists, polys))),
        ...
        f_points, g_points, fg_points = all_points = [
            [ax.i2gp(x, graph) for x in x_samples] for ax, graph in zip(axes, graphs)
        ]
        ...
        self.play(FadeIn(f_dots, lag_ratio=0.7), FadeIn(g_dots, lag_ratio=0.7), run_time=5)
        self.wait()
        self.play(
            TransformFromCopy(axes[1], axes[2]),
            TransformMatchingShapes(graph_labels[:2].copy(), graph_labels[2]),
            LaggedStart(
                *(Transform(Group(fd, gd).copy(), Group(fgd), remover=True) for fd, gd, fgd in zip(*all_dots)),
                lag_ratio=0.5,
                run_time=5,
            ),
        )
        ...
        fft_lists = VGroup(
            Tex(
                R"\hat{\textbf{a}} = [\hat{a}_0, \hat{a}_1, \hat{a}_2, \dots, \hat{a}_{m + n - 1}]",
                **fft_kw,
            ),
            Tex(
                R"\hat{\textbf{b}} = [\hat{b}_0, \hat{b}_1, \hat{b}_2, \dots, \hat{b}_{m + n - 1}]",
                **fft_kw,
            ),
            Tex(
                R"""\hat{\textbf{a}} \cdot \hat{\textbf{b}} = [
                    \hat{a}_0 \hat{b}_0,
                    \hat{a}_1 \hat{b}_1,
                    \hat{a}_2 \hat{b}_2,
                    \dots,
...
class FourierCoefficients(InteractiveScene):
    def construct(self):
        ...
        def f(x):
            return max(
                np.exp(-(x**2)) + math.cos(2 * PI * x) - 0.2 * math.sin(4 * PI * x),
                0.25,
            )
        ...
        equations = VGroup(
            ...
            Tex(
                Rf"f(t) = \cdots + c_{{-1}} {exp}^{{-1}} + c_0 + c_1 {exp} + c_2 {exp}^2 + \dots",
                **kw,
            ),
        ...
        for equation in equations:
            ...
            self.wait()
            self.play(FadeOut(last, UP), equation.animate.to_edge(UP))
            last = equation
