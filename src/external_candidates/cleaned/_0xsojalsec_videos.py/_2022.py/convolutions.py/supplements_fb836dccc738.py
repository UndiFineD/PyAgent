# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\convolutions\supplements.py
...
class Intimidation(InteractiveScene):
    def construct(self):
        ...
        self.play(LaggedStart(*(pi.change("pondering", look_at=3 * UR) for pi in pis)))
        self.play(LaggedStart(*(Blink(pi) for pi in pis)))
        self.wait()
        self.play(
            LaggedStart(*(pi.change(mode, look_at=3 * UR) for pi, mode in zip(pis, ("horrified", "maybe", "pleading"))))
        )
        for x in range(2):
...
class OtherVisualizations(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(self.change_students("confused", "erm", "thinking"))
        self.wait(4)
        self.play(
...
class MarioConvolutionLabel(BoxBlurMario):
    ...
    def construct(self):
        ...
        self.add(squares)
        self.remove(ka)
        self.play(LaggedStart(*(Rotate(square[0], -PI, **kw) for square in ka)))
        self.wait(2)
...
class SobelKernelLabel(MarioConvolutionLabel, SobelFilter2):
    ...
    def construct(self):
        for square in self.kernel_array:
            ...
            square.set_fill(rgb_to_color([max(-value, 0), max(value, 0), max(value, 0)]), 1)
...
class DumbIdea(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            stds[0].change("raise_right_hand"),
            FadeIn(fft, UP),
