...
class ConstructPolynomialWithGivenRoots(Scene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(x_term.copy(), dot) for x_term, dot in zip(x_terms, root_dots)),
                lag_ratio=0.7,
                run_time=3,
            )
        )
        ...
        rects = VGroup(
            *(
                SurroundingRectangle(factored[i : i + 5], stroke_width=1, stroke_color=BLUE, buff=0.05)
                for i in range(1, 12, 5)
            )
        )
        arrows = VGroup(*(Vector(DOWN).next_to(dot, UP, buff=0) for dot in root_dots))
        zeros_eqs = VGroup(
            *(OldTex(f"P({r}) = 0", font_size=24).next_to(rect, UP, SMALL_BUFF) for r, rect in zip(roots, rects))
        )
        ...
        for alt_roots in [(2, 4, 1), (4, 2, 1), (1, 4, 2), (1, 2, 4)]:
            ...
            movers, targets = [
                VGroup(*(group.get_parts_by_tex(str(root)) for root in alt_roots for group in groups))
                for groups in [
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeTransform(dot.copy(), term, remover=True) for dot, term in zip(root_dots, root_terms)),
                lag_ratio=0.5,
                run_time=3,
            ),
        )
        ...
        root_tuple_groups = VGroup(
            *(VGroup(*(VGroup(*tup) for tup in it.combinations(root_terms, k))) for k in range(1, 6))
        )
        ...
        for equation, tuple_group in zip(q_system, root_tuple_groups):
            ...
            rects_group = VGroup(
                *(VGroup(*(SurroundingRectangle(term).set_stroke(BLUE, 2) for term in tup)) for tup in tuple_group)
            )
...
class FactsAboutRootsToCoefficients(RootCoefScene):
    ...
    def add_graph(self):
        ...
        root_dots.add_updater(
            lambda d: d.set_points([axes.c2p(r.real, 0) for r in self.get_roots() if abs(r.imag) < 1e-5])
        )
    ...
    def add_solver_functions(self):
        ...
        self.play(
            LaggedStart(
                *(TransformFromCopy(r, line[0]) for r, line in zip(self.get_r_symbols(self.root_poly), lines)),
                **kw,
            ),
    ...
    def swap_root_symbols(self):
        ...
        for tup in [(1, 2, 0), (2, 0, 1), (0, 1, 2)]:
            ...
            self.play(
                *(
                    TransformMatchingTex(l1, l2, path_arc=PI / 2, lag_ratio=0.01, run_time=2)
