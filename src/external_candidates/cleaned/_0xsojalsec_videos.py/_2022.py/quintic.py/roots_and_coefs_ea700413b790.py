...
def expanded_poly_tex(roots, vertical=True, root_colors=[YELLOW, YELLOW], abbreviate=False):
    ...
    n = len(roots)
    kw = dict(
        tex_to_color_map=dict(((str(r), root_color) for r, root_color in zip(roots, root_colors))),
        arg_separator=" ",
    )
    result = VGroup()
...
class RootCoefScene(Scene):
    ...
    def on_mouse_release(self, point, button, mods):
        ...
        if self.lock_coef_imag or self.window.is_key_pressed(ord("r")):
            ...
            dot.add_updater(
                lambda d: d.move_to(self.coef_plane.c2p(*d.last_norm * normalize(self.coef_plane.p2c(d.get_center()))))
            )
