# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\puzzles\subsets.py
import mpmath
import sympy
...
def get_integer_parts(set_tex):
    result = VGroup(*(sm for sm in set_tex if isinstance(sm, Integer)))
    if len(result) == 0:
...
def set_tex_transform(set_tex1, set_tex2):
    ...
    mismatch_animations = [
        FadeInFromPoint(get_part_by_value(set_tex2, value), set_tex1.get_center())
...
def get_sum_wrapper(set_tex):
    wrapper = VGroup(OldTex("\\text{sum}\\big(\\big) = ")[0], Integer(sum(set_tex.values)))
...
def get_subsets(full_set):
    return list(it.chain(*(it.combinations(full_set, k) for k in range(len(full_set) + 1))))
...
def get_question_title():
    ...
    question = OldTexText(
        f"Find the number of subsets of {st},\\\\ the sum of whose elements is divisible by 5",
        isolate=[st],
    )
...
def massive_int(num, n_cols=42, width=7):
    ...
    for n in range(len(total) - 3, -3, -3):
        comma = Tex(",")
        triplet = total[n : n + 3]
        triplet.arrange_to_fit_width(triplet.get_width() - 2 * comma.get_width(), about_edge=LEFT)
...
class UnreasonableUsefulness(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(
                plane.coordinate_labels[-2:].set_opacity,
                0,
                time_span=(3, 4),
            ),
...
class ToyVsRH(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(toy, 0.5 * RIGHT))
        self.wait()
...
class GoThroughAllSubsets(InteractiveScene):
    ...
    def construct(self):
        ...
        counter = VGroup(Text("Count so far: "), Integer(0, edge_to_fix=LEFT))
...
class TwoThousandBinaryChoices(InteractiveScene):
    def construct(self):
        ...
        elem_rects = VGroup(*(SurroundingRectangle(elem, buff=0.05).round_corners() for elem in elems))
        elem_rects.set_stroke(BLUE, 2)

        twos = VGroup(*(Integer(2).match_width(elems[1]).next_to(er, UP) for er in elem_rects))
        dots = VGroup(
            *(Tex("\\cdot").move_to(midpoint(t1.get_right(), t2.get_left())) for t1, t2 in zip(twos, twos[1:]))
        )
...
class AskAboutGuess(InteractiveScene):
    def construct(self):
        ...
        self.play(ShowCreation(guess_rect), Write(nonint))
        self.wait()
...
class ExampleWith5(InteractiveScene):
    ...
    def construct(self):
        ...
        answer_word, answer_count = answer = VGroup(Text("Answer: "), Integer(0))
        ...
        # Contrast with 1/5
        count = OldTexText("Total subsets:", " $2^5 = $", " $32$")
    ...
    def construct_all_subsets(self, set_tex):
        ...
        value_parts = VGroup(*(get_part_by_value(set_tex, v) for v in range(1, 6)))
        rects = VGroup(*(SurroundingRectangle(vp, buff=0.1).round_corners() for vp in value_parts))
        rects.set_stroke(BLUE, 2)

        words = Text("5 binary choices")
        words.next_to(set_tex, DOWN, buff=1.5)
        lines = VGroup(*(Line(words.get_top(), rect.get_bottom(), buff=0.15) for rect in rects))
        lines.match_style(rects)

        def match_n(rects, n):
            bits = it.chain(str(bin(n)[-1:1:-1]), it.repeat("0"))
        ...
        for value in range(1, 6):
            ...
            added_subsets = VGroup(*(get_set_tex([*ss.values, value]).move_to(ss) for ss in subsets))
            ...
            self.play(
                ...
                *(set_tex_transform(st1, st2) for st1, st2 in zip(subsets_copy, added_subsets)),
    ...
    def group_by_sum(self, stacks, sum_stacks):
        ...
        for n in range(max_sum + 1):
            stack = VGroup(*filter(lambda ssg: sum(ssg[0].values) == n, subset_groups))
        ...
        self.play(MoveToTarget(common_sum_stacks), *rect_anims, run_time=2)
        self.play(
    ...
    def show_generating_function(self, set_tex):
        ...
        parts = VGroup(*(factored.get_part_by_tex(term) for term in factored_terms))
        rects = VGroup(*(SurroundingRectangle(part, buff=0.05).round_corners() for part in parts))
        rects.set_stroke(BLUE, 2)
        ...
        subset_groups.submobjects.sort(key=lambda ssg: sum(ssg[0].values))

        for subset_group in subset_groups:
            bits = [i + 1 in subset_group[0].values for i in range(5)]
            top_terms = [part[3:-1] if bit else part[1] for bit, part in zip(bits, fac_term_parts)]
            top_rects = VGroup(
                *(
            ...
            if len(super_expanded) > 33:
                super_expanded[33:].next_to(super_expanded[0], DOWN, MED_LARGE_BUFF, aligned_edge=LEFT)
            low_rect = SurroundingRectangle(new_term, buff=0.5 * SMALL_BUFF)
            low_rect.set_stroke(BLUE, 2).round_corners()
            collection_anims.append(FadeTransform(new_term, expanded_parts[n], path_arc=10 * DEGREES))
        ...
        self.play(
            ...
            MoveToTarget(lower_group, rate_func=squish_rate_func(smooth, 0.5, 1.0)),
            run_time=3,
        )
        self.add(expanded)
    ...
    def transition_to_full_generating_function(self, set_tex):
        ...
        get_integer_parts(new_set_tex)[-1].set_color(interpolate_color(BLUE_E, BLUE_D, 0.5))
        ...
        self.add(factored)
        self.wait()
        self.play(Write(factored_word), FadeOut(small_group, DOWN))
        self.play(
        ...
        self.play(VFadeIn(morty), morty.change("surprised", expanded), Write(words))
        self.play(Blink(morty))
        self.wait()
        ...
        # Show example term
        n = 25
        subsets = list(filter(lambda s: sum(s) == n, get_subsets(range(1, n + 1))))
        coef = len(subsets)
        ...
        self.play(FadeIn(top_rect), ShowIncreasingSubsets(subset_mobs, run_time=5))
        self.wait()
        self.play(
    ...
    def get_subset_stacks(self, full_set, buff=3.5):
        stacks = VGroup(
            *(
                VGroup(*(get_set_tex(subset) for subset in it.combinations(full_set, k)))
    ...
    def get_subset_sums(self, stacks):
        return VGroup(*(VGroup(*(get_sum_group(set_tex) for set_tex in stack)) for stack in stacks))
...
class ShowHypercubeConstruction(InteractiveScene):
    def construct(self):
        ...
        for n, vect, color in zip(indices, vects, colors):
            new_sets = VGroup(*(get_set_tex([*st.values, n]) for st in curr_sets))
            for ns in new_sets:
            ...
            self.play(*(set_tex_transform(csc, ns) for csc, ns in zip(curr_set_copies, new_sets)))
            self.wait()
...
class QuestionPolynomial(InteractiveScene):
    def construct(self):
        ...
        poly = Tex(
            ...
            tex_to_color_map=dict(zip((f"{{{n}}}" for n in n_range), color_gradient([BLUE_B, BLUE_D], 5))),
            isolate=["x"],
        )
        poly.move_to([-0.81438428, 2.25048456, 0.0])
        self.add(poly)
        ...
        # Expansion
        expansion = Tex(
            "1+x^{1}+x^{2}+2 x^{3}+2 x^{4}+3 x^{5}+3 x^{6}+3 x^{7}+3 x^{8}+3 x^{9}+3 x^{10}+2 x^{11}+2 x^{12}+x^{13}+x^{14}+x^{15}"
        )
...
class PolynomialConstruction(InteractiveScene):
    def construct(self):
        ...
        for n in range(2, 8):
            ...
            self.wait()
            new_term = Tex(f"x^{n}")
            new_poly = Tex("".join([f"\\left(1 + x^{k}\\right)" for k in range(1, n + 1)]))
            new_term.next_to(cp_r, LEFT, SMALL_BUFF)
            new_term.align_to(cp_r.family_members_with_points()[1], DOWN)
            new_sets = VGroup(*(get_set_tex([*st.values, n]) for st in sg_r))
            anims = [Write(new_term), Write(plus)]
            for ns, s in zip(new_sets, sg_r):
                ns.match_height(s)
            new_sets.arrange_in_grid(n_cols=2 ** int(np.ceil(np.log2(len(new_sets)) / 2)))
            ...
            new_set_group.target.arrange_in_grid(n_cols=2 ** int(np.ceil(np.log2(len(new_set_group)) / 2)))
...
class GeneratingFunctions(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(title), ShowCreation(underline), run_time=1)

        # First poly
        degree = 10
        poly = OldTex(
            "f(x) = 1+1 x^{1}+1 x^{2}+2 x^{3}+2 x^{4}+3 x^{5}+4 x^{6}+5x^{7}+6x^{8}+8x^{9} + 10x^{10} + \\cdots",
            isolate=["=", "+", "x"],
        )
        ...
        # Fibbonacci poly
        fib_poly = OldTex(
            "F(x) = 0+1 x^{1}+1 x^{2}+2 x^{3}+3 x^{4}+5 x^{5}+ 8 x^{6} + 13 x^{7}+21 x^{8}+\\cdots",
            isolate=["=", "+", "x"],
        )
        coefs = VGroup(
            *(
                ...
                for sep in (
                    fib_poly.get_part_by_tex("="),
                    *fib_poly.get_parts_by_tex("+"),
                )
            )
        )
        coefs.set_color(RED)
        ...
        for n in range(1, 7):
            ...
            self.play(FadeOut(coef_arrows), ShowCreation(new_arrows, lag_ratio=0.5))
        ...
        self.wait()
        self.play(TransformMatchingShapes(func_prop1.copy(), func_prop2))
        self.wait()
        ...
        func_prop2.target.next_to(details_title, DOWN, MED_LARGE_BUFF).align_to(details_box, LEFT).shift(
            SMALL_BUFF * RIGHT
        )
...
class EvaluationTricks(InteractiveScene):
    def construct(self):
        ...
        line.add_updater(lambda l: l.put_start_and_end_on(plane.c2p(get_x(), 0), plane.i2gp(get_x(), graph)))

        self.play(
            Write(plane, lag_ratio=0.01),
            LaggedStartMap(
                FadeOut,
                VGroup(factored_label, expanded_label, factored.arrow, expanded.arrow),
            ),
        )
        ...
        self.wait()
        self.play(Write(f1[5:11]), x_tracker.animate.set_value(1))
        ...
        self.play(TransformMatchingShapes(f1[:5].copy(), fm1[:5]), FadeOut(f0, DOWN))
        self.wait()
        self.play(Write(fm1[5:7]), ApplyMethod(x_tracker.set_value, -1, run_time=3))
        self.wait()
        ...
        filter_expr = Tex(
            "{1 \\over 2} \\Big(f({1}) + f({-1})\\Big)= c_0 + c_2 + c_4 + \\cdots + c_{N}",
            tex_to_color_map={
                "{1}": BLUE,
                "{-1}": RED,
            },
        ...
        desire = VGroup(VGroup(parens, words), Tex("= c_0 + c_5 + c_{10} + \\cdots + c_{N}"))
        ...
        for arrow, outcome in zip(arrows, outcomes):
            self.play(ShowCreation(arrow), GrowFromPoint(outcome, rect.get_right()))
            self.wait()
...
class MotivateRootsOfUnity(InteractiveScene):
    def construct(self):
        ...
        exp_parts = VGroup(*(polynomial.get_part_by_tex(f"x^{{{n}}}") for n in n_range))

        self.add(polynomial)

        # Add f(-1)
        m1_pieces = [p.replace("(x)", "x").replace("x", "\\left(-1\\right)") for p in pieces]
        ...
        self.play(
            ...
            PiCreatureSays(
                morty,
                OldTexText("Visualize the\\\\rotations", font_size=36),
        ...
        vect = v1.copy()
        neg1_powers = [Tex(f"(-1)^{{{n}}}") for n in range(8)]
        ...
        plane_unit = 1.75
        plane = ComplexPlane((-3, 3), (-2, 2), axis_config=dict(unit_size=plane_unit))
        ...
        self.play(ShowCreation(unit_circle), ShowCreation(dots))
        self.wait()
...
class FifthRootsOfUnity(InteractiveScene):
    def construct(self):
        ...
        root_dots = Group(*(GlowDot(point) for point in root_points))
        root_lines = VGroup(
            *(
                Arrow(
                    plane.get_origin(),
        ...
        # Add function label
        function = Tex("f(x) = \\sum_{n = 0}^N c_n x^n", tex_to_color_map={"x": BLUE})
        ...
        self.play(LaggedStartMap(FadeOut, VGroup(trig_rhs, x_line, low_cos, y_line, low_sin)))
        ...
        self.play(
            Write(title),
            LaggedStart(
                *(FlashAround(zl, time_width=1.5) for zl in zeta_labels),
                lag_ratio=0.1,
                run_time=3,
            ),
        )
        ...
        for k, ex_text, ex_sum in zip(it.count(1), example_texts, example_sums):
            ...
            if k == 1:
                ...
                # Show permutation
                arrows = VGroup(
                    *(Arrow(root_dots[n], root_dots[(k * n) % 5], buff=0, stroke_width=3) for n in range(1, 5))
                )
        ...
        self.play(GrowFromCenter(brace), MoveToTarget(example))
        ...
        answer = Tex(
            "c_0 + c_5 + c_{10} + \\cdots=\\frac{1}{5}\\sum_{k = 0}^4 f(\\zeta^k)",
            tex_to_color_map={"\\zeta": YELLOW},
        )
        answer.set_width(5)
        ...
        self.play(VShowPassingFlash(answer_rect.copy()), FadeIn(answer_rect))
        ...
        f_zeta = Tex(
            "f(\\zeta) = \\Big((1+\\zeta)(1+\\zeta^{2})(1+\\zeta^{3})(1+\\zeta^{4})(1+\\zeta^{5})\\Big)^{400}",
            **eq_kw,
        )
        ...
        self.play(TransformFromCopy(fz1, f_zeta[:5]), FadeOut(function, DOWN))
        self.wait()
        ...
        zp1_labels = VGroup(*(Tex(f"\\zeta^{{{n}}} + 1", **root_kw) for n in range(5)))
        ...
        root_groups = Group(*(Group(*parts) for parts in zip(root_dots, root_lines, zp1_labels)))
        ...
        terms = VGroup(*(Tex(f"\\zeta^{{{sum(ss)}}}") for ss in subsets))
        ...
        term_rects = VGroup(*(SurroundingRectangle(term).round_corners().set_stroke(TEAL, 2) for term in terms))
        ...
        arrows = VGroup(*(Vector(0.5 * UP).next_to(part, UP, SMALL_BUFF) for part in parts))
        values = VGroup(
        ...
        self.play(ShowCreation(arrows[1]), FadeIn(values[1], 0.5 * UP))
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(ShowCreation(arrows[0]), FadeIn(values[0], 0.5 * UP))
        self.wait()
        ...
        # Final answer
        final_answer = Tex("= \\frac{1}{5}\\Big(2^{2{,}000} + 4 \\cdot 2^{400}\\Big)")
        ...
        self.wait()
        self.play(FadeTransform(values.copy(), final_answer[5:-1]))
        self.play(ShowCreation(final_answer_rect))
        self.wait(note="Comment on dominant term")

        # Smaller case
        box = Rectangle(width=(RIGHT_SIDE[0] - plane.get_right()[0]) - 1, height=plane.get_height())
        box.to_corner(DR)
        ...
        rhs = Tex("\\rightarrow \\frac{1}{5}\\Big(2^5 + 4 \\cdot 2^1\\Big)= \\frac{1}{5}(32 + 8)=8")
        rhs.scale(0.9)
        ...
        self.wait()
        self.play(Write(rhs[0]), FadeTransform(final_answer.copy(), rhs[1:13]))
        self.wait()
    ...
    def animate_kth_power(self, plane, dots, k):
        ...
        self.play(*(MoveAlongPath(dot, path) for dot, path in zip(dots, paths)), run_time=4)
        if k == 5:
            self.wait()
...
class JustifyLinearity(InteractiveScene):
    ...
    def construct(self):
        ...
        zeta_polys = VGroup(*(self.get_polynomial("\\zeta", YELLOW, exp_multiple=n) for n in range(5)))
        ...
        simple_examples = VGroup(
            *(Tex(f"f(x) = x^{{{n}}}", tex_to_color_map={"x": BLUE}, font_size=36) for n in range(1, 6))
        )
        ...
        self.play(
            PiCreatureSays(
                randy,
                OldTexText("But $f$ is more\\\\complicated than that!", font_size=36),
        ...
        lhss = VGroup(*(poly[: poly.sep_indices[0]] for poly in zeta_polys))
        lhss.save_state()
        ...
        for j in range(len(x_poly.sep_indices) - 1):
            ...
            self.wait()
            self.play(Write(plus), *(FadeTransform(piece.copy(), term) for piece in col[1:]))
            self.wait()
...
class ReflectOnGeneratingFunctions(InteractiveScene):
    def construct(self):
        ...
        seq = [sum(sum(s) == n for s in subsets) for n in range(1, 15)]
        ...
        poly_str = " + ".join((f"{s} x^{{{n}}}" for n, s in enumerate(seq)))
        poly_str += "+ \\cdots"
        poly = Tex(
            poly_str,
        ...
        plus_indices = [
            -1,
            *(poly.submobjects.index(p[0]) for p in poly.select_parts("+")),
        ]
        coefs = VGroup(*[poly[i + 1 : i + 1 + len(str(n))] for i, n in zip(plus_indices, seq)])
        self.play(
            LaggedStart(*(Transform(sm.copy(), coef.copy(), remover=True) for sm, coef in zip(seq_mobs, coefs))),
            LaggedStart(
                *(FadeIn(sm, scale=3.0) for sm in poly if not set(sm.get_family()).intersection(coefs.get_family()))
            ),
            run_time=2,
        )
        self.add(poly)
        ...
        out_plane = ComplexPlane(
            (-10, 10),
            (-10, 10),
            background_line_style=dict(stroke_width=1),
            faded_line_ratio=0,
        )
...
class GeneratingFunctionForPrimes(InteractiveScene):
    def construct(self):
        ...
        vm_def = VGroup(
            Tex(
                "\\Lambda(n)= \\log p & \\text { if } n=p^{k} \\text { for some prime } p \\text { and integer } k \\geq 1"
            ),
        ...
        self.play(GrowFromCenter(zeta_brace), Write(zeta_sym))
        self.wait()
        self.play(
...
class DirichletSeries(InteractiveScene):
    def construct(self):
        ...
        vm_def = Tex(
            "n \\rightarrow \\log p & \\text { if } n=p^{k} \\text { for some prime } p \\text { and integer } k \\geq 1"
        )
        vm_def.scale(0.5)
        ...
        self.play(
            ...
            *(ReplacementTransform(number, denom) for number, denom in zip(numbers, denoms)),
        ...
        out_line.add_updater(
            lambda m: m.set_points([out_plane.n2p(func(in_plane.p2n(p))) for p in v_line.get_points()])
        )

        # Complex mapping
        self.disable_interaction(in_plane, out_plane)

        self.play(FadeIn(in_plane), FadeIn(out_plane), FadeIn(plane_labels), FadeIn(in_dot))
        self.play(
            ShowCreation(arrow),
        ...
        self.wait()
        self.play(v_line.animate.move_to(in_plane.n2p(-1)), run_time=6)
        self.wait()
...
class AskAboutComplex(TeacherStudentsScene):
    def construct(self):
        ...
        # Blank check
        new_line = Tex("f(\\dots) = \\text{(more information)}", tex_to_color_map={"\\dots": BLUE})
...
class ProblemStatement(TeacherStudentsScene):
    def construct(self):
        ...
        words = VGroup(*(title.get_part_by_text(text) for text in title.get_text().split(" ")))

        morty = self.teacher

        self.play(
            LaggedStart(*(FadeInFromPoint(word, morty.get_corner(UL)) for word in words)),
        ...
        self.play(
            LaggedStart(
                *(FlashAround(int_part) for int_part in get_integer_parts(set_tex)),
                lag_ratio=0.05,
                run_time=3,
            ),
        ...
        for subset, sum_wrapper, mode_triple in zip(subsets, sum_wrappers, mode_triples):
            ...
            elem_rects = VGroup(
                *(
                    SurroundingRectangle(get_part_by_value(set_tex, value)).set_stroke(TEAL, 2).round_corners()
            ...
            if subset is subsets[0]:
                self.play(sum_group.animate.scale(0.5).to_edge(LEFT).set_y(2), *anims)
            else:
                self.play(
...
class BookQuestionHighlight(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeTransform(lil_rect.copy().set_opacity(0), title))
        self.wait()
        self.play(
            LaggedStart(
                *(FlashAround(int_part) for int_part in get_integer_parts(title[1])),
                lag_ratio=0.05,
                run_time=3,
            ),
        )
...
class StrangeTurnsFraming(InteractiveScene):
    def construct(self):
        ...
        titles = VGroup(*(Text(f"Strange turn {n}", font_size=48) for n in [1, 2]))
    ...
    def construct(self):
        ...
        self.play(
            PiCreatureBubbleIntroduction(
                randy,
                OldTexText("Is there a\\\\geometric structure?"),
...
class RotationAnnotation(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            PiCreatureSays(
                morty,
                OldTexText("Think about\\\\the half turn", font_size=24),
                look_at=arc,
...
class ComingUpWrapper(VideoWrapper):
    ...
    def construct(self):
        ...
        self.wait(4)
        self.play(Write(parens), MoveToTarget(new_title))
        self.wait(10)
...
class QuestionMorph(InteractiveScene):
    def construct(self):
        question = OldTexText("How many subsets are there\\\\", "with a sum divisible by 5", "?")
...
class FirstTrick(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Here's our\nfirst trick", target_mode="tease", run_time=1)
        self.play_student_changes("pondering", "thinking", "sassy", look_at=self.screen)
        self.wait(4)
...
class SquaringZeta(InteractiveScene):
    def construct(self):
        ...
        # Explicit
        exp = Tex("\\zeta^2 =\\big(e^{2\\pi i / 5}\\big)^2 =e^{4\\pi i / 5}", **kw)
...
class EqualsZeta3(InteractiveScene):
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(tex, DOWN))
        self.wait()
...
class EvaluateF1(InteractiveScene):
    def construct(self):
        ...
        self.add(f1)
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(ans, 0.25 * DOWN))
        self.wait()
...
class TantalizinglyClose(InteractiveScene):
    def construct(self):
        ...
        self.play(Blink(randy))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(brace, words, randy), shift=DOWN))
...
class StudentSaysWhat(TeacherStudentsScene):
    def construct(self):
        self.student_says("I'm sorry, what?!", target_mode="angry")
        self.play(
...
class FocusOnPropertiesNotValues(TeacherStudentsScene):
    def construct(self):
        ...
        words = VGroup(
            Tex(
                "\\text{Don't worry about the}\\\\ \\text{numerical value of } \\zeta",
                **kw,
            ),
            Tex(
                "\\text{Focus on properties}\\\\ \\text{of } \\{\\zeta^1, \\zeta^2, \\zeta^3, \\zeta^4\\, \\zeta^5\\}",
                **kw,
            ),
        )
...
class ConjugatePairFact(InteractiveScene):
    def construct(self):
        ...
        lines.add_updater(
            lambda ls: [line.put_start_and_end_on(plane.n2p(0), dot.get_center()) for line, dot in zip(ls, dots)]
        )
        labels = VGroup(
        ...
        lhs = "(a + bi)(a - bi)"
        rhs = "||a + bi|| \\cdot ||a - bi||"
        eq = Tex(lhs + " = a^2 + b^2 = " + rhs, isolate=[lhs, rhs])
...
class RootOfUnityRearranging(InteractiveScene):
    def construct(self):
        ...
        subtexts = VGroup(
            Tex(
                "\\text{Solutions: } \\zeta^0,\\, \\zeta^1,\\, \\zeta^2,\\, \\zeta^3,\\, \\zeta^4",
                **kw,
            ),
            Tex(
                "\\text{Roots: } \\zeta^0,\\, \\zeta^1,\\, \\zeta^2,\\, \\zeta^3,\\, \\zeta^4",
                **kw,
            ),
        )
        ...
        self.wait()
        self.play(expressions[1].animate.move_to(expressions[2], LEFT))
        self.play(
...
class RotationalSum(InteractiveScene):
    ...
    def construct(self):
        ...
        values = VGroup(*(Integer(0, edge_to_fix=ORIGIN).move_to(point) for point in points))
        ...
        # Add labels
        labels = VGroup(*(OldTexText(f"sum $\\equiv$ {n}\\\\mod 5", font_size=30) for n in range(5)))
        ...
        for n in range(1, self.n_iterations + 1):
            ...
            self.remove(set_tex)
            self.play(
                TransformFromCopy(values, value_copies, path_arc=-path_arc),
                FadeIn(rot_label),
            ...
            self.play(
                ...
                *(MoveToTarget(vc, remover=True) for v, vc in zip(values, value_copies)),
            )
            self.wait()
...
class HighlightAnswerParts(InteractiveScene):
    def construct(self):
        ...
        e1 = "2^{2{,}000}"
        e2 = "4 \\cdot 2^{400}"
        answer = Tex(f"{frac} \\Big({e1} + {e2} \\Big)", isolate=[frac, e1, e2], font_size=60)
        answer.move_to(UP)
...
class FactorLargeInteger(InteractiveScene):
    def construct(self):
        ...
        p, q, N = [Tex("{:,}".format(value).replace(",", "{,}")) for value in (p_value, q_value, p_value * q_value)]
        p.set_color(TEAL)
        q.set_color(BLUE)
        N.to_edge(UP)
...
class Thumbnail(InteractiveScene):
    def construct(self):
        ...
        br = VGroup(
            BackgroundRectangle(question[:2], buff=SMALL_BUFF),
            BackgroundRectangle(question[2], buff=SMALL_BUFF),
        )
        br.set_fill(BLACK, 2)
...
class AltThumbnail(InteractiveScene):
    def construct(self):
        # Organize sets
        subsets = [subset for subset in get_subsets(range(1, 10)) if sum(subset) % 5 == 0]
        n_shown = 28
