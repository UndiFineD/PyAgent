    ...
    def construct(self):
        ...
        for n in range(2, 9):
            ...
            if n < 8:
                ...
                new_low_label = OldTex(Rf"{{\sin(x / {2 * n + 1}) \over x / {2 * n + 1}}}")
...
class SineLimit(InteractiveScene):
    def construct(self):
        ...
        value_label = VGroup(OldTex(R"\frac{\sin(x)}{x} = "), DecimalNumber(1, num_decimal_places=4))
...
class WriteFullIntegrals(InteractiveScene):
    def construct(self):
        # Integrals
        ints = VGroup(*(get_multi_sinc_integral(range(1, n, 2), rhs=R"= \pi") for n in range(3, 19, 2)))
...
class WriteMoreFullIntegrals(InteractiveScene):
    def construct(self):
        ...
        # Integrals
        ints = VGroup(*(get_multi_sinc_integral(range(1, n, 2), rhs=R"= \pi") for n in range(3, 17, 2)))
        ...
        key_map = dict((ints[n][-1].get_tex(), ints[n + 1][-1].get_tex()) for n in range(-4, -1, 1))
...
class WriteOutIntegralsWithPi(WriteOutIntegrals):
    def get_integrals(self):
        ...
        result = VGroup(
            OldTex(R"\int_{-\infty}^\infty", R"\frac{\sin(\pi x)}{\pi x}", R"dx = ", "1.0"),
            OldTex(
                R"\int_{-\infty}^\infty",
...
class InsertTwoCos(InteractiveScene):
    ...
    def construct(self):
        ...
        integrals = VGroup(
            get_multi_sinc_integral([1], rhs=R"= \pi"),
            *(get_multi_sinc_integral(range(1, n, 2), insertion=R"2\cos(x)", rhs=R"= \pi") for n in n_range),
        )
        ...
        areas = VGroup(
            *(
                axes.get_riemann_rectangles(graph, dx=self.dx, colors=(BLUE_D, BLUE_D), fill_opacity=1)
                for graph in graphs
            )
        )
...
class WriteTwoCosPattern(InteractiveScene):
    def construct(self):
        # Integrals
        integrals = VGroup(
            *(get_multi_sinc_integral(range(1, n, 2), insertion=R"2\cos(x)", rhs=R"= \pi") for n in range(1, 7, 2)),
            OldTex(R"\vdots"),
...
class MovingAverages(InteractiveScene):
    ...
    def construct(self):
        ...
        axes = self.get_axes()
        rs = self.rect_scalar
        rect_graph = axes.get_graph(lambda x: rect_func(x / rs), discontinuities=[-0.5 * rs, 0.5 * rs])
        ...
        for n in range(1, self.n_iterations):
            ...
            fn0_label = OldTex(
                f"f_{{{n + 1}}}(0) = " + ("1.0" if n < self.n_iterations - 1 else "0.9999999999852937..."),
                font_size=36,
            )
            ...
            if n < self.n_iterations - 1:
                anims = [FadeOut(mob, shift_value) for mob in [axes, prev_graph, prev_graph_label]]
            ...
            anims.extend(
                [
                    *(mob.animate.shift(shift_value) for mob in [low_axes, graphs[n], fn_label, fn0_label]),
        ...
        graph_groups = VGroup(*(VGroup(graph.axes, graph, graph.func_labels[1]) for graph in graphs[:8]))
        graph_groups.save_state()
        graph_groups.generate_target()
        graph_groups[:-2].set_opacity(0)
        graph_groups.target.arrange_in_grid(4, 2, v_buff=2.0, h_buff=1.0, aligned_edge=LEFT, fill_rows_first=False)
    ...
    def show_moving_average(self, top_axes, low_axes, low_graph, k):
        ...
        v_line = DashedLine(window.get_top(), low_axes.c2p(low_axes.x_axis.p2n(window.get_bottom()), 0))
        ...
        drawing_dot.add_updater(
            lambda m, w=window, g=low_graph.copy(): m.move_to(
                g.quick_point_from_proportion((low_axes.x_axis.p2n(w.get_center()) + rs) / (2 * rs))
            )
        )
...
class MoreGeneralFact(InteractiveScene):
    ...
    def construct(self):
        # Equation
        inters = VGroup(*(get_multi_sinc_integral(range(1, n, 2), rhs=R"= \pi") for n in range(3, 11, 2)))
        inters.scale(0.75)
        inters.to_edge(UP)
        ...
        graphs = VGroup(*(axes.get_graph(lambda x, n=n: multi_sinc(x, n)) for n in range(1, 5)))
        graphs.set_stroke(WHITE, 2)
        areas = VGroup(
            *(
...
class ReplaceXWithPiX(InteractiveScene):
    def construct(self):
        ...
        sinc_pi_label = Tex(R"\int_{-\infty}^\infty \frac{\sin(\pi x)}{\pi x} dx = 1.0", **kw)
...
class WhatWeNeedToShow(InteractiveScene):
    def construct(self):
        ...
        area = axes1.get_riemann_rectangles(axes1.get_graph(np.sinc), colors=(BLUE, BLUE), dx=0.01)
        ...
        self.wait()
        self.play(int_to_eval.animate.set_height(0.5).next_to(ft_sinck, DOWN, MED_LARGE_BUFF, aligned_edge=LEFT))
        ...
        # Many dots
        dx = 0.1
        dots = Group(*(GlowDot(axes2.c2p(x, rect_func(x)), color=TEAL) for x in np.arange(-1, 1 + dx, dx)))
        thick_graph = VGroup(
...
class ConvolutionTheoremDiagram(InteractiveScene):
    def construct(self):
        ...
        width = FRAME_WIDTH / 2 - 1
        height = 1.5
        left_axes = VGroup(*(Axes((-4, 4), (-0.5, 1, 0.5), width=width, height=height) for x in range(3)))
        right_axes = VGroup(*(Axes((-1, 1, 0.5), (0, 1), width=width, height=2 * height / 3) for x in range(3)))
        ...
        for label_group, axes_group, x in (
            (left_labels, left_axes, -2),
            (
                right_labels,
                right_axes,
                -0.85,
    ...
    def construct(self):
        ...
        for num in numbers:
            ...
            num.digit_highlights = VGroup(
                *(VHighlight(digit, color_bounds=(YELLOW, YELLOW_E), max_stroke_addition=8) for digit in num.digits)
            )
