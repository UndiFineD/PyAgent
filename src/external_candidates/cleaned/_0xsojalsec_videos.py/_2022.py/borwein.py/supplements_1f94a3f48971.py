...
class SubPiComment(InteractiveScene):
    def construct(self):
        ...
        self.play(morty.says(OldTex(R"x \rightarrow \pi x", tex_to_color_map={R"\pi": TEAL})))
...
class TranslateToFourierLand(InteractiveScene):
    def construct(self):
        ...
        left_axes = VGroup(*(Axes((0, 12, 1), (-2, 2), **axes_kw).scale(0.5) for x in range(3)))
        ...
        right_axes = VGroup(*(Axes((0, 5), (0, 3), **axes_kw).scale(0.5) for x in range(3)))
