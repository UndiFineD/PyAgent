# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\wordle\scenes.py
...
class WordleScene(Scene):
    ...
    def win_animation(self):
        ...
        self.play(
            LaggedStart(
                *(UpdateFromAlphaFunc(sm, lambda m, a: m.set_y(y + 0.2 * bf(a))) for sm in mover),
                lag_ratio=0.1,
                run_time=1.5,
            ),
            LaggedStart(
                *(Flash(letter, line_length=0.1, flash_radius=0.4) for letter in letters),
                lag_ratio=0.3,
                run_time=1.5,
            ),
        )
...
class WordleSceneWithAnalysis(WordleScene):
    ...
    def get_column_of_numbers(self, values, row_refs, col_ref, num_decimal_places=2, font_size=36):
        mobs = VGroup(
            *(DecimalNumber(value, num_decimal_places=num_decimal_places, font_size=font_size) for value in values)
        )
...
class WordleDistributions(WordleScene):
    ...
    def get_bar_indicator(self, bars, pattern_index):
        ...
        pattern_index_tracker.add_updater(
            lambda m: m.set_value(clip(self.axes.x_axis.p2n(self.mouse_drag_point.get_center()), 0, 3**5))
        )
...
class IntroduceGame(WordleScene):
    ...
    def show_pattern(self, *args, **kwargs):
        ...
        if guess == self.secret_word:
            ...
            added_anims.append(
                LaggedStart(
                    *(Transform(q_mark, letter) for q_mark, letter in zip(self.q_marks, letters)),
                    lag_ratio=0.7,
                    run_time=2,
                )
            )
...
class ButTheyreNotEquallyLikely(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                randy,
                OldTexText("But they're \\emph{not}\\\\equally likely!"),
                target_mode="angry",
            ),
        ...
        self.play(
            PiCreatureSays(
                morty,
                OldTexText("To warm up, let's\\\\assume they are."),
...
class ShowPatternInformationExamples(WordleDistributions):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                randy,
                OldTexText("I thought this\\\\was a word game"),
    ...
    def construct(self):
        ...
        for word in words:
            ...
            self.play(
                ...
                UpdateFromAlphaFunc(ent_rhs, lambda m, a: m.set_value(sum(ent_summands[: int(a * n)]))),
                rate_func=linear,
                run_time=4 * n / 3**5,
            )
            self.wait()
...
class HowThePriorWorks(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeInFromPoint(VGroup(word, bar), col1.get_center()) for word, bar in zip(col2, bars2)),
                lag_ratio=0.1,
                run_time=3,
            )
        )
...
class ShowScoreDistribution(Scene):
    ...
    def construct(self):
        ...
        def update_grid(grid, alpha):
            ...
            try:
                grid.words.set_submobjects(
                    [Text(guess.upper(), font="Consolas") for guess in (*game["guesses"], game["answer"])]
                )
            except Exception:
                return
