# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\wordle\footnote.py
...
class Confessions(Scene):
    def construct(self):
        titles = VGroup(*(Text(f"Confession {n}", font_size=60) for n in range(1, 4)))
        ...
        self.play(LaggedStart(*(FadeIn(title, RIGHT) for title in titles), lag_ratio=0.3))
        self.wait()

        last_phrase = VMobject()
        for title, phrase in zip(titles, phrases):
            others = VGroup(*(t for t in titles if t is not title))
...
class HowWordleColoringWorks(WordleScene):
    def construct(self):
        ...
        self.play(Write(answer_row.target, remover=True), Write(answer_label), run_time=1)
        answer_row.match_style(answer_row.target)
        self.write_word_in_row(
            answer,
            answer_row,
        )
        self.wait()
        ...
        self.play(guess_row.animate.set_fill(BLACK, 0), MoveToTarget(group3))
        ...
        self.play(MoveToTarget(groups, run_time=2), FadeOut(labels))
    ...
    def write_word_in_row(self, word, row, added_anims=[]):
        word_mob = VGroup(*(self.get_letter_in_square(letter, square) for letter, square in zip(word, row)))
        self.play(
...
class SneakAttackInformationTheory(Scene):
    def construct(self):
        ...
        self.play(FadeIn(words[0], scale=2), randy.change("horrified", words), run_time=0.5)
        self.play(
...
class RankingUpdates(Scene):
    def construct(self):
        ...
        v_lines = VGroup(*(Line(UP, DOWN).set_height(FRAME_HEIGHT).set_x(x * FRAME_WIDTH / 6) for x in (-1, 1)))
        ...
        with open(
            os.path.join(
                ...
                "wordle",
                "best_scores_with_entropy_method.json",
            )
        ) as fp:
            rows3 = VGroup()
        ...
        col1, col2, col3 = cols = [get_column(rows, title) for rows, title in zip(all_rows, titles)]
...
class ForgetTheBestWord(Scene):
    def construct(self):
        ...
        self.play(FadeOut(randy.bubble, scale=0.25, shift=2 * DL), randy.change("erm"))
        self.wait()
