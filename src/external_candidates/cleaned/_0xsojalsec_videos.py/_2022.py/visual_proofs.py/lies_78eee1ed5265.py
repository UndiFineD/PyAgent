# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\visual_proofs\lies.py
...
def get_flattened_slices(radius=1.0, n_slices=20, straightened=True):
    ...
    color_slices(north_slices)
    equator = Line(
        north_slices.get_corner(DL),
        north_slices.get_corner(DR),
        **EQUATOR_STYLE,
    )
...
class Intro(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes("pondering", "angry", "confused", look_at=self.screen)
        self.wait()
...
class SphereExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeIn(sh, rate_func=there_and_back) for sh in slice_highlights),
                lag_ratio=0.35,
                run_time=1.5,
            )
        )
        ...
        if self.show_true_slices:
            ...
            curve = ParametricCurve(
                lambda phi: [-math.sin(phi) * dtheta / 2, PI / 2 - phi, 0],
                t_range=(0, PI / 2),
            )
        ...
        vslices = VGroup(
            *(
                VGroup(*(tri_template.copy().rotate(rot).replace(slc, stretch=True) for slc in hemi))
        ...
        self.wait()
        self.play(Write(circ_formula), circ_label.animate.next_to(circ_formula, DOWN))
        self.wait()
        ...
        self.wait()
        self.play(TransformFromCopy(arc_form, arc_form2), FadeOut(q_marks, DOWN))
        self.wait()
...
class Proof2Slide(InteractiveScene):
    def construct(self):
        ...
        v_lines = VGroup(*(DashedLine(ORIGIN, circle.get_height() * UP).move_to(nl.n2p(x), DOWN) for x in range(5)))
        ...
        radial_line.add_updater(
            lambda m: m.set_angle(TAU * (0.75 - get_t())).shift(circle.get_center() - m.get_start())
        )
...
class CircleExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(DrawBorderThenFill(circle), Write(question, stroke_color=WHITE))
        self.wait()
        ...
        self.play(TransformMatchingShapes(VGroup(height_form, width_form).copy(), rhs))
        self.wait()
        self.play(ShowCreation(rect))
...
class SphereSectorAnalysis(Scene):
    ...
    def construct(self):
        ...
        width_label.add_updater(
            lambda m: m.next_to(width_line, OUT, buff=SMALL_BUFF).set_width(1.5 * width_line.get_width() + 1e-2)
        )
        ...
        angle_label[0].add_updater(
            lambda m: m.match_points(get_angle_label()[0]).set_stroke(WHITE, 2).set_fill(opacity=0)
        )
        ...
        self.play(ShowCreation(lat_line_radius), Write(lat_line_label))
        self.wait()
...
class FakeAreaManipulation(InteractiveScene):
    ...
    def construct(self):
        ...
        for target in targets:
            ...
            self.play(
                *[
                    Transform(sm1, sm2, path_arc=path_arc_factors[i] * 60 * DEGREES, **kw)
        ...
        self.play(*(Transform(sm1, sm2, **kw) for sm1, sm2 in zip(group1, targets[0])))
        self.wait()
...
class ContrastSphericalGeometry(InteractiveScene):
    def construct(self):
        ...
        shift_line, new_arcs, new_labels = self.get_aligned_angle_labels(pre_tri, pre_tri_arcs, pre_tri_angle_labels)
        ...
        self.play(
            LaggedStartMap(
                ...
                VGroup(
                    pre_tri,
                    pre_tri_arcs,
                    pre_tri_angle_labels,
                    sph_tri,
                    sph_tri_angle_labels,
                ),
            ),
    ...
    def get_flat_angle_labels(self, tri):
        ...
        for v1, v2, v3 in adjacent_n_tuples(tri.get_vertices(), 3):
            ...
            arc = Arc(start_angle=a2, angle=PI - (a2 - a1) % PI, radius=0.2)
            arc.shift(v2)
            arcs.add(arc)
...
class SquareCircleExample(InteractiveScene):
    def construct(self):
        ...
        circle.set_fill(BLUE_E, 1)
        circle.set_stroke(WHITE, 1)
        approx_curves = [self.get_square_approx(rich_circle, 4 * 2**n) for n in range(10)]
        ...
        self.play(ShowCreation(radial_line), Write(radius_label, stroke_color=WHITE))
        self.wait()

        self.play(Write(question), ShowCreation(circum))
        self.wait()
        self.play(
        ...
        new_radii = VGroup(*(Line(p1, p2).match_style(radial_line) for p1, p2 in adjacent_pairs(points)))
        ...
        for i, curve in enumerate(approx_curves[1:]):
            ...
            self.play(square.animate.set_stroke(width=1), TransformFromCopy(square, curve))
            self.wait()
            if i == 0:
                ...
                self.play(
                    ...
                    FadeOut(sc),
                    FadeOut(cc),
                    VGroup(square, curve).animate.set_stroke(opacity=1),
                )
        ...
        self.play(
            FadeOut(dot_shadows),
            FadeOut(lim_arrow),
            FadeTransform(c_labels[9], c_labels[-1]),
        )
        ...
        lim_len = Tex(
            "\\lim_{n \\to \\infty}\\Big(\\text{len}\\big(c_n(t)\\big) \\Big) = 8",
            tex_to_color_map=t2c,
        )
        len_lim = Tex(
            "\\text{len} \\Big( \\lim_{n \\to \\infty} c_n(t) \\Big) = 2\\pi",
            tex_to_color_map=t2c,
        )
        ...
        self.play(*(TransformFromCopy(lim_len.select_parts(tex), len_lim.select_parts(tex)) for tex in t2c.keys()))
    ...
    def get_square_approx(self, circle, n_samples):
        ...
        points = [radius * np.array([math.cos(a), math.sin(a), 0]) for a in np.linspace(0, TAU, n_samples + 1)]
        result = VMobject()
        result.start_new_path(points[0])
        for p1, p2 in zip(points, points[1:]):
            corners = np.array([[p2[0], p1[1], 0], [p1[0], p2[1], 0]])
...
class UpshotOfLimitExample(InteractiveScene):
    def construct(self):
        ...
        circle_example = Group(
            ...
            VGroup(*TexText("Len = 8").replicate(n), OldTexText("Len = $2\\pi$")),
        )
        fourier_example = Group(
            ...
            VGroup(*TexText("Continuous").replicate(n), OldTexText("Discontinuous")),
        )
...
class IntegralExample(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(ApplyMethod(rect.set_color, YELLOW, rate_func=there_and_back) for rect in rects),
                lag_ratio=0.5,
                run_time=4,
            )
        )
...
class IntegralError(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(ShowCreation(arrow) for arrow in error_arrows),
                lag_ratio=0.2,
                run_time=3,
            ),
        )
        ...
        self.play(
            CountInFrom(rhs[1], 0),
            LaggedStart(
                *(VFadeInThenOut(r.copy().set_fill(opacity=1), buff=0, color=RED) for r in all_error_boxes[0]),
                lag_ratio=0.2,
            ),
            run_time=2,
        )
...
class FalseEuclidProofAnnotation(InteractiveScene):
    def construct(self):
        ...
        # Points
        A = np.array([-1.94444444, 1.44444444, 0.0])
        B = np.array([-4.44444444, -0.02777778, 0.0])
        C = np.array([-1.09722222, -0.48611111, 0.0])
        D = np.array([-2.63888889, -0.27777778, 0.0])
        E = np.array([-1.56944444, 0.55555556, 0.0])
        F = np.array([-3.01388889, 0.83555556, 0.0])
        P = np.array([-2.58333333, 0.122222, 0.0])
...
class FalseEuclidFollowup(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(*(FadeIn(poss[:2], shift=LEFT) for poss in possibilities)),
            Write(poss_title),
        )
        self.wait()
...
class SideSumTruthiness(InteractiveScene):
    def construct(self):
        ...
        E.add_updater(lambda m: m.move_to(interpolate(A.get_center(), C.get_center(), alpha_tracker.get_value())))
