...
class ExampleChannels(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(UpdateFromAlphaFunc(tile, lambda m, a: m.set_opacity(a)) for tile in tiles),
                lag_ratio=0.5,
            ),
            LaggedStart(
...
class IntroScene(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(FadeTransform(init.copy(), two) for init in inits), lag_ratio=0.1),
            MoveToTarget(inits),
        )
        inits.add(two)
        self.wait()
...
class PeerReview(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(AnimationGroup(pi.change("raise_right_hand"), FadeIn(video, 0.5 * UP)) for video, pi in pairs)
            )
        )
...
class HistogramOfViews(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(*(ReplacementTransform(bar.copy().stretch(0, 1, about_edge=DOWN), bar) for bar in bars)),
            lag_ratio=0.2,
        )
        self.add(bars)
...
class OneHundredEntries(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(ReplacementTransform(vid, random.choice(videos)) for vid in (winners[0], *winners[2:]))),
    ...
    def construct(self):
        ...
        self.play(
            ...
            *(FadeIn(regions[i].tiles) for i in [0, 2, 3]),
            regions[1]
            .tiles.animate.scale(regions[0].tiles[0].get_height() / regions[1].tiles[0].get_height())
            .move_to(regions[1]),
    ...
    def construct(self):
        ...
        def left_shift(x, rects=rects, **kw):
            return (
                ...
                rects[1]
                .animate.set_width(rects[1].get_width() - x, about_edge=RIGHT, stretch=True)
                .set_anim_args(**kw),
            )
        ...
        def right_shift(x, rects=rects, **kw):
            return (
                ...
                rects[2]
                .animate.set_width(rects[2].get_width() - x, about_edge=RIGHT, stretch=True)
                .set_anim_args(**kw),
            )
...
class MusicSidenote(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(note.copy().animate.set_opacity(0).scale(0.5).move_to(randy.get_top()) for note in notes),
                lag_ratio=0.2,
            ),
