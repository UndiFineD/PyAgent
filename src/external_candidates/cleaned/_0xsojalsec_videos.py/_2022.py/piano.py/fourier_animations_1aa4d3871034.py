# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\piano\fourier_animations.py
...
def get_wave_sum(axes, freqs, amplitudes=None, phases=None):
    ...
    return axes.get_graph(
        lambda t: sum(amp * math.sin(TAU * freq * (t - phase)) for freq, amp, phase in zip(freqs, amplitudes, phases))
    )
...
class SumOfWaves(Scene):
    def construct(self):
        ...
        axes_labels = VGroup(*(Text(word, font_size=30) for word in ["Air pressure", "Time"]))
        ...
        self.play(LaggedStartMap(Write, axes_labels, lag_ratio=0.8))
        self.wait()
        ...
        low_axes_group = VGroup(*(Axes((0, 12), (-1, 1), height=0.65) for freq in freqs))
        low_axes_group.arrange(UP, buff=0.4)
        low_axes_group.to_edge(DOWN)
        low_axes_group.to_edge(RIGHT)

        waves = VGroup(*(get_wave_sum(la, [freq]) for la, freq in zip(low_axes_group, freqs)))
        waves.set_submobject_colors_by_gradient(BLUE, YELLOW)
        waves.set_stroke(width=2)

        axes_labels = VGroup(*(Text(f"{int(mult * 220)} Hz", font_size=24) for mult in freq_multiples))
        ...
        self.play(
            ...
            *(TransformFromCopy(axes, low_axes) for low_axes in low_axes_group[1:]),
        )
        self.play(
            LaggedStartMap(
                ShowCreation,
        ...
        comp_wave = always_redraw(
            lambda: get_wave_sum(
                top_axes,
                freqs,
                amplitudes=amp_tracker.get_value(),
            ).set_stroke(TEAL, 2)
        )
        ...
        self.play(
            ...
            FadeIn(top_axes),
            FadeIn(sum_label),
            *(Transform(wave.deepcopy(), comp_wave, remover=True) for wave in waves),
        )
        self.add(comp_wave)
        self.wait()
        ...
        for index in range(len(waves)):
            ...
            wave.add_updater(lambda w: w.set_height(amp_tracker.get_value()[w.index] * w.max_height, stretch=True))
...
class DecomposeAudioSegment(Scene):
    ...
    def construct(self):
        ...
        self.zoom_in_on_segment(self.axes, self.graph, self.graph_point, self.zoom_rect_dims)
        self.prepare_for_3d()
    ...
    def break_down_into_fourier_components(self):
        ...
        fft_graph = VMobject()
        fft_graph.set_points_as_corners([f_axes.c2p(freq, 2 * value) for freq, value in zip(freqs, signal_fft_abs)])
        ...
        lines = VGroup(
            *(Line(f_axes.c2p(freqs[i], 0), f_axes.i2gp(freqs[i], fft_graph)) for i in range(1, len(sine_waves)))
        )
        ...
        self.play(
            LaggedStart(
                *(TransformFromCopy(wave, line) for wave, line in zip(sine_waves, lines)),
                lag_ratio=0.1,
                run_time=8,
            ),
        ...
        glow_keys = VGroup(
            *(piano[np.argmin([get_norm(k.get_center() - wave.get_left()) for k in piano])] for wave in top_waves)
        )
        peak_dots = GlowDots(
            [
                lines[np.argmin([get_norm(line.get_start() - wave.get_left()) for line in lines])].get_end()
        ...
        self.play(
            ...
            frame.animate.set_euler_angles(0.72, 1.15).move_to([2.0, 4.0, 1.0]),
...
class WaveformDescription(DecomposeAudioSegment):
    def construct(self):
        ...
        dot = GlowDot()
        dot.add_updater(lambda d: d.move_to(axes.i2gp(axes.x_axis.p2n(line.get_x()), graph)))
        self.add(line, dot)
...
class SignalsAsVectors(DecomposeAudioSegment):
    ...
    def zoom_in_on_waveform(self):
        ...
        axes, graph = self.get_signal_graph()
        self.add(axes, graph)
        axes, graph = self.zoom_in_on_segment(axes, graph, 0.35, (0.2, 4.0), run_time=3, fade_in_new_axes=False)
        axes.set_stroke(opacity=0)
        axes, graph = self.zoom_in_on_segment(axes, graph, 0.5, (0.15, 6.0), run_time=3, fade_in_new_axes=False)
    ...
    def describe_fourier_basis(self):
        ...
        self.play(
            ...
            LaggedStart(
                ...
                *(GrowFromPoint(mob, point) for point, mob in zip(dots.get_points()[:3], vector.get_entries()[:3])),
                Write(vector.get_entries()[3]),
                *(GrowFromPoint(mob, point) for point, mob in zip(dots.get_points()[-2:], vector.get_entries()[-2:])),
            ),
            run_time=3,
        )
        self.wait()
        ...
        fourier_basis = np.array([np.fft.ifft(basis) for basis in np.identity(values.size)])
        component_values = np.array(
            [part * len(signal_fft) for coef, fb in zip(signal_fft, fourier_basis) for part in (fb.real, fb.imag)]
        )
        xs = list(range(len(signal_fft)))
        component_graphs = VGroup(
            *(self.get_graph_with_lines_and_dots(axes, axes.c2p(xs, vals))[0] for vals in component_values)
        )
        ...
        n_parts_shown = 6
        comp_groups = VGroup(*(VGroup(axes.deepcopy(), cg) for cg in component_graphs[2 : 2 + n_parts_shown]))
        ...
        for c_axes, c_graph in comp_groups[:-1]:
            g, c_lines, c_dots = self.get_graph_with_lines_and_dots(c_axes, c_graph.get_points()[::6])
    ...
    def get_graph_with_lines_and_dots(self, axes, points, color=BLUE):
        ...
        lines = VGroup(*(axes.get_v_line(point, line_func=Line) for point in points))
...
class SampleRateOverlay(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(GrowFromPoint(text[1], text[0].get_bottom()), FadeIn(text[2][0], DOWN))
        self.wait()
...
class ThreeDChangeOfBasisExample(Scene):
    def construct(self):
        ...
        # Show components
        components = always_redraw(self.get_linear_combination, signal, basis_mobs)
        ...
        for v in [[-1, 1, 0.5], [1, 1, 0.25], [2, 1, 2]]:
            self.play(signal.animate.put_start_and_end_on(ORIGIN, v), run_time=3)
            self.wait()
        self.wait(5)
    ...
    def get_coefficients(self, vect_mob, basis_mobs):
        cob_inv = np.array([b.get_vector() for b in basis_mobs]).T
    ...
    def get_coef_label(self, vect_mob, basis_mobs, lhs_tex="\\vec{\\textbf{s}}"):
        label = VGroup()
        lhs = OldTex(lhs_tex + "=")
        label.numbers = VGroup(*(DecimalNumber(include_sign=True) for b in basis_mobs))
        ...
        label.add(lhs)
        label.add(*it.chain(*zip(label.numbers, label.basis_labels)))
        label.fix_in_frame()
