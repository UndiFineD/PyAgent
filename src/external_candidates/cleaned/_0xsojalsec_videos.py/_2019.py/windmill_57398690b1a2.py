# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\windmill.py
import json
...
class IntroduceIMO(Scene):
    ...
    def show_students(self):
        ...
        self.play(
            LaggedStart(
                *[ReplacementTransform(student.preimage, student) for student in all_students],
                run_time=2,
                lag_ratio=0.2,
            )
        )
    ...
    def isolate_usa(self):
        ...
        self.play(
            LaggedStart(
                *[FadeOut(flag, DOWN) for flag in random_flags],
                lag_ratio=0.05,
                run_time=1.5,
            ),
    ...
    def get_flags(self):
        ...
        with open(os.path.join("assets", "country_codes.json")) as fp:
            ...
            country_to_code2 = dict([(country.lower(), code2.lower()) for country, code2, code3 in country_codes])
            country_to_code3 = dict([(country.lower(), code3.lower()) for country, code2, code3 in country_codes])
        ...
        for country, count in countries_with_counts:
            ...
            try:
                ...
                image.set_width(1)
                label = VGroup(*[OldTexText(l) for l in country_to_code3[country].upper()])
...
class ShowTest(Scene):
    ...
    def introduce_test(self):
        ...
        for word in proof_words:
            ...
            score.to_edge(RIGHT)
            scores.add(score)
            score[0].add_updater(lambda m: m.set_color(interpolate_color(RED, GREEN, m.get_value() / 7)))
        ...
        self.play(
            LaggedStart(*[FadeIn(word, LEFT) for word in proof_words]),
...
class USProcessAlt(IntroduceIMO):
    ...
    def show_tests(self):
        ...
        for test in tests:
            ...
            if isinstance(test.nq_label[0], Integer):
                ...
                n = int_mob.get_value()
                int_mob.set_value(0)
                self.play(ChangeDecimalToValue(int_mob, n), FadeIn(test.nq_label[1:]))
            else:
    ...
    def show_imo(self):
        ...
        self.play(LaggedStart(*[ApplyMethod(student.change, "hooray") for student in students]))
        for x in range(3):
    ...
    def get_test(self, name_parts, n_questions, time_string, hours, n_students):
        ...
        if n_questions:
            nq_label = VGroup(Integer(n_questions), OldTexText("questions"))
...
class Describe2011IMO(IntroduceIMO):
    ...
    def add_flags_and_students(self):
        ...
        all_students = VGroup(
            *it.chain(*[[student for student in group if student.get_fill_opacity() > 0] for group in student_groups])
        )
        ...
        flag_counter = VGroup(Integer(0), OldTexText("Countries"))
    ...
    def comment_on_primality(self):
        ...
        lines = VGroup(*[Line().match_width(number).next_to(number, DOWN, SMALL_BUFF) for number in numbers])
        ...
        words = VGroup(*[OldTexText("Prime").next_to(line, DOWN) for line in reversed(lines)])
        ...
        self.play(
            randy.change,
            "thinking",
            LaggedStart(*[FadeIn(word, UP) for word in words], run_time=3, lag_ratio=0.5),
        )
        self.play(Blink(randy))
        self.play(
            FadeOut(randy),
    ...
    def show_top_three_scorers(self):
        ...
        students_to_fade = VGroup(*filter(lambda s: s is not student, all_students))
        ...
        self.wait()
        self.play(
            LaggedStart(*[FadeIn(row, UP) for row in grid.rows[2:4]]),
    ...
    def get_score_grid(self):
        ...
        students = VGroup(*[OldTexText(row[1]) for row in data])
        flags = Group(*[ImageMobject("flags/{}.png".format(row[2])).set_height(0.3) for row in data])
        students = Group(
            *[Group(flag.next_to(student, LEFT, buff=0.2), student) for flag, student in zip(flags, students)]
        )
        ...
        titles = VGroup(
            *[
                VectorizedPoint(),
                VectorizedPoint(),
                *[OldTexText("P{}".format(i)) for i in range(1, 7)],
            ]
        )
        ...
        rows = Group(
            ...
            *[
                ...
                for rank, flag, student, score_row in zip(ranks, flags, students, score_rows)
            ],
        )
        rows.arrange(DOWN)
        rows.to_edge(UP)
...
class AskWhatsOnTest(ShowTest, MovingCameraScene):
    ...
    def ask_about_questions(self):
        ...
        for arrow in arrows.target:
            ...
            arrow.put_start_and_end_on(interpolate(question.get_left(), start, 0.1), end)
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                VGroup(research, *arrows, *self.problem_rects, self.time_labels[0]),
            ),
        )
...
class WindmillScene(Scene):
    ...
    def next_pivot_and_angle(self, windmill):
        ...
        non_pivots = list(filter(lambda p: not np.all(p == pivot), windmill.point_set))

        angles = np.array([-(angle_of_vector(point - pivot) - curr_angle) % PI for point in non_pivots])
    ...
    def get_orientation_arrows(self, windmill, n_tips=20):
        ...
        for tip, a in zip(tips, np.linspace(0, 1, n_tips)):
            tip.shift(windmill.point_from_proportion(a) - tip.get_points()[0])
        return tips
...
class IntroduceWindmill(WindmillScene):
    ...
    def add_line(self):
        ...
        arcs = VGroup(*[Arc(angle=-45 * DEGREES, radius=1.5) for x in range(2)])
...
class ContrastToOtherOlympiadProblems(AskWhatsOnTest):
    ...
    def zoom_to_other_questions(self):
        ...
        self.wait()
        for i in [2, 0, 3, 5]:
            self.play(frame.move_to, rects[i], Transform(big_rect, big_rects[i]))
            self.wait()
...
class TheQuestion(Scene):
    def construct(self):
        words = OldTexText("Will each point be hit infinitely many times?")
...
class SpiritOfIMO(PiCreatureScene):
    def construct(self):
        ...
        problems = VGroup(*[OldTexText("P{})".format(i)) for i in range(1, 7)])
        ...
        self.play(
            ...
            LaggedStartMap(LaggedStartMap, lights, lambda l: (VFadeInThenOut, l), run_time=3),
            randy.change,
            "thinking",
        )
        self.wait()
...
class PerfectScoreData(Describe2011IMO):
    ...
    def add_subtitle(self):
        ...
        subtitle = OldTexText("Number of perfect scores on each problem:")
        subtitle.scale(1.25)
    ...
    def analyze_data(self):
        ...
        rects = VGroup(*[SurroundingRectangle(p, color=p[1].get_color()) for p in problems])
    ...
    def get_bar(self, number, left_side):
        ...
        bar.set_height(0.25)
        bar.set_width(self.full_bar_width * number / self.n_students, stretch=True)
    ...
    def get_bar_counter(self, bar):
        counter = Integer()
        counter.add_updater(lambda m: m.set_value(self.n_students * bar.get_width() / self.full_bar_width))
...
class SixOnSix(Describe2011IMO):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ReplacementTransform(six_rect, two_rect), FadeIn(subtitle, UP))
        self.wait()
...
class TryOutSimplestExamples(WindmillScene):
    ...
    def move_starting_line(self):
        ...
        self.play(LaggedStart(*[FadeIn(counter, DOWN) for counter in counters]))
        self.wait()
...
class FearedCase(WindmillScene):
    ...
    def construct(self):
        ...
        pivot_dot = self.get_pivot_dot(windmill)
        # self.add_dot_color_updater(dots, windmill)
        counters = self.get_pivot_counters(windmill, counter_height=0.15, buff=0.1)
...
class WhereItStartsItEnds(WindmillScene):
    ...
    def show_stays_in_middle(self):
        ...
        windmill = self.get_windmill(points, sorted_points[self.n_points // 2], angle=0)
        pivot_dot = self.get_pivot_dot(windmill)
    ...
    def get_middle_rects(self):
        middle_rects = VGroup(*[SurroundingRectangle(words[1]) for words in [self.start_words, self.end_words]])
...
class FormalizeMiddle(WhereItStartsItEnds):
    ...
    def problem_solving_tip(self):
        ...
        self.play(FadeOut(pst), MoveToTarget(steps), Restore(mid_words), FadeOut(arrows))
        self.wait()
    ...
    def mention_odd_case(self):
        ...
        sf = 1.0
        words = OldTexText("Assume odd \\# points")
        words.scale(sf)
        words.to_corner(UL)
        example = VGroup(OldTexText("Example:"), Integer(0))
        ...
        self.play(
            ...
            ShowCreationThenFadeOut(SurroundingRectangle(counters[0], color=BLUE)),
        )
        self.play(
            ShowCreationThenFadeOut(dot_rects[-k:]),
            ShowCreationThenFadeOut(SurroundingRectangle(counters[1], color=GREY_BROWN)),
        )
        self.wait()
        self.play(
            FadeOut(words),
...
class SecondColoringExample(WindmillScene):
    ...
    def construct(self):
        ...
        dots = self.get_dots(points)
        windmill = self.get_windmill(points, pivot=sorted_points[self.n_points // 2], angle=PI / 2)
        pivot_dot = self.get_pivot_dot(windmill)
...
class TalkThroughPivotChange(WindmillScene):
    ...
    def change_pivot(self):
        ...
        old_pivot_word.next_to(old_pivot, LEFT, buff=SMALL_BUFF + MED_SMALL_BUFF)
...
class InsightNumber1(Scene):
    def construct(self):
        words = OldTexText("Key insight 1: ", "\\# Points on either side is constant")
...
class Rotate180Argument(WindmillScene):
    ...
    def show_parallel_lines(self):
        ...
        for rect in self.rects:
            self.play(rect.set_opacity, 0, rate_func=there_and_back, run_time=2)
...
class EvenCase(Rotate180Argument):
    ...
    def ask_about_even_number(self):
        ...
        blues_counter.add_updater(
            lambda c: c.set_value(len(list(filter(lambda d: d.get_fill_color() == Color(BLUE), dots))))
        )
        browns_counter.add_updater(
            lambda c: c.set_value(len(list(filter(lambda d: d.get_fill_color() == Color(GREY_BROWN), dots))))
        )
...
class EasyToFoolYourself(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.pi_creature_says("Isn't it\\\\obvious?", target_mode="maybe", added_anims=[FadeOut(bubble)])
        self.wait(4)
...
class FailureToEmpathize(PiCreatureScene):
    def construct(self):
        ...
        equation = OldTex("\\sum_{k=1}^n (2k - 1) = n^2")
        self.pi_creature_thinks(
            randy,
            equation,
...
class DifficultyEstimateVsReality(Scene):
    def construct(self):
        ...
        x_labels = VGroup(*[OldTexText("Q{}".format(i)).next_to(bp, DOWN) for i, bp in zip(it.count(1), bar_pairs)])
...
class KeepInMindWhenTeaching(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("I don't know\\\\what you know!", target_mode="pleading")
        self.wait(2)
        self.play(
            PiCreatureSays(
                self.students[0],
...
class VastSpaceOfConsiderations(Scene):
    def construct(self):
        ...
        self.play(LaggedStart(*[FadeIn(mob, UP) for mob in considerations], run_time=3, lag_ratio=0.2))
...
class LorenzTransform(Scene):
    def construct(self):
        ...
        equation = OldTex("d\\tau^2 = dt^2 - dx^2")
        equation.scale(1.7)
...
class TerryTaoQuote(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(tiny_tao, LEFT))
        self.wait()
        self.play(FadeOut(tiny_tao))

        #
        self.play(FadeIn(quote, lag_ratio=0.05, run_time=5, rate_func=bezier([0, 0, 1, 1])))
        self.wait()
        story_line = Line()
...
class SolveAProblemOneDay(SpiritOfIMO, PiCreatureScene):
    def construct(self):
        ...
        self.play(FadeIn(you, LEFT), GrowArrow(arrow))
        self.wait(2)
        self.play(
    ...
    def construct(self):
        ...
        if self.animate:
            ...
            self.play(
                ...
                FRAME_WIDTH * RIGHT,
                LaggedStart(
                    *[ApplyMethod(mob.scale, 0) for mob in [sorted_dots[6], *flash.mobject, windmill, pivot_dot]]
                ),
                LaggedStart(*[ApplyMethod(dot.to_edge, LEFT, {"buff": -1}) for dot in sorted_dots[:6]]),
                LaggedStart(*[ApplyMethod(dot.to_edge, RIGHT, {"buff": -1}) for dot in sorted_dots[7:]]),
                LaggedStart(*[FadeOut(word, RIGHT) for word in words]),
                LaggedStart(*[FadeOut(word, LEFT) for word in words2]),
                LaggedStartMap(
                    Uncreate,
