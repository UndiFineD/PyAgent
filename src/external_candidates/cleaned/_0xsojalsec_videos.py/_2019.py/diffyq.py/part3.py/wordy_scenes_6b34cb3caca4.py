# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part3\wordy_scenes.py
...
class ThreeMainObservations(Scene):
    def construct(self):
        ...
        observations = VGroup(
            ...
            OldTexText("2)", "Linearity"),
            OldTexText("3)", "Fourier series"),
        )
        ...
        self.play(
            LaggedStart(
                *[TransformFromCopy(bubble, observation[0]) for observation in observations],
                lag_ratio=0.2,
            )
        )
        self.play(
...
class ThreeConstraints(WriteHeatEquationTemplate):
    ...
    def show_three_conditions(self):
        ...
        title = OldTex(
            "\\text{Constraints }T({x}, {t})\\text{ must satisfy:}",
...
class SeekIdealized(Scene):
    def construct(self):
        ...
        models.shift(words[0].target.get_center() - words[0].saved_state.get_center())
        self.play(MoveToTarget(words[0]), ShowCreation(low_arrow), FadeIn(models, LEFT))
        self.wait()
...
class EquationAboveSineAnalysis(WriteHeatEquationTemplate):
    def construct(self):
        ...
        t_terms.save_state()
        zeros = VGroup(*[OldTex("0").replace(t, dim_to_match=1) for t in t_terms])
        zeros.align_to(t_terms, DOWN)
        new_rhs = OldTex("=", "-\\alpha \\cdot {T}", "({x}, 0)", **self.tex_mobject_config)
...
class ShowSinExpDerivatives(WriteHeatEquationTemplate):
    ...
    def construct(self):
        ...
        equation1 = OldTex("T({x}, {0}) = \\sin\\left({x}\\right)", **self.tex_mobject_config)
        equation2 = OldTex(
        ...
        self.add(equation1)
        self.wait()
        self.play(MoveToTarget(pde), FadeIn(new_rhs, LEFT))
        self.wait()
        self.play(
        ...
        dx = OldTex("\\cos\\left({x}\\right)", "e^{-\\alpha {t}}", **self.tex_mobject_config)
        ddx = OldTex("-\\sin\\left({x}\\right)", "e^{-\\alpha {t}}", **self.tex_mobject_config)
        ...
        self.play(
            ApplyMethod(minus_alpha_in_exp.replace, minus_alpha, path_arc=TAU / 4),
            FadeIn(dot),
        )
        self.play(
            FadeIn(minus_alpha),
        ...
        self.play(FadeInFromDown(checkmark), FadeOut(q_mark, UP))
        self.wait()
...
class DerivativesOfLinearFunction(WriteHeatEquationTemplate):
    ...
    def construct(self):
        func = OldTex("T({x}, {t}) = {c} \\cdot {x}", **self.tex_mobject_config)
        ...
        for arrow, char, vect in zip(arrows, "xxt", [UP, UP, RIGHT]):
            label = OldTex("\\partial \\over \\partial {%s}" % char, **self.tex_mobject_config)
            label.scale(0.7)
            label.next_to(arrow.get_center(), vect)
            arrow.add(label)

        dt_arrow.shift(
            func.target[-3:].get_bottom() + MED_SMALL_BUFF * DOWN - dt_arrow.get_start(),
        )
...
class WorryAboutGenerality(TeacherStudentsScene, WriteHeatEquationTemplate):
    def construct(self):
        ...
        self.play(
            self.change_students("sad", "tired", "pleading"),
            self.teacher.change,
            "raise_right_hand",
            FadeInFromDown(eq),
        )
    ...
    def get_diffyq_set(self):
        words = OldTexText("Differential\\\\equations")
        words.scale(1.5)
