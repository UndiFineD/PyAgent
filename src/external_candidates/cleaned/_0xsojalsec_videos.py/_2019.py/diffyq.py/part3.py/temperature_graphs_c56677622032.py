# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part3\temperature_graphs.py
...
class SimpleCosExpGraph(TemperatureGraphScene):
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                cos_exp_surface,
                lambda m, a: m.become(self.get_cos_exp_surface(axes, v_max=a * 10)),
                run_time=3,
            )
        )
        ...
        cos_graph.add_updater(
            lambda m: m.become(self.get_time_slice_graph(axes, lambda x: self.cos_exp(x, get_t()), t=get_t()))
        )
    ...
    def construct(self):
        ...
        for axes, As, omegas, ks in quads:
            graph = self.get_initial_state_graph(
                axes,
                lambda x: np.sum([self.cos_exp(x, 0, A, omega, k) for A, omega, k in zip(As, omegas, ks)]),
            )
            surface = self.get_surface(
                axes,
                lambda x, t: np.sum([self.cos_exp(x, t, A, omega, k) for A, omega, k in zip(As, omegas, ks)]),
            )
        ...
        self.play(
            Transform(axes1.copy().set_fill(opacity=0), axes3),
...
class BreakDownAFunction(SimpleCosExpGraph):
    CONFIG = {
        ...
        "low_axes_config": {"z_min": -3, "z_axis_config": {"unit_size": 1}},
        "n_low_axes": 4,
        "k": 0.2,
    }
    ...
    def setup_graphs(self):
        ...
        low_surfaces = VGroup(
            *[
                self.get_surface(
                    axes,
                    lambda x, t: np.prod([A, np.cos(n * x / 2), np.exp(-k * (n / 2) ** 2 * t)]),
                )
                for n, axes, A in zip(it.count(), low_axes_group, fourier_terms)
            ]
        )
        top_surface = self.get_surface(
            ...
            lambda x, t: np.sum(
                [
                    np.prod([A, np.cos(n * x / 2), np.exp(-k * (n / 2) ** 2 * t)])
    ...
    def show_break_down(self):
        ...
        plusses = VGroup(
            *[OldTex("+").next_to(axes.x_axis.get_end(), RIGHT, MED_SMALL_BUFF) for axes in low_axes_group]
        )
        dots = OldTex("\\cdots")
        ...
        self.play(
            ...
            *[TransformFromCopy(top_graph, low_graph) for low_graph in low_graphs],
        )
    ...
    def show_solutions_for_waves(self):
        ...
        low_checkmarks = VGroup(
            *[checkmark.copy().next_to(surface.get_top(), UP, SMALL_BUFF) for surface in low_surfaces]
        )
...
class OceanOfPossibilities(TemperatureGraphScene):
    ...
    def setup_surface(self):
        ...
        def func(x, t):
            return np.sum(
                [np.prod([A * np.cos(omega * x), np.exp(-k * omega**2 * t)]) for A, omega in zip(get_As(), omegas)]
            )
    ...
    def show_solution(self):
        ...
        plane = always_redraw(
            lambda: Polygon(
                *[axes.c2p(x, get_t(), T) for x, T in [(0, 0), (TAU, 0), (TAU, 4), (0, 4)]],
                stroke_width=0,
                fill_color=WHITE,
    ...
    def reference_boundary_conditions(self):
        ...
        surface_boundary_lines = always_redraw(
            lambda: VGroup(
                *[
                    ParametricCurve(
                        lambda t: axes.c2p(x, t, self.surface_func(x, t)),
                        t_max=axes.y_max,
        ...
        self.move_camera(
            ...
            added_anims=[
                ...
                LaggedStartMap(FadeInFrom, words, lambda m: (m, IN)),
                FadeOut(t_numbers),
            ],
        )
...
class AnalyzeSineCurve(TemperatureGraphScene):
    ...
    def show_curvature_matching_height(self):
        ...
        vector, x_tracker = self.get_v_line_with_x_tracker(
            d2_graph, line_creator=lambda p1, p2: Arrow(p1, p2, color=RED, buff=0)
        )
    ...
    def show_time_step_scalings(self):
        ...
        self.play(
            ReplacementTransform(graph_label[i + 3 :], new_label[i + 4 :]),
    ...
    def get_v_line_with_x_tracker(self, graph, line_creator=DashedLine):
        ...
        v_line = always_redraw(
            lambda: line_creator(
                axes.c2p(get_x(), 0, 0),
                graph.point_from_proportion(inverse_interpolate(x_min, x_max, get_x())),
            )
        )
...
class SimulateRealSineCurve(ShowEvolvingTempGraphWithArrows):
    ...
    def add_labels_to_axes(self):
        ...
        x_axis.add(
            *[
                OldTex(tex).scale(0.5).next_to(x_axis.n2p(n), DOWN, buff=MED_SMALL_BUFF)
                for tex, n in [
                    ("\\tau \\over 4", TAU / 4),
                    ("\\tau \\over 2", TAU / 2),
                    ("3 \\tau \\over 4", 3 * TAU / 4),
...
class SlopeToHeatFlow(FlatEdgesContinuousEvolution):
    ...
    def show_heat_flow(self):
        ...
        self.wait()
        self.play(
            LaggedStart(*[TransformFromCopy(sw[0], fw[0]) for sw, fw in zip(slope_words, flow_words)]),
    ...
    def get_flow(self, x, flow_rate):
        return VGroup(
            *[self.get_single_flow_arrow(x, flow_rate, t_offset=to) for to in np.linspace(0, 5, self.n_arrows)]
        )
    ...
    def get_single_flow_arrow(self, x, flow_rate, t_offset):
        ...
        animation = UpdateFromAlphaFunc(
            arrow,
            lambda m, a: m.move_to(interpolate(lp, rp, a))
            .set_color(interpolate_color(lc, rc, a))
...
class ManipulateSinExpSurface(TemperatureGraphScene):
    ...
    def get_graph(self):
        return always_redraw(
            lambda: self.get_time_slice_graph(self.axes, self.func, t=self.t_tracker.get_value(), **self.graph_config)
        )
