# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part4\fourier_series_scenes.py
...
class ComplexFourierSeriesExample(FourierOfTrebleClef):
    ...
    def transition_to_alt_path(self, new_path, morph_path=False):
        ...
        for vect, new_vect in zip(vectors, new_vectors):
            ...
            anims.append(ApplyMethod(line.put_start_and_end_on, *new_vect.get_start_and_end()))
            vect.freq = new_vect.freq
            vect.coefficient = new_vect.coefficient

            vect.line = line
            vect.add_updater(lambda v: v.put_start_and_end_on(*v.line.get_start_and_end()))
        if morph_path:
    ...
    def get_top_row_labels(self, vector_copies):
        ...
        for vector_copy in vector_copies:
            ...
            label = Integer(freq)
            label.move_to(np.array([freq * self.top_row_x_spacing, self.top_row_label_y, 0]))
            labels.add(label)
        return labels
    ...
    def focus_on_vector_pair(self):
        ...
        def get_opacity_animation(i1, i2, alpha_func):
            ...
            return AnimationGroup(
                UpdateFromAlphaFunc(VectorizedPoint(), lambda m, a: v_group.set_opacity(alpha_func(a))),
                UpdateFromAlphaFunc(
                    VectorizedPoint(),
...
class DemonstrateAddingArrows(PiFourierSeries):
    ...
    def construct(self):
        ...
        vectors = VGroup(
            *[Vector(**self.vector_config).put_start_and_end_on(*v.get_start_and_end()) for v in original_vectors]
        )
        original_top_vectors = self.top_row[0]
        top_vectors = VGroup(
            *[Vector(**self.vector_config).put_start_and_end_on(*v.get_start_and_end()) for v in original_top_vectors]
        )
        ...
        self.remove(self.path, self.drawn_path)
        anims1 = [TransformFromCopy(tv, v) for tv, v in zip(top_vectors, vectors)]
...
class LabelRotatingVectors(PiFourierSeries):
    ...
    def setup_top_row(self):
        vectors = self.get_rotating_vectors(coefficients=0.5 * np.ones(self.n_vectors))
    ...
    def ask_about_labels(self):
        ...
        q_marks = VGroup(*[OldTex("??").scale(1.0).next_to(circle, DOWN) for circle in circles])
    ...
    def show_complex_exponents(self):
        ...
        self.play(FadeOut(self.zero_vect), FadeOut(self.zero_circle), FadeIn(v1_rect))
        ...
        arclen_decimal.add_updater(lambda m: m.set_value(arclen_tracker.get_value()))
        ...
        self.play(ReplacementTransform(vgm1_copy, vgm1), FadeInFromDown(cps_m1))
        self.wait(2)
        self.play(
        ...
        self.play(ShowCreation(circle_copy), arclen_tracker.set_value, 2 * TAU, run_time=5)
        self.wait(3)
        self.play(
            FadeOut(circle_copy),
            FadeOut(arclen_decimal),
        ...
        self.play(FadeIn(f_exp_general, UP))
    ...
    def tweak_initial_states(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(VectorizedPoint(), lambda m, a: update_vectors(a)),
            run_time=2,
        )
        self.wait()
        self.play(
    ...
    def constant_examples(self):
        ...
        def update_v1(alpha):
            vectors[1].coefficient = 0.5 * interpolate(
                np.exp(complex(0, PI / 4)), 0.3 * np.exp(complex(0, PI / 4)), alpha
            )
    ...
    def get_vector_in_plane_group(self, top_vector, top_circle):
        ...
        vector = Vector()
        vector.add_updater(
            lambda v: v.put_start_and_end_on(origin, plane.n2p(2 * top_vector.coefficient)).set_angle(
    ...
    def get_cn_label(self, n, exp_label):
        ...
        term.scale(0.9)
        term.shift(exp_label.target[0].get_corner(LEFT) - term[0].get_corner(RIGHT) + 0.2 * LEFT)
...
class IntegralTrick(LabelRotatingVectors, TRangingFrom0To1):
    ...
    def introduce_sum(self):
        ...
        input_tracker.add_updater(lambda m: m.set_value(self.vector_clock.get_value() % 1))
        self.add(
            self.vector_clock,
            self.vectors,
            self.circles,
    ...
    def issolate_c0(self):
        ...
        for vect in vectors[1:]:
            vect.add_updater(lambda v: v.set_opacity(opacity_tracker.get_value()))
        for circle in circles[0:]:
            circle.add_updater(lambda c: c.set_stroke(opacity=opacity_tracker.get_value()))

        self.play(ShowCreation(c0_rect))
        self.play(opacity_tracker.set_value, 0.2, FadeOut(self.drawn_path), FadeIn(path))
    ...
    def show_center_of_mass(self):
        ...
        self.wait()
        self.play(*[Transform(od.copy(), v0_dot.copy(), remover=True) for od in output_dots])
        self.add(v0_dot)
        self.wait()
        ...
        for ds1, ds2 in zip(dot_sets, dot_sets[1:]):
            ...
            new_v0_dot.move_to(center_of_mass([od.get_center() for od in outd2]))
            self.play(
                FadeOut(ind1),
                LaggedStartMap(
                    FadeInFrom,
            ...
            self.play(
                FadeOut(v0_dot),
                *[Transform(od.copy(), v0_dot.copy(), remover=True) for od in outd2],
            )
            v0_dot = new_v0_dot
            self.add(v0_dot)
        self.wait()
...
class IncreaseOrderOfApproximation(ComplexFourierSeriesExample):
    ...
    def construct(self):
        ...
        n_label.add_updater(lambda n: n[1].set_value(n_tracker.get_value()).align_to(n[2], DOWN))
...
class ShowStepFunctionIn2dView(SimpleComplexExponentExample, ComplexFourierSeriesExample):
    ...
    def show_fourier_series(self):
        ...
        output_tip.add_updater(lambda m: m.move_to(self.vectors[-1].get_end(), DOWN))
...
class AddVectorsOneByOne(IntegralTrick):
    ...
    def show_sum(self):
        ...
        def update_vector(vector):
            vector.coefficient = interpolate(1, vector.original_coefficient, coef_tracker.get_value())
        ...
        self.input_tracker.add_updater(lambda m: m.set_value(vector_clock.get_value() % 1))
