# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part4\staging.py
...
class RelationToOtherVideos(Scene):
    ...
    def construct(self):
        ...
        self.play(MoveToTarget(group), FadeIn(fourier, LEFT))
    ...
    def get_video_thumbnails(self):
        ...
        for thumbnail in thumbnails:
            ...
            thumbnail.add(SurroundingRectangle(thumbnail, color=WHITE, stroke_width=2, buff=0))
...
class ShowInfiniteSum(FourierSeriesIllustraiton):
    ...
    def show_to_infinity(self):
        ...
        self.play(FadeIn(words, LEFT), GrowArrow(arrow))
        self.wait()
    ...
    def walk_through_constants(self):
        ...
        rects = VGroup(*[SurroundingRectangle(term, color=term[-1].get_color(), stroke_width=2) for term in terms])
        ...
        self.play(
            ...
            LaggedStart(
                *[
                    UpdateFromAlphaFunc(
                        ps,
                        lambda m, a: m.set_stroke(width=(3 * there_and_back(a) + 0.5 * a)),
                    )
    ...
    def ask_about_infinite_sum(self):
        ...
        question = OldTexText("What$\\dots$\\\\ does this mean?")
        question.scale(0.7)
    ...
    def show_inf_sum_of_numbers(self):
        ...
        number_line.add_numbers(*np.arange(0, 1.5, 0.5), num_decimal_places=1)
        ...
        n_braces = 7
        braces = VGroup(*[Brace(num_inf_sum[:n], DOWN) for n in range(1, n_braces + 1)])
        brace = braces[0].copy()
        ...
        term_count = VGroup(Integer(1), OldTexText("terms"))
        term_count_tracker = ValueTracker(1)
        term_count[0].set_color(YELLOW)
        term_count.add_updater(lambda m: m[0].set_value(term_count_tracker.get_value()))
        ...
        pi_fourths_label = OldTex("{\\pi \\over 4} =")
        ...
        new_marks = VGroup(*[self.leave_mark(number_line, ps) for ps in partial_sums[num_moving_values:]])
    ...
    def show_many_inputs_in_parallel(self):
        ...
        values = np.array(
            [
                [(4 / PI) * (u / n) * np.cos(n * PI * x) for x in inputs]
                for u, n in zip(
                    it.cycle([1, -1]),
    ...
    def follow_single_inputs(self):
        ...
        def get_brace_value_label(brace, u, n):
            ...
            result.add_updater(lambda d: d.set_value((u / n) * np.cos(PI * n * get_input())))
            return result
        ...
        n_waves_label = OldTexText("Sum of", "10", "waves")
...
class TechnicalNuances(Scene):
    def construct(self):
        ...
        for question in questions:
            ...
            self.play(FadeInFromDown(question), FadeOut(last_question, UP))
            self.wait(2)
...
class AskAboutComplexNotVector(Scene):
    def construct(self):
        ...
        for q, ex in zip(questions, group):
            self.play(FadeIn(q, LEFT), FadeIn(ex))
        self.wait()
...
class SwapIntegralAndSum(Scene):
    ...
    def perform_swap(self):
        ...
        int_ft = OldTex("\\int_0^1 f({t}) {dt}", **tex_config)
        int_sum = OldTex(
            """
            =
            \\int_0^1 \\left(
        ...
        breakdown_label = OldTexText("Break this down")
        arrow = Vector(LEFT)
    ...
    def show_average_of_individual_terms(self):
        ...
        braces = VGroup(*[Brace(term, DOWN, buff=SMALL_BUFF) for term in sum_int.int_terms])
    ...
    def show_individual_average(self, brace, n, cn_tex=None):
        ...
        vector = Arrow(
            plane.n2p(0),
            plane.n2p((0.2 + 0.8 * np.random.random()) * np.exp(complex(0, TAU * np.random.random()))),
            buff=0,
            color=WHITE,
        )
        circle = Circle()
        ...
        self.play(*[ApplyMethod(dot.move_to, dot_center) for dot in dot_copies])
...
class ShowRangeOfCnFormulas(Scene):
    def construct(self):
        formulas = VGroup(*[self.get_formula(n) for n in [-50, None, -1, 0, 1, None, 50]])
        formulas.scale(0.7)
...
class DescribeSVG(Scene):
    def construct(self):
        ...
        z_decimal.add_updater(lambda m: m.set_value(plane.p2n(dot.get_center())))
...
class NumericalIntegration(Scene):
    ...
    def add_integral(self):
        integral = OldTex(
            "c_{n}",
            "=",
            "\\int_0^1 f({t})e^{-{n} \\cdot 2\\pi i {t}}{dt}",
            **self.tex_config,
        )
        ...
        sum_tex = OldTex(
            " \\approx \\text{sum([} \\dots",
            "f({0.01}) e^{-{n} \\cdot 2\\pi i ({0.01})}({0.01})\\dots \\text{])}",
            **self.tex_config,
        )
        sum_tex.next_to(
...
class StepFunctionIntegral(Scene):
    def construct(self):
        ...
        words1 = OldTex(
            "\\text{Challenge 1: Show that }c_{n} = {2 \\over {n} \\pi i}",
            "\\text{ for odd }",
            "{n}",
            "\\text{ and 0 otherwise}",
            **tex_config,
        ...
        words2 = OldTex(
            ...
            "&\\text{Using }",
            "\\sin(x) = (e^{ix} - e^{-ix}) / 2i,",
            "\\text{ show that}\\\\&\\text{step}({t}) = \\sum_{n = -\\infty}^{\\infty}",
...
class ExponentialsMoreBroadly(Scene):
    ...
    def write_fourier_series(self):
        formula = OldTex(
            ...
            "c_{n}",
            "e^{{n} \\cdot 2\\pi i {t}}",
            tex_to_color_map={"{t}": LIGHT_PINK, "{n}": YELLOW},
        )
        ...
        self.play(formula.scale, 0.7, formula.to_corner, UL, ShowCreation(plane))
    ...
    def show_matrix_exponent(self):
        ...
        m_exp = OldTex(
            "\\text{exp}\\left(\\left[ \\begin{array}{cc}0 & -1 \\\\ 1 & 0 \\end{array} \\right]",
            "{t} \\right)",
            "=",
        ...
        self.play(FadeInFromDown(m_exp), FadeOut(formula, UP), FadeOut(c_exp))
