# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part1\phase_space.py
...
class VisualizeStates(Scene):
    ...
    def initialize_plane(self):
        ...
        plane.axis_labels = VGroup(
            plane.get_x_axis_label("\\theta", RIGHT, UL, buff=SMALL_BUFF),
            plane.get_y_axis_label("\\dot \\theta", UP, DR, buff=SMALL_BUFF).set_color(YELLOW),
        )
    ...
    def show_grid_of_states_creation(self):
        ...
        state_grid.set_height(FRAME_HEIGHT - title.get_height() - 2 * MED_SMALL_BUFF)
        ...
        right_column_copy = VGroup(*[row[-1] for row in state_grid]).deepcopy()
        ...
        self.play(ReplacementTransform(right_column_copy[-1], state_grid[-1][-1], remover=True))
        self.wait()
    ...
    def collapse_grid_into_points(self):
        ...
        self.play(
            ...
            LaggedStart(
                *[TransformFromCopy(m1, m2) for m1, m2 in zip(flat_state_group, flat_dot_group)],
                lag_ratio=0.1,
                run_time=4,
            ),
        )
    ...
    def show_state_with_pair_of_numbers(self):
        ...
        h_line = always_redraw(lambda: self.get_tracking_h_line(dot.get_center()))
        v_line = always_redraw(lambda: self.get_tracking_v_line(dot.get_center()))
        ...
        for line, label in zip([h_line, v_line], axis_labels):
            # self.add(line, dot)
            self.play(ShowCreation(line), ShowCreationThenFadeAround(label), run_time=1)
    ...
    def show_acceleration_dependence(self):
        ...
        ode.move_to(
            FRAME_WIDTH * RIGHT / 4 + FRAME_HEIGHT * UP / 4,
        )
    ...
    def show_evolution_from_a_start_state(self):
        ...
        index = np.argmin([trajectory.point_from_proportion(a)[1] for a in alphas])
        alpha = alphas[index]
        ...
        self.play(
            ...
            UpdateFromFunc(dot, lambda d: d.move_to(sub_traj.get_end())),
            run_time=6,
        )
        self.wait()
        ...
        self.play(ShowCreationThenFadeAround(td_label), ShowCreationThenFadeOut(y_axis_copy))
    ...
    def get_state_point(self, pendulum):
        return self.plane.coords_to_point(*self.get_state(pendulum))
    ...
    def get_flexible_state_picture(self):
        ...
        pendulum = Pendulum(top_point=rect.get_center(), **self.big_pendulum_config)
        pendulum.fixed_point_tracker.add_updater(lambda m: m.move_to(rect.get_center()))
    ...
    def tie_state_to_dot_position(self, state, dot):
        def update_pendulum(pend):
            theta, omega = self.plane.point_to_coords(dot.get_center())
            pend.set_theta(theta)
            pend.set_omega(omega)
            return pend

        state.pendulum.add_updater(update_pendulum)
        state.pendulum.get_arc_angle_theta = lambda: self.plane.x_axis.point_to_number(dot.get_center())
        return self

    def tie_dot_position_to_state(self, dot, state):
        dot.add_updater(lambda d: d.move_to(self.get_state_point(state.pendulum)))
        return dot
...
class IntroduceVectorField(VisualizeStates):
    ...
    def preview_vector_field(self):
        ...
        self.add(growth.mobject, vector_field, self.state, self.ode, self.ode_word)
    ...
    def write_vector_derivative(self):
        ...
        # Vector
        vect = Arrow(plane.coords_to_point(0, 0), dot.get_center(), buff=0, color=dot.get_color())
        vect_sym, d_vect_sym = [
        ...
        time_inputs = VGroup(*[e[-1][-2] for e in vect_sym.get_entries()])
        ...
        real_d_vect = always_redraw(
            lambda: self.vector_field.get_vector(dot.get_center()).scale(
        ...
        equation_group = VGroup(ddt, vect_sym, equals, d_vect_sym)
        self.play(
            # equation_group.shift, 4 * DOWN,
            equation_group.to_edge,
            RIGHT,
            LARGE_BUFF,
            GrowArrow(d_vect),
        )
        ...
        self.play(ShowCreationThenFadeAround(d_vect_sym), FadeOut(d_vect))
        self.wait()
        self.play(
    ...
    def interpret_second_coordinate(self):
        ...
        expanded_derivative = self.get_vector_symbol(
            "{\\dot\\theta}(t)",
            "-\\mu {\\dot\\theta}(t)" + "-(g / L) \\sin\\big({\\theta}(t)\\big)",
        )
    ...
    def show_full_vector_field(self):
        ...
        self.play(
            ...
            self.d_vect_magnitude_factor_tracker.set_value,
            1,
        )
    ...
    def vector_field_func(self, point):
        ...
        mu, g, L = [self.big_pendulum_config.get(key) for key in ["damping", "gravity", "length"]]
        return pendulum_vector_field_func(x * RIGHT + y * UP, mu=mu, g=g, L=L)
    ...
    def ask_about_change(self):
        ...
        self.tie_state_to_dot_position(state, dot)
        self.play(GrowArrow(d_vect), FadeInFromDown(q_mark))
        for x in range(4):
            angle = 90 * DEGREES
            self.play(
                Rotate(
...
class ShowHighVelocityCase(ShowPendulumPhaseFlow, MovingCameraScene):
    ...
    def show_high_vector(self):
        field = self.vector_field
        top_vectors = VGroup(*filter(lambda a: np.all(a.get_center() > [-10, 1.5, -10]), field)).copy()
    ...
    def show_trajectory(self):
        ...
        anims = [
            ...
            UpdateFromFunc(dot, lambda d: d.move_to(traj.get_points()[-1])),
        ]
        if get_norm(self.frame_shift_vect) > 0:
            anims += [
                ApplyMethod(
                    frame.shift,
                    self.frame_shift_vect,
...
class TweakMuInFormula(Scene):
    def construct(self):
        ...
        interval.add_numbers(*np.arange(0, 1.2, 0.2))
        ...
        big_rect = VGroup(*[DashedLine(v1, v2) for v1, v2 in adjacent_pairs(big_rect_seed.get_vertices())])
        ...
        equation[-1].add_updater(lambda d: d.set_value(get_mu()).shift(0.05 * UL))
...
class SpectrumOfStartingStates(ShowHighVelocityCase):
    ...
    def show_many_trajectories(self):
        ...
        start_points.sort(key=lambda p: np.dot(p, UL))
        time = self.run_time

        # Count points
        dots = VGroup(*[Dot(sp, radius=0.025) for sp in start_points])
        dots.set_color_by_gradient(PINK, BLUE, YELLOW)
        words = OldTexText("Spectrum of\\\\", "initial conditions")
        words.set_stroke(BLACK, 5, background=True)
        words.next_to(dots, UP)

        self.play(
            # ShowIncreasingSubsets(dots, run_time=2),
            LaggedStartMap(FadeInFromLarge, dots, lambda m: (m, 10), run_time=2),
...
class AskAboutStability(ShowHighVelocityCase):
    ...
    def label_fixed_points(self):
        ...
        self.play(self.vector_field.set_opacity, 0.5, FadeInFromDown(label))
    ...
    def show_nudges(self):
        ...
        dots.add_updater(update_dots)
        self.add(trajs, dots)
        self.play(ShowCreation(trajs, lag_ratio=0), rate_func=linear, run_time=time)
        self.wait()
...
class LovePhaseSpace(ShowHighVelocityCase):
    ...
    def setup_plane(self):
        ...
        h1, h2 = hearts = VGroup(*[get_heart_var(i) for i in (1, 2)])
        hearts.scale(0.5)
    ...
    def show_vector_field(self):
        field = VectorField(
            lambda p: np.array([self.a * p[1], -self.b * p[0], 0]),
            **self.vector_field_config,
        )
        field.sort(get_norm)
        x_range = np.arange(-7, 7.5, 0.5)
        y_range = np.arange(-4, 4.5, 0.5)
        x_axis_arrows = VGroup(*[field.get_vector([x, 0, 0]) for x in x_range])
        y_axis_arrows = VGroup(*[field.get_vector([0, y, 0]) for y in y_range])
        ...
        for arrow in axis_arrows:
            ...
            arrow.scale(0.5 * real_len / arrow.get_length(), about_point=arrow.get_start())
    ...
    def show_example_trajectories(self):
        ...
        start_points = self.start_points = [
            2.5 * np.random.random() * rotate_vector(RIGHT, TAU * np.random.random()) for x in range(n_points)
        ]
        ...
        self.play(
            LaggedStartMap(FadeInFromLarge, dots, lambda m: (m, 5)),
            FadeInFromDown(words),
        )
    ...
    def add_resistance_term(self):
        ...
        words = brace.get_text("``Resistance'' term")
        words.set_stroke(BLACK, 5, background=True)
    ...
    def show_new_trajectories(self):
        ...
        new_field = VectorField(
            lambda p: np.array([self.a * p[1], -self.mu * p[1] - self.b * p[0], 0]),
            **self.vector_field_config,
        )
        ...
        self.play(*map(FadeOut, [trajs, dots, self.possibility_words]))
...
class TakeManyTinySteps(IntroduceVectorField):
    ...
    def take_many_time_steps(self):
        ...
        count.add_updater(lambda c: c.set_value(count_tracker.get_value()))
        count_label = OldTexText("steps")
        count_label.scale(1.5)
        count_label.add_updater(lambda m: m.next_to(count[-1], RIGHT, submobject_to_align=m[0][0], aligned_edge=DOWN))

        scaled_vectors = vectors.copy()
        scaled_vectors.clear_updaters()
        for vector in scaled_vectors:
            vector.scale(1 / vector.get_length(), about_point=vector.get_start())
    ...
    def get_labels(self, get_t, get_delta_t):
        t_label, dt_label = labels = VGroup(
            *[
                VGroup(OldTex("{} = ".format(s)), DecimalNumber(0)).arrange(RIGHT, aligned_edge=DOWN)
                for s in ("t", "{\\Delta t}")
            ]
        )

        dt_label[-1].add_updater(lambda d: d.set_value(get_delta_t()))
        t_label[-1].add_updater(lambda d: d.set_value(int(np.ceil(get_t() / get_delta_t())) * get_delta_t()))

        init_labels = VGroup(
            OldTex(
                "\\theta_0",
                "=",
                self.initial_theta_tex,
                tex_to_color_map={"\\theta": BLUE},
            ),
            OldTex(
                "{\\dot\\theta}_0 =",
                self.initial_theta_dot_tex,
    ...
    def get_path_vectors(self, delta_t, total_time, theta_0, theta_dot_0):
        ...
        for a1, a2 in zip(corners, corners[1:]):
            ...
            vector.match_style(self.vector_field.get_vector(a1))
            result.add(vector)
        return result
...
class SetupToTakingManyTinySteps(TakeManyTinySteps):
    ...
    def show_step(self):
        ...
        vector.scale(get_norm(vector_value) / vector.get_length(), about_point=vector.get_start())
        scaled_vector = vector.copy()
        scaled_vector.scale(get_delta_t(), about_point=scaled_vector.get_start())
        ...
        for v in real_field:
            p = v.get_start()
            v.scale(get_norm(field.func(p)) / v.get_length(), about_point=p)
        ...
        rect = BackgroundRectangle(VGroup(v_label.target, dt_part.target))
