# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part1\wordy_scenes.py
...
class SmallAngleApproximationTex(Scene):
    def construct(self):
        ...
        self.play(Write(implies), FadeIn(period, LEFT))
        self.wait()
...
class ComingUp(Scene):
    ...
    def construct(self):
        frame = ScreenRectangle(stroke_width=0, fill_color=BLACK, fill_opacity=1, height=6)
...
class InputLabel(Scene):
    def construct(self):
        ...
        self.play(FadeIn(label, UP), GrowArrow(arrow))
        self.wait()
...
class ReasonForSolution(Scene):
    def construct(self):
        ...
        axes = Axes(x_min=-1, x_max=5.5, y_min=-1, y_max=4.5, y_axis_config={"unit_size": 0.5})
        ...
        self.wait()
        self.play(FadeIn(c_word, UL), GrowArrow(arrows[2]), FadeIn(computation, UP))
        self.wait()
        self.play(FadeIn(u_word, DL), GrowArrow(arrows[1]), FadeInFromDown(graph))
        self.wait(2)
...
class ThreeBodySymbols(Scene):
    ...
    def init_coord_groups(self):
        ...
        positions = VGroup(*[get_vector_symbol(*["{}_{}".format(s, i) for s in "xyz"], **kwargs) for i in range(1, 4)])
        velocities = VGroup(
            *[get_vector_symbol(*["p^{}_{}".format(s, i) for s in "xyz"], **kwargs) for i in range(1, 4)]
        )
    ...
    def count_coords(self):
        ...
        count.add_updater(lambda m: m.next_to(count_word[0][0], LEFT, aligned_edge=DOWN))
...
class RevisitQuote(StrogatzQuote, PiCreatureScene):
    def construct(self):
        ...
        new_langauge_part = OldTexText("\\Large Language of differential equations")
        ...
        self.play(
            ...
            *[FadeOut(part) for part in quote if part is not quote.language_part],
            run_time=2,
        )
        self.wait()
