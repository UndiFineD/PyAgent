# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part1\staging.py
from _2018.div_curl import PhaseSpaceOfPopulationModel, ShowTwoPopulations
from _2019.diffyq.part1.pendulum import Pendulum, ThetaVsTAxes
...
class VectorFieldTest(Scene):
    def construct(self):
        ...
        field = VectorField(
            lambda p: pendulum_vector_field_func(plane.point_to_coords(p), mu=mu_tracker.get_value()),
            delta_x=0.5,
            delta_y=0.5,
            max_magnitude=6,
            opacity=0.5,
...
class TourOfDifferentialEquations(MovingCameraScene):
    ...
    def add_title(self):
        title = OldTexText("A Tourist's Guide \\\\to Differential\\\\Equations")
        title.scale(1.5)
        title.to_corner(UR)
        self.add(title)
    ...
    def show_thumbnails(self):
        ...
        self.play(
            ShowCreation(line, rate_func=lambda t: np.clip(t * (n + 1) / n, 0, 1)),
            LaggedStart(
                *[
                    GrowFromCenter(
                        thumbnail,
    ...
    def add_fourier_series(self, thumbnail):
        ...
        waves = VGroup(*[self.get_square_wave_approx(N, color) for N, color in enumerate(colors)])
    ...
    def get_square_wave_approx(self, N, color):
        return FunctionGraph(
            lambda x: sum([(1 / n) * np.sin(n * PI * x) for n in range(1, 2 * N + 3, 2)]),
            x_min=0,
            x_max=2,
            color=color,
        )
    ...
    def add_laplace_symbol(self, thumbnail):
        mob = OldTex("\\mathcal{L}\\left\\{f(t)\\right\\}")
...
class ShowGravityAcceleration(Scene):
    ...
    def show_g_value(self):
        title = self.title
        g_eq = self.g_eq = OldTex("-9.8", "{\\text{m/s}", "\\over", "\\text{s}}", **Lg_formula_config)
        ...
        self.add(num)
        self.wait(0.75)
        self.play(FadeIn(ms, 0.25 * DOWN, run_time=0.5))
        self.wait(0.25)
        self.play(
            LaggedStart(
    ...
    def show_trajectory(self):
        ...
        for t1, t2 in zip(times, times[1:]):
            ...
            kw = {"rate_func": lambda alpha: interpolate(t1 / total_time, t2 / total_time, alpha)}
            anims = [
    ...
    def combine_v_vects(self):
        ...
        braces = VGroup(
            *[brace.copy().match_height(arrow).next_to(arrow, RIGHT, buff=0.2 * SMALL_BUFF) for arrow in delta_vects]
        )
        amounts = VGroup(*[OldTexText("9.8 m/s").scale(0.5).next_to(brace, RIGHT, SMALL_BUFF) for brace in braces])
...
class SimpleProjectileEquation(ShowGravityAcceleration):
    CONFIG = {
        ...
        "axes_config": {
            ...
            "y_axis_config": {
                "unit_size": 0.1,
                "big_tick_numbers": range(-30, 35, 10),
                "tick_size": 0.05,
    ...
    def setup_trajectory(self):
        ...
        graph.add_updater(lambda g: g.pointwise_become_partial(graph_template, 0, get_t() / total_time))
        traj.add_updater(lambda t: t.pointwise_become_partial(traj_template, 0, get_t() / total_time))
        ...
        graphed_v_vect = always_redraw(lambda: get_v_vect().shift(axes.coords_to_point(get_t(), 0)))
        ...
        d2y_dt2_label = OldTex("d^2", "{y}", "\\over dt}", "(t)")
    ...
    def solve_for_velocity(self):
        ...
        for x in range(2):
            answer1.submobjects.insert(4, VectorizedPoint(answer1[4].get_left()))
        for deriv in derivs:
        ...
        self.play(FadeIn(tex_question, DOWN), FadeIn(question, UP))
        self.wait()
    ...
    def solve_for_position(self):
        ...
        for deriv, terms in zip(derivs, all_terms):
            for x in range(len(all_terms[-1]) - len(terms)):
                n = 2 + len(terms)
                deriv.submobjects.insert(n, VectorizedPoint(deriv[n].get_left()))
            deriv.next_to(VGroup(equation, y_dot_equation), DOWN, MED_LARGE_BUFF + SMALL_BUFF)
...
class UniversalGravityLawSymbols(Scene):
    def construct(self):
        ...
        x_deriv = OldTex("{d", x1_tex, "\\over", "dt}", "=", v1_tex, **kw)
        x_deriv.to_corner(UL)
        v_deriv = OldTex("{d", v1_tex, "\\over", "dt}", "=", **kw)
...
class ExampleTypicalODE(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(FadeIn(examples[2], UP))
        self.wait(5)
...
class DefineODE(Scene):
    ...
    def dont_know_the_value(self):
        ...
        q_marks = VGroup(*[OldTex("?").move_to(graph.point_from_proportion(a)) for a in np.linspace(0, 1, 20)])
        ...
        self.play(
            ...
            LaggedStart(
                *[
                    UpdateFromAlphaFunc(
                        q_mark,
                        lambda m, a: m.set_height(0.5 * (1 + a)).set_fill(opacity=there_and_back(a)),
                    )
    ...
    def show_value_slope_curvature(self):
        ...
        graph = axes.get_graph(
            lambda t: p.initial_theta * np.cos(np.sqrt(p.gravity / p.length) * t) * np.exp(-p.mu * t / 2)
        )
        ...
        theta, d_theta, dd_theta = [OldTex("{" + s + "\\theta}(t)", **tex_config) for s in ("", "\\dot", "\\ddot")]
        ...
        def get_v_line():
            ...
            x_point = axes.x_axis.number_to_point(axes.x_axis.point_to_number(point))
            return DashedLine(
                x_point,
                point,
                dash_length=0.025,
        ...
        def get_curve():
            curve = VMobject()
            t = get_t()
            curve.set_points_smoothly([get_point(t + a) for a in np.linspace(-0.5, 0.5, 11)])
    ...
    def write_ode(self):
        ...
        self.play(
            ShowCreationThenFadeOut(t_rects, lag_ratio=0.8),
        ...
        ode_initials.target.arrange(RIGHT, buff=0.5 * SMALL_BUFF, aligned_edge=DOWN)
        ode_initials.target.to_edge(UP, buff=MED_SMALL_BUFF)

        ode_remaining_letters = VGroup(*it.chain(*[word[1:] for word in ode_word]))
...
class SecondOrderEquationExample(DefineODE):
    ...
    def show_value_slope_curvature(self):
        axes = self.axes
        graph = axes.get_graph(lambda t: -2.5 * np.cos(2 * t) * np.exp(-0.2 * t))
        ...
        x, d_x, dd_x = [OldTex("{" + s + "x}(t)", **tex_config) for s in ("", "\\dot ", "\\ddot ")]
...
class XComponentArrows(Scene):
    def construct(self):
        ...
        for u in (1, -1):
            ...
            self.play(LaggedStartMap(GrowArrow, field, lag_ratio=0.1, run_time=1))
...
class BreakingSecondOrderIntoTwoFirstOrder(IntroduceVectorField):
    def construct(self):
        ...
        theta_dots = VGroup(
            *filter(
                lambda m: (isinstance(m, SingleStringTex) and "{\\dot\\theta}" == m.get_tex()),
                system1.get_family(),
            )
        )
...
class FromODEToVectorField(Scene):
    def construct(self):
        ...
        equation = VGroup(OldTex("d \\over dt").scale(1.5), vect, OldTex("="), d_vect)
        equation.scale(0.8)
...
class ThreeBodiesInSpace(SpecialThreeDScene):
    ...
    def add_bodies(self):
        ...
        center_of_mass = reduce(op.add, [body.mass * body.get_center() / total_mass for body in bodies])
        average_momentum = reduce(op.add, [body.mass * body.velocity / total_mass for body in bodies])
        for body in bodies:
            body.shift(-center_of_mass)
            body.velocity -= average_momentum

    def get_initial_positions(self):
        return [np.dot(4 * (np.random.random(3) - 0.5), [RIGHT, UP, OUT]) for x in range(len(self.masses))]

    def get_initial_velocity(self, center, centers, mass):
        to_others = [center - center2 for center2 in centers]
        velocity = 0.2 * mass * normalize(np.cross(*filter(lambda diff: get_norm(diff) > 0, to_others)))
        return velocity
...
class TwoBodiesInSpace(ThreeBodiesInSpace):
    ...
    def get_initial_positions(self):
        return [np.dot(6 * (np.random.random(3) - 0.5), [RIGHT, UP, ORIGIN]) for x in range(len(self.masses))]

    def get_initial_velocity(self, center, centers, mass):
        return 0.75 * normalize(np.cross(center, OUT))

    def add_velocity_vectors(self):
        vectors = VGroup(*[self.get_velocity_vector(body) for body in self.bodies])
    ...
    def get_velocity_vector(self, body):
        def create_vector(b):
            ...
            v.shift(
                b.point + b.radius * normalize(b.velocity) - v.get_start(),
            )
    ...
    def add_force_vectors(self):
        vectors = VGroup(*[self.get_force_vector(b1, b2) for (b1, b2) in (self.bodies, self.bodies[::-1])])
    ...
    def get_force_vector(self, body1, body2):
        def create_vector(b1, b2):
            ...
            v.set_stroke(width=3)
            v.shift(
                b1.point + b1.radius * normalize(F) - v.get_start(),
            )
            v.set_shade_in_3d(True)
            return v
...
class LoveExample(PiCreatureScene):
    ...
    def show_hearts(self):
        ...
        self.play(
            LaggedStartMap(
                ...
                lambda heart: (
                    heart,
                    lambda h, a: h.set_opacity(there_and_back(a)).shift(0.02 * UP),
                ),
                lag_ratio=0.01,
                run_time=3,
    ...
    def break_down_your_rule(self):
        ...
        self.wait()
        self.play(equation.scale, 0.5, equation.to_corner, UL, FadeOut(down_arrow))
    ...
    def add_love_number_lines(self):
        ...
        dot1.add_updater(lambda d: d.move_to(nl1.number_to_point(self.get_love1())))
        dot2 = Dot(color=self.tau.get_color())
        dot2.add_updater(lambda d: d.move_to(nl2.number_to_point(self.get_love2())))
...
class ComparePhysicsToLove(Scene):
    def construct(self):
        ...
        love.move_to(DOWN)
        hearts = VGroup(*filter(lambda sm: isinstance(sm, SuitSymbol), love.get_family()))
...
class InaccurateComputation(Scene):
    def construct(self):
        ...
        errors = VGroup(
            OldTex("\\approx 8.7422 \\times 10^{-8}"),
            OldTex(
