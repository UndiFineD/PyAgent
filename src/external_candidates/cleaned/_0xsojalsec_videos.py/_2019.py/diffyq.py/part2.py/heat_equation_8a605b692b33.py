...
class BringTwoRodsTogether(Scene):
    ...
    def get_second_derivative(self, x, dx=0.001):
        ...
        ly, y, ry = [
            graph.point_from_proportion(inverse_interpolate(x_min, x_max, alt_x))[1] for alt_x in (x - dx, x, x + dx)
        ]
...
class TalkThrough1DHeatGraph(ShowEvolvingTempGraphWithArrows, SpecialThreeDScene):
    ...
    def show_surface(self):
        ...
        self.play(
            FadeIn(surface),
            LaggedStart(
                *[TransformFromCopy(graph, graph_slice) for graph_slice in graph_slices],
                lag_ratio=0.02,
            ),
        )
        self.wait(4)
...
class DiscreteSetup(ShowEvolvingTempGraphWithArrows):
    ...
    def focus_on_three_points(self):
        ...
        diff_value.add_updater(
            lambda m: m.set_value(y_axis.p2n(average_pointer.get_right()) - y_axis.p2n(d2.get_center()))
        )
        diff_value.next_to(brace, DOWN)

        self.play(
            ReplacementTransform(average_label.deepcopy(), formula[1 : 1 + len(average_label)]),
...
class TransitionToContinuousCase(DiscreteSetup):
    ...
    def get_line_group(
        ...
        def update_group(group):
            ...
            dx = get_dx()
            p0, p2 = [
                graph.point_from_proportion(inverse_interpolate(self.graph_x_min, self.graph_x_max, x))
                for x in [x, x + dx]
            ]
            p1 = np.array([p2[0], *p0[1:]])
