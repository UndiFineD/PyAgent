# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part2\fourier_series.py
...
class FourierCirclesScene(Scene):
    ...
    def get_circles(self, vectors):
        return VGroup(
            *[self.get_circle(vector, color=color) for vector, color in zip(vectors, self.get_color_iterator())]
        )
    ...
    def get_vector_sum_path(self, vectors, color=YELLOW):
        ...
        path = ParametricCurve(
            ...
            + reduce(
                op.add,
                [complex_to_R3(coef * np.exp(TAU * 1j * freq * t)) for coef, freq in zip(coefs, freqs)],
            ),
            t_min=0,
            t_max=1,
            color=color,
