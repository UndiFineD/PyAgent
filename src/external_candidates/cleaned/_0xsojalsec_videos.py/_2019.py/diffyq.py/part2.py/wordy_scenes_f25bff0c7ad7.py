# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part2\wordy_scenes.py
...
class TwoGraphTypeTitles(Scene):
    def construct(self):
        ...
        right_title = OldTexText("Represent time\\\\with an axis")
...
class ShowPartialDerivativeSymbols(Scene):
    def construct(self):
        ...
        dels = VGroup(*it.chain(*[del_deriv.get_parts_by_tex("\\partial") for del_deriv in del_derivs]))
...
class WriteHeatEquation(WriteHeatEquationTemplate):
    def construct(self):
        ...
        to_be_explained = OldTexText("To be explained shortly...")
...
class Show3DEquation(WriteHeatEquationTemplate):
    def construct(self):
        ...
        inputs = VGroup(*it.chain(*[equation.get_parts_by_tex(s) for s in ["{x}", "{y}", "{z}", "{t}"]]))
        inputs.sort()
        equation.to_edge(UP)
...
class Show1DAnd3DEquations(WriteHeatEquationTemplate):
    def construct(self):
        ...
        self.wait(2)
        self.play(Restore(d1_group), FadeIn(d3_group, LEFT))
        self.wait()
        self.play(
...
class CompareInputsOfGeneralCaseTo1D(WriteHeatEquation):
    def construct(self):
        ...
        x, y, z = [three_d_expr.get_part_by_tex(letter) for letter in ["x", "y", "z"]]
...
class ShowLaplacian(WriteHeatEquation):
    def construct(self):
        ...
        laplacian_name = OldTexText("``Laplacian''")
...
class AskAboutActuallySolving(WriteHeatEquationTemplate):
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(q3, LEFT))
        self.wait()
