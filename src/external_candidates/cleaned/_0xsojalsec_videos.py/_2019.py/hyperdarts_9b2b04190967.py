...
class ShowExampleHit(HyperdartScene):
    def construct(self):
        ...
        new_diam_word.add_updater(
            lambda m: m.set_width(min(ndw_width, chord_copy.get_width())).next_to(chord_copy, DOWN, SMALL_BUFF)
        )
...
class ContrastDistributions(HyperdartScene):
    def construct(self):
        ...
        left_dots, right_dots = [
            VGroup(*[Dot(p, radius=0.02) for p in points]) for points in [left_points, right_points]
        ]
...
class ShowProbabilityForFirstShot(HyperdartScene):
    def construct(self):
        ...
        self.play(
            ReplacementTransform(circle.copy().set_fill(opacity=0).set_stroke(WHITE, 1), square_frac[0]),
        )
        self.play(
...
class ExpectedValueComputation(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                TransformFromCopy(line1.get_part_by_tex("="), line2.get_part_by_tex("="), **kw),
                TransformFromCopy(
                    line1.get_parts_by_tex("\\cdot"),
                    line2.get_parts_by_tex("\\cdot"),
                    **kw,
                ),
                TransformFromCopy(line1.get_parts_by_tex("+"), line2.get_parts_by_tex("+"), **kw),
                TransformFromCopy(line1.get_part_by_tex("1"), line2.get_part_by_tex("1"), **kw),
                TransformFromCopy(line1.get_part_by_tex("2"), line2.get_part_by_tex("2"), **kw),
                TransformFromCopy(line1.get_part_by_tex("3"), line2.get_part_by_tex("3"), **kw),
                run_time=3,
                lag_ratio=0,
            ),
            LaggedStart(*[GrowFromCenter(bigs[i : i + 2]) for i in range(0, len(bigs), 2)]),
        )
        self.wait()
...
class IntroduceGame(HyperdartScene):
    ...
    def show_flurry_of_points(self):
        ...
        self.play(
            *[ApplyPointwiseFunction(func, pieces, run_time=1) for pieces in [*board[:3], *dots]],
