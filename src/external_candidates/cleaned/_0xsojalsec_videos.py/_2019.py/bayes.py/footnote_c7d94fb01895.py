# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\bayes\footnote.py
from _2019.bayes.part1 import (
    BayesDiagram,
    LibrarianIcon,
    Person,
...
class ThisIsAFootnote(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            self.change_students("pondering", "hooray", "tease", look_at=image),
        )
        self.play(
            Write(asterisk),
            self.teacher.change,
        ...
        self.play(self.change_students("thinking", "erm", "thinking"))
        self.wait(3)
        self.play(
            self.teacher.change,
            "raise_right_hand",
        ...
        self.play(
            ...
            4 * UP,
            self.change_students("thinking", "erm", "confused", look_at=4 * UP),
        )
        self.wait(2)
...
class ShowTwoPerspectives(Scene):
    ...
    def construct(self):
        ...
        space_words = OldTexText("Space of all\\\\possibilities")
    ...
    def get_venn_diagram(self):
        ...
        for title, circle, vect in zip(titles, circles, [UL, UR]):
            ...
            title.scale(2)
            title.next_to(circle.get_boundary_point(vect), vect, buff=SMALL_BUFF)
...
class Rearrange(ShowTwoPerspectives):
    def construct(self):
        ...
        eq1.target.move_to(midpoint(p_both.get_corner(DL), new_line.target[0].get_corner(UR)))
        eq2.generate_target()
        eq2.target.rotate(-PI / 3)
        eq2.target.move_to(midpoint(p_both.get_corner(DR), new_line.target[4].get_corner(UL)))
        ...
        books = VGroup(*[LibrarianIcon().replace(B, dim_to_match=0) for B in Bs])
        books.set_color(YELLOW)

        mags = VGroup(*[mag.copy().replace(A) for A in As])

        self.play(LaggedStart(*[ReplacementTransform(A, mag, path_arc=PI) for A, mag in zip(As, mags)]))
        self.play(LaggedStart(*[ReplacementTransform(B, book, path_arc=PI) for B, book in zip(Bs, books)]))
        self.wait()
...
class LandscapeOfTools(TeacherStudentsScene):
    ...
    def get_formulas(self):
        group = VGroup(
            ...
            OldTex("E[X + Y] = E[x] + E[y]"),
            OldTex("\\text{Var}(X + Y) = \\text{Var}(x) + \\text{Var}(y) + 2\\text{Cov}(X, Y)"),
            OldTex(
                "H = \\sum_{i} -p_i \\log",
                "(p_i)",
                tex_to_color_map={
...
class TemptingFormula(ShowTwoPerspectives, RandomnessVsProportions):
    def construct(self):
        ...
        row_rect = SurroundingRectangle(VGroup(grid[0], left_people[0]), buff=SMALL_BUFF)
        ...
        self.play(
            ...
            0,
            {"about_edge": LEFT},
            *[ApplyMethod(grid[i][0].stretch, 2 / 3, 0, {"about_edge": LEFT}) for i in range(1, 4)],
            *[ApplyMethod(grid[i][1:].stretch, 10 / 9, 0, {"about_edge": RIGHT}) for i in range(1, 4)],
        )
        self.wait()
    ...
    def get_grid(self, n, m, height=4):
        grid = VGroup(*[VGroup(*[Square() for x in range(m)]).arrange(RIGHT, buff=0) for y in range(n)]).arrange(
            DOWN, buff=0
        )
