# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\bayes\part1.py
...
class ProbabilityBar(VGroup):
    ...
    def get_percentages(self):
        ...
        for label, bar in zip(labels, self.bars):
            ...
            label.move_to(bar)
            label.set_stroke(BLACK, self.percentage_background_stroke_width, background=True)
        return labels
    ...
    def add_icons(self, *icons, buff=SMALL_BUFF):
        ...
        for icon, ref in zip(icons, refs):
            icon.ref = ref
            icon.add_updater(lambda i: i.next_to(i.ref, self.brace_direction, buff=buff))
...
class StateGoal(PiCreatureScene, Scene):
    ...
    def construct(self):
        ...
        self.play(
            Blink(you),
            FadeOut(VGroup(line, now_label, later_label, you_label, you_arrow)),
        )
        self.play(
            FadeIn(gold, LEFT),
            you.change,
        ...
        for n, point in enumerate(level_points):
            ...
            point.add(*[point[-1].copy().scale(1.2**k) for k in range(1, n + 1)])
        ...
        self.play(
            ...
            UR,
            FadeOut(you),
            *[ReplacementTransform(bubble.copy(), point) for point in level_points],
        )
        ...
        equal_signs = VGroup(*[OldTexText("=").next_to(fp, RIGHT) for fp in formula_parts.target])
        ...
        braces = VGroup(*[Brace(diagram.he_rect, vect, buff=SMALL_BUFF) for vect in [DOWN, LEFT]])
        ...
        pe_picture = VGroup(diagram.he_rect.copy(), OldTex("+"), diagram.nhe_rect.copy())
...
class DescriptionOfSteve(Scene):
    ...
    def write_description(self):
        ...
        lines = VGroup(*[Line(mob.get_corner(DL), mob.get_corner(DR), color=YELLOW) for mob in mt_parts])
        self.play(
            ShowCreation(lines),
            mt_parts.set_color,
            YELLOW,
...
class CorrectViewOfFarmersAndLibrarians(Scene):
    def construct(self):
        ...
        # Add arrows
        arrows = VGroup(*[Arrow(s.get_bottom(), m.get_top()) for s, m in zip(steves, icons)])
        words = VGroup(
...
class ComplainAboutNotKnowingTheStats(TeacherStudentsScene):
    def construct(self):
        self.student_says("Are people expected\\\\to know that?", index=2)
...
class ReasonByRepresentativeSample(CorrectViewOfFarmersAndLibrarians):
    ...
    def construct(self):
        ...
        colors = [interpolate_color(color, BLACK, 0.5) for color in [EVIDENCE_COLOR1, EVIDENCE_COLOR2]]
        ...
        for rect, vect in zip([dl_rect, dr_rect], [LEFT, RIGHT]):
            ...
            rect.number.add_updater(lambda d: d.set_value(100 * d.brace.get_height() / big_rect.get_height()))
        ...
        self.play(
            ...
            UP,
            SMALL_BUFF,
            l_brace.count_tracker.set_value,
            4,
        ...
        group.shift(
            (equation.get_part_by_tex("\\over").get_x() - prior_equation.get_part_by_tex("\\over").get_x()) * RIGHT
        )
...
class NewEvidenceUpdatesPriorBeliefs(DescriptionOfSteve):
    def construct(self):
        ...
        self.add(evid)
        self.play(GrowArrow(arrow), FadeIn(librarian, LEFT))
...
class HeartOfBayesTheorem(Scene):
    def construct(self):
        ...
        fraction = always_redraw(lambda: self.get_geometric_fraction(restricted_diagram))
        frac_box = always_redraw(
        ...
        self.play(Restore(label2), ReplacementTransform(diagram_copy, restricted_diagram))
        self.wait()
        ...
        self.play(*[ApplyMethod(d.set_prior, 0.4, run_time=2) for d in diagrams])
        self.play(*[ApplyMethod(d.set_likelihood, 0.3, run_time=2) for d in diagrams])
        self.play(*[ApplyMethod(d.set_antilikelihood, 0.6, run_time=2) for d in diagrams])
        ...
        self.play(FadeIn(words, DOWN), GrowArrow(arrow), FadeOut(prob), title.to_edge, LEFT)
        self.wait()
...
class WhenDoesBayesApply(DescriptionOfSteve):
    def construct(self):
        ...
        self.play(LaggedStart(*[TransformFromCopy(big_prob[i][j], prob[i]) for i, j in [(2, 0), (3, 1), (4, 3)]]))
        self.wait()
...
class CreateFormulaFromDiagram(Scene):
    ...
    def construct(self):
        ...
        prior_label.add_updater(lambda m: m.next_to(diagram.h_brace, UP, SMALL_BUFF))

        prior_example = OldTex("= 1 / 21")
        prior_example.add_updater(lambda m: m.next_to(prior_label, RIGHT).shift(0.03 * UP))
        ...
        self.wait()
        self.play(FadeIn(prior_word, RIGHT), GrowArrow(prior_arrow))
        self.wait()
        prior_group = VGroup(
            prior_word,
            prior_arrow,
        ...
        self.play(
            ShowCreationThenFadeAround(like_label.get_part_by_tex("H")),
        )
        self.wait()
        self.play(
        ...
        anti_example.add_updater(lambda m: m.next_to(anti_label, RIGHT).align_to(anti_label[0], DOWN))
        ...
        self.wait()
        self.play(FadeOut(not_word), Transform(neg_sym, anti_label.get_part_by_tex("\\neg")))
        self.remove(neg_sym)
        ...
        self.wait()
        self.play(Restore(parts), FadeOut(crosses), FadeOut(nps), answer.set_opacity, 0.2)
        self.wait()
        ...
        self.wait(2)
        self.play(randy.change, "pondering", prior_label, ShowCreation(prior_rect))
        self.play(Blink(randy))
        self.play(
...
class DiscussFormulaAndAreaModel(CreateFormulaFromDiagram):
    ...
    def construct(self):
        ...
        self.play(s_rect.become, SurroundingRectangle(alt_rhs[12:-1], color=BLUE))
        self.wait()
        ...
        h_label, he_label, nhe_label = labels = [
            OldTex(tex, tex_to_color_map=t2c) for tex in ["P(H)", "P(E|H)", "P(E|\\neg H)"]
        ]
...
class IntroduceLinda(DescriptionOfSteve):
    def construct(self):
        ...
        result = VGroup(Integer(85, unit="\\%"), OldTexText("chose this!"))
...
class LindaDescription(IntroduceLinda):
    def construct(self):
        ...
        self.wait(1)
        self.play(highlighted_part.set_color, YELLOW, lag_ratio=0.1, run_time=2)
        self.wait()
...
class WhenDiscreteChunksArentSoClean(Scene):
    def construct(self):
        ...
        self.add(squares)
        self.add(p_label[0])
        self.play(ChangeDecimalToValue(alt_percentage, 40), ShowIncreasingSubsets(icons[:40]))
...
class RandomnessVsProportions(Scene):
    def construct(self):
        ...
        example = VGroup(OldTexText("P(X = 5)", tex_to_color_map={"5": YELLOW}), Line(LEFT, RIGHT))
        example.arrange(RIGHT)
        example.next_to(grid, RIGHT, LARGE_BUFF)
        example.align_to(random_dice, RIGHT)
        example.shift(0.5 * DOWN)
        grid_copy = grid.copy()
        five_part = VGroup(*[square for i, row in enumerate(grid_copy) for j, square in enumerate(row) if i + j == 3])
        ...
        self.wait(9)
        self.play(FadeIn(prop_word, RIGHT), Write(arrows[0]))
    ...
    def get_die_faces(self):
        ...
        arrangements = VGroup(
            *[
                VGroup(*[dot.copy().move_to(square.get_bounding_box_point(ec)) for ec in edge_group])
                for edge_group in edge_groups
            ]
        )
        square.set_width(1)

        faces = VGroup(*[VGroup(square.copy(), arrangement) for arrangement in arrangements])
        faces.arrange(RIGHT)
    ...
    def get_dice_diagram(self):
        grid = VGroup(*[VGroup(*[Square() for x in range(6)]).arrange(RIGHT, buff=0) for y in range(6)]).arrange(
            DOWN, buff=0
        )
...
class GlimpseOfNextVideo(GraphScene):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(formula[:4]), Restore(rects1, lag_ratio=0.05, run_time=2))
        self.wait()
...
class ComingUp(Scene):
    CONFIG = {"camera_config": {"background_color": GREY_D}}
...
class QuestionSteveConclusion(HeartOfBayesTheorem, DescriptionOfSteve):
    def construct(self):
        ...
        self.play(LaggedStart(*[ApplyMethod(pi.change, "hooray", steve) for pi in pis]))
        ...
        self.play(
            ...
            LaggedStart(*[ApplyMethod(pi.change, "thinking", formula) for pi in pis]),
        )
        self.play(Blink(randy))
        self.play(
            LaggedStartMap(
...
class ReprogrammingThought(Scene):
    ...
    def construct(self):
        ...
        self.add(brain)
        self.play(ShowCreationThenFadeOut(brain_outline, lag_ratio=0.01, run_time=2))
        ...
        self.play(FadeIn(prior_outline), ApplyMethod(diagram.set_prior, 0.5, run_time=2))
...
class Thumbnail2(Scene):
    def construct(self):
        ...
        right_arrow = Arrow(diagram.get_right(), frac[1].get_left(), thickness=0.1, fill_color=GREEN)
