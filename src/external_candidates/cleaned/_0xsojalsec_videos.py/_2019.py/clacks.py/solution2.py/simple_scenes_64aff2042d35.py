...
class TwoSolutionsWrapper(WrapperScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(ShowCreation, screen_rects.copy().set_fill(opacity=0), lag_ratio=0.8),
            LaggedStartMap(FadeIn, screen_rects, lag_ratio=0.8),
            LaggedStartMap(ApplyMethod, pis, lambda pi: (pi.change, "pondering", screen_rects[0])),
        )
