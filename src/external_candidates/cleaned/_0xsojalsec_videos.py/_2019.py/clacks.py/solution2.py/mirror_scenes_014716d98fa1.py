# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\solution2\mirror_scenes.py
...
class MirrorScene(Scene):
    ...
    def setup(self):
        ...
        self.beam_point = VectorizedPoint(np.array([self.get_start_x_offset(), self.get_start_y_offset(), 0]))
    ...
    def get_theta_display(self):
        ...
        degrees.add_updater(lambda m: m.set_value(int(np.round(self.get_theta() / DEGREES))))
    ...
    def construct(self):
        ...
        for angle, y in zip(angles, ys):
            ...
            self.play(
                ...
                UpdateFromFunc(trajectory, lambda m: m.become(self.get_trajectory())),
                run_time=2,
            )
            self.show_bouncing()
        self.wait(2)
...
class ReflectWorldThroughMirrorNew(MirrorScene):
    ...
    def add_randy(self):
        ...
        randy.add_updater(lambda m: m.look_at(m.tracked_mobject.get_points()[-1]))
        self.add(randy)
    ...
    def add_ghost_beam_point(self):
        self.ghost_beam_point.add_updater(lambda m: m.move_to(self.ghost_trajectory.get_points()[-1]))
        self.add(self.ghost_beam_point)

    def up_through_first_bounce(self):
        self.play(*self.get_both_partial_trajectory_anims(*self.partial_trajectory_values[:2]))
        self.wait()

    def create_reflected_worlds(self):
        mirrors = self.mirrors
        triangle = Polygon(*[mirrors.get_corner(corner) for corner in (DR, DL, UR)])
    ...
    def create_reflected_trajectories(self):
        self.reflected_trajectories = always_redraw(lambda: self.get_reflected_worlds(self.trajectory))
    ...
    def first_reflection(self):
        ...
        self.play(
            *[ApplyMethod(m.set_stroke, GREY, 1) for m in (trajectory, reflected_trajectory)] + beam_anims,
            run_time=2,
        )
        for x in range(2):
    ...
    def next_reflection(self, index=2):
        i = index
        self.play(
            *self.get_both_partial_trajectory_anims(*self.partial_trajectory_values[i - 1 : i + 1]),
            UpdateFromFunc(
                VMobject(),  # Null
        ...
        anims = [
            ...
            for reflections in [self.reflected_worlds, self.reflected_trajectories]
        ]
    ...
    def unfold_all_reflected_worlds(self):
        ...
        self.play(LaggedStartMap(Restore, new_worlds, lag_ratio=0.4, run_time=3))

    def show_completed_beam(self):
        self.add(self.reflected_trajectories)
        self.add(self.ghost_trajectory)
        self.play(*self.get_both_partial_trajectory_anims(*self.partial_trajectory_values[-2:], run_time=7))
    ...
    def add_randy_updates(self):
        ...
        reflected_randys.add_updater(lambda m: m.become(self.get_reflected_worlds(self.randy)))
    ...
    def show_all_trajectories(self):
        ...
        all_mirrors = VGroup(*[world[1] for world in it.chain([self.world], self.reflected_worlds)])
            ...
            if x == num_repeats - 1:
                ...
                for flash in all_flashes:
                    if hasattr(flash.mobject, "time_width"):
                        flash.mobject.set_stroke(width=0.25 * flash.mobject.get_stroke_width())
    ...
    def focus_on_two_important_trajectories(self):
        ...
        self.ghost_beam_point.add_updater(lambda m: m.move_to(gt_beam_anims[0].mobject.get_points()[-1]))
    ...
    def get_partial_trajectory_anims(self, trajectory, a, b, **kwargs):
        ...
        return UpdateFromAlphaFunc(
            trajectory,
            lambda m, alpha: m.pointwise_become_partial(m.full_self, 0, interpolate(a, b, alpha)),
            **kwargs,
        )
    ...
    def get_both_partial_trajectory_anims(self, a, b, run_time=2, **kwargs):
        ...
        return [
            self.get_partial_trajectory_anims(mob, a, b, **kwargs) for mob in (self.trajectory, self.ghost_trajectory)
        ]
...
class MirrorAndWiresOverlay(MirrorScene):
    ...
    def add_diagram(self):
        ...
        wires = VGroup(*[Line(center + np.array([-1, -1.5, 0]), center) for x in range(4)])
    ...
    def show_angles(self):
        ...
        self.play(
            ShowCreation(dl_arc),
            UpdateFromAlphaFunc(VMobject(), lambda m, a: incidence.set_fill(opacity=a), remover=True),
        )
        self.wait()
        self.add(reflection)
        self.play(
            ShowCreation(dr_arc),
            UpdateFromAlphaFunc(VMobject(), lambda m, a: reflection.set_fill(opacity=a), remover=True),
        )
        self.wait()
