...
class SlidingBlocks(VGroup):
    ...
    def get_block(self, distance, **kwargs):
        block = Block(**kwargs)
        block.move_to(
            self.floor.get_top()[1] * UP + (self.wall.get_right()[0] + distance) * RIGHT,
            DL,
        )
        return block
...
class StepsOfTheAlgorithm(TeacherStudentsScene):
    ...
    def get_steps(self):
        return VGroup(
            ...
            OldTexText(
                "Step 2:",
                "Choose the number of digits, $d$,\\\\of $\\pi$ that you want to compute",
            ),
            OldTexText("Step 3:", "Set one mass to $100^{d - 1}$,\\\\the other to $1$"),
...
class CompareToGalacticMass(Scene):
    ...
    def add_digits_of_pi(self):
        ...
        highlighted_digits = VGroup(
            *[d.copy().set_background_stroke(color=BLUE, width=5) for d in [digits[0], *digits[2:-3]]]
        )
        counter = Integer(0)
        counter.scale(1.5)
