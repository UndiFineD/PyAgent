    ...
    def show_value_on_equations(self):
        ...
        self.play(
            *[
                ...
                *[ApplyMethod(b.next_to, vt, UP, SMALL_BUFF) for b, vt in zip(v_braces, momentum_v_terms)],
            ]
        )
        self.unhalt()
...
class IntroduceVelocityPhaseSpace(AskAboutFindingNewVelocities):
    ...
    def show_two_equations(self):
        ...
        v1_terms, v2_terms = v_terms = VGroup(
            *[VGroup(*[expr.get_parts_by_tex(tex) for expr in equations]) for tex in ("v_1", "v_2")]
        )
        for eq in equations:
    ...
    def draw_ellipse(self):
        ...
        ellipse.replace(
            Polygon(*[axes.coords_to_point(x, y * np.sqrt(mass_ratio)) for x, y in [(1, 0), (0, 1), (-1, 0), (0, -1)]]),
            stretch=True,
        )
...
class AnalyzeCircleGeometry(CircleDiagramFromSlidingBlocks, MovingCameraScene):
    ...
    def show_equal_arc_lengths(self):
        ...
        lower_arcs = VGroup(
            *[Arc(start_angle=(PI + n * 2 * theta), angle=(2 * theta), radius=radius) for n in range(n_arcs + 1)]
        )
