# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\QA_2to21.py
...
class Questions(Scene):
    def construct(self):
        ...
        questions = VGroup(
            OldTexText("Who is your favorite mathematician?"),
            OldTexText(
            ...
            OldTexText("Is Ben, Ben and Blue still a thing?"),
            OldTexText("Favorite podcasts?"),
            OldTexText(
...
class Cumulative(Scene):
    def construct(self):
        ...
        checks, crosses = [
            VGroup(*[mob.copy().next_to(rect, RIGHT, SMALL_BUFF) for rect in rects]) for mob in [check, cross]
        ]
        ...
        self.play(
            ...
            LaggedStart(*[ReplacementTransform(cross, check) for cross, check in zip(crosses[8:], checks[8:])]),
        )
        self.wait()
