# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\valentines.py
...
class PowersOfTwo(MovingCameraScene):
    def construct(self):
        ...
        dots = VGroup()
        numbers = VGroup()
        points = [rotate_vector(R * DOWN, k * TAU / n) for k in range(n)]
        dots = VGroup(*[Dot(point, radius=0.03, color=YELLOW) for point in points])
        numbers = VGroup(*[Integer(k).scale(0.2).move_to((1.03) * point) for k, point in enumerate(points)])
        lines = VGroup(*[Line(points[k], points[(2 * k) % n]) for k in range(n)])
        ...
        title = OldTex("\\mathds{Z} / (101 \\mathds{Z})")
        title.scale(2)
        ...
        self.play(
            ...
            LaggedStart(*[FadeIn(n, -normalize(n.get_center())) for n in numbers]),
            run_time=2,
        )
        self.play(frame.scale, 0.25, {"about_point": circle.get_bottom() + SMALL_BUFF * DOWN})
        n_examples = 6
        for k in range(1, n_examples):
            self.play(ShowCreation(lines[k]), transforms[k], ShowCreation(arrows[k]))
        self.play(frame.restore, FadeOut(arrows[:n_examples]))
        self.play(
        ...
        self.play(
            LaggedStart(*[ShowCreationThenFadeOut(line.copy().set_stroke(PINK, 3)) for line in lines]),
            run_time=3,
        )
        self.wait(4)
...
class Cardiod(Scene):
    def construct(self):
        ...
        def get_lil_circle():
            ...
            angle = 0.5 * time
            lil_circle.shift(rotate_vector(UP, angle) * 2 * r)
            return lil_circle
