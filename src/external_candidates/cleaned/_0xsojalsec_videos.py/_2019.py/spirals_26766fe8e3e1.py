...
class PrimesAndPi(Scene):
    ...
    def show_primes(self):
        ...
        self.play(
            LaggedStart(*[ApplyFunction(lambda m: m.set_color(TEAL).scale(1.2), prime) for prime in primes]),
...
class RefresherOnPolarCoordinates(Scene):
    ...
    def get_polar_grid(self, radius=25):
        ...
        labels = VGroup(
            *[Integer(n).scale(0.5).next_to(plane.c2p(n, 0), DR, SMALL_BUFF) for n in range(1, int(radius))]
        )
...
class ShowSpiralsForWholeNumbers(CountSpirals):
    ...
    def count_44_spirals(self):
        ...
        new_spirals = PGroup(
            *[self.get_p_spiral(range((INV_7_MOD_44 * k) % 44, self.max_prime, 44)) for k in range(44)]
        )
...
class ExplainSixSpirals(ShowSpiralsForWholeNumbers):
    ...
    def comment_on_arms(self):
        ...
        rect_groups = VGroup(
            *[
                VGroup(*[SurroundingRectangle(label, stroke_width=2, buff=0.05) for label in group])
    ...
    def talk_though_multiples_of_six(self):
        ...
        def show_steps(start, stop, added_anims=None, run_time=2):
            if added_anims is None:
                added_anims = []
            self.play(
                *[ShowSubmobjectsOneByOne(group[start : stop + 1]) for group in [arcs, boxes, lines]],
                *added_anims,
                rate_func=linear,
                run_time=run_time,
            )
...
class Explain44Spirals(ExplainSixSpirals):
    ...
    def show_44_steps(self):
        ...
        def show_steps(start, stop, added_anims=None, run_time=2):
            ...
            self.play(
                *[ShowSubmobjectsOneByOne(group[start : stop + 1]) for group in [arcs, opaque_labels, lines]],
                *added_anims,
                rate_func=rate_func,
                run_time=run_time,
            )
...
class IntroduceTotientJargon(TeacherStudentsScene):
    ...
    def eliminate_non_coprimes(self):
        number_grid = VGroup(
            *[VGroup(*[Integer(n) for n in range(11 * k, 11 * (k + 1))]).arrange(DOWN) for k in range(4)]
        ...
        for group in evens, div11:
            ...
            self.play(
                LaggedStart(*[ApplyMethod(number.set_opacity, 0.2) for number in group]),
...
class DiscussPrimesMod10(Scene):
    def construct(self):
        ...
        sequences = VGroup(
            *[VGroup(*[Integer(n).shift((n // 10) * RIGHT) for n in range(r, 100 + r, 10)]) for r in range(10)]
        )
        ...
        # Highlight primes
        self.play(
            LaggedStart(*[ApplyFunction(lambda m: m.scale(1.2).set_color(TEAL), prime) for prime in primes]),
            LaggedStart(
                *[ApplyFunction(lambda m: m.scale(0.8).set_opacity(0.8), non_prime) for non_prime in non_primes]
            ),
        )
        self.wait()
...
class DirichletIn1837(Scene):
    def construct(self):
        ...
        d_arrow, rh_arrow, pnt_arrow = arrows = VGroup(
            *[Vector(DOWN).next_to(timeline.n2p(date), UP) for date in [1837, 1859, 1896]]
        )
