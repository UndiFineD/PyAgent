# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\patterns.py
...
class PascalColored(Scene):
    ...
    def construct(self):
        ...
        for layer in layers[1:]:
            ...
            for i, dot in enumerate(layer):
                ...
                self.play(*[ReplacementTransform(pre_dot.copy(), dot, run_time=rt) for pre_dot in pre_dots])
            last_layer = layer
            triangle.add(layer)
    ...
    def get_dot_layer(self, n):
        ...
        for k in range(n + 1):
            ...
            dot = Dot(point, radius=2 * self.dot_radius, color=self.colors[nCk_residue])
            if n <= 9:
...
class TriominoGrid(Scene):
    ...
    def cover_grid(self, grid):
        ...
        for q in quads:
            squares = [square for row in q for square in row]
            q.has_covered = any([s.is_covered for s in squares])
            corner_index = np.argmin([get_norm(s.get_center() - grid.get_center()) for s in squares])
