# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\outside_videos\probability.py
        ...
        for x in range(self.n_iterations):
            ...
            for mob, vect in (bars, DOWN), (bars_copy, UP):
                ...
                if mob.target.get_width() > max_width:
                    ...
                    curr_buff = lx1 - rx0
                    mob.target.arrange(RIGHT, buff=0.9 * curr_buff, aligned_edge=DOWN)
            ...
            self.play(MoveToTarget(bars_copy, lag_ratio=0.5, run_time=np.sqrt(x + 1)))

            # Resize lower bars
            for top_bar, low_bar in zip(bars_copy[:-1], bars[1:]):
                bottom = low_bar.get_bottom()
                low_bar.replace(VGroup(low_bar, top_bar), stretch=True)
