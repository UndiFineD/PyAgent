# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\custom\drawings.py
...
class Car(SVGMobject):
    ...
    def add_treds_to_tires(self):
        for tire in self.get_tires():
            ...
            tred = Line(0.7 * radius * RIGHT, 1.1 * radius * RIGHT, stroke_width=2, color=BLACK)
...
class SunGlasses(SVGMobject):
    ...
    def __init__(self, pi_creature, **kwargs):
        ...
        self.set_width(self.glasses_width_to_eyes_width * pi_creature.eyes.get_width())
...
class DeckOfCards(VGroup):
    def __init__(self, **kwargs):
        ...
        super().__init__(
            *(PlayingCard(value=value, suit=suit, **kwargs) for value in possible_values for suit in possible_suits)
        )
...
class PlayingCard(VGroup):
    ...
    def get_symbol(self):
        ...
        if suit is None:
            if self.key is not None:
                suit = dict([(s[0].upper(), s) for s in self.possible_suits])[self.key[-1].upper()]
            else:
    ...
    def get_number_design(self, value, symbol):
        ...
        column_points = [interpolate(top, bottom, alpha) for alpha in np.linspace(0, 1, n_rows)]
        ...
        design.add(*[symbol.copy().move_to(center_of_mass(column_points[i : i + 2])) for i in insertion_indices])
    ...
    def get_corner_numbers(self, value, symbol):
        ...
        value_mob.next_to(self.get_corner(UP + LEFT), DOWN + RIGHT, buff=MED_LARGE_BUFF * width)
