# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\efvgt.py
...
def get_composite_rotation_angle_and_axis(angles, axes):
    ...
    for angle2, axis2 in zip(angles, axes):
        ...
        angle3 = 2 * np.arccos(np.cos(angle2 / 2) * np.cos(angle1 / 2) - np.sin(angle2 / 2) * np.sin(angle1 / 2) * dot)
        axis3 = (
...
def get_confetti_animations(num_confetti_squares):
    ...
    confetti_spirils = [
        ...
        for a, square in zip(np.linspace(0, 0.5, num_confetti_squares), confetti_squares)
    ]
...
class Anniversary(TeacherStudentsScene):
    ...
    def celebrate(self):
        ...
        confetti_spirils = get_confetti_animations(self.num_confetti_squares)
        self.play(
            Write(title, run_time=2),
            *[ApplyMethod(pi.change_mode, "hooray") for pi in self.get_pi_creatures()],
        )
        self.play(
            DrawBorderThenFill(
                hats,
    ...
    def get_party_hats(self):
        hats = VGroup(*[PartyHat(pi_creature=pi, height=0.5 * pi.get_height()) for pi in self.get_pi_creatures()])
        max_angle = np.pi / 6
        for hat in hats:
            hat.rotate(
...
class QuickExplanation(ComplexTransformationScene):
    ...
    def add_equation(self):
        equation = OldTex("\\frac{d(e^{it})}{dt}", "=", "i", "e^{it}")
    ...
    def add_vectors(self):
        ...
        self.play(ReplacementTransform(s_vector.copy(), v_vector, path_arc=np.pi / 2))
        self.wait()
        ...
        self.play(ShowCreation(circle, **kwargs), rotation)
        self.play(rotation)
        self.play(rotation)
...
class SymmetriesOfSquare(ThreeDScene):
    ...
    def add_labels(self):
        ...
        self.play(
            ...
            self.randy.change_mode,
            "happy",
            self.randy.look_at,
            self.square.randy.eyes,
        )
    ...
    def show_full_group(self):
        ...
        self.play(*[ApplyMethod(mob.set_color, GREY) for mob in self.title[1:]])
    ...
    def name_dihedral_group(self):
        new_title = OldTexText("``Dihedral group'' of order 8")
    ...
    def rotate_square(
        ...
        **kwargs,
        ...
        rotation = Rotate(square, angle=angle, axis=axis, about_point=square.get_center(), **kwargs)
        ...
        if show_axis:
            axis_line = self.get_axis_line(square, axis)
            self.play(ShowCreation(axis_line), Animation(square))
        self.play(rotation, *added_anims)
        if show_axis:
            self.play(FadeOut(axis_line), Animation(square))

    def flip_square(self, axis=UP, **kwargs):
        self.rotate_square(angle=np.pi, axis=axis, show_axis=True, **kwargs)

    def get_rotation_arcs(self, square, angle, angle_buff=SMALL_BUFF):
        square_radius = get_norm(square.get_points()[0] - square.get_center())
        arc = Arc(
        ...
        arcs = VGroup(arc, *[arc.copy().rotate(i * np.pi / 2) for i in angle_multiple_range])
...
class CircleSymmetries(Scene):
    ...
    def add_circle_and_title(self):
        ...
        self.wait()
        angles = [np.pi / 2, -np.pi / 3, 5 * np.pi / 6, 3 * np.pi / 2 + 0.1]
    ...
    def associate_rotations_with_points(self):
        ...
        self.play(ShowCreation(zero_arrow), DrawBorderThenFill(zero_dot))
        ...
        for alpha in 0.2, 0.6, 0.4, 0.8:
            ...
            self.play(ShowCreation(arrow), DrawBorderThenFill(dot))
            self.play(Rotate(self.circle, angle, run_time=2), Animation(dot))
            self.wait()
            self.play(
    ...
    def get_circle(self):
        ...
        circle.ticks = VGroup()
        for alpha in np.arange(0, 1, 1.0 / 8):
    ...
    def add_radial_line(self):
        radius = Line(self.circle.get_center(), self.circle.point_from_proportion(0))
    ...
    def get_arc_circle(self):
        ...
        arc_circle = Circle(radius=arc_radius, color=WHITE)
        return arc_circle
...
class GroupOfCubeSymmetries(ThreeDScene):
    ...
    def construct(self):
        ...
        for angle, axis in angle_axis_pairs:
            self.play(Rotate(cube, angle=angle, axis=axis, run_time=2))
            self.wait()
    ...
    def get_cube(self):
        ...
        cube.apply_function(lambda p: np.dot(p, pose_matrix.T), maintain_smoothness=False)
        return cube

    def get_pose_matrix(self):
        return np.dot(rotation_matrix(np.pi / 8, UP), rotation_matrix(np.pi / 24, RIGHT))
...
class AddSquareSymmetries(SymmetriesOfSquare):
    def construct(self):
        ...
        self.rotate_square(square=square, angle=np.pi / 2, added_anims=list(map(FadeIn, equation[:2])))
        self.wait()
        self.play(FadeOut(arcs))
        self.flip_square(square=square, axis=UP, added_anims=list(map(FadeIn, equation[2:4])))
        self.wait()
        ...
        self.play(square.next_to, equals, LEFT, LARGE_BUFF, alt_square.restore, Write(equals))
        self.flip_square(
            square=alt_square,
            axis=UP + RIGHT,
...
class AddCircleSymmetries(CircleSymmetries):
    def construct(self):
        ...
        arcs = [arc_circle.copy().scale(scalar) for scalar in [1, 1.2, 1.4]]
        ...
        def rotate(circle, angle, arc, terms):
            self.play(
                ...
                ShowCreation(arc, rate_func=lambda t: (angle / (2 * np.pi)) * smooth(t)),
                Write(VGroup(*terms)),
                run_time=2,
            )
        ...
        self.play(circle.next_to, equals, LEFT, LARGE_BUFF, alt_circle.restore, Write(equals))
        arcs[2].shift(alt_circle.get_center())
        alt_circle.remove(alt_static_radius)
        self.wait()
        rotate(alt_circle, angles[2], arcs[2], equation[4:])
        self.wait()
        self.play(Rotate(arcs[1], angles[0], about_point=circle.get_center()))
        self.wait(2)
        for term, arc in zip(equation[::2], arcs):
            self.play(*[ApplyMethod(mob.scale, 1.2, rate_func=there_and_back) for mob in (term, arc)])
            self.wait()
...
class AddCubeSymmetries(GroupOfCubeSymmetries):
    ...
    def construct(self):
        ...
        angle_axis_pairs.append(self.get_composition_angle_and_axis())
        self.pose_matrix = self.get_pose_matrix()
        cube = self.get_cube()

        equation = cube1, plus, cube2, equals, cube3 = VGroup(cube, OldTex("+"), cube.copy(), OldTex("="), cube.copy())
    ...
    def rotate_cube(self, cube, angle, axis, add_arrows=True, **kwargs):
        ...
        if add_arrows:
            arrows = VGroup(
                *[Arc(start_angle=np.pi / 12 + a, angle=5 * np.pi / 6, color=YELLOW).add_tip() for a in (0, np.pi)]
            )
            arrows.set_height(1.5 * cube.get_height())
            z_to_axis = z_to_vector(axis)
            arrows.apply_function(lambda p: np.dot(p, z_to_axis.T), maintain_smoothness=False)
            arrows.move_to(cube)
            arrows.shift(-axis * cube.get_height() / 2 / get_norm(axis))
            anims += list(map(ShowCreation, arrows))
        anims.append(Rotate(cube, axis=axis, angle=angle, in_place=True, **kwargs))
        self.play(*anims, run_time=1.5)

    def get_composition_angle_and_axis(self):
        return get_composite_rotation_angle_and_axis(*list(zip(*self.angle_axis_pairs)))


class DihedralGroupStructure(SymmetriesOfSquare):
    CONFIG = {
        "dashed_line_config": {"dash_length": 0.1},
    ...
    def demonstrate_sum(self, angle_axis_pairs):
        angle_axis_pairs = list(angle_axis_pairs) + [
            get_composite_rotation_angle_and_axis(*list(zip(*angle_axis_pairs)))
        ]
        ...
        for square, (angle, axis) in zip([s1, s2, s3], angle_axis_pairs):
            if np.cos(angle) > 0.5:
                ...
                square.action_illustration = self.get_rotation_arcs(square, angle)
            else:
                square.action_illustration = self.get_axis_line(square, axis)
        ...
        self.play(Write(equals), FadeIn(s3))
...
class ThisIsAVeryGeneralIdea(Scene):
    def construct(self):
        ...
        lines = VGroup(*[Line(groups.get_bottom(), ex.get_top(), buff=MED_SMALL_BUFF) for ex in examples])
        lines.set_color(groups.get_color())

        self.add(groups)

        for example, line in zip(examples, lines):
            self.play(ShowCreation(line), Write(example, run_time=2))
        self.wait()
        self.play(
            VGroup(*examples[:-1]).fade,
            0.7,
            VGroup(*lines[:-1]).fade,
            0.7,
        )
        ...
        self.play(
            numbers.scale,
            1.2,
            numbers.get_corner(UP + RIGHT),
        )
        self.wait(2)

        sub_categories = VGroup(
            *list(
                map(
                    TexText,
                    [
        ...
        sub_lines = VGroup(*[Line(numbers.get_bottom(), sc.get_top(), buff=MED_SMALL_BUFF) for sc in sub_categories])
        sub_lines.set_color(numbers.get_color())

        self.play(*it.chain(list(map(ShowCreation, sub_lines)), list(map(Write, sub_categories))))
        self.wait()
...
class AdditiveGroupOfReals(Scene):
    ...
    def construct(self):
        ...
        self.show_example_slides_labeled(5.5, added_anims=[self.get_write_name_of_group_anim()])
    ...
    def show_example_slides(self, *nums):
        for num in nums:
            ...
            self.play(self.number_line.shift, num_point - zero_point, run_time=2)
    ...
    def mark_zero(self):
        dot = Dot(self.number_line.number_to_point(0), color=self.zero_color)
    ...
    def show_example_slides_labeled(self, *nums, **kwargs):
        for num in nums:
            ...
            vect = self.number_line.number_to_point(num) - self.number_line.number_to_point(0)
            ...
            if "added_anims" in kwargs:
                ...
                self.wait()
            self.play(self.number_line.shift, -vect, FadeOut(line))
    ...
    def comment_on_zero_as_identity(self):
        ...
        self.play(ShowCreation(line), Write(words))
        self.wait(2)
    ...
    def show_example_additions(self, *num_pairs):
        for num_pair in num_pairs:
            ...
            for num in num_pair:
                zero_point, num_point, arrow, num_mob = self.get_adder_mobs(num)
                if len(num_mobs) > 0:
                ...
                self.play(ShowCreation(arrow), Write(num_mob, run_time=1))
                self.play(self.number_line.shift, num_point - zero_point)
                self.wait()
            # Reset
            self.play(FadeOut(num_mobs), FadeOut(self.number_line))
            ApplyMethod(self.number_line.restore).update(1)
            self.play(FadeIn(self.number_line))

            # Sum arrow
            num = sum(num_pair)
            zero_point, sum_point, arrow, sum_mob = self.get_adder_mobs(sum(num_pair))
            VGroup(arrow, sum_mob).shift(MED_LARGE_BUFF * UP)
            arrows.add(arrow)
            self.play(ShowCreation(arrow), Write(sum_mob, run_time=1))
            self.wait()
            self.play(self.number_line.shift, num_point - zero_point, run_time=2)
            self.wait()
            self.play(self.number_line.restore, *list(map(FadeOut, [arrows, sum_mob])))
...
class AdditiveGroupOfComplexNumbers(ComplexTransformationScene):
    ...
    def add_plane(self):
        ...
        zero_dot = Dot(self.z_to_point(0), color=ADDER_COLOR)
    ...
    def show_vertical_slide(self):
        dots = VGroup(*[Dot(self.z_to_point(complex(0, i))) for i in range(1, 4)])
    ...
    def show_example_point(self):
        ...
        self.play(Write(label), DrawBorderThenFill(dot))
        self.wait()
        self.play(ShowCreation(arrow))
    ...
    def show_example_addition(self):
        ...
        arrow2 = Vector(point2, buff=dot2.radius, color=dot2.get_color())
        label2 = OldTex("%d %di" % (z2.real, z2.imag))
        label2.next_to(point2, UP + RIGHT)
        label2.set_color(dot2.get_color())
        label2.add_background_rectangle()

        self.play(ShowCreation(arrow2))
        self.play(DrawBorderThenFill(dot2), Write(label2))
        ...
        real_arrow, imag_arrow = component_arrows = [
            Vector(self.z_to_point(z), color=ADDER_COLOR)
            for z in [
                z1.real + z2.real,
        ...
        plus = OldTex("+").next_to(real_arrow.get_center(), UP + RIGHT)
        ...
        real_background_rect = BackgroundRectangle(VGroup(rp1.target, rp2.target))
        imag_background_rect = BackgroundRectangle(VGroup(ip1.target, ip2.target))
    ...
    def write_group_name(self):
        title = OldTexText("Additive", "group of", "complex numbers")
    ...
    def slide(self, z, *added_anims, **kwargs):
        ...
        self.play(ApplyMethod(self.plane.shift, self.z_to_point(z), **kwargs), *added_anims)
...
class SchizophrenicNumbers(Scene):
    ...
    def blink_numbers(self, numbers):
        self.play(
            *[
                num.eyes.blink_anim(rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2))
                for num, alpha in zip(numbers[1:], 0.8 * np.random.random(len(numbers)))
            ]
        )
...
class MultiplicativeGroupOfReals(AdditiveGroupOfReals):
    ...
    def follow_one(self):
        ...
        self.play(ShowCreation(arrow), DrawBorderThenFill(dot), Write(words))
        self.number_line.add(dot)
        ...
        self.stretch(3)
        self.play(ShowCreation(three_line), Animation(self.one))
        dot_copy = dot.copy()
        self.play(dot_copy.move_to, three_line.get_bottom())
        self.play(Indicate(three_mob, run_time=2))
        self.wait()
        self.play(self.number_line.restore, *list(map(FadeOut, [three_line, dot_copy])))
        self.wait()
        self.stretch(0.5)
        self.play(ShowCreation(half_line), Animation(self.one))
        dot_copy = dot.copy()
        self.play(dot_copy.move_to, half_line.get_bottom())
        self.play(Write(half_label), ShowCreation(half_arrow))
        self.wait()
        self.play(
            self.number_line.restore,
            *list(map(FadeOut, [half_label, half_arrow, half_line, dot_copy])),
        )
        self.wait()
    ...
    def every_positive_number_association(self):
        ...
        self.play(self.zero_arrow.shift, 0.5 * UP, rate_func=there_and_back)
        self.wait()
        self.play(self.one_dot.shift, 0.25 * UP, rate_func=wiggle)
        self.stretch(3)
    ...
    def compose_actions(self, num1, num2):
        ...
        for num, word in zip([num1, num2], top_words):
            self.stretch(num, added_anims=[FadeIn(word)], run_time=3)
            self.wait()
    ...
    def write_group_name(self):
        new_title = OldTexText("Multiplicative group of positive real numbers")
        new_title.to_edge(UP)
        VGroup(*new_title[: len("Multiplicative")]).set_color(MULTIPLIER_COLOR)
        VGroup(*new_title[-len("positiverealnumbers") :]).set_color(self.positive_reals_color)
    ...
    def stretch(self, factor, run_time=2, **kwargs):
        ...
        for number in target.numbers:
            number.stretch_in_place(1.0 / factor, dim=0)
            if total_factor < 0.7:
...
class MultiplicativeGroupOfComplexNumbers(AdditiveGroupOfComplexNumbers):
    ...
    def add_title(self):
        title = OldTexText("Multiplicative", "group of", "complex numbers")
        title.to_edge(UP)
    ...
    def fix_zero_and_move_one(self):
        zero_arrow = Arrow(UP + 1.25 * LEFT, ORIGIN, buff=2 * self.dot_radius)
    ...
    def show_action_at_i(self):
        ...
        self.play(ShowCreation(i_arrow), DrawBorderThenFill(i_dot))
        self.wait()
        self.play(FadeOut(i_arrow), ShowCreation(arc))
    ...
    def show_action_at_i_again(self):
        ...
        self.multiply_by_z(complex(0, 1), run_time=3)
        self.wait()
        self.play(Transform(self.turn_arrow, half_turn_arc, path_arc=np.pi / 2))
        self.wait()
    ...
    def talk_through_specific_example(self):
        ...
        brace = Brace(Line(ORIGIN, self.z_to_point(np.sqrt(5))), UP)
        ...
        angle_label.next_to(arc, RIGHT, buff=SMALL_BUFF, aligned_edge=DOWN)
        ...
        self.multiply_by_z(
            ...
            added_anims=[ShowCreation(arc, run_time=2), Write(angle_label)],
        )
    ...
    def show_break_down(self):
        ...
        circle = Circle(radius=self.z_to_point(1)[0], color=MULTIPLIER_COLOR)
        real_actions = [3, 0.5, 1]
        rotation_actions = [np.exp(complex(0, angle)) for angle in np.linspace(0, 2 * np.pi, 4)[1:]]
...
class ExponentsAsRepeatedMultiplication(TeacherStudentsScene):
    ...
    def show_repeated_multiplication(self):
        ...
        for twos in three_twos, five_twos:
            ...
            twos.generate_target()
            twos.brace_anim = MaintainPositionRelativeTo(VGroup(twos.brace, twos.exp), twos)

        self.play(
            GrowFromCenter(three_twos.brace),
            Write(three_twos.exp),
            self.get_teacher().change_mode,
            "raise_right_hand",
        )
        ...
        self.play(FadeIn(five_twos, run_time=3, lag_ratio=0.5))
        self.wait(2)

        cdot = OldTex("\\cdot")
        lhs = OldTex("2^{%d + %d} = " % tuple(exponents))
        rule = VGroup(lhs, three_twos.target, cdot, five_twos.target)
        rule.arrange()
        ...
        self.play(
            *[
                ReplacementTransform(mob.copy(), term, run_time=2)
                for term, mob in zip(general_equation, [lhs, three_twos.exp, five_twos.exp])
            ]
        )
        self.wait(2)
    ...
    def show_non_counting_exponents(self):
        self.play(
            self.expanded_exponential_rule.scale,
            0.5,
            self.expanded_exponential_rule.to_corner,
            UP + LEFT,
        )
        ...
        self.play(
            ...
            *[ApplyMethod(pi.change_mode, "pondering") for pi in self.get_pi_creatures()],
        )
        for mob in alt_powers[1:]:
            self.play(Write(mob, run_time=1))
            self.wait()
        self.wait()
        self.play(*it.chain(*[[pi.change_mode, "confused", pi.look_at, half_power] for pi in self.get_students()]))
        ...
        half_expression = OldTex("\\big(", "2^{1/2}", "\\big)", "\\big(2^{1/2}\\big) = 2^{1}")
        neg_one_expression = OldTex("\\big(", "2^{-1}", "\\big)", "\\big( 2^{1} \\big) = 2^{0}")
        expressions = VGroup(half_expression, neg_one_expression)
        expressions.arrange(DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF)
        ...
        self.wait()
        self.play(Transform(neg_power, neg_one_expression[1]), Write(neg_one_expression))
        self.wait(2)
        self.play(
        ...
        self.play(
            ...
            imag_power.scale,
            1.5,
            imag_power.set_color,
            BLUE,
            self.exponential_rule.to_edge,
            RIGHT,
            self.get_teacher().change_mode,
            "speaking",
        )
        self.play(*it.chain(*[[pi.change_mode, "pondering", pi.look_at, imag_power] for pi in self.get_students()]))
        self.wait()

        group_theory_words = OldTexText("Group theory?")
        group_theory_words.next_to(self.exponential_rule, UP, buff=LARGE_BUFF)
        arrow = Arrow(group_theory_words, self.exponential_rule, color=WHITE, buff=SMALL_BUFF)
        group_theory_words.shift_onto_screen()
        self.play(Write(group_theory_words), ShowCreation(arrow))
        self.wait(2)
...
class ExponentsAsHomomorphism(Scene):
    ...
    def show_multipliers(self):
        ...
        self.play(
            multipliers.space_out_submobjects,
            4,
            multipliers.next_to,
            self.bottom_line_center,
            UP,
            MED_LARGE_BUFF,
            multipliers.set_color,
            YELLOW,
            FadeOut(self.rhs_brace_group),
            Write(bottom_line),
        )
        stretch_kwargs = {}
        for x in 3, 1.0 / 5, 5.0 / 3:
            self.play(self.get_stretch_anim(bottom_line, x), run_time=3)
        self.wait()
    ...
    def confused_at_mapping(self):
        ...
        self.play(ShowCreation(arrow), FadeIn(randy))
    ...
    def talk_through_composition(self):
        ...
        self.show_composition(*inputs, parallel_anims=list(map(MoveToTarget, self.adders)))
        self.play(
            FocusOn(self.bottom_line_center),
            randy.look_at,
            self.bottom_line_center,
        )
        self.show_composition(*inputs, parallel_anims=list(map(MoveToTarget, self.multipliers)))
        self.wait()
    ...
    def show_composition(self, *inputs, **kwargs):
        ...
        for x, parallel_anim in zip(inputs, parallel_anims):
            ...
            self.play(*anims)
            self.wait()
        self.play(*[line.restore for line in (self.top_line, self.bottom_line)])
    ...
    def get_stretch_anim(self, bottom_line, x):
        ...
        for number in target.numbers:
            number.stretch_in_place(1.0 / x, dim=0)
...
class DihedralCubeHomomorphism(GroupOfCubeSymmetries, SymmetriesOfSquare):
    def construct(self):
        ...
        title = OldTexText("``", "Homo", "morph", "ism", "''", arg_separator="")
        ...
        def_group = VGroup(homo_brace, homo_def, morph_brace, morph_def)
        ...
        VGroup(square, arrow, cube).next_to(equation, DOWN, buff=MED_LARGE_BUFF)
        ...
        for i, (angle, raw_axis) in enumerate(angle_axis_pairs):
            ...
            if i == 2:
                for group, color in (homo_group, YELLOW), (morph_group, BLUE):
                    ...
                    self.play(part.set_color, color, FadeIn(remainder))
...
class ComplexExponentiationAbstract:
    ...
    def show_example(self, z):
        self.apply_action(z, run_time=5, rate_func=there_and_back)
...
class ComplexExponentiationAdderHalf(ComplexExponentiationAbstract, AdditiveGroupOfComplexNumbers):
    CONFIG = {
        "group_type": "Additive",
        "color": GREEN_B,
        "vect": LEFT,
    ...
    def apply_action(self, z, run_time=2, **kwargs):
        kwargs["run_time"] = run_time
        self.play(
            ApplyMethod(self.plane.shift, self.z_to_point(z), **kwargs),
            *kwargs.get("added_anims", []),
        )
    ...
    def set_color_vertical_line(self):
        ...
        self.play(FadeOut(self.real_line), *list(map(ShowCreation, line)))
    ...
    def set_color_unit_circle(self):
        ...
        self.play(ReplacementTransform(line, circle, run_time=3))
        self.remove(circle)
        self.wait()
    ...
    def take_steps_for_e(self):
        ...
        braces = [Brace(Line(ORIGIN, x * UP), RIGHT, buff=SMALL_BUFF) for x in np.cumsum(slide_values)]
        labels = list(
            map(
                TexText,
                [
        ...
        for slide_value, label, brace in zip(slide_values, labels, braces):
            ...
            if curr_brace is None:
                curr_brace = brace
                curr_label = label
                self.play(GrowFromCenter(curr_brace), Write(curr_label))
...
class ComplexExponentiationMultiplierHalf(ComplexExponentiationAbstract, MultiplicativeGroupOfComplexNumbers):
    CONFIG = {
    ...
    def walk_input_up_vertical(self):
        ...
        arc, brace, curved_brace, radians_label = self.get_arc_braces_and_label(angle)
    ...
    def get_arc_braces_and_label(self, angle):
        ...
        curved_brace.apply_complex_function(np.exp, maintain_smoothness=False)

        half_point = arc.point_from_proportion(0.5)
        radians_label = OldTex("%.3f" % angle)
        radians_label.add_background_rectangle()
        radians_label.next_to(1.5 * half_point, np.round(half_point), buff=0)
    ...
    def take_steps_for_e(self):
        ...
        for last_angle, angle in zip([0] + angles, angles):
            ...
            if curr_brace is None:
                ...
                self.play(ReplacementTransform(brace, curr_brace), Write(curr_label))
...
class ExpComplexHomomorphismPreviewAdderHalf(ExpComplexHomomorphismPreviewAbstract, ComplexExponentiationAdderHalf):
    def construct(self):
...
class ECLPromo(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(self.pi_creature.change_mode, "hooray", ShowCreation(rect))
        self.wait(3)
        self.play(FadeIn(logo_part1, run_time=3, lag_ratio=0.5))
        ...
        self.play(logo_part2.scale, 0.5, logo_part2.to_edge, UP)
        self.play(logo_part2.restore, self.pi_creature.change_mode, "happy")
        self.play(self.pi_creature.look_at, rect)
        self.wait(10)
        self.play(self.pi_creature.change_mode, "pondering", self.pi_creature.look, DOWN)
        self.wait(10)
...
class ExpTransformation(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        self.play(Rotate(self.plane, -np.pi / 3, UP, run_time=5))
