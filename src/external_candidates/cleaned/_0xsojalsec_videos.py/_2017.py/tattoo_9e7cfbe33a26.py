# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\tattoo.py
...
class Introduce(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Something different today!", target_mode="hooray", run_time=2)
...
class ReactionsToTattoo(PiCreatureScene):
    def construct(self):
        ...
        self.wait(2)
        for mode in modes:
            self.play(self.pi_creature.change_mode, mode, self.pi_creature.look, UP + RIGHT)
            self.wait(2)
        self.play(
...
class ExplainTrigFunctionDistances(TrigRepresentationsScene, PiCreatureScene):
    ...
    def introduce_angle(self):
        ...
        self.play(Write(words), ShowCreation(arrow), ShowCreation(dot))
        self.wait()
        self.play(GrowFromCenter(brace), Write(one))
        self.wait(2)
        self.play(*list(map(FadeOut, [words, arrow, brace, one])))
    ...
    def show_sine_and_cosine(self):
        ...
        self.play(
            Succession(
                *[Transform(mover, target, rate_func=linear) for target in targets],
                run_time=5,
    ...
    def show_tangent_and_cotangent(self):
        ...
        self.wait()
        self.play(*list(map(FadeOut, [tan_frac, cot_frac, self.sin_group, self.cos_group])))
        self.wait()

        self.play(
            self.theta_group[-1].set_color,
            YELLOW,
            ShowCreation(line),
            self.pi_creature.change_mode,
            "pondering",
    ...
    def show_secant_and_cosecant(self):
        ...
        sec_dot, csc_dot = [Dot(line.get_end(), color=line.get_color()) for line in (sec_line, csc_line)]
        ...
        for text, frac in (sec_text, sec_frac), (csc_text, csc_frac):
            ...
            self.play(Write(frac), self.pi_creature.change_mode, "erm")
            self.wait()
        self.wait()
        ...
        for theta in thetas:
            ...
            for group in new_sec_group, new_csc_group:
                line = group[0]
                group.add(Dot(line.get_end(), color=line.get_color()))
            targets.append(
                VGroup(
                    new_sec_group,
                    new_csc_group,
        ...
        self.play(*list(map(FadeOut, [sec_group, sec_frac])))
    ...
    def explain_cosecant(self):
        ...
        opp_over_hyp = OldTex("\\frac{\\text{Opposite}}{\\text{Hypotenuse}} =")
        ...
        self.play(ApplyMethod(tri1.rotate, np.pi / 12, rate_func=wiggle))
        self.wait(2)
        self.play(
            Write(question),
            ShowCreation(arrow),
        ...
        self.play(tri1.restore, csc_subgroup.fade, 0.7)
        self.play(ShowCreation(sin_line), GrowFromCenter(sin_brace), Write(sin_text))
        self.wait()
        self.play(Write(one))
        self.wait()
        self.play(Write(frac1))
        self.wait()
        self.play(Transform(tri1, tri2), FadeOut(sin_group))
        self.play(radial_line.rotate, np.pi / 12, rate_func=wiggle)
        self.wait()
        ...
        self.play(
            *list(
                map(
                    FadeOut,
                    [one, self.csc_group, tri1, frac1, frac2, self.csc_frac, arc_theta],
                )
            )
        )
    ...
    def reciprocate(self, frac1, frac2):
        ...
        self.wait()
        self.play(dem2.move_to, frac2[2], VGroup(*frac2[1:3]).set_fill, BLACK, 0)
        self.wait()
    ...
    def summarize_full_group(self):
        ...
        for func_name in ["sin", "tan", "sec", "cos", "cot", "csc"]:
            ...
            if func_name in ["sin", "cos"]:
                ...
                self.play(ShowCreation(line), Write(text, run_time=1))
            else:
                self.play(ShowCreation(line), GrowFromCenter(brace), Write(text, run_time=1))
        ...
        self.play(FadeOut(to_fade), self.pi_creature.change_mode, "sassy")
        self.wait(2)
    ...
    def get_tangent_line(self):
        return Line(
            self.unit_length * (1.0 / np.sin(self.theta_value)) * UP,
            self.unit_length * (1.0 / np.cos(self.theta_value)) * RIGHT,
            color=GREY,
        )
...
class RenameAllInTermsOfSine(Scene):
    def construct(self):
        ...
        self.play(*it.chain(*[list(map(MoveToTarget, mobs)), [Write(rhs_mobs)]]))
        self.wait(2)
        ...
        for mob, rhs, new_rhs in zip(mobs[-2:], rhs_mobs[-2:], new_rhs_mobs):
            new_rhs.next_to(mob)
            VGroup(mob.target, new_rhs).move_to(VGroup(mob, rhs))
            anims += [MoveToTarget(mob), Transform(rhs, new_rhs)]
        self.play(*anims)
        self.wait(2)
...
class MisMatchOfCoPrefix(TeacherStudentsScene):
    def construct(self):
        eq1 = OldTex("\\text{secant}(\\theta) = \\frac{1}{\\text{cosine}(\\theta)}")
        eq2 = OldTex("\\text{cosecant}(\\theta) = \\frac{1}{\\text{sine}(\\theta)}")
        ...
        self.play(
            ...
            "speaking",
            Write(eqs),
            *[ApplyMethod(pi.look_at, eqs) for pi in self.get_students()],
        )
        self.random_blink()
        self.play(
            VGroup(*eq1[-9:-7]).set_color,
            YELLOW,
            VGroup(*eq2[:2]).set_color,
            YELLOW,
            *[ApplyMethod(pi.change_mode, "confused") for pi in self.get_students()],
        )
        self.random_blink(2)
...
class Credit(Scene):
    def construct(self):
        ...
        self.play(PiCreatureSays(morty, "Book recommendation!", target_mode="surprised"))
