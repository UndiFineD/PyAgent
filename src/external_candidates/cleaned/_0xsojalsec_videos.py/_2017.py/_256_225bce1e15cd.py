# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\256.py
from _2017.crypto import BitcoinLogo, bit_string_to_mobject, sha256_tex_mob
from manim_imports_ext import *


def get_google_logo():
    result = SVGMobject(file_name="google_logo", height=0.75)
    blue, red, yellow, green = ["#4885ed", "#db3236", "#f4c20d", "#3cba54"]
...
class BreakUp2To256(PiCreatureScene):
    ...
    def add_number(self):
        ...
        number, possibilities = expression = OldTexText("$2^{256}$", "possibilities")
        ...
        self.wait()
        self.play(self.pi_creature.change, "maybe", Write(words))
        self.wait(2)
        self.play(self.pi_creature.change, "happy", FadeOut(words))
        self.wait()
    ...
    def break_up_as_powers_of_two(self):
        ...
        subgroups = [VGroup(*bits.target[32 * i : 32 * (i + 1)]) for i in range(8)]
        subexpressions = VGroup()
        for i, subgroup in enumerate(subgroups):
            subgroup.shift(i * MED_LARGE_BUFF * DOWN)
            subexpression = OldTexText("$2^{32}$", "possibilities")
        ...
        self.play(
            ...
            ReplacementTransform(VGroup(self.expression), subexpressions),
            MoveToTarget(bits),
        )
    ...
    def break_up_as_four_billions(self):
        ...
        for subexpression in self.subexpressions:
            new_subexpression = OldTexText("4 Billion", "possibilities")
    ...
    def reorganize_four_billions(self):
        target = VGroup(*[OldTexText("$\\big($", "4 Billion", "$\\big)$", arg_separator="") for x in range(8)])
        target.arrange(RIGHT, buff=SMALL_BUFF)
        target.to_edge(UP)
        target.set_width(FRAME_WIDTH - LARGE_BUFF)
        parens = VGroup(*it.chain(*[[t[0], t[2]] for t in target]))
        target_four_billions = VGroup(*[t[1] for t in target])
        target_four_billions.set_color(YELLOW)
        four_billions, to_fade = [VGroup(*[se[i] for se in self.subexpressions]) for i in range(2)]
...
class MainBreakdown(Scene):
    ...
    def add_four_billions(self):
        ...
        for x in range(8):
            mob = OldTexText("$\\big($", "4 Billion", "$\\big)$", arg_separator="")
            top_line.add(mob)
    ...
    def show_gpu(self):
        ...
        hash_names = VGroup(*[OldTexText("hash") for x in range(10)])
        ...
        rate_words = OldTexText("$<$ 1 Billion", "Hashes/sec")
        rate_words.next_to(name, DOWN)

        self.play(FadeIn(name))
        self.play(DrawBorderThenFill(gpu))
        self.play(get_passing_flash(), FadeIn(hash_names))
        for x in range(2):
            self.play(get_passing_flash(), Animation(hash_names))
        self.play(Write(rate_words, run_time=2), get_passing_flash(), Animation(hash_names))
        self.play(get_passing_flash(), Animation(hash_names))
    ...
    def cram_computer_with_gpus(self):
        ...
        hps, h_line, target_laptop = self.get_fraction(0, OldTexText("H/s"), Laptop())
        hps.scale(0.7)
        ...
        self.play(
            LaggedStartMap(
                ApplyFunction,
                gpus,
                lambda g: (lambda m: m.scale(0.01).move_to(laptop), g),
                remover=True,
            )
        )
        self.wait()
        self.play(
            Transform(
                rate_words[0],
                four_billion.copy().set_color(BLUE),
                remover=True,
            ),
            four_billion.set_color,
            BLUE,
    ...
    def kilo_google(self):
        ...
        google.next_to(self.group_of_four_billion_things, UP, buff=LARGE_BUFF, aligned_edge=LEFT)
        google.shift(RIGHT)
        ...
        laptop, h_line, target_kilogoogle = self.get_fraction(1, Laptop(), self.get_kilogoogle())
        ...
        self.wait()
        self.play(four_billion.restore, FadeOut(self.group_of_four_billion_things))
        self.play(
    ...
    def half_all_people_on_earth(self):
        ...
        self.play(GrowFromCenter(earth), Write(people))
        self.wait()
    ...
    def four_billion_earths(self):
        ...
        earth, h_line, denom = self.get_fraction(3, self.get_earth(), self.get_galaxy())
        ...
        self.wait()
        self.play(
            ReplacementTransform(self.group_of_four_billion_things, VGroup(earth)),
            ShowCreation(h_line),
            FadeIn(denom),
    ...
    def four_billion_galxies(self):
        ...
        num, h_line, denom = fraction = self.get_fraction(4, self.get_galaxy(), OldTexText("GGSC").set_color(BLUE))

        name = OldTexText("Giga", "Galactic \\\\", " Super", " Computer", arg_separator="")
        for word in name:
            word[0].set_color(BLUE)
        ...
        self.play(
            ...
            ShowCreation(h_line),
            ReplacementTransform(name, denom),
        )
        self.wait()
    ...
    def show_time_scale(self):
        ...
        universe_lifetimes.next_to(years_to_eons[1], DOWN, aligned_edge=LEFT)

        for fb, words in (fb1, seconds_to_years), (fb2, years_to_eons):
            self.play(fb.scale, 1.3, fb.next_to, words, LEFT, fb.set_color, BLUE, Write(words))
            self.wait()
    ...
    def show_probability(self):
        ...
        words = OldTexText("1 in ", "4 Billion\\\\", "chance of success")
        words.next_to(four_billion, DOWN, buff=MED_LARGE_BUFF)
        words.to_edge(RIGHT)
        words[1].set_color(BLUE)

        self.play(Write(VGroup(*words[::2])), Transform(four_billion, words[1]))
        self.wait()
    ...
    def create_four_billion_copies(self, index, mobject):
        ...
        group = VGroup(
            *[
                VGroup(*[mobject.copy().set_height(0.25) for x in range(self.n_group_rows)]).arrange(
        ...
        self.play(
            four_billion.scale,
            2,
            four_billion.next_to,
            brace,
            LEFT,
            GrowFromCenter(brace),
            LaggedStartMap(FadeIn, group, run_time=3, lag_ratio=0.2),
        )
        self.wait()
...
class StateOfBitcoin(TeacherStudentsScene):
    def construct(self):
        ...
        rate = OldTexText("5 Billion Billion", "$\\frac{\\text{Hashes}}{\\text{Second}}$")
        ...
        asic = OldTexText("Application", "Specific\\\\", "Integrated", "Circuit")
        for word in asic:
        ...
        self.play(Write(title), DrawBorderThenFill(bitcoin_logo))
        self.play(
            self.teacher.change,
            "raise_right_hand",
            rate.restore,
        ...
        self.play_student_changes(
            *["plain"] * 3,
            added_anims=[all_text.to_edge, LEFT, self.teacher.change_mode, "happy"],
            look_at=gpu,
        )
        self.play(Write(gpu_name), DrawBorderThenFill(gpu))
        ...
        self.play_student_changes(*["pondering"] * 3, added_anims=[Write(asic_rate)])
        self.play(
            LaggedStartMap(
                FadeIn,
                circuit,
...
class QAndA(PiCreatureScene):
    def construct(self):
        ...
        num_subscriber_words = OldTex("> 2^{18} = 262{,}144", "\\text{ subscribers}")
        ...
        self.wait()
        self.play(FadeIn(twitter), self.pi_creature.change_mode, "shruggie")
        self.wait(2)
    ...
    def show_powers_of_two(self):
        ...
        cols = 64
        dots = VGroup(
            *[VGroup(*[Dot() for x in range(rows)]).arrange(DOWN, buff=SMALL_BUFF) for y in range(cols)]
        ).arrange(RIGHT, buff=SMALL_BUFF)
        dots.set_width(FRAME_WIDTH - 2 * LARGE_BUFF)
        dots.next_to(self.pi_creature, UP)
        dots = VGroup(*it.chain(*dots))
        top = dots.get_top()
        dots.sort(lambda p: get_norm(p - top))

        powers_of_two = VGroup(*[Integer(2**i) for i in range(int(np.log2(rows * cols)) + 1)])

        curr_power = powers_of_two[0]
        curr_power.to_edge(UP)
        self.play(Write(curr_power), FadeIn(dots[0]))
        for i, power_of_two in enumerate(powers_of_two):
            ...
            self.play(
                ...
                Transform(curr_power, power_of_two, rate_func=squish_rate_func(smooth, 0, 0.5)),
            )
        self.wait()
        self.play(FadeOut(dots), FadeOut(powers_of_two))
