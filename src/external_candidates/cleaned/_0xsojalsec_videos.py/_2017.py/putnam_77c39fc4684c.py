...
class IntroducePutnam(Scene):
    ...
    def construct(self):
        ...
        question_groups = VGroup(
            *[
                VGroup(*[OldTexText("%s%d)" % (c, i)) for i in range(1, 7)]).arrange(DOWN, buff=MED_LARGE_BUFF)
                for c in ("A", "B")
            ]
...
class TwoDCase(Scene):
    ...
    def get_point_mobs(self):
        points = np.array(
            [self.center + rotate_vector(self.radius * RIGHT, theta) for theta in 2 * np.pi * np.random.random(3)]
        )
        for index in 0, 1, 0:
    ...
    def change_point_mobs(self, d_thetas, *added_anims, **kwargs):
        anims = it.chain(
            self.update_animations,
            [self.get_point_mob_theta_change_anim(pm, dt) for pm, dt in zip(self.point_mobs, d_thetas)],
            added_anims,
        )
...
class BrilliantPuzzle(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(ApplyMethod, students, lambda m: (m.change, "horrified", m.test)),
        )
        self.wait()
    ...
    def create_pi_creatures(self):
        self.students = VGroup(
            *[
                PiCreature(color=random.choice([BLUE_C, BLUE_D, BLUE_E, GREY_BROWN])).scale(0.25).move_to(3 * vect)
