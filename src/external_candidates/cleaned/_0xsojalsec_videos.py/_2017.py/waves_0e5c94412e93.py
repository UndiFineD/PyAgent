...
class CurlRelationBetweenFields(ThreeDScene):
    ...
    def loop_in_M(self):
        M_vects = VGroup(
            *[
                Vector(rotate_vector(vect, np.pi / 2), normal_vector=IN, color=M_COLOR).shift(vect)
    ...
    def second_loop_in_E(self):
        E_vects = VGroup(
            *[Vector(1.5 * rotate_vector(vect, np.pi / 2)).shift(vect) for vect in compass_directions(8, LEFT)[1:]]
        )
        E_vects.set_color(E_COLOR)
...
class ShowVectorEquation(Scene):
    ...
    def show_graph(self):
        ...
        self.play(
            ReplacementTransform(self.components[0].get_part_by_tex("f_x").copy(), fx_equals_1),
        )
...
class ShowTipToTailSum(ShowVectorEquation):
    ...
    def add_kets(self):
        h_ket, v_ket = kets = VGroup(
            *[OldTex("\\cos(", "2\\pi", "f", "t", ")", "|\\!\\%sarrow\\rangle" % s) for s in ("right", "up")]
        )
        for ket in kets:
        ...
        self.play(
            *it.chain(
                ...
                [
                    UpdateFromAlphaFunc(
                        ov.vector,
                        self.generate_A_update(ov, A * np.array(ov.A_vect), np.array(ov.A_vect)),
                    )
    ...
    def add_phase_shift(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                v_ov.vector,
                self.generate_phi_update(v_ov, np.array([0, np.pi / 2, 0]), np.array(v_ov.phi_vect)),
            ),
        )
...
class EnergyOfWavesTeacherPortion(TeacherStudentsScene):
    ...
    def show_energy_equation(self):
        ...
        self.wait(3)
        self.play(
            ReplacementTransform(VGroup(*energy[-4:]).copy(), VGroup(*component_energy[:4])),
            ReplacementTransform(VGroup(*energy[-4:]).copy(), VGroup(*component_energy[4:])),
        )
...
class ShootPhotonThroughFilter(DirectionOfPolarizationScene):
    ...
    def expect_half_energy_to_be_absorbed(self):
        ...
        self.play(
            ...
            RED,
            3,
            *list(map(GrowFromCenter, lines)) + [Animation(self.pol_filter), Animation(self.frozen_photon.mobject)],
        )
        self.wait(2)
        self.move_camera(
            phi=0.8 * np.pi / 2,
...
class ThreeFilters(ShootPhotonThroughFilter):
    ...
    def show_changed_to_diagonal(self):
        ...
        self.move_camera(
            ...
            + [Rotate(v, np.pi / 2, axis=v.get_vector(), in_place=True, **kwargs) for v in photon.mobject]
            + [
...
class CompareWaveEquations(TeacherStudentsScene):
    ...
    def show_complex_plane(self):
        ...
        alpha_dot, beta_dot = [
            Dot(plane.coords_to_point(x, 0.5), radius=0.05, color=color) for x, color in [(-0.5, RED), (0.5, GREEN)]
        ]
    ...
    def show_vertically_polarized_light(self):
        ...
        for should_pass in bools:
            if should_pass:
                ...
                self.play(
                    blocked_v_photon,
                    self.get_filter_absorption_animation(self.pol_filter, blocked_v_photon),
                )
        self.wait()
