...
class IntroduceCostFunction(PreviewLearning):
    ...
    def reminder_of_weights_and_bias(self):
        ...
        self.play(
            ...
            *[ApplyFunction(self.color_edge_randomly, edge) for edge in neuron.edges_in],
        )
        self.play(
            LaggedStartMap(
                ApplyMethod,
    ...
    def feed_in_example(self):
        ...
        neurons.target = VGroup(
            *it.chain(
                ...
                [VectorizedPoint(layer0.dots.get_center()) for x in range(len(neurons) - n)],
                VGroup(*layer0.neurons[-n / 2 :]).set_fill(opacity=0),
            )
        )
    ...
    def show_small_cost_output(self):
        ...
        update_decimals = [
            ChangingDecimalWithColor(decimal, generate_decimal_update_func(decimal.number, t))
        ...
        self.play(
            *it.chain(
                update_decimals,
                term_updates,
                [ApplyMethod(neuron.set_fill, None, t) for neuron, t in zip(neurons, target_vect)],
            )
        )
        self.wait()
...
class ShowFullCostFunctionGradient(PreviewLearning):
    ...
    def show_gradient(self):
        ...
        for od, nd in zip(old_decimals, new_decimals):
            ...
            self.play(
                Transform(nd, VectorizedPoint(od.get_center()), lag_ratio=0.5, remover=True),
...
class GradientNudging(PreviewLearning):
    ...
    def change_weights_repeatedly(self):
        ...
        arrows = VGroup(
            *[Arrow(words.get_bottom(), edge_group.get_top(), color=WHITE) for edge_group in network_mob.edge_groups]
        )
...
class TestPerformance(PreviewLearning):
    ...
    def run_through_examples(self):
        ...
        rects = [
            ...
            for neuron, label in zip(self.network_mob.layers[-1].neurons, self.network_mob.output_labels)
        ]
...
class InterpretFirstWeightMatrixRows(TestPerformance):
    ...
    def show_all_pixel_array(self):
        ...
        self.play(
            ...
            None,
            2,
            *[ApplyMethod(edge.set_stroke, None, 0.25) for edge in edges if edge not in neurons[0].edges_in],
        )
        self.wait()
        last_neuron = None
...
class TrainOnImages(PreviewLearning, RandomlyLabeledImageData):
    ...
    def construct(self):
        ...
        groups = [
            self.get_training_group(image_name, label_name) for image_name, label_name in zip(image_names, label_names)
        ]
