# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\nn\part3.py
...
class LayOutPlan(Scene):
    def construct(self):
        ...
        self.play(Write(title), ShowCreation(h_line), ShowCreation(rect), run_time=2)
...
class TODOInsertFeedForwardAnimations(TODOStub):
    CONFIG = {"message": "Insert Feed Forward Animations"}


class TODOInsertStepsDownCostSurface(TODOStub):
    CONFIG = {"message": "Insert Steps Down Cost Surface"}


class TODOInsertDefinitionOfCostFunction(TODOStub):
    CONFIG = {"message": "Insert Definition of cost function"}


class TODOInsertGradientNudging(TODOStub):
    CONFIG = {"message": "Insert GradientNudging"}
...
class InterpretGradientComponents(GradientNudging):
    ...
    def add_cost(self):
        ...
        cost_expression = OldTex("C(", "w_0, w_1, \\dots, w_{13{,}001}", ")", "=")
        for tex in "()":
        ...
        self.set_variables_as_attrs(cost, cost_expression, cost_group, network_rect=rect)

    def change_weights_repeatedly(self):
        decimals = self.grad_vect.decimals
        grad_terms = self.grad_vect.contents
        edges = VGroup(*reversed(list(it.chain(*self.network_mob.edge_groups))))
        cost = self.cost

        for x in range(self.n_steps):
            self.move_grad_terms_into_position(grad_terms.copy(), *self.get_weight_adjustment_anims(edges, cost))
            self.play(*self.get_decimal_change_anims(decimals))

    def ask_about_high_dimensions(self):
        grad_vect = self.grad_vect

        words = OldTexText("Direction in \\\\ ${13{,}002}$ dimensions?!?")
        words.set_color(YELLOW)
        words.move_to(grad_vect).to_edge(DOWN)
        arrow = Arrow(words.get_top(), grad_vect.get_bottom(), buff=SMALL_BUFF)
    ...
    def isolate_particular_weights(self):
        ...
        new_w_terms = OldTex("\\dots", "w_n", "\\dots", "w_k", "\\dots")
        ...
        for d, edge in (d1, edge1), (d2, edge2):
            d.arrow = Arrow(d.get_right(), edge.get_center(), color=d.get_color())
        ...
        self.set_variables_as_attrs(edge1, edge2, new_w_terms, new_decimals=VGroup(d1, d2))
    ...
    def tweak_individual_weights(self):
        ...
        number_line_template = NumberLine(x_min=-1, x_max=1, tick_frequency=0.25, big_tick_numbers=[], color=WHITE)
        ...
        groups = [VGroup(d, d.arrow, edge, w) for d, edge, w in zip(decimals, edges, [wn, wk])]
        for group in groups:
            group.save_state()
        ...
        for i in range(2):
            ...
            for x in range(2):
                func = lambda a: interpolate(cost_num, cost_num - change, a)
                self.play(
                ...
                self.wait()
            self.play(*list(map(FadeOut, [w.dot, w.brace, w.number_line, w.words])))
    ...
    def move_grad_terms_into_position(self, grad_terms, *added_anims):
        ...
        w_terms = self.cost_expression[1]
        points = VGroup(*[VectorizedPoint() for term in grad_terms])
        ...
        self.play(MoveToTarget(grad_terms), FadeIn(words))
        self.play(
            Transform(grad_terms, points, remover=True, lag_ratio=0.5, run_time=1),
            FadeOut(words),
            *added_anims,
        )
    ...
    def get_weight_adjustment_anims(self, edges, cost):
        ...
        return [
            ...
            ChangingDecimal(cost, lambda a: interpolate(start_cost, target_cost, a), run_time=1.5),
        ]
...
class GetLostInNotation(PiCreatureScene):
    def construct(self):
        ...
        equations = VGroup(
            ...
            OldTex("{\\partial", "C", "\\over \\partial", "b", "_j^l} =", "\\delta", "_j^l"),
            OldTex(
                "{\\partial",
                "C",
                " \\over \\partial",
        ...
        self.play(Write(equations), morty.change, "confused", equations)
        self.wait()
...
class TODOInsertPreviewLearning(TODOStub):
    CONFIG = {"message": "Insert PreviewLearning"}
...
class ShowAveragingCost(PreviewLearning):
    ...
    def setup_diff_words(self):
        ...
        double_arrow = DoubleArrow(self.network_mob.output_labels, last_layer_copy, color=RED)
        brace = Brace(VGroup(self.network_mob.layers[-1], last_layer_copy), UP)
        ...
        self.set_variables_as_attrs(last_layer_copy, double_arrow, brace, cost_words)
    ...
    def show_one_example(self, train_in, train_out):
        ...
        image.next_to(self.network_mob.layers[0].neurons, UP, aligned_edge=LEFT)
        self.add(image)
...
class WalkThroughTwoExample(ShowAveragingCost):
    ...
    def single_example_influencing_weights(self):
        ...
        def adjust_edge_group_anim(edge_group):
            return LaggedStartMap(
                ApplyFunction,
                edge_group,
                lambda edge: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), edge),
                rate_func=wiggle,
                run_time=1,
            )
        ...
        self.play(
            two.next_to,
            edge_groups[0].get_corner(DOWN + RIGHT),
            DOWN,
        ...
        self.play(
            ApplyMethod(
                two.next_to,
                edge_groups[1].get_corner(UP + RIGHT),
                UP,
                path_arc=np.pi / 6,
            ),
        ...
        self.play(
            ApplyMethod(
                two.next_to,
                edge_groups[2].get_corner(DOWN + RIGHT),
                DOWN,
                path_arc=-np.pi / 6,
            ),
    ...
    def expand_last_layer(self):
        ...
        movers = VGroup(neurons, alt_neurons, output_labels, alt_output_labels, *edges)
        ...
        n_pairs = it.product(self.network_mob.layers[-2].neurons, neurons.target)
        for edge, (n1, n2) in zip(edges, n_pairs):
            ...
            edge.target.put_start_and_end_on(c1 + unit_vect * r1, c2 - unit_vect * r2)

        self.play(FadeOut(to_fade), *list(map(MoveToTarget, movers)))
    ...
    def show_desired_activation_nudges(self, neurons, output_labels, alt_output_labels):
        ...
        self.play(*anims, run_time=3, rate_func=there_and_back)
    ...
    def show_activation_formula(self):
        ...
        self.wait()
        self.play(ReplacementTransform(prev_neurons.copy(), a_terms, path_arc=np.pi / 2))
        self.wait()
        self.play(Write(plus_terms[-1]), Write(b))
        self.wait()
        ...
        self.set_variables_as_attrs(rhs, w_terms, a_terms, b, lhs=neuron_copy)
    ...
    def three_ways_to_increase(self):
        ...
        increase_words.arrange(DOWN, aligned_edge=LEFT, buff=LARGE_BUFF)
        increase_words.to_edge(LEFT)

        mobs = [b, w_terms[0], a_terms[0]]
        for words, mob in zip(increase_words, mobs):
            self.play(Write(words[0], run_time=1), ReplacementTransform(mob.copy(), words[1]))
            self.wait()
    ...
    def note_connections_to_brightest_neurons(self):
        ...
        added_words.next_to(increase_words[1], DOWN, 1.5 * SMALL_BUFF, LEFT)
        added_words.set_color(YELLOW)

        terms_rect = SurroundingRectangle(VGroup(w_terms[0], a_terms[0]), color=WHITE)

        self.play(
            LaggedStartMap(
                ApplyFunction,
                edges,
                lambda edge: (lambda m: m.rotate(np.pi / 12).set_stroke(YELLOW), edge),
                rate_func=wiggle,
            )
        )
        self.wait()
        self.play(ShowCreation(bright_edges), ShowCreation(bright_neurons))
        self.play(
            LaggedStartMap(
                ApplyMethod,
                bright_neurons,
        ...
        self.wait()
        self.play(self.curr_image.shift, MED_LARGE_BUFF * RIGHT, rate_func=wiggle)
        self.wait()
        self.play(Write(added_words))
        self.wait()

        self.set_variables_as_attrs(bright_neurons, bright_edges, in_proportion_to_a=added_words)
    ...
    def fire_together_wire_together(self):
        ...
        def get_edge_animation():
            return LaggedStartMap(ShowCreationThenDestruction, bright_edges, lag_ratio=0.7)

        neuron_arrows = VGroup(*[Vector(MED_LARGE_BUFF * RIGHT).next_to(n, LEFT) for n in bright_neurons])
        ...
        neuron_rects = VGroup(*list(map(SurroundingRectangle, bright_neurons)))
        ...
        self.play(Write(words), morty.change, "speaking", words)
        self.play(Blink(morty))
        self.play(get_edge_animation(), morty.change, "pondering", bright_edges)
        self.play(get_edge_animation())
        self.play(
            LaggedStartMap(GrowArrow, neuron_arrows),
            get_edge_animation(),
        )
        self.play(GrowArrow(two_neuron_arrow), morty.change, "raise_right_hand", two_neuron)
        self.play(
    ...
    def show_desired_increase_to_previous_neurons(self):
        ...
        for s_edges in positive_edges, negative_edges:
            s_edges.alt_position = VGroup(*[Line(LEFT, RIGHT, color=s_edge.get_color()) for s_edge in s_edges])
        ...
        added_words.next_to(increase_words[-1], DOWN, SMALL_BUFF, aligned_edge=LEFT)

        self.play(
            LaggedStartMap(
                ApplyFunction,
                prev_neurons,
                lambda neuron: (lambda m: m.scale(0.5).set_color(YELLOW), neuron),
                rate_func=wiggle,
            )
        )
        self.wait()
        for positive in [True, False]:
            ...
            self.play(LaggedStartMap(GrowArrow, arrows), neurons.set_stroke, color)
            self.play(
                ApplyMethod(
                    neurons.set_fill,
                    color,
        ...
        self.play(
            ...
            None,
            0.8,
            ChangingDecimal(two_decimal, lambda a: two_neuron.get_fill_opacity()),
            run_time=3,
            rate_func=there_and_back,
        )
        self.wait()
    ...
    def only_keeping_track_of_changes(self):
        ...
        self.play(LaggedStartMap(Indicate, prev_neurons, rate_func=wiggle))
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenDestruction, edges))
    ...
    def show_other_output_neurons(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn,
                VGroup(*it.starmap(VGroup, quads[5:])),
            ),
            LaggedStartMap(FadeIn, VGroup(*[n.edges_in for n in neurons[5:]])),
            Write(all_dots_plus),
            run_time=3,
        )
        self.wait(2)
        ...
        rect = SurroundingRectangle(VGroup(self.network_mob.layers[-1], self.network_mob.output_labels))
...
class NotANeuroScientist(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(ShowCreation(rect), Write(no_claim_words, run_time=1), brain_anim)
        self.wait()
...
class ConstructGradientFromAllTrainingExamples(Scene):
    ...
    def setup_grid(self):
        h_lines = VGroup(*[Line(LEFT, RIGHT).scale(0.85 * FRAME_X_RADIUS) for x in range(6)])
        ...
        v_lines = VGroup(*[Line(UP, DOWN).scale(FRAME_Y_RADIUS - MED_LARGE_BUFF) for x in range(self.n_examples + 1)])
    ...
    def setup_weights(self):
        weights = VGroup(*list(map(Tex, ["w_0", "w_1", "w_2", "\\vdots", "w_{13{,}001}"])))
        ...
        self.set_variables_as_attrs(weights, brace, weights_words, dots=weights[-2])
    ...
    def show_two_requesting_changes(self):
        ...
        for i in list(range(3)) + [4]:
            ...
            if i == 0:
                ...
                added_anims = [dots_copy.move_to, self.get_grid_position(3, 0)]
            ...
            self.play(
                ...
                change.restore,
                change.scale,
                self.change_scale_val,
                change.move_to,
                self.get_grid_position(i, 0),
                *added_anims,
            )
    ...
    def show_all_examples_requesting_changes(self):
        ...
        examples = VGroup(*[self.get_example(t[0], j) for t, j in zip(data, it.count(1))])
        h_dots = OldTex("\\dots")
        h_dots.next_to(examples, RIGHT, MED_LARGE_BUFF)
        more_h_dots = VGroup(*[OldTex("\\dots").move_to(self.get_grid_position(i, self.n_examples)) for i in range(5)])
        more_h_dots.shift(MED_LARGE_BUFF * RIGHT)
        more_h_dots[-2].rotate(-np.pi / 4)
        more_v_dots = VGroup(
            *[self.dots.copy().move_to(self.get_grid_position(3, j)) for j in range(1, self.n_examples)]
        )

        changes = VGroup(
            *[
                self.get_random_decimal().move_to(self.get_grid_position(i, j))
                for i in list(range(3)) + [4]
        ...
        self.play(
            ...
            Write(h_dots, run_time=2, rate_func=squish_rate_func(smooth, 0.7, 1)),
        )
        self.play(
            Write(changes),
            Write(more_v_dots),
            Write(more_h_dots),
            *[example.eyes.look_at_anim(random.choice(changes)) for example in examples],
        )
        for x in range(2):
        ...
        self.change_rows = VGroup(
            *[VGroup(two_change, *changes[k * i : k * (i + 1)]) for i, two_change in enumerate(self.two_changes)]
        )
        for i in list(range(3)) + [-1]:
            self.change_rows[i].add(more_h_dots[i])

        self.all_eyes = VGroup(*[m.eyes for m in [self.two] + list(examples)])
    ...
    def average_together(self):
        ...
        for row in self.change_rows:
            ...
            rect.arrow = arrow
            average = self.get_colored_decimal(3 * np.mean([m.number for m in row if isinstance(m, DecimalNumber)]))
        ...
        arrow_to_averages = Arrow(words.get_bottom(), averages.get_top(), color=WHITE)
        ...
        self.play(
            ...
            WHITE,
            1,
            ReplacementTransform(self.change_rows[0].copy(), self.change_rows[0].target),
            *look_at_anims(averages[0]),
        )
        self.play(GrowArrow(arrow_to_averages))
        self.play(LaggedStartMap(ShowCreation, VGroup(*rects[1:])), *look_at_anims(rects[1]))
        self.play(
            LaggedStartMap(
            ...
            VGroup(*rects[1:]).set_stroke,
            WHITE,
            1,
            LaggedStartMap(
                ReplacementTransform,
                VGroup(*self.change_rows[1:]).copy(),
                lambda m: (m, m.target),
                lag_ratio=0.7,
            ),
            Write(dots),
        ...
        averages.add(dots)
        self.set_variables_as_attrs(rects, arrows, averages, arrow_to_averages)
    ...
    def collapse_into_gradient_vector(self):
        ...
        self.blink(2)
        self.play(GrowFromCenter(eta), minus.shift, MED_SMALL_BUFF * LEFT)
        self.wait()
    ...
    def get_requested_change_bubble(self, example_mob):
        ...
        group.shift(example_mob.get_right() + SMALL_BUFF * RIGHT - bubble.get_corner(DOWN + LEFT))

        return VGroup(bubble, words), change

    def get_random_decimal(self):
        return self.get_colored_decimal(0.3 * (random.random() - 0.5))
...
class WatchPreviousScene(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            "raise_right_hand",
            screen,
            self.change_students(*["thinking"] * 3, look_at=screen),
            ShowCreation(screen),
        )
        self.wait(10)
...
class OpenCloseSGD(Scene):
    def construct(self):
        term = OldTex("\\langle", "\\text{Stochastic gradient descent}", "\\rangle")
...
class OrganizeDataIntoMiniBatches(Scene):
    ...
    def add_examples(self):
        ...
        self.play(LaggedStartMap(FadeIn, alt_order_examples, lag_ratio=0.2, run_time=4))
        self.wait()
    ...
    def divide_into_minibatches(self):
        ...
        rows = Group(*[Group(*examples[i * self.n_cols : (i + 1) * self.n_cols]) for i in range(self.n_rows)])
        ...
        self.play(LaggedStartMap(FadeIn, rects, lag_ratio=0.7, rate_func=there_and_back))
        self.wait()
    ...
    def one_step_per_batch(self):
        ...
        text = brace.get_text("Compute gradient descent step (using backprop)", buff=SMALL_BUFF)
        ...
        def indicate_row(row):
            ...
            return LaggedStartMap(
                ApplyFunction,
                row,
                lambda row: (lambda m: m.scale(0.75).set_color(YELLOW), row),
                rate_func=wiggle,
            )
        ...
        for last_row, row in zip(rows, rows[1:-1]):
            self.play(last_row.shift, UP, brace.next_to, row, UP, SMALL_BUFF)
    ...
    def get_examples(self):
        ...
        return Group(
            *[
                MNistMobject(t[0], rect_kwargs={"stroke_width": 2}).set_height(height)
    ...
    def arrange_examples_in_grid(self, examples):
        examples.arrange_in_grid(n_rows=self.n_rows, buff=SMALL_BUFF)
...
class SwimmingInTerms(TeacherStudentsScene):
    def construct(self):
        ...
        terms.arrange(DOWN)
        terms.to_edge(UP)
        self.play(LaggedStartMap(FadeIn, terms), self.change_students(*["horrified"] * 3))
        self.wait()
        self.play(
            terms[-1].next_to,
            self.teacher.get_corner(UP + LEFT),
            UP,
...
class BackpropCodeAddOn(PiCreatureScene):
    def construct(self):
        words = OldTexText("The code you'd find \\\\ in Nielsen's book")
        words.to_corner(DOWN + LEFT)
        morty = self.pi_creature
        morty.next_to(words, UP)
        self.add(words)
        for mode in ["pondering", "thinking", "happy"]:
            self.play(morty.change, "pondering", morty.look, UP + LEFT)
...
class CannotFollowCode(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("angry", "sad", "angry", look_at=self.teacher.eyes)
...
class SimplestNetworkExample(PreviewLearning):
    ...
    def show_weights_and_biases(self):
        ...
        neurons = VGroup(*[layer.neurons[0] for layer in network_mob.layers[1:]])
        expression = OldTex(
            "C",
            "(",
            "w_1",
    ...
    def focus_just_on_last_two_layers(self):
        ...
        for mob in to_fade:
            mob.save_state()
        self.play(LaggedStartMap(ApplyMethod, to_fade, lambda m: (m.fade, 0.9)))
        self.wait()

        self.prev_layers = to_fade

    def label_neurons(self):
        neurons = VGroup(*[self.network_mob.layers[i].neurons[0] for i in (-1, -2)])
        decimals = VGroup()
        a_labels = VGroup()
        ...
        for neuron, superscript in zip(neurons, superscripts):
            ...
            arrow = Arrow(label[0].get_top(), neuron.get_bottom(), buff=SMALL_BUFF, color=WHITE)
            ...
            self.play(
                ...
                None,
                0,
                ChangingDecimal(decimal, lambda a: interpolate(opacity, 0.01, a)),
                UpdateFromFunc(decimal, lambda d: d.set_fill(WHITE if d.number < 0.8 else BLACK)),
                run_time=2,
                rate_func=there_and_back,
            )
            self.wait()
        ...
        self.play(
            LaggedStartMap(ShowCreation, superscript_rects, lag_ratio=0.8, run_time=1.5),
            Write(not_exponents, run_time=2),
        )
        self.wait()
        self.play(*list(map(FadeOut, [not_exponents, superscript_rects])))

        self.set_variables_as_attrs(a_labels, a_label_arrows, decimals, last_neurons=neurons)
    ...
    def show_desired_output(self):
        ...
        y_label_arrow = Arrow(y_label, neuron, color=WHITE, buff=SMALL_BUFF)
        VGroup(words, rect, y_label).set_color(self.desired_output_color)

        self.play(*list(map(FadeIn, [neuron, decimal])))
        self.play(ShowCreation(rect), Write(words, run_time=1))
        self.wait()
        self.play(Write(y_label, run_time=1), GrowArrow(y_label_arrow))
        self.wait()
    ...
    def show_cost(self):
        ...
        cost_equation = OldTex("C_0", "(", "\\dots", ")", "=", "(", "a^{(L)}", "-", "y", ")", "^2")
        cost_equation.to_corner(UP + RIGHT)
        C0, a, y = [cost_equation.get_part_by_tex(tex) for tex in ("C_0", "a^{(L)}", "y")]
        y.set_color(YELLOW)

        cost_word = OldTexText("Cost")
        cost_word.next_to(C0[0], LEFT, LARGE_BUFF)
        cost_arrow = Arrow(cost_word, C0, buff=SMALL_BUFF)
        VGroup(C0, cost_word, cost_arrow).set_color(self.cost_color)

        expression = OldTex("\\text{For example: }(", "0.00", "-", "0.00", ")", "^2")
        ...
        decimals = VGroup(self.decimals[0], self.desired_output_decimal).copy()
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*[m for m in cost_equation if m not in [a, y]])))
        self.play(MoveToTarget(decimals), FadeIn(non_numbers))
        self.wait()
        self.play(Write(cost_word, run_time=1), GrowArrow(cost_arrow))
        self.play(C0.shift, MED_SMALL_BUFF * UP, rate_func=wiggle)
        self.wait()
        self.play(*list(map(FadeOut, [decimals, non_numbers])))

        self.set_variables_as_attrs(cost_equation, cost_word, cost_arrow)
    ...
    def show_activation_formula(self):
        ...
        formula = OldTex("a^{(L)}", "=", "\\sigma", "(", "w^{(L)}", "a^{(L-1)}", "+", "b^{(L)}", ")")
        ...
        self.play(ReplacementTransform(pre_aL, aL), Write(equals))
        self.play(ReplacementTransform(edge.copy(), wL))
        self.wait()
        ...
        self.set_variables_as_attrs(formula, weighted_sum_terms)

    def introduce_z(self):
        terms = self.weighted_sum_terms
        terms.generate_target()
        terms.target.next_to(self.formula, UP, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
    ...
    def break_into_computational_graph(self):
        network_early_layers = VGroup(*it.chain(self.network_mob.layers[:2], self.network_mob.edge_groups[:2]))
        ...
        top_lines = VGroup(
            *[Line(term.get_bottom(), zL.target.get_top(), buff=SMALL_BUFF) for term in top_terms.target]
        )
        z_to_a_line, a_to_c_line, y_to_c_line = all_lines = [
            Line(m1.target.get_bottom(), m2.target.get_top(), buff=SMALL_BUFF)
            for m1, m2 in [(zL, aL), (aL, C0), (y, C0)]
        ]
        ...
        self.play(ShowCreation(top_lines, lag_ratio=0), top_lines.flash)
        self.wait()
        self.play(MoveToTarget(aL))
        self.play(network_early_layers.fade, 1, ShowCreation(z_to_a_line), z_to_a_line.flash)
        self.wait()
        self.play(MoveToTarget(y))
        self.play(MoveToTarget(C0))
        self.play(*it.chain(*[[ShowCreation(line), line.flash] for line in (a_to_c_line, y_to_c_line)]))
        self.wait(2)
    ...
    def show_preceding_layer_in_computational_graph(self):
        ...
        self.play(ShowCreation(rect), FadeIn(prev_neuron), ShowCreation(prev_edge))
        self.play(
            ReplacementTransform(VGroup(prev_neuron, prev_edge).copy(), new_subgraph),
            UpdateFromAlphaFunc(new_terms, lambda m, a: m.set_fill(opacity=a)),
    ...
    def show_number_lines(self):
        comp_graph = self.comp_graph
        wL, aLm1, bL, zL, aL, C0 = [getattr(comp_graph, attr) for attr in ["wL", "aLm1", "bL", "zL", "aL", "C0"]]
        ...
        for mob in wL, zL, aL, C0:
            ...
            mob.dot.move_to(mob.number_line.number_to_point(mob.val))
            if mob is wL:
                path_arc = 0
            ...
            self.play(ShowCreation(mob.number_line, lag_ratio=0.5))
            self.play(
                ShowCreation(mob.arrow),
                ReplacementTransform(mob.copy(), mob.dot, path_arc=path_arc),
            )
        self.wait()

    def ask_about_w_sensitivity(self):
        wL, aLm1, bL, zL, aL, C0 = [getattr(self.comp_graph, attr) for attr in ["wL", "aLm1", "bL", "zL", "aL", "C0"]]
        aLm1_val = self.last_neurons[1].get_fill_opacity()
        bL_val = self.network.biases[-1][0]

        get_wL_val = lambda: wL.number_line.point_to_number(wL.dot.get_center())
        ...
        def shake_dot(run_time=2, rate_func=there_and_back):
            self.play(
                ApplyMethod(wL.dot.shift, LEFT, rate_func=rate_func, run_time=run_time),
                *dot_update_anims,
            )
        ...
        for sym in del_wL, del_C0:
            self.play(GrowFromCenter(sym.brace), Write(sym, run_time=1))
            shake_dot()
        self.wait()
    ...
    def show_derivative_wrt_w(self):
        ...
        dC_dw = OldTex("{\\partial C_0", "\\over", "\\partial w^{(L)} }")
        ...
        self.wait(2)
        self.play(FadeOut(words), ReplacementTransform(full_rect, denom_rect))
    ...
    def show_chain_of_events(self):
        comp_graph = self.comp_graph
        wL, zL, aL, C0 = [getattr(comp_graph, attr) for attr in ["wL", "zL", "aL", "C0"]]
        del_wL = self.del_wL
        del_C0 = self.del_C0
        ...
        for sym in del_zL, del_aL:
            ...
            sym.shift(sym.brace.get_bottom() + SMALL_BUFF * DOWN - sym[0].get_corner(UP + RIGHT))
    ...
    def show_chain_rule(self):
        dC_dw = self.dC_dw
        del_syms = [getattr(self, attr) for attr in ("del_wL", "del_zL", "del_aL", "del_C0")]

        dz_dw = OldTex("{\\partial z^{(L)}", "\\over", "\\partial w^{(L)}}")
        da_dz = OldTex("{\\partial a^{(L)}", "\\over", "\\partial z^{(L)}}")
        dC_da = OldTex("{\\partial C0}", "\\over", "\\partial a^{(L)}}")
        ...
        for frac, top_sym, bot_sym in zip(group[1:], del_syms[1:], del_syms):
            ...
            self.play(ReplacementTransform(bot_sym.copy(), frac[2]))
            self.wait()
        self.shake_dot()
        self.wait()
    ...
    def indicate_everything_on_screen(self):
        ...
        self.play(LaggedStartMap(Indicate, everything, rate_func=wiggle, lag_ratio=0.2, run_time=5))
        self.wait()
    ...
    def prepare_for_derivatives(self):
        ...
        Transform(VGroup(*cost_equation.target[1:index]), VectorizedPoint(cost_eq.get_left())).update(1)
    ...
    def compute_derivatives(self):
        ...
        # dC_da
        self.play(MoveToTarget(dC_da), Write(dC_da.equals))
        index = 4
        ...
        self.play(cost_equation.next_to, dC_da.rhs, DOWN, MED_LARGE_BUFF, LEFT)
        self.wait()
        self.play(
            ReplacementTransform(
                cost_rhs.copy(),
        ...
        moving_decimals = VGroup(self.decimals[0].copy(), self.desired_output_decimal.copy())
        minus = OldTex("-")
        ...
        self.play(ReplacementTransform(dC_da.rhs.copy(), double_arrow))
        self.wait()
        ...
        for target_o in 0, opacity:
            ...
            self.play(
                ...
                None,
                target_o,
                *[ChangingDecimal(d, lambda a: neuron.get_fill_opacity()) for d in (decimal, moving_decimals[0])],
            )
        self.play(*list(map(FadeOut, [double_arrow, moving_decimals])))

        # da_dz
        self.play(MoveToTarget(da_dz), Write(da_dz.equals))
        ...
        self.play(aL_formula.next_to, da_dz.rhs, DOWN, MED_LARGE_BUFF, LEFT)
        self.wait()
        self.play(
            ReplacementTransform(
                a_rhs.copy(),
        ...
        # dz_dw
        self.play(MoveToTarget(dz_dw), Write(dz_dw.equals))
    ...
    def fire_together_wire_together(self):
        ...
        def get_decimal_anims():
            return [
                ...
                UpdateFromFunc(
                    decimal,
                    lambda m: m.set_color(WHITE if neuron.get_fill_opacity() < 0.8 else BLACK),
                ),
            ]

        self.play(ShowCreation(rect))
        self.play(FadeOut(rect))
        self.play(del_wL.next_to, edge, UP, SMALL_BUFF)
        self.play(
            edge.set_stroke,
            None,
            10,
        ...
        self.play(rhs_copy.move_to, neuron, rhs_copy.set_fill, None, 0)
        ...
        opacity = neuron.get_fill_opacity()
        self.play(neuron.set_fill, None, 0.99, *get_decimal_anims())
        self.play(edge.set_stroke, None, 8)
        self.play(neuron.set_fill, None, opacity, *get_decimal_anims())
    ...
    def show_average_derivative(self):
        ...
        full_derivative.set_color_by_tex_to_color_map(
            {"partial C": self.cost_color, "partial w": self.del_wL.get_color()}
        )
        ...
        VGroup(full_derivative, lhs_brace, lhs_text, rhs_brace, rhs_text).to_corner(DOWN + LEFT)
    ...
    def cycle_through_altnernate_training_examples(self):
        neurons = VGroup(self.desired_output_neuron, *self.last_neurons)
        decimals = VGroup(self.desired_output_decimal, *self.decimals)
        ...
        for x in range(20):
            for n, d in zip(neurons, decimals):
                ...
                Transform(d, self.get_neuron_activation_decimal(n)).update(1)
            self.wait(0.2)
    ...
    def show_gradient(self):
        ...
        self.play(MoveToTarget(dC_dw), Write(vect, run_time=1))
        terms.add(dC_dw)
        self.play(Write(lhs))
        self.wait(2)
    ...
    def transition_to_derivative_wrt_b(self):
        ...
        all_comp_graph_parts.scale(1.3, about_point=all_comp_graph_parts.get_bottom())
        comp_graph = self.comp_graph
        wL, bL, zL, aL, C0 = [getattr(comp_graph, attr) for attr in ["wL", "bL", "zL", "aL", "C0"]]
        path_to_C = VGroup(wL, zL, aL, C0)

        top_expression = VGroup(self.chain_rule_equation, self.chain_rule_rhs)
    ...
    def show_derivative_wrt_b(self):
        ...
        dz_db = OldTex("{\\partial z^{(L)}", "\\over", "\\partial b^{(L)}}")
        dz_db.set_color_by_tex_to_color_map({"partial z": self.z_color, "partial b": MAROON_B})
        dz_db.replace(dz_dw)
        ...
        self.play(
            left_term_group.shift,
            DOWN,
            left_term_group.fade,
            1,
        )
        ...
        self.wait()
        self.play(ReplacementTransform(b_in_z_formula.copy(), one), FadeOut(b_in_z_rect))
        self.play(
            ShowCreation(arrow),
            ReplacementTransform(dz_db.copy(), one, path_arc=arrow.path_arc),
        )
        self.wait(2)
    ...
    def show_derivative_wrt_a(self):
        ...
        dz_daLm1 = OldTex("{\\partial z^{(L)}", "\\over", "\\partial a^{(L-1)}}")
        dz_daLm1.scale(0.8)
        ...
        comp_graph = self.comp_graph
        path_to_C = VGroup(*[getattr(comp_graph, attr) for attr in ["aLm1", "zL", "aL", "C0"]])
        ...
        for x in range(3):
            self.play(get_path_animation())
        self.play(numer.shift, SMALL_BUFF * UP, Transform(denom, del_aLm1))
        self.play(
            FadeIn(dz_daLm1),
            VGroup(*self.chain_rule_equation[-2:]).shift,
            SMALL_BUFF * RIGHT,
        )
        self.wait()
        self.play(ShowCreation(dz_daLm1_rect))
        self.wait()
        self.play(ReplacementTransform(dz_daLm1_rect, z_formula_rect))
        self.wait()
        self.play(ReplacementTransform(z_formula_rect, a_in_z_rect))
        self.play(ReplacementTransform(wL_in_z.copy(), wL), FadeOut(a_in_z_rect))
        self.play(
            ShowCreation(arrow),
            ReplacementTransform(dz_daLm1.copy(), wL, path_arc=arrow.path_arc),
        )
        self.wait(2)
    ...
    def show_previous_weight_and_bias(self):
        ...
        self.play(
            ...
            self.chain_rule_equation.to_edge,
            RIGHT,
        )
        ...
        VGroup(a_label, arrow).shift(neuron.get_center() - self.last_neurons[1].get_center())
    ...
    def animate_long_path(self):
        ...
        path_to_C = VGroup(
            self.wLm1,
            self.zLm1,
            *[getattr(comp_graph, attr) for attr in ["aLm1", "zL", "aL", "C0"]],
        )
        for x in range(2):
            self.play(
                LaggedStartMap(
    ...
    def get_all_comp_graph_parts(self):
        ...
        for attr in "wL", "zL", "aL", "C0":
            ...
            result.add(sym.arrow, sym.number_line, sym.dot)
...
class IsntThatOverSimplified(TeacherStudentsScene):
    def construct(self):
        self.student_says("Isn't that over-simplified?", target_mode="raise_right_hand", run_time=1)
        self.play_student_changes("pondering", "raise_right_hand", "pondering")
        self.wait()
        self.teacher_says("Not that much, actually!", run_time=1, target_mode="hooray")
        self.wait(2)
...
class GeneralFormulas(SimplestNetworkExample):
    ...
    def show_all_a_labels(self):
        ...
        start_labels, start_arrows = [
            VGroup(*list(map(VGroup, [group[i][0] for i in (0, 1)]))).copy() for group in (all_labels, all_arrows)
        ]
        for label in start_labels:
            label[0][-1].set_color(BLACK)

        self.add(all_decimals)
        self.play(*it.chain(list(map(Write, start_labels)), [GrowArrow(a[0]) for a in start_arrows]))
        self.wait()
        self.play(
    ...
    def only_show_abstract_a_labels(self):
        ...
        for x, layer in enumerate(self.network_mob.layers[-2:]):
            for y, neuron in enumerate(layer.neurons):
                if (x == 0 and y == 1) or (x == 1 and y == 0):
                    ...
                    rects.add(SurroundingRectangle(neuron.label.target[-1], buff=0.5 * SMALL_BUFF))
                else:
        ...
        for neuron, rect in zip(self.chosen_neurons, rects):
            self.play(MoveToTarget(neuron.label), ShowCreation(rect))
            self.play(FadeOut(rect))
            self.wait()
        self.wait()
    ...
    def show_cost(self):
        ...
        self.play(*[ReplacementTransform(mob, mob.target) for mob in (aj, yj)])
    ...
    def show_example_weight(self):
        ...
        self.play(
            LaggedStartMap(
                ApplyFunction,
                edges,
                lambda e: (lambda m: m.rotate(np.pi / 12).set_color(YELLOW), e),
                rate_func=wiggle,
            )
        )
    ...
    def show_values_between_weight_and_cost(self):
        ...
        a_formula = OldTex("a^{(L)}_j", "=", "\\sigma(", "z^{(L)}_j", ")")
        ...
        to_fade = VGroup(
            ...
            *[VGroup(n.arrow, n.label) for n in self.desired_output.neurons],
        )

        self.play(
            FadeOut(to_fade),
            self.cost_equation.next_to,
            a_formula,
            DOWN,
            MED_LARGE_BUFF,
            self.cost_equation.to_edge,
            RIGHT,
            ReplacementTransform(self.w_label[1].copy(), w_part),
            ReplacementTransform(self.chosen_neurons[0].label.copy(), aLm1_part),
        )
        self.play(Write(VGroup(*[m for m in z_formula if m not in [w_part, aLm1_part]])))
        self.wait()
        self.play(ReplacementTransform(self.chosen_neurons[1].label.copy(), aL_part))
        self.play(
            Write(VGroup(*a_formula[1:3] + [a_formula[-1]])),
            ReplacementTransform(z_formula[0].copy(), a_formula.get_part_by_tex("z^")),
        )
        self.wait()
    ...
    def show_weight_chain_rule(self):
        ...
        terms = VGroup(*[VGroup(*chain_rule[i : i + 3]) for i in range(4, len(chain_rule), 3)])
        rects = VGroup(*[SurroundingRectangle(term, buff=0.5 * SMALL_BUFF) for term in terms])
        rects.set_color_by_gradient(GREEN, WHITE, RED)

        self.play(Transform(self.z_formula, self.compact_z_formula))
        self.play(Write(chain_rule))
        self.wait()
        self.play(LaggedStartMap(ShowCreationThenDestruction, rects, lag_ratio=0.7, run_time=3))
        self.wait()
    ...
    def show_derivative_wrt_prev_activation(self):
        ...
        self.play(ReplacementTransform(self.chain_rule, VGroup(*chain_rule[:n] + chain_rule[n + 1 :])))
    ...
    def show_multiple_paths_from_prev_layer_neuron(self):
        ...
        labels, arrows, decimals = [
            VGroup(*[getattr(n, attr) for n in neurons]) for attr in ("label", "arrow", "decimal")
        ]
        ...
        for x in range(5):
            ...
            if x == 2:
                anims += [FadeIn(words), GrowFromCenter(brace)]
            self.play(*anims)
            self.wait()
        for path, path_line in zip(paths, path_lines):
            ...
            self.play(group.shift, MED_SMALL_BUFF * UP, rate_func=wiggle)
    ...
    def show_previous_layer(self):
        ...
        all_edges_out = VGroup(
            *[VGroup(*[n.edges_in[i] for n in mid_neurons]).copy() for i in range(len(layer.neurons))]
        )
        ...
        def get_neurons_decimal_anims(neuron):
            return [
                ...
                UpdateFromFunc(
                    neuron.decimal,
                    lambda m: m.set_fill(WHITE if neuron.get_fill_opacity() < 0.8 else BLACK),
                ),
            ]

        self.play(ShowCreation(deriv_rect))
        self.play(LaggedStartMap(ShowCreationThenDestruction, mid_neuron_outlines))
        self.play(
            *it.chain(
                *[
                    [
        ...
        self.play(
            LaggedStartMap(GrowFromCenter, layer.neurons, run_time=1),
            LaggedStartMap(ShowCreation, edges),
            FadeOut(to_fade),
        )
        for x in range(3):
    ...
    def get_chain_rule(self, *tex):
        ...
        chain_rule.set_color_by_tex_to_color_map(
            {"C_0": self.cost_color, "z^": self.z_color, "w^": self.w_label.get_color()}
        )
        return chain_rule
...
class PatYourselfOnTheBack(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Pat yourself on \\\\ the back!", target_mode="hooray")
...
class ThatsALotToThinkAbout(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("That's a lot to \\\\ think about!", target_mode="surprised")
...
class LayersOfComplexity(Scene):
    ...
    def get_chain_rule_equations(self):
        ...
        lil_rect = SurroundingRectangle(VGroup(*w_deriv[-3:]), buff=0.5 * SMALL_BUFF)
        a_deriv = OldTex(
            "\\sum_{j = 0}^{n_{l+1} - 1}",
            "w^{(l+1)}_{jk}",
            "\\sigma'(z^{(l+1)}_j)",
        ...
        result = VGroup(w_deriv, lil_rect, arrow, big_rect, a_deriv, or_word, last_a_deriv)
...
class SponsorFrame(PiCreatureScene):
    def construct(self):
        ...
        self.play(morty.change, "hooray", t_shirt_words, Write(t_shirt_words, run_time=2))
        self.wait()
        self.play(morty.change, "raise_right_hand", screen, ShowCreation(screen))
        self.play(t_shirt_words.scale, 1.0 / 1.5, t_shirt_words.next_to, arrow, RIGHT)
        self.play(Write(url))
        self.play(GrowArrow(arrow))
        self.wait(2)
        ...
        self.play(LaggedStartMap(Indicate, url, rate_func=wiggle, color=PINK, run_time=3))
        self.wait(3)
...
class Thumbnail(PreviewLearning):
    ...
    def construct(self):
        self.color_network_edges()
        network_mob = self.network_mob
        network_mob.scale(self.network_scale_val, about_point=network_mob.get_bottom())
