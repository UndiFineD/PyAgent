...
def get_edges(image_array):
    blurred = cv2.GaussianBlur(image_array, **DEFAULT_GAUSS_BLUR_CONFIG)
    edges = cv2.Canny(blurred, **DEFAULT_CANNY_CONFIG)
    return edges
...
class WrappedImage(Group):
    ...
    def __init__(self, image_mobject, **kwargs):
        ...
        rect = SurroundingRectangle(image_mobject, **self.rect_kwargs)
...
class PixelsFromVect(PixelsAsSquares):
    def __init__(self, vect, **kwargs):
        PixelsAsSquares.__init__(self, ImageMobject(layer_to_image_array(vect)), **kwargs)


class MNistMobject(WrappedImage):
    def __init__(self, vect, **kwargs):
        WrappedImage.__init__(self, ImageMobject(layer_to_image_array(vect)), **kwargs)
...
class NetworkMobject(VGroup):
    ...
    def activate_layer(self, layer, activation_vector):
        ...
        if len(av) > n_neurons:
            if self.average_shown_activation_of_large_layer:
                ...
                av = np.array([arr_to_num(av[i1:i2]) for i1, i2 in zip(indices[:-1], indices[1:])])
            else:
                av = np.append(
                    av[: n_neurons / 2],
                    av[-n_neurons / 2 :],
    ...
    def get_edge_propogation_animations(self, index):
        ...
        edge_group_copy.set_stroke(self.edge_propogation_color, width=1.5 * self.edge_stroke_width)
        return [ShowCreationThenDestruction(edge_group_copy, run_time=self.edge_propogation_time, lag_ratio=0.5)]
...
class NetworkScene(Scene):
    ...
    def show_activation_of_layer(self, layer_index, activation_vector, added_anims=None):
        ...
        if layer_index > 0:
            anims += self.network_mob.get_edge_propogation_animations(layer_index - 1)
        anims += added_anims
        self.play(*anims)
...
def make_transparent(image_mob):
    alpha_vect = np.array(image_mob.pixel_array[:, :, 0], dtype="uint8")
...
class ExampleThrees(PiCreatureScene):
    ...
    def show_initial_three(self):
        ...
        brace_labels = VGroup(*[brace.get_text("28px") for brace in braces])
        ...
        self.play(FadeIn(three_mob[0]), LaggedStartMap(FadeIn, three_mob[1]))
        self.wait()
        self.play(
            LaggedStartMap(
                DrawBorderThenFill,
            ...
            randy.change,
            "sassy",
            *it.chain(list(map(GrowFromCenter, braces)), list(map(FadeIn, brace_labels))),
        )
        self.wait()
        self.play(
            ShowCreation(bubble),
        ...
        self.play(ShowCreation(arrow), Write(real_three))
        self.wait()
    ...
    def show_alternate_threes(self):
        ...
        for square in three.target[1]:
            ...
            square.set_fill(rgba_to_color(yellow_rgb * square_rgb), opacity=0.5)
        ...
        self.play(
            randy.change,
            "plain",
            *list(map(FadeOut, [self.bubble, self.arrow, self.real_three])) + [MoveToTarget(three)],
        )
        ...
        for moving_three in three, alt_threes[1]:
            ...
            self.play(MoveToTarget(moving_three, lag_ratio=0.5))
            self.play(
                Animation(randy),
                moving_three.replace,
                randy.eyes[1],
                moving_three.scale,
                0.7,
                run_time=2,
            ...
            self.play(Animation(randy), FadeOut(moving_three))
    ...
    def show_alternate_digits(self):
        ...
        self.play(randy.change, "sassy", Transform(three, alt_mobs[0]), ShowCreation(cross))
        self.wait()
    ...
    def get_three_mobs(self):
        ...
        for three_array in three_arrays:
            ...
            three_mob = WrappedImage(pixel_mob, color=GREY_B, buff=0)
...
class BrainAndHow(Scene):
    def construct(self):
        ...
        self.play(Write(how))
        for x in range(2):
            self.play(ShowPassingFlash(brain_outline, time_width=0.5, run_time=2))
        self.wait()
...
class WriteAProgram(Scene):
    def construct(self):
        ...
        three_rect = SurroundingRectangle(three, color=BLUE, buff=SMALL_BUFF)

        numbers = VGroup()
        for square in three:
            rgb = square.fill_rgb
            num = DecimalNumber(square.fill_rgb[0], num_decimal_places=1)
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, three), ShowCreation(three_rect))
        ...
        for n in 8, 1, 5, 3:
            self.play(rect.move_to, choices[n], MaintainPositionRelativeTo(q_mark, rect))
            self.wait(1)
        choice = choices[3]
...
class LayOutPlan(TeacherStudentsScene, NetworkScene):
    ...
    def show_words(self):
        ...
        self.play_student_changes(*["pondering"] * 3, look_at=words)
    ...
    def show_network(self):
        ...
        self.play(
            ReplacementTransform(VGroup(self.words[1].copy()), network_mob.layers),
            self.change_students(*["confused"] * 3, lag_ratio=0),
            self.teacher.change,
            "plain",
            run_time=1,
        )
    ...
    def ask_about_layers(self):
        self.student_says("Why the layers?", index=2, bubble_config={"direction": LEFT})
        self.wait()
    ...
    def show_videos(self):
        ...
        videos = VGroup(*[VideoIcon().set_fill(RED) for x in range(2)])
        ...
        self.play(MoveToTarget(network_mob), MoveToTarget(learning))
        self.play(
...
class PreviewMNistNetwork(NetworkScene):
    ...
    def feed_in_image(self, in_vect):
        ...
        self.feed_forward(
            in_vect,
            added_anims=[self.get_image_to_layer_one_animation(image, start_neurons)],
        )
        n = np.argmax([neuron.get_fill_opacity() for neuron in self.network_mob.layers[-1].neurons])
    ...
    def get_image_to_layer_one_animation(self, image, start_neurons):
        image_mover = VGroup(*[pixel.copy() for pixel in image if pixel.fill_rgb[0] > 0.1])
        return Transform(
            image_mover,
            start_neurons,
            remover=True,
...
class AlternateNeuralNetworks(PiCreatureScene):
    def construct(self):
        ...
        self.play(Write(examples[0], run_time=2), morty.change, "raise_right_hand")
        self.wait()
        self.play(
            examples[0].shift,
            MED_LARGE_BUFF * UP,
        ...
        self.wait()
        self.play(examples.shift, UP, FadeIn(maybe_words), morty.change, "maybe")
        self.wait(2)
...
class NotPerfectAddOn(Scene):
    def construct(self):
        ...
        self.play(Write(words), ShowCreation(arrow), run_time=1)
        self.wait(2)


class MoreAThanI(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("More \\\\ A than I", target_mode="hesitant")
...
class BreakDownName(Scene):
    ...
    def ask_questions(self):
        ...
        name.to_edge(UP)
        q1 = OldTexText("What are \\\\ the ", "neuron", "s?", arg_separator="")
        ...
        self.play(Write(q2, run_time=1), ShowCreation(a2), name[1].set_color, q2.get_color())
        self.wait(2)

        self.play(*list(map(FadeOut, [name, randy, brain, q2, a1, a2, q1[0], q1[2]])))
    ...
    def show_neuron(self):
        ...
        self.play(ReplacementTransform(self.neuron_word, neuron_word), ShowCreation(neuron))
        self.play(ShowCreation(arrow), Write(description, run_time=1))
        self.wait()
        self.play(neuron.set_fill, None, 0.2, num.restore)
        self.wait()
        for value in 0.8, 0.4, 0.1, 0.5:
            mob = OldTex(str(value))
            mob.replace(num)
            self.play(neuron.set_fill, None, value, Transform(num, mob))
            self.wait()
...
class IntroduceEachLayer(PreviewMNistNetwork):
    ...
    def break_up_image_as_neurons(self):
        ...
        for pixel in image_mob:
            ...
            neuron = Circle(color=WHITE, stroke_width=1, radius=pixel.get_width() / 2)
        ...
        labels = VGroup(*[brace.get_tex("28", buff=SMALL_BUFF) for brace in braces])
    ...
    def show_activation_of_one_neuron(self):
        ...
        example_num.target = DecimalNumber(example_neuron.get_fill_opacity())
        example_num.target.move_to(example_neuron.target)

        def change_activation(num):
            self.play(
                example_neuron.set_fill,
                None,
                num,
                ChangingDecimal(
                    example_num,
                ...
                UpdateFromFunc(
                    example_num,
                    lambda m: m.set_fill(BLACK if example_neuron.get_fill_opacity() > 0.8 else WHITE),
                ),
            )

        self.play(LaggedStartMap(FadeIn, numbers))
        self.play(MoveToTarget(example_neuron), MoveToTarget(example_num))
        self.wait()
    ...
    def transform_into_full_network(self):
        ...
        rows = VGroup(*[VGroup(*neurons[28 * i : 28 * (i + 1)]) for i in range(28)])
        ...
        self.play(rows.arrange, RIGHT, buff=SMALL_BUFF, path_arc=np.pi / 2, run_time=2)
        self.play(
        ...
        self.play(
            ReplacementTransform(self.num_pixels_equation[-1], layer.brace_label),
            FadeIn(layer.brace),
        )
    ...
    def show_output_layer(self):
        ...
        rect = SurroundingRectangle(VGroup(layer, labels))
        ...
        def change_activation(num):
            self.play(
                ...
                UpdateFromFunc(
                    activation,
                    lambda m: m.set_fill(BLACK if neuron.get_fill_opacity() > 0.8 else WHITE),
                ),
            )
    ...
    def show_hidden_layers(self):
        ...
        self.play(ShowCreation(rect), Write(name))
        self.wait()
        self.play(Write(q_marks))
        self.wait()
        self.play(FadeOut(q_marks), Animation(q_marks[-1].copy()))
...
class DiscussChoiceForHiddenLayers(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(rects), ReplacementTransform(two_words, sixteen_words), neurons_anim)
        self.wait()
...
class MoreHonestMNistNetworkPreview(IntroduceEachLayer):
    ...
    def get_image_to_layer_one_animation(self, image, start_neurons):
        ...
        return Transform(mover, target, run_time=2, lag_ratio=0.5, remover=True)


class AskAboutPropogationAndTraining(TeacherStudentsScene):
    def construct(self):
        self.student_says("How does one layer \\\\ influence the next?", index=0, run_time=1)
        self.wait()
        self.student_says("How does \\\\ training work?", index=2, run_time=1)
        self.wait(3)
...
class AskAboutLayers(PreviewMNistNetwork):
    def construct(self):
        ...
        question.to_edge(UP)
        neuron_groups = [layer.neurons for layer in self.network_mob.layers]
        arrows = VGroup(*[Arrow(question[1].get_bottom(), group.get_top()) for group in neuron_groups])
        rects = list(map(SurroundingRectangle, neuron_groups[1:3]))

        self.play(
            Write(question, run_time=1),
            LaggedStartMap(GrowFromPoint, arrows, lambda a: (a, a.get_start()), run_time=2),
        )
        self.wait()
...
class BreakUpMacroPatterns(IntroduceEachLayer):
    ...
    def setup_added_patterns(self):
        image_map = get_organized_images()
        two, three, five = mobs = [MNistMobject(image_map[n][0]) for n in (2, 3, 5)]
    ...
    def show_nine(self):
        ...
        self.wait()
        self.play(Write(equation[1]), upper_loop[1].restore, FadeIn(upper_loop[0]))
        self.wait()
        self.play(
            Write(equation[3]),
    ...
    def show_eight(self):
        ...
        self.play(ReplacementTransform(self.upper_loop.copy(), upper_loop))
        self.wait()
    ...
    def show_four(self):
        ...
        equation.next_to(self.eight_equation, DOWN, aligned_edge=LEFT)

        self.play(FadeIn(four), Write(equation[1]))
        self.play(ReplacementTransform(self.right_line.copy(), right_line))
        self.play(LaggedStartMap(FadeIn, VGroup(*equation[3:])))
        self.wait(2)
    ...
    def show_second_to_last_layer(self):
        ...
        for pattern in patterns:
            pattern.add_to_back(pattern[1].copy().set_color(BLACK, alpha=1))
    ...
    def show_upper_loop_activation(self):
        ...
        words.shift(RIGHT)
        rect = SurroundingRectangle(VGroup(neuron, self.patterns[0]))
        nine = self.nine
    ...
    def show_what_learning_is_required(self):
        ...
        for x in range(3):
            self.play(LaggedStartMap(ShowCreationThenDestruction, edge_group, run_time=3))
            self.wait()
    ...
    def get_equation(self, *mobs):
        equation = VGroup(
            mobs[0],
            OldTex("=").scale(2),
            *list(it.chain(*[[m, OldTex("+").scale(2)] for m in mobs[1:-1]])) + [mobs[-1]],
        )
...
class HowWouldYouRecognizeSubcomponent(TeacherStudentsScene):
    def construct(self):
        self.student_says("Okay, but recognizing loops \\\\", "is just as hard!", target_mode="sassy")
...
class BreakUpMicroPatterns(BreakUpMacroPatterns):
    ...
    def break_down_loop(self):
        ...
        edges = Group(*[getattr(self, "loop_edge%d" % d) for d in range(1, 6)])
        ...
        self.add(randy)
        self.play(FadeIn(loop), randy.change, "pondering", loop)
        self.play(Blink(randy))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, edges, lambda e: (e.restore,), run_time=4))
        self.wait()
        self.play(
    ...
    def break_down_long_line(self):
        ...
        edges = Group(*[getattr(self, "right_line_edge%d" % d) for d in range(1, 4)])
        ...
        equation.next_to(self.loop_equation, DOWN, MED_LARGE_BUFF, LEFT)
        image_map = get_organized_images()
        digits = VGroup(*[MNistMobject(image_map[n][1]) for n in (1, 4, 7)])
        digits.arrange(RIGHT)
        digits.next_to(randy, RIGHT)

        self.revert_to_original_skipping_status()
        self.play(FadeIn(line), randy.change, "hesitant", line)
        self.play(Blink(randy))
...
class SecondLayerIsLittleEdgeLayer(IntroduceEachLayer):
    ...
    def setup_activations_and_nines(self):
        ...
        nine_im, loop_im, line_im = images = [
            Image.open(get_full_raster_image_path("handwritten_%s" % s)) for s in ("nine", "upper_loop", "right_line")
        ]
        nine_array, loop_array, line_array = [np.array(im)[:, :, 0] / 255.0 for im in images]
        ...
        self.activations = self.network.get_activation_of_all_layers(nine_array.flatten())
        ...
        loop, line = [ImageMobject(layer_to_image_array(array.flatten())) for array in (loop_array, line_array)]
    ...
    def describe_second_layer(self):
        ...
        self.play(ShowCreation(rect), Write(words, run_time=2))
        self.wait()
    ...
    def show_propogation(self):
        ...
        active_layers = [network_mob.get_active_layer(i, a) for i, a in enumerate(activations)]

        def activate_layer(i):
            self.play(
                ShowCreationThenDestruction(edge_groups[i - 1], run_time=2, lag_ratio=0.5),
                FadeIn(active_layers[i]),
            )

        self.play(FadeIn(nine))
        self.play(ReplacementTransform(v_nine, neurons))
        self.play(MoveToTarget(neurons, remover=True, lag_ratio=0.5, run_time=2))
    ...
    def ask_question(self):
        question = OldTexText("Does the network \\\\ actually do this?")
        ...
        self.wait()
        self.play(FadeIn(later), GrowFromPoint(arrow, arrow.get_start()))
        self.wait()
    ...
    def set_neurons_target(self, neurons, layer):
        ...
        Transform(VGroup(*neurons.target[n:-n]), VectorizedPoint(layer.get_center())).update(1)
...
class ManyTasksBreakDownLikeThis(TeacherStudentsScene):
    def construct(self):
        ...
        syllables = OldTexText("$\\cdot$".join(["re", "cog", "ni", "tion"]))
        word = OldTexText("re", "cognition", arg_separator="")
        ...
        sequence = VGroup(audio, get_arrow(), letters, get_arrow(), syllables, get_arrow(), word)
        ...
        self.teacher_says("Many", "recognition", "tasks\\\\", "break down like this")
        ...
        self.play(
            self.teacher.bubble.content.fade,
            1,
            ShowCreation(audio),
            pre_word.shift,
            MED_SMALL_BUFF,
        ...
        self.play(
            ...
            audio.restore,
            FadeIn(audio_label),
            *[ReplacementTransform(m1, m2) for m1, m2 in zip(pre_word, letters)],
        )
        self.play(
    ...
    def get_wave_form(self):
        ...
        result = VGroup(*[Line(func(x) * DOWN, func(x) * UP) for x in np.arange(0, 4, 0.1)])
...
class AskAboutWhatEdgesAreDoing(IntroduceEachLayer):
    ...
    def add_question(self):
        ...
        question = OldTexText("What are these connections actually doing?")
        question.to_edge(UP)
        question.shift(RIGHT)
        arrows = VGroup(*[Arrow(question.get_bottom(), edge_group.get_top()) for edge_group in edge_groups])
...
class IntroduceWeights(IntroduceEachLayer):
    ...
    def show_desired_pixel_region(self):
        ...
        pixels = PixelsAsSquares(ImageMobject(np.zeros((d, d, 4))))
        ...
        self.play(pixels_to_detect.set_fill, WHITE, 1, lag_ratio=0.5, run_time=2)
        self.wait(2)
    ...
    def ask_about_parameters(self):
        ...
        p_labels = VGroup(
            *[OldTex("p_%d\\!:" % (i + 1)).set_color(self.weights_color) for i in range(8)] + [Tex("\\vdots")]
        )
        ...
        for i, p_label in enumerate(p_labels[:-1]):
            ...
            changing_decimals.append(ChangingDecimal(decimal, get_alpha_func(i + 5)))
        for i, pixel in enumerate(pixels):
            pixel.func = get_alpha_func(i, pixel.get_fill_opacity())
        pixel_updates = [UpdateFromAlphaFunc(pixel, lambda p, a: p.set_fill(opacity=p.func(a))) for pixel in pixels]

        self.play(
            Write(question, run_time=2),
            GrowFromPoint(arrow, arrow.get_start()),
            pixels_group.set_height,
            3,
            pixels_group.to_edge,
            RIGHT,
            LaggedStartMap(FadeIn, p_labels),
            LaggedStartMap(FadeIn, decimals),
        )
        self.wait()
        self.play(*changing_decimals + pixel_updates, run_time=5, rate_func=linear)
    ...
    def show_weights(self):
        ...
        for p_label in p_labels:
            w_label = OldTex(p_label.get_tex().replace("p", "w"))
        ...
        self.play(
            MoveToTarget(edges),
            *[ChangingDecimal(decimal, get_alpha_func(r)) for decimal, r in zip(decimals, random_numbers)],
        )
        self.play(
            LaggedStartMap(
                ApplyMethod,
    ...
    def show_weighted_sum(self):
        ...
        w_labels = VGroup(*[VGroup(*label[:-1]).copy() for label in self.w_labels])
        layer = self.network_mob.layers[0]

        a_vect = np.random.random(16)
        active_layer = self.network_mob.get_active_layer(0, a_vect)

        a_labels = VGroup(*[OldTex("a_%d" % d) for d in range(1, 5)])

        weighted_sum = VGroup(*it.chain(*[[w, a, OldTex("+")] for w, a in zip(w_labels, a_labels)]))
        weighted_sum.add(
            OldTex("\\cdots"),
            OldTex("+"),
        ...
        self.play(
            ...
            *[ReplacementTransform(n.copy(), a) for n, a in zip(layer.neurons, a_labels)]
            + [ReplacementTransform(n.copy(), weighted_sum[-4]) for n in layer.neurons[4:-1]]
            + [ReplacementTransform(layer.neurons[-1].copy(), weighted_sum[-1])]
            + [Write(weighted_sum[i]) for i in list(range(2, 12, 3)) + [-4, -3]],
            run_time=1.5,
        )
        self.wait()
        self.play(
            *[ReplacementTransform(w1.copy(), w2) for w1, w2 in zip(self.w_labels, w_labels)[:4]]
            + [ReplacementTransform(w.copy(), weighted_sum[-4]) for w in self.w_labels[4:-1]]
            + [ReplacementTransform(self.w_labels[-1].copy(), weighted_sum[-2])],
            run_time=2,
        )
        self.wait(2)
    ...
    def organize_weights_as_grid(self):
        ...
        self.play(
            FadeIn(VGroup(*weight_grid[len(decimals) :]), lag_ratio=0.5, run_time=3),
            *[ReplacementTransform(decimal, pixel) for decimal, pixel in zip(decimals, weight_grid)],
        )
        self.wait()
    ...
    def make_most_weights_0(self):
        ...
        self.play(MoveToTarget(weight_grid, run_time=2, lag_ratio=0.5))
        self.wait()
        self.play(Transform(pixels, digit, run_time=2, lag_ratio=0.5))
        self.wait()
    ...
    def add_negative_weights_around_the_edge(self):
        ...
        self.play(
            *[
                ApplyMethod(weight_grid[28 * y + x].set_fill, self.negative_weights_color, 0.5)
                for y in (6, 10)
    ...
    def get_pixels_to_detect(self, pixels):
        ...
        return VGroup(*it.chain(*[pixels[d * n + d / 2 - 4 : d * n + d / 2 + 4] for n in range(7, 10)]))

    def get_surrounding_pixels_for_edge(self, pixels):
        d = int(np.sqrt(len(pixels)))
        return VGroup(*it.chain(*[pixels[d * n + d / 2 - 4 : d * n + d / 2 + 4] for n in (6, 10)]))
...
class MotivateSquishing(Scene):
    ...
    def add_weighted_sum(self):
        weighted_sum = OldTex(
            *it.chain(*[["w_%d" % d, "a_%d" % d, "+"] for d in range(1, 5)] + [["\\cdots", "+", "w_n", "a_n"]])
        )
    ...
    def show_real_number_line(self):
        ...
        self.play(Transform(arrow1, arrow2, run_time=5, rate_func=there_and_back))
    ...
    def show_interval(self):
        ...
        self.play(ReplacementTransform(self.number_line.copy(), lower_number_line))
        self.play(
    ...
    def squish_into_interval(self):
        ...
        u = line.unit_size
        line.target.apply_function(lambda p: np.array([u * sigmoid(p[0])] + list(p[1:])))
        line.target.move_to(lower_line.number_to_point(0.5))

        arrow = Arrow(line.numbers.get_bottom(), line.target.get_top(), color=YELLOW)

        self.play(MoveToTarget(line), GrowFromPoint(arrow, arrow.get_start()))
        self.wait(2)
...
class IntroduceSigmoid(GraphScene):
    ...
    def add_title(self):
        ...
        equation = OldTex("\\sigma(%s) = \\frac{1}{1+e^{-%s}}" % (char, char))
    ...
    def add_graph(self):
        graph = self.get_graph(lambda x: 1.0 / (1 + np.exp(-x)), color=YELLOW)
...
class IncludeBias(IntroduceWeights):
    ...
    def setup_start(self):
        ...
        self.get_pixels_to_detect(weight_grid).set_fill(GREEN, 0.5)
        self.get_surrounding_pixels_for_edge(weight_grid).set_fill(RED, 0.5)
        weight_grid.move_to(digit)

        edges = self.neuron.edges_in
        self.make_edges_weighted(edges, 1.5 * np.random.random(len(edges)) - 0.5)
    ...
    def comment_on_need_for_bias(self):
        ...
        words = OldTexText("Only activate meaningfully \\\\ when", "weighted sum", "$> 10$")
    ...
    def add_bias(self):
        ...
        self.play(
            ReplacementTransform(self.gt_ten.copy(), bias, run_time=2),
            MoveToTarget(rp),
        )
        self.wait(2)
        self.play(ShowCreation(rect), Write(name))
...
class ContinualEdgeUpdate(VGroup):
    ...
    def __init__(self, network_mob, **kwargs):
        ...
        for edge in edges:
            edge.colors = [random.choice(self.colors) for x in range(n_cycles)]
            msw = self.max_stroke_width
            edge.widths = [msw * random.random() ** self.stroke_width_exp for x in range(n_cycles)]
...
class ShowRemainingNetwork(IntroduceWeights):
    ...
    def show_all_of_second_layer(self):
        ...
        for neuron in neurons[:7]:
            ...
            if last_edges is not None:
                added_anims += [last_edges.set_stroke, None, 1]
        ...
        self.play(
            LaggedStartMap(
                ShowCreation,
                VGroup(*[n.edges_in for n in neurons[7:]]),
                run_time=3,
            ),
            LaggedStartMap(
                FadeIn,
            ...
            VGroup(*last_edges[1:]).set_stroke,
            None,
            1,
        )
        self.wait()
    ...
    def count_in_biases(self):
        ...
        arrows = VGroup(*[Arrow(words.get_left(), neuron.get_center(), color=BLUE) for neuron in neurons])
    ...
    def compute_layer_two_of_weights_and_biases_count(self):
        ...
        self.play(Write(times_16), MoveToTarget(ww2), FadeOut(ww3))
        self.wait()
        self.play(
    ...
    def show_remaining_layers(self):
        ...
        added_weights = OldTex("+16\\!\\times\\! 16 + 16 \\!\\times\\! 10")
        added_weights.to_corner(UP + RIGHT)
        weights_count.prefix.next_to(added_weights, LEFT, SMALL_BUFF)
        weights_count.target[-1].next_to(VGroup(weights_count.prefix, added_weights), DOWN)
        ...
        neurons = VGroup(*it.chain(*[layer.neurons for layer in network_mob.layers[2:]]))
    ...
    def tweak_weights(self):
        ...
        finding_words = OldTexText("Finding the right \\\\ weights and biases")
...
class ImagineSettingByHand(Scene):
    def construct(self):
        ...
        network_mob = NetworkMobject(Network(sizes=[8, 6, 6, 4]), neuron_stroke_color=WHITE)
...
class WhenTheNetworkFails(MoreHonestMNistNetworkPreview):
    CONFIG = {"network_mob_config": {"layer_to_layer_buff": 2}}
    ...
    def black_box(self):
        ...
        self.play(Write(words, run_time=2), DrawBorderThenFill(box))
        self.wait()
    ...
    def ask_about_weights(self):
        question = OldTexText("What weights are used here?\\\\", "What are they doing?")
...
class EvenWhenItWorks(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Even when it works,\\\\", "dig into why.")
        self.play_student_changes(*["pondering"] * 3)
        self.wait(7)
...
class IntroduceWeightMatrix(NetworkScene):
    ...
    def activate_first_layer(self):
        ...
        a_labels = VGroup(*[OldTex("a^{(0)}_%d" % d) for d in range(len(layer.neurons))])
        for label, neuron in zip(a_labels, layer.neurons):
            label.scale(0.75)
            label.move_to(neuron)

        self.play(Transform(layer, active_layer), Write(a_labels, run_time=2))
    ...
    def show_first_neuron_weighted_sum(self):
        ...
        w_labels = VGroup(*[OldTex("w_{0, %d}" % d) for d in range(len(a_labels))])
        weighted_sum = VGroup()
        symbols = VGroup()
        for a_label, w_label in zip(a_labels.target, w_labels):
            a_label.scale(1.0 / 0.75)
            plus = OldTex("+")
        ...
        VGroup(a1_label, equals, weighted_sum).scale(0.75, about_point=a1_label.get_left())

        w_labels.set_color(GREEN)
        w_labels.shift(0.6 * SMALL_BUFF * DOWN)
        a_labels.target.shift(0.5 * SMALL_BUFF * UP)

        self.play(Write(a1_label), Write(equals), neuron.set_fill, None, 0.3, run_time=1)
    ...
    def organize_activations_into_column(self):
        ...
        last_a = column[-1]
        new_last_a = OldTex(last_a.get_tex().replace("7", "n"))
        new_last_a.replace(last_a)
        Transform(last_a, new_last_a).update(1)

        VGroup(*column[:2] + [mid_as] + [column[-1]]).arrange(DOWN)
        ...
        self.play(LaggedStartMap(Indicate, self.a_labels, rate_func=there_and_back, run_time=1))
        self.play(MoveToTarget(a_labels), Transform(pre_brackets, post_bracketes), run_time=2)
        self.wait()
        self.play(*[LaggedStartMap(Indicate, mob, rate_func=there_and_back) for mob in (a_labels, a_labels_in_sum)])
        self.wait()
    ...
    def organize_weights_as_matrix(self):
        ...
        w_labels.submobjects.insert(2, self.symbols[-2].copy())
        ...
        dots_row = VGroup(*list(map(Tex, ["\\vdots", "\\vdots", "\\ddots", "\\vdots"])))
        ...
        self.play(MoveToTarget(w_labels), Write(w_brackets, run_time=1))
        self.play(
            FadeIn(
                lower_rows,
                run_time=3,
    ...
    def show_meaning_of_matrix_row(self):
        ...
        self.play(ShowCreation(rect))
        for x in range(2):
            self.play(LaggedStartMap(ShowCreationThenDestruction, edges, lag_ratio=0.8))
        self.wait()
    ...
    def connect_weighted_sum_to_matrix_multiplication(self):
        ...
        brace = Brace(VGroup(self.w_labels, self.a_labels_in_sum), DOWN)
        arrow = Arrow(brace.get_bottom(), result_terms[0].get_top(), buff=SMALL_BUFF)
        ...
        self.play(ReplacementTransform(VGroup(top_row_rect, column_rect).copy(), result_terms[0]))
        self.play(LaggedStartMap(FadeIn, VGroup(*result_terms[1:])))
        self.wait(2)
        self.show_meaning_of_lower_rows(arrow, brace, top_row_rect, result_terms)
        self.play(*list(map(FadeOut, [result_terms, result_brackets, equals, column_rect])))

    def show_meaning_of_lower_rows(self, arrow, brace, row_rect, result_terms):
        n1, n2, nk = neurons = VGroup(*[self.network_mob.layers[1].neurons[i] for i in (0, 1, -1)])
        for n in neurons:
            n.save_state()
        ...
        self.play(
            ...
            None,
            0,
            n1.edges_in.set_stroke,
            None,
            1,
            n2.set_stroke,
            WHITE,
            3,
            n2.edges_in.set_stroke,
            None,
            3,
    ...
    def add_bias_vector(self):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*b_column[1:])))
        self.wait()
    ...
    def apply_sigmoid(self):
        expression_bounds = VGroup(self.matrix_brackets[0], self.b_brackets[1])
    ...
    def write_clean_final_expression(self):
        ...
        expression.set_color_by_tex_to_color_map({"sigma": YELLOW, "big": YELLOW, "W": GREEN, "\\textbf{b}": BLUE})
        ...
        for neuron in neurons:
            neuron_anims += [neuron.restore, neuron.set_fill, None, random.random()]
            neuron_anims += [neuron.edges_in.restore]
        neurons.add_to_back(self.network_mob.layers[1].neurons[0])

        self.play(
            ReplacementTransform(
                VGroup(self.top_matrix_row, self.lower_matrix_rows, self.matrix_brackets).copy(),
                VGroup(W),
            )
        )
        self.play(
        ...
        self.play(
            ReplacementTransform(VGroup(self.b_column, self.b_brackets).copy(), VGroup(VGroup(b))),
            ReplacementTransform(self.bias_plus.copy(), plus),
        )
        self.play(ReplacementTransform(self.big_sigma_group.copy(), VGroup(sigma, lp, rp)))
        self.wait()
        self.play(*neuron_anims, run_time=2)
        self.play(ReplacementTransform(neurons.copy(), a1), FadeIn(equals))
        self.wait(2)
...
class EoLA3Wrapper(PiCreatureScene):
    def construct(self):
        ...
        self.play(ShowCreation(rect), FadeIn(title), morty.change, "raise_right_hand", rect)
        self.wait(4)
...
class NeuronIsFunction(MoreHonestMNistNetworkPreview):
    ...
    def activate_network(self):
        ...
        for i, activation in enumerate(activations):
            ...
            Transform(layer, self.network_mob.get_active_layer(i, activation)).update(1)
        self.add(mnist_mob)
    ...
    def write_neuron_holds_a_number(self):
        ...
        self.play(
            ...
            neuron_group.scale,
            3,
            neuron_group.next_to,
            neuron_word,
            LEFT,
            FadeIn(neuron_word),
            GrowArrow(arrow),
            FadeIn(thing_words, run_time=2, rate_func=squish_rate_func(smooth, 0.3, 1)),
        )
        self.wait()
    ...
    def neuron_is_function(self):
        ...
        self.play(thing_words.fade, ShowCreation(cross))
        self.play(
            FadeIn(function_word),
            VGroup(thing_words, cross).to_edge,
            DOWN,
        )
        self.wait()
    ...
    def show_neuron_as_function(self):
        ...
        self.play(GrowArrow(arrow), Transform(self.decimal, decimal))
        self.wait(2)

        self.non_faded_network_parts = VGroup(neuron, edges, prev_layer)
        self.neuron_arrow = arrow

    def fade_network_back_in(self):
        anims = [
            FadeIn(mob, run_time=2, lag_ratio=0.5) for mob in (self.network_mob.layers, self.network_mob.edge_groups)
        ]
        anims += [
    ...
    def network_is_a_function(self):
        ...
        self.play(ReplacementTransform(neuron_word, network_word), FadeIn(func_tex))
...
class NextVideo(MoreHonestMNistNetworkPreview, PiCreatureScene):
    ...
    def talk_about_subscription(self):
        ...
        morty.target.rotate(np.pi, axis=UP, about_point=morty.get_left())
        morty.target.shift(LEFT)
        video = self.video

        subscribe_word = OldTexText("Subscribe", "!", arg_separator="")
    ...
    def show_video_neural_network(self):
        ...
        neuron_pairs = VGroup(*[VGroup(*network_mob.layers[-1].neurons[2 * i : 2 * i + 2]) for i in range(1, 5)])
        for pair in neuron_pairs:
            video = video.copy()
            video.move_to(pair, LEFT)
            pair.target = video

        self.play(LaggedStartMap(MoveToTarget, neuron_pairs, run_time=3))
...
class IntroduceReLU(IntroduceSigmoid):
    ...
    def old_school(self):
        ...
        sigmoid_title = VGroup(self.sigmoid_name, self.equation)
        cross = Cross(sigmoid_title)
        old_school = OldTexText("Old school")
        old_school.to_corner(UP + RIGHT)
        old_school.set_color(RED)
        arrow = Arrow(old_school.get_bottom(), self.equation.get_right(), color=RED)

        self.play(ShowCreation(cross))
        self.play(Write(old_school, run_time=1), GrowArrow(arrow))
        self.wait(2)
        self.play(
            ApplyMethod(
        ...
        self.play(ShowCreation(self.sigmoid_graph, rate_func=lambda t: smooth(1 - t), remover=True))
    ...
    def show_ReLU(self):
        ...
        self.wait(2)
        self.play(Write(name), equation.shift, DOWN)
        self.wait(2)
    ...
    def label_input_regions(self):
        ...
        pos_words.next_to(self.coords_to_point(1, 1), DOWN + RIGHT)
...
class CompareSigmoidReLUOnDeepNetworks(PiCreatureScene):
    def construct(self):
        ...
        network_mob = NetworkMobject(Network(sizes=[6, 4, 5, 4, 3, 5, 2]))
        ...
        self.play(Write(slow_learner, run_time=1), GrowArrow(slow_arrow))
        self.wait()
        self.play(
            FadeIn(relu_name),
...
class ShowAmplify(PiCreatureScene):
    def construct(self):
        ...
        self.play(ShowCreation(rect), morty.change, "raise_right_hand")
        self.wait(2)
...
class NeuralNetImageAgain(Scene):
    def construct(self):
        ...
        for length in [16, 16, 16, 10]:
            circs = VGroup(*[Circle(radius=1) for x in range(length)])
        ...
        for l1, l2 in zip(layers, layers[1:]):
            ...
            for n1, n2 in it.product(l1, l2):
                edge = Line(n1.get_center(), n2.get_center(), buff=n1.get_height() / 2)
