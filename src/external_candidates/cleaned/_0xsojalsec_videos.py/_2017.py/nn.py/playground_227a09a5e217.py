...
class Test(Scene):
    ...
    def show_random_results(self):
        group = Group(
            *[
                Group(
                    *[MNistMobject(a) for a in network.get_activation_of_all_layers(np.random.randn(784, 1))]
                ).arrange(RIGHT)
                for x in range(10)
            ]
        ).arrange(DOWN)
    ...
    def show_all_activation_images(self, network, test_data):
        image_samples = Group(*[self.get_activation_images(digit, network, test_data) for digit in range(10)])
        image_samples.arrange_in_grid(n_rows=2, buff=LARGE_BUFF)
    ...
    def get_activation_images(self, digit, network, test_data, n_examples=8):
        ...
        activation_iamges = Group(
            *[
                Group(*[MNistMobject(a) for a in network.get_activation_of_all_layers(vect)]).arrange(RIGHT)
    ...
    def show_two_blend(self):
        training_data, validation_data, test_data = load_data_wrapper()
        vects = [data[0] for data in training_data[:30] if np.argmax(data[1]) == 2]
    ...
    def show_test_input(self, network):
        training_data, validation_data, test_data = load_data_wrapper()
        group = Group(*[self.get_set(network, test) for test in test_data[3:20] if test[1] in [4, 9]])
