...
class DescribeDigitalSignatures(LedgerScene):
    ...
    def show_digital_signatures(self):
        ...
        arrows = VGroup(
            *[Arrow(rect.get_corner(UP), sig.get_bottom(), tip_length=0.15, color=WHITE) for sig in digital_signatures]
        )
    ...
    def show_signing_functions(self):
        ...
        self.wait()
        self.play(
            *[FadeIn(part) for part in verify if part not in [verify.message, verify.signature, verify.pk, verify[-1]]]
        )
        self.wait()
        self.play(
...
class CharlieRacksUpDebt(SignedLedgerScene):
    ...
    def construct(self):
        ...
        self.play(
            *[ApplyMethod(pi.change, "sassy", self.charlie) for pi in self.pi_creatures if pi is not self.charlie]
        )
...
class LedgerWithInitialBuyIn(SignedLedgerScene):
    ...
    def add_initial_lines(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.change, "thinking", self.ledger) for pi in self.pi_creatures],
        )
        self.wait()
...
class ThisIsWellIntoTheWeeds(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            target_mode="sassy",
            added_anims=[ApplyMethod(pi.change, mode) for pi, mode in zip(self.students, ["hooray", "sad", "erm"])],
        )
        self.wait()
        self.play(
            idea.restore,
            RemovePiCreatureBubble(self.teacher, target_mode="hooray", look_at=lightbulb),
        )
...
class IntroduceSHA256(Scene):
    ...
    def issue_challenge(self):
        ...
        self.play(
            Transform(self.digest, new_digest, run_time=2, lag_ratio=0.5, path_arc=np.pi / 2),
...
class IntroduceBlockChain(Scene):
    ...
    def get_block(self):
        ...
        h_line1, h_line2 = [
            Line(rect.get_left(), rect.get_right()).shift(0.3 * rect.get_height() * vect) for vect in (UP, DOWN)
        ]
...
class IntroduceBlockCreator(DistributedBlockChainScene):
    ...
    def broadcast_block(self):
        ...
        self.play(
            *it.chain(
                arrow_creations,
                [ApplyMethod(pi.change, "hooray", pi.block_chain.get_right()) for pi in self.pi_creatures],
            )
        )
        self.wait(3)
...
class TwoBlockChains(DistributedBlockChainScene):
    ...
    def defer_to_longer(self):
        ...
        self.play(
            ...
            chain,
            Broadcast(corner),
            ApplyMethod(chain.restore, rate_func=squish_rate_func(smooth, 0.3, 0.7), run_time=3),
        )
...
class AliceRacesOtherMiners(DoubleSpendingAttack):
    ...
    def receive_broadcast_from_other_miners(self):
        ...
        self.play(
            Broadcast(block),
            *[MoveToTarget(mover, run_time=3, rate_func=squish_rate_func(smooth, 0.3, 0.8)) for mover in movers],
        )
        self.play(
            FadeIn(winner.block),
            winner.change_mode,
...
class ShowManyExchanges(Scene):
    ...
    def construct(self):
        ...
        exchange = VGroup(
            *[Arrow(p1, p2, path_arc=np.pi, buff=MED_LARGE_BUFF) for p1, p2 in [(LEFT, RIGHT), (RIGHT, LEFT)]]
        ).set_color(WHITE)
        exchanges = VGroup(
            *[VGroup(*[exchange.copy() for x in range(3)]).arrange(RIGHT, buff=2 * LARGE_BUFF) for y in range(3)]
