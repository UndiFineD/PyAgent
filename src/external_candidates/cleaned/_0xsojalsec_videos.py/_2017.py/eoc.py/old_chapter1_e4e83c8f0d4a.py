...
class HeartOfCalculus(GraphScene):
    ...
    def construct(self):
        ...
        rect_sets = [
            self.get_riemann_rectangles(0, self.x_max, 1.0 / (2**n), stroke_width=1.0 / (n + 1)) for n in range(6)
        ]
...
class NameDerivative(IntroduceTinyChangeInArea):
    ...
    def interpret_geometrically(self):
        ...
        self.play(
            ...
            MoveToTarget(circum_circle, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)),
            self.pi_creature.change_mode,
            "hooray",
        )
        self.wait()
...
class IntroduceConcentricRings(CircleScene):
    ...
    def ask_about_approx(self):
        ...
        self.wait(2)
        self.play(
            *[ApplyMethod(ring.set_stroke, ring.get_color(), width=1) for ring in self.rings]
...
class GraphIntegral(GraphScene):
    ...
    def add_rectangles(self):
        ...
        ticks = VGroup(
            *[Line(UP, DOWN).move_to(self.coords_to_point(x, 0)) for x in np.arange(0, self.R + self.dr, self.dr)]
        )
        ...
        self.wait()
        self.play(
            *[ApplyMethod(rect.fade, 0.7) for rect in rectangles if rect is not special_rect] + [Animation(ticks)]
        )
