...
class LengthOfDayGraph(GraphScene):
    ...
    def add_graph(self):
        ...
        self.play(
            ...
            FadeIn(graph_label, rate_func=squish_rate_func(smooth, 0.5, 1), lag_ratio=0.5),
            run_time=3,
        )
        self.wait()
...
class AverageOfFiniteSet(Scene):
    ...
    def construct(self):
        ...
        labels = VGroup(
            *[OldTex(str(d)).next_to(line, UP).set_color(line.get_color()) for d, line in zip(lengths, lines)]
        )
        ...
        dividing_lines = VGroup(
            *[
                ...
                for p in [interpolate(lines.target.get_left(), lines.target.get_right(), alpha)]
            ]
        )
...
class FiniteSample(TryToAddInfinitelyManyPoints):
    ...
    def show_finite_sample(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(v_lines.copy(), summed_v_lines, run_time=3, lag_ratio=0.5),
...
class GeneralAverage(AverageOfContinuousVariable):
    ...
    def ask_about_average(self):
        ...
        self.play(
            next_v_line_anim(),
            Write(
                VGroup(*[fraction.get_part_by_tex(tex) for tex in ("int", "f(x)", "dx", "over")]),
    ...
    def show_finite_sample(self):
        ...
        self.play(
            ...
            *[ApplyFunction(lambda m: m.fade().set_stroke(width=2), v_line) for v_line in v_lines],
        )
        self.play(
            *[
                ApplyFunction(
