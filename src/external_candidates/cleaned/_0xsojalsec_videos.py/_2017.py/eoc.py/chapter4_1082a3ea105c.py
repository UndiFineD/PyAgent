...
class TransitionFromLastVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi_creature.change_mode, "pondering") for pi_creature in self.get_pi_creatures()],
        )
        self.wait(2)
...
class SumRule(GraphScene):
    ...
    def add_graphs(self):
        ...
        self.play(
            *it.chain(
                [ApplyMethod(l2.move_to, l1.get_top(), DOWN) for l1, l2 in zip(*v_line_sets)],
    ...
    def show_example_stacking(self):
        ...
        example_v_lines, nudged_v_lines = [
            VGroup(*[v_lines[index] for v_lines in v_line_sets]) for index in (num_lines / 2, num_lines / 2 + 1)
        ]
        ...
        self.play(
            ...
            *it.chain(*[[line.restore, line.set_stroke, None, 4] for line in nudged_v_lines]),
        )
        self.wait()
        for line in nudged_v_lines:
...
class IntroduceProductAsArea(ReconfigurableScene):
    ...
    def get_df_boxes(self):
        ...
        df_boxes = VGroup(
            *[
                Rectangle(height=height, width=width, **self.df_box_kwargs).next_to(box, vect, buff=0)
    ...
    def expand_derivative(self):
        ...
        self.play(
            *[
                ReplacementTransform(expanded_deriv[i], final_deriv[j], path_arc=-np.pi / 2)
                for i, j in [
                    (0, 0),
                    (1, 2),
                    (2, 3),
...
class ShoveXSquaredInSine(Scene):
    def construct(self):
        ...
        for premob, pre_indices, comp_indicies in triplets:
            self.play(
                *[ReplacementTransform(premob[i].copy(), composition[j]) for i, j in zip(pre_indices, comp_indicies)]
            )
            self.wait()
        self.wait()
...
class ThreeLinesChainRule(ReconfigurableScene):
    ...
    def nudge_x(self):
        ...
        dx_squared_value.shift(
            dx_squared_brace.text.get_right() + MED_SMALL_BUFF * RIGHT - dx_squared_value[0].get_left()
        )
        ...
        self.animate_x_change(
            ...
            run_time=1,
            added_anims=it.chain([GrowFromCenter(dx_brace)], list(map(ShowCreation, nudge_lines))),
        )
...
class GeneralizeChainRule(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            *it.chain(*[[mob.scale, 1.2, mob.set_color, YELLOW] for mob in (example_inner, d_example_inner)]),
            rate_func=there_and_back,
        )
