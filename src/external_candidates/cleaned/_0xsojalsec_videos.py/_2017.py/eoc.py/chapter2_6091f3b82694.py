# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter2.py
...
class IncrementNumber(Succession):
    ...
    def __init__(self, num_mob, **kwargs):
        ...
        new_num_mobs = [
            OldTex(str(num)).move_to(num_mob, LEFT) for num in range(self.start_num, self.start_num + n_iterations)
        ]
        transforms = [
            Transform(
                num_mob,
...
class Introduction(TeacherStudentsScene):
    def construct(self):
        goals = OldTexText("Goals: ", "1) Learn derivatives", ", 2) Avoid paradoxes.", arg_separator="")
        ...
        self.student_says("What is a derivative?", run_time=2)
        self.play(self.get_teacher().change_mode, "happy")
        self.wait()
        self.teacher_says("It's actually a \\\\", "very subtle idea", target_mode="well")
        ...
        self.play(
            ...
            *it.chain(
                *[
                    [pi.change_mode, mode, pi.look_at, FRAME_Y_RADIUS * UP]
                    for pi, mode in zip(
        ...
        change_description = change_brace.get_text("Requires multiple \\\\ points in time")
        instantaneous_brace = Brace(instantaneous)
        instantaneous_description = instantaneous_brace.get_text("One point \\\\ in time")
        clock = Clock()
...
class IntroduceCar(Scene):
    ...
    def construct(self):
        ...
        if self.show_distance:
            self.play(GrowFromCenter(distance_brace), Write(distance))
            self.wait()
        ...
        if self.should_transition_to_graph:
            ...
            for mob in car_line_group:
                mob.generate_target()
            car_line_group.target = VGroup(*[m.target for m in car_line_group])
            B = car_line_group[2]
...
class GraphCarTrajectory(GraphScene):
    ...
    def graph_sigmoid_trajectory_function(self, **kwargs):
        graph = self.get_graph(lambda t: 100 * smooth(t / 10.0), **kwargs)
        self.s_graph = graph
        return graph

    def introduce_graph(self, graph, origin):
        h_line, v_line = [Line(origin, origin, color=color, stroke_width=2) for color in (TIME_COLOR, DISTANCE_COLOR)]
        ...
        self.play(
            ...
            MoveCar(car, car_target, rate_func=self.care_movement_rate_func),
        ...
        # Name graph
        s_of_t = OldTex("s(t)")
        s_of_t.next_to(graph.point_from_proportion(1), DOWN + RIGHT, buff=SMALL_BUFF)
        s = s_of_t[0]
        d = OldTex("d")
        d.move_to(s, DOWN)
    ...
    def comment_on_slope(self, graph, origin):
        delta_t = 1
        curr_time = 0
        ghost_line = Line(origin, self.coords_to_point(delta_t, self.y_max))
        ...
        for x in range(1, 10):
            ...
            self.play(
                rect.move_to,
                self.coords_to_point(curr_time, 0),
                DOWN + LEFT,
                Transform(change_lines, new_change_lines),
            )
            if curr_time == 5:
                text = change_lines[-1].get_text("$\\frac{\\text{meters}}{\\text{second}}$")
    ...
    def get_change_lines(self, curr_time, delta_t=1):
        p1 = self.input_to_graph_point(curr_time, self.s_graph)
        p2 = self.input_to_graph_point(curr_time + delta_t, self.s_graph)
    ...
    def show_velocity_graph(self):
        ...
        for time, show_slope in (4.5, True), (9, False):
            self.play(self.rect.move_to, self.coords_to_point(time, 0), DOWN + LEFT)
            if show_slope:
        ...
        def shallow_slope(t):
            return 100 * smooth(t / 10.0, inflection=4)

        def steep_slope(t):
            return 100 * smooth(t / 10.0, inflection=25)

        def double_smooth_graph_function(t):
            if t < 5:
                return 50 * smooth(t / 5.0)
            else:
                return 50 * (1 + smooth((t - 5) / 5.0))
    ...
    def ask_critically_about_velocity(self):
        ...
        self.play(PiCreatureSays(morty, "Think critically about \\\\", "what velocity means."))
...
class ShowSpeedometer(IntroduceCar):
    ...
    def setup(self):
        ...
        needle = Polygon(LEFT, UP, RIGHT, stroke_width=0, fill_opacity=1, fill_color=YELLOW)
...
class VelocityInAMomentMakesNoSense(Scene):
    def construct(self):
        ...
        self.play(randy.change_mode, "confused", Write(words))
...
class SnapshotOfACar(Scene):
    def construct(self):
        ...
        self.play(MoveCar(car, RIGHT + DOWN, run_time=2, rate_func=rush_into))
        car.get_tires().set_color(GREY)
        speed_lines.next_to(car, LEFT)
        self.add(speed_lines)
        self.play(flash_box.set_fill, None, 0, rate_func=rush_from)
...
class CompareTwoTimes(Scene):
    ...
    def show_equation(self):
        ...
        change_over_change = OldTex("\\frac{\\text{Change in distance}}{\\text{Change in time}}")
        formula = OldTex(
            "\\frac{(%s - %s) \\text{ meters}}{(%s - %s) \\text{ seconds}}"
            % (
                str(self.end_distance),
                str(self.start_distance),
                str(self.end_time),
                str(self.start_time),
            )
        )
        ...
        self.play(FadeIn(group, lag_ratio=0.5, run_time=3))
        self.wait(3)
        self.formula = formula

    def fade_all_but_one_moment(self):
        anims = [ApplyMethod(mob.fade, 0.5) for mob in self.get_mobjects()]
    ...
    def get_car_state(self, distance, time):
        ...
        distance_label = brace.get_text(str(distance), " meters")
        distance_label.set_color_by_tex(str(distance), DISTANCE_COLOR)
        brace.add(distance_label)
        time_label = OldTexText("Time:", str(time), "seconds")
        time_label.set_color_by_tex(str(time), TIME_COLOR)
        time_label.next_to(VGroup(line, car), UP, aligned_edge=LEFT)
...
class VelocityAtIndividualPointsVsPairs(GraphCarTrajectory):
    ...
    def construct(self):
        self.setup_axes(animate=False)
        distance_graph = self.graph_function(lambda t: 100 * smooth(t / 10.0))
        distance_label = self.label_graph(distance_graph, label="s(t)", proportion=1, direction=RIGHT, buff=SMALL_BUFF)
    ...
    def show_individual_times_to_velocity(self, velocity_graph):
        ...
        self.wait()
        self.play(UpdateFromAlphaFunc(line, line_update, run_time=4, rate_func=there_and_back))
        self.wait()
    ...
    def show_two_times_on_distance(self):
        ...
        for line, brace, text in (
            (dt_line, dt_brace, dt_text),
            (
                ds_line,
                ds_brace,
                ds_text,
            ...
            self.play(ShowCreation(line), GrowFromCenter(brace), Write(text))
            self.wait()
...
class FirstRealWorld(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("happy", "hooray", "happy")
        self.wait(3)
...
class SidestepParadox(Scene):
    def construct(self):
        ...
        title = OldTexText("Instantaneous", "rate of change")
        title.to_edge(UP)
        cross = OldTex("\\times")
...
class CompareTwoVerySimilarTimes(CompareTwoTimes):
    ...
    def construct(self):
        ...
        ds_symbols, dt_symbols = [
            VGroup(*[mob for mob in formula if mob.get_color() == Color(color)])
        ...
        self.play(GrowFromCenter(dt_brace), Write(dt_text))
        formula.add(dt_brace, dt_text)
        self.wait(2)

        formula.generate_target()
        VGroup(ds_brace, ds_text, formula.target).move_to(formula, UP).shift(0.5 * UP)
        self.play(MoveToTarget(formula), GrowFromCenter(ds_brace), Write(ds_text))
        self.wait(2)
...
class DsOverDtGraphically(GraphCarTrajectory, ZoomedScene):
    ...
    def construct(self):
        ...
        align_little_rectangle_on_ds_dt_group(self.little_rectangle.target)
        self.play(MoveToTarget(self.little_rectangle), run_time=3)
        ...
        rise_over_run = OldTex("=\\frac{\\text{rise}}{\\text{run}}")
        ...
        full_formula = OldTex("=\\frac{s(t+dt) - s(t)}{dt}")
        ...
        t_tick_marks = VGroup(
            *[
                ...
                .scale(0.1)
                .move_to(self.coords_to_point(t, 0))
                for t in np.linspace(0, 10, 75)
            ]
        )

        v_line_at_t, v_line_at_t_plus_dt = [
            self.get_vertical_line_to_graph(time, line_class=Line, line_kwargs={"color": MAROON_B})
        ...
        for anim_class in FadeIn, FadeOut:
            self.play(anim_class(t_tick_marks, lag_ratio=0.5, run_time=2))
        self.play(Write(equals), Write(numerator))
        self.wait()
        ...
        self.play(Write(frac_line), Write(lower_dt))
        self.wait(2)
        ...
        def sin_wiggle(t):
            return (10 / (2 * np.pi / 10.0)) * (np.sin(2 * np.pi * t / 10.0) + 2 * np.pi * t / 10.0)
        ...
        for graph_func in graph_funcs:
            new_graph = self.graph_function(graph_func, color=DISTANCE_COLOR, is_main_graph=False)
            self.remove(new_graph)
            new_velocity_graph = self.get_derivative_graph(
                graph=new_graph,
            )
...
class DefineTrueDerivative(Scene):
    def construct(self):
        ...
        numerical_rhs_list = [self.get_fraction("0.%s1" % ("0" * x)) for x in range(7)]
        ...
        self.play(GrowFromCenter(brace), Write(dt_to_zero))
        self.wait()

    def get_fraction(self, dt_string):
        tex_mob = OldTex("\\frac{s(t + %s) - s(t)}{%s}" % (dt_string, dt_string))
        part_lengths = [
            0,
            len("s(t+"),
        ...
        result = VGroup(*[VGroup(*tex_mob[i1:i2]) for i1, i2 in zip(pl_cumsum, pl_cumsum[1:])])
...
class SecantLineToTangentLine(GraphCarTrajectory, DefineTrueDerivative):
    ...
    def get_ds_dt_group(self, dt, animate=False):
        points = [self.input_to_graph_point(time, self.graph) for time in (self.curr_time, self.curr_time + dt)]
        dots = list(map(Dot, points))
        for dot in dots:
            dot.scale(0.5)
        secant_line = Line(*points)
        secant_line.set_color(VELOCITY_COLOR)
        secant_line.scale(self.secant_line_length / secant_line.get_length())
        ...
        if dt.get_width() > dt_line.get_width():
            dt.scale(dt_line.get_width() / dt.get_width(), about_point=dt.get_top())
        dt.next_to(dt_line, DOWN, buff=SMALL_BUFF)
        ds = OldTex("ds")
        ds.set_color(DISTANCE_COLOR)
        if ds.get_height() > ds_line.get_height():
            ds.scale(ds_line.get_height() / ds.get_height(), about_point=ds.get_left())
        ds.next_to(ds_line, RIGHT, buff=SMALL_BUFF)

        group = VGroup(secant_line, ds_line, dt_line, ds, dt, *dots)
        if animate:
            self.play(
            ...
            self.play(ShowCreation(secant_line), Animation(VGroup(*dots)))
        return group
    ...
    def draw_axes(self):
        ...
        self.play(
            Write(
                VGroup(self.x_axis, self.y_axis, self.graph, self.graph_label),
                run_time=4,
            )
        )
        self.wait()
    ...
    def show_tangent_line(self):
        ...
        self.play(UpdateFromAlphaFunc(ds_dt_group, update_ds_dt_group), run_time=15)
        self.wait()
        ...
        v_line = self.get_vertical_line_to_graph(
            ...
            self.graph,
            line_class=Line,
            line_kwargs={"color": MAROON_B, "stroke_width": 3},
        )
    ...
    def what_this_is_not_saying(self):
        phrases = [OldTexText("$dt$", "is", "not", s) for s in ("``infinitely small''", "0")]
    ...
    def best_constant_approximation_around_a_point(self):
        ...
        circle = Circle(radius=0.25, color=WHITE).shift(self.input_to_graph_point(self.curr_time))
...
class LeadIntoASpecificExample(TeacherStudentsScene, SecantLineToTangentLine):
    ...
    def construct(self):
        ...
        self.play(
            Write(deriv_def),
            *it.chain(*[[pi.change_mode, "thinking", pi.look_at, dot] for pi in self.get_students()]),
        )
        self.random_blink(3)
        # self.teacher_says(
        #     """
...
class TCubedExample(SecantLineToTangentLine):
    ...
    def draw_graph(self):
        ...
        self.label_graph(graph, label="s(t) = t^3", proportion=0.62, direction=LEFT, buff=SMALL_BUFF)
        self.wait()

    def show_vertical_lines(self):
        for t in 1, 2:
            v_line = self.get_vertical_line_to_graph(t, line_kwargs={"color": WHITE})
    ...
    def bear_with_me(self):
        ...
        self.play(PiCreatureSays(morty, "Bear with \\\\ me here", target_mode="sassy"))
        self.play(Blink(morty))
        self.wait()
    ...
    def add_ds_dt_group(self):
        ...
        self.play(FadeIn(lhs), *[mob.restore for mob in (ds, dt)])
        ...
        self.wait()
        self.play(v_lines[0].set_color, YELLOW, rate_func=there_and_back)
        self.wait()
        self.play(Write(non_numerator))
        self.wait(2)
        self.play(*list(map(MoveToTarget, s_pair)), **{"path_arc": -np.pi / 2})
    ...
    def brace_for_details(self):
        ...
        self.play(FadeIn(morty))
        self.play(morty.change_mode, "hooray", morty.look_at, self.rhs)
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change_mode, "sassy", morty.look, OUT)
        self.play(Blink(morty))
    ...
    def show_expansion(self):
        ...
        """)
        expression.set_width(VGroup(self.lhs, self.rhs).get_width())
        expression.next_to(self.lhs, DOWN, aligned_edge=LEFT, buff=LARGE_BUFF)
        term_lens = [
            len("23+"),
            len("3(2)2dt"),
            len("+3(2)(dt)2+"),
        ...
        terms = [VGroup(*expression[i1:i2]) for i1, i2 in zip([0] + list(np.cumsum(term_lens)), np.cumsum(term_lens))]
        ...
        self.play(
            *it.chain(
                *[
                    [tc.scale, 1.3, tc.get_corner(vect), tc.set_color, RED]
                    for tc, vect in zip(two_cubed_terms, [DOWN + RIGHT, DOWN + LEFT])
                ]
            )
        )
        ...
        self.play(Transform(self.ds_dt_group, self.get_ds_dt_group(self.start_dt)))
        self.play(
        ...
        self.play(ShowCreation(arrow), Write(approach_text))
        self.wait(2)
        self.wait()
        ...
        self.play(self.ds_dt_group.rotate, np.pi / 24, rate_func=wiggle)
        self.play(ShowCreation(v_line))
        self.wait()
        self.play(FadeOut(v_line))
        ...
        twos = [self.lhs[6], self.rhs[2], self.rhs[10], lhs_copy[6], deriv_term[2]]
        for two in twos:
            two.target = OldTex("t")
    ...
    def react_to_simplicity(self):
        ...
        self.play(FadeIn(morty))
        self.play(PiCreatureSays(morty, "That's \\\\ beautiful!", target_mode="hooray"))
        self.play(Blink(morty))
        self.play(
            morty.change_mode,
            "happy",
...
class YouWouldntDoThisEveryTime(TeacherStudentsScene):
    def construct(self):
        self.play_student_changes("pleading", "guilty", "hesitant", run_time=0)
        self.teacher_says("You wouldn't do this \\\\ every time")
...
class ContrastConcreteDtWithLimit(Scene):
    def construct(self):
        ...
        r_formula = OldTex("\\frac{d(t^3)}{dt} = 3t^2")
        ...
        self.play(
            *it.chain(
                *[
                    ...
                    for pi, mode in zip(self.get_students(), ["pondering", "happy", "hesitant"])
                ]
            )
        )
        self.wait(4)
...
class ParadoxAtTEquals0(TCubedExample):
    ...
    def draw_graph(self):
        ...
        self.play(
            MoveCar(car, car_target_point, rate_func=lambda t: (t * graph_x_max) ** 3),
        ...
        self.label_graph(graph, label="s(t) = t^3", proportion=0.8, direction=RIGHT, buff=SMALL_BUFF)
        self.wait()

        self.car = car

    def ask_question(self):
        question = OldTexText("At time $t=0$,", "is \\\\ the car moving?")
        VGroup(*question[0][-4:-1]).set_color(RED)
        question.next_to(self.coords_to_point(0, 10), RIGHT)
        origin = self.coords_to_point(0, 0)
    ...
    def show_tangent_line(self):
        ...
        def line_update(line, alpha):
            ...
            line.rotate(self.angle_of_tangent(time) - line.get_angle())
            line.move_to(point)
    ...
    def if_not_then_when(self):
        ...
        self.play(FadeIn(morty))
        self.play(PiCreatureSays(morty, "If not at $t=0$, when?", target_mode="maybe"))
        self.play(Blink(morty))
        self.play(
            MoveCar(
                self.car,
    ...
    def single_out_question(self):
        ...
        self.wait()
        self.play(self.tangent_line.rotate, np.pi / 24, rate_func=wiggle, run_time=1)
        self.play(Blink(morty))
        self.wait()
...
class TinyMovement(ZoomedScene):
    ...
    def show_initial_motion(self):
        ...
        dots = VGroup(*[Dot(point, radius=self.distance / 10) for point in (wheel_point, target_wheel_point)])
        ...
        self.play(zoom_rect.scale, 10 * self.distance / zoom_rect.get_width())
        ...
        self.play(zoom_rect.scale, 0.5, zoom_rect.move_to, brace)
        self.wait()

    def show_ratios(self):
        ratios = [self.get_ratio(n) for n in range(1, 5)]
        ratio = ratios[0]
...
class NextVideos(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), self.get_teacher().change_mode, "raise_right_hand")
        self.play(
            Write(brace_text),
            *it.chain(*[[pi.look_at, brace] for pi in self.get_students()]),
        )
        self.wait(2)
...
class Promotion(PiCreatureScene):
    ...
    def construct(self):
        ...
        url = OldTexText("AoPS.com/", "3blue1brown", arg_separator="")
        url.to_corner(UP + LEFT)
        url_rect = Rectangle(color=BLUE)
        url_rect.replace(url.get_part_by_tex("3blue1brown"), stretch=True)
        ...
        self.wait()
        self.play(
            RemovePiCreatureBubble(mathy, target_mode="happy"),
            aops_logo.to_corner,
            UP + RIGHT,
            aops_logo.shift,
            MED_SMALL_BUFF * DOWN,
        ...
        self.play(
            FadeOut(url_rect),
            url.get_part_by_tex("3blue1brown").set_color,
            BLUE,
        )
        self.wait(3)
