# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter8.py
import scipy
from _2017.eoc.chapter1 import Thumbnail as Chapter1Thumbnail
from _2017.eoc.chapter2 import (
    DISTANCE_COLOR,
    TIME_COLOR,
    VELOCITY_COLOR,
    Car,
...
class Chapter8OpeningQuote(OpeningQuote, PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait(3)
        self.play(author.restore, self.pi_creature.change_mode, "plain")
        self.play(
            words_copy.next_to,
            self.pi_creature,
            LEFT,
        ...
        self.wait(2)
        self.play(Write(formula), self.pi_creature.change_mode, "confused")
        self.wait()
...
class ThisVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            this_video.shift,
            this_video.get_height() * DOWN / 2,
            this_video.set_color,
            YELLOW,
        ...
        self.play(
            this_video.restore,
            next_video.shift,
            next_video.get_height() * DOWN / 2,
            next_video.set_color,
            YELLOW,
            integral[0].copy().next_to,
            next_video,
            DOWN,
            MED_LARGE_BUFF,
            FadeOut(formula),
            *it.chain(*[[pi.change_mode, "plain", pi.look_at, next_video] for pi in self.pi_creatures]),
        )
        self.wait(2)
...
class InCarRestrictedView(ShowSpeedometer):
    ...
    def construct(self):
        ...
        self.play(
            ...
            *self.get_added_movement_anims(rate_func=v_rate_func, radians=-(16.0 / 70) * 4 * np.pi / 3),
            run_time=8,
        )
        eight = OldTex("8").move_to(time_label[1])
        self.play(Transform(time_label[1], eight, rate_func=squish_rate_func(smooth, 0, 0.5)))
        self.wait()
...
class GraphDistanceVsTime(GraphCarTrajectory):
    ...
    def construct(self):
        ...
        graph_label = self.get_graph_label(graph, "s(t)", color=DISTANCE_COLOR)
...
class PlotVelocity(GraphScene):
    ...
    def plot_points(self):
        ...
        points = [self.coords_to_point(t, v_func(t)) for t in times]
        ...
        self.play(
            Succession(*dot_intro_anims, rate_func=linear),
            ApplyMethod(
    ...
    def get_v_graph_and_label(self):
        graph = self.get_graph(v_func, x_min=0, x_max=8, color=VELOCITY_COLOR)
        graph_label = OldTex("v(t)", "=t(8-t)")
        graph_label.set_color_by_tex("v(t)", VELOCITY_COLOR)
        graph_label.next_to(graph.point_from_proportion(7.0 / 8.0), UP + RIGHT)
        self.v_graph = graph
...
class AskAboutAntiderivative(PlotVelocity):
    ...
    def write_s_formula(self):
        ...
        self.play(Write(ds_dt, run_time=2), ShowCreation(arrow))
        self.wait()
...
class Antiderivative(PiCreatureScene):
    def construct(self):
        ...
        for pair in reversed(list(zip(functions, alt_functions))):
            self.play(Transform(*pair), self.pi_creature.change_mode, "pondering")
        self.wait(2)
...
class AreaUnderVGraph(PlotVelocity):
    ...
    def show_rects(self):
        ...
        rects = self.get_riemann_rectangles(flat_graph, x_min=0, x_max=8, dx=1.0)
        ...
        for new_rects in rect_list:
            ...
            self.play(Transform(rects, new_rects, run_time=2, lag_ratio=0.5))
        self.wait()
...
class ConstantVelocityPlot(PlotVelocity):
    ...
    def draw_graph(self):
        graph = self.get_graph(lambda t: 10, x_min=0, x_max=8, color=VELOCITY_COLOR)
    ...
    def show_product(self):
        rect = Rectangle(stroke_width=0, fill_color=DISTANCE_COLOR, fill_opacity=0.5)
        rect.replace(VGroup(self.graph, VectorizedPoint(self.graph_origin)), stretch=True)
        ...
        t_label = top_brace.get_text("8 seconds")
        ...
        self.play(FadeIn(rect), Write(s_label), Animation(self.graph))
        self.wait(2)
    ...
    def note_units(self):
        x_line, y_line = lines = VGroup(*[axis.copy() for axis in (self.x_axis, self.y_axis)])
        ...
        square.replace(
            VGroup(*[VectorizedPoint(self.coords_to_point(i, i)) for i in (0, 1)]),
            stretch=True,
        )
        units_of_area = VGroup(
            *[square.copy().move_to(self.coords_to_point(x, y), DOWN + LEFT) for x in range(8) for y in range(10)]
        )

        self.play(ShowCreation(x_line))
        self.play(Indicate(self.x_axis_label_mob))
        self.play(FadeOut(x_line))
        self.play(ShowCreation(y_line), self.randy.look_at, self.y_axis_label_mob)
        self.play(Indicate(self.y_axis_label_mob))
        self.play(FadeOut(y_line))

        for FadeClass in FadeIn, FadeOut:
            self.play(
                FadeClass(units_of_area, lag_ratio=0.5, run_time=3),
...
class PiecewiseConstantCar(Scene):
    def construct(self):
        ...
        for shift in 2, 6, 12:
            ...
            anim = MoveCar(car, start_point + shift * RIGHT, rate_func=linear)
...
class PiecewiseConstantPlot(PlotVelocity):
    ...
    def always_changing(self):
        ...
        def update_group(group, alpha):
            ...
            prop = interpolate(self.min_graph_proportion, self.max_graph_proportion, alpha)
            ...
            arrow.shift(graph_point + MED_SMALL_BUFF * vect - arrow.get_end())
            words.next_to(arrow.get_start(), UP)
            return group

        update_group(group, 0)

        self.play(Write(words), ShowCreation(arrow), DrawBorderThenFill(dot), run_time=1)
        self.play(UpdateFromAlphaFunc(group, update_group, rate_func=there_and_back, run_time=5))
        self.wait()
        self.play(FadeOut(group))

    def show_piecewise_constant_graph(self):
        pw_constant_graph = self.get_pw_constant_graph()
        alt_lines = [line.copy().set_color(YELLOW) for line in pw_constant_graph[:4]]
        ...
        for new_line in alt_lines[1:]:
            for mob in line.end_dot, new_line.start_dot, new_line:
                self.play(Transform(line, mob, run_time=1.0 / 3))
            self.remove(line)
            self.add(new_line)
            self.wait(2)
            line = new_line
    ...
    def compute_distance_on_each_interval(self):
        rect_list = self.get_riemann_rectangles_list(
            self.v_graph,
            self.num_riemann_approximations,
            max_dx=1,
            x_min=0,
            x_max=8,
        )
        ...
        flat_rects = self.get_riemann_rectangles(self.get_graph(lambda t: 0), x_min=0, x_max=8, dx=1)
        rects = rect_list[0]
        rect = rects[1]
    ...
    def approximate_original_curve(self):
        ...
        self.play(
            ...
            *[m.restore for m in (self.v_graph, self.v_graph_label)] + [Animation(self.rects)],
        )
    ...
    def revert_to_specific_approximation(self):
        ...
        example_text = OldTexText("For example, \\\\", "$dt$", "$=0.25$")
        example_text.to_corner(UP + RIGHT)
        example_text.set_color_by_tex("dt", YELLOW)

        self.play(ReplacementTransform(rects, target_rects, run_time=2, lag_ratio=0.5))
        rects.restore()
        self.wait()
        self.play(ShowCreation(ticks), FadeOut(self.x_axis.numbers))
        self.play(GrowFromCenter(brace), Write(dt_label))
        self.wait()
        self.play(
            FadeIn(
                example_text,
            ...
            ReplacementTransform(dt_label.copy(), example_text.get_part_by_tex("dt")),
        )
        self.wait()
    ...
    def show_specific_rectangle(self):
        ...
        t_vals = [1, 1.25]
        t_labels = VGroup(*[OldTex("t=%s" % str(t)) for t in t_vals])
        t_labels.scale(0.7)
        ...
        for t in t_vals:
            v_line = self.get_vertical_line_to_graph(t, self.v_graph, color=YELLOW)
        ...
        self.play(rects.set_fill, None, 0.25, Animation(rect))
        self.wait()
        ...
        self.play(*list(map(FadeOut, [group[1] for group in (v_lines, h_lines, height_labels)])))
        self.play(
            v_lines[0].set_color,
            RED,
            rate_func=there_and_back,
        ...
        area = OldTexText("7$\\frac{\\text{m}}{\\text{s}}$", "$\\times$", "0.25s", "=", "1.75m")
        area.next_to(rect, RIGHT, LARGE_BUFF)
        arrow = Arrow(area.get_left(), rect.get_center(), buff=0, color=WHITE)
        area.shift(SMALL_BUFF * RIGHT)

        self.play(Write(area), ShowCreation(arrow))
        self.wait(2)
        self.play(
            *list(
                map(
    ...
    def show_v_dt_for_all_rectangles(self):
        ...
        for rect in rects_subset:
            ...
            if last_rect is not None:
                ...
                anims += [GrowFromCenter(brace), Write(v_t)]
            self.play(*anims)
            self.wait()
    ...
    def write_integral_symbol(self):
        integral = OldTex("\\int", "^8", "_0", "v(t)", "\\,dt")
        ...
        for bound, num in zip(bounds, [0, 8]):
            ...
            self.play(bound_copy.scale, 1.5, bound_copy.next_to, point, DOWN, MED_LARGE_BUFF)
    ...
    def roles_of_dt(self):
        ...
        self.play(
            ReplacementTransform(dt_copy.copy(), int_dt, run_time=2),
            morty.look_at,
            int_dt,
        )
        self.wait(2)
        ...
        self.play(
            *[
                ...
                for tick, alpha in zip(self.ticks, np.linspace(0, 0.8, len(self.ticks)))
            ]
        )
        self.wait()
        ...
        self.play(
            ...
            None,
            0.75,
            *list(map(FadeOut, [dt_copy, self.v_t, self.v_t_brace])),
        )
        rects.align_family(next_rects)
        for every_other_rect in rects[::2]:
            every_other_rect.set_fill(opacity=0)
        self.play(
            self.dt_brace.stretch,
            0.5,
            0,
            self.dt_brace.move_to,
            self.dt_brace,
            LEFT,
            ReplacementTransform(rects, next_rects, run_time=2, lag_ratio=0.5),
            Transform(
                self.ticks,
                self.get_ticks(next_rects),
                run_time=2,
                lag_ratio=0.5,
            ),
        )
        ...
        self.play(
            *[
                ...
                for rect, alpha in zip(rects, np.linspace(0, 0.8, len(rects)))
            ]
            + [
                morty.change_mode,
                "thinking",
    ...
    def what_does_sum_approach(self):
        ...
        distance_words = OldTexText("Area", "= Distance traveled")
        distance_words.next_to(rects, UP)
        arrow = Arrow(distance_words[0].get_bottom(), rects.get_center(), color=WHITE)

        self.play(PiCreatureSays(morty, "Why not $\\Sigma$?", target_mode="sassy"))
        self.play(Blink(morty))
        self.wait()
        ...
        for new_rects in self.rect_list[4:]:
            ...
            for every_other_rect in rects[::2]:
                every_other_rect.set_fill(opacity=0)
            self.play(
                Transform(rects, new_rects, run_time=2, lag_ratio=0.5),
                morty.look_at,
                rects,
            )
            self.wait()
    ...
    def label_integral(self):
        ...
        words.to_edge(UP)
        arrow = Arrow(words.get_right(), self.integral.get_left())
        ...
        self.play(
            *[
                ...
                for rect, alpha in zip(self.rects, np.linspace(0, 0.8, len(self.rects)))
            ]
            + [
                Animation(self.area_arrow),
                self.morty.change_mode,
                "happy",
                self.morty.look_at,
                self.rects,
            ]
    ...
    def get_pw_constant_graph(self):
        ...
        for left_x in range(8):
            ...
            line = Line(*[self.coords_to_point(x, y) for x in xs])
    ...
    def get_ticks(self, rects):
        ticks = VGroup(
            *[
                Line(point + self.tick_size * UP / 2, point + self.tick_size * DOWN / 2)
...
class DontKnowHowToHandleNonConstant(TeacherStudentsScene):
    def construct(self):
        self.play(*[ApplyMethod(pi.change, "maybe", UP) for pi in self.get_pi_creatures()])
        self.wait(3)
...
class CarJourneyApproximation(Scene):
    ...
    def construct(self):
        ...
        self.play(
            *[
                MoveCar(car, point + 10 * RIGHT, run_time=5, rate_func=rf)
                for car, point, rf in zip(
                    cars,
                    points,
                    [s_rate_func, self.get_approximated_rate_func(self.n_jumps)],
                )
            ]
        )
        self.wait()
    ...
    def get_approximated_rate_func(self, n):
        ...
        max_integral, err = scipy.integrate.quad(v_rate_func, 0, 1)
...
class TellMeThatsNotSurprising(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Tell me that's \\\\ not surprising!", target_mode="hooray", run_time=1)
        self.wait(3)


class HowDoesThisHelp(TeacherStudentsScene):
    def construct(self):
        self.student_says("How does this help\\textinterrobang", target_mode="angry", run_time=1)
        ...
        self.wait(2)
        self.teacher_says("You're right.", target_mode="shruggie", run_time=1)
        self.play_student_changes(*["sassy"] * 3)
        self.wait(2)


class AreaUnderACurve(GraphScene):
    CONFIG = {"y_max": 4, "y_min": 0, "num_iterations": 7}

    def construct(self):
        self.setup_axes()
        graph = self.get_graph(self.func)
        rect_list = self.get_riemann_rectangles_list(graph, self.num_iterations)
...
class AreaIsDerivative(PlotVelocity, ReconfigurableScene):
    ...
    def introduce_variable_area(self):
        ...
        self.add_T_label(6)
        self.change_area_bounds(new_t_max=4, rate_func=there_and_back, run_time=2)
        self.wait()
    ...
    def write_integral(self):
        ...
        int_arrow, s_arrow = [Arrow(mob.get_left(), self.area.get_center(), color=WHITE) for mob in (integral, s_T)]
        ...
        self.play(ReplacementTransform(top_T.copy(), moving_T))
        self.change_area_bounds(new_t_max=3, rate_func=there_and_back, run_time=3)
        self.wait()
        self.play(Write(distance_word, run_time=2))
        self.play(ReplacementTransform(int_arrow, s_arrow), FadeIn(s_T))
        self.wait()
    ...
    def nudge_input(self):
        ...
        self.play(FadeOut(self.T_label_group), FadeIn(dT_label_group))
        self.wait()
        self.play(Write(ds_label))
        self.play(ShowCreation(ds_arrow))
        self.wait(2)
        self.play(GrowFromCenter(v_brace))
        self.play(ReplacementTransform(self.v_graph_label.get_part_by_tex("v").copy(), v_T_label, run_time=2))
        self.wait()
    ...
    def show_rectangle_approximation(self):
        ...
        self.play(DrawBorderThenFill(self.rect), Animation(self.ds_arrow))
        self.wait()
        self.play(
            *[
                ReplacementTransform(mob, formula1.get_part_by_tex(tex), run_time=2)
                for mob, tex in [
                    (self.ds_label, "ds"),
                    (self.ds_arrow, "="),
    ...
    def change_area_bounds(self, new_t_min=None, new_t_max=None, **kwargs):
        ...
        def update_group(group, alpha):
            ...
            new_v_line = self.get_vertical_line_to_graph(t_max, self.v_graph)
...
class FindAntiderivative(Antiderivative):
    ...
    def introduce(self):
        ...
        self.play(
            *[
                ReplacementTransform(rhs[i], expanded_rhs[j], run_time=2, path_arc=np.pi)
    ...
    def first_part(self):
        ...
        self.play(self.pi_creature.change, "plain", self.v_part1)
        self.play(ApplyWave(self.q_marks, direction=UP, amplitude=SMALL_BUFF))
        self.wait(2)
        self.play(
        ...
        self.play(
            *[
                ReplacementTransform(t_squared[i].copy(), two_t[1 - i], run_time=2, path_arc=-np.pi / 6.0)
                for i in (0, 1)
            ]
        )
    ...
    def second_part(self):
        ...
        part1_group = VGroup(self.s_part1, self.v_part1, self.arcs_copy, self.words_copy)

        neg_third_t_cubed, three_t_squared = self.get_functions("- \\frac{1}{3} t^3", "3t^2")
        ...
        self.play(
            part1_group.scale,
            0.5,
            part1_group.to_corner,
            UP + LEFT,
            self.pi_creature.change_mode,
            "plain",
        )
        self.play(self.v_part2.restore, self.v_part2.shift, LEFT)
        self.play(FadeIn(self.q_marks))
        self.wait()

        self.play(FadeOut(self.q_marks), FadeIn(t_cubed), self.v_part2.shift, DOWN + RIGHT)
        self.play(
            *[
                ReplacementTransform(
                    t_cubed[i].copy(),
        ...
        self.wait(2)
        self.play(Write(neg))
        self.play(FadeOut(t_squared), self.v_part2.shift, UP + LEFT)
        self.wait(2)
    ...
    def add_plus_C(self):
        ...
        plus_Cs = [OldTex("+%d" % d) for d in range(1, 8)]
...
class GraphSPlusC(GraphDistanceVsTime):
    ...
    def construct(self):
        ...
        tangent = self.get_secant_slope_group(6, graph, dx=0.01).secant_line
        v_line = self.get_vertical_line_to_graph(6, graph, line_class=DashedLine)
        v_line.scale(2)
        v_line.set_color(WHITE)
        graph_label, plus_C = full_label = OldTex("s(t) = 4t^2 - \\frac{1}{3}t^3", "+C")
        plus_C.set_color(YELLOW)
        ...
        self.wait()
        self.play(graph.shift, UP, run_time=2, rate_func=there_and_back)
...
class LowerBound(AreaIsDerivative):
    CONFIG = {"graph_origin": 2.5 * DOWN + 6 * LEFT}
    ...
    def drag_right_endpoint_to_zero(self):
        ...
        zero_int_bounds = list(reversed(zero_integral.get_parts_by_tex("0")))
        ...
        self.play(ReplacementTransform(self.zero_label.copy(), equals_zero))
        self.play(Transform(self.integral, zero_integral))
        self.wait(2)
        for bound in zero_int_bounds:
            self.play(ShowCreation(bound.circle))
            self.play(FadeOut(bound.circle))
        self.play(*[ReplacementTransform(bound.copy(), VGroup(equals_zero[1])) for bound in zero_int_bounds])
        self.wait(2)
        self.change_area_bounds(0, 5)
        self.play(self.integral.restore, FadeOut(equals_zero))
    ...
    def write_antiderivative_difference(self):
        ...
        value_at_eight = at_eight.brace.get_text("%.2f" % s_func(8))
        ...
        self.play(GrowFromCenter(at_T.brace), Write(antideriv_text, run_time=2))
        self.change_area_bounds(0, 5.5, rate_func=there_and_back)
        self.wait()
        self.play(ReplacementTransform(at_T.copy(), at_zero), Write(minus))
        self.wait()
        self.play(
        ...
        self.play(GrowFromCenter(at_eight.brace), Write(value_at_eight))
        self.wait(2)
        self.play(
            *list(
                map(
    ...
    def show_alternate_antiderivative_difference(self):
        ...
        numbers = [OldTex("%d" % d).next_to(self.coords_to_point(d, 0), DOWN, MED_LARGE_BUFF) for d in (1, 7)]
        ...
        self.play(self.T_label_group[0].set_fill, None, 0, *list(map(FadeIn, numbers)))
        self.play(
    ...
    def add_constant_to_antiderivative(self):
        ...
        group = VGroup(plus_fives[0], antideriv_diff[2].copy(), plus_fives[1])

        self.play(Write(plus_fives, run_time=2))
        self.wait(2)
        self.play(group.arrange, group.next_to, antideriv_diff, DOWN, MED_LARGE_BUFF)
        self.wait()
    ...
    def get_integral(self, lower_bound, upper_bound):
        result = OldTex("\\int", "^" + upper_bound, "_" + lower_bound, "t(8-t)", "\\,dt")
    ...
    def get_antiderivative_difference(self, lower_bound, upper_bound):
        ...
        parts = []
        for s in strings:
            part = OldTex("\\left(", "4", s, "^2", "-", "\\frac{1}{3}", s, "^3\\right))")
...
class FundamentalTheorem(GraphScene):
    ...
    def add_graph_and_integral(self):
        ...
        for bound, tex in (self.lower_bound, "a"), (self.upper_bound, "b"):
            ...
            v_line = self.get_vertical_line_to_graph(bound, graph, color=label.get_color())
    ...
    def show_f_dx_sum(self):
        ...
        self.rect_list = self.get_riemann_rectangles_list(self.graph, self.n_riemann_iterations, **kwargs)
        ...
        flat_rects = self.get_riemann_rectangles(self.get_graph(lambda x: 0), dx=0.5, **kwargs)
        ...
        self.play(*[ApplyMethod(rect.set_fill, None, 1 if rect is start_rect else low_opacity) for rect in rects])
        self.play(
            *it.chain(
        ...
        for i in range(start_rect_index + 1, last_rect_index):
            self.play(
                ...
                DOWN,
                0,
                *[MaintainPositionRelativeTo(brace.label, brace) for brace in (f_brace, dx_brace)],
            )
        self.wait()
        self.play(
            *it.chain(
                list(map(FadeOut, [f_brace, dx_brace, f_brace.label, dx_brace.label])),
    ...
    def show_rects_approaching_area(self):
        for new_rects in self.rect_list:
            self.transform_between_riemann_rects(self.rects, new_rects)

    def write_antiderivative(self):
        deriv = OldTex("{d", "F", "\\over\\,", "dx}", "(x)", "=", "f(x)")
        ...
        self.play(
            *it.chain(
                [ReplacementTransform(deriv_F.copy(), part) for part in rhs.get_parts_by_tex("F")],
                [Write(VGroup(*rhs.get_parts_by_tex(tex))) for tex in "=()-"],
            )
        )
        for tex in "b", "a":
            self.play(ReplacementTransform(self.integral.get_part_by_tex(tex).copy(), rhs.get_part_by_tex(tex)))
            self.wait()
        self.wait(2)
    ...
    def show_integral_considering_continuum(self):
        self.play(*[ApplyMethod(mob.set_fill, None, 0.2) for mob in (self.deriv, self.rhs)])
        self.play(self.rects.restore, run_time=3, rate_func=there_and_back)
        self.wait()
        for x in range(2):
            self.play(
                *[
                    ApplyFunction(
                        ...
                        rect,
                        run_time=3,
                        rate_func=squish_rate_func(there_and_back, alpha, alpha + 0.2),
                    )
                    for rect, alpha in zip(self.rects, np.linspace(0, 0.8, len(self.rects)))
                ]
            )
...
class NegativeArea(GraphScene):
    ...
    def show_negative_point(self):
        v_line = self.get_vertical_line_to_graph(self.sample_input, self.graph, color=RED)
        self.play(ShowCreation(v_line))
        self.wait()
        self.v_line = v_line
    ...
    def show_car_going_backwards(self):
        ...
        self.play(ShowCreation(arrow))
        self.play(MoveCar(car, end_point, moving_forward=False, run_time=3))
        self.wait()
        ghost_car = car.copy().fade()
        right_nose_line = self.get_car_nose_line(car)
        self.play(ShowCreation(right_nose_line))
        self.add(ghost_car)
        self.play(MoveCar(car, nudged_end_point, moving_forward=False))
    ...
    def write_v_dt(self):
        ...
        self.play(GrowFromCenter(brace), Write(equation))
        self.wait()
        self.play(FadeIn(negative))
        self.play(ShowCreation(v_arrow))
        self.wait(2)
        self.play(ReplacementTransform(v_arrow.copy(), ds_arrow))
        self.wait(2)
    ...
    def show_rectangle(self):
        ...
        self.play(
            *it.chain(
                ...
                list(map(FadeOut, [dt_brace, v_brace, dt_label, v_label])),
            )
        )
        self.wait()
        for new_rects in rect_list[1:]:
