# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter10.py
...
def taylor_approximation(func, highest_term, center_point=0):
    ...
    return lambda x: sum([c * ((x - center_point) ** n) for n, c in enumerate(coefficients)])
...
class Chapter10OpeningQuote(OpeningQuote):
    CONFIG = {
        "quote": [
            ...
            "; a ",
            "theorem",
            " is a statement about a collection of ",
            "examples",
            " and the purpose of proving ",
            "theorems",
            " is to classify and explain the ",
            "examples",
            ".",
        ],
        "quote_arg_separator": "",
...
class ExampleApproximation(GraphScene):
    ...
    def construct(self):
        ...
        approx_graphs = [
            self.get_graph(taylor_approximation(self.function, n), self.approximation_color)
            for n in self.order_sequence
        ]

        near_text = OldTexText("Near %s $= %d$" % (self.x_axis_label, self.center_point))
        near_text.to_corner(UP + RIGHT)
        near_text.add_background_rectangle()
        equation = OldTex(self.function_tex, "\\approx", *self.approximation_terms)
        equation.next_to(near_text, DOWN, MED_LARGE_BUFF)
        equation.to_edge(RIGHT)
        near_text.next_to(equation, UP, MED_LARGE_BUFF)
        equation.set_color_by_tex(self.function_tex, self.function_color, substring=False)
        approx_terms = VGroup(*[equation.get_part_by_tex(tex, substring=False) for tex in self.approximation_terms])
...
class Pendulum(ReconfigurableScene):
    CONFIG = {
        ...
        "weight_radius": 0.2,
        "angle": np.pi / 6,
        "approx_tex": ["\\approx 1 - ", "{\\theta", "^2", "\\over", "2}"],
    ...
    def show_oscillation(self):
        ...
        self.play(
            ShowCreation(trajectory_dots, rate_func=linear, run_time=kwargs["run_time"]),
    ...
    def show_height(self):
        ...
        to_write = VGroup(*[part for part in height_tex if part not in [height_tex_R, height_tex_theta]])

        self.play(ShowCreation(h_line), GrowFromCenter(height_brace))
        self.play(
            ShowCreation(v_line),
            ShowCreation(arc),
            Write(theta),
        ...
        self.play(
            Write(to_write),
            ReplacementTransform(radius_brace[-1].copy(), height_tex_R),
            ReplacementTransform(theta.copy(), height_tex_theta),
            run_time=2,
        )
        self.wait(2)
        ...
        self.cosine = VGroup(*[height_tex.get_part_by_tex(tex) for tex in ("cos", "theta", ")")])
        self.one_minus = VGroup(*[height_tex.get_part_by_tex(tex) for tex in ("\\big(1-", "\\big)")])
    ...
    def substitute_approximation(self):
        ...
        cosine.target.next_to(cosine_approx, LEFT, align_using_submobjects=True)
        kwargs = self.get_swing_kwargs()

        self.play(
            FadeIn(cosine_approx, run_time=2, lag_ratio=0.5),
            MoveToTarget(cosine),
            morty.change,
            "pondering",
            cosine_approx,
    ...
    def show_confusion(self):
        ...
        self.play(randy.change, "confused", self.cosine)
    ...
    def get_pendulum(self):
        line = Line(self.anchor_point, self.anchor_point + self.radius * DOWN, color=WHITE)
        weight = Circle(
        ...
        result.rotate(self.angle, about_point=self.anchor_point)
        result.line = line
    ...
    def get_trajectory_dots(self, n_dots=40, color=YELLOW):
        arc_angle = np.pi / 6
        proportions = self.swing_rate_func(np.linspace(0, 1, n_dots))
    ...
    def get_v_line(self):
        return DashedLine(self.anchor_point, self.anchor_point + self.radius * DOWN, color=WHITE)
    ...
    def get_height_brace_tex(self, brace):
        tex_mob = OldTex("R", "\\big(1-", "\\cos(", "\\theta", ")", "\\big)")
        tex_mob.set_color_by_tex("theta", YELLOW)
        tex_mob.next_to(brace, RIGHT)
        return tex_mob

    def get_arc_and_theta(self):
        arc = Arc(start_angle=-np.pi / 2, angle=self.angle, color=YELLOW)
        theta = OldTex("\\theta")
        theta.set_color(YELLOW)
        theta.next_to(arc.point_from_proportion(0.5), DOWN, SMALL_BUFF)
...
class ExampleApproximationWithCos(ExampleApproximationWithSine):
    ...
    def construct(self):
        ...
        v_lines, alt_v_lines = [
            VGroup(
                *[
                    self.get_vertical_line_to_graph(u * dx, high_graph, line_class=DashedLine, color=YELLOW)
                    for u in (-1, 1)
                ]
            )
            for dx in (0.01, 0.7)
...
class ConstructQuadraticApproximation(ExampleApproximationWithCos):
    ...
    def add_cosine_graph(self):
        ...
        self.play(ShowCreation(cosine_graph), UpdateFromFunc(dot, update_dot), run_time=4)
    ...
    def add_quadratic_graph(self):
        ...
        self.play(ReplacementTransform(self.cosine_graph.copy(), quadratic_graph, run_time=3))
    ...
    def introduce_quadratic_constants(self):
        ...
        self.play(FadeIn(quadratic_tex, run_time=3, lag_ratio=0.5))
        self.play(FadeIn(free_to_change), *list(map(ShowCreation, arrows)))
        self.play(
            *[
                ApplyMethod(
                    const.scale,
        ...
        for alt_consts in alt_consts_list:
            self.change_quadratic_graph(self.quadratic_graph, *alt_consts)
            self.wait()
    ...
    def show_value_at_zero(self):
        ...
        ax0_group.next_to(self.cosine_label, RIGHT, align_using_submobjects=True)
        one = OldTex("1")
        ...
        v_line = self.get_vertical_line_to_graph(0, self.cosine_graph, line_class=DashedLine, color=YELLOW)

        self.play(ShowCreation(v_line))
        self.play(ShowCreation(arrow), Write(x_equals_0, run_time=2))
    ...
    def set_c0_to_one(self):
        ...
        self.play(ReplacementTransform(self.quadratic_tex.copy(), poly_at_zero))
        self.wait(2)
        self.play(FadeIn(equals_c0))
        self.wait(2)
        self.play(Transform(poly_group, poly_group_target, run_time=2, lag_ratio=0.5))
        self.wait(2)
        self.play(*list(map(FadeOut, [poly_at_zero, equals_c0])))

        self.free_to_change_group.remove(self.free_to_change_group.arrows[0])
    ...
    def show_tangent_slope(self):
        graph_point_at_zero = self.input_to_graph_point(0, self.cosine_graph)
        tangent_line = self.get_tangent_line(0, self.cosine_graph)

        self.play(ShowCreation(tangent_line))
        self.change_quadratic_graph(self.quadratic_graph, 1, 0, -0.1)
        self.wait()
        self.change_quadratic_graph(self.quadratic_graph, 1, 1, -0.1)
        self.wait(2)
        self.change_quadratic_graph(self.quadratic_graph, 1, 0, -0.1)
        self.wait(2)
    ...
    def compute_cosine_derivative(self):
        ...
        self.play(FadeIn(VGroup(derivative, *rhs[:2]), run_time=2, lag_ratio=0.5))
        self.wait(2)
    ...
    def compute_polynomial_derivative(self):
        ...
        derivative_at_zero = self.get_quadratic_derivative("c_1", "c_2", arg="0")
        equals_c1 = OldTex("=", "c_1", "+0")
        equals_c1.next_to(
            derivative_at_zero.get_part_by_tex("="),
            DOWN,
            buff=MED_LARGE_BUFF,
            aligned_edge=LEFT,
        )
        equals_c1.set_color_by_tex("c_1", self.colors[1])
        poly_group = VGroup(equals_c1, derivative, self.quadratic_tex)
        poly_group_target = VGroup(
            OldTex("=", "0", "+0").set_color_by_tex("0", self.colors[1], substring=False),
        ...
        self.play(FadeOut(self.free_to_change_group))
        self.play(FadeIn(derivative, run_time=3, lag_ratio=0.5))
        self.wait()
        self.play(Transform(derivative, derivative_at_zero, run_time=2, lag_ratio=0.5))
        self.wait(2)
        self.play(Write(equals_c1))
        self.wait(2)
        self.play(Transform(poly_group, poly_group_target, run_time=3, lag_ratio=0.5))
        self.wait(2)

        self.play(*list(map(FadeOut, poly_group[:-1])))
        self.free_to_change_group.remove(self.free_to_change_group.arrows[1])
        self.play(FadeIn(self.free_to_change_group))

    def let_c2_vary(self):
        alt_c2_values = [-1, -0.05, 1, -0.2]
        for alt_c2 in alt_c2_values:
            self.change_quadratic_graph(self.quadratic_graph, 1, 0, alt_c2)
            self.wait()

    def point_out_negative_concavity(self):
        partial_cosine_graph = self.get_graph(np.cos, x_min=-1, x_max=1, color=PINK)

        self.play(ShowCreation(partial_cosine_graph, run_time=2))
        self.wait()
        for x, run_time in (-1, 2), (1, 4):
            self.play(self.get_tangent_line_change_anim(self.tangent_line, x, self.cosine_graph, run_time=run_time))
            self.wait()
        self.play(*list(map(FadeOut, [partial_cosine_graph, self.tangent_line])))

    def compute_cosine_second_derivative(self):
        second_deriv, rhs = self.get_cosine_second_derivative()

        self.play(FadeIn(VGroup(second_deriv, *rhs[1][:2]), run_time=2, lag_ratio=0.5))
        self.wait(3)
    ...
    def show_matching_tangent_lines(self):
        ...
        tangent_lines = [self.get_tangent_line(0, graph, color=color) for graph, color in zip(graphs, [WHITE, YELLOW])]
    ...
    def compute_polynomial_second_derivative(self):
        ...
        derivs = [self.get_quadratic_derivative("0", c2) for c2 in c2s]
        second_derivs = [OldTex("{d^2 P \\over dx^2}", "(x)", "=", "2", c2) for c2 in c2s]
        for deriv, second_deriv in zip(derivs, second_derivs):
            second_deriv[0].scale(0.7, about_point=second_deriv[0].get_right())
            second_deriv[-1].set_color(self.colors[-1])
            second_deriv.next_to(deriv, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)

        poly_group = VGroup(second_derivs[0], derivs[0], self.quadratic_tex)
        poly_group_target = VGroup(second_derivs[1], derivs[1], self.get_quadratic_tex("1", "0", c2s[1]))
    ...
    def box_final_answer(self):
        ...
        box.stretch_to_fit_width(self.quadratic_tex.get_width() + MED_LARGE_BUFF)
        box.stretch_to_fit_height(self.quadratic_tex.get_height() + MED_LARGE_BUFF)
    ...
    def get_quadratic_graph(self, c0=1, c1=0, c2=-0.5):
        return self.get_graph(lambda x: c0 + c1 * x + c2 * x**2, color=self.colors[2])

    def get_quadratic_tex(self, c0, c1, c2, arg="x"):
        tex_mob = OldTex("P(", arg, ")", "=", c0, "+", c1, arg, "+", c2, arg, "^2")
    ...
    def get_quadratic_derivative(self, c1, c2, arg="x"):
        result = OldTex("{dP \\over dx}", "(", arg, ")", "=", c1, "+", "2", c2, arg)
        ...
        if hasattr(self, "quadratic_tex"):
            result.next_to(self.quadratic_tex, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        return result
    ...
    def get_tangent_line_change_anim(self, tangent_line, new_x, graph, **kwargs):
        start_x = self.x_axis.point_to_number(tangent_line.get_center())

        def update(tangent_line, alpha):
            x = interpolate(start_x, new_x, alpha)
            new_line = self.get_tangent_line(x, graph, color=tangent_line.get_color())
    ...
    def get_cosine_derivative(self):
        ...
        derivative.next_to(self.cosine_label, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rhs = OldTex("=", "-\\sin(0)", "=", "0")
        rhs.set_color_by_tex("\\sin", self.colors[1])
        rhs.scale(0.75)
        rhs.next_to(derivative, RIGHT, align_using_submobjects=True)
    ...
    def get_cosine_second_derivative(self):
        ...
        second_deriv.next_to(self.cosine_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rhs = OldTex("=", "-\\cos(0)", "=", "-1")
        rhs.set_color_by_tex("cos", self.colors[2])
        rhs.scale(0.8)
        rhs.next_to(second_deriv, RIGHT, align_using_submobjects=True)
...
class ReflectOnQuadraticApproximation(TeacherStudentsScene):
    ...
    def show_example_approximation(self):
        approx_at_x, approx_at_point = [
            OldTex("\\cos(", s, ")", "\\approx", "1 - \\frac{1}{2}", "(", s, ")", "^2").next_to(
                self.get_students(), UP, 2
            )
            for s in (
                "x",
                "0.1",
        ...
        real_result = OldTex("\\cos(", "0.1", ")", "=", "%.7f\\dots" % np.cos(0.1))
        real_result.shift(approx_rhs.get_part_by_tex("=").get_center() - real_result.get_part_by_tex("=").get_center())
        for mob in approx_at_point, real_result:
            mob.set_color_by_tex("0.1", YELLOW)
        real_result.set_fill(opacity=0)

        self.play(Write(approx_at_x, run_time=2), self.teacher.change_mode, "raise_right_hand")
        self.wait(2)
        self.play(
        ...
        self.play_student_changes(
            *["plain"] * 3,
            added_anims=list(map(FadeOut, [approx_at_point, approx_rhs, real_result])),
            look_at=approx_at_x,
        )
    ...
    def add_polynomial(self):
        ...
        self.play(Write(polynomial), self.teacher.change, "pondering")
        self.wait(2)
        self.play(
            *[
                ApplyMethod(
    ...
    def show_c1(self):
        ...
        c1.save_state()
        equation = OldTex("\\frac{dP}{dx}(0) = \\frac{d(\\cos)}{dx}(0)")
        equation.to_corner(UP + RIGHT)
    ...
    def show_c2(self):
        ...
        c2.save_state()
        equation = OldTex("\\frac{d^2 P}{dx^2}(0) = \\frac{d^2(\\cos)}{dx^2}(0)")
        equation.to_corner(UP + RIGHT)
        alt_c2_tex = "\\text{\\tiny $\\left(-\\frac{1}{2}\\right)$}"
        new_polynomial = self.get_polynomial(c0="1", c1="0", c2=alt_c2_tex)
    ...
    def get_polynomial(self, c0="c_0", c1="c_1", c2="c_2"):
        polynomial = OldTex("P(x) = ", c0, "+", c1, "x", "+", c2, "x^2")
...
class ReflectionOnQuadraticSupplement(ConstructQuadraticApproximation):
    def construct(self):
        ...
        self.wait()
        for c0 in 0, 2, 1:
            self.change_quadratic_graph(quadratic_graph, c0=c0)
        self.wait(2)
        for c1 in 1, -1, 0:
            self.change_quadratic_graph(quadratic_graph, c1=c1)
        self.wait(2)
        for c2 in -0.1, -1, -0.5:
            self.change_quadratic_graph(quadratic_graph, c2=c2)
        self.wait(2)
...
class SimilarityOfChangeBehavior(ConstructQuadraticApproximation):
    def construct(self):
        ...
        dot_anims = [UpdateFromAlphaFunc(dot, update_dot, run_time=3) for dot in dots]
        tangent_lines = VGroup(*[self.get_tangent_line(0, graph, color) for graph, color in zip(graphs, colors)])
        ...
        self.play(*dot_anims)
        self.play(FadeIn(tangent_lines), FadeOut(dots))
...
class CubicAndQuarticApproximations(ConstructQuadraticApproximation):
    ...
    def add_background(self):
        self.setup_axes()
        self.cosine_graph = self.get_graph(np.cos, color=self.colors[0])
        ...
        self.add(self.cosine_graph, self.quadratic_graph, self.big_rect)
        ...
        self.polynomial = OldTex("P(x)=", "1", "-\\frac{1}{2}", "x^2")
        ...
        self.polynomial.quadratic_part = VGroup(*self.polynomial[1:])
    ...
    def take_third_derivative_of_cubic(self):
        ...
        self.play(MoveToTarget(polynomial), GrowFromCenter(plus_cubic_term))
        self.wait()

        brace = Brace(polynomial.quadratic_part, DOWN)
        third_derivative = OldTex("\\frac{d^3 P}{dx^3}(x) = ", "0")
        third_derivative.shift(
            brace.get_bottom() + MED_SMALL_BUFF * DOWN - third_derivative.get_part_by_tex("0").get_top()
        )

        self.play(Write(third_derivative[0]))
        self.play(GrowFromCenter(brace))
        self.play(ReplacementTransform(polynomial.quadratic_part.copy(), VGroup(third_derivative[1])))
        self.wait(2)
        self.play(plus_cubic_copy.next_to, third_derivative, RIGHT)
        derivative_term = self.take_derivatives_of_monomial(VGroup(*plus_cubic_copy[1:]))
    ...
    def show_third_derivative_of_cosine(self):
        ...
        self.play(dot.move_to, cosine_third_derivative, dot.set_fill, None, 0)
        self.play(ReplacementTransform(self.cosine_second_derivative.copy(), cosine_third_derivative))
        self.wait(2)
        dot.set_fill(opacity=0.5)
        self.play(
            dot.move_to,
            self.polynomial_third_derivative.get_right(),
            dot.set_fill,
            None,
            0,
        )
    ...
    def set_c3_to_zero(self):
        ...
        zeros = VGroup(*[OldTex("0").move_to(c3) for c3 in c3s])
        zeros.set_color(self.colors[3])
        zeros.shift(SMALL_BUFF * UP)
        zeros[0].shift(0.25 * SMALL_BUFF * (UP + LEFT))

        self.play(Transform(c3s, zeros, run_time=2, lag_ratio=0.5))
        self.wait(2)
    ...
    def show_fourth_derivative_of_cosine(self):
        ...
        self.play(ReplacementTransform(self.cosine_third_derivative.copy(), cosine_fourth_derivative))
        self.wait(3)
    ...
    def take_fourth_derivative_of_quartic(self):
        ...
        fourth_deriv_lhs.next_to(self.polynomial, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        alt_rhs = OldTex("=", "24 \\cdot", "c_4")
        alt_rhs.next_to(
            fourth_deriv_lhs.get_part_by_tex("="),
            DOWN,
            buff=LARGE_BUFF,
            aligned_edge=LEFT,
        )
        ...
        self.play(quartic_term.next_to, fourth_deriv_lhs, RIGHT)
        self.wait()
    ...
    def solve_for_c4(self):
        ...
        fractions = VGroup(*[fraction.copy().move_to(c4, LEFT) for c4 in c4s])
    ...
    def take_derivatives_of_monomial(self, term, *added_anims):
        ...
        for k in range(n, 0, -1):
            ...
            curr_term.target.shift((front_num.get_width() + SMALL_BUFF) * RIGHT)
            ...
            self.play(
                ApplyMethod(
                    exponent_copy.replace,
                    front_num[0],
                    path_arc=np.pi,
                ),
                Write(front_num[1], rate_func=squish_rate_func(smooth, 0.5, 1)),
    ...
    def get_cosine_third_derivative(self):
        ...
        third_deriv.next_to(self.cosine_second_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rhs = OldTex("=", "\\sin(0)", "=", "0")
        rhs.set_color_by_tex("sin", self.colors[3])
        rhs.scale(0.8)
        rhs.next_to(third_deriv, RIGHT, align_using_submobjects=True)
        rhs.add_background_rectangle()
    ...
    def get_cosine_fourth_derivative(self):
        ...
        fourth_deriv.next_to(self.cosine_third_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        rhs = OldTex("=", "\\cos(0)", "=", "1")
        rhs.set_color_by_tex("cos", self.colors[4])
        rhs.scale(0.8)
        rhs.next_to(fourth_deriv, RIGHT, align_using_submobjects=True)
        rhs.add_background_rectangle()
        rhs.background_rectangle.scale(1.2)
...
class NoticeAFewThings(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Notice a few things", target_mode="hesitant")
        self.wait(3)
...
class FactorialTerms(CubicAndQuarticApproximations):
    def construct(self):
        lhs_list = [
            OldTex("{d%s" % s, "\\over", "dx%s}" % s, "(", "c_8", "x^8", ")=")
            for i in range(9)
        ...
        added_anims = [
            ReplacementTransform(start_lhs, target_lhs, rate_func=squish_rate_func(smooth, 0, 0.5))
        ...
        words = OldTexText("Set", "$c_8$", "$ = \\frac{\\text{Desired derivative value}}{8!}")
        words.set_color_by_tex("c_8", YELLOW)
        words.shift(2 * UP)

        self.play(GrowFromCenter(brace), Write(eight_factorial))
        self.play(
            ReplacementTransform(coefficient.copy(), words.get_part_by_tex("c_8")),
            Write(words),
        )
        self.wait(2)
...
class HigherTermsDontMessUpLowerTerms(Scene):
    ...
    def add_polynomial(self):
        ...
        c0, c2, c4 = [polynomial.get_part_by_tex(tex) for tex in (c0_tex, c2_tex, c4_tex)]
        for term, color in zip([c0, c2, c4], self.colors):
            term.set_color(color)
        arrows = VGroup(
            *[
                Arrow(c4.get_top(), c.get_top(), path_arc=arc, color=c.get_color())
                for c, arc in [(c2, 0.9 * np.pi), (c0, np.pi)]
            ]
        )
        no_affect_words = OldTexText("Doesn't affect \\\\ previous terms")
        ...
        self.play(Write(no_affect_words), ShowCreation(arrows), run_time=3)
        self.wait(2)
    ...
    def show_second_derivative(self):
        ...
        second_deriv.next_to(self.polynomial, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
...
class EachTermControlsOneDerivative(Scene):
    def construct(self):
        ...
        polynomial = OldTex(
            ...
            "c_1",
            "x",
            *it.chain(*[["+", "c_%d" % n, "x^%d" % n] for n in range(2, 5)]),
        )
        ...
        deriv_words = VGroup(
            *[
                ...
                + ["\\frac{d^%d P}{dx^%d}" % (n, n) for n in range(2, 5)]
            ]
        )
        deriv_words.arrange(RIGHT, buff=LARGE_BUFF, aligned_edge=UP)
        ...
        for deriv in deriv_words:
            self.play(ShowCreation(deriv.arrow), FadeIn(deriv))
            self.wait()
...
class ApproximateNearNewPoint(CubicAndQuarticApproximations):
    ...
    def add_cosine_graph(self):
        self.cosine_graph = self.get_graph(np.cos, self.colors[0])
    ...
    def shift_approximation_center(self):
        ...
        for target, rt in zip(self.target_approx_centers, [3, 4, 4]):
            self.change_approximation_center(quartic_graph, dot, target, run_time=rt)
        self.play(ShowCreation(v_line), Animation(pi))
        self.wait()
    ...
    def show_polynomials(self):
        ...
        self.play(FadeIn(poly_around_pi, run_time=4, lag_ratio=0.5))
        self.wait(2)
        self.play(FadeIn(randy))
    ...
    def get_polynomial(self, arg, center_tex):
        result = OldTex(
            ...
            "=",
            "c_0",
            *it.chain(*[["+", "c_%d" % d, "%s^%d" % (arg, d)] for d in range(1, 5)]),
        )
    ...
    def get_quartic_approximation(self, a):
        coefficients = [derivative(np.cos, a, n) for n in range(5)]
        func = lambda x: sum([(c / math.factorial(n)) * (x - a) ** n for n, c in enumerate(coefficients)])
        return self.get_graph(func, color=GREEN)


class OnAPhilosophicalLevel(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("And on a \\\\ philosophical level", run_time=1)
        self.wait(3)
...
class TranslationOfInformation(CubicAndQuarticApproximations):
    ...
    def mention_information_exchange(self):
        deriv_info = OldTexText("Derivative \\\\ information \\\\ at a point")
        deriv_info.next_to(ORIGIN, LEFT, LARGE_BUFF)
        deriv_info.to_edge(UP)
        output_info = OldTexText("Output \\\\ information \\\\ near that point")
        output_info.next_to(ORIGIN, RIGHT, LARGE_BUFF)
        output_info.to_edge(UP)
        arrow = Arrow(deriv_info, output_info)

        center_v_line = self.get_vertical_line_to_graph(0, self.cosine_graph, line_class=DashedLine, color=YELLOW)
        outer_v_lines = VGroup(*[center_v_line.copy().shift(vect) for vect in (LEFT, RIGHT)])
        ...
        self.play(ReplacementTransform(VGroup(center_v_line).copy(), outer_v_lines))
        self.play(
            ReplacementTransform(self.cosine_graph.copy(), quadratic_graph),
            Animation(dot),
        )
        for x in -1, 1, 0:
            ...
            self.play(
                UpdateFromAlphaFunc(
                    dot,
                    lambda d, a: d.move_to(self.input_to_graph_point(interpolate(start_x, x, a), self.cosine_graph)),
                    run_time=2,
                )
            )
        self.wait()
        self.play(*list(map(FadeOut, [deriv_info, arrow, output_info, outer_v_lines])))
    ...
    def show_derivative_pattern(self):
        derivs_at_x, derivs_at_zero = [
            VGroup(*[OldTex(tex, "(", arg, ")") for tex in ["\\cos", "-\\sin", "-\\cos", "\\sin", "\\cos"]])
            for arg in ("x", "0")
        ]
        arrows = VGroup(*[Arrow(UP, ORIGIN, color=WHITE, buff=0, tip_length=MED_SMALL_BUFF) for d in derivs_at_x])
        group = VGroup(*it.chain(*list(zip(derivs_at_x, arrows))))
        ...
        derivative_values = VGroup(*[rhs[1] for rhs in rhs_group])
        for value, color in zip(derivative_values, self.colors):
            value.set_color(color)
        zeros = VGroup(*[deriv.get_part_by_tex("0") for deriv in derivs_at_zero])

        self.play(FadeIn(derivs_at_x[0]))
        self.wait()
        for start_d, arrow, target_d in zip(group[::2], group[1::2], group[2::2]):
            self.play(ReplacementTransform(start_d.copy(), target_d), ShowCreation(arrow))
            self.wait()
        self.wait()
        self.play(ReplacementTransform(derivs_at_x, derivs_at_zero))
        self.wait()
        ...
        self.cosine_derivative_group = VGroup(derivs_at_zero, arrows, group[-1], rhs_group)
    ...
    def show_polynomial(self):
        ...
        self.play(
            *[
                Transform(dv, pc, run_time=2, path_arc=np.pi / 2)
                for dv, pc, a in zip(
                    derivative_values,
        ...
        self.play(Write(polynomial, run_time=5), Animation(derivative_values))
    ...
    def name_taylor_polynomial(self):
        brace = Brace(VGroup(self.polynomial.coefficients, self.polynomial.factorials), DOWN)
        ...
        self.wait()
        self.play(
            Transform(self.quadratic_graph, quartic_graph, run_time=2),
            Animation(self.dot),
        )
        self.wait(2)
    ...
    def draw_new_function_graph(self):
        ...
        graph = self.get_graph(func, color=self.colors[0])
    ...
    def write_general_function_derivative(self):
        ...
        self.v_line.put_start_and_end_on(self.graph_origin, self.dot.get_center())
        ...
        for start, target in zip(derivs_at_x, derivs_at_x[1:]):
            self.play(ReplacementTransform(start.copy(), target))
            self.wait()
        self.wait()
        self.play(
            ReplacementTransform(
        ...
        self.play(ReplacementTransform(zeros.copy(), self.dot, run_time=2, lag_ratio=0.5))
    ...
    def replace_coefficients_in_generality(self):
        new_polynomial = self.get_polynomial("x", *[tex_mob.get_tex() for tex_mob in self.derivs_at_zero[:-1]])
        new_polynomial.to_corner(UP + RIGHT)
        polynomial_fourth_term = VGroup(*self.polynomial[-7:-1])
        self.polynomial.remove(*polynomial_fourth_term)

        self.play(
            ReplacementTransform(self.polynomial, new_polynomial, run_time=2, lag_ratio=0.5),
    ...
    def walk_through_terms(self):
        ...
        approx_graphs = [self.get_graph(taylor_approximation(func, n), color=WHITE) for n in range(7)]
        ...
        braces = [Brace(VGroup(left_mob, *right_mobs[:n]), DOWN) for n in range(len(approx_graphs))]
        brace = braces[0]
    ...
    def show_polynomial_around_a(self):
        new_polynomial = self.get_polynomial("(x-a)", *[tex_mob.get_tex() for tex_mob in self.derivs_at_a[:-2]])
        new_polynomial.to_corner(UP + RIGHT)
        new_polynomial.add_background_rectangle()

        polynomial_third_term = VGroup(*self.polynomial[1][-7:-1])
        ...
        self.play(
            UpdateFromAlphaFunc(group, get_update_function(1), run_time=2),
            Animation(self.polynomial),
            Animation(polynomial_third_term),
        )
        self.wait()
        self.play(Transform(self.derivs_at_zero, self.derivs_at_a))
        self.play(Transform(self.polynomial, new_polynomial), FadeOut(polynomial_third_term))
        self.wait()
        for x in -1, np.pi / 6:
            self.play(
                UpdateFromAlphaFunc(
                    group,
                    get_update_function(x),
                ),
                Animation(self.polynomial),
                run_time=4,
            )
...
class ThisIsAStandardFormula(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("You will see this \\\\ in your texts", run_time=1)
        self.play_student_changes(*["sad"] * 3, look_at=FRAME_Y_RADIUS * UP)
        self.wait(2)
...
class ExpPolynomial(TranslationOfInformation, ExampleApproximationWithExp):
    ...
    def add_graph(self):
        ...
        self.play(
            ShowCreation(graph),
            Write(e_to_x, rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=2,
        )
        self.wait()
    ...
    def show_derivatives(self):
        ...
        derivs_at_x, derivs_at_zero = [
            VGroup(*[OldTex("e^%s" % s).set_color(c) for c in self.colors]) for s in ("x", "0")
        ]
        ...
        for dx, d0 in zip(derivs_at_x, derivs_at_zero):
            for d in dx, d0:
                d.add_background_rectangle()
            d0.replace(dx)
        rhs_group = VGroup(*[OldTex("=", "1").scale(0.7).next_to(deriv, RIGHT) for deriv in derivs_at_zero])
        derivative_values = VGroup(*[rhs[1] for rhs in rhs_group])
        for value, color in zip(derivative_values, self.colors):
            value.set_color(color)
        ...
        for start_d, arrow, target_d in zip(group[::2], group[1::2], group[2::2]):
            self.play(ReplacementTransform(start_d.copy(), target_d), Write(arrow, run_time=1))
            self.wait()
        self.wait()
        self.play(ReplacementTransform(derivs_at_x, derivs_at_zero))
        self.wait()
    ...
    def show_polynomial(self):
        ...
        self.play(Write(polynomial, run_time=4), Animation(derivative_values))
...
class ShowSecondTerm(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says("Now for \\\\ something fun!", target_mode="hooray")
        self.wait(2)
        self.play(
...
class SecondTermIntuition(AreaIsDerivative):
    ...
    def write_derivative(self):
        ...
        deriv.next_to(self.input_to_graph_point(2.7, self.v_graph), RIGHT)
    ...
    def nudge_end_point(self):
        ...
        rect = Rectangle(stroke_width=0, fill_color=YELLOW, fill_opacity=0.75)
        rect.replace(
            VGroup(
        ...
        self.play(FadeIn(dark_area), *list(map(Animation, self.foreground_mobjects)))
        self.play(
    ...
    def point_out_triangle(self):
        ...
        triangle.replace(
            Line(self.rect.get_corner(UP + LEFT), self.right_v_line.get_top()),
            stretch=True,
        )
    ...
    def relabel_start_and_end(self):
        ...
        for label, arrow in zip(labels, arrows):
            ...
            self.wait()
        self.wait()
        self.play(ReplacementTransform(dx_label, x_minus_a))
        self.wait()
    ...
    def compute_triangle_area(self):
        ...
        base_line = Line(*[triangle.get_corner(DOWN + vect) for vect in (LEFT, RIGHT)])
        ...
        height_labels = [
            ...
            for s in ["(\\text{Slope})", "\\frac{d^2 f_{\\text{area}}}{dx^2}(a)"]
        ]
        ...
        self.play(FadeIn(base_label), ShowCreation(base_arrow), ShowCreation(base_line))
        self.wait(2)
        self.play(GrowFromCenter(height_brace), Write(height_labels[0]))
        self.wait(2)
        ...
        group.target[-1].next_to(group.target[-2], RIGHT, buff=SMALL_BUFF, align_using_submobjects=True)
        group.target[1].shift(0.02 * DOWN)
        group.target.to_corner(UP + RIGHT)
        exp_2 = OldTex("2").scale(0.8 * tex_scale_factor)
        exp_2.next_to(group.target[-2], UP + RIGHT, buff=0, align_using_submobjects=True)
        equals_half.scale(0.1)
        equals_half.move_to(triangle)
        equals_half.set_fill(opacity=0)

        self.play(FadeOut(self.deriv_label), MoveToTarget(group, run_time=2))
        self.wait(2)
        self.play(Transform(group[-1], exp_2))
        self.wait(2)

    def walk_through_taylor_terms(self):
        mini_area, mini_rect, mini_triangle = [mob.copy() for mob in (self.dark_area, self.rect, self.triangle)]
        ...
        geometric_taylor.arrange(RIGHT, buff=MED_SMALL_BUFF)
        ...
        analytic_taylor.next_to(geometric_taylor, DOWN, aligned_edge=LEFT)
        ...
        self.play(FadeIn(geometric_taylor, run_time=2, lag_ratio=0.5))
        self.wait()
        self.play(
            FadeIn(VGroup(*analytic_taylor[:3])),
            self.dark_area.set_fill,
            BLUE_E,
            1,
            Transform(self.__name__, new_func_name),
        )
        self.wait()
        self.play(self.rect.scale, 0.5, rate_func=there_and_back)
        self.play(FadeIn(VGroup(*analytic_taylor[3:5])))
        self.wait(2)
        self.play(self.triangle.scale, 0.5, rate_func=there_and_back)
...
class EachTermHasMeaning(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(*["thinking"] * 3, look_at=4 * UP)
        self.wait(3)
...
class AskAboutInfiniteSum(TeacherStudentsScene):
    ...
    def ask_question(self):
        ...
        self.teacher_says("We could call \\\\ it an end here")
        self.play_student_changes(*["erm"] * 3)
        self.wait(3)
        self.play(
            RemovePiCreatureBubble(self.teacher),
            self.get_students()[0].change_mode,
            "plain",
            self.get_students()[2].change_mode,
            "plain",
            FadeIn(big_rect),
            randy.change_mode,
            "pondering",
        ...
        self.play(ShowCreation(arrow), Write(words))
        self.wait(3)
    ...
    def name_taylor_series(self):
        series_def = OldTexText("Infinite sum $\\Leftrightarrow$ series")
        ...
        self.play(ReplacementTransform(self.words, series_def))
        self.wait()
        self.play(GrowFromCenter(brace), Write(taylor_polynomial))
        self.wait(2)
        self.play(
            series_def.scale,
            0.7,
            series_def.to_corner,
            UP + RIGHT,
...
class ConvergenceExample(Scene):
    def construct(self):
        ...
        series = OldTex(
            *list(it.chain(*[["\\frac{1}{%d}" % (3**n), "+"] for n in range(1, max_shown_power)])) + ["\\cdots"]
        )
        ...
        for i, new_brace in enumerate(braces[1:]):
            ...
            if i == 4:
                ...
                self.wait()
        self.play(FadeOut(brace), Write(rhs))
        self.wait(2)

    def get_partial_sum_braces(self, series, partial_sums):
        braces = [Brace(VGroup(*series[:n])) for n in range(1, len(series) - 1, 2)]
        last_brace = braces[-1]
        braces += [
            braces[-1]
            .copy()
...
class ExpConvergenceExample(ConvergenceExample):
    def construct(self):
        ...
        terms = [1.0 / math.factorial(n) for n in range(11)]
    ...
    def get_series(self, arg, n_terms=5):
        series = OldTex(
            "1",
            "+",
            *list(it.chain(*[["\\frac{%s^%d}{%d!}" % (arg, n, n), "+"] for n in range(1, n_terms + 1)])) + ["\\cdots"],
        )
...
class ExpGraphConvergence(ExpPolynomial, ExpConvergenceExample):
    ...
    def construct(self):
        ...
        v_line = self.get_vertical_line_to_graph(self.example_input, graph, line_class=DashedLine, color=YELLOW)
        dot = Dot(color=YELLOW)
        ...
        self.play(dot.move_to, self.input_to_graph_point(x, graph), ShowCreation(v_line))
        self.wait()
        self.play(GrowFromCenter(brace), ShowCreation(approx_graph))
        self.wait()
    ...
    def add_series(self):
        ...
        braces = self.get_partial_sum_braces(series, np.zeros(self.n_iterations))
        for brace in braces:
    ...
    def get_approx_graphs(self):
        def get_nth_approximation(n):
            return lambda x: sum([float(x**k) / math.factorial(k) for k in range(n + 1)])

        approx_graphs = [self.get_graph(get_nth_approximation(n)) for n in range(self.n_iterations)]
        ...
        for approx_graph, color in zip(approx_graphs, colors):
            ...
            dot = Dot(color=WHITE)
            dot.move_to(self.input_to_graph_point(self.example_input, approx_graph))
...
class BoundedRadiusOfConvergence(CubicAndQuarticApproximations):
    ...
    def construct(self):
        ...
        v_line = self.get_vertical_line_to_graph(0, graph, line_class=DashedLine, color=YELLOW)
        dot = Dot(color=YELLOW).move_to(v_line.get_top())
        two_graph = self.get_graph(lambda x: 2)
        outer_v_lines = VGroup(
            *[DashedLine(self.coords_to_point(x, -2), self.coords_to_point(x, 2), color=WHITE) for x in (-1, 1)]
        )

        colors = list(self.colors) + [GREEN, MAROON_B, PINK]
        approx_graphs = [self.get_graph(taylor_approximation(func, n), color=color) for n, color in enumerate(colors)]
        approx_graph = approx_graphs[1]

        self.add(graph, v_line, dot)
        self.play(ReplacementTransform(VGroup(v_line.copy()), outer_v_lines))
        self.play(ShowCreation(approx_graph), Animation(dot))
        self.wait()
        for new_graph in approx_graphs[2:]:
            self.play(Transform(approx_graph, new_graph), Animation(dot))
            self.wait()
        self.wait()
...
class RadiusOfConvergenceForLnX(ExpGraphConvergence):
    ...
    def add_graph(self):
        self.setup_axes()
        self.graph = self.get_graph(self.func, x_min=0.01)
        self.add(self.graph)
    ...
    def add_series(self):
        ...
        self.braces = self.get_partial_sum_braces(lhs, np.zeros(self.n_iterations))
        for brace in self.braces:
            brace.remove(brace[-1])

        self.play(FadeIn(series, run_time=3, lag_ratio=0.5))
        self.wait()
        self.series = series
    ...
    def show_bounds(self):
        ...
        v_lines = [DashedLine(*[self.coords_to_point(x, y) for y in (-2, 2)]) for x in (0, 1, 2)]
        outer_v_lines = VGroup(*v_lines[::2])
        center_v_line = VGroup(v_lines[1])
        input_v_line = Line(*[self.coords_to_point(self.convergent_example, y) for y in (-4, 3)])
        ...
        self.wait()
        self.play(GrowFromCenter(center_v_line), Animation(dot))
    ...
    def show_converging_point(self):
        approx_graphs = [
            self.get_graph(taylor_approximation(self.func, n, 1), color=WHITE) for n in range(1, self.n_iterations + 1)
        ]
        colors = it.chain(self.colors[1:], [GREEN, MAROON_B], it.repeat(PINK))
        for graph, color in zip(approx_graphs, colors):
            graph.set_color(color)
        for graph in approx_graphs:
            dot = Dot(color=WHITE)
            dot.move_to(self.input_to_graph_point(self.convergent_example, graph))
            graph.dot = dot
            graph.add(dot)
        ...
        self.play(
            approx_dot.move_to,
            self.coords_to_point(self.divergent_example, 0),
            *it.chain(
    ...
    def show_diverging_point(self):
        for graph in self.approx_graphs:
            graph.dot.move_to(self.input_to_graph_point(self.divergent_example, graph))

        approx_graph = self.approx_graphs[0].deepcopy()
        brace = self.braces[0].copy()

        self.play(
            ReplacementTransform(self.approx_dot, approx_graph.dot),
    ...
    def write_divergence(self):
        ...
        arrow = Arrow(word.get_left(), self.approx_dot, buff=SMALL_BUFF, color=WHITE)

        self.play(Write(word), ShowCreation(arrow))
        self.wait()
    ...
    def write_radius_of_convergence(self):
        line = Line(*[self.coords_to_point(x, 0) for x in (1, 2)])
        line.set_color(YELLOW)
        brace = Brace(line, DOWN)
        words = brace.get_text("``Radius of convergence''")
        words.add_background_rectangle()

        self.play(GrowFromCenter(brace), ShowCreation(line))
        self.wait()
...
class MoreToBeSaid(TeacherStudentsScene):
    ...
    def construct(self):
        words = OldTexText("Lagrange error bounds, ", "convergence tests, ", "$\\dots$")
