...
class RefreshOnDerivativeDefinition(GraphScene):
    ...
    def construct(self):
        ...
        start_x_v_line, nudged_x_v_line = [
            self.get_vertical_line_to_graph(self.start_x + nudge, graph, line_class=DashedLine, color=RED)
        ...
        self.wait()
        self.play(
            *[ReplacementTransform(*pair, run_time=2) for pair in list(deriv_to_new_deriv.items())]
...
class RantOpenAndClose(Scene):
    def construct(self):
        opening, closing = [
            OldTexText(start, "Rant on infinitesimals", "$>$", arg_separator="") for start in ("$<$", "$<$/")
        ]
...
class DiscussLowercaseDs(RefreshOnDerivativeDefinition, PiCreatureScene, ZoomedScene):
    ...
    def compare_rhs_and_lhs(self):
        ...
        self.animate_secant_slope_group_change(
            self.ss_group,
            target_dx=0.01,
            added_anims=[Transform(self.nudged_x_v_line, self.start_x_v_line, run_time=3)],
        )
    ...
    def construct(self):
        ...
        for statement in statements:
            statement.h, statement.dx = [
                VGroup(*statement.get_parts_by_tex(tex, substring=False)).set_color(GREEN) for tex in ("$h$", "$dx$")
            ]
        ...
        self.play(
            ...
            *it.chain(
                *[[pi.change_mode, "pondering", pi.look_at, bubble_intro.mobject] for pi in self.get_students()[1:]]
            ),
        )
        self.wait(3)
...
class GraphLimitExpression(GraphScene):
    ...
    def show_limit(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(ed_group.input_range.copy(), ed_group.output_range, run_time=2),
        )
    ...
    def get_epsilon_delta_group(
        ...
        result.delta_lines = VGroup(
            *[dashed_line.copy().rotate(np.pi / 2).move_to(point, DOWN) for point in x_axis_points]
        )
        ...
        result.epsilon_lines = VGroup(
            *[
                dashed_line.copy().move_to(
                    self.coords_to_point(limit_x, 0)[0] * RIGHT + result.output_range.get_edge_center(vect)[1] * UP
                )
                for vect in (DOWN, UP)
            ]
        )
...
class EpsilonDeltaExample(GraphLimitExpression, ZoomedScene):
    ...
    def introduce_epsilon(self):
        ...
        self.play(
            *it.chain(
                [ReplacementTransform(twelve_line.copy(), line) for line in epsilon_group.epsilon_lines],
    ...
    def match_epsilon(self):
        self.animate_epsilon_delta_group_change(
            ...
            run_time=2,
            added_anims=[ApplyMethod(hole.scale, 0.25, run_time=2) for hole in self.holes],
        )
    ...
    def introduce_delta(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(self.ed_group.input_range.copy(), self.ed_group.output_range, run_time=2),
    ...
    def get_epsilon_group(self, epsilon, limit_value=12):
        ...
        result.braces = VGroup(
            *[
                brace.copy().set_height(group.get_height()).next_to(group, RIGHT, SMALL_BUFF)
                for i in (1, 2)
...
class EpsilonDeltaCounterExample(LimitCounterExample, EpsilonDeltaExample):
    ...
    def introduce_epsilon(self):
        ...
        self.play(
            *it.chain(
                [ReplacementTransform(epsilon_group.limit_line.copy(), line) for line in epsilon_group.epsilon_lines],
                list(map(GrowFromCenter, epsilon_group.braces)),
            )
        )
        self.play(*list(map(Write, epsilon_group.labels)))
...
class TheoryHeavy(TeacherStudentsScene):
    def construct(self):
        ...
        Scene.play(
            ...
            *[
                ApplyFunction(lambda pi: pi.change_mode("pondering").look_at(epsilon_delta), pi)
        ...
        self.play(
            ...
            *[
                ...
                for (t1, t2), alpha in zip(part_tex_pairs, np.linspace(0, 0.5, len(part_tex_pairs)))
            ],
        )
        self.wait(2)
...
class LHopitalExample(LimitCounterExample, PiCreatureScene, ZoomedScene, ReconfigurableScene):
    ...
    def show_non_definedness_at_one(self):
        ...
        v_line, alt_v_line = [
            self.get_vertical_line_to_graph(x, self.graph, line_class=DashedLine, color=self.x_color) for x in (1, -1)
        ]
    ...
    def ask_about_systematic_process(self):
        ...
        self.wait(2)
        self.play(
            RemovePiCreatureBubble(morty, target_mode="raise_left_hand", look_at=self.func_1_group),
    ...
    def talk_through_sizes_of_nudges(self):
        ...
        d_sine_arrow, d_parabola_arrow = [
            Arrow(
                self.coords_to_point(1 + self.dx, 0),
                self.coords_to_point(1 + self.dx, graph.underlying_function(1 + self.dx)),
                tip_length=arrow_tip_length,
...
class GeneralLHoptial(LHopitalExample):
    ...
    def show_limit_in_symbols(self):
        ...
        self.play(
            ReplacementTransform(VGroup(*self.f_label).copy(), VGroup(frac_a.numerator)),
            ReplacementTransform(VGroup(*self.g_label).copy(), VGroup(frac_a.denominator)),
            Write(frac_a.over),
            run_time=2,
        )
        self.wait()
...
class CannotUseLHopital(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            Write(deriv),
            *it.chain(*[[pi.change_mode, "pondering", pi.look_at, deriv] for pi in self.get_pi_creatures()]),
        )
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            *[
            ...
            + [Write(VGroup(*ftc.get_parts_by_tex(part))) for part in ("-", "=", "over", "(x)")]
        )
