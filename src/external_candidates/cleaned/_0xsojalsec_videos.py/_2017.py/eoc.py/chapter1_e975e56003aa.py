# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter1.py
...
class CircleScene(PiCreatureScene):
    ...
    def increase_radius(self, numerical_dr=True, run_time=2):
        ...
        self.play(
            ...
            Write(nudge_label),
            run_time=run_time / 2.0,
        )
        self.wait(run_time / 2.0)
...
class Chapter1OpeningQuote(OpeningQuote):
    CONFIG = {
        ...
        "quote_arg_separator": " ",
        "highlighted_quote_terms": {"special case": BLUE},
...
class Introduction(TeacherStudentsScene):
    ...
    def show_series(self):
        ...
        words = OldTexText("Welcome to \\\\", "Essence of calculus")
        words.set_color_by_tex("Essence of calculus", YELLOW)

        self.teacher.change_mode("happy")
        self.play(FadeIn(series, lag_ratio=0.5, run_time=2), Blink(self.get_teacher()))
    ...
    def show_many_facts(self):
        ...
        self.play_student_changes(
            ...
            added_anims=[self.teacher.change_mode, "plain"],
        )
        self.wait(2)
        self.play(
            Write(rules[0]),
        ...
        for last_rule, rule, video_index in zip(rules, alt_rules_list, video_indices):
            ...
            self.wait()
        self.play(self.teacher.change_mode, "happy", self.teacher.look_at, student.eyes)
    ...
    def invent_calculus(self):
        ...
        self.play(FadeIn(fader), Animation(student), Animation(calculus))
        self.play(
            Write(invent),
...
class ProductRuleDiagram(Scene):
    def construct(self):
        ...
        rect.shift(DOWN)
        df_rect = Rectangle(height=rect.get_height(), width=df, **rect_kwargs)
        dg_rect = Rectangle(height=dg, width=rect.get_width(), **rect_kwargs)
        corner_rect = Rectangle(height=dg, width=df, **rect_kwargs)
        ...
        for d_rect, corner_pair in zip(d_rects, corner_pairs):
            line = Line(*[rect.get_corner(corner) for corner in corner_pair])
        ...
        for mob in df_brace, df_label, dg_brace, dg_label:
            mob.save_state()
            mob.scale(0.01, about_point=rect.get_corner(mob.get_center() - rect.get_center()))
        ...
        self.play(
            *[mob.restore for mob in (df_brace, df_label, dg_brace, dg_label)]
            + [ReplacementTransform(d_rect.line, d_rect) for d_rect in d_rects]
        )
        self.wait()
        self.play(
            d_rects.space_out_submobjects,
            1.2,
            MaintainPositionRelativeTo(VGroup(df_brace, df_label), df_rect),
            MaintainPositionRelativeTo(VGroup(dg_brace, dg_label), dg_rect),
        )
        self.wait()

        deriv = OldTex("d(", "fg", ")", "=", "f", "\\cdot", "dg", "+", "g", "\\cdot", "df")
        deriv.to_edge(UP)
            ...
            *[
            ...
            + [
                Write(
                    VGroup(
                        *it.chain(
                            *[deriv.get_parts_by_tex(tex, substring=False) for tex in ("d(", ")", "=", "\\cdot", "+")]
                        )
                    )
                )
            ],
...
class IntroduceCircle(CircleScene):
    CONFIG = {"include_pi_creature": True, "unwrapped_tip": 2 * RIGHT}
    ...
    def question_area(self):
        ...
        unwrapped_rings = VGroup(*[self.get_unwrapped(ring, to_edge=None) for ring in rings])
        unwrapped_rings.arrange(UP, buff=SMALL_BUFF)
        unwrapped_rings.move_to(self.unwrapped_tip, UP)
        ring_anim_kwargs = {"run_time": 3, "lag_ratio": 0.5}
    ...
    def show_calculus_symbols(self):
        ftc = OldTex("\\int_0^R", "\\frac{dA}{dr}", "\\,dr", "=", "A(R)")
        ftc.shift(2 * UP)

        self.play(
            ReplacementTransform(self.area.get_part_by_tex("R").copy(), ftc.get_part_by_tex("int")),
            self.pi_creature.change_mode,
            "plain",
        )
        self.wait()
        self.play(
            ReplacementTransform(self.area.get_part_by_tex("Area").copy(), ftc.get_part_by_tex("frac")),
            ReplacementTransform(self.area.get_part_by_tex("R").copy(), ftc.get_part_by_tex("\\,dr")),
        )
        self.wait()
...
class ApproximateOneRing(CircleScene, ReconfigurableScene):
    ...
    def write_radius_three(self):
        ...
        self.play(Transform(self.radius_label, three, path_arc=np.pi))
        self.wait()
    ...
    def try_to_understand_area(self):
        ...
        self.play(
            ShowCreation(lines, run_time=2, lag_ratio=0.5),
            Animation(self.radius_group),
            self.pi_creature.change_mode,
            "maybe",
        )
        ...
        for new_lines in line_sets[1:]:
            self.play(Transform(lines, new_lines), Animation(self.radius_group))
            self.wait()
        self.wait()
    ...
    def slice_into_rings(self):
        ...
        self.play(
            FadeIn(rings, lag_ratio=0.5, run_time=3),
            Animation(self.radius_group),
            self.pi_creature.change_mode,
            "pondering",
            self.pi_creature.look_at,
    ...
    def approximate_as_rectangle(self):
        ...
        alt_side_brace.shift(self.original_ring.get_boundary_point(RIGHT))
        ...
        two_pi_r_dr.target.arrange(RIGHT, buff=SMALL_BUFF, aligned_edge=DOWN)
        ...
        self.play(
            ...
            ReplacementTransform(self.ring_radius_group[1].copy(), width_label.get_part_by_tex("r")),
        )
        self.wait()
        self.play(GrowFromCenter(side_brace), Write(q_marks))
        ...
        self.play(
            ...
            FadeIn(approx),
            self.area_q.get_part_by_tex("?").fade,
            1,
        )
        self.wait()
        self.play(FadeOut(concrete_dr), dr_label.restore)
    ...
    def show_alternate_width(self, num_rings, **kwargs):
        self.transition_to_alt_config(dR=self.radius / num_rings, **kwargs)


class MoveForwardWithApproximation(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Move forward with \\\\", "the", "approximation")
        self.play_student_changes("hesitant", "erm", "sassy")
        self.wait()
        words = OldTexText("It gets better", "\\\\ for smaller ", "$dr$")
...
class GraphRectangles(CircleScene, GraphScene):
    ...
    def draw_ring_sum(self):
        ...
        area_label = OldTex("\\text{Area}", "\\approx", "2\\pi", "r", "\\,dr")
        ...
        arrows = VGroup(
            *[
                Arrow(
                    arrow_start,
                    ring.target.get_boundary_point(arrow_start - ring.target.get_center()),
    ...
    def draw_r_values(self):
        ...
        values_of_r.next_to(self.x_axis, UP, buff=2 * LARGE_BUFF, aligned_edge=LEFT)
        ...
        index = 2
        dr_brace = Brace(VGroup(*r_ticks[index : index + 2]), DOWN, buff=SMALL_BUFF)
        dr_label = OldTex("dr")
        dr_label.next_to(dr_brace, DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
    ...
    def unwrap_rings_onto_graph(self):
        ...
        for index, rect, r in zip(it.count(), rects, np.arange(0, 3, 0.1)):
            ...
            target.set_points_as_corners(
                [interpolate(ORIGIN, DOWN, a) for a in np.linspace(0, 1, n_anchors / 2)]
                + [interpolate(DOWN + RIGHT, RIGHT, a) for a in np.linspace(0, 1, n_anchors / 2)]
            )
        ...
        self.play(
            MoveToTarget(example_ring, path_arc=-np.pi / 2, run_time=2),
        ...
        self.play(
            *[
                ...
                for ring, alpha in zip(transformed_rings, np.linspace(0, 0.75, len(transformed_rings)))
            ]
        ...
        self.play(
            self.dr_group.arrange,
            DOWN,
            self.dr_group.next_to,
            highlighted_ring,
            DOWN,
            SMALL_BUFF,
        )
        self.wait()
        self.play(GrowFromCenter(side_brace), Write(height_label))
        self.wait()
        ...
        ##Rescale
        self.play(
            *[ApplyMethod(ring.replace, ring.rect, method_kwargs={"stretch": True}) for ring in transformed_rings]
            + [
                Write(self.y_axis),
    ...
    def draw_graph(self):
        graph_label = self.get_graph_label(self.graph, "2\\pi r", direction=UP + LEFT, x_val=2.5, buff=SMALL_BUFF)
        ...
        self.play(
            *[
                Transform(
                    ...
                    flat_rect,
                    run_time=2,
                    rate_func=squish_rate_func(lambda t: 0.1 * there_and_back(t), alpha, alpha + 0.5),
                    lag_ratio=0.5,
                )
                for rect, flat_rect, alpha in zip(self.rects, self.flat_rects, np.linspace(0, 0.5, len(self.rects)))
            ]
    ...
    def point_out_approximation(self):
        ...
        self.wait(2)
        self.play(rect.restore, FadeOut(approximation))
        self.wait()
    ...
    def let_dr_approah_zero(self):
        ...
        for new_rects in thinner_rects_list:
            self.play(
                Transform(self.rects, new_rects, lag_ratio=0.5, run_time=2),
                Animation(self.axes),
    ...
    def compute_area_under_graph(self):
        formula, formula_with_R = formulas = [self.get_area_formula(R) for R in ("3", "R")]
        for mob in formulas:
        ...
        base_line = Line(self.coords_to_point(0, 0), self.coords_to_point(3, 0), color=YELLOW)

        fresh_rings = self.get_rings(dR=0.025)
        fresh_rings.set_stroke(width=0)
        self.radius_label.restore()
        VGroup(fresh_rings, self.radius_group).to_corner(UP + LEFT, buff=SMALL_BUFF)

        self.play(Write(formula.top_line, run_time=2))
        self.play(FocusOn(base_line))
        self.play(ShowCreation(base_line))
        self.wait()
        self.play(GrowFromCenter(brace), Write(height_label))
        self.wait()
        ...
        self.play(*list(map(FadeOut, [self.ghost_rings, self.ring_sum.tex_mobs])))
    ...
    def get_ring_sum(self, rings):
        ...
        for ring, alpha in zip(middle_rings, alphas):
            ...
            ring.target.move_to(interpolate(dots.get_left(), last_ring.target.get_center(), alpha))
...
class ThinkLikeAMathematician(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(pi_R_squraed.shift, 2 * UP, pi_R_squraed.set_fill, None, 1)
        self.play_student_changes(*["hooray"] * 3)
        self.wait(2)
        self.play_student_changes(
            *["pondering"] * 3,
            look_at=self.teacher.eyes,
            added_anims=[PiCreatureSays(self.teacher, "But why did \\\\ that work?")],
        )
...
class RecapCircleSolution(GraphRectangles, ReconfigurableScene):
    ...
    def break_up_circle(self):
        ...
        self.add(rings, self.radius_group)
        self.play(FadeIn(integral_condition, lag_ratio=0.5))
        self.wait()
        self.play(*draw_ring_sum_anims)
    ...
    def show_sum(self):
        ...
        for ring, radius in zip(visible_rings, radii):
            label = OldTex("\\approx", "2\\pi", "(%s)" % str(radius), "(%s)" % str(self.dR))
        ...
        arrows = VGroup(
            *[Arrow(area_label.get_left(), approximation.get_right(), color=WHITE) for approximation in approximations]
        )
        ...
        self.play(
            ...
            FadeIn(radial_lines),
            *[ReplacementTransform(area_label.copy(), VGroup(*approximation[1:])) for approximation in approximations],
        )
        self.wait()
        self.play(Write(VGroup(*[approximation[0] for approximation in approximations])))
        self.wait()
    ...
    def dr_indicates_spacing(self):
        ...
        self.wait()
        self.play(GrowFromCenter(dr_brace), dr_copy.next_to, dr_brace.copy(), UP)
        self.wait()
    ...
    def show_riemann_sum(self):
        graph = self.get_graph(lambda r: 2 * np.pi * r)
        graph_label = self.get_graph_label(graph, "2\\pi r", x_val=2.5, direction=UP + LEFT)
        rects = self.get_riemann_rectangles(graph, x_min=0, x_max=3, dx=self.dR)
        ...
        self.play(
            ReplacementTransform(self.rings.copy(), rects, run_time=2, lag_ratio=0.5),
            Animation(self.x_axis),
        )
    ...
    def full_precision(self):
        ...
        arc_next_to_group = VGroup(self.integral_condition[0][0], words[0])
        arc.set_height(arc_next_to_group.get_height() - MED_LARGE_BUFF)
...
class ExampleIntegralProblems(PiCreatureScene, GraphScene):
    ...
    def write_integral_condition(self):
        words = OldTexText("Hard problem $\\Rightarrow$ Sum of many small values")
        words.to_edge(UP)

        self.play(Write(words), self.pi_creature.change_mode, "raise_right_hand")
        self.wait()
    ...
    def show_car(self):
        ...
        ticks = VGroup(
            *[
                ...
                for p in [line.point_from_proportion(smooth(t / self.t_max))]
            ]
        )
        ...
        self.play(FadeIn(car), self.pi_creature.change_mode, "plain")
        self.play(
            MoveCar(car, end),
            FadeIn(
                ticks,
            ...
            ShowCreation(line),
            FadeIn(v_dt_brace_group, rate_func=squish_rate_func(smooth, 0.6, 0.8)),
            run_time=self.t_max,
        )
        self.wait()
        for mob in v_dt:
    ...
    def show_graph(self):
        self.setup_axes()
        self.remove(self.axes)
        s_graph = self.get_graph(lambda t: 1.8 * self.y_max * smooth(t / self.t_max))
        v_graph = self.get_derivative_graph(s_graph)
        rects = self.get_riemann_rectangles(v_graph, x_min=0, x_max=self.t_max, dx=self.dt)
        ...
        self.play(FadeIn(pre_rects, run_time=2, lag_ratio=0.5), Animation(self.ticks))
        self.play(
            ReplacementTransform(pre_rects, rects, run_time=3, lag_ratio=0.5),
    ...
    def let_dt_approach_zero(self):
        ...
        for thinner_rects in thinner_rects_list:
            self.play(Transform(self.rects, thinner_rects, run_time=2, lag_ratio=0.5))
            self.wait()
    ...
    def show_confusion(self):
        ...
        self.play(randy.change_mode, "confused", randy.look_at, self.rects)
        self.play(
            self.pi_creature.change_mode,
            "confused",
            self.pi_creature.look_at,
...
class MathematicianPonderingAreaUnderDifferentCurves(PiCreatureScene):
    def construct(self):
        ...
        self.wait(4)
        self.play(self.pi_creature.change_mode, "pondering", self.pi_creature.look, UP + LEFT)
        self.wait(2)
...
class AreaUnderParabola(GraphScene):
    ...
    def ask_about_area(self):
        ...
        v_lines = VGroup(
            *[
                DashedLine(FRAME_HEIGHT * UP, ORIGIN, color=RED).move_to(self.coords_to_point(x, 0), DOWN)
    ...
    def name_integral(self):
        ...
        self.play(Write(words), GrowFromCenter(brace))
        self.wait()
    ...
    def move_right_point_to(self, target_x, **kwargs):
        ...
        self.play(
            UpdateFromAlphaFunc(group, update_group, **kwargs),
...
class WhoCaresAboutArea(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(*["pondering"] * 3, look_at=point, added_anims=[self.teacher.look_at, point])
        self.wait(3)


class PlayWithThisIdea(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Play with", "the", "thought!", target_mode="hooray")
...
class PlayingTowardsDADX(AreaUnderParabola, ReconfigurableScene):
    ...
    def nudge_x(self):
        ...
        self.play(FadeIn(shadow_rects), *list(map(Animation, self.foreground_mobjects)))
    ...
    def describe_sliver(self):
        ...
        difference_in_area = OldTexText("d", "ifference in ", "A", "rea", arg_separator="")
        ...
        self.play(FadeOut(self.right_point_slider), FadeIn(dx_group))
        ...
        self.play(
            graph_label_copy.set_color,
            WHITE,
            graph_label_copy.next_to,
            side_brace,
            LEFT,
            SMALL_BUFF,
        )
    ...
    def write_dA_dx(self):
        ...
        self.play(
            *[
                ReplacementTransform(mob.copy(), equation.get_part_by_tex(tex), run_time=2)
        ...
        for tex, mob in (f_tex, x_squared), ("dx", dx):
            self.play(*list(map(Indicate, [equation.get_part_by_tex(tex), mob])))
            self.wait(2)
        self.play(
            *[
                ReplacementTransform(
    ...
    def dA_remains_a_mystery(self):
        ...
        A_circle, dA_circle = [
            Circle(color=color).replace(mob, stretch=True).scale(1.5)
            for mob, color in [(self.A_func, RED), (self.deriv_equation, GREEN)]
        ]
        q_marks = OldTex("???")
        q_marks.next_to(A_circle, UP)

        self.play(FadeOut(self.integral_words_group), FadeIn(randy))
        self.play(ShowCreation(A_circle), randy.change_mode, "confused")
    ...
    def write_example_inputs(self):
        ...
        labels_lines_vects = list(zip([three, three_plus_dx], self.right_v_lines, [LEFT, RIGHT]))
        ...
        for label, line, vect in labels_lines_vects:
            ...
            label.arrow = Arrow(label, point, buff=SMALL_BUFF, color=WHITE, tip_length=0.15)
            line_copy = line.copy()
            line_copy.set_color(YELLOW)

            self.play(FadeIn(label), FadeIn(label.arrow), ShowCreation(line_copy))
    ...
    def show_dA_dx_in_detail(self):
        ...
        expression.next_to(self.deriv_equation, DOWN, MED_LARGE_BUFF)
        ...
        self.play(
            ...
            ReplacementTransform(self.graph_label_copy.copy(), VGroup(*expression[-2:]), run_time=2),
        )
        self.wait()

    def show_smaller_x(self):
        self.transition_to_alt_config(default_right_x=2, deriv_dx=0.04, transformation_kwargs={"run_time": 2})
...
class AlternateAreaUnderCurve(PlayingTowardsDADX):
    ...
    def approximation_improves_for_smaller_dx(self):
        ...
        dx_to_zero_words = OldTexText("Gets better \\\\ as", "$dx \\to 0$")
        ...
        self.wait()
        self.transition_to_alt_config(deriv_dx=self.deriv_dx / 4.0, transformation_kwargs={"run_time": 2})
    ...
    def name_derivative(self):
        ...
        self.play(FadeIn(deriv_words), MoveToTarget(moving_group))
        ...
        box.scale(1.3)
        brace = Brace(box, UP)
        faders = VGroup(self.dx_to_zero_words[0], self.dx_to_zero_words_arrow)
        dx_to_zero = self.dx_to_zero_words[1]

        self.play(*list(map(FadeIn, [box, brace])))
        self.wait()
        self.play(FadeOut(faders), dx_to_zero.next_to, box, DOWN)
        self.wait()
...
class NextVideos(TeacherStudentsScene):
    def construct(self):
        ...
        self.add(series)
        self.teacher_says("That's a high-level view")
        self.wait()
        self.play(
            ...
            *it.chain(*[[pi.change_mode, "pondering", pi.look_at, this_video] for pi in self.get_students()]),
        )
        self.play(*[ApplyMethod(pi.look_at, series) for pi in self.get_pi_creatures()])
        self.play(
            *[
                ApplyMethod(
                    video.shift,
        ...
        self.play(everything.fade, 0.75, student.change_mode, "plain")
        self.play(
            Write(words),
            student.look_at,
            words,
        )
        self.play(student.change_mode, "confused", student.look_at, words)
        self.wait(3)
...
class EndScreen(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
            FadeIn(words, run_time=2, lag_ratio=0.5),
            self.pi_creature.change_mode,
            "hooray",
        )
        self.wait()
