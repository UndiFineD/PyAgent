...
class Ladder(VMobject):
    ...
    def init_points(self):
        left_line, right_line = [
            Line(ORIGIN, self.height * UP).shift(self.width * vect / 2.0) for vect in (LEFT, RIGHT)
        ]
        rungs = [
            Line(
...
class RelatedRatesExample(ThreeDScene):
    ...
    def discuss_lhs_as_function(self):
        ...
        derivative.shift(  ##Align x terms
            equation[0][0].get_center()[0] * RIGHT - derivative[2].get_center()[0] * RIGHT
        )
    ...
    def let_dt_pass(self):
        ...
        for line, brace, label in (
            (dy_line, dy_brace, dy_label),
            (
                dx_line,
                dx_brace,
                dx_label,
    ...
    def replace_terms_in_final_form(self):
        ...
        def fill_in_equation_part(*indices):
            self.play(
                *[
                    ReplacementTransform(self.lhs_derivative[i].copy(), new_lhs_derivative[i], run_time=2)
                    for i in indices
                ]
            )
...
class CompareLadderAndCircle(PiCreatureScene, ThreeDScene):
    ...
    def show_derivatives(self):
        ...
        self.play(
            *[
                ReplacementTransform(equation[i].copy(), derivative[j], path_arc=np.pi / 2)
...
class TwoVariableFunctionAndDerivative(SlopeOfCircleExample):
    ...
    def plug_in_example_values(self):
        ...
        def add_example_parts(*indices):
            self.play(
                *[ReplacementTransform(self.derivative[1][i].copy(), deriv_example[1][i], run_time=2) for i in indices]
            )
        ...
        self.play(
            *[
                ApplyMethod(
                    ...
                    self.formula[1][j],
                    run_time=3,
                    rate_func=squish_rate_func(smooth, count / 6.0, count / 6.0 + 2.0 / 3),
                )
...
class AskAboutNaturalLog(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            UP,
            LARGE_BUFF,
            *[ApplyMethod(pi.change_mode, "confused") for pi in self.get_pi_creatures()],
        )
        self.wait(3)
...
class FinalWords(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            Write(lim),
            *it.chain(*[[pi.change_mode, "pondering", pi.look_at, lim] for pi in self.get_pi_creatures()]),
        )
        self.wait(3)
