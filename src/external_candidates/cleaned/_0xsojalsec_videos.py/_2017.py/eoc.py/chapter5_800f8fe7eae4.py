...
class DoublingPopulation(PiCreatureScene):
    ...
    def create_pi_creatures(self):
        ...
        creature_array = VGroup(
            *[
                VGroup(*[PiCreature(mode="plain") for y in range(height)]).arrange(UP, buff=MED_LARGE_BUFF)
...
class GraphOfTwoToT(GraphScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            ReplacementTransform(height_label.copy(), slope_label.get_part_by_tex("2^")),
        )
        self.wait()
...
class FakeDiagram(TeacherStudentsScene):
    def construct(self):
        ...
        background_graph, foreground_graph = graphs = VGroup(
            *[gs.get_graph(lambda t: np.log(2) * 2**t, x_min=-8, x_max=2 + dx) for dx in (0.25, 0)]
        )
        for graph in graphs:
...
class AnalyzeExponentRatio(PiCreatureScene):
    ...
    def construct(self):
        ...
        # Factor out 2^t
        self.play(
            *[FadeIn(mob, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)) for mob in (one, lp, rp)]
            + [
                ReplacementTransform(
                    mob,
                    extracted_two_to_t,
...
class GraphOfExp(GraphScene):
    ...
    def show_slopes(self):
        ...
        height_labels = [
            OldTex("e^%d" % x).next_to(vl, RIGHT, SMALL_BUFF) for vl, x in zip(v_lines, self.example_inputs)
        ]
        slope_labels = [
            OldTexText("Slope = $e^%d$" % x).next_to(vl.get_top(), UP + RIGHT).shift(0.7 * RIGHT / x)
...
class WhyNaturalLogOf2ShowsUp(TeacherStudentsScene):
    ...
    def show_e_to_log_2(self):
        ...
        self.play(
            Write(log_words),
            ShowCreation(log_words_arrow, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)),
        )
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            Write(this_tex[4]),
            ReplacementTransform(this_tex[3][0].copy(), this_tex[4][3], path_arc=np.pi, remover=True),
        )
        self.wait(2)
