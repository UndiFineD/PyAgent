# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter3.py
from _2017.eoc.chapter2 import DISTANCE_COLOR, TIME_COLOR, VELOCITY_COLOR, Car, MoveCar
...
class PoseAbstractDerivative(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            "Why?",
            target_mode="sassy",
            added_anims=[content_copy.scale, 0.8, content_copy.to_corner, UP + LEFT],
        )
...
class ContrastAbstractAndConcrete(Scene):
    def construct(self):
        ...
        functions = VGroup(
            *list(
                map(
                    Tex,
                    ["f(x) = 2x^2 - x^3", "f(x) = \\sin(x)", "f(x) = e^x", "\\v_dots"],
                )
            )
        )
        functions.arrange(DOWN, aligned_edge=LEFT, buff=LARGE_BUFF)
    ...
    def get_spring_anim(self, alignement_mob):
        ...
        for spring in compact_spring, extended_spring:
            ...
            spring.shift(alignement_mob.get_center()[1] * UP + SMALL_BUFF * RIGHT - spring.get_points()[0])
            weight = Square(
                side_length=0.5,
                stroke_width=0,
                fill_color=GREY_B,
    ...
    def get_population_anim(self, alignement_mob):
        ...
        pis = VGroup(
            *[
                Randolph(mode="happy", color=random.choice(colors)).shift(
                    4 * x * RIGHT + 4 * y * UP + 2 * random.random() * RIGHT + 2 * random.random() * UP
                )
                for x in range(20)
                for y in range(10)
            ]
        ...
        pis.shift(FRAME_X_RADIUS * RIGHT / 2.0)
        ...
        for index, pi in enumerate(pis):
            if index < 2:
                ...
                continue
            mom_index, dad_index = random.choice(list(it.combinations(list(range(index)), 2)))
...
class DerivativeOfXSquaredAsGraph(GraphScene, ZoomedScene, PiCreatureScene):
    ...
    def ask_about_df_dx(self):
        ...
        v_line, nudged_v_line = [
            self.get_vertical_line_to_graph(x, self.graph, line_class=DashedLine, color=RED, dash_length=0.025)
        ...
        self.play(Write(derivative_q, run_time=1), self.pi_creature.change_mode, "speaking")
        self.wait()
        self.play(FadeOut(derivative_q), self.pi_creature.change_mode, "plain")
    ...
    def show_differing_slopes(self):
        ...
        self.play(ShowCreation(ss_group.secant_line), self.pi_creature.change_mode, "thinking")
        ...
        for target_x in self.big_x, -self.dx / 2, 1, 2:
            self.animate_secant_slope_group_change(
                ss_group,
                target_x=target_x,
                added_anims=[UpdateFromFunc(self.little_rectangle, rect_update)],
            )
            self.wait()
...
class NudgeSideLengthOfSquare(PiCreatureScene):
    ...
    def add_function_label(self):
        ...
        label.next_to(ORIGIN, RIGHT, buff=(self.square_width - 3) / 2.0)
        label.to_edge(UP)
        self.add(label)
    ...
    def introduce_square(self):
        ...
        self.play(DrawBorderThenFill(square), self.pi_creature.change_mode, "plain")
    ...
    def increase_area(self):
        ...
        right_rect = Rectangle(width=self.dx, height=self.square_width, **color_kwargs)
        ...
        corner_square = Square(side_length=self.dx, **color_kwargs)
        ...
        corner_point = VectorizedPoint(self.square.get_corner(DOWN + RIGHT))
    ...
    def write_df_equation(self):
        ...
        df_equation.arrange()
        df_equation.next_to(self.function_label, DOWN, aligned_edge=LEFT, buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(
            *[mob.restore for mob in (r1, r2, s)] + [Write(symbol) for symbol in (equals, plus1, plus2)],
            run_time=2,
        )
    ...
    def set_color_shapes(self):
        ...
        tups = [(self.right_rect, self.bottom_rect, r1, r2), (self.corner_square, s)]
        for tup in tups:
            self.play(
                *it.chain(*[[m.scale, 1.2, m.set_color, RED] for m in tup]),
    ...
    def examine_thin_rectangles(self):
        ...
        self.play(Write(text), self.pi_creature.change_mode, "pondering")
        self.wait()

        xs = VGroup(*[brace[-1] for brace in self.side_braces])
        dxs = VGroup(*[brace[-1] for brace in self.little_braces])
        for group, tex, color in (
            (xs, "3", self.three_color),
            (
                dxs,
                "0.01",
                self.dx_color,
        ...
        self.wait()
        self.play(FadeOut(example_value), *[mob.restore for mob in (xs, dxs, text)])
        self.remove(text)
        text.restore()
        self.add(text)
    ...
    def examine_tiny_square(self):
        ...
        self.wait()
        self.play(MoveToTarget(self.dxs), self.pi_creature.change_mode, "plain")
        ...
        self.play(Transform(text, alt_text), self.pi_creature.change_mode, "raise_right_hand")
        self.wait(2)
        self.play(*[mob.restore for mob in (self.dxs, text)] + [self.pi_creature.change_mode, "erm"])
        self.dx_squared = text

    def show_smaller_dx(self):
        self.mobjects_at_start_of_show_smaller_dx = [mob.copy() for mob in self.get_mobjects()]
        if self.is_recursing_on_dx:
            return

        alt_scene = self.__class__(skip_animations=True, dx=self.alt_dx, is_recursing_on_dx=True)
        ...
        self.wait()
        self.play(*[mob.restore for mob in self.get_mobjects()])
        self.change_mode("happy")
        self.wait()
    ...
    def rule_of_thumb(self):
        ...
        self.play(
            ...
            dx_squared_group.fade,
            0.5,
            dx_squared_group.to_corner,
            UP + RIGHT,
            self.pi_creature.change_mode,
            "plain",
        )
    ...
    def write_out_derivative(self):
        ...
        brace_xs = [brace[-1] for brace in self.side_braces]
        xs = list(brace_xs) + [x]
        for x_mob in xs:
            number = OldTex("(%d)" % self.square_width)
            number.move_to(x_mob, LEFT)
            number.shift((x_mob.get_bottom()[1] - number[1].get_bottom()[1]) * UP)
            x_mob.save_state()
            x_mob.target = number
...
class ChangeInAreaOverChangeInX(Scene):
    def construct(self):
        ...
        for pair in ("Change in area", "Change in $x$"), ("$d(x^2)$", "$dx$"):
            ...
            fractions.append(VGroup(top, frac_line, bottom))
...
class NudgeSideLengthOfCube(Scene):
    ...
    def introduce_cube(self):
        ...
        self.play(
            *[ApplyMethod(piece.shift, 0.5 * (piece.get_center() - cube.get_center())) for piece in dv_pieces]
        ...
        self.faces, self.bars, self.corner_cube = [
            VGroup(*[piece for piece in dv_pieces if piece.type == target_type])
    ...
    def write_df_equation(self):
        ...
        ignore_text = extras_brace.get_text("Multiple \\\\ of $dx^2$")
        ...
        for submob, sides in zip(x_squared_dx, [face[0], VGroup(*face[1:])]):
            self.play(submob.set_color, RED, sides.set_color, RED, rate_func=there_and_back)
            self.wait()
        self.play(
            face.restore,
            Transform(x_squared_dx, derivative, replace_mobject_with_target_in_scene=True),
            GrowFromCenter(faces_brace),
        )
        self.wait()
        self.grab_pieces(self.bars, bars, plus1)
        self.grab_pieces(self.corner_cube, corner_cube, plus2)
        self.play(GrowFromCenter(extras_brace), Write(ignore_text))
        self.wait()
        self.play(*[ApplyMethod(mob.fade, 0.7) for mob in [plus1, bars, plus2, corner_cube, extras_brace, ignore_text]])
        self.wait()
    ...
    def write_derivative(self):
        ...
        frac_line2 = OldTex("-")
        frac_line2.stretch_to_fit_width(extra_stuff.target[1].get_width())
    ...
    def grab_pieces(self, start_pieces, end_pices, to_write=None):
        for piece in start_pieces:
            ...
            piece.target.rotate(np.pi / 12, piece.get_center() - self.cube.get_center())
        ...
        self.play(Transform(start_pieces.copy(), end_pices), *added_anims)

    def shrink_dx(self, state_name, restore=True):
        mobjects = self.get_mobjects()
        mobjects_with_points = [m for m in mobjects if m.get_num_points() > 0]
        # Alt_scene will reach this point, and save copy of self
        # in states dict
        self.states[state_name] = [mob.copy() for mob in mobjects_with_points]
        ...
        self.play(*[Transform(*pair) for pair in zip(movers, self.alt_scene.states[state_name])])
        self.wait()
        if restore:
    ...
    def get_dv_pices(self, cube):
        ...
        for vect in it.product([0, 1], [0, 1], [0, 1]):
            if np.all(vect == ORIGIN):
                continue
            args = [self.x if bit is 0 else self.dx for bit in vect]
    ...
    def organize_faces(self, faces):
        ...
        for face in faces:
            dimensions = [face.length_over_dim(dim) for dim in range(3)]
    ...
    def organize_bars(self, bars):
        ...
        for bar in bars:
            dimensions = [bar.length_over_dim(dim) for dim in range(3)]
    ...
    def get_prism(self, width, height, depth):
        ...
        front = Rectangle(width=width, height=height, **color_kwargs)
        face = VGroup(front)
        for vect in LEFT, RIGHT, UP, DOWN:
            if vect is LEFT or vect is RIGHT:
                side = Rectangle(height=height, width=depth, **color_kwargs)
            else:
                side = Rectangle(height=depth, width=width, **color_kwargs)
...
class GraphOfXCubed(GraphScene):
    ...
    def construct(self):
        ...
        deriv_label = self.get_graph_label(deriv_graph, "\\frac{df}{dx}(x) = 3x^2", x_val=-3, direction=LEFT)
        ...
        for x_val in -2, -self.dx / 2, 2:
            self.animate_secant_slope_group_change(ss_group, target_x=x_val, run_time=2)
            if x_val != -self.dx / 2:
                v_line = self.get_vertical_line_to_graph(x_val, deriv_graph, line_class=DashedLine)
                self.play(ShowCreation(v_line))
...
class PatternForPowerRule(PiCreatureScene):
    ...
    def introduce_pattern(self):
        ...
        derivatives.arrange(DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF)
        ...
        for d1, d2 in zip(derivatives, derivatives[1:]):
            self.play(Transform(d1.copy(), d2, replace_mobject_with_target_in_scene=True))
        self.change_mode("thinking")
        self.wait()
    ...
    def generalize_pattern(self):
        ...
        power_rule = OldTex("\\frac{d (x^n)}{dx} = ", "nx^{n-1}")
        ...
        self.wait()
        self.play(Write(title), self.pi_creature.change_mode, "speaking")
        self.wait()
    ...
    def show_hopping(self):
        ...
        for exp, color in zip(exp_range, self.colors[1:]):
            form = OldTex("x^", str(exp), "\\rightarrow", str(exp), "x^", str(exp - 1))
            ...
            self.play(Write(lhs))
            self.play(lhs_copy.move_to, rhs, DOWN + LEFT, Write(arrow))
            self.wait()
            self.play(
                ApplyMethod(
                    lhs_copy[1].replace,
                ...
                FadeIn(form[5], rate_func=squish_rate_func(smooth, 0.5, 1)),
            )
            self.wait()
            self.play(
                self.pi_creature.change_mode,
...
class PowerRuleAlgebra(Scene):
    ...
    def construct(self):
        ...
        equals = OldTex("=")
        equals2 = OldTex("=")
        full_product = OldTex(*paren_strings * 3 + ["\\cdots"] + paren_strings)
        ...
        self.play(
            ...
            GrowFromCenter(product_brace, rate_func=squish_rate_func(smooth, 0.6, 1)),
            run_time=3,
        )
        self.wait()
    ...
    def workout_product(self, equals, full_product):
        ...
        xs, dxs = [
            VGroup(*[submob for submob, tex in product_part_tex_pairs if tex == target_tex])
        ...
        self.play(Write(equals), Transform(xs_copy, x_to_n))
        self.wait()
        brace, derivative_term = self.pull_out_linear_terms(x_to_n, product_part_tex_pairs, xs, dxs)
        self.wait()
    ...
    def pull_out_linear_terms(self, x_to_n, product_part_tex_pairs, xs, dxs):
        ...
        for dx_index, dx in enumerate(dxs):
            if dx is dxs[-1]:
                ...
                h_dots_list = [submob for submob, tex in product_part_tex_pairs if tex == "\\cdots"]
            ...
            target_list.sort(key=lambda m: m.get_center()[0])
            dots = OldTex("+", ".", ".", "\\dots")
            ...
            self.play(MoveToTarget(dx_copy), MoveToTarget(xs_copy), Write(dots))
        ...
        VGroup(brace, all_linear_terms.target).shift(
            x_to_n[0].get_right() + MED_LARGE_BUFF * RIGHT - compact[0].get_left()
        )
...
class OneOverX(PiCreatureScene, GraphScene):
    ...
    def introduce_function(self):
        ...
        self.play(Write(func), self.pi_creature.change_mode, "pondering")
        self.wait()
        self.play(
            recip_copy.next_to,
            self.pi_creature,
    ...
    def introduce_graph(self):
        ...
        graph = self.get_graph(lambda x: 1.0 / x)
    ...
    def perform_nudge(self):
        ...
        new_rect = self.get_rectangle(self.start_x_value + self.dx)
        ...
        h_lines = VGroup(
            *[
                DashedLine(
                    ORIGIN,
                    (rect_copy.get_width() + LARGE_BUFF) * RIGHT,
                    color=self.df_color,
                    stroke_width=2,
        ...
        v_lines = VGroup(
            *[
                DashedLine(
                    ORIGIN,
                    (rect_copy.get_height() + MED_LARGE_BUFF) * UP,
                    color=self.dx_color,
                    stroke_width=2,
        ...
        negative_arrow = Arrow(negative.get_left(), df_label.get_corner(UP + RIGHT), color=RED)
        ...
        for color, point_pair in zip([self.dx_color, self.df_color], point_pairs):
            area_change_rect = Rectangle(fill_opacity=1, fill_color=color, stroke_width=0)
            area_change_rect.replace(VGroup(*list(map(VectorizedPoint, point_pair))), stretch=True)
        ...
        area_gained_label.next_to(rect_copy.get_corner(DOWN + RIGHT), UP + LEFT, buff=SMALL_BUFF)
        area_gained_arrow = Arrow(area_gained_label.get_top(), area_gained.get_center(), buff=0, color=WHITE)

        area_lost_label = OldTexText("Area lost")
        area_lost_label.scale(0.75)
        area_lost_label.next_to(rect_copy.get_left(), RIGHT)
        area_lost_arrow = Arrow(area_lost_label.get_top(), area_lost.get_center(), buff=0, color=WHITE)

        question = OldTex("\\frac{d(1/x)}{dx} = ???")
        question.next_to(
        ...
        self.play(GrowFromCenter(dx_brace), *list(map(ShowCreation, v_lines)))
        self.wait()
        self.play(GrowFromCenter(df_brace), *list(map(ShowCreation, h_lines)))
        ...
        self.play(Write(negative), ShowCreation(negative_arrow))
        self.wait()
        self.play(Write(question), self.pi_creature.change_mode, "raise_right_hand")
        self.wait(2)
    ...
    def create_pi_creature(self):
        ...
        morty.scale(self.morty_scale_val, about_point=morty.get_corner(DOWN + RIGHT))
        return morty
    ...
    def get_rectangle_group(self, x, x_label="x", one_over_x_label="\\frac{1}{x}"):
        result = VGroup()
        result.x_val = x
        result.rectangle = self.get_rectangle(x)

        result.x_brace, result.recip_brace = braces = [Brace(result.rectangle, vect) for vect in (UP, RIGHT)]
    ...
    def change_rectangle_group(
        ...
        **anim_kwargs,
        ...
        labels_transform = Transform(rect_group.labels, target_group.labels)
...
class AskRecipriocalQuestion(Scene):
    def construct(self):
        tex = OldTex("(\\text{What number?})", "\\cdot x = 1")
...
class SquareRootOfX(Scene):
    ...
    def introduce_square(self):
        square = Square(side_length=self.square_width, **self.square_color_kwargs)
        square.move_to(self.square_corner, UP + LEFT)
        area_label = OldTexText("Area $ = x$")
        area_label.move_to(square)

        bottom_brace, right_brace = braces = VGroup(*[Brace(square, vect) for vect in (DOWN, RIGHT)])
        for brace in braces:
            brace.add(brace.get_text("$\\sqrt{x}$"))

        self.play(DrawBorderThenFill(square), Write(area_label))
    ...
    def nudge_square(self):
        ...
        little_braces = VGroup(
            *[Brace(VGroup(*line_pair), vect, buff=0) for line_pair, vect in [(lines[:2], RIGHT), (lines[2:], DOWN)]]
        )
        ...
        question.next_to(area_increase, DOWN, aligned_edge=LEFT, buff=LARGE_BUFF)
        ...
        self.play(*it.chain(list(map(ShowCreation, lines)), list(map(FadeIn, little_braces))))
...
class DerivativeOfSineIsSlope(Scene):
    def construct(self):
        tex = OldTex("\\frac{d(\\sin(\\theta))}{d\\theta} = ", "\\text{Slope of this graph}")
...
class IntroduceUnitCircleWithSine(GraphScene):
    ...
    def draw_example_radians(self):
        ...
        line = Line(
            ORIGIN,
            self.example_radians * self.unit_length * UP,
            color=YELLOW,
        )
        ...
        self.wait()
        self.play(line.move_to, radial_line.get_end(), DOWN, FadeOut(brace))
        self.play(ReplacementTransform(line, arc))
        self.wait()
        self.play(Rotate(radial_line, self.example_radians), ShowCreation(arc_copy))
        self.wait()
        arc_copy.generate_target()
        arc_copy.target.scale(0.2)
        theta_copy.generate_target()
        theta_copy.target.next_to(arc_copy.target, RIGHT, aligned_edge=DOWN, buff=SMALL_BUFF)
    ...
    def walk_around_circle(self):
        ...
        def v_line_update(v_line):
            drop_point = radial_line.get_end()[0] * RIGHT
            v_line.put_start_and_end_on(radial_line.get_end(), drop_point)
            return v_line
        ...
        for x in range(5):
            self.play(
                Rotate(radial_line, 2 * np.pi),
                UpdateFromFunc(v_line, v_line_update),
                run_time=1.0 / self.rotations_per_second,
                rate_func=linear,
            )
...
class DerivativeIntuitionFromSineGraph(GraphScene):
    ...
    def draw_derivative_from_slopes(self):
        ...
        v_line = DashedLine(self.graph_origin, self.coords_to_point(0, 1), color=RED)
        ...
        while last_theta < self.x_max:
            ...
            self.animate_secant_slope_group_change(
                ...
                added_anims=[
                    ShowCreation(
                        deriv_copy,
                        rate_func=lambda t: interpolate(last_theta / self.x_max, next_theta / self.x_max, smooth(t)),
                        run_time=3,
                    ),
                    UpdateFromFunc(v_line, lambda v: self.v_line_update(v, deriv_copy), run_time=3),
                ],
            )
            self.wait()
            if next_theta == 2 * np.pi:
                ...
                words.to_edge(UP)
                arrow = Arrow(words.get_bottom(), deriv_graph.point_from_proportion(0.45))
    ...
    def alter_derivative_graph(self):
        ...
        for func in func_list:
            ...
            self.play(Transform(self.deriv_graph, new_graph), run_time=2)
            self.wait()
...
class DerivativeFromZoomingInOnSine(IntroduceUnitCircleWithSine, ZoomedScene):
    ...
    def zoom_in(self):
        ...
        self.play(*list(map(FadeIn, [self.little_rectangle, self.big_rectangle])))
        self.play(
    ...
    def perform_nudge(self):
        ...
        d_theta_brace = Brace(d_theta_arc, rotate_vector(RIGHT, self.example_radians))
        d_theta_label = OldTex("d\\theta")
        d_theta_label.next_to(d_theta_brace.get_center(), d_theta_brace.direction, MED_LARGE_BUFF)
        d_theta_label.set_color(d_theta_arc.get_color())

        group = VGroup(d_theta_arc, d_theta_brace, d_theta_label)
        group.scale(1.0 / self.zoom_factor)
        ...
        h_line = DashedLine(interim_point, point, dash_length=0.01)
        ...
        self.play(GrowFromCenter(d_theta_brace), FadeIn(d_theta_label))
        self.wait()
        self.play(ShowCreation(h_line), ShowCreation(d_sine_line))
        self.play(GrowFromCenter(d_sine_brace), Write(d_sine))
        self.wait()

        self.little_triangle = Polygon(nudged_point, point, interim_point)
    ...
    def show_similar_triangles(self):
        ...
        new_angle_label.scale(little_triangle.get_width() / big_triangle.get_height())
        ...
        little_triangle_lines = VGroup(
            *[
                ...
                for pair in [(DOWN + RIGHT, UP + LEFT), (UP + LEFT, DOWN + LEFT)]
            ]
        )
        ...
        self.play(
            little_triangle.scale,
            2,
            little_triangle.get_corner(DOWN + RIGHT),
            little_triangle.set_color,
            YELLOW,
            rate_func=there_and_back,
        )
        self.wait()
    ...
    def analyze_ratios(self):
        ...
        group.arrange()
        group.next_to(self.title, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *[ApplyMethod(pi.look_at, next_video) for pi in self.get_pi_creatures()],
        )
        self.play(next_video.set_color, YELLOW, next_video.shift, MED_LARGE_BUFF * DOWN)
        self.wait()
        for mob in group:
            self.play(
                Write(mob, run_time=1),
                *[ApplyMethod(pi.look_at, mob) for pi in self.get_pi_creatures()],
            )
        self.wait(3)
...
class Thumbnail(NudgeSideLengthOfCube):
    def construct(self):
        ...
        formula = OldTex("\\frac{d(x^3)}{dx} = 3x^2")
