# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\footnote.py
import math
...
class Introduce(TeacherStudentsScene):
    def construct(self):
        ...
        derivs = VGroup(
            *[OldTex("{d", "^%d" % n, "f \\over dx", "^%d}" % n).set_color_by_tex(str(n), YELLOW) for n in range(2, 5)]
        )
        ...
        arrow = Arrow(taylor_series, card_dot.target, buff=MED_SMALL_BUFF, color=WHITE)
...
class SecondDerivativeGraphically(GraphScene):
    ...
    def draw_f(self):
        ...
        self.play(
            ...
            Write(graph_label, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)),
        )
        self.wait()
    ...
    def show_derivative(self):
        ...
        ss_group = self.get_secant_slope_group(1, self.graph, dx=0.01, secant_line_color=self.deriv_color)

        self.play(Write(deriv), *list(map(ShowCreation, ss_group)))
        self.animate_secant_slope_group_change(ss_group, target_x=self.x3, run_time=5)
        self.wait()
        self.animate_secant_slope_group_change(ss_group, target_x=self.x2, run_time=3)
        self.wait()
    ...
    def write_second_derivative(self):
        ...
        points = [self.input_to_graph_point(x, self.graph) for x in (self.x2, self.x3)]
        ...
        self.play(
            Write(words),
            ShowCreation(arrows[0], rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=2,
        )
        self.animate_secant_slope_group_change(
            ...
            run_time=3,
            added_anims=[
                Transform(*arrows, run_time=3, path_arc=0.75 * np.pi),
            ],
        )
    ...
    def show_curvature(self):
        ...
        self.animate_secant_slope_group_change(self.ss_group, target_x=self.x1, run_time=0)
    ...
    def contrast_big_and_small_concavity(self):
        ...
        arg_rhs_list = [OldTex("(", str(x0), ")", "=", str(rhs)) for rhs in (10, 0.4, 0)]
        for graph, arg_rhs in zip(graphs, arg_rhs_list):
            graph.ss_group = self.get_secant_slope_group(x0 - 1, graph, dx=0.001, secant_line_color=YELLOW)
        ...
        self.play(Write(input_label), ShowCreation(v_line))
        self.play(
            ReplacementTransform(input_label.copy(), graph.arg_rhs.get_part_by_tex(str(x0))),
            self.second_deriv.next_to,
            graph.arg_rhs.copy(),
            LEFT,
            SMALL_BUFF,
            Write(
                VGroup(*[submob for submob in graph.arg_rhs if submob is not graph.arg_rhs.get_part_by_tex(str(x0))])
            ),
        )
        self.wait()
...
class IntroduceNotation(TeacherStudentsScene):
    def construct(self):
        clunky_deriv = OldTex("{d", "\\big(", "{df", "\\over", "dx}", "\\big)", "\\over", "dx }")
        over_index = clunky_deriv.index_of_part(clunky_deriv.get_parts_by_tex("\\over")[1])
        ...
        dfs, overs, dxs = list(map(clunky_deriv.get_parts_by_tex, ["df", "over", "dx"]))
        ...
        self.student_says("What's that notation?", target_mode="raise_left_hand")
        self.play_student_changes("confused", "raise_left_hand", "confused")
        self.play(
            FadeIn(clunky_deriv, run_time=2, lag_ratio=0.5),
        ...
        self.wait()
        self.play(FadeOut(denominator.circle), Write(dx_to_zero), dxs.set_color, YELLOW)
        self.wait()
        self.play(
            FadeOut(dx_to_zero),
        ...
        self.play(
            df_over_dx.scale,
            dxs[1].get_height() / dxs[0].get_height(),
            df_over_dx.move_to,
            d_over_dx,
            RIGHT,
        ...
        self.play(
            *[
                ReplacementTransform(group, VGroup(d2f_over_dx2.get_part_by_tex(tex)))
                for group, tex in [
                    (VGroup(d, dfs[0]), "d^2"),
                    (overs, "over"),
                    (dxs, "dx"),
...
class HowToReadNotation(GraphScene, ReconfigurableScene):
    ...
    def add_graph(self):
        ...
        graph_label = self.get_graph_label(graph, "f(x)", direction=LEFT, x_val=3.3)
    ...
    def take_two_steps(self):
        v_lines = [
            self.get_vertical_line_to_graph(self.x + i * self.dx, self.graph, line_class=DashedLine, color=WHITE)
            for i in range(3)
        ]
        braces = [Brace(VGroup(*v_lines[i : i + 2]), buff=0) for i in range(2)]
        for brace in braces:
            brace.dx = OldTex("dx")
    ...
    def show_dfs(self):
        ...
        for i, v_line1, v_line2 in zip(it.count(1), self.v_lines, self.v_lines[1:]):
            dx_line = Line(v_line1.get_bottom(), v_line2.get_bottom(), color=GREEN)
            dx_line.move_to(v_line1.get_top(), LEFT)
            dx_lines.add(dx_line)

            df_line = Line(dx_line.get_right(), v_line2.get_top(), color=YELLOW)
        ...
        for brace, dx_line, df_line in zip(self.braces, dx_lines, df_lines):
            ...
            self.play(ShowCreation(df_line.arrow), Write(df_line.label))
            self.wait(2)
    ...
    def show_ddf(self):
        ...
        df_dx_groups.target.arrange(RIGHT, buff=MED_LARGE_BUFF, aligned_edge=DOWN)
        df_dx_groups.target.next_to(self.df_dx_groups, RIGHT, buff=3, aligned_edge=DOWN)
        ...
        for group, label in zip(df_dx_groups.target, df_labels.target):
            ...
            h_line.move_to(
                group.get_corner(UP + RIGHT)[1] * UP + df_dx_groups.target.get_right()[0] * RIGHT,
                RIGHT,
            )
            h_lines.add(h_line)
        ...
        ddf.scale(df_labels[0].get_height() / ddf.get_height(), about_point=ddf.get_right())
        ddf.set_color(MAROON_B)

        self.play(*list(map(MoveToTarget, [df_dx_groups, df_labels])), run_time=2)
    ...
    def show_proportionality_to_dx_squared(self):
        ...
        rhs = OldTex("\\approx", "(\\text{Some constant})", "(dx)^2")
        rhs.scale(0.8)
        rhs.next_to(ddf.target, RIGHT)

        example_dx = OldTex("dx = 0.01 \\Rightarrow (dx)^2 = 0.0001")
    ...
    def write_second_derivative(self):
        ddf_over_dx_squared = OldTex("{d(df)", "\\over", "(dx)^2}")
        ...
        self.play(
            ...
            ddf_over_dx_squared.get_part_by_tex("over").scale,
            0.8,
        )
        self.wait(2)
...
class SecondDerivativeAsAcceleration(Scene):
    ...
    def introduce_acceleration(self):
        a_words = OldTex("{d^2 s \\over dt^2}(t)", "\\Leftrightarrow", "\\text{Acceleration}")
    ...
    def show_functions(self):
        def get_deriv(n):
            return lambda x: derivative(s_scene.graph.underlying_function, x, n)
        ...
        s_graph, v_graph, a_graph, j_graph = graphs = [
            VGroup(*scene.get_top_level_mobjects()) for scene in (s_scene, v_scene, a_scene, j_scene)
        ]
        ...
        s_words = OldTex("s(t)", "\\Leftrightarrow", "\\text{Displacement}")
        s_words.set_color_by_tex("s(t)", s_scene.graph.get_color())
        v_words = OldTex("\\frac{ds}{dt}(t)", "\\Leftrightarrow", "\\text{Velocity}")
        v_words.set_color_by_tex("ds", v_scene.graph.get_color())
        j_words = OldTex("\\frac{d^3 s}{dt^3}(t)", "\\Leftrightarrow", "\\text{Jerk}")
        ...
        words_group.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        ...
        self.play(s_graph.scale, 0.3, s_graph.next_to, s_words, LEFT)
        self.play(*list(map(FadeIn, [v_graph, v_words])))
        self.wait(2)
        self.play(v_graph.scale, 0.3, v_graph.next_to, v_words, LEFT)
        self.wait(2)
        self.play(
        ...
        for x in range(2):
            self.show_car_movement(run_time=3, rate_func=lambda t: smooth(t / 2.0))
            self.wait()
    ...
    def show_car_movement(self, *added_anims, **kwargs):
        distance = get_norm(self.car.get_center() - self.start_car_copy.get_center())
        if distance > 1:
        ...
        self.play(MoveCar(self.car, self.line.get_right(), **kwargs), *added_anims)
