...
class IntroduceStolenNecklaceProblem(ThreeDScene):
    ...
    def introduce_necklace(self):
        ...
        self.play(
            ...
            *it.chain(*[[pi.change_mode, "conniving", pi.look_at, necklace] for pi in self.thieves]),
        )
    ...
    def divvy_with_n_cuts(
        ...
        if with_thieves:
            self.play(
                *it.chain(*[[thief.change_mode, "happy", thief.look_at, self.necklace] for thief in self.thieves])
            )
            self.play(Blink(self.thieves[1]))
        else:
            self.wait()
    ...
    def get_necklace(self, **kwargs):
        colors = reduce(
            op.add,
            [num * [color] for num, color in zip(self.num_per_jewel, self.jewel_colors)],
        )
        ...
        for slice_indices in it.combinations(list(range(1, len(jewels))), n_types):
            ...
            for binary_choices in all_binary_choices:
                ...
                left_sets, right_sets = [
                    [subset for subset, index in zip(subsets, binary_choices) if index == target_index]
...
class WalkEquatorPostTransform(GraphScene):
    ...
    def get_great_arc_images(self):
        curves = VGroup(
            *[
                ParametricCurve(lambda t: self.sphere_point(t, s)).apply_function(self.sphere_to_plane)
...
class MakeTwoJewelCaseContinuous(IntroduceStolenNecklaceProblem):
    ...
    def color_necklace(self):
        ...
        segment_types = VGroup(
            *[VGroup(*[m for m in segments if m.get_color() == Color(color)]) for color in self.jewel_colors]
        )
    ...
    def find_continuous_fair_division(self):
        ...
        cut_proportions = [
            sorted([random.random(), random.random()]) for x in range(self.num_continuous_division_searches)
        ]
    ...
    def set_color_continuous_groups(self):
        ...
        weight_description = VGroup(
            *[
                VGroup(
                    OldTex("\\frac{%d}{%d}" % (len(jewel_type) / 2, len(self.segments))),
...
class ChoicesInNecklaceCutting(ReconfigurableScene):
    ...
    def add_necklace(self):
        width, colors, num_per_color = [
            MakeTwoJewelCaseContinuous.CONFIG[key] for key in ["necklace_width", "jewel_colors", "num_per_jewel"]
        ]
    ...
    def show_three_numbers_adding_to_one(self):
        ...
        braces = [
            Brace(Line(p1 + SMALL_BUFF * RIGHT / 2, p2 + SMALL_BUFF * LEFT / 2)) for p1, p2 in zip(points, points[1:])
        ]
            ...
            self.play(
                *[
                ...
                + [
                    ...
                    for curr_arrow, arrow_pair, choice in zip(curr_arrows, arrow_pairs, binary_choices)
                ]
            )
            self.wait()
    ...
    def get_boxes_and_labels(self):
        ...
        labels = VGroup(
            *[OldTexText("Thief %d" % (i + 1)).next_to(box, UP, aligned_edge=RIGHT) for i, box in enumerate(boxes)]
        )
        return boxes, labels
    ...
    def wiggle_v_lines(self):
        self.play(
            *it.chain(
                *[[line.rotate, np.pi / 12, vect, line.set_color, RED] for line, vect in zip(self.v_lines, [OUT, IN])]
            ),
            rate_func=wiggle,
        )
...
class NecklaceDivisionSphereAssociation(ChoicesInNecklaceCutting):
    ...
    def show_binary_choice_association(self):
        ...
        for brace, denom, group, final_choice in quads:
            ...
            self.play(
                ReplacementTransform(VGroup(brace.label[0], brace, brace.label[1]), choices[0]),
                group.move_to,
...
class TotalLengthOfEachJewelEquals(NecklaceDivisionSphereAssociation, ThreeDScene):
    ...
    def find_fair_division(self):
        ...
        self.wait()
        self.play(
            *[ApplyMethod(group.move_to, group.target_points[choice]) for group, choice in zip(groups, binary_choice)]
        )
        self.wait()
...
class ShowFunctionDiagram(TotalLengthOfEachJewelEquals, ReconfigurableScene):
    ...
    def add_number_pair(self):
        ...
        t1_plane, t2_plane = planes = VGroup(
            *[VGroup(*plane_class.get_top_level_mobjects()) for plane_class in plane_classes]
        )
