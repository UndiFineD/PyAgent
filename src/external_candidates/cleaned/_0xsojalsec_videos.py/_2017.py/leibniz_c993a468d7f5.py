...
class Introduction(PiCreatureScene):
    ...
    def introduce_three_objects(self):
        ...
        self.add(morty)
        self.play(
            ReplacementTransform(morty.body, pi_group.get_part_by_tex("pi"), run_time=1),
...
class ShowSum(TeacherStudentsScene):
    ...
    def show_sum(self):
        ...
        arrows = [
            Arrow(p1, p2, tip_length=0.2 * min(1, get_norm(p1 - p2)), buff=0) for p1, p2 in zip(points, points[1:])
        ]
        dot = Dot(points[0])
...
class Outline(PiCreatureScene):
    ...
    def count_lattice_points(self):
        ...
        self.play(
            *[
                ...
                for dot, a in zip(lattice_points, np.linspace(0, 0.75, len(lattice_points)))
            ]
        )
    ...
    def show_last_step(self):
        expression = OldTex(
            "=",
            "\\frac{\\quad}{1}",
            *it.chain(*[["+", "\\frac{\\quad}{%d}" % d] for d in range(2, len(self.numerators) + 1)] + [["+ \\cdots"]]),
        )
...
class CountLatticePoints(LatticePointScene):
        ...
        self.play(
            *[
                ApplyMethod(
                    ...
                    0,
                    run_time=3,
                    rate_func=squish_rate_func(lambda t: 1 - there_and_back(t), a, a + 0.5),
                    remover=True,
                )
    ...
    def write_pi_R_squared(self):
        equations = VGroup(
            *[
                VGroup(
                    OldTexText("\\# Lattice points\\\\", "within radius ", R, alignment=""),
...
class CountThroughRings(LatticePointScene):
    ...
    def count_through_rings(self):
        counts = [
            len(self.get_lattice_points_on_r_squared_circle(N)) for N in range(self.max_lattice_point_radius**2 + 1)
        ]
        left_list = VGroup(
            *[
                OldTex("\\sqrt{%d} \\Rightarrow" % n, str(count))
                for n, count in zip(list(range(self.num_rings_to_show_explicitly)), counts)
            ]
        )
        left_counts = VGroup()
        left_roots = VGroup()
        ...
        self.play(
            FadeOut(left_roots),
            FadeIn(top_rect),
            *[ReplacementTransform(lc, VGroup(tc), path_arc=np.pi / 2) for lc, tc in zip(left_counts, top_list)],
        )
...
class IntroduceComplexConjugate(LatticePointScene):
    ...
    def write_points_with_complex_coords(self):
        ...
        ticks = VGroup(
            *[Line(ORIGIN, MED_SMALL_BUFF * RIGHT).move_to(self.plane.coords_to_point(0, y)) for y in y_range]
        )
        imag_coords = VGroup()
...
class NameGaussianIntegers(LatticePointScene):
    ...
    def restrict_to_one_circle(self):
        ...
        self.play(
            ...
            ShowCreation(circle),
            Rotating(radius, run_time=1, rate_func=smooth, about_point=self.plane_center),
...
class IntroduceGaussianPrimes(LatticePointScene, PiCreatureScene):
    ...
    def construct(self):
        ...
        labels = [
            OldTex(tex).add_background_rectangle() for tex in ("5", "2+i", "2-i", "-1+2i", "-1-2i", "-2-i", "-2+i")
        ]
...
class SummarizeCountingRule(Show125Circle):
    ...
    def talk_through_rules(self):
        ...
        self.wait()
        for choices, power in (
            (five_choices, five_power),
            (
                thirteen_choices,
                thirteen_power,
            ),
...
class IntroduceChi(FactorizationPattern):
    ...
    def add_define_chi_label(self):
        ...
        chi_expressions = VGroup(
            *[self.get_chi_expression(numbers, color) for numbers, color in zip(self.numbers_list, self.colors)]
        )
    ...
    def describe_values(self, index):
        ...
        self.wait()
        self.play(
            Write(VGroup(*[part for part in chi_expression if part not in chi_expression.inputs])),
            *[ReplacementTransform(label.copy(), num_mob) for label, num_mob in zip(labels, chi_expression.inputs)],
        )
        self.wait()
    ...
    def introduce_dots_arrows_and_labels(self, dots, arrows, labels):
        if hasattr(self, "last_dots"):
            ...
            self.play(
                ...
                LaggedStartMap(DrawBorderThenFill, dots, stroke_width=4, stroke_color=YELLOW),
                run_time=2,
            )
        self.last_dots = dots
...
class WriteCountingRuleWithChi(SummarizeCountingRule):
    ...
    def write_chi_expression(self):
        ...
        self.play(
            *[
                ApplyMethod(c_part.move_to, e_part, LEFT, path_arc=-np.pi / 2, run_time=2)
...
class CountLatticePointsInBigCircle(LatticePointScene):
    ...
    def introduce_points(self):
        ...
        self.play(
            ...
            Write(VGroup(*[part for part in pi_R_squared if part is not pi_R_squared.get_part_by_tex("R")])),
        )
        self.remove(R_group)
...
class AddUpGrid(Scene):
    ...
    def add_row_lines(self):
        ...
        row_lines = VGroup(
            *[h_line.copy().next_to(radical, DOWN, buff=SMALL_BUFF, aligned_edge=LEFT) for radical in self.radicals]
        )
        ...
        for circle in circles:
            for word in circle.words:
                self.play(
                    ...
                    self.teacher.change,
                    "speaking",
                    *[ApplyMethod(pi.change, "pondering") for pi in self.get_students()],
                )
                self.wait()
