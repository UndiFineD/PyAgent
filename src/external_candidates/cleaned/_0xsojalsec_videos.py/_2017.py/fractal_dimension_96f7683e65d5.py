...
class ConstrastSmoothAndFractal(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            ApplyMethod(smooth_britain.scale, self.scale_factor, smooth_britain.zoom_point),
            Animation(v_line),
...
class FourSelfSimilarShapes(Scene):
    ...
    def get_shapes(self, titles):
        ...
        square = VGroup(
            *[Square().next_to(ORIGIN, vect, buff=0) for vect in compass_directions(start_vect=DOWN + LEFT)]
        )
...
class DefineSierpinskiDimension(DefineTwoDimensional):
    ...
    def isolate_equation(self):
        ...
        self.play(
            *[ApplyMethod(mob.fade, 0.5) for mob in self.get_mobjects()] + [Animation(mob) for mob in keepers_copies]
        )
...
class DimensionOfKoch(Scene):
    ...
    def break_up_curve(self):
        ...
        broken_curve = VGroup(
            *[VGroup(*curve_copy[i * length / n_parts : (i + 1) * length / n_parts]) for i in range(n_parts)]
        )
    ...
    def show_dimension(self):
        ...
        log_expression = OldTex(
            "D = \\log_%d(%d) %s" % (self.scaling_factor, self.mass_scaling_factor, self.dimension_rhs)
        )
...
class DimensionOfQuadraticKoch(DimensionOfKoch):
    ...
    def set_color_curve_subparts(self):
        ...
        broken_curve = VGroup(
            *[VGroup(*colored_curve[i * length / n_parts : (i + 1) * length / n_parts]) for i in range(n_parts)]
        )
...
class ShowDiskScaling(Scene):
    ...
    def try_fitting_small_disks(self):
        ...
        foursome = VGroup(
            *[disk.copy().next_to(ORIGIN, vect, buff=0) for vect in compass_directions(start_vect=UP + RIGHT)]
        )
...
class BoxCountingScene(Scene):
    ...
    def get_grid(self):
        ...
        h_lines = VGroup(
            *[h_line.copy().shift(u * y * self.box_width * UP) for y in range(self.num_rows / 2 + 1) for u in [-1, 1]]
        )
...
class DifferentSlopesAtDifferentScales(IntroduceLogLogPlot):
    def construct(self):
        ...
        data_points = [
            self.input_to_graph_point(x) + (0.3 * (random.random() - 0.5)) * UP for x in np.arange(1, self.x_max, 0.7)
        ]
        data_dots = VGroup(*[Dot(point, radius=0.05, color=YELLOW) for point in data_points])
...
class FromHandwavyToQuantitative(Scene):
    def construct(self):
        ...
        self.play(Blink(morty))
        self.play(
            PiCreatureSays(morty, "We can make \\\\ that quantitative!", target_mode="hooray"),
