# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\triples.py
import fractions
    ...
    def construct(self):
        ...
        for a, b, c in triples:
            ...
            for n, color in zip([a, b, c], SIDE_COLORS):
                square_group = VGroup(*[square.copy().shift(x * RIGHT + y * UP) for x in range(n) for y in range(n)])
            ...
            c_square.rotate(hyp_line.get_angle(), about_point=hyp_line.get_center())
            if c in [5, 13, 25]:
                if c == 5:
            ...
            equation = OldTex(str(a), "^2", "+", str(b), "^2", "=", str(c), "^2")
            ...
            self.play(FadeIn(triangle), self.teacher.change_mode, "raise_right_hand")
            ...
            for start, target in zip([a_square, b_square], c_square_parts):
                ...
                self.play(ReplacementTransform(mover, target, run_time=2, path_arc=np.pi / 2))
...
class CompareToFermatsLastTheorem(TeacherStudentsScene):
    def construct(self):
        expressions = [OldTex("a", "^%d" % d, "+", "b", "^%d" % d, "=", "c", "^%d" % d) for d in range(2, 9)]
        ...
        top_text = top_brace.get_text("Abundant integer solutions", buff=SMALL_BUFF)
        low_brace = Brace(low_expression, DOWN, buff=SMALL_BUFF)
        low_text = low_brace.get_text("No integer solutions", buff=SMALL_BUFF)
        ...
        self.play(
            ReplacementTransform(square_expression.copy(), low_expression),
            self.teacher.change_mode,
            "raise_right_hand",
            *[ApplyMethod(pi.change, "confused", expressions[1]) for pi in self.get_students()],
        )
        self.wait()
...
class ShowManyTriples(Scene):
    def construct(self):
        ...
        for i, (a, b, c) in enumerate(triples):
            ...
            title = OldTex(str(a), "^2", "+", str(b), "^2", "=", str(c), "^2")
            for num, color in zip([a, b, c], SIDE_COLORS):
        ...
        for i in range(1, 17):
            ...
            if i < 4:
                ...
                self.play(Transform(triangle, new_triangle), FadeIn(new_title.target))
                self.wait()
        self.play(FadeOut(triangle))
        self.play(LaggedStartMap(FadeIn, VGroup(*[title.target for title in titles[17:]]), run_time=5))
...
class BabylonianTablets(Scene):
    def construct(self):
        ...
        for a, c in ac_pairs:
            ...
            tex = "%s^2 + %s^2 = %s^2" % tuple(map("{:,}".format, [a, b, c]))
            tex = tex.replace(",", "{,}")
            triple = OldTex(tex)
            triples.add(triple)
...
class AskAboutFavoriteProof(TeacherStudentsScene):
    def construct(self):
        self.student_says("What's you're \\\\ favorite proof?", target_mode="raise_right_hand")
...
class PythagoreanProof(Scene):
    ...
    def show_proof(self):
        ...
        self.wait()
        self.add_new_triangles(triangle, self.get_added_triangles_to_c_square(triangle, c_square))
        self.wait()
        ...
        big_squares = VGroup(*list(map(self.get_big_square, [triangle, triangle_copy])))
        negative_space_words = OldTexText("Same negative \\\\ space")
    ...
    def get_big_square(self, triangle):
        ...
        square.replace(VGroup(triangle, triangle.added_triangles), stretch=True)
        square.scale(1.01)
        return square
    ...
    def get_added_triangles_to_c_square(self, triangle, c_square):
        return VGroup(*[triangle.copy().rotate(i * np.pi / 2, about_point=c_square.get_center()) for i in range(1, 4)])
...
class ReframeOnLattice(PiCreatureScene):
    ...
    def wander_over_lattice_points(self):
        ...
        self.integer_distance_dot_tuple_groups = VGroup(*dot_tuple_groups[len(initial_examples) :])
    ...
    def show_whole_distance_examples(self):
        ...
        for dot_tuple_group in dot_tuple_groups:
            ...
            hyp_label.next_to(triangle.get_center(), UP + LEFT, buff=SMALL_BUFF)
            line.add(hyp_label)
    ...
    def resize_plane(self):
        ...
        self.plane.target.shift(new_plane.coords_to_point(0, 0) - self.plane.target.coords_to_point(0, 0))
        self.plane.target.scale(self.new_unit_size / self.initial_unit_size)
        coordinate_labels = self.plane.coordinate_labels
        for coord in coordinate_labels:
            x = int(coord.get_tex())
            coord.generate_target()
            coord.target.scale(1.5)
            coord.target.next_to(new_plane.coords_to_point(x, 0), DOWN, buff=SMALL_BUFF)
    ...
    def mention_squaring_it(self):
        ...
        square_label.next_to(arrow.point_from_proportion(0.5), RIGHT, buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(RemovePiCreatureBubble(morty, target_mode="pondering", look_at=self.example_label))

    def work_out_square_algebraically(self):
        rect = Rectangle(height=3.5, width=6.5, stroke_width=0, fill_color=BLACK, fill_opacity=0.8)
        rect.to_corner(UP + LEFT, buff=0)
        top_line = OldTex("(2+i)", "(2+i)")
        top_line.next_to(rect.get_top(), DOWN)
        second_line = OldTex("2^2 + 2i + 2i + i^2")
        second_line.next_to(top_line, DOWN, MED_LARGE_BUFF)
        final_line = OldTex("3 + 4i")
        final_line.next_to(second_line, DOWN, MED_LARGE_BUFF)

        result_dot = Dot(self.plane.coords_to_point(3, 4), color=MAROON_B, radius=self.dot_radius)

        self.play(
            FadeIn(rect),
            ReplacementTransform(VGroup(self.example_label[1].copy()), top_line),
            run_time=2,
        )
        self.wait()
        ...
        for index_alignment in index_alignment_lists:
            self.play(
                *[ReplacementTransform(second_line[i].copy(), final_line[j], run_time=1.5) for i, j in index_alignment]
            )
            self.wait()
        ...
        self.play(DrawBorderThenFill(result_dot, stroke_width=4, stroke_color=PINK))
        self.wait(2)
    ...
    def walk_through_square_geometrically(self):
        ...
        arc = Arc(angle=line.get_angle(), radius=0.7, color=WHITE)
        double_arc = Arc(
        ...
        self.wait()
        self.play(ShowCreation(arc), Write(theta))
        self.wait()
    ...
    def get_all_plane_dots(self):
        x_min, y_min = list(
            map(
                int,
                self.plane.point_to_coords(FRAME_X_RADIUS * LEFT + FRAME_Y_RADIUS * DOWN),
            )
        )
        x_max, y_max = list(
            map(
                int,
                self.plane.point_to_coords(FRAME_X_RADIUS * RIGHT + FRAME_Y_RADIUS * UP),
            )
        )
        result = VGroup(
            *[
...
class OneMoreExample(Scene):
    ...
    def add_point(self):
        ...
        self.play(FadeIn(number_label), ShowCreation(line), DrawBorderThenFill(dot))
    ...
    def square_algebraically(self):
        ...
        second_line = OldTex("\\big( 3^2 + (2i)^2 \\big) + " + "\\big(3 \\cdot 2 + 2 \\cdot 3 \\big)i")
        for i in 1, 12, 18:
        ...
        self.play(FadeIn(rect), Transform(VGroup(number), top_line), run_time=2)
        self.wait()
        ...
        for index_alignment in index_alignment_lists[:2]:
            self.play(
                *[
                    ReplacementTransform(top_line[i][j].copy(), second_line[k], run_time=1.5)
                    for i, j, k in index_alignment
                ]
            )
            self.wait()
    ...
    def plot_result(self):
        ...
        self.play(
            *[
                ...
                for m1, m2 in [(self.line, line), (self.distance_label, distance_label)]
            ]
        )
        self.wait()

    def show_triangle(self):
        triangle = Polygon(*[self.plane.coords_to_point(x, y) for x, y in [(0, 0), (5, 0), (5, 12)]])
        triangle.set_stroke(WHITE, 1)
        triangle.set_fill(BLUE, opacity=0.75)

        self.play(
            FadeIn(triangle),
            Animation(VGroup(self.line, self.dot, self.number_label[1], *self.distance_label[1:])),
            run_time=2,
        )
        self.wait(2)
...
class ThisIsMagic(TeacherStudentsScene):
    def construct(self):
        self.student_says("This is magic", target_mode="hooray")
...
class GeneralExample(OneMoreExample):
    ...
    def square_point(self):
        ...
        result_length_label.next_to(square_line.get_center(), self.result_label_vect)
        ...
        z_to_z_squared.next_to(arrow.point_from_proportion(0.5), RIGHT, MED_SMALL_BUFF)
        z_to_z_squared.add_to_back(
            BackgroundRectangle(VGroup(z_to_z_squared[2][0], *z_to_z_squared[:-1])),
        ...
        self.play(
            *[
                ReplacementTransform(start.copy(), target, path_arc=np.pi / 2, run_time=1.5)
                for start, target in [
                    (dot, square_dot),
                    (line, square_line),
                    (label, square_label),
...
class WriteGeneralFormula(GeneralExample):
    ...
    def show_squaring(self):
        ...
        self.play(
            *[
                ReplacementTransform(start.copy(), target, run_time=1.5, path_arc=np.pi / 2)
                for start, target in [
                    (dot, square_dot),
                    (line, square_line),
                    (label, square_label),
    ...
    def expand_square(self):
        ...
        top_line.next_to(rect.get_top(), DOWN)
        second_line = OldTex("\\big(", "u^2 - v^2", "\\big)", "+", "\\big(", "2uv", "\\big)", "i")
        for i, j in (1, 0), (5, 1):
        ...
        self.play(
            FadeOut(
                VGroup(
                    ...
                    self.example_line,
                    self.z_to_z_squared,
                    self.z_to_z_squared_arrow,
                )
            ),
            imag_part.copy().next_to,
            imag_part_line.copy(),
            RIGHT,
            SMALL_BUFF,
    ...
    def show_uv_to_triples(self):
        ...
        uv_title.next_to(h_line.point_from_proportion(1.0 / 6), UP, SMALL_BUFF)
        triple_title.next_to(h_line.point_from_proportion(2.0 / 3), UP, SMALL_BUFF)
        ...
        self.play(*list(map(FadeIn, [rect, h_line, v_line, uv_title, triple_title])))
        self.play(*[LaggedStartMap(FadeIn, mob, run_time=5, lag_ratio=0.2) for mob in (pair_mobs, triple_mobs)])
...
class VisualizeZSquared(Scene):
    ...
    def add_plane(self):
        ...
        coordinate_labels = VGroup()
        z_list = np.append(self.x_label_range, complex(0, 1) * np.array(self.y_label_range))
        for z in z_list:
            if z == 0:
                continue
            if z.imag == 0:
    ...
    def draw_arrows(self):
        ...
        dots = VGroup()
        for z in z_list:
            z_point, square_point, mid_point = [self.background_plane.number_to_point(z**p) for p in (1, 2, 1.5)]
            ...
            self.play(ReplacementTransform(z_dot.copy(), square_dot, path_arc=angle))
            self.play(FadeIn(square_label))
            self.wait()
            self.play(FadeOut(z_label), FadeOut(square_label), Animation(arrow))

            arrows.add(arrow)
            dots.add(z_dot, square_dot)
        self.wait()
        self.play(*list(map(FadeOut, [dots, arrows, self.z_to_z_squared])))

    def draw_dots(self):
        min_corner, max_corner = [
            self.background_plane.point_to_coords(u * FRAME_X_RADIUS * RIGHT + u * FRAME_Y_RADIUS * UP) for u in (-1, 1)
        ]
    ...
    def add_colored_grid(self):
        ...
        self.play(
            self.background_planes.set_stroke,
            None,
            1,
            LaggedStartMap(FadeIn, color_grid, run_time=2),
            Animation(self.dots),
        )
        self.wait()
    ...
    def apply_transformation(self):
        ...
        self.play(
            ...
            *[UpdateFromAlphaFunc(dot, update_dot) for dot in self.dots],
            run_time=3,
        )
        self.wait(2)
    ...
    def show_triangles(self):
        z_list = [complex(u, v) ** 2 for u, v in [(2, 1), (3, 2), (4, 1)]]
    ...
    def zoom_out(self):
        ...
        for mover in movers:
            ...
            mover.target.scale(scale_factor, about_point=self.plane_center)
        for dot in self.dots.target:
            dot.scale(1.0 / scale_factor)
        self.background_plane.target.fade()

        self.revert_to_original_skipping_status()
        self.play(*list(map(MoveToTarget, movers)), run_time=3)
        self.wait(2)

    def show_more_triangles(self):
        z_list = [complex(u, v) ** 2 for u in range(4, 7) for v in range(1, u)]
        triangles = self.get_triangles(z_list)
        triangle = triangles[0]
    ...
    def get_color_grid(self):
        ...
        color_grids.set_color_by_gradient(*[GREEN, RED, MAROON_B, TEAL] * 2)
    ...
    def get_triangles(self, z_list):
        ...
        for z in z_list:
            ...
            b = int(z.imag)
            c = int(abs(z))
            a_label, b_label, c_label = labels = [OldTex(str(num)) for num in (a, b, c)]
...
class AskAboutHittingAllPoints(TeacherStudentsScene):
    def construct(self):
        self.student_says("Does this hit \\\\ all pythagorean triples?", target_mode="raise_left_hand")
        self.wait()
...
class PointsWeMiss(VisualizeZSquared):
    ...
    def add_dots(self):
        z_list = [complex(x, y) ** 2 for x in self.dot_x_range for y in self.dot_y_range]
        dots = VGroup(
            *[
                Dot(
    ...
    def show_missing_point(self):
        ...
        points = list(map(self.background_plane.number_to_point, z_list))
        dots = VGroup(*list(map(Dot, points)))
        for dot in dots[:2]:
            dot.set_stroke(RED, 4)
            dot.set_fill(opacity=0)
        labels = VGroup(*[OldTex(complex_string_with_i(z)) for z in z_list])
        ...
        for i in 1, 2:
            self.play(*[Transform(group[0], group[i]) for group in groups])
            self.wait(3)
        self.play(*[FadeOut(group[0]) for group in groups])
    ...
    def mention_one_half_rule(self):
        ...
        self.play(FadeIn(morty))
        self.play(PiCreatureSays(morty, "Never need to scale \\\\ by less than $\\frac{1}{2}$"))
        self.play(Blink(morty))
        self.wait(2)
...
class PointsWeMissAreMultiplesOfOnesWeHit(TeacherStudentsScene):
    def construct(self):
        words = OldTexText("Every point we", "miss", "is \\\\ a multiple of one we", "hit")
...
class DrawSingleRadialLine(PointsWeMiss):
    ...
    def draw_line(self):
        ...
        added_dots = VGroup(*[Dot(self.background_plane.coords_to_point(3 * k, 4 * k)) for k in (2, 3, 5)])
...
class DrawRadialLines(PointsWeMiss):
    ...
    def resize_plane(self):
        everything = VGroup(*self.get_top_level_mobjects())
        everything.scale(self.final_unit_size / self.initial_unit_size, about_point=self.plane_center)
        self.background_plane.set_stroke(width=1)
        ...
        for dot in [first_dot] + list(self.dots):
            ...
            for k in range(50):
                ...
                if abs(point[0]) > FRAME_X_RADIUS or abs(point[1]) > FRAME_Y_RADIUS:
                    continue
                new_dot = Dot(point, color=GREEN, radius=self.big_dot_radius)
                new_dots.add(new_dot)
            line = Line(self.plane_center, dot.get_center())
            line.scale(FRAME_WIDTH / line.get_length(), about_point=self.plane_center)
    ...
    def show_single_line(self):
        ...
        self.play(dot.scale, 2, dot.set_color, RED)
    ...
    def show_all_lines(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, seed_dots, run_time=2))
        self.play(ReplacementTransform(seed_dots, self.lines, run_time=3, lag_ratio=0.5))
        self.play(
            LaggedStartMap(
                DrawBorderThenFill,
                new_dots,
...
class RationalPointsOnUnitCircle(DrawRadialLines):
    ...
    def show_rational_points_on_unit_circle(self):
        ...
        for x, y in coord_list:
            ...
            point = self.background_plane.coords_to_point(x / norm, y / norm)
    ...
    def divide_by_c_squared(self):
        top_line = OldTex("a", "^2", "+", "b", "^2", "=", "c", "^2 \\phantom{1}")
        ...
        circle_label = OldTexText("All $x+yi$ where \\\\", "$x^2 + y^2 = 1$")
    ...
    def from_rational_point_to_triple(self):
        ...
        self.play(ReplacementTransform(self.curr_example_point_group, rational_point_group))
        self.wait(2)
        self.play(
            *[
                ApplyMethod(mob.scale_about_point, scale_factor, self.plane_center)
                for mob in [
                    self.background_plane,
                    self.coordinate_labels,
                    self.unit_circle,
        ...
        # mimic_group
        point = self.background_plane.coords_to_point(*rational_point_group.coords)
        dot = Dot(point, color=YELLOW)
        line = Line(self.plane_center, point)
        line.set_color(dot.get_color())
        ...
        distance_label = OldTex(str(int(np.sqrt(x**2 + y**2))))
        distance_label.add_background_rectangle()
        distance_label.next_to(line.get_center(), UP + LEFT, SMALL_BUFF)

        self.play(ReplacementTransform(rational_point_group, integer_point_group))
    ...
    def get_unit_circle(self):
        template_line = Line(*[self.background_plane.number_to_point(z) for z in (-1, 1)])
...
class ProjectPointsOntoUnitCircle(DrawRadialLines):
    ...
    def add_unit_circle(self):
        template_line = Line(*[self.background_plane.number_to_point(n) for n in (-1, 1)])
    ...
    def project_all_dots(self):
        ...
        dots.sort(lambda p: get_norm(p - self.plane_center))
        ...
        for dot in dots:
            ...
            dot.target.scale(unit_length / distance, about_point=self.plane_center)
            dot.target.set_width(0.01)

        self.play(LaggedStartMap(MoveToTarget, dots, run_time=3, lag_ratio=0.2))
    ...
    def zoom_in(self):
        ...
        self.play(group.shift, -self.plane_center, group.scale, scale_factor, run_time=2)
        self.wait(2)

    def draw_infinitely_many_lines(self):
        lines = VGroup(*[Line(ORIGIN, FRAME_WIDTH * vect) for vect in compass_directions(1000)])
...
class ICanOnlyDrawFinitely(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("I can only \\\\ draw finitely", run_time=2)
        self.wait(2)
...
class SupposeMissingPoint(PointsWeMiss):
    ...
    def draw_missing_triple(self):
        ...
        triangle.add(a, b, c)
        words = OldTexText("If we missed \\\\ a triple \\dots")
    ...
    def project_onto_unit_circle(self):
        ...
        line.target.scale(1.0 / z_norm, about_point=self.plane_center)

        rational_point_word = OldTex("(a/c) + (b/c)i")
        rational_point_word.next_to(self.background_plane.coords_to_point(0, 6), RIGHT)
        rational_point_word.add_background_rectangle()
        arrow = Arrow(rational_point_word.get_bottom(), dot.target, buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(Write(rational_point_word), ShowCreation(arrow))
        self.wait(2)
...
class FinalProof(RationalPointsOnUnitCircle):
    ...
    def draw_rational_point(self):
        circle = self.get_unit_circle()
        coords = (3.0 / 5.0, 4.0 / 5.0)
        point = self.background_plane.coords_to_point(*coords)
        dot = Dot(point, color=YELLOW)
        label = OldTex("(a/c) + (b/c)i")
        label.add_background_rectangle()
        label.next_to(dot, UP + RIGHT, buff=0)

        self.add(circle)
        self.play(Write(label, run_time=2), DrawBorderThenFill(dot))
        self.wait()
    ...
    def draw_line_from_example_point(self):
        ...
        line = Line(neg_one_point, self.example_dot.get_center(), color=RED)

        self.play(ShowCreation(line, run_time=2), Animation(self.example_label))
    ...
    def show_slope_is_rational(self):
        ...
        run_text = run_brace.get_text("Run = $1 + \\frac{a}{c}$")
        ...
        self.wait()
        self.play(GrowFromCenter(rise_brace), FadeIn(rise_text))
        self.wait()
        self.play(GrowFromCenter(run_brace), FadeIn(run_text))
        self.wait(3)
        self.play(
            *list(
                map(
    ...
    def show_all_rational_slopes(self):
        ...
        for u in range(2, 7):
            for v in range(1, u):
                ...
                label = OldTex("\\text{Slope = }", str(v), "/", str(u))
                label.add_background_rectangle()
                label.next_to(self.background_plane.coords_to_point(1, 1.5), RIGHT)
        ...
        self.play(ShowCreation(line), FadeIn(label))
        self.wait()
    ...
    def square_example_point(self):
        ...
        uv_line = Line(self.plane_center, point, color=YELLOW)
        uv_arc = Arc(angle=uv_line.get_angle(), radius=0.75)
        ...
        square_line = Line(self.plane_center, square_point, color=MAROON_B)
        square_arc = Arc(angle=square_line.get_angle(), radius=0.65)
        ...
        z_to_z_squared_arrow = Arrow(point, square_point, path_arc=np.pi / 3, color=WHITE)
        z_to_z_squared = OldTex("z", "\\to", "z^2")
        z_to_z_squared.set_color_by_tex("z", YELLOW)
        z_to_z_squared.set_color_by_tex("z^2", MAROON_B)
        z_to_z_squared.add_background_rectangle()
        z_to_z_squared.next_to(z_to_z_squared_arrow.point_from_proportion(0.5), RIGHT, SMALL_BUFF)

        self.play(Write(uv_label), DrawBorderThenFill(uv_dot))
        self.play(ShowCreation(uv_line))
        self.play(ShowCreation(uv_arc))
        self.play(Write(theta))
        self.wait()
        self.play(ShowCreation(z_to_z_squared_arrow), FadeIn(z_to_z_squared))
        self.play(
            *[
                ReplacementTransform(m1.copy(), m2, path_arc=np.pi / 3)
                for m1, m2 in [
                    (uv_dot, square_dot),
                    (uv_line, square_line),
        ...
        self.wait()
        self.play(Write(two_theta), ShowCreation(two_theta_arrow))
        self.wait(2)
    ...
    def project_onto_circle(self):
        ...
        line.generate_target()
        line.target.scale(radius / line.get_length(), about_point=line.get_start())
        dot.generate_target()
        dot.target.move_to(line.target.get_end())
    ...
    def show_same_slope(self):
        ...
        line_copies = VGroup(line.copy(), self.uv_line.copy())
        line_copies.generate_target()
        line_copies.target.next_to(same_slope_words, DOWN)

        self.play(ShowCreation(line))
        self.wait()
        self.play(theta_group_copy.shift, line.get_start() - self.uv_line.get_start())
        self.wait()
        self.play(Write(same_slope_words), MoveToTarget(line_copies))
        self.wait()
    ...
    def write_v_over_u_slope(self):
        ...
        self.play(
            *[
            ...
            + [Write(rhs.get_part_by_tex(tex)) for tex in ("=", "over")]
        )
        self.wait(2)
...
class BitOfCircleGeometry(Scene):
    def construct(self):
        ...
        p0, p1, p2 = [circle.point_from_proportion(alpha) for alpha in (0, 0.15, 0.55)]
        O = circle.get_center()
        ...
        for point, tex, color in (O, "2", MAROON_B), (p2, "", RED):
            ...
            label.next_to(arc.point_from_proportion(0.9), RIGHT)
        ...
        self.wait(2)
        self.play(ReplacementTransform(groups[0].copy(), groups[1]))
        self.wait(2)
