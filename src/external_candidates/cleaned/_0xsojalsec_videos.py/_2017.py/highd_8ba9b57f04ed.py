# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\highD.py
...
class Slider(NumberLine):
    ...
    def update_dial_by_re_dial(self, dial=None):
        ...
        if sign == 0:
            sign = -self.last_sign
            self.last_sign *= -1
        dial.move_to(self.number_to_point(self.center_value + sign * np.sqrt(abs(re))))
        return dial
...
class SliderScene(Scene):
    ...
    def setup(self):
        ...
        sliders = VGroup(*[Slider(center_value=cv, **self.slider_config) for cv in self.center_point])
        sliders.arrange(RIGHT, buff=self.slider_spacing)
        sliders[0].add_numbers()
        sliders[0].set_value(self.center_point[0] + np.sqrt(self.total_real_estate))
    ...
    def reset_dials(self, values, run_time=1, **kwargs):
        ...
        self.play(
            *[ApplyMethod(slider.set_value, value) for value, slider in zip(target_vector, self.sliders)]
    ...
    def get_current_total_real_estate(self):
        return sum([slider.get_real_estate() for slider in self.sliders])

    def get_all_dial_supplement_animations(self):
        return [slider.get_dial_supplement_animation() for slider in self.sliders]
...
class TODODeterminants(TODOStub):
    CONFIG = {"message": "Determinants clip"}
...
class CircleToPairsOfPoints(Scene):
    def construct(self):
        ...
        numbers = VGroup(*[DecimalNumber(val).replace(num, dim_to_match=1) for val, num in zip([x, y], fixed_numbers)])
        ...
        self.play(
            Write(equation),
            *[
                Transform(number.copy(), equation.get_parts_by_tex(tex), remover=True)
        ...
        self.play(
            ...
            *[
                ChangingDecimal(num, get_update_func(i), tracked_mobject=fixed_num)
                for num, i, fixed_num in zip(numbers, (0, 1), fixed_numbers)
            ],
        )
        self.wait()
        ...
        rot_equation = OldTex(
            "\\Rightarrow\\big(\\cos(\\theta)x - \\sin(\\theta)y\\big)^2 + ",
        ...
        arrow = Arrow(words.get_left(), rot_equation.get_bottom(), path_arc=-np.pi / 6)
        ...
        self.play(Write(words, run_time=1), ShowCreation(arrow), randy.look_at, words)
        plane.remove(*plane.coordinate_labels)
        self.play(
            Rotate(plane, np.pi / 3, run_time=4, rate_func=there_and_back),
            Animation(equation),
...
class GreatSourceOfMaterial(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("It's a great source \\\\ of material.", target_mode="hooray")
...
class BackAndForth(Scene):
    def construct(self):
        ...
        self.wait(3)
        for tup, eq, to_draw in (
            (pair, circle_eq, circle),
            (
                triplet,
                sphere_eq,
                VMobject(),
...
class TODOTease(TODOStub):
    CONFIG = {"message": "Tease"}


class AskAboutLongerLists(TeacherStudentsScene):
    def construct(self):
        question = OldTexText("What about \\\\", "$(x_1, x_2, x_3, x_4)?$")
        tup = question[1]
        alt_tups = list(
            map(
                TexText,
        ...
        self.play_student_changes(*["confused"] * 3, look_at=self.teacher.get_top() + 2 * UP)
        self.play(self.teacher.look, UP)
        self.wait(5)
        self.student_says("I...don't see it.", target_mode="maybe", index=0)
        self.wait(3)
...
class Professionals(PiCreatureScene):
    ...
    def add_equation(self):
        ...
        kets = OldTex("\\alpha", "|\\!\\uparrow\\rangle + ", "\\beta", "|\\!\\downarrow\\rangle")
        ...
        for term, pi in zip(terms, self.pi_creatures):
            self.play(Write(term, run_time=1), pi.change, "pondering", term)
        self.wait(2)
    ...
    def analogies(self):
        ...
        for label, example in zip(labels, examples):
            ...
            self.play(ShowCreation(example), Write(label, run_time=1))
            example.add(label)
            self.wait()
        self.wait()
        self.play(FadeOut(examples), self.terms.shift, UP, Write(title, run_time=2))
        self.play(
            *[
                ApplyMethod(
                    pi.change,
...
class OfferAHybrid(SliderScene):
    ...
    def construct(self):
        ...
        v_lines = VGroup(*[Line(UP, DOWN).scale(FRAME_Y_RADIUS) for x in range(2)])
    ...
    def get_titles(self):
        titles = VGroup(*list(map(TexText, ["Analytic", "Hybrid", "Geometric"])))
        titles.to_edge(UP)
...
class DismissProjection(PiCreatureScene):
    ...
    def discuss_4d_sphere_definition(self):
        ...
        self.play(
            Write(sphere_words),
            ShowCreation(arrow, rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=3,
        )
        self.wait()
    ...
    def talk_through_animation(self):
        ...
        self.wait(3)
        self.play(morty.change, "erm", alt_dims, FadeIn(alt_dim))
    ...
    def introduce_sliders(self):
        ...
        self.play(
            *[
                ...
                for num, dial, a in zip(numbers, dial_copies, np.linspace(0, 0.5, len(numbers)))
            ]
        )
...
class TwoDimensionalCase(Introduce4DSliders):
    ...
    def setup(self):
        ...
        for number in self.sliders[0].numbers:
            ...
            number.move_to(center_of_mass([slider.number_to_point(value) for slider in self.sliders]))
    ...
    def introduce_real_estate(self):
        ...
        decimals = VGroup(*[DecimalNumber(num**2) for num in self.get_vector()])
        ...
        for rect, decimal, color in zip(rects, decimals, colors):
            ...
            self.play(ShowCreation(rect))
            self.play(ShowCreation(arrow), Write(decimal))
            self.wait()
        ...
        self.decimal_update_anims = [
            ChangingDecimal(decimal, create_update_func(i)) for i, decimal in enumerate(decimals)
        ]
    ...
    def comment_on_cheap_vs_expensive_real_estate(self):
        ...
        for slider in self.sliders:
            for x1, x2 in (-0.5, 0.5), (0.75, 1.0), (-1.0, -0.75):
                ...
                rect = Rectangle(stroke_width=0, fill_opacity=0.5, width=0.25, height=(p2 - p1)[1])
    ...
    def nudge_x_from_one_example(self):
        ...
        x_brace, y_brace = [
            Brace(VGroup(slider.dial, Dot(slider.number_to_point(0))), vect)
        ...
        self.play(GrowFromCenter(x_brace), Write(x_text))
        self.play(ReplacementTransform(VGroup(x_text.copy()), x_re))
        self.wait(2)
        self.play(
    ...
    def add_tick_marks(self):
        ...
        for x in np.arange(0.95, 0.05, -0.05):
            self.reset_dials([np.sqrt(x), np.sqrt(1 - x)], run_time=0.5)
            self.wait(0.5)
        self.initialize_ambiant_slider_movement()
        self.wait(10)
...
class ThreeDCase(TwoDimensionalCase):
    ...
    def setup(self):
        SliderScene.setup(self)
        self.equation = OldTex("x^2", "+", "y^2", "+", "z^2", "=", "1")
    ...
    def add_real_estate_decimals(self):
        rects = VGroup(*[SurroundingRectangle(self.equation.get_part_by_tex(char)) for char in "xyz"])
...
class FourDCase(SliderScene, TeacherStudentsScene):
    ...
    def show_initial_exchange(self):
        ...
        self.play_student_changes(*["pondering"] * 3, look_at=self.sliders)
        self.wait(4)
    ...
    def fix_one_slider(self):
        ...
        self.play(ShowCreation(arrow), dial.set_color, arrow.get_color())
...
class TwoDBoxExample(Scene):
    ...
    def label_corner_coordinates(self):
        ...
        for x, y in it.product(*[[1, -1]] * 2):
            ...
            self.play(ShowCreation(dot), Write(coords, run_time=1))
    ...
    def add_corner_circles(self):
        ...
        circles = VGroup(*[circle.copy().move_to(dot) for dot in self.corner_dots])
        ...
        r_equals_1.next_to(radius.point_from_proportion(0.75), UP + RIGHT, SMALL_BUFF)

        self.play(LaggedStartMap(ShowCreation, circles))
        self.play(ShowCreation(radius), Write(r_equals_1))
        for angle in -np.pi / 4, -np.pi / 2, 3 * np.pi / 4:
            self.play(
                Rotating(
                    radius,
                    about_point=c0_center,
                    radians=angle,
                    run_time=1,
                    rate_func=smooth,
                )
    ...
    def add_center_circle(self):
        ...
        circle.replace(VGroup(radius, radius.copy().rotate(np.pi)), dim_to_match=0)
        radius.rotate(np.pi / 4)
        r_equals_q = OldTex("r", "= ???")
    ...
    def compute_radius(self):
        ...
        root_2_minus_1_value = OldTex("\\sqrt{2} - 1 \\approx 0.414")
        ...
        self.play(FadeIn(triangle), Write(VGroup(bottom_one, side_one, sqrt_1_plus_1)))
        self.wait(2)
        self.play(ReplacementTransform(sqrt_1_plus_1, sqrt_2))
        self.play(Write(root_2_value, run_time=1), *list(map(FadeOut, [bottom_one, side_one])))
        self.wait()
        self.play(ShowCreation(corner_radius))
        self.play(Rotating(corner_radius, about_point=c0_center, run_time=2, rate_func=smooth))
...
class ThreeDCubeCorners(Scene):
    def construct(self):
        coordinates = VGroup(*[OldTex("(%d,\\, %d,\\, %d)" % (x, y, z)) for x, y, z in it.product(*3 * [[1, -1]])])
...
class GeneralizePythagoreanTheoremBeyondTwoD(ThreeDScene):
    def construct(self):
        ...
        diag = Line(rect.get_corner(DOWN + LEFT), rect.get_corner(UP + RIGHT), color=YELLOW)
        ...
        for line, tex in (bottom, x), (side, y), (diag, hyp):
            self.play(ShowCreation(line), Write(tex, run_time=1))
        self.wait()
        self.play(group.rotate, 0.45 * np.pi, LEFT, group.shift, 2 * DOWN)
        ...
        self.play(ShowCreation(z_line), ShowCreation(dot), Write(z, run_time=1))
...
class ThreeDBoxFormulas(Scene):
    def construct(self):
        ...
        inner_r = OldTex("\\text{Inner radius}", "&=", "\\sqrt{3} - 1\\\\", "&\\approx", "0.73")
...
class AskAboutHigherDimensions(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("What happens for \\\\ higher dimensions?")
        self.play_student_changes(*["pondering"] * 3)
        self.wait(2)
        self.student_thinks("$\\sqrt{N} - 1$", target_mode="happy", index=1)
        self.wait()
        pi = self.students[1]
...
class TwoDBoxWithSliders(TwoDimensionalCase):
    ...
    def setup(self):
        ...
        for number in x_slider.numbers:
            value = int(number.get_tex())
            number.next_to(x_slider.number_to_point(value), LEFT, MED_SMALL_BUFF)
        self.plane.axes.set_color(BLUE)

        ##Add box material
        corner_circles = VGroup(
            *[
                self.circle.copy().move_to(self.plane.coords_to_point(*coords)).set_color(GREY)
    ...
    def recenter_circle(self):
        ...
        self.play(
            ...
            Animation(self.sliders),
            *[
                ApplyMethod(mob.shift, slider.number_to_point(x) - slider.number_to_point(0))
    ...
    def write_x_and_y_real_estate(self):
        ...
        x_brace, y_brace = [Brace(slider.real_estate_ticks, RIGHT) for slider in self.sliders]
        x_brace.set_color(GREEN)
        y_brace.set_color(RED)

        self.play(FadeOut(self.question))
        self.play(Write(phrases[0]), GrowFromCenter(x_brace))
        self.wait(3)
        self.play(Transform(*phrases), Transform(x_brace, y_brace))
        self.wait(5)
        self.wind_down_ambient_movement(wait=False)
    ...
    def swap_with_top_right_circle(self):
        ...
        self.play(
            ...
            Animation(slider),
            *[
                ApplyMethod(mob.shift, slider.number_to_point(1) - slider.number_to_point(-1))
                for mob in (slider.real_estate_ticks, slider.dial)
            ],
        )
    ...
    def show_center_circle(self):
        ...
        radius = get_norm(self.plane.coords_to_point(np.sqrt(2) - 1, 0) - origin)
    ...
    def describe_tangent_point(self):
        target_vector = np.array([1 - np.sqrt(2) / 2, 1 - np.sqrt(2) / 2])
    ...
    def perterb_point(self):
        ...
        ghost_dials = VGroup(*[slider.dial.copy() for slider in self.sliders])
        ...
        x_arrow, y_arrow = [
            Arrow(
                words[1].get_edge_center(vect),
                self.sliders[i].dial,
                tip_length=0.15,
            )
            for i, words, vect in zip((0, 1), [x_words, y_words], [RIGHT, LEFT])
        ]

        self.play(Write(x_words, run_time=2), ShowCreation(x_arrow))
        self.wait()
        self.play(FadeOut(self.evenly_shared_words))
        self.play(Write(y_words, run_time=2), ShowCreation(y_arrow))
        self.wait(2)
        ...
        self.play(LaggedStartMap(MoveToTarget, new_words), Animation(crosses))
        self.wait(3)
    ...
    def ask_about_inner_real_estate(self):
        ...
        half_line = Line(
            *[slider.number_to_point(0.5) + MED_LARGE_BUFF * vect for slider, vect in zip(self.sliders, [LEFT, RIGHT])]
        )
        half = OldTex("0.5")
        ...
        self.play(Write(question))
        self.wait(3)
        self.play(ShowCreation(half_line), Write(half))
        self.wait()
        self.play(Write(rhs))
        self.wait(3)
...
class AskWhy(TeacherStudentsScene):
    def construct(self):
        self.student_says("Wait, why?", target_mode="confused")
        self.wait(3)
...
class MentionComparisonToZeroPointFive(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "Comparing to $0.5$ will \\\\" + "be surprisingly useful!",
            target_mode="hooray",
        )
        self.play_student_changes(*["happy"] * 3)
        self.wait(3)
...
class ThreeDBoxExampleWithSliders(SliderScene):
    ...
    def name_corner_sphere(self):
        sphere_name = OldTexText("""Sphere with radius 1\\\\
            centered at (1, 1, 1)""")
        sphere_name.to_corner(UP + LEFT)
        arrow = Arrow(sphere_name, VGroup(*self.sliders[0].numbers[-2:]), color=BLUE)
    ...
    def point_out_closest_point(self):
        target_x = 1 - np.sqrt(1.0 / 3)
        target_vector = np.array(3 * [target_x])
        re_words = OldTexText("$x$, $y$ and $z$ each have \\\\", "$\\frac{1}{3}$", "units of real estate")
        re_words.to_corner(UP + LEFT)
        re_line = DashedLine(
            *[self.sliders[i].number_to_point(target_x) + MED_SMALL_BUFF * vect for i, vect in [(0, LEFT), (2, RIGHT)]]
        )
        new_arrow = Arrow(re_words.get_corner(DOWN + RIGHT), re_line.get_left(), color=BLUE)

        self.wind_down_ambient_movement()
        self.play(*[ApplyMethod(slider.set_value, x) for x, slider in zip(target_vector, self.sliders)])
        self.play(ShowCreation(re_line))
        self.play(
    ...
    def compare_to_halfway_point(self):
        half_line = Line(
            *[self.sliders[i].number_to_point(0.5) + MED_SMALL_BUFF * vect for i, vect in [(0, LEFT), (2, RIGHT)]]
        )
        ...
        ghost_dials = VGroup(*[slider.dial.copy().set_fill(WHITE, 0.5) for slider in self.sliders])
        ...
        self.play(FadeOut(self.arrow), Write(half_label, run_time=1), ShowCreation(half_line))
        self.wait()
        self.add(ghost_dials)
        self.play(*[ApplyMethod(slider.set_value, 0.5) for slider in self.sliders])
        ...
        self.play(
            FadeOut(new_re),
            FadeOut(cross),
            *[ApplyMethod(slider.set_value, x) for x, slider in zip(curr_vector, self.sliders)],
        )
    ...
    def reframe_as_inner_sphere_point(self):
        ...
        all_re_ticks = VGroup(*[slider.real_estate_ticks for slider in self.sliders])
        inner_sphere_words = OldTexText("Also a point on \\\\", "the inner sphere")
        ...
        self.play(
            ...
            LaggedStartMap(ApplyMethod, all_re_ticks, lambda m: (m.shift, shift_vect)),
        )
        self.initialize_ambiant_slider_movement()
        self.play(Write(inner_sphere_words))
        self.wait(5)
        self.wind_down_ambient_movement(0)
        self.play(*[ApplyMethod(slider.set_value, x) for slider, x in zip(self.sliders, curr_vector)])
        self.play(ReplacementTransform(inner_sphere_words, question))
        self.wait(2)

        self.re_question = question

    def place_bound_on_inner_real_estate(self):
        bound = OldTex("&< 3(0.5)^2 ", "= 0.75")
...
class FourDBoxExampleWithSliders(ThreeDBoxExampleWithSliders):
    ...
    def list_corner_coordinates(self):
        title = OldTexText("$2 \\!\\times\\! 2 \\!\\times\\! 2 \\!\\times\\! 2$ box vertices:")
        title.shift(FRAME_X_RADIUS * LEFT / 2)
        title.to_edge(UP)

        coordinates = list(it.product(*4 * [[1, -1]]))
        coordinate_mobs = VGroup(*[OldTex("(%d, %d, %d, %d)" % tup) for tup in coordinates])
    ...
    def show_16_corner_spheres(self):
        ...
        for coord_mob, coords in pairs[1:] + [pairs[0]]:
            ...
            self.sliders[0].dial.move_to(self.sliders[0].number_to_point(coords[0] + 1))
        ...
        self.play(
            ...
            sphere_words.shift,
            LEFT,
            *list(map(FadeOut, [self.coordinate_mobs, self.box_vertices_title])),
        )
        self.initialize_ambiant_slider_movement()
        self.wait(4)
    ...
    def show_closest_point(self):
        ...
        re_line = DashedLine(
            *[self.sliders[i].number_to_point(0.5) + MED_SMALL_BUFF * vect for i, vect in [(0, LEFT), (-1, RIGHT)]]
        )
        half_label = OldTex("0.5")
        half_label.scale(self.sliders[0].number_scale_val)
...
class TwoDInnerSphereTouchingBox(TwoDBoxWithSliders, PiCreatureScene):
    ...
    def construct(self):
        little_inner_circle, big_inner_circle = [
            Circle(radius=radius * self.plane.x_unit_size, color=GREEN).move_to(self.plane.coords_to_point(0, 0))
            for radius in (np.sqrt(2) - 1, 1)
        ]
        randy = self.randy
        tangency_points = VGroup(
            *[Dot(self.plane.coords_to_point(x, y)) for x, y in [(1, 0), (0, 1), (-1, 0), (0, -1)]]
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(little_inner_circle.copy(), big_inner_circle),
        ...
        self.wait()
        self.play(LaggedStartMap(DrawBorderThenFill, tangency_points, rate_func=double_smooth))
...
class FiveDBoxExampleWithSliders(FourDBoxExampleWithSliders):
    ...
    def show_closest_point(self):
        ...
        re_line = DashedLine(
            *[self.sliders[i].number_to_point(target_x) + MED_SMALL_BUFF * vect for i, vect in [(0, LEFT), (-1, RIGHT)]]
        )
        re_words = OldTexText("$0.2$", "units of real \\\\ estate each")
        re_words.next_to(self.sphere_words, DOWN, MED_LARGE_BUFF)
        ...
        self.play(ShowCreation(re_line), Write(re_words, run_time=2))
        self.play(LaggedStartMap(DrawBorderThenFill, re_rects, rate_func=double_smooth))
        self.wait()
    ...
    def show_halfway_point(self):
        ...
        curr_vector = self.get_vector()
        ghost_dials = VGroup(*[slider.dial.copy().set_fill(WHITE, 0.75) for slider in self.sliders])
        ...
        self.play(Write(half_label), ShowCreation(half_line))
        self.wait(2)
        self.add(ghost_dials)
        self.play(*[ApplyMethod(slider.set_value, 0.5) for slider in self.sliders])
        self.play(Transform(self.re_words[0], point_25))
        self.wait(2)
        self.play(*[ApplyMethod(slider.set_value, x) for x, slider in zip(curr_vector, self.sliders)])
        self.play(self.re_words.restore)

    def reframe_as_inner_sphere_point(self):
        s = self.sliders[0]
        shift_vect = s.number_to_point(0) - s.number_to_point(1)
        re_ticks = VGroup(*[slider.real_estate_ticks for slider in self.sliders])
        ...
        self.play(
            Animation(self.sliders),
            LaggedStartMap(ApplyMethod, re_ticks, lambda m: (m.shift, shift_vect), path_arc=np.pi),
            MoveToTarget(self.sphere_words),
        )
        self.play(
            MoveToTarget(re_rects, run_time=2, lag_ratio=0.5, path_arc=np.pi),
    ...
    def compute_radius(self):
        computation = OldTex("R_{\\text{inner}} &= \\sqrt{5}-1 \\\\", "&\\approx 1.24")
        computation.to_corner(UP + LEFT)
...
class SkipAheadTo10(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Let's skip ahead \\\\ to 10 dimensions", target_mode="hooray")
        self.play_student_changes("pleading", "confused", "horrified")
        self.wait(3)
...
class TenDBoxExampleWithSliders(FiveDBoxExampleWithSliders):
    ...
    def show_closest_point(self):
        target_x = 1 - np.sqrt(1.0 / self.n_sliders)
        re_line = DashedLine(
            *[self.sliders[i].number_to_point(target_x) + MED_SMALL_BUFF * vect for i, vect in [(0, LEFT), (-1, RIGHT)]]
        )
    ...
    def reframe_as_inner_sphere_point(self):
        ...
        re_rects.generate_target()
        for rect, slider in zip(re_rects.target, self.sliders):
            height = slider.unit_size * (1 - np.sqrt(1.0 / self.n_sliders))
    ...
    def compute_inner_radius_numerically(self):
        computation = OldTex("R_{\\text{inner}} &= \\sqrt{10}-1 \\\\", "&\\approx 2.16")
        computation.to_corner(UP + LEFT)
    ...
    def poke_outside_outer_box(self):
        ...
        words = OldTexText("Outside the \\emph{outer} \\\\", "bounding box!")
        words.to_edge(LEFT)
        words.set_color(RED)
        arrow = Arrow(
            words.get_top(),
        ...
        self.play(Write(words, run_time=2), ShowCreation(arrow))
        self.wait(3)
...
class ThreeDOuterBoundingBoxWords(Scene):
    def construct(self):
        words = OldTexText("$4 \\!\\times\\! 4\\!\\times\\! 4$ outer\\\\", "bounding box")
...
class ProportionOfSphereInBox(GraphScene):
    ...
    def construct(self):
        self.setup_axes()
        title = OldTexText("Proportion of inner sphere \\\\ inside box")
        ...
        footnote.scale(0.75)
        footnote.next_to(graph.point_from_proportion(0.3), UP + RIGHT, SMALL_BUFF)
...
class ShowingToFriend(PiCreatureScene, SliderScene):
    ...
    def setup(self):
        ...
        self.sliders.next_to(self.morty.get_corner(UP + LEFT), UP, MED_LARGE_BUFF)
        self.initialize_ambiant_slider_movement()
...
class FunHighDSpherePhenomena(Scene):
    def construct(self):
        title = OldTexText("Fun high-D sphere phenomena")
        title.to_edge(UP)
...
class CoordinateFree(PiCreatureScene):
    def construct(self):
        ...
        circles = VGroup(
            *[Circle(color=YELLOW).move_to(plane.coords_to_point(*coords)) for coords in it.product(*2 * [[-1, 1]])]
        )
        inner_circle = Circle(radius=np.sqrt(2) - 1, color=GREEN).move_to(plane.coords_to_point(0, 0))

        self.add_foreground_mobjects(circles, inner_circle)

        self.play(PiCreatureSays(self.pi_creature, "Lose the \\\\ coordinates!", target_mode="hooray"))
...
class Skeptic(TeacherStudentsScene, SliderScene):
    ...
    def construct(self):
        ...
        equation = OldTex("x", "^2 + ", "y", "^2 + ", "z", "^2 + ", "w", "^2 = 1")
        variables = VGroup(*[equation.get_part_by_tex(tex) for tex in "xyzw"])
        slider_labels = VGroup(*[slider.label for slider in self.sliders])
        equation.next_to(analytic_thought, DOWN)
        analytic_thought.add(equation)

        all_real_estate_ticks = VGroup(*it.chain(*[slider.real_estate_ticks for slider in self.sliders]))
        ...
        self.play(
            ...
            "pondering",
            slider_labels,
            RemovePiCreatureBubble(self.students[0], target_mode="hesitant"),
        )
        self.wait(4)
        bubble = self.teacher.get_bubble("It's much \\\\ more playful!", bubble_type=SpeechBubble)
        bubble.resize_to_content()
        VGroup(bubble, bubble.content).next_to(self.teacher, UP + LEFT)
        self.play(self.teacher.change, "hooray", ShowCreation(bubble), Write(bubble.content))
        self.wait(3)
        self.play(
            RemovePiCreatureBubble(self.teacher, target_mode="raise_right_hand", look_at=self.sliders),
            *[ApplyMethod(pi.change, "pondering") for pi in self.students],
        )
        self.play(
            Animation(self.sliders),
            LaggedStartMap(
                ApplyMethod,
                all_real_estate_ticks,
        ...
        self.play(ShowCreation(box), self.teacher.change, "happy")
        self.play(ShowCreation(line))
        self.wait(3)
...
class ClipFrom4DBoxExampleTODO(TODOStub):
    CONFIG = {"message": "Clip from 4d box example"}
...
class JustBecauseYouCantVisualize(Scene):
    def construct(self):
        ...
        for part in phrase_mob:
            self.play(LaggedStartMap(FadeIn, part, run_time=0.05 * len(part)))
        self.wait(2)
...
class Announcements(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*[ApplyMethod(pi.change, "hooray", items) for pi in self.pi_creatures])
...
class Promotion(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.remove(rect)
        self.play(url.next_to, self.pi_creature, UP + LEFT)
...
class Podcast(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(ApplyMethod, self.pi_creatures, lambda pi: (pi.change, "hooray", title)),
            Write(title),
        )
        self.wait(5)
        self.play(
...
class TenDThumbnail(Scene):
    def construct(self):
        ...
        circles = VGroup(*[Circle(radius=r).move_to(corner) for corner in square.get_vertices()])
        circles.set_stroke(BLUE, 5)
        circles.set_fill(BLUE, 0.5)
        circles.set_sheen(0.5, UL)
        lil_circle = Circle(radius=(np.sqrt(2) - 1) * r)
