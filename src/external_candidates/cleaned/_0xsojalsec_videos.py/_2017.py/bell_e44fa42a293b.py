# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\bell.py
...
class PhotonPassesCompletelyOrNotAtAll(DirectionOfPolarizationScene):
    ...
    def construct(self):
        ...
        passing_photon = WavePacket(run_time=2, get_filtered=False, em_wave=self.em_wave.copy())
        ...
        filtered_photon = WavePacket(run_time=2, get_filtered=True, em_wave=lower_em_wave.copy())
        ...
        self.play(
            ...
            lower_filter.shift,
            self.lower_portion_shift,
            FadeIn(filtered_words),
        )
        red_flash = ApplyMethod(
            lower_filter.set_fill,
            RED,
            rate_func=squish_rate_func(there_and_back, 0.4, 0.6),
...
class DirectionOfPolarization(DirectionOfPolarizationScene):
    def construct(self):
        ...
        self.add(em_wave)
        self.wait(2)
        self.move_camera(phi=self.target_phi, theta=self.target_theta)
...
class PhotonsThroughPerpendicularFilters(PhotonPassesCompletelyOrNotAtAll):
    ...
    def construct(self):
        ...
        self.move_camera(
            ...
            added_anims=[ApplyFunction(self.reposition_filter_label, zero_filter)],
        )
...
class MoreFiltersMoreLight(FilterScene):
    CONFIG = {
        ...
        "pol_filter_configs": [
            {"include_arrow_label": False, "filter_angle": angle} for angle in np.linspace(0, np.pi / 2, 5)
        ],
    ...
    def build_color_map(self, pfs):
        ...
        for bool_array in it.product(*5 * [[True, False]]):
            pfs_to_use = VGroup(*[pf for pf, b in zip(pfs, bool_array) if b])
    ...
    def get_frame(self):
        ...
        bool_arrays = [
            (frame[:, :, 0] == r) & (frame[:, :, 1] == g) & (frame[:, :, 2] == b) for (r, g, b) in self.original_rgbas
        ]
        for ba, new_rgb in zip(bool_arrays, self.new_rgbas):
            frame[ba] = new_rgb
        covered = reduce(lambda b1, b2: b1 | b2, bool_arrays)
...
class ShowALittleMath(TeacherStudentsScene):
    def construct(self):
        ...
        color_map = {"alpha": GREEN, "beta": RED, "psi": BLUE}
        ...
        self.play(Write(exp1, run_time=2), self.teacher.change, "raise_right_hand")
        ...
        self.play_student_changes(*["pondering"] * 3, look_at=exp2)
        self.wait(2)
...
class BasicsOfPolarization(DirectionOfPolarizationScene):
    ...
    def show_continual_wave(self):
        ...
        self.wait(2)
        self.play(Write(subtitle, run_time=2), FadeIn(self.rectangles))
    ...
    def show_photons(self):
        ...
        self.play(FadeIn(quantum_words, run_time=2, lag_ratio=0.5))
        anim_sets = [
            [passing_photon],
            [
                filtered_photon,
                self.get_filter_absorption_animation(self.pol_filter, filtered_photon),
            ],
        ]
...
class AngleToProbabilityChart(Scene):
    def construct(self):
        ...
        angle_mobs = VGroup(*[OldTex(str(angle) + "^\\circ") for angle in angles])
        angle_mobs.arrange(DOWN, buff=MED_LARGE_BUFF)
        angle_mobs.next_to(left_title, DOWN, LARGE_BUFF)

        probs = [np.cos(angle * np.pi / 180.0) ** 2 for angle in angles]
        prob_mobs = VGroup(*[OldTex("%.1f" % (100 * prob) + "\\%") for prob in probs])
        ...
        for i in [0, 4, 2, 1, 3]:
            ...
            self.play(ReplacementTransform(angle_mobs[i].copy(), prob_mobs[i]))
...
class ShowVariousFilterPairsWithPhotonsOverTime(PhotonsThroughPerpendicularFilters):
    CONFIG = {
        ...
        "pol_filter_configs": [{"filter_angle": angle} for angle in (0, 0, np.pi / 2, np.pi / 4, np.pi / 8)],
        "apply_filter": False,
    }
    ...
    def add_filters(self):
        ...
        self.wait()
        self.play(ReplacementTransform(self.pol_filters[0].copy().set_fill(BLACK, 1), self.pol_filters[1]))
        self.move_camera(
            theta=-0.65 * np.pi,
            added_anims=list(
                it.chain(
                    *[
                        [
                            pf.arrow_label.rotate,
                            np.pi / 2,
                            OUT,
                            pf.arrow_label.next_to,
                            pf.arrow,
                            RIGHT,
                        ]
    ...
    def show_photons(self, n_photons=5):
        ...
        first_absorption = self.get_filter_absorption_animation(self.pol_filters[0], blocked_photon)
        first_absorption.rate_func = squish_rate_func(
            first_absorption.rate_func,
            0,
            0.5,
        )

        photons = [copy.deepcopy(self.photons[2 if q <= p else 1]) for q in np.arange(0, 1, 1.0 / n_photons)]
        random.shuffle(photons)
        for photon in photons:
            photon.rate_func = squish_rate_func(lambda x: x, 0.5, 1)
            added_anims = []
            if photon.filter_distance == FRAME_X_RADIUS + 2:
                absorption = self.get_filter_absorption_animation(self.second_filter, photon)
                absorption.rate_func = squish_rate_func(absorption.rate_func, 0.5, 1)
                added_anims.append(absorption)
            self.play(blocked_photon, first_absorption, photon, *added_anims)
        self.wait()

    def change_to_new_filter(self, pol_filter, added_anims=None):
        if added_anims is None:
            added_anims = []
        self.play(Transform(self.second_filter, pol_filter), *added_anims)
...
class ShowVariousFilterPairs(ShowVariousFilterPairsWithPhotonsOverTime):
    ...
    def add_top_filters(self):
        ...
        self.add(pf1)
        self.play(ReplacementTransform(pf1.copy().fade(1), pf2), Animation(pf1))
        self.move_camera(0.9 * np.pi / 2, -0.6 * np.pi, added_anims=[pf.restore for pf in pair])
    ...
    def show_light(self, filter_pair):
        ...
        self.play(ShowCreation(lines_to_pf1, run_time=2.0 / 3, **kwargs))
        self.play(
        ...
        group = VGroup(lines_from_pf2, pf2, lines_to_pf2, pf1, lines_to_pf2)
    ...
    def get_probability_text(self, pol_filter=None):
        ...
        prob_mob.set_color(list(Color(RED).range_to(GREEN, 11))[int(prob * 10)])
        return prob_mob
    ...
    def get_lines(
        ...
        n = self.n_lines
        start, end = [(f.point_from_proportion(0.75) if f is not None else None) for f in (filter1, filter2)]
        if start is None:
            start = end + self.line_start_length * LEFT
        if end is None:
            end = start + self.line_end_length * RIGHT
        nudge = (float(self.lines_depth) / self.n_lines) * OUT
        lines = VGroup(*[Line(start, end).shift(z * nudge) for z in range(n)])
...
class ShowVariousFilterPairsFrom0To45(ShowVariousFilterPairs):
    CONFIG = {"angles": [0, np.pi / 8, np.pi / 4]}
    ...
    def mention_probabilities(self):
        ...
        cosines = VGroup(*[OldTex("\\cos^2(%s^\\circ)" % str(x)) for x in (45, 22.5)])
        cosines.scale(0.8)
...
class ForgetPreviousActions(ShowVariousFilterPairs):
    CONFIG = {
        ...
        "pol_filter_configs": [{"filter_angle": angle} for angle in (np.pi / 4, 0, np.pi / 4, np.pi / 3, np.pi / 6)],
    ...
    def construct(self):
        ...
        prob_words = OldTexText("Probabilities depend only\\\\", "on this angle difference")
        ...
        for index in 1, 2:
            self.shoot_photon()
            self.play(Transform(second_filter, possible_second_filters[index]))
...
class IntroduceLabeledFilters(ShowVariousFilterPairs):
    CONFIG = {
        ...
        "pol_filter_configs": [{"filter_angle": angle} for angle in [0, np.pi / 8, np.pi / 4]],
    ...
    def separate_cases(self):
        ...
        self.play(
            ...
            self.lower_pol_filters.shift,
            3 * IN,
            self.pol_filters.shift,
            1.5 * OUT,
        )
        self.wait()
    ...
    def show_bottom_lines(self):
        ...
        self.play(ShowCreation(lines_to_A), **kwargs)
        self.play(ShowCreation(lines_to_C), Animation(VGroup(A, lines_to_A)), **kwargs)
    ...
    def show_top_lines(self):
        ...
        self.play(ShowCreation(lines_to_A), **kwargs)
        self.play(ShowCreation(lines_to_B), Animation(VGroup(A, lines_to_A)), **kwargs)
        self.play(
        ...
        line_group = VGroup(lines_from_C, C, lines_to_C, B, lines_to_B, A, lines_to_A)
        self.remove(*line_group)
        self.add_foreground_mobject(line_group)
    ...
    def comment_on_those_blocked_by_B(self):
        ...
        arrow1 = Arrow(2 * LEFT, ORIGIN, path_arc=0.8 * np.pi, color=GREEN, stroke_width=5, buff=0)
        arrow2 = arrow1.copy()
        ...
        self.play(ShowCreation(arrow0), Write(words0, run_time=1))
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(ShowCreation(arrow2), Write(words2))
        self.wait(2)

        self.fifteens = VGroup(words1, words2)
        self.blocked_at_B_label_group = VGroup(words1, words2, arrow1, arrow2)
    ...
    def show_sum(self):
        ...
        self.play(fifteen1.set_color, YELLOW, Write(plus))
        self.play(fifteen2.set_color, YELLOW, Write(equals))
        self.play(fifty.next_to, equals, RIGHT, 2 * SMALL_BUFF, Write(q_mark), FadeIn(randy))
        self.play(Transform(randy, blinked, rate_func=squish_rate_func(there_and_back)))
        self.wait(3)
...
class NumbersSuggestHiddenVariablesAreImpossible(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("These numbers suggest\\\\", "no hidden variables")
        self.play_student_changes("erm", "sassy", "confused")
        self.wait(3)


class VennDiagramProofByContradiction(Scene):
    CONFIG = {"circle_colors": [RED, GREEN, BLUE]}
    ...
    def draw_venn_diagram(self, send_to_corner=True):
        A, B, C = venn_diagram = VGroup(
            *[
                ...
                for c, vect in zip(self.circle_colors, compass_directions(3, UP))
            ]
        )
        ...
        for circle, char, prop, angle in zip(venn_diagram, "ABC", props, angles):
            label, alt_label = [
                OldTexText("%s \\\\" % start, "through", char + "$\\! \\uparrow$").set_color_by_tex(
    ...
    def show_100_photons(self):
        ...
        hundred, photon_word, s = words = OldTexText("100 ", "Photon", "s", arg_separator="")
        words.next_to(eyes, UP)

        self.play(ShowCreation(photon), FadeIn(photon.eyes), Write(photon_word, run_time=1.5))
        ...
        self.play(
            Write(hundred),
            Write(s),
            ReplacementTransform(VGroup(photon), photons, lag_ratio=0.5),
        )
    ...
    def put_all_photons_in_A(self):
        ...
        A_group, B_group, C_group = [VGroup(circle, circle.label) for circle in circles]
        B_group.save_state()
        C_group.save_state()

        A.generate_target()
        A.target.scale(4)
        A.target.shift((FRAME_Y_RADIUS - MED_LARGE_BUFF) * UP - A.target.get_top())
        A.label.generate_target()
        A.label.target.scale(2)
        A.label.target.next_to(A.target.point_from_proportion(0.1), UP + RIGHT, SMALL_BUFF)
        ...
        self.play(
            ...
            self.photons.space_out_submobjects,
            0.8,
            self.photons.move_to,
            A.target,
        )
        self.wait()
    ...
    def separate_by_B(self):
        ...
        B.label.target.next_to(B.target.point_from_proportion(0.55), LEFT, SMALL_BUFF)

        B_center = B.target.get_center()
        photons.sort(lambda p: get_norm(p - B_center))
    ...
    def separate_by_C(self):
        ...
        C.label.target.next_to(C.target.point_from_proportion(0), DOWN + RIGHT, buff=SMALL_BUFF)

        in_B.sort(lambda p: get_norm(p - C_center))
        in_C = VGroup(*in_B[:-11])
    ...
    def show_two_relevant_subsets(self):
        ...
        self.play(
            *[ApplyMethod(m.scale, 0.7, method_kwargs={"about_point": FRAME_Y_RADIUS * DOWN}) for m in everything]
        )

        terms = VGroup(
            OldTex("N(", "A", "\\checkmark", ",", "C", ")", "\\le"),
            OldTex("N(", "A", "\\checkmark", ",", "B", "\\checkmark", ",", "C", ")"),
        ...
        self.play(
            VGroup(regions[0], all_out_of_C).shift,
            0.5 * UP,
            run_time=1.5,
            rate_func=wiggle,
        )
        ...
        self.play(faders.restore, regions[0].set_fill, None, 0, Animation(self.photons))

        # Funny business
        everything_copy = everything.copy().scale(1.0 / 3)
        braces = VGroup(
        ...
        self.play(FadeIn(funny_business), *list(map(Write, braces)), run_time=1)
        self.wait()
        self.play(FadeIn(less_than), *list(map(FadeOut, [funny_business, braces])))
        ...
        self.wait()
        self.play(FadeIn(should_be_50[1]), ShowCreation(cross))
        ...
        self.play(morty.change, "confused", should_be_50, Write(contradiction, run_time=1))
    ...
    def photons_jump_to_A_not_C_region(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, movers, path_arc=np.pi, lag_ratio=0.3))
    ...
    def photon_thinks_in_A_out_C(self):
        ...
        self.play(photon.eyes.blink_anim())
        self.play(photon.restore, FadeOut(bubble))
    ...
    def arrange_photons_in_circle(self, photons):
        ...
        for x, y in it.product(*[list(range(-int(R) - 1, int(R) + 2))] * 2):
            if x**2 + y**2 < R**2:
                ...
                rejected.append((x, y))
        rejected.sort(kay=lambda x, y: (x**2 + y**2))
...
class ReEmphasizeVennDiagram(VennDiagramProofByContradiction):
    ...
    def rescale_diagram(self):
        group = VGroup(self.venn_diagram, *[c.label for c in self.venn_diagram])
        self.play(
            group.scale,
            0.7,
            group.to_edge,
    ...
    def setup_faded_circles(self):
        ...
        self.black_circles = VGroup(*[circ.copy().set_stroke(width=0).set_fill(BLACK, 1) for circ in self.circles])
        self.filled_circles = VGroup(
            *[circ.copy().set_stroke(width=0).set_fill(circ.get_color(), 1) for circ in self.circles]
        )
    ...
    def shift_B_circle(self):
        ...
        words.shift(RIGHT)
        arrow = Arrow(words.get_bottom(), A.get_top() + MED_SMALL_BUFF * RIGHT, color=RED)

        self.play(FadeIn(A1))
        self.play(FadeIn(B0))
        self.play(FadeIn(words, lag_ratio=0.5), ShowCreation(arrow))
        self.wait()
    ...
    def shift_C_circle(self):
        ...
        words.shift(2.5 * UP)
        arrow = Arrow(words.get_bottom(), B.point_from_proportion(0.4), color=GREEN)

        self.play(
            FadeOut(A1),
            FadeOut(B0),
            self.in_A_out_B_words.fade,
            1,
            self.in_A_out_B_arrow.fade,
            1,
            FadeIn(B1),
    ...
    def show_A_not_C_region(self):
        ...
        self.play(
            FadeOut(B1),
            FadeOut(C0),
            self.in_B_out_C_words.fade,
            1,
            self.in_B_out_C_arrow.fade,
            1,
    ...
    def show_inequality_with_circle(self):
        ...
        groups = VGroup(
            *[
                VGroup(m2.copy(), m1.copy(), VGroup(*self.circles).copy())
    ...
    def emphasize_containment(self):
        ...
        self.play(FadeOut(c2), FadeOut(c1), Animation(foreground), run_time=2)
        self.wait()

    def write_50_percent(self):
        words = OldTexText("Should be 50\\% \\\\ of circle ", "A", "...somehow")
        words.scale(0.7)
        words.set_color_by_tex("A", RED)
...
class NoFirstMeasurementPreferenceBasedOnDirection(ShowVariousFilterPairs):
    ...
    def show_lines(self):
        ...
        for pf in self.pol_filters:
            ...
            VGroup(*random.sample(post_lines, self.n_lines / 2)).set_stroke(BLACK, 0)
            all_pre_lines.add(*pre_lines)
            all_post_lines.add(*post_lines)

        kwargs = {"rate_func": None, "lag_ratio": 0}
