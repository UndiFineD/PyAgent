...
class WhyPeopleMayKnowIt(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink()
        self.play(
            *it.chain(*[[pi.change_mode, "confused", pi.look_at, divergent_sum] for pi in self.get_pi_creatures()])
        )
        self.wait()
        self.random_blink()
    ...
    def get_definition(self, input_string, input_color=YELLOW):
        ...
        sum_terms = OldTex(
            *it.chain(
                *list(
                    zip(
                        ["\\frac{1}{%d^{%s}}" % (d, input_string) for d in range(1, 1 + num_shown_terms)],
                        it.cycle(["+"]),
                    )
                )
            )
...
class ComplexFunctionsAsTransformations(ComplexTransformationScene):
    ...
    def get_dots(self):
        ...
        arrows = VGroup(
            *[Arrow(in_dot, out_dot, buff=0.1, color=WHITE) for in_dot, out_dot in zip(input_dots, output_dots)]
        )
...
class VisualizingSSquared(ComplexTransformationScene):
    ...
    def show_transformation(self):
        ...
        self.apply_complex_homotopy(
            lambda z, t: z ** (1 + t),
            added_anims=[MoveAlongPath(dot, dot.path, run_time=5) for dot in self.input_dots],
            run_time=5,
        )
        self.wait(2)
...
class ShowZetaOnHalfPlane(ZetaTransformationScene):
    ...
    def set_color_i_line(self):
        right_i_lines, left_i_lines = [
            VGroup(
                *[Line(vert_vect + RIGHT, vert_vect + (FRAME_X_RADIUS + 1) * horiz_vect) for vert_vect in (UP, DOWN)]
            )
...
class IntroduceAnglePreservation(VisualizingSSquared):
    ...
    def note_grid_lines(self):
        intersection_inputs = [
            complex(x, y) for x in np.arange(-5, 5, 0.5) for y in np.arange(0, 3, 0.5) if not (x <= 0 and y == 0)
        ]
...
class DiscussZeros(ZetaTransformationScene):
    ...
    def give_dots_wandering_anims(self):
        ...
        for i, dot in enumerate(self.dots):
            ...
            dot.target.move_to(
                self.wandering_path.point_from_proportion((float(2 + 2 * i) / (4 * len(list(self.dots)))) % 1)
            )
...
class ZetaThumbnail(Scene):
    def construct(self):
        ...
        lines = VGroup(
            *(Line(plane.c2p(-7, y), plane.c2p(7, y)) for y in np.arange(-2, 2, 0.1) if y != 0),
            *(Line(plane.c2p(x, -4), plane.c2p(x, 4)) for x in np.arange(-2, 2, 0.1) if x != 0),
        )
