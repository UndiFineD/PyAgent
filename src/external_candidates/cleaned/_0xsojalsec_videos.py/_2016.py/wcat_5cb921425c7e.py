# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\wcat.py
...
class ClosedLoopScene(Scene):
    ...
    def get_default_loop(self):
        loop = VMobject()
        loop.set_points_smoothly(self.loop_anchor_points + [self.loop_anchor_points[0]])
        return loop
    ...
    def add_dots_at_alphas(self, *alphas):
        self.add_dots(*[Dot(self.loop.point_from_proportion(alpha), color=self.dot_color) for alpha in alphas])
    ...
    def add_connecting_lines(self, cyclic=False):
        ...
        for d1, d2 in pairs:
            ...
            line.update_anim = UpdateFromFunc(
                line,
                lambda l: l.put_start_and_end_on(l.start_dot.get_center(), l.end_dot.get_center()),
            )
    ...
    def get_line_anims(self):
        return [line.update_anim for line in self.connecting_lines] + [Animation(self.dots)]
    ...
    def let_dots_wonder(self, run_time=5, random_seed=None, added_anims=[]):
        ...
        anims = [
            MoveAlongPath(dot, self.loop, rate_func=generate_rate_func(start, rate))
    ...
    def find_square(self):
        alpha_quads = list(it.combinations(np.arange(0, 1, 0.02), 4))
        quads = np.array([[self.loop.point_from_proportion(alpha) for alpha in quad] for quad in alpha_quads])
    ...
    def square_scores(self, all_quads):
        ...
        dots = np.abs(
            unit_v1s[:, 0] * unit_v2s[:, 0] + unit_v1s[:, 1] * unit_v2s[:, 1] + unit_v1s[:, 2] * unit_v2s[:, 2]
        )
...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink(3)
        self.teacher_says("Here's why \\\\ I'm excited...", target_mode="hooray")
        for pi in self.get_students():
...
class WhenIWasAKid(TeacherStudentsScene):
    ...
    def grow_up(self):
        ...
        self.play(
            Transform(me, morty),
            *list(map(FadeOut, [self.students, self.teacher, me.bubble, me.bubble.content])),
        )
        self.remove(me)
        self.add(morty)
    ...
    def get_children(self):
        ...
        children = VGroup(*[BabyPiCreature(color=color) for color in colors])
...
class DefineInscribedSquareProblem(ClosedLoopScene):
    ...
    def draw_loop(self):
        ...
        self.wait()
        self.play(ShowCreation(self.loop, run_time=5, rate_func=linear))
        self.wait()
        ...
        self.play(
            ShowCreation(self.connecting_lines, lag_ratio=0, run_time=2),
            Animation(self.dots),
        )
        self.wait(2)
    ...
    def cycle_through_shapes(self):
        ...
        pi_loop.set_stroke(color=WHITE, width=DEFAULT_STROKE_WIDTH)
        ...
        self.wait()
        odd_eigths = np.linspace(1.0 / 8, 7.0 / 8, 4)
        ...
        self.wait()
        nudge = 0.055
        self.move_dots_to_alphas(odd_eigths + [nudge, -nudge, nudge, -nudge])
        self.wait(2)
        ...
        for anim in randy_anims:
            self.let_dots_wonder(run_time=1.5, random_seed=0, added_anims=[anim])
        self.remove(randy)
    ...
    def ask_about_rectangles(self):
        ...
        dot_pairs = [VGroup(self.dots[i], self.dots[j]) for i, j in [(0, 2), (1, 3)]]
        pair_colors = MAROON_B, PURPLE_B
        diag_lines = [Line(d1.get_center(), d2.get_center(), color=c) for (d1, d2), c in zip(dot_pairs, pair_colors)]
...
class RectangleProperties(Scene):
    def construct(self):
        ...
        vertex_dots = VGroup(*[Dot(anchor, color=YELLOW) for anchor in rect.get_anchors_and_handles()[0]])
        dot_pairs = [VGroup(vertex_dots[i], vertex_dots[j]) for i, j in [(0, 2), (1, 3)]]
        colors = [MAROON_B, PURPLE_B]
        diag_lines = [Line(d1.get_center(), d2.get_center(), color=c) for (d1, d2), c in zip(dot_pairs, colors)]
        ...
        a, b, c, d = labels = VGroup(
            *[OldTex(s).next_to(dot, dot.get_center(), buff=SMALL_BUFF) for s, dot in zip("abcd", vertex_dots)]
        )
        midpoint = Dot(ORIGIN, color=RED)

        self.play(ShowCreation(rect))
        self.wait()
        self.play(ShowCreation(vertex_dots), Write(labels))
        self.wait()
        mob_lists = [
            (a, c, dot_pairs[0]),
            (b, d, dot_pairs[1]),
        ...
        for color, mob_list in zip(colors, mob_lists):
            self.play(*[ApplyMethod(mob.set_color, color) for mob in mob_list])
            self.wait()
        for line, brace in zip(diag_lines, braces):
            self.play(ShowCreation(line), GrowFromCenter(brace))
            self.wait()
...
class PairOfPairBecomeRectangle(Scene):
    def construct(self):
        ...
        for dot, char in zip(dots, "acbd"):
            ...
            labels.add(label)
        lines = [
            Line(dots[i].get_center(), dots[j].get_center(), color=dots[i].get_color()) for i, j in [(0, 1), (2, 3)]
        ]
        groups = [
        ...
        self.wait()
        self.play(*[ApplyMethod(group.shift, -group[-1].get_center() + midpoint.get_center()) for group in groups])
        self.play(ShowCreation(midpoint), Write(words[0]))
        ...
        rectangle = Polygon(*[dots[i].get_center() for i in (0, 2, 1, 3)])
        rectangle.set_color(BLUE)
        self.play(ShowCreation(rectangle), Animation(dots))
...
class SearchForRectangleOnLoop(ClosedLoopScene):
    def construct(self):
        ...
        midpoint = Dot(center_of_mass([d.get_center() for d in self.dots]), color=RED)
        self.play(ShowCreation(midpoint))
        self.wait()
        angles = [line.get_angle() for line in self.connecting_lines]
        angle_mean = np.mean(angles)
        self.play(
            *[ApplyMethod(line.rotate, angle_mean - angle) for line, angle in zip(self.connecting_lines, angles)]
            + [Animation(midpoint)],
        ...
        self.wait()
        self.add_connecting_lines(cyclic=True)
        self.play(ShowCreation(self.connecting_lines), Animation(self.dots))
        self.wait()
...
class DeclareFunction(ClosedLoopScene):
    def construct(self):
        ...
        self.set_color_dots_by_pair()
        self.add_connecting_lines()
        VGroup(self.loop, self.dots, self.connecting_lines).scale(0.7).to_edge(LEFT).shift(DOWN)
...
class PairOfRealsToPlane(Scene):
    def construct(self):
        ...
        arrows = VGroup(*[Arrow(pair_label[i], dot, color=dot.get_color()) for i, dot in zip([1, 3], dots)])
        ...
        lines = VGroup(*[DashedLine(dot, two_d_point, color=dot.get_color()) for dot in dots])
        self.play(*list(map(ShowCreation, lines)))
        self.play(ShowCreation(two_d_point))
        everything = VGroup(*self.get_mobjects())
        self.play(FadeIn(plane), Animation(everything), Animation(dot2))
        self.wait()
...
class SeekSurfaceForPairs(ClosedLoopScene):
    def construct(self):
        ...
        anims = [ShowCreation(arrow), Write(words)]
        for anim in anims:
            self.let_dots_wonder(random_seed=1, added_anims=[anim], run_time=anim.run_time)
...
class AskAbouPairType(TeacherStudentsScene):
    def construct(self):
        self.student_says("""
            ...
            or unordered pairs?
        """)
        self.play(*[ApplyMethod(self.get_students()[i].change_mode, "confused") for i in (0, 2)])
        self.random_blink(3)
...
class DefineOrderedPair(ClosedLoopScene):
    def construct(self):
        ...
        self.wait()
        self.play(*[Transform(label.copy(), dot.label) for label, dot in zip(labels_start, dots)])
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(*[d.label for d in dots])
        self.wait()
        self.play(Write(first), ShowCreation(arrow))
        self.wait()
...
class DefineUnorderedPair(ClosedLoopScene):
    def construct(self):
        ...
        arrows = [Arrow(*pair, color=PURPLE_B) for pair in it.product(labels, dots)]
        ...
        for label, arrow_pair in zip(labels, arrow_pairs):
            ...
            self.wait()
        for x in range(2):
            self.play(dots[0].move_to, dots[1], dots[1].move_to, dots[0], path_arc=np.pi / 2)
            self.wait()
...
class DeformToInterval(ClosedLoopScene):
    def construct(self):
        ...
        self.wait()
        self.transform_loop(line, run_time=3, path_arc=np.pi / 2)
        self.wait()
        ...
        end_dots = VGroup(self.dots[0], self.dots[-1], dots_copy[0], dots_copy[-1])
        fading_dots.remove(*end_dots)

        self.play(Write(self.dots))
        self.add(dots_copy)
        self.wait()
        self.transform_loop(line, added_anims=[MoveToTarget(dot_at_1)], run_time=3)
        self.wait()
        self.loop = original_loop
        self.dots = dots_copy
        dot_at_1 = self.dots[-1]
        dot_at_1.target.move_to(cut_loop.get_points()[-1])
        self.transform_loop(cut_loop, added_anims=[MoveToTarget(dot_at_1)])
        self.wait()
        fading_dots.generate_target()
        fading_dots.target.set_fill(opacity=0.3)
        self.play(MoveToTarget(fading_dots))
        self.play(end_dots.shift, 0.2 * UP, rate_func=wiggle)
        self.wait()
...
class RepresentPairInUnitSquare(ClosedLoopScene):
    def construct(self):
        ...
        square.move_to(interval.get_left(), aligned_edge=DOWN + LEFT)
        ...
        self.play(Write(self.dots), Write(right_words[0]))
        self.wait()
        self.transform_loop(line)
        self.play(ShowCreation(interval), Write(numbers), Animation(self.dots))
        self.wait()
        self.play(*[Rotate(mob, np.pi / 2, about_point=interval.get_left()) for mob in (vert_interval, self.dots[1])])

        # Find interior point
        point = self.dots[0].get_center()[0] * RIGHT
        point += self.dots[1].get_center()[1] * UP
        inner_dot = Dot(point, color=YELLOW)
        dashed_lines = VGroup(*[DashedLine(dot, inner_dot, color=dot.get_color()) for dot in self.dots])
        self.play(ShowCreation(dashed_lines))
        self.play(ShowCreation(inner_dot))
        self.play(FadeIn(square), Animation(self.dots), *list(map(Write, right_words[1:])))
        self.wait()
        ...
        for shift_val in shift_vals:
            ...
            for line, dot in zip(dashed_lines, self.dots):
                line.target.put_start_and_end_on(dot.target.get_center(), inner_dot.target.get_center())
...
class EdgesOfSquare(Scene):
    ...
    def add_square(self):
        ...
        square.set_stroke(width=0)
        square.set_fill(color=BLUE, opacity=0.3)
        square.move_to(bottom_left, aligned_edge=DOWN + LEFT)
    ...
    def get_coordinate_labels(self, square):
        ...
        label_groups = [VGroup(*[OldTex("(%s, %s)" % (a, b)).scale(0.7) for b in alpha_range]) for a in (0, 1)]
...
class EndpointsGluedTogether(ClosedLoopScene):
    def construct(self):
        ...
        self.loop = line
        dots = VGroup(*[Dot(line.get_bounding_box_point(vect)) for vect in (LEFT, RIGHT)])
        dots.set_color(BLUE)
        ...
        self.transform_loop(
            original_loop,
            added_anims=[ApplyMethod(dot.move_to, original_loop.get_points()[0]) for dot in dots],
            run_time=3,
        )
        self.wait()
...
class WhatAboutUnordered(TeacherStudentsScene):
    def construct(self):
        self.student_says("What about \\\\ unordered pairs?")
...
class TrivialPairCollision(ClosedLoopScene):
    def construct(self):
        ...
        line = Line(a.get_corner(DOWN + LEFT), b.get_corner(UP + RIGHT), color=MAROON_B)
        ...
        for label in a_label, b_label:
            self.play(Write(label, run_time=1), randy.look_at, label)
        ...
        self.play(randy.look_at, a, *[m.restore for m in swappers], path_arc=-np.pi)
        self.play(Blink(randy))
        self.wait()
...
class FoldUnitSquare(EdgesOfSquare):
    ...
    def add_triangles(self):
        ...
        triangles.set_fill(color=square.get_color(), opacity=square.get_fill_opacity())
        self.remove(square)
        self.square = square
        self.add(triangles)
    ...
    def add_arrows(self):
        ...
        for a in 0, 1:
            for color in colors:
                ...
                for b1, b2 in zip(b_range, b_range[1:]):
                    ...
                    if color is BLUE:
                        arrow.rotate(-np.pi / 2, about_point=self.square.get_center())
                    if a is 0:
        ...
        self.end_arrows.set_color(color_gradient([MAROON_B, BLUE], 3)[1])
    ...
    def show_singleton_pairs(self):
        ...
        for x in xs:
            ...
            if old_label is None:
                self.play(ShowCreation(dot), Write(label))
                old_label = label
                old_dot = dot
            else:
                self.play(
    ...
    def ask_about_gluing(self):
        keepers = VGroup(self.triangles[0], self.start_arrows, self.diag_line).copy()
        ...
        self.play(FadeOut(faders), Animation(keepers))
        self.play(keepers.scale, 0.6, keepers.shift, 4 * RIGHT + UP, FadeIn(randy))
        self.play(
            randy.change_mode,
            "pondering",
            randy.look_at,
    ...
    def clarify_edge_gluing(self):
        ...
        for dot in dots[:2]:
            ...
            self.play(ShowCreation(dot), self.randy.look_at, dot)
        self.wait()
        for dot in dots[2:]:
    ...
    def get_point_from_coords(self, x, y):
        left, right, bottom, top = [self.triangles.get_edge_center(vect) for vect in (LEFT, RIGHT, DOWN, UP)]
...
class PrepareForMobiusStrip(Scene):
    ...
    def add_triangles(self):
        ...
        for tri in triangles:
            ...
            dashed_line = DashedLine(anchors[i], anchors[j], color=RED)
            tri.add(dashed_line)
    ...
    def rearrange_pieces(self):
        ...
        self.wait()
        self.play(*[ApplyMethod(tri.shift, tri.get_points()[0][0] * LEFT) for tri in self.triangles])
        self.play(*[FadeOut(tri.original_arrows) for tri in self.triangles])
        for tri in self.triangles:
...
class MobiusToPairToSurface(ClosedLoopScene):
    def construct(self):
        ...
        self.play(ShowCreation(top_arrow), randy.look_at, self.loop)
        self.wait()
        ...
        self.wait()
        self.play(ShowCreation(right_arrow), randy.look_at, surface_dot)
        self.play(Blink(randy))
...
class TryItYourself(TeacherStudentsScene):
    def construct(self):
        ...
        bubble = pi.get_bubble("thought", width=4, height=4, direction=RIGHT)
        bubble.set_fill(BLACK, opacity=1)
        bubble.write("Orientation seem\\\\ to matter...")
        self.play(FadeIn(bubble), Write(bubble.content))
        self.random_blink(3)
...
class CreditTWo(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(Blink(morty))
        self.play(FadeIn(headphones), Write(url), Animation(morty))
        self.play(morty.change_mode, "happy")
        self.wait()
        self.play(Blink(morty))
        self.wait()
        self.play(FadeIn(brother), morty.look_at, brother.eyes)
        self.play(brother.change_mode, "surprised")
        self.play(Blink(brother))
        self.wait()
        self.play(morty.look, LEFT, brother.change_mode, "happy", brother.look, LEFT)
        self.play(Blink(morty))
        self.wait()
...
class CreditThree(Scene):
    def construct(self):
        ...
        self.play(Blink(randy))
        self.play(randy.change_mode, "happy", randy.look_at, logo_dot)
        self.wait()
        self.play(ShowCreation(bubble), randy.change_mode, "pondering", run_time=2)
        self.play(Blink(randy))
        self.play(
        ...
        self.wait()
        self.play(Write(domains, run_time=5), randy.look_at, domains)
        self.wait()
        self.play(Blink(randy))
        self.play(randy.change_mode, "hooray", randy.look_at, logo_dot, FadeOut(domains))
        self.wait()
        self.play(Write(whois), randy.change_mode, "confused", randy.look_at, whois)
        self.wait(2)
