# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter5.py
...
class Blob(Circle):
    ...
    def __init__(self, **kwargs):
        ...
        self.apply_complex_function(lambda z: z * (1 + self.random_nudge_size * (random.random() - 0.5)))
...
class MovingForward(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(ShowCreation(bubble), student.change_mode, "pondering")
        self.wait(2)
...
class SquishingTransformation(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.apply_transposed_matrix([[1.0 / 2, -0.5], [1, 1.0 / 3]])
        self.wait()
...
class AskAboutStretching(LinearTransformationScene):
    def construct(self):
        ...
        self.apply_transposed_matrix([[2, 1], [-1, 3]], added_anims=[Write(words)])
        self.wait()
...
class DiagonalExample(LinearTransformationScene):
    ...
    def construct(self):
        ...
        if self.show_square:
            ...
            equation = VMobject(words, width_target, times, height_target, eq_det)
...
class ThisSquareTellsEverything(LinearTransformationScene):
    def construct(self):
        ...
        words.set_color(YELLOW)
        words.add_background_rectangle()
        arrow = Arrow(words.get_bottom(), self.square.get_right(), color=WHITE)
...
class WhatHappensToOneSquareHappensToAll(LinearTransformationScene):
    def construct(self):
        ...
        for position, side_length in pairs:
            ...
            squares.add(square)
        self.play(FadeIn(squares, lag_ratio=0.5, run_time=3))
...
class BreakBlobIntoGridSquares(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add_transformable_mobject(blob)
        arange = np.arange(0, self.blob_height + self.square_size, self.square_size)
...
class NameDeterminant(LinearTransformationScene):
    ...
    def construct(self):
        ...
        if np.round(det) == det:
            det = int(det)
        area_label_target = VMobject(OldTex(str(det)), OldTex("\\cdot"), A.copy())
        if det < 1 and det > 0:
        ...
        new_det_mob = det_mob_copy.copy().set_height(det_text.split()[0].get_height())
...
class SecondDeterminantExample(NameDeterminant):
    CONFIG = {"t_matrix": [[-1, -1], [1, -1]]}


class DeterminantIsThree(NameDeterminant):
    CONFIG = {"t_matrix": [[0, -1.5], [2, 1]]}
...
class SecondDeterminantIsZeroExamlpe(NameDeterminant):
    CONFIG = {"t_matrix": [[0, 0], [0, 0]], "show_basis_vectors": False}
...
class NextFewVideos(Scene):
    def construct(self):
        ...
        icon.center()
        icon.set_width(FRAME_WIDTH / 12.0)
        ...
        self.play(FadeIn(icons, lag_ratio=0.5), run_time=3)
        self.wait()
...
class FlipSpaceOver(Scene):
    def construct(self):
        ...
        plane2 = NumberPlane(y_radius=FRAME_X_RADIUS, color=RED_D, secondary_color=RED_E)
        axis = UP
        ...
        self.play(ShowCreation(plane1, lag_ratio=0.5, run_time=1))
        self.wait()
        self.play(
            Rotate(
                plane1,
...
class RandyThinking(Scene):
    def construct(self):
        ...
        self.play(randy.change_mode, "pondering", ShowCreation(bubble))
        self.wait()
...
class WriteNegativeDeterminant(NegativeDeterminantTransformation):
    def construct(self):
        ...
        det_text = get_det_text(matrix, determinant=np.linalg.det(self.t_matrix))
...
class WhyNegativeScaling(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*[ApplyMethod(student.change_mode, "confused") for student in other_students])
        self.random_blink()
        self.wait()
        self.random_blink()
...
class DeterminantGraphForRotatingIHat(Scene):
    def construct(self):
        ...
        det_axis = NumberLine(big_tick_numbers=[], x_min=-2, x_max=2, color=WHITE)
...
class DeterminantIsVolumeOfParallelepiped(Scene):
    def construct(self):
        ...
        words = OldTexText(["Volume of this\\\\", "parallelepiped"])
        pp = words.split()[1]
...
class RightHandRule(Scene):
    ...
    def construct(self):
        ...
        v1_label, v2_label, v3_label = [OldTex(label_tex).scale(1.5) for label_tex in self.labels_tex]
...
class TwoDDeterminantFormula(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(*[Transform(m, m.zero) for m in (mb, mc, b, c)])
        self.wait()
        for pair in (mb, b), (mc, c):
            self.play(*[Transform(m, m.original) for m in pair])
            self.wait()
...
class TwoDDeterminantFormulaIntuition(LinearTransformationScene):
    def construct(self):
        ...
        self.apply_transposed_matrix(
            [[1, 0], [float(b) / d, 1]],
            added_anims=[ApplyMethod(m.shift, b * RIGHT) for m in (side_brace, height)],
        )
        self.wait()
...
class FullFormulaExplanation(LinearTransformationScene):
    ...
    def add_polygons(self):
        ...
        shapes_colors_and_tex = [
            ...
            (
                Polygon(a + c, a + b + c, a + b + c, a + b + c + d),
                TEAL,
                "\\dfrac{bd}{2}",
            ),
        ...
        self.play(FadeIn(everyone, lag_ratio=0.5, run_time=1))
...
class ProductProperty(Scene):
    def construct(self):
        lhs = OldTex(["\\text{det}(", "M_1", "M_2", ")"])
        det, m1, m2, rp = lhs.split()
        m1.set_color(TEAL)
        m2.set_color(PINK)

        rhs = OldTex(["=\\text{det}(", "M_1", ")\\text{det}(", "M_2", ")"])
