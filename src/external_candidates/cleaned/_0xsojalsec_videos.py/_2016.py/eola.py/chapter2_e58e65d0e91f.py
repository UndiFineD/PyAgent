# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter2.py
...
class CoordinatesAsScalars(VectorScene):
    ...
    def general_idea_of_scalars(self, array, vector):
        ...
        new_i_hat = Vector(self.vector_coords[0] * i_hat.get_end(), color=X_COLOR)
        new_j_hat = Vector(self.vector_coords[1] * j_hat.get_end(), color=Y_COLOR)
        VMobject(i_hat, new_i_hat).shift(3 * LEFT)
        VMobject(j_hat, new_j_hat).shift(3 * RIGHT)

        new_array = Matrix([new_x.copy(), new_y.copy()])
        new_array.scale(0.5)
        new_array.shift(-new_array.get_boundary_point(-vector.get_end()) + 1.1 * vector.get_end())
    ...
    def scale_basis_vectors(self, new_array):
        ...
        i_hat_label = self.label_vector(i_hat, "\\hat{\\imath}", color=X_COLOR, label_scale_factor=1)
        self.add_vector(j_hat)
        j_hat_label = self.label_vector(j_hat, "\\hat{\\jmath}", color=Y_COLOR, label_scale_factor=1)
        self.wait()
        ...
        for coord, v, label, factor, shift_right in [
            ...
            if shift_right:
                ...
                self.play(ApplyMethod(group.shift, self.vector_coords[0] * RIGHT))
        self.wait()
...
class WhatIfWeChoseADifferentBasis(Scene):
    def construct(self):
        self.play(Write("What if we chose different basis vectors?", run_time=2))
        self.wait(2)
...
class ShowVaryingLinearCombinations(VectorScene):
    ...
    def construct(self):
        ...
        v1_label = self.label_vector(v1, self.vector1_label, color=self.vector1_color, buff_factor=3)
        v2_label = self.label_vector(v2, self.vector2_label, color=self.vector2_color, buff_factor=3)
        label_anims = [MaintainPositionRelativeTo(label, v) for v, label in [(v1, v1_label), (v2, v2_label)]]
    ...
    def get_scalar_anims(self, v1, v2, v1_label, v2_label):
        def get_val_func(vect):
            ...
            return lambda a: np.dot(original_vect, vect.get_end() - vect.get_start()) / square_norm

        return [
            RangingValues(
                tracked_mobject=label,
                tracked_mobject_next_to_kwargs={"direction": LEFT, "buff": 0.1},
                scale_factor=0.75,
    ...
    def get_rate_func_pair(self):
        return [squish_rate_func(smooth, a, b) for a, b in [(0, 0.7), (0.3, 1)]]

    def initial_scaling(self, v1, v2, label_anims, scalar_anims):
        scalar_pair = self.scalar_pairs.pop(0)
        anims = [
            ApplyMethod(v.scale, s, rate_func=rf) for v, s, rf in zip([v1, v2], scalar_pair, self.get_rate_func_pair())
        ]
        anims += [ApplyMethod(v.copy().fade, 0.7) for v in (v1, v2)]
    ...
    def show_sum(self, v1, v2, label_anims, scalar_anims):
        self.play(ApplyMethod(v2.shift, v1.get_end()), *label_anims + scalar_anims)
        self.sum_vector = self.add_vector(v2.get_end(), color=self.sum_color)
        self.wait()
    ...
    def get_sum_animations(self, v1, v2):
        v2_anim = UpdateFromFunc(v2, lambda m: m.shift(v1.get_end() - m.get_start()))
    ...
    def standard_basis_comparison(self, label_anims, scalar_anims):
        ...
        array = Matrix(
            [mob.copy().set_color(color) for mob, color in zip(alt_coords, [self.vector1_color, self.vector2_color])]
        )
        array.scale(0.8)
        array.to_edge(UP)
        array.shift(RIGHT)
        ...
        anims = [Transform(*pair) for pair in zip(alt_coords, array.get_mob_matrix().flatten())]
        # anims += [
        #     FadeOut(a.mobject)
        ...
        ij_array, x_line, y_line = self.vector_to_coords(self.sum_vector, integer_labels=False)
        ...
        neq = OldTex("\\neq")
        neq.next_to(array)
        self.play(ApplyMethod(ij_array.next_to, neq), Write(neq))
        self.wait()
    ...
    def draw_lines(self, v1, v2, label_anims, scalar_anims):
        ...
        for w1, w2 in (v2, v1), (v1, v2):
            ...
            for num in scale_factor, -1, -1.0 / scale_factor:
                ...
                self.play(
                    ...
                    UpdateFromFunc(
                        line,
                        lambda l: l.put_start_and_end_on(curr_tip, self.sum_vector.get_end()),
                    ),
                    *aux_anims,
                )
                self.add(line, v2)
...
class NameLinearCombinations(Scene):
    def construct(self):
        ...
        equation = OldTex(["a", "\\vec{\\textbf{v}}", "+", "b", "\\vec{\\textbf{w}}"])
...
class DefineSpan(Scene):
    def construct(self):
        ...
        vary_words = OldTexText("Let $a$ and $b$ vary \\\\ over all real numbers")
...
class VectorsToDotsScene(VectorScene):
    ...
    def construct(self):
        ...
        colors = Color(self.start_color).range_to(self.end_color, len(vectors))
        ...
        self.play(ShowCreation(vector_group, run_time=3))
        ...
        rate_functions = [squish_rate_func(smooth, float(x) / (len(vectors) + 2), 1) for x in range(len(vectors))]
        self.play(*[Transform(v, v_to_dot(v), rate_func=rf, run_time=2) for v, rf in zip(vectors, rate_functions)])
        self.wait()
...
class VectorsOnALine(VectorsToDotsScene):
    def get_vectors(self):
        return [Vector(a * np.array([1.5, 1])) for a in np.linspace(-FRAME_Y_RADIUS, FRAME_Y_RADIUS, self.num_vectors)]

    def play_final_animation(self, vectors, rate_functions):
        line_copies = [Line(vectors[0].get_end(), vectors[-1].get_end()) for v in vectors]
        self.play(
            *[Transform(v, mob, rate_func=rf, run_time=2) for v, mob, rf in zip(vectors, line_copies, rate_functions)]
        )
...
class VectorsInThePlane(VectorsToDotsScene):
    ...
    def play_final_animation(self, vectors, rate_functions):
        ...
        v_line = Line(FRAME_Y_RADIUS * UP, FRAME_Y_RADIUS * DOWN, stroke_width=0.5, color=BLUE_E)
        line_pairs = [
            VMobject(h_line.copy().shift(y), v_line.copy().shift(x)) for v in vectors for x, y, z in [v.get_center()]
        ]
        plane = NumberPlane()
        self.play(
            ShowCreation(plane),
            *[Transform(v, p, rate_func=rf) for v, p, rf in zip(vectors, line_pairs, rate_functions)],
        )
        self.remove(*vectors)
        self.wait()
...
class HowToThinkVectorsVsPoint(Scene):
    def construct(self):
        ...
        vector_group = VMobject(*[Vector([x, 1]) for x in np.linspace(-2, 2, 5)])
        ...
        self.add(randy)
        self.play(ApplyMethod(randy.change_mode, "pondering"), ShowCreation(bubble))
        self.play(FadeIn(text1))
        self.play(ShowCreation(single_vector))
        self.wait(3)
        self.play(Transform(text1, text2), Transform(single_vector, vector_group))
...
class LinearCombinationOfThreeVectorsText(Scene):
    def construct(self):
        ...
        arrows = VMobject(*[Arrow(span_comment, var) for var in (a, b, c)])

        self.play(Write(text))
        self.play(Write(equation))
        self.wait()
        self.play(ShowCreation(arrows), Write(span_comment))
        self.wait()
...
class AlternateDefOfLinearlyDependent(Scene):
    def construct(self):
        ...
        added_words1 = OldTexText("where at least one of $a$, $b$ and $c$ is not $0$")
        added_words2 = OldTexText("is a = b = c = 0")

        scalar_specification = OldTexText("For some choice of $a$ and $b$")
        scalar_specification.shift(1.5 * DOWN)
        scalar_specification.add(*[Arrow(scalar_specification, equations[0].split()[i]) for i in (2, 5)])
        ...
        for i, new_eq in enumerate(equations):
            if i == 0:
                ...
                self.play(GrowFromCenter(brace), Write(brace_words))
                self.wait(3)
                self.play(FadeOut(brace), FadeOut(brace_words))
            self.play(Transform(equation, new_eq, path_arc=(np.pi / 2 if i == 1 else 0)))
            self.wait(3)
        ...
        self.play(ApplyFunction(lambda m: m.scale(0.5).to_corner(UP + LEFT), everything))
        self.wait()
...
class MathematiciansLikeToConfuse(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*[ApplyMethod(student.change_mode, mode) for student, mode in zip(self.get_students(), modes)])
        self.wait(2)
