...
class IntroducePolynomialSpace(Scene):
    ...
    def split_individual_polynomial(self):
        ...
        self.play(
            ...
            Write(brace.text),
            *[ApplyMethod(self.poly1[index].set_color, color) for index, color in list(index_to_color.items())],
        )
        self.wait()
        self.brace = brace
    ...
    def derivative_as_matrix(self):
        matrix = Matrix(
            [[str(j) if j == i + 1 else "0" for j in range(4)] + ["\\cdots"] for i in range(4)]
            + [["\\vdots"] * 4 + ["\\ddots"]]
        )
        matrix.shift(2 * LEFT)
...
class ShowVectorSpaces(Scene):
    def construct(self):
        ...
        v_lines = [
            Line(h_line.get_center(), FRAME_Y_RADIUS * DOWN).shift(vect * FRAME_X_RADIUS / 3.0)
    ...
    def get_vectors(self, n_vectors=10):
        vectors = VGroup(
            *[
                ...
                for scalar, angle in zip(2 * np.random.random(n_vectors) + 0.5, np.linspace(0, 6, n_vectors))
            ]
        )
...
class VectorSpaceOfPiCreatures(Scene):
    ...
    def add_creatures(self):
        creatures = VGroup(
            *[VGroup(*[PiCreature() for x in range(4)]).arrange(RIGHT, buff=1.5) for y in range(4)]
...
class WhatIsThree(Scene):
    def construct(self):
        ...
        triplets = [
            VGroup(*[PiCreature(color=color).scale(0.4) for color in (BLUE_E, BLUE_C, BLUE_D)]),
