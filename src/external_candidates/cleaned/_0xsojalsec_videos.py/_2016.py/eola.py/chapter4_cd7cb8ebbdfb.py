# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter4.py
...
class RecapTime(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(ApplyFunction(lambda m: m.change_mode("pondering").look(LEFT), student))
        self.play(Blink(student))
        self.wait()
...
class DeterminedByTwoBasisVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add_transformable_label(i_hat, "\\hat{\\imath}", "\\hat{\\imath}", color=X_COLOR)
        j_hat = self.add_vector([0, 1], color=Y_COLOR)
        self.add_transformable_label(j_hat, "\\hat{\\jmath}", "\\hat{\\jmath}", color=Y_COLOR)
...
class FollowLinearCombination(LinearTransformationScene):
    ...
    def show_scaled_vectors(self, vect_array, vect_coords, i_label, j_label):
        ...
        self.play(*[ApplyMethod(mob.shift, scaled_i.get_end()) for mob in (scaled_j, scaled_j_label)])
        self.wait()
        self.play(
            *list(
                map(
    ...
    def record_basis_coordinates(self, vect_array, vect):
        ...
        self.wait()
        x, y = vect_array.get_entries().split()
        pre_formula = VMobject(x, i_label, OldTex("+"), y, j_label)
...
class IntroduceIdeaOfComposition(RotationThenShear):
    ...
    def show_composition(self):
        words = OldTexText(["``Composition''", "of a", "rotation", "and a", "shear"])
    ...
    def track_basis_vectors(self):
        ...
        words = OldTexText(["Record where", "$\\hat{\\imath}$", "and", "$\\hat{\\jmath}$", "land:"])
        ...
        col1, col2 = [VMobject(*matrix.get_mob_matrix()[:, i]) for i in (0, 1)]
        ...
        self.wait()
        self.play(ApplyMethod(j_coords.copy().get_entries().move_to, col2))
        self.wait()
    ...
    def show_overall_effect(self, matrix):
        everything = self.get_mobjects()
        everything = list_difference_update(everything, matrix.get_family())
        ...
        new_bases = VMobject(*[Vector(func(v.get_end()), color=v.get_color()) for v in bases.split()])
        self.play(
...
class ExplainWhyItsMatrixMultiplication(Scene):
    def construct(self):
        ...
        self.play(ApplyMethod(matrices.arrange, buff=0.1, aligned_edge=UP))
        self.wait()
...
class MoreComplicatedExampleVisually(LinearTransformationScene):
    ...
    def get_matrices(self):
        ...
        VMobject(m2_mob, m1_mob, comp_matrix).arrange(buff=0.1).to_corner(UP + LEFT).shift(DOWN)
...
class MoreComplicatedExampleNumerically(MoreComplicatedExampleVisually):
    ...
    def construct(self):
        ...
        col1, col2 = [VMobject(*m1_mob.split()[1].get_mob_matrix()[:, i]) for i in (0, 1)]
        ...
        for i, (col, char, color) in enumerate(triplets):
            ...
            question = OldTexText("Where does $\\hat{\\%smath}$ go?" % char)
            ...
            self.play(
                *[FadeOut(mob) for mob in self.get_mobjects() if mob not in start_state] + [Animation(product_entries)]
            )
            self.wait()
...
class GeneralMultiplication(MoreComplicatedExampleNumerically):
    def get_result(self):
        entries = list(map(Tex, ["ae+bg", "af+bh", "ce+dg", "cf+dh"]))
        for mob in entries:
    ...
    def get_matrices(self):
        ...
        self.play(Transform(m1_entries, m1_entries_target, lag_ratio=0.5))
        self.play(Transform(m2_entries, m2_entries_target, lag_ratio=0.5))
        self.wait()
...
class RoteMatrixMultiplication(NumericalMatrixMultiplication):
    CONFIG = {"left_matrix": [[-3, 1], [2, 5]], "right_matrix": [[5, 3], [7, -3]]}
...
class NeverForget(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(ApplyPointwiseFunction(warp, VMobject(*self.get_mobjects())))


class AskAboutCommutativity(Scene):
    def construct(self):
        l_m1, l_m2, eq, r_m2, r_m1 = OldTex(["M_1", "M_2", "=", "M_2", "M_1"]).scale(1.5).split()
        ...
        self.wait()
        self.play(Transform(VMobject(eq, q_marks), VMobject(neq), lag_ratio=0.5))
        self.wait()


class ShowShear(LinearTransformationScene):
    CONFIG = {"title": "Shear", "title_color": PINK, "t_matrix": [[1, 0], [1, 1]]}
...
class AskAssociativityQuestion(Scene):
    ...
    def show_full_matrices(self, morty, a, b, c, title):
        ...
        matrices = list(
            map(
                matrix_to_mobject,
                [np.array(list(m)).reshape((2, 2)) for m in ("abcd", "efgh", "ijkl")],
            )
        )
        ...
        for letter, matrix in zip([a, b, c], matrices):
            self.play(Transform(letter.copy(), matrix, lag_ratio=0.5))
    ...
    def move_matrix_parentheses(self, morty, matrices):
        ...
        lp, rp = parens.split()
        state1 = VMobject(VectorizedPoint(m1.get_left()), m1, m2, VectorizedPoint(m2.get_right()), m3)
        state2 = VMobject(*[m.copy() for m in (lp, m1, m2, rp, m3)])
        state3 = VMobject(*[m.copy() for m in (m1, lp, m2, m3, rp)])
...
class ThreeSuccessiveTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for t_matrix, sym in zip(self.t_matrices, [c, b, a]):
            self.play(brace.next_to, sym, DOWN, sym.set_color, YELLOW, last.set_color, WHITE)
...
class ExplanationTrumpsProof(Scene):
    def construct(self):
        ...
        self.play(Write(explanation), Write(greater), Write(proof), run_time=1)
        self.play(explanation.scale, 1.5, proof.scale, 0.7)
        self.wait()
...
class GoPlay(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*[ApplyMethod(student.change_mode, "happy") for student in self.get_students()])
        self.random_blink()
