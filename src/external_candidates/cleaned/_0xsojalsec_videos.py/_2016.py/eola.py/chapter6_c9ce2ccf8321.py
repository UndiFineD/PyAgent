# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter6.py
...
class OpeningQuote(Scene):
    def construct(self):
        words = OldTexText("To ask the", "right question\\\\", "is harder than to answer it.")
        words.to_edge(UP)
...
class NoComputations(TeacherStudentsScene):
    def construct(self):
        self.setup()
        self.student_says("Will you cover \\\\ computations?", target_mode="raise_left_hand")
        self.random_blink()
        self.teacher_says(
            "Well...uh...no",
            target_mode="guilty",
        ...
        self.play(
            *[
                ...
                for student, mode in zip(self.get_students(), ["dejected", "confused", "angry"])
            ]
        )
        self.random_blink()
        self.wait()
        ...
        self.play(*[ApplyMethod(student.change_mode, "pondering") for student in self.get_students()])
        self.random_blink()
...
class UsefulnessOfMatrices(Scene):
    def construct(self):
        ...
        new_syms = VMobject(*[m.copy().set_color(c) for m, c in zip(syms.split(), [X_COLOR, Y_COLOR, Z_COLOR])])
...
class StockLine(VMobject):
    CONFIG = {"num_points": 15, "step_range": 2}
...
class StockPrices(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(VMobject(*stock_lines), run_time=3, lag_ratio=0.5))
        self.wait()
...
class MachineLearningNetwork(Scene):
    def construct(self):
        ...
        for i, num_nodes in enumerate([3, 4, 4, 1]):
            layer = VMobject(*[Circle(radius=0.5, color=YELLOW) for x in range(num_nodes)])
...
class SystemOfEquations(Scene):
    ...
    def show_linearity_rules(self, equations):
        ...
        scaled_vars = VMobject(*[VMobject(*top_equation.split()[3 * i : 3 * i + 2]) for i in range(3)])
        ...
        self.play(*[ApplyMethod(m.scale, 1.2, rate_func=there_and_back) for m in scalars.split()])
        self.wait()
        self.remove(scalars)
        self.play(scaled_vars.restore)
        self.play(*[ApplyMethod(p.scale, 1.5, rate_func=there_and_back) for p in plusses])
        self.wait()
    ...
    def describe_organization(self, equations):
        variables = VMobject(*it.chain(*[eq.split()[:-2] for eq in equations.split()]))
        variables.words = "Throw variables on the left"
        constants = VMobject(*[eq.split()[-1] for eq in equations.split()])
        constants.words = "Lingering constants on the right"
        xs, ys, zs = [VMobject(*[eq.split()[i] for eq in equations.split()]) for i in (1, 4, 7)]
        ...
        zero_circles = VMobject(
            *[
                ...
                for mob in [VMobject(*equations.split()[i].split()[j : j + 2]) for i, j in [(1, 3), (2, 6)]]
            ]
        )
        ...
        for mob in variables, constants, ys:
            self.play(FadeIn(mob.square), FadeIn(mob.words))
            self.wait()
            self.play(*list(map(FadeOut, [mob.square, mob.words])))
        self.play(ShowCreation(zero_circles), Write(zero_circles.words, run_time=1))
        self.wait()
    ...
    def factor_into_matrix(self, equations):
        coefficients = np.array([np.array(eq.split())[[0, 3, 6]] for eq in equations.split()])
        variable_arrays = np.array([np.array(eq.split())[[1, 4, 7]] for eq in equations.split()])
        rhs_entries = np.array([eq.split()[-1] for eq in equations.split()])
        ...
        all_brackets = [mob.get_brackets() for mob in (matrix, x_array, v_array)]
        ...
        self.play(
            *it.chain(
                *[
                    [
                        Transform(m1.copy(), m2, run_time=2, path_arc=-np.pi / 2)
                        for m1, m2 in zip(start_array.flatten(), matrix_mobject.get_entries().split())
                    ]
        ...
        self.play(*[Write(mob) for mob in all_brackets + [equals]])
        self.wait()
    ...
    def label_matrix_product(self, matrix, x_array, v_array):
        ...
        x_array.words.set_submobject_colors_by_gradient(X_COLOR, Y_COLOR, Z_COLOR)
        x_array.symbol.set_color(PINK)
        v_array.symbol.set_color(YELLOW)
        for mob in parts:
            self.play(GrowFromCenter(mob.brace), FadeIn(mob.words))
            self.wait()
            self.play(*list(map(FadeOut, [mob.brace, mob.words])))
        self.wait()
        for mob in parts:
            self.play(FadeIn(mob.brace), Write(mob.symbol))
        compact_equation = VMobject(*[mob.symbol for mob in parts])
        compact_equation.submobjects.insert(2, OldTex("=").next_to(x_array, RIGHT))
        compact_equation.target = compact_equation.copy()
        compact_equation.target.arrange(buff=0.1)
        compact_equation.target.to_edge(UP)

        self.play(Transform(compact_equation.copy(), compact_equation.target))
        self.wait()
...
class MentionThatItsATransformation(LinearSystemTransformationScene):
    CONFIG = {"t_matrix": np.array([[2, 1], [2, 3]])}
...
class LookForX(MentionThatItsATransformation):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(v), Write(v_label))
        self.add_foreground_mobject(v_label)
        x = self.add_vector(x, animate=False)
        self.play(ShowCreation(x), Write(x_label))
        self.wait()
...
class ThinkAboutWhatsHappening(Scene):
    def construct(self):
        ...
        self.play(
            ApplyFunction(lambda m: m.shift(2 * DOWN).scale(5), everything),
            bubble.content.set_color,
            BLACK,
            run_time=2,
        )
...
class SystemOfTwoEquationsTwoUnknowns(Scene):
    def construct(self):
        ...
        matrix_system = VMobject(matrix, x, OldTex("="), v)
...
class ShowBijectivity(LinearTransformationScene):
    CONFIG = {"show_basis_vectors": False, "t_matrix": np.array([[0, -1], [2, 1]])}
    ...
    def construct(self):
        ...
        vectors = VMobject(
            *[
                ...
                for x, y in it.product(
                    *[np.arange(-int(val) + 0.5, int(val) + 0.5) for val in (FRAME_X_RADIUS, FRAME_Y_RADIUS)]
                )
            ]
        )
        vectors.set_submobject_colors_by_gradient(BLUE_E, PINK)
        dots = VMobject(*[Dot(v.get_end(), color=v.get_color()) for v in vectors.split()])
        titles = [
            OldTexText(["Each vector lands on\\\\", "exactly one vector"]),
            OldTexText(["Every vector has \\\\", "been landed on"]),
        ]
        for title in titles:
            title.to_edge(UP)
        ...
        kwargs = {"lag_ratio": 0.5, "run_time": 2}
        ...
        self.wait()
        self.apply_transposed_matrix(np.linalg.inv(self.t_matrix.T).T)
        self.wait()
...
class FullRankExmapleWithWords(LabeledExample):
    CONFIG = {"title": "$A$ keeps things 2D", "t_matrix": [[3, 0], [2, 1]]}
...
class StartWithNonzeroDetCase(TeacherStudentsScene):
    def construct(self):
        words = OldTexText("Let's start with \\\\", "the", "$\\det(A) \\ne 0$", "case")
        words[2].set_color(TEAL)
        self.teacher_says(words)
        self.random_blink()
        self.play(random.choice(self.get_students()).change_mode, "happy")
        self.wait()


class DeclareNewTransformation(TeacherStudentsScene):
    def construct(self):
        words = OldTexText("Playing a transformation in\\\\", "reverse gives a", "new transformation")
...
class DescribeInverse(LinearTransformationScene):
    ...
    def construct(self):
        ...
        if not self.show_actual_inverse:
            ...
            neg_1 = OldTex("-1")
            neg_1.move_to(inv_matrix.get_corner(UP + RIGHT), aligned_edge=LEFT)
...
class MultiplyToIdentity(LinearTransformationScene):
    def construct(self):
        ...
        self.apply_inverse_transpose(self.t_matrix)
        self.wait()
        self.play(Transform(brace, product.brace), Transform(text, product.text))
        self.wait()
        self.play(
...
class TwoDInverseFormula(Scene):
    def construct(self):
        ...
        formula = OldTex(
            ...
            %s^{-1} = \\dfrac{1}{ad-bc} %s
        """
            % (matrix_to_tex_string(matrix), matrix_to_tex_string(scaled_inv))
        )
...
class SymbolicInversion(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(words), *[Transform(m, m.interim) for m in interim_mobs])
        self.wait()

        product = VMobject(A, inv)
        product.brace = Brace(product)
        product.words = product.brace.get_text("The ``do nothing'' matrix")
        ...
        self.wait()
        self.play(*[ApplyMethod(m.set_color, BLACK) for m in (product, product.brace, product.words)])
        self.wait()
        self.play(ApplyFunction(lambda m: m.center().to_edge(UP), VMobject(x, eq, inv2, v)))
        self.wait()


class PlayInReverseWithSolution(PlayInReverse):
    CONFIG = {"t_matrix": [[2, 1], [2, 3]]}
...
class OneUniqueSolution(Scene):
    def construct(self):
        ...
        self.add(system)
        self.wait()
        self.play(GrowFromCenter(brace), Write(words.split()[0]))
        self.wait()
...
class InvertNonInvertable(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(grid.restore, *list(map(Animation, self.foreground_mobjects)), run_time=3)
        self.wait()


class OneInputMultipleOutputs(InvertNonInvertable):
    def construct(self):
        input_vectors = VMobject(*[Vector([x + 2, x]) for x in np.arange(-4, 4.5, 0.5)])
        ...
        self.play(
            grid.restore,
            *[Transform(output_vector.copy(), input_vector) for input_vector in input_vectors.split()]
            + list(map(Animation, self.foreground_mobjects)),
            run_time=3,
        )
        multiple_outputs = OldTexText("Must map to \\\\", "multiple vectors")
...
class PrepareForRank(Scene):
    def construct(self):
        new_term, rank = words = OldTexText("New terminology: ", "rank")
        rank.set_color(TEAL)
...
class DefineRank(Scene):
    def construct(self):
        ...
        dims = OldTexText("Number of\\\\", "dimensions \\\\", "in the output")
        ...
        self.play(ShowCreation(arrow), *list(map(Write, dims)))
        self.wait()
...
class DefineColumnSpace(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(arrow), Write(right_words))
        self.wait()
    ...
    def construct(self):
        ...
        for i, words in enumerate([i_hat_words, j_hat_words]):
            ...
            self.play(
                Write(words, run_time=1),
                ShowCreation(arrow),
                *[ApplyMethod(m.set_color, words.get_color()) for m in matrix.get_mob_matrix()[:, i]],
            )
        self.wait()
...
class TowDColumnsDontSpan(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(ApplyFunction(lambda m: m.scale(-1).shift(self.i_hat.get_end()), self.j_hat))
        ...
        for x in range(8):
            ...
            self.j_hat.target.shift(-self.j_hat.target.get_start() + self.i_hat.target.get_end())
            self.play(Transform(VMobject(*bases), VMobject(*[m.target for m in bases]), run_time=2))
...
class ThreeDColumnsDontSpan(Scene):
    def construct(self):
        ...
        matrix.set_column_colors(X_COLOR, Y_COLOR, Z_COLOR)
        brace = Brace(matrix)
        words = brace.get_text("Columns don't", "span \\\\", "full output space")
...
class NameColumnSpace(Scene):
    def construct(self):
        ...
        for i in 1, -1:
            ...
            span_text[i].do_in_place(span_text[i].set_height, span_text.get_height())
        ...
        self.play(ShowCreation(arrow), Write(column_space))
        self.wait()
        ...
        rank_words = brace.get_text("Number of dimensions \\\\ is called", "``rank''")
        rank_words[1].set_color(MAROON)
        self.play(GrowFromCenter(brace), Write(rank_words))
        self.wait()
    ...
    def cycle_through_span_possibilities(self, span_text):
        ...
        mob_matrix = np.array([two_d_span[i].get_entries().split() for i in (2, 4)])
        ...
        self.add(span_text)
        mob_matrix = np.array([span_text[i].get_entries().split() for i in (2, 4, 6)])
    ...
    def replace_number_matrix(self, matrix, new_numbers):
        ...
        self.play(*[Transform(*pair, path_arc=np.pi) for pair in zip(starters, targets)])
...
class ZeroMatirx(LinearTransformationScene):
    def construct(self):
        origin = Dot(ORIGIN)
        self.play(Transform(VMobject(self.plane, self.i_hat, self.j_hat), origin, run_time=3))
        self.wait()


class RankNumber(Scene):
    CONFIG = {"number": 3, "color": BLUE}
...
class RankNumber1(RankNumber):
    CONFIG = {"number": 1, "color": GREEN}


class RankNumber0(RankNumber):
    CONFIG = {"number": 0, "color": GREY}
...
class OriginIsAlwaysInColumnSpace(LinearTransformationScene):
    def construct(self):
        ...
        self.apply_transposed_matrix(self.t_matrix)
        self.wait()
        self.apply_transposed_matrix([[1.0 / 3, -1.0 / 2], [-1.0 / 3, 1.0 / 2]])
        self.wait()
...
class FullRankCase(LinearTransformationScene):
    ...
    def construct(self):
        t_matrices = [[[2, 1], [-3, 2]], [[1.0 / 2, 1], [1.0 / 3, -1.0 / 2]]]
        vector = Matrix([0, 0]).set_color(YELLOW)
        title = VMobject(OldTexText("Only"), vector, OldTexText("lands on"), vector.copy())
...
class NameNullSpace(LinearTransformationScene):
    CONFIG = {"show_basis_vectors": False, "t_matrix": [[1, -1], [-1, 1]]}
    ...
    def construct(self):
        ...
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Transform(vectors, dot)], path_arc=0)
        self.wait()
        self.play(
            vectors.restore,
            self.plane.restore,
        ...
        self.play(Transform(vectors, line, run_time=2, lag_ratio=0.5))
        self.wait()
    ...
    def get_vectors(self, offset=0):
        ...
        vectors = VMobject(*[Vector(a * vect + offset) for a in np.linspace(-5, 5, 18)])
...
class NullSpaceSolveForVEqualsZero(NameNullSpace):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(zero_vector_rect), Transform(v, zero_vector))
        self.wait()
...
class OffsetNullSpace(NameNullSpace):
    def construct(self):
        ...
        dots = VMobject(*[Dot(v.get_end(), color=v.get_color()) for v in offset_vectors])
        dot = Dot(self.get_matrix_transformation(self.t_matrix)(x.get_end()), color=RED)
        ...
        x_copies = VMobject(*[x.copy().shift(v.get_end()) for v in vectors])
        ...
        self.play(
            ...
            *[Transform(v, VectorizedPoint(v.get_end())) for v in x_copy],
        )
        self.remove(x_copy)
        self.wait()
        self.play(Transform(vectors, dots))
        self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Transform(vectors, dot)])
        self.wait()
        self.play(ShowCreation(circle), Write(words))
        self.wait()
...
class ShowAdditivityProperty(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.play(Write(initial_sum), FadeOut(w_copy))
        self.add_foreground_mobject(form_rect, initial_sum)
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Write(transformer)])
        self.wait()
...
class AddJustOneNullSpaceVector(NameNullSpace):
    def construct(self):
        ...
        x.label = "$\\vec{\\textbf{x}}$"
        sum_vect = Vector(x.get_end() + null_vector.get_end(), color=PINK)
        ...
        self.wait()
        self.play(FadeOut(vectors), Animation(null_vector))
        self.play(Write(null_vector.label))
        self.wait()
        x_copy = x.copy()
        self.play(x_copy.shift, null_vector.get_end())
        self.add_vector(sum_vect, animate=True)
        self.play(FadeOut(x_copy), Write(sum_vect.label))
        self.wait()
        self.play(ShowCreation(form_rect), sum_vect.label.replace, initial_sum)
        self.add_foreground_mobject(form_rect, sum_vect.label)
        self.remove(x.label, null_vector.label)
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Transform(null_vector, dot), Write(transformer)])
        self.play(Write(final_sum))
        self.wait()
...
class NullSpaceOffsetRule(Scene):
    def construct(self):
        ...
        x.set_color(PINK)
        v.set_color(YELLOW)
        A_text = OldTexText("When $A$ is not", "full rank")
        ...
        v_text = OldTexText("If", "$%s$" % vec("v"), "is in the", "column space", "of $A$")
...
class MuchLeftToLearn(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("That's the high \\\\", "level overview")
        self.random_blink()
        self.wait()
        self.teacher_says("There is still \\\\", "much to learn")
        for pi in self.get_students():
            target_mode = random.choice(["raise_right_hand", "raise_left_hand"])
...
class WhatAboutNonsquareMatrices(TeacherStudentsScene):
    def construct(self):
        self.student_says("What about \\\\ nonsquare matrices?", target_mode="raise_right_hand")
