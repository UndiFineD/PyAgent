# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter8.py
from _2016.eola.chapter5 import RightHandRule, get_det_text
...
class ListSteps(Scene):
    CONFIG = {"randy_corner": DOWN + RIGHT}
    ...
    def construct(self):
        ...
        self.play(MoveToTarget(step_1), step_2.fade, randy.change_mode, "happy")
        self.play(Blink(randy))
        self.play(
...
class SimpleDefine2dCrossProduct(LinearTransformationScene):
    ...
    def show_area(self):
        ...
        self.play(ShowCreation(self.square), *list(map(Animation, [self.v, self.w])))
        self.wait()
        ...
        self.wait()
        self.play(FadeIn(self.square), *list(map(Animation, [self.v, self.w, v_copy, w_copy])))
        self.wait()
    ...
    def write_area_words(self):
        ...
        arrow = Arrow(words.get_bottom(), self.square.get_center(), color=WHITE)

        self.play(
            FadeIn(cross_rect),
            Write(times),
            *[ApplyMethod(vect.label.target.restore, rate_func=lambda t: smooth(1 - t)) for vect in (self.v, self.w)],
        )
        self.wait()
        self.play(ApplyFunction(lambda m: m.scale(1.2).set_color(RED), times, rate_func=there_and_back))
        self.wait()
    ...
    def show_sign(self):
        ...
        for word in positive, negative:
            ...
            word.arrow = Arrow(word.get_top(), word.get_top() + 1.5 * UP, color=word.get_color())
            VGroup(word, word.arrow).next_to(self.area_words, DOWN, aligned_edge=LEFT, buff=SMALL_BUFF)
        minus_sign = OldTex("-")
    ...
    def swap_v_and_w(self):
        ...
        transform = self.get_matrix_transformation(np.dot(new_matrix.T, np.linalg.inv(curr_matrix.T)).T)
...
class VisualExample(SimpleDefine2dCrossProduct):
    ...
    def construct(self):
        ...
        self.play(Transform(arc, minus), val.next_to, minus, RIGHT)
        self.wait()
    ...
    def show_coords(self):
        for vect, edge in (self.v, DOWN), (self.w, UP):
            ...
            vect.coord_array.move_to(vect.coord_array.get_center(), aligned_edge=edge)
            self.play(Write(vect.coord_array, run_time=1))


class HowDoYouCompute(TeacherStudentsScene):
    def construct(self):
        self.student_says("How do you \\\\ compute this?", target_mode="raise_left_hand")
        self.random_blink(2)
...
class ContrastDotAndCross(Scene):
    ...
    def add_dot_products(self, max_width=FRAME_X_RADIUS - 1, dims=[2, 5]):
        ...
        dot_products = []
        for dim in dims:
            arrays = [[random.randint(0, 9) for in_count in range(dim)] for out_count in range(2)]
                ...
                *it.chain(
                    *list(
                        zip(
                            syms,
                            [
                                VGroup(e1.target, get_dot(), e2.target).arrange()
            ...
            dot_prod = VGroup(m1, OldTex("\\cdot"), m2, result)
        ...
        for dot_prod in dot_products:
            self.play(
                Write(dot_prod.syms),
                *[Transform(e.copy(), e.target, path_arc=-np.pi / 6) for e in dot_prod.entries],
                run_time=2,
            )
        self.wait()
    ...
    def add_cross_product(self):
        ...
        arrays = [[random.randint(0, 9) for in_count in range(3)] for out_count in range(2)]
        ...
        full_cross_product = VGroup(cross_product, OldTex("="), result)
        ...
        self.play(Write(result.get_brackets()), Write(syms), *movements, run_time=2)
        self.wait()
    ...
    def add_2d_cross_product(self):
        ...
        for entry in entries:
            entry.target = entry.copy()
        eq, dot1, minus, dot2 = syms = list(map(Tex, ["=", "\\cdot", "-", "\\cdot"]))
        result = VGroup(
            eq,
            x1.target,
            dot1,
    ...
    def emphasize_output_type(self):
        ...
        self.play(GrowFromCenter(two_d_brace), Write(number))
        self.wait()
        self.play(GrowFromCenter(three_d_brace), Write(vector))
        self.wait()
...
class Define2dCrossProduct(LinearTransformationScene):
    ...
    def initial_definition(self):
        ...
        times = OldTex("\\times")
        cross_product = VGroup(v.label.target, times, w.label.target)

        cross_product.arrange()
        matrix = Matrix(np.array([list(v.coords.target), list(w.coords.target)]).T)
        ...
        self.wait()
        self.play(ShowCreation(matrix_background), Write(matrix.get_brackets()), run_time=1)
    ...
    def show_transformation(self):
        ...
        self.play(ShowCreation(i_hat), ShowCreation(j_hat), Write(basis_labels))
        self.wait()
        ...
        col1, col2 = [VGroup(*matrix.get_mob_matrix()[i, :]) for i in (0, 1)]
        ...
        self.play(GrowFromCenter(side_brace), Write(transform_words))
        self.play(Write(i_words), ShowCreation(i_words.arrow), col1.set_color, X_COLOR)
        self.wait()
        self.play(
    ...
    def transform_square(self):
        ...
        vect_stuffs = VGroup(*it.chain(*[[m, m.label, m.coord_array] for m in (self.v, self.w)]))
        to_restore = [self.plane, self.i_hat, self.j_hat]
        for mob in to_restore:
            mob.fade(1)

        self.play(*list(map(FadeOut, vect_stuffs)))
        self.play(*[m.restore for m in to_restore] + [Animation(self.matrix)])
        self.add_unit_square(animate=True, opacity=0.2)
        self.square.save_state()
        self.wait()
        self.apply_transposed_matrix([self.v_coords, self.w_coords])
        self.wait()
        self.play(FadeOut(self.plane), Animation(self.matrix), *list(map(FadeIn, vect_stuffs)))
        ...
        area_arrow = Arrow(area_words.get_bottom(), self.square.get_center(), color=WHITE)
        self.play(GrowFromCenter(det_text_brace), Write(area_words), ShowCreation(area_arrow))
        self.wait()
        ...
        self.play(Transform(self.square.get_point_mobject(), pm), path_arc=-np.pi / 2)
        self.wait()
        self.play(*list(map(FadeOut, [area_arrow, self.v.coord_array, self.w.coord_array])))
    ...
    def show_orientation_rule(self):
        ...
        positive_words, negative_words = words_list = [
            OldTex(v_tex, "\\times", w_tex, "\\text{ is }", word) for word in ("\\text{positive}", "\\text{negative}")
        ]
        ...
        anticommute = OldTex(v_tex, "\\times", w_tex, "=-", w_tex, "\\times", v_tex)
...
class TwoDCrossProductExample(Define2dCrossProduct):
    ...
    def construct(self):
        ...
        equation_start = VGroup(cross_product, VGroup(matrix_background, det_text, matrix))
        equation_start.arrange()
        equation_start.next_to(ORIGIN, DOWN).to_edge(LEFT)

        for vect in v, w:
            self.play(ShowCreation(vect), Write(vect.coords), Write(vect.label))
            self.wait()
        self.play(
        ...
        equals, dot1, minus, dot2, equals_result = syms = VGroup(
            *list(map(Tex, ["=", "\\cdot", "-", "\\cdot", "=%d" % det]))
        )
        ...
        self.play(
            ...
            Transform(
                VGroup(v1, w2).copy(),
                VGroup(v1.target, w2.target),
                rate_func=squish_rate_func(smooth, 0, 1.0 / 3),
                path_arc=np.pi / 2,
            ),
            Transform(
                VGroup(v2, w1).copy(),
                VGroup(v2.target, w1.target),
    ...
    def show_transformation(self, v, w):
        ...
        self.apply_transposed_matrix(
            ...
            added_anims=[Transform(i_hat, v), Transform(j_hat, w)],
        )
...
class BiggerWhenPerpendicular(LinearTransformationScene):
    ...
    def construct(self):
        ...
        start_square, end_square = [square.copy().apply_function(transform) for transform in transforms]

        for vect in v, w:
            self.play(ShowCreation(vect))
        group.remove(bigger)
        self.play(FadeIn(group), ShowCreation(start_square), *list(map(Animation, [v, w])))
        ...
        self.play(Transform(start_words, end_words), Transform(bigger, smaller))
        self.wait()


class ScalingRule(LinearTransformationScene):
    CONFIG = {"v_coords": [2, -1], "w_coords": [1, 1], "show_basis_vectors": False}
    ...
    def construct(self):
        ...
        new_v.label = self.get_vector_label(new_v, "3\\vec{\\textbf{v}}", "right", color=V_COLOR)
        for vect in v, w, new_v:
            vect.add(vect.label)

        transform = self.get_matrix_transformation([self.v_coords, self.w_coords])
        square.apply_function(transform)
        new_squares = VGroup(*[square.copy().shift(m * v.get_end()) for m in range(3)])
        ...
        self.play(ShowCreation(square), *list(map(Animation, [v, w])))
        self.wait()
        self.play(
            Transform(v, new_v),
...
class WriteAreaOfParallelogram(Scene):
    def construct(self):
        words = OldTexText("Area of ", "parallelogram", " $=$ ", "$2.5$", arg_separator="")
...
class WriteCrossProductProperties(Scene):
    def construct(self):
        ...
        length_words = OldTexText("Length of ", p_cash, "\\\\ = ", "(parallelogram's area)")
        ...
        perpendicular = OldTexText("\\centering Perpendicular to", v_cash, "and", w_cash)
        ...
        self.play(Write(cross_product))
        self.play(GrowFromCenter(brace), Write(vector, run_time=1))
        self.wait()
...
def get_cross_product_right_hand_rule_labels():
    ...
    return [v_tex, w_tex, "%s \\times %s" % (v_tex, w_tex)]
...
class LabelingExampleVectors(Scene):
    def construct(self):
        ...
        equations = [
            ...
            OldTex(v_tex, "\\times", w_tex, "=%s" % matrix_to_tex_string([-4, 0, 0])),
        ]
...
class ShowCrossProductFormula(Scene):
    def construct(self):
        ...
        arrays = [["%s_%d" % (s, i) for i in range(1, 4)] for s in ("v", "w")]
        matrices = list(map(Matrix, arrays))
        for matrix in matrices:
            for entry, color in zip(matrix.get_entries(), colors):
                entry.set_color(color)
        ...
        for a, b, c in it.permutations(list(range(3))):
            ...
            cross_entry.arrange()
            movement_sets.append([e1, e1.target, e2, e2.target, syms])
        ...
        self.play(Write(equals), Write(result.get_brackets()))
        self.wait()
...
class ThisGetsWeird(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("This gets weird...", target_mode="sassy")
        self.random_blink(2)


class DeterminantTrick(Scene):
    def construct(self):
        v_terms, w_terms = [["%s_%d" % (s, d) for d in range(1, 4)] for s in ("v", "w")]
        v = Matrix(v_terms)
        w = Matrix(w_terms)
        v.set_color(V_COLOR)
        w.set_color(W_COLOR)
        matrix = Matrix(
            np.array(
                [
                    [OldTex("\\hat{%s}" % s) for s in ("\\imath", "\\jmath", "k")],
        ...
        det_text = get_det_text(matrix)
        equals = OldTex("=")
        equation = VGroup(v, OldTex("\\times"), w, equals, VGroup(det_text, matrix))
        equation.arrange()
        ...
        self.play(
            equation.to_corner,
            UP + LEFT,
            ApplyFunction(lambda r: r.change_mode("plain").look(UP + RIGHT), randy),
        )
        quints = [
            (i, v2, w3, v3, w2),
            (j, v3, w1, v1, w3),
        ...
        for quint in quints:
            ...
            term = VGroup(basis.t, lp, quint[1].t, quint[2].t, minus, quint[3].t, quint[4].t, rp)
            term.arrange()
            if last_mob:
                plus = OldTex("+")
                syms.add(plus)
            ...
            last_mob = term
            self.play(*it.chain(*[[mob.scale, 1.2] for mob in quint]))
            self.wait()
            self.play(
                *[Transform(mob.copy(), mob.t) for mob in quint] + [mob.restore for mob in quint] + [Write(syms)],
                run_time=2,
            )
            self.wait()
...
class ThereIsAReason(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "\\centering Sure, it's a \\\\",
            "notational",
            "trick",
        )
        self.random_blink(2)
        words = OldTexText("\\centering but there is a\\\\", "reason", "for doing it")
...
class CrossAndDualWords(Scene):
    def construct(self):
        ...
        cross = OldTex(p_tex, "=", v_tex, "\\times", w_tex)
        ...
        det_text.add(matrix)
        dot_with_cross = OldTex("%s \\cdot ( " % input_array_tex, v_tex, "\\times", w_tex, ")")
        ...
        VGroup(vector_word, cross).arrange(RIGHT, buff=MED_SMALL_BUFF).center().shift(LEFT).to_edge(UP)
