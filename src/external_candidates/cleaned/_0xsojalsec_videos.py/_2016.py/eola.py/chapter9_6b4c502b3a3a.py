# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter9.py
...
def get_small_bubble(pi_creature, height=4, width=3):
    ...
    kwargs = {
        "height": 4,
        "bubble_center_adjustment_factor": 1.0 / 6,
    }
...
class LinearCombinationScene(LinearTransformationScene):
    ...
    def setup(self):
        ...
        self.i_hat.label = self.get_vector_label(self.i_hat, "\\hat{\\imath}", "right")
        self.j_hat.label = self.get_vector_label(self.j_hat, "\\hat{\\jmath}", "left")

    def show_linear_combination(
        self,
        numerical_coords,
        basis_vectors,
        coord_mobs,
        ...
        for basis in basis_vectors:
            if not hasattr(basis, "label"):
                ...
                direction = np.round(rotate_vector(basis.get_end(), np.pi / 2))
        ...
        for coord, basis, scalar in zip(coord_mobs, basis_vectors, numerical_coords):
            ...
            new_label.move_to(basis.label, aligned_edge=basis.get_center() - basis.label.get_center())
            new_label.shift(basis.target.get_center() - basis.get_center())
            coord.target.next_to(basis.label.target, LEFT)
            coord.target.set_fill(basis.get_color(), opacity=1)
            self.play(*list(map(MoveToTarget, [coord, basis, basis.label])))
            self.wait()
        self.play(*[ApplyMethod(m.shift, basis_vectors[0].get_end()) for m in self.get_mobjects_from_last_animation()])
        if show_sum_vect:
            sum_vect = Vector(basis_vectors[1].get_end(), color=sum_vect_color)
...
class RemindOfCoordinates(LinearCombinationScene):
    ...
    def show_abstract_scalar_idea(self, x_coord, y_coord):
        ...
        self.wait()
        self.play(FadeOut(words), FadeIn(everything), *[mob.restore for mob in to_save])
        self.wait()
    ...
    def list_implicit_assumptions(self, x_coord, y_coord):
        ...
        self.wait(2)
        keepers = VGroup(*[self.i_hat, self.j_hat, self.i_hat.label, self.j_hat.label])
        self.play(FadeOut(everything), Animation(keepers.copy()), Animation(group))
        self.wait()
...
class JenniferScene(LinearCombinationScene):
    ...
    def setup(self):
        ...
        self.b1.label.next_to(self.b1.get_end() * 0.4, UP + LEFT, SMALL_BUFF / 2)
        self.b2.label.next_to(self.b2.get_end(), DOWN + LEFT, buff=SMALL_BUFF)
        self.basis_vectors = VGroup(self.b1, self.b2, self.b1.label, self.b2.label)
...
class IntroduceJennifer(JenniferScene):
    ...
    def introduce_jenny(self):
        ...
        self.add(jenny)
        self.play(jenny.change_mode, "wave_1", jenny.look, OUT, Write(name))
        self.play(jenny.change_mode, "happy", jenny.look, UP + RIGHT, FadeOut(name))
        self.wait()
    ...
    def add_basis_vectors(self):
        ...
        for vect in self.b1, self.b2:
            self.play(ShowCreation(vect), Write(vect.label))
            self.wait()
    ...
    def show_v_from_both_perspectives(self):
        ...
        self.play(ShowCreation(you.bubble), Write(you.coords))
        ...
        self.play(FadeOut(v), jenny.change_mode, "plain")
        self.play(
    ...
    def hacked_show_linear_combination(
        ...
        self.play(
            ...
            self.you.change_mode,
            "speaking",
            self.jenny.change_mode,
            "erm",
            *list(
                map(
                    MoveToTarget,
                    [self.jenny.coords, b1, b2, new_label1, new_label2, sum_vect],
                )
            ),
        )
        ...
        self.play(
            *it.chain(
                list(
                    map(
                        FadeOut,
                        [self.jenny.bubble, self.jenny.coords, coord_mobs, sum_vect],
                    )
                ),
                [ApplyMethod(pi.change_mode, "plain") for pi in (self.jenny, self.you)],
    ...
    def how_we_label_her_basis(self):
        ...
        self.play(ShowCreation(you.bubble), Write(b1_coords), you.change_mode, "speaking")
        self.play(
            b1_coords.mover.next_to,
            self.b1.get_end(),
            RIGHT,
            b1_coords.mover.set_color,
            X_COLOR,
        )
        self.play(Blink(you))
        self.wait()
        ...
        self.play(
            b2_coords.mover.next_to,
            self.b2.get_end(),
            LEFT,
            b2_coords.mover.set_color,
            Y_COLOR,
        )
...
class SpeakingDifferentLanguages(JenniferScene):
    def construct(self):
        ...
        for pi in you, jenny:
            self.play(
                pi.change_mode,
                "speaking" if pi is you else "sassy",
...
class JennysGrid(JenniferScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            ShowCreation(self.jenny_plane, run_time=3, lag_ratio=0.5),
            self.jenny.change_mode,
            "speaking",
            self.jenny.look_at,
            ORIGIN,
        ...
        self.play(Blink(self.jenny))
        self.play(FadeOut(bubble.content), FadeIn(coords_mob))
        self.show_linear_combination(
...
class ShowOriginOfGrid(JenniferScene):
    def construct(self):
        ...
        self.play(Write(origin_word), ShowCreation(arrow))
...
class TranslateFromJenny(JenniferScene):
    ...
    def add_players(self):
        for plane in self.jenny_plane, self.plane:
            plane.fade()
        self.add(self.jenny_plane, self.jenny, self.you, self.basis_vectors)
    ...
    def ask_question(self):
        ...
        self.play(Transform(self.jenny_plane, self.plane), *list(map(Animation, everything)))
        self.play(
            self.you.change_mode,
            "confused",
    ...
    def perform_arithmetic(self):
        ...
        self.play(self.you.change_mode, "hooray", Transform(result.copy(), result_copy))
        self.play(Blink(self.you))
        self.wait()
...
class WatchChapter3(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink()
        self.play(
            self.get_students()[0].look,
            LEFT,
            self.get_students()[1].change_mode,
            "happy",
...
class TalkThroughChangeOfBasisMatrix(JenniferScene):
    def construct(self):
        ...
        t_matrix2 = np.dot(self.cob_matrix(), np.linalg.inv(t_matrix1.T)).T
        ...
        self.add_foreground_mobject(self.jenny, self.you, self.you.bubble, basis_coords_pair, matrix)
        ...
        self.apply_transposed_matrix(
            ...
            added_anims=[
                ...
                Transform(VGroup(*matrix_copy.get_mob_matrix()[:, 0]), b1_coords.get_entries()),
            ],
        )
        ...
        self.apply_transposed_matrix(
            ...
            added_anims=[
                ...
                Transform(VGroup(*matrix_copy.get_mob_matrix()[:, 1]), b2_coords.get_entries()),
            ],
        )
        self.remove(matrix_copy)
...
class FeelsBackwards(Scene):
    def construct(self):
        ...
        self.play(ShowCreation(top_arrow), Write(jenny_grid))
        self.wait()
        self.play(Write(jenny_language))
        self.play(ShowCreation(bottom_arrow), Write(our_language))
        self.wait()
...
class RecallInverse(JenniferScene):
    def construct(self):
        ...
        inv_matrix = Matrix([["1/3", "1/3"], ["-1/3", "2/3"]])
        ...
        self.play(GrowFromCenter(brace), Write(inverse_word), Write(inverse_exponent))
        self.add_foreground_mobject(*self.get_mobjects_from_last_animation())
        self.wait()
        self.apply_inverse_transpose(numerical_t_matrix)
        self.wait()
        self.play(Write(equals), Transform(matrix.copy(), inv_matrix))
        self.remove(*self.get_mobjects_from_last_animation())
        ...
        self.play(*[ApplyMethod(mob.scale, 1.2, rate_func=there_and_back) for mob in inv_matrix.get_mob_matrix()[:, 0]])
        self.wait()
        inv_matrix.set_column_colors(X_COLOR, Y_COLOR)
        self.play(*[ApplyMethod(mob.scale, 1.2, rate_func=there_and_back) for mob in inv_matrix.get_mob_matrix()[:, 1]])
        self.wait()
...
class WorkOutInverseComputation(Scene):
    def construct(self):
        ...
        equals = OldTex("=")
        equation = VGroup(matrix, our_vector, equals, her_vector)
        for mob in equation:
        ...
        self.add(our_vector)
        self.play(GrowFromCenter(our_vector_brace), Write(our_text))
        self.wait()
        self.play(FadeIn(matrix), GrowFromCenter(matrix_brace), Write(matrix_text))
        self.wait()
        self.play(Write(equals), Write(her_vector))
        self.play(GrowFromCenter(her_vector_brace), Write(her_text))
        self.wait()
...
class SummarizeTranslationProcess(Scene):
    ...
    def define_matrix(self):
        ...
        self.play(Write(words), *list(map(ShowCreation, [arrow1, arrow2])))
        self.A_copy = A.copy()
    ...
    def show_translation(self):
        ...
        her_arrow = Arrow(her_words, her_vector, color=her_vector.get_color())
        ...
        our_arrow = Arrow(our_words.get_top(), our_vector.get_bottom(), color=our_vector.get_color())

        self.play(Write(equation), Transform(self.A_copy, A))
        self.remove(self.A_copy)
        self.play(Write(her_words), ShowCreation(her_arrow))
        self.play(Write(our_words), ShowCreation(our_arrow))
        self.wait(2)
        self.play(
            VGroup(her_vector, equals).next_to,
            A_inv,
            LEFT,
            her_arrow.rotate,
            -np.pi / 6,
...
class Prerequisites(Scene):
    def construct(self):
        ...
        for direction, words in zip([LEFT, RIGHT], prereqs):
            ...
            self.play(Write(words), ShowCreation(rect))
        self.wait()


class RotationExample(LinearTransformationScene):
    CONFIG = {"t_matrix": [[0, 1], [-1, 0]]}
    ...
    def construct(self):
        ...
        self.play(
            ...
            Transform(i_coords.get_entries(), VGroup(*matrix.get_mob_matrix()[:, 0])),
        )
        self.play(
            FadeOut(j_coords.rect),
            FadeOut(j_coords.get_brackets()),
            Transform(j_coords.get_entries(), VGroup(*matrix.get_mob_matrix()[:, 1])),
        )
        self.wait()
        self.add_words(matrix)

    def add_words(self, matrix):
        follow_basis = OldTexText("Follow", "our choice", "\\\\ of basis vectors")
        ...
        record = OldTexText("Record using \\\\", "our coordinates")
        record.set_color_by_tex("our coordinates", YELLOW)
        record.add_background_rectangle()
        record.next_to(matrix, DOWN, buff=MED_SMALL_BUFF, aligned_edge=LEFT)
...
class JennyWatchesRotation(JenniferScene):
    def construct(self):
        ...
        self.play(jenny.change_mode, "sassy", ShowCreation(jenny.bubble), Write(matrix))
        self.play(
            *it.chain(
                [Rotate(mob, np.pi / 2, run_time=3) for mob in (self.jenny_plane, self.b1, self.b2)],
...
class HowToTranslateAMatrix(Scene):
    def construct(self):
        ...
        inv_cob.add(neg_1)
        arrays.arrange(LEFT)
        arrays.to_edge(LEFT, buff=LARGE_BUFF / 2.0)
        for array in arrays:
        ...
        self.play(GrowFromCenter(brace), Write(bottom_words))
        self.wait()
        introduce(cob_matrix)
        self.play(GrowFromCenter(top_brace), Write(top_words))
        self.wait()
        introduce(transform)
        ...
        self.play(Transform(her_vector, equals), Write(result))
        self.wait(2)

        everything = VGroup(*self.get_mobjects())
        self.play(FadeOut(everything), result.to_corner, UP + LEFT)
        self.add(result)
        self.wait()
...
class JennyWatchesRotationWithMatrixAndVector(JenniferScene):
    def construct(self):
        ...
        self.play(
            *it.chain(
                [Rotate(mob, np.pi / 2, run_time=3) for mob in (self.jenny_plane, self.b1, self.b2, vector)],
                list(map(Animation, [self.jenny, matrix, vector_array])),
            )
        )
        self.play(self.jenny.change_mode, "pondering", Write(equals), Write(result))
        self.play(Blink(self.jenny))
        self.wait()
