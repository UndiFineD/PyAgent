# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter3.py
...
class WhyConfuseWithTerminology(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(*[ApplyMethod(student.change_mode, "happy") for student in other_students])
        self.random_blink()
        self.wait()
...
class TransformManyVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        if self.use_dots:
            self.play(Transform(vectors, self.vectors_to_dots(vectors), run_time=3, lag_ratio=0.5))
            transformed_vectors = self.vectors_to_dots(transformed_vectors)
            self.wait()
        self.play(Transform(vectors, transformed_vectors, run_time=3, path_arc=-np.pi / 2))
        self.wait()
        if self.use_dots:
            self.play(Transform(vectors, self.dots_to_vectors(vectors), run_time=2, lag_ratio=0.5))
            self.wait()

    def vectors_to_dots(self, vectors):
        return VMobject(*[Dot(v.get_end(), color=v.get_color()) for v in vectors.split()])

    def dots_to_vectors(self, dots):
        return VMobject(*[Vector(dot.get_center(), color=dot.get_color()) for dot in dots.split()])
...
class IntroduceLinearTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(Write(origin_rule, run_time=2), ShowCreation(arrow), GrowFromCenter(dot))
        self.wait()
...
class FollowIHatJHat(LinearTransformationScene):
    ...
    def construct(self):
        ...
        i_label = self.label_vector(i_hat, "\\hat{\\imath}", color=X_COLOR, label_scale_factor=1)
        j_hat = self.add_vector([0, 1], color=Y_COLOR)
        j_label = self.label_vector(j_hat, "\\hat{\\jmath}", color=Y_COLOR, label_scale_factor=1)
...
class TrackBasisVectorsExample(LinearTransformationScene):
    ...
    def show_linear_combination(self, clean_up=True):
        ...
        self.play(ApplyFunction(lambda m: m.scale(self.v_coords[0]).fade(0.3), i_hat_copy))
        self.play(ApplyFunction(lambda m: m.scale(self.v_coords[1]).fade(0.3), j_hat_copy))
...
class DeduceResultWithGeneralCoordinates(Scene):
    def construct(self):
        ...
        VMobject(row1.split()[1], row1.split()[4], row2.split()[2], row2.split()[5]).set_color(YELLOW)
...
class MatrixVectorMultiplication(LinearTransformationScene):
    ...
    def build_to_matrix(self):
        ...
        abstract_matrix = np.append(i_coords.get_mob_matrix(), j_coords.get_mob_matrix(), axis=1)
        concrete_matrix = Matrix(copy.deepcopy(abstract_matrix), add_background_rectangles_to_entries=True)
    ...
    def reposition_matrix_and_vector(self, matrix, vector, formula):
        ...
        self.play(Transform(start_state, end_state, lag_ratio=0), Write(equals, run_time=1))
        self.wait()
...
class ColumnsToBasisVectors(LinearTransformationScene):
    ...
    def move_matrix_columns(self, transposed_matrix, vector_coords=None):
        ...
        i_coords_start = VMobject(matrix_background.copy(), col1.copy(), matrix_brackets.copy())
        i_coords_end = VMobject(
            i_label_background,
            i_coords,
            i_brackets,
        )
        j_coords_start = VMobject(matrix_background.copy(), col2.copy(), matrix_brackets.copy())
...
class UsedToThinkinfOfFunctionsAsGraphs(VectorScene):
    ...
    def show_inputs_and_output(self):
        ...
        outputs = VMobject(*[OldTex(str(num**2)).next_to(arrow) for num, arrow in zip(numbers, arrows.split())])
...
class AdditivityProperty(LinearTransformationScene):
    ...
    def apply_transformation(self, added_anims):
        if self.nonlinear_transformation:
            ...
            self.apply_transposed_matrix(self.transposed_matrix, added_anims=added_anims)
        self.wait()
