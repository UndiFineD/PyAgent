# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter0.py
...
class VideoIcon(SVGMobject):
    def __init__(self, **kwargs):
        ...
        self.center()
        self.set_width(FRAME_WIDTH / 12.0)
...
class AboutLinearAlgebra(Scene):
    ...
    def show_dependencies(self):
        ...
        self.wait()
        self.play(ShowCreation(arrows, lag_ratio=0.5), FadeIn(all_subs), run_time=2)
        self.wait()
        self.linalg = linalg
    ...
    def to_thought_bubble(self):
        ...
        self.play(Transform(linalg, new_linalg), Write(bubble), FadeIn(randy))
        self.wait()
...
class NumericVsGeometric(Scene):
    ...
    def specifics_concepts(self):
        matrix_vector_product = OldTex(
            " ".join(
                [
                    matrix_to_tex_string(EXAMPLE_TRANFORM),
                    matrix_to_tex_string(TRANFORMED_VECTOR),
                    "&=",
                    matrix_to_tex_string([["1 \\cdot 1 + 0 \\cdot 2"], ["1 \\cdot 1 + (-1)\\cdot 2"]]),
                    "\\\\ &=",
        ...
        self.play(Write(self.numeric), FadeIn(matrix_vector_product), run_time=2)
        self.wait()
    ...
    def clear_way_for_geometric(self):
        ...
        self.play(
            ...
            ApplyMethod(self.matrix_vector_product.shift, (FRAME_HEIGHT + 1) * DOWN),
    ...
    def list_geometric_benefits(self):
        ...
        # Ugly hack
        diff = follow_words.submobjects[0].get_bottom()[1] - self.geometric.submobjects[0].get_bottom()[1]
        ...
        for randy, thought in zip(randys, thoughts):
            ...
            if curr_randy:
                self.play(Transform(curr_randy, randy), Transform(curr_thought, thought))
            else:
                self.play(FadeIn(randy), Write(thought, run_time=1))
                curr_randy = randy
...
class LinAlgPyramid(Scene):
    def construct(self):
        ...
        words = self.place_words_in_rects(["Geometric understanding", "Computations", "Uses"], rects)
        for word, rect in zip(words, rects):
            self.play(Write(word), ShowCreation(rect), run_time=1)
        self.wait()
        self.play(*[ApplyMethod(m.set_color, GREY_D) for m in (words[0], rects[0])])
        self.wait()
        self.list_applications(rects[-1])

    def get_rects(self):
        height = 1
        rects = [Rectangle(height=height, width=width) for width in (8, 5, 2)]
...
class IntimidatingProf(Scene):
    def construct(self):
        ...
        self.play(FadeIn(morty_name1), ShowCreation(arrow))
        self.play(Transform(morty_name1, morty_name2))
        self.wait()
        self.play(FadeOut(morty_name1), FadeOut(arrow))
        self.play(FadeIn(speech_bubble), ApplyMethod(morty.change_mode, "speaking"))
        self.play(FadeIn(thought_bubble))
        self.wait()
        self.play(ApplyMethod(randy.change_mode, "confused"), Write(q_marks, run_time=1))
...
class SineApproximations(Scene):
    ...
    def get_approx_series(self, val_str, val):
        # Default to 3 terms
        approximation = val - (val**3) / 6.0 + (val**5) / 120.0
        return OldTex(
            """
            \\sin(%s) \\approx 
...
class PhysicsExample(Scene):
    def construct(self):
        ...
        parabola = FunctionGraph(lambda x: (3 - x) * (3 + x) / 4, x_min=-4, x_max=4)
    ...
    def projectile(self, parabola):
        dot = Dot(radius=0.15)
        kwargs = {"run_time": 3, "rate_func": None}
        self.play(
    ...
    def velocity_vector(self, parabola):
        ...
        arc = Arc(angle_of_vector(vector), radius=vector_length / 4.0)
        arc.shift(p1)
    ...
    def approximate_sine(self):
        ...
        self.play(FadeIn(morty), ShowCreation(bubble), Write(approx), run_time=2)
        self.wait()
...
class LinearAlgebraIntuitions(Scene):
    def construct(self):
        ...
        self.play(Write(title), ShowCreation(h_line), run_time=2)
...
class MatrixMultiplicationIs(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(Write(matrix2), GrowFromCenter(brace2), Write(apply_second), run_time=1)
        self.wait()
...
class DeterminantsAre(Scene):
    def construct(self):
        ...
        self.play(Write(tex_mob), ShowCreation(arrow), run_time=1)
...
class ProfessorsTry(Scene):
    def construct(self):
        ...
        speech_bubble.shift(RIGHT)
        words = OldTexText(
            "It really is beautiful!  I want you to \\\\" + "see it the way I do...",
        )
...
class ResourceForTeachers(Scene):
    def construct(self):
        ...
        bubble.clear()
        randys = VMobject(*[Randolph(color=c) for c in (BLUE_D, BLUE_C, BLUE_E)])
...
class DifferingBackgrounds(Scene):
    def construct(self):
        ...
        students = VMobject(*[Randolph(color=c) for c in (BLUE_D, BLUE_C, BLUE_E)])
        ...
        for word, student, mode in zip(words, students.split(), modes):
            ...
            if last_word:
                ...
                last_word = word
                last_arrow = arrow
            self.play(word_anim, arrow_anim, ApplyMethod(student.change_mode, mode))
