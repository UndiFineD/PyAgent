...
class ShowNumericalDotProduct(Scene):
    ...
    def construct(self):
        ...
        dot = OldTex("\\cdot")
        products = VGroup(
            *[VGroup(p1.copy(), dot.copy(), p2.copy()).arrange(RIGHT, buff=SMALL_BUFF) for p1, p2 in pairs]
        )
...
class GeometricInterpretation(VectorScene):
    ...
    def project(self):
        ...
        projected = Vector(
            self.stable_vect.get_end() * dot_product / (self.stable_vect.get_length() ** 2),
...
class SymmetricVAndW(VectorScene):
    ...
    def show_doubling(self, v, w):
        ...
        self.add(*last_mob)
        Transform(VGroup(v.proj, v.proj_line), VGroup(new_v.proj, new_v.proj_line)).update(1)  ##Hacky
