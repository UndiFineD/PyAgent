# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter1.py
import random
...
class DifferentConceptions(Scene):
    ...
    def physics_conception(self, creatures, original_vector):
        ...
        random_vectors = VMobject(
            *[
                Vector(random.uniform(-2, 2) * RIGHT + random.uniform(-2, 2) * UP)
                .shift(random.uniform(0, 4) * RIGHT + random.uniform(-1, 2) * UP)
        ...
        self.add(vector)
        self.wait()
        self.play(GrowFromCenter(brace), Write(length), run_time=1)
        self.wait()
        self.remove(brace, length)
        self.play(Rotate(vector, np.pi / 3, in_place=True), Write(direction), run_time=1)
        for angle in -2 * np.pi / 3, np.pi / 3:
            self.play(Rotate(vector, angle, in_place=True, run_time=1))
    ...
    def cs_conception(self, creatures):
        ...
        self.play(ApplyMethod(compy.change_mode, "sassy"), Write(title, run_time=1))
    ...
    def house_example(self, starter_mobject, title):
        ...
        everything = VMobject(house, square_footage, price, brackets, brace, two_dimensional, title)
    ...
    def handle_mathy(self, creatures):
        ...
        arrays = VMobject(v_array, OldTex("+"), w_array, OldTex("="), sum_array)
        ...
        self.wait()
        self.play(ShowCreation(circle), ApplyMethod(mathy.change_mode, "plain"))
        self.wait()

    def add_scaling(self, arrows, syms, arrays):
        s_arrows = VMobject(
            OldTex("2"),
            Vector([1, 1]).set_color(YELLOW),
            OldTex("="),
            Vector([2, 2]).set_color(WHITE),
        )
    ...
    def fade_all_but(self, creatures, index):
        self.play(*[FadeOut(VMobject(pi, pi.title)) for pi in creatures[:index] + creatures[index + 1 :]])

    def restore_creatures(self, creatures):
        self.play(
            *[ApplyFunction(lambda m: m.change_mode("plain").set_color(m.color), pi) for pi in creatures]
            + [ApplyMethod(pi.title.set_fill, WHITE, 1.0) for pi in creatures]
        )
...
class HelpsToHaveOneThought(Scene):
    def construct(self):
        ...
        randys = VMobject(*[Randolph(color=color).scale(0.8) for color in (BLUE_D, BLUE_C, BLUE_E)])
        ...
        self.play(ShowCreation(speech_bubble), Transform(morty, new_morty), Write(words))
        self.wait(2)
        self.play(
...
class HowIWantYouToThinkAboutVectors(Scene):
    def construct(self):
        ...
        other_vectors = VMobject(*list(map(Vector, [[1, 2], [2, -1], [4, 0]])))
        ...
        self.wait(2)
        self.play(ShowCreation(plane, lag_ratio=0.5), Animation(vector))
        self.play(Write(axis_labels, run_time=1))
        self.wait()
        self.play(GrowFromCenter(dot), ShowCreation(line), Write(tail_word, run_time=1))
        self.wait()
        self.play(FadeOut(tail_word), ApplyMethod(VMobject(dot, line).scale, 0.01))
        self.remove(tail_word, line, dot)
        self.wait()

        self.play(ApplyMethod(vector.shift, shift_val, path_arc=3 * np.pi / 2, run_time=3))
        self.play(ApplyMethod(vector.shift, -shift_val, rate_func=rush_into, run_time=0.5))
        self.wait(3)

        self.play(ShowCreation(other_vectors, run_time=3))
        self.wait(3)
...
class ListsOfNumbersAddOn(Scene):
    def construct(self):
        arrays = VMobject(*list(map(matrix_to_mobject, [[-2, 3], [1, 2], [2, -1], [4, 0]])))
...
class CoordinateSystemWalkthrough(VectorScene):
    ...
    def introduce_coordinate_plane(self):
        ...
        self.wait(2)
        self.play(Write(origin_words), GrowFromCenter(dot), ShowCreation(line), run_time=1)
        self.wait(2)
        self.play(FadeOut(VMobject(origin_words, dot, line)))
        self.remove(origin_words, dot, line)
        self.wait()
        self.play(ShowCreation(tick_marks))
        self.play(GrowFromCenter(unit_brace), Write(one, run_time=1))
        self.wait(2)
        self.remove(unit_brace, one)
        self.play(*list(map(GrowFromCenter, plane_lines)) + [Animation(x_axis), Animation(y_axis)])
        self.wait()
        self.play(FadeOut(plane), Animation(VMobject(x_axis, y_axis, tick_marks)))
        self.remove(plane)
        self.add(tick_marks)
...
class VectorAddition(VectorScene):
    ...
    def ask_why(self, v1, v2, v_sum):
        ...
        new_v_sum.rotate(angle_of_vector(alt_vect_sum) - new_v_sum.get_angle())
        ...
        self.wait(2)
        self.play(FadeOut(why), Transform(v_sum, original_v_sum))
        self.remove(why)
        self.wait()
...
class AddingNumbersOnNumberLine(Scene):
    def construct(self):
        ...
        for vect, num in zip(vects, nums):
            self.play(ShowCreation(vect), Write(num, run_time=1))
            self.wait()
...
class VectorAdditionNumerically(VectorScene):
    def construct(self):
        ...
        self.play(ApplyFunction(lambda m: m.next_to(y_axis, RIGHT).to_edge(UP), coords1))
        ...
        self.play(
            ...
            *[ApplyMethod(mob.shift, v1.get_end()) for mob in (v2, x_line2, y_line2)],
        )
        ...
        self.play(Write(brackets), Write(equals), Transform(sum_x_start, sum_x), run_time=1)
        self.play(Transform(sum_y_start, sum_y))
        self.wait(2)

        starters = [x1, y1, x2, y2, sum_x_start, sum_y_start]
        variables = list(map(Tex, ["x_1", "y_1", "x_2", "y_2", "x_1+y_1", "x_2+y_2"]))
        for i, (var, starter) in enumerate(zip(variables, starters)):
            ...
            var.move_to(starter)
        self.play(
            Transform(VMobject(*starters[:4]), VMobject(*variables[:4])),
            FadeOut(sum_x_start),
            FadeOut(sum_y_start),
        )
...
class ShowScalarMultiplication(VectorScene):
    def construct(self):
        ...
        self.scale_vector(v, 1.0 / 3, label, factor_tex="\\dfrac{1}{3}")
    ...
    def scale_vector(self, v, factor, v_label, v_name="v", factor_tex=None):
        ...
        label = self.label_vector(scaled_vector, label_tex, animate=False, add_to_vector=False)
        self.remove(label)
    ...
    def describe_scalars(self, v, plane):
        ...
        scalars = VMobject(*list(map(Tex, ["2,", "\\dfrac{1}{3},", "-1.8,", "\\dots"])))
...
class ScalingNumerically(VectorScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            Transform(VMobject(two_dot.copy(), x.copy()), two_x_sym),
...
class ItDoesntMatterWhich(Scene):
    def construct(self):
        ...
        back_and_forth = VMobject(new_arrow, OldTex("\\Leftrightarrow"), matrix_to_mobject([2, 1]))
...
class DataAnalyst(Scene):
    def construct(self):
        ...
        ellipse_points = [ellipse.point_from_proportion(x) for x in np.arange(0, 1, 1.0 / 20)]
        string_vects = [matrix_to_mobject(("%.02f %.02f" % tuple(ep[:2])).split()) for ep in ellipse_points]
        string_vects_matrix = Matrix(np.array(string_vects).reshape((4, 5)))
        ...
        self.wait(2)
        self.play(FadeIn(plane), Transform(string_vects, vects))
...
class ManipulateSpace(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(FadeOut(self.plane), Write(matrix_rule), run_time=2)
        self.wait()
