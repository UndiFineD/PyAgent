# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\eola\chapter10.py
...
class StudentsFindThisConfusing(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            Write(title),
            *[ApplyMethod(pi.look_at, title) for pi in self.get_pi_creatures()],
        )
        self.play(
            self.get_teacher().look_at,
            students[-1].eyes,
            students[0].change_mode,
            "confused",
            students[1].change_mode,
            "tired",
            students[2].change_mode,
            "sassy",
        )
        self.random_blink()
        self.student_says(
        ...
        question2.target.next_to(question1, DOWN, aligned_edge=LEFT, buff=MED_SMALL_BUFF)
        equation = OldTex(
            "\\det\\left( %s \\right)=0"
        ...
        self.teacher_says(equation, added_anims=[MoveToTarget(question2)])
...
class EigenThingsArentAllThatBad(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Eigen-things aren't \\\\ actually so bad", target_mode="hooray")
        self.play_student_changes("pondering", "pondering", "erm")
        self.random_blink(4)
...
class VectorKnockedOffSpan(ExampleTranformationScene):
    def construct(self):
        ...
        vector.scale(0.7)
        top_words, off, span_label = all_words = OldTexText("\\centering Vector gets knocked", "\\\\ off", "Span")
        all_words.shift(line.point_from_proportion(0.75) - span_label.get_corner(DOWN + RIGHT) + MED_SMALL_BUFF * LEFT)
...
class VectorRemainsOnSpan(ExampleTranformationScene):
    def construct(self):
        ...
        self.wait()
        target_vectors = [vector.copy().scale(scalar) for scalar in (2, -2, 1)]
...
class IHatAsEigenVector(ExampleTranformationScene):
    ...
    def set_color_x_axis(self):
        ...
        targets = [self.i_hat.copy().scale(val) for val in (-FRAME_X_RADIUS, FRAME_X_RADIUS, 1)]
        lines = [Line(v1.get_end(), v2.get_end(), color=YELLOW) for v1, v2 in adjacent_pairs([self.i_hat] + targets)]
    ...
    def label_i_hat_landing_spot(self):
        ...
        self.play(
            ...
            Transform(VGroup(*matrix.get_mob_matrix()[:, 0]), array.get_entries()),
        )
        self.wait()
...
class FullSneakyEigenspace(ExampleTranformationScene):
    def construct(self):
        ...
        self.play(ShowCreation(vectors))
        self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[Transform(words.start, words)], path_arc=0)
        self.wait()
...
class NameEigenvectorsAndEigenvalues(ExampleTranformationScene):
    ...
    def construct(self):
        ...
        for words in x_words, sneak_words:
            ...
            eigen_val_words = OldTexText("with eigenvalue ", "%s" % words.get_tex()[-1])
        ...
        self.play(ShowCreation(non_eigen_span), Write(non_eigen_words), Animation(non_eigen))
...
class EigenvalueNegativeOneHalf(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.apply_transposed_matrix(self.t_matrix, added_anims=[FadeOut(words)])
        self.wait()
        self.play(
...
class ContrastMatrixUnderstandingWithEigenvalue(TeacherStudentsScene):
    def construct(self):
        axis_and_rotation = OldTexText("Rotate", "$30^\\circ$", "around", "$%s$" % matrix_to_tex_string([2, 3, 1]))
...
class WordsOnComputation(TeacherStudentsScene):
    def construct(self):
        ...
        self.random_blink()
        self.teacher_says("...but I'll hit the \\\\", "important parts")
...
class SymbolicEigenvectors(Scene):
    ...
    def introduce_terms(self):
        self.expression = OldTex("A", "\\vec{\\textbf{v}}", "=", "\\lambda", "\\vec{\\textbf{v}}")
        ...
        v_arrows = VGroup(*[Arrow(v_text.get_top(), v.get_bottom()) for v in vs])

        self.play(Write(self.expression))
        self.wait()
        self.play(GrowFromCenter(A_brace), Write(A_text))
        self.wait()
        self.play(Write(v_text), ShowCreation(v_arrows))
        self.wait()
        self.play(GrowFromCenter(lamb_brace), Write(lamb_text))
        self.wait(2)
        self.play(*list(map(FadeOut, [A_brace, A_text, lamb_brace, lamb_text, v_text, v_arrows])))
    ...
    def contrast_multiplication_types(self):
        ...
        for group in left_group, right_group:
            self.play(GrowFromCenter(group.brace), Write(group.text, run_time=2))
        ...
        solve_text = OldTexText("Solve for \\\\", "$\\lambda$", "and", "$\\vec{\\textbf{v}}$")
        ...
        self.play(FadeIn(morty), FadeIn(bubble), Write(solve_text))
        self.play(Blink(morty))
        self.wait(2)
        bubble.write("Fix different", "\\\\ multiplication", "types")
        self.play(Transform(solve_text, bubble.content), morty.change_mode, "sassy")
        self.play(Blink(morty))
        self.wait()
        self.play(
            *list(
    ...
    def rewrite_righthand_side(self):
        ...
        lamb_copy = lamb.copy()
        scaling_by = VGroup(OldTexText("Scaling by "), lamb_copy)
        scaling_by.arrange()
        arrow = OldTex("\\Updownarrow")
        matrix_multiplication = OldTexText("Matrix multiplication by")
        matrix = Matrix(np.identity(3, dtype=int))

        corner_group = VGroup(scaling_by, arrow, matrix_multiplication, matrix)
        corner_group.arrange(DOWN)
        corner_group.to_corner(UP + RIGHT)

        q_marks = VGroup(*[OldTex("?").replace(entry) for entry in matrix.get_entries()])
        q_marks.set_color_by_gradient(X_COLOR, Y_COLOR, Z_COLOR)
        diag_entries = VGroup(*[matrix.get_mob_matrix()[i, i] for i in range(3)])
        ...
        self.play(Transform(lamb.copy(), lamb_copy), Write(scaling_by))
        ...
        self.wait()
        self.play(Transform(q_marks, matrix.get_entries(), lag_ratio=0.5, run_time=2))
        self.remove(q_marks)
        self.add(*matrix.get_entries())
        self.wait()
        self.play(Transform(diag_entries.copy(), new_lamb), diag_entries.restore)
        self.remove(*self.get_mobjects_from_last_animation())
        self.add(new_lamb, diag_entries)
        self.play(GrowFromCenter(id_brace), Write(id_text))
        self.wait()
        ...
        for mob in lamb, id_text, v2:
            mob.target = mob.copy()
        VGroup(l_paren, lamb.target, id_text.target, r_paren, v2.target).arrange().next_to(equals).shift(
            SMALL_BUFF * UP
        )
        self.play(Write(parens), *list(map(MoveToTarget, [lamb, id_text, v2])))
        self.wait()
        self.play(*list(map(FadeOut, [corner_group, id_brace, id_text_copy, new_lamb])))
        self.expression = VGroup(A, v1, equals, VGroup(l_paren, lamb, id_text, r_paren), v2)
    ...
    def reveal_as_linear_system(self):
        ...
        for mob in movers:
            mob.target = mob.copy()
        VGroup(A.target, v1.target, minus, lamb_group.target, v2.target).arrange().next_to(equals, LEFT)
        self.play(
            Write(zero),
            Write(minus),
        ...
        matrix.scale(0.7)
        VGroup(matrix.get_brackets()[1], *matrix.get_mob_matrix()[:, 2]).shift(0.5 * RIGHT)
        matrix.next_to(brace.text, DOWN)
        for entry in matrix.get_entries():
            if len(entry.get_tex()) > 1:
                entry[-1].set_color(lamb.get_color())
        self.play(GrowFromCenter(brace), Write(brace.text), Write(matrix))
        self.wait()

        vect_words = OldTexText("We want a nonzero solution for")
        ...
        self.play(Write(vect_words), ShowCreation(arrow))
        self.wait()
    ...
    def bring_in_determinant(self):
        ...
        equation = OldTex("\\det(A-", "\\lambda", "I)", "=0")
        equation.set_color_by_tex("\\lambda", MAROON_B)
        equation.next_to(words, DOWN)
        words.add(equation)
        bubble.add_content(words)

        self.play(FadeIn(randy), ShowCreation(bubble), Write(words))
        self.play(Blink(randy))
        self.wait()
        everything = self.get_mobjects()
        equation_copy = equation.copy()
        self.play(FadeOut(VGroup(*everything)), Animation(equation_copy))
        self.play(equation_copy.center, equation_copy.scale, 1.5)
        self.wait()


class NonZeroSolutionsVisually(LinearTransformationScene):
    CONFIG = {"t_matrix": [[1, 1], [2, 2]], "v_coords": [2, -1]}

    def construct(self):
        equation = OldTex("(A-", "\\lambda", "I)", "\\vec{\\textbf{v}}", "= \\vec{\\textbf{0}}")
        ...
        self.apply_transposed_matrix(self.t_matrix)
        self.wait()
        transform = Transform(equation_matrix.copy(), det_equation_matrix)
...
class TweakLambda(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.add_foreground_mobject(self.matrix, *self.lambda_vals)
        ...
        for mob in self.plane_mobjects:
            mob.save_state()
        self.apply_transposed_matrix(self.t_matrix)
        self.play(Write(det_text), Write(equals), ShowCreation(det.rect), Write(det))
    ...
    def range_lambda(self, start_val, end_val, run_time=3):
        ...
        matrix_transform = self.get_matrix_transformation(self.get_t_matrix(end_val))
        transformations = []
        for mob in self.plane_mobjects:
            mob.target = mob.copy().restore().apply_function(matrix_transform)
            transformations.append(MoveToTarget(mob))
        transformations += [
            Transform(self.i_hat, Vector(matrix_transform(RIGHT), color=X_COLOR)),
            Transform(self.j_hat, Vector(matrix_transform(UP), color=Y_COLOR)),
        ]
        for alpha in alphas:
            self.clear()
...
class LineOfReasoning(Scene):
    def construct(self):
        ...
        expressions.arrange(DOWN, buff=LARGE_BUFF / 2.0)
...
class IfYouDidntKnowDeterminants(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(ShowCreation(bubble), Write(bubble.content), student.change_mode, "confused")
        self.random_blink(4)
...
class RevisitExampleTransformation(ExampleTranformationScene):
    ...
    def introduce_matrix(self):
        ...
        for coords, vect in (i_coords, self.i_hat), (j_coords, self.j_hat):
            ...
            coords.rect = BackgroundRectangle(coords)
            coords.add_to_back(coords.rect)
            coords.next_to(vect.get_end(), RIGHT + DOWN if coords is i_coords else RIGHT)
        self.play(
            Write(i_coords),
            Write(j_coords),
        )
        ...
        self.play(
            ...
            Transform(j_coords.get_entries(), VGroup(*self.matrix.get_mob_matrix()[:, 1])),
        )
        to_remove += self.get_mobjects_from_last_animation()
        self.wait()
        self.remove(*to_remove)
    ...
    def get_lambda_to_diag_movements(self, lamb):
        ...
        rect.target = BackgroundRectangle(VGroup(l_bracket.target, r_bracket.target))
        result = list(map(MoveToTarget, movers))
        result += list(map(Write, [minus1, minus2]))
        result += list(map(Animation, [self.matrix.get_mob_matrix()[i, 1 - i] for i in range(2)]))
        self.diag_entries = [
    ...
    def show_determinant(self, to_fade=None):
        ...
        equals = OldTex("=").next_to(det_text)
        three_minus_lamb, two_minus_lamb = diag_entries = [entry.copy() for entry in self.diag_entries]
        ...
        VGroup(
            lp1,
            three_minus_lamb.target,
            rp1,
            lp2,
            two_minus_lamb.target,
            rp2,
            minus,
            one.target,
            cdot,
        ...
        brace_text.target = brace.target.get_text("Quadratic polynomial in ", "$\\lambda$")
        ...
        final_text = OldTex("\\lambda", "=2", "\\text{ or }", "\\lambda", "=3")
        ...
        self.play(Write(det_text), Write(equals))
        self.wait()
        self.play(
            Write(parens),
        ...
        self.wait()
        self.play(Transform(brace, final_brace), Transform(brace_text, final_text))
        self.wait()
        faders = [
            det_text,
            equals,
        ...
        self.play(
            *it.chain(
                ...
                [
                    lambda_equals_two.scale,
                    1.3,
                    lambda_equals_two.next_to,
                    self.matrix,
                    DOWN,
                ],
            )
    ...
    def show_diagonally_altered_transform(self):
        ...
        vectors = VGroup(
            *[self.add_vector(u * x * (LEFT + UP), animate=False) for x in range(4, 0, -1) for u in [-1, 1]]
        )
        vectors.set_color_by_gradient(MAROON_B, YELLOW)
        vectors.save_state()
        self.play(
            ShowCreation(vectors, lag_ratio=0.5, run_time=2),
            *list(map(Animation, self.foreground_mobjects)),
        )
        self.wait()
        self.apply_transposed_matrix(self.t_matrix - 2 * np.identity(2))
        self.wait()
        self.play(
            *it.chain(
...
class Rotate90DegreesWithVector(Rotate90Degrees):
    CONFIG = {"example_vector_coords": [1, 2]}
...
class SolveRotationEigenvalues(Rotate90Degrees):
    def construct(self):
        ...
        self.wait()
        diag_entries = [self.matrix.get_mob_matrix()[i, i] for i in range(2)]
        off_diag_entries = [self.matrix.get_mob_matrix()[i, 1 - i] for i in range(2)]
        for entry in diag_entries:
        ...
        polynomial = OldTex("=", "\\lambda^2", "+1=0")
        ...
        result = OldTex("\\lambda", "= i", "\\text{ or }", "\\lambda", "= -i")
...
class ShearExample(RevisitExampleTransformation):
    ...
    def point_out_eigenvectors(self):
        ...
        vectors.set_color_by_gradient(YELLOW, X_COLOR)
        words = VGroup(OldTexText("Eigenvectors"), OldTexText("with eigenvalue", "1"))
        for word in words:
    ...
    def show_determinant(self):
        ...
        self.play(GrowFromCenter(brace), Write(brace_text))
        self.wait()
        self.play(*list(map(FadeOut, [one, zero, minus, cdot, new_rect, brace, brace_text])))
        self.wait()
        self.play(Write(equals_0))
        self.wait()
        self.play(FadeIn(final_brace), FadeIn(final_text))
        self.wait()
        # faders = [
...
class IntroduceEigenbasis(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("pondering", "raise_right_hand", "erm")
        self.random_blink()
        ...
        student = self.get_students()[0]
        self.play(student.change_mode, "guilty", student.look, LEFT)
        self.random_blink(2)


class BasisVectorsAreEigenvectors(LinearTransformationScene):
    CONFIG = {"t_matrix": [[-1, 0], [0, 2]]}
    ...
    def construct(self):
        ...
        words = OldTexText("What if both basis vectors \\\\", "are eigenvectors?")
        for word in words:
            word.add_to_back(BackgroundRectangle(word))
        ...
        self.play(
            ...
            i_coords.get_entries().move_to,
            VGroup(*matrix.get_mob_matrix()[:, 0]),
        )
        self.play(
            FadeOut(j_coords.rect),
            FadeOut(j_coords.get_brackets()),
            j_coords.get_entries().move_to,
        ...
        diag_entries = VGroup(*[matrix.get_mob_matrix()[i, i] for i in range(2)])
        off_diag_entries = VGroup(*[matrix.get_mob_matrix()[1 - i, i] for i in range(2)])
...
class DefineDiagonalMatrix(Scene):
    def construct(self):
        ...
        diag_entries = VGroup(*[matrix.get_mob_matrix()[i, i] for i in range(n_dims)])
        off_diag_entries = VGroup(
            *[matrix.get_mob_matrix()[i, j] for i in range(n_dims) for j in range(n_dims) if i != j]
        )
        ...
        self.play(Write(title))
        self.wait()
        self.play(matrix.set_column_colors, X_COLOR, Y_COLOR, Z_COLOR, YELLOW)
        self.wait()
...
class RepeatedMultiplicationInAction(Scene):
    def construct(self):
        ...
        l_bracket.target.next_to(VGroup(*[scalar.target for scalar in scalars]), LEFT)

        self.add(vector)
        self.play(*list(map(FadeIn, [matrix] + scalars)))
        self.wait()
        self.play(FadeOut(matrix), *list(map(MoveToTarget, scalars + [l_bracket])))
        self.wait()
        # nth multiplications
        for scalar in scalars:
            scalar.exp = VectorizedPoint(scalar.get_corner(UP + RIGHT))
            scalar.exp.shift(SMALL_BUFF * RIGHT / 2.0)
        for new_exp in range(2, 6):
            ...
            for v_entry, scalar, new_scalar in zip(vector.get_entries(), scalars, new_scalars):
                ...
                scalar.target = scalar.copy()
                VGroup(scalar.target, scalar.exp.target).next_to(v_entry, LEFT, aligned_edge=DOWN)
            ...
            self.wait()
            self.play(FadeOut(matrix), *list(map(MoveToTarget, movers)))
            self.remove(*to_remove)
            self.wait()
...
class RepeatedMultilpicationOfMatrices(Scene):
    ...
    def construct(self):
        ...
        if self.diagonal:
            ...
            VGroup(last_matrix.target, hundred.target, hundred_copy.target).next_to(vector, LEFT)
...
class ChangeToEigenBasis(ExampleTranformationScene):
    ...
    def introduce_eigenvectors(self):
        x_vectors, v_vectors = [
            VGroup(*[self.add_vector(u * x * vect, animate=False) for x in range(num, 0, -1) for u in [-1, 1]])
        ...
        self.apply_transposed_matrix(self.t_matrix, rate_func=there_and_back, path_arc=0)
    ...
    def write_change_of_basis_matrix(self):
        ...
        cob_matrix = Matrix(np.array([list(vect.entries.target) for vect in (b1, b2)]).T)
        ...
        inv_cob.target.next_to(self.matrix, LEFT, buff=neg_1.get_width() + 2 * SMALL_BUFF)
        neg_1.next_to(
            inv_cob.target.get_corner(UP + RIGHT),
            RIGHT,
        )
        self.play(MoveToTarget(inv_cob, path_arc=-np.pi / 2), Write(neg_1))
        self.wait()
        ...
        self.play(Write(equals), Write(final_matrix))
        self.wait()
    ...
    def ask_about_power(self):
        ...
        bubble = morty.get_bubble("speech", height=3, width=5, direction=RIGHT)
        ...
        self.play(morty.change_mode, "speaking", ShowCreation(bubble), Write(words))
        for x in range(2):
            self.play(Blink(morty))
            self.wait(2)
