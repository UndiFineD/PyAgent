...
class FromKochToSpaceFilling(Scene):
    ...
    def show_angles(self):
        ...
        koch, sharper_koch, duller_koch = curves = [
            CurveClass(order=1) for CurveClass in (StraightKoch, SharperKoch, DullerKoch)
        ]
        arcs = [
            Arc(2 * (np.pi / 2 - curve.angle), radius=r, start_angle=np.pi + curve.angle).shift(
                curve.get_points()[curve.get_num_points() / 2]
            )
    ...
    def show_change_side_by_side(self):
        ...
        sharper, duller, space_filling = [
            CurveClass(order=1).shift(3 * LEFT) for CurveClass in (SharperKoch, DullerKoch, SpaceFillingKoch)
        ]
