# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\hilbert\section1.py
...
class AboutSpaceFillingCurves(TransformOverIncreasingOrders):
    ...
    def show_infinite_objects(self):
        ...
        rationals, other_equals, naturals = OldTex(["|\\mathds{Q}|", "=", "|\\mathds{N}|"]).scale(2).split()
    ...
    def pose_question(self):
        infinity, rightarrow, N = OldTex(["\\infty", "\\rightarrow", "N"]).scale(2).split()
        question_mark = OldTexText("?").scale(2)

        self.add(question_mark)
        self.wait()
        self.play(
            *[ShimmerIn(mob) for mob in (infinity, rightarrow, N)]
            + [
...
class PostponePhilosophizing(Scene):
    def construct(self):
        abstract, arrow, concrete = OldTexText(["Abstract", " $\\rightarrow$ ", "Concrete"]).scale(2).split()

        self.add(abstract, arrow, concrete)
        self.wait()
        self.play(
            *[
                ApplyMethod(word1.replace, word2, path_func=path_along_arc(np.pi / 2))
...
class GrowHilbertWithName(Scene):
    def construct(self):
        ...
        self.play(ShimmerIn(words), Transform(curve, HilbertCurve(order=2)), run_time=2)
        for n in range(3, 8):
            self.play(Transform(curve, HilbertCurve(order=n)), run_time=5.0 / n)
...
class ImageToSound(Scene):
    def construct(self):
        ...
        self.add(picture)
        self.wait()
        self.play(Transform(picture, string.mobject, run_time=3, rate_func=rush_into))
        self.remove(picture)
        self.play(string)
        ...
        self.play(Transform(string.mobject, picture_copy, run_time=5, rate_func=rush_from))
...
class SoundDataIsOneDimensional(Scene):
    def construct(self):
        ...
        component_strings = [
            Vibrate(num_periods=k + 1, overtones=1, color=color, center=2 * DOWN + UP * k)
            for k, color in zip(list(range(overtones)), Color(BLUE_E).range_to(WHITE, overtones))
        ]
        dots = [Dot(string.mobject.get_center(), color=string.mobject.get_color()) for string in component_strings]
        ...
        self.play(*[TransformAnimations(main_string.copy(), string, run_time=5) for string in component_strings])
        self.clear()
        self.play(*[TransformAnimations(string, Animation(dot)) for string, dot in zip(component_strings, dots)])
        self.clear()
        self.play(
...
class AssociatePixelWithFrequency(Scene):
    def construct(self):
        ...
        pixel = MobjectFromRegion(
            region_from_polygon_vertices(*0.2 * np.array([RIGHT + DOWN, RIGHT + UP, LEFT + UP, LEFT + DOWN]))
        )
        ...
        vibration, loud_vibration, quiet_vibration = [
            Vibrate(string.copy(), amplitude=a, **vibration_config) for a in [0.5, 1.0, 0.25]
        ]
...
class ListenToAllPixels(Scene):
    def construct(self):
        ...
        colors = [Color(rgb=interpolate(np.array(red.rgb), np.array(blue.rgb), alpha)) for alpha in np.arange(4) / 3.0]
        ...
        sub_vibrations = [
            Vibrate(
                ...
                overtones=1,
                spatial_period=6.0 / (n + 1),
                temporal_period=1.0 / (n + 1),
                amplitude=0.5 / (n + 1),
            )
            for n in range(4)
        ]
        ...
        self.add(grid)
        self.wait()
        self.play(DelayByOrder(ApplyMethod(grid.set_color_by_gradient, red, blue)))
        self.play(Transform(grid, freq_line))
        self.wait()
        self.play(
            ShimmerIn(words, rate_func=squish_rate_func(smooth, 0, 0.2)),
            *sub_vibrations,
            run_time=5,
        )
        self.play(*[TransformAnimations(sub_vib, vibration) for sub_vib in sub_vibrations] + [FadeOut(words)])
        self.clear()
        self.add(freq_line)
        self.play(vibration)
...
class LayAsideSpeculation(Scene):
    def construct(self):
        ...
        self.add(words)
        self.wait()
        self.play(Transform(words, lower_right), Transform(lower_left, mapping))
        self.wait()
...
class ThinkInTermsOfReverseMapping(Scene):
    def construct(self):
        ...
        dot3, arrow3 = [mob.copy().shift(5 * LEFT + UP) for mob in (dot1, arrow1)]
        dot4, arrow4 = [mob.copy().shift(5 * LEFT + 0.9 * UP) for mob in (dot2, arrow2)]

        self.add(grid, freq_line, arrow)
        self.wait()
        self.play(ApplyMethod(arrow.rotate, np.pi, path_func=clockwise_path()))
        self.wait()
        ...
        self.play(Transform(dot1, dot3), Transform(dot2, dot4))
        self.play(ApplyMethod(grid.fade, 0.8), Animation(Mobject(dot3, dot4)))
...
class WeaveLineThroughPixels(Scene):
    ...
    def construct(self, order):
        ...
        unit = 6.0 / (2**order)  # sidelength of pixel
        up = unit * UP
        right = unit * RIGHT
        ...
        for square in squares.submobjects:
            ...
            distances = np.apply_along_axis(lambda p: get_norm(p - center), 1, curve.points)
        ...
        self.wait()
        self.play(Transform(curve, line), Transform(squares, targets), run_time=3)
        self.wait()
...
class WellPlayedGameOfSnake(Scene):
    def construct(self):
        ...
        self.add(grid)
        self.play(ShowCreation(snake_curve, run_time=7, rate_func=linear))
        self.wait()
...
class TellMathematicianFriend(Scene):
    def construct(self):
        ...
        squiggle_mouth.apply_function(lambda x_y_z: (x_y_z[0], x_y_z[1] + 0.02 * np.sin(50 * x_y_z[0]), x_y_z[2]))
        ...
        bubble.write(
            "Well, no, it \\emph{is} just one thing, but I need \\\\ \
        ...
        self.add(mathy)
        self.play(ShowCreation(arrow), ShimmerIn(description))
        self.wait()
        ...
        self.wait(2)
        self.play(
            ApplyPointwiseFunction(lambda p: 15 * p / get_norm(p), bubble),
...
class Order1PseudoHilbertCurve(Scene):
    def construct(self):
        ...
        self.add(words, s)
        self.wait()
        self.play(Transform(s, pre_words, path_func=path_along_arc(-np.pi / 3)))
        self.wait()
        self.play(ShowCreation(grid1))
        self.wait()
        self.play(ShowCreation(grid2))
        self.wait()
        kwargs = {"run_time": 5, "rate_func": None}
...
class GrowToOrder8PseudoHilbertCurve(Scene):
    ...
    def increase_order(self):
        ...
        self.play(*[GrowFromCenter(mini_curve) for mini_curve in mini_curves[1:]])
        self.wait()
        self.clear()
...
class UseOrder8(Scene):
    def construct(self):
        ...
        self.clear()
        self.add(words)
        self.play(ShowCreation(curve, run_time=7, rate_func=linear))
        self.wait()
...
class HilbertBetterThanSnakeQ(Scene):
    def construct(self):
        hilbert_curves, snake_curves = [
            [CurveClass(order=n) for n in range(2, 7)] for CurveClass in (HilbertCurve, SnakeCurve)
        ]
        ...
        for new_hc, new_sc in zip(hilbert_curves[1:], snake_curves[1:]):
            self.play(*[Transform(hilbert_curve, new_hc), Transform(snake_curve, new_sc)])
            self.wait()
...
class IncreaseResolution(Scene):
    def construct(self):
        grids = [Grid(2**order, 2**order, stroke_width=1).shift(0.3 * DOWN) for order in (6, 7)]
        grid = grids[0]
...
class IncreasingResolutionWithSnakeCurve(Scene):
    def construct(self):
        ...
        start_dots, end_dots = [
            Mobject(
                *[
                    Dot(curve.get_points()[int(x * curve.get_num_points())], color=color)
                    for x, color in [(0.202, GREEN), (0.48, BLUE), (0.7, RED)]
                ]
            )
        ...
        self.wait()
        self.play(ShowCreation(start_dots, run_time=2), ApplyMethod(start_curve.fade))
        end_curve.fade()
        self.play(Transform(start_curve, end_curve), Transform(start_dots, end_dots))
        self.wait()
...
class TrackSpecificCurvePoint(Scene):
    ...
    def construct(self):
        ...
        dots = [Dot(curve.get_points()[alpha * curve.get_num_points()]) for curve in curves]
