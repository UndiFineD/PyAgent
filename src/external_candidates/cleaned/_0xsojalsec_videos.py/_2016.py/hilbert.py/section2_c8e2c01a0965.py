...
class HistoryOfDiscover(Scene):
    def construct(self):
        ...
        squares = Mobject(
            *[Square(side_length=3, color=WHITE).replace(curve) for curve in (hilbert_curve, peano_curve)]
        )
...
class FormalDefinitionOfContinuity(Scene):
    ...
    def setup(self):
        ...
        self.output_homotopy = (
            lambda x_y_z_t1: (x_y_z_t1[0], x_y_z_t1[1], x_y_z_t1[2]) + output_points[int(x_y_z_t1[3] * output_size)]
        )
...
class VaryCircles(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        self.mobject = Mobject(
            *self.scene.get_circles_and_points(self.input_value - radius, self.input_value + radius)
        ).ingest_submobjects()
