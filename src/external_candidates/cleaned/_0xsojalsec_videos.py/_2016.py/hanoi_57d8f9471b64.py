# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\hanoi.py
        ...
        while continue_rolling_over:
            ...
            if run_all_at_once:
                ...
                def get_modified_rate_func(anim):
                    return lambda t: anim.original_rate_func(start_t + t / denom)
            ...
            self.play(MoveToTarget(moving_dot), *added_anims, run_time=run_time_per_anim)
...
class TowersOfHanoiScene(Scene):
    ...
    def add_pegs(self):
        ...
        self.pegs = VGroup(
            *[peg.copy().shift(vect) for vect in (self.peg_spacing * LEFT, ORIGIN, self.peg_spacing * RIGHT)]
        )
        self.add(self.pegs)
        if self.include_peg_labels:
            self.peg_labels = VGroup(*[OldTex(char).next_to(peg, DOWN) for char, peg in zip("ABC", self.pegs)])
        ...
        self.disks = VGroup(
            *[
                ...
                for width, color in zip(
                    np.linspace(self.disk_min_width, self.disk_max_width, self.num_disks),
                    color_gradient(self.disk_start_and_end_colors, self.num_disks),
                )
            ]
        )
    ...
    def reset_disks(self, **kwargs):
        ...
        self.play(MoveToTarget(self.disks), **kwargs)
...
class IntroduceKeith(Scene):
    def construct(self):
        ...
        self.play(morty.change_mode, "pondering", morty.look_at, bubble)
        ...
        self.play(morty.change_mode, "happy", morty.look_at, keith.eyes)
        self.play(Blink(keith))
        bubble.write("But \\emph{analyzing} puzzles!")
        VGroup(*bubble.content[3:12]).set_color(YELLOW)
        self.play(keith.change_mode, "hooray", Transform(original_content, bubble.content))
        self.play(Blink(morty))
        self.wait()
        ...
        self.play(Blink(keith))
        self.play(keith.change_mode, "hooray", keith.look_at, randy.eyes)
        self.wait()
...
class IntroduceTowersOfHanoi(TowersOfHanoiScene):
    ...
    def show_setup(self):
        ...
        self.play(
            ApplyMethod(self.pegs.restore, lag_ratio=0.5, run_time=2),
    ...
    def bring_in_disks(self):
        ...
        self.play(FadeIn(disk_groups, run_time=2, lag_ratio=0.5))
    ...
    def note_disk_labels(self):
        ...
        for label in labels:
            label.save_state()
            self.play(label.scale, 2, label.set_color, YELLOW, last.restore, run_time=0.5)
            last = label
    ...
    def cannot_move_disk_onto_smaller_disk(self):
        ...
        disk = self.disks[2]
        disk.save_state()
        self.move_disks_to_peg([2], 2, added_anims=[Transform(self.title, also_not_allowed, run_time=1)])
        cross.replace(disk)
        self.play(FadeIn(cross))
        self.wait()
        self.play(FadeOut(cross), FadeOut(self.title), disk.restore)
        self.wait()
...
class ExampleFirstMoves(TowersOfHanoiScene):
    def construct(self):
        ...
        self.wait()
        self.play(self.disks[0].set_fill, YELLOW, self.disks[0].label.set_color, BLACK)
        self.wait()
        self.move_disk(0)
        self.wait()
        self.play(self.disks[1].set_fill, YELLOW_D, self.disks[1].label.set_color, BLACK)
...
class KeithShowingBinary(Scene):
    def construct(self):
        ...
        def get_bit_flip():
            return Transform(bits, bits_list.pop(0), rate_func=squish_rate_func(smooth, 0, 0.7))
        ...
        self.play(morty.change_mode, "pondering", morty.look_at, bits, get_bit_flip())
        while bits_list:
            added_anims = []
...
class FocusOnRhythm(Scene):
    def construct(self):
        ...
        for x in sequence:
            ...
            self.play(movers.shift, 0.2 * DOWN, rate_func=there_and_back, run_time=0.25)
...
class IntroduceBase10(Scene):
    ...
    def expand_example_number(self):
        ...
        self.play(
            MoveToTarget(number),
            ShowCreation(arrows),
            ApplyMethod(expansion.restore, lag_ratio=0.5),
            run_time=2,
        )
        self.play(Write(title))
        self.wait()
...
class RhythmOfDecimalCounting(CountingScene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), Write(two_digits, run_time=1))
        self.wait(2)
        self.play(Transform(brace, one_brace), Transform(two_digits, tens_place))
        self.wait()
        ...
        self.wait()
        self.play(ten_group.restore, *list(map(FadeOut, [brace, two_digits])))
        ...
        self.play(hundred_group.scale, 14, hundred_group.to_corner, DOWN + LEFT)
        self.wait()
        self.play(hundred_group.restore)
        self.wait()
        groups = [VGroup(*pair) for pair in zip(self.dot_templates, self.curr_configurations)]
        self.play(
            groups[2].to_edge,
            RIGHT,
            MaintainPositionRelativeTo(groups[1], groups[2]),
            MaintainPositionRelativeTo(groups[0], groups[2]),
            self.number_mob.to_edge,
            RIGHT,
            LARGE_BUFF,
            FadeOut(self.arrows),
        )
...
class IntroduceBinaryCounting(BinaryCountingScene):
    ...
    def introduce_name(self):
        ...
        self.play(GrowFromCenter(brace), Write(bits))
        self.wait()
        bits.save_state()
    ...
    def initial_counting(self):
        ...
        bubble.write("Not ten, not ten \\\\", "\\quad not ten, not ten...")
        ...
        self.play(GrowFromCenter(brace), Write(twos_place))
        self.play(FadeIn(randy), ShowCreation(bubble))
        self.play(
            randy.change_mode,
            "hesitant",
            randy.look_at,
    ...
    def show_self_similarity(self):
        ...
        self.play(FadeIn(cover_rect), Animation(big_dot))
        self.play(
            big_dot.center,
            big_dot.set_height,
            FRAME_HEIGHT - 2,
...
class BinaryCountingAtEveryScale(Scene):
    ...
    def construct(self):
        title = OldTexText(
            "Count to %d (which is %s in binary)" % (2**self.num_bits - 1, bin(2**self.num_bits - 1)[2:])
        )
        title.to_edge(UP)
        if self.show_title:
            self.add(title)

        bit_mobs = [get_binary_tex_mob(n, self.num_bits) for n in range(2**self.num_bits)]
        ...
        def get_run_through(mobs):
            return Succession(
                *[Transform(curr_bits, mob, rate_func=squish_rate_func(smooth, 0, 0.5)) for mob in mobs],
                run_time=1,
            )
        ...
        self.wait()
        self.play(get_run_through(bit_mobs[2 ** (self.num_bits - 1) + 1 :]), Write(again))
        self.wait()
...
class IntroduceSolveByCounting(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        braces = [Brace(VGroup(*self.curr_bit_mob[:n])) for n in range(1, self.num_disks + 1)]
        word_list = [
            brace.get_text(text)
            for brace, text in zip(
                braces,
                [
                    "Only flip last bit",
                    "Roll over once",
        ...
        self.play(
            last_bit.restore,
            disk.set_fill,
            disk.original_fill_color,
            self.disks[0][1].set_fill,
            BLACK,
        )
        ...
        self.play(FadeIn(cross))
        self.wait()
        self.move_disk_to_peg(1, 2, stay_on_peg=False, added_anims=[FadeOut(cross)])
        self.play(
            disk.set_fill,
            disk.original_fill_color,
            disk[1].set_fill,
        ...
        self.move_disk(0, added_anims=[self.get_increment_animation()], run_time=2)
        self.wait()
        ...
        self.play(disk.set_fill, RED, disk[1].set_fill, BLACK, fours_bit.set_fill, RED)
        ...
        self.play(disk.set_fill, GREEN, disk[1].set_fill, BLACK, eights_bit.set_fill, GREEN)
    ...
    def initialize_bit_mobs(self):
        bit_mobs = VGroup(*[get_binary_tex_mob(n, self.num_disks) for n in range(2 ** (self.num_disks))])
        bit_mobs.scale(2)
...
class RecursionTime(Scene):
    def construct(self):
        ...
        self.play(
            ...
            bubble.content.set_color_by_gradient,
            BLUE_A,
            BLUE_E,
        )
        self.play(Blink(morty))
...
class RecursiveSolution(TowersOfHanoiScene):
    ...
    def construct(self):
        ...
        steps = VGroup(title, sub_step_brace, sub_steps, sub_sub_steps_brace, sub_sub_steps)
        steps.arrange(DOWN)
        steps.scale(0.7)
        steps.to_edge(UP)
        ...
        self.move_disk_to_peg(
            ...
            0,
            run_time=2,
            added_anims=[self.eyes.look_at_anim(self.pegs[0].get_top())],
        )
        self.shake(big_disk)
        self.move_disk_to_peg(
            0,
            2,
            run_time=2,
            added_anims=[self.eyes.look_at_anim(self.pegs[2].get_bottom())],
        )
        self.change_mode("angry")
        self.move_disk_to_peg(
            0,
            1,
            run_time=2,
            added_anims=[self.eyes.look_at_anim(self.disks[1].get_top())],
        )
        self.blink()

        # Final moves for big case
        self.move_disk(3, run_time=2, added_anims=[Write(sub_steps[1])])
        ...
        self.play(VGroup(*self.disks[:3]).move_to, self.pegs[1], DOWN)
        self.disk_tracker = [set([]), set([0, 1, 2]), set([3])]
        arc = Arc(-5 * np.pi / 6, start_angle=5 * np.pi / 6)
        arc.add_tip()
        arc.set_color(YELLOW)
        arc.set_width(VGroup(*self.pegs[1:]).get_width() * 0.8)
        arc.next_to(self.disks[0], UP + RIGHT, buff=SMALL_BUFF)
        q_mark = OldTexText("?")
        q_mark.next_to(arc, UP)
        self.play(ShowCreation(arc), Write(q_mark), sub_steps[-1].set_color, YELLOW)
        self.wait()
        self.play(GrowFromCenter(sub_sub_steps_brace), *list(map(FadeOut, [arc, q_mark])))

        # Disk 2 frustration
        big_disk = self.disks[2]
        self.eyes.move_to(big_disk.get_top(), DOWN)
        self.play(FadeIn(self.eyes), big_disk.set_fill, RED, big_disk.label.set_fill, BLACK)
        big_disk.add(self.eyes)
        ...
        self.blink()
        self.move_disk_to_peg(2, 2, run_time=2, added_anims=[Write(sub_sub_steps[1])])
        self.look_at(self.disks[0])
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(
            2,
        ...
        big_disk.add(self.eyes)
        self.move_subtower_to_peg(3, 1, run_time=2, added_anims=[self.eyes.look_at_anim(self.pegs[0])])
    ...
    def shake(self, mobject, direction=UP, added_anims=[]):
        self.play(mobject.shift, 0.2 * direction, rate_func=wiggle, *added_anims)
...
class KeithSaysBigToSmall(Scene):
    def construct(self):
        ...
        self.add(keith)
        self.play(Blink(keith))
        self.play(keith.change_mode, "speaking", ShowCreation(bubble), Write(bubble.content))
        self.wait()
        self.play(Blink(keith))
        self.wait()
...
class CodeThisUp(Scene):
    def construct(self):
        ...
        self.play(keith.change_mode, "speaking", keith.look_at, point)
        self.play(morty.change_mode, "pondering", morty.look_at, point)
        self.play(Blink(keith))
        self.wait(2)
        self.play(Blink(morty))
        self.wait()
        self.play(keith.change_mode, "hooray", keith.look_at, morty.eyes)
        self.play(Blink(keith))
        self.wait()
        self.play(
            keith.change_mode,
...
class NoRoomForInefficiency(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(morty.change_mode, "speaking", ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
...
class BothAreSelfSimilar(Scene):
    def construct(self):
        ...
        self.add(morty)
        self.play(morty.change_mode, "hooray", ShowCreation(bubble), Write(bubble.content))
        self.play(Blink(morty))
        self.wait()
...
class ShowFourDiskFourBitsParallel(IntroduceSolveByCounting):
    ...
    def counting_subtask(self):
        ...
        #     )
        self.play(
            Succession(*[self.get_increment_animation() for x in range(num_tasks)]),
            rate_func=linear,
...
class MentionLastVideo(Scene):
    def construct(self):
        ...
        self.play(Blink(morty))
        self.wait(2)
        self.play(morty.change_mode, "confused", morty.look_at, point)
        self.play(Blink(keith))
        self.wait(2)
        self.play(morty.change_mode, "surprised")
        self.wait()
...
class RecursiveSolutionToConstrained(RecursiveSolution):
    ...
    def construct(self):
        ...
        subdivisions.append(OldTexText("\\tiny Move disk 0, Move disk 0").set_color(BLUE))
        braces = [Brace(steps, UP) for steps in subdivisions]
        sub_steps_brace, sub_sub_steps_brace = braces[:2]
        steps = VGroup(title, *it.chain(*list(zip(braces, subdivisions))))
        steps.arrange(DOWN)
        steps.to_edge(UP)

        steps_to_fade = VGroup(title, sub_steps_brace, *list(sub_steps[:2]) + list(sub_steps[3:]))
        self.add(title)

        # Initially move big disk
        self.play(FadeIn(self.eyes), big_disk.set_fill, GREEN, big_disk.label.set_fill, BLACK)
        big_disk.add(self.eyes)
        big_disk.save_state()
        self.blink()
        ...
        self.play(Write(blocking[1]), ShowCreation(arrow))
        ...
        self.blink()
        self.move_disk_to_peg(num_disks - 1, 1, added_anims=[Write(sub_sub_steps[1], run_time=1)])
        self.wait()
        self.move_subtower_to_peg(
            num_disks - 1,
            0,
        ...
        self.blink()
        self.move_disk_to_peg(num_disks - 1, 2, added_anims=[Write(sub_sub_steps[3], run_time=1)])
        self.wait()
        big_disk.remove(self.eyes)
        self.move_subtower_to_peg(
            num_disks - 1,
        ...
        smaller_subdivision = VGroup(*list(subdivisions[2:]) + list(braces[2:]))
        ...
        self.play(
            ApplyMethod(
                VGroup(VGroup(braces[-2], subdivisions[-2])).set_fill,
                None,
                1,
                run_time=3,
                lag_ratio=0.5,
        ...
        for mob in last_subdivisions:
            self.play(ApplyMethod(mob.set_fill, None, 1), self.eyes.look_at_anim(mob))
        self.blink()
        self.play(FadeOut(self.eyes))
        self.wait()
        ...
        for disk_index, peg_index in movements:
            self.move_disk_to_peg(disk_index, peg_index, stay_on_peg=False)
        self.wait()
...
class SolveConstrainedByCounting(ConstrainedTowersOfHanoiScene):
    ...
    def construct(self):
        ...
        for index in get_ternary_ruler_sequence(self.num_disks - 1):
            self.move_disk(index, stay_on_peg=False, added_anims=[self.increment_animation()])

    def increment_animation(self):
        return Succession(
            Transform(
                self.curr_ternary_mob,
                next(self.ternary_mob_iter),
                lag_ratio=0.5,
                path_arc=np.pi / 6,
            ),
    ...
    def construct(self):
        ...
        for title, symbol in zip(titles, symbols):
            ...
            if title is not ternary:
                added_anims += [FadeIn(symbol.brace), FadeIn(symbol.name)]
        ...
        self.play(morty.change_mode, "sassy", morty.look_at, zero_one_two)
        self.play(Blink(morty))
        self.wait()
        self.play(
            morty.shrug,
...
class IntroduceTernaryCounting(CountingScene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), Write(threes_place))
        self.wait()
        for x in range(6):
            self.increment()
            self.wait()
...
class TernaryCountingSelfSimilarPattern(Scene):
    ...
    def construct(self):
        ...
        ternary_mobs = VGroup(*[get_ternary_tex_mob(n, n_trits=self.num_trits) for n in range(3**self.num_trits)])
        ...
        for i, step in enumerate(steps):
            ...
            if i % 2 == 0:
                self.play(
                    Succession(*[get_increment() for x in range(3 ** (self.num_trits - 1) - 1)]),
                    run_time=1,
                )
            else:
...
class SolveConstrainedWithTernaryCounting(ConstrainedTowersOfHanoiScene):
    ...
    def setup(self):
        ...
        ternary_mobs = VGroup(*[get_ternary_tex_mob(x) for x in range(3**self.num_disks)])
        ternary_mobs.scale(2)
        ternary_mobs.next_to(self.peg_labels, DOWN)
        ...
        self.disk_index_iter = it.cycle(get_ternary_ruler_sequence(self.num_disks - 1))
    ...
    def move_next_disk(self, run_time=None, stay_on_peg=False):
        self.move_disk(next(self.disk_index_iter), run_time=run_time, stay_on_peg=stay_on_peg)


class DescribeSolutionByCountingToConstrained(SolveConstrainedWithTernaryCounting):
    def construct(self):
        braces = [Brace(VGroup(*self.curr_ternary_mob[: n + 1])) for n in range(self.num_disks)]
        words = [
            brace.get_text(text)
            for brace, text in zip(
                braces,
        ...
        self.play(
            ...
            self.curr_ternary_mob[0].set_color,
            color,
        )
        self.wait()
        self.play(
            self.disks[0].set_fill,
            color,
            self.disks[0].label.set_fill,
            BLACK,
        )
        ...
        self.play(
            ...
            self.curr_ternary_mob[1].set_color,
            color,
        )
        self.wait()
        self.play(
            self.disks[1].set_fill,
            color,
            self.disks[1].label.set_fill,
            BLACK,
        )
        self.move_next_disk(stay_on_peg=True)
        self.wait()
        ...
        self.play(
            ...
            self.curr_ternary_mob[2].set_fill,
            color,
            self.disks[2].set_fill,
            color,
            self.disks[2].label.set_fill,
            BLACK,
        )
        self.wait()
        self.move_next_disk(stay_on_peg=True)
...
class RepeatingConfiguraiton(Scene):
    def construct(self):
        ...
        special_arrows = VGroup(*[Arrow(title.get_bottom(), dot, color=RED) for dot in special_dots])
        ...
        self.play(Write(title))
        self.play(ShowCreation(special_arrows), special_dots.set_color, RED)
        self.wait()
        self.play(mid_mobs.shift, 2 * DOWN, FadeOut(special_arrows))
        self.play(ShowCreation(mid_arrow), Write(more_efficient))
        self.wait()
...
class SierpinskiGraphScene(Scene):
    ...
    def distance_between_nodes(self, i, j):
        return get_norm(self.nodes[i].get_center() - self.nodes[j].get_center())
    ...
    def zoom_into_nodes(self, node_indices):
        nodes = VGroup(*[self.nodes[index].circle for index in node_indices])
        ...
        self.play(everything.shift, -nodes.get_center(), everything.scale, scale_factor)
...
class IntroduceGraphStructure(SierpinskiGraphScene):
    ...
    def introduce_nodes(self):
        ...
        for index in 3, 21, 8, 17, 10, 13:
            ...
            node.save_state()
            self.play(node.set_height, FRAME_HEIGHT - 2, node.next_to, ORIGIN, vect)
            self.wait()
    ...
    def define_edges(self):
        ...
        self.wait()
        self.play(*[FadeOut(node.arc) for node in nodes])
        self.play(MoveToTarget(edge), *[node.restore for node in nodes])
        self.wait()
    ...
    def tour_structure(self):
        ...
        self.play(
            self.disks[0].set_color,
            YELLOW,
            *[ApplyMethod(disk.label.set_color, BLACK) for disk in self.disks[0]],
        )
        self.wait()
...
class DescribeTriforcePattern(SierpinskiGraphScene):
    ...
    def construct(self):
        ...
        self.play(
            self.disks[self.scale - 1].set_color,
            self.disk_color,
            *[ApplyMethod(disk.label.set_color, BLACK) for disk in self.disks[self.scale - 1]],
        )
        ...
        bold_edges = [
            self.node_indices_to_edge(*pair).copy().set_stroke(self.disk_color, 6) for pair in self.index_pairs
        ]
        ...
        islands = [VGroup(*[self.nodes[i] for i in index_set]) for index_set in index_sets]

        def wiggle_island(island):
            return ApplyMethod(island.rotate, np.pi / 12, run_time=1, rate_func=wiggle)
...
class BiggerTowers(SierpinskiGraphScene):
    CONFIG = {"num_disks": 6, "include_towers": False}
...
class ShowPathThroughGraph(SierpinskiGraphScene):
    CONFIG = {"include_towers": True}

    def construct(self):
        arrows = VGroup(
            *[
                Arrow(n1.get_center(), n2.get_center(), tip_length=0.15, buff=0.15)
                for n1, n2 in zip(self.nodes, self.nodes[1:])
            ]
        )
        self.wait()
        self.play(ShowCreation(arrows, rate_func=linear, run_time=5))
        self.wait(2)
...
class PatreonThanks(Scene):
    ...
    def construct(self):
        ...
        left_patrons = VGroup(*list(map(TexText, self.specific_patrons[: n_patrons / 2])))
        right_patrons = VGroup(*list(map(TexText, self.specific_patrons[n_patrons / 2 :])))
...
class MortyLookingAtRectangle(Scene):
    def construct(self):
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(url.restore, morty.change_mode, "happy")
...
class ShowSierpinskiCurvesOfIncreasingOrder(Scene):
    CONFIG = {
        "sierpinski_graph_scene_config": {"include_towers": False},
        "min_order": 2,
        "max_order": 7,
        "path_stroke_width": 7,
    }
    ...
    def construct(self):
        graph_scenes = [
            SierpinskiGraphScene(num_disks=order, **self.sierpinski_graph_scene_config)
        ...
        for new_graph, new_path in zip(graphs[1:], paths[1:]):
            self.play(Transform(graph, new_graph), Transform(path, new_path), run_time=2)
            self.wait()
        self.path = path
    ...
    def get_path(self, graph_scene):
        ...
        path.set_stroke(width=self.path_stroke_width - graph_scene.num_disks / 2)
        return path


class Part1Thumbnail(Scene):
    CONFIG = {"part_number": 1, "sierpinski_order": 5}
