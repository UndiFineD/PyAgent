# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\misc.py
    ...
    def construct(self):
        ...
        def func(xxx_todo_changeme):
            ...
            if (np.abs(x % 1 - 0.5) < 0.01 and y < 0.01) or np.abs(z) < 0.01:
                return ORIGIN
            out_z = 1.0 / (2 * np.tan(np.pi * z) * (z**2))
            return out_z.real * RIGHT - out_z.imag * UP

        arrows = Mobject(*[Arrow(ORIGIN, np.sqrt(2) * point) for point in compass_directions(4, RIGHT + UP)])
        arrows.set_color(YELLOW)
        arrows.ingest_submobjects()
        all_arrows = Mobject(*[arrows.copy().scale(0.3 / (x)).shift(x * RIGHT) for x in range(1, n_terms + 2)])
        terms = OldTex(["\\dfrac{1}{%d^2} + " % (x + 1) for x in range(n_terms)] + ["\\cdots"])
        terms.shift(2 * UP)
        ...
        self.play(*[Transform(*pair) for pair in zip(terms.split(), all_arrows.split())])
        self.play(PhaseFlow(func, plane, run_time=5, virtual_time=8))
...
class TimeLine(Scene):
    def construct(self):
        ...
        centers = [Point(timeline.number_to_point(year)) for year in speical_dates]
        ...
        for point, event in zip(centers[1:], dated_events):
            self.play(ApplyMethod(timeline.shift, -point.get_center(), run_time=next(run_times)))
            ...
            self.play(ShimmerIn(event_mob), ShowCreation(line), ShimmerIn(date_mob))
...
class StayedUpAllNight(Scene):
    def construct(self):
        ...
        ticks = Mobject(*[Line(1.8 * vect, 2 * vect, color=GREY) for vect in compass_directions(12)])
        clock.add(ticks)
        ...
        solution = ImageMobject("Newton_brachistochrone_solution2", use_cache=False)
        ...
        newton_complaint = OldTexText(
            ["``I do not love to be", " \\emph{dunned} ", "and teased by foreigners''"],
            size="\\small",
        )
...
class ThetaTGraph(Scene):
    def construct(self):
        ...
        t_mob.to_edge(RIGHT)
        graph = ParametricCurve(lambda t: 4 * t * RIGHT + 2 * smooth(t) * UP)
...
class VideoLayout(Scene):
    def construct(self):
        ...
        for a in np.arange(0, 4.0 / 3, 1.0 / 3):
            vect = interpolate(left, right, a)
            line.add_line(vect + 0.2 * DOWN, vect + 0.2 * UP)
        left_brace = Brace(Mobject(Point(left), Point(interpolate(left, right, 2.0 / 3))), DOWN)
        right_brace = Brace(Mobject(Point(interpolate(left, right, 2.0 / 3)), Point(right)), UP)
        left_brace.words = list(
            map(
                TexText,
                ["Problem statement", "History", "Johann Bernoulli's cleverness"],
            )
        )
        curr = left_brace
        right_brace.words = list(
            map(
                TexText,
                [
                    "Challenge",
...
class ShortestPathProblem(Scene):
    def construct(self):
        ...
        self.play(ShimmerIn(words), *list(map(GrowFromCenter, dots)))
...
class MathBetterThanTalking(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(ApplyMethod(mathy.blink, rate_func=squish_rate_func(there_and_back, 0.4, 0.6)))
...
class TalkedAboutSnellsLaw(Scene):
    def construct(self):
        ...
        for talker, phrase in zip(talkers, phrases):
            ...
            self.play(FadeIn(talker.bubble), ShimmerIn(talker.bubble.content))
            self.play(ApplyMethod(talker.blink, rate_func=squish_rate_func(there_and_back)))
            self.wait()
