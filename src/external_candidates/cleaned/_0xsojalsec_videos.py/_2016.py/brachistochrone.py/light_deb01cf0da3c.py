# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\light.py
...
class PhotonScene(Scene):
    def wavify(self, mobject):
        ...
        lengths = np.apply_along_axis(get_norm, 1, tangent_vectors)
...
class SimplePhoton(PhotonScene):
    def construct(self):
        ...
        self.play(self.photon_run_along_path(Cycloid(), rate_func=linear))
        self.wait()
...
class MultipathPhotonScene(PhotonScene):
    ...
    def run_along_paths(self, **kwargs):
        ...
        photon_runs = [self.photon_run_along_path(path) for path in paths]
        for photon_run, path in zip(photon_runs, paths):
            self.play(
                photon_run,
                ShowCreation(path, rate_func=lambda t: 0.9 * smooth(t)),
                **kwargs,
            )
        self.wait()
...
class PhotonThroughLens(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        interval_values /= self.num_paths - 1.0
        first_contact = [self.lens.point_from_proportion(0.4 * v + 0.55) for v in reversed(interval_values)]
        second_contact = [self.lens.point_from_proportion(0.3 * v + 0.1) for v in interval_values]
        focal_point = 2 * RIGHT
        return [
            Mobject(
...
class TransitionToOptics(PhotonThroughLens):
    ...
    def play(self, *args, **kwargs):
        if not self.has_started:
            ...
            self.play(ApplyMethod(everything.shift, -vect, rate_func=rush_from))
...
class PhotonOffMirror(MultipathPhotonScene):
    ...
    def get_paths(self):
        ...
        anchor_points = [self.mirror.point_from_proportion(0.6 * v + 0.3) for v in interval_values]
        ...
        return [
            Mobject(Line(start_point, anchor_point), Line(anchor_point, end_point)).ingest_submobjects()
...
class ShowMultiplePathsScene(PhotonScene):
    def construct(self):
        ...
        for point, letter in [(point_a, A), (point_b, B)]:
            self.play(ShowCreation(point), ShimmerIn(letter))
        self.play(ShimmerIn(text))
...
class ShowMultiplePathsThroughLens(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        ...
        lower_right, upper_right, upper_left, lower_left = list(map(self.lens.point_from_proportion, alphas))
        return [
            Mobject(Line(self.start_point, a), Line(a, b), Line(b, self.end_point)).set_color(color)
...
class ShowMultiplePathsOffMirror(ShowMultiplePathsScene):
    ...
    def get_paths(self):
        return [
            Mobject(Line(self.start_point, midpoint), Line(midpoint, self.end_point)).set_color(color)
            for midpoint, color in zip([2 * UP, 2 * DOWN], Color(YELLOW).range_to(WHITE, 2))
        ]
...
class ShowMultiplePathsInWater(ShowMultiplePathsScene):
    def construct(self):
        ...
        ShowMultiplePathsScene.construct(self)
        self.play(Transform(self.path, straight))
        self.wait()
    ...
    def get_paths(self):
        self.leftmost, self.rightmost = result = [
            Mobject(Line(self.start_point, midpoint), Line(midpoint, self.end_point)).set_color(color)
            for midpoint, color in zip([3 * LEFT, 3 * RIGHT], Color(YELLOW).range_to(WHITE, 2))
        ]
        return result
...
class StraightLinesFastestInConstantMedium(PhotonScene):
    def construct(self):
        ...
        self.play(self.photon_run_along_path(squaggle, run_time=2, rate_func=linear))
...
class PhtonBendsInWater(PhotonScene, ZoomedScene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowPassingFlash(wave, run_time=3, rate_func=linear))
        self.wait()
...
class LightIsFasterInAirThanWater(ShowMultiplePathsInWater):
    def construct(self):
        ...
        self.wait()
        photon_runs = []
        photon_runs.append(self.photon_run_along_path(path1, rate_func=lambda t: min(1, 1.2 * t)))
...
class GeometryOfGlassSituation(ShowMultiplePathsInWater):
    def construct(self):
        ...
        axes = Mobject(NumberLine(), NumberLine().rotate(np.pi / 2).shift(7 * LEFT))
        graph = FunctionGraph(lambda x: 0.4 * (x + 1) * (x - 3) + 4, x_min=-2, x_max=4)
        graph.set_color(YELLOW)
        Mobject(axes, graph).scale(0.2).to_corner(UP + RIGHT, buff=1)
        axes.add(OldTex("x", size="\\small").next_to(axes, RIGHT))
        axes.add(OldTexText("Travel time", size="\\small").next_to(axes, UP))
        ...
        pairs_for_end_transform = [(mob, mob.copy()) for mob in (top_line, bottom_line, left_brace, x_mob)]

        self.add(glass, point_a, point_b, A, B)
        line = Mobject(top_line, bottom_line).ingest_submobjects()
        self.play(ShowCreation(line))
        self.wait()
        self.play(GrowFromCenter(left_brace), GrowFromCenter(x_mob))
        self.play(GrowFromCenter(right_brace), GrowFromCenter(w_minus_x))
        ...
        self.wait(3)
        self.clear()
        self.add(glass, point_a, point_b, A, B, top_line, bottom_line, left_brace, x_mob)
        ...
        self.add(line)
        self.play(*[Transform(*pair, rate_func=lambda x: 0.3 * smooth(x)) for pair in pairs_for_end_transform])
        self.wait()
...
class Spring(Line):
    CONFIG = {"num_loops": 5, "loop_radius": 0.3, "color": GREY}
    ...
    def init_points(self):
        ...
        def loop(t):
            return micro_radius * (RIGHT + np.cos(m * t) * LEFT + np.sin(m * t) * UP)

        new_epsilon = self.epsilon / (m * micro_radius) / length

        self.add_points([t * RIGHT + loop(t) for t in np.arange(0, 1, new_epsilon)])
...
class SpringSetup(ShowMultiplePathsInWater):
    def construct(self):
        ...
        self.slide_kwargs = {"rate_func": there_and_back, "run_time": 5}

        self.setup_background()
        rod = Region(lambda x, y: (abs(x) < 5) & (abs(y) < 0.05), color=GOLD_E)
        ring = Arc(angle=11 * np.pi / 6, start_angle=-11 * np.pi / 12, radius=0.2, color=YELLOW)
    ...
    def slide_ring(self, ring):
        self.play(ApplyMethod(ring.shift, self.ring_shift_val, **self.slide_kwargs))
    ...
    def slide_system(self, ring):
        ...
        equilibrium_slide_kwargs = {"rate_func": jiggle_to_equilibrium, "run_time": 3}
        start = Mobject(ring, self.start_springs)
        end = Mobject(ring.copy().shift(self.ring_shift_val), self.end_springs)
    ...
    def show_equation(self):
        ...
        v_air, v_water = [OldTex("v_{\\text{%s}}" % s, size="\\Large") for s in ("air", "water")]
        v_air.next_to(Point(frac1.get_center()), DOWN)
        v_water.next_to(Point(frac2.get_center()), DOWN)
        frac1.add(v_air)
        frac2.add(v_water)
        f1, f2 = [OldTex("F_%d" % d, size="\\Large") for d in (1, 2)]
...
class WhichPathWouldLightTake(PhotonScene, TryManyPaths):
    def construct(self):
        words = OldTexText(["Which path ", "would \\emph{light} take", "?"])
        words.split()[1].set_color(YELLOW)
        words.to_corner(UP + RIGHT)
        self.add_cycloid_end_points()

        anims = [self.photon_run_along_path(path, rate_func=smooth) for path in self.get_paths()]
