...
class TryManyPaths(PathSlidingScene):
    ...
    def get_paths(self):
        ...
        paths = [
            ...
            FunctionGraph(lambda x: x**2, x_min=-3, x_max=2, density=3 * DEFAULT_POINT_DENSITY_1D),
        ]
        cycloid = Cycloid()
...
class NotTheCircle(PathSlidingScene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(path.put_start_and_end_on, start, end, path_func=path_along_arc(-angle)),
            run_time=3,
        )
...
class WhatGovernsSpeed(PathSlidingScene):
    ...
    def construct(self):
        ...
        n = len(points)
        broken_points = [
            points[k * n / self.num_pieces : (k + 1) * n / self.num_pieces] for k in range(self.num_pieces)
        ]
        words = OldTexText("""
