# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\matrix_multiplication.py
import numpy as np
...
class NumericalMatrixMultiplication(Scene):
    ...
    def animate_product(self, left, right, result):
        ...
        circles = VGroup(*[entry.get_point_mobject() for entry in (l_matrix[0][0], r_matrix[0][0])])
        ...
        for a in range(m):
            for b in range(n):
                ...
                for c in range(k):
                    ...
                    new_circles = VGroup(*[circle.copy().shift(part.get_center()) for part in start_parts.split()])
