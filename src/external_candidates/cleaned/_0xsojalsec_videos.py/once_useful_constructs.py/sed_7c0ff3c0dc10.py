# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\sed.py
import csv
import time
...
class SEDTest(MovingCameraScene):
    def construct(self):
        ...
        hr_averages = np.array(
            [np.mean(heart_rates[i : i + average_over]) for i in range(len(heart_rates) - average_over)]
        )
        prop = 10
        ...
        self.play(LaggedStartMap(FadeInFromLarge, lines, run_time=5, lag_ratio=0.1))
