# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\butterfly_curve.py
...
class RotatingButterflyCurve(Animation):
    ...
    def __init__(self, **kwargs):
        ...
        self.points = np.array(
            [[x, y, x * x, y * y] for a in fine_range for b in corse_range for x, y in [(a, b), (b, a)]]
        )
        ...
        for i in range(4):
            vect = np.zeros(4)
            vect[i] = 1
            line_range = np.arange(-FRAME_Y_RADIUS, FRAME_Y_RADIUS, self.space_epsilon)
            self.points = np.append(self.points, [x * vect for x in line_range], axis=0)
            axis_rgb = Color(next(colors)).get_rgb()
            self.rgbas = np.append(self.rgbas, [axis_rgb] * len(line_range), axis=0)
