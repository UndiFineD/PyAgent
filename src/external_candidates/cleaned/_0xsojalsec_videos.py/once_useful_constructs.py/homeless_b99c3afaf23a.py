# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\homeless.py
...
class Cycloidify(Scene):
    def construct(self):
        ...
        self.add(lines)
        self.wait()
        self.play(Transform(lines, cycloids, run_time=3, path_func=path_along_arc(np.pi / 2)))
        self.wait()
        self.play(Transform(lines, polar, run_time=3, path_func=path_along_arc(-np.pi / 2)))
        self.wait()
...
class PythagoreanTransformation(Scene):
    def construct(self):
        ...
        a_region, b_region, c_region = regions = [
            MobjectFromRegion(
                region_from_polygon_vertices(*compass_directions(4, growth * start)),
                color,
            )
            .scale(1 / growth)
            .shift(shift_val)
        ...
        for mob, char in zip(regions, "abc"):
            mob.add(OldTex("%s^2" % char).shift(mob.get_center()))
        ...
        self.play(Transform(mover, arrangment2, run_time=3, path_func=path_along_arc(np.pi / 2)))
        self.remove(c_region)
        self.play(*[FadeIn(region) for region in (a_region, b_region)])
        self.wait(2)
        self.clear()
        self.add(mover, square)
        self.play(Transform(mover, arrangment1, run_time=3, path_func=path_along_arc(-np.pi / 2)))
        self.wait()
...
class PullCurveStraight(Scene):
    def construct(self):
        ...
        integral = integrate.quad(lambda x: np.sqrt(1 + 4 * x**2), start, end)
        length = integral[0]
        line = Line(0.5 * length * LEFT, 0.5 * length * RIGHT, color=BLUE_D)
        brace = Brace(line, UP)
        ...
        self.wait()
        self.play(Transform(parabola, line, path_func=path_along_arc(np.pi / 2), run_time=2))
        self.play(GrowFromCenter(brace), ShimmerIn(label))
        self.wait(3)
...
class StraghtenCircle(Scene):
    def construct(self):
        ...
        line = Line(np.pi * radius * LEFT, np.pi * radius * RIGHT, color=circle.get_color())
        ...
        self.play(
            ShowCreation(circle),
            Rotating(radius_line, axis=OUT, rate_func=smooth, in_place=False),
            run_time=2,
        )
        self.wait()
        self.remove(radius_line)
        self.play(Transform(circle, line, run_time=2))
        self.play(ShimmerIn(two_pi_r), GrowFromCenter(line_brace))
        self.wait(3)
...
class SingleVariableFunc(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(Transform(start, point, path_func=path_along_arc(np.pi / 2)))
        self.play(Transform(start, end, path_func=path_along_arc(-np.pi / 2)))
        self.wait()
