# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\vector_space_scene.py
...
class VectorScene(Scene):
    ...
    def get_basis_vector_labels(self, **kwargs):
        ...
        return VGroup(
            *[
                self.get_vector_label(vect, label, color=color, label_scale_factor=1, **kwargs)
    ...
    def coords_to_vector(self, vector, coords_start=2 * RIGHT + 2 * UP, clean_up=True):
        ...
        self.play(
            ApplyFunction(lambda y: self.position_y_coordinate(y, y_line, vector), y_coord),
...
class LinearTransformationScene(VectorScene):
    ...
    def apply_transposed_matrix(self, transposed_matrix, **kwargs):
        ...
        if "path_arc" not in kwargs:
            net_rotation = np.mean([angle_of_vector(func(RIGHT)), angle_of_vector(func(UP)) - np.pi / 2])
    ...
    def apply_function(self, function, added_anims=[], **kwargs):
        if "run_time" not in kwargs:
            kwargs["run_time"] = 3
        anims = (
            [ApplyPointwiseFunction(function, t_mob) for t_mob in self.transformable_mobjects]
            + [
