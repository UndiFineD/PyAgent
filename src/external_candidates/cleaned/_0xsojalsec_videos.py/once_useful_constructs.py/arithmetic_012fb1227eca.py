# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\arithmetic.py
import numpy as np
...
class RearrangeEquation(Scene):
    def construct(
        ...
        unmatched_start_indices.difference_update([n % len(start_mobs) for n in index_map])
        unmatched_end_indices.difference_update([n % len(end_mobs) for n in list(index_map.values())])
        mobject_pairs = [(start_mobs[a], end_mobs[b]) for a, b in index_map.items()] + [
            (Point(end_mobs[b].get_center()), end_mobs[b]) for b in unmatched_end_indices
        ]
        if not leave_start_terms:
            mobject_pairs += [(start_mobs[a], Point(start_mobs[a].get_center())) for a in unmatched_start_indices]
    ...
    def get_mobs_from_terms(self, start_terms, end_terms):
        ...
        all_mobs = np.array(OldTex(start_terms).split() + OldTex(end_terms).split())
        all_terms = np.array(start_terms + end_terms)
        for term in set(all_terms):
            matches = all_terms == term
            if sum(matches) > 1:
                base_mob = all_mobs[list(all_terms).index(term)]
                all_mobs[matches] = [base_mob.copy().replace(target_mob) for target_mob in all_mobs[matches]]
...
class FlipThroughSymbols(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        if not all(self.start_center == self.end_center):
            self.mobject.center().shift((1 - alpha) * self.start_center + alpha * self.end_center)
