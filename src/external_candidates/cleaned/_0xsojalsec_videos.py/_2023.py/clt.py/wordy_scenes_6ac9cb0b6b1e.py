# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\clt\wordy_scenes.py
import sympy
...
class GaltonBoardName(InteractiveScene):
    def construct(self):
        ...
        arrow = Arrow(point, point + FRAME_WIDTH * LEFT * 0.25, stroke_width=10)
...
class NormalName(InteractiveScene):
    def construct(self):
        ...
        self.wait()
        self.play(names[:2].animate.shift(shift), FadeIn(names[2], shift=0.5 * shift))
        self.wait()
        ...
        words.shift(0.5 * UL)
        arrow = Arrow(
            words.get_bottom() + LEFT,
            words.get_bottom() + DOWN,
            stroke_width=3,
            stroke_color=GREY_A,
        )
...
class ErdosKac(InteractiveScene):
    def construct(self):
        ...
        stacks = VGroup(*(VGroup(VectorizedPoint(number_line.n2p(x - 0.5))) for x in range(1, 20)))
        self.add(stacks)
...
class PopulationHeights(InteractiveScene):
    ...
    def get_pi(self, height=1):
        pi = Randolph(mode=random.choice(["happy", "hooray", "pondering", "tease"]), height=1)
...
class SumsOfSizeFive(TeacherStudentsScene):
    def construct(self):
        ...
        # Show sums
        def get_sum():
            values = [Integer(random.choice([-1, 1]), include_sign=True) for x in range(5)]
            lhs = Tex(R"\text{Sum} = ")
...
class GoalsThisLesson(TeacherStudentsScene):
    def construct(self):
        ...
        boxes = VGroup(*(Rectangle(height=1.2 * num.get_height(), width=4).next_to(num) for num in numbers))
        ...
        self.play(FadeIn(numbers, lag_ratio=0.25), morty.change("raise_left_hand"))
        self.wait(2)
        self.play(
...
class ReferenceMeanAndSD(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.play(stds[2].says("Great!", mode="hooray"), morty.change("happy"))
        self.play(
...
class AskWhy(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(stds[1].says("Why?", mode="raise_left_hand"))
        self.play(
            morty.change("tease"),
...
class AskAboutPi(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(stds[2].says("Where's the\ncircle?", mode="raise_left_hand"))
        self.wait(3)
        self.play(
            stds[1].debubble(),
            stds[2].debubble(),
...
class AskAboutFormalStatement(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(stds[2].says(question, mode="sassy", bubble_direction=LEFT))
...
class ExampleQuestion(TeacherStudentsScene):
    def construct(self):
        ...
        dice = VGroup(*(DieFace(value, fill_color=BLUE_E, stroke_width=1, dot_color=WHITE) for value in die_values))
        ...
        self.play(stds[2].says("Can we see a\nconcrete example?"), morty.change("happy"))
        self.wait()
        ...
        self.play(*(pi.animate.look_at(dice) for pi in [morty, *stds]))
...
class ExampleQuestionNoPiCreatures(InteractiveScene):
    def construct(self):
        ...
        num = sum_label.make_number_changeable("100")
        num.add_updater(lambda m: m.set_value(sum(die.value for die in dice)))

        # Add up dice
        self.play(FadeIn(question[0], lag_ratio=0.1, run_time=1.5))
        self.play(VFadeIn(sum_label), ShowIncreasingSubsets(dice, run_time=2))
        self.wait()
    ...
    def show_sample(self, avg_label, n_dice, added_anims=[]):
        ...
        self.play(
            LaggedStart(
                ...
                UpdateFromFunc(
                    avg_label[-1],
                    lambda m: m.set_value(np.mean([die.value for die in dice])),
                ),
                *added_anims,
            )
        )
...
class OneMoreSideNote(TeacherStudentsScene):
    def construct(self):
        # Test
        self.play(self.change_students("pondering", "pondering", "erm", look_at=self.screen))
        self.wait()
        self.play(
            self.teacher.says(
                "Will you tolerate\none more\nside note?",
        ...
        self.play(self.change_students("tired", "pondering", "hesitant"))
        self.wait(4)
...
class ThreeAssumptions(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(Write(point[:2]) for point in points), lag_ratio=0.7),
        ...
        self.wait(2)
        self.play(FadeOut(box), FadeOut(iid_words), FadeIn(title))
        self.wait()
        ...
        self.play(FadeOut(title), FadeIn(gen_words), FadeIn(box))
        self.wait(2)
        self.play(
            FadeOut(box),
            FadeOut(gen_words),
