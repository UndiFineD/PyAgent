...
class DiceSumDistributions(InteractiveScene):
    ...
    def construct(self):
        # Setup all plots
        plots = VGroup(
            *(self.get_sum_distribution_plot(n, y_max=(0.2 if n > 1 else 0.5)) for n in range(1, self.n_examples + 1))
        )
        plots.scale(0.9)
        ...
        for n in range(1, len(plots) - 1):
            ...
            self.play(
                LaggedStart(
                    ...
                    FadeTransform(plots[n].copy().set_opacity(0), plots[n + 1], run_time=2),
        ...
        self.play(
            LaggedStart(*(TransformFromCopy(graph, graph_copy) for graph_copy in graph_copies)),
            run_time=3,
        )
        self.wait()
    ...
    def rescale_distributions(self, all_bars, mean_lines, sd_lines):
        ...
        self.play(
            LaggedStart(
                *(
                    bars.animate.stretch(1 / get_factor(n), 0, about_point=lines.get_center()).stretch(
                        get_factor(n), 1, about_edge=DOWN
                    )
            ...
            LaggedStart(*(arrow.animate.shift(arrow.get_vector()) for arrow_pair in arrows for arrow in arrow_pair)),
            run_time=3,
        )
        self.wait()
    ...
    def get_rescaling_arrows(self, bars, n):
        ...
        arrows = VGroup(
            *(
                FillArrow(ORIGIN, length * vect, **kw).move_to(center - 3.0 * sigma * bar_width * vect, -vect)
...
class TransitionToSkewDistribution(DiceSumDistributions):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeTransform(*labels) for labels in zip(prob_labels, exp_prob_labels)),
                lag_ratio=0.05,
                run_time=2,
            ),
        )
...
class ExpDistSumDistributions(DiceSumDistributions):
    ...
    def construct(self):
        ...
        plots = plots_list[0]
        labels, die_labels = [
            VGroup(*(self.get_plot_label(plot, n, die_face) for plot, n in zip(plots, it.count(1))))
        ...
        arrows = VGroup(
            *(
                FillArrow(axes.c2p(0, 0), axes.c2p(n * mu, 0)).move_to(axes.y_axis.pfp(0.75), LEFT).scale(0.7)
...
class MeanAndStandardDeviation(InteractiveScene):
    def construct(self):
        ...
        mean_arrow.add_updater(
            lambda m: m.put_start_and_end_on(mean_name.get_bottom(), mu_label.get_corner(UL)).scale(0.7)
        )
        ...
        for label, square in zip(labels, squares):
            label.square = square
            label.add_updater(
                lambda m: m.set_width(0.7 * m.square.get_width()).next_to(m.square.get_bottom(), UP, SMALL_BUFF)
            )
        ...
        self.wait()
        self.play(*(TransformFromCopy(sd_equation[R"\sigma"][0], label) for label in sigma_labels))
        self.wait()
...
class BuildUpGaussian(InteractiveScene):
    def construct(self):
        ...
        pis, twos, threes = [VGroup(*(Tex(tex, **kw).move_to(e, DR) for e in es)) for tex in [R"\pi", "2", "3"]]
    ...
    def get_axes(
        ...
        axes = NumberPlane(
            ...
            width=FRAME_WIDTH,
            height=6,
            background_line_style=dict(stroke_color=GREY_A, stroke_width=2, stroke_opacity=0.5),
        )
    ...
    def change_distribution(
        ...
        for dist in alt_dists:
            ...
            self.play(
                ...
                UpdateFromFunc(
                    ss_plot.bars,
                    lambda m: m.set_submobjects(self.get_scaled_sum_plot(get_dist(top_plot.bars), n).bars),
                ),
...
class HowVarianceAdds(LimitingDistributions):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                TransformMatchingShapes(sum_label.copy(), var_form[R"\text{Var}(X + Y) = "]),
                TransformMatchingShapes(top_labels[0].copy(), var_form[R"\text{Var}(X)"]),
                TransformMatchingShapes(top_labels[1].copy(), var_form[R"+ \text{Var}(Y)"]),
        ...
        new_sigma_form = Tex(R"\sigma_{X_1 + \cdots + X_n} = \sqrt{n} \cdot \sigma_{X_1}", t2c=t2c)
