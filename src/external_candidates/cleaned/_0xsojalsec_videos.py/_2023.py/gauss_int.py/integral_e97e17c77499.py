# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\gauss_int\integral.py
...
class BellCurveArea(InteractiveScene):
    def construct(self):
        ...
        axes = NumberPlane(
            ...
            width=14,
            height=5,
            background_line_style=dict(stroke_color=GREY_C, stroke_width=2, stroke_opacity=0.5),
        )
        ...
        self.play(
            LaggedStart(
                *(r.animate.shift(0.25 * UP).set_color(YELLOW).set_anim_args(rate_func=there_and_back) for r in rects),
                run_time=5,
                lag_ratio=0.1,
            ),
            LaggedStart(
...
class AntiDerivative(InteractiveScene):
    def construct(self):
        ...
        def update_area(area: VMobject):
            ...
            area.pointwise_become_partial(high_graph, 0, inverse_interpolate(x_min, x_max, x))
        ...
        low_line = always_redraw(
            lambda: DashedLine(
                planes[1].c2p(get_x(), 0),
                planes[1].i2gp(get_x(), low_graph),
            ).set_stroke(WHITE, 2)
        )
...
class GaussianIntegral(ThreeDScene, InteractiveScene):
    ...
    def get_axes(
        ...
        axes = ThreeDAxes(x_range, y_range, z_range, width=width, height=height, depth=depth)
    ...
    def get_dynamic_cylinder(self, axes, r_init=1):
        ...
        cylinder.add_updater(lambda m: self.set_cylinder_r(m, axes, r_tracker.get_value()))
        return cylinder, r_tracker

    def get_cylinder(self, axes, r, color=BLUE_E, opacity=1):
    ...
    def set_cylinder_r(self, cylinder, axes, r):
        ...
        cylinder.set_depth(self.func(r, 0) * axes.z_axis.get_unit_size(), stretch=True)
    ...
    def get_dynamic_slice(
        ...
        x_slice.add_updater(lambda m: m.set_depth(self.func(0, get_y()) * z_unit, stretch=True))
...
class CylinderSlices(GaussianIntegral):
    def construct(self):
        ...
        bell_halves = Group(
            *(
                axes.get_parametric_surface(
                    lambda r, theta: np.array([r * np.cos(theta), r * np.sin(theta), np.exp(-(r**2))]),
                    u_range=(0, 3),
                    v_range=v_range,
                )
        ...
        self.play(ShowCreation(bell2d), Write(label2d))
        self.wait()
        ...
        for label, line in zip(labels, lines):
            ...
            label.next_to(
                line.get_center(),
                rotate_vector(line.get_vector(), -90 * DEGREES),
                SMALL_BUFF,
            )
        ...
        arrow = Arrow(
            r_label_rect,
            axes.c2p(-3, 3, 0) + 3.2 * LEFT + 0.25 * UP,
            path_arc=45 * DEGREES,
        )
        ...
        self.play(
            *(VShowPassingFlash(line.insert_n_curves(20).set_stroke(width=8), time_width=1.5) for line in line_copies),
        ...
        self.play(*map(FadeOut, [x_line, y_line, x_label, y_label, pythag]))
        ...
        delta_r = 0.1
        cylinders = Group(*(self.get_cylinder(axes, r, opacity=0.5) for r in np.arange(0, 3, delta_r)))
        ...
        dr_brace = Brace(Line(axes.get_origin(), axes.c2p(delta_r, 0, 0)), UP)
        ...
        thick_cylinders = Group(
            *(self.get_thick_cylinder(cyl, delta_r * axes.x_axis.get_unit_size()) for cyl in cylinders)
        )
        thick_cylinders.set_opacity(0.8)
        thick_cylinders.set_shading(0.25, 0.25, 0.25)
        small_dr = 0.02
        thin_cylinders = Group(
            *(self.get_thick_cylinder(self.get_cylinder(axes, r), small_dr) for r in np.arange(0, 5, small_dr))
        )
        ...
        self.play(
            Transform(
                thin_cylinder,
                thin_cylinders[int(np.round(r_tracker.get_value() / small_dr))],
            ),
...
class CylinderIntegral(InteractiveScene):
    def construct(self):
        ...
        exprs = VGroup(
            ...
            Tex(
                R"= \pi \left[ -e^{-{\infty}^2} - \left(-e^{-{0}^2} \right) \right]",
                **kw,
            ),
            Tex(R"= \pi", **kw),
        )
        ...
        self.play(
            TransformMatchingShapes(
                VGroup(*exprs[1][R"\pi \int_0^\infty"], *anti_derivative).copy(),
                exprs[2],
            )
        )
        self.wait()
        ...
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(integrand_rect, arrow, arrow_label, anti_derivative, *rects, *values),
            ),
...
class CartesianSlices(GaussianIntegral):
    def construct(self):
        ...
        tex_kw = dict(font_size=42, t2c={"x": BLUE, "y": YELLOW})
        get_y = y_tracker.get_value
        ...
        frame.add_updater(lambda m: m.set_theta(theta0 + -0.2 * math.sin(0.1 * (self.time - t0))))
        self.wait(10)
...
class CartesianSliceOverlay(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(area_rect), GrowArrow(arrows[0]), FadeIn(area_word))
        self.wait()
        self.play(
            Transform(*rects),
            Transform(*arrows),
        ...
        self.wait()
        self.play(FadeOut(box2), Write(rhs))
        self.wait()
