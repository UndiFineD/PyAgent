...
class ExplainNChoose2(InteractiveScene):
    ...
    def construct(self):
        ...
        pair_rects = VGroup(
            *(SurroundingRectangle(pair, buff=SMALL_BUFF).set_stroke(YELLOW, 1) for pair in pair_labels)
        )
        rhs = Tex("= 0")
...
class LeftDiagram(ExplainNChoose2):
    ...
    def construct(self):
        ...
        numbers = VGroup(
            *(Integer(2**n, font_size=60).next_to(diagram, RIGHT) for n, diagram in zip(it.count(1), diagrams))
        )
...
class CountIntersections(ExplainNChoose2):
    ...
    def construct(self):
        ...
        quad_rects = VGroup(
            *(SurroundingRectangle(label, buff=SMALL_BUFF).set_stroke(YELLOW, 1) for label in quad_labels)
        )
    ...
    def show_quadruplets(self, diagram, quad_words):
        ...
        for sub_indices in it.combinations(indices, 4):
            ...
            self.add(temp_lines)
            self.play(
                LaggedStart(*(TransformFromCopy(dots[i], int_dot) for i in sub_indices)),
                lag_ratio=0.1,
            )
            self.add(int_dot)
            self.wait(0.5)
...
class BalancedEquation(InteractiveScene):
    def construct(self):
            ...
            return LaggedStart(
                *(
                    AnimationGroup(
                        UpdateFromAlphaFunc(
                            increments[n],
                            lambda m, a: m.set_y(number_y - 0.75 * a).set_opacity(there_and_back(a)),
                        ),
                        ChangeDecimalToValue(numbers[n], numbers[n].get_value() + 1, run_time=0.25),
                    )
                    for n in ns
                ),
                lag_ratio=0.1,
        ...
        self.play(ShowCreation(edges[0]), FadeIn(new_vert_word[R"New edge $\rightarrow$"][0]))
        self.wait()
        self.play(
...
class ExamplesOfFormula(InteractiveScene):
    ...
    def get_example(self, n):
        result = VGroup(
            ...
            Tex(Rf"= 1 + {choose(n, 2)} + {choose(n, 4)} = {1 + choose(n, 2) + choose(n, 4)}"),
        )
