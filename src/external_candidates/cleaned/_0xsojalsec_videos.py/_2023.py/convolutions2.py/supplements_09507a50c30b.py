# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\convolutions2\supplements.py
...
class QuizTime(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.change_students("happy", "tease", "hooray"))
        self.wait(2)
...
class AlreadyCoveredConvolutions(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            stds[2].says("Wait, didn't you already\ncover convolutions?", mode="dance_1"),
            morty.change("guilty"),
        )
        self.play(self.change_students("hesitant", "awe", None, look_at=self.screen))
        self.wait(2)
        ...
        self.play(GrowArrow(arrow), morty.change("raise_left_hand", self.screen))
        self.wait(2)
...
class AskAboutAddingThreeUniforms(InteractiveScene):
    def construct(self):
        ...
        term_braces = VGroup(*(Brace(xi, DOWN, SMALL_BUFF) for expr in exprs for xi in expr[re.compile("X_.")]))
        expr_braces = VGroup(*(Brace(expr, DOWN) for expr in [*exprs]))
        ...
        for brace, func, color in zip(all_braces, funcs, colors):
            ...
            if brace is expr_braces[1]:
                ...
                graph = axes.get_graph(func, x_range=(*x_range, 0.025), use_smoothing=False)
...
class AskIfItsTheArea(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.change_students("confused", "erm", "hesitant", look_at=morty.eyes))
        self.wait(5)
...
class SimpleQuestion(InteractiveScene):
    def construct(self):
        ...
        self.play(Write(words[0], run_time=1), FadeIn(equations[0]))
        self.wait()
        self.play(
...
class ComplainAboutCalculation(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(self.change_students("horrified", "pleading", "concentrating", look_at=2 * UR))
...
class OrdinaryApproach(InteractiveScene):
    def construct(self):
        ...
        tex_kw = dict(font_size=30, t2c={"x": BLUE, "y": YELLOW})
        equations = VGroup(
            Tex(tex1, **tex_kw),
            Tex(tex2, **tex_kw),
        )
        ...
        self.play(GrowFromCenter(brace), FadeIn(brace_text, lag_ratio=0.1))
        self.wait()
        ...
        calc_text = low_brace.get_text("Not prohibitively difficult,\nbut a bit clunky", font_size=30)
        self.play(GrowFromCenter(low_brace), FadeIn(calc_text, lag_ratio=0.1))
        self.wait()
