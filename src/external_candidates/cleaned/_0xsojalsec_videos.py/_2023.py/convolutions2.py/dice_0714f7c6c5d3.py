# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\convolutions2\dice.py
...
def get_bar_group(
    ...
    labeled_bars = VGroup(
        *(VGroup(bar, value_label, p_label) for bar, value_label, p_label in zip(bars, value_labels, p_labels))
    )
...
def die_sum_labels(color1=BLUE_E, color2=RED_E, height=1.0):
    die1, die2 = dice = [DieFace(1, fill_color=color) for color in [color1, color2]]
    for die in dice:
        die.remove(die[1])
...
class SumAlongDiagonal(InteractiveScene):
    ...
    def construct(self):
        ...
        rects = VGroup(*(SurroundingRectangle(sum_label) for sum_label in sum_labels))
        ...
        marginal_labels = VGroup(*(VGroup(*(bar[2] for bar in group)).copy() for group in bar_groups))
        ...
        for n, square in enumerate(grid):
            ...
            prob = DecimalNumber(margin1.get_value() * margin2.get_value(), num_decimal_places=3)
            prob.set_height(margin1.get_height())
            prob.next_to(dice, DOWN, SMALL_BUFF)

            rects = VGroup(*(SurroundingRectangle(mob, buff=SMALL_BUFF) for mob in [margin1, margin2, prob]))
        ...
        self.play(
            ...
            *(MaintainPositionRelativeTo(bar, full_table) for bar in bars_3d),
            run_time=2,
        )
        self.wait()
        ...
        diagonals = VGroup(
            *(
                VGroup(*(VGroup(dice_pairs[n], grid_probs[n]) for n in range(36) if (n // 6) + (n % 6) == s))
                for s in range(11)
            )
        )
    ...
    def show_repeated_samples(
        ...
        markers = VGroup(*(VGroup(VectorizedPoint(bucket.get_bottom())) for bucket in buckets))
...
class ConvolveDiscreteDistributions(SumAlongDiagonal):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(rect), FadeIn(label))
        self.wait()
        self.play(MoveToTarget(low_bars, path_arc=PI / 3, lag_ratio=0.005))
        ...
        for n in [*range(6), *range(4, -1, -1)]:
            ...
            self.play(FadeIn(rect), FadeOut(last_rect), MoveToTarget(bar_groups), run_time=0.5)
            self.wait(0.5)
            last_rect = rect
        ...
        for n in [7, 5, *range(2, 13)]:
            ...
            pair_rects = VGroup(
                *(SurroundingRectangle(pair, buff=0.05).set_stroke(YELLOW, 2).round_corners() for pair in pairs)
            )
        ...
        x_args = VGroup(
            *(Tex(f"({x}) = {np.round(dist1[x - 1], 2)}").next_to(PX, RIGHT, SMALL_BUFF) for x in range(1, 7))
        )
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(func_group, func_group_Y, func_group_XY, x_args[-1])))
        ...
        self.wait()
        self.play(conv_rect.animate.become(SurroundingRectangle(conv_def["*"], buff=0.05, stroke_width=1)))
        self.wait()
        ...
        summands = VGroup(*(example[Rf"P_X({x}) \cdot P_Y({4 - x})"] for x in range(1, 7)))
        plusses = example["+"]
        ...
        # Highlight zeros
        zeroed_terms = VGroup(*(example[Rf"P_Y({n})"] for n in range(0, -4, -1)))
        ...
        for bar in bars:
            ...
            anims.extend(
                [
                    ...
                    UpdateFromAlphaFunc(
                        num,
                        lambda m, a: m.next_to(m.rect, UP, SMALL_BUFF).set_opacity(a),
                    ),
                ]
            )
    ...
    def construct(self):
        ...
        for list_word, bar_group in zip(list_words, bar_groups):
            self.play(
                ...
                LaggedStart(
                    *(FlashAround(bar[2], time_width=1.5, buff=0.05) for bar in bar_group),
                    lag_ratio=0.03,
                    run_time=2,
                ),
            )
...
class RepeatedDiscreteConvolutions(InteractiveScene):
    ...
    def get_bars(self, dist, colors=(BLUE, TEAL), y_unit=4, bar_width=0.35):
        axes = Axes((0, len(dist)), (0, 1), height=y_unit, width=bar_width * len(dist))
    ...
    def get_bar_value_labels(self, bars, min_value=1):
        values = VGroup(*(Integer(x + min_value, font_size=16) for x in range(len(bars))))
    ...
    def get_bar_prob_labels(self, bars, dist, num_decimal_places=2):
        probs = VGroup(*(DecimalNumber(p, font_size=16, num_decimal_places=num_decimal_places) for p in dist))
    ...
    def flip_bar_group(self, bar_group):
        ...
        self.play(
            ...
            *(MaintainPositionRelativeTo(VGroup(value, prob), bar) for bar, value, prob in zip(*bar_group)),
        )
        self.add(bar_group)
    ...
    def show_convolution(self, top_bars, low_bars):
        ...
        for n in range(len(new_bars[0])):
            ...
            self.play(low_bars.animate.shift(x_diff * RIGHT), run_time=0.5)
            index_pairs = [
                (k, n - k) for k in range(n + 1) if 0 <= n - k < len(low_bars[0]) if 0 <= k < len(top_bars[0])
            ]
            highlights = VGroup(*(VGroup(top_bars[0][i].copy(), low_bars[0][j].copy()) for i, j in index_pairs))
