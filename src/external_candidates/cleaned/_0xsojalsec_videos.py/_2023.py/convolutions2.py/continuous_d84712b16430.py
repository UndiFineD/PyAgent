...
class CompareFormulas(InteractiveScene):
    def construct(self):
        ...
        target_rects = VGroup(
            *(SurroundingRectangle(disc_formula[s][0]) for s in ["P_X", "P_Y", "P_X * P_Y", re.compile(R"\\sum.*")])
        )
...
class RepeatedSamplesFromContinuousDistributions(InteractiveScene):
    ...
    def get_plots(self):
        ...
        graphs = VGroup(
            *(
                ...
                for axes, func, color in zip(all_axes, self.get_pdfs(), self.graph_colors)
            )
        )
    ...
    def animate_samples(
        ...
        self.play(
            LaggedStart(
                *(dot.animate.set_radius(0.1).set_opacity(self.dot_fade_factor) for dot in dots),
                **kw,
            ),
...
class Convolutions(InteractiveScene):
    ...
    def setup(self):
        ...
        graph_dot.add_updater(
            lambda d: d.move_to(conv_graph.quick_point_from_proportion(inverse_interpolate(x_min, x_max, get_s())))
        )
...
class ProbConvolutions(Convolutions):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(*map(FadeOut, [x_line, x_dot, y_line, y_dot, *equation, *braces])),
            lag_ratio=0.2,
        )
