...
class MultipleBellishCurves(InteractiveScene):
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                plot[1],
                lambda m, a: m.pointwise_become_partial(graph_copy, 0.5 - 0.5 * a, 0.5 + 0.5 * a),
                run_time=2,
            )
        )
...
class PreviewExplicitCalculation(InteractiveScene):
    def construct(self):
        ...
        # Gaussian
        gauss_color = RED
        gaussian1 = Tex(R"f(x) = {1 \over \sigma_1 \sqrt{2 \pi}} e^{-x^2 / 2 \sigma_1^2}", **tex_kw)
        gaussian2 = Tex(R"g(y) = {1 \over \sigma_2 \sqrt{2 \pi}} e^{-y^2 / 2 \sigma_2^2}", **tex_kw)
...
class RotationalSymmetryAnnotations(InteractiveScene):
    def construct(self):
        ...
        self.play(
            TransformMatchingTex(rhs2.copy(), rhs3, key_map={"x^2 + y^2": "r^2"}, run_time=1),
...
class SliceLineAnnotations(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeOut, VGroup(words, conv, conv_eq, area, s_eq, decimal_rhs)),
...
class ShowGaussianConvolutionsAsEquations(InteractiveScene):
    def construct(self):
        ...
        # Equations
        kw = dict(font_size=30, t2c={"x": BLUE, "y": YELLOW, "{s}": TEAL, R"\sigma": RED})
        equations = VGroup(
            Tex(R"e^{-x^2}", **kw),
            Tex(R"e^{-y^2}", **kw),
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(eq1.copy(), eq2) for eq1, eq2 in zip(equations[:3], equations[3:])),
                lag_ratio=0.15,
            ),
            LaggedStart(
                *(TransformFromCopy(graph1, graph2) for graph1, graph2 in zip(graphs[:3], graphs[3:])),
                lag_ratio=0.15,
            ),
            LaggedStartMap(
                FadeIn,
...
class HerschelMaxwellWords(InteractiveScene):
    def construct(self):
        ...
        ideas = VGroup(
            ...
            TexText(R"Why $\pi$ is in this formula", font_size=60, t2c={R"\pi": YELLOW}),
        )
        for idea in ideas:
