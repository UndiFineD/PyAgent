...
class HoldUp(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(3)
        self.play(self.change_students("maybe", "pondering", "hesitant", look_at=3 * UR))
        self.wait(3)
...
class DidntSteveCover(TeacherStudentsScene):
    def construct(self):
        ...
        image.set_height(3)
        thumbnail = Group(SurroundingRectangle(image, buff=0).set_stroke(WHITE, 3), image)
...
class Questions(InteractiveScene):
    def construct(self):
        ...
        # Check marks
        marks = VGroup(
            *(Checkmark(font_size=60).next_to(question[0], RIGHT).shift(0.15 * UP) for question in questions)
        )
        for mark in marks:
...
class StrengthInDifferentDirectionsWithDecimal(InteractiveScene):
    def construct(self):
        ...
        angle_label.add_updater(
            lambda m: m.next_to(arc.pfp(0.3), normalize(arc.pfp(0.3)), SMALL_BUFF, aligned_edge=DOWN)
        )
...
class AskAboutQuantum(TeacherStudentsScene):
    def construct(self):
        ...
        checks = VGroup(*(Checkmark().match_x(title).match_y(point) for point in points for title in titles))
...
class LengthsOnDifferentColors(InteractiveScene):
    def construct(self):
        ...
        numbers = VGroup(*(DecimalNumber(line.get_length() / 7, font_size=36, unit=R" \text{m}") for line in lines))
