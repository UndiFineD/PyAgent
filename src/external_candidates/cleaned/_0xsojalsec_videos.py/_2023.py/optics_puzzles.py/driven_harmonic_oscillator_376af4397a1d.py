# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\optics_puzzles\driven_harmonic_oscillator.py
from _2023.optics_puzzles.objects import Calcite, Sucrose, TimeVaryingVectorField
...
class Spring(VMobject):
    def __init__(
        ...
        **kwargs,
        ...
        self.add_updater(lambda m: m.stretch(get_norm(base_point - mobject.get_edge_center(edge)) / width, 0))
        self.add_updater(lambda m: m.put_start_and_end_on(base_point, mobject.get_edge_center(edge)))
...
class DrivenHarmonicOscillator(InteractiveScene):
    def construct(self):
        ...
        springs = VGroup(
            *(Spring(sho, sho.get_center() + (spacing - small_radius) * vect) for vect in compass_directions(4))
        )
        ...
        self.play(GrowArrow(x_vect), springs.animate.set_stroke(opacity=0.35))
        ...
        equations.add(
            ...
            Tex(
                R"\frac{d^2 \vec{\textbf{x}}}{dt^2}(t) = -{k \over m} \vec{\textbf{x}}(t)",
                **tex_kw,
            ),
            Tex(
                R"\vec{\textbf{x}}(t) = \vec{\textbf{x}}_0 \cos( \sqrt{k \over m} \cdot t)",
                **tex_kw,
            ),
        ...
        self.play(
            TransformMatchingTex(
                ...
                matched_pairs=[
                    ...
                    (
                        eq1_copy[R"= -k \vec{\textbf{x}}(t)"],
                        eq2[R"= -k \vec{\textbf{x}}(t)"],
                    ),
                ],
                run_time=1,
            ),
        ...
        self.remove(F_vect)
        self.add(x_vect)
        self.play(FadeIn(ic_label), sho.animate.move_to(initial_position))
        ...
        self.play(
            ...
            Write(implies[1]),
            *(TransformFromCopy(eq3[tex], eq4[tex]) for tex in ["=", R"\vec{\textbf{x}}(t)"]),
        )
        self.play(
        ...
        self.wait()
        self.play(FadeTransform(ic_label, x0_rect))
        self.play(
        ...
        # Reset
        self.play(FadeOut(plot), FadeOut(x0_copy), FadeOut(x_vect), run_time=2)
        self.wait()
        ...
        sho.move_to(initial_position)
        sho.reset_velocity()
        self.play(ReplacementTransform(sqrt_km_rect, k_rect), GrowArrow(k_rect.arrow))
        ...
        z_axis_field = TimeVaryingVectorField(time_func, height=0, width=0, depth=16, z_density=5, **field_config)
        ...
        sho.set_k(8)
        sho.set_damping(1)
        sho.set_external_forces([lambda: 3 * planar_field.func(np.array([ORIGIN]))[0]])
        sho.center()
        sho.reset_velocity()
        sho.resume_updating()
        ...
        self.play(frame.animate.reorient(-100, 100, 90), run_time=6)
        ...
        self.play(frame.animate.reorient(-100, 80, 90), run_time=6)
        ...
        self.play(frame.animate.reorient(-100, 110, 90), run_time=12)
        self.play(frame.animate.reorient(-120, 80, 95), run_time=10)
        ...
        plot_rect, plot_axes, plot = self.get_plot_group(lambda: 2 * sho.get_y(), width=FRAME_WIDTH - 1, max_t=20)
        ...
        self.play(omega0_eq_copy.animate.set_height(0.4).move_to(plot_group1, UR).shift(SMALL_BUFF * DL))
        self.wait()
        self.play(omega_copy.animate.move_to(plot_axes.c2p(14, 0.8)))
        self.wait()
        ...
        freq_diff_rect = SurroundingRectangle(solution[R"\omega_r^2-\omega_l^2"], buff=0.05)
        lil_rects = VGroup(amp_rect, E_rect, q_rect, freq_diff_rect)
        steady_state_rect = plot_box2.copy().set_fill(opacity=0)
        VGroup(full_rect, amp_rect, E_rect, freq_diff_rect, steady_state_rect).set_stroke(YELLOW, 2)
        ...
        self.play(
            LaggedStart(
                ...
                *(TransformFromCopy(solution[tex][0], close_freq_words[tex][0]) for tex in [R"\omega_r", R"\omega_l"]),
            )
        )
        self.wait()
        self.play(
        ...
        self.play(Write(half), omega_r.animate.shift((half.get_width() + 0.05) * RIGHT))
    ...
    def scrap(self):
        ...
        self.add(plot_rect, plot_axes, plot)
...
class JigglesInCalcite(InteractiveScene):
    ...
    def construct(self):
        ...
        springs = VGroup(
            ...
            *(Spring(calcium, calcium.get_center() + spring_length * h_vect, edge=h_vect) for h_vect in [LEFT, RIGHT]),
        )
        ...
        calcium.set_external_forces([lambda: 3 * z_axis_field.func(np.array([calcium_center]))[0]])
...
class SpiralPaths(InteractiveScene):
    ...
    def construct(self):
        ...
        # Spiral
        helix = ParametricCurve(
            lambda t: [math.cos(self.sign * t), math.sin(self.sign * t), 0.25 * t],
