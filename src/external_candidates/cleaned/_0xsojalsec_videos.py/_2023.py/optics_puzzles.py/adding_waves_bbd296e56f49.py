...
class AddTwoSineWaves(InteractiveScene):
    def construct(self):
        ...
        frame = self.frame
        t2c = {R"\omega": GOLD}
        top_words = TexText(R"If both have matching \\ frequencies, $\omega$...", font_size=36, t2c=t2c)
...
class WavePlusLayerInfluence(InteractiveScene):
    ...
    def construct(self):
        ...
        for x in layer_xs:
            ...
            charges.add_updater(
                lambda m: m.move_to(
                    mid_axes.c2p(
                        m.x,
                        m.amplitude_tracker.get_value() * wave1.xt_to_yz(m.x, wave1.time)[0],
                        0,
                    )
                )
            )
        ...
        self.play(
            LaggedStart(
                *(FadeOut(charges, suspend_mobject_updating=False) for charges in layers[1:]),
                run_time=5,
                lag_ratio=0.25,
            ),
