...
class HoldUpAlbumCover(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        true_prism = Group(SurroundingRectangle(prism_image, buff=0).set_stroke(WHITE, 10), prism_image)
        ...
        rainbow = VGroup(*(Arc(PI, -PI, radius=radius, stroke_width=10) for radius in np.linspace(1.5, 2.0, 6)))
        rainbow.reverse_submobjects()
        rainbow.move_to(album, DOWN).shift(0.05 * UP)
        for mob in roygbiv, rainbow:
            mob.set_submobject_colors_by_gradient(RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE)
...
class DefineIndexOfRefraction(InteractiveScene):
    def construct(self):
        ...
        equation = Tex(Rf"{{{num_words} \over {den_words}}} \approx {ior}", font_size=36)
...
class PreviewQuarterBehindReason(InteractiveScene):
    def construct(self):
        ...
        big_vect.add_updater(
            lambda m: m.put_start_and_end_on(plane.n2p(0), vects[-1].get_end() if len(vects) > 0 else plane.n2p(amp))
        )
...
class FrequencyVsAngularFrequency(TeacherStudentsScene):
    def construct(self):
        ...
        morty.change_mode("raise_right_hand")
        self.play(ShowCreation(cross), self.change_students("sassy", "hesitant", "angry"))
        rf_words.add(cross)
        ...
        vect.add_updater(
            lambda m: m.put_start_and_end_on(plane.n2p(0), plane.n2p(np.exp(complex(0, angle_tracker.get_value()))))
        )
        ...
        line.add_updater(lambda m: m.put_start_and_end_on(vect.get_end(), dot.get_center()))
...
class GuessSteadyState(InteractiveScene):
    def construct(self):
        ...
        solution = Tex(Rf"\vec{{\textbf{{x}}}}(t) = {amp_tex} \cos(\omega_l t)", t2c=EQUATION_T2C)
...
class QuestionsFromPatrons(InteractiveScene):
    def construct(self):
        ...
        self.play(FadeIn(back_rect), FadeIn(grey_rect), DrawBorderThenFill(highlight_rect))
        self.wait()
