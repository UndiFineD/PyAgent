...
class SlicedWave(Group):
    ...
    def __init__(
        ...
        super().__init__(self.wave, self.vect_wave, self.layers, *self.phase_kick_trackers)
...
class RevertToOneLayerAtATime(PhaseKickBacks):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(layer.animate().set_stroke(opacity=0).shift(DOWN) for layer in left_layers[:0:-1]),
                **kw,
            ),
        ...
        while nls > 1:
            ...
            new_pk_decimal = DecimalNumber(-phase_kick, num_decimal_places=(2 if -phase_kick > 0.05 else 3))
            ...
            self.play(
                ...
                LaggedStart(*(pkt.animate.set_value(phase_kick) for pkt in pkts[::nls]), **kw),
                LaggedStart(
...
class DissolveLayers(PhaseKickBacks):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(layer.animate().set_stroke(WHITE, 2, 0).set_height(5) for layer in layers[:0:-1]),
                **kw,
            ),
...
class PhaseKickBackAddInLayers(PhaseKickBacks):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(*(FadeIn(line, 0.5 * DOWN) for line in sliced_wave.layers), **lag_kw),
            LaggedStart(
                *(pkt.animate.set_value(self.target_kick_back) for pkt in sliced_wave.phase_kick_trackers),
                **lag_kw,
            ),
        )
        self.wait(8)
