...
class IntroduceEField(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOut,
                Group(axes, graph, graph_dot, d_line, d_label, force_words, coulomb_vects),
            ),
        ...
        for point in [2 * RIGHT, ORIGIN]:
            self.play(
                charge.animate.move_to(point).set_anim_args(path_arc=PI, run_time=5, suspend_mobject_updating=False)
            )
        self.wait(5)
...
class RowOfCharges(ChargeOnZAxis):
    ...
    def get_particles(self):
        return Group(*(ChargedParticle(**self.particle_config) for n in range(self.n_charges))).arrange(
            UP, buff=self.particle_buff
        )
...
class AltRowOfCharges(RowOfCharges):
    def construct(self):
        # Test
        self.play(
            self.frame.animate.reorient(-4, 82, 0).move_to([3.12, -0.06, 1.0]).set_height(5.23),
            run_time=12,
        )
        self.play(self.frame.animate.reorient(-20, 69, 0).set_height(8.00), run_time=12)
        self.play(
            self.frame.animate.reorient(-13, 78, 0).move_to([4.32, -0.91, 0.42]).set_height(5.27),
            run_time=12,
        )
...
class OneOfManyCharges(InteractiveScene):
    ...
    def setup(self):
        ...
        plane_wave.add_updater(
            lambda m: m.set_stroke(opacity=m.opacity_multiplier.get_value() * m.get_stroke_opacities())
        )
...
class FullCompositeEffect(OneOfManyCharges):
    ...
    def construct(self):
        ...
        new_wave.add_updater(
            lambda m: m.set_stroke(opacity=m.get_stroke_opacities() * m.opacity_multiplier.get_value())
        )
