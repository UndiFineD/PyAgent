# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\dandelin.py
from _2018.lost_lecture import Orbiting, ShowWord
...
class ThinkingAboutAProof(PiCreatureScene):
    def construct(self):
        ...
        self.play(FadeInFromDown(love_math), GrowArrow(lm_arrow), randy.change, "erm")
        self.wait(2)
...
class SumOfIntegersProof(Scene):
    ...
    def construct(self):
        equation = OldTex("1", "+", "2", "+", "3", "+", "\\cdots", "+", "n", "=", "\\frac{n(n+1)}{2}")
        equation.scale(1.5)
        equation.to_edge(UP)
        ...
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, numbers, path_arc=-90 * DEGREES, lag_ratio=1, run_time=1))
        self.play(LaggedStartMap(Restore, rows))
        self.wait()
        self.play(ReplacementTransform(rows.copy().set_fill(opacity=0), flipped_rows, path_arc=-PI, run_time=2))
        self.wait()
...
class HoldUpProof(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            "raise_right_hand",
            self.screen,
            self.change_students("pondering", "confused", "maybe", look_at=title),
        )
        self.look_at(title)
        self.wait(5)
        self.play_student_changes("happy", "thinking", "hooray", look_at=title)
        self.wait(5)
...
class MultipleDefinitionsOfAnEllipse(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFrom, definitions, lambda m: (m, RIGHT), run_time=4))
        self.wait()
        for definition in definitions:
...
class ShowArrayOfEccentricities(Scene):
    def construct(self):
        ...
        arrows = VGroup(*[Arrow(name.get_bottom(), label.get_top()) for label in eccentricity_labels])
        ...
        for e1, e2 in zip(ellipses[:-1], ellipses[1:]):
            self.play(ReplacementTransform(e1.copy(), e2, path_arc=10 * DEGREES))
        self.wait()
        ...
        circle = ellipses[0]
        group = VGroup(*it.chain(eccentricity_labels, ellipses[1:], arrows, name, alt_name, [morty]))
        self.play(
    ...
    def get_ellipse_row(self, eccentricities, buff=MED_SMALL_BUFF, **kwargs):
        result = VGroup(*[self.get_ellipse(e, **kwargs) for e in eccentricities])
...
class ShowOrbits(ShowArrayOfEccentricities):
    ...
    def construct(self):
        ...
        eccentricity_label = DecimalNumber(earth_eccentricity, num_decimal_places=4)
        eccentricity_equals = OldTexText("Eccentricity = ")
        earth_orbit_words = OldTexText("Earth's orbit")
        earth_orbit_words.set_color(BLUE)
        full_label = VGroup(earth_orbit_words, eccentricity_equals, eccentricity_label)
        ...
        self.play(
            ...
            ChangingDecimal(eccentricity_label, lambda a: self.get_eccentricity(comet_orbit)),
...
class EccentricityInThumbtackCase(ShowArrayOfEccentricities):
    def construct(self):
        ...
        focal_lines_update = self.get_focal_lines_update(ellipse, ellipse_point_update.mobject)
        ...
        inner_brace_update = self.get_focus_line_to_focus_line_brace_update(focus_to_focus_line_update.mobject)
        ...
        fraction = OldTex("{\\text{Focus distance}", "\\over", "\\text{Diameter}}")
        ...
        numerator_update = Mobject.add_updater(
            numerator,
            lambda m: m.set_width(focus_distance.get_width()).next_to(fraction[1], UP, MED_SMALL_BUFF),
        )
    ...
    def get_ellipse_point_update(self, ellipse):
        dot = Dot(color=RED)
        return cycle_animation(MoveAlongPath(dot, ellipse, run_time=5, rate_func=linear))
    ...
    def get_focus_line_to_focus_line_brace_update(self, line):
        ...
        return Mobject.add_updater(
            brace,
            lambda b: b.match_width(line, stretch=True).next_to(line, DOWN, buff=SMALL_BUFF),
        )
    ...
    def get_eccentricity_value_update(self, eccentricity_label, ellipse):
        ...
        decimal.add_updater(lambda d: d.set_value(self.get_eccentricity(ellipse)))
        return decimal
...
class AskWhyAreTheyTheSame(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(morty.change, "awe", self.change_students("confused", "confused", "sassy"))
        ...
        self.wait(2)
        self.play(baby_morty.change, "thinking", LaggedStartMap(DrawBorderThenFill, bubble))
...
class TriangleOfEquivalences(Scene):
    def construct(self):
        ...
        arrows = VGroup(*[OldTex("\\Leftrightarrow") for x in range(3)])
        arrows.scale(2)
...
class ShowMeasurementBook(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(self.teacher.change, "raise_right_hand", FadeInFromDown(measurement))
        self.play_all_student_changes("hooray")
        self.wait()
        self.play(
            GrowArrow(arrow),
            FadeIn(words, RIGHT),
            self.change_students("thinking", "happy", "pondering", look_at=arrow),
        )
        self.wait(3)
...
class TangencyAnimation(Scene):
    def construct(self):
        rings = VGroup(*[Circle(color=YELLOW, stroke_width=3, radius=0.5) for x in range(5)])
        for ring in rings:
            ring.save_state()
            ring.scale(0)
...
class ShowSegmentSplit(Scene):
    ...
    def construct(self):
        ...
        words = VGroup(OldTexText("Top segment"), OldTexText("Bottom segment"))
        ...
        self.play(
            LaggedStartMap(Restore, braces, lag_ratio=0.7),
        )
        for word in words:
...
class WriteConjecture(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeInFromDown, VGroup(title, equation), lag_ratio=0.7))
        self.wait()
        for word, line in zip(words, lines):
...
class QuickGeometryProof(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(GrowFromCenter, everything, lag_ratio=0.25, run_time=4))
...
class NameDandelin(Scene):
    ...
    def construct(self):
        title = OldTexText("Proof by\\\\", "Germinal Pierre Dandelin (1822)")
        title.to_edge(UP)
        ...
        self.play(LaggedStartMap(FadeInFromDown, dandelions, lag_ratio=0.7, run_time=1))
        self.wait()
...
class DetailsLeftAsHomework(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.pi_creature_says("Details left \\\\ as homework", target_mode="hooray")
        self.wait(3)
...
class CreativeConstruction(PiCreatureScene):
    def construct(self):
        ...
        bubble.pin_to(kant)
        kant_words = OldTexText("How is synthetic a priori\\\\" + "reasoning possible?")
        ...
        self.add(dandelin)
        self.add(lightbulb)
        self.play(Write(lightbulb, run_time=1), self.get_light_shine(lightbulb))
        self.wait()
        self.play(
            lightbulb.next_to,
            randy,
        ...
        self.play(MoveToTarget(lightbulb, run_time=3, rate_func=there_and_back, lag_ratio=0.5))
    ...
    def get_light_shine(self, lightbulb, max_radius=15.0, delta_r=0.025):
        ...
        return LaggedStartMap(FadeIn, rings, rate_func=there_and_back, run_time=2, lag_ratio=0.5)
...
class ShowApollonianCircles(Scene):
    def construct(self):
        ...
        k4 = sum(curvatures) - 2 * np.sqrt(sum([k1 * k2 for k1, k2 in it.combinations(curvatures, 2)]))
        radii.append(1.0 / k4)
        centers = [
            ORIGIN,
            3 * RIGHT,
        ...
        circles = VGroup(*[Circle(radius=r).shift(c) for r, c in zip(radii, centers)])
...
class ReactionToGlimpseOfGenius(TeacherStudentsScene, CreativeConstruction):
    def construct(self):
        ...
        self.student_says(
            "I think Lockhart was \\\\speaking more generally.",
            target_mode="sassy",
        ...
        self.play(
            ...
            VFadeIn(clock),
            GrowArrow(arrow),
            self.change_students(*3 * ["pondering"], look_at=clock),
        )
        self.play(
            ...
            lightbulb.move_to,
            2 * UP,
            self.change_students(*3 * ["awe"], look_at=2 * UP),
            run_time=1,
        )
        self.play(self.get_light_shine(lightbulb))
        self.play(
            ReplacementTransform(VGroup(lightbulb), pi_lights),
            morty.change,
            "happy",
            *[
                ApplyMethod(pi.change, mode, pi.get_top())
                for pi, mode in zip(self.students, ["hooray", "tease", "surprised"])
            ],
        )
        self.wait(3)
