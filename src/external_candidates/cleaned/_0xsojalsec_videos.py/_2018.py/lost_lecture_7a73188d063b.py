# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\lost_lecture.py

from _2018.div_curl import VectorField, get_force_field_func
...
class SunAnimation(Group):
    ...
    def update(self, dt):
        ...
        self.rotated_sun.rotate(self.angle)
        self.rotated_sun.pixel_array = np.array(a * self.sun.pixel_array, dtype=self.sun.pixel_array.dtype)
...
class ShowEmergingEllipse(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(e_dot, LEFT), Write(eccentric_words, run_time=1))
        self.wait()
        self.play(
    ...
    def get_circle(self):
        circle = self.circle = Circle(radius=self.circle_radius, color=self.circle_color)
        return circle
    ...
    def get_ghost_lines(self, lines):
        return lines.copy().set_stroke(color=self.ghost_lines_stroke_color, width=self.ghost_lines_stroke_width)

    def get_elbow(self, line):
        elbow = VGroup(Line(UP, UL), Line(UL, LEFT))
        elbow.set_stroke(width=1)
        elbow.scale(0.2, about_point=ORIGIN)
        elbow.rotate(line.get_angle() - 90 * DEGREES, about_point=ORIGIN)
...
class ShowFullStory(Scene):
    def construct(self):
        directory = os.path.join(MEDIA_DIR, "animations/active_projects/lost_lecture/images")
        scene_names = [
            "ShowEmergingEllipse",
            "ShowFullStory",
            "FeynmanFameStart",
        ...
        images = Group(*[ImageMobject(os.path.join(directory, name + ".png")) for name in scene_names])
        for image in images:
            image.add(SurroundingRectangle(image, buff=0, color=WHITE))
        images.arrange_in_grid(n_rows=4)

        images.scale(1.01 * FRAME_WIDTH / images[0].get_width())
        ...
        self.play(
            ...
            -images[2].get_center(),
            images.scale,
            FRAME_WIDTH / images[2].get_width(),
            {"about_point": ORIGIN},
            run_time=3,
        )
        self.wait()
...
class FeynmanAndOrbitingPlannetOnEllipseDiagram(ShowEmergingEllipse):
    def construct(self):
        ...
        feynman_rect = BackgroundRectangle(feynman, fill_opacity=1)
...
class FeynmanFame(Scene):
    def construct(self):
        ...
        self.play(LaggedStartMap(Write, feynman_diagram, run_time=4))
        self.wait()
        self.play(
            self.get_book_intro(books[1]),
            self.get_book_outro(books[0]),
            LaggedStartMap(ApplyMethod, fd_parts, lambda m: (m.scale, 0), run_time=1),
        )
        ...
        joke = OldTexText("``Science is the belief \\\\ in the ignorance of \\\\ experts.''")
        ...
        self.wait()
        self.play(objects.shift, 2 * UP, VFadeOut(objects))
        ...
        self.play(
            ...
            ApplyMethod(feynman_smile.shift, FRAME_HEIGHT * DOWN, remover=True),
        )
    ...
    def get_book_animation(self, book, initial_shift, animated_shift, opacity_func):
        ...
        return AnimationGroup(
            ...
            UpdateFromAlphaFunc(
                rect,
                lambda r, a: r.move_to(book).set_fill(opacity=opacity_func(a)),
                remover=True,
            ),
        )

    def get_book_intro(self, book):
        return self.get_book_animation(book, 2 * DOWN, 2 * UP, lambda a: 1 - a)
    ...
    def get_feynman_diagram(self):
        ...
        labels = VGroup(*[OldTex(tex) for tex in ["e^-", "e^+", "\\text{\\=q}", "q"]])
        vects = [UR, DR, UL, DL]
        ...
        squiggle.next_to(arrows[2].point_from_proportion(0.75), DR, buff=0)
...
class TheMotionOfPlanets(Scene):
    ...
    def setup_orbits(self):
        ...
        archivist_words = OldTexText("Judith Goodstein (Caltech archivist)")
    ...
    def get_planets_ellipses_and_orbits(self, sun):
        ...
        ellipses = VGroup(*[Circle(radius=r, color=WHITE, stroke_width=1) for r in orbit_radii])
        ...
        for circle in ellipses:
            circle.rotate(TAU * np.random.random(), about_point=ORIGIN)

        ellipses.scale(3.5, about_point=ORIGIN)

        orbits = [
            Orbiting(planet, sun, circle, rate=0.25 * r ** (2 / 3))
...
class AskAboutEllipses(TheMotionOfPlanets):
    ...
    def add_sun(self):
        ...
        if self.animate_sun:
            ...
            self.add_foreground_mobjects(sun_animation.mobject)
        else:
    ...
    def add_focus_lines(self):
        ...
        animation = Mobject.add_updater(lines, update_lines)
        self.add(animation)
        self.wait(8)
    ...
    def add_force_labels(self):
        ...
        def update_radial_measurement(measurement):
            ...
            new_decimal = DecimalNumber(radial_line.get_length(), **radius_measurement_kwargs)
        ...
        radius_measurement_animation = Mobject.add_updater(radius_measurement, update_radial_measurement)
        ...
        radius_in_denominator = DecimalNumber(0, **radius_measurement_kwargs)
        ...
        update_radius_in_denominator = ContinualChangingDecimal(
            ...
            position_update_func=lambda mob: mob.move_to(radius_in_denominator_ref, LEFT),
        )

        # Force arrow
        force_arrow, force_arrow_animation = self.get_force_arrow_and_update(self.comet)

        inverse_square_law_words = OldTexText("``Inverse square law''")
        ...
        def v_fade_in(mobject):
            return UpdateFromAlphaFunc(mobject, lambda mob, alpha: mob.set_fill(opacity=alpha))
    ...
    def comment_on_imperfections(self):
        ...
        self.wait(7)
        self.play(ellipses.scale, scale_factor, {"about_point": center}, Animation(planets))
        self.remove(*orbits)
    ...
    def get_force_arrow_and_update(self, comet, scale_factor=1):
        ...
        def update_force_arrow(arrow):
            radial_line = Line(sun.get_center(), comet.get_center())
            radius = radial_line.get_length()
            # target_length = 1 / radius**2
            target_length = scale_factor / radius  # Lies!
            arrow.scale(target_length / arrow.get_length())
            arrow.rotate(np.pi + radial_line.get_angle() - arrow.get_angle())
            arrow.shift(radial_line.get_end() - arrow.get_start())

        force_arrow_animation = Mobject.add_updater(force_arrow, update_force_arrow)
...
class FeynmanSaysItBest(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            added_anims=[self.change_students("hooray", "happy", "erm")],
        )
        self.wait(3)
    ...
    def construct(self):
        ...
        for word in quote:
            if word is very:
                ...
                self.play(LaggedStartMap(FadeOut, nothing, run_time=1))
        ...
        for image, cross in zip(images, crosses):
            image.rect = SurroundingRectangle(image, stroke_width=3, stroke_color=WHITE, buff=0)
            cross.scale(1.1)
        self.play(FadeInFromDown(images[0]), FadeInFromDown(images[0].rect))
...
class AskAboutInfiniteIntelligence(TeacherStudentsScene):
    def construct(self):
        self.student_says("Infinite intelligence?", target_mode="confused")
        self.play(
...
class TableOfContents(Scene):
    def construct(self):
        ...
        for item in items:
            ...
            self.play(Transform(item, new_item), *list(map(MoveToTarget, other_items)))
            self.wait()
...
class DrawEllipseOverlay(Scene):
    def construct(self):
        ...
        image = ImageMobject(os.path.join(get_image_output_directory(self.__class__), "HeldUpEllipse.jpg"))
...
class ShowEllipseDefiningProperty(Scene):
    ...
    def add_focal_lines(self):
        ...
        dot_update = Mobject.add_updater(
            dot,
            lambda d: d.move_to(self.ellipse.point_from_proportion(position_tracker.get_value() % 1)),
        )
        always_shift(position_tracker, rate=0.05)

        lines, lines_update_animation = self.get_focal_lines_and_update(self.get_foci, dot)
    ...
    def add_distance_labels(self):
        ...
        distance_labels, distance_labels_animation = self.get_distance_labels_and_update(lines, colors)

        sum_expression, numbers, number_updates = self.get_sum_expression_and_update(
            lines, colors, lambda mob: mob.to_corner(UR)
        )

        sum_expression_fading_rect = BackgroundRectangle(sum_expression, fill_opacity=1)
        ...
        self.wait(7)
        self.play(ShowCreation(sum_rect), Write(constant_words))
        self.wait(7)
    ...
    def label_foci(self):
        foci = self.get_foci()
        focus_words = VGroup(*[OldTexText("Focus").next_to(focus, DOWN) for focus in foci])
        ...
        translation = OldTexText("``Foco'' $\\rightarrow$ Fireplace")
    ...
    def get_focal_lines_and_update(self, get_foci, focal_sum_point):
        ...
        def update_lines(lines):
            ...
            for line, focus in zip(lines, foci):
                line.put_start_and_end_on(focus, focal_sum_point.get_center())
            lines[1].rotate(np.pi)

        lines_update_animation = Mobject.add_updater(lines, update_lines)
    ...
    def get_distance_labels_and_update(self, lines, colors):
        ...
        def update_distance_labels(labels):
            for label, line, color in zip(labels, lines, colors):
                ...
                new_decimal.scale(self.distance_labels_scale_factor)
                max_width = 0.6 * line.get_width()
                if new_decimal.get_width() > max_width:
                    new_decimal.set_width(max_width)
                new_decimal.next_to(line, UP, SMALL_BUFF)
                new_decimal.set_color(color)
                new_decimal.add_to_back(new_decimal.copy().set_stroke(BLACK, 5))
                VGroup(new_decimal, line).rotate(-angle, about_point=ORIGIN)
                label.submobjects = list(new_decimal.submobjects)

        distance_labels_animation = Mobject.add_updater(distance_labels, update_distance_labels)
    ...
    def get_sum_expression_and_update(self, lines, colors, sum_position_func):
        ...
        number_updates = [
            ContinualChangingDecimal(
                numbers[0],
                lambda a: lines[0].get_length(),
                position_update_func=lambda m: m.move_to(number_refs[1], LEFT),
            ),
            ContinualChangingDecimal(
                numbers[1],
                lambda a: lines[1].get_length(),
                position_update_func=lambda m: m.move_to(number_refs[0], LEFT),
            ),
            ContinualChangingDecimal(
                numbers[2],
                lambda a: sum(map(Line.get_length, lines)),
                position_update_func=lambda m: m.move_to(number_refs[2], LEFT),
            ),
        ]
...
class GeometryProofLand(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, circles), run_time=2)
...
class ProveEllipse(ShowEmergingEllipse, ShowEllipseDefiningProperty):
    ...
    def hypothesize_foci(self):
        ...
        for label, dot, vect in zip(labels, dots, vects):
            ...
            label.add_to_back(label.copy().set_stroke(BLACK, 5))
            arrow = Arrow(label.get_corner(-vect), dot.get_corner(vect), buff=SMALL_BUFF)
            arrow.match_color(dot)
            arrow.add_to_back(arrow.copy().set_stroke(BLACK, 5))
            arrows.add(arrow)

        labels_target = labels.copy()
        labels_target.arrange(DOWN, aligned_edge=LEFT)
        ...
        self.play(LaggedStartMap(ShowPassingFlash, ghost_lines_copy))
        self.wait()
        ...
        self.wait()
        self.play(Write(guess_start), GrowFromCenter(brace), run_time=1)
        self.play(ReplacementTransform(labels.copy(), labels_target))
        self.wait()
    ...
    def setup_and_show_focal_sum(self):
        ...
        focal_lines, focal_lines_update_animation = self.get_focal_lines_and_update(get_foci, focal_sum_point)
        distance_labels, distance_labels_update_animation = self.get_distance_labels_and_update(focal_lines, colors)
        sum_expression, numbers, number_updates = self.get_sum_expression_and_update(
            focal_lines, colors, lambda mob: mob.to_edge(RIGHT).shift(UP)
        )
        ...
        for point in points:
            self.play(focal_sum_point.move_to, point)
            self.wait()
        self.remove(*to_add)
        self.play(*list(map(FadeOut, [focal_lines, distance_labels, sum_expression, numbers])))
    ...
    def show_circle_radius(self):
        ...
        radius_measurement.add_to_back(radius_measurement.copy().set_stroke(BLACK, 5))
        ...
        self.wait()
        self.play(Rotating(group, rate_func=smooth, run_time=7, about_point=center))
    ...
    def limit_to_just_one_line(self):
        ...
        for l in lines:
            l.generate_target()
            l.target.rotate(90 * DEGREES, about_point=l.get_center())

        self.set_variables_as_attrs(line, ghost_line, P_dot, P_label)
    ...
    def look_at_perpendicular_bisector(self):
        ...
        elbow = self.get_elbow(line)
        self.play(MoveToTarget(line, path_arc=90 * DEGREES), ShowCreation(elbow))
        ...
        focal_sum_point_animation = turn_animation_into_updater(MaintainPositionRelativeTo(self.focal_sum_point, Q_dot))
        ...
        Q_label_animation = turn_animation_into_updater(MaintainPositionRelativeTo(Q_label, Q_dot))
        ...
        self.focal_sum_things_to_add.append(distance_label_shift_update_animation)
        ...
        QE_line_update = Mobject.add_updater(
            QE_line,
            lambda l: l.put_start_and_end_on(Q_dot.get_center(), self.get_eccentricity_point()),
        )
        ...
        triangles = VGroup(
            *[
                ...
                for end_point in [P_dot.get_center(), self.get_eccentricity_point()]
            ]
        )
        ...
        distance_label_rotate_update_animation = Mobject.add_updater(
            self.distance_labels[0], distance_label_rotate_update
        )
        ...
        radial_line_update = UpdateFromFunc(
            radial_line,
            lambda l: l.put_start_and_end_on(circle.get_center(), P_dot.get_center()),
        )

        def put_dot_at_intersection(dot):
            point = line_intersection(line.get_start_and_end(), radial_line.get_start_and_end())
            dot.move_to(point)
            return dot

        keep_Q_dot_at_intersection = UpdateFromFunc(Q_dot, put_dot_at_intersection)
        Q_dot.restore()

        ghost_line_update_animation = UpdateFromFunc(
            ghost_line,
            lambda l: l.put_start_and_end_on(self.get_eccentricity_point(), P_dot.get_center()),
        )
        ...
        perp_bisector_update_animation = UpdateFromFunc(line, update_perp_bisector)
        elbow_update_animation = UpdateFromFunc(elbow, lambda e: Transform(e, self.get_elbow(ghost_line)).update(1))

        P_dot_movement_updates = [
            radial_line_update,
            keep_Q_dot_at_intersection,
            MaintainPositionRelativeTo(P_label, P_dot),
        ...
        sum_rect = SurroundingRectangle(self.numbers[-1])
        tangency_comment = OldTexText("Always $\\ge$ radius")
        tangency_comment.next_to(sum_rect, DOWN, aligned_edge=RIGHT)
        ...
        self.add(*self.focal_sum_things_to_add)
        self.play(FadeInFromDown(Q_label), GrowFromCenter(Q_dot))
        self.wait()
        self.add_foreground_mobjects(Q_dot)
        self.add(Q_label_animation)
        self.play(
            Q_dot.move_to,
            line.point_from_proportion(0.05),
            rate_func=there_and_back,
            run_time=4,
        )
        self.wait()
        ...
        self.play(FadeOut(triangles[1]), Animation(self.distance_labels))
        ...
        self.play(
            ApplyFunction(distance_label_rotate_update, distance_label_copy, path_arc=angle),
        ...
        self.focal_sum_things_to_add.append(distance_label_rotate_update_animation)
        self.wait()
        self.play(
            Q_dot.move_to,
            line.point_from_proportion(0),
            run_time=4,
            rate_func=there_and_back,
        )
    ...
    def show_orbiting_planet(self):
        ...
        def update_lines(lines):
            for gl, line in zip(self.ghost_lines, lines):
                intersection = line_intersection([self.circle.get_center(), gl.get_end()], line.get_start_and_end())
                distance = get_norm(intersection - planet.get_center())
                if distance < 0.025:
...
class KeplersSecondLaw(AskAboutEllipses):
    ...
    def add_title(self):
        ...
        subtitle = OldTexText("Orbits sweep a constant area per unit time")
    ...
    def contrast_close_to_far(self):
        ...
        arc_words.scale(0.8)
        arc_words.next_to(arc, RIGHT, buff=-SMALL_BUFF)
        ...
        self.play(ShowCreation(arc), Write(arc_words))
        ...
        self.play(ShowCreation(short_arc), Write(short_arc_words))
        self.wait(15)
    ...
    def show_area_sweep(self, time=1.0):
        ...
        area_update = UpdateFromFunc(
            area,
            lambda a: Transform(a, self.get_area(start_prop, orbit.proportion)).update(1),
        )
    ...
    def get_area(self, prop1, prop2):
        ...
        result = VMobject()
        result.append_vectorized_mobject(Line(sun_point, arc.get_points()[0]))
        result.append_vectorized_mobject(arc)
        result.append_vectorized_mobject(Line(arc.get_points()[-1], sun_point))
    ...
    def get_arc(self, prop1, prop2):
        ...
        if prop2 > prop1:
            arc = VMobject().pointwise_become_partial(ellipse, prop1, prop2)
        elif prop1 > prop2:
            arc, arc_extension = [
                VMobject().pointwise_become_partial(ellipse, p1, p2) for p1, p2 in [(prop1, 1.0), (0.0, prop2)]
            ]
            arc.append_vectorized_mobject(arc_extension)
        else:
            arc = VectorizedPoint(ellipse.point_from_proportion(prop1))
...
class NonEllipticalKeplersLaw(KeplersSecondLaw):
    ...
    def add_orbit(self):
        ...
        arrow, arrow_update = self.get_force_arrow_and_update(comet)
        alt_arrow_update = Mobject.add_updater(
            arrow, lambda a: a.scale(1.0 / a.get_length(), about_point=a.get_start())
        )
    ...
    def get_ellipse(self):
        orbit_shape = ParametricCurve(
            lambda t: (1 + 0.2 * np.sin(5 * TAU * t)) * np.array([np.cos(TAU * t), np.sin(TAU * t), 0])
        )
...
class AngularMomentumArgument(KeplersSecondLaw):
    ...
    def show_small_sweep(self):
        ...
        triangle.save_state()
        alt_triangle = Polygon(sun_center, interpolate(comet_start, comet_end, 0.9), comet_end)
        alt_triangle.match_style(triangle)

        comet = self.get_comet()
        comet.move_to(comet_start)

        velocity_vector = Arrow(comet_start, comet_end, color=WHITE, buff=0)
        velocity_vector_label = OldTex("\\vec{\\textbf{v}}")
        velocity_vector_label.next_to(velocity_vector.get_center(), UL, buff=SMALL_BUFF)
        ...
        self.add_foreground_mobjects(comet)
        self.play(
            ShowCreation(triangle, rate_func=lambda t: interpolate(1.0 / 3, 2.0 / 3, t)),
            MaintainPositionRelativeTo(velocity_vector, comet),
        ...
        self.comet = comet
        self.delta_t = small_time_label.get_part_by_tex("$\\Delta t$")
    ...
    def show_sweep_dimensions(self):
        ...
        base_point = line_intersection(base.get_start_and_end(), [top, top + DOWN])
        ...
        v_perp_label = OldTex("\\vec{\\textbf{v}}_\\perp")
        ...
        self.play(
            ...
            self.small_time_label.scale,
            0.5,
            self.small_time_label.to_corner,
            UL,
        )
        self.wait()
        self.play(
        ...
        self.play(ReplacementTransform(v_perp_delta_t.copy(), VGroup(*area_expression[-2:])))
        self.wait()
    ...
    def show_conservation_of_angular_momentum(self):
        ...
        vector_field = VectorField(get_force_field_func((sun_center, -1)))
        vector_field.set_fill(opacity=0.8)
        vector_field.sort(lambda p: -get_norm(p - sun_center))

        stays_constant = OldTexText("Stays constant")
        stays_constant.next_to(R_v_perp_rect, DR, buff=MED_LARGE_BUFF)
        ...
        self.play(Transform(big_dot, sun_dot, run_time=1, remover=True))
        self.wait()
        self.play(
        ...
        self.play(LaggedStartMap(GrowArrow, vector_field), Animation(foreground))
        for x in range(3):
            self.play(
                LaggedStartMap(
                    ApplyFunction,
        ...
        self.play(comet.scale, 2, comet.next_to, parens, RIGHT, {"buff": SMALL_BUFF})
        self.wait()
        self.play(
            FadeOut(parens),
            R_v_perp.restore,
...
class HistoryOfAngularMomentum(TeacherStudentsScene):
    CONFIG = {"camera_config": {"fill_opacity": 1}}
...
class FeynmanRecountingNewton(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(*[FadeIn(mob, direction=3 * LEFT) for mob in (principia, principia.rect)])
        self.wait()
...
class IntroduceShapeOfVelocities(AskAboutEllipses, MovingCameraScene):
    ...
    def reference_inverse_square_law(self):
        ellipse = self.ellipse
        force_equation = OldTex("F", "=", "{G", "M", "m", "\\over", "R^2}")
        ...
        radial_line, radial_line_update = self.get_radial_line_and_update(self.comet)
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(force_arrow, lambda m, a: m.set_fill(opacity=a)),
            UpdateFromAlphaFunc(radial_line, lambda m, a: m.set_stroke(width=a)),
        )
        self.wait(10)

    def show_velocity_vectors(self):
        alphas = np.linspace(0, 1, self.num_vectors, endpoint=False)
        vectors = VGroup(*[self.get_velocity_vector(alpha) for alpha in alphas])
        ...
        self.play(FadeOut(vectors), VFadeIn(moving_vector))
        self.wait(10)
        vectors.set_fill(opacity=0.5)
        self.play(LaggedStartMap(ShowCreation, vectors), Animation(moving_vector))
        self.wait(5)
    ...
    def collect_velocity_vectors(self):
        ...
        for vector in vectors:
            ...
            vector.target.add_to_back(vector.target.copy().set_stroke(BLACK, 5))
        ...
        self.wait(2)
        self.play(ShowCreation(circle), Animation(vectors))
        self.wait(24)
    ...
    def get_velocity_vector(self, alpha, d_alpha=0.01, scalar=3.0):
        ...
        p1, p2 = [ellipse.point_from_proportion(a) for a in (alpha, alpha + d_alpha)]
        vector = Arrow(p1, p2, buff=0)
        radius_vector = p1 - sun_center
        curr_v_perp = norm(np.cross(vector.get_vector(), radius_vector / norm(radius_vector)))
        vector.scale(
        ...
        vector.set_color(
            interpolate_color(
                BLUE,
                RED,
                inverse_interpolate(min_length, max_length, vector.get_length()),
            )
        )
    ...
    def get_velocity_vector_and_update(self):
        ...
        moving_vector_animation = Mobject.add_updater(moving_vector, update_moving_vector)
...
class AskWhy(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change,
            "happy",
            self.change_students("raise_left_hand", "sassy", "confused"),
        )
        self.wait(5)
...
class ShowEqualAngleSlices(IntroduceShapeOfVelocities):
    ...
    def show_equal_angle_slices(self):
        ...
        for angle in angles:
            ...
            while abs(coss_diff) > 0.00001:
                d_prop = 0.001
                alt_end = ellipse.point_from_proportion((prop + d_prop) % 1)
            ...
            theta.move_to(angle_arc.get_center() + 0.2 * vect)
            thetas.add(theta)
        ...
        for l1, l2 in adjacent_pairs(lines):
            arc = VMobject()
            arc.pointwise_become_partial(ellipse, l1.prop, (l2.prop or 1.0))
            arcs.add(arc)

            wedge = VMobject()
            wedge.append_vectorized_mobject(Line(sun_center, arc.get_points()[0]))
            wedge.append_vectorized_mobject(arc)
            wedge.append_vectorized_mobject(Line(arc.get_points()[-1], sun_center))
            wedges.add(wedge)

        lines.set_stroke(GREY_B, 2)
        angle_arcs.set_color_by_gradient(YELLOW, BLUE, RED, PINK, YELLOW)
        ...
        self.play(FadeIn(faders), *list(map(ShowCreation, lines)))
        self.play(
            *[LaggedStartMap(ApplyMethod, fader, lambda m: (m.set_fill, {"opacity": 1}), **kwargs) for fader in faders]
            + [Animation(lines)]
        )
        self.wait()
    ...
    def ask_about_time_per_slice(self):
        ...
        words1 = OldTexText("Time spent \\\\ traversing \\\\ this slice?")
    ...
    def areas_are_proportional_to_radius_squared(self):
        ...
        scaling_group = VGroup(wedge, thick_line, radius_word, thick_arc)

        expression = OldTexText("Time", "$\\propto$", "Area", "$\\propto$", "$(\\text{Radius})^2$")
        ...
        self.wait(2)
        self.play(ShowCreation(thick_line), FadeInFromDown(radius_word))
        self.wait()
    ...
    def show_inverse_square_law(self):
        ...
        force_expression = OldTex("ma", "=", "\\text{Force}", "\\propto", "\\frac{1}{(\\text{Radius})^2}")
        ...
        acceleration_expression.next_to(force_expression, DOWN, buff=0.75, aligned_edge=LEFT)

        delta_v_expression = OldTex("\\Delta v}", "\\propto", "{\\Delta t", "\\over", "(\\text{Radius})^2}")
        delta_v_expression.next_to(acceleration_expression, DOWN, buff=0.75, aligned_edge=LEFT)
        delta_t_numerator = delta_v_expression.get_part_by_tex("\\Delta t")
        ...
        self.play(randy.change, "confused", force_expression, VFadeIn(randy))
        self.wait(2)
        self.play(
            randy.change,
            "pondering",
        ...
        self.play(comet.move_to, arc.get_points()[0], path_arc=90 * DEGREES)
        force_vector_update.update(0)
        self.play(Blink(randy), GrowArrow(force_vector))
        ...
        self.play(GrowArrow(start_velocity_vector), randy.look_at, moving_vector)
        self.add(moving_vector_animation)
        self.add(orbit)
        ...
        self.wait(2)
        self.play(
            delta_t_numerator.scale,
            1.5,
            {"about_edge": DOWN},
            delta_t_numerator.set_color,
            YELLOW,
        )
        self.play(ShowCreationThenFadeAround(prop_exp[:-2]))
        self.play(
            delta_t_numerator.fade,
            1,
    ...
    def directly_compare_velocity_vectors(self):
        ...
        vectors = VGroup(*[self.get_velocity_vector(line.prop) for line in lines])
        index = len(vectors) / 2
        v1 = vectors[index]
        v2 = vectors[index + 1]
        ...
        for vector in vectors:
            vector.save_state()
            vector.target = Arrow(*vector.get_start_and_end(), color=vector.get_color(), buff=0)
            vector.target.scale(2)
            vector.target.shift(root_point - vector.target.get_start())
            vector.target.add_to_back(vector.target.copy().set_stroke(BLACK, 5))
        ...
        for vect1, vect2 in adjacent_pairs(vectors):
            ...
            diff_vect.add_to_back(diff_vect.copy().set_stroke(BLACK, 2))
        ...
        polygon = Polygon(*[vect.target.get_end() for vect in vectors])
        ...
        self.wait()
        self.play(ReplacementTransform(v1.saved_state.copy(), v2.saved_state, path_arc=self.theta))
        ...
        for i in range(n - 1):
            ...
            self.play(ReplacementTransform(v1.saved_state.copy(), v2.saved_state, path_arc=self.theta))
            self.play(MoveToTarget(v2), GrowArrow(diff_vect))
        self.add(self.orbit)
        self.wait()
        self.play(
...
class IKnowThisIsTricky(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            added_anims=[self.change_students(*3 * ["horrified"], look_at=self.screen)],
        )
        self.look_at(self.screen)
        self.wait(3)
...
class PonderOverOffCenterDiagram(PiCreatureScene):
    def construct(self):
        ...
        rect = SurroundingRectangle(velocity_diagram, buff=MED_LARGE_BUFF, color=GREY_B)
...
class UseVelocityDiagramToDeduceCurve(ShowEqualAngleSlices):
    ...
    def setup_velocity_diagram(self):
        ...
        for line in self.lines:
            ...
            vector.target = self.get_velocity_vector(line.prop, scalar=8.0)
            vector.target.shift(root_point - vector.target.get_start())
        ...
        circle.shift((root_point[0] - circle.get_center()[0]) * RIGHT)
        ...
        self.play(
            LaggedStartMap(MoveToTarget, vectors, lag_ratio=1, run_time=2),
            GrowFromCenter(root_dot),
    ...
    def show_theta_degrees(self):
        ...
        ellipse_arc.pointwise_become_partial(ellipse, 0, thick_line.prop)
        ellipse_arc.set_stroke(YELLOW, 3)

        ellipse_wedge = self.get_wedge(ellipse_arc, sun_center)
        ellipse_wedge_start = self.get_wedge(VectorizedPoint(ellipse.get_right()), sun_center)

        ellipse_angle_arc = Arc(self.theta * index, radius=0.5)
        ...
        vector.set_fill(YELLOW)
        circle_arc = VMobject()
        circle_arc.pointwise_become_partial(circle, 0, fdiv(index, len(vectors)))
        ...
        circle_wedge_start = self.get_wedge(Line(circle.get_top(), circle.get_top()), circle.get_center()).match_style(
            circle_wedge
        )
        ...
        self.play(
            ...
            Write(ellipse_theta),
            ReplacementTransform(lines[0].copy(), thick_line, path_arc=angle),
            MoveAlongPath(comet, ellipse_arc),
            run_time=2,
        )
        self.wait()
        self.play(
            ReplacementTransform(circle_wedge_start, circle_wedge, path_arc=angle),
    ...
    def match_velocity_vector_to_tangency(self):
        ...
        tangent_line = Line(*original_vector.get_start_and_end())
        tangent_line.set_stroke(GREY_B, 3)
        tangent_line.scale(5)
        tangent_line.move_to(comet)

        self.play(
            ReplacementTransform(vector.copy(), original_vector, run_time=2),
            Animation(comet),
        )
        self.wait()
        self.play(
    ...
    def replace_vectors_with_lines(self):
        ...
        highlighted_line = Line(*highlighted_vector.get_start_and_end(), stroke_color=YELLOW, stroke_width=4)

        self.play(
            LaggedStartMap(MoveToTarget, vectors),
            highlighted_vector.scale,
            0,
    ...
    def not_that_velocity_vector_is_theta(self):
        ...
        self.play(
            ReplacementTransform(
                self.ellipse_theta.copy(),
                self.circle_theta,
            ),
            ReplacementTransform(
                self.ellipse_angle_arc.copy(),
                self.circle_arc,
            ),
            run_time=2,
        )
    ...
    def ask_about_curve(self):
        ...
        self.play(
            ...
            "confused",
            ellipse,
            ShowCreationThenDestruction(ellipse.copy().set_stroke(BLUE, 3), run_time=2),
        )
        self.play(
            Blink(morty),
            ApplyWave(ellipse),
        ...
        self.play(line.move_to, vector, run_time=2)
    ...
    def show_90_degree_rotation(self):
        ...
        angle = -90 * DEGREES
        circle_setup.target.rotate(angle, about_point=circle.get_center())
        ...
        self.play(
            *[
                Restore(mob, path_arc=angle, run_time=4, rate_func=there_and_back_with_pause)
                for mob in list(lines) + [highlighted_line]
            ]
    ...
    def show_geometry_of_rotated_diagram(self):
        ...
        self.play(
            self.ghost_lines.set_stroke,
            {"width": 0.5},
            self.eccentric_lines.set_stroke,
            {"width": 0.5},
        ...
        self.wait(2)
        self.play(arc_copy.scale, 0.15, {"about_point": center}, run_time=2)
        self.wait(2)
        self.play(
            perp_line.move_to,
            velocity_vector,
    ...
    def get_wedge(self, arc, center_point, opacity=0.8):
        wedge = VMobject()
        wedge.append_vectorized_mobject(Line(center_point, arc.get_points()[0]))
        wedge.append_vectorized_mobject(arc)
        wedge.append_vectorized_mobject(Line(arc.get_points()[-1], center_point))
...
class TryToRememberProof(PiCreatureScene):
    def construct(self):
        ...
        self.play(randy.change, "telepath", FadeInFromDown(words))
        self.look_at(ORIGIN)
        self.wait(2)
...
class PatYourselfOnTheBack(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(*3 * ["pondering"], look_at=self.screen),
        )
        self.look_at(UP)
        self.wait(8)
...
class Thumbnail(ShowEmergingEllipse):
    ...
    def construct(self):
        ...
        l1 = Line(circle.point_from_proportion(0.175), 6.25 * RIGHT + 0.75 * DOWN)
        l2 = Line(circle.point_from_proportion(0.75), 6.25 * RIGHT + 2.5 * DOWN)
        ...
        title = OldTexText("Feynman's \\\\", "Lost \\\\", "Lecture", alignment="")
        title.scale(2.4)
        for part in title:
            part.add_to_back(part.copy().set_stroke(BLACK, 12).set_fill(BLACK, 1))
        title.to_corner(UR)
