# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\sphere_area.py
...
class AskAboutShadowRelation(SpecialThreeDScene):
    ...
    def show_surface_area(self):
        ...
        sa_equation = OldTex("\\text{Surface area} = 4\\pi R^2", tex_to_color_map={"R": BLUE})
    ...
    def show_four_circles(self):
        ...
        radial_line = Line(shadow.get_center(), shadow.get_right(), color=YELLOW)
        R_label = OldTex("R").set_color(self.R_color)
        R_label.scale(0.8)
        R_label.next_to(radial_line, DOWN, SMALL_BUFF)
        shadow.add(radial_line, R_label)
        shadow.move_to(self.camera.transform_points_pre_display(sphere, [sphere.get_center()])[0])

        shadows = VGroup(*[shadow.copy() for x in range(4)])
        shadows.arrange_in_grid(buff=MED_LARGE_BUFF)
        shadows.to_edge(RIGHT)

        area_label = OldTex("\\pi R^2", tex_to_color_map={"R": self.R_color})
        area_label.scale(1.2)
        area_labels = VGroup(
            *[area_label.copy().move_to(interpolate(mob.get_center(), mob.get_top(), 0.5)) for mob in shadows]
        )
    ...
    def show_all_pieces(self):
        ...
        self.play(FadeOut(shadows), FadeOut(area_labels), FadeIn(shadow_pieces_group))
    ...
    def unshow_area_pieces(self, *mobjects):
        self.play(*[ApplyMethod(mob.space_out_submobjects, 1.0 / self.space_out_factor) for mob in mobjects])
...
class PreviewTwoMethods(MovingCameraScene):
    def construct(self):
        ...
        for thumbnail in thumbnails:
            ...
            thumbnail.add_to_back(SurroundingRectangle(thumbnail, buff=0, stroke_color=WHITE, stroke_width=5))
...
class MapSphereOntoCylinder(SphereCylinderScene):
    def construct(self):
        ...
        self.play(ReplacementTransform(cylinder, unwrapped_cylinder), run_time=3)
        ...
        self.play(ShowCreation(top_line), FadeIn(two_pi_R, IN))
        self.wait()
        self.play(ShowCreation(side_line), FadeIn(two_R, RIGHT))
        self.wait()
...
class UnfoldCircles(Scene):
    ...
    def show_rectangle_with_formula(self):
        ...
        pre_area_label = OldTex("2\\cdot", "2", "\\pi", "R", "\\cdot R")
        area_label = OldTex("4", "\\pi", "R^2")
        for label in [area_label, pre_area_label]:
            label.next_to(rect.get_center(), UP, SMALL_BUFF)

        self.rect_group = VGroup(rect, h_line, w_line, two_R, two_pi_R, area_label)
    ...
    def add_four_circles(self):
        ...
        radii_lines = VGroup(*[Line(radius * UP, ORIGIN).set_stroke(WHITE, 2) for x in range(4)])
        radii_lines.arrange_in_grid(buff=1.3)
        radii_lines[2:].shift(RIGHT)
        radii_lines.next_to(rect_group, DOWN, buff=1.3)
        R_labels = VGroup(*[OldTex("R").next_to(line, LEFT, SMALL_BUFF) for line in radii_lines])
        ...
        self.play(
            ...
            R_labels[2:].shift,
            2.9 * RIGHT,
            VGroup(radii_lines[:2], R_labels[:2]).to_edge,
            LEFT,
            {"buff": 1.0},
        )
        ...
        for triangle in triangles:
            ...
            border = Polygon(*[triangle.get_corner(vect) for vect in (DL, UL, DR)])
...
class ShowProjection(SphereCylinderScene):
    ...
    def show_length_scaled_up(self):
        ...
        equation = self.equation
        equation.save_state()
        new_example_groups = [self.get_example_group([1, -1, z]) for z in [6, 0.25]]
    ...
    def show_length_stretch_of_rect(self, example_group):
        ...
        line1, line2 = lines = VGroup(*[Line(m.get_anchors()[1], m.get_anchors()[2]) for m in rects])
        ...
        self.play(FadeIn(lines[0]), FadeOut(rects), FadeOut(proj_lines_to_fade))
        for x in range(3):
            anims = []
    ...
    def show_height_scaled_down(self):
        ...
        r1, lines, r2 = example_group
        to_fade = VGroup(
            *[mob for mob in it.chain(ghost_sphere, ghost_cylinder) if np.dot(mob.get_center(), [1, 1, 0]) < 0]
        )
        to_fade.save_state()
    ...
    def get_projection_lines(self, piece):
        ...
        for corner in piece.get_anchors()[:-1]:
            ...
            result.add(VGroup(*[Line(p1, p2, **kwargs) for p1, p2 in [(start, corner), (corner, end)]]))
    ...
    def get_example_group(self, vect):
        pieces = self.pieces
        rect = pieces[np.argmax([np.dot(r.saved_state.get_center(), vect) for r in pieces])].saved_state.copy()
...
class JustifyLengthStretch(ShowProjection):
    ...
    def cut_cross_section(self):
        ...
        to_fade = VGroup(*[mob for mob in it.chain(sphere, cylinder) if np.dot(mob.get_center(), DOWN) > 0])
        self.lost_hemisphere = to_fade
        to_fade.save_state()

        circle = Circle(stroke_width=2, stroke_color=PINK, radius=self.sphere.radius)
        ...
        self.stop_ambient_camera_rotation()
        self.play(
            Rotate(to_fade, -PI, axis=OUT, about_point=sphere.get_left(), run_time=2),
    ...
    def show_similar_triangles(self):
        ...
        big_triangle = Polygon(p0, self.project_point(p1), self.project_point(p2))
        ...
        self.play(ReplacementTransform(point, triangle), Animation(self.camera.phi_tracker))
...
class JustifyLengthStretchHigherRes(JustifyLengthStretch):
    CONFIG = {
        "sphere_config": {"resolution": (2 * 24, 2 * 48)},
    }


class JustifyLengthStretchHighestRes(JustifyLengthStretch):
    CONFIG = {
        "sphere_config": {"resolution": (4 * 24, 4 * 48)},
    }
...
class WidthScaleLabel(Scene):
    def construct(self):
        text = OldTex("\\text{Width scale factor} =", "\\frac{R}{d}")
...
class HeightSquishLabel(Scene):
    def construct(self):
        text = OldTex("\\text{Height squish factor} =", "\\frac{R}{d}")
        self.play(Write(text))
        self.wait()
...
class JustifyHeightSquish(MovingCameraScene):
    ...
    def recreate_cross_section(self):
        ...
        R_label = OldTex("R")
        R_label.next_to(R_line, DOWN, SMALL_BUFF)
        d_lines = VGroup(*[self.get_d_line(phi) for phi in [self.low_phi, self.top_phi]])
        d_line = d_lines[0]
        d_line.set_color(self.d_color)
        d_label = OldTex("d")
        d_label.next_to(d_line, DOWN, SMALL_BUFF)

        proj_lines = VGroup(*[self.get_R_line(phi) for phi in [self.top_phi, self.low_phi]])
    ...
    def tweak_parameter(self):
        ...
        for new_phi in alt_phis:
            ...
            self.play(
                phi_tracker.set_value,
                new_phi,
                UpdateFromFunc(
                    tri_group,
                    lambda tg: tg.become(self.get_double_triangle_group(phi_tracker.get_value(), d_phi)),
                ),
                Rotate(
                    rad_tangent,
                    -(new_phi - curr_phi),
                ...
                UpdateFromFunc(
                    d_line,
                    lambda dl: dl.become(self.get_d_line(phi_tracker.get_value())),
                ),
    ...
    def label_angles(self):
        ...
        equation = OldTex("\\alpha", "+", "\\beta", "+", "90^\\circ", "=", "180^\\circ")
        equation.scale(0.6)
        equation.next_to(frame.get_corner(UR), DL)

        movers = VGroup(alpha_label.deepcopy(), beta_label.deepcopy(), elbow.copy())
        indices = [0, 2, 4]
        ...
        self.wait(0.25)
        self.play(ShowCreation(alpha_arc1), FadeIn(q_mark, 0.1 * RIGHT))
        self.wait()
        self.play(ShowPassingFlash(self.tangent.copy().scale(0.1).set_stroke(PINK, 0.5)))
        self.wait()
...
class WhyAreWeDoingThis(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("maybe", "pondering", "hesitant", added_anims=[self.teacher.change, "tease"])
        self.wait(3)
        self.play(
...
class SameEffectAsRotating(Scene):
    ...
    def construct(self):
        ...
        times_R_over_d.add_updater(lambda m: m.next_to(low_brace, DOWN, SMALL_BUFF))
        times_d_over_R.add_updater(lambda m: m.next_to(right_brace, RIGHT, SMALL_BUFF))
        ...
        self.play(TransformFromCopy(rect1, rotated_rect1, path_arc=-90 * DEGREES, run_time=2))
...
class RotateAllPiecesWithExpansion(ShowProjection):
    ...
    def rotate_all_pieces(self):
        ...
        self.wait()
        self.play(*[Rotate(piece, 90 * DEGREES, axis=piece.get_center()) for piece in sphere])
        self.wait()
...
class RectangulatedSphere(SphereCylinderScene):
    CONFIG = {
        "sphere_config": {"resolution": (10, 20)},
        "uniform_color": False,
        "wait_time": 10,
    }
...
class SequenceOfSpheres(SphereCylinderScene):
    def construct(self):
        ...
        spheres, cylinders = groups = VGroup(
            *[
                VGroup(*[func(resolution=(n, 2 * n)) for k in range(1, n_shapes + 1) for n in [3 * (2**k)]])
        ...
        for sphere, cylinder in zip(spheres, cylinders):
            ...
            equals.move_to(interpolate(sphere.get_nadir(), cylinder.get_zenith(), 0.5))
        ...
        tilted_final_arrows = VGroup(
            final_arrows[0].copy().rotate(-45 * DEGREES, axis=DOWN).shift(0.75 * IN),
            final_arrows[1].copy().rotate(45 * DEGREES, axis=DOWN).shift(0.75 * OUT),
        )
        ...
        for i in range(n_shapes):
            ...
            if i > 0:
                anims += [Write(arrow_group[i - 1]) for arrow_group in arrow_groups]
            self.play(*anims, run_time=1)
            self.play(GrowFromCenter(all_equals[i]))
        self.play(FadeIn(q_marks, LEFT), Write(final_arrows))
        self.wait()
        self.play(
...
class WhatIsSurfaceArea(SpecialThreeDScene):
    ...
    def construct(self):
        ...
        surface = always_redraw(lambda: self.get_surface(radius=3, amplitude=1, power=power_tracker.get_value()))
...
class UnwrappedCircleLogic(UnfoldCircles):
    ...
    def construct(self):
        ...
        def get_unwrapped_circle():
            ...
            self.get_submob_from_prop(result, get_highlight_prop()).set_stroke(YELLOW, 2)
            return result
        ...
        tri_R_line = always_redraw(lambda: Line(ORIGIN, radius * DOWN).shift(center_tracker.get_center()))
        ...
        self.play(
            ...
            FadeInFromDown(two_pi_r_label.copy().clear_updaters(), remover=True),
        )
...
class AskAboutDirectConnection(TeacherStudentsScene, SpecialThreeDScene):
    ...
    def construct(self):
        ...
        arrows = VGroup()
        for m1, m2 in zip(group, group[1:]):
            arrow = Arrow(m1.get_center(), m2.get_center(), buff=1, color=WHITE)
            arrows.add(arrow)
...
class ExercisesGiveLearning(MovingCameraScene):
    def construct(self):
        ...
        self.play(ShowCreation(cross))
        self.play(VGroup(lectures, cross).shift, DOWN, FadeIn(exercises, UP))
        self.wait()
...
class NobodyLikesHomework(TeacherStudentsScene):
    def construct(self):
        self.play_student_changes("angry", "pleading", "angry", added_anims=[self.teacher.change, "guilty"])
        self.wait()
        self.play_all_student_changes(
            "tired",
            look_at=8 * RIGHT + 4 * DOWN,
...
class SecondProof(SpecialThreeDScene):
    ...
    def cut_cross_section(self):
        ...
        self.set_variables_as_attrs(back_half, front_half, front_half_ghost, slice_circle=circle)
    ...
    def show_theta(self):
        ...
        theta_mob.add_updater(lambda m: m.set_fill(opacity=get_theta_mob_opacity()))
        theta_mob.add_updater(lambda m: m.set_background_stroke(width=get_theta_mob_opacity()))

        lit_ring = always_redraw(lambda: self.get_ring_from_theta(self.rings, get_theta()).copy().set_color(YELLOW))
    ...
    def ask_about_ring_circumference(self):
        ...
        to_fade = VGroup(self.R_label, self.radial_line, self.brace, self.brace_label)

        self.move_camera(phi=0 * DEGREES, theta=-90 * DEGREES, added_anims=[FadeOut(to_fade)])
        self.play(ShowCreation(circle))
        self.wait()
        self.move_camera(
            phi=70 * DEGREES,
    ...
    def ask_about_shadow_area(self):
        ...
        dashed_lines = VGroup(*[get_dashed_line(t) for t in [theta, theta + d_theta]])
    ...
    def ask_about_2_to_1_correspondance(self):
        theta_tracker = ValueTracker(0)
        get_theta = theta_tracker.get_value
        new_lit_ring = always_redraw(lambda: self.get_ring_from_theta(self.rings, get_theta()).copy().set_color(PINK))
    ...
    def show_all_shadow_rings(self):
        ...
        R_label = self.R_label
        radial_line = self.radial_line
        R_label.rotate(-90 * DEGREES, axis=RIGHT, about_point=radial_line.get_center())
        shadows = self.shadows
    ...
    def get_hemisphere(self, group, vect):
        if len(group.submobjects) == 0:
            ...
            return VGroup(*[self.get_hemisphere(submob, vect) for submob in group])
...
class YouCouldIntegrate(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.teacher_says("We'll be a bit \\\\ more Archimedean", target_mode="speaking")
...
class Question3(Scene):
    def construct(self):
        ...
        equation = OldTexText("(Shadow area)", "=", "$\\frac{1}{2}$", "(Area of one of the rings)")
...
class ExtraHint(Scene):
    def construct(self):
        ...
        title.scale(2.5)
        title.shift(UP)
        equation = OldTex("\\sin(2\\theta) = 2\\sin(\\theta)\\cos(\\theta)")
...
class Question4(Scene):
    def construct(self):
        question = OldTexText(
            "Question \\#4:",
            "Explain how the shadows relate to\\\\every other ring on the sphere.",
            tex_to_color_map={
                "shadows": YELLOW,
...
class SpherePatronThanks(Scene):
    ...
    def show_columns(self):
        ...
        patrons = VGroup(*[OldTexText(name) for name in self.specific_patrons])
        columns = VGroup()
        column_size = 15
