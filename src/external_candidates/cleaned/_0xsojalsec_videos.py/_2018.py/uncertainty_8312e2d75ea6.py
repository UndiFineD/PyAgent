...
class GaussianDistributionWrapper(Line):
    ...
    def get_random_points(self, size=1):
        ...
        return np.array(
            [
                np.array([np.random.normal(mu_coord, sigma_coord) for mu_coord, sigma_coord in zip(mu, sigma)])
                for x in range(size)
            ]
        )
...
class MentionDopplerRadar(TeacherStudentsScene):
    def construct(self):
        ...
        look_at_anims = [
            Mobject.add_updater(pi, lambda pi: pi.look_at(pulse.mobject)) for pi in self.get_pi_creatures()
        ]
...
class IntroduceDopplerRadar(Scene):
    ...
    def show_frequency_shift_in_fourier(self):
        ...
        self.play(
            FadeOut(pulse_graph),
            FadeIn(sum_graph),
            ReplacementTransform(pulse_graph.fourier_transform, sum_graph.fourier_transform),
        )
...
class MentionPRFNuance(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            UpdateFromFunc(
                brace,
                lambda b: b.match_width(peak_points, stretch=True).next_to(peak_points, UP, SMALL_BUFF),
            ),
            Transform(
...
class AmbiguityInLongEchos(IntroduceDopplerRadar, PiCreatureScene):
    ...
    def fourier_transform_of_one_pulse(self):
        ...
        self.play(
            ...
            LaggedStartMap(FadeOut, VGroup(self.curr_graph, self.dish, self.pi_creature)),
            run_time=2,
        )
    ...
    def show_echos_of_moving_objects(self):
        ...
        movements = self.object_movements = [
            always_shift(obj, direction=v / get_norm(v), rate=get_norm(v)) for v, obj in zip(object_velocities, objects)
        ]
        pulses = self.get_pulses()
    ...
    def concentrated_fourier_requires_long_time(self):
        ...
        self.play(
            randy.change,
            "angry",
            *[UpdateFromAlphaFunc(obj, lambda m, a: m.set_fill(opacity=a)) for obj in objects],
        )
        self.play(Blink(randy))
        self.wait(2)
        self.play(Blink(randy))
        ...
        self.play(
            randy.change,
            "plain",
            *[UpdateFromAlphaFunc(obj, lambda m, a: m.set_fill(opacity=1 - a)) for obj in objects],
        )
        self.wait()
...
class IntroduceDeBroglie(Scene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            MED_LARGE_BUFF * UP,
            *[
                ApplyFunction(lambda m: m.rotate(TAU / 4).set_stroke(width=0), mob, remover=True)
        ...
        for x in range(3):
            ...
            self.play(
                ApplyMethod(particle.move_to, axes.coords_to_point(22, 0), rate_func=linear),
        ...
        def get_wave():
            ...
            return VGroup(
                ...
                *[
                    ...
                    for x in np.arange(-4 * FRAME_X_RADIUS, FRAME_X_RADIUS, self.arrow_spacing)
                ],
            )
            return
    ...
    def introduce(self):
        ...
        self.play(
            *[
                UpdateFromAlphaFunc(
                    weight,
                    lambda w, a: w.set_width(2 * interpolate(w.start_radius, w.target_radius, a)),
                    run_time=2,
                )
    ...
    def moving_reference_frame(self):
        ...
        def zoom_into_reference_frame():
            ...
            self.play(
                UpdateFromAlphaFunc(
                    camera_frame,
                    lambda c, a: c.set_height(interpolate(original_height, 0.95 * rect.get_height(), a)).move_to(
    ...
    def construct(self):
        ...
        v_line_update = UpdateFromFunc(
            ...
            lambda l: l.put_start_and_end_on_with_projection(
                graph.get_points()[-1],
                axes.x_axis.number_to_point(axes.x_axis.point_to_number(graph.get_points()[-1])),
            ),
        )
...
class ProbabalisticDetection(FourierTransformOfWaveFunction):
    ...
    def detect_only_single_points(self):
        ...
        gdw_anim = Mobject.add_updater(
            gdw,
            lambda m: m.set_width(2.0 / (self.a_tracker.get_value() ** (0.5))).move_to(rect),
        )
    ...
    def show_probability_distribution(self):
        ...
        v_lines = VGroup(
            *[DashedLine(ORIGIN, 3 * UP).move_to(point, DOWN) for point in (rect.get_left(), rect.get_right())]
        )
...
class ThinkOfHeisenbergUncertainty(PiCreatureScene):
    def construct(self):
        ...
        continual_anims = [
            ...
            Mobject.add_updater(
                dot_gdw,
                lambda d: d.set_width((np.cos(freq * time_tracker.get_value()) + 1.1) / 2),
            ),
            Mobject.add_updater(
                vector_gdw,
                lambda d: d.set_width((-np.cos(freq * time_tracker.get_value()) + 1.1) / 2),
