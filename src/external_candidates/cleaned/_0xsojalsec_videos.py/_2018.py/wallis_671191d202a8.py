# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\wallis.py
# -*- coding: utf-8 -*-
...
def get_wallis_product(n_terms=6, show_result=True):
    ...
    for x in range(n_terms):
        ...
        tex_mob_args += ["{%d" % numerator, "\\over", "%d}" % denominator, "\\cdot"]
    ...
    def construct(self):
        ...
        def get_arrow_update():
            return ApplyFunction(
                lambda mob: mob.next_to(number_line.number_to_point(curr_product), UP, SMALL_BUFF),
                arrow,
            )

        get_arrow_update().update(1)
        decimal = DecimalNumber(curr_product, num_decimal_places=5, show_ellipsis=True)
        ...
        rects = VGroup(*[SurroundingRectangle(product_mob[:n]) for n in list(range(3, 4 * n_terms, 4)) + [4 * n_terms]])
        rect = rects[0].copy()

        pi_halves_arrow = Vector(UP, color=BLUE)
        pi_halves_arrow.next_to(number_line.number_to_point(np.pi / 2), DOWN, SMALL_BUFF)
...
class TableOfContents(Scene):
    def construct(self):
        topics = VGroup(
            ...
            OldTexText("Generalizing this proof to get \\\\ the product formula for sine"),
        )
        for topic in topics:
            dot = Dot(color=BLUE)
...
class SourcesOfOriginality(TeacherStudentsScene):
    ...
    def break_down_value_of_math_presentations(self):
        ...
        big_rect = self.big_rect = Rectangle(width=title.get_width() + 2 * MED_LARGE_BUFF, height=3.5, color=WHITE)
        ...
        based_on_wastlund = OldTexText("Previous video based on\\\\", 'a paper by Johan W\\"{a}stlund')
        ...
        self.play(
            ...
            self.change_students(*["pondering"] * 3, look_at=title),
        )
        ...
        self.play(
            presentation.match_color,
            right_rect,
            ReplacementTransform(presentation.copy(), communication),
        )
        self.play(FadeIn(communication_parts), FadeIn(lighthouse), SwitchOn(ambient_light))
        self.play(self.teacher.change, "tease")
        self.wait()

        self.play(
            FadeIn(based_on_wastlund),
            self.change_students("sassy", "erm", "plain", look_at=based_on_wastlund),
        )
        self.wait()
    ...
    def where_we_fit_in(self):
        ...
        self.play(FadeIn(wallis_product), Write(wallis_product_name, run_time=1))
        self.wait(2)
        self.play(
...
class ShowProduct(Scene):
    ...
    def setup_wallis_product(self):
        ...
        wallis_product_parts = VGroup(*[full_wallis_product[i : i + 4] for i in range(0, len(full_wallis_product), 4)])
    ...
    def show_larger_terms(self):
        ...
        dots = VGroup(*[Dot(axes.coords_to_point(n + 1, prod)) for n, prod in enumerate(partial_products)])
        dots.match_color(parts)
        lines = VGroup(*[Line(d1.get_center(), d2.get_center()) for d1, d2 in zip(dots, dots[1:])])

        braces = VGroup(*[Brace(parts[: n + 1], DOWN) for n in range(len(parts))])
        ...
        for part, line, dot, prod, new_brace in tuples:
            self.play(
                ...
                ChangeDecimalToValue(decimal, prod, position_update_func=lambda m: m.next_to(brace, DOWN)),
                ShowCreation(line),
        ...
        self.play(
            ...
            UpdateFromFunc(decimal, lambda d: d.shift(self.frame_duration * RIGHT)),
            UpdateFromAlphaFunc(decimal, lambda d, a: d.set_fill(opacity=1 - a)),
        )
        self.remove(decimal)
    ...
    def show_smaller_terms(self):
        ...
        for parts in larger_parts_mover, smaller_parts:
            parts.denominators = VGroup(parts[0][2], *[part[3] for part in parts[1:]])
        vect = op.sub(
        ...
        self.play(
            ...
            -vect,
            UpdateFromAlphaFunc(larger_parts_mover, lambda m, a: m.set_fill(opacity=1 - a), remover=True),
            UpdateFromAlphaFunc(smaller_parts, lambda m, a: m.set_fill(opacity=a)),
        )
    ...
    def interleave_terms(self):
        ...
        larger_parts.fade(0.5)
        full_product = VGroup(*it.chain(*list(zip(larger_parts.prefix.target, smaller_parts.prefix.target))))
    ...
    def show_answer(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(decimal, lambda m, a: m.set_fill(opacity=1 - a)),
...
class TeacherShowing(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            "raise_right_hand",
            screen,
            self.change_students(*["pondering"] * 3, look_at=screen),
        )
        self.wait(5)
...
class DistanceProductScene(MovingCameraScene):
    ...
    def get_observer_dot(self):
        self.observer_dot = Dot(self.get_observer_point(), color=self.observer_config["color"])
    ...
    def get_lights(self):
        self.lights = VGroup()
        for point in self.get_lh_points():
            light = AmbientLight(source_point=VectorizedPoint(point), **self.ambient_light_config)
    ...
    def get_distance_lines(self, start_point=None, line_class=Line):
        ...
        lines = VGroup(*[line_class(start_point, point) for point in self.get_lh_points()])
    ...
    def get_distance_product_column(self, column_top=None, labels=None, fraction=None):
        ...
        self.distance_product_column = VGroup(stacked_labels, h_line, times, product_decimal)
    ...
    def get_distance_product(self, fraction=None):
        ...
        distances = [get_norm(point - observer_point) / radius for point in self.get_lh_points()]
    ...
    def show_distance_product_in_column(self, **kwargs):
        ...
        self.play(ReplacementTransform(stacked_labels.copy(), VGroup(product_decimal)))
...
class IntroduceDistanceProduct(DistanceProductScene):
    ...
    def draw_circle_with_points(self):
        ...
        lh_dots = self.lh_dots = VGroup(*[Dot(point) for point in self.get_lh_points()])
        lh_dot_arrows = VGroup(
            *[
                Arrow(
                    *[interpolate(circle.get_center(), dot.get_center(), a) for a in (0.6, 0.9)],
                    buff=0,
                )
                for dot in lh_dots
            ]
        ...
        special_dot_label.next_to(special_dot_arrow.get_start(), UP, SMALL_BUFF)
    ...
    def show_sum_of_inverse_squares(self):
        ...
        sum_of_inverse_squares = OldTex(
            *it.chain(*[["{1", "\\over", "(", "d_%d" % i, ")", "^2}", "+"] for i in range(len(lines))])
        )
        ...
        self.wait()
        self.play(ShowCreation(basel_cross), FadeOut(VGroup(arrow, useful_for, brace)))
        ...
        self.play(
            d_terms.shift,
            0.75 * d_terms.get_height() * UP,
            d_terms.set_color,
            PRODUCT_COLOR,
            light_rings.set_fill,
            PRODUCT_COLOR,
            *[FadeOut(mob) for mob in sum_of_inverse_squares if mob not in d_terms and mob not in plusses],
        )
        self.wait()
        self.play(
            FadeOut(plusses),
    ...
    def transition_to_lemma_1(self):
        ...
        self.play(
            ...
            ApplyMethod(
                ...
                self.get_circle_point_at_proportion(self.observer_fraction / self.num_lighthouses),
            ),
...
class Lemma1(DistanceProductScene):
    ...
    def add_circle_group(self):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*circle_group.family_members_with_points())))

    def state_lemma_premise(self):
        premise = OldTexText("Lemma 1: If observer is halfway between lighthouses,")
    ...
    def wiggle_observer(self):
        ...
        def update_dot(dot):
            dot.move_to(self.get_circle_point_at_proportion(observer_angle_tracker.get_value() / TAU))
        ...
        self.play(
            ApplyMethod(
                observer_angle_tracker.set_value,
...
class Lemma2(Lemma1):
    ...
    def state_lemma_premise(self):
        premise = self.premise = OldTexText("Lemma 2: If the observer replaces a lighthouse,")
        premise.next_to(self.title, DOWN)
...
class FromGeometryToAlgebra(DistanceProductScene):
    ...
    def point_out_evenly_spaced(self):
        ...
        arcs = VGroup(
            *[VMobject().pointwise_become_partial(circle, a1, a2) for a1, a2 in zip(alpha_range, alpha_range[1:])]
        )
        arcs.set_stroke(YELLOW, 5)
        ...
        geometric_words = self.geometric_words = OldTexText("Geometric property")
        geometric_words.to_edge(UP)
        geometric_words.add_background_rectangle()

        self.add(geometric_words)
        self.play(FadeIn(spacing_words), arcs_anim, *list(map(GrowArrow, arrows)))
    ...
    def transition_to_complex_plane(self):
        ...
        self.wait(2)
        self.play(
            ChangeDecimalToValue(complex_decimal, values[1], tracked_mobject=complex_decimal_rect),
        ...
        self.wait()
        self.play(
            complex_plane_words.move_to,
            word_group,
            LaggedStartMap(FadeOut, VGroup(*it.chain(arrows, roots_of_unity_words))),
        )
    ...
    def discuss_powers(self):
        ...
        angle_arcs = VGroup(
            *[Arc(angle=alpha * TAU, radius=0.35).shift(origin) for alpha in np.arange(0, 1 + step, step)]
        )
        angle_labels = VGroup()
        ...
        for i, arrow in enumerate(outer_arrows[:-1]):
            label = OldTex("x^%d" % i)
            label.next_to(arrow.get_start(), -arrow.get_vector(), submobject_to_align=label[0])
            label.add_background_rectangle()
        ...
        L_labels = self.L_labels = VGroup(
            *[
                OldTex("L_%d" % i).move_to(power_label, DOWN).add_background_rectangle(opacity=1)
        ...
        self.wait()
        self.play(Rotate(self.lights, TAU / self.num_lighthouses / 2), rate_func=wiggle)
        self.wait()
        self.play(
            FadeOut(angle_arc),
            FadeOut(angle_label),
    ...
    def raise_everything_to_the_nth(self):
        ...
        factored_polynomial = OldTex("(x-L_0)(x-L_1)\\cdots(x-L_{%d - 1})" % self.num_lighthouses, "=", "0")
        ...
        for i, rotation in enumerate(rotations[:4]):
            if i == 3:
                ...
                self.play(FadeIn(polynomial), ShowCreationThenDestruction(rect))
...
class PlugObserverIntoPolynomial(DistanceProductScene):
    ...
    def add_circle_group(self):
        ...
        self.circle.set_width(2 * get_norm(self.plane.number_to_point(1) - self.origin))
    ...
    def label_roots(self):
        origin = self.origin
        labels = VGroup(*[OldTex("L_%d" % d) for d in range(self.num_lighthouses)])
    ...
    def introduce_observer(self):
        ...
        lines_update = self.lines_update = UpdateFromFunc(lines, lambda l: Transform(l, self.get_lines()).update(1))
        ...
        self.play(FadeIn(observer, direction=-vect), GrowArrow(arrow))
    ...
    def raise_observer_to_the_N(self):
        ...
        question = OldTexText("What fraction \\\\", "between $L_0$ and $L_1$", "?", arg_separator="")
        ...
        for angle in [full_angle - obs_angle, -full_angle, obs_angle]:
            ...
            self.play(
                ...
                UpdateFromAlphaFunc(
                    arc,
                    lambda arc, a: Transform(arc, get_arc(last_angle + a * angle)).update(1),
                ),
        ...
        self.play(Rotate(dot, 0.05 * TAU, about_point=origin, rate_func=wiggle))
        self.wait(2)
        ...
        dot.generate_target()
        dot.target.move_to(self.get_circle_point_at_proportion(obs_angle / TAU / 2))
        dot.save_state()
    ...
    def get_lines(self, start_point=None):
        return self.get_distance_lines(start_point=start_point, line_class=DashedLine)
...
class DefineChordF(Scene):
    def construct(self):
        ...
        full_chord_f = OldTexText("``", "Chord(", "$f$", ")", "''", arg_separator="")
        ...
        chord = Line(*[circle.point_from_proportion(f) for f in [0, 1.0 / 3]])
        ...
        third_arcs = VGroup(
            *[VMobject().pointwise_become_partial(circle, a1, a2) for a1, a2 in zip(alphas, alphas[1:])]
        )
        ...
        self.play(MoveToTarget(chord_f), FadeOut(VGroup(full_chord_f[0], full_chord_f[-1])))
        ...
        self.wait()
        self.play(ReplacementTransform(formula[0].copy(), formula[1], path_arc=45 * DEGREES))
        self.wait()
...
class DistanceProductIsChordF(PlugObserverIntoPolynomial):
    ...
    def show_all_animations(self):
        ...
        colum_group = self.get_distance_product_column(column_top=black_rect.get_top() + MED_SMALL_BUFF * DOWN)
        stacked_labels, h_line, times, product_decimal = colum_group

        chord = Line(*[self.get_circle_point_at_proportion(f) for f in (0, fraction)])
        ...
        half_terms = VGroup(*[OldTex("\\frac{1}{2}").scale(0.6).set_color(YELLOW).move_to(f) for f in f_terms])
        ...
        for decimal, line in zip(stacked_labels, new_lines):
            changing_decimals.append(ChangeDecimalToValue(decimal, line.get_length() / radius))

        equals_two_terms = VGroup(
            *[OldTex("=2").next_to(mob, DOWN, SMALL_BUFF) for mob in (chord_half, chord_f_as_product.target)]
        )
...
class ProveLemma2(PlugObserverIntoPolynomial):
    ...
    def replace_first_lighthouse(self):
        ...
        labels = self.get_numeric_distance_labels()
        column_group = self.get_distance_product_column(black_rect.get_top() + MED_SMALL_BUFF * DOWN)
        ...
        zero_rects = VGroup(*list(map(SurroundingRectangle, [dot, stacked_labels[0]])))
        ...
        self.wait(2)
        self.play(
            VGroup(light_to_remove, zero_rects[0]).shift,
            FRAME_WIDTH * RIGHT / 2,
            path_arc=-60 * DEGREES,
            rate_func=running_start,
            remover=True,
    ...
    def rearrange_polynomial(self):
        ...
        roots_of_unity_circle = VGroup(
            *[Circle(radius=0.2, color=YELLOW).move_to(point) for point in self.get_lh_points()]
        )
        ...
        self.play(LaggedStartMap(ApplyMethod, roots_of_unity_circle, lambda m: (m.restore,)))
        self.wait()
        ...
        # Expand rhs
        expanded_rhs = self.expanded_rhs = OldTex("=", "1", "+", "O", "+", "O", "^2", "+", "\\cdots", "O", "^{N-1}")
    ...
    def plug_in_one(self):
        ...
        ones = VGroup(*[OldTex("1").move_to(O_term, RIGHT) for O_term in O_terms])
        ...
        product = DecimalNumber(self.num_lighthouses, num_decimal_places=3, show_ellipsis=True)
    ...
    def construct(self):
        ...
        for circle, f, dp in (circle1, 0.5, "2"), (circle2, 0, "N"):
            ...
            self.play(
                ...
                LaggedStartMap(FadeIn, VGroup(*it.chain(group.family_members_with_points()))),
                self.teacher.change,
                "raise_right_hand",
        ...
        self.play(
            ...
            self.teacher.change_mode,
            "hooray",
            self.change_students(*["thinking"] * 3, look_at=wallis_product),
        )
        self.wait(2)
...
class KeeperAndSailor(DistanceProductScene, PiCreatureScene):
    ...
    def place_lighthouses(self):
        ...
        origin = circle.get_center()
        arrows = VGroup(
            *[Arrow(0.6 * (p - origin), 0.9 * (p - origin), buff=0).shift(origin) for p in self.get_lh_points()]
        )
    ...
    def write_distance_product_fraction(self):
        ...
        keeper_lines = self.get_distance_lines(self.get_keeper_point(), line_class=DashedLine)
        sailor_lines = self.get_distance_lines(self.get_sailor_point(), line_class=DashedLine)
        ...
        sailor_dp_column, keeper_dp_column = [
            self.get_distance_product_column(4 * RIGHT + 1.5 * UP, labels, frac)
            for labels, frac in [
        ...
        self.play(ReplacementTransform(keeper_lines.copy(), VGroup(fraction[0])))
        ...
        self.play(ReplacementTransform(sailor_line_lengths.copy(), sailor_dp_column[0]))
        ...
        self.play(ReplacementTransform(keeper_line_lengths.copy(), keeper_dp_column[0]))
        self.play(FadeIn(keeper_dp_column[1:]))
        self.wait()
        self.play(ShowCreation(keeper_dp_decimal_rect), ShowCreation(keeper_top_zero_rect))
        self.wait(2)
        ...
        self.play(Rotate(sailor_line_lengths[0], 30 * DEGREES, rate_func=wiggle))
        self.wait()
        self.play(
    ...
    def break_down_distance_product_by_parts(self):
        ...
        self.play(
            ...
            LaggedStartMap(FadeIn, sailor_lines, rate_func=there_and_back, remover=True),
            LaggedStartMap(FadeIn, keeper_lines, rate_func=there_and_back, remover=True),
        )
    ...
    def grow_circle_and_N(self, circle_scale_factor=2, N_multiple=3, added_anims=None):
        ...
        anims += [MoveToTarget(part) for pi in self.observers for part in [pi, pi.dot, pi.title]]
    ...
    def show_limit_for_each_fraction(self):
        ...
        def show_braces(n):
            ...
            for group in cw_group, VGroup(braces[0], *cw_group):
                self.play(group.scale, 0.95, {"about_point": center}, rate_func=there_and_back)
                self.wait(0.5)
    ...
    def show_limit_of_lhs(self):
        ...
        pi_halves_arrow = Arrow(result_rect.get_bottom(), pi_halves.get_top(), color=WHITE, buff=SMALL_BUFF)
        ...
        self.wait()
        self.play(
            ReplacementTransform(half_two_pi_over_N[-3:].copy(), pi_over_N[1:4]),
            FadeIn(pi_over_N[0]),
            FadeIn(pi_over_N[-1]),
            N.shift,
            N_shift * RIGHT,
        ...
        final_rect = SurroundingRectangle(VGroup(wallis_product, pi_halves), buff=MED_SMALL_BUFF)
    ...
    def let_N_approach_infinity(self, braces=None, factor=4, run_time=5, zoom_in_after=False):
        ...
        def update_lights(lights):
            for k in range(-half_N, half_N + 1):
                if k == 0:
                    continue
                light = lights[k]
                light = light.set_width((get_fraction() / start_fraction) * initial_light_width)
        ...
        self.play(fraction_tracker.set_value, target_fraction, *all_updates, run_time=run_time)
        if zoom_in_after:
            self.play(
                circle.scale,
                factor,
                {"about_point": circle.get_right()},
                *all_updates,
                run_time=1,
            )
            self.wait()
    ...
    def get_term_rects(self, terms):
        return VGroup(*[SurroundingRectangle(term, color=WHITE) for term in terms])
    ...
    def get_all_circle_braces(self):
        ...
        return VGroup(*[self.get_circle_brace(f1, f2) for f1, f2 in zip(fractions, fractions[1:])])
...
class HowThisArgumentRequiresCommunitingLimits(PiCreatureScene):
    def construct(self):
        ...
        equals = OldTex("=")
        result = OldTex("{N", "{\\text{distance} \\choose \\text{between obs.}}", "\\over", "2}")
        ...
        fractions = VGroup(*[factors[i : i + 3] for i in range(0, len(factors), 4)])
        fraction_limit_arrows = VGroup(*[Vector(0.5 * DOWN).next_to(fraction, DOWN) for fraction in fractions])
        fraction_limit_arrows.set_color(WHITE)

        wallis_product = get_wallis_product(6)
        fraction_limits = VGroup(*[wallis_product[i : i + 3] for i in range(0, 4 * 6, 4)])
...
class NonCommunitingLimitsExample(Scene):
    ...
    def construct(self):
        rows = VGroup(*[self.get_row(seven_index) for seven_index in range(self.num_rows)])
        ...
        columns = VGroup(*[VGroup(*[row[0][i] for row in rows]) for i in range(len(rows[0][0]))])
        column_rects = VGroup(*list(map(SurroundingRectangle, columns)))
        column_rects.set_color(BLUE)

        row_arrows = VGroup(*[OldTex("\\rightarrow").next_to(row, RIGHT) for row in rows[:-1]])
        row_products = VGroup(*[Integer(7).next_to(arrow) for arrow in row_arrows])
        ...
        column_arrows = VGroup(*[OldTex("\\downarrow").next_to(part, DOWN, SMALL_BUFF) for part in rows[-1][0]])
        column_limits = VGroup(*[Integer(1).next_to(arrow, DOWN) for arrow in column_arrows])
        column_limits.set_color(BLUE)
        column_limit_dots = self.get_row_dots(column_limits)

        column_limits_arrow = OldTex("\\rightarrow").next_to(column_limit_dots, RIGHT)
    ...
    def get_v_dot_row(self):
        row = VGroup(*[OldTex("\\vdots") for x in range(self.num_terms_per_row)])
        self.arrange_row(row)
        dots = self.get_row_dots(row)
        dots.fade(1)
        return VGroup(row, dots)
    ...
    def construct(self):
        ...
        top_product, bottom_product = products = VGroup(
            *[
                OldTex(
                    *it.chain(
                        *[["{%d" % (2 * x), "\\over", "%d}" % (2 * x + u), "\\cdot"] for x in range(1, n_terms + 1)]
                    )
                )
                for u in (-1, 1)
            ]
        ...
        for product in products:
            ...
            product.parts = VGroup(*[product[i : i + 4] for i in range(0, len(product), 4)])
        ...
        self.wait()
        for product in products:
            self.play(GrowArrow(product.arrow), FadeIn(product.limit, direction=LEFT))
            self.wait()
        ...
        movers2 = VGroup(*it.chain(*list(zip(top_product.parts, bottom_product.parts))))
...
class KeeperAndSailorForSineProduct(KeeperAndSailor):
    ...
    def move_sailor_to_new_spot(self):
        ...
        self.get_sailor_point = lambda: self.get_circle_point_at_proportion(fraction / self.num_lighthouses)
    ...
    def show_new_distance_product(self):
        ...
        sailor_lines = self.get_distance_lines(self.sailor.dot.get_center(), line_class=DashedLine)
        ...
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(sailor_lines, sailor_dp_rect, two_group)))
    ...
    def show_new_limit_of_ratio(self):
        ...
        frac1 = OldTex("{N", "(", "f", " \\cdot 2\\pi / N)", "\\over", "\\text{Chord}(", "f", ")}")
        frac2 = OldTex("{f", "\\cdot", "2", "\\pi", "\\over", "\\text{Chord}(", "f", ")}")
    ...
    def show_new_infinite_product(self):
        ...
        ratios = VGroup(*[fractions[i : i + 3] for i in range(1, len(fractions), 4)])
        limit_arrows = VGroup(*[Vector(0.5 * DOWN, color=WHITE).next_to(ratio, DOWN, SMALL_BUFF) for ratio in ratios])
        limits = VGroup(*[OldTex("{%d" % k, "\\over", "%d" % k, "-", "f}") for k in (-2, -1, 1, 2, 3)])
        ...
        self.play(
            ...
            product_lines[1].scale,
            0.9,
            {"about_point": product_lines[1].get_start()},
            product_lines[1].shift,
            SMALL_BUFF * UP,
        )
        self.wait()

        # Rearrange
        one_minus_f_over_k = OldTex("\\left(", "1", "-", "{f", "\\over", "k}", "\\right)")
        # 0 1 2 3 4
        # k / k - f
