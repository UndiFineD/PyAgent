...
class WhyLearnIt(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change,
            "raise_right_hand",
            self.change_students("erm", "happypondering"),
...
class SetupSimpleSystemOfEquations(LinearTransformationScene):
    ...
    def introduce_system(self):
        ...
        if self.compare_to_big_system:
            ...
            cutoff = 3 * dim - 1
            self.play(
                *[ReplacementTransform(big_system[i][:cutoff], system[i][:cutoff]) for i in range(dim)]
                + [ReplacementTransform(big_system[i][-2:], system[i][-2:]) for i in range(dim)]
                + [
...
class ThinkOfPuzzleAsLinearCombination(SetupSimpleSystemOfEquations):
    ...
    def rearrange_equation_as_linear_combination(self):
        ...
        self.play(
            ...
            *[MoveToTarget(mover, replace_mobject_with_target_in_scene=True) for mover in movers],
            path_arc=90 * DEGREES,
            run_time=2,
        )
    ...
    def show_dot_products(self):
        ...
        for equation in moving_equations:
            ...
            transformed_basis_rects.add(
                SurroundingRectangle(transformed_equation[5 : 5 + 4], color=basis.elements.get_color())
            )
            ...
            it.chain(
                *[
                    [
                        ...
                        ReplacementTransform(matrix_mobject.brackets.copy(), cm.brackets, **get_kwargs(i)),
                        ReplacementTransform(
...
class TransformingAreasYCoord(LinearTransformationScene):
    ...
    def show_coord_parallelogram(self):
        ...
        self.play(
            UpdateFromFunc(
                ip,
                lambda m: Transform(m, self.get_input_parallelogram(input_vect_mob)).update(1),
            ),
    ...
    def transform_space(self):
        ...
        self.apply_matrix(
            ...
            added_anims=[
                UpdateFromFunc(
                    area_arrow,
                    lambda a: a.put_start_and_end_on(area_arrow.get_start(), ip.get_center() + SMALL_BUFF * DL),
                ),
...
class AreYouPausingAndPondering(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_all_student_changes(
            "thinking",
            added_anims=[RemovePiCreatureBubble(self.teacher, target_mode="raise_right_hand")],
            look_at=self.screen,
        )
        self.wait(6)
