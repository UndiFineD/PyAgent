# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\WindingNumber_G.py
# -*- coding: utf-8 -*-
...
class IntroSceneWrapper(PiCreatureScene):
    ...
    def introduce_two_words(self):
        ...
        self.play(morty.change, "raise_left_hand", FadeInFromDown(main_topic))
        self.wait()
        self.play(
            morty.change,
            "raise_right_hand",
    ...
    def describe_main_topic(self):
        ...
        self.play(LaggedStartMap(FadeIn, xy_equation, run_time=1), morty.change, "pondering")
        self.wait(2)
        self.play(FadeOut(xy_equation), FadeIn(z_equation))
        self.wait()
        self.play(Write(zeta))
        self.wait()
        solver.save_state()
        for rainbow_solver in rainbow_solver1, rainbow_solver2:
            self.play(Transform(solver, rainbow_solver, run_time=2, lag_ratio=0.5))
        self.play(solver.restore)
        self.wait()

        self.play(LaggedStartMap(FadeOut, VGroup(solver, z_equation, zeta)))
        self.play(
            main_topic.move_to,
            meta_topic,
            main_topic.set_fill,
...
class Introduce1DFunctionCase(Scene):
    ...
    def show_axes_one_at_a_time(self):
        ...
        self.play(ShowCreation(axes.x_axis), Write(inputs))
        self.wait()
        self.play(
    ...
    def show_two_graphs(self):
        axes = self.axes
        f_graph = axes.get_graph(lambda x: 2 * x * (x - 0.75) * (x - 1.5) + 1, color=BLUE)
        g_graph = axes.get_graph(lambda x: 1.8 * np.cos(TAU * x / 2), color=YELLOW)
        ...
        g_label.next_to(axes.input_to_graph_point(label_x_corod, g_graph), UP, SMALL_BUFF)
        ...
        equation_arrow = Arrow(equation.get_bottom(), cross_point, buff=SMALL_BUFF, color=WHITE)
        ...
        self.set_variables_as_attrs(f_graph, f_label, g_graph, g_label, equation=equation.target)
    ...
    def transition_to_sqrt_2_case(self):
        ...
        squared_label.next_to(axes.input_to_graph_point(2, squared_graph), RIGHT)
        squared_label.match_color(squared_graph)
        two_label = OldTex("g(x)", "=", "2")
        two_label.next_to(
            axes.input_to_graph_point(3, two_graph),
            UP,
        )
    ...
    def show_example_binary_search(self):
        self.binary_search(self.squared_graph, self.two_graph, x0=1, x1=2, n_iterations=8)
    ...
    def binary_search(
        ...
        midpoint_line_update = UpdateFromFunc(midpoint_line, lambda l: l.move_to(rect))
        ...
        decimal_update = ChangingDecimal(
            decimal,
            lambda a: axes.x_axis.point_to_number(rect.get_center()),
        ...
        for x in range(n_iterations - 1):
            ...
            if zoom:
                ...
                everything.target[decimal_index].scale(1.0 / factor, about_edge=UP)
                everything.target[midpoint_line_index].scale(1.0 / factor)
                if factor > 1:
                    self.play(everything.scale, factor, {"about_point": rect.get_center()})
            else:
                self.wait()
    ...
    def compare_graphs_at_x(
        ...
        arrow = Arrow(g_point, f_point, buff=0, **self.arrow_config)
...
class PiCreaturesAreIntrigued(AltTeacherStudentsScene):
    def construct(self):
        self.teacher_says("You can extend \\\\ this to 2d", bubble_config={"width": 4, "height": 3})
...
class TransitionFromEquationSolverToZeroFinder(Introduce1DFunctionCase):
    ...
    def transition_to_difference_graph(self):
        ...
        new_label.next_to(axes.input_to_graph_point(2, new_graph), LEFT)
...
class RewriteEquationWithTeacher(AltTeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(Transform(*root_two_equations, run_time=1.5, path_arc=TAU / 2))
        self.play(self.change_students(*["pondering"] * 3))
        self.play(GrowFromCenter(brace), self.teacher.change, "happy")
        ...
        z_def = OldTexText("(", "$z$", " is complex, ", "$a + bi$", ")", arg_separator="")
...
class InputOutputScene(Scene):
    ...
    def get_colorings(self):
        in_cmos = ColorMappedObjectsScene(
            func=lambda p: self.non_renormalized_func((p[0] + self.x_shift, p[1] + self.y_shift))
        )
        scalar = self.output_scalar
        out_cmos = ColorMappedObjectsScene(
            func=lambda p: (
                scalar * (p[0] - self.x_shift),
                scalar * (p[1] + self.y_shift),
            )
        )
    ...
    def get_dots(self, input_plane, output_plane):
        ...
        for x in np.arange(x_min, x_max + step, step):
            for y in np.arange(y_max, y_min - step, -step):
                ...
                dot.target_color = rgba_to_color(point_to_rgba(tuple(self.output_scalar * np.array(out_coords))))
                dots.add(dot)
        return dots
...
class IntroduceInputOutputScene(InputOutputScene):
    ...
    def map_single_point_to_point(self):
        ...
        self.play(LaggedStartMap(MoveToTarget, dots, path_arc=path_arc))
        self.wait()
        self.play(FadeOut(dots))
        self.play(GrowFromCenter(in_dot), GrowArrow(input_label_arrow), FadeIn(input_label))
        self.wait()
        self.play(
            ShowCreation(curved_arrow),
            ReplacementTransform(in_dot.copy(), out_dot, path_arc=path_arc),
            FadeIn(function_label),
        )
        self.play(GrowArrow(output_label_arrow), FadeIn(output_label))
        self.wait()
        self.play(
            *list(
                map(
        ...
        for vect in UP, RIGHT:
            self.play(in_dot.shift, 0.25 * vect, rate_func=lambda t: wiggle(t, 8), run_time=2)
        for vect in compass_directions(4, UP + RIGHT):
            self.play(Rotating(in_dot, about_point=in_dot.get_corner(vect), radians=TAU, run_time=1))
        self.wait()
        for coords in (-2, 2), (-2, -2), (2, -2), (1.5, 1.5):
            self.play(
                in_dot.move_to,
                input_plane.coords_to_point(*coords),
                path_arc=-TAU / 4,
                run_time=2,
            )
        self.wait()
...
class IntroduceVectorField(IntroduceInputOutputScene):
    ...
    def construct(self):
        ...
        continual_in_vector_update = Mobject.add_updater(in_vector, update_in_vector)
        continual_updates = [
        ...
        for dot in dots:
            self.play(in_dot.move_to, dot, run_time=1.0 / 15)
            newer_in_vectors.add(in_vector.copy())
        self.remove(*continual_updates)
        self.remove()
        self.play(*list(map(FadeOut, [out_dot, out_vector, in_vectors, in_dot, in_vector])))
        self.wait()
        target_length = 0.4
...
class TwoDScreenInOurThreeDWorld(AltTeacherStudentsScene, ThreeDScene):
    ...
    def ask_about_2d_functions(self):
        ...
        arrow = Arrow(LEFT, RIGHT, path_arc=-TAU / 4, color=WHITE)
        ...
        for x in np.arange(-2.5, 3.0, 0.5):
            for y in np.arange(-2.5, 3.0, 0.5):
                ...
                dot.generate_target()
                dot.target.move_to(out_plane.coords_to_point(x + 0.25 * np.cos(5 * y), y + 0.25 * np.sin(3 * x)))
                dots.add(dot)
                dots_target.add(dot.target)
        dots.set_color_by_gradient(YELLOW, RED)
        dots_target.set_color_by_gradient(YELLOW, RED)

        self.play(self.teacher.change, "raise_right_hand", Write(in_plane, run_time=1))
        self.play(
            ShowCreation(arrow),
...
class DotsHoppingToColor(InputOutputScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            output_plane.white_parts.set_color,
            BLACK,
            output_plane.lines_to_fade.set_stroke,
            {"width": 0},
        )
        self.wait()
        ...
        self.play(
            ...
            input_plane.white_parts.set_color,
            BLACK,
            input_plane.lines_to_fade.set_stroke,
            {"width": 0},
            FadeOut(dots),
        )
        self.wait()
        ...
        inspector_image_update_anim = UpdateFromFunc(inspector_image, update_inspector_image)
        ...
        self.play(
            Rotating(
                inspector,
                about_point=inspector.get_center(),
                rate_func=smooth,
                run_time=2,
            ),
        ...
        for x, y, width, height in quads:
            self.play(
                square.stretch_to_fit_width,
                width,
                square.stretch_to_fit_height,
                height,
                square.move_to,
                input_plane.coords_to_point(x, y),
            )
            self.wait()
...
class SoWeFoundTheZeros(AltTeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.teacher_says("Er...only \\\\ kind of", target_mode="hesitant")
        self.wait(3)


class Rearrange2DEquation(AltTeacherStudentsScene):
    def construct(self):
        f_tex, g_tex, h_tex = ["%s(\\text{2d point})" % char for char in ("f", "g", "h")]
        ...
        self.play(Transform(*equations, run_time=1.5, path_arc=TAU / 2))
        self.play(
            Succession(GrowFromCenter(brace), Write(zero_eq, run_time=1)),
            self.change_students(*["happy"] * 3),
        )
        self.play(*[ApplyMethod(pi.change, "thinking", self.screen) for pi in self.pi_creatures])
        self.wait(3)
...
class OneDRegionBoundary(Scene):
    ...
    def construct(self):
        ...
        self.play(GrowFromCenter(region), FadeIn(region_words))
        self.wait()
        self.play(*it.chain(list(map(GrowArrow, arrows)), list(map(Write, signs))))
        self.wait()
        self.play(
            ShowCreation(graph),
    ...
    def get_graph_part(self, *interim_values):
        ...
        result.set_points_smoothly(
            [self.axes.coords_to_point(x, fx) for x, fx in zip(np.linspace(self.x0, self.x1, len(values)), values)]
        )
        return result
...
class DirectionOfA2DFunctionAlongABoundary(InputOutputScene):
    def construct(self):
        ...
        out_vect_update_anim = UpdateFromFunc(
            out_vect,
            lambda ov: ov.put_start_and_end_on(
                output_plane.coords_to_point(0, 0), rect_image.get_points()[-1]
            ).set_color(get_color()),
        )

        dot = Dot()
        dot.set_stroke(BLACK, 1)
        dot_update_anim = UpdateFromFunc(dot, lambda d: d.move_to(get_input_point()).set_fill(get_color()))
...
class HypothesisAboutFullyColoredBoundary(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        hypothesis = OldTexText(
            "Working Hypothesis: \\\\",
            "If a 2d function hits outputs of all possible colors \\\\" + "on the boundary of a 2d region,",
        ...
        self.play(LaggedStartMap(FadeIn, hypothesis[1]), ShowCreation(square, run_time=8))
...
class ForeverNarrowingLoop(InputOutputScene):
    ...
    def construct(self):
        ...
        self.play(
            *it.chain(
                [ApplyMethod(coloring.set_fill, {"opacity": 0.2}) for coloring in colorings],
                [ApplyMethod(plane.white_parts.set_color, WHITE) for plane in planes],
            ),
            run_time=2,
        )
        ...
        circle_image_update_anim = UpdateFromFunc(circle_image, update_circle_image)
        ...
        self.play(
            ...
            0,
            circle.move_to,
            input_plane.coords_to_point(*self.target_coords),
...
class FailureOfComposition(ColorMappedObjectsScene):
    CONFIG = {"func": lambda p: (np.cos(TAU * p[1] / 3.5), np.sin(TAU * p[1] / 3.5))}
    ...
    def construct(self):
        ...
        small_squares = VGroup(*[Square(side_length=2) for x in range(2)])
        ...
        equation = VGroup(
            no_answers_in_equation[0],
            plus,
            no_answers_in_equation[1],
            equals,
        ...
        self.wait()
        self.play(
            small_squares.arrange,
            DOWN,
            {"buff": 0},
            small_squares.move_to,
            big_square,
            no_answers.space_out_submobjects,
            0.9,
        )
        self.add(big_square)
        ...
        for square, char in zip(small_squares, [x, y]):
            ...
            self.play(small_squares.restore, ShowPassingFlash(ghost), Write(char))
        self.wait()
...
class PathContainingZero(InputOutputScene, PiCreatureScene):
    ...
    def draw_path_hitting_zero(self):
        ...
        self.play(
            ...
            morty.change,
            "pondering",
            *[ShowCreation(mob, rate_func=bezier([0, 0, 1, 1])) for mob in (path, out_path)],
            run_time=5,
        )
    ...
    def comment_on_zero(self):
        ...
        words = OldTexText("Output is zero \\\\", "which has no direction")
        origin = self.output_plane.coords_to_point(0, 0)
        words.to_edge(DOWN, buff=LARGE_BUFF)
        background_rect = BackgroundRectangle(words, buff=SMALL_BUFF, opacity=1.0)
        ...
        self.play(FadeIn(new_words), morty.change, "happy")
...
class TransitionFromPathsToBoundaries(ColorMappedObjectsScene):
    CONFIG = {
        "func": plane_func_by_wind_spec((-2, 0, 2), (2, 0, 1)),
        "dot_fill_opacity": 1,
        "dot_stroke_width": 1,
    ...
    def construct(self):
        ...
        path1, path2 = paths = VGroup(
            *[Line(square.get_corner(UP + LEFT), square.get_corner(UP + RIGHT)) for square in squares]
        )
        ...
        for path in path1, path2:
            ...
            self.play(MoveAlongPath(dot, path.copy()), ShowCreation(path), run_time=2)
        ...
        # Add regions
        self.play(FadeOut(paths), FadeOut(joint_path.brace), dot.move_to, path1.get_start())
        ...
        self.play(
            ...
            Transform(left_square.label[1], joint_rect.label[1][0], path_arc=TAU / 6),
            FadeIn(joint_rect.label[1][1]),
            FadeIn(joint_rect.label[1][3:]),
            FadeOut(right_square.label[0]),
            Transform(right_square.label[1], joint_rect.label[1][2], path_arc=TAU / 6),
            MoveAlongPath(dot, joint_rect, run_time=6, rate_func=bezier([0, 0, 1, 1])),
        )
        self.wait()
    ...
    def get_squares_and_joint_rect(self):
        squares = VGroup(*[Square(side_length=4).next_to(ORIGIN, vect, buff=0) for vect in (LEFT, RIGHT)])
    ...
    def get_dot_and_add_continual_animations(self):
        ...
        def get_total_winding(dt=0):
            ...
            possible_windings = [np.floor(self.curr_winding) + k + rev for k in (-1, 0, 1)]
        ...
        update_dot_color = Mobject.add_updater(dot, lambda d: d.set_fill(get_output_color(), self.dot_fill_opacity))
...
class BreakDownLoopWithNonzeroWinding(TransitionFromPathsToBoundaries):
    def construct(self):
        ...
        arrows = VGroup(*[Arrow(both_cannot_be_zero.get_bottom(), var.get_top(), buff=SMALL_BUFF) for var in (x, y)])
        ...
        self.play(FadeIn(both_cannot_be_zero), *list(map(GrowArrow, arrows)))
        self.wait()


class BackToEquationSolving(AltTeacherStudentsScene):
    def construct(self):
        self.teacher_says("Back to solving \\\\ equations")
        self.play_all_student_changes("hooray")
        self.play(*[ApplyMethod(pi.look_at, self.screen) for pi in self.pi_creatures])
        self.wait(3)
...
class MonomialTerm(PathContainingZero):
    ...
    def show_winding(self):
        ...
        def generate_label_update(label, point_func, origin):
            return UpdateFromFunc(
                label,
                lambda m: m.move_to((1 + buff) * point_func() - buff * origin, self.label_move_to_corner),
            )
        ...
        self.play(FadeOut(VGroup(dot, out_dot, x, fx)))
        self.loop = loop
    ...
    def rescale_output_plane(self):
        ...
        self.play(output_stuff.scale, 3.0 / 50, run_time=2)
        self.wait()
...
class PolynomialTerms(MonomialTerm):
    ...
    def sweep_through_loop_interior(self):
        ...
        self.play(
            Transform(line, line_target, run_time=10, rate_func=there_and_back),
            update_out_line,
            morty.change,
            "hooray",
        )
    ...
    def cinch_loop(self):
        ...
        self.play(
            ApplyMethod(
                loop.scale,
                0,
                {"about_point": self.input_plane.coords_to_point(0.2, 1)},
                run_time=12,
                rate_func=bezier([0, 0, 1, 1]),
            ),
            update_out_loop,
...
class SearchSpacePerimeterVsArea(EquationSolver2d):
    ...
    def construct(self):
        ...
        alt_path_parts.submobjects.sort(key=lambda m1: -size(m1))
...
class PiCreatureInAwe(Scene):
    def construct(self):
        ...
        self.play(randy.look, RIGHT, run_time=4, rate_func=there_and_back, path_arc=-TAU / 4)
        self.wait()
...
class ShowComplexFunction(Scene):
    def construct(self):
        ...
        arrow = Arrow(x_point, fx_point, path_arc=TAU / 3, color=YELLOW)
        arrow.pointwise_become_partial(arrow, 0, 0.95)
        ...
        self.play(Write(plane), FadeIn(rect), LaggedStartMap(FadeIn, title))
...
class WindingNumbersInInputOutputContext(PathContainingZero):
    ...
    def construct(self):
        ...
        in_loop.move_to(self.input_plane.coords_to_point(*self.in_loop_center_coords))
        ...
        update_in_dot = Mobject.add_updater(in_dot, lambda d: d.move_to(in_loop.point_from_proportion(1)))
        ...
        update_out_arrow_color = Mobject.add_updater(
            out_arrow, lambda a: a.set_color(rev_to_color(a.get_angle() / TAU))
        )
        self.add(update_out_arrow, update_out_arrow_color)

        words = OldTexText("How many times does \\\\ the output wind around?")
...
class AllOfTheVideos(Scene):
    ...
    def construct(self):
        ...
        n = 4
        images = Group(*[ImageMobject(os.path.join(thumbnail_dir, file)) for file in os.listdir(thumbnail_dir)[: n**2]])
        for image in images:
...
class EndingCredits(Scene):
    def construct(self):
        text = OldTexText("Written and animated by: \\\\", "Sridhar Ramesh \\\\", "Grant Sanderson")
...
class MentionFree(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(morty.change, "raise_right_hand", FadeInFromDown(items[0]))
        self.wait()
        self.play(
        ...
        self.wait()
        self.play(items[:2].shift, UP, FadeInFromDown(items[2]), morty.change, "surprised")
        self.wait(4)
        self.play(morty.change, "raise_left_hand", VectorizedPoint(3 * LEFT))
        self.wait(4)
...
class Thumbnail(SearchSpacePerimeterVsArea):
    CONFIG = {
        "num_iterations": 18,
        "func": plane_func_by_wind_spec((-3, -1.3, 2), (0.1, 0.2, 1), (2.8, -2, 1)),
    }
