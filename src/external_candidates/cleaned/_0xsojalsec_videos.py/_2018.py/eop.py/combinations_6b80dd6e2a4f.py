...
class ExperienceProblemSolver(PiCreatureScene):
    ...
    def think_about_patterns(self):
        ...
        for last_row, curr_row in zip(rows, rows[1:]):
            self.play(
                *[Transform(last_row.copy(), VGroup(*mobs), remover=True) for mobs in (curr_row[1:], curr_row[:-1])]
            )
            self.add(curr_row)
        self.wait(3)
...
class ProbabilityOfKWomenInGroupOfFive(Scene):
    ...
    def ask_question(self):
        ...
        prob_words = VGroup(
            *[
                OldTexText("Probability of", str(n), "women?").set_color_by_tex(str(n), YELLOW)
    ...
    def show_all_possibilities(self):
        ...
        two_anims = [ReplacementTransform(VectorizedPoint(twos[0].get_center()), twos[0])] + [
    ...
    def get_random_lineup_of_men_and_women(self):
        ...
        lineup = self.get_lineup(
            *[woman if random.choice([True, False]) else man for y in range(self.n_people_per_lineup)]
        )
        return lineup
            ...
            for stack in stacks:
                ...
                for lineup in stack:
                    new_lineup = self.get_lineup(
                        *[Female() if "female" in item.get_tex() else Male() for item in lineup.items] + [sym],
                        buff=SMALL_BUFF,
                    )
...
class ChooseThreeFromFive(InitialFiveChooseThreeExample, PiCreatureScene):
    ...
    def pattern_is_unambiguous(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, all_ones, lag_ratio=0.2, run_time=3, rate_func=there_and_back),
            LaggedStartMap(
                ApplyMethod,
                self.people,
...
class HowToComputeNChooseK(ChooseThreeFromFive):
    ...
    def get_choice_counts(self, n, k):
        ...
        choice_counts = VGroup(
            *[OldTexText("(", str(n0), " choices", ")", arg_separator="") for n0 in range(n, n - k, -1)]
        )
    ...
    def get_lines_and_place_words(self):
        ...
        place_words = VGroup(
            *[
                ...
                for i, s in zip(list(range(k)), it.chain(["st", "nd", "rd"], it.repeat("th")))
            ]
        )
...
class NineChooseFourExample(HowToComputeNChooseK):
    ...
    def choose_k_people(self):
        ...
        self.play(
            FadeIn(choose_k),
            LaggedStartMap(ApplyMethod, chosen_subset, lambda m: (m.shift, MED_LARGE_BUFF * DOWN)),
        )
        self.wait()
        self.play(
            chosen_subset.shift,
