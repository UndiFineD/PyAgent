...
class BrickRowScene(PiCreatureScene):
    def split_tallies(self, row, direction=DOWN):
        ...
        tally_targets_left = [rect.get_center() + 0.25 * rect.get_width() * LEFT for rect in row.rects]

        tally_targets_right = [rect.get_center() + 0.25 * rect.get_width() * RIGHT for rect in row.rects]
    ...
    def construct(self):
        ...
        outcomes = self.row.get_outcome_rects_for_level(2, with_labels=False, inset=True)
        ...
        tally.next_to(rect, UP)
        decimal_tallies.add(tally)
        self.play(FadeIn(tally), FadeOut(grouped_outcomes[0]), FadeIn(grouped_outcomes[1]))
        self.wait()
        ...
        tally.next_to(rect, UP)
        decimal_tallies.add(tally)
        self.play(FadeIn(tally), FadeOut(grouped_outcomes[1]), FadeIn(grouped_outcomes[2]))
        self.wait()
        ...
        for decimal_tally, tally_stack in zip(decimal_tallies, self.tallies):
            anims.append(ApplyFunction(tally_stack.position_decimal_tally, decimal_tally))
        ...
        rect_to_dice = self.row.get_outcome_rects_for_level(2, with_labels=False, inset=False)[1]
        N = 10
        ...
        right_target_pos = 0.5 * (
            self.row.get_outcome_centers_for_level(3)[2] + self.row.get_outcome_centers_for_level(3)[3]
        )
        ...
        # show individual outcomes
        outcomes = previous_row.get_outcome_rects_for_level(n, with_labels=False, inset=True)
        ...
        target_outcomes = self.row.get_outcome_rects_for_level(n + 1, with_labels=False, inset=True)
        ...
        new_tallies = VGroup(*[TallyStack(n + 1 - i, i).move_to(rect) for (i, rect) in enumerate(new_rects)])
        ...
        self.play(Transform(grouped_outcomes_copy[k - 1], original_grouped_outcomes[k - 1]))
...
class ShowOutcomesInBrickRow4(BrickRowScene):
    def construct(self):
        ...
        self.wait()
        previous_outcomes = previous_row.get_outcome_rects_for_level(3, with_labels=True, inset=True)
        ...
        new_outcomes = self.row.get_outcome_rects_for_level(4, with_labels=True, inset=True)
