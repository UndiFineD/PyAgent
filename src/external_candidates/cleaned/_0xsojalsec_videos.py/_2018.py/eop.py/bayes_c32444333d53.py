...
class BayesOpeningQuote(OpeningQuote):
    CONFIG = {
        "quote": ["Inside every non-Bayesian there \\\\ is a Bayesian struggling to get out."],
...
class IntroducePokerHand(PiCreatureScene, SampleSpaceScene):
    ...
    def place_high_bet(self):
        you, her = self.you, self.her
        pre_money = VGroup(
            *[VGroup(*[OldTex("\\$") for x in range(10)]).arrange(RIGHT, buff=SMALL_BUFF) for y in range(4)]
...
class UpdatePokerPrior(SampleSpaceScene):
    ...
    def show_restricted_space(self):
        high_bet_space, low_bet_space = [
            VGroup(*[self.sample_space.horizontal_parts[i].vertical_parts[j] for i in range(2)]) for j in range(2)
        ]
...
class GeneralizeBayesRule(SampleSpaceScene):
    ...
    def show_space_restriction(self):
        ...
        self.play(
            *it.chain(
                self.get_division_change_animations(self.sample_space, self.sample_space.horizontal_parts, 0.1),
...
class MusicExample(SampleSpaceScene, PiCreatureScene):
    ...
    def friend_gives_compliment(self):
        ...
        self.play(
            ...
            LaggedStartMap(ApplyMethod, VGroup(*friends[1:]), lambda pi: (pi.change_mode, "happy")),
        )
        self.wait(2)
