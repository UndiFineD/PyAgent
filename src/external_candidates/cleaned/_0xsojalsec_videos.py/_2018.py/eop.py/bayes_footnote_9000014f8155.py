...
class OneInOneThousandHaveDisease(Scene):
    def construct(self):
        ...
        all_creatures = VGroup(
            *[VGroup(*[creature.copy() for y in range(25)]).arrange(DOWN, SMALL_BUFF) for x in range(40)]
...
class TestNonDiseaseCase(TestScene):
    def construct(self):
        ...
        all_creatures = VGroup(*[VGroup(*[randy.copy() for y in range(10)]).arrange(DOWN) for y in range(10)]).arrange(
            RIGHT
        )
...
class ShowRestrictedSpace(Scene):
    ...
    def get_all_creatures(self):
        ...
        all_creatures = VGroup(
            *[
                VGroup(*[creature.copy() for y in range(self.n_rows)]).arrange(DOWN, SMALL_BUFF)
...
class DepressingForMedicalTestDesigners(TestScene):
    ...
    def show_99_percent_accuracy(self):
        ...
        checks = VGroup(
            *[VGroup(*[OldTex("\\checkmark").set_color(GREEN) for y in range(10)]).arrange(DOWN) for x in range(10)]
        ).arrange(RIGHT)
        cross = OldTex("\\times")
...
class PlaneCrashProbability(Scene):
    def construct(self):
        ...
        plane_prob.shift(UP)
        car_prob.shift(
            plane_prob.get_part_by_tex("approx").get_center() - car_prob.get_part_by_tex("approx").get_center() + DOWN
        )
