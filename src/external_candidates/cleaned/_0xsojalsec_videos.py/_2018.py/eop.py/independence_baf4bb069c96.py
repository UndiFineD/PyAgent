...
class IntroduceQuiz(PiCreatureScene):
    ...
    def ask_about_probabilities(self):
        probabilities, abbreviated_probabilities = [
            VGroup(
                *[
                    OldTex("P(", s_tex, "=", str(score), ")", rhs).set_color_by_tex_to_color_map(
                        {
                            str(score): YELLOW,
                            "text": GREEN,
                        }
    ...
    def show_distribution(self):
        ...
        self.play(
            *[
                ReplacementTransform(bar.copy().stretch_to_fit_height(0).move_to(bar.get_bottom()), bar)
...
class ThousandPossibleQuizzes(Scene):
    ...
    def ask_about_second_question(self):
        ...
        sg1, sg2 = slot_groups = VGroup(
            *[get_slot_group([True, b], include_qs=False, buff=SMALL_BUFF) for b in (True, False)]
        )
        question = VGroup(
...
class ShowFullDistribution(Scene):
    ...
    def show_bar_chart(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn,
                VGroup(*it.chain(*[submob for submob in chart if submob is not chart.bars])),
            ),
...
class NameBinomial(Scene):
    ...
    def add_quiz_questions(self):
        ...
        flipped_arrows, faded_crosses, full_checks = [
            VGroup(*[group[i] for i in flip_indices]) for group in (arrows, crosses, checkmarks)
        ]
    ...
    def write_probability_of_girl(self):
        ...
        probability_copies = VGroup(
            *[probability.copy().scale(0.7).next_to(girl, LEFT, MED_LARGE_BUFF) for girl in self.girls]
        )
...
class GeneralBinomialDistributionValues(Scene):
    ...
    def mention_center_concentration(self):
        ...
        self.play(
            bars.restore,
            *list(map(FadeOut, [brace, words, self.pascals_triangle, self.ten_choose_ks])),
        )
    ...
    def emphasize_underweighted_tails(self):
        ...
        for arrows in poisson_arrows, gaussian_arrows:
            self.play(
                ...
                *[ApplyMethod(pi.change, "thinking", arrows) for pi in self.pi_creatures],
            )
            self.wait()
        self.wait(2)
