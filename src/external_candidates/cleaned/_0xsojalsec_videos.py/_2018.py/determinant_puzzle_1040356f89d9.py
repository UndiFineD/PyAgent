# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\determinant_puzzle.py
...
class WorkOutNumerically(Scene):
    ...
    def add_example(self):
        ...
        for part in eq_parts.get_parts_by_tex("\\big"):
            part.scale(2)
            part.stretch(1.5, 1)
        i1, i2, i3 = [eq_parts.index_of_part(part) + 1 for part in eq_parts.get_parts_by_tex("\\big(")]
        equation = self.equation_with_numbers = VGroup(
            *it.chain(
                eq_parts[:i1],
                [M1, M2],
                eq_parts[i1:i2],
    ...
    def compute_rhs(self):
        ...
        line1 = VGroup(
            OldTex("\\big(", "2", "\\cdot", "2", "-", "(-1)", "\\cdot", "1", "\\big)"),
            OldTex("\\big(", "-1", "\\cdot", "1", "-", "4", "\\cdot", "1", "\\big)"),
        )
        ...
        for matrix, det in zip([M1, M2], line1):
            ...
            matrix_numbers = VGroup(*[matrix.mob_matrix[i][j].copy() for i, j in ((0, 0), (1, 1), (0, 1), (1, 0))])
            self.play(
    ...
    def compute_lhs(self):
        ...
        self.play(*[ReplacementTransform(M.copy(), matrix) for M in (self.M1, self.M2)])
...
class SuccessiveLinearTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.scale_area_successively("\\det(M_2)", "\\det(M_1)", "\\det(M_1 M_2)", reset=False)
    ...
    def scale_area_successively(self, tex2="3", tex1="5", tex_prod="15", reset=True):
        ...
        self.play(FadeIn(self.square), Write(area), Animation(self.basis_vectors))
