# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\holomorphic.py
...
class AnalyzeZSquared(ComplexTransformationScene, ZoomedScene):
    ...
    def preview_some_numbers(self):
        ...
        for dot in dots:
            ...
            dot_group.anim = Succession(
                ...
                AnimationGroup(
                    ShowCreation(arrow),
                    ReplacementTransform(dot_copy, out_dot, path_arc=path_arc),
                ),
            )
        ...
        self.apply_complex_homotopy(self.complex_homotopy, added_anims=[Animation(dot_groups)])
        self.wait()
    ...
    def zoom_in_to_one_plus_half_i(self):
        ...
        frame.move_to(point)
        tiny_plane = NumberPlane(x_radius=2, y_radius=2, color=GREEN, secondary_color=GREEN_E)
