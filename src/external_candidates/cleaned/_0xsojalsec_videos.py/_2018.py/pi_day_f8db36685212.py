...
def get_circle_drawing_terms(radius=1, positioning_func=lambda m: m.center()):
    ...
    one_update = UpdateFromFunc(
        one,
        lambda m: m.move_to(radius.get_center() + 0.25 * rotate_vector(radius.get_vector(), TAU / 4)),
    )
...
class HartlAndPalais(Scene):
    def construct(self):
        ...
        self.play(
            Homotopy(
                lambda x, y, z, t: (
                    x + 0.1 * np.sin(2 * TAU * t) * np.exp(-10 * (t - 0.5 - 0.5 * (y - 1.85)) ** 2),
                    y,
                    z,
                ),
...
class UsingTheta(Scene):
    def construct(self):
        ...
        update_theta = Mobject.add_updater(
            theta,
            lambda m: m.shift(rotate_vector(0.9 * RIGHT, radius.get_angle() / 2) - m[1][0].get_center()),
        )
        ...
        for frac in 1.0 / 4, 1.0 / 12, 3.0 / 8, 1.0 / 6, 5.0 / 6:
            self.play(
                ...
                UpdateFromAlphaFunc(
                    arc,
                    lambda m, a: m.pointwise_become_partial(start_arc, 0, interpolate(last_frac, frac, a)),
                ),
                run_time=3,
            )
            last_frac = frac
...
class AskPuzzle(TeacherStudentsScene):
    def construct(self):
        series = OldTex(
            "\\frac{1}{1} + \\frac{1}{4} + \\frac{1}{9} + \\cdots + " + "\\frac{1}{n^2} + \\cdots = ",
            "\\,???",
        )
