# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\turbulence.py
from _2018.div_curl import (
    PureAirfoilFlow,
...
class Eddy(VMobject):
    CONFIG = {
        "cd_mob_config": {"frequency": 0.2, "max_ratio_shown": 0.3},
        "n_spirils": 5,
        "n_layers": 20,
        "radius": 1,
    ...
    def __init__(self, **kwargs):
        ...
        # self.add(lines)
        self.add(*[CreationDestructionMobject(line, **self.cd_mob_config) for line in lines])
    ...
    def get_lines(self):
        a = 0.2
        return VGroup(*[self.get_line(r=self.radius * (1 - a + 2 * a * random.random())) for x in range(self.n_layers)])
...
class Chaos(Eddy):
    CONFIG = {
        ...
        "width": 2,
        "n_midpoints": 4,
        "cd_mob_config": {"use_copy": False, "frequency": 1, "max_ratio_shown": 0.8},
    }
    ...
    def get_line(self, y):
        ...
        rect = self.rect
        line = Line(y * UP, y * UP + self.width * RIGHT, stroke_width=1)
...
class Diffusion(VMobject):
    CONFIG = {"height": 1.5, "n_dots": 1000, "colors": [RED, BLUE]}
...
class EddyReference(Scene):
    ...
    def construct(self):
        ...
        for line in new_eddy:
            line.set_stroke(width=(3 + 3 * random.random()))
...
class EddyLabels(Scene):
    def construct(self):
        ...
        for label in labels:
            self.play(FadeIn(label, rate_func=there_and_back_with_pause, run_time=3))


class LargeEddyReference(EddyReference):
    CONFIG = {"radius": 2, "label": ""}


class MediumEddyReference(EddyReference):
    CONFIG = {"radius": 0.8, "label": "Medium eddy"}


class SmallEddyReference(EddyReference):
    CONFIG = {"radius": 0.25, "label": "Small eddy"}
...
class JokeRingEquation(Scene):
    def construct(self):
        ...
        for i in 0, 1:
            self.play(FadeInFromDown(items[i]), ShowCreation(arrows[i]))
            self.wait()
...
class LightBouncingOffFogParticle(Scene):
    def construct(self):
        words = OldTexText("Light bouncing\\\\", "off fog particles")
...
class CarefulWithLasers(TeacherStudentsScene):
    def construct(self):
        ...
        def update_laser(laser, dt):
            laser.time += dt
            laser.rotate(0.5 * dt * np.sin(laser.time), about_point=laser[0].get_center())
        ...
        self.play(
            ShowCreation(laser),
            self.change_students("surprised", "hooray", "horrified", look_at=laser),
        )
        self.teacher_says("Careful with \\\\ the laser!", target_mode="angry")
        self.wait(2.2)
        morty.save_state()
        randy2.save_state()
        self.play(morty.blink, randy2.blink, run_time=0.3)
        self.wait(2)
        self.play(morty.restore, randy2.restore, run_time=0.3)
        self.wait(2)


class SetAsideTurbulence(PiCreatureScene):
    def construct(self):
        self.pi_creature_says("Forget vortex rings", target_mode="speaking")
        self.wait()
        self.pi_creature_says("look at that\\\\ turbulence!", target_mode="surprised")
        self.wait()
...
class WavingRodLabel(Scene):
    def construct(self):
        words = OldTexText("(Waving a small flag \\\\ through the air)")
...
class AskAboutTurbulence(TeacherStudentsScene):
    ...
    def pi_creatures_ask(self):
        ...
        self.play(
            ...
            self.change_students(*3 * ["pondering"], look_at=question),
        )
        self.play(
            ShowCreation(h_line),
            LaggedStartMap(FadeOutAndShiftDown, self.pi_creatures, run_time=1, lag_ratio=0.8),
        )
        self.wait()
    ...
    def divide_by_qualitative_quantitative(self):
        ...
        self.play(ShowCreation(v_line), LaggedStartMap(FadeInFromDown, words))
        self.wait()
    ...
    def three_qualitative_descriptors(self):
        ...
        words.arrange(DOWN, buff=1.25, aligned_edge=LEFT)
        words.to_edge(LEFT)
...
class BumpyPlaneRide(Scene):
    def construct(self):
        ...
        while total_time < 10:
            ...
            self.play(plane.move_to, point, run_time=time, path_arc=arc)
...
class LaminarFlowLabel(Scene):
    def construct(self):
        ...
        words.to_edge(UP)
        subwords = OldTexText(
            "`Lamina', in Latin, means \\\\``a thin sheet of material''",
...
class HighCurlFieldBreakingLayers(Scene):
    ...
    def construct(self):
        lines = VGroup(*[self.get_line() for x in range(20)])
...
class HighCurlFieldBreakingLayersLines(HighCurlFieldBreakingLayers):
    CONFIG = {"flow_anim": move_points_along_vector_field}
...
class VorticityDoesNotImplyTurbulence(TeacherStudentsScene):
    def construct(self):
        ...
        morty = self.teacher
        self.play(morty.change, "raise_right_hand", FadeInFromDown(t_to_v))
        self.wait()
        self.play(
            t_to_v.shift,
            2 * UP,
        ...
        self.play(
            ...
            self.change_students("erm", "confused", "sassy", run_time=1),
...
class FeynmanOnTurbulence(Scene):
    def construct(self):
        ...
        self.play(FadeIn(feynman, UP), FadeIn(name, DOWN), Write(quote, run_time=4))
        self.wait()
...
class ShowNavierStokesEquations(Scene):
    ...
    def introduce_equations(self):
        ...
        newtons_second = OldTexText("Newton's 2nd law \\\\ $ma = F$")
        ...
        self.wait(3)
        self.play(LaggedStartMap(FadeOut, VGroup(*it.chain(labels, variables, newtons_second))))

        self.equations = equations

    def ask_about_evolution(self):
        words = OldTexText("Given a start state...", "...how does it evolve?")
    ...
    def ask_about_reasonable(self):
        question = OldTexText("Do ``reasonable'' \\\\solutions always\\\\exist?")
    ...
    def ask_about_blowup(self):
        ...
        self.play(
            ...
            self.reasonable_question.to_edge,
            LEFT,
            self.reasonable_question.shift,
            DOWN,
        )
        self.play(
            Write(question),
        ...
        self.play(LaggedStartMap(FadeOut, to_fade), morty.change, "pondering")
        self.wait(2)
    ...
    def show_money(self):
        ...
        problem = OldTexText("Navier-Stokes existence \\\\ and smoothness problems")
...
class NewtonsSecond(Scene):
    def construct(self):
        square = Square(stroke_color=WHITE, fill_color=GREY_B, fill_opacity=0.5, side_length=1)
        label = OldTex("m")
        label.scale(1.5)
        ...
        self.play(square.shift, 4 * RIGHT + 2 * UP, rate_func=lambda t: t**2, run_time=2)
        self.wait()
        square.restore()
...
class CandleLabel(Scene):
    def construct(self):
        ...
        self.play(FadeInFromDown(word), GrowArrow(arrow))
        self.wait()
...
class FiguresOfFluidDynamics(Scene):
    def construct(self):
        ...
        images = Group(*[ImageMobject(name.replace(" ", "_"), height=3) for name in names])
        ...
        self.play(LaggedStartMap(FadeInFromDown, image_groups, lag_ratio=0.5, run_time=3))
        self.wait()
...
class KineticEnergyBreakdown(Scene):
    def construct(self):
        ...
        self.play(Write(VGroup(*it.chain(title, h_line, v_line, lc_title, rc_title))))
        self.wait()
...
class Heat(Scene):
    def construct(self):
        ...
        balls = VGroup(*[self.get_ball(box) for x in range(20)])
    ...
    def get_ball(self, box):
        ...
        ball = Dot(radius=0.05, color=interpolate_color(BLUE, RED, speed_factor))
...
class SwirlDiameterD(Scene):
    def construct(self):
        kwargs = {"path_arc": PI, "buff": SMALL_BUFF, "color": WHITE}
        swirl = VGroup(
...
class KolmogorovGraph(Scene):
    def construct(self):
        ...
        prop_label.next_to(graph.point_from_proportion(0.5), UL, buff=0)
...
class FiveThirds(TeacherStudentsScene):
    def construct(self):
        words = OldTexText("5/3", "is a sort of fundamental\\\\ constant of turbulence")
...
class TurbulenceGifLabel(Scene):
    def construct(self):
        ...
        attribution = OldTexText("Animation by Gabe Weymouth (@gabrielweymouth)")
...
class VortedStretching(ThreeDScene):
    ...
    def construct(self):
        ...
        self.wait(10)
        self.play(Transform(circles, torus_circles, run_time=3))
        self.wait(10)
    ...
    def get_cylinder_circles(self, radius, radius_var, max_z):
        return VGroup(
            *[
                ParametricCurve(
                    lambda t: np.array([np.cos(TAU * t) * r, np.sin(TAU * t) * r, z]),
    ...
    def get_flow_lines(self, circle_group):
        ...
        def update_circle(circle, dt):
            ...
            diameter = get_norm(circle.template.point_from_proportion(0) - circle.template.point_from_proportion(0.5))
