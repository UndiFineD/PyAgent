# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\div_curl.py
...
def divergence(vector_func, dt=1e-7):
    def result(point):
        ...
        return sum([(vector_func(point + dt * RIGHT) - value)[i] / dt for i, vect in enumerate([RIGHT, UP, OUT])])
...
class JigglingSubmobjects(VGroup):
    ...
    def update(self, dt):
        for submob in self.submobjects:
            ...
            submob.shift(self.amplitude * submob.jiggling_direction * np.sin(submob.jiggling_phase) * dt)
...
class ShowWritingTrajectory(TeacherStudentsScene):
    ...
    def show_meandering_path(self):
        ...
        step = 1.0 / 80
        dashed_path = VGroup(
            *[VMobject().pointwise_become_partial(solid_path, x, x + step / 2) for x in np.arange(0, 1 + step, step)]
        )
        arrow = Arrow(solid_path.get_points()[-2], solid_path.get_points()[-1], buff=0)
        ...
        self.play(
            LaggedStartMap(ApplyMethod, dashed_path, lambda m: (m.scale, 0), remover=True),
            self.teacher.change,
            "tease",
            self.change_students(*["pondering"] * 3, look_at=self.screen),
        )
    ...
    def show_previous_video(self):
        ...
        complex_words.next_to(screen.target, LEFT, buff=2 * SMALL_BUFF + arrow.get_length())

        self.play(GrowArrow(arrow), Write(prev_words))
        self.wait(3)
        self.play(
            arrow.flip,
            arrow.next_to,
        ...
        bubble = self.teacher.get_bubble(bubble_type=SpeechBubble, height=3, width=5)
...
class CylinderModel(Scene):
    ...
    def show_numbers(self):
        ...
        dot = Dot()
        dot_update = UpdateFromFunc(dot, lambda d: d.move_to(unit_circle.point_from_proportion(1)))
        exp_tex = OldTex("e^{", "0.00", "i}")
        zero = exp_tex.get_part_by_tex("0.00")
        zero.fade(1)
        exp_tex_update = UpdateFromFunc(exp_tex, lambda et: et.next_to(dot, UR, SMALL_BUFF))
        exp_decimal = DecimalNumber(0, num_decimal_places=2, include_background_rectangle=True, color=YELLOW)
        ...
        self.play(
            ...
            VFadeIn(exp_tex),
            UpdateFromAlphaFunc(exp_decimal, lambda ed, a: ed.set_fill(opacity=a)),
            dot_update,
            exp_tex_update,
            exp_decimal_update,
            LaggedStartMap(
    ...
    def show_contour_lines(self):
        ...
        self.play(
            warped_grid.apply_complex_function,
    ...
    def show_flow(self):
        ...
        stream_lines_animation = self.get_stream_lines_animation(stream_lines)
        ...
        fast_lines, slow_lines = [
            VGroup(
                *[v_lines.copy().next_to(point, vect, tiny_buff) for point, vect in it.product(h_points, [UP, DOWN])]
            )
            for h_points in [
                [0.5 * LEFT, 0.5 * RIGHT],
                [2 * LEFT, 2 * RIGHT],
        ...
        self.wait(3)
        self.play(FadeOut(slow_lines), VFadeOut(stream_lines_animation.mobject))
...
class OkayNotToUnderstand(Scene):
    def construct(self):
        words = OldTexText("It's okay not to \\\\ understand this just yet.")
        morty = Mortimer()
...
class ElectricField(CylinderModel, MovingCameraScene):
    ...
    def show_uniform_field(self):
        vector_field = self.vector_field = VectorField(lambda p: UP, colors=[BLUE_E, WHITE, RED])
        protons, electrons = groups = [
            VGroup(*[method(radius=0.2) for x in range(20)]) for method in (get_proton, get_electron)
        ]
        for group in groups:
    ...
    def show_moving_charges(self):
        ...
        protons = VGroup(
            *[
                get_proton().move_to(rotate_vector(0.275 * n * RIGHT, angle))
                for n in range(4)
                for angle in np.arange(0, TAU, TAU / (6 * n) if n > 0 else TAU)
            ]
        )
        jiggling_protons = JigglingSubmobjects(protons)
        electrons = VGroup(
            *[
                get_electron().move_to(proton.get_center() + proton.radius * rotate_vector(RIGHT, angle))
        ...
        new_vector_field = VectorField(new_electric_field, colors=self.vector_field.colors)

        warped_grid = self.warped_grid

        self.play(GrowFromCenter(unit_circle))
        self.add(jiggling_protons, jiggling_electrons)
        self.add_foreground_mobjects(self.vector_field, unit_circle, protons, electrons)
        self.play(
            LaggedStartMap(VFadeIn, protons),
            LaggedStartMap(VFadeIn, electrons),
        )
        self.play(self.camera.frame.scale, 0.7, run_time=3)
        self.play(
            MoveToTarget(electrons),  # More indication?
            warped_grid.apply_complex_function,
            inverse_joukowsky_map,
            Transform(self.vector_field, new_vector_field),
            run_time=3,
        )
        self.wait(5)
    ...
    def show_field_lines(self):
        ...
        vp_prop = 0.1
        voltage_point = VectorizedPoint(h_lines[4].point_from_proportion(vp_prop))
        ...
        voltage.add_updater(lambda d: d.next_to(voltage_point, UP, SMALL_BUFF))

        self.play(ShowCreation(h_lines, run_time=2, lag_ratio=0))
        self.add(voltage_update)
        self.add_foreground_mobjects(voltage)
        self.play(
            UpdateFromAlphaFunc(voltage, lambda m, a: m.set_fill(opacity=a)),
            h_lines[4].set_stroke,
            YELLOW,
            4,
        )
...
class AskQuestions(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(FadeInFromDown(curl), div.restore)
        self.wait()
        ...
        self.play(
            MoveToTarget(topics),
            self.change_students("confused", "sassy", "erm", look_at=topics.target),
            self.teacher.change,
            "pondering",
            screen,
        )
        self.play(Write(arrow), FadeInFromDown(complex_analysis))
        self.look_at(screen)
        self.wait(6)
...
class ScopeMeiosis(PiCreatureScene):
            ...
            *list(
                map(
                    ...
                    [
                        ...
                        "Conformal maps",
                        "Conformal map $\\Rightarrow" + "\\text{div}\\textbf{F} = " + "\\text{curl}\\textbf{F} = 0$",
                        "Complex derivatives",
                    ],
                )
            )
        )
        sections = VGroup(*[VGroup(title, self.get_lines(title, 3)) for title in section_titles])
        sections.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        sections.to_edge(UP)
    ...
    def get_lines(self, title, n_lines):
        lines = VGroup(*[Line(3 * LEFT, 3 * RIGHT, color=GREY_B) for x in range(n_lines)])
        lines.arrange(DOWN, buff=MED_SMALL_BUFF)
        lines.next_to(title, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
        lines[-1].pointwise_become_partial(lines[-1], 0, random.random())
        return lines
...
class TopicsAndConnections(Scene):
    ...
    def construct(self):
        dots = VGroup(*[Dot(8 * np.array([random.random(), random.random(), 0])) for n in range(5)])
        topics = VGroup(
            *[
                OldTexText(word).next_to(dot, RIGHT, SMALL_BUFF)
                for dot, word in zip(
                    dots,
                    [
                        "Divergence/curl",
        ...
        for topic in topics:
            topic.add_to_back(topic.copy().set_stroke(BLACK, 2))
        ...
        connections = VGroup(*[DashedLine(d1.get_center(), d2.get_center()) for d1, d2 in it.combinations(dots, 2)])
        connections.set_stroke(YELLOW, 2)

        full_rect = FullScreenFadeRectangle()

        self.play(
            LaggedStartMap(ApplyMethod, dots, lambda d: (d.restore,)),
...
class IntroduceVectorField(Scene):
    ...
    def points_to_vectors(self):
        vector_field = self.vector_field = VectorField(four_swirls_function, **self.vector_field_config)
        dots = VGroup()
    ...
    def show_fluid_flow(self):
        ...
        stream_lines = StreamLines(vector_field.func, **self.stream_line_config)
        stream_line_animation = AnimatedStreamLines(stream_lines, **self.stream_line_animation_config)

        self.add(stream_line_animation)
        self.play(vector_field.set_fill, {"opacity": 0.5})
        self.wait(7)
        self.play(
            vector_field.set_fill,
            {"opacity": 1},
    ...
    def show_gravitational_force(self):
        ...
        gravity_field = VectorField(gravity_func, **self.vector_field_config)
    ...
    def show_magnetic_force(self):
        magnetic_func = get_force_field_func((3 * LEFT, -1), (3 * RIGHT, +1))
        magnetic_field = VectorField(magnetic_func, **self.vector_field_config)
        magnet = VGroup(
            *[
                Rectangle(
                    width=3.5,
...
class QuickNoteOnDrawingThese(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            added_anims=[self.change_students("confused", "erm", "sassy")],
        )
        self.look_at(self.screen)
        self.wait(3)
...
class ShorteningLongVectors(IntroduceVectorField):
    ...
    def contrast_adjusted_and_non_adjusted(self):
        ...
        self.add(adjusted)
        self.wait()
        self.play(LaggedStartMap(MoveToTarget, adjusted, run_time=3))
        self.wait()
        self.play(LaggedStartMap(ApplyMethod, adjusted, lambda m: (m.restore,), run_time=3))
        self.wait()
...
class GravityFluidFlow(IntroduceVectorField):
    def construct(self):
        self.vector_field = VectorField(lambda p: np.array(ORIGIN), **self.vector_field_config)
        self.show_gravitational_force()
        self.show_fluid_flow()
...
class TotallyToScale(Scene):
    def construct(self):
        words = OldTexText("Totally drawn to scale. \\\\ Don't even worry about it.")
...
class FluidFlowAsHillGradient(CylinderModel, ThreeDScene):
    ...
    def construct(self):
        def potential(point):
            x, y = point[:2]
            result = 2 - 0.01 * op.mul(((x - 4) ** 2 + y**2), ((x + 4) ** 2 + y**2))
        ...
        for line in stream_lines:
            line.get_points()[:, 2] = np.apply_along_axis(potential, 1, line.get_points())
        stream_lines_animation = self.get_stream_lines_animation(stream_lines)
        ...
        self.move_camera(phi=70 * DEGREES, run_time=2)
        self.wait(5)
...
class DefineDivergence(ChangingElectricField):
    ...
    def draw_vector_field(self):
        ...
        particles.arrange_in_grid(n_cols=4, buff=3)
        for particle in particles:
            particle.shift(
    ...
    def show_flow(self):
        stream_lines = StreamLines(self.vector_field.func, **self.stream_line_config)
        stream_line_animation = AnimatedStreamLines(stream_lines, **self.stream_line_animation_config)
        self.add(stream_line_animation)
    ...
    def point_out_sources_and_sinks(self):
        ...
        self.positive_points, self.negative_points = [
            [particle.get_center() for particle in particles if u * particle.charge > 0] for u in (+1, -1)
        ]
        ...
        for point_set in self.positive_points, self.negative_points:
            ...
            self.play(
                self.vector_field.set_fill,
                {"opacity": 0.5},
                LaggedStartMap(
                    LaggedStartMap,
                    vector_circle_groups,
    ...
    def show_divergence_values(self):
        ...
        div_tex = OldTex("\\text{div} \\, \\textbf{F}(x, y) = ")
        div_tex.add_background_rectangle()
        div_tex_update = Mobject.add_updater(div_tex, lambda m: m.next_to(circle, UP, SMALL_BUFF))
...
class DefineDivergenceSymbols(Scene):
    def construct(self):
        ...
        output_animation = ContinualChangingDecimal(
            output,
            lambda a: 3 * np.cos(int(time_tracker.get_value())),
        )
        ...
        output_label = output_brace.get_text("Measure of how much \\\\ $(x, y)$ ``generates'' fluid")
        brace_label_pairs = [
            (F_brace, F_label),
            (xy_brace, xy_label),
        ...
        for brace, label in brace_label_pairs:
            self.play(GrowFromCenter(brace), FadeInFromDown(label), *fade_anims)
            self.wait(2)
...
class DivergenceAtSlowFastPoint(Scene):
    ...
    def construct(self):
        ...
        div_tex = OldTex("\\text{div}\\,\\textbf{F}(x, y) > 0")
        div_tex.add_background_rectangle()
...
class DivergenceAsNewFunction(Scene):
    ...
    def show_vector_field_function(self):
        ...
        # Changing decimals
        in_x, in_y, out_x, out_y = [DecimalNumber(0, include_sign=True) for x in range(4)]
        ...
        self.out_vect_update = Mobject.add_updater(out_vect, lambda ov: Transform(ov, get_out_vect()).update(1))

        self.add(self.out_vect_update)
        self.add(out_x_update, out_y_update)

        self.add(Mobject.add_updater(VGroup(out_x, out_y), lambda m: m.match_style(out_vect)))
        self.wait()

        for vect in DOWN, 2 * RIGHT, UP:
            self.play(in_dot.shift, 3 * vect, run_time=3)
            self.wait()
    ...
    def show_divergence_function(self):
        ...
        vector_field.remove(*[v for v in vector_field if v.get_start()[0] < 0 and v.get_start()[1] > 2])
        ...
        def get_neighboring_points(step_sizes=[0.3], n_angles=12):
            ...
            return list(
                it.chain(
                    *[[point + step_size * step for step in compass_directions(n_angles)] for step_size in step_sizes]
                )
            )

        def get_vector_ring():
            return VGroup(*[vector_field.get_vector(point) for point in get_neighboring_points()])

        def get_stream_lines():
            return StreamLines(
                self.func,
                start_points_generator=get_neighboring_points,
                start_points_generator_config={"step_sizes": np.arange(0.1, 0.5, 0.1)},
                virtual_time=1,
                stroke_width=3,
            )
        ...
        def show_flow():
            ...
            self.play(LaggedStartMap(ShowCreationThenDestruction, stream_lines, remover=True))

        vector_ring = get_vector_ring()
        vector_ring_update = Mobject.add_updater(vector_ring, lambda vr: Transform(vr, get_vector_ring()).update(1))
        ...
        self.add(*[Mobject.add_updater(mob, lambda m: m.set_fill(None, 0)) for mob in (out_x, out_y)])
        ...
        div_rhs = DecimalNumber(0, include_sign=True, include_background_rectangle=True)
        div_rhs_update = ContinualChangingDecimal(
            div_rhs,
            lambda a: div_func(in_dot.get_center()),
...
class DivergenceZeroCondition(Scene):
    def construct(self):
        title = OldTexText("For actual (incompressible) fluid flow:")
        title.to_edge(UP)
        equation = OldTex("\\text{div} \\, \\textbf{F} = 0 \\quad \\text{everywhere}")
...
class IntroduceCurl(IntroduceVectorField):
    ...
    def show_vector_field(self):
        vector_field = self.vector_field = VectorField(four_swirls_function, **self.vector_field_config)
        vector_field.submobjects.sort(key=lambda v: v.get_length())
    ...
    def show_rotation(self):
        clockwise_arrows, counterclockwise_arrows = [
            VGroup(*[self.get_rotation_arrows(clockwise=cw).move_to(point) for point in points])
            for cw, points in [
                (True, [2 * UP, 2 * DOWN]),
        ...
        for group in counterclockwise_arrows, clockwise_arrows:
            ...
            self.play(
                UpdateFromAlphaFunc(
                    group,
                    lambda mob, alpha: mob.set_color(interpolate_color(WHITE, PINK, alpha)).set_stroke(
    ...
    def get_rotation_arrows(self, clockwise=True, width=1):
        result = VGroup(
            *[
                Arrow(*points, buff=2 * SMALL_BUFF, path_arc=90 * DEGREES).set_stroke(width=5)
...
class ShearCurl(IntroduceCurl):
    ...
    def show_vector_field(self):
        vector_field = self.vector_field = VectorField(self.func, **self.vector_field_config)
        vector_field.submobjects.key = sort(key=lambda a: a.get_length())
        self.play(LaggedStartMap(GrowArrow, vector_field))
...
class FromKAWrapper(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[self.change_students("happy", "hooray", "happy")],
        )
        self.wait()
...
class ShowCurlAtVariousPoints(IntroduceCurl):
    CONFIG = {
        ...
        "vector_field_config": {"fill_opacity": 0.75},
        "stream_line_config": {
            "virtual_time": 5,
            "start_points_generator_config": {
                "delta_x": 0.25,
    ...
    def show_curl_at_points(self):
        ...
        circle.move_to(dot)
        circle_update = Mobject.add_updater(circle, lambda m: m.move_to(dot))

        curl_tex = OldTex("\\text{curl} \\, \\textbf{F}(x, y) = ")
        curl_tex.add_background_rectangle(buff=0.025)
        curl_tex_update = Mobject.add_updater(curl_tex, lambda m: m.next_to(circle, UP, SMALL_BUFF))
        ...
        curl_value_update = ContinualChangingDecimal(
            ...
            position_update_func=lambda m: m.next_to(curl_tex, RIGHT, buff=0),
            include_background_rectangle=True,
            include_sign=True,
        )
...
class IllustrationUseVennDiagram(Scene):
    def construct(self):
        ...
        illustrated_by.next_to(big_circle.point_from_proportion(3.0 / 8), UL)
        ...
        illustrated_by_arrow.pointwise_become_partial(illustrated_by_arrow, 0, 0.95)
...
class MaxwellsEquations(Scene):
    ...
    def add_equations(self):
        ...
        equations = self.equations = VGroup(
            *[
                OldTex(tex, tex_to_color_map=tex_to_color_map)
                for tex in [
                    """
                    \\text{div} \\, \\textbf{E} =
        ...
        equations.arrange(DOWN, aligned_edge=LEFT, buff=MED_LARGE_BUFF)
        ...
        field_definitions.arrange(RIGHT, buff=MED_LARGE_BUFF)
        ...
        self.play(LaggedStartMap(FadeIn, equations, run_time=3, lag_range=0.4))
        self.wait()
    ...
    def circle_gauss_law(self):
        ...
        self.play(
            ...
            self.equations[1:].set_fill,
            {"opacity": self.faded_opacity},
        )
        self.wait(2)
        self.play(
    ...
    def circle_magnetic_divergence(self):
        ...
        self.play(
            equations[0].set_fill,
            {"opacity": self.faded_opacity},
            equations[1].set_fill,
            {"opacity": 1.0},
        )
...
class IllustrateGaussLaw(DefineDivergence, MovingCameraScene):
    ...
    def zoom_in(self):
        self.play(self.camera_frame.set_width, self.final_frame_width, run_time=2)
...
class IllustrateGaussMagnetic(IllustrateGaussLaw):
    ...
    def add_wires(self):
        top, bottom = [Circle(radius=0.275, stroke_color=WHITE, fill_color=BLACK, fill_opacity=1) for x in range(2)]
    ...
    def show_vector_field(self):
        vector_field = self.vector_field = VectorField(self.func, **self.vector_field_config)
        vector_field.submobjects.sort(key=lambda a: -a1.get_length())
        self.play(LaggedStartMap(GrowArrow, vector_field))
        self.add_foreground_mobjects(vector_field, *self.foreground_mobjects)
...
class RelevantInNonSpatialCircumstances(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("sassy", "confused", "hesitant")
        self.wait(3)
...
class ShowTwoPopulations(Scene):
    ...
    def introduce_animals(self):
        foxes = self.foxes = VGroup(*[self.get_fox() for n in range(self.total_num_animals)])
        rabbits = self.rabbits = VGroup(*[self.get_rabbit() for n in range(self.total_num_animals)])
        ...
        preditor, prey = words = VGroup(OldTexText("Predator"), OldTexText("Prey"))
        ...
        self.play(
            LaggedStartMap(ApplyMethod, examples, lambda m: (m.restore,)),
    ...
    def evolve_system(self):
        ...
        phase_point = VectorizedPoint(self.start_num_rabbits * RIGHT + self.start_num_foxes * UP)
        self.add(
        ...
        def get_updater(pop_size_getter):
            def update(animals):
                ...
                for n, animal in enumerate(animals):
                    animal.set_fill(opacity=np.clip(target_num - n, 0, 1))
        ...
        self.add(Mobject.add_updater(foxes, get_updater(get_num_foxes)))
        self.add(Mobject.add_updater(rabbits, get_updater(get_num_rabbits)))
        ...
        self.play(
            FadeIn(labels),
            *[UpdateFromAlphaFunc(count, lambda m, a: m.set_fill(opacity=a)) for count in (num_foxes, num_rabbits)],
        )
    ...
    def get_animal(self, name, color):
        ...
        #     )
        x_shift, y_shift = [
            (2 * random.random() - 1) * max_val for max_val in [FRAME_WIDTH / 2 - 2, FRAME_HEIGHT / 2 - 2]
        ]
    ...
    def update_count_color(self, count):
        count.set_fill(interpolate_color(BLUE, RED, (count.number - 20) / 30.0))
        return count
...
class PhaseSpaceOfPopulationModel(ShowTwoPopulations, PiCreatureScene, MovingCameraScene):
    ...
    def add_axes(self):
        ...
        for axis, label, vect in zip(axes, axes_labels, [RIGHT, UP]):
            label.next_to(axis, vect, submobject_to_align=label[0])
    ...
    def add_example_point(self):
        ...
        coord_pair = OldTex("(10, 10)", isolate=["10"])
        ...
        def get_pop_size_update(i):
            return ContinualChangingDecimal(
                pop_sizes[i],
                lambda a: int(np.round(axes.point_to_coords(dot.get_center())[i])),
    ...
    def write_differential_equations(self):
        ...
        self.differential_equation_group = VGroup(rect, equations, title)
        self.differential_equation_group.to_corner(UR)

        randy = self.pi_creature
        randy.next_to(rect, DL)

        self.play(Write(title, run_time=1), ShowCreation(rect))
        self.play(
    ...
    def add_vectors(self):
        ...
        vector_field = VectorField(rescaled_field, **self.vector_field_config)
        ...
        dot_movement = move_along_vector_field(dot, lambda p: 0.3 * vector_field.func(p))
        ...
        self.remove(dot_movement, self.coord_pair_update, *self.pop_sizes_updates)
        self.wait()
    ...
    def show_phase_flow(self):
        ...
        self.wait(self.flow_time)
        self.play(
            self.camera_frame.scale,
            1.5,
            {"about_point": self.origin},
...
class PhaseFlowQuestions(Scene):
    def construct(self):
        questions = VGroup(
            OldTexText(
                "Which points does the flow \\\\" + "converge to?  Diverge away from?",
            ),
...
class ToolsBeyondDivAndCurlForODEs(PiCreatureScene):
    ...
    def construct(self):
        ...
        flow_intuitions.next_to(VGroup(div_curl, jacobian), UP, buff=1.5)
        ...
        self.wait()
        self.play(FadeInFromDown(jacobian), morty.change, "raise_right_hand")
        self.wait()
        self.play(
            ReplacementTransform(
...
class AskAboutComputation(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "Sure, but how do you \\\\" + "\\emph{compute} $\\textbf{div}$ and $\\textbf{curl}$?",
            target_mode="sassy",
        )
        self.play_student_changes("confused", "sassy", "angry", added_anims=[self.teacher.change, "guilty"])
        self.wait()
        self.teacher_says(
            "Are you familiar \\\\" + "with my work \\\\" + "at Khan Academy?",
...
class NablaNotation(PiCreatureScene, MovingCameraScene):
    ...
    def show_notation(self):
        ...
        self.play(FadeInFromDown(div_equation), morty.change, "raise_right_hand")
        self.wait()
        self.play(WiggleOutThenIn(div_nabla, scale_value=1.5))
        self.wait()
        self.play(FadeInFromDown(curl_equation), morty.change, "raise_left_hand")
        self.wait()
    ...
    def show_expansion(self):
        ...
        div_rhs = OldTex("{\\partial F_x \\over \\partial x} + " + "{\\partial F_y \\over \\partial y}")
        curl_rhs = OldTex("{\\partial F_y \\over \\partial x} - " + "{\\partial F_x \\over \\partial y}")
        ...
        for rhs, tex, color in zip(rhs_groups, ["div", "curl"], colors):
            ...
            rhs.label = OldTex("\\text{%s}" % tex, "\\, \\textbf{F}")
            rhs.label.set_color(color)
            rhs.label.next_to(rhs.rect, UP)

        for i in 0, 1:
            self.play(
                ReplacementTransform(equations[i][2].copy(), lhs_groups[i][0].brackets),
                ReplacementTransform(
                    equations[i][3].copy(),
                    lhs_groups[i][2],
                ),
        ...
        for rhs in rhs_groups:
            self.play(Write(rhs), morty.change, "confused")
            self.play(
...
class DivCurlDotCross(Scene):
    def construct(self):
        rects = VGroup(*[ScreenRectangle(height=2.5) for n in range(4)])
        rects.arrange_in_grid(n_rows=2, buff=LARGE_BUFF)
        rects[2:].shift(MED_LARGE_BUFF * DOWN)
        titles = VGroup(*list(map(TexText, ["Divergence", "Curl", "Dot product", "Cross product"])))
...
class ShowDotProduct(MovingCameraScene):
    ...
    def construct(self):
        ...
        dot_product = OldTex("\\vec{\\textbf{v}}", self.prod_tex, "\\vec{\\textbf{w}}", "=")
...
class ShowCrossProduct(ShowDotProduct):
    CONFIG = {"prod_tex": "\\times"}

    def get_product(self, v1, v2):
        return get_norm(np.cross(v1.get_vector(), v2.get_vector()))
...
class DivergenceTinyNudgesView(MovingCameraScene):
    ...
    def add_vector_field(self):
        ...
        def func(p):
            x, y = p[:2]
            result = np.array([np.sin(x + 0.1), np.cos(2 * y), 0])
    ...
    def take_tiny_step(self):
        ...
        step_vector = Arrow(point, nudged_point, buff=0, color=TEAL, **vector_field.vector_config)
        ...
        diff_words.rotate(diff_vector.get_angle(), about_point=diff_vector.get_start())
        ...
        self.wait()
        self.play(ReplacementTransform(vector.copy(), shifted_vector, path_arc=-TAU / 4))
        self.wait()
        self.play(
        ...
        self.play(
            ...
            shifted_vector.scale,
            0,
            {"about_point": shifted_vector.get_start()},
            ReplacementTransform(new_vector, diff_vector.copy().shift(-vector.get_vector()), remover=True),
            diff_vector.shift,
            -vector.get_vector(),
    ...
    def show_dot_product(self):
        ...
        self.moving_diff_vector_update = Mobject.add_updater(moving_diff_vector, update_moving_diff_vector)
        ...
        group.next_to(self.camera_frame.get_top(), DOWN, buff=sf * MED_SMALL_BUFF)
        ...
        brace = Brace(dot_product.copy().scale(1 / sf, about_point=ORIGIN), DOWN, buff=SMALL_BUFF).scale(
            sf, about_point=ORIGIN
        )
        dp_kwargs = {
            "include_sign": True,
        }
        ...
        self.play(Write(dot_product), LaggedStartMap(MoveToTarget, copies))
        self.remove(copies)
        self.play(FadeIn(div_text))
        self.play(ShowPassingFlashAround(div_text[1:3], surrounding_rectangle_config={"buff": sf * SMALL_BUFF}))
    ...
    def show_circle_of_values(self):
        ...
        self.remove(
            self.step_vector,
            self.diff_vector,
            self.moving_step_vector,
            self.moving_diff_vector,
        ...
        for s in 0.6, -0.6:
            for step, diff in zip(all_step_vectors, all_diff_vectors):
                ...
                diff.target.put_start_and_end_on(step.get_end(), step.get_end() + s * step.get_vector())
            self.play(
                all_step_vectors.set_fill,
                {"opacity": 0.5},
                LaggedStartMap(MoveToTarget, all_diff_vectors, run_time=3),
            )
            self.wait()
    ...
    def switch_to_curl_words(self):
        ...
        cross_product = OldTexText(
            ...
            tex_to_color_map={"Step": TEAL, "Difference": RED},
            arg_separator="",
        ).scale(sf)
        ...
        self.play(dot_product.shift, sf * DOWN, dot_product.fade, 1, remover=True)
        self.play(FadeIn(cross_product, sf * DOWN))
        self.play(div_text.shift, sf * DOWN, div_text.fade, 1, remover=True)
        ...
        for s in 0.6, -0.6:
            for step, diff in zip(all_step_vectors, all_diff_vectors):
                ...
                diff.target.put_start_and_end_on(
                    step.get_end(),
                    step.get_end() + s * rotate_vector(step.get_vector(), 90 * DEGREES),
                )
            self.play(
                LaggedStartMap(MoveToTarget, all_diff_vectors, run_time=2),
            )
            self.wait()
            self.show_stream_lines(lambda p: s * rotate_vector((p - point), 90 * DEGREES))
            self.wait()
...
class ZToHalfFlowNearWall(ComplexTransformationScene, MovingCameraScene):
    CONFIG = {"num_anchors_to_add_per_line": 200, "plane_config": {"y_radius": 8}}
    ...
    def construct(self):
        ...
        self.apply_complex_function(lambda x: x ** (1.0 / 2), added_anims=[Write(equation)])
...
class BroughtToYouBy(PiCreatureScene):
    ...
    def brought_to_you_by(self):
        ...
        only_group.next_to(VGroup(btyb[0][-1], btyb[1][0]), UP, SMALL_BUFF)
        ...
        self.play(
            Write(so_words[0]),
            LaggedStartMap(DrawBorderThenFill, so_words[1], run_time=5),
        )
        self.play(so_words.shift, DOWN, so_words.fade, 1, remover=True)
        self.play(FadeInFromDown(btyb_group))
        self.wait()
        self.play(Write(you_word))
        self.play(GrowFromCenter(only_brace), Write(only_word))
        self.wait()
        self.play(
            MoveToTarget(
                full_group,
        ...
        self.play(LaggedStartMap(DrawBorderThenFill, patreon_logo))
        self.wait()
    ...
    def just_you_and_me(self):
        ...
        step = 0.1
        spiral = VGroup(*[VMobject().pointwise_become_partial(spiral, a, a + step) for a in np.arange(0, 1, step)])
...
class ThoughtsOnAds(Scene):
    def construct(self):
        ...
        line = NumberLine(color=GREY_B, x_min=0, x_max=12, big_tick_numbers=[])
        line.move_to(DOWN)
        ...
        left_text = OldTexText("Any website presented \\\\", "as a click-maximizing \\\\ slideshow")
        left_text.scale(0.8)
        left_text.next_to(line, UP, buff=MED_LARGE_BUFF)
        left_text.to_edge(LEFT)

        viewer, brand, creator = vcb = VGroup(*list(map(TexText, ["viewer", "brand", "creator"])))
        ...
        right_rect_label = Group(ImageMobject("3b1b_logo", height=1), OldTexText("(hopefully)").scale(0.8))
        right_rect_label.arrange(DOWN, buff=SMALL_BUFF)
        # OldTexText(
        #     "Where I hope \\\\ I've been"
        # )
        right_rect_label.next_to(right_rect, UP, SMALL_BUFF)
        # right_rect_label.set_color(BLUE)

        self.add(title)
        self.play(ShowCreation(line))
        self.play(Write(misaligned), Write(aligned), *list(map(GrowArrow, arrows)), run_time=1)

        self.play(FadeIn(left_text), FadeIn(knob, 2 * RIGHT))
        self.wait()
        self.play(
            LaggedStartMap(FadeInFromDown, vcb),
            LaggedStartMap(ShowCreation, vcb_arrows),
            ApplyMethod(knob.move_to, line.get_right(), DOWN, run_time=2),
        )
        self.wait(2)
...
class GoalWrapper(Scene):
    def construct(self):
        goal = OldTexText("Goal: Teach/remind people \\\\ that they love math")
        goal.to_edge(UP)
        self.add(goal)
...
class PeopleValueGraph(GraphScene):
    ...
    def add_curve(self):
        ...
        self.play(ShowCreation(graph), rate_func=bezier([0, 0, 1, 1]), run_time=3)

    def comment_on_incentives(self):
        reach_arrow = Vector(5 * RIGHT)
        reach_arrow.next_to(self.x_axis, DOWN, buff=SMALL_BUFF, aligned_edge=RIGHT)
        ...
        area = self.area = self.get_riemann_rectangles(self.graph, **self.riemann_rectangles_config)
        area_words = OldTexText("Maximize this area")
        area_words.set_color(BLUE)
        area_words.move_to(self.coords_to_point(4, 5))
        area_arrow = Arrow(area_words.get_bottom(), self.coords_to_point(1.5, 2))
    ...
    def change_curve(self):
        new_graph = self.get_graph(
            lambda x: interpolate(7 * np.exp(-0.01 * x), 7 * np.exp(-3 * x), smooth(np.clip(x / 5, 0, 1)))
        )
        new_area = self.get_riemann_rectangles(new_graph, **self.riemann_rectangles_config)

        self.play(
            Transform(self.area, new_area),
            Transform(self.graph, new_graph),
            self.area_label_group[0].shift,
            RIGHT,
