# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\for_flammy.py
...
class SphereRings(SecondProof):
    ...
    def grow_rings(self):
        ...
        southern_mesh = VGroup(*[face.copy() for face in sphere if face.get_center()[2] < 0])
    ...
    def show_one_ring(self):
        ...
        to_fade = VGroup(*[nr for nr in north_rings if nr is not ring])
    ...
    def show_radial_line(self):
        ...
        theta = np.arccos(np.dot(normalize(R_line.get_vector()), normalize(xy_line.get_vector())))
        ...
        R_label.next_to(R_line.get_center(), OUT + LEFT, buff=SMALL_BUFF)
        ...
        self.wait()
        self.move_camera(phi=70 * DEGREES, theta=-110 * DEGREES, run_time=3)
        self.wait(2)
    ...
    def show_thickness(self):
        ...
        self.play(GrowFromCenter(brace), Write(R_dtheta))
        self.wait(3)
...
class IntegralSymbols(Scene):
    def construct(self):
        ...
        circumference, times, thickness = ctt = OldTexText("circumference", "$\\times$", "thickness")
        ...
        self.play(FadeInFromDown(thickness), Write(times))
        self.play(Write(all_rings))
        self.wait()

        self.play(
            circumference.next_to,
            circum_brace,
            UP,
            MED_SMALL_BUFF,
            circumference.shift,
