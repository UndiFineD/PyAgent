# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\basel\basel2.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class AngleUpdater(ContinualAnimation):
    ...
    def update_mobject(self, dt):
        new_arc = self.angle_arc.copy().set_bound_angles(
            start=self.spotlight.start_angle(), stop=self.spotlight.stop_angle()
        )
        new_arc.init_points()
...
class LightIndicator(Mobject):
    ...
    def measured_intensity(self):
        distance = get_norm(self.get_measurement_point() - self.light_source.get_source_point())
...
class UpdateLightIndicator(AnimationGroup):
    def __init__(self, indicator, intensity, **kwargs):
        ...
        change_opacity = Transform(indicator.foreground, target_foreground)
...
class ThreeDSpotlight(VGroup):
    ...
    def update(self):
        ...
        distance = get_norm(screen.get_center() - source_point)
...
class ThinkAboutPondScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(randy.change, "thinking", ShowCreation(bubble))
        self.wait(2)
...
class IntroScene(PiCreatureScene):
    ...
    def build_up_euler_sum(self):
        ...
        terms = [1.0 / (n**2) for n in range(1, max_n + 1)]
        series_terms = list(np.cumsum(terms))
        series_terms.append(np.pi**2 / 6)  ##Just force this up there

        partial_sum_decimal = self.partial_sum_decimal = DecimalNumber(series_terms[1], num_decimal_places=2)
        ...
        for i, t1, t2 in zip(it.count(1), [0] + series_terms, series_terms):
            ...
            rect = Rectangle(stroke_width=0, fill_opacity=1, fill_color=color)
            ...
            if i <= 5:
                ...
                rect_label_anim = ReplacementTransform(term_mobject.copy(), rect_label)
            else:
        ...
        self.play(rect_anims[0], rect_label_anims[0])
        for i in range(4):
            self.play(
            ...
            if i == 0:
                anims += [FadeIn(equals_sign), FadeIn(partial_sum_decimal)]
            elif i <= 5:
                anims += [
        ...
        for i in range(4, len(series_terms) - 2):
            ...
            self.play(*anims, run_time=2.0 / i)
        ...
        self.number_line_group = VGroup(number_line, rects, rect_labels)
    ...
    def show_history(self):
        ...
        self.play(
            ...
            LEFT,
            LARGE_BUFF,
            self.number_line_group.next_to,
            FRAME_Y_RADIUS * DOWN,
            DOWN,
            morty.change,
            "pondering",
    ...
    def other_pi_formulas(self):
        self.play(FadeOut(self.rects), FadeOut(self.number_line))
    ...
    def refocus_on_euler_sum(self):
        ...
        self.play(
            ...
            ApplyMethod(self.euler_sum.shift, ORIGIN + 2 * UP - self.euler_sum.get_center()),
        )
        ...
        self.pi_creature_thinks(thought, target_mode="confused", bubble_config={"height": 2, "width": 3})
...
class MathematicalWebOfConnections(PiCreatureScene):
    ...
    def complain_that_pi_is_not_about_circles(self):
        ...
        words = self.words = OldTexText("I am not", "fundamentally \\\\", "about circles")
        words.set_color_by_tex("fundamentally", YELLOW)

        self.play(PiCreatureSays(jerk, words, target_mode="angry"))
    ...
    def show_other_pi_formulas(self):
        ...
        for formula in formulas[1:]:
            self.play(
                FadeIn(formula, lag_ratio=0.5, run_time=3),
            )
        self.wait()
    ...
    def show_web_of_connections(self):
        ...
        path_edges = VGroup(
            *[Line(d1.get_center(), d2.get_center(), color=RED) for d1, d2 in zip(path_dots, path_dots[1:])]
        )
        ...
        for dot, edge in zip(path_dots[1:], path_edges):
            self.play(ShowCreation(edge), dot.set_color, RED)
...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        ...
        lighthouses = self.lighthouses = VGroup(*[ls.lighthouse for ls in light_sources[: NUM_VISIBLE_CONES + 1]])
        ...
        self.play(morty.change, "pondering", LaggedStartMap(FadeIn, lighthouses, run_time=1))
        self.play(
            LaggedStartMap(
                SwitchOn,
                VGroup(*[ls.ambient_light for ls in light_sources]),
                run_time=5,
                lag_ratio=0.1,
                rate_func=rush_into,
            ),
    ...
    def describe_brightness_of_each(self):
        ...
        intensities = np.cumsum(np.array([1.0 / n**2 for n in range(1, NUM_CONES + 1)]))
        ...
        triangle_anim = GrowFromPoint(triangle, triangle.get_right(), point_color=YELLOW)
        ...
        self.play(UpdateLightIndicator(light_indicator, 1), FadeIn(euler_sum_terms[0]))
        self.wait(2)

        # Second lighthouse is 1/4, third is 1/9, etc.
        for i in range(1, 5):
            self.play(
                ApplyMethod(ls0.move_to, light_sources[i], run_time=3),
                UpdateLightIndicator(light_indicator, 1.0 / (i + 1) ** 2, run_time=3),
                FadeIn(
                    euler_sum_terms[i],
                    run_time=3,
            ...
            self.wait()
        self.play(ApplyMethod(ls0.restore), UpdateLightIndicator(light_indicator, 1))
        ...
        self.play(
            LaggedStartMap(
                SwitchOn,
                VGroup(*[ls.ambient_light for ls in light_sources[1:]]),
                run_time=5,
                rate_func=rush_into,
            ),
            Animation(lighthouses),
...
class ThatJustSeemsUseless(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.teacher.change, "guilty", self.change_students(*3 * ["sassy"]))
        self.wait()
...
class IntroduceScreen(Scene):
    ...
    def setup_elements(self):
        ...
        self.play(Write(screen_label), GrowArrow(screen_arrow), FadeIn(screen))
        self.wait()
    ...
    def setup_angle(self):
        ...
        self.angle_indicator = DecimalNumber(arc_angle / DEGREES, num_decimal_places=0, unit="^\\circ")
        ...
        self.angle_indicator.add_updater(lambda d: d.set_value(angle_update_func()))
        self.add(self.angle_indicator)

        arc_tracker = AngleUpdater(self.angle_arc, self.light_source.spotlight)
        self.add(arc_tracker)

        self.play(ShowCreation(self.angle_arc), ShowCreation(self.angle_indicator))

        self.wait()

    def rotate_screen(self):
        self.add(
            Mobject.add_updater(self.light_source, lambda m: m.update()),
        )
        self.add(
            Mobject.add_updater(
    ...
    def shoot_rays(self, show_creation_kwargs=None):
        ...
        rays = VGroup(
            *[
                ...
                for angle in np.arange(self.min_ray_angle, self.max_ray_angle, step_size)
            ]
        )
        rays.shift(source_point)
        rays.set_stroke(YELLOW, 1)
        max_angle = np.max([angle_of_vector(point - source_point) for point in screen.get_points()])
        min_angle = np.min([angle_of_vector(point - source_point) for point in screen.get_points()])
        for ray in rays:
        ...
        self.play(*[ShowCreation(ray, run_time=3, **show_creation_kwargs) for ray in rays])
        self.play(*[ApplyMethod(ray.set_color, ray.target_color) for ray in rays])
        self.wait()
...
class EarthScene(IntroduceScreen):
    ...
    def construct(self):
        ...
        screen.target.rotate(-60 * DEGREES, about_point=earth_circle.get_center())
        ...
        pole_arrow.shift(screen.target.get_center() + SMALL_BUFF * LEFT - pole_arrow.get_end())
        ...
        sunlight.opacity_function = lambda r: 5.0 / (r + 1)
        ...
        self.play(Write(equator_words), GrowArrow(equator_arrow))
        self.add_foreground_mobjects(equator_words, equator_arrow)
        self.shoot_rays(show_creation_kwargs={"rate_func": lambda t: interpolate(0.98, 1, smooth(t))})
        self.wait()
        # Point to patch
        self.play(
            MoveToTarget(screen),
...
class ShowLightInThreeDimensions(IntroduceScreen, ThreeDScene):
    ...
    def construct(self):
        ...
        cone = ThreeDSpotlight(screen, light_source.ambient_light, light_source.get_source_point)
        ...
        self.wait(2)
        self.play(screen.stretch, 0.5, 1, screen.stretch, 2, 2, **kwargs)
        self.play(screen.stretch, 3, 1, screen.stretch, 0.7, 2, **kwargs)
        self.wait(2)
...
class InverseSquareLaw(ThreeDScene):
    ...
    def move_screen_farther_away(self):
        ...
        screen.shift(source_point + unit_distance * RIGHT - screen.get_reference_point())
        ...
        shadow_update = Mobject.add_updater(shadow, lambda m: light_source.update_shadow())
        ...
        light_indicator_update = Mobject.add_updater(light_indicator, update_light_indicator)
        light_indicator_update.update(0)

        continual_updates = self.continual_updates = [
            spotlight_update,
            light_indicator_update,
            shadow_update,
        ]
        ...
        self.play(SwitchOn(ambient_light), morty.change, "pondering")
        self.play(SwitchOn(spotlight), FadeIn(light_indicator))
        ...
        self.play(Write(q_marks), morty.change, "confused", q_marks)
        self.play(Blink(morty))
    ...
    def morph_into_3d(self):
        ...
        cone = ThreeDSpotlight(new_screen, ambient_light, source_point_func=lambda: source_point)
        cone_update_anim = ContinualThreeDLightConeUpdate(cone)

        self.remove(self.spotlight_update, self.light_indicator_update)
        self.add(ContinualAnimation(new_screen), cone_update_anim)
        self.remove(spotlight)
        self.move_camera(
            phi=60 * DEGREES,
            theta=-145 * DEGREES,
            added_anims=[
                ...
                #     run_time = 2,
                # ),
                ApplyFunction(lambda m: m.fade(1).shift(1.5 * DOWN), light_indicator, remover=True),
                FadeOut(morty),
            ],
            run_time=2,
        )
        ...
        self.play(*[ApplyMethod(mob.fade, 1) for mob in screen_copy_groups[:2]])
    ...
    def shift_by_distance(self, distance, *added_anims):
        anims = [
            self.screen.shift,
            self.unit_distance * distance * RIGHT,
        ]
...
class OtherInstanceOfInverseSquareLaw(Scene):
    def construct(self):
        ...
        self.play(
            ...
            Succession(*[get_broadcast() for x in range(2)]),
        )
...
class ManipulateLightsourceSetups(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                ...
                lambda ls, a: ls.move_to(
                    observer_point + rotate_vector(unit_distance * RIGHT, (1 + 1.0 / 8) * a * TAU)
                ),
                run_time=6,
        ...
        self.play(
            ReplacementTransform(light_source, light_source_copies[0]),
            ReplacementTransform(light_source.copy().fade(1), light_source_copies[1]),
            FadeIn(plus),
            UpdateFromFunc(
...
class TwoLightSourcesScene(ManipulateLightsourceSetups):
    ...
    def construct(self):
        ...
        morty.target.next_to(origin_point, LEFT, buff=0, submobject_to_align=morty.target.eyes[1])
        ...
        perp_mark = VMobject().set_points_as_corners([RIGHT, RIGHT + DOWN, DOWN])
        ...
        def update_indicator(indicator):
            ...
            for ls in lsA, lsB, lsC:
                if ls in self.mobjects:
                    ...
                    d_indensity = fdiv(3.0 / (distance**2), indicator.opacity_for_unit_intensity)
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(indicator, lambda i, a: i.set_intensity(a * intensity)),
        )
        ...
        # Draw line
        self.play(ShowCreation(line_h), morty.change, "pondering")
        self.wait()
        self.play(ShowCreation(line_c), ShowCreation(perp_mark))
        self.wait()
        ...
        for ls in lsA, lsB:
            ...
            ls.fade(1)
            self.add(ls)
            self.play(ls.restore, run_time=2)
        self.wait()
        A_label.save_state()
        A_label.center().fade(1)
        self.play(A_label.restore)
        self.wait()
        self.play(ReplacementTransform(A_label.copy().fade(1), B_label))
        self.wait(2)
        ...
        self.wait()
        self.play(Write(theorem_name), ShowCreation(theorem_box))
...
class MathologerVideoWrapper(Scene):
    def construct(self):
        ...
        self.play(logo.shift, FRAME_WIDTH * LEFT, LaggedStartMap(FadeIn, title), run_time=2)
        self.play(ShowCreation(screen))
        self.wait(5)
...
class SimpleIPTProof(Scene):
    def construct(self):
        ...
        for arrow, line in zip(argument_lines[1::2], argument_lines[2::2]):
            line.save_state()
            line.shift(prev_line.alignment_mob.get_center() - line.alignment_mob.get_center())
            line.fade(1)
            self.play(line.restore, GrowFromPoint(arrow, arrow.get_top()))
            self.wait()
            prev_line = line
...
class WeCanHaveMoreFunThanThat(TeacherStudentsScene):
    def construct(self):
        point = VectorizedPoint(FRAME_X_RADIUS * LEFT / 2 + FRAME_Y_RADIUS * UP / 2)
        self.teacher_says("We can have \\\\ more fun than that!", target_mode="hooray")
...
class IPTScene(TwoLightSourcesScene, ZoomedScene):
    ...
    def construct(self):
        ...
        spotlights_update_anim = Mobject.add_updater(spotlights, update_spotlights)
        ...
        for x in range(2):
            self.play(*[Transform(m, m.alt_version) for m in m_hyps])
            self.wait()
        ...
        def show_key_point(spotlight, new_point):
            ...
            show_beaming_light(spotlight)
            self.wait()
            self.play(lsC.move_source_to, H, screen.restore, update_spotlight_anim, run_time=2)
            self.wait()

        self.remove(spotlights_update_anim)
        self.add(spotlight_b)
        self.play(*list(map(FadeOut, [spotlight_a, lsA.ambient_light, lsB.ambient_light])))
...
class DiameterTheorem(TeacherStudentsScene):
    def construct(self):
        ...
        triangle_update_anim = Mobject.add_updater(triangle, update_triangle)
        ...
        self.play(ShowCreation(perp_mark), self.change_students(*["pondering"] * 3))
        ...
        for angle in 0.2 * TAU, -0.4 * TAU, 0.3 * TAU:
            ...
            perp_mark.target.shift(point.target.get_center() - perp_mark.target[1].get_center())
    ...
    def construct(self):
        ...
        def update_shape(shape):
            shape.set_points_as_corners(
                [
                    point.get_center(),
                    circle.point_from_proportion(7.0 / 8),
                    circle.get_center(),
                    circle.point_from_proportion(1.0 / 8),
                    point.get_center(),
                ]
            )

        shape_update_anim = Mobject.add_updater(shape, update_shape)
        ...
        for angle in 0.25 * TAU, -0.4 * TAU, 0.3 * TAU, -0.35 * TAU:
            ...
            half_angle_mark.target.shift(point.target.get_center() - half_angle_mark.target[1].get_center())
    ...
    def construct(self):
        ...
        def zoom_out_scene(factor):
            ...
            self.play(
                ...
                self.zoomable_mobs.shift,
                self.obs_dot.get_center(),
                self.unzoomable_mobs.scale,
                2,
                {"about_point": ORIGIN},
            )
        ...
        def shift_scene(v):
            self.play(self.zoomable_mobs.shift, v, self.unzoomable_mobs.shift, v)
        ...
        indicator = LightIndicator(precision=2, radius=INDICATOR_RADIUS, show_reading=False, color=LIGHT_COLOR)
        ...
        self.play(DrawBorderThenFill(lake0, stroke_width=1), Write(lake_word))
        self.play(
            lake0.restore,
            lake_word.scale,
            0.5,
            {"about_point": lake0.get_bottom()},
            lake_word.fade,
            1,
        )
        self.remove(lake_word)
        self.play(morty.restore)
        self.play(GrowFromCenter(obs_dot), GrowFromCenter(ls0_dot), FadeIn(ls0.lighthouse))
        ...
        self.play(
            lake0.set_stroke,
            {"color": LAKE_STROKE_COLOR, "width": LAKE_STROKE_WIDTH},
        )
        self.wait()
        ...
        self.play(
            ...
            ReplacementTransform(new_diameter_text_copy, new_reading, parth_arc=30 * DEGREES),
        )
        ...
        def split_light_source(i, step, show_steps=True, animate=True, run_time=1):
            ...
            if show_steps == True:
                self.play(ShowCreation(hyp, run_time=run_time))
            ...
            if show_animation or animate:
                ...
                ls1.fade(1)
                self.play(MoveToTarget(ls1), MoveToTarget(ls2), run_time=run_time)
            else:
                ls1.move_source_to(ls_new_loc1)
                ls2.move_source_to(ls_new_loc1)

        def construction_step(n, show_steps=True, run_time=1, simultaneous_splitting=False):
            ...
            new_outer_lake = Circle(
                radius=self.lake_radius,
                stroke_width=LAKE_STROKE_WIDTH,
                fill_color=LAKE_COLOR,
                fill_opacity=LAKE_OPACITY,
                stroke_color=LAKE_STROKE_COLOR,
            ...
            if show_steps == True:
                self.play(FadeIn(new_outer_lake, run_time=run_time), FadeIn(self.ls0_dot))
            else:
                self.play(
            ...
            if simultaneous_splitting == False:
                for i in range(2**n):
                    ...
                    if n == 1 and i == 0:
                        ...
                        triangle1 = triangle(A, C, B)
                        right_angle1 = right_angle(A, C, B, size=2 * RIGHT_ANGLE_SIZE)

                        self.play(FadeIn(triangle1), FadeIn(right_angle1))

                        self.wait()

                        self.play(FadeOut(triangle1), FadeOut(right_angle1))

                        self.wait()

                        H = self.inner_lake.get_center() + self.lake_radius / 2 * RIGHT
                        L = self.outer_lake.get_center()
                        triangle2 = triangle(L, H, C)

                        right_angle2 = right_angle(L, H, C, size=2 * RIGHT_ANGLE_SIZE)

                        self.play(FadeIn(triangle2), FadeIn(right_angle2))

                        self.wait()

                        self.play(FadeOut(triangle2), FadeOut(right_angle2))
        ...
        self.play(FadeIn(angle_sign1), FadeIn(my_triangle))
        ...
        self.play(FadeIn(angle_sign2))

        self.wait()

        self.play(FadeOut(angle_sign1), FadeOut(angle_sign2), FadeOut(my_triangle))
        ...
        self.play(FadeOut(self.altitudes), FadeOut(self.hypotenuses), FadeOut(self.legs))
        ...
        for n in range(3, max_it + 1):
            ...
            for i in range(2**n):
                ...
                shift_list += (
                    self.light_sources.submobjects[i + 2**n].move_source_to,
                    pos2,
                )
        ...
        open_sea = (
            ...
            .flip()
            .next_to(origin_point, UP, buff=0)
        )
        ...
        self.play(
            indicator.move_to,
            origin_point + scale * UP,
            indicator_reading.move_to,
            origin_point + scale * UP,
            FadeOut(open_sea),
            FadeOut(morty),
...
class ThinkBackToHowAmazingThisIs(ThreeDScene):
    ...
    def show_sum(self):
        ...
        positive_dots, negative_dots = [
            VGroup(*[Dot(number_line.number_to_point(u * x)) for x in range(1, int(self.x_radius), 2)]) for u in (1, -1)
        ]
        ...
        decimal.to_edge(UP)
        terms = [2.0 / (n**2) for n in range(1, 100, 2)]
        ...
        plusses = VGroup(
            *[
                VGroup(
                    *[
                        OldTex("+").next_to(
                            number_line.number_to_point(u * n),
                            UP,
                            buff=1.25,
                        )
                        for u in (-1, 1)
        ...
        zoom_out = always_shift(self.camera.rotation_mobject, direction=OUT, rate=0.4)
        ...
        self.play(
            *list(
                map(
                    FadeOut,
                    it.chain(term_mobjects, plusses, number_line.numbers, [decimal]),
                )
            )
        )
    ...
    def show_giant_circle(self):
        ...
        circle.apply_function(lambda x_y_z: complex_to_R3(7 * np.exp(complex(0, 0.0315 * x_y_z[0]))))
...
class FinalSumManipulationScene(PiCreatureScene):
    def construct(self):
        ...
        result1 = OldTex("{\pi^2\over 8} =", fill_color=LIGHT_COLOR, stroke_color=LIGHT_COLOR)
        ...
        self.play(LaggedStartMap(SwitchOn, full_ambient_lights, lag_ratio=0.2, run_time=3))
        ...
        for ls, term in zip(even_lights.submobjects, even_terms.submobjects):
            self.play(SwitchOn(ls.ambient_light, run_time=switch_on_time), Write(term))
        self.wait()
        ...
        self.play(Transform(even_lights, full_lights, run_time=2))

        self.wait()

        for i in range(6):
            self.play(Transform(even_lights[i], even_lights_copy[i]))
            self.wait()
        ...
        quarter_arrow = Arrow(Q1, Q2, color=LIGHT_COLOR2)
        ...
        three_quarters_arrow = Arrow(R1, R2, color=LIGHT_COLOR)
        ...
        self.play(ShowCreation(three_quarters_arrow), Write(three_quarters_label))
        self.wait()
        ...
        for i in range(1, 8):  # full_range:
            if i == 1:
                term = OldTex(
                    "\phantom{+\,\,\,}{1\over " + str(i) + "^2}",
                    fill_color=LIGHT_COLOR3,
                    stroke_color=LIGHT_COLOR3,
                )
            elif i == 7:
        ...
        self.play(
            ...
            result1.shift,
            v,
            four_thirds_arrow.shift,
            v,
            four_thirds_label.shift,
            v,
            odd_terms.shift,
            v,
            full_terms.shift,
        ...
        result_box = Rectangle(
            width=1.1 * equation.get_width(),
            height=2 * equation.get_height(),
            color=LIGHT_COLOR3,
        )
    ...
    def construct(self):
        ...
        def flash_arcs(n):
            ...
            arcs.append(
                LabeledArc(
                    angle / 2,
                    start_angle=-TAU / 4 - angle / 2,
                    radius=LAKE0_RADIUS,
                    length=1,
                )
            )

            self.play(FadeIn(arcs[0], run_time=FLASH_TIME))
...
class InfiniteCircleScene(PiCreatureScene):
    def construct(self):
        ...
        self.wait()
        self.play(LaggedStartMap(FadeIn, infsum, lag_ratio=0.2))
        self.wait()

        A = infsum.submobjects[-1].get_center() + 0.5 * RIGHT
        B = A + RIGHT + 1.3 * UP + 0.025 * LEFT
        right_arc = DashedLine(TAU / 4 * UP, ORIGIN, stroke_color=YELLOW, stroke_width=8).apply_complex_function(np.exp)
        ...
        C = B + 3.2 * UP
        right_line = DashedLine(B + 0.2 * DOWN, C + 0.2 * UP, stroke_color=YELLOW, stroke_width=8)
        ...
        up_line = DashedLine(D, E, stroke_color=YELLOW, stroke_width=8)
