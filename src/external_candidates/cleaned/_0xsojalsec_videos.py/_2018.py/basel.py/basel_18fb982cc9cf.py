        ...
        for i in range(5, max_n1):
            if i == 5:
                ...
                self.play(
                    ...
                    GrowFromPoint(self.rects[i], self.euler_sum[10].get_center(), run_time=0.5),
                )
            else:
    ...
    def show_lighthouses_on_number_line(self):
        ...
        for i, light_source in zip(
            ...
            indicator_start_time = (
                0.5 * (i + 1) * FAST_SWITCH_ON_RUN_TIME / light_source.radius * self.number_line.unit_size
            )
...
class BackToEulerSumScene(PiCreatureScene):
    def construct(self):
        ...
        for i in range(NUM_VISIBLE_CONES, NUM_CONES):
            indicator_start_time = (
                0.5 * (i + 1) * FAST_SWITCH_ON_RUN_TIME / light_source.ambient_light.radius * self.number_line.unit_size
            )
            indicator_stop_time = indicator_start_time + FAST_INDICATOR_UPDATE_TIME
            indicator_rate_func = squish_rate_func(  # smooth, 0.8, 0.9)
                smooth, indicator_start_time, indicator_stop_time
