...
class MinimizeSharding(Scene):
    def construct(self):
        piece_groups = VGroup(
            *[VGroup(*[self.get_piece() for x in range(3)]).arrange(RIGHT, buff=SMALL_BUFF) for y in range(4)]
...
class TopologyWordBreak(Scene):
    def construct(self):
        ...
        genus_labels = VGroup(
            *[OldTexText("Genus %d:" % d).scale(1.5).next_to(classes[d], LEFT, MED_LARGE_BUFF) for d in range(3)]
        )
...
class FunctionGInputSpace(SpecialThreeDScene):
    ...
    def get_start_path(self):
        path = ParametricCurve(
            lambda t: np.array([-np.sin(TAU * t + TAU / 4), np.cos(2 * TAU * t + TAU / 4), 0]),
            color=RED,
        )
        path.scale(0.5)
        path.shift(0.5 * OUT)
...
class FunctionGOutputSpace(FunctionGInputSpace):
    ...
    def deform_towards_north_pole(self):
        ...
        self.play(
            ...
            UpdateFromFunc(
                flat_eq,
                lambda m: m.become(equator.deepcopy().apply_function(self.odd_func).set_stroke(color=RED, width=3)),
            ),
        )
        self.wait()
...
class DivisionOfUnity(Scene):
    def construct(self):
        ...
        v_lines = VGroup(
            *[DashedLine(0.2 * UP, 0.2 * DOWN).shift(factor * v) for v in [LEFT, 0.3 * LEFT, 0.1 * RIGHT, RIGHT]]
        )
