# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\alt_calc.py
# -*- coding: utf-8 -*-
...
class NumberlineTransformationScene(ZoomedScene):
    ...
    def setup_titles(self):
        input_title, output_title = self.titles = VGroup(*[OldTexText(word) for word in ("Inputs", "Outputs")])
        vects = [UP, DOWN]
    ...
    def get_sample_input_points(self, x_min=None, x_max=None, delta_x=None):
        ...
        return [self.get_input_point(x) for x in np.arange(x_min, x_max + delta_x, delta_x)]

    def get_sample_dots(self, x_min=None, x_max=None, delta_x=None, dot_radius=None, colors=None):
        dot_radius = dot_radius or self.default_sample_dot_radius
        colors = colors or self.default_sample_dot_colors

        dots = VGroup(*[Dot(point, radius=dot_radius) for point in self.get_sample_input_points(x_min, x_max, delta_x)])
    ...
    # Mapping animations
    def get_mapping_animation(self, func, mobject, how_to_apply_func=apply_function_to_center, **kwargs):
    ...
    def get_line_mapping_animation(self, func, **kwargs):
        ...
        return AnimationGroup(
            self.get_mapping_animation(func, input_line_copy, apply_function_to_points),
            self.get_mapping_animation(func, input_line_copy.tick_marks, apply_function_to_submobjects),
        )

    def get_sample_dots_mapping_animation(self, func, dots, **kwargs):
        return self.get_mapping_animation(func, dots, how_to_apply_func=apply_function_to_submobjects)
    ...
    def apply_function(
        ...
        **kwargs,
        ...
        if hasattr(self, "mini_line"):  # Test for if mini_line is in self?
            anims.append(self.get_mapping_animation(func, self.mini_line, how_to_apply_func=apply_function_to_center))
        if sample_dots:
            anims.append(self.get_sample_dots_mapping_animation(func, sample_dots))
        ...
        if target_coordinate_values:
            coordinates = self.get_local_coordinates(self.output_line, *target_coordinate_values)
            anims.append(FadeIn(coordinates))
            zcbr_group.add(coordinates)
            self.local_target_coordinates = coordinates
        if local_sample_dots:
            anims.append(self.get_sample_dots_mapping_animation(func, local_sample_dots))
    ...
    def zoom_in_on_input(
        ...
        if zoom_factor:
            frame.target.set_height(self.zoomed_display.get_height() * zoom_factor)
        ...
        local_coordinates = self.get_local_coordinates(self.input_line, *local_coordinate_values)
    ...
    def get_local_coordinates(self, line, *x_values, **kwargs):
        num_decimal_places = kwargs.get("num_decimal_places", self.local_coordinate_num_decimal_places)
        result = VGroup()
...
class ExampleNumberlineTransformationScene(NumberlineTransformationScene):
    CONFIG = {
        "number_line_config": {"x_min": 0, "x_max": 5, "unit_size": 2.0},
        "output_line_config": {
            "x_max": 20,
        },
    }
...
class WriteOpeningWords(Scene):
    def construct(self):
        ...
        raw_string2 = (
            "You're about to go through your first course. Like "
            + "any new topic, it will take some hard work to understand,"
        )
        words1, words2 = [OldTexText("\\Large", *rs.split(" ")) for rs in (raw_string1, raw_string2)]
...
class StartingCalc101(PiCreatureScene):
    ...
    def show_mystery_topic(self):
        ...
        for image in images:
            ...
            self.play(image.shift, DOWN, image.fade, 1, randy.change, "erm", run_time=1.5)
            self.remove(image)
        self.wait()
    ...
    def adjust_size(self, group):
        group.set_width(min(group.get_width(), self.image_frame_width - 2 * MED_SMALL_BUFF))
        group.set_height(min(group.get_height(), self.image_frame_height - 2 * MED_SMALL_BUFF))
        return group
    ...
    def get_neat_example_image(self):
        filled_circle = Circle(stroke_width=0, fill_color=BLUE_E, fill_opacity=1)
        area = OldTex("\\pi r^2")
        ...
        return VGroup(filled_circle, area, unfilled_circle, circumference, equation)
    ...
    def get_physics_image(self):
        ...
        group.continual_animations = [
            t_tracker.add_udpater(lambda tracker, dt: tracker.set_value(tracker.get_value() + dt)),
            spring.add_updater(
                lambda s: s.stretch_to_fit_height(1.5 + 0.5 * np.cos(3 * t_tracker.get_value()), about_edge=UP)
            ),
            weight.add_updater(lambda w: w.move_to(spring.get_points()[-1])),
        ]
        ...
        group.fade_in_anim = UpdateFromAlphaFunc(group, lambda g, a: update_group_style(a))
        group.fade_out_anim = UpdateFromAlphaFunc(group, lambda g, a: update_group_style(1 - a))
        return group
    ...
    def get_aha_image(self):
        ...
        scene = NudgeSideLengthOfCube(end_at_animation_number=7, skip_animations=True)
        group = VGroup(
            scene.cube,
            scene.faces,
            scene.bars,
    ...
    def get_graphical_intuition_image(self):
        ...
        rects = gs.get_riemann_rectangles(graph, x_min=2, x_max=8, stroke_width=0.5, dx=0.25)
...
class GraphicalIntuitions(GraphScene):
    ...
    def construct(self):
        ...
        rects = self.get_riemann_rectangles(graph, x_min=2, x_max=8, dx=0.01, stroke_width=0)
...
class ChangingVectorField(Scene):
    ...
    def update_vectors(self, vectors):
        ...
        for vector in vectors:
            ...
            vector.put_start_and_end_on(point, point + new_x * RIGHT + new_y * UP)

    def func(self, point, time):
        x, y, z = point
        return np.array([np.sin(time + 0.5 * x + y), np.cos(time + 0.2 * x * y + 0.7), 0])
...
class MoreTopics(Scene):
    def construct(self):
        ...
        lines = VGroup(*[Line(calculus.get_right(), word.get_left(), buff=MED_SMALL_BUFF) for word in others])
...
class SetTheStage(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_holds_up(ordinary, added_anims=[self.change_students(*3 * ["sassy"])])
        self.wait()
        self.play(
            ordinary.shift,
            UP,
...
class StandardDerivativeVisual(GraphScene):
    ...
    def show_function_graph(self):
        ...
        input_triangle_update = input_tracker.add_updater(lambda m: m.move_to(get_x_point(), UP))
        output_triangle_update = output_triangle.add_updater(lambda m: m.move_to(get_y_point(), RIGHT))

        x_label = OldTex("x")
        x_label_update = Mobject.add_updater(x_label, lambda m: m.next_to(input_triangle, DOWN, SMALL_BUFF))

        output_label = OldTex("f(x)")
        output_label_update = Mobject.add_updater(output_label, lambda m: m.next_to(output_triangle, LEFT, SMALL_BUFF))

        v_line = get_v_line()
        v_line_update = Mobject.add_updater(v_line, lambda vl: Transform(vl, get_v_line()).update(1))

        h_line = get_h_line()
        h_line_update = Mobject.add_updater(h_line, lambda hl: Transform(hl, get_h_line()).update(1))

        graph_dot = Dot(color=YELLOW)
        graph_dot_update = Mobject.add_updater(graph_dot, lambda m: m.move_to(get_graph_point()))
        ...
        self.play(input_tracker.set_value, 8, run_time=6, rate_func=there_and_back)
    ...
    def show_slope_of_graph(self):
        ...
        slope_line_update = Mobject.add_updater(slope_line, lambda sg: Transform(sg, get_slope_line()).update(1))

        def position_deriv_label(deriv_label):
            deriv_label.next_to(slope_line, UP)
            return deriv_label

        deriv_label = OldTex("\\frac{df}{dx}(x) =", "\\text{Slope}", "=")
        deriv_label.get_part_by_tex("Slope").match_color(slope_line)
        deriv_label_update = Mobject.add_updater(deriv_label, position_deriv_label)

        slope_decimal = DecimalNumber(slope_line.get_slope())
        slope_decimal.match_color(slope_line)
        slope_decimal.add_updater(lambda d: d.set_value(slope_line.get_slope()))
        slope_decimal.add_upater(lambda d: d.next_to(deriv_label, RIGHT, SMALL_BUFF).shift(0.2 * SMALL_BUFF * DOWN))
        ...
        for x in 9, 2, 4:
            self.play(input_tracker.set_value, x, deriv_input_tracker.set_value, x, run_time=3)
            self.wait()
    ...
    def encourage_not_to_think_of_slope_as_definition(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(morty, lambda m, a: m.set_fill(opacity=1 - a), remover=True),
        )
    ...
    def wiggle_input(self, dx=0.5, run_time=3):
        ...
        x_min = x - dx
        x_max = x + dx
        y, y_min, y_max = list(map(self.graph.underlying_function, [x, x_min, x_max]))
        x_line = Line(
...
class IntroduceTransformationView(NumberlineTransformationScene):
    CONFIG = {
        ...
        "number_line_config": {"x_min": 0, "x_max": 6, "unit_size": 2.0},
    }
    ...
    def show_animation_preview(self):
        ...
        output_points = list(map(self.number_func_to_point_func(self.func), input_points))
        sample_dots = self.get_sample_dots()
        sample_dot_ghosts = sample_dots.copy().fade(0.5)
        arrows = VGroup(*[Arrow(ip, op, buff=MED_SMALL_BUFF) for ip, op in zip(input_points, output_points)])
        arrows = arrows[1::3]
        ...
        for sd in sample_dots:
            ...
            sd.scale(2)
            sd.fade(1)
        self.play(LaggedStartMap(ApplyMethod, sample_dots, lambda sd: (sd.restore,), run_time=2))
        self.play(
            LaggedStartMap(
                GrowArrow,
                arrows,
    ...
    def indicate_indicate_point_densities(self):
        ...
        new_title = OldTexText("$\\frac{df}{dx}(x)$ measures stretch/squishing")
...
class TalkThroughXSquaredExample(IntroduceTransformationView):
    ...
    def show_specific_points_mapping(self):
        ...
        for dot, dot_ghost, arrow in zip(int_dots, int_dot_ghosts, int_arrows):
            ...
            self.add(dot_ghost)
            self.play(GrowArrow(arrow), dot.apply_function_to_position, point_func)
        self.wait()
    ...
    def get_stretch_words(self, factor, color=RED, less_than_one=False):
        ...
        result = OldTexText("Scale \\\\ by", factor_str, tex_to_color_map={factor_str: color})
        result.scale(0.7)
...
class ZoomInOnXSquaredNearOne(TalkThroughXSquaredExample):
    ...
    def construct(self):
        ...
        new_local_coordinates = self.get_local_coordinates(self.input_line, *new_coordinate_values)
        ...
        self.play(
            ...
            *[ApplyMethod(dot.scale, extra_zoom_factor) for dot in local_sample_dots],
        )
        self.remove(one_group, local_sample_dots)
        zcbr_group.remove(self.local_coordinates, self.local_target_coordinates, local_sample_dots)
...
class ZoomInOnXSquaredNearZero(ZoomInOnXSquaredNearOne):
    ...
    def construct(self):
        zoom_words = OldTexText("Zoomed %sx \\\\ near 0" % "{:,}".format(int(1.0 / self.zoom_factor)))
        zoom_words.next_to(self.zoomed_display, DOWN)

        x = 0
        local_sample_dots = self.get_local_sample_dots(x, sample_radius=2 * self.zoomed_camera.frame.get_width())
        local_coordinate_values = self.get_local_coordinate_values(x, dx=self.zoom_factor)
        # target_coordinate_values = self.get_local_coordinate_values(
        ...
        color = self.sample_dots[len(self.sample_dots) / 2].get_color()
        sample_dot_ghost_copies = self.sample_dot_ghosts.copy()
        stretch_words = self.get_stretch_words(self.scale_by_term, color, less_than_one=True)
        deriv_equation = self.get_deriv_equation(x, 2 * x, color)
        ...
        self.apply_function(
            ...
            added_anims=[
                Write(stretch_words),
                MaintainPositionRelativeTo(self.local_coordinates, self.zoomed_camera.frame),
            ],
        )
        self.wait(2)
    ...
    def construct(self):
        ...
        for factor in 0.1, 0.01, 0.001, 0.0001:
            ...
            if last_zoom_words is not None:
                added_anims.append(ReplacementTransform(last_zoom_words, zoom_words))
            else:
...
class XSquaredForNegativeInput(TalkThroughXSquaredExample):
    CONFIG = {
        ...
        "output_line_config": {},
        "default_mapping_animation_config": {"path_arc": 30 * DEGREES},
        "zoomed_display_width": 4,
    }
    ...
    def show_full_transformation(self):
        sample_dots = self.get_sample_dots(x_min=-4.005, delta_x=0.05, dot_radius=0.05)
...
class FeelsALittleCramped(TeacherStudentsScene):
    def construct(self):
        self.student_says("Kind of cramped,\\\\ isn't it?", target_mode="sassy")
        self.wait()
...
class IntroduceContinuedFractionPuzzle(PiCreatureScene):
    ...
    def ask_question(self):
        ...
        anims = [
            RemovePiCreatureBubble(friend, target_mode="pondering", look_at=frac),
        ]
    ...
    def set_equal_to_x(self):
        ...
        inner_frac = frac[4:]
        inner_frac_rect = SurroundingRectangle(inner_frac, stroke_width=2, buff=0.5 * SMALL_BUFF)
        ...
        fixed_point_words = VGroup(
            ...
            OldTex("f(x) = 1 + \\frac{1}{x}", tex_to_color_map={"x": YELLOW}),
        )
        ...
        self.wait()
        self.play(
            inner_frac_group.scale,
            0.75,
            inner_frac_group.center,
            inner_frac_group.to_edge,
            LEFT,
        ...
        self.play(Write(fixed_point_words), morty.change, "hooray", friend.change, "happy")
        self.wait(3)
...
class GraphOnePlusOneOverX(GraphScene):
    ...
    def draw_graphs(self, animate=True):
        ...
        identity_graph = self.get_graph(lambda x: x, color=self.identity_graph_color)
        identity_graph.label = self.get_graph_label(identity_graph, "y = x", x_val=3, direction=UL, buff=SMALL_BUFF)
    ...
    def show_solutions(self):
        ...
        for num in phi, phi_bro:
            ...
            line.fade(0.5)
            line_anim = ShowCreationThenDestruction(line_copy, lag_ratio=0.5, run_time=2)
        ...
        phi_arrow = Arrow(phi_decimal[:4].get_top(), phi_line.get_bottom(), **arrow_kwargs)
        ...
        phi_bro_arrow = Arrow(phi_bro_decimal[:6].get_bottom(), phi_bro_line.get_top(), **arrow_kwargs)
        ...
        brother_words.next_to(phi_bro_decimal[-2], UP, buff=MED_SMALL_BUFF, aligned_edge=RIGHT)
...
class ThinkAboutWithRepeatedApplication(IntroduceContinuedFractionPuzzle):
    ...
    def obviously_not(self):
        ...
        plusses = frac[1::4]
        plus_rects = VGroup(*[SurroundingRectangle(plus, buff=0) for plus in plusses])
    ...
    def plug_func_into_self(self, value=1, value_str="1"):
        ...
        for n in range(1, len(lines)):
            ...
            anims = [ReplacementTransform(mover.copy().fade(1), target, path_arc=30 * DEGREES)]
            if n == 4:
...
class RepeatedApplicationWithNegativeSeed(RepeatedApplicationWithPhiBro, MovingCameraScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(rect))
        self.play(Write(question), self.camera.frame.set_height, FRAME_HEIGHT + 1.5)
        self.wait()
        self.play(FadeOut(question), FadeOut(rect), self.camera.frame.set_height, FRAME_HEIGHT)
...
class ShowRepeatedApplication(Scene):
    ...
    def add_func_title(self):
        title = self.title = VGroup(OldTex("f(", "x", ")"), OldTex("="), get_nested_one_plus_one_over_x(1))
        title.arrange(RIGHT)
        title.to_corner(UL)
    ...
    def show_repeated_iteration(self):
        ...
        rects = VGroup(*[SurroundingRectangle(mob) for mob in (line[0], line[1:-1], line[-1])])
        rects.set_stroke(BLUE, 2)

        braces = VGroup(*[Brace(rect, DOWN, buff=SMALL_BUFF) for rect in rects])
...
class NumericalPlayOnNumberLineFromOne(Scene):
    ...
    def bounce_around(self):
        ...
        self.play(FadeIn(start_here), GrowArrow(arrow), GrowFromPoint(dot, arrow.get_start()))
    ...
    def draw_spider_web(self):
        ...
        for n in range(self.n_jumps):
            ...
            if n < self.n_times_to_show_identity_property:
                ...
                lines = VGroup(*[Line(dot.get_center(), point) for point in (x_point, y_point)])
...
class AnalyzeFunctionWithTransformations(NumberlineTransformationScene):
    ...
    def repeatedly_apply_function(self):
        ...
        for x in range(self.num_initial_applications):
            self.apply_function(self.func, apply_function_to_number_line=False, sample_dots=sample_dots)
            self.wait()
    ...
    def show_phi_and_phi_bro(self):
        ...
        self.play(
            LaggedStartMap(ApplyFunction, self.all_arrows, lambda a: (fade_arrow, a)),
            FadeIn(phi_arrow),
    ...
    def zoom_in_on_phi(self):
        ...
        local_sample_dots = self.get_local_sample_dots(phi, dot_radius=0.005, sample_radius=1)
        ...
        self.wait()
        self.play(
            deriv_text_lhs.next_to,
            approx_value,
            LEFT,
            deriv_text_rhs.next_to,
            approx_value,
            RIGHT,
            FadeIn(approx_value),
        )
...
class StabilityAndInstability(AnalyzeFunctionWithTransformations):
    ...
    def write_derivative_fact(self):
        ...
        for char, label in zip("<>", labels):
            deriv_label = OldTex("\\big|", "\\frac{df}{dx}(", "x", ")", "\\big|", char, "1")
            deriv_label.get_parts_by_tex("\\big|").match_height(deriv_label, stretch=True)
...
class StaticAlgebraicObject(Scene):
    def construct(self):
        ...
        self.add(title)
        self.play(LaggedStartMap(GrowFromCenter, frac, lag_ratio=0.1, run_time=3))
        self.wait()
        factor = 1.1
...
class NotBetterThanGraphs(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[self.change_students(*3 * ["thinking"], look_at=self.teacher.eyes)],
        )
        self.wait(3)
...
class TopicsAfterSingleVariable(PiCreatureScene, MoreTopics):
    ...
    def create_pi_creatures(self):
        creatures = VGroup(*[PiCreature(color=color) for color in [BLUE_E, BLUE_C, BLUE_D]])
...
class ShowJacobianZoomedIn(LinearTransformationScene, ZoomedScene):
    ...
    def construct(self):
        ...
        zd_rect.anim = UpdateFromFunc(zd_rect, lambda rect: rect.replace(zoomed_display).scale(1.1))
...
class ManyInfiniteExpressions(Scene):
    def construct(self):
        ...
        n = 12
        power_tower = OldTex(*["\\sqrt{2}^{"] * n + ["\\dots"] + ["}"] * n)
