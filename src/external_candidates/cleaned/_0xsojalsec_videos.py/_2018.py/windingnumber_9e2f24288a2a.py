    ...
    def solveEquation(self):
        ...
        for i in range(self.num_iterations):
            ...
            if in_negative_branch:
                self.play(
                    UpdateFromAlphaFunc(lowerGroup, makeUpdater(lowerX, fixed_guess_x=upperX)),
                ...
                self.play(
                    UpdateFromAlphaFunc(upperGroup, makeUpdater(upperX, fixed_guess_x=lowerX)),
                    FadeOut(guess_labels),
                )
                upperX = midX
                upperY = midY
    ...
    def construct(self):
        if self.in_background_pass:
            self.camera.set_background_from_func(
                lambda x_y: point_to_rgba(self.pixel_pos_to_color_func((x_y[0], x_y[1])))
            )
...
class PiWalker(ColorMappedByFuncScene):
    ...
    def construct(self):
        ...
        for i in range(len(walk_coords)):
            ...
            val_alpha_func = lambda a, start_coords=start_coords, end_coords=end_coords: self.func(
            ...
            if self.display_wind:
                ...
                number_update_func = (
                    lambda alpha, alpha_winder=alpha_winder, start_wind=start_wind: alpha_winder(alpha)
                    - alpha_winder(0)
                    + start_wind
                )
                ...
                number_update_func = lambda a, val_alpha_func=val_alpha_func: point_to_rescaled_size(
                    val_alpha_func(a)
    ...
    def construct(self):
        ...
        def line_fade(start, end, mob):
            return UpdateFromAlphaFunc(
                mob,
                lambda m, a: m.set_stroke(width=interpolate(start, end, a) * stroke_width),
            )
