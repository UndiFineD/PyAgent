...
class IntroduceHamilton(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(FadeOutAndShiftDown, VGroup(*it.chain(quote, quote_rect, quote_label))),
        )
        self.wait()
...
class HereWeTackle4d(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            "hooray",
            FadeIn(titles[1]),
            ApplyMethod(titles[0].shift, 2 * UP, rate_func=squish_rate_func(smooth, 0.2, 1)),
        )
    ...
    def show_multiplication(self):
        ...
        for i in range(4):
            ...
            self.play(
                ...
                linus.move_to,
                4 * LEFT + DOWN,
                *[ReplacementTransform(part.copy(), part.targets.pop(0)) for part in pair],
            )
    ...
    def show_example_angles(self):
        ...
        for angle in self.example_angles:
            ...
            self.play(
                ...
                ChangingDecimal(angle_label, lambda a: (self.get_angle() % TAU) / DEGREES),
                run_time=4,
            )
            self.wait()
...
class ShowRotationsJustWithReferenceCircles(TwoDStereographicProjection):
    ...
    def show_rotations(self):
        ...
        arrows = VGroup(
            arrow,
            *[arrow.copy().rotate(angle, about_point=ORIGIN) for angle in np.arange(TAU / 4, TAU, TAU / 4)],
        )
...
class ReferernceSpheresFelixView(ShowRotationsJustWithReferenceCircles):
    ...
    def capture_mobjects_in_camera(self, mobjects, **kwargs):
        mobs_on_xy = [
            sm
            for sm in self.camera.extract_mobject_family_members(mobjects, only_those_with_points=True)
...
class SphereProjectionsWrapper(Scene):
    def construct(self):
        rect_rows = VGroup(
            *[VGroup(*[ScreenRectangle(height=3) for x in range(3)]).arrange(RIGHT, buff=LARGE_BUFF) for y in range(2)]
...
class HypersphereStereographicProjection(SpecialThreeDScene):
    ...
    def show_quaternions_with_nonzero_real_part(self):
        ...
        self.set_quat(
            [-1, 1, 2, 0],
            added_anims=[ApplyFunction(lambda s: s.scale(10).fade(1), self.sphere.copy(), remover=True)],
        )
        self.wait(2)
    ...
    def show_multiplication(self):
        ...
        for q in [quat, [1, 0, 0, 0], quat]:
            self.play(
                m_tracker.set_value,
                q,
                UpdateFromFunc(m_tracker, lambda m: m.set_value(normalize(m.get_value()))),
                run_time=5,
            )
            self.wait()
