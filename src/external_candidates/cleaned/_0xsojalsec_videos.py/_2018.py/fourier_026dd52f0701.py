# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\fourier.py
# -*- coding: utf-8 -*-
    ...
    graph.set_points_smoothly(
        [
            axes.coords_to_point(
                x,
                complex_to_real_func(y) / n_samples,
            )
            for x, y in zip(frequencies, fft_output[: n_samples // 2])
        ]
    )
    graph.set_color(color)
    f_min, f_max = [axes.x_axis.point_to_number(graph.get_points()[i]) for i in (0, -1)]
...
def get_fourier_transform(
    ...
    **kwargs,  ##Just eats these
    ...
    scalar = 1.0 / (t_max - t_min) if use_almost_fourier else 1.0

    def fourier_transform(f):
        z = scalar * scipy.integrate.quad(lambda t: func(t) * np.exp(complex(0, -TAU * f * t)), t_min, t_max)[0]
...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        fourier_graph = FunctionGraph(get_fourier_transform(func, 0, 5), x_min=0, x_max=5)
        ...
        self.play(GrowArrow(arrow), ReplacementTransform(graph.copy(), fourier_graph))
        self.wait(2)
        self.student_thinks(
            "Pssht, I got this",
            target_mode="tease",
...
class TODOInsertUnmixingSound(TODOStub):
    CONFIG = {"message": "Show unmixing sound"}
...
class AddingPureFrequencies(PiCreatureScene):
    ...
    def play_A440(self):
        ...
        self.broadcast(
            FadeIn(A_label),
            Succession(ApplyMethod, randy.change, "pondering", Animation, randy, Blink, randy),
        )
    ...
    def measure_air_pressure(self):
        ...
        self.play(FadeIn(pressure), ShowCreation(axes.y_axis))
        self.play(Write(time), ShowCreation(axes.x_axis))
        self.broadcast(
            ShowCreation(graph, run_time=4, rate_func=linear),
            ShowCreation(equilibrium_line),
        )
        axes.add(equilibrium_line)
        self.play(randy.change, "erm", graph, GrowFromCenter(brace), Write(words))
        self.wait()
        graph.save_state()
        self.play(
            FadeOut(brace),
            FadeOut(words),
            VGroup(axes, graph, axes.labels).shift,
            0.8 * UP,
            graph.fade,
            0.85,
            graph.shift,
    ...
    def play_lower_pitch(self):
        ...
        self.set_variables_as_attrs(D_label, D_graph=graph)
    ...
    def play_mix(self):
        ...
        self.broadcast(
            ...
            "thinking",
            *[ShowCreation(graph, run_time=4, rate_func=linear) for graph in (self.A_graph, self.D_graph)],
        )
        self.wait()
    ...
    def separate_out_parts(self):
        ...
        self.play(
            *it.chain(
                ...
                [ApplyMethod(mob.shift, FRAME_Y_RADIUS * DOWN, remover=True) for mob in (randy, speaker)],
            )
        )
        self.wait()
    ...
    def add_more_notes(self):
        ...
        def new_sum_func(x):
            result = sum([graph.underlying_function(x) - self.equilibrium_height for graph in graphs])
            result *= 0.5
            return result + self.equilibrium_height

        new_sum_graph = self.axes.get_graph(new_sum_func, num_graph_points=200)
        ...
        self.play(ReplacementTransform(graphs.copy(), thin_new_sum_graph))
        kwargs = {"rate_func": None, "run_time": 10}
        self.play(
            ShowCreation(new_sum_graph.copy(), **kwargs),
            *[
...
class BreakApartSum(Scene):
    ...
    def show_initial_sound(self):
        def func(x):
            return self.equilibrium_height + 0.2 * np.sum([np.cos(2 * np.pi * f * x) for f in self.frequencies])
    ...
    def decompose_sound(self):
        ...
        self.play(FadeOut(axes), graph.to_edge, UP)
...
class UnmixMixedPaint(Scene):
    ...
    def construct(self):
        ...
        quadrants.add(*it.chain(*[quadrants.copy().rotate(angle) for angle in np.linspace(0, 0.02 * TAU, 10)]))
        ...
        run_time = 30
        self.play(
            *[UpdateFromAlphaFunc(quadrant, update_quadrant) for quadrant in quadrants]
...
class MachineThatTreatsOneFrequencyDifferently(Scene):
    def construct(self):
        ...
        arrow1, q_marks, arrow2 = group = VGroup(Vector(DOWN), OldTexText("???").scale(1.5), Vector(DOWN))
    ...
    def change_graph_frequency(self, frequency, run_time=2):
        ...
        self.play(
            UpdateFromAlphaFunc(
                graph,
                self.get_signal_update_func(graph, frequency),
            ),
            ChangingDecimal(frequency_mob, lambda a: 440 * interpolate(curr_frequency, frequency, a)),
            run_time=run_time,
        )
    ...
    def get_cosine_graph(self, frequency, num_steps=200, color=YELLOW):
        result = FunctionGraph(lambda x: 0.5 * np.cos(2 * np.pi * frequency * x), num_steps=num_steps)
...
class FourierMachineScene(Scene):
    ...
    def get_time_axes(self):
        ...
        time_label.next_to(time_axes.coords_to_point(self.time_label_t, 0), DOWN)
    ...
    def get_frequency_axes(self):
        ...
        frequency_axes.y_axis.add_numbers(*frequency_axes.y_axis.get_tick_numbers())
    ...
    def get_cosine_wave(self, freq=1, shift_val=1, scale_val=0.9):
        return self.get_time_graph(lambda t: shift_val + scale_val * np.cos(TAU * freq * t))
    ...
    def get_polarized_animation(self, mobject, freq=1.0):
        ...
        def update_p_mob(p_mob):
            Transform(p_mob, self.get_polarized_mobject(mobject, freq=freq)).update(1)
    ...
    def animate_frequency_change(self, mobjects, new_freq, **kwargs):
        ...
        self.play(*[self.get_frequency_change_animation(mob, new_freq, **kwargs) for mob in mobjects] + added_anims)
    ...
    def get_polarized_vector_animation(self, polarized_graph, **kwargs):
        ...
        def update_vector(vector, alpha):
            ...
            # jump around
            point = center_of_mass([graph_copy.point_from_proportion(alpha + d) for d in np.linspace(-0.001, 0.001, 5)])
    ...
    def animate_time_sweep(self, freq, n_repeats=1, t_max=None, **kwargs):
        ...
        for x in range(n_repeats):
            ...
            self.play(
                ApplyMethod(v_line.move_to, ctp(t_max, 0), DOWN),
    ...
    def get_v_lines_indicating_periods(self, freq, n_lines=None):
        ...
        period = np.divide(1.0, max(freq, 0.01))
        v_lines = VGroup(
            *[
                DashedLine(ORIGIN, 1.5 * UP).move_to(self.time_axes.coords_to_point(n * period, 0), DOWN)
    ...
    def get_period_v_lines_update_anim(self):
        def update_v_lines(v_lines):
            ...
            Transform(v_lines, self.get_v_lines_indicating_periods(freq)).update(1)

        return UpdateFromFunc(self.v_lines_indicating_periods, update_v_lines)
...
class WrapCosineGraphAroundCircle(FourierMachineScene):
    ...
    def show_initial_signal(self):
        ...
        v_lines = VGroup(
            *[DashedLine(ORIGIN, 2 * UP, color=RED).move_to(axes.coords_to_point(x, 0), DOWN) for x in (1, 2)]
        )
    ...
    def wrap_around_circle(self):
        ...
        self.play(ReplacementTransform(moving_graph, polarized_graph, run_time=3, path_arc=-TAU / 2))
    ...
    def show_time_sweeps(self):
        ...
        self.play(FadeIn(winding_freq_label), *self.get_vector_animations(graph))
        self.wait()
    ...
    def change_wrapping_frequency(self):
        ...
        for target_freq in [1.23, 0.2, 0.79, 1.55, self.signal_frequency]:
            self.play(
                Transform(v_lines, self.get_v_lines_indicating_periods(target_freq)),
            ...
            count += 1
        self.wait()
        self.play(*self.get_vector_animations(graph, False), run_time=15)

    ##

    def get_winding_frequency_label(self):
        freq = self.initial_winding_frequency
        winding_freq_label = VGroup(DecimalNumber(freq, num_decimal_places=2), OldTexText("cycles/second"))
        winding_freq_label.arrange(RIGHT)
        winding_freq_label.next_to(self.circle_plane, RIGHT, aligned_edge=UP)
        self.winding_freq_label = winding_freq_label
        return winding_freq_label

    def get_peak_braces(self, **kwargs):
        peak_points = [
            self.time_axes.input_to_graph_point(x, self.graph) for x in np.arange(0, 3.5, 1.0 / self.signal_frequency)
        ]
        return VGroup(*[Brace(Line(p1, p2), UP, **kwargs) for p1, p2 in zip(peak_points, peak_points[1:])])
...
class DrawFrequencyPlot(WrapCosineGraphAroundCircle, PiCreatureScene):
    ...
    def setup_graph(self):
        ...
        self.add(self.graph)
        self.add(self.get_polarized_mobject(self.graph, self.initial_winding_frequency))
        ...
        self.v_lines_indicating_periods = self.get_v_lines_indicating_periods(self.initial_winding_frequency)
    ...
    def indicate_weight_of_wire(self):
        ...
        self.play(pol_graph.restore, morty.change, "pondering")
    ...
    def change_to_various_frequencies(self):
        self.change_frequency(3.0, run_time=30, rate_func=bezier([0, 0, 1, 1]))
        self.wait()
        self.play(*self.get_vector_animations(self.graph), run_time=15)
    ...
    def introduce_frequency_plot(self):
        ...
        x_coord_label.next_to(frequency_axes.y_axis.get_top(), RIGHT, aligned_edge=UP, buff=LARGE_BUFF)
        ...
        self.play(ReplacementTransform(com_label.copy(), x_coord_label))
        self.wait()
    ...
    def draw_full_frequency_plot(self):
        ...
        fourier_graph_update = self.get_fourier_graph_drawing_update_anim(fourier_graph)
        v_line = DashedLine(
        ...
        self.play(GrowFromCenter(self.fourier_graph_dot), FadeOut(v_line))
    ...
    def lower_graph(self):
        ...
        new_graph = self.get_cosine_wave(self.signal_frequency, shift_val=0)
        ...
        self.play(
            ...
            graph.shift,
            -shift_vect / 2,
            self.get_frequency_change_animation(self.graph, new_freq),
        ...
        self.play(
            graph.restore,
            time_axes.restore,
            self.get_frequency_change_animation(self.graph, 3.0),
            self.center_of_mass_dot_anim,
            self.get_period_v_lines_update_anim(),
            fourier_graph.restore,
    ...
    def label_as_almost_fourier(self):
        ...
        self.play(MoveToTarget(x_coord_label), Write(almost_fourier_label))
        self.wait(2)

    ##

    def get_center_of_mass_dot(self):
        dot = Dot(self.get_pol_graph_center_of_mass(), color=self.center_of_mass_color)
    ...
    def generate_fourier_dot_transform(self, fourier_graph):
        ...
        def update_dot(dot):
            ...
            dot.move_to(self.frequency_axes.input_to_graph_point(f, fourier_graph))

        self.fourier_graph_dot_anim = UpdateFromFunc(self.fourier_graph_dot, update_dot)
    ...
    def get_fourier_graph_drawing_update_anim(self, fourier_graph):
        ...
        def update_fourier_graph(fg):
            freq = self.graph.polarized_mobject.frequency
            fg.pointwise_become_partial(fourier_graph_copy, 0, freq / max_freq)
            return fg

        self.fourier_graph_drawing_update_anim = UpdateFromFunc(fourier_graph, update_fourier_graph)
    ...
    def generate_center_of_mass_dot_update_anim(self, multiplier=1):
        ...
        self.center_of_mass_dot_anim = UpdateFromFunc(
            self.center_of_mass_dot,
            lambda d: d.move_to(multiplier * (com() - origin) + origin),
        )
...
class StudentsHorrifiedAtScene(TeacherStudentsScene):
    def construct(self):
        self.play_student_changes(*3 * ["horrified"], look_at=2 * UP + 3 * LEFT)
        self.wait(4)


class AskAboutAlmostFouierName(TeacherStudentsScene):
    def construct(self):
        self.student_says("``Almost'' Fourier transform?", target_mode="sassy")
...
class ShowLowerFrequency(DrawFrequencyPlot):
    ...
    def show_lower_frequency_signal(self):
        ...
        v_lines = VGroup(*[DashedLine(ORIGIN, 1.5 * UP).move_to(axes.coords_to_point(x, 0), DOWN) for x in (1, 2)])
    ...
    def play_with_lower_frequency_signal(self):
        ...
        self.play(GrowArrow(arrow), dot.restore, Write(com_words))
        self.wait()
        ...
        fourier_graph = self.get_fourier_transform_graph(graph)
        fourier_graph_update = self.get_fourier_graph_drawing_update_anim(fourier_graph)
        x_coord_label = OldTexText("x-coordinate of center of mass")
        x_coord_label.scale(self.text_scale_val)
        x_coord_label.next_to(
            self.frequency_axes.input_to_graph_point(self.signal_frequency, fourier_graph),
            UP,
        )
        x_coord_label.set_color(self.center_of_mass_color)
        self.generate_fourier_dot_transform(fourier_graph)
...
class MixingUnmixingTODOStub(TODOStub):
    CONFIG = {"message": "Insert mixing and unmixing of signals"}
...
class ShowLinearity(DrawFrequencyPlot):
    ...
    def show_sum_of_signals(self):
        ...
        VGroup(axes_copy, high_freq_graph).next_to(axes, DOWN, MED_LARGE_BUFF)
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(high_freq_label, low_freq_label)))
        self.wait()
        self.play(
            ...
            ReplacementTransform(VGroup(low_freq_label, high_freq_label), sum_label),
        )
        self.wait()
        self.graph = graph
    ...
    def show_winding_with_sum_graph(self):
        ...
        x_coord_label.set_color(self.center_of_mass_color)
        almost_fourier_label = OldTexText("``Almost-Fourier transform''")
        ...
        self.play(ReplacementTransform(graph.copy(), pol_graph), GrowFromCenter(dot))
        freqs = [self.low_freq, self.high_freq, self.frequency_axes.x_max]
        for freq in freqs:
            self.change_frequency(
                freq,
                run_time=8,
...
class ShowCommutativeDiagram(ShowLinearity):
    ...
    def show_diagram(self):
        ...
        fa_group = VGroup(frequency_axes, frequency_axes.deepcopy(), frequency_axes.deepcopy())
        VGroup(ta_group[1], fa_group[1]).shift(MED_LARGE_BUFF * UP)
        for ta, fa in zip(ta_group, fa_group):
            fa.next_to(ta.x_axis, RIGHT, submobject_to_align=fa.x_axis)
            fa.to_edge(RIGHT)
            ta.remove(ta.labels)
            fa.remove(fa.label)
        ...
        for func, color, label, ta, fa in zip(funcs, colors, labels, ta_group, fa_group):
            ...
            fourier = get_fourier_transform(func, ta.x_min, 4 * ta.x_max)
        ...
        for group in ta_group, fa_group:
            arrow = Arrow(group[1].graph, group[2].graph, color=WHITE, buff=SMALL_BUFF)
        ...
        def apply_transform(index):
            ...
            fa = fa_group[index]
            anims = [
                ReplacementTransform(getattr(ta, attr), getattr(fa, attr)) for attr in ("x_axis", "y_axis", "graph")
            ]
            anims += [
                GrowArrow(ta.arrow),
            ...
            if index == 0:
                anims.append(ReplacementTransform(ta.labels[0], fa.label))
        ...
        self.play(
            ...
            *[ReplacementTransform(fa.copy(), fa_group[2]) for fa in fa_group[:2]],
        )
        self.wait(2)
        self.play(
            GrowArrow(sum_arrows[0]),
            Write(sum_arrows[0].words),
            *[ReplacementTransform(mob.copy(), ta_group[2], run_time=1) for mob in ta_group[:2]],
        )
        self.wait()
        apply_transform(2)
        self.wait()
    ...
    def point_out_spikes(self):
        ...
        sum_spike_rects = VGroup(*[self.get_spike_rect(fa_group[2], freq) for freq in freqs])
        self.play(ReplacementTransform(flat_rects, sum_spike_rects))
        self.play(
            LaggedStartMap(
                WiggleOutThenIn,
                sum_spike_rects,
    ...
    def get_spike_rect(self, axes, freq):
        peak_point = axes.input_to_graph_point(freq, axes.graph)
...
class PauseAndPonder(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Pause and \\\\ ponder!", target_mode="hooray")
...
class BeforeGettingToTheFullMath(TeacherStudentsScene):
    def construct(self):
        formula = OldTex("\\hat{g}(f) = \\int_{-\\infty}^{\\infty}" + "g(t)e^{-2\\pi i f t}dt")
...
class FilterOutHighPitch(AddingPureFrequencies, ShowCommutativeDiagram):
    ...
    def show_intensity_vs_time_graph(self):
        ...
        labels = VGroup(time_label, intensity_label)
        labels.scale(0.75)
        time_label.next_to(axes.x_axis, DOWN, aligned_edge=RIGHT, buff=SMALL_BUFF)
        intensity_label.next_to(axes.y_axis, RIGHT, aligned_edge=UP, buff=SMALL_BUFF)
        axes.labels = labels
        ...
        self.play(
            ...
            "pondering",
            axes,
            ShowCreation(graph, run_time=4, rate_func=bezier([0, 0, 1, 1])),
    ...
    def filter_out_high_pitch(self):
        ...
        new_graph = frequency_axes.get_graph(filtered_func, num_graph_points=300)
    ...
    def mention_inverse_transform(self):
        ...
        filtered_graph = time_axes.get_graph(lambda t: time_graph.underlying_function(t) - np.cos(TAU * 3 * t))
        ...
        self.play(GrowArrow(arrow), Write(inv_fourier_words))
        self.wait()
        self.play(
            time_graph.fade,
            0.9,
            ReplacementTransform(fourier_graph.copy(), filtered_graph),
        )
        self.wait()
...
class ApplyFourierToFourier(DrawFrequencyPlot):
    ...
    def swap_graphs(self):
        ...
        f_max = self.frequency_axes.x_max
        new_fourier_graph = time_axes.get_graph(lambda t: 2 * fourier_graph.underlying_function(t))
        new_fourier_graph.match_style(fourier_graph)

        self.remove(fourier_graph)
        self.play(
            ReplacementTransform(fourier_graph.copy(), new_fourier_graph),
            ApplyMethod(
                time_graph.shift,
                3 * UP + 10 * LEFT,
                remover=True,
        ...
        self.play(wound_up_graph.next_to, FRAME_X_RADIUS * LEFT, LEFT, remover=True)
        self.wait()

        self.graph = new_fourier_graph
        wound_up_graph = self.get_polarized_mobject(new_fourier_graph, freq=0)
        double_fourier_graph = frequency_axes.get_graph(lambda t: 0.25 * np.cos(TAU * 2 * t)).set_color(PINK)
...
class WriteComplexExponentialExpression(DrawFrequencyPlot):
    ...
    def setup_plane(self):
        ...
        circle.apply_complex_function(lambda z: R3_to_complex(circle_plane.number_to_point(np.exp(z))))
        circle_plane.add(circle)
    ...
    def show_winding_with_both_coordinates(self):
        ...
        def lines_update(lines):
            ...
            h_line.put_start_and_end_on(plane.coords_to_point(0, y), point)
            v_line.put_start_and_end_on(plane.coords_to_point(x, 0), point)
    ...
    def show_plane_as_complex_plane(self):
        to_fade = VGroup(self.time_axes_group, self.pol_graph, self.wps_label)
        plane = self.circle_plane
        ...
        number_label_update_anim = ContinualChangingDecimal(
            ...
            position_update_func=lambda l: l.next_to(dot, DOWN + RIGHT, buff=SMALL_BUFF),
        )
        ...
        self.play(MoveAlongPath(dot, flower_path, run_time=10, rate_func=bezier([0, 0, 1, 1])))
        self.wait()
        self.play(
            ShowCreation(
                self.pol_graph,
        ...
        self.wait()
        self.play(FadeOut(VGroup(dot, self.dot_component_anim.mobject, number_label)))
    ...
    def show_eulers_formula(self):
        ...
        def get_circle_point(scalar=1, t_shift=0):
            return plane.number_to_point(scalar * np.exp(complex(0, get_t() + t_shift)))
        ...
        exp_decimal_update = ContinualChangingDecimal(
            exp_decimal,
            lambda a: get_t(),
            position_update_func=lambda d: d.move_to(exp_base.get_corner(UP + RIGHT), DOWN + LEFT),
        )
        exp_base_update = Mobject.add_updater(
            exp_base,
            lambda e: e.move_to(get_circle_point(scalar=1.1, t_shift=0.01 * TAU)),
        )
        vector_update = Mobject.add_updater(
            vector,
            lambda v: v.put_start_and_end_on(plane.number_to_point(0), get_circle_point()),
        )
        ...
        # Show initial vector
        self.play(GrowArrow(vector), FadeIn(exp_base), Write(exp_decimal))
        self.add(*updates)
        self.play(ghost_dot.shift, 2 * RIGHT, run_time=3)
        self.wait()

        # Show arc
        arc, circle = [Line(ORIGIN, t * UP) for t in (get_t(), TAU)]
        for mob in arc, circle:
            mob.insert_n_curves(20)
            mob.set_stroke(RED, 4)
            mob.apply_function(lambda p: plane.number_to_point(np.exp(R3_to_complex(p))))
        distance_label = DecimalNumber(exp_decimal.number, unit="\\text{units}")
        ...
        self.play(ReplacementTransform(exp_decimal.copy(), distance_label))
        self.wait()
        ...
        self.play(
            ...
            ShowCreation(
                circle,
                rate_func=lambda a: interpolate(2.0 / TAU, 1, smooth(a)),
            ),
            run_time=6,
        )
        self.wait()
        ...
        exp_expression.next_to(plane.coords_to_point(1, 1), UP + RIGHT)
        f.set_color(RED)
        t.set_color(YELLOW)
        ...
        self.play(ReplacementTransform(VGroup(exp_base[1], exp_decimal[1]).copy(), exp_expression))
        ...
        self.add(ghost_dot)
        self.wait(3)
        self.play(FadeOut(frequency_label), FadeIn(example_frequency))
        ...
        self.play(
            *list(
                map(
                    FadeOut,
                    [update.mobject for update in updates if update.mobject is not vector],
                )
            )
        )
        ...
        self.set_variables_as_attrs(ambient_ghost_dot_movement, ghost_dot, vector, vector_update, exp_expression)
    ...
    def show_winding_graph_expression(self):
        ...
        self.play(FadeIn(time_axes_group), FadeOut(self.complex_plane_title))
        ...
        self.play(ShowCreation(rect), FadeIn(morty))
        self.play(morty.change, "raise_right_hand")
        self.play(Blink(morty))
        self.play(morty.change, "hooray", rect)
        self.wait(2)
        self.play(*list(map(FadeOut, [morty, rect, graph_y_vector, vector])))
    ...
    def find_center_of_mass(self):
        ...
        integral = OldTex("{1", "\\over", "t_2 - t_1}", "\\int", "_{t_1}", "^{t_2}")
        ...
        randy.to_edge(RIGHT)
        full_expression_rect = SurroundingRectangle(VGroup(integral, dt_mob), color=RED)
        com_dot = self.center_of_mass_dot
        self.center_of_mass_dot_anim.update(0)
        com_arrow = Arrow(full_expression_rect.get_left(), com_dot, buff=SMALL_BUFF)
    ...
    def show_center_of_mass_sampling(self, n_dots):
        ...
        pre_dots = VGroup(
            *[dot.copy().move_to(axes.coords_to_point(t, 0)) for t in np.linspace(axes.x_min, axes.x_max, n_dots)]
        )
        ...
        for graph in time_graph, pol_graph:
            ...
            graph.is_faded = True
            graph.dots = VGroup(
                *[dot.copy().move_to(graph.point_from_proportion(a)) for a in np.linspace(0, 1, n_dots)]
            )
        ...
        self.play(
            ReplacementTransform(time_graph.copy(), pol_graph.target),
...
class ScaleUpCenterOfMass(WriteComplexExponentialExpression):
    CONFIG = {"time_axes_scale_val": 0.6, "initial_winding_frequency": 2.05}
    ...
    def add_expression(self):
        ...
        self.winding_freq_label.align_to(self.circle_plane.coords_to_point(0, 0.1), DOWN)
    ...
    def scale_up_center_of_mass(self):
        ...
        com_dot = self.center_of_mass_dot
        com_vector = Arrow(origin, com_dot.get_center(), buff=0)
        com_vector.match_style(com_dot)
        vector_to_scale = com_vector.copy()

        def get_com_vector_copies(n):
            com_vector_copies = VGroup(*[com_vector.copy().shift(x * com_vector.get_vector()) for x in range(1, n + 1)])
            com_vector_copies.set_color(TEAL)
            return com_vector_copies

        com_vector_update = UpdateFromFunc(com_vector, lambda v: v.put_start_and_end_on(origin, com_dot.get_center()))

        circle = Circle(color=TEAL)
        circle.surround(com_dot, buffer_factor=1.2)

        time_span = Rectangle(stroke_width=0, fill_color=TEAL, fill_opacity=0.4)
        axes = self.time_axes
        time_span.replace(Line(axes.coords_to_point(0, 0), axes.coords_to_point(3, 1.5)), stretch=True)
        ...
        self.play(com_dot.fade, 0.5, FadeIn(vector_to_scale))
        self.wait()
        ...
        self.play(*[ReplacementTransform(com_vector.copy(), cvc, path_arc=-TAU / 10) for cvc in com_vector_copies])
        self.wait()
        ...
        self.play(MoveToTarget(to_squish), FadeOut(com_vector_copies))
        ...
        self.play(
            ...
            {"about_edge": LEFT},
            *[
                ShowCreation(mob, rate_func=lambda a: interpolate(0.5, 1, smooth(a)))
        ...
        to_fade = VGroup(self.integral_brace, self.integral_label, self.frac, self.frac_cross)
        self.play(
            to_shift.shift,
            2 * DOWN,
            FadeOut(to_fade),
            axes.background_rectangle.stretch,
            2,
            0,
            {"about_edge": LEFT},
            Animation(axes),
        ...
        self.play(
            ...
            com_vector_update,
            ShowCreation(very_long_graph, rate_func=lambda a: interpolate(0.5, 1, a)),
            ShowCreation(very_long_graph.polarized_mobject),
        )
        self.remove(graph, graph.polarized_mobject)
        self.graph = very_long_graph
        self.wait()
        self.play(com_vector.scale, 12, {"about_point": origin}, run_time=2)
    ...
    def comment_on_current_signal(self):
        ...
        new_com_vector_update = UpdateFromFunc(
            com_vector,
            lambda v: v.put_start_and_end_on(origin, com_dot.get_center()).scale(12, about_point=origin),
        )

        v_lines = self.get_v_lines_indicating_periods(freq=1.0, n_lines=3)[:2]
        graph_portion = axes.get_graph(graph.underlying_function, x_min=1, x_max=2)
        ...
        self.wait()
        self.play(ReplacementTransform(bps_label[1][0].copy(), wps_label[1]))
        self.wait()
        self.play(
            com_vector.scale,
            0.5,
...
class TakeAStepBack(TeacherStudentsScene):
    def construct(self):
        (self.student_says("Hang on, go over \\\\ that again?", target_mode="confused"),)
...
class SimpleCosineWrappingAroundCircle(WriteComplexExponentialExpression):
    ...
    def construct(self):
        ...
        VGroup(self.circle_plane, self.graph.polarized_mobject).move_to(ORIGIN)
        self.add(self.get_center_of_mass_dot())
        self.generate_center_of_mass_dot_update_anim()

        self.change_frequency(2.0, rate_func=linear, run_time=30)
        self.wait()
...
class SummarizeTheFullTransform(DrawFrequencyPlot):
    ...
    def setup_all_axes(self):
        ...
        time_label.next_to(time_axes.x_axis.get_right(), DOWN, SMALL_BUFF)
    ...
    def show_transform_function(self):
        ...
        g_hat_f.next_to(frequency_axes.input_to_graph_point(2, fourier_graph), UP)
        ...
        self.wait(2)
        for label in time_label, frequency_label:
            self.play(ShowCreation(label.rect), morty.change, "thinking")
            self.play(FadeOut(label.rect))
        self.wait()

        self.set_variables_as_attrs(graph, fourier_graph, g_t, g_hat_f)
    ...
    def show_winding(self):
        ...
        imaginary_fourier_graph.shift(
            frequency_axes.x_axis.get_right() - imaginary_fourier_graph.get_points()[-1],
        )

        real_part = OldTexText("Real part of", "$\\hat g(f)$")
        real_part[1].match_style(g_hat_f)
        real_part.move_to(g_hat_f)
        real_part.to_edge(RIGHT)

        self.get_polarized_mobject(graph, freq=0)
        update_pol_graph = UpdateFromFunc(graph.polarized_mobject, lambda m: m.set_stroke(width=2))
        ...
        g_hat_f_indication = Succession(
            ...
            {"run_time": 3},
            ShowCreation,
            imaginary_fourier_graph,
            {"run_time": 3},
            rate_func=squish_rate_func(lambda x: x, 0, 31.0 / winding_run_time),
            run_time=winding_run_time,
        )

        self.play(
            FadeIn(plane),
            ReplacementTransform(graph.copy(), graph.polarized_mobject),
            morty.change,
            "happy",
        )
...
class BoundsAtInfinity(SummarizeFormula):
    def construct(self):
        ...
        bound_rects = VGroup(*[SurroundingRectangle(b, buff=0.5 * SMALL_BUFF) for b in bounds])
        bound_rects.set_color(TEAL)
        inf_bounds = VGroup(*[VGroup(OldTex(s + "\\infty")) for s in ("-", "+")])
        ...
        def get_db_num_update(vect):
            return lambda a: axes.x_axis.point_to_number(time_interval.get_edge_center(vect))

        decimal_updates = [
            ChangingDecimal(
                db,
                get_db_num_update(vect),
                position_update_func=lambda m: m.move_to(m.bound, LEFT),
            )
        ...
        self.wait()
        self.play(ReplacementTransform(self.get_time_interval(0, 0.01), time_interval))
        ...
        self.play(Transform(time_interval, wide_interval, run_time=4, rate_func=there_and_back))
        self.play(
            ReplacementTransform(inf_bounds, decimal_bounds),
            time_interval.restore,
        )
        self.play(
            VGroup(axes, graph).stretch,
            0.05,
            0,
            Transform(time_interval, wide_interval),
            UpdateFromAlphaFunc(axes.x_axis.numbers, lambda m, a: m.set_fill(opacity=1 - a)),
            *decimal_updates,
            run_time=12,
            rate_func=bezier([0, 0, 1, 1]),
        )
    ...
    def get_time_interval(self, t1, t2):
        line = Line(*[self.axes.coords_to_point(t, 0) for t in (t1, t2)])
        rect = Rectangle(
            stroke_width=0,
            fill_color=TEAL,
            fill_opacity=0.5,
...
class ShowUncertaintyPrinciple(Scene):
    def construct(self):
        ...
        group = VGroup(title, top_axes, arrow, bottom_axes)
        group.arrange(DOWN)
        ...
        for factor in factors:
            self.play(ghost_dot.move_to, factor * RIGHT, run_time=2)
            self.wait()
...
class SubscribeOrBinge(PiCreatureScene):
    def construct(self):
        ...
        self.play(subscribe.restore, morty.change, "raise_left_hand")
...
class SponsorScreenGrab(PiCreatureScene):
    def construct(self):
        ...
        self.play(morty.change, "raise_right_hand", ShowCreation(screen))
        self.play(Write(url))
        self.wait(2)
...
class Thumbnail(Scene):
    def construct(self):
        ...
        parts.arrange(DOWN, buff=LARGE_BUFF, aligned_edge=RIGHT)
        parts.to_edge(RIGHT)
        self.add(parts)
