...
class ParameterizeTheLoop(InteractiveScene):
    def construct(self):
        ...
        circle_ticks = VGroup(
            Line(1.1 * point, 0.9 * point) for a in np.linspace(0, 1, 11) for point in [circular_interval.pfp(a)]
        )
        ...
        self.play(
            UpdateFromAlphaFunc(
                surface,
                lambda m, a: m.set_points(get_partial_strip(interpolate(2, TAU, smooth(a))).get_points()),
            ),
            run_time=5,
        )
        self.play(
...
class ShowAngleInformation(ShowTheSurface):
    def construct(self):
        ...
        cross_point = connecting_line.pfp(
            inverse_interpolate(connecting_line.get_y(DOWN), connecting_line.get_y(UP), -1)
        )
        ...
        self.play(
            *(ReplacementTransform(corner_coords[substr], new_corner_coords[substr]) for substr in ["(x, y, d", ")"]),
        ...
        cross_point2 = connecting_line2.pfp(
            inverse_interpolate(connecting_line2.get_start()[1], connecting_line2.get_end()[1], 0)
        )
