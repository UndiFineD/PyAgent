...
class Project4DCube(Project3DCube):
    def construct(self):
        ...
        proj_basis_labels = VGroup(Tex(Rf"P{tuple(basis)}", font_size=24) for basis in np.identity(4).astype(int))
...
class ShowRhombicDodecTesselation(Project4DCube):
    def construct(self):
        ...
        self.play(
            LaggedStart((Restore(piece) for piece in pieces), lag_ratio=0.5, group_type=Group),
...
class AskStripQuestion(InteractiveScene):
    def construct(self):
        ...
        fan_covering.add(
            *(self.get_strip(circle, 0.6, 0.9, theta=theta) for theta in np.arange(TAU / 12, TAU, TAU / 3))
        )
...
class StruggleWithStrips(AskStripQuestion):
    def construct(self):
        ...
        self.play(
            ...
            *(ChangeDecimalToValue(width_term, new_width) for width_term, new_width in zip(width_terms, new_widths)),
...
class SphereStrips(InteractiveScene):
    def construct(self):
        ...
        strip.add_updater(
            lambda m: m.become(self.get_strip(x0_tracker.get_value(), x0_tracker.get_value() + delta_x, theta=0))
        )
...
class GeneralCentersOfSimilarity(MongesTheorem):
    def construct(self):
        ...
        theorem_line.add_updater(
            lambda m: m.put_start_and_end_on(cos_dots[0].get_center(), cos_dots[2].get_center()).scale(100)
        )
        labels = VGroup()
        ...
        # Change shapes
        pis = VGroup(Tex(R"\pi")[0].match_style(circle).replace(circle) for circle in circles)
    ...
    def get_cone(self, shape):
        ...
        return VGroup(
            shape.copy().scale(a).set_z(z).set_stroke(width=1) for a, z in zip(np.linspace(1, 0), np.linspace(0, top_z))
        )
...
class RandomVectorStatistics(InteractiveScene):
    def construct(self):
        ...
        for _ in range(0):
            vects = VGroup(
                Vector(1.5 * rotate_vector(RIGHT, random.uniform(0, TAU)), thickness=4).set_fill(
                    random_bright_color(), border_width=1
                )
                for _ in range(2)
            )
            vects.shift(center)
...
class IntersectingCircles(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (FadeTransform(radial_lines[i1].copy(), radial_lines[i2]) for i1, i2 in [(5, 9), (8, 10), (4, 11)]),
                lag_ratio=0.75,
                run_time=3,
            ),
        )
        ...
        radial_lines.add_updater(
            lambda m: m.become(self.get_radial_lines(circles, [vt.get_center() for vt in vector_trackers]))
        )
...
class TriPod(MongesTheorem):
    def construct(self):
        ...
        tri1, tri2 = tris = VGroup(
            Polygon(*(proj_point + f * v for f, v in zip(factors, vects))) for factors in [factors1, factors2]
        )
