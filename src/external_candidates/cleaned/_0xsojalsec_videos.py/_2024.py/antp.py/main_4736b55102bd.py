...
class CrankEmail(InteractiveScene):
    ...
    def create_table(
        ...
        # Numbers
        row1_mobs, row2_mobs = (
            VGroup(*(Integer(n, font_size=font_size) for n in content)) for content in [row1_content, row2_content]
        )
        ...
        v_lines = VGroup(
            *(
                ...
                .set_x(0.5 * (row1_mobs[i].get_right()[0] + row1_mobs[i + 1].get_left()[0]))
                .align_to(grid, UP)
...
class SieveOfEratosthenes(InteractiveScene):
    ...
    def construct(self):
        ...
        for n in range(self.n_iterations):
            ...
            rects = VGroup(
                *(SurroundingRectangle(tr, buff=self.rect_buff) for tr in to_remove if tr.get_fill_opacity() > 0.5)
            )
    ...
    def get_number_mobs(self, grid):
        return VGroup(
            *(Integer(i).set_height(0.3 * box.get_height()).move_to(box) for i, box in zip(it.count(1), grid))
        )
