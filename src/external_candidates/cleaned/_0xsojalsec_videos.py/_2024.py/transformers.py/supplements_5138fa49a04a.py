...
class DifferentUsesOfModel(InteractiveScene):
    def construct(self):
        ...
        path_arc = -0.65 * PI
        left_arrows = VGroup(
            Arrow(s0[word].get_top(), s0["model"].get_top(), path_arc=path_arc) for word in ["machine", "learning"]
        )
...
class LongListOFQuestions(InteractiveScene):
    def construct(self):
        ...
        connections = VGroup(
            VGroup(
                Line(elem.get_right(), neuron.get_center(), buff=neuron.get_width() / 2).set_stroke(
...
class WriteSuperposition(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStartMap(VShowPassingFlash, outline, time_width=2, run_time=5, lag_ratio=0.01),
        )
        self.wait()
