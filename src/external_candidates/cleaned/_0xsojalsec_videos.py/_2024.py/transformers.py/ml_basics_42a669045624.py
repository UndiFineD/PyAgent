# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\ml_basics.py
...
class MLWithinDeepL(InteractiveScene):
    def construct(self):
        ...
        matrices = VGroup(*(WeightMatrix(shape=(8, 6), ellipses_row=None, ellipses_col=None) for x in range(10)))
        ...
        self.play(
            LaggedStart(
                *(Succession(FadeIn(matrix, shift=mat_shift), MoveToTarget(matrix)) for matrix in matrices[:-1]),
                lag_ratio=0.25,
                run_time=5,
            ),
            Animation(Point()),
    ...
    def get_titled_box(self, text, color, font_size=48, y_space=0.5, x_space=0.5, opacity=0.1):
        ...
        box = Rectangle(title.get_width() + x_space, title.get_height() + y_space)
    ...
    def get_regression_drawing(self):
        ...
        dots = VGroup(Dot(axes.c2p(x, y0 + m * x + np.random.normal())) for x in np.random.uniform(0, 10, 15))
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                ...
                *(FadeIn(col, shift=col.get_center() - vector.get_center()) for col in in_array.get_columns()),
            )
        )
        self.wait()
        ...
        tensor = VGroup(*(dot_array.copy() for n in range(5)))
        for layer in tensor:
        ...
        dial_arrows = VGroup(*(Arrow(weights_label.get_bottom(), rect.get_top(), buff=0.05) for rect in dial_rects))
        ...
        self.play(
            LaggedStart(
                *(VFadeInThenOut(VGroup(arrow, rect)) for arrow, rect in zip(dial_arrows, dial_rects)),
                lag_ratio=0.25,
                run_time=3,
            )
        )
        ...
        indices = [0, 1, 2, -1]
        dial_lines = VGroup(*(Line(top_dials[n].get_top(), weight_parts[n].get_bottom(), buff=0.1) for n in indices))
        ...
        self.play(
            ...
            LaggedStart(*(TransformFromCopy(column[n], data_parts[n]) for n in indices)),
            Group(in_data, in_word).animate.to_edge(LEFT, buff=0.25),
        )
        self.play(
            Write(weighted_sum["+"]),
            Write(weighted_sum[R"\cdots"]),
            LaggedStart(*(FadeTransform(top_dials[n].copy(), weight_parts[n]) for n in indices)),
            LaggedStartMap(ShowCreation, dial_lines),
            run_time=1,
        )
        self.wait()
        for x in range(3):
            self.play(*(dial.animate_set_value(dial.get_random_value()) for dial in top_dials))
        ...
        dial_lines.target = VGroup(
            *(Line(weights_label.target, weight_parts[index].get_top(), buff=SMALL_BUFF) for index in indices)
        )
        ...
        rect_lines = VGroup(*(Line(expr_rect.get_corner(DOWN + v), x01_rect.get_corner(UP + v)) for v in [LEFT, RIGHT]))
        ...
        in_subset = VGroup(*(elem for row in in_array.get_rows()[:3] for elem in row[:3]))
        ...
        # Show added layers
        to_fade = VGroup(func_wrapper, expr_rect, rect_lines, x01_rect, weighted_sum)
        ...
        self.play(
            LaggedStart(
                *(dot.animate.shift(0.1 * UP).set_anim_args(rate_func=there_and_back) for dot in dots),
                lag_ratio=0.25,
            )
        )
        self.wait()
        ...
        self.add(ghost_row)
        show_symbolic_matrix_vector_product(self, matrix, vector, rhs, run_time_per_row=1.5)
        ...
        self.play(
            LaggedStart(
                *(dial.animate_set_value(dial.get_random_value()) for dial in moving_dials),
                lag_ratio=0.02,
                run_time=3,
            )
        )
        ...
        mv_arrows = VGroup(*(Arrow(mv_label.get_bottom(), smp.get_top(), buff=0.1) for smp in small_mat_products))
...
class LinearRegression(InteractiveScene):
    ...
    def construct(self):
        ...
        data = np.array(
            [(x, y0 + m * x + 0.75 * np.random.normal(0, 1)) for x in np.random.uniform(2, x_max, n_data_points)]
        )
        ...
        self.play(y0_tracker.animate.set_value(-2), run_time=2)
        self.wait()
...
class ShowGPT3Numbers(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (dial.animate_set_value(dial.get_random_value()) for dial in dials),
        ...
        matrices = VGroup(WeightMatrix(shape=(5, 5)) for n in range(np.product(mat_grid_shape)))
        ...
        mat_dots = VGroup(
            *(Tex(R"\dots").next_to(mat, RIGHT) for mat in matrices[m - 1 :: m]),
            *(Tex(R"\vdots").next_to(mat, DOWN) for mat in matrices[-m:]),
        )
        ...
        all_entries = VGroup(entry for mat in matrices for row in mat.get_rows() for entry in row)
        ...
        product_expressions = VGroup(
            get_product_expression(category, consts) for category, consts in zip(category_names, const_lists)
        )
        ...
        self.play(FadeOut(mat_groups), FadeOut(mat_dots[1:7]), highlight_category(0))
        self.play(
            FadeIn(exprs[0]),
        ...
        for group in [per_head_exprs, per_layer_exprs, full_att_exprs]:
            sum_box = SurroundingRectangle(VGroup(expr[0].rhs for expr in group))
            sum_box.set_stroke(BLUE, 2)
            sum_label = Integer(sum(np.product(list(count.get_value() for count in expr[1])) for expr in group))
        ...
        self.play(
            *(product_expressions[i].animate.set_fill(opacity=0.25, border_width=0) for i in covered_categories),
        ...
        per_layer_exprs = VGroup(
            get_product_expression(category_names[i], const_lists[i][:2], suffix="per layer") for i in mlp_categories
        )
        ...
        all_rhss = VGroup(VGroup(expr[0]["="][0], expr[0].rhs) for expr in product_expressions)
...
class DistinguishWeightsAndData(InteractiveScene):
    def construct(self):
        # Set up titles
        weights_title, data_title = titles = VGroup(Text(word, font_size=60) for word in ["Weights", "Data"])
        ...
        vectors = VGroup(NumericEmbedding(length=8, ellipses_row=None) for n in range(8))
        ...
        mat_nums = VGroup(elem for matrix in matrices for elem in matrix.get_entries())
        mat_braces = VGroup(brace for matrix in matrices for brace in matrix.get_brackets())
        vec_nums = VGroup(elem for vector in vectors for elem in vector.get_entries())
        vec_braces = VGroup(brace for vector in vectors for brace in vector.get_brackets())
        ...
        self.play(
            LaggedStart(
                (Transform(num, num.states[2], path_arc=PI) for num in all_nums),
                lag_ratio=lag_ratio,
                run_time=3,
            ),
        )
        ...
        self.play(
            LaggedStart(
                (
                    LaggedStart(
                        (Transform(num, num.states[1]) for num in group),
                        lag_ratio=lag_ratio,
                        run_time=2,
                    )
        ...
        self.play(
            Write(weights_title),
            LaggedStart(
                (Transform(num, num.states[0]) for num in mat_nums),
                lag_ratio=lag_ratio,
                run_time=2,
            ),
        ...
        self.play(
            Write(data_title),
            LaggedStart(
                (Transform(num, num.states[0]) for num in vec_nums),
                lag_ratio=lag_ratio,
                run_time=2,
            ),
...
class SoftmaxBreakdown(InteractiveScene):
    def construct(self):
        ...
        plusses = VGroup(*(Tex("+").move_to(VGroup(p1, p2)) for p1, p2 in zip(prob_copies, prob_copies[1:])))
        ...
        self.play(LaggedStart(*(FadeTransform(pc.copy(), rhs) for pc in prob_copies), lag_ratio=0.07))
        self.wait()
        ...
        neg_rects = VGroup(*(SurroundingRectangle(entry) for entry in rhs.get_entries() if entry.get_value() < 0))
        gt1_rects = VGroup(*(SurroundingRectangle(entry) for entry in rhs.get_entries() if entry.get_value() > 1))
        ...
        variables = VGroup(
            *(Tex(f"x_{{{n}}}", font_size=48).move_to(elem) for n, elem in enumerate(rhs_entries, start=1))
        )
        ...
        exp_parts = VGroup(*(Tex(f"e^{{{var.get_tex()}}}", font_size=48).move_to(var) for var in variables))
        exp_parts.align_to(softmax_box, LEFT)
        exp_parts.shift(0.75 * RIGHT)
        exp_parts.space_out_submobjects(1.5)
        gt0s = VGroup(Tex(R"> 0").next_to(exp_part, aligned_edge=DOWN) for exp_part in exp_parts)

        self.play(
            softmax_label.animate.next_to(softmax_box, UP, buff=0.15),
            LaggedStart(
                *(TransformMatchingStrings(var.copy(), exp_part) for var, exp_part in zip(variables, exp_parts)),
                run_time=1,
                lag_ratio=0.01,
            ),
        )
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(exp_part.copy(), exp_sum) for exp_part in exp_parts),
                lag_ratio=0.01,
            ),
        ...
        self.play(
            ...
            LaggedStart(
                *(FadeTransform(exp_sum.copy(), denom) for denom in lil_denoms),
                lag_ratio=0.01,
            ),
        )
        self.wait()
        ...
        # Resize box
        sm_terms = VGroup(*(VGroup(exp_part, denom) for exp_part, denom in zip(exp_parts, lil_denoms)))
        ...
        self.play(
            ...
            in_arrow.animate.become(Arrow(rhs.target, softmax_box.target).match_style(in_arrow)),
        )
        self.wait()
        ...
        # Tweak values
        index_value_pairs = [(6, 4.0), (4, 4.2), (2, 4.0), (0, 6.0), (4, 9.9)]
        ...
        self.play(
            LaggedStart(*(FadeTransform(old_term, new_term) for old_term, new_term in zip(sm_terms, new_sm_terms))),
            LaggedStart(*(TransformFromCopy(t_label[0], t_mob[0]) for t_mob in all_Ts)),
        )
        self.wait()
        ...
        prob_arrows, logit_arrows = (
            VGroup(*(Vector(-vect).next_to(entry, vect, buff=0.25) for entry in matrix.get_entries()))
        ...
        for arrows, word, rects in [prob_group, logit_group]:
            self.play(
                ...
                Write(
                    rects,
                    stroke_width=5,
                    stroke_color=rects[0].get_stroke_color(),
                    lag_ratio=0.3,
                    run_time=3,
                ),
            )
