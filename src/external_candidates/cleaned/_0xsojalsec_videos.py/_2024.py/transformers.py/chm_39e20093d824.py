# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\chm.py
...
class HoldUpThumbnail(TeacherStudentsScene):
    def construct(self):
        # Test
        im = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2024/transformers/Thumbnails/Chapter5_TN3.png"
        )
        im_group = Group(SurroundingRectangle(im, buff=0).set_stroke(WHITE, 3), im)
...
class PredictTheNextWord(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(VectorizedPoint(machine.get_right()), letter) for letter in prediction),
                lag_ratio=0.05,
            ),
            GrowArrow(out_arrow),
        )
    ...
    def get_machine_dials(self, blocks):
        dials = VGroup(Dial().get_grid(8, 12).set_width(0.9 * block.get_width()).move_to(block) for block in blocks)
...
class FirthQuote(InteractiveScene):
    def construct(self):
        ...
        image = ImageMobject(
            "JohnRFirth"
        )  # From https://www.cambridge.org/core/journals/bulletin-of-the-school-of-oriental-and-african-studies/article/john-rupert-firth/D926AFCBF99AD17D5C7A7A9C0558DFDC
        ...
        self.play(FadeIn(image, 0.25 * UP), FadeIn(name, lag_ratio=0.1))
        ...
        self.remove(word)
        self.play(
            *(FadeIn(phrase[phrase.get_text().replace("bank", "")]) for phrase in phrases),
        ...
        self.play(
            LaggedStart(
                (FadeTransform(Group(word).copy(), image) for word, image in zip(banks, images)),
                lag_ratio=0.5,
                group_type=Group,
            )
        )
...
class RiverBankProbParts(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.play(LaggedStartMap(FadeIn, bar_groups, shift=0.25 * DOWN, run_time=3))
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (
                    LaggedStart(
                        (GrowFromPoint(dial, machine.get_top()) for dial in machine.dials),
                        lag_ratio=0.025,
                    )
        ...
        for _ in range(2):
            self.play(
                LaggedStart(
                    (machine.random_change_animation() for machine in machines),
                    lag_ratio=0.2,
                )
            )
...
class SequentialProcessing(InteractiveScene):
    def construct(self):
        ...
        for n in range(len(blocks) - 1):
            ...
            self.play(vect.animate.next_to(blocks[n], UP), MoveToTarget(blocks))
            self.play(
                LaggedStart(
                    (ContextAnimation(elem, blocks[n][1], lag_ratio=0.01) for elem in vect.get_entries()),
                    lag_ratio=0.01,
                ),
...
class PartialScript(SimpleAutogregression):
    ...
    def construct(self):
        ...
        curled_script_templates = Group(
            ParametricSurface(lambda u, v: (*curve.pfp(v)[:2], u), **surface_kw) for curve in curves
        )
        curled_script_templates[1].rotate(PI / 2, UP)
        curled_script_templates[0].rotate(-PI / 2)
        flat_script_template = ParametricSurface(lambda u, v: (u, v, 0), **surface_kw)
        ...
        for n in range(6):
            ...
            self.animate_text_input(mover, machine, position_text_over_machine=False, lag_ratio=1e-3)
...
class ShowMachineWithDials(PredictTheNextWord):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                (dial.animate_set_value(dial.get_random_value()) for dial in last_dials[1:]),
                lag_ratio=1.0 / len(last_dials),
            ),
            *(self.bar_group_change_animation(bg, value) for bg, value in zip(bar_groups[:-1], true_probs)),
            run_time=3,
        )
        self.wait()
        ...
        self.play(
            Write(random_words),
            LaggedStart(
                (dial.animate_set_value(dial.get_random_value()) for dial in last_dials),
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(all_dials.copy().set_opacity(0), nd) for nd in new_dials),
                lag_ratio=0.05,
            ),
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                ...
                LaggedStart(
                    (
                        block.animate.set_color(block.get_color() if block is blocks[-1] else TEAL).set_anim_args(
...
class EnormousAmountOfTrainingText(PremiseOfMLWithText):
    def construct(self):
        ...
        for n in range(n_examples):
            self.play(
                LaggedStart(
                    *(self.change_example_animation(screen, show_dial_change=True) for screen in inner_screens),
                    lag_ratio=0.1,
                    run_time=0.5,
                )
            )
    ...
    def get_screen(self):
        ...
        in_data, out_data = training_example = VGroup(*self.new_input_output_example(in_arrow, out_arrow))

        screen = VGroup(border, machine, arrows, training_example)
...
class BadChatBot(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (Transform(line, self.get_scribble(line)) for line in lines),
                lag_ratio=0.1,
                run_time=2,
            )
        )
    ...
    def blink(self, bot):
        self.play(bot.eyes.animate.stretch(0, 1).set_anim_args(rate_func=squish_rate_func(there_and_back)))
...
class SerialProcessing(InteractiveScene):
    ...
    def construct(self):
        ...
        vectors = VGroup(self.get_abstract_vector().next_to(word, DOWN, LARGE_BUFF) for word in words)
...
class ParallelProcessing(SerialProcessing):
    def construct(self):
        ...
        # Animate in the vectors
        vectors = VGroup(self.get_abstract_vector().next_to(word, DOWN, buff=1.5) for word in words)
...
class ManyComputationsPerUnitTimeV2(InteractiveScene):
    def construct(self):
        ...
        new_boxes = VGroup(box.copy().move_to(tick.get_center(), DL) for tick in minute_line.ticks[1:-1])
        for new_box in new_boxes:
            new_box.save_state()
            new_box.move_to(box)
        computations = VGroup(self.get_computations(new_box, n_iterations=1) for new_box in new_boxes)
        ...
        self.play(
            ...
            self.frame.animate.set_y(number_lines[-1].get_y() + 2)
            .set_width(18)
            .set_anim_args(
        ...
        self.play(
            self.frame.animate.reorient(0, 0, 0, (-0.03, -11.55, 0.0), 31.76),
            run_time=4,
        )
        self.wait(4)
    ...
    def get_computations(self, box, n_lines=10, n_iterations=3, n_digits=4, cycle_time=0.5):
        ...
        for iteration in range(n_iterations):
            ...
            for n in range(n_lines):
                ...
                if random.choice([True, False]):
                    ...
                    sym = Tex(R"+")
                line = VGroup(
                    DecimalNumber(x, num_decimal_places=3),
                    sym,
                    DecimalNumber(y, num_decimal_places=3),
                    Tex("="),
        ...
        def update_lines(lines):
            ...
            for n, line in enumerate(lines):
                x = min((abs(a - n * step) for a in (alpha - 1, alpha, alpha + 1)))
    ...
    def old(self):
        ...
        self.play(
            LaggedStart(
                (TransformFromCopy(lines[0].copy().set_opacity(0), line) for line in lines),
                lag_ratio=0.03,
                run_time=2,
            ),
...
class VectorLabel(InteractiveScene):
    def construct(self):
        ...
        self.play(FlashUnder(text, color=YELLOW))
        self.wait()
...
class ParameterToVectorAnnotation(InteractiveScene):
    def construct(self):
        ...
        arrows = VGroup(Vector(0.5 * RIGHT, thickness=2).next_to(dial, RIGHT, buff=SMALL_BUFF) for dial in dials)
        ...
        self.play(
            LaggedStart(
                (dial.animate_set_value(value) for dial, value in zip(dials, values)),
                lag_ratio=0.05,
            )
        )
        self.wait()
...
class ThreeWordsToOne(InteractiveScene):
    def construct(self):
        ...
        rect.set_color(RED)
        chm_image = ImageMobject(
            "/Users/grant/3Blue1Brown Dropbox/3Blue1Brown/videos/2024/transformers/chm/images/CHM_Exterior.jpeg"
        )
        chm_image.match_width(rect)
        chm_image.next_to(rect, DOWN)

        self.play(Transform(words, og_words))
        self.play(ShowCreation(rect), FadeIn(chm_image, DOWN))
        self.wait()
        ...
        self.play(
            ...
            Restore(words),
            *(FadeTransform(chm_image.copy(), icon) for icon in icons),
        )
        self.wait()
...
class TrainingDataCHM(InteractiveScene):
    def construct(self):
        ...
        items = VGroup(get_paragraph(passage.split(" "), line_len=35, font_size=30) for passage in passages)
...
class DivyUpParameters(ShowMachineWithDials):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (dial.animate_set_value(dial.get_random_value()) for dial in all_dials),
                lag_ratio=1 / len(all_dials),
                run_time=6,
            ),
            LaggedStart(
                (FlashAround(dial, buff=0, color=YELLOW) for dial in all_dials),
                lag_ratio=1 / len(all_dials),
                run_time=6,
            ),
        )
