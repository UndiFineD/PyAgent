...
def next_token_bar_chart(
    ...
    bars = VGroup(Rectangle(prob ** (prob_exp) * width_100p, bar_height) for prob, label in zip(probs, labels))
...
class SimpleAutogregression(InteractiveScene):
    ...
    def animate_prediction_ouptut(self, machine, cur_str):
        ...
        self.play(
            LaggedStart(
                (FadeInFromPoint(bar_group, machine[0][-1].get_right()) for bar_group in bar_groups),
                lag_ratio=0.025,
                group=bar_groups,
                run_time=1,
            )
    ...
    def new_selection_cycle(self, text_mob, next_word_line, machine, quick=False, skip_anims=False):
        ...
        new_text_mob = self.animate_word_addition(bar_groups, text_mob, next_word_line, force_unskip=skip_anims)
        return new_text_mob
    ...
    def predict_next_token(self, text):
        ...
        if self.model == "gpt3":
            try:
                result = gpt3_predict_next_token(text, n_shown, random_seed=self.random_seed)
...
class ChatBotPrompt(SimpleAutogregression):
    ...
    def construct(self):
        ...
        system_prompt, user_prompt, ai_seed = all_text = VGroup(
            get_paragraph(s.split(" "), font_size=self.font_size, line_len=self.line_len) for s in all_strs
        )
...
class PredictionVsGeneration(SimpleAutogregression):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                (FadeInFromPoint(bar, prediction[1].get_center()) for bar in prediction[2]),
                lag_ratio=0.05,
            ),
        )
...
class ManyParallelPredictions(SimpleAutogregression):
    ...
    def construct(self):
        ...
        predictions = VGroup(self.get_prediction_group(substring) for substring in substrings)
