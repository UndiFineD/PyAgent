# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\embedding.py
...
class LyingAboutTokens2(InteractiveScene):
    def construct(self):
        ...
        self.play(
            FadeOut(big_rect),
            LaggedStart(*(DrawBorderThenFill(rect) for rect in rects[:-1]), lag_ratio=0.02),
            LaggedStart(*(word.animate.match_color(rect) for word, rect in zip(words, rects))),
            FadeOut(arrow),
        )
        self.wait()

        # Show words into vectors
        vectors = VGroup(*(NumericEmbedding(length=8) for word in words))
        ...
        arrows = VGroup(*(Arrow(block, vect, stroke_width=3) for block, vect in zip(blocks.target, vectors)))
        ...
        self.play(
            ...
            LaggedStart(*(token.animate.set_color(rect.get_color()) for token, rect in zip(tokens, token_rects))),
        )
        self.wait()
        self.play(
        ...
        self.play(
            ...
            LaggedStart(*(token.animate.set_color(rect.get_color()) for token, rect in zip(words, word_rects))),
        )
        self.wait()
...
class ImageTokens(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (patch.animate.set_stroke(TEAL, 3).set_anim_args(rate_func=there_and_back) for patch in patches),
...
class IntroduceEmbeddingMatrix(InteractiveScene):
    def construct(self):
        ...
        matrix = WeightMatrix(shape=(10, len(shown_words)), ellipses_col=dots_index)
        ...
        shown_words.target[dots_index].rotate(45 * DEGREES).move_to(shown_words.target[dots_index - 1 : dots_index + 2])
        new_brace = Brace(shown_words.target, UP, buff=0.0)
        column_rects = VGroup(*(SurroundingRectangle(column, buff=0.05) for column in columns))
        ...
        self.play(
            ...
            LaggedStart(
                *(Write(column, lag_ratio=0.01, stroke_width=1) for column in columns),
                lag_ratio=0.2,
                run_time=2,
            ),
        ...
        self.play(frame.animate.set_width(FRAME_WIDTH + 3, about_edge=RIGHT), Write(lhs))
        self.wait()
        ...
        self.play(FadeOut(word_group, DOWN), matrix_group.animate.scale(2.0).move_to(frame))
        ...
        shown_tokens[dots_index].move_to(shown_tokens[dots_index - 1 : dots_index + 2 : 2])
        ...
        self.play(FadeTransform(top_equation[1:5:2].copy(), top_equation[-1]))
...
class AmbientWordEmbedding(Word2VecScene):
    def construct(self):
        ...
        coords = np.array([self.basis @ self.model[word.lower()] for word in words])
        ...
        # Transition
        self.add(turn_animation_into_updater(Write(arrow_label, time_span=(1, 3))))
        for label, vect in zip(pre_labels, embeddings):
            self.add(turn_animation_into_updater(TransformFromCopy(label, vect.label, run_time=2)))
            self.add(turn_animation_into_updater(FadeIn(vect, run_time=1)))
            self.wait(0.5)
...
class ThreeDSpaceExample(InteractiveScene):
    def construct(self):
        ...
        bases = [normalize(point) for point in plane.get_points()[:2]]

        def project(points):
            return np.array([sum(np.dot(point, b) * b for b in bases) for point in points])

        projected_points = points.copy()
        projected_points.apply_points_function(project)

        projection_lines = VGroup(*(Line(p1, p2) for p1, p2 in zip(points.get_points(), projected_points.get_points())))
        ...
        self.play(FadeOut(points), FadeOut(projection_lines), FadeOut(axes))
        self.wait(15)
...
class HighDimensionalSpaceCompanion(InteractiveScene):
    def construct(self):
        ...
        def get_bubble():
            ...
            result.apply_complex_function(lambda z: z * (1 + 0.025 * np.cos(5 * np.log(z).imag + self.time)))
        ...
        dimension_label = VGroup(label[0].copy(), Text("-dimensional"))
...
class LearningEmbeddings(Word2VecScene):
    def construct(self):
        ...
        true_embeddings = np.array([self.basis @ self.model[word] for word in words])
        ...
        initial_coords = [rotate_vector(amp * OUT, theta, axis=UP) for theta, amp in zip(thetas, amps)]
        ...
        for vect in word_vects:
            ...
            label.vect = vect
            label.add_updater(lambda m: m.move_to(m.vect.get_end() + 0.25 * normalize(m.vect.get_vector())))
            labels.add(label)
...
class KingQueenExample(Word2VecScene):
    ...
    def construct(self):
        ...
        self.play(frame.animate.reorient(-110, 10, 110, (0.22, 1.6, -0.07), 6.72), run_time=10)
        ...
        self.play(LaggedStartMap(MoveToTarget, [minus1, ek, approx], path_arc=PI / 2))
        ...
        self.play(
            LaggedStart(
                *(Transform(src, trg) for src, trg in zip(src_circles, trg_circles)),
                lag_ratio=0.15,
                run_time=3,
            )
        )
        ...
        turn_animation_into_updater(ApplyMethod(frame.reorient, -116, 21, 114, (0.37, 1.45, 0.23), 7.59, run_time=12))
        ...
        for word1, word2 in word_pairs:
            ...
            vect2.put_start_and_end_on(
                ORIGIN,
                vect1.get_end() + diff.get_vector() + np.random.uniform(-0.1, 0.1, 3),
            )
            ...
            self.play(
                ...
                *(FadeTransform(sm1, sm2) for sm1, sm2 in zip(last_equation, new_equation)),
            )
        ...
        self.play(LaggedStartMap(VShowPassingFlash, lines, lag_ratio=1 / len(lines), run_time=4))
...
class HitlerMussoliniExample(KingQueenExample):
    ...
    def construct(self):
        ...
        # Initialize vectors
        v1, v2, v3, v4 = vects = [
            self.get_labeled_vector(word, color=color) for word, color in zip(self.words, self.colors)
        ]
        ...
        self.play(TransformFromCopy(diff, diff_copy), rect.animate.surround(equation[:5]))
        self.wait(2)
        self.play(
...
class SizeDirection(Word2VecScene):
    def construct(self):
        ...
        vect_groups = VGroup(
            VGroup(self.get_labeled_vector(word, color=color, func_name=None) for word, color in zip(word_list, colors))
...
class ShowNearestNeighbors(Word2VecScene):
    ...
    def construct(self):
        ...
        items = VGroup(*(Text(f"E({word})", font_size=36) for word in nearest_words))
...
class DotProducts(InteractiveScene):
    def construct(self):
        ...
        # Vector labels
        vect_labels = VGroup(*(Tex(Rf"\vec{{\textbf{{ {char} }} }}") for char in "vw"))
        for label, vect in zip(vect_labels, vects):
            label.vect = vect
            label.match_color(vect)
            label.add_updater(lambda l: l.move_to(l.vect.get_end() + 0.25 * normalize(l.vect.get_vector())))
        ...
        vect_coords = VGroup(
            *(
                TexMatrix(
                    [[char + f"_{{{str(n)}}}"] for n in [1, 2, 3, 4, "n"]],
                    bracket_h_buff=0.1,
                    ellipses_row=-2,
                )
                for char in "vw"
        ...
        # Show computation
        equals = rhs[0].copy()
        entry_pairs = VGroup(*(VGroup(*pair) for pair in zip(*[vc.get_columns()[0] for vc in vect_coords])))
        ...
        self.play(
            LaggedStart(
                *(TransformFromCopy(m1, m2) for m1, m2 in zip(entry_pairs, prod_terms)),
                lag_ratio=0.1,
                run_time=2,
            ),
            MoveToTarget(rhs),
...
class DotProductWithPluralDirection(InteractiveScene):
    ...
    def construct(self):
        ...
        rhs = Tex(
            ...
            tex_to_color_map={Rf"\text{{{ref_word}}}": color for ref_word, color in zip(self.ref_words, self.colors)},
        )
    ...
    def add_dot(self, word, dot):
        self.play(
            ...
            LaggedStart(
                (FadeTransform(char.copy(), dot.copy().set_opacity(0)) for char in word),
                lag_ratio=2e-2,
                group_type=Group,
            ),
            run_time=1,
    ...
    def get_equation_lhs(self, word):
        tex_pieces = [self.vec_tex, R"\cdot", Rf"E(\text{{{word}}})", "="]
        expression = Tex(" ".join(tex_pieces), tex_to_color_map={self.vec_tex: YELLOW})
        parts = [expression[tex_piece][0] for tex_piece in tex_pieces]
        gen_part = parts[0]
...
class RicherEmbedding(InteractiveScene):
    def construct(self):
        ...
        # Mention position
        index_labels = VGroup(
            *(Integer(n + 1, font_size=36).next_to(rect, DOWN, buff=0.2) for n, rect in enumerate(rects))
        )
        ...
        for args in new_labeled_vector_args:
            new_labeled_vects.add(self.get_added_vector(last_vect.get_end(), *args))
        ...
        self.play(frame.animate.reorient(-19, -25, 0, (0.61, 0.01, 0.0), 6.10), run_time=5)

    def get_added_vector(
        self,
        curr_tip,
        direction,
        label,
...
class MultipleMoleEmbeddings(Word2VecScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(gen_vector, ref_vect) for ref_vect in ref_vects),
                lag_ratio=0.25,
                run_time=2,
            ),
            LaggedStart(
                (FadeInFromPoint(image, gen_vector.label.get_center()) for image in images),
                lag_ratio=0.25,
                run_time=2,
                group_type=Group,
            ),
        ...
        ref_vect_groups = Group(Group(*pair) for pair in zip(ref_vects, images))

        # Oscillate between meanings based on context
        diff_vects = VGroup(Arrow(gen_vector.get_end(), ref_vect.get_end(), buff=0) for ref_vect in ref_vects)
...
class RefineTowerMeaning(MultipleMoleEmbeddings):
    def construct(self):
        ...
        tower_images = Group(ImageMobject(f"Tower{n}") for n in range(1, 5))
        eiffel_tower_images = Group(ImageMobject(f"EiffelTower{n}") for n in range(1, 4))
        mini_eiffel_tower_images = Group(ImageMobject("MiniEiffelTower1"))
        image_groups = Group(tower_images, eiffel_tower_images, mini_eiffel_tower_images)
        ...
        for ti1, ti2 in zip(tower_images, tower_images[1:]):
            self.play(FadeTransform(ti1, ti2), run_time=2)
        self.wait(2)
...
class UpdatingPoetryEmbedding(RicherEmbedding):
    def construct(self):
        ...
        self.play(frame.animate.reorient(22, -23, 0, (-0.86, 0.4, -0.35), 7.15), run_time=5)
...
class MJSpace(SimpleSpaceExample):
    def construct(self):
        ...
        arrs = VGroup(NumericEmbedding().scale(0.25).next_to(rect, DOWN, buff=1.0) for rect in token_rects)
        arrows = VGroup(Arrow(rect, arr, buff=0.1) for rect, arr in zip(token_rects, arrs))
        vects = VGroup(Vector(np.random.uniform(-3, 3, 3)) for arr in arrs)
        ...
        self.play(
            ...
            LaggedStart(
                (Transform(arrow, vect) for arrow, vect in zip(arrs, vects)),
                lag_ratio=0.05,
            ),
            run_time=3,
        )
        ...
        self.play(
            LaggedStart(
                (vect.animate.scale(0, about_point=vect.get_start()) for vect in vects),
                lag_ratio=0.05,
                remover=True,
            )
        )
        ...
        for vect, label, orientation in zip(vects, labels, orientations):
            ...
            self.play(LaggedStartMap(ShowCreationThenFadeOut, lines, lag_ratio=2 / len(lines)))
            self.wait(2)
        ...
        formula = Tex(
            R"\vec{\textbf{E}} \cdot \big(\overrightarrow{\text{First Name Michael}}\big) = ",
            font_size=36,
        )
        ...
        self.wait(2)
        self.play(
            emb.animate.put_start_and_end_on(axes.get_origin(), 1.5 * all_coords[1:3].sum(0)),
            run_time=3,
        )
        ...
        self.play(
            emb.animate.put_start_and_end_on(ORIGIN, [-1.5, -1.5, 0]).set_anim_args(
        ...
        self.play(*map(FadeOut, [m_vect_copy, j_vect_copy, emb]))
        self.play(
