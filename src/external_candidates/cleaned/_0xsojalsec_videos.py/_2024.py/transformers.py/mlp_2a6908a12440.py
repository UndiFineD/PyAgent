...
class LastTwoChapters(InteractiveScene):
    ...
    def get_thumbnails(self):
        ...
        thumbnails = Group(
            Group(SurroundingRectangle(image, buff=0).set_stroke(WHITE, 3), image) for n, image in zip([5, 6], images)
        )
...
class BasicMLPWalkThrough(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                (TransformFromCopy(simple_phases, sp_copy) for sp_copy in simple_phase_copies),
                lag_ratio=0.1,
            ),
        ...
        n, m = up_proj.shape
        n_rows_shown = 5
        R_labels = VGroup(Tex(R"\vec{\textbf{R}}_" + f"{{{n}}}") for n in [*range(n_rows_shown - 1), "n"])
        ...
        self.play(
            ...
            LaggedStart(*map(FadeOut, [fe_rect, fe_eq, fe_expr, under_brace, *conditions])),
        )
        ...
        self.wait()
        self.play(
            *(TransformFromCopy(relu_title_full[substr], relu_graph_label[substr]) for substr in ["Re", "L", "U"])
        )
        ...
        scaled_cols = VGroup(
            VGroup(n_label, col_label[0]).copy() for n_label, col_label in zip(n_labels, col_matrix[1])
        )
    ...
    def get_col_matrix(
        ...
        C_labels = VGroup(Tex(Rf"\vec{{\textbf{{{sym}}}}}_{{{n}}}") for n in [*range(n_cols_shown - 1), top_index])
...
class NonlinearityOfLanguage(InteractiveScene):
    def construct(self):
        ...
        plane = NumberPlane(
            ...
            background_line_style=dict(stroke_color=BLUE_D, stroke_width=1, stroke_opacity=0.75),
            faded_line_ratio=1,
            unit_size=unit_size,
        )
...
class BreakDownThreeSteps(BasicMLPWalkThrough):
    def construct(self):
        ...
        n, m = up_proj.shape
        n_rows_shown = 8
        R_labels = VGroup(Tex(R"\vec{\textbf{R}}_{" + str(n) + "}") for n in [*range(n_rows_shown - 1), "n-1"])
        R_labels[-2].become(Tex(R"\vdots").replace(R_labels[-2], dim_to_match=1))
        R_labels.arrange(DOWN, buff=0.5)
        R_labels.match_height(up_proj)
        R_labels.move_to(up_proj)
