# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\helpers.py
...
def load_image_net_data(dataset_name="image_net_1k"):
    ...
    return [(Path(image_dir, f"{index}.jpeg"), label) for index, label in enumerate(labels)]
...
def show_matrix_vector_product(scene, matrix, vector, buff=0.25, x_max=999, fix_in_frame=False):
    ...
    for n, row, entry in zip(it.count(), matrix.get_rows(), rhs[:-2]):
        if matrix.ellipses_row is not None and n == (matrix.ellipses_row % n_rows):
            ...
            last_rects = matrix_row_vector_product(scene, row, vector, entry, last_rects, fix_in_frame=fix_in_frame)
...
def matrix_row_vector_product(scene, row, vector, entry, to_fade, fix_in_frame=False):
    ...
    scene.play(
        ...
        UpdateFromAlphaFunc(
            entry,
            lambda m, a: m.set_value(partial_values[min(int(np.round(a * n_values)), n_values - 1)]),
        ),
        FadeOut(to_fade),
        rate_func=linear,
    )
...
def get_full_matrix_vector_product(
    ...
    matrix = TexMatrix(
        [[Rf"{mat_sym}_{{{m}, {n}}}" for n in n_indices] for m in m_indices],
    ...
    result_terms = [[Rf"w_{{{m}, {n}}} x_{n}" for n in n_indices] for m in m_indices]
    rhs = TexMatrix(
        result_terms,
        ellipses_row=ellipses_row,
        ellipses_col=ellipses_col,
...
def show_symbolic_matrix_vector_product(scene, matrix, vector, rhs, run_time_per_row=0.75):
    ...
    for mat_row, rhs_row in zip(matrix.get_rows(), rhs.get_rows()):
        ...
        scene.play(
            FadeOut(last_rects),
            *(ShowIncreasingSubsets(group, rate_func=linear) for group in [mat_rects, vect_rects, rhs_row]),
...
def data_flying_animation(
    ...
    return UpdateFromAlphaFunc(
        word,
        lambda m, a: m.move_to(interpolate(point, point + vect, a)).set_opacity(there_and_back(a) * max_opacity),
    )
...
def get_data_modifying_matrix_anims(
    ...
    return [
        LaggedStart(
            (data_flying_animation(p, vect=shift_vect, fix_in_frame=fix_in_frame, font_size=font_size) for p in points),
...
def create_pixels(image_mob, pixel_width=0.1):
    ...
    points = np.array([[x, y, 0] for y in np.arange(y0, y1, -pixel_width) for x in np.arange(x0, x1, pixel_width)])
    square = Square(pixel_width).set_fill(WHITE, 1).set_stroke(width=0)
    pixels = VGroup(
        square.copy().move_to(point, UL).set_color(Color(rgb=image_mob.point_to_rgb(point))) for point in points
    )
    return pixels
...
class NeuralNetwork(VGroup):
    def __init__(
        ...
        layers = VGroup(*(Dot(radius=neuron_radius).get_grid(n, 1, v_buff_ratio=v_buff_ratio) for n in layer_sizes))
...
class EmbeddingArray(VGroup):
    def __init__(
        ...
        # Embeddings
        embeddings = VGroup(NumericEmbedding(length=shape[0]) for n in range(shape[1]))
...
class RandomizeMatrixEntries(Animation):
    def __init__(self, matrix, **kwargs):
        ...
        self.target_values = np.random.uniform(matrix.value_range[0], matrix.value_range[1], len(self.entries))
...
class Dial(VGroup):
    def __init__(
        ...
        **kwargs,
        ...
        self.ticks = VGroup(*(Line((1.0 - relative_tick_size) * point, point) for point in tick_points))
        ...
        # Needle
        self.needle = Line()
        self.needle.set_stroke(color=needle_color, width=[needle_stroke_width, 0])
    ...
    def set_value(self, value):
        self.needle.put_start_and_end_on(self.get_center(), self.value_to_point(value))
    ...
    def animate_set_value(self, value, **kwargs):
        ...
        angle1, angle2 = [(angle_of_vector(vect) + TAU / 4) % TAU - TAU / 4 for vect in vects]
        ...
        return AnimationGroup(
            ...
            *(VShowPassingFlash(diff_arc, time_width=1.5, **kwargs) for diff_arc in diff_arcs),
        )
...
class MachineWithDials(VGroup):
    ...
    def random_change_animation(self, lag_factor=0.5, run_time=3.0, **kwargs):
        return LaggedStart(
            *(dial.animate_set_value(dial.get_random_value()) for dial in self.dials),
    ...
    def rotate_all_dials(self, run_time=2, lag_factor=1.0):
        ...
        return LaggedStart(
            *(Rotate(dial.needle, TAU, about_point=dial.get_center()) for dial in shuffled_dials),
            lag_ratio=lag_factor / len(self.dials),
        )
