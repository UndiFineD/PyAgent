# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\transformers\network_flow.py
import torch
...
class HighLevelNetworkFlow(InteractiveScene):
    ...
    def show_initial_text_embedding(self, word_scale_factor=0.6, bump_first=False):
        ...
        self.play(
            ...
            LaggedStart(*(token.animate.match_color(rect) for token, rect in zip(words, rects))),
            FadeOut(arrow),
        )
        self.wait()
        ...
        arrows = VGroup(Arrow(token_label.get_bottom(), rect.get_top()).match_color(rect) for rect in rects[:-1])
    ...
    def progress_through_attention_block(
        ...
        if False:
            ...
            new_arrow = Arrow(
                self.token_blocks[index].get_bottom(),
                rect.get_top(),
    ...
    def progress_through_mlp_block(
        ...
        for embedding in layer.embeddings:
            ...
            l2_points = np.array([[x0, y, block_z] for y in np.linspace(y_min, y_max, n_neurons)])
        ...
        if show_one_by_one:
            ...
            self.play(
                LaggedStart(
                    *(FadeTransform(e1.copy(), e2) for e1, e2 in [*emb_pairs[:index], *emb_pairs[index + 1 :]]),
                    **lag_kw,
                ),
                LaggedStart(
                    *(Write(network[1]) for network in [*networks[:index], *networks[index + 1 :]]),
                    **lag_kw,
                ),
                LaggedStart(
                    *(FadeIn(network[0]) for network in [*networks[:index], *networks[index + 1 :]]),
                    **lag_kw,
                ),
    ...
    def focus_on_last_layer(self):
        ...
        self.play(
            ...
            *map(
                FadeOut,
                [block.title for block in self.blocks if hasattr(block, "title")],
            ),
...
class SimplifiedFlow(HighLevelNetworkFlow):
    ...
    def show_simple_flow(self, x_range, orientation=None):
        ...
        self.frame.add_updater(
            lambda f: f.set_height(curr_height + self.frame_height_growth_rate * (self.time - curr_time))
        )
        for x in x_range:
...
class MentionContextSizeAndUnembedding(SimplifiedFlow):
    ...
    def construct(self):
        ...
        self.show_simple_flow(np.linspace(-2, -5, 2), orientation=(-50, -19, 0))
    ...
    def cycle_through_embeddings(self):
        ...
        rect_groups = VGroup(
            *(
                VGroup(*(BackgroundRectangle(mob, buff=0.2 if mob in arrows else 0.1) for mob in tup))
    ...
    def show_context_size(self):
        ...
        self.wait()
        self.play(MoveToTarget(label), Write(rhs))
        self.wait()
        self.play(
        ...
        self.play(
            ...
            LaggedStartMap(FadeOut, VGroup(brace, label, rhs, dim_brace, dim_label), shift=DOWN),
        )
    ...
    def show_unembedding_matrix(self, vector_index=-1):
        ...
        self.play(
            ...
            *map(
                FadeOut,
                [self.token_blocks, self.token_arrows, self.final_word_question],
            ),
        )
        self.play(ShowCreation(rect))
        ...
        rects = VGroup(*(SurroundingRectangle(emb) for emb in layer.embeddings[:-1]))
        ...
        for rect in rects:
            question_arrows.add(Arrow(question.get_top(), rect.get_bottom(), stroke_color=rect.get_color()))
        ...
        self.wait()
        self.play(FadeIn(label, DOWN), name.animate.next_to(label, UP, buff=1))
        self.wait()
        ...
        self.play(FadeTransform(top_equation[1:4].copy(), top_equation[-1]))
        self.wait()
...
class ThumbnailBase(HighLevelNetworkFlow):
    ...
    def construct(self):
        ...
        self.show_initial_text_embedding(word_scale_factor=0.6)
        for x in range(4):
            self.progress_through_attention_block(depth=1.5, label_text="Att")
            self.progress_through_mlp_block(depth=2.0, label_text="MLP")
...
class MoleExample1(HighLevelNetworkFlow):
    ...
    def construct(self):
        ...
        mole_vect_rects = VGroup(SurroundingRectangle(embs[index]) for index in mole_indices)
        ...
        self.wait()
        self.play(FadeOut(mole_vect_rects))

        # Prepare to pass through an attention block
        wg_lens = [len(wg) for wg in word_groups]
        indices = [0, *np.cumsum(wg_lens)]
        full_groups = VGroup(VGroup(wg, arrows[i:j], embs[i:j]) for wg, i, j in zip(word_groups, indices, indices[1:]))
