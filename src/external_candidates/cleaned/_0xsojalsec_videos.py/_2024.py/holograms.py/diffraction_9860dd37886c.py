# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2024\holograms\diffraction.py
...
class LightIntensity(LightWaveSlice):
    def __init__(self, *args, color: ManimColor = BLUE, show_intensity: bool = True, **kwargs):
...
class LightFieldAroundScene(InteractiveScene):
    def construct(self):
        ...
        # Slow zoom out
        self.play(frame.animate.to_default_state(), run_time=12)
        ...
        laser_light = VGroup(
            Line(laser_point, film_rect.pfp(a + random.random() / 250)) for a in np.linspace(0, 1, 250)
        )
...
class DiffractionGratingScene(InteractiveScene):
    ...
    def get_point_sources_from_wall(self, wall, z=0):
        sources = GlowDots(np.array([midpoint(p1.get_right(), p2.get_left()) for p1, p2 in zip(wall, wall[1:])]))
...
class LightExposingFilm(DiffractionGratingScene):
    def construct(self):
        ...
        graph = self.get_graph_over_wave(
            wave_line,
            wave,
            direction=UP,
            scale_factor=wave_amp_tracker.get_value(),
            n_curves=200,
        )
        ...
        phase_arrow = Arrow(
            phase_text.get_corner(IN + UP) + 0.2 * (OUT + UP),
            phase_line.get_start(),
            buff=0,
        )
        ...
        ref_amp = 0.75
        graph2 = self.get_graph_over_wave(
            line2,
            wave2,
            direction=UP,
            scale_factor=ref_amp,
            n_curves=int(200 * math.sqrt(2)),
        )
        ...
        # Zoom in
        self.play(frame.animate.reorient(-58, -18, 0, (3.29, 0.39, 0.55), 10.68), run_time=4)
        self.play(exposure.animate.set_opacity(1).set_max_amp(0.1), run_time=2)
        self.wait(3)
...
class TwoInterferingWaves(DiffractionGratingScene):
    def construct(self):
        ...
        self.play(frame.animate.reorient(17, -24, 0, (-5.5, -0.82, -0.67), 15.86), run_time=30)
...
class SinglePointOnFilm(DiffractionGratingScene):
    def construct(self):
        ...
        source_point.move_to([0.0, -0.75, 4.62])
        ...
        # Spherical waves
        wave_stack = Group(obj_wave.copy().rotate(PI / 2, OUT).set_opacity(0.1) for x in range(30))
...
class ExplainWaveVisualization(DiffractionGratingScene):
    def construct(self):
        ...
        self.play(
            sample_vect.base_point.animate.move_to(1.315 * UP).set_color(RED),
            run_time=3,
        )
        self.wait()
        self.play(
            sample_vect.base_point.animate.move_to(1.01 * UP).set_color(GREY_E),
            run_time=2,
        )
        self.wait()
        self.play(
        ...
        added_waves = Group(
            wave.copy().rotate(PI / 2, UP).shift(z * RIGHT).set_opacity(0.1) for z in np.arange(-2, 2, 0.1)
        )
...
class CreateZonePlate(DiffractionGratingScene):
    ...
    def construct(self):
        ...
        wave_fronts = Group(plate.copy().set_color([BLUE, RED][z % 2], 0.15).shift(0.5 * z * OUT) for z in range(4, 16))
        ...
        obj_line, ref_line = lines = VGroup(
            Line(source_point.get_center(), film.get_center()).set_stroke(color, 1, 0.5) for color in colors
        )
        ...
        obj_label.add_updater(
            lambda m: m.rotate(
                angle_of_vector((m[-1].get_center() - m[0].get_center())[0::2])
                - angle_of_vector(obj_line.get_vector()[0::2]),
                axis=UP,
        ...
        self.play(frame.animate.reorient(-85, -9, 0, (0.65, -0.24, -0.14), 6.52), run_time=4)
    ...
    def get_round_exposure(self, exposure, radius=1.0, n_pieces=128):
        ...
        result = Group(exposure.copy().set_points([ORIGIN, v1, v2]) for v1, v2 in zip(vects, vects[1:]))
    ...
    def get_3d_waves(self, wave, x_range=(-4, 4, 0.5), opacity=0.25):
        waves = Group(wave.copy().rotate(PI / 2, OUT).move_to(x * RIGHT) for x in np.arange(*x_range))
    ...
    def construct(self):
        ...
        self.play(
            Rotate(
                ...
                60 * DEGREES,
                axis=UP,
                about_point=source_points.get_points()[0],
                run_time=5,
            ),
...
class DoubleSlit(DiffractionGratingScene):
    def construct(self):
        ...
        self.play(frame.animate.reorient(0, 52, 0, (0.91, 35.42, 33.91), 102.49), run_time=16)
        ...
        self.play(frame.animate.reorient(11, 65, 0, (-0.03, 0.06, 0.14), 8.66), run_time=3)
        ...
        line.f_always.put_start_and_end_on(source.get_center, exposure_glow.get_center)
        ...
        self.wait(4)
        self.play(frame.animate.reorient(-25, 48, 0, (0.23, 4.15, -0.03), 9.80), run_time=8)
        self.play(frame.animate.reorient(28, 50, 0, (0.23, 4.15, -0.03), 9.80), run_time=8)
        ...
        self.play(frame.animate.reorient(0, 68, 0, (0.23, 4.15, -0.03), 9.80), run_time=4)
        ...
        self.play(
            *(wave.animate.unpause().set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2])
        )
        self.wait(4)
        self.play(
            frame.animate.reorient(0, 66, 0, (0.12, 4.03, -0.38), 7.52),
            *(wave.animate.pause().set_anim_args(suspend_mobject_updating=False) for wave in [out_wave, wave1, wave2]),
            run_time=3,
        )
        self.wait()

        # Shift to a destructive point
        self.play(exposure_point.animate.shift(0.9 * RIGHT).set_opacity(0.25), run_time=2)
        self.wait()
        self.play(
        ...
        self.play(exposure_point.animate.shift(0.9 * RIGHT).set_opacity(1), run_time=2)
        self.wait(2)
        self.play(
        ...
        self.play(exposure_point.animate.shift(5 * LEFT), run_time=6)
        self.play(exposure_point.animate.move_to(film.get_center()), run_time=5)
        ...
        self.play(frame.animate.reorient(-80, 68, 0, (-0.22, 2.44, 0.8), 6.42), run_time=3)
        self.play(
...
class FullDiffractionGrating(DiffractionGratingScene):
    def construct(self):
        ...
        individual_sources = Group(
            sources.copy().set_points(sources.get_points()[i : i + 1]) for i in range(sources.get_num_points())
        )

        waves = Group(out_wave.copy().set_sources(src).set_opacity(0) for src in individual_sources)
        waves.scale(0)

        graphs = VGroup(self.get_graph_over_wave(line, wave, scale_factor=0.25) for line, wave in zip(lines, waves))
        ...
        self.wait(3)
        self.play(Rotate(beam_point, -theta, about_point=ORIGIN), run_time=1)
        self.wait()
        self.play(Rotate(beam_point, 2 * theta, about_point=ORIGIN), run_time=1)
        self.wait(8)
        ...
        self.play(TransformFromCopy(d_line, v_line), ShowCreation(arc), Write(theta_sym))
        self.wait(2)
        self.play(
        ...
        self.wait(4)
        self.play(point_tracker.animate.move_to(1.15 * point_tracker.get_center()), run_time=2)
        ...
        diff_label_group = always_redraw(
            lambda: self.get_diff_label_group(
        ...
        self.wait()
        self.play(TransformFromCopy(line2, matched_segment), line1.animate.set_stroke(width=1))
        self.wait()
        self.play(
            LaggedStart(
                GrowFromCenter(brace),
        ...
        sine.add_updater(
            lambda m: m.put_start_and_end_on(diff_segment.get_start(), diff_segment.get_end()).scale(
                n_cycles, about_point=diff_segment.get_start()
            )
        )
        ...
        self.wait()
        self.play(FadeIn(lock_label), GrowArrow(lock_arrow))
        self.wait()
        ...
        for scale_factor in scale_factors:
            ...
            self.play(
                UpdateFromFunc(
                    point_tracker,
                    lambda m: m.move_to(get_dist_point(1.0 / wave_number)),
                ),
        ...
        if False:  # This was just a temporary insert, not to be run in general
            d_angle2, d_angle3 = [
                angle_of_vector(get_dist_point(n / wave_number))
                - angle_of_vector(get_dist_point((n + 1) / wave_number))
                for n in (1, 2)
            ]
            sine.clear_updaters()
        ...
        self.play(MoveToTarget(angle_group), run_time=2)
        self.wait()

        # Write conclusion
        conclusion = VGroup(Text("Difference in distance:", font_size=36), Tex(R"d \cdot \sin(\theta)"))
        ...
        self.play(
            ...
            FadeOut(
                VGroup(d_sine_theta, matched_segment, line1, line2, angle_group),
                lag_ratio=0.1,
                time_span=(0, 1.5),
            ),
        ...
        arc = always_redraw(
            lambda: Arc(
                PI / 2,
                angle_of_vector(point_tracker.get_center()) - PI / 2,
                radius=50,
                stroke_color=WHITE,
                stroke_width=100,
            )
        ...
        for n in [1, 2]:
            d_angle = angle_of_vector(get_dist_point(n / wave_number)) - angle_of_vector(
                get_dist_point((n + 1) / wave_number)
            )
            self.play(
...
class PlaneWaveThroughZonePlate(DiffractionGratingScene):
    def construct(self):
        ...
        terms = VGroup(d_lines, lil_brace, big_brace, L_label, x_label, d_label)
        ...
        equations = VGroup(Tex(eq, t2c={R"\lambda": YELLOW, "L": BLUE, "x": RED}, font_size=36) for eq in equations_tex)
        ...
        annotations = VGroup(
            ...
            TexText(
                R"Linear approximation:\\ \quad \\$\sqrt{X + \epsilon} \approx \sqrt{X} + \frac{1}{2\sqrt{X}} \epsilon$"
            ),
        )
        ...
        self.play(*(GrowFromPoint(beam[1], film_point, run_time=2, rate_func=linear) for beam in out_beams))
        self.wait(4)
        ...
        arc.add_updater(lambda m: m.become(Arc(PI, line.get_angle()).shift(obj_dot.get_center())))
        upper_arc.add_updater(lambda m: m.match_points(arc).shift(film_dot.get_center() - obj_dot.get_center()))
        theta_prime_sym.add_updater(
            lambda m: m.set_height(min(0.8 * arc.get_height(), 0.35)).next_to(arc.pfp(0.6), LEFT, SMALL_BUFF)
        )
        theta_sym.add_updater(
            lambda m: m.replace(theta_prime_sym[0]).shift(film_dot.get_center() - obj_dot.get_center())
        )

        d_group = VGroup(d_label, arrow, d_lines, lil_brace)

        self.play(FadeOut(d_group, time_span=(0, 1)), film_dot.animate.set_y(1), run_time=3)
        ...
        # Move film point
        self.play(film_dot.animate.match_y(randy.eyes), run_time=2)
        self.wait(6)
        self.play(randy.animate.move_to(2.4 * LEFT), run_time=2)
        ...
        conj_lines = VGroup(Line(point, obj_dot.get_center()) for point in mid_line_points.get_points())
    ...
    def get_ghost_lines(self, point_cloud, obj_dot, dash_length=0.15, stroke_color=WHITE, stroke_width=1):
        lines = VGroup(
            DashedLine(point, obj_dot.get_center(), dash_length=dash_length) for point in point_cloud.get_points()
        )
    ...
    def get_triple_beam(self, film_point, obj_point, **kwargs):
        ...
        beams = Group(self.get_beam(angle=angle, **kwargs) for angle in [-theta, 0, theta])
...
class SuperpositionOfPoints(InteractiveScene):
    def construct(self):
        ...
        for point_set in point_sets:
            self.play(dot_cloud.animate.set_points([*point_set, dist_point]), run_time=3)
            self.wait(2)
        ...
        plate.set_max_amp(4)
        self.play(frame.animate.reorient(61, -7, 0, (0.61, -0.11, -2.44), 8.66), run_time=5)
        ...
        self.play(
            UpdateFromAlphaFunc(
                dot_cloud,
                lambda m, a: m.set_points([*all_points[: int(2 + a * 29)], dist_point]),
            ),
        ...
        beam = VGroup(Line(25 * OUT, rect.pfp(a)) for a in np.linspace(0, 1, 500))
        ...
        self.play(
            UpdateFromAlphaFunc(
                dot_cloud,
                lambda m, a: m.set_points([*all_points[: int(1 + a * 29)], dist_point]),
            ),
            UpdateFromFunc(plate, lambda m: m.set_max_amp(2 * np.sqrt(dot_cloud.get_num_points()))),
        ...
        self.play(
            UpdateFromAlphaFunc(
                dot_cloud,
                lambda m, a: m.set_points([*all_points[: int(interpolate(31, 500, a))], dist_point])
                .set_glow_factor(interpolate(1, 0, a**0.25))
                .set_radius(interpolate(0.1, 0.02, a**0.25))
                .set_opacity(interpolate(1, 0.5, a**0.25)),
            ),
            run_time=4,
        )
        self.play(
            UpdateFromAlphaFunc(
                dot_cloud,
                lambda m, a: m.set_points([*all_points[: int(interpolate(500, len(all_points), a**3))]])
                .set_radius(interpolate(0.02, 0.01, a))
                .set_opacity(interpolate(0.5, 0.2, a)),
            ),
            run_time=5,
        )
        ...
        self.play(
            ...
            UpdateFromFunc(
                sheet_dots,
                lambda m: self.color_sheet_by_exposure(m, dot_cloud.get_points()[:1000], wave_number=32),
            ),
            run_time=3,
        )
        self.wait(3)
...
class ComplexWaves(InteractiveScene):
    def construct(self):
        ...
        R_arrow.add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), plane.n2p(get_R())))
        O_arrow.add_updater(
            lambda m: m.put_start_and_end_on(
                plane.n2p(get_R()),
                plane.n2p(get_R() + get_O()),
            )
        )
        comb_arrow.add_updater(
            lambda m: m.put_start_and_end_on(
                plane.get_origin(),
                plane.n2p(get_R() + get_O()),
            )
        ...
        self.play(
            ...
            O_phase_tracker.animate.set_value(-45 * DEGREES),
            run_time=2,
        )
        self.wait()
        self.play(
    ...
    def get_arrow_label(self, arrow, symbol, font_size=24, buff=0.25):
        ...
        result.add_updater(
            lambda m: m.move_to(arrow.get_center() + buff * normalize(rotate_vector(arrow.get_vector(), PI / 2)))
        )
        return result
...
class StateOnA2DScreen(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(wave.animate.match_points(wave2d).set_opacity(1).shift(1e-2 * IN) for wave in wave3d),
                lag_ratio=0,
            ),
