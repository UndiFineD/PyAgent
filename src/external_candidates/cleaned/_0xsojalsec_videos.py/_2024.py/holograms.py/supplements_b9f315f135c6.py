    ...
    def construct(self):
        ...
        def isolate(rects, titles, index, faded_opacity=0.15):
            return AnimationGroup(
                *(title.animate.set_opacity(1 if i == index else faded_opacity) for i, title in enumerate(titles)),
                *(rect.animate.set_opacity(1 if i == index else faded_opacity) for i, rect in enumerate(rects)),
            )
        ...
        self.play(
            LaggedStart(
                (mid_rect.animate.replace(top_rects[1]).set_opacity(0) for mid_rect in mid_rects),
                lag_ratio=0.05,
                group_type=Group,
            ),
            FadeOut(mid_titles),
...
class AskWhyForSinusoidalGratingFact(TeacherStudentsScene):
    def construct(self):
        ...
        implication = VGroup(Tex(R"\Leftarrow").scale(2), Text("More formal\nholography explanation"))
