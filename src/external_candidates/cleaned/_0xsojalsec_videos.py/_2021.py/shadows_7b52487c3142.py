# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\shadows.py
...
def get_pre_shadow(mobject, opacity):
    ...
    for sm in result.family_members_with_points():
        ...
        if isinstance(sm, VMobject):
            sm.set_stroke(interpolate_color(sm.get_stroke_color(), BLACK, opacity))
...
def get_area(shadow):
    return 0.5 * sum(get_norm(sm.get_area_vector()) for sm in shadow.get_family())
...
def cube_sdf(point, cube):
    ...
    return max(*(abs(np.dot(fv, vect) / np.dot(fv, fv)) for fv in face_vects)) - 1
...
class ShadowScene(ThreeDScene):
    ...
    def get_shadow_area_label(self):
        ...
        decimal.add_updater(lambda d: d.set_value(get_area(self.shadow) / (self.unit_size**2)).set_backstroke())
        return label
    ...
    def init_frame_rotation(self, factor=0.0025, max_speed=0.01):
        ...
        def update_frame(frame, dt):
            ...
            frame.increment_theta(clip(factor * frame.d_theta, -max_speed * dt, max_speed * dt))
...
class BobWords(SimpleWriting):
    ...
    def construct(self):
        words = VGroup(*(Text(text, font=self.font, font_size=self.font_size) for text in (self.words1, self.words2)))
        words.arrange(DOWN)
...
class IntroduceShadow(ShadowScene):
    ...
    def construct(self):
        ...
        self.play(
            FadeIn(question, UP),
            *(LaggedStartMap(DrawBorderThenFill, mob, lag_ratio=0.1, run_time=3) for mob in (cube, shadow)),
        )
        ...
        self.play(light.animate.shift(1.0 * IN), rate_func=there_and_back, run_time=3)
        ...
        vert_proj_lines = VGroup(*(DashedLine(*pair) for pair in zip(verts, proj_dots.get_points())))
        ...
        self.play(
            LaggedStart(
                *map(
                    FadeOut,
                    (vert_dots, vert_proj_lines, proj_dots, xyz_label, xy0_label),
                )
            )
        )
        ...
        self.play(Uncreate(diagonal), FadeOut(hex_area_label), Rotate(cube, 4, RIGHT))
        ...
        for x in range(9):
            ...
            sample.fix_in_frame()
            self.play(sample.animate.next_to(samples, DOWN), run_time=0.5)
            samples.add(sample)
        ...
        text = OldTexText("Take the mean.", "\\\\What does that\\\\approach?", font_size=30)
    ...
    def construct(self):
        ...
        self.play(
            PiCreatureBubbleIntroduction(
                sts[2],
                OldTexText("What does that\\\\mean, exactly?"),
...
class MeanCalculation(Scene):
    def construct(self):
        ...
        nums = VGroup(*(DecimalNumber(x) for x in values))
...
class PauseAndPonder(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[self.change_students("tease", "well", "pondering", look_at=self.screen)],
        )
        self.wait(2)
        self.play(
        ...
        self.teacher_says(
            "Pause and ponder!",
            target_mode="well",
            added_anims=[self.change_students("pondering", "tease", "thinking")],
            run_time=1,
        )
        self.wait(5)
...
class StartSimple(Scene):
    def construct(self):
        ...
        words = Text("Start with the simplest non-trivial\nvariant of the problem you can.")
        ...
        self.wait()
        self.play(FadeIn(rect), FadeIn(words, lag_ratio=0.1), run_time=2)
        self.wait()
...
class FocusOnOneFace(ShadowScene):
    ...
    def construct(self):
        ...
        theta.add_updater(lambda m: m.move_to(get_fc() + 1.3 * (arc.pfp(0.5) - get_fc())))
        ...
        for sm in slices.target:
            sm.stretch(0.5, 1)
        self.play(MoveToTarget(slices), FadeOut(slice_outlines), run_time=2)
        self.wait(2)
        ...
        self.play(top_theta_group.animate.move_to(line.get_start(), LEFT + IN))

        elbow = Elbow(angle=-get_theta())
        elbow.set_stroke(WHITE, 2)
        ul_arc = Arc(radius=0.4, start_angle=-get_theta(), angle=-(PI / 2 - get_theta()))
        ...
        lower_formula = OldTex("\\text{shadow}", "=", "\\cos(\\theta)", "\\cdot", "\\text{slice}")
        ...
        self.play(Rotate(face, PI / 2 - get_theta(), UP), run_time=2)
        ...
        self.play(final_formula.animate.next_to(axes, DOWN))
        self.wait()
...
class DiscussLinearity(Scene):
    def construct(self):
        ...
        arrows = VGroup(
            *(
                Arrow(p1.get_top(), p2.get_top(), path_arc=-0.6 * PI).scale(0.75, about_edge=DOWN)
        ...
        for arrow, label in zip(arrows, arrow_labels):
            self.play(ShowCreation(arrow), FadeIn(label, lag_ratio=0.1))
            self.wait()

        # Linear!
        lin_text = Text("Both are linear transformations!", t2c={"linear": YELLOW})
        ...
        self.play(
            ...
            LaggedStart(FadeOut(lin_text, lag_ratio=0.1), *map(FadeOut, uniform_words)),
        )
        ...
        self.play(*map(Write, area_labels), run_time=1)
        self.play(
        ...
        self.play(FadeTransform(area_exprs[1][0], f_rot), MoveToTarget(times_A))
        ...
        self.wait()
        self.play(LaggedStart(*map(FadeOut, (*det, rot_brace, details)), lag_ratio=0.3, run_time=2))
...
class AmbientShapeRotationPreimage(ShadowScene):
    ...
    def construct(self):
        ...
        if display_mode == "full_3d":
            ...
            orientation_arrows.add_updater(
                lambda m: m.restore().rotate(
            ...
            proj_lines = always_redraw(
                lambda: VGroup(
                    *(
                        Line(rotated.pfp(a), flat_project(rotated.pfp(a))).set_stroke(WHITE, 0.5, 0.2)
        ...
        for axis, diag in zip((0, 1, 0, 1), (False, False, True, True)):
            ...
            if diag:
                preimage.target.rotate(-PI / 4)
            self.play(MoveToTarget(preimage), rate_func=there_and_back, run_time=4)
        self.wait(5)
        ...
        self.play(rotated.rot_speed_tracker.animate.set_value(0))
        anims = [light.animate.move_to(4 * OUT)]
        ...
        self.play(rotated.angle_tracker.animate.set_value(70 * DEGREES + TAU), run_time=2)
        self.play(
...
class StretchLabel(Scene):
    def construct(self):
        label = VGroup(Vector(0.5 * LEFT), OldTex("1.5 \\times"), Vector(0.5 * RIGHT))
...
class WonderAboutAverage(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureBubbleIntroduction(
                randy,
                OldTexText("How do you think\\\\about this average"),
...
class AverageShadowAnnotation(Scene):
    def construct(self):
        ...
        formula = VGroup(
            ...
            OldTex("=").rotate(PI / 2),
            OldTexText("$c$", " $\\cdot$", f"(Area({shape_name}))", tex_to_color_map=t2c),
        )
        ...
        self.wait()
        self.play(Write(formula[1]), FadeIn(formula[2], DOWN))
        self.wait()
...
class AlicesFaceAverage(Scene):
    def construct(self):
        ...
        # Label the rotations
        indices = ["1", "2", "3", "n"]
        rot_labels = VGroup(*(OldTex(f"R_{i}") for i in indices))
        ...
        self.wait(2)
        self.play(LaggedStartMap(FadeIn, rot_labels, shift=0.25 * DOWN, lag_ratio=0.5))
        self.wait()
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(area, area_s) for area, area_s in zip(area_shadow_labels, s_labels)),
                lag_ratio=lr,
            ),
        ...
        self.play(TransformMatchingShapes(fra_labels.copy(), sample_average[4:-1]))
        self.wait()
        self.play(Write(VGroup(*sample_average[2:4], sample_average[-1])))
        self.wait()
        ...
        new_pieces = VGroup(*(sm for sm in sample_average.target if sm.get_tex() not in ["A", "\\cdot"]))
        new_A = As[0].copy()
        ...
        self.play(
            LaggedStart(
                *(FlashAround(mob, time_width=3) for mob in sample_average.get_parts_by_tex("A")),
                lag_ratio=0.1,
                run_time=2,
            )
        )
        ...
        self.play(FlashAround(sample_average[2:], run_time=3, time_width=1.5))
        self.play(FlashUnder(sample_average[:2], color=RED), run_time=2)
        self.play(GrowFromCenter(brace), FadeIn(lim, 0.25 * DOWN), ShowCreation(cross))
        self.play(
            LaggedStart(
                *map(
                    FadeOut,
        ...
        self.play(ShowCreation(rect), FadeIn(words, 0.25 * DOWN))
        self.wait()
...
class ManyShadows(SingleFaceRandomRotation):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeIn(plane, scale=1.1) for plane in planes),
                lag_ratio=0.6,
                run_time=10,
            )
        )
    ...
    def construct(self):
        ...
        sphere_points = np.array(
            [
                ...
                for phi in np.linspace(0, TAU, int(2 * n_lat_lines * math.sin(theta)) + 1)
            ]
        )
        sphere_points[:, 2] *= -1
        ...
        sphere_lines = VGroup(*(Line(sphere.get_center(), p) for p in sphere_dots.get_points()))
        ...
        for x in range(20):
            ...
            self.add(dot)
            self.play(
                face.animate.apply_matrix(
                    rotation_between_vectors(normal_vect.get_vector(), point - fc),
                    about_point=fc,
                ),
        ...
        frame.reorient(-5)
        self.play(
            Rotate(face, angle_between_vectors(v1, v2), axis=normalize(cross(v1, v2))),
            UpdateFromAlphaFunc(self.plane, lambda m, a: square.scale(0.9).set_opacity(0.5 - a * 0.5)),
        )
        self.play(
        ...
        prob = Group(*Tex("P(", "0.", ")", "=", "{Num ", "\\over ", "Den}", font_size=60))
        prob.fix_in_frame()
        prob.to_corner(UR)
        prob.shift(DOWN)
        ...
        self.wait()
        for i in (4, 6):
            self.play(ShowCreationThenFadeOut(SurroundingRectangle(prob[i], stroke_width=2).fix_in_frame()))
            self.wait()
        ...
        self.play(GrowFromCenter(sum_brace), FadeIn(continuum, 0.5 * DOWN))
        self.wait(4)
        self.play(
        ...
        self.play(FadeTransformPieces(loose_sum[1:4], integral[1:4]), Write(integral[4]))
        self.wait(5)
        face.clear_updaters()
        self.wait(5)
        ...
        def set_theta(face, theta):
            face.apply_matrix(rotation_between_vectors(normal_vect.get_vector(), OUT), about_point=fc)
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                face,
                lambda f, a: set_theta(face, theta_samples[int(a * (len(theta_samples) - 2))]),
            ),
            run_time=4,
        )
        self.add(tick_marks)
        ...
        dt_brace = Brace(Line(ORIGIN, RIGHT), UP)
        dt_brace.scale(0.5)
        ...
        def get_band(index):
            band = Sphere(
                u_range=(0, TAU),
                v_range=theta_samples[index : index + 2],
        ...
        radial_line = Line([0, 0, normal_vect.get_end()[2]], normal_vect.get_end())
        ...
        brace_text = brace.get_text("What does this approach for finer subdivisions?", font_size=30)
        ...
        for n in range(1, 4):
            new_ticks = get_tick_marks(np.linspace(0, PI, sphere_mesh.resolution[0] * 2**n), tl=0.05 / n)
            self.play(
        ...
        new_lines = VGroup(
            ...
            OldTex(
                "{1 \\over 2} s^2 \\cdot \\left(-\\left(-\\frac{1}{2}\\right) - \\left(-\\frac{1}{2}\\right)\\right)"
            ),
            OldTex("{1 \\over 2} s^2"),
        )
...
class AskAboutAverageCosValue(AllPossibleOrientations):
    def construct(self):
        ...
        theta.rotate(PI / 2, RIGHT)
        theta.set_backstroke(width=2)
        theta.add_updater(lambda m: m.next_to(arc.pfp(0.5), arc.pfp(0.5) - fc, buff=0.05))
...
class ThreeCamps(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(student.change("pondering", image.target) for student in students),
                run_time=2,
                lag_ratio=0.2,
            ),
        ...
        self.play(
            LaggedStart(*(FadeOut(VGroup(student.bubble, student.bubble.content)) for student in reversed(students))),
            LaggedStart(*(student.change("pondering", integral) for student in students)),
            FadeIn(integral, UP),
...
class IntegralOverlay(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FlashAround(sm, time_width=3) for sm in integral[0][:0:-1]),
                lag_ratio=0.5,
                run_time=3,
            )
        )
...
class AlicesInsights(Scene):
    def construct(self):
        ...
        self.play(LaggedStart(*(FadeIn(insight[:2], 0.25 * DOWN) for insight in insights)))
        self.wait()
...
class TwoToOneCover(ShadowScene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(wireframe, lag_ratio=0.1), Write(lhs[2:-1]))
        ...
        self.play(
            ...
            FadeIn(lhs[-1]),
            Write(outline),
            VShowPassingFlash(outline.copy().set_stroke(YELLOW, 4), time_width=1.5),
        )
        self.wait()
        ...
        face_shadow_lines = VGroup(
            *(
                VGroup(*(Line(v1, v2) for v1, v2 in zip(f.get_vertices(), fs.get_vertices())))
                for f, fs in zip(faces, face_shadows)
            )
        )
        face_shadow_lines.set_stroke(YELLOW, 0.5, 0.5)

        self.play(Restore(lhs), FadeIn(af_label, shift=0.5 * RIGHT))
        self.play(
            *(
                LaggedStart(
                    *(VFadeInThenOut(sm) for sm in reversed(mobject)),
                    lag_ratio=0.5,
                    run_time=6,
                )
        ...
        cube.clear_updaters()
        self.play(MoveToTarget(cube), ApplyMethod(frame.reorient, 0, run_time=2))
        self.play(
            FadeOut(lt, UP),
            Write(eq_half),
        ...
        for part in z_sorted_faces:
            ...
            lines = VGroup(*(Line(v, flat_project(v)) for v in pc.get_vertices()))
        ...
        line = DashedLine(shadow_point + 5 * OUT, shadow_point, dash_length=0.025)
        ...
        def update_line(line):
            ...
            for dash in line:
                ...
                dash.set_stroke(opacity=interpolate(0.1, 1.0, clip(10 * dist, -0.5, 0.5) + 0.5))
                dash.inside = dist < 0
        ...
        arrows = VGroup(*(Arrow(point + RIGHT, point, buff=0.1) for point in (entry_point, exit_point)))
        ...
        outline = self.get_shadow_outline()
        outline.rotate(PI)
        self.play(MoveToTarget(cube), dot.animate.move_to(outline.get_start()))
        ...
        self.remove(cube)
        self.play(
            *(f.animate.set_fill(opacity=0) for f in cube),
            FadeOut(outline),
            FadeOut(area_label),
            Write(face),
            FadeIn(face_shadow),
        ...
        def isolate_face_anims(i, color=YELLOW):
            return (
                shadow.animate.set_fill(interpolate_color(color, BLACK, 0.75)),
                *(
                    f.animate.set_fill(
        ...
        cube.update()
        self.play(
            *(f.animate.set_style(**cube_style) for f in cube),
...
class ConvexityPrelude(Scene):
    def construct(self):
        ...
        words.to_edge(LEFT)
        arrows = VGroup(*(Arrow(words[0].get_right(), point) for point in points))
...
class DefineConvexity(Scene):
    def construct(self):
        ...
        line.target.put_start_and_end_on(*(pi.get_boundary_point(v) for v in (DL, DR)))
        ...
        self.play(FadeIn(not_convex_label, scale=2))
        self.wait()
        shape.insert_n_curves(80)
        self.play(
            Transform(shape, letter_c),
            line.animate.put_start_and_end_on(*(letter_c.pfp(a) for a in (0, 0.5))),
            run_time=2,
        )
        self.play(
            UpdateFromAlphaFunc(
                line,
                lambda l, a: l.put_start_and_end_on(letter_c.pfp(0.4 * a), l.get_end()),
                run_time=6,
            )
        )
        self.wait()
        ...
        polygon.target.set_points_as_corners([new_tip, *polygon.get_vertices()[1:], new_tip])
        self.play(
...
class ShowGridSum(TwoToOneCover):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(ReplacementTransform(lhs.alt2.copy(), target) for target in lhss)),
        ...
        self.play(
            FadeIn(corner_rect),
            *(FadeTransform(term.copy(), sum_part[1:]) for term in lhss),
            Restore(sigma),
        )
        ...
        self.play(
            ...
            LaggedStart(*(FadeTransform(summand.alt2.copy(), part) for part in grid[0][0:7:2])),
        ...
        cols = VGroup(*(VGroup(*(row[i] for row in grid)).copy() for i in [0, 2, 6]))
        col_rects = VGroup(*(SurroundingRectangle(col, buff=SMALL_BUFF) for col in cols))
        ...
        arrows = VGroup(*(Arrow(rect.get_bottom(), mean_face) for rect in col_rects))
        ...
        self.add(cols[0])
        self.play(grid.animate.set_opacity(0.4), ShowCreation(col_rects[0]))
        self.wait()
        self.play(
        ...
        for i in [1, 2]:
            self.play(
                FadeOut(cols[i - 1]),
                FadeIn(cols[i]),
                *(ReplacementTransform(group[i - 1], group[i]) for group in (col_rects, arrows, arrow_labels)),
            )
            self.wait()
        self.wait()
        ...
        self.play(
            LaggedStart(
                VGroup(corner_rect, mean_sa).animate.scale(1.25).move_to(frame.target, UL).shift(0.1 * DR),
                MoveToTarget(frame),
                FadeOut(mean_face_words),
                FadeOut(mean_face),
                grid.animate.set_opacity(1),
                *(FadeOut(group[-1]) for group in (cols, col_rects, arrows, arrow_labels)),
                run_time=3,
            )
        )
        ...
        self.play(MoveToTarget(corner_rect), Write(rhss[0]))
        ...
        self.wait()
        self.play(LaggedStart(*map(FadeOut, (*rects, rows_first, cols_first))))

        self.play(TransformMatchingShapes(rhss[0].copy(), rhss[1]))
        self.wait()
        self.play(
...
class LimitBrace(Scene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(tex, shift=0.25 * UP))
        self.wait()
...
class FromRowsToColumns(Scene):
    def construct(self):
        ...
        buff = 0.2
        row_rects = VGroup(*(SurroundingRectangle(grids[0][k : k + n], buff=buff) for k in range(0, n * n, n)))
        col_rects = VGroup(*(SurroundingRectangle(grids[1][k::n], buff=buff) for k in range(n)))
        ...
        self.wait()
        self.play(TransformFromCopy(grids[0], grids[1]), ShowCreation(arrow))
...
class SupposedlyObviousProportionality(ShadowScene):
    ...
    def construct(self):
        ...
        # Ask about constant
        question = OldTexText("What is $c$?!", font_size=72, tex_to_color_map={"$c$": RED})
        question.fix_in_frame()
        ...
        self.play(ShowCreation(c_arrow), Write(question))
        self.wait(4)
        ...
        obvious_arrow = Arrow(obvious_words, equation.get_corner(DL) + RIGHT)
        ...
        for face in cube[3:]:
            ...
            lc.apply_matrix(rotation_between_vectors(OUT, normal))
            lc.move_to(face)
            face.label = lc
            face_labels.add(lc)
        ...
        self.play(
            *(mob.animate.scale(0.5) for mob in scalars),
            rate_func=there_and_back,
            run_time=3,
        )
        self.play(
            *(mob.animate.stretch(2, 0) for mob in scalars),
            rate_func=there_and_back,
            run_time=3,
        )
        self.wait()
...
class ShowSeveralConvexShapes(Scene):
    def construct(self):
        ...
        blob.apply_function(lambda p: [*(2 - p[2]) * p[:2], p[2]])
        blob.set_color(BLUE_E)
        ...
        examples = Group(*(Group(*mob) for mob in (dodec, spike, blob, cylinder)))
        ...
        self.play(
            LaggedStart(
                *(
                    (LaggedStartMap(Write, ex, run_time=1) if isinstance(ex[0], VMobject) else ShowCreation(ex[0]))
                    for ex in examples
                ),
                lag_ratio=0.5,
                run_time=8,
...
class KeyResult(Scene):
    def construct(self):
        eq1, eq2 = [get_key_result(word) for word in ("Cube", "Solid")]
        ...
        const_rect = SurroundingRectangle(
            VGroup(eq.get_part_by_tex("frac"), eq.get_part_by_tex("{c}")),
            buff=SMALL_BUFF,
        )
...
class ShadowsOfDodecahedron(ShadowScene):
    ...
    def construct(self):
        ...
        def dodec_sdf(point):
            return max(*(np.dot(point - pent.get_center(), pent.get_unit_normal()) for pent in dodec))
...
class AmbientDodecahedronShadow(ShadowsOfDodecahedron):
    ...
    def construct(self):
        ...
        self.play(*map(VFadeIn, (outline, area_label, sa_label)))
        self.add(outline)
        self.add(area_label)
...
class SphereShadow(ShadowScene):
    ...
    def construct(self):
        ...
        self.begin_ambient_rotation(sphere, speed=0.3, initial_axis=[-1, -1, 0.5])
        self.wait(60)
...
class SphereInfo(Scene):
    def construct(self):
        kw = {"tex_to_color_map": {"{c}": RED, "=": WHITE, "R": BLUE}, "font_size": 36}
...
class SwapConstantForFourth(Scene):
    def construct(self):
        ...
        eq.to_edge(UP)
        parts = VGroup(eq.get_part_by_tex("frac"), eq.get_part_by_tex("{c}"))
...
class RepeatedRelation(Scene):
    def construct(self):
        ...
        for repeat in repeats:
            ...
            self.play(Write(repeat[1]), FadeIn(repeat[2], 0.5 * DOWN))
        self.wait()
...
class PopularizaitonVsDoing(Scene):
    def construct(self):
        ...
        self.play(bar_group.animate.shift(doing.get_center() - popular.get_center()))
...
class MultipleMathematicalBackgrounds(TeacherStudentsScene):
    def construct(self):
        ...
        lines = VGroup(*(DashedLine(words, label, buff=0.5) for label in labels))
...
class CleverProofExample(Scene):
    def construct(self):
        ...
        self.play(initial_sum.animate.set_width(8).to_edge(UP), FadeIn(tri, lag_ratio=0.1))
        self.wait()
        self.play(Restore(tri), FadeIn(tripple_tris[1:]))
        for i in (0, 1):
            bt1 = tris[i].copy()
            bt1.generate_target()
            ...
            self.play(MoveToTarget(bt1, remover=True), Restore(tris[i + 1]))
        self.wait()
...
class FamousMathematicians(Scene):
    ...
    def construct(self):
        ...
        self.play(LaggedStart(*map(FadeIn, (*im_groups, dots)), lag_ratio=0.25), run_time=5)
        self.wait()

        self.play(
            im_groups[0].animate.set_height(6).center().to_edge(LEFT),
            LaggedStart(*(FadeOut(mob, DR) for mob in (*im_groups[1:], dots)), lag_ratio=0.25),
            run_time=2,
        )
        self.wait()
...
class QuantifyConvexity(Scene):
    def construct(self):
        ...
        frac = VGroup(four_shadow, Line().match_width(four_shadow).set_stroke(width=2), sa)
        frac.arrange(DOWN)
        frac.set_width(3)
        frac.to_corner(UR)
