...
class RasterizingBezier(Scene):
    def construct(self):
        ...
        dist_line.add_updater(
            lambda l: l.put_start_and_end_on(pixel.get_center(), curve_copy.pfp(t_tracker.get_value()))
        )
...
class ShowManyGraphs(Scene):
    def construct(self):
        ...
        plots = Group(
            *(self.get_plot(coefs, scalar, color) for coefs, scalar, color in zip(coef_groups, scalars, colors))
        )
...
class RealNewtonsMethod(Scene):
    ...
    def highlight_roots(self):
        ...
        self.play(
            LaggedStart(
                *[FadeIn(dot, scale=0.1) for dot in dots]
                + [VShowPassingFlash(square, time_width=2.0, run_time=2) for square in squares],
                lag_ratio=0.15,
            ),
        )
        self.wait()
        ...
        equations = VGroup(
            *(
                OldTex("P(", root_str, ")", "=", "0", font_size=self.rule_font_size).set_color_by_tex(root_str, YELLOW)
...
class ComplexNewtonsMethodManySeeds(ComplexNewtonsMethod):
    ...
    def add_z0_def(self):
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(z0_def[0].copy(), guess_dot) for guess_dot in guess_dots),
    ...
    def add_true_root_circles(self):
        ...
        root_circles = VGroup(
            *(Dot(radius=0.1).set_fill(color, opacity=0.75).move_to(rp) for rp, color in zip(root_points, colors))
        )
    ...
    def get_root_dots(self, plane, fractal):
        self.root_dots = VGroup(
            *(
                ...
                for root, color in zip(coefficients_to_roots(fractal.coefs), fractal.colors)
            )
        )
...
class WhatsGoingOn(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            ...
            added_anims=[
                LaggedStart(
                    *(pi.change("guilty", self.students[2].eyes) for pi in [self.teacher, *self.students[:2]]),
                    run_time=2,
                )
            ],
        )
...
class RepeatedNewton(Scene):
    ...
    def add_true_roots(self):
        ...
        root_dots = self.root_dots = VGroup(
            *(glow_dot(self.plane.n2p(root), color=color, opacity_mult=2.0) for root, color in zip(roots, self.colors))
        )
...
class DefineBoundary(Scene):
    def construct(self):
        ...
        self.play(
            ApplyMethod(group.scale, 2, {"about_point": point.get_center()}, run_time=4),
            ApplyMethod(
                circle.set_height,
                0.5,
                run_time=2,
...
class InterpretBoundaryProperty(RepeatedNewton):
    ...
    def construct(self):
        ...
        def get_arrows():
            ...
            return VGroup(
                *(Arrow(dots, root_dot, buff=0.1, stroke_color=root_dot[0].get_color()) for root_dot in root_dots)
            )
