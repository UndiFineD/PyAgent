# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\matrix_exp.py
...
class ZoomInOnProblem(Scene):
    def construct(self):
        ...
        # Write problem
        problem = OldTexText("Compute the {{matrix}} {{$e^{At}$}}\\\\if the {{matrix A}} has the form...")
        ...
        self.play(Transform(full_rect, full_rect2))
        self.play(
...
class IntroduceTheComputation(Scene):
    def construct(self):
        ...
        brace_label = VGroup(matrix.copy().scale(0.5), Text("times?"))
        ...
        false_equation = VGroup(matrix_exp, rhs, brace, brace_label)
        ...
        self.play(
            ...
            LaggedStart(*(FadeIn(part) for part in real_equation[4:] if part not in xs)),
        ...
        rects = VGroup(*(Rectangle(height=2**n / math.factorial(n), width=1) for n in range(11)))
        ...
        threes = VGroup(*(OldTex("3").set_color(YELLOW).replace(two) for two in twos))
        ...
        crosses = VGroup(*(Cross(lhs[:3], stroke_width=[0, 3, 3, 3, 0]).scale(1.3) for lhs in [pii_lhs, mat_lhs]))
        bases = VGroup()
        powers = VGroup()
        equals = VGroup()
        ...
        sum_eq = VGroup(
            ...
            OldTex("="),
            Matrix(
                np.array([f"{m1} + {m2}" for m1, m2 in zip(mat1.flatten(), mat2.flatten())]).reshape((3, 3)),
                h_buff=1.8,
            ),
        )
        ...
        self.play(morty.change("raise_right_hand"), FadeIn(pi_mat, UP))
        self.play(Blink(morty))
        self.play(
        ...
        self.play(Blink(morty))
        self.play(Write(later_text, run_time=1), randy.change("hesitant", morty.eyes))
...
class ShowHigherMatrixPowers(IntroduceTheComputation):
    ...
    def construct(self):
        ...
        def get_mat_mob(matrix):
            ...
            return IntegerMatrix(matrix, h_buff=max(0.8 + 0.3 * len(term), 1.0))
...
class DefinitionFirstVsLast(Scene):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(top_rect), FadeIn(top_title, 0.25 * UP))
        self.play(
            FadeOut(top_dots),
            FadeIn(top_prog[1]),
        )
        ...
        self.wait()
        self.play(Restore(love), FadeIn(quantum, 0.5 * LEFT))
        self.wait()
        self.play(
...
class RomeoAndJuliet(Scene):
    def construct(self):
        ...
        for lover, scale, arrow, label, final_love in zip(
            reversed(lovers), scales, scale_arrows, scale_labels, [1, -1]
        ):
            self.add(scale)
    ...
    def get_romeo_juilet_name_labels(self, lovers, font_size=36, spacing=1.2, buff=MED_SMALL_BUFF):
        name_labels = VGroup(*(Text(name, font_size=font_size) for name in ["Romeo", "Juliet"]))
    ...
    def make_character_dynamic(self, pi_creature, lover, cutoff_values, modes):
        ...
        copies = [pi_creature.deepcopy().change(mode).set_height(height).move_to(bottom, DOWN) for mode in modes]
...
class DiscussSystem(Scene):
    def construct(self):
        ...
        self.play(FadeIn(ic_label), LaggedStartMap(GrowArrow, ic_arrows, run_time=1))
        self.wait()
...
class MoreGeneralSystem(Scene):
    def construct(self):
        ...
        equations = VGroup(
            OldTex(
                "{dx \\over d{t} } = a\\cdot x({t}) + b\\cdot y({t}) + c\\cdot z({t})",
                **kw,
            ),
            OldTex(
                "{dy \\over d{t} } = d\\cdot x({t}) + e\\cdot y({t}) + f\\cdot z({t})",
                **kw,
            ),
            OldTex(
                "{dz \\over d{t} } = g\\cdot x({t}) + h\\cdot y({t}) + i\\cdot z({t})",
                **kw,
            ),
        )
...
class HowExampleLeadsToMatrixExponents(Scene):
    def construct(self):
        ...
        # Mat exp
        mat_exp = get_matrix_exponential([["a", "b"], ["c", "d"]], height=2, h_buff=0.95, v_buff=0.75)
        ...
        self.play(GrowArrow(arrow), FadeIn(mat_exp, RIGHT))
        self.wait()
...
class RomeoJulietVectorSpace(RomeoAndJuliet):
    def construct(self):
        ...
        deriv_vect.add_updater(
            lambda m: m.put_start_and_end_on(
                axes.get_origin(), axes.c2p(-0.5 * get_psp()[1], 0.5 * get_psp()[0])
            ).shift(ps_dot.get_center() - axes.get_origin())
        )
        ...
        # Show equation
        rhs = VGroup(OldTex("="), Matrix([["-y(t)"], ["x(t)"]], bracket_h_buff=SMALL_BUFF))
...
class From2DTo1D(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(rhs_rect), LaggedStartMap(GrowArrow, vector_field, lag_ratio=0))
        self.wait(14)
        ...
        if self.show_solution:
            ...
            arrow1 = Arrow(
                equation.target.get_corner(UR),
                mat_exp.get_corner(UL),
                path_arc=-45 * DEGREES,
            )
            arrow2 = Arrow(
                mat_exp.get_corner(DL),
                equation.target.get_corner(DR),
                path_arc=-45 * DEGREES,
            )
        ...
        self.play(MoveToTarget(number_line))
        self.wait(11)
...
class SchroedingersEquationIntro(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(randy.animate.set_opacity(1).change("horrified", original_equation))
        self.play(Blink(randy))
        self.play(
...
class SimpleDerivativeOfExp(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            *(pi.animate.look_at(eq[4]) for pi in self.pi_creatures),
        )
        self.play(FadeTransform(eq[4][1].copy(), eq[3][0], path_arc=90 * DEGREES))
        self.play(
            LaggedStart(
                s0.change("thinking"),
                s1.change("tease"),
...
class GraphAndHistoryOfExponential(Scene):
    def construct(self):
        ...
        curr_time = self.time
        exp_graph.add_updater(
            lambda m: m.pointwise_become_partial(
                graph_template,
                0,
                (self.time - curr_time) / 20,
            )
        )
        ...
        words.add_updater(
            lambda m: m.become(original_words).set_fill(opacity=fill_tracker.get_value()).apply_function(func)
        )
        ...
        for sec_dot in dot1, dot2:
            ...
            sec_dot.add_updater(lambda d: d.move_to(axes.input_to_graph_point(d.x_tracker.get_value(), exp_graph)))
        line = Line(LEFT, RIGHT, stroke_color=GREY_A, stroke_width=2)
        line.add_updater(lambda l: l.put_start_and_end_on(dot1.get_center(), dot2.get_center()).set_length(10))
        ...
        self.play(dot2.x_tracker.animate.set_value(15 + 1e-6), run_time=3)
        self.play(
        ...
        self.play(
            TransformFromCopy(
                solution_with_number[12],
                solution_with_number[7],
                path_arc=-90 * DEGREES,
            )
        )
        self.wait()
        ...
        for name1, name2, letter, year in [
            ("Leibniz", "Huygens", "b", "1690"),
            ("Euler", "Goldbach", "e", "1731"),
        ]:
...
class BernoullisThoughts(Scene):
    def construct(self):
        ...
        continuous_money = VGroup(
            *(dollar.copy().shift(smooth(x) * RIGHT * 2.0 + np.exp(smooth(x) - 1) * UP) for x in np.linspace(0, 1, 300))
        )
        ...
        self.play(ShowCreation(bubble), Write(dollars))
        self.wait()
        ...
        self.play(FadeOut(continuous_money, lag_ratio=0.01), FadeIn(dollars))
        self.wait()
...
class CompoundInterestPopulationAndEpidemic(Scene):
    def construct(self):
        ...
        points = np.array(list(sorted(points, key=lambda p: get_norm(p) + 0.5 * random.random())))
        ...
        dollars, people, viruses = groups = [
            VGroup(*(mob.copy().move_to(point) for point in points)) for mob in templates
        ]
        ...
        self.play(
            ...
            LaggedStart(
                *(
                    ...
                    for m1, m2 in zip([eq1d[:2], eq1d[2:5], eq1d[5], eq1d[6], eq1d[7:]], eq2d)
                ),
                lag_ratio=0.05,
            ),
        )
    ...
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                randy,
                OldTexText(
                    "This function is\\\\about about $e$",
        ...
        cross.scale(1.25)
        self.play(ShowCreation(cross), randy.change("guilty"))
        self.wait()
...
class ManySolutionsDependingOnInitialCondition(VideoWrapper):
    def construct(self):
        ...
        step = 0.05
        graphs = VGroup(*(get_graph(x0) for x0 in np.arange(step, axes.y_range[1], step)))
        ...
        self.wait()
        for x in [5, 9]:
            self.play(x0_tracker.animate.set_value(x), run_time=5)
            self.wait()
            self.play(
        ...
        self.play(ShowCreation(exp_rect), Write(words1, run_time=1))
        self.wait(2)
        self.play(
            FadeOut(words1),
            FadeIn(words2),
...
class ExoticExponentsWithEBase(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FlashAround(e, buff=0.05, time_width=3, run_time=2) for e in e_mobs),
                lag_ratio=0.1,
            )
        )
        self.wait()
        ...
        self.wait()
        self.play(
            VGroup(labels[0], labels[2:], exps[0], exps[2:], equations[0], equations[2:]).animate.set_opacity(0.3),
...
class SolutionToRomeoJuliet(Scene):
    def construct(self):
        ...
        self.play(rect.animate.become(rect_copy), Write(rot_question))
        self.wait()
...
class ExpRotMatrixComputation(Scene):
    def construct(self):
        ...
        equation = OldTex(
            "e^X",
            ":=",
            "X^0 + X^1 + \\frac{1}{2} X^2 + \\frac{1}{6} X^3 + \\cdots + \\frac{1}{n!} X^n + \\cdots",
            isolate=["X", "+"],
        )
        ...
        for mat_power in mat_powers:
            self.play(
                rect.animate.become(SurroundingRectangle(mat_power.source, buff=0.05)),
                run_time=0.5,
            )
        ...
        final_result = Matrix([["\\cos(t)", "-\\sin(t)"], ["\\sin(t)", "\\cos(t)"]], h_buff=2.0)
        ...
        self.play(FadeIn(question, shift=0.5 * RIGHT), GrowArrow(arrow))
        self.wait()
...
class ThatsHorrifying(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.teacher_says("Just wait", bubble_config={"height": 3, "width": 3.5}, target_mode="tease")
        self.play_student_changes(
            "pondering",
            "thinking",
            "hesitant",
...
class HowBasisVectorMultiplicationPullsOutColumns(Scene):
    def construct(self):
        ...
        def show_basis_product(index, matrix):
            ...
            row_rects = [SurroundingRectangle(row, **rect_kw) for row in matrix.get_rows()]
...
class ReadColumnsOfRotationMatrix(Scene):
    ...
    def construct(self):
        ...
        equation = VGroup(
            ...
            OldTex("="),
            Matrix([["\\cos(t)", "-\\sin(t)"], ["\\sin(t)", "\\cos(t)"]], h_buff=2.0),
        )
        ...
        rects = VGroup(
            *(SurroundingRectangle(entry, color=WHITE, stroke_width=2, buff=0.05) for entry in rbl0.get_entries())
        )
        self.play(
            ShowCreation(h_line),
            FadeIn(rects[0]),
...
class AnalyzeRomeoAndJulietSpace(RomeoJulietVectorSpace):
    def construct(self):
        ...
        self.wait()
        self.play(
            *(TransformFromCopy(solution[i], solution2[i]) for i in [0, 1, 3]),
    ...
    def get_ps_arrow(self, axes, color=BLUE, add_shadow=False):
        arrow = Arrow(LEFT, RIGHT, fill_color=color, thickness=0.04)
        arrow.add_updater(
            lambda m: m.put_start_and_end_on(
                axes.get_origin(),
                axes.ps_dot.get_center(),
            )
        )
...
class GeometricReasoningForRomeoJuliet(AnalyzeRomeoAndJulietSpace):
    def construct(self):
        ...
        self.play(GrowFromCenter(mat_brace), Write(mat_label, run_time=1))
        self.wait()

        # Spiral around to various points
        curve = VMobject()
        curve.set_points_smoothly(
            [axes.c2p(x, y) for x, y in [(4, 3), (0, 2), (0, -1), (-3, -4), (-3, 2), (3, 1)]],
            true_smooth=True,
        )
...
class ComplexEquation(Scene):
    def construct(self):
        ...
        braces = VGroup(*(Brace(equation[i:j], UP, buff=SMALL_BUFF) for i, j in [(2, 8), (11, 17)]))
        ...
        self.play(FadeIn(solution[8:], 0.1 * DOWN))
        self.wait()
...
class JulietChidingRomeo(Scene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                juliet,
                OldTexText("It just seems like \\\\ your feelings aren't \\\\ real"),
...
class SchrodingerSum(Scene):
    def construct(self):
        ...
        self.play(
            ...
            Restore(lil_axes[1]),
            GrowFromCenter(brace),
            *(UpdateFromAlphaFunc(g.fade_tracker, lambda m, a: m.set_value(a)) for g in (*graphs[::2], comb_graph)),
        )
        self.wait(15)
        self.play(FlashAround(i_part, color=RED), i_part.animate.set_color(RED))
        self.wait(30)
...
class BasicVectorFieldIdea(Scene):
    ...
    def construct(self):
        ...
        plane = NumberPlane()
        vector_field = VectorField(func, plane, magnitude_range=(0, 2), vector_config={"thickness": 0.025})
        dots = VGroup(*(Dot(v.get_start(), radius=0.02, fill_color=YELLOW) for v in vector_field))
        ...
        self.wait()
        self.play(FadeOut(dots), FadeOut(term_labels), vector_field.animate.set_opacity(0.25))
        ...
        self.wait()
        self.play(TransformFromCopy(Mv, attached_Mv))
        ...
        solution = OldTex(v_tex, "=", "e^{M t}", "\\vec{\\textbf{v} }(0)")
        ...
        self.play(Transform(background_rect, new_br), FadeIn(solution, 0.5 * DOWN))
        self.wait()
        foreground = VGroup(background_rect, equation, solution)
        self.play(FadeOut(foreground))

        # Show flow of all initial conditions
        initial_points = np.array([plane.c2p(x, y) for x in np.arange(-16, 16, 0.5) for y in np.arange(-6, 6, 0.5)])
...
class DerivativeOfExpMt(Scene):
    def construct(self):
        ...
        self.add(solution)
        self.play(GrowArrow(arrow), FadeIn(equation, RIGHT), FadeIn(solves, lag_ratio=0.1))
        self.wait()
        ...
        lines = VGroup(*(OldTex(tex.replace("v_0", v0_tex), **kw) for tex in tex_expressions))
        ...
        self.play(
            TransformMatchingTex(
                VGroup(*(sm for sm in lines[0][5:] if sm.get_tex() not in ["\\left(", "\\right)"])).copy(),
                lines[1][5:],
            ),
            TransformFromCopy(lines[0][:5], lines[1][:5]),
        )
        self.play(lines[1].animate.set_y(1), FadeOut(lines[0], UP))
        self.wait()
        ...
        self.play(FadeIn(lines[1], DOWN), FadeOut(lines[2], DOWN))
        self.wait()
        self.play(
            LaggedStart(
                *(FlashUnder(sm, time_width=2, run_time=1) for sm in lines[3].get_parts_by_tex("M")[1:]),
                lag_ratio=0.2,
            )
        )
        self.wait()
...
class ExponentialPhaseFlow(Scene):
    ...
    def construct(self):
        ...
        basis_vectors[0].add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), box.pfp(7 / 8)))
        basis_vectors[1].add_updater(lambda m: m.put_start_and_end_on(plane.get_origin(), box.pfp(1 / 8)))
