...
def get_diag_rects(matrix, color=MEAN_COLOR, off_diagonal=False):
    ...
    return VGroup(*(SurroundingRectangle(entry, buff=SMALL_BUFF, color=color) for entry in entries))
    ...
    def construct(self):
        ...
        for pair, bt in zip(it.product(*top_terms), bottom_terms):
            ...
            if bt is alt_mid:
                self.remove(bt)
            self.remove(rects)
        self.play(FadeIn(VGroup(rhs2[0], rhs2[8])), FadeTransform(rhs[2:].copy(), rhs2[9:]))
        self.wait()
...
class OutlineThreeFacts(Scene):
    def construct(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(
                    AnimationGroup(*(Transform(entry, lambdas[i]) for entry in mat.get_entries().deepcopy()))
                    for i in [0, 2]
                ),
                lag_ratio=0.3,
            ),
        ...
        self.play(FadeIn(form_lhs, 0.25 * UP), FadeOut(third_point_placeholder, 0.25 * UP))
        self.play(
...
class ShowSquishingAndStretching(Scene):
    def construct(self):
        ...
        eigenlines = VGroup(*(Line(10 * ev, -10 * ev, color=color) for ev, color in zip(eigenvectors, EIGEN_COLORS)))
        eigenlines.set_stroke(GREY_B, 2)

        eigenvect_mobs = VGroup(*(Vector(ev, fill_color=color) for ev, color in zip(eigenvectors, EIGEN_COLORS)))
        eigenvect_mobs[0].add_updater(
            lambda m: m.put_start_and_end_on(plane.c2p(0, 0), plane.c2p(*eigenvectors[0][:2]))
        )
        eigenvect_mobs[1].add_updater(
            lambda m: m.put_start_and_end_on(plane.c2p(0, 0), plane.c2p(*eigenvectors[1][:2]))
        )
...
class MeanProductExample(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(FadeOut(minus_d, 0.25 * UP), FadeOut(plus_d, 0.25 * UP), lag_ratio=0.25),
            LaggedStart(FadeIn(minus_3, 0.25 * UP), FadeIn(plus_3, 0.25 * UP), lag_ratio=0.25),
        )
        self.wait()
...
class Example1(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.teacher.change("raise_right_hand", mat), FadeIn(mat.get_brackets(), UP))
        self.play(
        ...
        self.play(Write(m_eq[:2]), *(pi.animate.look_at(m_eq) for pi in self.pi_creatures))
        self.play(
...
class PauliMatrices(Scene):
    def construct(self):
        ...
        forms = VGroup(
            *(OldTex("0 \\pm \\sqrt{\\,0^2 - {p}}", **kw).next_to(matrix, RIGHT).to_edge(RIGHT) for matrix in matrices)
        )
        simple_forms = VGroup(*(OldTex("\\pm \\sqrt{-{p}}", **kw).move_to(form, LEFT) for form in forms))
        ...
        self.wait()
        self.play(
            LaggedStart(*(TransformMatchingShapes(form, simple_form) for form, simple_form in zip(forms, simple_forms)))
        )
        self.wait()
        ...
        final_forms = VGroup(*(OldTex("= \\pm 1").move_to(sf.get_center(), LEFT) for sf in simple_forms))

        self.play(
            LaggedStart(
                *(FadeTransform(pe[2].copy(), ff) for pe, ff in zip(prod_eqs, final_forms)),
                lag_ratio=0.3,
            ),
            LaggedStart(*(sf.animate.next_to(ff, LEFT) for sf, ff in zip(simple_forms, final_forms))),
        )
        self.wait()
...
class PauliMatricesWithCharacteristicPolynomial(PauliMatrices):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(TransformMatchingShapes(mat.copy(), det_term) for mat, det_term in zip(matrices, det_terms)),
                lag_ratio=0.8,
            ),
        ...
        self.play(VFadeIn(randy), randy.animate.set_opacity(1).change("hesitant", full_mat))
        self.play(ShowCreation(diag_rects))
...
class QuadraticPolynomials(Scene):
    def construct(self):
        ...
        # Show final roots
        d_terms = VGroup(*(OldTex(u, "\\sqrt{25 - 9}", font_size=24) for u in ["+", "-"]))
        my = m_label.get_y()
        arrows = VGroup(
...
class SkipTheMiddleStep(Scene):
    def construct(self):
        ...
        char_poly = OldTex("\\lambda^2 - 4\\lambda - 1", tex_to_color_map={"\\lambda": EIGEN_COLORS[0]})
        mp_formula = OldTex(
            "{2} \\pm \\sqrt{\\,{2}^2 - (-1)}",
            tex_to_color_map={
                "{2}": MEAN_COLOR,
