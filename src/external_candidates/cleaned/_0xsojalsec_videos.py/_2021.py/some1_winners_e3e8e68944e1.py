# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\some1_winners.py
...
def save_thumbnail_locally(slug, rewrite=False):
    ...
    urls = [
        *(f"https://img.youtube.com/vi/{slug}/{suffix}.jpg" for suffix in suffixes),
        *(f"https://i.ytimg.com/vi/{slug}/{suffix}.jpg" for suffix in suffixes),
    ]
    for url in urls:
        try:
...
def yt_slug_to_image_file(slug):
    return os.path.join(get_raster_image_dir(), "some1_thumbnails", slug + ".jpg")
...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        # Mention blog post
        url = VGroup(Text("Full details: "), Text("https://3b1b.co/some1-results"))
        ...
        self.play(
            LaggedStart(*(TransformFromCopy(bubble, bc) for bc in bubble_copies), lag_ratio=0.5),
            LaggedStart(
                *(student.change("thinking", UR) for student in self.students[1:]),
                lag_ratio=0.5,
            ),
            run_time=2,
        )
...
class FiltrationProcess(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(arrow), FadeIn(peer_words, lag_ratio=0.1))
        self.play(FadeIn(hundred), FadeIn(peer_subwords, 0.25 * DOWN))
        self.wait()
...
class RevealingTiles(Scene):
    ...
    def five_winners(self):
        ...
        # Animations
        self.play(Write(title), LaggedStartMap(GrowFromCenter, tiles, lag_ratio=0.15))
        self.wait()
        self.play(
            FadeIn(subtitle, lag_ratio=0.2),
            LaggedStart(
                *(ApplyMethod(tile.move_to, tiles[reorder[i]], path_arc=PI / 2) for i, tile in enumerate(tiles)),
                lag_ratio=0.25,
                run_time=2,
            ),
        )
    ...
    def honorable_mentions(self):
        ...
        new_tiles = Group(*tiles.target, *new_tile.replicate(len(HONORABLE_MENTIONS)))
    ...
    def reveal_tile(self, tile, title, author, image_name):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                tile,
                lambda m, a: m.set_opacity(1 if a < 0.5 else 0),
            ),
            UpdateFromAlphaFunc(
                image,
                lambda m, a: m.set_opacity(0 if a < 0.5 else 1),
            ),
        )
        tile.remove(tile[1])
...
class Narrative(Scene):
    def construct(self):
        ...
        for prop in properties:
            ...
            self.play(FadeIn(dot, scale=0.5), FadeIn(prop, lag_ratio=0.1))
...
class Traction(Scene):
    def construct(self):
        ...
        for elem in groups:
            self.play(FadeIn(elem[:2]), FadeIn(elem[-1], 0.5 * DOWN, scale=2))
        self.wait()
