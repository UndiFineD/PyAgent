...
class LayersOfAbstraction(Scene):
    ...
    def get_layers(self):
        ...
        arrow_labels = VGroup(
            ...
            OldTex("m_2 \\circ m_1").rotate(-np.arctan(1 / 2)).move_to(arrows[2]).shift(MED_SMALL_BUFF * DL),
        )
...
class CalculusStatement(Scene):
    def construct(self):
        statement = VGroup(
            OldTexText(
                "If $f(x)$ has a local maximum or minimum\\\\at $x_0$, and $f$ is differentiable at $x_0$, then"
            ),
...
class DifferenceOfSquares(Scene):
    def construct(self):
        x = 6
        y = 1
        squares = VGroup(*[VGroup(*[Square() for x in range(x)]).arrange(RIGHT, buff=0) for y in range(x)]).arrange(
            DOWN, buff=0
        )
