# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2021\holomorphic_dynamics.py
...
class HolomorphicDynamics(Scene):
    ...
    def complex_functions(self):
        ...
        self.play(LaggedStartMap(FadeIn, examples, shift=0.25 * DOWN, run_time=3, lag_ratio=0.5))
    ...
    def repeated_functions(self):
        ...
        self.play(FadeIn(words), FadeIn(iterations, lag_ratio=0.2, run_time=2))
        self.play(FadeIn(rule, 0.5 * DOWN))
        self.wait()
        self.play(LaggedStartMap(FadeOut, VGroup(words, iterations, rule, self.title_rect)))
...
class HolomorphicPreview(Scene):
    def construct(self):
        ...
        tiny_plane_image.apply_function(lambda p: out_plane.n2p(f(in_plane.p2n(p))))
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(
                    FadeTransform(
                        equation[4][i].copy().set_opacity(0),
                        equation[6][j].copy().set_opacity(0),
                    )
                    for i, j in zip(
                        [0, 0, 0, 0],
                        [0, 0, 0, 0],
        ...
        functions = VGroup(
            ...
            OldTex("a_n z^n + \\cdots + a_0 \\over b_m z^m + \\cdots + b_0"),
            OldTex("z^2 + c \\over 1"),
        )
...
class ShowFatouAndJulia(Scene):
    def construct(self):
        ...
        self.play(LaggedStart(*(Write(VGroup(*figure[2:])) for figure in figures), lag_ratio=0.7))
        self.play(
...
class MandelbrotIntro(Scene):
    ...
    def show_iterations(self):
        ...
        self.play(ShowCreationThenFadeOut(lines[0].copy().set_stroke(BLUE, 5)))
        self.wait(3)
...
class ParameterSpaceVsSeedSpace(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(TransformMatchingTex(top_labels[0].copy(), top_labels[1]))
        self.wait()
...
class ClassicJuliaSetDemo(MandelbrotIntro):
    def construct(self):
        ...
        planes[0].add_coordinate_labels((-1, 0, 1, 1j, -1j), font_size=18)
...
class FixedPoints(Scene):
    def construct(self):
        ...
        glow_dots = VGroup(*(glow_dot(plane.n2p(root)) for root in example_roots))
        ...
        def get_arrows(point, inward=False):
            arrows = VGroup(*(Arrow(ORIGIN, vect, buff=0.3) for vect in compass_directions(8)))
            arrows.set_height(1)
        ...
        stability_question = Text("When are fixed points stable?")
        stability_question.move_to(question)
        stable_underline = Underline(stability_question.get_part_by_text("stable"))
        ...
        for words, arrows in zip(stability_words, arrow_groups):
            self.play(FadeIn(words), ShowCreation(arrows, lag_ratio=0.2, run_time=3))
            self.wait()
        ...
        self.play(Blink(morty))
        self.wait()
        self.play(morty.change("pondering", newton_case[2]), FadeIn(newton_case[2]))
        self.play(Blink(morty))
        self.wait()

        # Show super-attraction
        super_arrows = VGroup(*(get_arrows(dot, inward=True) for dot in glow_dots))
        attraction_anims = []
        for cluster in super_arrows:
            for arrow in cluster:
                ...
                attraction_anims.append(ShowCreationThenFadeOut(new_arrow))
        ...
        self.play(morty.change("thinking"), ShowCreation(rect), FadeIn(super_words))
        self.play(
            LaggedStartMap(
                FadeOut,
                VGroup(
        ...
        self.play(LaggedStart(*attraction_anims, lag_ratio=0.02), run_time=3)
        self.wait()
        self.play(
            LaggedStartMap(
                FadeOut,
        ...
        part2 = OldTexText(
            "Exercise 1b: Determine when at least\\\\one fixed point is attracting.",
        )
        part3 = OldTexText(
        ...
        self.play(cardioid.animate.set_fill(YELLOW, 0.25), run_time=2)
        self.wait()
...
class UseNewton(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            ...
            target_mode="hooray",
            added_anims=[self.change_students("confused", "erm", "maybe")],
        )
        self.wait(2)
        self.student_says(
            Text("Too meta..."),
...
class DescribeDerivative(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(df_question, 0.2 * RIGHT), ShowCreation(df_arrow))
        self.wait()
        self.play(
        ...
        dot_radius = 0.025
        dots = DotCloud(
            [plane.c2p(x, y) for x in np.arange(-3.7, 3.7, 1.0 / density) for y in np.arange(-2.0, 2.1, 1.0 / density)]
        )
        ...
        epsilon = 5e-3
        tiny_dots = DotCloud(
            [plane.n2p(self.z + epsilon * complex(x, y)) for x in np.arange(-20, 20) for y in np.arange(-10, 10)]
        )
...
class Cycles(FixedPoints):
    def construct(self):
        ...
        self.play(*map(ShowCreation, eq_arrows), FadeIn(julia_f2_eqs[0], 0.5 * DOWN))
        self.wait()
        self.play(TransformMatchingShapes(julia_f2_eqs[0].copy(), julia_f2_eqs[1]))
        self.wait()
        ...
        arrow_loops = VGroup(*(self.get_arrow_loop(dot) for dot in fixed_dots))
        ...
        # N cycles
        fn_eq = OldTex("f(f(\\cdots f(z) \\cdots)) = z", **kw)
        fn_eq.move_to(f2_equation, LEFT)
        fn_eq.shift(SMALL_BUFF * DOWN)
        brace = Brace(fn_eq[: fn_eq.index_of_part_by_tex("z")], DOWN)
        ...
        for k in range(1, n + 1):
            dot_anims.append(ApplyMethod(z_dot.move_to, z_dots[k % n], path_arc=PI / 8))
        ...
        self.play(
            PiCreatureSays(morty, "How many solutions?", bubble_config={"height": 2, "width": 4}),
        )
        self.play(Blink(morty))
        self.wait()
        self.play(
    ...
    def show_composition(self, ref_mob, morty, **kwargs):
        ...
        for n, poly in enumerate(polys[1:]):
            ...
            self.play(
                ...
                UpdateFromAlphaFunc(
                    degree,
                    lambda m, a: m[1].set_value((2 ** (n + 1) if a < 0.5 else 2 ** (n + 2))),
                ),
                *anims,
...
class TwoToMillionPoints(Scene):
    ...
    def get_julia_set_points(self, plane, n_points, n_steps):
        values = np.array([complex(math.cos(x), math.sin(x)) for x in np.linspace(0, TAU, n_points)])
...
class AmbientNewtonBoundary(AmbientNewtonRepetition):
    def construct(self):
        ...
        self.play(GrowFromPoint(fractal, fractal.get_corner(UL), run_time=5))
        self.wait()
...
class CyclicAttractorSmallRadius(CyclicAttractor):
    ...
    def construct(self):
        ...
        dots = VGroup(*(Dot(rd.get_center()) for rd in self.root_dots))
        dots.set_stroke(BLACK, 3)
        dots.set_fill(opacity=0)

        self.add(fractal, *self.mobjects, dots)
        self.play(FadeIn(fractal), self.plane.animate.fade(0.5))
        self.wait()
...
class ShowCenterOfMassPoint(PlayWithRootsSeekingCycles):
    ...
    def construct(self):
        ...
        mean_dot.add_updater(lambda m: m.move_to(sum([rd.get_center() for rd in self.root_dots]) / 3))
        self.add(mean_dot)
        ...
        self.play(MoveAlongPath(self.root_dots[2], circle, rate_func=linear, run_time=20))
        self.play(
...
class GenerateCubicParameterPlot(Scene):
    ...
    def construct(self):
        ...
        root_dots = VGroup(*(Dot(planes[0].n2p(z), color=color) for color, z in zip(colors, [-1, 1, 1j])))
        ...
        left_fractal.add_updater(lambda m: m.set_roots([planes[0].p2n(rd.get_center()) for rd in root_dots]))
        ...
        self.play(root_dots[2].animate.move_to(planes[0].get_corner(UL)), run_time=2)
        self.wait()
        ...
        com_dot.add_updater(lambda m: m.move_to(np.array([rd.get_center() for rd in root_dots]).mean(0)))
        ...
        for y, back in zip(y_range, it.cycle([False, True])):
            ...
            for x in x_range[::-1] if back else x_range:
                ...
                square.move_to(col_meta_fractal, DOWN + (LEFT if back else RIGHT))
...
class MentionFatouSetsAndJuliaSets(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        for words in lhs[-1]:
            self.play(FlashUnder(words, buff=0, time_width=1.5))
    ...
    def get_fractals(self, jy=1.5, fy=-2.5):
        ...
        for fractal, plane in zip((*fatou, julia), (*f_planes, j_plane)):
            fractal.plane = plane
            fractal.add_updater(
                lambda m: m.set_offset(m.plane.get_center()).set_scale(m.plane.get_x_unit_size()).replace(m.plane)
            )
        ...
        for n in range(self.n_steps):
            ...
            if self.show_disk:
                ...
                if disk.target.get_height() > frame.get_height():
                    anims.extend([mob.animate.scale(2.0) for mob in [frame, fractal]])
            self.play(
...
class AboutFatouDisks(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(arrows[-1]), Write(shrink_words, run_time=2))
        self.wait()
...
class AboutJuliaDisks(AboutFatouDisks):
    def construct(self):
        ...
        words4 = OldTexText("``Stuff goes everywhere'' principle of Julia sets", font_size=60)
...
class DescribeChaos(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(ShowCreation(strike), FadeIn(new_words, shift=0.25 * DOWN))
...
class SimulationOfTinyDisk(RepeatedNewton):
    ...
    def construct(self):
        ...
        julia_set.add_updater(lambda m: m.set_julia_highlight(get_height_ratio() * 1e-3))
        julia_set.set_opacity(0.5)

        # Generate dots
        point = [1.10049904, 1.38962415, 0.0]
        ...
        dots.set_points(
            [
                [
                    cluster_radius * r * math.cos(theta),
                    cluster_radius * r * math.sin(theta),
                    0,
                ]
...
class SimulationAnnotations(Scene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(brace_tex, DOWN))
        self.wait()
...
class LattesExample(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.teacher.change("raise_right_hand", 3 * UR),
            self.change_students("pondering", "happy", "tease", look_at=3 * UR),
        )
        self.wait(3)
        self.play(self.teacher.change("sassy", example), Write(example))
        self.play_student_changes(
            "pondering",
            "pondering",
            "pondering",
...
class MoreAmbientChaos(TwoToMillionPoints):
    def construct(self):
        ...
        epsilon = 1e-6
        dots = DotCloud([point + epsilon * rotate_vector(RIGHT, random.random() * TAU) for x in range(10)])
        dots.set_gloss(0.5)
        dots.set_shadow(0.5)
        ...
        for n in range(n_steps):
            ...
            lines = VGroup(*(Line(p1, p2) for p1, p2 in zip(points, new_points)))
            lines.set_stroke(WHITE, 1)
...
class MetaFractal(IntroNewtonFractal):
    ...
    def construct(self):
        ...
        root_dots = self.root_dots = VGroup(
            *(Dot(plane.n2p(root), color=color) for root, color in zip(self.fixed_roots, colors))
        )
        ...
        fractal.add_updater(lambda f: f.set_fixed_roots([plane.p2n(dot.get_center()) for dot in root_dots]))
        ...
        self.play(MoveToTarget(frame), run_time=8, rate_func=bezier([0, 0, 1, 1]))
        self.play(fractal.animate.set_saturation_factor(4), run_time=3)
        self.play(
            UpdateFromAlphaFunc(
                frame,
        ...
        self.wait(2)
        self.play(Restore(frame), fractal.animate.set_saturation_factor(0), run_time=7)
        self.wait()
...
class AmbientJulia(Scene):
    def construct(self):
        ...
        fractal.add_updater(lambda m: m.set_c(plane.p2n(cardioid.pfp(get_t()))))
