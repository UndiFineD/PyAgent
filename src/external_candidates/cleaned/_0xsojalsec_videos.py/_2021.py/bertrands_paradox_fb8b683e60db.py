...
class ChordsInSpaceWithCircle(RandomChordScene):
    def construct(self):
        ...
        # Show colors
        key = OldTexText(
            "Blue $\\Rightarrow$ Chord > \\text{Triangle side}\\\\Red $\\Rightarrow$ Chord < \\text{Triangle side}\\\\",
            tex_to_color_map={
                "Blue": BLUE,
                "Red": RED,
            },
...
class NonTransitive(Scene):
    def construct(self):
        ...
        words = OldTexText(
            "Rotational symmetries do \\emph{not} act\\\\transitively on the space of all chords",
