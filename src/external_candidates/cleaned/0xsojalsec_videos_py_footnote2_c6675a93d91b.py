# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\eola.py\footnote2_c6675a93d91b.py
...
class ColumnsRepresentBasisVectors(Scene):
    ...
    def put_in_thought_bubble(self):
        ...
        self.play(
            ApplyFunction(lambda m: bubble.position_mobject_inside(m.set_height(2.5)), everything),
            ShowCreation(bubble),
            randy.change_mode,
            "pondering",
        )
...
class DotProductPreview(VectorScene):
    ...
    def show_scaling(self):
        ...
        start_brace, interim_brace, final_brace = braces = [
            Brace(Line(ORIGIN, norm * RIGHT), UP) for norm in (1, self.v.get_length(), dot_product)
        ]
