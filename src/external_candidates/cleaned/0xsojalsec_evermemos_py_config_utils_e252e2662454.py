# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\es.py\config_utils_e252e2662454.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\es\config_utils.py
...
def get_tenant_es_config() -> Optional[Dict[str, Any]]:
    ...
    if not tenant_info:
        logger.debug("⚠️ Tenant context is not set, unable to get tenant Elasticsearch configuration")
    ...
    final_config = {
        "hosts": tenant_hosts or env_fallback_config.get("hosts", ["http://localhost:9200"]),
        ...
        "verify_certs": es_config.get("verify_certs", env_fallback_config.get("verify_certs", False)),
    }
