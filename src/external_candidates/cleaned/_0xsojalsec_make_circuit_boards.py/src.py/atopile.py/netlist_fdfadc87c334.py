...
from atopile import components, errors, layout, nets
...
class NetlistBuilder:
    ...
    def make_component(self, comp_addr: AddrStr, libsource: KicadLibpart) -> KicadComponent:
    ...
    def build(self, root) -> KicadNetlist:
        ...
        for footprint, group_components in groupby(components.get_footprint, all_components).items():
        ...
        for code, (net_name, pin_signal_list) in enumerate(nets.get_nets_by_name(root).items(), start=1):
...
def get_netlist_as_str(root: AddrStr) -> str:
    ...
    env = Environment(loader=FileSystemLoader(Path(__file__).parent), undefined=StrictUndefined)
