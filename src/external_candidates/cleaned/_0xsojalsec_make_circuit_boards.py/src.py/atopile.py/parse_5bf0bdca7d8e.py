# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\parse.py
import logging
...
class ErrorListenerConverter(ErrorListener):
    ...
    def syntaxError(self, recognizer, offendingSymbol, line, column, msg, e: Exception):
        raise AtoSyntaxError.from_token(
            offendingSymbol,
            f"{str(e)} '{msg}'",
        )
...
class ErrorListenerCollector(ErrorListenerConverter):
    ...
    def syntaxError(self, recognizer, offendingSymbol, line, column, msg, e: Exception):
        self.errors.append(AtoSyntaxError.from_token(offendingSymbol, f"{str(e)} '{msg}'"))
