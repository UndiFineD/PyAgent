# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\config.py
...
def get_project_dir_from_path(path: Path) -> Path:
    ...
    for p in [path] + list(path.parents):
        ...
        if (clean_path / CONFIG_FILENAME).exists():
            return clean_path
    raise atopile.errors.AtoFileNotFoundError(f"Could not find {CONFIG_FILENAME} in {path} or any parents")
...
def find_layout(layout_base: Path) -> Optional[Path]:
    ...
    if layout_base.with_suffix(".kicad_pcb").exists():
        ...
        layout_candidates = list(filter(match_user_layout, layout_base.glob("*.kicad_pcb")))
        ...
        if len(layout_candidates) == 1:
            ...
            raise atopile.errors.AtoError(
                f"Layout directories must contain only 1 layout, but {len(layout_candidates)} found in {layout_base}"
            )
