# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\layout.py
"""
...
def generate_module_map(build_ctx: config.BuildContext) -> None:
    ...
    for module_instance in filter(match_modules, all_descendants(build_ctx.entry)):
        ...
        uuid_map = {}
        for inst_addr, layout_addr in zip(all_descendants(module_instance), all_descendants(module_super_ctx.entry)):
            ...
            if address.get_name(inst_addr) != address.get_name(layout_addr):
                errors.AtoError(
                    "Mismatched names in layout and instance addresses, skipping component in auto-layout generation"
