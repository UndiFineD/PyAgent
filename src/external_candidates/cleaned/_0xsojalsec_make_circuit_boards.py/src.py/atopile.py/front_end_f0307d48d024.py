...
def _get_unit_from_ctx(ctx: ParserRuleContext) -> pint.Unit:
    ...
    try:
        ...
        raise errors.AtoUnknownUnitError.from_ctx(ctx, f"Unknown unit '{unit_str}'") from ex
        ...
        if tol_ctx.name():
            ...
            try:
                ...
                raise errors.AtoTypeError.from_ctx(
                    tol_ctx.name(),
                    f"Tolerance unit '{tolerance_unit}' is not dimensionally compatible with nominal unit '{unit}'",
                ) from ex
    ...
    def visitBound_quantity(self, ctx: ap.Bound_quantityContext) -> RangedValue:
        ...
        if start_unit is None and end_unit is None:
            ...
            try:
                ...
                raise errors.AtoTypeError.from_ctx(
                    ctx,
                    f"Tolerance unit '{end_unit}' is not dimensionally compatible with nominal unit '{start_unit}'",
                ) from ex
        elif start_unit:
            unit = start_unit
        else:
...
class Roley(HandlesPrimaries):
    ...
    def visitArithmetic_expression(self, ctx: ap.Arithmetic_expressionContext) -> expressions.NumericishTypes:
        if ctx.ADD():
...
class Scoop(HandleStmtsFunctional, HandlesPrimaries):
    ...
    def _register_obj_tree(self, obj: ClassDef, addr: AddrStr, closure: tuple[ClassDef]) -> None:
    ...
    def _do_import(self, ctx: ParserRuleContext, from_file: str, what_refs: list[str]) -> KeyOptMap:
    ...
    def visitImport_stmt(self, ctx: ap.Import_stmtContext) -> KeyOptMap:
        ...
        what_refs = [self.visit_ref_helper(name_of_attr) for name_of_attr in ctx.name_or_attr()]
...
def lookup_class_in_closure(context: ClassDef, ref: Ref) -> AddrStr:
    ...
    for scope in context.closure:
        ...
        import_leads = {imp_ref: imp for imp_ref, imp in scope.imports.items() if ref[0] == imp_ref[0]}
    ...
    raise errors.AtoKeyError.from_ctx(context.src_ctx, f"Couldn't find {ref} in the scope of {context}")
...
class Dizzy(HandleStmtsFunctional, HandlesPrimaries):
    ...
    def __init__(
        ...
        self._output_cache: dict[AddrStr, ClassLayer] = {k: v for k, v in BUILTINS_BY_ADDR.items()}
    ...
    def build_layer(self, cls_def: ClassDef) -> ClassLayer:
        ...
        obj = ClassLayer(
            ...
            assignments={ref[0]: v for ref, v in strainer.strain(lambda x: isinstance(x.value, Assignment))},
    ...
    def visitDeclaration_stmt(self, ctx: ap.Declaration_stmtContext) -> KeyOptMap:
        ...
        assigned_value_ref = self.visitName_or_attr(ctx.name_or_attr())
        if len(assigned_value_ref) > 1:
            raise errors.AtoSyntaxError(f"Can't declare fields in a nested object {assigned_value_ref}")
...
class Lofty(HandleStmtsFunctional, HandlesPrimaries):
    ...
    @contextmanager
    def apply_replacements_from_objs(self, objs: Iterable[ClassLayer]) -> Iterable[AddrStr]:
        """
        ...
        for obj in objs:
            for ref, replacement in obj.obj_def.replacements.items():
                to_be_replaced_addr = address.add_instances(self._instance_addr_stack.top, ref)
    ...
    def build_instance(
        ...
        new_instance = Instance.from_super(new_addr, super_obj, parent=parent_instance, src_ctx=src_ctx)
    ...
    def handle_new_assignment(self, ctx: ap.Assign_stmtContext) -> KeyOptMap:
        ...
        if new_addr in self._known_replacements:
            ...
            try:
                current_obj_def = self.obj_layer_getter(self._class_addr_stack.top).obj_def
                new_class_addr = lookup_class_in_closure(current_obj_def, new_class_ref)
            except KeyError as ex:
                raise errors.AtoKeyError.from_ctx(current_obj_def.src_ctx, f"Couldn't find ref {new_class_ref}") from ex
    ...
    def visitAssign_stmt(self, ctx: ap.Assign_stmtContext) -> KeyOptMap:
        ...
        instance_addr_assigned_to = address.add_instances(self._instance_addr_stack.top, assigned_ref[:-1])
    ...
    def visit_pin_or_signal_helper(self, ctx: ap.Pindef_stmtContext | ap.Signaldef_stmtContext) -> AddrStr:
