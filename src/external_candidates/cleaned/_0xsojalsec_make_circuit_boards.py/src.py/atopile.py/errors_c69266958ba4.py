# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\errors.py
...
def _error_accumulator(
    ...
    if errors:
        ...
        displayed_errors = []
        for error in errors:
            if not any(existing_error.__dict__ == error.__dict__ for existing_error in displayed_errors):
