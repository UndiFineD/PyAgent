...
def create(
    ...
    for _ in stuck_user_helper_generator:
        if not name:
            name = rich.prompt.Prompt.ask(":rocket: What's your project [cyan]name?[/]", default=kebab_name)
        ...
        if name != kebab_name:
            ...
            if rich.prompt.Confirm.ask(f"Do you want to use [cyan]{kebab_name}[/] instead?", default=True):
                name = kebab_name
    ...
    if not repo and not rich.prompt.Confirm.ask("Would you like to create a new repo for this project?"):
    ...
    if repo_obj.is_dirty():
        ...
        rich.print("[yellow]No changes to commit! Seems like the template you used mightn't be configurable?[/]")
    ...
    if repo_obj.remotes.origin.url == PROJECT_TEMPLATE:
        try:
            ...
            errors.AtoError(f"Failed to remove .git directory: {repr(ex)}").log(log, logging.WARNING)
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        if not ato_yaml_path.exists():
            ...
            if "builds" not in ato_config:
                ato_config["builds"] = {}
            ato_config["builds"][build_name] = {"entry": f"elec/src/{entry_file}:{entry_module}"}
...
def do_configure(name: str, _repo_path: str, debug: bool):
    ...
    for template_path in repo_path.glob("**/*.j2"):
        ...
        target_path = template_path.parent / template_path.name.replace(".j2", "").replace(
            "__name__", caseconverter.kebabcase(name)
        )
