# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\cli\configure.py
"""
...
def do_configure() -> None:
    ...
    if config.install_kicad_plugin is None:
        config.install_kicad_plugin = rich.prompt.Confirm.ask(":wrench: Install KiCAD plugin?", default="y")
    ...
    # final steps
    config.version = str(atopile.version.clean_version(atopile.version.get_installed_atopile_version()))
    _save_config()
