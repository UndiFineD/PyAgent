# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\cli\common.py
"""
...
def project_options(f):
    ...
    def wrapper(
        ...
        **kwargs,
        ...
        if option:
            try:
                config: atopile.config.ProjectConfig = OmegaConf.merge(project_config, OmegaConf.from_dotlist(option))
        ...
        if entry:
            if entry_arg_file_path.is_file():
                if entry_section := address.get_entry_section(entry):
                    ...
                    raise click.BadParameter(
                        "If an entry of a file is specified, you must specify the node within it you want to build.",
                        param_hint="entry",
                    )
                ...
                raise ValueError(f"Unexpected entry path type {entry_arg_file_path} - this should never happen!")
        ...
        with errors.handle_ato_errors():
            if build_names := build or config.builds.keys():
                build_ctxs: list[atopile.config.BuildContext] = [
                    atopile.config.BuildContext.from_config_name(config, build_name) for build_name in build_names
                ]
            else:
                build_ctxs = [
                    atopile.config.BuildContext.from_config("default", atopile.config.ProjectBuildConfig(), project_ctx)
                ]
...
def check_compiler_versions(config: atopile.config.ProjectConfig):
    ...
    with errors.handle_ato_errors():
        ...
        for cltr, cfg in errors.iter_through_errors(itertools.chain([config], dependency_cfgs)):
            if cfg is None:
                continue
