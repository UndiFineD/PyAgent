# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\bom.py
"""
...
_get_mpn = errors.downgrade(components.get_mpn, (components.MissingData, components.NoMatchingComponent))
...
def _get_footprint(addr: address.AddrStr) -> str:
    ...
    if value := errors.downgrade(components.get_footprint, components.MissingData)(addr):
        return value
...
def _get_value(addr: address.AddrStr) -> str:
    ...
    value = str(errors.downgrade(components.get_specd_value, components.MissingData)(addr))
...
def generate_designator_map(entry_addr: address.AddrStr) -> str:
    ...
    for row_index, (s_des, n_comp) in enumerate(sorted_designator_dict.items()):
        sorted_des_table.add_row(s_des, n_comp, style=dark_row if row_index % 2 else light_row)
    for row_index, (s_comp, n_des) in enumerate(sorted_comp_name_dict.items()):
        sorted_name_table.add_row(s_comp, n_des, style=dark_row if row_index % 2 else light_row)
...
def generate_bom(entry_addr: address.AddrStr) -> str:
    ...
    for mpn, components_in_group in bom.items():
        if mpn:
            ...
            friendly_designators = ",".join(components.get_designator(component) for component in components_in_group)
