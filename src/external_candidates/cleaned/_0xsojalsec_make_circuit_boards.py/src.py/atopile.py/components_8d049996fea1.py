# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\components.py
import json
import logging
import time
...
def _get_generic_from_db(component_addr: str) -> dict[str, Any]:
    ...
    specd_data_dict = {k: v.to_dict() if isinstance(v, RangedValue) else v for k, v in specd_data.items()}
    ...
    try:
        ...
        raise errors.AtoInfraError(f"Error connecting to database: {str(ex)}", addr=component_addr) from ex
...
def download_footprint(addr: AddrStr, footprint_dir: Path):
    ...
    try:
        ...
        raise MissingData("db component for $addr has no footprint", title="No Footprint", addr=addr) from ex
...
def get_footprint(addr: AddrStr) -> str:
    ...
    if _is_generic(addr):
        ...
        try:
            ...
            raise errors.AtoInfraError("db component for $addr has no footprint", addr=addr) from ex
        return footprint
...
class DesignatorManager:
    ...
    def _make_designators(self, root: str) -> dict[str, str]:
        ...
        for component in filter(instance_methods.match_components, instance_methods.all_descendants(root)):
        ...
        for component in unnamed_components:
            prefix = instance_methods.get_data_dict(component).get("designator_prefix", "U")
