# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Schema\VdbResult.py
import asyncio
...
class VectorIndexEdgeResult(RelationResult):
    ...
    async def get_edge_data(self, graph, score=False):
        nodes = await asyncio.gather(
            *[graph.get_edge(r.metadata["src_id"], r.metadata["tgt_id"]) for r in self.results]
        )
        if score:
            return nodes, [r.score for r in self.results]
        else:
...
class VectorIndexSubgraphResult(SubgraphResult):
    ...
    async def get_subgraph_data(self, score=False):
        subgraphs_data = list(
            map(
                lambda x: {
                    "source_id": x.metadata["source_id"],
                    "subgraph_content": x.text,
                },
                self.results,
            )
        )
