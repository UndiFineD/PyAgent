# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\MedQuery.py
import asyncio
...
from Core.Common.Constants import GRAPH_FIELD_SEP, Retriever
...
class MedQuery(BaseQuery):
    ...
    async def _concatenate_information(self, metagraph_relation: str, metagraph_edge: tuple[str, str]):
        ...
        return list(
            map(
                lambda x: metagraph_edge[0] + " " + x + " " + metagraph_edge[1],
                metagraph_relation_seperated,
            )
        )

    async def _retrieve_relevant_contexts(self, query: str):
        # find the most relevant subgraph
        context = await self._retriever.retrieve_relevant_content(
            type=Retriever.SUBGRAPH,
            mode="concatenate_information_return_list",
            seed=query,
        )  # return list
        ...
        metagraph_nodes = await self._retriever.retrieve_relevant_content(
            seed=list(metagraph_nodes),
            k=self.config.k_hop,
            type=Retriever.SUBGRAPH,
        ...
        metagraph = await self._retriever.retrieve_relevant_content(
            seed=list(metagraph_nodes),
            type=Retriever.SUBGRAPH,
            mode="induced_subgraph_return_networkx",
        )  # return networkx
        metagraph_edges = list(metagraph.edges())
        metagraph_super_relations = await self._retriever.retrieve_relevant_content(
            seed=metagraph_edges, type=Retriever.RELATION, mode="by_source&target"
        )  # super relations
        zip_combined = tuple(zip(metagraph_super_relations, metagraph_edges))
        metagraph_concatenate = await asyncio.gather(
            *[
                self._concatenate_information(metagraph_super_relation, metagraph_edge)
                for metagraph_super_relation, metagraph_edge in zip_combined
            ]
        )  # list[list]
    ...
    async def generation_qa(self, query: str, context: str):
        messages = [
            ...
            },
            {
                ...
                + query
                + ", the provided information is (list seperated by ,): "
                + context,
            },
        ]
