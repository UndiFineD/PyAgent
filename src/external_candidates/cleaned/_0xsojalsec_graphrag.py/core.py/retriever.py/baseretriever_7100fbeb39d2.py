# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Retriever\BaseRetriever.py
import asyncio
from abc import ABC
...
class BaseRetriever(ABC):
    ...
    async def _construct_relationship_context(self, edge_datas: list[dict]):
        ...
        edge_degree = await asyncio.gather(*[self.graph.edge_degree(r["src_id"], r["tgt_id"]) for r in edge_datas])
        edge_datas = [
