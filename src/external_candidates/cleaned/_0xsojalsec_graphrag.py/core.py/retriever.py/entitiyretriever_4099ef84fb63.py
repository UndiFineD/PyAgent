# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Retriever\EntitiyRetriever.py
import asyncio
...
class EntityRetriever(BaseRetriever):
        ...
        try:
            ...
            if tree_node:
                try:
                    ...
                    logger.warning(
                        "Only tree graph support this! Please check the `graph_type' item in your config file"
                    )
            node_degrees = await asyncio.gather(*[self.graph.node_degree(node["entity_name"]) for node in node_datas])
            node_datas = [
    ...
    async def _link_entities(self, query_entities):
        ...
        entities = await asyncio.gather(
            *[
                self.entities_vdb.retrieval_nodes(query_entity, top_k=1, graph=self.graph)
        ...
        try:
            ...
            for index, entity in enumerate(total_entity_relation_list):
                ...
                if len(candidate_list) == 1:
                    ...
                    # agent
                    prompt = (
                        score_entity_candidates_prompt.format(query, entity["relation"])
                        + "; ".join(candidate_list)
                        + ";"
                        + "\nScore: "
                    )
    ...
    async def _find_relevant_entities_by_relationships(self, seed):
        ...
        node_datas = await asyncio.gather(*[self.graph.get_node(entity_name) for entity_name in entity_names])

        node_degrees = await asyncio.gather(*[self.graph.node_degree(entity_name) for entity_name in entity_names])
        for k, n, d in zip(entity_names, node_datas, node_degrees):
            if "description" not in n:
                n["description"] = ""

            node_datas = [{**n, "entity_name": k, "rank": d}]
