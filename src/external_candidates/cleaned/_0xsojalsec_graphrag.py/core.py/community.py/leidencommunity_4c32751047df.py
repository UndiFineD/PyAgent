# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Community\LeidenCommunity.py
"""
...
class LeidenCommunity(BaseCommunity):
    ...
    async def _clustering(self, largest_cc, max_cluster_size, random_seed):
        if largest_cc is None:
            logger.warning(
                "No largest connected component found, skipping Leiden clustering; Please check the input graph."
            )
            return None
        ...
        for partition in community_mapping:
            ...
            node_communities[clean_str(partition.node)].append({"level": level_key, "cluster": str(cluster_id)})
    ...
    async def _generate_community_report(self, er_graph):
        ...
        community_keys, community_values = (
            list(self._communities_schema.keys()),
            list(self._communities_schema.values()),
        )
    ...
    async def _form_single_community_report(
        self, er_graph, community, already_reports: dict[str, CommunityReportsResult]
    ) -> dict:
    ...
    async def _pack_single_community_describe(
        ...
        if need_to_use_sub_communities or self.enforce_sub_communities:
            ...
            (
                ...
                contain_nodes,
                contain_edges,
            ) = await self._pack_single_community_by_sub_communities(community, max_token_size, already_reports)
