# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\data.py\vqa.py\tasks.py\blueprints.py\action_prediction.py\solver_4ba6012dee23.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\data\vqa\tasks\blueprints\action_prediction\solver.py
...
def generate_next_action_questions(num_questions: int = 3) -> Solver:
    ...
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        ...
        for _ in range(min(num_questions, len(action_sequence) - 1)):
            ...
            prompt = Templates.action_prediction(previous_actions=previous_action_texts, blueprint=blueprint)
...
def generate_construction_order_questions(num_questions: int = 2) -> Solver:
    ...
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        ...
        if len(entities) < 3:
            state.metadata["error"] = "Not enough entities for construction order questions"
        ...
        for _ in range(num_questions):
            ...
            sorted_entities = sorted(
                selected_entities,
                key=lambda e: priority_order.get(e.get("name", "unknown").split("-")[0], 5),
            )
