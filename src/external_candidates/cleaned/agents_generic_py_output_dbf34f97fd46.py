# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agents_generic.py\livekit_agents.py\livekit.py\agents.py\voice.py\room_io.py\output_dbf34f97fd46.py
...
class _ParticipantLegacyTranscriptionOutput:
    ...
    def set_participant(
        ...
        self._participant_identity = participant.identity if isinstance(participant, rtc.Participant) else participant
    ...
    def _on_track_published(self, track: rtc.RemoteTrackPublication, participant: rtc.RemoteParticipant) -> None:
        if not self._is_local_proxy_participant(participant) or track.source != rtc.TrackSource.SOURCE_MICROPHONE:
            return
    ...
    def _on_local_track_published(self, track: rtc.LocalTrackPublication, _: rtc.Track | None) -> None:
        if (
...
class _ParticipantStreamTranscriptionOutput:
    ...
    async def _create_text_writer(self, attributes: dict[str, str] | None = None) -> rtc.TextStreamWriter:
...
class _ParticipantTranscriptionOutput(io.TextOutput):
    def __init__(
        ...
        *,
        ...
        self.__outputs: list[_ParticipantLegacyTranscriptionOutput | _ParticipantStreamTranscriptionOutput] = [
