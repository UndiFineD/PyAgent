# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentcloud.py\agent_backend.py\src.py\tools.py\retrievers.py\custom.py\time_weighted_retriever_0f0c6bb8cc4d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\tools\retrievers\custom\time_weighted_retriever.py
...
class CustomTimeWeightedVectorStoreRetriever(BaseRetriever):
    """Retriever that combines embedding similarity with

    recency in retrieving values."""

    time_weight_field_name: str = Field(default="last_accessed_at")

    """The name of the field used for recency lookup"""
    ...
    def _get_combined_score(
        ...
        hours_passed = _get_hours_passed(
            current_time,
            self._document_get_date(self.time_weight_field_name, document),
        )
    ...
    async def aget_salient_docs(self, query: str) -> Dict[int, Tuple[Document, float]]:
        ...
        docs_and_scores = await self.vectorstore.asimilarity_search_with_relevance_scores(query, **self.search_kwargs)
    ...
    def _get_rescored_docs(self, docs_and_scores: Dict[Any, Tuple[Document, Optional[float]]]) -> List[Document]:
        ...
        rescored_docs = [
            (doc, self._get_combined_score(doc, relevance, current_time)) for doc, relevance in docs_and_scores.values()
        ]
    ...
    def add_documents(self, documents: List[Document], **kwargs: Any) -> List[str]:
        ...
        for i, doc in enumerate(dup_docs):
            if self.time_weight_field_name not in doc.metadata:
                doc.metadata[self.time_weight_field_name] = current_time
