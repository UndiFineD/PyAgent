# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\bvinci1_design.py\flow.py\natural_language_parser_658659480797.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bvinci1-design\flow\natural_language_parser.py
...
class NaturalLanguageParser:
    ...
    def parse(self, request: str) -> ParsedIntent:
        ...
        confidence = self._calculate_confidence(primary_action, capabilities, steps)
    ...
    def _infer_steps(self, request: str) -> List[str]:
        ...
        if not steps:
            steps = ["Initialize flow", "Execute main action", "Return results"]
    ...
    def _generate_name(self, action: str, capabilities: List[str]) -> str:
        ...
        action_words = [w for w in words[:2] if w not in ["the", "and", "for", "with", "that", "this"]]
    ...
    def _calculate_confidence(self, action: str, capabilities: List[str], steps: List[str]) -> float:
    ...
    def suggest_refinements(self, intent: ParsedIntent) -> List[str]:
        ...
        if intent.confidence < 0.5:
            suggestions.append("Try being more specific about what you want to accomplish")

        if len(intent.capabilities) == 1 and intent.capabilities[0] == "general":
            suggestions.append("Mention specific technologies or data types involved")

        if len(intent.steps) <= 1:
            suggestions.append("Consider breaking down the task into clear steps")
