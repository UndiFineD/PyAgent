# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\query.py\basequery_28f15e746fa6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\BaseQuery.py
...
class BaseQuery(ABC):
    ...
    async def _map_global_communities(self, query: str, communities_data):
        ...
        async def _process(community_truncated_datas: list[Any]) -> dict:
            ...
            response = await self.llm.aask(query, system_msgs=[sys_prompt])
