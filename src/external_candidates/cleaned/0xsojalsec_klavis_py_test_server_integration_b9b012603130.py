# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\open_strata.py\tests.py\test_server_integration_b9b012603130.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\tests\test_server_integration.py
        ...
        async def call_tool(name: str, arguments: Dict[str, Any]):
            ...
            if name == "get_time":
                ...
                return [
                    {
                        "type": "text",
                        "text": json.dumps({"result": result, "operation": operation, "a": a, "b": b}),
                    }
                ]
    ...
    if tool_name == TOOL_DISCOVER_SERVER_ACTIONS:
        ...
        try:
            ...
            if action_found:
                ...
                result = {"error": f"Action '{action_name}' not found in server '{server_name}'"}
...
class TestServerToolCalls:
            ...
            if data["servers"]:
                for server_name, actions in data["servers"].items():
                    ...
                    if isinstance(actions, list):
                        ...
                        logger.warning(f"Server {server_name} error: {actions['error']}")
    ...
    async def test_discover_server_actions_with_query(self, setup_test_environment):
        ...
        try:
            ...
            if "github" in data["servers"] and isinstance(data["servers"]["github"], list):
    ...
    async def test_get_action_details(self, setup_test_environment):
        ...
        try:
            ...
            if "github" in discover_data["servers"] and isinstance(discover_data["servers"]["github"], dict):
    ...
    async def test_search_documentation(self, setup_test_environment):
        ...
        try:
            ...
            if data and not any("error" in item for item in data if isinstance(item, dict)):
                # Found some results
...
class TestServerIntegrationWithRealServer:
    ...
    async def test_full_workflow(self):
        ...
        try:
            ...
            discover_result = await call_server_tool(TOOL_DISCOVER_SERVER_ACTIONS, {"user_query": ""})
            ...
            for server_name, server_data in discover_data["servers"].items():
                if isinstance(server_data, dict) and "actions" in server_data and server_data["actions"]:
                    ...
                    if "error" not in details_data:
                        ...
                        logger.info(f"Input schema: {details_data['action'].get('inputSchema', {})}")
