# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\config\settings.py
import json
import logging
import os
import sys
...
class AppSettings(BaseSettings):
    ...
    log_level: str = "INFO"
    model_config = SettingsConfigDict(env_prefix="ROAMPAL_APP_", extra="ignore", case_sensitive=False)
...
class WebSearchSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="ROAMPAL_WEB_", extra="ignore", case_sensitive=False)
...
class ScoringSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="ROAMPAL_SCORING_", extra="ignore", case_sensitive=False)
...
class ChromaDBSettings(BaseModel):
    ...
    persistence_directory: str = Field(
        "backend/data/chromadb_server",
        description="ChromaDB persistence directory for server mode",
    )
...
class MemoryInjectorSettings(BaseModel):
    ...
    max_items_per_type: Dict[str, int] = Field(
        {"user": 3, "assistant": 3, "book_quote": 2, "summary": 2},
...
def load_additional_fragment_configs(
    settings_instance: Settings, config_file_name: str = "fragments_memory_config.json"
):
            ...
            with open(config_file_path, "r", encoding="utf-8") as f:
                ...
                for frag_id, config_data in fragment_configs_dict.items():
                    if "base_data_path" in config_data:
                        ...
                        logger.info(
                            f"Loaded memory config for fragment: '{frag_id}' from {config_file_path} using base_data_path: {OG_DATA_PATH}"
                        )
                    else:
                        logger.warning(
                            f"Skipping fragment config for '{frag_id}' due to missing 'base_data_path' in {config_file_path}"
                        )
        except Exception as e:
            logger.error(
                f"Error loading fragment memory configurations from {config_file_path}: {e}",
                exc_info=True,
            )
    else:
        logger.info(
            f"Fragment memory configuration file '{config_file_name}' not found at {config_file_path}. No additional fragments loaded by this function."
        )
