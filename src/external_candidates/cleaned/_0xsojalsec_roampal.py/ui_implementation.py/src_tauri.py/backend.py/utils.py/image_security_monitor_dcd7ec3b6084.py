# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\image_security_monitor.py
"""
...
class ImageSecurityMonitor:
    ...
    def log_security_violation(
        ...
        # Also log as WARNING
        logger.warning(
            f"SECURITY_VIOLATION: {violation_type} from {user_id}@{ip_address} on shard {shard_id}: {details}"
        )
    ...
    def log_chat_integration(
        ...
        event = ImageSecurityEvent(
            ...
            success=success,
            processing_time_ms=processing_time_ms,
            details={"session_id": session_id, "has_image_context": has_image_context},
        )
        self.log_event(event)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_events": len([e for e in self.events_log if (datetime.utcnow() - e.timestamp).seconds < 3600]),
