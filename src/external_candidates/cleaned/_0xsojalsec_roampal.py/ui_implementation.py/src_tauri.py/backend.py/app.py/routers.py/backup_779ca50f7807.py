...
async def estimate_export_size(
    ...
    try:
        ...
        if "memory" in include_types:
            chromadb_dir = Path(DATA_PATH) / "chromadb"
            if chromadb_dir.exists():
                size_bytes = sum(f.stat().st_size for f in chromadb_dir.rglob("*") if f.is_file())
        ...
        if "books" in include_types:
            books_dir = Path(DATA_PATH) / "books"
            if books_dir.exists():
                size_bytes = sum(f.stat().st_size for f in books_dir.rglob("*") if f.is_file())
        ...
        return ExportSizeEstimate(total_mb=total_mb, breakdown=breakdown, file_counts=file_counts)
