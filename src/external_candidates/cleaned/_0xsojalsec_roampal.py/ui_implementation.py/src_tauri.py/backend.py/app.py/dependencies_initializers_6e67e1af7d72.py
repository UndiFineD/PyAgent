    ...
    for filename in os.listdir(og_books_dir):
        if filename.endswith(".txt"):
            ...
            keywords = [title.lower()] + (
                ["coding"]
                if any(kw in title.lower() for kw in ["sicp", "programming", "mit", "cleanarchitecture"])
                else ["philosophy"]
            )
...
async def initialize_fragment_memory_adapter(
    ...
    if adapter_type == "chromadb":
        ...
        collection_name = (
            getattr(fragment_memory_config_obj, "collection_name", None) or f"roampal_{fragment_id}_soul_fragments"
        )
