# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\model_contexts.py
"""
...
async def get_model_context(model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "model": model_name, **info}
    except Exception as e:
...
async def set_model_context(model_name: str, request: ContextUpdateRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail="Failed to save context override")
...
async def reset_model_context(model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail="Failed to reset context override")
