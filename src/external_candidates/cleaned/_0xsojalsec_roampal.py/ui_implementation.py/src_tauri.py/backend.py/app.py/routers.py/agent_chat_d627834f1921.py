...
def _format_search_results_as_citations(search_results: List[Dict], max_citations: int = 10) -> List[Dict]:
    """
...
class AgentChatService:
        ...
        try:
            ...
            if current_message == 1:
                ...
                try:
                    ...
                    context_summary = await asyncio.wait_for(self.memory.get_cold_start_context(limit=5), timeout=25.0)
            ...
            try:
                ...
                system_prompts = [
                    m["content"] for m in self.conversation_histories[conversation_id] if m["role"] == "system"
                ]
                ...
                if has_content_insights or action_stats or relevant_facts:
                    ...
                    self.conversation_histories[conversation_id].append({"role": "system", "content": guidance_msg})
                    logger.info(
                ...
                try:
                    ...
                    if prev_assistant:
                        ...
                        if outcome_result.get("outcome") in [
                            ...
                            await self.memory.record_outcome(doc_id=prev_assistant["doc_id"], outcome=outcome)
            ...
            memory_tool_used = any(
                event.get("tool") in ["create_memory", "update_memory", "archive_memory"] for event in tool_events
            )
            yield {
    ...
    def _build_openai_prompt(self, message: str, conversation_history: List[Dict[str, str]] = None) -> str:
        """
    ...
    def _build_complete_prompt(self, message: str, conversation_history: List[Dict[str, str]] = None) -> str:
        """
    ...
    async def _execute_tool_and_continue(
        ...
        if tool_name == "search_memory":
            ...
            if tool_results:
                ...
                tool_response_content = (
                    "No relevant memories found for this query. I'll answer based on my general knowledge."
                )
            ...
            if old_content.strip() and new_content.strip() and self.memory:
                ...
                if results:
                    ...
                    await self.memory.update_memory_bank(doc_id=doc_id, new_text=new_content, reason="llm_update")
                ...
                try:
                    ...
                    if entry["action"] == "create":
                        ...
                        if results:
                            ...
                            await self.memory.archive_memory_bank(doc_id=doc_id, reason="llm_decision")
    ...
    try:
        ...
        async for event in agent_service.stream_message(
            ...
            if not websocket_disconnected and (
                not websocket or (hasattr(websocket, "client_state") and websocket.client_state.name != "CONNECTED")
            ):
            ...
            if event["type"] == "token":
                ...
                full_response.append(event["content"])
                if not websocket_disconnected:
                    await websocket.send_json({"type": "response", "content": event["content"]})
                ...
                if not websocket_disconnected:
                    if "content" in event:
                        ...
                        await websocket.send_json({"type": "validation_error", "message": event["content"]})
                    else:
        ...
        if websocket:
            try:
                await websocket.send_json({"type": "cancelled", "message": "Generation cancelled by user"})
            except:
        ...
        if websocket:
            await websocket.send_json({"type": "error", "message": "Generation timed out", "code": "TIMEOUT"})
        # Fall back to batch
        ...
        if websocket:
            await websocket.send_json({"type": "error", "message": str(e), "code": "STREAM_ERROR"})
        # Fall back to batch
        await _run_generation_task(conversation_id, request, user_id, app_state)
...
async def agent_chat_stream(request: AgentChatRequest, req: Request):
    ...
    if not agent_service:
        agent_service = AgentChatService(memory=req.app.state.memory, llm=req.app.state.llm_client)

    else:
        agent_service.llm = req.app.state.llm_client

    conversation_id = (
        request.conversation_id or f"conv_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{secrets.token_hex(8)}"
    )
    ...
    if use_streaming and has_websockets and has_this_websocket:
        ...
        task = asyncio.create_task(_run_generation_task_streaming(conversation_id, request, user_id, req.app.state))
    else:
        logger.info(
        ...
        task = asyncio.create_task(_run_generation_task(conversation_id, request, user_id, req.app.state))
