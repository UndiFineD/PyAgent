...
async def get_system_health(request: Request) -> SystemHealthResponse:
    ...
    try:
        ...
        if chromadb_dir.exists():
            chromadb_size = sum(f.stat().st_size for f in chromadb_dir.rglob("*") if f.is_file()) / (1024**2)  # MB
        ...
        if backups_dir.exists():
            ...
            backup_size = sum(f.stat().st_size for f in backups_dir.glob("*.zip")) / (1024**2)  # MB
