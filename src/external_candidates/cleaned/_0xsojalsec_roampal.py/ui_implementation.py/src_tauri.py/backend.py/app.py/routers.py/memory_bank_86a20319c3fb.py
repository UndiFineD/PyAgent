...
async def get_archived_memories(
    ...
    try:
        results = await memory.search_memory_bank(query=None, tags=None, include_archived=True, limit=limit * 2)
...
@router.get("/search")
async def search_memories(request: Request, q: str, tags: Optional[str] = None, limit: int = 20):
    """
    ...
    try:
        ...
        results = await memory.search_memory_bank(query=q, tags=tag_list, include_archived=False, limit=limit)
...
async def get_memory_stats(request: Request):
    ...
    try:
        # Get all memories
        all_memories = await memory.search_memory_bank(query=None, include_archived=True, limit=1000)
...
async def update_memory(request: Request, doc_id: str, memory_data: MemoryUpdate):
    ...
    try:
        updated_id = await memory.update_memory_bank(doc_id=doc_id, new_text=memory_data.text, reason="api_update")
