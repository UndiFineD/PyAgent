# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\services\session_manager.py
"""
...
class SecureSessionManager:
    ...
    def create_secure_session(self, user_id: str, shard_id: str, metadata: Optional[Dict] = None) -> Tuple[str, str]:
        """
        ...
        logger.info(f"Secure session created: User={user_id}, Shard={shard_id}, SessionID={session_id}")

        return session_id, session_token

    def validate_session(
        self, session_token: str, user_id: str, shard_id: str
        ...
        try:
            ...
            if session_data["user_id"] != user_id:
                self._stats["validation_failures"] += 1
                logger.warning(f"Session user mismatch: Expected {user_id}, got {session_data['user_id']}")
                return False, "Session does not belong to this user", None

            if session_data["shard_id"] != shard_id:
                self._stats["validation_failures"] += 1
                logger.warning(f"Session shard mismatch: Expected {shard_id}, got {session_data['shard_id']}")
    ...
    def validate_session_ownership(self, session_id: str, user_id: str, shard_id: str) -> bool:
        """
        ...
        if session_id in self._sessions:
            ...
            return (
                ...
                and session_data["shard_id"] == shard_id
                and session_data.get("valid", False)
            )
    ...
    def invalidate_session(self, session_id: str):
        ...
        if session_id in self._sessions:
            ...
            if user_id in self._user_sessions:
                self._user_sessions[user_id] = [sid for sid in self._user_sessions[user_id] if sid != session_id]
