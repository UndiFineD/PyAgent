# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\services\enhanced_web_search_service.py
...
from config.enhanced_web_search_config import (
    get_enhanced_web_search_config,
    get_shard_web_search_config,
)
...
class EnhancedWebSearchService:
    ...
    async def _initialize_shard_adapters(self):
        ...
        try:
            ...
            for shard_id in shard_ids:
                ...
                self.chromadb_adapters[shard_id] = ChromaDBAdapter(persistence_directory=str(vector_db_dir))
    ...
    async def search(
        ...
        try:
            ...
            # Perform the search
            logger.info(
                f"Calling enhanced_scraper.search with query: '{query}', num_results: {num_results}, shard_id: {shard_id}"
            )
            try:
                search_results = await self.enhanced_scraper.search(
                    query=query, num_results=num_results, shard_id=shard_id
                )
            ...
            filtered_results = self._filter_results_by_relevance(search_results, shard_config.relevance_threshold)
    ...
    async def scrape_url(self, url: str, shard_id: str = "og") -> Dict[str, Any]:
        """
    ...
    async def search_with_memory_context(
        ...
        try:
            ...
            # Perform search
            search_response = await self.search(query=enhanced_query, shard_id=shard_id)
    ...
    async def get_search_history(self, shard_id: str = "og", limit: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            results = await adapter.query_vectors(query_vector=None, top_k=limit, filters={"source": "web_search"})
