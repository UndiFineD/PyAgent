# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\scoring_service.py
"""
Scoring Service
...
class ScoringService:
    ...
    def calculate_wilson_score(self, successes: float, total: int, confidence: Optional[float] = None) -> float:
        """
    ...
    def calculate_learned_score(self, raw_score: float, uses: int, outcome_history: str = "") -> Tuple[float, float]:
        """
    ...
    def get_dynamic_weights(
        ...
        if uses >= 5 and learned_score >= 0.8:
            ...
            return (
                self.config.embedding_weight_proven,
                self.config.learned_weight_proven,
            )
    ...
    def calculate_final_score(self, metadata: Dict[str, Any], distance: float, collection: str) -> Dict[str, float]:
        """
        ...
        learned_score, wilson_score = self.calculate_learned_score(raw_score, uses, outcome_history)
    ...
    def apply_scoring_to_results(self, results: list, sort: bool = True) -> list:
        """
