# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\context_service.py
"""
...
class ContextService:
    ...
    async def analyze_conversation_context(
        ...
        try:
            ...
            # 3. Topic Continuity
            continuity = self._detect_topic_continuity(current_concepts, recent_conversation)
            ...
            if self.embed_fn and "working" in self.collections:
                repetitions = await self._detect_repetition(current_message, conversation_id)
    ...
    async def _find_relevant_patterns(self, concepts: List[str]) -> List[Dict[str, Any]]:
    ...
    def _check_failure_patterns(self, concepts: List[str]) -> List[Dict[str, Any]]:
    ...
    def _detect_topic_continuity(
        self, current_concepts: List[str], recent_conversation: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _get_proactive_insights(self, concepts: List[str]) -> List[Dict[str, Any]]:
    ...
    async def _detect_repetition(self, current_message: str, conversation_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            working_items = await self.collections["working"].query_vectors(query_vector=query_vector, top_k=3)
