# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\outcome_service.py
"""
...
class OutcomeService:
    ...
    async def record_outcome(
        ...
        if doc_id.startswith("books_"):
            logger.info(
                f"[KG] Learned routing pattern for books, but skipping score update (static reference material)"
            )
            return None
        ...
        if doc_id.startswith("memory_bank_"):
            logger.info(
                f"[KG] Learned routing pattern for memory_bank, but skipping score update (persistent user facts)"
            )
            return None
        ...
        score_delta, new_score, uses = self._calculate_score_update(outcome, current_score, uses, time_weight)
        ...
        if outcome == "worked" and context:
            ...
            reasons.append({"reason": failure_reason, "timestamp": datetime.now().isoformat()})
    ...
    def _calculate_score_update(self, outcome: str, current_score: float, uses: int, time_weight: float) -> tuple:
        """
    ...
    async def _update_kg_with_outcome(
        ...
        if outcome == "worked" and problem_text and solution_text:
            ...
            self.kg_service.add_solution_pattern(doc_id, solution_text, new_score, [problem_key], solution_concepts[:5])
            ...
            if failure_reason:
                self.kg_service.add_failure_pattern(failure_reason[:50], doc_id, problem_text[:100])
