...
class SearchService:
    ...
    async def search(
        ...
        known_solutions = await self.kg_service.find_known_solutions(query, self._get_fragment)
    ...
    async def _rerank_with_cross_encoder(self, query: str, candidates: List[Dict], top_k: int) -> List[Dict]:
        """
        ...
        try:
            ...
            top_candidates = sorted(candidates, key=lambda x: x.get("final_rank_score", 0.0), reverse=True)[:30]
    ...
    def _track_search_results(self, query: str, results: List[Dict], transparency_context: Optional[Any]):
