# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\file_memory_adapter.py
import datetime
import json
import logging
import os
...
import aiofiles
from config.settings import DATA_PATH, settings  # Import settings
...
class FileMemoryAdapter(MemoryAdapterInterface):
    ...
    async def remove_goal(self, session_id: str, goal_to_remove: str) -> bool:
        return False

    async def add_value(self, session_id: str, value: str) -> None:
        pass

    async def get_values(self, session_id: str) -> List[str]:
        return []

    async def remove_value(self, session_id: str, value_to_remove: str) -> bool:
        return False

    async def delete_arbitrary_data(self, key: str) -> bool:
        return False

    def _get_learnings_path(self) -> Path:
        raise NotImplementedError

    async def add_learning(self, learning_content: Union[str, Dict[str, Any]], **kwargs) -> Dict[str, Any]:
        raise NotImplementedError
