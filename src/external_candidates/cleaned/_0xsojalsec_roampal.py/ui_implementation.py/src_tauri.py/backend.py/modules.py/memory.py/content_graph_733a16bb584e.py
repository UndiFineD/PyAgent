# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\content_graph.py
"""
...
class ContentGraph:
    ...
    def add_entities_from_text(
        ...
        logger.info(
            f"Indexed {len(entities)} entities from doc {doc_id}: {entities[:5]}{'...' if len(entities) > 5 else ''}"
        )
    ...
    def remove_entity_mention(self, doc_id: str):
        ...
        logger.info(
            f"Removed doc {doc_id} from content graph ({len(entities_in_doc)} entities, {len(rels_to_remove)} relationships)"
        )
    ...
    def find_path(self, from_entity: str, to_entity: str, max_depth: int = 3) -> Optional[List[str]]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_mentions_per_entity": (
                round(
                    sum(e["mentions"] for e in self.entities.values()) / len(self.entities),
                    2,
                )
                if self.entities
                else 0.0
            ...
            "avg_relationships_per_entity": (
                round((len(self.relationships) * 2) / len(self.entities), 2) if self.entities else 0.0
            ),
