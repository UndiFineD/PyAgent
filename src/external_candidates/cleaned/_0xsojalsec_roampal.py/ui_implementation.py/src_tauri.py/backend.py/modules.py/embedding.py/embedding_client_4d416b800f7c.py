# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\embedding\embedding_client.py
"""
...
class EmbeddingClient:
    ...
    async def initialize(self):
        ...
        connector = aiohttp.TCPConnector(limit=100, limit_per_host=30, keepalive_timeout=600)
    ...
    async def _check_service_health(self) -> bool:
        ...
        try:
            async with self.session.get(
                f"{self.service_url}/health", timeout=aiohttp.ClientTimeout(total=5)
            ) as response:
    ...
    async def clear_cache(self) -> bool:
        ...
        try:
            async with self.session.delete(
                f"{self.service_url}/cache", timeout=aiohttp.ClientTimeout(total=5)
            ) as response:
