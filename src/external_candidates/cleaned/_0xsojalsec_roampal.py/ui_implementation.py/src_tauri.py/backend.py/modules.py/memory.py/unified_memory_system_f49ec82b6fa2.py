# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\unified_memory_system.py
"""
...
from .types import ActionOutcome, CollectionName, MemoryResult, SearchMetadata
...
class UnifiedMemorySystem:
    ...
    def _init_services(self):
        ...
        # Routing service
        self._routing_service = RoutingService(kg_service=self._kg_service, config=self.config)
    ...
    async def _generate_contextual_prefix(self, text: str, metadata: Optional[Dict[str, Any]], collection: str) -> str:
        """
...
Prefix (one sentence, max 20 words):"""
            ...
            try:
                response = await asyncio.wait_for(self.llm_service.generate(prompt, max_tokens=50), timeout=5.0)
    ...
    async def store(
        ...
        # Store in collection
        await self.collections[collection].upsert_vectors(ids=[doc_id], vectors=[embedding], metadatas=[final_metadata])
    ...
    async def store_memory_bank(
        ...
        return await self._memory_bank_service.store(text=text, tags=tags, importance=importance, confidence=confidence)

    async def update_memory_bank(self, doc_id: str, new_text: str, reason: str = "llm_update") -> str:
        """Update memory_bank item."""
        if not self.initialized:
            await self.initialize()

        return await self._memory_bank_service.update(doc_id=doc_id, new_text=new_text, reason=reason)

    async def archive_memory_bank(self, doc_id: str, reason: str = "llm_decision") -> bool:
...
Session type (1-2 words only):"""
        ...
        try:
            response = await asyncio.wait_for(self.llm_service.generate(prompt, max_tokens=10), timeout=3.0)
    ...
    def get_action_effectiveness(
        self, context_type: str, action_type: str, collection: Optional[str] = None
    ...
    async def get_cold_start_context(self, limit: int = 5) -> Optional[str]:
        ...
        try:
            history_results = await self.search(query="recent conversation context", collections=["history"], limit=1)
    ...
    def _format_cold_start_results(self, results: List[Dict]) -> Optional[str]:
        ...
        safe_results = [
            r
            for r in results
            if not any(x in (r.get("content") or "").lower() for x in ["ignore all previous", "ignore instructions"])
        ][:10]
        ...
        if memory_bank_items:
            sections.append(
                "[User Profile]:\n" + "\n".join([f"- {(r.get('content') or '')[:250]}" for r in memory_bank_items])
            )
        if pattern_items:
            sections.append(
                "[Proven Patterns]:\n" + "\n".join([f"- {(r.get('content') or '')[:200]}" for r in pattern_items])
            )
        if history_items:
            sections.append(
                "[Recent Context]:\n" + "\n".join([f"- {(r.get('content') or '')[:200]}" for r in history_items])
            )
    ...
    async def search_books(
        ...
        if title_filter:
            results = [r for r in results if title_filter.lower() in r.get("metadata", {}).get("title", "").lower()]

        return results[:limit]

    async def save_conversation_turn(self, role: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        return await self.store(text=content, collection="working", metadata=turn_metadata)
