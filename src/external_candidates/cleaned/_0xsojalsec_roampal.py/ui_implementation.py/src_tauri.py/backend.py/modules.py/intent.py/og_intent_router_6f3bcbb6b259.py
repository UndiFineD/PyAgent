...
class OGIntentRouter(IntentRouterInterface):
            ...
            if matched_kw:
                ...
                book = next(
                    (
                        ...
                        for b in BOOKS
                        if any(
                            kw in [b["title"].lower(), b["author"].lower()] + [k.lower() for k in b["keywords"]]
        ...
        if not inp.startswith("Based on this image content:"):
            if (
                ...
                or (
                    ...
                    and len([x for x in ["1.", "2.", "3.", "4.", "5."] if x in inp]) >= 2
                )
                or (inp.count("?") >= 2)
            ):  # Multiple questions
    ...
    async def get_available_commands(self) -> List[str]:
        ...
        if ActionType.SAVE_LEARNING in self.keywords:
            cmds.append('/save_learning "Your insight here" --topic "optional" --keywords "k1,k2"')
        cmds.append("/og help")
        return cmds
