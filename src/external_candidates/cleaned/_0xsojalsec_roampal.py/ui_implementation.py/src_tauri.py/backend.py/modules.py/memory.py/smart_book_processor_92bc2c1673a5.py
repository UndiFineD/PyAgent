...
class SmartBookProcessor:
    ...
    async def check_duplicate(self, title: str) -> Optional[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(str(self.db_path)) as db:
            ...
            async with db.execute("SELECT book_id, title, author FROM books WHERE title = ?", (title,)) as cursor:
    ...
    async def _store_embeddings_with_metadata(
        ...
        try:
            ...
            for batch_start in range(0, len(chunks_with_metadata), batch_size):
                ...
                embedding_tasks = [self.embedding_service.embed_text(chunk_text) for chunk_text in batch_texts]
                ...
                await self.chromadb_adapter.upsert_vectors(ids=chunk_ids, vectors=embeddings, metadatas=metadatas)
        ...
        try:
            ...
            for book_id, upload_timestamp in books:
                ...
                try:
                    ...
                    results = collection.get(where={"book_id": book_id}, include=["metadatas"])
    ...
    async def _send_progress(self, task_id: str, status: str, message: str, progress: int, **kwargs):
        ...
        try:
            ...
            await send_progress_update(task_id, status=status, message=message, progress=progress, **kwargs)
        except Exception as e:
    ...
    async def get_surrounding_chunks(self, chunk_id: str, radius: int = 2) -> Dict[str, Any]:
        """
        ...
        async with aiosqlite.connect(str(self.db_path)) as db:
            ...
            async with db.execute(sql, (book_id, start_idx, end_idx)) as cursor:
                async for row in cursor:
                    results.append({"content": row["content"], "chunk_index": row["chunk_index"]})
