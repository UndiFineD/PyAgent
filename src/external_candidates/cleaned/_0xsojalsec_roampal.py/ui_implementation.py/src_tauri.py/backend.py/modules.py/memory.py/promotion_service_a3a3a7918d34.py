...
class PromotionService:
    ...
    async def _demote_patterns_to_history(self, doc_id: str, metadata: Dict[str, Any], score: float) -> Optional[str]:
    ...
    async def _handle_deletion(self, doc_id: str, collection: str, metadata: Dict[str, Any], score: float):
        ...
        deletion_threshold = (
            self.config.deletion_score_threshold if age_days > 7 else self.config.new_item_deletion_threshold
        )
    ...
    async def promote_valuable_working_memory(self, conversation_id: Optional[str] = None) -> int:
        """
