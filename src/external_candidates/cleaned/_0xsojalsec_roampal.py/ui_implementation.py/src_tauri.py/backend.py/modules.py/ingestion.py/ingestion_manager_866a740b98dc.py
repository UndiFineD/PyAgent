...
class IngestionManager(IngestionManagerInterface):
    ...
    async def submit_job_request(self, job_request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        job = IngestionJob(
            ...
            is_foundational=(
                job_create_model.is_foundational if job_create_model.is_foundational is not None else False
            ),
            ...
            force_flags=(
                job_create_model.force_flags if job_create_model.force_flags is not None else IngestionJobForceFlags()
            ),
        )
    ...
    async def process_single_job(self, job_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if job.source_type != "file":
            job.status = "failed"
            job.error_message = f"Source type '{job.source_type}' not yet supported for download."
            PROCESSED_JOBS[job.job_id] = job
            return job.model_dump()
        ...
        try:
            ...
            if current_meta and (
                ...
                if (
                    current_meta.get("summary_status") in ["fully_summarized", "partially_summarized", "chunked"]
                    or job.force_flags.force_regenerate_full_summary
                ):
            ...
            current_meta = await processor._get_book_metadata(book_id)
            if current_meta and (
                job.force_flags.force_reextract_content or current_meta.get("models_status") != "extracted"
            ):
            ...
            current_meta = await processor._get_book_metadata(book_id)
            if current_meta and (
                job.force_flags.force_reextract_content or current_meta.get("quotes_status") != "extracted"
            ):
