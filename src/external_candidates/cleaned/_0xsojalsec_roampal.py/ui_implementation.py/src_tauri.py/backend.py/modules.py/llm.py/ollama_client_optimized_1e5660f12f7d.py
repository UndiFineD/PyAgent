# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\llm\ollama_client_optimized.py
...
class OllamaClientOptimized(LLMClientInterface):
    ...
    async def generate_response(
        ...
        logger.critical(
            f"[MODEL DEBUG] LLM Client - Requested model: {model}, Default model: {self.model_name}, Using: {actual_model}"
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.request_count % 100 == 0:
                    ...
                    logger.info(
                        f"Ollama performance: {self.request_count} requests, avg latency: {avg_latency:.2f}s, errors: {self.error_count}"
                    )
                ...
                if e.response.status_code == 503 and attempt < max_retries - 1:
                    wait_time = 2**attempt
                    logger.warning(
                        f"Ollama service unavailable (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s..."
                    )
                    await asyncio.sleep(wait_time)
                else:
    ...
    async def close(self) -> None:
        ...
        if self.client:
            ...
            self.client = None
            logger.info(
                f"OllamaClientOptimized closed. Total requests: {self.request_count}, Avg latency: {self.total_latency / max(1, self.request_count):.2f}s"
            )
