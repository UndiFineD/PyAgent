...
class ChromaDBAdapter:
    ...
    async def hybrid_query(
        ...
        try:
            ...
            final_results = sorted(merged.values(), key=lambda x: x.get("rrf_score", 0.0), reverse=True)[:top_k]
    ...
    def update_fragment_metadata(self, fragment_id: str, metadata_updates: Dict[str, Any]):
        ...
        if frag.get("vector") is None:
            logger.warning(f"Skipping metadata update for fragment {fragment_id} because it has no associated vector.")
            return
        metadata = frag.get("metadata", {}) or {}
        metadata.update(metadata_updates)
        self.collection.upsert(ids=[fragment_id], embeddings=[frag.get("vector")], metadatas=[metadata])
    ...
    def update_fragment_score(self, fragment_id: str, new_score: float):
        ...
        if not frag:
            ...
            return
        if frag.get("vector") is None:
            logger.warning(f"Skipping score update for fragment {fragment_id} because it has no associated vector.")
            return
        metadata = frag.get("metadata", {}) or {}
    ...
    async def update_metadata(self, doc_id: str, metadata: Dict[str, Any]):
        ...
        try:
            ...
            if frag.get("vector") is None:
                logger.warning(f"Skipping metadata update for {doc_id} because it has no associated vector.")
                return
