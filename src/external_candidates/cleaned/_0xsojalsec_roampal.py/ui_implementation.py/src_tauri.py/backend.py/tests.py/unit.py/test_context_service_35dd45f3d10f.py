# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\unit\test_context_service.py
"""
...
class TestContextServiceInit:
    ...
    def test_init_with_services(self):
        ...
        service = ContextService(collections={}, kg_service=kg_mock, embed_fn=embed_mock)
...
class TestConceptExtraction:
    ...
    def test_basic_extraction(self, service):
        ...
        concepts = service._basic_concept_extraction("How do I configure Python logging?")
    ...
    def test_filters_stopwords(self, service):
        ...
        concepts = service._basic_concept_extraction("The quick brown fox is a test")
    ...
    def test_filters_short_words(self, service):
        ...
        concepts = service._basic_concept_extraction("I am a developer")
...
class TestPatternRecognition:
    ...
    def mock_kg_service(self):
        ...
        kg.get_problem_categories = MagicMock(return_value={"config_logging_python": ["patterns_doc123"]})
        return kg

    @pytest.fixture
    def mock_collections(self):
        patterns = MagicMock()
        patterns.get_fragment = MagicMock(
            return_value={
                "content": "Use logging.basicConfig() for simple setup",
                "metadata": {"score": 0.85, "uses": 5, "last_outcome": "worked"},
            }
        )
    ...
    def service(self, mock_collections, mock_kg_service):
        return ContextService(collections=mock_collections, kg_service=mock_kg_service)

    @pytest.mark.asyncio
    async def test_finds_relevant_patterns(self, service):
        """Should find patterns from past conversations."""
        patterns = await service._find_relevant_patterns(["python", "logging", "config"])
    ...
    async def test_filters_low_score_patterns(self, service, mock_collections):
        ...
        mock_collections["patterns"].get_fragment = MagicMock(
            return_value={
...
class TestFailureAwareness:
    ...
    def mock_kg_service(self):
        kg = MagicMock()
        kg.get_failure_patterns = MagicMock(
            return_value={
                "asyncio deadlock": [
...
class TestTopicContinuity:
    ...
    def test_detects_continuation(self, service):
        ...
        continuity = service._detect_topic_continuity(["logging", "debug", "level"], recent)
    ...
    def test_detects_topic_shift(self, service):
        ...
        continuity = service._detect_topic_continuity(["database", "sql", "connection"], recent)
...
class TestProactiveInsights:
    ...
    def mock_kg_service(self):
        kg = MagicMock()
        kg.get_routing_patterns = MagicMock(
            return_value={
                "python": {"success_rate": 0.85, "best_collection": "patterns"},
                "config": {
...
class TestRepetitionDetection:
    ...
    def mock_collections(self):
        working = MagicMock()
        working.query_vectors = AsyncMock(
            return_value=[
                {
    ...
    def service(self, mock_collections):
        return ContextService(collections=mock_collections, embed_fn=AsyncMock(return_value=[0.1] * 384))

    @pytest.mark.asyncio
    async def test_detects_repetition(self, service):
        """Should detect similar recent messages."""
        repetitions = await service._detect_repetition("How can I setup logging?", "conv123")
    ...
    async def test_ignores_different_conversation(self, service, mock_collections):
        ...
        mock_collections["working"].query_vectors = AsyncMock(
            return_value=[
                {
        ...
        repetitions = await service._detect_repetition("Similar message", "conv123")
...
class TestAnalyzeConversationContext:
    ...
    async def test_returns_context_structure(self, service):
        ...
        context = await service.analyze_conversation_context("test message", [], "conv123")
...
class TestFindKnownSolutions:
    ...
    def mock_kg_service(self):
        ...
        kg.get_problem_solutions = MagicMock(
            return_value={
                "logging_python": [
                    {
                        "doc_id": "patterns_123",
