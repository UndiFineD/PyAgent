...
async def call_search_memory(
    ...
    # Call search
    results = await memory.search(query=query, collections=collections, limit=limit, metadata_filters=metadata)
...
async def call_get_context_insights(
    ...
    # Detect context type
    context_type = await memory.detect_context_type(system_prompts=[], recent_messages=[])
...
async def call_record_response(memory, arguments: Dict[str, Any], session_id: str = "default") -> str:
    """
