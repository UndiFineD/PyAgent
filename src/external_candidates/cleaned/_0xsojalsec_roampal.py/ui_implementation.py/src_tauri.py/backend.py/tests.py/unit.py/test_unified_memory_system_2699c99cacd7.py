# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\unit\test_unified_memory_system.py
"""
...
class TestUnifiedMemorySystemInit:
    ...
    def test_init_with_custom_config(self, tmp_path):
        ...
        ums = UnifiedMemorySystem(data_dir=str(tmp_path / "data"), config=config)
...
class TestStore:
    ...
    async def test_store_with_metadata(self, mock_ums):
        ...
        await mock_ums.store("test text", metadata={"custom": "value"})
...
class TestSearch:
    ...
    def mock_ums(self, tmp_path):
        ...
        ums._search_service.search = AsyncMock(
            return_value=[
    ...
    async def test_search_passes_collections(self, mock_ums):
        ...
        await mock_ums.search("test query", collections=["patterns", "history"])
...
class TestRecordOutcome:
    ...
    async def test_record_outcome_delegates(self, mock_ums):
        ...
        await mock_ums.record_outcome(doc_id="working_123", outcome="worked")
    ...
    async def test_record_outcome_with_reason(self, mock_ums):
        ...
        await mock_ums.record_outcome(doc_id="working_123", outcome="failed", failure_reason="Test failure")
...
class TestMemoryBankAPI:
    ...
    async def test_store_memory_bank(self, mock_ums):
        ...
        doc_id = await mock_ums.store_memory_bank(text="User prefers dark mode", tags=["preference"])
    ...
    async def test_update_memory_bank(self, mock_ums):
        ...
        await mock_ums.update_memory_bank(doc_id="memory_bank_123", new_text="Updated text")
...
class TestContextAPI:
    ...
    def mock_ums(self, tmp_path):
        ...
        ums._context_service.analyze_conversation_context = AsyncMock(
            return_value={
                "relevant_patterns": [],
                "past_outcomes": [],
                "topic_continuity": [],
    ...
    async def test_analyze_context(self, mock_ums):
        ...
        context = await mock_ums.analyze_conversation_context(
            current_message="test", recent_conversation=[], conversation_id="conv123"
        )
...
class TestKGVisualization:
    ...
    def mock_ums(self, tmp_path):
        ...
        ums._kg_service.get_kg_entities = MagicMock(return_value=[{"id": "concept1", "weight": 10}])
        ums._kg_service.get_kg_relationships = MagicMock(return_value=[{"source": "c1", "target": "c2", "weight": 1}])
