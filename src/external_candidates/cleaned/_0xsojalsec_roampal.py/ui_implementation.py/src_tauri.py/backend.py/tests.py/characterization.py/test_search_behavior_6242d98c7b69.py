# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\characterization\test_search_behavior.py
r"""
...
class TestSearchBehavior:
    ...
    async def test_search_result_structure(self, memory_system):
        ...
        if results:  # Only check if we got results
            ...
            assert "text" in result or "text" in result.get("metadata", {}), "Missing text in result"
    ...
    async def test_search_with_collection_filter(self, memory_system):
        ...
        for coll in collections:
            results = await memory_system.search("test", collections=[coll], limit=3)
            for result in results:
                assert result.get("collection") == coll, f"Expected collection {coll}, got {result.get('collection')}"

    @pytest.mark.asyncio
    async def test_search_with_metadata(self, memory_system):
        """Search with return_metadata=True should include scoring details."""
        results = await memory_system.search("python programming", limit=3, return_metadata=True)
...
class TestWilsonScore:
    ...
    def test_wilson_score_perfect_record(self):
        ...
        assert proven_score > new_score, f"Proven ({proven_score}) should beat newcomer ({new_score})"
