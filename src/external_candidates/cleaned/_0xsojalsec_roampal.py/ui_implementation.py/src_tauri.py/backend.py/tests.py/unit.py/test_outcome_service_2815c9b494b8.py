# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\unit\test_outcome_service.py
"""
...
class TestOutcomeServiceInit:
    ...
    def test_init_with_services(self):
        ...
        service = OutcomeService(collections={}, kg_service=kg_mock, promotion_service=promo_mock)
...
class TestRecordOutcome:
    ...
    def mock_collections(self):
        ...
        working = MagicMock()
        working.get_fragment = MagicMock(
            return_value={
    ...
    async def test_record_worked_outcome(self, service, mock_collections):
        ...
        result = await service.record_outcome(doc_id="working_test123", outcome="worked")
    ...
    async def test_record_failed_outcome(self, service, mock_collections):
        ...
        result = await service.record_outcome(doc_id="working_test123", outcome="failed", failure_reason="Did not work")
    ...
    async def test_record_partial_outcome(self, service, mock_collections):
        ...
        result = await service.record_outcome(doc_id="working_test123", outcome="partial")
    ...
    async def test_safeguard_books(self, service):
        ...
        result = await service.record_outcome(doc_id="books_test123", outcome="worked")
        assert result is None

    @pytest.mark.asyncio
    async def test_safeguard_memory_bank(self, service):
        """Should not score memory bank items."""
        result = await service.record_outcome(doc_id="memory_bank_test123", outcome="worked")
        assert result is None
    ...
    async def test_not_found(self, service, mock_collections):
        ...
        result = await service.record_outcome(doc_id="working_nonexistent", outcome="worked")
        assert result is None
    ...
    async def test_failure_reason_tracking(self, service, mock_collections):
        ...
        result = await service.record_outcome(doc_id="working_test123", outcome="failed", failure_reason="Test failure")
    ...
    async def test_success_context_tracking(self, service, mock_collections):
        ...
        result = await service.record_outcome(doc_id="working_test123", outcome="worked", context={"topic": "test"})
...
class TestScoreCalculation:
    ...
    def test_worked_increases_score(self, service):
        ...
        delta, new_score, uses = service._calculate_score_update("worked", 0.5, 0, 1.0)
        assert delta > 0
    ...
    def test_failed_decreases_score(self, service):
        ...
        delta, new_score, uses = service._calculate_score_update("failed", 0.5, 0, 1.0)
        assert delta < 0
        assert new_score < 0.5
        assert uses == 0
    ...
    def test_partial_slightly_increases(self, service):
        ...
        delta, new_score, uses = service._calculate_score_update("partial", 0.5, 0, 1.0)
        assert delta > 0
    ...
    def test_score_capped_at_1(self, service):
        ...
        delta, new_score, uses = service._calculate_score_update("worked", 0.95, 0, 1.0)
        assert new_score <= 1.0

    def test_score_capped_at_0(self, service):
        """Score should not go below 0.0."""
        delta, new_score, uses = service._calculate_score_update("failed", 0.1, 0, 1.0)
...
class TestKGIntegration:
    ...
    def service(self, mock_collections, mock_kg_service):
        return OutcomeService(collections=mock_collections, kg_service=mock_kg_service)
    ...
    async def test_tracks_failure_patterns(self, service, mock_kg_service):
        ...
        await service.record_outcome("working_test123", "failed", failure_reason="Test failure")
...
class TestPromotionIntegration:
    ...
    def service(self, mock_collections, mock_promotion_service):
        return OutcomeService(collections=mock_collections, promotion_service=mock_promotion_service)
