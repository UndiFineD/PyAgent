...
class TestWilsonScoreLower:
    ...
    def test_monotonic_increase(self):
        ...
        for i in range(1, len(scores)):
            assert scores[i] >= scores[i - 1], f"Score should increase: {scores[i - 1]} -> {scores[i]}"
...
class TestScoringService:
    ...
    def test_dynamic_weights_memory_bank_high_quality(self, service):
        ...
        emb_w, learn_w = service.get_dynamic_weights(0, 0.5, "memory_bank", importance=0.95, confidence=0.9)
        assert emb_w == 0.45
        assert learn_w == 0.55

    def test_dynamic_weights_memory_bank_normal(self, service):
        """Normal memory_bank should be balanced."""
        emb_w, learn_w = service.get_dynamic_weights(0, 0.5, "memory_bank", importance=0.7, confidence=0.7)
        assert emb_w == 0.5
        assert learn_w == 0.5
