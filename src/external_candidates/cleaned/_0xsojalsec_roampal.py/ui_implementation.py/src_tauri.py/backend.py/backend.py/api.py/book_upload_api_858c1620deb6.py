# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\backend\api\book_upload_api.py
"""
...
from modules.memory.format_extractor import (
    ExtractionError,
    FormatDetector,
    detect_and_extract,
)
...
async def process_book(book_id: str, request: Request, background_tasks: BackgroundTasks):
...
async def process_book_task(book_id: str, file_path: str, metadata: Dict, task_id: str, request: Request):
    ...
    try:
        ...
        try:
            ...
            if extracted.is_tabular:
                logger.info(
                    f"[BOOK_PROCESSOR] Extracted tabular data: {extracted.row_count} rows, {extracted.column_count} cols"
                )
            if extracted.extraction_warnings:
...
async def cancel_task(task_id: str):
    ...
    try:
        if task_id in manager.tasks:
            ...
            manager.update_progress(task_id, {"status": "cancelled", "message": "Task cancelled by user"})
...
async def search_books(
    ...
    try:
        ...
        # Perform search
        results = await processor.search_full_text(query=query, book_title=book_title, limit=limit, offset=offset)
                    ...
                    try:
                        ...
                        if books_adapter:
                            ...
                            try:
                                ...
                                book_chunk_ids = [id for id in all_items["ids"] if id.startswith(f"{book_id}_chunk_")]
