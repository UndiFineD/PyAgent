...
async def _inject_cold_start_if_needed(session_id: str, tool_response: str, memory_system) -> str:
    ...
    if session_id not in _mcp_first_tool_call:
        ...
        try:
            context_summary = await asyncio.wait_for(memory_system.get_cold_start_context(limit=5), timeout=10.0)
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                app.state.memory = UnifiedMemorySystem(data_dir=DATA_PATH, use_server=use_chromadb_server)
        ...
        if not detected_providers:
            ...
            if selected_model:
                ...
                await app.state.llm_client.initialize({"ollama_base_url": base_url, "ollama_model": selected_model})
        ...
        agent_chat_module.agent_service = AgentChatService(memory=app.state.memory, llm=app.state.llm_client)
...
This closes the loop. Without it, the system can't learn what worked OR what didn't.""",
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        ...
        try:
            context_type = await memory.detect_context_type(system_prompts=[], recent_messages=recent_conv)
        ...
        try:
            if name == "search_memory":
                        ...
                        if timestamp:
                            try:
                                ...
                                if isinstance(timestamp, str):
                                    ...
                                    ts = timestamp
                                age_days = (
                                    (datetime.now(ts.tzinfo) if ts.tzinfo else datetime.now() - ts).days
                                    if hasattr(ts, "days")
                ...
                collections_used = (
                    list(result_collections) if result_collections else (collections if collections else ["all"])
                )
                ...
                response_text += (
                    f"Your takeaway stored in working memory (will promote to history if score stays ≥0.7)\n"
                )
                if outcome == "worked":
                    response_text += (
                        "Outcome 'worked': High initial score (0.7) - this learning is on track for promotion\n"
                    )
                elif outcome == "failed":
                    response_text += "Outcome 'failed': Low initial score (0.2) - consider what went wrong for future\n"
                elif outcome == "partial":
                    response_text += (
                        "Outcome 'partial': Medium score (0.55) - needs more positive outcomes to promote\n"
                    )
                else:
                ...
                return types.CallToolResult(content=[types.TextContent(type="text", text=response_text)])
