...
async def test_learning_curve():
    ...
    try:
        ...
        for i, (query, doc_id, collection, outcome) in enumerate(training_scenarios):
            # Record outcome
            await memory.record_outcome(doc_id=doc_id, outcome=outcome, context={"quiz_question": query})
...
async def test_pattern_recognition():
    ...
    try:
        ...
        for query in training_queries:
            await memory.record_outcome(doc_id=error_doc, outcome="worked", context={"quiz_question": query})
    ...
    try:
        ...
        for context_name, doc_id, expected_collection, queries in contexts:
            for query in queries:
                await memory.record_outcome(doc_id=doc_id, outcome="worked", context={"quiz_question": query})
