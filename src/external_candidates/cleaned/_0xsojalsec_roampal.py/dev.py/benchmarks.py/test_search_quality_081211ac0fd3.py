...
async def test_synonym_understanding():
    ...
    try:
        ...
        for fact, _ in facts:
            await memory.store_memory_bank(text=fact, tags=["synonym_test"], importance=0.8, confidence=0.8)
...
async def test_typo_tolerance():
    ...
    try:
        ...
        for typo_query, correct_word, description in typo_queries:
            results = await memory.search(query=typo_query, collections=["memory_bank"], limit=3)
...
async def test_result_diversity():
    ...
    try:
        ...
        for fact in pet_facts:
            await memory.store_memory_bank(text=fact, tags=["pet", "max"], importance=0.7, confidence=0.7)
            await asyncio.sleep(0.02)

        print(f"  [STORE] Stored {len(pet_facts)} facts about Max the dog")

        # Query for pet info
        results = await memory.search(query="Tell me about the user's pet", collections=["memory_bank"], limit=5)
