...
async def test_cross_collection_contamination():
    ...
    try:
        ...
        history_doc = await memory.store(text="History document", collection="history", metadata={"score": 0.5})
...
async def test_invalid_outcome_handling():
    ...
    try:
        ...
        try:
            await memory.record_outcome(doc_id="nonexistent_doc_12345", outcome="worked")
            # Should not crash
            tests_passed += 1
        ...
        doc_id = await memory.store(text="Test document", collection="working", metadata={"score": 0.5})
        try:
        ...
        books_doc = await memory.store(text="Book content", collection="books", metadata={"title": "Test Book"})
        try:
