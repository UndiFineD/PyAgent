# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_contradictions.py
"""
...
Each test uses an ISOLATED memory instance to prevent cross-contamination.
"""
...
async def create_fresh_memory():
    ...
    test_dir = f"test_data_contradictions_{uuid.uuid4().hex[:8]}"
...
async def test_many_wrong_vs_one_right():
    ...
    try:
        ...
        for wrong in wrong_answers:
            await memory.store_memory_bank(text=wrong, tags=["job", "uncertain"], importance=0.3, confidence=0.3)
...
async def test_temporal_update():
    ...
    try:
        ...
        results = await memory.search(query="Where does the user live?", collections=["memory_bank"], limit=5)
...
async def test_confidence_conflict():
    ...
    try:
        ...
        results = await memory.search(query="Is the user vegetarian?", collections=["memory_bank"], limit=5)
...
async def test_implicit_contradiction():
    ...
    try:
        ...
        results = await memory.search(query="Does the user like coffee?", collections=["memory_bank"], limit=5)
