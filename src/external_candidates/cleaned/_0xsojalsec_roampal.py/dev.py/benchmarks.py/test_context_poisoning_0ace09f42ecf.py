...
async def test_exact_duplicate_poisoning():
    ...
    try:
        ...
        await memory.store_memory_bank(text=base_text, tags=["personal"], importance=0.6, confidence=0.6)

        print("  [POISON] Stored same text 3 times: q=0.09, q=0.90, q=0.36")

        # Query and check results
        results = await memory.search(query="When is the user's birthday?", collections=["memory_bank"], limit=5)
...
async def test_near_duplicate_confusion():
    ...
    try:
        ...
        for text, imp, conf in variants:
            await memory.store_memory_bank(text=text, tags=["preference"], importance=imp, confidence=conf)
...
async def test_entity_confusion():
    ...
    try:
        ...
        for fact in confusing_facts:
            await memory.store_memory_bank(text=fact, tags=["random", "unverified"], importance=0.3, confidence=0.3)
            await asyncio.sleep(0.02)
...
async def test_temporal_confusion():
    ...
    try:
        ...
        results = await memory.search(query="When did the user get married?", collections=["memory_bank"], limit=5)
