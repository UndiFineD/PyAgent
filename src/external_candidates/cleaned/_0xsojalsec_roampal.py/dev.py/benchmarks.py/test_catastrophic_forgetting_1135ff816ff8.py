...
async def test_unrelated_domain_retention():
    ...
    try:
        ...
        for fact in family_facts:
            doc_id = await memory.store_memory_bank(text=fact, tags=["family"], importance=0.9, confidence=0.9)
        ...
        for fact in cooking_facts:
            await memory.store_memory_bank(text=fact, tags=["cooking", "recipes"], importance=0.7, confidence=0.8)
        ...
        found_brother = any(
            "david" in r.get("text", "").lower() or "brother" in r.get("text", "").lower() for r in results_brother
        )
...
async def test_bulk_insert_survival():
    ...
    try:
        ...
        found_spouse = any(
            "spouse" in r.get("text", "").lower() or "555-1234" in r.get("text", "") for r in results_emergency
        )
...
async def test_quality_preservation():
    ...
    try:
        ...
        results = await memory.search(query="Did the user attend MIT?", collections=["memory_bank"], limit=10)
...
async def test_cross_domain_isolation():
    ...
    try:
        ...
        for domain, facts in domains.items():
            for fact in facts:
                await memory.store_memory_bank(text=fact, tags=[domain], importance=0.85, confidence=0.85)
                await asyncio.sleep(0.01)
        ...
        for domain, (query, keyword) in queries.items():
            results = await memory.search(query=query, collections=["memory_bank"], limit=3)
