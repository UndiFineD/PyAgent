...
async def test_semantic_confusion():
    ...
    # Initialize system
    memory = UnifiedMemorySystem(data_dir="test_data_semantic_confusion", use_server=False, llm_service=None)
    ...
    for test in test_cases:
        ...
        results = await memory.search(query=test["query"], collections=["memory_bank"], limit=5)
        ...
        for i, r in enumerate(results[:5]):
            ...
            quality = (
                ...
                if isinstance(importance, (int, float)) and isinstance(confidence, (int, float))
                else 0.5
            )
