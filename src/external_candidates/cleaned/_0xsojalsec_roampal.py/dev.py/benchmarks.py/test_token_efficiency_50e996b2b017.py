...
sys.path.insert(
    0,
    str(Path(__file__).parent.parent.parent / "ui-implementation" / "src-tauri" / "backend"),
)
...
async def run_control_topk(scenario: Dict, data_dir: str, embedding_service, k: int = 3) -> Dict:
    ...
    collection = client.create_collection(name=f"control_top{k}", metadata={"hnsw:space": "l2"})
...
async def run_treatment_roampal(scenario: Dict, data_dir: str, embedding_service) -> Dict:
    ...
    """
    system = UnifiedMemorySystem(data_dir=data_dir, use_server=False, llm_service=MockLLMService())
    ...
    for i, text in enumerate(
        ...
        await system.store(text=text, collection="working", metadata={"type": "padding"})
                ...
                "rag_baselines_comparison": {
                    key: {
                        ...
                        "multiplier_vs_roampal": (
                            baseline["tokens"] / agg_roampal["avg_tokens"] if agg_roampal["avg_tokens"] > 0 else None
                        ),
