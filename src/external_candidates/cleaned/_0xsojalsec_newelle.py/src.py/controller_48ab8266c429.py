...
class NewelleSettings:
    ...
    def compare_settings(self, new_settings) -> list[ReloadType]:
        ...
        if (
            ...
            or self.use_secondary_language_model != new_settings.use_secondary_language_model
            or self.secondary_language_model_settings != new_settings.secondary_language_model_settings
        ):
...
class HandlersManager:
    ...
    def cache_handlers(self):
        ...
        # Secondary LLMs
        for key in AVAILABLE_LLMS:
            self.handlers[(key, self.convert_constants(AVAILABLE_LLMS), True)] = self.get_object(
                AVAILABLE_LLMS, key, True
            )
