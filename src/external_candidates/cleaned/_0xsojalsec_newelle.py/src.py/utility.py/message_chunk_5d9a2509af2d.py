# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\utility\message_chunk.py
import re
    ...
    for chunk in merged_flat_chunks:
        ...
        if is_inline_constituent:
            ...
            if current_inline_sequence:
                ...
                is_mixed_or_multiple = len(current_inline_sequence) > 1 or any(
                    c.type == "latex_inline" for c in current_inline_sequence
                )
                ...
                if is_mixed_or_multiple:
                    grouped_chunks.append(
                        MessageChunk(
                            type="inline_chunks",
                            text="",
                            subchunks=current_inline_sequence,
                        )
                    )
                else:
