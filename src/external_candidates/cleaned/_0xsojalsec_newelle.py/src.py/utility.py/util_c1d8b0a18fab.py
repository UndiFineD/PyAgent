# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\utility\util.py
import time
...
def convert_history_openai(history: list, prompts: list, vision_support: bool = False):
    ...
    for message in history:
        if message["User"] == "Console":
            result.append({"role": "user", "content": "Console: " + message["Message"]})
        else:
...
def convert_history_newelle(openai_history: list, vision_support: bool = False):
    ...
    for message in openai_history:
        ...
        # Handle Console messages (role "user" with a "Console: " prefix)
            ...
            newelle_history.append({"User": "Console", "Message": content[len("Console: ") :]})
        # Regular text messages.
        else:
            if role == "user":
                newelle_history.append({"User": "User", "Message": content})
            elif role == "assistant":
                newelle_history.append({"User": "Assistant", "Message": content})
            else:
                # Fallback for any unexpected role.
                newelle_history.append({"User": role, "Message": content})
