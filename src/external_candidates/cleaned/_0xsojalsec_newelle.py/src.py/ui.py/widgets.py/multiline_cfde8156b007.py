# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\ui\widgets\multiline.py
...
class MultilineEntry(Gtk.Box):
    def __init__(self, enter_on_ctrl=False):
        ...
        key_controller.connect(
            ...
            lambda controller, keyval, keycode, state: (
                ...
                and (
                    not self.enter_on_ctrl
                    and not (state & (Gdk.ModifierType.SHIFT_MASK | Gdk.ModifierType.CONTROL_MASK))
                    or self.enter_on_ctrl
                    and (state & (Gdk.ModifierType.SHIFT_MASK | Gdk.ModifierType.CONTROL_MASK))
                )
        ...
        apply_css_to_widget(
            self.input_panel,
            ".multilineentry { background-color: rgba(0,0,0,0); font-size: 15px;}",
        )
    ...
    def get_text(self):
        return self.input_panel.get_buffer().get_text(
            self.input_panel.get_buffer().get_start_iter(),
            self.input_panel.get_buffer().get_end_iter(),
            False,
        )
