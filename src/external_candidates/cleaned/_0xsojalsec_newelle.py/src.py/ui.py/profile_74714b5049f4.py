...
class ProfileDialog(Adw.PreferencesDialog):
    def __init__(self, parent, profile_settings, profile_name=None):
        ...
        if not editing:
            ...
            warning = Gtk.Label(
                label=_("The settings of the current profile will be copied into the new one"),
                wrap=True,
            )
            g.add(warning)
            self.page.add(g)
    ...
    def on_create_clicked(self, button):
        ...
        copied_settings = [
            setting for setting in self.settings_switches if self.settings_switches[setting].get_active()
        ]
        if self.editing:
