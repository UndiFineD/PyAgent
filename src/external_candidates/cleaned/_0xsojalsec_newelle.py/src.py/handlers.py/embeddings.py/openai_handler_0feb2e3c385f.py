# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\embeddings\openai_handler.py
import numpy as np
...
class OpenAIEmbeddingHandler(EmbeddingHandler):
    ...
    def build_extra_settings(
        self,
        provider_name: str,
        has_api_key: bool,
        endpoint_change: bool,
        supports_automatic_models: bool,
        model_list_url: str | None,
        default_automatic_models: bool = False,
    ) -> list:
        ...
        endpoint_settings = [
            ExtraSettings.EntrySetting(
                "endpoint",
                _("API Endpoint"),
                _("API base url, change this to use different APIs"),
        ...
        models_settings = [
            ExtraSettings.EntrySetting(
                "model",
                _("Model"),
                _("Name of the Embedding Model to use"),
                self.models[0][0],
            ),
        ]
    ...
    def get_embedding(self, text: list[str]) -> np.ndarray:
        ...
        embedding = client.embeddings.create(input=text, model=self.get_setting("model"))
        res = []
