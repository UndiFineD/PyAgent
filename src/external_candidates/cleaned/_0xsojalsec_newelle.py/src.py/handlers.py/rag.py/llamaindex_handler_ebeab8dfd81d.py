...
class LlamaIndexHanlder(RAGHandler):
        ...
        if self.get_setting("subdirectory_on", False, False):
            r += [
                ExtraSettings.ComboSetting(
                    ...
                    (self.get_subdirectories()[0][1] if len(self.get_subdirectories()) > 0 else "."),
                    update_settings=True,
                )
            ]
    ...
    def load(self):
        if self.index_exists() and (
            (self.index is None and self.loading_thread is None) or self.loaded_index != self.get_paths()[1]
        ):
    ...
    def install(self):
        if not find_module("llama_index") and not find_module("faiss"):
            dependencies = (
                "tiktoken faiss-cpu llama-index-core llama-index-readers-file llama-index-vector-stores-faiss"
            )
    ...
    def create_index(self, button=None):
        ...
        try:
            ...
            for document in documents[1:]:
                print(
                    "Indexing document " + str(i) + " of " + str(len(documents)) + ": " + document.metadata["file_name"]
                )
    ...
    def build_index(self, documents: list[str], chunk_size: int | None = None) -> RAGIndex:
        ...
        counter = TokenCountingHandler(tokenizer=tiktoken.encoding_for_model("gpt-4o").encode)
        self.llm.load_model(None)
        self.embedding.load_model()
        Settings.embed_model = self.get_embedding_adapter(self.embedding)
...
class LlamaIndexIndex(RAGIndex):
    ...
    def query(self, query: str) -> list[str]:
        ...
        if self.retriever is None:
            retriever = VectorIndexRetriever(index=self.index, similarity_top_k=int(self.return_documents))
            self.retriever = retriever
        r = []
