# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\memory\user_summary_handler.py
    ...
    def get_extra_settings(self) -> list:
        return [
            ...
            ExtraSettings.ScaleSetting(
                ...
                "How often to update the summary",
                5,
                1,
                10,
                0,
            ...
            ExtraSettings.MultilineEntrySetting(
                "user_summary",
                "User Summary",
                "Current summary of the interactions with the assistant",
                "",
            ),
            ExtraSettings.MultilineEntrySetting(
                "prompt",
                "Summary prompt",
                "Prompt to get the summary. {prompt} will be replaced with the user summary.",
                self.DEFAULT_PROMPT,
                refresh=self.restore_prompt,
                refresh_icon="star-filled-rounded-symbolic",
            ),
        ]
...
Only output the summary with no other details.
        ...
        if len(self.seen_messages) % update_frequency == 0:
            prompt = PROMPT.format(
                history="\n".join([i["User"] + ": " + i["Message"] for i in history[-update_frequency - 2 :]]),
                summary=self.get_setting("user_summary"),
            )
