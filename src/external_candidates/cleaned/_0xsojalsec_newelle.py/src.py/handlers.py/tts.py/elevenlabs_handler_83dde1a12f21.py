# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\tts\elevenlabs_handler.py
...
class ElevenLabs(TTSHandler):
    ...
    def save_audio(self, message, file):
        ...
        sett = VoiceSettings(
            stability=self.get_setting("stability"),
            similarity_boost=self.get_setting("similarity"),
            style=self.get_setting("style_exaggeration"),
        )
        audio = client.text_to_speech.convert(
            text=message,
            voice_id=self.get_setting("voice"),
            model_id=self.get_setting("model"),
