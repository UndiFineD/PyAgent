        ...
        if not (self.get_setting("system_prompt", False) is None or self.get_setting("system_prompt", False)):
            r += [
                ExtraSettings.ToggleSetting(
                    ...
                    _("Even if the model doesn't support system prompts, put the prompts on top of the user message"),
                    True,
                )
            ]
        r += [
            ExtraSettings.NestedSetting(
                ...
                [
                    ExtraSettings.ToggleSetting(
                        "thinking",
                        _("Enable Thinking"),
                        _("Show thinking, disable it if your model does not support it"),
                        True,
                    ),
                    ExtraSettings.ToggleSetting(
        ...
        for message in history:
            if message["User"] == "Console":
                result.append(
                    types.Content(
                        role="user",
                        parts=[types.Part.from_text(text="Console: " + message["Message"])],
                    )
                )
            else:
                    ...
                    types.Content(
                        ...
                        parts=(
                            ...
                            if img is None
                                ...
                                types.Part.from_text(text=text),
                                types.Part.from_uri(file_uri=img.uri, mime_type=img.mime_type),
                            ]
                        ),
                    )
                )
    ...
    def generate_text_stream(
        ...
        if not self.get_setting("advanced_params"):
            generate_content_config = GenerateContentConfig(
                ...
                response_modalities=(["text"] + ["image"] if self.get_setting("img_output") else ["text"]),
            )
        else:
            generate_content_config = GenerateContentConfig(
                system_instruction=(instructions if not self.get_setting("img_output") else None),
                safety_settings=safety,
                response_modalities=(["text"] + ["image"] if self.get_setting("img_output") else ["text"]),
        ...
        if self.get_setting("thinking"):
            generate_content_config.thinking_config = types.ThinkingConfig(include_thoughts=True)
