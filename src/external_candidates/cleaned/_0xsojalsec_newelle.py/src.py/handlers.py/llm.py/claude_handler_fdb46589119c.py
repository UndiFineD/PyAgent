# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\llm\claude_handler.py
import base64
import threading
...
class ClaudeHandler(LLMHandler):
    ...
    def get_models(self):
        if (
            ...
            or self.get_setting("api", False) == ""
            or self.get_setting("endpoint", False) is None
        ):
            return
        import anthropic
    ...
    def get_extra_settings(self) -> list:
        ...
        if self.get_setting("custom_model", False):
            settings.append(ExtraSettings.EntrySetting("model", _("Model"), _("The model to use"), ""))
        else:
            settings.append(
