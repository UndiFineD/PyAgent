# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\biguntroll.py\tesla_tessie.py\scripts.py\tessie_dd0ae61473ca.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\biguntroll\tesla-tessie\scripts\tessie.py
...
def main():
    ...
    try:
        if action == "vehicles":
            ...
            result = invoke_command(api_key, args.vin, "set_temperature", {"temperature": int(args.value)})
            ...
            result = invoke_command(api_key, args.vin, "set_charge_limit", {"percent": int(args.value)})
            ...
            print(
                "Available actions: vehicles, status, battery, location, wake, lock, unlock, "
                ...
                "open_frunk, open_trunk, fart, schedule_update, cancel_update",
                file=sys.stderr,
            )
