# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VolWeb\VolWeb\voltools.py
import datetime
import hashlib
import io
import logging
...
from volatility3.cli import MuteProgress, text_renderer
...
class DictRendererPsTree(text_renderer.CLIRenderer):
    ...
    _type_renderers = {
        ...
        datetime.datetime: lambda x: (
            x.isoformat() if not isinstance(x, interfaces.renderers.BaseAbsentValue) else None
        ),
        "default": lambda x: x,
    }
...
def build_context(evidence_data, context, base_config_path, plugin):
    ...
    """
    volatility3.symbols.__path__ = [os.path.abspath(f"media/{UPLOAD_PATH}")] + constants.SYMBOL_BASEPATHS
