# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-google\livekit\plugins\google\utils.py
        ...
        if is_raw_function_tool(fnc):
            ...
            if use_parameters_json_schema:
                ...
                fnc_kwargs["parameters"] = types.Schema.from_json_schema(
                    json_schema=types.JSONSchema.model_validate(info.raw_schema.get("parameters", {}))
                )
...
def create_tools_config(
    *,
    ...
    if len(tools) > 1:
        ...
        logger.warning("Multiple kinds of tools are not supported in Gemini. Only the first tool will be used.")
        tools = tools[:1]
...
def get_tool_results_for_realtime(
    ...
    return types.LiveClientToolResponse(function_responses=function_responses) if function_responses else None
