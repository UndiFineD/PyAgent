# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-browser\livekit\plugins\browser\proc_main.py
...
def main(mp_cch: socket.socket):
    ...
    with importlib.resources.as_file(res) as path:
        opts.framework_path = str(path / "Contents" / "Frameworks" / "Chromium Embedded Framework.framework")
        opts.main_bundle_path = str(path)
        opts.subprocess_path = str(
            path / "Contents" / "Frameworks" / "lkcef Helper.app" / "Contents" / "MacOS" / "lkcef Helper"
        )
