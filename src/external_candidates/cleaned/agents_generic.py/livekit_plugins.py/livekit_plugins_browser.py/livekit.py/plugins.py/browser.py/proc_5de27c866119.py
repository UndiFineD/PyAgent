# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-browser\livekit\plugins\browser\proc.py
...
class BrowserPage(utils.EventEmitter[EventTypes]):
    ...
    async def aclose(self) -> None:
        await ipc.channel.asend_message(self._ctx_duplex, proto.CloseBrowserRequest(page_id=self.id))
...
class BrowserContext:
    ...
    async def new_page(self, *, url: str, width: int = 800, height: int = 600, framerate: int = 30) -> BrowserPage:
        if not self._initialized:
            raise RuntimeError("BrowserContext not initialized")
