# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-deepgram\livekit\plugins\deepgram\_utils.py
import time
...
def _to_deepgram_url(opts: dict, base_url: str, *, websocket: bool) -> str:
    ...
    if opts.get("keywords"):
        ...
        opts["keywords"] = [f"{keyword}:{intensifier}" for (keyword, intensifier) in opts["keywords"]]
