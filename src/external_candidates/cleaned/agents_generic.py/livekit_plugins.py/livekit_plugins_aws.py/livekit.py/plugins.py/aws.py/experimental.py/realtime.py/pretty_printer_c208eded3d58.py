# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-aws\livekit\plugins\aws\experimental\realtime\pretty_printer.py
import json
import logging
...
def log_event_data(event_data: dict) -> None:
    ...
    logger.debug(f"{color}{event_type.upper()}: {json.dumps(event_data, indent=2)}{AnsiColors.ENDC}")
