# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-aws\livekit\plugins\aws\experimental\realtime\events.py
import json
import uuid
        ...
        event = Event(
            event=PromptStartEvent(
                promptStart=PromptStart(
                    promptName=self.prompt_name,
                    textOutputConfiguration=TextOutputConfiguration(),
                    audioOutputConfiguration=AudioOutputConfiguration(voiceId=voice_id, sampleRateHertz=sample_rate),
                    toolUseOutputConfiguration=ToolUseOutputConfiguration(),
