# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\test_tts.py
...
async def assert_valid_synthesized_audio(*, frames: AudioBuffer, text: str, sample_rate: int, num_channels: int):
...
async def _do_synthesis(tts_v: tts.TTS, segment: str, *, conn_options: APIConnectOptions):
    ...
    assert all(not event.is_final for event in audio_events[:-1]), "expected all audio events to be non-final"
    ...
    assert all(e.request_id == first_id for e in audio_events), "expected all frames to have the same request_id, "
...
async def test_tts_synthesize_timeout(tts_factory, toxiproxy: Toxiproxy):
    ...
    try:
        ...
        assert 1.5 <= elapsed_time <= 3.5, f"expected timeout around 2 seconds, got {elapsed_time:.2f}s"
        ...
        assert 1 <= elapsed_time <= 3, f"expected total timeout around 2 seconds, got {elapsed_time:.2f}s"
...
async def _do_stream(tts_v: tts.TTS, segments: list[str], *, conn_options: APIConnectOptions):
    async with tts_v.stream(conn_options=conn_options) as tts_stream:
        ...
        assert all(e.request_id == request_id for e in audio_events), "expected all frames to have the same request_id"
        ...
        assert len(by_segment) == len(segments), "expected one unique segment_id per pushed text segment"
        ...
        for _, (segment_text, segment_events) in enumerate(zip(segments, by_segment.values())):
            *non_final, final = segment_events
            ...
            assert all(not e.is_final for e in non_final), "only the last frame within a segment may be final"
...
async def test_tts_audio_emitter(monkeypatch):
    ...
    m0, m1, m2, m3 = msgs
    assert (m0.segment_id, m0.is_final, m0.frame.data.tobytes()) == (
        "seg1",
        False,
        pcm_chunk,
    )
    assert (m1.segment_id, m1.is_final, m1.frame.data.tobytes()) == (
        "seg1",
        True,
        pcm_chunk,
    )
    ...
    assert (m2.segment_id, m2.is_final, m2.frame.data.tobytes()) == (
        "seg2",
        False,
        pcm_chunk,
    )
    assert (m3.segment_id, m3.is_final, m3.frame.data.tobytes()) == (
        "seg2",
        True,
        pcm_chunk,
    )
