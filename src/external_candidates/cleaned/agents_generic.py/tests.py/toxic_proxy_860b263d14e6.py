# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\toxic_proxy.py
    ...
    def toxics(self) -> dict[str, Toxic]:
        ...
        for toxic_data in toxics_list:
            ...
            toxic_name = toxic_data.get(
                "name",
                f"{toxic_data.get('type')}_{toxic_data.get('stream', 'downstream')}",
            )
...
class Toxiproxy:
    ...
    def populate(self, proxies: list[dict[str, Any]]) -> list[Proxy]:
        ...
        for proxy_conf in proxies:
            ...
            if existing and (existing.upstream != proxy_conf["upstream"] or existing.listen != proxy_conf["listen"]):
