# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\test_agent_session.py
...
async def test_events_and_metrics() -> None:
    ...
    actions.add_tts(2.0, ttfb=0.2, duration=0.3)  # audio playout starts at 3.0+0.3+0.2=3.5s, ends at 5.5s
...
async def test_tool_call() -> None:
    ...
    actions.add_llm(
        ...
        tool_calls=[FunctionToolCall(name="get_weather", arguments='{"location": "Tokyo"}', call_id="1")],
    )
    ...
    assert agent_state_events[1].new_state == "thinking"
    assert agent_state_events[2].new_state == "speaking"
    assert agent_state_events[3].new_state == "thinking"  # from speaking to thinking when tool call is executed
    check_timestamp(agent_state_events[3].created_at - t_origin, 5.5, speed_factor=speed)
...
async def test_interruption_options() -> None:
    ...
    session = create_session(actions, speed_factor=speed, extra_kwargs={"min_interruption_words": 3})
    playback_finished_events: list[PlaybackFinishedEvent] = []
    session.output.audio.on("playback_finished", playback_finished_events.append)
    ...
    session = create_session(actions, speed_factor=speed, extra_kwargs={"allow_interruptions": False})
...
async def test_interruption_before_speaking() -> None:
    ...
    check_timestamp(agent_state_events[2].created_at - t_origin, 3.5, speed_factor=speed)  # interrupted at 3.5s
...
async def test_generate_reply() -> None:
    ...
    check_timestamp(
        conversation_events[2].created_at - t_origin,
        5.5,
        speed_factor=speed,
        max_abs_diff=1.0,
    )
...
class FakeActions:
    ...
    def add_llm(
        ...
        *,
        ...
        if not utils.is_given(input) and self._items and isinstance(self._items[-1], FakeUserSpeech):
    ...
    def add_tts(
        ...
        *,
        ...
        if not utils.is_given(input) and self._items and isinstance(self._items[-1], FakeLLMResponse):
