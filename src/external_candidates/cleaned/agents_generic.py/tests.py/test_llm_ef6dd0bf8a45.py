# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\test_llm.py
...
async def test_runtime_addition(llm_factory: Callable[[], llm.LLM]):
    ...
    stream = await _request_fnc_call(input_llm, "Can you show 'Hello LiveKit!' on the screen?", fnc_ctx)
...
async def test_cancelled_calls(llm_factory: Callable[[], llm.LLM]):
    ...
    assert len(calls) == 1
    assert isinstance(calls[0].exception, asyncio.CancelledError), "toggle_light should have been cancelled"
...
async def test_calls_choices(llm_factory: Callable[[], llm.LLM]):
    ...
    def change_volume(
        volume: Annotated[int, TypeInfo(description="The volume level", choices=[0, 11, 30, 83, 99])],
    ) -> None: ...
...
async def test_optional_args(llm_factory: Callable[[], llm.LLM]):
    ...
    stream = await _request_fnc_call(input_llm, "Using a tool call update the user info to name Theo", fnc_ctx)
...
@pytest.mark.parametrize("description, user_request, tool_choice, expected_calls", test_tool_choice_cases)
@pytest.mark.parametrize("llm_factory", LLMS)
...
with open(_HEARTS_RGBA_PATH, "rb") as f:
    ...
    _HEARTS_IMAGE_VIDEO_FRAME = VideoFrame(width=512, height=512, type=VideoBufferType.RGBA, data=image_data)
