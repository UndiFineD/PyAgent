# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\stt\fallback_adapter.py
...
class FallbackAdapter(
    ...
    async def _try_recognize(
        ...
        *,
        ...
        try:
            ...
            if recovering:
                logger.exception(f"{stt.label} recovery unexpected error", extra={"streamed": False})
                raise
    ...
    def _try_recovery(
        ...
        *,
        ...
        if stt_status.recovering_synthesize_task is None or stt_status.recovering_synthesize_task.done():
...
class FallbackRecognizeStream(RecognizeStream):
    ...
    def _try_recovery(self, stt: STT) -> None:
        stt_status = self._fallback_adapter._status[self._fallback_adapter._stt_instances.index(stt)]
