# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\tts\fallback_adapter.py
    ...
    def __init__(
        ...
        *,
        ...
        for t in tts:
            ...
            self._status.append(_TTSStatus(available=True, recovering_task=None, resampler=resampler))
...
class FallbackChunkedStream(ChunkedStream):
    def __init__(self, *, tts: FallbackAdapter, input_text: str, conn_options: APIConnectOptions) -> None:
    ...
    async def _try_synthesize(self, *, tts: TTS, recovering: bool = False) -> AsyncGenerator[SynthesizedAudio, None]:
        try:
            ...
            if tts_status.available or all_failed:
                try:
                    ...
                    if output_emitter.pushed_duration() > 0.0:
                        logger.warning(f"{tts.label} already synthesized of audio, ignoring fallback")
                        return
