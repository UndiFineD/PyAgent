# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\agent_activity.py
...
class AgentActivity(RecognitionHooks):
    def __init__(self, agent: Agent, sess: AgentSession) -> None:
        ...
        self._turn_detection_mode = self.turn_detection if isinstance(self.turn_detection, str) else None
        ...
        if self._turn_detection_mode == "stt" and not self.stt:
            logger.warning(
                "turn_detection is set to 'stt', but no STT model is provided, ignoring the turn_detection setting"
            )
            self._turn_detection_mode = None
        ...
        if isinstance(self.llm, llm.RealtimeModel):
            ...
            if self._turn_detection_mode == "realtime_llm" and not self.llm.capabilities.turn_detection:
                logger.warning(
            ...
            if not self.llm.capabilities.turn_detection and self.vad and self._turn_detection_mode is None:
                self._turn_detection_mode = "vad"
        elif self._turn_detection_mode == "realtime_llm":
            logger.warning("turn_detection is set to 'realtime_llm', but the LLM is not a RealtimeModel")
            self._turn_detection_mode = None
    ...
    def turn_detection(self) -> TurnDetectionMode | None:
        return cast(
            ...
            (self._agent.turn_detection if is_given(self._agent.turn_detection) else self._session.turn_detection),
        )

    @property
    def mcp_servers(self) -> list[mcp.MCPServer] | None:
        return self._agent.mcp_servers if is_given(self._agent.mcp_servers) else self._session.mcp_servers
    ...
    async def update_instructions(self, instructions: str) -> None:
        ...
        if self._rt_session is not None:
            ...
            update_instructions(self._agent._chat_ctx, instructions=instructions, add_if_missing=True)
    ...
    async def start(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                self._on_enter_task = task = self._create_speech_task(_traceable_on_enter(), name="AgentTask_on_enter")
    ...
    async def _start_session(self) -> None:
        ...
        if isinstance(self.llm, llm.RealtimeModel):
            ...
            if not self.llm.capabilities.audio_output and not self.tts and self._session.output.audio:
                logger.error(
    ...
    async def drain(self) -> None:
        ...
        @tracer.start_as_current_span("on_exit", attributes={trace_types.ATTR_AGENT_LABEL: self._agent.label})
        @utils.log_exceptions(logger=logger)
        async def _traceable_on_exit() -> None:
            await self._agent.on_exit()

        async with self._lock:
            self._on_exit_task = task = self._create_speech_task(_traceable_on_exit(), name="AgentTask_on_exit")
            _set_activity_task_info(task, inline_task=True)
    ...
    async def _pause_scheduling_task(self, *, blocked_tasks: list[asyncio.Task] | None = None) -> None:
    ...
    async def _resume_scheduling_task(self) -> None:
        ...
        self._scheduling_atask = asyncio.create_task(self._scheduling_task(), name="_scheduling_task")
    ...
    def say(
        ...
        *,
        ...
        if not is_given(audio) and not self.tts and self._session.output.audio and self._session.output.audio_enabled:
        ...
        handle = SpeechHandle.create(
            allow_interruptions=(allow_interruptions if is_given(allow_interruptions) else self.allow_interruptions)
        )
        self._session.emit(
            "speech_created",
    ...
    def _schedule_speech(self, speech: SpeechHandle, priority: int, force: bool = False) -> None:
        ...
        if self._scheduling_paused and not force:
            raise RuntimeError("cannot schedule new speech, the speech scheduling is draining/pausing")

        if self._scheduling_atask and self._scheduling_atask.done():
            logger.warning("attempting to schedule a new SpeechHandle, but the scheduling_task is not running.")
            return
        ...
        while True:
            ...
            while self._speech_q:
                ...
                if self.min_consecutive_speech_delay > 0.0:
                    await asyncio.sleep(self.min_consecutive_speech_delay - (time.time() - last_playout_ts))
    ...
    def _on_input_speech_stopped(self, ev: llm.InputSpeechStoppedEvent) -> None:
        ...
        if ev.user_transcription_enabled:
            self._session._user_input_transcribed(UserInputTranscribedEvent(transcript="", is_final=False))

    def _on_input_audio_transcription_completed(self, ev: llm.InputTranscriptionCompleted) -> None:
        self._session._user_input_transcribed(UserInputTranscribedEvent(transcript=ev.transcript, is_final=ev.is_final))
    ...
    def on_vad_inference_done(self, ev: vad.VADEvent) -> None:
        ...
        if (
            ...
            if len(split_words(text, split_character=True)) < self._session.options.min_interruption_words:
                return
    ...
    def on_preemptive_generation(self, info: _PreemptiveGenerationInfo) -> None:
        ...
        self._preemptive_generation = _PreemptiveGeneration(
            speech_handle=self._generate_reply(user_message=user_message, chat_ctx=chat_ctx, schedule_speech=False),
            info=info,
    ...
    @utils.log_exceptions(logger=logger)
    async def _user_turn_completed_task(self, old_task: asyncio.Task[None] | None, info: _EndOfTurnInfo) -> None:
        ...
        if speech_handle is None:
            ...
            speech_handle = self._generate_reply(user_message=user_message, chat_ctx=temp_mutable_chat_ctx)
    ...
    async def _tts_task(
        ...
        tr_output = self._session.output.transcription if self._session.output.transcription_enabled else None
        ...
        if audio_output is not None:
            if audio is None:
                ...
                forward_task, audio_out = perform_audio_forwarding(audio_output=audio_output, tts_output=audio)
                tasks.append(forward_task)
    ...
    async def _pipeline_reply_task(
        ...
        *,
        ...
        audio_output = self._session.output.audio if self._session.output.audio_enabled else None
        text_output = self._session.output.transcription if self._session.output.transcription_enabled else None
        chat_ctx = chat_ctx.copy()
        ...
        if tr_node_result is not None:
            text_forward_task, text_out = perform_text_forwarding(text_output=text_output, source=tr_node_result)
        ...
        if generated_msg:
            ...
            current_span.set_attribute(trace_types.ATTR_RESPONSE_TEXT, generated_msg.text_content or "")
                ...
                tool_response_task = self._create_speech_task(
                    self._pipeline_reply_task(
                        ...
                        model_settings=ModelSettings(
                            ...
                            tool_choice=("none" if draining or model_settings.tool_choice == "none" else "auto"),
                        ),
    ...
    async def _realtime_reply_task(
        ...
        *,
        ...
        if utils.is_given(model_settings.tool_choice):
            self._rt_session.update_options(tool_choice=cast(llm.ToolChoice, model_settings.tool_choice))

        try:
            generation_ev = await self._rt_session.generate_reply(instructions=instructions or NOT_GIVEN)
            ...
            if utils.is_given(model_settings.tool_choice) and model_settings.tool_choice != ori_tool_choice:
    ...
    async def _realtime_generation_task(
        ...
        *,
            ...
            try:
                async for msg in generation_ev.message_stream:
                    if len(forward_tasks) > 0:
                        logger.warning("expected to receive only one message generation from the realtime API")
                        break
                    ...
                    if "audio" not in msg_modalities and self.tts:
                        if self.llm.capabilities.audio_output:
                            logger.warning("text response received from realtime API, falling back to use a TTS model.")
                    ...
                    if audio_output is not None:
                        ...
                        if tts_text_input is not None:
                            ...
                            realtime_audio = self._agent.realtime_audio_output_node(msg.audio_stream, model_settings)
                            realtime_audio_result = (
                                await realtime_audio if asyncio.iscoroutine(realtime_audio) else realtime_audio
                            )
        ...
        message_outputs: list[tuple[MessageGeneration, _TextOutput | None, _AudioOutput | None]] = []
        tasks.append(
            asyncio.create_task(
        ...
        tool_output.first_tool_started_fut.add_done_callback(lambda _: self._session._update_agent_state("thinking"))
        await exe_task
