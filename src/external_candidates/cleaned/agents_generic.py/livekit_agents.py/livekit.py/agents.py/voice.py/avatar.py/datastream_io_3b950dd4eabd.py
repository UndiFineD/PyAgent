# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\avatar\_datastream_io.py
...
class DataStreamAudioReceiver(AudioReceiver):
    ...
    async def start(self) -> None:
        ...
        def _handle_stream_received(reader: rtc.ByteStreamReader, remote_participant_id: str) -> None:
            if not self._remote_participant or remote_participant_id != self._remote_participant.identity:
                return
    ...
    async def _send_task(self) -> None:
        async for event in self._playback_finished_ch:
            ...
            while retry_count < self._rpc_max_retries:
                logger.debug(
                    f"notifying playback finished: {event.playback_position:.3f}s, interrupted: {event.interrupted}"
                )
                try:
