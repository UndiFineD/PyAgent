# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\recorder_io\recorder_io.py
...
class RecorderIO:
    ...
    def record_output(self, audio_output: io.AudioOutput) -> RecorderAudioOutput:
        self._out_record = RecorderAudioOutput(recording_io=self, audio_output=audio_output, write_fnc=self._write_cb)
    ...
    def _encode_thread(self) -> None:
        ...
        stream: av.AudioStream = container.add_stream("opus", rate=self._sample_rate, layout="stereo")  # type: ignore
        ...
        def remix_and_resample(frames: list[rtc.AudioFrame], channel_idx: int) -> int:
            ...
            for f in frames:
                ...
                arr_i16 = np.frombuffer(f.data, dtype=np.int16, count=count).reshape(-1, f.num_channels)
