...
class _TurnDetector(Protocol):
    ...
    async def predict_end_of_turn(self, chat_ctx: llm.ChatContext, *, timeout: float | None = None) -> float: ...
...
class AudioRecognition:
    ...
    def commit_user_turn(self, *, audio_detached: bool, transcript_timeout: float) -> None:
        async def _commit_user_turn() -> None:
            ...
            if self._audio_interim_transcript:
                ...
                self._audio_transcript = f"{self._audio_transcript} {self._audio_interim_transcript}".strip()
            self._audio_interim_transcript = ""
    ...
    async def _on_stt_event(self, ev: stt.SpeechEvent) -> None:
        ...
        if ev.type == stt.SpeechEventType.FINAL_TRANSCRIPT:
            ...
            if not self._last_language or (language and len(transcript) > MIN_LANGUAGE_DETECTION_LENGTH):
    ...
    async def _on_vad_event(self, ev: vad.VADEvent) -> None:
        if ev.type == vad.VADEventType.START_OF_SPEECH:
            ...
            if self._vad_base_turn_detection or (self._turn_detection_mode == "stt" and self._user_turn_committed):
                chat_ctx = self._hooks.retrieve_chat_ctx().copy()
                self._run_eou_detection(chat_ctx)
                ...
                if not await turn_detector.supports_language(self._last_language):
                    ...
                    with (
                        ...
                        try:
                            ...
                            if unlikely_threshold is not None and end_of_turn_probability < unlikely_threshold:
    ...
    async def _stt_task(
        ...
        if isinstance(node, AsyncIterable):
            async for ev in node:
                assert isinstance(ev, stt.SpeechEvent), f"STT node must yield SpeechEvent, got: {type(ev)}"
