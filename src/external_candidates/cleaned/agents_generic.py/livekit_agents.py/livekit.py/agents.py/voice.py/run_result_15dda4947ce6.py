# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\run_result.py
...
class RunAssert:
    ...
    def __getitem__(self, key: [int, slice]) -> EventAssert | EventRangeAssert:  # type: ignore
        ...
        raise TypeError(f"{type(self).__name__} indices must be int or slice, not {type(key).__name__}")
    ...
    def skip_next_event_if(
        ...
        *,
        ...
        is_error: NotGivenOr[bool] = NOT_GIVEN,
        new_agent_type: NotGivenOr[type[Agent]] = NOT_GIVEN,
    ) -> ChatMessageAssert | AgentHandoffAssert | FunctionCallAssert | FunctionCallOutputAssert | None:
        """
        ...
        __tracebackhide__ = True
        try:
            ev: ChatMessageAssert | FunctionCallAssert | FunctionCallOutputAssert | AgentHandoffAssert | None = None
            if type == "message":
    ...
    def skip_next(self, count: int = 1) -> RunAssert:
        ...
        for i in range(count):
            if self._current_index >= len(self._events_list):
                self._raise_with_debug_info(f"Tried to skip {count} event(s), but only {i} were available.")
            self._current_index += 1
        return self
    ...
    def no_more_events(self) -> None:
        ...
        if self._current_index < len(self._events_list):
            ...
            self._raise_with_debug_info(f"Expected no more events, but found: {type(event).__name__}")
...
class EventAssert:
    ...
    def is_agent_handoff(self, *, new_agent_type: NotGivenOr[type[Agent]] = NOT_GIVEN) -> AgentHandoffAssert:
        ...
        if is_given(new_agent_type) and not isinstance(self._event.new_agent, new_agent_type):
            self._raise(f"Expected new_agent '{new_agent_type.__name__}', got '{type(self._event.new_agent).__name__}'")
...
class EventRangeAssert:
    ...
    def contains_function_call(
        ...
        *,
        ...
        self._parent._raise_with_debug_info(f"No FunctionCallEvent satisfying criteria found in range {self._rng!r}")
    ...
    def contains_message(
        ...
        *,
        ...
        self._parent._raise_with_debug_info(f"No ChatMessageEvent matching criteria found in range {self._rng!r}")
        raise RuntimeError("unreachable")
    ...
    def contains_agent_handoff(self, *, new_agent_type: NotGivenOr[type[Agent]] = NOT_GIVEN) -> AgentHandoffAssert:
        ...
        self._parent._raise_with_debug_info(f"No AgentHandoffEvent matching criteria found in range {self._rng!r}")
        raise RuntimeError("unreachable")
...
class ChatMessageAssert:
    ...
    async def judge(self, llm_v: llm.LLM, *, intent: str) -> ChatMessageAssert:
        ...
        fnc_args, fnc_kwargs = llm_utils.prepare_function_arguments(fnc=check_intent, json_arguments=arguments)
...
def _format_events(events: list[RunEvent], *, selected_index: int | None = None) -> list[str]:
    ...
    for i, event in enumerate(events):
        ...
        if isinstance(event, (ChatMessageEvent, FunctionCallEvent, FunctionCallOutputEvent)):
            ...
            line = f"{prefix} [{i}] AgentHandoffEvent(old_agent={event.old_agent}, new_agent={event.new_agent})"
        else:
