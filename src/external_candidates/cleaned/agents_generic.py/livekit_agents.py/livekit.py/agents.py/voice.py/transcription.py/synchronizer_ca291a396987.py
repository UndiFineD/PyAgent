...
class _SpeakingRateData:
    ...
    def accumulate_to(self, timestamp: float) -> float:
        ...
        rate = self.speaking_rate[idx] if idx < len(self.speaking_rate) else self.speaking_rate[idx - 1]
        integral_t += rate * dt
...
class _SegmentSynchronizerImpl:
    ...
    def _reestimate_speed(self) -> None:
        ...
        pushed_speaking_units = self._audio_data.sr_data_est.accumulate_to(self._audio_data.pushed_duration)
    ...
    async def _speaking_rate_task(self) -> None:
        async for ev in self._audio_data.sr_stream:
            self._audio_data.sr_data_est.add_by_rate(timestamp=ev.timestamp, speaking_rate=ev.speaking_rate)
    ...
    async def _main_task(self) -> None:
        ...
        async for text_seg in self._text_data.sentence_stream:
            ...
            for word, _, end_pos in self._opts.split_words(sentence):
                ...
                if (annotated := self._audio_data.sr_data_annotated) and (annotated.pushed_duration >= elapsed):
...
class TranscriptSynchronizer:
    ...
    def __init__(
        ...
        *,
        ...
        self._opts = _TextSyncOptions(
            ...
            sentence_tokenizer=(sentence_tokenizer or tokenize.basic.SentenceTokenizer(retain_format=True)),
            speaking_rate_detector=SpeakingRateDetector(),
        )
        self._enabled = True
        self._closed = False
    ...
    async def _rotate_segment_task(self, old_task: asyncio.Task[None] | None) -> None:
        ...
        await self._impl.aclose()
        self._impl = _SegmentSynchronizerImpl(options=self._opts, next_in_chain=self._text_output._next_in_chain)
    ...
    def rotate_segment(self) -> None:
        ...
        self._rotate_segment_atask = asyncio.create_task(self._rotate_segment_task(self._rotate_segment_atask))
...
class _SyncedAudioOutput(io.AudioOutput):
    def __init__(self, synchronizer: TranscriptSynchronizer, *, next_in_chain: io.AudioOutput) -> None:
        super().__init__(
    ...
    async def capture_frame(self, frame: rtc.AudioFrame) -> None:
        ...
        if self._synchronizer._impl.audio_input_ended:
            ...
            logger.warning("_SegmentSynchronizerImpl audio marked as ended in capture audio, rotating segment")
    ...
    def on_playback_finished(
        ...
        *,
        ...
        self._synchronizer._impl.mark_playback_finished(playback_position=playback_position, interrupted=interrupted)
        super().on_playback_finished(
            playback_position=playback_position,
            interrupted=interrupted,
...
class _SyncedTextOutput(io.TextOutput):
    def __init__(self, synchronizer: TranscriptSynchronizer, *, next_in_chain: io.TextOutput) -> None:
    ...
    async def capture_text(self, text: str) -> None:
        ...
        if self._synchronizer._impl.text_input_ended:
            # this should not happen if `on_playback_finished` is called after each flush
            logger.warning("_SegmentSynchronizerImpl text marked as ended in capture text, rotating segment")
            self._synchronizer.rotate_segment()
            await self._synchronizer.barrier()
