# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\agent_session.py
...
class AgentSession(rtc.EventEmitter[EventTypes], Generic[Userdata_T]):
        ...
        *,
        ...
        async with self._lock:
            ...
            if cli.CLI_ARGUMENTS is not None and cli.CLI_ARGUMENTS.console:
                ...
                room_input_options = copy.copy(room_input_options or room_io.DEFAULT_ROOM_INPUT_OPTIONS)
                room_output_options = copy.copy(room_output_options or room_io.DEFAULT_ROOM_OUTPUT_OPTIONS)
            ...
            try:
                ...
                if not self._job_context_cb_registered:
                    job_ctx.add_shutdown_callback(lambda: self._aclose_impl(reason=CloseReason.JOB_SHUTDOWN))
                    self._job_context_cb_registered = True
            except RuntimeError:
                pass  # ignore

            # it is ok to await it directly, there is no previous task to drain
            tasks.append(asyncio.create_task(self._update_activity(self._agent, wait_on_enter=False)))
            ...
            if self.input.audio is not None:
                self._forward_audio_atask = asyncio.create_task(self._forward_audio_task(), name="_forward_audio_task")

            if self.input.video is not None:
                self._forward_video_atask = asyncio.create_task(self._forward_video_task(), name="_forward_video_task")
    ...
    def _close_soon(
        ...
        *,
        ...
        self._closing_task = asyncio.create_task(self._aclose_impl(error=error, drain=drain, reason=reason))
    ...
    def generate_reply(
        ...
        *,
        ...
        user_message = llm.ChatMessage(role="user", content=[user_input]) if is_given(user_input) else NOT_GIVEN
    ...
    @utils.log_exceptions(logger=logger)
    async def _update_activity_task(self, old_task: asyncio.Task[None] | None, agent: Agent) -> None:
    ...
    def _on_error(
        ...
        self._closing_task = asyncio.create_task(self._aclose_impl(error=error, reason=CloseReason.ERROR))
    ...
    def _set_user_away_timer(self) -> None:
        ...
        if (room_io := self._room_io) and room_io.subscribed_fut and not room_io.subscribed_fut.done():
            # skip the timer before user join the room
            return

        self._user_away_timer = self._loop.call_later(self._opts.user_away_timeout, self._update_user_state, "away")
    ...
    def _update_user_state(self, state: UserState, *, last_speaking_time: float | None = None) -> None:
    ...
    def _schedule_agent_false_interruption(self, ev: AgentFalseInterruptionEvent) -> None:
        ...
        self._cancel_agent_false_interruption()
        self._false_interruption_timer = self._loop.call_later(self._opts.agent_false_interruption_timeout, _emit_event)
        self._false_interrupted_event = ev
