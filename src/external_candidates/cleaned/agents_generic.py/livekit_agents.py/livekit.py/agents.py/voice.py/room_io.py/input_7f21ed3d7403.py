# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\room_io\_input.py
...
class _ParticipantInputStream(Generic[T], ABC):
    ...
    def __init__(
        ...
        *,
        ...
        self._accepted_sources = (
            {track_source} if isinstance(track_source, rtc.TrackSource.ValueType) else set(track_source)
        )
    ...
    def on_attached(self) -> None:
        logger.debug(
            ...
            extra={
                ...
                "accepted_sources": [rtc.TrackSource.Name(source) for source in self._accepted_sources],
            },
        )
        self._attached = True
    ...
    def set_participant(self, participant: rtc.RemoteParticipant | str | None) -> None:
        ...
        participant_identity = participant.identity if isinstance(participant, rtc.RemoteParticipant) else participant
...
class _ParticipantAudioInputStream(_ParticipantInputStream[rtc.AudioFrame], AudioInput):
    def __init__(
        ...
        *,
        ...
        _ParticipantInputStream.__init__(self, room=room, track_source=rtc.TrackSource.SOURCE_MICROPHONE)
    ...
    def _resample_frames(self, frames: Iterable[rtc.AudioFrame]) -> Iterable[rtc.AudioFrame]:
        ...
        for frame in frames:
            if not resampler and self._sample_rate is not None and frame.sample_rate != self._sample_rate:
                resampler = rtc.AudioResampler(input_rate=frame.sample_rate, output_rate=self._sample_rate)
