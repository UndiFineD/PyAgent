# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\generation.py
...
def perform_llm_inference(
    *,
    ...
    llm_task = asyncio.create_task(_llm_inference_task(node, chat_ctx, tool_ctx, model_settings, data))
...
async def _llm_inference_task(
    ...
    current_span.set_attribute(
        ...
        json.dumps(chat_ctx.to_dict(exclude_audio=True, exclude_image=True, exclude_timestamp=False)),
    )
    current_span.set_attribute(
        trace_types.ATTR_FUNCTION_TOOLS,
        json.dumps(list(tool_ctx.function_tools.keys())),
    )
    ...
    current_span.set_attribute(
        ...
        json.dumps([fnc.model_dump(exclude={"type", "created_at"}) for fnc in data.generated_functions]),
    )
    return True
    ...
    try:
        async for fnc_call in function_stream:
            ...
            try:
                ...
                mock_tools: dict[str, Callable] = _MockToolsContextVar.get({}).get(type(session.current_agent), {})
                ...
                async def _traceable_fnc_tool(function_callable: Callable, fnc_call: llm.FunctionCall) -> None:
                    current_span = trace.get_current_span()
                    current_span.set_attribute(trace_types.ATTR_FUNCTION_TOOL_NAME, fnc_call.name)
                    current_span.set_attribute(trace_types.ATTR_FUNCTION_TOOL_ARGS, fnc_call.arguments)
                    ...
                    if fnc_call_out := output.fnc_call_out:
                        current_span.set_attribute(trace_types.ATTR_FUNCTION_TOOL_OUTPUT, fnc_call_out.output)
                        current_span.set_attribute(
                            trace_types.ATTR_FUNCTION_TOOL_IS_ERROR,
                            fnc_call_out.is_error,
                        )
...
def _is_valid_function_output(value: Any) -> bool:
    ...
    if isinstance(value, VALID_TYPES):
        return True
    elif isinstance(value, list) or isinstance(value, set) or isinstance(value, frozenset) or isinstance(value, tuple):
        return all(_is_valid_function_output(item) for item in value)
    elif isinstance(value, dict):
        return all(isinstance(key, VALID_TYPES) and _is_valid_function_output(val) for key, val in value.items())
    return False
...
def update_instructions(chat_ctx: ChatContext, *, instructions: str, add_if_missing: bool) -> None:
    ...
    if idx is not None:
        if chat_ctx.items[idx].type == "message":
            ...
            raise ValueError("expected the instructions inside the chat_ctx to be of type 'message'")
    elif add_if_missing:
