...
class PreConnectAudioHandler:
    ...
    def register(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"pre-connect audio handler for {PRE_CONNECT_AUDIO_BUFFER_STREAM} already registered, ignoring"
            )
    ...
    async def _read_audio_task(self, reader: rtc.ByteStreamReader, participant_id: str) -> None:
        ...
        try:
            if "sampleRate" not in reader.info.attributes or "channels" not in reader.info.attributes:
            ...
            if reader.info.mime_type:
                ...
                is_opus = reader.info.mime_type == "audio/opus" or "codecs=opus" in reader.info.mime_type

            if is_opus:
                decoder = utils.codecs.AudioStreamDecoder(sample_rate=sample_rate, num_channels=num_channels)
