...
class AgentTask(Agent, Generic[TaskResult_T]):
    ...
    async def __await_impl(self) -> TaskResult_T:
        ...
        if current_task is None:
            raise RuntimeError(f"{self.__class__.__name__} must be executed inside an async context")
        ...
        def _handle_task_done(_: asyncio.Task[Any]) -> None:
            ...
            logger.error(f"The asyncio.Task finished before {self.__class__.__name__} was completed.")

            self.complete(RuntimeError(f"The asyncio.Task finished before {self.__class__.__name__} was completed."))
        ...
        await session._update_activity(self, previous_activity="pause", blocked_tasks=[current_task])
