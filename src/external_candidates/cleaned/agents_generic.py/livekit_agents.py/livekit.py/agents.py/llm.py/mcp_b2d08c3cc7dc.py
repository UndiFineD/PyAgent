# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\llm\mcp.py
...
class MCPServer(ABC):
        ...
        try:
            ...
            self._client = await self._exit_stack.enter_async_context(
                ClientSession(
                    receive_stream,
                    send_stream,
                    read_timeout_seconds=(timedelta(seconds=self._read_timeout) if self._read_timeout else None),
                )
            )
    ...
    async def list_tools(self) -> list[MCPTool]:
        ...
        lk_tools = [
            self._make_function_tool(tool.name, tool.description, tool.inputSchema, tool.meta) for tool in tools.tools
        ]
