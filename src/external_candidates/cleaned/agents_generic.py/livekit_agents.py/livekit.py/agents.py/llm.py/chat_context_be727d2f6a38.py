# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\llm\chat_context.py
    ...
    def copy(
        ...
        *,
        ...
        for item in self.items:
            ...
            if exclude_instructions and item.type == "message" and item.role in ["system", "developer"]:
                continue
    ...
    def is_equivalent(self, other: ChatContext) -> bool:
        ...
        for a, b in zip(self.items, other.items):
            ...
            if a.type == "message" and b.type == "message":
                ...
                if a.name != b.name or a.call_id != b.call_id or a.output != b.output or a.is_error != b.is_error:
                    return False
