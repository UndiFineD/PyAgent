# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\llm\fallback_adapter.py
...
DEFAULT_FALLBACK_API_CONNECT_OPTIONS = APIConnectOptions(max_retry=0, timeout=DEFAULT_API_CONNECT_OPTIONS.timeout)
...
class FallbackAdapter(
    ...
    def __init__(
        ...
        *,
        ...
        self._status = [_LLMStatus(available=True, recovering_task=None) for _ in self._llm_instances]
...
class FallbackLLMStream(LLMStream):
    ...
    async def _try_generate(self, *, llm: LLM, check_recovery: bool = False) -> AsyncIterable[ChatChunk]:
        """
    ...
    def _try_recovery(self, llm: LLM) -> None:
        llm_status = self._fallback_adapter._status[self._fallback_adapter._llm_instances.index(llm)]
