    ...
    if isinstance(image.image, str):
        if image.image.startswith("data:"):
            ...
            if mime_type not in supported_types:
                raise ValueError(f"Unsupported mime_type {mime_type}. Must be jpeg, png, webp, or gif")
...
def function_arguments_to_pydantic_model(func: Callable[..., Any]) -> type[BaseModel]:
    ...
    for param_name, param in signature.parameters.items():
        ...
        if get_origin(type_hint) is Annotated:
            ...
            field_info = next((x for x in annotated_args[1:] if isinstance(x, FieldInfo)), field_info)
