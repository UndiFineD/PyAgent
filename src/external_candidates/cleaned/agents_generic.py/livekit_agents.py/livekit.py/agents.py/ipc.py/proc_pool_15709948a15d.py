...
class ProcPool(utils.EventEmitter[EventTypes]):
    ...
    async def launch_job(self, info: RunningJobInfo) -> None:
        ...
        if self._warmed_proc_queue.empty() and len(self._spawn_tasks) < self._jobs_waiting_for_process:
    ...
    async def _main_task(self) -> None:
        try:
            while not self._closed:
                ...
                to_spawn = min(self._target_idle_processes, self._default_num_idle_processes) - current_pending
