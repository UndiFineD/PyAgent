...
def get_job_context() -> JobContext:
    ...
    if ctx is None:
        raise RuntimeError("no job context found, are you running this code inside a job entrypoint?")
...
class JobContext:
    _PARTICIPANT_ENTRYPOINT_CALLBACK = Callable[["JobContext", rtc.RemoteParticipant], Coroutine[None, None, None]]
    ...
    def __init__(
        ...
        *,
        ...
        self._participant_tasks = dict[tuple[str, JobContext._PARTICIPANT_ENTRYPOINT_CALLBACK], asyncio.Task[None]]()
    ...
    def add_shutdown_callback(
        self,
        callback: (Callable[[], Coroutine[None, None, None]] | Callable[[str], Coroutine[None, None, None]]),
    ) -> None:
        """
    ...
    def delete_room(self) -> asyncio.Future[api.DeleteRoomResponse]:  # type: ignore
        ...
        task = asyncio.create_task(self.api.room.delete_room(api.DeleteRoomRequest(room=self._room.name)))
    ...
    def transfer_sip_participant(
        ...
        """
        if cli.CLI_ARGUMENTS is not None and cli.CLI_ARGUMENTS.console:
            logger.warning("job_ctx.transfer_sip_participant() is not executed while in console mode")
            fut = asyncio.Future[api.SIPParticipantInfo]()
            fut.set_result(api.SIPParticipantInfo())
            return fut
...
def _apply_auto_subscribe_opts(room: rtc.Room, auto_subscribe: AutoSubscribe) -> None:
    ...
    def _subscribe_if_needed(pub: rtc.RemoteTrackPublication) -> None:
        if (auto_subscribe == AutoSubscribe.AUDIO_ONLY and pub.kind == rtc.TrackKind.KIND_AUDIO) or (
            auto_subscribe == AutoSubscribe.VIDEO_ONLY and pub.kind == rtc.TrackKind.KIND_VIDEO
        ):
