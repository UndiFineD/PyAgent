# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\worker.py
...
class _DefaultLoadCalc:
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(target=self._calc_load, daemon=True, name="worker_cpu_load_monitor")
...
class WorkerOptions:
    ...
    def validate_config(self, devmode: bool) -> None:
        ...
        if load_threshold > 1 and not devmode:
            logger.warning(f"load_threshold in prod env must be less than 1, current value: {load_threshold}")
...
class Worker(utils.EventEmitter[EventTypes]):
    def __init__(
        ...
        *,
        ...
        if not opts.api_secret:
            raise ValueError("api_secret is required, or add LIVEKIT_API_SECRET in your environment")

        if opts.job_memory_limit_mb > 0 and opts.job_executor_type != JobExecutorType.PROCESS:
            logger.warning(
                "max_job_memory_usage is only supported for process-based job executors, ignoring max_job_memory_usage"
            )
    ...
    async def run(self) -> None:
        ...
        async def _load_task() -> None:
            ...
            while True:
                ...
                default_num_idle_processes = _WorkerEnvOption.getvalue(self._opts.num_idle_processes, self._devmode)
    ...
    async def simulate_job(
        ...
        participant_identity = info.participant_identity if isinstance(info, SimulateJobInfo) else "unknown-participant"
    ...
    async def _update_worker_status(self) -> None:
        ...
        status = agent.WorkerStatus.WS_AVAILABLE if currently_available else agent.WorkerStatus.WS_FULL
