# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\utils\codecs\decoder.py
        ...
        try:
            ...
            while True:
                ...
                while remaining > 0:
                    ...
                    remaining -= len(part)
                if chunk_id == b"fmt ":
                    audio_format, wave_channels, wave_rate, _, _, bits_per_sample = struct.unpack("<HHIIHH", data[:16])
                    if audio_format != 1:
            ...
            resampler = (
                rtc.AudioResampler(
                    input_rate=wave_rate,
                    output_rate=self._sample_rate,
                    num_channels=wave_channels,
                )
