# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\warm-transfer\warm_transfer.py
import asyncio
import logging
import os
...
class SessionManager:
    ...
    def on_customer_participant_disconnected(self, participant: rtc.RemoteParticipant):
        ...
        self.customer_room.off("participant_disconnected", self.on_customer_participant_disconnected)
    ...
    async def merge_calls(self):
        ...
        try:
            ...
            self.customer_room.on("participant_disconnected", self.on_customer_participant_disconnected)

            # wait for this to be fully played out and close the agent's session with customer
            await self.customer_session.say("you are on the line with my supervisor. I'll be hanging up now.")
