# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\frontdesk\test_agent.py
...
async def test_slot_scheduling() -> None:
    ...
    async with _llm_model() as llm, AgentSession(llm=llm, userdata=userdata) as sess:
        ...
        expected_times_text = ", ".join(slot.start_time.strftime("%-I:%M %p") for slot in expected_tomorrow_slots)
        ...
        result.expect.next_event().is_function_call(name="schedule_appointment", arguments={"slot_id": slot_id})
        result.expect.next_event().is_agent_handoff(new_agent_type=beta.workflows.GetEmailTask)
        await (
            result.expect.next_event().is_message(role="assistant").judge(llm, intent="must ask for the email address")
        )

        result = await sess.run(user_input="My email address is theo@livekit.io")
        result.expect.next_event().is_function_call(name="update_email_address", arguments={"email": "theo@livekit.io"})
        result.expect.next_event().is_function_call_output()
        await (
            result.expect.next_event()
            .is_message(role="assistant")
