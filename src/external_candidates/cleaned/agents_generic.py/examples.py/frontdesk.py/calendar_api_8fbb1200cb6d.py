# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\frontdesk\calendar_api.py
...
class FakeCalendar(Calendar):
    ...
    async def schedule_appointment(self, *, start_time: datetime.datetime, attendee_email: str) -> None:
        ...
        async with self._http_session.get(
            ...
            for _, slots in raw_data.items():
                for slot in slots:
                    ...
                    available_slots.append(AvailableSlot(start_time=start_dt, duration_min=EVENT_DURATION_MIN))
