# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\voice_agents\error_callback.py
import asyncio
import logging
import os
import pathlib
...
async def entrypoint(ctx: JobContext):
    ...
    def on_close(_: CloseEvent):
        ...
        participant = [
            p for p in ctx.room.remote_participants.values() if p.kind == ParticipantKind.PARTICIPANT_KIND_SIP
        ][0]
        ...
        if participant.kind == ParticipantKind.PARTICIPANT_KIND_SIP:
            ctx.transfer_sip_participant(participant, "tel:+18003310500").add_done_callback(on_sip_transfer_done)
