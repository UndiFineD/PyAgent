# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\voice_agents\toggle_io.py
import logging
...
async def entrypoint(ctx: JobContext):
    ...
    await room_io.start()
    await session.start(
        agent=Agent(instructions="You are a helpful assistant that interfaces with the user via voice.")
    )
