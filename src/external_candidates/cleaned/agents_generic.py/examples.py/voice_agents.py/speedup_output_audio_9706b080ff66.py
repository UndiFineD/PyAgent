# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\voice_agents\speedup_output_audio.py
import logging
...
try:
    ...
    raise ImportError("librosa is required to run this example, install it with `pip install librosa`") from None
...
class MyAgent(Agent):
    ...
    async def realtime_audio_output_node(
        ...
        return self._process_audio_stream(Agent.default.realtime_audio_output_node(self, audio, model_settings))

    async def _process_audio_stream(self, audio: AsyncIterable[rtc.AudioFrame]) -> AsyncIterable[rtc.AudioFrame]:
