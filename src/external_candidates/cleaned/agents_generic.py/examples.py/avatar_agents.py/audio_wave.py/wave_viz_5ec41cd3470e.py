# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\avatar_agents\audio_wave\wave_viz.py
import time
...
class WaveformVisualizer:
    ...
    def draw_current_wave(self, canvas: np.ndarray, audio_samples: np.ndarray) -> float:
        ...
        if len(normalized_samples) >= self.n_fft:
            ...
            self.prev_fft = self.prev_fft * (1 - self.smoothing_factor) + plot_data * self.smoothing_factor
        else:
            volume = 0
            self.prev_fft *= 0.5
        ...
        bottom_points = np.column_stack((x_smooth, np.full_like(x_smooth, center_y))).astype(np.int32)
