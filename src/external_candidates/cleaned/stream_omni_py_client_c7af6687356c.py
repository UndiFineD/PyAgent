# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\stream_omni.py\cosyvoice.py\runtime.py\python.py\grpc.py\client_c7af6687356c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Stream-Omni\CosyVoice\runtime\python\grpc\client.py
...
def main():
    ...
    with grpc.insecure_channel("{}:{}".format(args.host, args.port)) as channel:
        ...
        if args.mode == "sft":
            ...
            zero_shot_request.prompt_audio = (

                (prompt_speech.numpy() * (2**15)).astype(np.int16).tobytes()
            ...
            cross_lingual_request.prompt_audio = (
