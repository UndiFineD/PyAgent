# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alphafactor.py\explorer.py\scripts.py\github_projects_c985e35f9ba1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alphafactor\explorer\scripts\github_projects.py
...
示例:
  ...
  %(prog)s --topic rust --stars 5000 --limit 50
    ...
    parser.add_argument("--stars", "-s", type=int, default=100, help="最少 Star 数量（默认: 100）")
    ...
    parser.add_argument("--limit", type=int, default=30, help="返回结果数量（默认: 30，最大: 100）")

    parser.add_argument(
        "--sort",
        choices=["stars", "forks", "updated", "created"],
        default="stars",
        help="排序方式（默认: stars）",
    )
    ...
    projects = search_repositories(query=query, sort=args.sort, per_page=args.limit)
