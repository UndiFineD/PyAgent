# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\moltbro.py\humanize_ai_text.py\scripts.py\transform_bbb39f230764.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\moltbro\humanize-ai-text\scripts\transform.py
...
def replace_bounded(text: str, old: str, new: str) -> tuple[str, int]:
    pattern = (
        re.compile(re.escape(old), re.IGNORECASE)
        if " " in old or old.endswith(",")
        else re.compile(r"\b" + re.escape(old) + r"\b", re.IGNORECASE)
    )
...
def remove_citations(text: str) -> tuple[str, list]:
    ...
    patterns = [
        ...
        (r"turn0search\d+", "turn0search"),
        (r"turn0image\d+", "turn0image"),
...
def transform(text: str, aggressive: bool = False) -> tuple[str, list]:
    ...
    text, changes = remove_citations(text)

    all_changes.extend(changes)

    text, changes = strip_markdown(text)

    all_changes.extend(changes)

    text, changes = remove_chatbot_sentences(text)

    all_changes.extend(changes)

    text, changes = apply_replacements(text, PATTERNS["copula_avoidance"])

    all_changes.extend(changes)

    text, changes = apply_replacements(text, PATTERNS["filler_replacements"])
    ...
    if aggressive:
        text, changes = simplify_ing(text)
