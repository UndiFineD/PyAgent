# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\Aligner\CodecAlignerDataset.py
import os
import random
...
class CodecAlignerDataset(Dataset):
    ...
    def _build_dataset_cache(
        ...
        _, _ = torch.hub.load(
            repo_or_dir="snakers4/silero-vad",  # make sure it gets downloaded during single-processing first, if it's not already downloaded
            model="silero_vad",
            force_reload=False,
            onnx=False,
            verbose=False,
    ...
    def _cache_builder_process(
        ...
        for path in tqdm(path_list):
            ...
            try:
                ...
                for silence_timestamp in silence_timestamps:
                    ...
                    norm_wave = torch.cat(
                        [
                            norm_wave[:begin],
                            torch.zeros([end - begin], device=device),
                            norm_wave[end:],
                        ]
                    )
            ...
            try:
                try:
                    cached_text = (
                        tf.string_to_tensor(transcript, handle_missing=False, input_phonemes=phone_input)
                        .squeeze(0)
                        .cpu()
                        .numpy()
                    )
                    ...
                    cached_text = (
                        tf.string_to_tensor(transcript, handle_missing=True, input_phonemes=phone_input)
                        .squeeze(0)
                        .cpu()
                        .numpy()
                    )
            ...
            cached_speech = (
                ap.audio_to_codebook_indexes(audio=norm_wave, current_sampling_rate=16000).transpose(0, 1).cpu().numpy()
            )
            process_internal_dataset_chunk.append([cached_text, cached_speech, norm_wave.cpu().detach().numpy(), path])
    ...
    def remove_samples(self, list_of_samples_to_remove):
        ...
        torch.save(
            (self.datapoints, None, self.speaker_embeddings, self.filepaths),
