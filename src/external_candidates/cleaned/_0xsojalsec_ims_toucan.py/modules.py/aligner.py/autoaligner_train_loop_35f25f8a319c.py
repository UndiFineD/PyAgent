        ...
        for batch in tqdm(train_loader):
            ...
            for datapoint in batch[2]:
                with torch.inference_mode():
                    ...
                    mel = (
                        spectrogram_extractor.audio_to_mel_spec_tensor(speech, explicit_sampling_rate=16000)
                        .transpose(0, 1)
                        .cpu()
                    )
            ...
            ctc_loss = asr_model.ctc_loss(pred.transpose(0, 1).log_softmax(2), tokens, mel_len, tokens_len)

            if use_reconstruction:
                speaker_embeddings_expanded = (
                    torch.nn.functional.normalize(speaker_embeddings).unsqueeze(1).expand(-1, pred.size(1), -1)
                )
