# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\EnergyCalculator.py
...
class EnergyCalculator(torch.nn.Module):
    ...
    def forward(
        ...
        if feats_lengths is not None:
            energy = [
                self._adjust_num_frames(e[:el].view(-1), fl) for e, el, fl in zip(energy, energy_lengths, feats_lengths)
            ]
        ...
        if self.use_token_averaged_energy:
            energy = [
                self._average_by_duration(e[:el].view(-1), d, text)
