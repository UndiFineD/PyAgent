# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\CodecDiscriminator.py
import torch
...
class SpectrogramDiscriminator(torch.nn.Module):
    ...
    def _generator_feedback(self, data_generated, data_real):
        ...
        discr_loss = nn.functional.mse_loss(
            input=score_fake,
            target=torch.ones(score_fake.shape, device=score_fake.device),
            reduction="mean",
        )
    ...
    def _discriminator_feature_matching(self, data_generated, data_real):
        ...
        discr_loss = 0.0
        discr_loss = discr_loss + nn.functional.mse_loss(
            input=score_fake,
            target=torch.zeros(score_fake.shape, device=score_fake.device),
            reduction="mean",
        )
        discr_loss = discr_loss + nn.functional.mse_loss(
            input=score_real,
            target=torch.ones(score_real.shape, device=score_real.device),
            reduction="mean",
        )
...
class DiscriminatorNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(
            900, 1
        )  # this needs to be changed everytime the window length is changes. It would be nice if this could be done dynamically.
