# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\PitchCalculator.py
...
class Parselmouth(torch.nn.Module):
    ...
    def _calculate_f0(self, input):
        ...
        f0 = snd.to_pitch(
            ...
            pitch_floor=self.f0min,
            pitch_ceiling=self.f0max,
    ...
    def _adjust_num_frames(x, num_frames):
        if num_frames > len(x):
            ...
            x = F.pad(
                x,
                (
                    math.ceil((num_frames - len(x)) / 2),
                    math.floor((num_frames - len(x)) / 2),
                ),
            )
