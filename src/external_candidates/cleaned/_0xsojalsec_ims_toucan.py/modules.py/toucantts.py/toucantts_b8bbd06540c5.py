# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\ToucanTTS.py
import torch
...
from Utility.utils import initialize, make_non_pad_mask
...
class ToucanTTS(torch.nn.Module):
    ...
    def forward(
        ...
        """
        outs, stochastic_loss, duration_loss, pitch_loss, energy_loss = self._forward(
    ...
    def _forward(
        ...
        if is_inference:
            ...
            reduced_energy_space = torchfunc.dropout(
                self.energy_latent_reduction(encoded_texts + embedded_pitch_curve),
                p=0.1,
            ).transpose(1, 2)
            ...
            reduced_duration_space = torchfunc.dropout(
                self.duration_latent_reduction(encoded_texts + embedded_pitch_curve + embedded_energy_curve),
                p=0.1,
            ).transpose(1, 2)
            ...
            reduced_pitch_space = torchfunc.dropout(self.pitch_latent_reduction(encoded_texts), p=0.1).transpose(1, 2)
            pitch_loss, _ = self.pitch_predictor.compute_loss(
                mu=reduced_pitch_space,
            ...
            energy_loss, _ = self.energy_predictor.compute_loss(
                mu=reduced_energy_space,
            ...
            duration_loss, _ = self.duration_predictor.compute_loss(
                mu=reduced_duration_space,
        ...
        decoder_masks = (
            make_non_pad_mask(speech_lengths, device=speech_lengths.device).unsqueeze(-2)
        ...
        if is_inference:
            if run_stochastic:
                refined_codec_frames = self.flow_matching_decoder(
                    ...
                    mask=make_non_pad_mask([len(decoded_speech[0])], device=decoded_speech.device)
                    .unsqueeze(-2)
                    .float(),
                    n_timesteps=15,
                    temperature=0.2,
            ...
            if run_stochastic:
                stochastic_loss, _ = self.flow_matching_decoder.compute_loss(
                    x1=gold_speech.transpose(1, 2),
    ...
    def inference(
        ...
        outs, duration_predictions, pitch_predictions, energy_predictions = self._forward(
            text_pseudobatched,
            ilens,
...
if __name__ == "__main__":
    ...
    dummy_pitch = torch.Tensor([[[1.0], [0.0], [0.0]], [[1.1], [1.2], [0.8]], [[1.1], [1.2], [0.8]]])
    dummy_energy = torch.Tensor([[[1.0], [1.3], [0.0]], [[1.1], [1.4], [0.8]], [[1.1], [1.2], [0.8]]])
