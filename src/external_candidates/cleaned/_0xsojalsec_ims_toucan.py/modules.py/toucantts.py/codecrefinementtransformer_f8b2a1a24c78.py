# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Modules\ToucanTTS\CodecRefinementTransformer.py
import torch
...
class CodecRefinementTransformer(torch.nn.Module):
    ...
    def forward(
        ...
        sequence_of_continuous_tokens = self.indexes_per_codebook_to_stacked_embedding_vector(
        ...
        for head_index, classifier_head in enumerate(self.hierarchical_classifier):
            ...
            predicted_indexes_one_hot.append(
                classifier_head(torch.cat([contextualized_sequence] + backtranslated_indexes, dim=2))
            )
    ...
    def indexes_per_codebook_to_stacked_embedding_vector(self, index_sequence_per_codebook):
        ...
        for codebook_id, backtranslation_head in enumerate(self.backtranslation_heads):
            continuous_frame_sequences.append(
                backtranslation_head(index_sequence_per_codebook.transpose(0, 1)[codebook_id])
            )
...
if __name__ == "__main__":
    ...
    gold_pitch = torch.Tensor([[[1.0], [0.0], [0.0]], [[1.1], [1.2], [0.8]], [[1.1], [1.2], [0.8]]])
    gold_energy = torch.Tensor([[[1.0], [1.3], [0.0]], [[1.1], [1.4], [0.8]], [[1.1], [1.2], [0.8]]])
    ...
    loss = refiner(
        index_sequence=one_hot_sequence_to_token_sequence(gold_speech_batch.transpose(3, 2)).transpose(0, 1),
