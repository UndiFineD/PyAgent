# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\multilinguality\visualize_nearest_neighbors.py
import pickle
...
cache_root = "."
supervised_iso_codes = load_json_from_path(
    hf_hub_download(
        cache_dir=MODEL_DIR,
        repo_id="Flux9665/ToucanTTS",
        filename="supervised_languages.json",
    )
)
    ...
    for lang_1 in supervised_iso_codes:
        ...
        for lang_2 in supervised_iso_codes:
            if lang_2 not in seen_langs:  # it's symmetric
                l1_dist[lang_1][lang_2] = torch.nn.functional.mse_loss(
                    embedding_provider(torch.LongTensor([iso_codes_to_ids[lang_1]])).squeeze(),
                    embedding_provider(torch.LongTensor([iso_codes_to_ids[lang_2]])).squeeze(),
                )
...
if distance_type == "tree":
    tree_lookup_path = hf_hub_download(
        cache_dir=MODEL_DIR,
        repo_id="Flux9665/ToucanTTS",
        filename="lang_1_to_lang_2_to_tree_dist.json",
    )
    tree_dist = load_json_from_path(tree_lookup_path)
    distance_measure = tree_dist

if distance_type == "map":
    map_lookup_path = hf_hub_download(
        cache_dir=MODEL_DIR,
        repo_id="Flux9665/ToucanTTS",
        filename="lang_1_to_lang_2_to_map_dist.json",
    )
...
if distance_type == "learned":
    learned_lookup_path = hf_hub_download(
        cache_dir=MODEL_DIR,
        repo_id="Flux9665/ToucanTTS",
        filename="lang_1_to_lang_2_to_learned_dist.json",
    )
...
iso_codes_to_names = load_json_from_path(
    hf_hub_download(
        cache_dir=MODEL_DIR,
        repo_id="Flux9665/ToucanTTS",
...
for lang_1 in distance_measure:
    ...
    for lang_2 in distance_measure[lang_1]:
        ...
        distances.append(
            (
                iso_codes_to_names[lang_1],
                iso_codes_to_names[lang_2],
                distance_measure[lang_1][lang_2],
            )
        )
...
for entity1, entity2, d in tqdm(normalized_distances):
    if (neighbor == entity2 or neighbor == entity1) and (
        entity1 in supervised_iso_codes and entity2 in supervised_iso_codes
    ):
        if entity1 != entity2:
            d_dist.append(d)
...
neighbors = list()
for entity1, entity2, d in tqdm(normalized_distances):
    if (d < thresh and (neighbor == entity2 or neighbor == entity1)) and (
        entity1 in supervised_iso_codes and entity2 in supervised_iso_codes
    ):
...
for entity1, entity2, d in tqdm(normalized_distances):
    if (entity2 in unique_neighbors and entity1 in unique_neighbors) and (
        entity1 in supervised_iso_codes and entity2 in supervised_iso_codes
    ):
        if entity1 != entity2:
            spring_tension = 1 - d
...
nx.draw_networkx_edges(
    G,
    pos,
    edgelist=edges_connected_to_specific_node,
    edge_color="orange",
    alpha=0.3,
    width=3,
)
for u, v, d in edges:
    if u == neighbor or v == neighbor:
        nx.draw_networkx_edge_labels(
            G,
            pos,
            edge_labels={(u, v): round((thresh - (d["weight"] / 20000) ** (1 / 2)) * 10, 2)},
            font_color="red",
            alpha=0.3,
