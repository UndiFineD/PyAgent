...
class CacheCreator:
    ...
    def create_tree_cache(self, cache_root="."):
        ...
        for pair in tqdm(self.pairs, desc="Generating tree pairs"):
            ...
            depth_of_lca = len(
                set(iso_to_family_memberships[pair[0]]).intersection(set(iso_to_family_memberships[pair[1]]))
            )
        ...
        for pair in self.pair_to_tree_distance:
            if pair[0] == pair[1]:
                ...
                self.pair_to_tree_distance[pair] = (self.pair_to_tree_distance[pair] + abs(min_dist)) / (
    ...
    def create_required_files(self, model_path, create_oracle=False):
        if not os.path.exists(os.path.join(self.cache_root, "lang_1_to_lang_2_to_tree_dist.json")) or os.path.exists(
            hf_hub_download(
                cache_dir=MODEL_DIR,
                repo_id="Flux9665/ToucanTTS",
        ...
        if not os.path.exists(os.path.join(self.cache_root, "lang_1_to_lang_2_to_map_dist.json")) or os.path.exists(
            hf_hub_download(
                cache_dir=MODEL_DIR,
                repo_id="Flux9665/ToucanTTS",
        ...
        if not os.path.exists(os.path.join(self.cache_root, "asp_dict.pkl")) or os.path.exists(
            hf_hub_download(
                cache_dir=MODEL_DIR,
                repo_id="Flux9665/ToucanTTS",
        ...
        if not os.path.exists(os.path.join(self.cache_root, "lang_1_to_lang_2_to_learned_dist.json")) or os.path.exists(
            hf_hub_download(
                cache_dir=MODEL_DIR,
                repo_id="Flux9665/ToucanTTS",
