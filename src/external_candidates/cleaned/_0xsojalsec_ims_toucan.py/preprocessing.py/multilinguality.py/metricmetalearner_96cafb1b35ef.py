# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\multilinguality\MetricMetaLearner.py
import json
import os
import pickle
import random
...
def create_learned_cache(model_path, cache_root="."):
    ...
    for m in range(n_models):
        ...
        for epoch in tqdm(range(25), desc=f"MetricsCombiner {m + 1}/{n_models} - Epoch"):
            for i in range(2000):
                ...
                loss = (
                    loss / (target_batch + 0.0001)
                )  # the smaller the distance, the higher the impact. We don't care about the large distances, only the nearest neighbors are important
                loss = loss.mean()
    ...
    for i in range(100):
        ...
        loss = torch.nn.functional.mse_loss(
            scores.squeeze(), torch.stack(embedding_distance_batch).squeeze().to(DEVICE)
        )
