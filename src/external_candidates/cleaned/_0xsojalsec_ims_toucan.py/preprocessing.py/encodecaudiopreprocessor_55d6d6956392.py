# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Preprocessing\EnCodecAudioPreprocessor.py
...
def remove_encodec_weight_norm(model):
    from Preprocessing.Codec.seanet import SConv1d, SConvTranspose1d, SEANetResnetBlock
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        ...
        print(t1 - t0)
        soundfile.write(
            file=f"../audios/1_reconstructed_in_{t1 - t0}_encodec.wav",
            data=audio1,
            samplerate=16000,
        )
        soundfile.write(
            file=f"../audios/2_reconstructed_in_{t1 - t0}_encodec.wav",
            data=audio2,
            samplerate=16000,
        )
        soundfile.write(
            file=f"../audios/3_reconstructed_in_{t1 - t0}_encodec.wav",
            data=audio3,
            samplerate=16000,
        )
        soundfile.write(
            file=f"../audios/4_reconstructed_in_{t1 - t0}_encodec.wav",
            data=audio4,
            samplerate=16000,
        )
