# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Recipes\BigVGAN_combined.py
import time
...
def run(
    ...
    fl = list(
        build_path_to_transcript_RAVDESS().keys()
    )  # these three datasets are kind of important to represent some out-of-distribution data for what we expect.
    wav, sr = sf.read(fl[0])
    if sr >= 24000 or take_all:
        selection += fl
    ...
    fl = list(
        build_path_to_transcript_ears().keys()
    )  # these three datasets are kind of important to represent some out-of-distribution data for what we expect.
    wav, sr = sf.read(fl[0])
    if sr >= 24000 or take_all:
        selection += fl
