# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Utility\path_to_transcript_dicts.py
import glob
import json
import os
import random
...
def indic_voices_template(root, lang):
    ...
    for jpath in [
        f"{root}/{lang}/metadata_test.json",
    ...
    for transcript_dir in os.listdir("/mount/resources/speech/corpora/VCTK/txt"):
        for transcript_file in os.listdir(f"/mount/resources/speech/corpora/VCTK/txt/{transcript_dir}"):
            if transcript_file.endswith(".txt"):
                with open(
                    f"/mount/resources/speech/corpora/VCTK/txt/{transcript_dir}/" + transcript_file,
                    "r",
                    encoding="utf8",
                ) as tf:
                    transcript = tf.read()
...
def build_path_to_transcript_jenny():
    ...
    for line in trans_lines:
        if line.strip() != "":
            path_to_transcript["/mount/resources/speech/corpora/Jenny/" + line.split("|")[0] + "_silence.flac"] = (
                line.split("|")[1]
            )
    return path_to_transcript
...
def build_path_to_transcript_thorsten_neutral():
    ...
    for line in trans_lines:
        if line.strip() != "":
            path_to_transcript[root + "/wavs/" + line.split("|")[0] + ".wav"] = line.split("|")[1]
    return path_to_transcript
...
def build_path_to_transcript_css10fr():
    ...
    for line in trans_lines:
        if line.strip() != "":
            path_to_transcript[f"/mount/resources/speech/corpora/CSS10/{language}/{line.split('|')[0]}"] = line.split(
                "|"
            )[2]
    return path_to_transcript
    ...
    for data_dir in os.listdir(root):
        if os.path.isdir(os.path.join(root, data_dir)):
            for data_type in ["parallel100", "nonpara30"]:
                with open(
                    os.path.join(root, data_dir, data_type, "transcripts_utf8.txt"),
                    encoding="utf8",
                ) as f:
                    transcriptions = f.read()
                for line in transcriptions.split("\n"):
...
def build_path_to_transcript_alffa_sw():
    ...
    path_to_transcript = build_path_to_transcript_kaldi_template(
        root=root, split="train", replace_in_path=("asr_swahili/data/", "")
    )
    path_to_transcript.update(
        build_path_to_transcript_kaldi_template(root=root, split="test", replace_in_path=("/my_dir/wav", "test/wav5"))
    )
    return path_to_transcript
...
def build_path_to_transcript_alffa_am():
    ...
    path_to_transcript.update(
        build_path_to_transcript_kaldi_template(
...
def build_path_to_transcript_iban():
    ...
    path_to_transcript = build_path_to_transcript_kaldi_template(root, "train", replace_in_path=("asr_iban/data/", ""))
    path_to_transcript.update(
        build_path_to_transcript_kaldi_template(root, "dev", replace_in_path=("asr_iban/data/", ""))
    )
    return path_to_transcript
