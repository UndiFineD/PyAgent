# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\Utility\corpus_preparation.py
...
def prepare_aligner_corpus(transcript_dict, corpus_dir, lang, device, phone_input=False, gpu_count=1, rank=0):
    return CodecAlignerDataset(
        transcript_dict,
    ...
    if not os.path.exists(os.path.join(corpus_dir, "tts_train_cache.pt")):
        if fine_tune_aligner:
            ...
            if not os.path.exists(os.path.join(aligner_dir, "aligner.pt")):
                aligner_datapoints = prepare_aligner_corpus(
                    transcript_dict,
                    corpus_dir=corpus_dir,
                    lang=lang,
                    phone_input=phone_input,
                ...
                train_aligner(
                    ...
                    path_to_checkpoint=hf_hub_download(
                        cache_dir=MODEL_DIR,
