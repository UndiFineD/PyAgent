...
def plot_code_spec(pitch, energy, sentence, durations, mel, save_path, tf, step):
    ...
    plt.subplots_adjust(left=0.05, bottom=0.1, right=0.95, top=0.95, wspace=0.0, hspace=0.0)
...
def delete_old_checkpoints(checkpoint_dir, keep=5):
    ...
    if len(checkpoint_list) <= keep:
        ...
        checkpoints_to_delete = [
            os.path.join(checkpoint_dir, "checkpoint_{}.pt".format(step)) for step in checkpoint_list[:-keep]
        ]
