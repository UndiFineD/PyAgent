    ...
    def run_tts(self):
        ...
        while self.tts_update_required:
            ...
            forced_durations = (
                ...
                else torch.LongTensor(insert_zeros_at_indexes(self.durations, self.word_boundaries)).unsqueeze(0)
            )
            forced_pitch = (
                None
                if self.pitch is None or len(self.pitch) != len(self.phonemes)
                else torch.tensor(insert_zeros_at_indexes(self.pitch, self.word_boundaries)).unsqueeze(0)
            )
