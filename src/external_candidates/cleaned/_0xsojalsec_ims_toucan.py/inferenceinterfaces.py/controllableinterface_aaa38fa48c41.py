# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\InferenceInterfaces\ControllableInterface.py
import os
...
class ControllableInterface:
    def __init__(
        self,
        gpu_id="cpu",
        available_artificial_voices=50,
        tts_model_path=None,
        ...
        if embedding_gan_path is None:
            embedding_gan_path = hf_hub_download(
                cache_dir=MODEL_DIR,
        ...
        self.wgan = GanWrapper(
            embedding_gan_path,
            num_cached_voices=available_artificial_voices,
            device=self.device,
        )
    ...
    def read(
        ...
        if len(phones) > 1800:
            if language == "deu":
                ...
                prompt = (
                    "Su entrada es demasiado larga. Por favor, intente un texto más corto o divídalo en varias partes."
                )
            elif language == "fin":
                ...
                prompt = (
                    "Túl hosszú volt a bevitele. Kérjük, próbáljon meg rövidebb szöveget írni, vagy ossza több részre."
                )
            elif language == "nld":
