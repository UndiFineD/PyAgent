# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IMS-Toucan\InferenceInterfaces\UtteranceCloner.py
import librosa
import numpy
...
class UtteranceCloner:
    ...
    def __init__(self, model_id, device, language="eng"):
        ...
        self.device = device
        acoustic_checkpoint_path = hf_hub_download(
            cache_dir=MODEL_DIR, repo_id="Flux9665/ToucanTTS", filename="Aligner.pt"
        )
    ...
    def extract_prosody(self, transcript, ref_audio_path, lang="eng", on_line_fine_tune=True):
        ...
        if on_line_fine_tune:
            ...
            tokens = self.tf.text_vectors_to_id_sequence(
                text_vector=text
            )  # we need an ID sequence for training rather than a sequence of phonological features
        ...
        alignment_path = self.acoustic_model.inference(
        ...
        energy = (
            self.energy_calc(
                ...
                durations_lengths=torch.LongTensor([len(duration)]),
            )[0]
            .squeeze(0)
            .cpu()
        )
    ...
    def clone_utterance(
        ...
        duration, pitch, energy, silence_frames_start, silence_frames_end = self.extract_prosody(
            transcription_of_intonation_reference,
            path_to_reference_audio_for_intonation,
            lang=lang,
        )
        self.tts.set_language(lang)
        start_sil = numpy.zeros(
            [int(silence_frames_start * 1.5)]
        )  # timestamps are from 16kHz, but now we're using 24000Hz, so upsampling required
        end_sil = numpy.zeros(
            [int(silence_frames_end * 1.5)]
        )  # timestamps are from 16kHz, but now we're using 24000Hz, so upsampling required
        cloned_speech, sr = self.tts(
            transcription_of_intonation_reference,
            view=False,
            durations=duration,
        ...
        if filename_of_result is not None:
            sf.write(
                file=filename_of_result,
                data=float2pcm(cloned_utt),
                samplerate=sr,
                subtype="PCM_16",
            )
