# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_market_claude_code_skills.py\transcript_fixer.py\scripts.py\cli.py\argument_parser_1ba7e10c38bc.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-market-claude-code-skills\transcript-fixer\scripts\cli\argument_parser.py
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--init", action="store_true", help="Initialize ~/.transcript-fixer/")
    ...
    parser.add_argument("--domain", "-d", default="general", help="Correction domain")

    # Learning commands

    parser.add_argument("--review-learned", action="store_true", help="Review learned suggestions")

    parser.add_argument("--approve", nargs=2, metavar=("FROM", "TO"), help="Approve suggestion")

    # Utility commands

    parser.add_argument("--validate", action="store_true", help="Validate configuration and JSON files")

    parser.add_argument("--health", action="store_true", help="Perform system health check (P1-4 fix)")
    ...
    parser.add_argument("--metrics", action="store_true", help="Display collected metrics (P1-7 fix)")
    ...
    parser.add_argument("--config-path", dest="config_path", help="Path for config file operations")
