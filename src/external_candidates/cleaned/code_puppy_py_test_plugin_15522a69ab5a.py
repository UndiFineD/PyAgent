# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\plugins.py\chatgpt_oauth.py\test_plugin_15522a69ab5a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\chatgpt_oauth\test_plugin.py
...
def test_jwt_parsing_with_nested_org():
    ...
    if organizations:
        default_org = next((org for org in organizations if org.get("is_default")), organizations[0])
...
def test_save_and_load_tokens(tmp_path):
    ...
    with patch.object(config, "get_token_storage_path", return_value=tmp_path / "tokens.json"):
        tokens = {
...
def test_save_and_load_chatgpt_models(tmp_path):
    ...
    with patch.object(config, "get_chatgpt_models_path", return_value=tmp_path / "chatgpt_models.json"):
        models = {
            "chatgpt-gpt-4o": {
                "type": "openai",
                "name": "gpt-4o",
