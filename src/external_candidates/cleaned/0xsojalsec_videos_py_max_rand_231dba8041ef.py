# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2024.py\puzzles.py\max_rand_231dba8041ef.py
...
class MaxProcess(InteractiveScene):
    def construct(self):
        ...
        x1_group, x2_group, max_group = groups = Group(
            get_random_var_label_group(interval, "", color=color) for interval, color in zip(intervals, colors)
        )
...
class VisualizeMaxOfPairCDF(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                (ReplacementTransform(max_lines.copy().set_stroke(opacity=0), mlc) for mlc in inner_lines),
                lag_ratio=0.01,
                run_time=8,
            ),
            Animation(xy_dot),
