# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kesslerio.py\finance_news.py\tests.py\test_earnings_450171f03a02.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\finance-news\tests\test_earnings.py
...
def test_get_briefing_section_output():
    ...
    with (
        patch("earnings.load_portfolio", return_value=mock_portfolio),
...
def test_refresh_earnings_force(mock_finnhub_response):
    ...
    with (
        patch("earnings.get_finnhub_key", return_value="fake_key"),
        patch(
            "earnings.fetch_all_earnings_finnhub",
            return_value={"AAPL": mock_finnhub_response["earningsCalendar"][0]},
        ),
