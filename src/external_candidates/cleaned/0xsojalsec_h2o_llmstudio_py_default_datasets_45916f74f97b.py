# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\app_utils.py\default_datasets_45916f74f97b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\default_datasets.py
...
def prepare_default_dataset_causal_language_modeling(path):
    ...
    df_assistant = df_assistant[["instruction", "output", "message_id", "parent_id", "lang", "rank"]].rename(
    ...
    df_assistant[df_assistant["lang"] == "en"][["instruction", "output", "id", "parent_id"]].to_parquet(
        os.path.join(path, "train_full_allrank.pq"), index=False
    )

    df_assistant[df_assistant["rank"] == 0.0][["instruction", "output", "id", "parent_id"]].to_parquet(
        os.path.join(path, "train_full_multilang.pq"), index=False
    )
...
def prepare_default_dataset_dpo_modeling() -> pd.DataFrame:
    if os.environ.get("H2O_LLM_STUDIO_DEMO_DATASETS") is None:
        ...
        df = pd.read_parquet(os.path.join(os.environ.get("H2O_LLM_STUDIO_DEMO_DATASETS"), "dpo_modeling.pq"))
