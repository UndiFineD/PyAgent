# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\retriever.py\chunkretriever_dc5c67eb67c2.py
...
class ChunkRetriever(BaseRetriever):
    ...
    async def _find_relevant_chunks_from_entity_occurrence(self, node_datas: list[dict]):
        ...
        edges = await asyncio.gather(*[self.graph.get_node_edges(dp["entity_name"]) for dp in node_datas])
        ...
        all_one_hop_nodes_data = await asyncio.gather(*[self.graph.get_node(e) for e in all_one_hop_nodes])
    ...
    async def _find_relevant_chunks_from_relationships(self, seed: list[dict]):
        text_units = [split_string_by_multi_markers(dp["source_id"], [GRAPH_FIELD_SEP]) for dp in seed]
