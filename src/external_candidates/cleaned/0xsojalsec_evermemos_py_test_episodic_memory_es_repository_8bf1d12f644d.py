# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\tests.py\test_episodic_memory_es_repository_8bf1d12f644d.py
...
async def test_search_and_filter():
    ...
    try:
        ...
        results = await repo.multi_search(query=["technology", "project"], user_id=test_user_id, size=10, explain=True)
        ...
        user_results = await repo.multi_search(
            query=[],
            user_id=test_user_id,
            size=20,  # Empty query, pure filtering
        )

        assert len(user_results) >= 5, f"At least 5 user records should be found, actually found {len(user_results)}"
        ...
        group_results = await repo.multi_search(query=[], user_id=test_user_id, group_id=test_group_id, size=10)

        assert len(group_results) >= 3, f"At least 3 group records should be found, actually found {len(group_results)}"
        ...
        type_results = await repo.multi_search(query=[], user_id=test_user_id, event_type="Conversation", size=10)
        ...
        keyword_results = await repo.multi_search(query=[], user_id=test_user_id, keywords=["technology"], size=10)
        ...
        time_results = await repo.multi_search(query=[], user_id=test_user_id, date_range=date_range, size=10)

        assert len(time_results) >= 2, (
        ...
        assert len(timerange_results) >= 5, (
        ...
        try:
            cleanup_count = await repo.delete_by_filters(user_id=test_user_id, refresh=True)
...
async def test_delete_operations():
    ...
    try:
        ...
        deleted_count = await repo.delete_by_filters(user_id=test_user_id, group_id=test_group_id, refresh=True)
        ...
        deleted_count = await repo.delete_by_filters(user_id=test_user_id, date_range=date_range, refresh=True)
        ...
        remaining_count = await repo.delete_by_filters(user_id=test_user_id, refresh=True)
...
async def test_timezone_handling():
    ...
    try:
        ...
        assert retrieved_doc.timestamp.astimezone(ZoneInfo("UTC")).replace(microsecond=0) == utc_time.replace(
            microsecond=0
        )
        ...
        shanghai_time = get_now_with_timezone(ZoneInfo("Asia/Shanghai"))  # Current Shanghai time
        ...
        if len(time_results) == 0:
            ...
            fallback_results = await repo.multi_search(query=[], user_id=test_user_id, size=10)
...
async def test_edge_cases():
    ...
    try:
        ...
        future_results = await repo.multi_search(query=[], user_id=test_user_id, date_range=invalid_date_range, size=10)
...
async def test_multi_search():
    ...
    try:
        ...
        results = await repo.multi_search(query=["DSL", "search"], user_id=test_user_id, size=10, explain=True)
        ...
        results = await repo.multi_search(query=[], user_id=test_user_id, event_type="TestDSL", size=10)
        ...
        results = await repo.multi_search(query=["BM25", "preference"], user_id=test_user_id, size=10, explain=True)
        ...
        results = await repo.multi_search(query=[], user_id=test_user_id, event_type="TestBM25", size=10)
        ...
        bm25_results = await repo.multi_search(query=["preference", "test"], user_id=test_user_id, size=10)
