# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\mcp.py\start_command_646bcc424095.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\mcp\start_command.py
        ...
        try:
            ...
            try:
                if hasattr(self.manager, "get_server_by_name"):
                    ...
                    server_type = getattr(server_config, "type", "unknown") if server_config else "unknown"
            ...
            if success:
                if server_type == "stdio":
                    ...
                    emit_info(
                        Text.from_markup("[dim]Tip: Use /mcp status to check if the server is fully initialized[/dim]"),
                        message_group=group_id,
                    )
