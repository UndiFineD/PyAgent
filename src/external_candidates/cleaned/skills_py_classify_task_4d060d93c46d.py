# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\digitaladaption.py\model_router.py\scripts.py\classify_task_4d060d93c46d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\digitaladaption\model-router\scripts\classify_task.py
...
def classify_task(task: str) -> Dict[str, Optional[str]]:
    ...
    cost_priority = {"haiku-4.5": 0, "sonnet": 1, "codex-5.2": 2, "opus": 3}

    # Sort by score, then by cost (prefer cheaper models for equal scores)

    sorted_models = sorted(scores.items(), key=lambda x: (-x[1]["score"], cost_priority.get(x[0], 4)))
...
def format_output(classification: Dict, format: str = "text") -> str:
    ...
    if format == "json":
        ...
        lines.append(
            f'  sessions_spawn --task "{classification.get("task", "your task")}" --model {classification["model"] or "GLM"}'
        )
...
def main():
    parser = argparse.ArgumentParser(description="Classify task and recommend optimal model")
