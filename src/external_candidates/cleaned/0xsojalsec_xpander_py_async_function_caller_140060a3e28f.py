# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_xpander.py\getting_started.py\hello_world.py\tools.py\async_function_caller_140060a3e28f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-xpander.ai\Getting-Started\hello-world\tools\async_function_caller.py
...
async def execute_local_functions(
    ...
    results = await asyncio.gather(*(executor(call, functions_by_name) for call in function_calls))
...
async def _execute_single_function(function_call: Any, functions_by_name: Dict[str, Callable]) -> ToolCallResult:
    ...
    logger.info(f"🔦 Requesting function: {function_call.name} with payload: {function_call.payload}")
    ...
    def _run_function():
        ...
        params = {k: v for k, v in function_call.payload.items()}
