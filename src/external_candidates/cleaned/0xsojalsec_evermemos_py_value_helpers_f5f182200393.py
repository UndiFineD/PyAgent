# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\profile_memory.py\value_helpers_f5f182200393.py
    ...
    *,
    ...
    def add_entry(key: Any, evidence_list: Any, level: Any = None) -> None:
        ...
        for item in evidence_items:
            formatted = format_evidence_entry(item, conversation_date_map=conversation_date_map)
            ...
            if valid_conversation_ids is not None:
                ...
                if not conversation_id or conversation_id not in valid_conversation_ids:
                    logger.debug("LLM Generated Unknown Conversation ID %s", conversation_id)
        ...
        level_str = (
            level.strip() if isinstance(level, str) else str(level).strip() if level is not None and level != "" else ""
        )
    ...
    if isinstance(raw_value, dict):
        ...
        for entry in raw_value:
            if isinstance(entry, dict):
                if "value" in entry:
                    add_entry(entry.get("value"), entry.get("evidences"), entry.get("level"))
