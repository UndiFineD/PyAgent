# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\actions.py\customize_action_9b04367fe8ac.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\actions\customize_action.py
...
class CustomizeAction(Action):
    parse_mode: Optional[str] = Field(
        default="title",
        description="the parse mode of the action, must be one of: ['title', 'str', 'json', 'xml', 'custom']",
    )

    parse_func: Optional[Callable] = Field(
        default=None,
        exclude=True,
        description="the function to parse the LLM output. It receives the LLM output and returns a dict.",
    )

    title_format: Optional[str] = Field(
        default="## {title}",
        exclude=True,
        description="the format of the title. It is used when the `parse_mode` is 'title'.",
    )

    custom_output_format: Optional[str] = Field(
        default=None,
        exclude=True,
        description="the format of the output. It is used when the `prompt_template` is provided.",
    )
    ...
    def __init__(self, **kwargs):
        ...
        if self.prompt and self.prompt_template:
            logger.warning(
                "Both `prompt` and `prompt_template` are provided for CustomizeAction action. Prioritizing `prompt_template` and ignoring `prompt`."
            )
        ...
        for toolkit in tools:
            try:
                ...
                if not isinstance(tool_callers, list):
                    logger.warning(
                        f"Expected list of tool functions from '{toolkit.name}.get_tools()', got {type(tool_callers)}."
                    )
                ...
                for tool_caller in tool_callers:
                    ...
                    if tool_caller_name in self.tools_caller:
                        logger.warning(
                            f"Duplicate tool function '{tool_caller_name}' detected. Overwriting previous function."
                        )
                ...
                if valid_tools_count > 0:
                    self.tools_caller.update(
                        {name: caller for name, caller in zip(valid_tools_names, valid_tool_callers)}
                    )
                ...
                if valid_tools_count > 0:
                    logger.info(
                        f"Added toolkit '{toolkit.name}' with {valid_tools_count} valid tools in {self.name}: {valid_tools_names}."
                    )
        ...
        **kwargs,
        ...
        if self.prompt_template:
            if isinstance(self.prompt_template, ChatTemplate):
                ...
                conversation = [
                    {
                        "role": "system",
                        "content": self.prepare_action_prompt(inputs=inputs, system_prompt=sys_msg),
                    }
                ]

            else:
                raise ValueError(
                    f"`prompt_template` must be a StringTemplate or ChatTemplate instance, but got {type(self.prompt_template)}"
                )
        ...
        while True:
            ...
            if time_out > self.max_tool_try:
                ...
                content_to_extract = (
                    final_llm_response if final_llm_response is not None else "{content}".format(content=conversation)
                )

                if return_prompt:
                    return (
                        self._extract_output(content_to_extract, llm=llm),
                        current_prompt,
                    )
            ...
            conversation.append(
                {
                    "role": "assistant",
                    "content": TOOL_CALLING_HISTORY_PROMPT.format(
        ...
        **kwargs,
        ...
        while True:
            ...
            if time_out > self.max_tool_try:
                ...
                if return_prompt:
                    return (
                        await self._async_extract_output(content_to_extract, llm=llm),
                        current_prompt,
                    )
