# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\env.py\gym_env.py\observation_cc658e97c197.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\gym_env\observation.py
...
class Observation:
    ...
    def from_dict(cls, obs_dict: Dict[str, Any]) -> "Observation":
        ...
        transformed_flows = {
            ...
            "static_items": (
                {item["type"]: item["value"] for item in flows_dict.get("static_items", [])}
        ...
        messages = [
            AgentMessage(sender=msg["sender"], content=msg["content"], timestamp=msg["timestamp"])
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "research": {
                ...
                "current_research": (
                    self.research.current_research if self.research.current_research is not None else "None"
                ),
            ...
            "task_verification": (
                {
                    ...
                    "meta": [{"key": k, "value": json.dumps(v)} for k, v in self.task_verification.meta.items()],
                }
            ...
            "task_info": {
                "goal_description": (self.task_info.goal_description if self.task_info else ""),
                "agent_instructions": (
                    (self.task_info.agent_instructions or "") if self.task_info else ""
                ),  # Convert None to empty string for gym space
                "task_key": self.task_info.task_key if self.task_info else "",
                "trajectory_length": (self.task_info.trajectory_length if self.task_info else 0),
