# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\brachistochrone.py\graveyard_1aba730c29b9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\graveyard.py
...
class MultilayeredGlass(PhotonScene, ZoomedScene):
    ...
    def construct(self):
        ...
        photon_run = self.photon_run_along_path(self.discrete_path, run_time=1, rate_func=rush_into)
    ...
    def continuous_to_smooth(self):
        ...
        self.play(ShowCreation(continuous, rate_func=lambda t: smooth(1 - t)))
    ...
    def generate_layer_info(self):
        ...
        top_rgb, bottom_rgb = [np.array(Color(color).get_rgb()) for color in (self.top_color, self.bottom_color)]

        epsilon = 1.0 / (self.num_discrete_layers - 1)

        self.layer_colors = [
            Color(rgb=interpolate(top_rgb, bottom_rgb, alpha)) for alpha in np.arange(0, 1 + epsilon, epsilon)
        ]
    ...
    def generate_discrete_path(self):
        ...
        self.discrete_path = Mobject1D(color=YELLOW, density=3 * DEFAULT_POINT_DENSITY_1D)
        ...
        for start, end in zip(indices, indices[1:]):
            ...
            self.path_angles.append(angle_of_vector(start_point - end_point) - np.pi / 2)
    ...
    def show_layer_variables(self):
        ...
        for (top1, top2), x in zip(layer_top_pairs, it.count(1)):
            eq_mob = OldTex(["v_%d" % x, "=", "\sqrt{\phantom{y_1}}"], size="\\Large")
            ...
            center_paths.append(Line(midpoint + FRAME_X_RADIUS * LEFT, midpoint + FRAME_X_RADIUS * RIGHT))

            brace_endpoints = Mobject(Point(self.top * UP + x * RIGHT), Point(top2 * UP + x * RIGHT))
        ...
        for v_eq, path, time in zip(v_equations, center_paths, [2, 1, 0.5]):
            photon_run = self.photon_run_along_path(path, rate_func=linear)

            self.play(ShimmerIn(v_eq[0]), photon_run, run_time=time)
        ...
        for v_eq, start_y, end_y, brace in quads:
            ...
            self.play(ShowCreation(v_eq[1]), ShowCreation(v_eq[2]), Transform(start_y, end_y))
    ...
    def isolate_bend_points(self):
        ...
        for index in range(3):
            ...
            for i in [index + 1, index + 2]:
                ...
                theta.next_to(Point(bend_point), horiz, buff=0.01)
            ...
            equation_frame.shift(little_square.get_corner(UP + RIGHT) - equation_frame.get_corner(UP + RIGHT))
    ...
    def show_snells(self, index, frame):
        ...
        for x, numerator in [(index, left), (index + 1, right)]:
            v, sqrt_y = [
                OldTex(text, size="\\Large").next_to(numerator, DOWN) for text in ("v_%d" % x, "\\sqrt{y_%d}" % x)
            ]
        ...
        start, end = [
            Mobject(left.copy(), mobs[0], equals.copy(), right.copy(), mobs[1]).replace(frame) for mobs in (vs, sqrt_ys)
        ]
        ...
        self.play(Transform(start, end, path_func=counterclockwise_path()))
    ...
    def show_main_equation(self):
        ...
        self.equation.shift((self.layer_tops[0] - self.equation.get_top()) * UP)
    ...
    def ask_continuous_question(self):
        ...
        self.play(ShowCreation(continuous), Animation(self.equation))
        ...
        indices = np.arange(0, self.cycloid.get_num_points() - 1, 10)
