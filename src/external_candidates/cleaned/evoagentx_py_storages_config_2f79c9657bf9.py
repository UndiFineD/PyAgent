# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\storages.py\storages_config_2f79c9657bf9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\storages\storages_config.py
...
class DBConfig(BaseConfig):
    ...
    db_name: str = Field(
        default="sqlite",
        description="Name of the database provider (e.g., 'sqlite', 'posgre_sql')",
    )
...
class VectorStoreConfig(BaseConfig):
    ...
    vector_name: str = Field(
        default="faiss",
        description="Name of the vector database provider (e.g., 'faiss', 'qdrant')",
    )
    ...
    index_type: Optional[str] = Field(
        default="flat_l2",
        description="Index type for FAISS (e.g., 'flat_l2', 'ivf_flat')",
    )

    qdrant_url: Optional[str] = Field(
        default=None,
        description="URL for Qdrant server (e.g., 'http://localhost:6333')",
    )
    ...
    qdrant_collection_name: Optional[str] = Field(
        default="default_collection", description="Name of the Qdrant collection"
    )
...
class GraphStoreConfig(BaseConfig):
    ...
    graph_name: str = Field(
        default="neo4j",
        description="Name of the graph database provider (e.g., 'neo4j')",
    )

    uri: Optional[str] = Field(
        default=None,
        description="URI for Neo4j database (e.g., 'bolt://localhost:7687')",
    )
