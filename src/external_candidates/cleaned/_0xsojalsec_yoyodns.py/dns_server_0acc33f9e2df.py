# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-yoyoDNS\dns-server.py
#!/usr/bin/env python3
...
def process_command_line_args():
    ...
    parser.add_argument("-s", "--server", help="IP address for listen. Default: all addresses")
    parser.add_argument("-p", "--port", help="Port for listen. Default: 53")
    parser.add_argument("-v", "--verbose", action="store_true", help="Print verbose output")
    parser.add_argument("-d", "--debug", action="store_true", help='Same as "--verbose"')
    parser.add_argument("-f", "--file", help='The zone file to use. Default: "./test.com.zone"')
...
def handle_connection(sock, is_tcp):
    ...
    if DEBUG_MODE:
        ...
        print("\n{} query: {} {} {} from {}#{}, id: {}".format(transport, qname, qtype, qclass, addr, port, query.id))
...
def main():
    ...
    while True:
        ...
        for fd in fd_read_ready:
            threading.Thread(target=handle_connection, args=(sockets[fd], is_tcp_dict[fd])).start()
