# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-yoyoDNS\measurements.py
#!/usr/bin/env python3
...
from ripe.atlas.cousteau import AtlasCreateRequest, AtlasSource, Dns, Traceroute
...
parser = argparse.ArgumentParser(
    description="Generate measurements to understand where DNS resolvers are in a country or ASN"
)
        ...
        if probes["count"] > 0:
            for probe in probes["results"]:
                ...
                if probe["asn_v4"] is not None:
                    ...
                    print(
                        "Host for probe {} in AS{} would be: {}.valid.keyrollover.ch".format(
                            probe["id"], probe["asn_v4"], host_uuid
                        )
                    )
                    source = AtlasSource(requested=1, type="probes", value=probe["id"])
                    dns = Dns(
                        af=4,
                    ...
                    while done == False:
                        ...
                        if "measurements" in response:
                            ...
                            query = "INSERT INTO measurements values('{}', '{}', '{}', '{}', '{}','{}','4')".format(
                                host_uuid,
                                asn,
                                ts,
                ...
                if probe["asn_v6"] is not None:
                    ...
                    print(
                        "Host for probe {} in AS{} would be: {}.valid.keyrollover.ch".format(
                            probe["id"], probe["asn_v6"], host_uuid
                        )
                    )
                    source = AtlasSource(requested=1, type="probes", value=probe["id"])
                    dns = Dns(
                    ...
                    while done == False:
                        ...
                        if "measurements" in response:
                            ...
                            query = "INSERT INTO measurements values('{}', '{}', '{}', '{}', '{}', '{}', '6')".format(
                                host_uuid,
                                asn,
                                ts,
                                response["measurements"][0],
