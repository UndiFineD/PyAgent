# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\tests.py\tools.py\telephony.py\test_transfer_tool_74189e42c6f1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\tests\tools\telephony\test_transfer_tool.py
...
class TestTransferCallTool:
    ...
    @pytest.mark.asyncio
    async def test_transfer_by_extension_number(self, transfer_tool, tool_context, mock_ari_client):
        """Test transfer to extension by number (e.g., '6000')."""

        result = await transfer_tool.execute(parameters={"target": "6000", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_by_department_alias(self, transfer_tool, tool_context, mock_ari_client):
        """Test transfer using department alias (e.g., 'support')."""

        result = await transfer_tool.execute(parameters={"target": "support", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_multiple_aliases(self, transfer_tool, tool_context):
        """Test that all aliases resolve to correct extension."""

        aliases = ["support", "agent", "human", "representative"]

        for alias in aliases:
            result = await transfer_tool.execute(parameters={"target": alias, "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_to_sales_department(self, transfer_tool, tool_context, mock_ari_client):
        """Test transfer to sales department."""

        result = await transfer_tool.execute(parameters={"target": "sales", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_to_technical_support(self, transfer_tool, tool_context):
        """Test transfer to technical support with longer timeout."""

        result = await transfer_tool.execute(parameters={"target": "tech", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_invalid_target(self, transfer_tool, tool_context):
        """Test transfer to non-existent extension."""

        result = await transfer_tool.execute(parameters={"target": "9999", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_empty_target(self, transfer_tool, tool_context):
        """Test transfer with empty target."""

        # Empty target should return error (not raise exception)

        result = await transfer_tool.execute(parameters={"target": "", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_warm_transfer_mode(self, transfer_tool, tool_context, mock_ari_client):
    ...
    @pytest.mark.asyncio
    async def test_blind_transfer_mode(self, transfer_tool, tool_context, mock_ari_client):
        """Test blind transfer mode (immediate redirect)."""

        # Use extension 7000 which has mode: blind in config

        result = await transfer_tool.execute(parameters={"target": "7000"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_default_transfer_mode_is_warm(self, transfer_tool, tool_context):
        """Test that default mode is warm when not specified."""

        result = await transfer_tool.execute(parameters={"target": "6000"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_ari_channel_origination(self, transfer_tool, tool_context, mock_ari_client):
        """Test ARI channel origination parameters."""

        await transfer_tool.execute(parameters={"target": "6000", "mode": "warm"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_stasis_app_parameter(self, transfer_tool, tool_context, mock_ari_client):
    ...
    @pytest.mark.asyncio
    async def test_ari_originate_failure(self, transfer_tool, tool_context, mock_ari_client):
    ...
    @pytest.mark.asyncio
    async def test_transfer_context_preserved(self, transfer_tool, tool_context, mock_ari_client):
    ...
    @pytest.mark.asyncio
    async def test_transfer_uses_config_dial_string(self, transfer_tool, tool_context, mock_ari_client):
    ...
    @pytest.mark.asyncio
    async def test_transfer_timeout_from_config(self, transfer_tool, tool_context):
        """Test that timeout is read from extension config."""

        # Extension 6002 has timeout: 45

        result = await transfer_tool.execute(parameters={"target": "6002", "mode": "warm"}, context=tool_context)
    ...
    async def test_missing_target_parameter(self, transfer_tool, tool_context):
        ...
        with pytest.raises(Exception):  # Should raise validation error
            await transfer_tool.execute(
                parameters={"mode": "warm"},
                context=tool_context,  # Missing target
            )
    ...
    async def test_invalid_mode_parameter(self, transfer_tool, tool_context):
        ...
        with pytest.raises(ValueError, match="Invalid value for mode"):
            await transfer_tool.execute(parameters={"target": "6000", "mode": "invalid"}, context=tool_context)

    # ==================== Integration Scenarios ====================

    @pytest.mark.asyncio
    async def test_transfer_during_active_conversation(self, transfer_tool, tool_context, sample_call_session):
    ...
    @pytest.mark.asyncio
    async def test_case_insensitive_alias_matching(self, transfer_tool, tool_context):
