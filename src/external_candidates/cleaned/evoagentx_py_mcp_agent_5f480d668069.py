# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\examples.py\mcp_agent_5f480d668069.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\mcp_agent.py
...
def test_MCP_server():
    ...
    mcp_agent = CustomizeAgent(
        ...
        prompt_template=StringTemplate(instruction="Do some operations based on the user's instruction."),
        llm_config=openai_config,
        inputs=[
            {
                "name": "instruction",
                "type": "string",
                "description": "The goal you need to achieve",
            }
        ],
        outputs=[
            {
    ...
    mcp_agent.load_module(
        "examples/output/mcp_agent/mcp_agent.json",
        llm_config=openai_config,
        tools=tools,
    )
