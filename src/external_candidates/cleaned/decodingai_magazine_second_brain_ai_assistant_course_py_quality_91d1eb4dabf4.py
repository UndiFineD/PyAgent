# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\workshops.py\rag.py\solution.py\src.py\rag_workshop.py\quality_91d1eb4dabf4.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\workshops\rag\solution\src\rag_workshop\quality.py
...
class HeuristicQualityJudge:
    ...
    def __score_document(self, document: Document) -> Document:
        ...
        url_based_content = sum(len(url) for url in document.metadata.get("child_urls", []))
...
"""
    ...
    def __score_document(self, document: Document) -> Document:
        ...
        input_user_prompt = self.SYSTEM_PROMPT_TEMPLATE.format(document=document.page_content)
