# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\audio_processor.py
import asyncio
import logging
import math
import traceback
        ...
        while True:
            try:
                ...
                asr_internal_buffer_duration_s = (
                    len(getattr(self.online, "audio_buffer", [])) / self.online.SAMPLING_RATE
                )
                ...
                await self.update_transcription(new_tokens, buffer_text, new_end_buffer, self.sep)
                self.transcription_queue.task_done()
            ...
            try:
                ...
                async with self.lock:
                    self.tokens = diarization_obj.assign_speakers_to_tokens(
                        self.tokens, use_punctuation_split=self.args.punctuation_split
                    )
