...
END_SILENCE_DURATION = (
    8  # in seconds. you should keep it important to not have false positive when the model lag is important
)
    ...
    for token in tokens:
        ...
        if token.start - last_end >= MIN_SILENCE_DURATION:  # if token is not silence but important gap
            if new_tokens and new_tokens[-1].speaker == -2:
                ...
                silence_token = ASRToken(start=last_end, end=token.start, speaker=-2, probability=0.95)
...
def ends_with_silence(tokens, buffer_transcription, buffer_diarization, current_time, vac_detected_silence):
    ...
    if (
        ...
        if last_token.speaker == -2:
            ...
            tokens.append(ASRToken(start=tokens[-1].end, end=current_time, speaker=-2, probability=0.95))
        buffer_transcription = (
            ""  # for whisperstreaming backend, we should probably validate the buffer has because of the silence
        )
        buffer_diarization = ""
