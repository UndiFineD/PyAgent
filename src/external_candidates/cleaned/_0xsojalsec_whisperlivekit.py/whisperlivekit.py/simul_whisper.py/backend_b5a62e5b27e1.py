# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\simul_whisper\backend.py
import gc
import logging
import os
import sys
...
class SimulStreamingOnlineProcessor:
    ...
    def __init__(self, asr, logfile=sys.stderr, warmup_file=None):
        self.asr = asr
    ...
    def load_new_backend(self):
        ...
        self.model = PaddedAlignAttWhisper(cfg=self.asr.cfg, loaded_model=model)
        ...
        for word, word_tokens in zip(split_words, split_tokens):
            ...
            for expected_token in word_tokens:
                ...
                if actual_token != expected_token:
                    raise ValueError(
                        f"Token mismatch: expected '{expected_token}', got '{actual_token}' at frame {current_frame}"
                    )
    ...
    def process_iter(self, is_last=False) -> Tuple[List[ASRToken], float]:
        ...
        try:
            ...
            for ts_word in ts_words:
                ...
                ).with_offset(self.global_time_offset)
...
class SimulStreamingASR:
    ...
    def load_model(self):
        ...
        whisper_model.transcribe(
            warmup_audio,
            language=(self.original_language if self.original_language != "auto" else None),
        )
