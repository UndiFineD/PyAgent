# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\simul_whisper\simul_whisper.py
...
from .whisper import DecodingOptions, load_model, tokenizer
...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig, loaded_model=None) -> None:
        ...
        self.CIFLinear, self.always_fire, self.never_fire = load_cif(
    ...
    def insert_audio(self, segment=None):
        ...
        while len(self.segments) > 1 and segments_len > self.cfg.audio_max_len:
            ...
            logger.debug(
                f"remove segments: {len(self.segments)} {len(self.tokens)}, cumulative offset: {self.cumulative_time_offset:.2f}s"
            )
    ...
    def infer(self, is_last=False):
        ...
        if self.cfg.language == "auto" and self.detected_language is None:
            ...
            self.init_tokens()
            logger.info(
                f"Tokenizer language: {self.tokenizer.language}, {self.tokenizer.sot_sequence_including_notimestamps}"
            )
        ...
        while not completed and current_tokens.shape[1] < self.max_text_len:  # bos is 3 tokens
            ...
            absolute_timestamps = [
                (frame * 0.02 + self.cumulative_time_offset) for frame in most_attended_frames.tolist()
            ]
