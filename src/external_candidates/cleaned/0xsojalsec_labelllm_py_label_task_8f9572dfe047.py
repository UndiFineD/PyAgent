# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_labelllm.py\backend.py\app.py\api.py\v1.py\endpoints.py\label_task_8f9572dfe047.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\api\v1\endpoints\label_task.py
...
async def list_label_task(
    ...
    with sentry_sdk.start_transaction(op="task", name="list label task"):
        ...
        open_task_ids = [task.task_id for task in tasks if task.status == schemas.task.TaskStatus.OPEN]
...
async def get_data(
    ...
    resp = schemas.data.RespGetData(
        ...
        prompt=data.prompt,
        conversation=[
            schemas.message.MessageBase.model_validate(message, from_attributes=True) for message in data.conversation
        ],
        reference_evaluation=data.reference_evaluation,
        remain_time=task.expire_time + record.create_time,
    )
...
async def commit_data(
    ...
    await crud.data.update(
        db_obj=data,
        obj_in=models.data.DataUpdate(status=schemas.data.DataStatus.COMPLETED, evaluation=evaluation),
    )
...
async def get_task_user(
    ...
    if req.inlet == schemas.task.PreviewDataKind.USER:
        ...
        user_ids = list(set([record.creator_id for record in records]) & set(team_user_ids))
...
async def preview_record(
    ...
    resp = schemas.operator.task.RespPreviewRecord(
        ...
        evaluation=schemas.evaluation.SingleEvaluation.model_validate(record.evaluation, from_attributes=True),
        reference_evaluation=data.reference_evaluation,
