# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\analysis.py\analysis_utils_08998ccb50e9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\analysis\analysis_utils.py
...
def group_results_by_task(df: pd.DataFrame, task_column: str = "version_description") -> Dict[str, pd.DataFrame]:
    ...
    for (version, instance), group in trajectory_groups:
        ...
        for col in ["model", "version_description", "token_usage", "created_at"]:
            if col in group.columns:
                summary[col] = group[col].iloc[0]  # Take first value (should be same for all steps)
...
def calculate_efficiency_metrics(
    ...
    if token_col in df.columns:
        ...
        if not successful.empty:
            ...
            metrics["token_efficiency_ratio"] = metrics["mean_tokens_successful"] / metrics["mean_tokens_all"]
    ...
    metrics["reward_efficiency"] = metrics["mean_reward"] / metrics["reward_std"] if metrics["reward_std"] > 0 else 0
...
def aggregate_results_by_time(
    ...
    aggregated = (
        df_copy[metric_col].resample(window).agg(["count", "mean", "std", "min", "max", "median"]).reset_index()
    )
    ...
    success_counts = (
        df_copy[df_copy[metric_col] > success_threshold].set_index(time_col)[metric_col].resample(window).count()
    )
...
def create_leaderboard(
    ...
    column_order = ["rank", "model", primary_metric] + secondary_metrics + ["rank_score"]

    leaderboard = leaderboard[[col for col in column_order if col in leaderboard.columns]]
