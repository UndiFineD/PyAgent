# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\gemini_model_9db70d9d5e35.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\gemini_model.py
        ...
        if item.get("type") == "object" or "properties" in item:
            ...
            for prop_name, prop_schema in props.items():
                if prop_name not in merged_properties:
                    merged_properties[prop_name] = resolve_fn(copy_module.deepcopy(prop_schema))
                ...
                if union_key in obj:
                    ...
                    if isinstance(union, list):
                        ...
                        if object_count > 1 or has_refs:
                            flattened = _flatten_union_to_object_gemini(union, defs, resolve_refs)
    ...
    def _build_tools(self, tools: list[ToolDefinition]) -> list[dict[str, Any]]:
        ...
        for tool in tools:
            ...
            if tool.parameters_json_schema:
                ...
                func_decl["parameters"] = _sanitize_schema_for_gemini(tool.parameters_json_schema)

            function_declarations.append(func_decl)

        return [{"functionDeclarations": function_declarations}]

    def _build_generation_config(self, model_settings: ModelSettings | None) -> dict[str, Any]:
    ...
    def _parse_response(self, data: dict[str, Any]) -> ModelResponse:
        ...
        for part in parts:
            if part.get("thought") and part.get("text") is not None:
                ...
                response_parts.append(ThinkingPart(content=part["text"], signature=signature))
        ...
        async def stream_chunks() -> AsyncIterator[dict[str, Any]]:
            async with client.stream("POST", url, json=body, headers=headers) as response:
                if response.status_code != 200:
                    ...
                    raise RuntimeError(f"Gemini API error {response.status_code}: {text.decode()}")
