# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-xsssniper\core\packages\clint\textui\core.py
# -*- coding: utf-8 -*-
...
class Writer(object):
    ...
    def __init__(self, indent=0, quote="", indent_char=" "):
        ...
        if self.indent > 0:
            self.indent_string = "".join((str(quote), (self.indent_char * (indent - len(self.indent_quote)))))
        else:
            self.indent_string = "".join((("\x08" * (-1 * (indent - len(self.indent_quote)))), str(quote)))
    ...
    def __call__(self, s, newline=True, stream=STDOUT):
        ...
        _str = "".join(("".join(self.shared["indent_strings"]), str(s), "\n" if newline else ""))
        stream(_str)
