# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\agentic_layer.py\retrieval_utils_c7f8230f218d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\agentic_layer\retrieval_utils.py
...
async def search_with_bm25(query: str, bm25, candidates, stemmer, stop_words, top_k: int = 50) -> List[Tuple]:
    ...
    if has_chinese:
        ...
        tokenized_query = [
            stemmer.stem(token) for token in tokens if token.isalpha() and len(token) >= 2 and token not in stop_words
        ]
...
def reciprocal_rank_fusion(results1: List[Tuple], results2: List[Tuple], k: int = 60) -> List[Tuple]:
    ...
    fused_results = [(doc_map[doc_id], rrf_score) for doc_id, rrf_score in doc_rrf_scores.items()]
...
async def lightweight_retrieval(
    ...
    if bm25 is not None:
        bm25_results = await search_with_bm25(query, bm25, candidates, stemmer, stop_words, top_k=bm25_top_n)
...
async def multi_query_retrieval(
    ...
    tasks = [lightweight_retrieval(q, candidates, emb_top_n, bm25_top_n, final_top_n) for q in queries]
...
async def rerank_candidates(query: str, candidates: List[Tuple], top_n: int, rerank_service) -> List[Tuple]:
    """
    ...
    try:
        ...
        reranked_hits = await rerank_service._rerank_all_hits(query, candidates_for_rerank, top_k=top_n)
