# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\models.py\model_utils_9b2a662eb2c8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\models\model_utils.py
...
class CostManager:
    ...
    def update_cost(self, cost: Cost, model: str):
        ...
        if not suppress_cost_logs.get():
            logger.info(
                f"Total cost: ${total_cost:.3f} | Total tokens: {total_tokens} | Current cost: ${current_total_cost:.3f} | Current tokens: {current_total_tokens}"
            )
