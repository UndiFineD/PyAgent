# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\di.py\tests.py\test_fixtures_1dbdbdc92a54.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\di\tests\test_fixtures.py
...
def create_readonly_connection() -> DatabaseConnection:
    ...
    return DatabaseConnection(host="readonly.example.com", port=3306, database="test_db")
...
def register_standard_beans(container: DIContainer):
    ...
    container.register_bean(bean_type=MySQLUserRepository, bean_name="mysql_user_repo", is_primary=True)
    ...
    container.register_bean(bean_type=MockUserRepository, bean_name="mock_user_repo", is_mock=True)
    ...
    container.register_bean(bean_type=SMSNotificationService, bean_name="sms_notification")

    container.register_bean(bean_type=PushNotificationService, bean_name="push_notification")

    # Register CacheService implementations

    container.register_bean(bean_type=RedisCacheService, bean_name="redis_cache", is_primary=True)
