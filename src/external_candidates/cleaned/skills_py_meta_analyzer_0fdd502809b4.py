# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\dgriffin831.py\skill_scan.py\skill_scan.py\meta_analyzer_0fdd502809b4.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dgriffin831\skill-scan\skill_scan\meta_analyzer.py
...
class MetaAnalyzer:
    ...
    def analyze(
        ...
        return asyncio.run(self._analyze_async(report, file_contents, llm_analyzer))
    ...
    def apply_to_report(report: dict, meta_result: dict) -> None:
        ...
        report["metaAnalysis"]["false_positive_count"] = sum(1 for r in reviews if r.get("verdict") == "false_positive")

        report["metaAnalysis"]["true_positive_count"] = sum(1 for r in reviews if r.get("verdict") == "true_positive")
