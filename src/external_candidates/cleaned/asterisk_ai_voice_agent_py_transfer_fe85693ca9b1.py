# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\tools.py\telephony.py\transfer_fe85693ca9b1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\telephony\transfer.py
...
class TransferCallTool(Tool):
    ...
    async def execute(self, parameters: Dict[str, Any], context: ToolExecutionContext) -> Dict[str, Any]:
        ...
        logger.info(
            f"🔀 Transfer requested: {target} ({ai_suggested_mode} mode suggested by AI)",
    ...
    async def _resolve_target(self, target: str, context: ToolExecutionContext) -> Optional[Dict[str, Any]]:
        """
        ...
        logger.info(
            f"DEBUG: extensions_config result: {extensions_config}",
            call_id=context.call_id,
        )
    ...
    async def _warm_transfer(
        ...
        logger.info(
            f"⏳ Warm transfer: AI continues conversation, no MOH",
            call_id=context.call_id,
            target=extension,
        )
    ...
    def _build_transfer_context(
        self, session, extension_info: Dict[str, Any], context: ToolExecutionContext
    ) -> Dict[str, Any]:
        """
    ...
    async def _originate_to_agent_outbound(
        ...
        logger.info(
            f"🔀 Direct SIP origination for warm transfer",
    ...
    async def _stop_moh(self, channel_id: str, context: ToolExecutionContext):
        ...
        try:
            await context.ari_client.send_command(method="DELETE", resource=f"channels/{channel_id}/moh")
