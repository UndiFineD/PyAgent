# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\biz_layer.py\mem_sync_a01ba50404b3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\biz_layer\mem_sync.py
...
class MemorySyncService:
    ...
    def __init__(
        ...
        self.foresight_milvus_repo = foresight_milvus_repo or get_bean_by_type(ForesightMilvusRepository)

        self.eventlog_milvus_repo = eventlog_milvus_repo or get_bean_by_type(EventLogMilvusRepository)

        self.foresight_es_repo = foresight_es_repo or get_bean_by_type(ForesightEsRepository)

        self.eventlog_es_repo = eventlog_es_repo or get_bean_by_type(EventLogEsRepository)
    ...
    async def sync_foresight(
        ...
        try:
            ...
            if not foresight.vector:
                logger.warning(f"Foresight {foresight.id} has no embedding, skipping sync")
    ...
    async def sync_event_log(
        ...
        try:
            ...
            if not event_log.vector:
                logger.warning(f"Event log {event_log.id} has no embedding, skipping sync")
    ...
    async def sync_batch_foresights(
        ...
        for foresight_mem in foresights:
            try:
                stats = await self.sync_foresight(foresight_mem, sync_to_es=sync_to_es, sync_to_milvus=sync_to_milvus)
        ...
        logger.info(f"✅ Foresight Milvus flush completed: {total_stats['foresight']} records")
    ...
    async def sync_batch_event_logs(
        ...
        for evt_log in event_logs:
            try:
                stats = await self.sync_event_log(evt_log, sync_to_es=sync_to_es, sync_to_milvus=sync_to_milvus)
        ...
        logger.info(f"✅ Event log Milvus flush completed: {total_stats['event_log']} records")
