# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\tonimelisma.py\email_to_calendar.py\scripts.py\utils.py\invite_ops_42a5b1e05cc1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tonimelisma\email-to-calendar\scripts\utils\invite_ops.py
...
def main():
    ...
    if args.get("status") not in valid_statuses:
        print(
            f"Error: Invalid status. Must be: {', '.join(valid_statuses)}",
            file=sys.stderr,
        )
