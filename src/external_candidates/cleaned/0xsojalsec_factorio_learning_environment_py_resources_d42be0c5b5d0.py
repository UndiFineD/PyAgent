# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\agents.py\data.py\sprites.py\extractors.py\resources_d42be0c5b5d0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\agents\data\sprites\extractors\resources.py
...
class ResourceSpriteExtractor:
    ...
    def extract_resource_sprites(self, resource_name: str):
        ...
        config = self.resource_configs.get(resource_name, {"columns": 8, "rows": 8, "width": 64, "height": 64})
            ...
            try:
                ...
                for row in range(config["rows"]):
                    for col in range(config["columns"]):
                        ...
                        sprite = sprite_sheet.crop((x, y, x + config["width"], y + config["height"]))
                    ...
                    if glow_path.exists():
                        ...
                        for row in range(config["rows"]):
                            for col in range(config["columns"]):
                                ...
                                glow_sprite = glow_sheet.crop((x, y, x + config["width"], y + config["height"]))

                                volume = config["rows"] - row

                                variant = col + 1

                                output_name = f"{prefix}uranium-ore-glow_{variant}_{volume}.png"
