# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\test_claude_cache_client_fb3ec76b3ca0.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_claude_cache_client.py
    ...
    async def test_proactive_refresh_on_old_token(self):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                client = ClaudeCacheAsyncClient(headers={"Authorization": f"Bearer {old_token}"})
    ...
    async def test_no_proactive_refresh_on_fresh_token(self):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                client = ClaudeCacheAsyncClient(headers={"Authorization": f"Bearer {fresh_token}"})
...
class TestTokenRefreshOnCloudflareError:
        ...
        with patch.object(httpx.AsyncClient, "send", new_callable=AsyncMock) as mock_send:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    client = ClaudeCacheAsyncClient(headers={"Authorization": "Bearer old_token"})
    ...
    async def test_no_refresh_on_json_400(self):
        ...
        with patch.object(httpx.AsyncClient, "send", new_callable=AsyncMock, return_value=response):
            with patch.object(ClaudeCacheAsyncClient, "_refresh_claude_oauth_token") as mock_refresh:
                # Mock stored token expiry check to prevent proactive refresh
                ...
                with patch.object(
                    ...
                    client = ClaudeCacheAsyncClient(headers={"Authorization": "Bearer token"})
...
class TestToolPrefixing:
    ...
    def test_unprefix_tool_names_in_text(self):
        ...
        response_text = (
            f'event: content_block_start\ndata: {{"type": "content_block_start", "name": "{TOOL_PREFIX}read_file"}}'
        )
