...
class VideoContentQA(BaseTool):
    ...
    class OutputSchema(BaseModel):
        transcript_path: str = Field(..., description="Path where the combined transcript was saved")
        processed_videos: List[str] = Field(..., description="List of video files that were processed")
        total_videos_found: int = Field(..., description="Total number of video files found in the directory")
        qa_session_completed: bool = Field(..., description="Whether the Q&A session was completed")
        total_questions_asked: int = Field(..., description="Total number of questions asked during the session")
        history_saved: bool = Field(..., description="Whether the conversation history was saved")
        history_path: Optional[str] = Field(default=None, description="Path where conversation history was saved")
        status: str = Field(..., description="Overall status of the transcription and Q&A process")
