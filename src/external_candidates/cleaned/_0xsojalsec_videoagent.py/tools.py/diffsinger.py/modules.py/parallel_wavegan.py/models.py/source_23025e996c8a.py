# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\modules\parallel_wavegan\models\source.py
import sys
...
class PulseGen(torch.nn.Module):
    ...
    def forward(self, f0):
        ...
        with torch.no_grad():
            ...
            loc = (pure_sine > sine_1) * (pure_sine > sine_2) * (uv_1 > 0) * (uv_2 > 0) * (uv > 0) + (uv_1 < 1) * (
                uv > 0
            )
