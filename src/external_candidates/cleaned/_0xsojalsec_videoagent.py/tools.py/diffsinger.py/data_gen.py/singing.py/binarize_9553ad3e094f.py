    ...
    def load_meta_data(self):
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            ...
            for piece_path in all_wav_pieces:
                item_name = raw_item_name = piece_path[len(processed_data_dir) + 1 :].replace("/", "-")[
                    : -len(wav_suffix)
                ]
        ...
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            ...
            for song_item in meta_midi:
                ...
                self.item2wdb[item_name] = [
                    1 if x in ALL_YUNMU + ["AP", "SP", "<SIL>"] else 0 for x in song_item["phs"]
                ]
    ...
    def load_meta_data(self):
        ...
        for utterance_label in utterance_labels:
            ...
            self.item2midi[item_name] = [
                librosa.note_to_midi(x.split("/")[0]) if x != "rest" else 0 for x in song_info[3].split(" ")
            ]
