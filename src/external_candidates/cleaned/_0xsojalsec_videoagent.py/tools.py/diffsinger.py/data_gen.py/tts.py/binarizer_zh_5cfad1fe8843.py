# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\data_gen\tts\binarizer_zh.py
import os
        ...
        for i in range(len(dur)):
            ...
            if p in ALL_SHENMU:
                ...
                if not (dur[i] > 0 and p_next[0].isalpha() and p_next not in ALL_SHENMU):
                    print(
                        f"assert dur[i] > 0 and p_next[0].isalpha() and p_next not in ALL_SHENMU, "
