# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\tasks\tts\fs2.py
import matplotlib
...
from utils.plot import dur_to_figure, f0_to_figure, spec_to_figure
...
class FastSpeech2Task(TtsTask):
    ...
    def add_dur_loss(self, dur_pred, mel2ph, txt_tokens, losses=None):
        ...
        if hparams["dur_loss"] == "mse":
            ...
            losses["pdur"] = -self.model.dur_predictor.crf(
                dur_pred,
                dur_gt.long().clamp(min=0, max=31),
                mask=nonpadding > 0,
                reduction="mean",
            )
    ...
    def add_pitch_loss(self, output, sample, losses):
        if hparams["pitch_type"] == "ph":
            ...
            losses["f0"] = (
                (pitch_loss_fn(output["pitch_pred"][:, :, 0], sample["f0"], reduction="none") * nonpadding).sum()
                / nonpadding.sum()
                * hparams["lambda_f0"]
            )
            return
        ...
        if hparams["pitch_type"] == "cwt":
            ...
            if hparams["use_uv"]:
                ...
                losses["uv"] = (
                    (F.binary_cross_entropy_with_logits(uv_pred, uv, reduction="none") * nonpadding).sum()
                    / nonpadding.sum()
                    * hparams["lambda_uv"]
                )
    ...
    def add_f0_loss(self, p_pred, f0, uv, losses, nonpadding):
        ...
        if hparams["use_uv"]:
            ...
            losses["uv"] = (
                (F.binary_cross_entropy_with_logits(p_pred[:, :, 1], uv, reduction="none") * nonpadding).sum()
                / nonpadding.sum()
                * hparams["lambda_uv"]
            )
        ...
        if hparams["pitch_loss"] in ["l1", "l2"]:
            pitch_loss_fn = F.l1_loss if hparams["pitch_loss"] == "l1" else F.mse_loss
            losses["f0"] = (
                (pitch_loss_fn(f0_pred, f0, reduction="none") * nonpadding).sum()
                / nonpadding.sum()
                * hparams["lambda_f0"]
            )
    ...
    def plot_dur(self, batch_idx, sample, model_out):
        ...
        txt = txt.split(" ")
        self.logger.experiment.add_figure(f"dur_{batch_idx}", dur_to_figure(dur_gt, dur_pred, txt), self.global_step)
    ...
    def plot_pitch(self, batch_idx, sample, model_out):
        ...
        if hparams["pitch_type"] == "ph":
            ...
            self.logger.experiment.add_figure(
                f"f0_{batch_idx}",
                f0_to_figure(f0[0], None, f0_pred[0]),
                self.global_step,
            )
            return
        ...
        if hparams["pitch_type"] == "cwt":
            ...
            self.logger.experiment.add_figure(
                f"f0_{batch_idx}",
                f0_to_figure(f0[0], f0_cwt[0], f0_pred[0]),
                self.global_step,
            )
        elif hparams["pitch_type"] == "frame":
            # f0
            ...
            self.logger.experiment.add_figure(
                f"f0_{batch_idx}",
                f0_to_figure(f0[0], None, pitch_pred[0]),
                self.global_step,
            )
        ...
        for num_predictions, prediction in enumerate(t):
            ...
            if not hparams["profile_infer"]:
                ...
                self.saving_results_futures.append(
                    self.saving_result_pool.apply_async(
                        self.Fsave_result,
                        args=[
                            wav_pred,
                            mel_pred,
                ...
                t.set_description(f"Pred_shape: {mel_pred.shape}, gt_shape: {mel_gt.shape}")
            else:
