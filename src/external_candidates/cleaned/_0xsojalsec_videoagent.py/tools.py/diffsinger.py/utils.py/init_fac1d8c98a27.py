# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\DiffSinger\utils\__init__.py
import glob
import logging
import os
import re
...
def batch_by_size(
    ...
    for i in range(len(indices)):
        ...
        assert sample_len <= max_tokens, "sentence at index {} of size {} exceeds max_tokens limit of {}!".format(
...
def load_ckpt(cur_model, ckpt_base_dir, prefix_in_ckpt="model", force=True, strict=True):
    if os.path.isfile(ckpt_base_dir):
        ...
        all_files = glob.glob(os.path.join(base_dir, "model_ckpt_steps_*.ckpt"))
        # 修改这里：只匹配文件名，不包含路径
    ...
    if len(checkpoint_path) > 0:
        ...
        state_dict = {
            k[len(prefix_in_ckpt) + 1 :]: v for k, v in state_dict.items() if k.startswith(f"{prefix_in_ckpt}.")
        }
        if not strict:
