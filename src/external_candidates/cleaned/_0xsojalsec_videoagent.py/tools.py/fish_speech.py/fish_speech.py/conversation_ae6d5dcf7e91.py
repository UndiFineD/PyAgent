# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\fish_speech\conversation.py
    ...
    def encode(
        ...
        for part in parts:
            if isinstance(part, TextPart):
                ...
                tokens = torch.tensor(
                    [tokenizer.semantic_id_to_token_id[i.item()] for i in curr_codes[0].int()],
                    dtype=torch.int,
                )
...
class Conversation:
    ...
    def visualize(
        ...
        encoded = self.encode(tokenizer, add_shift=False, ignore_loss_tokens=ignore_loss_tokens)
