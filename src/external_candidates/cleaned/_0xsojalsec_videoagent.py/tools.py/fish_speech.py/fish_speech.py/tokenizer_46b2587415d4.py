# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\fish_speech\tokenizer.py
import base64
import json
import logging
...
class FishTokenizer:
    def __init__(self, model_path: str) -> None:
        ...
        self.semantic_id_to_token_id = {
            i: self.all_special_tokens_with_ids[token] for i, token in enumerate(SEMANTIC_TOKENS)
        }
    ...
    def encode(self, s: str, allowed_special: bool | set[str] = True) -> list[int]:
        ...
        return sum(
            self.tkt_model.encode_batch(subs, allowed_special=allowed_special, disallowed_special=set()),
            start=[],
        )
...
if __name__ == "__main__":
    ...
    print([tokenizer.decode([i]) for i in tokenizer.encode(f"{BOS_TOKEN}你好，世界！{EOS_TOKEN}")])
