# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\fish_speech\models\text2semantic\inference.py
import os
import queue
import threading
import time
    ...
    **sampling_kwargs,
    ...
    for i in range(model.config.num_codebooks):
        codebooks.append(
            sample_agent(
                x.codebook_logits[:, :, i],
                previous_tokens=(previous_tokens[:, i + 1] if previous_tokens is not None else None),
                **sampling_kwargs,
            )[0]
        )
...
def decode_n_tokens_agent(
    ...
    **sampling_kwargs,
    ...
    for i in tqdm(range(num_new_tokens), desc="Decoding: ", total=num_new_tokens):
        ...
        previous_tokens[:, :, i : i + 1] = next_token.view(batch_size, model.config.num_codebooks + 1, -1)

        yield cur_token.cpu()

        finished = finished | (cur_token[:, 0, -1] == im_end_id)
        if finished.all() or (
            0 < early_stop_threshold < 1 and finished.sum() >= round(batch_size * early_stop_threshold)
        ):
            break
...
def generate_agent(
    ...
    **sampling_kwargs,
    ...
    # Use non-accelerated version for now, to avoid compilation overhead
    prefill_decode = decode_one_token_naive_agent if isinstance(model, NaiveTransformer) else decode_one_token_ar_agent
    next_token = prefill_decode(
        model,
        prompt,
        input_pos,
...
def generate_long(
    *,
    ...
    assert use_prompt is False or len(prompt_text) == len(prompt_tokens), (
    ...
    repetition_penalty = torch.tensor(repetition_penalty, device=device, dtype=torch.float)
    ...
    for sample_idx in range(num_samples):
        ...
        while seg_idx < len(encoded):
            logger.info(f"Generating sentence {seg_idx + 1}/{len(encoded)} of sample {sample_idx + 1}/{num_samples}")
            ...
            for i, length in enumerate(lengths):
                count += length
                if count + length > max_length - 1024 - sum(t.shape[1] for t in encoded_prompts):
                    break
            ...
            logger.info(f"Generated {tokens_generated} tokens in {t:.02f} seconds, {tokens_sec:.02f} tokens/sec")
...
def launch_thread_safe_queue_agent(
    ...
    def worker():
        model, decode_one_token = load_model(checkpoint_path, device, precision, compile=compile, is_agent=True)
