...
class BaseTransformer(nn.Module):
    ...
    def embed(self, inp: Tensor, share_codebook_embeddings=True) -> Tensor:
        embeds = []
        semantic_token_ids_tensor = torch.tensor(self.semantic_token_ids, device=inp.device, dtype=inp.dtype)
...
class DualARTransformer(BaseTransformer):
    ...
    def forward(
        ...
        for layer in self.fast_layers:
            if self.config.use_gradient_checkpointing and self.training:
                x = checkpoint(layer, x, self.fast_freqs_cis, fast_mask, use_reentrant=True)
            else:
