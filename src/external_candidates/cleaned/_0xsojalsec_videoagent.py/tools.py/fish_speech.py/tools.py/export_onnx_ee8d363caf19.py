...
class Decoder(torch.nn.Module):
    ...
    def get_codes_from_indices(self, cur_index, indices):
        ...
        if quantize_dim < self.model.quantizer.residual_fsq.rvqs[cur_index].num_quantizers:
            assert self.model.quantizer.residual_fsq.rvqs[cur_index].quantize_dropout > 0.0, (
            ...
            indices = F.pad(
                ...
                (
                    0,
                    self.model.quantizer.residual_fsq.rvqs[cur_index].num_quantizers - quantize_dim,
                ),
                value=-1,
            )
