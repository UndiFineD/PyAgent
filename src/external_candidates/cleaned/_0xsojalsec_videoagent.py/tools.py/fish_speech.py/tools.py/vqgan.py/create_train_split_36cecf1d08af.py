# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\tools\vqgan\create_train_split.py
import math
...
def main(root, val_ratio, val_count, filelist, min_duration, max_duration):
        ...
        for file in tqdm(files):
            try:
                ...
                if min_duration is not None and duration < min_duration:
                    logger.info(f"Skipping {file} due to duration {duration:.2f} < {min_duration:.2f}")
                    continue

                if max_duration is not None and duration > max_duration:
                    logger.info(f"Skipping {file} due to duration {duration:.2f} > {max_duration:.2f}")
                    continue
    ...
    logger.info(f"Found {len(files)} files, remaining {len(filtered_files)} files after filtering")
