# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\tools\server\views.py
import io
import os
import time
...
async def asr(req: Annotated[ServeASRRequest, Body(exclusive=True)]):
    ...
    transcriptions = batch_asr(asr_model, lock, audios=audios, sr=req.sample_rate, language=req.language)
...
async def chat(req: Annotated[ServeChatRequest, Body(exclusive=True)]):
    ...
    response_generator = get_response_generator(llama_queue, tokenizer, config, req, device, json_mode)
    ...
    return StreamResponse(iterable=response_generator(), content_type="text/event-stream")
