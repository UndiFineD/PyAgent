# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\tools\server\model_utils.py
import io
import re
...
def batch_encode(model, audios_list: list[bytes]):
    audios: list[torch.Tensor] = [
        (
            torch.from_numpy(librosa.load(io.BytesIO(audio), sr=model.spec_transform.sample_rate)[0])[None]
    ...
    padded = torch.stack(
        [torch.nn.functional.pad(audio, (0, int(max_length - audio.shape[-1]))) for audio in audios]
    ).to(model.device)
...
def batch_vqgan_decode(model, features):
    lengths = torch.tensor([feature.shape[-1] for feature in features], device=model.device)
    max_length = lengths.max().item()
    padded = torch.stack(
        [torch.nn.functional.pad(feature, (0, max_length - feature.shape[-1])) for feature in features]
    ).to(model.device)
...
def batch_asr(model, lock, audios, sr, language="auto"):
    ...
    for r, audio in zip(res, audios):
        ...
        if "timestamp" in r and len(r["timestamp"]) > 2:
            for timestamp_a, timestamp_b in zip(r["timestamp"][:-1], r["timestamp"][1:]):
