...
def task_generator_folder(root: Path, text_extension: str):
    ...
    logger.info(f"Found {len(grouped_files)} groups in {root}, {list(grouped_files.keys())[:5]}...")
...
@click.option("--output", type=click.Path(path_type=Path), default="data/quantized-dataset-ft")
@click.option("--num-workers", type=int, default=16)
@click.option("--text-extension", type=str, default=[".txt"], multiple=True)
@click.option("--shard-size", type=int, default=10, help="The maximum size of each shard in mb")
