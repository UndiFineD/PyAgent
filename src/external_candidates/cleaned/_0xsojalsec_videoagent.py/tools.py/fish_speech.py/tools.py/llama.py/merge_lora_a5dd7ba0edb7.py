# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoAgent\tools\fish-speech\tools\llama\merge_lora.py
import shutil
...
def merge(lora_config, base_weight, lora_weight, output):
    output = Path(output)
    logger.info(f"Merging {base_weight} and {lora_weight} into {output} with {lora_config}")
    ...
    if any(k.startswith("model.") for k in llama_state_dict.keys()):
        llama_state_dict = {k.replace("model.", ""): v for k, v in llama_state_dict.items() if k.startswith("model.")}
    if any(k.startswith("model.") for k in lora_state_dict.keys()):
        lora_state_dict = {k.replace("model.", ""): v for k, v in lora_state_dict.items() if k.startswith("model.")}
    ...
    if diff_l1 <= tolerance:
        logger.warning("Merged model seems identical to the original model. Further validation might be needed.")
    else:
