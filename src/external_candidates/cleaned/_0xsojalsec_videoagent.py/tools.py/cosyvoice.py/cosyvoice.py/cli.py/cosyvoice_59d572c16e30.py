...
class CosyVoice:
    def __init__(self, model_dir, load_jit=False, load_trt=False, fp16=False):
        ...
        assert get_model_type(configs) != CosyVoice2Model, "do not use {} for CosyVoice initialization!".format(
            model_dir
        )
        ...
        if load_jit:
            self.model.load_jit(
                "{}/llm.text_encoder.{}.zip".format(model_dir, "fp16" if self.fp16 is True else "fp32"),
...
class CosyVoice2(CosyVoice):
    def __init__(self, model_dir, load_jit=False, load_trt=False, fp16=False):
        ...
        assert get_model_type(configs) == CosyVoice2Model, "do not use {} for CosyVoice2 initialization!".format(
            model_dir
        )
        self.frontend = CosyVoiceFrontEnd(
            configs["get_tokenizer"],
