...
def calc_mos(computor, audio, orin_sr):
    ...
    target_sr = 16000
    if orin_sr != 16000:
        audio = librosa.resample(audio, orig_sr=orin_sr, target_sr=target_sr, res_type="kaiser_fast")
    ...
    for source_i, source_line in enumerate(tqdm(source_audio_list)):
        ...
        for target_i, target_name in enumerate(target_audio_list):
            ...
            if args.xvector_extractor == "wavlm":
                ...
                vc_inputs = wavlm_feature_extractor(vc_wave_16k.squeeze(0).cpu(), padding=True, return_tensors="pt").to(
                    device
                )
                vc_embeddings = wavlm_model(**vc_inputs).embeddings
                vc_embeddings = torch.nn.functional.normalize(vc_embeddings, dim=-1).cpu()

                similarity = torch.nn.functional.cosine_similarity(ref_embeddings, vc_embeddings, dim=-1)
            elif args.xvector_extractor == "resemblyzer":
                ref_wav_resemblyzer = preprocess_wav(target_path)
                vc_wav_resemblyzer = preprocess_wav(osp.join(conversion_result_dir, source_index, f"{target_name}"))
            ...
            # perform asr
            vc_asr_inputs = asr_processor(vc_wave_16k.squeeze(0).cpu(), return_tensors="pt", padding=True).to(device)
            ...
            source_asr_inputs = asr_processor(source_wav_16k, return_tensors="pt", padding=True).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--xvector-extractor", type=str, default="wavlm-large")  # wavlm or resemblyzer
