# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-rebuff\python-sdk\rebuff\rebuff.py
import secrets
...
class Rebuff:
    ...
    def add_canary_word(
        ...
        raise TypeError(f"prompt_template must be a PromptTemplate or a str, but was {type(prompt)}")
    ...
    def log_leakage(self, user_input: str, completion: str, canary_word: str) -> None:
        ...
        response = requests.post(f"{self.api_url}/api/log", json=data, headers=self._headers)
        response.raise_for_status()
        return
