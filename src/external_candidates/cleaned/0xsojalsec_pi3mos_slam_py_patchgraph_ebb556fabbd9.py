# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_pi3mos_slam.py\dpvo.py\patchgraph_ebb556fabbd9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\dpvo\patchgraph.py
...
class PatchGraph:
    ...
    def __init__(self, cfg, P, DIM, pmem, **kwargs):
        ...
        self.prior_inv_depth_ = torch.zeros(
            self.N, self.M, 1, dtype=torch.float, device="cuda"
        )  # prior inverse depth from pi3.

        self.confidences_ = torch.zeros(
            self.N, self.M, 1, dtype=torch.float, device="cuda"
        )  # confidence score of depth prediction.

        self.var_ = torch.zeros(
            self.N, self.M, 1, dtype=torch.float, device="cuda"
        )  # variance of estimated inverse depth
    ...
    def edges_loop(self):
        ...
        jj, kk = flatmeshgrid(
            torch.arange(
                self.n - self.cfg.GLOBAL_OPT_FREQ,
                self.n - self.cfg.KEYFRAME_INDEX,
                device="cuda",
            ),
            torch.arange(max(l - lc_range, 0) * self.M, l * self.M, device="cuda"),
            indexing="ij",
        )
