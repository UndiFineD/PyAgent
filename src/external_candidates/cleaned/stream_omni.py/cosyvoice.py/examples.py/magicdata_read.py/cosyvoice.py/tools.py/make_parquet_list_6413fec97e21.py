# Extracted from: C:\DEV\PyAgent\.external\Stream-Omni\CosyVoice\examples\magicdata-read\cosyvoice\tools\make_parquet_list.py
#!/usr/bin/env python3
...
def job(utt_list, parquet_file, utt2parquet_file, spk2parquet_file):
    ...
    with open(spk2parquet_file, "w") as f:
        json.dump(
            {k: parquet_file for k in list(set(spk_list))},
            f,
            ensure_ascii=False,
            indent=2,
        )
...
if __name__ == "__main__":
    ...
    with (
        open("{}/data.list".format(args.des_dir), "w", encoding="utf8") as f1,
