...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model):
        ...
        hift_state_dict = {
            k.replace("generator.", ""): v for k, v in torch.load(hift_model, map_location=self.device).items()
        }
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid):
        ...
        with self.llm_context:
            for i in self.llm.inference(
                ...
                prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32).to(
                    self.device
                ),
        ...
        if stream is True:
            ...
            while True:
                ...
                if (
                    ...
                    and len(self.tts_speech_token_dict[this_uuid]) < token_hop_len + self.token_overlap_len
                ):
                    break
            p.join()
