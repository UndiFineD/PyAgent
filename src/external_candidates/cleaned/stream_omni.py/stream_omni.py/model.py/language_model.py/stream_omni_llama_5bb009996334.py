...
class StreamOmniLlamaModel(StreamOmniMetaModel, LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.top_audio_layers = nn.ModuleList(
            [TopAudioLlamaDecoderLayer(config, layer_idx) for layer_idx in range(num_top_audio_layers)]
        )
    ...
    def infer_text_to_speech(
        ...
        if input_ids is not None:
            ...
            if bottom_audio_labels.item() != 128002 and bottom_audio_labels.item() != self.pre_llm_token:
                ...
                print(
                    bottom_audio_labels,
                    self.tok.decode(bottom_audio_labels.clamp(min=0)[0], skip_special_tokens=True).strip(),
                )
                gen_new_token = True
            else:
                llm_inputs_embeds = None
        ...
        if gen_new_token:
            ...
            if len(self.prev_llm_ids) > 0 and (
                self.prev_llm_ids[-1].item() == 128009 or self.prev_llm_ids[-1].item() == 128002
            ):
