# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\memory\vector_store.py
"""
...
class MemoryStore:
    def __init__(self, path: str = ".chroma", model_name: str | None = None):
        ...
        use_in_memory = os.getenv("PYTEST_CURRENT_TEST") or os.getenv("TESTING") or os.getenv("USE_IN_MEMORY_CHROMA")
    ...
    def add(self, doc_id: str, text: str, meta: dict):
        ...
        emb_list = emb.tolist()
        self.collection.add(ids=[doc_id], documents=[text], metadatas=[meta], embeddings=[emb_list])
