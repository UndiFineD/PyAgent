# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\memory\hybrid_retriever.py
...
class HybridRetriever:
    ...
    def _get_all_documents_for_bm25(self, task_name: str | None = None) -> tuple[list[str], list[str], list[dict]]:
        """
        ...
        try:
            ...
            for query in queries:
                try:
                    ...
                    logger.debug(f"Error in BM25 document retrieval query '{query}': {e}")
                    continue
    ...
    def _bm25_search(self, query: str, k: int = 10, task_name: str | None = None) -> list[dict[str, Any]]:
        """
                    ...
                    results.append(
                        {
                            ...
                            "doc_id": (self.bm25_doc_ids[idx] if idx < len(self.bm25_doc_ids) else f"doc_{idx}"),
                            "method": "bm25",
                        }
                    )
    ...
    def _vector_search(self, query: str, k: int = 10) -> list[dict[str, Any]]:
        ...
        try:
            ...
            doc_ids = results["ids"][0] if results.get("ids") else [f"doc_{i}" for i in range(len(documents))]

            # Convert distance to similarity score (1 - normalized distance)
            # ChromaDB returns cosine distance (0 = identical, 2 = opposite)
            # Convert to similarity: similarity = 1 - (distance / 2)
            results_list = []
            for doc, meta, dist, doc_id in zip(documents, metadatas, distances, doc_ids):
    ...
    def _normalize_scores(self, results: list[dict[str, Any]], method: str) -> list[dict[str, Any]]:
    ...
    def _fuse_results(self, vector_results: list[dict], bm25_results: list[dict]) -> list[dict]:
        ...
        for rank, result in enumerate(vector_results, start=1):
            ...
            if doc_id not in fused_map:
                ...
                fused_map[doc_id]["vector_score"] = result.get("normalized_score", result["score"])
        ...
        for rank, result in enumerate(bm25_results, start=1):
            ...
            if doc_id not in fused_map:
                ...
                fused_map[doc_id]["bm25_score"] = result.get("normalized_score", result["score"])
        ...
        for doc_id, result in fused_map.items():
            ...
            weighted_score = self.vector_weight * result["vector_score"] + self.bm25_weight * result["bm25_score"]
    ...
    def retrieve(
        ...
        if task_name:
            vector_results = [r for r in vector_results if r["metadata"].get("task") == task_name]
