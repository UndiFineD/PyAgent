# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\agents\research_orchestrator.py
"""
...
MAX_CONTEXT_CHARS = 80_000  # Maximum characters for recent notes in review (legacy fallback)
...
class TaskState:
    ...
    def create_subtask_agent(
        ...
        agent = WebResearchAgent(agent_id=subtask_id, llm=llm, memory=memory, config=config)
...
class ResearchOrchestrator(OrchestratorAgent):
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(f"Orchestrator context management enabled with model: {model_name}")
        except Exception as e:
            logger.warning(f"Failed to initialize orchestrator context management: {e}. Continuing without it.")
    ...
    async def _review(self, task: Task, latest_notes: str) -> OrchestratorDecision:
        ...
        while response is None and retry_count < max_retries:
            try:
                ...
                if self.context_management_enabled and self.token_counter and self.token_budget:
                    try:
                        ...
                        available_for_user = (
                            self.token_budget.max_context - system_tokens - self.token_budget.response_reserve
                        )
                        ...
                        if notes_tokens > max_notes_tokens and self.context_management_enabled and self.summarizer:
                            ...
                            processed_notes = self.token_counter.truncate_to_tokens(latest_notes, max_notes_tokens)
                            logger.debug(
                        ...
                        if total_with_system > self.token_budget.max_context - self.token_budget.response_reserve:
                            # Emergency truncation if still too large
                            excess = total_with_system - (
                                self.token_budget.max_context - self.token_budget.response_reserve
                            )
                            processed_notes = self.token_counter.truncate_to_tokens(
                                processed_notes,
                                self.token_counter.count_tokens(processed_notes) - excess,
                            )
                            context_parts = [
                                f"Task: {full_task}",
                                f"Recent notes:\n{processed_notes}",
                            ...
                            user_content = "\n".join(context_parts)
                            logger.warning("Emergency truncation applied: reduced notes to fit context limit")
...
- Be explicit in 'success_criteria' so the research agent knows exactly when the subtask is done."""
    ...
    async def run_subtask_research(self, task: Task) -> dict[str, any]:
        ...
        if not current_agent:
            ...
            if not task_state.research_plan or task_state.subtask_index >= len(task_state.research_plan.subtasks):
                return {
    ...
    async def next_action(self, task: Task, notes: str = "") -> str:
        ...
        async def _finalize_task() -> str:
            ...
            final_report_sources = task_state.all_findings if task_state.all_findings else [latest_notes]
            final_report = await self.synthesize_final_report(task, final_report_sources)
        ...
        if decision.subtask == "proceed" or decision.decision == "switch":
            ...
            if task_state.research_plan and task_state.subtask_index < len(task_state.research_plan.subtasks):
                logger.info(f"Proceeding to subtask {task_state.subtask_index + 1} for task {task.name}")
                ...
                logger.info(f"All subtasks completed for {task.name} after orchestrator decision")
                return await _finalize_task()
...
Focus on: Specific programs, exact deadlines, concrete requirements, direct application links."""
        ...
        for i, report in enumerate(research_reports):
            ...
            if self.context_management_enabled and self.summarizer and self.token_budget:
                try:
                    ...
                    logger.warning(f"Error summarizing report {i + 1}, using full content: {e}")
        ...
        if self.context_management_enabled and self.summarizer and self.token_budget:
            try:
                ...
                logger.warning(f"Error summarizing combined findings, using full content: {e}")
