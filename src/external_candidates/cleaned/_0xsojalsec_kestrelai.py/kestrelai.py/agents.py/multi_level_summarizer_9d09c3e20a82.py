# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\agents\multi_level_summarizer.py
"""
...
class MultiLevelSummarizer:
    ...
    def create_summary_hierarchy(self, content: str, preserve_facts: bool = True) -> dict[str, Any]:
        """
        ...
        for level in self.levels[1:]:  # Skip "detailed" level
            ...
            target_tokens = int(self.counter.count_tokens(current_content) * level.compression_ratio)
            ...
            summary = self._summarize_with_facts(current_content, target_tokens, level, extracted_facts)
...
JSON:"""
        ...
        try:
            ...
            if facts_dict:
                ...
                logger.warning(f"Could not extract JSON from response, using regex fallback. Response text: {response}")
        except Exception as e:
            # Capture exception details for better debugging
            exception_type = type(e).__name__
            exception_msg = str(e)
            response_text = response if response is not None else "Response not available"
    ...
    def _summarize_with_facts(
        ...
        prompt = self._build_summarization_prompt_with_facts(content, summary_target, level, facts)
...
""",
    ...
    def _build_summarization_prompt(self, content: str, target_tokens: int, level: SummaryLevel) -> str:
...
Summary (MUST include all facts listed above):"""
    ...
    def get_summary_at_level(self, summaries: dict[str, str], level_name: str) -> str | None:
        """
    ...
    def estimate_tokens_for_level(self, original_tokens: int, level_name: str) -> int:
        ...
        """
        level_cfg = next((level for level in self.levels if level.name == level_name), None)
        if not level_cfg:
    ...
    def create_summary_on_demand(
        ...
        for level in self.levels:
            ...
            estimated_tokens = self.estimate_tokens_for_level(original_tokens, level.name)

            # If this level should fit, create it
            if estimated_tokens <= max_tokens * 0.9:  # 90% threshold for safety
                summary = self._summarize_with_facts(content, estimated_tokens, level, facts)
        ...
        summary = self._summarize_with_facts(content, target_tokens, most_compressed, facts)
    ...
    def validate_summary_quality(
        ...
        summary_tokens = self.counter.count_tokens(summary)
        quality["compression_ratio"] = summary_tokens / original_tokens if original_tokens > 0 else 0.0
        ...
        if facts:
            ...
            for url in facts.urls:
                if url in summary or any(part in summary for part in url.split("/")[-2:]):
