# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\agents\context_manager.py
"""
...
class TokenCounter:
    ...
    def __init__(self, model_name: str = "gpt-4"):
        ...
        """
        if tiktoken is None:
            raise ImportError("tiktoken is required for token counting. Install it with: pip install tiktoken")
...
class ContextManager:
    ...
    def build_context(self, components: dict[str, Any], prioritize_recent: bool = True) -> tuple[str, dict[str, int]]:
        """
        ...
        if task:
            ...
            if task_tokens <= self.budget.task_description:
                ...
                truncated = self.counter.truncate_to_tokens(task, self.budget.task_description)
        ...
        if previous_findings:
            ...
            if findings_tokens <= self.budget.previous_findings:
                ...
                truncated = self.counter.truncate_to_tokens(previous_findings, self.budget.previous_findings)
        ...
        if checkpoints:
            ...
            for checkpoint in checkpoint_list:
                ...
                if checkpoint_tokens_used + checkpoint_tokens <= self.budget.checkpoints:
                    ...
                    if remaining > 100:  # Minimum viable chunk
                        truncated = self.counter.truncate_to_tokens(checkpoint, remaining)
        ...
        if rag_content:
            ...
            if rag_tokens <= self.budget.rag_content:
                ...
                # Truncate
                truncated = self.counter.truncate_to_tokens(rag_content, self.budget.rag_content)
        ...
        if url_reference:
            ...
            if url_tokens <= self.budget.url_reference:
                ...
                # Truncate
                truncated = self.counter.truncate_to_tokens(url_reference, self.budget.url_reference)
        ...
        available_budget = self.budget.max_context - self.budget.system_prompt - self.budget.response_reserve
        ...
        if total_tokens > available_budget and self.summarizer:
            try:
                ...
                logger.warning(f"Error in full context summarization, falling back to truncation: {e}")
                # Fall through to truncation

        # If still too large (or summarization failed), truncate as last resort
        if total_tokens > available_budget:
            logger.warning("Context still exceeds budget after summarization attempt, truncating")
