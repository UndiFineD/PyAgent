# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\agents\url_utils.py
"""
...
class URLFlagManager:
    ...
    def replace_flags_with_urls(self, text: str, flag_to_url: dict[str, str] = None) -> str:
        """
        ...
        if missing_flags:
            logger.warning(f"Found {len(missing_flags)} missing URL flags in output: {sorted(missing_flags)}")
    ...
    def get_url_reference_table(self) -> str:
        ...
        lines.append("IMPORTANT: Use the flags (e.g., [URL_1]) in your response, NOT the full URLs.")
        return "\n".join(lines)
