...
class WebResearchAgent(BaseResearchAgent):
    ...
    def __init__(self, agent_id: str, llm, memory: MemoryStore, config: ResearchConfig = None):
        ...
        try:
            ...
            logger.warning(f"Failed to initialize context management: {e}. Continuing without it.")
    ...
    async def run_step(self, task: Task) -> str:
        ...
        for loop_idx in range(self.config.think_loops):
            ...
            if state.is_in_loop():
                ...
                action: Literal["think", "search", "mcp_tool", "summarize", "complete"] = "summarize"
                ...
                if not plan:
                    if self.config.debug:
                        logger.warning(f"[WARN] Invalid JSON response, defaulting to think. Response text: {plan_raw}")
        ...
        if self.context_management_enabled and self.summarizer:
            try:
                hierarchy = self.summarizer.create_summary_hierarchy(final_report, preserve_facts=True)
    ...
    async def _handle_search_action(self, plan: dict, state: AgentState):
        ...
        for hit in hits:
            ...
            if clean_href is None:
                # Skip invalid URLs
                logger.warning(f"Skipping invalid URL from search results: {hit['href'][:100]}")
                continue
            ...
            if url_flag is None:
                ...
                logger.warning(f"Failed to create flag for valid URL: {clean_href[:100]}")
                continue
...
Do NOT write out full URLs - use the flags instead."""
    ...
    async def _create_checkpoint(self, task: Task, state: AgentState):
        ...
        recent_context_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(recent_context)
        previous_checkpoint_with_flags = ""
        if state.last_checkpoint:
            prev_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(state.last_checkpoint)
...
Be concise but include all actionable information the user can immediately use."""
        ...
        checkpoint = self.url_flag_manager.replace_flags_with_urls(checkpoint, flag_mapping)
        ...
        checkpoint_id = self._add_to_rag(
            ...
            metadata={
                ...
                "action_count": state.action_count,
                "importance_score": self._calculate_importance(checkpoint),  # Based on fact count
            },
        )
        ...
        if self.context_management_enabled and self.summarizer:
            try:
                hierarchy = self.summarizer.create_summary_hierarchy(checkpoint, preserve_facts=True)
                summaries = hierarchy.get("summaries", {})
                ...
                logger.warning(f"Failed to create/store checkpoint summaries: {e}", exc_info=True)
        ...
        if self.context_management_enabled and self.summarizer and self.token_budget:
            try:
                ...
                if previous_reports_context:
                    reports_tokens = self.token_counter.count_tokens(previous_reports_context)
                ...
                logger.warning(f"Error in report summarization, using full content: {e}")

        # Replace URLs with flags in all content before sending to LLM
        # Process all texts to build up the flag manager's complete mapping
        checkpoints_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(all_checkpoints)
        rag_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(rag_content)
        previous_reports_with_flags = ""
        if previous_reports_context:
            prev_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(previous_reports_context)
...
Focus on: Specific programs, exact deadlines, concrete requirements, direct application links, and building upon previous research with new findings."""
        ...
        final_report = self.url_flag_manager.replace_flags_with_urls(final_report, combined_flag_mapping)

        return final_report

    def _retrieve_from_rag(self, task: Task, query: str = None, max_tokens: int = None) -> str:
        """
        ...
        try:
            ...
            if self.hybrid_retrieval_enabled and self.hybrid_retriever:
                ...
                results = self.memory.search(search_query, k=20)  # Get more results, we'll filter by layer

                if not results or not results.get("documents") or not results["documents"][0]:
                    # Fallback to scratchpad if no RAG results
                    if self.scratchpad:
                        recent_entries = self.scratchpad[-5:] if len(self.scratchpad) > 5 else self.scratchpad
                        return "\n\n".join(recent_entries)
            ...
            task_docs.sort(
                key=lambda x: (
                    x.get("fused_score", 0.0),  # Primary: use fused_score directly (higher = better)
                    x["checkpoint_index"],  # Secondary: more recent checkpoints first
                    -x["distance"],  # Tertiary: lower distance = better (for tie-breaking)
                ),
    ...
    def _select_documents_by_budget(self, documents: list[dict], max_tokens: int) -> list[dict]:
        """
    ...
    def _calculate_importance(self, content: str) -> float:
        ...
        fact_count = sum(len(re.findall(pattern, content, re.IGNORECASE)) for pattern in fact_indicators)
        ...
        try:
            ...
            for doc_id, doc, meta in zip(ids, documents, metadatas):
                ...
                if current_layer == "episodic":
                    ...
                    has_semantic = any(
                        m.get("checkpoint_index") == checkpoint_idx and m.get("layer") == "semantic" for m in metadatas
                    )

                    if not has_semantic and checkpoint_idx in old_checkpoint_indices[:5]:
                        # Create semantic summary if it doesn't exist
                        # The summary should already exist from checkpoint creation, but verify
                        logger.debug(f"Checkpoint {checkpoint_idx} should be in semantic layer")
                    ...
                    if checkpoint_idx < state.checkpoint_count - SEMANTIC_LIMIT:
                        ...
                        has_summary = any(
                            m.get("checkpoint_index") == checkpoint_idx and m.get("layer") == "summary"
                            for m in metadatas
                        )
                        ...
                        if not has_summary:
                            ...
                            logger.debug(f"Compressed checkpoint {checkpoint_idx} from semantic to summary layer")
