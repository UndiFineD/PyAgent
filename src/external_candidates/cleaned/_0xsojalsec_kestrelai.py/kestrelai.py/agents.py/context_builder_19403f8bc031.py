...
class ContextBuilder:
        ...
        try:
            ...
            if self.config.is_subtask_agent:
                ...
                if self.config.previous_findings:
                    ...
                    (
                        findings_with_flags,
                        _,
                    ) = self.url_flag_manager.replace_urls_with_flags(self.config.previous_findings)
            ...
            if state.history:
                ...
                for entry in state.history:
                    # Replace URLs with flags
                    entry_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(entry)
            ...
            context, token_usage = self.context_manager.build_context(components, prioritize_recent=True)
    ...
    def _build_context_legacy(self, task: "Task", state: "AgentState") -> str:
        ...
        if self.config.is_subtask_agent:
            ...
            if self.config.previous_findings:
                # Replace URLs with flags in previous findings
                findings_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(self.config.previous_findings)
        ...
        if state.last_checkpoint:
            ...
            checkpoint_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(state.last_checkpoint)
        ...
        if state.history:
            ...
            history_with_flags, _ = self.url_flag_manager.replace_urls_with_flags(recent_history)
