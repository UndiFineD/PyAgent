...
class DashboardState:
    ...
    def add_report(self, task_name: str, content: str):
        ...
        self.report_history.append({"timestamp": datetime.now(), "task": task_name, "content": content})
        ...
        try:
            ...
            if orch.current and orch.current in orch.tasks:
                ...
                if task_state.research_plan:
                    ...
                    try:
                        state.latest_feedback = f"🧠 Generating research plan for: {first_task.name}..."
                        ...
                        if task_state.research_plan:
                            ...
                            logger.warning(f"Planning may not have completed for {first_task.name}")
                ...
                if hasattr(orch, "task_states") and orch.task_states and orch.current in orch.task_states:
                    task_state = orch.task_states[orch.current]
                    if task_state.research_plan and task_state.research_plan.subtasks:
                        ...
                        plan_data = {
                            ...
                            "subtasks": [
                                ...
                                for i, st in enumerate(task_state.research_plan.subtasks)
                            ],
            ...
            if orch.current in orch.task_states and orch.task_states[orch.current].research_plan is None:
            ...
            if notes and len(notes) > 10:
                ...
                try:
                    if hasattr(orch, "task_states") and orch.task_states and task.name in orch.task_states:
                        task_state = orch.task_states[task.name]
                        if task_state and task_state.research_plan:
                            ...
                            state.orchestrator_state[task.name] = {
                                ...
                                "feedback_history": task_state.feedback_history[-5:],  # Last 5
                                "completed_subtasks": list(task_state.completed_subtasks),
                                "subtask_index": task_state.subtask_index,
                                "total_findings": (
                                    len(task_state.all_findings) if hasattr(task_state, "all_findings") else 0
                                ),
                            }
                except Exception:
                        ...
                        if hasattr(orch, "task_states") and orch.task_states and task.name in orch.task_states:
                            ...
                            if (
                                ...
                                if agent and hasattr(agent, "get_task_metrics"):
                                    ...
                                    if task_metrics:
                                        agent_search_queries = task_metrics.get("searches", [])
                ...
                for search_query in search_queries:
                    if search_query and isinstance(search_query, str):
                        ...
                        if not any(
                            s.get("query") == search_query and s.get("task") == task.name for s in state.search_history
                        ):
                            ...
                            if task_state.subtask_index in task_state.subtask_agents:
                                ...
                                if (
                                    ...
                                    if agent_state:
                                        ...
                                        last_action_count = task_info_for_activity.get("last_agent_action_count", 0)
                                        if agent_state.action_count > last_action_count:
                                            has_meaningful_activity = True

                                            # Determine activity type based on recent actions
                                            if agent_state.search_count > task_info_for_activity.get(
                                                "last_search_count", 0
                                            ):
                                                activity_type = "search"
                                                message = f"🔍 Searching: {current_subtask or 'research'}"
                                            elif agent_state.think_count > task_info_for_activity.get(
                                                "last_think_count", 0
                                            ):
                                                activity_type = "thinking"
                                                message = f"🤔 Analyzing: {current_subtask or 'research'}"
                                            elif agent_state.summary_count > task_info_for_activity.get(
                                                "last_summary_count", 0
                                            ):
                                                activity_type = "summary"
                                                message = "📝 Summarizing findings"
                                            elif agent_state.checkpoint_count > task_info_for_activity.get(
                ...
                orchestrator_progress = progress_info.get("progress", 0) if progress_info else 0
                if task.status == TaskStatus.COMPLETE or progress >= 100.0 or orchestrator_progress >= 100.0:
                    ...
                    if orch.current is not None and orch.current in orch.tasks:
                        ...
                        try:
                            if hasattr(orch, "task_states") and orch.task_states and orch.current in orch.task_states:
                                task_state = orch.task_states[orch.current]
                                if task_state and (task_state.research_plan and task_state.research_plan.subtasks):
                                    state.latest_subtask = task_state.research_plan.subtasks[0].description
...
task_budget_input = pn.widgets.IntInput(name="Budget (minutes)", value=180, start=1, end=1440, width=200)
...
def create_task():
    ...
    task_description = task_description_raw.strip() if isinstance(task_description_raw, str) else ""

    if not task_name:
        pn.state.notifications.error("Task name is required. Please enter a task name.", duration=3000)
        return
    ...
    try:
        new_task = Task(name=task_name, description=task_description, budgetMinutes=budget_minutes)
    except Exception as e:
        error_msg = f"Error creating task: {str(e)}"
        print(error_msg)
        pn.state.notifications.error(f"Failed to create task: {str(e)[:100]}", duration=5000)
        return
    ...
    pn.state.notifications.success(f"Task '{task_name}' created successfully!", duration=3000)
    update_dashboard()
...
def start_task():
    ...
    if not tasks:
        pn.state.notifications.error("No tasks available. Please create a task first.", duration=3000)
        return
    ...
    if not task:
        ...
        if not task:
            pn.state.notifications.error(f"Task '{orch.current}' not found", duration=3000)
            return
    ...
    state.latest_feedback = f"🚀 Starting task: {task.name}\n\nInitializing research plan..."
...
task_form = pn.Column(
    ...
    pn.pane.HTML("<hr style='margin: 20px 0; border: 1px solid rgba(139, 111, 71, 0.2);'>"),
    sizing_mode="stretch_width",
)
...
def update_dashboard():
    try:
        ...
        if state.current_task in state.research_plans:
            ...
            completed_count = sum(1 for st in plan["subtasks"] if st["status"] == "completed")
            ...
            for subtask in plan["subtasks"]:
                ...
                if state.current_task in state.task_history:
                    ...
                    if "subtask_metrics" in task_info and subtask_idx in task_info["subtask_metrics"]:
        ...
        if state.current_task in state.orchestrator_state:
            ...
            stuck_class = "stuck-warning" if orch_state.get("stuck_count", 0) > 0 else ""
            ...
            if feedback_history:
                ...
                for feedback in feedback_history[-3:]:  # Last 3
                    ...
                    orch_html += (
                        f'<div class="feedback-item">{feedback_text}{"..." if len(str(feedback)) > 100 else ""}</div>'
                    )
                orch_html += "</div>"
