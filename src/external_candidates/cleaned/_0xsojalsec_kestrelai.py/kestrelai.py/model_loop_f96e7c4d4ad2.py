...
class KestrelAgentWorker:
    ...
    def __init__(self):
        ...
        self.task_reports: dict[str, list[str]] = {}  # Track all reports per task for accumulation
    ...
    def start_task(self, task_id: str, config: dict[str, Any]):
        ...
        self.redis_client.send_update(task_id, status=TaskStatus.ACTIVE.value, progress=0.0)
    ...
    def pause_task(self):
        ...
        if self.running and not self.paused and self.current_task_id:
            ...
            self.redis_client.send_update(self.current_task_id, status=TaskStatus.PAUSED.value)
    ...
    def resume_task(self):
        ...
        if self.running and self.paused and self.current_task_id:
            ...
            self.redis_client.send_update(self.current_task_id, status=TaskStatus.ACTIVE.value)
    ...
    def stop_task(self):
        ...
        if self.running and self.current_task_id:
            ...
            self.redis_client.send_update(
                ...
                progress=100.0,
                elapsed=int(time.time() - self.task_metrics.get(task_id, {}).get("start_time", time.time())),
            )
            self.redis_client.send_activity(task_id, "task_complete", "✅ Task completed")
    ...
    def process_task_step(self):
        ...
        if task_state and task_state.research_plan:
            ...
            last_completed_set = set(last_completed) if isinstance(last_completed, (set, frozenset)) else set()

            subtask_changed = (
                current_subtask_index != last_state.get("subtask_index", -1) or current_completed != last_completed_set
            )
        ...
        self.redis_client.send_update(
            task_id,
            status=(task.status.value if hasattr(task, "status") else TaskStatus.ACTIVE.value),
            progress=progress,
            elapsed=int(elapsed),
        ...
        for query in search_queries:
            if query and query not in self.task_metrics[task_id]["sent_searches"]:
                self.redis_client.send_search(task_id, query, results=4, sources=["orchestrator_search"])
        ...
        if has_meaningful_activity and (activity_type in ["summary", "checkpoint"] or (notes and len(notes) > 200)):
            # Save notes to file
        ...
        if task.status == TaskStatus.COMPLETE or progress >= 100.0 or progress_info.get("progress", 0) >= 100.0:
            self.stop_task()
