# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\KestrelAI\backend\main.py
"""
                    ...
                    if task:
                        ...
                        if "metrics" in update_data:
                            ...
                            system_metrics = {
                                ...
                                "llmCalls": metrics_dict.get("llmTokensUsed", 0) // 1000,  # Approximate
                                ...
                                "timestamp": update_data.get("timestamp", int(datetime.now().timestamp() * 1000)),
                            }
                            # Publish as separate metrics event
                            await r.publish(
                                f"kestrel:task:{update_data['taskId']}:updates",
                                json.dumps({"type": "metrics", "payload": system_metrics}),
                            )

                            # Also store in TASK_METRICS key for /metrics endpoint
                            metrics_key = RedisKeys.TASK_METRICS.format(task_id=update_data["taskId"])
...
async def export_task(
    ...
    if format == ExportFormat.JSON:
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename={task.name.replace(' ', '_')}_{task_id}.json"},
        )
...
"""
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename={task.name.replace(' ', '_')}_{task_id}.md"},
        )
