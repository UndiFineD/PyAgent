# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\tests\unit\test_context_manager.py
"""
...
class TestTokenBudget:
    ...
    def test_token_budget_custom(self):
        ...
        budget = TokenBudget(max_context=16384, system_prompt=1000, response_reserve=1000)
    ...
    def test_available_for_context(self):
        ...
        budget = TokenBudget(max_context=10000, system_prompt=500, response_reserve=1000)
...
class TestContextManager:
    ...
    def token_counter(self):
        ...
        counter.count_tokens = Mock(side_effect=lambda x: len(str(x).split()) * 1.3)  # Rough estimate
        counter.truncate_to_tokens = Mock(side_effect=lambda x, max_t: x[:max_t] + "...")
        return counter
    ...
    def test_context_manager_initialization(self, context_manager, token_counter, token_budget):
    ...
    def test_build_context_prioritizes_recent(self, context_manager, token_counter):
        ...
        context, usage = context_manager.build_context(components, prioritize_recent=True)
    ...
    def test_build_context_checkpoint_prioritization(self, context_manager, token_counter):
    ...
    def test_build_context_token_usage_tracking(self, context_manager, token_counter):
        ...
        component_sum = sum(v for k, v in usage.items() if k != "total" and k != "summarized")
