# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\tests\unit\test_agents.py
...
class TestResearchOrchestrator:
    ...
    def orchestrator(self, mock_llm, mock_task):
        ...
        with patch("KestrelAI.agents.research_orchestrator.MemoryStore"):
            with patch("KestrelAI.agents.research_orchestrator.WebResearchAgent") as mock_agent_cls:
                mock_agent = Mock()
    ...
    @pytest.mark.asyncio
    async def test_next_action_advances_subtask_on_review(self, orchestrator, mock_task, mock_research_plan):
...
class TestWebResearchAgent:
    ...
    async def test_web_search(self, research_agent):
        ...
        research_agent.searxng_service.extract_text = Mock(return_value="Fetched page content")
