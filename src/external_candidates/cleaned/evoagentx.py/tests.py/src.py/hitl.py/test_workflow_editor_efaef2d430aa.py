# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\hitl\test_workflow_editor.py
import json
import os
import shutil
import sys
...
from evoagentx.hitl.workflow_editor import (
    MockLLM,
    MockLLMConfig,
    WorkFlowEditor,
...
class TestWorkFlowEditor(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.test_workflow_file = os.path.join(
        ...
        if self.expected_optimized_workflow["nodes"]:
            ...
            self.expected_optimized_workflow["edges"] = [
                edge
                for edge in self.expected_optimized_workflow["edges"]
...
class TestWorkFlowEditorIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        if self.optimized_workflow["nodes"]:
            ...
            self.optimized_workflow["edges"] = [
                edge
                for edge in self.optimized_workflow["edges"]
                if edge["target"] != last_node["name"] and edge["source"] != last_node["name"]
            ]
    ...
    async def test_full_workflow_editing_process(self, mock_workflow_graph, mock_workflow):
        ...
        result1 = await editor.edit_workflow(file_path=self.test_workflow_file, instruction=self.test_instruction)
