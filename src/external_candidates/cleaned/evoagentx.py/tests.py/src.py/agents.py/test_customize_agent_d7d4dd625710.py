# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\tests\src\agents\test_customize_agent.py
import os
import unittest
...
class TestModule(unittest.TestCase):
    ...
    def test_simple_agent(self, mock_generate):
        ...
        self.assertEqual(
            simple_agent.get_prompts()["SimpleAgentAction"]["prompt"],
            "You are a simple agent that prints hello world.",
        )
    ...
    def test_agent_with_inputs_and_outputs(self, mock_generate):
        ...
        msg = new_agent_with_inputs(
            inputs={"requirement": "Write Python code that prints hello world"},
        ...
        new_agent_with_inputs_outputs: CustomizeAgent = CustomizeAgent.from_file(
            self.save_files[3], llm_config=llm_config
        )
        ...
        msg = new_agent_with_inputs_outputs(
            inputs={"requirement": "Write Python code that prints hello world"},
            return_msg_type=MessageType.RESPONSE,
        )
        self.assertEqual(msg.msg_type, MessageType.RESPONSE)
        ...
        self.assertEqual(
            new_agent_with_parser.action.outputs_format.__name__,
            "CodeWriterActionOutput",
        )

        msg = new_agent_with_parser(
            inputs={"requirement": "Write Python code that prints hello world"},
            return_msg_type=MessageType.RESPONSE,
        )
        self.assertEqual(msg.msg_type, MessageType.RESPONSE)
