# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\customize_agent.py
import os
...
from evoagentx.prompts import ChatTemplate, StringTemplate
    ...
    code_writer = CustomizeAgent(
        ...
        inputs=[
            ...
            },
            {
                "name": "file_path",
                "type": "string",
                "description": "The path to save the code",
            },
        ],
        # outputs=[
...
def build_customize_agent_with_MCP(config_path):
    ...
    customize_agent = CustomizeAgent(
        ...
        prompt_template=StringTemplate(instruction="Do some tasks using the tools"),
        llm_config=model_config,
        inputs=[
            {
                "name": "instruction",
                "type": "string",
                "description": "The instruction to the tool user",
            }
        ],
        outputs=[
            {
            ...
            },
            {
                "name": "tool_calls",
                "type": "string",
                "description": "The tool calls used to get the result (if any)",
            },
        ],
        tools=tools,
    )
        ...
        inputs=[
            {
                "name": "recipe_text",
                "type": "string",
                "description": "The recipe text to analyze",
            }
        ],
        outputs=[
...
def test_title_parse_mode():
    ...
    report_agent = CustomizeAgent(
        ...
        llm_config=model_config,
        inputs=[{"name": "topic", "type": "string", "description": "The topic to analyze"}],
        outputs=[
