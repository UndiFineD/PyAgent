        ...
        try:
            ...
            if index_type not in self.indices[corpus_id]:
                ...
                self.retrievers[corpus_id][index_type] = self.retriever_factory.create(
                    ...
                    query=Query(
                        query_str="",
                        top_k=(self.config.retrieval.top_k if self.config.retrieval else 5),
                    ),
                    storage_handler=self.storage_handler,
    ...
    def save(
        ...
        try:
            ...
            for cid in target_corpora:
                ...
                target_indices = (
                    [index_type] if index_type and index_type in self.indices[cid] else self.indices[cid].keys()
                )
                for idx_type in target_indices:
                    ...
                    chunks = [
                        self.chunk_class.from_llama_node(node_data) for node_id, node_data in index.id_to_node.items()
                    ]
    ...
    async def query_async(
        ...
        try:
            ...
            query.similarity_cutoff = (
                self.config.retrieval.similarity_cutoff if query.similarity_cutoff is None else query.similarity_cutoff
            )
            query.keyword_filters = (
                self.config.retrieval.keyword_filters if query.keyword_filters is None else query.keyword_filters
            )
