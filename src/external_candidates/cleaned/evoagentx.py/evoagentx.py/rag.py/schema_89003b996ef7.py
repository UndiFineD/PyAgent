# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\schema.py
import hashlib
import json
...
class DocumentMetadata(BaseModule):
    ...
    last_modified_date: Optional[str] = Field(
        default=None, description="The last modified date and time of the document."
    )
...
class GraphNodeData(BaseModule):
    ...
    label: Optional[str] = Field(
        default="entity",
        description="The label name of the 'LabelNode', 'EntityNode', 'Relation' in llama_index node.",
    )
...
class ChunkMetadata(DocumentMetadata):
    ...
    content: Optional[str] = Field(
        default=None,
        description="the content of the message, will be dumps by 'dumps' from json lib.",
    )
    ...
    msg_type: Optional[str] = Field(
        default=None,
        description="The type of the message (e.g., 'request', 'response').",
    )
    prompt: Optional[Union[str, List[dict]]] = Field(
        default=None, description="The prompt used to generate the message."
    )
    ...
    action: Optional[str] = Field(
        default=None,
        description="the trigger of the message, normally set as the action name.",
    )
...
class IndexMetadata(BaseModule):
    ...
    collection_name: Optional[str] = Field(
        ...
        description="Vector store collection name or FAISS file path",
    )
    dimension: Optional[int] = Field(default=1536, description="Vector dimension")
    vector_db_type: Optional[str] = Field(
        default=None,
        description="Vector database type (e.g., 'faiss', 'qdrant', 'chroma')",
    )
...
class Corpus(BaseModule):
    ...
    def __init__(
        self,
        chunks: Optional[List[Union[TextChunk, ImageChunk]]] = None,
    ...
    def add_chunk(
        self,
        batch_chunk: Union[TextChunk, ImageChunk, List[Union[TextChunk, ImageChunk]]],
    ):
    ...
    def filter_by_similarity(self, threshold: float) -> List[Union[TextChunk, ImageChunk]]:
        ...
        return [
            ...
            if chunk.metadata.similarity_score and chunk.metadata.similarity_score >= threshold
        ]
        ...
        with open(input_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                chunk = Chunk(
                    ...
                    relationships={k: RelatedNodeInfo(**v) for k, v in chunk_dict["relationships"].items()},
                )
                chunks.append(chunk)
...
class Query(BaseModule):
    ...
    custom_embedding_strs: Optional[List[str]] = Field(
        default=None,
        description="The List to store additional strings need to be embed with the query.",
    )
