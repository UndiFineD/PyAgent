...
class MultiAgentDebateActionGraph(ActionGraph):
    ...
    llm_config: LLMConfig = Field(
        default_factory=lambda: OpenAILLMConfig(model="gpt-4o-mini", openai_key=os.getenv("OPENAI_API_KEY")),
    ...
    def from_dict(cls, data: Dict[str, Any], **kwargs) -> "MultiAgentDebateActionGraph":
        ...
        if data.get("debater_agents"):
            ...
            for agent_info in data["debater_agents"]:
                try:
                    ...
                    agent_config_clean = {
                        k: v for k, v in agent_config.items() if k not in ["name", "description", "llm_config"]
                    }
        ...
        if data.get("judge_agent"):
            try:
                ...
                judge_config_clean = {
                    k: v for k, v in judge_config.items() if k not in ["name", "description", "llm_config"]
                }
