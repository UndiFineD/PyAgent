...
class WorkFlowGraph(BaseModule):
    ...
    def get_next_candidate_nodes(self) -> List[str]:
        ...
        children_nodes_with_complete_predecessors = self.filter_nodes_with_uncompleted_predecessors(
    ...
    def get_workflow_description(self) -> str:
        ...
        def format_parameters(params: List[Parameter]) -> str:
            if not params:
                return "None"
            return "\n".join(
                f"  - {param.name} ({param.type}, {format_param_requirement(param.required)}): {param.description}"
                for param in params
            )
