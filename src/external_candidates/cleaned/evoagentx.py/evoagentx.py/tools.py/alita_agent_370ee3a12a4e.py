...
class GeneratedCodeTool(Tool):
    ...
    description: str = "A dynamic tool that executes LLM-generated source code using a shared code execution backend."
    inputs: Dict[str, Dict[str, str]] = {
        "payload": {
            "type": "object",
            "description": ("Optional payload object made available to the generated code as the variable `payload`."),
        }
    }
    ...
    def __call__(self, payload: dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            payload_json = json.dumps({"raw_payload_repr": repr(payload)}, ensure_ascii=False)
...
class AlitaDynamicToolkit(Toolkit):
    ...
    def __init__(
        ...
        if self._code_executor is None:
            logger.warning(
                "AlitaDynamicToolkit initialized without a code execution tool; generated tools will not be executable."
            )
...
class ReloadDynamicTools(Tool):
    ...
    description: str = "Reload dynamic generated tools from the configured persistence path."
    inputs: Dict[str, Dict[str, str]] = {}
    required: Optional[List[str]] = []
...
def create_alita_agent(
    ...
    if use_docker:
        try:
            ...
            logger.warning(
                "Failed to initialize DockerInterpreterToolkit (%s); falling back to PythonInterpreterToolkit.",
                exc,
            )
