# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\gmail_tools.py
"""
...
class GmailBase(BaseModule):
    ...
    def __init__(self, client_secret_file: str = None, token_file: str = None, **kwargs):
        ...
        self.client_secret_file = client_secret_file or os.getenv("GMAIL_CLIENT_SECRET_FILE", CLIENT_SECRET_FILE)
        self.token_file = token_file or os.getenv("GMAIL_TOKEN_FILE", TOKEN_FILE)
    ...
    def _modify_message_labels(
        self, message_id: str, add_labels: list = None, remove_labels: list = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not body:
                return {
                    "success": False,
                    "error": "No labels provided to add or remove.",
                }
    ...
    def _fetch_messages(
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to fetch messages: {str(e)}"}
        except Exception as e:
    ...
    def _fetch_threads(self, query: str = None, label_ids: list = None, max_results: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if not threads:
                return {"success": True, "threads_count": 0, "threads": []}
            ...
            return {"success": False, "error": f"Failed to fetch threads: {str(e)}"}
        except Exception as e:
...
class SearchMessagesTool(Tool):
    ...
    def __call__(
        ...
        """
        result = self.gmail_base._fetch_messages(
            keyword, label_ids, max_results, include_attachments=include_attachments
        )
...
class TrashMessagesTool(Tool):
    ...
    def __call__(self, message_ids: list = None, query: str = None, max_results: int = 50) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not target_ids and query:
                results = (
                    service.users().messages().list(userId="me", q=query, maxResults=min(max_results, 500)).execute()
                )
            ...
            return {"success": False, "error": f"Failed to trash messages: {str(e)}"}
        except Exception as e:
...
class ModifyLabelsTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        ...
        "query": {
            "type": "string",
            "description": "Gmail query to select messages (optional)",
        },
        "add_labels": {
            "type": "array",
            "description": "List of label names to add (e.g., ['STARRED','MyLabel'])",
        },
        "remove_labels": {
            "type": "array",
        ...
        "max_results": {
            "type": "integer",
            "description": "Max messages when using query (default 50)",
        },
    }
    required: List[str] = []
    ...
    def __call__(
        ...
        try:
            ...
            if not targets:
                return {
                    "success": False,
                    "error": "No target messages. Provide message_ids or a query.",
                }
...
class ListDraftsTool(Tool):
    ...
    description: str = "List all Gmail drafts. Returns a list of draft IDs and basic information."
    inputs: Dict[str, Dict[str, str]] = {
        "max_results": {
            "type": "integer",
    ...
    def __call__(self, max_results: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to list drafts: {str(e)}"}
        except Exception as e:
...
class SearchDraftTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        "draft_id": {
            "type": "string",
            "description": "Draft ID to retrieve directly (optional; if provided, returns that draft immediately)",
        },
        "keyword": {
            "type": "string",
            "description": "Keyword to search in drafts (optional; scans multiple drafts if provided)",
        },
        "max_results": {
            "type": "integer",
            "description": "Max drafts to scan when using keyword search (default 50)",
        },
    }
    required: List[str] = []
    ...
    def __call__(self, draft_id: str = None, keyword: str = None, max_results: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            if not keyword:
                return {
                    "success": False,
                    "error": "Provide draft_id or keyword to search.",
                }
            ...
            for d in drafts:
                try:
                    ...
                    k = keyword.lower()
                    if any(
                        k in (str(formatted.get(k2, "")).lower()) for k2 in ("subject", "from", "to", "snippet")
            ...
            return {
                "success": False,
                "error": f"Failed to search/retrieve drafts: {str(e)}",
            }
        except Exception as e:
...
class DeleteDraftTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {"draft_id": {"type": "string", "description": "ID of the draft to delete"}}
...
class UpdateDraftTool(Tool):
    ...
    def __call__(
        ...
        try:
            ...
            # Get existing draft
            existing_draft = service.users().drafts().get(userId="me", id=draft_id, format="full").execute()
            ...
            # Update draft
            updated_draft = (
                service.users().drafts().update(userId="me", id=draft_id, body={"message": message}).execute()
            )
...
class SendDraftTool(Tool):
    ...
    name: str = "send_draft"
    description: str = "Send a Gmail draft by its ID. The draft will be sent as an email and removed from drafts."
    inputs: Dict[str, Dict[str, str]] = {"draft_id": {"type": "string", "description": "The ID of the draft to send"}}
    required: List[str] = ["draft_id"]
    ...
    def __call__(self, draft_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to send draft: {str(e)}"}
        except Exception as e:
