# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\google_maps_tool.py
"""
...
class PlacesSearchTool(Tool):
    ...
    description: str = (
        "Search for places (restaurants, shops, landmarks) using text queries. Can search near a specific location."
    )
    inputs: Dict[str, Dict[str, str]] = {
        "query": {
            "type": "string",
        ...
        if result["success"] and result["data"].get("rows"):
            ...
            for i, row in enumerate(result["data"]["rows"]):
                ...
                for j, element in enumerate(row.get("elements", [])):
                    destination_result = {
                        "destination_address": (
                            destination_addresses[j] if j < len(destination_addresses) else f"Destination {j + 1}"
                        ),
