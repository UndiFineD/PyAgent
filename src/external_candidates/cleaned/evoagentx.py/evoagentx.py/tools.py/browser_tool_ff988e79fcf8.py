# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\browser_tool.py
import time
...
class BrowserBase(BaseModule):
    ...
    browser_type: str = Field(
        default="chrome",
        description="Type of browser to use ('chrome', 'firefox', 'safari', 'edge')",
    )
    ...
    user_data_dir: Optional[str] = Field(
        default=None, description="User data directory for persistent browser sessions"
    )
    ...
    def _parse_element_reference(self, ref: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if not self.element_references:
            return (
                None,
                None,
                "No page snapshot available. Use browser_snapshot or navigate_to_url first.",
            )
        ...
        if not stored_ref:
            return (
                None,
                None,
                f"Element reference '{ref}' not found. Use browser_snapshot or navigate_to_url first.",
            )
    ...
    def _find_element_with_wait(
        ...
        try:
            ...
            return (
                None,
                f"Element not found or condition not met with selector: {selector}",
            )
        except Exception as e:
            logger.error(f"Error finding element {selector}: {str(e)}")
            return None, str(e)

    def _handle_function_params(
        self,
        function_params: Optional[list],
        function_name: str,
    ...
    def initialize_browser(self, function_params: list = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.browser_type == "chrome":
                ...
                return {
                    "status": "error",
                    "message": f"Unsupported browser type: {self.browser_type}",
                }

            self.driver.set_page_load_timeout(self.timeout)
            return {
                "status": "success",
                "message": f"Browser {self.browser_type} initialized successfully",
            }
        except Exception as e:
        ...
        try:
            ...
            if snapshot_result["status"] == "success":
                return {
                    ...
                    "snapshot": {"interactive_elements": snapshot_result.get("interactive_elements", [])},
                }
            else:
    ...
    def find_element(self, selector: str, selector_type: str = "css", timeout: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if error:
                return {
                    "status": "not_found",
                    "message": f"Element not found with {selector_type}: {selector}",
                }
    ...
    def find_multiple_elements(self, selector: str, selector_type: str = "css", timeout: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if error:
                return {
                    "status": "not_found",
                    "message": f"No elements found with {selector_type}: {selector}",
                }
    ...
    def click_element(self, selector: str, selector_type: str = "css", timeout: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if error:
                return {
                    "status": "not_found",
                    "message": f"Element not clickable with {selector_type}: {selector}",
                }
    ...
    def input_text(
        ...
        if not ref or not text:
            return {
                "status": "error",
                "message": "Both ref and text parameters are required",
            }
        ...
        try:
            ...
            if submit:
                ...
                if snapshot_result["status"] != "success":
                    logger.warning(
                        f"Failed to capture snapshot after form submission: {snapshot_result.get('message')}"
                    )

            return {
                "status": "success",
                "message": f"Successfully input text into {element_desc}" + (" and submitted" if submit else ""),
                "element": element_desc,
                "text": text,
            }
        ...
        try:
            if reference_type == "index":
                try:
                    ...
                    return {
                        "status": "error",
                        "message": f"Invalid frame index: {frame_reference}",
                    }
                ...
                if len(selector_parts) != 2:
                    return {
                        "status": "error",
                        "message": "Element reference must be in format 'selector_type:selector'",
                    }
                ...
                if element_result["status"] != "success":
                    return {
                        "status": "error",
                        "message": f"Could not find frame element: {element_result['message']}",
                    }
                ...
                return {
                    "status": "error",
                    "message": f"Invalid reference type: {reference_type}",
                }
            ...
            if reference_type == "index":
                try:
                    ...
                    if index < 0 or index >= len(window_handles):
                        return {
                            "status": "error",
                            "message": f"Window index out of range: {index}",
                        }
                    ...
                    return {
                        "status": "error",
                        "message": f"Invalid window index: {window_reference}",
                    }
            elif reference_type == "handle":
                if window_reference not in window_handles:
                    return {
                        "status": "error",
                        "message": f"Window handle not found: {window_reference}",
                    }
                ...
                if not window_found:
                    ...
                    return {
                        "status": "error",
                        "message": f"No window with title '{window_reference}' found",
                    }
            else:
                return {
                    "status": "error",
    ...
    def select_dropdown_option(
        ...
        try:
            ...
            if error:
                return {
                    "status": "not_found",
                    "message": f"Dropdown element not found with {selector_type}: {select_selector}",
                }
            ...
            if select_by.lower() == "value":
                ...
                try:
                    ...
                    return {
                        "status": "error",
                        "message": f"Invalid index value: {option_value}. Must be an integer.",
                    }
            else:
                return {
                    "status": "error",
                    "message": f"Invalid select_by option: {select_by}",
                }

            return {
                "status": "success",
                "message": f"Selected option with {select_by}: {option_value}",
            }
        except Exception as e:
    ...
    def browser_click(self, element: str = None, ref: str = None, function_params: list = None) -> Dict[str, Any]:
        ...
        if function_params and not ref:
            params = self._handle_function_params(
                function_params, "browser_click", {"element": "element", "ref": "ref"}
            )
            element = params.get("element", element)
            ref = params.get("ref", ref)
    ...
    def _classify_element_interactivity(self, element_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_disabled = (
            element_data.get("attributes", {}).get("disabled") is not None
            or element_data.get("attributes", {}).get("aria-disabled") == "true"
            or element_data.get("attributes", {}).get("aria-hidden") == "true"
        )
        ...
        if not is_disabled and is_visible:
            ...
            element_data["interactable"] = (
                tag_name in interactive_tags or role in interactive_roles or has_interactive_attrs
            )
            ...
            element_data["editable"] = (
                ...
                (
                    tag_name == "INPUT"
                    and element_data.get("attributes", {}).get("type", "text").lower() in editable_input_types
                )
                or tag_name == "TEXTAREA"
                or
                # Rich text editing
                element_data.get("attributes", {}).get("contenteditable") == "true"
                or
...
class InputTextTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        ...
        "text": {"type": "string", "description": "Text to input into the element"},
        "submit": {
            "type": "boolean",
...
class BrowserConsoleMessagesTool(Tool):
    ...
    def __call__(self, function_params: list = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Error getting console messages: {str(e)}",
            }
