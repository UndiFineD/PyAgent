...
class WorkFlowEditorReturn(BaseModel):
    ...
    workflow_json: Dict[str, Any] | None = Field(description="The workflow JSON structure after editing")

    workflow_json_path: str | None = Field(description="The file path where the workflow JSON is saved")

    error_message: Optional[str] | None = Field(default=None, description="Error message if the operation failed")
...
class WorkFlowEditor(BaseModule):
    ...
    async def edit_workflow(self, file_path: str, instruction: str, new_file_path: Optional[str] = None):
        ...
        if new_file_path is None:
            new_file_path = (
                "new_json_for__" + os.path.split(file_path)[-1] + "__" + time.strftime("%Y%m%d_%H%M%S") + ".json"
            )
