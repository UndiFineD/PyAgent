# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\hitl\special_hitl_agent.py
import asyncio
import json
import sys
...
class HITLOutsideConversationAction(Action):
        ...
        while True:
            try:
                ...
                if choice == "1":
                    ...
                    if user_advice == "q":
                        continue
                    workflow_json = await self._llm_optimize_workflow(
                        llm, workflow_json, user_advice if user_advice else None
                    )
    ...
    async def _llm_optimize_workflow(
        self, llm: BaseLLM, workflow_json: Dict[str, Any], user_advice: str = None
    ) -> Dict[str, Any]:
        ...
        **kwargs,
        ...
        if hasattr(self, "actions") and len(self.actions) > 0:
            if isinstance(self.actions[0], HITLOutsideConversationAction):
                ...
                raise ValueError(
                    f"The first action of {self.name} must be HITLOutsideConversationAction, but got {self.actions[0].__class__}"
                )
        else:
    ...
    def conversation_action(self):
        ...
        for action in self.actions:
            if isinstance(action, HITLOutsideConversationAction):
                return action
        raise ValueError(
            f"Action of class {HITLOutsideConversationAction.__name__} not found in {self}, please check the initialization of this Agent"
        )
