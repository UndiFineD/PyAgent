...
async def create_agent(agent: AgentCreate, current_user: Dict[str, Any] = Depends(get_current_active_user)):
    """Create a new agent."""
    try:
        created_agent = await AgentService.create_agent(agent, user_id=str(current_user["_id"]))
...
async def get_agent(agent_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)):
...
async def delete_agent(agent_id: str, current_user: Dict[str, Any] = Depends(get_current_admin_user)):
...
async def create_workflow(
    ...
    try:
        created_workflow = await WorkflowService.create_workflow(workflow, user_id=str(current_user["_id"]))
...
async def get_workflow(workflow_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)):
...
async def delete_workflow(workflow_id: str, current_user: Dict[str, Any] = Depends(get_current_admin_user)):
...
async def list_workflows(
    ...
    converted_workflows = [{**workflow, "_id": str(workflow["_id"])} for workflow in workflows]
...
async def get_execution(execution_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)):
...
async def stop_execution(execution_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)):
...
async def list_executions(
    ...
    executions, total = await WorkflowExecutionService.list_executions(params=pagination, search=search)
