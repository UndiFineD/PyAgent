# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\prompts\template.py
...
class PromptTemplate(BaseModule):
    ...
    context: Optional[str] = Field(
        default=None,
        description="Additional context that can help the LLM understand the instruction.",
    )
    constraints: Optional[Union[List[str], str]] = Field(
        default=None, description="Constraints that the LLM must follow."
    )
    ...
    history: Optional[List[Any]] = Field(
        default=None,
        description="History of the conversation between the user and the LLM.",
    )
    ...
    def get_output_template(
        ...
        if parse_mode not in valid_modes:
            raise ValueError(
                f"Invalid parse mode `{parse_mode}` for `{self.__class__.__name__}.get_output_template`. Valid modes are: {valid_modes}."
            )
    ...
    def render_outputs(
        ...
        ouptut_template, output_keys = self.get_output_template(
            outputs_format, parse_mode=parse_mode, title_format=title_format
        )
        output_str = (
...
class StringTemplate(PromptTemplate):
    def render_demonstrations(
        ...
        **kwargs,
        ...
        for i, demo in enumerate(self.demonstrations):
            ...
            if custom_output_format is not None or parse_mode in [
                None,
                "str",
                "custom",
            ]:
                ...
                output_template, output_keys = self.get_output_template(
                    outputs_format, parse_mode=parse_mode, title_format=title_format
                )
    ...
    def render_inputs(self, inputs_format: Type[LLMOutputParser], values: dict) -> str:
        if (inputs_format is None and values is None) or (
            inputs_format is not None and len(inputs_format.get_attrs()) == 0
        ):
            return ""
    ...
    def format(
        ...
        **kwargs,
        ...
        if parse_mode not in PARSER_VALID_MODE:
            raise ValueError(
                f"Invalid parse mode `{parse_mode}` for `{self.__class__.__name__}.format`. Valid modes are: {PARSER_VALID_MODE}."
            )
...
class ChatTemplate(StringTemplate):
    ...
    def format(
        ...
        **kwargs,
        ...
        if parse_mode not in PARSER_VALID_MODE:
            raise ValueError(
                f"Invalid parse mode `{parse_mode}` for `{self.__class__.__name__}.prompt`. Valid modes are: {PARSER_VALID_MODE}."
            )
...
class MiproPromptTemplate(ChatTemplate):
    def render_demonstrations(
        self,
        inputs_format: LLMOutputParser,
        outputs_format: LLMOutputParser,
        parse_mode: str,
        title_format: str = None,
        custom_output_format: str = None,
    ) -> List[dict]:
        ...
        return super().render_demonstrations(
            inputs_format,
            outputs_format,
            parse_mode,
            title_format,
