# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\memory\long_term_memory.py
import asyncio
import hashlib
import json
...
from evoagentx.rag.schema import Chunk, ChunkMetadata, Corpus, Query, RagResult
...
class LongTermMemory(BaseMemory):
        ...
        for msg, memory_id in zip(messages, memory_ids):
            ...
            if content_hash in existing_hashes:
                ...
                existing_id = next(
                    (
                        r["memory_id"]
                        for r in self.storage_handler.load(tables=[self.memory_table]).get(self.memory_table, [])
    ...
    async def get(
        self, memory_ids: Union[str, List[str]], return_chunk: bool = True
    ) -> List[Tuple[Union[Chunk, Message], str]]:
        ...
        try:
            ...
            results = [
                (
                    (self._chunk_to_message(chunk), chunk.metadata.memory_id)
                    if not return_chunk
                    else (chunk, chunk.metadata.memory_id)
                )
                for chunk in chunks
                if chunk
            ]
    ...
    async def search_async(
        ...
        if isinstance(query, str):
            ...
            if metadata_filters:
                query_obj.metadata_filters = (
                    {**query_obj.metadata_filters, **metadata_filters}
                    if query_obj.metadata_filters
