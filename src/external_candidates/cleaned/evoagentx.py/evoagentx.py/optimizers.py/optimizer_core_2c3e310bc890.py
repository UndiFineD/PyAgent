# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\optimizer_core.py
...
class BaseCodeBlockOptimizer(abc.ABC):
    ...
    def _check_codeblock_compat(self, code_block: CodeBlock):
        ...
        if unknown:
            ...
            warnings.warn(
                f"PromptRegistry fields {unknown} are not present in "
                f"{code_block.name}() signature; they will be ignored."
            )

    def run(
        self, code_block: CodeBlock, evaluator: Callable[[Dict[str, Any], Any], float]
...
class RandomSearchOptimizer(BaseCodeBlockOptimizer):
    def sample_cfg(self) -> Dict[str, Any]:
        return {
            ...
            "sys_prompt": random.choice(["You are a helpful assistant.", "You are a super-concise assistant."]),
        }
