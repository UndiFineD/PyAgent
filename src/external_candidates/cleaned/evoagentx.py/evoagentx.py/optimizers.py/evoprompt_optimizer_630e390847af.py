...
[Your evolved prompt here]""",
    ...
    async def optimize(self, benchmark: BIGBenchHard) -> tuple[Dict[str, str], dict, dict]:
        ...
        if self.full_evaluation:
            ...
            combination_scores = await self._evaluate_combinations_and_update_node_scores(
            ...
            self.best_scores_per_gen["Gen_0"] = {
                name: max(scores) if scores else 0 for name, scores in self.node_scores.items()
            }
            self.avg_scores_per_gen["Gen_0"] = {
                name: np.mean(scores) if scores else 0 for name, scores in self.node_scores.items()
            }
        ...
        if self.full_evaluation:
            ...
            best_config, _ = (
                max(combo_population_with_scores, key=lambda x: x[1]) if combo_population_with_scores else ({}, 0)
            )
...
[Your evolved prompt here]""",
    ...
    async def optimize(self, benchmark: BIGBenchHard) -> tuple[Dict[str, str], dict, dict]:
        ...
        best_combination, best_score = (
            max(combo_pop_with_scores, key=lambda x: x[1]) if combo_pop_with_scores else ({}, 0)
        )
