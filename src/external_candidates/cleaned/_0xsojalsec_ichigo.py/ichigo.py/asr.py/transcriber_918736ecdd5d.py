# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ichigo\ichigo\asr\transcriber.py
import time
import warnings
...
class IchigoASR:
    ...
    def transcribe(
        ...
        # Handle folder
            ...
            audio_files = [f for f in input_path.iterdir() if f.is_file() and f.suffix.lower() in extensions]
            non_audio = [f for f in input_path.iterdir() if f.is_file() and f.suffix.lower() not in extensions]
            ...
            if not audio_files:
                warnings.warn(f"No audio files found with extensions {extensions} in {input_path}")
                return {}
            ...
            with open(output_path, "w", encoding="utf-8", newline="") as f:
                ...
                for audio_file in sorted(audio_files):
                    try:
                        ...
                        safe_transcript = f'"{transcript}"' if "," in transcript else transcript
