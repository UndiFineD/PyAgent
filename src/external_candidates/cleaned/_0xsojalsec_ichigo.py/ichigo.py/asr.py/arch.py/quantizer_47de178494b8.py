# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ichigo\ichigo\asr\arch\quantizer.py
import math
...
class Quantizer(nn.Module):
    ...
    def _init_model_components(self):
        ...
        self.mlp = nn.Sequential(nn.Linear(self.width, n_mlp), nn.GELU(), nn.Linear(n_mlp, self.width))
        self.mlp_ln = LayerNorm(self.width)

        if self.downsample_conv:
            self.downsample_conv = nn.Conv1d(self.width, self.width, kernel_size=3, stride=self.downsample, padding=1)
        else:
