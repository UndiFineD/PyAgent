# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ichigo\ichigo\asr\arch\layers.py
import math
...
class MultiHeadAttention(nn.Module):
    ...
    def merge_linears(self, layers, mults):
        ...
        with torch.no_grad():
            ...
            new.bias[:] = torch.cat(
                [torch.zeros_like(bias) if x.bias is None else x.bias * m for x, m in zip(layers, mults)]
            )
        return new
    ...
    def convert_for_eval(self):
        ...
        if self.cross:
            ...
            self.kv = self.merge_linears([self.key, self.value], [self.sqrt_qk_scale, 1])
        else:
