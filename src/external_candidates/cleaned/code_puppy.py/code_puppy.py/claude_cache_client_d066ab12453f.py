# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\claude_cache_client.py
...
class ClaudeCacheAsyncClient(httpx.AsyncClient):
    ...
    _TOOL_UNPREFIX_PATTERN = re.compile(rf'"name"\s*:\s*"{re.escape(TOOL_PREFIX)}([^"]+)"')
    ...
    async def send(self, request: httpx.Request, *args: Any, **kwargs: Any) -> httpx.Response:  # type: ignore[override]
        ...
        try:
            if response.status_code in (400, 401, 403) and not request.extensions.get("claude_oauth_refresh_attempted"):
                is_auth_error = response.status_code in (401, 403)

                if response.status_code == 400:
                    is_auth_error = self._is_cloudflare_html_error(response)
                    if is_auth_error:
                        logger.info("Detected Cloudflare 400 error (likely auth-related), attempting token refresh")
    ...
    def _wrap_response_with_tool_unprefixing(self, response: httpx.Response, request: httpx.Request) -> httpx.Response:
    ...
    @staticmethod
    def _update_auth_headers(headers: MutableMapping[str, str], access_token: str) -> None:
