# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\config.py
import configparser
import datetime
import json
import os
...
DBOS_DATABASE_URL = os.environ.get("DBOS_SYSTEM_DATABASE_URL", f"sqlite:///{_DEFAULT_SQLITE_FILE}")
    ...
    if missing:
        ...
        for key in missing:
            if key == "puppy_name":
                ...
                val = input("What's your name (so Code Puppy knows its owner)? ").strip()
            else:
...
def set_openai_reasoning_effort(value: str) -> None:
    ...
    if normalized not in allowed_values:
        raise ValueError(f"Invalid reasoning effort '{value}'. Allowed: {', '.join(sorted(allowed_values))}")
...
def set_openai_verbosity(value: str) -> None:
    ...
    normalized = (value or "").strip().lower()
    if normalized not in allowed_values:
        raise ValueError(f"Invalid verbosity '{value}'. Allowed: {', '.join(sorted(allowed_values))}")
...
def get_model_setting(model_name: str, setting: str, default: Optional[float] = None) -> Optional[float]:
...
def clear_model_settings(model_name: str) -> None:
    ...
    if DEFAULT_SECTION in config:
        keys_to_remove = [key for key in config[DEFAULT_SECTION] if key.startswith(prefix)]
...
def normalize_command_history():
    ...
    try:
        ...
        with open(COMMAND_HISTORY_FILE, "r", encoding="utf-8", errors="surrogateescape") as f:
            content = f.read()
        ...
        if content != updated_content:
            with open(COMMAND_HISTORY_FILE, "w", encoding="utf-8", errors="surrogateescape") as f:
                f.write(updated_content)
    except Exception as e:
        from code_puppy.messaging import emit_error

        emit_error(f"An unexpected error occurred while normalizing command history: {str(e)}")
...
def initialize_command_history_file():
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(os.path.expanduser("~"), ".code_puppy_history.txt")
            ...
            emit_error(f"An unexpected error occurred while trying to initialize history file: {str(e)}")
...
def save_command_to_history(command: str):
    ...
    try:
        ...
        try:
            command = command.encode("utf-8", errors="surrogatepass").decode("utf-8", errors="replace")
        except (UnicodeEncodeError, UnicodeDecodeError):
            # If that fails, do a more aggressive cleanup
            command = "".join(char if ord(char) < 0xD800 or ord(char) > 0xDFFF else "\ufffd" for char in command)

        with open(COMMAND_HISTORY_FILE, "a", encoding="utf-8", errors="surrogateescape") as f:
            f.write(f"\n# {timestamp}\n{command}\n")
    except Exception as e:
        from code_puppy.messaging import emit_error

        emit_error(f"An unexpected error occurred while saving command history: {str(e)}")
...
def auto_save_session_if_enabled() -> bool:
    ...
    try:
        ...
        emit_info(f"🐾 Auto-saved session: {metadata.message_count} messages ({metadata.total_tokens} tokens)")
