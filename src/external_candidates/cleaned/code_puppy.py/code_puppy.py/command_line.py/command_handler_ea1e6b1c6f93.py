# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\command_handler.py
...
def handle_command(command: str):
    ...
    if command.startswith("/"):
        ...
        if name:
            emit_warning(Text.from_markup(f"Unknown command: {command}\n[dim]Type /help for options.[/dim]"))
        else:
            # Show current model ONLY here
            from code_puppy.command_line.model_picker_completion import get_active_model

            current_model = get_active_model()
            emit_info(Text.from_markup(f"[bold green]Current Model:[/bold green] [cyan]{current_model}[/cyan]"))
        return True
