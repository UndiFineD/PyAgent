...
def run_interactive_install_wizard(manager, group_id: str) -> bool:
    ...
    try:
        ...
        return interactive_configure_server(manager, selected_server, server_name, group_id, env_vars, cmd_args)
...
def interactive_server_selection(group_id: str):
    ...
    try:
        ...
        for i, server in enumerate(servers, 1):
            ...
            emit_info(f"{i:2}. {server.display_name}{indicator_str}", message_group=group_id)
            emit_info(f"    {server.description[:80]}...", message_group=group_id)

        choice = emit_prompt("Enter number (1-{}) or 'q' to quit: ".format(len(servers)))
...
def interactive_configure_server(
    ...
    try:
        ...
        if existing_server:
            override = emit_prompt(f"Server '{server_name}' already exists. Override? [y/N]: ")
        ...
        return install_server_from_catalog(manager, selected_server, server_name, env_vars, cmd_args, group_id)
...
def install_server_from_catalog(
    ...
    try:
        ...
        emit_info(
            Text.from_markup(f"[green]✓ Successfully installed server: {server_name}[/green]"),
            message_group=group_id,
        )
        emit_info(
