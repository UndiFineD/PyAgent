# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\mcp\status_command.py
"""
...
class StatusCommand(MCPCommandBase):
    ...
    def _show_detailed_server_status(self, server_id: str, server_name: str, group_id: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            state_display = format_state_indicator(
                ServerState(state) if state in [s.value for s in ServerState] else ServerState.STOPPED
            )
            ...
            try:
                ...
                if lifecycle_mgr.is_running(server_id):
                    status_lines.append("[bold]Process:[/bold] [green]✓ Active (subprocess/connection running)[/green]")
                else:
            ...
            if uptime:
                uptime_str = format_uptime(uptime.total_seconds() if hasattr(uptime, "total_seconds") else uptime)
            ...
            panel = Panel(panel_content, title=f"🔌 {server_name} Status", border_style="cyan")
            ...
            if recent_events:
                ...
                for event in recent_events[-5:]:  # Show last 5 events
                    ...
                    emit_info(f"  {time_str}: {event['message']}", message_group=group_id)

        except Exception as e:
            logger.error(f"Error getting detailed status for server '{server_name}': {e}")
