# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\diff_menu.py
...
class DiffConfiguration:
    ...
    def has_changes(self) -> bool:
        ...
        return self.current_add_color != self.original_add_color or self.current_del_color != self.original_del_color

    def next_language(self):
        """Cycle to the next language."""
        self.current_language_index = (self.current_language_index + 1) % len(SUPPORTED_LANGUAGES)
...
async def _split_panel_selector(
    ...
    def get_left_panel_text():
        ...
        try:
            ...
            lines.append(("fg:ansicyan", "↑↓ Navigate  │  Enter Confirm  │  Ctrl-C Cancel"))
...
def _get_preview_text_for_prompt_toolkit(config: DiffConfiguration) -> ANSI:
    ...
    header_parts.append("[bold cyan] LIVE PREVIEW - Syntax Highlighted Diff[/bold cyan]")
    header_parts.append("[bold]═" * 50 + "[/bold]")
    header_parts.append("")
    ...
    header_parts.append("")
    header_parts.append(f" [bold yellow]Language: {current_lang.upper()}[/bold yellow]  [dim](← → to cycle)[/dim]")
    header_parts.append("")
