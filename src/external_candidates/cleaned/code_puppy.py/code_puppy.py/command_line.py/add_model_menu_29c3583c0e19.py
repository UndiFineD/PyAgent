# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\add_model_menu.py
...
class AddModelMenu:
    ...
    def _render_model_details(self) -> List:
        ...
        if self.view_mode == "providers":
            ...
            if self._is_custom_model_selected():
                ...
                lines.append(("", "\n"))
                lines.append(("fg:ansibrightblack", f"  3. Uses {provider.name}'s API endpoint"))
                lines.append(("", "\n\n"))
    ...
    def _add_current_model(self):
        ...
        if self._is_custom_model_selected():
            ...
            self.result = "pending_custom_model"  # Signal to prompt for custom model name
            return
    ...
    def _create_custom_model_info(self, model_name: str, context_length: int = 128000) -> ModelInfo:
        ...
        return ModelInfo(
            ...
            max_output=min(16384, context_length // 4),  # Reasonable default based on context
    ...
    def _prompt_for_custom_model(self) -> Optional[tuple[str, int]]:
        ...
        emit_info("  Examples: gpt-4-turbo-preview, claude-3-opus-20240229, gemini-1.5-pro-latest\n")
    ...
    def run(self) -> bool:
        ...
        if self.result == "pending_custom_model" and self.pending_provider:
            ...
            self.pending_model = self._create_custom_model_info(model_name, context_length)
        ...
        if self.result == "pending_credentials" and self.pending_model and self.pending_provider:
            ...
            if not self.pending_model.tool_call:
                ...
                try:
                    confirm = safe_input("\n  Are you sure you want to add this model? (y/N): ").lower()
