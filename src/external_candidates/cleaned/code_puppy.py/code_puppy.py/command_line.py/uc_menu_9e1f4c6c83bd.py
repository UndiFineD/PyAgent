# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\uc_menu.py
...
def _toggle_tool_enabled(tool: UCToolInfo) -> bool:
    ...
    try:
        ...
        if count == 0:
            ...
            new_content, meta_count = re.subn(meta_pattern, f'\\1\n    "enabled": {new_enabled},', content)
            if meta_count == 0:
...
def _highlight_python_line(line: str) -> List[Tuple[str, str]]:
    ...
    for token in tokens:
        ...
        if not in_string and (token.startswith('"') or token.startswith("'")):
            ...
            if len(token) > 1 and token.endswith(string_char) and not token.endswith("\\" + string_char):
                in_string = False
            continue
...
async def interactive_uc_picker() -> Optional[str]:
    ...
    def update_source_display():
        ...
        if tool:
            source_control.text = _render_source_panel(tool, source_lines[0], source_scroll[0], source_error[0])

    # Windows for list view
    menu_window = Window(content=menu_control, wrap_lines=False, width=Dimension(weight=40))
    preview_window = Window(content=preview_control, wrap_lines=False, width=Dimension(weight=60))

    # Window for source view (full width)
    source_window = Window(content=source_control, wrap_lines=True, width=Dimension(weight=100))
    ...
    source_frame = Frame(source_window, width=Dimension(weight=100), title="Source Code")
