# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\autosave_menu.py
...
def _render_preview_panel(base_dir: Path, entry: Optional[Tuple[str, dict]]) -> List:
    ...
    lines.append(("fg:ansibrightblack", f"  Messages: {msg_count} • Tokens: {tokens:,}"))
    lines.append(("", "\n\n"))
...
async def interactive_autosave_picker() -> Optional[str]:
    ...
    def update_display():
        """Update both panels."""
        menu_control.text = _render_menu_panel(entries, current_page[0], selected_idx[0], browse_mode[0])
        ...
        if browse_mode[0] and cached_history[0] is not None:
            ...
            preview_control.text = _render_message_browser_panel(cached_history[0], message_idx[0], session_name)
        else:
            preview_control.text = _render_preview_panel(base_dir, get_current_entry())

    menu_window = Window(content=menu_control, wrap_lines=True, width=Dimension(weight=30))
    preview_window = Window(content=preview_control, wrap_lines=True, width=Dimension(weight=70))
