# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\agent_menu.py
...
async def interactive_agent_picker() -> Optional[str]:
    ...
    def update_display():
        """Update both panels."""
        menu_control.text = _render_menu_panel(entries, current_page[0], selected_idx[0], current_agent_name)
        preview_control.text = _render_preview_panel(get_current_entry(), current_agent_name)

    menu_window = Window(content=menu_control, wrap_lines=False, width=Dimension(weight=35))
    preview_window = Window(content=preview_control, wrap_lines=False, width=Dimension(weight=65))
