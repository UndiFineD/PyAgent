# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\attachments.py
...
def _detect_path_tokens(prompt: str) -> tuple[list[_DetectedPath], list[str]]:
    ...
    while index < len(tokens):
        ...
        candidate_placeholder = "".join(
            ot.replace(ESCAPE_MARKER, r"\ ") if ESCAPE_MARKER in ot else ot for ot in original_tokens_for_slice
        )
