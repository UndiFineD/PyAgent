# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\colors_menu.py
...
class ColorConfiguration:
    ...
    def next_banner(self):
        ...
        self.selected_banner_index = (self.selected_banner_index + 1) % len(self.banner_keys)
...
async def _split_panel_selector(
    ...
    def get_left_panel_text():
        ...
        try:
            ...
            lines.append(("", "\n"))
            lines.append(("fg:ansicyan", "↑↓ Navigate  │  Enter Select  │  Ctrl-C Cancel"))
...
def _get_preview_text_for_prompt_toolkit(config: ColorConfiguration) -> ANSI:
    ...
    for key in config.banner_keys:
        ...
        banner_text = f"[bold white on {color}] {display_name} [/bold white on {color}]{icon_str}"
