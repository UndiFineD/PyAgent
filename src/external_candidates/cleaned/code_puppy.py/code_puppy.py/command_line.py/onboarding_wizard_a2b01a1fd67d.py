# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\onboarding_wizard.py
...
class OnboardingWizard:
    ...
    def get_progress_indicator(self) -> str:
        """Progress dots: ● ○ ○ ○ ○"""
        return " ".join("●" if i == self.current_slide else "○" for i in range(self.TOTAL_SLIDES))
...
def _get_slide_panel_content(wizard: OnboardingWizard) -> ANSI:
    ...
    console.print(f"[dim]Slide {wizard.current_slide + 1} of {wizard.TOTAL_SLIDES}[/dim]\n")
...
async def run_onboarding_wizard() -> Optional[str]:
    ...
    try:
        ...
        slide_panel = Window(content=FormattedTextControl(lambda: _get_slide_panel_content(wizard)))
