# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\model_settings_menu.py
...
def _get_setting_choices(setting_key: str, model_name: Optional[str] = None) -> List[str]:
...
class ModelSettingsMenu:
    ...
    def _render_main_list(self) -> List:
        ...
        if self.view_mode == "models":
            ...
            if not self.supported_settings:
                lines.append(("fg:ansiyellow", "  No configurable settings for this model."))
                lines.append(("", "\n\n"))
    ...
    def _render_details_panel(self) -> List:
        ...
        if self.view_mode == "models":
            ...
            if not self.supported_settings:
                lines.append(("fg:ansibrightblack", "  This model doesn't expose any settings."))
                return lines
            ...
            if self.editing_mode:
                ...
                if self.edit_value is not None:
                    ...
                    lines.append(("fg:ansibrightblack", "    New value: (model default)"))
                lines.append(("", "\n"))
    ...
    def _start_editing(self):
        ...
        if current is not None:
            ...
            choices = _get_setting_choices(setting_key, self.selected_model)
            self.edit_value = setting_def.get("default", choices[0] if choices else None)
        elif setting_def.get("type") == "boolean":
    ...
    def _adjust_value(self, direction: int):
        ...
        if setting_def.get("type") == "choice":
            ...
            choices = _get_setting_choices(setting_key, self.selected_model)
            current_idx = choices.index(self.edit_value) if self.edit_value in choices else 0
    ...
    def run(self) -> bool:
        ...
        details_window = Window(content=self.details_control, wrap_lines=True, width=Dimension(weight=60))

        menu_frame = Frame(menu_window, width=Dimension(weight=40), title="Models")
        details_frame = Frame(details_window, width=Dimension(weight=60), title="Details")
        ...
        def _(event):
            if self.view_mode == "models":
                ...
                if not self.editing_mode and self.setting_index < len(self.supported_settings) - 1:
                    self.setting_index += 1
                    self.update_display()
