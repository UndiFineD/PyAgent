# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\http_utils.py
"""
...
def _resolve_proxy_config(verify: Union[bool, str, None] = None) -> ProxyConfig:
    ...
    disable_retry = os.environ.get("CODE_PUPPY_DISABLE_RETRY_TRANSPORT", "").lower() in ("1", "true", "yes")
    ...
    async def send(self, request: httpx.Request, **kwargs: Any) -> httpx.Response:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # Determine wait time
                wait_time = 1.0 * (2**attempt)  # Default exponential backoff: 1s, 2s, 4s...
                ...
                if attempt < self.max_retries:
                    emit_warning(f"HTTP connection error: {e}. Retrying in {wait_time}s...")
                    await asyncio.sleep(wait_time)
                else:
                    raise
            except Exception:
...
def create_reopenable_async_client(
    ...
    if ReopenableAsyncClient is not None:
        client_class = RetryingAsyncClient if not config.disable_retry else httpx.AsyncClient
        ...
        if not config.disable_retry:
            return RetryingAsyncClient(retry_status_codes=retry_status_codes, **base_kwargs)
        else:
