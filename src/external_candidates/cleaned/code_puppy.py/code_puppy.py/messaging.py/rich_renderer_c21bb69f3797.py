...
class RichConsoleRenderer:
    ...
    def _render_file_listing(self, msg: FileListingMessage) -> None:
        ...
        dir_stats: dict = defaultdict(lambda: {"files": [], "subdirs": set(), "total_size": 0})
        ...
        def render_dir_tree(dir_path: str, depth: int = 0) -> None:
            ...
            stats = dir_stats.get(dir_path, {"files": [], "subdirs": set(), "total_size": 0})
            files = stats["files"]
            ...
            if dir_path == root_key:
                ...
                for f in sorted(files, key=lambda x: x.path):
                    ...
                    size_str = f" [dim]({self._format_size(f.size)})[/dim]" if f.size > 0 else ""
                    self._console.print(f"{indent}{icon} [green]{name}[/green]{size_str}")
                ...
                parts = []
                if rec_file_count > 0:
                    parts.append(f"{rec_file_count} file{'s' if rec_file_count != 1 else ''}")
                if subdir_count > 0:
                    parts.append(f"{subdir_count} subdir{'s' if subdir_count != 1 else ''}")
                if rec_size > 0:
                    parts.append(self._format_size(rec_size))

                summary = f" [dim]({', '.join(parts)})[/dim]" if parts else ""
                self._console.print(f"{indent}📁 [bold blue]{dir_name}/[/bold blue]{summary}")
    ...
    def _render_grep_result(self, msg: GrepResultMessage) -> None:
        ...
        if not msg.matches:
            self._console.print(f"[dim]No matches found for '{msg.search_term}' in {msg.directory}[/dim]")
            return
        ...
        if msg.verbose:
            ...
            for file_path in sorted(by_file.keys()):
                file_matches = by_file[file_path]
                match_word = "match" if len(file_matches) == 1 else "matches"
                self._console.print(f"[dim]📄 {file_path} ({len(file_matches)} {match_word})[/dim]")

        # Summary - subtle
        match_word = "match" if msg.total_matches == 1 else "matches"
        file_word = "file" if len(by_file) == 1 else "files"
        num_files = len(by_file)
        self._console.print(f"[dim]Found {msg.total_matches} {match_word} across {num_files} {file_word}[/dim]")
    ...
    def _render_diff(self, msg: DiffMessage) -> None:
        ...
        self._console.print(
            f"\n{banner} {icon} [{op_color}]{msg.operation.upper()}[/{op_color}] [bold cyan]{msg.path}[/bold cyan]"
        )
    ...
    def _render_subagent_invocation(self, msg: SubAgentInvocationMessage) -> None:
        ...
        session_type = "New session" if msg.is_new_session else f"Continuing ({msg.message_count} messages)"
        banner = self._format_banner("invoke_agent", "🤖 INVOKE AGENT")
        self._console.print(f"\n{banner} [bold cyan]{msg.agent_name}[/bold cyan] [dim]({session_type})[/dim]")

        # Session ID
        self._console.print(f"[dim]Session:[/dim] [bold]{msg.session_id}[/bold]")

        # Prompt (truncated if too long, rendered as markdown)
        prompt_display = msg.prompt[:200] + "..." if len(msg.prompt) > 200 else msg.prompt
    ...
    def _render_subagent_response(self, msg: SubAgentResponseMessage) -> None:
        ...
        self._console.print(f"\n[dim]Session [bold]{msg.session_id}[/bold] saved ({msg.message_count} messages)[/dim]")
            ...
            for i, opt in enumerate(msg.options):
                if choice == opt.lower() or choice == opt[0].lower():
                    ...
                    if msg.allow_feedback:
                        feedback = self._console.input("[dim]Feedback (optional): [/dim]")
    ...
    def _render_version_check(self, msg: VersionCheckMessage) -> None:
        ...
        if msg.update_available:
            ...
            self._console.print(f"[dim]✓ You're on the latest version ({msg.current_version})[/dim]")
