    ...
    def load_config() -> Dict[str, Any]:
        ...
        if len(load_model_config_callbacks) > 0:
            if len(load_model_config_callbacks) > 1:
                logging.getLogger(__name__).warning("Multiple load_model_config callbacks registered, using the first")
        ...
        for source_path, label, use_filtered in extra_sources:
            ...
            try:
                ...
                logging.getLogger(__name__).warning(f"Failed to load {label} config from {source_path}: {exc}")
        return config
    ...
    def get_model(model_name: str, config: Dict[str, Any]) -> Any:
        ...
        # via the register_model_type callback. See plugins/claude_code_oauth/register_callbacks.py
            ...
            if not azure_endpoint_config:
                raise ValueError("Azure OpenAI model type requires 'azure_endpoint' in its configuration.")
            ...
            if not api_version_config:
                raise ValueError("Azure OpenAI model type requires 'api_version' in its configuration.")
            ...
            if not api_key_config:
                raise ValueError("Azure OpenAI model type requires 'api_key' in its configuration.")
        ...
        # via the register_model_type callback. See plugins/antigravity_oauth/register_callbacks.py
            ...
            if model_config.get("antigravity"):
                ...
                for handler_info in registered_handlers:
                    handlers = (
                        handler_info if isinstance(handler_info, list) else [handler_info] if handler_info else []
                    )
                    for handler_entry in handlers:
                        if isinstance(handler_entry, dict) and handler_entry.get("type") == "antigravity":
            ...
            url, headers, verify, api_key = get_custom_config(model_config)
            if not api_key:
                emit_warning(f"API key is not set for Cerebras endpoint; skipping model '{model_config.get('name')}'.")
                return None
            ...
            try:
                ...
                emit_warning(
                    f"Gemini OAuth plugin not available; skipping model '{model_config.get('name')}'. Error: {exc}"
                )
                return None
                    ...
                    if handler_entry.get("type") == model_type:
                        ...
                        if callable(handler):
                            try:
                                ...
                                logger.error(f"Plugin handler for model type '{model_type}' failed: {e}")
                                return None
