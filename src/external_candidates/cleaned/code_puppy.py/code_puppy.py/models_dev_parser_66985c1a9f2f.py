...
class ModelsDevRegistry:
    ...
    def filter_by_cost(
        ...
        if max_input_cost is not None:
            filtered_models = [
                m for m in filtered_models if m.cost_input is not None and m.cost_input <= max_input_cost
            ]

        if max_output_cost is not None:
            filtered_models = [
                m for m in filtered_models if m.cost_output is not None and m.cost_output <= max_output_cost
            ]

        return filtered_models

    def filter_by_context(self, models: List[ModelInfo], min_context_length: int) -> List[ModelInfo]:
        """
