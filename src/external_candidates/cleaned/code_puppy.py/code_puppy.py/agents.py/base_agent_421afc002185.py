# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\agents\base_agent.py
...
class BaseAgent(ABC):
    ...
    def estimate_context_overhead_tokens(self) -> int:
        ...
        try:
            ...
            model_name = self.get_model_name() if hasattr(self, "get_model_name") else ""
                ...
                for tool_name, tool_func in tools.items():
                    try:
                        ...
                        if schema:
                            ...
                            if annotations:
                                total_tokens += self.estimate_token_count(str(annotations))
                    except Exception:
    ...
    def summarize_messages(
        ...
        if with_protection:
            messages_to_summarize, protected_messages = self.split_messages_for_protected_summarization(messages)
        else:
        ...
        try:
            new_messages = run_summarization_sync(instructions, message_history=messages_to_summarize)

            if not isinstance(new_messages, list):
                emit_warning("Summarization agent returned non-list output; wrapping into message request")
    ...
    def message_history_processor(self, ctx: RunContext, messages: List[ModelMessage]) -> List[ModelMessage]:
        ...
        context_summary = SpinnerBase.format_context_info(total_current_tokens, model_max, proportion_used)
        ...
        if proportion_used > compaction_threshold:
            ...
            if compaction_strategy == "truncation":
                ...
                result_messages = self.truncation(self.filter_huge_messages(messages), protected_tokens)
                summarized_messages = []  # No summarization in truncation mode
            else:
                # Default to summarization (safe to proceed - no pending tool calls)
                result_messages, summarized_messages = self.summarize_messages(self.filter_huge_messages(messages))

            final_token_count = sum(self.estimate_tokens_for_message(msg) for msg in result_messages)
            # Update spinner with final token count
            final_summary = SpinnerBase.format_context_info(final_token_count, model_max, final_token_count / model_max)
    ...
    def truncation(self, messages: List[ModelMessage], protected_tokens: int) -> List[ModelMessage]:
        ...
        if len(messages) > 1:
            ...
            has_thinking = any(isinstance(part, ThinkingPart) for part in second_msg.parts)
            if has_thinking:
    ...
    def _load_model_with_fallback(
        ...
        try:
            ...
            available_str = ", ".join(sorted(available_models)) if available_models else "no configured models"
            emit_warning(
            ...
            friendly_message = (
                "No valid model could be loaded. Update the model configuration or set a valid model with `config set`."
            )
            emit_error(
                friendly_message,
                message_group=message_group,
    ...
    def reload_code_generation_agent(self, message_group: Optional[str] = None):
        ...
        prepared = prepare_prompt_for_model(model_name, instructions, "", prepend_system_to_user=False)
    ...
    def _create_agent_with_output_type(self, output_type: Type[Any]) -> PydanticAgent:
        ...
        model_name = self.get_model_name()
        models_config = ModelFactory.load_config()
        model, resolved_model_name = self._load_model_with_fallback(model_name, models_config, str(uuid.uuid4()))
    ...
    def _spawn_ctrl_x_key_listener(
        ...
        thread = threading.Thread(target=listener, name="code-puppy-key-listener", daemon=True)
        thread.start()
        return thread
    ...
    def _listen_for_ctrl_x_windows(
        ...
        while not stop_event.is_set():
            try:
                ...
                emit_warning("Windows key listener error; Ctrl+C is still available for cancel.")
                return
            time.sleep(0.05)
    ...
    async def run_with_mcp(
        ...
        **kwargs,
        ...
        if prompt:
            try:
                prompt = prompt.encode("utf-8", errors="surrogatepass").decode("utf-8", errors="replace")
            except (UnicodeEncodeError, UnicodeDecodeError):
                # Fallback: filter out surrogate characters directly
                prompt = "".join(char if ord(char) < 0xD800 or ord(char) > 0xDFFF else "\ufffd" for char in prompt)
        ...
        if is_claude_code_model(self.get_model_name()) or is_antigravity_model(self.get_model_name()):
        ...
        async def run_agent_task():
            try:
                self.set_message_history(self.prune_interrupted_tool_calls(self.get_message_history()))
