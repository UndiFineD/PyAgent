# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\agents\json_agent.py
...
class JSONAgent(BaseAgent):
    ...
    def _load_config(self) -> Dict:
        ...
        try:
            ...
            raise ValueError(f"Failed to load JSON agent config from {self.json_path}: {e}")
    ...
    def _validate_config(self) -> None:
        ...
        for field in required_fields:
            if field not in self._config:
                raise ValueError(f"Missing required field '{field}' in JSON agent config: {self.json_path}")

        # Validate tools is a list
        if not isinstance(self._config["tools"], list):
            raise ValueError(f"'tools' must be a list in JSON agent config: {self.json_path}")

        # Validate system_prompt is string or list
        system_prompt = self._config["system_prompt"]
        if not isinstance(system_prompt, (str, list)):
            raise ValueError(f"'system_prompt' must be a string or list in JSON agent config: {self.json_path}")
