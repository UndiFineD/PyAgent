# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\agents\agent_manager.py
    ...
    for _, modname, _ in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if isinstance(attr, type) and issubclass(attr, BaseAgent) and attr not in [BaseAgent, JSONAgent]:
        ...
        try:
            ...
            for _, modname, _ in pkgutil.iter_modules(subpkg.__path__):
                ...
                try:
                    ...
                    module = importlib.import_module(f"code_puppy.agents.{subpkg_name}.{modname}")
...
def _next_clone_index(base_name: str, existing_names: set[str], agents_dir: Path) -> int:
...
def clone_agent(agent_name: str) -> Optional[str]:
    ...
    try:
        if isinstance(agent_ref, str):
            ...
            clone_config["display_name"] = _build_clone_display_name(source_display_name, clone_index)

            tools = source_config.get("tools", [])
            clone_config["tools"] = _filter_available_tools(tools) if isinstance(tools, list) else []
            ...
            clone_config = {
                "name": clone_name,
                "display_name": _build_clone_display_name(agent_instance.display_name, clone_index),
