...
def _validate_session_id(session_id: str) -> None:
    ...
    if len(session_id) > SESSION_ID_MAX_LENGTH:
        raise ValueError(f"Invalid session_id '{session_id}': must be {SESSION_ID_MAX_LENGTH} characters or less")
    ...
    async def invoke_agent(
        ...
        if session_id is not None:
            try:
                ...
                return AgentInvokeOutput(response=None, agent_name=agent_name, error=str(e))
        ...
        try:
            ...
            if not (mcp_disabled and str(mcp_disabled).lower() in ("1", "true", "yes", "on")):
            ...
            emit_success(f"✓ {agent_name} completed successfully", message_group=group_id)

            return AgentInvokeOutput(response=response, agent_name=agent_name, session_id=session_id)
