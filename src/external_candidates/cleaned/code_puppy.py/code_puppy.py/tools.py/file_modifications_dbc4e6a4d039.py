# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\tools\file_modifications.py
...
def _create_rejection_response(file_path: str) -> Dict[str, Any]:
    ...
    rejection_message = "USER REJECTED: The user explicitly rejected these file changes."
    if user_feedback:
        rejection_message += f" User feedback: {user_feedback}"
    else:
        rejection_message += (
            " Please do not retry the same changes or any other changes - immediately ask for clarification."
        )
...
def _parse_diff_lines(diff_text: str) -> List[DiffLine]:
    ...
    for line in diff_text.splitlines():
        ...
        if line.startswith("+") and not line.startswith("+++"):
            ...
            if match:
                line_number = int(match.group(1)) - 1  # Will be incremented on next line
            line_type = "context"
            content = line
...
def _log_error(msg: str, exc: Exception | None = None, message_group: str | None = None) -> None:
...
def delete_snippet_from_file(
    ...
    res = _delete_snippet_from_file(context, file_path, snippet, message_group=message_group)
    diff = res.get("diff", "")
    if diff:
...
def write_to_file(
    ...
    permission_results = on_file_permission(context, path, "write", None, message_group, operation_data)

    # If any permission handler denies the operation, return cancelled result
    if permission_results and any(not result for result in permission_results if result is not None):
        return _create_rejection_response(path)

    res = _write_to_file(context, path, content, overwrite=overwrite, message_group=message_group)
    diff = res.get("diff", "")
    if diff:
...
def replace_in_file(
    ...
    permission_results = on_file_permission(context, path, "replace text in", None, message_group, operation_data)
...
def _edit_file(context: RunContext, payload: EditFilePayload, group_id: str | None = None) -> Dict[str, Any]:
    """
    ...
    try:
        if isinstance(payload, DeleteSnippetPayload):
            return delete_snippet_from_file(context, file_path, payload.delete_snippet, message_group=group_id)
        elif isinstance(payload, ReplacementsPayload):
            # Convert Pydantic Replacement models to dict format for legacy compatibility
            replacements_dict = [{"old_str": rep.old_str, "new_str": rep.new_str} for rep in payload.replacements]
            return replace_in_file(context, file_path, replacements_dict, message_group=group_id)
...
def _delete_file(context: RunContext, file_path: str, message_group: str | None = None) -> Dict[str, Any]:
    file_path = os.path.abspath(file_path)
    ...
    permission_results = on_file_permission(context, file_path, "delete", None, message_group, operation_data)
