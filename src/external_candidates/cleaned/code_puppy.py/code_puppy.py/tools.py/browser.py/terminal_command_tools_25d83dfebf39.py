# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\tools\browser\terminal_command_tools.py
        ...
        for selector in selectors_to_try:
            ...
            if element:
                ...
                if "textarea" not in selector:
                    textarea = await page.query_selector("textarea.xterm-helper-textarea")
                    if textarea:
...
async def run_terminal_command(
    ...
    emit_info(Text.from_markup(f"{banner} [dim]{command}[/dim]"), message_group=group_id)
...
def register_send_terminal_keys(agent):
    ...
    async def terminal_send_keys(
        ...
        """
        # Session is set by invoke_agent via contextvar
        return await send_terminal_keys(keys=keys, modifiers=modifiers, repeat=repeat, delay_ms=delay_ms)
