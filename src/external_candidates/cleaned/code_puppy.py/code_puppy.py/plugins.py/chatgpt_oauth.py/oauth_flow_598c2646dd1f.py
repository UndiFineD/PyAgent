# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\chatgpt_oauth\oauth_flow.py
...
class _OAuthServer(HTTPServer):
    def __init__(
        ...
        *,
        ...
        super().__init__(("localhost", REQUIRED_PORT), _CallbackHandler, bind_and_activate=True)
        self.exit_code = 1
    ...
    def exchange_code(self, code: str) -> Tuple[AuthBundle, str]:
        ...
        bundle = AuthBundle(api_key=api_key, token_data=token_data, last_refresh=last_refresh)
...
class _CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if save_tokens(tokens):
            ...
            self._send_failure(500, "Unable to persist auth file — a puppy probably chewed it.")
            self._shutdown()
        self._shutdown_after_delay(2.0)

    def do_POST(self) -> None:  # noqa: N802
        self._send_failure(404, "POST not supported — the pups only fetch GET requests.")
        self._shutdown()
...
def run_oauth_flow() -> None:
    ...
    if api_key:
        ...
        emit_info(f"Access token saved and available via {CHATGPT_OAUTH_CONFIG['api_key_env_var']}")
    else:
        emit_warning("No API key obtained. You may need to configure projects at platform.openai.com.")
    ...
    if api_key:
        ...
        if models:
            if add_models_to_extra_config(models):
                emit_success("ChatGPT models registered. Use the `chatgpt-` prefix in /model.")
