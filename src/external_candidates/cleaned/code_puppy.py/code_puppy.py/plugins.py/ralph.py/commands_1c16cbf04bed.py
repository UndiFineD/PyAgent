# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\ralph\commands.py
...
def _handle_status(args: str) -> bool:
    ...
    if not manager.prd_exists():
        ...
        emit_info("Use `/ralph prd` to create a PRD, then `/ralph convert` to generate prd.json")
        return True
...
def _handle_convert(args: str) -> MarkdownCommandResult:
    ...
    # Return a MarkdownCommandResult so it's processed as agent input
    if args:
        return MarkdownCommandResult(f"/agent ralph-converter\nPlease convert the PRD in {args} to prd.json format.")
    else:
        return MarkdownCommandResult("/agent ralph-converter\nPlease help me convert my PRD to prd.json format.")
...
def _handle_start(args: str) -> str | bool:
    ...
    if not manager.prd_exists():
        ...
        emit_info("First create a PRD with `/ralph prd` and convert it with `/ralph convert`")
        return True
