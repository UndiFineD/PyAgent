...
def _preview_write_to_file(file_path: str, content: str, overwrite: bool = False) -> str | None:
...
def _preview_replace_in_file(file_path: str, replacements: list[dict[str, str]]) -> str | None:
...
def handle_edit_file_permission(
    ...
    confirmed, user_feedback = prompt_for_file_permission(file_path, operation_desc, preview, message_group)
...
def handle_delete_file_permission(
    ...
    confirmed, user_feedback = prompt_for_file_permission(file_path, "delete", preview, message_group)
    # Store feedback in thread-local storage so the tool can access it
    _set_user_feedback(user_feedback)
    return confirmed
...
def handle_file_permission(
    ...
    if operation_data is not None:
        preview = _generate_preview_from_operation_data(file_path, operation, operation_data)

    confirmed, user_feedback = prompt_for_file_permission(file_path, operation, preview, message_group)
    # Store feedback in thread-local storage so the tool can access it
    _set_user_feedback(user_feedback)
    return confirmed


def _generate_preview_from_operation_data(file_path: str, operation: str, operation_data: Any) -> str | None:
    ...
    try:
        if operation == "delete":
            ...
            if "delete_snippet" in operation_data:
                return _preview_delete_snippet(file_path, operation_data["delete_snippet"])
            elif "replacements" in operation_data:
                return _preview_replace_in_file(file_path, operation_data["replacements"])
