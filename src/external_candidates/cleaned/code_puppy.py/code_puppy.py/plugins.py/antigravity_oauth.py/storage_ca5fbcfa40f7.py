# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\antigravity_oauth\storage.py
...
class AccountMetadata:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AccountMetadata":
        ...
        return cls(
            ...
            rate_limit_reset_times=RateLimitState.from_dict(data.get("rateLimitResetTimes")),
        )
...
def load_accounts() -> Optional[AccountStorage]:
    ...
    try:
        ...
        if storage.accounts:
            storage.active_index = max(0, min(storage.active_index, len(storage.accounts) - 1))
        else:
