# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\antigravity_oauth\utils.py
    ...
    try:
        ...
        for model_id, model_info in ANTIGRAVITY_MODELS.items():
            ...
            if model_info.get("thinking_budget"):
                models_config[prefixed_name]["thinking_budget"] = model_info["thinking_budget"]
...
def remove_antigravity_models() -> int:
    ...
    try:
        ...
        to_remove = [name for name, config in models.items() if config.get("oauth_source") == "antigravity-plugin"]
