# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\antigravity_oauth\accounts.py
...
def _is_rate_limited_for_family(account: ManagedAccount, family: ModelFamily) -> bool:
    ...
    return _is_rate_limited_for_quota_key(account, "gemini-antigravity") and _is_rate_limited_for_quota_key(
        account, "gemini-cli"
    )
...
def _clear_expired_rate_limits(account: ManagedAccount) -> None:
    ...
    now = _now_ms()
    keys_to_remove = [key for key, reset_time in account.rate_limit_reset_times.items() if now >= reset_time]
...
class AccountManager:
        ...
        if stored and stored.accounts:
            ...
            for i, acc in enumerate(stored.accounts):
                ...
                if acc.rate_limit_reset_times.gemini_antigravity:
                    rate_limits["gemini-antigravity"] = acc.rate_limit_reset_times.gemini_antigravity
            ...
            if self._accounts:
                ...
                self._current_index_by_family["claude"] = stored.active_index_by_family.get(
                    "claude", default_idx
                ) % len(self._accounts)
                self._current_index_by_family["gemini"] = stored.active_index_by_family.get(
                    "gemini", default_idx
                ) % len(self._accounts)
            return
    ...
    @classmethod
    def load_from_disk(cls, initial_refresh_token: Optional[str] = None) -> "AccountManager":
