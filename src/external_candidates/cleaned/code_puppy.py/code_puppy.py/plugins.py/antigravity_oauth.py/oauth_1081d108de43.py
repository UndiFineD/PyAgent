# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\antigravity_oauth\oauth.py
...
def fetch_antigravity_status(access_token: str) -> AntigravityStatus:
    ...
    endpoints = list(dict.fromkeys(ANTIGRAVITY_LOAD_ENDPOINTS + list(ANTIGRAVITY_ENDPOINT_FALLBACKS)))
    ...
    for base_endpoint in endpoints:
        try:
            ...
            allowed_tier_ids = [t.get("id", "") for t in allowed_tiers_data if t.get("id")]
...
def _fetch_project_id(access_token: str) -> str:
    ...
    for base_endpoint in endpoints:
        try:
            ...
            if not response.ok:
                errors.append(f"loadCodeAssist {response.status_code} at {base_endpoint}: {response.text[:200]}")
                continue
    ...
    if allowed_tiers:
        ...
        if default_tier and not default_tier.get("userDefinedCloudaicompanionProject"):
            ...
            logger.debug("No project found, attempting onboarding with tier: %s", tier_id)
            project_id = _onboard_user(access_token, tier_id)
            if project_id:
                return project_id

    if errors:
        logger.debug("Could not resolve Antigravity project (non-fatal): %s", "; ".join(errors))
