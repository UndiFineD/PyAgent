# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\claude_code_oauth\utils.py
...
def get_valid_access_token() -> Optional[str]:
    ...
    if is_token_expired(tokens):
        ...
        if not _is_token_actually_expired(tokens):
            logger.warning("Claude Code token refresh failed; using existing access token until expiry")
            return access_token
...
def exchange_code_for_tokens(auth_code: str, context: OAuthContext) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            token_data["expires_at"] = _calculate_expires_at(token_data.get("expires_in"))
            return token_data
        logger.error(
...
def remove_claude_code_models() -> int:
    try:
        claude_models = load_claude_models()
        to_remove = [
            name for name, config in claude_models.items() if config.get("oauth_source") == "claude-code-plugin"
        ]
        if not to_remove:
            return 0
