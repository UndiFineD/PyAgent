# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\claude_code_oauth\register_callbacks.py
"""
...
def _await_callback(context: OAuthContext) -> Optional[str]:
    ...
    try:
        ...
        if should_suppress_browser():
            emit_info("[HEADLESS MODE] Would normally open browser for Claude Code OAuth…")
...
def _perform_authentication() -> None:
    ...
    if not save_tokens(tokens):
        emit_error("Tokens retrieved but failed to save locally. Check file permissions.")
        return
    ...
    if not models:
        emit_warning("Claude Code authentication succeeded but no models were returned.")
        return

    emit_info(f"Discovered {len(models)} models: {', '.join(models)}")
    if add_models_to_extra_config(models):
        emit_success("Claude Code models added to your configuration. Use the `claude-code-` prefix!")
...
def _handle_custom_command(command: str, name: str) -> Optional[bool]:
    ...
    if name == "claude-code-auth":
        ...
        if tokens and tokens.get("access_token"):
            emit_warning("Existing Claude Code tokens found. Continuing will overwrite them.")
...
def _create_claude_code_model(model_name: str, model_config: Dict, config: Dict) -> Any:
    ...
    if not api_key:
        emit_warning(f"API key is not set for Claude Code endpoint; skipping model '{model_config.get('name')}'.")
        return None
