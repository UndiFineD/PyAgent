# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\mcp_\blocking_startup.py
"""
...
class StartupMonitor:
    ...
    async def wait_all_ready(self, timeout: float = 30.0) -> dict:
        ...
        tasks = [asyncio.create_task(wait_server(name, server)) for name, server in self.servers.items()]
...
async def start_servers_with_blocking(
    *servers: BlockingMCPServerStdio,
    ...
    ready_servers = [server for name, server in monitor.servers.items() if results.get(name, False)]
