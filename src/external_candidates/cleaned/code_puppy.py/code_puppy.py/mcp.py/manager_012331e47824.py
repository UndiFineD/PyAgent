...
class MCPManager:
    ...
    def sync_from_config(self) -> None:
        ...
        try:
            ...
            if synced_count > 0 or updated_count > 0:
                logger.info(f"Synced {synced_count} new and updated {updated_count} servers from mcp_servers.json")
    ...
    def get_servers_for_agent(
        ...
        for server_id, managed_server in self._managed_servers.items():
            try:
                ...
                logger.error(f"Error getting server {managed_server.config.name} for agent: {e}")
                # Record error event
                self.status_tracker.record_event(
                    server_id,
                    "agent_access_error",
    ...
    def start_server_sync(self, server_id: str) -> bool:
        ...
        try:
            ...
            task = loop.create_task(start_server_background(), name=f"start_server_{server_id}")
        ...
        try:
            ...
            try:
                ...
                if stopped:
                    ...
                    self.status_tracker.record_event(server_id, "disabled", {"message": "Server disabled"})
            except Exception as e:
    ...
    def stop_server_sync(self, server_id: str) -> bool:
        ...
        try:
            ...
            # Create the task - it will run when the event loop gets control
            task = loop.create_task(stop_server_background(), name=f"stop_server_{server_id}")
    ...
    def reload_server(self, server_id: str) -> bool:
        ...
        try:
            ...
            # Record reload event
            self.status_tracker.record_event(server_id, "reloaded", {"message": "Server configuration reloaded"})
    ...
    def remove_server(self, server_id: str) -> bool:
        ...
        if registry_removed or managed_removed:
            self.status_tracker.record_event(server_id, "removed", {"message": "Server removed"})
