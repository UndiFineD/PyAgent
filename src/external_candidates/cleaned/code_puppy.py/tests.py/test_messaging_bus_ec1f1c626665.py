# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_messaging_bus.py
"""
...
class TestMessageBusEmission:
    ...
    def test_emit_respects_existing_session_id(self):
        ...
        bus._has_active_renderer = True
        bus.set_session_context("session-123")
        message = TextMessage(level=MessageLevel.INFO, text="Test", session_id="custom-id")
        bus.emit(message)
        emitted = bus._outgoing.get_nowait()
...
class TestSessionContext:
    ...
    def test_session_context_thread_safe(self):
        ...
        threads = [threading.Thread(target=set_context, args=(f"session-{i}",)) for i in range(3)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
...
class TestUserInputRequest:
    ...
    async def test_request_input_basic(self):
        ...
        with patch("code_puppy.messaging.bus.uuid4", return_value="test-id"):
            result = await asyncio.wait_for(bus.request_input("Enter text:"), timeout=2.0)
    ...
    async def test_request_input_with_default(self):
        ...
        with patch("code_puppy.messaging.bus.uuid4", return_value="test-id"):
            result = await asyncio.wait_for(bus.request_input("Enter:", default="default-value"), timeout=2.0)
    ...
    async def test_request_input_password(self):
        ...
        with patch("code_puppy.messaging.bus.uuid4", return_value="pwd-id"):
            result = await asyncio.wait_for(bus.request_input("Password:", input_type="password"), timeout=2.0)
...
class TestThreadSafety:
    ...
    def test_concurrent_session_context(self):
        ...
        threads = [threading.Thread(target=update_context, args=(f"session-{i}",)) for i in range(5)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
