# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\integration\test_network_traffic_monitoring.py
...
class TrafficReport:
    ...
    def generate_markdown_report(self) -> str:
        ...
        sorted_domains = sorted(self.domains_contacted.items(), key=lambda x: x[1], reverse=True)
...
def test_network_traffic_on_simple_message(
    ...
    for domain, count in sorted(proxy.report.domains_contacted.items(), key=lambda x: x[1], reverse=True):
    ...
    assert len(proxy.report.domains_contacted) > 0, "Expected at least one domain to be contacted"
