# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\command_line\test_diff_menu_coverage.py
...
class TestKeyBindingHandlers:
        ...
        with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test", choices, track_change, lambda: ANSI("preview"))
    ...
    async def test_move_down_handler_cycles_forward(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test", choices, lambda x: None, lambda: ANSI("preview"))
    ...
    async def test_left_right_handlers_with_config(self):
        ...
        for binding in kb.bindings:
            if hasattr(binding, "keys") and "right" in str(binding.keys):
                ...
                assert config.current_language_index == (initial_index + 1) % len(SUPPORTED_LANGUAGES)
                mock_event.app.invalidate.assert_called()
                break
    ...
    async def test_cancel_handler_sets_none_and_exits(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test", ["Option"], lambda x: None, lambda: ANSI("preview"))
    ...
    async def test_enter_handler_with_empty_choices(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test", [], lambda x: None, lambda: ANSI("preview"))
...
class TestPanelTextGeneration:
    ...
    async def test_left_panel_text_empty_choices(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Empty Menu", [], lambda x: None, lambda: ANSI("preview"))
    ...
    async def test_right_panel_preview_error_handling(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test", ["Choice"], lambda x: None, broken_preview)
...
class TestInteractiveDiffPickerInnerFunctions:
    ...
    async def test_get_main_preview_returns_ansi(self):
        ...
        with patch("code_puppy.tools.common.format_diff_with_colors", return_value="mock diff"):
    ...
    async def test_get_preview_header_in_color_menu(self):
        ...
        with patch(
            ...
            with patch("code_puppy.tools.common.format_diff_with_colors", return_value="mock"):
                await _handle_color_menu(config, "additions")
...
class TestFormattedTextControlInvocation:
    ...
    async def test_invoke_panel_text_functions_directly(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Window", side_effect=capture_window):
            with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
                mock_instance = MagicMock()
                mock_instance.run_async = AsyncMock()
                mock_app.return_value = mock_instance
...
class TestInnerFunctionExecution:
    ...
    async def test_result_none_raises_keyboard_interrupt(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
            ...
            with patch("sys.stdout.write"):
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test", ["Choice"], lambda x: None, lambda: ANSI("preview"))
