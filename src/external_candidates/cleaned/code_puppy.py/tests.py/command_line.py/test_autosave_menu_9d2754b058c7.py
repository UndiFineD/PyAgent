...
class TestInteractiveAutosavePicker:
    ...
    @patch("code_puppy.command_line.autosave_menu._get_session_entries")
    @patch("sys.stdout.write")
    async def test_handles_keyboard_interrupt(self, mock_stdout, mock_entries, mock_awaiting):
    ...
    @patch("code_puppy.command_line.autosave_menu._render_preview_panel")
    @patch("sys.stdout.write")
    async def test_navigation_key_bindings(self, mock_stdout, mock_preview, mock_menu, mock_entries, mock_awaiting):
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_with_nonexistent_autosave_dir(self):
        ...
        with patch("code_puppy.command_line.autosave_menu.AUTOSAVE_DIR", "/nonexistent/path"):
            with patch(
...
class TestExtractMessageContent:
    ...
    def test_mixed_parts_in_request_returns_user(self):
        ...
        msg = MockModelMessage(
            ...
            parts=[
                ...
                MockMessagePart(part_kind="tool-return", tool_name="grep", content="results"),
            ],
        )
        role, content = _extract_message_content(msg)
    ...
    def test_tool_call_truncates_long_args(self):
        ...
        msg = MockModelMessage(
            kind="response",
            parts=[MockMessagePart(part_kind="tool-call", tool_name="edit_file", args=long_args)],
        )
        role, content = _extract_message_content(msg)
        assert "..." in content
    ...
    def test_tool_call_without_args(self):
        ...
        msg = MockModelMessage(
            kind="response",
            parts=[MockMessagePart(part_kind="tool-call", tool_name="list_files", args={})],
        )
        role, content = _extract_message_content(msg)
...
class TestRenderMessageBrowserPanel:
    ...
    def test_tool_role_shows_tool_icon(self):
        ...
        msg = MockModelMessage(
            kind="request",
            parts=[MockMessagePart(part_kind="tool-return", tool_name="test", content="result")],
        )
        result = _render_message_browser_panel([msg], 0, "test")
        lines_str = str(result)
