# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\command_line\test_core_commands_extended.py
            ...
            with patch("os.path.expanduser", side_effect=lambda x: x):
                with patch("os.path.isabs", return_value=True):
                    with patch("os.path.isdir", return_value=False):
                        ...
                        assert result is True
                        mock_error.assert_called_once_with("Not a directory: /nonexistent/dir")
...
class TestHandleToolsCommand:
    ...
    def test_tools_command_with_markdown_rendering(self):
        ...
        mock_tools_content = "# Available Tools\n\n- Tool 1: Description\n- Tool 2: Description"

        with patch("code_puppy.command_line.core_commands.tools_content", mock_tools_content):
            with patch("code_puppy.messaging.emit_info") as mock_emit:
    ...
    def test_tools_command_with_unicode_content(self):
        ...
        with patch("code_puppy.command_line.core_commands.tools_content", unicode_content):
            with patch("code_puppy.messaging.emit_info") as mock_emit:
                result = handle_tools_command("/tools")
                assert result is True
                mock_emit.assert_called_once()
...
class TestHandleExitCommand:
    ...
    def test_exit_command_with_emit_error(self):
        ...
        with patch("code_puppy.messaging.emit_success", side_effect=Exception("Emit failed")):
            # Should not raise an exception
            result = handle_exit_command("/exit")
            assert result is True
...
class TestHandleAgentCommand:
        ...
        with patch("code_puppy.agents.get_available_agents", return_value=mock_agents):
            with patch("code_puppy.messaging.emit_error") as mock_error:
                with patch("code_puppy.messaging.emit_warning") as mock_warning:
                    ...
                    mock_error.assert_called_with("Agent 'invalid_agent' not found", message_group=ANY)
                    mock_warning.assert_called()
    ...
    def test_agent_command_switch_failure_handling(self):
        ...
        with patch("code_puppy.agents.get_available_agents", return_value={"target": "Target"}):
            with patch("code_puppy.agents.get_current_agent", return_value=mock_current):
                with patch("code_puppy.agents.set_current_agent", return_value=False):  # Switch fails
                    with patch(
                        "code_puppy.config.finalize_autosave_session",
                        ...
                        with patch("code_puppy.messaging.emit_warning") as mock_warning:
                            ...
                            assert any("switch failed" in msg.lower() for msg in warning_calls)
...
class TestHandleModelCommand:
    ...
    def test_model_command_interactive_success(self):
        ...
        with patch(
            ...
            with patch("code_puppy.command_line.model_picker_completion.set_active_model") as mock_set:
                with patch(
                    ...
                    with patch("code_puppy.messaging.emit_success") as mock_success:
                        ...
                        mock_success.assert_called_with("Active model set and loaded: gpt-4")
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("code_puppy.messaging.emit_success") as mock_success:
                    ...
                    assert result is True
                    mock_success.assert_called_with("Active model set and loaded: synthetic-GLM-4.7")
...
class TestInteractiveModelPicker:
    ...
    @patch("time.sleep")
    async def test_model_picker_displays_panel(self, mock_sleep, mock_stderr_flush, mock_stdout_flush):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("code_puppy.tools.common.arrow_select_async", return_value="gpt-3.5"):
    ...
    @patch("sys.stderr.flush")
    @patch("time.sleep")
    async def test_model_picker_with_current_indicator(self, mock_sleep, mock_stderr_flush, mock_stdout_flush):
        ...
        def capture_selector(prompt, choices):
            ...
            return choices[1]  # Return the gpt-4 choice (which should be marked current)
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    current_choice = next(
                        (choice for choice in captured_choices if "(current)" in choice),
                        None,
                    )
    ...
    @patch("sys.stderr.flush")
    @patch("time.sleep")
    async def test_model_picker_keyboard_interrupt(self, mock_sleep, mock_stderr_flush, mock_stdout_flush):
...
class TestHandleMcpCommand:
    ...
    def test_mcp_command_delegates_to_handler(self):
        ...
        with patch("code_puppy.command_line.mcp.MCPCommandHandler") as mock_handler_class:
...
class TestHandleGeneratePrDescriptionCommand:
    ...
    def test_pr_description_with_directory_context(self):
        ...
        result = handle_generate_pr_description_command("/generate-pr-description @src/components")
    ...
    def test_pr_description_with_multiple_at_tokens(self):
        ...
        result = handle_generate_pr_description_command("/generate-pr-description @first @second @third")
    ...
    def test_pr_description_with_special_characters(self):
        ...
        result = handle_generate_pr_description_command("/generate-pr-description @path-with_spaces & symbols")
...
class TestEdgeCasesAndErrorHandling:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch("code_puppy.messaging.emit_success") as mock_success:
                        ...
                        assert result is True
                        mock_success.assert_called_with("Active model set and loaded: 世界-model")
    ...
    def test_tools_command_with_malformed_markdown(self):
        ...
        with patch("code_puppy.command_line.core_commands.tools_content", malformed_content):
            with patch("code_puppy.messaging.emit_info") as mock_info:
                result = handle_tools_command("/tools")
                assert result is True
    ...
    def test_async_functions_exception_safety(self):
        ...
        with patch("code_puppy.command_line.core_commands.interactive_agent_picker") as mock_picker:
    ...
    def test_concurrent_futures_timeout_handling(self):
        ...
        with patch("concurrent.futures.ThreadPoolExecutor") as mock_executor_class:
            ...
            # Simulate timeout
            mock_future = MagicMock()
            mock_future.result.side_effect = concurrent.futures.TimeoutError("Operation timed out")
            mock_executor.submit.return_value = mock_future
                    ...
                    with patch(
                        ...
                        with patch("code_puppy.messaging.emit_success") as mock_success:
                            with patch("code_puppy.messaging.emit_info") as mock_info:
                                ...
                                assert any("New agent description" in call for call in info_calls)
            ...
            with patch("code_puppy.command_line.model_picker_completion.set_active_model") as mock_set:
                with patch(
                    ...
                    with patch("code_puppy.messaging.emit_success") as mock_success:
                        ...
                        mock_success.assert_called_with("Active model set and loaded: claude-3-opus")
