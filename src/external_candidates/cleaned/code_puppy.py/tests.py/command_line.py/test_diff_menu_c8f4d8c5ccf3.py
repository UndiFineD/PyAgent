# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\command_line\test_diff_menu.py
...
class TestDiffConfiguration:
    ...
    def test_language_cycling(self):
        ...
        assert config.current_language_index == (original_index + 1) % len(SUPPORTED_LANGUAGES)
...
class TestPreviewTextGeneration:
    ...
    @patch("code_puppy.config.get_diff_addition_color")
    @patch("code_puppy.config.get_diff_deletion_color")
    def test_preview_contains_headers(self, mock_get_del, mock_get_add, mock_set_del, mock_set_add, mock_format):
...
class TestSplitPanelSelector:
    ...
    async def test_left_panel_text_generation(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
            ...
            with patch("code_puppy.command_line.diff_menu.Application", side_effect=capture_app):
                # Will raise KeyboardInterrupt when result[0] is None (user cancel)
                with pytest.raises(KeyboardInterrupt):
                    await _split_panel_selector("Test Title", choices, lambda x: None, lambda: "Preview")
    ...
    async def test_right_panel_text_handling(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
            ...
            with pytest.raises(KeyboardInterrupt):
                await _split_panel_selector("Test", ["Option"], lambda x: None, valid_preview)

            # Should raise KeyboardInterrupt for error preview too
            with pytest.raises(KeyboardInterrupt):
                await _split_panel_selector("Test", ["Option"], lambda x: None, error_preview)
...
class TestInteractiveDiffPicker:
    ...
    @patch("sys.stdout.write")
    @patch("time.sleep")
    async def test_complete_flow_with_changes(self, mock_sleep, mock_stdout, mock_awaiting, mock_selector):
    ...
    @patch("sys.stdout.write")
    @patch("time.sleep")
    async def test_flow_without_changes(self, mock_sleep, mock_stdout, mock_awaiting, mock_selector):
    ...
    @patch("sys.stdout.write")
    @patch("time.sleep")
    async def test_keyboard_interrupt_handling(self, mock_sleep, mock_stdout, mock_awaiting, mock_selector):
    ...
    @patch("sys.stdout.write")
    @patch("time.sleep")
    async def test_unexpected_error_handling(self, mock_sleep, mock_stdout, mock_awaiting, mock_selector):
    ...
    @patch("sys.stdout.write")
    @patch("time.sleep")
    async def test_console_buffer_management(self, mock_sleep, mock_stdout, mock_awaiting):
...
class TestEdgeCasesAndErrorHandling:
    ...
    async def test_empty_choices_list(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
            ...
            with pytest.raises(KeyboardInterrupt):
                await _split_panel_selector("Empty Test", [], lambda x: None, lambda: "Empty Preview")
    ...
    async def test_unicode_in_choices_and_titles(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
            ...
            with pytest.raises(KeyboardInterrupt):
                await _split_panel_selector(title, choices, lambda x: None, lambda: "Unicode Preview")
    ...
    async def test_very_long_choices_and_titles(self):
        ...
        with patch("code_puppy.command_line.diff_menu.Application") as mock_app:
            ...
            with pytest.raises(KeyboardInterrupt):
                await _split_panel_selector(long_title, long_choices, lambda x: None, lambda: "Long Preview")
...
class TestIntegrationScenarios:
    ...
    @patch("code_puppy.config.get_diff_addition_color")
    @patch("code_puppy.config.get_diff_deletion_color")
    def test_full_preview_pipeline(self, mock_get_del, mock_get_add, mock_set_del, mock_set_add, mock_format):
        ...
        mock_format.return_value = "--- a/test.py\n+++ b/test.py\n@@ -1,1 +1,1 @@\n-old\n+new"
    ...
    async def test_complete_interactive_workflow(self):
        ...
        with patch("code_puppy.command_line.diff_menu._split_panel_selector") as mock_selector:
    ...
    def test_all_language_samples_render_correctly(self):
        ...
        for lang_index in range(len(SUPPORTED_LANGUAGES)):
            ...
            try:
                ...
                pytest.fail(f"Language {SUPPORTED_LANGUAGES[lang_index]} failed to render: {e}")
