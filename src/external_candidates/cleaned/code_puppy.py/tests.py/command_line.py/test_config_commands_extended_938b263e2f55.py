# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\command_line\test_config_commands_extended.py
...
class TestReasoningCommand:
        ...
        for effort in valid_efforts:
            with patch("code_puppy.config.set_openai_reasoning_effort") as mock_set:
                with patch(
                    ...
                    with patch("code_puppy.agents.agent_manager.get_current_agent") as mock_get_agent:
    ...
    def test_reasoning_command_invalid_effort(self):
        ...
        for effort in invalid_efforts:
            expected_error = f"Invalid reasoning effort '{effort}'. Allowed: high, low, medium"
            with patch(
    ...
    def test_reasoning_command_no_arguments(self):
        ...
        with patch("code_puppy.messaging.emit_warning") as mock_warning:
            ...
            mock_warning.assert_called_once_with("Usage: /reasoning <minimal|low|medium|high|xhigh>")
    ...
    def test_reasoning_command_agent_reload_failure(self):
        ...
        with patch("code_puppy.config.set_openai_reasoning_effort"):
            with patch("code_puppy.config.get_openai_reasoning_effort", return_value="medium"):
                with patch("code_puppy.agents.agent_manager.get_current_agent") as mock_get_agent:
                    mock_agent = MagicMock()
...
class TestSetCommand:
    ...
    def test_set_command_configuration_failure(self):
        ...
        with patch("code_puppy.config.set_config_value", side_effect=Exception("Set failed")):
...
class TestPinModelCommand:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch("code_puppy.messaging.emit_success"):
                            result = handle_pin_model_command("/pin_model test_agent gpt-4")
                            assert result is True
    ...
    def test_pin_model_builtin_agent_success(self):
        ...
        with patch(
            ...
            with patch("code_puppy.agents.json_agent.discover_json_agents", return_value={}):
                with patch(
                    "code_puppy.agents.agent_manager.get_agent_descriptions",
        ...
        with patch(
            ...
            with patch("code_puppy.agents.json_agent.discover_json_agents", return_value={}):
                with patch(
                    ...
                    with patch("code_puppy.messaging.emit_error") as mock_error:
                        result = handle_pin_model_command("/pin_model unknown_agent model")
                        assert result is True
        ...
        with patch(
            ...
            with patch("code_puppy.agents.json_agent.discover_json_agents", return_value={}):
                with patch(
                    ...
                    with patch("code_puppy.messaging.emit_error") as mock_error:
                        result = handle_pin_model_command("/pin_model test_agent unavailable_model")
                        assert result is True
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch("code_puppy.messaging.emit_error") as mock_error:
                            result = handle_pin_model_command("/pin_model test_agent model")
                            assert result is True
            ...
            with patch(
                ...
                with patch("code_puppy.messaging.emit_warning") as mock_warning:
                    with patch("code_puppy.messaging.emit_info") as mock_info:
                        ...
                        mock_warning.assert_called_once_with("Usage: /unpin <agent-name>")
        ...
        with patch("code_puppy.agents.json_agent.discover_json_agents", return_value={}):
            with patch(
                ...
                with patch("code_puppy.messaging.emit_error") as mock_error:
                    ...
                    mock_error.assert_called_once_with("Agent 'invalid_agent' not found")
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("code_puppy.messaging.emit_error") as mock_error:
                    ...
                    mock_error.assert_called_once_with("Failed to apply diff settings: Set failed")
...
class TestShowColorOptions:
    ...
    def test_show_addition_color_options(self):
        ...
        with patch("code_puppy.messaging.emit_info") as mock_emit:
            ...
            usage_call = [call[0][0] for call in mock_emit.call_args_list if "Usage:" in call[0][0]][0]
