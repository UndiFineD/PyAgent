...
class TestGetCustomConfig:
    ...
    def test_get_custom_config_env_var_in_header(self):
        ...
        with patch("code_puppy.model_factory.get_api_key", return_value="resolved-token"):
    ...
    def test_get_custom_config_inline_env_vars_with_spaces(self):
        ...
        with patch("code_puppy.model_factory.get_api_key", side_effect=mock_get_api_key):
    ...
    def test_get_custom_config_api_key_from_env(self):
        ...
        with patch("code_puppy.model_factory.get_api_key", return_value="resolved-api-key"):
            url, headers, verify, api_key = get_custom_config(config)
...
class TestLoadConfigExtended:
    ...
    def test_load_config_filtered_claude_models(self):
        ...
        filtered_claude_config = {"claude-oauth": {"type": "claude_code", "name": "claude-3-opus"}}
            ...
            with patch(
                ...
                MagicMock(
                    return_value=MagicMock(
                        __enter__=MagicMock(
                            return_value=MagicMock(
                                read=MagicMock(return_value='{"base-model": {"type": "openai", "name": "gpt-4"}}')
                            )
                        ),
    ...
    def test_load_config_filtered_loading_import_error(self):
        ...
        with patch("code_puppy.model_factory.callbacks.get_callbacks", return_value=[]):
            with patch("builtins.open", MagicMock()) as mock_open:
                mock_open.return_value.__enter__.return_value.read.return_value = '{"base-model": {"type": "openai"}}'
...
class TestClaudeCodeModel:
    ...
    def test_claude_code_model_basic(self):
        ...
        mock_handler_return = [{"type": "claude_code", "handler": _create_claude_code_model}]
        with patch(
    ...
    def test_claude_code_model_interleaved_thinking_header(self):
        ...
        with patch(
            ...
            with patch("code_puppy.http_utils.get_cert_bundle_path", return_value=None):
                with patch("code_puppy.http_utils.get_http2", return_value=True):
                    with patch("code_puppy.claude_cache_client.ClaudeCacheAsyncClient") as mock_client:
                        with patch("anthropic.AsyncAnthropic"):
                            with patch("code_puppy.claude_cache_client.patch_anthropic_client_messages"):
                                with patch("pydantic_ai.providers.anthropic.AnthropicProvider"):
                                    with patch("pydantic_ai.models.anthropic.AnthropicModel"):
                                        with patch(
                                            ...
                                            assert "interleaved-thinking-2025-05-14" in headers.get(
                                                "anthropic-beta", ""
                                            )
        ...
        config = {
            "claude-code-test": {
                ...
                "custom_endpoint": {
                    "url": "https://api.anthropic.com",
                    "api_key": "test-key",
                    "headers": {"anthropic-beta": "interleaved-thinking-2025-05-14,other-feature"},
                },
            }
        }
                                    ...
                                    with patch("pydantic_ai.providers.anthropic.AnthropicProvider"):
                                        with patch("pydantic_ai.models.anthropic.AnthropicModel"):
                                            with patch(
                                                ...
                                                ModelFactory.get_model("claude-oauth", config)
                                                # Token should be refreshed
                                                call_args = mock_anthropic.call_args
                                                assert call_args[1]["auth_token"] == "new-refreshed-token"
    ...
    def test_claude_code_missing_api_key(self):
        ...
        with patch(
            ...
            with patch("code_puppy.plugins.claude_code_oauth.register_callbacks.emit_warning") as mock_warn:
...
class TestCustomAnthropicModel:
            ...
            with patch("code_puppy.model_factory.get_http2", return_value=True):
                with patch("code_puppy.model_factory.ClaudeCacheAsyncClient"):
                    with patch("code_puppy.model_factory.AsyncAnthropic"):
                        with patch("code_puppy.model_factory.patch_anthropic_client_messages"):
                            with patch("code_puppy.model_factory.AnthropicProvider"):
                                with patch("code_puppy.model_factory.AnthropicModel") as mock_model:
                                    with patch(
                                        "code_puppy.config.get_effective_model_settings",
                                        return_value={},
                                    ):
    ...
    def test_custom_anthropic_interleaved_thinking(self):
        ...
        with patch("code_puppy.model_factory.get_cert_bundle_path", return_value=None):
            with patch("code_puppy.model_factory.get_http2", return_value=True):
                with patch("code_puppy.model_factory.ClaudeCacheAsyncClient"):
                    with patch("code_puppy.model_factory.AsyncAnthropic") as mock_anthropic:
                        with patch("code_puppy.model_factory.patch_anthropic_client_messages"):
                            with patch("code_puppy.model_factory.AnthropicProvider"):
                                with patch("code_puppy.model_factory.AnthropicModel"):
                                    with patch(
                                        ...
                                        assert "anthropic-beta" in headers or headers is None
...
class TestCerebrasModel:
    ...
    def test_cerebras_model_basic(self):
        ...
        with patch("code_puppy.model_factory.create_async_client") as mock_create_client:
            with patch("code_puppy.model_factory.CerebrasProvider"):
                with patch("code_puppy.model_factory.OpenAIChatModel") as mock_model:
                    ...
                    headers = call_args[1]["headers"]
                    assert headers.get("X-Cerebras-3rd-Party-Integration") == "code-puppy"
    ...
    def test_openai_codex_uses_responses_model(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            with patch("code_puppy.model_factory.OpenAIProvider"):
                with patch("code_puppy.model_factory.OpenAIResponsesModel") as mock_responses:
...
class TestChatGPTOAuthErrorPaths:
    ...
    def test_chatgpt_oauth_plugin_not_available(self):
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="Unsupported model type: chatgpt_oauth"):
    ...
    def test_chatgpt_oauth_missing_token(self):
        ...
        mock_handlers = [{"type": "chatgpt_oauth", "handler": _create_chatgpt_oauth_model}]
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("code_puppy.plugins.chatgpt_oauth.register_callbacks.emit_warning") as mock_warn:
