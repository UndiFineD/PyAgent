# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_summarization_agent.py
...
class TestSummarizationAgent:
    ...
    def test_reload_summarization_agent_basic(self, mock_model, mock_models_config):
        ...
        with (
            patch("code_puppy.summarization_agent.ModelFactory.load_config") as mock_load_config,
            patch("code_puppy.summarization_agent.ModelFactory.get_model") as mock_get_model,
            patch("code_puppy.summarization_agent.get_global_model_name") as mock_get_name,
            ...
            assert mock_load_config.call_count >= 1  # May be called multiple times due to imports
    ...
    @pytest.mark.skip(reason="DBOSAgent import issue - module doesn't have DBOSAgent attribute")
    ...
    def test_reload_summarization_agent_instructions(self, mock_model, mock_models_config):
    ...
    def test_get_summarization_agent_force_reload(self, mock_model, mock_models_config):
        ...
        with patch("code_puppy.summarization_agent.reload_summarization_agent") as mock_reload:
...
class TestRunSummarizationSync:
    ...
    def test_run_summarization_sync_no_event_loop(self, mock_sync_result):
        ...
        with (
            patch("code_puppy.summarization_agent.get_summarization_agent") as mock_get_agent,
    ...
    def test_run_summarization_sync_with_event_loop(self, mock_sync_result):
        ...
        with (
            patch("code_puppy.summarization_agent.get_summarization_agent") as mock_get_agent,
            patch("code_puppy.summarization_agent.asyncio.get_running_loop") as mock_get_loop,
    ...
    def test_summarization_instructions_completeness(self):
        ...
        with (
            ...
            for phrase in required_phrases:
                assert phrase.lower() in instructions.lower(), f"Missing phrase: {phrase}"
...
class TestSummarizationAgentIntegration:
    ...
    def test_concurrent_summarization_requests(self):
        ...
        threads = [threading.Thread(target=summarization_worker, args=(i,)) for i in range(5)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
    ...
    def test_token_aware_summarization(self):
        ...
        with patch("tests.test_summarization_agent.run_summarization_sync") as mock_run:
            expected_summary = ["Token-efficient summary preserving all key information while reducing context."]
            mock_run.return_value = expected_summary
    ...
    def test_error_recovery_summarization(self):
        ...
        with patch("tests.test_summarization_agent.run_summarization_sync") as mock_run:
            ...
            result = run_summarization_sync("Summarize despite data issues", partial_history)
