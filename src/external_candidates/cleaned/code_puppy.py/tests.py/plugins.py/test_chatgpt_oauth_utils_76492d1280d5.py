# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_chatgpt_oauth_utils.py
...
class TestPrepareOAuthContext:
    ...
    def test_prepare_oauth_context_structure(self):
        ...
        assert context.expires_at > context.created_at
        assert context.expires_at - context.created_at == pytest.approx(240, rel=1e-2)  # 4 minutes
...
class TestParseJwtClaims:
    ...
    def test_parse_jwt_valid_token(self):
        ...
        payload = base64.urlsafe_b64encode(json.dumps({"sub": "123", "name": "test"}).encode()).decode().rstrip("=")
...
class TestTokenStorage:
    ...
    @patch("code_puppy.plugins.chatgpt_oauth.utils.get_token_storage_path")
    def test_save_stored_tokens_serialization_error(self, mock_get_path, temp_token_file):
...
class TestAddModelsToConfig:
    ...
    def test_add_models_to_extra_config_success(self, mock_load, mock_save):
        ...
        assert gpt4_config["custom_endpoint"]["url"] == CHATGPT_OAUTH_CONFIG["api_base_url"]
        # No api_key in custom_endpoint for chatgpt_oauth type
        assert "api_key" not in gpt4_config["custom_endpoint"]
        assert gpt4_config["context_length"] == CHATGPT_OAUTH_CONFIG["default_context_length"]
...
class TestErrorHandling:
    ...
    def test_fetch_chatgpt_models_various_http_errors(self, mock_get):
        ...
        for status_code, error_text in test_cases:
            ...
            assert result == DEFAULT_CODEX_MODELS, f"Should return default models for {status_code} error"
