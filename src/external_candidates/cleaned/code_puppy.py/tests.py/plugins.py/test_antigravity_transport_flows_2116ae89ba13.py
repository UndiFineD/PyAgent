# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_transport_flows.py
...
async def test_send_wraps_request_and_sets_headers(
    ...
    with patch.object(httpx.AsyncClient, "send", AsyncMock(return_value=response)) as mock_send:
...
async def test_send_retries_rate_limit_with_delay(
    ...
    rate_limited = httpx.Response(
        status_code=429,
        content=(
            b'{"error": {"details": [{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "0.5s"}]}}'
        ),
    )
