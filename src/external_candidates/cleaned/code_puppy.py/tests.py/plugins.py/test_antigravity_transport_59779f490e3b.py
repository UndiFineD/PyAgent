# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_transport.py
...
class TestUnwrappedResponse:
    ...
    def test_unwrapped_response_invalid_json(self):
        ...
        original = httpx.Response(status_code=200, content=b"not json", headers={"content-type": "text/plain"})
        original.read()
...
class TestAntigravityClientWrapRequest:
    ...
    def test_wrap_request_basic_structure(self):
        ...
        new_content, new_path, new_query, is_claude_thinking = self.client._wrap_request(content, url)
    ...
    def test_wrap_request_tools_transformation(self):
        ...
        assert "parameters_json_schema" not in wrapped["request"]["tools"][0]["functionDeclarations"][0]
...
class TestAntigravityClientSend:
    ...
    async def test_send_successful_request(self):
        ...
        with patch.object(httpx.AsyncClient, "send", AsyncMock(return_value=mock_response)):
    ...
    async def test_send_claude_thinking_headers(self):
        ...
        with patch.object(httpx.AsyncClient, "send", AsyncMock(return_value=mock_response)) as mock_send:
            ...
            assert "interleaved-thinking-2025-05-14" in sent_request.headers["anthropic-beta"]
...
class TestCreateAntigravityClient:
    ...
    def test_create_client_base_url(self):
        ...
        client = create_antigravity_client(access_token="test-token", base_url=custom_url)
