# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_chatgpt_oauth_flow.py
...
class TestOAuthServer:
    ...
    def test_exchange_code_success(self, mock_post, mock_context, mock_tokens_data):
        ...
        # Mock JWT parsing
        with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.parse_jwt_claims") as mock_parse:
...
class TestCallbackHandler:
    ...
    def callback_handler(self, mock_server):
        ...
        mock_request.rfile.readline = Mock(return_value=b"")  # Return empty bytes with length 0
        with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow._CallbackHandler.handle_one_request"):
            handler = _CallbackHandler(mock_request, ("localhost", 1455), mock_server)
            handler.server = mock_server
            handler.requestline = "GET / HTTP/1.1"  # Add missing requestline for log_request
            return handler
    ...
    def test_do_get_success_endpoint(self, callback_handler):
        ...
        with patch.object(callback_handler, "_send_html") as mock_send:
            with patch.object(callback_handler, "_shutdown_after_delay") as mock_shutdown:
    ...
    def test_do_get_invalid_path(self, callback_handler):
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(404, "Callback endpoint not found for the puppy parade.")
    ...
    def test_do_get_missing_code(self, callback_handler):
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(400, "Missing auth code — the token treat rolled away.")
                mock_shutdown.assert_called_once()

    def test_do_get_code_exchange_failure(self, callback_handler):
        """Test callback handler handles code exchange failure."""
        callback_handler.server.exchange_code.side_effect = Exception("Token exchange failed")
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(500, "Token exchange failed: Token exchange failed")
                mock_shutdown.assert_called_once()
    ...
    def test_do_get_token_save_failure(self, mock_save_tokens, callback_handler):
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(500, "Unable to persist auth file — a puppy probably chewed it.")
                mock_shutdown.assert_called_once()
    ...
    def test_do_post_not_supported(self, callback_handler):
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(404, "POST not supported — the pups only fetch GET requests.")
                mock_shutdown.assert_called_once()
    ...
    def test_log_message_verbose_mode(self, callback_handler):
        ...
        with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.BaseHTTPRequestHandler.log_message") as mock_log:
    ...
    def test_send_html(self, callback_handler):
        ...
        callback_handler.send_header.assert_any_call("Content-Type", "text/html; charset=utf-8")
        callback_handler.send_header.assert_any_call("Content-Length", str(len(test_html.encode("utf-8"))))
...
class TestRunOAuthFlow:
    ...
    def test_existing_tokens_warning(self, mock_info, mock_warning, mock_server_class, mock_load_tokens):
        ...
        mock_load_tokens.side_effect = [
            {"access_token": "existing_token"},  # Initial check - should trigger warning
    ...
    @patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.emit_info")
    def test_server_start_error(self, mock_info, mock_error, mock_server_class, mock_load_tokens):
    ...
    def test_successful_oauth_flow(
        ...
        with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens") as mock_reload:
            mock_reload.return_value = mock_tokens

            with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.add_models_to_extra_config") as mock_add:
        ...
        success_info_calls = [call for call in info_calls if "Successfully obtained" in call]
    ...
    def test_authentication_timeout(self, mock_info, mock_error, mock_server_class, mock_load_tokens):
    ...
    @patch("code_puppy.messaging.emit_error")
    @patch("code_puppy.messaging.emit_info")
    def test_tokens_cannot_be_loaded_after_success(self, mock_info, mock_error, mock_server_class, mock_load_tokens):
    ...
    @patch("code_puppy.messaging.emit_info")
    def test_no_api_key_obtained(self, mock_info, mock_warning, mock_server_class, mock_load_tokens):
    ...
    def test_browser_auto_open(
        ...
        if any("HEADLESS MODE" in str(call) for call in mock_info.call_args_list):
            mock_info.assert_any_call("[HEADLESS MODE] Would normally open: http://test.auth.url")
        else:
