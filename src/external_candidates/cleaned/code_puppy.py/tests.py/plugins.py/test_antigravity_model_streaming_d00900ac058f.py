# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_model_streaming.py
...
class TestStreamingMethod:
    ...
    @pytest.mark.asyncio
    async def test_streaming_success(self, mock_google_model, mock_httpx_client, model_request_params) -> None:
        ...
        with patch.object(mock_google_model, "prepare_request") as mock_prepare:
            ...
            with patch.object(mock_google_model, "_map_user_prompt") as mock_map:
                ...
                async with mock_google_model.request_stream(messages, settings, model_request_params) as stream:
                    chunks = []
    ...
    async def test_streaming_corrupted_signature_retry(
        ...
        with patch.object(mock_google_model, "prepare_request") as mock_prepare:
            ...
            with patch.object(mock_google_model, "_map_user_prompt") as mock_map:
                mock_map.return_value = [{"text": "Test"}]
                with patch("code_puppy.plugins.antigravity_oauth.antigravity_model._backfill_thought_signatures"):
                    async with mock_google_model.request_stream(messages, settings, model_request_params) as stream:
                        chunks = []
                        async for chunk in stream._get_event_iterator():
                            chunks.append(chunk)
