# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_oauth.py
...
class TestStateHandling:
    ...
    def test_decode_state_missing_verifier(self):
        ...
        invalid_state = base64.urlsafe_b64encode(json.dumps(payload).encode("utf-8")).decode("utf-8").rstrip("=")
...
class TestTokenExchange:
    ...
    def test_exchange_code_for_tokens_success(self, mock_requests_post, mock_requests_get):
        ...
        with patch("code_puppy.plugins.antigravity_oauth.oauth._fetch_project_id") as mock_fetch_project:
    ...
    def test_exchange_code_for_tokens_with_state_project_id(self, mock_requests_post, mock_requests_get):
    ...
    def test_exchange_code_for_tokens_invalid_state(self, mock_requests_post):
        ...
        result = exchange_code_for_tokens("auth_code_123", "invalid_state", context.redirect_uri)

        assert isinstance(result, TokenExchangeFailure)

    def test_exchange_code_for_tokens_userinfo_error(self, mock_requests_post, mock_requests_get):
    ...
    def test_exchange_code_uses_pkce_verifier(self, mock_requests_post, mock_requests_get):
...
class TestEdgeCases:
    ...
    def test_token_exchange_sets_expiry_timestamp(self, mock_requests_post, mock_requests_get):
