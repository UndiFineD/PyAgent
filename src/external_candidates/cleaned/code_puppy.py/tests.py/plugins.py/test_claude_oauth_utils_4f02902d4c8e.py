# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_claude_oauth_utils.py
...
class TestModelStorage:
    ...
    @patch("code_puppy.plugins.claude_code_oauth.utils.get_claude_models_path")
    def test_load_claude_models_filtered_oauth_models(self, mock_get_path, temp_models_file):
...
class TestAddModelsToConfig:
    ...
    def test_add_models_to_extra_config_success(self, mock_get_token, mock_save):
        ...
        assert "claude-code-claude-sonnet-3-0-20240229" not in saved_config  # Older sonnet
        assert "claude-code-claude-haiku-3-0-20240307" not in saved_config  # Older haiku
        ...
        assert haiku_config["custom_endpoint"]["url"] == CLAUDE_CODE_OAUTH_CONFIG["api_base_url"]
        ...
        assert haiku_config["context_length"] == CLAUDE_CODE_OAUTH_CONFIG["default_context_length"]
    ...
    def test_add_models_to_extra_config_no_tokens(self, mock_get_token, mock_save):
        ...
        api_key = saved_config["claude-code-claude-sonnet-3-5-20241022"]["custom_endpoint"]["api_key"]
    ...
    @patch("code_puppy.plugins.claude_code_oauth.utils.save_claude_models")
    @patch("code_puppy.plugins.claude_code_oauth.utils.get_valid_access_token")
    def test_add_models_to_extra_config_load_token_failure(self, mock_get_token, mock_save):
...
class TestErrorHandling:
    ...
    def test_filter_models_edge_cases(self):
        ...
        for input_models, expected_output in test_cases:
            ...
            assert set(result) == set(expected_output), f"Failed for input: {input_models}"
