...
def test_help_outputs_help():
    ...
    try:
        ...
        assert any("Built-in Commands" in str(call) for call in (mock_emit_info.call_args_list))
    finally:
...
def test_set_config_only_key():
    ...
    try:
        with (
            ...
            mock_emit_success.assert_called()
            assert any("Set" in str(call) and "pony" in str(call) for call in mock_emit_success.call_args_list)
    finally:
        mocks["emit_success"].stop()
...
def test_unknown_command():
    ...
    try:
        ...
        assert result is True
        mock_emit_warning.assert_called()
        assert any("Unknown command" in str(call) for call in mock_emit_warning.call_args_list)
    finally:
        mocks["emit_warning"].stop()
...
def test_bare_slash_shows_current_model():
    ...
    try:
        with patch(
            ...
            assert result is True
            mock_emit_info.assert_called()
            assert any("Current Model:" in str(call) and "yarn" in str(call) for call in mock_emit_info.call_args_list)
    finally:
        mocks["emit_info"].stop()
...
def test_set_no_args_prints_usage():
    ...
    try:
        with patch("code_puppy.config.get_config_keys", return_value=["foo", "bar"]):
            ...
            assert result is True
            mock_emit_warning.assert_called()
            assert any("Usage" in str(call) and "Config keys" in str(call) for call in mock_emit_warning.call_args_list)
    finally:
        mocks["emit_warning"].stop()
...
def test_bare_slash_with_spaces():
    ...
    try:
        with patch(
            ...
            assert result is True
            mock_emit_info.assert_called()
            assert any("Current Model:" in str(call) and "zoom" in str(call) for call in mock_emit_info.call_args_list)
    finally:
        mocks["emit_info"].stop()
...
class TestSessionCommand:
    ...
    def test_session_new_rotates(self):
        ...
        with (
            patch("code_puppy.config.rotate_autosave_id", return_value="new-id") as mock_rotate,
...
class TestGetCommandsHelp:
    ...
    def test_help_parses_list_of_tuples_format(self):
        ...
        with patch("code_puppy.callbacks.on_custom_command_help") as mock_callback:
            mock_callback.return_value = [[("cmd1", "First command"), ("cmd2", "Second command")]]
            help_text = str(get_commands_help())
...
def test_m_command_case_sensitive_baseline():
    ...
    with (
        ...
        patch("code_puppy.command_line.model_picker_completion.set_active_model") as mock_set_model,
    ):
