# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_rich_renderer.py
...
def test_render_agent_reasoning_outputs_markdown() -> None:
    ...
    markdown_calls = [call for call in console.print.call_args_list if call.args and isinstance(call.args[0], Markdown)]
...
def test_render_diff_uses_formatter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    printed = "".join(call.args[0] for call in console.print.call_args_list if isinstance(call.args[0], str))
...
def test_do_render_dispatches_multiple_messages() -> None:
    ...
    messages = [
        ...
        ShellOutputMessage(command="echo", stdout="ok", stderr="", exit_code=0, duration_seconds=0.1),
        ...
        VersionCheckMessage(current_version="1.0", latest_version="1.0", update_available=False),
        UserInputRequest(prompt_id="p1", prompt_text="Input", default_value=None, input_type="text"),
        ConfirmationRequest(
            prompt_id="p2",
            title="Confirm",
            description="Sure?",
        ...
        SelectionRequest(prompt_id="p3", prompt_text="Pick", options=["A", "B"], allow_cancel=False),
    ]
