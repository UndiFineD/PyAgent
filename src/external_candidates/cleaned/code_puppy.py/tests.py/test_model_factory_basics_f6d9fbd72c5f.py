# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_model_factory_basics.py
import json
import os
...
class TestModelFactoryBasics:
    ...
    def test_load_config_with_extra_models(
        ...
        extra_config = {"custom-model": {"type": "custom_openai", "name": "custom-gpt-4"}}
    ...
    def test_load_config_invalid_json(
        ...
        # Configure exists() for each path
        mock_main_path.exists.return_value = True  # models.json exists
        mock_extra_path.exists.return_value = True  # extra models exists (but has invalid JSON)
    ...
    def test_get_model_not_found(self):
        ...
        with pytest.raises(ValueError, match="Model 'nonexistent-model' not found in configuration"):
    ...
    @patch("code_puppy.model_factory.pathlib.Path.exists", return_value=False)
    def test_load_config_with_callbacks(self, mock_exists, mock_file, mock_on_load, mock_get_callbacks):
