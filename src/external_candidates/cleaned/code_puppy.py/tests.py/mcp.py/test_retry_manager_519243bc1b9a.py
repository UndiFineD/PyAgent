# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_retry_manager.py
"""
...
class TestRetryManager:
    ...
    async def test_non_retryable_error(self):
        ...
        mock_func = AsyncMock(side_effect=httpx.HTTPStatusError("Unauthorized", request=Mock(), response=response))
    ...
    def test_calculate_backoff_exponential_jitter(self):
        ...
        delays = [self.retry_manager.calculate_backoff(3, "exponential_jitter") for _ in range(10)]
    ...
    def test_should_retry_retryable_errors(self):
        ...
        http_error_500 = httpx.HTTPStatusError("Server error", request=Mock(), response=response_500)
        assert self.retry_manager.should_retry(http_error_500)

        # Rate limit (429)
        response_429 = Mock()
        response_429.status_code = 429
        http_error_429 = httpx.HTTPStatusError("Rate limit", request=Mock(), response=response_429)
        ...
        response_429_json.json.return_value = {"error": {"message": "Rate limit exceeded. Please try again later."}}
        ...
        http_error_408 = httpx.HTTPStatusError("Request timeout", request=Mock(), response=response_408)
    ...
    def test_should_retry_non_retryable_errors(self):
        ...
        http_error_401 = httpx.HTTPStatusError("Unauthorized", request=Mock(), response=response_401)
        assert not self.retry_manager.should_retry(http_error_401)

        response_403 = Mock()
        response_403.status_code = 403
        http_error_403 = httpx.HTTPStatusError("Forbidden", request=Mock(), response=response_403)
        assert not self.retry_manager.should_retry(http_error_403)

        # Client errors (4xx except 408)
        response_400 = Mock()
        response_400.status_code = 400
        http_error_400 = httpx.HTTPStatusError("Bad request", request=Mock(), response=response_400)
        assert not self.retry_manager.should_retry(http_error_400)

        response_404 = Mock()
        response_404.status_code = 404
        http_error_404 = httpx.HTTPStatusError("Not found", request=Mock(), response=response_404)
        assert not self.retry_manager.should_retry(http_error_404)

        # Schema/validation errors
        assert not self.retry_manager.should_retry(ValueError("Schema validation failed"))
...
class TestGlobalRetryManager:
    ...
    async def test_retry_mcp_call_convenience_function(self):
        ...
        result = await retry_mcp_call(func=mock_func, server_id="test-server", max_attempts=2, strategy="linear")
...
class TestConcurrentOperations:
    ...
    async def test_concurrent_stats_operations(self):
        ...
        for server_id, stats in all_stats.items():
            assert stats.total_retries > 0
            assert stats.successful_retries == stats.total_retries  # All were successful
