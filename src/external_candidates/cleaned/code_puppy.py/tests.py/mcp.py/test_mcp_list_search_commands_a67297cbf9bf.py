# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_mcp_list_search_commands.py
"""
...
class TestListCommand:
    ...
    def test_execute_manager_exception(self):
        ...
        for message, _ in error_messages:
            ...
            if "Error listing servers" in message_str and "Manager error" in message_str:
                error_found = True
                break

        assert error_found, f"Expected error message not found in: {[str(msg) for msg, _ in error_messages]}"
...
class TestSearchCommand:
    ...
    def test_execute_no_args_shows_popular(self, mock_emit_info, mock_server_catalog):
        ...
        hint_found = any("✓ = Verified" in msg[0] and "⭐ = Popular" in msg[0] for msg in mock_emit_info.messages)
    ...
    def test_execute_with_many_results_limits_to_20(self, mock_emit_info, mock_server_catalog):
        ...
        assert len(mock_emit_info.messages) >= 2
        table_message = mock_emit_info.messages[1][0]
        assert hasattr(table_message, "show_header") or isinstance(table_message, str)  # Rich Table created
    ...
    @pytest.mark.skip("Search functionality not implemented")
    def test_execute_with_verified_and_popular_indicators(self, mock_emit_info, mock_server_catalog):
    ...
    def test_execute_import_error(self, mock_emit_info):
        ...
        with patch("code_puppy.mcp_.server_registry_catalog.catalog", side_effect=ImportError):
            self.command.execute(["test"])
    ...
    def test_execute_with_empty_server_fields(self, mock_emit_info, mock_server_catalog):
...
class TestCommandIntegration:
    ...
    def test_commands_use_different_group_ids(self, mock_emit_info, mock_mcp_manager):
        ...
        with patch("code_puppy.mcp_.server_registry_catalog.catalog") as mock_catalog:
            ...
            list_cmd.execute([])
            list_group_id = mock_emit_info.messages[0][1]  # First list message's group ID

            mock_emit_info.messages.clear()  # Reset
            search_cmd.execute([])
            search_group_id = mock_emit_info.messages[0][1]  # First search message's group ID
