...
class TestStderrCapture:
    ...
    async def test_read_pipe_encoding_errors(self):
        ...
        assert "\ufffd" in capture._captured_lines[1] or "invalid" in capture._captured_lines[1]
        assert len(handler_calls) == 2
...
class TestCapturedMCPServerStdio:
    ...
    async def test_client_streams_context_manager(self):
        ...
        with (
            ...
            patch("code_puppy.mcp_.captured_stdio_server.stdio_client") as mock_stdio_client,
            patch("code_puppy.mcp_.captured_stdio_server.StderrCapture") as mock_stderr_capture,
        ):
            mock_stdio_client.return_value.__aenter__ = AsyncMock(return_value=(mock_read_stream, mock_write_stream))
    ...
    async def test_stderr_line_handler(self):
        ...
        with patch("code_puppy.mcp_.captured_stdio_server.logger") as mock_logger:
            ...
            mock_logger.debug.assert_called_once_with("[MCP Server python] default test")
...
class TestIntegration:
    ...
    def test_concurrent_server_handling(self):
        ...
        for i in range(5):
            ...
            assert all("Message" in line and f"server {i}" in line for line in server_output)
