# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\conftest.py
"""
...
def mock_mcp_manager():
    ...
    manager.add_mock_server(MockServerInfo(id="test-server-1", name="test-server", state=ServerState.STOPPED))
...
def mock_emit_info():
    ...
    patches = [
        patch("code_puppy.command_line.mcp.start_command.emit_info", side_effect=capture),
        patch("code_puppy.command_line.mcp.stop_command.emit_info", side_effect=capture),
        patch("code_puppy.command_line.mcp.restart_command.emit_info", side_effect=capture),
        patch("code_puppy.command_line.mcp.list_command.emit_info", side_effect=capture),
        patch("code_puppy.command_line.mcp.search_command.emit_info", side_effect=capture),
        patch("code_puppy.command_line.mcp.status_command.emit_info", side_effect=capture),
    ]
...
def mock_emit_prompt():
    ...
    with patch("code_puppy.messaging.emit_prompt", side_effect=capture_response) as mock:
