# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_server_registry_catalog.py
"""
...
class TestMCPServerTemplate:
    ...
    def test_to_server_config_env_substitution(self):
        ...
        config = template.to_server_config(api_key="secret123", host="localhost", port=5432)

        assert config["env"]["API_KEY"] == "secret123"
        assert config["env"]["DATABASE_URL"] == "postgresql://user:pass@localhost:5432/db"
        assert config["env"]["DEBUG"] == "true"
...
class TestMCP_SERVER_REGISTRY:
    ...
    def test_registry_contains_filesystem_home_template(self):
        ...
        fs_home_templates = [t for t in MCP_SERVER_REGISTRY if t.id == "filesystem-home"]
...
class TestRegistryFunctionality:
    ...
    def test_search_by_description(self):
        ...
        file_results = [t for t in MCP_SERVER_REGISTRY if "file" in t.description.lower()]
        code_results = [t for t in MCP_SERVER_REGISTRY if "code" in t.description.lower()]
    ...
    def test_filter_by_requirements(self):
        ...
        node_requirement_templates = [t for t in MCP_SERVER_REGISTRY if "node" in t.get_required_tools()]
