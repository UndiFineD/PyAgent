# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\agents\test_subagent_stream_handler.py
...
class TestHandleEvent:
    ...
    async def test_handle_event_no_session_id(self, mock_manager):
        ...
        with patch("code_puppy.agents.subagent_stream_handler._fire_callback") as mock_fire:
            await _handle_event(
                event=event,
    ...
    async def test_handle_part_start_thinking(self, mock_manager):
        ...
        with patch("code_puppy.agents.subagent_stream_handler._fire_callback") as mock_fire:
            ...
            mock_manager.update_agent.assert_called_once_with("session-123", status="thinking")
    ...
    async def test_handle_part_start_text(self, mock_manager):
        ...
        with patch("code_puppy.agents.subagent_stream_handler._fire_callback") as mock_fire:
            ...
            mock_manager.update_agent.assert_called_once_with("session-123", status="running")
            mock_fire.assert_called_once()
            call_args = mock_fire.call_args
            assert call_args[0][0] == "part_start"
    ...
    async def test_handle_part_start_tool_call(self, mock_manager):
        ...
        tool_part = ToolCallPart(tool_name="my_tool", args="{}", tool_call_id="call-123")
...
class TestSubagentStreamHandler:
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("code_puppy.agents.subagent_stream_handler._fire_callback"):
                    ...
                    delta_calls = [c for c in mock_manager.update_agent.call_args_list if "token_count" in c[1]]
    ...
    async def test_stream_handler_mixed_event_types(self, mock_ctx, mock_manager):
        ...
        events = [
            ...
            PartDeltaEvent(index=2, delta=ToolCallPartDelta(args_delta='{"query": "test"}')),
