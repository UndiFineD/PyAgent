# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\agents\test_pack_agents.py
...
class TestPackIntegration:
    ...
    def test_pack_agents_tool_consistency(self):
        ...
        for agent_name in pack_agents:
            ...
            assert "agent_share_your_reasoning" in tools, f"{agent_name} missing reasoning tool"

            # All pack agents should be able to run shell commands
            # (for bd, gh, git operations)
            assert "agent_run_shell_command" in tools, f"{agent_name} missing shell command tool"
