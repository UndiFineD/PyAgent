# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\agents\test_event_stream_handler.py
...
class TestEventStreamHandler:
    ...
    @pytest.mark.asyncio
    async def test_handles_part_end_event_preserves_spinner_for_next_text(self, mock_ctx):
    ...
    async def test_tool_call_token_counting(self, mock_ctx):
        ...
        deltas = [
            PartDeltaEvent(index=0, delta=ToolCallPartDelta(tool_name_delta="test_tool")),
...
class TestSubAgentSuppression:
    ...
    @pytest.mark.asyncio
    async def test_subagent_suppresses_output_when_verbose_false(self, mock_ctx, monkeypatch):
    ...
    async def test_suppression_works_with_tool_calls(self, mock_ctx, monkeypatch):
        ...
        async def mock_events():
            ...
            yield PartDeltaEvent(index=0, delta=ToolCallPartDelta(tool_name_delta="my_tool"))
