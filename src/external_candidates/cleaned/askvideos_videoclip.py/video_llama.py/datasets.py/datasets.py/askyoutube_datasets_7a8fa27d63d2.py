...
class AskYoutubeInstructDataset(BaseDataset):
    def __init__(
        ...
        self.transform = AlproVideoTrainProcessor(image_size=self.resize_size, n_frms=self.num_frm).transform
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            if video is None or video.size() != torch.Size([3, self.vis_processor.n_frms, 224, 224]):
                print(
    ...
    def collater(self, instances):
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("text_input", "labels"))
...
def preprocess_multimodal(conversation_list: Sequence[str], multimodal_cfg: dict, cur_token_len: int, msg="") -> Dict:
    is_multimodal = True
    image_token_len = cur_token_len
    conversation_list[0]["q"] = (
        "<Video>" + DEFAULT_IMAGE_PATCH_TOKEN * image_token_len + "</Video> " + msg + conversation_list[0]["q"]
    )
