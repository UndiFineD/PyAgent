# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hawk-eye\hawk_scanner\commands\mongodb.py
import pymongo
...
def check_data_patterns(
    ...
    for collection_name in collections_to_scan:
        if collection_name not in all_collections:
            system.print_error(
                args,
                f"Collection {collection_name} not found in the database. Skipping.",
            )
            continue
        ...
        if mongodb_config:
            ...
            for key, config in mongodb_config.items():
                ...
                if uri:
                    ...
                    system.print_info(
                        args,
                        f"Checking MongoDB Profile {key} with host and authentication",
                    )
                else:
                ...
                if db:
                    results += check_data_patterns(
                        ...
                        limit_end=limit_end,
                        whitelisted_collections=collections,
                    )
        else:
