# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\app_utils.py\hugging_face_utils_153fc650c89a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\hugging_face_utils.py
...
def get_model_card(cfg, model, repo_id) -> huggingface_hub.ModelCard:
    ...
    cfg_kwargs = dict(
        ...
        end_of_sentence=(cfg.tokenizer._tokenizer_eos_token if cfg.dataset.add_eos_token_to_prompt else ""),
    )
...
{% if add_generation_prompt %}{{ 'text_answer_separator' }}{% endif %}"""
    ...
    if cfg.dataset.system_column != "None":
        ...
        chat_template = chat_template.replace("chat_template_for_checking_system_role", "")
...
def publish_model_to_hugging_face(
    ...
    if device == "cpu" or device == "cpu_shard":
        ...
        raise ValueError("Invalid device value. Use 'cpu', 'cpu_shard' or 'cuda:INTEGER'.")
    ...
    card.push_to_hub(repo_id=repo_id, repo_type="model", commit_message="Upload model card")
