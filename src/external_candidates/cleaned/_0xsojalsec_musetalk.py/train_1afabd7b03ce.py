# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MuseTalk\train.py
import argparse
import logging
import math
import os
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(dataloader_dict["train_dataloader"]):
            ...
            with torch.no_grad():
                ...
                ref_pixel_values = (
                    batch["pixel_values_ref_img"].to(weight_dtype).to(accelerator.device, non_blocking=True)
                )
                ...
                if cfg.loss_params.sync_loss > 0:
                    ...
                    if cfg.use_adapted_weight:
                        ...
                        image_audio_sim_gt = F.cosine_similarity(audio_embed, vision_embed_gt, dim=1)[0]
                ...
                pixel_values_face_mask_backward = rearrange(pixel_values_face_mask_backward, "b f c h w -> (b f) c h w")
                audio_prompts_backward = rearrange(audio_prompts_backward, "b f c h w-> (b f) c h w")
                audio_prompts_backward = rearrange(audio_prompts_backward, "(b f) c h w -> (b f) (c h) w", b=bsz)
            ...
            if cfg.loss_params.mouth_gan_loss > 0:
                frames_mouth, image_pred_mouth = get_mouth_region(frames, image_pred, pixel_values_face_mask_backward)
            ...
            if cfg.loss_params.mouth_gan_loss > 0:
                ...
                mouth_loss_D = loss_dict["mouth_discriminator_full"](frames_mouth, image_pred_mouth.detach())
                avg_mouth_loss_D = accelerator.gather(mouth_loss_D.repeat(cfg.data.train_bs)).mean()
