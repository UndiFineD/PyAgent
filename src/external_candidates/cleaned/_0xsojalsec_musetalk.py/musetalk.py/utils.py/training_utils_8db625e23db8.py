# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-MuseTalk\musetalk\utils\training_utils.py
import json
import logging
import os
...
class Net(nn.Module):
    ...
    def forward(
        ...
        model_pred = self.unet(input_latents, timesteps, encoder_hidden_states=audio_prompts).sample
        return model_pred
...
def initialize_loss_functions(cfg, accelerator, scheduler_max_steps):
    ...
    if cfg.loss_params.gan_loss > 0:
        loss_dict["discriminator"] = MultiScaleDiscriminator(**cfg.model_params.discriminator_params).to(
            accelerator.device
        )
    ...
    if cfg.loss_params.mouth_gan_loss > 0:
        loss_dict["mouth_discriminator"] = MultiScaleDiscriminator(**cfg.model_params.discriminator_params).to(
            accelerator.device
        )
