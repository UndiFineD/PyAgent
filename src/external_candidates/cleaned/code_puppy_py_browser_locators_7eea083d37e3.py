# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\tools.py\browser.py\browser_locators_7eea083d37e3.py
    ...
    try:
        ...
        for i in range(min(count, 10)):
            ...
            if await element.is_visible():
                ...
                elements.append({"index": i, "tag": tag_name, "text": full_text, "visible": True})
    ...
    try:
        ...
        for i in range(min(count, 10)):
            ...
            if await element.is_visible():
                ...
                value = await element.input_value() if tag_name in ["input", "textarea"] else None
...
async def find_buttons(text_filter: Optional[str] = None, timeout: int = 10000) -> Dict[str, Any]:
    ...
    try:
        ...
        emit_success(
            f"Found {filtered_count} buttons" + (f" containing '{text_filter}'" if text_filter else ""),
            message_group=group_id,
        )
...
async def find_links(text_filter: Optional[str] = None, timeout: int = 10000) -> Dict[str, Any]:
    ...
    try:
        ...
        emit_success(
            f"Found {filtered_count} links" + (f" containing '{text_filter}'" if text_filter else ""),
            message_group=group_id,
        )
