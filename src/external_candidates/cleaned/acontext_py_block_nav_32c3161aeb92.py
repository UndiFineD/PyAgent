# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\acontext.py\src.py\server.py\core.py\acontext_core.py\service.py\data.py\block_nav_32c3161aeb92.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Acontext\src\server\core\acontext_core\service\data\block_nav.py
...
async def assert_block_type(
    ...
    if par_type["type"] != block_type:
        return Result.reject(f"Block {block_id}(type {par_type['type']}) is not a {block_type}")
...
async def recover_path_by_id(
    ...
    while block_id is not None:
        query = select(Block.id, Block.title, Block.parent_id).where(Block.space_id == space_id, Block.id == block_id)
...
async def get_path_info_by_id(
    ...
    if block["type"] == BLOCK_TYPE_PAGE:
        ...
        r = await recover_path_by_id(db_session, space_id, block["id"], add_folder_slash=True)
