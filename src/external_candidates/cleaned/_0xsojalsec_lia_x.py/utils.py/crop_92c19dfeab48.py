# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LIA-X\utils\crop.py
...
def compute_bbox(bbox, frame_shape, increase_area=0.1, increase_area_top=0.0):
    ...
    top = int(top - height_increase * height * (1 + increase_area_top))
...
def detect_face(frame, increase_area, increase_area_top):
    ...
    w, h, left, top = compute_bbox(bbox, frame_shape, increase_area, increase_area_top)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--increase_scale", type=float, default=0.25)
    parser.add_argument("--increase_top_scale", type=float, default=0.5)
    args = parser.parse_args()

    print("==> running")
    if args.mode == "img":
        save_path = "./data/source/" + Path(args.data_path).stem + "_crop.png"
        img = imageio.v3.imread(args.data_path)
        w, h, left, top = detect_face(img, args.increase_scale, args.increase_top_scale)
        img_crop = crop(img, w, h, left, top)
        print("The size of cropped image: ", img_crop.shape)
        ...
        for i, frame in tqdm(enumerate(reader)):
            # print(frame.shape)
            if i == 0:
                w, h, left, top = detect_face(frame, args.increase_scale, args.increase_top_scale)
