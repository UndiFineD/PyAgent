# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\cache.py\redis_cache_queue.py\redis_length_cache_manager_dc39f6a2140c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\cache\redis_cache_queue\redis_length_cache_manager.py
...
class RedisLengthCacheFactory:
    ...
    async def _ensure_length_cleanup_script_registered(self):
        ...
        if self._length_cleanup_script is None:
            ...
            self._length_cleanup_script = client.register_script(LENGTH_CLEANUP_LUA_SCRIPT)
...
class RedisLengthCacheManager:
    ...
    async def _cleanup_if_needed(self, key: str) -> int:
        ...
        if random.random() < self.cleanup_probability:
            ...
            cleaned_count = await self._length_cleanup_script(keys=[key], args=[self.max_length])
    ...
    async def append(
        ...
        try:
            ...
            logger.error("Failed to append data to Redis: key=%s, error=%s", key, str(e))
