# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\loops\evaluation_loop.py
...
class _EvaluationLoop(_Loop):
    ...
    def _evaluation_step(
        ...
        hook_kwargs = self._build_kwargs(
            batch,
            batch_idx,
            (dataloader_idx if self._is_sequential and self.num_dataloaders > 1 else None),
        )
            ...
            if isinstance(v, dict):
                for new_key in apply_to_collection(v, dict, _EvaluationLoop._get_keys):
                    yield (
                        k,
                        *new_key,
                    )  # this need to be in parenthesis for older python versions
            else:
                yield (k,)
    ...
    def _print_results(results: list[_OUT_DICT], stage: str) -> None:
        ...
        max_length = int(
            min(
                max(len(max(metrics_strs, key=len)), len(max(headers, key=len)), 25),
                term_size / 2,
            )
        )
