# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\parity_fabric\utils.py
...
def is_timing_close(timings_torch, timings_fabric, rtol=1e-2, atol=0.1):
    ...
    return bool(
        torch.isclose(
            torch.median(timings_torch[3:]),
            torch.median(timings_fabric[3:]),
            rtol=rtol,
            atol=atol,
        )
    )
