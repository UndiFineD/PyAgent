# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_ddp_integration.py
...
def test_find_unused_parameters_ddp_spawn_raises():
    ...
    with pytest.raises(
        ...
        match="It looks like your LightningModule has parameters that were not used in",
    ):
...
def test_incorrect_ddp_script_spawning(tmp_path):
    ...
    with pytest.raises(
        RuntimeError,
        match="Lightning attempted to launch new distributed processes with `local_rank > 0`.",
    ):
        trainer.fit(model)
