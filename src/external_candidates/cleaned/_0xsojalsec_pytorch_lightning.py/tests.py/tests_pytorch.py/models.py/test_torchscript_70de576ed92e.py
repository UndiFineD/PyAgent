# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\models\test_torchscript.py
...
@pytest.mark.skipif(
    _IS_WINDOWS and _TORCH_GREATER_EQUAL_2_4,
    reason="not close on Windows + PyTorch 2.4",
)
...
def test_torchscript_with_no_input():
    ...
    with pytest.raises(
        ValueError,
        match="requires either `example_inputs` or `model.example_input_array`",
    ):
