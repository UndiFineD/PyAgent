# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\checkpointing\test_model_checkpoint.py
...
def test_model_checkpoint_link_checkpoint(tmp_path):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.model_checkpoint.os.symlink",
...
def test_none_monitor_top_k(tmp_path):
    ...
    with pytest.raises(
        ...
        match=r"ModelCheckpoint\(save_top_k=3, monitor=None\) is not a valid*",
    ):
        ModelCheckpoint(dirpath=tmp_path, save_top_k=3)
...
def test_invalid_trigger_combination(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match=r".*Combination of parameters every_n_train_steps",
    ):
...
def test_configure_model_checkpoint(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match="`enable_checkpointing=False` but found `ModelCheckpoint`",
    ):
...
def test_ckpt_version_counter_disabled_after_rerun_new_trainer(tmp_path):
    ...
    for i in range(epochs):
        ...
        assert {Path(f).name for f in mc.best_k_models} == {
            "epoch=0.ckpt",
            "epoch=1.ckpt",
        }
...
def test_resume_training_preserves_old_ckpt_last(tmp_path):
    ...
    assert set(os.listdir(tmp_path / "checkpoints")) == {
        "last.ckpt",
        "step=2.ckpt",
        "step=3.ckpt",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "last",
            ".ckpt",
            {"log.txt", "last-v0.ckpt", "last-v1.ckpt"},
            {"last-v0.ckpt", "last-v1.ckpt"},
        ),
        (
            "other",
            ".pt",
            {"log.txt", "last-v0.ckpt", "other-v0.pt", "other-v1.pt"},
            {"other-v0.pt", "other-v1.pt"},
        ),
    ],
)
