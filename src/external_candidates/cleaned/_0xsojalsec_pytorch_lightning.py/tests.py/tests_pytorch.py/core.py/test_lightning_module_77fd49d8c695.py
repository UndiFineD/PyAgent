# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\core\test_lightning_module.py
...
def test_lightning_module_configure_gradient_clipping_different_argument_values(
    tmp_path,
):
...
def test_fabric_attributes():
    ...
    with pytest.raises(
        AttributeError,
        match="Your LightningModule code tried to access `self.trainer.current_epoch`",
    ):
...
def test_grad_clipping_lm_fabric(algo):
    ...
    model.clip_gradients(
        ...
        gradient_clip_algorithm=GradClipAlgorithmType(algo),
    )
