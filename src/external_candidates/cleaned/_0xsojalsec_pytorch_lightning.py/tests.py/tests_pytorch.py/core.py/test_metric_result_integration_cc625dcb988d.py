# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\core\test_metric_result_integration.py
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            marks=RunIf(min_cuda_gpus=2, standalone=True),
        ),
    ],
)
...
def test_metric_result_respects_dtype(default_type, converted_type):
    ...
    with pytest.warns(
        UserWarning,
        match=rf"`self.log\('bar', ...\)` in your `foo` .* Converting it to {converted_type}",
    ):
...
def test_logger_sync_dist(distributed_env, log_val):
    ...
    with (
        warning_ctx(
            PossibleUserWarning,
            match=r"recommended to use `self.log\('bar', ..., sync_dist=True\)`",
        ),
        patch_ctx,
    ):
