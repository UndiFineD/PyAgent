# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\test_loops.py
...
def test_restart_parity(tmp_path):
    ...
    assert (
        compare_state_dicts(
            end_of_epoch_ckpt["lr_schedulers"][0],
            end_of_epoch_ckpt_v1["lr_schedulers"][0],
        )
        == {}
    )
...
def test_workers_are_shutdown(tmp_path, should_fail, persistent_workers):
    ...
    if persistent_workers:
        ...
        expected = [
            ...
            trainer.current_epoch,
        ]  # once epoch end, once on teardown
    elif should_fail:
        expected = [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda: CombinedLoader([StatefulIterable(3), StatefulIterable(1)]),
            True,
            [[3, 1], [4, 2]],
            [[5, 3], [6, 4]],
        ),
