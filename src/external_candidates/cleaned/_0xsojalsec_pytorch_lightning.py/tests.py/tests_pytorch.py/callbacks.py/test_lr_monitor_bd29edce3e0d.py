# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_lr_monitor.py
...
def test_lr_monitor_multiple_param_groups_no_lr_scheduler(tmp_path):
    ...
    assert list(lr_monitor.last_momentum_values) == [
        "lr-Adam/pg1-momentum",
        "lr-Adam/pg2-momentum",
    ]
    assert all(val == momentum for val in lr_monitor.last_momentum_values.values())
    assert list(lr_monitor.last_weight_decay_values) == [
        "lr-Adam/pg1-weight_decay",
        "lr-Adam/pg2-weight_decay",
    ]
