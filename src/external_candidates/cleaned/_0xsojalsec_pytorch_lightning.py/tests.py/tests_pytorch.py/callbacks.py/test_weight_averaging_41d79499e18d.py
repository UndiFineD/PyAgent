# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_weight_averaging.py
...
def _train_and_resume(model: TestModel, dataset: Dataset, tmp_path: str, devices: int = 1, **kwargs) -> TestModel:
    _train(
        ...
        dataset,
        tmp_path,
        EMATestCallback(devices=devices),
        devices=devices,
        will_crash=True,
        **kwargs,
    )
