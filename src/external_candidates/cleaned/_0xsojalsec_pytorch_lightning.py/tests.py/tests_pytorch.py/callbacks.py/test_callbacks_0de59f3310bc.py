# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_callbacks.py
...
def test_resume_incomplete_callbacks_list_warning(tmp_path):
    ...
    with pytest.warns(
        UserWarning,
        match=escape(f"Please add the following callbacks: [{repr(callback0.state_key)}]"),
    ):
