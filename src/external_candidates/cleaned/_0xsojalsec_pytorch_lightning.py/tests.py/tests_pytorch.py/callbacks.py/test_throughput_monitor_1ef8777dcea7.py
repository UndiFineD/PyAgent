# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_throughput_monitor.py
...
def test_throughput_monitor_fit(tmp_path):
    ...
    monitor = ThroughputMonitor(
        length_fn=lambda x: 3 * 2,
        batch_size_fn=lambda x: 3,
        window_size=4,
        separator="|",
    )
    model = BoringModel()
    ...
    with (
        mock.patch(
            "lightning.pytorch.callbacks.throughput_monitor.get_available_flops",
            return_value=100,
        ),
