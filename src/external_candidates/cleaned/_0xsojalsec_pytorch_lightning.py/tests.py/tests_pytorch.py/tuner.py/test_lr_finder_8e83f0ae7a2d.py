# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\tuner\test_lr_finder.py
    ...
    class CustomBoringModel(BoringModel):
        ...
        def configure_optimizers(self):
            return torch.optim.SGD(
                self.parameters(),
                lr=self.hparams.my_fancy_lr if use_hparams else self.my_fancy_lr,
            )
...
def test_exponential_vs_linear_mode_gradient_difference(tmp_path):
    ...
    for lr_finder, mode in [
        (lr_finder_linear, "linear"),
        (lr_finder_exp, "exponential"),
    ]:
