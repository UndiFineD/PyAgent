# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_compile.py
...
def test_trainer_compiled_model_deepspeed(_, tmp_path, monkeypatch, mps_count_0):
    ...
    with pytest.raises(
        RuntimeError,
        match="Using a compiled model is incompatible with the current strategy.*",
    ):
        trainer.fit(compiled_model)
...
@pytest.mark.skipif(sys.platform == "darwin", reason="fatal error: 'omp.h' file not found")
@pytest.mark.skipif(
    not _PYTHON_GREATER_EQUAL_3_9_0,
    reason="AssertionError: failed to reach fixed point",
)
@pytest.mark.xfail(
    sys.platform == "win32" and _TORCH_GREATER_EQUAL_2_2,
    strict=False,
    reason="RuntimeError: Failed to import",
)
@RunIf(dynamo=True)
