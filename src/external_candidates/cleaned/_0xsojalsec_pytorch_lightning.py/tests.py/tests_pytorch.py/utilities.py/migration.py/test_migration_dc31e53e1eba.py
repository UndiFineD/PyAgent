# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\migration\test_migration.py
    ...
    [
        (
            {"epoch": 1, "global_step": 23, "checkpoint_callback_best": 0.34},
            {
                "epoch": 1,
                "global_step": 23,
                "callbacks": {ModelCheckpoint: {"best_model_score": 0.34}},
            },
        ),
        (
            {
            ...
            },
            {
                "epoch": 1,
                "global_step": 23,
                "callbacks": {ModelCheckpoint: {"best_model_score": 0.99}},
            },
        ),
        (
            {
            ...
            },
            {
                "epoch": 1,
                "global_step": 23,
                "callbacks": {ModelCheckpoint: {"best_model_path": "path"}},
            },
        ),
        (
            {
...
def test_migrate_model_checkpoint_save_on_train_epoch_end_default(
    ...
    old_checkpoint = {
        "callbacks": {legacy_state_key: {"dummy": 0}},
        "global_step": 0,
        "epoch": 1,
    }
...
def test_migrate_model_checkpoint_save_on_train_epoch_end_default_collision():
    ...
    with pytest.warns(
        PossibleUserWarning,
        match="callback states in this checkpoint.* colliding with each other",
    ):
    ...
    assert updated_checkpoint["loops"] == {
        "fit_loop": {
            "epoch_loop.state_dict": {
                "any": "state",
                "old_batch_loop_state_dict": old_batch_loop_state,
            },
