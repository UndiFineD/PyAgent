# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\connectors\test_data_connector.py
...
@mock.patch(
    "lightning.pytorch.trainer.connectors.data_connector.mp.get_start_method",
    return_value="not_spawn",
)
def test_worker_check(_, cpu_count_mock, num_workers, cpu_count, expected_warning, monkeypatch):
    ...
    if expected_warning:
        ctx = pytest.warns(
            UserWarning,
            match="Consider increasing the value of the `num_workers` argument`",
        )
    else:
        ctx = no_warning_call()
...
@mock.patch(
    "lightning.pytorch.trainer.connectors.data_connector.suggested_max_num_workers",
    return_value=2,
)
...
def test_attach_data_input_validation_with_none_dataloader(trainer_fn_name, dataloader_name, tmp_path):
    ...
    with pytest.raises(
        TypeError,
        match=f"An invalid .*dataloader was passed to `Trainer.{trainer_fn_name}",
    ):
...
def test_non_iterables_raise(tmp_path, trainer_fn_name, dataloader_name, stage, dataloader):
    ...
    with pytest.raises(
        TypeError,
        match=rf"invalid dataloader was passed to `Trainer.{trainer_fn_name}\({dataloader_name}",
    ):
    ...
    with pytest.raises(
        TypeError,
        match=f"invalid dataloader was returned from `BoringModel.{dl_method}",
    ):
        trainer_fn(model)
