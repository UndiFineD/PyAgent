# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\connectors\test_callback_connector.py
...
def test_checkpoint_callbacks_are_last(tmp_path):
    ...
    trainer = Trainer(
        callbacks=[
            progress_bar,
            lr_monitor,
            ModelCheckpoint(tmp_path, filename="ckpt_trainer"),
        ]
    )
    trainer.strategy._lightning_module = model
    cb_connector = _CallbackConnector(trainer)
...
def test_validate_unique_callback_state_key():
    ...
    with pytest.raises(
        RuntimeError,
        match="Found more than one stateful callback of type `MockCallback`",
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [ConflictingCallback(), AnotherConflictingCallback()],
            "Found more than one stateful callback",
        ),
    ],
)
