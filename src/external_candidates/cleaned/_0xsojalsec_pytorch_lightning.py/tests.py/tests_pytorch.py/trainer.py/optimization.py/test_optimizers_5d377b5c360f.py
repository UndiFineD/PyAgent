# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\optimization\test_optimizers.py
...
def test_reducelronplateau_with_no_monitor_raises(tmp_path):
    ...
    with pytest.raises(
        ...
        match="`configure_optimizers` must include a monitor when a `ReduceLROnPlateau`",
    ):
        trainer.fit(model)
...
def test_reducelronplateau_with_no_monitor_in_lr_scheduler_dict_raises(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match="must include a monitor when a `ReduceLROnPlateau`",
    ):
        trainer.fit(model)
...
def test_lr_scheduler_strict(step_mock, tmp_path, complete_epoch):
    ...
    if complete_epoch:
        ...
        with pytest.warns(
            RuntimeWarning,
            match=r"ReduceLROnPlateau conditioned on metric .* which is not available but strict",
        ):
            trainer.fit(model)
...
def test_optimizer_config_dict_with_extra_keys_warns(tmp_path):
    ...
    with pytest.warns(
        RuntimeWarning,
        match=r"Found unsupported keys in the optimizer configuration: \{.+\}",
    ):
...
def test_lr_scheduler_with_unknown_interval_raises(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match=r'The "interval" key in lr scheduler dict must be',
    ):
        trainer.fit(model)
...
def test_lr_scheduler_with_no_actual_scheduler_raises(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match='The lr scheduler dict must have the key "scheduler"',
    ):
        trainer.fit(model)
...
def test_invalid_optimizer_in_scheduler(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match="attached with an optimizer that wasn't returned",
    ):
        trainer.fit(model)
