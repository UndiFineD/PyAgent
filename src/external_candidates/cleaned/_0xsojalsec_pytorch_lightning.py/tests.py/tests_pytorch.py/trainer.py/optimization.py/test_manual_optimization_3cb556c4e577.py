# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\optimization\test_manual_optimization.py
    ...
    def training_step(self, batch, batch_idx):
        ...
        def gen_closure():
            ...
            make_manual_backward(
                loss_ones_gen,
                retain_graph=True,
                make_optimizer_step=make_gen_optimizer_step,
            )
...
@RunIf(min_cuda_gpus=2, standalone=True)
def test_step_with_optimizer_closure_with_different_frequencies_ddp_with_toggle_model(
    tmp_path,
):
