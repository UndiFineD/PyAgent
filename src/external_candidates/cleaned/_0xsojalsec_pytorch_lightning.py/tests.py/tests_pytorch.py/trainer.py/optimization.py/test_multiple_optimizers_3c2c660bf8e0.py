# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\optimization\test_multiple_optimizers.py
...
def test_multiple_optimizers_automatic_optimization_raises():
    ...
    with pytest.raises(
        RuntimeError,
        match="multiple optimizers is only supported with manual optimization",
    ):
        trainer.fit(model)
