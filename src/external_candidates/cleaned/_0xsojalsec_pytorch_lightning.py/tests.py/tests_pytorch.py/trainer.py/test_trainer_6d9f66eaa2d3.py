# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\test_trainer.py
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            2,
            False,
            [f"epoch={i}.ckpt" for i in (2, 4)],
            id="CASE K=2 (2.5 epoch 4, 2.8 epoch 2)",
        ),
        pytest.param(
            4,
            False,
            [f"epoch={i}.ckpt" for i in range(1, 5)],
            id="CASE K=4 (save all 4 base)",
        ),
        pytest.param(
            3,
            False,
            [f"epoch={i}.ckpt" for i in range(2, 5)],
            id="CASE K=3 (save the 2nd, 3rd, 4th model)",
        ),
        pytest.param(
            1,
            True,
            {"epoch=4.ckpt", "last.ckpt"},
...
def test_checkpoint_path_input(tmp_path, ckpt_path, save_top_k, fn):
    ...
    if ckpt_path == "best":
        ...
        if save_top_k > 0:
            ...
            with pytest.warns(
                UserWarning,
                match="The best model of the previous `fit` call will be used",
            ):
                trainer_fn(ckpt_path=ckpt_path)
            assert trainer.ckpt_path == trainer.checkpoint_callback.best_model_path
    else:
...
@pytest.mark.parametrize(
    "trainer_params",
    [
        {"max_epochs": 1, "accelerator": "gpu", "devices": 1},
        {"max_epochs": 1, "accelerator": "gpu", "devices": [0]},
    ],
)
...
class CustomPredictionWriter(BasePredictionWriter):
    ...
    def on_predict_epoch_end(self, trainer, pl_module):
        if trainer._accelerator_connector.is_distributed:
            for idx in range(2):
                assert isinstance(
                    trainer.predict_dataloaders[idx].batch_sampler.sampler,
                    UnrepeatedDistributedSampler,
                )
                assert isinstance(
                    trainer.predict_dataloaders[idx].batch_sampler,
                    _IndexBatchSamplerWrapper,
                )
...
def predict(
    ...
    if strategy == "ddp_spawn":
        with pytest.raises(
            ...
            match="`return_predictions` should be set to `False`",
        ):
...
@pytest.mark.parametrize(
    ("strategy", "devices"),
    [("auto", 1), pytest.param("ddp_spawn", 1, marks=RunIf(skip_windows=True))],
)
...
@pytest.mark.parametrize(
    "strategy",
    ["auto", pytest.param("ddp_spawn", marks=RunIf(skip_windows=True, mps=False))],
)
...
def test_trainer_metrics_reset_before_each_task(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path,
        fast_dev_run=4,
        callbacks=[TestMetricRestartCallback()],
    )
    trainer.fit(model)
    trainer.validate(model)
    trainer.test(model)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"strategy": "ddp", "accelerator": "cuda", "devices": 1},
            DDPStrategy,
            CUDAAccelerator,
            1,
        ),
        ...
        (
            {"strategy": "auto", "accelerator": "cuda", "devices": 2},
            DDPStrategy,
            CUDAAccelerator,
            2,
        ),
        (
            {"strategy": "ddp", "accelerator": "cuda", "devices": 2},
            DDPStrategy,
            CUDAAccelerator,
            2,
        ),
...
def test_trainer_save_checkpoint_no_model_attached():
    ...
    with pytest.raises(
        AttributeError,
        match="Saving a checkpoint is only possible if a model is attached",
    ):
