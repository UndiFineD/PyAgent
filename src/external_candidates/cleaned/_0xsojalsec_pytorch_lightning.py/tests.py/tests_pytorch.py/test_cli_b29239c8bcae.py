# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\test_cli.py
...
@pytest.mark.parametrize(
    ("trainer_class", "model_class"),
    [(Trainer, Model), (_trainer_builder, _model_builder)],
)
def test_lightning_cli(trainer_class, model_class, monkeypatch):
    ...
    with mock.patch(
        "sys.argv",
        ["any.py", "fit", "--model.model_param=7", "--trainer.limit_train_batches=100"],
    ):
        cli = LightningCLI(
            model_class,
...
def test_lightning_cli_args_callbacks(cleandir):
    ...
    with mock.patch("sys.argv", ["any.py", "fit", f"--trainer.callbacks={json.dumps(callbacks)}"]):
        cli = LightningCLI(
            ...
            trainer_defaults={
                "fast_dev_run": True,
                "logger": lazy_instance(CSVLogger, save_dir="."),
            },
        )
...
@pytest.mark.skipif(
    compare_version("jsonargparse", operator.lt, "4.21.3"),
    reason="vulnerability with failing imports",
)
def test_lightning_env_parse(cleandir):
    out = StringIO()
    with (
        mock.patch("sys.argv", ["", "fit", "--help"]),
        redirect_stdout(out),
...
def test_lightning_cli_ckpt_path_argument_hparams(cleandir):
    ...
    err = StringIO()
    with (
        mock.patch("sys.argv", ["any.py"] + cli_args),
        redirect_stderr(err),
        pytest.raises(SystemExit),
    ):
        cli = LightningCLI(BoringModel)
...
def test_lightning_cli_optimizer(run):
    ...
    with (
        mock.patch("sys.argv", ["any.py"] + argv),
...
def test_lightning_cli_optimizer_and_lr_scheduler():
    ...
    assert isinstance(
        cli.trainer.lr_scheduler_configs[0].scheduler,
...
def test_lightning_cli_optimizers_and_lr_scheduler_with_link_to(use_generic_base_class):
    class MyLightningCLI(LightningCLI):
        def add_arguments_to_parser(self, parser):
            ...
            parser.add_optimizer_args(
                (torch.optim.ASGD, torch.optim.SGD),
                nested_key="optim2",
                link_to="model.optim2",
            )
            parser.add_lr_scheduler_args(
...
def test_lightning_cli_load_from_checkpoint_dependency_injection_subclass_mode(
    cleandir,
):
...
def test_lightning_cli_save_hyperparameters_untyped_module(cleandir):
    ...
    with mock.patch(
        "sys.argv",
        ["any.py", f"--config={json.dumps(config)}", "--trainer.max_epochs=1"],
    ):
        cli = LightningCLI(
            BoringModel,
            run=False,
...
def test_lightning_cli_save_hyperparameters_merge(cleandir):
    ...
    with mock.patch(
        "sys.argv",
        ["any.py", "fit", f"--config={json.dumps(config)}", "--trainer.max_epochs=1"],
    ):
        cli = LightningCLI(auto_configure_optimizers=False)
    assert set(cli.model.hparams) == {
        "optimizer",
        "scheduler",
        "activation",
        "_instantiator",
    ...
    assert set(cli.datamodule.hparams) == {
        "batch_size",
        "num_workers",
        "_instantiator",
        "_class_path",
...
def test_lightning_cli_custom_subcommand():
    ...
    out = StringIO()
    with (
        mock.patch("sys.argv", ["any.py", "foo", "-h"]),
        redirect_stdout(out),
        pytest.raises(SystemExit),
    ):
        TestCLI(BoringModel, trainer_class=TestTrainer)
...
def test_lightning_cli_datamodule_short_arguments():
    ...
    with (
        mock.patch(
            "sys.argv",
            ["any.py", "fit", "--model", "BoringModel", "--data=BoringDataModule"],
        ),
        mock.patch("lightning.pytorch.Trainer._fit_impl") as run,
...
def test_optimizers_and_lr_schedulers_reload(cleandir):
    ...
    out = StringIO()
    with (
        mock.patch("sys.argv", input + ["--print_config"]),
        redirect_stdout(out),
        pytest.raises(SystemExit),
    ):
...
def test_optimizers_and_lr_schedulers_add_arguments_to_parser_implemented_reload(
    cleandir,
):
...
def test_lightning_cli_config_before_subcommand_two_configs():
    ...
    with (
        mock.patch(
            "sys.argv",
            ["any.py", f"--config={config1}", f"--config={config2}", "validate"],
        ),
        mock.patch("lightning.pytorch.Trainer.validate", autospec=True) as validate_mock,
    ):
        cli = LightningCLI(BoringModel)
...
def test_cli_configure_optimizers_warning():
    ...
    with mock.patch("sys.argv", ["any.py"]), no_warning_call(UserWarning, match=match):
        LightningCLI(BoringModel, run=False)
    with (
        mock.patch("sys.argv", ["any.py", "--optimizer=Adam"]),
        pytest.warns(UserWarning, match=match),
    ):
        LightningCLI(BoringModel, run=False)
...
def test_cli_help_message():
    ...
    with (
        mock.patch("sys.argv", cli_args),
        redirect_stdout(classpath_help),
        pytest.raises(SystemExit),
    ):
        LightningCLI(BoringModel, run=False)
    ...
    with (
        mock.patch("sys.argv", cli_args),
        redirect_stdout(shorthand_help),
        pytest.raises(SystemExit),
    ):
        LightningCLI(BoringModel, run=False)
...
def test_unresolvable_import_paths():
    ...
    out = StringIO()
    with (
        mock.patch("sys.argv", ["any.py", "--print_config"]),
        redirect_stdout(out),
        pytest.raises(SystemExit),
    ):
        LightningCLI(TestModel, run=False)
...
def test_pytorch_profiler_init_args():
    ...
    with (
        mock.patch("sys.argv", ["any.py"] + cli_args),
        mock_subclasses(Profiler, PyTorchProfiler),
    ):
        cli = LightningCLI(TestModel, run=False)
...
def test_lightning_cli_args_and_sys_argv_warning():
    with (
        mock.patch("sys.argv", ["", "--model.foo=456"]),
        pytest.warns(Warning, match="LightningCLI's args parameter "),
    ):
