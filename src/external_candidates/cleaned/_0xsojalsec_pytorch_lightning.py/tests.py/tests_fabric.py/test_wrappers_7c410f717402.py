# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\test_wrappers.py
...
def test_fabric_module_method_lookup():
    ...
    with pytest.raises(
        RuntimeError,
        match=r"You are calling the method `OriginalModule.method_with_submodule_invocation\(\)` from",
    ):
        assert fabric_module.method_with_submodule_invocation() == 101
    with pytest.raises(
        RuntimeError,
        match=r"You are calling the method `OriginalModule.method_with_self_invocation\(\)` from",
    ):
...
def test_fabric_module_mark_forward_method():
    ...
    with pytest.raises(
        AttributeError,
        match="`OriginalModule.not_exist` does not exist or is not a method.",
    ):
        fabric_module.mark_forward_method("not_exist")

    with pytest.raises(
        AttributeError,
        match="`OriginalModule.attribute` does not exist or is not a method.",
    ):
...
def test_unwrap_objects(compile):
    ...
    expected = {
        ...
        "nested": [
            module,
            wrapped_module._forward_module,
            optimizer,
            optimizer,
            dataloader,
            dataloader,
...
def test_unwrap_compiled():
    ...
    assert compiled._compile_kwargs == {
        "fullgraph": True,
        "dynamic": True,
        "disable": False,
    }
    ...
    with pytest.raises(
        RuntimeError,
        match="Failed to determine the arguments that were used to compile the module",
    ):
