# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_throughput.py
...
def test_get_available_flops(xla_available):
    ...
    with (
        pytest.warns(match="not found for 'CocoNut"),
        mock.patch("torch.cuda.get_device_name", return_value="CocoNut"),
    ):
...
def test_get_available_flops_cuda_mapping_exists(_, device_name):
    ...
    with mock.patch(
        "lightning.fabric.utilities.throughput.torch.cuda.get_device_name",
