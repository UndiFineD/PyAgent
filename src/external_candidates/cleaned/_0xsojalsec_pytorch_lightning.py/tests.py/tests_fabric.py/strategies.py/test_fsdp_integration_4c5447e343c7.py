# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_fsdp_integration.py
...
def test_save_full_state_dict(tmp_path):
    ...
    fabric.save(
        ...
        {"model": trainer.model, "optimizer": trainer.optimizer, "steps": 2},
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "norm",
            marks=pytest.mark.skip("FSDP gradient clipping by norm is not correct."),
        ),
        "val",
    ],
)
