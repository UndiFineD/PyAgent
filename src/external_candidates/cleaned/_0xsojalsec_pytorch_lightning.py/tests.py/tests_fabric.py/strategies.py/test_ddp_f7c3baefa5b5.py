# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_ddp.py
...
def test_ddp_process_group_backend(process_group_backend, device_str, expected_process_group_backend):
    ...
    strategy = MockDDPStrategy(
        process_group_backend=process_group_backend,
        root_device=torch.device(device_str),
    )
...
def test_ddp_no_backward_sync():
    ...
    with (
        pytest.raises(
            TypeError,
            match="is only possible if the module passed to .* is wrapped in `DistributedDataParallel`",
        ),
...
def test_ddp_module_state_dict():
    ...
    with mock.patch(
        "lightning.fabric.strategies.ddp.DistributedDataParallel",
        DistributedDataParallelMock,
    ):
