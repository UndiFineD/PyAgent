# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_xla_fsdp.py
...
def test_xla_fsdp_setup_optimizer_validation():
    ...
    with pytest.raises(
        ValueError,
        match="The optimizer does not seem to reference any XLAFSDP parameter",
    ):
...
def test_xla_fsdp_no_backward_sync():
    ...
    with (
        pytest.raises(
            TypeError,
            match="is only possible if the module passed to .* is wrapped in `XlaFullyShardedDataParallel`",
        ),
