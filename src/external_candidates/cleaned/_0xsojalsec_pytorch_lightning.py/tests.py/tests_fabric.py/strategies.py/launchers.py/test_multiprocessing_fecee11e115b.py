# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\launchers\test_multiprocessing.py
...
@mock.patch(
    "lightning.fabric.strategies.launchers.multiprocessing.mp.get_all_start_methods",
    return_value=[],
)
...
def test_check_for_bad_cuda_fork(mp_mock, _, start_method):
    ...
    with pytest.raises(
        RuntimeError,
        match="Lightning can't create new processes if CUDA is already initialized",
    ):
        launcher.launch(function=Mock())
