# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_fsdp.py
...
def test_hybrid_shard_configuration(sharding_strategy, monkeypatch):
    ...
    with pytest.raises(
        RuntimeError,
        match="The hybrid sharding strategy requires you to pass at least one of",
    ):
...
def test_setup_use_orig_params(_):
    ...
    with pytest.raises(
        ValueError,
        match=r"`FSDPStrategy\(use_orig_params=False\)` but this is not supported",
    ):
...
def test_no_backward_sync():
    ...
    with (
        pytest.raises(
            TypeError,
            match="is only possible if the module passed to .* is wrapped in `FullyShardedDataParallel`",
        ),
...
def test_save_checkpoint_storage_options(tmp_path):
    ...
    with pytest.raises(
        TypeError,
        match=escape("FSDPStrategy.save_checkpoint(..., storage_options=...)` is not"),
    ):
...
def test_save_checkpoint_one_fsdp_module_required(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match="Could not find a FSDP model in the provided checkpoint state.",
    ):
