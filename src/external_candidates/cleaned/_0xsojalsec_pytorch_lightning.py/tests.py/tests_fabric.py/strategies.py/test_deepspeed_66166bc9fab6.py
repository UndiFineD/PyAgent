...
def test_deepspeed_requires_joint_setup():
    ...
    with pytest.raises(
        NotImplementedError,
        match=escape("does not support setting up the module and optimizer(s) independently"),
    ):
...
def test_deepspeed_save_checkpoint_storage_options(tmp_path):
    ...
    with pytest.raises(
        TypeError,
        match=escape("DeepSpeedStrategy.save_checkpoint(..., storage_options=...)` is not"),
    ):
