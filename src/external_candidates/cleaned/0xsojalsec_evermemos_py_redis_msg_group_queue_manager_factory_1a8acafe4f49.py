# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\queue.py\redis_group_queue.py\redis_msg_group_queue_manager_factory_1a8acafe4f49.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\queue\redis_group_queue\redis_msg_group_queue_manager_factory.py
...
class RedisGroupQueueConfig:
    ...
    def get_cache_key(self) -> str:
        ...
        sort_func_name = getattr(self.sort_key_func, "__name__", "default") if self.sort_key_func else "default"
    ...
    def from_env(cls, prefix: str = "") -> "RedisGroupQueueConfig":
        ...
        key_prefix = f"{global_redis_prefix}:{base_key_prefix}" if global_redis_prefix else base_key_prefix

        # Serialization mode configuration

        serialization_mode_str = _env("REDIS_QUEUE_SERIALIZATION_MODE", "json").lower()

        serialization_mode = SerializationMode.JSON if serialization_mode_str == "json" else SerializationMode.BSON
        ...
        activity_expire_seconds = int(_env("REDIS_QUEUE_ACTIVITY_EXPIRE_SECONDS", str(24 * 3600)))
        ...
        cleanup_interval_seconds = int(_env("REDIS_QUEUE_CLEANUP_INTERVAL_SECONDS", "300"))
    ...
    def __repr__(self) -> str:
        return f"RedisGroupQueueConfig(key_prefix={self.key_prefix}, max_total_messages={self.max_total_messages})"
        ...
        async with self._lock:
            ...
            try:
                ...
                if config.serialization_mode == SerializationMode.BSON:
                    ...
                    redis_client = await self.redis_provider.get_named_client("binary_cache", decode_responses=False)
