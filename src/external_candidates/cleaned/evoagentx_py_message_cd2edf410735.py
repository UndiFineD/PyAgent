# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\core.py\message_cd2edf410735.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\core\message.py
...
class Message(BaseModule):
    ...
    def sort_by_timestamp(cls, messages: List["Message"], reverse: bool = False) -> List["Message"]:
        ...
        messages.sort(
            key=lambda msg: datetime.strptime(msg.timestamp, "%Y-%m-%d %H:%M:%S"),
            reverse=reverse,
        )
    ...
    def sort(
        ...
        key: Optional[Callable[["Message"], Any]] = None,
        reverse: bool = False,
    ) -> List["Message"]:
        """
