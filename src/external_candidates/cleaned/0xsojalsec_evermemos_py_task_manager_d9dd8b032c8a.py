# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\asynctasks.py\task_manager_d9dd8b032c8a.py
...
class TaskManager:
            ...
            for attr_name in dir(module):
                ...
                try:
                    ...
                    if isinstance(attr, TaskFunction):
                        ...
                        logger.info(f"Task found in module {module.__name__}: {attr.name}")

                except Exception as e:
                    logger.debug(f"Failed to get module attribute: {module.__name__}.{attr_name}, error: {e}")
    ...
    async def enqueue_task(
        ...
        **kwargs,
        ...
        current_retry_config = retry_config or task_function.retry_config or self._default_retry_config
        ...
        logger.info(f"Task added to queue: {task_id}, task name: {task_name}, user: {user_id_for_log}")
    ...
    async def execute_task_with_context(
        ...
        **kwargs,
        ...
        logger.info(f"Task execution completed (independent session): {task_id}, user: {user_id}")
