# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\ad_miner.py\ad_miner.py\sources.py\modules.py\objects_b3048320dd21.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\objects.py
...
class Objects:
    ...
    def genNodesDCsyncLightPage(self, neo4j):
        ...
        for k in raw_data.keys():
            ...
            raw_data[k]["target graph"]["value"] = (
                f"<i class='bi bi-shuffle {sortClass}' aria-hidden='true'></i> View paths ({len(raw_data[k]['paths'])}) <i class='bi bi-box-arrow-up-right'></i>"
            )
    ...
    def get_unpriv_users_to_GPO(self):
        ...
        def formatGPOGrid(dictOfGPO, headers):
            ...
            self.computers_with_admin_rights = [
                d["Computer Admin"].split("</i> ")[-1] for d in self.computers.computers_admin_data_grid
            ]
            ...
            for _, dict in dictOfGPO.items():
                ...
                icon = (
                    f"<span class='{interest}'></span><i class='bi bi-star-fill' style='color: {color}'></i>" * interest
                ...
                output.append(
                    {
                        ...
                        headers[1]: f'<i class="bi bi-shuffle {str(dict[headers[1]]).zfill(6)}"></i> '
                        + str(dict[headers[1]]),
                        headers[2]: {
                        ...
                        headers[4]: f'<i class="bi bi-bullseye {str(len(list(set(dict["end_list"])))).zfill(6)}"></i> '
        ...
        if not self.arguments.gpo_low:
            ...
            self.domain.unpriv_users_to_GPO_parsed = parseGPOData(self.domain.unpriv_users_to_GPO, headers)
        ...
        for _, GPO in self.domain.unpriv_users_to_GPO_parsed.items():
            ...
            entry_grid.setData(json.dumps(formatSmallGrid(list(set(GPO["entry_list"])), GPO[headers[0]])))
            ...
            end_grid.setData(json.dumps(formatSmallGrid(list(set(GPO["end_list"])), GPO[headers[0]])))
