# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\command_line.py\test_tutorial_5d4fdfaed57b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\command_line\test_tutorial.py
...
def test_tutorial_chatgpt_flow() -> None:
    ...
    with patch("concurrent.futures.ThreadPoolExecutor") as mock_executor_class:
        ...
        with patch("code_puppy.command_line.onboarding_wizard.reset_onboarding") as mock_reset:
            with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.run_oauth_flow") as mock_oauth:
                with patch("code_puppy.model_switching.set_model_and_reload_agent") as mock_set_model:
...
def test_tutorial_claude_flow() -> None:
    ...
    with patch("concurrent.futures.ThreadPoolExecutor") as mock_executor_class:
        ...
        with patch("code_puppy.command_line.onboarding_wizard.reset_onboarding") as mock_reset:
            with patch("code_puppy.plugins.claude_code_oauth.register_callbacks._perform_authentication") as mock_auth:
                with patch("code_puppy.model_switching.set_model_and_reload_agent") as mock_set_model:
                    with patch("code_puppy.command_line.core_commands.emit_info"):
                        result = handle_tutorial_command("/tutorial")
        ...
        with patch("code_puppy.command_line.onboarding_wizard.reset_onboarding") as mock_reset:
            with patch("code_puppy.plugins.chatgpt_oauth.oauth_flow.run_oauth_flow") as mock_oauth:
                with patch(
                    ...
                    with patch("code_puppy.model_switching.set_model_and_reload_agent") as mock_set_model:
                        with patch("code_puppy.command_line.core_commands.emit_info") as mock_emit_info:
                            result = handle_tutorial_command("/tutorial")
