# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-magnitude\evals\webvoyager\data\patch_tasks.py
#!/usr/bin/env python3
import argparse
import json
import sys
...
def main():
    ...
    parser.add_argument(
        "--input",
        default="originalTasks.jsonl",
        help="Input file (default: originalTasks.jsonl)",
    )
    parser.add_argument(
        "--output",
        default="patchedTasks.jsonl",
        help="Output file (default: patchedTasks.jsonl)",
    )
        ...
        for line in input_file:
            ...
            if task_id in update_patches:
                ...
                if task["ques"] == patch["prev"]:
                    ...
                    print(
                        f"Warning: Task {task_id} doesn't match expected text",
                        file=sys.stderr,
                    )
