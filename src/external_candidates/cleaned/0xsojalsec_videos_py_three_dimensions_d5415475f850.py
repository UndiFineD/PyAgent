# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2015.py\three_dimensions_d5415475f850.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2015\three_dimensions.py
...
class OldOctohedron(Mobject1D):
    def init_points(self):
        ...
        vertex_pairs += [(b[0] * x + b[1] * y, b[2] * np.sqrt(2) * z) for b in it.product(*[(-1, 1)] * 3)]

        for pair in vertex_pairs:
            self.add_points(Line(pair[0], pair[1], density=1 / self.epsilon).points)
...
class OldDodecahedron(Mobject1D):
    def init_points(self):
        ...
        five_lines_points = Mobject(
            *[Line(pair[0], pair[1], density=1.0 / self.epsilon) for pair in vertex_pairs]
        ).points
