# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_offline.py\tools.py\run_310144943bd7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\tools\run.py
...
def main(
    ...
    if run_etl_precomputed_pipeline:
        ...
        pipeline_args["run_name"] = f"etl_precomputed_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
    ...
    if run_generate_dataset_pipeline:
        ...
        pipeline_args["run_name"] = f"generate_dataset_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
    ...
    if run_compute_rag_vector_index_openai_contextual_simple_pipeline:
        ...
        pipeline_args["config_path"] = root_dir / "configs" / "compute_rag_vector_index_openai_contextual_simple.yaml"
    ...
    if run_compute_rag_vector_index_openai_contextual_pipeline:
        ...
        pipeline_args["config_path"] = root_dir / "configs" / "compute_rag_vector_index_openai_contextual.yaml"
    ...
    if run_compute_rag_vector_index_openai_parent_pipeline:
        ...
        pipeline_args["config_path"] = root_dir / "configs" / "compute_rag_vector_index_openai_parent.yaml"
