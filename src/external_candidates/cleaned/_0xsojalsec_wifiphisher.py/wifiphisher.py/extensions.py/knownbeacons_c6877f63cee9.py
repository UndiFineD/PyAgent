...
class Knownbeacons(object):
    ...
    def get_packet(self, pkt):
        ...
        if time.time() - self._starttime > constants.KB_INTERVAL:
            # Do a list shift
            self._full_pkt_list = (
                self._full_pkt_list[constants.KB_BUCKET_SIZE :] + self._full_pkt_list[: constants.KB_BUCKET_SIZE]
            )
            self._starttime = time.time()
            ...
            self._msg.append(
                "Sending %s known beacons (%s ... %s)" % (str(constants.KB_BUCKET_SIZE), first_essid, last_essid)
            )
