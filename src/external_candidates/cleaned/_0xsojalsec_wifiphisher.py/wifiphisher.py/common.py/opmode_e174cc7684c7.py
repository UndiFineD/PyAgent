# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wifiphisher\wifiphisher\common\opmode.py
"""
...
class OpMode(object):
    ...
    def initialize(self, args):
        ...
        self._perfect_card, self._use_one_phy = interfaces.is_add_vif_required(
            args.interface, args.internetinterface, args.wpspbc_assoc_interface
        )
    ...
    def _check_args(self, args):
        ...
        if args.presharedkey and (len(args.presharedkey) < 8 or len(args.presharedkey) > 64):
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] Pre-shared key must be between 8 and 63 printablecharacters."
            )
        ...
        if (
            (args.extensionsinterface and not args.apinterface) or (not args.extensionsinterface and args.apinterface)
        ) and not (args.noextensions and args.apinterface):
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] --apinterface (-aI) and --extensionsinterface (-eI)"
                "(or --noextensions (-nE)) are used in conjuction."
            )

        if args.noextensions and args.extensionsinterface:
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] --noextensions (-nE) and --extensionsinterface (-eI)"
                "cannot work together."
            )

        if args.lure10_exploit and args.noextensions:
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] --lure10-exploit (-lE) and --noextensions (-eJ)"
                "cannot work together."
            )

        if args.lure10_exploit and not os.path.isfile(constants.LOCS_DIR + args.lure10_exploit):
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] Lure10 capture does not exist. Listing directory"
        ...
        if (args.mac_ap_interface and args.no_mac_randomization) or (
            ...
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] --no-mac-randomization (-iNM) cannot work together with"
                "--mac-ap-interface or --mac-extensions-interface (-iDM)"
            )

        if args.deauth_essid and args.noextensions:
            sys.exit(
                "[" + constants.R + "-" + constants.W + "] --deauth-essid (-dE) cannot work together with"
                "--noextension (-nE)"
            )
        ...
        if args.deauth_essid and self._use_one_phy:
            print(
                (
                    "[" + constants.R + "!" + constants.W + "] Only one card was found. Wifiphisher will deauth only "
...
def validate_ap_interface(interface):
    ...
    if not (
        pyric.pyw.iswireless(interface)
        and pyric.pyw.isinterface(interface)
        and interfaces.does_have_mode(interface, "AP")
    ):
        raise argparse.ArgumentTypeError(
            "Provided interface ({}) either does not exist or does not support AP mode".format(interface)
        )
    return interface
