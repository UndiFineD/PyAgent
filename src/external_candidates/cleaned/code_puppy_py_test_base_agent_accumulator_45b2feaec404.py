# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\agents.py\test_base_agent_accumulator_45b2feaec404.py
...
class TestBaseAgentAccumulator:
    ...
    def test_message_history_accumulator_deduplication(self, agent, mock_run_context):
        ...
        text_messages = [m for m in result if hasattr(m, "parts") and any(isinstance(p, TextPart) for p in m.parts)]

        assert len(text_messages) == 1

        assert text_messages[0].parts[0].content == "Hello world"

    def test_message_history_accumulator_new_message_added(self, agent, mock_run_context):
        ...
        contents = [p.content for m in result for p in m.parts if isinstance(p, TextPart)]

        assert "First message" in contents

        assert "Second message" in contents

    def test_message_history_accumulator_filters_empty_thinking(self, agent, mock_run_context):
        ...
        valid_thinking_msg = ModelResponse(parts=[ThinkingPart(content="Valid thinking")])
        ...
        has_empty_thinking = any(
            len(m.parts) == 1 and isinstance(m.parts[0], ThinkingPart) and m.parts[0].content == "" for m in result
        )
        ...
        has_valid_thinking = any(
            len(m.parts) == 1 and isinstance(m.parts[0], ThinkingPart) and m.parts[0].content == "Valid thinking"
            for m in result
        )

        assert has_valid_thinking

        has_text = any(any(isinstance(p, TextPart) and p.content == "Real message" for p in m.parts) for m in result)

        assert has_text

    def test_message_history_accumulator_respects_compacted_hashes(self, agent, mock_run_context):
    ...
    def test_message_history_accumulator_multi_part_messages(self, agent, mock_run_context):
        """Test accumulator with multi-part messages."""

        # Create message with multiple parts

        tool_call = ToolCallPart(tool_call_id="test123", tool_name="test_tool", args={"param": "value"})
    ...
    def test_message_history_accumulator_mixed_message_types(self, agent, mock_run_context):
        ...
        has_thinking = any(any(isinstance(p, ThinkingPart) for p in m.parts) for m in result)
    ...
    def test_message_history_accumulator_calls_processor(self, mock_processor, agent, mock_run_context):
    ...
    def test_message_history_accumulator_tool_call_deduplication(self, agent, mock_run_context):
        """Test deduplication of tool call messages."""

        tool_call = ToolCallPart(tool_call_id="tool123", tool_name="test_tool", args={"input": "test_value"})
    ...
    def test_message_history_accumulator_only_empty_thinking_filtered(self, agent, mock_run_context):
        ...
        agent.set_message_history([text_empty, thinking_empty, thinking_content, multi_with_thinking])
        ...
        has_empty_text = any(any(isinstance(p, TextPart) and p.content == "" for p in m.parts) for m in result)
        ...
        has_thinking_content = any(
            any(isinstance(p, ThinkingPart) and p.content == "Some thoughts" for p in m.parts) for m in result
        )
