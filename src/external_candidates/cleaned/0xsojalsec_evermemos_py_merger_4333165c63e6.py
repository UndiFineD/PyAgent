# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\profile_memory.py\merger_4333165c63e6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\profile_memory\merger.py
...
class ProfileMemoryMerger:
    ...
    def _truncate_evidences(evidences: Iterable[Any]) -> List[str]:
        ...
        for idx, entry in enumerate(normalized):
            ...
            records.append({"entry": entry, "index": idx, "has_date": has_date, "date": date_value})
    ...
    def _profile_memory_to_prompt_dict(cls, profile: ProfileMemory) -> Dict[str, Any]:
        ...
        return {
            ...
            "projects_participated": [project_to_dict(project) for project in profile.projects_participated or []],
        }

    async def merge_group_profiles(self, group_profiles: List[ProfileMemory], user_id: str) -> ProfileMemory:
        """
        ...
        for profile in group_profiles:
            if profile is not None and profile.user_id == user_id:
                ...
                if profile.group_importance_evidence is None or profile.group_importance_evidence.is_important is True:
        ...
        merged_hard_skills = merge_skill_lists_keep_highest_level(*[p.hard_skills for p in matching_profiles])

        merged_soft_skills = merge_skill_lists_keep_highest_level(*[p.soft_skills for p in matching_profiles])
        ...
        for profile in all_matching_profiles:
            merged_projects = merge_projects_participated(merged_projects, profile.projects_participated)
