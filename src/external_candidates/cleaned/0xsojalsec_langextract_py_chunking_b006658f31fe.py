# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\chunking_b006658f31fe.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\chunking.py
...
from langextract.core import data, exceptions, tokenizer
...
class TextChunk:
    ...
    _char_interval: data.CharInterval | None = dataclasses.field(default=None, init=False, repr=False)

    def __str__(self):
        interval_repr = f"start_index: {self.token_interval.start_index}, end_index: {self.token_interval.end_index}"

        doc_id_repr = f"Document ID: {self.document_id}" if self.document_id else "Document ID: None"
...
def create_token_interval(start_index: int, end_index: int) -> tokenizer.TokenInterval:
...
def get_token_interval_text(
    ...
    if token_interval.start_index >= token_interval.end_index:
        raise ValueError(f"Start index {token_interval.start_index} must be < end index {token_interval.end_index}.")
...
class SentenceIterator:
    ...
    def __init__(
        ...
        if curr_token_pos < 0:
            ...
            raise IndexError(
                f"Current token position {curr_token_pos} is past the length of the document {self.token_len}."
            )
...
class ChunkIterator:
    ...
    * "And so are you." len=15
    ...
    def _tokens_exceed_buffer(self, token_interval: tokenizer.TokenInterval) -> bool:
    ...
    def __next__(self) -> TextChunk:
        ...
        if self._tokens_exceed_buffer(curr_chunk):
            self.sentence_iter = SentenceIterator(self.tokenized_text, curr_token_pos=sentence.start_index + 1)
        ...
        for token_index in range(curr_chunk.start_index, sentence.end_index):
            ...
            if self._tokens_exceed_buffer(test_chunk):
                ...
                self.sentence_iter = SentenceIterator(self.tokenized_text, curr_token_pos=curr_chunk.end_index)
