# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\search.py\milvus.py\converter.py\foresight_milvus_converter_2919d98c747d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\search\milvus\converter\foresight_milvus_converter.py
...
class ForesightMilvusConverter(BaseMilvusConverter[ForesightCollection]):
    ...
    def _parse_time_field(cls, time_value, field_name: str, doc_id) -> int:
        ...
        try:
            ...
            logger.warning(f"Failed to parse {field_name} (doc_id={doc_id}): {time_value}, error: {e}")
    ...
    def from_mongo(cls, source_doc: MongoForesightRecord) -> Dict[str, Any]:
        ...
        try:
            ...
            milvus_entity = {
                ...
                # Audit fields
                "created_at": (int(source_doc.created_at.timestamp()) if source_doc.created_at else 0),
                "updated_at": (int(source_doc.updated_at.timestamp()) if source_doc.updated_at else 0),
                # Vector field
            ...
            logger.error("Failed to convert MongoDB foresight document to Milvus entity: %s", e)
