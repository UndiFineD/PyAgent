# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\document.py\memory.py\event_log_record_448a1ccf82a1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\persistence\document\memory\event_log_record.py
...
class EventLogRecord(DocumentBase, AuditBase):
    ...
    user_id: Optional[str] = Field(default=None, description="User ID, required for personal events")
    ...
    participants: Optional[List[str]] = Field(default=None, description="Related participants")

    # Vector and model

    vector: Optional[List[float]] = Field(default=None, description="Atomic fact vector")

    vector_model: Optional[str] = Field(default=None, description="Vectorization model used")

    # Event type and extension information

    event_type: Optional[str] = Field(default=None, description="Event type, such as Conversation")

    extend: Optional[Dict[str, Any]] = Field(default=None, description="Extension field")
