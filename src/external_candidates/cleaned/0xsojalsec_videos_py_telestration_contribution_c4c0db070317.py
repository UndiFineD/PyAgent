# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2020.py\telestration_contribution_c4c0db070317.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\telestration_contribution.py
...
class TelestrationContribution(Scene):
    def construct(self):
        ...
        self.play(FadeOut(q_marks, lag_ratio=0.1), FadeInFromDown(beer))

        self.play(Restore(beer), FadeIn(arrow, 0.2 * LEFT), DrawBorderThenFill(muscle))
        ...
        self.play(randy.change, "angry", imply_group, Write(bangs, run_time=1))
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOutAndShift,
                VGroup(randy, bangs, arrow, muscle),
                lambda m: (m, DOWN),
            ),
            Uncreate(lines),
