# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\plugins.py\antigravity_oauth.py\register_callbacks_feb6a0f09e30.py
...
def _await_callback(context: Any) -> Optional[Tuple[str, str, str]]:
    ...
    try:
        ...
        if should_suppress_browser():
            emit_info("[HEADLESS MODE] Would normally open browser for Antigravity OAuth…")
...
def _perform_authentication(
    ...
    if add_models_to_config(result.access_token, result.project_id):
        ...
        emit_info("   Use the `antigravity-` prefix (e.g., antigravity-gemini-3-pro-high)")
...
def _handle_status() -> None:
    ...
    if status.error:
        ...
        current = tier_display.get(status.current_tier, status.current_tier or "Unknown")
    ...
    antigravity_models = [name for name, cfg in models.items() if cfg.get("oauth_source") == "antigravity-plugin"]
...
def _handle_custom_command(command: str, name: str) -> Optional[bool]:
    ...
    if name == "antigravity-auth":
        ...
        if tokens and tokens.get("access_token"):
            emit_warning("Existing tokens found. This will refresh your authentication.")
...
def _create_antigravity_model(model_name: str, model_config: Dict, config: Dict) -> Any:
    ...
    if not api_key:
        emit_warning(f"API key is not set for Antigravity endpoint; skipping model '{model_config.get('name')}'.")
    ...
    if is_token_expired(expires_at):
        ...
        if new_tokens:
            ...
            emit_warning("Failed to refresh Antigravity token; run /antigravity-auth again.")
