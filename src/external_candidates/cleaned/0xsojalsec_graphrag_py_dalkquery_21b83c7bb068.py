# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\query.py\dalkquery_21b83c7bb068.py
...
class DalkQuery(BaseQuery):
    ...
    async def _select_most_relative_paths(self, path_list, query):
        ...
        path_str = [
            (path[0]["src_id"] + ("->" + e["content"] + "-> " + e["tgt_id"]) for edge in path) for path in path_list
        ]
    ...
    async def _select_most_relative_neis(self, nei_list, query):
        ...
        nei_str_list = ["->".join([e["src_id"], e["content"], e["tgt_id"]]) for e in nei_list]
