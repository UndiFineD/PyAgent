# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\compat.py\inference_c7609941379b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\_compat\inference.py
...
def __getattr__(name: str):
    ...
    if name in moved:
        ...
        warnings.warn(
            f"`langextract.inference.{name}` is deprecated and will be removed in v2.0.0; use `{mod}.{attr}` instead.",
            FutureWarning,
            stacklevel=2,
        )
