# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\app_utils.py\sections.py\histogram_card_9ed326be404b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\app_utils\sections\histogram_card.py
...
def histogram_card(
    ...
    assert " " not in x_axis_description, (
        "x_axis_description in histogram card must not contain spaces, as the card would not be rendered."
    )
...
def compute_quantile_df(x: list[int], a: float, b: float):
    ...
    if not 0.05 <= a <= b <= 0.95:
        raise ValueError("Values of a and b must be in [0.05, 0.95] and a should be less than or equal to b")
    ...
    middle_quantile_max = min(len(df_quantile), (len(df_quantile) - len(df_last) - 1)) + 1
