...
class WMIEXEC_EVENT:
    ...
    def execute_handler(self, command):
        ...
        self.logger.debug(
            f"{self.__target}: Execute command via wmi event, job instance id: {self.__instanceID}, command result instance id: {self.__instanceID_StoreResult}"
        )
    ...
    def execute_vbs(self, vbs_content):
        ...
        sys.stdout = current
        self.check_error(
            f'Adding ActiveScriptEventConsumer.Name="{self.__instanceID}"',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )
        ...
        sys.stdout = current
        self.check_error(
            f'Adding IntervalTimerInstruction.TimerId="{self.__instanceID}"',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )
        ...
        sys.stdout = current
        self.check_error(
            f'Adding EventFilter.Name={self.__instanceID}"',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )
        ...
        sys.stdout = current
        self.check_error(
            rf'Adding FilterToConsumerBinding.Consumer="ActiveScriptEventConsumer.Name=\"{self.__instanceID}\"", Filter="__EventFilter.Name=\"{self.__instanceID}\""',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )

    def get_command_result(self):
        try:
            command_result_object, _ = self.__iWbemServices.GetObject(
                f'ActiveScriptEventConsumer.Name="{self.__instanceID_StoreResult}"'
            )
            ...
            self.logger.fail(
                "WMIEXEC-EVENT: Could not retrieve output file, it may have been detected by AV. Please try increasing the timeout with the '--exec-timeout' option. If it is still failing, try the 'smb' protocol or another exec method"
            )

    def remove_instance(self):
        if self.__retOutput:
            resp = self.__iWbemServices.DeleteInstance(
                f'ActiveScriptEventConsumer.Name="{self.__instanceID_StoreResult}"'
            )
            self.check_error(
                f'Removing ActiveScriptEventConsumer.Name="{self.__instanceID}"',
                resp.GetCallStatus(0) & 0xFFFFFFFF,
            )
        ...
        self.check_error(
            f'Removing IntervalTimerInstruction.TimerId="{self.__instanceID}"',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )

        resp = self.__iWbemServices.DeleteInstance(f'__EventFilter.Name="{self.__instanceID}"')
        self.check_error(
            f'Removing EventFilter.Name="{self.__instanceID}"',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )

        resp = self.__iWbemServices.DeleteInstance(
            rf'__FilterToConsumerBinding.Consumer="ActiveScriptEventConsumer.Name=\"{self.__instanceID}\"",Filter="__EventFilter.Name=\"{self.__instanceID}\""'
        )
        self.check_error(
            rf'Removing FilterToConsumerBinding.Consumer="ActiveScriptEventConsumer.Name=\"{self.__instanceID}\"", Filter="__EventFilter.Name=\"{self.__instanceID}\""',
            resp.GetCallStatus(0) & 0xFFFFFFFF,
        )
