...
class rdp(connection):
    ...
    def print_host_info(self):
        nla = (
            colored(f"nla:{self.nla}", host_info_colors[3], attrs=["bold"])
            if self.nla
            else colored(f"nla:{self.nla}", host_info_colors[2], attrs=["bold"])
        )
    ...
    def check_nla(self):
        ...
        for proto in self.protoflags_nla:
            try:
                ...
                asyncio.run(self.connect_rdp())
                if (
                    proto.value == SUPP_PROTOCOLS.RDP
                    or proto.value == SUPP_PROTOCOLS.SSL
                    or proto.value == SUPP_PROTOCOLS.SSL | SUPP_PROTOCOLS.RDP
                ):
                    self.nla = False
                    return
            except Exception:
        ...
        try:
            ...
            if "KDC_ERR" in str(e):
                ...
                self.logger.fail(
                    ...
                    color=(
                        "magenta"
                        if ((reason or "CredSSP" in str(e)) and reason != "KDC_ERR_C_PRINCIPAL_UNKNOWN")
                        else "red"
                    ),
                )
                ...
                self.logger.fail(
                    ...
                    color=(
                        "magenta" if ((reason or "CredSSP" in str(e)) and reason != "STATUS_LOGON_FAILURE") else "red"
                    ),
                )
            return False
