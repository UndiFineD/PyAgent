...
from dploot.triage.browser import BrowserTriage, Cookie, GoogleRefreshToken, LoginData
from dploot.triage.credentials import CredentialsTriage
from dploot.triage.sccm import SCCMCollection, SCCMCred, SCCMSecret, SCCMTriage
from dploot.triage.vaults import VaultsTriage
from impacket.dcerpc.v5 import lsad, lsat, rrp, scmr, srvs, transport
...
from impacket.examples.regsecrets import LSASecrets as RegSecretsLSASecrets
from impacket.examples.regsecrets import RemoteOperations as RegSecretsRemoteOperations
from impacket.examples.regsecrets import SAMHashes as RegSecretsSAMHashes
...
from impacket.smb3structs import FILE_SHARE_DELETE, FILE_SHARE_WRITE
from impacket.smbconnection import SessionError, SMBConnection
from nxc.config import host_info_colors, process_secret
from nxc.connection import connection, dcom_FirewallChecker, requires_admin, sem
...
class smb(connection):
    ...
    def enum_host_info(self):
        ...
        try:
            self.signing = (
                self.conn.isSigningRequired() if self.smbv1 else self.conn._SMBConnection._Connection["RequireSigning"]
            )
        except Exception as e:
            self.logger.debug(e)

        self.os_arch = self.get_os_arch()
        self.output_file_template = os.path.expanduser(
            f"{NXC_PATH}/logs/{{output_folder}}/{self.hostname}_{self.host}_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}".replace(
                ":", "-"
            )
        )
    ...
    def print_host_info(self):
        ...
        self.logger.display(
            f"{self.server_os}{f' x{self.os_arch}' if self.os_arch else ''} (name:{self.hostname}) (domain:{self.targetDomain}) ({signing}) ({smbv1}) {ntlm}"
        )
...
"""
        ...
        try:
            ...
            if self.args.delegate:
                ...
                serverName = Principal(
                    f"cifs/{self.hostname}",
                    type=constants.PrincipalNameType.NT_SRV_INST.value,
                )
                tgs = kerberos_login_with_S4U(
                    domain,
                    self.hostname,
                    username,
                    password,
            ...
            with TSTS.TermSrvSession(self.conn, self.host, self.kerberos) as TermSrvSession:
                for SessionId in sessions:
                    ...
                    sessflags = TSTS.enum2value(
                        TSTS.SESSIONFLAGS,
                        sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"]["SessionFlags"],
                    )
                    ...
                    sessions[SessionId]["ConnectTime"] = sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"][
                        "ConnectTime"
                    ]
                    sessions[SessionId]["DisconnectTime"] = sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"][
                        "DisconnectTime"
                    ]
                    sessions[SessionId]["LogonTime"] = sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"][
                        "LogonTime"
                    ]
                    sessions[SessionId]["LastInputTime"] = sessdata["LSMSessionInfoExPtr"]["LSM_SessionInfo_Level1"][
                        "LastInputTime"
                    ]
    ...
    def interfaces(self):
        ...
        try:
            ...
            if remoteOps._RemoteOperations__rrp:
                ...
                sub_keys = [
                    rrp.hBaseRegEnumKey(remoteOps._RemoteOperations__rrp, key_handle, i)["lpNameOut"][:-1]
                    for i in range(sub_key_list)
                ]

                self.logger.highlight(
                    f"{'-Name-':<11} | {'-IP Address-':<15} | {'-SubnetMask-':<15} | {'-Gateway-':<15} | -DHCP-"
                )
                for sub_key in sub_keys:
                    interface = {}
                    try:
                        interface_key = f"SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters\\Interfaces\\{sub_key}"
                        interface_handle = rrp.hBaseRegOpenKey(
                            remoteOps._RemoteOperations__rrp, reg_handle, interface_key
                        )["phkResult"]
                        ...
                        interface_name_handle = rrp.hBaseRegOpenKey(
                            remoteOps._RemoteOperations__rrp,
                            reg_handle,
                            interface_name_key,
                        )["phkResult"]
                        interface_name = rrp.hBaseRegQueryValue(
                            remoteOps._RemoteOperations__rrp,
                            interface_name_handle,
                            "Name",
                        )[1].rstrip("\x00")
                        ...
                        # Retrieve DHCP
                        try:
                            dhcp_enabled = rrp.hBaseRegQueryValue(
                                remoteOps._RemoteOperations__rrp,
                                interface_handle,
                                "EnableDHCP",
                            )[1]
                        ...
                        try:
                            ip_address = (
                                rrp.hBaseRegQueryValue(
                                    remoteOps._RemoteOperations__rrp,
                                    interface_handle,
                                    "DhcpIPAddress" if dhcp_enabled else "IPAddress",
                                )[1]
                                .rstrip("\x00")
                                .replace("\x00", ", ")
                            )
                        ...
                        self.logger.highlight(
                            f"{interface['Name']:<11} | {interface['IPAddress']!s:<15} | {interface['SubnetMask']!s:<15} | {interface['DefaultGateway']!s:<15} | {interface['DHCP']}"
                        )
    ...
    def rid_brute(self, max_rid=None):
        ...
        for _j in range(max_rid // simultaneous + 1):
            sids_to_check = (
                (max_rid - so_far) % simultaneous if (max_rid - so_far) // simultaneous == 0 else simultaneous
            )
    ...
    def dpapi(self):
        ...
        def firefox_callback(secret):
            ...
            if isinstance(secret, FirefoxData):
                ...
                line = (
                    f"[{secret.winuser}][{tag}] {secret.host}{secret.path} {secret.cookie_name}:{secret.cookie_value}"
                )
                self.logger.highlight(line)
                if self.output_file:
                    self.output_file.write(line + "\n")
