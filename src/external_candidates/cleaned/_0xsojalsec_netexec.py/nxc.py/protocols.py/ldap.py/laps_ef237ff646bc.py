...
class LDAPConnect:
    ...
    def kerberos_login(
        ...
        try:
            ...
            ldap_connection = ldap_impacket.LDAPConnection(
                f"ldap://{kdcHost}", baseDN, dns_server if dns_server else domain
            )
            ...
            if str(e).find("strongerAuthRequired") >= 0:
                ...
                try:
                    ldap_connection = ldap_impacket.LDAPConnection(
                        f"ldaps://{kdcHost}",
                        baseDN,
                        dns_server if dns_server else domain,
                    )
    ...
    def auth_login(self, domain, username, password, ntlm_hash, dns_server):
        ...
        try:
            ldap_connection = ldap_impacket.LDAPConnection(
                f"ldap://{domain}", base_dn, dns_server if dns_server else domain
            )
    ...
    def run(self):
        ...
        if key_id["RootKeyId"] in kds_cache:
            ...
            string_binding = hept_map(
                destHost=self.domain if not self.dns_server else self.dns_server,
        ...
        enc_content_parameter = bytes(
            parsed_enveloped_data["encryptedContentInfo"]["contentEncryptionAlgorithm"]["parameters"]
        )
