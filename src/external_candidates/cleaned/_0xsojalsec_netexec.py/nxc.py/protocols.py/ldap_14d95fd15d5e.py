...
class ldap(connection):
    ...
    def check_if_admin(self):
        ...
        if (
            resp
            and (self.password != "" or self.lmhash != "" or self.nthash != "" or self.aesKey != "")
            and self.username != ""
        ):
            ...
            for user in resp_parsed:
                ...
                if pwd_last_set:
                    pwd_last_set = (
                        "<never>"
                        if pwd_last_set == "0"
                        else datetime.fromtimestamp(self.getUnixTime(int(pwd_last_set))).strftime("%Y-%m-%d %H:%M:%S")
                    )
    ...
    def active_users(self):
        ...
        if resp:
            ...
            active_users = [
                ...
                if not (int(user.get("userAccountControl", UF_ACCOUNTDISABLE)) & UF_ACCOUNTDISABLE)
            ]
                ...
                if TGT:
                    ...
                    try:
                        ...
                        pwdLastSet = (
                            ...
                            else str(datetime.fromtimestamp(self.getUnixTime(int(user["pwdLastSet"]))))
                        )
                        lastLogon = (
                            "<never>"
                            if str(user.get("lastLogon", 0)) == "0"
                            else str(datetime.fromtimestamp(self.getUnixTime(int(user["lastLogon"]))))
                        )
                        self.logger.display(
        ...
        if gmsa_accounts_parsed:
            ...
            for acc in gmsa_accounts_parsed:
                ...
                if "msDS-GroupMSAMembership" in acc:
                    ...
                    sids = [
                        ace["Ace"]["Sid"].formatCanonical() for ace in dacl["Dacl"]["Data"] if ace["AceType"] == 0x00
                    ]
        ...
        if self.args.gmsa_convert_id:
            if len(self.args.gmsa_convert_id) != 64:
                ...
                if gmsa_accounts_parsed:
                    ...
                    for acc in gmsa_accounts_parsed:
                        if (
                            self.decipher_gmsa_name(self.domain.split(".")[0], acc["sAMAccountName"][:-1])
