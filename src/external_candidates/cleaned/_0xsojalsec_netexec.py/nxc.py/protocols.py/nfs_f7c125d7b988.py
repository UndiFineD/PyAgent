...
class nfs(connection):
                ...
                for entry in entries:
                    if "name" in entry and entry["name"] not in [b".", b".."]:
                        ...
                        if entry.get("name_attributes", {}).get("present", False):
                            if (
                                entry["name_attributes"]["attributes"]["type"] == 2 and recurse > 0
    ...
    def get_permissions(self, file_handle):
        ...
        try:
            read_perm = (
                self.nfs3.access(file_handle, ACCESS3_READ, self.auth).get("resok", {}).get("access", 0) is ACCESS3_READ
            )
        except Exception:
            read_perm = False
        try:
            write_perm = (
                self.nfs3.access(file_handle, ACCESS3_MODIFY, self.auth).get("resok", {}).get("access", 0)
                is ACCESS3_MODIFY
            )
        except Exception:
            write_perm = False
        try:
            exec_perm = (
                self.nfs3.access(file_handle, ACCESS3_EXECUTE, self.auth).get("resok", {}).get("access", 0)
                is ACCESS3_EXECUTE
            )
        except Exception:
            exec_perm = False
