# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-NetExec\nxc\modules\spider_plus.py
import errno
import json
import time
from os import makedirs, remove, stat
from os.path import abspath, exists, getsize, join, sep, split, splitext
...
class SMBSpiderPlus:
    ...
    def parse_file(self, share_name, file_path, file_info):
        ...
        if file_size > self.max_file_size:
            self.logger.info(
                f"File {file_path} has size {human_size(file_size)} > max size {human_size(self.max_file_size)}."
            )
            self.stats["num_files_filtered"] += 1
            return
    ...
    def print_stats(self):
        ...
        if shares_readable:
            ...
            shares_readable_str = (
                ", ".join(shares_readable[:10]) + "..." if len(shares_readable) > 10 else ", ".join(shares_readable)
            )
        ...
        if shares_writable:
            ...
            shares_writable_str = (
                ", ".join(shares_writable[:10]) + "..." if len(shares_writable) > 10 else ", ".join(shares_writable)
            )
