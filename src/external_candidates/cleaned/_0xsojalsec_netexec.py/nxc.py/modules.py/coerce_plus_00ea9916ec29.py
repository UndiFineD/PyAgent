...
class PrinterBugTrigger:
    ...
    def get_dynamic_endpoint(self, interface: bytes, target: str, timeout: int = 5) -> str:
        ...
        dce = rpctransport.get_dce_rpc()
        self.context.log.debug("Trying to resolve dynamic endpoint %s" % repr(uuid.bin_to_string(interface)))
        try:
            dce.connect()
        except Exception as e:
        ...
        try:
            ...
            self.context.log.debug(f"Resolved dynamic endpoint {uuid.bin_to_string(interface)!r} to {endpoint!r}")
            return endpoint
        except Exception as e:
            self.context.log.debug("Failed to resolve dynamic endpoint %s" % repr(uuid.bin_to_string(interface)))
            raise e
