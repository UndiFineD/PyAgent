...
class NXCModule:
    ...
    def on_login(self, context, connection):
        ...
        for item in list_sites:
            ...
            if "dnsRecord" in site:
                ...
                if RECORD_TYPE_MAPPING[dr["Type"]] == "A":
                    ...
                    if dr["Type"] in [
                        a for a in RECORD_TYPE_MAPPING if RECORD_TYPE_MAPPING[a] in ["CNAME", "NS", "PTR"]
                    ]:
        ...
        if not self.showall and not self.showhosts:
            context.log.display(
                f"To extract CIDR from the {len(outdata)} ip, run  the following command: cat your_file | mapcidr -aa -silent | mapcidr -a -silent"
            )
