    ...
    def on_admin_login(self, context, connection):
        ...
        for user in users:
            ...
            for path in self.custom_user_path:
                ...
                self.dig_confCons_in_files(
                    conn=dploot_conn,
                    directory_path=user_path,
                    recurse_level=0,
                    recurse_max=self.recurse_max,
                )
    ...
    def extract_remoteng_passwords(self, encrypted_password, encryption_attributes: MRemoteNgEncryptionAttributes):
        ...
        if encryption_attributes.encryption_engine == "AES":
            ...
            key = hashlib.pbkdf2_hmac(
                ...
                salt,
                encryption_attributes.kdf_iterations,
                dklen=32,
            )
            if encryption_attributes.block_cipher_mode == "GCM":
                ...
                self.context.log.debug(
                    f"Could not decrypt MRemoteNG password with encryption algorithm {encryption_attributes.encryption_engine}-{encryption_attributes.block_cipher_mode}: Not yet implemented"
                )
            cipher.update(associated_data)
            return cipher.decrypt_and_verify(ciphertext, tag).decode("latin-1")
        else:
            self.context.log.debug(
                f"Could not decrypt MRemoteNG password with encryption algorithm {encryption_attributes.encryption_engine}: Not yet implemented"
            )
