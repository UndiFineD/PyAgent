...
class NXCModule:
    ...
    def reg_query_value(self, remote_ops, path, key, hku=False):
        if remote_ops._RemoteOperations__rrp:
            ans = (
                rrp.hOpenUsers(remote_ops._RemoteOperations__rrp)
                if hku
                else rrp.hOpenLocalMachine(remote_ops._RemoteOperations__rrp)
            )
    ...
    def vnc_client_proxyconf_extract(self, dploot_conn, remote_ops=None):
        vnc_client_softwares = [
            (
                "RealVNC Viewer 7.x Proxy Conf",
                "Software\\RealVNC\\vncviewer",
                ["ProxyUserName", "ProxyPassword", "ProxyServer"],
            ),
        ]
            ...
            for vnc_name, registry_path, registry_keys in vnc_client_softwares:
                ...
                if ntuser_dat_bytes is None and remote_ops is not None:
                    ...
                    try:
                        ...
                        cred["server"] = self.reg_query_value(
                            remote_ops, user_registry_path, registry_keys[2], hku=True
                        )
                        ...
                        if "ERROR_FILE_NOT_FOUND" not in str(e):
                            self.context.log.debug(
                                f"Error while RegQueryValues {registry_keys} from {user_registry_path}: {e}"
                            )
                        continue
                else:
                    ...
                    password = (
                        reg.getValue(ntpath.join(registry_path, registry_keys[1]))[1]
                        .decode("utf-16le")
                        .rstrip("\0")
                        .encode()
                    )
