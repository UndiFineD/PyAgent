# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\env.py\instance_ddc4c072d61b.py
...
class GameControl:
    ...
    def unpause(self):
        ...
        if self._is_paused:
            ...
            self._render_pause_message(f"[{timestamp}] Game unpaused (speed: {self._speed}x)")
    ...
    def get_elapsed_ticks(self):
        response = self.rcon_client.send_command("/sc rcon.print(global.elapsed_ticks or 0)")
...
class FactorioInstance:
    ...
    def connect_to_server(address, tcp_port):
        try:
            rcon_client = RCONClient(address, tcp_port, RCON_PASSWORD)  #'quai2eeha3Lae7v')
