# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videorag.py\videorag_algorithm.py\videorag.py\videoutil.py\split_0e4a228271f6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoRAG\VideoRAG-algorithm\videorag\_videoutil\split.py
    ...
    with VideoFileClip(video_path) as video:
        ...
        for start in tqdm(start_times, desc=f"Spliting Video {video_name}"):
            ...
            try:
                ...
                logger.warning(
                    f"Warning: Failed to extract audio for video {video_name} ({start}-{end}). Probably due to lack of audio track."
                )
