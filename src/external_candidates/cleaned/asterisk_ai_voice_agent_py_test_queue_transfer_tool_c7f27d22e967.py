# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\tests.py\tools.py\telephony.py\test_queue_transfer_tool_c7f27d22e967.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\tests\tools\telephony\test_queue_transfer_tool.py
...
class TestTransferToQueueTool:
    ...
    @pytest.mark.asyncio
    async def test_transfer_to_sales_queue(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
        """Test transfer to sales queue."""

        result = await queue_tool.execute(parameters={"queue": "sales"}, context=tool_context)
        ...
        mock_ari_client.send_command.assert_called_once_with(
            ...
            params={"context": "ext-queues", "extension": "sales-queue", "priority": 1},
        )

    @pytest.mark.asyncio
    async def test_transfer_to_support_queue(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
        """Test transfer to support queue."""

        result = await queue_tool.execute(parameters={"queue": "support"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_transfer_to_billing_queue(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
        """Test transfer to billing queue."""

        result = await queue_tool.execute(parameters={"queue": "billing"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_queue_name_case_insensitive(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
        """Test queue resolution is case-insensitive."""

        # Test different cases

        for queue_name in ["Sales", "SALES", "sAlEs"]:
            result = await queue_tool.execute(parameters={"queue": queue_name}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_queue_name_with_whitespace(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
    ...
    @pytest.mark.asyncio
    async def test_invalid_queue_name(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
        """Test error handling for invalid queue names."""

        result = await queue_tool.execute(parameters={"queue": "nonexistent"}, context=tool_context)
    ...
    @pytest.mark.asyncio
    async def test_tool_not_configured(self, queue_tool, tool_context, mock_ari_client):
    ...
    @pytest.mark.asyncio
    async def test_tool_disabled(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
    ...
    @pytest.mark.asyncio
    async def test_session_state_updated(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
    ...
    def test_format_queue_message_basic(self, queue_tool):
        ...
        message = queue_tool._format_queue_message(description="Sales team", queue_status={}, max_wait_time=300)
    ...
    def test_format_queue_message_with_position(self, queue_tool):
        ...
        message = queue_tool._format_queue_message(
            description="Sales team", queue_status={"position": 3}, max_wait_time=300
        )
    ...
    def test_format_queue_message_with_position_and_wait(self, queue_tool):
        ...
        message = queue_tool._format_queue_message(
            description="Sales team",
            queue_status={"position": 5, "estimated_wait_seconds": 300},
            max_wait_time=600,
        )
    ...
    def test_resolve_queue_returns_config(self, queue_tool, tool_context, queue_enabled_config):
    ...
    def test_resolve_queue_returns_none_for_invalid(self, queue_tool, tool_context, queue_enabled_config):
    ...
    @pytest.mark.asyncio
    async def test_return_includes_all_fields(self, queue_tool, tool_context, mock_ari_client, queue_enabled_config):
