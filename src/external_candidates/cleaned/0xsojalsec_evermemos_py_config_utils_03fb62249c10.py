# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\milvus.py\config_utils_03fb62249c10.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\milvus\config_utils.py
...
def get_tenant_milvus_config() -> Optional[Dict[str, Any]]:
    ...
    if not tenant_info:
        logger.debug("⚠️ Tenant context not set, unable to retrieve tenant Milvus configuration")
    ...
    final_config = {
        "host": milvus_config.get("host") or env_fallback_config.get("host", "localhost"),
        ...
        "password": milvus_config.get("password") or env_fallback_config.get("password", ""),
    }
