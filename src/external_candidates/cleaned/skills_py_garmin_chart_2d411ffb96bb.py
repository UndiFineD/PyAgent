# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\eversonl.py\garmin_health_analysis.py\scripts.py\garmin_chart_2d411ffb96bb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\eversonl\garmin-health-analysis\scripts\garmin_chart.py
...
def create_hrv_chart(hrv_data, hr_data):
    ...
    return {
        "stats": {"Avg HRV": f"{avg_hrv:.0f} ms", "Avg RHR": f"{avg_rhr:.0f} bpm"},
        "chart": {
...
def main():
    ...
    parser.add_argument(
        "chart",
        choices=["sleep", "body_battery", "hrv", "activities", "dashboard"],
    ...
    if not client:
        print(
            "❌ Not authenticated. Run: python3 scripts/garmin_auth.py login",
            file=sys.stderr,
        )
