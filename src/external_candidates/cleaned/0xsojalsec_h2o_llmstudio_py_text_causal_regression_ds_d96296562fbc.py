# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\src.py\datasets.py\text_causal_regression_ds_d96296562fbc.py
...
class CustomDataset(TextCausalLanguageModelingCustomDataset):
    def __init__(self, df: pd.DataFrame, cfg: Any, mode: str = "train"):
        ...
        if cfg.dataset.parent_id_column != "None":
            raise LLMDataException("Parent ID column is not supported for regression datasets.")
    ...
    def postprocess_output(self, cfg, df: pd.DataFrame, output: dict) -> dict:
        ...
        for col in np.arange(len(cfg.dataset.answer_column)):
            preds.append(np.round(output["predictions"][:, col].cpu().numpy(), 3).astype(str))
