# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\relationship_raw_repository_4d42a49a4ef6.py
...
class RelationshipRawRepository(BaseRepository[Relationship]):
    ...
    async def get_by_source_entity(
        ...
        try:
            results = (
                await self.model.find({"source_entity_id": source_entity_id}, session=session).limit(limit).to_list()
            )
            ...
            logger.error("❌ Failed to retrieve relationships by source entity ID: %s", e)
    ...
    async def get_by_target_entity(
        ...
        try:
            results = (
                await self.model.find({"target_entity_id": target_entity_id}, session=session).limit(limit).to_list()
            )
            ...
            logger.error("❌ Failed to retrieve relationships by target entity ID: %s", e)
    ...
    async def count_by_entity(self, entity_id: str, session: Optional[AsyncClientSession] = None) -> int:
