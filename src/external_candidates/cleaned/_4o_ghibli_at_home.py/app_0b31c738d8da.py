# Extracted from: C:\DEV\PyAgent\.external\4o-ghibli-at-home\app.py
#
...
try:
    ...
    TORCH_DTYPE = torch.bfloat16 if DEVICE == "cuda" else torch.float32  # bfloat16 not supported on CPU for all ops

    if not torch.cuda.is_available():
        logger.warning("CUDA not available. Running on CPU, which will be extremely slow.")
...
def parse_request_args(form_data, image_file):
    ...
    if not image_file or not is_allowed_file(image_file.filename):
        raise ValueError("Invalid or no image file provided. Allowed types: " + ", ".join(Config.ALLOWED_EXTENSIONS))
    ...
    try:
        args = {
            ...
            "num_inference_steps": int(form_data.get("num_inference_steps", Config.DEFAULT_STEPS)),
            "guidance_scale": float(form_data.get("guidance_scale", Config.DEFAULT_GUIDANCE_SCALE)),
            "true_cfg_scale": float(form_data.get("true_cfg_scale", Config.DEFAULT_TRUE_CFG_SCALE)),
                ...
                for job_id in expired_jobs:
                    ...
                    if "result_path" in job_to_delete:
                        try:
                            ...
                            logger.debug(f"Deleted result file: {job_to_delete['result_path']}")
                        except OSError as e:
                            logger.warning(f"Could not delete result file {job_to_delete['result_path']}. Error: {e}")
...
def image_generation_worker():
    ...
    while True:
        ...
        if job_id:
            ...
            log_params = {k: v for k, v in pipe_kwargs.items() if k not in ["image", "generator"]}
            ...
            try:
                ...
                error_message = (
                    "Processing failed due to insufficient GPU memory. Try a smaller image size or reduce batch size."
                )
                logger.error(f"Job {job_id} failed: {error_message} - {e}")
                with queue_lock:
                    job_results[job_id].update({"status": "failed", "error": error_message})
            except RuntimeError as e:
                # Catch other generic PyTorch/CUDA runtime errors
                error_message = f"A runtime error occurred during processing. Details: {e}"
...
def generate_image_endpoint():
    ...
    with queue_lock:
        if len(job_queue) >= app.config["MAX_QUEUE_SIZE"]:
            ...
            return (
                jsonify({"error": "Server is currently busy. Please try again in a moment."}),
                503,
            )  # Service Unavailable
...
def get_result(job_id):
    ...
    if job.get("status") == "completed":
        ...
        return (
            jsonify({"error": f"Job is not yet complete. Current status: {job.get('status')}"}),
            202,
        )  # Accepted
...
if __name__ == "__main__":
    ...
    logger.warning("This is a development server. For production, use a WSGI server like Gunicorn.")
    # 2. Use the parsed port argument in app.run()
    app.run(host="0.0.0.0", port=args.port, debug=False)  # Debug mode should be False for this setup
