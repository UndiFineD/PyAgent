# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alirezarezvani.py\tdd_guide.py\scripts.py\tdd_workflow_b79a4033bb54.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\tdd-guide\scripts\tdd_workflow.py
...
class TDDWorkflow:
    ...
    def validate_red_phase(self, test_code: str, test_result: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        has_assertion = any(keyword in test_code.lower() for keyword in ["assert", "expect", "should"])
    ...
    def validate_green_phase(self, implementation_code: str, test_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        validations.append(
            {
                "valid": is_minimal,
                "message": (
                    "Implementation appears minimal" if is_minimal else "Implementation may be over-engineered"
                ),
            }
        )
    ...
    def validate_refactor_phase(
        ...
        validations.append(
            {
                "valid": test_passed,
                "message": ("Tests still pass after refactoring" if test_passed else "Tests broken by refactoring"),
            }
        )
        ...
        validations.append(
            {
                "valid": code_changed,
                "message": ("Code was refactored" if code_changed else "No refactoring applied (optional)"),
            }
        )
