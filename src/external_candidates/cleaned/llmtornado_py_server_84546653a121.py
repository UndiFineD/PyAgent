# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\llmtornado.py\src.py\llmtornado.py\llmtornado.py\fskb.py\mcp_server.py\server_84546653a121.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\mcp_server\server.py
...
class MCPServer:
    ...
    async def _handle_add_root(self, arguments: dict) -> list[TextContent]:
        ...
        if success:
            ...
            return [TextContent(type="text", text=f"Failed to add root: {root_path}")]
    ...
    async def _handle_remove_root(self, arguments: dict) -> list[TextContent]:
        ...
        if success:
            return [TextContent(type="text", text=f"Successfully removed root: {root_path}")]

        else:
            return [TextContent(type="text", text=f"Failed to remove root: {root_path}")]
    ...
    async def _handle_get_status(self, arguments: dict) -> list[TextContent]:
        ...
        return [TextContent(type="text", text=f"Indexing Status:\n{stats_json}")]
