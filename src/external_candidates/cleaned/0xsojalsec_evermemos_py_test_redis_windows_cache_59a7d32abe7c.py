# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\tests.py\test_redis_windows_cache_59a7d32abe7c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_redis_windows_cache.py
...
async def test_basic_operations():
    ...
    cache = await factory.create_cache_manager(expire_minutes=1)  # Use 1 minute expiration for testing
    ...
    recent_data = await cache.get_by_timestamp_range(test_key, start_timestamp=one_minute_ago)
    ...
    for item in recent_data:
        ...
        assert all(k in item for k in ["id", "data", "timestamp", "datetime"]), "Data item missing required fields"
...
async def test_timestamp_range_query():
    ...
    cache = await factory.create_cache_manager(expire_minutes=10, cleanup_probability=0.0)  # Disable random cleanup
    ...
    recent_data = await cache.get_by_timestamp_range(test_key, start_timestamp=five_minutes_ago)

    assert len(recent_data) == 8, f"Failed to get last 5 minutes data, expected 8, actual {len(recent_data)}"
    ...
    old_data = await cache.get_by_timestamp_range(test_key, end_timestamp=one_minute_ago)
    ...
    dt_data = await cache.get_by_timestamp_range(test_key, start_timestamp=dt_start, end_timestamp=dt_end)
...
async def test_json_pickle_mixed_data():
    ...
    cache = await factory.create_cache_manager(expire_minutes=10, cleanup_probability=0.0)
    ...
    for i, item in enumerate(all_data):
        ...
        if isinstance(retrieved_data, dict) and "windows_set" in retrieved_data:
            ...
            assert isinstance(retrieved_data["windows_bytes"], bytes), "Bytes data type error"
        ...
        # Check Pickle data
            ...
            assert "bytes_data" in retrieved_data.complex_data, "Pickle object missing bytes data"
        ...
        elif isinstance(retrieved_data, (dict, list, str, int, float, bool)) and not isinstance(
            retrieved_data, NonSerializableTestClass
        ):
    ...
    assert json_count == len(json_data), f"JSON data count mismatch: expected {len(json_data)}, actual {json_count}"

    assert pickle_count == len(pickle_data), (
    ...
    if len(all_data) > 2:
        ...
        first_half = await cache.get_by_timestamp_range(test_key, end_timestamp=mid_timestamp)

        assert len(first_half) > 0, "Timestamp range query first half data failed"

        second_half = await cache.get_by_timestamp_range(test_key, start_timestamp=mid_timestamp)
...
async def test_pickle_performance():
    ...
    json_data = [{"index": i, "type": "json", "data": f"json_item_{i}"} for i in range(batch_size)]

    pickle_data = [NonSerializableTestClass(f"perf_{i}", i * 10) for i in range(batch_size)]
    ...
    assert len(all_data) == batch_size * 2, f"Data count error: expected {batch_size * 2}, actual {len(all_data)}"
    ...
    json_retrieved = sum(
        ...
        if isinstance(item["data"], dict) and "type" in item["data"] and item["data"]["type"] == "json"
    )

    pickle_retrieved = sum(1 for item in all_data if isinstance(item["data"], NonSerializableTestClass))

    assert json_retrieved == batch_size, f"JSON data read count error: expected {batch_size}, actual {json_retrieved}"

    assert pickle_retrieved == batch_size, (
...
async def test_compatibility_layer():
    ...
    assert len(range_data) == 3, (
        f"Backward compatibility layer timestamp range query failed, expected 3, actual {len(range_data)}"
    )  # 1 original data + 2 new data
