# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2018.py\gauss_8ee359a93a04.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\gauss.py
...
class ShowRowReduction(Scene):
    ...
    def initialize_terms(self):
        ...
        for i, row in enumerate(rows):
            for j, term in enumerate(row):
                term.move_to(i * self.v_spacing * DOWN + j * self.h_spacing * RIGHT)
        ...
        for partial_sum in np.cumsum(lengths)[:-1]:
            VGroup(*mobject_matrix[:, partial_sum:].flatten()).shift(self.extra_h_spacing * RIGHT)
    ...
    def apply_row_rescaling(self, row_index, scale_factor):
        ...
        scalar_mob.add_to_back(OldTex("\\times").next_to(scalar_mob, LEFT, SMALL_BUFF))
