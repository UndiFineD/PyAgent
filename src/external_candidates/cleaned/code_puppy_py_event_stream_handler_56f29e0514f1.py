# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\agents.py\event_stream_handler_56f29e0514f1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\agents\event_stream_handler.py
...
def _fire_stream_event(event_type: str, event_data: Any) -> None:
    ...
    try:
        ...
        asyncio.create_task(callbacks.on_stream_event(event_type, event_data, agent_session_id))
...
async def event_stream_handler(
    ...
    def _print_response_banner() -> None:
        ...
        console.print(
            Text.from_markup(f"[bold white on {response_color}] AGENT RESPONSE [/bold white on {response_color}]")
        )
    ...
    async for event in events:
        ...
        if isinstance(event, PartStartEvent):
            ...
            if isinstance(part, ThinkingPart):
                ...
                termflow_renderers[event.index] = TermflowRenderer(output=console.file, width=console.width)
                ...
                banner_printed.add(event.index)  # Use banner_printed to track if we've shown tool info
        ...
        # PartDeltaEvent - stream the content as it arrives
            ...
            if event.index in streaming_parts:
                ...
                if isinstance(delta, (TextPartDelta, ThinkingPartDelta)):
                    ...
                    if tool_name_delta:
                        tool_names[event.index] = tool_names.get(event.index, "") + tool_name_delta
