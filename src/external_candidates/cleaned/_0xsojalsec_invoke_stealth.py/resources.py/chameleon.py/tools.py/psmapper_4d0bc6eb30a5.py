# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Invoke-Stealth\Resources\Chameleon\tools\psmapper.py
import argparse
import json
import os
import re
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chameleon PSMapper - Helper to create obfuscated function mappings")
    ...
    tree = PSTree(psctx=PSContext(name="main", ctx_type=PSContextType.MAIN))
    ...
    for n, line in enumerate(content, start=1):
        ...
        if tree.current_ctx_type in [
            PSContextType.MAIN,
            PSContextType.FUNCTION,
            PSContextType.NESTED_FUNCTION,
        ]:
    ...
    for k, v in mapping.items():
        ...
        new_mapping[k] = {"repl": scramble(k), "params": new_params}
