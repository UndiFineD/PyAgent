# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\events.py\event_publisher_8e1d514c3b8f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\events\event_publisher.py
...
class ApplicationEventPublisher:
    ...
    def get_listeners_for_event(self, event_type: Type[BaseEvent]) -> List[EventListener]:
        """
    ...
    async def publish(self, event: BaseEvent) -> None:
        ...
        if not listeners:
            logger.debug(f"No listeners for event [{event_type_name}], skipping publish")

            return

        logger.debug(f"Publishing event [{event_type_name}] (id={event.event_id}), {len(listeners)} listeners")
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ApplicationEventPublisher(listeners={len(self._listeners)}, event_types={len(self._event_listeners_map)})"
        )
