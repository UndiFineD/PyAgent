# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\gui\main_window.py
"""
...
class MainWindow(QMainWindow):
    ...
    def _create_search_tab(self) -> QWidget:
        ...
        self.results_table.setHorizontalHeaderLabels(["File", "Lines", "Score", "Preview"])
    ...
    def _add_root(self):
        ...
        directory = QFileDialog.getExistingDirectory(self, "Select Root Directory", str(Path.home()))
    ...
    async def _debug_test_similarity_async(self, query: str, content_hash: str, root_path: Path, task: str = None):
        ...
        try:
            ...
            self.debug_result_label.setText(f"Score: {similarity:.4f} | {file_path} | Lines: {lines}")
            logger.info(
        ...
        try:
            ...
            for root_path, root_state in self.indexing_engine.roots.items():
                ...
                current_snapshot[str(root_path)] = {
                    ...
                    "chunks": (
                        "Loading..." if (is_initializing or chunks_loading) else str(root_state.stats.chunks_created)
                    ),
    ...
    def _on_search_completed(self, results: list):
        ...
        for result in results:
            ...
            self.results_table.setItem(row, 1, QTableWidgetItem(f"{result['line_start']}-{result['line_end']}"))
