# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.Tests\Static\Files\Skills\codebase-context-extractor\context_extractor.py
#!/usr/bin/env python3
"""
        ...
        for dir_name in sorted(files_by_dir.keys()):
            ...
            for fc in files_by_dir[dir_name]:
                ...
                if fc.entities:
                    ...
                    summary_str = ", ".join(
                        f"{count} {etype}{'s' if count > 1 else ''}" for etype, count in entity_summary.items()
                    )
...
def main():
    ...
    parser.add_argument("--target-path", required=True, help="Path to the codebase to analyze")
    parser.add_argument(
        "--mode",
        required=True,
        choices=[
