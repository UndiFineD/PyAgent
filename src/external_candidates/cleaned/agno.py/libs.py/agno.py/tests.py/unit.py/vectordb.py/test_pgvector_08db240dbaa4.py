# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\tests\unit\vectordb\test_pgvector.py
import uuid
...
async def test_async_create(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "create"),
...
async def test_async_doc_exists(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "doc_exists", return_value=True),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
...
async def test_async_name_exists(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "name_exists", return_value=True),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = True
...
async def test_async_insert(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "insert"),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = None
...
async def test_async_upsert(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "upsert"),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = None
...
async def test_async_drop(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "drop"),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = None
...
async def test_async_exists(mock_pgvector):
    ...
    with (
        patch.object(mock_pgvector, "exists", return_value=True),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = True
