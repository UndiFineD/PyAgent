# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\reasoning\tools\capture_reasoning_content_reasoning_tools.py
...
if hasattr(streaming_agent, "run_response") and streaming_agent.run_response:
    if hasattr(streaming_agent.run_response, "reasoning_content") and streaming_agent.run_response.reasoning_content:
        print("✅ reasoning_content FOUND in agent's run_response after streaming")
        print(f"   Length: {len(streaming_agent.run_response.reasoning_content)} characters")
