# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\storage\mongodb_storage\mongodb_storage_for_workflow.py
import json
...
if __name__ == "__main__":
    # Run workflow
    storage = MongoDbStorage(collection_name="agent_sessions", db_url=db_url, db_name="agno")
    storage.drop()
    report: Iterator[RunResponse] = HackerNewsReporter(storage=storage, debug_mode=False).run(num_stories=5)
    # Print the report
