# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\tools\mcp\notion_mcp_agent.py
"""
...
async def run_agent():
    ...
    if not token:
        raise ValueError("Missing Notion API key: provide --NOTION_API_KEY or set NOTION_API_KEY environment variable")

    command = "npx"
    args = ["-y", "@notionhq/notion-mcp-server"]
    env = {"OPENAPI_MCP_HEADERS": json.dumps({"Authorization": f"Bearer {token}", "Notion-Version": "2022-06-28"})}
