# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\universal_agent_interface\app.py
import asyncio
...
async def header():
    st.markdown("<h1 class='heading'>Universal Agent Interface</h1>", unsafe_allow_html=True)
    st.markdown(
...
async def body() -> None:
    ...
    # Create UAgI
    ####################################################################
    uagi_config = UAgIConfig(user_id=user_id, model_id=model_id, tools=tools, agents=agents)
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Thinking..."):
                ...
                try:
                    ...
                    run_response = await uagi.arun(user_message, stream=True, stream_intermediate_steps=True)
                    async for resp_chunk in run_response:
                        ...
                        if resp_chunk.event == "RunResponse" and resp_chunk.content is not None:
                            response += resp_chunk.content
                            resp_container.markdown(response)

                    # Add the response to the messages
                    if uagi.run_response is not None:
                        await add_message("assistant", response, uagi.run_response.tools)
                    else:
