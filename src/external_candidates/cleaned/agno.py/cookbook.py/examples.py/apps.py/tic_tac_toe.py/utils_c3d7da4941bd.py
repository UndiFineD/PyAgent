# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\tic_tac_toe\utils.py
...
def create_mini_board_html(board_state: list, highlight_pos: tuple = None, is_player1: bool = True) -> str:
    """Create HTML for a mini board with player-specific highlighting"""
    html = '<div class="mini-board">'
    for i in range(3):
        for j in range(3):
            highlight = f"highlight player{1 if is_player1 else 2}" if highlight_pos and (i, j) == highlight_pos else ""
