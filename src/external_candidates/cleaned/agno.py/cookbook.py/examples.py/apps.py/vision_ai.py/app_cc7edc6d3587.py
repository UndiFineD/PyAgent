# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\vision_ai\app.py
import os
import time
...
def main():
    ...
    with st.sidebar:
        ...
        # Model Selection
        model_choice = st.selectbox("🔍 Select Model Provider", ["OpenAI", "Gemini", "Mistral"], index=0)
    ...
    ####################################################################
    uploaded_file = st.file_uploader("📤 Upload an Image (Max: 20MB) 📷", type=["png", "jpg", "jpeg"])
    image_path = None
    ...
    if prompt := st.chat_input("💬 Ask follow-up questions on the image extracted data..."):
        ...
        if last_message and last_message["role"] == "user":
            ...
            if "last_image_response" not in st.session_state or not st.session_state["last_image_response"]:
                st.warning("⚠️ No extracted insights available. Please process an image first.")
            else:
