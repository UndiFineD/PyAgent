# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\gemini-tutor\agents.py
"""
...
class TutorAppAgent:
    ...
    def _create_agent(self):
        ...
        if "gemini-2." in self.model_id or "gemini-1.5" in self.model_id:
            try:
                ...
                logger.warning(f"Model {self.model_id} does not support grounding attribute.")
            except Exception as e:
                logger.warning(f"Could not enable model grounding: {e}")

        # Format description and instructions directly
        tutor_description = TUTOR_DESCRIPTION_TEMPLATE.format(education_level=self.education_level)
    ...
    def create_learning_experience(self, search_topic, education_level=None):
        ...
        if education_level and self.education_level != education_level:
            logger.info(f"Using temporary education level for this request: {education_level}")
        else:
            # Use the agent's default education level if not overridden
            current_education_level = self.education_level

        logger.info(f"Creating learning experience for '{search_topic}' at {current_education_level} level")

        # Construct a focused prompt for the agent, relying on its core instructions
        grounding_instructions = (
            SEARCH_GROUNDING_INSTRUCTIONS if "gemini-2." in self.model_id or "gemini-1.5" in self.model_id else ""
        )
