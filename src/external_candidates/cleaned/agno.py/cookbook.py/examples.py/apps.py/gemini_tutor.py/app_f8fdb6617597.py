# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\gemini-tutor\app.py
"""
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        selected_model_name = st.selectbox(
            ...
            index=list(MODEL_OPTIONS.values()).index(st.session_state.model_id),  # Maintain selection
        ...
        st.session_state.education_level = st.selectbox(
            ...
            index=EDUCATION_LEVELS.index(st.session_state.education_level),  # Maintain selection
            key="education_level_selector",
        )

        # Check if settings changed
        if st.session_state.model_id != prev_model_id or st.session_state.education_level != prev_education_level:
            st.session_state.agent_needs_reset = True
            st.info("Settings changed. Agent will be updated on next interaction or reset.")
...
def render_chat_history():
    ...
    for message in st.session_state.messages:
        ...
        if not message.get("content") and not message.get("tools") and not message.get("citations"):
            continue
    ...
    if st.session_state.processing and st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        ...
        try:
            ...
            with st.spinner("🤔 Thinking..."):
                ...
                full_content = st.session_state.pop("full_content_from_stream", "[No content received]")
                assistant_message["content"] = full_content if full_content else "[No content received]"
                if not full_content:
            ...
            if "final_assistant_message" in st.session_state:
                st.session_state.messages.append(st.session_state.pop("final_assistant_message"))
