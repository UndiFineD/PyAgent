# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\parallel_world_builder\utils.py
...
def sidebar_widget() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("A post-apocalyptic world with a nuclear wasteland and a small community living in a dome."):
            add_message(
                "user",
                "A post-apocalyptic world with a nuclear wasteland and a small community living in a dome.",
            )
        if st.button("A world with a mix of ancient and modern civilizations, where magic and technology coexist."):
            add_message(
                "user",
