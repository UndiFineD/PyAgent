# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\examples\apps\chess_team\app.py
import logging
    ...
    if st.session_state.game_started:
        ...
        if not st.session_state.game_paused and not game_over:
            ...
            legal_moves_descriptions = "\n".join(
                [f"- {move['san']} ({move['uci']}): {move['description']}" for move in legal_moves]
            )
            ...
            try:
                ...
                if success:
                    ...
                    move_description = next(
                        (move["description"] for move in legal_moves if move["uci"] == move_str),
                        "",
                    )
