# Extracted from: C:\DEV\PyAgent\.external\agno\cookbook\getting_started\16_agent_session.py
...
def print_messages(agent):
    ...
    console.print(
        Panel(
            JSON(
                json.dumps([m.model_dump(include={"role", "content"}) for m in agent.get_messages_for_session()]),
                indent=4,
            ),
