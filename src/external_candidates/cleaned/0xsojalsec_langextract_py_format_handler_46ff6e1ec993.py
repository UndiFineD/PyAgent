# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\core.py\format_handler_46ff6e1ec993.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\core\format_handler.py
...
class FormatHandler:
    ...
    def parse_output(self, text: str, *, strict: bool | None = None) -> Sequence[Mapping[str, ExtractionValueType]]:
        ...
        if parsed is None:
            if self.use_wrapper:
                raise exceptions.FormatParseError(f"Content must be a mapping with an '{self.wrapper_key}' key.")

            else:
                raise exceptions.FormatParseError("Content must be a list of extractions or a dict.")
        ...
        if isinstance(parsed, dict):
            if require_wrapper:
                if self.wrapper_key not in parsed:
                    raise exceptions.FormatParseError(f"Content must contain an '{self.wrapper_key}' key.")
        ...
        if not isinstance(items, list):
            raise exceptions.FormatParseError("The extractions must be a sequence (list) of mappings.")
        ...
        for item in items:
            ...
            for k in item.keys():
                if not isinstance(k, str):
                    raise exceptions.FormatParseError("All extraction keys must be strings (got a non-string key).")
    ...
    def _is_valid_language_tag(self, lang: str | None, valid_tags: dict[data.FormatType, set[str]]) -> bool:
    ...
    def _extract_content(self, text: str) -> str:
        ...
        candidates = [m for m in matches if self._is_valid_language_tag(m.group("lang"), valid_tags)]

        if self.strict_fences:
            if len(candidates) != 1:
                if len(candidates) == 0:
                    raise exceptions.FormatParseError("Input string does not contain valid fence markers.")
    ...
    def from_kwargs(cls, **kwargs) -> FormatHandler:
        ...
        if format_type is None:
            ...
            format_type = data.FormatType.JSON if str(format_type).lower() == "json" else data.FormatType.YAML
