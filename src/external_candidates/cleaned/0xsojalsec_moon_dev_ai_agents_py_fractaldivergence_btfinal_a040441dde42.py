# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_moon_dev_ai_agents.py\src.py\data.py\rbi.py\_03_13_2025.py\backtests_final.py\fractaldivergence_btfinal_a040441dde42.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_final\FractalDivergence_BTFinal.py
...
class FractalDivergence(bt.Strategy):
    ...
    def next(self):
        ...
        if self.position is None:
            ...
            if (
                self.checkFractalBuyCondition()
                and len(self.getHistData("data", N=10)) >= 2
                ...
                self.checkFractalSellCondition()
                and len(self.getHistData("data", N=10)) >= 2
                and not self.isInDivergencePhase()
            ):
                self.position = self.getposition()
