# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\messaging.py\messages_4525000a8258.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\messaging\messages.py
...
class BaseMessage(BaseModel):
    ...
    category: MessageCategory = Field(description="Category for routing and rendering decisions")
...
class FileEntry(BaseModel):
    ...
    type: Literal["file", "dir"] = Field(description="Whether this is a file or directory")
...
class DiffMessage(BaseMessage):
    ...
    operation: Literal["create", "modify", "delete"] = Field(description="Type of file operation")
...
class ShellStartMessage(BaseMessage):
    ...
    background: bool = Field(default=False, description="Whether command runs in background mode")
...
class ShellLineMessage(BaseMessage):
    ...
    stream: Literal["stdout", "stderr"] = Field(default="stdout", description="Which output stream this line came from")
...
class SubAgentInvocationMessage(BaseMessage):
    ...
    is_new_session: bool = Field(description="Whether this is a new session or continuation")

    message_count: int = Field(default=0, description="Number of messages in history (for continuation)")
...
class SubAgentResponseMessage(BaseMessage):
    ...
    message_count: int = Field(default=0, description="Number of messages now in session history")
...
class SubAgentStatusMessage(BaseMessage):
    ...
    status: Literal["starting", "running", "thinking", "tool_calling", "completed", "error"] = Field(
        description="Current status of the agent"
    )

    tool_call_count: int = Field(default=0, ge=0, description="Number of tools called so far")

    token_count: int = Field(default=0, ge=0, description="Estimated tokens in context")

    current_tool: Optional[str] = Field(default=None, description="Name of tool currently being called")

    elapsed_seconds: float = Field(default=0.0, ge=0, description="Time since agent started")
...
class UniversalConstructorMessage(BaseMessage):
    ...
    action: str = Field(description="The UC action performed (list/call/create/update/info)")
...
class ConfirmationRequest(BaseMessage):
    ...
    description: str = Field(description="Detailed description of what's being confirmed")
