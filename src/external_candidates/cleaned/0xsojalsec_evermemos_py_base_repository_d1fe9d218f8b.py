# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\oxm.py\es.py\base_repository_d1fe9d218f8b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\es\base_repository.py
...
class BaseRepository(ABC, Generic[T]):
    ...
    async def update(self, document: T, refresh: bool = False) -> T:
        ...
        try:
            ...
            logger.debug("✅ Document updated successfully [%s]: %s", self.model_name, doc_id)
    ...
    async def delete_by_id(self, doc_id: str, refresh: bool = False) -> bool:
        ...
        try:
            ...
            if document:
                ...
                logger.debug("✅ Document deleted successfully [%s]: %s", self.model_name, doc_id)
    ...
    async def create_batch(self, documents: List[T], refresh: bool = False) -> List[T]:
        ...
        try:
            ...
            logger.error("❌ Failed to batch create documents [%s]: %s", self.model_name, e)

            raise

    # ==================== Search Methods ====================

    async def search(self, query: Dict[str, Any], size: int = 10, from_: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = await client.search(index=index_name, body={"query": query, "size": size, "from": from_})
    ...
    async def match_all(self, size: int = 10, from_: int = 0) -> List[T]:
        ...
        try:
            response = await self.search(query={"match_all": {}}, size=size, from_=from_)
    ...
    async def delete_index(self) -> bool:
        ...
        try:
            ...
            logger.debug("✅ Index deletion succeeded [%s]: %s", self.model_name, index_name)
