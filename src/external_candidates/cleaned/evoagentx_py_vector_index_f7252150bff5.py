# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\rag.py\indexings.py\vector_index_f7252150bff5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\indexings\vector_index.py
...
class VectorIndexing(BaseIndexWrapper):
    ...
    def _create_storage_context(
        ...
        assert self.storage_handler.vector_store is not None, (
            "VectorIndexing must init a vector backend in 'storageHandler'"
        )
            ...
            if node_ids:
                for node_id in node_ids:
                    if node_id in self.id_to_node:
                        ...
                        if (
                            self.index.storage_context.docstore._kvstore._collections_mappings.get(node_id, None)
                            is not None
                        ):
