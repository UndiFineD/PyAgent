# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\milvus.py\tenant_aware_collection_with_suffix_b761dbda2f5c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\milvus\tenant_aware_collection_with_suffix.py
...
class TenantAwareMilvusCollectionWithSuffix(MilvusCollectionWithSuffix):
    ...
    def create_new_collection(self) -> TenantAwareCollection:
        ...
        logger.info("New tenant-aware Collection created: %s", tenant_aware_new_real_name)
    ...
    def switch_alias(self, new_collection: TenantAwareCollection, drop_old: bool = False) -> None:
        """
        ...
        if drop_old and old_real_name:
            try:
                ...
                logger.warning("Failed to delete old collection (can handle manually): %s", e)
    ...
    def drop(self) -> None:
        ...
        try:
            ...
            logger.warning("Collection '%s' does not exist or deletion failed: %s", name, e)
