# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\hughpyle.py\keep.py\keep.py\store_ceddeac9cde5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hughpyle\keep\keep\store.py
...
class ChromaStore:
    ...
    def __init__(self, store_path: Path, embedding_dimension: int):
        ...
        try:
            ...
            raise RuntimeError("ChromaStore requires 'chromadb' library. Install with: pip install chromadb")
    ...
    def upsert(
        ...
        if len(embedding) != self._embedding_dimension:
            raise ValueError(
                f"Embedding dimension mismatch: expected {self._embedding_dimension}, got {len(embedding)}"
            )
