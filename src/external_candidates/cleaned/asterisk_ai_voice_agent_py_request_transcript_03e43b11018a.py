# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\tools.py\business.py\request_transcript_03e43b11018a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\business\request_transcript.py
...
class RequestTranscriptTool(Tool):
    ...
    async def execute(self, parameters: Dict[str, Any], context: ToolExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not config.get("enabled", False):
                logger.info("Request transcript tool disabled", call_id=call_id)
            ...
            if not raw_email:
                logger.warning("No caller email provided", call_id=call_id)
            ...
            if not self._validator.validate_email(parsed_email):
                logger.warning("Invalid email format", call_id=call_id, parsed_email=parsed_email)

                return {
                    "status": "error",
                    "message": (
                        f"The email address {parsed_email} doesn't seem to be valid. Could you please provide it again?"
                    ),
                    "ai_should_speak": True,
                }
            ...
            return {
                "status": "success",
                "message": (f"Perfect! I'll send the complete transcript to {email_for_speech} when our call ends."),
                "ai_should_speak": True,
                "caller_email": parsed_email,
    ...
    def _prepare_email_data(
        self, caller_email: str, session: Any, config: Dict[str, Any], call_id: str
    ) -> Dict[str, Any]:
