# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\digisal.py\youtrack_digisal.py\scripts.py\youtrack_api_c520b1df489b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\digisal\youtrack-digisal\scripts\youtrack_api.py
...
class YouTrackAPI:
    ...
    def __init__(self, base_url: str, token: Optional[str] = None):
        ...
        if not self.token:
            raise ValueError("YouTrack token required. Set YOUTRACK_TOKEN env var or pass as argument.")
    ...
    def get_project(self, project_id: str) -> Dict:
        ...
        return self._make_request(
            "GET",
            f"/api/admin/projects/{project_id}?fields=id,name,shortName,description",
        )
    ...
    def get_issues(
        ...
        fields: str = "id,summary,description,created,updated,project(id,name),customFields(name,value)",
    ) -> List[Dict]:
        """
    ...
    def get_work_items(self, issue_id: str) -> List[Dict]:
        ...
        result = self._make_request(
            "GET",
            f"/api/issues/{issue_id}/timeTracking/workItems?fields=id,date,duration(minutes),author(name),text",
        )
    ...
    def create_article(self, project_id: str, title: str, content: str) -> Dict:
        ...
        data = {"project": {"id": project_id}, "title": title, "content": content}
