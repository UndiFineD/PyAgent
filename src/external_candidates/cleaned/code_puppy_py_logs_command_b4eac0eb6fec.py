# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\mcp.py\logs_command_b4eac0eb6fec.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\mcp\logs_command.py
...
class LogsCommand(MCPCommandBase):
    ...
    def _list_servers_with_logs(self, group_id: str) -> None:
        ...
        for server in servers:
            ...
            rotated = f" (+{stats['rotated_count']} rotated)" if stats["rotated_count"] else ""

            lines.append(f"  • **{server}** - {stats['line_count']} lines, {size_str}{rotated}")
    ...
    def _show_logs(self, server_name: str, lines: Optional[int], group_id: str) -> None:
        ...
        try:
            ...
            if not log_lines:
                emit_info(
                    f"📋 No logs found for server: **{server_name}**\nLog file: `{get_log_file_path(server_name)}`",
                    message_group=group_id,
                )
            ...
            if lines is None:
                ...
                header = f"📋 Logs for {server_name} (last {showing} of {total_lines} lines)"
