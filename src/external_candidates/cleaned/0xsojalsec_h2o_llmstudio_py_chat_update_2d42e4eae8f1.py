# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\app_utils.py\sections.py\chat_update_2d42e4eae8f1.py
...
async def chat_copy(q: Q) -> None:
    chat_messages = [
        f"{'USER' if t[1] == USER else 'ASSISTANT'}: {t[0]}" for t in q.client["experiment/display/chat/messages"]
    ]

    chat_to_copy = "\n".join(chat_messages)

    q.page["meta"].script = ui.inline_script(f"navigator.clipboard.writeText(`{chat_to_copy}`);")
...
async def answer_chat(q: Q) -> str:
    ...
    if len(q.client["experiment/display/chat/messages"]):
        for prev_message in q.client["experiment/display/chat/messages"][-(cfg.prediction.num_history + 1) :]:
            if prev_message[1] is USER:
                prev_message = cfg.dataset.dataset_class.parse_prompt(cfg, prev_message[0])
    ...
    inputs = cfg.dataset.dataset_class.encode(tokenizer, full_prompt, cfg.tokenizer.max_length, "left")

    inputs["prompt_input_ids"] = inputs.pop("input_ids").unsqueeze(0).to(cfg.environment._device)

    inputs["prompt_attention_mask"] = inputs.pop("attention_mask").unsqueeze(0).to(cfg.environment._device)

    def text_cleaner(text: str) -> str:
        return cfg.dataset.dataset_class.clean_output(output={"predicted_text": np.array([text])}, cfg=cfg)[
            "predicted_text"
        ][0]
            ...
            for current_wait_time in range(max_wait_time_in_seconds):
                ...
                if streaming_finished or takes_too_much_time or thread_is_dead:
                    if takes_too_much_time:
                        ...
                        logger.warning("Chat generation took too much time. Stopping chat generation.")

                    if thread_is_dead:  # some error occurred during streaming
                        logger.warning("Chat generation thread is not alive anymore. Please check logs!")
...
def generate(model: Model, inputs: dict, cfg: Any, streamer: TextStreamer = None):
    with torch.cuda.amp.autocast(dtype=get_torch_dtype(cfg.environment.mixed_precision_dtype)):
