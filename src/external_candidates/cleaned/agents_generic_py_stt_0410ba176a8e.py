# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agents_generic.py\livekit_plugins.py\livekit_plugins_speechmatics.py\livekit.py\plugins.py\speechmatics.py\stt_0410ba176a8e.py
    ...
    def __init__(
        ...
        *,
        ...
        if is_given(transcription_config):
            ...
            additional_vocab = additional_vocab if is_given(additional_vocab) else config.additional_vocab

            punctuation_overrides = (
                punctuation_overrides if is_given(punctuation_overrides) else config.punctuation_overrides
            )
        ...
        self._base_url: str = (
            base_url if is_given(base_url) else os.getenv("SPEECHMATICS_RT_URL", "wss://eu2.rt.speechmatics.com/v2")
        )
    ...
    def _process_config(self) -> None:
        ...
        if self._stt_options.enable_diarization:
            ...
            if self._stt_options.known_speakers:
                dz_cfg["speakers"] = {s.label: s.speaker_identifiers for s in self._stt_options.known_speakers}
