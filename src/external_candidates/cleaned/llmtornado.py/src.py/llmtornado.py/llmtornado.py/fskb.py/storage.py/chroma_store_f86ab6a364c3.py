# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\storage\chroma_store.py
"""
...
class ChromaStore:
    ...
    async def get_cached_embeddings(self, content_hashes: List[str]) -> Dict[str, Optional[np.ndarray]]:
        """
        ...
        try:
            ...
            results = await asyncio.to_thread(cache.get, ids=unique_hashes, include=["embeddings"])
    ...
    async def cache_embeddings(self, content_hashes: List[str], embeddings: List[np.ndarray]):
        """
    ...
    async def cleanup_orphaned_files(
        ...
        try:
            ...
            for i in range(0, len(orphaned_list), batch_size):
                ...
                await asyncio.to_thread(
                    collection.delete,
                    where={"$and": [{"branch": branch_name}, {"file_path": {"$in": batch}}]},
                )
    ...
    async def get_branch_stats(self, root_path: Path, branch_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = await asyncio.to_thread(collection.get, where={"branch": branch_name}, include=["metadatas"])
    ...
    async def load_ignore_file_metadata(self, root_path: Path, branch_name: str) -> Dict[str, tuple[float, int, str]]:
        """
        ...
        try:
            ...
            collection_name = f"{self._get_collection_name(root_path)}_metadata"
            try:
                metadata_collection = await asyncio.to_thread(self.client.get_collection, name=collection_name)
            except:
