# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\mcts.py\parallel_mcts_4edb88355609.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\mcts\parallel_mcts.py
...
class ParallelMCTS:
    ...
    def _validate_instance_count(self, total_instances: int, n_parallel: int):
        ...
        if total_instances < min_required:
            raise ValueError(
                f"Need at least {min_required} instances for {n_parallel} parallel searches (got {total_instances})"
            )
    ...
    async def search(self, n_iterations: int, skip_failures: bool = False):
        ...
        try:
            search_tasks = [
                self._run_group_search(group, n_iterations, skip_failures) for group in self.instance_groups
            ]
