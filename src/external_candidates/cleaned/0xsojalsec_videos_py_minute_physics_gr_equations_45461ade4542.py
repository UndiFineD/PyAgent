# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\minute_physics_gr_equations_45461ade4542.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\minute_physics_gr_equations.py
...
class GREquations(Scene):
    ...
    def construct(self):
        ...
        sys1, sys2 = [VGroup(*[self.get_field_eq(i, j, simple=simple) for i, j in indices]) for simple in (True, False)]
        ...
        self.play(
            LaggedStart(*[TransformFromCopy(eq1, eq) for eq in sys1], lag_ratio=0.2),
        )
        ...
        sys1_rects, sys2_rects = [VGroup(*map(self.get_rects, sys)) for sys in [sys1, sys2]]
    ...
    def get_field_eq(self, mu, nu, simple=True):
        ...
        if simple:
            ...
            tex_args = [
                ...
                (
                    ...
                    "\\Gamma^{\\rho}_{\\rho\\lambda}",
                    "\\Gamma^{\\lambda}_{\\beta\\alpha} -\\Gamma^{\\rho}_{\\beta \\lambda}",
        ...
        result = VGroup(*[OldTex(*args, **config) for args in tex_args])
    ...
    def get_rects(self, equation):
        return VGroup(
            *[
                ...
                for i, color in zip([0, 3], [GREY, GREY])
            ]
        )
