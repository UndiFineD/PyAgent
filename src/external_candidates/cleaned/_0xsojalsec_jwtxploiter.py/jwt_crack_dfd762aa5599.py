...
{Bcolors.HEADER}Command:{Bcolors.ENDC} {Bcolors.OKCYAN}{" ".join(command)}{Bcolors.ENDC}
    ...
    def __init__(
        ...
        self.original_token_header, self.original_token_payload = Cracker.decode_encoded_token(
            self.token_dict, quiet=self.quiet
        )
        ...
        if not any(self.no_key_validation_args):
            ...
            if self.alg[:2] == "RS" or self.alg[:2] == "PS":
                ...
                if any([self.x5u_basic, self.x5u_header_injection]):
                    ...
                    self.x5c = "".join(
                        [line.strip() for line in certificate_object_content.split("\n") if not line.startswith("---")]
                    )
            elif self.alg[:2] == "ES":
                """Check for key conflicts"""
                if any(self.cant_asymmetric_args):
    ...
    def sign_token_with_hmac(key, partial_token, sign_hash):
        ...
        """
        signature = (
            base64.urlsafe_b64encode(hmac.new(key.encode(), partial_token.encode(), sign_hash).digest())
            .decode()
            .rstrip("=")
        )
        return signature
    ...
    def gen_self_signed_certificate(private_key, public_key, days, sign_hash):
        ...
        certificate = (
            ...
            .add_extension(x509.SubjectAlternativeName([x509.DNSName("test")]), critical=False)
    ...
    def gen_new_jwk(kid, public_numbers, jwa="RS256"):
        ...
        if jwa[:2] in ["RS", "PS"]:
            jwk["kty"] = "RSA"
            jwk["n"] = (
                base64.urlsafe_b64encode(n_or_x.to_bytes(n_or_x.bit_length() // 8 + 1, byteorder="big"))
                .decode("utf-8")
                .rstrip("=")
            )
            jwk["e"] = (
                base64.urlsafe_b64encode(e_or_y.to_bytes(e_or_y.bit_length() // 8 + 1, byteorder="big"))
                .decode("utf-8")
                .rstrip("=")
            )
        elif jwa[:2] == "ES":
