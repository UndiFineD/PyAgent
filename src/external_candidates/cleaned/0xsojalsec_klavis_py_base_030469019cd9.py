# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\open_strata.py\src.py\strata.py\mcp_proxy.py\transport.py\base_030469019cd9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\src\strata\mcp_proxy\transport\base.py
...
class Transport(ABC):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._session = await self._exit_stack.enter_async_context(ClientSession(streams[0], streams[1]))
        ...
        if self._exit_stack:
            try:
                ...
                if "cancel scope" in str(e).lower():
                    logger.warning(
                        "Cross-task cleanup detected and handled. This typically happens with pytest fixtures."
                    )
