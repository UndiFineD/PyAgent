# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\segment_anything_3.py
...
class SegmentAnything3(BaseModel):
    ...
    def load(self):
        ...
        if "cuda" in device and torch.cuda.is_available():
            ...
            self.device = "cpu"
            if "cuda" in device:
                logger.warning(f"CUDA device requested but not available, falling back to CPU")
    ...
    def _predict_with_text(self, image: np.ndarray, text_prompt: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conf_thresh = params.get("conf_threshold", self.params.get("conf_threshold", 0.50))
        show_boxes = params.get("show_boxes", self.params.get("show_boxes", True))
        show_masks = params.get("show_masks", self.params.get("show_masks", False))

        logger.info(f"Processing text prompt: '{text_prompt}' with conf_threshold={conf_thresh}")

        processor = Sam3Processor(self.model, confidence_threshold=conf_thresh, device=self.device)
        ...
        if separator_used:
            prompts = [p.strip() for p in text_prompt.split(separator_used) if p.strip()]
        else:
        ...
        if all_masks:
            ...
            shapes = self._convert_results_to_shapes(combined_results, show_boxes, show_masks, epsilon_factor)
        else:
            shapes = []
    ...
    def _predict_with_boxes(
        ...
        logger.info(f"Processing visual prompt: '{marks}' with conf_threshold={conf_thresh}")
        ...
        for i, (box, label) in enumerate(zip(box_input_xywh, box_labels)):
            ...
            logger.debug(f"Box {i}: XYWH={box}, Label={label} (value={label_value})")
        ...
        box_input_cxcywh = box_xywh_to_cxcywh(torch.tensor(box_input_xywh, device=self.device).view(-1, 4))
        ...
        shapes = self._convert_results_to_shapes(inference_state, show_boxes, show_masks, epsilon_factor)
