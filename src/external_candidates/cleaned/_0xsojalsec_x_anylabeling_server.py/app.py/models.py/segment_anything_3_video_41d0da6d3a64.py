# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\segment_anything_3_video.py
import os
import shutil
import sys
import tempfile
...
class _VideoSession:
    ...
    def _init_predictor_session(self):
        ...
        logger.info(f"Created video session {self.session_id} with {len(self.frames)} frames")

    def cleanup(self):
        """Clean up session resources."""
        try:
            self.predictor.handle_request(request=dict(type="close_session", session_id=self.session_id))
...
class SegmentAnything3Video(BaseModel):
    ...
    def load(self):
        ...
        if isinstance(devices, list) and devices:
            ...
            gpus_to_use = range(torch.cuda.device_count()) if torch.cuda.is_available() else range(1)

        logger.info(f"Loading SAM3 video model from {model_path} on devices {gpus_to_use}")
        ...
        if hasattr(self.predictor, "model") and hasattr(self.predictor.model, "model"):
    ...
    def predict(self, image: np.ndarray, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.warning("Single frame predict called on video model. Use video API endpoints instead.")
        return {"shapes": [], "description": ""}

    def init_session(self, frames: List[np.ndarray], start_frame_index: int) -> Dict[str, Any]:
        ...
        with self._sessions_lock:
            ...
            session = _VideoSession(session_id, frames, start_frame_index, self.predictor)
    ...
    def add_prompt(
        ...
        if relative_frame_index < 0 or relative_frame_index >= len(session.frames):
    ...
    def add_point_prompt(
        ...
        try:
            ...
            if (
                ...
                or relative_frame_index not in inference_state["cached_frame_outputs"]
            ):
                if "cached_frame_outputs" not in inference_state:
                    inference_state["cached_frame_outputs"] = {}
                inference_state["cached_frame_outputs"][relative_frame_index] = {}
        except (AttributeError, RuntimeError, KeyError) as e:
            logger.warning(f"Could not initialize cache for frame {relative_frame_index}: {e}")
        ...
        try:
            ...
            if len(out_obj_ids) > 0:
                ...
                H_video = inference_state.get("orig_height", session.frames[0].shape[0])
                W_video = inference_state.get("orig_width", session.frames[0].shape[1])

                for i, obj_id in enumerate(out_obj_ids):
                    obj_id_int = int(obj_id) if hasattr(obj_id, "__int__") else obj_id
            ...
            for fidx in range(num_frames):
                if fidx not in inference_state["cached_frame_outputs"]:
                    inference_state["cached_frame_outputs"][fidx] = obj_id_to_mask.copy()
                else:
                    inference_state["cached_frame_outputs"][fidx].update(obj_id_to_mask)
    ...
    def start_propagation(
        ...
        logger.info(f"Submitting propagation task: task_id={task_id}, session_id={session_id}")

        try:
            future = self._executor.submit(self._run_propagation, task, session)
    ...
    def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        response_data = {
            ...
            "current_frame": (task.current_frame + start_frame_offset if task.current_frame else 0),
    ...
    def propagate_stream(
        ...
        orig_height, orig_width = self._get_frame_dimensions(session.frames)
        text_prompt = session.text_prompt if session.text_prompt else "AUTOLABEL_OBJECT"
        ...
        try:
            if torch.cuda.is_available():
                ...
                bf16_context = torch.autocast(device_type="cuda", dtype=torch.bfloat16)
                bf16_context.__enter__()

            generator = session.predictor.handle_stream_request(request=request_dict)

            prompt_frame_relative_idx = None
            prompt_absolute_idx = None
            if session.last_prompt_frame is not None:
                prompt_frame_relative_idx = session.last_prompt_frame - session.start_frame_index
            ...
            if session.prompt_frame_outputs is not None and prompt_absolute_idx is not None:
                prompt_outputs = session.prompt_frame_outputs
                prompt_obj_ids = self._normalize_obj_ids(prompt_outputs.get("out_obj_ids", []))
    ...
    def _run_propagation(self, task: _PropagationTask, session: _VideoSession):
        ...
        try:
            with task.lock:
                if task._cancelled:
                    logger.info(f"Task {task.task_id} was cancelled before starting")
                    return
            ...
            logger.info(f"Starting propagation for task {task.task_id}, total_frames={total_frames}")
            ...
            for response in generator:
                ...
                with task.lock:
                    ...
                    task.progress = frame_count / total_frames if total_frames > 0 else 0.0

            logger.info(
                f"Propagation finished for task {task.task_id}, collected {len(outputs_per_frame)} frame results"
            )
            ...
            logger.error(f"Propagation task {task.task_id} failed: {e}", exc_info=True)
            with task.lock:
                task.status = _TaskStatus.FAILED
                task.error = str(e)
        finally:
    ...
    def _build_completed_results(
        ...
        results = {}
        start_frame_offset = session.start_frame_index if session else 0
        orig_height, orig_width = self._get_frame_dimensions(session.frames if session else None)
        ...
        for frame_idx, outputs in task.results.items():
            ...
            params = self._get_inference_params(session.prompt_frame_params if session else None)
            shapes = self._convert_outputs_to_shapes(
                out_binary_masks,
                out_probs,
                out_obj_ids,
        ...
        if session and session.prompt_frame_outputs is not None and prompt_absolute_idx is not None:
            prompt_outputs = session.prompt_frame_outputs
            prompt_obj_ids = self._normalize_obj_ids(prompt_outputs.get("out_obj_ids", []))
    ...
    def _get_inference_params(self, prompt_params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _get_frame_dimensions(self, frames: Optional[List[np.ndarray]]) -> Tuple[int, int]:
    ...
    def _convert_outputs_to_shapes(
        ...
        num_objects = len(out_binary_masks) if isinstance(out_binary_masks, (list, np.ndarray)) else 0
        if num_objects == 0:
            return shapes
