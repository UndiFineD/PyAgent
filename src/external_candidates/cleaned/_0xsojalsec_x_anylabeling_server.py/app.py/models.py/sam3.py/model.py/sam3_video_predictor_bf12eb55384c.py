# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\sam3\model\sam3_video_predictor.py
...
class Sam3VideoPredictor:
    ...
    def start_session(self, resource_path, session_id=None):
        ...
        logger.debug(
            f"started new session {session_id}; {self._get_session_stats()}; {self._get_torch_and_gpu_properties()}"
        )
    ...
    def remove_object(
        ...
        logger.debug(f"remove object {obj_id} in session {session_id}: {is_user_action=}")
        session = self._get_session(session_id)
        inference_state = session["state"]
