# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\sam3\eval\hota_eval_toolkit\trackeval\datasets\youtube_vis.py
# flake8: noqa
...
class YouTubeVIS(_BaseDataset):
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            raw_data["dets"][t] = [track[result_key][t] for track in tracks if track[result_key][t]]
            raw_data["ids"][t] = np.atleast_1d([track["id"] for track in tracks if track[result_key][t]]).astype(int)
        ...
        raw_data["classes_to_tracks"] = {
            cls: [{i: track[result_key][i] for i in range(len(track[result_key]))} for track in tracks]
        ...
        if is_gt:
            raw_data["classes_to_gt_track_iscrowd"] = {
                cls: [track["iscrowd"] for track in tracks] for cls, tracks in classes_to_tracks.items()
            }
        else:
            raw_data["classes_to_dt_track_scores"] = {
                cls: np.array([track["score"] for track in tracks]) for cls, tracks in classes_to_tracks.items()
            }
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        if self.iou_type == "segm":
            ...
            similarity_scores = self._calculate_box_ious(gt_dets_t, tracker_dets_t, box_format="xywh", do_ioa=False)
