# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\sam3\train\data\sam3_image_dataset.py
        ...
        for query_id, query in enumerate(queries):
            ...
            if "input_box" in query and query["input_box"] is not None and len(query["input_box"]) > 0:
                ...
                if "input_box_label" in query and query["input_box_label"] is not None:
                    bbox_label = torch.as_tensor(query["input_box_label"], dtype=torch.long).view(-1)
            ...
            try:
                ...
                coco_image_id = (
                    int(img_metadata_query["coco_img_id"]) if "coco_img_id" in img_metadata_query else query["id"]
                )
            except KeyError:
                coco_image_id = -1
            ...
            find_queries.append(
                FindQueryLoaded(
                    ...
                    input_points=points,
                    object_ids_output=[id2index_obj[obj_id] for obj_id in query["object_ids_output"]],
