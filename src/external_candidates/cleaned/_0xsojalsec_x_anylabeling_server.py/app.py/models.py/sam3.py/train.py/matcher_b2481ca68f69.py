# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\sam3\train\matcher.py
...
class BinaryHungarianMatcherV2(nn.Module):
    ...
    def forward(
        ...
        if self.remove_samples_with_0_gt:
            ...
            batch_idx = torch.as_tensor(
                sum(
                    [[kept_inds[i]] * len(src) for i, src in enumerate(indices)],
                    [],
                ),
                dtype=torch.long,
                device=device,
