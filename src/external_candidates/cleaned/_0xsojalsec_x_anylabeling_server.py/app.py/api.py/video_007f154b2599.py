# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\api\video.py
import base64
import json
...
async def init_video_session(request: VideoInitRequest):
    ...
    if not model_id:
        return ErrorResponse(error=ErrorDetail(code="MISSING_MODEL", message="Model ID is required"))

    if not frames_data:
        return ErrorResponse(error=ErrorDetail(code="MISSING_FRAMES", message="Frames are required"))
    ...
    try:
        ...
        if "error" in result:
            return ErrorResponse(error=ErrorDetail(code="INIT_ERROR", message=result["error"]))
...
async def prompt_video_frame(request: VideoPromptRequest):
    ...
    if not session_id:
        return ErrorResponse(error=ErrorDetail(code="MISSING_SESSION", message="Session ID is required"))
    ...
    try:
        if text_prompt:
            ...
            result = model.add_point_prompt(session_id, points, point_labels, obj_id, frame_index, params)

        if "error" in result:
            return ErrorResponse(error=ErrorDetail(code="PROMPT_ERROR", message=result["error"]))
...
async def propagate_video(request: VideoPropagateRequest):
    ...
    logger.info(f"Received propagate request: session_id={session_id}, model={model_id}")
    ...
    try:
        result = model.start_propagation(session_id, request.start_frame, request.end_frame)

        if "error" in result:
            return ErrorResponse(error=ErrorDetail(code="PROPAGATION_ERROR", message=result["error"]))
...
async def propagate_video_stream(request: VideoPropagateRequest):
    ...
    def run_propagation():
        try:
            for event in model.propagate_stream(session_id, request.start_frame, request.end_frame):
            ...
            while not propagation_done.is_set() or not event_queue.empty():
                try:
                    event = await asyncio.wait_for(
                        loop.run_in_executor(None, lambda: event_queue.get(timeout=0.1)),
                        timeout=0.2,
                    )
...
async def get_propagation_status(task_id: str):
    ...
    return ErrorResponse(error=ErrorDetail(code="TASK_NOT_FOUND", message=f"Task {task_id} not found"))
...
async def cleanup_video_session(session_id: str, model: str):
    ...
    try:
        ...
        if success:
            return SuccessResponse(data={"message": f"Session {session_id} cleaned up"})
        return ErrorResponse(
            error=ErrorDetail(
