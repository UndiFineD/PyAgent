    ...
    def decorator(cls: Type[BaseModel]) -> Type[BaseModel]:
        for model_id in model_ids:
            if model_id in _MODEL_REGISTRY:
                logger.warning(f"Model '{model_id}' is already registered. Overwriting with {cls.__name__}")
...
class ModelRegistry:
    ...
    def _validate_widgets(self, model_id: str, widgets: List[Dict]):
        ...
        for widget_name, rules in required_defaults.items():
            if widget_name in widget_dict:
                ...
                if value is None:
                    raise ValueError(f"Model [{model_id}]: Widget '{widget_name}' requires a default value")

                if not isinstance(value, rules["type"]):
                    raise ValueError(f"Model [{model_id}]: Widget '{widget_name}' value must be {rules['type']}")
    ...
    def load_all_models(self):
        ...
        if len(self.models) == 0:
            ...
            logger.info(f"Successfully loaded {len(self.models)}/{len(enabled)} model(s)")
    ...
    def _load_single_model(self, model_id: str):
        ...
        if model_id not in self.model_registry:
            raise ValueError(f"Model '{model_id}' not registered. Add it to _build_registry() in loader.py")
    ...
    def get_all_models_info(self) -> Dict[str, Dict]:
        ...
        """
        return {model_id: model.get_metadata() for model_id, model in self.models.items()}
