# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\mcts.py\chunked_mcts_95e0b3749943.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\mcts\chunked_mcts.py
...
class ChunkedMCTS(MCTS):
            ...
            for chunk, entities, achievements in zip(evaluated_chunks, entity_list, achievement_list):
                try:
                    chunk_program = Program(
                        ...
                        value=chunk.value
                        - (
                            abs(self.error_penalty) if "error" in chunk.response.lower() else 0
                    ...
                    chunk_program.id = hash(
                        (
                            chunk.code,
                            json.dumps(chunk_program.conversation.model_dump()["messages"]),
                        )
                    )
    ...
    async def _generate_programs_batch(
        ...
        programs = (await super()._generate_programs_batch(conversation, generation_params=generation_parameters))[
            :n_samples
        ]
