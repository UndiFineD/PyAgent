# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\diffyq.py\part2.py\pi_scenes_a243491729cd.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\diffyq\part2\pi_scenes.py
...
class ContrastPDEToODE(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        pde, ode = words = VGroup(
            *[OldTexText(text + "\\\\", "Differential\\\\", "Equation") for text in ("Partial", "Ordinary")]
        )
...
class AskAboutWhereEquationComesFrom(TeacherStudentsScene, WriteHeatEquationTemplate):
    def construct(self):
        ...
        self.play(FadeInFromDown(equation), self.teacher.change, "raise_right_hand")
...
class AskWhyRewriteIt(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("thinking", "thinking", "thinking", look_at=4 * LEFT + 2 * UP)
...
class ReferenceKhanVideo(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes("thinking", "pondering", "thinking", look_at=self.screen)
