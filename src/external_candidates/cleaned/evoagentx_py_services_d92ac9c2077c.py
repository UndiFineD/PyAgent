# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\app.py\services_d92ac9c2077c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\app\services.py
...
class AgentService:
    ...
    async def update_agent(agent_id: str, agent_data: AgentUpdate) -> Optional[Dict[str, Any]]:
        ...
        if "name" in update_data:
            ...
            existing = await Database.agents.find_one({"name": update_data["name"], "_id": {"$ne": ObjectId(agent_id)}})

            if existing:
                raise ValueError(f"Agent with name '{update_data['name']}' already exists")

        await Database.agents.update_one({"_id": ObjectId(agent_id)}, {"$set": update_data})
    ...
    async def list_agents(
        params: PaginationParams, search: Optional[SearchParams] = None
        ...
        cursor = Database.agents.find(query).sort("created_at", -1).skip(params.skip).limit(params.limit)
...
class WorkflowService:
    ...
    async def update_workflow(workflow_id: str, workflow_data: WorkflowUpdate) -> Optional[Dict[str, Any]]:
        ...
        if "name" in update_data:
            existing = await Database.workflows.find_one(
                {"name": update_data["name"], "_id": {"$ne": ObjectId(workflow_id)}}
            )

            if existing:
                raise ValueError(f"Workflow with name '{update_data['name']}' already exists")

        await Database.workflows.update_one({"_id": ObjectId(workflow_id)}, {"$set": update_data})
    ...
    async def list_workflows(
        ...
        cursor = Database.workflows.find(query).sort("created_at", -1).skip(params.skip).limit(params.limit)
...
class WorkflowExecutionService:
    ...
    async def update_execution_status(
        execution_id: str, status: ExecutionStatus, error_message: Optional[str] = None
        ...
        result = await Database.executions.find_one_and_update(
            {"_id": ObjectId(execution_id)}, {"$set": update_data}, return_document=True
        )
    ...
    async def list_executions(
        ...
        cursor = Database.executions.find(query).sort("created_at", -1).skip(params.skip).limit(params.limit)
    ...
    async def get_execution_logs(
        execution_id: str, params: PaginationParams = PaginationParams()
    ) -> Tuple[List[Dict[str, Any]], int]:
        ...
        cursor = Database.logs.find(query).sort("timestamp", 1).skip(params.skip).limit(params.limit)
