# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-prettymapp\prettymapp\plotting.py
import colorsys
...
class Plot:
    ...
    df: GeoDataFrame
    aoi_bounds: List[float]  # Not df bounds as could lead to weird plot shapes with unequal geometry distribution.
    ...
    def __post_init__(self):
        ...
        self.fig, self.ax = subplots(1, 1, figsize=(12, 12), constrained_layout=True, dpi=1200)
    ...
    def set_geometries(self):
        ...
        for lc_class in self.df["landcover_class"].unique():
            ...
            if lc_class == "streets":
                ...
                linewidth_values = list(df_class["highway"].map(STREETS_WIDTH).fillna(1))
                draw_settings_class["ec"] = draw_settings_class.pop("fc")
                linecollection = _plot_linestring_collection(ax=self.ax, geoms=df_class.geometry, **draw_settings_class)
