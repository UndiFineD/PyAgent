# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-yesitsme\yesitsme.py
import argparse
import hashlib
import hmac
import json
...
def getInfo(username, sessionId):
    ...
    if userId["error"] != None:
        ...
        response = get(
            "https://i.instagram.com/api/v1/users/" + userId["id"] + "/info/",
            headers=headers,
            cookies=cookies,
        )
...
def main():
    ...
    parser.add_argument("-n", "--name", help="Target name & surname", required=True)
    parser.add_argument("-e", "--email", help="Target email", required=True)
    parser.add_argument("-p", "--phone", help="Target phone number", required=True)
    parser.add_argument("-t", "--timeout", help="Timeout between requests", required=False)
        ...
        for account in accounts["user"]:
            ...
            if infos["user"] == None:
                ...
                if infos["full_name"].lower() == name.lower():
                    print(Fore.GREEN + "Full Name              : " + infos["full_name"] + " \u2713")
                    name_f = 1
                else:
                ...
                print("Followers              : " + str(infos["follower_count"]))
                print("Following              : " + str(infos["following_count"]))
                ...
                if "public_email" in infos.keys():
                    if infos["public_email"] != "":
                        if (
                            email != " "
                            and infos["public_email"][0] == email[0]
                            and infos["public_email"].split("@")[0][-1] == email.split("@")[0][-1]
                            and infos["public_email"].split("@")[1] == email.split("@")[1]
                        ):
                            print(Fore.GREEN + "Public email           : " + infos["public_email"] + " \u2713")
                            email_f = 1
                        else:
                ...
                if "public_phone_number" in infos.keys():
                    if str(infos["public_phone_number"]) != "":
                        if (
                            phone != " "
                            and str(infos["public_phone_number"]).split()[0] == phone.split()[0]
                            and str(infos["public_phone_number"])[-2:] == phone[-2:]
                        ):
                            print(
                                Fore.GREEN + "Public phone number    : " + str(infos["public_phone_number"]) + " \u2713"
                            )
                            phone_f = 1
                        else:
                            print("Public phone    : " + str(infos["public_phone_number"]))
                ...
                if other_infos["error"] == "rate limit":
                    ...
                    if other_infos["user"]["message"] == "No users found":
                        ...
                        sys.exit(Fore.RED + "Rate limit! Please, wait a few minutes before running the script again.")
                else:
                    if "obfuscated_email" in other_infos["user"].keys():
                        if other_infos["user"]["obfuscated_email"] != "":
                            if (
                                email != " "
                                and other_infos["user"]["obfuscated_email"][0] == email[0]
                                and other_infos["user"]["obfuscated_email"][8] == email.split("@")[0][-1]
                    ...
                    if "obfuscated_phone" in other_infos["user"].keys():
                        if str(other_infos["user"]["obfuscated_phone"]) != "":
                            if (
                                phone != " "
                                and str(other_infos["user"]["obfuscated_phone"].split()[0]) == phone.split()[0]
                                and str(other_infos["user"]["obfuscated_phone"])[-2:] == phone[-2:]
                            ):
                                print(
                                    Fore.GREEN
                                    + "Obfuscated phone       : "
            ...
            if name_f + email_f + phone_f == 3:
                print(Fore.CYAN + "[*] " + Fore.GREEN + "Profile ID " + infos["userID"] + " match level: HIGH\n")
                ...
                print(Fore.CYAN + "[*] " + Fore.YELLOW + "Profile ID " + infos["userID"] + " match level: MEDIUM\n")

            elif name_f + email_f + phone_f == 1:
                print(Fore.CYAN + "[*] " + Fore.RED + "Profile with ID " + infos["userID"] + " match level: LOW\n")
