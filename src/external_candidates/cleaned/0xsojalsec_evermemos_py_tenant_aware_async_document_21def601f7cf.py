# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\es.py\tenant_aware_async_document_21def601f7cf.py
...
class TenantAwareAsyncDocument(AliasSupportDoc):
    ...
    def _register_connection(cls, config: Dict[str, Any], using: str) -> None:
        ...
        try:
            ...
            logger.error("Failed to register Elasticsearch connection [using=%s]: %s", using, e)
...
def TenantAwareAliasDoc(doc_name: str, number_of_shards: int = 2) -> Type[TenantAwareAsyncDocument]:
    """
