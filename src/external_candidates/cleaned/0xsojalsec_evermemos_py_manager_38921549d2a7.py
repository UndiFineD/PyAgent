# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\profile_manager.py\manager_38921549d2a7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\profile_manager\manager.py
...
class ProfileManager:
    ...
    async def extract_profiles(
        ...
        if len(memcells) > self.config.batch_size:
            logger.warning(f"Got {len(memcells)} memcells, limiting to {self.config.batch_size} most recent")
