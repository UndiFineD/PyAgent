# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\falderebet.py\dirigera_control.py\scripts.py\helpers_fc4be2ef9ed1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\falderebet\dirigera-control\scripts\helpers.py
...
def init_hub() -> dirigera.Hub:
    ...
    return dirigera.Hub(token="token_goes_here", ip_address="ip_address_goes_here")


def get_lights_by_room(hub: dirigera.Hub, room_name: str) -> List:
    """Get all lights in a specific room"""

    return [light for light in hub.get_lights() if light.room and light.room.name == room_name]


def get_lights_by_state(hub: dirigera.Hub, on: bool = True) -> List:
    """Get all lights that are ON or OFF"""

    return [light for light in hub.get_lights() if light.attributes.is_on == on]


def get_outlets_by_state(hub: dirigera.Hub, on: bool = True) -> List:
    """Get all outlets that are ON or OFF"""

    return [outlet for outlet in hub.get_outlets() if outlet.attributes.is_on == on]


def get_unreachable_devices(hub: dirigera.Hub) -> List:
    """Get all offline/unreachable devices"""

    return [device for device in hub.get_all_devices() if not device.is_reachable]
...
def get_low_battery_controllers(hub: dirigera.Hub, threshold: int = 20) -> List:
    ...
    return [ctrl for ctrl in hub.get_controllers() if ctrl.attributes.battery_percentage < threshold]


def safe_light_control(
    hub: dirigera.Hub,
    light_name: str,
    turn_on: bool,
...
def safe_outlet_control(hub: dirigera.Hub, outlet_name: str, turn_on: bool) -> str:
    """
...
def status_report(hub: dirigera.Hub) -> dict:
    ...
    return {
        ...
        "controllers": {"total": len(all_controllers), "low_battery": len(low_battery)},
        "unreachable_devices": len(unreachable),
        "unreachable_list": [{"name": d.attributes.custom_name, "type": d.device_type} for d in unreachable],
        "low_battery_list": [
            {
                "name": c.attributes.custom_name,
                "battery": c.attributes.battery_percentage,
            }
