# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\di.py\tests.py\test_bean_order_strategy_835597d45234.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\di\tests\test_bean_order_strategy.py
...
class TestCalculateOrderKey:
    ...
    def test_mock_priority_in_mock_mode(self):
        ...
        mock_key = BeanOrderStrategy.calculate_order_key(mock_bean, is_direct_match=True, mock_mode=True)

        normal_key = BeanOrderStrategy.calculate_order_key(normal_bean, is_direct_match=True, mock_mode=True)
    ...
    def test_mock_priority_in_normal_mode(self):
        ...
        mock_key = BeanOrderStrategy.calculate_order_key(mock_bean, is_direct_match=True, mock_mode=False)

        normal_key = BeanOrderStrategy.calculate_order_key(normal_bean, is_direct_match=True, mock_mode=False)
    ...
    def test_direct_match_priority(self):
        ...
        direct_match_key = BeanOrderStrategy.calculate_order_key(bean, is_direct_match=True, mock_mode=False)

        impl_match_key = BeanOrderStrategy.calculate_order_key(bean, is_direct_match=False, mock_mode=False)
    ...
    def test_primary_priority(self):
        ...
        primary_key = BeanOrderStrategy.calculate_order_key(primary_bean, is_direct_match=True, mock_mode=False)
    ...
    def test_factory_scope_priority(self):
        ...
        factory_key = BeanOrderStrategy.calculate_order_key(factory_bean, is_direct_match=True, mock_mode=False)

        singleton_key = BeanOrderStrategy.calculate_order_key(singleton_bean, is_direct_match=True, mock_mode=False)

        prototype_key = BeanOrderStrategy.calculate_order_key(prototype_bean, is_direct_match=True, mock_mode=False)
    ...
    def test_comprehensive_priority_ordering(self):
        ...
        bean1 = BeanDefinition(ServiceA, is_mock=True, is_primary=True, scope=BeanScope.FACTORY)

        key1 = BeanOrderStrategy.calculate_order_key(bean1, is_direct_match=True, mock_mode=True)

        # Second highest priority: Mock + direct match + Primary + non-Factory

        bean2 = BeanDefinition(ServiceA, is_mock=True, is_primary=True, scope=BeanScope.SINGLETON)

        key2 = BeanOrderStrategy.calculate_order_key(bean2, is_direct_match=True, mock_mode=True)

        # Third priority: Mock + direct match + non-Primary + Factory

        bean3 = BeanDefinition(ServiceA, is_mock=True, is_primary=False, scope=BeanScope.FACTORY)

        key3 = BeanOrderStrategy.calculate_order_key(bean3, is_direct_match=True, mock_mode=True)

        # Fourth priority: Mock + implementation match + Primary + Factory

        bean4 = BeanDefinition(ServiceA, is_mock=True, is_primary=True, scope=BeanScope.FACTORY)

        key4 = BeanOrderStrategy.calculate_order_key(bean4, is_direct_match=False, mock_mode=True)

        # Fifth priority: non-Mock + direct match + Primary + Factory

        bean5 = BeanDefinition(ServiceA, is_mock=False, is_primary=True, scope=BeanScope.FACTORY)

        key5 = BeanOrderStrategy.calculate_order_key(bean5, is_direct_match=True, mock_mode=True)

        # Verify sort order - key1 has highest priority

        assert key1 < key2  # Factory has higher priority than non-Factory (same Mock+direct+Primary)

        assert key1 < key3  # Primary has higher priority than non-Primary (same Mock+direct+Factory)

        assert key1 < key4  # Direct match has higher priority than implementation match (same Mock+Primary+Factory)

        assert key1 < key5  # Mock has higher priority than non-Mock (same direct+Primary+Factory)

        # Verify secondary priorities - earlier positions in the tuple have higher weight

        assert key2 < key3  # (0,0,0,1) < (0,0,1,0): scope difference vs primary difference

        assert key3 < key4  # (0,0,1,0) < (0,1,0,0): primary difference vs match difference
...
class TestSortBeansWithContext:
    ...
    def test_factory_scope_sorting(self):
        ...
        bean_defs = [
            BeanDefinition(ServiceA, bean_name="singleton_a", scope=BeanScope.SINGLETON),
            BeanDefinition(ServiceB, bean_name="factory_b", scope=BeanScope.FACTORY),
            BeanDefinition(ServiceC, bean_name="prototype_c", scope=BeanScope.PROTOTYPE),
    ...
    def test_comprehensive_sorting(self):
        ...
        bean_defs = [
            ...
            BeanDefinition(ServiceC, bean_name="high1", is_primary=True, scope=BeanScope.SINGLETON),
            # Highest priority: direct match + Primary + Factory
            BeanDefinition(ServiceD, bean_name="highest", is_primary=True, scope=BeanScope.FACTORY),
            # Second highest priority: direct match + non-Primary + Factory
            BeanDefinition(ServiceA, bean_name="high2", is_primary=False, scope=BeanScope.FACTORY),
        ]
        ...
        assert sorted_beans[4].bean_name == "lowest"  # non-direct+non-Primary+non-Factory
    ...
    def test_complex_mock_mode_sorting(self):
        ...
        assert sorted_beans[1].bean_name == "high"  # Mock+direct+non-Primary+non-Factory

        assert sorted_beans[2].bean_name == "medium"  # Mock+non-direct+non-Primary+non-Factory

        assert sorted_beans[3].bean_name == "lowest"  # non-Mock+non-direct+non-Primary+non-Factory
...
class TestSortBeans:
    ...
    def test_primary_and_factory_combined(self):
        ...
        bean_defs = [
            ...
            BeanDefinition(ServiceB, bean_name="medium", is_primary=False, scope=BeanScope.FACTORY),
            # Higher priority: Primary + non-Factory
            BeanDefinition(ServiceC, bean_name="high", is_primary=True, scope=BeanScope.SINGLETON),
    ...
    def test_mock_beans_not_filtered_in_simple_sort(self):
        ...
        bean_defs = [
            BeanDefinition(ServiceA, bean_name="mock_a", is_mock=True, is_primary=False),
            BeanDefinition(ServiceB, bean_name="normal_b", is_mock=False, is_primary=True),
        ]
    ...
    def test_same_priority_beans(self):
        ...
        bean_defs = [
            BeanDefinition(ServiceA, bean_name="a", is_primary=False, scope=BeanScope.SINGLETON),
            BeanDefinition(ServiceB, bean_name="b", is_primary=False, scope=BeanScope.SINGLETON),
            BeanDefinition(ServiceC, bean_name="c", is_primary=False, scope=BeanScope.SINGLETON),
        ]
...
class TestEdgeCases:
    ...
    def test_multiple_beans_same_type(self):
        ...
        bean_defs = [
            BeanDefinition(ServiceA, bean_name="a1", is_primary=False, scope=BeanScope.SINGLETON),
            BeanDefinition(ServiceA, bean_name="a2", is_primary=True, scope=BeanScope.SINGLETON),
            BeanDefinition(ServiceA, bean_name="a3", is_primary=False, scope=BeanScope.FACTORY),
            BeanDefinition(ServiceA, bean_name="a4", is_primary=True, scope=BeanScope.FACTORY),
        ]
