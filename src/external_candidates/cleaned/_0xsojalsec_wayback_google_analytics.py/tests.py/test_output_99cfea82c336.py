# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wayback-google-analytics\tests\test_output.py
import json
import os
...
class OutputTestCase(TestCase):
    ...
    def test_init_output_valid_types(self, mock_datetime):
        ...
        mock_now = Mock(return_value=datetime.strptime(self.test_timestamp, "%d-%m-%Y(%H-%M-%S)"))
        mock_datetime.now = mock_now

        for type in self.valid_types:
            with self.subTest(type=type):
                expected_file_path = os.path.join(self.test_path, f"{self.test_timestamp}.{type}")
                ...
                if type == "csv":
                    self.assertTrue(os.path.exists(os.path.join(self.test_path, f"{self.test_timestamp}_codes.csv")))
                    self.assertTrue(os.path.exists(os.path.join(self.test_path, f"{self.test_timestamp}_urls.csv")))
                else:
    ...
    def test_get_codes_df_empty_list(self):
        ...
        self.assertEqual(
            ...
            expected_results.to_dict(orient="records"),
        )
        self.assertTrue(type(actual_results) is pd.DataFrame)
