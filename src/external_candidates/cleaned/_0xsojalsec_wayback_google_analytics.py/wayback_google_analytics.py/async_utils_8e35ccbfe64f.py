# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wayback-google-analytics\wayback_google_analytics\async_utils.py
import asyncio
import re

from wayback_google_analytics.codes import get_GA_code, get_GTM_code, get_UA_code
from wayback_google_analytics.utils import DEFAULT_HEADERS, get_date_from_timestamp
...
async def get_codes_from_snapshots(session, url, timestamps, semaphore=asyncio.Semaphore(10)):
    ...
    tasks = [
        get_codes_from_single_timestamp(session, base_url, timestamp, results, semaphore) for timestamp in timestamps
    ]
    await asyncio.gather(*tasks)

    for code_type in results:
        for code in results[code_type]:
            results[code_type][code]["first_seen"] = get_date_from_timestamp(results[code_type][code]["first_seen"])
            results[code_type][code]["last_seen"] = get_date_from_timestamp(results[code_type][code]["last_seen"])
...
async def get_codes_from_single_timestamp(session, base_url, timestamp, results, semaphore=asyncio.Semaphore(10)):
    ...
    async with semaphore:
        async with session.get(base_url.format(timestamp=timestamp), headers=DEFAULT_HEADERS) as response:
            try:
