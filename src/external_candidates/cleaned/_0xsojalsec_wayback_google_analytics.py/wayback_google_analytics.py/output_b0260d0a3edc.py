# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wayback-google-analytics\wayback_google_analytics\output.py
import json
import os
...
def init_output(type, output_dir="./output"):
    ...
    if type not in valid_types:
        raise ValueError(f"Invalid output type: {type}. Please use csv, txt, xlsx or json.")
            ...
            url_list.append(
                {
                    ...
                    "Archived_UA_Codes": format_archived_codes(info.get("archived_UA_codes", {})),
                    "Archived_GA_Codes": format_archived_codes(info.get("archived_GA_codes", {})),
                    "Archived_GTM_Codes": format_archived_codes(info.get("archived_GTM_codes", {})),
                }
            )
...
def get_codes_df(results):
    ...
    codes_df = codes_df.groupby("code").agg({"websites": lambda x: ", ".join(x), "active": format_active}).reset_index()
