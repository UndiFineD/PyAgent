# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\inference\registry.py
# -*- coding: utf-8 -*-
...
class BaseInference:
    ...
    def infer_model(self, cfg, module_paras=None):
        ...
        if module_paras is None:
            return module
        function_info = {}
        paras = {k.lower(): v for k, v in module_paras.get("PARAS", {}).items()}
    ...
    def load(self, module):
        if module["device"] == "offline":
            ...
            if (
                LazyImportModule.get_module_type(("MODELS", module["cfg"].NAME))
                or module["cfg"].NAME in MODELS.class_map
            ):
                model = MODELS.build(module["cfg"], logger=self.logger).eval()
            elif (
                LazyImportModule.get_module_type(("BACKBONES", module["cfg"].NAME))
                or module["cfg"].NAME in BACKBONES.class_map
            ):
                model = BACKBONES.build(module["cfg"], logger=self.logger).eval()
            elif (
                LazyImportModule.get_module_type(("EMBEDDERS", module["cfg"].NAME))
                or module["cfg"].NAME in EMBEDDERS.class_map
            ):
                model = EMBEDDERS.build(module["cfg"], logger=self.logger).eval()
            else:
    ...
    def dynamic_load(self, module=None, name=""):
        ...
        if name == "all":
            for subname in self.loaded_model_name:
                self.loaded_model[subname] = self.dynamic_load(getattr(self, subname), subname)
