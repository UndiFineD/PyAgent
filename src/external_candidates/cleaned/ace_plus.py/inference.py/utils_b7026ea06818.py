# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\inference\utils.py
# -*- coding: utf-8 -*-
...
class ACEPlusImageProcessor:
    ...
    def preprocess(
        ...
        if edit_image is None:
            ...
            edit_mask = (
                edit_mask.astype(np.float32) if np.any(edit_mask) else np.ones_like(edit_mask).astype(np.float32)
            )
        ...
        if reference_image is not None:
            ...
            edit_mask = torch.cat(
                [
                    torch.zeros([1, reference_image.shape[1], reference_image.shape[2]]),
                    edit_mask,
                ],
                dim=-1,
            )
