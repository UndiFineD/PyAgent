...
class DemoUI(object):
    ...
    def create_ui(self):
        with gr.Row(equal_height=True, visible=True):
            with gr.Column(scale=2):
                self.gallery_image = gr.Image(height=600, interactive=False, type="pil", elem_id="Reference_image")
        ...
        with gr.Row():
            self.generation_info_preview = gr.Markdown(label="System Log.", show_label=True)
        ...
        with gr.Accordion(label="Advance", open=True):
            ...
            with gr.Row():
                ...
                self.seed = gr.Slider(minimum=-1, maximum=10000000, value=-1, label="Seed")
    ...
    def set_callbacks(self, *args, **kwargs):
        ...
        self.chat_btn.click(run_chat, inputs=[self.text] + chat_inputs, outputs=chat_outputs, queue=True)

        self.text.submit(run_chat, inputs=[self.text] + chat_inputs, outputs=chat_outputs, queue=True)
        ...
        with self.eg:
            self.example_edit_image = gr.Image(label="Edit Image", type="pil", image_mode="RGB", visible=False)
            self.example_edit_mask = gr.Image(label="Edit Image Mask", type="pil", image_mode="L", visible=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server_port", dest="server_port", help="", type=int, default=2345)
