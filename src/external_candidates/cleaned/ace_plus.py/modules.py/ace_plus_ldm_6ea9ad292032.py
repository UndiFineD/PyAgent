...
class LatentDiffusionACEPlus(LatentDiffusion):
    ...
    def construct_network(self):
        ...
        if self.use_ema:
            if self.model_ema_config:
                self.model_ema = BACKBONES.build(self.model_ema_config, logger=self.logger)
            else:
        ...
        if self.first_stage_config:
            self.first_stage_model = MODELS.build(self.first_stage_config, logger=self.logger)
    ...
    def parse_ref_and_edit(
        ...
        for sample_id, (
            ...
            if hasattr(self, "ref_cond_stage_model") and self.ref_cond_stage_model:
                ref_image.append(
                    self.ref_cond_stage_model.encode_list([((i + 1.0) / 2.0 * 255).type(torch.uint8) for i in one_src])
                )
            else:
            ...
            # process modi image
            current_modify_image = to_device([one_modify[i] for i in one_edit_id], strict=False)
            current_modify_image = [v.squeeze(0) for v in self.encode_first_stage(current_modify_image)]
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        if check_list_of_list(image):
            ...
            x_mask = [
                ([self.reshape_func(i).squeeze(0) for i in im] if im is not None else [None] * len(ix))
            ...
            x_mask = (
                [self.reshape_func(i).squeeze(0) for i in image_mask] if image_mask is not None else [None] * len(image)
            )
    ...
    def forward_editing(
        ...
        **kwargs,
        ...
        model = self.model
        embedding_context = (
            model.no_sync if isinstance(model, torch.distributed.fsdp.FullyShardedDataParallel) else nullcontext
        )
        with embedding_context():
        ...
        for i in range(len(prompt)):
            ...
            if src_image is not None and src_image[i] is not None:
                ...
                for edit_img, modify_img, edit_mask in zip(src_image[i], modify_image_list[i], src_image_mask[i]):
                    ...
                    modify_img = torch.clamp((modify_img.float() + 1.0) / 2.0, min=0.0, max=1.0)
            ...
            if image is not None:
                ...
                one_tup["target_mask"] = (
                    image_mask[i] if image_mask[i] is not None else torch.ones_like(ori_img[[0], :, :])
                )
