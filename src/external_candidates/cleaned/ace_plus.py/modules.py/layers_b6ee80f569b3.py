# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\layers.py
# -*- coding: utf-8 -*-
...
class DoubleStreamBlockC(DoubleStreamBlock):
    ...
    def __init__(
        ...
        super().__init__(hidden_size, num_heads, mlp_ratio, qkv_bias, backend)
...
class SingleStreamBlockC(SingleStreamBlock):
    ...
    def __init__(
        ...
        super().__init__(hidden_size, num_heads, mlp_ratio, qk_scale, backend)
        self.abondon_cond = abondon_cond
...
class DoubleStreamBlockD(DoubleStreamBlock):
    ...
    def forward(
        ...
        if edit_length is not None:
            txt, edit, img = (
                x[:, :txt_length],
                x[:, txt_length : txt_length + edit_length],
                x[:, txt_length + edit_length :],
            )
        else:
            txt, img = x[:, :txt_length], x[:, txt_length:]
        img_mod1, img_mod2 = self.img_mod(vec)
        ...
        attn = attention(q, k, v, pe=pe, mask=mask, backend=self.backend)
        if edit_length is not None:
            txt_attn, edit_attn, img_attn = (
                attn[:, :txt_length],
                attn[:, txt_length : txt_length + edit_length],
                attn[:, txt_length + edit_length :],
            )
        else:
        ...
        if edit_length is not None:
            ...
            edit = edit + edit_mod2.gate * self.edit_mlp(
                (1 + edit_mod2.scale) * self.edit_norm2(edit) + edit_mod2.shift
            )
