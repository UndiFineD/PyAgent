...
class Flux(BaseModel):
    ...
    def get_config_template():
        return dict_to_yaml("BACKBONE", __class__.__name__, Flux.para_dict, set_name=True)
...
class FluxMR(Flux):
    ...
    @staticmethod
    def get_config_template():
        return dict_to_yaml("MODEL", __class__.__name__, FluxMR.para_dict, set_name=True)
...
class FluxMRACEPlus(FluxMR):
    ...
    def prepare_input(self, x, cond):
        ...
        batch_frames, batch_frames_ids = [], []
        for ix, shape, imask, ie, ie_mask in zip(x, cond["x_shapes"], cond["x_mask"], cond["edit"], cond["edit_mask"]):
            # unpack image from sequence
            ix = ix[:, : shape[0] * shape[1]].view(-1, shape[0], shape[1])
            ...
            if len(ie) > 0:
                ...
                ie_mask = [
                    (torch.ones((ix.shape[0] * 4, ix.shape[1], ix.shape[2])) if iime is None else iime.squeeze(0))
                    for iime in ie_mask
                ]
    ...
    @staticmethod
    def get_config_template():
        return dict_to_yaml("MODEL", __class__.__name__, FluxMRACEPlus.para_dict, set_name=True)
