...
class ACECheckpointHook(CheckpointHook):
    ...
    def after_iter(self, solver):
        ...
        if solver.total_iter != 0 and (
            (solver.total_iter + 1) % self.interval == 0 or solver.total_iter == solver.max_steps - 1
        ):
            from swift import SwiftModel

            if isinstance(solver.model, SwiftModel) or (
                hasattr(solver.model, "module") and isinstance(solver.model.module, SwiftModel)
            ):
                save_path = osp.join(
                    solver.work_dir,
                    "checkpoints/{}-{}".format(self.save_name_prefix, solver.total_iter + 1),
                )
                if we.rank == 0:
    ...
    def get_config_template():
        return dict_to_yaml("hook", __class__.__name__, ACECheckpointHook.para_dict, set_name=True)
...
class ACEBackwardHook(BackwardHook):
    def grad_clip(self, optimizer):
        for params_group in optimizer.param_groups:
            ...
            torch.nn.utils.clip_grad_norm_(parameters=train_params, max_norm=self.gradient_clip)
    ...
    @staticmethod
    def get_config_template():
        return dict_to_yaml("hook", __class__.__name__, ACEBackwardHook.para_dict, set_name=True)
