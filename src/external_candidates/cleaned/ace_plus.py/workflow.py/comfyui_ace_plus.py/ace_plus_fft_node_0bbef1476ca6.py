...
class AcePlusFFTProcessor:
    def __init__(self, max_aspect_ratio=4, d=16, max_seq_len=1024):
        ...
        self.transforms = T.Compose([T.ToTensor(), T.Normalize(mean=[0, 0, 0], std=[1.0, 1.0, 1.0])])
    ...
    def preprocess(
        ...
        if task_type in self.task_list:
            edit_image = self.edit_preprocess(self.task_list[task_type]["ANNOTATOR"], 0, edit_image, edit_mask)
