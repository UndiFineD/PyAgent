# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\coqui_ai_tts.py\tts.py\tts.py\layers.py\delightful_tts.py\acoustic_model_1f6831f88926.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\delightful_tts\acoustic_model.py
...
class AcousticModel(torch.nn.Module):
    ...
    def inference(
        ...
        mel_mask = ~sequence_mask(
            torch.tensor(
                [encoder_outputs_ex.shape[2]],
                dtype=torch.int64,
                device=encoder_outputs_ex.device,
            )
        )
