# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graph_r1.py\agent.py\llm_agent.py\generation_71ed967581e4.py
...
class ToolGenerationManager:
    ...
    def _process_tool_responses(self, tool_responses: List[str]) -> torch.Tensor:
        ...
        tool_responses_ids = self.tokenizer(tool_responses, padding="longest", return_tensors="pt")["input_ids"]
    ...
    def _update_rolling_state(self, rollings, cur_responses: torch.Tensor, tool_responses_ids: torch.Tensor) -> Dict:
    ...
    def run_llm_loop(
        ...
        for step in range(self.config.max_turns):
            ...
            rollings = self._update_rolling_state(rollings, responses_ids, tool_responses_ids)

            original_right_side = self._update_right_side(original_right_side, responses_ids, tool_responses_ids)
