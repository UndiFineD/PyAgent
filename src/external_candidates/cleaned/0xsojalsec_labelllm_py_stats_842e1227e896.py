# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_labelllm.py\backend.py\app.py\util.py\stats_842e1227e896.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LabelLLM\backend\app\util\stats.py
...
class Evaluation(BaseModel):
    ...
    question_evaluation: dict | None = Field(description="针对单条消息的评价", default=None)
    ...
    questionnaire_evaluation: QuestionnaireEvaluation | None = Field(description="针对整个问卷的评价", default=None)
            ...
            "$set": {
                "message_evaluation_stage3": {
                    "$cond": {
                        ...
                        "else": [{"__default_bottom_placeholder": "__default_bottom_placeholder"}],
                    }
                }
            }
        },
...
def build_agg_choice_count(scope: ANSWER_SCOPE, kind: CHOICE_KIND, choice_name: str) -> list[dict]:
                    ...
                    "$match": (
                        {
                            op: [
                                (
                                    {f"evaluation.{str_scope(opt.scope)}.{opt.question}": "".join(opt.answer)}
                                    if opt.type == CHOICE_KIND.ENUM
                                    else {
                                        "$and": [
                                            {
                                                f"evaluation.{str_scope(opt.scope)}.{opt.question}": {
                                                    "$all": [{"$elemMatch": {"$eq": answer}} for answer in opt.answer]
                                                }
                                            },
                                            {
                                                f"evaluation.{str_scope(opt.scope)}.{opt.question}": {
...
def filter_questionnaire(
    ...
    for idx, v in enumerate(records):
        ...
        questionnaire_eval = [vv.questionnaire_evaluation or None for vv in v.evaluations]
        ...
        for predicate in filters:
            ...
            is_invalid_questionnaires: list[bool] = [
                q.is_invalid_questionnaire if q is not None else False for q in questionnaire_eval
            ]
            ...
            answer_with_votes = [len(set(ans_by_group[key])) == len(set(data_ids)) for key in ans_by_group]
