# Extracted from: C:\DEV\PyAgent\.external\whatsapp-msgstore-viewer\decryption\dbs\WhatsAppCrypt14Crypt15Decrypter\decrypt14_15.py
#!/usr/bin/env python
"""
...
def parsecmdline() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--buffer-size",
        type=int,
        help="How many bytes of data to process at a time. Implies -nm. Default: {}".format(io.DEFAULT_BUFFER_SIZE),
    )
    parser.add_argument(
        "-ng",
        "--no-guess",
                ...
                if len(p.c15_iv.IV) != 0:
                    ...
                    if key.key_version != p.c14_cipher.key_version:
                        if key.key_version > p.c14_cipher.key_version:
                            ...
                            logger.e(
                                "Key version mismatch: {} != {} (?)".format(key.key_version, p.c14_cipher.key_version)
                            )
