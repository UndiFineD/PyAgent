# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_guardian_cli.py\core.py\analyst_agent_4e65498f2f7f.py
...
class AnalystAgent(BaseAgent):
    ...
    async def interpret_output(self, tool: str, target: str, command: str, output: str) -> Dict[str, Any]:
        """
        ...
        prompt = ANALYST_INTERPRET_PROMPT.format(tool=tool, target=target, command=command, output=output)
    ...
    async def correlate_findings(self) -> Dict[str, Any]:
        ...
        prompt = ANALYST_CORRELATION_PROMPT.format(target=self.memory.target, tool_results=tool_results)
