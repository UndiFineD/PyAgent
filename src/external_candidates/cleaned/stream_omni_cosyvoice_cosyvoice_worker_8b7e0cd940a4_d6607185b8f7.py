...
def speech_to_token(audio_file):
    ...
    speech_16k = torchaudio.transforms.Resample(orig_freq=sample_rate, new_freq=16000)(
        speech
    )
...
def token_to_speech(tokens):
    tts_speech_token = (
        torch.tensor(extract_speech_token(tokens)).long().unsqueeze(0).to(device)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Path to CosyVoice model"
    )
    parser.add_argument(
        "--wav_dir", type=str, required=True, help="Directory to save generated audio"
    )
