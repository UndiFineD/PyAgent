# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-waf-bypass\bypass.py
#!/usr/bin/env python3
...
def get_delimiter(p):
    try:
        ...
        print(
            "{}An error occurred while processing path delimiter for {}: {}{}".format(Fore.RED, p, e, Style.RESET_ALL)
        )
        return "/"
...
def payload_encoding(z, payload, encode):
    try:
                ...
                if not data_list:
                    ...
                    for item in data_list:
                        ...
                        # value only
                            ...
                            res.append(quote_plus(escape(str(item[0]))))
                    return "&".join(res)
                ...
                if not data_list:
                    ...
                    for item in data_list:
                        ...
                        # value only
                            ...
                            res.append("".join([hex(ord(x)).replace("0x", "\\u00") for x in str(item[0])]))
                    return "&".join(res)
                ...
                if not data_list:
                    ...
                    for item in data_list:
                        ...
                        # value only
                            ...
                            res.append(base64.b64encode(str(item[0]).encode("UTF-8")).decode("UTF-8"))
                    return "&".join(res)

            else:
                print(
                    "{}An error occurred while encoding payload ({}) with {}: incorrect encoding type{}".format(
                        Fore.YELLOW, payload, encode, Style.RESET_ALL
                    )
                )
                return payload
        ...
        print(
            "{}An error occurred while encoding payload ({}) with {}: {}{}".format(
                Fore.YELLOW, payload, encode, e, Style.RESET_ALL
            )
        )
        return payload
...
class WAFBypass:
    ...
    def start(self):
            ...
            try:
                ...
                if not payload:
                    ...
                    if self.wb_result_json:
                        self.wb_result["FAILED"].append(
                            {json_path.split(pdl + "payload" + pdl, 1)[1]: "No payloads found"}
                        )
                ...
                # MFD (multipart/form-data) dir processing
                    ...
                    if payload["BODY"]:
                        ...
                        print(
                            "{}An error occurred while processing payload from file {}: empty BODY{}".format(
                                Fore.YELLOW, json_path, Style.RESET_ALL
                            )
                        )
                        return
        ...
        for dir_path, _, filenames in os.walk(work_dir_payload):
            for filename in filenames:
                ...
                all_files_list.append(os.path.join(dir_path, filename))
    ...
    def test_resp_status_processing(self, k, v):
        try:
            ...
            print("{}An error occurred while processing test's status: {}{}".format(Fore.YELLOW, e, Style.RESET_ALL))
    ...
    def test_err_resp_code_processing(self, json_path, z, encode, result):
        try:
            if self.wb_result_json:
                ...
                self.wb_result["FAILED"].append({json_path.split(pdl + "payload" + pdl, 1)[1] + " in " + z: err})
            else:
                err = "An incorrect response was received while processing request from file {} in {}: {}".format(
                    json_path, z, result[1]
                )
                print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))

        except Exception as e:
            print(
                "{}An error occurred while processing test's error response code: {}{}".format(
    ...
    def test_fail_response_processing(self, json_path, z, encode, error):
        try:
            ...
            print(
                "{}An error occurred while processing test's fail response code: {}{}".format(
                    Fore.YELLOW, e, Style.RESET_ALL
                )
            )
    ...
    def test_noblocked(self, method, headers):
        try:
            ...
            if result[0] == "PASSED":
                ...
                print(
                    "{}An incorrect response was received while processing test request to {}: {}{}".format(
                        Fore.YELLOW, self.host, result[1], Style.RESET_ALL
                    )
                )
            else:
                print(
    ...
    def test_header(self, json_path, z, payload, method, headers, encode):
        try:
            ...
            headers = {
                **self.headers,
                **headers,
                f"WBH-{secrets.token_hex(3)}": encoded_payload,
            }
