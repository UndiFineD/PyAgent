# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\oxm.py\es.py\migration.py\utils_8c84cf3b4731.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\es\migration\utils.py
...
def find_document_class_by_index_name(index_name: str) -> Type[AsyncDocument]:
    ...
    if not matched_classes:
        ...
        logger.error("Cannot find document class corresponding to index alias '%s'", index_name)

        logger.info("Available index aliases: %s", ", ".join(available_indexes))

        raise ValueError(f"Cannot find document class corresponding to index alias '{index_name}'")
    ...
    if not hasattr(document_class, "PATTERN"):
        raise ValueError(f"Document class {document_class.__name__} must have PATTERN attribute")

    if not hasattr(document_class, "dest"):
        raise ValueError(f"Document class {document_class.__name__} must have dest() method")
...
async def rebuild_index(
    ...
    if not hasattr(document_class, "PATTERN"):
        raise ValueError("Document class %s must have PATTERN attribute" % document_class.__name__)

    if not hasattr(document_class, "dest"):
        raise ValueError("Document class %s must have dest() method" % document_class.__name__)
    ...
    if await es_connect.indices.exists(index=dest_index):
        logger.warning("Destination index %s already exists, skipping rebuild", dest_index)
        ...
        try:
            ...
            if status:
                ...
                if total > 0:
                    ...
                    logger.info("Rebuild progress: %d/%d (%.1f%%)", created, total, progress)
...
async def update_aliases(
    ...
    try:
        existing_indices = list((await es_connect.indices.get_alias(name=alias_name)).keys())

        logger.info("Current indices pointed by alias %s: %s", alias_name, existing_indices)
    ...
    actions.append({"add": {"alias": alias_name, "index": dest_index, "is_write_index": True}})
