# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\test_model_factory_errors_04e5712b24bb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\test_model_factory_errors.py
...
class TestModelFactoryErrors:
    ...
    def test_get_model_invalid_name(self):
        ...
        with pytest.raises(ValueError, match="Model 'nonexistent-model-xyz' not found in configuration"):
    ...
    def test_unsupported_model_type(self):
        ...
        with pytest.raises(ValueError, match="Unsupported model type: unsupported-type"):
    ...
    def test_azure_openai_missing_required_configs(self):
        ...
        with pytest.raises(ValueError, match="Azure OpenAI model type requires 'azure_endpoint'"):
        ...
        with pytest.raises(ValueError, match="Azure OpenAI model type requires 'api_version'"):
        ...
        with pytest.raises(ValueError, match="Azure OpenAI model type requires 'api_key'"):
    ...
    def test_custom_endpoint_missing_custom_endpoint_config(self):
        ...
        with pytest.raises(ValueError, match="Custom model requires 'custom_endpoint' configuration"):
    ...
    def test_round_robin_missing_models_list(self):
        ...
        with pytest.raises(ValueError, match="Round-robin model 'rr-bad' requires a 'models' list"):
    ...
    def test_model_instantiation_errors_missing_api_keys(self):
        ...
        with patch("code_puppy.model_factory.get_api_key", return_value=None):
            ...
            config_anthropic = {"anthropic-test": {"type": "anthropic", "name": "claude-3"}}
            ...
            config_openrouter = {"openrouter-test": {"type": "openrouter", "name": "anthropic/claude-3"}}
                        ...
                        with patch(
                            ...
                            with patch(
                                ...
                                def path_side_effect(path_arg):
                                    ...
                                    elif "models.json" in path_str and "extra" not in path_str:
                                ...
                                with patch(
                                    ...
                                    with patch("json.load") as mock_json_load:
                                        ...
                                        mock_json_load.side_effect = [
                                            json.loads(valid_models_json),  # Success for main config
                                            Exception("General error"),  # Fail for extra models
                                        ]
                                        ...
                                        with patch("logging.getLogger") as mock_logger:
                                            ...
                                            warning_call_args = mock_logger.return_value.warning.call_args[0]

                                            assert "Failed to load" in warning_call_args[0]
    ...
    def test_invalid_model_config_structure(self):
        ...
        with pytest.raises(ValueError, match="Model 'bad-model' not found in configuration"):
