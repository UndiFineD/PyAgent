# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\ai_eng.py\feathr_project.py\test.py\test_feature_registry_cf8041cb4412.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\test\test_feature_registry.py
...
from test_fixture import (
    ...
    registry_test_setup_append,

    registry_test_setup_for_409,
...
class FeatureRegistryTests(unittest.TestCase):
        ...
        for config_path in config_paths:
            ...
            with self.subTest(config_path=config_path):
                ...
                test_workspace_dir = (
                    ...
                    Path(__file__).parent.resolve() / "test_user_workspace"

                )

                client: FeathrClient = registry_test_setup(

                    os.path.join(test_workspace_dir, config_path)
                ...
                assert (
                    ...
                    "f_is_long_trip_distance" in all_feature_names

                )  # test regular ones

                assert (

                    "f_trip_time_rounded" in all_feature_names

                )  # make sure derived features are there

                assert (

                    "f_location_avg_fare" in all_feature_names

                )  # make sure aggregated features are there

                assert (

                    "f_trip_time_rounded_plus" in all_feature_names

                )  # make sure derived features are there

                assert (

                    "f_trip_time_distance" in all_feature_names
    ...
    def test_feathr_register_features_partially(self):
        ...
        full_registration, keys = client.get_features_from_registry(

            client.project_name, return_keys=True, verbose=True
        ...
        client: FeathrClient = registry_test_setup_append(
    ...
    def test_get_feature_from_registry(self):
        ...
        inputs = registry.search_input_anchor_features(
            ...
            entity_array_to_dict(
                ...
                + [derived_feature_with_multiple_inputs, hierarchical_derived_feature]
        ...
        assert (
            ...
            "input_anchorA" in inputs

            and "input_anchorB" in inputs

            and "input_anchorC" in inputs
