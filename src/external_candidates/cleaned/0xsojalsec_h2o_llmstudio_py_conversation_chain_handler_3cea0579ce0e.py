# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\src.py\datasets.py\conversation_chain_handler_3cea0579ce0e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\datasets\conversation_chain_handler.py
...
class ConversationChainHandler:
    ...
    def get_conversation_chain_ids(self, cfg, df):
        ...
        sample_ids = df[cfg.dataset.id_column].astype(df[cfg.dataset.parent_id_column].dtype).tolist()
        ...
        id2parent_id = {
            ...
            and (not isinstance(parent_id, float) or (not np.isnan(parent_id) and not np.isinf(parent_id)))
        }
        ...
        if cfg.dataset.limit_chained_samples:
            ...
            conversation_end_ids = [idx for idx in sample_ids if idx not in valid_parent_ids]

        else:
            conversation_end_ids = sample_ids

        conversation_chain_ids = [
            self.get_conversation_ids(id2parent_id, conversation_end_id) for conversation_end_id in conversation_end_ids
        ]
    ...
    def get_systems(self, cfg, df):
        if cfg.dataset.system_column != "None":
            if cfg.dataset.system_column not in df.columns:
                logger.warning(f"System column '{cfg.dataset.system_column}' not found. Disabling functionality.")
    ...
    def get_conversation_end_ids(self):
        ...
        return [conversation_chain[-1] for conversation_chain in self.conversation_chain_ids]
