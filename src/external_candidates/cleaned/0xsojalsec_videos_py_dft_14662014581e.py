# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2020.py\_18s191.py\dft_14662014581e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2020\18S191\dft.py
...
class IntroduceDFT(Scene):
    def construct(self):
        ...
        vectors = VGroup(*(Arrow(axes.c2p(x, 0), axes.c2p(x, signal[x]), buff=0) for x in range(N)))
        ...
        s_labels = VGroup(*(OldTex("s", f"[{x}]") for x in range(N)))
        ...
        self.play(
            ...
            LaggedStart(*(Transform(s_title[1].copy(), label[1].copy(), remover=True) for label in s_labels)),
            lag_ratio=0.3,
            run_time=2,
        )
        ...
        self.play(FadeOut(s_title, UP), Write(lhs), Write(plusses), MoveToTarget(s_labels))
        ...
        zeta_power_vectors = VGroup(*(Arrow(plane.n2p(0), plane.n2p(zeta ** (-n)), buff=0) for n in range(N)))
        ...
        zeta_label = OldTex("\\zeta = e^{-2\\pi i / N}")
        ...
        for z_power, vect in zip(zeta_powers, zeta_power_vectors):
            ...
            z_power.target.next_to(vect, np.round(vect.get_vector(), 2), buff=SMALL_BUFF)
        ...
        self.play(
            ...
            plane.coordinate_labels.set_opacity,
            0,
        )
        ...
        side_axes = Axes((0, 8), (0, 3), width=3, height=2, axis_config={"include_tip": False})
        ...
        formula = OldTex("\\hat s[f] = \\sum_{n=0}^{N - 1} s[n] \\zeta^{f \\cdot n}")
        ...
        for omega in range(3, 8):
            ...
            self.play(*anims, FadeIn(new_count, 0.5 * UP), FadeOut(omega_label[1], 0.5 * UP))
