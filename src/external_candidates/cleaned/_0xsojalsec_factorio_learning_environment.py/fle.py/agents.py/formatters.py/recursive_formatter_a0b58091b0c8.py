# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\agents\formatters\recursive_formatter.py
import hashlib
import json
import os
import re
...
class RecursiveFormatter(ConversationFormatter):
    ...
    async def format_conversation(self, conversation: Conversation) -> List[Message]:
        ...
        if len(messages) <= self.chunk_size:
            return [
                self._truncate_entity_data(msg, is_recent=(i >= len(messages) - 1)) for i, msg in enumerate(messages)
            ]
