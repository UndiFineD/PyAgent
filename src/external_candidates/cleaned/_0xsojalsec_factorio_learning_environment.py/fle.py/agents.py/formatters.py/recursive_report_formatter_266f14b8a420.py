# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\agents\formatters\recursive_report_formatter.py
import copy
import hashlib
import json
import os
...
class RecursiveReportFormatter(ConversationFormatter):
    ...
    async def _generate_summary(self, messages: List[Message], previous_report: str, last_summary_step: int) -> Message:
        ...
        if last_summary_step == 0:
            ...
            steps = (
                f"Here is the report from step 0 until step {int(last_summary_step / 2) - 1}\n\n{previous_report}\n\n"
            )
