# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\agents\data\sprites\extractors\trees.py
#!/usr/bin/env python3
"""
...
class TreeSpriteExtractor:
        ...
        for (tree_num, variation, is_hr), layers in tree_files.items():
            try:
                ...
                if "leaves" in layers:
                    ...
                    if aspect_ratio > 2:
                        print(f"Processing triptych leaves for tree {tree_num}-{variation}")
                        ...
                        # Single leaves image
                        print(f"Processing single leaves for tree {tree_num}-{variation}")
                        if "trunk" in layers:
                            trunk_img = Image.open(layers["trunk"]).convert("RGBA")
                            self._process_single_leaves(leaves_img, trunk_img, tree_num, variation, prefix)
                        else:
                ...
                if "shadow" in layers:
                    self._extract_shadow_states(layers["shadow"], tree_num, variation, prefix)
    ...
    def _process_triptych_leaves(
        ...
        try:
            ...
            for i, state in enumerate(foliage_states):
                ...
                print(f"  Extracting {state} from region: ({left}, {upper}, {right}, {lower})")
                ...
                if sprite_img.width == triptych_width:
                    print("  ERROR: Sprite still has triptych width! Trying alternative method...")
                ...
                print(f"  Extracted {state} sprite: {sprite_img.width}x{sprite_img.height}")
                ...
                if trunk_img:
                    ...
                    final_composite = Image.new("RGBA", (composite_width, composite_height), (0, 0, 0, 0))
                    ...
                    final_composite.paste(sprite_img, (foliage_x, foliage_y), sprite_img)
            ...
            if not extraction_failed:
                ...
                for state, (output_name, img) in temp_results.items():
                    ...
                    print(f"  Saved tree state: {output_path} (size: {img.width}x{img.height})")

                # Add trunk-only state if we have trunk
                if trunk_img:
                    # Create image with same dimensions as other sprites
                    trunk_only = Image.new("RGBA", (composite_width, composite_height), (0, 0, 0, 0))
                    trunk_x = (composite_width - trunk_img.width) // 2
                    trunk_y = (composite_height - trunk_img.height) // 2
                    ...
                    print(f"  Saved trunk-only state: {output_path} (size: {trunk_only.width}x{trunk_only.height})")
            else:
                print(f"  WARNING: Skipping tree {tree_num}-{variation} due to extraction failure")
    ...
    def _extract_shadow_states(self, shadow_path: Path, tree_num: str, variation: str, prefix: str):
        ...
        try:
            ...
            for i, state in enumerate(shadow_states):
                ...
                shadow = shadow_quad.crop((x_offset, 0, x_offset + shadow_width, shadow_height))
