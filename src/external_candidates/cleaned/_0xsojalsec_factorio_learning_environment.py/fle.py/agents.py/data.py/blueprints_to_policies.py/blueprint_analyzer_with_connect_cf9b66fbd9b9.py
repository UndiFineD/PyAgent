# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\agents\data\blueprints_to_policies\blueprint_analyzer_with_connect.py
import json
import os
...
from fle.env import EntityGroup, FactorioInstance
...
class BlueprintAnalyzerWithConnect:
    ...
    def find_patterns(self):
        ...
        for (name_dir, x), entities in vertical_groups.items():
            if len(entities) >= 3:
                ...
                if all(
                    round((entities[i + 1].position["y"] - entities[i].position["y"]) * 2) / 2 == step
        ...
        for (name_dir, y), entities in horizontal_groups.items():
            if len(entities) >= 3:
                ...
                if all(
                    round((entities[i + 1].position["x"] - entities[i].position["x"]) * 2) / 2 == step
                    for i in range(len(entities) - 1)
                ):
    ...
    def generate_program(self) -> str:
        ...
        if miners:
            origin_calc = (
                f"game.nearest_buildable({self._name_to_prototype_string(miners[0].name)}, bounding_box=miner_box)"
            )
        else:
        ...
        for entity in singles:
            ...
            if entity.recipe:
                lines.append(f"game.set_entity_recipe({var_name}, Prototype.{prototype_by_name[entity.recipe].name})")
                lines.append("")
    ...
    for filename in os.listdir(execution_dir):
        if filename.endswith(".json"):
            ...
            with open(execution_dir + filename, "r") as f:
                ...
                with open(
                    execution_dir + filename.split(".json")[0] + " with connect" + ".py",
                    "w",
                ) as f1:
                    f1.write(code)
