# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\commons\db_client.py
import json
import logging
import math
import os
...
class DBClient(ABC):
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    weights = [(row["id"], transform_reward(row["advantage"])) for row in results]
    ...
    ) -> tuple[Optional[GameState], Optional[Conversation], Optional[int], Optional[int]]:
        ...
        if self._pool is None:
            async with self._lock:
                if self._pool is None:  # Double check pattern
                    self._pool = ThreadedConnectionPool(self.min_connections, self.max_connections, **self.db_config)
