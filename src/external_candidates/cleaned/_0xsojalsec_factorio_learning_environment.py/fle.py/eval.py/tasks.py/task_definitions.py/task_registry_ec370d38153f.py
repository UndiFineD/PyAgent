# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\tasks\task_definitions\task_registry.py
"""
...
class TaskRegistry:
    ...
    def get_task_config(self, task_key: str) -> BaseModel:
        ...
        if task_key not in self._all_tasks:
            ...
            raise KeyError(
                f"Unknown task: {task_key}. Available tasks: {', '.join(available[:5])}... ({len(available)} total)"
            )
