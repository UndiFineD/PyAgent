# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\analysis\analyze_sweep_results.py
"""
...
async def analyze_sweep_by_id(sweep_id: str):
    ...
    try:
        ...
        if not basic_df.empty:
            ...
            print(f"Version descriptions: {basic_df['version_description'].unique().tolist()}")
            ...
            trajectory_summaries = await analyzer.get_trajectory_summaries_by_sweep(sweep_id)
        ...
        if not trajectory_summaries.empty:
            ...
            available_cols = [col for col in display_cols if col in trajectory_summaries.columns]
            print(trajectory_summaries[available_cols].head(10))

            # Success rate by model
            success_by_model = trajectory_summaries.groupby("model")["success"].agg(["mean", "count"])
            print("\n📈 Success Rate by Model:")
            ...
            if "task" in trajectory_summaries.columns:
                success_by_task = trajectory_summaries.groupby("task")["success"].agg(["mean", "count"])
                print("\n🎯 Success Rate by Task:")
            ...
            for model in trajectory_summaries["model"].unique():
                model_data = trajectory_summaries[trajectory_summaries["model"] == model]
                print(f"\n  {model}:")
                print(f"    Trajectories: {len(model_data)}")
                print(f"    Success Rate: {model_data['success'].mean():.2%}")
                print(f"    Mean Reward: {model_data['max_reward'].mean():.2f} ± {model_data['max_reward'].std():.2f}")
                print(f"    Reward Range: [{model_data['max_reward'].min():.1f}, {model_data['max_reward'].max():.1f}]")
            ...
            trajectory_summaries["task"] = trajectory_summaries["version_description"].str.extract(r"type:([^\\n]+)")

            if "task" in trajectory_summaries.columns:
                print("Task Performance Breakdown:")
                for task in trajectory_summaries["task"].unique():
                    task_data = trajectory_summaries[trajectory_summaries["task"] == task]
                    if len(task_data) > 0:
                        print(f"\n  {task}:")
            ...
            version_results = await analyzer.db_client.execute_query(version_query, (sweep_id,))
            ...
            if version_results:
                ...
                for _, row in trajectory_summaries.iterrows():
                    ...
                    if task == "unknown" and "version_description" in row:
                        task_match = pd.Series([row["version_description"]]).str.extract(r"type:([^\n]+)")
                        task = task_match.iloc[0, 0] if not task_match.isna().iloc[0, 0] else "unknown"
                    ...
                    version_infos_data.append(
                        {
                            ...
                            "completion_status": ("completed" if row.get("success", False) else "partial"),
...
async def compare_models(models: List[str], task_pattern: Optional[str] = None, min_trajectories: int = 5):
    ...
    try:
        ...
        detailed_results = {}
        for model in models:
            model_results = await analyzer.get_results_by_model_and_task(model=model, task_pattern=task_pattern or "")
            if not model_results.empty:
                trajectory_summary = await analyzer.get_trajectory_summaries(model_results["version"].unique().tolist())
                model_trajectory_data = trajectory_summary[trajectory_summary["model"] == model]
...
async def analyze_task_difficulty(model: Optional[str] = None):
    ...
    try:
        ...
        print(f"  Hardest task: {task_df.iloc[0]['task_name']} ({task_df.iloc[0]['success_rate']:.1%} success)")
        print(f"  Easiest task: {task_df.iloc[-1]['task_name']} ({task_df.iloc[-1]['success_rate']:.1%} success)")
...
async def create_comprehensive_report(sweep_id: str, output_dir: str = "./analysis_results"):
    ...
    try:
        ...
        if "version_description" in trajectory_df.columns and "task" not in trajectory_df.columns:
            trajectory_df = trajectory_df.copy()
            trajectory_df["task"] = trajectory_df["version_description"].str.extract(r"type:([^\n]+)")
        ...
        if visualizer.enabled:
            ...
            print("Visualization tools not available - install matplotlib and seaborn for plots")
                ...
                version_infos_data.append(
                    {
                        ...
                        "max_production_score": 0,  # Not available in trajectory summaries
                        "completion_status": ("completed" if row.get("final_reward", 0) > 0 else "partial"),
        ...
        results_summary = {
            ...
            "model_metrics": {model_name: metrics.to_dict() for model_name, metrics in model_metrics.items()},
        }
...
async def monitor_ongoing_sweep(
    ...
    try:
        while True:
            ...
            recent_df = await analyzer.get_recent_results(hours=2 * check_interval_minutes / 60)
            ...
            if not recent_df.empty:
                ...
                if len(versions) > 0:
                    trajectory_df = await analyzer.get_trajectory_summaries(versions.tolist())
                    ...
                    if not trajectory_df.empty:
                        ...
                        for model in trajectory_df["model"].unique():
                            ...
                            print(f"  {model}: {len(model_data)} trajectories, {success_rate:.1%} success")
            else:
...
if __name__ == "__main__":
    ...
    if command == "sweep":
        sweep_id = sys.argv[2] if len(sys.argv) > 2 else "test_sweep_small_20250904_151635_2e06c621"
