# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\analysis\results_visualizer.py
"""
...
class ResultsVisualizer:
    ...
    def plot_reward_distribution(
        ...
        ax.set_title(title or f"Reward Distribution by {group_by.replace('_', ' ').title()}")
    ...
    def plot_efficiency_scatter(
        ...
        for group in results_df[color_by].unique():
            ...
            ax.scatter(group_data[x_col], group_data[y_col], label=group, alpha=0.7, s=50)
    ...
    def plot_learning_curves(
        ...
        for group in results_df[group_by].unique():
            ...
            try:
                ...
                agg_data = aggregate_results_by_time(group_data, time_col, metric_col, window)
    ...
    def plot_task_difficulty_heatmap(
        ...
        for i, model in enumerate(models):
            for j, task in enumerate(tasks):
                subset = results_df[(results_df[model_col] == model) & (results_df["task_name"] == task)]
                if len(subset) > 0:
        ...
        # Create heatmap
        fig, ax = plt.subplots(figsize=(max(10, len(tasks) * 0.8), max(6, len(models) * 0.5)), dpi=self.dpi)
    ...
    def plot_task_success_classification_heatmap(
        ...
        model_scores = {}
        for model in models:
            full_successes = sum(1 for task in tasks if model_task_data.get((model, task), 0) == 2)
        ...
        ax.set_title(title or "Task Success Classification by Model", fontsize=14, pad=20)
        ...
        for i, task in enumerate(tasks):
            ...
            for model in models:
                ...
                if len(model_task_data) >= 2:
                    ...
                    for k in k_values:
                        if k <= len(successes):
                            pass_k = PerformanceAnalyzer.calculate_pass_at_k(successes, k)
    ...
    def create_comprehensive_report(
        ...
        if "version_description" in results_df.columns:
            ...
            if "task" not in results_df.columns:
                results_df = results_df.copy()
                results_df["task"] = results_df["version_description"].str.extract(r"type:([^\n]+)")
            task_col = "task"
        ...
        try:
            if "final_reward" in results_df.columns:
                fig = self.plot_reward_distribution(results_df, title="Reward Distribution by Model")
                if fig:
        ...
        try:
            if "total_tokens" in results_df.columns and "final_reward" in results_df.columns:
                fig = self.plot_efficiency_scatter(results_df, title="Efficiency: Reward vs Token Usage")
                if fig:
        ...
        try:
            if "version_description" in results_df.columns:
                fig = self.plot_task_difficulty_heatmap(results_df, title="Task Difficulty Heatmap")
                if fig:
        ...
        try:
            if "version_description" in results_df.columns:
                ...
                if fig:
                    file_path = output_path / f"{report_name}_task_classification_heatmap.png"
                    fig.savefig(file_path, bbox_inches="tight", dpi=self.dpi)
                    plt.close(fig)
        ...
        try:
            if "created_at" in results_df.columns:
                fig = self.plot_learning_curves(results_df, title="Performance Over Time")
                if fig:
