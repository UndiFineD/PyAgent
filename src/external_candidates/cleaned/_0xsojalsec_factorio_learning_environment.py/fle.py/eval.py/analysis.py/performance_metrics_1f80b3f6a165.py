# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\analysis\performance_metrics.py
"""
...
class PerformanceAnalyzer:
    ...
    def calculate_metrics(
        ...
        if token_column and token_column in df.columns:
            ...
            if success_count > 0:
                tokens_per_success = float(df[df[reward_column] > success_threshold][token_column].mean())
    ...
    @staticmethod
    def _empirical_pass_at_k(successes: np.ndarray, k: int, num_samples: int = 10000) -> float:
