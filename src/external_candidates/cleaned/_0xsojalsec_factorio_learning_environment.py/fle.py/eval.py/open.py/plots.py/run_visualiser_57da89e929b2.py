# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\open\plots\run_visualiser.py
import asyncio
import json
import os
import sys
                        ...
                        if item_name not in seen_items:
                            self.achievements[version].append(
                                Achievement(
                                    ...
                                    quantity=quantity,
                                    ingredients=(
                                        self.count_total_ingredients(recipes[item_name]) if item_name in recipes else 0
                                    ),
                                    is_dynamic=False,
                                )
                            )
...
class MatplotlibRunVisualizer(BaseRunVisualizer):
    ...
    def export_visualization(self, output_file: str, max_depth: int = 100, xmin=0, xmax=10**6):
        ...
        for i, (version, label, achievements, color, gdp) in enumerate(version_achievements):
            ...
            achievement_positions = self.organize_achievement_positions(achievements_by_x, stats, ax, i, used_positions)
...
class BootstrappedRunVisualizer(MatplotlibRunVisualizer):
    ...
    def calculate_version_stats(self, version: int, max_depth: int):
        if self.plot_type == "scatter" and self.x_axis == "ticks":
            ...
            def traverse_tree(node, run_index, depth=0, path_value=0, path_ticks=0):
                ...
                values_by_x_by_run[x_coord][run_index] = max(values_by_x_by_run[x_coord][run_index], total_value)
    ...
    def smooth_data(self, x_values, y_values, window_size):
        ...
        for i in range(len(y_values)):
            ...
            if self.x_axis == "ticks":
                center_x = x_values[i]
                valid_window_indices = [j for j, x in enumerate(window_x) if 0.1 <= x / center_x <= 10]
