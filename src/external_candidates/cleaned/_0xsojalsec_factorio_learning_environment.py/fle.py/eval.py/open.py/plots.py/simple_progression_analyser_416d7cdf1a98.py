# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\open\plots\simple_progression_analyser.py
import asyncio
import os
import pickle
...
class ProgressionAnalyzer:
    ...
    def _calculate_per_step_growth_rate(
        ...
        relative_error = np.sqrt(
            (current_stats.std / current_stats.mean) ** 2 + (prev_stats.std / prev_stats.mean) ** 2
        )
        rate_std = mean_rate * relative_error

        # Convert to percentages
        return StatsResult(mean=mean_rate * 100, median=median_rate * 100, std=rate_std * 100)
