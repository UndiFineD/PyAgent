# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\open\plots\visualise_production_figures.py
import asyncio
import json
import os
import pickle
        ...
        if os.path.exists(self.cache_file):
            try:
                with open(self.cache_file, "rb") as f:
                    ...
                    if all(
                        [
                            cached_data.get("model_groups")[model].version_ids == model_groups[model]
    ...
    def _plot_achievement_stack(
        ...
        if minimum_complexity > 0:
            filtered_achievements = {
                item for item in all_achievements if achievement_complexities.get(item, 0) >= minimum_complexity
            }
