...
class RunResults:
    ...
    def save_plots(self):
        ...
        for metric in metrics:
            ...
            plots[f"{metric}_percentiles_cumulative.png"] = self.plot_reward_percentiles(
                results[0],
            ...
            plots[f"{metric}_mean_std_raw.png"] = self.plot_reward_mean_std(results[0], metric=metric, cumulative=False)
    ...
    def _log_step_metrics(self, root_node: Node):
        ...
        def traverse_tree(node: Node, depth: int = 0, cumulative_reward: float = 0):
            ...
            if node.response and not any(
                error_term in node.response.lower() for error_term in ["error", "exception", "failed"]
            ):
        ...
        for depth in range(max_depth + 1):
            ...
            if rewards_by_depth[depth]:
                self.neptune_run["charts/avg_reward_by_depth"].append(np.mean(rewards_by_depth[depth]))
                self.neptune_run["charts/max_reward_by_depth"].append(max(rewards_by_depth[depth]))

            # Achievements
            self.neptune_run["charts/static_achievements_by_depth"].append(len(static_achievements_by_depth[depth]))
            self.neptune_run["charts/dynamic_achievements_by_depth"].append(len(dynamic_achievements_by_depth[depth]))

            # Node count
            self.neptune_run["charts/nodes_by_depth"].append(total_nodes_by_depth[depth])

            # Success rate
            if success_rate_by_depth[depth]["total"] > 0:
                success_rate = success_rate_by_depth[depth]["success"] / success_rate_by_depth[depth]["total"] * 100
        ...
        for depth in range(max_depth + 1):
            ...
            self.neptune_run["charts/cumulative_static_achievements"].append(len(cumulative_static))
            self.neptune_run["charts/cumulative_dynamic_achievements"].append(len(cumulative_dynamic))
    ...
    def plot_reward_tree(self, root_nodes: List[Node], cumulative: bool = True, metric="raw_reward"):
        ...
        def add_nodes(node: Node, depth: int = 0):
            if cumulative:
                ...
                value = (
                    len(node.metrics[metric]) if isinstance(node.metrics[metric], set) else (node.metrics[metric] or 0)
                )
                G.add_node(node.id, value=value)
                nodes_by_depth[depth].append(node.id)
        ...
        nodes = nx.draw_networkx_nodes(G, pos, node_color=values, node_size=18, cmap=plt.cm.viridis, ax=ax)
