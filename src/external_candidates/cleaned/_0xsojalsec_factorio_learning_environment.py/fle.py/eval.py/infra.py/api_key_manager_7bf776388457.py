# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\infra\api_key_manager.py
"""
...
class APIKeyConfig:
    ...
    def mark_error(self):
        ...
        if self.error_count >= self.max_errors:
            logging.warning(f"API key {self.key[:8]}... disabled due to {self.error_count} errors")
    ...
    def load_from_environment(self):
        ...
        for provider, env_prefix in provider_env_map.items():
            ...
            if keys_str:
                ...
                if single_key:
                    key_config = APIKeyConfig(key=single_key, provider=provider, priority=1)
                    self._keys[provider].append(key_config)
        ...
        with self._lock:
            for key_config in self._keys[provider]:
                if key_config.key == key:
                    ...
                    logging.warning(
                        f"Error with {provider} key {key[:8]}...: {error} (error count: {key_config.error_count})"
                    )
                    break
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "template":
        ...
        for provider, provider_stats in stats.items():
            print(f"{provider}: {provider_stats['available_keys']}/{provider_stats['total_keys']} keys available")
