...
class SweepConfig:
    ...
    # Resource management
    api_keys: List[str] = field(default_factory=list)  # Deprecated - use api_key_config_file
    ...
    task_inner_loop_mode: bool = False  # Cycle through tasks before repeating model-task pairs
    early_stop_on_success: bool = False  # Skip (model, task) pairs that already have one success
        ...
        try:
            ...
            for idx, row in existing_df.iterrows():
                try:
                    ...
                    is_finished = num_steps >= 40  # Must have at least 64 steps to be considered finished
                    ...
                    logger.log_metrics(
                        {
                            ...
                            "job/duration_minutes": (
                                (job.end_time - job.start_time).total_seconds() / 60 if job.start_time else 0
                            ),
            ...
            if self.wandb_logger:
                ...
                if logger:
                    ...
                    logger.log_metrics(
                        {
                            "job/status": "failed",
                            "job/completion_status": ("will_retry" if will_retry else "failed_final"),
                            "job/exit_code": process.exitcode,
                    ...
                    temp_logger.log_metrics(
                        {
                            ...
                            "progress/completed_trajectories": stats["completed_trajectories"],
                        }
                    )
        ...
        try:
            ...
            report = {
                ...
                "execution_summary": {
                    ...
                    "early_stopped_jobs": len([j for j in self.jobs if j.status == "early_stopped"]),
                ...
                "results_by_model": {model: metrics.to_dict() for model, metrics in results_by_model.items()},
                "results_by_task": {task: metrics.to_dict() for task, metrics in results_by_task.items()},
                ...
                summary_logger.log_metrics(
                    {
                        ...
                        "sweep/total_completed_jobs": report["execution_summary"]["completed_jobs"],
                        "sweep/total_failed_jobs": report["execution_summary"]["failed_jobs"],
                        "sweep/success_rate": report["execution_summary"]["completed_jobs"]
            ...
            if self.config.output_dir:
                report_path = (
                    Path(self.config.output_dir) / f"sweep_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                )
