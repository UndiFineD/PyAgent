# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\entrypoints\sweep_run.py
#!/usr/bin/env python3
"""
...
from fle.commons.constants import (  # Direct API models; OpenRouter models
    CLAUDE_OPUS_4_1,
    CLAUDE_SONNET_4,
...
from fle.eval.infra import SweepConfig, SweepManager
...
class SweepRunner:
    ...
    @staticmethod
    async def run_sweep(config: SweepConfig, existing_sweep_id: Optional[str] = None) -> dict:
        ...
        print(f"Total runs: {len(config.models) * len(config.tasks) * config.num_trials_per_config}")
    ...
    @classmethod
    async def run_pass_at_k_optimized(cls, existing_sweep_id: Optional[str] = None) -> dict:
        """Run pass_at_k_optimized sweep with optional resume"""
        config = SweepProfiles.pass_at_k_optimized()
        return await cls.run_sweep(config, existing_sweep_id)

    @classmethod
    async def run_production_large(cls, existing_sweep_id: Optional[str] = None) -> dict:
        """Run production_large sweep with optional resume"""
        config = SweepProfiles.production_large()
        return await cls.run_sweep(config, existing_sweep_id)

    @classmethod
    async def run_openrouter_tournament(cls, existing_sweep_id: Optional[str] = None) -> dict:
        """Run openrouter_tournament sweep with optional resume"""
        config = SweepProfiles.openrouter_tournament()
        return await cls.run_sweep(config, existing_sweep_id)


async def run_sweep_profile(profile: str, existing_sweep_id: Optional[str] = None, **custom_kwargs) -> dict:
...
async def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python consolidated_sweep_config.py pass_at_k --resume pass_at_k_optimized_20250106_120000_abc123")
        print(
