# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\entrypoints\gym_eval.py
import asyncio
    ...
    if WANDB_ANALYSIS_AVAILABLE and os.getenv("ENABLE_WANDB", "").lower() in [
        ...
        try:
            ...
            if config.version_description and "type:" in config.version_description:
                task_name = config.version_description.split("type:")[1].split("\n")[0].strip()
            ...
            if config.agents and len(config.agents) > 0:
                ...
                model_name = config.version_description.split("model:")[1].split("\n")[0].strip()
...
async def main(config_path):
    ...
    for run_idx, run_config in enumerate(run_configs):
        ...
        for agent_idx in range(num_agents):
            system_prompt = generator.generate_for_agent(agent_idx=agent_idx, num_agents=num_agents)
            # Get API key config file from environment (set by sweep_manager)
            api_key_config_file = os.getenv("FLE_API_KEY_CONFIG_FILE") or os.getenv("API_KEY_CONFIG_FILE")
