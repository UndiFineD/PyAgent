# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\beam\beam_search.py
import asyncio
import copy
import json
import logging
...
class ParallelBeamSearch:
    ...
    def _validate_instance_count(self, total_instances: int, beam_width: int):
        ...
        if total_instances < min_required:
            raise ValueError(
                f"Need at least {min_required} instances for beam width {beam_width} (got {total_instances})"
            )
    ...
    async def _run_beam_iteration(self, n_iterations: int):
        ...
        try:
            for iteration in range(n_iterations * 5):
                ...
                best_programs = await self._select_top_k_programs(all_programs, self.config.beam_width)
                ...
                try:
                    ...
                    await asyncio.wait_for(self._update_beam_groups(best_programs), timeout=60)
                except asyncio.TimeoutError:
    ...
    def get_group_metrics(self, group_id: int) -> Dict[str, Any]:
        ...
        if 0 <= group_id < len(self.beam_groups):
            ...
            return {
                ...
                "total_programs": sum(
                    inst.total_programs for inst in group.evaluator.logger.groups[group_id].instances.values()
                ),
                "error_count": sum(
                    inst.error_count for inst in group.evaluator.logger.groups[group_id].instances.values()
                ),
    ...
    async def start_monitors(self):
        ...
        if self._monitor_task is None:
            self._monitor_task = asyncio.create_task(self._monitor_tasks(), name="task_monitor")
        if self._progress_monitor_task is None:
            self._progress_monitor_task = asyncio.create_task(self._monitor_progress(), name="progress_monitor")
