# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\mcts\__main__chunked.py
import asyncio
...
async def create_blueprint_seeds(mcts: ChunkedMCTS, db_config: Dict[str, str], n_seeds: int = 10) -> List[Program]:
    """Generate seed programs from blueprint scenarios"""
    sampler = BlueprintScenarioSampler(db_config=db_config, system_prompt=mcts.system_prompt)
    return await sampler.sample_scenarios(instance=mcts.evaluator.instances[0], n_samples=n_seeds)
...
async def main():
    ...
    parallel_mcts = ParallelMCTS(instances=instances, db_client=db_client, api_factory=llm, config=mcts_config)
