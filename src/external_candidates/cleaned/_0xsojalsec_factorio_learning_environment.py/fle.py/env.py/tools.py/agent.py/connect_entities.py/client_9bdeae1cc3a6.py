# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\agent\connect_entities\client.py
...
class ConnectEntities(Tool):
    ...
    def _connect_pair_of_waypoints(
        ...
        if (
            (Prototype.UndergroundPipe in connection_types and Prototype.Pipe in connection_types)
            and (isinstance(target, (ChemicalPlant, OilRefinery)) or isinstance(source, (ChemicalPlant, OilRefinery)))
            and not dry_run
        ):
            for source_pos, target_pos in prioritised_list_of_position_pairs:
                try:
        ...
        source_error_message_addition = (
            f"{source}" if isinstance(source, Position) else f"{source.name} at {source.position}"
        )
        target_error_message_addition = (
            f"{target}" if isinstance(target, Position) else f"{target.name} at {target.position}"
        )
    ...
    def _create_connection(
        ...
        match connection_types:
            ...
            case _ if (
                (connection_types & {Prototype.TransportBelt, Prototype.UndergroundBelt})
