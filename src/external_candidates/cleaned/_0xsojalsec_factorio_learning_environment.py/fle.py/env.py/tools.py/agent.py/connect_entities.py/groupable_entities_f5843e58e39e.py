# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\agent\connect_entities\groupable_entities.py
...
def construct_belt_groups(belts: List[Union[TransportBelt, UndergroundBelt]], prototype):
    ...
    for group in initial_groups:
        ...
        for final_group in final_groups:
            ...
            if any(
                (belt.output_position.x, belt.output_position.y) in {(b.position.x, b.position.y) for b in final_group}
                for belt in group
            ):
                # Merge groups
                for belt in group:
...
def agglomerate_groupable_entities(
    ...
    if hasattr(connected_entities[0], "__class__") and connected_entities[0].__class__.__name__ in [
        "BeltGroup",
        "PipeGroup",
        "ElectricityGroup",
        "WallGroup",
