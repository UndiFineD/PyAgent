# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\admin\render\renderer.py
#!/usr/bin/env python3
"""
...
from fle.env import Entity, EntityStatus, Position, UndergroundBelt
...
from fle.env.tools.admin.render.profiler import profile_method, profiler
...
class Renderer:
    ...
    def render(self, width: int, height: int, image_resolver) -> Image.Image:
        ...
        tree_entities = [
            e.model_dump() if not isinstance(e, dict) else e for e in self.entities if is_tree_entity(e.name)
        ]
        rock_entities = [
            e.model_dump() if not isinstance(e, dict) else e for e in self.entities if is_rock_entity(e.name)
        ]
