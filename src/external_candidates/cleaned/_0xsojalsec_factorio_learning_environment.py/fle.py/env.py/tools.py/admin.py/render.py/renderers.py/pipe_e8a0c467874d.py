# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\admin\render\renderers\pipe.py
# renderers/pipe.py
"""
...
def get_around(entity: Dict, grid) -> list:
    ...
    return [
        # North
        is_pipe(grid.get_relative(0, -1), 4) or is_entity_in_direction(grid.get_relative(0, -1), "offshore-pump", 0),
        # East
        is_pipe(grid.get_relative(1, 0), 6) or is_entity_in_direction(grid.get_relative(1, 0), "offshore-pump", 2),
        # South
        is_pipe(grid.get_relative(0, 1), 0) or is_entity_in_direction(grid.get_relative(0, 1), "offshore-pump", 4),
        # West
        is_pipe(grid.get_relative(-1, 0), 2) or is_entity_in_direction(grid.get_relative(-1, 0), "offshore-pump", 6),
    ]
