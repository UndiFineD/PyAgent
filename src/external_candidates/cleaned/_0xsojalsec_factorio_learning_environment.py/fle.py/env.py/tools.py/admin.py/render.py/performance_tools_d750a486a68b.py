...
class PerformanceAnalyzer:
    ...
    def analyze_bottlenecks(self, min_time: float = 0.001) -> Dict[str, Any]:
        ...
        significant_ops = {op: data for op, data in stats.items() if data["total"] >= min_time}

        # Sort by total time
        sorted_ops = sorted(significant_ops.items(), key=lambda x: x[1]["total"], reverse=True)
    ...
    def get_image_cache_stats(self) -> Dict[str, Any]:
        ...
        image_ops = {op: data for op, data in stats.items() if "ImageResolver" in op or "image_resolver" in op}
    ...
    def get_entity_rendering_stats(self) -> Dict[str, Any]:
        ...
        entity_render_ops = {
            op: data for op, data in stats.items() if any(keyword in op for keyword in ["_render_", "render_"])
        }
    ...
    def generate_performance_report(self, output_file: Optional[Path] = None, include_raw_data: bool = False) -> str:
...
def stop_profiling_and_report(analyzer: PerformanceAnalyzer, output_file: Optional[Path] = None) -> str:
