# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\admin\render\utils\legend_renderer.py
import math
...
class LegendRenderer:
    ...
    def calculate_legend_dimensions(
        ...
        total_items += (
            len(resource_types) + len(natural_types) + len(status_items) + len(electricity_items) + len(player_items)
        )
    ...
    def draw_combined_legend(
        ...
        for entity_name, count in all_entities:
            ...
            entities_section["items"].append({"name": display_text, "color": color, "shape": shape_type})
        ...
        if player_items:
            ...
            for player_item in player_items:
                player_section["items"].append({"name": player_item["name"], "is_player": True})
                            ...
                            for i in range(0, shape_sample_size, hatch_spacing):
                                ...
                                draw.line(
                                    [
                                        ...
                                        (
                                            shape_x + shape_sample_size,
                                            shape_y + shape_sample_size - min(i, shape_sample_size),
                                        ),
                                    ],
                                    fill=(
                                        min(color[0] + 30, 255),
