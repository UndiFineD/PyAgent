# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\protocols\a2a\server.py
import logging
...
class AgentRegistry:
    ...
    def list_agents(self) -> List[Dict[str, Any]]:
        return [{"id": agent_id, **agent_data} for agent_id, agent_data in self.agents.items()]
...
async def handle_jsonrpc(request: JSONRPCRequest) -> JSONRPCResponse:
    try:
        if request.method == "register":
            ...
            return JSONRPCResponse(result={"agents": registry.list_agents()}, id=request.id)
            ...
            supported = {k: v for k, v in agent_capabilities.items() if k in capabilities}
            ...
            if recipient_id is None:
                ...
                agents_to_notify = [agent_id for agent_id in registry.agents.keys() if agent_id != sender_id]
            ...
            try:
                ...
                return JSONRPCResponse(result={"status": "messages_loaded"}, id=request.id)
