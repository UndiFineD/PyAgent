# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\protocols\_mcp\resources.py
import asyncio
...
async def manual(method: str) -> str:
    ...
    if method not in available_tools:
        return f"Error: '{method}' is not a valid agent tool. Available tools: {', '.join(sorted(available_tools))}"
...
async def render_at(center_x: str, center_y: str, radius: int = 32) -> ImageContent:
    ...
    try:
        ...
        try:
            ...
            img = instance.namespace._render_simple(position=Position(cx, cy), radius=radius)
