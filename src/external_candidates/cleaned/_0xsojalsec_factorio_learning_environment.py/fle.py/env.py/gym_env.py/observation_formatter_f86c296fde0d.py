# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\gym_env\observation_formatter.py
import pickle
import re
...
class BasicObservationFormatter:
    ...
    def format(self, observation: Observation, last_message_timestamp: float = 0.0) -> FormattedObservation:
        ...
        return FormattedObservation(
            ...
            task_info_str=(self.format_task_info(obs_dict.get("task_info")) if self.include_task else ""),
            messages_str=(
            ...
            functions_str=(
                self.format_functions(obs_dict.get("serialized_functions", [])) if self.include_functions else ""
            ),
            game_info_str=(self.format_game_info(obs_dict.get("game_info", {})) if self.include_game_info else ""),
