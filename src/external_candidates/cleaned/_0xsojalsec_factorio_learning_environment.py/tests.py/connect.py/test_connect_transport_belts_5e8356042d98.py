...
def test_multiple_inserter_connections(game):
    ...
    for i in range(test_iterations):
        ...
        assert len(belt_group.belts) >= expected_belt_count, (
            f"Iteration {i}: Expected at least {expected_belt_count} belts, got {len(belt_group.belts)}"
        )
...
def test_failure_to_connect_adjacent_furnaces(game):
    ...
    inserter = game.rotate_entity(inserter, Direction.LEFT)
    game.connect_entities(target=inserter, source=drill, connection_type=Prototype.TransportBelt)
...
def test_connect_transport_belts_to_inserter_row(game):
    ...
    assert belt_group.outputs[0].position.is_close(current_inserter.pickup_position, 0.5), (
