# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\actions\test_place_entity_next_to.py
import pytest
...
def calculate_expected_position(ref_pos, direction, spacing, ref_entity, entity_to_place):
    ...
    if direction == Direction.RIGHT:
        ...
        return align_to_grid(
            Position(
                x=ref_pos.x,
                y=ref_pos.y + ref_tile_height / 2 + entity_tile_height / 2 + spacing + y_offset,
            )
        )
...
def test_mining_drill_output_flexibility(game):
    ...
    drill = game.place_entity(Prototype.ElectricMiningDrill, position=iron_position, direction=Direction.DOWN)
    ...
    assert successful_placements > 0, "Should place at least one output inserter around drill"
...
def test_collision_resolution_with_helpful_errors(game):
    ...
    game.place_entity(Prototype.SmallElectricPole, position=Position(x=222, y=220))  # East
    game.place_entity(Prototype.SmallElectricPole, position=Position(x=218, y=220))  # West
...
def test_alternative_position_scoring(game):
    ...
    if inserter:
        ...
        distance = (
            (inserter.position.x - assembler.position.x) ** 2 + (inserter.position.y - assembler.position.y) ** 2
        ) ** 0.5
