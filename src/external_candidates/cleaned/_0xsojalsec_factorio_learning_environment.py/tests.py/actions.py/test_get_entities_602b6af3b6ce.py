# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\actions\test_get_entities.py
import pytest
...
def test_group_prototype_support_electricity_group(game):
    ...
    pole_connection = game.connect_entities(start_pos, end_pos, Prototype.SmallElectricPole)
    ...
    for group in electricity_groups:
        assert hasattr(group, "electric_poles") or group.__class__.__name__ == "ElectricityGroup", (
...
def test_entity_expansion_from_group_requests(game):
    ...
    if belt_groups:
        ...
        assert total_belts_found >= 3, f"Should find at least 3 belts, found {total_belts_found}"
...
def test_enhanced_grouping_vs_individual_logic(game):
    ...
    pole1 = game.place_entity(Prototype.SmallElectricPole, position=ent.Position(x=40, y=40))
    ...
    poles_with_position = game.get_entities({Prototype.SmallElectricPole}, position=pole1.position)
    ...
    grouped_count = sum(
        ...
        if hasattr(e, "__class__") and e.__class__.__name__ in ["ElectricityGroup", "BeltGroup", "PipeGroup"]
    )

    assert grouped_count >= 0, "Should have some grouped entities when getting all"

    print(f"✓ Enhanced grouping logic working - found {grouped_count} groups in get_entities()")
...
def test_individual_belt_extraction_without_position(game):
    ...
    game.connect_entities(belt_positions[0], belt_positions[-1], Prototype.TransportBelt)
    ...
    for entity in transport_belts:
        if (
            (hasattr(entity, "prototype") and entity.prototype == Prototype.TransportBelt)
...
def test_mixed_entity_grouping_behavior(game):
    ...
    mixed_entities = game.get_entities({Prototype.StoneFurnace, Prototype.ElectricityGroup})
    assert len(mixed_entities) > 0, "Should find mixed entity types"

    # Should find the furnace and electricity group
    found_furnace = any(hasattr(e, "prototype") and e.prototype == Prototype.StoneFurnace for e in mixed_entities)
    found_electricity = any(hasattr(e, "__class__") and "Electric" in e.__class__.__name__ for e in mixed_entities)

    assert found_furnace or found_electricity, "Should find at least one of the requested types"
...
def test_position_filtering_with_groups(game):
    ...
    close_pole = game.place_entity(Prototype.SmallElectricPole, position=ent.Position(x=70, y=70))
    ...
    for entity in nearby_entities:
        if hasattr(entity, "position"):
            distance = (
                (entity.position.x - close_pole.position.x) ** 2 + (entity.position.y - close_pole.position.y) ** 2
            ) ** 0.5
...
def test_group_prototype_backwards_compatibility(game):
    ...
    assert len(all_entities_after) >= len(all_entities_before), "Entity count should increase"
