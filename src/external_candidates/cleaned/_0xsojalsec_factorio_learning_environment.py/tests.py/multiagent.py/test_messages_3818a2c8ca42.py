# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\multiagent\test_messages.py
import asyncio
import time
...
def trajectory_runner(multi_instance, mock_db_client, mock_config):
    ...
    runner = TrajectoryRunner(mock_config.agents, mock_db_client, evaluator, mock_config, process_id=1)
...
def test_collaborative_scenario(trajectory_runner, mock_config):
    ...
    with patch.object(TrajectoryRunner, "_generate_program", return_value=MagicMock()):
        ...
        instance.namespaces[0].send_message("I've gathered 100 iron plates and 50 copper plates.", recipient=1)
        ...
        assert any(
            msg.sender == "1" and "need 20 steel plates" in msg.content for msg in trajectory_runner.agent_messages[0]
        )
        ...
        assert any(msg.sender == "0" and "steel plates" in msg.content for msg in trajectory_runner.agent_messages[1])
