# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\data\vqa\tasks\factory\entity_status\solver.py
...
def entity_status_questions(instance, questions_per_position: int = 5, multiple_choice: bool = True) -> Solver:
    ...
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        ...
        if len(entities_with_status) < questions_per_position:
            normal_entities = [
                e for e in entities if hasattr(e, "status") and e.status in [EntityStatus.NORMAL, EntityStatus.WORKING]
            ]
        ...
        for entity in selected_entities:
            ...
            if not multiple_choice:
                ...
                alphabet = ["a", "b", "c", "d"]
                option_strings = [
                    f"{alphabet[i]}) {STATUS_DESCRIPTIONS.get(opt, opt.value)}" for i, opt in enumerate(options)
                ]
