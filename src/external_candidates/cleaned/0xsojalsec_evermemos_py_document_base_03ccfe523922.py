# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\oxm.py\mongo.py\document_base_03ccfe523922.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\mongo\document_base.py
...
class DocumentBase(Document):
    ...
    def _recursive_datetime_check(self, obj, path: str = "", depth: int = 0):
        ...
        if isinstance(obj, (list, tuple, set)):
            ...
            if isinstance(obj, list):
                ...
                first_checked = self._recursive_datetime_check(first_item, f"{path}[0]", depth + 2)
            ...
            # Set: check any one element (set is unordered, take the first one)
                ...
                sample_checked = self._recursive_datetime_check(sample_item, f"{path}[sample]", depth + 2)
            ...
            # Tuple: only check the first 3 elements
                ...
                for idx in range(check_count):
                    ...
                    checked = self._recursive_datetime_check(item, f"{path}[{idx}]", depth + 2)
            ...
            return cls(self._recursive_datetime_check(item, f"{path}[{i}]", depth + 2) for i, item in enumerate(obj))

        # Case 4: Object is dictionary

        if isinstance(obj, dict):
            return {
                key: self._recursive_datetime_check(value, f"{path}[{repr(key)}]", depth + 2)
    ...
    async def insert_many(
        ...
        **pymongo_kwargs,
        ...
        return await super().insert_many(documents, session=session, link_rule=link_rule, **pymongo_kwargs)
