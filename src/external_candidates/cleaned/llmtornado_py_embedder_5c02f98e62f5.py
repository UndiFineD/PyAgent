# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\llmtornado.py\src.py\llmtornado.py\llmtornado.py\fskb.py\indexing.py\embedder_5c02f98e62f5.py
...
class EmbeddingProvider:
    ...
    def _initialize_local(self):
        ...
        try:
            ...
            self._tokenizer = AutoTokenizer.from_pretrained(self.model, padding_side="left", trust_remote_code=True)
            ...
            logger.info(f"Local model loaded: {self.model}, dimension: {self._dimension}, device: {device}")
    ...
    async def _embed_local(self, texts: List[str], is_query: bool = False, task: str = None) -> List[np.ndarray]:
        ...
        for i in range(0, len(texts_with_instruction), self.batch_size):
            ...
            try:
                ...
                embeddings = await asyncio.wait_for(loop.run_in_executor(None, encode_batch, batch), timeout=timeout)
