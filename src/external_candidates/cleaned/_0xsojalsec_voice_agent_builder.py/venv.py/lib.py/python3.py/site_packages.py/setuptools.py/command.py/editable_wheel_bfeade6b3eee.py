# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-voice-agent-builder\venv\lib\python3.11\site-packages\setuptools\command\editable_wheel.py
"""
...
def _simple_layout(packages: Iterable[str], package_dir: Dict[str, str], project_dir: Path) -> bool:
    ...
    return all(
        _normalize_path(Path(parent, *key.split("."))) == _normalize_path(value) for key, value in layout.items()
    )
