# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_owl.py\owl.py\camel.py\societies.py\workforce.py\single_agent_worker_c0896ed0bcfa.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\societies\workforce\single_agent_worker.py
...
class SingleAgentWorker(Worker):
    ...
    async def _process_task(self, task: Task, dependencies: List[Task]) -> TaskState:
        ...
        try:
            ...
            print(f"{Fore.RED}Error occurred while processing task {task.id}:\n{e}{Fore.RESET}")
