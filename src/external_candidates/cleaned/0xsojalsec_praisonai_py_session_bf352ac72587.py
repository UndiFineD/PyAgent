# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_praisonai.py\src.py\praisonai_agents.py\praisonaiagents.py\session_bf352ac72587.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\session.py
...
class Session:
    ...
    def Agent(
        ...
        **kwargs,
        ...
        if self.is_remote:
            raise ValueError(
                "Cannot create local agents in remote sessions. Use chat() to communicate with the remote agent."
            )

        agent_kwargs = {"name": name, "role": role, "user_id": self.user_id, **kwargs}
    ...
    def restore_state(self) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if metadata.get("type") == "session_state" and metadata.get("session_id") == self.session_id:
                # Extract state data from metadata (excluding system fields)

                state_data = {k: v for k, v in metadata.items() if k not in ["type", "session_id", "user_id"]}
    ...
    def _restore_agent_chat_history(self, agent_key: str) -> List[Dict[str, Any]]:
        ...
        results = self.memory.search_short_term(query="Agent chat history for", limit=10)
        ...
        for result in results:
            ...
            if (
                metadata.get("type") == "agent_chat_history"
                and metadata.get("session_id") == self.session_id
                and metadata.get("agent_key") == agent_key
            ):
    ...
    def _restore_agent_chat_histories(self) -> None:
        ...
        for result in results:
            ...
            if metadata.get("type") == "agent_chat_history" and metadata.get("session_id") == self.session_id:
    ...
    def add_memory(self, text: str, memory_type: str = "long", **metadata) -> None:
        ...
        metadata.update({"session_id": self.session_id, "user_id": self.user_id})
    ...
    def get_context(self, query: str, max_items: int = 3) -> str:
        ...
        return self.memory.build_context_for_task(task_descr=query, user_id=self.user_id, max_items=max_items)
    ...
    def chat(self, message: str, **kwargs) -> str:
        ...
        if not self.is_remote:
            raise ValueError(
                "chat() method is only available for remote agent sessions. Use Agent.chat() for local agents."
            )
