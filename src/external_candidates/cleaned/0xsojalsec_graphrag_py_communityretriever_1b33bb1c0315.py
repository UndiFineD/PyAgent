# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\retriever.py\communityretriever_1b33bb1c0315.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Retriever\CommunityRetriever.py
...
class CommunityRetriever(BaseRetriever):
    ...
    async def _find_relevant_community_from_entities(self, seed: list[dict]):
        ...
        related_community_dup_keys = [
            str(dp["cluster"]) for dp in related_communities if dp["level"] <= self.config.level
        ]
        ...
        related_community_datas = {
            k: v for k, v in zip(related_community_keys_counts.keys(), _related_community_datas) if v is not None
        }
    ...
    async def find_relevant_community_by_level(self, seed=None):
        ...
        community_schema = {k: v for k, v in community_schema.items() if v.level <= self.config.level}
        ...
        sorted_community_schemas = sorted_community_schemas[: self.config.global_max_consider_community]
        ...
        community_datas = [
            c for c in community_datas if c["report_json"].get("rating", 0) >= self.config.global_min_community_rating
        ]

        community_datas = sorted(
            community_datas,
            key=lambda x: (
                x["community_info"]["occurrence"],
