...
class ChartPatternDetector:
    ...
    def __init__(self, min_pattern_length: int = 20, similarity_threshold: float = 0.02):
        """
        ...
        logger.info(
            f"Initialized ChartPatternDetector (min_length={min_pattern_length}, threshold={similarity_threshold})"
        )
        ...
        try:
            ...
            if len(troughs) >= 2:
                for i in range(len(troughs) - 1):
                    ...
                    if (
                        trough1_price > 0
                        and abs(trough1_price - trough2_price) / trough1_price < self.similarity_threshold
                    ):
    ...
    def detect_wedges(self, df: pd.DataFrame) -> List[Dict]:
        ...
        try:
            ...
            if upper_slope > 0 and lower_slope > 0 and lower_slope > upper_slope:
                ...
                logger.debug(f"Rising Wedge detected: upper_slope={upper_slope:.4f}, lower_slope={lower_slope:.4f}")
            ...
            # Falling Wedge: Both lines falling, but converging (bullish)
                ...
                logger.debug(f"Falling Wedge detected: upper_slope={upper_slope:.4f}, lower_slope={lower_slope:.4f}")
        ...
        try:
            ...
            if abs(first_half_trend) > 0.05:
                ...
                if second_half_volatility < 0.01:
                    ...
                    logger.debug(f"{pattern_type} detected: bias={bias}, initial_move={first_half_trend * 100:.2f}%")
    ...
    def detect_triangles(self, df: pd.DataFrame) -> List[Dict]:
        ...
        try:
            ...
            # Symmetrical Triangle: Both converging (direction uncertain)
                ...
                logger.debug(f"Symmetrical Triangle detected: slopes=[{high_slope:.4f}, {low_slope:.4f}]")
