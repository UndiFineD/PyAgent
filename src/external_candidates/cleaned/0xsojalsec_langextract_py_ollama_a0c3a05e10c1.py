# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\providers.py\ollama_a0c3a05e10c1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\providers\ollama.py
...
class OllamaLanguageModel(base_model.BaseLanguageModel):
    ...
    _constraint: schema.Constraint = dataclasses.field(default_factory=schema.Constraint, repr=False, compare=False)

    _extra_kwargs: dict[str, Any] = dataclasses.field(default_factory=dict, repr=False, compare=False)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if structured_output_format is not None:
            warnings.warn(
                "'structured_output_format' is deprecated and will be removed in v2.0.0. Use 'format_type' instead.",
                FutureWarning,
                stacklevel=2,
            )
            ...
            if format_type is None:
                format_type = (
                    core_types.FormatType.JSON if structured_output_format == "json" else core_types.FormatType.YAML
                )

        fmt = kwargs.pop("format", None)

        if format_type is None and fmt in ("json", "yaml"):
            format_type = core_types.FormatType.JSON if fmt == "json" else core_types.FormatType.YAML
        ...
        for prompt in batch_prompts:
            try:
                response = self._ollama_query(
                    prompt=prompt,
                    model=self._model,
                    structured_output_format=("json" if self.format_type == core_types.FormatType.JSON else "yaml"),
                ...
                raise exceptions.InferenceRuntimeError(f"Ollama API error: {str(e)}", original=e) from e
        ...
        **kwargs,
        ...
        try:
            ...
            if isinstance(e, self._requests.exceptions.ReadTimeout):
                msg = f"Ollama Model timed out (timeout={request_timeout}, num_threads={num_threads})"
        ...
        if response.status_code == 404:
            raise exceptions.InferenceConfigError(f"Can't find Ollama {model}. Try: ollama run {model}")
