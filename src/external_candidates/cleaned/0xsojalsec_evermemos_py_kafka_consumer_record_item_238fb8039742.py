# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\queue.py\redis_group_queue.py\kafka_consumer_record_item_238fb8039742.py
...
class KafkaConsumerRecordItem(RedisGroupQueueItem):
    ...
    def __init__(self, consumer_record: ConsumerRecord):
        ...
        self.headers = [
            (
                name,
                (self._encode_bytes_to_base64(data) if isinstance(data, bytes) else str(data)),
            )
    ...
    def from_bson_bytes(cls, bson_bytes: bytes) -> "KafkaConsumerRecordItem":
        ...
        try:
            ...
            logger.error("BSON deserialization of KafkaConsumerRecordItem failed: %s", e)
