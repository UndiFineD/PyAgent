# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\tests.py\inference_test_8b111af9ef8c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\tests\inference_test.py
...
class TestOllamaLanguageModel(absltest.TestCase):
    ...
    def test_ollama_infer(self, mock_ollama_query):
        ...
        expected_results = [[types.ScoredOutput(score=1.0, output="{'bus' : '**autóbusz**'} \n\n\n  \n")]]
    ...
    def test_ollama_default_timeout(self):
        ...
        with mock.patch.object(model._requests, "post", return_value=mock_response) as mock_post:
...
class TestGeminiLanguageModel(absltest.TestCase):
    ...
    def test_gemini_runtime_kwargs_filtered(self, mock_client_class):
        ...
        self.assertNotIn("unknown_runtime_param", config, "Unknown kwargs should be filtered out")
    ...
    def test_gemini_vertexai_initialization(self, mock_client_class):
        ...
        model = gemini.GeminiLanguageModel(vertexai=True, project="test-project", location="us-central1")
    ...
    @mock.patch("google.genai.Client")
    def test_gemini_warns_when_both_auth_provided(self, mock_client_class, mock_warning):
...
class TestOpenAILanguageModelInference(parameterized.TestCase):
    ...
    def test_openai_infer_with_parameters(self, api_key, base_url, model_id, temperature, mock_openai_class):
        mock_client = mock.Mock()
        ...
        mock_response.choices = [mock.Mock(message=mock.Mock(content='{"name": "John", "age": 30}'))]
