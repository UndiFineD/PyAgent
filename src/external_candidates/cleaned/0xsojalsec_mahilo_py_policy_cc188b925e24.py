# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_mahilo.py\mahilo.py\policy_cc188b925e24.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\policy.py
...
class Policy:
    ...
    async def evaluate(
        self, message: MessageEnvelope, context: Dict, model_name: Optional[str] = None
...
class PolicyManager:
    ...
    async def evaluate_message(self, message: MessageEnvelope, context: Dict) -> Tuple[bool, List[PolicyViolation]]:
...
class MessageValidator:
    ...
    async def validate(self, message: MessageEnvelope, context: Dict) -> Tuple[bool, List[PolicyViolation]]:
    ...
    def anti_loop_policy(message: MessageEnvelope, context: Dict) -> Tuple[bool, Optional[str]]:
        ...
        if sender_messages:
            ...
            if similarity > 0.8:  # High similarity threshold
                return (
                    False,
                    "Your message is too similar to your previous message. Please provide new information or a different approach.",
                )
        ...
        if len(history) >= 4:
            ...
            if (
                last_4_messages[0].sender == sender
                and last_4_messages[1].sender == recipient
                and last_4_messages[2].sender == sender
                and last_4_messages[3].sender == recipient
            ):
                # Check content similarity

                similarity_1_3 = SequenceMatcher(None, last_4_messages[0].payload, last_4_messages[2].payload).ratio()

                similarity_2_4 = SequenceMatcher(None, last_4_messages[1].payload, last_4_messages[3].payload).ratio()

                if similarity_1_3 > 0.7 and similarity_2_4 > 0.7:
                    return (
                        False,
                        "It seems you're in a repetitive conversation pattern. Try a different approach or provide new information.",
                    )
