# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2022.py\piano.py\midi_animations_ad2ba3f330c1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\piano\midi_animations.py

import mido

from _2022.piano.wav_to_midi import DATA_DIR, midi_to_wav, piano_midi_range
...
class AnimatedMidi(Scene):
    ...
    def add_note_rects(self):
        ...
        track.extend([msg for msg in mido.merge_tracks(mid.tracks) if msg.type not in ["pitchwheel", "time_signature"]])
        ...
        for msg in track:
            ...
            if msg.type == "note_on" and msg.velocity > 0:
                ...
                notes_to_time_spans[msg.note].append((start_time_in_ticks * sec_per_tick, time_in_ticks * sec_per_tick))
    ...
    def scroll(self):
        ...
        def update_piano(piano, dt):
            ...
            for note, key in zip(piano_midi_range, piano):
                ...
                if hit:
                    ...
                    key.set_fill(interpolate_color(key[0].get_fill_color(), self.hit_color, 0.5))
        ...
        note_rects.add_updater(lambda m: m.move_to(note_rects_start + self.dist_per_sec * self.time * DOWN))
