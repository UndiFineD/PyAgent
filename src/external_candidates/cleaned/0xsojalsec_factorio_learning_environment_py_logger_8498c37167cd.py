# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\mcts.py\logger_8498c37167cd.py
...
class FactorioLogger:
    ...
    def _generate_layout(self) -> Layout:
        ...
        metrics_layout.split_row(
            *[Layout(self._generate_instance_panel(instance), ratio=1) for instance in self.instances.values()]
        )

        # Add progress bar if it exists

        if self.progress_task is not None:
            layout.split_column(Layout(metrics_layout, ratio=4), Layout(self.progress, ratio=1))
