# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kesslerio.py\oura_analytics.py\scripts.py\briefing_79355b2e08ee.py
...
class Baseline:
    ...
    def from_history(cls, nights: List[NightRecord]) -> "Baseline":
        ...
        rhr_values = sorted(
            [n.sleep.lowest_heart_rate_bpm for n in nights if n.sleep and n.sleep.lowest_heart_rate_bpm]
        )
...
def format_json_briefing(night: NightRecord, baseline: Optional[Baseline] = None) -> Dict[str, Any]:
    ...
    if night.readiness:
        briefing["readiness"] = {
            "score": night.readiness.score,
            "delta_vs_baseline": (
                round(night.readiness.score - formatter.baseline.avg_readiness, 1) if baseline else None
            ),
