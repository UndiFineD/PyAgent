# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\inclinedadarsh.py\manimce_best_practices.py\examples.py\updater_patterns_8223a67b3d94.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\inclinedadarsh\manimce-best-practices\examples\updater_patterns.py
...
class CircleRadiusTracker(Scene):
    ...
    def construct(self):
        ...
        circle = always_redraw(lambda: Circle(radius=tracker.get_value(), color=BLUE, fill_opacity=0.3))
...
class SineWaveTracker(Scene):
    ...
    def construct(self):
        ...
        sine_wave = always_redraw(
            lambda: axes.plot(lambda x: np.sin(x + phase.get_value()), color=BLUE, x_range=[0, 2 * PI])
        )

        # Dot that follows the wave

        dot = always_redraw(lambda: Dot(color=RED).move_to(axes.c2p(PI, np.sin(PI + phase.get_value()))))
...
class ArrowUpdater(Scene):
    ...
    def construct(self):
        ...
        arrow = always_redraw(lambda: Arrow(dot1.get_center(), dot2.get_center(), buff=0.3, color=YELLOW))
        ...
        self.play(dot1.animate.shift(RIGHT * 3), dot2.animate.shift(UP * 2), run_time=2)
...
class ParametricCurveTracer(Scene):
    ...
    def construct(self):
        ...
        dot = always_redraw(lambda: Dot(color=RED, radius=0.15).move_to(parametric_func(t_tracker.get_value())))
...
class MultipleTrackers(Scene):
    ...
    def construct(self):
        ...
        dot = always_redraw(
            lambda: Dot(color=RED, radius=0.2).move_to(RIGHT * x_tracker.get_value() + UP * y_tracker.get_value())
        )

        # Coordinate display

        coords = always_redraw(
            lambda: MathTex(f"({x_tracker.get_value():.1f}, {y_tracker.get_value():.1f})").to_corner(UL)
        )
...
class SpringMassSimulation(Scene):
    ...
    def construct(self):
        ...
        mass_obj = always_redraw(
            lambda: Square(side_length=0.8, color=BLUE, fill_opacity=0.8).move_to(UP * position.get_value())
        )

        # Spring (simplified as line)

        spring = always_redraw(lambda: Line(ground.get_center() + UP * 0.1, mass_obj.get_bottom(), color=GREY))
