...
class MarketContextAnalyzer:
    ...
    def _analyze_btc(self) -> Dict:
        ...
        try:
            ...
            price_change_24h = ((df_1h["close"].iloc[-1] - df_1h["close"].iloc[-24]) / df_1h["close"].iloc[-24]) * 100

            price_change_7d = (
                ((df_1h["close"].iloc[-1] - df_1h["close"].iloc[-168]) / df_1h["close"].iloc[-168]) * 100
                if len(df_1h) >= 168
                else 0
            )

            price_change_30d = (
                ((df_1d["close"].iloc[-1] - df_1d["close"].iloc[-30]) / df_1d["close"].iloc[-30]) * 100
                if len(df_1d) >= 30
                else 0
            )
    ...
    def _determine_market_regime(self, btc_analysis: Dict) -> str:
        ...
        if price_change_30d > 10 and price_change_7d > 3 and trend_aligned and btc_analysis.get("trend_short") == "up":
            return "bull"
