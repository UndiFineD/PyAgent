# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kesslerio.py\oura_analytics.py\scripts.py\oura_briefing_790c57e2b751.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\oura_briefing.py
...
from briefing import (
    Baseline,
    BriefingFormatter,
    format_brief_briefing,
    format_hybrid_briefing,
    format_json_briefing,
)
...
def _analyze_week(sleep_data, readiness_data=None):
    ...
    for i, d in enumerate(sleep_data[-2:]):
        ...
        last_2_days.append({"day": day, "sleep_score": score, "readiness": r_score, "hours": hours})
...
def main():
    ...
    parser.add_argument(
        "--format",
        choices=["text", "brief", "json", "hybrid"],
        default="text",
    ...
    try:
        ...
        baseline_start = (datetime.strptime(target_date, "%Y-%m-%d") - timedelta(days=args.baseline_days)).strftime(
            "%Y-%m-%d"
        )
        ...
        for sleep_entry in baseline_sleep:
            ...
            baseline_night = create_night_record(date=date, sleep=sleep_entry, readiness=readiness_entry)
