# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\infra.py\server_manager_6c800b0731fd.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\infra\server_manager.py
...
class ServerManager:
    ...
    def allocate_server(self, job_id: str, process_id: Optional[int] = None) -> Optional[ServerAllocation]:
        ...
        with self._lock:
            ...
            available_servers = [
                server_id for server_id in self._available_servers if server_id not in self._allocated_servers
            ]
    ...
    def get_allocation_status(self) -> Dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "available_servers": len(self._available_servers) - len(self._allocated_servers),
                "allocations": [allocation.to_dict() for allocation in self._allocations.values()],
