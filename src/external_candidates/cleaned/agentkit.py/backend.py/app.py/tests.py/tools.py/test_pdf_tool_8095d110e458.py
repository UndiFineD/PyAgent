# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\tests\tools\test_pdf_tool.py
# -*- coding: utf-8 -*-
...
def mock_llm_call(llm: BaseLanguageModel):  # pylint: disable=redefined-outer-name
    with patch(
        "app.services.chat_agent.tools.library.pdf_tool.pdf_tool.get_llm",
        return_value=llm,
    ):
        yield
...
def fake_get_pdf_pipeline(
    pdf_pipeline: FakePDFExtractionPipeline,
):  # pylint: disable=redefined-outer-name
    with patch(
        "app.services.chat_agent.tools.library.pdf_tool.pdf_tool.get_pdf_pipeline",
        return_value=pdf_pipeline,
    ):
        yield
