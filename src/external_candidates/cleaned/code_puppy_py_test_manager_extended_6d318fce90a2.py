# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\mcp.py\test_manager_extended_6d318fce90a2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_manager_extended.py
...
class TestMCPManagerExtended:
    ...
    def test_register_server_failure_cleanup(self):
        ...
        with (
            patch.object(manager.registry, "register", return_value="test-server-id") as mock_register,
            ...
            with patch(
                ...
                config = ServerConfig(id="", name="test-server", type="stdio", config={"command": "echo"})
    ...
    def test_get_server_by_name(self):
        ...
        expected_config = ServerConfig(id="test-id", name="test-server", type="stdio", config={"command": "echo"})

        with patch.object(manager.registry, "get_by_name", return_value=expected_config):
    ...
    def test_update_server_not_found(self):
        ...
        with patch.object(manager.registry, "update", return_value=False):
            config = ServerConfig(id="non-existent", name="test", type="stdio", config={})
    ...
    async def test_start_server_success(self):
        ...
        with (
            ...
            patch.object(manager.status_tracker, "record_start_time") as mock_record_start,
    ...
    async def test_stop_server_success(self):
        ...
        with (
            ...
            patch.object(manager.status_tracker, "set_status") as mock_set_status,
            patch.object(manager.status_tracker, "record_stop_time") as mock_record_stop,
        ):
    ...
    def test_remove_server_success(self):
        ...
        with (
            patch.object(manager.registry, "get", return_value=config),
            patch.object(manager.registry, "unregister", return_value=True) as mock_unregister,
            ...
            mock_record_event.assert_called_once_with("test-id", "removed", {"message": "Server removed"})
    ...
    def test_list_servers_success(self):
        ...
        with (
            patch.object(manager.status_tracker, "get_uptime", return_value=timedelta(hours=2)),
            patch.object(
                manager.status_tracker,
                "get_server_summary",
    ...
    def test_initialization_loads_existing_servers(self):
        ...
        configs = [
            ServerConfig(id="server1", name="server1", type="stdio", config={"command": "echo"}),
            ServerConfig(
                id="server2",
                name="server2",
                type="sse",
        ...
        with patch("code_puppy.mcp_.manager.ServerRegistry") as mock_registry_class:
            ...
            with (
                ...
                manager.status_tracker.set_status.assert_any_call("server1", ServerState.STOPPED)

                manager.status_tracker.set_status.assert_any_call("server2", ServerState.STOPPED)
    ...
    def test_initialization_handles_server_creation_failures(self):
        ...
        with patch("code_puppy.mcp_.manager.ServerRegistry") as mock_registry_class:
            ...
            with (
                patch("code_puppy.mcp_.manager.ManagedMCPServer", side_effect=side_effect),
                patch("code_puppy.mcp_.manager.ServerStatusTracker") as mock_tracker_class,
            ):
                mock_tracker = Mock()
    ...
    def test_sync_from_config_registers_new_servers(self):
        ...
        with (
            patch("code_puppy.config.load_mcp_server_configs", return_value=mock_configs),
            patch(
