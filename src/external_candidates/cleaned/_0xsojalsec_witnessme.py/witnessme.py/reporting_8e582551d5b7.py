# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WitnessMe\witnessme\reporting.py
import csv
import logging
...
async def generate_html_report(scan_folder, db):
    ...
    async with ScanDatabase(connection=db) as db:
        ...
        offset = 0
        while True:
            services = await db.get_services_with_host(limit=results_per_page, offset=offset)
            if not services:
                break
...
async def generate_csv_report(scan_folder, db):
    ...
    with open(csv_path, "w", newline="") as csvfile:
        ...
        async with ScanDatabase(connection=db) as db:
            offset = 0
            while True:
                services = await db.get_services_with_host(limit=result_limit, offset=offset)
                if not services:
                    break
