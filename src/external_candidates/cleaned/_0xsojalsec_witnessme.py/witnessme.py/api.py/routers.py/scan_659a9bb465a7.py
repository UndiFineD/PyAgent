# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WitnessMe\witnessme\api\routers\scan.py
import asyncio
import functools
import logging
import uuid
...
from witnessme.commands.screenshot import ScanState, ScreenShot
...
async def get_scan_result(scan_id: uuid.UUID, request: Request):
    ...
    if not scan.stats.done:
        return JSONResponse({"error": "scan has not finished"}, status_code=status.HTTP_400_BAD_REQUEST)

    loop = asyncio.get_running_loop()
    zip_file = await loop.run_in_executor(None, functools.partial(zip_scan_folder, scan.report_folder))
