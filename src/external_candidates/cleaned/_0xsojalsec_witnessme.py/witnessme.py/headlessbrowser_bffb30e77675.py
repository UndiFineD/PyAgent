# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WitnessMe\witnessme\headlessbrowser.py
import asyncio
import logging
import os
...
class HeadlessChromium:
    def __init__(self, threads: int = 15, timeout: int = 15, on_new_tab=None, on_finished=None) -> None:
    ...
    async def open_browser_tab(self, context):
        ...
        try:
            results = await asyncio.wait_for(self.on_new_tab(self, url, page), timeout=self.timeout)
    ...
    async def start_browser(self, n_urls: int):
        ...
        try:
            worker_threads = [self.open_browser_tab(context) for _ in range(n_urls)]
    ...
    async def run(self, targets: List[str], ports: List[int] = [80, 8080, 443, 8443]) -> None:
