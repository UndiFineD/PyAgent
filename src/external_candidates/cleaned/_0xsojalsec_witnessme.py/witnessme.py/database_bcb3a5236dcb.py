# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WitnessMe\witnessme\database.py
import json
import logging
...
class ScanDatabase:
    ...
    async def get_hosts(self, limit=-1, offset=-1):
        async with self.db.execute("SELECT * FROM hosts LIMIT (?) OFFSET (?)", [limit, offset]) as cursor:
            return await cursor.fetchall()

    async def get_services(self, limit=-1, offset=-1):
        async with self.db.execute("SELECT * FROM services LIMIT (?) OFFSET (?)", [limit, offset]) as cursor:
            return await cursor.fetchall()
