# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WitnessMe\witnessme\console\wmdb.py
#! /usr/bin/env python3
...
from witnessme.reporting import generate_csv_report, generate_html_report
...
class WMDBShell:
    ...
    async def _print_hosts(self, hosts, table_title=None):
        ...
        table_data.insert(0, ["Id", "IP", "Hostname", "Discovered Services", "Matched Signature(s)"])
    ...
    async def scan(self):
        ...
        start_time = time()
        async with ScanDatabase(connection=self.db) as db:
            tasks = [self.signatures.find_match(service) for service in await db.get_services()]

            matches = list(filter(lambda x: len(x[0]) > 0, await asyncio.gather(*tasks)))

            for match in matches:
                await db.add_matched_sigs_to_service(match[1][0], ",".join([sig["name"] for sig in match[0]]))
