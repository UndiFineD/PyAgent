# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WitnessMe\witnessme\console\witnessme.py
#! /usr/bin/env python3

import argparse
import logging

from witnessme import __codename__, __version__
from witnessme.commands import Grab, ScreenShot
from witnessme.utils import WitnessMeArgFormatter, patch_pyppeteer, start_event_loop
...
def run():
    ...
    parser = argparse.ArgumentParser(description="WitnessMe!", formatter_class=WitnessMeArgFormatter)
    ...
    parser.add_argument("-v", "--version", action="version", version=f"{__version__} - '{__codename__}'")
    ...
    exclusive_group.add_argument("-l", "--links", action="store_true", help="Get all links")
