# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\plugins.py\universal_constructor.py\models_ac68bdf099f4.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\plugins\universal_constructor\models.py
...
class ToolMeta(BaseModel):
    ...
    namespace: str = Field(default="", description="Namespace for the tool (from subdirectory path)")
    ...
    created_at: Optional[str] = Field(default=None, description="When the tool was created (ISO format string)")
...
class UCListOutput(BaseModel):
    ...
    tools: List[UCToolInfo] = Field(default_factory=list, description="List of available tools")
...
class UCCreateOutput(BaseModel):
    ...
    source_path: Optional[str] = Field(default=None, description="Path where tool was saved")

    preview: Optional[str] = Field(default=None, description="Preview of the first 10 lines of source code")
...
class UCUpdateOutput(BaseModel):
    ...
    source_path: Optional[str] = Field(default=None, description="Path to the updated tool")

    preview: Optional[str] = Field(default=None, description="Preview of the first 10 lines of updated source code")
...
class UCInfoOutput(BaseModel):
    ...
    tool: Optional[UCToolInfo] = Field(default=None, description="Tool information if found")

    source_code: Optional[str] = Field(default=None, description="Source code of the tool")
