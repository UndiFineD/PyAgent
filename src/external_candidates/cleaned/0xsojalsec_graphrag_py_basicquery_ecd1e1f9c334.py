# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\query.py\basicquery_ecd1e1f9c334.py
...
class BasicQuery(BaseQuery):
    ...
    async def _retrieve_relevant_contexts_global(self, query):
        ...
        map_communities_points = await self._map_global_communities(query, community_datas)
    ...
    async def generation_qa(self, query, context):
        ...
        response = await self.llm.aask(
            query,
            system_msgs=[sys_prompt_temp.format(report_data=context, response_type=self.config.response_type)],
        )
