# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\tests.py\test_rawdata_json_serialization_e7b369f24b7d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_rawdata_json_serialization.py
...
class TestRawDataJsonSerialization:
    ...
    def test_datetime_serialization(self):
        ...
        assert abs((restored_data.metadata["created_at"] - test_time).total_seconds()) < 1
    ...
    def test_nested_structure_serialization(self):
        ...
        assert restored_data.content["replyInfo"]["author"]["settings"]["notify"] is True
    ...
    def test_round_trip_consistency(self):
        ...
        original_data = RawData(
            ...
            data_type="Test",
            metadata={"test_metadata": {"created": get_now_with_timezone(), "version": 1.0}},
        )
        ...
        assert isinstance(data.content["mixed_types"]["nested"]["deep"]["timestamp"], datetime)
    ...
    def test_datetime_field_recognition(self):
        ...
        for field in datetime_fields:
            ...
            assert raw_data._is_datetime_field(field), f"Field '{field}' should be recognized as a time field"  # pylint: disable=protected-access
        ...
        for field in non_datetime_fields:
            ...
            assert not raw_data._is_datetime_field(field), f"Field '{field}' should not be recognized as a time field"  # pylint: disable=protected-access
    ...
    def test_nested_structure_with_mixed_content(self):
        ...
        assert isinstance(restored_data.content["message_info"]["author"]["last_login_time"], datetime)

        assert isinstance(restored_data.content["system_info"]["build_timestamp"], datetime)

        assert isinstance(restored_data.content["system_info"]["config"]["start_time"], datetime)

        # Verify non-time fields remain as strings

        assert isinstance(restored_data.content["message_info"]["content"], str)

        assert isinstance(restored_data.content["message_info"]["author"]["profile_description"], str)

        assert isinstance(restored_data.content["system_info"]["version"], str)

        assert isinstance(restored_data.content["system_info"]["config"]["timeout"], str)

        assert isinstance(restored_data.content["system_info"]["config"]["schedule"], str)

        # Verify string content remains unchanged

        assert "2024-01-01T10:00:00Z" in restored_data.content["message_info"]["content"]

        assert "2024-01-01T08:00:00+00:00" in restored_data.content["message_info"]["author"]["profile_description"]

        assert restored_data.content["system_info"]["version"] == "2024.01.01T10.00.00"

        assert restored_data.content["system_info"]["config"]["timeout"] == "2024-01-01T10:00:00Z"

        assert "2024-01-01T10:00:00Z" in restored_data.content["system_info"]["config"]["schedule"]
    ...
    def test_edge_cases_improved(self):
        ...
        assert "2024-01-01T10:00:00Z" in restored_data.content["events"][0]["description"]
