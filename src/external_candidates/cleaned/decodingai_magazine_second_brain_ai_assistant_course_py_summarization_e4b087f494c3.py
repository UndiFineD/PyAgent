# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_offline.py\src.py\second_brain_offline.py\application.py\agents.py\summarization_e4b087f494c3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\application\agents\summarization.py
...
"""
    ...
    def __call__(self, documents: Document | list[Document], temperature: float = 0.0) -> Document | list[Document]:
    ...
    async def __summarize_batch(self, documents: list[Document], temperature: float = 0.0) -> list[Document]:
        ...
        summarized_documents = await self.__process_batch(documents, temperature, await_time_seconds=7)

        documents_with_summaries = [doc for doc in summarized_documents if doc.summary is not None]
        ...
        if documents_without_summaries:
            logger.info(f"Retrying {len(documents_without_summaries)} failed documents with increased await time...")

            retry_results = await self.__process_batch(documents_without_summaries, temperature, await_time_seconds=20)
        ...
        logger.info(
            f"Summarization completed: {success_count}/{total_docs} succeeded ✓ | {failed_count}/{total_docs} failed ✗"
        )
    ...
    async def __process_batch(
        ...
        tasks = [
            self.__summarize(document, semaphore, temperature, await_time_seconds=await_time_seconds)
