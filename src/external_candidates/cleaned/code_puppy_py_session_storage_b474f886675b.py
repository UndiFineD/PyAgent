# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\session_storage_b474f886675b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\session_storage.py
...
async def restore_autosave_interactively(base_dir: Path) -> None:
    ...
    def render_page() -> None:
        ...
        for idx, (name, timestamp, message_count) in enumerate(page_entries, start=1):
            ...
            message_display = f"{message_count} messages" if message_count is not None else "unknown size"

            emit_system_message(f"  [{idx}] {name} ({message_display}, saved at {timestamp_display})")
    ...
    emit_success(f"✅ Autosave loaded: {len(history)} messages ({total_tokens} tokens)\n📁 From: {session_path}")
