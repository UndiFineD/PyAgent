# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\tests.py\test_pickle_size_analysis_94cfd70e21ee.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_pickle_size_analysis.py
    ...
    def __init__(self, name: str, data_size: int):
        ...
        self.metadata = {
            ...
            "nested": {
                ...
                "timestamps": [get_now_with_timezone() + timedelta(seconds=i) for i in range(10)],
            },
        }
...
class LargeDataContainer:
    ...
    def __init__(self, size_mb: float):
        ...
        self.large_dict = {f"key_{i}": f"value_{i}_{'x' * 100}" for i in range(target_size // 100)}
...
async def test_complex_objects_size():
    ...
    for name, obj in test_objects:
        ...
        obj_memory = format_size(sys.getsizeof(obj) + sys.getsizeof(obj.data) + sys.getsizeof(obj.metadata))
...
async def test_redis_storage_efficiency():
    ...
    for name, data in test_data:
        ...
        storage_size = len(processed_data) if isinstance(processed_data, bytes) else len(processed_data.encode("utf-8"))
        ...
        if success and retrieved_data:
            ...
            logger.error("%-15s | Storage: %-12s | Status: ❌", name, format_size(storage_size))
