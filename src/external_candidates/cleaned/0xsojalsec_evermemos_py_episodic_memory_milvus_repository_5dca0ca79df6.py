# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\search.py\repository.py\episodic_memory_milvus_repository_5dca0ca79df6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\search\repository\episodic_memory_milvus_repository.py
...
class EpisodicMemoryMilvusRepository(BaseMilvusRepository[EpisodicMemoryCollection]):
        ...
        try:
            ...
            if metadata is None:
                metadata_dict = {
                    ...
                    "parent_event_id": parent_event_id or "",  # Parent event ID, used to associate split records
                }
            ...
            entity = {
                "id": id,
                "vector": vector,
                "user_id": user_id or "",  # Milvus VARCHAR does not accept None, convert to empty string
            ...
            logger.error("❌ Failed to create episodic memory document: id=%s, error=%s", id, e)
    ...
    async def vector_search(
        ...
        try:
            ...
            logger.info(f"Milvus raw return: {raw_hit_count} results, limit={limit}, filter_str={filter_str}, ")
            ...
            logger.debug("✅ Vector search successful: Found %d results", len(search_results))
    ...
    async def delete_by_event_id(self, event_id: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.debug("✅ Deleted episodic memory by event_id: event_id=%s", event_id)
