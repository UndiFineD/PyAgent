# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\tests.py\actions.py\test_elapsed_ticks_da306ad5faf2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\actions\test_elapsed_ticks.py
...
def test_sleep_elapsed_ticks_and_timing(game):
    ...
    assert ticks_added == sleep_seconds * 60, f"Expected {sleep_seconds * 60} ticks, got {ticks_added}"

    assert abs(real_sleep_time - sleep_seconds) < 0.1, f"Expected ~{sleep_seconds}s sleep, got {real_sleep_time:.2f}s"
...
def test_harvest_resource_elapsed_ticks_and_timing(game):
    ...
    assert 50 <= ticks_added <= 80, f"Expected ~60 ticks for iron ore harvest, got {ticks_added}"
