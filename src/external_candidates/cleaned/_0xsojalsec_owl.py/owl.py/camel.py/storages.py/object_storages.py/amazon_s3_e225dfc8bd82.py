...
class AmazonS3Storage(BaseObjectStorage):
    ...
    def _prepare_and_check(self) -> None:
        ...
        try:
            ...
            if error_code == "403":
                raise PermissionError(f"Failed to access bucket {self._bucket_name}: No permission.")
            elif error_code == "404":
                if self._create_if_not_exists:
                    self._client.create_bucket(Bucket=self._bucket_name)
                    warn(f"Bucket {self._bucket_name} not found. Automatically created.")
                else:
                    raise FileNotFoundError(f"Failed to access bucket {self._bucket_name}: Not found.")
            else:
                raise e
