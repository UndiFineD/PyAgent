# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\storages\object_storages\azure_blob.py
    ...
    def _prepare_and_check(self) -> None:
        ...
        try:
            ...
            if not exists and self._create_if_not_exists:
                ...
                warn(f"Container {self._client.container_name} not found. Automatically created.")
            elif not exists:
                raise FileNotFoundError(f"Failed to access container {self._client.container_name}: Not found.")
        except ClientAuthenticationError:
            raise PermissionError(f"Failed to access container {self._client.container_name}: No permission.")
