# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\storages\graph_storages\neo4j_graph.py
...
INCLUDE_DOCS_QUERY = (
    "MERGE (d:Element {id:$element['element_id']}) SET d.text = $element['text'] SET d += $element['metadata'] WITH d "
)
    ...
    def add_graph_elements(
        ...
        if base_entity_label:  # check if constraint already exists
            ...
            if not constraint_exists:
                # Create constraint
                self.query(f"CREATE CONSTRAINT IF NOT EXISTS FOR(b:{BASE_ENTITY_LABEL}) REQUIRE b.id IS UNIQUE;")
