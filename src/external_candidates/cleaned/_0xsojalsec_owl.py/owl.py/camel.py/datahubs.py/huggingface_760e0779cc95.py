# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\datahubs\huggingface.py
...
class HuggingFaceDatasetManager(BaseDatasetManager):
    ...
    def add_records(
        ...
        **kwargs: Any,
        ...
        if existing_records:
            raise ValueError(f"Dataset '{filepath}' already exists. Use `update_records` to modify.")
    ...
    def delete_record(
        ...
        **kwargs: Any,
        ...
        if not existing_records:
            raise ValueError(f"Dataset '{dataset_name}' does not have an existing file to delete records from.")
