# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\toolkits\ask_news_toolkit.py
...
class AskNewsToolkit(BaseToolkit):
    ...
    def search_reddit(
        ...
        """
        try:
            response = self.asknews_client.news.search_reddit(keywords=keywords, n_threads=n_threads, method=method)
    ...
    def query_finance(
        ...
        try:
            ...
            if return_type == "list":
                ...
                descriptive_text = "\n".join(
                    [f"On {entry.datetime}, the sentiment value was {entry.value}." for entry in time_series_data]
                )
