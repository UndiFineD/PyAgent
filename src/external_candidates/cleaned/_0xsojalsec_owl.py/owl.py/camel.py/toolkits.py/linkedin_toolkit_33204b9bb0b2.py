# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\toolkits\linkedin_toolkit.py
...
class LinkedInToolkit(BaseToolkit):
    ...
    def delete_post(self, post_id: str) -> str:
        ...
        if response.status_code != HTTPStatus.NO_CONTENT:
            ...
            return f"Request returned a(n) {error_type!s}: {response.status_code!s} {response.text}"
    ...
    def get_profile(self, include_id: bool = False) -> dict:
        ...
        if response.status_code != HTTPStatus.OK:
            raise Exception(
                f"Failed to retrieve profile. Status code: {response.status_code}, Response: {response.text}"
            )
