# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\toolkits\image_analysis_toolkit.py
...
from camel.models import BaseModelBackend, ModelFactory, OpenAIModel
...
class ImageAnalysisToolkit(BaseToolkit):
    ...
    def ask_question_about_image(self, image_path: str, question: str) -> str:
        ...
        if not (image_path.endswith(".jpg") or image_path.endswith(".jpeg") or image_path.endswith(".png")):
            logger.warning(
                f"The image path `{image_path}` is not a valid image path. Please provide a valid image path."
            )
