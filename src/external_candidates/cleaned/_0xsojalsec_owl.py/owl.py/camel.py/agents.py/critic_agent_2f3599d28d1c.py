# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\agents\critic_agent.py
...
class CriticAgent(ChatAgent):
    ...
    def flatten_options(self, messages: Sequence[BaseMessage]) -> str:
        ...
        flatten_options = (
            f"> Proposals from {messages[0].role_name} ({messages[0].role_type}). Please choose an option:\n"
        )
        for index, option in enumerate(options):
            flatten_options += f"Option {index + 1}:\n{option}\n\n"
            self.options_dict[str(index + 1)] = option
        format = (
            f"Please first enter your choice ([1-{len(self.options_dict)}]) and then your explanation and comparison: "
        )
    ...
    def get_option(self, input_message: BaseMessage) -> str:
        ...
        while i < self.retry_attempts:
            ...
            if choice in self.options_dict:
                ...
                i += 1
        warnings.warn(
            f"Critic failed to get a valid option. After {self.retry_attempts} attempts. Returning a random option."
        )
