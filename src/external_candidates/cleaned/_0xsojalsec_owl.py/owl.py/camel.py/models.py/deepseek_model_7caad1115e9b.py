            ...
            response = ChatCompletion.construct(
                ...
                choices=[
                    dict(
                        ...
                        finish_reason=(
                            response.choices[0].finish_reason if response.choices[0].finish_reason else None
                        ),
                    )
                ],
    ...
    def check_model_config(self):
        ...
        for param in self.model_config_dict:
            if param not in DEEPSEEK_API_PARAMS:
                raise ValueError(f"Unexpected argument `{param}` is input into DeepSeek model backend.")
