# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\models\fish_audio_model.py
...
class FishAudioModel:
    ...
    def __init__(
        ...
        self._url = url or os.environ.get("FISHAUDIO_API_BASE_URL", "https://api.fish.audio")
        ...
        **kwargs: Any,
        ...
        if not reference_audio:
            with open(f"{storage_path}", "wb") as f:
                for chunk in self.session.tts(TTSRequest(reference_id=reference_id, text=input, **kwargs)):
                    f.write(chunk)
        else:
