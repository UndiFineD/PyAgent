# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\societies\role_playing.py
...
class RolePlaying:
    ...
    def __init__(
        ...
        *,
        ...
        if model is not None:
            logger.warning("Model provided globally is set for all agents if not already specified in agent_kwargs.")
    ...
    def _reduce_message_options(
        ...
        if len(messages) > 1 and not self.with_critic_in_the_loop:
            raise ValueError(f"Got than one message to process. Num of messages: {len(messages)}.")
