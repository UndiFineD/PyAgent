# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\societies\workforce\worker.py
...
class Worker(BaseNode, ABC):
    ...
    def _get_dep_tasks_info(dependencies: List[Task]) -> str:
        result_lines = [
            f"id: {dep_task.id}, content: {dep_task.content}. result: {dep_task.result}." for dep_task in dependencies
        ]
        result_str = "\n".join(result_lines)
        return result_str
    ...
    async def _listen_to_channel(self):
        ...
        while True:
            ...
            print(f"{Fore.YELLOW}{self} get task {task.id}: {task.content}{Fore.RESET}")
