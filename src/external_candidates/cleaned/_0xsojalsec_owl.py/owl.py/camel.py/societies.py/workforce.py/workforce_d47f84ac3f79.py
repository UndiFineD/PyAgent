# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\societies\workforce\workforce.py
...
class Workforce(BaseNode):
    ...
    def _get_child_nodes_info(self) -> str:
        ...
        for child in self._children:
            ...
            info += f"<{child.node_id}>:<{child.description}>:<{additional_info}>\n"
        return info
    ...
    async def _listen_to_channel(self) -> None:
        ...
        while self._task is None or self._pending_tasks:
            ...
            if returned_task.state == TaskState.DONE:
                ...
                if not halt:
                    continue
                print(f"{Fore.RED}Task {returned_task.id} has failed for 3 times, halting the workforce.{Fore.RESET}")
                break
