# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\annotation_4432277c753e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\annotation.py
    ...
    def _annotate_documents_single_pass(
        ...
        **kwargs,
            ...
            for text_chunk, scored_outputs in zip(batch, batch_scored_outputs):
                ...
                while curr_document.document_id != text_chunk.document_id:
                    ...
                    assert curr_document is not None, (
                        f"Document should be defined for {text_chunk} per _document_chunk_iterator(...) specifications."
                    )
                ...
                annotated_chunk_extractions = resolver.resolve(top_inference_result, debug=debug, **kwargs)
        ...
        if curr_document is not None:
            logging.info("Finalizing annotation for document ID %s.", curr_document.document_id)
