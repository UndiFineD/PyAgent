...
class LongJobManager:
    ...
    def __init__(self):
        ...
        self.auto_discover = os.getenv("LONGJOB_AUTO_DISCOVER", "true").lower() == "true"

        self.auto_start_mode = os.getenv("LONGJOB_AUTO_START_MODE", "all").lower()

        self.job_whitelist = self._parse_job_list(os.getenv("LONGJOB_JOB_WHITELIST", ""))

        self.job_blacklist = self._parse_job_list(os.getenv("LONGJOB_JOB_BLACKLIST", ""))
        ...
        self.wait_for_current_task = os.getenv("LONGJOB_WAIT_FOR_CURRENT_TASK", "true").lower() == "true"

        self.log_level = os.getenv("LONGJOB_LOG_LEVEL", "INFO")

        self.log_startup_details = os.getenv("LONGJOB_LOG_STARTUP_DETAILS", "true").lower() == "true"

        self.log_job_lifecycle = os.getenv("LONGJOB_LOG_JOB_LIFECYCLE", "true").lower() == "true"
    ...
    async def add_job(self, job: LongJobInterface) -> bool:
        ...
        if self._is_shutdown:
            self.logger.warning("Cannot add job %s: manager is shutting down", job.job_id)
        ...
        try:
            ...
            for job in job_implementations:
                ...
                try:
                    ...
                    if success:
                        self.logger.info("Auto-discovered and added job: %s", job.job_id)

                    else:
                        self.logger.warning("Failed to add auto-discovered job: %s", job.job_id)
            ...
            self.logger.info("Auto-discovery completed. Found %d job implementations", len(results))

            return results

        except Exception as e:
            self.logger.error("Error during job auto-discovery: %s", str(e), exc_info=True)

            return {}

    async def default_start(self, auto_discover: Optional[bool] = None) -> Dict[str, Any]:
        """
        ...
        if self.log_startup_details:
            ...
            self.logger.info("Starting default startup process with config: %s", config_info)
        ...
        try:
            ...
            should_auto_discover = auto_discover if auto_discover is not None else self.auto_discover
            ...
            if jobs_to_start:
                ...
                results["total_started"] = sum(1 for success in started.values() if success)
    ...
    async def start_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error("Failed to start job %s: %s", job_id, str(e), exc_info=True)

            return False

    async def stop_job(self, job_id: str, timeout: float = 30.0, wait_for_current_task: bool = True) -> bool:
        """
        ...
        try:
            ...
            if hasattr(job, "shutdown") and "wait_for_current_task" in job.shutdown.__code__.co_varnames:
                await asyncio.wait_for(
                    job.shutdown(timeout=timeout, wait_for_current_task=wait_for_current_task),
            ...
            if job_id in self._job_tasks:
                ...
                if not task.done():
                    try:
                        ...
                        self.logger.warning("Task for job %s did not complete, cancelling", job_id)
            ...
            self.logger.error("Failed to stop job %s: %s", job_id, str(e), exc_info=True)
        ...
        if stop_tasks:
            ...
            for job_id, result in zip(running_jobs, stop_results):
                if isinstance(result, Exception):
                    self.logger.error("Exception stopping job %s: %s", job_id, str(result))
    ...
    async def shutdown(self, timeout: float = 60.0) -> None:
        ...
        try:
            ...
            await asyncio.wait_for(self.stop_all_jobs(timeout=timeout / 2), timeout=timeout)

        except asyncio.TimeoutError:
            self.logger.warning("Manager shutdown timeout, some jobs may not have stopped gracefully")

        except Exception as e:
            self.logger.error("Error during manager shutdown: %s", str(e), exc_info=True)
        ...
        stats.update(
            {
                ...
                "uptime": (get_now_with_timezone() - stats["manager_start_time"]).total_seconds(),
            }
        )
