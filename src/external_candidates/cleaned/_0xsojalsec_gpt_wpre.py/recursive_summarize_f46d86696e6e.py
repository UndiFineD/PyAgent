...
def summarize_long_code(decomp, summaries, callees, max_lines=100, strategy="long"):
    ...
    for i in range(0, len(codelines), max_lines):
        ...
        chunk_summaries.append(summarize(prompt, max_tokens=(512 if strategy == "long" else 256)))
...
def summarize_all(topo_order, callgraph, decompilations, max_lines=100, already_summarized=None):
    ...
    for func in topo_order:
        ...
        while summary is None:
            try:
                if DEBUG:
                    print(f"Trying to summarize {func} in chunks of {num_lines} lines with paragraphs...")
        ...
        while summary is None:
            try:
                if DEBUG:
                    print(f"Trying to summarize {func} in chunks of {num_lines} lines with sentences...")
