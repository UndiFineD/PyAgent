# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\mcp.py\help_command_67ae91f47637.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\mcp\help_command.py
...
class HelpCommand(MCPCommandBase):
    ...
    def execute(self, args: List[str], group_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            help_lines.append(
                Text("/mcp search", style="cyan") + Text(" [query]     Search 30+ pre-configured servers")
            )
            ...
            help_lines.append(
                Text("/mcp status", style="cyan") + Text(" [name]      Show detailed status (all servers or specific)")
            )

            help_lines.append(Text("/mcp test", style="cyan") + Text(" <name>        Test connectivity to a server"))

            help_lines.append(
                Text("/mcp logs", style="cyan") + Text(" <name> [limit] Show recent events (default limit: 10)")
            )

            help_lines.append(Text("/mcp edit", style="cyan") + Text(" <name>        Edit existing server config"))
