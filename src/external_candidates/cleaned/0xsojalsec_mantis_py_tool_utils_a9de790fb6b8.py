# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_mantis.py\mantis.py\utils.py\tool_utils_a9de790fb6b8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\utils\tool_utils.py
...
async def get_assets_grouped_by_type(self, args, asset_type):
    ...
    pipeline_type_assets.extend(
        [
            ...
            {"$group": {"_id": None, "assets": {"$push": "$asset"}}},
        ]
    )
...
async def get_assets_with_empty_fields(self, args, field_name):
    ...
    pipeline_empty_fields.extend(
        [
            {"$match": {"org": args.org}},
            {"$match": {field_name: {"$in": [None, "", []]}}},
            {"$group": {"_id": None, "asset": {"$push": "$asset"}}},
        ]
    )
...
async def get_assets_with_non_empty_fields(self, args, field_name):
    ...
    pipeline_non_empty_fields.extend(
        [
            ...
            {"$group": {"_id": "$asset", field_name: {"$push": f"${field_name}"}}},
        ]
    )
...
def get_pipeline(self, args):
    ...
    if args.app:
        pipeline.append({"$match": {"org": args.app}})

    if args.ignore_stale:
        pipeline.append({"$match": {"stale": {"$eq": False}}})

    if self is not None:
        if ConfigUtils.is_scanNewOnly_tool(type(self).__name__, args):
            pipeline.append({"$match": {"created_timestamp": {"$gte": datetime.today().strftime("%Y-%m-%d")}}})
