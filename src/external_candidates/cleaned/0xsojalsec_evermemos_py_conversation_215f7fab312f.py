# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\memory_layer.py\memory_extractor.py\profile_memory.py\conversation_215f7fab312f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\memory_layer\memory_extractor\profile_memory\conversation.py
...
def _append_user_ids_to_content(content: Any, refer_list: Any, *, use_at_symbol: bool = True) -> Any:
    ...
    for refer in refer_list:
        ...
        updated_content, count = pattern.subn(lambda match: replacement, updated_content)
...
def build_conversation_text(memcell: MemCell, user_id_to_name: Dict[str, str]) -> Tuple[str, Optional[str]]:
    ...
    for data in data_list:
        ...
        if timestamp:
            lines.append(f"[{timestamp}][conversation_id:{conversation_id_str}] {speaker}: {content}")

        else:
            lines.append(f"[conversation_id:{conversation_id_str}] {speaker}: {content}")

    return "\n".join(lines), conversation_id_str or None


def build_episode_text(memcell: MemCell, user_id_to_name: Dict[str, str]) -> Tuple[str, Optional[str]]:
...
def build_evidence_completion_prompt(conversation_text: str, profiles_without_evidences: List[Dict[str, Any]]) -> str:
