...
class CertManager:
    ...
    def create_ssl_context(self):
        ...
        self.ssl_context.load_cert_chain(certfile=self.ip_cert_path, keyfile=self.ip_key_path)
    ...
    def load_ip_certificate(self):
        ...
        if (
            os.path.exists(self.ip_cert_path)
            and os.path.exists(self.ip_key_path)
        ...
        return self.generate_server_certificate(
            self.ip_cert_path,
            self.ip_key_path,
            self.ip_address,
    ...
    def load_dns_certificate(self):
        ...
        if (
            os.path.exists(self.dns_cert_path)
            and os.path.exists(self.dns_key_path)
    ...
    def cert_is_valid(self, cert_path, common_name):
        ...
        if (
            os.getenv("USE_DYNAMIC_SERVER_THUMBPRINT", "false").lower() == "true"
            or os.getenv("SERVER_SHA1_THUMBPRINT", "") != ""
            or os.getenv("SERVER_MD5_THUMBPRINT", "") != ""
        ):
            return True
        ...
        date_valid = (
            cert.not_valid_before_utc <= datetime.datetime.now(datetime.timezone.utc) <= cert.not_valid_after_utc
        )
    ...
    def generate_ca_certificate(self):
        ...
        self.ca_key = rsa.generate_private_key(public_exponent=65537, key_size=2048, backend=default_backend())
    ...
    def generate_codesign_certificate(self, common_name, pfx_path=None, cert_path=None, key_path=None):
        ...
        if (
            ...
            and os.path.exists(pfx_path)
            and self.cert_is_valid(cert_path, common_name)
        ):
        ...
        codesign_private_key = rsa.generate_private_key(public_exponent=65537, key_size=2048, backend=default_backend())
        ...
        builder = (
            ...
            .subject_name(subject)
            .issuer_name(self.ca_cert.subject)
            .public_key(codesign_private_key.public_key())
            .serial_number(x509.random_serial_number())
            .not_valid_before(datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1))
        ...
        with open(key_path, "wb") as f:
            f.write(
                codesign_private_key.private_bytes(
                    encoding=serialization.Encoding.PEM,
                    format=serialization.PrivateFormat.TraditionalOpenSSL,
                    encryption_algorithm=serialization.NoEncryption(),
                )
    ...
    def generate_apple_certificate(self, common_name="Developer ID Installer", cert_path=None, key_path=None):
        ...
        apple_private_key = rsa.generate_private_key(public_exponent=65537, key_size=2048, backend=default_backend())
        ...
        builder = (
            ...
            .subject_name(subject)
            .issuer_name(self.ca_cert.subject)
            .public_key(apple_private_key.public_key())
            .serial_number(x509.random_serial_number())
            .not_valid_before(datetime.datetime.utcnow() - datetime.timedelta(days=1))
            .not_valid_after(datetime.datetime.utcnow() + datetime.timedelta(days=365))
            .add_extension(
        ...
        with open(key_path, "wb") as f:
            f.write(
                apple_private_key.private_bytes(
                    encoding=serialization.Encoding.PEM,
                    format=serialization.PrivateFormat.TraditionalOpenSSL,
                    encryption_algorithm=serialization.NoEncryption(),
                )
