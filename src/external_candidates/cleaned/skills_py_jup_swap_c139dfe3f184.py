# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\spendit_ai.py\solana_skills.py\scripts.py\jup_swap_c139dfe3f184.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\spendit-ai\solana-skills\scripts\jup_swap.py
...
async def get_quote(input_mint: str, output_mint: str, amount: int, taker: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{JUPITER_ULTRA_API_URL}/order", params=params, headers=headers) as response:
                if response.status != 200:
                    error_data = await response.text()

                    print(f"Error: Jupiter API error ({response.status}): {error_data}")
                ...
                if data.get("error") or data.get("errorMessage"):
                    print(f"Error: {data.get('error') or data.get('errorMessage')}")
...
async def execute_swap(signed_tx: VersionedTransaction, request_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{JUPITER_ULTRA_API_URL}/execute", json=payload, headers=headers) as response:
                if response.status != 200:
                    error_data = await response.text()

                    print(f"Error: Execute failed ({response.status}): {error_data}")
...
def cmd_quote(args):
    ...
    print(f"  You pay: {format_amount(in_amount, input_mint)} {get_token_name(input_mint)}")

    print(f"  You get: {format_amount(out_amount, output_mint)} {get_token_name(output_mint)}")
    ...
    if in_amount > 0 and out_amount > 0:
        ...
        rate = (Decimal(out_amount) / Decimal(10**out_dec)) / (Decimal(in_amount) / Decimal(10**in_dec))

        print(f"  Rate: 1 {get_token_name(input_mint)} = {rate:.6f} {get_token_name(output_mint)}")
...
Environment variables (required):
  ...
  JUPITER_API_KEY       Jupiter API key (get from https://portal.jup.ag/)
    ...
    quote_parser.add_argument("input_mint", help="Input token (symbol or mint)")

    quote_parser.add_argument("output_mint", help="Output token (symbol or mint)")
    ...
    swap_parser.add_argument("output_mint", help="Output token (symbol or mint)")
