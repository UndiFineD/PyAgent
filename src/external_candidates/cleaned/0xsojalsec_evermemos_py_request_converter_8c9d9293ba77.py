# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\api_specs.py\request_converter_8c9d9293ba77.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\api_specs\request_converter.py
...
async def convert_single_message_to_raw_data(
    ...
    content = {
        ...
        "timestamp": from_iso_format(input_data.get("createTime"), ZoneInfo("UTC")),  # Use converted UTC time
        "createBy": input_data.get("createBy"),
        "updateTime": from_iso_format(input_data.get("updateTime"), ZoneInfo("UTC")),  # Use converted UTC time
    ...
    metadata = {
        "original_id": data_id,
        "createTime": from_iso_format(input_data.get("createTime"), ZoneInfo("UTC")),  # Use converted UTC time
        "updateTime": from_iso_format(input_data.get("updateTime"), ZoneInfo("UTC")),  # Use converted UTC time
        "createBy": input_data.get("createBy"),
        "orgId": input_data.get("orgId"),
    }
...
async def convert_conversation_to_raw_data_list(
    ...
    return [
        await convert_single_message_to_raw_data(data, data_id_field=data_id_field, group_name=group_name)
...
async def handle_conversation_format(data: Dict[str, Any]) -> MemorizeRequest:
    ...
    raw_data_list = await convert_conversation_to_raw_data_list(messages, group_name=group_name)
