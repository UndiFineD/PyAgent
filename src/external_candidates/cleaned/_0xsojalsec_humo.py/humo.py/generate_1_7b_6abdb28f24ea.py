# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\generate_1_7B.py
...
class Generator:
    ...
    def configure_dit_model(self, device=get_device()):
        ...
        self.logger.info(f"[RANK:{get_global_rank()}] DiT Parameters: {clever_format(params, '%.3f')}")
        ...
        for lt_i in range(iter_):
            if lt_i == 0:
                ...
                wind_feat = torch.stack(
                    [(audio_emb[i] if (0 <= i < audio_emb.shape[0]) else zero_audio_embed) for i in range(st, ed)],
                    dim=0,
                )
    ...
    def forward_tia(self, latents, latents_ref, latents_ref_neg, timestep, arg_t, arg_ta, arg_null):
        neg = self.dit(
            [
                torch.cat([latent[:, : -latent_ref_neg.shape[1]], latent_ref_neg], dim=1)
        ...
        noise_pred = (
            self.config.generation.scale_i * (pos_tia - pos_ta)
    ...
    def forward_ta(self, latents, latents_ref_neg, timestep, arg_t, arg_ta, arg_null):
        ...
        noise_pred = (
            self.config.generation.scale_a * (pos_ta - pos_t) + self.config.generation.scale_t * (pos_t - neg) + neg
        )
    ...
    def save_sample(self, *, sample: torch.Tensor, audio_path: str, itemname: str):
        ...
        if sample.ndim == 4:
            if audio_path is not None:
                tensor_to_video(sample.numpy(), pathname, audio_path, fps=gen_config.fps)
            else:
                mediapy.write_video(
                    path=pathname,
                    images=sample.numpy(),
