...
class Generator:
    ...
    def forward_tia(self, latents, timestep, t, step_change, arg_tia, arg_ti, arg_i, arg_null):
        ...
        if t > step_change:
            ...
            neg, _ = self.parse_output(self.dit(latents, t=timestep, **arg_null))  # img not included in null
            torch.cuda.empty_cache()
    ...
    def inference(
        ...
        with amp.autocast(dtype=torch.bfloat16), torch.no_grad(), no_sync():
            ...
            y_null = self.zero_vae[:, : target_shape[1]].to(device=get_device(), dtype=latents_ref[0].dtype)
