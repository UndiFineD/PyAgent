# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HuMo\humo\models\wan_modules\model_humo.py
import math
import types
...
class AudioCrossAttentionWrapper(nn.Module):
    def __init__(
        ...
        self.audio_cross_attn = WanT2VCrossAttentionGather(kv_dim, dim, num_heads, (-1, -1), qk_norm, eps)
    ...
    def forward(self, x, audio, seq_lens, grid_sizes, freqs, audio_seq_len):
        x = x + self.audio_cross_attn(self.norm1_audio(x), audio, seq_lens, grid_sizes, freqs, audio_seq_len)
        return x
...
class WanModel(nn.Module):
    ...
    def __init__(
        ...
        if is_unified_parallel_initialized():
            ...
            from humo.models.distributed.dit_ulysses_sequence_parallel import (
                ulysses_attn_forward,
                ulysses_audio_cross_attn_forward,
                ulysses_dit_forward,
            )
            ...
            for block in self.blocks:
                ...
                if block.use_audio:
                    block.audio_cross_attn_wrapper.audio_cross_attn.forward = types.MethodType(
                        ulysses_audio_cross_attn_forward,
                        block.audio_cross_attn_wrapper.audio_cross_attn,
                    )
    ...
    def forward(
        ...
        if self.insert_audio:
            ...
            audio_seq_len = torch.tensor(
                max([au.shape[2] for au in audio]) * audio[0].shape[3],
                device=get_device(),
            )
