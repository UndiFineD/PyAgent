# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2023.py\standup_maths.py\pool_39826f1ff81e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2023\standup_maths\pool.py
...
class PoolTableReflections(InteractiveScene):
    def construct(self):
        ...
        def line_to_trajectory(line, n_reflections=2):
            ...
            if n_reflections == 1:
                trajectory.set_points_as_corners([line.get_start(), p1, get_reflection(line.get_end(), [1])])
        ...
        self.play(FadeOut(traj_one_ref), trajectory.animate.set_stroke(opacity=1))
        ...
        self.play(
            ...
            UpdateFromFunc(trajectory, lambda m: m.become(line_to_trajectory(straight_line))),
            UpdateFromFunc(trajectory, lambda m: m.become(line_to_trajectory(straight_line))),
            UpdateFromFunc(ball, lambda m: m.move_to(trajectory.get_end())),
            run_time=4,
        ...
        vect1 = np.array([0.1, 0.315, 0.0])

        vect2 = np.array([0.309, 0.1, 0.0])
        ...
        self.play(
            ...
            *(tg[1].animate.set_stroke(width=1) for tg in [table_group1, table_group2, table_group3]),
            run_time=2,
        )
        ...
        for line in lines:
            ...
            traj.add_updater(lambda t: t.become(line_to_trajectory(t.line)).set_color(BLUE))
        ...
        for n in [70, 60, 90, 100, 80]:
            self.play(Transform(lines, n_to_lines(n)), run_time=3)
        ...
        for point in [
            ...
            self.play(Transform(lines, get_clean_lines(point)), run_time=2)
