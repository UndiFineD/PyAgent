# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\FastBlend\api.py
import os
...
def check_input_for_blending(video_guide, video_guide_folder, video_style, video_style_folder):
    ...
    if len(frames_guide) < len(frames_style):
        message += (
            f"The number of frames mismatches. Only the first {len(frames_guide)} frames of style video will be used.\n"
        )
        frames_style.set_length(len(frames_guide))
    elif len(frames_guide) > len(frames_style):
        message += (
            f"The number of frames mismatches. Only the first {len(frames_style)} frames of guide video will be used.\n"
        )
