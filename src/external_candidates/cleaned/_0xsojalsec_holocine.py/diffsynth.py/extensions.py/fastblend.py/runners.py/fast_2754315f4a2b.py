# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\FastBlend\runners\fast.py
import functools
import os
    ...
    def task_list(self, n):
        ...
        for i in range(n):
            ...
            for level in range(max_level):
                ...
                if j >= n:
                    break
                meta_data = {"source": i, "target": j, "level": level + 1}
    ...
    def process_window_sum(
        ...
        for target in range(n):
            ...
            for source, level in node_list:
                if source != target:
                    meta_data = {"source": source, "target": target, "level": level}
                    tasks.append(meta_data)
                else:
...
class FastModeRunner:
    ...
    def run(
        ...
        # left part
        table_l = table_manager.build_remapping_table(
            frames_guide,
            frames_style,
            patch_match_engine,
            batch_size,
            desc="Fast Mode Step 1/4",
        )
        table_l = table_manager.remapping_table_to_blending_table(table_l)
        table_l = table_manager.process_window_sum(
            frames_guide,
            table_l,
            patch_match_engine,
            window_size,
            batch_size,
            desc="Fast Mode Step 2/4",
        )
        # right part
        table_r = table_manager.build_remapping_table(
            frames_guide[::-1],
            frames_style[::-1],
            patch_match_engine,
            batch_size,
            desc="Fast Mode Step 3/4",
        )
        table_r = table_manager.remapping_table_to_blending_table(table_r)
        table_r = table_manager.process_window_sum(
            frames_guide[::-1],
            table_r,
            patch_match_engine,
            window_size,
            batch_size,
            desc="Fast Mode Step 4/4",
        )[::-1]
        # merge
        frames = []
