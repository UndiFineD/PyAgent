# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\FastBlend\runners\interpolation.py
import os
    ...
    def run(
        ...
        for tasks in task_group:
            ...
            for batch_id in tqdm(
                ...
                desc=f"Rendering frames {index_start}...{index_end}",
            ):
...
class InterpolationModeSingleFrameRunner:
    ...
    def run(
        ...
        for i in tqdm(
            range(0, n, batch_size - tracking_window_size * 2),
            desc=f"Rendering frames 0...{n}",
        ):
            if i + batch_size > n:
