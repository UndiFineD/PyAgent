# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\extensions\ImageQualityMetric\hps.py
import os
        ...
        try:
            if isinstance(images, (str, Image.Image)):
                ...
                if isinstance(images, str):
                    image = (
                        self.preprocess_val(Image.open(images)).unsqueeze(0).to(device=self.device, non_blocking=True)
                    )
                else:
