# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\dancer.py
import torch

from ..controlnets import MultiControlNetManager
from ..models import SDMotionModel, SDUNet, SDXLMotionModel, SDXLUNet
from ..models.sd_unet import PopBlock, PushBlock
...
def lets_dance(
    ...
    for block_id, block in enumerate(unet.blocks):
        ...
        if motion_modules is not None:
            if block_id in motion_modules.call_block_id:
                ...
                hidden_states, time_emb, text_emb, res_stack = motion_modules.motion_modules[motion_module_id](
                    hidden_states, time_emb, text_emb, res_stack, batch_size=1
                )
        # 4.3 ControlNet
        if block_id == controlnet_insert_block_id and additional_res_stack is not None:
            ...
            if vram_limit_level >= 1:
                res_stack = [
                    (res.to(device) + additional_res.to(device)).cpu()
                    for res, additional_res in zip(res_stack, additional_res_stack)
                ]
            else:
...
def lets_dance_xl(
    ...
    text_emb = (
        ...
        if unet.text_intermediate_proj is None
        else unet.text_intermediate_proj(encoder_hidden_states)
    )
