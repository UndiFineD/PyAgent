...
class FluxImagePipeline(BasePipeline):
    ...
    def preprocess_masks(self, masks, height, width, dim):
        ...
        for mask in masks:
            mask = (
                self.preprocess_image(mask.resize((width, height), resample=Image.NEAREST)).mean(dim=1, keepdim=True)
                > 0
            )
    ...
    def prepare_entity_inputs(
        ...
        if enable_eligen_inpaint:
            ...
            fg_masks = torch.cat(
                [
                    self.preprocess_image(mask.resize((width // 8, height // 8))).mean(dim=1, keepdim=True)
                    for mask in masks_
                ]
            )
        ...
        if controlnet_image is not None:
            ...
            if len(masks) > 0 and controlnet_inpaint_mask is not None:
                ...
                local_controlnet_kwargs = [
                    {"controlnet_frames": self.prepare_controlnet_input(controlnet_image, mask, tiler_kwargs)}
                    for mask in masks
                ]
            else:
    ...
    def prepare_prompts(
        ...
        prompt_emb_nega = (
            self.encode_prompt(negative_prompt, positive=False, t5_sequence_length=t5_sequence_length)
            if cfg_scale != 1.0
            else None
        )
        prompt_emb_locals = [
            self.encode_prompt(prompt_local, t5_sequence_length=t5_sequence_length) for prompt_local in local_prompts
        ]
...
class InfinitYou:
    ...
    def prepare_infinite_you(self, model, id_image, controlnet_image, infinityou_guidance, height, width):
        ...
        )[-1]["kps"]  # only use the maximum face
...
def lets_dance_flux(
    ...
    **kwargs,
    ...
    if tiled:
        ...
        def flux_forward_fn(hl, hr, wl, wr):
            tiled_controlnet_frames = (
                [f[:, :, hl:hr, wl:wr] for f in controlnet_frames] if controlnet_frames is not None else None
            )
    ...
    if controlnet is not None and controlnet_frames is not None:
        ...
        controlnet_res_stack, controlnet_single_res_stack = controlnet(controlnet_frames, **controlnet_extra_kwargs)
