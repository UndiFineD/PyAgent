    ...
    **kwargs,
    ...
    if shot_mask_type is not None and shot_indices is not None:
        ...
        if shot_mask_type == "id":
            ...
            shot_mask_tensor = (
                shot_indices.to(x.dtype) / (20) if num_shots > 1 else torch.zeros_like(shot_indices, dtype=x.dtype)
            )
    ...
    if dit.shot_embedding is not None and shot_indices is not None:
        assert shot_indices.shape[0] == x.shape[0], (
    ...
    if use_attn_mask:
        if text_cut_positions["global"] == None:
            ...
            try:
                ...
                num_img_tokens = (
                    clip_embdding.shape[1] if (clip_feature is not None and dit.require_clip_embedding) else 0
                )
