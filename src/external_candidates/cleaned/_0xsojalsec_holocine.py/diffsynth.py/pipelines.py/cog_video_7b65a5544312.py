# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\cog_video.py
import numpy as np
import torch
...
class CogVideoPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, prompt_refiner_classes=[]):
        pipe = CogVideoPipeline(device=model_manager.device, torch_dtype=model_manager.torch_dtype)
    ...
    def prepare_extra_input(self, latents):
        return {
            "image_rotary_emb": self.dit.prepare_rotary_positional_embeddings(
                latents.shape[3], latents.shape[4], latents.shape[2], device=self.device
            )
        }
    ...
    def __call__(
        ...
        noise = self.generate_noise(
            (1, 16, num_frames // 4 + 1, height // 8, width // 8),
            seed=seed,
            device="cpu",
        ...
        if denoising_strength == 1.0:
            ...
            latents = self.vae_encoder.encode_video(input_video, **tiler_kwargs, progress_bar=progress_bar_cmd).to(
                dtype=self.torch_dtype
            )
