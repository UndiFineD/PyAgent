# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\pipelines\hunyuan_video.py
import numpy as np
import torch
...
class HunyuanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        if input_images is not None:
            ...
            image_pixel_values, height, width = self.prepare_vae_images_inputs(
                input_images, i2v_resolution=i2v_resolution
            )
        ...
        self.load_models_to_device(
            ["text_encoder_1"]
            if self.vram_management and input_images is None
            else ["text_encoder_1", "text_encoder_2"]
        )
        ...
        tea_cache_kwargs = {
            "tea_cache": (
                TeaCache(num_inference_steps, rel_l1_thresh=tea_cache_l1_thresh)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if step_processor is not None:
                ...
                noise_pred = self.scheduler.return_to_timestep(
                    self.scheduler.timesteps[progress_id], latents, target_latents
                )
                self.load_models_to_device([] if self.vram_management else ["dit"])
...
def lets_dance_hunyuan_video(
    ...
    **kwargs,
    ...
    vec = (
        dit.time_in(t, dtype=torch.float32)
        + dit.vector_in(pooled_prompt_emb)
        + dit.guidance_in(guidance * 1000, dtype=torch.float32)
    )
    img = dit.img_in(x)
...
def lets_dance_hunyuan_video_i2v(
    ...
    **kwargs,
    ...
    vec = vec + vec_2
    vec = vec + dit.guidance_in(guidance * 1000.0, dtype=torch.bfloat16)
