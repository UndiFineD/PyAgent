...
class WanVideoPipeline(BasePipeline):
    ...
    def prepare_vace_kwargs(
        ...
        if vace_video is not None or vace_mask is not None or vace_reference_image is not None:
            ...
            inactive = self.encode_video(inactive, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(
                dtype=self.torch_dtype, device=self.device
            )
            reactive = self.encode_video(reactive, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(
                dtype=self.torch_dtype, device=self.device
            )
