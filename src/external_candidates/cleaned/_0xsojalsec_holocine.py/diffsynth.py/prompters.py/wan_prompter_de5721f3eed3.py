...
class WanPrompter(BasePrompter):
    ...
    def encode_prompt_separately(self, prompt, positive=True, device="cuda"):
        ...
        for part in prompt_parts:
            ...
            enc_output = self.tokenizer(text, return_mask=True, add_special_tokens=True, return_tensors="pt")
        ...
        if not embeddings_list:
            return torch.zeros(1, self.text_len, self.text_encoder.config.hidden_size, device=device), {
                "global": None,
                "shots": [],
            }
