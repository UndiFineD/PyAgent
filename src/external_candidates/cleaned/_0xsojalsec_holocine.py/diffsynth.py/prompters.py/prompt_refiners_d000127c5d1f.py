# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\prompters\prompt_refiners.py
import torch
    ...
    def __call__(self, raw_prompt, positive=True, **kwargs):
        if positive:
            ...
            prompt = (
                raw_prompt
                + ", "
                + self.tokenizer.batch_decode(outputs[:, input_ids.size(1) :], skip_special_tokens=True)[0].strip()
            )
...
class QwenPrompt(torch.nn.Module):
    ...
    def from_model_manager(model_nameger: ModelManager):
        ...
        qwen_prompt = QwenPrompt(tokenizer_path=model_path, model=model, system_prompt=system_prompt)
        return qwen_prompt
