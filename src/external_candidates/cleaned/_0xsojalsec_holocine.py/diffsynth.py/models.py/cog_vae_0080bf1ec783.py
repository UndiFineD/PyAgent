# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\cog_vae.py
import torch
...
class Resnet3DBlock(torch.nn.Module):
    ...
    def forward(self, hidden_states, zq):
        ...
        hidden_states = (
            self.norm1(hidden_states, zq)
            if isinstance(self.norm1, CogVideoXSpatialNorm3D)
        ...
        hidden_states = (
            self.norm2(hidden_states, zq)
            if isinstance(self.norm2, CogVideoXSpatialNorm3D)
    ...
    def decode_video(
        ...
        if tiled:
            ...
            return TileWorker2Dto3D().tiled_forward(
                ...
                computation_device=sample.device,
                computation_dtype=sample.dtype,
    ...
    def encode_video(
        ...
        if tiled:
            ...
            return TileWorker2Dto3D().tiled_forward(
                ...
                model_input=sample,
                tile_size=(i * 8 for i in tile_size),
                tile_stride=(i * 8 for i in tile_stride),
                tile_device=sample.device,
                tile_dtype=sample.dtype,
                computation_device=sample.device,
                computation_dtype=sample.dtype,
