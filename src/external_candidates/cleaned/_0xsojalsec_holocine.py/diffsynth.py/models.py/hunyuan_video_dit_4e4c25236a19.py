# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\hunyuan_video_dit.py
...
from .sd3_dit import RMSNorm, TimestepEmbeddings
    ...
    def forward(
        ...
        if token_replace_vec is not None:
            ...
            (
                ...
                tr_mod2_scale,
                tr_mod2_gate,
            ) = self.mod(token_replace_vec).chunk(6, dim=-1)
        else:
            (
                tr_mod1_shift,
                tr_mod1_scale,
        ...
        norm_hidden_states = modulate(
            ...
            shift=mod1_shift,
            scale=mod1_scale,
            tr_shift=tr_mod1_shift,
            tr_scale=tr_mod1_scale,
            tr_token=tr_token,
        )
        ...
        return (
            (q, k, v),
            (mod1_gate, mod2_shift, mod2_scale, mod2_gate),
            (tr_mod1_gate, tr_mod2_shift, tr_mod2_scale, tr_mod2_gate),
        )
    ...
    def process_ff(self, hidden_states, attn_output, mod, mod_tr=None, tr_token=None):
        ...
        x = self.ff(
            modulate(
                ...
                shift=mod2_shift,
                scale=mod2_scale,
                tr_shift=tr_mod2_shift,
                tr_scale=tr_mod2_scale,
                tr_token=tr_token,
            )
        )
...
class MMDoubleStreamBlock(torch.nn.Module):
    ...
    def forward(
        ...
        (q_a, k_a, v_a), mod_a, mod_tr = self.component_a(
            hidden_states_a, conditioning, freqs_cis, token_replace_vec, tr_token
        )
        (q_b, k_b, v_b), mod_b, _ = self.component_b(hidden_states_b, conditioning, freqs_cis=None)

        q_a, q_b = (
            torch.concat([q_a, q_b[:, :split_token]], dim=1),
            q_b[:, split_token:].contiguous(),
        )
        k_a, k_b = (
            torch.concat([k_a, k_b[:, :split_token]], dim=1),
            k_b[:, split_token:].contiguous(),
        )
        v_a, v_b = (
            torch.concat([v_a, v_b[:, :split_token]], dim=1),
            v_b[:, split_token:].contiguous(),
        )
        attn_output_a = attention(q_a, k_a, v_a)
        attn_output_b = attention(q_b, k_b, v_b)
        attn_output_a, attn_output_b = (
            attn_output_a[:, :-split_token].contiguous(),
            torch.concat([attn_output_a[:, -split_token:], attn_output_b], dim=1),
        )
            ...
            class Linear(torch.nn.Linear):
                ...
                def block_forward_(self, x, i, j, dtype, device):
                    ...
                    if self.bias is None or i > 0:
                        ...
                        bias_ = cast_to(
                            self.bias[j * self.block_size : (j + 1) * self.block_size],
                            dtype=dtype,
                            device=device,
                        )
                ...
                def block_forward(self, x, **kwargs):
                    ...
                    for i in range((self.in_features + self.block_size - 1) // self.block_size):
                        for j in range((self.out_features + self.block_size - 1) // self.block_size):
                            y[..., j * self.block_size : (j + 1) * self.block_size] += self.block_forward_(
                                x, i, j, dtype=x.dtype, device=x.device
                            )
                    return y
        ...
        def replace_layer(model, dtype=torch.bfloat16, device="cuda"):
            for name, module in model.named_children():
                if isinstance(module, torch.nn.Linear):
                    ...
                    new_layer = quantized_layer.RMSNorm(module, dtype=dtype, device=device)
                    setattr(model, name, new_layer)
