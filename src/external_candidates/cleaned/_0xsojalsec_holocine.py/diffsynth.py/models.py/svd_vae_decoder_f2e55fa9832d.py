# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HoloCine\diffsynth\models\svd_vae_decoder.py
import torch
    ...
    def decode_video(
        ...
        for t in range(0, T, stride_time):
            for h in range(0, H, stride_height):
                for w in range(0, W, stride_width):
                    if (
                        (t - stride_time >= 0 and t - stride_time + batch_time >= T)
        ...
        for tl, tr, hl, hr, wl, wr in progress_bar(tasks):
            ...
            mask = self.build_mask(
                sample_batch,
                is_bound=(tl == 0, tr >= T, hl == 0, hr >= H, wl == 0, wr >= W),
            )
