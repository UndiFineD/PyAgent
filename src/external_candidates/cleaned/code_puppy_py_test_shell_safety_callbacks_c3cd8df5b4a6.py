# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\plugins.py\test_shell_safety_callbacks_c3cd8df5b4a6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_shell_safety_callbacks.py
...
class TestShellSafetyCallbackOAuthBypass:
    ...
    async def test_callback_skips_for_oauth_model_anthropic(self):
        ...
        with patch(
            ...
            result = await shell_safety_callback(context=None, command="rm -rf /", cwd=None, timeout=60)
...
class TestShellSafetyCallbackCacheHit:
    ...
    async def test_cached_assessment_blocked_high_risk(self):
        ...
        with (
            ...
            patch("code_puppy.plugins.shell_safety.register_callbacks.emit_info") as mock_emit,
        ):
            result = await shell_safety_callback(context=None, command="rm -rf /", cwd=None, timeout=60)
    ...
    async def test_cached_assessment_allowed_low_risk(self):
        ...
        with (
            ...
            result = await shell_safety_callback(context=None, command="ls -la", cwd=None, timeout=60)
    ...
    async def test_cached_assessment_at_threshold_allowed(self):
        ...
        with (
            ...
            result = await shell_safety_callback(context=None, command="npm install", cwd=None, timeout=60)
    ...
    async def test_cached_assessment_with_none_risk(self):
        ...
        with (
            ...
            result = await shell_safety_callback(context=None, command="unknown", cwd=None, timeout=60)
...
class TestShellSafetyCallbackCacheMiss:
    ...
    async def test_llm_assessment_blocked_high_risk(self):
        ...
        mock_assessment = ShellSafetyAssessment(risk="critical", reasoning="Deletes entire filesystem")
        ...
        with (
            ...
            patch("code_puppy.plugins.shell_safety.register_callbacks.cache_assessment") as mock_cache,
            patch.dict(
                "sys.modules",
                {"code_puppy.plugins.shell_safety.agent_shell_safety": MagicMock(ShellSafetyAgent=mock_agent_class)},
            ),
            patch("code_puppy.plugins.shell_safety.register_callbacks.emit_info") as mock_emit,
        ):
            result = await shell_safety_callback(context=None, command="rm -rf /", cwd=None, timeout=60)
            ...
            mock_cache.assert_called_once_with("rm -rf /", None, "critical", "Deletes entire filesystem")
    ...
    async def test_llm_assessment_allowed_low_risk(self):
        ...
        mock_assessment = ShellSafetyAssessment(risk="low", reasoning="Lists directory contents")
        ...
        with (
            ...
            mock_cache.assert_called_once_with("ls -la", None, "low", "Lists directory contents")

    @pytest.mark.anyio
    async def test_llm_assessment_with_cwd_in_prompt(self):
        """Test LLM assessment includes cwd in prompt."""

        mock_assessment = ShellSafetyAssessment(risk="low", reasoning="Safe in temp directory")
        ...
        with (
            ...
            await shell_safety_callback(context=None, command="rm -rf *", cwd="/tmp/safe", timeout=60)
            ...
            mock_cache.assert_called_once_with("rm -rf *", "/tmp/safe", "low", "Safe in temp directory")

    @pytest.mark.anyio
    async def test_fallback_assessment_not_cached(self):
        """Test fallback assessment is not cached."""

        mock_assessment = ShellSafetyAssessment(risk="high", reasoning="Fallback assessment")
        ...
        with (
            ...
            result = await shell_safety_callback(context=None, command="dangerous", cwd=None, timeout=60)
...
class TestShellSafetyCallbackExceptionHandling:
    ...
    async def test_exception_blocks_with_high_risk(self):
        ...
        mock_agent_instance.run_with_mcp = AsyncMock(side_effect=Exception("LLM connection failed"))
        ...
        with (
            ...
            result = await shell_safety_callback(context=None, command="some command", cwd=None, timeout=60)
    ...
    async def test_cache_exception_blocks_command(self):
        ...
        with (
            ...
            result = await shell_safety_callback(context=None, command="ls", cwd=None, timeout=60)
...
class TestRegisterCallback:
    ...
    def test_register_calls_register_callback(self):
        ...
        with patch("code_puppy.plugins.shell_safety.register_callbacks.register_callback") as mock_register:
            register()

            mock_register.assert_called_once_with("run_shell_command", shell_safety_callback)
