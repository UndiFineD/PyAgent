# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\python_configs.py\base_2f641d14c977.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\python_configs\base.py
...
class DefaultConfig:
    ...
    def _get_possible_values(
        ...
        if isinstance(poss_values, possible_values.DatasetValue):
            ...
            poss_values, value = poss_values.get_value(dataset=dataset, value=value, type_annotation=type_annotation)

        elif isinstance(poss_values, Sequence):
            if all(isinstance(x, (float, int)) for x in poss_values):
                poss_values = possible_values.Number(min=poss_values[0], max=poss_values[1], step=poss_values[2])

            elif all(isinstance(x, str) for x in poss_values):
                poss_values = possible_values.String(tuple(poss_values))

            else:
                raise ValueError(f"Could not interpret {poss_values} as any possible value class.")
    ...
    def _get_order(self, warn_if_unset=True) -> list[str]:
        ...
        unordered_ui_keys = [
            key for key in unordered_keys if not (key.startswith("_") or self._get_visibility(key) == -1)
        ]
    ...
    def from_dict(cls, d: dict):
        ...
        if len(d) != len(d_filtered):
            logger.warning(f"Keys {set(d.keys()) - set(d_filtered.keys())} are not in the config.")
