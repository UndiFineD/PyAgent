# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pi0-lerobot\src\pi0_lerobot\custom_types.py
...
class CustomPoints2D(rr.AsComponents):
    ...
    def as_component_batches(self) -> list[rr.DescribedComponentBatch]:
        return list(self.points2d.as_component_batches()) + [  # The components from Points2D
            self.confidences
...
class CustomPoints3D(rr.AsComponents):
    ...
    def as_component_batches(self) -> list[rr.DescribedComponentBatch]:
        return list(self.points3d.as_component_batches()) + [  # The components from Points3D
            self.confidences
        ]  # Custom confidence data
