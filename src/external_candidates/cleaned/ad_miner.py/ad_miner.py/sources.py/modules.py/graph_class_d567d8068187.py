# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\graph_class.py
...
class Graph:
    ...
    def render(self, page_f):
        # Write header
        with open(self.template_base_path / (self.template + "_header.html"), "r") as header_f:
        ...
        for index, path in enumerate(self.paths):
            for i in range(len(path.nodes)):
                ...
                if self.ghost_computer.get(path.nodes[i].name) or self.ghost_user.get(path.nodes[i].name):
                    attribute1 = "ghost"
                else:
                    attribute1 = "none"
                ...
                if not self.nodes.get(path.nodes[i].id):
                    ...
                    if node_position != self.nodes[path.nodes[i].id]["group"].split("_")[0]:
                        self.nodes[path.nodes[i].id]["group"] = (
                ...
                if i != 0:
                    ...
                    hash_rel = hash(str(relation["from"]) + str(relation["to"]) + str(relation["label"]))
