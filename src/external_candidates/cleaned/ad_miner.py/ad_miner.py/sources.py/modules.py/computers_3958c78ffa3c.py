# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\computers.py
import json
import re
import time
from urllib.parse import quote

from ad_miner.sources.modules import generic_computing, generic_formating, logger
...
class Computers:
    def __init__(self, arguments, neo4j, domain):
        ...
        self.list_computers_admin_computers = neo4j.all_requests["computers_admin_on_computers"]["result"]
        self.list_computers_os_obsolete = self.manageComputersOs(neo4j.all_requests["os"]["result"])
        # print(neo4j.all_requests['os']['result'][0])
        # print(self.list_computers_os_obsolete[0])
        self.list_computers_unconstrained_delegations = neo4j.all_requests["nb_computer_unconstrained_delegations"][
            "result"
        ]
        self.list_users_unconstrained_delegations = neo4j.all_requests["nb_users_unconstrained_delegations"]["result"]
        self.users_constrained_delegations = neo4j.all_requests["users_constrained_delegations"]["result"]
        ...
        if self.computer_administrable_per_users is not None and self.computer_administrable_per_users != {}:
        ...
        self.computers_members_high_privilege = neo4j.all_requests["computers_members_high_privilege"]["result"]
        ...
        self.computers_non_dc_unconstrained_delegations = neo4j.all_requests["nb_computer_unconstrained_delegations"][
            "result"
        ]
        self.users_non_dc_unconstrained_delegations = neo4j.all_requests["nb_users_unconstrained_delegations"]["result"]
        ...
        if self.users_constrained_delegations is not None:
            self.users_constrained_delegations = dict(
                sorted(
                    self.parseConstrainedData(self.users_constrained_delegations).items(),
        ...
        if self.list_computers_os_obsolete is not None:
            count_computers_os_obsolete_per_domain = generic_computing.getCountValueFromKey(
    ...
    def genUsersConstrainedPage(self):
        ...
        formated_data = generic_formating.formatGridValues3Columns(
            generic_formating.formatFor3Col(self.users_constrained_delegations, headers),
            headers,
    ...
    def genComputersAdminOfPages(self):
        ...
        for admin_computer, computers_list in computers_admin_to_list.items():
            if admin_computer is not None and computers_list is not None:
                num_path, nb_domains = self.domain.findAndCreatePathToDaFromComputersList(
                    admin_computer, computers_list
                )
        ...
        self.computers_admin_data_grid.sort(key=lambda x: x["Computers count"], reverse=True)
        ...
        for computer, values in computers_admin_to_list.items():
            if computer is not None:
                ...
                computers_admin_to_list = generic_formating.formatGridValues1Columns(values, grid.getHeaders())
    ...
    def genComputersAdministrablePage(self):
        ...
        for computer, values in self.computer_administrable_per_users_list.items():
            for v in values:
                allValues.append("<span class='administrable-by-%s'> %s </span>" % (computer, v))
        page = Page(
            self.arguments.cache_prefix,
            "computer_administrable",
        ...
        users_admin_to_list = generic_formating.formatGridValues1Columns(allValues, grid.getHeaders())
