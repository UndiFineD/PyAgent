# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\rating.py
import datetime
import json
import time
...
def rating(users, domains, computers, objects, arguments):
    ...
    d[presence_of(computers.list_computers_unconstrained_delegations, criticity=2)].append(
        "non-dc_with_unconstrained_delegations"
    )
    d[constrainedDelegation(computers.users_constrained_delegations)].append("users_constrained_delegations")
    d[hasPathToDA(computers.list_computers_admin_computers)].append("computers_admin_of_computers")
    ...
    ].append("users_pwd_not_changed_since")  # TODO CHANGE DESCRIPTION = 3MONTHS NOT 1Y
    d[containsDAs(users.users_pwd_cleartext)].append("users_pwd_cleartext")
    d[
        min(
            hasPathToDA(users.users_admin_computer),
            percentage_superior(users.users_admin_computer, users.users, criticity=2, percentage=0.5),
        )
    ].append("users_admin_of_computers")
    d[hasPathToDA(users.users_admin_on_servers_all_data)].append("server_users_could_be_admin")
    d[presence_of(computers.computers_members_high_privilege_uniq)].append("computers_members_high_privilege")
    ...
    ].append("computers_last_connexion")  # TODO: percentage TBD

    # RDP access | TODO: percentage TBD
    d[percentage_superior(users.users_rdp_access_1, users.users, criticity=3, percentage=0.5)].append(
        "users_rdp_access"
    )
    d[percentage_superior(users.users_rdp_access_2, users.users, criticity=3, percentage=0.5)].append(
    ...
    ].append("dormants_accounts")  # TODO: percentage TBD
    ...
    d[presence_of(domains.objects_to_domain_admin, criticity=1, threshold=1)].append("graph_path_objects_to_da")
    ...
    if rate_vuln_functional_level(domains.vuln_functional_level) is not None:
        d[rate_vuln_functional_level(domains.vuln_functional_level)].append("vuln_functional_level")

    d[presence_of(users.vuln_permissions_adminsdholder, criticity=1)].append("vuln_permissions_adminsdholder")
    ...
    d[presence_of(domains.objects_to_ou_handlers)].append("graph_path_objects_to_ou_handlers")
    d[
        min(
    ...
    d[1 if domains.total_dangerous_paths > 0 else 5].append("dangerous_paths")
    ...
    d[
        (
            ...
            if len(domains.empty_groups) / len(domains.groups) > 0.20
            else 5
        )
    ].append("empty_groups")
    d[
        (
            ...
            if len(domains.empty_ous) / len(domains.groups) > 0.20
            else 5
        )
    ].append("empty_ous")
    ...
    (
        d[rate_admincount(users.unpriviledged_users_with_admincount, users.users_nb_domain_admins)].append(
            "up_to_date_admincount"
        ),
    )
    (
        d[
            presence_of([dic for dic in users.users_nb_domain_admins if "Protected Users" not in dic["admin type"]])
    ...
    d[rate_pre_windows_2000(users.pre_windows_2000_compatible_access_group)].append(
