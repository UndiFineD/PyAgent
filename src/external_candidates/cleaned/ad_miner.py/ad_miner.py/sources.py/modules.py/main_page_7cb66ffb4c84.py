# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\main_page.py
import json
import os
...
def getData(arguments, neo4j, domains, computers, users, objects, extract_date):
    ...
    data["computers_per_domain"] = [k[2] for k in domains.getUserComputersCountPerDomain()]
...
def complete_data_evolution_time(data, raw_other_list_data):
    ...
    if raw_other_list_data != None:
        ...
        for k in range(len(raw_other_list_data)):
            ...
            data["label_evolution_time"].append(f"{date_k[-4:]}-{date_k[3:5]}-{date_k[:2]}")
            ...
            for key in dico_data_evolution_time:
                if key in [
                    ...
                    dico_data_evolution_time[key].append(raw_other_list_data[k]["general_statistic"][key])
                else:
                    if raw_other_list_data[k]["value"].get(key) != None:
                        dico_data_evolution_time[key].append(raw_other_list_data[k]["value"][key])
...
def create_dico_data(data, arguments, domains, computers, users, objects, dico_rating_color):
    """
    ...
    dico_data["value"] = {
        ...
        "users_pwd_not_changed_since": (
            len(domains.users_pwd_not_changed_since_3months) if domains.users_pwd_not_changed_since_3months else 0
        ),
        ...
        "non-dc_with_unconstrained_delegations": (
            ...
            if computers.computers_non_dc_unconstrained_delegations
            else 0
        ),
        "users_constrained_delegations": (
            len(computers.users_constrained_delegations) if computers.users_constrained_delegations else 0
        ),
        "krb_last_change": max(
        ...
        "users_shadow_credentials": (
            len(users.users_shadow_credentials_uniq) if users.users_shadow_credentials_uniq else 0
        ),
        ...
        "computers_members_high_privilege": (
            ...
            if computers.computers_members_high_privilege_uniq
            else 0
        ),
        ...
        "graph_path_objects_to_da": (
            len(list(set([item for sublist in domains.users_to_domain_admin.values() for item in sublist])))
            if domains.users_to_domain_admin
            else 0
        ),
        "computers_last_connexion": (
            len(domains.computers_not_connected_since_60) if domains.computers_not_connected_since_60 else 0
        ),
        ...
        "objects_to_operators_member": (
            len(users.objects_to_operators_member) if len(users.objects_to_operators_member) else 0
        ),
        "computers_os_obsolete": (
            len(computers.list_computers_os_obsolete) if computers.list_computers_os_obsolete else 0
        ),
        ...
        "vuln_permissions_adminsdholder": (
            len(users.vuln_permissions_adminsdholder) if users.vuln_permissions_adminsdholder else 0
        ),
        ...
        "can_read_gmsapassword_of_adm": (
            len(users.can_read_gmsapassword_of_adm) if users.can_read_gmsapassword_of_adm else 0
        ),
        ...
        "priviledge_users_without_admincount": len(
            [dic for dic in users.users_nb_domain_admins if not dic["admincount"]]
        ),
        "privileged_accounts_outside_Protected_Users": len(
            [dic for dic in users.users_nb_domain_admins if "Protected Users" not in dic["admin type"]]
        ),
...
def render(arguments, neo4j, domains, computers, users, objects, data_rating, extract_date):
    ...
    dico_data = create_dico_data(data, arguments, domains, computers, users, objects, dico_rating_color)
    ...
    data = complete_data_evolution_time(data, raw_other_list_data)  # for the chart evolution over time
    ...
    for risk_control in global_risk_controls:
        ...
        for category in categories:
            if categories[category] > 0 and f"{category}_letter_grade" not in data:
                ...
                data[f"{category}_graph_summary"] = f"""
        ...
        if global_risk_controls[risk_control]["panel_key"]:
            ...
            for issue in data[f"{risk_control}_list"]:
                ...
                data[global_risk_controls[risk_control]["panel_key"]] += f"""
                    <a href="{issue}.html">
        ...
        for category in dico_category:
            for indicator in dico_category[category]:
                ...
                dico_js[indicator] = {
                    ...
                    "category": category,
                    "position": dico_position[category][dico_position_instance[category]],
                    "title": (
