...
class Neo4j:
        ...
        try:
            ...
            for request_key in self.all_requests.keys():
                ...
                for variable in variables_to_replace.keys():
                    self.all_requests[request_key]["request"] = self.all_requests[request_key]["request"].replace(
    ...
    @staticmethod
    def executeParallelRequest(value, identifier, query, arguments, output_type, server):
    ...
    def process_request(self, request_key):
        if self.cache_enabled:  # If cache enable, try to retrieve from cache
            ...
            if result is not False:  # Sometimes result = []
                logger.print_debug(
                    "From cache : %s - %d objects" % (self.all_requests[request_key]["name"], len(result))
                )
            ...
            if path is not None:
                ...
                for relation in path.relationships:
                    for node in relation.nodes:
                        label = next(iter(node.labels.difference({"Base"})))  # e.g. : {"User","Base"} -> "User"
