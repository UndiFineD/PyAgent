# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\users.py
import json
import time
...
class Users:
    def __init__(self, arguments, neo4j, domain):
        ...
        self.users_admin_computer = neo4j.all_requests["users_admin_on_computers"]["result"]
        self.users_admin_computer_count = generic_computing.getCountValueFromKey(self.users_admin_computer, "user")

        self.users_admin_computer_list = generic_computing.getListAdminTo(self.users_admin_computer, "user", "computer")

        self.users_kerberos_as_rep = neo4j.all_requests["nb_as-rep_roastable_accounts"]["result"]

        self.users_password_never_expires = neo4j.all_requests["user_password_never_expires"]["result"]
        self.users_kerberoastable_users = neo4j.all_requests["nb_kerberoastable_accounts"]["result"]
        self.users_krb_pwd_last_set = neo4j.all_requests["krb_pwd_last_change"]["result"]
        self.users_domain_admin_on_nondc = neo4j.all_requests["dom_admin_on_non_dc"]["result"]

        self.users_pwd_cleartext = neo4j.all_requests["nb_user_password_cleartext"]["result"]

        self.objects_admincount_enabled = neo4j.all_requests["objects_admincount"]["result"]

        self.users_shadow_credentials = neo4j.all_requests["users_shadow_credentials"]["result"]
        self.users_shadow_credentials_uniq = []

        self.users_shadow_credentials_to_non_admins = neo4j.all_requests["users_shadow_credentials_to_non_admins"][
            "result"
        ]
        ...
        self.users_rbcd_attacks = self.removeAlreadyAdmins(neo4j.all_requests["rbcd"]["result"])
        ...
        rbcd_attacks_infos = self.formatRBCD(self.users_rbcd_attacks) if self.users_rbcd_attacks is not None else None
        self.users_rbcd_attacks_data = rbcd_attacks_infos[0] if self.users_rbcd_attacks is not None else None
        self.nb_users_rbcd_attacks = rbcd_attacks_infos[1] if self.users_rbcd_attacks is not None else None
        self.nb_computers_rbcd_attacks = rbcd_attacks_infos[2] if self.users_rbcd_attacks is not None else None
        if self.users_rbcd_attacks_data is not None:
            self.users_rbcd_attacks_data.sort(key=lambda x: x["Number of users"], reverse=True)
        self.can_read_laps = neo4j.all_requests["can_read_laps"]["result"]

        self.get_users_linked_admin_group = neo4j.all_requests["get_users_linked_admin_group"]["result"]
        self.get_groups_linked_admin_group = neo4j.all_requests["get_groups_linked_admin_group"]["result"]
        self.get_computers_linked_admin_group = neo4j.all_requests["get_computers_linked_admin_group"]["result"]
        self.get_users_direct_admin = neo4j.all_requests["get_users_direct_admin"]["result"]
        ...
        if self.users_admin_on_servers is not None and self.users_admin_on_servers != {}:
            self.servers_with_most_paths = self.users_admin_on_servers[list(self.users_admin_on_servers.keys())[0]]
        else:
            self.servers_with_most_paths = []

        self.unpriv_to_dnsadmins = neo4j.all_requests["unpriv_to_dnsadmins"]["result"]
        self.vuln_sidhistory_dangerous = neo4j.all_requests["vuln_sidhistory_dangerous"]["result"]
        self.can_read_gmsapassword_of_adm = neo4j.all_requests["can_read_gmsapassword_of_adm"]["result"]
        self.objects_to_operators_member = neo4j.all_requests["objects_to_operators_member"]["result"]
        ...
        self.vuln_permissions_adminsdholder = neo4j.all_requests["vuln_permissions_adminsdholder"]["result"]
        ...
        self.pre_windows_2000_compatible_access_group = neo4j.all_requests["pre_windows_2000_compatible_access_group"][
            "result"
        ]
    ...
    def genServersCompromisablePage(self):
        ...
        for computer, values in self.users_admin_on_servers_list.items():
            for v in values:
                allValues.append("<span class='compromisable-by-%s'> %s </span>" % (computer, v))
        page = Page(
            self.arguments.cache_prefix,
            "server_compromisable",
        ...
        users_admin_of_servers = generic_formating.formatGridValues1Columns(allValues, grid.getHeaders())
    ...
    def genUsersAdminOfComputersPage(self, domain):
        ...
        def get_last_pass_change(account, domain):
            for elem in domain.users_pwd_not_changed_since:
                if elem["user"] == account:
                    sortClass = str(elem["days"]).zfill(6)  # used to make the sorting feature work with icons
        ...
        for key in self.users_admin_computer_list.keys():
            ...
            nb_path_to_da, nb_domain = self.domain.findAndCreatePathToDaFromUsersList(key, partDict[headers[3]])
            if nb_path_to_da > 0:
        ...
        for key in self.users_to_computer_admin.keys():
            if key not in self.users_admin_computer_list.keys():
                ...
                (
                    nb_path_to_da,
                    nb_domain,
                ) = self.domain.findAndCreatePathToDaFromUsersList(key, partDict[headers[3]])
                if nb_path_to_da > 0:
                    sortClass = str(nb_path_to_da).zfill(6)
                    partDict[headers[5]] = grid_data_stringify(
                        {
        ...
        for dict in tmp_rslt:
            ...
            if dict[headers[0]] in domain.admin_list:
                formated_data.append(
                    {
                        headers[0]: '<i class="bi bi-gem" title="This user is domain admin"></i> ' + dict[headers[0]],
    ...
    def genKerberoastablesUsersPage(self):
        ...
        grid.setheaders(["domain", "name", "Last password change", "Account Creation Date", "SPN"])
        ...
        for elem in range(len(self.users_kerberoastable_users)):
            if self.users_kerberoastable_users[elem]["is_Domain_Admin"] == True:
                ...
                self.users_kerberoastable_users[elem]["name"] = (
                    '<i class="bi bi-person-fill"></i> ' + self.users_kerberoastable_users[elem]["name"]
                )
    ...
    @staticmethod
    def createGraphPage(render_prefix, page_name, page_title, page_description, graph_data, domain):
    ...
    def generatePathToOperatorsMember(self, domain):
        ...
        for d in data.values():
            ...
            page_graph = Page(
                self.arguments.cache_prefix,
                f"objects_to_operators_member_from_{d['link']}",
                f"{d['link']} can have a path to an Operator member",
                "objects_to_operators_member",
            )
            graph = Graph()
            graph.setPaths(d["paths"])
    ...
    def genRBCDGraph(self, domain):
        ...
        for path in self.rbcd_paths:
            ...
            if ending_node_name not in self.rbcd_graphs[starting_node_name]["destinations"]:
                self.rbcd_graphs[starting_node_name]["destinations"].append(ending_node_name)
        ...
        for object_name in list(self.rbcd_graphs.keys()):
            ...
            for destination in self.rbcd_graphs[object_name]["destinations"]:
                ...
                if destination in list(self.rbcd_to_da_graphs.keys()):
                    self.rbcd_graphs[object_name]["nb_paths_to_da"] += len(self.rbcd_to_da_graphs[destination]["paths"])
    ...
    def genGroupAnomalyAcl(self, domain):
        ...
        for k in range(len(self.anomaly_acl)):
            ...
            if (
                ...
                and formated_data[name_label_instance]["type"] == self.anomaly_acl[k]["type(r2)"]
                and formated_data[name_label_instance]["label"] == self.anomaly_acl[k]["LABELS(g)[0]"]
            ):
                formated_data[name_label_instance]["targets"].append(self.anomaly_acl[k]["n.name"])
            elif (
                formated_data.get(name_label_instance)
                and formated_data[name_label_instance]["targets"] == [self.anomaly_acl[k]["n.name"]]
                and self.anomaly_acl[k]["type(r2)"] not in formated_data[name_label_instance]["type"]
                and formated_data[name_label_instance]["label"] == self.anomaly_acl[k]["LABELS(g)[0]"]
            ):
                ...
                for u in self.users_admin_of_computers:
                    ...
                    if k == name_user:
                        if "No data to show" not in u["List of computers"]:
                            count = int(
                                u["List of computers"][
                                    u["List of computers"].find("'>", 55) + 2 : u["List of computers"].find("Computer")
                                ].strip()
                            )
            ...
            anomaly_acl_extract.append(
                {
                    ...
                    "interest": f"<span class='{interest}'></span><i class='bi bi-star-fill'></i>" * interest
                    + "<i class='bi bi-star'></i>" * (3 - interest),
                }
            )

        page = Page(self.arguments.cache_prefix, "anomaly_acl", "ACL Anomaly", "anomaly_acl")
    ...
    def genHasSIDHistory(self):
        ...
        for row in self.has_sid_history:
            ...
            if row["Type_a"] == "User":
                ...
                row["Has SID History"] = (
                    "<i class='bi bi-question-circle-fill' title='Unknown'></i> " + row["Has SID History"]
                )
            if row["Type_b"] == "User":
        ...
        for domain, account_name, objectid, type in sorted_list:
            ...
            tmp_data["Name"] = (
                ...
                if "User" in type
                    ...
                    '<i class="bi bi-pc-display"></i> ' + account_name
                    if "Computer" in type
