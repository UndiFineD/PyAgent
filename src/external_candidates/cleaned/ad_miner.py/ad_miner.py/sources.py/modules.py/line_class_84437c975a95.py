# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\line_class.py
import re
...
class Line:
    def __init__(self, template="card", text=None, icon=None, href=None, color="secondary"):
    ...
    def decorateTextNumbers(text):
        ...
        for num in num_to_decorate:
            working_text = working_text.replace(num, '<span class="number">%s</span>' % (num))
        return working_text

    def render(self, page_f):
        with open(self.template_base_path / (self.template + "_line.html"), "r") as line_f:
