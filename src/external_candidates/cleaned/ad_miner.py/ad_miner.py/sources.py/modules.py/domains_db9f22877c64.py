# Extracted from: C:\DEV\PyAgent\.external\AD_Miner\ad_miner\sources\modules\domains.py
import copy
import time
from urllib.parse import quote

from ad_miner.sources.modules import generic_formating, logger
...
class Domains:
    ...
    def __init__(self, arguments, neo4j):
        ...
        self.objects_to_domain_admin = neo4j.all_requests["objects_to_domain_admin"]["result"]
        self.objects_to_unconstrained_delegation = neo4j.all_requests["objects_to_unconstrained_delegation"]["result"]
        self.objects_to_unconstrained_delegation_2 = neo4j.all_requests["users_to_unconstrained_delegation"]["result"]
        ...
        self.computers_with_last_connection_date = neo4j.all_requests["computers_not_connected_since"]["result"]
        ...
        self.computers_nb_domain_controllers = neo4j.all_requests["nb_domain_controllers"]["result"]
        ...
        self.users_pwd_not_changed_since = neo4j.all_requests["password_last_change"]["result"]
        self.users_pwd_not_changed_since_3months = (
            [user for user in self.users_pwd_not_changed_since if user["days"] > neo4j.password_renewal]
        ...
        if not arguments.gpo_low:
            self.unpriv_users_to_GPO_init = neo4j.all_requests["unpriv_users_to_GPO_init"]["result"]
            self.unpriv_users_to_GPO_user_enforced = neo4j.all_requests["unpriv_users_to_GPO_user_enforced"]["result"]
            self.unpriv_users_to_GPO_computer_enforced = neo4j.all_requests[
                "unpriv_users_to_GPO_computer_not_enforced"
            ]["result"]
            self.unpriv_users_to_GPO_user_not_enforced = neo4j.all_requests["unpriv_users_to_GPO_user_not_enforced"][
                "result"
            ]
            ...
            self.unpriv_users_to_GPO = neo4j.all_requests["unpriv_users_to_GPO"]["result"]
        self.domain_OUs = neo4j.all_requests["domain_OUs"]["result"]
        self.objects_to_ou_handlers = neo4j.all_requests["objects_to_ou_handlers"]["result"]
        self.nb_starting_nodes_to_ous = 0
        self.nb_ous_with_da = 0

        self.vuln_functional_level = neo4j.all_requests["vuln_functional_level"]["result"]
        ...
        def analyse_cache(cache):
            ...
            for path in cache:
                for i in range(1, len(path.nodes) - 2):
                    node_rel_node_instance = (
                        f"{path.nodes[i].name} ⮕ {path.nodes[i].relation_type} ⮕ {path.nodes[i + 1].name}"
                    )
    ...
    def genAllGroupsPage(self):
        if self.groups is None:
            return
        page = Page(self.arguments.cache_prefix, "groups", "List of all groups", "groups")
        grid = Grid("Groups")
        ...
        group_extract = [
            {
                ...
                "name": (
                    '<i class="bi bi-gem" title="This group is domain admin"></i> ' + self.groups[k]["name"]
    ...
    def genDormantsUsersPage(self):
        ...
        for dict in self.users_dormant_accounts:
            ...
            tmp_data["name"] = (
                ('<i class="bi bi-gem" title="This user is domain admin"></i> ' + dict["name"])
    ...
    def getUsersUnusedSince(self):
        ...
        result.append(
            [
                "Never",
                len([user for user in self.users_dormant_accounts if user["days"] > dict_since["never"]]),
            ]
        )
        return result
        """
...
#			print(self.user_dormant_accounts)
    ...
    def createGraphPage(self, render_prefix, page_name, page_title, page_description, graph_data):
    ...
    def generatePathToDcsync(self):
        ...
        for domain in self.domains:
            ...
            if len(self.users_to_dcsync[domain]) != 0:
                ...
                tmp_data["Users"] = "<i class='bi bi-person-fill %s' aria-hidden='true'></i> %s" % (
                    str(len(self.users_to_dcsync[domain])).zfill(6),
                    len(self.users_to_dcsync[domain]),
                )
            if len(self.computers_to_dcsync[domain]) != 0:
                ...
                tmp_data["Computers"] = "<i class='bi bi-pc-display-horizontal %s' aria-hidden='true'></i> %s" % (
                    str(len(self.computers_to_dcsync[domain])).zfill(6),
                    len(self.computers_to_dcsync[domain]),
                )
            if len(self.groups_to_dcsync[domain]) != 0:
                ...
                tmp_data["Groups"] = "<i class='bi bi-people-fill %s' aria-hidden='true'></i> %s" % (
                    str(len(self.groups_to_dcsync[domain])).zfill(6),
                    len(self.groups_to_dcsync[domain]),
                )
            if len(self.ou_to_dcsync[domain]) != 0:
                ...
                tmp_data["Ou"] = "<i class='bi bi-building %s' aria-hidden='true'></i> %s" % (
                    str(len(self.ou_to_dcsync[domain])).zfill(6),
                    len(self.ou_to_dcsync[domain]),
                )
            if len(self.gpo_to_dcsync[domain]) != 0:
                ...
                tmp_data["GPO"] = "<i class='bi bi-journal-text %s' aria-hidden='true'></i> %s" % (
                    str(len(self.gpo_to_dcsync[domain])).zfill(6),
                    len(self.gpo_to_dcsync[domain]),
                )
            grid_data.append(tmp_data)
    ...
    def generatePathToUnconstrainedDelegation(self):
        ...
        for path in self.objects_to_unconstrained_delegation:
            if "User" in path.nodes[0].labels:
                self.users_to_unconstrained_delegation[path.nodes[-1].domain].append(path)
            elif "Computer" in path.nodes[0].labels:
                self.computers_to_unconstrained_delegation[path.nodes[-1].domain].append(path)
            elif "Group" in path.nodes[0].labels:
                self.groups_to_unconstrained_delegation[path.nodes[-1].domain].append(path)
            elif "OU" in path.nodes[0].labels:
                self.ou_to_unconstrained_delegation[path.nodes[-1].domain].append(path)
            elif "GPO" in path.nodes[0].labels:
                self.gpo_to_unconstrained_delegation[path.nodes[-1].domain].append(path)
    ...
    def generatePathToUnconstrainedDelegation_2(self):
        ...
        for path in self.objects_to_unconstrained_delegation_2:
            if "User" in path.nodes[0].labels:
                self.users_to_unconstrained_delegation_2[path.nodes[-1].domain].append(path)
            elif "Computer" in path.nodes[0].labels:
                self.computers_to_unconstrained_delegation_2[path.nodes[-1].domain].append(path)
            elif "Group" in path.nodes[0].labels:
                self.groups_to_unconstrained_delegation_2[path.nodes[-1].domain].append(path)
            elif "OU" in path.nodes[0].labels:
                self.ou_to_unconstrained_delegation_2[path.nodes[-1].domain].append(path)
            elif "GPO" in path.nodes[0].labels:
                self.gpo_to_unconstrained_delegation_2[path.nodes[-1].domain].append(path)
    ...
    def getUserComputersCountPerDomain(self):
        ...
        for domain in self.domains:
            ...
            nb_computer = len([element for element in computers if element["domain"] == domain])
    ...
    def generatePathToOUHandlers(self):
        if self.objects_to_ou_handlers is None:
            return
        logger.print_debug("Generate paths to objects that can GPLink GPOs on OUs")
        ...
        for path in self.objects_to_ou_handlers:
            ...
            if (
                ...
                and path.nodes[0].name not in starting_nodes_by_admin[path.nodes[-1].domain]
            ):
                starting_nodes_by_admin[path.nodes[-1].domain].append(path.nodes[0].name)
                self.nb_starting_nodes_to_ous += 1
            elif path.nodes[0].labels == "OU" and path.nodes[0].name not in ous_with_da_by_admin[path.nodes[-1].domain]:
                ous_with_da_by_admin[path.nodes[-1].domain].append(path.nodes[0].name)
    ...
    def get_domain_OUs(self):
        ...
        def formatOU(listOfDict, headers):
            ...
            for dict in listOfDict:
                ...
                if nameOfOU not in nameList:
                    ...
                    dictData[nameOfOU] += 1
                allValues.append("<span class='corresponding-OU-%s'> %s </span>" % (nameOfOU, dict["name"]))
        ...
        grid.setData(
            sorted(
                generic_formating.formatGridValues2Columns(dictOfOUs, headers, "OU_member"),
        ...
        # Secondary grid
        page = Page(self.arguments.cache_prefix, "OU_member", "Member of OU", "OU_member")
        grid = Grid("Member of OU")
        grid.addheader("TO CHANGE")
        OU_members = generic_formating.formatGridValues1Columns(allValues, grid.getHeaders())
    ...
    def genInsufficientForestDomainsLevels(self):
        ...
        for dico in self.vuln_functional_level:
            ...
            if d["Level maturity"] is None:
                ...
                color = "green"
            d["Level maturity"] = f'<i class="bi bi-star-fill" style="color: {color}"></i>' * d[
                "Level maturity"
            ] + f'<i class="bi bi-star" style="color: {color}"></i>' * (5 - d["Level maturity"])
            final_data.append(d)
        ...
        for path in paths:
            ...
            try:
                pathLengthss[rowIndex][path.nodes[-1].name.split("@")[1]] = {
                    "value": pathLengthss[rowIndex][path.nodes[-1].name.split("@")[1]]["value"] + 1,
                    "link": quote(path.nodes[0].name.split("@")[1] + "_to_" + path.nodes[-1].name.split("@")[1])
                    + ".html",
                }
            except KeyError:
                pathLengthss[rowIndex][path.nodes[-1].name.split("@")[1]] = {
    ...
    def genPathsCrossDomainsAdminPrivileges(self):
        ...
        for key in unique_users_keys:
            ...
            grid_list_domain_admin_targets = Grid(
                "List of foreign domains where " + user + " happens to be a domain admin"
            )
