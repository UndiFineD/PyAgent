# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\examples.py\agent_with_memory_action_f8167095cc54.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\agent_with_memory_action.py
        ...
        **kwargs,
        ...
        if return_prompt:
            prompt = self.prompt.format(
                updates=[{"memory_id": mid, "message": msg.model_dump()} for mid, msg in updates]
            )
...
def main():
    ...
    store_config = StoreConfig(
        dbConfig=DBConfig(db_name="sqlite", path="./debug/data/hotpotqa/cache/test_hotpotQA.sql"),
    ...
    embedding = EmbeddingConfig(provider="huggingface", model_name=r"debug/bge-small-en-v1.5", device="cpu")
