# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\train_20e97349a53e.py
...
def run_eval(
    ...
    if cfg.environment._distributed and cfg.environment._distributed_inference:
        for key, value in val_data.items():
            val_data[key] = sync_across_processes(value, cfg.environment._world_size, group=cfg.environment._cpu_comm)
...
def run_train(
    ...
    if hasattr(cfg.augmentation, "neftune_noise_alpha") and cfg.augmentation.neftune_noise_alpha > 0:
        activate_neftune(model, cfg.augmentation.neftune_noise_alpha)

    scaler: GradScaler | None = None

    if cfg.environment.mixed_precision:
        scaler = GradScaler(enabled=(cfg.environment.mixed_precision_dtype == "float16"))
    ...
    if cfg.training.evaluate_before_training:
        val_loss, val_metric = run_eval(cfg=cfg, model=model, val_dataloader=val_dataloader, val_df=val_df)
    ...
    for epoch in range(start_epoch, cfg.training.epochs):
        ...
        for itr, data in enumerate(tr_it):
            cfg.environment._curr_step += cfg.training.batch_size * cfg.environment._world_size
            ...
            if cfg.environment.mixed_precision and len(cfg.environment.gpus) and not cfg.environment.use_deepspeed:
            ...
            if cfg.environment._local_rank == 0:
                ...
                if (itr + 1) % log_update_steps == 0 or itr == epoch_steps - 1:
                    progress_bar.set_description(f"train loss: {np.mean(losses[-10:]):.2f}", refresh=False)
            ...
            if (itr + 1) % evaluation_step == 0:
                ...
                if cfg.training.save_checkpoint == "last":
                    logger.info(f"Saving last model checkpoint to {cfg.output_directory}")

                    save_checkpoint(model=model, path=cfg.output_directory, cfg=cfg)

                elif cfg.training.save_checkpoint == "each_evaluation_epoch":
                    checkpoint_path = os.path.join(cfg.output_directory, f"epoch_{epoch}_step_{itr}")
...
def run(cfg: DefaultConfigProblemBase) -> float:
    ...
    cfg.training.evaluate_before_training = cfg.training.evaluate_before_training or cfg.training.epochs == 0
    ...
    if any(error_type == "error" for error_type in errors["type"]):
        raise LLMTrainingException("Configuration contains errors. Please fix them before proceeding.")
    ...
    if cfg.environment._distributed:
        ...
        cfg.environment._device = "cuda:0" if (torch.cuda.is_available() and len(cfg.environment.gpus) > 0) else "cpu"
    ...
    if hasattr(cfg.dataset, "system_column") and cfg.dataset.system_column != "None":
        if cfg.dataset.system_column not in train_df.columns:
            raise LLMTrainingException(f"System column '{cfg.dataset.system_column}' not found in train DataFrame.")

    if len(val_df) > int(os.getenv("GPT_EVAL_MAX", 100)) and "GPT" in cfg.prediction.metric:
        logger.warning(
    ...
    if cfg.environment._local_rank == 0:
        total_training_steps = (
            cfg.training.epochs * len(train_dataloader) * cfg.training.batch_size * cfg.environment._world_size
        )
    ...
    if cfg.environment.compile_model:
        ...
        if cfg.environment.use_deepspeed:
            logger.warning(
                "Deepspeed is active, but it doesn't support torch.compile.Skipping compilation for this experiment."
            )
    ...
    if cfg.environment._local_rank == 0:
        ...
        cfg.logging._logger.log("internal", "total_training_steps", total_training_steps)

        cfg.logging._logger.log("internal", "total_validation_steps", total_validation_steps)
    ...
    if cfg.environment._local_rank == 0:
        ...
        if time_took > 86400:
            ...
            time_took_formatted = time.strftime("%-jd %H:%M:%S", time.gmtime(float(time_took - 86400)))

        else:
            time_took_formatted = time.strftime("%H:%M:%S", time.gmtime(float(time_took)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-C", "--config", help="config filename", type=(str), default=argparse.SUPPRESS)

    parser.add_argument("-Y", "--yaml", help="yaml filename", type=(str), default=argparse.SUPPRESS)

    parser_args, unknown = parser.parse_known_args(sys.argv)

    if "config" in parser_args:
        logging.warning("Using deprecated -C argument. Please use -Y instead to load yaml.")
