# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_online.py\src.py\second_brain_online.py\application.py\agents.py\agents_2256f9076126.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-online\src\second_brain_online\application\agents\agents.py
...
def get_agent(retriever_config_path: Path) -> "AgentWrapper":
    agent = AgentWrapper.build_from_smolagents(retriever_config_path=retriever_config_path)
...
class AgentWrapper:
    ...
    def build_from_smolagents(cls, retriever_config_path: Path) -> "AgentWrapper":
        ...
        if settings.USE_HUGGINGFACE_DEDICATED_ENDPOINT:
            ...
            logger.warning(f"Using OpenAI as the summarizer with model: {settings.OPENAI_MODEL_ID}")
...
def extract_tool_responses(agent: ToolCallingAgent) -> str:
    ...
    tool_responses = [msg["content"] for msg in agent.input_messages if msg["role"] == MessageRole.TOOL_RESPONSE]

    return "\n".join(f"-------- OBSERVATION {i + 1} --------\n{response}" for i, response in enumerate(tool_responses))
