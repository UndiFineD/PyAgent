# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\session_commands_8a465dbf07d2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\session_commands.py
...
def handle_compact_command(command: str) -> bool:
    ...
    try:
        ...
        if compaction_strategy == "truncation":
            ...
            compacted, summarized_messages = current_agent.summarize_messages(history, with_protection=True)
        ...
        strategy_info = (
            f"using {compaction_strategy} strategy" if compaction_strategy == "truncation" else "via summarization"
        )
...
def handle_load_context_command(command: str) -> bool:
    ...
    success_msg = Text(f"✅ Context loaded: {len(history)} messages ({total_tokens} tokens)\n📁 From: {session_path}")
