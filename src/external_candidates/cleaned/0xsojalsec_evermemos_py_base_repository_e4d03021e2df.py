# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\oxm.py\mongo.py\base_repository_e4d03021e2df.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\oxm\mongo\base_repository.py
...
class BaseRepository(ABC, Generic[T]):
    ...
    async def create(self, document: T, session: Optional[AsyncClientSession] = None) -> T:
        """
    ...
    async def update(self, document: T, session: Optional[AsyncClientSession] = None) -> T:
        """
    ...
    async def delete(self, document: T, session: Optional[AsyncClientSession] = None) -> bool:
        """
    ...
    async def create_batch(self, documents: List[T], session: Optional[AsyncClientSession] = None) -> List[T]:
        """
    ...
    async def count_all(self, filter_query: Optional[dict] = None) -> int:
        ...
        try:
            if filter_query is None or filter_query == {}:
                ...
                logger.info("✅ Fast estimated count [%s]: %d records", self.model_name, count)
