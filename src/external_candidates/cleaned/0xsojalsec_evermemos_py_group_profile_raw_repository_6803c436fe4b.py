# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\group_profile_raw_repository_6803c436fe4b.py
...
class GroupProfileRawRepository(BaseRepository[GroupProfile]):
    ...
    async def ensure_latest(self, group_id: str, session: Optional[AsyncClientSession] = None) -> bool:
        """
        ...
        try:
            ...
            latest_version = await self.model.find_one({"group_id": group_id}, sort=[("version", -1)], session=session)

            if not latest_version:
                logger.debug("ℹ️  No group profile found to update: group_id=%s", group_id)
    ...
    async def update_by_group_id(
        ...
        try:
            ...
            if version is not None:
                ...
                existing_doc = await self.model.find_one({"group_id": group_id, "version": version}, session=session)
        ...
        try:
            ...
            if version is not None:
                ...
                if success:
                    ...
                    logger.warning("⚠️  No group profile found for deletion: group_id=%s", group_id)
    ...
    async def upsert_by_group_id(
        ...
        try:
            ...
            if existing_doc:
                ...
                new_doc = GroupProfile(group_id=group_id, timestamp=timestamp, **update_data)
