# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\graph.py\ergraph_069eb4c930ac.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Graph\ERGraph.py
...
from Core.Common.Constants import NODE_PATTERN, REL_PATTERN
...
class ERGraph(BaseGraph):
    ...
    async def _openie_post_ner_extract(self, chunk, entities):
        ...
        openie_messages = GraphPrompt.OPENIE_POST_NET.format(
            passage=chunk, named_entity_json=json.dumps(named_entity_json)
        )
    ...
    async def _build_graph(self, chunk_list: List[Any]):
        try:
            results = await asyncio.gather(*[self._extract_entity_relationship(chunk) for chunk in chunk_list])
