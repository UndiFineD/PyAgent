# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_maltego_telegram.py\transforms.py\channeltoforwardedusers_6ffdafb28a00.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-maltego-telegram\transforms\ChannelToForwardedUsers.py
...
async def find_forwarded_messages_from_users(username):
    ...
    async with app:
        async for message in app.get_chat_history(username, limit=limit):
            if message.forward_sender_name is not None or isinstance(message.forward_from, User):
