# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\env.py\tools.py\agent.py\connect_entities.py\resolvers.py\power_connection_resolver_a083484bc533.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\tools\agent\connect_entities\resolvers\power_connection_resolver.py
    ...
    def resolve(
        ...
        if isinstance(source, ElectricityGroup):
            ...
            if isinstance(target, Entity):
                ...
                target_pos = (
                    target.position if (isinstance(target, Entity) or isinstance(target, ElectricityGroup)) else target
                )
