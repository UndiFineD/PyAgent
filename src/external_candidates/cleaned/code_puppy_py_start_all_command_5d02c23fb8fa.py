# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\mcp.py\start_all_command_5d02c23fb8fa.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\mcp\start_all_command.py
...
class StartAllCommand(MCPCommandBase):
    ...
    def execute(self, args: List[str], group_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            if started_count > 0:
                emit_info(
                    Text.from_markup(f"[green]Started {started_count} server(s)[/green]"),
                    message_group=group_id,
                )
            ...
            if failed_count > 0:
                emit_info(
                    Text.from_markup(f"[yellow]Failed to start {failed_count} server(s)[/yellow]"),
                    message_group=group_id,
                )
            ...
            if started_count > 0:
                ...
                try:
                    ...
                    emit_info(
                        Text.from_markup("[dim]Agent reloaded with updated servers[/dim]"),
                        message_group=group_id,
                    )
