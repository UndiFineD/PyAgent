# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alirezarezvani.py\social_media_analyzer.py\scripts.py\calculate_metrics_51859cc873be.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\social-media-analyzer\scripts\calculate_metrics.py
...
class SocialMediaMetricsCalculator:
    ...
    def identify_top_posts(self, metric: str = "engagement_rate", limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if metric == "engagement_rate":
            sorted_posts = sorted(posts_with_metrics, key=lambda x: x["engagement_rate"], reverse=True)

        else:
            sorted_posts = sorted(posts_with_metrics, key=lambda x: x.get(metric, 0), reverse=True)
