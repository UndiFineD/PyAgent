# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\actalker.py\src.py\utils.py\enhance.py\bfr_enhance_96937433be5d.py
...
class test_pipeline(torch.nn.Module):
    ...
    def enhance_cropface(self, cropface):
        ...
        if cropface.dtype == "uint16":
            crop_face_tensor = (
                torch.from_numpy(cv2.cvtColor(cropface, cv2.COLOR_BGR2RGB).astype("float32") / 32767.5 - 1)
                .permute(2, 0, 1)
                .to(self.device)
                .unsqueeze(0)
            )
            ...
            crop_face_tensor = (
                torch.from_numpy(cv2.cvtColor(cropface, cv2.COLOR_BGR2RGB).astype("float32") / 127.5 - 1)
                .permute(2, 0, 1)
                .to(self.device)
                .unsqueeze(0)
            )
        ...
        if cropface.dtype == "uint16":
            result_crop_bgr = cv2.cvtColor(
                (result_merge[0] * 32767.5 + 32767.5).cpu().numpy().astype("uint16").transpose(1, 2, 0),
                cv2.COLOR_RGB2BGR,
            )

        else:
            result_crop_bgr = cv2.cvtColor(
                (result_merge[0] * 127.5 + 127.5).cpu().numpy().astype("uint8").transpose(1, 2, 0),
                cv2.COLOR_RGB2BGR,
            )
