# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\scripts.py\test_externalmedia_call_9d864ea97df2.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\scripts\test_externalmedia_call.py
...
class ExternalMediaCallTester:
    ...
    async def test_call_flow(self):
        ...
        call_started = await self.wait_for_condition(lambda status: status.get("active_calls", 0) > 0, timeout=60)
...
async def main():
    ...
    parser.add_argument("--url", default="http://localhost:15000/health", help="Health endpoint URL")
