# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\tools.py\telephony.py\unified_transfer_e6923d5f2253.py
...
class UnifiedTransferTool(Tool):
    ...
    async def _transfer_to_extension(
        ...
        if result and result.get("status") == 204:
            ...
            logger.error("Extension transfer failed", call_id=context.call_id, result=result)
    ...
    async def _transfer_to_queue(self, context: ToolExecutionContext, queue: str, description: str) -> Dict[str, Any]:
        """
        ...
        await context.update_session(transfer_active=True, transfer_state="in_queue", transfer_target=description)
