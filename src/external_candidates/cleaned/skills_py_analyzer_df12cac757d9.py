# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\robbyczgw_cla.py\personal_analytics.py\scripts.py\analyzer_df12cac757d9.py
...
def analyze_productivity(sessions: List[Dict]) -> Dict:
    ...
    hourly_avg = {hour: sum(scores) / len(scores) for hour, scores in hourly_productivity.items()}

    daily_avg = {day: sum(scores) / len(scores) for day, scores in daily_productivity.items()}
...
def analyze_sentiment(sessions: List[Dict]) -> Dict:
    ...
    hourly_avg = {hour: sum(scores) / len(scores) for hour, scores in sentiment_by_hour.items()}

    daily_avg = {day: sum(scores) / len(scores) for day, scores in sentiment_by_day.items()}

    total = sum(sentiment_counter.values())

    distribution = {sentiment: count / total for sentiment, count in sentiment_counter.items()} if total > 0 else {}
