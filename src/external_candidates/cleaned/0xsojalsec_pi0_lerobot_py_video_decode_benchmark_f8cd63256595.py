# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_pi0_lerobot.py\src.py\pi0_lerobot.py\apis.py\video_decode_benchmark_f8cd63256595.py
...
class BenchmarkConfig:
    ...
    example_name: str = "nusar-2021_action_both_9051-b03a_9051_user_id_2021-02-22_114140"
...
def benchmark_decode(config: BenchmarkConfig) -> None:
    ...
    gpu_mv_decoder: list[VideoDecoder] = [
        VideoDecoder(exo_path, device="cuda", dimension_order="NHWC") for exo_path in exo_video_paths
    ]
    ...
    for frame_idx in tqdm(range(num_frames), desc="CPU Decoding"):
        bgr_list: list[UInt8[np.ndarray, "H W 3"]] = assembly101_data.exo_video_readers[frame_idx]
    ...
    for frame_idx in tqdm(range(gpu_num_frames), desc="GPU Decoding"):
        bgr_list: list[torch.Tensor] = [decoder[frame_idx] for decoder in gpu_mv_decoder]
    ...
    for frame_idx in tqdm(range(gpu_num_frames), desc="GPU Decoding w/ Jaxtyping"):
        bgr_list: list[UInt8[torch.Tensor, "H W 3"]] = [decoder[frame_idx] for decoder in gpu_mv_decoder]
    ...
    for frame_idx in tqdm(range(gpu_num_frames), desc="GPU Decoding w/ Numpy Mem Copy"):
        bgr_list: list[UInt8[np.ndarray, "H W 3"]] = [decoder[frame_idx].cpu().numpy() for decoder in gpu_mv_decoder]
    ...
    gpu_numpy_typing_message = (
        f"GPU w/ Numpy mem copy  + typingdecoding: {total_time:.2f} seconds, FPS: {gpu_numpy_typing_fps:.2f}"
    )
