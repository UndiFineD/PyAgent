# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kesslerio.py\oura_analytics.py\scripts.py\drivers_0415b989f88f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\drivers.py
...
class DriverAnalyzer:
    ...
    def analyze_sleep_drivers(self, sleep_data: Dict) -> List[Driver]:
        ...
        duration_hours = (
            sleep_data.get("total_sleep_duration", 0) / 3600 if sleep_data.get("total_sleep_duration") else 0
        )
        ...
        deep_sleep_hours = (
            sleep_data.get("deep_sleep_duration", 0) / 3600 if sleep_data.get("deep_sleep_duration") else 0
        )
        ...
        drivers.sort(
            key=lambda d: (
                impact_order.get(d.impact, 1),
                severity_order.get(d.severity, 1),
                -abs(d.delta),
            )
        )
