# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-UNO\train.py
...
def inference(
    ...
    with torch.no_grad():
        ref_imgs = [
            ae.encode(ref_img_.to(accelerator.device, torch.float32)).to(torch.bfloat16) for ref_img_ in ref_imgs
        ]
        inp_cond = prepare_multi_ip(t5=t5, clip=clip, img=x, prompt=prompt, ref_imgs=ref_imgs, pe=pe)
        ...
        if accelerator.sync_gradients and global_step % args.checkpointing_steps == 0:
            ...
            for batch_idx in range(min(bs, 8)):
                ...
                for ref_idx in range(len(ref_imgs)):
                    images_ref_log.append(
                        wandb.Image(
                            (ref_imgs[ref_idx][batch_idx : batch_idx + 1] * 0.5 + 0.5).float(),
            ...
            if args.ema:
                ...
                for batch in eval_dataloader:
                    result = inference(batch, dit, t5, clip, vae, accelerator, seed=0)
                    accelerator.log(
                        {f"eval_ema_gen_{i}": wandb.Image(result, caption=batch["txt"][-1])},
                        step=global_step,
                    )
