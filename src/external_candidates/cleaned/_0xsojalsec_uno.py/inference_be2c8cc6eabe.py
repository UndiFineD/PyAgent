# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-UNO\inference.py
...
def main(args: InferenceArgs):
    ...
    for (i, data_dict), j in itertools.product(enumerate(data_dicts), range(args.num_images_per_prompt)):
        ...
        ref_imgs = [Image.open(os.path.join(data_root, img_path)) for img_path in data_dict["image_paths"]]
        if args.ref_size == -1:
