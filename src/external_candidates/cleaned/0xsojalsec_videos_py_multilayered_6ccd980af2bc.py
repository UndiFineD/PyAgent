# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2016.py\brachistochrone.py\multilayered_6ccd980af2bc.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2016\brachistochrone\multilayered.py
...
class MultilayeredScene(Scene):
    ...
    def get_layers(self, n_layers=None):
        ...
        rgb_pair = [np.array(Color(color).get_rgb()) for color in (self.top_color, self.bottom_color)]

        rgb_range = [interpolate(*rgb_pair + [x]) for x in np.arange(0, 1, 1.0 / n_layers)]

        tops = [self.top + x * height * DOWN for x in range(n_layers)]
        ...
        for top, rgb in zip(tops, rgb_range):
            ...
            rect = self.RectClass(height=height, width=width, color=color)
...
class TwoToMany(MultilayeredScene):
    ...
    def construct(self):
        ...
        self.play(
            *[FadeIn(layer, rate_func=squish_rate_func(smooth, x, 1)) for layer, x in zip(layers[1:], it.count(0, 0.2))]
...
class RaceLightInLayers(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        self.play(
            *[
                self.photon_run_along_path(line, rate_func=rate_maker(rate), run_time=2)
...
class ShowDiscretePath(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        self.play(self.photon_run_along_path(self.discrete_path, rate_func=rush_into, run_time=3))
    ...
    def generate_discrete_path(self):
        ...
        self.discrete_path = Mobject1D(color=WHITE, density=3 * DEFAULT_POINT_DENSITY_1D)
        ...
        self.discrete_path.add_line(points[end], FRAME_X_RADIUS * RIGHT + (tops[-1] - 0.5) * UP)
...
class ShowLayerVariables(MultilayeredScene, PhotonScene):
    ...
    def construct(self):
        ...
        for layer, x in zip(self.layers[:3], it.count(1)):
            ...
            brace_endpoints = Mobject(Point(self.top), Point(layer.get_bottom()))
        ...
        for v_eq, path, time in zip(v_equations, center_paths, [2, 1, 0.5]):
            ...
            self.play(FadeToColor(v_eq[0], WHITE), photon_run, run_time=time)
        ...
        self.play(
            *it.chain(
                *[
                    [
                        GrowFromCenter(mob, rate_func=squish_rate_func(smooth, start, 1))
...
class LimitingProcess(MultilayeredScene):
    ...
    def construct(self):
        ...
        glass_sets = [
            Mobject(
                *[Mobject(*layer_sets[x][(2**x) * index : (2**x) * (index + 1)]) for index in range(self.n_layers)]
            ).ingest_submobjects()
...
class ShowLightAndSlidingObject(MultilayeredScene, TryManyPaths, PhotonScene):
    ...
    def construct(self):
        ...
        photon_run = self.photon_run_along_path(loop, rate_func=lambda t: smooth(1.2 * t, 2), run_time=4.1)
        ...
        for path in paths:
            self.play(Transform(loop, path, path_func=path_along_arc(np.pi / 2), run_time=2))
...
class ContinuouslyObeyingSnellsLaw(MultilayeredScene):
    ...
    def show_equation(self, chopped_cycloid, ref_mob):
        ...
        self.play(GrowFromCenter(sin), ShowCreation(arc), GrowFromCenter(theta))
        ...
        self.play(GrowFromCenter(sqrt_y), GrowFromCenter(brace), GrowFromCenter(y_mob))
