# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2023.py\clt.py\galton_board_3f702cf4725b.py
...
class GaltonBoard(InteractiveScene):
    ...
    def show_corner_sum(self, pm_arrows, bits, font_size=48):
        ...
        anim2 = LaggedStart(
            *(
                ReplacementTransform(part.copy().set_opacity(0), sum_term, path_arc=-30 * DEGREES)
...
class NotIdenticallyDistributed(GaltonBoard):
    def construct(self):
        ...
        def get_bounce_distribution(peg, sigma=30 * DEGREES):
            ...
            arrows = VGroup(
                *(get_peg_arrow(peg, angle, denom * gauss_func(angle, 0, sigma) * max_arrow_len) for angle in angles)
            )
