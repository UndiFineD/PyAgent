# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\mcts.py\evaluator_66cf8facf148.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\mcts\evaluator.py
            ...
            for i, (
                ...
                if self.logger:
                    self.logger.update_instance(
                        ...
                        total_programs=self.logger.groups[self.port_to_group[self.instances[i].tcp_port]]
                ...
                conversation.add_result(
                    ...
                    objectives=(program.meta["objectives"] if "objectives" in program.meta else []),
                )  #
            ...
            if self.logger:
                for instance in self.instances:
                    self.logger.update_instance(
                        instance.tcp_port,
                        status="error",
                        error_count=self.logger.groups[self.port_to_group[instance.tcp_port]]
