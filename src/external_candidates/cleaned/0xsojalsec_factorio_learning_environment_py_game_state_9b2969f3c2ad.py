# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\commons.py\models.py\game_state_9b2969f3c2ad.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\commons\models\game_state.py
...
class GameState:
    ...
    agent_messages: List[Any] = field(default_factory=list)  # Can be List[Dict] or List[List[Dict]]
    ...
    def parse_raw(cls, json_str: str) -> "GameState":
        ...
        if "research" in data:
            research = ResearchState(
                technologies={name: TechnologyState(**tech) for name, tech in data["research"]["technologies"].items()},
    ...
    def to_raw(self) -> str:
        ...
        data = {
            ...
            "inventories": [
                inventory.__dict__ if hasattr(inventory, "__dict__") else inventory for inventory in self.inventories
            ],
        ...
        if self.research:
            data["research"] = {
                "technologies": {name: asdict(tech) for name, tech in self.research.technologies.items()},
