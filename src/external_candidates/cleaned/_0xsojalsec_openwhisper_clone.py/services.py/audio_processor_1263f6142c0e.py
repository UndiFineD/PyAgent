...
class AudioProcessor:
    ...
    def preview_file(self, file_path: str) -> AudioFilePreview:
        ...
        logging.info(
            f"Audio preview: {file_name}, {file_size_mb:.2f} MB, {duration_seconds:.1f}s, {estimated_chunks} chunk(s)"
        )
    ...
    def _find_split_points(self, audio_data: np.ndarray, sample_rate: int) -> List[int]:
        ...
        while search_start < len(audio_data):
            ...
            best_split = self._find_best_silence(audio_smooth, search_start, search_end, silence_samples, sample_rate)
