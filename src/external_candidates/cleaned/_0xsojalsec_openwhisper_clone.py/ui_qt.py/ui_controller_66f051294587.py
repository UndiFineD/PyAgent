...
class UIController(QObject):
    ...
    def set_transcription_stats(self, transcription_time: float, audio_duration: float, file_size: int):
        ...
        """
        self.main_window.set_transcription_stats(transcription_time, audio_duration, file_size)
    ...
    def open_upload_audio_dialog(self):
        ...
        audio_filters = (
            "Audio Files (*.wav *.mp3 *.m4a *.ogg *.flac *.wma);;WAV Files (*.wav);;MP3 Files (*.mp3);;All Files (*.*)"
        )

        file_path, _ = QFileDialog.getOpenFileName(self.main_window, "Select Audio File", "", audio_filters)
        ...
        try:
            ...
            QMessageBox.critical(self.main_window, "Error", f"Failed to analyze audio file:\n{e}")
