# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nudge\baselines\adapter_utils.py
"""Adapter utils.
...
def get_val_acc(q_embs, data_embs, nontrain_embeddings_or_shard, labels, val_batch_size, k):
    ...
    for val_batch_i in range(int(math.ceil(data_embs.shape[0] / val_batch_size))):
        sim_preds.append(
            torch.matmul(
                q_embs,
                torch.t(data_embs[val_batch_i * val_batch_size : (val_batch_i + 1) * val_batch_size]),
            ).squeeze()
        )
...
def train_model(
    ...
    if (
        nontrain_embeddings_or_shard[0] is not None and nontrain_embeddings_or_shard[0].shape[0] > 0
    ) or nontrain_embeddings_or_shard[1] is not None:
