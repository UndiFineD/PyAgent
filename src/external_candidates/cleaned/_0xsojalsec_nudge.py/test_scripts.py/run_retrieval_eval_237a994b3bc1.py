# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nudge\test_scripts\run_retrieval_eval.py
import argparse
import gc
import json
import math
...
from utils import calc_metrics, calc_metrics_batch, get_data
...
def run_test(
    ...
    if finetuner == "NUDGEN":
        ...
        new_embs = ft.finetune_embeddings(
            embeddings,
            query_sets["train"],
            query_sets["dev"],
            nontrain_embeddings_or_shard,
            **finetuner_config,
        )
        ...
        ret = kNNRetriever(
            ...
            nontrain_embeddings,
            dist_metric="cos",
            path_to_embeddings_nontrain_shards=path_to_sharded_emb_nontrain,
            device=device,
        )
        ...
        ft = PTFT(device)
        embedding_model = ft.finetune_model(
            query_sets["train"],
            query_sets["dev"],
            dataset,
            nontrain_dataset,
        ...
        if nontrain_dataset is not None:
            nontrain_embeddings = np.array(
                embedding_model.encode(
                    list(nontrain_dataset["text"].values),
                    batch_size=batch_size,
                    show_progress_bar=True,
                )
            ).astype(np.float32)
        test_q_embeddings = np.array(
            embedding_model.encode(
                list(query_sets["test"]["q_df"]["input"].values),
                batch_size=batch_size,
                show_progress_bar=True,
            )
        ).astype(np.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument("test_set", type=str, help="name of test dataset")
    parser.add_argument("retriever", type=str, help="retriever and finetuning method to use")
    parser.add_argument("test_name", type=str, help="test name to store results")
    parser.add_argument("out_path", type=str, help="directory to store results")
    parser.add_argument(
        "finetuner_config_path",
        type=str,
        help="path to json file with finetuner config",
    )
