        ...
        if dist == "cos":
            curr_topk = torch.topk(
                torch.matmul(
                    ...
                    torch.t(
                        normalize(
                            torch.from_numpy(embeddings_nontrain[i * batch_size : (i + 1) * batch_size]).to(
                                q_embs.device
                            )
                        )
                    ),
            ...
            curr_topk = torch.topk(
                torch.matmul(
                    q_embs,
                    torch.t(
                        torch.from_numpy(embeddings_nontrain[i * batch_size : (i + 1) * batch_size]).to(q_embs.device)
                    ),
                ),
                k=k,
                dim=1,
    ...
    def _get_all_I_i(self, qs_val, val_batch_size, embs, upd, nontrain_sims, val_anss):
        ...
        for val_batch_i in range(int(math.ceil(qs_val.shape[0] / val_batch_size))):
            ...
            curr_mins, curr_maxs = self._get_I_i(
                qs_val[val_batch_i * val_batch_size : (val_batch_i + 1) * val_batch_size],
                embs,
                upd,
                curr_nontrain_sims,
                val_anss[val_batch_i * val_batch_size : (val_batch_i + 1) * val_batch_size],
            )
        ...
        for gamma in gamma_vals:
            ...
            for val_batch_i in range(int(math.ceil(embeddings.shape[0] / val_batch_size))):
                preds.append(
                    torch.matmul(
                        qs_val,
                        torch.t(new_embs[val_batch_i * val_batch_size : (val_batch_i + 1) * val_batch_size]),
                    )
                )
