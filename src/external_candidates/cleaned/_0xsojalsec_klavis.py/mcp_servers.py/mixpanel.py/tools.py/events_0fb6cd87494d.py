# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\mixpanel\tools\events.py
import json
import logging
import time
import uuid
...
async def send_events(project_id: str, events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, event_data in enumerate(events):
            ...
            event_obj = {"event": event_name, "properties": properties}

            batch_events.append(event_obj)

        # Use Service Account auth with /import endpoint
        result = await MixpanelIngestionClient.make_request("POST", "/import", data=batch_events, project_id=project_id)
...
async def get_event_count(from_date: str, to_date: str, event: Optional[str] = None) -> Dict[str, Any]:
...
async def get_top_events(from_date: str, to_date: str, limit: Optional[int] = 10) -> Dict[str, Any]:
...
async def get_todays_top_events(limit: Optional[int] = 10) -> Dict[str, Any]:
...
async def get_events(project_id: str) -> List[str]:
    ...
    try:
        ...
        result = await MixpanelQueryClient.make_request("GET", "/query/events/names", params=params)
...
async def get_event_properties(project_id: str, event: str) -> List[str]:
...
async def get_event_property_values(project_id: str, event: str, property_name: str) -> List[str]:
