# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\onedrive\server.py
import base64
import contextlib
import json
import logging
...
from tools import (  # Base; Both Items (Files & Folders); Files; Folders; Search & List; Sharing
    auth_token_context,
...
def main(
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "file_id": {
                            "type": "string",
                            "description": "ID of the file/folder to rename",
                        },
                        "new_name": {
                            "type": "string",
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "parent_folder": {
                            "type": "string",
                            "description": "'root' to create in root or ID of the parent folder",
                        },
                        "new_file_name": {
                            "type": "string",
                        ...
                        "data": {
                            "type": "string",
                            "description": "Content for the new file (optional)",
                        },
                        "if_exists": {
                            "type": "string",
    ...
    async def call_tool(
        ...
        if name == "onedrive_rename_item":
            try:
                result = await onedrive_rename_item(file_id=arguments["file_id"], new_name=arguments["new_name"])
                return [
                    types.TextContent(
                        type="text",
            ...
            try:
                result = await onedrive_delete_item(item_id=arguments["item_id"])
                return [
                    types.TextContent(
                        type="text",
                        text=result,
        ...
        # File Content Operations
            ...
            try:
                result = await onedrive_read_file_content(file_id=arguments["file_id"])
                return [
                    types.TextContent(
                        type="text",
        ...
        # Listing & Searching
            ...
            try:
                result = await onedrive_list_inside_folder(folder_id=arguments["folder_id"])
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
            ...
            try:
                result = await onedrive_search_item_by_name(itemname=arguments["itemname"])
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
            ...
            try:
                result = await onedrive_search_folder_by_name(folder_name=arguments["folder_name"])
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
            ...
            try:
                result = await onedrive_get_item_by_id(item_id=arguments["item_id"])
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
