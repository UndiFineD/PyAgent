# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\quickbooks\server.py
#!/usr/bin/env python3
"""
...
qb_credentials_context: ContextVar[dict] = ContextVar("qb_credentials", default=None)
...
async def call_tool(
    ...
    try:
        ...
        session = session_manager_instance.get_session(qb_access_token, qb_realm_id, qb_environment)
...
def main(
    ...
    async def handle_sse(request):
        ...
        if any([qb_access_token, qb_realm_id, qb_environment]):
            ...
            logger.info(f"QB credentials extracted from SSE headers for realm: {qb_realm_id or 'env'}")
    ...
    async def handle_streamable_http(scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # Extract QB credentials from headers
        qb_access_token, qb_realm_id, qb_environment = session_manager_instance.extract_credentials_from_headers(scope)
        ...
        if any([qb_access_token, qb_realm_id, qb_environment]):
            ...
            qb_credentials_context.set(credentials)
            logger.info(f"QB credentials extracted from headers for realm: {qb_realm_id or 'env'}")
