# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\quickbooks\tools\customers.py
...
customer_properties_user_define = {
    ...
    "WebAddr": {"type": "string", "description": "Website address of the Customer."},
    "CompanyName": {
        "type": "string",
...
get_customer_tool = Tool(
    ...
    inputSchema={
        "type": "object",
        "properties": {"Id": {"type": "string", "description": "The QuickBooks customer ID"}},
        "required": ["Id"],
    },
    annotations=types.ToolAnnotations(**{"category": "QUICKBOOKS_CUSTOMER"}),
)
...
list_customers_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "ActiveOnly": {
                "type": "boolean",
                "description": "Return only active customers",
                "default": True,
            },
        },
    },
...
deactivate_customer_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            "Id": {
                "type": "string",
                "description": "The QuickBooks customer ID to deactivate",
            }
        },
        "required": ["Id"],
    },
...
activate_customer_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            "Id": {
                "type": "string",
                "description": "The QuickBooks customer ID to activate",
            }
        },
        "required": ["Id"],
    },
...
search_customers_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            "DisplayName": {
                "type": "string",
                "description": "Search by customer display name (partial match)",
            },
            "GivenName": {
                "type": "string",
                "description": "Search by given/first name (partial match)",
            },
            "FamilyName": {
                "type": "string",
                "description": "Search by family/last name (partial match)",
            },
            "MiddleName": {
                "type": "string",
                "description": "Search by middle name (partial match)",
            },
            "CompanyName": {
                "type": "string",
            ...
            "CurrencyRefName": {
                "type": "string",
                "description": "Search by currency name (partial match)",
            },
            "PaymentMethodRefValue": {
                "type": "string",
            ...
            "PaymentMethodRefName": {
                "type": "string",
                "description": "Search by payment method name (partial match)",
            },
            # Date filters
            "OpenBalanceDateFrom": {
                "type": "string",
                "description": "Search by opening balance date from (YYYY-MM-DD format)",
            },
            "OpenBalanceDateTo": {
                "type": "string",
                "description": "Search by opening balance date to (YYYY-MM-DD format)",
            },
            # Other filters
            "WebAddr": {
                "type": "string",
                "description": "Search by website address (partial match)",
            },
            "Notes": {
                "type": "string",
                "description": "Search by notes/description (partial match)",
            },
            # Pagination
            "MaxResults": {
                "type": "integer",
...
def mcp_object_to_customer_data(**kwargs) -> Dict[str, Any]:
    ...
    if "PrimaryEmailAddr" in kwargs:
        customer_data["PrimaryEmailAddr"] = {"Address": kwargs["PrimaryEmailAddr"]}

    # Handle phone fields - use PrimaryPhone as the standard input field
    if "PrimaryPhone" in kwargs:
        customer_data["PrimaryPhone"] = {"FreeFormNumber": kwargs["PrimaryPhone"]}

    # Handle Mobile input (for backward compatibility with existing code)
    if "Mobile" in kwargs:
        customer_data["Mobile"] = {"FreeFormNumber": kwargs["Mobile"]}
...
class CustomerManager:
    ...
    async def create_customer(self, **kwargs) -> Dict[str, Any]:
        ...
        if not display_name_provided and not name_components_provided:
            raise ValueError("At least one of: DisplayName, GivenName, FamilyName, MiddleName must be provided.")
    ...
    async def update_customer(self, **kwargs) -> Dict[str, Any]:
        ...
        sync_token = current_customer_response.get("Customer", {}).get("SyncToken", "0")

        # Use the mcp_object_to_customer_data function to convert the input
        customer_data = mcp_object_to_customer_data(**kwargs)

        # Add required fields for update
        customer_data.update({"Id": customer_id, "SyncToken": sync_token, "sparse": True})
    ...
    async def search_customers(self, **kwargs) -> List[Dict[str, Any]]:
        ...
        if kwargs.get("BillWithParent") is not None:
            conditions.append(f"BillWithParent = {str(kwargs['BillWithParent']).lower()}")
        ...
        if kwargs.get("SalesTermRefValue"):
            conditions.append(f"SalesTermRef.value = '{kwargs['SalesTermRefValue']}'")

        if kwargs.get("PaymentMethodRefValue"):
            conditions.append(f"PaymentMethodRef.value = '{kwargs['PaymentMethodRefValue']}'")

        if kwargs.get("DefaultTaxCodeRefValue"):
            conditions.append(f"DefaultTaxCodeRef.value = '{kwargs['DefaultTaxCodeRefValue']}'")
        ...
        if kwargs.get("PreferredDeliveryMethod"):
            conditions.append(f"PreferredDeliveryMethod = '{kwargs['PreferredDeliveryMethod']}'")

        if kwargs.get("GSTRegistrationType"):
            conditions.append(f"GSTRegistrationType = '{kwargs['GSTRegistrationType']}'")
        ...
        if kwargs.get("MinBalanceWithJobs") is not None:
            conditions.append(f"BalanceWithJobs >= {kwargs['MinBalanceWithJobs']}")
        if kwargs.get("MaxBalanceWithJobs") is not None:
            conditions.append(f"BalanceWithJobs <= {kwargs['MaxBalanceWithJobs']}")
        ...
        if kwargs.get("OpenBalanceDateFrom"):
            conditions.append(f"OpenBalanceDate >= '{kwargs['OpenBalanceDateFrom']}'")
        if kwargs.get("OpenBalanceDateTo"):
            conditions.append(f"OpenBalanceDate <= '{kwargs['OpenBalanceDateTo']}'")
        ...
        if kwargs.get("ParentRefName"):
            ...
            conditions.append(f"ParentRef.value IN (SELECT Id FROM Customer WHERE DisplayName LIKE '%{parent_name}%')")
            ...
            for customer in customers:
                ...
                for field, search_value in partial_match_filters.items():
                    if field in [
                        ...
                        addr_line = customer["BillAddr"].get("Line1", "").lower()
                        if search_value not in addr_line:
                            should_include = False
                            break
                    elif field == "ShipAddrLine1" and "ShipAddr" in customer and isinstance(customer["ShipAddr"], dict):
                        addr_line = customer["ShipAddr"].get("Line1", "").lower()
                        if search_value not in addr_line:
                            should_include = False
                            break
                    elif (
                        field == "CurrencyRefName"
                        and "CurrencyRef" in customer
                        and isinstance(customer["CurrencyRef"], dict)
                    ):
                        currency_name = customer["CurrencyRef"].get("name", "").lower()
                        if search_value not in currency_name:
                            should_include = False
                            break
                    elif (
                        field == "SalesTermRefName"
                        and "SalesTermRef" in customer
                        and isinstance(customer["SalesTermRef"], dict)
                    ):
                        term_name = customer["SalesTermRef"].get("name", "").lower()
                        if search_value not in term_name:
                            should_include = False
                            break
                    elif (
                        field == "PaymentMethodRefName"
                        and "PaymentMethodRef" in customer
                        and isinstance(customer["PaymentMethodRef"], dict)
                    ):
                        method_name = customer["PaymentMethodRef"].get("name", "").lower()
                        if search_value not in method_name:
                            should_include = False
                            break
                    elif (
                        field == "DefaultTaxCodeRefName"
                        and "DefaultTaxCodeRef" in customer
                        and isinstance(customer["DefaultTaxCodeRef"], dict)
                    ):
                        tax_name = customer["DefaultTaxCodeRef"].get("name", "").lower()
