...
async def list_events(
    ...
    try:
        ...
        events = [{key: event[key] for key in items_keys if key in event} for event in events_result.get("items", [])]
    ...
    try:
        ...
        if attendees_to_remove:
            event["attendees"] = [
                ...
                if attendee.get("email", "").lower() not in [email.lower() for email in attendees_to_remove]
            ]

        if attendees_to_add:
            existing_emails = {attendee.get("email", "").lower() for attendee in event.get("attendees", [])}
            new_attendees = [{"email": email} for email in attendees_to_add if email.lower() not in existing_emails]
...
async def add_attendees_to_event(
    ...
    try:
        ...
        new_attendees = [{"email": email} for email in attendees if email.lower() not in existing_emails]
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "GOOGLE_CALENDAR_CALENDAR"}),
            ),
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "GOOGLE_CALENDAR_EVENT"}),
            ),
            types.Tool(
