    ...
    try:
        ...
        for job in jobs:
            job_info = {
                ...
                "job_id": job.get("job_id", "N/A"),
                "description_snippet": (
                    job.get("description", "N/A")[:150] + "..." if job.get("description") else "N/A"
                ),
            }
            formatted_jobs.append(job_info)
...
async def search_remote_jobs(
    ...
    try:
        ...
        for job in jobs:
            ...
            if any(
                remote_keyword in location or remote_keyword in title or remote_keyword in description
    ...
    try:
        ...
        suggestions = {
            ...
            "companies_hiring": list(
                set([job.get("company_name", "") for job in jobs[:10] if job.get("company_name")])
            ),
