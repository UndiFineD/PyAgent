# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\tools\agents.py
import logging
...
async def list_agents(
    ...
    try:
        ...
        response = await make_freshdesk_request("GET", "/agents", options={"query_params": params})
...
async def bulk_create_agents(agents_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    try:
        return await make_freshdesk_request("POST", "/agents/bulk", data={"agents": agents_data})
    except Exception as e:
