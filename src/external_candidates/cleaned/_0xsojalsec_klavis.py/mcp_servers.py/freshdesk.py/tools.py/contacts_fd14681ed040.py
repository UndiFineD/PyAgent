...
async def delete_contact(contact_id: int, hard_delete: bool = False, force: bool = False) -> Dict[str, Any]:
    """
    Delete a contact.
...
async def filter_contacts(query: str, page: int = 1, updated_since: Optional[str] = None) -> Dict[str, Any]:
    """
...
async def make_contact_agent(
    ...
    try:
        return await make_freshdesk_request("PUT", f"/contacts/{contact_id}/make_agent", data=agent_data)
    except Exception as e:
...
async def send_contact_invite(contact_id: int) -> Dict[str, Any]:
    ...
    """
    try:
        return await make_freshdesk_request("POST", f"/contacts/{contact_id}/send_invite")
    except Exception as e:
