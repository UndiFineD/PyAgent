# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\tools\thread.py
import logging
    ...
    **kwargs,
    ...
    try:
        ...
        if anchor_id and anchor_type:
            thread_data["anchor"] = {"id": str(anchor_id), "type": anchor_type}
        ...
        response = await make_freshdesk_request(method="POST", endpoint="/collaboration/threads", data=thread_data)
...
async def get_thread_by_id(thread_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await make_freshdesk_request(method="GET", endpoint=f"/collaboration/threads/{thread_id}")
...
async def update_thread(
    ...
    **kwargs,
    ...
    """
    try:
        thread_data = {"title": title, "description": description, **kwargs}
...
async def delete_thread(thread_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await make_freshdesk_request(method="DELETE", endpoint=f"/collaboration/threads/{thread_id}")
    ...
    **kwargs,
    ...
    try:
        ...
        if any(
            ...
            message_data["participants"] = {"email": {}}
        ...
        response = await make_freshdesk_request(method="POST", endpoint="/collaboration/messages", data=message_data)
...
async def get_thread_message_by_id(message_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await make_freshdesk_request(method="GET", endpoint=f"/collaboration/messages/{message_id}")
...
async def delete_thread_message(message_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await make_freshdesk_request(method="DELETE", endpoint=f"/collaboration/messages/{message_id}")

        logger.info(f"Successfully deleted thread message")
        return {
            "success": True,
            "message": f"Thread message {message_id} deleted successfully",
        }
