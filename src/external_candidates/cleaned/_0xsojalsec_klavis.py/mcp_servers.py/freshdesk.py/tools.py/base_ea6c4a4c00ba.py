...
def get_config_value(
    ...
    raise RuntimeError(
        f"No {env_name} found. Please set either the {env_name} environment variable or provide it in the context."
    )


def get_auth_token() -> str:
    return get_config_value(context_var=auth_token_context, env_name="FRESHDESK_API_KEY")
    ...
    if isinstance(e, requests.exceptions.RequestException):
        ...
        if hasattr(e, "response") and e.response is not None:
            ...
            if isinstance(e, requests.exceptions.Timeout):
                ...
                error_response["error"].update({"code": "request_failed", "message": f"{str(e)}"})
