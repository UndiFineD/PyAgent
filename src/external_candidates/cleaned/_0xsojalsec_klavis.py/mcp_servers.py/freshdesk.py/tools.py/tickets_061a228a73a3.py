# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\freshdesk\tools\tickets.py
import logging
...
async def delete_multiple_tickets(ticket_ids: List[int]) -> Dict[str, Any]:
    ...
    try:
        ...
        data = {"bulk_action": {"ids": ticket_ids}}
        await make_freshdesk_request("POST", endpoint, data=data)
        return {
            "success": True,
            "message": f"Tickets {ticket_ids} deleted successfully",
        }
    except Exception as e:
...
async def list_tickets(
    ...
    **filters,
    ...
    try:
        ...
        response = await make_freshdesk_request("GET", "/tickets", options={"query_params": params})
...
async def add_note_to_ticket(
    ...
    try:
        ...
        response = await make_freshdesk_request("POST", f"/tickets/{ticket_id}/notes", data=note_data, options=options)
        return response
...
async def reply_to_a_ticket(
    ...
    try:
        ...
        response = await make_freshdesk_request("POST", f"/tickets/{ticket_id}/reply", data=data, options=options)
        return response
...
async def update_note(note_id: int, body: str, attachments: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        response = await make_freshdesk_request("PUT", f"/conversations/{note_id}", data=note_data, options=options)
        return response
...
async def filter_tickets(query: str, page: int = 1, per_page: int = 30) -> Dict[str, Any]:
    """
    ...
    """
    try:
        params = {"query": f'"{query}"', "page": page, "per_page": min(per_page, 30)}

        response = await make_freshdesk_request("GET", "/search/tickets", options={"query_params": params})
...
async def merge_tickets(
    ...
    try:
        ...
        await make_freshdesk_request("PUT", f"/tickets/merge", data=merge_data)
        return {
            "success": True,
...
async def watch_ticket(ticket_id: int, user_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        await make_freshdesk_request("POST", f"/tickets/{ticket_id}/watch", data=data)
...
async def forward_ticket(
    ...
    **kwargs,
    ...
    try:
        ...
        await make_freshdesk_request("POST", f"/tickets/{ticket_id}/forward", data=data)
        return {
            "success": True,
            "message": f"Ticket {ticket_id} forwarded successfully",
        }
    except Exception as e:
...
async def delete_archived_ticket(ticket_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Archived ticket {ticket_id} deleted successfully",
        }
    except Exception as e:
