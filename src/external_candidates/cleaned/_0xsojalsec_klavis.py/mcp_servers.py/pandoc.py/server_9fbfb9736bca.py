...
def upload_blob_and_get_signed_url(bucket_name, source_file_name, destination_blob_name):
    """
    ...
    try:
        ...
        logger.error(f"Error: Bucket '{bucket_name}' not found or insufficient permissions.")
        return None
...
async def convert_markdown_to_file(markdown_text: str, output_format: str) -> str:
    ...
    with tempfile.NamedTemporaryFile(delete=True, suffix=f".{output_format}", delete_on_close=True) as temp_file:
