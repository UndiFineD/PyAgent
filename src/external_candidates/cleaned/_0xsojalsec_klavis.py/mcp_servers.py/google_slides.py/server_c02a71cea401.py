# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_slides\server.py
import base64
import contextlib
import json
import logging
...
def get_credentials():
    ...
    if os.path.exists("service-account.json"):
        return service_account.Credentials.from_service_account_file("service-account.json", scopes=SCOPES)
        ...
        if title or content:
            ...
            if content_requests:
                service.presentations().batchUpdate(
                    presentationId=presentation_id, body={"requests": content_requests}
                ).execute()
...
async def get_presentation(presentation_id: str, fields: Optional[str] = None) -> str:
    ...
    try:
        ...
        presentation = service.presentations().get(presentationId=presentation_id, fields=fields).execute()
...
async def batch_update_presentation(presentation_id: str, requests: List[Dict]) -> str:
    ...
    try:
        ...
        created_slides = [r.get("createSlide", {}).get("objectId") for r in replies if "createSlide" in r]
        if created_slides:
...
def main(
    ...
    async def call_tool(
        ...
        if name == "create_presentation":
            ...
            if not title:
                return [types.TextContent(type="text", text="Error: 'title' parameter is required")]
            ...
            if not requests:
                return [types.TextContent(type="text", text="Error: 'requests' parameter is required")]
