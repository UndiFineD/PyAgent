# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\airtable\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option("--port", default=AIRTABLE_MCP_SERVER_PORT, help="Port to listen on for HTTP")
@click.option(
    "--log-level",
    default="INFO",
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AIRTABLE_BASE"}),
            ),
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AIRTABLE_TABLE"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AIRTABLE_FIELD"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "AIRTABLE_RECORD"}),
            ),
            types.Tool(
        ...
        if name == "airtable_list_bases_info":
            ...
            try:
                result = await list_records(
                    ...
                    return_fields_by_field_id=arguments.get("return_fields_by_field_id"),
                )
                return [
                    types.TextContent(
                        type="text",
