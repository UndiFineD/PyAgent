# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\airtable\tools\base.py
import logging
import os
...
async def make_airtable_request(
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            ...
            logger.error(f"Airtable API request failed: {e.status} {e.message} for {method} {url}")
            ...
            try:
                ...
                pass
            raise RuntimeError(f"Airtable API Error ({e.status}): {error_details}") from e
        except Exception as e:
            logger.error(f"An unexpected error occurred during Airtable API request: {e}")
