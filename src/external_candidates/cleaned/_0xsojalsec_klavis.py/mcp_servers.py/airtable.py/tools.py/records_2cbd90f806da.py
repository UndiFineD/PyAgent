# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\airtable\tools\records.py
import logging
...
async def get_record(base_id: str, table_id: str, record_id: str) -> Dict[str, Any]:
    ...
    logger.info(f"Executing tool: get_record for record {record_id} in table {table_id}, base {base_id}")
    return await make_airtable_request("GET", endpoint)
...
async def create_records(
    ...
    logger.info(f"Executing tool: create_records for table {table_id} in base {base_id}")
...
async def update_records(
    ...
    logger.info(f"Executing tool: update_records for table {table_id} in base {base_id}")
...
async def delete_records(
    ...
    logger.info(f"Executing tool: delete_records for table {table_id} in base {base_id}")
