# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_drive\server.py
import base64
import contextlib
import json
import logging
...
async def get_document_content_by_id(document_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Google Docs API Error ({e.resp.status}): {error_detail.get('error', {}).get('message', 'Unknown error')}"
        )
...
def build_files_list_query(
    ...
    if document_contains:
        for keyword in document_contains:
            ...
            keyword_query = f"(name contains '{name_contains}' or fullText contains '{full_text_contains}')"
    ...
    if document_not_contains:
        for keyword in document_not_contains:
            ...
            keyword_query = (
                f"(name not contains '{name_not_contains}' and fullText not contains '{full_text_not_contains}')"
            )
            query.append(keyword_query)
...
def build_files_list_params(
    ...
    if include_shared_drives or search_only_in_shared_drive_id or include_organization_domain_documents:
...
def build_file_tree_request_params(
    ...
    params = {
        ...
        "fields": ("files(id, name, parents, mimeType, driveId, size, createdTime, modifiedTime, owners)"),
        "orderBy": ",".join([item.value for item in order_by]),
    }
    ...
    if include_shared_drives or restrict_to_shared_drive_id or include_organization_domain_documents:
        params["includeItemsFromAllDrives"] = "true"
        params["supportsAllDrives"] = "true"
...
async def create_shared_drive(
    ...
    try:
        ...
        result = service.drives().create(body=drive_metadata, requestId=request_id).execute()
        ...
        for drive_id, drive_files in file_tree.items():
            if drive_id == "My Drive":
                ...
                try:
                    ...
                    drive_name = f"Shared Drive (name unavailable: 'HttpError {e.status_code}: {e.reason}')"
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "GOOGLE_DRIVE_DOCUMENT"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "GOOGLE_DRIVE_FILE"}),
            ),
            types.Tool(
    ...
    async def call_tool(
        ...
        if name == "google_drive_search_documents":
            ...
            try:
                ...
                result = await create_shared_drive(name=name, request_id=request_id)
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
