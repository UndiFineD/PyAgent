# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\google_docs\server.py
import base64
import contextlib
import json
import logging
...
async def insert_text_at_end(document_id: str, text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        requests = [{"insertText": {"location": {"index": int(end_index) - 1}, "text": text}}]
...
async def create_document_from_text(title: str, text_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        service.documents().batchUpdate(documentId=document["document_id"], body={"requests": requests}).execute()
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "GOOGLE_DOCS_DOCUMENT"}),
            ),
            types.Tool(
