# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\close\tools\base.py
import asyncio
import json
import logging
    ...
    def response_cleaner(data: dict[str, Any]) -> dict[str, Any]:
        ...
        for k, v in data.items():
            if isinstance(v, dict):
                ...
                data[k] = [item if not isinstance(item, dict) else response_cleaner(item) for item in v]
        ...
        try:
            ...
            if "error" in data:
                ...
                if len(errors) == 1:
                    ...
                    developer_message = (
                        f"Multiple errors occurred: {json.dumps(errors)} (HTTP status code: {response.status_code})"
                    )
            else:
