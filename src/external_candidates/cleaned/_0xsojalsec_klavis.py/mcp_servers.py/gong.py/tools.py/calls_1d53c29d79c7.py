# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\gong\tools\calls.py
import logging
...
async def list_calls(
    ...
    """
    if not from_date:
        from_date = (datetime.now(timezone.utc) - timedelta(days=30)).isoformat(timespec="seconds")
    if not to_date:
