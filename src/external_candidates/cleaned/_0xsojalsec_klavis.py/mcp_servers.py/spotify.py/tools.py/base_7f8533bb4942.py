# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\spotify\tools\base.py
import os
...
def get_spotify_client() -> Spotify:
    ...
    if not client_id or not client_secret:
        raise ValueError("Spotify client ID/secret not found. Set SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET.")
            ...
            output.append(
                {
                    ...
                    "name": item.get("name"),
                    "authors": (
                        [author.get("name") for author in item.get("authors", [])] if item.get("authors") else []
                    ),
                    "narrators": (
                        [narrator.get("name") for narrator in item.get("narrators", [])]
                        if item.get("narrators")
                        else []
                    ),
                    "release_date": item.get("release_date"),
                    "publisher": item.get("publisher"),
