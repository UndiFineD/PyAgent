# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\confluence\utils.py
import re
...
def validate_ids(ids: list[str] | None, max_length: int) -> None:
    ...
    if len(ids) > max_length:
        raise RetryableToolError(message=f"The 'ids' parameter must have less than {max_length} items. Got {len(ids)}")
