...
def get_notion_client() -> Client:
    ...
    if not token:
        raise ValueError(
            "Notion API key not found. Please set NOTION_API_KEY environment variable or provide x-auth-token header."
        )
...
def clean_notion_response(response: dict) -> dict:
    ...
    if isinstance(response, dict):
        ...
        cleaned = {k: v for k, v in response.items() if k not in ["request_id", "developer_survey"]}
        ...
        for key, value in cleaned.items():
            if isinstance(value, dict):
                ...
                cleaned[key] = [clean_notion_response(item) if isinstance(item, dict) else item for item in value]
