# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\cal_com\server.py
import base64
import contextlib
import json
import logging
...
from tools import (  # base.py; schedule.py
    auth_token_context,
...
def main(
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "CAL_SCHEDULE"}),
            ),
            # Create a schedule
            types.Tool(
    ...
    async def call_tool(
        ...
        if name == "cal_get_all_schedules":
            ...
            try:
                result = await cal_get_schedule(schedule_id=arguments["schedule_id"])
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
            ...
            try:
                result = await cal_delete_a_schedule(schedule_id=arguments["schedule_id"])
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps(result, indent=2),
