# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\salesforce\tools\leads.py
import logging
...
async def get_leads(
    ...
    logger.info(
        f"Executing tool: get_leads with status: {status}, limit: {limit}, name_contains: {name_contains}, company_contains: {company_contains}, email_contains: {email_contains}, industry: {industry}"
    )
    try:
        ...
        if company_contains:
            ...
            company_like_conditions = " OR ".join(
                [f"Company LIKE '%{variation}%'" for variation in set(company_variations)]
            )
