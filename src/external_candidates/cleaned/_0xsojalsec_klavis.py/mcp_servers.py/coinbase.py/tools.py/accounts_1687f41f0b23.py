# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\coinbase\tools\accounts.py
import logging
...
async def coinbase_get_transactions(
    ...
    return await make_coinbase_request(method="GET", endpoint=endpoint, query_params=params)
...
async def coinbase_get_portfolio_value() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"data": {"accounts": account_values, "total_accounts": len(accounts)}}
    except Exception as e:
        logger.error(f"Unexpected error in Coinbase portfolio value request: {e}")
        return {"error": f"Unexpected error in Coinbase portfolio value request: {str(e)}"}
