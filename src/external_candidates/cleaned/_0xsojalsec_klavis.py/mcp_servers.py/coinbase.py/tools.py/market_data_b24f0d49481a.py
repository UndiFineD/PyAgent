# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\coinbase\tools\market_data.py
import logging
...
async def coinbase_get_prices(symbols: List[str]) -> Dict[str, Any]:
    ...
    for symbol in symbols:
        try:
            ...
            result = await make_coinbase_request(method="GET", endpoint=endpoint, require_auth=False)

            if "error" in result:
                prices_data.append({"error": f"Could not get price for {symbol}: {result['error']}"})
            else:
                prices_data.append(result)
        except Exception as e:
            logger.error(f"Coinbase price request failed for {symbol}: {e}")
            prices_data.append({"error": f"Could not get price for {symbol}: {str(e)}"})
...
async def coinbase_get_current_exchange_rate(symbols: List[str]) -> Dict[str, Any]:
    ...
    for symbol in symbols:
        try:
            ...
            if "error" in result:
                exchange_rates_data.append({"error": f"Could not get exchange rate for {symbol}: {result['error']}"})
            else:
                exchange_rates_data.append(result)

        except Exception as e:
            logger.error(f"Coinbase exchange rate request failed for {symbol}: {e}")
            exchange_rates_data.append({"error": f"Could not get exchange rate for {symbol}: {str(e)}"})
