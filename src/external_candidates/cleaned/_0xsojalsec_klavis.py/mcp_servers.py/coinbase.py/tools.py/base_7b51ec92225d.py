# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\coinbase\tools\base.py
import logging
import os
...
def get_auth_token() -> str:
    ...
    try:
        ...
        if not token:
            ...
            if not token:
                raise RuntimeError("No Coinbase auth token found in context or environment")
...
def get_coinbase_config() -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        return {"base_url": base_url, "exchange_url": exchange_url}
    except Exception as e:
    ...
    try:
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            try:
                ...
                logger.error(f"Coinbase API request failed: {e.status} {e.message} for {method} {url}")
                return {"error": f"API request failed: {str(e)}"}
            except Exception as e:
                logger.error(f"Unexpected error occurred during Coinbase API request: {e}")
                return {"error": f"Unexpected error: {str(e)}"}
    except CoinbaseValidationError as e:
        raise e
    except aiohttp.ClientResponseError as e:
        logger.error(f"Coinbase API request failed: {e.status} {e.message} for {method} {url}")
        raise RuntimeError(f"Coinbase API Error ({e.status}): {e.message}") from e
    except Exception as e:
        logger.error(f"An unexpected error occurred during Coinbase API request: {e}")
