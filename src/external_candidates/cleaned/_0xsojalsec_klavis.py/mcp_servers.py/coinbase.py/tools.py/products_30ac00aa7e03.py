# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\coinbase\tools\products.py
import logging
import os
...
async def coinbase_get_product_details(product_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Could not get Coinbase product details for {product_id}: {str(e)}"}


@rate_limited(api_type="products")
async def coinbase_get_historical_prices(
    symbol: str, start: str, end: str, granularity: Optional[int] = 3600
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        params = [f"start={start}", f"end={end}", f"granularity={granularity}"]
        ...
        return {"error": f"Could not get Coinbase historical prices for {symbol}: {str(e)}"}
