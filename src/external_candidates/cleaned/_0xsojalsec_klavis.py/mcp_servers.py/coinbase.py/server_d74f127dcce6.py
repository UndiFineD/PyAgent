# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\coinbase\server.py
import base64
import contextlib
import json
import logging
...
@click.command()
@click.option("--port", default=COINBASE_MCP_SERVER_PORT, help="Port to listen on for HTTP")
@click.option(
    "--log-level",
    default="INFO",
    ...
    async def list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "COINBASE_MARKET"}),
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                annotations=types.ToolAnnotations(**{"category": "COINBASE_ACCOUNT"}),
            ),
            types.Tool(
    ...
    async def call_tool(
        ...
        if name == "coinbase_get_prices":
            ...
            try:
                ...
                result = await coinbase_get_historical_prices(symbol, start, end, granularity)
