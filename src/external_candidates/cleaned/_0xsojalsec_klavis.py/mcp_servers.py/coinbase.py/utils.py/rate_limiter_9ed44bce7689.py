# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp_servers\coinbase\utils\rate_limiter.py
import asyncio
import functools
import logging
import time
...
class RateLimiter:
    def __init__(self, max_requests_per_second: Optional[int] = None):
        self.max_requests_per_second = max_requests_per_second or config.default_max_requests_per_second
        self.token_bucket = TokenBucketRateLimiter(self.max_requests_per_second)
    ...
    async def with_retry(self, operation: Callable, context: str = "API request") -> Any:
        ...
        while attempt <= config.max_retry_attempts:
            try:
                ...
                if self._is_rate_limited():
                    ...
                    logger.warning(f"Rate limit active for {context}. Waiting {wait_time:.2f}s")
        ...
        raise Exception(f"Max retry attempts ({config.max_retry_attempts}) exceeded for {context}")
...
def get_rate_limiter(api_type: str = "default", max_requests_per_second: Optional[int] = None) -> RateLimiter:
    """
    ...
    if api_type not in _rate_limiters:
        _rate_limiters[api_type] = RateLimiter(max_requests_per_second=max_requests_per_second)

    return _rate_limiters[api_type]


def rate_limited(api_type: str = "default", max_requests_per_second: Optional[int] = None):
    """
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            rate_limiter = get_rate_limiter(api_type=api_type, max_requests_per_second=max_requests_per_second)
