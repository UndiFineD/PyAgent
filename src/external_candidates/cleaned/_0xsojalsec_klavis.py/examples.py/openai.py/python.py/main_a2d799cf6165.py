...
def stream_chat_completion(
    ...
    """
    try:
        tools_info = klavis_client.mcp_server.list_tools(server_url=server_url, format=ToolFormat.OPENAI)
        ...
        if is_tool_call and tool_calls:
            ...
            print("\n🤖 Assistant: ", end="", flush=True)
            final_stream = client.chat.completions.create(model="gpt-4o-mini", messages=messages, stream=True)
...
def main():
    ...
    mcp_instance = klavis_client.mcp_server.create_server_instance(
        ...
        user_id="1234",  # Close CRM as an example
    )
