# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\examples\crewai\python\salesforce-gmail.py
"""
...
def multi_server_crew():
    ...
    try:
        ...
        salesforce_mcp_instance = klavis_client.mcp_server.create_server_instance(
            server_name=McpServerName.SALESFORCE, user_id="1234"
        )
        ...
        salesforce_mcp_server = MCPServerAdapter(
            {"url": salesforce_mcp_instance.server_url, "transport": "streamable-http"}
        )

        gmail_mcp_server = MCPServerAdapter({"url": gmail_mcp_instance.server_url, "transport": "streamable-http"})
