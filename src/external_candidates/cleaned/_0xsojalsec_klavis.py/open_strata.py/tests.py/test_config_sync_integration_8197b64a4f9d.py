# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\tests\test_config_sync_integration.py
...
class TestConfigSyncIntegration:
    ...
    async def test_config_change_triggers_sync(self, mock_transport, mock_client):
        ...
        try:
            ...
            watch_task = asyncio.create_task(
                manager.server_list.watch_config(lambda servers: asyncio.create_task(manager.sync_with_config(servers)))
            )
    ...
    @patch("strata.mcp_client_manager.MCPClient")
    @patch("strata.mcp_client_manager.StdioTransport")
    async def test_config_enable_disable_triggers_sync(self, mock_transport, mock_client):
