# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\tests\test_cli.py
...
class TestCLICommands:
    ...
    def test_remove_server(self, temp_config):
        ...
        test_server = MCPServerConfig(name="test-server", type="stdio", command="echo", args=["hello"])
    ...
    def test_list_servers(self, temp_config, capsys):
        ...
        server1 = MCPServerConfig(name="server1", type="sse", url="http://example.com/mcp", enabled=True)
        server2 = MCPServerConfig(name="server2", type="stdio", command="echo", args=["hello"], enabled=False)
    ...
    def test_cursor_existing_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            existing_config = {
                "mcpServers": {"existing-server": {"command": "some-command", "args": ["--flag"]}},
            ...
            try:
                ...
                assert config["mcpServers"]["existing-server"]["command"] == "some-command"
