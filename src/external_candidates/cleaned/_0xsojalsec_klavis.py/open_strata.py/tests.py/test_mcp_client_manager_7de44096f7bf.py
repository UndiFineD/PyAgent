# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\tests\test_mcp_client_manager.py
...
class TestMCPClientManager:
    ...
    async def test_initialize_from_config(self, mock_transport, mock_client, manager_with_config):
        ...
        mock_transport.assert_called_once_with(command="python", args=["-m", "test_server"], env={"TEST_VAR": "value1"})
    ...
    @patch("strata.mcp_client_manager.MCPClient")
    @patch("strata.mcp_client_manager.StdioTransport")
    async def test_disconnect_all(self, mock_transport, mock_client, manager_with_config):
    ...
    @patch("strata.mcp_client_manager.MCPClient")
    @patch("strata.mcp_client_manager.StdioTransport")
    async def test_list_active_servers(self, mock_transport, mock_client, manager_with_config):
    ...
    @patch("strata.mcp_client_manager.MCPClient")
    @patch("strata.mcp_client_manager.StdioTransport")
    async def test_reconnect_server(self, mock_transport, mock_client, manager_with_config):
...
class TestMCPClientManagerIntegration:
    ...
    async def test_use_real_tool(self, manager_with_github_server):
        ...
        user_tool = next((t for t in tools if "user" in t.get("name", "").lower()), None)

        if user_tool:
            # Try to call the tool
            try:
                result = await client.call_tool(user_tool["name"], {"username": "octocat"})
