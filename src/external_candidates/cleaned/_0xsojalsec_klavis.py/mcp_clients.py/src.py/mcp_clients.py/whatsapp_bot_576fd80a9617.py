# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\whatsapp_bot.py
import logging
import os
import time
...
class WhatsAppBot(BaseBot):
    ...
    async def send_message(self, context: WhatsAppBotContext, message: str) -> Any:
        """
        ...
        try:
            ...
            if len(message) > 4096:
                ...
                responses = []
                for part in parts:
                    response = await wa.send_message(to=context.user_id, text=part)
                    responses.append(response)
                return responses[-1]  # Return the last response
            else:
                return await wa.send_message(to=context.user_id, text=message)
        ...
        try:
            ...
            async for chunk in mcp_client.process_query_stream(
                ...
                if chunk.startswith("<special>") and chunk.endswith("\n"):
                    ...
                    special_content = (
                        chunk.replace("<special>", "").split(mcp_client.get_message_split_token())[0].strip()
                    )
                ...
                should_send_update = (
                    current_time - last_send_time >= update_interval and len(current_chunk) > 0
                ) or len(current_chunk) >= buffer_size
...
async def handle_message(client: WhatsApp, message: Message):
    ...
    try:
        ...
        context = WhatsAppBotContext(platform_name="whatsapp", user_id=message.from_user.wa_id, message=message)
        ...
        try:
            await client.send_message(to=message.from_user.wa_id, text=f"Sorry, an error occurred: {str(e)}")
        except:
            pass
