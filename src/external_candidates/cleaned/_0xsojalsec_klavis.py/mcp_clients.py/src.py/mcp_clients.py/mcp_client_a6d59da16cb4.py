...
class MCPClient:
    ...
    def _initialize_llm_client(self, api_name: str = None, provider: str = None) -> BaseLLM:
        """
    ...
    async def connect_to_server(
        ...
        try:
            ...
            if urlparse(url).scheme in ("http", "https"):
                ...
                server_parameters = StdioServerParameters(command=url, args=args, env=env)
                streams = await exit_stack.enter_async_context(stdio_client(server_parameters))
    ...
    async def refresh_tool_cache(self, server_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Updated cache with {len(server_tools)} tools from server {server_id}")
            return server_tools
        except Exception as e:
            logger.error(f"Error refreshing tool cache for server {server_id}: {str(e)}")
            return []

    async def get_tools_for_server(self, server_id: str, use_cache: bool = True) -> List[Dict[str, Any]]:
        """
    ...
    def is_final_response(self, messages: List[ChatMessage]) -> bool:
        ...
        return all(content.type != ContentType.TOOL_CALL for message in messages for content in message.content)
    ...
    async def process_query_stream(
        ...
        while has_tool_call:
            ...
            last_chat_messages = self.llm_client.to_chat_messages([last_provider_message])
            ...
            for message in last_chat_messages:
                for content in message.content:
                    if content.type == ContentType.TOOL_CALL:
                        ...
                        tool_result_content.append(ToolResultContent(tool_call_id=content.tool_id, result=result_text))

            # Add user message with tool results
            if tool_result_content:
                chat_messages.append(ChatMessage(role=MessageRole.TOOL, content=tool_result_content))

        if store_new_messages_callback:
            start_time = time.time()
            await store_new_messages_callback(self.conversation.id, chat_messages[messages_history_len - 1 :])
    ...
    async def cleanup(self):
        ...
        for server_id in list(self.exit_stacks.keys()):
            try:
                ...
                logger.error(f"Error cleaning up resources for server {server_id}: {str(e)}")
