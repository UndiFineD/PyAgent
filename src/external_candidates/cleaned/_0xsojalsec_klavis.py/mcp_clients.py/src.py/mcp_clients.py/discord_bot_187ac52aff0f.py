# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\discord_bot.py
import asyncio
import logging
import os
...
import discord
from discord import Color, DMChannel, Embed, Thread
...
class DiscordBotContext(BotContext):
    ...
    def __init__(
        ...
        """
        super().__init__(platform_name=platform_name, user_id=user_id, user_message=user_message)
    ...
    def get_thread_id(self) -> Optional[str]:
        ...
        """
        if self.user_message and hasattr(self.user_message, "thread") and self.user_message.thread:
...
class DiscordBot(BaseBot):
    ...
    async def on_message(self, message):
        ...
        if not is_dm:
            ...
            if isinstance(message.channel, Thread):
                ...
                thread = await message.create_thread(name=message.content[:60].replace(f"<@{self.client.user.id}>", ""))
        ...
        try:
            ...
            async with asyncio.timeout(200), self.user_locks[user_id]:
                ...
                mcp_client = await self.initialize_mcp_client(context=context, server_urls=server_urls)
    ...
    def _setup_commands(self):
        ...
        @self.client.tree.command(name="welcome", description="Display the welcome message with buttons")
    ...
    def create_login_view(
        ...
        login_url = (
            f"{login_url}?platform=discord&external_user_id={platform_user_id}&external_username={encoded_username}"
        )

        # Add URL buttons
        login_button = discord.ui.Button(style=discord.ButtonStyle.primary, label="Login", emoji="🔐", url=login_url)
        ...
        try:
            ...
            async for message in discord_context.channel.history(limit=limit + 1):
                ...
                if message.author.id == self.client.user.id:
                    ...
                    content.append(TextContent(text=f"User {username} says: {message.content}"))
