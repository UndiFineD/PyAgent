# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\slack_bot.py
import asyncio
import logging
import os
...
class SlackBot(BaseBot):
    ...
    def __init__(self):
        ...
        self.app: AsyncApp = AsyncApp(signing_secret=settings.SLACK_SIGNING_SECRET, authorize=self.authorize)
    ...
    @staticmethod
    async def authorize(
        enterprise_id: Optional[str], team_id: Optional[str], logger: logging.Logger
    ...
    async def send_message(self, context: SlackBotContext, message: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            message_params = {"channel": context.channel_id, "text": message}
    ...
    async def _create_special_message_blocks(self, special_content: str) -> tuple:
        ...
        if special_content.startswith("[Calling tool"):
            ...
            blocks = [
                ...
                {
                    "type": "context",
                    "elements": [{"type": "mrkdwn", "text": waiting_message}],
                },
                {"type": "divider"},
            ]
            ...
            blocks = [
                ...
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": special_content.strip("[]")},
                },
                {
                    "type": "context",
                    "elements": [
    ...
    async def get_messages_history(
        self, conversation: Conversation, slack_context: SlackBotContext, limit: int = 6
    ) -> List[ChatMessage]:
        """
        ...
        try:
            ...
            params = {"channel": slack_context.channel_id, "limit": limit + 1}
            ...
            if response["ok"]:
                ...
                for message in messages:
                    ...
                    if is_bot_message:
                        ...
                        content.append(TextContent(text=message.get("text", "")))
                    else:
                        ...
                        try:
                            ...
                            username = user_id
                        content.append(TextContent(text=f"User {username} says: {message.get('text', '')}"))
