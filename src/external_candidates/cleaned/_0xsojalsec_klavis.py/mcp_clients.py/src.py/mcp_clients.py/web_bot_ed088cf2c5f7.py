# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\web_bot.py
import asyncio
import json
import logging
        ...
        try:
            async with asyncio.timeout(200), web_bot.user_locks[user_id]:
                try:
                    ...
                    mcp_client = await web_bot.initialize_mcp_client(context=context, server_urls=server_urls)
                    messages_history = await web_bot.get_messages_history(
                        conversation=mcp_client.conversation,
                        context=context,
                    )

                    response = await web_bot.process_query_with_streaming(mcp_client, messages_history, context)
