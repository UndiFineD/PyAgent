# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\base_bot.py
import json
import logging
import os
import time
...
from mcp_clients.llms.base import ChatMessage, Conversation
...
class BaseBot(ABC):
    ...
    async def verify_user(self, context: BotContext) -> Dict[str, Any]:
        ...
        if USE_PRODUCTION_DB:
            verification_result = await database.get_user_connection_information(context.platform_name, context.user_id)
            logger.info(f"Verify user took {time.time() - start_time} seconds to complete")

            if verification_result["error"]:
                await self.send_message(context, f"Error verifying user: {verification_result['error']}")
        else:
        ...
        if USE_PRODUCTION_DB:
            ...
            if result["error"]:
                await self.send_message(
                    context,
                    f"Error checking and updating usage limit: {result['error']}",
                )
                return False
    ...
    async def get_server_urls(self, context: BotContext) -> List[str]:
        ...
        if USE_PRODUCTION_DB:
            mcp_servers = await database.get_connected_mcp_servers(context.mcp_client_id)
            logger.info(f"Get server urls took {time.time() - start_time} seconds to complete")
            if mcp_servers["error"]:
                await self.send_message(context, f"Error getting connected servers: {mcp_servers['error']}")
                return []
            ...
            for server in mcp_servers["servers"]:
                if server["instance_id"]:
                    server_urls.append(f"{server['server_url']}/sse?instance_id={server['instance_id']}")
                else:
    ...
    async def initialize_mcp_client(self, context: BotContext, server_urls: List[str] = None) -> Any:
        """
        ...
        if USE_PRODUCTION_DB:
            ...
            if conversation_result["error"]:
                await self.send_message(
                    context,
                    f"Error creating conversation: {conversation_result['error']}",
                )
                return
        else:
        ...
        mcp_client = MCPClient(self.platform_name, api_name, provider, conversation_result["conversation"])
    ...
    async def store_new_messages(self, conversation_id: str, messages: List[ChatMessage]) -> None:
        """
