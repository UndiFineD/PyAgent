# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\llms\anthropic.py
import json
import logging
import uuid
...
class Anthropic(BaseLLM):
            ...
            async for chunk in stream:
                ...
                if chunk.type == "message_start":
                    ...
                    if delta.type == "text_delta":
                        ...
                        yield delta.text
                    elif delta.type == "input_json_delta" and block_index in current_tool_calls:
                        # Accumulate JSON for tool input
                        current_tool_calls[block_index]["partial_json"] += delta.partial_json

                elif chunk.type == "content_block_stop":
                    block_index = chunk.index
                    if current_text:
                        message["content"].append({"type": "text", "text": current_text})
                        current_text = ""
                    ...
                    if hasattr(chunk, "delta") and hasattr(chunk.delta, "stop_reason"):
                        logger.info(f"Message delta received with stop_reason: {chunk.delta.stop_reason}")
    ...
    async def non_streaming_response(self, messages: list, available_tools: Optional[list] = None) -> Dict[str, Any]:
        ...
        # Prepare request parameters
        system_message = ""
        request_messages = messages.copy()  # Make a copy to avoid modifying the original
                ...
                for content_item in anthropic_content:
                    ...
                    # Handle object content types
                        ...
                        # Handle plain text content in object format
                            ...
                            content_list.append(TextContent(text=content_item.get("text", "")))
