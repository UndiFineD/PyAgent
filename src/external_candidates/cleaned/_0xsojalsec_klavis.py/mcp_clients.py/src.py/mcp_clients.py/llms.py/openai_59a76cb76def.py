# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\llms\openai.py
import logging
import time
import uuid
...
class OpenAI(BaseLLM):
    ...
    async def create_streaming_generator(
        ...
        if system_message_content and not messages[0].get("role") == "system":
            request_params["messages"].insert(0, {"role": "system", "content": system_message_content})
        ...
        try:
            ...
            if assistant_message.get("content") or assistant_message.get("tool_calls"):
                ...
                logger.warning("OpenAI stream finished without generating content or tool calls.")
    ...
    async def non_streaming_response(self, messages: list, available_tools: Optional[list] = None) -> Dict[str, Any]:
        ...
        if self.platform and system_message_content:
            if not request_messages or request_messages[0].get("role") != "system":
                request_messages.insert(0, {"role": "system", "content": system_message_content})
                logger.info("Prepended system message for OpenAI request.")
            else:
                logger.warning("System message provided but message history already starts with one. Using existing.")
        ...
        try:
            ...
            logger.error(f"Error in non-streaming OpenAI request: {str(e)}", exc_info=True)
            raise
    ...
    def to_chat_messages(raw_messages: List[Dict[str, Any]]) -> List[ChatMessage]:
        ...
        for openai_message in raw_messages:
                    ...
                    if tool_call.get("type") == "function":
                        ...
                        if isinstance(arguments, str):
                            ...
                            try:
                                ...
                                logger.error(f"Error parsing tool arguments {arguments} JSON: {e}")
            ...
            # Handle tool results
            if role == MessageRole.TOOL and openai_message.get("content") and openai_message.get("tool_call_id"):
                content_list.append(
                    ToolResultContent(
    ...
    def from_chat_messages(chat_messages: List[ChatMessage]) -> List[Dict[str, Any]]:
        ...
        for chat_message in chat_messages:
                ...
                openai_messages.append(
                    {
                        ...
                        "content": "".join([content.text for content in chat_message.content]),
                    }
                )
            ...
            if chat_message.role == MessageRole.USER:
                ...
                openai_messages.append({"role": chat_message.role.value, "content": str_content})
            ...
            if chat_message.role == MessageRole.ASSISTANT:
                for content in chat_message.content:
                    if content.type == ContentType.TEXT and content.text:
                        openai_messages.append({"role": chat_message.role.value, "content": content.text})
