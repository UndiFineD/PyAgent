# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\mcp-clients\src\mcp_clients\llms\base.py
import logging
import uuid
...
class BaseLLMConfig(BaseModel):
    ...
    model: Optional[str] = Field(default=None, description="The name of the model to use.")
    api_key: Optional[str] = Field(default=None, description="API key for the LLM provider.")
    temperature: float = Field(default=0, ge=0, le=2, description="Randomness in generation (0 to 2).")
    max_tokens: int = Field(default=3000, gt=0, description="The maximum number of tokens to generate.")
...
class FileContent(BaseModel):
    ...
    auth_token: str = Field(default=None, description="Authentication token for the file.")
...
class ChatMessage(BaseModel):
    ...
    role: MessageRole = Field(description="Role of the message sender (system, user, assistant, or tool).")
    content: List[Union[TextContent, ToolCallContent, ToolResultContent, FileContent]] = Field(
        ...
        if self.role == MessageRole.TOOL:
            for item in self.content:
                if not isinstance(item, ToolResultContent):
                    raise ValueError("When role is 'tool', content can only contain ToolResultContent objects")
            ...
            for item in self.content:
                if not isinstance(item, TextContent):
                    raise ValueError(f"When role is '{self.role}', content can only contain TextContent objects")
...
class Conversation(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(default={}, description="Additional metadata associated with the conversation.")
    created_at: datetime = Field(
        default_factory=datetime.now,
...
class BaseLLM(ABC):
    ...
    def set_platform(self, platform: str):
        ...
        if platform and platform.lower() in PLATFORM_CONFIGS:
            ...
            self.platform_config = PLATFORM_CONFIGS["default"]
            logger.info(f"Platform '{platform}' not recognized, using default configuration")
    ...
    @abstractmethod
    async def non_streaming_response(self, messages: list, available_tools: Optional[list] = None) -> Dict[str, Any]:
        """
