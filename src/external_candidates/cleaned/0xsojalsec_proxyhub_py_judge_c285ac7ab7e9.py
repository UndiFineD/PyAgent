# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_proxyhub.py\proxyhub.py\judge_c285ac7ab7e9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-proxyhub\proxyhub\judge.py
...
class Judge:
    ...
    async def check(self, real_ext_ip):
        ...
        connector = aiohttp.TCPConnector(loop=self._loop, ssl=self.verify_ssl, force_close=True)

        try:
            timeout = aiohttp.ClientTimeout(total=self.timeout)

            async with (
                aiohttp.ClientSession(connector=connector, timeout=timeout, loop=self._loop) as session,
        ...
        if resp.status == 200 and real_ext_ip in page and rv in page:
            ...
            log.debug(
                (
                    "{j} is failed. HTTP status code: {code}; Real IP on page: {ip}; Version: {word}; Response: {page}"
                ).format(
                    j=self,
                    code=resp.status,
                    page=page,
