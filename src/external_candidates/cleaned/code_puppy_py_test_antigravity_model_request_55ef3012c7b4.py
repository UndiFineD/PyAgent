# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\plugins.py\test_antigravity_model_request_55ef3012c7b4.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_model_request.py
...
class TestRequestMethod:
    ...
    @pytest.mark.asyncio
    async def test_successful_request(self, mock_google_model, mock_httpx_client, model_request_params) -> None:
        ...
        messages = [ModelRequest(parts=[UserPromptPart(content="Hello", timestamp=datetime.now())])]
        ...
        with patch.object(mock_google_model, "prepare_request") as mock_prepare:
            ...
            with patch.object(mock_google_model, "_map_user_prompt") as mock_map:
                ...
                response = await mock_google_model.request(messages, settings, model_request_params)
    ...
    @pytest.mark.asyncio
    async def test_request_builds_tools_and_config(self, mock_google_model, mock_httpx_client, monkeypatch) -> None:
    ...
    @pytest.mark.asyncio
    async def test_request_missing_http_client(self, mock_google_model, model_request_params):
        ...
        with patch.object(mock_google_model, "prepare_request") as mock_prepare:
            ...
            with patch.object(mock_google_model, "_map_user_prompt") as mock_map:
                ...
                with patch.object(mock_google_model, "_get_client", side_effect=raise_client_error):
                    with pytest.raises(RuntimeError, match="underlying httpx client"):
                        await mock_google_model.request(messages, ModelSettings(), model_request_params)
    ...
    async def test_request_with_corrupted_signature_retry(
        ...
        mock_error_response.text = '{"error": {"message": "Corrupted thought signature."}}'
        ...
        messages = [ModelRequest(parts=[UserPromptPart(content="Test", timestamp=datetime.now())])]
    ...
    @pytest.mark.asyncio
    async def test_request_api_error(self, mock_google_model, mock_httpx_client, model_request_params) -> None:
        ...
        with patch.object(mock_google_model, "prepare_request") as mock_prepare:
            ...
            with patch.object(mock_google_model, "_map_user_prompt") as mock_map:
                ...
                with pytest.raises(RuntimeError) as exc_info:
                    await mock_google_model.request(messages, settings, model_request_params)

                assert "Antigravity API Error 500" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_request_empty_response(self, mock_google_model, mock_httpx_client, model_request_params) -> None:
