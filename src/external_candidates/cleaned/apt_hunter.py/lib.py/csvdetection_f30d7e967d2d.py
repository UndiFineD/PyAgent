    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list2:
                ...
                try:
                    if len(re.findall(".*user.*/add.*", row["Details"])) > 0:
                        ...
                        Security_events[0]["Date and Time"].append(
                            datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p").isoformat()
                        )
                        Security_events[0]["timestamp"].append(
                            datetime.timestamp(datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"))
                        )
                    ...
                    if (
                        ...
                        or Process_Command_Line[0].strip().lower().find("\\program data\\") > -1
                    ):
                        # print("test")
            ...
            if row["Event ID"] == "4726":
                ...
                Event_desc = (
                    "User ( " + Account_Name[0].strip() + " ) removed user " + "( " + Account_Name[1].strip() + " )"
                )
                Security_events[0]["Date and Time"].append(
                    datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p").isoformat()
                )
                ...
                try:
                    if Process_Command_Line[0] not in Executed_Process_Summary[0]["Process Name"]:
                        ...
                        Executed_Process_Summary[0]["Number of Execution"][
                            Executed_Process_Summary[0]["Process Name"].index(Process_Command_Line[0].strip())
                        ] = (
                            Executed_Process_Summary[0]["Number of Execution"][
                                Executed_Process_Summary[0]["Process Name"].index(Process_Command_Line[0].strip())
                            ]
            ...
            if row["Event ID"] == "517" or row["Event ID"] == "1102":
                ...
                print(
                        ...
                        Account_Name[0].strip()))
                """
                Event_desc = "Audit log cleared by user ( %s )" % (Account_Name[0].strip())
                Security_events[0]["Date and Time"].append(
                    datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p").isoformat()
                )
                Security_events[0]["timestamp"].append(
        ...
        for row in list:
            ...
            if row["Event ID"] == "106":
                ...
                if (
                    ...
                    and task_register.group(2).find("\\Microsoft\\Windows\\WindowsUpdate") != 0
                ):
                    # print("schedule task registered with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (task_register.group(2)))
                    Event_desc = "schedule task registered with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (
            ...
            if row["Event ID"] == "140":
                ...
                if (
                    ...
                    and task_update.group(2).find("\\Microsoft\\Windows\\WindowsUpdate") != 0
                ):
                    # print("schedule task updated with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (task_update.group(2)))
                    Event_desc = "schedule task updated with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (
                        task_update.group(2)
                    )
                else:
            ...
            if row["Event ID"] == "141":
                ...
                if (
                    ...
                    and task_delete.group(2).find("\\Microsoft\\Windows\\WindowsUpdate") != 0
                ):
                    # print("schedule task deleted with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (task_delete.group(2)))
                    Event_desc = "schedule task deleted with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (
                        task_delete.group(2)
                    )
                else:
                    # print("schedule task deleted with Name ( %s ) by user ( %s ) " % (
                    # task_delete.group(2), task_delete.group(1)))
                    Event_desc = "schedule task deleted with Name ( %s ) by user ( %s ) " % (
                        task_delete.group(2),
                        task_delete.group(1),
                    )
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if (row["Event ID"] == "7045" or row["Event ID"] == "601") and (
                row["Details"].strip().find("\\temp\\") > -1 or row["Details"].strip().find("\\tmp\\") > -1
            ):
                ...
                System_events[0]["Detection Rule"].append("Service Installed with executable in TEMP Folder ")
            ...
            if (
                ...
                and (Service_and_state[0][1].strip() == "stopped" or Service_and_state[0][1].strip() == "disabled")
            ):
                # print("##### " + row['Date and Time'] + " ####  ", end='')
...
def detect_events_powershell_operational_log(file_name="powershell-logs.csv", winevent=False):
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Details"].strip().find("\\temp\\") > -1 or row["Details"].strip().find("\\tmp\\") > -1:
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if (
                ...
                if len(Suspicious) > 0:
                    Event_desc = (
                        "Found  Suspicious PowerShell commands that include (" + ",".join(Suspicious) + ") in event "
                    )
                    Powershell_events[0]["Date and Time"].append(
                        datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p").isoformat()
                    )
                ...
                if len(Source_Network_Address) > 0:
                    ...
                    try:
                        if (
                            ...
                            and not IPAddress(Source_Network_Address[0][0].strip()).is_private()
                        ):
                            # print("##### " + row['Date and Time'] + " #### EventID=" + row['Event ID'].strip() + " ### Remote Desktop Services: Session logon succeeded: #### ", end='')
        ...
        for d in i.findall("{http://schemas.microsoft.com/win/2004/08/events/event}System"):
            if d.find("{http://schemas.microsoft.com/win/2004/08/events/event}EventID").text == "6":
                try:
                    print(
                        "##### "
                        + d.find("{http://schemas.microsoft.com/win/2004/08/events/event}TimeCreated").attrib[
                            "SystemTime"
                        ]
                        + " #### EventID= "
                        + d.find("{http://schemas.microsoft.com/win/2004/08/events/event}EventID").text
                        + "  ### connection is initiated using WinRM - Powershell remoting  ##### User with ID ("
                        + d.find("{http://schemas.microsoft.com/win/2004/08/events/event}Security").attrib["UserID"]
                        + ") is connecting from current machine ("
                        + d.find("{http://schemas.microsoft.com/win/2004/08/events/event}Computer").text
                        + ") to ("
                        + i.find("{http://schemas.microsoft.com/win/2004/08/events/event}EventData")
                        .find("{http://schemas.microsoft.com/win/2004/08/events/event}Data")
                        .text
...
def detect_events_Sysmon_log(file_name="sysmon-logs.csv", winevent=False):
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Event ID"] == "13" and (TargetObject[0].strip().find("psexec") > -1):
                """print("##### " + row[
            ...
            if (
                row["Event ID"] == "1"
                and (Image[0].strip().find("\\taskeng.exe") > -1 or Image[0].strip().find("\\svchost.exe") > -1)
            ...
            if row["Event ID"] == "1" and (
                ParentCommandLine[0].strip().find("WmiPrvSE.exe") > -1 or Image[0].strip().find("WmiPrvSE.exe") > -1
            ):
                Event_desc = (
                    "User ("
                    + User[0].strip()
            ...
            if row["Event ID"] == "1" and (
                ParentCommandLine[0].strip().find("w3wp.exe") > -1 or Image[0].strip().find("w3wp.exe") > -1
            ):
                Event_desc = (
