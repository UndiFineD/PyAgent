# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\tests.py\functional.py\test_small_iron_factory_c33442f9bdfc.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\functional\test_small_iron_factory.py
...
def test_basic_iron_smelting_chain(game):
    ...
    assert iron_plates > 0, "No iron plates produced after 60 seconds. Check fuel levels and connections."
...
def test_steel_smelting_chain(game):
    ...
    assert steel > 0, "No steel produced after 60 seconds. Check fuel levels and connections."
...
def test_build_iron_plate_factory(game):
    ...
    output_belt = game.connect_entities(
        Position(x=furnace_line_start.position.x, y=furnace_line_start.position.y + 2.5),
