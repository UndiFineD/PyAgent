# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\pi3\models\layers\attention.py
...
class FlashAttentionRope(AttentionRope):
    def forward(self, x: Tensor, attn_bias=None, xpos=None) -> Tensor:
        ...
        # Debug information
        # print(f"FlashAttentionRope: B={B}, N={N}, C={C}, q.dtype={q.dtype}, q.shape={q.shape}")

        if q.dtype == torch.bfloat16:
            # print("Attempting FLASH_ATTENTION backend")
            with nn.attention.sdpa_kernel(SDPBackend.FLASH_ATTENTION):
                x = scaled_dot_product_attention(q, k, v)
        else:
            # print("Attempting MATH/EFFICIENT_ATTENTION backend")
            with nn.attention.sdpa_kernel([SDPBackend.MATH, SDPBackend.EFFICIENT_ATTENTION]):
                x = scaled_dot_product_attention(q, k, v)
