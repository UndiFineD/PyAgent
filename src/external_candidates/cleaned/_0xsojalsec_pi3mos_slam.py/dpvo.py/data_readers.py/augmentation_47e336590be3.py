# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\dpvo\data_readers\augmentation.py
import numpy as np
import torch
...
class RGBDAugmentor:
    ...
    def spatial_transform(self, images, depths, poses, intrinsics):
        ...
        min_scale = np.log2(np.maximum((self.crop_size[0] + 1) / float(ht), (self.crop_size[1] + 1) / float(wd)))
