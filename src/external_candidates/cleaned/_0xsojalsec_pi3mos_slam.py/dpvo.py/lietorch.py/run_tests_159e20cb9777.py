# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\dpvo\lietorch\run_tests.py
import lietorch
import torch
...
def test_inv(Group, device="cuda"):
    """check X * X^{-1} == 0"""
    X = Group.exp(0.1 * torch.randn(2, 3, 4, 5, Group.manifold_dim, device=device).double())
    a = (X * X.inv()).log()
...
def test_exp_log_grad(Group, device="cuda", tol=1e-8):
    ...
    analytical, reentrant, correct_grad_sizes, correct_grad_types = get_analytical_jacobian((a,), fn(a))

    assert torch.allclose(analytical[0], torch.eye(D, device=device).double(), atol=tol)

    a = 0.2 * torch.randn(1, Group.manifold_dim, requires_grad=True, device=device).double()
    analytical, reentrant, correct_grad_sizes, correct_grad_types = get_analytical_jacobian((a,), fn(a))
