# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\scripts.py\patterns.py\candlestick_patterns_0b3a4dca1d79.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\patterns\candlestick_patterns.py
...
class CandlestickPatternDetector:
    ...
    def __init__(self, doji_threshold: float = 0.1, wick_body_ratio: float = 2.0):
        """
        ...
        logger.info(
            f"Initialized CandlestickPatternDetector (doji_threshold={doji_threshold}, wick_ratio={wick_body_ratio})"
        )
    ...
    def detect_engulfing_patterns(self, df: pd.DataFrame) -> List[Dict]:
        ...
        try:
            ...
            if (
                ...
                logger.debug(f"Bullish Engulfing detected: ratio={curr_body / (prev_body + 1e-10):.2f}")
            ...
            if (
                ...
                logger.debug(f"Bearish Engulfing detected: ratio={curr_body / (prev_body + 1e-10):.2f}")
