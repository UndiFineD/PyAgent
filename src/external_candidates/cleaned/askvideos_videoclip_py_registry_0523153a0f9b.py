# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\askvideos_videoclip.py\video_llama.py\common.py\registry_0523153a0f9b.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\AskVideos-VideoCLIP\video_llama\common\registry.py
...
class Registry:
    ...
    def register_builder(cls, name):
        ...
        def wrap(builder_cls):
            from video_llama.datasets.builders.base_dataset_builder import (
                BaseDatasetBuilder,
            )
            ...
            if name in cls.mapping["builder_name_mapping"]:
                raise KeyError(
                    "Name '{}' already registered for {}.".format(name, cls.mapping["builder_name_mapping"][name])
                )
    ...
    def register_task(cls, name):
        ...
        def wrap(task_cls):
            ...
            if name in cls.mapping["task_name_mapping"]:
                raise KeyError(
                    "Name '{}' already registered for {}.".format(name, cls.mapping["task_name_mapping"][name])
                )
