# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\theplasmak.py\faster_whisper.py\scripts.py\transcribe_20abf07719af.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\theplasmak\faster-whisper\scripts\transcribe.py
...
def main():
    ...
    if not args.quiet:
        if device == "cuda" and gpu_name:
            print(
                f"Loading model: {args.model} ({device}, {compute_type}) on {gpu_name}",
                file=sys.stderr,
            )

        else:
            print(
                f"Loading model: {args.model} ({device}, {compute_type})",
                file=sys.stderr,
            )
