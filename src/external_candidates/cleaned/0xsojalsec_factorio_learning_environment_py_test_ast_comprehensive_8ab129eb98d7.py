# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\tests.py\actions.py\test_ast_comprehensive_8ab129eb98d7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\actions\test_ast_comprehensive.py
...
""",
    ...
    assert "This should not execute" not in output_str, "Should not execute after return"
...
""",
    ...
    assert "Caught runtime error: New error" in output_str, "Raise with cause should work"
...
""",
    ...
    assert "Assertion failed: x should be 10" in str(result), "Assertion with message should work"
...
""",
    ...
    assert "Assertion failed without message: AssertionError" in str(result), "Assertion without message should work"
...
""",
    ...
    assert "Path join: a/b" in output_str or "Path join: a\\b" in output_str, "Dotted import should work"
...
""",
    ...
    assert "Inner modified x to 20" in output_str, "Should modify nonlocal in inner function"
...
""",
    ...
    assert "After *= 2: 84" in str(result3), "Multiple augmented assignments should persist"
...
""",
    ...
    assert "List after +=: [1, 2, 3, 4, 5]" in str(result4), "List augmented assignment should work"
...
""",
    ...
    assert "Mapped squares: [1, 4, 9, 16, 25]" in str(result), "Lambda with map should work"
...
""",
    ...
    assert "Filtered evens: [2, 4, 6, 8, 10]" in str(result), "Lambda with filter should work"
...
""",
    ...
    assert "Sorted by second element: [(3, 1), (1, 2), (5, 4)]" in str(result), "Lambda with sorted should work"
...
""",
    ...
    assert "Sum found: 0 + 3 = 3" in output_str or "Found 3" in output_str, "Return should work in nested loops"

    assert "Sum found" in output_str or "Found 7" in output_str, "Return should work for different values"
...
""",
    ...
    assert "Syntax error caught: SyntaxError" in str(result), "Syntax errors should still be caught"
...
""",
    ...
    assert "Error occurred" not in output_str, "Complex statements should execute without errors"
