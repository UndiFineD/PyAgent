# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\core.py\transport_orchestrator_142b2a00d32f.py
...
class TransportOrchestrator:
    ...
    def resolve_transport(
        ...
        if not profile:
            raise ValueError(f"Audio profile '{profile_name}' not found. Available: {list(self.profiles.keys())}")
    ...
    def _negotiate_formats(
        ...
        if provider_caps:
            provider_in_enc = self._select_encoding(pref_in_enc, provider_caps.input_encodings, "input")

            provider_out_enc = self._select_encoding(pref_out_enc, provider_caps.output_encodings, "output")

            provider_in_rate = self._select_sample_rate(pref_in_rate, provider_caps.input_sample_rates_hz, "input")

            provider_out_rate = self._select_sample_rate(pref_out_rate, provider_caps.output_sample_rates_hz, "output")
