...
class HunYuanImageRefinerPipeline(HunyuanImagePipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image_tensor = _pil_to_tensor(image).unsqueeze(0).to("cuda", dtype=self.vae.dtype)
        ...
        if hasattr(self.vae.config, "shift_factor") and self.vae.config.shift_factor:
            cond_latents.sub_(self.vae.config.shift_factor).mul_(self.vae.config.scaling_factor)
        else:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if model_name == "hunyuanimage-refiner":
            ...
            raise ValueError(f"Unsupported refiner model name: {model_name}. Supported names: 'hunyuanimage-refiner'")

        config = HunYuanImageRefinerPipelineConfig.create_default(version=version, **kwargs)
