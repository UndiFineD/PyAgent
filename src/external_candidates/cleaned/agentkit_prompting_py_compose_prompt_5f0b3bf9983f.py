# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentkit_prompting.py\examples.py\crafter.py\compose_prompt_5f0b3bf9983f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\examples\crafter\compose_prompt.py
...
class ComposeObservationReasoningPrompt(ComposeBasePrompt):
    ...
    def before_dependencies(self, messages, db):
        ...
        messages.append(
            {
                "role": "system",
                "content": "Most recent two steps of the player's in-game observation:\n\n{}".format(
                    db["environment"]["observation_2step"]
                ),
            }
        )
...
Finally, make sure that each planned items is both quantifiable and achievable with the game actions, and the requirements are well-addressed."""
    ...
    def before_dependencies(self, messages, db):
        skill_feedback = (
            db["feedback"]["skill_feedback"][db["skills"]["skill"]]
            if db["skills"]["skill"] in db["feedback"]["skill_feedback"].keys()
            else ""
        )
        ...
        messages.append(
            {
                "role": "system",
                "content": "Gameplay history for the previous {} steps:\n\n{}".format(len(qa_history), text_desc),
            }
        )
        ...
        if len(db["history"]["qa_history"]) > 0:
            strategy_desc = "\n\n".join(
                [
                    "## {}\n{}".format(d, db["history"]["qa_history"][-1][q])
                    for q, d in db["prompts"]["strategy_questions_desc"].items()
                ]
            )
...
def print_skill_library(skill_library):
    output = {k: v["skill_desc"] for k, v in skill_library.items()}
...
class ComposeActorPlannerPrompt(ComposeBasePrompt):
    ...
    def before_dependencies(self, messages, db):
        ...
        messages.append(
            {
                "role": "system",
                "content": "Most recent two steps of in-game observation\n\n{}".format(
                    db["environment"]["observation_2step_with_action"]
                ),
            }
        )
    ...
    def after_dependencies(self, messages, db):
        if db["skills"]["skill"] is not None and db["skills"]["skill"] == db["skills"]["skill_old"]:
            ...
            messages.append(
                {
                    "role": "system",
                    "content": "Notes from the last step:\n\n{}".format(db["action_notes"]),
                }
            )
            ...
            messages.append(
                {
                    "role": "system",
                    "content": "Previous plan: Not available, please re-plan.",
                }
            )
...
def compose_filtered_gamestep_prompt(
    qa_history,
    attention_rounds,
    game_questions,
    plan_questions=None,
...
def compose_feedback_prompt(CTXT_dict, qa_history, current_objective, questions):
    ...
    messages_feedback = [
        {
            "role": "system",
            "content": "Provide concrete feedback for the player using concise language. The feedback must be on the current skill.",
        }
    ]
    ...
    for question, dependencies, shorthand, filtered in questions:
        if filtered:
            text_desc = compose_filtered_gamestep_prompt(
                qa_history, attention_rounds, dependencies, plan_questions=None, db=db
            )
...
def compose_gameplay_prompt(CTXT_dict, qa_history, Q_CTXT, question):
    ...
    messages = [
        {
            "role": "system",
            "content": "The player just finished playing the game of crafter. Provide concrete feedback for the player's gameplay using very concise language.",
        }
    ]
    ...
    text_desc = compose_filtered_gamestep_prompt(
        qa_history,
        attention_rounds,
        db["prompts"]["gamestep_questions"],
        plan_questions=db["prompts"]["gameplan_questions"],
        db=db,
    )
