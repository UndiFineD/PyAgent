# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_mahilo.py\mahilo.py\message_store_e8a3b9b81851.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mahilo\mahilo\message_store.py
...
class SQLiteMessageStore(MessageStore):
    ...
    def get_retry_count(self, message_id: str) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute("SELECT retry_count FROM messages WHERE message_id = ?", (message_id,)).fetchone()
