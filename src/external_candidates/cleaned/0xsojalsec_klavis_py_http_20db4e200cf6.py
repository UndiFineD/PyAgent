# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\open_strata.py\src.py\strata.py\mcp_proxy.py\transport.py\http_20db4e200cf6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\src\strata\mcp_proxy\transport\http.py
...
class HTTPTransport(Transport):
    ...
    async def _get_streams(self, exit_stack: AsyncExitStack) -> Tuple:
        ...
        if self.mode == "sse":
            ...
            if self.auth == "oauth":
                return await exit_stack.enter_async_context(
                    sse_client(
                        ...
                        auth=create_oauth_provider(self.server_name, self.url),
                    )
                )

            return await exit_stack.enter_async_context(sse_client(self.url, headers=self.headers))
            ...
            return await exit_stack.enter_async_context(streamablehttp_client(self.url, headers=self.headers))

        else:
            raise ValueError(f"Invalid transport mode: {self.mode}. Use 'http' or 'sse'.")
