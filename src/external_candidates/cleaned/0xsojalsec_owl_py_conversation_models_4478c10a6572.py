# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_owl.py\owl.py\camel.py\messages.py\conversion.py\conversation_models_4478c10a6572.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\messages\conversion\conversation_models.py
...
class ShareGPTMessage(BaseModel):
    ...
    from_: Literal["human", "gpt", "system", "tool"] = Field(alias="from", description="The role of the message sender")
        ...
        for i in range(1, len(messages)):
            ...
            if curr.from_ == "tool":
                if prev.from_ != "gpt" or "<tool_call>" not in prev.value:
                    raise ValueError(f"Tool response at position {i} must follow an gpt message with a tool call")

            if curr.from_ == "gpt" and prev.from_ not in (
                "human",
                "tool",
            ):
                raise ValueError(f"Assistant message at position {i} must follow a human or tool message")
...
class ToolResponse(BaseModel):
    ...
    content: Any = Field(description="The response content from the tool. Must be JSON serializable literal or object")
