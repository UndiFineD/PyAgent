# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\infrastructure\aws\s3.py
import os
import tempfile
import zipfile
...
class S3Client:
    ...
    def upload_folder(self, local_path: Union[str, Path], s3_prefix: str = "") -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".zip", delete=False) as temp_zip:
            ...
            # Upload zip file
            logger.debug(f"Uploading {local_path} to {self.bucket_name} with key {s3_key}")
        ...
        try:
            ...
            if error_code == "404":
                try:
                    ...
                    raise Exception(f"Failed to create bucket {self.bucket_name}: {str(create_error)}")
            elif error_code == "403":
