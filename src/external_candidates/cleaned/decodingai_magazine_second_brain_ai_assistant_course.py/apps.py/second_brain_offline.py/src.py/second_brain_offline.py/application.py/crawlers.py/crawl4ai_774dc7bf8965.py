# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\application\crawlers\crawl4ai.py
import asyncio
import os
...
class Crawl4AICrawler:
    ...
    async def __crawl_batch(self, pages: list[Document]) -> list[Document]:
        ...
        async with AsyncWebCrawler(cache_mode=CacheMode.BYPASS) as crawler:
            for page in pages:
                tasks = [self.__crawl_url(crawler, page, url, semaphore) for url in page.child_urls]
        ...
        logger.info(
            f"Crawling completed: {success_count}/{total_count} succeeded ✓ | {failed_count}/{total_count} failed ✗"
        )
