# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\domain\dataset.py
import json
import random
...
class InstructDataset(BaseModel):
    ...
    def from_samples(
        ...
        train_samples = shuffled_samples[: int(len(shuffled_samples) * (1 - val_split_ratio - test_split_ratio))]
        ...
        test_samples = shuffled_samples[int(len(shuffled_samples) * (1 - test_split_ratio)) :]
