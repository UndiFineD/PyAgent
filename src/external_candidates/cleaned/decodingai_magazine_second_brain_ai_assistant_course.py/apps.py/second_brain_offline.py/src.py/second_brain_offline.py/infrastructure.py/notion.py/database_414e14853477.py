# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\infrastructure\notion\database.py
import json
...
class NotionDatabaseClient:
    ...
    def __init__(self, api_key: str | None = settings.NOTION_SECRET_KEY) -> None:
        ...
        assert api_key is not None, "NOTION_SECRET_KEY environment variable is required. Set it in your .env file."

        self.api_key = api_key

    def query_notion_database(self, database_id: str, query_json: str | None = None) -> list[DocumentMetadata]:
    ...
    def __build_page_metadata(self, page: dict[str, Any]) -> DocumentMetadata:
        ...
        return DocumentMetadata(id=page["id"], url=page["url"], title=title, properties=properties)
    ...
    def __flatten_properties(self, properties: dict) -> dict:
        ...
        for key, value in properties.items():
            ...
            if prop_type == "select":
                ...
                flattened[key] = " ".join(item.get("plain_text", "") for item in value.get("rich_text", []))
