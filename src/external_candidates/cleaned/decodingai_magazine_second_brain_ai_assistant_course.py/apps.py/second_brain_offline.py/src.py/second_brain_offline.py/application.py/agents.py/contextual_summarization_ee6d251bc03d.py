# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\src\second_brain_offline\application\agents\contextual_summarization.py
import asyncio
import os
...
"""
    ...
    def __call__(self, content: str, chunks: list[str]) -> list[str]:
        ...
        try:
            ...
            results = loop.run_until_complete(self.__summarize_context_batch(content, chunks))
    ...
    async def __summarize_context_batch(self, content: str, chunks: list[str]) -> list[str]:
        ...
        summarized_documents = await self.__process_batch(documents, await_time_seconds=7)
        documents_with_summaries = [doc for doc in summarized_documents if doc.contextual_summarization is not None]
        documents_without_summaries = [doc for doc in documents if doc.contextual_summarization is None]
        ...
        if documents_without_summaries:
            ...
            retry_results = await self.__process_batch(documents_without_summaries, await_time_seconds=20)
    ...
    async def __process_batch(
        ...
        tasks = [
            self.__summarize_context(document, semaphore, await_time_seconds=await_time_seconds)
                    ...
                    messages=[
                        {
                            ...
                            "content": self.SYSTEM_PROMPT_TEMPLATE.format(
                                ...
                                content=document.content[:6000],  # Keep it short to lower latency and costs.
...
"""
    ...
    async def __summarize_context_batch(self, content: str, chunks: list[str]) -> list[str]:
        ...
        process = psutil.Process(os.getpid())
        start_mem = process.memory_info().rss
        logger.debug(f"Starting summarizing document.Initial memory usage: {start_mem // (1024 * 1024)} MB")

        document = await self.__summarize(document=ContextualDocument(content=content), await_time_seconds=20)
