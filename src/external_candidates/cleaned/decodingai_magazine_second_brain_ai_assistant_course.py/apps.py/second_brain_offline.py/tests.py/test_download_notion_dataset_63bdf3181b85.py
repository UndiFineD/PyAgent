# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\tests\test_download_notion_dataset.py
import json
...
class TestDownloadNotionDataset:
    ...
    def test_database_directories_exist(self, notion_dir: Path) -> None:
        ...
        for dir_path in database_dirs:
            assert dir_path.name.startswith("database_"), f"Invalid directory name format: {dir_path.name}"
    ...
    def test_json_files_in_databases(self, notion_dir: Path) -> None:
        ...
        for database_dir in database_dirs:
            ...
            assert len(non_empty_json_files) > 0, f"No non-empty JSON files found in {database_dir}"
