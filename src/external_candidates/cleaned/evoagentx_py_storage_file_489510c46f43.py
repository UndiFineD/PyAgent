# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\tools.py\storage_file_489510c46f43.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\tools\storage_file.py
                ...
                if isinstance(content, list):
                    ...
                    try:
                        ...
                        if not isinstance(parsed_content, list):
                            return {
                                "success": False,
                                "error": "Excel content must be a list of lists",
                            }

                    except json.JSONDecodeError:
                        return {
                            "success": False,
                            "error": "Excel content must be valid JSON array",
                        }
...
class CopyTool(Tool):
    ...
    inputs: Dict[str, Dict[str, str]] = {
        "source": {"type": "string", "description": "Source path of the file to copy"},
        "destination": {
            "type": "string",
...
class StorageToolkit(Toolkit):
    ...
    def __init__(
        self,
        name: str = "StorageToolkit",
        base_path: str = "./workplace/storage",
        storage_handler: LocalStorageHandler = None,
    ):
        """
