# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_praisonai.py\src.py\praisonai_agents.py\praisonaiagents.py\knowledge.py\chunking_dee813b46b12.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\knowledge\chunking.py
...
class Chunking:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if chunker_type not in self.CHUNKER_PARAMS:
            raise ValueError(
                f"Unsupported chunker type: {chunker_type}. Must be one of: {list(self.CHUNKER_PARAMS.keys())}"
            )
    ...
    def embedding_model(self):
        ...
        if self._embedding_model is None and self.chunker_type in [
            "semantic",
            "sdpm",
            "late",
        ]:
