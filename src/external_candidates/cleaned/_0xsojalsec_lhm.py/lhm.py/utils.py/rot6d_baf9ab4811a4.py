# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\utils\rot6d.py
...
def check_root_pose(root_pose):
    # 检查人物整体任意一个轴旋转不超过20度
    if not isinstance(root_pose, torch.Tensor):
        root_pose = torch.tensor(root_pose)
    PI = 3.1415926
    root_euler = axis_angle_to_euler_angles(root_pose)
    root_euler[0] = (root_euler[0] + 2 * PI) % (2 * PI) - PI

    if abs(root_euler[0]) <= PI / 9 and abs(root_euler[1]) < PI / 9 and abs(root_euler[2]) < PI / 9:
        return True
...
if __name__ == "__main__":
    with open(
        "/mnt/data/peihao/HumanVideoProcessing/train_data/deepfashion_processed/smplx_params/MEN-Denim-id_00000089-02_7_additional.json",
        "r",
    ) as f:
        smplx_param = json.load(f)

    orient_pose = torch.tensor(smplx_param["body_pose"][0])
    orient_euler = axis_angle_to_euler_angles(orient_pose)
    print(orient_euler)
