# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\runners\infer\human_lrm.py
# -*- coding: utf-8 -*-
...
class HumanLRMInferrer(Inferrer):
    ...
    def __init__(self):
        ...
        self.pose_estimator = PoseEstimator("./pretrained_models/human_model_files/", device=avaliable_device())
        try:
    ...
    def infer_video(
        ...
        render_bg_colors = torch.ones(N, render_cameras.shape[1], 1, device=self.device, dtype=torch.float32) * 1.0
    ...
    def infer_mesh(
        ...
        try:
            ...
            src_head_rgb = np.zeros((self.cfg.src_head_size, self.cfg.src_head_size, 3), dtype=np.uint8)

        src_head_rgb = torch.from_numpy(src_head_rgb / 255.0).float().permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
    ...
    def infer_single(
        ...
        motion_name = os.path.dirname(motion_seqs_dir[:-1] if motion_seqs_dir[-1] == "/" else motion_seqs_dir)
        ...
        for batch_i in range(0, camera_size, batch_size):
            with torch.no_grad():
                ...
                for key in keys:
                    batch_smplx_params[key] = motion_seq["smplx_params"][key][:, batch_i : batch_i + batch_size].to(
                        device
                    )
                ...
                res = self.model.animation_infer(
                    gs_model_list,
                    query_points,
                    batch_smplx_params,
