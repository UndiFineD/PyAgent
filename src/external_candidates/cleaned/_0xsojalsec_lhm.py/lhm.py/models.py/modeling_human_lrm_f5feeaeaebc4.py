# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\modeling_human_lrm.py
# -*- coding: utf-8 -*-
...
class ModelHumanLRM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.latent_query_points_type == "embedding":
            ...
            print(f"==========load smplx points:{latent_query_points_file}, shape:{pcl_embeddings.shape}")
    ...
    def forward_latent_points(self, image, camera, query_points=None):
        ...
        tokens = self.forward_transformer(image_feats, camera_embeddings=None, query_points=query_points)
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert image.shape[0] == smplx_params["betas"].shape[0], "Batch size mismatch for image and smplx_params"
        assert image.shape[0] == smplx_params["body_pose"].shape[0], "Batch size mismatch for image and smplx_params"
        ...
        if self.latent_query_points_type.startswith("e2e_smplx"):
            query_points, smplx_params = self.renderer.get_query_points(smplx_params, device=image.device)
        ...
        assert render_results["comp_rgb"].shape[0] == N, "Batch size mismatch for render_results"
        assert render_results["comp_rgb"].shape[1] == M, (
...
class ModelHumanLRMSapdinoBodyHeadSD3_5(ModelHumanLRM):
    ...
    def obtain_facesr(self, head_image):
        ...
        def image_to_tensor(head_image_numpy):
            ...
            head_image_tensor = torch.from_numpy(head_image_numpy.copy()).permute(0, 3, 1, 2).float()
    ...
    def forward_latent_points(self, image, head_image, camera, query_points=None):
        ...
        assert image_feats.shape[-1] == self.fine_encoder_feat_dim, (
    ...
    def forward_transformer(self, image_feats, camera_embeddings, query_points, motion_embed=None):
        """
        Applies forward transformation to the input features.
        Args:
            image_feats (torch.Tensor): Input image features. Shape [B, C, H, W].
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert image.shape[0] == render_c2ws.shape[0], "Batch size mismatch for image and render_c2ws"
        assert head_image.shape[0] == render_c2ws.shape[0], "Batch size mismatch for image and render_c2ws"
        assert image.shape[0] == render_bg_colors.shape[0], "Batch size mismatch for image and render_bg_colors"
        assert image.shape[0] == smplx_params["betas"].shape[0], "Batch size mismatch for image and smplx_params"
        assert image.shape[0] == smplx_params["body_pose"].shape[0], "Batch size mismatch for image and smplx_params"
        assert len(smplx_params["betas"].shape) == 2
