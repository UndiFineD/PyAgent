# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\transformer_dit.py
# -*- coding: utf-8 -*-
...
class SD3BodyHeadMMJointTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states, body_encoder_hidden_states = self.body_dit(hidden_states, body_encoder_hidden_states, body_temb)

        if body_encoder_hidden_states is not None:
            encoder_hidden_states = torch.cat([body_encoder_hidden_states, head_encoder_hidden_states], dim=1)
        else:
            encoder_hidden_states = None
