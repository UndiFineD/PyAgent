# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\encoders\dinov2_unet.py
#!/usr/bin/env python
        ...
        self.resize_layers = nn.ModuleList(
            [
                nn.Sequential(
                    ...
                    nn.Conv2d(out_dims[0], out_dims[0], kernel_size=3, stride=1, padding=1),
                    nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True),
                    nn.Conv2d(out_dims[0], out_dims[0], kernel_size=3, stride=1, padding=1),
                ),
                nn.Sequential(
                ...
                nn.Sequential(nn.Conv2d(out_dims[2], out_dims[2], kernel_size=3, stride=1, padding=1)),
                nn.Sequential(nn.Conv2d(out_dims[3], out_dims[3], kernel_size=3, stride=2, padding=1)),
            ]
        )
