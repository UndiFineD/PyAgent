# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\encoders\sapiens_warpper.py
import functools
import gc
...
def feat_save(feature, output_path):
    pred_save_path = os.path.join(output_path.replace(".jpg", ".npy").replace(".jpeg", ".npy").replace(".png", ".npy"))
...
class SapiensWrapper(nn.Module):
    def __init__(
        ...
        self.register_buffer("mean", torch.Tensor([0.4844, 0.4570, 0.4062]), persistent=False)
        self.register_buffer("std", torch.Tensor([0.2295, 0.2236, 0.2256]), persistent=False)
    ...
    def _preprocess_image(self, image: torch.tensor, resolution: int = 1024) -> torch.Tensor:
