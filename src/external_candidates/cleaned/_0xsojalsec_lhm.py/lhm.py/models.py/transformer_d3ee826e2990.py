# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\transformer.py
# -*- coding: utf-8 -*-
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        if block_type == "sd3_cond" or block_type == "sd3_mm_cond" or block_type == "sd3_mm_bh_cond":
    ...
    def _block_fn(self, inner_dim, cond_dim, mod_dim):
        ...
        if self.block_type == "basic":
            assert cond_dim is None and mod_dim is None, f"Condition and modulation are not supported for BasicBlock"
            ...
            return partial(SD3MMJointTransformerBlock, dim=inner_dim, qk_norm="rms_norm")
        elif self.block_type == "sd3_mm_bh_cond":
            logger.debug(f"Using SD3MMJointTransformerBlock")
            from LHM.models.transformer_dit import SD3BodyHeadMMJointTransformerBlock

            return partial(SD3BodyHeadMMJointTransformerBlock, dim=inner_dim, qk_norm="rms_norm")
        else:
    ...
    def assert_runtime_integrity(self, x: torch.Tensor, cond: torch.Tensor, mod: torch.Tensor):
        ...
        if self.block_type == "basic":
            assert cond is None and mod is None, f"Condition and modulation are not supported for BasicBlock"
