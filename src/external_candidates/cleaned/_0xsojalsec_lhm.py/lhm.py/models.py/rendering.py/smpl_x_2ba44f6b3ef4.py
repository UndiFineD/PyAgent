# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\rendering\smpl_x.py
# -*- coding: utf-8 -*-
...
class SMPLX(object):
    def __init__(
        ...
        if shape_param_dim == 10 and expr_param_dim == 10:
            ...
            self.layer = {
                gender: self.get_expr_from_flame(self.layer[gender]) for gender in ["neutral", "male", "female"]
            }
        ...
        # SMPLX joint set
        self.joint_num = 55  # 22 (body joints: 21 + 1) + 3 (face joints) + 30 (hand joints)
        self.joints_name = (
            "Pelvis",
            "L_Hip",
            "R_Hip",
        ...
        self.face_upsampled = (
            self.subdivider_list[-1]._subdivided_faces.cpu().numpy() if self.subdivide_num > 0 else self.face
        )
    ...
    def upsample_mesh_batch(self, vert, device="cuda"):
        if self.subdivide_num > 0:
            face = torch.LongTensor(self.face).to(device).unsqueeze(0).repeat(vert.shape[0], 1, 1)
    ...
    def get_expr_vertex_idx(self):
        ...
        with open(osp.join(self.human_model_path, "flame", "2019", "generic_model.pkl"), "rb") as f:
            flame_2019 = pickle.load(f, encoding="latin1")
        vertex_idxs = np.where((flame_2019["shapedirs"][:, :, 300 : 300 + self.expr_param_dim] != 0).sum((1, 2)) > 0)[
            0
...
class SMPLXModel(nn.Module):
    ...
    def smplx_init(self):
        ...
        pose_dirs = pose_dirs.reshape(smpl_x.vertex_num_upsampled * 3, (smpl_x.joint_num - 1) * 9).permute(
            1, 0
        )  # (J * 9, V * 3)
    ...
    def get_neutral_pose_human(self, jaw_zero_pose, use_id_info, shape_param, device, face_offset, joint_offset):
        smpl_x = self.smpl_x
        batch_size = shape_param.shape[0]

        zero_pose = torch.zeros((batch_size, 3)).float().to(device)
        neutral_body_pose = smpl_x.neutral_body_pose.view(1, -1).repeat(batch_size, 1).to(device)  # 大 pose
        zero_hand_pose = torch.zeros((batch_size, len(smpl_x.joint_part["lhand"]) * 3)).float().to(device)
        zero_expr = torch.zeros((batch_size, smpl_x.expr_param_dim)).float().to(device)

        if jaw_zero_pose:
            jaw_pose = torch.zeros((batch_size, 3)).float().to(device)
        else:
            jaw_pose = smpl_x.neutral_jaw_pose.view(1, 3).repeat(batch_size, 1).to(device)  # open mouth
        ...
        if use_id_info:
            ...
            joint_offset = smpl_x.get_joint_offset(joint_offset) if joint_offset is not None else None

        else:
            shape_param = torch.zeros((batch_size, smpl_x.shape_param_dim)).float().to(device)
            face_offset = None
            joint_offset = None
        ...
        mesh_neutral_pose_upsampled = smpl_x.upsample_mesh_batch(output.vertices, device=device)

        mesh_neutral_pose = output.vertices
        joint_neutral_pose = output.joints[:, : smpl_x.joint_num, :]  # 大 pose human  [B, 55, 3]

        # compute transformation matrix for making 大 pose to zero pose
        neutral_body_pose = neutral_body_pose.view(batch_size, len(smpl_x.joint_part["body"]) - 1, 3)
        zero_hand_pose = zero_hand_pose.view(batch_size, len(smpl_x.joint_part["lhand"]), 3)
    ...
    def get_zero_pose_human(self, shape_param, device, face_offset, joint_offset, return_mesh=False):
        smpl_x = self.smpl_x
        batch_size = shape_param.shape[0]

        zero_pose = torch.zeros((batch_size, 3)).float().to(device)
        zero_body_pose = torch.zeros((batch_size, (len(smpl_x.joint_part["body"]) - 1) * 3)).float().to(device)
        zero_hand_pose = torch.zeros((batch_size, len(smpl_x.joint_part["lhand"]) * 3)).float().to(device)
        zero_expr = torch.zeros((batch_size, smpl_x.expr_param_dim)).float().to(device)
    ...
    def get_transform_mat_joint(self, transform_mat_neutral_pose, joint_zero_pose, smplx_param):
        ...
        transform_mat_joint = torch.matmul(transform_mat_joint_2, transform_mat_joint_1)  # [B, 55, 4, 4]
    ...
    def get_transform_mat_vertex(self, transform_mat_joint, nn_vertex_idxs):
        ...
        skinning_weight = skinning_weight.view(-1, skinning_weight.shape[-1])[nn_vertex_idxs.view(-1)].view(
    ...
    def get_posed_blendshape(self, smplx_param):
        ...
        pose = (axis_angle_to_matrix(pose) - torch.eye(3)[None, None, :, :].float().cuda()).view(
            batch_size, (self.smpl_x.joint_num - 1) * 9
        )
        # (B, 54 * 9) x (54*9, V)
    ...
    def lbs(self, xyz, transform_mat_vertex, trans):
        ...
        xyz = torch.cat((xyz, torch.ones_like(xyz[:, :, :1])), dim=-1)  # 大 pose. xyz1 [B, N, 4]
    ...
    def transform_to_posed_verts_from_neutral_pose(
        ...
        try:
            smplx_expr_offset = (smplx_data["expr"].unsqueeze(1).unsqueeze(1) * self.expr_dirs).sum(
                -1
        ...
        if self.apply_pose_blendshape:
            ...
            mask = ((self.is_rhand + self.is_lhand + self.is_face_expr) > 0).unsqueeze(0).repeat(batch_size, 1)
        ...
        nn_vertex_idxs = knn_points(mean_3d[:, :, :], mesh_neutral_pose[:, :, :], K=1, return_nn=True).idx[
            :, :, 0
        ...
        nn_vertex_idxs[mask] = (
            torch.arange(self.smpl_x.vertex_num_upsampled).to(device).unsqueeze(0).repeat(batch_size, 1)[mask]
        )
        ...
        transform_mat_joint, j3d = self.get_transform_mat_joint(transform_mat_neutral_pose, joint_zero_pose, smplx_data)

        # compute vertices-LBS function
        transform_mat_vertex = self.get_transform_mat_vertex(transform_mat_joint, nn_vertex_idxs)

        mean_3d = self.lbs(mean_3d, transform_mat_vertex, smplx_data["trans"])  # posed with smplx_param
    ...
    def get_query_points(self, smplx_data, device):
        ...
        mesh_neutral_pose, mesh_neutral_pose_wo_upsample, transform_mat_neutral_pose = self.get_neutral_pose_human(
            jaw_zero_pose=True,
            use_id_info=True,
            shape_param=smplx_data["betas"],
            device=device,
    ...
    def transform_to_posed_verts(self, smplx_data, device):
        ...
        # neutral posed verts
        mesh_neutral_pose, _, transform_mat_neutral_pose = self.get_query_points(smplx_data, device)
...
def read_smplx_param(smplx_data_root, shape_param_file, batch_size=1, device="cuda"):
    ...
    for smplx_param_path in smplx_param_path_list:
        ...
        with open(smplx_param_path) as f:
            smplx_params_all_frames[frame_idx] = {k: torch.FloatTensor(v) for k, v in json.load(f).items()}
    ...
    for b_idx in range(batch_size):
        ...
        with open(osp.join(data_root_path, "cam_params", str(frame_idx) + ".json")) as f:
    ...
    smplx_params["locator_offset"] = locator_offset.unsqueeze(0).repeat(batch_size, 1, 1)
...
def test():
    ...
    smplx_data_root = (
        "/data1/projects/ExAvatar_RELEASE/avatar/data/Custom/data/gyeongsik/smplx_optimized/smplx_params_smoothed"
    )
    shape_param_file = (
        "/data1/projects/ExAvatar_RELEASE/avatar/data/Custom/data/gyeongsik/smplx_optimized/shape_param.json"
    )
    ...
    posed_verts = smplx_model.transform_to_posed_verts(smplx_data=smplx_data, device="cuda")

    smplx_face = smplx_model.smpl_x.face_upsampled
    trimesh.Trimesh(vertices=posed_verts[0].detach().cpu().numpy(), faces=smplx_face).export("./posed_obj1.obj")
    trimesh.Trimesh(vertices=posed_verts[1].detach().cpu().numpy(), faces=smplx_face).export("./posed_obj2.obj")

    neutral_posed_verts, _, _ = smplx_model.get_query_points(smplx_data=smplx_data, device="cuda")
    smplx_face = smplx_model.smpl_x.face
    trimesh.Trimesh(vertices=neutral_posed_verts[0].detach().cpu().numpy(), faces=smplx_face).export(
        "./neutral_posed_obj1.obj"
    )
    trimesh.Trimesh(vertices=neutral_posed_verts[1].detach().cpu().numpy(), faces=smplx_face).export(
        "./neutral_posed_obj2.obj"
    )
    ...
    for idx, (cam_param, img) in enumerate(zip(cam_param_list, ori_image_list)):
        ...
        cv2.imwrite(
            ...
            np.clip((0.9 * mesh_render + 0.1 * img) * (1 - is_bkg) + is_bkg * img, 0, 255).astype(np.uint8),
        )
...
def read_smplx_param_humman(imgs_root, smplx_params_root, img_size=896, batch_size=1, device="cuda"):
    import json
    import os
    from glob import glob
    ...
    img_path_list = [
        os.path.join(imgs_root, file) for file in os.listdir(imgs_root) if file.endswith(suffixes) and file[0] != "."
    ]
    ...
    for img_path in img_path_list:
        smplx_path = os.path.join(smplx_params_root, os.path.splitext(os.path.basename(img_path))[0] + ".json")
...
def test_humman():
    ...
    smplx_model = SMPLXModel(human_model_path, gender, shape_param_dim=10, expr_param_dim=10, subdivide_num=2)
    smplx_model.to("cuda")
    ...
    smplx_data["betas"] = smplx_data["betas"].unsqueeze(0).repeat(smplx_data["body_pose"].shape[0], 1)
    ...
    ori_image_list = [
        (e.permute(1, 2, 0)[:, :, (2, 1, 0)].numpy() * 255).astype(np.uint8) for e in data["render_image"]
    ]
    ...
    smplx_face = smplx_model.smpl_x.face_upsampled
    trimesh.Trimesh(vertices=posed_verts[0].detach().cpu().numpy(), faces=smplx_face).export(
        "./debug_vis/smplx/posed_obj1.obj"
    )
    if len(posed_verts) > 1:
        trimesh.Trimesh(vertices=posed_verts[1].detach().cpu().numpy(), faces=smplx_face).export(
            "./debug_vis/smplx/posed_obj2.obj"
        )

    neutral_posed_verts, _, _ = smplx_model.get_query_points(smplx_data=smplx_data, device="cuda")
    smplx_face = smplx_model.smpl_x.face
    trimesh.Trimesh(vertices=neutral_posed_verts[0].detach().cpu().numpy(), faces=smplx_face).export(
        "./debug_vis/smplx/neutral_posed_obj1.obj"
    )
    if len(neutral_posed_verts) > 1:
        trimesh.Trimesh(vertices=neutral_posed_verts[1].detach().cpu().numpy(), faces=smplx_face).export(
            "./debug_vis/smplx/neutral_posed_obj2.obj"
        )
...
def generate_smplx_point():
    ...
    mesh_neutral_pose, mesh_neutral_pose_wo_upsample, transform_mat_neutral_pose = smplx_model.get_query_points(
    ...
    trimesh.Trimesh(vertices=mesh_neutral_pose[0].detach().cpu().numpy(), faces=smplx_face).export(
        f"debug/smplx_points/smplx_subdivide{subdivide_num}.obj"
    )
