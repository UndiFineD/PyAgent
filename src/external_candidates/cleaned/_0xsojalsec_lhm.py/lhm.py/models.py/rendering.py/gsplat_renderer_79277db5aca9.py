# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\rendering\gsplat_renderer.py
import math
import pdb
...
class GSPlatRenderer(GS3DRenderer):
    ...
    def forward_single_view(
        ...
        xyz, shs, colors_precomp, opacity, scales, rotations, cov3D_precomp = self.get_gaussians_properties(
            viewpoint_camera, gaussian_model
        )

        intrinsics = viewpoint_camera.intrinsic
        extrinsics = viewpoint_camera.world_view_transform.transpose(0, 1).contiguous()  # c2w -> w2c
