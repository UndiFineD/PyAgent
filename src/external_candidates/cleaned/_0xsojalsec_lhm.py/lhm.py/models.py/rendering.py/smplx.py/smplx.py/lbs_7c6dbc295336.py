# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\rendering\smplx\smplx\lbs.py
# -*- coding: utf-8 -*-
...
def lbs(
    ...
    locator_offset: Tensor,  # added by Gyeongsik
    parents: Tensor,
    lbs_weights: Tensor,
    pose2rot: bool = True,
) -> Tuple[Tensor, Tensor]:
    ...
    joint_offset: torch.tensor BxJx3
        Joint offset to adjust joint locations
    locator_offset: torch.tensor BxJx3
        Locator offset to adjust joint locations for the supervision (not used for the skinning)
    ...
    J_transformed, A = batch_rigid_transform(rot_mats, J, parents, dtype=dtype)  # added by Gyeongsik
    if locator_offset is not None:
        J_transformed, _ = batch_rigid_transform(rot_mats, J_lo, parents, dtype=dtype)  # added by Gyeongsik
