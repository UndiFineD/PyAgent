...
class SMPLX_Mesh(object):
    ...
    def register_constrain_prior(self):
        ...
        constrain_body = np.load("./pretrained_models/voxel_grid/human_prior_constrain.npz")["masks"]
...
class SMPLXVoxelMeshModel(nn.Module):
    ...
    @torch.no_grad()
    def voxel_smooth_register(self, voxel_v, template_v, lbs_weights, k=3, smooth_k=30, smooth_n=3000):
        ...
        for _ in tqdm(range(smooth_n)):
            ...
            if update_weights is None:
                ...
                knn_lbs_weights = (1.0 - update_weights) * knn_lbs_weights + update_weights * new_lbs_weights
    ...
    def voxel_skinning_init(self, scale_ratio=1.05, voxel_size=256):
        ...
        # build coordinate
        x_range = np.linspace(0, voxel_size - 1, voxel_size) / (voxel_size - 1)  # from 0 to 255，
        ...
        coordinates[..., 0] = mini_size_bbox[0, 0] + coordinates[..., 0] * (mini_size_bbox[0, 1] - mini_size_bbox[0, 0])
        coordinates[..., 1] = mini_size_bbox[1, 0] + coordinates[..., 1] * (mini_size_bbox[1, 1] - mini_size_bbox[1, 0])
        coordinates[..., 2] = mini_size_bbox[2, 0] + coordinates[..., 2] * (mini_size_bbox[2, 1] - mini_size_bbox[2, 0])
        ...
        if os.path.exists(f"./pretrained_models/voxel_grid/voxel_{voxel_size}.pth"):
            ...
            voxel_flat = self.voxel_smooth_register(coordinates, template_verts, skinning_weight, k=1, smooth_n=3000)
        ...
        # x, y, z, C
        voxel_grid_original = voxel_flat.view(voxel_size, voxel_size, z_voxel_size, LBS_F)
    ...
    def smplx_init(self):
        ...
        shape_dirs = self.smplx_layer.shapedirs.view(smpl_x.vertex_num, 3 * smpl_x.shape_param_dim)
        ...
        pose_dirs = pose_dirs.reshape(vertex_num_upsampled * 3, (smpl_x.joint_num - 1) * 9).permute(1, 0)
    ...
    def query_voxel_debug(self):
        ...
        query_skinning = self.query_voxel_skinning_weights(template_verts).squeeze(0).detach().cpu()
        skinning_weight = self.smplx_layer.lbs_weights.float()
    ...
    def lbs(self, xyz, transform_mat_vertex, trans):
        ...
        xyz = torch.matmul(transform_mat_vertex, xyz[:, :, :, None]).view(batch_size, self.vertex_num_upsampled, 4)[
            :, :, :3
        ]  # [B, N, 3]
    ...
    def transform_to_posed_verts_from_neutral_pose(
        ...
        transform_mat_null_vertex = self.get_transform_mat_vertex(transform_mat_neutral_pose, mean_3d, mask)
        ...
        # compute vertices-LBS function
        transform_mat_vertex = self.get_transform_mat_vertex(transform_mat_joint, mean_3d, mask)
        ...
        neutral_to_posed_vertex = torch.matmul(transform_mat_vertex, transform_mat_null_vertex)  # [B, N, 4, 4]
    ...
    def upsample_mesh_batch(
        ...
        neutral_pose = axis_angle_to_matrix(neutral_pose.view(-1, 55, 3))  # [B, 55, 3, 3]
        ...
        transform_mat_vertex = torch.einsum("blij,bnl->bnij", transform_mat_joint, skinning_weight)
        mesh_neutral_pose_upsampled = self.lbs(dense_pts, transform_mat_vertex, None)

        return mesh_neutral_pose_upsampled

    def transform_to_neutral_pose(self, mean_3d, smplx_data, mesh_neutral_pose, transform_mat_neutral_pose, device):
        """
        Transform the mean 3D vertices to posed vertices from the neutral pose.
