...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        f_dc = features_dc.float().detach().flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = features_rest.float().detach().flatten(start_dim=1).contiguous().cpu().numpy()
...
class GSLayer(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(clip_scaling, omegaconf.listconfig.ListConfig) or isinstance(clip_scaling, list):
...
class GS3DRenderer(nn.Module):
    def __init__(
        ...
        self.gs_net = GSLayer(
            ...
            use_fine_feat=(
                True if decode_with_extra_info is not None and decode_with_extra_info["type"] is not None else False
            ),
        )
    ...
    def animate_gs_model(self, gs_attr: GaussianAppOutput, query_points, smplx_data, debug=False):
        """
        query_points: [N, 3]
        """
        ...
        if debug:
            ...
            rotation = matrix_to_quaternion(torch.eye(3).float()[None, :, :].repeat(N, 1, 1)).to(
                device
            )  # constant rotation
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float32):
            mean_3d = query_points + gs_attr.offset_xyz  # [N, 3]  # canonical space offset.

            # matrix to warp predefined pose to zero-pose
            transform_mat_neutral_pose = merge_smplx_data["transform_mat_neutral_pose"]  # [55, 4, 4]
            ...
            mean_3d, transform_matrix = self.smplx_model.transform_to_posed_verts_from_neutral_pose(
                mean_3d,
                merge_smplx_data,
                query_points,
            ...
            rigid_rotation_matrix = torch.nn.functional.normalize(matrix_to_quaternion(transform_rotation), dim=-1)
    ...
    def get_query_points(self, smplx_data, device):
        with torch.no_grad():
            with torch.autocast(device_type=device.type, dtype=torch.float32):
                ...
                positions, _, transform_mat_neutral_pose = self.smplx_model.get_query_points(
                    smplx_data, device=device
                )  # [B, N, 3]
        smplx_data["transform_mat_neutral_pose"] = transform_mat_neutral_pose  # [B, 55, 4, 4]
    ...
    def decoder_cross_attn_wrapper(self, pcl_embed, latent_feat, extra_info):
        ...
        #     )
        # else:
        gs_feats = self.decoder_cross_attn(pcl_embed.to(dtype=latent_feat.dtype), latent_feat, extra_info)
        return gs_feats
    ...
    def query_latent_feat(
        ...
        if self.skip_decoder:
            ...
            gs_feats = self.decoder_cross_attn_wrapper(pcl_embed, latent_feat, extra_info)
    ...
    def forward_single_batch(
        ...
        if debug:
            ...
            cv2.imwrite(
                "fuck.png",
                (out["comp_rgb"].detach().cpu().numpy()[0, ..., ::-1] * 255).astype(np.uint8),
            )
    ...
    def forward_cano_batch(
        ...
        for rotation_degree, gs in zip(degree_list, gs_list_copy):
            ...
            self_R = torch.from_numpy(generate_rotation_matrix_y(rotation_degree)).to(_R)
            _R = self_R @ _R
        ...
        if debug:
            ...
            for i in range(4):
                cv2.imwrite(
                    f"fuck_{i}.png",
                    (out["comp_rgb"].detach().cpu().numpy()[i, ..., ::-1] * 255).astype(np.uint8),
                )
        ...
        **kwargs,
        ...
        for b in range(batch_size):
            if isinstance(query_gs_features, dict):
                ...
                gs_attr = self.forward_gs_attr(query_gs_features[b], query_points[b], None, debug)
    ...
    def forward_animate_gs(
        ...
        out["comp_rgb"] = out["comp_rgb"].permute(0, 1, 4, 2, 3)  # [B, NV, H, W, 3] -> [B, NV, 3, H, W]
        out["comp_mask"] = out["comp_mask"].permute(0, 1, 4, 2, 3)  # [B, NV, H, W, 3] -> [B, NV, 1, H, W]
        out["comp_depth"] = out["comp_depth"].permute(0, 1, 4, 2, 3)  # [B, NV, H, W, 3] -> [B, NV, 1, H, W]
        return out
...
def test():
    ...
    out = gs_render.forward(
        ...
        background_color=torch.tensor([1.0, 1.0, 1.0]).float().view(1, 1, 3).repeat(batch_size, 2, 1).to(device),
        debug=False,
    )
        ...
        for b_idx in range(len(v)):
            ...
            for v_idx in range(v.shape[1]):
                ...
                if "mask" in k:
                    ...
                    cv2.imwrite(
                        save_path,
                        np.hstack([np.tile(img, (1, 1, 3)), render_img.astype(np.uint8)]),
                    )
                else:
