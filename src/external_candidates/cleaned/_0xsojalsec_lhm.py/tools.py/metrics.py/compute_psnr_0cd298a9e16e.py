# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\tools\metrics\compute_psnr.py
# -*- coding: utf-8 -*-
...
def psnr_compute(
    ...
    for mask_i, (gt, result) in tqdm(enumerate(zip(gt_imgs, result_imgs))):
        result_img = (cv2.imread(result, cv2.IMREAD_UNCHANGED) / 255.0).astype(np.float32)
        gt_img = (cv2.imread(gt, cv2.IMREAD_UNCHANGED) / 255.0).astype(np.float32)

        if mask_imgs is not None:
            mask_img = (cv2.imread(mask_imgs[mask_i], cv2.IMREAD_UNCHANGED) / 255.0).astype(np.float32)
            mask_img = mask_img[..., -1]
        ...
        result_img = cv2.resize(result_img, (512, scale_h), interpolation=cv2.INTER_AREA)
        ...
        if mask_imgs is not None:
            ...
            psnr_mean += [compute_psnr(result_img[mask_label > 0.5], gt_img[mask_label > 0.5])]
        else:
...
if __name__ == "__main__":
    ...
    for item in items:
        ...
        if os.path.exists(input_item_folder) and os.path.exists(target_item_folder):
            psnr = psnr_compute(input_item_folder, target_item_folder, mask_item_folder, opt.pad)
