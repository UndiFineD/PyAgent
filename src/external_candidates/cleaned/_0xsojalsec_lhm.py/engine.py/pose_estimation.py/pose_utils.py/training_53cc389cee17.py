# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\pose_utils\training.py
# Multi-HMR
...
def match_2d_greedy(
    ...
    while np.sum(gtAssigned) < len(gtAssigned) and np.sum(opAssigned) + falsePositiveCounter < len(pred_kps):
        ...
        while not (found):
            ...
            # compute IOU
            iou = get_bbx_overlap(pred_kps[minComb[0]], gtkp[minComb[1]])  # , imgPath, baseline)
