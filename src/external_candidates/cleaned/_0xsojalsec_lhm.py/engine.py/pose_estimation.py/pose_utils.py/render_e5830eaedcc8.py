# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\pose_utils\render.py
import os
...
class Renderer:
    def __init__(self, width, height, K, device, faces=None):
        ...
        if faces is not None:
            self.faces = torch.from_numpy((faces).astype("int")).unsqueeze(0).to(self.device)
...
class RendererUtil:
    ...
    def render_video(self, results, pil_bis_frames, fps, out_path):
        writer = imageio.get_writer(out_path, fps=fps, mode="I", format="FFMPEG", macro_block_size=1)
