# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\blocks\camera_embed.py
# Multi-HMR
...
def _generate_fourier_features(pos, num_bands, max_resolution):
    ...
    freq_bands = torch.stack(
        [torch.linspace(start=min_freq, end=res / 2, steps=num_bands, device=device) for res in max_resolution],
        dim=0,
    )
