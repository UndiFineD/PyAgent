# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\smplify.py
# -*- coding: utf-8 -*-
...
class FastFirstFittingLoss(torch.nn.Module):
    ...
    def find_orient_jitter(self, root_orient, transl, j3d, input_keypoints, bbox):
        ...
        reprojection_error = (((pred_keypoints - input_keypoints[..., :-1]) ** 2) * joints_conf) / scale
...
class SMPLifyLoss(torch.nn.Module):
    ...
    def forward(
        ...
        loss = {
            "reprojection": reprojection_weight * reprojection_error,
            "regularize": regularize_weight * regularize_error + head_regularize_error * head_regularize_weight,
            "shape": shape_error,
            "smooth": smooth_weight * smooth_error,
        }
    ...
    def create_closure(self, optimizer, smpl, params, bbox, input_keypoints):
        def closure():
            ...
            loss_dict = self.forward(out, [poses, params[2], params[3]], input_keypoints, bbox)
            loss = sum(loss_dict.values())
            loss.backward()
...
class TemporalSMPLify:
    ...
    def fit(
        ...
        init_betas = torch.mean(init_betas, dim=0, keepdim=True).repeat(init_poses.shape[0], 1)
        ...
        if cam_intrinsic.dtype == torch.float16:
            ...
            init_transl = inverse_perspective_projection(init_loc.unsqueeze(1), cam_intrinsic, init_dist.unsqueeze(1))[
                :, 0
            ]
        ...
        keypoints_2d[:, [17, 18, 19], 2] = keypoints_2d[:, [17, 18, 19], 2] * keypoints_2d[:, 15:16, 2]
        keypoints_2d[:, [20, 21, 22], 2] = keypoints_2d[:, [20, 21, 22], 2] * keypoints_2d[:, 16:17, 2]
        ...
        init_poses_ = torch.cat([params[0].detach().clone(), params[1].detach().clone()], dim=1)
        loss_fn = SMPLifyLoss(
            cam_intrinsics=cam_intrinsic,
        ...
        closure = loss_fn.create_closure(optimizer, self.smpl, params, bbox, keypoints_2d)
