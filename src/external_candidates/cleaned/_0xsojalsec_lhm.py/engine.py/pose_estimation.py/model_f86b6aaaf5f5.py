# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\model.py
# Multi-HMR
    ...
    def detection(
        ...
        if not is_training:
            ...
            if pseudo_idx is not None:
                ...
                mask = (torch.abs(idx[1] - pseudo_idx[1]) <= max_dist) & (torch.abs(idx[2] - pseudo_idx[2]) <= max_dist)
                idx_num = torch.sum(mask)
                if idx_num < 1:
                    top = torch.clamp(pseudo_idx[1] - max_dist, min=0, max=_scores.shape[1] - 1)
                    bottom = torch.clamp(pseudo_idx[1] + max_dist, min=0, max=_scores.shape[1])
                    left = torch.clamp(pseudo_idx[2] - max_dist, min=0, max=_scores.shape[2] - 1)
                    right = torch.clamp(pseudo_idx[2] + max_dist, min=0, max=_scores.shape[2])
...
class HPH(nn.Module):
    ...
    def set_smpl_init(self, smplx_dir):
        ...
        init_expression = 0.0 * torch.from_numpy(mean_params["shape"].astype("float32")).unsqueeze(0)
