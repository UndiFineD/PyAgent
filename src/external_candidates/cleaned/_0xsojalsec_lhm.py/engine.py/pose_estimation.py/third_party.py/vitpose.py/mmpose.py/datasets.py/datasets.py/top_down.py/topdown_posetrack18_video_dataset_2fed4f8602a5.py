# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\top_down\topdown_posetrack18_video_dataset.py
...
class TopDownPoseTrack18VideoDataset(Kpt2dSviewRgbVidTopDownDataset):
    ...
    def __init__(
        ...
        self.frame_weight = self.frame_weight_test if self.test_mode else self.frame_weight_train
        ...
        for obj in objs:
            ...
            for index in indices:
                ...
                if osp.exists(sup_image_file):
                    ...
                    warnings.warn(
                        f"{sup_image_file} does not exist, use {cur_image_file} instead.",
                        UserWarning,
                    )
                    image_files.append(cur_image_file)
            rec.append(
        ...
        for det_res in all_boxes:
            ...
            for index in indices:
                ...
                if osp.exists(sup_image_file):
                    ...
                    warnings.warn(f"{sup_image_file} does not exist, use {cur_image_file} instead.")
                    image_files.append(cur_image_file)
