# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\top_down\topdown_mpii_trb_dataset.py
...
class TopDownMpiiTrbDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def _get_db(self, ann_file):
        ...
        for anno in data["annotations"]:
            ...
            newitem["image_file"] = osp.join(self.img_prefix, imid2info[image_id]["file_name"])
    ...
    def _report_metric(self, res_file):
        ...
        assert len(preds) == len(self.db), f"len(preds)={len(preds)}, len(self.db)={len(self.db)}"
