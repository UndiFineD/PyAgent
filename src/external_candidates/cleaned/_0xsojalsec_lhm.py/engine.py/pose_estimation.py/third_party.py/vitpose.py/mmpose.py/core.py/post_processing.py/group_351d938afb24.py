# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\core\post_processing\group.py
# ------------------------------------------------------------------------------
...
class _Params:
    ...
    def __init__(self, cfg):
        ...
        if self.num_joints == 17:
            self.joint_order = [i - 1 for i in [1, 2, 3, 4, 5, 6, 7, 12, 13, 8, 9, 10, 11, 14, 15, 16, 17]]
        else:
            ...
            if self.use_udp:
                for i in range(len(results)):
                    if results[i].shape[0] > 0:
                        results[i][..., :2] = post_dark_udp(results[i][..., :2].copy(), heatmaps[i : i + 1, :])
            else:
