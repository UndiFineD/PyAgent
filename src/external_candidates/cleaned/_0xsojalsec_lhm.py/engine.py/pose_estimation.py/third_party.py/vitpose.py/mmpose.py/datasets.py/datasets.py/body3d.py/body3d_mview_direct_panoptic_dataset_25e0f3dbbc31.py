# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\body3d\body3d_mview_direct_panoptic_dataset.py
...
class Body3DMviewDirectPanopticDataset(Kpt3dMviewRgbImgDirectDataset):
    ...
    def __init__(
        ...
        if ann_file is None:
            self.db_file = osp.join(img_prefix, f"group_{self.subset}_cam{self.num_cameras}.pkl")
        else:
            ...
            for i, file in enumerate(anno_files):
                if i % self.seq_frame_interval == 0:
                    ...
                    for k, cam_param in cameras.items():
                        ...
                        all_poses_3d = np.zeros((self.max_persons, self.num_joints, 3), dtype=np.float32)
                        all_poses_vis_3d = np.zeros((self.max_persons, self.num_joints, 3), dtype=np.float32)
                        all_roots_3d = np.zeros((self.max_persons, 3), dtype=np.float32)
                        all_poses = np.zeros((self.max_persons, self.num_joints, 3), dtype=np.float32)
                        ...
                        for body in bodies:
                            ...
                            all_poses_vis_3d[cnt] = np.repeat(np.reshape(joints_vis, (-1, 1)), 3, axis=1)

                            pose2d = np.zeros((pose3d.shape[0], 3))
                            # get pose_2d from pose_3d
                            pose2d[:, :2] = single_view_camera.world_to_pixel(pose3d[:, :3])
    ...
    def evaluate(self, results, res_folder=None, metric="mpjpe", **kwargs):
        ...
        _results = [dict(sample_id=sample_id, pose_3d=pose_3d) for (sample_id, pose_3d) in zip(sample_ids, pose_3ds)]
        _results = self._sort_and_unique_outputs(_results, key="sample_id")

        metrics = metric if isinstance(metric, list) else [metric]
        for _metric in metrics:
            if _metric not in self.ALLOWED_METRICS:
                raise ValueError(f'Unsupported metric "{_metric}"Supported metrics are {self.ALLOWED_METRICS}')
