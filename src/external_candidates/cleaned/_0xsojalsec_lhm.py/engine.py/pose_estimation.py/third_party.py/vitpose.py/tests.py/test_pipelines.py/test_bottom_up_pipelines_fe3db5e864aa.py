# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_pipelines\test_bottom_up_pipelines.py
...
def _get_mask(coco, anno, img_id):
    ...
    for obj in anno:
        if obj["iscrowd"]:
            ...
            rles = xtcocotools.mask.frPyObjects(obj["segmentation"], img_info["height"], img_info["width"])
            for rle in rles:
                m += xtcocotools.mask.decode(rle)
...
def _get_joints(anno, ann_info, int_sigma):
    ...
    for i, obj in enumerate(anno):
        joints[i, : ann_info["num_joints"], :3] = np.array(obj["keypoints"]).reshape([-1, 3])
        if ann_info["scale_aware_sigma"]:
            # get person box
            box = obj["bbox"]
...
def test_bottomup_pipeline():
    ...
    results_copy["joints"] = [joints.copy() for _ in range(results_copy["ann_info"]["num_scales"])]
    ...
    with pytest.raises(ValueError):
        random_affine_transform = BottomUpRandomAffine(30, [0.75, 1.5], "short-long", 40)
        results_affine_transform = random_affine_transform(copy.deepcopy(results))
    ...
    assert len(results_generate_multi_target["targets"]) == results["ann_info"]["num_scales"]
...
def test_BottomUpGenerateHeatmapTarget():
    ...
    assert len(results_generate_heatmap_target["target"]) == results["ann_info"]["num_scales"]
