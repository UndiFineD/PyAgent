# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\.mim\demo\webcam_demo.py
...
def parse_args():
    ...
    parser.add_argument("--enable-human-pose", type=int, default=1, help="Enable human pose estimation")
    parser.add_argument(
        "--enable-animal-pose",
        type=int,
        default=0,
    ...
    parser.add_argument(
        ...
        default=[1],
        nargs="+",
        help="Object category label of human in detection results.Default is [1(person)], following COCO definition.",
    )
    parser.add_argument(
        "--animal-pose-config",
        type=str,
        default="configs/animal/2d_kpt_sview_rgb_img/topdown_heatmap/animalpose/hrnet_w32_animalpose_256x256.py",
    ...
    parser.add_argument("--det-score-thr", type=float, default=0.5, help="bbox score threshold")
    parser.add_argument("--kpt-thr", type=float, default=0.3, help="bbox score threshold")
    parser.add_argument(
        "--vis-mode",
        type=int,
        default=2,
    ...
    parser.add_argument("--sunglasses", action="store_true", help="Apply `sunglasses` effect.")
...
def process_mmdet_results(mmdet_results, class_names=None, cat_ids=1):
    ...
    labels = np.concatenate([np.full(bbox.shape[0], i - 1, dtype=np.int32) for i, bbox in zip(cat_ids, bbox_results)])
    ...
    while True:
        ...
        for model_info, pose_history in zip(pose_model_list, pose_history_list):
            ...
            with stop_watch.timeit(model_name):
                ...
                det_results = process_mmdet_results(mmdet_results, class_names=det_model.CLASSES, cat_ids=cat_ids)
        ...
        with stop_watch.timeit("_FPS_"):
                ...
                for model_info, pose_results in zip(pose_model_list, pose_results_list):
                    ...
                    if args.sunglasses:
                        if dataset_name in {
                            ...
                            raise ValueError(f"Sunglasses effect does not support{dataset_name}")
                    ...
                    if args.bugeye:
                        if dataset_name in {
                            ...
                            raise ValueError(f"Bug-eye effect does not support{dataset_name}")
                        img = apply_bugeye_effect(img, pose_results, left_eye_idx, right_eye_idx)
            ...
            if args.out_video_file is not None:
                if vid_out is None:
                    ...
                    vid_out = cv2.VideoWriter(args.out_video_file, fourcc, fps, frame_size)
