...
class PoseLifter(BasePose):
    ...
    def forward_train(self, input, target, target_weight, metas, **kwargs):
        ...
        if self.with_keypoint:
            ...
            keypoint_accuracy = self.keypoint_head.get_accuracy(output, target, target_weight, metas)
        ...
        if self.with_traj:
            ...
            traj_losses = self.traj_head.get_loss(traj_output, kwargs["traj_target"], None)
    ...
    @deprecated_api_warning({"pose_limb_color": "pose_link_color"}, cls_name="PoseLifter")
    def show_result(
        self,
        result,
        img=None,
        ...
        if img is not None:
            ...
            if len(bbox_result) > 0:
                ...
                imshow_bboxes(img, bboxes, colors="green", thickness=thickness, show=False)
