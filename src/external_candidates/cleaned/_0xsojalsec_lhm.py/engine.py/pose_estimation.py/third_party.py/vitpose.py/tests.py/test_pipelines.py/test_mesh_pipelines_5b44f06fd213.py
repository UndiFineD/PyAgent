# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_pipelines\test_mesh_pipelines.py
...
def _check_rot90(origin_imgs, result_imgs):
    if origin_imgs.shape[0] == result_imgs.shape[1] and origin_imgs.shape[1] == result_imgs.shape[0]:
        return True
    else:
        return False
...
def _load_test_data():
    ...
    ann_info["joint_weights"] = np.ones(ann_info["num_joints"], dtype=np.float32).reshape(ann_info["num_joints"], 1)
