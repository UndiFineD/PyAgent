# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\hand\interhand3d_dataset.py
...
class InterHand3DDataset(Kpt3dSviewRgbImgTopDownDataset):
    ...
    def __init__(
        ...
        if dataset_info is None:
            warnings.warn(
                "dataset_info is missing. Check https://github.com/open-mmlab/mmpose/pull/663 for details.",
                DeprecationWarning,
            )
    ...
    def _get_db(self):
        ...
        for img_id in self.img_ids:
            ...
            camera_pos = np.array(cameras[capture_id]["campos"][camera_name], dtype=np.float32)
            camera_rot = np.array(cameras[capture_id]["camrot"][camera_name], dtype=np.float32)
            focal = np.array(cameras[capture_id]["focal"][camera_name], dtype=np.float32)
            principal_pt = np.array(cameras[capture_id]["princpt"][camera_name], dtype=np.float32)
            joint_world = np.array(joints[capture_id][frame_idx]["world_coord"], dtype=np.float32)
            joint_cam = self._world2cam(joint_world.transpose(1, 0), camera_rot, camera_pos.reshape(3, 1)).transpose(
                1, 0
            )
    ...
    def _report_metric(self, res_file, metrics):
        ...
        for pred, item in zip(preds, self.db):
            # mrrpe
            if "MRRPE" in metrics:
                if item["hand_type"].all() and item["joints_3d_visible"][20, 0] and item["joints_3d_visible"][41, 0]:
                    rel_root_masks.append(True)

                    pred_left_root_img = np.array(pred["keypoints"][41], dtype=np.float32)[None, :]
                    pred_left_root_img[:, 2] += item["abs_depth"][0] + pred["rel_root_depth"]
                    pred_left_root_cam = self._pixel2cam(pred_left_root_img, item["focal"], item["princpt"])

                    pred_right_root_img = np.array(pred["keypoints"][20], dtype=np.float32)[None, :]
                    pred_right_root_img[:, 2] += item["abs_depth"][0]
                    pred_right_root_cam = self._pixel2cam(pred_right_root_img, item["focal"], item["princpt"])

                    preds_rel_root.append(pred_left_root_cam - pred_right_root_cam)
                    gts_rel_root.append([item["joints_cam"][41] - item["joints_cam"][20]])
                else:
            ...
            if "MPJPE" in metrics:
                ...
                pred_joint_coord_cam = self._pixel2cam(pred_joint_coord_img, item["focal"], item["princpt"])
                ...
                if item["hand_type"].all():
                    single_masks.append(np.zeros(self.ann_info["num_joints"], dtype=bool))
                    interacting_masks.append(mask)
                    all_masks.append(mask)
                else:
                    single_masks.append(mask)
                    interacting_masks.append(np.zeros(self.ann_info["num_joints"], dtype=bool))
                    all_masks.append(mask)
        ...
        if "MRRPE" in metrics:
            info_str.append(("MRRPE", keypoint_epe(preds_rel_root, gts_rel_root, rel_root_masks)))
        ...
        if "MPJPE" in metrics:
            ...
            info_str.append(
                (
                    "MPJPE_single",
                    keypoint_epe(preds_joint_coord_cam, gts_joint_coord_cam, single_masks),
                )
            )
            info_str.append(
                (
                    "MPJPE_interacting",
                    keypoint_epe(preds_joint_coord_cam, gts_joint_coord_cam, interacting_masks),
                )
            )
