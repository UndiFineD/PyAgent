# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\docs\en\collect.py
#!/usr/bin/env python
...
minisections = [x.split("/")[-2:] for x in glob("../../configs/*/*") if "_base_" not in x]
...
for subtopic, topic, task in sorted(subtopics):
    ...
    datasets = sorted(list(set(x.split("/")[-2] for x in glob(f"../../configs/{topic}/{task}/*/*/"))))
