...
class PoseWarper(TopDown):
    ...
    def forward_test(self, imgs, img_metas, return_heatmap=False, **kwargs):
        ...
        if self.test_cfg.get("flip_test", True):
            ...
            if self.concat_tensors:
                ...
                features_flipped = [self.backbone(img_flipped) for img_flipped in imgs_flipped]

            if self.with_neck:
                features_flipped = self.neck(features_flipped, frame_weight=frame_weight)
    ...
    def forward_dummy(self, img):
        ...
        if batch_size == 2:
            warnings.warn("Current batch size: 2, for pytorch2onnx and getting flops both.")
        else:
