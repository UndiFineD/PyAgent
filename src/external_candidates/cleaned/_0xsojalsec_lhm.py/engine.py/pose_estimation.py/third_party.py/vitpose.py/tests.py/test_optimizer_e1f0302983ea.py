# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_optimizer.py
...
def test_build_optimizers():
    ...
    for i in range(2):
        ...
        assert torch.equal(param_groups["params"][0], param_dict[f"model{i + 1}.weight"])
    ...
    for i in range(2):
        ...
        assert optimizer.defaults["weight_decay"] == base_wd
        assert len(param_groups["params"]) == 2
        assert torch.equal(param_groups["params"][0], param_dict[f"module.model{i + 1}.weight"])
        assert torch.equal(param_groups["params"][1], param_dict[f"module.model{i + 1}.bias"])
