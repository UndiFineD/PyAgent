# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_models\test_pose_lifter_forward.py
...
def _create_inputs(
    ...
    target_weight = np.ones((batch_size, joint_num_out, joint_channel_out), dtype=np.float32)
    ...
    if semi:
        ...
        unlabeled_pose_in = rng.rand(batch_size, joint_num_in * joint_channel_in, seq_len)
        unlabeled_target_2d = np.zeros((batch_size, joint_num_out, joint_channel_in), dtype=np.float32)
        intrinsics = np.ones((batch_size, 4))

        inputs["traj_target"] = torch.FloatTensor(traj_target)
        inputs["unlabeled_input"] = torch.FloatTensor(unlabeled_pose_in).requires_grad_(True)
