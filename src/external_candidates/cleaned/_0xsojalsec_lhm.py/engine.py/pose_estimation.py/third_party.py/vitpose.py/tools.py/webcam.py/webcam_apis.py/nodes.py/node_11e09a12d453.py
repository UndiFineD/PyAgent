...
class Node(Thread, metaclass=ABCMeta):
    ...
    def register_input_buffer(self, buffer_name: str, input_name: str, essential: bool = False):
    ...
    def set_runner(self, runner):
        ...
        buffer_names = [buffer.buffer_name for buffer in self._input_buffers + self._output_buffers]
    ...
    def _get_input_from_buffer(self) -> Tuple[bool, Optional[Dict]]:
        ...
        for buffer_info in self._input_buffers:
            if buffer_info.essential and buffer_manager.is_empty(buffer_info.buffer_name):
                return False, None
        ...
        for buffer_info in self._input_buffers:
            try:
                result[buffer_info.input_name] = buffer_manager.get(buffer_info.buffer_name, block=False)
            except Empty:
    ...
    def run(self):
        ...
        for event_info in self._registered_events:
            ...
            def event_listener():
                while True:
                    with self._event_manager.wait_and_handle(event_info.event_name, event_info.is_keyboard):
