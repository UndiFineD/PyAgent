# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_pipelines\test_top_down_pipelines.py
...
def test_top_down_pipeline():
    ...
    halfbody_transform = TopDownHalfBodyTransform(num_joints_half_body=8, prob_half_body=1.0)
    ...
    generate_target = TopDownGenerateTarget(sigma=2, target_type="GaussianHeatMap", unbiased_encoding=True)
    ...
    generate_target = TopDownGenerateTarget(kernel=(11, 11), encoding="Megvii", unbiased_encoding=False)
    results_target = generate_target(copy.deepcopy(results_tensor))
    assert "target" in results_target
    assert results_target["target"].shape == (
        num_joints,
    ...
    generate_target = TopDownGenerateTarget(kernel=[(11, 11), (7, 7)], encoding="Megvii", unbiased_encoding=False)
    results_target = generate_target(copy.deepcopy(results_tensor))
    assert "target" in results_target
    assert results_target["target"].shape == (
        2,
