# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tools\webcam\webcam_apis\nodes\frame_effect_node.py
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in frame_msg.get_pose_results():
            ...
            if isinstance(self.bbox_color, dict):
                bbox_colors = [self.bbox_color.get(label, (0, 255, 0)) for label in bbox_labels]
            else:
...
class SunglassesNode(FrameDrawingNode):
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = apply_sunglasses_effect(canvas, preds, self.src_img, left_eye_idx, right_eye_idx)
        return canvas
...
class SpriteNode(FrameDrawingNode):
    def __init__(
        ...
        if src_img_path is None:
            ...
            src_img_path = (
                "https://user-images.githubusercontent.com/26739999/151532276-33f968d9-917f-45e3-8a99-ebde60be83bb.png"
            )
        self.src_img = load_image_from_disk_or_url(src_img_path, cv2.IMREAD_UNCHANGED)[:144, :108]
    ...
    def apply_sprite_effect(self, img, pose_results, left_hand_index, right_hand_index, kpt_thr=0.5):
        """Apply sprite effect.
        ...
        img[x : x + hm, y : y + wm] = (
            img[x : x + hm, y : y + wm] * (1 - sprite[:, :, 3:] / 255) + sprite[:, :, :3] * (sprite[:, :, 3:] / 255)
        ).astype("uint8")
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_sprite_effect(canvas, preds, left_hand_idx, right_hand_idx)
        return canvas
...
class BackgroundNode(FrameDrawingNode):
    def __init__(
        ...
        if src_img_path is None:
            src_img_path = (
                "https://user-images.githubusercontent.com/11788150/149731957-abd5c908-9c7f-45b2-b7bf-821ab30c6a3e.jpg"
            )
        self.src_img = load_image_from_disk_or_url(src_img_path)

    def apply_background_effect(self, img, det_results, background_img, effect_region=(0.2, 0.2, 0.8, 0.8)):
        """Change background.
...
class SaiyanNode(FrameDrawingNode):
    def __init__(
        ...
        if hair_img_path is None:
            hair_img_path = (
                "https://user-images.githubusercontent.com/11788150/149732117-fcd2d804-dc2c-426c-bee7-94be6146e05c.png"
            )
        self.hair_img = load_image_from_disk_or_url(hair_img_path)

        if light_video_path is None:
            light_video_path = get_cached_file_path(
                "https://user-images.githubusercontent.com/11788150/149732080-ea6cfeda-0dc5-4bbb-892a-3831e5580520.mp4"
            )
    ...
    def apply_saiyan_effect(
        ...
        pts_src = np.array(
            [[84, 398], [331, 393], [84, 145], [331, 140]],  # face kpt 0  # face kpt 16
            dtype=np.float32,
        )
        ...
        for pose in pose_results:
            ...
            kpts = pose["keypoints"]
            if kpts[face_indices[0], 2] < kpt_thr or kpts[face_indices[16], 2] < kpt_thr:
                continue
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_saiyan_effect(canvas, preds, self.hair_img, frame, face_indices)
...
class MoustacheNode(FrameDrawingNode):
    def __init__(
        ...
        if src_img_path is None:
            src_img_path = (
                "https://user-images.githubusercontent.com/11788150/149732141-3afbab55-252a-428c-b6d8-0e352f432651.jpeg"
            )
        self.src_img = load_image_from_disk_or_url(src_img_path)

    def apply_moustache_effect(self, img, pose_results, moustache_img, face_indices, kpt_thr=0.5):
        ...
        pts_src = np.array([[1164, 741], [1729, 741], [1164, 1244], [1729, 1244]], dtype=np.float32)
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            preds = pose_result["preds"]
            face_indices = get_face_keypoint_ids(model_cfg)
            canvas = self.apply_moustache_effect(canvas, preds, self.src_img, face_indices)
        return canvas
...
class HatNode(FrameDrawingNode):
    def __init__(
        ...
        if src_img_path is None:
            ...
            src_img_path = (
                "https://user-images.githubusercontent.com/28900607/149766271-2f591c19-9b67-4d92-8f94-c272396ca141.png"
            )
        self.src_img = load_image_from_disk_or_url(src_img_path, cv2.IMREAD_UNCHANGED)

    @staticmethod
    def apply_hat_effect(img, pose_results, hat_img, left_eye_index, right_eye_index, kpt_thr=0.5):
        """Apply hat effect.
        Args:
            img (np.ndarray): Image data.
            pose_results (list[dict]): The pose estimation results containing:
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_hat_effect(canvas, preds, self.src_img, left_eye_idx, right_eye_idx)
        return canvas
...
class FirecrackerNode(FrameDrawingNode):
    def __init__(
        ...
        if src_img_path is None:
            self.src_img_path = (
                "https://user-images.githubusercontent.com/28900607/149766281-6376055c-ed8b-472b-991f-60e6ae6ee1da.gif"
            )
    ...
    @staticmethod
    def apply_firecracker_effect(img, pose_results, firecracker_img, left_wrist_idx, right_wrist_idx, kpt_thr=0.5):
    ...
    def draw(self, frame_msg):
        ...
        for pose_result in pose_results:
            ...
            canvas = self.apply_firecracker_effect(canvas, preds, frame, left_wrist_idx, right_wrist_idx)
