# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_pipelines\test_pose3d_transform.py
...
def test_joint_transforms():
    ...
    # manually do transformations
    center = np.array([0.5 * results["image_width"], 0.5 * results["image_height"]], dtype=np.float32)
...
def test_camera_projection():
    ...
    camera_param["K"] = np.concatenate((np.diagflat(camera_param["f"]), camera_param["c"]), axis=-1)
    pipeline_2 = [
        dict(
            type="CameraProjection",
            item="input_3d",
        ...
        dict(type="Collect", keys=["input_3d_wp", "input_3d_p", "input_2d"], meta_keys=[]),
    ]
    ...
    np.testing.assert_allclose(output2["input_3d_p"], output2["input_2d"], rtol=1e-3, atol=1e-1)
    ...
    _kpt_camera = camera.pixel_to_camera(np.concatenate([kpt_pixel, kpt_camera[:, [2]]], -1))
