...
def apply_bugeye_effect(img, pose_results, left_eye_index, right_eye_index, kpt_thr=0.5):
    ...
    for pose in pose_results:
        ...
        img = cv2.remap(img, xx, yy, interpolation=cv2.INTER_AREA, borderMode=cv2.BORDER_REPLICATE)
    return img


def apply_sunglasses_effect(img, pose_results, sunglasses_img, left_eye_index, right_eye_index, kpt_thr=0.5):
    ...
    for pose in pose_results:
        ...
        pts_tar = np.vstack([kpt_reye + vo, kpt_reye - vo, kpt_leye + vo, kpt_leye - vo])
