# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\pipelines\bottom_up_transform.py
...
def _get_multi_scale_size(image, input_size, current_scale, min_scale, use_udp=False):
    ...
    if w < h:
        ...
        h_resized = int(_ceil_to_multiples_of(min_input_w / w * h, 64) * current_scale / min_scale)
        if use_udp:
            scale_w = w - 1.0
        ...
        w_resized = int(_ceil_to_multiples_of(min_input_h / h * w, 64) * current_scale / min_scale)
        if use_udp:
            scale_h = h - 1.0
...
def _resize_align_multi_scale_udp(image, input_size, current_scale, min_scale):
    ...
    """
    assert len(input_size) == 2
    size_resized, _, _ = _get_multi_scale_size(image, input_size, current_scale, min_scale, True)
    ...
    image_resized = cv2.warpAffine(image.copy(), trans, size_resized, flags=cv2.INTER_LINEAR)
    ...
    def __call__(self, joints):
        ...
        for i in range(len(joints)):
            ...
            for idx, pt in enumerate(joints[i]):
                x, y = int(pt[0]), int(pt[1])
                if pt[2] > 0 and 0 <= y < self.output_size[1] and 0 <= x < self.output_size[0]:
...
class PAFGenerator:
    ...
    def _accumulate_paf_map_(self, pafs, src, dst, count):
        ...
        max_x = min(np.ceil(max(src[0], dst[0]) + self.limb_width), self.output_size[0] - 1)
        min_y = max(np.floor(min(src[1], dst[1]) - self.limb_width), 0)
        max_y = min(np.ceil(max(src[1], dst[1]) + self.limb_width), self.output_size[1] - 1)
    ...
    def __call__(self, joints):
        ...
        for idx, sk in enumerate(self.skeleton):
            count = np.zeros((self.output_size[1], self.output_size[0]), dtype=np.float32)

            for p in joints:
                src = p[sk[0]]
                dst = p[sk[1]]
                if src[2] > 0 and dst[2] > 0:
                    self._accumulate_paf_map_(pafs[2 * idx : 2 * idx + 2], src[:2], dst[:2], count)
...
class BottomUpRandomAffine:
    ...
    def __init__(self, rot_factor, scale_factor, scale_type, trans_factor, use_udp=False):
        ...
        if self.use_udp:
            for i, _output_size in enumerate(self.output_size):
                ...
                trans = get_warp_matrix(
                    ...
                    size_dst=np.array((_output_size[0], _output_size[1]), dtype=np.float32) - 1.0,
                    size_target=scale,
                )
                mask[i] = (
                    cv2.warpAffine(
                ...
                joints[i][:, :, 0:2] = warp_affine_joints(joints[i][:, :, 0:2].copy(), trans)
            ...
            mat_input = get_warp_matrix(
                theta=aug_rot,
                size_input=center * 2.0,
                size_dst=np.array((self.input_size[0], self.input_size[1]), dtype=np.float32) - 1.0,
                size_target=scale,
            )
            ...
            image = cv2.warpAffine(image, mat_input, (int(self.input_size[0]), int(self.input_size[1])))
...
class BottomUpGenerateHeatmapTarget:
    ...
    def _generate(self, num_joints, heatmap_size):
        ...
        heatmap_generator = [
            HeatmapGenerator(output_size, num_joints, self.sigma, self.use_udp) for output_size in heatmap_size
        ]
        return heatmap_generator

    def __call__(self, results):
        """Generate multi-scale heatmap target for bottom-up."""
        heatmap_generator = self._generate(results["ann_info"]["num_joints"], results["ann_info"]["heatmap_size"])
        target_list = list()
...
class BottomUpGenerateTarget:
    ...
    def _generate(self, num_joints, heatmap_size):
        ...
        joints_encoder = [
            JointsEncoder(self.max_num_people, num_joints, output_size, True) for output_size in heatmap_size
        ]
...
class BottomUpGeneratePAFTarget:
    ...
    def _generate(self, heatmap_size, skeleton):
        """Get PAF generator."""
        paf_generator = [PAFGenerator(output_size, self.limb_width, skeleton) for output_size in heatmap_size]
    ...
    def __call__(self, results):
        ...
        paf_generator = self._generate(results["ann_info"]["heatmap_size"], self.skeleton)
        target_list = list()
        joints_list = results["joints"]
...
class BottomUpGetImgSize:
    ...
    def __call__(self, results):
        ...
        if w < h:
            ...
            h_resized = int(_ceil_to_multiples_of(min_input_w / w * h, 64) * self.current_scale / self.min_scale)
            if self.use_udp:
                scale_w = w - 1.0
                scale_h = (h_resized - 1.0) / (w_resized - 1.0) * (w - 1.0)
            else:
            ...
            w_resized = int(_ceil_to_multiples_of(min_input_h / h * w, 64) * self.current_scale / self.min_scale)
            if self.use_udp:
                scale_h = h - 1.0
                scale_w = (w_resized - 1.0) / (h_resized - 1.0) * (h - 1.0)
            else:
