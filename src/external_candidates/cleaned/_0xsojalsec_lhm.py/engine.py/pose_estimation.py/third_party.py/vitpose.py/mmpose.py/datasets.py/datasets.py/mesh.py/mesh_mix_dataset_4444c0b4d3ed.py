# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\mesh\mesh_mix_dataset.py
...
class MeshMixDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self, configs, partition):
        ...
        weights = [np.ones(len(ds)) * p / len(ds) for (p, ds) in zip(partition, datasets)]
