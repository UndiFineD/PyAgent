...
class XDwenDwenNode(FrameDrawingNode):
    ...
    def _init_resource(self, resource_file):
        ...
        for ann in anns:
            ...
            mask_pts = np.array([[pt["x"], pt["y"]] for pt in rst["step_2"]["result"][0]["pointList"]])
            ...
            img = cv2.resize(img, dsize=None, fx=w_scale, fy=h_scale, interpolation=cv2.INTER_CUBIC)
    ...
    def draw(self, frame_msg):
        ...
        if pred is not None:
            if pred["dataset"] == "ap10k":
                ...
                raise ValueError(f"Can not obtain face landmark informationfrom dataset: {pred['type']}")
    ...
    def _get_transform(self, kpts_src, kpts_tar):
        if self.rigid:
            ...
            if (np.linalg.norm(kpts_tar[0] - kpts_tar[2]) - np.linalg.norm(kpts_tar[1] - kpts_tar[2])) * (
                np.linalg.norm(kpts_src[0] - kpts_src[2]) - np.linalg.norm(kpts_src[1] - kpts_src[2])
            ) < 0:
