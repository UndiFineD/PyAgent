# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\apis\inference_tracking.py
...
def _track_by_oks(res, results_last, thr):
    ...
    pose_last = np.array([res_last["keypoints"].reshape((-1)) for res_last in results_last])
...
def _get_area(results):
    ...
    for result in results:
        if "bbox" in result:
            result["area"] = (result["bbox"][2] - result["bbox"][0]) * (result["bbox"][3] - result["bbox"][1])
        else:
            xmin = np.min(result["keypoints"][:, 0][result["keypoints"][:, 0] > 0], initial=1e10)
            xmax = np.max(result["keypoints"][:, 0])
            ymin = np.min(result["keypoints"][:, 1][result["keypoints"][:, 1] > 0], initial=1e10)
...
def _temporal_refine(result, match_result, fps=None):
    ...
    if "one_euro" in match_result:
        result["keypoints"][:, :2] = match_result["one_euro"](result["keypoints"][:, :2])
...
def get_track_id(
    ...
    for result in results:
        track_id, results_last, match_result = _track(result, results_last, tracking_thr)
        if track_id == -1:
