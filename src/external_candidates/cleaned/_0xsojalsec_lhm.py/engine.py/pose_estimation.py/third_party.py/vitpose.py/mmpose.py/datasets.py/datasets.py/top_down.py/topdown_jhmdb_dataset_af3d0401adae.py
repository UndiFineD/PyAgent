...
class TopDownJhmdbDataset(TopDownCocoDataset):
    ...
    def _report_metric(self, res_file, metrics, pck_thr=0.2):
        ...
        for pred, item in zip(preds, self.db):
            ...
            if "tPCK" in metrics:
                torso_thr = np.linalg.norm(item["joints_3d"][4, :2] - item["joints_3d"][5, :2])
                if torso_thr < 1:
                    torso_thr = np.linalg.norm(np.array(pred["keypoints"])[4, :2] - np.array(pred["keypoints"])[5, :2])
        ...
        if "PCK" in metrics:
            pck_p, pck, _ = keypoint_pck_accuracy(outputs, gts, masks, pck_thr, threshold_bbox)
        ...
        if "tPCK" in metrics:
            pck_p, pck, _ = keypoint_pck_accuracy(outputs, gts, masks, pck_thr, threshold_torso)
