...
def test_mesh_Mosh_dataset():
    ...
    custom_dataset = dataset_class(ann_file="tests/data/mosh/test_mosh.npz", pipeline=[])
...
def test_mesh_H36M_dataset():
    ...
    for item in custom_dataset:
        pred = dict(keypoints_3d=item["joints_3d"][None, ...], image_path=item["image_file"])
        outputs.append(pred)
...
def test_mesh_Adversarial_dataset():
    ...
    adversarial_dataset = dict(type="MoshDataset", ann_file="tests/data/mosh/test_mosh.npz", pipeline=[])
