# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\body3d\body3d_h36m_dataset.py
...
class Body3DH36MDataset(Kpt3dSviewKpt2dDataset):
    ...
    def load_config(self, data_cfg):
        ...
        if self.joint_2d_src not in self.SUPPORTED_JOINT_2D_SRC:
            raise ValueError(
                f'Unsupported joint_2d_src "{self.joint_2d_src}". Supported options are {self.SUPPORTED_JOINT_2D_SRC}'
            )
    ...
    def evaluate(self, results, res_folder=None, metric="mpjpe", **kwargs):
        ...
        for _metric in metrics:
            if _metric not in self.ALLOWED_METRICS:
                raise ValueError(
                    f'Unsupported metric "{_metric}" for human3.6 dataset.Supported metrics are {self.ALLOWED_METRICS}'
                )
