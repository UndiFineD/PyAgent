# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\models\losses\regression_loss.py
...
class BoneLoss(nn.Module):
    ...
    def forward(self, output, target, target_weight=None):
        ...
        if self.use_target_weight:
            assert target_weight is not None
            loss = torch.mean(
                torch.abs((output_bone * target_weight).mean(dim=0) - (target_bone * target_weight).mean(dim=0))
            )
        else:
