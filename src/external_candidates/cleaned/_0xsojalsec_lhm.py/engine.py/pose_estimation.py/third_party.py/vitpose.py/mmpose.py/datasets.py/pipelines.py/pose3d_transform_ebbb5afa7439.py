# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\pipelines\pose3d_transform.py
...
class GetRootCenteredPose:
    ...
    def __init__(self, item, root_index, visible_item=None, remove_root=False, root_name=None):
        self.item = item
    ...
    def __call__(self, results):
        ...
        assert joints.ndim >= 2 and joints.shape[-2] > root_idx, f"Got invalid joint shape {joints.shape}"
        ...
        if self.remove_root:
            ...
            if self.visible_item is not None:
                ...
                results[self.visible_item] = np.delete(results[self.visible_item], root_idx, axis=-2)
...
class CollectCameraIntrinsics:
    ...
    def __call__(self, results):
        if self.static_camera:
            ...
            camera_param = results["camera_param"]
        assert "f" in camera_param and "c" in camera_param
        intrinsics = np.concatenate([camera_param["f"].reshape(2), camera_param["c"].reshape(2)])
        if self.need_distortion:
            assert "k" in camera_param and "p" in camera_param
            intrinsics = np.concatenate([intrinsics, camera_param["k"].reshape(3), camera_param["p"].reshape(2)])
...
class CameraProjection:
    ...
    def __init__(
        ...
        if mode not in allowed_mode:
            raise ValueError(f"Got invalid mode: {mode}, allowed modes are {allowed_mode}")
    ...
    def __call__(self, results):
        ...
        if np.random.rand() <= self.flip_prob:
            ...
            for i, item in enumerate(self.item):
                ...
                joints_flipped = fliplr_regression(joints, flip_pairs, **self.flip_cfg[i])
...
class Generate3DHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        local_target = np.exp(-((xx - mu_x) ** 2 + (yy - mu_y) ** 2 + (zz - mu_z) ** 2) / (2 * self.sigma**2))
...
class GenerateVoxel3DHeatmapTarget:
    ...
    def __call__(self, results):
        ...
        grids_x = np.linspace(-space_size[0] / 2, space_size[0] / 2, cube_size[0]) + space_center[0]
        grids_y = np.linspace(-space_size[1] / 2, space_size[1] / 2, cube_size[1]) + space_center[1]
        grids_z = np.linspace(-space_size[2] / 2, space_size[2] / 2, cube_size[2]) + space_center[2]

        target = np.zeros((num_joints, cube_size[0], cube_size[1], cube_size[2]), dtype=np.float32)
        ...
        for n in range(num_people):
            for idx, joint_id in enumerate(joint_indices):
                ...
                g = np.exp(
                    -((kernel_xs - mu_x) ** 2 + (kernel_ys - mu_y) ** 2 + (kernel_zs - mu_z) ** 2) / (2 * self.sigma**2)
                )
                target[idx, i_x[0] : i_x[1], i_y[0] : i_y[1], i_z[0] : i_z[1]] = np.maximum(
