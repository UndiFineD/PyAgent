# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_necks\test_posewarper_neck.py
...
def test_posewarper_neck():
    ...
    with pytest.raises(AssertionError):
        ...
        _ = PoseWarperNeck(out_channels=3, in_channels=512, inner_channels=128, trans_conv_kernel=2)

    with pytest.raises(TypeError):
        # test type of res_blocks_cfg
        _ = PoseWarperNeck(out_channels=3, in_channels=512, inner_channels=128, res_blocks_cfg=2)

    with pytest.raises(AssertionError):
        # test value of dilations
        neck = PoseWarperNeck(out_channels=3, in_channels=512, inner_channels=128, dilations=[])
