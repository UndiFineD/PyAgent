# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tools\webcam\webcam_apis\utils\buffer.py
        ...
        def wrapped(manager, name, *args, **kwargs):
            if exist:
                ...
                if name not in manager:
                    raise ValueError(f'Fail to call {func.__name__}: buffer "{name}" is not registered.')
            else:
                # Assert buffer not exist
                if name in manager:
                    raise ValueError(f'Fail to call {func.__name__}: buffer "{name}" is already registered.')
    ...
    def __init__(self, buffer_type: type = Buffer, buffers: Optional[Dict] = None):
        ...
        if buffers is None:
            ...
            if is_seq_of(list(buffers.values()), buffer_type):
                ...
                raise ValueError(f"The values of buffers should be instance of {buffer_type}")
