# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\.mim\demo\interhand3d_img_demo.py
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Root of the output visualization images. Default not saving the visualization images.",
    )
    ...
    for image_id, image in coco.imgs.items():
        ...
        for ann_id in ann_ids:
            ...
            if camera_params is not None:
                camera_param = {
                    key: camera_params[capture_key][key][camera_key] for key in camera_params[capture_key].keys()
                }
    ...
    for i, det_results in enumerate(mmcv.track_iter_progress(det_results_list)):
        ...
        pose_results = inference_interhand_3d_model(pose_model, image_name, det_results, dataset=dataset)
        ...
        for idx, res in enumerate(pose_results):
            ...
            if args.rebase_keypoint_height:
                ...
                keypoints_3d[..., 2] -= np.min(keypoints_3d[valid, 2], axis=-1, keepdims=True)
            ...
            if args.show_ground_truth:
                if keypoints_3d_gt is None:
                    print("Fail to show ground truth. Please make sure that gt-joints-file is provided.")
                else:
                    gt = res.copy()
                    if args.rebase_keypoint_height:
                        valid = keypoints_3d_gt[..., 3] > 0
                        keypoints_3d_gt[..., 2] -= np.min(keypoints_3d_gt[valid, 2], axis=-1, keepdims=True)
