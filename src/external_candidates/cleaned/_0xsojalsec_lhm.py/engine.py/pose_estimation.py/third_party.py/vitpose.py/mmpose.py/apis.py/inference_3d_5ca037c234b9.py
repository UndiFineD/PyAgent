# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\apis\inference_3d.py
...
def _gather_pose_lifter_inputs(pose_results, bbox_center, bbox_scale, norm_pose_2d=False):
    ...
    for frame in pose_results:
        ...
        for res in frame:
            ...
            if norm_pose_2d:
                ...
                inputs["keypoints"] = (res["keypoints"][:, :2] - center) / scale * bbox_scale + bbox_center
            else:
                inputs["keypoints"] = res["keypoints"][:, :2]

            if res["keypoints"].shape[1] == 3:
                inputs["keypoints"] = np.concatenate([inputs["keypoints"], res["keypoints"][:, 2:]], axis=1)
...
def _collate_pose_sequence(pose_results, with_track_id=True, target_frame=-1):
    ...
    for idx in range(N):
        ...
        if not with_track_id:
            pose_seq["keypoints"] = np.stack([frame[idx]["keypoints"] for frame in pose_results])
        else:
...
def inference_pose_lifter_model(
    ...
    pose_lifter_inputs = _gather_pose_lifter_inputs(pose_results_2d, bbox_center, bbox_scale, norm_pose_2d)
    pose_sequences_2d = _collate_pose_sequence(pose_lifter_inputs, with_track_id, target_idx)
    ...
    with torch.no_grad():
        result = model(input=batch_data["input"], metas=batch_data["metas"], return_loss=False)
...
def vis_3d_pose_result(
    ...
    if dataset_info is not None:
        ...
        if dataset == "Body3DH36MDataset":
            ...
            pose_kpt_color = palette[[9, 0, 0, 0, 16, 16, 16, 9, 9, 9, 9, 16, 16, 16, 0, 0, 0]]
            pose_link_color = palette[[0, 0, 0, 16, 16, 16, 9, 9, 9, 9, 16, 16, 16, 0, 0, 0]]
...
def inference_interhand_3d_model(
    ...
    # forward the model
    with torch.no_grad():
        result = model(img=batch_data["img"], img_metas=batch_data["img_metas"], return_loss=False)
...
def inference_mesh_model(
    ...
    for bbox in bboxes:
        ...
        data = {
            ...
            "joints_2d_visible": np.zeros((cfg.data_cfg.num_joints, 1), dtype=np.float32),
            "joints_3d": np.zeros((cfg.data_cfg.num_joints, 3), dtype=np.float32),
            "joints_3d_visible": np.zeros((cfg.data_cfg.num_joints, 3), dtype=np.float32),
