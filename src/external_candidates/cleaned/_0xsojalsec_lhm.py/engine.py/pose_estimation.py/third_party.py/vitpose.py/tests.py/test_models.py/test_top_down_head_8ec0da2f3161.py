# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_models\test_top_down_head.py
...
def test_vipnas_simple_head():
    ...
    with pytest.raises(TypeError):
        head = ViPNASHeatmapSimpleHead(out_channels=3, in_channels=512, extra={"final_conv_kernel": 1})
...
def test_top_down_simple_head():
    ...
    with pytest.raises(TypeError):
        head = TopdownHeatmapSimpleHead(out_channels=3, in_channels=512, extra={"final_conv_kernel": 1})
