...
class Interhand3D(TopDown):
    ...
    def forward_test(self, img, img_metas, **kwargs):
        ...
        if self.test_cfg.get("flip_test", True):
            ...
            if self.with_keypoint:
                output_flipped = self.keypoint_head.inference_model(features_flipped, img_metas[0]["flip_pairs"])
                output = [(out + out_flipped) * 0.5 for out, out_flipped in zip(output, output_flipped)]

        if self.with_keypoint:
            result = self.keypoint_head.decode(img_metas, output, img_size=[img_width, img_height])
        else:
            result = {}
        return result

    @deprecated_api_warning({"pose_limb_color": "pose_link_color"}, cls_name="Interhand3D")
    def show_result(
        self,
        result,
        img=None,
