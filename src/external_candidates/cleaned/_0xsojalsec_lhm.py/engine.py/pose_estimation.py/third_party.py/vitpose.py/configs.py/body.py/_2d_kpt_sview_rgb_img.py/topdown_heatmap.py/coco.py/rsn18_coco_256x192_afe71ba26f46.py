# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\configs\body\2d_kpt_sview_rgb_img\topdown_heatmap\coco\rsn18_coco_256x192.py
_base_ = [
...
model = dict(
    ...
    keypoint_head=dict(
        ...
        use_prm=False,
        norm_cfg=dict(type="BN"),
        loss_keypoint=[dict(type="JointsMSELoss", use_target_weight=True, loss_weight=0.25)] * 3
        + [dict(type="JointsOHKMMSELoss", use_target_weight=True, loss_weight=1.0)],
    ),
    train_cfg=dict(),
    test_cfg=dict(flip_test=True, post_process="megvii", shift_heatmap=False, modulate_kernel=5),
)
