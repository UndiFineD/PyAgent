# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\pipelines\top_down_transform.py
...
class TopDownHalfBodyTransform:
    ...
    def __call__(self, results):
        ...
        if np.sum(joints_3d_visible[:, 0]) > self.num_joints_half_body and np.random.rand() < self.prob_half_body:
            c_half_body, s_half_body = self.half_body_transform(results["ann_info"], joints_3d, joints_3d_visible)
...
class TopDownGenerateTargetRegression:
    ...
    def __call__(self, results):
        ...
        target, target_weight = self._generate_target(results["ann_info"], joints_3d, joints_3d_visible)
