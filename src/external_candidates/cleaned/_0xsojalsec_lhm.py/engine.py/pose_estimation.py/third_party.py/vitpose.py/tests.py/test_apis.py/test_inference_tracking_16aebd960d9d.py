# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_apis\test_inference_tracking.py
...
def test_top_down_pose_tracking_demo():
    ...
    # InterHand2D demo
    pose_model = init_pose_model(
        "configs/hand/2d_kpt_sview_rgb_img/topdown_heatmap/interhand2d/res50_interhand2d_all_256x256.py",
        None,
        device="cpu",
    )
...
def test_bottom_up_pose_tracking_demo():
    ...
    # build the pose model from a config file and a checkpoint file
    pose_model = init_pose_model(
        "configs/body/2d_kpt_sview_rgb_img/associative_embedding/coco/res50_coco_512x512.py",
        None,
        device="cpu",
    )
    ...
    pose_results, _ = inference_bottom_up_pose_model(pose_model, image_name, dataset_info=dataset_info)
    ...
    # oks
    pose_results, next_id = get_track_id(pose_results, pose_results_last, next_id=next_id, use_oks=True)

    pose_results_last = pose_results
    # one_euro
    pose_results, next_id = get_track_id(pose_results, pose_results_last, next_id=next_id, use_one_euro=True)
