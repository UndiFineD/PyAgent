# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_datasets\test_top_down_dataset.py
...
def test_top_down_PoseTrack18_dataset():
    ...
    data_cfg = dict(
        ...
        use_gt_bbox=True,
        det_bbox_thr=0.0,
        bbox_file="tests/data/posetrack18/annotations/test_posetrack18_human_detections.json",
    )
    # Test det bbox
    data_cfg_copy = copy.deepcopy(data_cfg)
    data_cfg_copy["use_gt_bbox"] = False
...
def test_top_down_COCO_wholebody_dataset():
    ...
    dataset_info = Config.fromfile("configs/_base_/datasets/coco_wholebody.py").dataset_info
    # test COCO datasets
    dataset_class = DATASETS.get(dataset)
    dataset_class.load_annotations = MagicMock()
    dataset_class.coco = MagicMock()
