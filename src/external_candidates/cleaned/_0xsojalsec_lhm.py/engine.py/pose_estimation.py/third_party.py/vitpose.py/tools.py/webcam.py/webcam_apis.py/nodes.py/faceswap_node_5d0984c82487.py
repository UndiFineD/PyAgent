...
class FaceSwapNode(FrameDrawingNode):
    def __init__(
        ...
        self.mode_index = 0
        self.register_event(self.mode_key, is_keyboard=True, handler_func=self.switch_mode)
    ...
    def draw(self, frame_msg):
        ...
        cv2.putText(canvas, mode_txt, (10, 50), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 126, 0), 1)
        ...
        if num_target >= 2:
            ...
            if num_target != len(self.history["target_map"]):
                if self.mode == Mode.SHUFFLE:
                    ...
                    self.history["target_map"] = np.repeat(np.random.choice(num_target), num_target)
    ...
    def _swap_face(self, img_src, img_tar, face_src, face_tar):
        ...
        bbox_src, patch_src, mask_src = self._crop_face_by_contour(img_src, face_src["contour"])
        ...
        trans_mat, _ = cv2.estimateAffine2D(kpts_src.astype(np.float32), kpts_tar.astype(np.float32))
        ...
        img_tar = cv2.seamlessClone(patch_warp, img_tar, mask_tar_soft, center, cv2.NORMAL_CLONE)
        return img_tar
    ...
    def _get_face_info(pose_pred):
        ...
        if face_info["dataset"] == "coco":
            ...
            contour_ids = list(range(33))[::-1] + list(range(33, 38)) + list(range(42, 47))
            face_info["contour"] = keypoints[contour_ids]
        else:
            raise ValueError(f"Can not obtain face landmark informationfrom dataset: {face_info['type']}")
        ...
        if face_info["contour"] is None:
            ...
            face_info["contour"] = np.stack([left_top, right_top, right_bottom, left_bottom])
