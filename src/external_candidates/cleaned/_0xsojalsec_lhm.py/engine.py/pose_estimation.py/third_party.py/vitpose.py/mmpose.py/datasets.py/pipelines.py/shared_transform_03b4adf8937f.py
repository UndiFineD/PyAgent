# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\pipelines\shared_transform.py
...
class NormalizeTensor:
    ...
    def __call__(self, results):
        if isinstance(results["img"], (list, tuple)):
            results["img"] = [F.normalize(img, mean=self.mean, std=self.std) for img in results["img"]]
        else:
    ...
    def albu_builder(self, cfg):
        ...
        if mmcv.is_str(obj_type):
            ...
            if not hasattr(albumentations.augmentations.transforms, obj_type):
                warnings.warn("{obj_type} is not pixel-level transformations. Please use with caution.")
...
class MultitaskGatherTarget:
    ...
    def __init__(self, pipeline_list, pipeline_indices=None, keys=("target", "target_weight")):
        self.keys = keys
        self.pipelines = []
