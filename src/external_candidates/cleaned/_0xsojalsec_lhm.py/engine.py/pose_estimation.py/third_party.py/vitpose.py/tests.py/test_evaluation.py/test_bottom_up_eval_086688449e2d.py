# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\test_evaluation\test_bottom_up_eval.py
...
def test_aggregate_scale():
    ...
    output = aggregate_scale(fake_outputs, align_corners=False, aggregate_scale="average")
    assert isinstance(output, torch.Tensor)
    assert output.shape == fake_outputs[0].shape

    output = aggregate_scale(fake_outputs, align_corners=False, aggregate_scale="unsqueeze_concat")
