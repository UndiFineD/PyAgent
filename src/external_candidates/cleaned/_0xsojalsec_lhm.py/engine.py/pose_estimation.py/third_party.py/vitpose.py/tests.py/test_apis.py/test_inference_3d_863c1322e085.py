...
def test_pose_lifter_demo():
    ...
    pose_results_2d = extract_pose_sequence(pose_results_2d, frame_idx=0, causal=False, seq_len=1, step=1)

    _ = inference_pose_lifter_model(pose_model, pose_results_2d, dataset_info=dataset_info, with_track_id=False)
    ...
    # test special cases
    # Empty 2D results
    _ = inference_pose_lifter_model(pose_model, [[]], dataset_info=dataset_info, with_track_id=False)
    ...
    pose_results_2d_seq = extract_pose_sequence(pose_results_2d, 1, causal=False, seq_len=seq_len, step=1)
...
def test_interhand3d_demo():
    ...
    if torch.cuda.is_available():
        _ = inference_interhand_3d_model(pose_model.cuda(), image_name, det_results, dataset=dataset)

    with pytest.raises(NotImplementedError):
        _ = inference_interhand_3d_model(pose_model, image_name, det_results, dataset="test")
...
def test_body_mesh_demo():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config.model.smpl.joints_regressor = osp.join(tmpdir, "test_joint_regressor.npy")
