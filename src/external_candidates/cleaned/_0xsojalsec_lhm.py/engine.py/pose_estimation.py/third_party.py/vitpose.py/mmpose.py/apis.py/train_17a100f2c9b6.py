# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\apis\train.py
...
try:
    ...
    warnings.warn(
        "Fp16OptimizerHook from mmpose will be deprecated from v0.15.0. Please install mmcv>=1.1.4",
        DeprecationWarning,
    )
...
def train_model(model, dataset, cfg, distributed=False, validate=False, timestamp=None, meta=None):
    ...
    if distributed:
        ...
        if digit_version(mmcv.__version__) >= digit_version("1.4.4") or torch.cuda.is_available():
