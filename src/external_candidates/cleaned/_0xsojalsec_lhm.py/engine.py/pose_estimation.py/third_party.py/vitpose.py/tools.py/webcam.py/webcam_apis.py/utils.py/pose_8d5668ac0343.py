# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tools\webcam\webcam_apis\utils\pose.py
...
def get_hand_keypoint_ids(model_cfg: Config) -> List[int]:
    ...
    try:
        ...
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"left_thumb{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"left_forefinger{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"left_middle_finger{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"left_ring_finger{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"left_pinky_finger{id}", None))

        hand_indices.append(dataset_info.keypoint_name2id.get("right_hand_root", None))

        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"right_thumb{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"right_forefinger{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"right_middle_finger{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"right_ring_finger{id}", None))
        for id in range(1, 5):
            hand_indices.append(dataset_info.keypoint_name2id.get(f"right_pinky_finger{id}", None))
