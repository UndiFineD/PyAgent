# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\tests\utils\data_utils.py
...
def convert_db_to_output(db, batch_size=2, keys=None, is_3d=False):
    ...
    for i in range(0, len_db, batch_size):
        ...
        image_paths = [db[j]["image_file"] for j in range(i, min(i + batch_size, len_db))]
        ...
        if keys is not None:
            ...
            for key in keys:
                output[key] = [db[j][key] for j in range(i, min(i + batch_size, len_db))]
