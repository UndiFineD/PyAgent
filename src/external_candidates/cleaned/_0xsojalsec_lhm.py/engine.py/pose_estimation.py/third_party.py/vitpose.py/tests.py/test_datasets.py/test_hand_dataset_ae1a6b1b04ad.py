...
def test_hand_coco_wholebody_dataset():
    ...
    dataset_info = Config.fromfile("configs/_base_/datasets/coco_wholebody_hand.py").dataset_info
    dataset_class = DATASETS.get(dataset)
...
def test_InterHand3D_dataset():
    ...
    results = convert_db_to_output(custom_dataset.db, keys=["rel_root_depth", "hand_type"], is_3d=True)
    infos = custom_dataset.evaluate(results, metric=["MRRPE", "MPJPE", "Handedness_acc"])
