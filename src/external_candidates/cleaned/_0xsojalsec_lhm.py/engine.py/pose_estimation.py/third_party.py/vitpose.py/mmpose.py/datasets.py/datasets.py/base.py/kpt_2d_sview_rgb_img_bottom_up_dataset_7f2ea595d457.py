# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\datasets\datasets\base\kpt_2d_sview_rgb_img_bottom_up_dataset.py
...
class Kpt2dSviewRgbImgBottomUpDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if dataset_info is None:
            raise ValueError("Check https://github.com/open-mmlab/mmpose/pull/663 for details.")
        ...
        if coco_style:
            ...
            if not test_mode:
                self.img_ids = [
                    img_id for img_id in self.img_ids if len(self.coco.getAnnIds(imgIds=img_id, iscrowd=None)) > 0
                ]
