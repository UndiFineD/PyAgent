# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\pose_estimator.py
# -*- coding: utf-8 -*-
...
def normalize_rgb_tensor(img, imgenet_normalization=True):
    ...
    if imgenet_normalization:
        img = (img - torch.tensor(IMG_NORM_MEAN, device=img.device).view(1, 3, 1, 1)) / torch.tensor(
            IMG_NORM_STD, device=img.device
        ).view(1, 3, 1, 1)
    return img
...
class PoseEstimator:
    ...
    def _preprocess(self, img_np):
        ...
        img_tensor = torch.Tensor(img_np).to(self.device).unsqueeze(0).permute(0, 3, 1, 2)

        _, _, h, w = img_tensor.shape
        scale_factor = min(self.img_size / w, self.img_size / h)
        img_tensor = F.interpolate(img_tensor, scale_factor=scale_factor, mode="bilinear")
    ...
    def __call__(self, img_path):
        ...
        j2d = (j2d - torch.tensor([pad_left, pad_top], device=self.device).unsqueeze(0)) / scale_factor
