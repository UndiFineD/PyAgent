# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\BiRefNet\config.py
import math
import os
    ...
    def __init__(self) -> None:
        ...
        if run_sh_file:
            with open(run_sh_file[0], "r") as f:
                lines = f.readlines()
                self.save_last = int(
                    [l.strip() for l in lines if "'{}')".format(self.task) in l and "val_last=" in l][0]
                    .split("val_last=")[-1]
                    .split()[0]
                )
                self.save_step = int(
                    [l.strip() for l in lines if "'{}')".format(self.task) in l and "step=" in l][0]
                    .split("step=")[-1]
                    .split()[0]
                )
