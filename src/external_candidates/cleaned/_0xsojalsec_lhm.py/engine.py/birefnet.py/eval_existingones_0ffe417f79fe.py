# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\BiRefNet\eval_existingOnes.py
import argparse
import os
...
def do_eval(args):
    ...
    for _data_name in args.data_lst.split("+"):
        ...
        for _model_name in args.model_lst[:]:
            ...
            pred_paths = [
                p.replace(args.gt_root, os.path.join(args.pred_root, _model_name)).replace("/gt/", "/")
                for p in gt_paths
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, help="candidate competitors", default="e_results")
    parser.add_argument(
        "--check_integrity",
        type=bool,
    ...
    parser.add_argument(
        ...
        default="+".join(
            ["S", "MAE", "E", "F", "WF", "MBA", "BIoU", "MSE", "HCE"][: 100 if "DIS5K" in config.task else -1]
        ),
    )
    args = parser.parse_args()
    args.metrics = "+".join(
        ["S", "MAE", "E", "F", "WF", "MBA", "BIoU", "MSE", "HCE"][
            : (100 if sum(["DIS-" in _data for _data in args.data_lst.split("+")]) else -1)
        ]
    )
        ...
        for _data_name in args.data_lst.split("+"):
            for _model_name in args.model_lst:
                ...
                if not sorted(os.listdir(gt_pth)) == sorted(os.listdir(pred_pth)):
                    ...
                    print(
                        "The {} Dataset of {} Model is not matching to the ground-truth".format(_data_name, _model_name)
                    )
    else:
