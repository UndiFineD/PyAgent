# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_h2o_llmstudio.py\llm_studio.py\src.py\possible_values_2ed93ecdb9b1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-h2o-llmstudio\llm_studio\src\possible_values.py
...
def _scan_files(dirname: str, extensions: tuple[str, ...] = (".csv", ".pq", ".parquet", ".json")) -> list[str]:
    """
    ...
    path_list = [
        ...
        if any(map(filename.__contains__, extensions)) and not filename.startswith("__meta_info__")
    ]

    return sorted(path_list)


def strip_common_prefix(paths: Sequence[str], ignore_set: set[str] = set()) -> tuple[str, ...]:
    """
    ...
    paths_to_check = [os.path.split(os.path.normpath(path))[0] for path in paths if path not in ignore_set]
    ...
    stripped = tuple([path if path in ignore_set else os.path.relpath(path, prefix) for path in paths])
...
class Number:
    ...
    def __post_init__(self):
        if self.max is not None and self.min > self.max:
            raise ValueError(f"Expected `min <= max`, got min={self.min} > max={self.max}")
...
class DatasetValue:
    ...
    @abstractmethod
    def get_value(self, dataset: Any, value: Any, type_annotation: type) -> tuple[String, Any]:
        """
    ...
    def _compute_current_values(
        ...
        current_values = list(filter(lambda value: value in possible_values, current_values))
...
class Files(DatasetValue):
    ...
    def get_value(self, dataset: Any, value: Any, type_annotation: type) -> tuple[String, Any]:
        ...
        value = DatasetValue._compute_current_values(value, available_files, self.prefer_with)
