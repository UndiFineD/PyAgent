# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-NoDPI\src\sample_version.py
import asyncio
import logging
import random
...
async def fragment_data(local_reader, remote_writer):
    ...
    if host_end_index != -1:
        parts.append(
            bytes.fromhex("160304") + int(host_end_index + 1).to_bytes(2, byteorder="big") + data[: host_end_index + 1]
        )
