# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\lock.py\redis_distributed_lock_4a940f4f15f3.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\lock\redis_distributed_lock.py
...
class RedisDistributedLock:
    ...
    @asynccontextmanager
    async def acquire(self, timeout: Optional[float] = None, blocking_timeout: Optional[float] = None):
        """
        ...
        try:
            ...
            if acquired:
                try:
                    ...
                    logger.error("Failed to release lock: %s, error: %s", self.resource, e)
...
class RedisDistributedLockManager:
    ...
    def _get_owner_id(self) -> str:
        ...
        try:
            ...
            raise DistributedLockError(f"Distributed lock must be used in coroutine environment: {e}") from e

    async def _acquire_lock(self, resource: str, timeout: float, blocking_timeout: float) -> bool:
        """
        ...
        for attempt in range(retry_count):
            try:
                ...
                result = await self._lua_acquire(keys=[lock_key], args=[owner_id, timeout_ms], client=redis_client)
    ...
    async def _release_lock(self, resource: str):
        ...
        try:
            ...
            result = await self._lua_release(keys=[lock_key], args=[owner_id], client=redis_client)
    ...
    async def is_locked(self, resource: str) -> bool:
        ...
        try:
            ...
            result = await self._lua_status(keys=[lock_key], args=[owner_id], client=redis_client)
            ...
            logger.error("Failed to check reentrant lock status: %s, error: %s", resource, e)
    ...
    async def is_owned_by_current_coroutine(self, resource: str) -> bool:
        ...
        try:
            ...
            logger.error("Failed to check reentrant lock ownership: %s, error: %s", resource, e)
    ...
    async def force_unlock(self, resource: str) -> bool:
        ...
        try:
            ...
            logger.warning("Forcibly released reentrant lock: %s, result: %s", resource, result)

            return result > 0

        except (ConnectionError, TimeoutError, OSError) as e:
            logger.error("Failed to forcibly release reentrant lock: %s, error: %s", resource, e)
...
async def distributed_lock(
    ...
    async with lock.acquire(timeout=timeout, blocking_timeout=blocking_timeout) as acquired:
        yield acquired
        ...
        async def wrapper(*args, **kwargs):
            ...
            async with lock.acquire(timeout=timeout, blocking_timeout=blocking_timeout) as acquired:
                if acquired:
                    ...
                    raise RuntimeError(f"Failed to acquire distributed lock: {resource}")
