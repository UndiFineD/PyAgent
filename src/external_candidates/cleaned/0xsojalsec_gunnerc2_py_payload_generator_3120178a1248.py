# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\core.py\payload_generator.py\payload_generator_3120178a1248.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\payload_generator.py
...
def generate_payload_windows(
    ...
    if payload_type == "tcp":
        ...
        if format_type == "ps1":
            raw = powershell_reverse_http.generate_windows_powershell_http(
                ip,
                port,
                obs,
                beacon_interval,
            ...
            raw = exe_reverse_http.generate_exe_reverse_http(
                ip,
                port,
                obs,
                beacon_interval,
        ...
        if format_type == "ps1":
            raw = powershell_reverse_https.generate_windows_powershell_https(
                ip,
                port,
                obs,
                beacon_interval,
            ...
            raw = exe_reverse_https.generate_exe_reverse_https(
                ip,
                port,
                obs,
                beacon_interval,
            ...
            raw = gunnerplant_reverse_https.generate_gunnerplant_reverse_https(
                ip,
                port,
                obs,
                beacon_interval,
