# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\plugins.py\test_antigravity_accounts_8e24f21b71c5.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_accounts.py
...
def mock_now(monkeypatch):
    ...
    monkeypatch.setattr("code_puppy.plugins.antigravity_oauth.accounts.time.time", mock_time)
...
class TestRateLimiting:
    ...
    def test_is_rate_limited_for_header_style(self, sample_storage):
        ...
        assert manager.is_rate_limited_for_header_style(account, "gemini", "antigravity")

        assert not manager.is_rate_limited_for_header_style(account, "gemini", "gemini-cli")
    ...
    def test_get_min_wait_time_with_rate_limit(self, sample_storage):
        ...
        assert wait_time > 0, "Wait time should be positive when all accounts are rate limited"

        assert wait_time <= 70000, "Wait time should not exceed rate limit duration by much"
