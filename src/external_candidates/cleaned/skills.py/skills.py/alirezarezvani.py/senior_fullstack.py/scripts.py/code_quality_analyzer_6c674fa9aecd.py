# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\senior-fullstack\scripts\code_quality_analyzer.py
#!/usr/bin/env python3
"""
...
def analyze_project(project_path: Path) -> Dict:
    ...
    for filepath in project_path.rglob("*"):
        ...
        lang = (
            ...
            if filepath.suffix in {".ts", ".tsx"}
                ...
                else ("python" if filepath.suffix == ".py" else "go" if filepath.suffix == ".go" else "other")
            )
        )
    ...
    if complexity_scores:
        results["complexity"]["average_complexity"] = round(sum(complexity_scores) / len(complexity_scores), 1)
