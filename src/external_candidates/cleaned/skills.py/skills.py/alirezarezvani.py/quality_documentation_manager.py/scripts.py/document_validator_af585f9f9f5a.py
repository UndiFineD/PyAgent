# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\quality-documentation-manager\scripts\document_validator.py
#!/usr/bin/env python3
"""
...
class DocumentValidator:
    ...
    def _validate_approvals(self):
        ...
        if self.document.status in [
            ...
            DocumentStatus.EFFECTIVE.value,
        ]:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if critical:
            recommendations.append(f"URGENT: {len(critical)} critical finding(s) require immediate attention")

        # Major findings
        major = [f for f in self.findings if f.severity == Severity.MAJOR]
        if major:
            recommendations.append(f"ACTION: {len(major)} major finding(s) should be addressed within 30 days")

        # Review overdue
        review_overdue = [f for f in self.findings if f.rule == "DOC-DTE-003"]
        if review_overdue:
            recommendations.append("REVIEW: Document is overdue for periodic review. Initiate review process.")

        # Part 11 gaps
        p11_findings = [f for f in self.findings if f.rule.startswith("P11")]
        if p11_findings:
            recommendations.append(f"COMPLIANCE: {len(p11_findings)} 21 CFR Part 11 gap(s) identified")
...
def main():
    parser = argparse.ArgumentParser(description="Quality Documentation Validator")
    parser.add_argument("--doc", type=str, help="JSON file with document metadata")
    parser.add_argument("--output", choices=["text", "json"], default="text", help="Output format")
    parser.add_argument("--interactive", action="store_true", help="Run in interactive mode")
    parser.add_argument("--sample", action="store_true", help="Generate sample document JSON")
        ...
        sample = {
            ...
            "change_history": [
                ...
                },
                {
                    "revision": "03",
                    "date": "2024-01-15",
                    "description": "Added electronic signature requirements",
                },
            ],
