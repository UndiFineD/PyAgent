# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\app-store-optimization\scripts\keyword_analyzer.py
"""
...
class KeywordAnalyzer:
    ...
    def analyze_keyword(
        ...
        potential_score = self._calculate_potential_score(search_volume, competing_apps, relevance_score)
        ...
        analysis = {
            ...
            "recommendation": self._generate_recommendation(potential_score, difficulty_score, relevance_score),
    ...
    def compare_keywords(self, keywords_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Categorize keywords
        primary_keywords = [
            kw for kw in ranked_keywords if kw["potential_score"] >= 70 and kw["relevance_score"] >= 0.8
        ]

        secondary_keywords = [
            kw for kw in ranked_keywords if 50 <= kw["potential_score"] < 70 and kw["relevance_score"] >= 0.6
        ]

        long_tail_keywords = [kw for kw in ranked_keywords if kw["is_long_tail"] and kw["relevance_score"] >= 0.7]
        ...
        return {
            ...
            "summary": self._generate_comparison_summary(primary_keywords, secondary_keywords, long_tail_keywords),
        }

    def find_long_tail_opportunities(self, base_keyword: str, modifiers: List[str]) -> List[Dict[str, Any]]:
        """
    ...
    def extract_keywords_from_text(self, text: str, min_word_length: int = 3) -> List[Tuple[str, int]]:
        """
    ...
    def _calculate_potential_score(self, search_volume: int, competing_apps: int, relevance_score: float) -> float:
        """
    ...
    def _generate_recommendation(self, potential_score: float, difficulty_score: float, relevance_score: float) -> str:
    ...
    def _generate_comparison_summary(
        ...
        summary_parts.append(f"Found {len(secondary_keywords)} secondary keywords for description and metadata.")

        summary_parts.append(f"Discovered {len(long_tail_keywords)} long-tail opportunities with lower competition.")
