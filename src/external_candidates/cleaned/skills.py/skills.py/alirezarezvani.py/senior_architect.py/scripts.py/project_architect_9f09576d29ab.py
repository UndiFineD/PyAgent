...
class ProjectArchitect:
    ...
    def analyze(self) -> Dict:
        ...
        if self.verbose:
            print(
                f"Detected pattern: {pattern_result['detected_pattern']} (confidence: {pattern_result['confidence']}%)"
            )
        ...
        violation_detector = LayerViolationDetector(self.project_path, pattern_result["layer_assignments"])
    ...
    def _generate_recommendations(self, pattern_result: Dict, code_result: Dict, violations: List[Dict]) -> List[str]:
        ...
        if large_files:
            recommendations.append(f"Consider refactoring {len(large_files)} large file(s) into smaller modules")
        ...
        if pattern in ["layered", "clean", "hexagonal"]:
            ...
            if missing:
                recommendations.append(f"Consider adding missing architectural layer(s): {', '.join(missing)}")
