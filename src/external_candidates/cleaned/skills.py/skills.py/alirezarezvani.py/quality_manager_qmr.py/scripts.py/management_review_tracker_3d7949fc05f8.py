...
class ManagementReviewTracker:
    ...
    def check_input_readiness(self) -> Dict:
        ...
        readiness["readiness_score"] = round((readiness["complete"] / readiness["total_required"]) * 100, 1)
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if readiness["readiness_score"] < 100:
            recommendations.append(f"Complete remaining review inputs: {', '.join(readiness['missing_topics'])}")

        # Check actions
        action_analysis = self.analyze_actions()
        if action_analysis["overdue"]:
            recommendations.append(f"Address {len(action_analysis['overdue'])} overdue action(s) immediately")
        ...
        if self.review.metrics.capa_overdue > 0:
            recommendations.append(f"Expedite closure of {self.review.metrics.capa_overdue} overdue CAPA(s)")
...
def format_text_report(report: Dict) -> str:
    ...
    for metric in report["metrics_assessment"]["metrics"]:
        lines.append(f"{metric['name']:<25} {metric['value']:<10} {metric['target']:<10} {metric['status']:<10}")
