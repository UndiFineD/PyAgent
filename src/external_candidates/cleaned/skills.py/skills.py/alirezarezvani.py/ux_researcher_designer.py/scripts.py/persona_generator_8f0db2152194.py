# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\ux-researcher-designer\scripts\persona_generator.py
#!/usr/bin/env python3
"""
...
class PersonaGenerator:
    ...
    def generate_persona_from_data(self, user_data: List[Dict], interview_insights: List[Dict] = None) -> Dict:
    ...
    def _identify_archetype(self, patterns: Dict) -> str:
        ...
        freq_pattern = (
            max(patterns["usage_frequency"].items(), key=lambda x: x[1])[0] if patterns["usage_frequency"] else "medium"
        )
