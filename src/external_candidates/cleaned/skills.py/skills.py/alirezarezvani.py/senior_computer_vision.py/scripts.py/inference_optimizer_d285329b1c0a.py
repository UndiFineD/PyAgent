# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\senior-computer-vision\scripts\inference_optimizer.py
#!/usr/bin/env python3
"""
Inference Optimizer
...
class InferenceOptimizer:
    ...
    def benchmark(
        ...
        try:
            if self.model_format == "onnx":
                results.update(self._benchmark_onnx(input_size, batch_sizes, num_iterations, warmup))
            elif self.model_format == "pytorch":
                results.update(self._benchmark_pytorch(input_size, batch_sizes, num_iterations, warmup))
            else:
    ...
    def _benchmark_onnx(
        ...
        try:
            ...
            for batch_size in batch_sizes:
                ...
                logger.info(
                    f"Batch {batch_size}: {batch_result['mean_latency_ms']:.2f}ms, "
        ...
        if self.model_info:
            ...
            if file_size > 100:
                recommendations.append(
                    {
                        "step": "quantization",
                        "description": f"Model size is {file_size:.1f}MB. INT8 quantization can reduce by 75%.",
...
def main():
    parser = argparse.ArgumentParser(description="Analyze and optimize vision model inference")
    parser.add_argument("model_path", help="Path to model file")
    parser.add_argument("--analyze", action="store_true", help="Analyze model structure")
    ...
    parser.add_argument(
        "--target",
        choices=["gpu", "cpu", "edge", "mobile", "apple", "intel"],
        default="gpu",
