        ...
        if "annotations" in self.stats and "classes" in self.stats["annotations"]:
            ...
            if classes:
                ...
                if max_count > 0 and min_count / max_count < 0.1:
                    checks["warnings"].append(f"Severe class imbalance detected: ratio {min_count / max_count:.2%}")
                    checks["recommendations"].append("Consider oversampling minority classes or using focal loss")
                elif max_count > 0 and min_count / max_count < 0.3:
                    checks["warnings"].append(f"Moderate class imbalance: ratio {min_count / max_count:.2%}")

        # Check image count
        if self.stats.get("total_images", 0) < 100:
            checks["warnings"].append(f"Small dataset: only {self.stats.get('total_images', 0)} images")
            checks["recommendations"].append("Consider data augmentation or transfer learning")
...
class DatasetSplitter:
    ...
    def _stratified_split(self, images: List[Path], format: str, train: float, val: float, test: float) -> Dict:
...
class AugmentationConfigGenerator:
    ...
    @staticmethod
    def generate(task: str, intensity: str = "medium", framework: str = "albumentations") -> Dict:
    ...
    def _to_torchvision(config: Dict, task: str) -> Dict:
        ...
        for aug_name, params in config.items():
            if aug_name == "horizontal_flip":
                ...
                transforms.append({"type": "RandomRotation", "degrees": params.get("limit", 15)})
            ...
            try:
                ...
                for ann in data["annotations"]:
                    ...
                    if "bbox" in ann:
                        bbox = ann["bbox"]
                        if len(bbox) != 4:
                            results["errors"].append(f"Annotation {ann.get('id')}: Invalid bbox format")
                        elif any(v < 0 for v in bbox[:2]) or any(v <= 0 for v in bbox[2:]):
                            results["warnings"].append(f"Annotation {ann.get('id')}: Suspicious bbox values {bbox}")
        ...
        for txt_file in label_files:
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if len(parts) < 5:
                        results["errors"].append(f"{txt_file}:{line_num}: Expected 5 values, got {len(parts)}")
                        invalid_labels += 1
                        continue
                    ...
                    try:
                        ...
                        if not (0 < w <= 1 and 0 < h <= 1):
                            results["warnings"].append(f"{txt_file}:{line_num}: Size outside (0,1]: ({w}, {h})")

                        valid_labels += 1

                    except ValueError as e:
                        results["errors"].append(f"{txt_file}:{line_num}: Invalid values - {e}")
                        invalid_labels += 1
  ...
  Validate dataset:
    ...
    try:
        if args.command == "analyze":
            ...
            config = AugmentationConfigGenerator.generate(args.task, args.intensity, args.framework)
