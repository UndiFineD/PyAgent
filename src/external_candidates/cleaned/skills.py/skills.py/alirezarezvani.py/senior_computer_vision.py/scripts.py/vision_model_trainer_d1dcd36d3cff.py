# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\senior-computer-vision\scripts\vision_model_trainer.py
#!/usr/bin/env python3
"""
...
class VisionModelTrainer:
    ...
    def __init__(self, data_dir: str, task: str = "detection", framework: str = "ultralytics"):
    ...
    def analyze_dataset(self) -> Dict[str, Any]:
        ...
        if not self.data_dir.exists():
            analysis["recommendations"].append(f"Directory {self.data_dir} does not exist")
            return analysis
                ...
                if ann_file.exists():
                    with open(ann_file, "r") as f:
                        ...
                        if not analysis["annotations"]["classes"]:
                            analysis["annotations"]["classes"] = [c["name"] for c in data.get("categories", [])]
        ...
        if total_images < 100:
            analysis["recommendations"].append(
                f"Dataset has only {total_images} images. Consider collecting more data or using transfer learning."
            )
        if total_images < 1000:
            analysis["recommendations"].append("Use aggressive data augmentation (mosaic, mixup) for small datasets.")

        num_classes = len(analysis["annotations"]["classes"])
        if num_classes > 80:
            analysis["recommendations"].append(
                f"Large number of classes ({num_classes}). Consider using larger model (yolov8l/x) or longer training."
            )
    ...
    def generate_yolo_config(
        self, arch: str, epochs: int = 100, batch: int = 16, imgsz: int = 640, **kwargs
    ) -> Dict[str, Any]:
    ...
    def generate_detectron2_config(self, arch: str, epochs: int = 12, batch: int = 16, **kwargs) -> Dict[str, Any]:
    ...
    def generate_mmdetection_config(self, arch: str, epochs: int = 12, batch: int = 16, **kwargs) -> Dict[str, Any]:
        ...
        config = {
            ...
            "model": {"roi_head": {"bbox_head": {"num_classes": len(self._get_classes())}}},
            "data": {
    ...
    def generate_training_command(self) -> str:
        ...
        if self.framework == "ultralytics":
            return (
                f"yolo detect train data={self.config.get('data', 'data.yaml')} "
...
def main():
    parser = argparse.ArgumentParser(description="Generate vision model training configurations")
    ...
    parser.add_argument(
        "--framework",
        choices=["ultralytics", "detectron2", "mmdetection"],
        default="ultralytics",
        help="Training framework",
    )
    parser.add_argument("--arch", default="yolov8m", help="Model architecture")
    ...
    trainer = VisionModelTrainer(data_dir=args.data_dir, task=args.task, framework=args.framework)
    ...
    if args.analyze_only:
        if args.json:
            ...
            print(
                f"  Images - Train: {analysis['images']['train']}, "
    ...
    try:
        if args.framework == "ultralytics":
            config = trainer.generate_yolo_config(
                arch=args.arch, epochs=args.epochs, batch=args.batch, imgsz=args.imgsz
            )
        elif args.framework == "detectron2":
            config = trainer.generate_detectron2_config(arch=args.arch, epochs=args.epochs, batch=args.batch)
        elif args.framework == "mmdetection":
            config = trainer.generate_mmdetection_config(arch=args.arch, epochs=args.epochs, batch=args.batch)
