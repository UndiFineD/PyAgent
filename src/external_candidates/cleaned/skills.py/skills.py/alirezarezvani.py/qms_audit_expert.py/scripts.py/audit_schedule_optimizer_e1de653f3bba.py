# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\qms-audit-expert\scripts\audit_schedule_optimizer.py
#!/usr/bin/env python3
"""
    ...
    def _generate_recommendations(self, prioritized: List) -> List[str]:
        ...
        if overdue_count > 0:
            recommendations.append(
                f"URGENT: {overdue_count} process(es) overdue for audit. Prioritize these to maintain compliance."
            )
        ...
        if high_risk_count > 3:
            recommendations.append(
                f"High audit burden: {high_risk_count} high-risk processes. Consider quarterly resource allocation."
            )
        ...
        if finding_processes:
            ...
            recommendations.append(
                f"Recurring issues in: {names}. Consider focused audits or process improvement initiatives."
            )

        # Check for never-audited processes
        never_audited = [p.name for p, _, _ in prioritized if not p.last_audit_date]
        if never_audited:
            recommendations.append(f"Never audited: {', '.join(never_audited[:3])}. Include in next audit cycle.")
