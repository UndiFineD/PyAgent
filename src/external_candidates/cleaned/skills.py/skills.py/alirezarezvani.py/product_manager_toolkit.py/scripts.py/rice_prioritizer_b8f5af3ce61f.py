# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\product-manager-toolkit\scripts\rice_prioritizer.py
#!/usr/bin/env python3
"""
...
class RICECalculator:
    ...
    def analyze_portfolio(self, features: List[Dict]) -> Dict:
        ...
        quick_wins = [
            f
            for f in features
            if f.get("impact", "").lower() in ["massive", "high"] and f.get("effort", "").lower() in ["xs", "s"]
        ]
        ...
        big_bets = [
            f
            for f in features
            if f.get("impact", "").lower() in ["massive", "high"] and f.get("effort", "").lower() in ["l", "xl"]
        ]
