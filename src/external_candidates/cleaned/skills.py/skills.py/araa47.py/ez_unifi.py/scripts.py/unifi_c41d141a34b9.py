# Extracted from: C:\DEV\PyAgent\.external\skills\skills\araa47\ez-unifi\scripts\unifi.py
#!/usr/bin/env python3
# /// script
...
def output_table(items: list[Any], keys: list[str], headers: list[str] | None = None) -> None:
...
async def cmd_poe(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    response = await ctrl.request(DeviceSetPoePortModeRequest.create(device, port_idx=args.port_idx, mode=args.mode))
    output(response, raw=True)
...
async def cmd_power_cycle(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    mac = args.mac.lower()
    response = await ctrl.request(DevicePowerCyclePortRequest.create(mac, args.port_idx))
    output(response, raw=True)
...
async def cmd_port_enable(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    response = await ctrl.request(DeviceSetPortEnabledRequest.create(device, port_idx=args.port_idx, enabled=True))
    output(response, raw=True)
...
async def cmd_port_disable(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    response = await ctrl.request(DeviceSetPortEnabledRequest.create(device, port_idx=args.port_idx, enabled=False))
    output(response, raw=True)
...
async def cmd_outlet(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    response = await ctrl.request(DeviceSetOutletRelayRequest.create(device, args.idx, state))
    output(response, raw=True)
...
async def cmd_outlet_cycle(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    state = args.state.lower() == "on"
    response = await ctrl.request(DeviceSetOutletCycleEnabledRequest.create(device, args.idx, state))
    output(response, raw=True)
...
async def cmd_wlan_password(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    response = await ctrl.request(WlanChangePasswordRequest.create(args.id, args.password))
    output(response, raw=True)
...
async def cmd_raw(ctrl: Controller, args: argparse.Namespace) -> None:
    ...
    class RawRequest(ApiRequest):
        @classmethod
        def create(cls, method: str, path: str, data: dict[str, Any] | None = None) -> "RawRequest":
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("mac", help="Switch MAC address")
    p.add_argument("port_idx", type=int, help="Port index")
    p.add_argument("mode", choices=["auto", "off", "passthrough", "24v"], help="PoE mode")
    ...
    p.add_argument("method", choices=["GET", "POST", "PUT", "DELETE"], help="HTTP method")
