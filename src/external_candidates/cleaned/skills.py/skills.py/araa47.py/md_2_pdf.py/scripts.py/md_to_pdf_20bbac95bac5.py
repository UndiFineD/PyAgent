# Extracted from: C:\DEV\PyAgent\.external\skills\skills\araa47\md-2-pdf\scripts\md-to-pdf.py
    ...
    for block_type, content in blocks:
        try:
            if block_type in (
                ...
                code_para = Paragraph(f'<font face="Courier" size="8">{code}</font>', styles["CodeBlock"])
                ...
                row_paras = [
                    [Paragraph(process_inline_formatting(cell), styles["TableCell"]) for cell in row] for row in rows
                ]
                ...
                if full_img_path.exists():
                    try:
                        ...
                        if alt_text:
                            story.append(
                                Paragraph(
                                    f'<font size="8" color="#666666"><i>{alt_text}</i></font>',
                                    styles["Normal"],
                                )
                            )
                        story.append(Spacer(1, 0.15 * inch))
                    ...
                    story.append(
                        Paragraph(
                            f'<font color="red">[Image not found: {img_path}]</font>',
                            styles["Normal"],
                        )
                    )
