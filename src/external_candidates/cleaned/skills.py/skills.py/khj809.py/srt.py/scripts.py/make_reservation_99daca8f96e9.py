# Extracted from: C:\DEV\PyAgent\.external\skills\skills\khj809\srt\scripts\make_reservation.py
#!/usr/bin/env python3
"""
...
def make_reservation_with_retry(credentials, args):
    ...
    if args.train_id:
        ...
        for tid in train_ids:
            ...
            if train_idx < 0 or train_idx >= len(all_trains):
                print(
                    f"❌ 잘못된 열차 번호: {tid} (1부터 {len(all_trains)} 사이여야 함)",
                    file=sys.stderr,
                )
                return 2
    ...
    while True:
        ...
        if logger:
            ...
            logger.log(
                f"열차: {train.train_number} ({train.dep_time} → {train.arr_time})",
                "INFO",
            )
...
def main():
    ...
    parser.add_argument("--wait-seconds", type=int, default=10, help="재시도 대기 시간 (초, 기본값: 10)")
