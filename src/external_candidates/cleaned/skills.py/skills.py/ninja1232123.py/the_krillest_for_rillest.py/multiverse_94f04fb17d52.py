...
def render_branch_card(branch: UniverseBranch, index: int = None) -> str:
    ...
    lines = [header]
    lines.append(
        f"{border_color}║{C.END} {idx_str}{C.BOLD}{branch.branch_id}{C.END}  {type_color}[{branch.branch_type.value}]{C.END}".ljust(
            70
        )
        + f"{border_color}║{C.END}"
    )
    ...
    # Divergence point
    lines.append(
        f"{border_color}║{C.END} {C.DIM}Diverged when:{C.END} {branch.divergence_point[:40]}".ljust(70)
        + f"{border_color}║{C.END}"
    )
    lines.append(f"{border_color}╠{'─' * 58}╣{C.END}")
    ...
    lines.append(
        f"{border_color}║{C.END} {physics_bar(branch.physics.gravity, 'Gravity    ')}".ljust(70)
        + f"{border_color}║{C.END}"
    )
    lines.append(
        f"{border_color}║{C.END} {physics_bar(branch.physics.atomic_binding, 'Chemistry  ')}".ljust(70)
        + f"{border_color}║{C.END}"
    )
    lines.append(f"{border_color}╠{'─' * 58}╣{C.END}")
    ...
    if branch.physics.stable:
        lines.append(
            f"{border_color}║{C.END} {C.CYAN}Age:{C.END} {branch.age_billion_years:.1f} Gyr   {C.CYAN}Stars:{C.END} {format_big_number(branch.stars)}".ljust(
                70
            )
            + f"{border_color}║{C.END}"
        )
        lines.append(
            f"{border_color}║{C.END} {C.GREEN}Habitable:{C.END} {format_big_number(branch.habitable_worlds)}   {C.GREEN}With Life:{C.END} {format_big_number(branch.life_bearing_worlds)}".ljust(
                70
            )
            + f"{border_color}║{C.END}"
        )
        ...
        if branch.civilizations > 0:
            lines.append(
                f"{border_color}║{C.END} {C.YELLOW}Civilizations:{C.END} {format_big_number(branch.civilizations)}".ljust(
                    70
                )
                + f"{border_color}║{C.END}"
            )
            lines.append(
                f"{border_color}║{C.END} {C.YELLOW}Dominant:{C.END} {branch.dominant_life_form[:35]}".ljust(70)
                + f"{border_color}║{C.END}"
            )
            lines.append(
                f"{border_color}║{C.END} {C.YELLOW}Tech Level:{C.END} {branch.technology_level[:35]}".ljust(70)
                + f"{border_color}║{C.END}"
            )
    else:
        lines.append(
            f"{border_color}║{C.END} {C.RED}Universe collapsed - incompatible physics{C.END}".ljust(70)
            + f"{border_color}║{C.END}"
        )
    ...
    lines.append(
        f"{border_color}║{C.END} {C.DIM}Fate:{C.END} {fate_color}{branch.fate[:45]}{C.END}".ljust(70)
        + f"{border_color}║{C.END}"
    )
    ...
    if branch.special_note:
        ...
        lines.append(
            f"{border_color}║{C.END} {note_color}✦ {branch.special_note[:50]}{C.END}".ljust(70)
            + f"{border_color}║{C.END}"
        )

    if branch.has_contacted_other_branches:
        lines.append(
            f"{border_color}║{C.END} {C.PURPLE}★ Has made contact with other branches!{C.END}".ljust(70)
            + f"{border_color}║{C.END}"
        )
