    ...
    def _generate_noise(self) -> str:
        ...
        for _ in range(random.randint(30, 50)):
            ...
            if choice < 0.4:
                ...
                elements.append(
                    str(random.choice(SIGNIFICANT_NUMBERS) if random.random() < 0.3 else random.randint(0, 999))
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Find patterns in noise (or anywhere else)")
    parser.add_argument(
        "input",
        nargs="?",
        help="Text to analyze for patterns (or leave empty for generated noise)",
    )
    parser.add_argument("--depth", "-d", type=int, default=3, help="Perception depth 1-5 (default: 3)")
    parser.add_argument("--seed", "-s", type=int, help="Random seed for reproducible patterns")
    parser.add_argument("--no-animate", "-q", action="store_true", help="Disable animations")
