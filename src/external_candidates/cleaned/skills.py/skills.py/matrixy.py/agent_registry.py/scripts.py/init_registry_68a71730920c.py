...
def build_page_choices(
    ...
    for category, agent in page_items:
        ...
        choices.append(questionary.Choice(title=label, value=agent, checked=page_state.is_selected(agent["name"])))
...
def interactive_selection_paged(agents: List[Dict]) -> List[Dict]:
    ...
    while True:
        try:
            ...
            selected = questionary.checkbox(message="Select agents (Space=toggle, Enter=next)", choices=choices).ask()
            ...
            if page_select_all:
                ...
                # Remove deselections
                deselected = [
                    a for a in page_agents if a["name"] not in selected_names and page_state.is_selected(a["name"])
                ]
