...
def extract_json(url: str, schema: Dict[str, Any], api_key: Optional[str] = None) -> Dict[str, Any]:
    ...
    response = requests.post(f"{TABSTACK_BASE_URL}/extract/json", headers=headers, json=payload, timeout=30)
...
def extract_markdown(url: str, api_key: Optional[str] = None) -> str:
    ...
    response = requests.post(
        f"{TABSTACK_BASE_URL}/extract/markdown",
        headers=headers,
        json=payload,
        timeout=30,
    )
