# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jabbslad\uk-trains\scripts\trains.py
#!/usr/bin/env python3
...
def parse_service(svc) -> dict:
    ...
    # Get destination(s)
    destinations = []
    for dest in svc.findall(".//lt5:destination/lt4:location", NS) or svc.findall(
        ".//lt4:destination/lt4:location", NS
    ):
        destinations.append(
            {
    ...
    coaches = svc.findall(".//lt7:coach", NS) or svc.findall(
        ".//{http://thalesgroup.com/RTTI/2017-10-01/ldb/types}coach"
    )
...
def cmd_departures(args):
    ...
    if not station:
        print(
            "Usage: trains.py departures <station> [to <dest>] [--rows N]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_arrivals(args):
    ...
    if not station:
        print(
            "Usage: trains.py arrivals <station> [from <origin>] [--rows N]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    if not TOKEN:
        print(
            json.dumps(
                {
                    "error": "NATIONAL_RAIL_TOKEN not set. Register at https://realtime.nationalrail.co.uk/OpenLDBWSRegistration/"
                }
            )
        )
        sys.exit(1)
