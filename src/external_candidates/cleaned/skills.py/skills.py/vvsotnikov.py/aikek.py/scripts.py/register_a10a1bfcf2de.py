# Extracted from: C:\DEV\PyAgent\.external\skills\skills\vvsotnikov\aikek\scripts\register.py
#!/usr/bin/env python3
...
try:
    ...
    print(
        "Missing dependencies. Install with: pip install solders requests",
        file=sys.stderr,
    )
    sys.exit(1)
...
def load_config():
    ...
    print(
        "Error: AIKEK_PRIVATE_KEY not found in environment or ~/.aikek/config",
        file=sys.stderr,
    )
    sys.exit(1)
...
def main():
    ...
    message = (
        f"Sign this message to authenticate with AIKEK API.\n\nAddress: {keypair.pubkey()}\nTimestamp: {timestamp}"
    )
