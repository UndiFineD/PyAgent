# Extracted from: C:\DEV\PyAgent\.external\skills\skills\red777777\inkedin-automation-that-really-works\scripts\lib\style_learner.py
#!/usr/bin/env python3
...
def _save_style(style):
    os.makedirs(
        os.path.dirname(STYLE_FILE) if os.path.dirname(STYLE_FILE) else ".",
        exist_ok=True,
    )
...
def learn_profile(post_count=15):
    ...
    with browser_session() as (ctx, page):
        ...
        page.goto(
            f"https://www.linkedin.com/in/{slug}/recent-activity/comments/",
            wait_until="domcontentloaded",
            timeout=30000,
        )
        time.sleep(5)
        ...
        style = {
            ...
            "tone": (
                "casual"
                if casual_markers > 3 or exclamation_count > len(all_content)
                else "professional"
                if casual_markers == 0
