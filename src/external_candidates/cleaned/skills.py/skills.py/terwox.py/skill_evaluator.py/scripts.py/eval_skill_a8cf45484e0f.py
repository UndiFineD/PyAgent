# Extracted from: C:\DEV\PyAgent\.external\skills\skills\terwox\skill-evaluator\scripts\eval-skill.py
#!/usr/bin/env python3
...
def check_name_matches_dir(skill_path):
    ...
    return CheckResult(
        "Skill name matches directory",
        CheckResult.WARN,
        f"name='{name}' but directory='{dir_name}'",
        category="structure",
    )
...
def check_body_length(skill_path):
    ...
    if body_lines < 10:
        return CheckResult(
            "SKILL.md body length",
            CheckResult.FAIL,
            f"Only {body_lines} lines — too short to be useful",
            category="documentation",
        )
    if body_lines > 500:
        return CheckResult(
            "SKILL.md body length",
            CheckResult.WARN,
            f"{body_lines} lines — consider splitting into reference files",
            category="documentation",
        )
    return CheckResult(
        "SKILL.md body length",
...
def check_references_linked(skill_path):
    ...
    if unlinked:
        return CheckResult(
            "References are linked from SKILL.md",
            CheckResult.WARN,
            f"Unlinked references: {', '.join(unlinked)}",
            category="documentation",
        )
    return CheckResult(
        "References are linked from SKILL.md",
...
def check_env_vars_documented(skill_path):
    ...
    return CheckResult(
        "Environment variables documented",
        CheckResult.PASS,
        f"All {len(env_vars)} env vars documented",
        category="security",
    )
...
def run_checks(skill_path, verbose=False):
    ...
    for check_fn in ALL_CHECKS:
        try:
            ...
            results.append(
                CheckResult(
                    check_fn._check_name,
                    CheckResult.FAIL,
                    f"Check crashed: {e}",
                    check_fn._check_category,
                )
            )
    return results
...
def print_report(results, skill_path, verbose=False):
    ...
    print(f"{'=' * 50}")
    print(
        f"  ✅ Pass: {counts[CheckResult.PASS]}  "
