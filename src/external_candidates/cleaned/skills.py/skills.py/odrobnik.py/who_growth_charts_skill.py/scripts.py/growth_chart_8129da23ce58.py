...
def _get_who_emblem_img():
    ...
    if not emblem_path.exists():
        ...
        req = urllib.request.Request(
            WHO_EMBLEM_URL,
            headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Clawdbot/1.0"},
        )
        try:
...
def plot_growth_chart(child_name, birthdate, sex, heights, weights, chart_type="height", output_dir=None):
    ...
    ax.xaxis.set_major_formatter(
        FuncFormatter(lambda x, pos: f"{int(x / 12)}" if x / 12 == int(x / 12) else f"{x / 12:.1f}")
    )
...
def main():
    ...
    for ct in chart_types:
        ...
        output = plot_growth_chart(args.name, args.birthdate, args.sex, heights, weights, ct, args.output)
        print(f"✓ {output}")
