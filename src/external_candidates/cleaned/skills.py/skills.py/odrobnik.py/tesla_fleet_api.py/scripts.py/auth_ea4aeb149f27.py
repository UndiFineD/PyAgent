# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\tesla-fleet-api\scripts\auth.py
#!/usr/bin/env python3
...
from store import default_dir, ensure_migrated
from store import env as _env
from store import get_auth, get_config, load_env_file, save_auth, save_config
...
def load_runtime(dir_path: str, *, args: argparse.Namespace) -> TeslaRuntime:
    ...
    redirect_uri = (
        args.redirect_uri or cfg.get("redirect_uri") or _env("TESLA_REDIRECT_URI") or DEFAULT_REDIRECT_URI
    ).strip()
...
def cmd_login(rt: TeslaRuntime, dir_path: str) -> int:
    if not rt.client_id:
        print(
            "Missing client_id (set TESLA_CLIENT_ID in ~/.clawdbot/tesla-fleet-api/.env)",
            file=sys.stderr,
        )
        return 1
...
def cmd_exchange(rt: TeslaRuntime, dir_path: str, code: str) -> int:
    if not rt.client_id or not rt.client_secret:
        print(
            "Missing client_id/client_secret (set TESLA_CLIENT_ID and TESLA_CLIENT_SECRET in .env)",
            file=sys.stderr,
        )
        return 1
