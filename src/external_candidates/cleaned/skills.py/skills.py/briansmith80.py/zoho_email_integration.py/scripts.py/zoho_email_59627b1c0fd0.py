...
class ZohoRestAPIClient:
    ...
    def get_message(self, message_id):
        ...
        response = self._make_request("GET", f"/accounts/{account_id}/messages/{message_id}")
    ...
    def send_message(self, to, subject, body, html_body=None, cc=None, bcc=None):
        ...
        response = self._make_request("POST", f"/accounts/{account_id}/messages", json=payload)
    ...
    def bulk_action(self, query, action, folder="INBOX", limit=100, search_days=None, dry_run=False):
        ...
        if self.api_mode == "rest":
            try:
                ...
                messages = self.rest_client.list_messages(folder=folder, limit=limit, search_query=rest_query)
...
if __name__ == "__main__":
    ...
    if command == "doctor":
        ...
        if report["token_file_exists"]:
            try:
                ...
                report["oauth_token_expired_or_expiring"] = (
                    is_token_expired(tokens) if report["oauth_access_token_present"] else None
                )
                ...
                report["oauth_email_in_token"] = bool(
                    tokens.get("email") or tokens.get("user") or tokens.get("user_email")
                )
            except Exception as e:
        ...
        report["rest_reachable"] = http_ok(report["rest_base_url"].rstrip("/") + "/accounts") or http_ok(
            "https://mail.zoho.com/"
        )
