    ...
    def _extract_text_from_blocks(self, blocks: Dict[str, Any]) -> str:
        ...
        def process_block(block):
            ...
            if block_type == "text":
                ...
                text_parts.append(
                    "# " + self._extract_text_from_elements(block.get("heading1", {}).get("elements", []))
                )
            elif block_type == "heading2":
                text_parts.append(
                    "## " + self._extract_text_from_elements(block.get("heading2", {}).get("elements", []))
                )
            elif block_type == "heading3":
                text_parts.append(
                    "### " + self._extract_text_from_elements(block.get("heading3", {}).get("elements", []))
                )
            elif block_type == "bullet":
                text_parts.append("- " + self._extract_text_from_elements(block.get("bullet", {}).get("elements", [])))
            elif block_type == "ordered":
                text_parts.append(
                    "1. " + self._extract_text_from_elements(block.get("ordered", {}).get("elements", []))
                )
