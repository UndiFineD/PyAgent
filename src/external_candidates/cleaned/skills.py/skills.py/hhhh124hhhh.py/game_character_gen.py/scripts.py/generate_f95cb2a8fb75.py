# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hhhh124hhhh\game-character-gen\scripts\generate.py
#!/usr/bin/env python3
...
def _generate_character_prompt(
    ...
    character_data = {
        ...
        "gender": gender_expr,
        "equipment": (
            equipment.split(", ") if equipment else (_default_equipment_list(char_class) if char_class else [])
        ),
...
def _race_word(race: str, gender: str | None) -> str:
    ...
    gender_map = {
        "male": {
            "elf": "elven man",
            "dwarf": "dwarven man",
            "halfling": "halfling man",
        },
        "female": {
            "elf": "elven woman",
            "dwarf": "dwarven woman",
            "halfling": "halfling woman",
        },
        "non-binary": {
            "elf": "elven person",
            "dwarf": "dwarven person",
            "halfling": "halfling person",
        },
    }
...
def _generate_batch_prompts(
    ...
    if races and classes:
        ...
        for race in races:
            for char_class in classes:
                for _ in range(count_per):
                    ...
                    prompt, char_data = _generate_character_prompt(race, char_class, gender, equipment, style, expr)
        ...
        for race in races:
            for _ in range(count_per):
                expr = random.choice(expressions)
                prompt, char_data = _generate_character_prompt(race, "", gender, equipment, style, expr)
                prompts.append((prompt, char_data))
    elif classes:
        # Only classes
        for char_class in classes:
            for _ in range(count_per):
                expr = random.choice(expressions)
                prompt, char_data = _generate_character_prompt("", char_class, gender, equipment, style, expr)
                prompts.append((prompt, char_data))
...
def _write_index(out_dir: str, items: list[dict]) -> None:
    ...
    for it in items:
        ...
        if it.get("character"):
            ...
            meta = ", ".join(f"{k}: {v}" for k, v in char.items() if k != "equipment" and v and v != "unspecified")
            if meta:
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument(
        ...
        default=None,
        dest="char_class",
        help="Character class (repeatable)",
    )
    ...
    if args.prompt:
        ...
        prompts_with_data = _generate_batch_prompts(races, classes, args.style, args.equipment, args.gender, args.count)
    else:
