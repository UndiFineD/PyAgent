# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hhhh124hhhh\style-transfer\scripts\transfer.py
#!/usr/bin/env python3
...
def _load_style_prompts() -> dict[str, str]:
    ...
    return {
        ...
        "polaroid": (
            "instant polaroid look, soft faded tones, white borders, vintage 1970s aesthetic, nostalgic warm colors"
        ),
        "film-noir": (
        ...
        "candid-snapshot": (
            "authentic snapshot look, natural lighting, slight motion blur, unposed composition, genuine moment capture"
        ),
        "studio-portrait": (
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument(
        ...
        action="append",
        default=None,
        help="Pre-defined style name (repeatable)",
    )
    p.add_argument(
        "--prompt",
        action="append",
        default=None,
        help="Full custom style prompt (repeatable)",
    )
    p.add_argument(
        "--size",
        default="1024x1024",
        help="Image size: 1024x1024, 1792x1024, 1024x1792",
    )
    ...
    if args.prompt:
        ...
        for style_name in args.style:
            if style_name not in style_prompts:
                print(
                    f"warning: unknown style '{style_name}' (using as custom prompt)",
                    file=sys.stderr,
                )
    ...
    for i, (prompt, style_name) in enumerate(prompts, 1):
        ...
        if not b64:
            ...
            data = _post_json(
                url=url.replace("/edits", "/generations"),
                api_key=api_key,
                payload=payload,
