# Extracted from: C:\DEV\PyAgent\.external\skills\skills\anthonyfrancis\daily-rhythm\scripts\sync-stripe-arr.py
#!/usr/bin/env python3
"""
...
def fetch_active_subscriptions():
    ...
    try:
        subscriptions = stripe.Subscription.list(status="active", limit=100, expand=["data.customer"])
...
def calculate_arr(subscriptions):
    ...
    for sub in subscriptions:
        ...
        interval = (
            sub.get("items", {}).get("data", [{}])[0].get("price", {}).get("recurring", {}).get("interval", "month")
        )
