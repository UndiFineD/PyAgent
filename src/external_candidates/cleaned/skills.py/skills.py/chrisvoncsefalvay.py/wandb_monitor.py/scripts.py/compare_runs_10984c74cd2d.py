# Extracted from: C:\DEV\PyAgent\.external\skills\skills\chrisvoncsefalvay\wandb-monitor\scripts\compare_runs.py
#!/usr/bin/env python3
"""
...
def print_comparison(run_a, run_b, history_a: list, history_b: list, config_diff: dict):
    ...
    print("-" * 70)
    grads_a = [
        get_metric(r, "train/grad_norm", "grad_norm")
        for r in history_a
        if get_metric(r, "train/grad_norm", "grad_norm")
    ]
    grads_b = [
        get_metric(r, "train/grad_norm", "grad_norm")
        for r in history_b
    ...
    if args.json:
        ...
        output = {
            "run_a": {
                "id": run_a.id,
                "name": run_a.name,
                "project": run_a.project,
                "state": run_a.state,
            },
            "run_b": {
                "id": run_b.id,
                "name": run_b.name,
                "project": run_b.project,
                "state": run_b.state,
            },
