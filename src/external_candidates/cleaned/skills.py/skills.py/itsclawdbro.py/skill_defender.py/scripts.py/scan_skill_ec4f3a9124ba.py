# Extracted from: C:\DEV\PyAgent\.external\skills\skills\itsclawdbro\skill-defender\scripts\scan_skill.py
#!/usr/bin/env python3
"""
...
PROMPT_INJECTION_PATTERNS = [
    ...
    (
        re.compile(r"you\s+are\s+now\s+(?!going|able|ready)", re.I),
        "critical",
        "prompt_injection",
        "likely",
    ...
    (
        re.compile(
            r"override\s+your\s+(instructions|rules|guidelines|safety|constraints)",
            re.I,
        ),
        "critical",
        "prompt_injection",
    ...
    (
        re.compile(r"from\s+now\s+on,?\s+(you\s+are|act\s+as|pretend|behave)", re.I),
        "critical",
        "prompt_injection",
        "likely",
    ...
    (
        re.compile(r"system\s*:\s*(you\s+are|ignore|forget|override)", re.I),
        "critical",
        "prompt_injection",
        "certain",
...
DECEPTION_PATTERNS = [
    ...
    (
        re.compile(r"hide\s+this\s+from\s+(the\s+)?user", re.I),
        "critical",
        "deception",
        "certain",
    ...
    (
        re.compile(r"don'?t\s+mention\s+(this|that|the\s+following)\s+to", re.I),
        "critical",
        "deception",
        "certain",
    ...
    (
        re.compile(r"keep\s+this\s+(a\s+)?secret", re.I),
        "critical",
        "deception",
        "certain",
    ...
    (
        re.compile(r"never\s+(reveal|disclose|share|show)\s+(this|that|the)", re.I),
        "critical",
        "deception",
        "likely",
...
RCE_PATTERNS = [
    ...
    (
        re.compile(r"bash\s*<\s*\(\s*curl", re.I),
        "critical",
        "rce",
        "certain",
...
CREDENTIAL_THEFT_PATTERNS = [
    (
        re.compile(r"~/\.clawdbot/credentials", re.I),
        "critical",
        "credential_theft",
        "certain",
    ...
    (
        re.compile(r"~/\.clawdbot/clawdbot\.json", re.I),
        "critical",
        "credential_theft",
        "certain",
    ...
    (
        re.compile(r"auth-profiles\.json", re.I),
        "critical",
        "credential_theft",
        "certain",
    ...
    (
        re.compile(r"CLAWDBOT_GATEWAY_TOKEN", re.I),
        "critical",
        "credential_theft",
        "certain",
    ...
    (
        re.compile(r"(API_SECRET|SECRET_KEY|AUTH_TOKEN|ACCESS_TOKEN)\b", re.I),
        "high",
        "credential_theft",
        "likely",
...
CREDENTIAL_EXFIL_PATTERNS = [
    (
        re.compile(r"os\.environ\s*\[.*(?:KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL)", re.I),
        "high",
        "credential_theft",
        "likely",
    ...
    (
        re.compile(r"os\.environ\.get\s*\(.*(?:KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL)", re.I),
        "high",
        "credential_theft",
        "likely",
        "Reads secret-looking environment variable",
...
OBFUSCATION_PATTERNS = [
    ...
    (
        re.compile(r"(?<!sha512-)(?<!sha256-)(?<!sha384-)[A-Za-z0-9+/]{50,}={0,2}(?!\.tgz)"),
        "high",
        "obfuscation",
        "possible",
    ...
    (
        re.compile(r"chr\s*\(\s*\d+\s*\)\s*(?:\+\s*chr\s*\(\s*\d+\s*\)){5,}"),
        "high",
        "obfuscation",
        "likely",
    ...
    (
        re.compile(r"String\.fromCharCode\s*\([^)]{30,}\)"),
        "high",
        "obfuscation",
        "likely",
    ...
    (
        re.compile(r"codecs\.decode\s*\([^)]*,\s*['\"]rot", re.I),
        "high",
        "obfuscation",
        "likely",
...
EXFILTRATION_PATTERNS = [
    ...
    (
        re.compile(r"requests\.put\s*\(", re.I),
        "high",
        "exfiltration",
        "possible",
    ...
    (
        re.compile(r"urllib\.request\.urlopen.*data=", re.I),
        "high",
        "exfiltration",
        "possible",
    ...
    (
        re.compile(r"fetch\s*\([^)]*method\s*:\s*['\"]POST", re.I),
        "high",
        "exfiltration",
        "possible",
    ...
    (
        re.compile(r"httpx?\.(post|put)\s*\(", re.I),
        "high",
        "exfiltration",
        "possible",
...
PERSISTENCE_PATTERNS = [
    (
        re.compile(
            r"(modify|edit|write|append|change|update|overwrite)\s+(to\s+)?(SOUL\.md|AGENTS\.md|MEMORY\.md)",
            re.I,
        ),
        "high",
        "persistence",
    ...
    (
        re.compile(
            r"(modify|edit|write|append|change|update|overwrite)\s+(to\s+)?(clawdbot\.json|config\s+files?)",
            re.I,
        ),
        "high",
        "persistence",
    ...
    (
        re.compile(
            r"(add|insert|inject)\s+(to|into)\s+(SOUL\.md|AGENTS\.md|MEMORY\.md|TOOLS\.md)",
            re.I,
        ),
        "high",
        "persistence",
    ...
    (
        re.compile(r"echo\s+.*>>\s*(~/|.*SOUL|.*AGENTS|.*MEMORY)", re.I),
        "high",
        "persistence",
        "certain",
...
DYNAMIC_EXEC_PATTERNS = [
    ...
    (
        re.compile(r"subprocess\.call\s*\(.*\bshell\s*=\s*True", re.I | re.S),
        "medium",
        "dynamic_execution",
        "likely",
    ...
    (
        re.compile(r"subprocess\.(Popen|run|call)\s*\(.*\+", re.S),
        "medium",
        "dynamic_execution",
        "possible",
...
PRIVESC_PATTERNS = [
    ...
    (
        re.compile(r"chmod\s+[0-7]*[67][0-7]{2}\b"),
        "medium",
        "privilege_escalation",
        "possible",
    ...
    (
        re.compile(r"setuid|setgid|SUID", re.I),
        "medium",
        "privilege_escalation",
        "likely",
...
BACKDOOR_PATTERNS = [
    (
        re.compile(r"http\.server|HTTPServer|BaseHTTPRequestHandler", re.I),
        "medium",
        "backdoor",
        "possible",
    ...
    (
        re.compile(r"reverse\s*shell|bind\s*shell|nc\s+-[el]", re.I),
        "critical",
        "backdoor",
        "certain",
    ...
    (
        re.compile(r"ngrok|localtunnel|serveo", re.I),
        "high",
        "backdoor",
        "likely",
...
SCOPE_CREEP_PATTERNS = [
    ...
    (
        re.compile(
            r"(/Users/|/home/|/root/|~/)[^\s]*\.(json|yaml|yml|env|key|pem|crt|conf)",
            re.I,
        ),
        "medium",
        "scope_creep",
    ...
    (
        re.compile(r'open\s*\(\s*["\']/(etc|Users|home|root|tmp)/', re.I),
        "medium",
        "scope_creep",
        "likely",
    ...
    (
        re.compile(r"Path\s*\(\s*[\"']/(etc|Users|home|root)/", re.I),
        "medium",
        "scope_creep",
        "likely",
...
class SkillScanner:
    ...
    _SAFE_BASE64_LINE_RE = re.compile(r'"integrity"\s*:|sha(256|384|512)-|data:image/|\.woff|\.ttf|\.eot', re.I)
...
def format_human(result: dict[str, Any]) -> str:
    ...
    clean_files = summary.get("files_scanned", 0) - len(set(f["file"] for f in findings))

    lines.append(f"🔴 Critical: {crit}  🟠 High: {high}  🟡 Medium: {med}  ✅ Clean files: {max(0, clean_files)}")
    lines.append(f"Files scanned: {summary.get('files_scanned', 0)}  Lines scanned: {summary.get('lines_scanned', 0)}")
    lines.append("═" * 43)
