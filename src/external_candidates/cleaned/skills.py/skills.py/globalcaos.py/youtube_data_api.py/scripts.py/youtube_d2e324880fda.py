...
def cmd_subscriptions(args):
    ...
    request = youtube.subscriptions().list(part="snippet", mine=True, maxResults=args.limit)
...
def cmd_playlists(args):
    """List playlists."""
    youtube = get_youtube_service()
    request = youtube.playlists().list(part="snippet,contentDetails", mine=True, maxResults=args.limit)
    response = request.execute()
...
def cmd_playlist_items(args):
    ...
    youtube = get_youtube_service()
    request = youtube.playlistItems().list(part="snippet", playlistId=args.playlist_id, maxResults=args.limit)
    response = request.execute()
...
def cmd_search(args):
    """Search YouTube."""
    youtube = get_youtube_service()
    request = youtube.search().list(part="snippet", q=args.query, type="video", maxResults=args.limit)
    response = request.execute()
...
def cmd_video(args):
    """Get video details."""
    youtube = get_youtube_service()
    request = youtube.videos().list(part="snippet,contentDetails,statistics", id=args.video_id)
    response = request.execute()
...
def cmd_liked(args):
    """List liked videos."""
    youtube = get_youtube_service()
    request = youtube.videos().list(part="snippet", myRating="like", maxResults=args.limit)
    response = request.execute()
