# Extracted from: C:\DEV\PyAgent\.external\skills\skills\aviv4339\indirect-prompt-injection\scripts\run_tests.py
#!/usr/bin/env python3
"""
...
from sanitize import determine_risk_level, sanitize
...
def run_all_tests(test_cases: list, category_filter: str = None, verbose: bool = False) -> dict:
...
def main():
    parser = argparse.ArgumentParser(description="Run prompt injection detection tests")
    parser.add_argument(
        "--verbose",
        "-v",
        action="store_true",
    ...
    parser.add_argument("--category", "-c", help="Filter tests by category")
    parser.add_argument("--json", "-j", action="store_true", help="Output as JSON")
    parser.add_argument("--test-file", "-f", default=None, help="Path to test cases JSON file")
    ...
    results = run_all_tests(data["test_cases"], category_filter=args.category, verbose=args.verbose)
