# Extracted from: C:\DEV\PyAgent\.external\skills\skills\aviv4339\indirect-prompt-injection\scripts\sanitize.py
#!/usr/bin/env python3
"""
...
def detect_patterns(text: str) -> tuple[list, int]:
    ...
    for name, config in PATTERNS.items():
        ...
        if matches:
            ...
            unique_matches = list(set(m if isinstance(m, str) else (m[0] if m else "") for m in matches))[:3]
...
def main():
    parser = argparse.ArgumentParser(description="Sanitize content and detect prompt injection attempts")
    parser.add_argument("--file", "-f", help="Read content from file")
    parser.add_argument("--analyze", "-a", help="Analyze content string directly")
