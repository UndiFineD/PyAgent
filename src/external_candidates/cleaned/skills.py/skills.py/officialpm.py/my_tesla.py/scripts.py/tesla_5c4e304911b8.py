...
def get_vehicle(tesla, name: str = None):
    ...
    if target_name:
        ...
        if s and not s.isdigit():
            s_l = s.lower()
            matches = [(i + 1, v) for i, v in enumerate(vehicles) if s_l in v.get("display_name", "").lower()]
            ambiguous = len(matches) > 1

        options = "\n".join(f"   {i + 1}. {v.get('display_name')}" for i, v in enumerate(vehicles))

        if ambiguous:
            match_lines = "\n".join(f"   {idx}. {v.get('display_name')}" for idx, v in matches)
            print(
...
def _report(vehicle, data):
    ...
    if any([fl, fr, rl, rr]):
        lines.append(f"Tires (TPMS): FL {fl or '(?)'} | FR {fr or '(?)'} | RL {rl or '(?)'} | RR {rr or '(?)'}")
...
def mileage_init_db(path: Path):
    ...
    try:
        conn.execute("""
            ...
            """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_mileage_points_vehicle_ts ON mileage_points(vehicle_id, ts_utc);")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_mileage_points_ts ON mileage_points(ts_utc);")
        conn.commit()
    finally:
        conn.close()
