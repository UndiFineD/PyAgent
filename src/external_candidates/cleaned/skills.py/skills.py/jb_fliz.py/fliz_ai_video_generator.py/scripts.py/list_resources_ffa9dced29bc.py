# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jb-fliz\fliz-ai-video-generator\scripts\list_resources.py
#!/usr/bin/env python3
"""
...
def get_voices(api_key: str) -> dict:
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/rest/voices", headers=headers, timeout=TIMEOUT)
...
def get_musics(api_key: str) -> dict:
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/rest/musics", headers=headers, timeout=TIMEOUT)
