# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jb-fliz\fliz-ai-video-generator\assets\examples\python_client.py
"""
...
class FlizClient:
    ...
    def _request(self, method: str, endpoint: str, params: dict = None, json_data: dict = None) -> dict:
        ...
        try:
            ...
            if response.status_code == 401:
                raise FlizAPIError("Invalid or expired API key", status_code=401)

            if response.status_code == 429:
                raise FlizAPIError("Rate limit exceeded. Please retry later.", status_code=429)
    ...
    def list_videos(self, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        """
        response = self._request("GET", "/api/rest/videos", params={"limit": limit, "offset": offset})
    ...
    def wait_for_video(
        ...
        while True:
            ...
            if status["is_error"]:
                raise FlizAPIError(f"Video failed: {status['step']}", response=status["video"])
    ...
    def translate_video(
        ...
        response = self._request("POST", f"/api/rest/videos/{video_id}/translate", params=params)
    ...
    def duplicate_video(self, video_id: str) -> Dict[str, Any]:
        ...
        """
        response = self._request("POST", f"/api/rest/videos/{video_id}/duplicate")
...
if __name__ == "__main__":
    ...
    print("\n⏳ Waiting for video...")
    try:
        video = client.wait_for_video(result["video_id"], poll_interval=10, callback=on_progress)
