# Extracted from: C:\DEV\PyAgent\.external\skills\skills\purch-agent\agentic-commerce\scripts\sign_transaction.py
#!/usr/bin/env python3
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sign and submit a Solana transaction for Purch checkout")
    parser.add_argument("serialized_transaction", help="Base58-encoded transaction from /buy endpoint")
    parser.add_argument("private_key", help="Base58-encoded Solana private key")
    parser.add_argument(
        "--rpc-url",
    ...
    result = sign_and_send_transaction(args.serialized_transaction, args.private_key, args.rpc_url)
