# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tvdofficial\sportsbet-advisor\scripts\scrape_sportsbet_upcoming.py
    ...
    for result in query_results:
        ...
        if "odds" in snippet.lower() or "prediction" in snippet.lower() or "vs." in title.lower():
            ...
            event_info = {
                ...
                "teams": (
                    [t.strip() for t in title.split(" vs. ")]
                    if " vs. " in title
                    else [title.split("-")[0].strip(), title.split("-")[-1].strip()]
                ),
...
def get_top_bets_via_web_search(
    sports_categories: List[str], num_bets: int = 5, timeframe_days: int = 1
) -> List[Dict[str, Any]]:
    all_analyzed_events = []
