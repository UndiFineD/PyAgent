# Extracted from: C:\DEV\PyAgent\.external\skills\skills\asif2bd\wordpress-publishing-skill-for-claude\scripts\content_to_gutenberg.py
#!/usr/bin/env python3
"""
...
def html_to_gutenberg(content: str) -> str:
    ...
    for part in parts:
        ...
        if pre_match:
            ...
            blocks.append(f'<!-- wp:code -->\n<pre class="wp-block-code">{code_content}</pre>\n<!-- /wp:code -->')
            continue
...
def create_table_block(
    ...
    tbody_rows = "".join("<tr>" + "".join(f"<td>{escape_cell(c)}</td>" for c in row) + "</tr>" for row in rows)
...
def create_button_block(text: str, url: str, style: str = "fill", align: str = "center") -> str:
    """
...
Examples:
  ...
  Validate existing Gutenberg content:
    ...
    parser.add_argument("--type", "-T", choices=["markdown", "html"], help="Force content type")
    parser.add_argument(
        "--validate",
        "-v",
        action="store_true",
