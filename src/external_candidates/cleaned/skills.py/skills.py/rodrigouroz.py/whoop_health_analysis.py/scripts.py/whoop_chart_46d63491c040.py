# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rodrigouroz\whoop-health-analysis\scripts\whoop_chart.py
#!/usr/bin/env python3
"""
...
from whoop_data import get_cycles, get_recovery, get_sleep, get_workouts
...
def build_sleep_chart(days=30, start=None, end=None):
    ...
    avg_perf = round(
        sum(p for p in performances if p) / max(len([p for p in performances if p]), 1),
        1,
    )
...
def generate_html(title, subtitle, stats_html, charts_html, chart_data, chart_scripts, output=None):
    ...
    has_data = any(isinstance(value, (list, tuple)) and len(value) > 0 for value in (chart_data or {}).values())
    if not has_data:
        stats_html = ""
