# Extracted from: C:\DEV\PyAgent\.external\skills\skills\andrewdmwalker\apollo-enrichment\apollo.py
#!/usr/bin/env python3
...
def api_request(method, endpoint, params=None, data=None):
    ...
    try:
        ...
        try:
            ...
            print(
                f"Error {e.code}: {error_json.get('message', error_body)}",
                file=sys.stderr,
            )
        except:
...
def cmd_bulk_enrich(args):
    ...
    if len(contacts) > 10:
        print(
            f"Warning: Apollo limits bulk to 10. Processing first 10 of {len(contacts)}",
            file=sys.stderr,
        )
...
def should_exclude_competitor(person, exclude_competitors):
    ...
    COMPETITOR_NAMES = {"hathora", "edgegap", "heroic labs", "nakama"}
...
def cmd_search(args):
    """Search for people."""
    data = {"page": 1, "per_page": args.limit or 25}
    ...
    if args.exclude_competitors:
        ...
        if filtered_count > 0:
            print(
                f"Filtered out {filtered_count} competitor employees (Hathora/Edgegap/Nakama)\n",
                file=sys.stderr,
            )
...
Examples:
  ...
  apollo.py bulk-enrich --file contacts.json
    ...
    search.add_argument(
        ...
        "-x",
        action="store_true",
        help="Filter out Hathora/Edgegap/Nakama employees",
    )
