    ...
    def __init__(self, corpus: List[str], k1: float = 1.5, b: float = 0.75):
        ...
        for term, df in self.df.items():
            ...
            self.idf[term] = (
                ...
                if NUMPY_AVAILABLE
                else __import__("math").log((self.doc_count - df + 0.5) / (df + 0.5) + 1.0)
            )
...
class ConversationAnalyzer:
    ...
    def extract_features(self, conversation: Dict[str, Any], corpus: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Message lengths
        user_lengths = [len(self.word_pattern.findall(m.get("content", ""))) for m in user_messages]
        assistant_lengths = [len(self.word_pattern.findall(m.get("content", ""))) for m in assistant_messages]
        ...
        features = {
            ...
            "question_ratio": sum(1 for m in user_messages if self.question_pattern.search(m.get("content", "")))
            / len(user_messages),
...
Examples:
  ...
  %(prog)s --input conversations.json --sample 1000 --output profile.json
    ...
    parser.add_argument("--archetypes-config", "-c", help="Path to custom archetypes YAML config")
    parser.add_argument(
        "--sample",
        "-s",
        type=int,
    ...
    # Analyze
    profiler = CognitiveProfiler(num_archetypes=args.archetypes, custom_archetypes=custom_archetypes)
