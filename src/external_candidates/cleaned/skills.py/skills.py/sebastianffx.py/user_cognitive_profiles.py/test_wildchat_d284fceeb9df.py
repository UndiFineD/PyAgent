...
def format_wildchat_conversation(conv: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    title = (
        first_user_msg[:50] + "..." if first_user_msg and len(first_user_msg) > 50 else (first_user_msg or "Untitled")
    )
...
Examples:
  ...
  %(prog)s --output results.json        # Save results to JSON
    ...
    parser.add_argument("--compare", action="store_true", help="Show comparative analysis between users")
