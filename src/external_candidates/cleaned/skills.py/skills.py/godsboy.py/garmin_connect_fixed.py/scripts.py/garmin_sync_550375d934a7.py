# Extracted from: C:\DEV\PyAgent\.external\skills\skills\godsboy\garmin-connect-fixed\scripts\garmin-sync.py
#!/usr/bin/env python3
"""
...
def make_garmin_client(garth_client):
    """Create a Garmin client with proper garth session and display name"""
    gc = Garmin()
    gc.garth = garth_client
    # Fetch display name from social profile so API calls don't use None
    try:
        profile = garth_client.connectapi("/userprofile-service/socialProfile")
        gc.display_name = profile.get("displayName", "")
    except Exception:
        pass
    return gc
...
def get_daily_summary(garth_client, date_str):
    ...
    try:
        gc = make_garmin_client(garth_client)
