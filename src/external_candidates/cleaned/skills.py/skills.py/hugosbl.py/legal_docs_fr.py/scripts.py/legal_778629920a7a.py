# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hugosbl\legal-docs-fr\scripts\legal.py
#!/usr/bin/env python3
...
def parse_item(item_str):
    ...
    if len(parts) != 3:
        print(
            f"Erreur : format d'item invalide « {item_str} ». Attendu: 'description:quantité:prix'",
            file=sys.stderr,
        )
        sys.exit(1)
    desc = parts[0]
    try:
        ...
        print(
            f"Erreur : quantité ou prix non numérique dans « {item_str} »",
            file=sys.stderr,
        )
        sys.exit(1)
    return {
...
def generate_cgv(args):
    ...
    tribunal = (
        args.tribunal or address.split(",")[-1].strip() if address != "Non renseignée" else "du siège du prestataire"
    )

    tva_mention = "TVA non applicable, article 293 B du Code général des impôts." if micro else ""

    body = (
        provider_header_html(provider, "CONDITIONS GÉNÉRALES DE VENTE")
        + f"""
...
def generate_contrat(args):
    ...
    body = (
        provider_header_html(provider, "CONTRAT DE PRESTATION DE SERVICES")
        + f"""
...
def generate_devis(args):
    ...
    body = (
        provider_header_html(provider, "DEVIS", devis_date_display)
        + f"""
...
def cmd_generate(args):
    ...
    if doc_type == "cgv":
        ...
        if not args.client or not args.mission or args.montant is None or not args.duree:
            print(
                "Erreur : --client, --mission, --montant et --duree sont requis pour un contrat.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if not args.client or not args.items:
            print(
                "Erreur : --client et --items sont requis pour un devis.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"Erreur : type de document inconnu « {doc_type} ». Types disponibles : cgv, mentions, contrat, devis",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    p_gen.add_argument(
        "doc_type",
        choices=["cgv", "mentions", "contrat", "devis"],
    ...
    p_gen.add_argument(
        "--non-sollicitation",
        action="store_true",
        help="Inclure clause de non-sollicitation",
    )
    # Devis
