# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hugosbl\ai-skill-scanner\scripts\advanced_checks.py
...
def find_string_concatenation_evasion(content):  # noscan
    ...
    rev_pat = re.compile(
        r"""['"][a-zA-Z]{3,}['"]\s*\.split\s*\(\s*['"]['""]\s*\)\s*\.reverse\s*\(\s*\)\s*\.join"""
    )  # noscan
        ...
        for m in b64_pat.finditer(line):  # noscan
            try:  # noscan
                ...
                if DANGER_DECODED.search(decoded):  # noscan
                    findings.append(
                        {
                            "check": "PAYLOAD_B64_MALICIOUS",
                            "severity": "CRITICAL",  # noscan
        ...
        try:  # noscan
            ...
            for name in deps:  # noscan
                if name in BAD_NPM:  # noscan
                    findings.append(
                        {
                            "check": "DEP_KNOWN_MALICIOUS",
                            "severity": "CRITICAL",  # noscan
                            "description": f"Known compromised package: {name}",  # noscan
                            "line_num": 0,
                            "line_content": f"package.json → {name}",
                        }
                    )  # noscan
            ...
            for sn in ("preinstall", "postinstall", "preuninstall"):  # noscan
                if sn in scripts:  # noscan
                    findings.append(
                        {
                            "check": "DEP_INSTALL_SCRIPT",
                            "severity": "HIGH",  # noscan
                            "description": f"Has '{sn}' script: {scripts[sn][:100]}",  # noscan
                            "line_num": 0,
                            "line_content": f"scripts.{sn}",
                        }
                    )  # noscan
        except Exception:  # noscan
            pass  # noscan
        ...
        if rp.exists():  # noscan
            for ln, line in enumerate(rp.read_text().split("\n"), 1):  # noscan
                ...
                if (
                    ...
                    and any(x in line for x in ("git+", "http://", "https://", "svn+"))
                ):  # noscan
                    findings.append(
                        {
                            "check": "DEP_REMOTE_INSTALL",
                            "severity": "HIGH",  # noscan
                ...
                findings.append(
                    {
                        ...
                        "line_num": node.lineno,
                        "line_content": f"from {node.module} import {names}",
                    }
                )  # noscan
            ...
            if fn == "open" and node.args:  # noscan
                ...
                if isinstance(arg, ast.Constant) and isinstance(arg.value, str):  # noscan
                    if any(s in arg.value for s in SENSITIVE_PATHS):  # noscan
                        ...
                        findings.append(
                            {
                                "check": "AST_SENSITIVE_FILE_READ",
                                "severity": "HIGH",  # noscan
                                "description": f"Opens sensitive: {arg.value}",  # noscan
                                "line_num": node.lineno,
                                "line_content": f'open("{arg.value}")',
                            }
                        )  # noscan
    ...
    for root, dirs, files in os.walk(skill_path):  # noscan
        ...
        for f in files:  # noscan
            ...
            if f.startswith(".") and f not in safe_hidden:  # noscan
                findings.append(
                    {
                        "check": "STRUCT_HIDDEN_FILE",
                        "severity": "MEDIUM",  # noscan
                        "description": f"Hidden file: {f}",  # noscan
                        "line_num": 0,
                        "line_content": str(fpath),
                    }
                )  # noscan
        ...
        for ln, line in _lines(content):  # noscan
            for pat, desc in unsafe:  # noscan
                if re.search(pat, line, re.IGNORECASE):  # noscan
                    findings.append(
                        {
                            "check": "STRUCT_UNSAFE_INSTRUCTION",
                            "severity": "HIGH",  # noscan
...
def analyze_js_ts(filepath, content):  # noscan
    ...
    net_var_pat = re.compile(  # noscan
        ...
        r"""|request|http\.(?:get|request)|https\.(?:get|request))\s*\(\s*(?!['"`]|https?://)"""
    )  # noscan
    net_any_pat = re.compile(  # noscan
        r"""(?:fetch|axios|got|request|http\.(?:get|request)|https\.(?:get|request))\s*\("""
    )  # noscan
    env_pat = re.compile(r"""process\.env(?:\s*[\[.])""")  # noscan
    fs_pat = re.compile(  # noscan
        r"""(?:fs|require\s*\(\s*['"]fs['"]\s*\))\s*\.\s*"""  # noscan
        r"""(?:readFile|readFileSync|readdir|readdirSync|createReadStream|writeFile|writeFileSync)\s*\("""
    )  # noscan
    fs_sensitive = re.compile(  # noscan
        ...
        r"""secret|private.?key|\.clawdbot|\.openai)"""
    )  # noscan
    cp_pat = re.compile(  # noscan
        r"""(?:child_process|require\s*\(\s*['"]child_process['"]\s*\))\s*\."""  # noscan
        r"""(?:exec|execSync|spawn|spawnSync|fork|execFile)\s*\("""
    )  # noscan
...
_NETWORK_FUNCS = {  # noscan
    ...
    "requests.post",
    "requests.put",
    "requests.patch",  # noscan
    "requests.delete",
    "requests.request",  # noscan
    "urllib.request.urlopen",
    "urllib.request.urlretrieve",  # noscan
    "http.client.HTTPConnection",
    "http.client.HTTPSConnection",  # noscan
    "socket.connect",
    "socket.create_connection",  # noscan
}  # noscan
...
_LEGIT_API_DOMAINS = {  # noscan
    ...
    "api.sncf.com",  # noscan
    "api.laposte.fr",
    "developer.laposte.fr",  # noscan
    "prim.iledefrance-mobilites.fr",  # noscan
    "api.open-meteo.com",  # noscan
    "api.github.com",
    "api.openai.com",
    "api.anthropic.com",  # noscan
    "api.google.com",
    "googleapis.com",  # noscan
    "api.stripe.com",
    "api.twilio.com",  # noscan
    "api.sendgrid.com",
    "api.mailgun.net",  # noscan
...
def _is_legitimate_api_auth(call_node, tree):  # noscan
    ...
    Returns True if it looks like normal API auth (env key → auth header → known API).
    """  # noscan
    ...
    has_auth_header = any(  # noscan
        s.lower() in _AUTH_HEADER_KEYS
        for s in all_strings  # noscan  # noscan
    )  # noscan
    has_auth_var = any(  # noscan
        _AUTH_VAR_PATTERNS.search(s)
        for s in all_strings  # noscan  # noscan
    )  # noscan
                    ...
                    findings.append(
                        {
                            ...
                            "line_content": f"{fn}(...) uses file-tainted vars",
                        }
                    )  # noscan
                    ...
                    findings.append(
                        {
                            ...
                            "line_num": getattr(node, "lineno", 0),  # noscan
                            "line_content": f"{fn}(...) uses env-tainted vars",
                        }
                    )  # noscan
    return findings  # noscan
...
EXT_EXPECTED_TYPE = {  # noscan
    ".png": "image/png",
    ".jpg": "image/jpeg",
    ".jpeg": "image/jpeg",  # noscan
    ".gif": "image/gif",
    ".pdf": "application/pdf",
    ".zip": "application/zip",  # noscan
    ".gz": "application/gzip",
    ".tar": "application/tar",  # noscan
    ".exe": "application/x-executable",
    ".dll": "application/x-executable",  # noscan
    ".so": "application/x-elf",
    ".dylib": "application/x-macho",  # noscan
}  # noscan
...
SUSPICIOUS_SERVICES = {  # noscan
    "webhook.site",
    "requestbin.com",
    "requestbin.net",  # noscan
    "pipedream.net",
    "pipedream.com",  # noscan
    "ngrok.io",
    "ngrok.app",
    "ngrok-free.app",  # noscan
    "serveo.net",
    "localtunnel.me",  # noscan
...
LEGIT_DOMAINS = [  # noscan
    "github.com",
    "gitlab.com",
    "bitbucket.org",  # noscan
    "npmjs.com",
    "npmjs.org",
    "pypi.org",  # noscan
    "openai.com",
    "anthropic.com",  # noscan
    "google.com",
    "googleapis.com",
    "microsoft.com",
    "apple.com",  # noscan
...
def check_temporal_triggers(content, filepath):  # noscan
    ...
    env_gate_js = re.compile(  # noscan
        r"""(?:if|while|&&|\|\|)\s*\(?.*process\.env\.\w+\s*(?:===?|!==?|==)\s*['"]"""
    )  # noscan
    env_gate_py = re.compile(  # noscan
        r"""(?:if|while)\s+.*(?:os\.environ|os\.getenv)\s*[\[(].*(?:==|!=|in\s)"""
    )  # noscan
    ...
    platform_cond = re.compile(  # noscan
        r"""(?:if|while|&&|\|\|)\s*\(?.*(?:process\.platform|os\.platform\s*\(\)|sys\.platform|os\.name)\s*(?:===?|!==?|==)\s*['"]"""
    )  # noscan
    for line_num, line in _lines(content):  # noscan
        if ext in JS_EXTENSIONS or ext in (".js", ".mjs", ".cjs", ".ts"):  # noscan
            ...
            if m:  # noscan
                ...
                if delay_ms >= 3600000:  # noscan
                    ...
                    findings.append(
                        {
                            "check": "TRIGGER_DELAYED_INTERVAL",
                            "severity": "HIGH",  # noscan
