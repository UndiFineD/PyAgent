# Extracted from: C:\DEV\PyAgent\.external\skills\skills\charbeld\people-memories\scripts\people_memory.py
#!/usr/bin/env python3
import argparse
import calendar
import json
...
def add_note(person, note, source, tags, event_type=None, event_date=None):
    ...
    event = detect_event_metadata(
        note,
        clean_tags,
        provided_type=event_type,
        provided_date=event_date,
        source=source,
    )
    if event:
        note_entry["event"] = event
        entry.setdefault("events", {})[event["type"]] = {
            **event,
            "updatedAt": timestamp,
        }
...
def format_reminder_message(events, reference_date, lookahead, fmt="text"):
    ...
    if lookahead == 0:
        ...
        header = (
            f"People memories reminders from {reference_date.strftime('%b %d')} through {end_date.strftime('%b %d')}:"
        )
    lines = []
    for event in events:
...
def main():
    ...
    remember.add_argument(
        "--event-type",
        choices=["birthday", "anniversary"],
        help="Optional event type to associate with this note.",
    )
    remember.add_argument(
        "--event-date",
        help="Optional event date (natural language) for birthdays or anniversaries.",
    )
    ...
    remind_parser.add_argument(
        ...
        type=int,
        default=0,
        help="Days from today to anchor the reminder window.",
    )
    remind_parser.add_argument(
        "--window",
        type=int,
        default=0,
        help="How many days forward (inclusive) to check for events.",
    )
    remind_parser.add_argument(
        "--event-types",
        choices=["all", "birthday", "anniversary"],
        default="all",
        help="Beware: choose a specific event type or all.",
    )
    remind_parser.add_argument(
        "--format",
        choices=["text", "message"],
        default="text",
        help="Output style for the reminder text.",
    )
