# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cpojer\orf\scripts\orf.py
#!/usr/bin/env python3
import argparse
...
def score_item(title: str, focus: str) -> int:
    ...
    if focus == "inland" and has(
        [
            r"\bösterreich\b|\boesterreich\b",
            r"\bwien\b",
            r"\bparlament\b",
            r"\bregierung\b",
        ]
        ...
        items.append(
            {
                ...
                "score": score_item(
                    title,
                    ("inland" if focus == "inland" else "ausland" if focus == "ausland" else "auto"),
                ),
            }
        )
