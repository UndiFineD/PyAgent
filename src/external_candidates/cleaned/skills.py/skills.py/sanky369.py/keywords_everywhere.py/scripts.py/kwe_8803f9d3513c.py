# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sanky369\keywords-everywhere\scripts\kwe.py
#!/usr/bin/env python3
"""
...
def api_request(endpoint, data=None):
    ...
    if not api_key:
        print(
            "Error: No API key found. Set KEYWORDS_EVERYWHERE_API_KEY or configure in clawdbot config.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            print(
                f"Error: {error_json.get('message', error_json.get('error', str(e)))}",
                file=sys.stderr,
            )
...
def format_table(data, columns):
    ...
    for row in data:
        line = " | ".join(
            str(col.get("format", lambda x, r: str(x if x is not None else ""))(row.get(col["key"]), row)).ljust(
                widths[col["key"]]
            )
            for col in columns
        )
...
def cmd_keywords(args):
    ...
    if not args.keywords:
        print(
            "Usage: kwe keywords 'keyword1' 'keyword2' ... [--country us] [--currency usd]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_domain_keywords(args):
    ...
    if not args.domain:
        print(
            "Usage: kwe domain-keywords example.com [--country us] [--num 10]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_url_keywords(args):
    ...
    if not args.url:
        print(
            "Usage: kwe url-keywords 'https://example.com/page' [--country us] [--num 10]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_page_backlinks(args):
    ...
    if not args.url:
        print(
            "Usage: kwe page-backlinks 'https://example.com/page' [--num 10]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_unique_page_backlinks(args):
    ...
    if not args.url:
        print(
            "Usage: kwe unique-page-backlinks 'https://example.com/page' [--num 10]",
            file=sys.stderr,
        )
        sys.exit(1)
