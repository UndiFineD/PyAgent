...
class StyleAuditor:
    ...
    def _check_focus_indicators(self, node: Dict[str, Any]):
        ...
        has_focus_effect = any(
            effect.get("type") == "DROP_SHADOW" and "focus" in str(effect).lower() for effect in effects
        )
    ...
    def _analyze_cross_file_patterns(self, all_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "files_with_errors": len(
                [r for r in all_results.values() if "error" not in r and r["summary"]["by_severity"]["error"] > 0]
            ),
        }
