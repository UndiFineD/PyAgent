# Extracted from: C:\DEV\PyAgent\.external\skills\skills\architect-sis\sis-skill\core\validator.py
"""
═══════════════════════════════════════════════════════════════════════════════
...
class SISValidator:
    ...
    def validate_symbol(self, symbol: SISSymbol) -> ValidationResult:
        ...
        if abs(sigma) >= self.EPSILON:
            violations.append(f"Symbol {symbol.glyph} has ΣΔ = {sigma:.6f} (expected 0)")
        ...
        for symbol in symbols:
            ...
            if symbol._validation:
                if not symbol._validation(symbol.value):
                    violations.append(f"Symbol {symbol.glyph} failed custom validation")

        if abs(sigma) >= self.EPSILON:
            violations.append(f"Sequence has ΣΔ = {sigma:.6f} (expected 0)")
    ...
    def suggest_correction(self, symbols: List[SISSymbol]) -> Optional[SISSymbol]:
        """
    ...
    def get_imbalance_trace(self, symbols: List[SISSymbol]) -> List[Tuple[str, float, float]]:
        """
