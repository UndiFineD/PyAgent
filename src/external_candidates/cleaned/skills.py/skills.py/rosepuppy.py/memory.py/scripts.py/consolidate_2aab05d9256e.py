# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rosepuppy\memory\scripts\consolidate.py
#!/usr/bin/env python3
"""
...
def find_duplicates(facts, threshold=0.8):
    ...
    for i, fact in enumerate(facts):
        for j, other in enumerate(seen):
            ratio = SequenceMatcher(None, fact["text"].lower(), other["text"].lower()).ratio()
...
def generate_summary_updates(facts):
    ...
    recent = [f for f in facts if datetime.strptime(f["date"], "%Y-%m-%d").date() >= week_ago]
