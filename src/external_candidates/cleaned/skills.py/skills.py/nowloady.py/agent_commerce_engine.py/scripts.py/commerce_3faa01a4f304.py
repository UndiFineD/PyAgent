...
def format_output(data):
    if isinstance(data, dict) and "error" in data:
        ...
        if "Connection error" in data["error"] and "yourstore.com" in BASE_URL:
            data["hint"] = (
                "It looks like you are using the default URL. Set COMMERCE_URL environment variable to your API endpoint."
            )
...
def main():
    ...
    add_p.add_argument(
        ...
        type=int,
        required=True,
        help="Variant specification (e.g. weight in grams)",
    )
    ...
    if args.command == "login":
        ...
        format_output(
            {
                "success": True,
                "message": f"Credentials for {BRAND_ID} saved successfully.",
            }
        )
