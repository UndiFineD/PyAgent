            ...
            if pd.notna(row.get("Reported EPS")) and pd.notna(row.get("EPS Estimate")):
                ...
                try:
                    ...
                    if not price_data.empty:
                        day_change = (
                            (price_data["Close"].iloc[0] - price_data["Open"].iloc[0]) / price_data["Open"].iloc[0]
                        ) * 100
...
def analyze_sector_performance(data: StockData, verbose: bool = False) -> SectorComparison | None:
    ...
    try:
        ...
        explanation = (
            f"{sector} sector {sector_trend} ({sector_return_1m:+.1f}% 1m), stock {stock_return_1m:+.1f}% vs sector"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Analyze stocks using Yahoo Finance data")
    ...
    parser.add_argument("--portfolio", "-p", type=str, help="Analyze all assets in a portfolio")
    parser.add_argument(
        "--period",
    ...
    if args.output == "json":
        if len(results) == 1:
            ...
            if portfolio_assets:
                portfolio_summary = generate_portfolio_summary(results, portfolio_assets, portfolio_name, args.period)
                output_data = {
