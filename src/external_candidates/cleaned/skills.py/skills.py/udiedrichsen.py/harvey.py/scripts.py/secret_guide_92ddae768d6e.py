...
def create_secret_plan(start_location: str, vibe: str = "explore", max_distance: int = 800) -> dict:
    ...
    destinations = find_nearby_destinations(start["lat"], start["lng"], vibe=vibe, radius_m=max_distance)
    ...
    distance = calculate_distance(start["lat"], start["lng"], destination["lat"], destination["lng"])

    direction = get_direction(start["lat"], start["lng"], destination["lat"], destination["lng"])
...
def main():
    ...
    parser.add_argument("--distance", "-d", type=int, default=800, help="Max distance in meters")
