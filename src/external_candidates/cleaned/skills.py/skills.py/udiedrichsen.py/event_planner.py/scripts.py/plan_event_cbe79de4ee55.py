    ...
    for i, item in enumerate(itinerary.items):
        ...
        if i < len(itinerary.items) - 1:
            ...
            if next_travel > 0:
                ...
                lines.append(f"| *Walk {next_travel} min{source_note}* | - | - | - | - | {distance_km:.1f} km |")
...
def main():
    ...
    parser.add_argument("--party-size", type=int, default=2, help="Number of people (default: 2)")
    parser.add_argument(
        "--budget",
        default="medium",
        help="Budget: 'low/medium/high' or '$X per person' (default: medium)",
    )
    parser.add_argument("--duration", help="Duration (e.g., '3h', '4h', 'full day') - currently unused")
    parser.add_argument(
        "--preferences",
        help="Comma-separated preferences (e.g., 'vegetarian, outdoor seating')",
    )
    parser.add_argument("--start-time", help="Start time in HH:MM format (e.g., '19:00')")
    parser.add_argument(
        "--output",
        choices=["text", "json"],
        default="text",
