# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tonimelisma\email-to-calendar\scripts\tests\test_invite_ops.py
#!/usr/bin/env python3
...
class TestUpdateInviteStatus(unittest.TestCase):
    ...
    def test_update_status_by_invite_id(self):
        ...
        captured = io.StringIO()
        with patch("sys.stdout", captured):
            invite_ops.update_invite_status(invite_id="inv1", event_title="Project Review", new_status="dismissed")
    ...
    def test_update_status_wrong_email_id_exits(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            invite_ops.update_invite_status(email_id="wrong_email", event_title="Team Meeting", new_status="created")
        self.assertEqual(cm.exception.code, 1)
    ...
    def test_update_multiple_events_in_invite(self):
        ...
        captured = io.StringIO()
        with patch("sys.stdout", captured):
            invite_ops.update_invite_status(email_id="email123", event_title="Event B", new_status="created")
    ...
    def test_update_prints_confirmation(self):
        ...
        captured = io.StringIO()
        with patch("sys.stdout", captured):
            invite_ops.update_invite_status(email_id="email123", event_title="Team Meeting", new_status="created")
