# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\web-search-plus\scripts\search.py
#!/usr/bin/env python3
"""
...
class QueryAnalyzer:
    ...
    def _calculate_signal_score(self, query: str, signals: Dict[str, float]) -> Tuple[float, List[Dict[str, Any]]]:
        """
        ...
        for pattern, weight in signals.items():
            ...
            if found:
                ...
                matches.append({"pattern": pattern, "matched": match_text, "weight": weight})
                total_score += weight
    ...
    def analyze(self, query: str) -> Dict[str, Any]:
        ...
        shopping_score, shopping_matches = self._calculate_signal_score(query, self.SHOPPING_SIGNALS)
        research_score, research_matches = self._calculate_signal_score(query, self.RESEARCH_SIGNALS)
        discovery_score, discovery_matches = self._calculate_signal_score(query, self.DISCOVERY_SIGNALS)
        local_news_score, local_news_matches = self._calculate_signal_score(query, self.LOCAL_NEWS_SIGNALS)
        ...
        if detected_url:
            discovery_score += 5.0
            discovery_matches.append({"pattern": "url_detected", "matched": detected_url, "weight": 5.0})
    ...
    def route(self, query: str) -> Dict[str, Any]:
        ...
        available = {p: s for p, s in scores.items() if p not in disabled and get_env_key(p)}
        ...
        return {
            ...
            "top_signals": [{"matched": s["matched"], "weight": s["weight"]} for s in top_signals],
...
def explain_routing(query: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "all_matches": {
            provider: [{"matched": m["matched"], "weight": m["weight"]} for m in matches]
...
Full docs: See README.md and SKILL.md
    ...
    parser.add_argument("--images", action="store_true", help="Include images (Serper/Tavily)")
    ...
    parser.add_argument("--time-range", choices=["hour", "day", "week", "month", "year"])
        ...
        try:
            ...
            if len(providers_to_try) > 1:
                remaining = [
                    ...
                    if p != current_provider and p not in [err["provider"] for err in errors]
                ]
                if remaining:
                    print(
                        json.dumps(
