# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\web-search-plus\scripts\setup.py
#!/usr/bin/env python3
"""
...
def ask_api_key(provider: str, signup_url: str) -> str:
    ...
    while True:
        ...
        if not key:
            print(
                color(
                    "    ⚠️  No key entered. This provider will be disabled.",
                    Colors.YELLOW,
                )
            )
            return None
        ...
        if len(key) < 10:
            print(
                color(
                    "    ⚠️  Key seems too short. Please check and try again.",
                    Colors.YELLOW,
                )
            )
            continue
