...
def create_buy_instruction(user: Keypair, mint: Pubkey, amount: int, max_sol_cost: int) -> Instruction:
...
def cmd_launch(args):
    ...
    # Upload to IPFS
    try:
        metadata_uri = asyncio.run(upload_to_ipfs(args.image, args.name, args.symbol, args.description or ""))
    except Exception as e:
    ...
    if args.buy and args.buy > 0:
        ...
        ata_ix = create_ata_instruction(payer=keypair.pubkey(), owner=keypair.pubkey(), mint=mint)
