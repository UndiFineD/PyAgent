# Extracted from: C:\DEV\PyAgent\.external\skills\skills\spendit-ai\solana-skills\scripts\wallet.py
#!/usr/bin/env python3
"""
...
def cmd_token_balance(args):
    ...
    try:
        ...
        response = client.get_token_accounts_by_owner_json_parsed(
            owner_pubkey, TokenAccountOpts(mint=mint_pubkey), commitment=Confirmed
        )
...
def cmd_send(args):
    ...
    try:
        ...
        transfer_ix = transfer(TransferParams(from_pubkey=keypair.pubkey(), to_pubkey=recipient, lamports=lamports))
        ...
        print(
            f"◎ Explorer: https://solscan.io/tx/{signature}" + ("?cluster=devnet" if args.network == "devnet" else "")
        )
