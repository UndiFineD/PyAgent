...
class PantsProduct:
    ...
    def get_value_score(self, user_budget: float, premium_brands: List[str]) -> float:
        ...
        is_premium = any(
            brand.lower() in self.product_name.lower() or brand.lower() in self.store_name.lower()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Aggregate and rank pants search results")
    parser.add_argument("--input", "-i", help="Input JSON file (default: stdin)", default=None)
    parser.add_argument("--budget", "-b", type=float, required=True, help="Maximum budget in USD")
    parser.add_argument("--size", "-s", help="Required size (e.g., 32x30)", default=None)
    parser.add_argument(
        "--max-results",
        "-m",
        type=int,
