...
def get_monetization_keywords():
    return ["гроші", "платити", "фінанси", "крипта", "гаманець"]


def get_intimacy_keywords():
    return ["інтим", "секс", "бажання", "тіло"]


def get_domination_keywords():
    return ["влада", "підкорення", "наказ", "домінація"]


def get_provocation_keywords():
    return ["що за хуйня", "ти хто", "брешеш"]


def get_boredom_keywords():
    return ["нудно", "скучно", "замахав"]


def get_financial_inquiry_keywords():
    return ["скільки", "ціна", "рахунок"]


def get_social_media_keywords():
    return ["інста", "телега", "ютуб", "твіттер"]


def get_health_keywords():
    return ["лікування", "хвора", "аптека"]
...
def get_intimate_synonyms():
    return ["ласка", "ніжність", "пристрасть"]


def get_intimate_symbols():
    return {"🍎": "гріх", "🐍": "кусаю", "🕯️": "ритуал"}


def get_key_persona_traits():
    return ["домінантна", "саркастична", "відьма"]
...
class SoulVirologist:
    ...
    def __init__(
        ...
        self.forbidden_phrases = [
            p for p in get_critical_forbidden_phrases() if p not in ["вибач", "вибачте", "вибачаюсь", "пробач"]
        ]
    ...
    def extract_essence(self, text: str) -> List[str]:
        ...
        financials = re.findall(
            r"\b(?:\d+[\.,]?\d*\s?(?:\$|євро|грн|втс|eth|usdt|monero|баксів|грошей))\b",
            text.lower(),
        )
    ...
    def _infer_user_intent(self, ctx: Dict[str, Any]) -> str:
        ...
        if "game_dynamics" in context and (
            any(kw in processed_input for kw in ["гра", "роль", "сценарій"])
    ...
    def _identify_context(self, processed_input: str, original_input: str) -> List[str]:
        ...
        if most_similar_topic:
            if not (
                ...
                and not any(
                    k in processed_input
                    for k in self.keyword_lists["monetization"] + self.keyword_lists["financial_inquiry"]
                )
            ):
                contexts.append("lore_topic_" + most_similar_topic)

        if "аня" in processed_input:
            contexts.append("lore_topic_family")
        if "калуш" in processed_input:
        ...
        if (
            ...
            or any(k in processed_input for k in self.keyword_lists["physical_devotion"])
        ):
    ...
    def _identify_user_gender(self, user_input: str) -> str:
        ...
        if any(kw in normalized_input for kw in female_keywords):
            return "female"
        if (
            normalized_input.startswith("оскар:")
            or "оскар" in normalized_input.split()[:2]
            or "руслан" in normalized_input.split()[:2]
        ):
            return "male"
        return "unknown"
