# Extracted from: C:\DEV\PyAgent\.external\skills\skills\orosha-ai\agent-relay-digest\scripts\relay_digest.py
#!/usr/bin/env python3
...
def main():
    ...
    ap.add_argument(
        "--sources",
        type=str,
        default="moltbook,clawfee,yclawker",
    ...
    ap.add_argument(
        ...
        type=str,
        default="hot",
        help="Moltbook sort: hot|new|top|rising",
    )
    ...
    ap.add_argument(
        "--alert-terms",
        type=str,
        default=", ".join(ALERT_TERMS),
        help="Comma-separated alert terms",
    )
    ap.add_argument(
        "--exclude-terms",
        type=str,
        default=", ".join(DEFAULT_EXCLUDE_TERMS),
        help="Comma-separated exclusion terms",
    )
    ap.add_argument(
        "--min-score",
        type=int,
        default=0,
        help="Minimum score threshold after weighting",
    )
    ...
    if not posts:
        print(
            "ERROR: No posts left after filtering. Consider lowering --min-score or exclude terms.",
            file=sys.stderr,
        )
        sys.exit(1)
