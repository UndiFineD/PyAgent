# Extracted from: C:\DEV\PyAgent\.external\skills\skills\orosha-ai\agentic-compass\scripts\agentic-compass.py
#!/usr/bin/env python3
"""
...
def score_memory_consistency(content, long_content):
    ...
    references = count_markers(
        content + long_content,
        r"earlier i mentioned|as we discussed|following up on|continuing",
    )
    # Check if contradictions exist (simplistic heuristic)
    contradictions = count_markers(
        content + long_content,
        r"actually i changed my mind|contradicting my earlier|never mind",
    )
    if references == 0:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to daily memory file",
    )
    parser.add_argument(
        "--long",
        type=str,
