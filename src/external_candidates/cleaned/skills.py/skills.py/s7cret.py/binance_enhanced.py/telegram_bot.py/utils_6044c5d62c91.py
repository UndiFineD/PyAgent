# Extracted from: C:\DEV\PyAgent\.external\skills\skills\s7cret\binance-enhanced\telegram-bot\utils.py
import csv
import json
import os
import sys
...
def format_order_table(order: dict) -> str:
    ...
    sym = order.get(
        "symbol",
        order.get("base_asset") and order.get("quote_asset") and f"{order['base_asset']}{order['quote_asset']}" or "—",
    )
