# Extracted from: C:\DEV\PyAgent\.external\skills\skills\s7cret\binance-enhanced\ux\templates.py
#!/usr/bin/env python3
"""
...
def render_confirmation(state: dict) -> str:
    ...
    sym = (
        state.get("symbol")
        or (state.get("base_asset") and state.get("quote_asset") and f"{state['base_asset']}{state['quote_asset']}")
        or "—"
    )
