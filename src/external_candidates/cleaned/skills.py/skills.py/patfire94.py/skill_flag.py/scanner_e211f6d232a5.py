# Extracted from: C:\DEV\PyAgent\.external\skills\skills\patfire94\skill-flag\scanner.py
#!/usr/bin/env python3
"""
                ...
                for pattern in patterns:
                    for match in pattern["compiled"].finditer(content):
                        ...
                        if whitelisted:
                            if self.verbose:
                                print(
                                    f"  {Colors.BLUE}[whitelist] {pattern['name']} - {file_path.name}:{line_start}{Colors.END}"
                                )
                            continue
        ...
        for report in reports:
            ...
            if report.findings:
                ...
                for f in report.findings[:10]:  # Limit to first 10
                    ...
                    if f.line_content:
                        print(
                            f"      └─ {Colors.CYAN}{f.line_content[:80]}...{Colors.END}"
                            if len(f.line_content) > 80
                            else f"      └─ {Colors.CYAN}{f.line_content}{Colors.END}"
                        )
...
def main():
    parser = argparse.ArgumentParser(description="🛡️ Clawdbot Skill Flag - Scan skills for security issues")
