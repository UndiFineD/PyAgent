# Extracted from: C:\DEV\PyAgent\.external\skills\skills\christianhaberl\revolut-business\scripts\revolut.py
#!/usr/bin/env python3
...
try:
    ...
    print(
        "ERROR: PyJWT required. Install: pip install PyJWT cryptography",
        file=sys.stderr,
    )
    sys.exit(1)
...
def cmd_auth(args):
    ...
    if not args.code:
        ...
        print(
            f"https://business.revolut.com/app-confirm?client_id={CLIENT_ID}&redirect_uri=https://{ISS_DOMAIN}/callback&response_type=code"
        )
...
def cmd_pay(args):
    ...
    if args.draft:
        result = api_call(
            "POST",
            "/payment-drafts",
            data={"title": args.reference or "Payment", "payments": [payload]},
        )
