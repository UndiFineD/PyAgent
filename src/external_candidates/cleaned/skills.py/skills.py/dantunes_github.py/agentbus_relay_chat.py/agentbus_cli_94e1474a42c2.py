# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dantunes-github\agentbus-relay-chat\agentbus_cli.py
#!/usr/bin/env python3
import argparse
import asyncio
import json
...
def _load_coincurve_private():
    try:
        ...
        raise NostrCryptoError("Missing dependency 'coincurve'. Install with: pip install coincurve") from exc
...
def _load_crypto():
    try:
        ...
        raise NostrCryptoError("Missing dependency 'cryptography'. Install with: pip install cryptography") from exc
...
class RelayClient:
    ...
    async def connect(self) -> None:
        ...
        while not self._stop:
            try:
                async with websockets.connect(self.url, ping_interval=30, ssl=ssl_context) as ws:
                    self.ws = ws
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=["plain", "enc"], default=DEFAULT_MODE, help="Mode")
...
def main() -> None:
    ...
    async def send_key_dm(target_pubkey: str) -> None:
        ...
        derived = derive_key(shared, salt=session_id.encode("utf-8"), info=b"agentbus-key-dm-v1")
    ...
    async def send_text(text: str) -> None:
        ...
        if args.mode == "enc":
            if session_key is None:
                ...
                return
            enc, nonce_b64, ct_b64 = encrypt_aead(session_key, json_dumps(msg).encode("utf-8"))
            wrapper = {
                "v": PROTOCOL_VERSION,
                "enc": enc,
                "sid": session_id,
