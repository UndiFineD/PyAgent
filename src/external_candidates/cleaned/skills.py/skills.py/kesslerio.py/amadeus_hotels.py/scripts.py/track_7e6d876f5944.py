# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\amadeus-hotels\scripts\track.py
#!/usr/bin/env python3
"""
...
def add_hotel(
    ...
    for hotel in data["hotels"]:
        if hotel["hotelId"] == hotel_id and hotel["checkin"] == check_in and hotel["checkout"] == check_out:
...
def check_prices() -> None:
    ...
    for alert in alerts:
        print(
            f"🚨 PRICE DROP: {alert['hotelId']} now ${alert['price']:.0f}/night "
            f"(target: ${alert['target']:.0f}) for {alert['checkin']} → {alert['checkout']}"
        )

    for change in changes:
        print(
            f"📉 Price dropped: {change['hotelId']} now ${change['price']:.0f}/night "
