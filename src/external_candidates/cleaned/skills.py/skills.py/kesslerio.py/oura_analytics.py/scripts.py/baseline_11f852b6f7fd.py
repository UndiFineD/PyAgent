...
def main():
    ...
    try:
        ...
        baseline_sleep = client.get_sleep(baseline_start.strftime("%Y-%m-%d"), baseline_end.strftime("%Y-%m-%d"))
        ...
        current_sleep = client.get_sleep(current_start.strftime("%Y-%m-%d"), current_end.strftime("%Y-%m-%d"))
        current_readiness = client.get_readiness(current_start.strftime("%Y-%m-%d"), current_end.strftime("%Y-%m-%d"))
        ...
        current_sleep_hours = [
            s.get("total_sleep_duration", 0) / 3600 for s in current_sleep if s.get("total_sleep_duration")
        ]
