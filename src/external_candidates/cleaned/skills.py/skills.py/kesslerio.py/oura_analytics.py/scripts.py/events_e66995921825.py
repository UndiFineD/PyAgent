# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\events.py
#!/usr/bin/env python3
"""
    ...
    def analyze_tag_correlation(
        ...
        for date_str in event_dates:
            ...
            for offset in range(-14, -7):  # 1-2 weeks before
                ...
                is_event_day = any(control_date == e["date"] for e in events)
...
def main():
    ...
    corr_parser.add_argument(
        "--metric",
        default="readiness",
        choices=["readiness", "sleep_score", "hrv"],
        help="Metric to correlate",
    )
