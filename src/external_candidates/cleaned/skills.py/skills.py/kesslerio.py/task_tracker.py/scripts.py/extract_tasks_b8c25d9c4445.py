# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\task-tracker\scripts\extract_tasks.py
#!/usr/bin/env python3
"""
...
TASK_PATTERNS = [
    ...
    (
        r"^\d+[.)]\s*([A-Z][a-z]+(?:\s+[a-z]+)*\s+(?:to|for|on|complete|finish|review|create|add|set up|post|analyze|update)\b.+)",
        "medium",
    ),
]
