# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\dialpad\sms_storage.py
#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    ...
    if cmd == "migrate":
        ...
        for t in threads:
            last = (
                datetime.fromtimestamp(t.get("last_message", 0) / 1000).strftime("%Y-%m-%d %H:%M")
                if t.get("last_message")
                else "N/A"
            )
            print(
                f"{t['phone_number']:<20} {t.get('contact_name', 'Unknown'):<25} {t['message_count']:>6} {t.get('unread_count', 0):>7} {last}"
            )
        ...
        for msg in messages:
            ts = (
                datetime.fromtimestamp(msg.get("timestamp", 0) / 1000).strftime("%H:%M:%S")
        ...
        for number, messages in results:
            ...
            for msg in messages[:3]:  # Show first 3 matches
                ts = (
                    datetime.fromtimestamp(msg.get("timestamp", 0) / 1000).strftime("%Y-%m-%d %H:%M")
                    if msg.get("timestamp")
                    else "Unknown"
                )
