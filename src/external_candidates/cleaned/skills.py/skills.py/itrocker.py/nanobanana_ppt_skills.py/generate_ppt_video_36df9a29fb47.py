# Extracted from: C:\DEV\PyAgent\.external\skills\skills\itrocker\nanobanana-ppt-skills\generate_ppt_video.py
#!/usr/bin/env python3
"""
...
def scan_slide_images(slides_dir: str) -> List[str]:
    ...
    if not slides_paths:
        raise FileNotFoundError(f"No PPT images found in {slides_dir} (expected format: slide-*.png)")
...
def generate_ppt_video_from_images(
    ...
    content_contexts = [f"Transition from slide {i + 1} to slide {i + 2}" for i in range(num_slides - 1)]
    ...
    if video_mode in ("both", "local"):
        ...
        transitions_dict = {
            key: result["video_path"] for key, result in materials_result["transitions"].items() if result["success"]
        }
    ...
    print(f"  Videos: {materials_result['success_count']} success, {materials_result['failed_count']} failed")
