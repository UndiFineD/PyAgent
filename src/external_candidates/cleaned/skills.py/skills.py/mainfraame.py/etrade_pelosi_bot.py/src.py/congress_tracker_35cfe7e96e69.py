...
class CongressTracker:
    ...
    def _fetch_senate_direct(self, senator_name=None, max_pages=3, days_back=90):
        ...
        try:
            ...
            try:
                checkbox = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID, "agree_statement")))
                checkbox.click()
                time.sleep(2)
            except Exception as e:
            ...
            try:
                ...
                WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.ID, "filedReports")))
    ...
    def fetch_mock_data(self):
        ...
        for trade in mock_trades:
            if trade["transaction_type"] in self.trade_types and trade["amount"] >= self.min_trade_size:
                trades.append(trade)
