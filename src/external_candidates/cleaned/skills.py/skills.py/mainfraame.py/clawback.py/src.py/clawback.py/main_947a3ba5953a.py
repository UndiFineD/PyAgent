# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\main.py
"""
...
class TradingBot:
    ...
    def _load_saved_tokens(self):
        ...
        if db_tokens and db_tokens.get("access_token"):
            self.broker.set_tokens(db_tokens["access_token"], db_tokens.get("access_secret"))
        ...
        if os.path.exists(token_file):
            try:
                ...
                if tokens.get("access_token"):
                    self.broker.set_tokens(tokens["access_token"], tokens.get("access_secret"))
                    # Migrate to database
    ...
    def authenticate(self, verifier_code=None):
        ...
        if success:
            ...
            if accounts:
                ...
                for acc in accounts:
                    self.logger.info(
                        f"  - {acc.get('accountId')}: {acc.get('accountName', acc.get('accountDesc', ''))}"
                    )
            return True
        else:
    ...
    def check_for_new_disclosures(self):
        ...
        try:
            ...
            self.notifier.send_disclosure_error(source="Disclosure Check", error_message=error_msg)
            import traceback
    ...
    def execute_pending_trades(self):
        ...
        if not self.trade_engine.is_market_open():
            ...
            self.logger.info(
                f"Market closed. {len(self.pending_trades)} trades queued for {next_open.strftime('%Y-%m-%d %H:%M %Z')}"
            )
            return
    ...
    def run_scheduled(self):
        ...
        self.logger.info(
            f"Market hours: {self.config['trading']['marketOpen']} - {self.config['trading']['marketClose']} ET"
        )
            ...
            if choice == "1":
                ...
                if house:
                    ...
                    for trade in house[:5]:
                        ...
                        print(
                            f"    {trade['transaction_date']}: {trade['transaction_type'].upper():8} "
                ...
                for trade in unprocessed[:10]:
                    print(
                        f"  {trade['disclosure_date']}: {trade['transaction_type'].upper():8} "
                ...
                if not positions:
                    ...
                    print(f"{'=' * 80}")
                    print(
                        f"  {'Symbol':<8} {'Qty':>6} {'Entry':>10} {'Current':>10} {'P/L':>10} {'P/L%':>8} {'Stop':>10} {'Days':>5}"
                    )
                    print(f"  {'-' * 75}")
                    total_pnl = 0
                    for p in positions:
                        ...
                        print(
                            f"  {p['symbol']:<8} {p['quantity']:>6} ${p['entry_price']:>8.2f} "
