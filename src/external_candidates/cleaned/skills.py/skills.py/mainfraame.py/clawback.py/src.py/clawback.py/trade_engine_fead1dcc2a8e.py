...
class TradeEngine:
    ...
    def add_position(
        ...
        if symbol in self.positions:
            ...
            avg_price = ((existing.entry_price * existing.quantity) + (entry_price * quantity)) / total_qty
        ...
        for symbol, pos in self.positions.items():
            try:
                ...
                if pos.check_stop_loss(current_price):
                    ...
                    logger.warning(
                        f"STOP-LOSS TRIGGERED: {symbol} at ${current_price:.2f} (stop: ${pos.stop_loss_price:.2f})"
                    )
