# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alphafactor\crypto\scripts\crypto.py
#!/usr/bin/env python3
"""
加密货币行情获取与价格监控预警工具
    ...
    try:
        while True:
            ...
            if ticker:
                ...
                print(
                    f"\r[{timestamp}] {symbol}: {ticker['last']:,.4f} ({ticker['percentage']:+.2f}%) {change_emoji}",
                    end="",
                    flush=True,
                )
...
示例:
  ...
  %(prog)s watch BTC/USDT --interval 5
    ...
    ohlcv_parser.add_argument("--limit", "-l", type=int, default=24, help="获取条数")

    # orderbook 命令
    orderbook_parser = subparsers.add_parser("orderbook", help="获取订单簿")
    orderbook_parser.add_argument("symbol", help="交易对")
    orderbook_parser.add_argument("--limit", "-l", type=int, default=10, help="深度")
    ...
    alert_add_parser.add_argument(
        "condition",
        choices=["above", "below", "up_percent", "down_percent"],
