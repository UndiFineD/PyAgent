# Extracted from: C:\DEV\PyAgent\.external\skills\skills\starbuck100\clawdhub-contributor\auditor\state.py
...
class AuditState:
    ...
    def get_all_audited_slugs(self) -> dict[str, str]:
        ...
        cur = self._conn.execute("SELECT skill_slug, version FROM audits ORDER BY audited_at DESC")
        result: dict[str, str] = {}
    ...
    def build_queue(self, available: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        """
        return [s for s in available if not self.is_audited(s["slug"], s.get("version", "unknown"))]
