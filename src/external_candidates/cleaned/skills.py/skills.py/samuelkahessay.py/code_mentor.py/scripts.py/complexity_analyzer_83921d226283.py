# Extracted from: C:\DEV\PyAgent\.external\skills\skills\samuelkahessay\code-mentor\scripts\complexity_analyzer.py
#!/usr/bin/env python3
"""
...
class ComplexityAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if has_recursion:
            ...
            analysis["suggestions"].append("Recursive function - consider memoization or iterative approach")
        elif loop_depth >= 3:
            analysis["time_complexity"] = f"O(n^{loop_depth})"
            analysis["suggestions"].append(f"Deep nesting ({loop_depth} levels) - consider optimization")
        elif loop_depth == 2:
            analysis["time_complexity"] = "O(n²)"
            analysis["suggestions"].append("Nested loop detected - can this be optimized with hash map?")
        elif loop_depth == 1:
            analysis["time_complexity"] = "O(n)"
