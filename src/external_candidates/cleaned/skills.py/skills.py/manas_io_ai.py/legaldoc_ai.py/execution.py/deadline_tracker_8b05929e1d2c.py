...
DEFAULT_DB_PATH = os.environ.get("LEGALDOC_DEADLINES_DB", os.path.expanduser("~/.legaldoc/deadlines.db"))
...
def extract_deadlines_from_document(file_path: str) -> list[Deadline]:
    ...
    for pattern, category in date_patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            ...
            # Generate ID
            deadline_id = hashlib.md5(f"{iso_date}:{context[:50]}".encode()).hexdigest()[:12]
...
def calculate_sol_deadline(incident_date: str, claim_type: str, jurisdiction: str = "CA") -> Optional[Deadline]:
    ...
    deadline_id = hashlib.md5(f"sol:{claim_type}:{incident_date}".encode()).hexdigest()[:12]
...
def format_deadlines(
    ...
    lines.extend(["---", "", "*Managed by LegalDoc AI. Set up alerts to never miss a deadline.*"])
...
def main():
    ...
    list_parser.add_argument("--overdue", action="store_true", help="Show only overdue deadlines")
    list_parser.add_argument("--output", "-o", choices=["markdown", "json"], default="markdown")
