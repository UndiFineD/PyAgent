...
class ReportGenerator:
    ...
    def _analyze_file_access(self, logs: List[Dict]) -> Dict:
        ...
        for log in logs:
            ...
            if "sensitive" in path.lower() or any(s in path.lower() for s in [".env", "secret", "password", "key"]):
    ...
    def _analyze_api_calls(self, logs: List[Dict]) -> Dict:
        ...
        analysis["by_domain"] = dict(sorted(analysis["by_domain"].items(), key=lambda x: x[1], reverse=True)[:15])
    ...
    def _analyze_communications(self, logs: List[Dict]) -> Dict:
        ...
        analysis["by_type"] = dict(analysis["by_type"])
        analysis["destinations"] = dict(sorted(analysis["destinations"].items(), key=lambda x: x[1], reverse=True)[:10])
    ...
    def _calculate_threat_level(self, alert_analysis: Dict, file_analysis: Dict, api_analysis: Dict) -> Tuple[str, str]:
    ...
    def _generate_recommendations(self, alert_analysis: Dict, file_analysis: Dict, api_analysis: Dict) -> List[str]:
        ...
        if alert_analysis.get("unacknowledged", 0) > 5:
            recommendations.append(
                f"📋 **Review unacknowledged alerts** - {alert_analysis['unacknowledged']} alerts need attention"
            )

        if alert_analysis["by_severity"].get("critical", 0) > 0:
            recommendations.append(
                "🚨 **URGENT: Address critical alerts immediately** - These require immediate investigation"
            )

        # Based on file access
        if len(file_analysis.get("sensitive_accesses", [])) > 0:
            recommendations.append(
                "🔐 **Review sensitive file accesses** - Verify all credential file accesses were authorized"
            )

        # Based on API calls
        if len(api_analysis.get("errors", [])) > 10:
            recommendations.append(
                "⚠️ **Investigate API errors** - High error rate may indicate configuration issues or attacks"
            )

        if len(api_analysis.get("large_requests", [])) > 5:
            recommendations.append("📤 **Review large outbound requests** - Verify large data transfers are expected")
    ...
    def generate_report(self, period: str = "daily") -> str:
        ...
        threat_level, threat_reason = self._calculate_threat_level(alert_analysis, file_analysis, api_analysis)

        # Generate recommendations
        recommendations = self._generate_recommendations(alert_analysis, file_analysis, api_analysis)
