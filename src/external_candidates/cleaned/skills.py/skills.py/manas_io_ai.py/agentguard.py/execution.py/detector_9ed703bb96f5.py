...
class AnomalyDetector:
    ...
    def detect_exfiltration_pattern(self, file_events: List[Dict], api_events: List[Dict]) -> Optional[Anomaly]:
        ...
        recent_reads = [
            e for e in file_events if datetime.fromisoformat(e.get("timestamp", "1970-01-01")) > one_hour_ago
        ]
    ...
    def run_all_detections(self, file_events: List[Dict], api_events: List[Dict]) -> List[Anomaly]:
...
def main():
    ...
    parser.add_argument("command", choices=["detect", "baseline", "status"], help="Command to execute")
    parser.add_argument(
        "--sensitivity",
