# Extracted from: C:\DEV\PyAgent\.external\skills\skills\manas-io-ai\agentguard\execution\logger.py
#!/usr/bin/env python3
"""
    ...
    def sanitize_dict(cls, data: Dict) -> Dict:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                sanitized[key] = [
                    (cls.sanitize(v) if isinstance(v, str) else cls.sanitize_dict(v) if isinstance(v, dict) else v)
                    for v in value
                ]
            else:
                sanitized[key] = value
...
class AgentLogger:
    ...
    def log_file_access(self, path: str, event_type: str, size: Optional[int] = None) -> LogEntry:
    ...
    def get_logs(self, category: LogCategory, date: Optional[str] = None, limit: int = 100) -> List[Dict]:
...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["log", "view", "stats", "cleanup", "compress"],
        help="Command to execute",
    )
    parser.add_argument(
        "--category",
        choices=["file_access", "api_call", "communication", "system"],
        help="Log category",
    )
