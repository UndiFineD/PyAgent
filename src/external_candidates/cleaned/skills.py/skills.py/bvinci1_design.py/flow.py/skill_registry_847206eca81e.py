# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bvinci1-design\flow\skill_registry.py
#!/usr/bin/env python3
"""
...
class SkillRegistry:
    ...
    def _calculate_reuse_score(self, skill: SkillMetadata) -> float:
        ...
        security_scores = {"passed": 20, "warning": 10, "unscanned": 5, "failed": 0}
    ...
    def find_skills(
        ...
        results = [self.skills[name] for name in candidates if self.skills[name].reuse_score >= min_reuse_score]
        ...
        return [
            {
                ...
                "reuse_score": s.reuse_score,
                "capabilities": s.capabilities,
            }
            for s in sorted(self.skills.values(), key=lambda x: x.reuse_score, reverse=True)
        ]
