# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bvinci1-design\flow\skill_composer.py
#!/usr/bin/env python3
"""
...
class SkillComposer:
    ...
    def compose(self, skills: List, intent: "ParsedIntent", name: Optional[str] = None) -> ComposedSkill:
        """
        ...
        output_path = self._generate_skill_code(name=name, skills=ordered_skills, intent=intent)
    ...
    def _generate_skill_code(self, name: str, skills: List, intent: "ParsedIntent") -> str:
        """
        ...
        if skills:
            ...
            for skill in skills:
                code_lines.append(f"        # Component: {skill.name}")
                code_lines.append(f'        self.components["{skill.name}"] = self._load_skill("{skill.path}")')
    ...
    def compose_from_templates(self, intent: "ParsedIntent", templates: List[Dict]) -> ComposedSkill:
        """
    ...
    def _generate_from_templates(self, name: str, intent: "ParsedIntent", templates: List[Dict]) -> str:
        """
