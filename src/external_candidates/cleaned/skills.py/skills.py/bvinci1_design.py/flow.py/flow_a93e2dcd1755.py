...
class Flow:
    ...
    def process(self, user_request: str) -> FlowResult:
        ...
        try:
            ...
            matching_skills = self.registry.find_skills(capabilities=intent.capabilities, tags=intent.tags)
            ...
            flow_skill = self.composer.compose(skills=scanned_skills, intent=intent, name=intent.suggested_name)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Flow - Intelligent Skill Orchestrator")
    parser.add_argument("request", nargs="?", help="Natural language request describing what to build")
