# Extracted from: C:\DEV\PyAgent\.external\skills\skills\akegaviar\polyclaw\scripts\markets.py
#!/usr/bin/env python3
...
async def cmd_trending(args):
    ...
    if args.json:
        ...
        for m in markets:
            ...
            print(
                f"{m.id[:12]:<12} {format_price(m.yes_price):>6} {format_price(m.no_price):>6} {format_volume(m.volume_24h):>10} {question}"
            )
    ...
    if args.json:
        ...
        for e in events:
            ...
            for m in e.markets[:3]:
                question = (
                    m.question if args.full else (m.question[:70] + "..." if len(m.question) > 70 else m.question)
                )
