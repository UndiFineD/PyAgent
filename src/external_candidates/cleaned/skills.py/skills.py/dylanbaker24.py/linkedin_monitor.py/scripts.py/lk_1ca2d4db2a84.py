# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dylanbaker24\linkedin-monitor\scripts\lk.py
#!/usr/bin/env python3
"""
...
try:
    ...
    print(
        json.dumps(
            {
                "error": "linkedin-api not installed",
                "action": "pip3 install linkedin-api",
            }
        )
    )
    sys.exit(1)
...
def get_client():
    ...
    try:
        ...
        api = Linkedin("", "", cookies={"li_at": li_at, "JSESSIONID": f'"{jsessionid}"'})
        return api
...
def cmd_message_list(args):
    ...
    try:
        ...
        for conv in conversations.get("elements", []):
            ...
            if events:
                ...
                last_message = {
                    ...
                    "fromMe": last_event.get("from", {})
                    .get("com.linkedin.voyager.messaging.MessagingMember", {})
                    .get("miniProfile", {})
                    .get("publicIdentifier", "")
                    == api.get_user_profile().get("publicIdentifier", ""),
                }
        ...
        if args.json:
            ...
            for conv in results:
                name = (
                    f"{conv['participants'][0]['firstName']} {conv['participants'][0]['lastName']}"
                    if conv["participants"]
                    else "Unknown"
                )
                msg = (
                    conv["lastMessage"]["text"][:50] + "..."
                    if conv["lastMessage"] and len(conv["lastMessage"]["text"]) > 50
                    else (conv["lastMessage"]["text"] if conv["lastMessage"] else "")
                )
