# Extracted from: C:\DEV\PyAgent\.external\skills\skills\heiheimaoya\xiaohongshu\scripts\sign_server.py
#!/usr/bin/env python3
"""
...
async def init_browser():
    ...
    for key, value in cookie_dict.items():
        cookies_to_add.append({"name": key, "value": value, "domain": ".xiaohongshu.com", "path": "/"})
...
async def sign_request(request):
    ...
    try:
        ...
        # 调用页面的签名函数
        try:
            result = await page.evaluate("([url, data]) => window._webmsxyw(url, data)", [uri, payload])

            return web.json_response({"x-s": result.get("X-s", ""), "x-t": str(result.get("X-t", ""))})
