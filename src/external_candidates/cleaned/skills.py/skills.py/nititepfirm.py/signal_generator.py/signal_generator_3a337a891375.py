...
class SignalGenerator:
    ...
    def bb_breakout_strategy(self, df: pd.DataFrame) -> Dict:
        ...
        long_signal = (df["Close"] > df["BB_Upper"]) & (df["Volume"] > df["Vol_SMA"]) & (prev_squeeze)

        short_signal = (df["Close"] < df["BB_Lower"]) & (df["Volume"] > df["Vol_SMA"]) & (prev_squeeze)
