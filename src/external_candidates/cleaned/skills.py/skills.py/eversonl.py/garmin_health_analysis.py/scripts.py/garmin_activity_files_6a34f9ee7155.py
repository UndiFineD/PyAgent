...
def analyze_activity(data):
    ...
    elevation_values = [
        r.get("altitude") or r.get("elevation") for r in records if r.get("altitude") or r.get("elevation")
    ]
    ...
    analysis = {
        ...
        "cadence": (
            {"avg": (sum(cadence_values) / len(cadence_values) if cadence_values else None)} if cadence_values else None
        ),
        "power": (
            {
