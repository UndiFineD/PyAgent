# Extracted from: C:\DEV\PyAgent\.external\skills\skills\johanesalxd\vision-sandbox\scripts\vision_executor.py
import argparse
import os
import sys
...
def run_vision_sandbox(image_path, prompt, model_id="gemini-3-flash-preview"):
    ...
    try:
        response = client.models.generate_content(model=model_id, contents=[prompt, image_part], config=config)
...
def main():
    ...
    parser.add_argument("-m", "--model", default="gemini-3-flash-preview", help="Model ID")
