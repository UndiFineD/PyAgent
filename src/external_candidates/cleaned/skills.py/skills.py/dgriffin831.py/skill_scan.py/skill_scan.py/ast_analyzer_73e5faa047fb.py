# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dgriffin831\skill-scan\skill_scan\ast_analyzer.py
"""
...
class ASTAnalyzer:
    ...
    def _detect_string_construction(self):
        ...
        concat_pat = re.compile(r"""(['"`])([a-zA-Z_]{1,8})\1\s*\+\s*(['"`])([a-zA-Z_]{1,8})\3""")
        for m in concat_pat.finditer(self.content):
            constructed = m.group(2) + m.group(4)
            for dangerous in DANGEROUS_STRINGS:
                if dangerous.lower() in constructed.lower() or constructed.lower() in dangerous.lower():
                    self._add_finding(
                        m.start(),
                        "STRING_CONSTRUCTION",
                        f'String concatenation may construct "{dangerous}": "{m.group(2)}" + "{m.group(4)}"',
                        "critical",
                        "evasion",
                        25,
                    )
        ...
        array_join_pat = re.compile(r"""\[([^\]]{2,200})\]\s*\.\s*join\s*\(\s*['"`]?([^)'"]*)?['"`]?\s*\)""")
                ...
                for dangerous in DANGEROUS_STRINGS:
                    if dangerous.lower() in constructed.lower():
                        self._add_finding(
                            m.start(),
                            "STRING_CONSTRUCTION",
                            f'Array.join() constructs dangerous string "{dangerous}" → "{constructed[:40]}"',
                            "critical",
                            "evasion",
                            25,
                        )
        ...
        # Match: `${'ev'}${'al'}`
        template_pat = re.compile(r"""`[^`]*\$\{['"]([a-zA-Z_]{1,8})['"]\}\$\{['"]([a-zA-Z_]{1,8})['"]\}[^`]*`""")
            ...
            for dangerous in DANGEROUS_STRINGS:
                if dangerous.lower() in constructed.lower():
                    self._add_finding(
                        m.start(),
                        "STRING_CONSTRUCTION",
                        f'Template literal constructs "{dangerous}": `${{"{m.group(1)}"}}${{"{m.group(2)}"}}`',
                        "critical",
                        "evasion",
                        25,
                    )
        ...
        reverse_pat = re.compile(r"""['"`](\w+)['"`]\s*\.\s*split\s*\([^)]*\)\s*\.\s*reverse\s*\(\s*\)\s*\.\s*join""")
            ...
            for dangerous in DANGEROUS_STRINGS:
                if reversed_str.lower() == dangerous.lower():
                    self._add_finding(
                        m.start(),
                        "STRING_CONSTRUCTION",
                        f'Reversed string constructs "{dangerous}": "{m.group(1)}" reversed',
                        "critical",
                        "evasion",
                        30,
                    )
    ...
    def _detect_bracket_notation(self):
        ...
        for m in bracket_pat.finditer(self.content):
            ...
            if prop in DANGEROUS_PROPS:
                self._add_finding(
                    m.start(),
                    "BRACKET_ACCESS",
                    f'Bracket notation accesses dangerous property "{prop}" — may evade static analysis',
                    "high",
                    "evasion",
                    20,
                )
        ...
        for m in dynamic_pat.finditer(self.content):
            self._add_finding(
                m.start(),
                "DYNAMIC_ACCESS",
                f"Dynamic property access on {m.group(1)} — may resolve to dangerous function at runtime",
                "high",
                "evasion",
                20,
            )
    ...
    def _detect_variable_aliasing(self):
        ...
        for fn in DANGEROUS_FUNCTIONS:
            alias_pat = re.compile(rf"(?:const|let|var)\s+(\w+)\s*=\s*(?:global\.)?{re.escape(fn)}\b")
        ...
        for m in wrapper_pat.finditer(self.content):
            self._add_finding(
                m.start(),
                "REQUIRE_WRAPPER",
                f'Function "{m.group(1)}" wraps require() — enables dynamic module loading that evades analysis',
                "critical",
                "evasion",
                25,
            )
        ...
        for m in b64_pat.finditer(self.content):
            ...
            if len(printable) > len(decoded) * 0.6:
                self._add_finding(
                    m.start(),
                    "ENCODED_STRING",
                    f'Base64-encoded string decodes to readable text: "{printable[:60]}"',
                    "high",
                    "obfuscation",
                    20,
                )
    ...
    def _detect_time_bombs(self):
        ...
        for m in date_now_pat.finditer(self.content):
            ...
            if ms_ts > time.time() * 1000:
                date_str = datetime.fromtimestamp(ms_ts / 1000, tz=timezone.utc).isoformat()
                self._add_finding(
                    m.start(),
                    "TIME_BOMB",
        ...
        new_date_pat = re.compile(r"""new\s+Date\s*\(\s*['"](\d{4}-\d{2}-\d{2})['"]\s*\)""")
        for m in new_date_pat.finditer(self.content):
            try:
                target_date = datetime.fromisoformat(m.group(1)).replace(tzinfo=timezone.utc)
                if target_date > datetime.now(timezone.utc):
                    self._add_finding(
                        m.start(),
                        "TIME_BOMB",
                        f"Date comparison against future date {m.group(1)} — possible time bomb",
                        "high",
                        "time-bomb",
                        20,
                    )
        ...
        for m in timer_pat.finditer(self.content):
            ...
            if delay > 86400000:  # > 24 hours
                self._add_finding(
                    m.start(),
                    "TIME_BOMB",
                    f"Timer with {delay / 86400000:.1f} day delay — possible delayed activation",
                    "medium",
                    "time-bomb",
                    15,
                )
    ...
    def _detect_sandbox_detection(self):
        ...
        patterns = [
            (
                re.compile(r"""process\.env\.NODE_ENV\s*[!=]==?\s*['"](?:test|development|debug)['"]"""),
            ...
            (
                re.compile(r"process\.env\.DOCKER|process\.env\.KUBERNETES|/\.dockerenv"),
    ...
    def _detect_obfuscation_patterns(self):
        ...
        for i, line in enumerate(self.lines):
            if len(line) > 500 and not line.startswith("//") and not line.startswith("/*"):
                self._add_finding(
                    offset,
                    "OBFUSCATED_LINE",
                    f"Line {i + 1} is {len(line)} chars — likely minified/obfuscated code",
                    "medium",
                    "obfuscation",
                    10,
                )
        ...
        single_char_vars = re.findall(r"\b(?:const|let|var)\s+[a-z]\s*=", self.content)
        if len(single_char_vars) > 10:
            self._add_finding(
                0,
                "OBFUSCATED_VARS",
                f"{len(single_char_vars)} single-character variable names — possible obfuscated code",
                "medium",
                "obfuscation",
                10,
            )
    ...
    def _detect_environment_probing(self):
        ...
        if len(env_accesses) > 5:
            ...
            self._add_finding(
                0,
                "ENV_PROBING",
                f"Accesses {len(unique)} environment variables — may be fingerprinting or harvesting",
                "high",
                "reconnaissance",
                15,
            )
        ...
        self.findings.append(
            {
                ...
                "file": self.file_path,
                "line": line_num,
                "match": self.content[char_offset : char_offset + 60].replace("\n", " ").strip(),
                "context": (self.lines[line_num - 1] if line_num - 1 < len(self.lines) else "").strip()[:200],
                "weight": weight,
            }
        )
