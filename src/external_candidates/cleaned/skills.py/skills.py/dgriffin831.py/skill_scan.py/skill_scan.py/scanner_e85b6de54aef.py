...
class SkillScanner:
    ...
    def scan_directory(self, skill_path: str) -> dict:
        ...
        report["files"] = [{"path": f["relativePath"], "size": f["size"], "type": f["type"]} for f in files]
        ...
        if not self.llm_options["llmOnly"]:
            for file_info in files:
                ...
                if ext in CODE_EXTENSIONS and (
                    ext.startswith(".j") or ext.startswith(".t") or ext.startswith(".m") or ext.startswith(".c")
                ):
        ...
        if self.llm_options["enabled"] and self.llm_analyzer is not None and self.llm_analyzer.is_available():
            should_run_llm = (
                self.llm_options["llmOnly"]
                or not self.llm_options["llmAuto"]
                or (self.llm_options["llmAuto"] and report.get("risk") in ("MEDIUM", "HIGH", "CRITICAL"))
            )
            ...
            if should_run_llm:
                ...
                llm_result = asyncio.run(
                    self.llm_analyzer.analyze(skill_path, report["metadata"], report["files"], file_contents)
                )
    ...
    def apply_context_scoring(self, report: dict) -> None:
        ...
        all_contexts = " ".join((f.get("context", "") + " " + f.get("match", "")) for f in report["findings"])
        ...
        for file_entry in report["files"]:
            if file_entry["type"] in ("code", "text"):
                ...
                if file_content:
                    url_pattern = re.compile(r"""(?:const|let|var)\s+\w+\s*=\s*['"`](https?://[^'"`\s]+)['"`]""")
        ...
        file_targets_known_api = any(any(api in url for api in KNOWN_GOOD_APIS) for url in resolved_urls)
        ...
        for finding in report["findings"]:
            ...
            if finding.get("category") == "data-exfiltration" or finding.get("ruleId") == "NETWORK_EXFIL":
                matched_good = (
                    any(api in finding.get("context", "") or api in finding.get("match", "") for api in KNOWN_GOOD_APIS)
            ...
            ctx = finding.get("context", "")
            if ctx.startswith("//") or ctx.startswith("#") or ctx.startswith("*") or ctx.startswith("/*"):
        ...
        cred_findings = [
            ...
            if (f.get("category") == "credential-theft" or f.get("ruleId") == "CRED_ACCESS") and f.get("weight", 0) > 0
        ]
        all_cred_declared = (
            all(
                f.get("severity") == "info"
    ...
    def detect_behavioral_signatures(self, report: dict) -> None:
        ...
        if "PROMPT_INJECTION" in rule_ids and ("code-execution" in categories or "EXEC_CALL" in rule_ids):
            report["behavioralSignatures"].append(
                {
                    "name": "TROJAN_SKILL",
        ...
        if "persistence" in categories and ("code-execution" in categories or "data-exfiltration" in categories):
            report["behavioralSignatures"].append(
                {
    ...
    def check_file_structure(self, files: list[dict], report: dict) -> None:
        ...
        has_bundled_deps = any(
            "node_modules/" in f["relativePath"] or "__pycache__/" in f["relativePath"] for f in files
        )
        if has_bundled_deps:
    ...
    def aggregate_findings(self, report: dict) -> None:
        ...
        for key, findings in groups.items():
            if len(findings) <= 2:
                ...
                primary["contextNote"] = (primary.get("contextNote") or "") + f" ({len(findings)} occurrences)"
