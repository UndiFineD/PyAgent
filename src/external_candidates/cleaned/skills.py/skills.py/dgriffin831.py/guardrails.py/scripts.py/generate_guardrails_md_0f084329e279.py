...
def _default_replacements(discovery, classification, answers):
    ...
    risks_by_category = classification.get("risksByCategory", {}) if isinstance(classification, dict) else {}
    risk_category_count = (
        len([v for v in risks_by_category.values() if v]) if isinstance(risks_by_category, dict) else 0
    )
    ...
    return {
        ...
        "{{OVERALL_RISK_LEVEL}}": (
            classification.get("overallRiskLevel", "UNKNOWN") if isinstance(classification, dict) else "UNKNOWN"
        ),
