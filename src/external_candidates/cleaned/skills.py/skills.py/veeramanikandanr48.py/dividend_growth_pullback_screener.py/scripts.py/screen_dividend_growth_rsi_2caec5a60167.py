# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\dividend-growth-pullback-screener\scripts\screen_dividend_growth_rsi.py
#!/usr/bin/env python3
"""
...
class FINVIZClient:
    ...
    def screen_stocks(self) -> Set[str]:
        ...
        try:
            print(
                f"Fetching pre-screened stocks from FINVIZ Elite API...",
                file=sys.stderr,
            )
            print(
                f"FINVIZ Filters: Div Yield 0.5-3%, Div Growth 10%+, EPS Growth 5%+, Sales Growth 5%+, RSI <40",
                file=sys.stderr,
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"ERROR: FINVIZ API authentication failed. Check your API key.",
                    file=sys.stderr,
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.retry_count <= 1:
                    print(
                        f"WARNING: Rate limit exceeded. Waiting 60 seconds...",
                        file=sys.stderr,
                    )
                    time.sleep(60)
                    ...
                    print(
                        f"ERROR: Daily API rate limit reached. Stopping analysis.",
                        file=sys.stderr,
                    )
...
def screen_dividend_growth_pullbacks(
    ...
    print(
        f"Note: Analysis will continue until API rate limit is reached\n",
        file=sys.stderr,
    )
    ...
    for i, stock in enumerate(candidates, 1):
        ...
        div_cagr, div_consistent, annual_dividend, div_years_of_growth = analyzer.analyze_dividend_growth(
            dividend_history
        )
...
| Stop Loss Suggestion | {
                f"{((stock['rsi'] - 30) / 2 + 3):.0f}% below entry" if stock["rsi"] >= 30 else "8% below entry"
            } | |
...
| Metric | Value | Status |
|--------|-------|--------|
| Revenue CAGR (3Y) | {f"{stock['revenue_cagr_3y']:.2f}%" if stock["revenue_cagr_3y"] else "N/A"} | {
...
| Metric | Value | Status |
|--------|-------|--------|
| Debt-to-Equity | {f"{stock['debt_to_equity']:.2f}" if stock["debt_to_equity"] is not None else "N/A"} | {
                "✓ Very Low"
...
Environment Variables:
  ...
  FINVIZ_API_KEY    - FINVIZ Elite API key (required for --use-finviz)
    ...
    parser.add_argument("--rsi-max", type=float, default=40.0, help="Maximum RSI value (default: 40.0)")
    parser.add_argument(
        "--max-candidates",
        type=int,
        default=None,
