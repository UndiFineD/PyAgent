# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\pair-trade-screener\scripts\find_pairs.py
#!/usr/bin/env python3
"""
...
def screen_all_pairs(price_data, min_correlation=0.70):
    ...
    for symbol_a, symbol_b in combinations(symbols, 2):
        ...
        if pairs_analyzed % 10 == 0 or pairs_analyzed == total_pairs:
            print(
                f"  [{pairs_analyzed}/{total_pairs}] pairs analyzed...",
                end="\r",
                flush=True,
            )
...
def rank_pairs(pairs):
    ...
    ranked = sorted(pairs, key=lambda x: (x["cointegration_pvalue"], -abs(x["current_zscore"] or 0)))
