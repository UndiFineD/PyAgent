        ...
        if len(quarterly_metrics) >= 2:
            ...
            for name, holder in previous_holders_map.items():
                if name not in current_holders_map:
                    closed_positions.append({"name": name, "previous_shares": holder.get("totalShares", 0)})
...
"""
        ...
        for i, q in enumerate(metrics):
            ...
            report += (
                f"| {q['quarter']} | {q['total_shares']:,} | ${q['total_value']:,} | {q['num_holders']} | {qoq_str} |\n"
            )
...
"""
        ...
        if analysis["increased_positions"]:
            ...
            for pos in analysis["increased_positions"][:10]:
                report += (
                    f"| {pos['name']} | {pos['current_shares']:,} | {pos['change']:+,} | {pos['pct_change']:+.2f}% |\n"
                )
        else:
