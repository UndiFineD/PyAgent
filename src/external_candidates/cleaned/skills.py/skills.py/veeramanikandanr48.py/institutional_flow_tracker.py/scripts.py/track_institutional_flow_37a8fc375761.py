# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\institutional-flow-tracker\scripts\track_institutional_flow.py
#!/usr/bin/env python3
"""
...
class InstitutionalFlowTracker:
    ...
    def get_stock_screener(self, market_cap_min: int = 1000000000, limit: int = 500) -> List[Dict]:
    ...
    def calculate_ownership_metrics(self, symbol: str, company_name: str, market_cap: float) -> Optional[Dict]:
        ...
        # Get top holders
        top_holders = sorted(current_holders, key=lambda x: x.get("totalShares", 0), reverse=True)[:10]
...
Examples:
  ...
  python3 track_institutional_flow.py --min-market-cap 2000000000 --top 100 --output results.json
    ...
    if results:
        ...
        for r in results[:10]:
            print(
                f"{r['symbol']:<8} {r['company_name'][:30]:<30} {r['percent_change']:>9.2f}% {r['institution_count_change']:>+11d}"
            )
