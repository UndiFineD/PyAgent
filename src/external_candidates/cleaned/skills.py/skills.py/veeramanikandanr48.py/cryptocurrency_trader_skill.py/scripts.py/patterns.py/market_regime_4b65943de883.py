...
class MarketRegimeDetector:
    ...
    def __init__(
        ...
        if not (0 < trending_threshold < 1):
            raise ValueError(f"trending_threshold must be between 0 and 1, got {trending_threshold}")

        if not (0 < low_vol_threshold < high_vol_threshold):
            raise ValueError(
                f"Volatility thresholds must be 0 < low < high, got {low_vol_threshold}, {high_vol_threshold}"
            )

        if min_history_periods < 10:
            raise ValueError(f"min_history_periods must be >= 10, got {min_history_periods}")
    ...
    def detect_regime(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            logger.info(f"Market regime: {regime}, Volatility: {vol_regime}, Autocorr: {autocorrelation:.3f}")
    ...
    def detect_regime_change(self, df: pd.DataFrame, lookback: int = 20) -> Dict:
        ...
        try:
            ...
            # Detect changes
            market_changed = current_regime["market_regime"] != previous_regime["market_regime"]
            volatility_changed = current_regime["volatility_regime"] != previous_regime["volatility_regime"]
