...
class HistoricalAccuracyTracker:
        ...
        if accuracies_file.exists():
            try:
                with open(accuracies_file, "r") as f:
                    data = json.load(f)
                    self.accuracies = {name: IndicatorAccuracy(**acc_data) for name, acc_data in data.items()}
    ...
    def record_trade_outcome(
        ...
        logger.info(f"Recorded trade outcome: {symbol} {action} {'✓' if success else '✗'} ({pnl_pct:+.2f}%)")
    ...
    def _update_accuracies(self, outcome: TradeOutcome) -> None:
        ...
        for indicator_name, indicator_signal in outcome.indicators_used.items():
            ...
            if accuracy_record.total_signals >= self.min_samples:
                ...
                new_accuracy = (1 - self.learning_rate) * accuracy_record.accuracy + self.learning_rate * raw_accuracy

                accuracy_record.accuracy = new_accuracy
                accuracy_record.last_updated = datetime.now().isoformat()

                logger.debug(
                    f"Updated {indicator_name} accuracy: {new_accuracy:.3f} (samples: {accuracy_record.total_signals})"
                )

    def get_indicator_accuracy(self, indicator_name: str, symbol: Optional[str] = None) -> float:
        """
