# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\test-specialist\scripts\analyze_coverage.py
#!/usr/bin/env python3
"""
...
def analyze_file_coverage(file_data: Dict) -> Dict:
    ...
    return {
        "statements": {
            "pct": pct(stmt_covered, stmt_total),
            "covered": stmt_covered,
            "total": stmt_total,
        },
        "branches": {
            "pct": pct(branch_covered, branch_total),
            "covered": branch_covered,
            "total": branch_total,
        },
        "functions": {
            "pct": pct(func_covered, func_total),
...
def identify_coverage_gaps(coverage_data: Dict, threshold: float = 80.0) -> List[Tuple[str, Dict]]:
    ...
    for filepath, file_data in coverage_data.items():
        ...
        below_threshold = any(metric["pct"] < threshold for metric in analysis.values())
...
def print_coverage_report(gaps: List[Tuple[str, Dict]]):
    ...
    for filepath, analysis in gaps:
        print(f"📄 {filepath}")
        print(
            f"   Statements: {analysis['statements']['pct']:.1f}% ({analysis['statements']['covered']}/{analysis['statements']['total']})"
        )
        print(
            f"   Branches:   {analysis['branches']['pct']:.1f}% ({analysis['branches']['covered']}/{analysis['branches']['total']})"
        )
        print(
            f"   Functions:  {analysis['functions']['pct']:.1f}% ({analysis['functions']['covered']}/{analysis['functions']['total']})"
        )
        print()
