# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\test-specialist\scripts\find_untested_code.py
#!/usr/bin/env python3
"""
...
def find_source_files(src_dir: str, extensions: List[str] = None) -> Set[Path]:
    ...
    source_files = {f for f in source_files if not any(pattern in str(f) for pattern in test_patterns)}
...
def print_report(source_files: Set[Path], test_files: Set[Path], untested: Set[Path], src_dir: str):
