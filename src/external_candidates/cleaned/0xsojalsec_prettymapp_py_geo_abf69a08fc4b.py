# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_prettymapp.py\prettymapp.py\geo_abf69a08fc4b.py
...
def get_aoi(
    ...
    if address is not None:
        if coordinates is not None:
            raise ValueError("Both address and latlon coordinates were provided, please select only one!")
    ...
    df = GeoDataFrame(DataFrame([0], columns=["id"]), crs="EPSG:4326", geometry=[Point(lon, lat)])
