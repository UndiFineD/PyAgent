# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\adamkristopher.py\seo_dataforseo.py\scripts.py\api.py\trends_0945b1442812.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\adamkristopher\seo-dataforseo\scripts\api\trends.py
...
def get_trending_now(location_name: str = None, save: bool = True) -> Dict[str, Any]:
    """
    ...
    try:
        response = client.keywords_data.google_trends_trending_now_live([{"location_name": location}])

        result = response.to_dict() if hasattr(response, "to_dict") else response

        if save:
            save_result(result, category="trends", operation="trending_now", keyword=location)
