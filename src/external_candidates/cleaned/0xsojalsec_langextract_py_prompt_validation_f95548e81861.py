# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\langextract.py\prompt_validation_f95548e81861.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\langextract\prompt_validation.py
    ...
    if level is PromptValidationLevel.ERROR:
        ...
        if failed:
            ...
            raise PromptAlignmentError(
                f"Prompt alignment validation failed: {len(failed)} extraction(s) could not be aligned (e.g., {sample})"
            )
