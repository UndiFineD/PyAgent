# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\alirezarezvani.py\app_store_optimization.py\scripts.py\metadata_optimizer_bfb39fa8151f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\app-store-optimization\scripts\metadata_optimizer.py
...
class MetadataOptimizer:
    ...
    def optimize_title(self, app_name: str, target_keywords: List[str], include_brand: bool = True) -> Dict[str, Any]:
        """
        ...
        if len(target_keywords) > 1:
            option3 = self._build_title_with_keywords(app_name, target_keywords[:2], max_length)
    ...
    def validate_character_limits(self, metadata: Dict[str, str]) -> Dict[str, Any]:
        ...
        for field_name, value in metadata.items():
            if field_name not in self.limits:
                validation_results["warnings"].append(f"Unknown field '{field_name}' for {self.platform} platform")
            ...
            if actual_length > max_length:
                ...
                validation_results["errors"].append(f"'{field_name}' exceeds limit: {actual_length}/{max_length} chars")
    ...
    def _build_title_with_keywords(self, app_name: str, keywords: List[str], max_length: int) -> Optional[str]:
    ...
    def _optimize_short_description(self, app_info: Dict[str, Any], target_keywords: List[str]) -> Dict[str, Any]:
    ...
    def _optimize_subtitle(self, app_info: Dict[str, Any], target_keywords: List[str]) -> Dict[str, Any]:
    ...
    def _optimize_full_description(self, app_info: Dict[str, Any], target_keywords: List[str]) -> Dict[str, Any]:
    ...
    def _generate_density_recommendations(self, keyword_densities: Dict[str, Dict[str, Any]]) -> List[str]:
        ...
        for keyword, data in keyword_densities.items():
            if data["status"] == "too_low":
                recommendations.append(f"Increase usage of '{keyword}' - currently only {data['occurrences']} times")
...
def optimize_app_metadata(platform: str, app_info: Dict[str, Any], target_keywords: List[str]) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "description": optimizer.optimize_description(app_info, target_keywords, "full"),
        "keyword_field": (optimizer.optimize_keyword_field(target_keywords) if platform == "apple" else None),
    }
