# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\didelco.py\pvpc_spain.py\scripts.py\optimize_appliance_67a2ead2ac23.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\didelco\pvpc-spain\scripts\optimize_appliance.py
...
def find_best_window(prices, duration):
    ...
    for i in range(len(sorted_prices) - duration + 1):
        ...
        if all(window[j]["hour"] == window[j - 1]["hour"] + 1 for j in range(1, len(window))):
...
def find_alternatives(prices, duration, best_window, threshold=1.10):
    ...
    for i in range(len(sorted_prices) - duration + 1):
        ...
        if all(window[j]["hour"] == window[j - 1]["hour"] + 1 for j in range(1, len(window))):
            ...
            if start_hour != best_window["start"] and total_cost <= max_cost:
                alternatives.append(
                    {
                        "start": start_hour,
                        "end": window[-1]["hour"] + 1,
                        ...
                        "diff_percent": round(
                            ((total_cost - best_window["total_cost"]) / best_window["total_cost"]) * 100,
                            1,
                        ),
                    }
                )
...
def main():
    ...
    parser.add_argument(
        "--name",
        default="electrodoméstico",
        help="Nombre del electrodoméstico (ej: lavadora)",
    )

    parser.add_argument("--alternatives", type=int, default=2, help="Número de alternativas a mostrar")
