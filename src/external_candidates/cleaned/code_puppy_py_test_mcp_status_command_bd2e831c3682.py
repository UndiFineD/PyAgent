# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\mcp.py\test_mcp_status_command_bd2e831c3682.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_mcp_status_command.py
...
class TestStatusCommand:
    ...
    def test_execute_no_args_shows_list(self, mock_emit_info, mock_mcp_manager):
        ...
        with patch("code_puppy.command_line.mcp.status_command.ListCommand") as mock_list_cmd:
            mock_instance = Mock()
    ...
    def test_execute_with_server_name_success(self, mock_emit_info, mock_mcp_manager):
        ...
        with patch("code_puppy.command_line.mcp.status_command.find_server_id_by_name") as mock_find:
            mock_find.return_value = "test-server-1"

            with patch.object(command, "_show_detailed_server_status") as mock_show_status:
                command.execute(["test-server"])

                mock_find.assert_called_once_with(mock_mcp_manager, "test-server")

                mock_show_status.assert_called_once_with("test-server-1", "test-server", ANY)
    ...
    def test_execute_server_not_found(self, mock_emit_info, mock_mcp_manager):
        ...
        with patch("code_puppy.command_line.mcp.status_command.find_server_id_by_name") as mock_find:
            ...
            with patch("code_puppy.command_line.mcp.status_command.suggest_similar_servers") as mock_suggest:
    ...
    def test_show_detailed_server_status_basic_info(self, mock_emit_info, mock_mcp_manager):
        ...
        command._show_detailed_server_status("test-server-1", "test-server", "group-123")
    ...
    def test_show_detailed_server_status_server_not_accessible(self, mock_emit_info, mock_mcp_manager):
    ...
    def test_show_detailed_server_status_all_states(self, mock_emit_info, mock_mcp_manager):
        ...
        for state_str, expected_state in states_to_test:
            ...
            command._show_detailed_server_status("test-1", "test-server", f"group-{state_str}")

            assert len(mock_emit_info.messages) > 0

    def test_show_detailed_server_status_with_error(self, mock_emit_info, mock_mcp_manager):
    ...
    def test_show_detailed_server_status_quarantined(self, mock_emit_info, mock_mcp_manager):
    ...
    def test_show_detailed_server_status_with_uptime(self, mock_emit_info, mock_mcp_manager):
    ...
    def test_show_detailed_server_status_with_events(self, mock_emit_info, mock_mcp_manager):
    ...
    def test_show_detailed_server_status_exception_handling(self, mock_mcp_manager):
        ...
        mock_mcp_manager.get_server_status.side_effect = Exception("Status fetch failed")
    ...
    def test_show_detailed_server_status_without_group_id(self, mock_emit_info, mock_mcp_manager):
