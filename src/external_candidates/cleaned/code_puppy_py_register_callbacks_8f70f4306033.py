# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\plugins.py\chatgpt_oauth.py\register_callbacks_8f70f4306033.py
...
def _handle_chatgpt_status() -> None:
    ...
    if tokens and tokens.get("access_token"):
        ...
        chatgpt_models = [
            name for name, cfg in load_chatgpt_models().items() if cfg.get("oauth_source") == "chatgpt-oauth-plugin"
        ]
...
def _create_chatgpt_oauth_model(model_name: str, model_config: Dict, config: Dict) -> Any:
    ...
    base_url = model_config.get("custom_endpoint", {}).get("url", CHATGPT_OAUTH_CONFIG["api_base_url"])
