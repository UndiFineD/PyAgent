# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\eval.py\algorithms.py\mcts.py\samplers.py\kld_achievement_sampler_40568ddb885e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\algorithms\mcts\samplers\kld_achievement_sampler.py
            ...
            with self.db_client.get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    programs: List[Tuple[int, Counter]] = [
                        (
                            row["id"],
                            self._compute_achievement_frequencies(row["achievements_json"]),
                        )
                        for row in results
                    ]
