# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\code_puppy.py\command_line.py\model_picker_completion_ebaf27d61fa6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\code_puppy\command_line\model_picker_completion.py
...
class ModelNameCompleter(Completer):
    ...
    def get_completions(self, document: Document, complete_event) -> Iterable[Completion]:
        ...
        text_after_trigger = text_before_cursor[symbol_pos + len(self.trigger) + 1 :].lstrip()
        ...
        for model_name in self.model_names:
            if text_after_trigger and not model_name.lower().startswith(text_after_trigger.lower()):
                continue  # Skip models that don't match the typed text

            meta = "Model (selected)" if model_name.lower() == get_active_model().lower() else "Model"
...
def update_model_in_input(text: str) -> Optional[str]:
    ...
    if content.lower().startswith("/model "):
        ...
        for model in model_names:
            if rest.lower().startswith(model.lower()):
                ...
                if idx != -1:
                    new_text = (text[:idx] + text[idx + len(cmd_and_model_pattern) :]).strip()
    ...
    elif content.lower().startswith("/m ") and not content.lower().startswith("/model "):
