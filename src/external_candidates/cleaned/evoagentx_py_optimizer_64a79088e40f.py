# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\optimizers.py\optimizer_64a79088e40f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\optimizers\optimizer.py
...
class Optimizer(BaseModule):
    ...
    eval_rounds: int = Field(
        default=1,
        description="Run evaluation for `eval_rounds` times and compute the average score.",
    )

    convergence_threshold: int = Field(
        default=5,
        description="If the optimization has not improved the score for `convergence_threshold` steps, the optimization will be stopped.",
    )
