# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\examples.py\engine_integration_example_2acbf6dc553c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\examples\engine_integration_example.py
...
from src.core import CallSession, PlaybackManager, SessionStore
...
class EngineIntegrationExample:
    ...
    async def handle_playback_finished(self, event):
        ...
        if success:
            logger.info("PlaybackFinished handled successfully", playback_id=playback_id)

        else:
            logger.warning("PlaybackFinished for unknown playback", playback_id=playback_id)
    ...
    async def handle_agent_audio(self, event):
        ...
        if not session.audio_capture_enabled:
            logger.debug("Audio capture disabled, skipping processing", call_id=call_id)
            ...
            if transcript:
                ...
                if response:
                    ...
                    if tts_audio:
                        ...
                        playback_id = await self.playback_manager.play_audio(call_id, tts_audio, "response")
