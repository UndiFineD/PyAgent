# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\bark.py
import logging
import os
import warnings
...
class Bark(BaseTTS):
    ...
    def load_bark_models(self):
        self.semantic_model, self.config = load_model(
            ckpt_path=self.config.LOCAL_MODEL_PATHS["text"],
            device=self.device,
            config=self.config,
            model_type="text",
        )
        ...
        self.fine_model, self.config = load_model(
            ckpt_path=self.config.LOCAL_MODEL_PATHS["fine"],
            device=self.device,
            config=self.config,
            model_type="fine",
        )
