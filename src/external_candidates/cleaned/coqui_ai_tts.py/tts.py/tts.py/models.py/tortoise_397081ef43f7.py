# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\tortoise.py
import logging
import os
import random
    ...
    def synthesize(
        ...
        **kwargs,
        ...
        if speaker_wav is not None or speaker is not None:
            ...
            conditioning_latents = (
                voice["auto_conditioning"],
                voice["diffusion_conditioning"],
            )
        ...
        **kwargs,
        ...
        if os.path.exists(diff_path):
            self.diffusion.load_state_dict(
                torch.load(diff_path, weights_only=is_pytorch_at_least_2_4()),
                strict=strict,
            )

        if os.path.exists(clvp_path):
            self.clvp.load_state_dict(
                torch.load(clvp_path, weights_only=is_pytorch_at_least_2_4()),
                strict=strict,
            )
