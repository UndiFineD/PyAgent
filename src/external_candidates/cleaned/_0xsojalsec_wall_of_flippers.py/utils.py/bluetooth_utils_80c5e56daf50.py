# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Wall-of-Flippers\utils\bluetooth_utils.py
# -*- coding: utf-8 -*-
"""
    ...
    try:
        while True:
            ...
            if packet_length and plen != packet_length:
                ...
                if debug:
                    print(
                        "packet with non-matching length: mac=%s adv_type=%02x plen=%s" % (mac_addr_str, adv_type, plen)
                    )
