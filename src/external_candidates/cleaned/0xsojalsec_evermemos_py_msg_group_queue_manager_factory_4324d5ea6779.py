# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\queue.py\msg_group_queue.py\msg_group_queue_manager_factory_4324d5ea6779.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\queue\msg_group_queue\msg_group_queue_manager_factory.py
...
class MsgGroupQueueConfig:
    ...
    def get_cache_key(self) -> str:
        ...
        return (
            f"{self.name}:{self.num_queues}:{self.max_total_messages}:{self.enable_metrics}:{self.log_interval_seconds}"
        )
...
class MsgGroupQueueManagerFactory:
    ...
    async def get_manager(
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.error("❌ Failed to create MsgGroupQueueManager: %s, error: %s", config, e)

                raise

    async def get_default_manager(self, auto_start: bool = True) -> MsgGroupQueueManager:
        """
    ...
    async def get_named_manager(self, name: str, auto_start: bool = True) -> MsgGroupQueueManager:
        """
    ...
    async def get_default_msg_group_queue_manager(self, auto_start: bool = True) -> MsgGroupQueueManager:
        """
