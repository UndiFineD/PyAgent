# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\andretuta.py\polymarket_agent.py\trade_8c19907661f6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\andretuta\polymarket-agent\trade.py
...
def place_order(token_id, side, price, size):
    ...
    try:
        resp = client.create_and_post_order(
            OrderArgs(token_id=token_id, price=float(price), size=float(size), side=order_side)
        )
