# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\tools\global_tools.py
import json
import logging
...
class HumanInputParams(BaseModel):
    text: Optional[str] = Field(
        description="The text message content to be sent to the client.",
        default=None,
    )
    ...
    def _run(self, text: Optional[str]) -> str:
        ...
        try:
            ...
            self.socket_client.emit(
                "message",
                {"room": self.session_id, "type": "error", "message": "TimeOutError"},
            )
            return "exit"
