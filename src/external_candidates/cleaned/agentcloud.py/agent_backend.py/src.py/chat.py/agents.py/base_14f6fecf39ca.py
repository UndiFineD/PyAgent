# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\chat\agents\base.py
import logging
import traceback
import uuid
    ...
    async def stream_execute(self):
        ...
        while True:
            ...
            if past_messages:
                ...
                try:
                    chat_history = self.chat_assistant_obj.mongo_conn.get_chat_history(self.session_id)
                    if chat_history:
                        # Convert chat history to LangChain messages
                        messages = self._convert_chat_to_langchain(chat_history)
                        self.logger.info(
                            f"Reconstructed {len(messages)} messages from chat history for session {self.session_id}"
                        )
                    else:
                        messages = [SystemMessage(content=self.system_message)]
                        self.logger.info(f"Starting fresh conversation for session {self.session_id}")
