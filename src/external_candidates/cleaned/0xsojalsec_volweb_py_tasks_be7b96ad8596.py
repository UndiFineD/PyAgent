# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_volweb.py\evidences.py\tasks_be7b96ad8596.py
...
def start_analysis(dump_id):
    ...
    if instance.dump_os == "Windows":
        ...
        with allow_join_result():
            ...
            if result[17]:
                windows.TimeLineChart(evidence=instance, artefacts=build_timeline(result[17])).save()
    ...
    if instance.dump_os == "Linux":
        ...
        with allow_join_result():
            ...
            if result[6]:
                linux.NetGraph(evidence=instance, artefacts=generate_linux_network_graph(result[6])).save()

            linux.TimeLineChart.objects.filter(evidence=instance).delete()

            if result[7]:
                linux.TimeLineChart(evidence=instance, artefacts=build_timeline(result[7])).save()
