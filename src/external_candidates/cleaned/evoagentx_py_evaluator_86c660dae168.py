# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\evaluators.py\evaluator_86c660dae168.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\evaluators\evaluator.py
...
class Evaluator:
    ...
    def _get_eval_data(
        ...
        assert eval_mode in [
            ...
            "dev",
            "train",
        ], f"Invalid eval_mode: {eval_mode}. Choices: ['test', 'dev', 'train']"
        ...
        **kwargs,
        ...
        if isinstance(graph, WorkFlowGraph) and update_agents:
            if self.agent_manager is None:
                raise ValueError(
                    f"`agent_manager` is not provided in {type(self).__name__}. Please provide an agent manager when evaluating a WorkFlowGraph."
                )
    ...
    def _execute_workflow_graph(
        ...
        **kwargs,
    ) -> Union[str, Tuple[str, List[Message]]]:
        """
        ...
        try:
            ...
            if isinstance(graph, ActionGraph):
                ...
                workflow_graph_outputs = self._execute_workflow_graph(
                    graph=graph, inputs=inputs, return_trajectory=True, **kwargs
                )
            ...
            logger.warning(
                f"Error evaluating example and set the metrics to None:\nExample: {example}\nError: {str(e)}"
            )
    ...
    def _evaluate_single_example_with_context(
        self,
        graph: Union[WorkFlowGraph, ActionGraph],
        example: dict,
        benchmark: Benchmark,
    ...
    def get_evaluation_record_by_id(
        self, benchmark: Benchmark, example_id: str, eval_mode: str = "test"
    ) -> Optional[dict]:
        """
            ...
            async with sem:
                try:
                    return await self._async_evaluate_single_example(
                        graph=graph, example=example, benchmark=benchmark, **kwargs
                    )
    ...
    async def _async_evaluate_single_example(
        self,
        graph: Union[WorkFlowGraph, ActionGraph],
        example: dict,
        benchmark: Benchmark,
    ...
    async def _async_execute_workflow_graph(
        self,
        graph: WorkFlowGraph,
        inputs: dict,
        return_trajectory: bool = False,
        **kwargs,
        ...
        if self.agent_manager is None:
            raise ValueError(
                "`agent_manager` is not provided. Please provide an agent manager when evaluating a WorkFlowGraph."
            )
