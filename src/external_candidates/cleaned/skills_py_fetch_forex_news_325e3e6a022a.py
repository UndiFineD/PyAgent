...
try:
    ...
    print(
        "Error: yfinance package not found. Install with: pip install yfinance>=0.2.40",
        file=sys.stderr,
    )
    ...
    try:
        ...
        for article in news[:limit]:
            news_item = {
                ...
                "published": datetime.fromtimestamp(article.get("providerPublishTime", 0)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fetch FOREX news and market data from Yahoo Finance")

    parser.add_argument("pair", type=str, help=f"Currency pair ({', '.join(PAIR_SYMBOLS.keys())})")
