# Extracted from: C:\DEV\PyAgent\.external\ADFSpoof\ADFSpoof.py
# POC for PFX
import base64
import json
import sys
...
from utils import die, encode_object_guid, print_intro, random_string
...
def parse_args():
    ...
    arg_parser.add_argument(
        "--assertionid",
        help="AssertionID string. Defaults to a random string",
        default=random_string(),
    )
    arg_parser.add_argument(
        "--responseid",
        help="The Response ID. Defaults to random string",
        default=random_string(),
    )
    arg_parser.add_argument(
        "-s",
        "--server",
        help="Identifier for the federation service. Usually the fqdn of the server. e.g. sts.example.com DO NOT include HTTPS://",
    )
    ...
    (
        parser_office365.add_argument(
            "--objectguid",
            help="Object GUID of user to spoof. You can get this from AD",
            required=True,
        ),
    )
    ...
    parser_generic_saml2.add_argument(
        "--endpoint",
        help="The destination/recipient attribute for SAML 2.0 token. Where the SAML token will be sent.",
        default=None,
    )
    parser_generic_saml2.add_argument(
        "--nameidformat",
        help="The format attribute for the NameIdentifier element",
        default=None,
    )
    ...
    parser_dump.add_argument(
        "--path",
        help="Filepath where the signing token will be output.",
        default="token.pfx",
    )
