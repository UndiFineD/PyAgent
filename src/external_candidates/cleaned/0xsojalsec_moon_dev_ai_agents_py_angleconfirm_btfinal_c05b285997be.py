# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_moon_dev_ai_agents.py\src.py\data.py\rbi.py\_03_13_2025.py\backtests_final.py\angleconfirm_btfinal_c05b285997be.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_final\AngleConfirm_BTFinal.py

import numpy as np

import pandas as pd

from talib import GANNAAngLine

from talib import MA_Type as MA_TYPE
...
class AngleConfirm:
    ...
    def on_data(self, data):
        ...
        gann_lines = {MA_TYPE.SMA: [self.risk_percent], MA_TYPE.EWMA: [5]}
    ...
    def on_bar(self):
        ...
        if last_ma_ewma[0] > last_ma_ewma[1] and ((data["high"][-2] + data["low"][-2]) / 2) > last_gann_angle[-1]:
            ...
            if last_ma_ewma[0] > last_gann_angle[-1]:
                ...
                if not position:
                    size = (self.risk_percent * 2) / (
                        (last_ma_ewma[0] - last_gann_angle[-1]) * np.diff(data["high"][-5:]).max()
                    )
                ...
                if not position:
                    size = (self.risk_percent * 2) / (
                        (last_ma_ewma[1] - last_gann_angle[-1]) * np.diff(data["low"][-5:]).max()
                    )
