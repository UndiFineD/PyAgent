# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2022.py\infinity.py\roar_to_picture_3abc3a3a42d4.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\infinity\roar_to_picture.py
...
class DecomposeRoar_SlowFPS(DecomposeAudioSegment):
    ...
    def show_all_waves(self):
        ...
        self.play(
            ...
            MoveToTarget(frame),
            LaggedStart(
                *(ReplacementTransform(graph.copy().set_opacity(0), wave) for wave in sine_waves),
...
class CombineWavesToImage_SlowFPS(FourierCirclesScene):
    ...
    def show_sine_waves(self):
        ...
        for vect, circ in zip(shown_vectors, shown_circles):
            ...
            vect.add_updater(lambda v: v.set_angle(np.log(v.coefficient).imag + self.get_vector_time() * v.freq * TAU))
        ...
        for vect, axes, color in zip(shown_vectors, all_axes, colors):
            ...
            line.add_updater(lambda l: l.put_start_and_end_on(l.vect.get_end(), l.graph.get_start()))
    ...
    def piece_together_circles(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(TransformFromCopy(v1, v2) for v1, v2 in zip(self.shown_vectors, self.vectors[1:])),
                lag_ratio=0.1,
            ),
            LaggedStart(
                *(TransformFromCopy(c1, c2) for c1, c2 in zip(self.shown_circles, self.circles[1:])),
                lag_ratio=0.1,
            ),
            FadeIn(label),
            Write(terms),
        ...
        anims = [
            UpdateFromAlphaFunc(
                drawn_path,
                lambda m, a: m.set_stroke(interpolate_color(BLACK, YELLOW, a)),
            ),
        ...
        self.play(
            UpdateFromAlphaFunc(
                drawn_path,
                lambda m, a: m.set_stroke(interpolate_color(YELLOW, BLACK, a)),
            ),
            VFadeIn(true_path),
