# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\fle.py\env.py\exceptions.py\hinting_name_error_75691269610d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\exceptions\hinting_name_error.py
...
class HintingNameError(NameError):
    def __init__(self, original_error: str, suggestions: list[tuple[str, str]]):
        ...
        if suggestions:
            suggestion_strs = [f"{name}: {type_hint}" if type_hint else name for name, type_hint in suggestions]
