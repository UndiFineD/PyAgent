# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\workflow.py
"""
...
class Workflow:
    ...
    async def workflow_executor(args: ArgsModel):
        ...
        for module in ordered_modules:
                    ...
                    with tqdm(total=len(commands_list), colour="green") as pbar:
                        ...
                        for tool_tuple in commands_list:
                            ...
                            task = loop.create_task(
                                run_with_semaphore(execute_threadpool_obj.execute_and_store(tool_tuple))
                            )
                            tasks.append(task)
            ...
            module_log["module_time_taken"] = str(
                timedelta(
                    seconds=round(
                        module_log["module_end_time"] - module_log["module_start_time"],
                        0,
                    )
                )
            )
