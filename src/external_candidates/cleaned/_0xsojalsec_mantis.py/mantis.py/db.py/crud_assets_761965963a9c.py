# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\db\crud_assets.py
import logging
...
async def update_asset_query(asset: str, org: str, mongodb_query):
    ...
    if asset_exists:
        updated_asset = await assets_collection.update_one({"_id": asset, "org": org}, mongodb_query, True)
        if updated_asset:
            return True
        return False
    else:
