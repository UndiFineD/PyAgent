# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\asterisk_ai_voice_agent.py\src.py\core.py\session_store_bd3e9d4a2c36.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\session_store.py
...
class SessionStore:
    ...
    async def set_gating_token(self, call_id: str, playback_id: str) -> bool:
        ...
        async with self._lock:
            ...
            if not session:
                logger.warning(
                    "Cannot set gating token - call not found",
                    call_id=call_id,
    ...
    async def clear_gating_token(self, call_id: str, playback_id: str) -> bool:
        ...
        async with self._lock:
            ...
            if not session:
                logger.warning(
                    "Cannot clear gating token - call not found",
                    call_id=call_id,
                    playback_id=playback_id,
                )
    ...
    async def cleanup_expired_sessions(self, max_age_seconds: float = 3600) -> int:
        ...
        if expired_calls:
            logger.info("Cleaned up expired sessions", expired_count=len(expired_calls))
