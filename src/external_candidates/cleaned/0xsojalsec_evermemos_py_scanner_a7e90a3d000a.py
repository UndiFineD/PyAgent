# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\di.py\scanner_a7e90a3d000a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\di\scanner.py
...
class ComponentScanner:
    ...
    def _preload_critical_modules(self):
        ...
        self.logger.info("🔄 Preloading critical modules to avoid circular dependencies...")
    ...
    def register_scan_context(self, path: str, metadata: Dict[str, Any]) -> "ComponentScanner":
        """
    ...
    def scan(self) -> "ComponentScanner":
        ...
        if self.parallel and len(python_files) > 1:
            self.logger.info("⚡ Using parallel scan mode (max %d worker threads)", self.max_workers)
    ...
    def _parallel_scan(self, python_files: List[Path]):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(self._scan_file, file_path): file_path for file_path in python_files}
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    self.logger.error("Failed to scan file in parallel %s: %s", file_path, e)
    ...
    def _sequential_scan(self, python_files: List[Path]):
        ...
        for file_path in python_files:
            try:
                ...
                self.logger.error("Failed to scan file sequentially %s: %s", file_path, e)
    ...
    def _scan_file(self, file_path: Path):
        ...
        try:
            ...
            self.logger.error("Unknown error occurred while scanning file %s: %s", file_path, e)
