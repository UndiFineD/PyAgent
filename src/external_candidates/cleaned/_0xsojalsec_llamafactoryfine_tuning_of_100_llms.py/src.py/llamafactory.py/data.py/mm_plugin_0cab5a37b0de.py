# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LlamaFactoryFine-Tuning-of-100-LLMs\src\llamafactory\data\mm_plugin.py
...
class MMPluginMixin:
    ...
    def _get_video_sample_indices(
        ...
        sample_frames = max(
            1,
            math.floor(float(video_stream.duration * video_stream.time_base) * video_fps),
        )
...
class MiniCPMVPlugin(BasePlugin):
    ...
    def get_mm_inputs(
        ...
        if "tgt_sizes" not in mm_inputs:
            ...
            mm_inputs.update(
                {
                    "tgt_sizes": dummy_data,
                    "pixel_values": dummy_data,
                    "image_sizes": dummy_data,
                }
            )
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while VIDEO_PLACEHOLDER in content:
                ...
                content = content.replace(
                    VIDEO_PLACEHOLDER,
                    f"<|begin_of_video|>{video_structure}<|end_of_video|>",
                    1,
                )
                num_video_tokens += 1
