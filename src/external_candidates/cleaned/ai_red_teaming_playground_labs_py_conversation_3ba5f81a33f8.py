# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\ai_red_teaming_playground_labs.py\src.py\chat_score.py\webapi.py\server.py\models.py\conversation_3ba5f81a33f8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\src\chat-score\webapi\server\models\conversation.py
...
from server.keys import (
    REDIS_CONVERSATION_ASSIGNMENT,
    REDIS_CONVERSATION_COUNT,
    REDIS_CONVERSATION_KEY,
    REDIS_CONVERSATION_QUEUE,
    REDIS_CONVERSATION_TTL_KEY,
)
...
class ConversationModel:
    ...
    def add(self, conversation_details: ConversationReviewRequest):
        ...
        try:
            self.__r.set(
                f"{REDIS_CONVERSATION_KEY}{conversation_details.conversation_id}",
                conversation_details.to_json(),
            )
            ...
            for i, c in enumerate(queue):
                if c.assigned_to is None or c.assigned_to == "":
                    ...
                    with self.__r.pipeline() as pipe:
                        ...
                        pipe.set(
                            f"{REDIS_CONVERSATION_TTL_KEY}{assigned_to}",
                            c.guid,
                            ex=CONVERSATION_TTL,
                        )
