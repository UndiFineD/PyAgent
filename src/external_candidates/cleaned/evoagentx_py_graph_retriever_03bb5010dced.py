# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\evoagentx.py\rag.py\retrievers.py\graph_retriever_03bb5010dced.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\evoagentx\rag\retrievers\graph_retriever.py
...
from llama_index.core.graph_stores.types import KG_SOURCE_REL, PropertyGraphStore
...
class BasicLLMSynonymRetriever(BasePGRetriever):
    ...
    def retrieve_from_graph(self, query_bundle: Query, limit: Optional[int] = None) -> List[NodeWithScore]:
        ...
        synonym_prompt = self._synonym_prompt.format_map(
            {"max_keywords": self._max_keywords, "query_str": query_bundle.query_str}
        )
    ...
    async def aretrieve_from_graph(self, query_bundle: Query, limit: Optional[int] = None) -> List[NodeWithScore]:
        synonym_prompt = self._synonym_prompt.format_map(
            {"max_keywords": self._limit, "query_str": query_bundle.query_str}
        )
