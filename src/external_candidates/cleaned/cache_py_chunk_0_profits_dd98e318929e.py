# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\cache.py\work.py\copilot_tmp.py\chunk_0_profits_dd98e318929e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\cache\work\copilot_tmp\chunk_0_profits.py

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\env\utils\profits.py
...
def get_static_profits(new_production_flows, price_list, max_craft_cap=5):
    ...
    for crafted_item in new_production_flows["crafted"]:
        ...
        static_profits += (
            unit_craft_profit * min(max_craft_cap, count) if max_craft_cap > 0 else unit_craft_profit * count
        )
...
def process_achievements(pre_sleep_production_flows, post_sleep_production_flows, achievements):
    for item_key in post_sleep_production_flows["output"]:
        ...
        pre_output_value = (
            pre_sleep_production_flows["output"][item_key] if item_key in pre_sleep_production_flows["output"] else 0
        )
