# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\component.py\llm.py\llm_adapter.py\anthropic_adapter_583cfd5eed3d.py
...
class AnthropicAdapter(LLMBackendAdapter):
    ...
    async def chat_completion(
        ...
        if request.thinking_budget is not None and request.thinking_budget > 0:
            ...
            if any(model_name in request.model for model_name in thinking_supported_models):
                data["thinking"] = {
                    "type": "enabled",
        ...
        for attempt in range(self.max_retries):
            try:
                if request.stream:
                    ...
                    return self._convert_anthropic_response(response.json(), request.model)

            except httpx.HTTPStatusError as e:
                if attempt == self.max_retries - 1 or e.response.status_code < 500:
                    raise RuntimeError(f"Anthropic chat completion request failed: {e.response.text}") from e

                await asyncio.sleep(2**attempt)

            except Exception as e:
                if attempt == self.max_retries - 1:
                    raise RuntimeError(f"An unexpected error occurred in AnthropicAdapter: {e}") from e

                await asyncio.sleep(2**attempt)

    def _convert_anthropic_response(self, response_data: Dict[str, Any], model: str) -> ChatCompletionResponse:
    ...
    async def _stream_chat_completion(self, data: Dict[str, Any]) -> AsyncGenerator[str, None]:
