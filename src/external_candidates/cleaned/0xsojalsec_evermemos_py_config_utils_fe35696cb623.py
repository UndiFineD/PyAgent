# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\tenants.py\tenantize.py\oxm.py\mongo.py\config_utils_fe35696cb623.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\core\tenants\tenantize\oxm\mongo\config_utils.py
...
def get_tenant_mongo_config() -> Optional[Dict[str, Any]]:
    ...
    if not mongo_config:
        ...
        logger.info(
            ...
            final_config.get("uri") or f"host={final_config.get('host')}:{final_config.get('port')}",
    ...
    if mongo_config.get("uri"):
        final_config = {
            ...
            "database": mongo_config.get("database") or generate_tenant_database_name(DEFAULT_DATABASE_NAME),
        }
        ...
        final_config = {
            "host": mongo_config.get("host") or env_fallback_config.get("host", "localhost"),
            "port": mongo_config.get("port") or env_fallback_config.get("port", 27017),
            "username": mongo_config.get("username") or env_fallback_config.get("username"),
            "password": mongo_config.get("password") or env_fallback_config.get("password"),
            # database: use if specified in tenant configuration, otherwise generate tenant-aware name
            "database": mongo_config.get("database") or generate_tenant_database_name(DEFAULT_DATABASE_NAME),
        }
    ...
    logger.debug(
        ...
        tenant_info.tenant_id,
        ("uri" if final_config.get("uri") else f"host={final_config.get('host')}:{final_config.get('port')}"),
