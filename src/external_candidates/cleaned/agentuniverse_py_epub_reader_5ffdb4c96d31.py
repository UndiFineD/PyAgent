# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentuniverse.py\agentuniverse.py\agent.py\action.py\knowledge.py\reader.py\file.py\epub_reader_5ffdb4c96d31.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\file\epub_reader.py
...
class EpubReader(Reader):
    ...
    def _load_data(self, file: Union[str, Path], ext_info: Optional[Dict] = None) -> List[Document]:
        ...
        try:
            ...
            raise ImportError("EbookLib is required to read EPUB files: `pip install EbookLib`")
        ...
        book_metadata = {
            ...
            "language": (
                book.get_metadata("DC", "language")[0][0] if book.get_metadata("DC", "language") else "Unknown"
            ),
            "publisher": (
                book.get_metadata("DC", "publisher")[0][0] if book.get_metadata("DC", "publisher") else "Unknown"
            ),
        }
