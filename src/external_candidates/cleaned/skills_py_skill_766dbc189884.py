# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\genkin_he.py\longbridge_openapi.py\skill_766dbc189884.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\genkin-he\longbridge-openapi\skill.py
...
class LongbridgeSkill:
    ...
    async def cancel_order(self, order_id: str) -> Dict[str, Any]:
        ...
        return {"status": "success", "order_id": order_id, "message": "订单已撤销"}

    async def get_today_orders(
        self, symbol: Optional[str] = None, status: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_history_orders(
        ...
        orders = self.trade_ctx.history_orders(symbol=symbol, start_at=start, end_at=end)
    ...
    async def get_candlesticks(
        self, symbol: str, period: str, count: int, adjust_type: str = "NoAdjust"
    ) -> List[Dict[str, Any]]:
        """
...
async def submit_order(
    ...
    return await _skill.submit_order(symbol, order_type, side, quantity, time_in_force, price, remark)
