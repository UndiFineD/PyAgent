# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_factorio_learning_environment.py\data.py\vqa.py\tasks.py\factory.py\nearest_entity.py\solver_30a910735223.py
    ...
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        ...
        if valid_entity_types:
            for _ in range(min(questions_per_position, len(valid_entity_types))):
                ...
                closest = min(
                    instances,
                    key=lambda e: ((e.position.x - center_x) ** 2 + (e.position.y - center_y) ** 2) ** 0.5,
                )
