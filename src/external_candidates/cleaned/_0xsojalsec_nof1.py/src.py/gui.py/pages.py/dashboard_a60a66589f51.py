                        ...
                        for asset_data in market_data:
                            ...
                            with ui.card().classes("p-4 bg-gradient-to-br from-gray-700 to-gray-800"):
                                ...
                                with ui.column().classes("gap-1 text-sm"):
                                    ui.label(f"EMA20 (5m): {ema20:.2f}" if ema20 else "EMA20: N/A").classes(
                                        "text-gray-300"
                                    )
                                    ui.label(f"RSI14 (5m): {rsi14:.2f}" if rsi14 else "RSI14: N/A").classes(
                                        "text-gray-300"
                                    )
                                    ui.separator()
                                    ui.label(f"EMA20 (4h): {lt_ema20:.2f}" if lt_ema20 else "EMA20 (4h): N/A").classes(
                                        "text-gray-400"
                                    )
                                    ui.label(f"EMA50 (4h): {lt_ema50:.2f}" if lt_ema50 else "EMA50 (4h): N/A").classes(
                                        "text-gray-400"
                                    )
            else:
                with market_data_container:
