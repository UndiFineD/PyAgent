# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\pages\reasoning.py
"""
                            ...
                            for decision in filtered_decisions:
                                ...
                                with ui.timeline_entry(
                                    ...
                                    with ui.grid(columns=2).classes("gap-2 text-xs text-gray-400"):
                                        ...
                                        ui.label(
                                            (
                                                f"Exit Plan: {exit_plan[:50]}..."
                                                if len(str(exit_plan)) > 50
                                                else f"Exit Plan: {exit_plan}"
                                            ),
                                            column_span=2,
                                        )
                    else:
                        ui.label(f"No {action_filter.value} decisions in current batch").classes(
