    ...
    with ui.column().classes("w-full items-start"):
        ...
        with ui.card().classes("w-full p-4 mb-4"):
            ...
            with ui.row().classes("w-full gap-4 items-center"):
                ...
                # Action filter
                action_filter = ui.select(label="Action", options=["All", "buy", "sell", "hold"], value="All").classes(
                    "flex-1"
                )

                # Limit filter
                limit_filter = ui.number(label="Max Results", value=100, min=10, max=1000, step=10).classes("flex-1")
