# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\pages\settings.py
"""
...
def create_settings(bot_service: BotService, state_manager: StateManager):
            ...
            with ui.tab_panel(tab_strategy):
                ...
                with ui.column().classes("gap-4 w-full max-w-2xl"):
                    ...
                    async def save_strategy_config():
                        try:
                            ...
                            if save_config(config_data):
                                ...
                                assets_list = [
                                    a.strip()
                                    for a in config_data["strategy"]["assets"].replace(",", " ").split()
                                    if a.strip()
                                ]
                    ...
                    with ui.row().classes("gap-2"):
                        ui.button(
                            "Save Configuration",
                            on_click=save_strategy_config,
                            icon="save",
                        ).props("color=primary")
                        ui.button(
                            "Load Configuration",
                            on_click=load_strategy_config,
                            icon="refresh",
                ...
                with ui.column().classes("gap-4 w-full max-w-2xl"):
                    ...
                    async def test_api_connections():
                        ...
                        try:
                            ...
                            taapi_status.text = (
                                f"TAAPI: {'🟢 Connected' if results.get('TAAPI', False) else '🔴 Failed'}"
                            )
                            hyperliquid_status.text = (
                                f"Hyperliquid: {'🟢 Connected' if results.get('Hyperliquid', False) else '🔴 Failed'}"
                            )
                            openrouter_status.text = (
                                f"OpenRouter: {'🟢 Connected' if results.get('OpenRouter', False) else '🔴 Failed'}"
                            )
                            ...
                            if connected_count == total_count:
                                ui.notify(
                                    f"All APIs connected successfully! ({connected_count}/{total_count})",
                                    type="positive",
                                )
                            elif connected_count > 0:
                                ui.notify(
                                    f"Partially connected: {connected_count}/{total_count} APIs",
                                    type="warning",
                                )
                            else:
                                ui.notify(
                                    "All connections failed. Check your API keys.",
                                    type="negative",
                                )
                    ...
                    with ui.card().classes("bg-orange-900 p-4"):
                        ...
                        ui.label(
                            "Never share your private keys. Keys are stored in data/config.json - keep this file secure!"
            ...
            with ui.tab_panel(tab_risk):
                ...
                with ui.column().classes("gap-6 w-full max-w-2xl"):
                    ...
                    with ui.row().classes("items-center w-full gap-4"):
                        ...
                        max_position_label = ui.label(
                            f"${config_data['risk_management']['max_position_size']:,.0f}"
                        ).classes("text-white font-bold min-w-[100px]")
                        max_position_slider.on(
                            "update:model-value",
                            lambda e: max_position_label.set_text(f"${e.args:,.0f}"),
                        )
                    ...
                    with ui.row().classes("items-center w-full gap-4"):
                        ...
                        max_leverage_label = ui.label(f"{config_data['risk_management']['max_leverage']:.1f}x").classes(
                            "text-white font-bold min-w-[100px]"
                        )
                        max_leverage_slider.on(
                            "update:model-value",
                            lambda e: max_leverage_label.set_text(f"{e.args:.1f}x"),
                        )
                    ...
                    with ui.row().classes("items-center w-full gap-4"):
                        ...
                        stop_loss_label = ui.label(f"{config_data['risk_management']['stop_loss_pct']:.1f}%").classes(
                            "text-white font-bold min-w-[100px]"
                        )
                        stop_loss_slider.on(
                            "update:model-value",
                            lambda e: stop_loss_label.set_text(f"{e.args:.1f}%"),
                        )
                    ...
                    with ui.row().classes("items-center w-full gap-4"):
                        ...
                        take_profit_label = ui.label(
                            f"{config_data['risk_management']['take_profit_pct']:.0f}%"
                        ).classes("text-white font-bold min-w-[100px]")
                        take_profit_slider.on(
                            "update:model-value",
                            lambda e: take_profit_label.set_text(f"{e.args:.0f}%"),
                        )
                    ...
                    with ui.card().classes("bg-red-900 p-4"):
                        ...
                        ui.label(
                            "High leverage increases both potential profits and losses. Use with caution!"
                        ).classes("text-sm text-gray-200")
                ...
                with ui.column().classes("gap-4 w-full max-w-2xl"):
                    ...
                    async def test_notification():
                        try:
                            if desktop_enabled_checkbox.value:
                                ui.notify(
                                    "Test notification sent! This is how trade alerts will look.",
                                    type="info",
                                )
                            else:
                            ...
                            if telegram_enabled_checkbox.value and telegram_token_input.value:
                                ui.notify(
                                    "Telegram test notification would be sent here",
                                    type="info",
                                )
                    ...
                    async def save_notification_config():
                        try:
                            ...
                            ui.notify(
                                f"Error saving notification config: {str(e)}",
                                type="negative",
                            )
                    ...
                    with ui.row().classes("gap-2"):
                        ui.button(
                            ...
                            on_click=save_notification_config,
                            icon="save",
                        ).props("color=primary")
                        ui.button(
                            "Test Notification",
                    ...
                    with ui.card().classes("bg-blue-900 p-4"):
                        ...
                        ui.label("Configure notifications for trade executions, errors, and daily summaries.").classes(
    ...
    async def load_initial_config():
        ...
        try:
            current_config = (
                await bot_service.get_current_config() if hasattr(bot_service, "get_current_config") else None
            )
            if current_config:
