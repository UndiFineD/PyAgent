# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\backend\agent\decision_maker.py
...
class TradingAgent:
    ...
    def __init__(self):
        ...
        if ":free" in self.model.lower() or "deepseek" in self.model.lower():
            logging.info(
                f"[INFO] Using model {self.model} - dynamic tool use may not be available. Bot will work with pre-fetched indicators only."
            )
        ...
        for _ in range(6):
            ...
            try:
                ...
                if "no endpoints found" in error_message.lower() and "tool" in error_message.lower():
                    logging.warning(
                        f"Model {self.model} doesn't support tool use on OpenRouter; retrying without tools."
                    )
                    if allow_tools:
                        allow_tools = False
                        continue
                ...
                if allow_structured and (
                    "response_format" in err_text or "structured" in err_text or e.response.status_code in (400, 422)
                ):
            ...
            try:
                ...
                if not isinstance(parsed, dict):
                    logging.error(
                        "Expected dict payload, got: %s; attempting sanitize",
                        type(parsed),
                    )
