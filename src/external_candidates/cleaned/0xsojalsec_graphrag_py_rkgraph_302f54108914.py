# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_graphrag.py\core.py\graph.py\rkgraph_302f54108914.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Graph\RKGraph.py
...
from Core.Common.Utils import clean_str, is_float_regex, split_string_by_multi_markers
...
class RKGraph(BaseGraph):
    ...
    async def _build_graph(self, chunk_list: List[Any]):
        try:
            elements = await asyncio.gather(*[self._extract_entity_relationship(chunk) for chunk in chunk_list])
    ...
    async def _extract_records_from_chunk(self, chunk_info: TextChunk):
        ...
        prompt_template = (
            GraphPrompt.ENTITY_EXTRACTION_KEYWORD if self.config.enable_edge_keywords else GraphPrompt.ENTITY_EXTRACTION
        )
        ...
        return split_string_by_multi_markers(final_result, [DEFAULT_RECORD_DELIMITER, DEFAULT_COMPLETION_DELIMITER])
