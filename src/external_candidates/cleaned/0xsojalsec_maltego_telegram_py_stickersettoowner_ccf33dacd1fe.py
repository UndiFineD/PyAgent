# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_maltego_telegram.py\transforms.py\stickersettoowner_ccf33dacd1fe.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-maltego-telegram\transforms\StickerSetToOwner.py
...
async def fetch_sticker_set_owner(short_name: str):
    async with app:
        ...
        try:
            inline_search = await app.get_inline_bot_results("tgdb_bot", str(owner_id))
    ...
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        ...
        if owner.username is not None:
            ...
            stickerset_owner_entity.addProperty(
                ...
                value=(owner_info["full_name"] if owner_info["full_name"] else owner.username),
            )
