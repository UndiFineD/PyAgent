# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_gunnerc2.py\gui.py\create_listener_dialog_21f2c31ab225.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\create_listener_dialog.py
...
class CreateListenerDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        rlay.addWidget(self.ed_name)

        rlay.addWidget(self.chip_name)
        ...
        lay_h = QHBoxLayout(row_host)

        lay_h.setContentsMargins(0, 0, 0, 0)

        lay_h.addWidget(self.ed_host)

        lay_h.addWidget(self.chip_host)
        ...
        lay_p = QHBoxLayout(row_port)

        lay_p.setContentsMargins(0, 0, 0, 0)

        lay_p.addWidget(self.ed_port)

        lay_p.addWidget(self.chip_port)
        ...
        self.ed_cert.setPlaceholderText("Optional: PEM certificate")

        self.btn_cert = QPushButton("Choose…")

        row_cert = QWidget()

        lc = QHBoxLayout(row_cert)

        lc.setContentsMargins(0, 0, 0, 0)

        lc.addWidget(self.ed_cert)

        lc.addWidget(self.btn_cert)

        fa.addRow("TLS Cert", row_cert)

        self.ed_key = QLineEdit()

        self.ed_key.setPlaceholderText("Optional: PEM private key")

        self.btn_key = QPushButton("Choose…")

        row_key = QWidget()

        lk = QHBoxLayout(row_key)

        lk.setContentsMargins(0, 0, 0, 0)

        lk.addWidget(self.ed_key)

        lk.addWidget(self.btn_key)
    ...
    def _update_create_enabled(self):
        ok = all(lab.styleSheet().find("#18c964") != -1 for lab in (self.chip_name, self.chip_host, self.chip_port))
