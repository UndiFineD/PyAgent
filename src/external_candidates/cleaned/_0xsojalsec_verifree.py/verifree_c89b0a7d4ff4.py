# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VeriFree\verifree.py
                ...
                trajectory_data.append(
                    VeriFreeTrajectoryData(
                        ...
                        gt_text=self.tokenizer.decode(
                            list(prompt_token_ids[i]) + list(think_ids[i][j]) + list(answer_ids[i][j])
                        ),
...
class VeriFreeLearner(PPOLearner):
    def _init(self, args: VeriFreeArgs, actors: List[ActorBase]) -> None:
        ...
        self.collector = FeedbackCollector(args, actors, PlasmaShmClient(self.ipc_server))
    ...
    def prepare_data(self, strategy, tokenizer):
        ...
        self.eval_prompts_dataset = self.eval_prompts_dataloader = None  # We use our own `self.eval_dataset_dict`.
    ...
    def eval_dataloader_collate_fn(self, item_list):
        ...
        for item in item_list:
            ...
            formatted_problems.append(TEMPLATE_FACTORY[args.prompt_template](item["problem"]))
    ...
    def evaluate(self, dataloader, steps):
        ...
        for benchmark_name, dataset in self.eval_dataset_dict.items():
            ...
            metrics = super().evaluate(eval_prompts_dataloader, f"{steps}_{benchmark_name}")
            all_metrics.update({k.replace("eval/", f"eval/{benchmark_name}/"): v for k, v in metrics.items()})
    ...
    def learn(self, learning_round: int):
        ...
        logging.info(f"start learn() buffer_len={len(self.pi_buffer)} dl_len={len(dataloader)}")
    ...
    def compute_ppo_advantages(self, rewards, input_ids, att_mask, response_masks):
        ...
        with torch.no_grad():
            for i in range(0, len(input_ids), self.args.train_batch_size_per_device):
                ...
                batch_values = self.critic(input_ids=input_ids[batch_inds], attention_mask=att_mask[batch_inds])
    ...
    def compute_grpo_advantages(self, rewards, response_masks):
        ...
        mean_grouped_rewards = mean_grouped_rewards.repeat_interleave(self.args.num_samples, dim=0)
    ...
    def learning_step(self, trajectory):
        ...
        final_rewards = (torch.tensor(trajectory["logp_reward"]).to(device).reshape(-1, 1)).float() * args.reward_scale
        ################  exp(logp) ################
        ...
        action_logprobs = [torch.tensor(lp).to(device) for lp in trajectory["gt_action_logprobs"]]
        ...
        indices = torch.arange(response_masks.size(1), device=response_masks.device).expand_as(response_masks)
        masked_indices = torch.where(response_masks, indices, torch.full_like(indices, -1))
        ...
        if self.ref_model is not None:
            ...
            with torch.no_grad():
                for i in range(0, len(input_ids), args.train_batch_size_per_device):
                    ...
                    batch_ref_logits = self.ref_model(input_ids[batch_inds], attention_mask=att_mask[batch_inds])[
                        "logits"
                    ].float()
        ...
        for _ in range(args.num_ppo_epochs):
            ...
            for b_st in range(0, len(input_ids), args.train_batch_size_per_device):
                ...
                mb_last_valid_token_pos = torch.where(mb_valid_token_count_per_pos == 0)[0]
        ...
        infos.update({f"{k}_nan": torch.tensor(stats[k]).isnan().sum() for k in stats.keys()})
        infos.update({f"{k}_inf": torch.tensor(stats[k]).isinf().sum() for k in stats.keys()})
        if not args.reinforce_update:
            infos["ratio_max"] = torch.tensor(stats["ratio_max"]).max()
            infos["ratio_max"] = torch.tensor(stats["ratio_max"]).max()
            infos["zero_pg_loss_count"] = torch.tensor(stats["zero_pg_loss_count"]).float().mean()
