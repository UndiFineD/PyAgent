# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-web-brutator\lib\core\Requester.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import enum
...
class Requester:
    ...
    def get_http_auth_type(url, headers={}):
        headers["User-Agent"] = USER_AGENT
        try:
            r = requests.get(url, verify=False, headers=headers, timeout=Globals.timeout)
        except Exception as e:
            raise RequestException("Network error: {}".format(e))
    ...
    def http_auth(url, auth_type, username, password, headers={}):
        ...
        headers["User-Agent"] = USER_AGENT
        try:
            r = requests.get(url, headers=headers, auth=auth, verify=False, timeout=Globals.timeout)
            return r
        except Exception as e:
            raise RequestException("Network error: {}".format(e))
