# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\ace_plus.py\inference.py\ace_plus_inference_639c19d5af27.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\inference\ace_plus_inference.py
...
from .registry import INFERENCES, BaseInference
...
class ACEInference(BaseInference):
    ...
    def __init__(self, cfg, logger=None):
        ...
        self.input = {
            k.lower(): (dict(v).get("DEFAULT", None) if isinstance(v, (dict, OrderedDict, Config)) else v)
            for k, v in cfg.SAMPLE_ARGS.items()
        }
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image, mask, change_image, content_image, out_h, out_w, slice_w = self.image_processor.preprocess(
            reference_image,
            edit_image,
            edit_mask,
        ...
        src_image_list, src_mask_list, modify_image_list, edit_id, prompt = (
            [image],
            [mask],
            [change_image],
            [[0]],
        ...
        imgs = [x_i["reconstruct_image"].float().permute(1, 2, 0).cpu().numpy() for x_i in out_image]

        imgs = [Image.fromarray((img * 255).astype(np.uint8)) for img in imgs]

        edit_image = Image.fromarray(
            (torch.clamp(image[0] / 2 + 0.5, min=0.0, max=1.0) * 255)
            .float()
            .permute(1, 2, 0)
            .cpu()
            .numpy()
        ...
        change_image = Image.fromarray(
            (torch.clamp(change_image[0] / 2 + 0.5, min=0.0, max=1.0) * 255)
            .float()
            .permute(1, 2, 0)
            .cpu()
            .numpy()
        ...
        return (
            self.image_processor.postprocess(imgs[0], slice_w, out_w, out_h),
            edit_image,
            change_image,
            mask,
            seed,
