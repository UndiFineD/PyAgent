# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\coqui_ai_tts.py\tts.py\encoder.py\utils.py\generic_utils_021caf97eff7.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\encoder\utils\generic_utils.py
        ...
        if "rir" in augmentation_config.keys():
            ...
            if self.rir_config["rir_path"]:
                self.rir_files = glob.glob(
                    os.path.join(self.rir_config["rir_path"], "**/*.wav"),
                    recursive=True,
                )
            ...
            logger.info(
                "Using RIR Noise Augmentation: with %d audios instances",
                len(self.rir_files),
            )
