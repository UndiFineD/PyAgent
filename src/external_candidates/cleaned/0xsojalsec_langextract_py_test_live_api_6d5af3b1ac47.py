# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\tests.py\test_live_api_6d5af3b1ac47.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\tests\test_live_api.py
...
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY") or os.environ.get("LANGEXTRACT_API_KEY")

OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY")

skip_if_no_gemini = pytest.mark.skipif(
    not GEMINI_API_KEY,
    reason=("Gemini API key not available (set GEMINI_API_KEY or LANGEXTRACT_API_KEY)"),
)
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    is_transient = any(err in error_str for err in transient_errors) or error_type in [
                        "ServiceUnavailable",
                        "RateLimitError",
                        "Timeout",
                    ]
                    ...
                    if is_transient and attempt < max_retries:
                        print(f"\nTransient error ({error_type}) on attempt {attempt + 1}/{max_retries + 1}: {e}")
...
class TestLiveAPIGemini(unittest.TestCase):
    ...
    @retry_on_transient_errors(max_retries=2)
    @pytest.mark.xfail(
        reason=("Known tokenizer issue with non-Latin characters - see GitHub issue #13"),
        strict=True,
    )
    def test_multilingual_medication_extraction(self):
        ...
        medication_extractions = [e for e in result.extractions if e.extraction_class == "medication"]
    ...
    """)
        ...
        for extraction in result.extractions:
            assert extraction.attributes is not None, f"Missing attributes for {extraction.extraction_text}"
        ...
        assert len(medication_groups) >= 2, f"Expected at least 2 medications, found {len(medication_groups)}"
        ...
        for med_name, extractions in medication_groups.items():
            ...
            assert "medication" in extraction_classes, f"{med_name} group missing medication entity"

            # Dosage is expected but might be formatted differently

            assert any(c in extraction_classes for c in ["dosage", "dose"]), f"{med_name} group missing dosage"
