# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\hiehoo.py\mingli.py\scripts.py\calculate_bazi_0e4ca0cfd797.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hiehoo\mingli\scripts\calculate-bazi.py
...
def analyze_elements(elements):
    ...
    if lacking:
        parts.append(f"Lacks {', '.join(lacking)}")
...
def calculate_bazi(date_str, time_str, tz_str):
    ...
    bazi_chart = {
        "year": format_pillar(year_stem, year_branch),
    ...
    day_master = {
        "chinese": day_master_stem["zh"],
        "viet": day_master_stem["viet"],
        "element": day_master_stem["element"],
        "polarity": day_master_stem["polarity"],
    }

    elements = calculate_five_elements(
        year_stem,
        year_branch,
        month_stem,
        month_branch,
