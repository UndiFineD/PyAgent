...
class Trainer:
    def __init__(self, args, world_size, rank):
        ...
        if self.args.num_steps != None:
            ...
            self.args.num_epochs = (
                math.ceil(self.total_step / math.floor(self.train_dataset_length / self.args.batch_size))
    ...
    def validate(self, valid_loader=None, hide_progress=True):
        ...
        if self.rank == 0:
            ...
            logging.info(
                f"Step [{self.progress['step']}/{self.total_step}]\t Time elapsed {(time.time() - self.start_time) / 3600.0:.2f}h, Val Loss: {val_loss:.4f}, Val Top10Acc: {val_top10acc:.4f}"
            )
    ...
    def _setup_optimizer(self):
        if self.args.optimizer_name == "ScaledAdam":
            ...
            def lr_lambda(current_step: int):
                ...
                return max(
                    0.0,
                    float(self.total_step - current_step) / float(max(1, self.total_step - warmup_steps)),
                )
