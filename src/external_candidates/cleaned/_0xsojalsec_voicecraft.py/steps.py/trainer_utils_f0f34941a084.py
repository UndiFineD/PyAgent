# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\steps\trainer_utils.py
...
def print_model_info(model, print_model=False, print_params=True):
    ...
    if print_params:
        ...
        for name in all_params:
            logging.info(f"{name}: {all_params[name] / 1000000.0:.2f}m")
...
class DistributedDynamicBatchSampler(Sampler):
    ...
    def __init__(
        ...
        assert len(self._replica_batches) == self.num_samples, (
