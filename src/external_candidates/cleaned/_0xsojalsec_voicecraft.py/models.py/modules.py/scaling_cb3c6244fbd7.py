# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\models\modules\scaling.py
...
def TransposeConv1d(*args, kernel_size: int = 3, padding: str = "same", **kwargs) -> nn.Sequential:
    """
    Transpose -> Conv1d
    """
    return nn.Sequential(
...
def Conv1dTranspose(*args, kernel_size: int = 3, padding: str = "same", **kwargs) -> nn.Sequential:
    """
...
def TransposeSRConv1d(*args, kernel_size: int = 3, padding: str = "same", **kwargs) -> nn.Sequential:
    """
    Transpose -> SRConv1d
    """
    return nn.Sequential(
...
def SRConv1dTranspose(*args, kernel_size: int = 3, padding: str = "same", **kwargs) -> nn.Sequential:
    """
    SRConv1d -> Transpose
    """
    return nn.Sequential(
    ...
    def backward(ctx, x_grad: Tensor):
        ...
        with torch.enable_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                scale = ctx.grad_scale * (x_grad.to(torch.float32).norm() / (penalty_grad.norm() + 1.0e-20))
...
class WithLoss(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, ans_grad: Tensor):
        return ans_grad, torch.ones(ctx.y_shape, dtype=ans_grad.dtype, device=ans_grad.device)
