# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\models\modules\transformer.py
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if need_weights:
            if self.norm_first:
                out, attn = self._sa_block_attn(self.norm1(x, stage_embedding), src_mask, src_key_padding_mask, past)
            ...
            if self.norm_first:
                out = self._sa_block(self.norm1(x, stage_embedding), src_mask, src_key_padding_mask, past)
                out, present = out  # present is the kvcache of the present timestep
                x = x + out
                x = x + self._ff_block(self.norm2(x, stage_embedding))
            else:
