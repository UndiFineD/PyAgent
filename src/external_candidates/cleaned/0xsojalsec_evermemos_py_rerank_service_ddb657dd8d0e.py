# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\agentic_layer.py\rerank_service_ddb657dd8d0e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\agentic_layer\rerank_service.py
...
class RerankService(RerankServiceInterface):
    ...
    async def _make_rerank_request(self, query: str, documents: List[str], instruction: str = None) -> Dict[str, Any]:
        if not documents:
            return {"results": []}
    ...
    def _convert_response_format(self, combined_response: Dict[str, Any], num_documents: int) -> Dict[str, Any]:
    ...
    def _extract_text_from_hit(self, hit: Dict[str, Any]) -> str:
        ...
        if not hasattr(self, "_debug_logged"):
            ...
            logger.info(
                f"[DEBUG] _extract_text_from_hit: memory_type={memory_type!r}, hit_keys={list(hit.keys())[:10]}, source_keys={list(source.keys())[:10]}"
            )
    ...
    async def _rerank_all_hits(
        ...
        try:
            logger.debug(f"Starting reranking, query text: {query}, number of texts: {len(all_texts)}")
            ...
            sorted_hits = sorted(all_hits, key=lambda x: x.get("score", 0), reverse=True)
