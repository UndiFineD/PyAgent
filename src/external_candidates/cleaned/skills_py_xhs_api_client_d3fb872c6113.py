# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\ty_teo.py\xiaohongshu_scraper.py\scripts.py\xhs_api_client_d3fb872c6113.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ty-teo\xiaohongshu-scraper\scripts\xhs_api_client.py
...
def main():
    parser = argparse.ArgumentParser(description="小红书笔记信息获取（轻量级客户端）")
    ...
    if args.json:
        ...
        print(
            f"❤️  点赞: {data.get('点赞数量', '')} | 收藏: {data.get('收藏数量', '')} | 评论: {data.get('评论数量', '')}"
        )
