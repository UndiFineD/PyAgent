# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\once_useful_constructs.py\fractals_af80e12c43b9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\once_useful_constructs\fractals.py
...
from manimlib.mobject.geometry import Circle, Polygon, RegularPolygon
...
def fractalification_iteration(vmobject, dimension=1.05, num_inserted_anchors_range=list(range(1, 4))):
    ...
    if num_points > 0:
        ...
        original_anchors = [vmobject.point_from_proportion(x) for x in np.linspace(0, 1 - 1.0 / num_points, num_points)]
        ...
        for (
            ...
            inserted_points = [interpolate(p1, p2, alpha) for alpha in np.linspace(0, 1, num_inserts + 2)[1:-1]]
            ...
            inserted_points = [
                point + u * offset_len * offset_unit_vect for u, point in zip(it.cycle([-1, 1]), inserted_points)
            ]
    ...
    vmobject.set_submobjects(
        [fractalification_iteration(submob, dimension, num_inserted_anchors_range) for submob in vmobject.submobjects]
    )
...
class SnakeCurve(FractalCurve):
    ...
    def get_anchor_points(self):
        ...
        lower_left = ORIGIN + LEFT * (self.radius - step / 2) + DOWN * (self.radius - step / 2)
