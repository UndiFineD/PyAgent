# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_videos.py\_2019.py\clacks.py\solution2.py\wordy_scenes_94f9aa6b8e2a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2019\clacks\solution2\wordy_scenes.py
...
class ConnectionToOptics(Scene):
    def construct(self):
        ...
        for g1, g2 in zip(k_groups, o_groups):
            ...
            arrow.move_to(interpolate(g1[0].get_right(), g2[0].get_left(), 0.5))
        ...
        for k in [0, 2]:
            rect = DashedVMobject(Rectangle(height=FRAME_HEIGHT - 1.5, width=width), num_dashes=100)
        ...
        self.play(Write(arrows[0]), FadeIn(c_group, LEFT))

        self.wait()

        self.play(FadeInFromDown(m_group))

        self.play(Write(arrows[1]), FadeIn(a_group, LEFT))
    ...
    def get_kinematics_groups(self):
        ...
        momentum_eq = OldTex("m_1 v_1 + m_2 v_2 = \\text{const.}", tex_to_color_map=tex_to_color_map)

        energy_label = OldTexText("Conservation of energy")

        momentum_label = OldTexText("Conservation of momentum")
    ...
    def get_speed_group(self):
        ...
        def update_speed_light(light, period=2, time_width=0.05):
            ...
            light.pointwise_become_partial(speed_light_template, max(a, 0), min(b, 1))
    ...
    def get_angle_group(self):
        ...
        dashed_lines = VGroup(DashedLine(*points[0:2]), DashedLine(*points[1:3]))
...
class RearrangeMomentumEquation(ShowMomentumConservation):
    ...
    def show_with_x_and_y(self):
        ...
        self.play(
            FadeIn(VGroup(m_array.get_brackets(), dot, d_array.get_brackets(), rhs)),
