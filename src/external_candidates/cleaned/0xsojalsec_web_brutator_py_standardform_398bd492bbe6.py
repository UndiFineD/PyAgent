# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_web_brutator.py\lib.py\modules.py\standardform_398bd492bbe6.py
...
class Standardform:
    ...
    def __extract_form_fields(self, soup):
        ...
        for select in soup.find_all("select", attrs={"name": True}):
            ...
            selected_options = [
                option for option in options if option.has_attr("selected") and option.has_attr("value")
            ]
            ...
            if not is_multiple:
                ...
                value = [option["value"] for option in selected_options if option.has_attr("value")]
    ...
    def check(self):
        ...
        logger.warning("This module is based on heuristics and is prone to false negatives/positives")
        ...
        if target_form:
            ...
            logger.error("No standard web auth <form> with password field has been detected on the page")
        ...
        if not self.username_field:
            inputs_no_type = target_form.find_all("input", type=False, attrs={"name": True})
    ...
    def try_auth(self, username, password):
        ...
        if self.has_csrftoken:
            ...
            try:
                ...
                raise AuthException("Problem occured when reloading page. Maybe some WAF/Protection is blocking us ?")

            self.parameters = self.__extract_form_fields(target_form)

            if self.password_field not in self.parameters.keys() or (
                self.username_field and self.username_field not in self.parameters.keys()
            ):
                raise AuthException("Problem occured when reloading page. Maybe some WAF/Protection is blocking us ?")
        ...
        if self.method == "GET":
            r = Requester.get(self.action_url, params=self.parameters, cookies=self.cookies)

        else:
            r = Requester.post(self.action_url, data=self.parameters, cookies=self.cookies)
