# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_langextract.py\tests.py\init_test_3c07bdbe742d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-langextract\tests\init_test.py
...
from langextract.core import schema, types
...
class InitTest(parameterized.TestCase):
    ...
    def test_lang_extract_as_lx_extract(self, mock_create_model, mock_gemini_schema):
        ...
        mock_prompt_template = prompting.PromptTemplateStructured(description=mock_description, examples=mock_examples)
        ...
        prompt_generator = prompting.QAPromptGenerator(template=mock_prompt_template, format_handler=format_handler)
    ...
    @mock.patch("langextract.extraction.factory.create_model")
    def test_extract_resolver_params_docs_path_passthrough(self, mock_create_model, mock_annotate_docs):
        mock_model = mock.MagicMock()
    ...
    @mock.patch("langextract.extraction.resolver.Resolver")
    @mock.patch("langextract.extraction.factory.create_model")
    def test_extract_resolver_params_none_threshold(self, mock_create_model, mock_resolver_cls, mock_annotate):
        mock_model = mock.MagicMock()
    ...
    def test_show_progress_controls_progress_bar(
        ...
        mock_progress_bar.__iter__ = mock.MagicMock(return_value=iter([mock.MagicMock()]))
    ...
    def test_schema_validation_warning_issued(self, mock_create_model):
        ...
        mock_model.infer.return_value = [[types.ScoredOutput(output='{"extractions": []}', score=1.0)]]
    ...
    def test_gemini_schema_deprecation_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertTrue(
                any(
                    issubclass(warning.category, FutureWarning) and "gemini_schema" in str(warning.message)
                    for warning in w
                ),
