# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\persistence.py\repository.py\behavior_history_raw_repository_40df3158910d.py
...
class BehaviorHistoryRawRepository(BaseRepository[BehaviorHistory]):
    ...
    async def get_by_user_id(
        ...
        try:
            results = (
                await self.model.find({"user_id": user_id}, session=session).sort("-timestamp").limit(limit).to_list()
            )
    ...
    async def get_by_time_range(
        ...
        try:
            ...
            results = await self.model.find(query, session=session).sort("-timestamp").limit(limit).to_list()
    ...
    async def delete_by_user_and_timestamp(
        ...
        try:
            result = await self.model.find_one({"user_id": user_id, "timestamp": timestamp}, session=session)
            ...
            logger.error("❌ Failed to delete behavior history by user ID and timestamp: %s", e)
    ...
    async def count_by_user(self, user_id: str, session: Optional[AsyncClientSession] = None) -> int:
