# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\decodingai_magazine_second_brain_ai_assistant_course.py\apps.py\second_brain_offline.py\steps.py\etl.py\crawl_d08d74554ecf.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\decodingai-magazine-second-brain-ai-assistant-course\apps\second-brain-offline\steps\etl\crawl.py
...
@step
def crawl(documents: list[Document], max_workers: int = 10) -> Annotated[list[Document], "crawled_documents"]:
    ...
    logger.info(f"After crawling, we have {len(augmented_pages) - len(documents)} new documents.")
