# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Neton\NetonWeb\core\views.py
import json
import logging
import os
...
from .libs.execution_helpers import (
    check_module,
    get_execution_helper,
    get_executions_helper,
)
...
def collect_data(request):
    if request.method == "POST":
        try:
            ...
            log_info.info("collect_data -  IP: < " + str(request.META["REMOTE_ADDR"]) + ">")
            print(
                f"[{b64decode(received_json_data['executionId'])}] Source: {request.META['REMOTE_ADDR']} -> module {b64decode(received_json_data['module'])}"
            )
