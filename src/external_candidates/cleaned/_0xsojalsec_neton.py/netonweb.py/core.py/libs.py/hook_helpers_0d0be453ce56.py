# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Neton\NetonWeb\core\libs\hook_helpers.py
import logging
...
def get_hooks_helper(execution_id=None):
    ...
    try:
        ...
        for elem in hook_obj:
            ...
            # print(hook_l_aux)
            hook_l_aux = [
                {"hook": line[0].strip(), "status": line[1].strip()} for line in hook_l_aux[1:-1] if len(line) > 1
            ]
            hook_l.append(
                {
