# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\tests\tools\telephony\test_hangup_tool.py
"""
...
class TestHangupCallTool:
    ...
    async def test_hangup_multiple_times(self, hangup_tool, tool_context):
        ...
        # First hangup
        result1 = await hangup_tool.execute(parameters={}, context=tool_context)

        # Second hangup (edge case, shouldn't happen but handle gracefully)
        result2 = await hangup_tool.execute(parameters={}, context=tool_context)
