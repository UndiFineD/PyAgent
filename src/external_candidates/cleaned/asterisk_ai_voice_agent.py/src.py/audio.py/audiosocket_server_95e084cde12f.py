# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\audio\audiosocket_server.py
...
class AudioSocketServer:
        ...
        try:
            while True:
                ...
                if not handshake_complete:
                    ...
                    if not uuid_str:
                        logger.warning(
                            "Invalid UUID payload from AudioSocket client",
                            conn_id=conn_id,
                        )
    ...
    async def send_audio(self, conn_id: str, audio_payload: bytes) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to send audio over AudioSocket",
                conn_id=conn_id,
                error=str(exc),
                exc_info=True,
            )
    ...
    def _decode_uuid(payload: bytes) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                "Failed to decode UUID from payload",
                error=str(e),
                payload_len=len(payload),
            )
            return None
