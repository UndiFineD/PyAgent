...
class OpenAIRealtimeProvider(AIProviderInterface):
    ...
    def describe_alignment(
        ...
        *,
        ...
        if inbound_enc in ("ulaw", "mulaw", "g711_ulaw", "mu-law") and inbound_rate and inbound_rate != 8000:
            issues.append(f"OpenAI inbound μ-law sample rate is {inbound_rate} Hz; μ-law transport should be 8000 Hz.")
        ...
        if _class(target_enc) == _class(streaming_encoding):
            ...
            issues.append(f"OpenAI target_encoding={target_enc} but streaming manager emits {streaming_encoding}.")
    ...
    async def _ensure_response_request(self):
        ...
        response_payload: Dict[str, Any] = {
            ...
            "response": {
                ...
                "modalities": [m for m in (self.config.response_modalities or []) if m in ("audio", "text")]
                or ["audio"],
    ...
    def _start_farewell_timeout(self):
        ...
        logger.debug("⏱️  Farewell timeout started (5s fallback)", call_id=self._call_id)
    ...
    def _log_session_assumptions(self) -> None:
        ...
        info_payload = {
            ...
            "output_sample_rate_hz": str(
                int(self._active_output_sample_rate_hz or getattr(self.config, "output_sample_rate_hz", "") or 0)
            ),
    ...
    def _update_output_meter(self, chunk_bytes: int) -> None:
        ...
        try:
            assumed_now = float(
                self._active_output_sample_rate_hz or getattr(self.config, "output_sample_rate_hz", 0) or 0
            )
        except Exception:
