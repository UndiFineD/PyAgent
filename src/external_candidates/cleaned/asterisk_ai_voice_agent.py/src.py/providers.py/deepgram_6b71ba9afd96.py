...
class DeepgramProvider(AIProviderInterface):
    ...
    async def _configure_agent(self):
        ...
        listen_model = (
            self._get_config_value("model", None) or getattr(self.llm_config, "listen_model", None) or "nova-2-general"
        )
        speak_model = (
        ...
        think_prompt = (
            self._get_config_value("instructions", None)  # Context injection uses 'instructions' for Deepgram
            ...
            try:
                ...
                if encoding and sample_rate:
                    ...
                    if chunk_len == 160:
                        ...
                        try:
                            src_rate = int(self._get_config_value("input_sample_rate_hz", 0) or 0) or (
                                16000 if actual_format == "pcm16" else 8000
                            )
                        except Exception:
                            src_rate = 8000
                        ...
                        if et == "SettingsApplied":
                            ...
                            try:
                                if self._settings_ts:
                                    latency_ms = max(
                                        0.0,
                                        (time.monotonic() - float(self._settings_ts)) * 1000.0,
                                    )
                                    if self.call_id:
