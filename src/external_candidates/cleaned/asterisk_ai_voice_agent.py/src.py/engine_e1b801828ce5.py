...
class Engine:
    ...
    def __init__(self, config: AppConfig):
        ...
        if hasattr(config, "streaming") and config.streaming:
            ...
            streaming_config = {
                ...
                "normalizer": {
                    "enabled": (
                        bool(getattr(getattr(config, "streaming", {}), "normalizer", {}).get("enabled", True))
                        if hasattr(config, "streaming")
                        else True
                    ),
                    "target_rms": (
                        int(getattr(getattr(config, "streaming", {}), "normalizer", {}).get("target_rms", 1400))
                        if hasattr(config, "streaming")
                        else 1400
                    ),
                    "max_gain_db": (
                        float(getattr(getattr(config, "streaming", {}), "normalizer", {}).get("max_gain_db", 9.0))
                        if hasattr(config, "streaming")
                        else 9.0
                    ),
                },
                ...
                try:
                    for prov_name, provider in self.providers.items():
                        if hasattr(provider, "config"):
                            ...
                            provider_input_rate = getattr(cfg, "provider_input_sample_rate_hz", None) or getattr(
                                cfg, "input_sample_rate_hz", None
                            )
    ...
    async def _handle_caller_stasis_start_hybrid(self, caller_channel_id: str, channel: dict):
        ...
        try:
            ...
            resolved_provider = (
                provider_aliases.get(ai_provider_value, ai_provider_value) if ai_provider_value else None
            )
            ...
            if resolved_provider and resolved_provider in self.providers:
                ...
                pipeline_resolution = await self._assign_pipeline_to_session(session, pipeline_name=ai_provider_value)
    ...
    async def _handle_agent_action_stasis(self, channel_id: str, channel: dict, args: list):
        ...
        """
        if len(args) < 2:
            logger.error("🔀 AGENT ACTION - Insufficient args", channel_id=channel_id, args=args)
        ...
        if provider:
            try:
                ...
                if hasattr(provider, "stop_session"):
                    ...
                    logger.info("✅ AI provider session stopped", provider=session.provider_name)
            except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if call_id in _call_start_times:
                    ...
                    _CALL_DURATION.labels(call_id=call_id, pipeline=pipeline_name, provider=provider_name).observe(
                        duration
                    )
    ...
    def _audit_transport_alignment(self) -> None:
        ...
        try:
            ...
            streaming_target_fmt = (
                getattr(self.streaming_playback_manager, "audiosocket_format", None) or as_fmt
            ).lower()
            ...
            if expected_rate and streaming_rate != expected_rate:
                logger.warning(
                    ...
                    suggestion=(
                        "Set streaming.sample_rate to %d or change audiosocket.format to match" % expected_rate
                    ),
                )
        ...
        try:
            ...
            if etype == "AgentAudio":
                ...
                try:
                    ...
                    diag_rate = int(
                        fmt_entry.get("sample_rate") or sample_rate_int or session.transport_profile.sample_rate
                    )
                ...
                if coalesce_enabled and q is None and not isinstance(out_chunk, list):
                    ...
                    try:
                        ...
                        buf_ms = round(
                            (len(buf) / float(max(1, bps * max(1, wire_rate)))) * 1000.0,
                            3,
                        )
                    except Exception:
                        buf_ms = 0.0
                    ...
                    if buf_ms < coalesce_min_ms:
                        ...
                        try:
                            self._provider_bytes[call_id] = int(self._provider_bytes.get(call_id, 0)) + (
                                len(chunk) if isinstance(chunk, (bytes, bytearray)) else len(out_chunk)
                            )
                        except Exception:
                            pass
                    ...
                    try:
                        ...
                        self._provider_bytes[call_id] = int(self._provider_bytes.get(call_id, 0)) + (
                            ...
                            else sum(len(f) for f in (out_chunk if isinstance(out_chunk, list) else [out_chunk]))
                        )
        ...
        try:
            ...
            if greeting:
                try:
                    ...
                    greeting = greeting.format(caller_name=caller_name, caller_number=caller_number)
                    logger.debug(
            ...
            base_commit_ms = 160
            stt_chunk_ms = (
                int(pipeline.stt_options.get("chunk_ms", base_commit_ms)) if pipeline.stt_options else base_commit_ms
            )
            ...
            if use_streaming:
                try:
                    logger.info(
                        ...
                        buffer_max=(
                            getattr(buffer_queue, "_maxsize", 200) if hasattr(buffer_queue, "_maxsize") else 200
                        ),
                    )
                except Exception:
            ...
            async def dialog_worker() -> None:
                ...
                accumulation_timeout = float((pipeline.llm_options or {}).get("aggregation_timeout_sec", 2.0))
    ...
    async def _update_transport_profile(
        ...
        *,
        ...
        changed = (
            profile.format != canonical_fmt or profile.sample_rate != final_rate or profile.source != incoming_source
        )
    ...
    def _resolve_stream_targets(
        ...
        try:
            ...
            if provider_cfg:
                ...
                raw_input_rate = getattr(provider_cfg, "provider_input_sample_rate_hz", None) or getattr(
                    provider_cfg, "input_sample_rate_hz", None
                )
    ...
    async def _health_handler(self, request):
        ...
        try:
            ...
            audiosocket_listening = (
                self.audio_socket_server is not None if self.config.audio_transport == "audiosocket" else True
            )
