...
class LocalSTTAdapter(_LocalAdapterBase, STTComponent):
    ...
    async def start_stream(
        ...
        if session.receiver_task and not session.receiver_task.done():
            return
        session.receiver_task = asyncio.create_task(self._stream_receive_loop(session, runtime_options))
        logger.debug(
    ...
    async def send_audio(
        ...
        *,
        ...
        if not session or session.send_lock is None:
            raise RuntimeError(f"Streaming session not started for call {call_id}; call start_stream first")
