# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\business\email_summary.py
"""
...
class SendEmailSummaryTool(Tool):
    ...
    async def execute(self, parameters: Dict[str, Any], context: ToolExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not config.get("enabled", False):
                logger.info("Email summary tool disabled, skipping send", call_id=call_id)
                return {
                    "status": "skipped",
    ...
    def _prepare_email_data(self, session: Any, config: Dict[str, Any], call_id: str) -> Dict[str, Any]:
