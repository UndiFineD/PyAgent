# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\telephony\hangup.py
"""
...
class HangupCallTool(Tool):
    ...
    async def execute(self, parameters: Dict[str, Any], context: ToolExecutionContext) -> Dict[str, Any]:
        ...
        if not farewell:
            ...
            farewell = context.get_config_value("tools.hangup_call.farewell_message", "Thank you for calling. Goodbye!")

        logger.info("📞 Hangup requested", call_id=context.call_id, farewell=farewell)
