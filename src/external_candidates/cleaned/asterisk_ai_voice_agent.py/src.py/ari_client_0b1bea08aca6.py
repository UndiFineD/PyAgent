# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\ari_client.py
"""
...
class ARIClient:
    ...
    async def hangup_channel(self, channel_id: str):
        ...
        if response and response.get("status") == 404:
            logger.debug(
                "Channel hangup failed (404), likely already hung up.",
    ...
    async def execute_application(self, channel_id: str, app_name: str, app_data: str) -> bool:
        ...
        try:
            logger.info(
                "Executing application on channel",
                channel_id=channel_id,
                app_name=app_name,
                app_data=app_data,
            )
    ...
    async def stop_playback(self, playback_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                "Playback stop response without status; assuming success",
                playback_id=playback_id,
            )
            return True
        except Exception:
    ...
    async def add_channel_to_bridge(self, bridge_id: str, channel_id: str) -> bool:
        ...
        try:
            response = await self.send_command("POST", f"bridges/{bridge_id}/addChannel", data={"channel": channel_id})
    ...
    async def play_audio_response(self, channel_id: str, audio_data: bytes):
        ...
        logger.info(
            "Starting audio playback process",
            channel_id=channel_id,
        ...
        try:
            ...
            logger.debug(
                "Writing ulaw audio file to ai-generated subdirectory",
                path=container_path,
            )
            ...
            logger.debug(
                "Ulaw audio file written (generated as ulaw at 8000 Hz)",
                path=container_path,
            )
            ...
            try:
                ...
                logger.debug(
                    "Changed file ownership to asterisk user",
                    path=container_path,
                    uid=asterisk_uid,
                    gid=asterisk_gid,
                )
            ...
            if playback and "id" in playback:
                ...
                logger.error(
                    "Failed to initiate audio playback",
                    channel_id=channel_id,
    ...
    async def play_audio_file(self, channel_id: str, file_path: str) -> bool:
        ...
        try:
            ...
            for attempt in range(15):  # Try up to 15 times (1.5 seconds total)
                if os.path.exists(file_path):
                    if os.access(file_path, os.R_OK):
                        ...
                        logger.warning(
                            f"File exists but not readable: {file_path} (permissions: {oct(os.stat(file_path).st_mode)[-3:]}) - attempt {attempt + 1}"
                        )
                else:
            ...
            if not os.access(file_path, os.R_OK):
                logger.error(
                    f"Audio file not readable: {file_path} (permissions: {oct(os.stat(file_path).st_mode)[-3:]})"
                )
                return False
            ...
            if result:
                logger.info(
                    f"Playing audio file {file_path} ({file_size} bytes) on channel {channel_id} - took {elapsed_time:.1f}ms"
                )
                return True
            else:
    ...
    async def stop_audio_streaming(self, channel_id: str) -> bool:
        ...
        if not media_info:
            logger.warning(
                "No active media channel found to stop for channel.",
                channel_id=channel_id,
            )
        ...
        logger.info(
            "Stopping audio streaming and cleaning up resources...",
            channel_id=channel_id,
        ...
        try:
            ...
            logger.info(
                "Successfully cleaned up bridge and media channel.",
                bridge_id=bridge_id,
                media_channel_id=media_channel_id,
            )
            return True
    ...
    async def is_channel_active(self, channel_id: str) -> bool:
        ...
        try:
            ...
            if result and result.get("id") == channel_id:
                ...
                logger.debug("Channel not found in ARI", channel_id=channel_id, result=result)
                return False
        except Exception as e:
            logger.debug("Error checking channel status", channel_id=channel_id, error=str(e))
            return False
    ...
    async def validate_channel_for_playback(self, channel_id: str) -> bool:
        ...
        try:
            ...
            if not await self.is_channel_active(channel_id):
                logger.warning(
                    "Channel validation failed: channel not active",
                    channel_id=channel_id,
                )
                return False
            ...
            if not result:
                logger.warning(
                    "Channel validation failed: cannot retrieve channel info",
                    channel_id=channel_id,
                )
                return False
            ...
            if state not in ["Up"]:
                logger.warning(
                    "Channel validation failed: not in correct state for playback",
                    channel_id=channel_id,
                    state=state,
                )
                return False

            logger.debug("Channel validation successful", channel_id=channel_id, state=state)
            return True

        except Exception as e:
            logger.warning(
                "Channel validation failed: exception occurred",
                channel_id=channel_id,
                error=str(e),
            )
            return False
    ...
    async def play_audio_via_bridge(self, bridge_id: str, media_uri: str) -> Optional[str]:
        ...
        try:
            ...
            if response and response.get("id"):
                ...
                logger.error(
                    "Failed to start bridge playback",
                    bridge_id=bridge_id,
                    media_uri=media_uri,
                    response=response,
                )
    ...
    async def play_media_on_bridge_with_id(self, bridge_id: str, media_uri: str, playback_id: str) -> bool:
        ...
        try:
            ...
            if response and response.get("id") == playback_id:
                logger.info(
                    "Bridge playback started with deterministic ID",
                    bridge_id=bridge_id,
                    media_uri=media_uri,
                    playback_id=playback_id,
                )
                ...
                logger.error(
                    "Failed to start bridge playback with deterministic ID",
                    bridge_id=bridge_id,
                    media_uri=media_uri,
                    playback_id=playback_id,
                    response=response,
    ...
    async def remove_channel_from_bridge(self, bridge_id: str, channel_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                "Channel remove-from-bridge response without status; assuming success",
                bridge_id=bridge_id,
                channel_id=channel_id,
                response=response,
            )
    ...
    async def destroy_bridge(self, bridge_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                "Bridge destroy response without status; assuming success",
                bridge_id=bridge_id,
                response=response,
            )
            return True
