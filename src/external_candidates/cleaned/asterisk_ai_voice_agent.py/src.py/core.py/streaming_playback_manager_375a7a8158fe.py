...
class StreamingPlaybackManager:
    ...
    def __init__(
        ...
        try:
            self.diag_out_dir = str(
                self.streaming_config.get("diag_out_dir", "/tmp/ai-engine-taps") or "/tmp/ai-engine-taps"
            )
        except Exception:
    ...
    async def start_streaming_playback(
        ...
        try:
            ...
            if not session:
                logger.error("Cannot start streaming - call session not found", call_id=call_id)
                return None
            ...
            base_config_ms = max(
                1,
                int(self.greeting_min_start_ms if playback_type == "greeting" else self.min_start_ms),
            )
            ...
            try:
                logger.info(
                    ...
                    provider_grace_ms=(
                        int(self.provider_grace_ms) if getattr(self, "provider_grace_ms", None) is not None else 0
                    ),
                )
            except Exception:
                pass
            ...
            pacer_task = asyncio.create_task(self._pacer_loop(call_id, stream_id, jitter_buffer))
            # Start keepalive task
            keepalive_task = asyncio.create_task(self._keepalive_loop(call_id, stream_id))
            ...
            try:
                resolved_target_rate = (
                    int(target_sample_rate) if target_sample_rate is not None else int(self.sample_rate)
                )
            except Exception:
                        ...
                        if not characterizer.characterization_done:
                            ...
                            if characterizer.should_analyze():
                                ...
                                if pattern and call_id in self.adaptive_controllers:
                                    ...
                                    sess = await self.session_store.get_by_call_id(call_id)
                                    if sess:
                                        provider_name = getattr(sess, "provider_name", None) or getattr(
                                            sess, "provider", "unknown"
                                        )
                                        wire_rate = 8000
                                        if hasattr(sess, "transport_profile") and hasattr(
    ...
    async def _drain_next_frame(
        ...
        if stream_info.get("startup_ready") and not sentinel_seen and jitter_buffer.empty():
    ...
    async def _process_audio_chunk(self, call_id: str, chunk: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            if self._is_mulaw(src_encoding_raw) and self._is_mulaw(target_fmt) and src_rate == target_rate:
                self._resample_states[call_id] = None
                    ...
                    if not stream_info.get("src_endian_probe_done", False):
                        ...
                        if swapped is not None:
                            if rms_swapped >= max(1024, 4 * max(1, rms_native)):
                                ...
                                try:
                                    if abs(avg_native) >= 8 * max(1, abs(avg_swapped)) and rms_swapped >= max(
                                        256, rms_native // 2
                                    ):
                                        prefer_swapped = True
                                except Exception:
    ...
    def _trim_leading_silence(self, pcm_bytes: bytes, threshold_rms: int = 100) -> bytes:
        ...
        try:
            ...
            logger.warning("ENTIRE CHUNK SILENT", chunk_size=len(buf), threshold=threshold_rms)
            return pcm_bytes
        ...
        try:
            ...
            if self.audio_diag_callback:
                try:
                    ...
                    try:
                        effective_rate = int(target_rate or stream_info.get("target_sample_rate") or self.sample_rate)
                    except Exception:
            ...
            if self.audio_transport == "audiosocket":
                ...
                try:
                    if (
                        ...
                        and not self.active_streams[call_id].get("first_frame_observed", False)
                        and success
                    ):
    ...
    async def _fallback_to_file_playback(self, call_id: str, stream_id: str) -> None:
        ...
        try:
            ...
            if remaining_audio:
                ...
                try:
                    info = self.active_streams.get(call_id, {})
                    src_encoding = self._canonicalize_encoding(info.get("source_encoding")) or "slin16"
                    try:
    ...
    async def _keepalive_loop(self, call_id: str, stream_id: str) -> None:
        ...
        try:
            ...
            logger.debug("Keepalive loop cancelled", call_id=call_id, stream_id=stream_id)
        except Exception as e:
            logger.error(
    ...
    async def _cleanup_stream(self, call_id: str, stream_id: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if rem:
                    ...
                    if self.audio_transport == "audiosocket":
                        ...
                        bytes_per_sample = 1 if self._is_mulaw(fmt) else 2
                        frame_size = int(sr * (self.chunk_size_ms / 1000.0) * bytes_per_sample) or (
                            160 if bytes_per_sample == 1 else 320
                        )
            ...
            if self.conversation_coordinator:
                await self.conversation_coordinator.on_tts_end(call_id, stream_id, "streaming-ended")
            ...
            try:
                if call_id in self.active_streams:
                    ...
                    try:
                        drift_pct = (
                            0.0 if wall_seconds <= 0.0 else ((eff_seconds - wall_seconds) / wall_seconds) * 100.0
                        )
                    except Exception:
                        drift_pct = 0.0
                    logger.info(
            ...
            logger.debug("Streaming cleanup completed", call_id=call_id, stream_id=stream_id)
