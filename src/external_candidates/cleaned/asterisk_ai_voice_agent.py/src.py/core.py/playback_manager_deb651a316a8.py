...
class PlaybackManager:
    ...
    async def play_audio(self, call_id: str, audio_bytes: bytes, playback_type: str = "response") -> Optional[str]:
        """
        ...
        try:
            ...
            if not session:
                logger.error("Cannot play audio - call session not found", call_id=call_id)
                return None
    ...
    async def on_playback_finished(self, playback_id: str) -> bool:
        ...
        try:
            ...
            if self.conversation_coordinator:
                ...
                success = await self.session_store.clear_gating_token(playback_ref.call_id, playback_id)
    ...
    async def _create_audio_file(self, audio_bytes: bytes, playback_id: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                chowned = True
                logger.debug("Audio file ownership set for Asterisk", file_path=file_path)
            except OSError as e:
                logger.warning(
            ...
            logger.debug("Audio file created", file_path=file_path, size=len(audio_bytes))
    ...
    async def _play_via_ari(self, session: CallSession, audio_file: str, playback_id: str) -> bool:
    ...
    async def _cleanup_audio_file(self, audio_file: str) -> None:
        ...
        try:
            ...
            logger.warning("Error cleaning up audio file", file_path=audio_file, error=str(e))
