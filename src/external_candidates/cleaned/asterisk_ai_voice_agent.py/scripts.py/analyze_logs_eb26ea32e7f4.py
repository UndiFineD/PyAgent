...
class LogAnalyzer:
    ...
    def _analyze_audio_flow(self):
        ...
        if audio_events:
            ...
            tts_generated = any(
                "tts" in event["event"].lower() or "playback" in event["event"].lower() for event in audio_events
            )
...
"""
    ...
    def _generate_phase_analysis(self, phases):
        ...
        for phase_name, phase_data in phases.items():
            ...
            status_icon = (
                "✅" if phase_data["status"] == "success" else "❌" if phase_data["status"] == "failure" else "⚠️"
            )
            status_text = (
                "SUCCESS"
                if phase_data["status"] == "success"
                else "FAILURE"
                if phase_data["status"] == "failure"
                else "UNKNOWN"
            )
    ...
    def _generate_framework_summary(self, phases):
        ...
        for phase_name, phase_data in phases.items():
            if phase_data["events"]:
                ...
                summary += (
                    f"| {phase_names[phase_name]} | {status_icon} {phase_data['status'].title()} | {issue_text} |\n"
                )
