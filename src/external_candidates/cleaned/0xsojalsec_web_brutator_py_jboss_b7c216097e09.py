# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_web_brutator.py\lib.py\modules.py\jboss_b7c216097e09.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-web-brutator\lib\modules\Jboss.py
...
class Jboss:
    ...
    def check(self):
        ...
        if r.status_code == 200:
            ...
            logger.info("Jboss authentication interface detected: {}".format(self.interface_url))
        ...
        if auth_type is not AuthMode.UNKNOWN:
            ...
            logger.info("Jboss jmx-console interface detected: {}".format(self.interface_url))
        ...
        if auth_type is not AuthMode.UNKNOWN:
            ...
            logger.info("Jboss web-console interface detected: {}".format(self.interface_url))
        ...
        if auth_type is not AuthMode.UNKNOWN:
            ...
            logger.info("Jboss management interface detected: {}".format(self.interface_url))
    ...
    def try_auth(self, username, password):
        ...
        if self.interface == "admin-console":
            ...
            m = re.search(
                ...
                'id="javax\.faces\.ViewState" value="(?P<viewstate>.*?)"',
                r.text,
            )
            ...
            if not m:
                raise RequestException("Unable to retrieve JSESSIONID value from {}".format(self.interface_url))
            ...
            status = 'name="login_form:password"' not in r.text and "Not logged in" not in r.text
