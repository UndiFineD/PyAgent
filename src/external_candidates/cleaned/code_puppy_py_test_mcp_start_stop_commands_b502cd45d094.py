# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\mcp.py\test_mcp_start_stop_commands_b502cd45d094.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\mcp\test_mcp_start_stop_commands.py
...
class TestStartCommand:
    ...
    def test_execute_server_not_found(self):
        ...
        with patch("code_puppy.command_line.mcp.start_command.find_server_id_by_name") as mock_find:
            mock_find.return_value = None

            with patch("code_puppy.command_line.mcp.start_command.suggest_similar_servers") as mock_suggest:
                with patch(
    ...
    def test_execute_start_success(self, mock_emit_info, mock_get_current_agent, mock_mcp_manager):
    ...
    def test_execute_start_failure(self, mock_emit_info, mock_get_current_agent, mock_mcp_manager):
    ...
    def test_execute_with_agent_reload_exception(self, mock_emit_info, mock_mcp_manager):
...
class TestStopCommand:
    ...
    def test_execute_server_not_found(self, mock_emit_info):
        ...
        with patch("code_puppy.command_line.mcp.stop_command.find_server_id_by_name") as mock_find:
            mock_find.return_value = None

            with patch("code_puppy.command_line.mcp.stop_command.suggest_similar_servers") as mock_suggest:
                self.command.execute(["nonexistent"])
    ...
    def test_execute_stop_success(self, mock_emit_info, mock_get_current_agent, mock_mcp_manager):
...
class TestRestartCommand:
    ...
    def test_execute_server_not_found(self, mock_emit_info):
        ...
        with patch("code_puppy.command_line.mcp.restart_command.find_server_id_by_name") as mock_find:
            mock_find.return_value = None

            with patch("code_puppy.command_line.mcp.restart_command.suggest_similar_servers") as mock_suggest:
                self.command.execute(["nonexistent"])
    ...
    def test_execute_restart_start_failure_after_reload(self, mock_emit_info, mock_mcp_manager):
