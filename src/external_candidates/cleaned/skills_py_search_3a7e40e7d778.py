# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\doanbactam.py\agnxi_search_skill.py\search_3a7e40e7d778.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\doanbactam\agnxi-search-skill\search.py
...
def parse_sitemap(xml_content, query):
    ...
    try:
        ...
        print(
            f"Debug: XML parse warning ({e}). Falling back to simple text search.",
            file=sys.stderr,
        )

        return [
            line
            for line in xml_content.decode("utf-8").splitlines()
            if query.lower() in line.lower() and "<loc>" in line
        ]
