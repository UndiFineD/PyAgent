# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\visualizer_service.py
...
def create_visualization(
    ...
    if visualization_type == "bar":
        ...
        ax = sns.heatmap(
            df.pivot(x_column, y_column, values=df.columns[-1]),
            annot=True,
            cmap="YlGnBu",
        )
    else:
    ...
    if y_column_provided:
        description = (
            f"{visualization_type.capitalize()} chart visualization of {x_column} vs {y_column} for the query: {query}"
        )
    else:
