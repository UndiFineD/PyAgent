# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\graph_service.py
...
from app.services.sql_executor_service import (
    execute_sql,
    execute_sql_corrected,
    execute_sql_reflection,
)
from app.services.sql_generator_service import (
    sql_generator_optimizer,
    sql_generator_optimizer_reflection,
)
...
def should_regenerate_sql(state: AgentState) -> str:
    if (state["validation_result"] is None or not state["validation_result"].is_sql_valid) and len(
        state.get("regenerate_list", [])
    ) < 3:
...
def should_visualize(state: AgentState) -> str:
    ...
    if (
        evaluation_result
        and isinstance(evaluation_result, EvaluationResult)
        and evaluation_result.requires_visualization
    ):
        return "visualizer"
    return "summarizer"
...
def create_analysis_graph(memory_service) -> StateGraph:
    ...
    graph.add_conditional_edges(
        "analyzer",
        is_query_relevant,
        {"generator": "generator", "summarizer": "summarizer"},
    )
...
def save_graph_visualization(graph: StateGraph):
    try:
        ...
        response = requests.post("https://mermaid.ink/img", json={"mermaid": mermaid_string})
