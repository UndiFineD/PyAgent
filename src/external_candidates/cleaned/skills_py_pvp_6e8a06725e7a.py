# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\synthpolis.py\clawdcasino.py\script.py\pvp_6e8a06725e7a.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\synthpolis\clawdcasino\script\pvp.py
...
def request_bet(statement: str, stake: float, deadline: Optional[str] = None):
    ...
    if "http" not in statement.lower():
        ...
        print("Example: 'BTC above $100k on Feb 1 per https://coinmarketcap.com/currencies/bitcoin/'")
...
def list_open_bet():
    ...
    if result.get("success"):
        ...
        for b in bets:
            proposer = b.get("proposer", {}).get("name", b["proposer_wallet"][:10] + "...")
...
def list_quotes(bet_id: str):
    ...
    if result.get("success"):
        ...
        for q in quotes:
            ...
            print(f"    Their stake: ${q['quoter_stake']} USDC | Your odds: {odds:.2f}x")
...
Examples:
  ...
  /pvp stats
    ...
    approve_parser = subparsers.add_parser("approve", help="Approve USDC for PvP contract (gasless)")

    approve_parser.add_argument("--amount", type=float, help="Amount to approve (default: 1M USDC)")

    # request - Create a bet request

    request_parser = subparsers.add_parser("request", help="Create a bet request")

    request_parser.add_argument("statement", help="What you're betting on (must include URL)")

    request_parser.add_argument("--stake", type=float, required=True, help="Your stake in USDC")

    request_parser.add_argument("--deadline", help="Resolution deadline (ISO format, default: 7 days)")
    ...
    quote_parser = subparsers.add_parser("quote", help="Submit a quote on a bet request")

    quote_parser.add_argument("bet_id", help="Bet ID to quote on")

    quote_parser.add_argument("--stake", type=float, required=True, help="Your stake in USDC")

    quote_parser.add_argument("--ttl", type=int, default=5, help="Quote validity in minutes (default: 5)")
