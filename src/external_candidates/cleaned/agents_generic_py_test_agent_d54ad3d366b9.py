# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agents_generic.py\examples.py\drive_thru.py\test_agent_d54ad3d366b9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\drive-thru\test_agent.py
...
async def test_item_ordering() -> None:
    ...
    async with (
        ...
        result.expect.next_event().is_function_call(name="order_regular_item", arguments={"item_id": "big_mac"})
...
async def test_meal_order() -> None:
    ...
    async with (
        ...
        result = await sess.run(user_input="Can I get a large Combo McCrispy Original with mayonnaise?")
...
async def test_failure() -> None:
    ...
    async with (
        ...
        with mock_tools(DriveThruAgent, {"order_regular_item": lambda: RuntimeError("test failure")}):
            await sess.start(DriveThruAgent(userdata=userdata))
            ...
            result.expect.next_event().is_function_call(
                ...
                arguments={"item_id": "shake_vanilla", "size": "L"},
            )
...
async def test_unavailable_item() -> None:
    ...
    async with (
        ...
        try:
            ...
            result.expect.next_event().is_function_call(
                name="order_regular_item",
                arguments={"item_id": "coca_cola", "size": "L"},
            )
...
async def test_ask_for_size() -> None:
    ...
    async with (
        ...
        await result.expect.next_event().is_message(role="assistant").judge(llm, intent="should ask for the drink size")
        ...
        result.expect.next_event().is_function_call(
            name="order_regular_item",
            arguments={"item_id": "fanta_orange", "size": "S"},
        )
...
async def test_consecutive_order() -> None:
    ...
    async with _llm_model() as llm, AgentSession(llm=llm, userdata=userdata) as sess:
        ...
        result.expect.contains_function_call(name="order_regular_item", arguments={"item_id": "ketchup"})

        result.expect.contains_function_call(name="order_regular_item", arguments={"item_id": "sweet_mcflurry_oreo"})
