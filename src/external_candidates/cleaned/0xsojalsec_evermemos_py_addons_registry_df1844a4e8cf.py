# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\addons.py\addons_registry_df1844a4e8cf.py
...
class AddonsRegistry:
    ...
    def _should_load_entrypoint(self, entrypoint_name: str) -> bool:
        ...
        allowed_entrypoints: Set[str] = {name.strip() for name in filter_config.split(",") if name.strip()}
        ...
        try:
            ...
            try:
                ...
                if hasattr(eps, "select"):
                    ...
                    addon_eps = eps.get("memsys.addons", []) if isinstance(eps, dict) else []
            ...
            logger.info("✅ Addons entry points loading completed, total %d", self.count())

        except ImportError:
            logger.warning("⚠️  importlib.metadata is not available, skipping entry points loading")
