# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\evoagentx.py\examples.py\action_agent_60fa1681c377.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\EvoAgentX\examples\action_agent.py
...
from evoagentx.core.registry import ACTION_FUNCTION_REGISTRY, register_action_function
...
def demo_save_load_functionality():
    ...
    math_agent = ActionAgent(
        ...
        outputs=[{"name": "result", "type": "int", "description": "Result", "required": True}],
        execute_func=add_numbers,
    )
...
def demo_input_validation():
    ...
    agent = ActionAgent(
        ...
        outputs=[{"name": "result", "type": "int", "description": "Sum", "required": True}],
        execute_func=add_numbers,
    )
        ...
        inputs=[
            {
                "name": "input_data",
                "type": "any",
                "description": "Input data that may cause errors",
                "required": True,
            }
        ],
        outputs=[
