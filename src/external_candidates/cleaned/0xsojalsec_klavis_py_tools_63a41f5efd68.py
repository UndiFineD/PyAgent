# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_klavis.py\open_strata.py\src.py\strata.py\tools_63a41f5efd68.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-klavis\open-strata\src\strata\tools.py
...
async def execute_tool(name: str, arguments: dict, client_manager: MCPClientManager) -> List[types.ContentBlock]:
    ...
    try:
        ...
        return [
            types.TextContent(
                type="text",
                text=(json.dumps(result, separators=(",", ":")) if isinstance(result, (dict, list)) else str(result)),
            )
        ]
