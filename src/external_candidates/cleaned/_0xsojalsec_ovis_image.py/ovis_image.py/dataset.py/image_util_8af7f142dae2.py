...
def process_pil_img_to_tensor(
    ...
    pil_crop_img = torchvision.transforms.functional.center_crop(pil_resize_img, (output_height, output_width))
    image_tensor = torchvision.transforms.functional.to_tensor(pil_crop_img)
    image_tensor = torchvision.transforms.functional.normalize(image_tensor, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
