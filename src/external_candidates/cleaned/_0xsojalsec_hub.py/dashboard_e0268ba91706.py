    ...
    if cam_number == None:
        ...
        if indicator == "Safety Score":
            for cam in cam_number:
                ...
                for date in columns:
                    number_of_violations = df_int[df_int["Date"] == date].count().Timings
            ...
            df1 = pd.DataFrame(score_list).T.reset_index().rename(columns={"index": "Source"})
            df1 = df1.rename_axis("Date").melt(id_vars=["Source"], var_name="Date", value_name="Score")
            bar_chart = px.bar(
                df1,
                x="Date",
                y="Score",
            ...
            df_violations = df1.groupby(["Camera_no", "Date"]).agg({"Timings": "count"}).reset_index()
            df_violations.rename(columns={"Timings": "Count", "Camera_no": "Source"}, inplace=True)
            bar_chart = px.bar(
                df_violations,
                x="Date",
                y="Count",
...
date_dropdown = html.Div(
    [
        ...
        dcc.Dropdown(
            id="dates",
            options=[{"label": name, "value": name} for name in sorted(updateDates(None))],
            clearable=False,
        ),
    ],
    className="mb-4",
