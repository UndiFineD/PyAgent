# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\plugins\safety_transform_plugin.py
import os
...
class SafetyTransformPlugin(BasePlugin):
        ...
        try:
            ...
            if self.inpaint_model is None:
                ...
                self.inpaint_model = StableDiffusionInpaintPipeline.from_pretrained(
                    "stabilityai/stable-diffusion-2-inpainting", safety_checker=None
                ).to(self.device)
    ...
    def generate_mask(self, image, bbox):
        ...
        try:
            ...
            cv2.rectangle(mask_viz, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
