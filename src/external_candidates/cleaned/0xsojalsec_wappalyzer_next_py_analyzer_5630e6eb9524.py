# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_wappalyzer_next.py\wappalyzer.py\browser.py\analyzer_5630e6eb9524.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wappalyzer-next\wappalyzer\browser\analyzer.py
    ...
    try:
        ...
        for i in range(5):
            ...
            driver.execute_script(
                "var event = new MouseEvent('mousemove', { 'view': window, 'bubbles': true, 'cancelable': true, 'clientX': Math.random() * window.innerWidth, 'clientY': Math.random() * window.innerHeight }); document.dispatchEvent(event);"
            )
...
def merge_technologies(detections):
    ...
    for detection in detections:
        ...
        if tech_name not in tech_map:
            ...
            existing["confidence"] = min(existing["confidence"] + detection["pattern"]["confidence"], 100)
