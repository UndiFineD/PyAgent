# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\agentcloud.py\agent_backend.py\src.py\tools.py\retrievers.py\filters_dd89b466e370.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\tools\retrievers\filters.py
...
def create_pinecone_filters(rag_filters: Dict) -> Dict:
    ...
    if should_conditions:
        ...
        if combined_filter:
            combined_filter = {"$and": [combined_filter, {"$or": should_clauses}]}
...
def create_qdrant_filters(rag_filters: Dict) -> models.Filter:
    ...
    def construct_field_condition(condition: Dict) -> models.FieldCondition:
        ...
        # Check for 'except' (match except condition)
            ...
            return models.FieldCondition(key=key, match=models.MatchExcept(**{"except": match["except"]}))
        ...
        if "range" in condition:
            ...
            is_datetime_range = any(
                value is not None and (isinstance(value, str) and (value.endswith("Z") or "T" in value))
                for value in [
                    range_.get("gt"),
                    range_.get("gte"),
                    range_.get("lt"),
