# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\code_puppy.py\tests.py\plugins.py\test_antigravity_model_mapping_819e08302f27.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\code_puppy\tests\plugins\test_antigravity_model_mapping.py
...
class TestMessageMapping:
    ...
    async def test_map_empty_messages(self, mock_google_model, model_request_params):
        ...
        system_instruction, contents = await mock_google_model._map_messages(messages, model_request_params)
        ...
        assert any("Antigravity" in part.get("text", "") for part in system_instruction["parts"])
    ...
    @pytest.mark.asyncio
    async def test_map_instructions_injected(self, mock_google_model, model_request_params):
        ...
        with patch.object(mock_google_model, "_get_instructions", return_value="Injected"):
            system_instruction, _contents = await mock_google_model._map_messages(messages, model_request_params)
    ...
    async def test_map_user_prompt(self, mock_google_model, model_request_params):
        ...
        with patch.object(mock_google_model, "_map_user_prompt") as mock_map:
            ...
            _system_instruction, contents = await mock_google_model._map_messages(messages, model_request_params)
    ...
    @pytest.mark.asyncio
    async def test_map_retry_prompt_without_tool(self, mock_google_model, model_request_params):
        """Test mapping retry prompt without tool name."""

        messages = [ModelRequest(parts=[RetryPromptPart(content="Please try again", tool_name=None)])]
    ...
    @pytest.mark.asyncio
    async def test_map_retry_prompt_with_tool(self, mock_google_model, model_request_params):
    ...
    @pytest.mark.asyncio
    async def test_map_model_response_with_text(self, mock_google_model, model_request_params):
    ...
    @pytest.mark.asyncio
    async def test_map_file_part_with_bytes(self, mock_google_model, model_request_params):
        """Test mapping file part with bytes data via user prompt."""

        image_data = b"fake_image_data"

        messages = [ModelRequest(parts=[UserPromptPart(content="Check this image", timestamp=datetime.now())])]
        ...
        assert part["inline_data"]["data"] == base64.b64encode(image_data).decode("utf-8")

    @pytest.mark.asyncio
    async def test_merge_consecutive_user_messages(self, mock_google_model, model_request_params):
    ...
    @pytest.mark.asyncio
    async def test_merge_consecutive_model_messages(self, mock_google_model, model_request_params):
