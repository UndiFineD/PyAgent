# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\infra_layer.py\adapters.py\out.py\search.py\repository.py\foresight_milvus_repository_969b1374b93f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\src\infra_layer\adapters\out\search\repository\foresight_milvus_repository.py
...
class ForesightMilvusRepository(BaseMilvusRepository[ForesightCollection]):
    ...
    async def vector_search(
        ...
        try:
            ...
            if participant_user_id:
                filter_expr.append(f'array_contains(participants, "{participant_user_id}")')
            ...
            if current_time:
                ...
                filter_expr.append(f"(start_time <= {current_ts} and end_time >= {current_ts})")
            ...
            logger.info(f"Milvus raw response: {raw_hit_count} results, limit={limit}, filter_str={filter_str}, ")
                ...
                for hit in hits:
                    if hit.score >= score_threshold:
                        ...
                        result = {
                            ...
                            "start_time": datetime.fromtimestamp(hit.entity.get("start_time", 0)),
                            "end_time": datetime.fromtimestamp(hit.entity.get("end_time", 0)),
            ...
            logger.debug("✅ Vector search succeeded: found %d results", len(search_results))
    ...
    async def delete_by_filters(
        ...
        try:
            ...
            if user_id is not None:  # Use is not None instead of truthy check, to support empty string
            ...
            logger.error("❌ Failed to batch delete foresight by filter conditions: %s", e)
