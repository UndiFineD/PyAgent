# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\src.py\core.py\context.py\context_manager_ead35e9b31a1.py
    ...
    async def run_with_session(
        ...
        **kwargs,
        ...
        if force_new_session:
            ...
            logger.debug("Forcing creation of a new database session (to avoid concurrency conflicts)")
        ...
        try:
            ...
            if auto_commit and need_cleanup and session.is_active:
                ...
                logger.debug("Database session manager: automatically committed transaction")
            ...
            if need_cleanup and session.is_active:
                try:
                    ...
                    logger.debug("Database session manager: automatically rolled back transaction")

                except Exception as rollback_error:
                    logger.error(f"Error occurred during transaction rollback: {str(rollback_error)}")
            ...
            if need_cleanup:
                try:
                    ...
                    logger.error(f"Error occurred when closing database session: {str(close_error)}")
    ...
    async def run_with_user_context(
        ...
        **kwargs,
        ...
        if actual_user_data is not None:
            ...
            logger.debug(f"User context manager: setting user context user_id={actual_user_data.get('user_id')}")
...
def with_user_context(user_data: Optional[Dict[str, Any]] = None, auto_inherit: bool = True):
    """
