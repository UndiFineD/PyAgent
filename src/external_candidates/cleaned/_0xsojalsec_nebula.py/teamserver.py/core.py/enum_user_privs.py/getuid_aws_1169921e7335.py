# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\enum_user_privs\getuid_aws.py
import json
import sys
...
def getuid(profile_dict, workspace):
    ...
    try:
        ...
        if identityType in ["user", "federated-user"]:
...
def checkrole(client, rolename, response):
    ...
    if not rolename == "":
        try:
            response = client.get_role(RoleName=rolename)
            del response["ResponseMetadata"]
        ...
        try:
            ...
            while response["IsTruncated"]:
                response.update(client.list_role_policies(RoleName=rolename, Marker=response["Marker"]))
        ...
        try:
            ...
            while response["IsTruncated"]:
                response.update(client.list_attached_role_policies(RoleName=rolename, Marker=response["Marker"]))
...
def checkuser(client, username, response):
    ...
    if not username == "":
        ...
        try:
            ...
            while response["IsTruncated"]:
                response.update(client.list_user_policies(UserName=username, Marker=response["Marker"]))
        ...
        try:
            ...
            while response["IsTruncated"]:
                response.update(client.list_attached_user_policies(UserName=username, Marker=response["Marker"]))
        ...
        try:
            groups_for_users = client.list_groups_for_user(UserName=username)
            groups_json_data = groups_for_users["Groups"]
            while groups_for_users["IsTruncated"]:
                groups_for_users = client.list_groups_for_user(UserName=username, Marker=groups_for_users["Marker"])
            ...
            for group in all_info["UserGroups"]:
                try:
                    group_response = client.list_group_policies(GroupName=group["GroupName"])
                ...
                try:
                    group_response = client.list_attached_group_policies(GroupName=group["GroupName"])
                ...
                for pol in group["AttachedPolicies"]:
                    try:
                        response = client.get_policy(PolicyArn=pol["PolicyArn"])
                        if response["ResponseMetadata"]:
                            del response["ResponseMetadata"]
