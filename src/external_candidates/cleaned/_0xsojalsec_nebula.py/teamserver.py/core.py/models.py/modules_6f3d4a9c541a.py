# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\models\Modules.py
import json
import os
import sys
...
from core.run_module import (
    run_aws_module,
    run_azure_module,
    run_azuread_module,
    run_digitalocean_module,
    run_gcp_module,
    run_office365_module,
)
...
def module_count():
    ...
    return {
        "all_count": all_count,
        "nr_of_modules": nr_of_modules,
        "nr_of_cloud_modules": nr_of_cloud_modules,
    }, 200
    ...
    try:
        ...
        if m_name == "aws":
            try:
                print(
                    colored(
                        f"[*] User {username} ran module {module} at {str(datetime.now())} on region {awsregion}",
                        "yellow",
                    )
                )
                return run_aws_module.run_aws_module(
                    imported_module,
                    all_sessions,
                    cred_prof,
                    useragent,
            ...
            try:
                print(
                    colored(
                        f"[*] User {username} ran module {module} at {str(datetime.now())}",
                        "yellow",
                    )
                )
                return run_digitalocean_module.run_digitalocean_module(
                    imported_module, all_sessions, cred_prof, useragent
                )
            ...
            try:
                ...
                return run_azure_module.run_azure_module(imported_module, all_sessions, cred_prof, workspace, useragent)
            ...
            try:
                ...
                return run_azuread_module.run_azuread_module(imported_module, all_sessions, cred_prof, useragent)
            except Exception as e:
                return {"error": str(e)}, 500
            ...
            try:
                ...
                return run_office365_module.run_o365_module(imported_module, all_sessions, cred_prof, useragent)
            except Exception as e:
                return {"error": str(e)}, 500
