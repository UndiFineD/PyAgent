# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\listeners\aws_s3_c2.py
import json
import os
import random
import string
...
def exploit(profile, workspace):
    ...
    serverprofile = boto3.client(**serverargs)

    stagerprofile = boto3.client(**args)
    ...
    if not checkBucket(bucket):
        ...
        if status["Status"] == "Successfully Created":
            ...
            return {"error": f"Error from module: {status['Error']}"}
    else:
        return {"BucketName": {"BucketName": bucket, "Status": "Bucket Exists. C2 Setup."}}
...
def setupBucket(profile, bucketname, commandkey, outputkey, stageruser, kmskey, serveruser):
    try:
        ...
        stsprofile = boto3.client(**stsargs)
        ...
        kmsprofile = boto3.client(**kmsargs)
        if kmskey == "":
                ...
                Policy=json.dumps(
                    {
                        ...
                        "Statement": [
                            ...
                            },
                            {
                                ...
                                "Effect": "Allow",
                                "Principal": {"AWS": f"arn:aws:iam::{accountid}:root"},
                                "Action": "kms:*",
                                "Resource": "*",
                            },
                        ],
        ...
        profile.create_bucket(Bucket=bucketname)
        ...
        profile.put_bucket_policy(Bucket=bucketname, Policy=bucketPolicy)

        profile.put_bucket_versioning(Bucket=bucketname, VersioningConfiguration={"Status": "Enabled"})

        return {"Status": "Successfully Created", "KMSKey": kmskey}
    except Exception as e:
        return {"Status": "Error Creating", "Error": str(e)}
