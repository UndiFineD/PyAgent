# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\cleanup\aws_guardduty_revert_finding_time.py
import sys
...
aws_command = (
    "aws guardduty update-detector --detector-id <detector-id>  --no-enable  --region <region> --profile <profile>"
)
...
def exploit(profile, workspace):
    ...
    try:
        profile.update_detector(DetectorId=detectorID, FindingPublishingFrequency=detectorTime)
    ...
    return {"Detector": {"Detector": detectorID, "Time": detectorTime, "Status": status}}
