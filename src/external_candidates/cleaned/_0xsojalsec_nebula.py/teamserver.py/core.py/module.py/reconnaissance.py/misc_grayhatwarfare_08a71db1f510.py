...
def exploit(workspace):
    ...
    for bucket_keyword in bucket_keywords:
        returned_output = bucket_keyword_func(bucket_keyword, nr_of_results_to_be_returned, api_token, cloud_vendor)
        ...
        for bucket in returned_output:
            ...
            if list_objects_bool.replace("\n", "").replace(" ", "").strip() == "true":
                if len(file_keywords) == 0:
                    bucket_files = object_keyword_func([], nr_of_results_to_be_returned, api_token, bucket["id"])
