# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\aws_secretsmanager_download_secrets.py
...
calls = ["secretsmanager:ListSecrets", "secretsmanager:GetSecretValue"]
...
def exploit(all_sessions, cred_prof, useragent, web_proxies, workspace):
    try:
        ...
        smProfile = giveMeClient(all_sessions, cred_prof, useragent, web_proxies, "secretsmanager")
        ...
        if secretName != "":
            ...
            while "NextToken" in result and result["NextToken"] != "":
                result = smProfile.list_secrets(NextToken=result["NextToken"])
        ...
        for secret in secretnames:
            try:
                ...
                if "SecretString" in secretValueReq:
                    returndict.append({"Name": secret, "SecretString": secretValueReq["SecretString"]})
            except Exception as e:
                returndict.append({"Name": secret, "SecretString": str(e)})
        """
		database_data = {
