# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\digitalocean_space_enum_all.py
import json
import sys
    ...
    for bucket in all_buckets:
        ...
        try:
            ...
            if bucketObjects["IsTruncated"]:
                bucketObjects = profile.list_objects_v2(
                    Bucket=bucket["Name"],
            ...
            if bucketObjects["KeyCount"] > 0:
                for bucketObject in bucketObjects["Contents"]:
                    ...
                    try:
                        bucketObjectLegalHold = profile.get_object_legal_hold(
                            Bucket=bucket["Name"], Key=bucketObject["Key"]
                        )
                    ...
                    try:
                        bucketObjectRetention = profile.get_object_retention(
                            Bucket=bucket["Name"], Key=bucketObject["Key"]
                        )
