# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\digitalocean_enum_all.py
import json
import sys
...
def exploit(profile):
    ...
    return_dict = {"ResourceType": resourceType, "ResourceInfo": None}
    RESOURCETYPES = [
        "accounts",
        "projects",
        "actions",
    ...
    if resourceType == "all":
        ...
        return {
            "error": "RESOURCE-TYPE can only be one of the values: accounts, projects, actions, apps, "
    ...
    return {"ResourceType": return_dict}, 200
...
def get1clicksResourceInfo(token):
    ...
    if infoRetrieved.status_code == 200:
        if "1_clicks" in infoRetrieved.json():
            ...
            return []
    elif infoRetrieved.status_code == 401 or infoRetrieved.status_code == 429 or infoRetrieved.status_code == 500:
...
def getContainerregistryResourceInfo(token):
    ...
    if infoRetrieved.status_code == 200:
        ...
        infoRetrieved.status_code == 401
        or infoRetrieved.status_code == 429
        or infoRetrieved.status_code == 404
        or infoRetrieved.status_code == 500
    ):
        return infoRetrieved.json()
    else:
        return infoRetrieved.text
            ...
            if functionInfo is not None:
                if len(functionInfo) > 0:
                    for function in functionInfo:
                        triggerinfoRetrieved = runDOAPICalls(
                            token,
                            "GET",
                            f"functions/namespaces/{function['namespace']}/triggers",
                        )
...
def getAllResourceInfo(token):
    return_dict = {
        "Accounts": getAccountsResourceInfo(token),
        "Projects": getProjectsResourceInfo(token),
        "Actions": getActionsResourceInfo(token),
        "Apps": getAppsResourceInfo(token),
        "Blockstorage": getBlockstorageResourceInfo(token),
        "Cdn": getCdnResourceInfo(token),
        "Certificates": getCertificatesResourceInfo(token),
        "Containerregistry": getContainerregistryResourceInfo(token),
        "Databases": getDatabasesResourceInfo(token),
        "Domains": getDomainsResourceInfo(token),
        "Droplets": getDropletsResourceInfo(token),
        "Firewalls": getFirewallsResourceInfo(token),
        "Floatingips": getFloatingipsResourceInfo(token),
        "Functions": getFunctionsResourceInfo(token),
        "Images": getImagesResourceInfo(token),
        "Kubernetes": getKubernetesResourceInfo(token),
        "Loadbalancers": getLoadbalancersResourceInfo(token),
        "Regions": getRegionsResourceInfo(token),
        "Reservedips": getReservedipsResourceInfo(token),
        "Snapshots": getSnapshotsResourceInfo(token),
        "Sshkeys": getSshkeysResourceInfo(token),
        "Tags": getTagsResourceInfo(token),
        "Uptime": getUptimeResourceInfo(token),
        "Vpcs": getVpcsResourceInfo(token),
    }
    return return_dict
