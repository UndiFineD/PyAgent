...
def filterTagsAndResources(resourceArn, tags):
    ...
    for tag in tags:
        if tag["Key"] == "aws:ssmmessages:target-id":
            tagFilter.append(f"Identity {resourceArn} has SSM:StartSession access to {tag['value']}")
        else:
    ...
    try:
        ...
        try:
            ...
            while response["PaginationToken"] != "":
                response = profile.get_resources(PaginationToken=response["PaginationToken"])
            ...
            for resource in resources:
                ...
                if len(returnList) == 0:
                    returnList[resourceInfo[2]] = {
                        resourceInfo[1]: {resourceInfo[0]: [{resource["ResourceARN"]: resource["Tags"]}]}
                    }
