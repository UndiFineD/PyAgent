...
def exploit(profile, workspace):
    ...
    try:
        ...
        if resourceType == "USERS":
            ...
            return {"error": "RESOURCE-TYPE should be either USERS, GROUPS, ROLES, LOCALPOLICIES, AWSPOLICIES, or ALL"}
        iamData = {
            "UserDetailList": [],
            "GroupDetailList": [],
            "RoleDetailList": [],
        ...
        if filter is None:
            ...
            while result["IsTruncated"]:
                result = profile.get_account_authorization_details(Marker=result["Marker"])
...
def otherGetUsers(profile, resourceType):
    ...
    for user in userlist:
        ...
        try:
            userInlinePolicies = profile.list_user_policies(User=user["UserName"])["PolicyNames"]
        ...
        try:
            userAttachedPolicies = profile.list_attached_user_policies(User=user["UserName"])["AttachedPolicies"]
...
def otherGetGroups(profile, resourceType):
    ...
    for group in groupList:
        ...
        try:
            groupPolicies = profile.list_group_policies(GroupName=group["GroupName"])["PolicyNames"]
        ...
        try:
            groupAttachedPolicies = profile.list_attached_group_policies(GroupName=group["GroupName"])[
                "AttachedPolicies"
            ]
...
def otherGetRoles(profile, resourceType):
    ...
    for role in iamData:
        try:
            roleInlinePolicies = profile.list_user_policies(User=role["RoleName"])["PolicyNames"]
        ...
        try:
            roleAttachedPolicies = profile.list_attached_user_policies(User=role["RoleName"])["AttachedPolicies"]
...
def otherGetIAM(profile, resourceType):
    if resourceType == "USERS":
        ...
        iamData = {
            ...
            "LocalPolicies": otherGetUsers(profile=profile, resourceType="LOCALPOLICIES")["ResourceType"][
                "LocalPolicies"
            ],
