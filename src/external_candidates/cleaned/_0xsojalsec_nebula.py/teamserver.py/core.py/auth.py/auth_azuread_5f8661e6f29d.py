...
def authenticate_device_code(proxies, verify, client_id, tenant):
    ...
    context = adal.AuthenticationContext(authority_host_uri, api_version=None, proxies=proxies, verify_ssl=verify)
...
def authenticate_clientid_clientsecret(client_id, client_secret, tenant, resource):
    ...
    token = context.acquire_token_with_client_credentials(GRAPH_RESOURCE, client_id, client_secret)
