# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_mantis.py\mantis.py\modules.py\dns.py\cloudflare_dadb31ceba45.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\dns\Cloudflare.py
        ...
        try:
            for zone_page in range(zones["result_info"]["total_pages"]):
                ...
                for record_page in range(1, records["result_info"]["total_pages"] + 1):
                    records = cf.zones.dns_records.get(zone["id"], params={"per_page": per_page, "page": record_page})[
                        "result"
                    ]
