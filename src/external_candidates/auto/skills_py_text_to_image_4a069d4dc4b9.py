# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\sabatesduran.py\clawdbot_meshyai_skill.py\scripts.py\text_to_image_4a069d4dc4b9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sabatesduran\clawdbot-meshyai-skill\scripts\text_to_image.py
...
from meshy_client import MeshyError, create_task, download, poll_task
...
def main() -> None:
    ...
    if status != "SUCCEEDED":
        ...
        raise MeshyError(
            ...
            f"Text-to-Image task did not succeed. status={status} task={task}"
    ...
    out_dir = os.path.join(
        ...
        args.out_dir, f"text-to-image_{task_id}_{_slug(args.prompt)}"
