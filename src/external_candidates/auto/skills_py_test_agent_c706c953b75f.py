# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kulotzkih.py\lex.py\scripts.py\test_agent_c706c953b75f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kulotzkih\lex\scripts\test-agent.py
...
class AgentTester:
    ...
    def test_invoke(self, input_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(f"{self.url}/invoke", json=payload, headers=self.headers, timeout=30)
...
def main():
    parser = argparse.ArgumentParser(description="Test Warden Protocol agent API")

    parser.add_argument("url", help="Agent API URL (e.g., https://api.example.com)")
    ...
    parser.add_argument("--invoke-only", action="store_true", help="Only test invoke endpoint")

    parser.add_argument("--stream-only", action="store_true", help="Only test stream endpoint")
