# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\spliff7777.py\windows_control.py\scripts.py\click_element_38eee59526ff.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\spliff7777\windows-control\scripts\click_element.py
...
def find_and_click(
    ...
    desktop, element_name, window_title=None, control_type=None, exact=False
    ...
    try:
        ...
        return (
            ...
            f"Clicked [{target['type']}] '{target['name']}' in {target['window']} @ {target['center']}",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Control type filter (Button, Hyperlink, etc.)",

    )

    parser.add_argument(

        "--exact", "-e", action="store_true", help="Exact name match only"

    )

    parser.add_argument(

        "--list", "-l", action="store_true", help="List clickable elements"

    )

    parser.add_argument(

        "--delay", "-d", type=float, default=0, help="Delay before clicking (seconds)"
