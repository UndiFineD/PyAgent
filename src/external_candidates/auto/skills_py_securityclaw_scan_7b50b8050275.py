# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\mallen_lbx.py\securityclaw.py\scripts.py\securityclaw_scan_7b50b8050275.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mallen-lbx\securityclaw\scripts\securityclaw_scan.py
...
RULES = [
    ...
    (
        ...
        "high",
        re.compile(
            r"\b(fetch\(|axios\.|curl\b|wget\b|requests\.|http\.client|net\.Socket|WebSocket\b)\b",
            re.I,
        ),
    ...
    (
        ...
        "high",
        re.compile(
            r"(OPENAI_API_KEY|GEMINI_API_KEY|SEARXNG|PORTAINER_TOKEN|BOT_TOKEN|PRIVATE_KEY|BEGIN\s+PRIVATE\s+KEY)",
            re.I,
        ),
        "Possible secret access/exfil target.",
    ),
    ...
    (
        ...
        "medium",
        re.compile(
            r"(ignore\s+previous\s+instructions|system\s+prompt|developer\s+message|exfiltrate|BEGIN\s+SYSTEM\s+PROMPT)",
            re.I,
        ),
    ...
    (
        ...
        "high",
        re.compile(
            r"(~/?\.openclaw/|/etc/|\.ssh/|id_rsa|authorized_keys|keychain|\.env\b)",
            re.I,
        ),
...
def scan_skill(skill_path: Path, base_dir: Path) -> Dict:
    ...
    return {
        ...
        "recommendation": ("quarantine" if SEVERITY_ORDER.get(sev, 0) >= SEVERITY_ORDER["high"] else "review"),
    }
...
def main() -> int:
    ...
    ap.add_argument(
        "--skills-dir",
        required=True,
        help="Path to skills directory (e.g., ~/.openclaw/skills)",
    )
    ...
    ap.add_argument(
        "--quarantine",
        action="store_true",
        help="Move high-severity skills to quarantine dir",
    )
