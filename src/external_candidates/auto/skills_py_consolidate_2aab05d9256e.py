# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\rosepuppy.py\memory_complete.py\scripts.py\consolidate_2aab05d9256e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rosepuppy\memory-complete\scripts\consolidate.py
    ...
    for i, fact in enumerate(facts):
        ...
        for j, other in enumerate(seen):
            ...
            ratio = SequenceMatcher(
                ...
                None, fact["text"].lower(), other["text"].lower()
...
def get_stats():
    ...
    return {
        ...
        "daily_logs": len(
            ...
            [f for f in daily_logs if re.match(r"\d{4}-\d{2}-\d{2}", f.name)]
...
def generate_summary_updates(facts):
    ...
    recent = [
        ...
        f for f in facts if datetime.strptime(f["date"], "%Y-%m-%d").date() >= week_ago
