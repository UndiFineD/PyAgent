# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\scripts.py\patterns.py\volume_analyzer_356f2ef579d9.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\patterns\volume_analyzer.py
        ...
        try:
            ...
            if pd.notna(obv_current) and pd.notna(obv_past):
                ...
                logger.debug(
                    ...
                    f"OBV trend: {trend} (current={obv_current:.2f}, past={obv_past:.2f})"
        ...
        try:
            ...
            if pd.notna(vpt_current) and pd.notna(vpt_past):
                ...
                logger.debug(
                    ...
                    f"VPT trend: {trend} (current={vpt_current:.2f}, past={vpt_past:.2f})"
        ...
        try:
            ...
            if surge_detected:
                ...
                logger.info(
                    ...
                    f"Volume surge detected: {volume_ratio:.2f}x average volume"
    ...
    def calculate_volume_profile(self, df: pd.DataFrame, num_bins: int = 10) -> Dict:
        ...
        try:
            ...
            df_copy["price_bin"] = pd.cut(
                ...
                df_copy["close"], bins=bins, labels=bin_labels

            )

            # Sum volume for each bin

            volume_by_price = df_copy.groupby("price_bin", observed=True)[
            ...
            logger.info(
                ...
                f"Volume profile: POC at {poc_price:.2f} with volume {poc_volume:.2f}"
