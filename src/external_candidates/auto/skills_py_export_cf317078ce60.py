# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\macstenk.py\skill_exporter.py\scripts.py\export_cf317078ce60.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\macstenk\skill-exporter\scripts\export.py
...
def export_skill(skill_path: Path, output_path: Path, target: str, llm: str, port: int):
    ...
    dockerfile = TEMPLATES["Dockerfile"].format(system_deps=build_dockerfile_deps(deps["system_deps"]), port=port)
    ...
    compose = TEMPLATES["docker-compose.yml"].format(skill_name=skill_info["name"].replace("-", "_"), port=port)
    ...
    if target == "railway":
        ...
        fly_toml = TEMPLATES["fly.toml"].format(skill_name=skill_info["name"], port=port)
...
def main():
    parser = argparse.ArgumentParser(description="Export Clawdbot skills as standalone microservices")

    parser.add_argument("--skill", "-s", required=True, help="Path to skill directory")

    parser.add_argument("--output", "-o", help="Output directory (default: ./<skill-name>-service)")
