# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kevin37li.py\gettr_transcribe_summarize.py\scripts.py\extract_gettr_og_video_3926c5284537.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kevin37li\gettr-transcribe-summarize\scripts\extract_gettr_og_video.py
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                print(
                    f"[retry] Attempt {attempt + 1} failed: {e}. Retrying in {wait:.1f}s...",
                    file=sys.stderr,
                )
...
def main(argv: list[str]) -> int:
    ...
    if post_type == "image":
        ...
        print(
            "[hint] This appears to be a text/article post, not a video.",
            file=sys.stderr,
        )

    else:
        print(
            "[hint] This post may not contain a video, or it may require authentication.",
            file=sys.stderr,
        )
