# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\stream_omni.py\stream_omni.py\eval.py\run_stream_omni_44d1a4dd6465.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\Stream-Omni\stream_omni\eval\run_stream_omni.py
...
from stream_omni.conversation import SeparatorStyle, conv_templates
...
class CosyVoiceModel:
    ...
    def token_to_speech(self, tokens: str) -> torch.Tensor:
        ...
        tts_speech_token = (
            ...
            torch.tensor(self.extract_speech_token(tokens))
...
def eval_model(args):
    ...
    query = (
        ...
        cosyvoice.speech_to_token(args.query)
    ...
    llm_outputs = tokenizer.batch_decode(

        torch.cat(model.llm_ids, dim=-1), skip_special_tokens=True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Evaluate a multimodal model with speech and image inputs."

    )

    parser.add_argument(

        "--model-path", type=str, default="facebook/opt-350m", help="Path to the model"
    ...
    parser.add_argument(
        ...
        "--model-name", type=str, default="llava.llama.audio", help="Name of the model"

    )

    parser.add_argument(

        "--image-file", type=str, default=None, help="Path or URL to image file(s)"

    )

    parser.add_argument(

        "--query", type=str, required=True, help="Input query (text or audio file)"
    ...
    parser.add_argument(
        ...
        "--sep", type=str, default=",", help="Separator for multiple image files"
    ...
    parser.add_argument(
        ...
        "--top_p", type=float, default=None, help="Top-p sampling probability"
