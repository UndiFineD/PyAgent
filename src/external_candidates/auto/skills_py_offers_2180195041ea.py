# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\kesslerio.py\amadeus_hotels.py\scripts.py\offers_2180195041ea.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\amadeus-hotels\scripts\offers.py
...
def make_request(url: str, params: dict, retries: int = 3) -> dict:
    ...
    for attempt in range(retries):
        ...
        if response.status_code == 401:
            raise EnvironmentError("Authentication failed. Check AMADEUS_API_KEY and AMADEUS_API_SECRET.")
...
def main():
    ...
    parser.add_argument("--hotels", required=True, help="Comma-separated hotel IDs from search")
