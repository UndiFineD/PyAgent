# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\salen_project.py\finance_tracker.py\lib.py\insights_220b2eb0e168.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\salen-project\finance-tracker\lib\insights.py
...
def get_spending_velocity(days: int = 30) -> Dict[str, Any]:
    ...
    month_tx = [
        ...
        tx for tx in transactions if datetime.fromisoformat(tx["date"]) >= month_start
...
def compare_to_last_period(days: int = 7) -> Dict[str, Any]:
    ...
    previous = [
        ...
        tx for tx in all_tx if previous_cutoff <= tx["timestamp"] < current_cutoff
...
def get_insights() -> str:
    ...
    if velocity.get("has_data"):
        ...
        lines.append(
            ...
            f"   Projected month total: {velocity['projected_month']:,} {currency}"
    ...
    lines.append(
        ...
        f"   {emoji} Spending {direction} {abs(comparison['change_pct']):.0f}%"
    ...
    if significant_changes:
        ...
        for cat, data in sorted(
            ...
            significant_changes, key=lambda x: abs(x[1]["change_pct"]), reverse=True
                ...
                for goal in goals[:1]:
                    ...
                    if progress.get("days_left"):
                        ...
                        lines.append(
                            ...
                            f"   Next deadline: {goal['name']} in {progress['days_left']} days"
...
def get_weekly_digest() -> str:
    ...
    sorted_cats = sorted(
        ...
        comparison["by_category"].items(), key=lambda x: x[1]["current"], reverse=True
