# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\xenofex7.py\bookstack.py\scripts.py\bookstack_ef38c438b8e6.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\xenofex7\bookstack\scripts\bookstack.py
...
def api_call(method, endpoint, data=None, params=None):
    ...
    if not BASE_URL or not TOKEN_ID or not TOKEN_SECRET:
        ...
        print(
            ...
            "❌ Error: BOOKSTACK_URL, BOOKSTACK_TOKEN_ID, and BOOKSTACK_TOKEN_SECRET required!"
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                f"❌ HTTP {e.code}: {error_data.get('error', {}).get('message', 'Unknown error')}"
...
def list_books(args):
    ...
    for book in result["data"]:
        ...
        desc = (
            ...
            book.get("description", "")[:50] + "..." if book.get("description") else ""
...
def create_chapter(args):
    ...
    data = {"book_id": args.book_id, "name": args.name, "description": args.description}
...
def list_pages(args):
    ...
    for page in result["data"]:
        ...
        location = (
            ...
            f"Chapter {page['chapter_id']}"

            if page.get("chapter_id")

            else f"Book {page['book_id']}"
...
def search(args):
    ...
    params = {"query": args.query, "count": args.count or 20}
    ...
    print(
        ...
        f"🔍 {result.get('total', len(result['data']))} Results for '{args.query}':\n"

    )

    for item in result["data"]:

        icon = {"page": "📄", "chapter": "📑", "book": "📚", "bookshelf": "📁"}.get(
