# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\evaluation.py\src.py\evaluators.py\exact_match_7fe23e59597e.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\evaluators\exact_match.py
...
class ExactMatch(BaseEvaluator):
    ...
    def __init__(self, config: dict):
        ...
        self.case_sensitive = config.get(

            "case_sensitive", False

        )  # Default: case insensitive

        self.normalize_whitespace = config.get(

            "normalize_whitespace", True

        )  # Default: normalize whitespace

        self.extract_choice = config.get(

            "extract_choice", True

        )  # Default: extract choices like (a), (b), (c)
    ...
    async def evaluate(self, answer_results: List[AnswerResult]) -> EvaluationResult:
        ...
        for answer_result in answer_results:
            ...
            is_correct = self._check_match(
                ...
                answer_result.golden_answer, answer_result.answer
