# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\jb_fliz.py\fliz_ai_video_generator.py\scripts.py\poll_status_cad5e1a24128.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jb-fliz\fliz-ai-video-generator\scripts\poll_status.py
...
def get_video_status(api_key: str, video_id: str) -> dict:
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/rest/videos/{video_id}", headers=headers, timeout=TIMEOUT)
...
def poll_until_complete(api_key: str, video_id: str, interval: int = 15, timeout: int = 600) -> dict:
    ...
    while True:
        ...
        if step in COMPLETE_STATES:
            ...
            return {"success": True, "video": video, "url": url, "elapsed": elapsed}
...
def main():
    ...
    parser.add_argument("--once", action="store_true", help="Check status once without polling")
    ...
    if args.once:
        ...
        result = poll_until_complete(api_key, args.video_id, interval=args.interval, timeout=args.timeout)
