# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\seojoonkim.py\prompt_guard.py\scripts.py\analyze_log_8c80e78fc770.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\seojoonkim\prompt-guard\scripts\analyze_log.py
...
def filter_entries(
    ...
    if since:
        ...
        result = [
            ...
            if e["date"] and datetime.strptime(e["date"], "%Y-%m-%d") >= since_date
...
def generate_summary(entries: List[Dict]) -> Dict:
    ...
    total_risk = sum(
        ...
        severity_counts[s] * risk_weights.get(s, 0) for s in severity_counts
...
def main():
    ...
    parser.add_argument(
        ...
        "--log", default="memory/security-log.md", help="Path to log file"
        ...
        if args.json:
            ...
            for entry in filtered:
                ...
                print(
                    ...
                    f"{emoji.get(entry['severity'], '❓')} [{entry['date']} {entry['time']}] "
