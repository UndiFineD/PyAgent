# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\jdrhyne.py\gsc.py\scripts.py\gsc_query_e5fb6a0ed411.py
...
def low_ctr_opportunities(site_url: str, days: int = 28, min_impressions: int = 100):
    ...
    opportunities = [row for row in rows if row.get("impressions", 0) >= min_impressions and row.get("ctr", 0) < 0.03]
