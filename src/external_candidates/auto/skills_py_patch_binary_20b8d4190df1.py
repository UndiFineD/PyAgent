# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\masonc15.py\clawd_modifier.py\scripts.py\patch_binary_20b8d4190df1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\masonc15\clawd-modifier\scripts\patch_binary.py
    ...
    for patch in patch_def["patches"]:
        ...
        if context:
            ...
            if count > 0:
                ...
                patch_details.append({"count": count, "description": patch.get("description", "Unknown")})
...
def main():
    parser = argparse.ArgumentParser(description="Patch Clawd's appearance in compiled Claude Code binary")
    ...
    if not binary_path or not binary_path.exists():
        print(
            f"Error: Claude binary not found. Use --binary to specify path.",
            file=sys.stderr,
        )
