# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\salen_project.py\finance_tracker.py\lib.py\trends_c030c69d194d.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\salen-project\finance-tracker\lib\trends.py
...
def analyze_trends(days: int = 90) -> str:
    ...
    lines = [f"📊 Spending Trends (last {days} days)", "━━━━━━━━━━━━━━━━━━━━━", ""]
    ...
    if weekday_avgs:
        ...
        lines.append(
            ...
            f"   📈 Highest spending: {max_day} ({weekday_avgs[max_day]:,} avg)"

        )

        lines.append(

            f"   📉 Lowest spending: {min_day} ({weekday_avgs[min_day]:,} avg)"
    ...
    if len(sorted_cats) > 0:
        ...
        if top_pct > 40:
            ...
            lines.append(
                ...
                f"   ⚠️ {get_emoji(top_cat)} {top_cat.capitalize()} is {top_pct:.0f}% of spending"
...
def get_budget_status(daily_budget: int) -> str:
    ...
    lines.append(
        ...
        f"{status} Today: {today_total:,} / {daily_budget:,} ({today_pct:.0f}%)"
    ...
    lines.append(
        ...
        f"{status} This Week: {week_total:,} / {week_budget:,} ({week_pct:.0f}%)"
