# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\visionik.py\ouracli.py\src.py\ouracli.py\charts_mermaid_be4f2608a071.py
...
def create_mermaid_heartrate_chart(heartrate_data: list[dict]) -> str:
    ...
    lines.append(
        ...
        '    x-axis "Hour" [' + ", ".join([f'"{h:02d}"' for h, _ in hourly_data]) + "]"
    ...
    lines.append(
        ...
        "    bar [" + ", ".join([f"{bpm:.0f}" for _, bpm in hourly_data]) + "]"
...
def create_mermaid_bar_chart(met_items: list[float]) -> str:
    ...
    lines.append(
        ...
        + ", ".join([f'"{h:02d}"' for h, _ in hourly_buckets])
    ...
    lines.append(
        ...
        "    bar [" + ", ".join([f"{met:.2f}" for _, met in hourly_buckets]) + "]"
