# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\johnsonfarmsus.py\claw_conductor.py\examples.py\complex_feature_15dfe46fa5e7.py
...
def main():
    ...
    if total_cost_bonus > 0:
        free_tasks = sum(
            1 for r in results if r.get("details", {}).get("breakdown", {}).get("cost", {}).get("points", 0) > 0
        )
