# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\wuminmin.py\retail_trade_report_generator.py\scripts.py\retail_trade_report_generator_05c972ab1b65.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\wuminmin\retail-trade-report-generator\scripts\retail_trade_report_generator.py
...
class RetailTradeReportGenerator:
    ...
    def identify_files(self):
        ...
        for filename in all_files:
            ...
            if file_type:
                ...
                start_date, end_date, date_str = self.extract_date_from_filename(
        ...
        if len(self.current_week_files) != 6:
            ...
            raise ValueError(
                ...
                f"Expected 6 current week files, found {len(self.current_week_files)}"
        ...
        if len(self.previous_week_files) != 6:
            ...
            raise ValueError(
                ...
                f"Expected 6 previous week files, found {len(self.previous_week_files)}"
    ...
    def extract_drp_data(self, filepath):
        ...
        for idx in range(8, 16):  # Row 8-15 are data rows
            ...
            data[region] = {
                ...
                "fwa_5g": self._safe_float(df.iloc[idx, 10])

                + self._safe_float(df.iloc[idx, 11]),
    ...
    def extract_ls_data(self, filepath):
        ...
        stores_data = {"mobile_prepaid": {}, "mobile_postpaid": {}, "fwa_4g": {}}
        ...
        for idx in range(8, len(df)):
            ...
            stores_data["fwa_4g"][store] = self._safe_float(
    ...
    def process_all_files(self):
        ...
        for week_label, file_dict in [

            ("current", self.current_week_files),
            ...
            if "TECNO" in file_dict:
                ...
                week_data["tecno"] = self.extract_tecno_data(

                    file_dict["TECNO"]["filepath"]
            ...
            if "LS" in file_dict:
                ...
                week_data["ls"] = {
                    ...
                    "mobile_prepaid": self.aggregate_stores_by_region(

                        ls_raw["mobile_prepaid"]

                    ),

                    "mobile_postpaid": self.aggregate_stores_by_region(

                        ls_raw["mobile_postpaid"]
            ...
            if "DXS_Prepaid" in file_dict:
                ...
                dxs_prepaid_raw = self.extract_dxs_prepaid_data(

                    file_dict["DXS_Prepaid"]["filepath"]

                )

                week_data["dxs_prepaid"] = self.aggregate_stores_by_region(
            ...
            if "DXS_Postpaid" in file_dict:
                ...
                dxs_postpaid_raw = self.extract_dxs_postpaid_data(

                    file_dict["DXS_Postpaid"]["filepath"]

                )

                week_data["dxs_postpaid"] = self.aggregate_stores_by_region(
            ...
            if "DXS_FWA" in file_dict:
                ...
                dxs_fwa_raw = self.extract_dxs_fwa_data(

                    file_dict["DXS_FWA"]["filepath"]
    ...
    def generate_report(self, output_path):
        ...
        ws[f"A{row}"] = (
            ...
            f"Last Week: {previous_date_str} | This Week: {current_date_str}"
    ...
    def _write_product_section(self, ws, start_row, product_name, product_key):
        ...
        for region in self.REGIONS:
            ...
            drp_current = (
                ...
                self.current_week_data["drp"].get(region, {}).get(product_key, 0)

            )

            dxs_current = self._get_dxs_value(

                self.current_week_data, product_key, region
            ...
            drp_previous = (
                ...
                self.previous_week_data["drp"].get(region, {}).get(product_key, 0)

            )

            dxs_previous = self._get_dxs_value(

                self.previous_week_data, product_key, region

            )

            ls_previous = self._get_ls_value(
    ...
    def _write_drp_program_section(self, ws, start_row):
        ...
        for region in self.REGIONS:
            ...
            double_current = (
                ...
                self.current_week_data["drp"].get(region, {}).get("double_data", 0)

            )

            camon_current = (

                self.current_week_data["tecno"].get(region, {}).get("camon_40", 0)

            )

            pova_current = (

                self.current_week_data["tecno"].get(region, {}).get("pova_7", 0)
            ...
            double_previous = (
                ...
                self.previous_week_data["drp"].get(region, {}).get("double_data", 0)

            )

            camon_previous = (

                self.previous_week_data["tecno"].get(region, {}).get("camon_40", 0)

            )

            pova_previous = (

                self.previous_week_data["tecno"].get(region, {}).get("pova_7", 0)
        ...
        ws.cell(
            ...
            self.calculate_wow(total_current["double"], total_previous["double"]),
        ...
        ws.cell(
            ...
            row, 5, self.calculate_wow(total_current["tecno"], total_previous["tecno"])
        ...
        ws.cell(
            ...
            row, 7, self.calculate_wow(total_current["camon"], total_previous["camon"])
        ...
        ws.cell(
            ...
            row, 9, self.calculate_wow(total_current["pova"], total_previous["pova"])
