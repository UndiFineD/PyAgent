# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\_0xsojalsec_evermemos.py\evaluation.py\src.py\evaluators.py\hybrid_b7c829cb077c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\evaluators\hybrid.py
...
class HybridEvaluator(BaseEvaluator):
    ...
    async def evaluate(self, answer_results: List[AnswerResult]) -> EvaluationResult:
        ...
        if choice_questions:
            ...
            choice_eval_result = await self.exact_match_evaluator.evaluate(
        ...
        choice_results_map = {
            ...
            result["question_id"]: result for result in choice_detailed_results

        }

        open_results_map = {

            result["question_id"]: result for result in open_detailed_results
        ...
        print(
            ...
            f"   - Multiple-choice: {choice_correct}/{len(choice_questions)} ({choice_correct/len(choice_questions)*100:.1f}%)"

            if choice_questions

            else "   - Multiple-choice: 0/0"

        )

        print(

            f"   - Open-ended: {open_correct}/{len(open_questions)} ({open_correct/len(open_questions)*100:.1f}%)"
        ...
        if category_stats:
            ...
            for cat, stats in sorted(category_stats.items()):
                ...
                print(
                    ...
                    f"   Category {cat}: {stats['correct']}/{stats['total']} ({stats['accuracy']:.2%})"
        ...
        combined_metadata = {
            ...
            "choice_accuracy": (
                ...
                choice_correct / len(choice_questions) if choice_questions else 0.0

            ),

            "open_accuracy": (

                open_correct / len(open_questions) if open_questions else 0.0
