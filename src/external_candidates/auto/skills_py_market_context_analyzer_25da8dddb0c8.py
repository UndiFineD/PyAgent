# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\cryptocurrency_trader_skill.py\scripts.py\market_context_analyzer_25da8dddb0c8.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\market_context_analyzer.py
...
class MarketContextAnalyzer:
    ...
    def analyze_market_context(self) -> Dict:
        ...
        context["market_regime"] = self._determine_market_regime(

            context["btc_analysis"]

        )

        # Estimate fear/greed

        context["fear_greed_estimate"] = self._estimate_fear_greed(
        ...
        context["trading_recommendation"] = self._generate_trading_recommendation(
        ...
        logger.info(
            ...
            f"Market context: {context['market_regime']}, "
    ...
    def _analyze_btc(self) -> Dict:
        ...
        try:
            ...
            df_1h = pd.DataFrame(
                ...
                btc_1h, columns=["timestamp", "open", "high", "low", "close", "volume"]

            )

            df_4h = pd.DataFrame(

                btc_4h, columns=["timestamp", "open", "high", "low", "close", "volume"]

            )

            df_1d = pd.DataFrame(

                btc_1d, columns=["timestamp", "open", "high", "low", "close", "volume"]
            ...
            price_change_24h = (
                ...
                (df_1h["close"].iloc[-1] - df_1h["close"].iloc[-24])
            ...
            price_change_7d = (
                ...
                (
                    ...
                    (df_1h["close"].iloc[-1] - df_1h["close"].iloc[-168])

                    / df_1h["close"].iloc[-168]
            ...
            price_change_30d = (
                ...
                (
                    ...
                    (df_1d["close"].iloc[-1] - df_1d["close"].iloc[-30])
            ...
            ma_50 = (
                ...
                df_1d["close"].rolling(50).mean().iloc[-1]
    ...
    def _determine_market_regime(self, btc_analysis: Dict) -> str:
        ...
        if (
            ...
            and price_change_7d > 3

            and trend_aligned

            and btc_analysis.get("trend_short") == "up"
        ...
        if (
            ...
            and btc_analysis.get("trend_short") == "down"
    ...
    def _estimate_fear_greed(self, btc_analysis: Dict) -> Dict:
        ...
        if not btc_analysis.get("success"):
            ...
            return {"score": 50, "sentiment": "neutral", "confidence": 0.0}
    ...
    def _generate_trading_recommendation(self, context: Dict) -> Dict:
        ...
        recommendation = {"action": "NEUTRAL", "reason": [], "adjustments": {}}
    ...
    def should_trade_altcoin(self, symbol: str, context: Optional[Dict] = None) -> Dict:
