# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\sabatesduran.py\clawdbot_meshyai_skill.py\scripts.py\image_to_3d_obj_ee985d5f6794.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sabatesduran\clawdbot-meshyai-skill\scripts\image_to_3d_obj.py
...
from meshy_client import MeshyError, create_task, download, file_to_data_uri, poll_task
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=[None, "standard", "lowpoly"],

        help="standard|lowpoly",
    ...
    ap.add_argument(
        ...
        "--should-texture", action="store_true", help="Force should_texture=true"

    )

    ap.add_argument(

        "--no-texture", action="store_true", help="Force should_texture=false"
    ...
    if status != "SUCCEEDED":
        ...
        raise MeshyError(
            ...
            f"Image-to-3D task did not succeed. status={status} task={task}"
    ...
    out_dir = os.path.join(
        ...
        args.out_dir, f"image-to-3d_{task_id}_{_slug(args.image or args.image_url)}"
