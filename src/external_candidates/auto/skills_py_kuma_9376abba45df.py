# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\msarheed.py\uptime_kuma.py\scripts.py\kuma_9376abba45df.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\msarheed\uptime-kuma\scripts\kuma.py
...
def cmd_add_monitor(args):
    ...
    if not monitor_type:
        print(
            f"Error: Unknown monitor type '{args.type}'. Valid types: {', '.join(monitor_types.keys())}",
            file=sys.stderr,
        )
