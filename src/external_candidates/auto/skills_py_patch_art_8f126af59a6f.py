# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\masonc15.py\clawd_modifier.py\scripts.py\patch_art_8f126af59a6f.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\masonc15\clawd-modifier\scripts\patch_art.py
...
def apply_variant(cli_path: str, variant_name: str, dry_run: bool = False) -> dict:
    ...
    if total_replacements == 0:
        return {
            "success": False,
            "error": "No matching patterns found (already modified?)",
        }
...
def restore_original(cli_path: str, dry_run: bool = False) -> dict:
    ...
    if total == 0:
        return {
            "success": True,
            "message": "Clawd appears to already be in original form",
        }
