# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\mastergoogler.py\opensoul.py\research_agent_c0f6883d86f1.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mastergoogler\opensoul\research_agent.py
...
class ResearchAgent:
    ...
    async def generate_summary(self):
        ...
        current_session_logs = [log for log in history if log.get("session_id") == self.logger.config["session_id"]]

        # Calculate metrics

        total_searches = sum(
            len([m for m in log.get("metrics", []) if m.get("action") == "web_search"]) for log in current_session_logs
        )

        total_tokens = sum(
            log.get("total_tokens_in", 0) + log.get("total_tokens_out", 0) for log in current_session_logs
        )
        ...
        summary = {
            ...
            "searches_per_minute": (
                total_searches / (duration.total_seconds() / 60) if duration.total_seconds() > 0 else 0
            ),
        }
