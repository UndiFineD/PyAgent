# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\santyr.py\skill_hexmem.py\review_2b498fdbee96.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\santyr\skill-hexmem\review.py
...
def show_due_items(conn, limit: int = 10):
    ...
    for row in rows:
        ...
        priority_emoji = {"URGENT": "🔴", "DUE": "🟡", "OPTIONAL": "🟢"}.get(
        ...
        print(
            ...
            f"   Retention: {retention*100:.0f}% | Importance: {importance:.1f} | Last review: {hours:.0f}h ago"
...
def review_item(conn, source_table: str, source_id: int, quality: int):
    ...
    last_dt = datetime.fromisoformat(
        ...
        last_reviewed.replace("Z", "+00:00").replace("+00:00", "")
...
def main():
    ...
    parser.add_argument(
        ...
        "--review", type=str, help="Review an item (format: events:ID or lessons:ID)"
    ...
    parser.add_argument(
        ...
        "--apply", action="store_true", help="Apply decay changes (not dry run)"
