# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\veeramanikandanr48.py\options_strategy_advisor.py\scripts.py\black_scholes_c21fff8f1daa.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\options-strategy-advisor\scripts\black_scholes.py
...
if __name__ == "__main__":
    ...
    print(
        ...
        f"  Delta: {call_greeks['delta']:.4f} (${call_greeks['delta']*100:.2f} per $1 move)"

    )

    print(

        f"  Gamma: {call_greeks['gamma']:.4f} (delta changes by {call_greeks['gamma']:.4f})"
    ...
    print(
        ...
        f"  Delta: {put_greeks['delta']:.4f} (${put_greeks['delta']*100:.2f} per $1 move)"

    )

    print(

        f"  Gamma: {put_greeks['gamma']:.4f} (delta changes by {put_greeks['gamma']:.4f})"
    ...
    simulated_prices = [
        ...
        180 * np.exp(np.sum(np.random.randn(i) * 0.01)) for i in range(90)
