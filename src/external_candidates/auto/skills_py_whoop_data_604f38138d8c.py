# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\rodrigouroz.py\whoop_health_analysis.py\scripts.py\whoop_data_604f38138d8c.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rodrigouroz\whoop-health-analysis\scripts\whoop_data.py
    ...
    while True:
        ...
        if pages > max_pages:
            ...
            print(
                ...
                f"Pagination cap reached ({max_pages} pages) for {endpoint}",
...
def get_summary(days=7):
    ...
    if sleep_perfs:
        ...
        summary["sleep"]["avg_performance"] = round(

            sum(sleep_perfs) / len(sleep_perfs), 1

        )

    if sleep_durations:

        summary["sleep"]["avg_hours"] = round(

            sum(sleep_durations) / len(sleep_durations), 1
