# Extracted from: C:\DEV\PyAgent\src\external_candidates\ingested\skills.py\skills.py\weird_aftertaste.py\literature_review.py\scripts.py\lit_search_794c26423b53.py
# NOTE: extracted with static-only rules; review before use

# Extracted from: C:\DEV\PyAgent\.external\skills\skills\weird-aftertaste\literature-review\scripts\lit_search.py
...
USER_EMAIL = os.getenv(
    ...
    "USER_EMAIL", os.getenv("CLAWDBOT_EMAIL", "anonymous@example.org")
...
def search_s2(query, limit=10):
    ...
    try:
        ...
        response = requests.get(
            ...
            url, params=params, headers=get_s2_headers(), timeout=REQUEST_TIMEOUT
...
def search_cr(query, limit=10):
    ...
    try:
        ...
        response = requests.get(
            ...
            CR_BASE_URL, params=params, headers=get_headers(), timeout=REQUEST_TIMEOUT
            ...
            results.append(

                {
                    ...
                    "title": (
                        ...
                        " ".join(item.get("title", [])) if item.get("title") else None
...
def search_pm(query, limit=10):
    ...
    try:
        ...
        search_res = requests.get(
            ...
            search_url, params=search_params, timeout=REQUEST_TIMEOUT
        ...
        fetch_res = requests.get(
            ...
            fetch_url, params=fetch_params, timeout=REQUEST_TIMEOUT
...
def main():
    ...
    search_parser.add_argument(
        ...
        "--limit", type=int, default=10, help="Max results per source"
    ...
    search_parser.add_argument(
        ...
        help="Deduplicate results by DOI (auto-enabled for multi-source)",
    ...
    if args.command == "search":
        ...
        if args.source in ["s2", "both", "all"]:

            output["semantic_scholar"] = search_s2(args.query, args.limit)

        if args.source in ["oa", "both", "all"]:

            output["openalex"] = search_oa(args.query, args.limit)

        if args.source in ["cr", "all"]:

            output["crossref"] = search_cr(args.query, args.limit)

        if args.source in ["pm", "all"]:

            output["pubmed"] = search_pm(args.query, args.limit)
            ...
            if args.dedupe or args.source in ["both", "all"]:
                ...
                combined = {
                    ...
                    "total_before_dedupe": sum(
                        ...
                        len(r.get("data", [])) for r in output.values()
        ...
        try:
            ...
            params = {
                ...
                "fields": "title,year,abstract,authors,citationCount,venue,externalIds,tldr,referenceCount"
