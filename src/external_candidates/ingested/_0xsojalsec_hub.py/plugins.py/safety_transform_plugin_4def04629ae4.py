...
class SafetyTransformPlugin(BasePlugin):
    ...
    def generate_mask(self, image, bbox):
        ...
        try:
            ...
            print(
                f"[DEBUG] Input box shape (should be [[[x1, y1, x2, y2]]]): {input_boxes}"
            )
            ...
            if len(masks.shape) == 3:
                ...
                mask = masks[mask_idx]
                print(
                    f"[DEBUG] Selected mask index: {mask_idx}, score: {scores[mask_idx]}"
                )
            else:
                mask = masks
            ...
            cv2.rectangle(
                mask_viz, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2
            )
    ...
    def inpaint_safety_gear(self, image, mask, num_inference_steps, guidance_scale):
        ...
        try:
            ...
            mask_pil = Image.fromarray((mask * 255).astype(np.uint8)).convert(
                "L"
    ...
    def run(self, image: np.ndarray, **kwargs) -> np.ndarray:
        ...
        try:
            ...
            result = self.inpaint_safety_gear(
                image, mask, num_inference_steps, guidance_scale
            )
