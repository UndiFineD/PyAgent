# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\plugins\agent_detection_plugin.py
import base64
import io
import json
import logging
...
class GeminiAgentPlugin(BasePlugin):
    ...
    def validate_and_adjust_bbox(
        ...
        try:
            ...
            if (x2_px - x1_px) < 10 or (y2_px - y1_px) < 10:
                self.logger.warning(
                    f"Bounding box too small: {[x1_px, y1_px, x2_px, y2_px]}"
                )
                return None
    ...
    def extract_bounding_boxes(
        self, image: np.ndarray, analysis: str, prompt: str, confidence_threshold
    ) -> List[Dict]:
        ...
        try:
            ...
            final_detections = self.non_max_suppression(
                valid_detections, iou_threshold=confidence_threshold
            )
    ...
    def draw_detections(
        self, image: np.ndarray, detections: List[Dict], debug: bool = False
    ) -> np.ndarray:
        ...
        for det in detections:
            ...
            if debug:
                ...
                cv2.rectangle(
                    output, (x1, y1 - text_h - 8), (x1 + text_w + 4, y1), color, -1
                )
