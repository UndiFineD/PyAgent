# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\plugins\content_moderation_plugin.py
import base64
import io
import json
import logging
...
class ContentModerationPlugin(BasePlugin):
    ...
    def analyze_content(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            json_str = self.extract_json_from_response(
    ...
    def normalize_coordinates(
        self, coords: List[float], width: int, height: int
    ) -> List[int]:
    ...
    def draw_analysis_results(
        ...
        for category_name, category_data in categories.items():
            ...
            for region in regions:
                ...
                cv2.rectangle(
                    output, (x1, y1 - text_h - 5), (x1 + text_w, y1), color, -1
                )
    ...
    def run(self, image: np.ndarray, **kwargs) -> np.ndarray:
        ...
        try:
            ...
            self.logger.info(
                f"Starting content moderation with threshold: {self.safety_threshold}"
            )
            ...
            self.logger.info(
                f"Content analysis completed in {analysis_time:.2f} seconds"
            )
            ...
            safe = analysis.get("safe_for_marketplace", True)
            self.logger.info(
                f"Content moderation result: Safe={safe}, Score={safety_score:.2f}"
            )
