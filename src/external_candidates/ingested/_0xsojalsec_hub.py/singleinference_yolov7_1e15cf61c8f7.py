# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\singleinference_yolov7.py
import logging
import os
import platform
import random
...
class SingleInference_YOLOV7:
    ...
    def load_model(self):
        ...
        self.model = self.attempt_load(
            self.path_yolov7_weights, map_location=self.device
        )  # load FP32 model
    ...
    def load_cv2mat(self, im0=None):
        ...
        if type(self.im0) != type(None):
            ...
            self.im = (
                self.im.half() if self.half else self.im.float()
            )  # uint8 to fp16/32
        ...
        if type(self.im) != type(None):
            ...
            if self.device.type == "cpu" and self.path_yolov7_weights is None:
                ...
                self.outputs = torch.from_numpy(
                    self.open_vino_model(self.im)[self.outputs]
                )

            else:
                if not self.path_yolov7_weights.endswith(
                    "safety.pt"
                ) and not self.path_yolov7_weights.endswith("yolov7-tiny.pt"):
                    ...
                    for j, (*bboxes, score, cls_id) in enumerate(reversed(det)):
                        ...
                        if model_object_map is not None:
                            ...
                            self.predicted_bboxes_PascalVOC.append(
                                [label, x0, y0, x1, y1, score]
                            )  # PascalVOC annotations
                        else:
                            self.predicted_bboxes_PascalVOC.append(
                ...
                if name == "Person":
                    ...
                    for object in self.predicted_bboxes_PascalVOC:
                        ...
                        if object_name != "Person":
                            ...
                            # print(name)
                    # print(person)
                    color_green = self.should_color(
                        hardhats, vests, masks, "Hardhat", "Safety Vest", "Mask", name
                    )
                    # print(no_hardhat,no_vest,no_mask)
                    color_red = self.may_color(
                        no_hardhats,
                        no_vests,
                        no_masks,
                        "NO-Hardhat",
        ...
        if len(self.predicted_bboxes_PascalVOC) > 0:
            for item in self.predicted_bboxes_PascalVOC:
                ...
                if name == "Person":
                    ...
                    # print(result)
                    self.draw_box(
                        color_image, px0, py0, px1, py1, self.ratio, self.dwdh, color
                    )
                # prob = str(round(100*item[-1],2))
        # st.image(img, caption='Input Image', use_column_width=True)
        # st.write(os.listdir())
    ...
    def detect_zone(
        self, img, poly, persons, machines, vehicles, inclusion, max_number_allowed
    ):
        red_color = (255, 0, 0)
        self.load_cv2mat(img)
        # print("image loaded ...")
        ...
        if len(self.predicted_bboxes_PascalVOC) > 0:
            for item in self.predicted_bboxes_PascalVOC:
                ...
                name = str(item[0])
                color = (0, 255, 0)  # green
                person_count = functools.reduce(
                    lambda x, y: x + 1 if str(y[0]) == "Person" else x,
                    self.predicted_bboxes_PascalVOC,
                    0,
                )
                machine_count = functools.reduce(
                    lambda x, y: x + 1 if str(y[0]) == "machinery" else x,
                    self.predicted_bboxes_PascalVOC,
                    0,
                )
                vehicle_count = functools.reduce(
                    lambda x, y: x + 1 if str(y[0]) == "vehicle" else x,
                    self.predicted_bboxes_PascalVOC,
                    0,
                )
                if persons and name == "Person":
                    px0, py0, px1, py1 = self.scale_coords_box(
                        item[1], item[2], item[3], item[4], self.ratio, self.dwdh
                    )
                    if (
                        self.does_intersect_poly(px0, py0, px1, py1, poly)
                        and person_count > max_number_allowed
                    ):
                        if not inclusion:
                            color = red_color  # red
                    elif inclusion:
                ...
                if machines and name == "machinery":
                    ...
                    if (
                        self.does_intersect_poly(px0, py0, px1, py1, poly)
                        and machine_count > max_number_allowed
                    ):
                        if not inclusion:
                            color = red_color  # red
                    elif inclusion:
                ...
                if vehicles and name == "vehicle":
                    ...
                    if (
                        self.does_intersect_poly(px0, py0, px1, py1, poly)
                        and vehicle_count > max_number_allowed
                    ):
                        if not inclusion:
                            color = red_color  # red
                    elif inclusion:
