# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\Configure_Camera.py
import platform
import queue
...
if check_password():
    ...
    def get_detector_model():
        ...
        if multi_inputdevice == "cpu" and CPU:
            ...
            model.colors = [
                [random.randint(0, 255) for _ in range(3)] for _ in model.names
            ]
        else:
            model = SingleInference_YOLOV7(
                img_size=IMG_SIZE,
    ...
    data = None
    policy_json = st.file_uploader(
        "Upload the safety policy file (e.g. ppe_policy.json)",
        type=["json"],
        key="policy",
    )
    ...
    st.write(
        "_Model currently in use is " + MODEL_WEIGHTS + ", you can preview it below._"
    )
    camera_id = st.text_input(
        "Enter the camera id or the full path to the video file", "./videos/video_2.mp4"
    )
            ...
            if n == 4:  # read every 4th frame
                ...
                if data is not None:
                    if data["type"] == "ppe_detection":
                        hardhats, vests, masks = (
                            data["hardhats"],
                            data["vests"],
                            data["masks"],
                        )
                        no_hardhats, no_vests, no_masks = (
                            data["no_hardhats"],
                            data["no_vests"],
                            data["no_masks"],
                        )
                        annotated_image = yolov7_detector.detect_ppe(
                            image,
                            hardhats,
                            vests,
                            masks,
                        ...
                        annotated_image = yolov7_detector.detect_proximity(
                            image, machinery, vehicles
                        )
                    elif data["type"] == "exclusion_zones":
                        persons, machinery, vehicles, max_number_allowed = (
                            data["persons"],
                            data["machinery"],
                            data["vehicle"],
                            data["max_allowed"],
                            ...
                            if isinstance(paths[0], list):
                                for pt in paths[0]:
                                    if pt[0] != "z":
                                        ...
                                        poly.append([x0, y0])
                        annotated_image = yolov7_detector.detect_zone(
                            image,
                            poly,
                            persons,
                            machinery,
                            ...
                            if isinstance(paths[0], list):
                                for pt in paths[0]:
                                    if pt[0] != "z":
                                        ...
                                        y0 = y0 / 400 * image.shape[0]
                                        poly.append([x0, y0])
                        zone_image = yolov7_detector.detect_zone(
                            image,
                            poly,
                            persons,
                            machinery,
                ...
                if update_source:
                    ...
                    now = datetime.now()
                    policy_file_name = (
                        "policy_" + now.strftime("%d_%m_%Y_%H_%M_%S") + ".json"
                    )
                    dict = {
                        "url": url,
