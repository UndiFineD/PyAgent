# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\safety_app.py
import functools
...
def detect_zone(
    img, box_list, poly, persons, machines, vehicles, inclusion, max_number_allowed
):
    if poly is None:
    ...
    if len(predicted_bboxes_PascalVOC) > 0:
        for item in predicted_bboxes_PascalVOC:
            ...
            if persons and name == "Person":
                px0, py0, px1, py1 = item[1], item[2], item[3], item[4]
                if (
                    does_intersect_poly(px0, py0, px1, py1, poly)
