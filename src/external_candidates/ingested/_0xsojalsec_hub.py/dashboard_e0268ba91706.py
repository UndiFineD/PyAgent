# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\dashboard.py
import base64
import io
import json
...
# Header
header = html.H4("Dashboard", className="bg-info text-white p-2 mb-2 text-center")
...
def update_graph(indicator, cam_number, data):
    if cam_number == None:
        ...
        df = pd.DataFrame(
            ...
            columns=["Camera_no", "Date", "Timings"],
        )
            ...
            for cam in cam_number:
                ...
                for date in columns:
                    number_of_violations = (
                        df_int[df_int["Date"] == date].count().Timings
                    )
...
def updateDates(cam_number):
    if cam_number == None:
        ...
        return [
            {"label": name, "value": name} for name in sorted(set(dates), reverse=True)
        ]
...
date_dropdown = html.Div(
    [
        ...
        dcc.Dropdown(
            id="dates",
            options=[
                {"label": name, "value": name} for name in sorted(updateDates(None))
            ],
            clearable=False,
        ),
    ],
                    ...
                    if dating == date:
                        ...
                        for timing in sorted(violations[cam][dating], reverse=True):
                            ...
                            if prev_time - total_seconds > violation_length:
                                image_path = (
                                    f"{OUTPUT_PATH}/{cam}/{dating}/{timing}.jpg"
                                )
                                with open(image_path, "rb") as image_file:
                                    ...
                                    img_data = "{}{}".format(
                                        "data:image/jpg;base64, ", img_data
                                    )
                                # print(image_path)
                                violation_type = get_violation(CONFIG_PATH, cam)
                                images.append(
                                    create_card(img_data, timing, violation_type)
                                )
...
def serve_layout():
    return dbc.Container(
        [
            ...
            dbc.Row(
                [
                    dbc.Col([dbc.Card(date_dropdown)], width=4),
                    dbc.Col([image_table], width=8),
                ]
            ),
