# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\pages\6_Browse_Videos.py
import os
        ...
        try:
            for item in os.listdir(folder_path):
                ...
                if os.path.isdir(item_path):
                    subfolders.append(
                        {"name": item, "path": os.path.normpath(item_path)}
                    )
                else:
            ...
            if os.path.isfile(new_sub):
                with open(new_sub, "rb") as f:
                    ...
                    st.sidebar.download_button(
                        "Download " + new_sub, f, file_name=os.path.basename(new_sub)
                    )
