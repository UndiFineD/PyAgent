# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\pages\1_PPE_Detection.py
import json
...
if check_password():
    ...
    st.sidebar.markdown(
        "Configure PPE detection, detect all persons (not) wearing hardhats, safety vests and masks."
    )
    ...
    # MODEL_WEIGHTS = "./weights/yolov7-tiny-construction-custom.pt"
    IMG_SIZE = 640

    DEFAULT_CONFIDENCE_THRESHOLD = 0.25
    DEFAULT_OVERLAP_THRESHOLD = 0.45
    DEFAULT_IMG = "./images/ppe-example1.jpeg"
    ...
    image = st.file_uploader(
        "Upload an image (above image will be used as default)",
        type=["jpg", "jpeg"],
        key="ppe",
    )
    ...
    with col1:
        st.subheader("PPE Definition (Green):")
        hardhats = st.checkbox("Wearing a hardhat", True)
        vests = st.checkbox("Wearing a vest", True)
        masks = st.checkbox("Wearing a mask")

    with col2:
        st.subheader("PPE Violation (Red):")
        no_hardhats = st.checkbox("Not wearing a hardhat", True)
        no_vests = st.checkbox("Not wearing a vest")
        no_masks = st.checkbox("Not wearing a mask")
    ...
    if predictions:
        ...
        img = asarray(img)
        color_image = yolov7_detector.detect_ppe(
            img, hardhats, vests, masks, no_hardhats, no_vests, no_masks
        )
