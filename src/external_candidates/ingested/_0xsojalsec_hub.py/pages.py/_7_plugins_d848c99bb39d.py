# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\pages\7_Plugins.py
import json
...
if check_password():
    ...
    default_index = (
        plugin_options.index("yolo_detector")
        if "yolo_detector" in plugin_options
        else 0
    )

    # File uploader
    image = st.file_uploader(
        "Upload an image", type=["jpg", "jpeg", "png"], key="plugin_image"
    )
    ...
    if cache_key not in st.session_state:
        yolov7_detector = get_detector_model(
            DEFAULT_CONFIDENCE_THRESHOLD, DEFAULT_OVERLAP_THRESHOLD
        )
    ...
    def get_plugin_params(selected_plugin):
        ...
        if selected_plugin == "yolo_detector":
            ...
            if (
                confidence_threshold != current_model.conf_thres
                or overlap_threshold != current_model.iou_thres
            ):
                yolov7_detector = get_detector_model(
        ...
        # Add plugin-specific parameters
            ...
            with col1:
                plugin_kwargs["debug"] = st.checkbox(
                    "Enable Debug Mode", help="Save intermediate detection results"
                )
            with col2:
    ...
    if process:
        if image is None:
            ...
            with st.spinner("Processing image..."):
                try:
                    processed_image = plugin_manager.run_plugin(
                        selected_plugin, img_array, **plugin_kwargs
                    )
                    ...
                    if selected_plugin == "agent_detector" and plugin_kwargs.get(
                        "debug"
                    ):
