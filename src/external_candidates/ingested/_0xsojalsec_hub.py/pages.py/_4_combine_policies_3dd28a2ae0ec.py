# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hub\pages\4_Combine_Policies.py
import json
...
if check_password():
    ...
    policy_json_1 = st.file_uploader(
        "Upload the first safety policy file (e.g. ppe_policy.json)",
        type=["json"],
        key="policy1",
    )
    policy_json_2 = st.file_uploader(
        "Upload the second safety policy file (e.g. exclusion_policy.json)",
        type=["json"],
        key="policy2",
    )
    ...
    if policy_json_1 is not None and policy_json_2 is not None:
        ...
        if data_1 is not None and data_2 is not None:
            if (
                data_1["type"] == "ppe_detection"
                and data_2["type"] == "exclusion_zones"
            ):
                policy = {
                ...
                st.download_button(
                    ...
                    file_name="ppe_detection_exclusion_zones.json",
                    mime="text/plain",
                )
            else:
                st.write(
                    "Only ppe_detection.json and exclusion_policy.json can be combined at the moment."
                )
