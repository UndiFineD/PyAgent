...
class PSTree:
    ...
    def extract_data(self):
        ...
        matches = [
            match.group() for match in var_pattern.finditer(self.current.content)
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--outfile", required=True, type=str, default=None, help="Output file"
    )
    ...
    for n, line in enumerate(content, start=1):
        ...
        if tree.current_ctx_type in [
            ...
            function_match = re.search(
                r"(filter|function)\s+([\w][^\s]+)[\{\s]?", line, re.IGNORECASE
            )
            ...
            if function_match:
                ...
                if tree.current_ctx_type == PSContextType.MAIN:
                    ...
                    tree.change_context(
                        ctx_name=function, ctx_type=PSContextType.FUNCTION
                    )
                    see_next = True
                elif tree.current_ctx_type == PSContextType.FUNCTION:
                    print(f"Found new nested function {function} at line: {n}")
                    tree.change_context(
                        ctx_name=function, ctx_type=PSContextType.NESTED_FUNCTION
                    )
                    see_next = True
                ob = line.count("{")
                if ob == 0:
                ...
                tree.change_context(
                    ctx_name=tree.current.name + "-param", ctx_type=PSContextType.PARAMS
                )
                ob = line.count("(")
                cb = line.count(")")
                if ob == 0:
