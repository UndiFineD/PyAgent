    ...
    for epoch in range(1, hparams.epochs + 1):
        ...
        for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset)):
            ...
            val_loader = DataLoader(
                dataset, batch_size=batch_size, sampler=SubsetRandomSampler(val_ids)
            )
            ...
            # train and validate
            train_dataloader(
                model, train_loader, optimizer, fabric, epoch, hparams, fold
            )
            epoch_acc += validate_dataloader(
                model, val_loader, fabric, hparams, fold, acc_metric
            )
            acc_metric.reset()
