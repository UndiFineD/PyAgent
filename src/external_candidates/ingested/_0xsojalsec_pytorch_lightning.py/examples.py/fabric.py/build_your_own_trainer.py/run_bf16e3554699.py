...
class MNISTModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        accuracy_train = accuracy(
            logits.argmax(-1), y, num_classes=10, task="multiclass", top_k=1
        )
...
def train(model):
    ...
    train_set = MNIST(
        root="/tmp/data/MNIST", train=True, transform=ToTensor(), download=True
    )
    val_set = MNIST(
        root="/tmp/data/MNIST", train=False, transform=ToTensor(), download=False
    )
