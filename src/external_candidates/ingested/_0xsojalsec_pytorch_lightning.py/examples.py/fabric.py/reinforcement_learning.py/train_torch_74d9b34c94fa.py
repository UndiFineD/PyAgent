# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\examples\fabric\reinforcement_learning\train_torch.py
"""
    ...
    for epoch in range(args.update_epochs):
        ...
        for batch_idxes in sampler:
            _, newlogprob, entropy, newvalue = agent(
                data["obs"][batch_idxes], data["actions"].long()[batch_idxes]
            )
        ...
        if logger is not None:
            per_epoch_losses = per_epoch_losses / (
                len(sampler) * distributed.get_world_size()
            )
...
def main(args: argparse.Namespace):
    ...
    agent: PPOAgent = PPOAgent(
        envs, act_fun=args.activation_function, ortho_init=args.ortho_init
    ).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        global_stats = torch.tensor(
            [local_rew, local_ep_len, local_num_episodes],
            device=device,
            dtype=torch.float32,
        )
        ...
        if global_rank == 0 and global_stats[2] != 0.0:
            logger.add_scalar(
                "Rewards/rew_avg", global_stats[0] / global_stats[2], global_step
            )
            logger.add_scalar(
                "Game/ep_len_avg", global_stats[1] / global_stats[2], global_step
            )
            ...
            for i in range(1, len(gathered_data)):
                for k in processed_gathered_data:
                    processed_gathered_data[k] = torch.cat(
                        (
                            processed_gathered_data[k].to(device),
                            gathered_data[i][k].to(device),
                        ),
                        dim=0,
                    )
        else:
