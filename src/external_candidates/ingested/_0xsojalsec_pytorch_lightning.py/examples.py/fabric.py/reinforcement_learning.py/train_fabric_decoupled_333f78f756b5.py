# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\examples\fabric\reinforcement_learning\train_fabric_decoupled.py
"""
...
@torch.no_grad()
def player(
    args, world_collective: TorchCollective, player_trainer_collective: TorchCollective
):
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(
                args.env_id, args.seed + i, 0, args.capture_video, log_dir, "train"
            )
    ...
    torch.nn.utils.convert_parameters.vector_to_parameters(
        flattened_parameters, agent.parameters()
    )
    ...
    if not args.share_data:
        ...
        chunks_sizes = [
            ...
            for chunk in torch.tensor_split(
                torch.arange(single_global_step), world_collective.world_size - 1
            )
        ]
    ...
    for _ in range(1, num_updates + 1):
        ...
        fabric.log_dict(
            {"Time/step_per_second": int(global_step / (time.time() - start_time))},
            global_step,
        )
    ...
    if args.share_data:
        ...
        world_collective.scatter_object_list(
            [None], [None] + [-1] * (world_collective.world_size - 1), src=0
        )
    envs.close()
...
def trainer(
    ...
    while True:
        ...
        if args.share_data:
            ...
            world_collective.scatter_object_list(
                data, [None for _ in range(world_collective.world_size)], src=0
            )
        data = data[0]
        if data == -1:
            return
            ...
            for epoch in range(args.update_epochs):
                ...
                for batch_idxes in sampler:
                    loss = agent.training_step(
                        {k: v[batch_idxes].to(device) for k, v in data.items()}
                    )
...
def main(args: argparse.Namespace):
    ...
    world_collective.setup(
        backend="nccl" if args.player_on_gpu and args.cuda else "gloo"
    )
    ...
    optimization_pg = world_collective.new_group(
        ranks=list(range(1, world_collective.world_size))
    )
    if global_rank == 0:
