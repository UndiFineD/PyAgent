# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\examples\fabric\fp8_distributed_transformer\train.py
...
def configure_model(model: nn.Module, device_mesh: DeviceMesh) -> nn.Module:
    ...
    for module in model.modules():
        if isinstance(
            module, (torch.nn.TransformerEncoderLayer, torch.nn.TransformerDecoderLayer)
        ):
...
def train():
    ...
    strategy = ModelParallelStrategy(
        data_parallel_size=4, tensor_parallel_size=1, parallelize_fn=configure_model
    )
