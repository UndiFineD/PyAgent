# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\examples\pytorch\tensor_parallel\train.py
...
class Llama3(L.LightningModule):
    ...
    def train_dataloader(self):
        dataset = RandomTokenDataset(
            vocab_size=self.model_args.vocab_size, seq_length=128
        )
...
def train():
    ...
    trainer.print(
        f"Number of model parameters: {sum(p.numel() for p in model.parameters()) / 1e9:.1f} B"
    )
...
if __name__ == "__main__":
    ...
    ), "This example requires at least 4 GPUs with 24 GB of memory each."
