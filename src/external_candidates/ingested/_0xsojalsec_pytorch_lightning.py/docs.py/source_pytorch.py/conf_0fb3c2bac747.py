# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\docs\source-pytorch\conf.py
#
...
assist_local = _load_py_module(
    "assistant", os.path.join(_PATH_ROOT, ".actions", "assistant.py")
)
if os.path.isdir(_PATH_RAW_NB_ACTIONS):
    assist_nb = _load_py_module(
        "assistant", os.path.join(_PATH_RAW_NB_ACTIONS, "assistant.py")
    )
else:
...
if _PIN_RELEASE_VERSIONS:
    ...
    adjust_linked_external_docs(
        ...
        "https://pytorch.org/docs/{torch.__version__}/",
        _PATH_ROOT,
    )
    adjust_linked_external_docs(
...
html_theme_path = [
    os.environ.get("LIT_SPHINX_PATH", lai_sphinx_theme.get_html_theme_path())
]
...
nitpick_ignore = [
    ...
    (
        "py:class",
        "lightning.fabric.plugins.precision.transformer_engine.TransformerEnginePrecision",
    ),
    (
        "py:class",
        "lightning.fabric.plugins.precision.bitsandbytes.BitsandbytesPrecision",
    ),
    ...
    (
        "py:meth",
        "lightning.pytorch.accelerators.accelerator.Accelerator.register_accelerators",
    ),
    (
        "py:paramref",
        "lightning.pytorch.callbacks.Checkpoint._sphinx_paramlinks_save_top_k",
    ),
    ...
    (
        "py:meth",
        "lightning.pytorch.callbacks.progress.progress_bar.ProgressBar.get_metrics",
    ),
    (
        "py:class",
        "lightning.pytorch.callbacks.progress.rich_progress.RichProgressBarTheme",
    ),
    ...
    (
        "py:meth",
        "lightning.pytorch.core.mixins.hparams_mixin.HyperparametersMixin.save_hyperparameters",
    ),
    ("py:class", "lightning.pytorch.loggers.Logger"),
    ("py:func", "lightning.pytorch.loggers.logger.rank_zero_experiment"),
    (
        "py:class",
        "lightning.pytorch.plugins.environments.cluster_environment.ClusterEnvironment",
    ),
    (
        "py:class",
        "lightning.pytorch.plugins.environments.slurm_environment.SLURMEnvironment",
    ),
    ...
    (
        "py:class",
        "lightning.pytorch.serve.servable_module_validator.ServableModuleValidator",
    ),
...
if _SPHINX_MOCK_REQUIREMENTS:
    _path_require = lambda fname: os.path.join(
        _PATH_ROOT, "requirements", "pytorch", fname
    )
