# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\.actions\assistant.py
...
class _RequirementWithComment(Requirement):
    ...
    def __init__(
        ...
        *args: Any,
        comment: str = "",
        pip_argument: Optional[str] = None,
        **kwargs: Any,
    ) -> None:
        super().__init__(*args, **kwargs)
        self.comment = comment
        assert (
            pip_argument is None or pip_argument
        )  # sanity check that it's not an empty str
...
def load_requirements(
    path_dir: str, file_name: str = "base.txt", unfreeze: str = "all"
) -> list[str]:
...
def load_readme_description(path_dir: str, homepage: str, version: str) -> str:
    ...
    text = text.replace(
        "docs/source-pytorch/_static/",
        f"{os.path.join(github_source_url, 'docs/source-app/_static/')}",
    )
...
def _load_aggregate_requirements(
    req_dir: str = "requirements", freeze_requirements: bool = False
) -> None:
    ...
    requires = [
        ...
        if os.path.isdir(d)
        and len(glob.glob(os.path.join(d, "*"))) > 0
    ...
    for source_import, target_import in mapping:
        for i, ln in enumerate(out):
            ...
            if lightning_by:  # in addition, replace base package
                out[i] = out[i].replace(
                    "from lightning import ", f"from {lightning_by} import "
                )
...
def copy_replace_imports(
    ...
    for fp in ls:
        ...
        lines = _replace_imports(
            lines, list(zip(source_imports, target_imports)), lightning_by=lightning_by
        )
...
def create_mirror_package(source_dir: str, package_mapping: dict[str, str]) -> None:
    ...
    mapping.pop(
        "lightning", None
    )  # pop this key to avoid replacing `lightning` to `lightning.lightning`
...
class AssistantCLI:
    ...
    def pull_docs_files(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Requesting file '{zip_url}' does not exist or it is just unavailable."
                )
            ...
            zip_dirs = [
                d for d in glob.glob(os.path.join(tmp, "*")) if os.path.isdir(d)
            ]
            ...
            if single_page:  # special case for copying single page
                ...
                assert os.path.isfile(
                    single_page
                ), f"File '{single_page}' does not exist."
            ...
            for rst in ls_pages:
                rel_rst = rst.replace(
                    os.path.join(repo_dir, source_dir) + os.path.sep, ""
                )
                rel_dir = os.path.dirname(rel_rst)
                os.makedirs(
                    os.path.join(_PROJECT_ROOT, target_dir, rel_dir), exist_ok=True
                )
                new_rst = os.path.join(_PROJECT_ROOT, target_dir, rel_rst)
                if os.path.isfile(new_rst):
                    logging.warning(
                        f"Page {new_rst} already exists in the local tree so it will be skipped."
                    )
                    continue
    ...
    def generate_docker_tags(
        ...
        if release_version:
            tags += [
                f"{release_version}-py{python_version}-torch{torch_version}-cuda{cuda_version}"
            ]
        if add_latest:
            tags += ["latest"]
    ...
    def prune_pytest_as_errors(
        ...
        errors: tuple = ("FutureWarning", "DeprecationWarning"),
    ) -> None:
        ...
        filterwarnings = [
            wrn
            for wrn in filterwarnings
            if not any(f"error::{err}" in wrn for err in errors)
        ]
