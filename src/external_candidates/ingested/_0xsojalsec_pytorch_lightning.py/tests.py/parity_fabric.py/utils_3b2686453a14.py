...
def is_state_dict_equal(state0, state1):
    return all(
        torch.equal(w0.cpu(), w1.cpu())
        for w0, w1 in zip(state0.values(), state1.values())
    )
...
def is_cuda_memory_close(memory_stats_torch, memory_stats_fabric):
    ...
    return (
        memory_stats_torch["allocated_bytes.all.peak"]
        >= memory_stats_fabric["allocated_bytes.all.peak"]
    )
