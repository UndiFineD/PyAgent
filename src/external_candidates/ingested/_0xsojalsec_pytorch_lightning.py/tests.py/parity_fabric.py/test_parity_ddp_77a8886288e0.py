# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\parity_fabric\test_parity_ddp.py
...
def train_torch_ddp(
    ...
    dataloader = DataLoader(
        dataloader.dataset, sampler=sampler, batch_size=model.batch_size
    )
...
def run_parity_test(
    accelerator: str = "cpu", devices: int = 2, tolerance: float = 0.02
):
    cuda_reset()
    ...
    assert all(
        fabric.all_gather(is_state_dict_equal(state_dict_torch, state_dict_fabric))
    )
    ...
    assert all(
        fabric.all_gather(
            is_timing_close(
                timings_torch, timings_fabric, rtol=tolerance, atol=tolerance
            )
        )
    )
    ...
    if accelerator == "cuda":
        assert all(
            fabric.all_gather(
                is_cuda_memory_close(memory_torch["start"], memory_fabric["start"])
            )
        )
        assert all(
            fabric.all_gather(
                is_cuda_memory_close(memory_torch["end"], memory_fabric["end"])
            )
        )
