...
def test_tensorboard_log_sub_dir(tmp_path):
    ...
    logger = TensorBoardLogger(
        root_dir, name="name", version="version", sub_dir="sub_dir"
    )
...
def test_tensorboard_expand_home():
    ...
    assert logger.log_dir == os.path.join(
        explicit_root_dir, "name", "version", "sub_dir"
    )
...
def test_tensorboard_log_graph_with_batch_transfer_hooks(tmp_path, example_input_array):
    ...
    with (
        mock.patch.object(
            model, "_on_before_batch_transfer", return_value=example_input_array
        ) as before_mock,
        mock.patch.object(
