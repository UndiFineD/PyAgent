# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\accelerators\test_cuda.py
...
def test_force_nvml_based_cuda_check():
    ...
    importlib.reload(
        lightning.fabric
    )  # reevaluate top-level code, without becoming a different object
...
def test_tf32_message(_, __, ___, caplog, monkeypatch):
    ...
    monkeypatch.setattr(
        lightning.fabric.accelerators.cuda, "rank_zero_info", logging.info
    )
...
def test_find_usable_cuda_devices_error_handling():
    ...
    with (
        ...
        pytest.raises(
            ValueError,
            match="You requested to find 2 devices but there are no visible CUDA",
        ),
    ):
    ...
    with (
        mock.patch(
            "lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=2
        ),
        mock.patch("lightning.fabric.accelerators.cuda.torch.tensor", tensor_mock),
        pytest.raises(
            RuntimeError,
            match=escape("The devices [0, 1] are occupied by other processes"),
        ),
    ):
        find_usable_cuda_devices(2)
    ...
    with (
        mock.patch(
            "lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=5
        ),
