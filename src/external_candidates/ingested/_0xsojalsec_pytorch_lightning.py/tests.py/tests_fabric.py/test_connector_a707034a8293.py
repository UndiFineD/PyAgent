# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\test_connector.py
...
@pytest.mark.parametrize(
    ("accelerator", "devices"),
    [("tpu", "auto"), ("tpu", 1), ("tpu", [1]), ("tpu", 8), ("auto", 1), ("auto", 8)],
)
...
def test_custom_accelerator(*_):
    ...
    strategy = TestStrategy(
        device=torch.device("cpu"), accelerator=Accel(), precision=Prec()
    )
...
@mock.patch(
    "pytorch_lightning_enterprise.plugins.environments.lsf.LSFEnvironment._read_hosts",
    return_value=["node0", "node1"],
)
@mock.patch(
    "pytorch_lightning_enterprise.plugins.environments.lsf.LSFEnvironment._get_node_rank",
    return_value=0,
)
def test_fallback_from_ddp_spawn_to_ddp_on_cluster(
    _, __, env_vars, expected_environment
):
...
def test_interactive_incompatible_backend_error(_, monkeypatch):
    ...
    with pytest.raises(
        RuntimeError, match=r"strategy='ddp_spawn'\)`.*is not compatible"
    ):
...
@mock.patch("lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=2)
@mock.patch(
    "lightning.fabric.accelerators.mps.MPSAccelerator.is_available", return_value=False
)
...
def test_invalid_ddp_strategy_with_mps(accelerator, strategy, strategy_class):
    with pytest.raises(
        ValueError, match="strategies from the DDP family are not supported"
    ):
...
def test_strategy_choice_multi_node_gpu(_, strategy, strategy_class, devices):
    connector = _Connector(
        num_nodes=2, accelerator="gpu", strategy=strategy, devices=devices
    )
...
@mock.patch(
    "pytorch_lightning_enterprise.accelerators.xla._using_pjrt", return_value=True
)
def test_tpu_accelerator_can_not_run_on_system(_):
    with pytest.raises(
        RuntimeError, match="XLAAccelerator` can not run on your system"
    ):
...
@mock.patch("lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=2)
@pytest.mark.parametrize(
    "device_count", [["0"], [0, "1"], ["GPU"], [["0", "1"], [0, 1]], [False]]
)
def test_accelerator_invalid_type_devices(_, device_count):
    with pytest.raises(
        TypeError, match=r"must be an int, a string, a sequence of ints, but you"
    ):
...
@pytest.mark.parametrize(
    ("devices", "strategy_class"), [(1, SingleDeviceStrategy), (5, DDPStrategy)]
)
...
@pytest.mark.parametrize(
    ("devices", "strategy_class"),
    [(1, SingleDeviceStrategy), ([1], SingleDeviceStrategy), (2, DDPStrategy)],
)
def test_accelerator_gpu_with_devices(devices, strategy_class):
    connector = _Connector(accelerator="gpu", devices=devices)
    assert (
        len(connector._parallel_devices) == len(devices)
...
def test_unsupported_strategy_types_on_cpu_and_fallback():
    with pytest.warns(
        UserWarning, match="is not supported on CPUs, hence setting `strategy='ddp"
    ):
...
def test_invalid_accelerator_choice():
    with pytest.raises(
        ValueError,
        match="You selected an invalid accelerator name: `accelerator='cocofruit'`",
    ):
...
def test_precision_conversion(
    patch1, patch2, precision, expected_precision, should_warn
):
...
@pytest.mark.parametrize(
    ...
    [("some_name", SLURMEnvironment), ("bash", LightningEnvironment)],
)
...
def test_unsupported_tpu_choice(_, tpu_available):
    ...
    with pytest.raises(
        ValueError,
        match="XLAAccelerator` can only be used with a `SingleDeviceXLAStrategy`",
    ):
    ...
    strategy = DDPStrategy(
        accelerator=XLAAccelerator(), precision=XLAPrecision(precision="16-true")
    )
    with pytest.raises(
        ValueError,
        match="XLAAccelerator` can only be used with a `SingleDeviceXLAStrategy`",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([torch.device("cuda", i) for i in range(8)], "tpu"),
    ],
)
def test_parallel_devices_in_strategy_conflict_with_accelerator(
    parallel_devices, accelerator
):
    with pytest.raises(ValueError, match=r"parallel_devices set through"):
        _Connector(
            strategy=DDPStrategy(parallel_devices=parallel_devices),
...
def test_gpu_accelerator_backend_choice(
    expected_accelerator_flag, expected_accelerator_class
):
    connector = _Connector(accelerator="gpu")
...
@mock.patch(
    "lightning.fabric.accelerators.mps.MPSAccelerator.is_available", return_value=True
)
@mock.patch(
    "lightning.fabric.accelerators.mps._get_all_available_mps_gpus", return_value=[0]
)
...
@mock.patch(
    "lightning.fabric.accelerators.cuda.CUDAAccelerator.is_available",
    return_value=False,
)
...
def test_ddp_fork_on_unsupported_platform(_, __, strategy):
    with pytest.raises(
        ValueError, match="process forking is not supported on this platform"
    ):
        _Connector(strategy=strategy)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "deepspeed",
            DeepSpeedPrecision,
            marks=RunIf(deepspeed=True, mps=False),
        ),
        pytest.param(
            "16-true",
            "deepspeed",
...
def test_precision_selection_model_parallel(_, precision, raises):
    error_context = (
        pytest.raises(ValueError, match=f"does not support .*{precision}")
        if raises
        else nullcontext()
    )
    with error_context:
...
def test_bitsandbytes_precision_cuda_required(monkeypatch):
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.precision.bitsandbytes._BITSANDBYTES_AVAILABLE",
        True,
    )
    monkeypatch.setitem(sys.modules, "bitsandbytes", Mock())
    with pytest.raises(
        RuntimeError, match="Bitsandbytes is only supported on CUDA GPUs"
    ):
        _Connector(accelerator="cpu", plugins=BitsandbytesPrecision(mode="int8"))


@pytest.mark.parametrize(
    ("strategy", "strategy_cls"), [("DDP", DDPStrategy), ("Ddp", DDPStrategy)]
)
@mock.patch(
    "lightning.fabric.accelerators.mps.MPSAccelerator.is_available", return_value=False
)
...
def test_precision_from_environment(_, precision, expected):
    ...
    with mock.patch.dict(os.environ, env_vars):
        connector = _Connector(
            accelerator="cuda"
        )  # need to use cuda, because AMP not available on CPU
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "mps", None, MPSAccelerator, SingleDeviceStrategy, marks=RunIf(mps=True)
        ),
        pytest.param(
            "cuda",
            "dp",
        ...
        pytest.param(
            ...
            CUDAAccelerator,
            DeepSpeedStrategy,
            marks=RunIf(min_cuda_gpus=1, deepspeed=True),
        ),
    ],
)
def test_accelerator_strategy_from_environment(
    accelerator, strategy, expected_accelerator, expected_strategy
):
...
def test_devices_from_environment(*_):
    ...
    with mock.patch.dict(
        os.environ, {"LT_DEVICES": "2", "LT_NUM_NODES": "3", "LT_CLI_USED": "1"}
    ):
...
def test_arguments_from_environment_collision():
    ...
    with (
        ...
        pytest.raises(
            ValueError,
            match="`Fabric\\(accelerator='cuda', ...\\)` but .* `--accelerator=cpu`",
        ),
    ):
        _Connector(accelerator="cuda")
    ...
    with (
        ...
        pytest.raises(
            ValueError,
            match="`Fabric\\(strategy='ddp_spawn', ...\\)` but .* `--strategy=ddp`",
        ),
    ):
    ...
    with (
        ...
        pytest.raises(
            ValueError,
            match="`Fabric\\(precision='64-true', ...\\)` but .* `--precision=16-mixed`",
        ),
    ):
...
def test_fsdp_unsupported_on_cpu(_):
    ...
    with pytest.raises(
        ValueError,
        match="You selected the FSDP strategy but FSDP is only available on GPU",
    ):
...
def test_connector_auto_selection(monkeypatch, is_interactive):
    no_cuda = mock.patch(
        "lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=0
    )
    single_cuda = mock.patch(
        "lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=1
    )
    multi_cuda = mock.patch(
        "lightning.fabric.accelerators.cuda.num_cuda_devices", return_value=4
    )
    no_mps = mock.patch(
        "lightning.fabric.accelerators.mps.MPSAccelerator.is_available",
        return_value=False,
    )
    single_mps = mock.patch(
        "lightning.fabric.accelerators.mps.MPSAccelerator.is_available",
        return_value=True,
    )
    ...
    def _mock_interactive():
        monkeypatch.setattr(
            lightning.fabric.utilities.imports, "_IS_INTERACTIVE", is_interactive
        )
        monkeypatch.setattr(
            lightning.fabric.connector, "_IS_INTERACTIVE", is_interactive
        )
        if _IS_WINDOWS:
    ...
    assert isinstance(connector.accelerator, CUDAAccelerator)
    assert isinstance(
        connector.strategy, (SingleDeviceStrategy if is_interactive else DDPStrategy)
    )
    ...
    with no_cuda, no_mps, monkeypatch.context():
        ...
        monkeypatch.setattr(
            lightning.fabric.accelerators.XLAAccelerator,
            "auto_device_count",
            lambda *_: 1,
        )
        monkeypatch.setattr(torch, "device", DeviceMock())
