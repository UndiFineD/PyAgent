...
def test_fabric_module_wraps():
    ...
    assert (
        _FabricModule(wrapped_module, Mock(), original_module=original_module).module
        is original_module
    )
...
def test_fabric_module_attribute_lookup():
    ...
    fabric_module = _FabricModule(
        wrapped_module, Mock(), original_module=original_module
    )
...
def test_fabric_module_method_lookup():
    ...
    original_module = OriginalModule()
    fabric_module = _FabricModule(
        forward_module=original_module, strategy=Mock(), original_module=original_module
    )
...
def test_fabric_module_mark_forward_method():
    ...
    original_module = OriginalModule()
    fabric_module = _FabricModule(
        original_module, Mock(), original_module=original_module
    )
...
def test_fabric_module_state_dict_access():
    ...
    fabric_module.load_state_dict(
        {"layer.weight": weight, "layer.bias": bias}, assign=True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            32, torch.int, torch.int, "gpu", "cuda:0", marks=RunIf(min_cuda_gpus=1)
        ),
        pytest.param(
            16,
            torch.float32,
        ...
        pytest.param(
            16, torch.long, torch.long, "gpu", "cuda:0", marks=RunIf(min_cuda_gpus=1)
        ),
        pytest.param(
            "bf16",
            torch.float32,
        ...
        pytest.param(
            32, torch.float32, torch.float32, "mps", "mps:0", marks=RunIf(mps=True)
        ),
    ],
)
def test_fabric_module_forward_conversion(
    precision, input_type, expected_type, accelerator, device_str
):
...
@pytest.mark.parametrize(
    "compile", [False, pytest.param(True, marks=RunIf(dynamo=True))]
)
...
def test_step_method_redirection():
    ...
    assert (
        fabric_module.training_step("train_arg", kwarg="train_kwarg")
