# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\precision\test_transformer_engine.py
...
def test_transformer_engine_plugin(monkeypatch):
    ...
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.precision.transformer_engine._TRANSFORMER_ENGINE_AVAILABLE",
        True,
    )
    ...
    assert connector.precision is precision
    recipe_mock.DelayedScaling.assert_called_once_with(
        foo=0, fp8_format=recipe_mock.Format.HYBRID
    )
...
def test_convert_module_handles_linear_without_bias(monkeypatch):
    module = (
        pytorch_lightning_enterprise.utils.imports
    )  # Set up mock transformer_engine
    monkeypatch.setattr(module, "_TRANSFORMER_ENGINE_AVAILABLE", True)
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.precision.transformer_engine._TRANSFORMER_ENGINE_AVAILABLE",
        True,
    ...
    monkeypatch.setitem(sys.modules, "transformer_engine", transformer_engine_mock)
    monkeypatch.setitem(
        sys.modules, "transformer_engine.pytorch", transformer_engine_mock.pytorch
    )
    monkeypatch.setitem(
        sys.modules, "transformer_engine.common.recipe", transformer_engine_mock.recipe
    )
