# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\precision\test_fsdp.py
...
def test_fsdp_precision_scaler_with_bf16():
    with pytest.raises(
        ValueError, match="`precision='bf16-mixed'` does not use a scaler"
    ):
...
def test_fsdp_precision_forward_context_bf16():
    ...
    assert precision.scaler is None
    with (
        precision.forward_context()
    ):  # forward context is not using autocast ctx manager
...
def test_invalid_precision_with_fsdp_precision():
    ...
    with pytest.raises(
        ValueError, match="is not supported in FSDP. `precision` must be one of"
    ):
