# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\precision\test_utils.py
import pytest
import torch
...
def test_class_replacement_context_manager():
    ...
    context_manager = _ClassReplacementContextManager(
        {"torch.nn.Linear": MyLinear, "torch.nn.LayerNorm": MyLayerNorm}
    )
    assert context_manager._originals == {
        "torch.nn.Linear": original_linear,
        "torch.nn.LayerNorm": original_layernorm,
    }
    assert context_manager._modules == {
        "torch.nn.Linear": torch.nn,
        "torch.nn.LayerNorm": torch.nn,
    }
