# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\precision\test_amp_integration.py
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "cpu", "bf16-mixed", torch.bfloat16, marks=RunIf(skip_windows=True)
        ),
...
def test_amp(accelerator, precision, expected_dtype):
    fabric = Fabric(
        accelerator=accelerator, precision=precision, devices=2, strategy="ddp_spawn"
    )
...
def test_amp_fused_optimizer_parity():
    def run(fused=False):
        ...
        model = nn.Linear(10, 10).to(
            fabric.device
        )  # TODO: replace with individual setup_module call
