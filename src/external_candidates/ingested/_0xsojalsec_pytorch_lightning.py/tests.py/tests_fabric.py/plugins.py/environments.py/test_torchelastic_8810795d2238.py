# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\environments\test_torchelastic.py
...
def test_attributes_from_environment_variables(caplog):
    ...
    with caplog.at_level(
        logging.DEBUG,
        logger="pytorch_lightning_enterprise.plugins.environments.torchelastic",
    ):
...
def test_validate_user_settings():
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("the product (2 * 2) does not match the world size (8)"),
    ):
