# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\environments\test_mpi.py
...
def test_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        lightning.fabric.plugins.environments.mpi, "_MPI4PY_AVAILABLE", False
    )
    ...
    monkeypatch.setattr(
        lightning.fabric.plugins.environments.mpi, "_MPI4PY_AVAILABLE", True
    )
...
def test_init_local_comm(monkeypatch):
    ...
    with (
        mock.patch.dict("sys.modules", {"mpi4py": mpi4py_mock}),
        mock.patch("socket.gethostname", hostname_mock),
    ):
        env = MPIEnvironment()
