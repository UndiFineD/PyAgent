# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\environments\test_xla.py
...
def test_detect(monkeypatch):
    ...
    monkeypatch.setattr(
        lightning.fabric.accelerators.xla.XLAAccelerator, "is_available", lambda: False
    )
    assert not XLAEnvironment.detect()

    monkeypatch.setattr(
        lightning.fabric.accelerators.xla.XLAAccelerator, "is_available", lambda: True
    )
...
def test_global_rank_from_xla_runtime_greater_2_1(xla_available):
    ...
    with mock.patch(
        "torch_xla.runtime.global_ordinal", return_value=2
    ) as mock_global_ordinal:
...
def test_local_rank_from_xla_runtime_greater_2_1(xla_available):
    ...
    with mock.patch(
        "torch_xla.runtime.local_ordinal", return_value=1
    ) as mock_local_ordinal:
