# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\environments\test_slurm.py
...
def test_attributes_from_environment_variables(caplog):
    ...
    with caplog.at_level(
        logging.DEBUG, logger="pytorch_lightning_enterprise.plugins.environments.slurm"
    ):
...
@pytest.mark.skipif(
    ...
    reason="must run on a machine where srun is not available",
)
def test_srun_available_and_not_used(monkeypatch):
    ...
    with mock.patch(
        "lightning.fabric.plugins.environments.slurm.shutil.which",
...
def test_validate_user_settings():
    ...
    with pytest.raises(
        ValueError, match="the number of tasks per node configured .* does not match"
    ):
        env.validate_settings(num_devices=2, num_nodes=2)

    with pytest.raises(
        ValueError, match="the number of nodes configured in SLURM .* does not match"
    ):
    ...
    with mock.patch(
        "pytorch_lightning_enterprise.plugins.environments.slurm.SLURMEnvironment.job_name",
