# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\plugins\collectives\test_torch_collective.py
import contextlib
import datetime
import os
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "reduce_scatter",
            {
                "output": PASSED_TENSOR,
                "input_list": [PASSED_TENSOR],
                "op": ReduceOp.SUM,
            },
            "output",
        ),
        (
            "all_to_all",
            {
                "output_tensor_list": [PASSED_TENSOR],
                "input_tensor_list": [PASSED_TENSOR],
            },
            "output_tensor_list",
        ),
        ...
        (
            "scatter_object_list",
            {
                "scatter_object_output_list": [PASSED_OBJECT],
                "scatter_object_input_list": [PASSED_OBJECT],
                "src": 0,
            },
...
def test_convert_ops():
    ...
    # Test RedOpType
    assert (
        TorchCollective._convert_to_native_op(ReduceOp.RedOpType.AVG)
        == ReduceOp.RedOpType.AVG
    )
...
def _test_distributed_collectives_fn(strategy, collective):
    ...
    # all_gather
    tensor_list = [
        torch.zeros(2, dtype=torch.long) for _ in range(strategy.num_processes)
    ]
    ...
    expected = (
        torch.tensor(strategy.num_processes) if strategy.global_rank == 0 else this
    )
...
def test_default_process_group():
    collective_launch(
        _test_default_process_group, [torch.device("cpu")] * 3, num_groups=2
    )
