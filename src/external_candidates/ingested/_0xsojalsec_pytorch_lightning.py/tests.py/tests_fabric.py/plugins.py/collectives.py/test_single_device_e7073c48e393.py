...
def test_create_group():
    ...
    with pytest.raises(
        RuntimeError, match=r"SingleDeviceCollective` does not own a group"
    ):
        _ = collective.group

    with mock.patch(
        "lightning.fabric.plugins.collectives.single_device.SingleDeviceCollective.new_group"
    ) as new_mock:
    ...
    with mock.patch(
        "lightning.fabric.plugins.collectives.single_device.SingleDeviceCollective.destroy_group"
    ):
        collective.teardown()
