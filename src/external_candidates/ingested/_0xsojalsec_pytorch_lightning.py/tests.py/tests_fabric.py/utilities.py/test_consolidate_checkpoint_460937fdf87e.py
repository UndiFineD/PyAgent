# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_consolidate_checkpoint.py
...
import pytest
from lightning.fabric.utilities.consolidate_checkpoint import (
    _parse_cli_args,
    _process_cli_args,
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["path/to/checkpoint"],
            {"checkpoint_folder": "path/to/checkpoint", "output_file": None},
        ),
        (
...
def test_process_cli_args(tmp_path, caplog, monkeypatch):
    ...
    with (
        caplog.at_level(
            logging.ERROR, logger="lightning.fabric.utilities.consolidate_checkpoint"
        ),
    ...
    assert (
        "Only FSDP-sharded checkpoints saved with Lightning are supported"
        in caplog.text
    )
    caplog.clear()
