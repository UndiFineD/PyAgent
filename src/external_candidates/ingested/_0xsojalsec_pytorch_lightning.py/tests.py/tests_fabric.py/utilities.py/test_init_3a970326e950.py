# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_init.py
...
def test_materialize_meta_tensors():
    ...
    with pytest.raises(
        TypeError, match="MyModel.reset_parameters` method is implemented"
    ):
...
def test_has_meta_device_parameters_or_buffers():
    ...
    assert _has_meta_device_parameters_or_buffers(
        torch.nn.Sequential(module, meta_module, torch.nn.ReLU())
    )
    ...
    # unsupported objects
    with pytest.raises(
        TypeError, match="Expected `torch.nn.Module` or `torch.optim.Optimizer`"
    ):
