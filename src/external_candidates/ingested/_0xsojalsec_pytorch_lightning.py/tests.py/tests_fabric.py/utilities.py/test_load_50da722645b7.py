# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_load.py
...
def test_materialize_tensors(tmp_path):
    ...
    assert torch.equal(
        materialized["nested"]["list"][0], collection["nested"]["list"][0]
    )
    assert torch.equal(
        materialized["nested"]["list"][1], collection["nested"]["list"][1]
    )
