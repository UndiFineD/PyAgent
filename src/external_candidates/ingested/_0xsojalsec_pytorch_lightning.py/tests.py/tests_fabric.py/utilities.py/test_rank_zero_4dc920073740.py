# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_rank_zero.py
import os
import sys
...
def test_rank_zero_known_environment_variables(env_vars, expected, monkeypatch):
    ...
    with mock.patch.dict(os.environ, env_vars):
        ...
        monkeypatch.delitem(
            sys.modules, "lightning_utilities.core.rank_zero", raising=False
        )
        monkeypatch.delitem(
            sys.modules, "lightning.fabric.utilities.rank_zero", raising=False
        )
