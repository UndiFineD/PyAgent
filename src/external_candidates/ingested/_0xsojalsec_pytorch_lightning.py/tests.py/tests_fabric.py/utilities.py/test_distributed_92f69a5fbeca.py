# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_distributed.py
import functools
import os
...
def test_is_shared_filesystem(tmp_path, monkeypatch):
    ...
    test_fn = functools.partial(
        _test_is_shared_filesystem, tmp_path=tmp_path, monkeypatch=monkeypatch
    )
...
def _test_is_shared_filesystem(strategy, tmp_path, monkeypatch):
    ...
    with pytest.raises(
        FileNotFoundError,
        match="Unable to determine if the path belongs to a shared filesystem",
    ):
...
@mock.patch(
    "lightning.fabric.utilities.distributed._num_cpus_available", return_value=4
)
@pytest.mark.parametrize(
    ("num_processes", "expected"), [(1, 4), (2, 2), (3, 1), (4, 1), (8, 1)]
)
...
def test_infinite_barrier():
    ...
    with mock.patch(
        "lightning.fabric.utilities.distributed._distributed_is_initialized",
        return_value=False,
    ):
        barrier.__enter__()
        assert barrier.group is None
    ...
    with (
        ...
        mock.patch(
            "lightning.fabric.utilities.distributed.torch.distributed"
        ) as dist_mock,
    ):
        barrier.__enter__()
...
@mock.patch(
    "lightning.fabric.utilities.distributed.torch.distributed.init_process_group"
)
...
def test_get_default_process_group_backend_for_device():
    ...
    torch.distributed.Backend.register_backend(
        "pccl",
        lambda store, group_rank, group_size, timeout: mock_backend(
...
def test_is_dtensor(monkeypatch):
    ...
    monkeypatch.setattr(
        lightning.fabric.utilities.distributed, "_TORCH_GREATER_EQUAL_2_4", False
    )
