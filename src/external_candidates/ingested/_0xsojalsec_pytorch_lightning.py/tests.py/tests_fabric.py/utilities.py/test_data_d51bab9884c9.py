# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\utilities\test_data.py
import contextlib
import os
import random
...
def test_replace_dunder_methods_dataloader(
    cls, args, kwargs, arg_names, dataset, checked_values
):
    with _replace_dunder_methods(DataLoader, "dataset"):
...
def test_replace_value_in_args(
    ...
    assert _replace_value_in_saved_args(
        replace_key, replace_value, args, kwargs, default_kwargs, arg_names
    ) == (
        expected_status,
        expected_args,
        expected_kwargs,
...
def test_update_dataloader_typerror_custom_exception():
    ...
    with pytest.raises(
        MisconfigurationException, match="implementation has an error.*`dataset`"
    ):
    ...
    with pytest.raises(
        MisconfigurationException, match="implementation has an error.*`shuffle`"
    ):
        _update_dataloader(dataloader, dataloader.sampler)
...
def test_custom_torch_batch_sampler_doppelganger():
    ...
    batch_sampler = BatchSamplerDoppelganger(
        sampler=Mock(), batch_size=2, drop_last=True
    )
...
def test_custom_batch_sampler():
    ...
    with pytest.raises(
        TypeError,
        match=r"can't inject a \(distributed\) sampler into your batch sampler",
    ):
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                ...
                reason="OS does not support restricting CPU cores",
            ),
        ),
    ],
)
@mock.patch("lightning.fabric.utilities.data.os.cpu_count")
def test_suggested_max_num_workers(
    cpu_count_mock, affinity, cpu_count, local_world_size, expected, monkeypatch
):
    if affinity:
        monkeypatch.setattr(
            lightning.fabric.utilities.data.os,
            "sched_getaffinity",
            lambda _: list(range(cpu_count)),
        )
    else:
        monkeypatch.delattr(
            lightning.fabric.utilities.data.os, "sched_getaffinity", raising=False
        )
...
def test_suggested_max_num_workers_not_triggering_torch_warning(
    local_world_size, cpu_count, monkeypatch
):
    ...
    monkeypatch.delattr(
        torch.utils.data.dataloader.os, "sched_getaffinity", raising=False
    )
    monkeypatch.setattr(
        lightning.fabric.utilities.data.os, "cpu_count", lambda: cpu_count
    )
