...
def test_device_id_passed_for_cuda_devices(init_process_group_mock):
    ...
    if _TORCH_GREATER_EQUAL_2_3:
        kwargs["device_id"] = (
            cpu_strategy.root_device if cpu_strategy.root_device.type != "cpu" else None
        )
    init_process_group_mock.assert_called_with(
        process_group_backend,
        rank=global_rank,
    ...
    if _TORCH_GREATER_EQUAL_2_3:
        kwargs["device_id"] = (
            cuda_strategy.root_device
            if cuda_strategy.root_device.type != "cpu"
            else None
        )
    init_process_group_mock.assert_called_with(
        process_group_backend,
