# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_strategy.py
...
def test_save_checkpoint_rank_zero_only(is_rank_zero, tmp_path):
    ...
    )  # surrogate class to test implementation in base class
...
def test_save_checkpoint_empty_state(tmp_path):
    ...
    save_checkpoint_mock.assert_called_with(
        checkpoint=state, path=tmp_path, storage_options=None
    )
...
def test_save_checkpoint_convert_stateful_objects(tmp_path):
    ...
    assert all(
        torch.equal(p0, p1)
        for p0, p1 in zip(saved_model_state.values(), expected["model"].values())
    )
    assert (
        save_checkpoint_mock.call_args[1]["checkpoint"]["optimizer"]
        == expected["optimizer"]
    )
    assert (
        save_checkpoint_mock.call_args[1]["checkpoint"]["scheduler"]
        == expected["scheduler"]
    )
    assert (
        save_checkpoint_mock.call_args[1]["checkpoint"]["anything"]
        == expected["anything"]
    )
...
def test_load_checkpoint_model_optimizer_from_raw_checkpoint(tmp_path):
    ...
    strategy.load_checkpoint(
        tmp_path / "optimizer.ckpt", state=new_optimizer, strict=False
    )
...
def test_load_checkpoint_strict_loading(tmp_path):
    ...
    requested_state = {
        ...
        "b": 2,
        "c": 3,
    }  # key `c` does not exist in the saved state
...
def test_load_checkpoint_non_strict_loading(tmp_path):
    ...
    state = {
        ...
        "optimizer": optimizer,
        "int": 2,
        "new": "not_present_in_saved_state",
    }
    assert not torch.equal(model.weight, saved_model.weight)
    assert optimizer.state_dict() != saved_optimizer.state_dict()

    remainder = strategy.load_checkpoint(
        tmp_path / "checkpoint.ckpt", state, strict=False
    )
    assert torch.equal(model.weight, saved_model.weight)
    assert optimizer.state_dict() == saved_optimizer.state_dict()
    assert state["int"] == saved_state["int"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            HalfPrecision("bf16-true"), torch.bfloat16, marks=RunIf(mps=False)
        ),
...
def test_module_init_context(device, precision, dtype, empty_init, monkeypatch):
    ...
    )  # surrogate class to test base class
...
def test_tensor_init_context(device, precision, dtype):
    ...
    assert (
        tensor1.dtype == torch.long
    )  # `.init_tensor()` only affects floating point dtypes
    assert (
        tensor2.dtype == torch.half
    )  # this tensor was created with an explicit dtype assignment
