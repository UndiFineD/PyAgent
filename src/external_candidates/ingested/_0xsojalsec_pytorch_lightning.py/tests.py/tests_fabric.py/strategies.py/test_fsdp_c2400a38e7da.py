...
def test_hybrid_shard_configuration(sharding_strategy, monkeypatch):
    ...
    monkeypatch.setattr(
        "lightning.fabric.strategies.fsdp._TORCH_GREATER_EQUAL_2_2", False
    )
    with pytest.raises(
    ...
    monkeypatch.setattr(
        "lightning.fabric.strategies.fsdp._TORCH_GREATER_EQUAL_2_2", True
    )
    device_mesh = Mock()
    strategy = FSDPStrategy(
...
def test_setup_use_orig_params(_):
    ...
    strategy = FSDPStrategy(
        parallel_devices=[torch.device("cpu")], use_orig_params=False
    )
...
def test_save_checkpoint_one_fsdp_module_required(tmp_path):
    ...
    with pytest.raises(
        ValueError, match="Found multiple FSDP models in the given state."
    ):
...
def test_load_checkpoint_no_state(tmp_path):
    ...
    strategy = FSDPStrategy()
    with pytest.raises(
        ValueError, match=escape("Got FSDPStrategy.load_checkpoint(..., state=None")
    ):
        strategy.load_checkpoint(path=tmp_path, state=None)
    with pytest.raises(
        ValueError, match=escape("Got FSDPStrategy.load_checkpoint(..., state={})")
    ):
...
@mock.patch(
    "torch.distributed.fsdp.fully_sharded_data_parallel.FullyShardedDataParallel.set_state_dict_type"
)
def test_get_full_state_dict_context_offload(set_type_mock, monkeypatch):
    ...
    with _get_full_state_dict_context(
        module=Mock(spec=FullyShardedDataParallel), world_size=1
    ):
    ...
    with _get_full_state_dict_context(
        module=Mock(spec=FullyShardedDataParallel), world_size=4
    ):
        assert set_type_mock.call_args_list[0][0][2].offload_to_cpu  # model config
        assert set_type_mock.call_args_list[0][0][3].offload_to_cpu  # optim config
