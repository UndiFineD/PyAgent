# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\launchers\test_subprocess_script.py
...
@mock.patch(
    "lightning.fabric.strategies.launchers.subprocess_script._ChildProcessObserver"
)
def test_subprocess_script_launcher_can_launch(*_):
    ...
    with pytest.raises(
        RuntimeError,
        match="attempted to launch new distributed processes with `local_rank > 0`",
    ):
        launcher.launch(Mock())

    launcher.procs = [Mock()]  # there are already processes running
    with pytest.raises(
        RuntimeError, match="The launcher can only create subprocesses once"
    ):
        launcher.launch(Mock())
...
def test_subprocess_script_launcher_launch_processes(_, popen_mock):
    ...
    world_sizes = [
        int(calls[i][1]["env"]["WORLD_SIZE"]) for i in range(num_new_processes)
    ]
    assert (
        world_sizes == [launcher.num_processes * launcher.num_nodes] * num_new_processes
    )

    # local rank in child processes
    local_ranks = [
        int(calls[i][1]["env"]["LOCAL_RANK"]) for i in range(num_new_processes)
    ]
