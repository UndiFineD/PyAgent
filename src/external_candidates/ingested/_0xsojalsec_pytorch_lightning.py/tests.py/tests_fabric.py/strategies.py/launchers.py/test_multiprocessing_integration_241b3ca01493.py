# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\launchers\test_multiprocessing_integration.py
...
def test_memory_sharing_disabled(strategy):
    ...
    if strategy == "ddp_fork" and sys.platform == "darwin":
        pytest.skip(
            "ddp_fork is unsafe / unsupported on macOS due to fork + ObjC runtime issues"
        )
    tensor = torch.rand(4)
    model = SimpleModel()
