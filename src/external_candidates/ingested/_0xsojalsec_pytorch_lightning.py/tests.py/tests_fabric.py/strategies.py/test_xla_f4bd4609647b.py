...
def broadcast_on_tpu_fn(strategy):
    ...
    tensor = torch.tensor(
        strategy.global_rank, device=strategy.root_device, dtype=torch.bfloat16
    )
...
def tpu_all_gather_fn(strategy):
    ...
    for sync_grads in (True, False):
        ...
        assert torch.equal(
            summed, torch.tensor(strategy.world_size, dtype=torch.float32)
        )
...
def tpu_sync_module_states_fn(sync_module_states, strategy):
    seed_everything(
        strategy.local_rank
    )  # force the model to have different weights across ranks
