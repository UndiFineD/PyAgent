# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_deepspeed.py
...
def test_deepspeed_only_compatible_with_cuda():
    ...
    with pytest.raises(
        RuntimeError, match="The DeepSpeed strategy is only supported on CUDA GPUs"
    ):
...
def test_deepspeed_requires_joint_setup():
    ...
    with pytest.raises(
        NotImplementedError,
        match=escape(
            "does not support setting up the module and optimizer(s) independently"
        ),
    ):
...
def test_deepspeed_save_checkpoint_storage_options(tmp_path):
    ...
    with pytest.raises(
        TypeError,
        match=escape(
            "DeepSpeedStrategy.save_checkpoint(..., storage_options=...)` is not"
        ),
    ):
...
def test_deepspeed_save_checkpoint_one_deepspeed_engine_required(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match="Could not find a DeepSpeed model in the provided checkpoint state.",
    ):
    ...
    with pytest.raises(
        ValueError, match="Found multiple DeepSpeed engine modules in the given state."
    ):
...
def test_deepspeed_save_checkpoint_client_state_separation(tmp_path):
    ...
    model.modules.return_value = [model]
    strategy.save_checkpoint(
        path=tmp_path, state={"model": model, "optimizer": optimizer, "test": "data"}
    )
    # the client_state should not contain any deepspeed engine or deepspeed optimizer
    model.save_checkpoint.assert_called_with(
        tmp_path,
...
def test_deepspeed_save_checkpoint_warn_colliding_keys(tmp_path):
    ...
    with pytest.warns(
        UserWarning, match="Your state has keys that collide with DeepSpeed's internal"
    ):
        strategy.save_checkpoint(
            path=tmp_path,
            state={"model": model, "optimizer": optimizer, "mp_world_size": 2},
        )
...
def test_deepspeed_save_checkpoint_exclude_frozen_parameters(exclude_frozen_parameters):
    ...
    assert (
        strategy.deepspeed_impl.exclude_frozen_parameters is exclude_frozen_parameters
    )
...
def test_deepspeed_load_checkpoint_no_state(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match=escape("Got DeepSpeedStrategy.load_checkpoint(..., state=None"),
    ):
        strategy.load_checkpoint(path=tmp_path, state=None)
    with pytest.raises(
        ValueError, match=escape("Got DeepSpeedStrategy.load_checkpoint(..., state={})")
    ):
...
@RunIf(deepspeed=True)
@mock.patch(
    "pytorch_lightning_enterprise.strategies.deepspeed._is_deepspeed_checkpoint",
    return_value=True,
)
...
def test_deepspeed_load_checkpoint_client_state_missing(tmp_path):
    ...
    with pytest.raises(
        ...
        strategy.load_checkpoint(
            path=tmp_path,
            state={"model": model, "optimizer": optimizer, "test": "data"},
        )
...
def test_deepspeed_load_checkpoint_optimzer_state_requested(
    _, optimzer_state_requested, tmp_path
):
