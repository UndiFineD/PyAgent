# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_model_parallel.py
...
@mock.patch(
    "lightning.fabric.strategies.model_parallel._TORCH_GREATER_EQUAL_2_4", False
)
def test_torch_greater_equal_2_4():
    with pytest.raises(
        ImportError, match="ModelParallelStrategy requires PyTorch 2.4 or higher"
    ):
...
def test_validate_device_mesh_dimensions(
    num_nodes, devices, invalid_dp_size, invalid_tp_size
):
...
def test_checkpoint_io_unsupported():
    ...
    strategy = ModelParallelStrategy(parallelize_fn=(lambda m, _: m))
    with pytest.raises(
        NotImplementedError, match="does not use the `CheckpointIO` plugin"
    ):
        _ = strategy.checkpoint_io

    with pytest.raises(
        NotImplementedError, match="does not support setting a `CheckpointIO` plugin"
    ):
...
def test_fsdp_v1_modules_unsupported():
    ...
    with pytest.raises(
        TypeError, match="only supports the new FSDP2 APIs in PyTorch >= 2.4"
    ):
...
def test_parallelize_fn_call():
    ...
    parallel_model_mock = Mock(
        spec=nn.Module, parameters=Mock(return_value=[]), buffers=Mock(return_value=[])
    )
    ...
    model_setup, [optimizer_setup], _ = strategy.setup_module_and_optimizers(
        model, [optimizer]
    )
    ...
    strategy.parallel_devices = [torch.device("cpu")]
    with pytest.raises(
        TypeError, match="The `parallelize_fn` must return a `nn.Module` instance"
    ):
...
def test_save_checkpoint_storage_options(tmp_path):
    ...
    with pytest.raises(
        TypeError,
        match=escape(
            "ModelParallelStrategy.save_checkpoint(..., storage_options=...)` is not"
        ),
    ):
...
@RunIf(min_torch="2.4")
@mock.patch(
    "lightning.fabric.strategies.model_parallel.ModelParallelStrategy.broadcast",
    lambda _, x: x,
)
@mock.patch(
...
@mock.patch(
    "torch.distributed.checkpoint.state_dict.get_optimizer_state_dict", return_value={}
)
...
def test_save_checkpoint_path_exists(
    shutil_mock, torch_save_mock, _, __, ___, tmp_path
):
    strategy = ModelParallelStrategy(
        parallelize_fn=(lambda m, _: m), save_distributed_checkpoint=False
    )
    ...
    strategy = ModelParallelStrategy(
        parallelize_fn=(lambda m, _: m), save_distributed_checkpoint=True
    )
...
def test_save_checkpoint_one_dist_module_required(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match="Could not find a distributed model in the provided checkpoint state.",
    ):
    ...
    with mock.patch(
        "lightning.fabric.strategies.model_parallel._has_dtensor_modules",
        return_value=True,
    ):
        ...
        with pytest.raises(
            ValueError, match="Found multiple distributed models in the given state."
        ):
            strategy.save_checkpoint(
                path=tmp_path, state={"model1": model1, "model2": model2}
            )
...
def test_load_checkpoint_no_state(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match=escape("Got ModelParallelStrategy.load_checkpoint(..., state=None"),
    ):
        strategy.load_checkpoint(path=tmp_path, state=None)
    with pytest.raises(
        ValueError,
        match=escape("Got ModelParallelStrategy.load_checkpoint(..., state={})"),
    ):
...
def test_load_unknown_checkpoint_type(_, tmp_path):
    ...
    path = (
        tmp_path / "empty_dir"
    )  # neither a single file nor a directory with meta file
    path.mkdir()
...
def test_load_raw_checkpoint_validate_single_file(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match="The given path must be a single file containing the full state dict",
    ):
        strategy.load_checkpoint(path=path, state=model)
...
def test_load_raw_checkpoint_optimizer_unsupported(tmp_path):
    ...
    with pytest.raises(
        NotImplementedError,
        match="Loading a single optimizer object from a checkpoint is not supported",
    ):
...
def test_set_timeout(init_process_group_mock, _):
    ...
    strategy = ModelParallelStrategy(
        parallelize_fn=(lambda m, _: m), timeout=test_timedelta
    )
    strategy.parallel_devices = [torch.device("cpu")]
    ...
    if _TORCH_GREATER_EQUAL_2_3:
        kwargs["device_id"] = (
            strategy.root_device if strategy.root_device.type != "cpu" else None
        )
...
def test_meta_device_materialization():
    ...
    with pytest.warns(
        UserWarning,
        match=r"`reset_parameters\(\)` method for re-initialization: NoResetParameters",
    ):
