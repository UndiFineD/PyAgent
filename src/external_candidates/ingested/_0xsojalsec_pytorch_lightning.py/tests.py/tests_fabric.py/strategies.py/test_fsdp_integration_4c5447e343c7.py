...
class _Trainer(BasicTrainer):
    ...
    def step(self, model, batch):
        wrapped_layers = [
            m for m in model.modules() if isinstance(m, FullyShardedDataParallel)
        ]
        assert len(wrapped_layers) == self.num_wrapped
        assert (self.num_wrapped == 4) == isinstance(
            model._forward_module, FullyShardedDataParallel
        )
...
@pytest.mark.parametrize(
    "precision", ["16-mixed", pytest.param("bf16-mixed", marks=RunIf(bf16_cuda=True))]
)
...
def test_save_full_state_dict(tmp_path):
    ...
    fabric = Fabric(
        ...
        strategy=FSDPStrategy(
            auto_wrap_policy=always_wrap_policy, state_dict_type="full"
        ),
        devices=2,
    )
    fabric.launch()
    ...
    with FullyShardedDataParallel.summon_full_params(trainer.model):
        ...
        for param_name in state_dict:
            assert torch.equal(
                loaded_state_dict[param_name], state_dict[param_name].cpu()
            )
    ...
    metadata = fabric.load(
        checkpoint_path, {"model": trainer.model, "optimizer": trainer.optimizer}
    )
    ...
    trainer.run()
    metadata = fabric.load(
        normal_checkpoint_path, {"model": trainer.model, "optimizer": trainer.optimizer}
    )
...
def test_load_full_state_dict_into_sharded_model(tmp_path):
    ...
    with FSDP.summon_full_params(trainer.model, writeback=False, rank0_only=False):
        params_before = torch.cat(
            [p.cpu().view(-1) for p in trainer.model.parameters()]
        )
...
def test_setup_module_move_to_device(fabric_module_mock, move_to_device):
    ...
    assert next(fabric_model.parameters()).device == torch.device(
        "cuda", fabric.local_rank
    )
...
def test_reapply_compile():
    ...
    torch.compile.assert_called_with(
        fabric_model._forward_module._orig_mod, **compile_kwargs
    )
...
def test_save_filter(tmp_path):
    fabric = Fabric(
        accelerator="cuda", strategy=FSDPStrategy(state_dict_type="full"), devices=2
    )
    fabric.launch()
    ...
    with pytest.raises(
        NotImplementedError, match="doesn't support loading sharded filtered"
    ):
        fabric.save(checkpoint_path, state, filter=filter)
...
def test_manual_activation_checkpointing():
    ...
    wrappers = {
        ...
        if isinstance(mod, CheckpointWrapper)
    }
    assert wrappers == {"0", "1"}

    # let fabric set up the model, it shouldn't apply activation checkpointing again
    with pytest.warns(
        match="is configured, but the model already contains checkpointed"
    ):
        model = fabric.setup(model)

    wrappers = {
        name
        for name, mod in model._forward_module.named_modules()
        if isinstance(mod, CheckpointWrapper)
    }
...
def test_rewrap_warnings():
    ...
    with fabric.init_module():
        model = torch.nn.Sequential(
            torch.nn.Linear(1, 1), torch.nn.ReLU(), wrap(torch.nn.Linear(1, 1))
        )
    ...
    if clip_type == "norm":
        with FSDP.summon_full_params(model._forward_module, with_grads=True):
            norm = torch.linalg.vector_norm(
                model.weight.grad.detach().cpu(), 2, dtype=torch.float32
            ).item()
        new_norm = norm / 10
        ...
        assert torch.allclose(
            model.weight.grad, torch.full_like(model.weight.grad, new_val)
        )
    else:
...
def test_save_sharded_and_consolidate_and_load(tmp_path):
    ...
    fabric = Fabric(
        accelerator="cuda",
        strategy=FSDPStrategy(
            auto_wrap_policy=always_wrap_policy, state_dict_type="sharded"
        ),
        devices=2,
    )
    fabric.launch()
...
def test_no_call_to_apply(monkeypatch):
    ...
    monkeypatch.setattr(
        torch.distributed.fsdp.FullyShardedDataParallel, "apply", Mock()
    )
