# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_xla_fsdp_integration.py
...
def _xla_fsdp_rewrap_warning(fabric: Fabric):
    ...
    with fabric.init_module():
        model = torch.nn.Sequential(
            torch.nn.Linear(1, 1),
            torch.nn.ReLU(),
            XlaFullyShardedDataParallel(torch.nn.Linear(1, 1)),
        )
    if fabric.node_rank:
...
def xla_fsdp_train_save_load(fabric: Fabric, tmp_path, state_dict_type):
    ...
    with fabric.init_module():
        model_1 = torch.nn.Sequential(
            torch.nn.Linear(32, 32), torch.nn.ReLU(), torch.nn.Linear(32, 2)
        )
    ...
    if state_dict_type == "full" and is_multihost:
        with pytest.raises(
            OSError, match="Multihost setups do not have a shared filesystem"
        ):
    ...
    if state_dict_type == "sharded":
        ...
        with pytest.raises(
            KeyError,
            match="The requested state contains a key 'coconut' that does not exist",
        ):
...
def test_xla_fsdp_train_save_load(
    tmp_path, use_auto_wrap_policy, state_dict_type, sequential_save
):
