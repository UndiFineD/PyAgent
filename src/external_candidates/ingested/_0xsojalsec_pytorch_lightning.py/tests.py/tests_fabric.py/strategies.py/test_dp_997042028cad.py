# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_dp.py
...
def test_dp_module_state_dict():
    ...
    assert (
        strategy.get_module_state_dict(original_module).keys()
        == original_module.state_dict().keys()
    )
    ...
    with mock.patch("lightning.fabric.strategies.dp.DataParallel", DataParallelMock):
        ...
        assert (
            strategy.get_module_state_dict(wrapped_module).keys()
            == original_module.state_dict().keys()
        )
