# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_model_parallel_integration.py
...
def test_setup_device_mesh(distributed):
    ...
    fabric.launch()
    assert fabric.strategy.device_mesh.mesh_dim_names == (
        "data_parallel",
        "tensor_parallel",
    )
...
@pytest.mark.xfail(
    ...
    strict=False,
    run=True,
    condition=lambda e: isinstance(e, AssertionError)
    and str(e).startswith("Test left zombie thread"),
)
...
def test_fsdp2_tensor_parallel(distributed, compile):
    ...
    assert all(isinstance(weight, DTensor) for weight in model.parameters())
    assert all(
        isinstance(tensor, DTensor) for tensor in optimizer.param_groups[0]["params"]
    )
    ...
    assert all(
        tensor.device_mesh.ndim == 2 for tensor in optimizer.param_groups[0]["params"]
    )
    assert all(
        tensor.device.type != "meta" for tensor in optimizer.param_groups[0]["params"]
    )
    ...
    for _, batch in enumerate(dataloader):
        ...
        assert all(
            torch.equal(batches_tp[0], batches_tp[i]) for i in range(1, len(batches_tp))
        )
...
def test_train_save_load(distributed, precision, tmp_path):
    ...
    fabric = Fabric(
        accelerator="cuda", devices=4, strategy=strategy, precision=precision
    )
    fabric.launch()
...
def test_save_full_state_dict(distributed, tmp_path):
    ...
    for param_name in state_dict:
        assert torch.equal(
            loaded_state_dict[param_name], state_dict[param_name].full_tensor().cpu()
        )
    ...
    assert (
        set(checkpoint["optimizer"].keys())
        == set(optimizer_state_before.keys())
        == {"state", "param_groups"}
    )

    # 1. verify the FSDP state can be loaded back into a FSDP model/strategy directly
    strategy = ModelParallelStrategy(
        _parallelize_feed_forward_fsdp2, data_parallel_size=2, tensor_parallel_size=1
    )
    fabric = Fabric(accelerator="cuda", strategy=strategy, devices=2)
    fabric.launch()
    model, optimizer = _train(fabric)
    ...
    assert all(
        torch.equal(p0.cpu(), p1.cpu()) for p0, p1 in zip(params_before, params_after)
    )

    optimizer_state_after = get_optimizer_state_dict(model, optimizer)
    optimizer_state_after["param_groups"][0]["betas"] = tuple(
        optimizer_state_after["param_groups"][0]["betas"]
    )
    assert (
        set(optimizer_state_after.keys())
        == set(optimizer_state_before.keys())
        == {"state", "param_groups"}
    )
    torch.testing.assert_close(
        optimizer_state_after["state"], optimizer_state_before["state"], atol=0, rtol=0
    )
    assert (
        optimizer_state_after["param_groups"] == optimizer_state_before["param_groups"]
    )
    ...
    normal_checkpoint_path = Path(
        fabric.broadcast(str(tmp_path / "normal-checkpoint.pt"))
    )
    fabric.save(
        normal_checkpoint_path, {"model": model, "optimizer": optimizer, "steps": 2}
    )

    optimizer_state_after = torch.load(normal_checkpoint_path, weights_only=True)[
        "optimizer"
    ]
    assert (
        set(optimizer_state_after.keys())
    ...
    assert torch.equal(
        ...
        optimizer_state_before["state"]["_forward_module.w1.weight"]["exp_avg"]
        .full_tensor()
        .cpu(),
    )
    ...
    metadata = fabric.load(
        normal_checkpoint_path, {"model": model, "optimizer": optimizer}
    )
...
def test_load_full_state_dict_into_sharded_model(distributed, tmp_path):
    ...
    params_after = torch.cat(
        [p.full_tensor().cpu().view(-1) for p in model.parameters()]
    )
...
def test_module_init_context(distributed, precision, expected_dtype):
    ...
    strategy = ModelParallelStrategy(parallelize_fn=_parallelize_feed_forward_fsdp2)
    fabric = Fabric(
        accelerator="cuda", devices=2, strategy=strategy, precision=precision
    )
    fabric.launch()
    ...
    def _run_setup_assertions(empty_init, expected_device):
        ...
        assert all(
            weight.device == torch.device("cuda", fabric.local_rank)
...
def test_save_filter(distributed, tmp_path):
    ...
    with pytest.raises(
        NotImplementedError, match="doesn't support loading distributed filtered"
    ):
        fabric.save(checkpoint_path, state, filter=filter)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "norm", marks=pytest.mark.skip("Gradient clipping by norm is not correct.")
        ),
        "val",
    ],
)
def test_clip_gradients(distributed, clip_type, precision):
    ...
    fabric = Fabric(
        accelerator="auto", devices=2, precision=precision, strategy=strategy
    )
    fabric.launch()
    ...
    if clip_type == "norm":
        norm = torch.linalg.vector_norm(
            model.weight.grad.full_tensor().detach().cpu(), 2, dtype=torch.float32
        ).item()
        new_norm = norm / 10
        ...
        assert torch.allclose(
            ...
            torch.full_like(model.weight.grad.full_tensor(), new_val),
        )
    else:
...
def test_load_raw_module_state(distributed):
    ...
    class CustomModel(nn.Module):
        def __init__(self):
            ...
            self.register_buffer(
                "non_persistent_buffer", torch.rand(2), persistent=False
            )
    ...
    with pytest.raises(
        KeyError, match="The model contains a key 'parameter' that does not exist"
    ):
        _load_raw_module_state(state_dict, model, strict=True)
