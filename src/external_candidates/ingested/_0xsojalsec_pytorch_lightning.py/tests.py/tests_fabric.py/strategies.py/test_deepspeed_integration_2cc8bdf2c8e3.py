# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_deepspeed_integration.py
...
def test_deepspeed_multiple_models():
    fabric = Fabric(
        strategy=DeepSpeedStrategy(stage=3, logging_batch_size_per_gpu=1),
        devices=2,
        accelerator="gpu",
    )
    fabric.launch()
...
def test_deepspeed_env_variables_on_platforms(_, deepspeed_dist_mock, platform):
    ...
    if platform == "Windows":
        ...
        assert os.environ["MASTER_ADDR"] == str(
            strategy.cluster_environment.main_address
        )
    ...
    if fabric.is_global_zero:
        if fabric.strategy.config["zero_optimization"]["stage"] in (2, 3):
            ...
            convert_zero_checkpoint_to_fp32_state_dict(
                checkpoint_path, single_ckpt_path, tag="checkpoint"
            )

            is_ckpt_path_a_file = os.path.isfile(single_ckpt_path)
            single_ckpt_path = (
                single_ckpt_path
                if is_ckpt_path_a_file
                else single_ckpt_path / "pytorch_model.bin"
            )
            state_dict = torch.load(single_ckpt_path, weights_only=False)
        else:
            # 'checkpoint' is the tag, hardcoded in DeepSpeedStrategy
            single_ckpt_path = (
                checkpoint_path / "checkpoint" / "mp_rank_00_model_states.pt"
            )
...
def test_deepspeed_init_module_with_stage_3(empty_init):
    ...
    strategy = DeepSpeedStrategy(stage=3)
    fabric = Fabric(
        accelerator="cuda", devices=2, strategy=strategy, precision="bf16-true"
    )
    fabric.launch()
...
def test_deepspeed_init_module_with_stages_1_2(stage, empty_init):
    ...
    zero_init_mock.assert_called_with(
