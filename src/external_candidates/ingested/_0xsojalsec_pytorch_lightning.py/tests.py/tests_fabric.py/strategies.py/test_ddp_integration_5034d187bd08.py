# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_fabric\strategies\test_ddp_integration.py
...
def _run_ddp_save_load(fabric, tmp_path):
    ...
    def assert_params_equal(params0, params1):
        assert all(
            torch.equal(p0, p1.to(p0.device)) for p0, p1 in zip(params0, params1)
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "norm", "cuda", "bf16-mixed", marks=RunIf(min_cuda_gpus=2, bf16_cuda=True)
        ),
        pytest.param(
            "val", "cuda", "bf16-mixed", marks=RunIf(min_cuda_gpus=2, bf16_cuda=True)
        ),
    ],
)
...
def test_clip_gradients(clip_type, accelerator, precision):
    ...
    fabric = Fabric(
        accelerator=accelerator, devices=2, precision=precision, strategy="ddp"
    )
    fabric.launch()
