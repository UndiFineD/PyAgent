# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\parity_pytorch\test_basic_parity.py
...
_MARK_SHORT_BM = pytest.mark.skipif(
    _SHORT_BENCHMARKS, reason="Only run during Benchmarking"
)
_MARK_XFAIL_LOSS = pytest.mark.xfail(strict=False, reason="bad loss")


def assert_parity_relative(
    pl_values, pt_values, norm_by: float = 1, max_diff: float = 0.1
):
    # assert speeds
    ...
    ), f"Lightning diff {diffs} was worse than vanilla PT (threshold {max_diff})"


def assert_parity_absolute(
    pl_values, pt_values, norm_by: float = 1, max_diff: float = 0.55
):
    # assert speeds
    diffs = np.asarray(pl_values) - np.mean(pt_values)
    # norm by event count
    ...
    assert (
        np.mean(diffs) < max_diff
    ), f"Lightning {diffs} was worse than vanilla PT (threshold {max_diff})"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ParityModuleMNIST, 0.3, 0.001, 4, 3, marks=_MARK_XFAIL_LOSS
        ),  # FixME: investigate!
        pytest.param(  # FixME: investigate!
            ...
            2,
            2,
            marks=[_MARK_SHORT_BM, _MARK_XFAIL_LOSS],
        ),
    ],
)
...
def test_pytorch_parity(
    ...
    assert_parity_absolute(
        lightning["durations"][1:],
        vanilla["durations"][1:],
        norm_by=num_epochs,
        max_diff=max_diff_speed,
    )

    assert_parity_relative(
        lightning["memory"], vanilla["memory"], max_diff=max_diff_memory
    )
