# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\parity_pytorch\generate_comparison.py
    ...
    for i, cls_model in enumerate(MODEL_CLASSES):
        ...
        if os.path.isfile(path_csv):
            ...
            vanilla = measure_loops(
                cls_model, kind="Vanilla PT", num_epochs=NUM_EPOCHS, num_runs=NUM_RUNS
            )
            lightning = measure_loops(
                cls_model, kind="PT Lightning", num_epochs=NUM_EPOCHS, num_runs=NUM_RUNS
            )
            ...
            df_time /= NUM_RUNS
            df_time.to_csv(
                os.path.join(PATH_HERE, f"dump-times_{cls_model.__name__}.csv")
            )
