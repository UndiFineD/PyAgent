# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\tuner\test_tuning.py
...
def test_tuner_with_already_configured_batch_size_finder():
    ...
    with pytest.raises(
        ValueError, match=r"Trainer is already configured with a `BatchSizeFinder`"
    ):
        tuner.scale_batch_size(model)
...
def test_tuner_with_already_configured_learning_rate_finder():
    ...
    with pytest.raises(
        ValueError, match=r"Trainer is already configured with a `LearningRateFinder`"
    ):
        tuner.lr_find(model)
