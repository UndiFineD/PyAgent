...
def test_error_with_multiple_optimizers(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException, match="only works with single optimizer"
    ):
        tuner.lr_find(model)
    ...
    class CustomBoringModel(BoringModel):
        ...
        def configure_optimizers(self):
            return torch.optim.SGD(
                self.parameters(), lr=self.hparams.lr if use_hparams else self.lr
            )
...
def test_suggestion_not_enough_finite_points(
    losses, skip_begin, skip_end, expected_error, caplog
):
...
def test_lr_attribute_when_suggestion_invalid(tmp_path):
    ...
    lr_finder = tuner.lr_find(
        model=model, update_attr=True, num_training=1
    )  # force insufficient data points
    assert lr_finder.suggestion() is None
    assert (
        model.learning_rate == 0.123
    )  # must remain unchanged because suggestion is not possible
...
def test_lr_finder_callback_restarting(tmp_path):
    ...
    class CustomLearningRateFinder(LearningRateFinder):
        ...
        def lr_find(self, trainer, pl_module) -> None:
            pl_module._expected_max_steps = (
                trainer.global_step + self._num_training_steps
            )
...
def test_lr_finder_training_step_none_output(tmp_path):
    ...
    lr_finder = tuner.lr_find(
        model=model, update_attr=True, num_training=20, early_stop_threshold=None
    )
...
def test_lr_finder_with_early_stopping(tmp_path):
    ...
    assert (
        lr_finder_callback is not None
    ), "LearningRateFinder callback should be present"
    assert (
        early_stopping_callback is not None
    ), "EarlyStopping callback should be present"

    # Verify learning rate finder ran and has results
    assert (
        lr_finder_callback.optimal_lr is not None
    ), "Learning rate finder should have results"
...
def test_gradient_correctness():
    ...
    assert (
        abs(suggestion - math.pi) < 1e-2
    ), "Suggestion should be close to pi for this synthetic example"
...
def test_lr_finder_callback_applies_lr_after_restore(tmp_path):
    ...
    lr_finder_cb = (
        LearningRateFinder()
    )  # default update_attr=True should apply suggestion
    ...
    ), "LR Finder should produce a suggestion for this setup"
    ...
    assert current_lr == pytest.approx(
        suggestion
...
def test_exponential_vs_linear_mode_gradient_difference(tmp_path):
    ...
    lr_finder_linear = tuner_linear.lr_find(
        model_linear, min_lr=1e-6, max_lr=1e-1, num_training=50, mode="linear"
    )
    lr_finder_exp = tuner_exp.lr_find(
        model_exp, min_lr=1e-6, max_lr=1e-1, num_training=50, mode="exponential"
    )
    ...
    for lr_finder, mode in [
        ...
        if len(losses_filtered) >= 2:
            ...
            assert torch.isfinite(
                gradients
            ).all(), f"Non-finite gradients in {mode} mode"
            ...
            if mode == "exponential":
                assert not torch.allclose(
                    gradients, gradients_no_spacing, rtol=0.1
...
def test_lr_finder_checkpoint_cleanup_on_error(tmp_path):
    ...
    assert (
        len(lr_find_checkpoints) == 0
    ), "No lr_find checkpoint files should exist initially"
...
def test_lr_finder_with_backbone_finetuning_callback(tmp_path):
    ...
    for param in model.backbone.parameters():
        assert (
            param.requires_grad
        ), "Backbone parameters should be unfrozen after epoch 1"
