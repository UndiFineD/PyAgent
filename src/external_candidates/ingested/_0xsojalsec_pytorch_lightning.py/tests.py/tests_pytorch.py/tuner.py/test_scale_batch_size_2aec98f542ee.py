# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\tuner\test_scale_batch_size.py
...
class BatchSizeModel(BoringModel):
    ...
    def train_dataloader(self):
        return DataLoader(
            RandomDataset(32, 64), batch_size=getattr(self, "batch_size", 1)
        )
...
@pytest.mark.parametrize(
    ("model_bs", "dm_bs"), [(2, -1), (2, 2), (2, None), (None, 2), (16, 16)]
)
...
def test_trainer_reset_correctly(tmp_path, trainer_fn):
    ...
    assert not any(
        f for f in os.listdir(tmp_path) if f.startswith(".scale_batch_size_temp_model")
    )
...
def test_auto_scale_batch_size_trainer_arg(tmp_path, scale_arg):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=1, accelerator="gpu", devices=1
    )
    tuner = Tuner(trainer)
...
def test_auto_scale_batch_size_set_datamodule_attribute(tmp_path, use_hparams):
    ...
    datamodule_class = (
        HparamsBatchSizeDataModule if use_hparams else BatchSizeDataModule
    )
    ...
    tuner.scale_batch_size(
        model, datamodule=datamodule, steps_per_trial=2, max_trials=4
    )
    after_batch_size = (
        datamodule.hparams.batch_size if use_hparams else datamodule.batch_size
    )
...
def test_error_on_dataloader_passed_to_fit(tmp_path):
    ...
    with pytest.raises(
        ...
        tuner.scale_batch_size(
            model, train_dataloaders=model.train_dataloader(), mode="power"
        )
...
def test_dataloader_reset_with_scale_batch_size(
    tmp_path, caplog, scale_method, init_batch_size
):
    ...
    expected_tries = (
        max_trials - 1 if init_batch_size < dataset_len and max_trials > 0 else 0
    )
    ...
    largest_tested_batch_size = (
        new_batch_size
        if scale_method == "power"
        else int(math.ceil(new_batch_size * 100 / 95))
    )
    assert caplog.text.count("greater or equal than the length") == int(
        largest_tested_batch_size >= dataset_len
    )
...
def test_batch_size_finder_callback(tmp_path, trainer_fn):
    ...
    batch_size_finder = BatchSizeFinder(
        max_trials=max_trials, batch_arg_name="batch_size"
    )
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=max_epochs, callbacks=[batch_size_finder]
    )
    ...
    if trainer_fn == "fit":
        ...
        assert (
            loop.epoch_loop.batch_progress.total.completed
            == expected_steps * max_epochs
        )
    else:
...
def test_error_if_train_or_val_dataloaders_passed_with_eval_method():
    ...
    with pytest.raises(
        MisconfigurationException, match="please consider setting `dataloaders` instead"
    ):
...
def test_error_if_dataloaders_passed_with_fit_method():
    ...
    with pytest.raises(
        MisconfigurationException,
        match="please consider setting `train_dataloaders` and `val_dataloaders` instead",
    ):
...
def test_batch_size_finder_with_distributed_strategies():
    ...
    with pytest.raises(
        MisconfigurationException,
        match="Batch size finder is not supported with distributed strategies.",
    ):
...
def test_batch_size_finder_with_multiple_eval_dataloaders(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match="Batch size finder cannot be used with multiple .* dataloaders",
    ):
        tuner.scale_batch_size(model, method="validate")


@pytest.mark.parametrize(
    ("scale_method", "expected_batch_size"), [("power", 62), ("binsearch", 95)]
)
@patch("lightning.pytorch.tuner.batch_size_scaling.is_oom_error", return_value=True)
def test_dataloader_batch_size_updated_on_failure(
    _, tmp_path, scale_method, expected_batch_size
):
...
def test_scale_batch_size_max_val_limit(tmp_path, mode):
    ...
    result = tuner.scale_batch_size(
        model, mode=mode, max_trials=5, steps_per_trial=1, max_val=max_val
    )
...
def test_scale_batch_size_checkpoint_cleanup_on_error(tmp_path):
    ...
    assert (
        len(scale_checkpoints) == 0
    ), "No scale_batch_size checkpoint files should exist initially"
...
def test_scale_batch_size_max_trials_modes(
    tmp_path, max_trials, mode, init_val, expected
):
