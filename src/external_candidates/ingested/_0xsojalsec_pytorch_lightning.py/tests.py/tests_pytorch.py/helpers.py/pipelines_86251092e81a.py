...
def run_model_test_without_loggers(
    ...
    model2 = load_model_from_checkpoint(
        trainer.checkpoint_callback.best_model_path, type(model)
    )
...
def run_model_test(
    ...
    change_ratio = torch.norm(initial_values - post_train_values) / torch.norm(
        initial_values
    )
...
def run_model_prediction(trained_model, dataloader, min_acc=0.50):
    ...
    assert (
        acc >= min_acc
    ), f"This model is expected to get > {min_acc} in test set (it got {acc})"
