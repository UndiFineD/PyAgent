# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\helpers\datasets.py
...
class MNIST(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.data, self.targets = self._try_load(
...
class TrialMNIST(MNIST):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cache_folder_name = (
            f"digits-{'-'.join(str(d) for d in self.digits)}_nb-{self.num_samples}"
        )
    ...
    @staticmethod
    def _prepare_subset(
        full_data: Tensor, full_targets: Tensor, num_samples: int, digits: Sequence
    ):
    ...
    def _download(self, data_folder: str) -> None:
        ...
        for fname in (self.TRAIN_FILE_NAME, self.TEST_FILE_NAME):
            ...
            data, targets = self._prepare_subset(
                data, targets, self.num_samples, self.digits
            )
...
class SklearnDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.x[idx], dtype=self._x_type), torch.tensor(
