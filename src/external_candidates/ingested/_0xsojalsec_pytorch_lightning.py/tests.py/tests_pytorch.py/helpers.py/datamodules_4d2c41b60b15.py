# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\helpers\datamodules.py
...
class MNISTDataModule(LightningDataModule):
    def __init__(
        self, data_dir: str = "./", batch_size: int = 32, use_trials: bool = False
    ) -> None:
        super().__init__()
...
class ClassifDataModule(SklearnDataModule):
    def __init__(
        ...
        super().__init__(
            data, x_type=torch.float32, y_type=torch.long, batch_size=batch_size
        )
...
class RegressDataModule(SklearnDataModule):
    def __init__(self, num_features=16, length=800, batch_size=10):
        ...
        x, y = make_regression(
            n_samples=length, n_features=num_features, random_state=42
        )
        y = [[v] for v in y]
        super().__init__(
            (x, y), x_type=torch.float32, y_type=torch.float32, batch_size=batch_size
        )
