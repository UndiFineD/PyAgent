# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\connectors\test_logger_connector.py
...
@patch(
    "lightning.pytorch.trainer.connectors.logger_connector.logger_connector.convert_tensors_to_scalars"
)
def test_uses_provided_step(mock_convert):
    ...
    logger.log_metrics.assert_called_once_with(
        metrics=mock_convert.return_value, step=step
    )
...
def test_uses_step_metric(mock_convert):
    ...
    logger.log_metrics.assert_called_once_with(
        metrics=mock_convert.return_value, step=int(step)
    )
    logger.save.assert_called_once_with()
...
def test_uses_batches_that_stepped(mock_convert):
    ...
    mock_convert.assert_called_once_with(metrics)
    logger.log_metrics.assert_called_once_with(
        metrics=mock_convert.return_value,
        step=trainer.fit_loop.epoch_loop._batches_that_stepped,
    )
    logger.save.assert_called_once_with()
    mock_convert.return_value.setdefault.assert_called_once_with(
