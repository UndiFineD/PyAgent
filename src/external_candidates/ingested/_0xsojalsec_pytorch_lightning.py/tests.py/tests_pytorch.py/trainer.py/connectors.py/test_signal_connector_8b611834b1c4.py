...
def test_auto_requeue_custom_signal_flag(auto_requeue, requeue_signal):
    trainer = Trainer(
        plugins=[
            SLURMEnvironment(auto_requeue=auto_requeue, requeue_signal=requeue_signal)
        ]
    )
    connector = _SignalConnector(trainer)
    connector.register_signal_handlers()
    ...
    if auto_requeue:
        ...
        assert (
            sigterm_handlers[1].__qualname__ == "_SignalConnector._sigterm_handler_fn"
        )

        sigusr_handlers = signal.getsignal(requeue_signal).signal_handlers
        assert len(sigusr_handlers) == 1
        assert (
            sigusr_handlers[0].__qualname__
            == "_SignalConnector._slurm_sigusr_handler_fn"
        )
    else:
        sigterm_handlers = signal.getsignal(signal.SIGTERM).signal_handlers
        assert len(sigterm_handlers) == 1
        assert (
            sigterm_handlers[0].__qualname__ == "_SignalConnector._sigterm_notifier_fn"
        )
...
def test_signal_connector_in_thread():
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        for future in concurrent.futures.as_completed(
            [executor.submit(_registering_signals)]
        ):
