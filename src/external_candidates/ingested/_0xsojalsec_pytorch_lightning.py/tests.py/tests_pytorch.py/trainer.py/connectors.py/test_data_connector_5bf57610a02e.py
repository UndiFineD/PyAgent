        ...
        def create_dataset(self):
            ...
            if self._mode == 1:
                ...
                batch_sampler = CustomBatchSampler(
                    SequentialSampler(dataset), batch_size=1, drop_last=True
                )
...
def test_worker_check(
    _, cpu_count_mock, num_workers, cpu_count, expected_warning, monkeypatch
):
    monkeypatch.delattr(
...
def test_worker_check_reload_dataloaders_every_n_epochs_limitations(_):
    ...
    dataloader = DataLoader(
        range(2), num_workers=1, pin_memory=True, persistent_workers=True
    )
    with pytest.warns(
        UserWarning, match="The combination of .*reload_dataloaders_every_n_epochs"
    ):
        _worker_check(trainer, dataloader=dataloader, name="train_dataloader")
...
def test_update_dataloader_raises():
    with pytest.raises(
        ValueError, match="needs to subclass `torch.utils.data.DataLoader"
    ):
...
def test_dataloader_reinit_for_subclass():
    ...
    result = trainer._data_connector._prepare_dataloader(
        CustomDummyObj(), shuffle=True, mode=mode
    )
    assert isinstance(result, CustomDummyObj), "Wrongly reinstantiated data loader"

    dataset = list(range(10))
    result = trainer._data_connector._prepare_dataloader(
        CustomDataLoader(dataset), shuffle=True, mode=mode
    )
    ...
    result = trainer._data_connector._prepare_dataloader(
        dataloader, shuffle=False, mode=mode
    )
...
def test_non_sequential_sampler_warning_is_raised_for_eval_dataloader(val_dl, warns):
    ...
    with context(
        PossibleUserWarning, match="recommended .* turn shuffling off for val/test"
    ):
        trainer.validate_loop.setup_data()
...
class TestDataHookSelector:
    ...
    def test_no_datamodule_no_overridden(self, hook_name):
        ...
        with no_warning_call(match=f"have overridden `{hook_name}` in"):
            instance = trainer._data_connector._datahook_selector.get_instance(
                hook_name
            )
    ...
    def test_with_datamodule_override_model(self, hook_name):
        ...
        setattr(model, hook_name, self.overridden_func)
        with pytest.warns(
            UserWarning, match=f"have overridden `{hook_name}` in `LightningModule`"
        ):
            instance = trainer._data_connector._datahook_selector.get_instance(
                hook_name
            )
...
@pytest.mark.parametrize(
    ("devices", "warn_context"), [(1, no_warning_call), (2, pytest.warns)]
)
def test_eval_distributed_sampler_warning(devices, warn_context):
    ...
    with warn_context(
        PossibleUserWarning, match="multi-device settings use `DistributedSampler`"
    ):
        trainer.validate_loop.setup_data()
...
def test_attach_data_input_validation_with_none_dataloader(
    trainer_fn_name, dataloader_name, tmp_path
):
...
def test_non_iterables_raise(
    tmp_path, trainer_fn_name, dataloader_name, stage, dataloader
):
    model = BoringModel()
