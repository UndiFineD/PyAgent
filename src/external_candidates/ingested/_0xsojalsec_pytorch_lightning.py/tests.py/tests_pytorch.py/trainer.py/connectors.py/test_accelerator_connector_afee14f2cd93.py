# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\connectors\test_accelerator_connector.py
...
def test_accelerator_invalid_choice():
    with pytest.raises(
        ValueError,
        match="You selected an invalid accelerator name: `accelerator='invalid'`",
    ):
...
def test_custom_accelerator(cuda_count_0):
    ...
    strategy = TestStrategy(
        device=torch.device("cpu"), accelerator=Accel(), precision_plugin=Prec()
    )
...
def test_interactive_incompatible_backend_error(cuda_count_2, monkeypatch):
    ...
    with pytest.raises(
        MisconfigurationException, match=r"strategy='ddp'\)`.*is not compatible"
    ):
        Trainer(strategy="ddp", accelerator="gpu", devices=2)

    with pytest.raises(
        MisconfigurationException, match=r"strategy='ddp_spawn'\)`.*is not compatible"
    ):
...
def test_accelerator_choice_multi_node_gpu(
    cuda_count_2, tmp_path, strategy, strategy_class, devices
):
    trainer = Trainer(
        default_root_dir=tmp_path,
        num_nodes=2,
...
def test_invalid_ddp_strategy_with_mps(
    accelerator, strategy, strategy_class, mps_count_1, cuda_count_0
):
    with pytest.raises(
...
@RunIf(skip_windows=True)
@mock.patch(
    "lightning.pytorch.trainer.connectors.accelerator_connector._IS_INTERACTIVE", True
)
...
@pytest.mark.parametrize(
    ("strategy", "expected_cls"), [("ddp", DDPStrategy), ("ddp_spawn", DDPStrategy)]
)
def test_strategy_choice_ddp_cuda(strategy, expected_cls, mps_count_0, cuda_count_2):
    trainer = Trainer(
        fast_dev_run=True, strategy=strategy, accelerator="gpu", devices=1
    )
    assert isinstance(trainer.accelerator, CUDAAccelerator)
...
def test_strategy_choice_ddp_slurm(cuda_count_2, strategy, job_name, expected_env):
    ...
    with mock.patch.dict(
        ...
        trainer = Trainer(
            fast_dev_run=True, strategy=strategy, accelerator="cuda", devices=2
        )
        assert isinstance(trainer.accelerator, CUDAAccelerator)
        assert isinstance(trainer.strategy, DDPStrategy)
...
def test_strategy_choice_ddp_kubeflow(_, __, mps_count_0, cuda_count_2):
    trainer = Trainer(
        fast_dev_run=True, accelerator="gpu", devices=2, plugins=KubeflowEnvironment()
    )
    assert isinstance(trainer.accelerator, CUDAAccelerator)
    assert isinstance(trainer.strategy, DDPStrategy)
...
def test_strategy_choice_ddp_cpu_kubeflow(cuda_count_0):
    trainer = Trainer(
        fast_dev_run=True, accelerator="cpu", devices=2, plugins=KubeflowEnvironment()
    )
    assert isinstance(trainer.accelerator, CPUAccelerator)
    assert isinstance(trainer.strategy, DDPStrategy)
    assert isinstance(trainer.strategy.cluster_environment, KubeflowEnvironment)
...
def test_strategy_choice_ddp_cpu_slurm(cuda_count_0, strategy):
    trainer = Trainer(
        fast_dev_run=True, strategy=strategy, accelerator="cpu", devices=2
    )
    assert isinstance(trainer.accelerator, CPUAccelerator)
    assert isinstance(trainer.strategy, DDPStrategy)
...
def test_check_fsdp_strategy_and_fallback():
    with pytest.raises(
        ValueError, match="The strategy `fsdp` requires a GPU accelerator"
    ):
...
def test_parallel_devices_in_strategy_confilict_with_accelerator(
    ...
    with pytest.raises(
        MisconfigurationException, match=r"parallel_devices set through"
    ):
        Trainer(
...
def test_benchmark_option(cudnn_benchmark, benchmark_, deterministic, expected):
    ...
    if benchmark_ and deterministic:
        with pytest.warns(
            UserWarning, match="You passed `deterministic=True` and `benchmark=True`"
        ):
...
def test_plugin_only_one_instance_for_one_type(plugins, expected):
    with pytest.raises(
        MisconfigurationException, match=f"Received multiple values for {expected}"
    ):
...
def test_passing_zero_and_empty_list_to_devices_flag(accelerator, devices):
    with pytest.raises(
        MisconfigurationException, match="value is not a valid input using"
    ):
...
@mock.patch(
    "lightning.pytorch.accelerators.mps.MPSAccelerator.is_available", return_value=False
)
@mock.patch(
    "lightning.pytorch.accelerators.cuda.CUDAAccelerator.is_available",
    return_value=False,
)
def test_gpu_accelerator_misconfiguration_exception(*_):
    with pytest.raises(
        MisconfigurationException, match="No supported gpu backend found!"
    ):
...
@pytest.mark.parametrize(
    ...
    [("DDP", DDPStrategy), ("DDP_FIND_UNUSED_PARAMETERS_FALSE", DDPStrategy)],
)
...
def test_connector_auto_selection(monkeypatch, is_interactive):
    ...
    assert isinstance(trainer.accelerator, CUDAAccelerator)
    assert isinstance(
        trainer.strategy, (SingleDeviceStrategy if is_interactive else DDPStrategy)
    )
    assert (
        trainer._accelerator_connector._devices_flag == [0]
        if is_interactive
        else list(range(4))
    )
...
def test_bitsandbytes_precision_cuda_required(monkeypatch):
    monkeypatch.setattr(
        lightning.fabric.plugins.precision.bitsandbytes, "_BITSANDBYTES_AVAILABLE", True
    )
    monkeypatch.setattr(
    ...
    with pytest.raises(
        ...
        _AcceleratorConnector(
            accelerator="cpu", plugins=BitsandbytesPrecision(mode="int8")
        )
