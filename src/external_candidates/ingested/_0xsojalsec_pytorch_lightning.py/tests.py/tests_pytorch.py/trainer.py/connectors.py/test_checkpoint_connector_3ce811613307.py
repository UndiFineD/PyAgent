# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\connectors\test_checkpoint_connector.py
...
def test_hpc_restore_attempt(_, tmp_path):
    ...
    model = BoringModel()
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=1, enable_checkpointing=False, logger=False
    )
    trainer.fit(model)
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=2, enable_checkpointing=False, logger=False
    )
    trainer.fit(model)
    ...
    with pytest.raises(
        FileNotFoundError, match="Checkpoint file not found: not existing"
    ):
...
def test_hpc_max_ckpt_version(tmp_path):
    ...
    assert trainer._checkpoint_connector._hpc_resume_path == str(
        tmp_path / "hpc_ckpt_33.ckpt"
    )
    assert (
...
def test_local_cross_device_checkpoint(tmpdir):
    ...
    trainer = Trainer(
        default_root_dir="memory://test_ckpt_for_fsspec",
...
def test_ckpt_for_fsspec():
    ...
    assert (
        trainer._checkpoint_connector._hpc_resume_path
        == "memory://test_ckpt_for_fsspec/hpc_ckpt_33.ckpt"
    )
    assert (
        trainer._checkpoint_connector._CheckpointConnector__max_ckpt_version_in_folder(
            "memory://test_ckpt_for_fsspec"
        )
        == 33
    )
    assert (
...
def test_stateful_trainer_ckpt_path_support(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, fast_dev_run=True, callbacks=model_checkpoint
    )
...
@pytest.mark.parametrize(
    ("strict_loading", "expected"), [(None, True), (True, True), (False, False)]
)
