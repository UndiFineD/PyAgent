...
def test_checkpoint_callbacks_are_last(tmp_path):
    ...
    checkpoint1 = ModelCheckpoint(
        tmp_path / "path1", filename="ckpt1", monitor="val_loss_c1"
    )
    checkpoint2 = ModelCheckpoint(
        tmp_path / "path2", filename="ckpt2", monitor="val_loss_c2"
    )
    ...
    # no model reference
    trainer = Trainer(
        callbacks=[checkpoint1, progress_bar, lr_monitor, model_summary, checkpoint2]
    )
...
def test_attach_model_callbacks():
    ...
    def _attach_callbacks(trainer_callbacks, model_callbacks):
        ...
        has_progress_bar = any(
            isinstance(cb, ProgressBar) for cb in trainer_callbacks + model_callbacks
        )
        trainer = Trainer(
    ...
    trainer = _attach_callbacks(
        trainer_callbacks=[early_stopping1], model_callbacks=[progress_bar]
    )
    ...
    trainer = _attach_callbacks(
        trainer_callbacks=[progress_bar], model_callbacks=[custom_progress_bar]
    )
    ...
    trainer = _attach_callbacks(
        trainer_callbacks=[bare_callback], model_callbacks=[custom_progress_bar]
    )
...
def test_attach_model_callbacks_override_info(caplog):
    ...
    assert (
        "existing callbacks passed to Trainer: EarlyStopping, LearningRateMonitor"
        in caplog.text
    )
...
def test_raising_error_validate_callbacks_list_function(
    callbacks: list, match_msg: str
):
