# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\connectors\test_rich_integration.py
...
class TestRichIntegration:
    @patch(
        "lightning.pytorch.trainer.connectors.callback_connector._RICH_AVAILABLE", False
    )
    def test_no_rich_defaults_tqdm_and_model_summary(self, tmp_path):
        trainer = Trainer(
            default_root_dir=tmp_path, logger=False, enable_checkpointing=False
        )
    ...
    def test_no_rich_respects_user_provided_model_summary(self, tmp_path):
        ...
        assert (
            trainer.callbacks[trainer.callbacks.index(user_model_summary)]
            == user_model_summary
        )
    ...
    def test_no_rich_respects_user_provided_rich_model_summary(self, tmp_path):
        ...
        assert sum(isinstance(cb, ModelSummary) for cb in trainer.callbacks) == 1
        # Check that the specific instance is the one from the trainer's list of ModelSummary callbacks
        model_summary_callbacks = [
            cb for cb in trainer.callbacks if isinstance(cb, ModelSummary)
        ]
        assert user_model_summary in model_summary_callbacks
        assert isinstance(model_summary_callbacks[0], RichModelSummary)

    @patch(
        "lightning.pytorch.trainer.connectors.callback_connector._RICH_AVAILABLE", True
    )
    ...
    def test_rich_progress_bar_tensor_metric(self, tmp_path):
        ...
        with patch(
            "lightning.pytorch.callbacks.progress.rich_progress.MetricsTextColumn.update"
        ) as mock_update:
            trainer.fit(model)
