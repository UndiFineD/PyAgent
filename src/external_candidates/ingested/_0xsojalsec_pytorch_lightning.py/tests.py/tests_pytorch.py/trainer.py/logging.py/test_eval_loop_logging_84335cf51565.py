# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\logging_\test_eval_loop_logging.py
...
@pytest.mark.parametrize(
    ("batches", "log_interval", "max_epochs"), [(1, 1, 1), (64, 32, 2)]
)
...
def test_eval_logging_auto_reduce(tmp_path):
    ...
    assert torch.allclose(
        trainer.logged_metrics["val_loss_epoch"], model.manual_epoch_end_mean
    )
    assert torch.allclose(
        trainer.callback_metrics["val_loss_epoch"], model.manual_epoch_end_mean
    )
    assert torch.allclose(
        trainer.callback_metrics["val_loss"], model.manual_epoch_end_mean
    )
...
def test_log_works_in_val_callback(tmp_path):
    ...
    class TestCallback(callbacks.Callback):
        ...
        def make_logging(self, pl_module, func_name, on_steps, on_epochs, prob_bars):
            ...
            for idx, (on_step, on_epoch, prog_bar) in enumerate(
                itertools.product(on_steps, on_epochs, prob_bars)
            ):
                fx = f"{func_name}_{idx}"
                if not on_step and not on_epoch:
                    with pytest.raises(
                        MisconfigurationException, match="is not useful"
                    ):
                        pl_module.log(
                            fx, self.count, on_step=on_step, on_epoch=on_epoch
                        )
                    continue
                pl_module.log(
                    fx,
...
def test_log_works_in_test_callback(tmp_path):
    ...
    for dl_idx in range(num_dataloaders):
        ...
        assert torch.stack(
            model.seen_losses[dl_idx]
        ).mean() == trainer.callback_metrics.pop(key)
...
def test_validation_step_log_with_tensorboard(mock_log_metrics, tmp_path):
    ...
    assert mock_log_metrics.mock_calls == [
        ...
        call(
            metrics={"valid_loss_0_epoch": ANY, "valid_loss_1": ANY, "epoch": 0}, step=0
        ),
        ...
        call(
            metrics={"valid_loss_0_epoch": ANY, "valid_loss_1": ANY, "epoch": 1}, step=1
        ),
    ]
    ...
    assert (
        get_metrics_at_idx(4)["valid_loss_1"]
        == torch.stack(model.val_losses[2:4]).mean()
    )
    ...
    assert (
        get_metrics_at_idx(8)["valid_loss_1"]
        == torch.stack(model.val_losses[4:]).mean()
    )
...
def test_multiple_dataloaders_reset(val_check_interval, tmp_path):
    class TestModel(BoringModel):
        ...
        def on_validation_epoch_end(self):
            ...
            assert (
                self.trainer._results["validation_step.val_loss.0"].cumulated_batch_size
                == 5
            )
            assert (
                self.trainer._results["validation_step.val_loss.1"].cumulated_batch_size
                == 5
            )
...
def test_logging_results_with_no_dataloader_idx(tmp_path):
    ...
    class CustomBoringModel(BoringModel):
        def test_step(self, batch, batch_idx, dataloader_idx):
            ...
            self.log_dict(
                log_key_dl0 if dataloader_idx == 0 else log_key_dl1,
                add_dataloader_idx=False,
            )
...
def test_logging_multi_dataloader_on_epoch_end(mock_log_metrics, tmp_path):
    ...
    assert results == [
        {"foo/dataloader_idx_0": 1, "foobar": 3},
        {"foo/dataloader_idx_1": 2, "foobar": 3},
    ]
...
inputs1 = (
    [
        ...
        {
            "different value": torch.tensor(1.5),
            "tes:t": {"no_log1": torch.tensor(6), "no_log2": torch.tensor(1)},
        },
    ],
    RunningStage.TESTING.value,
)
...
def test_eval_step_logging(mock_log_metrics, tmp_path, num_dataloaders):
    ...
    class CustomBoringModel(BoringModel):
        def validation_step(self, batch, batch_idx, dataloader_idx=None):
            self.log(
                f"val_log_{self.trainer.state.fn.value}",
                batch_idx,
                on_step=True,
                on_epoch=False,
            )
    ...
    fit_calls = [
        call(
            metrics={f"val_log_fit{get_suffix(dl_idx)}": float(step)},
            step=step + (limit_batches * epoch),
        )
