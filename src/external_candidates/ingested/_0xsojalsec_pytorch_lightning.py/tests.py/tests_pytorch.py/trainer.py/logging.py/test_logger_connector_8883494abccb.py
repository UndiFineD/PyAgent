# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\logging_\test_logger_connector.py
...
def test_fx_validator():
    ...
    for func_name in funcs_name:
        ...
        is_stage = (
            "train" in func_name or "test" in func_name or "validation" in func_name
        )
        ...
        if allowed:
            validator.check_logging_levels(
                fx_name=func_name, on_step=on_step, on_epoch=on_epoch
            )
            if not is_start and is_stage:
                with pytest.raises(MisconfigurationException, match="must be one of"):
                    validator.check_logging_levels(
                        fx_name=func_name, on_step=True, on_epoch=on_epoch
                    )
        else:
    ...
    def __init__(self, not_supported):
        def call(hook, trainer=None, model=None, *_, **__):
            ...
            if hook in not_supported:
                with pytest.raises(
                    MisconfigurationException, match=not_supported[hook]
                ):
...
def test_fx_validator_integration(tmp_path):
    ...
    not_supported.update(
        dict.fromkeys(not_supported, "result collection is not registered yet")
    )
    not_supported.update(
        {
    ...
    class TestModel(BoringModel):
        ...
        def validation_step(self, *args, **kwargs):
            ...
            name = (
                ...
                else f"val_loss_custom_naming_{args[-1]}"
            )
    ...
    class TestModel(LightningModule):
        ...
        def _create_metrics(self):
            ...
            ap = (
                AvgPre(task="binary")
                if _TM_GE_0_11
                else AvgPre(num_classes=1, pos_label=1)
            )
...
@pytest.mark.skipif(
    compare_version("torchmetrics", operator.lt, "0.8.0"),
    reason="torchmetrics>=0.8.0 required for compute groups",
)
    ...
    class DummyModule(LightningModule):
        def __init__(self):
            ...
            if compare_version("torchmetrics", operator.ge, "0.10.0"):
                ...
                metrics = [
                    Accuracy(num_classes=10, average="micro"),
                    Precision(num_classes=10, average="micro"),
                ]
...
def test_result_collection_on_tensor_with_mean_reduction():
    ...
    assert (
        result_collection["training_step.loss_on_step_on_epoch"].cumulated_batch_size
        == total_batches
    )
...
def test_logged_metrics_has_logged_epoch_value(tmp_path, logger):
    ...
    if not logger:
        with pytest.warns(
            match=r"log\('epoch', ..., logger=True\)` but have no logger"
        ):
            trainer.fit(model)
    else:
        trainer.fit(model)
