# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\logging_\test_train_loop_logging.py
...
def test__training_step__log(tmp_path):
    ...
    class TestModel(BoringModel):
        def training_step(self, batch, batch_idx):
            ...
            self.log(
                "l_s", loss, on_step=True, on_epoch=False, prog_bar=False, logger=True
            )

            # on_step F on_epoch T
            self.log(
                "l_e", loss, on_step=False, on_epoch=True, prog_bar=False, logger=True
            )

            # on_step T on_epoch T
            self.log(
                "l_se", loss, on_step=True, on_epoch=True, prog_bar=False, logger=True
            )
            ...
            # on_step T on_epoch F
            self.log(
                "p_s", loss, on_step=True, on_epoch=False, prog_bar=True, logger=False
            )

            # on_step F on_epoch T
            self.log(
                "p_e", loss, on_step=False, on_epoch=True, prog_bar=True, logger=False
            )

            # on_step T on_epoch T
            self.log(
                "p_se", loss, on_step=True, on_epoch=True, prog_bar=True, logger=False
            )
    ...
    assert set(trainer.callback_metrics) == (
        logged_metrics | pbar_metrics | {"p_se", "l_se"}
    )
...
@pytest.mark.parametrize(
    ("batches", "fx", "result"),
    [(3, min, 0), (3, torch.max, 2), (11, max, 10), (5, "avg", 2), (5, "SUM", 10)],
)
...
def test_log_works_in_train_callback(tmp_path):
    ...
    assert (
        trainer.progress_bar_callback.get_metrics(trainer, model)["train_loss"]
        == model.seen_losses[-1]
    )
...
class LoggingSyncDistModel(BoringModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "foo", value, on_step=True, on_epoch=False, sync_dist=True, reduce_fx="sum"
        )
        self.log(
            "foo_2", 2, on_step=True, on_epoch=False, sync_dist=True, reduce_fx="sum"
        )
        self.log(
            "foo_3", 2, on_step=True, on_epoch=False, sync_dist=True, reduce_fx="mean"
        )
        self.log(
            "foo_4",
            value,
        ...
        self.log(
            "foo_7", 2, on_step=False, on_epoch=True, sync_dist=True, reduce_fx="sum"
        )
        self.log(
            "foo_8", 2, on_step=False, on_epoch=True, sync_dist=True, reduce_fx="mean"
        )
        self.log(
            "foo_9",
            value,
...
def test_logging_sync_dist_true(tmp_path, devices, accelerator):
    ...
    assert (
        metrics["foo_5"] == fake_result * 2 + 1
        if use_multiple_devices
        else fake_result * 2
    )
    assert (
        metrics["foo_6"] == (0 + 1 + 1 + 2 + 2 + 3)
        if use_multiple_devices
        else fake_result * 3 * 2
    )
    assert metrics["foo_7"] == 2 * devices * 3
    assert metrics["foo_8"] == 2
    assert (
        metrics["foo_9"] == (fake_result * 2 + 1) / devices
        if use_multiple_devices
        else fake_result
    )
    assert metrics["foo_10"] == 2
    assert (
        metrics["foo_11_step"] == (2 + 3) / 2
        if use_multiple_devices
        else fake_result * 2
    )
    assert (
        metrics["foo_11"] == (0 + 1 + 1 + 2 + 2 + 3) / (devices * 3)
        if use_multiple_devices
        else fake_result
    )
...
def test_logging_sync_dist_true_ddp(tmp_path):
    ...
    class TestLoggingSyncDistModel(BoringModel):
        def training_step(self, batch, batch_idx):
            acc = self.step(batch[0])
            self.log(
                "foo", 1, on_step=False, on_epoch=True, sync_dist=True, reduce_fx="SUM"
            )
        ...
        def validation_step(self, batch, batch_idx):
            loss = self.step(batch)
            self.log(
                "bar", 2, on_step=False, on_epoch=True, sync_dist=True, reduce_fx="AVG"
            )
            return {"x": loss}
...
def test_metric_are_properly_reduced(tmp_path, accelerator):
    ...
    checkpoint = ModelCheckpoint(
        monitor="val_acc", save_last=True, save_top_k=2, mode="max"
    )
...
def test_logging_raises(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException,
        match="fix this by setting an attribute for the metric in your",
    ):
        trainer.fit(model)
    ...
    with pytest.raises(
        MisconfigurationException,
        match=r"reduce_fx={min,max,mean,sum}\)` are supported",
    ):
        trainer.fit(model)
...
def test_on_epoch_logging_with_sum_and_on_batch_start(tmp_path):
    class TestModel(BoringModel):
        def on_train_epoch_end(self):
            ...
            assert (
                self.trainer._results["on_train_epoch_end.on_train_epoch_end"].value
                == 3.0
            )
        ...
        def on_train_batch_end(self, *_):
            self.log(
                "on_train_batch_end", 1.0, on_step=False, on_epoch=True, reduce_fx="sum"
            )
...
def test_log_on_train_start(mock_log_metrics, tmp_path):
    ...
    assert mock_log_metrics.mock_calls == [
        call(metrics={"foo": 123.0, "epoch": 0}, step=0)
    ]
