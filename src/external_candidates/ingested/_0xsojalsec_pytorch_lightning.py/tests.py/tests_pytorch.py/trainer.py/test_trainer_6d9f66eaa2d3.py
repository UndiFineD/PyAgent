...
def test_no_val_module(monkeypatch, tmp_path, tmpdir_server, url_ckpt):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=1, limit_val_batches=1, logger=logger
    )
    # fit model
    trainer.fit(model)
    # training complete
    ...
    assert (
        LightningModule.CHECKPOINT_HYPER_PARAMS_KEY in ckpt
    ...
    model_2 = CustomModel.load_from_checkpoint(
        checkpoint_path=ckpt_path, hparams_file=hparams_path
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            -1, False, [f"epoch={i}.ckpt" for i in range(5)], id="CASE K=-1  (all)"
        ),
...
def test_model_checkpoint_only_weights(tmp_path):
    ...
    assert (
        "optimizer_states" not in checkpoint
    ), "checkpoint should contain only model weights"
    assert (
        "lr_schedulers" not in checkpoint
    ), "checkpoint should contain only model weights"
...
def test_fit_ckpt_path_epoch_restored(monkeypatch, tmp_path, tmpdir_server, url_ckpt):
    ...
    assert (
        model.num_batches_seen
        == trainer.num_training_batches * max_epochs
        == trainer.global_step
    )
...
def test_trainer_max_steps_and_epochs(tmp_path):
    ...
    ), "Model did not stop at max_epochs"
...
def test_trainer_min_steps_and_epochs(tmp_path):
    ...
    assert (
        trainer.global_step >= num_train_samples
    ...
    assert trainer.state.finished, f"Training failed with {trainer.state}"
    assert trainer.current_epoch > 0
    assert trainer.global_step >= math.floor(
...
def test_trainer_min_steps_and_min_epochs_not_reached(tmp_path, caplog):
    ...
    class TestModel(BoringModel):
        ...
        def training_step(self, batch, batch_idx):
            ...
            output["loss"] = (
                output["loss"] * 0.0
            )  # force minimal loss to trigger early stopping
...
def test_checkpoint_path_input_last_fault_tolerant(tmp_path, ckpt_path, fn):
    ...
    if ckpt_path == "last":
        ...
        ctxt = pytest.warns(
            UserWarning, match="There is also an on-exception checkpoint available"
        )
        final_path = "foobar"
...
def test_checkpoint_path_input(tmp_path, ckpt_path, save_top_k, fn):
    ...
    if ckpt_path == "best":
        ...
        if save_top_k == 0:
            with pytest.raises(
                ValueError, match=".*is not configured to save the best.*"
            ):
                trainer_fn(ckpt_path=ckpt_path)
            with pytest.raises(
                ValueError, match=".*is not configured to save the best.*"
...
def test_best_ckpt_evaluate_raises_warning_with_multiple_ckpt_callbacks():
    ...
    with pytest.warns(
        UserWarning, match="best checkpoint path from first checkpoint callback"
    ):
...
def test_disabled_training(tmp_path):
    ...
    assert (
        model.training_step_invoked
    ), "did not run `training_step` with `fast_dev_run=True`"
...
def test_keyboard_interrupt(tmp_path):
    ...
    trainer.strategy._launcher.launch = (
        lambda function, *args, trainer, **kwargs: function(*args, **kwargs)
    )
    ...
    class TestModel(ClassificationModel):
        def configure_gradient_clipping(self, *args, **kwargs):
            ...
            torch.testing.assert_close(
                grad_norm, torch.tensor(0.05, device=self.device)
            )
...
def test_gradient_clipping_by_value(tmp_path, precision):
    ...
    class TestModel(BoringModel):
        def configure_gradient_clipping(self, *args, **kwargs):
            ...
            torch.testing.assert_close(
                grad_max.abs(), torch.tensor(1e-10, device=self.device)
            )
            self.assertion_called = True
...
def test_invalid_gradient_clip_value(tmp_path):
    with pytest.raises(
        TypeError, match="`gradient_clip_val` should be an int or a float"
    ):
...
def test_log_every_n_steps(
    log_metrics_mock, tmp_path, train_batches, max_steps, log_interval
):
    class TestModel(BoringModel):
        def training_step(self, *args, **kwargs):
            self.log("foo", -1)
    ...
    trainer.fit(model)
    expected_calls = [
        call(metrics=ANY, step=s)
        for s in range(log_interval - 1, max_steps, log_interval)
    ]
...
def test_spawn_predict_return_predictions(tmp_path):
    ...
    with pytest.raises(
        ...
        trainer.predict(
            model, dataloaders=model.train_dataloader(), return_predictions=True
        )
...
def test_predict_return_predictions_cpu(return_predictions, precision, tmp_path):
    ...
    if return_predictions or return_predictions is None:
        ...
        assert preds[0].dtype == (
            torch.float64 if precision == "64-true" else torch.float32
        )


@pytest.mark.parametrize(
    ("max_steps", "max_epochs", "global_step"), [(10, 5, 10), (20, None, 20)]
)
def test_repeated_fit_calls_with_max_epochs_and_steps(
    tmp_path, max_steps, max_epochs, global_step
):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=max_steps, max_epochs=max_epochs
    )
...
def test_train_loop_system(tmp_path):
    ...
    assert (
        called_methods
        == ["step", "training_step", "zero_grad", "backward"]
...
def test_fit_test_synchronization(tmp_path):
    ...
    checkpoint = ModelCheckpoint(
        dirpath=tmp_path, monitor="x", mode="min", save_top_k=1
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        max_epochs=2,
...
def test_multiple_trainer_constant_memory_allocated(tmp_path):
    ...
    assert list(trainer.optimizers[0].state.values())[0][
        "exp_avg_sq"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"strategy": "ddp"}, DDPStrategy, CPUAccelerator, 1, marks=RunIf(mps=False)
        ),
        pytest.param(
...
def test_dataloaders_are_not_loaded_if_disabled_through_limit_batches(running_stage):
    ...
    dataloader_attribute = (
        f"{dl_prefix}_dataloader{'' if running_stage == 'train' else 's'}"
    )
