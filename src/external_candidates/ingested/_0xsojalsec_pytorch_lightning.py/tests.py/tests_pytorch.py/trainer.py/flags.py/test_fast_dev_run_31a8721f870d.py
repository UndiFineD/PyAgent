# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\flags\test_fast_dev_run.py
import os
...
def test_skip_on_fast_dev_run_tuner(tmp_path):
    ...
    with pytest.warns(
        UserWarning,
        match="Skipping learning rate finder since `fast_dev_run` is enabled.",
    ):
        tuner.lr_find(model)

    with pytest.warns(
        UserWarning, match="Skipping batch size scaler since `fast_dev_run` is enabled."
    ):
...
def test_callbacks_and_logger_not_called_with_fastdevrun(tmp_path, fast_dev_run):
    ...
    class FastDevRunModel(BoringModel):
        ...
        def training_step(self, batch, batch_idx):
            ...
            self.logger.experiment.dummy_log(
                "some_distribution", torch.randn(7) + batch_idx
            )
    ...
    def _make_fast_dev_run_assertions(trainer, model):
        ...
        assert (
            model.validation_step_call_count == 0
            if model.validation_step is None
            else fast_dev_run
        )
        assert (
            model.on_validation_epoch_end_call_count == 0
            if model.validation_step is None
            else 1
        )
