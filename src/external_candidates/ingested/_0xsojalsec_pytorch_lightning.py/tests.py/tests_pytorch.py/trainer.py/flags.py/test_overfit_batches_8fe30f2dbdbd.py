# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\flags\test_overfit_batches.py
...
def test_overfit_basic(tmp_path, overfit_batches):
    ...
    assert trainer.num_training_batches == int(
        overfit_batches
        * (1 if isinstance(overfit_batches, int) else total_train_samples)
    )
...
def test_overfit_batches_raises_warning_in_case_of_sequential_sampler(tmp_path):
    ...
    with pytest.warns(
        UserWarning, match="requested to overfit but enabled train dataloader shuffling"
    ):
        trainer.fit(model)
...
def test_overfit_batch_limits_train(overfit_batches):
    ...
    expected_batches = (
        int(overfit_batches * full_train_samples)
        if isinstance(overfit_batches, float)
        else overfit_batches
    )
