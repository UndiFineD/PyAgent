# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\flags\test_val_check_interval.py
...
def test_validation_check_interval_exceed_data_length_correct(
    tmp_path, use_infinite_dataset, accumulate_grad_batches
):
...
def test_validation_check_interval_exceed_data_length_wrong():
    ...
    with pytest.raises(
        ValueError,
        match="must be less than or equal to the number of the training batches",
    ):
        trainer.fit(model)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            2,
            [0, 1, 0, 1, 0],
            "val_check_interval timer only, no epoch gating",
        ),
        (
            1,
            "00:00:00:06",
        ...
        (
            ...
            9,
            [0, 2, 0, 2, 0],
            "epoch gating, timer shorter than epoch",
        ),
        (
            2,
            "00:00:00:03",
            9,
            [0, 3, 0, 3, 0],
            "epoch gating, timer much shorter than epoch",
        ),
    ...
    class TestModel(BoringModel):
        ...
        def on_train_epoch_end(self, *args, **kwargs):
            ...
            self.val_batches.append(
                trainer.fit_loop.epoch_loop.val_loop.batch_progress.total.completed
            )
