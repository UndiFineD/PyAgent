# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\flags\test_min_max_epochs.py
import pytest
...
def test_max_epochs_not_set_warning(tmp_path):
    ...
    model = CustomModel()
    trainer = Trainer(
        logger=False, enable_checkpointing=False, max_epochs=None, limit_train_batches=1
    )
