# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\test_config_validator.py
...
def test_wrong_train_setting(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException, match=r"No `training_step\(\)` method defined."
    ):
        model = BoringModel()
...
def test_eval_loop_config(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException, match=r"No `validation_step\(\)` method defined"
    ):
    ...
    with pytest.raises(
        MisconfigurationException, match=r"No `test_step\(\)` method defined"
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        MisconfigurationException, match=r"`predict_step` cannot be None."
    ):
    ...
    model.forward = None
    with pytest.raises(
        MisconfigurationException, match=r"requires `forward` method to run."
    ):
        trainer.predict(model)
...
def test_trainer_manual_optimization_config():
    ...
    with pytest.raises(
        MisconfigurationException, match="Automatic gradient clipping is not supported"
    ):
        trainer.fit(model)
...
def test_legacy_epoch_end_hooks():
    ...
    trainer = Mock()
    with pytest.raises(
        NotImplementedError, match="training_epoch_end` has been removed in v2.0"
    ):
        __verify_train_val_loop_configuration(trainer, TrainingEpochEndModel())
    with pytest.raises(
        NotImplementedError, match="validation_epoch_end` has been removed in v2.0"
    ):
    ...
    with pytest.raises(
        NotImplementedError, match="test_epoch_end` has been removed in v2.0"
    ):
