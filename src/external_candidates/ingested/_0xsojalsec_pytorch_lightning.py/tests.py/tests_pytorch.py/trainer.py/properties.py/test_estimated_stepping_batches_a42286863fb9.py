...
@pytest.mark.parametrize(
    ("accumulate_grad_batches", "expected_steps"), [(2, 32), (3, 22)]
)
def test_num_stepping_batches_accumulate_gradients(
...
def test_num_stepping_batches_gpu(trainer_kwargs, estimated_steps, monkeypatch):
    ...
    trainer = Trainer(
        max_epochs=1, devices=num_devices_per_node, accelerator="gpu", **trainer_kwargs
    )
    ...
    trainer.strategy.parallel_devices = [
        torch.device("cpu", index=i) for i in range(num_devices_per_node)
    ]
...
class MultiprocessModel(BoringModel):
    def on_train_start(self):
        assert (
            ...
            == len(self.train_dataloader()) // self.trainer.world_size
        )
