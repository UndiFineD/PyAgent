# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\properties\test_log_dir.py
...
def test_log_dir_no_checkpoint_cb(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=2, enable_checkpointing=False
    )
...
def test_log_dir_no_logger_no_checkpoint(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=2, logger=False, enable_checkpointing=False
    )
...
def test_log_dir_custom_callback(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path,
        max_steps=2,
        callbacks=[ModelCheckpoint(dirpath=(tmp_path / "ckpts"))],
    )
...
def test_log_dir_multiple_loggers(tmp_path):
    ...
    trainer = Trainer(
        ...
        max_steps=2,
        logger=[
            TensorBoardLogger(save_dir=save_dir, name="custom_logs"),
            CSVLogger(tmp_path),
        ],
    )
