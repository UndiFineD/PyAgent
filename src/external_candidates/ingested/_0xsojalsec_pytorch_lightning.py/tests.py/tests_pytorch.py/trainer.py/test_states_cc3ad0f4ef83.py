# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\test_states.py
...
def test_initialize_state():
    ...
    trainer = Trainer()
    assert trainer.state == TrainerState(
        status=TrainerStatus.INITIALIZING, fn=None, stage=None
    )
...
@pytest.mark.parametrize(
    "extra_params",
    [
        pytest.param({"fast_dev_run": True}, id="Fast-Run"),
        pytest.param({"max_steps": 1}, id="Single-Step"),
    ],
)
...
def test_interrupt_state_on_keyboard_interrupt(tmp_path, extra_params):
    ...
    trainer = Trainer(
        callbacks=[InterruptCallback()], default_root_dir=tmp_path, **extra_params
    )
