# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\trainer\optimization\test_backward_calls.py
...
def test_backward_count_with_grad_accumulation(torch_backward):
    ...
    trainer = Trainer(
        max_steps=6, accumulate_grad_batches=2, logger=False, enable_checkpointing=False
    )
    trainer.fit(model)
...
def test_backward_count_with_closure(torch_backward, tmp_path):
    ...
    trainer = Trainer(
        max_steps=5, accumulate_grad_batches=2, logger=False, enable_checkpointing=False
    )
    trainer.fit(model)
