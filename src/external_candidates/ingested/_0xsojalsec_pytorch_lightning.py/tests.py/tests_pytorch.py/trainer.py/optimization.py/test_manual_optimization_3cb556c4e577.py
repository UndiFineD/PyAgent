...
def test_multiple_optimizers_manual_call_counts(tmp_path, kwargs):
    ...
    with mock.patch.object(
        Strategy, "backward", wraps=trainer.strategy.backward
    ) as bwd_mock:
        trainer.fit(model)
...
@pytest.mark.parametrize(
    "accelerator", [pytest.param("gpu", marks=RunIf(min_cuda_gpus=1))]
)
    ...
    def on_train_batch_end(self, *_):
        ...
        if self.should_update:
            ...
            # todo: specify the possible exception
                ...
                assert (
                    torch.abs(
                        torch.sum(self.weight_before) - torch.sum(after_before)
                    ).item()
                    < 10e-6
                )
        assert_emtpy_grad(self.layer.weight.grad)
...
def test_step_with_optimizer_closure(tmp_path):
    ...
    assert (
        trainer.progress_bar_metrics["train_loss_epoch"]
        == torch.stack(model._losses).mean()
    )
...
def test_step_with_optimizer_closure_with_different_frequencies(
    mock_sgd_step, mock_adam_step, tmp_path
):
    class TestModel(BoringModel):
        def __init__(self):
            super().__init__()
    ...
    trainer.fit(model)
    assert mock_sgd_step.mock_calls == [
        call(closure=ANY, optim="sgd") for _ in range(4)
    ]
...
class TesManualOptimizationDDPModel(BoringModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        def gen_closure():
            ...
            make_manual_backward(
                loss_ones_gen, make_optimizer_step=make_gen_optimizer_step
            )
        ...
        def dis_closure():
            ...
            make_manual_backward(
                loss_ones_gen, make_optimizer_step=make_dis_optimizer_step
            )
...
def train_manual_optimization(
    tmp_path, strategy, model_cls=TesManualOptimizationDDPModel
):
    seed_everything(42)
...
def test_step_with_optimizer_closure_with_different_frequencies_ddp_with_toggle_model(
    ...
    train_manual_optimization(
        tmp_path, "ddp", model_cls=TestManualOptimizationDDPModelToggleModel
    )
...
def test_lr_schedulers_reduce_lr_on_plateau(tmp_path, scheduler_as_dict):
    ...
    if scheduler_as_dict:
        with pytest.warns(
            RuntimeWarning, match=r"\['monitor'\], but the keys will be ignored"
        ):
            trainer.fit(model)
...
def test_manual_optimization_with_non_pytorch_scheduler(automatic_optimization):
    ...
    model = Model()
    trainer = Trainer(
        accelerator="cpu", max_epochs=0, logger=False, enable_checkpointing=False
    )
    if automatic_optimization:
        with pytest.raises(
            MisconfigurationException, match="doesn't follow PyTorch's LRScheduler"
        ):
            trainer.fit(model)
    else:
