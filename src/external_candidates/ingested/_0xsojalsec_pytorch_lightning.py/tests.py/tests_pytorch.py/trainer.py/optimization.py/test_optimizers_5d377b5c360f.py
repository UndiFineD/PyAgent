...
def test_onecyclelr_with_epoch_interval_warns():
    ...
    with pytest.warns(
        RuntimeWarning, match="Are you sure you didn't mean 'interval': 'step'?"
    ):
...
def test_lr_scheduler_strict(step_mock, tmp_path, complete_epoch):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=max_epochs, max_steps=max_steps
    )
...
def test_unknown_configure_optimizers_raises(tmp_path):
    ...
    trainer = Trainer(default_root_dir=tmp_path, fast_dev_run=True)
    with pytest.raises(
        MisconfigurationException, match="Unknown configuration for model optimizers"
    ):
        trainer.fit(model)
...
def test_lr_scheduler_with_extra_keys_warns(tmp_path):
    ...
    trainer = Trainer(default_root_dir=tmp_path, fast_dev_run=True)
    with pytest.warns(
        RuntimeWarning, match=r"Found unsupported keys in the lr scheduler dict: \{.+\}"
    ):
        trainer.fit(model)
...
def test_lr_scheduler_epoch_step_frequency(
    mocked_sched, check_val_every_n_epoch, tmp_path
):
    epochs = 4
...
@pytest.mark.parametrize(
    ("every_n_train_steps", "epoch_interval"), [(None, True), (2, False), (2, True)]
)
def test_lr_scheduler_state_updated_before_saving(
    tmp_path, every_n_train_steps, epoch_interval
):
    batches = 2
    max_epochs = 1
    lr, gamma = 1, 10
    trainer = Trainer(
        ...
        limit_val_batches=1,
        callbacks=[
            ModelCheckpoint(dirpath=tmp_path, every_n_train_steps=every_n_train_steps)
        ],
    )
    ...
    class TestModel(BoringModel):
        ...
        def on_save_checkpoint(self, checkpoint):
            ...
            assert (
                lr_scheduler_config["_step_count"] - 1 == compare_to
            )  # step count starts at 1
...
def test_plateau_scheduler_lr_step_interval_updated_after_saving(
    tmp_path, save_on_train_epoch_end
):
    batches = 4
    trainer = Trainer(
        default_root_dir=tmp_path,
        ...
        callbacks=[
            ModelCheckpoint(
                dirpath=tmp_path, save_on_train_epoch_end=save_on_train_epoch_end
            )
        ],
    )
    ...
    class Model(BoringModel):
        ...
        def on_save_checkpoint(self, checkpoint):
            ...
            assert last_epoch == batches - (
                not save_on_train_epoch_end
            )  # last epoch starts at 0

            lr_scheduler_config_2 = checkpoint["lr_schedulers"][1]
            assert (
                lr_scheduler_config_2["_step_count"] - 1 == batches
            )  # step count starts at 1
...
def test_invalid_scheduler_missing_state_dict():
    ...
    with pytest.raises(
        TypeError, match="provided lr scheduler `CustomScheduler` is invalid"
    ):
...
def test_invalid_lr_scheduler_with_custom_step_method(override):
    ...
    class CustomScheduler:
        ...
        def step(
            self, foobar
        ):  # breaks the API, forces user to override `lr_scheduler_step`
            ...
    ...
    if override:
        ...
        with pytest.raises(
            MisconfigurationException, match="CustomScheduler` doesn't follow"
        ):
            _init_optimizers_and_lr_schedulers(model)
