# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_finetuning_callback.py
...
def test_finetuning_callback(tmp_path):
    ...
    class FinetuningBoringModel(BoringModel):
        def __init__(self):
            ...
            self.backbone = nn.Sequential(
                nn.Linear(32, 32, bias=False), nn.BatchNorm1d(32), nn.ReLU()
            )
    ...
    callback = TestBackboneFinetuningCallback(
        unfreeze_backbone_at_epoch=3, verbose=False
    )
...
def test_finetuning_callback_warning(tmp_path):
    ...
    callback = TestBackboneFinetuningWarningCallback(
        unfreeze_backbone_at_epoch=3, verbose=False
    )
...
def test_freeze_unfreeze_function(tmp_path):
    ...
    class FreezeModel(LightningModule):
        def __init__(self):
            super().__init__()
            self.backbone = nn.Sequential(
                nn.Linear(32, 32), nn.BatchNorm1d(32), nn.ReLU(), nn.Linear(32, 2)
            )
...
def test_unfreeze_and_add_param_group_function(tmp_path):
    ...
    with pytest.warns(UserWarning, match="The provided params to be frozen already"):
        BaseFinetuning.unfreeze_and_add_param_group(
            model.backbone[0], optimizer=optimizer
        )
    ...
    with pytest.warns(UserWarning, match="The provided params to be frozen already"):
        BaseFinetuning.unfreeze_and_add_param_group(
            model, optimizer=optimizer, lr=100, train_bn=False
        )
    ...
    for group_idx, group in enumerate(optimizer.param_groups):
        if group_idx == 0:
            assert torch.equal(
                optimizer.param_groups[0]["params"][0], model.backbone[0].weight
            )
        if group_idx == 2:
            assert torch.equal(
                optimizer.param_groups[2]["params"][0], model.backbone[2].weight
            )
            assert torch.equal(
                optimizer.param_groups[2]["params"][1], model.backbone[3].weight
            )
            assert torch.equal(
                optimizer.param_groups[2]["params"][2], model.backbone[4].weight
            )
...
def test_base_finetuning_internal_optimizer_metadata(tmp_path):
    ...
    assert cb._internal_optimizer_metadata[0][1]["params"] == [
        "layer.1.weight",
        "layer.1.bias",
    ]
    assert cb._internal_optimizer_metadata[0][2]["params"] == ["layer.2.weight"]
    assert cb._internal_optimizer_metadata[0][3]["params"] == [
        "layer.3.weight",
        "layer.3.bias",
    ]
    assert cb._internal_optimizer_metadata[0][4]["params"] == ["layer.4.weight"]
    assert cb._internal_optimizer_metadata[0][5]["params"] == [
        "layer.5.weight",
        "layer.5.bias",
    ]
...
class ConvBlockParam(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.module_dict = nn.ModuleDict(
            {"conv": nn.Conv2d(in_channels, out_channels, 3), "act": nn.ReLU()}
        )
...
def test_complex_nested_model():
    ...
    assert (
        not model.encoder[0].module_dict["conv"].weight.requires_grad
    )  # Validate a leaf module parameter is frozen
    assert not model.encoder[
        0
    ].parent_param.requires_grad  # Validate the parent module parameter is frozen
...
class FinetuningBoringModel(BoringModel):
    def __init__(self):
        super().__init__()
        self.layer = nn.Sequential(
            nn.Linear(32, 32), nn.Linear(32, 32), nn.Linear(32, 32), nn.Linear(32, 2)
        )
...
def test_unsupported_strategies(tmp_path):
    ...
    with pytest.raises(
        NotImplementedError,
        match="does not support running with the DeepSpeed strategy",
    ):
