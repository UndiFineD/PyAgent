...
def test_lr_monitor_single_lr(tmp_path):
    ...
    assert all(
        v is None for v in lr_monitor.last_momentum_values.values()
...
def test_lr_monitor_single_lr_with_momentum(tmp_path, opt: str):
    ...
    class LogMomentumModel(BoringModel):
        ...
        def configure_optimizers(self):
            ...
            optimizer = getattr(optim, self.opt)(
                self.parameters(), lr=1e-2, **opt_kwargs
            )
            lr_scheduler = optim.lr_scheduler.OneCycleLR(
                optimizer, max_lr=1e-2, total_steps=10_000
            )
    ...
    assert all(
        v is not None for v in lr_monitor.last_momentum_values.values()
    ...
    assert all(
        k == f"lr-{opt}-weight_decay" for k in lr_monitor.last_weight_decay_values
    )
...
def test_log_momentum_no_momentum_optimizer(tmp_path):
    ...
    assert all(
        v == 0 for v in lr_monitor.last_momentum_values.values()
    ), "Expected momentum to be logged"
    assert len(lr_monitor.last_momentum_values) == len(trainer.lr_scheduler_configs)
...
def test_lr_monitor_no_logger(tmp_path):
    ...
    lr_monitor = LearningRateMonitor()
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=1, callbacks=[lr_monitor], logger=False
    )
    ...
    class TestModel(BoringModel):
        def configure_optimizers(self):
            optimizer = torch.optim.SGD(
                [{"params": list(self.layer.parameters()), "name": "linear"}], lr=0.1
            )
        ...
        def finetune_function(self, pl_module, epoch: int, optimizer):
            ...
            if epoch == 1 and isinstance(optimizer, torch.optim.SGD):
                self.unfreeze_and_add_param_group(
                    pl_module.backbone[0], optimizer, lr=0.1
                )
            ...
            if epoch == 3 and type(optimizer) is torch.optim.Adam:
                ...
                self.unfreeze_and_add_param_group(
                    pl_module.backbone[1], optimizer, lr=0.1
                )
...
def test_lr_monitor_multiple_param_groups_no_lr_scheduler(tmp_path):
    ...
    class TestModel(BoringModel):
        ...
        def configure_optimizers(self):
            ...
            return torch.optim.Adam(
                param_groups, lr=self.hparams.lr, betas=self.hparams.momentum
            )
    ...
    assert all(
        val == weight_decay for val in lr_monitor.last_weight_decay_values.values()
    )
    assert all(
        all(val == lr for val in lr_monitor.lrs[lr_key]) for lr_key in lr_monitor.lrs
    )
