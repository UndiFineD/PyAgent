# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_spike.py
import contextlib
...
class MyTrainerSpikeDetection(SpikeDetection):
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        context = (
            pytest.raises(TrainingSpikeException)
            if batch_idx == 4 and self.should_raise
...
@pytest.mark.skipif(
    not _TORCHMETRICS_GREATER_EQUAL_1_0_0, reason="requires torchmetrics>=1.0.0"
)
def test_trainer_spike_detection_integration(
    tmp_path, global_rank_spike, num_devices, spike_value, finite_only
):
