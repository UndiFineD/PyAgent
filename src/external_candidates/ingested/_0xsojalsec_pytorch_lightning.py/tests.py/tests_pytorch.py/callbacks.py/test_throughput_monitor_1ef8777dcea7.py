...
def test_throughput_monitor_variable_batch_size(tmp_path):
    ...
    monitor = ThroughputMonitor(
        batch_size_fn=variable_batch_size_fn, window_size=5, separator="|"
    )
    ...
    for i, log_call in enumerate(log_calls):
        metrics = (
            log_call.kwargs["metrics"]
            if "metrics" in log_call.kwargs
            else log_call.args[0]
        )
        expected_samples = expected_cumulative_samples[i]
        assert (
            metrics["train|samples"] == expected_samples
        ), f"Step {i}: expected {expected_samples}, got {metrics['train|samples']}"
        assert (
            metrics["train|batches"] == i + 1
        ), f"Step {i}: expected batches {i + 1}, got {metrics['train|batches']}"
...
def test_throughput_monitor_variable_batch_size_with_validation(tmp_path):
    ...
    def variable_batch_size_fn(batch):
        if hasattr(batch, "size") and batch.size(0) > 0:
            if train_call_count[0] < len(train_batch_sizes):
                ...
                return current_batch_size
            current_batch_size = val_batch_sizes[
                val_call_count[0] % len(val_batch_sizes)
            ]
            val_call_count[0] += 1
            return current_batch_size
        return 1
    ...
    log_calls = logger_mock.log_metrics.call_args_list
    train_calls = [
        call for call in log_calls if "train/" in str(call) or "train|" in str(call)
    ]
    val_calls = [
        call
        for call in log_calls
        if "validate/" in str(call) or "validate|" in str(call)
    ]
    ...
    for train_call in train_calls:
        metrics = train_call.kwargs.get(
            "metrics", train_call.args[0] if train_call.args else {}
        )
        ...
        for k, v in metrics.items():
            if k.endswith("train/time"):
                ...
                if end_train_timings_idx < len(timings):
                    cur_train += (
                        timings[end_train_timings_idx]
                        - timings[start_train_timings_idx]
                    )
