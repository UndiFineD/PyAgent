# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_gradient_accumulation_scheduler.py
...
def test_trainer_accumulate_grad_batches_zero_grad(tmp_path, accumulate_grad_batches):
    with patch("torch.optim.SGD.zero_grad") as sgd_zero_grad:
        ...
        trainer.fit(model)
        assert sgd_zero_grad.call_count == math.ceil(
            trainer.limit_train_batches / accumulate_grad_batches
        )
...
def test_trainer_accumulate_grad_batches_with_callback(
    tmp_path, accumulate_grad_batches, expected_call_count
):
    with patch("torch.optim.SGD.zero_grad") as sgd_zero_grad:
        model = BoringModel()
        trainer = Trainer(
...
def test_invalid_values_for_grad_accum_scheduler(scheduling):
    with pytest.raises(
        MisconfigurationException, match="Accumulation factor should be an int"
    ):
        _ = GradientAccumulationScheduler(scheduling=scheduling)
...
def test_unsupported_strategies(strategy_class):
    ...
    with pytest.raises(
        RuntimeError,
        match="does not support `accumulate_grad_batches` changing between epochs",
    ):
...
def test_unsupported_manual_optimization():
    ...
    with pytest.raises(
        RuntimeError,
        match="Automatic gradient accumulation and the `GradientAccumulationScheduler`",
    ):
        scheduler.on_train_start(trainer, model)
...
def test_raises_when_accumulate_grad_batches_with_callback(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match="`accumulate_grad_batches` and are using the `GradientAccumulationScheduler`",
    ):
