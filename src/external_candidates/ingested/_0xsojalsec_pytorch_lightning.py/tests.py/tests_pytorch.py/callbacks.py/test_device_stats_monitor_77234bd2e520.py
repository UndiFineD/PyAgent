# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_device_stats_monitor.py
...
@mock.patch(
    "lightning.pytorch.accelerators.cpu.get_cpu_stats", side_effect=get_cpu_stats
)
def test_device_stats_cpu(cpu_stats_mock, tmp_path, cpu_stats):
    ...
    expected = (
        4 if cpu_stats is not False else 0
    )  # (batch_start + batch_end) * train_batches
...
@RunIf(tpu=True)
@pytest.mark.xfail(
    raises=RuntimeError, reason="`xm.get_memory_info` is not implemented with PJRT"
)
...
def test_device_stats_monitor_no_logger(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException, match="Cannot use `DeviceStatsMonitor` callback."
    ):
        trainer.fit(model)
