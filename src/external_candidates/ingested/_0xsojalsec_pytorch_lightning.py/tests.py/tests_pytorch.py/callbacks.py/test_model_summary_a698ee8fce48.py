# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_model_summary.py
...
def test_model_summary_callback_with_enable_model_summary_true():
    ...
    model_summary_callback = list(
        filter(lambda cb: isinstance(cb, ModelSummary), trainer.callbacks)
    )[0]
...
def test_custom_model_summary_callback_summarize(tmp_path):
    ...
    model = BoringModel()
    trainer = Trainer(
        default_root_dir=tmp_path, callbacks=CustomModelSummary(), max_steps=1
    )
