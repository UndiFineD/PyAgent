# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_model_checkpoint_additional_cases.py
import math
import os
...
def test_model_checkpoint_time_interval_with_val_metric_defers_until_validation(
    tmp_path, val_scores
):
...
def test_model_checkpoint_defer_until_next_validation_when_val_every_2_epochs(
    tmp_path, val_scores
):
...
def test_model_checkpoint_save_last_link_symlink_bug(tmp_path):
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                dirpath=tmp_path, every_n_epochs=10, save_last="link", save_top_k=-1
            )
        ],
        enable_checkpointing=True,
        enable_model_summary=False,
