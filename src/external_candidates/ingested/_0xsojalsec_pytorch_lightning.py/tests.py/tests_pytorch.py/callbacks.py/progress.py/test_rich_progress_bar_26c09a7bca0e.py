# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\progress\test_rich_progress_bar.py
...
@pytest.mark.parametrize(
    "dataset", [RandomDataset(32, 64), RandomIterableDataset(32, 64)]
)
def test_rich_progress_bar(tmp_path, dataset):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.progress.rich_progress.Progress.update"
    ) as mocked:
        trainer.fit(model)
...
def test_rich_progress_bar_import_error(monkeypatch):
    ...
    with pytest.raises(
        ModuleNotFoundError, match="`RichProgressBar` requires `rich` >= 10.2.2."
    ):
        RichProgressBar()
...
def test_rich_progress_bar_keyboard_interrupt(tmp_path):
    ...
    with (
        mock.patch(
            "lightning.pytorch.callbacks.progress.rich_progress.Progress.stop",
            autospec=True,
...
def test_rich_progress_bar_leave(tmp_path, leave, reset_call_count):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.progress.rich_progress.Progress.reset",
        autospec=True,
...
def test_rich_progress_bar_with_refresh_rate(
    ...
    with mock.patch.object(
        trainer.progress_bar_callback.progress,
        "update",
    ...
    assert metrics_update.call_count == train_batches + (
...
def test_rich_progress_bar_num_sanity_val_steps(tmp_path, limit_val_batches):
    ...
    trainer.fit(model)
    assert progress_bar.progress.tasks[0].completed == min(
        num_sanity_val_steps, limit_val_batches
    )
    assert progress_bar.progress.tasks[0].total == min(
        num_sanity_val_steps, limit_val_batches
    )
...
def test_rich_progress_bar_metrics_fast_dev_run(tmp_path):
    ...
    progress_bar = RichProgressBar()
    trainer = Trainer(
        default_root_dir=tmp_path, fast_dev_run=True, callbacks=progress_bar
    )
    model = BoringModel()
    trainer.fit(model)
...
def test_rich_progress_bar_disabled(tmp_path):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.progress.rich_progress.CustomProgress"
    ) as mocked:
        trainer.fit(model)
        trainer.validate(model)
        trainer.test(model)
...
def test_rich_progress_bar_metrics_format(tmp_path, metrics_format):
    ...
    progress_bar = RichProgressBar(
        theme=RichProgressBarTheme(metrics_format=metrics_format)
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        fast_dev_run=True,
...
def test_rich_progress_bar_metrics_theme_update(*_):
    ...
    theme = RichProgressBar(
        theme=RichProgressBarTheme(metrics_format=".3e", metrics_text_delimiter="\n")
    ).theme
