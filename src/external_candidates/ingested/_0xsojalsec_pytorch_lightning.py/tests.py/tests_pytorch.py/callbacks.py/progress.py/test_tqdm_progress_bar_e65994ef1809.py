# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\progress\test_tqdm_progress_bar.py
...
def test_tqdm_progress_bar_misconfiguration():
    ...
    with pytest.raises(
        MisconfigurationException, match=r"^You added multiple progress bar callbacks"
    ):
        Trainer(callbacks=callbacks)

    with pytest.raises(
        MisconfigurationException,
        match=r"enable_progress_bar=False` but found `TQDMProgressBar",
    ):
...
def test_tqdm_progress_bar_totals(tmp_path, num_dl):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.progress.tqdm_progress.Tqdm", MockTqdm
    ):
        trainer.fit(model)
    ...
    assert (
        pbar.val_progress_bar.n_values == list(range(num_sanity_val_steps + 1)) * num_dl
    )
    assert pbar.val_progress_bar.descriptions == [
        f"Sanity Checking DataLoader {i}: " for i in range(num_dl)
    ]
    ...
    assert pbar.val_progress_bar.descriptions == [
        f"Validation DataLoader {i}: " for i in range(num_dl)
    ]
    assert not pbar.val_progress_bar.leave
    ...
    assert pbar.test_progress_bar.descriptions == [
        f"Testing DataLoader {i}: " for i in range(num_dl)
    ]
    assert pbar.test_progress_bar.leave
    ...
    assert pbar.predict_progress_bar.descriptions == [
        f"Predicting DataLoader {i}: " for i in range(num_dl)
    ]
    assert pbar.predict_progress_bar.leave
...
def test_tqdm_progress_bar_progress_refresh(tmp_path, refresh_rate: int):
    ...
    assert (
        pbar.val_batches_seen
        == 3 * pbar.val_progress_bar.total + trainer.num_sanity_val_steps
    )
    ...
    assert (
        pbar.val_batches_seen
        == 4 * pbar.val_progress_bar.total + trainer.num_sanity_val_steps
    )
    assert pbar.test_batches_seen == 0
...
def test_tqdm_progress_bar_value_on_colab(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, callbacks=TQDMProgressBar(refresh_rate=19)
    )
...
def test_tqdm_progress_bar_refresh_rate_via_env_variable(
    refresh_rate, env_value, expected
):
...
@pytest.mark.parametrize(
    ("test_batches", "refresh_rate", "updates"),
    [(1, 3, [0, 1]), (3, 1, [0, 1, 2, 3]), (5, 3, [0, 3, 5])],
)
def test_test_progress_bar_update_amount(
    tmp_path, test_batches: int, refresh_rate: int, updates: list
):
...
def test_progress_bar_max_val_check_interval(
    ...
    val_check_batch = (
        max(1, int(limit_batches * val_check_interval))
...
def test_progress_bar_max_val_check_interval_ddp(tmp_path, val_check_interval):
    ...
    if trainer.is_global_zero:
        ...
        assert (
            pbar_callback.train_progress_bar.total == total_train_batches // world_size
        )
        assert pbar_callback.is_enabled
...
@mock.patch(
    "lightning.pytorch.trainer.trainer.Trainer.is_global_zero",
...
def test_tqdm_progress_bar_reset_behavior(tmp_path):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.progress.tqdm_progress.Tqdm", ResetTrackingTqdm
    ):
        trainer.fit(model)

    train_bar = pbar.train_progress_bar
    assert (
        None in train_bar.reset_calls_with_params
