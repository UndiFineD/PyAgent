# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_timer.py
...
def test_trainer_flag(caplog, tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, logger=False, max_time={"seconds": 1337}
    )
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, logger=False, max_time={"seconds": 1}, max_epochs=-1
    )
    with pytest.raises(SystemExit):
        trainer.fit(TestModel())
    timer = [c for c in trainer.callbacks if isinstance(c, Timer)][0]
