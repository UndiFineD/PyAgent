# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_prediction_writer.py
...
def test_prediction_writer_invalid_write_interval():
    ...
    with pytest.raises(
        MisconfigurationException, match=r"`write_interval` should be one of \['batch"
    ):
...
def test_prediction_writer_hook_call_intervals(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, logger=False, limit_predict_batches=4, callbacks=cb
    )
...
def test_prediction_writer_batch_indices(num_workers, tmp_path):
    ...
    dataloader = DataLoader(
        RandomDataset(32, 64), batch_size=4, num_workers=num_workers
    )
    model = BoringModel()
