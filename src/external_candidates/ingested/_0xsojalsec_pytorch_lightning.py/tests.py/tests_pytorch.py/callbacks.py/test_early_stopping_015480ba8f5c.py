# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_early_stopping.py
...
def test_resume_early_stopping_from_checkpoint(tmp_path):
    ...
    with pytest.raises(
        MisconfigurationException, match=r"You restored a checkpoint with current_epoch"
    ):
...
@pytest.mark.parametrize(
    "stop_value", [torch.tensor(torch.inf), torch.tensor(torch.nan)]
)
...
def test_min_epochs_min_steps_global_step(
    ...
    with mock.patch.object(
        es_callback, "_evaluate_stopping_criteria", side_effect=side_effect
    ):
        trainer.fit(model)
...
def test_early_stopping_mode_options():
    with pytest.raises(
        MisconfigurationException, match="`mode` can be .* got unknown_option"
    ):
...
@pytest.mark.parametrize(
    ...
    [
        (
            [EarlyStopping("abc"), EarlyStopping("cba", patience=3)],
            3,
            False,
            "auto",
            1,
        ...
        (
            [EarlyStopping("cba", patience=3), EarlyStopping("abc")],
            3,
            False,
            "auto",
            1,
        ...
        pytest.param(
            [EarlyStopping("abc", patience=1), EarlyStopping("cba")],
            2,
            False,
            "ddp_spawn",
            2,
...
def test_multiple_early_stopping_callbacks(
    ...
    model = EarlyStoppingModel(
        expected_stop_epoch,
        check_on_train_epoch_end,
        dist_diverge_epoch=dist_diverge_epoch,
    )
...
def test_check_on_train_epoch_end_smart_handling(tmp_path, case):
    ...
    with mock.patch(
        "lightning.pytorch.callbacks.EarlyStopping._evaluate_stopping_criteria",
    ...
    if case == "val_check_interval":
        assert trainer.global_step == len(side_effect) * int(
            trainer.limit_train_batches * trainer.val_check_interval
        )
    else:
        assert (
            trainer.current_epoch == len(side_effect) * trainer.check_val_every_n_epoch
        )
...
def test_early_stopping_log_info(
    log_rank_zero_only, world_size, global_rank, expected_log
):
...
class ModelWithDecreasingLoss(BoringModel):
    ...
    def on_validation_epoch_end(self):
        loss = (
            ...
            if self.current_epoch < len(self.epoch_losses)
            else 0.1
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            {
                "stopping_reason": EarlyStoppingReason.PATIENCE_EXHAUSTED,
                "stopping_reason_message": "Test message",
            },
        ),
...
def test_early_stopping_reasons(
    ...
    if state_dict_override is not None:
        ...
        if "stopping_reason" in state_dict_override:
            ...
            early_stopping.stopping_reason_message = state_dict_override[
                "stopping_reason_message"
            ]
