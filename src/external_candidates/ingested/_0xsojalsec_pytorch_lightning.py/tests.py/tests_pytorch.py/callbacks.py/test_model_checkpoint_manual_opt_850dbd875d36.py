...
def test_model_checkpoint_manual_opt():
    with cleanup_after_test(), tempfile.TemporaryDirectory() as tmpdir:
        ...
        for layer_name, layer_value in best_ckpt.items():
            assert torch.equal(
                model_before_opt[layer_name.removeprefix("layer.")], layer_value.cpu()
...
def test_model_checkpoint_manual_opt_warning():
    ...
    with cleanup_after_test(), tempfile.TemporaryDirectory() as tmpdir:
        ...
        manual_opt_warnings = [
            str(warning.message)
            for warning in w
            if "Using ModelCheckpoint with manual optimization and every_n_train_steps"
            in str(warning.message)
        ]

        # Verify our warning was raised
        assert (
            len(manual_opt_warnings) > 0
        ), "Expected warning about manual optimization not found"
        assert (
            "The checkpoint will contain the model state AFTER optimization"
