# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_pruning.py
...
def train_with_pruning_callback(
    ...
    if (
        isinstance(pruning_fn, str)
        and pruning_fn.endswith("_structured")
        and use_global_unstructured
    ):
        with pytest.raises(
    ...
    if ModelPruning._is_pruning_method(pruning_fn) and not use_global_unstructured:
        with pytest.raises(
            MisconfigurationException, match="currently only supported with"
        ):
            ModelPruning(**pruning_kwargs)
        return
...
def test_pruning_misconfiguration():
    with pytest.raises(
        MisconfigurationException, match=r"chocolate isn't in \('weight', 'bias'\)"
    ):
...
def test_permanent_when_model_is_saved_multiple_times(
    ...
    model = TestModel.load_from_checkpoint(
        trainer.checkpoint_callback.kth_best_model_path
    )
    assert not hasattr(model.layer.mlp_3, "weight_orig")
...
def test_original_issue_reproduction():
    ...
    parameters_to_prune = ModelPruning.sanitize_parameters_to_prune(
        model,
        parameters_to_prune=(),
        parameter_names=["weight", "bias", "training", "requires_grad"],
    )
...
def test_lottery_ticket_hypothesis_correctly_reset(tmp_path):
        ...
        def __init__(self):
            ...
            with torch.no_grad():
                ...
                self.layer.weight.copy_(
                    torch.arange(1, 65, dtype=torch.float32).reshape(2, 32)
                )
    ...
    assert hasattr(
        model.layer, "weight_mask"
    ), "Pruning should have created weight_mask"
    assert hasattr(
        model.layer, "weight_orig"
    ), "Pruning should have created weight_orig"
...
def test_sparsity_calculation(tmp_path, caplog, pruning_amount: float, model_type: str):
    ...
    sparsity_logs = [
        msg for msg in caplog.messages if "Applied `L1Unstructured`. Pruned:" in msg
    ]
    ...
    assert (
        total_params_after == expected_total_params
