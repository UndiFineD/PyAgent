# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\callbacks\test_stochastic_weight_avg.py
...
def train_with_swa(
    ...
    model = SwaTestModel(
        batchnorm=batchnorm, interval=interval, iterable_dataset=iterable_dataset
    )
    swa_start = 2
    max_epochs = 5
...
def test_swa_warns(tmp_path, caplog):
    ...
    with (
        ...
        pytest.warns(UserWarning, match="SWA is currently only supported"),
    ):
        trainer.fit(model)
...
def test_swa_raises():
    with pytest.raises(
        MisconfigurationException, match=">0 integer or a float between 0 and 1"
    ):
    ...
    with pytest.raises(
        MisconfigurationException, match="positive float, or a list of positive floats"
    ):
...
def test_swa_deepcopy(tmp_path):
    ...
    class TestSWA(StochasticWeightAveraging):
        ...
        def setup(self, trainer, pl_module, stage) -> None:
            ...
            assert (
                self._average_model.train_dataloader is not pl_module.train_dataloader
            )
    ...
    class TestModel(BoringModel):
        ...
        def configure_optimizers(self):
            params = [
                ...
                {"params": self.layer2.parameters(), "lr": 0.2},
            ]
...
def _swa_resume_training_from_checkpoint(tmp_path, model, resume_model, ddp=False):
    ...
    trainer = Trainer(
        callbacks=SwaTestCallback(swa_epoch_start=swa_start, swa_lrs=0.1),
        **trainer_kwargs,
    )
...
def test_misconfiguration_error_with_sharded_model(tmp_path, strategy: str):
    ...
    with pytest.raises(
        MisconfigurationException, match="SWA does not currently support sharded models"
    ):
        trainer.fit(model)
