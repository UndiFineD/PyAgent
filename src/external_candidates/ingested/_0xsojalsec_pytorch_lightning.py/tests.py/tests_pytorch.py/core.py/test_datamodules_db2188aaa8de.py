# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\core\test_datamodules.py
...
@mock.patch(
    "lightning.pytorch.trainer.trainer.Trainer.node_rank", new_callable=PropertyMock
)
@mock.patch(
    "lightning.pytorch.trainer.trainer.Trainer.local_rank", new_callable=PropertyMock
)
...
def test_train_loop_only(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=1, enable_model_summary=False
    )
...
def test_dm_init_from_datasets_dataloaders(iterable):
    ...
    with pytest.raises(
        MisconfigurationException, match="`val_dataloader` must be implemented"
    ):
        _ = dm.val_dataloader()
    with pytest.raises(
        MisconfigurationException, match="`test_dataloader` must be implemented"
    ):
    ...
    with pytest.raises(
        MisconfigurationException, match="`train_dataloader` must be implemented"
    ):
    ...
    predict_dss = [ds(), ds()]
    dm = LightningDataModule.from_datasets(
        train_ds, valid_dss, test_dss, predict_dss, batch_size=4, num_workers=0
    )
    with mock.patch("lightning.pytorch.core.datamodule.DataLoader") as dl_mock:
        dm.val_dataloader()
        dm.test_dataloader()
...
def test_dm_init_from_datasets_with_init_params():
    ...
    dm = KnownExtraParametersDataModule.from_datasets(
        DummyDS(), batch_size=4, num_workers=2
    )
    ...
    dm = UnknownExtraParametersDataModule.from_datasets(
        DummyDS(), batch_size=4, num_workers=2, other=5
    )
    assert dm.batch_size == 4
    assert dm.other == 5
    ...
    with pytest.raises(
        TypeError, match="missing 1 required positional argument: 'other'"
    ):
        UnknownExtraParametersDataModule.from_datasets(
            DummyDS(), batch_size=4, num_workers=2
        )
    ...
    dm = KwargsParametersDataModule.from_datasets(
        DummyDS(), batch_size=10, num_workers=100, another=None
    )
...
def test_hyperparameters_saving():
    ...
    if _OMEGACONF_AVAILABLE:
        data = DataModuleWithHparams_1(
            OmegaConf.create({"hello": "world"}), "foo", kwarg0="bar"
        )
...
def test_define_as_dataclass():
    ...
    @dataclass(
        init=True, repr=True, eq=True, order=True, unsafe_hash=True, frozen=False
    )
