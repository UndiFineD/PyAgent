# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\core\test_lightning_optimizer.py
...
def test_init_optimizers_resets_lightning_optimizers(tmp_path):
    ...
    def compare_optimizers():
        assert (
            trainer.strategy._lightning_optimizers[0].optimizer is trainer.optimizers[0]
        )
...
class OptimizerWithHooks(Optimizer):
    def __init__(self, model):
        ...
        for _, mod in model.named_modules():
            ...
            handle = mod.register_backward_hook(
