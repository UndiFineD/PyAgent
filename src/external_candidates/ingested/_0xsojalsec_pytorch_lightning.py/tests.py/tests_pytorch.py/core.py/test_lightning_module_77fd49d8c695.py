...
def test_toggle_untoggle_3_optimizers_shared_parameters(tmp_path):
    class TestModel(BoringModel):
        ...
        def configure_optimizers(self):
            ...
            optimizer_2 = Adam(
                self.combine_generators(
                    self.layer_2.parameters(), self.layer_3.parameters()
                ),
                lr=0.1,
            )
            optimizer_3 = SGD(
                self.combine_generators(
                    self.layer_3.parameters(), self.layer_1.parameters()
                ),
                lr=0.1,
            )
...
def test_lightning_module_configure_gradient_clipping_different_argument_values(
    ...
    class TestModel(BoringModel):
        ...
        def configure_gradient_clipping(
            ...
            self.clip_gradients(
                optimizer, gradient_clip_val=self.custom_gradient_clip_val
            )
...
def test_fabric_log():
    ...
    logger.log_metrics.assert_called_with(
        metrics={"tensor": torch.tensor(0.1)}, step=None
    )
...
def test_fabric_log_dict():
    ...
    logger.log_metrics.assert_called_with(
        metrics={"int": 1, "float": 0.1, "tensor": torch.tensor(0.1)}, step=None
    )
...
def test_grad_clipping_lm_fabric(algo):
    ...
    if algo == "value":
        fabric.clip_gradients.assert_called_once_with(
            orig_model, optimizer, clip_val=1e-3, max_norm=None
        )
    else:
        fabric.clip_gradients.assert_called_once_with(
            orig_model, optimizer, clip_val=None, max_norm=1e-3
        )
