# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\core\test_saving.py
...
def test_load_from_checkpoint_map_location_automatic(
...
@pytest.mark.parametrize(
    "map_location",
    [None, "cpu", torch.device("cpu"), lambda storage, loc: storage, {"cpu": "cpu"}],
)
def test_load_from_checkpoint_map_location_cpu(tmp_path, map_location):
    ...
    model = BoringModel.load_from_checkpoint(
        f"{tmp_path}/checkpoint.ckpt", map_location=map_location
    )
    _ = BoringDataModule.load_from_checkpoint(
        f"{tmp_path}/checkpoint.ckpt", map_location=map_location
    )
    assert model.device.type == "cpu"
...
@pytest.mark.parametrize(
    "map_location",
    ["cpu", torch.device("cpu"), lambda storage, loc: storage, {"cuda": "cpu"}],
)
...
def test_load_from_checkpoint_device_placement_with_extra_state(tmp_path):
    ...
    model = ExtraStateModel.load_from_checkpoint(
        f"{tmp_path}/checkpoint.ckpt", map_location=None
    )
    assert model.device.type == "cuda"
...
def test_load_from_checkpoint_warn_on_empty_state_dict(tmp_path):
    ...
    with pytest.warns(UserWarning, match="contains no parameters"):
        model = BoringModel.load_from_checkpoint(
            tmp_path / "checkpoint.ckpt", strict=False
        )
    assert model.device.type == "cpu"
...
def test_load_from_checkpoint_strict(strict, strict_loading, expected, tmp_path):
    ...
    if expected == "error":
        with pytest.raises(ValueError, match="in conflict with .*strict_loading"):
            LoadingModel.load_from_checkpoint(
                tmp_path / "checkpoint.ckpt", strict=strict
            )
    else:
        model = LoadingModel.load_from_checkpoint(
            tmp_path / "checkpoint.ckpt", strict=strict
        )
