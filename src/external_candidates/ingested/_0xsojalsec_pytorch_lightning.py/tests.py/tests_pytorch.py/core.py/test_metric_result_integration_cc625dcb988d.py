    ...
    class ExtendedBoringModel(BoringModel):
        ...
        def training_step(self, batch, batch_idx):
            ...
            if self.trainer.fit_loop.restarting:
                ...
                self.log(
                    "tracking_2", batch_idx, on_step=True, on_epoch=True, sync_dist=True
                )

                self.dummy_metric(batch_idx)
                self.log(
                    "tracking_metric", self.dummy_metric, on_step=True, on_epoch=True
                )
...
def test_metric_collections(tmp_path):
    ...
    class TestModel(BoringModel):
        ...
        def on_train_epoch_end(self) -> None:
            ...
            assert (
                results["training_step.e"].meta.metric_attribute
                == "metrics_collection_dict.a"
            )
            assert (
                results["training_step.f"].meta.metric_attribute
                == "metrics_collection_dict.b"
            )

            assert (
                results["training_step.g"].meta.metric_attribute
                == "metrics_collection_dict_nested.a.0.b"
            )
            assert (
                results["training_step.h"].meta.metric_attribute
                == "metrics_collection_dict_nested.a.1"
            )
...
def test_compute_not_a_tensor_raises():
    ...
    with pytest.raises(
        ValueError, match=r"compute\(\)` return of.*foo' must be a tensor"
    ):
        trainer.fit(model)
