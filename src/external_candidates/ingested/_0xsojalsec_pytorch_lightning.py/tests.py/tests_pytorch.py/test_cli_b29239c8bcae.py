...
def mock_subclasses(baseclass, *subclasses):
    ...
    with ExitStack() as stack:
        ...
        for mgr in [
            mock.patch.object(s, "__subclasses__", return_value=[]) for s in subclasses
        ]:
            stack.enter_context(mgr)
        yield None
...
@pytest.mark.parametrize(
    "cli_args", [["--callbacks=1", "--logger"], ["--foo", "--bar=1"]]
)
...
def test_lightning_cli_args_callbacks(cleandir):
    ...
    class TestModel(BoringModel):
        def on_fit_start(self):
            callback = [
                c for c in self.trainer.callbacks if isinstance(c, LearningRateMonitor)
            ]
            assert len(callback) == 1
            assert callback[0].logging_interval == "epoch"
            assert callback[0].log_momentum is True
            callback = [
                c for c in self.trainer.callbacks if isinstance(c, ModelCheckpoint)
            ]
            assert len(callback) == 1
    ...
    with mock.patch(
        "sys.argv", ["any.py", "fit", f"--trainer.callbacks={json.dumps(callbacks)}"]
    ):
        cli = LightningCLI(
            TestModel,
            trainer_defaults={
...
def test_lightning_cli_single_arg_callback():
    ...
    assert (
        cli.config.trainer.callbacks.class_path
...
def test_lightning_cli_args_cluster_environments(cleandir):
    ...
    with mock.patch(
        "sys.argv", ["any.py", "fit", f"--trainer.plugins={json.dumps(plugins)}"]
    ):
...
def test_lightning_env_parse(cleandir):
    ...
    with (
        ...
        LightningCLI(
            BoringModel, DataDirDataModule, parser_kwargs={"default_env": True}
        )
    out = out.getvalue()
...
def test_lightning_cli_save_config_only_once(cleandir):
    ...
    save_config_callback = next(
        c for c in cli.trainer.callbacks if isinstance(c, SaveConfigCallback)
    )
...
def test_lightning_cli_help():
    ...
    assert ("--data.data_dir" in out.getvalue()) or (
        "--data.init_args.data_dir" in out.getvalue()
    )
    ...
    class CkptPathCLI(LightningCLI):
        def add_arguments_to_parser(self, parser):
            parser.link_arguments(
                "model.out_dim", "model.hidden_dim", compute_fn=lambda x: x * 2
            )
...
def test_lightning_cli_ckpt_path_argument_hparams_subclass_mode(cleandir):
    ...
    assert cli.config.fit.model.init_args == Namespace(
        out_dim=4, hidden_dim=8, extra=True
    )
    hparams_path = Path(cli.trainer.log_dir) / "hparams.yaml"
    assert hparams_path.is_file()
    hparams = yaml.safe_load(hparams_path.read_text())
...
def test_lightning_cli_link_arguments(cleandir):
    ...
    with mock.patch("sys.argv", ["any.py"] + cli_args):
        cli = MyLightningCLI(
            BoringModelRequiredClasses, BoringDataModuleBatchSizeAndClasses, run=False
        )
...
@pytest.mark.parametrize(
    "logger", [False, lazy_instance(TensorBoardLogger, save_dir=".")]
)
...
def test_lightning_cli_optimizer_and_lr_scheduler_subclasses(cleandir):
    ...
    assert len(cli.trainer.lr_scheduler_configs) == 1
    assert isinstance(
        cli.trainer.lr_scheduler_configs[0].scheduler, torch.optim.lr_scheduler.StepLR
    )
...
class TestModelSaveHparams(BoringModel):
    def __init__(
        ...
        optimizer: OptimizerCallable = torch.optim.Adam,
        scheduler: LRSchedulerCallable = torch.optim.lr_scheduler.ConstantLR,
        activation: torch.nn.Module = lazy_instance(
...
def test_lightning_cli_load_from_checkpoint_dependency_injection(cleandir):
    with mock.patch("sys.argv", ["any.py", "--trainer.max_epochs=1"]):
        cli = LightningCLI(
            TestModelSaveHparams, run=False, auto_configure_optimizers=False
        )
    cli.trainer.fit(cli.model)
    ...
    assert (
        hparams["optimizer"] == expected_optimizer
        or hparams["optimizer"]["class_path"] == expected_optimizer
    )
    assert (
        hparams["scheduler"] == expected_scheduler
        or hparams["scheduler"]["class_path"] == expected_scheduler
    )
...
def test_lightning_cli_load_from_checkpoint_dependency_injection_subclass_mode(
    ...
    with mock.patch(
        "sys.argv", ["any.py", "--trainer.max_epochs=1", "--model=TestModelSaveHparams"]
    ):
        cli = LightningCLI(
...
def test_lightning_cli_run(cleandir):
    ...
    with mock.patch("sys.argv", ["any.py", "fit"]):
        cli = LightningCLI(
            BoringModel, trainer_defaults={"max_steps": 1, "max_epochs": 1}
        )
...
def test_lightning_cli_datamodule_short_arguments():
    ...
    with (
        mock.patch(
            "sys.argv", ["any.py", "--model", "BoringModel", "--data=MyDataModule"]
        ),
        mock_subclasses(LightningModule, BoringModel),
        mock_subclasses(LightningDataModule, MyDataModule),
    ):
...
def test_optimizers_and_lr_schedulers_reload(cleandir):
    ...
    assert (
        dict_config["lr_scheduler"]["class_path"]
...
def test_lightning_cli_config_with_subcommand():
    ...
    test_mock.assert_called_once_with(
        cli.trainer, cli.model, verbose=True, ckpt_path="foobar", weights_only=None
    )
...
def test_lightning_cli_config_before_subcommand():
    ...
    validate_mock.assert_called_once_with(
        cli.trainer, cli.model, verbose=False, ckpt_path="barfoo", weights_only=None
    )
...
def test_lightning_cli_config_before_subcommand_two_configs():
    ...
    with (
        mock.patch(
            "sys.argv", ["any.py", f"--config={config1}", f"--config={config2}", "test"]
        ),
        mock.patch("lightning.pytorch.Trainer.test", autospec=True) as test_mock,
    ):
        cli = LightningCLI(BoringModel)
...
def test_lightning_cli_config_before_and_after_subcommand():
    ...
    with (
        mock.patch(
            "sys.argv", ["any.py", f"--config={config1}", "test", f"--config={config2}"]
        ),
        mock.patch("lightning.pytorch.Trainer.test", autospec=True) as test_mock,
    ):
        cli = LightningCLI(BoringModel)
...
def test_lightning_cli_reinstantiate_trainer():
    ...
    assert {c.__class__ for c in trainer.callbacks} == {
        c.__class__ for c in cli.trainer.callbacks
    }.union({TestCallback})
...
def test_cli_configureoptimizers_can_be_overridden():
    ...
    with mock.patch(
        "sys.argv", ["any.py", "--lr_scheduler=StepLR", "--lr_scheduler.step_size=50"]
    ):
        cli = MyCLI()
    [optimizer], [scheduler] = cli.model.configure_optimizers()
    assert isinstance(optimizer, SGD)
...
def test_ddpstrategy_instantiation_and_find_unused_parameters(mps_count_0):
    ...
    with mock.patch(
        "sys.argv", ["any.py", "--trainer.strategy.process_group_backend=group"]
    ):
        cli = LightningCLI(
            BoringModel,
            run=False,
...
def test_neptune_logger_init_args():
    _test_logger_init_args(
        ...
        unresolved={
            "description": "neptune"
        },  # Unsupported resolving from neptune.internal.init.run.init_run
    )
...
def test_pytorch_profiler_init_args():
    ...
    cli_args += [
        f"--trainer.profiler.dict_kwargs.{k}={v}" for k, v in unresolved.items()
    ]
...
def test_lightning_cli_jsonnet(cleandir):
    ...
    with mock.patch("sys.argv", ["any.py"] + cli_args):
        cli = LightningCLI(
            MainModule, run=False, parser_kwargs={"parser_mode": "jsonnet"}
        )
