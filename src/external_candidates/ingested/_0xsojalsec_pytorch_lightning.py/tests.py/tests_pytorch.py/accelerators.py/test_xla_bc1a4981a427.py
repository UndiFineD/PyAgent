# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\accelerators\test_xla.py
...
def test_resume_training_on_cpu(tmp_path):
    ...
    model = BoringModel()
    trainer = Trainer(
        max_epochs=1, accelerator="tpu", devices="auto", default_root_dir=tmp_path
    )
    trainer.fit(model)
...
@pytest.mark.parametrize(
    ("accelerator", "devices"), [("auto", 8), ("auto", "auto"), ("tpu", "auto")]
)
...
def test_tpu_invalid_raises(tpu_available, mps_count_0):
    strategy = DDPStrategy(
        accelerator=XLAAccelerator(), precision_plugin=XLAPrecision()
    )
    with pytest.raises(
        ValueError,
        match="XLAAccelerator` can only be used with a `SingleDeviceXLAStrategy`",
    ...
    with pytest.raises(
        ValueError,
        match="The `XLAAccelerator` can only be used with a `SingleDeviceXLAStrategy` or `XLAStrategy",
    ):
        Trainer(strategy=strategy, devices=8)
...
def test_xla_mp_device_dataloader_attribute(_, monkeypatch):
    ...
    assert (
        processed_dataloader.batch_sampler == processed_dataloader._loader.batch_sampler
    )
...
def test_trainer_config_device_ids(
    devices, expected_device_ids, tpu_available, monkeypatch
):
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.accelerators.xla._using_pjrt", lambda: True
    )
    ...
    if _IS_WINDOWS:
        ...
        monkeypatch.setattr(
            torch.multiprocessing, "get_all_start_methods", lambda: ["fork", "spawn"]
        )
