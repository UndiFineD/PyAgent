# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_model_parallel.py
...
@mock.patch(
    "lightning.pytorch.strategies.model_parallel._TORCH_GREATER_EQUAL_2_4", False
)
...
def test_configure_model_required():
    ...
    with pytest.raises(
        TypeError, match="you are required to override the `configure_model"
    ):
        strategy.setup(Mock())
...
def test_save_checkpoint_storage_options(tmp_path):
    ...
    with pytest.raises(
        ...
        strategy.save_checkpoint(
            checkpoint=Mock(), filepath=tmp_path, storage_options=Mock()
        )


@RunIf(min_torch="2.4")
@mock.patch(
    "lightning.pytorch.strategies.model_parallel.ModelParallelStrategy.broadcast",
    lambda _, x: x,
)
...
def test_save_checkpoint_path_exists(shutil_mock, torch_save_mock, tmp_path):
    ...
    with save_mock:
        strategy.save_checkpoint(
            {"state_dict": {}, "optimizer_states": {"": {}}}, filepath=path
        )
