# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_registry.py
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"stage": 3, "offload_parameters": True, "offload_optimizer": True},
        ),
    ],
)
...
@pytest.mark.parametrize(
    "strategy", ["deepspeed", "deepspeed_stage_2_offload", "deepspeed_stage_3"]
)
def test_deepspeed_strategy_registry_with_trainer(tmp_path, strategy, mps_count_0):
    trainer = Trainer(
        default_root_dir=tmp_path, strategy=strategy, precision="16-mixed"
    )
