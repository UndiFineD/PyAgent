# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\launchers\test_multiprocessing.py
...
@mock.patch(
    "lightning.pytorch.strategies.launchers.multiprocessing.mp.get_all_start_methods",
    return_value=[],
)
def test_multiprocessing_launcher_forking_on_unsupported_platform(_):
    with pytest.raises(
        ValueError, match="The start method 'fork' is not available on this platform"
    ):
        _MultiProcessingLauncher(strategy=Mock(), start_method="fork")


@pytest.mark.parametrize(
    "start_method", ["spawn", pytest.param("fork", marks=RunIf(standalone=True))]
)
...
def test_collect_rank_zero_results(
    trainer_fn, fake_node_rank, fake_local_rank, tmp_path
):
    ...
    strategy = DDPStrategy(
        cluster_environment=cluster_environment, start_method="spawn"
    )
...
def test_transfer_weights(tmp_path, trainer_fn):
    ...
    assert model.load_state_dict.call_count == int(
        spawn_output.weights_path is not None
    )
...
def test_non_strict_loading(tmp_path):
    ...
    trainer.state.fn = (
        TrainerFn.FITTING
    )  # state dict loading only relevant for the FITTING case
...
def test_check_for_missing_main_guard():
    ...
    with (
        mock.patch(
            ...
            return_value=Mock(
                _inheriting=True
            ),  # pretend that main is importing itself
        ),
