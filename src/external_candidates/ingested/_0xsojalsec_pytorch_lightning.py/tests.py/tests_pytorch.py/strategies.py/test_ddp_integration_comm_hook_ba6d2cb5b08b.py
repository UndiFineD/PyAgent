# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_ddp_integration_comm_hook.py
...
def test_ddp_spawn_fp16_compress_comm_hook(tmp_path):
    ...
    strategy = DDPStrategy(
        ddp_comm_hook=default.fp16_compress_hook, start_method="spawn"
    )
    trainer = Trainer(
        max_epochs=1,
        accelerator="gpu",
...
@mock.patch(
    "torch.distributed.algorithms.model_averaging.averagers.PeriodicModelAverager.average_parameters"
)
...
def test_post_local_sgd_model_averaging_raises(average_parameters_mock, tmp_path):
    ...
    class OptimizerModel(BoringModel):
        def configure_optimizers(self):
            return ZeroRedundancyOptimizer(
                params=self.parameters(), optimizer_class=torch.optim.Adam, lr=0.01
            )
    ...
    with pytest.raises(
        ValueError,
        match="Currently model averaging cannot work with a distributed optimizer",
    ):
        trainer.fit(model)
