# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_model_parallel_integration.py
...
def test_tensor_parallel(distributed, compile):
    ...
    class Model(TensorParallelModel):
        def on_train_start(self):
            ...
            assert all(
                tensor.device_mesh == device_mesh["tensor_parallel"]
                for tensor in optimizer.param_groups[0]["params"]
            )
            assert all(
                isinstance(weight, DTensor) for weight in self.model.parameters()
            )
        ...
        def training_step(self, batch):
            ...
            assert all(
                torch.equal(batches[0], batches[i]) for i in range(1, len(batches))
            )
...
def test_fsdp2_tensor_parallel(distributed, compile):
    ...
    class Model(FSDP2TensorParallelModel):
        def on_train_start(self):
            ...
            assert all(
                weight.device.type != "meta" for weight in self.model.parameters()
            )
            assert all(
                tensor.device_mesh.ndim == 2
                for tensor in optimizer.param_groups[0]["params"]
            )
            assert all(
        ...
        def training_step(self, batch):
            ...
            assert all(
                torch.equal(batches_tp[0], batches_tp[i])
                for i in range(1, len(batches_tp))
            )
            # Batches across the DP dimension must be different
            batches_dp = batches[dp_mesh.mesh]
            assert all(
                not torch.equal(batches_dp[0], batches_dp[i])
                for i in range(1, len(batches_dp))
            )
...
def test_save_load_sharded_state_dict(distributed, tmp_path):
    ...
    checkpoint_path = Path(
        trainer.strategy.broadcast(trainer.checkpoint_callback.best_model_path)
    )
