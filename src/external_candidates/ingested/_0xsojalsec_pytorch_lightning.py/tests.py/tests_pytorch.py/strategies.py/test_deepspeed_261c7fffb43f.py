# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_deepspeed.py
...
def test_warn_deepspeed_ignored(tmp_path):
    ...
    with pytest.warns(
        UserWarning, match="will be ignored since DeepSpeed handles the backward"
    ):
        trainer.fit(model)
...
@mock.patch(
    "lightning.pytorch.Trainer.log_dir",
...
def test_deepspeed_auto_batch_size_config_select(_, __, tmp_path, dataset_cls, value):
    ...
    class TestModel(BoringModel):
        ...
        def configure_model(self) -> None:
            ...
            if dataset_cls == RandomDataset:
                expected_value = (
                    self.train_dataloader().batch_size if value == "auto" else value
                )
    ...
    trainer = Trainer(
        ...
        devices=1,
        strategy=DeepSpeedStrategy(
            logging_batch_size_per_gpu=value, zero_optimization=False
        ),
    )
...
def test_deepspeed_run_configure_optimizers(tmp_path):
    ...
    class TestModel(BoringModel):
        def configure_optimizers(self):
            ...
            return {
                "optimizer": optimizer,
                "lr_scheduler": {"scheduler": scheduler, "name": "Sean"},
            }
...
def test_deepspeed_multigpu(tmp_path):
    ...
    with mock.patch(
        "deepspeed.init_distributed", wraps=deepspeed.init_distributed
    ) as mock_deepspeed_distributed:
        trainer.fit(model)
...
def test_deepspeed_multigpu_single_file(tmp_path):
    ...
    checkpoint_path = os.path.join(tmp_path, "model.pt")
    trainer = Trainer(
        default_root_dir=tmp_path, fast_dev_run=True, accelerator="cpu", devices=1
    )
    trainer.fit(model)
    trainer.save_checkpoint(checkpoint_path)
    ...
    with pytest.raises(
        FileNotFoundError, match="The provided path is not a valid DeepSpeed checkpoint"
    ):
...
class ModelParallelClassificationModel(LightningModule):
    ...
    def configure_model(self) -> None:
        ...
        if self.model is None:
            ...
            self.model = nn.Sequential(
                *(self.make_block() for x in range(self.num_blocks)), nn.Linear(32, 3)
            )
    ...
    def test_step(self, batch, batch_idx):
        ...
        logits = self.forward(x)
        self.log(
            "test_loss", F.cross_entropy(logits, y), prog_bar=False, sync_dist=True
        )
...
@pytest.mark.xfail(
    strict=False, reason="skipped due to deepspeed/#2449, keep track @rohitgr7"
)
@pytest.mark.parametrize(
    ("accumulate_grad_batches", "automatic_optimization"), [(1, False), (2, True)]
)
@RunIf(min_cuda_gpus=2, standalone=True, deepspeed=True, sklearn=True)
def test_deepspeed_multigpu_stage_3_checkpointing(
    tmp_path, automatic_optimization, accumulate_grad_batches
):
    model = (
        ModelParallelClassificationModel()
        if automatic_optimization
        else ManualModelParallelClassificationModel()
    )
...
def test_deepspeed_multigpu_stage_3_resume_training(tmp_path):
    ...
    class TestCallback(Callback):
        def on_train_epoch_start(
            ...
            original_optimizer_dict = (
                original_deepspeed_strategy.deepspeed_engine.optimizer.state_dict()
            )
            current_optimizer_dict = (
                current_deepspeed_strategy.deepspeed_engine.optimizer.state_dict()
            )
            ...
            assert (
                original_lr_scheduler.state_dict() == current_lr_scheduler.state_dict()
            )
...
def test_deepspeed_multigpu_stage_2_accumulated_grad_batches(
    ...
    class VerificationCallback(Callback):
        ...
        def on_train_batch_start(
            self, trainer, pl_module: LightningModule, batch: Any, batch_idx: int
        ) -> None:
    ...
    assert (
        trainer.limit_train_batches % trainer.accumulate_grad_batches != 0
    ), "leftover batches should be tested"
    trainer.fit(model, datamodule=dm)
...
@pytest.mark.xfail(
    strict=False,
    reason="Partial parameter partitioning for DeepSpeed is currently broken.",
)
@RunIf(min_cuda_gpus=1, standalone=True, deepspeed=True)
...
def test_deepspeed_strategy_env_variables(
    mock_deepspeed_distributed, tmp_path, platform
):
    ...
    if platform == "Windows":
        ...
        assert all(
            k not in os.environ
            for k in ("MASTER_PORT", "MASTER_ADDR", "RANK", "WORLD_SIZE", "LOCAL_RANK")
        )
    else:
...
def _assert_save_model_is_equal(model, tmp_path, trainer):
    ...
    if trainer.is_global_zero:
        ...
        for orig_param, saved_model_param in zip(
            model.parameters(), state_dict.values()
        ):
...
def test_scheduler_step_count(
    mock_step, tmp_path, max_epoch, limit_train_batches, interval
):
...
def test_deepspeed_gradient_clip_by_value(tmp_path):
    ...
    with pytest.raises(
        ValueError, match="does not support clipping gradients by value"
    ):
        trainer.fit(model)
...
def test_error_with_invalid_accelerator(tmp_path):
    ...
    model = BoringModel()
    with pytest.raises(
        RuntimeError, match="DeepSpeed strategy is only supported on CUDA"
    ):
        trainer.fit(model)
...
def test_validate_parallel_devices_indices(device_indices):
    ...
    with pytest.raises(
        RuntimeError,
        match=escape(
            f"device indices {device_indices!r} don't match the local rank values of processes"
        ),
    ):
        strategy.setup_environment()
    accelerator.setup_device.assert_called_once_with(
        torch.device("cuda", device_indices[0])
    )
...
def test_deepspeed_init_module_with_stage_3():
    ...
    zero_init_mock.assert_called_once_with(
        enabled=True, remote_device=None, config_dict_or_path=ANY
    )
...
def test_deepspeed_init_module_with_stages_1_2(stage):
    ...
    zero_init_mock.assert_called_once_with(
        enabled=False, remote_device=None, config_dict_or_path=ANY
    )
    assert model.layer.weight.dtype == torch.bfloat16
...
def test_deepspeed_load_checkpoint_validate_path(tmp_path):
    ...
    with pytest.raises(
        FileNotFoundError,
        match=f"Try to load using this parent directory instead: {tmp_path}",
    ):
