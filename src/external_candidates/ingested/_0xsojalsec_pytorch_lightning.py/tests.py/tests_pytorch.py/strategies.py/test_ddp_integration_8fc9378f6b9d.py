...
def test_ddp_torch_dist_is_available_in_setup(
    _, __, ___, cuda_count_1, mps_count_0, tmp_path
):
...
def test_ddp_wrapper(tmp_path, precision):
    ...
    class CustomCallback(Callback):
        def on_train_start(
            ...
            assert (
                trainer.strategy.model.module._ddp_params_and_buffers_to_ignore
                == expected
            )
...
def test_find_unused_parameters_ddp_spawn_raises():
    ...
    trainer = Trainer(
        accelerator="cpu", devices=1, strategy="ddp_spawn", max_steps=2, logger=False
    )
    with pytest.raises(
...
def test_find_unused_parameters_ddp_exception():
    """Test that the DDP strategy can change PyTorch's error message so that it's more useful for Lightning users."""
    trainer = Trainer(
        accelerator="cpu", devices=1, strategy="ddp", max_steps=2, logger=False
    )
    with pytest.raises(
        RuntimeError,
        match="It looks like your LightningModule has parameters that were not used in",
...
def test_ddp_cpu():
    ...
    trainer = Trainer(
        devices=2, strategy="ddp_spawn", accelerator="cpu", fast_dev_run=True
    )
...
class BoringZeroRedundancyOptimizerModel(BoringModel):
    def configure_optimizers(self):
        return ZeroRedundancyOptimizer(
            self.layer.parameters(), optimizer_class=torch.optim.Adam, lr=0.1
        )
...
@pytest.mark.parametrize(
    "strategy", [pytest.param("ddp", marks=RunIf(standalone=True)), "ddp_spawn"]
)
def test_ddp_strategy_checkpoint_zero_redundancy_optimizer(strategy, tmp_path):
    ...
    for trained_param, loaded_param in zip(
