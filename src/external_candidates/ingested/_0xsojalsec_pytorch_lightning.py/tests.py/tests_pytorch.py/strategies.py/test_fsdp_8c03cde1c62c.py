# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\strategies\test_fsdp.py
import os
...
class TestBoringModel(BoringModel):
    def __init__(self, wrap_min_params: int = 2):
        ...
        self.should_be_wrapped = [
            wrap_min_params < (32 * 32 + 32),
            None,
            wrap_min_params < (32 * 2 + 2),
        ]
...
def _run_multiple_stages(trainer, model, model_path: Optional[str] = None):
    ...
    ):  # TODO (@awaelchli): Add support for asserting equality of sharded checkpoints
...
def test_modules_without_parameters(tmp_path):
    ...
    class MetricsModel(BoringModel):
        ...
        def training_step(self, batch, batch_idx):
            ...
            assert (
                self.metric.device == self.metric.tp.device == torch.device("cuda", 0)
            )
            self.metric(
...
@pytest.mark.parametrize(
    "precision", ["32-true", pytest.param("bf16-mixed", marks=RunIf(bf16_cuda=True))]
)
...
def test_strategy_full_state_dict(tmp_path, wrap_min_params):
    ...
    assert all(
        _ex == _co
        for _ex, _co in zip(full_state_dict.keys(), correct_state_dict.keys())
    )
...
def test_invalid_parameters_in_optimizer(use_orig_params):
    ...
    error_context = (
        ...
        if use_orig_params is not False
            ...
            ValueError,
            match="The optimizer does not seem to reference any FSDP parameters",
        )
    )
...
def test_hybrid_shard_configuration(sharding_strategy, monkeypatch):
    ...
    strategy = FSDPStrategy(
        auto_wrap_policy={nn.Linear}, sharding_strategy=sharding_strategy
    )
    ...
    monkeypatch.setattr(
        "lightning.pytorch.strategies.fsdp._TORCH_GREATER_EQUAL_2_2", False
    )
    with pytest.raises(
        ValueError, match="`device_mesh` argument is only supported in torch >= 2.2."
    ):
        FSDPStrategy(device_mesh=Mock())

    monkeypatch.setattr(
        "lightning.pytorch.strategies.fsdp._TORCH_GREATER_EQUAL_2_2", True
    )
    device_mesh = Mock()
    strategy = FSDPStrategy(
        sharding_strategy=sharding_strategy, device_mesh=device_mesh
    )
    assert strategy.sharding_strategy.name == sharding_strategy
    assert strategy.kwargs["device_mesh"] is device_mesh

    with pytest.raises(
        ValueError, match="process_group.* device_mesh=.* are mutually exclusive"
    ):
        FSDPStrategy(
...
def test_set_timeout(init_process_group_mock):
    ...
    strategy = FSDPStrategy(
        timeout=test_timedelta, parallel_devices=[torch.device("cpu")]
    )
...
def test_strategy_load_optimizer_states_multiple(_, tmp_path):
    strategy = FSDPStrategy(
        parallel_devices=[torch.device("cpu")], state_dict_type="full"
    )
    trainer = Trainer()
    ...
    with pytest.raises(
        RuntimeError,
        match="1 optimizers but the checkpoint contains 2 optimizers to load",
    ):
    ...
    with pytest.raises(
        RuntimeError,
        match="2 optimizers but the checkpoint contains 1 optimizers to load",
    ):
...
def test_save_checkpoint_storage_options(tmp_path):
    ...
    with pytest.raises(
        ...
        strategy.save_checkpoint(
            filepath=tmp_path, checkpoint=Mock(), storage_options=Mock()
        )
...
def test_save_sharded_and_consolidate_and_load(tmp_path):
    ...
    checkpoint_path_sharded = trainer.strategy.broadcast(
    ...
    checkpoint_path_full = trainer.strategy.broadcast(
        str(tmp_path / "checkpoint_full.ckpt")
    )
    if trainer.global_rank == 0:
