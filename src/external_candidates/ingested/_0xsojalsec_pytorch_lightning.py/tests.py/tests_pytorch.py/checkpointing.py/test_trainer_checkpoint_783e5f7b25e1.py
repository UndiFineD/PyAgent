# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\checkpointing\test_trainer_checkpoint.py
...
def test_finetuning_with_ckpt_path(tmp_path):
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss", dirpath=tmp_path, filename="{epoch:02d}", save_top_k=-1
    )
...
def test_trainer_save_checkpoint_storage_options(tmp_path, xla_available):
    ...
    with mock.patch(
        "lightning.fabric.plugins.io.torch_io.TorchCheckpointIO.save_checkpoint"
    ) as io_mock:
        trainer.save_checkpoint(instance_path, storage_options=instance_storage_options)
        io_mock.assert_called_with(
            ANY, instance_path, storage_options=instance_storage_options
        )
    ...
    with (
        ...
        save_mock.assert_called_with(
            checkpoint_mock, instance_path, storage_options=None
        )
        trainer.save_checkpoint(instance_path, False, instance_storage_options)
        dump_mock.assert_called_with(False)
        save_mock.assert_called_with(
            checkpoint_mock, instance_path, storage_options=instance_storage_options
        )
