...
def test_model_checkpoint_score_and_ckpt(
    ...
    checkpoint = ModelCheckpoint(
        dirpath=tmp_path, filename=filename, monitor=monitor, save_top_k=-1
    )
...
def test_model_checkpoint_score_and_ckpt_val_check_interval(
    ...
    per_epoch_val_checks, leftover_train_batches = divmod(
        limit_train_batches, per_val_train_batches
    )

    class CustomBoringModel(BoringModel):
        def __init__(self):
            super().__init__()
            self.val_logs = torch.randn(
                per_epoch_val_checks * max_epochs, limit_val_batches
            )
    ...
    def _make_assertions(epoch, ix):
        ...
        chk = pl_load(os.path.join(checkpoint.dirpath, expected_filename))
        assert chk["epoch"] == epoch
        expected_global_step = per_val_train_batches * (global_ix + 1) + (
            leftover_train_batches * epoch
        )
...
def test_model_checkpoint_with_non_string_input(tmp_path, save_top_k: int):
    ...
    checkpoint = ModelCheckpoint(
        monitor="early_stop_on", dirpath=None, filename="{epoch}", save_top_k=save_top_k
    )
    max_epochs = 2
    trainer = Trainer(
        default_root_dir=tmp_path,
...
def test_model_checkpoint_to_yaml(tmp_path, save_top_k: int):
    ...
    checkpoint = ModelCheckpoint(
        dirpath=tmp_path, monitor="early_stop_on", save_top_k=save_top_k
    )
...
def test_model_checkpoint_path(
    tmp_path, logger_version: Union[None, int, str], expected: str
):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, overfit_batches=0.2, max_epochs=2, logger=logger
    )
    trainer.fit(model)
...
def test_model_checkpoint_no_extraneous_invocations(tmp_path):
    ...
    model_checkpoint = ModelCheckpointTestInvocations(
        monitor="early_stop_on", expected_count=num_epochs, save_top_k=-1
    )
    trainer = Trainer(
        strategy="ddp_spawn",
        accelerator="cpu",
...
def test_model_checkpoint_format_checkpoint_name(tmp_path, monkeypatch):
    ...
    ckpt_name = model_checkpoint._format_checkpoint_name(
        None, {"epoch": 3, "step": 2}, prefix="test"
    )
    ...
    # no prefix
    ckpt_name = model_checkpoint._format_checkpoint_name(
        "{epoch:03d}-{acc}", {"epoch": 3, "acc": 0.03}
    )
    assert ckpt_name == "epoch=003-acc=0.03"

    # one metric name is substring of another
    ckpt_name = model_checkpoint._format_checkpoint_name(
        "{epoch:03d}-{epoch_test:03d}", {"epoch": 3, "epoch_test": 3}
    )
    ...
    ckpt_name = model_checkpoint._format_checkpoint_name(
        "{epoch},{acc:.5f}", {"epoch": 3, "acc": 0.03}, prefix="test"
    )
    ...
    ckpt_name = ModelCheckpoint(
        monitor="early_stop_on", dirpath=None
    ).format_checkpoint_name({"epoch": 3, "step": 2})
    assert ckpt_name == "epoch=3-step=2.ckpt"
    ckpt_name = ModelCheckpoint(
        monitor="early_stop_on", dirpath=""
    ).format_checkpoint_name({"epoch": 5, "step": 4})
    assert ckpt_name == "epoch=5-step=4.ckpt"

    # CWD
    ckpt_name = ModelCheckpoint(
        monitor="early_stop_on", dirpath="."
    ).format_checkpoint_name({"epoch": 3, "step": 4})
    ...
    # with prefix
    ckpt_name = ModelCheckpoint(
        monitor="early_stop_on", dirpath=tmp_path, filename="name"
    ).format_checkpoint_name({}, prefix="test")
...
def test_model_checkpoint_save_last(save_last, tmp_path, monkeypatch):
    ...
    model_checkpoint = ModelCheckpoint(
        monitor="early_stop_on", dirpath=tmp_path, save_top_k=-1, save_last=save_last
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        callbacks=[model_checkpoint],
    ...
    assert set(os.listdir(tmp_path)) == set(
        [f"epoch={i}-step={j}.ckpt" for i, j in zip(range(epochs), [10, 20, 30])]
        + [last_filename]
    )
    if save_last == "link":
    ...
    assert (
        os.path.realpath(tmp_path / last_filename)
        == model_checkpoint._last_checkpoint_saved
    )


def test_model_checkpoint_save_last_as_link_not_local(tmp_path):
    callback = ModelCheckpoint(
        dirpath="memory://not-a-filesystem-path", save_last="link"
    )
    with pytest.raises(
        ValueError, match="save_last='link'.* is only supported for local file paths"
    ):
...
def test_model_checkpoint_link_checkpoint(tmp_path):
    ...
    new_file1.touch()
    ModelCheckpoint._link_checkpoint(
        trainer, filepath=str(new_file1), linkpath=str(link)
    )
    assert os.path.islink(link)
    ...
    new_file2.touch()
    ModelCheckpoint._link_checkpoint(
        trainer, filepath=str(new_file2), linkpath=str(link)
    )
    assert os.path.islink(link)
    ...
    folder_link.mkdir()
    ModelCheckpoint._link_checkpoint(
        trainer, filepath=str(folder), linkpath=str(folder_link)
    )
    ...
    new_folder.mkdir()
    ModelCheckpoint._link_checkpoint(
        trainer, filepath=str(new_folder), linkpath=str(folder_link)
    )
    assert os.path.islink(folder_link)
    ...
    with mock.patch(
        ...
        ModelCheckpoint._link_checkpoint(
            trainer, filepath=str(file), linkpath=str(link)
        )
...
def test_model_checkpoint_link_checkpoint_relative_path(tmp_path, monkeypatch):
    ...
    link = folder / "link"
    ModelCheckpoint._link_checkpoint(
        trainer, filepath=str(file.absolute()), linkpath=str(link.absolute())
    )
    assert os.path.islink(link)
...
def test_none_monitor_not_alternating(tmp_path):
    ...
    model = ListDirModel()
    model_checkpoint = ModelCheckpoint(
        dirpath=tmp_path, monitor=None, save_top_k=1, filename="model"
    )
    trainer = Trainer(
        callbacks=model_checkpoint,
...
def test_model_checkpoint_save_last_none_monitor(tmp_path, caplog):
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor=None, dirpath=tmp_path, save_top_k=-1, save_last=True
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
...
def test_model_checkpoint_every_n_epochs(tmp_path, every_n_epochs):
    ...
    expected = (
        [f"epoch={e}.ckpt" for e in range(epochs) if not (e + 1) % every_n_epochs]
        if every_n_epochs > 0
        else []
    )
    assert set(os.listdir(tmp_path)) == set(expected)
...
def test_ckpt_every_n_train_steps(tmp_path):
    ...
    expected = [
        f"step={i}.ckpt"
        for i in range(
            every_n_train_steps, max_epochs * epoch_length + 1, every_n_train_steps
        )
    ]
    assert set(os.listdir(tmp_path)) == set(expected)
...
def test_model_checkpoint_on_exception_run_condition_on_validation_start(tmp_path):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=tmp_path, filename="sanity_check", save_on_exception=True
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
...
def test_model_checkpoint_on_exception_fast_dev_run_on_train_batch_start(tmp_path):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=tmp_path, filename="fast_dev_run", save_on_exception=True
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        fast_dev_run=2,
...
def test_model_checkpoint_train_time_interval(mock_datetime, tmp_path) -> None:
    ...
    mock_datetime.monotonic.side_effect = [
        start_time + seconds_per_batch * i for i in range(max_batches)
    ]
...
def test_model_checkpoint_topk_zero(tmp_path):
    ...
    model = LogInTwoMethods()
    checkpoint_callback = ModelCheckpoint(
        dirpath=tmp_path, save_top_k=0, save_last=True
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        callbacks=[checkpoint_callback],
...
def test_model_checkpoint_topk_all(tmp_path):
    ...
    assert set(checkpoint_callback.best_k_models.keys()) == {
        str(tmp_path / f"epoch={i}.ckpt") for i in range(epochs)
    }
...
def test_ckpt_metric_names(tmp_path):
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                monitor="early_stop_on", dirpath=tmp_path, filename="{val_loss:.2f}"
            )
        ],
    )
...
def test_default_checkpoint_behavior(tmp_path):
    ...
    with patch.object(
        trainer, "save_checkpoint", wraps=trainer.save_checkpoint
    ) as save_mock:
        trainer.fit(model)
...
def test_checkpointing_with_nan_as_first(tmp_path, mode):
    ...
    assert (
        mode == "min"
        and callback.best_model_score == 5
        or mode == "max"
        and callback.best_model_score == 8
    )
...
def test_checkpoint_repeated_strategy(tmp_path):
    ...
    nothing run."""
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss", dirpath=tmp_path, filename="{epoch:02d}"
    )
...
def test_configure_model_checkpoint(tmp_path):
    ...
    assert [c for c in trainer.callbacks if isinstance(c, ModelCheckpoint)] == [
        callback1
    ]
...
def test_val_check_interval_checkpoint_files(tmp_path):
    ...
    model = LogInTwoMethods()
    model_checkpoint = ModelCheckpoint(
        dirpath=tmp_path, save_top_k=-1, monitor="val_acc", mode="max"
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
...
def test_current_score(tmp_path):
    ...
    model_checkpoint = ModelCheckpoint(
        dirpath=tmp_path, save_top_k=3, monitor="foo", mode="min"
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        max_epochs=3,
...
def test_current_score_when_nan(tmp_path, mode: str):
    ...
    model_checkpoint = ModelCheckpoint(
        dirpath=tmp_path, save_top_k=1, monitor="foo", mode=mode
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        max_epochs=1,
...
@pytest.mark.parametrize(
    "use_omegaconf", [False, pytest.param(True, marks=RunIf(omegaconf=True))]
)
...
def test_ckpt_version_after_rerun_new_trainer(tmp_path):
    ...
    for i in range(epochs):
        mc = ModelCheckpoint(
            dirpath=tmp_path, save_top_k=-1, monitor="epoch", filename="{epoch}"
        )
        trainer = Trainer(
            max_epochs=epochs,
            limit_train_batches=1,
        ...
        expected = (
            {"epoch=0-v1.ckpt", "epoch=1-v1.ckpt"}
            if i
            else {"epoch=0.ckpt", "epoch=1.ckpt"}
        )
...
def test_ckpt_version_after_rerun_same_trainer(tmp_path):
    ...
    used."""
    mc = ModelCheckpoint(
        dirpath=tmp_path, save_top_k=-1, monitor="epoch", filename="test"
    )
    ...
    ckpt_range = range(
        mc.STARTING_VERSION, trainer.max_epochs + mc.STARTING_VERSION - 1
    )
...
def test_model_checkpoint_mode_options():
    with pytest.raises(
        MisconfigurationException, match="`mode` can be .* but got unknown_option"
    ):
...
def test_check_val_every_n_epochs_top_k_integration(tmp_path):
    model = BoringModel()
    mc = ModelCheckpoint(
        dirpath=tmp_path, monitor="epoch", save_top_k=-1, filename="{epoch}"
    )
    trainer = Trainer(
        default_root_dir=tmp_path,
        limit_train_batches=1,
...
def test_model_checkpoint_saveload_ckpt(tmp_path):
    ...
    cb_restore = ModelCheckpoint(
        dirpath=(tmp_path / "restore"), monitor=None, save_top_k=-1, save_last=True
    )
...
def test_save_last_versioning(tmp_path):
    ...
    assert all(
        not os.path.islink(tmp_path / path) for path in set(os.listdir(tmp_path))
    )
...
def test_resume_and_old_checkpoint_files_remain(same_resume_folder, tmp_path):
    ...
    callback = ModelCheckpoint(
        dirpath=first, monitor="step", mode="max", save_top_k=2, every_n_train_steps=2
    )
...
def test_save_last_only_when_checkpoint_saved(tmp_path):
    ...
    class SelectiveModel(BoringModel):
        ...
        def validation_step(self, batch, batch_idx):
            ...
            loss = (
                torch.tensor(1.0 - epoch * 0.1)
                if epoch % 2 == 0
                else torch.tensor(1.0 + epoch * 0.1)
            )
    ...
    assert (
        "last.ckpt" in checkpoint_names
    ), "last.ckpt should exist since checkpoints were saved"
    expected_files = 2  # best checkpoint + last.ckpt
    assert (
        len(checkpoint_files) == expected_files
