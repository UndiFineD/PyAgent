...
def test_on_before_zero_grad_called(tmp_path, max_steps):
    ...
    trainer = Trainer(
        devices=1, default_root_dir=tmp_path, max_steps=max_steps, max_epochs=2
    )
...
def test_on_train_epoch_end_metrics_collection(tmp_path):
    ...
    model = CurrentModel()
    trainer = Trainer(
        max_epochs=num_epochs, default_root_dir=tmp_path, overfit_batches=2
    )
    trainer.fit(model)
...
def test_apply_batch_transfer_handler(
    model_getter_mock, accelerator, expected_device_str
):
    ...
    class TestModel(BoringModel):
        ...
        def train_dataloader(self):
            return torch.utils.data.DataLoader(
                RandomDataset(32, 64), collate_fn=collate_fn
            )
...
def get_members(cls):
    return {
        h for h, _ in getmembers(cls, predicate=isfunction) if not h.startswith("_")
    }
...
def test_trainer_model_hook_system_fit(
    ...
    assert (
        is_overridden("on_validation_model_train", model)
        == override_on_validation_model_train
    )
...
def test_trainer_model_hook_system_eval(
    ...
    hooks = [
        ...
        *model._eval_epoch(
            noun, trainer, model, batches, key, trainer.strategy.root_device
        ),
...
def test_load_from_checkpoint_hook_calls(override_configure_model, tmp_path):
    ...
    expected_lm_called = (
        [{"name": "configure_model"}] if override_configure_model else []
    )
    expected_lm_called += [
        {
