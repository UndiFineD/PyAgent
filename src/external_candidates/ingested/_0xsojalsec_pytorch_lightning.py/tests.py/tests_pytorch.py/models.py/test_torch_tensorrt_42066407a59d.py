# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\models\test_torch_tensorrt.py
import os
import re
...
@pytest.mark.skipif(
    _TORCH_TRT_AVAILABLE, reason="Run this test only if tensorrt is not available."
)
...
def test_missing_tensorrt_package():
    ...
    with pytest.raises(
        ModuleNotFoundError,
        match=re.escape(
            f"`{type(model).__name__}.to_tensorrt` requires `torch_tensorrt` to be installed. "
        ),
    ):
        model.to_tensorrt("model.trt")
