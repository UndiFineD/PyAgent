...
def test_dataloaders_passed_to_fit(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=1, accelerator="tpu", devices="auto"
    )
    trainer.fit(
        model,
...
def test_exception_when_no_tpu_found(_, xla_available):
    ...
    with pytest.raises(
        MisconfigurationException, match="XLAAccelerator` can not run on your system"
    ):
...
def test_accelerator_set_when_using_tpu(devices):
    ...
    assert isinstance(
        Trainer(accelerator="tpu", devices=devices).accelerator, XLAAccelerator
    )
...
def xla_launch(fn):
    ...
    strategy = XLAStrategy(
        ...
        parallel_devices=XLAAccelerator.get_parallel_devices(
            XLAAccelerator.auto_device_count()
        ),
    )
...
class AssertXLADebugModel(BoringModel):
    def on_train_start(self):
        assert (
            os.environ.get("PT_XLA_DEBUG") == "1"
        ), "PT_XLA_DEBUG was not set in environment variables"
