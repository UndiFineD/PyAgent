# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\models\test_hparams.py
...
def _run_standard_hparams_test(
    ...
    trainer.fit(
        model, datamodule=datamodule if issubclass(cls, LightningDataModule) else None
    )
    ...
    if try_overwrite:
        ...
        obj3 = cls.load_from_checkpoint(
            raw_checkpoint_path, test_arg=78, weights_only=weights_only
        )
...
def test_omega_conf_hparams(tmp_path, cls):
    ...
    raw_checkpoint_path = _run_standard_hparams_test(
        tmp_path, model, cls, datamodule=datamodule, weights_only=False
    )
...
if _OMEGACONF_AVAILABLE:
    ...
    class DictConfSubClassBoringModel(SubClassBoringModel):
        def __init__(
            self, *args, dict_conf=OmegaConf.create({"my_param": "something"}), **kwargs
        ):
            super().__init__(*args, **kwargs)
            self.save_hyperparameters()
...
def test_collect_init_arguments(tmp_path, cls: BoringModel):
    ...
    if isinstance(
        model, (SubClassBoringModel, NonSavingSubClassBoringModel, MixinForBoringModel)
    ):
    ...
    assert LightningModule.CHECKPOINT_HYPER_PARAMS_KEY in raw_checkpoint
    assert (
        raw_checkpoint[LightningModule.CHECKPOINT_HYPER_PARAMS_KEY]["batch_size"] == 179
    )
    ...
    model = cls.load_from_checkpoint(
        raw_checkpoint_path, batch_size=99, weights_only=weights_only
    )
...
@pytest.mark.parametrize(
    "base_class", [HyperparametersMixin, LightningModule, LightningDataModule]
)
    ...
    class ParentModule(base_class):
        def __init__(self, batch_size, optimizer):
            ...
            self.child = ChildModule(
                dropout=0.1, activation="relu", init_method="xavier"
            )
...
@pytest.mark.parametrize(
    ("cls", "config"),
    [(AnotherArgModel, {"arg1": 42}), (OtherArgsModel, {"arg1": 3.14, "arg2": "abc"})],
)
...
def test_args(tmp_path):
    ...
    raw_checkpoint_path = _raw_checkpoint_path(trainer)
    with pytest.raises(
        TypeError, match=r"__init__\(\) got an unexpected keyword argument 'test'"
    ):
...
@pytest.mark.skipif(
    RequirementCache("hydra-core<1.1"), reason="Requires Hydra's Compose API"
)
def test_model_save_hyper_parameters_interpolation_with_hydra(tmp_path):
    ...
    with initialize(config_path="conf"):
        ...
        checkpoint_callback = ModelCheckpoint(
            monitor=None, dirpath=tmp_path, save_top_k=-1
        )
        trainer = Trainer(
            default_root_dir=tmp_path,
...
@pytest.mark.parametrize(
    "model", [IgnoreAllParametersModel(arg1=14, arg2=90, arg3=50), NoParametersModel()]
)
...
@pytest.mark.parametrize(
    "model", [SaveHparamsModel({"arg1": 5, "arg2": "abc"}), NoHparamsModel()]
)
@pytest.mark.parametrize(
    "data", [DataModuleWithHparams({"data_dir": "foo"}), DataModuleWithoutHparams()]
)
...
def test_colliding_hparams(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=1, logger=CSVLogger(tmp_path)
    )
...
def test_nn_modules_warning_when_saved_as_hparams():
    ...
    with pytest.warns(
        UserWarning, match="is an instance of `nn.Module` and is already saved"
    ):
