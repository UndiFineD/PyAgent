# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\models\test_restore.py
        ...
        def _check_optimizers(self):
            return all(
                ...
                for optimizer, state in zip(
                    self.trainer.optimizers, state_dict["optimizer_states"]
                )
            )
        ...
        def _check_schedulers(self):
            return all(
                ...
                for config, state in zip(
                    self.trainer.lr_scheduler_configs, state_dict["lr_schedulers"]
                )
            )
...
def test_correct_step_and_epoch(tmp_path):
    ...
    class TestModel(BoringModel):
        def on_train_start(self) -> None:
            ...
            assert (
                ...
                == first_max_epochs * train_batches
            )
    ...
    assert (
        trainer.fit_loop.epoch_loop._batches_that_stepped == max_epochs * train_batches
    )
...
def test_callbacks_state_fit_ckpt_path(tmp_path):
    ...
    def get_trainer_args():
        checkpoint = ModelCheckpoint(
            dirpath=tmp_path, monitor="val_loss", save_last=True
        )
    ...
    for before, after in zip(callbacks_before_resume, callback_capture.callbacks):
        if isinstance(before, ModelCheckpoint):
            for attribute in (
                ...
                assert getattr(before, attribute) == getattr(
                    after, attribute
                ), f"{attribute}"
...
@pytest.mark.parametrize(
    "model_template", [ValTestLossBoringModel, GenericValTestLossBoringModel]
)
def test_load_model_from_checkpoint(tmp_path, model_template):
    ...
    trainer_options = {
        ...
        "limit_test_batches": 2,
        "callbacks": [
            ModelCheckpoint(dirpath=tmp_path, monitor="val_loss", save_top_k=-1)
        ],
        "default_root_dir": tmp_path,
        "accelerator": "cpu",
    }
    ...
    assert (
        model_template.CHECKPOINT_HYPER_PARAMS_KEY in ckpt
    ...
    for (old_name, old_p), (new_name, new_p) in zip(
        model.named_parameters(), pretrained_model.named_parameters()
    ):
        assert torch.all(
            torch.eq(old_p, new_p)
        ), "loaded weights are not the same as the saved weights"
...
def test_strict_model_load_more_params(monkeypatch, tmp_path, tmpdir_server, url_ckpt):
    ...
    hparams_path = os.path.join(
        tutils.get_data_path(logger, path_dir=tmp_path), "hparams.yaml"
    )
    ...
    with pytest.raises(
        RuntimeError,
        match=r'Unexpected key\(s\) in state_dict: "c_d3.weight", "c_d3.bias"',
    ):
        BoringModel.load_from_checkpoint(
