...
def test_torchelastic_gpu_parsing(cuda_count_1, devices):
    ...
    assert isinstance(
        trainer._accelerator_connector.cluster_environment, TorchElasticEnvironment
    )
    # when using gpu
    if _parse_gpu_ids(devices, include_cuda=True) is not None:
        assert isinstance(trainer.accelerator, CUDAAccelerator)
        assert (
            trainer.num_devices == len(devices)
...
def test_input_tensors_cast_before_transfer_to_device(
    strategy, precision, expected_dtype
):
    ...
    trainer = Trainer(
        strategy=strategy, devices=1, precision=precision, barebones=True, max_steps=2
    )
    trainer.fit(model)
