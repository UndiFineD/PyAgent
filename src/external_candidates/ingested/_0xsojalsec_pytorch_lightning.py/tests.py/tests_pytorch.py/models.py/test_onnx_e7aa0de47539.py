# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\models\test_onnx.py
...
@pytest.mark.parametrize(
    ...
    [
        None,
        pytest.param(
            False, marks=RunIf(min_torch="2.5.0", dynamo=True, onnxscript=True)
        ),
        pytest.param(
            True, marks=RunIf(min_torch="2.5.0", dynamo=True, onnxscript=True)
        ),
    ],
)
@RunIf(onnx=True)
def test_if_inference_output_is_valid(tmp_path, dynamo):
    ...
    ort_kwargs = (
        ...
        if compare_version("onnxruntime", operator.ge, "1.16.0")
        else {}
    )
...
@pytest.mark.skipif(
    _ONNXSCRIPT_AVAILABLE, reason="Run this test only if onnxscript is not available."
)
