# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\plugins\precision\test_xla.py
...
def test_precision_input_validation(xla_available):
    ...
    with pytest.raises(
        ValueError, match=re.escape("`precision='16')` is not supported in XLA")
    ):
        XLAPrecision("16")
    with pytest.raises(
        ValueError, match=re.escape("`precision='16-mixed')` is not supported in XLA")
    ):
        XLAPrecision("16-mixed")
    with pytest.raises(
        ValueError, match=re.escape("`precision='bf16-mixed')` is not supported in XLA")
    ):
        XLAPrecision("bf16-mixed")
    with pytest.raises(
        ValueError, match=re.escape("`precision='64-true')` is not supported in XLA")
    ):
