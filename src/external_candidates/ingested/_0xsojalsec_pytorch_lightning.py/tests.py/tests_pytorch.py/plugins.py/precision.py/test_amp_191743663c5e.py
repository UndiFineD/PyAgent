# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\plugins\precision\test_amp.py
...
def test_clip_gradients():
    ...
    precision.clip_gradients(
        optimizer, clip_val=1.0, gradient_clip_algorithm=GradClipAlgorithmType.VALUE
    )
    ...
    precision.clip_gradients(
        optimizer, clip_val=1.0, gradient_clip_algorithm=GradClipAlgorithmType.NORM
    )
    precision.clip_grad_by_value.assert_not_called()
...
def test_optimizer_amp_scaling_support_in_step_method():
    ...
    with pytest.raises(
        RuntimeError,
        match="The current optimizer.*does not allow for gradient clipping",
    ):
