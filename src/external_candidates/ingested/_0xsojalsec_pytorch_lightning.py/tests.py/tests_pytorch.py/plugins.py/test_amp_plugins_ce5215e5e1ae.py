# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\plugins\test_amp_plugins.py
...
def test_amp_precision_plugin_parameter_validation():
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            "Passed `MixedPrecision(precision='16')`. Precision must be '16-mixed' or 'bf16-mixed'"
        ),
    ):
