# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\plugins\test_checkpoint_io_plugin.py
...
def test_checkpoint_plugin_called(tmp_path):
    ...
    assert trainer.checkpoint_callback.best_model_path == str(
        tmp_path / "epoch=1-step=2.ckpt"
    )
    ...
    checkpoint_plugin.load_checkpoint.assert_called_with(
        str(tmp_path / "last.ckpt"), weights_only=None
    )
    ...
    assert ckpt_files == {
        "epoch=1-step=2.ckpt",
        "last.ckpt",
        "epoch=1-step=2-v1.ckpt",
        "last-v1.ckpt",
    }
    assert trainer.checkpoint_callback.best_model_path == str(
        tmp_path / "epoch=1-step=2-v1.ckpt"
    )
    ...
    checkpoint_plugin.load_checkpoint.assert_called_with(
        str(tmp_path / "last-v1.ckpt"), weights_only=None
    )
...
def test_async_checkpoint_plugin(tmp_path):
    ...
    checkpoint_plugin.remove_checkpoint = Mock(
        wraps=checkpoint_plugin.remove_checkpoint
    )
