# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\deprecated_api\test_no_removal_version.py
import sys
...
def test_configure_sharded_model():
    ...
    with pytest.raises(
        RuntimeError,
        match="Both `MyModelBoth.configure_model`, and `MyModelBoth.configure_sharded_model`",
    ):
        trainer.fit(model)
...
def test_fsdp_activation_checkpointing():
    with pytest.raises(ValueError, match="cannot set both `activation_checkpointing"):
        FSDPStrategy(
            ...
            activation_checkpointing_policy=lambda *_: True,
        )


def test_double_precision_wrapper():
    with pytest.deprecated_call(
        match=r"The `LightningDoublePrecisionModule` is deprecated and no longer needed"
    ):
...
def test_bitsandbytes_precision_plugin(monkeypatch):
    ...
    monkeypatch.setattr(
        pytorch_lightning_enterprise.plugins.precision.bitsandbytes,
    ...
    with pytest.deprecated_call(
        match=r"The `BitsandbytesPrecisionPlugin` is deprecated"
    ):
...
def test_transformer_engine_precision_plugin(monkeypatch):
    monkeypatch.setattr(
        pytorch_lightning_enterprise.plugins.precision.transformer_engine,
    ...
    with pytest.deprecated_call(
        match=r"The `TransformerEnginePrecisionPlugin` is deprecated"
    ):
