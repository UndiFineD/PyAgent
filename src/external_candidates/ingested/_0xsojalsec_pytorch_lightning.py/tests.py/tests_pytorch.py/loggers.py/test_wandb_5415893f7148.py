# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loggers\test_wandb.py
...
def test_wandb_logger_init(wandb_mock):
    ...
    wandb_mock.init().config.update.assert_called_once_with(
        expected, allow_val_change=True
    )

    # watch a model
    logger.watch("model", "log", 10, False)
    wandb_mock.init().watch.assert_called_once_with(
        "model", log="log", log_freq=10, log_graph=False
    )
...
def test_wandb_logger_sync_tensorboard(wandb_mock):
    ...
    wandb_mock.init.return_value.define_metric.assert_called_once_with(
...
def test_wandb_logger_experiment_called_first(wandb_mock, tmp_path):
    ...
    with (
        mock.patch.object(
            TensorBoardLogger,
            "experiment",
            new_callable=lambda: mock.PropertyMock(
...
def test_wandb_logger_dirs_creation(wandb_mock, tmp_path):
    ...
    assert trainer.checkpoint_callback.dirpath == str(
        tmp_path / "project" / version / "checkpoints"
    )
    assert set(os.listdir(trainer.checkpoint_callback.dirpath)) == {
        "epoch=0-step=3.ckpt"
    }
    assert trainer.log_dir == logger.save_dir
...
def test_wandb_log_model(wandb_mock, tmp_path):
    ...
    wandb_mock.Artifact.reset_mock()
    logger = WandbLogger(
        save_dir=tmp_path, log_model=True, checkpoint_name="my-test-model"
    )
    logger.experiment.id = "1"
    logger.experiment.name = "run_name"
    trainer = Trainer(
    ...
    wandb_mock.init().log_artifact.assert_called_with(
        wandb_mock.Artifact(), aliases=["latest"]
    )
    ...
    wandb_mock.init().log_artifact.assert_called_with(
        wandb_mock.Artifact(), aliases=["latest", "best"]
    )
    ...
    for name, val, version in [("epoch", 0, 2), ("step", 3, 3)]:
        ...
        wandb_mock.init().log_artifact.assert_any_call(
            wandb_mock.Artifact(), aliases=["latest"]
        )
...
def test_wandb_log_media(wandb_mock, tmp_path):
    ...
    logger.log_image(
        key="samples", images=["1.jpg", "2.jpg"], caption=["caption 1", "caption 2"]
    )
    ...
    with pytest.raises(
        TypeError, match="""Expected a list as "images", found <class 'str'>"""
    ):
        logger.log_image(key="samples", images="1.jpg")

    # test log_image with wrong number of captions
    with pytest.raises(
        ValueError, match="Expected 2 items but only found 1 for caption"
    ):
        logger.log_image(
            key="samples", images=["1.jpg", "2.jpg"], caption=["caption 1"]
        )
    ...
    logger.log_audio(
        key="samples", audios=["1.mp3", "2.mp3"], caption=["caption 1", "caption 2"]
    )
    ...
    with pytest.raises(
        TypeError, match="""Expected a list as "audios", found <class 'str'>"""
    ):
    ...
    with pytest.raises(
        ...
        logger.log_audio(
            key="samples", audios=["1.mp3", "2.mp3"], caption=["caption 1"]
        )
    ...
    logger.log_video(
        key="samples", videos=["1.mp4", "2.mp4"], caption=["caption 1", "caption 2"]
    )
    ...
    with pytest.raises(
        TypeError, match="""Expected a list as "videos", found <class 'str'>"""
    ):
    ...
    with pytest.raises(
        ...
        logger.log_video(
            key="samples", videos=["1.mp4", "2.mp4"], caption=["caption 1"]
        )
    ...
    wandb_mock.init().log.assert_called_once_with(
        {"samples": wandb_mock.Table(), "trainer/global_step": 5}
    )
...
def test_wandb_logger_offline_log_model(wandb_mock, tmp_path):
    ...
    with pytest.raises(
        ValueError, match="checkpoints cannot be uploaded in offline mode"
    ):
...
@pytest.mark.parametrize(
    ("log_model", "expected"), [("True", True), ("False", False), ("all", "all")]
)
def test_wandb_logger_cli_integration(
    log_model, expected, wandb_mock, monkeypatch, tmp_path
):
