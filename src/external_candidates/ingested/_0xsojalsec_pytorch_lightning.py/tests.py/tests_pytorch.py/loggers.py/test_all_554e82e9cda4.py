# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loggers\test_all.py
...
ALL_LOGGER_CLASSES_WO_NEPTUNE = tuple(
    filter(lambda cls: cls is not NeptuneLogger, ALL_LOGGER_CLASSES)
)
...
def test_loggers_fit_test_all(
    ...
    class CustomModel(BoringModel):
        ...
        def on_validation_epoch_end(self):
            self.log_dict(
                {"early_stop_on": torch.tensor(1), "val_loss": torch.tensor(0.5)}
            )
...
@mock.patch.dict(os.environ, {})
@pytest.mark.parametrize(
    "logger_class", [*ALL_LOGGER_CLASSES_WO_NEPTUNE, CustomLoggerWithoutExperiment]
)
...
@mock.patch.dict(os.environ, {})
@mock.patch("pytorch_lightning_enterprise.loggers.mlflow._get_resolve_tags", Mock())
def test_logger_with_prefix_all(
    mlflow_mock, wandb_mock, comet_mock, neptune_mock, monkeypatch, tmp_path
):
    ...
    logger.experiment.log.assert_called_once_with(
        {"tmp-test": 1.0, "trainer/global_step": 0}
    )
...
def test_logger_default_name(mlflow_mock, monkeypatch, tmp_path):
    ...
    _ = logger.experiment
    logger.logger_impl._mlflow_client.create_experiment.assert_called_with(
