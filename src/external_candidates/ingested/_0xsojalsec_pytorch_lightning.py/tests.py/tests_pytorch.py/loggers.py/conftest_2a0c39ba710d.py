# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loggers\conftest.py
...
def mlflow_mock(monkeypatch):
    ...
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.loggers.mlflow._MLFLOW_AVAILABLE", True
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.loggers.mlflow._MLFLOW_SYNCHRONOUS_AVAILABLE",
        True,
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.utils.imports._MLFLOW_AVAILABLE", True
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.utils.imports._MLFLOW_SYNCHRONOUS_AVAILABLE", True
    )
    return mlflow
...
def wandb_mock(monkeypatch):
    ...
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.loggers.wandb._WANDB_AVAILABLE", True
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.utils.imports._WANDB_AVAILABLE", True
    )
    return wandb
...
def comet_mock(monkeypatch):
    ...
    comet.ExistingExperiment = MagicMock(
        name="ExistingExperiment", return_value=comet_experiment
    )
    comet.OfflineExperiment = MagicMock(
        name="OfflineExperiment", return_value=comet_experiment
    )
    ...
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.loggers.comet._COMET_AVAILABLE", True
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.utils.imports._COMET_AVAILABLE", True
    )
    return comet
...
def neptune_mock(monkeypatch):
    ...
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.loggers.neptune._NEPTUNE_AVAILABLE", True
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.utils.imports._NEPTUNE_AVAILABLE", True
    )
    return neptune
