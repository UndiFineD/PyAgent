...
def mock_mlflow_run_creation(
    logger, experiment_name=None, experiment_id=None, run_id=None
):
    """Helper function to simulate mlflow client creating a new (or existing) experiment."""
    run = MagicMock()
    run.info.run_id = run_id
    logger.logger_impl._mlflow_client.get_experiment_by_name = MagicMock(
        return_value=experiment_name
    )
    logger.logger_impl._mlflow_client.create_experiment = MagicMock(
...
def test_mlflow_logger_dirs_creation(tmp_path):
    ...
    assert (
        set(os.listdir(tmp_path / exp_id / run_id / "params")) == model.hparams.keys()
    )
    assert trainer.checkpoint_callback.dirpath == str(
        tmp_path / exp_id / run_id / "checkpoints"
    )
    assert os.listdir(trainer.checkpoint_callback.dirpath) == [
        f"epoch=0-step={limit_batches}.ckpt"
    ]


@mock.patch("pytorch_lightning_enterprise.loggers.mlflow._get_resolve_tags", Mock())
@mock.patch(
    "pytorch_lightning_enterprise.utils.imports._MLFLOW_AVAILABLE", return_value=True
)
...
@mock.patch("pytorch_lightning_enterprise.loggers.mlflow._get_resolve_tags", Mock())
def test_mlflow_logger_experiment_calls_with_synchronous(
    mlflow_mock, tmp_path, synchronous
):
...
@mock.patch("pytorch_lightning_enterprise.loggers.mlflow._get_resolve_tags", Mock())
@mock.patch(
    "pytorch_lightning_enterprise.loggers.mlflow._MLFLOW_SYNCHRONOUS_AVAILABLE", False
)
