# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loggers\test_neptune.py
...
def _fit_and_test(logger, model, tmp_path):
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=1, limit_train_batches=0.05, logger=logger
    )
...
def _get_logger_with_mocks(**kwargs):
    ...
    logger.logger_impl._run_instance.__getitem__.return_value.fetch.return_value = (
        "exp-name"
    )
...
def test_neptune_online(neptune_mock):
    ...
    created_run_mock.__getitem__.assert_has_calls(
        [call("sys/id"), call("sys/name")], any_order=True
    )
    created_run_mock.__setitem__.assert_called_once_with(
        "source_code/integrations/pytorch-lightning", pl.__version__
    )
...
def test_online_with_wrong_kwargs(neptune_mock):
    ...
    with pytest.raises(
        ValueError, match="Run parameter expected to be of type `neptune.Run`*"
    ):
        NeptuneLogger(run="some string")

    with pytest.raises(
        ValueError, match="When an already initialized run object is provided*"
    ):
...
def test_neptune_additional_methods(neptune_mock):
    logger, run_instance_mock, _ = _get_logger_with_mocks(
        api_key="test", project="project"
    )
...
def test_neptune_log_metrics_on_trained_model(neptune_mock, tmp_path, monkeypatch):
    ...
    run_instance_mock.__getitem__.return_value.append.assert_has_calls(
        [call(42, step=2)]
    )
...
def test_log_metrics(neptune_mock):
    ...
    for prefix, (metrics_foo_key, metrics_bar_key) in test_variants:
        logger, run_instance_mock, run_attr_mock = _get_logger_with_mocks(
            api_key="test", project="project", **prefix
        )
        ...
        run_attr_mock.append.assert_has_calls(
            [call(42, step=None), call(555, step=None)]
        )
...
def test_log_model_summary(neptune_mock):
    ...
    for prefix, model_summary_key in test_variants:
        ...
        run_instance_mock.__setitem__.assert_called_once_with(
            model_summary_key, file_from_content_mock
        )
...
def test_after_save_checkpoint(neptune_mock):
    ...
    for prefix, model_key_prefix in test_variants:
        ...
        assert (
            run_instance_mock.__getitem__.call_count == 4
        )  # last_model_path, best_k_models, best_model_path
        assert (
            run_attr_mock.upload.call_count == 4
        )  # last_model_path, best_k_models, best_model_path
        assert mock_file.from_stream.call_count == 0

        run_instance_mock.__getitem__.assert_any_call(
            f"{model_key_prefix}/checkpoints/model1"
        )
        run_instance_mock.__getitem__.assert_any_call(
            f"{model_key_prefix}/checkpoints/model2/with/slashes"
        )
