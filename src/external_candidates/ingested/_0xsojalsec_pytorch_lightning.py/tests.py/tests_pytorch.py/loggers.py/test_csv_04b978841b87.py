# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loggers\test_csv.py
...
def test_manual_versioning_file_exists(tmp_path):
    ...
    with pytest.warns(
        UserWarning, match="Experiment logs directory .* exists and is not empty"
    ):
        _ = logger.experiment
...
def test_fit_csv_logger(tmp_path):
    ...
    trainer = Trainer(
        default_root_dir=tmp_path, max_steps=10, logger=logger, log_every_n_steps=1
    )
