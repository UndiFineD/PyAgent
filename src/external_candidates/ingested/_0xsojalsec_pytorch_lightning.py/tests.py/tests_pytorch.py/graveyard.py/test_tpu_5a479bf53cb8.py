# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\graveyard\test_tpu.py
import os
...
def test_graveyard_single_tpu(import_path, name):
    ...
    with (
        pytest.deprecated_call(match="is deprecated"),
        pytest.raises(ModuleNotFoundError, match="torch_xla"),
    ):
        cls(device)
...
def test_graveyard_no_device(import_path, name, monkeypatch):
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.accelerators.xla._XLA_AVAILABLE",
        MockXLAAvailable(False),
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.precision.xla._XLA_AVAILABLE",
        MockXLAAvailable(False),
    )
