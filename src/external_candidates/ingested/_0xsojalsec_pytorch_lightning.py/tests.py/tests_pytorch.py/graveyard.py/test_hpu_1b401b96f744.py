# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\graveyard\test_hpu.py
...
def test_extracted_hpu(import_path, name):
    ...
    with pytest.raises(
        NotImplementedError,
        match=f"{name}` class has been removed. Please contact developer@lightning.ai",
    ):
        cls()
