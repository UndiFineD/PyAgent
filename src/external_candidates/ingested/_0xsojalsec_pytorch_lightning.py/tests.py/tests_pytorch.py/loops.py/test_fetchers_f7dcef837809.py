...
def test_prefetch_iterator(multiple_iterables, dataset_cls, prefetch_batches):
    ...
    def generate():
        generated = [
            (fetcher.fetched, data, fetcher.done)
            for data, batch_idx, dataloader_idx in fetcher
        ]
    ...
    fetched = (
        [1, 2, 3]
        if dataset_cls is SizedDataset
        else [1, 2, 3, 3, 3, 3, 3][prefetch_batches : prefetch_batches + 3]
    )
...
def test_fetching_dataloader_iter_opt(automatic_optimization, tmp_path):
    class TestModel(BoringModel):
        ...
        def training_step(self, dataloader_iter):
            assert isinstance(
                self.trainer.fit_loop._data_fetcher, _DataLoaderIterDataFetcher
            )
            # fetch 2 batches
...
def test_training_step_with_dataloader_iter(tmp_path) -> None:
    ...
    trainer.fit(m)
    assert (
        m.num_batches_processed == DATASET_LEN
    ), f"Expect all {DATASET_LEN} batches to be processed."
...
def test_step_methods_with_dataloader_iter(
    limit_sanity_val_batches, limit_train_batches, limit_eval_batches, tmp_path
):
    global_batch_size = 4
    micro_batch_size = 2
    ...
    limit_sanity_val_batches = (
        2 if limit_sanity_val_batches is None else limit_sanity_val_batches
    )
    ...
    assert (
        model.record["sanity_validation"]["entered"]
        == length(data, limit_sanity_val_batches) // fetches_per_step
    )
    assert model.record["sanity_validation"]["fetched"] == length(
        data, limit_sanity_val_batches
    )
    assert model.record["sanity_validation"]["raised"] == 0
    assert (
        model.record["training"]["entered"]
        == length(data, limit_train_batches) // fetches_per_step
    )
    assert model.record["training"]["fetched"] == length(data, limit_train_batches)
    assert model.record["training"]["raised"] == 0
    assert (
        model.record["validation"]["entered"]
        == length(data, limit_eval_batches) // fetches_per_step
    )
...
def test_stop_iteration_with_dataloader_iter(trigger_stop_iteration, tmp_path):
    ...
    class TestModel(AsyncBoringModel):
        ...
        def training_step(self, dataloader_iter: Iterator) -> STEP_OUTPUT:
            ...
            if (
                self.trigger_stop_iteration
                and batch_idx == EXPECT_NUM_BATCHES_PROCESSED
            ):
                raise StopIteration
            return output
        ...
        def train_dataloader(self):
            if self.trigger_stop_iteration:
                return DataLoader(
                    RandomDataset(BATCH_SIZE, 2 * EXPECT_NUM_BATCHES_PROCESSED)
                )
...
def test_done_dataloader_iter(iterable):
    ...
    for i in range(
        5
    ):  # doesn't matter how many times you next this, the dataloader_iter needs to be consumed
