...
def test_training_starts_with_seed(tmp_path, seed_once):
    ...
    if seed_once:
        ...
        sequence0 = run_training(
            default_root_dir=tmp_path, max_steps=2, num_sanity_val_steps=0
        )
        sequence1 = run_training(
            default_root_dir=tmp_path, max_steps=2, num_sanity_val_steps=2
        )
...
def test_on_train_batch_start_return_minus_one(max_epochs, batch_idx_, tmp_path):
    ...
    model = CurrentModel()
    trainer = Trainer(
        default_root_dir=tmp_path, max_epochs=max_epochs, limit_train_batches=10
    )
    trainer.fit(model)
...
def test_fit_loop_done_log_messages(caplog):
    ...
    with caplog.at_level(
        level=logging.DEBUG, logger="lightning.pytorch.utilities.rank_zero"
    ):
        assert fit_loop.done
...
def test_should_stop_early_stopping_conditions_met(
    ...
    trainer = Trainer(
        min_epochs=min_epochs, min_steps=min_steps, limit_val_batches=0, max_epochs=100
    )
...
def test_tqdm_total_steps_with_iterator_no_length(tmp_path, max_steps):
    ...
    infinite_iter = (
        torch.randn(batch_size, 32, dtype=torch.float32) for _ in itertools.count(0)
    )
    ...
    if warn:
        ...
        with warnings.catch_warnings(record=True) as warning_list:
            ...
            eval_warnings = [
                ...
                if issubclass(w.category, PossibleUserWarning)
                and "eval mode" in str(w.message)
            ]
            assert len(eval_warnings) == 0, "Expected no eval mode warnings"


@pytest.mark.parametrize(("max_epochs", "batch_idx_"), [(2, 5), (3, 8)])
def test_lr_updated_on_train_batch_start_returns_minus_one(
    tmp_path, max_epochs, batch_idx_
):
    ...
    init_lr = 0.1
    trainer = Trainer(
        default_root_dir=tmp_path, limit_train_batches=10, max_epochs=max_epochs
    )
    trainer.fit(model)
