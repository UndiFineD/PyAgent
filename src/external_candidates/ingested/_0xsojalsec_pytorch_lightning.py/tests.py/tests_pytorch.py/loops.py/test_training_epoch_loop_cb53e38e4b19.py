# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\test_training_epoch_loop.py
...
@pytest.mark.parametrize(
    ("min_epochs", "min_steps", "val_count"), [(3, None, 3), (None, 3, 2)]
)
def test_should_stop_triggers_validation_once(
    min_epochs, min_steps, val_count, tmp_path
):
    ...
    stopping = EarlyStopping(
        monitor="loss", check_on_train_epoch_end=True, stopping_threshold=100
    )
    trainer = Trainer(
...
def test_resume_mid_epoch_warning(tmp_path):
    ...
    def train_and_resume(dataloader, resume_step, expected_warning):
        ...
        trainer = Trainer(
            **trainer_kwargs,
            callbacks=ModelCheckpoint(
                dirpath=checkpoint_dir, every_n_train_steps=1, save_top_k=-1
            ),
        )
        ...
        with warn_assert(
            PossibleUserWarning, match="resuming from a checkpoint that ended before"
        ):
            trainer.fit(BoringModel(), dataloader, ckpt_path=resume_from)

    # Resume mid-epoch, no stateful dataloader -> warning
    train_and_resume(
        dataloader=NotStatefulIterable(), resume_step=1, expected_warning=True
    )

    # Resume end-of-epoch, no stateful dataloader -> no warning
    train_and_resume(
        dataloader=NotStatefulIterable(), resume_step=3, expected_warning=False
    )

    # Resume mid-epoch, stateful dataloader -> no warning
    train_and_resume(
        dataloader=StatefulIterable(), resume_step=1, expected_warning=False
    )
