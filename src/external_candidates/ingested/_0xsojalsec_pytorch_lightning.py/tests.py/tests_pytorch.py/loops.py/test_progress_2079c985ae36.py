# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\test_progress.py
...
def test_progress_from_defaults():
    ...
    expected = _Progress(
        total=_StartedTracker(completed=5), current=_StartedTracker(completed=5)
    )
...
def test_progress_increment_sequence():
    ...
    assert batch.total == _ProcessedTracker(
        ready=1, started=1, processed=1, completed=1
    )
    assert batch.current == _ProcessedTracker(
        ready=1, started=1, processed=1, completed=1
    )
...
def test_progress_raises():
    ...
    with pytest.raises(
        TypeError, match="_ReadyCompletedTracker` doesn't have a `started` attribute"
    ):
        p.increment_started()
    with pytest.raises(
        TypeError, match="_ReadyCompletedTracker` doesn't have a `processed` attribute"
    ):
