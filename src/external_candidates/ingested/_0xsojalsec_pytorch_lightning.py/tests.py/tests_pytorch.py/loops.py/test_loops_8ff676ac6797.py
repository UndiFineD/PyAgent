...
def test_loop_hierarchy():
    ...
    assert (
        loop_parent_copy.loop_child.on_save_checkpoint()
        == state_dict["loop_child.state_dict"]
    )
...
def test_loop_restart_progress_multiple_dataloaders(
    tmp_path, n_dataloaders, stop_dataloader, stop_epoch, stop_batch
):
    n_batches = 5
    n_epochs = 3
    ...
    class ValidationModel(BoringModel):
        ...
        def validation_step(self, batch, batch_idx, dataloader_idx):
            if (
                self.current_epoch == stop_epoch
                and batch_idx == stop_batch
                and dataloader_idx == stop_dataloader
            ):
                raise CustomException
        ...
        def val_dataloader(self):
            return [
                super(ValidationModel, self).val_dataloader()
...
def test_loop_state_on_exception(
    accumulate_grad_batches, stop_epoch, stop_batch, tmp_path
):
    n_epochs = 3
    n_batches = 3
    ...
    assert (
        optim_progress.optimizer.zero_grad.total.completed
        == nbe_total_zero_grad + be_total_zero_grad
    )
...
def test_fit_loop_reset(tmp_path):
    ...
    mid_epoch_ckpt = torch.load(
        str(tmp_path / "epoch=0-step=2.ckpt"), weights_only=True
    )
    ...
    assert (
        fit_loop.epoch_progress.total.completed == 0
    )  # the checkpoint was saved mid epoch
    ...
    assert (
        epoch_loop.batch_progress.total.completed == 1
    )  # the checkpoint was saved on train_batch_end
    assert (
        epoch_loop.batch_progress.current.ready == 2
    )  # currents get set to the completed value
    ...
    end_of_epoch_ckpt = torch.load(
        str(tmp_path / "epoch=0-step=4.ckpt"), weights_only=True
    )
    fit_loop = trainer.fit_loop
    epoch_loop = fit_loop.epoch_loop
...
def test_restart_parity(tmp_path):
    ...
    end_of_epoch_ckpt_v1 = torch.load(
        str(tmp_path / "epoch=0-step=4-v1.ckpt"), weights_only=True
    )

    assert (
        compare_state_dicts(end_of_epoch_ckpt["loops"], end_of_epoch_ckpt_v1["loops"])
        == {}
    )
    assert (
        compare_state_dicts(
    ...
    assert (
        compare_state_dicts(
            end_of_epoch_ckpt["state_dict"], end_of_epoch_ckpt_v1["state_dict"]
        )
        == {}
    )

    mid_epoch_ckpt = torch.load(
        str(tmp_path / "epoch=1-step=6.ckpt"), weights_only=True
    )
    mid_epoch_ckpt_v1 = torch.load(
        str(tmp_path / "epoch=1-step=6-v1.ckpt"), weights_only=True
    )

    assert (
        compare_state_dicts(mid_epoch_ckpt["loops"], mid_epoch_ckpt_v1["loops"]) == {}
    )
    assert (
        compare_state_dicts(
            mid_epoch_ckpt["lr_schedulers"][0], mid_epoch_ckpt_v1["lr_schedulers"][0]
        )
        == {}
    )
    ...
    assert (
        compare_state_dicts(
            mid_epoch_ckpt["state_dict"], mid_epoch_ckpt_v1["state_dict"]
        )
        == {}
    )

    end_of_epoch_ckpt = torch.load(
        str(tmp_path / "epoch=1-step=8.ckpt"), weights_only=True
    )
    end_of_epoch_ckpt_v1 = torch.load(
        str(tmp_path / "epoch=1-step=8-v1.ckpt"), weights_only=True
    )
...
def test_fit_can_fail_during_validation(
    train_datasets, val_datasets, val_check_interval, tmp_path
):
    size, n_batches = 2, 4
    stop_batch = 1
    ...
    assert (
        state_dict_after_restart["epoch_loop.batch_progress"]
        == expected["epoch_loop.batch_progress"]
    )
...
def test_workers_are_shutdown(tmp_path, should_fail, persistent_workers):
    ...
    train_dataloader = TestDataLoader(
        RandomDataset(32, 64), num_workers=1, persistent_workers=persistent_workers
    )
...
def test_validation_during_gradient_accumulation_window(tmp_path):
    ...
    class ValidationModel(BoringModel):
        def on_validation_start(self):
            batch_idx = (
                self.trainer.fit_loop.epoch_loop.batch_progress.current.completed
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda: CombinedLoader(
                [NotStatefulIterable(3), StatefulIterable(1), NotStatefulIterable(2)]
            ),
            True,
            [[3, 1, 2], [4, 2, 3]],
            [[3, 3, 2], [4, 4, 3]],
