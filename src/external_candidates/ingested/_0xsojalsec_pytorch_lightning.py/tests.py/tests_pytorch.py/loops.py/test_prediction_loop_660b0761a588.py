# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\test_prediction_loop.py
...
def test_prediction_loop_batch_sampler_set_epoch_called(
    tmp_path, use_distributed_sampler
):
...
def test_invalid_dataloader_idx_raises_step(tmp_path):
    ...
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.predict_step",
    ):
        trainer.predict(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.predict_step",
    ):
        trainer.predict(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.predict_step",
    ):
        trainer.predict(model)
...
def test_invalid_dataloader_idx_raises_batch_start(tmp_path):
    ...
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.on_predict_batch_start",
    ):
        trainer.predict(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.on_predict_batch_start",
    ):
        trainer.predict(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.on_predict_batch_start",
    ):
        trainer.predict(model)
...
def test_invalid_dataloader_idx_raises_batch_end(tmp_path):
    ...
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.on_predict_batch_end",
    ):
        trainer.predict(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.on_predict_batch_end",
    ):
        trainer.predict(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.on_predict_batch_end",
    ):
        trainer.predict(model)
