# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\test_evaluation_loop.py
...
@mock.patch(
    "lightning.pytorch.loops.evaluation_loop._EvaluationLoop._on_evaluation_epoch_end"
)
...
def test_evaluation_loop_sampler_set_epoch_called(tmp_path, use_batch_sampler):
    ...
    val_sampler = (
        val_dataloader.batch_sampler.sampler
        if use_batch_sampler
        else val_dataloader.sampler
    )
...
def test_log_epoch_metrics_before_on_evaluation_end(
    update_eval_epoch_metrics_mock, tmp_path
):
    """Test that the epoch metrics are logged before the `on_evaluation_end` hook is fired."""
    order = []
    update_eval_epoch_metrics_mock.side_effect = lambda _: order.append(
        "log_epoch_metrics"
    )
...
def test_invalid_dataloader_idx_raises_step(tmp_path):
    ...
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.validation_step",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.test_step",
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        TypeError, match="missing 1 required positional argument: 'dl_idx"
    ):
        trainer.validate(model)
    with pytest.raises(
        TypeError, match="missing 1 required positional argument: 'dl_idx"
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.validation_step",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.test_step",
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.validation_step",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError, match="no `dataloader_idx` argument in `IgnoringModel2.test_step"
    ):
        trainer.test(model)
...
def test_invalid_dataloader_idx_raises_batch_start(tmp_path):
    ...
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.on_validation_batch_start",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.on_test_batch_start",
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.on_validation_batch_start",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.on_test_batch_start",
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.on_validation_batch_start",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.on_test_batch_start",
    ):
        trainer.test(model)
...
def test_invalid_dataloader_idx_raises_batch_end(tmp_path):
    ...
    class ExtraDataloaderIdx(BoringModel):
        def on_validation_batch_end(
            self, outputs, batch, batch_idx, dataloader_idx
        ): ...
    ...
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.on_validation_batch_end",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="have included `dataloader_idx` in `ExtraDataloaderIdx.on_test_batch_end",
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.on_validation_batch_end",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `MultipleDataloader.on_test_batch_end",
    ):
        trainer.test(model)
    ...
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.on_validation_batch_end",
    ):
        trainer.validate(model)
    with pytest.raises(
        RuntimeError,
        match="no `dataloader_idx` argument in `IgnoringModel2.on_test_batch_end",
    ):
        trainer.test(model)
