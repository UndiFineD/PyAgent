# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\optimization\test_automatic_loop.py
...
def test_skip_training_step_not_allowed(world_size, tmp_path):
    ...
    trainer.strategy.world_size = (
        world_size  # mock world size without launching processes
    )
    error_context = (
        pytest.raises(
            RuntimeError, match="Skipping the `training_step` .* is not supported"
        )
        if world_size > 1
        else nullcontext()
    )
