# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\loops\test_training_loop_flow_scalar.py
...
def test__training_step__epoch_end__flow_scalar(tmp_path):
    ...
    train_step_out = trainer.fit_loop.epoch_loop.automatic_optimization.run(
        trainer.optimizers[0], 0, kwargs
    )
    ...
    opt_closure = trainer.fit_loop.epoch_loop.automatic_optimization._make_closure(
        kwargs, trainer.optimizers[0], 0
    )
...
def test_training_step_no_return_when_even(tmp_path):
    ...
    for batch_idx, batch in enumerate(model.train_dataloader()):
        ...
        out = trainer.fit_loop.epoch_loop.automatic_optimization.run(
...
def test_training_step_none_batches(tmp_path):
    ...
    class TestModel(BoringModel):
        ...
        def train_dataloader(self):
            return DataLoader(
                RandomDataset(32, 4), collate_fn=self.collate_none_when_even
            )
