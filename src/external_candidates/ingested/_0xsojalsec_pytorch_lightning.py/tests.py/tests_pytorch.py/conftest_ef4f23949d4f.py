# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\conftest.py
...
def preserve_global_rank_variable():
    ...
    from lightning.fabric.utilities.rank_zero import (
        rank_zero_only as rank_zero_only_fabric,
    )
    from lightning.pytorch.utilities.rank_zero import (
        rank_zero_only as rank_zero_only_pytorch,
    )
    from lightning_utilities.core.rank_zero import (
        rank_zero_only as rank_zero_only_utilities,
    )

    functions = (
        rank_zero_only_pytorch,
        rank_zero_only_fabric,
        rank_zero_only_utilities,
    )
...
def restore_env_variables():
    ...
    assert (
        not leaked_vars
    ), f"test is leaking environment variable(s): {set(leaked_vars)}"
...
def mock_cuda_count(monkeypatch, n: int) -> None:
    monkeypatch.setattr(
        lightning.fabric.accelerators.cuda, "num_cuda_devices", lambda: n
    )
    monkeypatch.setattr(
        lightning.pytorch.accelerators.cuda, "num_cuda_devices", lambda: n
    )
...
def mock_mps_count(monkeypatch, n: int) -> None:
    monkeypatch.setattr(
        lightning.fabric.accelerators.mps,
        "_get_all_available_mps_gpus",
        lambda: [0] if n > 0 else [],
    )
    monkeypatch.setattr(
        lightning.fabric.accelerators.mps.MPSAccelerator,
        "is_available",
        lambda *_: n > 0,
    )
...
def mock_xla_available(monkeypatch: pytest.MonkeyPatch, value: bool = True) -> None:
    ...
    monkeypatch.setitem(
        sys.modules, "torch_xla.distributed.xla_multiprocessing", Mock()
    )
    ...
    monkeypatch.setattr(
        pytorch_lightning_enterprise.utils.imports, "_XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        pytorch_lightning_enterprise.utils.imports, "_XLA_GREATER_EQUAL_2_1", value
    )
    monkeypatch.setattr(
        pytorch_lightning_enterprise.utils.imports, "_XLA_GREATER_EQUAL_2_5", value
    )
    monkeypatch.setattr(lightning.fabric.accelerators.xla, "_XLA_AVAILABLE", value)
    monkeypatch.setattr(
        lightning.fabric.accelerators.xla, "_XLA_GREATER_EQUAL_2_1", value
    )
    monkeypatch.setattr(
        lightning.fabric.accelerators.xla, "_XLA_GREATER_EQUAL_2_5", value
    )
    # Patch in the modules where they're used after import
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.accelerators.xla._XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.accelerators.xla._XLA_GREATER_EQUAL_2_1", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.accelerators.xla._XLA_GREATER_EQUAL_2_5", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.environments.xla._XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.environments.xla._XLA_GREATER_EQUAL_2_1",
        value,
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.plugins.precision.xla._XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.strategies.xla.single._XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.strategies.xla.ddp._XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.strategies.xla.ddp._XLA_GREATER_EQUAL_2_1", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.strategies.xla.fsdp._XLA_AVAILABLE", value
    )
    monkeypatch.setattr(
        "pytorch_lightning_enterprise.strategies.xla.launcher._XLA_AVAILABLE", value
    )
...
def mock_tpu_available(monkeypatch: pytest.MonkeyPatch, value: bool = True) -> None:
    ...
    monkeypatch.setattr(
        lightning.fabric.accelerators.xla.XLAAccelerator, "is_available", lambda: value
    )
    monkeypatch.setattr(
        lightning.fabric.accelerators.xla.XLAAccelerator,
        "auto_device_count",
        lambda *_: 8,
    )
    ...
    monkeypatch.setattr(
        pytorch_lightning_enterprise.accelerators.xla.XLAAccelerator,
        "is_available",
        lambda: value,
    )
    monkeypatch.setattr(
