# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_data.py
...
def test_extract_batch_size():
    ...
    def _check_warning_raised(data, expected):
        with pytest.warns(
            UserWarning,
            match=f"Trying to infer the `batch_size` .* we found is {expected}.",
        ):
    ...
    def _check_error_raised(data):
        with pytest.raises(
            MisconfigurationException, match="We could not infer the batch_size"
        ):
...
def test_custom_torch_batch_sampler(predicting):
    ...
    dataloader = _update_dataloader(
        dataloader,
        dataloader.sampler,
        mode=(RunningStage.PREDICTING if predicting else None),
    )
...
def test_custom_batch_sampler(predicting):
    ...
    if predicting:
        with pytest.warns(
            PossibleUserWarning,
            match=r"Make sure your sampler is configured correctly to return all",
        ):
            _ = _update_dataloader(
                dataloader, sampler=Mock(), mode=RunningStage.PREDICTING
            )
    else:
        with pytest.raises(
            TypeError,
...
def test_custom_batch_sampler_no_drop_last():
    ...
    with pytest.warns(UserWarning, match="drop_last=False"):
        _ = _update_dataloader(
            dataloader, dataloader.sampler, mode=RunningStage.PREDICTING
        )
...
def test_batch_sampler_shuffle_setting():
    ...
    sequential_dataloader = DataLoader(
        range(10),
        batch_sampler=BatchSampler(seq_sampler, batch_size=2, drop_last=False),
    )
...
@pytest.mark.parametrize(
    "mode", [RunningStage.TRAINING, RunningStage.PREDICTING, RunningStage.TESTING]
)
def test_dataloader_kwargs_replacement_with_iterable_dataset(mode):
    ...
    _, dl_kwargs = _get_dataloader_init_args_and_kwargs(
        dataloader, dataloader.sampler, mode=mode
    )
