# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\migration\test_utils.py
...
@pytest.mark.skipif(
    module_available("lightning"),
    reason="This test is ONLY relevant for the STANDALONE package",
)
def test_test_patch_legacy_imports_standalone(pl_version):
    ...
    path_ckpts = sorted(
        glob.glob(os.path.join(path_legacy, f"*{CHECKPOINT_EXTENSION}"))
    )
...
@pytest.mark.parametrize("pl_version", LEGACY_BACK_COMPATIBLE_PL_VERSIONS)
@pytest.mark.skipif(
    not module_available("lightning"),
...
def test_patch_legacy_imports_unified(pl_version):
    ...
    assert not any(
        key.startswith("pytorch_lightning")
        and not key.startswith("pytorch_lightning_enterprise")
...
def test_migrate_checkpoint(monkeypatch):
    ...
    assert (
        ...
        == old_checkpoint
        == {
            "legacy_pytorch-lightning_version": "0.0.0",
    ...
    assert (
        ...
        == old_checkpoint
        == {
            "legacy_pytorch-lightning_version": "1.0.3",
            "pytorch-lightning_version": pl.__version__,
            "content": 123,
        }
    )
    ...
    assert (
        new_checkpoint
        == old_checkpoint
        == {"pytorch-lightning_version": pl.__version__, "content": 123}
    )
...
def test_migrate_checkpoint_for_pl(caplog):
    ...
    with caplog.at_level(
        logging.INFO, logger="lightning.pytorch.utilities.migration.utils"
    ):
        new_checkpoint = _pl_migrate_checkpoint(loaded_checkpoint, "path/to/ckpt")
    assert new_checkpoint == {
    ...
    assert (
        f"Lightning automatically upgraded your loaded checkpoint from v0.0.1 to v{pl.__version__}"
        in caplog.text
    )
...
def test_migrate_checkpoint_legacy_version(monkeypatch):
    ...
    assert (
        new_new_checkpoint["legacy_pytorch-lightning_version"] == "0.0.1"
    )  # remains the same
