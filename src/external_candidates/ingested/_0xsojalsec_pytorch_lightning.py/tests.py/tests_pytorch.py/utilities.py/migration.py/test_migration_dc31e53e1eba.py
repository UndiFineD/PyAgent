...
def test_migrate_loop_global_step_to_progress_tracking():
    ...
    assert (
        updated_checkpoint["loops"]["fit_loop"][
...
def test_migrate_loop_current_epoch_to_progress_tracking():
    ...
    assert (
        updated_checkpoint["loops"]["fit_loop"]["epoch_progress"]["current"][
            "completed"
        ]
        == 2
    )
...
def test_migrate_model_checkpoint_save_on_train_epoch_end_default_collision():
    ...
    with pytest.warns(
        ...
        updated_checkpoint, _ = migrate_checkpoint(
            old_checkpoint.copy(), target_version="1.9.0"
        )
    assert (
        updated_checkpoint["callbacks"] == old_checkpoint["callbacks"]
...
def test_migrate_dropped_apex_amp_state(monkeypatch):
    ...
    )  # pretend this version of Lightning is >= 2.0.0
...
def test_migrate_loop_structure_after_tbptt_removal():
    ...
    updated_checkpoint, _ = migrate_checkpoint(
        old_checkpoint.copy(), target_version="2.0.0"
    )
