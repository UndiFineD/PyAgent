# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_grads.py
...
@pytest.mark.parametrize(
    ...
    [
        (
            1,
            {
                "grad_1.0_norm/param0": 1 + 2 + 3,
                "grad_1.0_norm/param1": 4 + 5,
                "grad_1.0_norm_total": 15.0,
            },
        ),
        (
            2,
        ...
        (
            3.14,
            {
                ...
                "grad_3.14_norm_total": pow(
                    1 + 2**3.14 + 3**3.14 + 4**3.14 + 5**3.14, 1 / 3.14
                ),
            },
        ),
        (
...
def test_grad_norm_invalid_norm_type(norm_type):
    with pytest.raises(
        ValueError, match="`norm_type` must be a positive number or 'inf'"
    ):
