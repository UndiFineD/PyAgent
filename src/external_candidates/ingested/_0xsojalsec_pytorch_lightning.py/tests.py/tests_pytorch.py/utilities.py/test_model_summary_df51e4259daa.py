# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_model_summary.py
        ...
        self.branch1 = nn.Sequential(
            ...
            nn.Sequential(
                ...
                nn.Sequential(
                    ...
                    nn.Sequential(
                        nn.Linear(5, 5),
                        nn.Sequential(nn.Linear(5, 5), nn.Sequential(nn.Linear(5, 3))),
                    ),
                ),
            ),
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"tensor0": torch.zeros(1, 2, 3), "tensor1": torch.zeros(4, 5, 6)},
            [[1, 2, 3], [4, 5, 6]],
        ),
...
def test_model_size_warning_on_unsupported_precision(tmp_path):
    ...
    with pytest.warns(
        UserWarning, match="Precision .* is not supported by the model summary.*"
    ):
        summary = summarize(model)
    assert model.pre_calculated_model_size == summary.model_size
...
def test_lazy_model_summary():
    ...
    with pytest.warns(
        UserWarning, match="The total number of parameters detected may be inaccurate."
    ):
...
@mock.patch(
    "lightning.pytorch.utilities.model_summary.model_summary._is_dtensor",
    return_value=True,
)
...
def test_raise_invalid_max_depth_value(max_depth):
    with pytest.raises(
        ValueError, match=f"`max_depth` can be -1, 0 or > 0, got {max_depth}"
    ):
