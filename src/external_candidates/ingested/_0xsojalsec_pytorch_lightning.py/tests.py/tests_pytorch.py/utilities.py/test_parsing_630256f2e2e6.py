# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_parsing.py
...
def model_and_trainer_cases():
    ...
    class TestModel6(
        LightningModule
    ):  # test for datamodule w/ hparams w/o attribute (should use datamodule)
        hparams = TestHparamsDict
    ...
    class TestModel7(
        LightningModule
    ):  # test for datamodule w/ hparams w/ attribute (should use datamodule)
...
def test_lightning_getattr():
    ...
    *__, model5, model6, model7, model8 = models
    ...
    assert (
        lightning_getattr(model5, "batch_size") == 8
    ), "batch_size not correctly extracted"
    assert (
        lightning_getattr(model6, "batch_size") == 8
    ), "batch_size not correctly extracted"
    assert (
        lightning_getattr(model7, "batch_size") == 8
    ), "batch_size not correctly extracted"
    assert (
        lightning_getattr(model8, "batch_size") == 2
    ), "batch_size not correctly extracted"
...
def test_lightning_setattr():
    ...
    *__, model5, model6, model7, model8 = models
    for m in models[:3]:
        ...
        assert (
            lightning_getattr(m, "learning_rate") == 10
    ...
    assert (
        lightning_getattr(model5, "batch_size") == 128
    ), "batch_size not correctly set"
    assert (
        lightning_getattr(model6, "batch_size") == 128
    ), "batch_size not correctly set"
    assert (
        lightning_getattr(model7, "batch_size") == 128
    ), "batch_size not correctly set"
    assert (
        lightning_getattr(model8, "batch_size") == 128
    ), "batch_size not correctly set"
...
def test_get_init_args():
    ...
    assert my_class.result == (
        my_class,
        {"anyarg": "test", "anykw": 32, "otherkw": 123},
    )
