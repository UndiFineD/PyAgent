# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_auto_restart.py
...
class TestAutoRestartModelUnderSignal(BoringModel):
    def __init__(
        self,
        should_signal: bool,
        failure_on_step: bool,
        failure_on_training: bool,
        on_last_batch: bool,
    ):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        should_signal = (
            self.trainer.fit_loop.epoch_loop._is_training_done
...
def _fit_model(
    ...
    seed_everything(42)
    model = TestAutoRestartModelUnderSignal(
        should_signal, failure_on_step, failure_on_training, on_last_batch
    )
...
def test_auto_restart_under_signal(
    on_last_batch, val_check_interval, failure_on_training, failure_on_step, tmp_path
):
    if failure_on_step:
        if on_last_batch:
            if failure_on_training:
                ...
                status = (
                    "_FitLoop:on_advance_end"
                    if val_check_interval == 1.0
                    else "_TrainingEpochLoop:on_advance_end"
                )
        else:
            status = (
                "_TrainingEpochLoop:on_advance_end"
                if failure_on_training
                else "_EvaluationLoop:_evaluation_step"
            )
    else:
        if val_check_interval == 1.0:
