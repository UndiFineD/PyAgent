# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_deepspeed_collate_checkpoint.py
...
def _assert_checkpoint_equal(model, output_path):
    ...
    for orig_param, saved_model_param in zip(
        state_dict.values(), single_output["state_dict"].values()
    ):
