# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_combined_loader.py
...
@pytest.mark.parametrize(
    "mode", ["min_size", "max_size_cycle", "max_size", "sequential"]
)
...
@pytest.mark.parametrize(
    "iterator_cls", [_Sequential, _MinSize, _MaxSize, _MaxSizeCycle]
)
def test_iterator_mode_limits_raises(iterator_cls):
    with pytest.raises(
        ValueError, match=r"number of limits \(0\) and number of iterables \(2\)"
    ):
        iterator_cls([0, 1], [])
...
def test_combined_loader_flattened_setter():
    ...
    with pytest.raises(
        ValueError,
        match=r"Mismatch in flattened length \(1\) and existing length \(3\)",
    ):
...
def test_combined_data_loader_validation_test(use_distributed_sampler):
    ...
    if use_distributed_sampler:
        ...
        assert all(
            isinstance(s, (SequentialSampler, CustomSampler))
            for s in samplers_flattened
        )
...
@pytest.mark.parametrize(
    "accelerator", ["cpu", pytest.param("gpu", marks=RunIf(min_cuda_gpus=2))]
)
@pytest.mark.parametrize("use_distributed_sampler", [False, True])
def test_combined_data_loader_with_max_size_cycle_and_ddp(
    monkeypatch, accelerator, use_distributed_sampler
):
    """This test makes sure distributed sampler has been properly injected in dataloaders when using CombinedLoader
    ...
    for a_length in [6, 8, 10]:
        ...
        monkeypatch.setattr(
            ...
            "_prepare_dataloader",
            non_shuffle_process_dataloader,
        )
        trainer.fit_loop.setup_data()
        monkeypatch.undo()

        assert (
            len(combined_loader) == length // 2 if use_distributed_sampler else length
        )
        if use_distributed_sampler:
...
def test_combined_dataloader_for_training_with_ddp(
    use_distributed_sampler, mode, mps_count_0
):
...
def test_load_state_dicts():
    ...
    with pytest.raises(
        RuntimeError, match="has 0 stateful loaders, but found 1 states"
    ):
    ...
    cl = CombinedLoader([range(2), stateful1, range(3)])
    with pytest.raises(
        RuntimeError, match="has 1 stateful loaders, but found 2 states"
    ):
