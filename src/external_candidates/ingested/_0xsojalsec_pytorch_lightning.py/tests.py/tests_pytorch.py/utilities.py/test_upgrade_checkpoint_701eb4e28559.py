# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_upgrade_checkpoint.py
...
def test_upgrade_checkpoint_file_missing(tmp_path, caplog):
    ...
    with (
        mock.patch("sys.argv", ["upgrade_checkpoint.py", str(file)]),
    ...
    with (
        mock.patch(
            "sys.argv",
            ["upgrade_checkpoint.py", str(tmp_path), "--extension", ".other"],
        ),
        caplog.at_level(logging.ERROR),
    ):
        with pytest.raises(SystemExit):
