# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\tests\tests_pytorch\utilities\test_model_helpers.py
...
def test_mixed_imports_unified():
    ...
    with pytest.raises(
        TypeError,
        match=r"`pytorch_lightning` object \(BoringModel\) to a `lightning.pytorch`",
    ):
        new_unwrap(model)

    with pytest.raises(
        TypeError,
        match=r"`pytorch_lightning` object \(EarlyStopping\) to a `lightning.pytorch`",
    ):
...
def test_restricted_classmethod():
    ...
    )  # no exception when getting restricted method
...
def test_module_mode_restore_new_module(caplog):
    ...
    with caplog.at_level(
        logging.DEBUG, logger="lightning.pytorch.utilities.model_helpers"
    ):
        mode.restore(model)
