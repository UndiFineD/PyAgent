# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning_fabric\__setup__.py
import glob
import os
...
def _prepare_extras() -> dict[str, Any]:
    ...
    common_args = {
        "path_dir": _PATH_REQUIREMENTS,
        "unfreeze": "none" if _FREEZE_REQUIREMENTS else "all",
    }
...
def _setup_args() -> dict[str, Any]:
    ...
    return {
        ...
        "packages": find_packages(
            where="src", include=["lightning_fabric", "lightning_fabric.*"]
        ),
        "package_dir": {"": "src"},
