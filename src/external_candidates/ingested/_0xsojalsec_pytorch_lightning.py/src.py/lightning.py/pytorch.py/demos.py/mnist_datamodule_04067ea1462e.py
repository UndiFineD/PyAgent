# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\demos\mnist_datamodule.py
...
class _MNIST(Dataset):
    ...
    @staticmethod
    def _try_load(
        path_data: str, trials: int = 30, delta: float = 1.0
    ) -> tuple[Tensor, Tensor]:
    ...
    def normalize_tensor(
        tensor: Tensor, mean: Union[int, float] = 0.0, std: Union[int, float] = 1.0
    ) -> Tensor:
...
class MNISTDataModule(LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        extra = (
            {"transform": self.default_transforms} if self.default_transforms else {}
        )
