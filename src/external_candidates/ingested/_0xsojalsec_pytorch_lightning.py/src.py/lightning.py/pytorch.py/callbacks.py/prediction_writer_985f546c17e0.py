# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\callbacks\prediction_writer.py
...
class BasePredictionWriter(Callback):
    ...
    def __init__(
        self, write_interval: Literal["batch", "epoch", "batch_and_epoch"] = "batch"
    ) -> None:
        if write_interval not in list(WriteInterval):
            raise MisconfigurationException(
                f"`write_interval` should be one of {[i.value for i in WriteInterval]}."
            )
    ...
    def setup(
        ...
        if is_param_in_hook_signature(
            pl_module.predict_step, "dataloader_iter", explicit=True
        ):
