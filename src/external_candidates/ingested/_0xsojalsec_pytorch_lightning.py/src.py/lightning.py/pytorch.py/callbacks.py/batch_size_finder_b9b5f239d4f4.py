# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\callbacks\batch_size_finder.py
...
class BatchSizeFinder(Callback):
    ...
    def __init__(
        ...
        assert (
            0.0 <= margin < 1.0
        ), f"`margin` should be between 0 and 1. Found {margin=}"
    ...
    def setup(
        ...
        if trainer._accelerator_connector.is_distributed:
            raise MisconfigurationException(
                "The Batch size finder is not supported with distributed strategies."
            )
