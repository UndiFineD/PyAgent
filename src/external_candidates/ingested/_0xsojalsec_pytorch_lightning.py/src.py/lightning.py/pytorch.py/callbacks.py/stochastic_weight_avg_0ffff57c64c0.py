# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\callbacks\stochastic_weight_avg.py
...
class StochasticWeightAveraging(Callback):
    ...
    def on_train_epoch_start(
        ...
        if (not self._initialized) and (
            ...
            self._swa_scheduler = cast(
                ...
                SWALR(
                    ...
                    last_epoch=(
                        trainer.max_epochs if self._annealing_strategy == "cos" else -1
                    ),
                ),
            )
            if self._scheduler_state is not None:
        ...
        if (self.swa_start <= trainer.current_epoch <= self.swa_end) and (
            ...
            self.update_parameters(
                self._average_model, pl_module, self.n_averaged, self._avg_fn
            )
        ...
        if trainer.current_epoch == self.swa_end + 1:
            ...
            assert isinstance(
                trainer.fit_loop.max_batches, int
    ...
    def on_train_end(
        ...
        if (
            ...
            and trainer.current_epoch - 1 == self.swa_end + 1
        ):
