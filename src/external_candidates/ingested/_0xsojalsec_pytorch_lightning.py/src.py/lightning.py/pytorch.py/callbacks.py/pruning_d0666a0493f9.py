# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\callbacks\pruning.py
...
class ModelPruning(Callback):
    ...
    def filter_parameters_to_prune(
        self, parameters_to_prune: _PARAM_LIST = ()
    ) -> _PARAM_LIST:
        """This function can be overridden to control which module to prune."""
        return parameters_to_prune

    def _create_pruning_fn(
        self, pruning_fn: str, **kwargs: Any
    ) -> Union[Callable, pytorch_prune.BasePruningMethod]:
    ...
    def _copy_param(new: nn.Module, old: nn.Module, name: str) -> None:
        ...
        dst = (
            getattr(new, name + "_orig")
            if hasattr(new, name + "_orig")
            else getattr(new, name)
        )
        src = getattr(old, name)
        if (
            dst is None
            or src is None
            or not isinstance(dst, Tensor)
            or not isinstance(src, Tensor)
        ):
            return
    ...
    def apply_lottery_ticket_hypothesis(self) -> None:
        ...
        for d in self._original_layers.values():
            ...
            names = d["names"]
            if (
                self._resample_parameters
                and hasattr(copy, "reset_parameters")
                and callable(copy.reset_parameters)
            ):
    ...
    def apply_pruning(self, amount: Union[int, float]) -> None:
        ...
        if self._verbose:
            prev_stats = [
                self._get_pruned_stats(m, n) for m, n in self._parameters_to_prune
            ]
    ...
    @override
    def setup(
        self, trainer: "pl.Trainer", pl_module: LightningModule, stage: str
    ) -> None:
        ...
        if self._use_lottery_ticket_hypothesis:
            ...
            for i, (module, name) in enumerate(self._parameters_to_prune):
                id_ = id(module)
                self._original_layers.setdefault(
                    id_, _LayerRef(data=deepcopy(module), names=[])
                )
    ...
    def _make_pruning_permanent_on_state_dict(
        ...
        map_pruned_params = {
            k.replace("_mask", "") for k in state_dict if k.endswith("_mask")
        }
    ...
    def sanitize_parameters_to_prune(
        ...
        if not parameters_to_prune:
            parameters_to_prune = [
                ...
                and isinstance(getattr(m, p, None), nn.Parameter)
            ]
        elif (
