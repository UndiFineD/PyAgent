...
class ModelCheckpoint(Checkpoint):
    ...
    def on_train_batch_end(
        ...
        if not pl_module.automatic_optimization:
            ...
            if self._every_n_train_steps < 1 or (
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    original_state = {
                        k: v.detach().clone()
                        for k, v in pl_module.layer.state_dict().items()
                    }
                    try:
    ...
    def on_exception(
        ...
        self._save_last_checkpoint(trainer, monitor_candidates)
        rank_zero_info(f"An {type(exception).__name__} was raised with message: \
    ...
    def _link_checkpoint(trainer: "pl.Trainer", filepath: str, linkpath: str) -> None:
        if trainer.is_global_zero and os.path.abspath(filepath) != os.path.abspath(
            linkpath
        ):
    ...
    def _should_save_on_exception(self, trainer: "pl.Trainer") -> bool:
        return (
            ...
            and self._last_global_step_saved
            != trainer.global_step  # already saved at the last step
        )
    ...
    def _find_last_checkpoints(self, trainer: "pl.Trainer") -> set[str]:
        ...
        def _is_last(path: Path) -> bool:
            return path.suffix == self.FILE_EXTENSION and bool(
                re.match(last_pattern, path.stem)
            )
        ...
        if self._enable_version_counter:
            ...
            while self.file_exists(filepath, trainer) and filepath != del_filepath:
                filepath = self.format_checkpoint_name(
                    monitor_candidates, ver=version_cnt
                )
                version_cnt += 1
    ...
    def _save_none_monitor_checkpoint(
        ...
        filepath = self._get_metric_interpolated_filepath_name(
            monitor_candidates, trainer, self.best_model_path
        )
