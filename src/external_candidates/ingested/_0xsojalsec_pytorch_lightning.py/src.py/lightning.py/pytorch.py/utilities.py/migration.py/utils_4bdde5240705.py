# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\utilities\migration\utils.py
...
def migrate_checkpoint(
    ...
    for migration_version, migration_functions in index.items():
        ...
        applied_migrations[migration_version] = [
            fn.__name__ for fn in migration_functions
        ]
...
class pl_legacy_patch:
    ...
    def __enter__(self) -> "pl_legacy_patch":
        ...
        legacy_argparse_module = ModuleType(
            "lightning.pytorch.utilities.argparse_utils"
        )
        sys.modules["lightning.pytorch.utilities.argparse_utils"] = (
            legacy_argparse_module
        )
...
def _pl_migrate_checkpoint(
    checkpoint: _CHECKPOINT, checkpoint_path: Optional[_PATH] = None
) -> _CHECKPOINT:
    ...
    path_hint = (
        os.path.relpath(checkpoint_path, os.getcwd())
        if not _IS_WINDOWS
        else os.path.abspath(checkpoint_path)
    )
    _log.info(
...
def _should_upgrade(
    checkpoint: _CHECKPOINT, target: str, max_version: Optional[str] = None
) -> bool:
...
class _RedirectingUnpickler(pickle._Unpickler):
    ...
    def find_class(self, module: str, name: str) -> Any:
        ...
        if module != new_module:
            warnings.warn(
                f"Redirecting import of {module}.{name} to {new_module}.{name}"
            )
...
def _patch_pl_to_mirror_if_necessary(module: str) -> str:
    _pl = (
        "pytorch_" + "lightning"
    )  # avoids replacement during mirror package generation
