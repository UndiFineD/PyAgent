# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\utilities\data.py
...
def has_len_all_ranks(
    ...
    total_length = strategy.reduce(
        torch.tensor(local_length, device=strategy.root_device), reduce_op="sum"
    )
    if total_length == 0:
        rank_zero_warn(
...
def _get_dataloader_init_args_and_kwargs(
    ...
    if isinstance(dataset, IterableDataset):
        ...
        dl_kwargs.update(
            _dataloader_init_kwargs_resolve_sampler(dataloader, sampler, mode)
        )
...
def _dataloader_init_kwargs_resolve_sampler(
    ...
    if batch_sampler is not None and (
        batch_sampler_cls is not BatchSampler or is_predicting
    ):
...
def _is_dataloader_shuffled(dataloader: object) -> bool:
    if hasattr(dataloader, "__pl_saved_kwargs"):
        ...
        if "shuffle" in dataloader.__pl_saved_arg_names:
            return dataloader.__pl_saved_args[
                dataloader.__pl_saved_arg_names.index("shuffle")
            ]
    if hasattr(dataloader, "dataset") and isinstance(
