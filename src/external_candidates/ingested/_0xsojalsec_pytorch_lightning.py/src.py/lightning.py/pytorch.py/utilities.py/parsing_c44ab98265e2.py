...
def _lightning_get_all_attr_holders(
    model: "pl.LightningModule", attribute: str
) -> list[Any]:
    ...
    if trainer is not None and trainer.datamodule is not None:
        ...
        if (
            hasattr(trainer.datamodule, "hparams")
            and attribute in trainer.datamodule.hparams
        ):
...
def _lightning_get_first_attr_holder(
    model: "pl.LightningModule", attribute: str
) -> Optional[Any]:
    """Special attribute finding for Lightning.
