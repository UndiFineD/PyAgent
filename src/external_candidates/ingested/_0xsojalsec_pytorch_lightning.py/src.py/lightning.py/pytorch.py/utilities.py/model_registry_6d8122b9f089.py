# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\utilities\model_registry.py
...
def _determine_model_name(
    ckpt_path: Optional[_PATH], default_model_registry: Optional[str]
) -> str:
...
def find_model_local_ckpt_path(
    ckpt_path: Optional[_PATH],
    default_model_registry: Optional[str],
...
def download_model_from_registry(
    ckpt_path: Optional[_PATH], trainer: "pl.Trainer"
) -> None:
    ...
    if trainer.local_rank == 0:
        ...
        local_model_dir = _determine_model_folder(
            model_registry, trainer.default_root_dir
        )
