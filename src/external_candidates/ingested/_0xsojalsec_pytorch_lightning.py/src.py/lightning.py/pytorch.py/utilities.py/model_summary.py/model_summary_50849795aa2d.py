# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\utilities\model_summary\model_summary.py
...
class LayerSummary:
    ...
    def _register_hook(self) -> Optional[RemovableHandle]:
        ...
        if not isinstance(self._module, torch.jit.ScriptModule):
            handle = self._module.register_forward_hook(
                hook_with_kwargs, with_kwargs=True
            )
    ...
    def num_parameters(self) -> int:
        ...
        return sum(
            p.numel() if not _tensor_has_shape(p) else 0
...
class ModelSummary:
    ...
    def __init__(self, model: "pl.LightningModule", max_depth: int = 1) -> None:
        ...
        if (
            self._model._trainer
            and self._model.trainer.precision not in precision_to_bits
        ):
            rank_zero_warn(
        ...
        precision = (
            precision_to_bits.get(self._model.trainer.precision, 32)
            if self._model._trainer
            else 32
        )
    ...
    def _forward_example_input(self) -> None:
        ...
        flop_context = (
            ...
            if (
                ...
                and any(
                    isinstance(m, torch.jit.ScriptModule) for m in self._model.modules()
                )
            )
        ...
        forward_context = (
            contextlib.nullcontext()
            if trainer is None
            else trainer.precision_plugin.forward_context()
        )
            ...
            (
                ...
                list(
                    map(
                        ...
                        (sum(x.values()) for x in self.flop_counts.values()),
                    )
                ),
            ),
        ]
    ...
    def _add_leftover_params_to_summary(
        ...
        layer_summaries["Params"].append(
            get_human_readable_count(total_leftover_params)
        )
...
def summarize(
    lightning_module: "pl.LightningModule", max_depth: int = 1
) -> ModelSummary:
