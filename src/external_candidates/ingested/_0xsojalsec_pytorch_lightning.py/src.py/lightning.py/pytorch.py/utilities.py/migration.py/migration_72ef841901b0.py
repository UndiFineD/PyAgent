# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\utilities\migration\migration.py
...
def _migration_index() -> dict[str, list[Callable[[_CHECKPOINT], _CHECKPOINT]]]:
    ...
    return {
        ...
        "1.6.0": [
            _migrate_loop_global_step_to_progress_tracking,
            _migrate_loop_current_epoch_to_progress_tracking,
        ],
...
def _migrate_model_checkpoint_early_stopping(checkpoint: _CHECKPOINT) -> _CHECKPOINT:
    ...
    for key, new_path in keys_mapping.items():
        if key in checkpoint:
            ...
            checkpoint["callbacks"][callback_type] = (
                checkpoint["callbacks"].get(callback_type) or {}
            )
...
def _migrate_loop_global_step_to_progress_tracking(
    ...
    optim_progress = checkpoint["loops"]["fit_loop"][
...
def _migrate_loop_current_epoch_to_progress_tracking(
    checkpoint: _CHECKPOINT,
) -> _CHECKPOINT:
...
def _migrate_loop_batches_that_stepped(checkpoint: _CHECKPOINT) -> _CHECKPOINT:
    ...
    """
    global_step = checkpoint["global_step"]
    checkpoint["loops"]["fit_loop"]["epoch_loop.state_dict"].setdefault(
        "_batches_that_stepped", global_step
    )
    return checkpoint
...
def _migrate_model_checkpoint_save_on_train_epoch_end_default(
    checkpoint: _CHECKPOINT,
) -> _CHECKPOINT:
    ...
    new_callback_states = {
        new_key(old_key): state
        for old_key, state in checkpoint["callbacks"].items()
...
def _drop_apex_amp_state(checkpoint: _CHECKPOINT) -> _CHECKPOINT:
    ...
    if key in checkpoint:
        rank_zero_warn(
            "This checkpoint contains apex AMP data, but apex support has been removed in v2.0.0."
        )
        del checkpoint[key]
    return checkpoint
...
def _migrate_loop_structure_after_tbptt_removal(checkpoint: _CHECKPOINT) -> _CHECKPOINT:
    ...
    if (
        "epoch_loop.batch_loop.state_dict" in fit_loop
        and fit_loop["epoch_loop.batch_loop.state_dict"]
    ):
        fit_loop["epoch_loop.state_dict"]["old_batch_loop_state_dict"] = fit_loop[
            "epoch_loop.batch_loop.state_dict"
        ]
...
def _migrate_loop_structure_after_optimizer_loop_removal(
    checkpoint: _CHECKPOINT,
) -> _CHECKPOINT:
    ...
    if "epoch_loop.optimizer_loop.optim_progress" in fit_loop:
        fit_loop["epoch_loop.optimizer_loop.optim_progress"].pop(
            "optimizer_position", None
        )

    # the subloop attribute names have changed
    if "epoch_loop.optimizer_loop.state_dict" in fit_loop:
        fit_loop["epoch_loop.automatic_optimization.state_dict"] = fit_loop.pop(
    ...
    if "epoch_loop.manual_loop.state_dict" in fit_loop:
        fit_loop["epoch_loop.manual_optimization.state_dict"] = fit_loop.pop(
...
def _migrate_loop_structure_after_dataloader_loop_removal(
    checkpoint: _CHECKPOINT,
) -> _CHECKPOINT:
    ...
    for loop_key in ("predict_loop", "validate_loop", "test_loop"):
        ...
        epoch_loop_dict = {
            k[len(epoch_loop_key) :]: loop.pop(k)
            for k in list(loop)
            if k.startswith(epoch_loop_key)
        }
