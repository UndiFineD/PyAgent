...
class DeepSpeedLayerSummary(LayerSummary):
    ...
    def average_shard_parameters(self) -> int:
        ...
        def partitioned_size(p: Parameter) -> int:
            return (
                p.partitioned_size()
                if RequirementCache("deepspeed<0.6.6")
                else p.partition_numel()
            )

        return sum(
            partitioned_size(p) if not _tensor_has_shape(p) else 0
            for p in self._module.parameters()
        )
...
class DeepSpeedSummary(ModelSummary):
    ...
    def summarize(self) -> dict[str, DeepSpeedLayerSummary]:  # type: ignore[override]
        summary = OrderedDict(
            (name, DeepSpeedLayerSummary(module)) for name, module in self.named_modules
        )
    ...
    def parameters_per_layer(self) -> list[int]:
        return [
            layer.average_shard_parameters for layer in self._layer_summary.values()
        ]
