# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\utilities\upgrade_checkpoint.py
...
def _upgrade(args: Namespace) -> None:
    ...
    extension: str = (
        args.extension if args.extension.startswith(".") else f".{args.extension}"
    )
    files: list[Path] = []
    ...
    if path.is_dir():
        files = [
            Path(p)
            for p in glob.glob(str(path / "**" / f"*{extension}"), recursive=True)
        ]
    if not files:
        _log.error(
    ...
    _log.info(
        "Creating a backup of the existing checkpoint files before overwriting in the upgrade process."
    )
    for file in files:
    ...
    for file in tqdm(files):
        with pl_legacy_patch():
            checkpoint = torch.load(
                file, map_location=(torch.device("cpu") if args.map_to_cpu else None)
            )
