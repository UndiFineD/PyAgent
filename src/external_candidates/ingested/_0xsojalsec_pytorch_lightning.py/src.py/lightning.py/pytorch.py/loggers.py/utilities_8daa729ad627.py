# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\loggers\utilities.py
...
def _scan_checkpoints(
    ...
    checkpoints = [
        ...
        if c[1] not in logged_model_time or logged_model_time[c[1]] < c[0]
    ]
    return checkpoints
...
def _log_hyperparams(trainer: "pl.Trainer") -> None:
    ...
    datamodule_log_hyperparams = (
        trainer.datamodule._log_hyperparams if trainer.datamodule is not None else False
    )
    ...
    if hparams_initial is not None:
        hparams_initial = {
            k: v for k, v in hparams_initial.items() if k != "_class_path"
        }
