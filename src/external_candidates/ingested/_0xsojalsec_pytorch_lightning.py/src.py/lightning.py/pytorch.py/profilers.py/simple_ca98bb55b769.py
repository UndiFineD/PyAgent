# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\profilers\simple.py
        ...
        if self.extended:
            if len(self.recorded_durations) > 0:
                ...
                def log_row_extended(
                    action: str, mean: str, num_calls: str, total: str, per: str
                ) -> str:
                ...
                report_extended, total_calls, total_duration = (
                    self._make_report_extended()
                )
            ...
            for action, mean_duration, total_duration in report:
                output_string += log_row(
                    action, f"{mean_duration:.5}", f"{total_duration:.5}"
                )
            output_string += sep_lines
        output_string += sep
