# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\call.py
...
def _call_and_handle_interrupt(
    trainer: "pl.Trainer", trainer_fn: Callable, *args: Any, **kwargs: Any
) -> Any:
    ...
    try:
        if trainer.strategy.launcher is not None:
            return trainer.strategy.launcher.launch(
                trainer_fn, *args, trainer=trainer, **kwargs
            )
...
def _call_setup_hook(trainer: "pl.Trainer") -> None:
    ...
    loggers = sorted(
        trainer.loggers, key=lambda logger: not isinstance(logger, WandbLogger)
    )
...
def _call_lightning_module_hook(
    ...
    **kwargs: Any,
    ...
    log.debug(
        f"{trainer.__class__.__name__}: calling lightning module hook: {hook_name}"
    )
    ...
    with trainer.profiler.profile(
        f"[LightningModule]{pl_module.__class__.__name__}.{hook_name}"
    ):
...
def _call_lightning_datamodule_hook(
    ...
    **kwargs: Any,
    ...
    log.debug(
        f"{trainer.__class__.__name__}: calling lightning datamodule hook: {hook_name}"
    )
    ...
    if callable(fn):
        with trainer.profiler.profile(
            f"[LightningDataModule]{trainer.datamodule.__class__.__name__}.{hook_name}"
        ):
...
def _call_callback_hooks(
    ...
    **kwargs: Any,
    ...
    for callback in callbacks:
        ...
        if callable(fn):
            with trainer.profiler.profile(
                f"[Callback]{callback.state_key}.{hook_name}"
            ):
...
def _call_callbacks_on_save_checkpoint(
    trainer: "pl.Trainer", checkpoint: dict[str, Any]
) -> None:
    ...
    for callback in trainer.callbacks:
        with trainer.profiler.profile(
            f"[Callback]{callback.state_key}.on_save_checkpoint"
        ):
...
def _call_callbacks_on_load_checkpoint(
    trainer: "pl.Trainer", checkpoint: dict[str, Any]
) -> None:
    ...
    callback_states: Optional[dict[Union[type, str], dict]] = checkpoint.get(
        "callbacks"
    )
    ...
    is_legacy_ckpt = Version(checkpoint["pytorch-lightning_version"]) < Version(
        "1.5.0dev"
    )
    ...
    for callback in trainer.callbacks:
        with trainer.profiler.profile(
            f"[Callback]{callback.state_key}.on_load_checkpoint"
        ):
...
def _call_callbacks_load_state_dict(
    trainer: "pl.Trainer", checkpoint: dict[str, Any]
) -> None:
...
def _call_strategy_hook(
    ...
    **kwargs: Any,
    ...
    with trainer.profiler.profile(
        f"[Strategy]{trainer.strategy.__class__.__name__}.{hook_name}"
    ):
        output = fn(*args, **kwargs)
