# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\connectors\checkpoint_connector.py
...
class _CheckpointConnector:
    ...
    def _parse_ckpt_path(
        ...
        if (
            ...
            and self._hpc_resume_path is not None
        ):
            ckpt_path = "hpc"
        ...
        ft_checkpoints = [
            cb for cb in self.trainer.callbacks if isinstance(cb, OnExceptionCheckpoint)
        ]
        fn = state_fn.value
        if (
            ckpt_path is None
    ...
    def restore_datamodule(self) -> None:
        ...
        if (
            ...
            and datamodule.__class__.__qualname__ in self._loaded_checkpoint
        ):
            call._call_lightning_datamodule_hook(
                trainer,
                "load_state_dict",
                self._loaded_checkpoint[datamodule.__class__.__qualname__],
            )
    ...
    def restore_precision_plugin_state(self) -> None:
        ...
        if prec_plugin.__class__.__qualname__ in self._loaded_checkpoint:
            prec_plugin.load_state_dict(
                self._loaded_checkpoint[prec_plugin.__class__.__qualname__]
            )

        # old checkpoints compatibility
        if "native_amp_scaling_state" in self._loaded_checkpoint and isinstance(
            prec_plugin, MixedPrecision
        ):
            prec_plugin.load_state_dict(
                self._loaded_checkpoint["native_amp_scaling_state"]
            )
    ...
    def dump_checkpoint(self, weights_only: Optional[bool] = None) -> dict:
        ...
        if datamodule is not None:
            datamodule_state_dict = call._call_lightning_datamodule_hook(
                trainer, "state_dict"
            )
