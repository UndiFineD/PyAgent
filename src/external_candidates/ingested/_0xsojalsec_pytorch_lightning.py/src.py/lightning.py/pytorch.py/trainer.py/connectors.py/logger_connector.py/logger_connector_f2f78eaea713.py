# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\connectors\logger_connector\logger_connector.py
...
class _LoggerConnector:
        ...
        if not logger:
            ...
            if _TENSORBOARD_AVAILABLE or _TENSORBOARDX_AVAILABLE:
                logger_ = TensorBoardLogger(
                    ...
                    version=SLURMEnvironment.job_id(),
                )
            else:
                warning_cache.warn(
    ...
    def update_train_step_metrics(self) -> None:
        if (
            self.trainer.fit_loop._should_accumulate()
