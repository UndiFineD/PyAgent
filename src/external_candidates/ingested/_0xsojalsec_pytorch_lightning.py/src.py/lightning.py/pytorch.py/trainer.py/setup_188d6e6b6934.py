# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\setup.py
...
def _init_debugging_flags(
    ...
    trainer.overfit_batches = _determine_batch_limits(
    ...
    if fast_dev_run:
        ...
        if not overfit_batches_enabled:
            trainer.limit_train_batches = _determine_batch_limits(
                limit_train_batches, "limit_train_batches"
            )
            trainer.limit_val_batches = _determine_batch_limits(
                limit_val_batches, "limit_val_batches"
            )
        trainer.limit_test_batches = _determine_batch_limits(
            limit_test_batches, "limit_test_batches"
        )
        trainer.limit_predict_batches = _determine_batch_limits(
            limit_predict_batches, "limit_predict_batches"
        )
        trainer.num_sanity_val_steps = (
            float("inf") if num_sanity_val_steps == -1 else num_sanity_val_steps
        )
        ...
        if isinstance(val_check_interval, (str, dict, timedelta)):
            trainer._val_check_time_interval = _parse_time_interval_seconds(
                val_check_interval
            )
        else:
            trainer.val_check_interval = _determine_batch_limits(
...
def _determine_batch_limits(
    batches: Optional[Union[int, float]], name: str
) -> Union[int, float]:
    if batches is None:
...
def _init_profiler(
    trainer: "pl.Trainer", profiler: Optional[Union[Profiler, str]]
) -> None:
...
def _parse_time_interval_seconds(value: Union[str, timedelta, dict]) -> float:
    ...
    raise MisconfigurationException(
        f"Unsupported type for `val_check_interval`: {type(value)!r}"
    )
