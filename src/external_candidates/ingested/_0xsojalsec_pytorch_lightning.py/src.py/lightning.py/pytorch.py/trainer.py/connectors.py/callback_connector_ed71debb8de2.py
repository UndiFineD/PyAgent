...
class _CallbackConnector:
    ...
    def on_trainer_init(
        ...
        self.trainer.callbacks.extend(
            _load_external_callbacks("lightning.pytorch.callbacks_factory")
        )
    ...
    def _configure_checkpoint_callbacks(self, enable_checkpointing: bool) -> None:
        if self.trainer.checkpoint_callbacks:
            ...
            if RequirementCache("litmodels >=0.1.7") and self.trainer._model_registry:
                ...
                if trainer_source is None or not isinstance(
                    trainer_source.__package__, str
                ):
                ...
                model_checkpoint = LitModelCheckpoint(
                    model_registry=self.trainer._model_registry
                )
            else:
                rank_zero_info(
    ...
    def _configure_model_summary_callback(self, enable_model_summary: bool) -> None:
        ...
        model_summary_cbs = [
            type(cb) for cb in self.trainer.callbacks if isinstance(cb, ModelSummary)
        ]
    ...
    def _configure_timer_callback(
        self, max_time: Optional[Union[str, timedelta, dict[str, int]]] = None
    ) -> None:
        if max_time is None:
            return
    ...
    def _attach_model_callbacks(self) -> None:
        ...
        model_callbacks = call._call_lightning_module_hook(
            trainer, "configure_callbacks"
        )
        ...
        model_callbacks = (
            [model_callbacks]
            if not isinstance(model_callbacks, Sequence)
