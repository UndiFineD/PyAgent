# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\trainer.py
...
class Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        val_check_interval: Optional[
            Union[int, float, str, timedelta, dict[str, int]]
        ] = None,
        ...
        super().__init__()
        log.debug(
            f"{self.__class__.__name__}: Initializing trainer with parameters: {locals()}"
        )
        ...
        self.fit_loop.epoch_loop = _TrainingEpochLoop(
            self, min_steps=min_steps, max_steps=max_steps
        )
        ...
        self.test_loop = _EvaluationLoop(
            self, TrainerFn.TESTING, RunningStage.TESTING, inference_mode=inference_mode
        )
        ...
        if (
            gradient_clip_algorithm is not None
            and not GradClipAlgorithmType.supported_type(
    ...
    def fit(
        ...
        train_dataloaders: Optional[
            Union[TRAIN_DATALOADERS, LightningDataModule]
        ] = None,
    ...
    def _validate_impl(
        ...
        # links data to the trainer
        self._data_connector.attach_data(
            model, val_dataloaders=dataloaders, datamodule=datamodule
        )
    ...
    def _test_impl(
        ...
        # links data to the trainer
        self._data_connector.attach_data(
            model, test_dataloaders=dataloaders, datamodule=datamodule
        )
    ...
    def _predict_impl(
        ...
        # links data to the trainer
        self._data_connector.attach_data(
            model, predict_dataloaders=dataloaders, datamodule=datamodule
        )
    ...
    def val_dataloaders(self) -> Optional[EVAL_DATALOADERS]:
        ...
        if (
            combined_loader := self.fit_loop.epoch_loop.val_loop._combined_loader
        ) is not None or (
    ...
    def _active_loop(
        self,
    ) -> Optional[Union[_FitLoop, _EvaluationLoop, _PredictionLoop]]:
        if self.training:
    ...
    def estimated_stepping_batches(self) -> Union[int, float]:
        ...
        max_estimated_steps = math.ceil(
            total_batches / self.accumulate_grad_batches
        ) * max(self.max_epochs, 1)

        max_estimated_steps = (
            min(max_estimated_steps, self.max_steps)
            if self.max_steps != -1
