...
class _DataConnector:
    ...
    def on_trainer_init(
        ...
        if check_val_every_n_epoch is not None and not isinstance(
        ...
        if not isinstance(reload_dataloaders_every_n_epochs, int) or (
        ...
        self.trainer.reload_dataloaders_every_n_epochs = (
    ...
    def prepare_data(self) -> None:
        ...
        if datamodule is not None and is_overridden("prepare_data", datamodule):
            ...
            with _InfiniteBarrier():
                if (prepare_data_per_node and local_rank_zero) or (
                    not prepare_data_per_node and global_rank_zero
                ):
                    call._call_lightning_datamodule_hook(trainer, "prepare_data")

        # handle lightning module prepare data:
        if lightning_module is not None and is_overridden(
            "prepare_data", lightning_module
        ):
    ...
    def attach_dataloaders(
        ...
        trainer.fit_loop._data_source.instance = (
            train_dataloaders if train_dataloaders is not None else model
        )
        trainer.fit_loop.epoch_loop.val_loop._data_source.instance = (
            val_dataloaders if val_dataloaders is not None else model
        )
        trainer.validate_loop._data_source.instance = (
            val_dataloaders if val_dataloaders is not None else model
        )
        trainer.test_loop._data_source.instance = (
            test_dataloaders if test_dataloaders is not None else model
        )
        trainer.predict_loop._data_source.instance = (
            predict_dataloaders if predict_dataloaders is not None else model
        )
    ...
    def _prepare_dataloader(
        self, dataloader: object, shuffle: bool, mode: RunningStage
    ) -> object:
        ...
        if not isinstance(dataloader, DataLoader):
            return dataloader
        if (
            self._requires_distributed_sampler(
                dataloader
            )  # sets the distributed sampler
...
def _resolve_overfit_batches(
    combined_loader: CombinedLoader, mode: RunningStage
) -> None:
    ...
    all_have_sequential_sampler = all(
        isinstance(dl.sampler, SequentialSampler)
    ...
    updated = [
        (
            _update_dataloader(dl, sampler=SequentialSampler(dl.dataset), mode=mode)
...
class _DataLoaderSource:
    ...
    def dataloader(self) -> Union[TRAIN_DATALOADERS, EVAL_DATALOADERS]:
        ...
        if isinstance(self.instance, pl.LightningModule):
            return call._call_lightning_module_hook(
                self.instance.trainer, self.name, pl_module=self.instance
            )
        if isinstance(self.instance, pl.LightningDataModule):
            ...
            return call._call_lightning_datamodule_hook(
                self.instance.trainer, self.name
            )
...
def _worker_check(trainer: "pl.Trainer", dataloader: object, name: str) -> None:
    ...
    if (
        ...
        and not dataloader.persistent_workers
    ):
        rank_zero_warn(
    ...
    if (
        ...
        and dataloader.pin_memory
        and trainer.reload_dataloaders_every_n_epochs > 0
    ):
        rank_zero_warn(
...
def _parse_num_batches(
    ...
    if (
        num_batches == 0
        and limit_batches > 0.0
        and isinstance(limit_batches, float)
        and length != float("inf")
    ):
...
def _process_dataloader(
    ...
    dataloader = trainer._data_connector._prepare_dataloader(
        dataloader, shuffle=is_shuffled, mode=stage
    )
    ...
    ):  # if we are fitting, we need to do this in the loop
        # some users want validation shuffling based on the training progress
        _set_sampler_epoch(
            dataloader, trainer.fit_loop.epoch_progress.current.processed
        )
