# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\configuration_validator.py
...
def _verify_loop_configurations(trainer: "pl.Trainer") -> None:
    ...
    if trainer.state.fn is None:
        raise ValueError(
            "Unexpected: Trainer state fn must be set before validating loop configuration."
        )
...
def __verify_train_val_loop_configuration(
...
def __verify_manual_optimization_support(
    trainer: "pl.Trainer", model: "pl.LightningModule"
) -> None:
