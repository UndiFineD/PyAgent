# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\trainer\connectors\logger_connector\result.py
...
_VALUE = Union[
    Metric, Tensor
]  # Do not include scalars as they were converted to tensors
...
class _Sync:
    ...
    def _generate_sync_fn(self) -> None:
        ...
        fn = (
            self.no_op
            if self.fn is None or not self.should or self.rank_zero_only
            else self.fn
        )
...
class _Metadata:
    ...
    def is_custom_reduction(self) -> bool:
        return not (
            ...
            or self.is_max_reduction
            or self.is_min_reduction
            or self.is_sum_reduction
        )
...
class _ResultMetric(Metric):
    ...
    def __init__(self, metadata: _Metadata, is_tensor: bool) -> None:
        ...
        if is_tensor:
            ...
            self.add_state(
                "value",
                torch.tensor(default, dtype=_get_default_dtype()),
                dist_reduce_fx=torch.sum,
            )
            if self.meta.is_mean_reduction:
                self.cumulated_batch_size: Tensor
                self.add_state(
                    "cumulated_batch_size", torch.tensor(0), dist_reduce_fx=torch.sum
                )
    ...
    def update(self, value: _VALUE, batch_size: int) -> None:
        if self.is_tensor:
            ...
            if self.meta.on_step:
                self._forward_cache = self.meta.sync(
                    value.clone()
                )  # `clone` because `sync` is in-place
    ...
    def to(self, *args: Any, **kwargs: Any) -> "_ResultMetric":
        ...
        self.__dict__.update(
            apply_to_collection(
                d, (Tensor, Metric), move_data_to_device, *args, **kwargs
            )
        )
        return self
...
class _ResultCollection(dict):
    ...
    def _extract_batch_size(
        self, value: _ResultMetric, batch_size: Optional[int], meta: _Metadata
    ) -> int:
    ...
    def valid_items(self) -> Generator:
        ...
        return (
            ...
            if not v.has_reset and self.dataloader_idx == v.meta.dataloader_idx
        )

    def _forked_name(
        self, result_metric: _ResultMetric, on_step: bool
    ) -> tuple[str, str]:
    ...
    def to(self, *args: Any, **kwargs: Any) -> "_ResultCollection":
        ...
        self.update(
            apply_to_collection(
                dict(self), (Tensor, Metric), move_data_to_device, *args, **kwargs
            )
        )
        return self
