# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\_graveyard\precision.py
import sys
...
def _patch_classes() -> None:
    ...
    for module_name, deprecated_name, new_class in classes_map:
        ...
        setattr(
            getattr(pl.plugins.precision, module_name),
            deprecated_name,
            deprecated_class,
        )
    ...
    setattr(
        pl.plugins.precision.fsdp, "FSDPMixedPrecisionPlugin", FSDPMixedPrecisionPlugin
    )
