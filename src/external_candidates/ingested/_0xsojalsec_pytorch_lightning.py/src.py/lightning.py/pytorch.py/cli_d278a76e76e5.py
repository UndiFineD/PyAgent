# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\cli.py
...
class ReduceLROnPlateau(torch.optim.lr_scheduler.ReduceLROnPlateau):
    ...
    def __init__(
        self, optimizer: Optimizer, monitor: str, *args: Any, **kwargs: Any
    ) -> None:
    ...
    def add_optimizer_args(
        ...
        if isinstance(optimizer_class, tuple):
            ...
            self.add_class_arguments(
                optimizer_class, nested_key, sub_configs=True, **kwargs
            )
        self._optimizers[nested_key] = (optimizer_class, link_to)

    def add_lr_scheduler_args(
        self,
        lr_scheduler_class: Union[
            LRSchedulerType, tuple[LRSchedulerType, ...]
        ...
        if isinstance(lr_scheduler_class, tuple):
            ...
            self.add_class_arguments(
                lr_scheduler_class, nested_key, sub_configs=True, **kwargs
            )
...
class LightningCLI:
    ...
    def _setup_parser_kwargs(
        ...
        main_kwargs = {
            k: v for k, v in parser_kwargs.items() if k not in subcommand_names
        }
        subparser_kwargs = {
            k: v for k, v in parser_kwargs.items() if k in subcommand_names
        }
    ...
    def _prepare_subcommand_parser(
        self, klass: type, subcommand: str, **kwargs: Any
    ...
    def _dump_config(self) -> None:
        ...
        self.config_dump = yaml.safe_load(
            self.parser.dump(
                self.config, skip_link_targets=False, skip_none=False, format="yaml"
            )
        )
    ...
    def _instantiate_trainer(
        self, config: dict[str, Any], callbacks: list[Callback]
    ) -> Trainer:
        key = "callbacks"
        if key in config:
    ...
    def _add_configure_optimizers_method_to_model(
        ...
        fn = partial(
            self.configure_optimizers, optimizer=optimizer, lr_scheduler=lr_scheduler
        )
...
def _global_add_class_path(
    ...
    return {
        "class_path": _class_path_from_class(class_type),
...
def _add_class_path_generator(
    class_type: type,
) -> Callable[[Namespace], dict[str, Any]]:
...
def instantiate_module(
    class_type: type[ModuleType], config: dict[str, Any]
) -> ModuleType:
