# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\serve\servable_module_validator.py
import contextlib
import logging
import time
...
class ServableModuleValidator(Callback):
    ...
    def __init__(
        ...
        if optimization is not None:
            raise NotImplementedError(
                f"The optimization {optimization} is currently not supported."
            )
    ...
    @rank_zero_only
    def on_train_start(
        self, trainer: "pl.Trainer", servable_module: "pl.LightningModule"
    ) -> None:
        ...
        if not isinstance(servable_module, ServableModule):
            raise TypeError(
                f"The provided model should be subclass of {ServableModule.__qualname__}."
            )
        ...
        if not is_overridden(
            "configure_serialization", servable_module, ServableModule
        ):
            raise NotImplementedError(
        ...
        process = Process(
            ...
            args=(servable_module, self.host, self.port, self.optimization),
        )
        process.start()
        ...
        while not ready:
            ...
            if time.time() - t0 > self.timeout:
                process.kill()
                raise Exception(
                    f"The server didn't start within {self.timeout} seconds."
                )
            time.sleep(0.1)
        ...
        if "body" not in payload:
            raise Exception(
                f'Your provided payload {payload} should have a field named "body".'
            )
        ...
        if is_overridden("configure_response", servable_module, ServableModule):
            ...
            if self.resp.json() != response:
                raise Exception(
                    f"The expected response {response} doesn't match the generated one {self.resp.json()}."
                )
        ...
        if self.successful:
            _logger.info(
                f"Your model is servable and the received payload was {self.resp.json()}."
            )
    ...
    @staticmethod
    def _start_server(
        servable_model: ServableModule, host: str, port: int, _: bool
    ) -> None:
        ...
        async def serve(payload: dict = Body(...)) -> dict[str, Any]:
            ...
            if not isinstance(output, dict):
                raise Exception(
                    f"Please, return your outputs as a dictionary. Found {output}"
                )
