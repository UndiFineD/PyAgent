# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\plugins\io\async_plugin.py
...
class AsyncCheckpointIO(_WrappingCheckpointIO):
    ...
    def save_checkpoint(self, *args: Any, **kwargs: Any) -> None:
        ...
        if "checkpoint" in kwargs:
            kwargs = {
                **kwargs,
                "checkpoint": apply_to_collection(
                    kwargs["checkpoint"], torch.Tensor, _clone_tensor
                ),
            }
        elif len(args) >= 1:
            args = (
                apply_to_collection(args[0], torch.Tensor, _clone_tensor),
                *args[1:],
            )
