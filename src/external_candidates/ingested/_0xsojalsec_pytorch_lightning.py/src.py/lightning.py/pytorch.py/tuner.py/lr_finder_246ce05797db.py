# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\tuner\lr_finder.py
...
class _LRFinder:
    ...
    def __init__(
        self, mode: str, lr_min: float, lr_max: float, num_training: int
    ) -> None:
        assert mode in (
            "linear",
            "exponential",
    ...
    def suggestion(self, skip_begin: int = 10, skip_end: int = 1) -> Optional[float]:
        ...
        gradients = torch.gradient(losses, spacing=[lrs])[
            0
        ]  # Compute the gradient of losses w.r.t. learning rates
...
def _lr_find(
    ...
    try:
        ...
        lr_finder.results.update(
            {"lr": trainer.callbacks[0].lrs, "loss": trainer.callbacks[0].losses}
        )
        lr_finder._total_batch_idx = (
            trainer.fit_loop.total_batch_idx
        )  # for debug purpose
        ...
        trainer.fit_loop.restarting = (
            False  # reset restarting flag as checkpoint restoring sets it to True
        )
        trainer.fit_loop.epoch_loop.restarting = (
            False  # reset restarting flag as checkpoint restoring sets it to True
        )
        trainer.fit_loop.epoch_loop.val_loop._combined_loader = None
        trainer.fit_loop._combined_loader = (
            None  # reset data fetcher to avoid issues with the next fit
        )
...
def __lr_finder_reset_params(
    trainer: "pl.Trainer", num_training: int, early_stop_threshold: Optional[float]
) -> None:
    ...
    trainer.callbacks = [
        _LRCallback(num_training, early_stop_threshold, progress_bar_refresh_rate=1)
    ]
    # No logging
    ...
    def on_train_batch_start(
        ...
        if self.progress_bar_refresh_rate and self.progress_bar is None:
            self.progress_bar = tqdm(
                desc="Finding best initial lr", total=self.num_training
            )
