# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\tuner\batch_size_scaling.py
    ...
    try:
        ...
        if mode == "power":
            new_size = _run_power_scaling(
                trainer, new_size, batch_arg_name, max_trials, params, max_val
            )
        elif mode == "binsearch":
            new_size = _run_binsearch_scaling(
                trainer, new_size, batch_arg_name, max_trials, params, margin, max_val
            )
...
def _run_power_scaling(
    ...
    for i in range(max_trials):
        ...
        if new_size >= max_val:
            rank_zero_info(
                f"Reached the maximum batch size limit of {max_val}. Stopping search."
            )
            break
...
def _adjust_batch_size(
    ...
    if new_size > max_val:
        if desc:
            rank_zero_info(
                f"Batch size {new_size} exceeds max_val limit {max_val}, capping at {max_val}"
            )
        new_size = max_val
    if desc:
