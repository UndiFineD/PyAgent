...
class Tuner:
    ...
    def scale_batch_size(
        ...
        """
        _check_tuner_configuration(
            train_dataloaders, val_dataloaders, dataloaders, method
        )
        ...
        self._trainer.callbacks = [
            cb for cb in self._trainer.callbacks if cb is not batch_size_finder
        ]
    ...
    def lr_find(
        ...
        self._trainer.callbacks = [
            cb for cb in self._trainer.callbacks if cb is not lr_finder_callback
        ]
...
def _check_tuner_configuration(
    ...
    if method not in supported_methods:
        raise ValueError(
            f"method {method!r} is invalid. Should be one of {supported_methods}."
        )
...
def _check_lr_find_configuration(trainer: "pl.Trainer") -> None:
    ...
    configured_callbacks = [
        cb for cb in trainer.callbacks if isinstance(cb, LearningRateFinder)
    ]
...
def _check_scale_batch_size_configuration(trainer: "pl.Trainer") -> None:
    ...
    configured_callbacks = [
        cb for cb in trainer.callbacks if isinstance(cb, BatchSizeFinder)
    ]
    if configured_callbacks:
        raise ValueError(
