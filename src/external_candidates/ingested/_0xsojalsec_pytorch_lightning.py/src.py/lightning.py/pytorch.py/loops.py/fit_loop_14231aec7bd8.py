...
class _FitLoop(_Loop):
    ...
    def restarting(self, restarting: bool) -> None:
        ...
        epoch_unfinished = any(
            v != self.epoch_progress.current.processed for v in values
        )
    ...
    def _can_stop_early(self) -> bool:
        met_min_epochs = (
            self.epoch_progress.current.processed >= self.min_epochs
            if self.min_epochs
            else True
        )
        met_min_steps = (
            self.epoch_loop.global_step >= self.min_steps if self.min_steps else True
        )
    ...
    def _should_reload_train_dl(self) -> bool:
        ...
        return (
            n_epochs
            and self.trainer.current_epoch - self._last_train_dl_reload_epoch
            >= n_epochs
        )
    ...
    def done(self) -> bool:
        ...
        if stop_steps:
            rank_zero_info(
                f"`Trainer.fit` stopped: `max_steps={self.max_steps!r}` reached."
            )
            return True
        ...
        if stop_epochs:
            ...
            rank_zero_info(
                f"`Trainer.fit` stopped: `max_epochs={self.max_epochs!r}` reached."
            )
            return True
    ...
    def setup_data(self) -> None:
        ...
        if trainer.limit_train_batches == 0 or not is_overridden(
            "training_step", pl_module
        ):
            return
        ...
        has_len_all_ranks_ = has_len_all_ranks(
            combined_loader, trainer.strategy, allow_zero_length
        )
        ...
        if getattr(trainer, "_val_check_time_interval", None) is not None:
            ...
            if not has_len_all_ranks_:
                ...
                trainer.val_check_batch = int(
                    self.max_batches * trainer.val_check_interval
                )
                trainer.val_check_batch = max(1, trainer.val_check_batch)

        if (
            trainer.loggers
            and self.max_batches < trainer.log_every_n_steps
            and not trainer.fast_dev_run
        ):
            rank_zero_warn(
    ...
    def update_restart_stage(self) -> None:
        if (
            ...
            and self.epoch_progress.total.completed
            == self.epoch_progress.total.processed
        ):
            ...
            and self.epoch_progress.total.processed
            == self.epoch_progress.total.started - 1
            and self.epoch_progress.total.completed
            == self.epoch_progress.total.processed
        ):
            ...
            and self.epoch_progress.total.processed == self.epoch_progress.total.started
            and self.epoch_progress.total.completed
            == self.epoch_progress.total.processed - 1
        ):
    ...
    def on_run_start(self) -> None:
        ...
        if (
            self.epoch_loop._should_check_val_epoch()
            and trainer.val_dataloaders is None
        ):
    ...
    def on_advance_end(self) -> None:
        ...
        call._call_callback_hooks(
            trainer, "on_train_epoch_end", monitoring_callbacks=False
        )
        call._call_lightning_module_hook(trainer, "on_train_epoch_end")
        call._call_callback_hooks(
            trainer, "on_train_epoch_end", monitoring_callbacks=True
        )
        ...
        if not self.restarting and self.epoch_loop._num_ready_batches_reached():
            ...
            self.epoch_loop.update_lr_schedulers(
                "epoch", update_plateau_schedulers=not self.restarting
            )
    ...
    def on_save_checkpoint(self) -> dict:
        ...
        if self._combined_loader is not None and (
            loader_states := self._combined_loader._state_dicts()
        ):
    ...
    def _load_combined_loader_states(self) -> None:
        if (
            not self.restarting
            or not self._combined_loader_states_to_load
            or self._combined_loader is None
        ):
            return
