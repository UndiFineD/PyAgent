# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\loops\training_epoch_loop.py
...
_BATCH_OUTPUTS_TYPE = Optional[
    Union[_OPTIMIZER_LOOP_OUTPUTS_TYPE, _MANUAL_LOOP_OUTPUTS_TYPE]
]
...
class _TrainingEpochLoop(loops._Loop):
    ...
    def update_restart_stage(self) -> None:
        if (
            ...
            and self.batch_progress.total.completed
            == self.batch_progress.total.processed - 1
        ):
            ...
            and self.batch_progress.total.processed == self.batch_progress.total.started
            and self.batch_progress.total.completed
            == self.batch_progress.total.processed
        ):
    ...
    def reset(self) -> None:
        ...
        if self.restarted_on_train_batch_end:
            ...
            if (
                self.batch_progress.current.completed
        ...
        if self.restarting:
            ...
            if trainer.num_training_batches != float("inf"):
                ...
                is_resumable_loader = all(
                    isinstance(loader, _Stateful) for loader in loader.flattened
                )
    ...
    def on_run_start(self, data_fetcher: _DataFetcher) -> None:
        ...
        if (
            ...
            and not self.trainer.fit_loop.is_resuming
    ...
    def _on_before_fetch(self) -> None:
        self.trainer.profiler.start(
            f"[{self.__class__.__name__}].train_dataloader_next"
        )
    ...
    def advance(self, data_fetcher: _DataFetcher) -> None:
        ...
        if not using_dataloader_iter:
            ...
            batch = trainer.lightning_module._on_before_batch_transfer(
                batch, dataloader_idx=0
            )
            batch = call._call_strategy_hook(
                trainer, "batch_to_device", batch, dataloader_idx=0
            )
        ...
        if batch is None and not using_dataloader_iter:
            ...
            response = call._call_lightning_module_hook(
                trainer, "on_train_batch_start", batch, batch_idx
            )
            ...
            if should_skip_rest_of_epoch:
                ...
                with trainer.profiler.profile("run_training_batch"):
                    if trainer.lightning_module.automatic_optimization:
                        ...
                        batch_output = self.automatic_optimization.run(
                            trainer.optimizers[0], batch_idx, kwargs
                        )
                    else:
        ...
        call._call_callback_hooks(
            trainer, "on_train_batch_end", batch_output, batch, batch_idx
        )
    ...
    def on_advance_end(self, data_fetcher: _DataFetcher) -> None:
        ...
        if should_check_val:
            ...
            if not self._should_accumulate():
                ...
                call._call_lightning_module_hook(
                    self.trainer, "on_validation_model_zero_grad"
                )
    ...
    def _accumulated_batches_reached(self) -> bool:
        ...
        return (
            self.batch_progress.current.ready % self.trainer.accumulate_grad_batches
            == 0
        )

    def _num_ready_batches_reached(self) -> bool:
        """Checks if we are in the last batch or if there are more batches to follow."""
        epoch_finished_on_ready = (
            self.batch_progress.current.ready == self.trainer.num_training_batches
        )
    ...
    def _should_accumulate(self) -> bool:
        ...
        strategy_accumulates_on_final_batch = (
            self.trainer.strategy.handles_gradient_accumulation or not is_final_batch
        )
        return not accumulation_done and strategy_accumulates_on_final_batch

    def update_lr_schedulers(
        self, interval: str, update_plateau_schedulers: bool
    ) -> None:
        """Updates the lr schedulers based on the given interval."""
        if interval == "step" and self._should_accumulate():
            return
        self._update_learning_rates(
            interval=interval, update_plateau_schedulers=update_plateau_schedulers
        )
    ...
    def _update_learning_rates(
        ...
        if (
            not trainer.lr_scheduler_configs
            or not trainer.lightning_module.automatic_optimization
        ):
            return
        ...
        for config in trainer.lr_scheduler_configs:
            ...
            current_idx = (
                self.batch_idx if interval == "step" else trainer.current_epoch
            )
    ...
    def _should_check_val_epoch(self) -> bool:
        return self.trainer.enable_validation and (
            ...
            or (self.trainer.current_epoch + 1) % self.trainer.check_val_every_n_epoch
            == 0
        )
    ...
    def _build_kwargs(
        self, kwargs: OrderedDict, batch: Any, batch_idx: int
    ) -> OrderedDict:
