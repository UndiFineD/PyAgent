# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\loops\prediction_loop.py
...
class _PredictionLoop(_Loop):
    ...
    def return_predictions(self, return_predictions: Optional[bool] = None) -> None:
        ...
        return_supported = not isinstance(
            self.trainer.strategy.launcher, _MultiProcessingLauncher
        )
        ...
        self._return_predictions = (
            return_supported if return_predictions is None else return_predictions
        )
    ...
    def setup_data(self) -> None:
        ...
        allow_zero_length = (
            trainer.lightning_module.allow_zero_length_dataloader_with_multiple_devices
        )
        if trainer.datamodule is not None:
            allow_zero_length |= (
                trainer.datamodule.allow_zero_length_dataloader_with_multiple_devices
            )
        ...
        for dl in combined_loader.flattened:
            ...
            length = (
                len(dl)
                if has_len_all_ranks(dl, trainer.strategy, allow_zero_length)
                else float("inf")
            )
            num_batches = _parse_num_batches(
                stage, length, trainer.limit_predict_batches
            )
    ...
    def _predict_step(
        ...
        batch_idx: int,
        dataloader_idx: int,
        dataloader_iter: Optional[Iterator],
    ) -> None:
        ...
        if not (
            using_dataloader_iter := isinstance(
            ...
            batch = call._call_strategy_hook(
                trainer, "batch_to_device", batch, dataloader_idx=dataloader_idx
            )

        self.batch_progress.increment_ready()

        any_on_epoch = (
            self._store_data_for_prediction_writer(batch_idx, dataloader_idx)
        ...
        hook_kwargs = self._build_kwargs(
            batch, batch_idx, dataloader_idx if self.num_dataloaders > 1 else None
        )

        call._call_callback_hooks(
            trainer, "on_predict_batch_start", *hook_kwargs.values()
        )
        ...
        call._call_callback_hooks(
            trainer, "on_predict_batch_end", predictions, *hook_kwargs.values()
        )
        call._call_lightning_module_hook(
            trainer, "on_predict_batch_end", predictions, *hook_kwargs.values()
        )
        ...
        if self._return_predictions or any_on_epoch:
            self._predictions[dataloader_idx].append(
    ...
    def _build_step_args_from_hook_kwargs(
        self, hook_kwargs: OrderedDict, step_hook_name: str
    ) -> tuple:
    ...
    ) -> list[list[int]]:  # batches x samples
    ...
    def _store_data_for_prediction_writer(
        self, batch_idx: int, dataloader_idx: int
    ) -> bool:
        prediction_writers = [
            cb for cb in self.trainer.callbacks if isinstance(cb, BasePredictionWriter)
        ]
