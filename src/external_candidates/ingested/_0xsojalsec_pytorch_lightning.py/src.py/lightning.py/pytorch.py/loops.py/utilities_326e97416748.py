# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\loops\utilities.py
...
def _parse_loop_limits(
    ...
    if max_epochs is None:
        if max_steps == -1 and not any(
            isinstance(cb, Timer) for cb in trainer.callbacks
        ):
            rank_zero_warn(
...
@contextmanager
def _block_parallel_sync_behavior(
    strategy: Strategy, block: bool = True
...
def _no_grad_context(loop_run: Callable) -> Callable:
    def _decorator(self: _Loop, *args: Any, **kwargs: Any) -> Any:
        ...
        if not hasattr(self, "inference_mode"):
            raise TypeError(
                f"`{type(self).__name__}.inference_mode` needs to be defined"
            )
    ...
    for hook in hooks:
        ...
        if not is_expected:
            if param_present:
                ...
                if (
                    "dataloader_idx" in params
                    and params["dataloader_idx"].default is inspect.Parameter.empty
                ):
                    raise RuntimeError(
