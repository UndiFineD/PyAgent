...
class _EvaluationLoop(_Loop):
    ...
    def __init__(
        ...
        self._stage = stage
        self._data_source = _DataLoaderSource(
            None, f"{stage.dataloader_prefix}_dataloader"
        )
    ...
    def max_batches(self) -> list[Union[int, float]]:
        ...
        if not self.trainer.sanity_checking:
            return max_batches
        return [
            min(self.trainer.num_sanity_val_steps, batches) for batches in max_batches
        ]
    ...
    def _should_reload_val_dl(self) -> bool:
        ...
        return bool(
            n_epochs
            and self.trainer.current_epoch - self._last_val_dl_reload_epoch >= n_epochs
        )
    ...
    def setup_data(self) -> None:
        ...
        if (
            ...
            and trainer_fn == TrainerFn.FITTING
            and not self._should_reload_val_dl
        ):
            return

        pl_module = trainer.lightning_module
        limit_batches = (
            trainer.limit_test_batches if trainer.testing else trainer.limit_val_batches
        )
        ...
        if trainer_fn == TrainerFn.FITTING and (
            (
                ...
                and trainer.fit_loop.epoch_loop._should_check_val_epoch()
            )
    ...
    def update_restart_stage(self) -> None:
        if (
            ...
            and self.batch_progress.total.processed
            == self.batch_progress.total.started - 1
    ...
    def reset(self) -> None:
        ...
        if fn == TrainerFn.FITTING:
            for i, dl in enumerate(combined_loader.flattened):
                ...
                _set_sampler_epoch(
                    dl, trainer.fit_loop.epoch_progress.current.processed
                )
    ...
    def on_run_end(self) -> list[_OUT_DICT]:
        ...
        epoch_end_logged_outputs = (
            self.trainer._logger_connector.update_eval_epoch_metrics()
        )
    ...
    def _on_evaluation_model_eval(self) -> None:
        ...
        trainer = self.trainer
        hook_name = (
            "on_test_model_eval" if trainer.testing else "on_validation_model_eval"
        )
    ...
    def _on_evaluation_model_train(self) -> None:
        ...
        trainer = self.trainer
        hook_name = (
            "on_test_model_train" if trainer.testing else "on_validation_model_train"
        )
    ...
    def _on_evaluation_epoch_start(self, *args: Any, **kwargs: Any) -> None:
        ...
        hook_name = (
            "on_test_epoch_start" if trainer.testing else "on_validation_epoch_start"
        )
        call._call_callback_hooks(trainer, hook_name, *args, **kwargs)
        call._call_lightning_module_hook(trainer, hook_name, *args, **kwargs)
    ...
    def _on_evaluation_epoch_end(self) -> None:
        ...
        hook_name = (
            "on_test_epoch_end" if trainer.testing else "on_validation_epoch_end"
        )
    ...
    def _store_dataloader_outputs(self) -> None:
        ...
        self._logged_outputs.append(
            trainer._logger_connector.update_eval_epoch_metrics()
        )

    def _on_before_fetch(self) -> None:
        self.trainer.profiler.start(
            f"[{type(self).__name__}].{self._stage.dataloader_prefix}_next"
        )
    ...
    def _evaluation_step(
        ...
        hook_kwargs = self._build_kwargs(
            ...
            (
                dataloader_idx
                if self._is_sequential and self.num_dataloaders > 1
                else None
            ),
        )
        ...
        hook_name = (
            "on_test_batch_start" if trainer.testing else "on_validation_batch_start"
        )
        ...
        hook_name = (
            "on_test_batch_end" if trainer.testing else "on_validation_batch_end"
        )
        call._call_callback_hooks(trainer, hook_name, output, *hook_kwargs.values())
        call._call_lightning_module_hook(
            trainer, hook_name, output, *hook_kwargs.values()
        )
        ...
        if not trainer.sanity_checking:
            ...
            # log batch metrics
            trainer._logger_connector.update_eval_step_metrics(
                self._seen_batches_per_dataloader[dataloader_idx]
            )
    ...
    def _print_results(results: list[_OUT_DICT], stage: str) -> None:
        ...
        metrics_paths = {
            k
            for keys in apply_to_collection(results, dict, _EvaluationLoop._get_keys)
            for k in keys
        }
        ...
        for i in range(0, len(headers), num_cols):
            ...
            if _RICH_AVAILABLE:
                ...
                columns = [
                    Column(h, justify="center", style="magenta", width=max_length)
