# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\loops\optimization\automatic.py
...
class _AutomaticOptimization(_Loop):
    ...
    def run(
        self, optimizer: Optimizer, batch_idx: int, kwargs: OrderedDict
    ) -> _OUTPUTS_TYPE:
    ...
    def _make_closure(
        self, kwargs: OrderedDict, optimizer: Optimizer, batch_idx: int
    ) -> Closure:
        ...
        return Closure(
            step_fn=step_fn, backward_fn=backward_fn, zero_grad_fn=zero_grad_fn
        )
    ...
    def _make_zero_grad_fn(
        self, batch_idx: int, optimizer: Optimizer
    ...
    def _optimizer_zero_grad(
        self, batch_idx: int, optimizer: torch.optim.Optimizer
    ) -> None:
        ...
        trainer = self.trainer
        call._call_lightning_module_hook(
            trainer, "optimizer_zero_grad", trainer.current_epoch, batch_idx, optimizer
        )
    ...
    def _training_step(self, kwargs: OrderedDict) -> ClosureResult:
        ...
        training_step_output = call._call_strategy_hook(
            trainer, "training_step", *kwargs.values()
        )
