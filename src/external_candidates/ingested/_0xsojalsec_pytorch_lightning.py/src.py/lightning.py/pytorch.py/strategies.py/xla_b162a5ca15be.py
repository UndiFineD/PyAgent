# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\strategies\xla.py
...
class XLAStrategy(DDPStrategy):
    ...
    def __init__(
        ...
        **_: Any,
        ...
        from pytorch_lightning_enterprise.strategies.xla.ddp import (
            XLAStrategyTrainer as EnterpriseXLAStrategy,
        )
    ...
    def checkpoint_io(self, io: Optional[CheckpointIO]) -> None:
        if io is not None and not isinstance(
            io, (XLACheckpointIO, _WrappingCheckpointIO)
        ):
            raise TypeError(
    ...
    def precision_plugin(self, precision_plugin: Optional[Precision]) -> None:
        if precision_plugin is not None and not isinstance(
    ...
    def reduce(
        ...
        return self.xla_strategy_impl.reduce(
    ...
    def all_gather(
        ...
        """
        return self.xla_strategy_impl.all_gather(
    ...
    def register_strategies(cls, strategy_registry: _StrategyRegistry) -> None:
        strategy_registry.register(
            "xla_debug",
            cls,
            description="XLA strategy with `debug` as True",
            debug=True,
        )
        strategy_registry.register(
            cls.strategy_name,
