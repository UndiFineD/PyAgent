# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\strategies\strategy.py
...
class Strategy(ABC):
    ...
    def optimizers(self, optimizers: list[Optimizer]) -> None:
        ...
        self._lightning_optimizers = [
            LightningOptimizer._to_lightning_optimizer(opt, self) for opt in optimizers
        ]
    ...
    def backward(
        ...
        **kwargs: Any,
        ...
        assert self.lightning_module is not None
        closure_loss = self.precision_plugin.pre_backward(
            closure_loss, self.lightning_module
        )

        self.precision_plugin.backward(
            closure_loss, self.lightning_module, optimizer, *args, **kwargs
        )

        closure_loss = self.precision_plugin.post_backward(
            closure_loss, self.lightning_module
        )
    ...
    def batch_to_device(
        ...
        if model is not None:
            return model._apply_batch_transfer_handler(
                batch, device=device, dataloader_idx=dataloader_idx
            )
    ...
    def load_checkpoint(
        ...
        return self.checkpoint_io.load_checkpoint(
            checkpoint_path, weights_only=weights_only
        )
        ...
        with self.precision_plugin.train_step_context():
            if self.model != self.lightning_module:
                return self._forward_redirection(
                    self.model, self.lightning_module, "training_step", *args, **kwargs
                )
    ...
    def test_step(self, *args: Any, **kwargs: Any) -> STEP_OUTPUT:
        ...
        with self.precision_plugin.test_step_context():
            if self.model != self.lightning_module:
                return self._forward_redirection(
                    self.model, self.lightning_module, "test_step", *args, **kwargs
                )
    ...
    def predict_step(self, *args: Any, **kwargs: Any) -> Any:
        ...
        with self.precision_plugin.predict_step_context():
            if self.model != self.lightning_module:
                return self._forward_redirection(
                    self.model, self.lightning_module, "predict_step", *args, **kwargs
                )
    ...
    def tensor_init_context(
        ...
        with (
            empty_init_context,
            self.root_device,
            self.precision_plugin.tensor_init_context(),
        ):
            yield
