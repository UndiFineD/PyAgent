...
class FSDPStrategy(ParallelStrategy):
    ...
    def setup_optimizers(self, trainer: "pl.Trainer") -> None:
        ...
        if invalid_params_error or any(
            not _optimizer_has_flat_params(optimizer) for optimizer in self.optimizers
        ):
    ...
    def load_checkpoint(
        ...
        if _is_full_checkpoint(path):
            ...
            if (
                ...
                or self.lightning_module.trainer.state.fn != TrainerFn.FITTING
            ):
            ...
            with _get_full_state_dict_context(
                self.model, world_size=self.world_size, rank0_only=False
            ):
