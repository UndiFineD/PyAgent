# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\strategies\ddp.py
...
class DDPStrategy(ParallelStrategy):
    ...
    def is_distributed(self) -> bool:  # pragma: no-cover
        ...
        rank_zero_deprecation(
            f"`{type(self).__name__}.is_distributed` is deprecated. Use is discouraged.",
            stacklevel=6,
        )
        return True
    ...
    def _configure_launcher(self) -> None:
        ...
        if self._start_method == "popen":
            ...
            self._launcher = _MultiProcessingLauncher(
    ...
    def _enable_model_averaging(self) -> None:
        ...
        for optimizer in self.optimizers:
            ...
            if (
                isinstance(optimizer, (ZeroRedundancyOptimizer, PostLocalSGDOptimizer))
                or is_distributed_optimizer
            ):
                raise ValueError(
        ...
        self._model_averager = torch.distributed.algorithms.model_averaging.averagers.PeriodicModelAverager(
            period=self._model_averaging_period,
            warmup_steps=self._ddp_comm_state.start_localSGD_iter,
        )
    ...
    def teardown(self) -> None:
        ...
        if isinstance(self.model, DistributedDataParallel):
            if not self.model.static_graph and self.model._get_ddp_logging_data().get(
...
class _DDPForwardRedirection(_ForwardRedirection):
    ...
    def on_after_inner_forward(
        self, wrapper_module: Module, original_module: "pl.LightningModule"
    ) -> None:
        # In manual_optimization, we need to prevent DDP reducer as
        # it is done manually in `LightningModule.manual_backward`
        if (
            isinstance(wrapper_module, DistributedDataParallel)
            and not original_module.automatic_optimization
        ):
