...
class _MultiProcessingLauncher(_Launcher):
    ...
    def _wrapping_function(
        ...
        if self._start_method == "spawn" and isinstance(
            self._strategy.accelerator, CPUAccelerator
        ):
    ...
    def _recover_results_in_main_process(
        self, worker_output: "_WorkerOutput", trainer: "pl.Trainer"
    ) -> None:
    ...
    def get_extra_results(self, trainer: "pl.Trainer") -> dict[str, Any]:
        ...
        callback_metrics = apply_to_collection(
            trainer.callback_metrics, Tensor, lambda t: t.cpu()
        )
        buffer = io.BytesIO()
    ...
    def update_main_process_results(
        self, trainer: "pl.Trainer", extra: dict[str, Any]
    ) -> None:
        ...
        callback_metrics = torch.load(
            io.BytesIO(callback_metrics_bytes), weights_only=True
        )
    ...
    def kill(self, signum: _SIGNUM) -> None:
        for proc in self.procs:
            if proc.is_alive() and proc.pid is not None:
                log.debug(
                    f"Process {os.getpid()} is terminating {proc.pid} with {signum}"
                )
