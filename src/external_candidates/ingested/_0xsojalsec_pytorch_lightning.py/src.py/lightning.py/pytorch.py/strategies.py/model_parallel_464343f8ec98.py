# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\strategies\model_parallel.py
...
class ModelParallelStrategy(ParallelStrategy):
    ...
    def setup(self, trainer: "pl.Trainer") -> None:
        ...
        if any(
            isinstance(mod, FullyShardedDataParallel) for mod in self.model.modules()
        ):
            raise TypeError(
    ...
    def lightning_module_state_dict(self) -> dict[str, Any]:
        ...
        state_dict_options = StateDictOptions(
            full_state_dict=(not self._save_distributed_checkpoint), cpu_offload=True
        )
        assert self.model is not None
    ...
    def optimizer_state(self, optimizer: Optimizer) -> dict[str, Any]:
        ...
        state_dict = get_optimizer_state_dict(
            self.model, optimizer, options=state_dict_options
        )
    ...
    def save_checkpoint(
        ...
        if (
            path.is_dir()
            and not self._save_distributed_checkpoint
