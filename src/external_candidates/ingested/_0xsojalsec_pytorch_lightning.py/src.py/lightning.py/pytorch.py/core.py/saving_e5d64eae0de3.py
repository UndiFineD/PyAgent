...
def _default_map_location(
    storage: "UntypedStorage", location: str
) -> Optional["UntypedStorage"]:
    if (
...
def _load_state(
    ...
    **cls_kwargs_new: Any,
    ...
    if isinstance(obj, pl.LightningModule):
        if (
            obj._strict_loading is not None
            and strict is not None
            and strict != obj.strict_loading
        ):
            raise ValueError(
...
def load_hparams_from_yaml(
    config_yaml: _PATH, use_omegaconf: bool = True
) -> dict[str, Any]:
    """Load hparams from a file.
...
def save_hparams_to_yaml(
    config_yaml: _PATH, hparams: Union[dict, Namespace], use_omegaconf: bool = True
) -> None:
    """
    Args:
