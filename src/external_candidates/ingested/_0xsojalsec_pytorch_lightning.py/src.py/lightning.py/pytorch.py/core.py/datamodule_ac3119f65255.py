# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\core\datamodule.py
        ...
        def format_loader_info(
            ...
            for loader_name, loader_info in info.items():
                ...
                if isinstance(loader_info, dataset_info):
                    loader_info_formatted = (
                        "None"
                        if not loader_info.available
                        else f"size={loader_info.length}"
                    )
                # Iterable of datasets
                    ...
                    loader_info_formatted = " ; ".join(
                        (
                            "None"
                            if not loader_info_i.available
                            else f"{i}. size={loader_info_i.length}"
                        )
