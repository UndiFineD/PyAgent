# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\core\optimizer.py
...
class LightningOptimizer:
    ...
    def _to_lightning_optimizer(
        ...
        lightning_optimizer = (
            optimizer if isinstance(optimizer, LightningOptimizer) else cls(optimizer)
        )
...
def _init_optimizers_and_lr_schedulers(
    ...
    optim_conf = call._call_lightning_module_hook(
        model.trainer, "configure_optimizers", pl_module=model
    )
...
def _configure_optimizers(
    ...
    # single dictionary
        ...
        lr_schedulers = (
            [optim_conf["lr_scheduler"]] if "lr_scheduler" in optim_conf else []
        )
    # multiple dictionaries
        ...
        scheduler_dict = lambda scheduler: (
            dict(scheduler) if isinstance(scheduler, dict) else {"scheduler": scheduler}
        )
        lr_schedulers = [
            scheduler_dict(opt_dict["lr_scheduler"])
            for opt_dict in optim_conf
            if "lr_scheduler" in opt_dict
        ]
    # single list or tuple, multiple optimizer
    elif isinstance(optim_conf, (list, tuple)) and all(
        isinstance(opt, Optimizable) for opt in optim_conf
    ):
...
def _configure_schedulers_automatic_opt(
    schedulers: list, monitor: Optional[str]
    ...
    for scheduler in schedulers:
        if isinstance(scheduler, dict):
            ...
            if "interval" in scheduler and scheduler["interval"] not in (
                "step",
                "epoch",
            ):
                raise MisconfigurationException(
            ...
            config = LRSchedulerConfig(
                scheduler, reduce_on_plateau=True, monitor=monitor
            )
        else:
            config = LRSchedulerConfig(scheduler)
...
def _configure_schedulers_manual_opt(schedulers: list) -> list[LRSchedulerConfig]:
    ...
    for scheduler in schedulers:
        if isinstance(scheduler, dict):
            ...
            config = LRSchedulerConfig(
                **{key: scheduler[key] for key in scheduler if key not in invalid_keys}
            )
        else:
            config = LRSchedulerConfig(scheduler)
        lr_scheduler_configs.append(config)
...
def _validate_scheduler_api(
    lr_scheduler_configs: list[LRSchedulerConfig], model: "pl.LightningModule"
) -> None:
...
def _validate_multiple_optimizers_support(
    optimizers: list[Optimizer], model: "pl.LightningModule"
) -> None:
...
def _validate_optimizers_attached(
    optimizers: list[Optimizer], lr_scheduler_configs: list[LRSchedulerConfig]
) -> None:
    for config in lr_scheduler_configs:
