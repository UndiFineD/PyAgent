# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\pytorch\core\module.py
    ...
    if _TORCH_GREATER_EQUAL_2_5:
        if _TORCH_GREATER_EQUAL_2_6:
            ...
            from torch.onnx._internal.exporter import (
                ONNXProgram,  # type: ignore[no-redef]
            )
...
class LightningModule(
    ...
    @overload
    def optimizers(
        self, use_pl_optimizer: Literal[False]
    ...
    def lr_schedulers(self) -> Union[None, list[LRSchedulerPLType], LRSchedulerPLType]:
        ...
        lr_schedulers: list[LRSchedulerPLType] = [
            config.scheduler for config in self.trainer.lr_scheduler_configs
        ]
    ...
    def trainer(self) -> "pl.Trainer":
        ...
        if not self._jit_is_scripting and self._trainer is None:
            raise RuntimeError(
                f"{self.__class__.__qualname__} is not attached to a `Trainer`."
            )
    ...
    def example_input_array(
        self, example: Optional[Union[Tensor, tuple, dict]]
    ) -> None:
    ...
    def log(
        ...
        apply_to_collection(
            ...
            name,
            value,
            wrong_dtype=(numbers.Number, Metric, Tensor),
        )
        ...
        value = apply_to_collection(
            value, (Tensor, numbers.Number), self.__to_tensor, name
        )
        ...
        if (
            trainer.training
            and is_param_in_hook_signature(
                self.training_step, "dataloader_iter", explicit=True
            )
    ...
    def _log_dict_through_fabric(
        ...
        if any(isinstance(v, dict) for v in dictionary.values()):
            raise ValueError(
                f"`self.log_dict({dictionary})` was called, but nested dictionaries cannot be logged"
            )
        for name, value in dictionary.items():
            apply_to_collection(
                ...
                name,
                value,
                wrong_dtype=(numbers.Number, Tensor),
            )
    ...
    def __check_not_nested(value: dict, name: str) -> None:
        ...
        if any(isinstance(v, dict) for v in value.values()):
            raise ValueError(
                f"`self.log({name}, {value})` was called, but nested dictionaries cannot be logged"
            )

    @staticmethod
    def __check_allowed(v: Any, name: str, value: Any) -> None:
        raise ValueError(
            f"`self.log({name}, {value})` was called, but `{type(v).__name__}` values cannot be logged"
        )
    ...
    def all_gather(
        ...
        return apply_to_collection(
            data, Tensor, all_gather, group=group, sync_grads=sync_grads
        )
    ...
    def untoggle_optimizer(
        self, optimizer: Union[Optimizer, LightningOptimizer]
    ) -> None:
        ...
        for opt in self.trainer.optimizers:
            if not (
                opt is optimizer
                or (
                    isinstance(optimizer, LightningOptimizer)
                    and opt is optimizer.optimizer
                )
            ):
                for group in opt.param_groups:
                    for param in group["params"]:
        ...
        **kwargs: Any,
        ...
        if not _ONNX_AVAILABLE:
            raise ModuleNotFoundError(
                f"`{type(self).__name__}.to_onnx()` requires `onnx` to be installed."
            )

        if kwargs.get("dynamo", False) and not (
            _ONNXSCRIPT_AVAILABLE and _TORCH_GREATER_EQUAL_2_5
        ):
            raise ModuleNotFoundError(
        ...
        **kwargs: Any,
        ...
        if method == "script":
            ...
            if kwargs.get("check_inputs") is not None:
                kwargs["check_inputs"] = self._on_before_batch_transfer(
                    kwargs["check_inputs"]
                )
                kwargs["check_inputs"] = self._apply_batch_transfer_handler(
                    kwargs["check_inputs"]
                )
            ...
            with _jit_is_scripting():
                torchscript_module = torch.jit.trace(
                    func=self.eval(), example_inputs=example_inputs, **kwargs
                )
        else:
            raise ValueError(
    ...
    def to_tensorrt(
        ...
        **compile_kwargs: Any,
        ...
        input_sample = copy.deepcopy(
            (input_sample,) if isinstance(input_sample, torch.Tensor) else input_sample
        )
        input_sample = self._on_before_batch_transfer(input_sample)
        input_sample = self._apply_batch_transfer_handler(input_sample)
        ...
        if file_path is not None:
            if ir == "ts":
                ...
                assert isinstance(
                    trt_obj, (torch.jit.ScriptModule, torch.jit.ScriptFunction)
