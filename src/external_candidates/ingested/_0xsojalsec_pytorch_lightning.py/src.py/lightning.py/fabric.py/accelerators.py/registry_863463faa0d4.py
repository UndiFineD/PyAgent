# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\accelerators\registry.py
...
class _AcceleratorRegistry(dict):
        ...
        **init_params: Any,
        ...
        if name in self and not override:
            raise MisconfigurationException(
                f"'{name}' is already present in the registry. HINT: Use `override=True`."
            )
    ...
    def __str__(self) -> str:
        return "Registered Accelerators: {}".format(
            ", ".join(self.available_accelerators())
        )


def call_register_accelerators(
    registry: _AcceleratorRegistry, base_module: str
