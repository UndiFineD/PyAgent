# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\connector.py
...
class _Connector:
    ...
    def __init__(
        ...
        accelerator = self._argument_from_env(
    ...
    def _check_config_and_set_final_flags(
        ...
        is_parallel_strategy = (
            isinstance(strategy, ParallelStrategy)
            or is_ddp_str
            or is_dp_str
            or is_deepspeed_str
        )
        ...
        if plugins:
            ...
            if (
                plugins_flags_types.get(Precision.__name__)
                and precision_input is not None
            ):
                raise ValueError(
        ...
        self._precision_input = (
            "32-true" if precision_input is None else precision_input
        )
    ...
    def _check_device_config_and_set_final_flags(
        self, devices: Union[list[int], str, int], num_nodes: int
    ) -> None:
        if not isinstance(num_nodes, int) or num_nodes < 1:
            raise ValueError(
                f"`num_nodes` must be a positive integer, but got {num_nodes}."
            )
    ...
    def _check_and_init_precision(self) -> Precision:
        if isinstance(self._precision_instance, Precision):
            if isinstance(
                self._precision_instance, BitsandbytesPrecision
            ) and not isinstance(self.accelerator, CUDAAccelerator):
        ...
        if (
            isinstance(self.strategy, ModelParallelStrategy)
            and self._precision_input not in mp_precision_supported
        ):
            raise ValueError(
        ...
        if self._precision_input in ("16-mixed", "bf16-mixed"):
            ...
            device = (
                ...
                if self._accelerator_flag in ("cpu", "mps")
                else "cuda"
            )
    ...
    def _argument_from_env(name: str, current: Any, default: Any) -> Any:
        ...
        if (
            ...
            and env_value != str(current)
            and str(current) != str(default)
            and _is_using_cli()
        ):
            raise ValueError(
...
def _convert_precision_to_unified_args(
    precision: Optional[_PRECISION_INPUT],
) -> Optional[_PRECISION_INPUT_STR]:
    if precision is None:
        return None

    supported_precision = (
        get_args(_PRECISION_INPUT_STR)
        + get_args(_PRECISION_INPUT_INT)
        + get_args(_PRECISION_INPUT_STR_ALIAS)
    )
    ...
    precision = str(
        precision
    )  # convert int flags to str here to enable the legacy-conversion below
