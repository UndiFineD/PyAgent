# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\wrappers.py
...
_LIGHTNING_MODULE_STEP_METHODS = (
    "training_step",
    "validation_step",
    "test_step",
    "predict_step",
...
class _FabricOptimizer:
    def __init__(
        ...
        self.__class__ = type(
            "Fabric" + optimizer.__class__.__name__,
    ...
    def step(self, closure: Optional[Callable] = None) -> Any:
        ...
        if hasattr(self._strategy, "model") and isinstance(
            self._strategy.model, Optimizable
        ):
...
class _FabricModule(_DeviceDtypeModuleMixin):
    ...
    def load_state_dict(  # type: ignore[override]
        ...
        return self._original_module.load_state_dict(
            state_dict=state_dict, strict=strict, **kwargs
        )
    ...
    def mark_forward_method(self, method: Union[MethodType, str]) -> None:
        ...
        if not isinstance(method, (MethodType, str)):
            raise TypeError(
                f"Expected a method or a string, but got: {type(method).__name__}"
            )
    ...
    def _wrap_method_with_module_call_tracker(
        self, method: Callable, name: str
    ) -> Callable:
...
) -> tuple[Union[Any, nn.Module], Optional[dict[str, Any]]]:
