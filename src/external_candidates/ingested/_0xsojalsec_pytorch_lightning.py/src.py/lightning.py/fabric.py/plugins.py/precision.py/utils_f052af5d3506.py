# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\plugins\precision\utils.py
...
class _ClassReplacementContextManager:
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        for class_string, replacement in self._mapping.items():
            _, class_name = class_string.rsplit(".", 1)
            setattr(
                self._modules[class_string], class_name, self._originals[class_string]
            )
