...
class Collective(ABC):
    ...
    @abstractmethod
    def reduce_scatter(
        self, output: Tensor, input_list: list[Tensor], op: str
    ) -> Tensor: ...
    ...
    @abstractmethod
    def recv(
        self, tensor: Tensor, src: Optional[int] = None, tag: int = 0
    ) -> Tensor: ...
    ...
    def teardown(self) -> Self:
        if self._group is None:
            raise RuntimeError(
                f"`{type(self).__name__}` does not own a group to destroy."
            )
