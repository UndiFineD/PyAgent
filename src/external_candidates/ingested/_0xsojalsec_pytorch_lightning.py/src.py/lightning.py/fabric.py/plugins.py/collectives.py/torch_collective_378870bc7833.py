# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\plugins\collectives\torch_collective.py
import datetime
import os
...
class TorchCollective(Collective):
    ...
    @override
    def all_reduce(
        self, tensor: Tensor, op: Union[str, ReduceOp, RedOpType] = "sum"
    ) -> Tensor:
    ...
    @override
    def reduce(
        self, tensor: Tensor, dst: int, op: Union[str, ReduceOp, RedOpType] = "sum"
    ) -> Tensor:
        op = self._convert_to_native_op(op)
    ...
    @override
    def gather(
        self, tensor: Tensor, gather_list: list[Tensor], dst: int = 0
    ) -> list[Tensor]:
    ...
    @override
    def scatter(
        self, tensor: Tensor, scatter_list: list[Tensor], src: int = 0
    ) -> Tensor:
    ...
    @override
    def all_to_all(
        self, output_tensor_list: list[Tensor], input_tensor_list: list[Tensor]
    ) -> list[Tensor]:
    ...
    def gather_object(
        self, obj: Any, object_gather_list: list[Any], dst: int = 0
    ) -> list[Any]:
    ...
    def monitored_barrier(
        self, timeout: Optional[datetime.timedelta] = None, wait_all_ranks: bool = False
    ) -> None:
    ...
    def teardown(self) -> Self:
        ...
        if (
            ...
            and (default_group := dist.GroupMember.WORLD)
            is not None  # not destroyed already
            and len(dist.distributed_c10d._pg_map)
            == 1  # only the default group is left
        ):
    ...
    def _convert_to_native_op(
        cls, op: Union[str, ReduceOp, RedOpType]
