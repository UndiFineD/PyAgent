# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\plugins\precision\amp.py
...
class MixedPrecision(Precision):
    ...
    def __init__(
        ...
        if scaler is not None and self.precision == "bf16-mixed":
            raise ValueError(
                f"`precision='bf16-mixed'` does not use a scaler, found {scaler}."
            )
        self.device = device
        self.scaler = scaler

        self._desired_input_dtype = (
