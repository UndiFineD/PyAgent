# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\plugins\precision\fsdp.py
...
class FSDPPrecision(Precision):
    ...
    def __init__(
        self, precision: _PRECISION_INPUT, scaler: Optional["ShardedGradScaler"] = None
    ) -> None:
        ...
        if scaler is not None and self.precision != "16-mixed":
            raise ValueError(
                f"`precision={precision!r}` does not use a scaler, found {scaler}."
            )

        self.scaler = (
            ShardedGradScaler() if scaler is None and precision == "16-mixed" else None
        )
    ...
    def mixed_precision_config(self) -> "TorchMixedPrecision":
        ...
        if self.precision in ("16-true", "16-mixed"):
            ...
            raise ValueError(
                f"Was unable to infer precision type, received {self.precision!r}."
            )
    ...
    def module_init_context(self) -> AbstractContextManager:
        return _DtypeContextManager(
            self.mixed_precision_config.param_dtype or torch.float32
        )
    ...
    def forward_context(self) -> AbstractContextManager:
        if "mixed" in self.precision:
            return torch.autocast(
                "cuda",
                dtype=(
                    torch.bfloat16 if self.precision == "bf16-mixed" else torch.float16
                ),
            )
    ...
    @override
    def backward(
        self, tensor: Tensor, model: Optional[Module], *args: Any, **kwargs: Any
    ) -> None:
