# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\plugins\environments\mpi.py
...
class MPIEnvironment(ClusterEnvironment):
    ...
    def set_world_size(self, size: int) -> None:
        log.debug(
            "MPIEnvironment.set_world_size was called, but setting world size is not allowed. Ignored."
        )
    ...
    def set_global_rank(self, rank: int) -> None:
        log.debug(
            "MPIEnvironment.set_global_rank was called, but setting global rank is not allowed. Ignored."
        )
    ...
    def _init_comm_local(self) -> None:
        ...
        all_hostnames = self._comm_world.gather(
            hostname, root=0
