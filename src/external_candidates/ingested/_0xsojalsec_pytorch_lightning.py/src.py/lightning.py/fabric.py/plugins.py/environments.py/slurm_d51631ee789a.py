# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\plugins\environments\slurm.py
...
class SLURMEnvironment(ClusterEnvironment):
    ...
    def __init__(
        self, auto_requeue: bool = True, requeue_signal: Optional[signal.Signals] = None
    ) -> None:
        super().__init__()
        _raise_enterprise_not_available()
        ...
        self.slurm_impl = EnterpriseSLURMEnvironment(
            auto_requeue=auto_requeue, requeue_signal=requeue_signal
        )
