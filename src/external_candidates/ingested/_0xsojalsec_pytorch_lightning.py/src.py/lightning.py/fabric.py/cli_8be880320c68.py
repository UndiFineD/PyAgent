# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\cli.py
...
def _get_supported_strategies() -> list[str]:
    ...
    return [
        ...
        if not re.match(excluded, strategy)
    ]
...
if _CLICK_AVAILABLE:
    ...
    @click.option(
        "--precision",
        type=click.Choice(
            get_args(_PRECISION_INPUT_STR) + get_args(_PRECISION_INPUT_STR_ALIAS)
        ),
        default=None,
        help=(
...
def _get_num_processes(accelerator: str, devices: str) -> int:
    ...
    if devices == "auto":
        if accelerator == "cuda" or accelerator == "mps" or accelerator == "cpu":
            ...
            raise ValueError(
                f"Cannot default to '1' device for accelerator='{accelerator}'"
            )
    if accelerator == "gpu":
        ...
        raise ValueError(
            "Launching processes for TPU through the CLI is not supported."
        )
    else:
...
def _torchrun_launch(args: Namespace, script_args: list[str]) -> None:
    ...
    num_processes = (
        1
        if args.strategy == "dp"
        else _get_num_processes(args.accelerator, args.devices)
    )
