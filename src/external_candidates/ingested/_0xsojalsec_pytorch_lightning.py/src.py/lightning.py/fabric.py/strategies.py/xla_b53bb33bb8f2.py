# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\strategies\xla.py
...
class XLAStrategy(ParallelStrategy):
    ...
    def __init__(
        ...
        from pytorch_lightning_enterprise.strategies.xla.ddp import (
            XLAStrategyFabric as EnterpriseXLAStrategy,
        )

        self.xla_strategy_impl = EnterpriseXLAStrategy(
            outer_object=self, sync_module_states=sync_module_states
        )
    ...
    def precision(self, precision: Optional[Precision]) -> None:
        if precision is not None and not isinstance(precision, XLAPrecision):
            raise TypeError(
                f"The XLA strategy can only work with the `XLAPrecision` plugin, found {precision}"
            )
    ...
    def all_reduce(
        ...
        return self.xla_strategy_impl.all_reduce(
