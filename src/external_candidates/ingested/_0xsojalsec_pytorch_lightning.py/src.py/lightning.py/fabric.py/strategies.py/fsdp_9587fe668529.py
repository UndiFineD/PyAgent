# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\strategies\fsdp.py
...
if TYPE_CHECKING:
    ...
    _POLICY = Union[
        set[type[Module]], Callable[[Module, bool, int], bool], ModuleWrapPolicy
    ]
...
warnings.filterwarnings(
    "ignore", category=FutureWarning, message=".*FSDP.state_dict_type.*"
)  # from torch >= 2.4
...
class FSDPStrategy(ParallelStrategy, _Sharded):
        ...
        **kwargs: Any,
        ...
        if device_mesh is not None:
            if not _TORCH_GREATER_EQUAL_2_2:
                raise ValueError(
                    "The `device_mesh` argument is only supported in torch >= 2.2."
                )
        ...
        self.sharding_strategy = _init_sharding_strategy(
            sharding_strategy, self._fsdp_kwargs
        )
    ...
    def checkpoint_io(self) -> CheckpointIO:
        raise NotImplementedError(
            f"The `{type(self).__name__}` does not use the `CheckpointIO` plugin interface."
        )
    ...
    def checkpoint_io(self, io: CheckpointIO) -> None:
        raise NotImplementedError(
            f"The `{type(self).__name__}` does not support setting a `CheckpointIO` plugin."
        )
    ...
    def precision(self, precision: Optional[Precision]) -> None:
        if precision is not None and not isinstance(precision, FSDPPrecision):
            raise TypeError(
                f"The FSDP strategy can only work with the `FSDPPrecision` plugin, found {precision}"
            )
    ...
    def setup_environment(self) -> None:
        ...
        if isinstance(self._fsdp_kwargs.get("device_mesh"), tuple):
            ...
            self._fsdp_kwargs["device_mesh"] = init_device_mesh(
                "cuda", self._fsdp_kwargs["device_mesh"]
            )
    ...
    def module_init_context(
        self, empty_init: Optional[bool] = None
            ...
            with state_dict_ctx:
                for key, obj in state.items():
                    ...
                    if isinstance(obj, Module):
                        ...
                        converted = (
                            obj.state_dict() if isinstance(obj, _Stateful) else obj
                        )
    ...
    def load_checkpoint(
        ...
        if isinstance(state, Module):
            ...
            _load_raw_module_state_from_path(
                path, module=state, world_size=self.world_size, strict=strict
            )
            return {}
        ...
        modules = {
            key: module for key, module in state.items() if _has_fsdp_modules(module)
        }
        if len(modules) == 0:
            raise ValueError(
                "Could not find a FSDP model in the provided checkpoint state. Please provide the model as"
        ...
        if _is_full_checkpoint(path):
            ...
            for optim_key, optim in optimizers.items():
                ...
                with _get_full_state_dict_context(
                    module, world_size=self.world_size, rank0_only=False
                ):
                    temp_state_dict = _rekey_optimizer_state_if_needed(
                        checkpoint.pop(optim_key), module
                    )
    ...
    def _setup_distributed(self) -> None:
        ...
        if _TORCH_GREATER_EQUAL_2_3:
            kwargs["device_id"] = (
                self.root_device if self.root_device.type != "cpu" else None
            )
        _init_dist_connection(
            self.cluster_environment, self._process_group_backend, **kwargs
        )
...
def _activation_checkpointing_kwargs(
    ...
    if activation_checkpointing is None and activation_checkpointing_policy is None:
        return {}
    if (
        activation_checkpointing is not None
        and activation_checkpointing_policy is not None
    ):
        raise ValueError(
...
def _setup_activation_checkpointing(
    module: Module, activation_checkpointing_kwargs: dict
) -> None:
...
def _init_cpu_offload(cpu_offload: Optional[Union[bool, "CPUOffload"]]) -> "CPUOffload":
    ...
    return (
        cpu_offload
        if isinstance(cpu_offload, CPUOffload)
        else CPUOffload(offload_params=bool(cpu_offload))
    )


def _init_sharding_strategy(
    sharding_strategy: "_SHARDING_STRATEGY", kwargs: dict
) -> "ShardingStrategy":
    from torch.distributed.fsdp import ShardingStrategy

    if (
        kwargs.get("process_group") is not None
        and kwargs.get("device_mesh") is not None
    ):
        raise ValueError(
    ...
    strategy = (
        ShardingStrategy[sharding_strategy.upper()]
...
def _optimizer_has_flat_params(optimizer: Optimizer) -> bool:
    return any(
        getattr(param, "_fsdp_flattened", False)
