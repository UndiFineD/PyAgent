# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\strategies\model_parallel.py
...
class ModelParallelStrategy(ParallelStrategy):
    ...
    def distributed_sampler_kwargs(self) -> dict[str, Any]:
        ...
        return {
            "num_replicas": data_parallel_mesh.size(),
            "rank": data_parallel_mesh.get_local_rank(),
        }
    ...
    def load_checkpoint(
        ...
        return _load_checkpoint(
            path=path, state=state, strict=strict, weights_only=weights_only
        )
...
def _save_checkpoint(
    ...
    state_dict_options = StateDictOptions(
        full_state_dict=full_state_dict, cpu_offload=True
    )
    ...
    for key, obj in state.items():
        ...
        if isinstance(obj, Module):
            ...
            converted = get_optimizer_state_dict(
                module, obj, options=state_dict_options
            )
            target_dict = converted_state
...
def _load_checkpoint(
    ...
    modules = {
        key: module for key, module in state.items() if _has_dtensor_modules(module)
    }
    if len(modules) == 0:
        raise ValueError(
            "Could not find a distributed model in the provided checkpoint state. Please provide the model as"
    ...
    if _is_full_checkpoint(path):
        checkpoint = torch.load(
            path, mmap=True, map_location="cpu", weights_only=weights_only
        )
...
def _has_dtensor_modules(module: object) -> TypeGuard[Module]:
    ...
    return isinstance(module, Module) and any(
        isinstance(t, DTensor) for t in module.parameters()
    )


def _load_raw_module_state_from_path(
    path: Path, module: Module, world_size: int, strict: bool = True
) -> None:
    ...
    _load_raw_module_state(
        state_dict=state_dict, module=module, world_size=world_size, strict=strict
    )
    ...
    if _has_dtensor_modules(module):
        ...
        for submodule_name, submodule in module.named_modules():
            for param_name, _ in _named_parameters_and_buffers_to_load(submodule):
                full_param_name = (
                    f"{submodule_name}{'.' if submodule_name else ''}{param_name}"
                )
                ...
                set_model_state_dict(
                    submodule, local_state_dict, options=state_dict_options
                )

    elif isinstance(module, FSDP):
        with _get_full_state_dict_context(
            module, world_size=world_size, rank0_only=False
        ):
...
def _rekey_optimizer_state_if_needed(
    optimizer_state_dict: dict[str, Any], module: Module
) -> dict[str, Any]:
