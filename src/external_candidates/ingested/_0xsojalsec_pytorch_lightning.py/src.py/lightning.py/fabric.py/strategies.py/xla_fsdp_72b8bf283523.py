# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\strategies\xla_fsdp.py
...
class XLAFSDPStrategy(ParallelStrategy, _Sharded):
    ...
    def checkpoint_io(self, io: Optional[CheckpointIO]) -> None:
        if io is not None and not isinstance(io, XLACheckpointIO):
            raise TypeError(
                f"The XLA strategy can only work with the `XLACheckpointIO` plugin, found {io}"
            )
    ...
    def precision(self, precision: Optional[Precision]) -> None:
        if precision is not None and not isinstance(precision, XLAPrecision):
            raise TypeError(
                f"The XLA FSDP strategy can only work with the `XLAPrecision` plugin, found {precision}"
            )
    ...
    def setup_module_and_optimizers(
        ...
        return self.xla_fsdp_impl.setup_module_and_optimizers(
    ...
    def clip_gradients_value(
        ...
        return self.xla_fsdp_impl.clip_gradients_value(
            module=module, optimizer=optimizer, clip_val=clip_val
        )
    ...
    def all_gather(
        ...
        """
        return self.xla_fsdp_impl.all_gather(
            tensor=tensor, group=group, sync_grads=sync_grads
        )
    ...
    def all_reduce(
        ...
        return self.xla_fsdp_impl.all_reduce(
            output=output, group=group, reduce_op=reduce_op
        )
    ...
    def load_checkpoint(
        ...
        """
        return self.xla_fsdp_impl.load_checkpoint(
