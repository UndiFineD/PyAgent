# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\testing\_runif.py
...
def _runif_reasons(
    *,
    ...
    if min_torch and compare_version(
        "torch", operator.lt, min_torch, use_base_version=True
    ):
        reasons.append(f"torch>={min_torch}, {torch.__version__} installed")

    # set use_base_version for nightly support
    if max_torch and compare_version(
        "torch", operator.ge, max_torch, use_base_version=True
    ):
    ...
    if bf16_cuda:
        try:
            ...
            is_unrelated = "Found no NVIDIA driver" not in str(
                ex
            ) or "Torch not compiled with CUDA" not in str(ex)
            if is_unrelated:
                raise ex
            cond = True
        if cond:
