# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\data.py
...
def _get_dataloader_init_args_and_kwargs(
    ...
    if not was_wrapped:
        ...
        non_defaults = {
            ...
            if name in attrs and p.default is not attrs[name]
        }
    ...
    if required_args:
        ...
        missing_args_message = ", ".join(
            f"`self.{arg_name}`" for arg_name in sorted_required_args
        )
...
def _dataloader_init_kwargs_resolve_sampler(
    ...
    if batch_sampler is not None and type(batch_sampler) is not BatchSampler:
        ...
        elif hasattr(batch_sampler, "batch_size") and hasattr(
...
def _reinstantiate_wrapped_cls(
    orig_object: Any, *args: Any, explicit_cls: Optional[type] = None, **kwargs: Any
) -> Any:
...
def _wrap_init_method(
    init: Callable, store_explicit_arg: Optional[str] = None
) -> Callable:
    ...
    def wrapper(obj: Any, *args: Any, **kwargs: Any) -> None:
        ...
        if store_explicit_arg is not None:
            if store_explicit_arg in param_names:
                object.__setattr__(
                    obj,
                    f"__{store_explicit_arg}",
                    args[param_names.index(store_explicit_arg)],
                )
            elif store_explicit_arg in kwargs:
                object.__setattr__(
                    obj, f"__{store_explicit_arg}", kwargs[store_explicit_arg]
                )
...
def _wrap_attr_method(method: Callable, tag: _WrapAttrTag) -> Callable:
    ...
    def wrapper(obj: Any, *args: Any) -> None:
        ...
        name, *_ = args
        prev_call_name, prev_call_method = getattr(
            obj, "__pl_current_call", (None, "method")
        )
...
def _replace_dunder_methods(
    base_cls: type, store_explicit_arg: Optional[str] = None
    ...
    for cls in classes:
        ...
        for patch_fn_name, tag in (
            ("__setattr__", _WrapAttrTag.SET),
            ("__delattr__", _WrapAttrTag.DEL),
        ):
            if patch_fn_name in cls.__dict__ or cls is base_cls:
                ...
                setattr(
                    cls,
                    patch_fn_name,
                    _wrap_attr_method(getattr(cls, patch_fn_name), tag),
                )
    yield
    for cls in classes:
...
def suggested_max_num_workers(local_world_size: int) -> int:
    ...
    return max(
        1, cpu_count // local_world_size - 1
    )  # -1 to leave some resources for main process
