# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\distributed.py
import atexit
import contextlib
import logging
import os
...
def is_shared_filesystem(
    strategy: "Strategy", path: Optional[_PATH] = None, timeout: int = 3
) -> bool:
...
def _sync_ddp(
    ...
    if isinstance(reduce_op, str):
        ...
        if (
            reduce_op.lower() == "avg"
            and torch.distributed.get_backend(group) == "gloo"
        ):
...
def _init_dist_connection(
    ...
    **kwargs: Any,
    ...
    if torch.distributed.is_initialized():
        ...
        return
    global_rank = (
        global_rank if global_rank is not None else cluster_environment.global_rank()
    )
    world_size = (
        world_size if world_size is not None else cluster_environment.world_size()
    )
    ...
    def __enter__(self) -> Self:
        if _distributed_is_initialized():
            ...
            self.group = torch.distributed.new_group(
                backend="gloo", timeout=timedelta(days=10000)
            )
