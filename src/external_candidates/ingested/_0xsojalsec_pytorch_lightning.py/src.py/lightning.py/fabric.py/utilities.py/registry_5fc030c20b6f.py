# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\registry.py
...
def _load_external_callbacks(group: str) -> list[Any]:
    ...
    for factory in factories:
        ...
        callbacks_list = (
            [callbacks_list] if not isinstance(callbacks_list, list) else callbacks_list
        )
        if callbacks_list:
            _log.info(
...
def _register_classes(
    registry: Any, method: str, module: ModuleType, parent: type[object]
) -> None:
