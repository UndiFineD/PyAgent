# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\init.py
...
def _has_meta_device_parameters_or_buffers(
    obj: Union[Module, Optimizer], recurse: bool = True
) -> bool:
    ...
    if isinstance(obj, Module):
        return any(
            ...
            for t in itertools.chain(
                obj.parameters(recurse=recurse), obj.buffers(recurse=recurse)
            )
        )
    raise TypeError(
        f"Expected `torch.nn.Module` or `torch.optim.Optimizer`, got: {type(obj).__name__}"
    )
