# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\optimizer.py
...
def _optimizer_to_device(optimizer: Optimizer, device: _DEVICE) -> None:
    ...
    for p, v in optimizer.state.items():
        if not isinstance(v, MutableMapping):
            ...
            optimizer.state[p] = apply_to_collection(
                v, Tensor, move_data_to_device, device, allow_frozen=True
            )
            continue
