# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\load.py
...
class _NotYetLoadedTensor:
    ...
    def __torch_function__(
        ...
        kwargs = kwargs or {}
        loaded_args = [
            (arg._load_tensor() if isinstance(arg, _NotYetLoadedTensor) else arg)
            for arg in args
        ]
...
def _materialize_tensors(collection: Any) -> Any:
    ...
    return apply_to_collection(
        collection, dtype=_NotYetLoadedTensor, function=_load_tensor
    )


def _move_state_into(
    source: dict[str, Any],
    destination: dict[str, Union[Any, _Stateful]],
