# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\fabric\utilities\throughput.py
...
class ThroughputMonitor(Throughput):
    ...
    def __init__(self, fabric: "Fabric", **kwargs: Any) -> None:
        ...
        super().__init__(
            available_flops=available_flops, world_size=fabric.world_size, **kwargs
        )
        self._fabric = fabric
        self.step = -1
    ...
    def compute_and_log(
        self, step: Optional[int] = None, **kwargs: Any
    ) -> _THROUGHPUT_METRICS:
...
def get_available_flops(
    device: torch.device, dtype: Union[torch.dtype, str]
) -> Optional[int]:
