# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytorch-lightning\src\lightning\__setup__.py
import glob
import os.path
...
_ASSISTANT = _load_py_module(
    name="assistant", location=os.path.join(_PROJECT_ROOT, ".actions", "assistant.py")
)
...
def _prepare_extras() -> dict[str, Any]:
    ...
    req_files = [
        Path(p) for p in glob.glob(os.path.join(_PATH_REQUIREMENTS, "*", "*.txt"))
    ]
    common_args = {"unfreeze": "none" if _FREEZE_REQUIREMENTS else "major"}
    # per-project extras
    extras = {
        f"{p.parent.name}-{p.stem}": _ASSISTANT.load_requirements(
    ...
    extras["fabric-all"] = (
        extras["fabric-extra"] + extras["fabric-strategies"] + extras["fabric-examples"]
    )
    extras["fabric-dev"] = extras["fabric-all"] + extras["fabric-test"]
    extras["pytorch-all"] = (
        extras["pytorch-extra"]
        + extras["pytorch-strategies"]
        + extras["pytorch-examples"]
    )
