# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-upscale-video\src\pipelines\flashvsr_tiny_long.py
import os
import time
import types
...
from ..models.wan_video_vae import CausalConv3d, RMS_norm, Upsample, WanVideoVAE
    ...
    def __call__(
        ...
        with torch.no_grad():
            for cur_process_idx in progress_bar_cmd(range(process_total_num)):
                ...
                # Decode
                cur_LQ_frame = LQ_video[:, :, LQ_pre_idx:LQ_cur_idx, :, :].to(
                    self.device
                )
                cur_frames = (
                    self.TCDecoder.decode_video(
