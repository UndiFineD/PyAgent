...
def build_signature_toc(certificates, signature_length):
    ...
    return SIGNATURE_TOC_ENTRY.format(
        signature_length=signature_length, x509_certs=x509_certs
    ).rstrip()
...
def generate_pkg(
    version, command, package_name, cert_file=None, key_file=None, ca_file=None
):
    ...
    if key_file and cert_file:
        buf.write(get_signature(key_file, toc_compressed))  # write signature
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a .pkg file for macOS and optionally sign it"
    )
    parser.add_argument(
        "-v", "--version", required=True, help="CFBundleVersion for the PKG file"
    )
    ...
    parser.add_argument(
        ...
        "--name",
        required=True,
        help="Package name. Defaults to the output file name",
    )
