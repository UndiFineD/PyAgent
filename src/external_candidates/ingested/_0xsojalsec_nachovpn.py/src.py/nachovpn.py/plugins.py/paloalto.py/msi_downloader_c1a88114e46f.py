...
class MSIDownloader:
    ...
    def get_latest_versions(self):
        ...
        x86_keys = [
            ...
            for c in contents
            if "GlobalProtect.msi" in c.find("s3:Key", ns).text
        ]
        x64_keys = [
            c.find("s3:Key", ns).text
            for c in contents
            if "GlobalProtect64.msi" in c.find("s3:Key", ns).text
        ]
        ...
        with open(output_path, "wb") as f:
            for data in response.iter_content(block_size):
                ...
                if total_size:
                    ...
                    sys.stdout.write(
                        f"\rDownloading: [{'=' * progress}{' ' * (50-progress)}] {current_size}/{total_size} bytes"
                    )
                    sys.stdout.flush()
    ...
    def download_latest_msi(self):
        ...
        print(
            f"Downloading latest MSI files (version: {latest_version_x86.split('/')[0]})"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--force", action="store_true", help="Force download even if files exist"
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "-d", "--download", action="store_true", help="Download latest MSI files"
    )
    group.add_argument(
        "-v",
        "--version",
        action="store_true",
        help="Show latest version information only",
    )
    ...
    elif args.download and (
        not os.path.exists(os.path.join(args.output_dir, "GlobalProtect.msi"))
        or not os.path.exists(os.path.join(args.output_dir, "GlobalProtect64.msi"))
        or args.force
    ):
