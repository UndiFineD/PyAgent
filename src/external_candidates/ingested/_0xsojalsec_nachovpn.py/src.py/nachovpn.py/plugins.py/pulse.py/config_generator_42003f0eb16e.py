...
class VPNConfigGenerator:
    ...
    def create_routes(self):
        ...
        routes_section += self.write_be32(
            len(self.routes)
    ...
    def create_config(self):
        ...
        final_attrs += self.create_attribute(
            CFG_DNS_SUFFIX, self.dns_suffix.encode() + b"\x00"
        )
        ...
        final_attrs += self.create_attribute(
            CFG_ESP_ENC_ALG, self.write_be16(ENC_AES_256_CBC)
        )
        final_attrs += self.create_attribute(
            CFG_ESP_HMAC_ALG, self.write_be16(HMAC_SHA256)
        )
        final_attrs += self.create_attribute(
            CFG_ESP_KEY_LIFETIME, self.write_be32(1200)
        )
        final_attrs += self.create_attribute(CFG_ESP_KEY_BYTES, self.write_be32(0))
        final_attrs += self.create_attribute(
            CFG_ESP_REPLAY_PROTECTION, self.write_be32(1)
        )
        ...
        final_attrs += self.create_attribute(
            CFG_ESP_TO_SSL_FALLBACK_SECS, self.write_be32(15)
        )
        final_attrs += self.create_attribute(CFG_UNKNOWN_4018, self.write_be32(60))
        final_attrs += self.create_attribute(
            CFG_INTERNAL_LEGACY_IP, self.write_be32(self.ipv4_to_int("10.10.1.1"))
        )
        final_attrs += self.create_attribute(
            CFG_NETMASK, self.write_be32(self.ipv4_to_int("255.255.255.255"))
        )
        final_attrs += self.create_attribute(
            CFG_INTERNAL_GATEWAY_IP, self.write_be32(self.ipv4_to_int("10.200.200.200"))
        )
        final_attrs += self.create_attribute(
            CFG_LOGON_SCRIPT, self.logon_script.encode() + b"\x00"
        )
        final_attrs += self.create_attribute(0x400D, b"\x00")
        final_attrs += self.create_attribute(0x400E, b"\x00")
        final_attrs += self.create_attribute(
            CFG_LOGON_SCRIPT_MAC, self.logon_script_macos.encode() + b"\x00"
        )
        ...
        final_attrs[4:6] = self.write_be16(
            len(final_attrs)
        ...
        data[config_len_offset : config_len_offset + 4] = self.write_be32(
            total_length - 0x10
        )
