...
class VPNPlugin:
    def __init__(
        ...
        self.pcap_filename = os.path.join(
            "pcaps", f"{self.__class__.__name__.lower().rstrip('plugin')}.pcap"
        )
        ...
        if template_dir:
            self.template_env = Environment(
                loader=FileSystemLoader([template_dir, default_dir])
            )
        else:
    ...
    def get_thumbprint(self):
        ...
        if os.getenv("USE_DYNAMIC_SERVER_THUMBPRINT", "false").lower() == "true":
            dynamic_thumbprint = self.cert_manager.get_thumbprint_from_server(
                self.dns_name
            )
            if dynamic_thumbprint:
                self.logger.debug(
                    f"Using dynamic thumbprint for {self.dns_name}: {dynamic_thumbprint}"
                )
    ...
    def handle_post(self, handler):
        ...
        with self.flask_app.test_client() as client:
            response = client.post(
                handler.path, data=body, headers=dict(handler.headers)
            )
            self._send_flask_response(response, handler)
        return True
