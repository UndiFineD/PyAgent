...
class ThreadedVPNServer(socketserver.ThreadingTCPServer):
    def __init__(
        self, server_address, RequestHandlerClass, cert_manager, plugin_manager
    ):
        self.cert_manager = cert_manager
        self.plugin_manager = plugin_manager
        super().__init__(server_address, RequestHandlerClass)
        self.socket = cert_manager.ssl_context.wrap_socket(
...
class VPNServer:
    def __init__(
        self, host="0.0.0.0", port=443, cert_dir=os.path.join(os.getcwd(), "certs")
    ):
        self.host = host
        self.port = port
