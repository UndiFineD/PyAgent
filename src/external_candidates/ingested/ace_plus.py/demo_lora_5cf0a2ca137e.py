# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\demo_lora.py
# -*- coding: utf-8 -*-
...
class DemoUI(object):
    def __init__(
        ...
        for task_name, task_model in self.task_model_cfg.MODEL.items():
            ...
            for preprocessor in task_model.get("PREPROCESSOR", []):
                if preprocessor["TYPE"] in self.edit_type_dict:
                    continue
                preprocessor["REPAINTING_SCALE"] = task_model.get(
        ...
        self.all_examples = [
            [
                one_example["task_type"],
                one_example["edit_type"],
                one_example["instruction"],
                one_example["input_reference_image"],
                one_example["input_image"],
                one_example["input_mask"],
                one_example["output_h"],
                one_example["output_w"],
                one_example["seed"],
            ]
    ...
    def create_ui(self):
        ...
        with gr.Row():
            self.generation_info_preview = gr.Markdown(
                label="System Log.", show_label=True
            )
        ...
        with gr.Accordion(label="Advance", open=True):
            ...
            with gr.Row():
                ...
                self.seed = gr.Slider(
                    minimum=-1, maximum=10000000, value=-1, label="Seed"
                )
    ...
    def set_callbacks(self, *args, **kwargs):
        ...
        def change_task_type(task_type):
            ...
            for preprocessor in task_info.get("PREPROCESSOR", []):
                preprocessor["REPAINTING_SCALE"] = task_info.get(
                    "REPAINTING_SCALE", 1.0
                )
                self.edit_type_dict[preprocessor["TYPE"]] = preprocessor
        ...
        self.task_type.change(
            change_task_type, inputs=[self.task_type], outputs=[self.edit_type]
        )
        ...
        self.edit_type.change(
            change_edit_type, inputs=[self.edit_type], outputs=[self.repainting_scale]
        )
        ...
        def run_chat(
            ...
            pre_edit_image, pre_edit_mask, pre_ref_image, is_suc, err_msg = (
                preprocess_input(ref_image, edit_image)
            )
            if not is_suc:
            ...
            pre_edit_image = edit_preprocess(
                edit_info, we.device_id, pre_edit_image, pre_edit_mask
            )
            ...
            return (
                ...
                gr.Image(
                    value=(
                        pre_edit_image if pre_edit_image is not None else pre_ref_image
                    )
                ),
        ...
        self.chat_btn.click(
            run_chat, inputs=[self.text] + chat_inputs, outputs=chat_outputs, queue=True
        )
        ...
        def run_example(
            ...
            pre_edit_image, pre_edit_mask, pre_ref_image, _, _ = preprocess_input(
                ref_image, edit_image
            )
            pre_edit_image = edit_preprocess(
                edit_info, we.device_id, pre_edit_image, pre_edit_mask
            )
            ...
            if pre_edit_image is not None:
                ret_image = Image.composite(
                    Image.new("RGB", pre_edit_image.size, (0, 0, 0)),
                    pre_edit_image,
                    pre_edit_mask,
                )
            else:
        ...
        with self.eg:
            self.example_edit_image = gr.Image(
                label="Edit Image", type="pil", image_mode="RGB", visible=False
            )
            self.example_edit_mask = gr.Image(
                label="Edit Image Mask", type="pil", image_mode="L", visible=False
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--server_port", dest="server_port", help="", type=int, default=2345
    )
