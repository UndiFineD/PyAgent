...
class ACEInference(BaseInference):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        image, mask, change_image, content_image, out_h, out_w, slice_w = (
        ...
        change_image = (
            [None] if change_image is None else [change_image.to(we.device_id)]
        )
        ...
        imgs = [
            x_i["reconstruct_image"].float().permute(1, 2, 0).cpu().numpy()
        ...
        mask = Image.fromarray(
            (mask[0] * 255).squeeze(0).cpu().numpy().astype(np.uint8)
        )
        return (
