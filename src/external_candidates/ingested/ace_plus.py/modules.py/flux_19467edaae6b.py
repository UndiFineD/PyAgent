# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\flux.py
# -*- coding: utf-8 -*-
...
class Flux(BaseModel):
    ...
    para_dict = {
        "IN_CHANNELS": {"value": 64, "description": "model's input channels."},
        "OUT_CHANNELS": {"value": 64, "description": "model's output channels."},
        "HIDDEN_SIZE": {"value": 1024, "description": "model's hidden size."},
        "NUM_HEADS": {
            "value": 16,
        ...
        "THETA": {"value": 10_000, "description": "theta for positional encoding."},
        "VEC_IN_DIM": {"value": 768, "description": "dimension of the vector input."},
        "GUIDANCE_EMBED": {
            "value": False,
        ...
        "DEPTH": {"value": 19, "description": "number of transformer blocks."},
        "DEPTH_SINGLE_BLOCKS": {
            "value": 38,
        ...
        for k, v in key_map.items():
            ...
            for block_id in range(block_num):
                for k_list in key_list:
                    if (
                        k_list[0].format(block_id) in lora_sd
                        and k_list[1].format(block_id) in lora_sd
                    ):
                        cover_lora_keys.add(k_list[0].format(block_id))
                        cover_lora_keys.add(k_list[1].format(block_id))
                        current_weight = torch.matmul(
                            lora_sd[k_list[0].format(block_id)].permute(1, 0),
                            lora_sd[k_list[1].format(block_id)].permute(1, 0),
                        ).permute(1, 0)
                        ori_sd[k.format(block_id)][
                            k_list[2][0] : k_list[2][1], ...
                        ] += (scale * current_weight)
                        cover_ori_keys.add(k.format(block_id))
                        # lora_sd.pop(k_list[0].format(block_id))
                        # lora_sd.pop(k_list[1].format(block_id))
        self.logger.info(
            f"merge_blackforest_lora loads lora'parameters lora-paras: \n"
    ...
    def merge_swift_lora(self, ori_sd, lora_sd, scale=1.0):
        ...
        self.logger.info(
            f"merge_swift_lora loads lora'parameters {len(have_lora_keys)}"
        )
        for key, v in have_lora_keys.items():
            current_weight = torch.matmul(
                v["lora_A"].permute(1, 0), v["lora_B"].permute(1, 0)
            ).permute(1, 0)
    ...
    def merge_comfyui_lora(self, ori_sd, lora_sd, scale=1.0):
        ...
        for key, v in parse_ckpt.items():
            current_weight = torch.matmul(
                v["lora_down"].permute(1, 0), v["lora_up"].permute(1, 0)
            ).permute(1, 0)
        ...
        if pretrained_model is not None:
            if not hasattr(self, "ckpt"):
                with FS.get_from(pretrained_model, wait_finish=True) as local_model:
                    if local_model.endswith("safetensors"):
                        ...
                        ckpt = torch.load(
                            local_model, map_location=map_location, weights_only=True
                        )
            ...
            if self.swift_lora_model is not None:
                ...
                for lora_model in self.swift_lora_model:
                    ...
                    new_ckpt = self.merge_swift_lora(
                        new_ckpt, lora_sd, scale=lora_model[1]
                    )

            if self.blackforest_lora_model is not None:
                with FS.get_from(
                    self.blackforest_lora_model, wait_finish=True
                ) as local_model:
                    if local_model.endswith("safetensors"):
                        from safetensors.torch import load_file as load_safetensors
                ...
                new_ckpt = self.merge_blackforest_lora(
                    new_ckpt, lora_sd, scale=lora_scale
                )

            if self.comfyui_lora_model is not None:
                if (
                    hasattr(self, "current_lora")
                    and self.current_lora == self.comfyui_lora_model
                ):
                    return
                if (
                    hasattr(self, "lora_dict")
                    and self.comfyui_lora_model in self.lora_dict
                ):
                    new_ckpt = self.easy_lora_merge(
                        new_ckpt,
                        self.lora_dict[self.comfyui_lora_model],
                        scale=lora_scale,
                    )
                    ...
                    with FS.get_from(
                        self.comfyui_lora_model, wait_finish=True
                    ) as local_model:
                        if local_model.endswith("safetensors"):
                            from safetensors.torch import load_file as load_safetensors
                    ...
                    new_ckpt = self.merge_comfyui_lora(
                        new_ckpt, lora_sd, scale=lora_scale
                    )
            ...
            if self.pretrain_adapter is not None:
                with FS.get_from(
                    self.pretrain_adapter, wait_finish=True
                ) as local_adapter:
                    if local_adapter.endswith("safetensors"):
                        from safetensors.torch import load_file as load_safetensors

                        adapter_ckpt = load_safetensors(
                            local_adapter, device=safe_device
                        )
                    else:
                        adapter_ckpt = torch.load(
                            local_adapter, map_location=map_location, weights_only=True
                        )
            new_ckpt.update(adapter_ckpt)

            missing, unexpected = self.load_state_dict(
                new_ckpt, strict=False, assign=True
            )
            self.logger.info(
    ...
    def get_config_template():
        return dict_to_yaml(
            "BACKBONE", __class__.__name__, Flux.para_dict, set_name=True
        )
...
class FluxMR(Flux):
    def prepare_input(self, x, cond):
        if isinstance(cond["context"], list):
            context, y = torch.cat(cond["context"], dim=0).to(x), torch.cat(
                cond["y"], dim=0
            ).to(x)
        else:
        ...
        for frames, frame_ids in zip(batch_frames, batch_frames_ids):
            ...
            mask_x_list.append(
                torch.ones(ix.shape[0]).to(ix.device, non_blocking=True).bool()
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, x_ids, txt, txt_ids, y, mask_x, mask_txt, seq_length_list = (
            self.prepare_input(x, cond)
        )
        # running on sequences img
        vec = self.time_in(timestep_embedding(t, 256))
    ...
    def get_config_template():
        return dict_to_yaml(
            "MODEL", __class__.__name__, FluxMR.para_dict, set_name=True
        )
...
class FluxMRACEPlus(FluxMR):
    ...
    def prepare_input(self, x, cond):
        ...
        batch_frames, batch_frames_ids = [], []
        for ix, shape, imask, ie, ie_mask in zip(
            x, cond["x_shapes"], cond["x_mask"], cond["edit"], cond["edit_mask"]
        ):
            # unpack image from sequence
            ix = ix[:, : shape[0] * shape[1]].view(-1, shape[0], shape[1])
            imask = (
                torch.ones_like(ix[[0], :, :]) if imask is None else imask.squeeze(0)
            )
            if len(ie) > 0:
                ie = [iie.squeeze(0) for iie in ie]
                ie_mask = [
                    (
                        torch.ones((ix.shape[0] * 4, ix.shape[1], ix.shape[2]))
                        if iime is None
                        else iime.squeeze(0)
                    )
                    for iime in ie_mask
                ...
                ie, ie_mask = (
                    torch.zeros_like(ix).to(x),
                    torch.ones_like(imask).to(x),
                )
        ...
        if isinstance(context, list):
            ...
            for sample_id, (ctx, yy) in enumerate(zip(context, y)):
                ...
                mask_txt_list.append(
                    torch.ones(txt_list[-1].shape[0])
                    .to(ctx.device, non_blocking=True)
                    .bool()
                )
    ...
    def get_config_template():
        return dict_to_yaml(
            "MODEL", __class__.__name__, FluxMRACEPlus.para_dict, set_name=True
        )
...
class FluxMRModiACEPlus(FluxMR):
    ...
    def prepare_input(self, x, cond):
        context, y = cond["context"], cond["y"]
        batch_frames, batch_frames_ids = [], []
        for ix, shape, imask, ie, im, ie_mask in zip(
            x,
            cond["x_shapes"],
            cond["x_mask"],
            cond["edit"],
