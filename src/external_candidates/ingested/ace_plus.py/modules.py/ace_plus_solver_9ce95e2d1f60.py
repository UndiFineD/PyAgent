# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\ace_plus_solver.py
# -*- coding: utf-8 -*-
...
class FormalACEPlusSolver(LatentDiffusionSolver):
    ...
    def run_eval(self):
        ...
        all_results = []
        for batch_idx, batch_data in tqdm(enumerate(self.datas[self._mode].dataloader)):
            self.before_iter(self.hooks_dict[self._mode])
            if self.sample_args:
                batch_data.update(self.sample_args.get_lowercase_dict())
            with torch.autocast(
                device_type="cuda", enabled=self.use_amp, dtype=self.dtype
            ):
                results = self.run_step_eval(
        ...
        self.register_probe(
            {
                "eval_image": ProbeData(
                    log_data, is_image=True, build_html=True, build_label=log_label
                )
            }
        )
    ...
    def save_results(self, results):
        ...
        for result in results:
            ...
            if edit_image is not None:
                for i, edit_img in enumerate(result["edit_image"]):
                    if edit_img is None:
                        continue
                    ret_images.append(
                        (edit_img.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
                    )
                    ret_labels.append(f"edit_image{i}; ")
                    ret_images.append(
                        (modify_image[i].permute(1, 2, 0).cpu().numpy() * 255).astype(
                            np.uint8
                        )
                    )
                    ...
                    if edit_mask is not None:
                        ret_images.append(
                            (edit_mask[i].permute(1, 2, 0).cpu().numpy() * 255).astype(
                                np.uint8
                            )
                        )
            ...
            if target_image is not None:
                ret_images.append(
                    (target_image.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
                )
                ret_labels.append(f"target_image; ")
                if target_mask is not None:
                    ret_images.append(
                        (target_mask.permute(1, 2, 0).cpu().numpy() * 255).astype(
                            np.uint8
                        )
                    )
            ...
            if teacher_image is not None:
                ret_images.append(
                    (teacher_image.permute(1, 2, 0).cpu().numpy() * 255).astype(
                        np.uint8
                    )
                )
            ...
            if reconstruct_image is not None:
                ret_images.append(
                    (reconstruct_image.permute(1, 2, 0).cpu().numpy() * 255).astype(
                        np.uint8
                    )
                )
                ...
                for prompt in self.probe_prompt:
                    with torch.autocast(
                        ...
                        batch_data = {
                            "prompt": [[prompt]],
                            "image": [
                                torch.zeros(3, self.probe_hw[0], self.probe_hw[1])
                            ],
                            "image_mask": [
                                torch.ones(1, self.probe_hw[0], self.probe_hw[1])
                            ],
                            "src_image_list": [[]],
                            "modify_image_list": [[]],
                            "src_mask_list": [[]],
