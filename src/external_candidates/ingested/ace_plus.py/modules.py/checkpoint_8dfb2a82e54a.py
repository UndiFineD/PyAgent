# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\checkpoint.py
# -*- coding: utf-8 -*-
...
from scepter.modules.solver.hooks import BackwardHook, CheckpointHook
...
def convert_to_comfyui_lora(ori_sd, prefix="lora_unet"):
    ...
    for k, v in ori_sd.items():
        new_k = k.replace(".lora_A.0_SwiftLoRA.", ".lora_down.").replace(
            ".lora_B.0_SwiftLoRA.", ".lora_up."
        )
        new_k = (
            prefix
            + "_"
            + new_k.split(".lora")[0].replace("model.", "").replace(".", "_")
            + ".lora"
            + new_k.split(".lora")[1]
        )
...
class ACECheckpointHook(CheckpointHook):
    ...
    def after_iter(self, solver):
        ...
        if solver.total_iter != 0 and (
            ...
            if isinstance(solver.model, SwiftModel) or (
                ...
                if we.rank == 0:
                    tuner_model = os.path.join(
                        save_path, "0_SwiftLoRA", "adapter_model.bin"
                    )
                    save_model = os.path.join(
                        save_path, "0_SwiftLoRA", "comfyui_model.safetensors"
                    )
    ...
    def get_config_template():
        return dict_to_yaml(
            "hook", __class__.__name__, ACECheckpointHook.para_dict, set_name=True
        )
...
class ACEBackwardHook(BackwardHook):
    def grad_clip(self, optimizer):
        for params_group in optimizer.param_groups:
            ...
            torch.nn.utils.clip_grad_norm_(
                parameters=train_params, max_norm=self.gradient_clip
            )
    ...
    def get_config_template():
        return dict_to_yaml(
            "hook", __class__.__name__, ACEBackwardHook.para_dict, set_name=True
        )
