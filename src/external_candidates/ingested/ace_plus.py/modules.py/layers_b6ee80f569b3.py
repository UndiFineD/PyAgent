...
class CrossAttention(nn.Module):
    def __init__(
        self, dim: int, context_dim: int, num_heads: int = 8, qkv_bias: bool = False
    ):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(
        self, x: Tensor, context: Tensor, pe: Tensor, mask: Tensor | None = None
    ) -> Tensor:
        qkv = self.qkv(x)
        q, k, v = rearrange(qkv, "B L (K H D) -> K B H L D", K=3, H=self.num_heads)
        q, k = self.norm(q, k, v)
...
class DoubleStreamBlockD(DoubleStreamBlock):
    ...
    def forward(
        ...
        if edit_length is not None:
            ...
            edit_q, edit_k, edit_v = rearrange(
                edit_qkv, "B L (K H D) -> K B H L D", K=3, H=self.num_heads
            )
        ...
        # run actual attention
        q = torch.cat(
            (txt_q,) + ((edit_q,) if edit_q is not None else ()) + (img_q,), dim=2
        )
        k = torch.cat(
            (txt_k,) + ((edit_k,) if edit_k is not None else ()) + (img_k,), dim=2
        )
        v = torch.cat(
            (txt_v,) + ((edit_v,) if edit_v is not None else ()) + (img_v,), dim=2
        )
        if mask is not None:
            mask = repeat(mask, "B L S->  B H L S", H=self.num_heads)
        attn = attention(q, k, v, pe=pe, mask=mask, backend=self.backend)
