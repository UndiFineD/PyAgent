# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\embedder.py
# -*- coding: utf-8 -*-
...
try:
    ...
    warnings.warn(f"Import transformers error, please deal with this problem: {e}")


@EMBEDDERS.register_class()
class ACEHFEmbedder(BaseEmbedder):
    para_dict = {
        "HF_MODEL_CLS": {"value": None, "description": "huggingface cls in transfomer"},
        "MODEL_PATH": {"value": None, "description": "model folder path"},
        "HF_TOKENIZER_CLS": {
            "value": None,
            "description": "huggingface cls in transfomer",
        },
        "TOKENIZER_PATH": {"value": None, "description": "tokenizer folder path"},
        "MAX_LENGTH": {"value": 77, "description": "max length of input"},
        "OUTPUT_KEY": {"value": "last_hidden_state", "description": "output key"},
        "D_TYPE": {"value": "float", "description": "dtype"},
    ...
    def __init__(self, cfg, logger=None):
        ...
        with FS.get_dir_to_local_dir(tokenizer_path, wait_finish=True) as local_path:
            self.tokenizer = getattr(transformers, hf_tokenizer_cls).from_pretrained(
                local_path,
        ...
        with FS.get_dir_to_local_dir(model_path, wait_finish=True) as local_path:
            self.hf_module = getattr(transformers, hf_model_cls).from_pretrained(
    ...
    def forward(self, text: list[str], return_mask=False):
        ...
        if return_mask:
            return outputs[self.output_key], batch_encoding["attention_mask"].to(
    ...
    def get_config_template():
        return dict_to_yaml(
            "EMBEDDER", __class__.__name__, ACEHFEmbedder.para_dict, set_name=True
        )
