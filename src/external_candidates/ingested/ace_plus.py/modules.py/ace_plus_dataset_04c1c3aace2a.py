# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\ace_plus_dataset.py
# -*- coding: utf-8 -*-
...
def ensure_src_align_target_h_mode(
    src_image, size, image_id, interpolation=InterpolationMode.BILINEAR
):
    # padding mode
    H, W = size
    ret_image = []
    for one_id in image_id:
        ...
        tH, tW = H, W
        ret_image.append(
            T.Resize((tH, tW), interpolation=interpolation, antialias=True)(edit_image)
        )
    return ret_image
...
def ensure_src_align_target_padding_mode(
    ...
    for idx, one_id in enumerate(image_id):
        ...
        edit_image = T.Resize((tH, tW), interpolation=interpolation, antialias=True)(
            edit_image
        )
        # padding
        delta_w = 0
        ...
        ret_data.append(
            T.Pad(padding, fill=0, padding_mode="constant")(edit_image).float()
        )
    return ret_data, ret_h


def ensure_limit_sequence(
    image, max_seq_len=4096, d=16, interpolation=InterpolationMode.BILINEAR
):
...
class ACEPlusDataset(BaseDataset):
    para_dict = {
        ...
        "D": {"value": 16, "description": "Patch size for resized image."},
    }
    ...
    def __init__(self, cfg, logger=None):
        ...
        self.items = self.read_data_list(
            delimiter, fields, prefix, edit_type_list, prompt_file
        )
    ...
    def _get(self, index):
        ...
        edit_image = (
            load_image(prefix, edit_image, cvt_type="RGB") if edit_image != "" else None
        )
        edit_mask = (
            load_image(prefix, edit_mask, cvt_type="L") if edit_mask != "" else None
        )
        ref_image = (
            load_image(prefix, ref_image, cvt_type="RGB") if ref_image != "" else None
        )
        target_image = (
            load_image(prefix, target_image, cvt_type="RGB")
            if target_image != ""
            else None
        )
        ...
        if edit_mask is not None:
            image_mask = transform_mask(
                torch.tensor(np.array(edit_mask).astype(np.float32))
            )
        else:
            image_mask = Image.new("L", target_image.size, 255)
            image_mask = transform_mask(
                torch.tensor(np.array(image_mask).astype(np.float32))
            )

        src_image_list = [
            transform_image(torch.tensor(np.array(im).astype(np.float32)))
            for im in src_image_list
        ]
        src_mask_list = [
            transform_mask(torch.tensor(np.array(im).astype(np.float32)))
            for im in src_mask_list
        ]
        ...
        for e_i in edit_id:
            src_image_list[e_i] = src_image_list[e_i] * (
                1 - repainting_scale * src_mask_list[e_i]
            )
        size = image.shape[1:]
        ref_image_list, ret_h = ensure_src_align_target_padding_mode(
            src_image_list,
            size,
            image_id=ref_id,
            interpolation=InterpolationMode.NEAREST_EXACT,
        )
        ref_mask_list, ret_h = ensure_src_align_target_padding_mode(
            src_mask_list,
            size,
            size_h=ret_h,
            image_id=ref_id,
        ...
        edit_image_list = ensure_src_align_target_h_mode(
            src_image_list,
            size,
            image_id=edit_id,
            interpolation=InterpolationMode.NEAREST_EXACT,
        )
        edit_mask_list = ensure_src_align_target_h_mode(
            src_mask_list,
            size,
            image_id=edit_id,
            interpolation=InterpolationMode.NEAREST_EXACT,
        ...
        image = ensure_limit_sequence(
            image,
            max_seq_len=self.max_seq_len,
            d=self.d,
            interpolation=InterpolationMode.NEAREST_EXACT,
        )
        image_mask = ensure_limit_sequence(
            image_mask,
            max_seq_len=self.max_seq_len,
            d=self.d,
            interpolation=InterpolationMode.NEAREST_EXACT,
        ...
        src_image_list = [
            ensure_limit_sequence(
                i,
                max_seq_len=self.max_seq_len,
                d=self.d,
                interpolation=InterpolationMode.NEAREST_EXACT,
            )
            for i in src_image_list
        ]
        src_mask_list = [
            ensure_limit_sequence(
                i,
            ...
            for i in src_mask_list
        ]

        if self.modify_mode:
            # To be modified regions according to mask
            modify_image_list = [
                ii * im for ii, im in zip(src_image_list, src_mask_list)
            ]
            # To be edited regions according to mask
            src_image_list = [
                ii * (1 - im) for ii, im in zip(src_image_list, src_mask_list)
            ]
        else:
