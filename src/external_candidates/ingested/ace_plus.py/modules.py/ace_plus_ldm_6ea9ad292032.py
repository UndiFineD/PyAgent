# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\modules\ace_plus_ldm.py
# -*- coding: utf-8 -*-
...
from scepter.modules.model.registry import (
    BACKBONES,
    DIFFUSIONS,
    EMBEDDERS,
    LOSSES,
...
class LatentDiffusionACEPlus(LatentDiffusion):
    ...
    def init_params(self):
        ...
        self.default_n_prompt = (
            "" if self.default_n_prompt is None else self.default_n_prompt
        )
    ...
    def construct_network(self):
        ...
        self.model = BACKBONES.build(self.model_config, logger=self.logger).to(
            torch.bfloat16
        )
        self.logger.info("all parameters:{}".format(count_params(self.model)))
        if self.use_ema:
            if self.model_ema_config:
                self.model_ema = BACKBONES.build(
                    self.model_ema_config, logger=self.logger
                )
            else:
        ...
        if self.tokenizer_config is not None:
            self.tokenizer = TOKENIZERS.build(self.tokenizer_config, logger=self.logger)
        if self.first_stage_config:
            self.first_stage_model = MODELS.build(
                self.first_stage_config, logger=self.logger
            )
        ...
        if self.tokenizer_config is not None:
            self.cond_stage_config.KWARGS = {"vocab_size": self.tokenizer.vocab_size}
    ...
    def parse_ref_and_edit(
        ...
        for sample_id, (
            ...
            if hasattr(self, "ref_cond_stage_model") and self.ref_cond_stage_model:
                ref_image.append(
                    self.ref_cond_stage_model.encode_list(
                        [((i + 1.0) / 2.0 * 255).type(torch.uint8) for i in one_src]
                    )
                )
            else:
            ...
            current_edit_image = to_device(
                [one_src[i] for i in one_edit_id], strict=False
            )
            current_edit_image = [
                v.squeeze(0) for v in self.encode_first_stage(current_edit_image)
            ]
            # process modi image
            current_modify_image = to_device(
                [one_modify[i] for i in one_edit_id], strict=False
            )
            current_modify_image = [
                v.squeeze(0) for v in self.encode_first_stage(current_modify_image)
            ]
            current_edit_image_mask = to_device(
                [one_src_mask[i] for i in one_edit_id], strict=False
            )
            current_edit_image_mask = [
                self.reshape_func(m).squeeze(0) for m in current_edit_image_mask
            ]
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        if check_list_of_list(image):
            ...
            x_mask = [
                (
                    [self.reshape_func(i).squeeze(0) for i in im]
                    if im is not None
                    else [None] * len(ix)
                )
            ...
            x_mask = (
                [self.reshape_func(i).squeeze(0) for i in image_mask]
        ...
        ref_edit_context = self.parse_ref_and_edit(
            src_image_list, modify_image_list, src_mask_list, context, edit_id
        )
        ...
        loss = self.diffusion.loss(
            x_0=x_start,
            model=self.model,
            model_kwargs={"cond": context, "gc_seg": gc_seg, "guidance": guide_scale},
            noise=noise,
            reduction="none",
            **kwargs,
        )
    ...
    def forward_editing(
        ...
        **kwargs,
        ...
        # gc_seg is unused
        prompt, image, image_mask, src_image, modify_image, src_image_mask, edit_id = (
            limit_batch_data(
                [
                    prompt,
                    image,
        ...
        if image is not None:
            ...
            noise = [
                self.noise_sample(1, ix.shape[1], ix.shape[2], seed) for ix in image
            ]
        else:
        ...
        context["x_mask"] = x_mask
        ref_edit_context = self.parse_ref_and_edit(
            src_image, modify_image, src_image_mask, context, edit_id
        )
        context.update(ref_edit_context)
        ...
        for i in range(len(prompt)):
            ...
            if src_image is not None and src_image[i] is not None:
                ...
                for edit_img, modify_img, edit_mask in zip(
                    src_image[i], modify_image_list[i], src_image_mask[i]
                ):
                    edit_img = torch.clamp(
                        (edit_img.float() + 1.0) / 2.0, min=0.0, max=1.0
                    )
                    edit_imgs.append(edit_img.squeeze(0))
                    modify_img = torch.clamp(
                        (modify_img.float() + 1.0) / 2.0, min=0.0, max=1.0
                    )
            ...
            if image is not None:
                ...
                one_tup["target_mask"] = (
                    image_mask[i]
                    if image_mask[i] is not None
                    else torch.ones_like(ori_img[[0], :, :])
                )
    ...
    def get_config_template():
        return dict_to_yaml(
            "MODEL", __class__.__name__, LatentDiffusionACEPlus.para_dict, set_name=True
        )
