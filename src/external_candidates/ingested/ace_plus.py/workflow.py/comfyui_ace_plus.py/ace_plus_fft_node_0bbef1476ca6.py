# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\workflow\ComfyUI-ACE_Plus\ace_plus_fft_node.py
# -*- coding: utf-8 -*-
...
class ACEPlusFFTConditioning:
        ...
        return {
            "required": {
                ...
                "noise_mask": (
                    ...
                    {
                        "default": True,
                        "tooltip": "Add a noise mask to the latent "
    ...
    def encode(self, positive, negative, vae, ucpixels, cpixels, mask, noise_mask=True):
        x = (ucpixels.shape[1] // 8) * 8
        y = (ucpixels.shape[2] // 8) * 8
        mask = torch.nn.functional.interpolate(
            mask.reshape((-1, 1, mask.shape[-2], mask.shape[-1])),
            size=(ucpixels.shape[1], ucpixels.shape[2]),
            mode="bilinear",
        )
            ...
            c = node_helpers.conditioning_set_values(
                conditioning,
                {
                    "concat_latent_image": torch.cat(
                        [concat_latent, c_concat_latent], dim=1
                    ),
                    "concat_mask": mask,
                },
            )
...
class AcePlusFFTProcessor:
    def __init__(self, max_aspect_ratio=4, d=16, max_seq_len=1024):
        ...
        self.transforms = T.Compose(
            [T.ToTensor(), T.Normalize(mean=[0, 0, 0], std=[1.0, 1.0, 1.0])]
        )
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "height": (
                    "INT",
                    {"default": 1024, "min": 256, "max": 1436, "step": 16},
                ),
                "width": (
                    "INT",
                    {"default": 1024, "min": 256, "max": 1436, "step": 16},
                ...
                "keep_pixels_rate": (
                    "FLOAT",
                    {"default": 0.8, "min": 0, "max": 1, "step": 0.01},
                ),
                "max_seq_length": (
                    "INT",
                    {"default": 3072, "min": 1024, "max": 5120, "step": 0.01},
                ),
            },
            "optional": {
    ...
    def edit_preprocess(self, processor, device, edit_image, edit_mask):
        ...
        if not SCEPTER:
            raise ImportError(
                f"Please install scepter to use edit processor {processor} by "
        ...
        edit_image = Image.fromarray(
            np.array(edit_image[0] * 255).astype(np.uint8)
        ).convert("RGB")
        ...
        if edit_mask is not None:
            ...
            edit_mask = Image.fromarray(
                np.array(edit_mask[0]).astype(np.uint8)
            ).convert("L")

        if new_edit_image.size != edit_image.size:
            edit_image = T.Resize(
                (edit_image.size[1], edit_image.size[0]),
    ...
    def preprocess(
        ...
        if task_type in self.task_list:
            edit_image = self.edit_preprocess(
                self.task_list[task_type]["ANNOTATOR"], 0, edit_image, edit_mask
            )
        ...
        if edit_image is None:
            ...
            edit_mask = (
                edit_mask.astype(np.float32)
                if np.any(edit_mask)
                else np.ones_like(edit_mask).astype(np.float32)
            )
        ...
        if reference_image is not None:
            ...
            if not True:
                ...
                reference_image = T.Resize(
                    (tH, tW), interpolation=T.InterpolationMode.BILINEAR, antialias=True
                )(reference_image)
            else:
                # padding
                ...
                reference_image = T.Pad(padding, fill=0, padding_mode="constant")(
                    reference_image
                )
            ...
            edit_mask = torch.cat(
                [
                    torch.zeros(
                        [1, reference_image.shape[1], reference_image.shape[2]]
                    ),
                    edit_mask,
                ],
                dim=-1,
        ...
        edit_image = T.Resize(
            (rH, rW), interpolation=T.InterpolationMode.NEAREST_EXACT, antialias=True
        )(edit_image)
        edit_mask = T.Resize(
            (rH, rW), interpolation=T.InterpolationMode.NEAREST_EXACT, antialias=True
        )(edit_mask)
...
class AcePlusLoraProcessor:
    ...
    def preprocess(
        ...
        H, W = edit_image.shape[-2:]
        scale = min(
            1.0, math.sqrt(self.max_seq_len * 2 / ((H / self.d) * (W / self.d)))
        )
        rH = int(H * scale) // self.d * self.d
        rW = int(W * scale) // self.d * self.d
        slice_w = int(slice_w * scale) // self.d * self.d
