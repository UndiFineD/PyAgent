# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\infer_fft.py
# -*- coding: utf-8 -*-
...
def run_one_case(
    ...
    **kwargs,
    ...
    if input_reference_image is not None:
        input_reference_image = Image.open(
            io.BytesIO(FS.get_object(input_reference_image))
        )
...
def run():
    ...
    parser.add_argument(
        "--input_image", dest="input_image", help="The input image!", default=None
    )
    parser.add_argument(
        "--input_mask", dest="input_mask", help="The input mask!", default=None
    )
    parser.add_argument(
        "--save_path",
        dest="save_path",
    ...
    parser.add_argument(
        "--seed", dest="seed", help="The seed for generation!", type=int, default=-1
    )
