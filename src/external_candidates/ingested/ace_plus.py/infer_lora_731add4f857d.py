...
def run():
    ...
    if (
        ...
        for example in all_examples:
            example["model_path"] = FS.get_from(
                task_model_dict[example["task_type"].upper()]["MODEL_PATH"]
            )
            example.update(params)
            if example["edit_type"] == "repainting":
                example["repainting_scale"] = 1.0
            else:
                example["repainting_scale"] = task_model_dict[
                    example["task_type"].upper()
                ].get("REPAINTING_SCALE", 1.0)
            print(example)
        ...
        params = {
            ...
            "model_path": FS.get_from(
                task_model_dict[cfg.args.task_type.upper()]["MODEL_PATH"]
            ),
        }
