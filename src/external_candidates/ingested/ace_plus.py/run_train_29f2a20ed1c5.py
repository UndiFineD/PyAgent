# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\run_train.py
# -*- coding: utf-8 -*-
...
def run_task(cfg):
    ...
    if we.rank == 0:
        FS.put_object_from_local_file(
            cfg.args.cfg_file, os.path.join(solver.work_dir, "train.yaml")
        )
...
def update_config(cfg):
    ...
    if hasattr(cfg.args, "max_steps") and cfg.args.max_steps:
        print(f"max_steps change from {cfg.SOLVER.MAX_STEPS} to {cfg.args.max_steps}")
        cfg.SOLVER.MAX_STEPS = int(cfg.args.max_steps)
    cfg.SOLVER.WORK_DIR = os.path.join(
        cfg.SOLVER.WORK_DIR, "{0:%Y%m%d%H%M%S}".format(datetime.now())
    )
    return cfg
