# Extracted from: C:\DEV\PyAgent\.external\ACE_plus\demo_fft.py
# -*- coding: utf-8 -*-
...
fs_list = [
    Config(
        cfg_dict={"NAME": "HuggingfaceFs", "TEMP_DIR": os.environ["TEMP_DIR"]},
        load=False,
    ),
    Config(
        cfg_dict={"NAME": "ModelscopeFs", "TEMP_DIR": os.environ["TEMP_DIR"]},
        load=False,
    ),
    Config(cfg_dict={"NAME": "HttpFs", "TEMP_DIR": os.environ["TEMP_DIR"]}, load=False),
    Config(
        cfg_dict={"NAME": "LocalFs", "TEMP_DIR": os.environ["TEMP_DIR"]}, load=False
    ),
    Config(
        cfg_dict={"NAME": "LocalFs", "TEMP_DIR": os.environ["TEMP_DIR"]}, load=False
    ),
...
class DemoUI(object):
    def __init__(self, infer_dir="./config/ace_plus_fft.yaml"):
    ...
    def create_ui(self):
        ...
        with gr.Row():
            ...
            self.edit_type = gr.Dropdown(
                choices=self.default_type_list,
                interactive=True,
        ...
        with gr.Row():
            ...
            self.seed = gr.Slider(
                minimum=-1, maximum=1000000000000, value=-1, label="Seed"
            )
    ...
    def set_callbacks(self, *args, **kwargs):
        ...
        def run_chat(
            ...
            pre_edit_image = edit_preprocess(
                edit_info.ANNOTATOR, we.device_id, pre_edit_image, pre_edit_mask
            )
