# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\other\translation\multi-user-translator.py
...
class RoomTranslator:
    ...
    def __init__(
        self, room: rtc.Room, *, additional_languages: list[str] | None = None
    ):
        ...
        self.additional_languages = (
            list(additional_languages) if additional_languages else []
        )
    ...
    def start(self):
        ...
        existing_track_ids = [
            input_track.track.sid for input_track in self.input_tracks
        ]
    ...
    def _remove_track(self, track: rtc.RemoteAudioTrack):
        input_track = next(
            (t for t in self.input_tracks if t.track.sid == track.sid), None
        )
        if input_track:
