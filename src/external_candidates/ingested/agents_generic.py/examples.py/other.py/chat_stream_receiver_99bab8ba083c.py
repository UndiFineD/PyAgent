# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\other\chat-stream-receiver.py
import asyncio
import logging
import os
import sys
...
class TextStreamPrinter:
    ...
    def on_text_received(self, reader: rtc.TextStreamReader, participant_identity: str):
        async def _on_text_received():
            ...
            segment_id = reader.info.attributes.get(
                ATTRIBUTE_TRANSCRIPTION_SEGMENT_ID, None
            )
            ...
            track_id = reader.info.attributes.get(
                ATTRIBUTE_TRANSCRIPTION_TRACK_ID, None
            )
