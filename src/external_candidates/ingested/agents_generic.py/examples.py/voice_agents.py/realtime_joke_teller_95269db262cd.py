...
class Assistant(Agent):
    def __init__(self, tools: list[llm.FunctionTool | llm.RawFunctionTool]) -> None:
        ...
        chat_ctx.add_message(
            role="user", content="hey sonic, tell me a children's story"
        )
    ...
    @function_tool
    async def get_user_name_and_age(
        self, context: RunContext[MySessionInfo]
    ) -> dict[str, Any]:
        """
...
async def entrypoint(ctx: agents.JobContext):
    ...
    try:
        ...
        while True:
            ...
            room_empty_future: asyncio.Future[None] = (
                asyncio.get_running_loop().create_future()
            )

            def _on_participant_disconnected(
                _: rtc.Participant, fut=room_empty_future
            ) -> None:
