# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\voice_agents\timed_agent_transcript.py
import asyncio
import logging
    ...
    async def transcription_node(
        ...
        async for chunk in text:
            if isinstance(chunk, TimedString):
                logger.info(
                    f"TimedString: '{chunk}' ({chunk.start_time} - {chunk.end_time})"
                )
            yield chunk
