# Extracted from: C:\DEV\PyAgent\.external\agents_generic\examples\voice_agents\resume_interrupted_agent.py
import logging
...
async def entrypoint(ctx: JobContext):
    ...
    def _agent_false_interruption(ev: AgentFalseInterruptionEvent):
        logger.info(
            ...
            extra={"instructions": ev.extra_instructions},
        )
        session.generate_reply(instructions=ev.extra_instructions or NOT_GIVEN)

    await session.start(
        agent=Agent(instructions="You are a helpful assistant."), room=ctx.room
    )
