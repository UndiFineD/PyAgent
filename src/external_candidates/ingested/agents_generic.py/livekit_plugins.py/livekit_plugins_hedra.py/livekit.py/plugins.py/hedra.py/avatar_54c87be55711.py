# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-hedra\livekit\plugins\hedra\avatar.py
...
class AvatarSession:
    ...
    async def _start_agent(self, livekit_url: str, livekit_token: str) -> None:
        ...
        data = aiohttp.FormData(
            {"livekit_url": livekit_url, "livekit_token": livekit_token}
        )
        ...
        for i in range(self._conn_options.max_retry):
            try:
                ...
                if isinstance(e, APIConnectionError):
                    logger.warning(
                        "failed to call hedra avatar api", extra={"error": str(e)}
                    )
                else:
        ...
        raise APIConnectionError(
            "Failed to start Hedra Avatar Session after all retries"
        )
