...
class SonicEventBuilder:
    ...
    def create_prompt_start_block(
        ...
        tool_configuration: Optional[
            Union[ToolConfiguration, dict[str, Any], str]
        ] = None,
        max_tokens: int = 1024,
        top_p: float = 0.9,
        ...
        init_events = [
            ...
            *self.create_text_content_block(
                system_content_name, "SYSTEM", system_content
            ),
        ]
        ...
        event = Event(
            event=InputContentEndEvent(
                contentEnd=ContentEndEvent(
                    promptName=self.prompt_name,
                    contentName=(
                        content_name if not is_audio else self.audio_content_name
                    ),
                )
            )
        )
    ...
    def create_prompt_start_event(
        ...
        if tool_configuration is None:
            ...
            tool_configuration = ToolConfiguration.model_validate_json(
                tool_configuration
            )
