# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-aws\livekit\plugins\aws\experimental\realtime\realtime_model.py
...
class RealtimeSession(  # noqa: F811
    llm.RealtimeSession[
        Literal["bedrock_server_event_received", "bedrock_client_event_queued"]
    ]
):
    ...
    def __init__(self, realtime_model: RealtimeModel) -> None:
        ...
        self._bstream = utils.audio.AudioByteStream(
            ...
            DEFAULT_CHANNELS,
            samples_per_channel=DEFAULT_CHUNK_SIZE,
        )
    ...
    async def _send_raw_event(self, event_json: str) -> None:
        ...
        if not self._stream_response:
            logger.warning(
                "stream not initialized; dropping event (this should never occur)"
            )
            return
        ...
        if self.tools.function_tools:
            ...
            for name, f in self.tools.function_tools.items():
                if llm.tool_context.is_function_tool(f):
                    ...
                    description = llm.tool_context.get_raw_function_info(
                        f
                ...
                tools.append(tool)
            tool_choice = self._tool_choice_adapter(
                self._realtime_model._opts.tool_choice
            )
        ...
        try:
                ...
                try:
                    if pending_events:
                        await asyncio.wait_for(
                            asyncio.gather(*pending_events), timeout=0.5
                        )
                except asyncio.TimeoutError:
                    if not self._tools_ready.done():
                        logger.warning(
                            "Tools not ready after 500ms, continuing without them"
                        )
            ...
            if not is_restart:
                self._audio_input_task = asyncio.create_task(
                    ...
                    name="RealtimeSession._process_audio_input",
                )
    ...
    def _create_response_generation(self) -> None:
        ...
        if self._current_generation is None:
            ...
            self._current_generation.messages[self._current_generation.response_id] = (
                msg_gen
            )
    ...
    async def _handle_text_output_content_start_event(self, event_data: dict) -> None:
        ...
        if role == "USER":
            ...
            self._current_generation.user_messages[content_id] = (
            ...
            role == "ASSISTANT"
            and "SPECULATIVE"
            in event_data["event"]["contentStart"]["additionalModelFields"]
        ):
            assert self._current_generation is not None, "current_generation is None"
    ...
    async def _handle_text_output_content_event(self, event_data: dict) -> None:
        ...
        if self._current_generation is not None:
            ...
            if (
                ...
                curr_gen = self._current_generation.messages[
                    self._current_generation.response_id
                ]
    ...
    def _update_chat_ctx(self, role: llm.ChatRole, text_content: str) -> None:
        ...
        """
        logger.debug(
            f"Updating chat context with role: {role} and text_content: {text_content}"
        )
    ...
    def _close_current_generation(self) -> None:
        ...
        if self._current_generation is not None:
            if (
                self._current_generation.response_id
                in self._current_generation.messages
            ):
                curr_gen = self._current_generation.messages[
                    self._current_generation.response_id
                ]
        ...
        try:
            ...
            while self._is_sess_active.is_set():
                ...
                try:
                    ...
                    err_msg = getattr(e, "message", str(e))
                    logger.error(
                        f"Response processing error: {err_msg} (type: {type(e)})"
                    )
                    request_id = None
                    try:
                        request_id = err_msg.split(" ")[0].split("=")[1]
    ...
    async def update_tools(
        self, tools: list[llm.FunctionTool | llm.RawFunctionTool | Any]
    ) -> None:
    ...
    def _resample_audio(self, frame: rtc.AudioFrame) -> Iterator[rtc.AudioFrame]:
        ...
        if self._input_resampler is None and (
            frame.sample_rate != DEFAULT_INPUT_SAMPLE_RATE
            or frame.num_channels != DEFAULT_CHANNELS
        ):
    ...
    async def _process_audio_input(self) -> None:
        ...
        await self._send_raw_event(
            self._event_builder.create_audio_content_start_event()
        )
