        ...
        *,
        ...
        if self._mode == "local":
            ...
            if not utils.is_given(avatar_image) and not utils.is_given(avatar_id):
                raise BitHumanException(
                    "`avatar_image` or `avatar_id` must be set for cloud mode"
                )
            if self._api_secret is None:
                raise BitHumanException(
                    "BITHUMAN_API_SECRET are required for cloud mode"
                )
    ...
    async def _start_cloud_agent(
        ...
        json_data = {
            ...
            "mode": (
                "gpu"
                if (
                    utils.is_given(self._avatar_image)
                    and self._avatar_image is not None
                )
        ...
        if isinstance(self._avatar_image, Image.Image):
            ...
            json_data["image"] = base64.b64encode(img_byte_arr.getvalue()).decode(
                "utf-8"
            )
        ...
        for i in range(self._conn_options.max_retry):
            try:
                ...
                if isinstance(e, APIConnectionError):
                    logger.warning(
                        "failed to call bithuman avatar api", extra={"error": str(e)}
                    )
                else:
