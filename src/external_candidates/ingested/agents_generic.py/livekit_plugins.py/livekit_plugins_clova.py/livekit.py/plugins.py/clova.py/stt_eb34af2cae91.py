# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-clova\livekit\plugins\clova\stt.py
...
class STT(stt.STT):
    def __init__(
        ...
        *,
        ...
        clova_secret = (
            secret if is_given(secret) else os.environ.get("CLOVA_STT_SECRET_KEY")
        )
        self._invoke_url = (
            invoke_url
            if is_given(invoke_url)
            else os.environ.get("CLOVA_STT_INVOKE_URL")
        )
