# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-azure\livekit\plugins\azure\tts.py
...
class ChunkedStream(tts.ChunkedStream):
    ...
    def _build_ssml(self) -> str:
        ...
        if is_given(self._opts.prosody):
            ...
            ssml += (
                f"<prosody{rate_attr}{vol_attr}{pitch_attr}>{self.input_text}</prosody>"
            )
        else:
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        headers = {
            ...
            "X-Microsoft-OutputFormat": SUPPORTED_OUTPUT_FORMATS[
