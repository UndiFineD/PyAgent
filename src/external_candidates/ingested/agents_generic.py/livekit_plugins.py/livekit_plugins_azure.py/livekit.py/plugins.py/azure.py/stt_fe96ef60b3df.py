# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-azure\livekit\plugins\azure\stt.py
...
class STT(stt.STT):
    def __init__(
        ...
        *,
        ...
        if speech_region and speech_endpoint:
            logger.warning(
                "speech_region and speech_endpoint both are set, using speech_endpoint"
            )
    ...
    def update_options(
        self, *, language: NotGivenOr[list[str] | str] = NOT_GIVEN
    ) -> None:
        if is_given(language):
            if isinstance(language, str):
                language = [language]
    ...
    async def _run(self) -> None:
        while True:
            ...
            try:
                ...
                wait_stopped_task = asyncio.create_task(
                    self._session_stopped_event.wait()
                )
...
def _create_speech_recognizer(
    *, config: STTOptions, stream: speechsdk.audio.AudioInputStream
    ...
    speech_config = speechsdk.SpeechConfig(
        ...
        auth_token=(
            config.speech_auth_token if is_given(config.speech_auth_token) else None
        ),
    )
    ...
    if (
        is_given(config.phrase_list)
        and isinstance(config.phrase_list, list)
        and config.phrase_list
    ):
