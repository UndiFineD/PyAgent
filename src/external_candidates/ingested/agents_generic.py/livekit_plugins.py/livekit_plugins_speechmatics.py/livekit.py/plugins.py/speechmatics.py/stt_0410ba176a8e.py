# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-speechmatics\livekit\plugins\speechmatics\stt.py
...
class STTOptions:
    ...
    known_speakers: list[DiarizationKnownSpeaker] = dataclasses.field(
        default_factory=list
    )
    ...
    def __init__(
        ...
        *,
        ...
        if is_given(transcription_config):
            ...
            output_locale = (
                output_locale if is_given(output_locale) else config.output_locale
            )
        ...
        self._stt_options = STTOptions(
            ...
            punctuation_overrides=(
                punctuation_overrides if is_given(punctuation_overrides) else {}
            ),
    ...
    def _process_config(self) -> None:
        ...
        if self._stt_options.enable_diarization:
            ...
            if self._stt_options.diarization_sensitivity is not None:
                dz_cfg["speaker_sensitivity"] = (
                    self._stt_options.diarization_sensitivity
                )
            if self._stt_options.prefer_current_speaker is not None:
                dz_cfg["prefer_current_speaker"] = (
                    self._stt_options.prefer_current_speaker
                )
            if self._stt_options.known_speakers:
                dz_cfg["speakers"] = {
                    s.label: s.speaker_identifiers
                    for s in self._stt_options.known_speakers
                }
            if dz_cfg:
        ...
        if self._stt_options.punctuation_overrides:
            transcription_config.punctuation_overrides = (
                self._stt_options.punctuation_overrides
            )
...
class SpeechStream(stt.RecognizeStream):
    ...
    def _end_of_utterance_timer_start(self) -> None:
        ...
        self._end_of_utterance_timer = asyncio.get_event_loop().call_later(
            delay, send_after_delay
        )
    ...
    def _add_speech_fragments(
        self, message: dict[str, Any], is_final: bool = False
    ) -> bool:
        ...
        for result in message.get("results", []):
            ...
            if alt.get("content", None):
                ...
                if fragment.speaker:
                    ...
                    if (
                        opts.ignore_speakers
                        and fragment.speaker in opts.ignore_speakers
                    ):
                        continue
        ...
        self._speech_fragments = [
            frag for frag in self._speech_fragments if frag.is_final
        ]
