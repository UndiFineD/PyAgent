# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-upliftai\livekit\plugins\upliftai\tts.py
"""
...
class WebSocketClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.sio.on(
                "disconnect", self._on_disconnect, namespace=WEBSOCKET_NAMESPACE
            )
    ...
    async def synthesize(
        ...
        logger.debug(
            f"Sending synthesis request {request_id[:8]} for text: '{text[:50]}...'"
        )
...
class ChunkedStream(tts.ChunkedStream):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        ...
        try:
            ...
            # Get audio queue
            audio_queue = await self._tts._client.synthesize(
                self._input_text, request_id
            )
...
class SynthesizeStream(tts.SynthesizeStream):
    ...
    def __init__(self, *, tts: TTS, conn_options: APIConnectOptions):
        ...
        self._segments_ch = utils.aio.Chan[
            tokenize.WordStream | tokenize.SentenceStream
        ]()
