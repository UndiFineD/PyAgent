...
class STT(stt.STT):
    def __init__(
        ...
        *,
        ...
        assemblyai_api_key = (
            api_key if is_given(api_key) else os.environ.get("ASSEMBLYAI_API_KEY")
        )
...
class SpeechStream(stt.SpeechStream):
    ...
    async def _connect_ws(self) -> aiohttp.ClientWebSocketResponse:
        live_config = {
            ...
            "format_turns": (
                self._opts.format_turns if is_given(self._opts.format_turns) else None
            ),
            "end_of_turn_confidence_threshold": (
                self._opts.end_of_turn_confidence_threshold
    ...
    def _process_stream_event(self, data: dict) -> None:
        ...
        if message_type == "Turn":
            ...
            if transcript and end_of_turn:
                ...
                if not self._opts.format_turns or (
                    self._opts.format_turns and turn_is_formatted
                ):
                    final_event = stt.SpeechEvent(
                        type=stt.SpeechEventType.FINAL_TRANSCRIPT,
                        # TODO: We can't know the language?
                        alternatives=[
                            stt.SpeechData(language="en-US", text=transcript)
                        ],
                    )
                else:
                ...
                non_final_words = [
                    word["text"] for word in words if not word["word_is_final"]
                ]
                interim = " ".join(non_final_words)
                interim_event = stt.SpeechEvent(
                    type=stt.SpeechEventType.INTERIM_TRANSCRIPT,
                    alternatives=[
                        stt.SpeechData(language="en-US", text=f"{transcript} {interim}")
                    ],
                )
