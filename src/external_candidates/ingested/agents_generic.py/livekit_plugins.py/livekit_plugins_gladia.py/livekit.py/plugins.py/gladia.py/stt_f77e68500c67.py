# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-gladia\livekit\plugins\gladia\stt.py
...
class STT(stt.STT):
    def __init__(
        ...
        *,
        ...
        language_config = LanguageConfiguration(
            languages=languages, code_switching=code_switching
        )
    ...
    async def _recognize_impl(
        ...
        *,
        ...
        config = self._sanitize_options(
            languages=[language] if is_given(language) else None
        )
        streaming_config = _build_streaming_config(config)

        try:
            # Initialize a session with Gladia
            session_response = await self._init_live_session(
                streaming_config, conn_options
            )
            ...
            async with self._ensure_session().ws_connect(
                ...
                timeout=aiohttp.ClientWSTimeout(
                    ws_receive=receive_timeout, ws_close=10
                ),
            ) as ws:
                ...
                bytes_per_second = (
                    config.sample_rate * config.channels * (config.bit_depth // 8)
                )
                ...
                for i in range(0, len(pcm_data), chunk_size):
                    ...
                    await ws.send_str(
                        json.dumps(
                            {"type": "audio_chunk", "data": {"chunk": chunk_b64}}
                        )
                    )
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            # Check for translation as the final result if enabled
                                ...
                                and config.translation_config.enabled
                            ):
                                pass
                            ...
                            logger.error(
                                f"Gladia WebSocket connection error: {ws.exception()}"
                            )
            ...
            raise APITimeoutError(
                "Timeout connecting to or initializing Gladia session"
            ) from e
        except aiohttp.ClientResponseError as e:
            # Error during session initialization POST request
            logger.error(
                f"Gladia API status error during session init: {e.status} {e.message}"
            )
    ...
    async def _init_live_session(
        self, config: dict, conn_options: APIConnectOptions
    ) -> dict:
        ...
        try:
            ...
            raise APIConnectionError(
                f"Failed to initialize Gladia session: {str(e)}"
            ) from e
        ...
        *,
        ...
        if (
            ...
            translation_config = dataclasses.replace(
                ...
                context_adaptation=(
                    translation_context_adaptation
        ...
        if (
            pre_processing_audio_enhancer is not None
            or pre_processing_speech_threshold is not None
        ):
...
class SpeechStream(stt.SpeechStream):
        ...
        while True:
            try:
                ...
                if e.status_code == 429:
                    ...
                    raise APIStatusError(
                        f"Error in speech stream: {e}", retryable=True
                    ) from e
    ...
    async def _init_live_session(self) -> dict:
        ...
        try:
            ...
            url = (
                f"{self._base_url}?{urlencode({'region': streaming_config['region']})}"
            )
            streaming_config = {
                k: v for k, v in streaming_config.items() if k != "region"
            }
