# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-hume\livekit\plugins\hume\tts.py
...
class TTS(tts.TTS):
    def __init__(
        ...
        *,
        ...
        if not key:
            raise ValueError(
                "Hume API key is required via api_key or HUME_API_KEY env var"
            )
        ...
        if not is_given(instant_mode):
            ...
            raise ValueError(
                "Hume TTS: instant_mode cannot be enabled without specifying a voice"
            )
        else:
