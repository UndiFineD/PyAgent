# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-google\livekit\plugins\google\tts.py
    ...
    def __init__(
        ...
        *,
        ...
        ssml_gender = _gender_from_str(
            DEFAULT_GENDER if not is_given(gender) else gender
        )
        ...
        if is_given(voice_cloning_key):
            ...
            voice_params.name = (
                voice_name if is_given(voice_name) else DEFAULT_VOICE_NAME
            )
        ...
        pronunciations = (
            None if not is_given(custom_pronunciations) else custom_pronunciations
        )
