# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-google\livekit\plugins\google\beta\realtime\realtime_api.py
...
class _RealtimeOptions:
    ...
    context_window_compression: NotGivenOr[types.ContextWindowCompressionConfig] = (
        NOT_GIVEN
    )
...
class RealtimeSession(llm.RealtimeSession):
    def __init__(self, realtime_model: RealtimeModel) -> None:
        ...
        if not api_version and (
            self._opts.enable_affective_dialog or self._opts.proactivity
        ):
            api_version = "v1alpha"
    ...
    async def update_instructions(self, instructions: str) -> None:
        if (
            not is_given(self._opts.instructions)
            or self._opts.instructions != instructions
        ):
        ...
        for f in self._resample_audio(frame):
            for nf in self._bstream.write(f.data.tobytes()):
                realtime_input = types.LiveClientRealtimeInput(
                    media_chunks=[
                        types.Blob(data=nf.data.tobytes(), mime_type="audio/pcm")
                    ]
                )
    ...
    def generate_reply(
        ...
        if is_given(instructions):
            turns.append(
                types.Content(parts=[types.Part(text=instructions)], role="model")
            )
    ...
    async def aclose(self) -> None:
        ...
        for fut in self._response_created_futures.values():
            if not fut.done():
                fut.set_exception(
                    llm.RealtimeError("Session closed before response created")
                )
        ...
        while not self._msg_ch.closed:
            ...
            try:
                ...
                async with self._client.aio.live.connect(
                    ...
                    for task in done:
                        if task is not restart_wait_task and task.exception():
                            logger.error(
                                f"error in task {task.get_name()}: {task.exception()}"
                            )
                            raise task.exception() or Exception(
                                f"{task.get_name()} failed"
                            )
                ...
                if not self._msg_ch.closed:
                    ...
                    retry_interval = self._opts.conn_options._interval_for_retry(
                        self._num_retries
                    )
                    logger.warning(
        ...
        try:
            async for msg in self._msg_ch:
                ...
                if isinstance(msg, types.LiveClientContent):
                    ...
                    isinstance(msg, types.LiveClientToolResponse)
                    and msg.function_responses
                ):
            ...
            while True:
                async with self._session_lock:
                    if self._session_should_close.is_set() or (
                        ...
                        logger.debug(
                            "receive task: Session changed or closed, stopping receive."
                        )
                        break
    ...
    def _build_connect_config(self) -> types.LiveConnectConfig:
        ...
        tools_config = create_tools_config(
            ...
            gemini_tools=(
                self._opts.gemini_tools if is_given(self._opts.gemini_tools) else None
            ),
        )
        conf = types.LiveConnectConfig(
            ...
            generation_config=types.GenerationConfig(
                ...
                presence_penalty=(
                    self._opts.presence_penalty
                    if is_given(self._opts.presence_penalty)
                    else None
                ),
                frequency_penalty=(
                    self._opts.frequency_penalty
                    if is_given(self._opts.frequency_penalty)
                    else None
                ),
            ),
            system_instruction=(
                types.Content(parts=[types.Part(text=self._opts.instructions)])
            ...
            speech_config=types.SpeechConfig(
                ...
                language_code=(
                    self._opts.language if is_given(self._opts.language) else None
                ),
            ),
        ...
        if is_given(self._opts.proactivity):
            conf.proactivity = types.ProactivityConfig(
                proactive_audio=self._opts.proactivity
            )
                ...
                if part.inline_data:
                    ...
                    try:
                        ...
                        frame = rtc.AudioFrame(
                            ...
                            // (2 * OUTPUT_AUDIO_CHANNELS),
                        )
                        current_gen.audio_ch.send_nowait(frame)
                    except ValueError as e:
                        logger.error(
                            f"Error creating audio frame from Gemini data: {e}"
                        )
    ...
    def _handle_usage_metadata(self, usage_metadata: types.UsageMetadata) -> None:
        ...
        metrics = RealtimeModelMetrics(
            ...
            tokens_per_second=(
                (usage_metadata.response_token_count or 0) / duration
                if duration > 0
                else 0
            ),
