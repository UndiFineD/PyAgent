# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-google\livekit\plugins\google\llm.py
...
from livekit.agents.llm import FunctionTool, RawFunctionTool, ToolChoice
...
class LLM(llm.LLM):
        ...
        if is_given(thinking_config):
            ...
            if _thinking_budget is not None:
                if not isinstance(_thinking_budget, int):
                    raise ValueError(
                        "thinking_budget inside thinking_config must be an integer"
                    )
    ...
    def chat(
        ...
        *,
        ...
        tool_choice = (
            cast(ToolChoice, tool_choice)
            if is_given(tool_choice)
            else self._opts.tool_choice
        )
        if is_given(tool_choice):
        ...
        gemini_tools = (
            gemini_tools if is_given(gemini_tools) else self._opts.gemini_tools
        )
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            tools_config = create_tools_config(
                ...
                gemini_tools=(
                    self._gemini_tools if is_given(self._gemini_tools) else None
                ),
            )
            if tools_config:
