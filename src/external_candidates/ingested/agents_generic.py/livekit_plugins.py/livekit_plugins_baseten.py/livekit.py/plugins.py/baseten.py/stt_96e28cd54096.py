# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-baseten\livekit\plugins\baseten\stt.py
...
class SpeechStream(stt.SpeechStream):
            ...
            async for data in self._input_ch:
                ...
                for frame in frames:
                    if len(frame.data) % 2 != 0:
                        logger.warning(
                            "Frame data size not aligned to float32 (multiple of 4)"
                        )
            ...
            while True:
                ...
                try:
                    ...
                    if not is_final:
                        if text:
                            start_time = (
                                segments[0].get("start", 0.0) if segments else 0.0
                            )
    ...
    async def _connect_ws(self) -> aiohttp.ClientWebSocketResponse:
        ...
        ws = await self._session.ws_connect(
            self._model_endpoint, headers=headers, ssl=ssl_context
        )
