# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-playai\livekit\plugins\playai\tts.py
...
from pyht.client import TTSOptions as PlayOptions  # type: ignore
...
class TTS(tts.TTS):
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        pyht_api_key = (
            api_key if is_given(api_key) else os.environ.get("PLAYHT_API_KEY")
        )
        pyht_user_id = (
            user_id if is_given(user_id) else os.environ.get("PLAYHT_USER_ID")
        )
