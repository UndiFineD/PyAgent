# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-plugins\livekit-plugins-elevenlabs\livekit\plugins\elevenlabs\tts.py
...
class TTS(tts.TTS):
    def __init__(
        ...
        *,
        ...
        word_tokenizer: NotGivenOr[
            tokenize.WordTokenizer | tokenize.SentenceTokenizer
        ] = NOT_GIVEN,
        ...
        elevenlabs_api_key = (
            api_key if is_given(api_key) else os.environ.get("ELEVEN_API_KEY")
        )
        ...
        try:
            async with self._tts._ensure_session().post(
                ...
                if not resp.content_type.startswith("audio/"):
                    ...
                    raise APIError(
                        message="11labs returned non-audio data", body=content
                    )
...
class SynthesizeStream(tts.SynthesizeStream):
    ...
    def __init__(self, *, tts: TTS, conn_options: APIConnectOptions):
        ...
        self._segments_ch = utils.aio.Chan[
            Union[tokenize.WordStream, tokenize.SentenceStream]
        ]()
    ...
    async def _run_ws(
        ...
        if is_given(self._opts.voice_settings):
            init_pkt["voice_settings"] = _strip_nones(
                ...
                if (
                    ...
                    if any(data.token.find(end) > -1 for end in xml_end_tokens):
                        text = (
                            ...
                            if isinstance(
                                self._opts.word_tokenizer, tokenize.WordTokenizer
                            )
            ...
            while True:
                ...
                if data.get("audio") is not None:
                    ...
                    output_emitter.push_timed_transcript(
                        _to_timed_words(
                            text_buffer, start_times_ms, durations_ms, flush=True
                        )[0]
                    )
...
def _dict_to_voices_list(data: dict[str, Any]) -> list[Voice]:
    ...
    for voice in data["voices"]:
        voices.append(
            Voice(id=voice["voice_id"], name=voice["name"], category=voice["category"])
        )
...
def _to_timed_words(
    ...
    if flush:
        ...
        timed_words.append(
            TimedString(text=text[end:], start_time=start_t, end_time=end_t)
        )
        end = len(text)
