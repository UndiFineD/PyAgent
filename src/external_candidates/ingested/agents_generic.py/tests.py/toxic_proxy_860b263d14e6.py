...
class APIConsumer:
    ...
    @classmethod
    def get(
        cls, url: str, params: Optional[dict[str, Any]] = None, **kwargs
    ) -> requests.Response:
    ...
    def post(
        ...
        return validate_response(
            requests.post(url=endpoint, data=data, json=json, **kwargs)
        )
...
class Proxy:
    ...
    def toxics(self) -> dict[str, Toxic]:
        ...
        for toxic_data in toxics_list:
            toxic_data["proxy"] = (
                self.name
            )  # optionally add proxy info if needed elsewhere
