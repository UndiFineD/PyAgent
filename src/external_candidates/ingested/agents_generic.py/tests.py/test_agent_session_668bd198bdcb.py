...
class MyAgent(Agent):
    ...
    async def on_enter(self) -> None:
        if self.generate_reply_on_enter:
            self.session.generate_reply(
                instructions="instructions:say hello to the user"
            )
    ...
    async def goodbye(self) -> None:
        await self.session.generate_reply(
            instructions="instructions:say goodbye to the user"
        )
...
async def test_events_and_metrics() -> None:
    ...
    actions.add_user_speech(
        0.5, 2.5, "Hello, how are you?", stt_delay=0.2
    )  # EOU at 2.5+0.5=3.0s
    ...
    t_origin = await asyncio.wait_for(
        run_session(session, agent), timeout=SESSION_TIMEOUT
    )
    ...
    check_timestamp(
        conversation_events[0].created_at - t_origin, 3.0, speed_factor=speed
    )
    ...
    check_timestamp(
        conversation_events[1].created_at - t_origin, 5.5, speed_factor=speed
    )
    ...
    check_timestamp(
        metrics_events[0].metrics.end_of_utterance_delay, 0.5, speed_factor=speed
    )
    check_timestamp(
        metrics_events[0].metrics.transcription_delay, 0.2, speed_factor=speed
    )
...
async def test_tool_call() -> None:
    ...
    check_timestamp(
        playback_finished_events[0].playback_position, 2.0, speed_factor=speed
    )
    check_timestamp(
        playback_finished_events[1].playback_position, 3.0, speed_factor=speed
    )
    ...
    assert (
        tool_executed_events[0].function_calls[0].arguments == '{"location": "Tokyo"}'
    )
...
async def test_interruption_options() -> None:
    ...
    assert playback_finished_events[0].interrupted is True
    check_timestamp(
        playback_finished_events[0].playback_position, 4.0, speed_factor=speed
    )
    ...
    check_timestamp(
        playback_finished_events[0].playback_position, 5.0, speed_factor=speed
    )
...
async def test_interruption_by_text_input() -> None:
    ...
    assert chat_ctx_items[2].role == "assistant"
    assert (
        chat_ctx_items[2].interrupted is True
    )  # assistant message should be before text input
...
async def test_generate_reply() -> None:
    ...
    actions.add_llm(
        "What can I do for you!", input="instructions:say hello to the user"
    )
    actions.add_tts(2.0)
    actions.add_llm(
        "Goodbye! have a nice day!", input="instructions:say goodbye to the user"
    )
    actions.add_tts(3.0)
    ...
    check_timestamp(
        playback_finished_events[1].playback_position, 1.0, speed_factor=speed
    )
    assert playback_finished_events[2].interrupted is False
    check_timestamp(
        playback_finished_events[2].playback_position, 3.0, speed_factor=speed
    )
    ...
    check_timestamp(
        conversation_events[0].created_at - t_origin, 2.5, speed_factor=speed
    )
    assert conversation_events[1].item.type == "message"
    assert conversation_events[1].item.role == "user"
    assert conversation_events[1].item.text_content == "bye"
    check_timestamp(
        conversation_events[1].created_at - t_origin, 4.5, speed_factor=speed
    )
    assert conversation_events[2].item.type == "message"
    assert conversation_events[2].item.role == "assistant"
    assert (
        conversation_events[2].item.text_content
        == "session.say from on_user_turn_completed"
    )
    check_timestamp(
    ...
    assert (
        agent.chat_ctx.items[3].text_content
        == "session.say from on_user_turn_completed"
    )
...
async def test_preemptive_generation(
    preemptive_generation: bool, expected_latency: float
) -> None:
    speed = 5.0
    actions = FakeActions()
...
async def test_interrupt_during_on_user_turn_completed(
    ...
    actions.add_user_speech(
        2.6, 3.0, "about a firefighter."
    )  # interrupt before speaking
    ...
    assert (
        conversation_events[2].item.text_content
        == "Here is a story about a firefighter..."
    )
...
async def run_session(
    session: AgentSession, agent: Agent, *, drain_delay: float = 1.0
) -> float:
    stt = session.stt
...
class FakeActions:
    ...
    def add_llm(
        ...
        *,
        ...
        if not utils.is_given(input):
            raise ValueError(
                "input is required or previous item needs to be a user speech"
            )
    ...
    def add_tts(
        ...
        *,
        ...
        if not utils.is_given(input):
            raise ValueError(
                "input is required or previous item needs to be a llm response"
            )
    ...
    def get_llm_responses(self, *, speed_factor: float = 1.0) -> list[FakeLLMResponse]:
        return [
            item.speed_up(speed_factor) for item in self._items if item.type == "llm"
        ]

    def get_tts_responses(self, *, speed_factor: float = 1.0) -> list[FakeTTSResponse]:
        return [
            item.speed_up(speed_factor) for item in self._items if item.type == "tts"
        ]
