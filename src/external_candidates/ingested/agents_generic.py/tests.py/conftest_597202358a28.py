# Extracted from: C:\DEV\PyAgent\.external\agents_generic\tests\conftest.py
import asyncio
import dataclasses
import gc
import inspect
...
def is_pytest_asyncio_task(task) -> bool:
    try:
        ...
        for frame in task.get_stack():
            if (
                "pytest" in frame.f_code.co_filename
                or "pytest_asyncio" in frame.f_code.co_filename
            ):
                return True
    except Exception:
        pass
...
async def fail_on_leaked_tasks():
    ...
    if leaked_async_gens:
        gens_msg = "\n\n".join(
            format_async_generator_by_id(gen_id) for gen_id in leaked_async_gens
        )
