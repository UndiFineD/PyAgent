...
def setup_oai_proxy(toxiproxy: Toxiproxy) -> Proxy:
    return toxiproxy.create(
        "api.openai.com:443", "oai-stt-proxy", listen=OAI_LISTEN, enabled=True
    )
...
async def _do_synthesis(
    ...
    ), f"expected second last frame to not be empty, got {audio_events[-2].frame.duration}"  # now we flush then end_input, the second last frame might be a non-full frame from flush
...
async def test_tts_synthesize(
    tts_factory, toxiproxy: Toxiproxy, logger: logging.Logger
):
        ...
        with pytest.raises(APITimeoutError):
            await _do_synthesis(
                ...
                conn_options=APIConnectOptions(
                    max_retry=3, timeout=0.5, retry_interval=0.0
                ),
            )
...
async def _do_stream(
    ...
    async with tts_v.stream(conn_options=conn_options) as tts_stream:
        ...
        assert all(
            e.segment_id for e in audio_events
        ), "expected all events to have a segment_id"
        ...
        for _, (segment_text, segment_events) in enumerate(
            ...
            *non_final, final = segment_events
            ...
            assert (
                0 < final.frame.duration < 0.25
            ), "expected final frame to be non-empty (<250 ms)"
...
async def test_tts_stream_timeout(tts_factory, toxiproxy: Toxiproxy):
    ...
    try:
        ...
        if isinstance(tts_v, tts.StreamAdapter):
            ...
            assert (
                error_events.count == 1
            ), f"expected 1 errors, got {error_events.count}"
        else:
            assert (
                error_events.count == 4
            ), f"expected 4 errors, got {error_events.count}"
...
async def test_tts_audio_emitter(monkeypatch):
    ...
    assert (
        msgs5[1].is_final is True and msgs5[1].frame.data.tobytes() == b"\x00\x00" * 10
    )
