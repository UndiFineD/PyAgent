...
class FakeVADStream(VADStream):
    ...
    async def _main_task(self) -> None:
        ...
        for fake_speech in self._vad._fake_user_speeches:
            next_start_of_speech_time = (
                fake_speech.start_time + self._vad._min_speech_duration
            )
            next_end_of_speech_time = (
                fake_speech.end_time + self._vad._min_silence_duration
            )

            if current_time() < next_start_of_speech_time:
                await asyncio.sleep(next_start_of_speech_time - current_time())

            self._send_vad_event(
                VADEventType.START_OF_SPEECH, fake_speech, current_time()
            )

            inference_interval = (
                self._vad._min_speech_duration
            ...
            while current_time() < next_end_of_speech_time - inference_interval * 2:
                ...
                self._send_vad_event(
                    VADEventType.INFERENCE_DONE, fake_speech, current_time()
                )

            await asyncio.sleep(next_end_of_speech_time - current_time())
            self._send_vad_event(
                VADEventType.END_OF_SPEECH, fake_speech, current_time()
            )
    ...
    def _send_vad_event(
        ...
        self._event_ch.send_nowait(
            VADEvent(
                ...
                samples_index=0,
                timestamp=curr_time,
                speech_duration=min(curr_time, fake_speech.end_time)
                - fake_speech.start_time,
