...
class AudioStreamDecoder:
    ...
    def push(self, chunk: bytes) -> None:
        ...
        if not self._started:
            self._started = True
            target = (
                self._decode_wav_loop if self._av_format == "wav" else self._decode_loop
            )
    ...
    def _decode_loop(self) -> None:
        ...
        try:
            ...
            container.flags |= cast(
                int,
                av.container.Flags.no_buffer.value
                | av.container.Flags.flush_packets.value,
            )
        ...
        try:
            ...
            while True:
                ...
                while remaining > 0:
                    ...
                    remaining -= len(part)
                if chunk_id == b"fmt ":
                    audio_format, wave_channels, wave_rate, _, _, bits_per_sample = (
                        struct.unpack("<HHIIHH", data[:16])
                    )
                    if audio_format != 1:
                        raise ValueError(
                            f"Unsupported WAV audio format: {audio_format}"
                        )
                    break
