# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\job.py
...
class JobContext:
        ...
        *,
        ...
        async with self._lock:
            ...
            await self._room.connect(
                self._info.url, self._info.token, options=room_options
            )
            self._on_connect()
    ...
    def delete_room(self) -> asyncio.Future[api.DeleteRoomResponse]:  # type: ignore
        ...
        if cli.CLI_ARGUMENTS is not None and cli.CLI_ARGUMENTS.console:
            logger.warning(
                "job_ctx.delete_room() is not executed while in console mode"
            )
    ...
    def add_sip_participant(
        ...
        *,
        ...
        if cli.CLI_ARGUMENTS is not None and cli.CLI_ARGUMENTS.console:
            logger.warning(
                "job_ctx.add_sip_participant() is not executed while in console mode"
            )
        ...
        task = asyncio.create_task(
            self.api.sip.create_sip_participant(
                api.CreateSIPParticipantRequest(
                    ...
                    sip_call_to=call_to,
                    participant_name=(
                        participant_name if is_given(participant_name) else None
                    ),
                )
            ),
        )
