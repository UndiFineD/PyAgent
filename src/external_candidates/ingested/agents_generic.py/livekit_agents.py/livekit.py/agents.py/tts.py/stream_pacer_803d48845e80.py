...
class SentenceStreamPacer:
    def __init__(
        self, *, min_remaining_audio: float = 5.0, max_text_length: int = 300
    ) -> None:
        """
    ...
    async def _send_task(self) -> None:
        ...
        while not self._closing:
            ...
            if first_sentence or (
                generation_stopped
                and remaining_audio <= self._options.min_remaining_audio
            ):
                batch: list[str] = []
            ...
            if generation_started and not generation_stopped:
                ...
                wait_time = max(
                    0.5, remaining_audio - self._options.min_remaining_audio
                )
