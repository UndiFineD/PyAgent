# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\tts\tts.py
        ...
        for i in range(self._conn_options.max_retry + 1):
            ...
            try:
                ...
                current_span.set_attribute(
                    trace_types.ATTR_TTS_INPUT_TEXT, self._input_text
                )
                return
    ...
    async def _main_task(self) -> None:
        ...
        for i in range(self._conn_options.max_retry + 1):
            ...
            try:
                ...
                current_span.set_attribute(
                    trace_types.ATTR_TTS_INPUT_TEXT, self._pushed_text
                )
                return
            except APIError as e:
                retry_interval = self._conn_options._interval_for_retry(i)
...
class AudioEmitter:
    ...
    def initialize(
        ...
        *,
        ...
        self._main_atask = asyncio.create_task(
            self._main_task(), name="AudioEmitter._main_task"
        )

        if not self._streaming:
            self.__start_segment(
                segment_id=""
            )  # always start a segment with stream=False
    ...
    async def _main_task(self) -> None:
        ...
        def _emit_frame(
            frame: rtc.AudioFrame | None = None, *, is_final: bool = False
        ) -> None:
            ...
            if last_frame is None:
                if not is_final:
                    ...
                    if frame is None:
                        ...
                        frame = rtc.AudioFrame(
                            data=b"\0\0"
                            * (self._sample_rate // 100 * self._num_channels),
                ...
                if audio_byte_stream is None:
                    audio_byte_stream = audio.AudioByteStream(
                        ...
                        samples_per_channel=int(
                            frame.sample_rate // 1000 * self._frame_size_ms
                        ),
                    )
        ...
        try:
            async for data in self._write_ch:
                ...
                if isinstance(data, AudioEmitter._StartSegment):
                    ...
                    segment_ctx = AudioEmitter._SegmentContext(
                ...
                if not segment_ctx:
                    if self._streaming:
                        if isinstance(
                            data, (AudioEmitter._EndSegment, AudioEmitter._FlushSegment)
                        ):
                ...
                if self._is_raw_pcm:
                    ...
                    if isinstance(data, bytes):
                        ...
                        if (
                            isinstance(data, AudioEmitter._FlushSegment)
                            and audio_decoder
                        ):
                            ...
                            _emit_frame(is_final=True)
                            dump_segment()
                            audio_decoder = segment_ctx = audio_byte_stream = (
                                last_frame
                            ) = None
                        else:
                            logger.warning("unknown data type: %s", type(data))
