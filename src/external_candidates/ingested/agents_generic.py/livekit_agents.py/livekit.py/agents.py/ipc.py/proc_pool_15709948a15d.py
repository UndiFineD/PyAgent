# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\ipc\proc_pool.py
    ...
    async def _proc_spawn_task(self) -> None:
        ...
        async with self._init_sem:
            ...
            try:
                ...
                logger.exception(
                    "error initializing process", extra=proc.logging_extra()
                )
    ...
    async def _main_task(self) -> None:
        try:
            while not self._closed:
                current_pending = self._warmed_proc_queue.qsize() + len(
                    self._spawn_tasks
                )
                to_spawn = (
