...
RunEvent = Union[
    ChatMessageEvent, FunctionCallEvent, FunctionCallOutputEvent, AgentHandoffEvent
]


class RunResult(Generic[Run_T]):
    def __init__(
        self, *, user_input: str | None = None, output_type: type[Run_T] | None
    ) -> None:
    ...
    def _agent_handoff(self, *, old_agent: Agent | None, new_agent: Agent) -> None:
        self._recorded_items.append(
            AgentHandoffEvent(old_agent=old_agent, new_agent=new_agent)
        )
    ...
    def _mark_done(self) -> None:
        with contextlib.suppress(asyncio.InvalidStateError):
            ...
            if not isinstance(final_output, BaseException):
                if self._output_type and not isinstance(
                    final_output, self._output_type
                ):
...
class RunAssert:
    ...
    def _raise_with_debug_info(self, message: str, index: int | None = None) -> None:
        ...
        events_str = "\n".join(
            _format_events(self._events_list, selected_index=marker_index)
        )
    ...
    def next_event(
        ...
        *,
        type: (
            Literal["message", "function_call", "function_call_output", "agent_handoff"]
            | None
        ) = None,
    ) -> (
        EventAssert
        ...
        *,
        ...
        try:
            ...
            if type == "message":
                ...
                ev = self._current_event().is_function_call(
                    name=name, arguments=arguments
                )
            elif type == "function_call_output":
                ev = self._current_event().is_function_call_output(
                    output=output, is_error=is_error
                )
            elif type == "agent_handoff":
                ev = self._current_event().is_agent_handoff(
...
class EventAssert:
        ...
        if is_given(arguments):
            ...
            for key, value in arguments.items():
                if key not in actual or actual[key] != value:
                    self._raise(
                        f"For key '{key}', expected {value}, got {actual.get(key)}"
                    )
    ...
    def is_function_call_output(
        ...
        *,
        ...
        if is_given(is_error) and self._event.item.is_error != is_error:
            self._raise(
                f"Expected is_error={is_error}, got {self._event.item.is_error}"
            )
    ...
    def is_agent_handoff(
        ...
        if is_given(new_agent_type) and not isinstance(
            self._event.new_agent, new_agent_type
        ):
            self._raise(
    ...
    def contains_function_call_output(
        ...
        *,
        ...
        for idx, ev in enumerate(self._events):
            candidate = EventAssert(ev, self._parent, (self._rng.start or 0) + idx)
            with contextlib.suppress(AssertionError):
                return candidate.is_function_call_output(
                    output=output, is_error=is_error
                )
...
def _format_events(
    ...
    for i, event in enumerate(events):
        ...
        if isinstance(
            event, (ChatMessageEvent, FunctionCallEvent, FunctionCallOutputEvent)
        ):
