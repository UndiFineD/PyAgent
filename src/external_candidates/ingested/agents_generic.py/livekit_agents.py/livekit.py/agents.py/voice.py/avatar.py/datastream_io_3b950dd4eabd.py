...
class DataStreamAudioOutput(AudioOutput):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            label="DataStreamIO", next_in_chain=None, sample_rate=sample_rate
        )
        self._room = room
    ...
    async def _start_task(self) -> None:
        async with self._lock:
            ...
            await utils.wait_for_participant(
                room=self._room, identity=self._destination_identity
            )
                ...
                logger.warning(
                    ...
                    extra={
                        ...
                        "expected_identities": list(
                            cls._playback_finished_handlers.keys()
                        ),
                    },
                )
                return "reject"
        ...
        async for event in self._playback_finished_ch:
            ...
            while retry_count < self._rpc_max_retries:
                ...
                try:
                    ...
                    retry_count += 1
                    logger.warning(
                        "failed to notify the agent playback finished, retrying..."
                    )
        ...
        while not self._data_ch.closed:
            ...
            while self._stream_readers:
                ...
                if (
                    ...
                    raise ValueError(
                        "sample_rate or num_channels not found in byte stream"
                    )
                ...
                bstream = utils.audio.AudioByteStream(
                    ...
                    samples_per_channel=int(
                        math.ceil(sample_rate * self._frame_size_ms / 1000)
                    ),
                )
