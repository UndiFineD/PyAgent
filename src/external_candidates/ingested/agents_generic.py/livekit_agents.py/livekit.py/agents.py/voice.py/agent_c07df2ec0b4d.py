# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\agent.py
...
class Agent:
    ...
    class default:
        ...
        async def llm_node(
            ...
            activity = agent._get_activity_or_raise()
            assert (
                activity.llm is not None
            ), "llm_node called but no LLM node is available"
            assert isinstance(
