# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\room_io\_output.py
...
class _ParticipantAudioOutput(io.AudioOutput):
    ...
    async def _wait_for_playout(self) -> None:
        ...
        if interrupted:
            pushed_duration = max(
                pushed_duration - self._audio_source.queued_duration, 0
            )
...
class _ParticipantLegacyTranscriptionOutput:
    ...
    def _is_local_proxy_participant(self, participant: rtc.Participant) -> bool:
        ...
        if participant.identity == self._participant_identity or (
            (on_behalf := participant.attributes.get(ATTRIBUTE_PUBLISH_ON_BEHALF))
            is not None
