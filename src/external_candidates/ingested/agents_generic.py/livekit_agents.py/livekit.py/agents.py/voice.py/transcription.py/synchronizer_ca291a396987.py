# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\transcription\synchronizer.py
        ...
        *,
        ...
        if end_time is not None:
            self.add_by_annotation(
                ...
                start_time=end_time,
                end_time=None,
                text_to_hyphens=text_to_hyphens,
            )
...
class _AudioData:
    ...
    sr_data_annotated: _SpeakingRateData | None = (
        None  # speaking rate from `start_time`
    )
...
class _SegmentSynchronizerImpl:
    ...
    def __init__(
        self, options: _TextSyncOptions, *, next_in_chain: io.TextOutput
    ) -> None:
        self._opts = options
        ...
        self._audio_data = _AudioData(
            sr_stream=self._opts.speaking_rate_detector.stream()
        )
        ...
        async for text_seg in self._text_data.sentence_stream:
            ...
            for word, _, end_pos in self._opts.split_words(sentence):
                ...
                if (annotated := self._audio_data.sr_data_annotated) and (
                    ...
                    target_speaking_units = self._audio_data.sr_data_est.accumulate_to(
                        elapsed
                    )
                    target_hyphens = (
                        target_speaking_units * self._speed_on_speaking_unit
                    )
    ...
    def __init__(
        ...
        *,
        ...
        self._opts = _TextSyncOptions(
            ...
            sentence_tokenizer=(
                sentence_tokenizer
                or tokenize.basic.SentenceTokenizer(retain_format=True)
            ),
            speaking_rate_detector=SpeakingRateDetector(),
        )
        self._enabled = True
        ...
        self._impl = _SegmentSynchronizerImpl(
            options=self._opts, next_in_chain=next_in_chain_text
        )
        self._rotate_segment_atask = asyncio.create_task(
            self._rotate_segment_task(None)
        )
...
class _SyncedAudioOutput(io.AudioOutput):
    def __init__(
        ...
        self._next_in_chain: io.AudioOutput = (
            next_in_chain  # redefined for better typing
        )
...
class _SyncedTextOutput(io.TextOutput):
    def __init__(
        ...
        self._next_in_chain: io.TextOutput = (
            next_in_chain  # redefined for better typing
        )
        self._synchronizer = synchronizer
        self._capturing = False
    ...
    async def capture_text(self, text: str) -> None:
        ...
        if (
            not self._synchronizer.enabled
        ):  # passthrough text if the synchronizer is disabled
            await self._next_in_chain.capture_text(text)
            return
