# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\transcription\_speaking_rate.py
...
class SpeakingRateStream:
    ...
    def __init__(
        ...
        self._input_ch = aio.Chan[
            Union[rtc.AudioFrame, SpeakingRateStream._FlushSentinel]
        ]()
        ...
        async for input_frame in self._input_ch:
            if not isinstance(input_frame, rtc.AudioFrame):
                ...
                if available_samples > self._window_size_samples * 0.5:
                    ...
                    frame_f32_data = np.divide(
                        frame.data, np.iinfo(np.int16).max, dtype=np.float32
                    )

                    sr = self._compute_speaking_rate(
                        frame_f32_data, _inference_sample_rate
                    )
            ...
            if not self._input_sample_rate:
                ...
                _inference_sample_rate = (
                    self._opts.sample_rate or self._input_sample_rate
                )

                self._window_size_samples = int(
                    self._opts.window_duration * _inference_sample_rate
                )
                self._step_size_samples = int(
                    self._opts.step_size * _inference_sample_rate
                )
                inference_f32_data = np.empty(
                    self._window_size_samples, dtype=np.float32
                )
    ...
    def _compute_speaking_rate(
        ...
        if (
            len(tail_audio_sq) > 0
            and np.sqrt(np.mean(tail_audio_sq)) < silence_threshold * 0.5
        ):
            return 0.0
    ...
    def _spectral_flux(
        ...
        for i in range(1, spectral_magnitudes.shape[1]):
            ...
            flux = np.sum(
                np.abs(spectral_magnitudes[:, i] - spectral_magnitudes[:, i - 1])
            )
