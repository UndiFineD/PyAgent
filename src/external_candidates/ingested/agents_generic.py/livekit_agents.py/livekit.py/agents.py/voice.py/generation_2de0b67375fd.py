...
async def _llm_inference_task(
    ...
    current_span.set_attribute(
        ...
        json.dumps(
            chat_ctx.to_dict(
                exclude_audio=True, exclude_image=True, exclude_timestamp=False
            )
        ),
    )
    current_span.set_attribute(
    ...
    current_span.set_attribute(
        ...
        json.dumps(
            [
                fnc.model_dump(exclude={"type", "created_at"})
...
def perform_tts_inference(
    *, node: io.TTSNode, input: AsyncIterable[str], model_settings: ModelSettings
    ...
    tts_task = asyncio.create_task(
        _tts_inference_task(node, input, model_settings, data)
    )
    ...
    try:
        async for fnc_call in function_stream:
            ...
            try:
                ...
                _tool_completed(
                    make_tool_output(fnc_call=fnc_call, output=None, exception=e)
                )
                continue
            ...
            try:
                ...
                if mock := mock_tools.get(fnc_call.name):
                    ...
                    async def _run_mock(
                        mock: Callable, *fnc_args: Any, **fnc_kwargs: Any
                    ) -> Any:
                    ...
                    function_callable = functools.partial(
                        _run_mock, mock, *fnc_args, **fnc_kwargs
                    )
                else:
                    logger.debug(
                        "executing tool",
                ...
                async def _traceable_fnc_tool(
                    ...
                    try:
                        ...
                        output = make_tool_output(
                            fnc_call=fnc_call, output=val, exception=None
                        )
                        ...
                        output = make_tool_output(
                            fnc_call=fnc_call, output=None, exception=e
                        )
