# Extracted from: C:\DEV\PyAgent\.external\agents_generic\livekit-agents\livekit\agents\voice\room_io\_pre_connect_audio.py
import asyncio
import contextlib
import time
...
class PreConnectAudioHandler:
    ...
    def register(self) -> None:
        ...
        try:
            ...
            self._room.register_byte_stream_handler(
                PRE_CONNECT_AUDIO_BUFFER_STREAM, _handler
            )
        except ValueError:
            logger.warning(
    ...
    async def _read_audio_task(
        self, reader: rtc.ByteStreamReader, participant_id: str
    ) -> None:
        if not reader.info.attributes or not (
            track_id := reader.info.attributes.get("trackId")
        ):
            logger.warning(
                "pre-connect audio received but no trackId",
                extra={"participant": participant_id},
            )
            return
        ...
        try:
            if (
                ...
                raise ValueError(
                    "sampleRate or channels not found in pre-connect byte stream"
                )
            ...
            if reader.info.mime_type:
                ...
                is_opus = (
                    reader.info.mime_type == "audio/opus"
                    or "codecs=opus" in reader.info.mime_type
                )
