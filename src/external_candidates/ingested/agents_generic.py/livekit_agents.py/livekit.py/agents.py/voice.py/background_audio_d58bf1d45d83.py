...
class BackgroundAudioPlayer:
    def __init__(
        ...
        *,
        ...
        self._audio_source = rtc.AudioSource(
            48000, 1, queue_size_ms=_AUDIO_SOURCE_BUFFER_MS
        )
        ...
        self._republish_task: asyncio.Task[None] | None = (
            None  # republish the task on reconnect
        )
    ...
    def _normalize_builtin_audio(
        self, source: AudioSource
    ) -> AsyncIterator[rtc.AudioFrame] | str:
        if isinstance(source, BuiltinAudioClip):
            return source.path()
        else:
            return source
    ...
    def play(
        ...
        *,
        ...
        task = asyncio.create_task(
            self._play_task(play_handle, sound_source, volume, loop)
        )
        ...
        async with self._lock:
            ...
            if self._agent_session:
                self._agent_session.off(
                    "agent_state_changed", self._agent_state_changed
                )
    ...
    async def _publish_track(self) -> None:
        ...
        track = rtc.LocalAudioTrack.create_audio_track(
            "background_audio", self._audio_source
        )
