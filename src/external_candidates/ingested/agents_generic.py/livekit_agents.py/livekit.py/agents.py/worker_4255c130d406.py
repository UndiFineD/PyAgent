    ...
    def __init__(
        ...
        *,
        ...
        if not is_given(opts.http_proxy):
            opts.http_proxy = os.environ.get("HTTPS_PROXY") or os.environ.get(
                "HTTP_PROXY"
            )
    ...
    async def run(self) -> None:
        ...
        async def _load_task() -> None:
            ...
            while True:
                ...
                load_threshold = _WorkerEnvOption.getvalue(
                ...
                if not math.isinf(load_threshold):
                    ...
                    if active_jobs > 0:
                        ...
                        if job_load > 0.0:
                            available_load = max(
                                load_threshold - self._worker_load, 0.0
                            )
                            available_job = min(
                        ...
                        self._proc_pool.set_target_idle_processes(
                            default_num_idle_processes
                        )
    ...
    async def simulate_job(
        ...
        if isinstance(info, SimulateJobInfo):
            ...
            if cli.CLI_ARGUMENTS is None or not cli.CLI_ARGUMENTS.console:
                room_info = await self._api.room.create_room(
