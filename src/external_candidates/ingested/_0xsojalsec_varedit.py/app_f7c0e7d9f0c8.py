        ...
        if (
            ...
            if model_size == "2B":
                ...
                if image_size == 512:
                    ...
                    raise ValueError(
                        f"Unsupported image size: {image_size}, expected 512 or 1024"
                    )

            self.model_components = load_model(
                pretrain_root, model_path, model_size, image_size
            )
...
def create_interface():
    with gr.Blocks(title="VAREdit Image Editor") as demo:
        ...
        gr.Markdown(
            "Edit images using natural language instructions with the VAREdit model."
        )
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Accordion("Advanced Settings", open=False):
                    image_size = gr.Radio(
                        choices=[512, 1024], value=1024, label="Image Size"
                    )
