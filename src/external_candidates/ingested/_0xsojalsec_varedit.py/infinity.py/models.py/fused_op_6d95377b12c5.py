# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\models\fused_op.py
import gc
...
@torch.compile(fullgraph=True)
def fused_ada_layer_norm(
    C: int, eps: float, x: torch.Tensor, scale: torch.Tensor, shift: torch.Tensor
):
    x = x.float()
