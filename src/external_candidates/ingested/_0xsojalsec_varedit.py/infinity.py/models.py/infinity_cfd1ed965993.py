# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\models\infinity.py
"""
...
class Infinity(nn.Module):
    ...
    def compile_flex_attn(self):
        ...
        for h_div_w in self.train_h_div_w_list:
            ...
            for scales_num in apply_flex_attn_scales:
                ...
                attn_fn = FlexAttn(
                    block_scales=edit_patchs_nums_tuple,
                    mask_type=mask_type,
                    B=self.batch_size,
                    H=self.num_heads,
                    L=aligned_L,
    ...
    def add_lvl_embeding_for_x_BLC(self, x_BLC, scale_schedule, need_to_pad=0):
        ...
        x_BLC_this_scale = self.add_lvl_embeding(
            x_BLC_this_scale, len(scale_schedule) - 1, scale_schedule
        )
    ...
    def forward(
        ...
        source_x_BLC_wo_prefix: torch.Tensor,
        target_x_BLC_wo_prefix: torch.Tensor,
    ...
    def autoregressive_infer_cfg(
        ...
        for block_idx, b in enumerate(self.block_chunks):
            if self.add_lvl_embeding_only_first_block and block_idx == 0:
                src_last_stage = self.add_lvl_embeding(
                    src_last_stage, num_stages_minus_1, scale_schedule
                )
            if not self.add_lvl_embeding_only_first_block:
                src_last_stage = self.add_lvl_embeding(
                    src_last_stage, num_stages_minus_1, scale_schedule
...
@register_model
def infinity_8b(depth=40, embed_dim=3584, num_heads=28, drop_path_rate=0.1, **kwargs):
    return Infinity(
        depth=depth,
        embed_dim=embed_dim,
        num_heads=num_heads,
