# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\models\bitwise_self_correction.py
import os
...
def labels2image(all_indices, label_type="int_label", scale_schedule=None):
    summed_codes, recons_imgs = self.vae.decode_from_indices(
        all_indices, scale_schedule, label_type
    )
    recons_img = recons_imgs[0]
    recons_img = (recons_img + 1) / 2
    recons_img = (
        recons_img.permute(1, 2, 0).mul_(255).cpu().numpy().astype(np.uint8)[:, :, ::-1]
    )
    return recons_img
...
class BitwiseSelfCorrection(object):
    ...
    def flip_requant(
        self, vae_scale_schedule, inp_B3HW, raw_features, device, src=False
    ):
        with torch.amp.autocast("cuda", enabled=False):
            ...
            if src:
                ...
                if self.apply_spatial_patchify:
                    ...
                    residual = torch.nn.functional.pixel_unshuffle(
                        residual.squeeze(-3), 2
                    )
                x_BLC_wo_prefix = residual.reshape(*residual.shape[:2], -1).permute(
                    0, 2, 1
                )
                gt_ms_idx_Bl = None
            else:
                ...
                for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                    ...
                    quantized, _, bit_indices, loss = self.vae.quantizer.lfq(
                        residual
                    )  # quantized shape: [B, d_vae, 1, h, w], bit_indices shape: [B,1,h,w,d_vae]
                    gt_all_bit_indices.append(bit_indices)
                    if not src and si < self.noise_apply_layers:
                        noise_apply_strength = (
                            np.random.randint(0, 100 * self.noise_apply_strength + 1)
                            * 0.01
                        )
                        mask = (
                            torch.rand(*bit_indices.shape).to(device)
                            < noise_apply_strength
                        )
                    ...
                    if si < len(vae_scale_schedule) - 1:
                        this_scale_input = F.interpolate(
                            cum_var_input,
                        ...
                        if self.apply_spatial_patchify:
                            # (B,d,1,H,W) -> (B,d,H,W) -> (B,4d,H/2,W/2)
                            this_scale_input = torch.nn.functional.pixel_unshuffle(
                                this_scale_input.squeeze(-3), 2
                            )
                        x_BLC_wo_prefix.append(
                            this_scale_input.reshape(
                                *this_scale_input.shape[:2], -1
                            ).permute(0, 2, 1)
                        )  # (B,H/2*W/2,4C) or (B,H*W,C)
                ...
                if self.apply_spatial_patchify:
                    ...
                    for item in gt_all_bit_indices:
                        ...
                        item = item.permute(0, 2, 3, 1).reshape(
                            B, -1, 4 * self.vae.codebook_dim
                        )
    ...
    def visualize(
        self, vae_scale_schedule, inp_B3HW, gt_all_bit_indices, pred_all_bit_indices
    ):
