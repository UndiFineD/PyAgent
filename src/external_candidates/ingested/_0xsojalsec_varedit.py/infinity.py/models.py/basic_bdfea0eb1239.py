...
def precompute_rope2d_freqs_grid(
    ...
    t_height = torch.arange(max_height * 2, device=device, dtype=torch.int64).type_as(
        inv_freq
    )
    t_width = torch.arange(max_width * 2, device=device, dtype=torch.int64).type_as(
        inv_freq
    )
    ...
    for h_div_w in dynamic_resolution_h_w:
        ...
        for i, (_, ph, pw) in enumerate(scale_schedule):
            ...
            if rope2d_normalized_by_hw == 1:  # downsample
                ...
                tgt_rope_cache = freqs_grid_map[
                    :, tgt_indices[:, 0], tgt_indices[:, 1], :
                ]  # (2, ph*pw, half_head_dim)
    ...
    def forward(
        ...
        def down_func(tensor, src_scale, tgt_scale):
            ...
            if src_h == tgt_h and src_w == tgt_w:
                ...
                tensor = tensor.reshape(B, H, C, src_h, src_w).reshape(
                    B, -1, src_h, src_w
                )
        ...
        if not self.caching and start_layer:  # train & first layer
            length_list = [
                int(np.array(scale_schedule[i]).prod())
            ...
            for i in range(len(length_list) - 1):
                ...
                k_cal = (
                    torch.cat([k_src, *k_list[1 : i + 1]], dim=L_dim)
                    if i > 0
                    else k_src
                )
                v_src = down_func(v_list[0], scale_schedule[-1], scale_schedule[i - 1])
                v_cal = (
                    torch.cat([v_src, *v_list[1 : i + 1]], dim=L_dim)
                    if i > 0
                    else v_src
                )
                output = (
            ...
            pad_zeros = torch.zeros_like(
                v_list[-1], dtype=v.dtype, device=v.device
            ).permute(0, 2, 1, 3)
