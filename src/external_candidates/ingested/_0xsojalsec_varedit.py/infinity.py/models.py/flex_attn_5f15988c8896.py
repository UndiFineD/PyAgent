...
try:
    ...
    print(
        f"[Warning] flex attention need pytorch 2.5.0+ but your version is {torch.__version__}"
    )
    ...
    def __init__(
        ...
        if self.offsets[-1] < L:
            self.offsets = torch.cat(
                (self.offsets, torch.tensor([L], device="cuda")), dim=0
            )
        ...
        if mask_type == "var":
            ...
            raise NotImplementedError(
                f"{mask_type} not supportted in FlexAttn, support type:{self.support_mask_type}"
            )
