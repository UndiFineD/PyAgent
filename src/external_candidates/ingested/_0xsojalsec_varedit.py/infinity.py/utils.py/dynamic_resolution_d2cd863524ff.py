# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\utils\dynamic_resolution.py
import json
...
for ratio, hws in ratio2hws.items():
    ...
    if ratio != 1.000:
        full_ratio2hws[int(1 / ratio * 1000) / 1000] = [
            (item[1], item[0]) for item in hws
        ]
...
for ratio in full_ratio2hws:
    ...
    for ind, leng in enumerate([7, 10, 12, 13]):
        h_div_w = (
            full_ratio2hws[ratio][leng - 1][0] / full_ratio2hws[ratio][leng - 1][1]
        )
        assert (
            np.abs(h_div_w - ratio) < 0.01
        ), f"{full_ratio2hws[ratio][leng-1]}: {h_div_w} != {ratio}"
        pixel = (
            full_ratio2hws[ratio][leng - 1][0] * vae_stride,
            full_ratio2hws[ratio][leng - 1][1] * vae_stride,
        )
        if ind == 0:
            total_pixels = "0.06M"
        elif ind == 1:
        ...
        dynamic_resolution_h_w[ratio][total_pixels] = {"pixel": pixel, "scales": scales}
...
def get_h_div_w_template2indices(h_div_w_list, h_div_w_templates):
    ...
    for h_div_w, index in zip(h_div_w_list, indices):
        pbar.update(1)
        nearest_h_div_w_template_ = h_div_w_templates[
            np.argmin(np.abs(h_div_w - h_div_w_templates))
        ]
...
if __name__ == "__main__":
    for h_div_w_template in dynamic_resolution_h_w:
        for total_pixels in dynamic_resolution_h_w[h_div_w_template]:
            scales = np.array(
                dynamic_resolution_h_w[h_div_w_template][total_pixels]["scales"]
            )
