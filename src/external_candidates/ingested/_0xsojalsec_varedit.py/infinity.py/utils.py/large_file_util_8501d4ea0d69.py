# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\utils\large_file_util.py
import argparse
import glob
import itertools
import os
...
def get_part_jsonls(filepath, total_line_number, parts=512):
    dirname, filename, ext = (
        osp.dirname(filepath),
        osp.splitext(osp.basename(filepath))[0],
        osp.splitext(osp.basename(filepath))[1],
    )
    if parts == 1:
    ...
    for chunk_id in range(1, parts + 1):
        ...
        chunk_id2save_files[chunk_id] = osp.join(
            save_dir, f"{filename}_{chunk_id:04d}_{parts:04d}_{num_of_lines:09d}{ext}"
        )
...
def split_large_txt_files(filepath, chunk_id2save_files):
    ...
    with open(filepath, "r") as f:
        ...
        for line in f:
            chunk.append(line)
            cur_chunk_size = int(
                osp.splitext(osp.basename(chunk_id2save_files[chunk_id]))[0].split("_")[
                    -1
                ]
            )
            if len(chunk) >= cur_chunk_size:
                ...
                thread_list.append(
                    threading.Thread(
                        target=save_lines, args=(chunk, chunk_id2save_files[chunk_id])
                    )
                )
                thread_list[-1].start()
                chunk = []
...
if __name__ == "__main__":
    ...
    for jsonl_filepath in sorted(glob.glob(osp.join(args.jsonl_folder, "*.jsonl"))):
        ...
        missing, chunk_id2save_files = get_part_jsonls(
            jsonl_filepath, line_num, parts=args.parts
        )
