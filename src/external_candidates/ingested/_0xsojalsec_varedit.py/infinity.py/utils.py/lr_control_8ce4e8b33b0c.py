# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\utils\lr_control.py
import math
...
def filter_params(model, ndim_dict, nowd_keys=(), lr_scale=0.0) -> Tuple[
    ...
    print(
        f"[get_param_groups][lr decay] with_lr_scale={with_lr_scale}, lr_scale={lr_scale}"
    )
    ...
    for name, para in model.named_parameters():
        ...
        if (
            ndim_dict.get(name, 2) == 1
            or name.endswith("bias")
            or any(k in name for k in nowd_keys)
        ):
...
def plot():
    ...
    for sche in ("lin0",):
        ...
        for cur_it in range(max_it):
            ...
            lr.append(
                lr_wd_annealing(
                    sche, op, 0.1, 1e-5, 1e-5, cur_it, wp_it, max_it, wpe=0.3
                )[0]
            )
