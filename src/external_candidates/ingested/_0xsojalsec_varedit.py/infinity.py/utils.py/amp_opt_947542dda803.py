# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\infinity\utils\amp_opt.py
import math
import os
import signal
import sys
...
def per_param_clip_grad_norm_(
    parameters, thresh: float, stable=False, fp=None
) -> (float, float):
    skipped, max_grad = [], 0
        ...
        if p.grad is not None:
            ...
            if clip_coef < 1:
                if stable and clip_coef < 0.2:
                    ...
                    p.grad.data.mul_(
                        0
                    )  # todo NOTE: inf.mul_(0)==nan will shrink the scale ratio, but inf.zero_()==0 won't
                else:
...
class AmpOptimizer:
    def __init__(
        ...
        model_maybe_fsdp: Union[torch.nn.Module, FSDP],
        r_accu: float,
        grad_clip: float,
        zero: int,
    ):
        ...
        if self.enable_amp:
            ...
            )  # todo: cache_enabled=False
            if self.using_fp16_rather_bf16:
                self.scaler = torch.cuda.amp.GradScaler(
                    init_scale=2.0**11, growth_interval=1000
                )
            else:
                self.scaler = None
        else:
        ...
        self.early_clipping = grad_clip > 0 and not hasattr(
            optimizer, "global_grad_norm"
        )
        self.late_clipping = grad_clip > 0 and hasattr(
            optimizer, "global_grad_norm"
        )  # deepspeed's optimizer
    ...
    def log_param(self, ep: int):
        if self.zero == 0:
            for name, values in get_param_for_log(
                self.model_name_3letters, self.model_maybe_fsdp.named_parameters()
            ).items():
                values: List[float]
        ...
        if stepping:
            if self.scaler is not None:
                self.scaler.unscale_(
                    self.optimizer
                )  # now the gradient can be correctly got
            ...
            if self.early_clipping:
                ...
                if self.zero:
                    orig_norm: Optional[torch.Tensor] = (
                        self.model_maybe_fsdp.clip_grad_norm_(c)
                    )
                else:
                    orig_norm: Optional[torch.Tensor] = torch.nn.utils.clip_grad_norm_(
                        self.model_maybe_fsdp.parameters(), c
                    )
                ...
                if self.zero:
                    ...
                    for optimizer_state in self.scaler._per_optimizer_states.values():
                        for t in optimizer_state["found_inf_per_device"].values():
                            dist.allreduce(
                                t
                            )  # ideally, each rank only has one single t; so no need to use async allreduce

                self.scaler.step(self.optimizer)
                scaler_sc: Optional[float] = self.scaler.get_scale()
                if (
                    scaler_sc > self.max_sc
