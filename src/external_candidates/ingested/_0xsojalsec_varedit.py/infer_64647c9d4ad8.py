...
def transform(pil_img, target_image_size):
    ...
    padded_image = padded_image.resize(
        (target_image_size, target_image_size), resample=PImage.LANCZOS
    )
...
def load_model(
    pretrain_root: str, model_path: str, model_size: str, image_size: int
) -> Tuple[Any, ...]:
    """
...
def generate_image(
    ...
    scale_schedule = [
        (1, h, w)
        for (_, h, w) in dynamic_resolution_h_w[h_div_w_template][pn]["scales"]
    ]
...
def main():
    ...
    try:
        ...
        model_components = load_model(
            "HiDream-ai/VAREdit", "HiDream-ai/VAREdit/8B-1024.pth", "8B", 1024
        )
