# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VAREdit\tools\run_infinity.py
import os
...
def aug_with_positive_prompt(prompt):
    for key in [
        ...
        if key in prompt:
            prompt = (
                prompt
                + ". very smooth faces, good looking faces, face to the camera, perfect facial features"
            )
            break
    return prompt
...
def get_image_prefix(
    input_raw_features, vae, scale_schedule, apply_spatial_patchify=False
):
    with torch.amp.autocast("cuda", enabled=False):
        ...
        residual = F.interpolate(
            codes_out, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_down
        ).contiguous()
        if apply_spatial_patchify:
...
def gen_one_img(
    ...
    text_cond_tuple = encode_prompt(
        text_tokenizer, text_encoder, prompt, enable_positive_prompt
    )
    if negative_prompt:
        negative_label_B_or_BLT = encode_prompt(
            text_tokenizer, text_encoder, negative_prompt
        )
    else:
        negative_label_B_or_BLT = None

    src_img_3HW = src_img_3HW.unsqueeze(0).to("cuda", non_blocking=True)
    src_img_features, _, _ = vae.encode_for_raw_features(
        src_img_3HW, scale_schedule=scale_schedule
    )
    print(f"cfg: {cfg_list}, tau: {tau_list}")

    src_img_prefix = get_image_prefix(
        src_img_features, vae, scale_schedule, apply_spatial_patchify
    )
...
def load_infinity(
    ...
    text_maxlen = 512
    with (
        torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True),
        torch.no_grad(),
    ):
        ...
        print(
            f"[you selected Infinity with {model_kwargs=}] model size: {sum(p.numel() for p in infinity_test.parameters())/1e9:.2f}B, bf16={bf16}"
        )
...
def joint_vi_vae_encode_decode(vae, image_path, scale_schedule, device, tgt_h, tgt_w):
    ...
    t1 = time.time()
    h, z, _, all_bit_indices, _, infinity_input = vae.encode(
        inp, scale_schedule=scale_schedule
    )
    t2 = time.time()
...
def load_transformer(vae, args):
    ...
    if args.checkpoint_type == "torch":
        ...
        if osp.exists(args.cache_dir):
            local_model_path = osp.join(
                args.cache_dir, "tmp", model_path.replace("/", "_")
            )
        else:
        ...
        if args.enable_model_cache:
            ...
            print(
                f"local_model_path: {local_model_path}, local_slim_model_path: {local_slim_model_path}"
            )
            if not osp.exists(local_slim_model_path):
                if osp.exists(slim_model_path):
                    ...
                    save_slim_model(
                        local_model_path, save_file=local_slim_model_path, device=device
                    )
...
def add_common_arguments(parser):
    ...
    parser.add_argument(
        "--pn", type=str, required=True, choices=["0.06M", "0.25M", "1M"]
    )
    ...
    parser.add_argument(
        "--rope2d_normalized_by_hw", type=int, default=2, choices=[0, 1, 2]
    )
    parser.add_argument(
        "--use_scale_schedule_embedding", type=int, default=0, choices=[0, 1]
    )
    parser.add_argument(
        "--sampling_per_bits", type=int, default=1, choices=[1, 2, 4, 8, 16]
    )
