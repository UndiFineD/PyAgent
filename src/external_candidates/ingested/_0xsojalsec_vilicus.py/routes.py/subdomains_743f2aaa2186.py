# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-vilicus\routes\subdomains.py
# routes/subdomain.py
import logging
...
@main.route(
    "/update_subdomain/<string:main_domain>/<string:subdomain_name>", methods=["POST"]
)
def update_subdomain(main_domain, subdomain_name):
    ...
    if not domain:
        return "Main domain not found", 404
    subdomain = subdomains_collection.find_one(
        {"name": subdomain_name, "domain_id": domain["_id"]}
    )
    if not subdomain:
...
@main.route(
    "/delete_subdomain/<string:main_domain>/<string:subdomain_name>", methods=["POST"]
)
...
def get_last_updated():
    ...
    new_subdomains = list(
        subdomains_collection.find({"added_at": {"$gte": half_hour_ago}})
    )
