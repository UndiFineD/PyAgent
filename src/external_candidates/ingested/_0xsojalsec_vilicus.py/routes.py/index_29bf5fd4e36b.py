...
def add_domain():
    ...
    try:
        ...
        logging.info(
            f"Subdomains added for {name}: {subdomains}"
        )  # Log the added subdomains
        return (
            jsonify(
                {
...
def add_domains():
    ...
    for name in names:
        ...
        try:
            ...
            logging.error(f"Error while getting subdomains for {name}: {e}")
            response[name] = {
                "message": "Domain added, but failed to get its subdomains"
            }
...
def upload_subdomains():
    ...
    if file and allowed_file(file.filename):
        ...
        for name in subdomain_names:
            ...
            subdomain_name_with_main_domain = "{}.{}".format(
                subdomain_name, main_domain
            )
