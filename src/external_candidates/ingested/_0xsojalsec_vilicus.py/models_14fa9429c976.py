# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-vilicus\models.py
# models.py
...
class Domain:
    ...
    def last_updated(self):
        ...
        subdomain_data = (
            subdomains_collection.find({"domain_id": self.id})
            .sort("added_at", -1)
            .limit(1)
        )
    ...
    def add_subdomain(self, subdomain):
        existing_subdomain = subdomains_collection.find_one(
            {"name": subdomain, "domain_id": self.id}
        )
        if not existing_subdomain:
            ...
            domains_collection.update_one(
                ...
                {
                    "$inc": {"subdomains_count": 1},
                    "$set": {"last_updated": self.last_updated},
                },
            )  # Update the count and last_updated in the database.
    ...
    def remove_subdomain(self, subdomain):
        ...
        domains_collection.update_one(
            {"_id": ObjectId(self.id)}, {"$inc": {"subdomains_count": -1}}
        )  # Update the count in the database.

    def get_subdomains(self):
        subdomains_data = subdomains_collection.find({"domain_id": self.id})
        self.subdomains = [
            Subdomain(subdomain["domain_id"], subdomain["name"])
...
class Subdomain:
    def __init__(self, domain_id, name):
        ...
        self.added_at = (
            datetime.utcnow()
        )  # the timestamp for when the subdomain is created
        self.id = None  # Store the subdomain ID

    def save(self):
        subdomains_collection.insert_one(
            {"domain_id": self.domain_id, "name": self.name, "added_at": self.added_at}
        )
    ...
    def update_domain_subdomains(domain_id, subdomains):
        ...
        if domain:
            for subdomain_name in subdomains:
                ...
                existing_subdomain = subdomains_collection.find_one(
                    {"name": subdomain_name, "domain_id": domain_id}
                )
