...
def pretty_convert(obj):
    ...
    if isinstance(obj, (float)):
        ...
        return '{1}"{0}{2}{3}{0}{1}"{0}'.format(
            color.ENDC, color.REDL, color.RED, obj.encode()
        )

    elif isinstance(obj, (dict)):

        return dict(
            ("{1}{2}:{0}".format(color.ENDC, color.YELLOW, k), pretty_convert(v))
...
def walk_obj(
    buf, obj, indent_num=2, depth=0, eol="", wrap_len=60, wrap_total=100, lf="\n"
):
    ...
    if isinstance(obj, (str, int, float, complex)):
        ...
        if (
            max(map(len, obj if obj else " ")) > wrap_len
            or sum(map(len, obj)) > wrap_total
        ):
