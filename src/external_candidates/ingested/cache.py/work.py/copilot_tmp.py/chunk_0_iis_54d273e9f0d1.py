# Extracted from: C:\DEV\PyAgent\.external\cache\work\copilot_tmp\chunk_0_iis.py
...
class NXCModule:
    ...
    def check_appcmd(self, context, connection):
        ...
        try:
            ...
            context.log.fail(
                f"appcmd.exe not found, this module is not applicable - {e}"
            )
            ...
            if apppool_name and username is not None and password is not None:
                ...
                if current_credentials not in credentials_set:
                    ...
                    if username:
                        ...
                        context.log.success(
                            f"Credentials Found for APPPOOL: {apppool_name}"
                        )

                        if password == "":

                            context.log.highlight(
                                f"Username: {username} - User Does Not Have A Password"
                            )
