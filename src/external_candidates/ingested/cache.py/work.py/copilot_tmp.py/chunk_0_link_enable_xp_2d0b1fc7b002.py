# Extracted from: C:\DEV\PyAgent\.external\cache\work\copilot_tmp\chunk_0_link_enable_xp.py
...
class NXCModule:
    ...
    def on_login(self, context, connection):
        ...
        if not self.linked_server:
            ...
            self.context.log.fail(
                "Please provide a linked server name using the LINKED_SERVER option."
            )
    ...
    def enable_xp_cmdshell(self):
        ...
        self.context.log.display(
            f"Enabling advanced options on {self.linked_server}..."
        )
