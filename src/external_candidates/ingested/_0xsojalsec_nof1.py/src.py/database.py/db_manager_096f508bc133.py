...
class DatabaseManager:
    ...
    def close_trade(
        ...
        with self.session_scope() as session:
            ...
            session.refresh(trade)
            logger.info(
                f"Closed trade: {trade_id} (PnL: {realized_pnl:.2f}, {realized_pnl_pct:.2f}%)"
            )
            return trade
    ...
    def get_trade_stats(self) -> Dict[str, Any]:
        ...
        with self.session_scope() as session:
            ...
            winning_trades = [
                t for t in closed_trades if t.realized_pnl and t.realized_pnl > 0
            ]
            losing_trades = [
                t for t in closed_trades if t.realized_pnl and t.realized_pnl < 0
            ]
    ...
    def save_bot_state(
        ...
        with self.session_scope() as session:
            ...
            logger.debug(
                f"Saved bot state: balance={balance:.2f}, total_value={total_value:.2f}"
            )
            return state
    ...
    def reject_proposal(
        self, proposal_id: int, reason: Optional[str] = None
    ) -> TradeProposal:
    ...
    def get_database_stats(self) -> Dict[str, int]:
        ...
        with self.session_scope() as session:
            return {
                ...
                "pending_proposals": session.query(TradeProposal)
