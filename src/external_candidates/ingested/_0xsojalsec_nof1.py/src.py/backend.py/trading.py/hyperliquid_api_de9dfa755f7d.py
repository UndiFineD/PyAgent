# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\backend\trading\hyperliquid_api.py
...
class HyperliquidAPI:
    ...
    def __init__(self):
        ...
        if private_key and private_key != "your_private_key_here":
            ...
            raise ValueError(
                "Either HYPERLIQUID_PRIVATE_KEY/LIGHTER_PRIVATE_KEY or MNEMONIC must be provided with valid values (not placeholder)"
            )
    ...
    def _reset_clients(self):
        ...
        try:
            self._build_clients()
            logging.warning(
                "Hyperliquid clients re-instantiated after connection issue"
            )
        ...
        **kwargs,
        ...
        for attempt in range(max_attempts):
            try:
                ...
                last_err = e
                logging.warning(
                    "HL call unexpected error (attempt %s/%s): %s",
                    attempt + 1,
                    max_attempts,
                    e,
                )
    ...
    async def place_buy_order(self, asset, amount, slippage=0.01):
        ...
        return await self._retry(
            lambda: self.exchange.market_open(asset, True, amount, None, slippage)
        )
    ...
    async def place_sell_order(self, asset, amount, slippage=0.01):
        ...
        amount = self.round_size(asset, amount)
        return await self._retry(
            lambda: self.exchange.market_open(asset, False, amount, None, slippage)
        )
    ...
    async def place_take_profit(self, asset, is_buy, amount, tp_price):
        ...
        return await self._retry(
            lambda: self.exchange.order(
                asset, not is_buy, amount, tp_price, order_type, True
            )
        )
    ...
    async def place_stop_loss(self, asset, is_buy, amount, sl_price):
        ...
        return await self._retry(
            lambda: self.exchange.order(
                asset, not is_buy, amount, sl_price, order_type, True
            )
        )
    ...
    async def cancel_all_orders(self, asset):
        ...
        try:
            open_orders = await self._retry(
                lambda: self.info.frontend_open_orders(self.wallet.address)
            )
            ...
            return {
                "status": "ok",
                "cancelled_count": len(
                    [o for o in open_orders if o.get("coin") == asset]
                ),
            }
        ...
        try:
            ...
            if hasattr(self.info, "user_fills"):
                fills = await self._retry(
                    lambda: self.info.user_fills(self.wallet.address)
                )
    ...
    async def get_user_state(self):
        ...
        for pos_wrap in positions:
            ...
            current_px = (
                await self.get_current_price(pos["coin"]) if entry_px and size else 0.0
            )
            pnl = (
                (current_px - entry_px) * abs(size)
                if side == "long"
        ...
        if not total_value:
            total_value = balance + sum(
                max(p.get("pnl", 0.0), 0.0) for p in enriched_positions
            )
        return {
            "balance": balance,
