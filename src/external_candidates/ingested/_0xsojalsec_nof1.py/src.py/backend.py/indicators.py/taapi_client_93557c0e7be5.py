# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\backend\indicators\taapi_client.py
...
class TAAPIClient:
            ...
            try:
                ...
                if (
                    ...
                    if e.response.status_code == 429:
                        ...
                        logging.warning(
                            f"TAAPI {e.response.status_code}, retrying in {wait}s"
                        )
                    time.sleep(wait)
                else:
                    raise
            ...
            try:
                ...
                if (
                    ...
                    if e.response.status_code == 429:
                        ...
                        logging.warning(
                            f"TAAPI Bulk {e.response.status_code}, retrying in {wait}s"
                        )
                    time.sleep(wait)
                else:
                    raise
    ...
    def fetch_asset_indicators(self, asset):
        ...
        result[interval]["macd"] = self._extract_series(
            bulk_4h.get("macd"), "valueMACD"
        )
    ...
    def _extract_series(self, data, value_key="value"):
        ...
        if isinstance(data, dict) and value_key in data:
            ...
            if isinstance(values, list):
                return [
                    round(v, 4) if isinstance(v, (int, float)) else v for v in values
                ]
        return []
    ...
    def get_historical_indicator(
        self, indicator, symbol, interval, results=10, params=None
    ):
    ...
    def fetch_series(
        ...
        """
        try:
            data = self.get_historical_indicator(
                indicator, symbol, interval, results=results, params=params
            )
            if isinstance(data, dict):
                ...
                if "error" in data:
                    ...
                    logging.error(
                        f"TAAPI error for {indicator} {symbol} {interval}: {data.get('error')}"
                    )
                    return []
            return []
        except Exception as e:
