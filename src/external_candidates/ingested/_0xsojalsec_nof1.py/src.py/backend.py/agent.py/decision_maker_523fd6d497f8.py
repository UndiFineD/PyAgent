...
class TradingAgent:
    ...
    def _decide(self, context, assets):
        ...
        def _post(payload):
            ...
            logging.info(
                "Sending request to OpenRouter (model: %s)", payload.get("model")
            )
            ...
            logging.info(
                "Received response from OpenRouter (status: %s)", resp.status_code
            )
        ...
        for _ in range(6):
            ...
            try:
                ...
                provider = (err.get("error", {}).get("metadata", {}) or {}).get(
                    "provider_name", ""
                )
                ...
                if (
                    ...
                    and "tool" in error_message.lower()
                ):
                    logging.warning(
                ...
                if allow_structured and (
                    ...
                    or e.response.status_code in (400, 422)
                ):
                    logging.warning(
                        "Provider rejected structured outputs; retrying without response_format."
                    )
            ...
            try:
                ...
                if not isinstance(parsed, dict):
                    ...
                    sanitized = _sanitize_output(
                        content if "content" in locals() else json.dumps(parsed), assets
                    )
                            ...
                            normalized.append(
                                {
                                    ...
                                    "tp_price": (
                                        float(item[3])
                                        if item[3] and item[3] != "null"
                                        else None
                                    ),
                                    "sl_price": (
                                        float(item[4])
                                        if item[4] and item[4] != "null"
                                        else None
                                    ),
