# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\services\bot_service.py
"""
...
class BotService:
    ...
    async def start(
        self, assets: Optional[List[str]] = None, interval: Optional[str] = None
    ):
        """
        ...
        if not CONFIG.get("taapi_api_key"):
            raise ValueError(
                "TAAPI_API_KEY not configured. Please set it in .env file."
            )
        ...
        if not CONFIG.get("hyperliquid_private_key") and not CONFIG.get("mnemonic"):
            raise ValueError(
                "HYPERLIQUID_PRIVATE_KEY or MNEMONIC not configured. Please set it in .env file."
            )
        ...
        if not assets or not interval:
            raise ValueError(
                "Assets and interval must be configured. Set ASSETS and INTERVAL in .env file."
            )
        ...
        try:
            ...
            self.logger.info(
                f"Bot started successfully - Assets: {assets}, Interval: {interval}"
            )
    ...
    async def test_api_connections(self) -> Dict[str, bool]:
        ...
        try:
            ...
            taapi = TAAPIClient()
            try:
                test_result = taapi.fetch_value(
                    "rsi", "BTC/USDT", "5m", params={"period": 14}
                )
    ...
    async def refresh_market_data(self) -> bool:
        ...
        try:
            ...
            self._add_event(
                f"📊 Market data refreshed - Balance: ${state.balance:,.2f}"
            )
        ...
        try:
            ...
            config_data = {
                ...
                "api_keys": {
                    ...
                    "hyperliquid_private_key": CONFIG.get(
            ...
            if config_path.exists():
                ...
                if "api_keys" in data:
                    ...
                    if "hyperliquid_private_key" in data["api_keys"]:
                        CONFIG["hyperliquid_private_key"] = data["api_keys"][
                            "hyperliquid_private_key"
                        ]
                    if "openrouter_api_key" in data["api_keys"]:
                        CONFIG["openrouter_api_key"] = data["api_keys"][
                            "openrouter_api_key"
                        ]
                ...
                if "risk_management" in data:
                    if "max_position_size" in data["risk_management"]:
                        CONFIG["max_position_size"] = data["risk_management"][
                            "max_position_size"
                        ]
                ...
                if "notifications" in data:
                    if "desktop_enabled" in data["notifications"]:
                        CONFIG["desktop_notifications"] = data["notifications"][
                            "desktop_enabled"
                        ]
                    if "telegram_enabled" in data["notifications"]:
                        CONFIG["telegram_notifications"] = data["notifications"][
                            "telegram_enabled"
                        ]
                    if "telegram_token" in data["notifications"]:
                        CONFIG["telegram_token"] = data["notifications"][
                            "telegram_token"
                        ]
                    if "telegram_chat_id" in data["notifications"]:
                        CONFIG["telegram_chat_id"] = data["notifications"][
                            "telegram_chat_id"
                        ]
                    ...
                    try:
                        async with session.post(
                            ...
                            if resp.status in [
                                200,
                                401,
                            ]:  # 401 means key exists but might be invalid
