# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\pages\dashboard.py
"""
...
def create_dashboard(bot_service: BotService, state_manager: StateManager):
            ...
            equity_chart = ui.plotly(
                go.Figure(
                    ...
                    layout=go.Layout(
                        ...
                        yaxis=dict(
                            title="Value ($)", showgrid=True, gridcolor="#374151"
                        ),
    ...
    with ui.card().classes("w-full p-4 mb-6"):
        ...
        with market_data_container:
            ui.label("No market data available").classes(
    ...
    with ui.card().classes("w-full p-4 mb-6"):
        ...
        activity_log = ui.log(max_lines=10).classes(
            "w-full h-48 bg-gray-900 text-gray-300 p-4 rounded"
        )
    ...
    with ui.card().classes("w-full p-4"):
        ...
        with ui.row().classes("gap-4 items-center"):
            ...
            refresh_data_btn = ui.button(
                "🔄 Refresh Data", on_click=lambda: refresh_market_data()
            )
            refresh_data_btn.classes(
        ...
        with ui.row().classes("gap-4 items-center mt-4"):
            last_refresh_label = ui.label("Last refreshed: Never").classes(
        ...
        try:
                    ...
                    with ui.grid(columns=len(market_data)).classes("w-full gap-4"):
                        for asset_data in market_data:
                            ...
                            with ui.card().classes(
                                "p-4 bg-gradient-to-br from-gray-700 to-gray-800"
                            ):
                                ui.label(asset).classes(
                                    "text-2xl font-bold text-white mb-2"
                                )
                                ui.label(f"${price:,.2f}").classes(
                                    "text-xl text-green-400 mb-3"
                                )
                                ...
                                with ui.column().classes("gap-1 text-sm"):
                                    ...
                                    ui.separator()
                                    ui.label(
                                        f"EMA20 (4h): {lt_ema20:.2f}"
                                        if lt_ema20
                                        else "EMA20 (4h): N/A"
                                    ).classes("text-gray-400")
                                    ui.label(
                                        f"EMA50 (4h): {lt_ema50:.2f}"
                                        if lt_ema50
                                        else "EMA50 (4h): N/A"
                                    ).classes("text-gray-400")
            else:
            ...
            if last_refresh_time:
                ...
                if refresh_seconds_ago < 60:
                    last_refresh_label.text = (
                        f"Last refreshed: {refresh_seconds_ago} seconds ago"
                    )
                else:
