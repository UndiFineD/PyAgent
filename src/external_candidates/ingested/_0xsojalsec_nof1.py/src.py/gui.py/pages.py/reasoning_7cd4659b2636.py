...
def create_reasoning(bot_service: BotService, state_manager: StateManager):
    ...
    with ui.card().classes("w-full p-4 mb-6"):
        ui.label("Trade Decisions Timeline").classes(
    ...
    async def update_reasoning():
        ...
        if reasoning_data and (
            reasoning_data.get("reasoning") or reasoning_data.get("trade_decisions")
        ):
            ...
            with timeline_container:
                ...
                for decision in trade_decisions:
                    ...
                    if not any(
                        d.get("_id") == decision_id for d in historical_decisions
                    ):
                ...
                if trade_decisions:
                    ...
                    filtered_decisions = [
                        ...
                        or d.get("action", "HOLD").upper() == selected_filter
                    ]
                    ...
                    with stats_row:
                        ...
                        hold_count = len(
                            [
                                d
                                for d in trade_decisions
                                if d.get("action", "").upper() == "HOLD"
                            ]
                        )

                        ui.label(f"🟢 Buys: {buy_count}").classes(
                            "text-xs text-green-400 font-bold"
                        )
                        ui.label(f"🔴 Sells: {sell_count}").classes(
                            "text-xs text-red-400 font-bold"
                        )
                        ui.label(f"⚫ Holds: {hold_count}").classes(
                    ...
                    if filtered_decisions:
                                ...
                                with ui.timeline_entry(
                                    ...
                                    if confidence:
                                        ...
                                        with ui.row().classes(
                                            ...
                                            ui.label(f"{confidence_pct}%").classes(
                                                "text-xs text-gray-400 w-12"
                                            )
                        ...
                        ui.label(
                            f"No {action_filter.value} decisions in current batch"
                        ).classes("text-gray-400 text-center py-4")
                else:
                    ui.label("No trade decisions yet").classes(
            ...
            with timeline_container:
                with ui.column().classes("items-center py-8"):
                    ...
                    ui.label("No reasoning data available").classes(
                        "text-xl text-gray-400 mb-2"
                    )
                    ui.label(
                        "Start the bot from the Dashboard to see AI decisions"
