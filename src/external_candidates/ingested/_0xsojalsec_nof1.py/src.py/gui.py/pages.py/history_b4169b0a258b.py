# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\pages\history.py
"""
...
def create_history(bot_service: BotService, state_manager: StateManager):
    ...
    with ui.column().classes("w-full items-start"):
        ...
        with ui.card().classes("w-full p-4 mb-4"):
            ...
            with ui.row().classes("w-full gap-4 items-center"):
                ...
                action_filter = ui.select(
                    label="Action", options=["All", "buy", "sell", "hold"], value="All"
                ).classes("flex-1")

                # Limit filter
                limit_filter = ui.number(
                    label="Max Results", value=100, min=10, max=1000, step=10
                ).classes("flex-1")

                # Apply button
                apply_btn = ui.button("Apply Filters", on_click=lambda: None).classes(
                    "bg-blue-600 px-6"
                )

                # Export CSV button
                export_btn = ui.button("📥 Export CSV", on_click=lambda: None).classes(
                    "bg-green-600 px-6"
                )
        ...
        with ui.row().classes("w-full gap-4 mb-6"):
            ...
            with ui.card().classes(
                "flex-1 p-4 bg-gradient-to-br from-cyan-600 to-cyan-800"
            ):
            ...
            # Win Rate
            with ui.card().classes(
                "flex-1 p-4 bg-gradient-to-br from-green-600 to-green-800"
            ):
            ...
            # Total PnL
            with ui.card().classes(
                "flex-1 p-4 bg-gradient-to-br from-purple-600 to-purple-800"
            ):
                total_pnl_label = ui.label("$0.00").classes(
        ...
        with ui.card().classes("w-full p-4"):
            ...
            with detail_dialog, ui.card().classes("w-[600px]"):
                ...
                with ui.column().classes("gap-2"):
                    ...
                    ui.separator()
                    detail_rationale = ui.label("").classes(
            ...
            def show_detail(e):
                ...
                exit_str = (
                    f"${trade['exit_price']:.2f}" if trade.get("exit_price") else "N/A"
                )
        ...
        # Empty state message
        empty_message = ui.label("No trade history available").classes(
            "text-center text-gray-500 text-lg mt-8"
        )
        empty_message.visible = True

    # Current filter state
    current_filters = {"asset": "All", "action": "All", "limit": 100}
    ...
    async def update_history():
        ...
        try:
            ...
            asset = (
                None if current_filters["asset"] == "All" else current_filters["asset"]
            )
            action = (
                None
                if current_filters["action"] == "All"
                else current_filters["action"]
            )
            limit = int(current_filters["limit"])

            trades = bot_service.get_trade_history(
                asset=asset, action=action, limit=limit
            )
            ...
            if trades:
                ...
                win_rate_pct = (
                    (profitable_trades / total_count * 100) if total_count > 0 else 0
                )
                total_pnl_value = sum(
