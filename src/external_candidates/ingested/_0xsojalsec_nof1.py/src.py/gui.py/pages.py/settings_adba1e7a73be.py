...
def create_settings(bot_service: BotService, state_manager: StateManager):
        ...
        with ui.tab_panels(tabs, value=tab_strategy).classes("w-full"):
            ...
            with ui.tab_panel(tab_strategy):
                ...
                with ui.column().classes("gap-4 w-full max-w-2xl"):
                    ...
                    ui.label("Example: BTC, ETH, SOL or BTC ETH SOL").classes(
                    ...
                    # Interval selection
                    ui.label("Trading Interval").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Timeframe for trading decisions").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    ui.label("LLM model for trading decisions").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    with ui.row().classes("items-center gap-4"):
                        ...
                        ui.label("Use extended reasoning for better decisions").classes(
                    ...
                    async def save_strategy_config():
                        try:
                            ...
                            config_data["strategy"][
                                "llm_model"
                            ] = llm_model_select.value
                            config_data["strategy"][
                                "reasoning_enabled"
                            ] = reasoning_enabled.value
                            config_data["strategy"][
                                "reasoning_effort"
                            ...
                            if save_config(config_data):
                                ...
                                assets_list = [
                                    a.strip()
                                    for a in config_data["strategy"]["assets"]
                                    .replace(",", " ")
                                    .split()
                                    if a.strip()
                                ]
                                ...
                                ui.notify(
                                    "Failed to save configuration", type="negative"
                                )
                        except Exception as e:
                    ...
                    async def load_strategy_config():
                        try:
                            ...
                            interval_select.value = loaded_config["strategy"][
                                "interval"
                            ]
                            llm_model_select.value = loaded_config["strategy"][
                                "llm_model"
                            ]
                            reasoning_enabled.value = loaded_config["strategy"][
                                "reasoning_enabled"
                            ]
                            reasoning_effort.value = loaded_config["strategy"][
                                "reasoning_effort"
                            ]
                            ui.notify(
                            ...
                            ui.notify(
                                f"Error loading config: {str(e)}", type="negative"
                            )
            ...
            with ui.tab_panel(tab_api):
                ui.label("API Keys Configuration").classes(
                    "text-2xl font-bold mb-4 text-white"
                )
                ...
                with ui.column().classes("gap-4 w-full max-w-2xl"):
                    ...
                    with ui.row().classes("gap-4 items-center"):
                        taapi_status = ui.label("TAAPI: 🔴 Not Connected").classes(
                            "text-sm"
                        )
                        hyperliquid_status = ui.label(
                            "Hyperliquid: 🔴 Not Connected"
                        ).classes("text-sm")
                        openrouter_status = ui.label(
                    ...
                    # TAAPI Key
                    ui.label("TAAPI.io API Key").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Get your API key from https://taapi.io").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # Hyperliquid Key
                    ui.label("Hyperliquid Private Key").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Your wallet private key for trading").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # OpenRouter Key
                    ui.label("OpenRouter API Key").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Get your API key from https://openrouter.ai").classes(
                        "text-xs text-gray-400"
                    )
                        ...
                        try:
                            ...
                            if hyperliquid_input.value:
                                os.environ["HYPERLIQUID_PRIVATE_KEY"] = (
                                    hyperliquid_input.value
                                )
                            if openrouter_input.value:
                                os.environ["OPENROUTER_API_KEY"] = (
                                    openrouter_input.value
                                )
                            ...
                            ui.notify(
                                f"Error testing connections: {str(e)}", type="negative"
                            )
                    ...
                    async def save_api_keys():
                        ...
                        try:
                            ...
                            if save_config(config_data):
                                ...
                                ui.notify(
                                    "API keys saved successfully!", type="positive"
                                )
                                ui.notify(
                            ...
                            ui.notify(
                                f"Error saving API keys: {str(e)}", type="negative"
                            )

                    with ui.row().classes("gap-2"):
                        ui.button(
                            "Save API Keys", on_click=save_api_keys, icon="save"
                        ).props("color=primary")
                        ui.button(
                            "Test Connections",
                    ...
                    with ui.card().classes("bg-orange-900 p-4"):
                        ui.label("⚠️ Security Warning").classes(
            ...
            with ui.tab_panel(tab_risk):
                ui.label("Risk Management").classes(
                    "text-2xl font-bold mb-4 text-white"
                )

                with ui.column().classes("gap-6 w-full max-w-2xl"):
                    # Max Position Size
                    ui.label("Maximum Position Size").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Maximum USD allocation per position").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # Max Leverage
                    ui.label("Maximum Leverage").classes(
                        "text-lg font-semibold text-white"
                    )
                    with ui.row().classes("items-center w-full gap-4"):
                        ...
                        max_leverage_label = ui.label(
                            f"{config_data['risk_management']['max_leverage']:.1f}x"
                        ).classes("text-white font-bold min-w-[100px]")
                        max_leverage_slider.on(
                            "update:model-value",
                            lambda e: max_leverage_label.set_text(f"{e.args:.1f}x"),
                        )
                    ui.label("Maximum leverage for perpetual futures (1x-20x)").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # Stop Loss %
                    ui.label("Default Stop Loss").classes(
                        "text-lg font-semibold text-white"
                    )
                    with ui.row().classes("items-center w-full gap-4"):
                        ...
                        stop_loss_label = ui.label(
                            f"{config_data['risk_management']['stop_loss_pct']:.1f}%"
                        ).classes("text-white font-bold min-w-[100px]")
                        stop_loss_slider.on(
                            "update:model-value",
                            lambda e: stop_loss_label.set_text(f"{e.args:.1f}%"),
                        )
                    ui.label("Default stop loss percentage from entry").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # Take Profit %
                    ui.label("Default Take Profit").classes(
                        "text-lg font-semibold text-white"
                    )
                    with ui.row().classes("items-center w-full gap-4"):
                    ...
                    ui.label("Default take profit percentage from entry").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # Max Open Positions
                    ui.label("Maximum Open Positions").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Maximum number of concurrent open positions").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    async def save_risk_config():
                        try:
                            # Update config data
                            config_data["risk_management"]["max_position_size"] = int(
                                max_position_slider.value
                            )
                            config_data["risk_management"]["max_leverage"] = float(
                                max_leverage_slider.value
                            )
                            config_data["risk_management"]["stop_loss_pct"] = float(
                                stop_loss_slider.value
                            )
                            config_data["risk_management"]["take_profit_pct"] = float(
                                take_profit_slider.value
                            )
                            config_data["risk_management"]["max_open_positions"] = int(
                                max_positions_input.value
                            )
                            ...
                            if save_config(config_data):
                                ...
                                ui.notify(
                                    "Failed to save risk settings", type="negative"
                                )
                        except Exception as e:
                            ui.notify(
                                f"Error saving risk config: {str(e)}", type="negative"
                            )

                    ui.button(
                        "Save Risk Settings", on_click=save_risk_config, icon="save"
                    ).props("color=primary")
                    ...
                    with ui.card().classes("bg-red-900 p-4"):
                        ui.label("⚠️ Risk Warning").classes(
                            "text-lg font-bold text-white mb-2"
                        )
                        ui.label(
            ...
            with ui.tab_panel(tab_notifications):
                ...
                with ui.column().classes("gap-4 w-full max-w-2xl"):
                    ...
                    ui.label("Desktop Notifications").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    ui.label("Show system notifications for important events").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    # Telegram Bot
                    ui.label("Telegram Notifications").classes(
                        "text-lg font-semibold text-white"
                    )
                    ...
                    telegram_enabled_checkbox.on(
                        "update:model-value", toggle_telegram_inputs
                    )

                    ui.label("Get your bot token from @BotFather on Telegram").classes(
                        "text-xs text-gray-400"
                    )
                    ...
                    async def test_notification():
                        try:
                            if desktop_enabled_checkbox.value:
                                ...
                                ui.notify(
                                    "Desktop notifications are disabled", type="warning"
                                )

                            if (
                                telegram_enabled_checkbox.value
                                and telegram_token_input.value
                            ):
                                ui.notify(
                    ...
                    async def save_notification_config():
                        try:
                            ...
                            config_data["notifications"][
                                "desktop_enabled"
                            ] = desktop_enabled_checkbox.value
                            config_data["notifications"][
                                "telegram_enabled"
                            ] = telegram_enabled_checkbox.value
                            config_data["notifications"][
                                "telegram_token"
                            ] = telegram_token_input.value
                            config_data["notifications"][
                                "telegram_chat_id"
                            ] = telegram_chat_input.value
                    ...
                    with ui.card().classes("bg-blue-900 p-4"):
                        ui.label("ℹ️ Notification Features").classes(
                            "text-lg font-bold text-white mb-2"
                        )
                        ui.label(
                            "Configure notifications for trade executions, errors, and daily summaries."
                        ).classes("text-sm text-gray-200")
    ...
    async def load_initial_config():
        ...
        try:
            current_config = (
                await bot_service.get_current_config()
                if hasattr(bot_service, "get_current_config")
                else None
            )
            if current_config:
