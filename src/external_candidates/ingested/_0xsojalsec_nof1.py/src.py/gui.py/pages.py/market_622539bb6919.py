# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\pages\market.py
"""
...
def create_market(bot_service: BotService, state_manager: StateManager):
    ...
    with ui.row().classes("w-full items-center gap-4 mb-6"):
        ...
        configured_assets = (
            bot_service.get_assets()
            if bot_service.is_running()
            else ["BTC", "ETH", "SOL"]
        )
        available_assets = (
            configured_assets if configured_assets else ["BTC", "ETH", "SOL"]
        )
        ...
        interval_select = ui.select(
            label="Interval", options=["1m", "5m", "15m", "1h", "4h", "1d"], value="5m"
        ).classes("w-32")
    ...
    with ui.grid(columns=4).classes("w-full gap-4 mb-6"):
        ...
        with ui.card().classes("metric-card"):
            current_price_label = ui.label("$0.00").classes(
        ...
        # 24h Change Card
        with ui.card().classes("metric-card"):
            change_24h_label = ui.label("+0.00%").classes(
        ...
        # 24h Volume Card
        with ui.card().classes("metric-card"):
            volume_24h_label = ui.label("$0.00M").classes(
                "text-4xl font-bold text-white"
            )
        ...
        # Open Interest Card
        with ui.card().classes("metric-card"):
            open_interest_label = ui.label("$0.00M").classes(
                "text-4xl font-bold text-white"
            )
    ...
    with ui.card().classes("w-full p-4 mb-6"):
        ...
        # Candlestick chart
        price_chart = ui.plotly(
            go.Figure(
                data=[
                    go.Candlestick(
                        x=[], open=[], high=[], low=[], close=[], name="Price"
                    )
                ],
                layout=go.Layout(
    ...
    with ui.row().classes("w-full gap-4 mb-6"):
        ...
        with ui.card().classes("flex-1 p-4"):
            ...
            with ui.column().classes("gap-3 w-full"):
                ...
                with ui.row().classes("w-full justify-between items-center"):
                    ...
                    macd_line_label = ui.label("0.00").classes(
                        "text-white font-semibold"
                    )

                with ui.row().classes("w-full justify-between items-center"):
                    ui.label("Signal Line").classes("text-gray-300")
                    macd_signal_label = ui.label("0.00").classes(
                        "text-white font-semibold"
                    )

                with ui.row().classes("w-full justify-between items-center"):
                    ui.label("Histogram").classes("text-gray-300")
                    macd_hist_label = ui.label("0.00").classes(
        ...
        with ui.card().classes("flex-1 p-4"):
            ...
            with ui.column().classes("gap-3 w-full"):
                ...
                # RSI Bar
                rsi_progress = ui.linear_progress(value=0.5, show_value=False).classes(
                    "w-full"
                )
                ...
                with ui.row().classes("w-full justify-between items-center"):
                    ...
                    stoch_k_label = ui.label("50.00").classes(
                        "text-white font-semibold"
                    )

                with ui.row().classes("w-full justify-between items-center"):
                    ui.label("%D").classes("text-gray-300")
                    stoch_d_label = ui.label("50.00").classes(
                        "text-white font-semibold"
                    )
        ...
        indicator_chart = ui.plotly(
            go.Figure(
                ...
                layout=go.Layout(
                    ...
                    xaxis=dict(title="Time", showgrid=True, gridcolor="#374151"),
                    yaxis=dict(
                        title="RSI", showgrid=True, gridcolor="#374151", range=[0, 100]
                    ),
                    yaxis2=dict(
                        title="MACD", overlaying="y", side="right", showgrid=False
                    ),
                    paper_bgcolor="#1f2937",
                    plot_bgcolor="#1f2937",
                    font=dict(color="#e5e7eb"),
    ...
    with ui.card().classes("w-full p-4"):
        ...
        with ui.row().classes("w-full gap-6 items-center"):
            ...
            with ui.column().classes("flex-1"):
                sentiment_label = ui.label("NEUTRAL").classes(
                    "text-3xl font-bold text-gray-400"
                )
                sentiment_desc = ui.label("Waiting for clear signals").classes(
    ...
    async def update_market_data():
        ...
        if state.market_data:
            ...
            if isinstance(state.market_data, dict):
                ...
                market_data = next(
                    (m for m in state.market_data if m.get("asset") == selected_asset),
                    None,
                )
