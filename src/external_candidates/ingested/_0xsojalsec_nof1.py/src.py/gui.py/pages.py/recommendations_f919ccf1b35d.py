# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nof1.ai-alpha-arena\src\gui\pages\recommendations.py
"""
...
def create_recommendations(bot_service: BotService, state_manager: StateManager):
    ...
    async def update_proposals():
        ...
        with stats_row:
            ...
            with ui.card().classes(
                "flex-1 p-4 bg-gradient-to-br from-purple-800 to-purple-900"
            ):
            ...
            if state.is_running:
                with ui.card().classes(
                    "flex-1 p-4 bg-gradient-to-br from-green-800 to-green-900"
                ):
                    ui.label("Active").classes("text-3xl font-bold text-white")
                    ui.label("Bot Status").classes("text-sm text-green-200 mt-1")
            else:
                with ui.card().classes(
                    "flex-1 p-4 bg-gradient-to-br from-gray-700 to-gray-800"
                ):
        ...
        if not proposals:
            with proposals_container:
                with ui.card().classes(
                    "w-full p-8 bg-gradient-to-br from-gray-800 to-gray-900"
                ):
                    with ui.column().classes("items-center gap-4"):
                        ui.icon("inbox", size="64px").classes("text-gray-600")
                        ui.label("No pending recommendations").classes(
                            "text-2xl text-gray-400"
                        )
                        ui.label(
                            "AI will create proposals when the bot analyzes the market."
    ...
    def create_proposal_card(proposal: dict):
        ...
        with ui.card().classes(
            f"w-full p-6 bg-gradient-to-br {gradient} border border-gray-700"
        ):
            # Header row
            ...
            with ui.row().classes("w-full items-center gap-3 mb-4"):
                ...
                with ui.linear_progress(value=confidence / 100).classes("flex-1"):
                    pass
                ui.label(f"{confidence:.0f}%").classes(
            ...
            with ui.grid(columns=3).classes("w-full gap-4 mb-4"):
                ...
                with ui.card().classes("p-3 bg-gray-800 bg-opacity-50"):
                    ...
                    ui.label(f"${entry_price:,.2f}").classes(
                ...
                with ui.card().classes("p-3 bg-green-900 bg-opacity-30"):
                    ...
                    if tp_price:
                        ui.label(f"${tp_price:,.2f}").classes(
                ...
                with ui.card().classes("p-3 bg-red-900 bg-opacity-30"):
                    ...
                    if sl_price:
                        ui.label(f"${sl_price:,.2f}").classes(
            ...
            with ui.grid(columns=2).classes("w-full gap-4 mb-4"):
                with ui.card().classes("p-3 bg-gray-800 bg-opacity-50"):
                    ...
                    ui.label(f"{size:.4f} {asset}").classes(
                        "text-lg text-white font-bold"
                    )

                with ui.card().classes("p-3 bg-gray-800 bg-opacity-50"):
                    ui.label("Allocation").classes("text-xs text-gray-400 mb-1")
                    ui.label(f"${allocation:,.2f}").classes(
                        "text-lg text-white font-bold"
                    )
            ...
            if risk_reward:
                with ui.card().classes("w-full p-3 bg-blue-900 bg-opacity-30 mb-4"):
                    with ui.row().classes("items-center gap-3"):
                        ...
                        ui.label(f"Risk/Reward Ratio: 1:{risk_reward:.2f}").classes(
                            "text-lg text-blue-300 font-bold"
                        )

            # AI Rationale (expandable)
            with ui.expansion("🧠 AI Rationale", icon="psychology").classes(
                "w-full bg-gray-800 bg-opacity-50 mb-4"
            ):
                ui.label(rationale).classes("text-gray-300 whitespace-pre-wrap p-3")

            # Timestamp
            ui.label(f'Created: {timestamp[:19] if timestamp else "N/A"}').classes(
            ...
            with ui.row().classes("gap-3 w-full justify-end"):
                ui.button(
                    "❌ Reject", on_click=lambda pid=proposal_id: reject_proposal(pid)
    ...
    async def approve_proposal(proposal_id: str):
        ...
        try:
            ...
            if success:
                ui.notify(
                    "✅ Trade approved and executing!", type="positive", position="top"
                )
    ...
    async def reject_proposal(proposal_id: str):
        """Reject a proposal"""
        try:
            success = bot_service.reject_proposal(
                proposal_id, reason="Rejected by user via GUI"
            )
            if success:
