...
class Registry:
    ...
    async def async_execute(self, name: str, input: dict, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if not name:
                raise ValueError(
                    "Action Name was None or empty. The LLM failed to choose a valid action."
                )

            if tool is None:
                raise ValueError(
                    f'Tool "{name}" not found. Please choose from the available tools.'
                )
            ...
            print(
                f"DEBUG: Tool execution failed. Name: {name}, Input: {input}, Error: {e}"
            )
