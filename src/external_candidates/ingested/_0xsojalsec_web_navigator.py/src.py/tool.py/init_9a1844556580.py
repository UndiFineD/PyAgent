# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Web-Navigator\src\tool\__init__.py
...
class Tool:
        ...
        if self.params:
            ...
            if self.params is not None:
                self.schema = {
                    k: {
                        term: content
                        for term, content in v.items()
                        if term not in skip_keys
                    }
                    for k, v in self.params.model_json_schema()
                    .get("properties")
                    .items()
        ...
        try:
            if self.params:
                ...
                return await self.func(
                    **args.dict()
                )  # Call the function with validated arg
            else:
    ...
    def __repr__(self):
        ...
        return (
            f"Tool(name={self.name}, description={self.description}, params={params})"
        )
