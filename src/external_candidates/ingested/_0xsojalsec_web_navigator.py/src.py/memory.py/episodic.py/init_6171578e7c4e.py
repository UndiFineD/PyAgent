...
class EpisodicMemory(BaseMemory):
    ...
    def router(self, conversation: list[BaseMessage]):
        ...
        router = LLMRouter(
            instructions=instructions, routes=routes, llm=self.llm, verbose=False
        )
        route = router.invoke(
    ...
    def update_memory(self, conversation: list[BaseMessage]):
        ...
        with open(f"./memory_data/{self.knowledge_base}", "r+") as f:
            ...
            updated_knowledge_base = list(
                filter(lambda memory: memory.id not in memory_ids, knowledge_base)
            )
    ...
    def replace_memory(self, conversation: list[BaseMessage]):
        ...
        with open(f"./memory_data/{self.knowledge_base}", "r+") as f:
            ...
            f.seek(0)
            json.dump(
                [memory.model_dump() for memory in updated_knowledge_base], f, indent=2
            )
            f.truncate()
