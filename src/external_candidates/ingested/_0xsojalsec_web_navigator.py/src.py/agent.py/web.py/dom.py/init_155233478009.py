# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Web-Navigator\src\agent\web\dom\__init__.py
...
class DOM:
    ...
    async def get_state(
        self, use_vision: bool = False, freeze: bool = False
    ) -> tuple[str | None, DOMState]:
    ...
    async def get_elements(
        self, frames: list[Frame | Page]
    ) -> tuple[list[DOMElementNode], list[DOMTextualNode], list[ScrollElementNode]]:
        ...
        try:
            for index, frame in enumerate(frames):
                ...
                if index > 0:
                    ...
                    frame_xpath = await self.context.execute_script(
                        frame, "(frame_element)=>getXPath(frame_element)", frame_element
                    )
                else:
                    frame_xpath = ""
