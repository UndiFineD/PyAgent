...
class Context:
    ...
    async def setup_context(
        ...
        if self.browser.config.device is not None:
            parameters = {
                **self.browser.playwright.devices.get(self.browser.config.device)
            }
        ...
        if browser is not None:
            ...
            if browser == "chrome":
                context = (
                    await self.browser.playwright.chromium.launch_persistent_context(
                        channel="chrome", **parameters
                    )
                )
            elif browser == "firefox":
                context = (
                    await self.browser.playwright.firefox.launch_persistent_context(
                        **parameters
                    )
                )
            elif browser == "edge":
    ...
    async def get_viewport(self) -> tuple[int, int]:
        page = await self.get_current_page()
        viewport: dict = await self.execute_script(
    ...
    async def get_screenshot(
        self, save_screenshot: bool = False, full_page: bool = False
    ):
        page = await self.get_current_page()
        if save_screenshot:
        ...
        screenshot = await page.screenshot(
            path=path, full_page=full_page, animations="disabled", type="jpeg"
        )
        return screenshot
