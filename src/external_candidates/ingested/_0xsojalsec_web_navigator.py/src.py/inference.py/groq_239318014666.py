# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Web-Navigator\src\inference\groq.py
import mimetypes
...
class AudioGroq(BaseInference):
    def __init__(
        self,
        mode: Literal["transcriptions", "translations"] = "transcriptions",
        model: str = "",
        api_key: str = "",
        base_url: str = "",
    ...
    def invoke(
        self, file_path: str = "", language: str = "en", json: bool = False
    ) -> AIMessage:
        path = Path(file_path)
        ...
        files = {"file": (path.name, self.__read_audio(path), mime_type)}
        try:
            with Client() as client:
                response = client.post(
                    url=url, data=data, files=files, headers=headers, timeout=None
                )
            response.raise_for_status()
            if json:
