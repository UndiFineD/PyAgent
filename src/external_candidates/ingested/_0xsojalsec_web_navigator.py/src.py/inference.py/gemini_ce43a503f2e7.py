        ...
        try:
            with Client() as client:
                response = client.post(
                    url, json=payload, headers=self.headers, timeout=None
                )
    ...
    def invoke(
        ...
        payload = {
            ...
            "generationConfig": {
                "temperature": temperature,
                "responseMimeType": (
                    "application/json" if json or model else "text/plain"
                ),
        ...
        try:
            with Client() as client:
                response = client.post(
                    url=url, headers=self.headers, json=payload, timeout=None
                )
            json_obj = response.json()
            # print(json_obj)
            if json_obj.get("error"):
            ...
            if message["text"]:
                ...
                return ToolMessage(
                    id=str(uuid4()), name=tool_call["name"], args=tool_call["args"]
                )
