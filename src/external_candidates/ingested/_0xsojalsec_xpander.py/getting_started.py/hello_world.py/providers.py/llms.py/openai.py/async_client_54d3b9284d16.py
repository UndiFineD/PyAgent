# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-xpander.ai\Getting-Started\hello-world\providers\llms\openai\async_client.py
"""
...
class AsyncOpenAIProvider(LLMProviderBase):
        ...
        try:
            ...
            if resp.choices[0].message.tool_calls:
                for tool_call in resp.choices[0].message.tool_calls:
                    logger.info(
                        f"🔄 Tool call function name: {tool_call.function.name}"
                    )
                    logger.debug(
                        f"🔄 Tool call function arguments: {tool_call.function.arguments}"
                    )
            return resp
    ...
    def handle_token_accounting(
        self, execution_tokens: Tokens, response: ChatCompletion
    ) -> LLMTokens:
        """
