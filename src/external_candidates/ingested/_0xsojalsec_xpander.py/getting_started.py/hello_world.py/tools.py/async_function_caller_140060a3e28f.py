...
async def execute_local_functions(
    ...
    if len(results) > 1:
        logger.info(
            f"⚙️ Executed {len(results)} functions in {time.time() - start:.2f} s"
        )
...
async def _execute_single_function(
    function_call: Any, functions_by_name: Dict[str, Callable]
) -> ToolCallResult:
    """
    ...
    logger.info(
        f"🔧 Function {function_call.name} completed in {time.time() - tool_start_time:.2f} s"
    )
