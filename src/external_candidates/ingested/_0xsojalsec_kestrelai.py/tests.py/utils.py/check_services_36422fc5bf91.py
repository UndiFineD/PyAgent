# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-KestrelAI\tests\utils\check_services.py
#!/usr/bin/env python3
"""
...
def get_service_config() -> dict[str, dict[str, str]]:
    ...
    return {
        ...
        "Frontend": {"url": "http://localhost:5173", "expected_status": [200]},
        "Ollama": {"url": "http://localhost:11434/api/tags", "expected_status": [200]},
        "SearXNG": {
...
def check_single_service(
    service_name: str, config: dict[str, str], timeout: int = 5
) -> tuple[bool, str]:
...
def check_services_for_testing() -> bool:
    ...
    required_available = all(
        service in available_services for service in required_services
    )
