...
class DNSRecon:
    ...
    def dns_resolver(
        ...
        try:
            ...
            Domain_scans = requests.get(
                f"https://api.hackertarget.com/dnslookup/?q={target}"
            )
            print("✅ Reverse DNS Scanned")
            reverse_dns = requests.get(
                f"https://api.hackertarget.com/reversedns/?q={target}"
            )
            print("✅ Zone Transfer Scanned")
            zone_transfer = requests.get(
                f"https://api.hackertarget.com/zonetransfer/?q={target}"
            )
            self.analyze = f"""
Domain Names:
{Domain_scans.text}
...
"""
        ...
        match AI:
            ...
            case "llama-api":
                try:
                    response = AIModels.llama_AI(
                        self.analyze, "runpod", lkey, lendpoint
                    )
                except KeyboardInterrupt:
                    print("Bye")
                    quit()
