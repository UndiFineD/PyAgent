# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GPT_Vuln-analyzer\components\models.py
import json
import re
...
class JWT_AI_MODEL:
    ...
    def llama_AI(self, jwt_data: str, mode: str, lkey, lendpoint):
        ...
        if mode == "local":
            bot_response = self.chat_with_api(
                api_url, user_message, user_instruction, model_name, file_name
            )
        elif mode == "runpod":
...
def dns_ai_data_regex(json_string: str) -> Any:
    ...
    TXT_pattern = r'"TXT": \["(.*?)"\]'
    Reverse_DNS_pattern = (
        r'"Reverse_DNS": \{ "IP_Address": "(.*?)", "Domain": "(.*?)" \}'
    )
    Zone_Transfer_Scan_pattern = (
        r'"Zone_Transfer_Scan": \{ "Allowed": (.*?), "Name_Servers": \["(.*?)"\] \}'
    )
    ...
    data = {
        ...
        "Reverse_DNS": {"IP_Address": Reverse_DNS_IP, "Domain": Reverse_DNS_Domain},
        "Zone_Transfer_Scan": {
            "Allowed": Zone_Transfer_Allowed,
            "Name_Servers": (
                [Zone_Transfer_Name_Servers] if Zone_Transfer_Name_Servers else []
            ),
        },
    }
