...
class JWTAnalyzer:
    ...
    def analyze(
        ...
        try:
            ...
            header = json.loads(
                base64.urlsafe_b64decode(parts[0] + "===").decode("utf-8", "replace")
            )
            self.algorithm_used = header.get("alg", "Unknown Algorithm")
            payload = json.loads(
                base64.urlsafe_b64decode(parts[1] + "===").decode("utf-8", "replace")
            )
            ...
            match AI:
                ...
                case "llama":
                    try:
                        response = AIModels.llama_AI(
                            str_data, "local", llama_api_token, llama_endpoint
                        )
                    except KeyboardInterrupt:
                        print("Bye")
                        quit()
                case "llama-api":
                    try:
                        response = AIModels.Llama_AI(
                            str_data, "runpod", llama_api_token, llama_endpoint
                        )
                    except KeyboardInterrupt:
                        print("Bye")
                        quit()
