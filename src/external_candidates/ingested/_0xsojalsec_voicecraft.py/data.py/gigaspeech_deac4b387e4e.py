# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\data\gigaspeech.py
import copy
import logging
import os
import random
...
class dataset(torch.utils.data.Dataset):
    def __init__(self, args, split):
        ...
        manifest_fn = os.path.join(
            self.args.dataset_dir, self.args.manifest_name, self.split + ".txt"
        )
        ...
        logging.info(
            f"number of data points for {self.split} split: {len(self.lengths_list)}"
        )
    ...
    def _load_phn_enc(self, index):
        ...
        pf = os.path.join(
            self.args.dataset_dir, self.args.phn_folder_name, item[1] + ".txt"
        )
        ef = os.path.join(
            self.args.dataset_dir, self.args.encodec_folder_name, item[1] + ".txt"
        )
        try:
            with open(pf, "r") as p, open(ef, "r") as e:
                ...
                ]  # drop ["<SIL>", "<MUSIC>", "<NOISE>", "<OTHER>"], as they are not in training set annotation
                encos = [
                    l.strip().split()
    ...
    def __getitem__(self, index):
        ...
        if self.args.drop_long:
            while (
                ...
                or y_len > self.args.encodec_sr * self.args.audio_max_length
            ):
                index = random.choice(range(len(self)))  # regenerate an index
                x, y = self._load_phn_enc(index)
                x_len, y_len = len(x), len(y[0])
        ...
        if (
            audio_start > 0 and len(x) > self.args.text_max_length
        ):  # if audio is longer than max and text is long than max, start text the way audio started
            x = x[int(len(x) * audio_start / orig_y_len) :]
            if (
                len(x) > self.args.text_max_length
            ):  # if text is still longer than max, cut the end
    ...
    def collate(self, batch):
        ...
        if self.args.dynamic_batching:
            if out["y"][0].ndim == 2:
                ...
                res["y"] = torch.nn.utils.rnn.pad_sequence(
                    out["y"], batch_first=True, padding_value=self.args.audio_pad_token
                )
        else:
