# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\data\phonemize_encodec_encode_hf.py
import argparse


def parse_args():
    parser = argparse.ArgumentParser(
        description="encode the librilight dataset using encodec model"
    )
    parser.add_argument(
        "--dataset_size",
        type=str,
        default="xs",
        help="sizes of gigaspeech, xs, s, m, l, xl. we use xl for VoiceCraft training, xs is good for debugging",
    )
    parser.add_argument(
        "--download_to",
        type=str,
        default="/data/scratch/pyp/datasets/gigaspeech_debug",
        help="dir where you want the huggingface gigaspeech dataset to be downloaded to",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/data/scratch/pyp/datasets/gigaspeech_phn_enc_manifest_debug",
        help="path to the manifest, phonemes, and encodec codes dirs",
    )
    parser.add_argument(
        "--encodec_model_path",
        type=str,
    ...
    parser.add_argument(
        "--n_workers", type=int, default=4, help="Number of parallel worker processes"
    )
    parser.add_argument(
        "--mega_batch_size",
        type=int,
    ...
    parser.add_argument(
        "--model_code_sr", type=int, default=50, help="encodec model code sample rate"
    )
    parser.add_argument(
        "--len_cap",
        type=float,
...
if __name__ == "__main__":
    ...
    codes_save_root = os.path.join(
        args.save_dir, args.dataset_size, "encodec_16khz_4codebooks"
    )
    ...
    def sort_by_audio_len(lens):
        ...
        logging.info(
            f"longest: {lens[inds[-1]]*args.model_code_sr} encodec codes, {lens[inds[-1]]:.2f} sec."
        )
        logging.info(
            f"shortest: {lens[inds[0]]*args.model_code_sr} encodec codes, {lens[inds[0]]:.2f} sec."
        )
        logging.info(
    ...
    punc2sym = {
        ...
        " <QUESTIONMARK>": "?",
        " <EXCLAMATIONPOINT>": "!",
    }  # note the space in front of each punc name
    gar2sym = {
        "<SIL>": "#%#",
        "<MUSIC>": "##%",
        "<NOISE>": "%%#",
        "<OTHER>": "%#%",
    }  # so that they are savely keep as the original sym when using tokenize_text
    ...
    gs = load_dataset(
        ...
        args.dataset_size,
        use_auth_token=True,
        cache_dir=args.download_to,
        download_config=dc,
    )
    logging.info(
        f"time spend on loading the dataset: {time.time() - stime:.2f} seconds"
    )
    ...
    for split in tqdm.tqdm(splits):
        ...
        for item in tqdm.tqdm(gs[split]):
            ...
            if sum(word in forbidden_words for word in text.split(" ")):
                logging.info(
                    f"skip {item['segment_id']}, because it contains forbiden words. It's transcript: {text}"
                )
                skip += 1
                continue
            ...
            with open(save_fn, "w") as f:
                f.write(phn_seq)
        logging.info(
            f"split {split} has {len(gs[split])} samples in total, skipped {skip} due to forbiden words"
        )
    ...
    class mydataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, ind):
            try:
                segment_id, audio, sr, text, begin_time, end_time = (
                    self.data[ind]["segment_id"],
                    torch.from_numpy(self.data[ind]["audio"]["array"]).float(),
                    self.data[ind]["audio"]["sampling_rate"],
                    self.data[ind]["text"],
                    self.data[ind]["begin_time"],
                    self.data[ind]["end_time"],
                )
            except:
    ...
    train_loader = torch.torch.utils.data.DataLoader(
        train_dataset,
    ...
    validation_loader = torch.torch.utils.data.DataLoader(
        validation_dataset,
        batch_size=args.mega_batch_size,
        shuffle=False,
        drop_last=False,
    ...
    test_loader = torch.torch.utils.data.DataLoader(
        test_dataset,
        batch_size=args.mega_batch_size,
        shuffle=False,
        drop_last=False,
    ...
    for split, loader in zip(splits, loaders):
        ...
        logging.info(
            f"partition the split {split} into {mega_n_steps} parts, each has {args.mega_batch_size} samples"
        )
        for m, mega_batch in enumerate(loader):
            logging.info(f"====================================")
            logging.info(f"====================================")
            logging.info(f"now processing mega step {m+1}/{mega_n_steps}")
            lengths = np.array(mega_batch["end_time"]) - np.array(
                mega_batch["begin_time"]
            )
            sorted_inds = sort_by_audio_len(lengths)
            for j in range(len(sorted_inds))[::-1]:
                if (
                    lengths[sorted_inds[j]] < 0.2
                    or lengths[sorted_inds[j]] > args.len_cap
            ...
            for n in tqdm.tqdm(range(n_steps), disable=True):
                ...
                with torch.no_grad():
                    if (
                        max(all_lens) > args.max_len and len(all_lens) > 1
                    ):  # NOTE decrease args.max_len if OOM, or chunk it into more than 2 forward passes
                        codes = []
                ...
                for i, length in enumerate(all_lens):
                    save_fn = os.path.join(
                        codes_save_root, segment_id_batch[i] + ".txt"
                    )
                    actual_len = round(
                        length * args.model_code_sr
