# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VoiceCraft\steps\trainer.py
import logging
import math
import os
import pickle
...
class Trainer:
    ...
    def __init__(self, args, world_size, rank):
        ...
        self.model, self.trainables, self.optim_states, self.scheduler_states = (
            self._setup_models()
        )
        ...
        if self.rank == 0:
            ...
            if self.args.dynamic_batching:
                ...
                logging.info(
                    f"batch size (summed over all GPUs): {self.args.batch_size}"
                )
        ...
        while flag:
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                for j in range(self.args.gradient_accumulation_steps):
                    ...
                    is_nan = torch.tensor(
                        int(torch.isnan(record_loss).any()),
                        dtype=torch.float32,
                        device=self.rank,
                    )
                    ...
                    if is_nan.item() > 0:
                        logging.info(
                            f"loss at step {self.progress['step']} is nan, therefore skip this batch"
                        )
                        skip_flag = True
                        continue
                    ...
                    if self.rank == 0:
                        ...
                        if (
                            self.progress["step"] % self.args.tb_write_every_n_steps
                            == 0
                        ):
    ...
    def _setup_dataloader(self):
        ...
        train_dataset, val_dataset = gigaspeech.dataset(
            self.args, "train"
        ), gigaspeech.dataset(self.args, "validation")
