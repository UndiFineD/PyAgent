# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\api.py
...
class TTS(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if gpu:
            warnings.warn(
                "`gpu` will be deprecated. Please use `tts.to(device)` instead."
            )
    ...
    def is_multi_speaker(self) -> bool:
        if self.synthesizer is not None:
            if self.synthesizer.tts_model.config.supports_cloning:
                return True
            if (
                hasattr(self.synthesizer.tts_model, "speaker_manager")
    ...
    def is_multi_lingual(self) -> bool:
        ...
        if (
            ...
            or self.config
            and (
                "xtts" in self.config.model
                or "languages" in self.config
                and len(self.config.languages) > 1
            )
        ):
            return True
        if (
    ...
    def speakers(self) -> list[str] | None:
        ...
        if self.synthesizer.tts_model.config.supports_cloning:
            speakers.extend(
                self.synthesizer.tts_model.get_voices(self.synthesizer.voice_dir).keys()
            )
    ...
    def list_models() -> list[str]:
        return ModelManager(
            models_file=TTS.get_models_file_path(), progress_bar=False
        ).list_models()
    ...
    def load_model_by_name(
        self, model_name: str, vocoder_name: str | None = None, *, gpu: bool = False
    ) -> None:
    ...
    def load_vc_model_by_name(
        ...
        model_path, config_path, vocoder_path, vocoder_config_path, model_dir = (
            self.download_model_by_name(model_name, vocoder_name)
        )
    ...
    def load_tts_model_by_path(
        self, model_path: str, config_path: str, *, gpu: bool = False
    ) -> None:
    ...
    def _check_arguments(
        ...
        **kwargs,
        ...
        if emotion is not None:
            raise ValueError(
                "Emotion can only be used with Coqui Studio models. Which is discontinued."
            )
    ...
    def voice_conversion(
        ...
        **kwargs,
        ...
        if self.synthesizer is not None and hasattr(
            self.synthesizer.tts_model, "voice_conversion"
        ):
    ...
    def tts_with_vc(
        ...
        *,
        ...
        if self.voice_converter is None:
            self.load_vc_model_by_name(
                "voice_conversion_models/multilingual/vctk/freevc24"
            )
        wav = self.voice_converter.voice_conversion(
