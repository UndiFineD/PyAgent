...
def build_mel_basis(
    ...
    **kwargs,
    ...
    return librosa.filters.mel(
        sr=sample_rate, n_fft=fft_size, n_mels=num_mels, fmin=mel_fmin, fmax=mel_fmax
    )


def millisec_to_length(
    *, frame_length_ms: float, frame_shift_ms: float, sample_rate: int, **kwargs
) -> tuple[int, int]:
...
def amp_to_db(
    *, x: np.ndarray, gain: float = 1, base: float = 10, **kwargs
) -> np.ndarray:
...
def mel_to_wav(
    *, mel: np.ndarray, mel_basis: np.ndarray, power: float = 1.5, **kwargs
) -> np.ndarray:
...
def istft(
    ...
    **kwargs,
    ...
    return librosa.istft(
        y, hop_length=hop_length, win_length=win_length, center=center, window=window
    )
...
def compute_stft_paddings(
    *, x: np.ndarray, hop_length: int, pad_two_sides: bool = False, **kwargs
) -> tuple[int, int]:
...
def compute_f0(
    ...
    **kwargs,
    ...
    if sample_rate / pitch_fmin >= win_length - 1:
        logger.warning(
            "pitch_fmin=%.2f is too small for win_length=%d", pitch_fmin, win_length
        )
...
def trim_silence(
    ...
    **kwargs,
    ...
    return librosa.effects.trim(
        wav, top_db=trim_db, frame_length=win_length, hop_length=hop_length
    )[0]
