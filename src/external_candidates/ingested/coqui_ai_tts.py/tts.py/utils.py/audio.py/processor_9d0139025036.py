# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\utils\audio\processor.py
import logging
import os
...
class AudioProcessor:
    ...
    def spectrogram(self, y: np.ndarray) -> np.ndarray:
        ...
        S = (
            amp_to_db(x=np.abs(D), gain=self.spec_gain, base=self.base)
    ...
    ### save and load ###
    def load_wav(
        self, filename: str | os.PathLike[Any], sr: int | None = None
    ) -> np.ndarray:
        ...
        if sr is not None:
            ...
            x = load_wav(
                filename=filename, sample_rate=self.sample_rate, resample=self.resample
            )
