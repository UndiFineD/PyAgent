# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\utils\capacitron_optimizer.py
...
class CapacitronOptimizer:
    ...
    def __init__(self, config: dict, model_params: Generator) -> None:
        self.primary_params, self.secondary_params = self.split_model_parameters(
            model_params
        )
    ...
    def state_dict(self):
        return [
            self.primary_optimizer.state_dict(),
            self.secondary_optimizer.state_dict(),
        ]
