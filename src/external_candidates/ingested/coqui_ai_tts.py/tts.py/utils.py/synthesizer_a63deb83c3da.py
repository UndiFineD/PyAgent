# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\utils\synthesizer.py
import logging
import os
import time
...
class Synthesizer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.voice_dir = (
            Path(voice_dir) if voice_dir is not None else self.checkpoint_dir / "voices"
        )
    ...
    def _load_fairseq_from_dir(self, model_dir: str, use_cuda: bool) -> None:
        ...
        self.tts_model.load_fairseq_checkpoint(
            self.tts_config, checkpoint_dir=model_dir, eval=True
        )
    ...
    def save_wav(
        self, wav: list[int] | torch.Tensor | np.ndarray, path: str, pipe_out=None
    ) -> None:
        ...
        if isinstance(wav, list):
            wav = np.array(wav)
        save_wav(
            wav=wav, path=path, sample_rate=self.output_sample_rate, pipe_out=pipe_out
        )
    ...
    def tts(
        ...
        **kwargs,
        ...
        voice_dir = (
            Path(d)
            if (d := kwargs.pop("voice_dir", None)) is not None
            else self.voice_dir
        )
        vocoder_device = "cpu"
        ...
        if not source_wav:  # not voice conversion
            for sen in sens:
                ...
                if (
                    isinstance(waveform, torch.Tensor)
            ...
            outputs = self.tts_model.voice_conversion(
                source_wav,
                speaker_wav,
                source_speaker=source_speaker_name,
                speaker=speaker_name,
                voice_dir=voice_dir,
            )
            waveform = outputs
            ...
            if isinstance(waveform, torch.Tensor) and waveform.device != torch.device(
                "cpu"
            ):
                waveform = waveform.cpu()
            if not use_gl:
