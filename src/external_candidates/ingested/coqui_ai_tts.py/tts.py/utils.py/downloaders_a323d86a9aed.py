# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\utils\downloaders.py
import logging
import os
...
def download_vctk(path: str, use_kaggle: bool | None = False):
    ...
    if use_kaggle:
        download_kaggle_dataset(
            "mfekadu/english-multispeaker-corpus-for-voice-cloning", "VCTK", path
        )
    else:
        os.makedirs(path, exist_ok=True)
...
def download_tweb(path: str):
    ...
    """
    download_kaggle_dataset(
        "bryanpark/the-world-english-bible-speech-dataset", "TWEB", path
    )
