# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\utils\manage.py
import json
import logging
import os
import re
...
class ModelManager:
    ...
    def log_model_details(
        self, model_type: str, lang: str, dataset: str, model: str
    ) -> None:
        ...
        if "description" in self.models_dict[model_type][lang][dataset][model]:
            logger.info(
                "Description: %s",
                self.models_dict[model_type][lang][dataset][model]["description"],
            )
        else:
    ...
    def model_info_by_idx(self, model_query: str) -> None:
        ...
        try:
            ...
            if model_query_idx <= 0:
                logger.error(
                    "model_query_idx [%d] should be a positive integer!",
                    model_query_idx,
                )
                return
        ...
        if model_query_idx > model_count:
            logger.error(
                "model_query_idx exceeds the number of available models [%d]",
                model_count,
            )
        else:
    ...
    def model_info_by_full_name(self, model_query_name: str) -> None:
        ...
        if dataset not in self.models_dict[model_type][lang]:
            logger.error(
                "Dataset %s does not exist for %s/%s.", dataset, model_type, lang
            )
            return
        if model not in self.models_dict[model_type][lang][dataset]:
            logger.error(
                "Model %s does not exist for %s/%s/%s.",
                model,
                model_type,
                lang,
                dataset,
        ...
        if "license" in model_item and model_item["license"].strip() != "":
            ...
            if model_item["license"].lower() in LICENSE_URLS:
                logger.info(
                    ...
                    LICENSE_URLS[model_item["license"].lower()],
                )
            else:
    ...
    def _download_hf_model(self, model_item: ModelItem, output_path: Path) -> None:
        if isinstance(model_item["hf_url"], list):
            self._download_model_files(
                model_item["hf_url"], output_path, self.progress_bar
            )
        else:
            self._download_zip_file(
                model_item["hf_url"], output_path, self.progress_bar
    ...
    def _set_model_item(
        self, model_name: str
    ) -> tuple[ModelItem, str, str, str | None]:
    ...
    def create_dir_and_download_model(
        self, model_name: str, model_item: ModelItem, output_path: Path
    ) -> None:
    ...
    def check_if_configs_are_equal(
        self, model_name: str, model_item: ModelItem, output_path: Path
    ) -> None:
        ...
        if not config_local == config_remote:
            logger.info(
                "%s is already downloaded however it has been changed. Redownloading it...",
                model_name,
            )
    ...
    def download_model(self, model_name: str) -> tuple[Path, Path | None, ModelItem]:
                ...
                if md5sum_file.is_file():
                    with md5sum_file.open() as f:
                        if not f.read() == md5sum:
                            logger.info(
                                "%s has been updated, clearing model cache...",
                                model_name,
                            )
                            self.create_dir_and_download_model(
                                model_name, model_item, output_path
                            )
                        else:
                            logger.info("%s is already downloaded.", model_name)
                else:
                    logger.info(
                        "%s has been updated, clearing model cache...", model_name
                    )
                    self.create_dir_and_download_model(
                        model_name, model_item, output_path
                    )
        ...
        if (
            model not in ["tortoise-v2", "bark", "knnvc"]
            and "fairseq" not in model_name
            and "xtts" not in model_name
    ...
    def _download_zip_file(
        file_url: str, output_folder: Path, progress_bar: bool
    ) -> None:
    ...
    def _download_model_files(
        file_urls: list[str], output_folder: str | os.PathLike[Any], progress_bar: bool
    ) -> None:
        """Download the github releases"""
