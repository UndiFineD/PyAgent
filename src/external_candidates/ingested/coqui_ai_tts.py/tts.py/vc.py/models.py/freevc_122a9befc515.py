# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vc\models\freevc.py
import logging
import os
...
from TTS.vc.layers.freevc.speaker_encoder.speaker_encoder import (
    SpeakerEncoder as SpeakerEncoderEx,
)
...
class FreeVC(CloningMixin, BaseVC):
    ...
    def __init__(self, config: Coqpit, speaker_manager: SpeakerManager = None):
        ...
        self.enc_p = Encoder(
            self.args.ssl_dim, self.inter_channels, self.hidden_channels, 5, 1, 16
        )
        self.dec = Generator(
            self.inter_channels,
            self.resblock,
        ...
        if not self.use_spk:
            self.enc_spk = SpeakerEncoder(
                model_hidden_size=self.gin_channels,
                model_embedding_size=self.gin_channels,
            )
        else:
    ...
    def _extract_target_se(
        self, speaker_wav: list[str | os.PathLike[Any] | torch.Tensor]
    ):
        target_ses = []
        for wav in speaker_wav:
            ...
            if self.config.model_args.use_spk:
                target_ses.append(
                    self.enc_spk_ex.embed_utterance(wav_tgt)[None, :, None]
                )
            else:
                ...
                target_ses.append(
                    self.enc_spk.embed_utterance(mel_tgt.transpose(1, 2)).unsqueeze(-1)
                )
