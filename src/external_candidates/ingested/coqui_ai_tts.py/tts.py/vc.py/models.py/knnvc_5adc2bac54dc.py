# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vc\models\knnvc.py
import logging
import os
...
class KNNVC(CloningMixin, BaseVC):
    ...
    def get_features(self, audio: PathOrTensor, vad_trigger_level=0) -> torch.Tensor:
        ...
        if not sr == self.config.audio.sample_rate:
            logger.info(
                "Resampling %d to %d in %s", sr, self.config.audio.sample_rate, audio
            )
            x = torchaudio.functional.resample(
                x, orig_freq=sr, new_freq=self.config.audio.sample_rate
            )
            sr = self.config.audio.sample_rate
        ...
        if vad_trigger_level > 1e-3:
            transform = torchaudio.transforms.Vad(
                sample_rate=sr, trigger_level=vad_trigger_level
            )
        ...
        features = self.wavlm.extract_features(
            wav_input_16khz,
            output_layer=self.config.wavlm_layer,
            ret_layer_results=False,
        )[0]
        return features.squeeze(0)

    def get_matching_set(
        self, wavs: list[PathOrTensor], vad_trigger_level=7
    ) -> torch.Tensor:
    ...
    def match(
        ...
        synth_set = (
            matching_set.to(self.device)
            if synth_set is None
            else synth_set.to(self.device)
        )
