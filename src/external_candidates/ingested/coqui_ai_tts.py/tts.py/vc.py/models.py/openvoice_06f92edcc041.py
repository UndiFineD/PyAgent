# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vc\models\openvoice.py
import json
import logging
import os
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(
        self, spec_channels: int, embedding_dim: int = 0, layernorm: bool = True
    ) -> None:
        super().__init__()
...
class OpenVoice(CloningMixin, BaseVC):
    ...
    def __init__(
        self, config: Coqpit, speaker_manager: SpeakerManager | None = None
    ) -> None:
    ...
    def load_checkpoint(
        ...
        *,
        ...
        super().load_checkpoint(
            config, checkpoint_path, eval=eval, strict=strict, cache=cache
        )
    ...
    @staticmethod
    def _set_x_lengths(
        x: torch.Tensor, aux_input: Mapping[str, torch.Tensor | None]
    ) -> torch.Tensor:
    ...
    def inference(
        self,
        x: torch.Tensor,
        aux_input: Mapping[str, torch.Tensor | None] = {
            "x_lengths": None,
            "g_src": None,
            "g_tgt": None,
        },
    ...
    def load_audio(
        ...
        if isinstance(wav, (str, os.PathLike)):
            out = torch.from_numpy(
                librosa.load(wav, sr=self.config.audio.input_sample_rate)[0]
            )
    ...
    def extract_se(
        self, audio: str | os.PathLike[Any] | torch.Tensor
    ...
    def _extract_target_se(
        self, tgt: list[str | os.PathLike[Any] | torch.Tensor]
    ) -> torch.Tensor:
        tgt_ses = []
        for tg in tgt:
