...
class BaseVocoderConfig(BaseTrainingConfig):
    ...
    # dataloading
    use_noise_augment: bool = (
        False  # enable/disable random noise augmentation in spectrograms.
    )
    eval_split_size: int = 10  # number of samples used for evaluation.
    # dataset
    data_path: str = (
        ""  # root data path. It finds all wav files recursively from there.
    )
    ...
    use_cache: bool = (
        False  # use in memory cache to keep the computed features. This might cause OOM.
    )
    # OPTIMIZER
...
class BaseGANVocoderConfig(BaseVocoderConfig):
    ...
    target_loss: str = (
        "loss_0"  # loss value to pick the best model to save after each epoch
    )
    ...
    lr_scheduler_gen: str = (
        "ExponentialLR"  # one of the schedulers from https:#pytorch.org/docs/stable/optim.html
    )
    ...
    use_pqmf: bool = (
        False  # enable/disable using pqmf for multi-band training. (Multi-band MelGAN)
    )
    steps_to_start_discriminator: int = (
        0  # start training the discriminator after this number of steps.
    )
    diff_samples_for_G_and_D: bool = (
        False  # use different samples for G and D training steps.
    )
