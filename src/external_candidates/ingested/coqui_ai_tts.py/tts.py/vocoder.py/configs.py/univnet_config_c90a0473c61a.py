# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\configs\univnet_config.py
...
class UnivnetConfig(BaseGANVocoderConfig):
    ...
    use_feat_match_loss: bool = (
        False  # requires MelGAN Discriminators (MelGAN and univnet)
    )
    ...
    lr_scheduler_gen: str = (
        None  # one of the schedulers from https:#pytorch.org/docs/stable/optim.html
    )
    ...
    optimizer_params: dict = field(
        default_factory=lambda: {"betas": [0.5, 0.9], "weight_decay": 0.0}
    )
