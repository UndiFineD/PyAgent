# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\configs\wavegrad_config.py
...
class WavegradConfig(BaseVocoderConfig):
    ...
    target_loss: str = (
        "loss"  # loss value to pick the best model to save after each epoch
    )
    ...
    train_noise_schedule: dict = field(
        default_factory=lambda: {"min_val": 1e-6, "max_val": 1e-2, "num_steps": 1000}
    )
