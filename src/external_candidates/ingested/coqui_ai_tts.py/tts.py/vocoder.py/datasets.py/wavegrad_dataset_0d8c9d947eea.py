# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\datasets\wavegrad_dataset.py
import glob
import os
import random
...
class WaveGradDataset(Dataset):
    ...
    def __init__(
        ...
        if return_segments:
            assert (
                seq_len % hop_len == 0
            ), " [!] seq_len has to be a multiple of hop_len."
        ...
        if self.use_cache and self.cache[idx] is not None:
            ...
            if self.return_segments:
                ...
                if audio.shape[-1] < self.seq_len + self.pad_short:
                    audio = np.pad(
                        audio,
                        (0, self.seq_len + self.pad_short - len(audio)),
                        mode="constant",
                        constant_values=0.0,
                    )
                assert (
