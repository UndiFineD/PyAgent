...
class GBlock(nn.Module):
    def __init__(self, in_channels, cond_channels, downsample_factor):
        ...
        self.end = nn.Sequential(
            nn.ReLU(),
            nn.Conv1d(
                in_channels * 2, in_channels * 2, kernel_size=3, dilation=2, padding=2
            ),
        )
...
class DBlock(nn.Module):
    def __init__(self, in_channels, out_channels, downsample_factor):
        ...
        self.donwsample_layer = nn.AvgPool1d(
...
class RandomWindowDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        # check arguments
        assert (
            len(cond_disc_downsample_factors)
            == len(cond_disc_out_channels)
            == len(window_sizes)
        )
    ...
    def forward(self, x, c):
        ...
        feats = []
        # unconditional pass
        for window_size, layer in zip(
            self.window_sizes, self.unconditional_discriminators
        ):
        ...
        # conditional pass
        for window_size, layer in zip(
            self.window_sizes, self.conditional_discriminators
        ):
