# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\models\wavernn.py
import sys
import time
...
class Wavernn(BaseVocoder):
    def __init__(self, config: Coqpit):
        ...
        if self.args.use_aux_net:
            self.I = nn.Linear(
                self.args.feat_dims + self.aux_dims + 1, self.args.rnn_dims
            )
            self.rnn1 = nn.GRU(self.args.rnn_dims, self.args.rnn_dims, batch_first=True)
            self.rnn2 = nn.GRU(
                self.args.rnn_dims + self.aux_dims, self.args.rnn_dims, batch_first=True
            )
