...
class WavegradArgs(Coqpit):
    ...
    ublock_out_channels: list[int] = field(
        default_factory=lambda: [512, 512, 256, 128, 128]
    )
...
class Wavegrad(BaseVocoder):
    ...
    def forward(self, x, spectrogram, noise_scale):
        ...
        for layer, (film_shift, film_scale) in zip(
            self.ublocks, reversed(shift_and_scale)
        ):
    ...
    def load_noise_schedule(self, path):
        beta = np.load(path, allow_pickle=True).item()[
            "beta"
    ...
    def inference(self, x, y_n=None):
        ...
        for n in range(len(self.alpha) - 1, -1, -1):
            y_n = self.c1[n] * (
                y_n
                - self.c2[n]
                * self.forward(y_n, x, sqrt_alpha_hat[n].repeat(x.shape[0]))
            )
            if n > 0:
    ...
    def compute_noise_level(self, beta):
        ...
        self.sigma = (
            (1.0 - self.alpha_hat[:-1]) / (1.0 - self.alpha_hat[1:]) * self.beta[1:]
        ) ** 0.5
    ...
    def test(
        self, assets: dict, test_loader: "DataLoader", outputs=None
    ...
    def get_data_loader(
        ...
        loader = DataLoader(
            ...
            sampler=sampler,
            num_workers=(
                self.config.num_eval_loader_workers
                if is_eval
