# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\layers\losses.py
import torch
...
def _apply_D_loss(scores_fake, scores_real, loss_func):
    ...
    if isinstance(scores_fake, list):
        ...
        for score_fake, score_real in zip(scores_fake, scores_real):
            total_loss, real_loss_, fake_loss_ = loss_func(
...
class GeneratorLoss(nn.Module):
    ...
    def __init__(self, C):
        ...
        self.use_subband_stft_loss = (
            C.use_subband_stft_loss if "use_subband_stft_loss" in C else False
        )
        self.use_mse_gan_loss = C.use_mse_gan_loss if "use_mse_gan_loss" in C else False
        self.use_hinge_gan_loss = (
            C.use_hinge_gan_loss if "use_hinge_gan_loss" in C else False
        )
        self.use_feat_match_loss = (
            C.use_feat_match_loss if "use_feat_match_loss" in C else False
        )
        ...
        self.subband_stft_loss_weight = (
            C.subband_stft_loss_weight if "subband_stft_loss_weight" in C else 0.0
        )
        self.mse_gan_loss_weight = (
            C.mse_G_loss_weight if "mse_G_loss_weight" in C else 0.0
        )
        self.hinge_gan_loss_weight = (
            C.hinge_G_loss_weight if "hinde_G_loss_weight" in C else 0.0
        )
        self.feat_match_loss_weight = (
            C.feat_match_loss_weight if "feat_match_loss_weight" in C else 0.0
        )
        self.l1_spec_loss_weight = (
            C.l1_spec_loss_weight if "l1_spec_loss_weight" in C else 0.0
        )
    ...
    def forward(
        ...
        if self.use_subband_stft_loss:
            subband_stft_loss_mg, subband_stft_loss_sc = self.subband_stft_loss(
                y_hat_sub, y_sub
            )
            return_dict["G_subband_stft_loss_mg"] = subband_stft_loss_mg
            return_dict["G_subband_stft_loss_sc"] = subband_stft_loss_sc
            gen_loss = gen_loss + self.subband_stft_loss_weight * (
