# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\layers\hifigan.py
...
class ResStack(nn.Module):
    def __init__(self, kernel, channel, padding, dilations=[1, 3, 5]):
        ...
        self.shortcut = nn.utils.parametrizations.weight_norm(
            nn.Conv1d(channel, channel, kernel_size=1)
        )
...
class MRF(nn.Module):
    def __init__(
        self, kernels, channel, dilations=[1, 3, 5]
    ):  # # pylint: disable=dangerous-default-value
        super().__init__()
