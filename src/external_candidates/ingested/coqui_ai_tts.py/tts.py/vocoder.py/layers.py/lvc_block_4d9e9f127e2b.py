# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\vocoder\layers\lvc_block.py
import torch
...
class KernelPredictor(torch.nn.Module):
    ...
    def __init__(  # pylint: disable=dangerous-default-value
        ...
        self.input_conv = torch.nn.Sequential(
            torch.nn.Conv1d(
                cond_channels, kpnet_hidden_channels, 5, padding=(5 - 1) // 2, bias=True
            ),
            getattr(torch.nn, kpnet_nonlinear_activation)(
        ...
        self.kernel_conv = torch.nn.Conv1d(
            kpnet_hidden_channels, l_w, kpnet_conv_size, padding=padding, bias=True
        )
        self.bias_conv = torch.nn.Conv1d(
            kpnet_hidden_channels, l_b, kpnet_conv_size, padding=padding, bias=True
        )
...
class LVCBlock(torch.nn.Module):
    ...
    def forward(self, x, c):
        ...
        for i in range(self.conv_layers):
            ...
            x = x + torch.sigmoid(y[:, :in_channels, :]) * torch.tanh(
                y[:, in_channels:, :]
            )
        return x
