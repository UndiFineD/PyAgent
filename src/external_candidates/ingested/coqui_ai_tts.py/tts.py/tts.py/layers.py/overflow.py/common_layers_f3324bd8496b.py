...
class Encoder(nn.Module):
    ...
    def __init__(
        self, num_chars, state_per_phone, in_out_channels=512, n_convolutions=3
    ):
        super().__init__()
...
class ParameterModel(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                Linear(inp, out)
                for inp, out in zip([input_size] + outputnet_size[:-1], outputnet_size)
            ]
        )
    ...
    def flat_start_output_layer(self, mean, std, transition_p):
        ...
        self.last_layer.bias.data[self.frame_channels : 2 * self.frame_channels] = (
            OverflowUtils.inverse_softplus(std)
        )
        self.last_layer.bias.data[2 * self.frame_channels :] = (
            OverflowUtils.inverse_sigmod(transition_p)
        )
...
class OverflowUtils:
    ...
    def get_data_parameters_for_flat_start(
        ...
        data_std = torch.sqrt(
            (total_mel_sq_sum / (total_mel_len * out_channels))
            - torch.pow(data_mean, 2)
        )
    ...
    def update_flat_start_transition(model, transition_p):
        model.neural_hmm.output_net.parametermodel.flat_start_output_layer(
            0.0, 1.0, transition_p
        )
    ...
    def double_pad(list_of_different_shape_tensors):
        ...
        """
        second_dim_lens = [
            len(a) for a in [i[0] for i in list_of_different_shape_tensors]
        ]
        second_dim_max = max(second_dim_lens)
        padded_x = [
            F.pad(x, (0, second_dim_max - len(x[0])))
            for x in list_of_different_shape_tensors
        ]
