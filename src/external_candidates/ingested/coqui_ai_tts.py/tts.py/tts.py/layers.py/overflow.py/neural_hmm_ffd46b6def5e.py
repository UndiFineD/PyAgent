...
class NeuralHMM(nn.Module):
    ...
    def forward(self, inputs, inputs_len, mels, mel_lens):
        ...
        log_c, log_alpha_scaled, transition_matrix, means = (
            self._initialize_forward_algorithm_variables(mels, N)
        )

        # Initialize autoregression elements
        ar_inputs = self._add_go_token(mels)
        h_memory, c_memory = self._init_lstm_states(
            batch_size, self.memory_rnn_dim, mels
        )

        for t in range(T_max):
            # Process Autoregression
            h_memory, c_memory = self._process_ar_timestep(
                t, ar_inputs, h_memory, c_memory
            )
            ...
            if self.use_grad_checkpointing and self.training:
                ...
                mean, std, transition_vector = checkpoint(
                    self.output_net, h_memory, inputs, use_reentrant=True
                )
            else:
            ...
            if t == 0:
                log_alpha_temp = log_state_priors + self.emission_model(
                    mels[:, 0], mean, std, inputs_len
                )
            else:
                log_alpha_temp = self.emission_model(
                    mels[:, t], mean, std, inputs_len
            ...
            transition_matrix[:, t] = (
                transition_vector  # needed for absorption state calculation
            )
    ...
    def get_absorption_state_scaling_factor(
        self, mels_len, log_alpha_scaled, inputs_len, transition_vector
    ):
        ...
        last_transition_vector = torch.gather(
            transition_vector, 1, last_log_alpha_scaled_index
        ).squeeze(1)
        last_transition_probability = torch.sigmoid(last_transition_vector)
        log_probability_of_transitioning = OverflowUtils.log_clamped(
            last_transition_probability
        )

        last_transition_probability_index = self.get_mask_for_last_item(
            inputs_len, inputs_len.device
        )
    ...
    def inference(
        ...
        outputs["hmm_outputs"] = nn.utils.rnn.pad_sequence(
            outputs["hmm_outputs"], batch_first=True
        )
    ...
    @torch.inference_mode()
    def sample(
        self, inputs, input_lens, sampling_temp, max_sampling_time, duration_threshold
    ):
        ...
        prenet_input = self.go_tokens.unsqueeze(0).expand(
            1, self.ar_order, self.frame_channels
        )
        h_memory, c_memory = self._init_lstm_states(
            1, self.memory_rnn_dim, prenet_input
        )
        ...
        while True:
            ...
            h_memory, c_memory = self.memory_rnn(
                memory_input.squeeze(0), (h_memory, c_memory)
            )
            ...
            if (current_state == input_lens) or (
                max_sampling_time and t == max_sampling_time - 1
            ):
                break
...
class TransitionModel(nn.Module):
    ...
    def forward(
        self, log_alpha_scaled, transition_vector, inputs_len
        ...
        out = out.masked_fill(
            ~inputs_len_mask, -float("inf")
        )  # There are no states to contribute to the loss
        return out
...
class EmissionModel(nn.Module):
    ...
    def sample(self, means, stds, sampling_temp):
        return (
            self.distribution_function(means, stds * sampling_temp).sample()
            if sampling_temp > 0
            else means
        )
