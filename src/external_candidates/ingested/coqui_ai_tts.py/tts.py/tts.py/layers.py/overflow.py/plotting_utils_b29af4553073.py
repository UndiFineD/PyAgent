# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\overflow\plotting_utils.py
...
def get_spec_from_most_probable_state(log_alpha_scaled, means, decoder=None):
    ...
    max_state_numbers = (
        max_state_numbers.unsqueeze(1).unsqueeze(1).expand(max_len, 1, n_mel_channels)
    )
    means = (
        torch.gather(means, 1, max_state_numbers).squeeze(1).to(log_alpha_scaled.dtype)
    )
    if decoder is not None:
        mel = (
            decoder(
                means.T.unsqueeze(0),
                torch.tensor([means.shape[0]], device=means.device),
                reverse=True,
            )[0]
            .squeeze(0)
            .T
...
def plot_transition_probabilities_to_numpy(
    states, transition_probabilities, output_fig=False
):
