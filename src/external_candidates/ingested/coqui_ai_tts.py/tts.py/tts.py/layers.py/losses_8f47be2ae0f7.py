# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\losses.py
import logging
import math
...
class SSIMLoss(torch.nn.Module):
    ...
    def forward(self, y_hat, y, length):
        ...
        if ssim_loss.item() > 1.0:
            logger.info(
                "SSIM loss is out-of-range (%.2f), setting it to 1.0", ssim_loss.item()
            )
            ssim_loss = torch.tensor(1.0, device=ssim_loss.device)

        if ssim_loss.item() < 0.0:
            logger.info(
                "SSIM loss is out-of-range (%.2f), setting it to 0.0", ssim_loss.item()
            )
...
class GuidedAttentionLoss(torch.nn.Module):
    ...
    def _make_ga_mask(ilen, olen, sigma):
        grid_x, grid_y = torch.meshgrid(
            torch.arange(olen).to(olen), torch.arange(ilen).to(ilen), indexing="ij"
        )
...
class Huber(nn.Module):
    ...
    def forward(self, x, y, length=None):
        ...
        """
        mask = (
            sequence_mask(sequence_length=length, max_len=y.size(1))
            .unsqueeze(2)
            .float()
        )
        return (
            torch.nn.functional.smooth_l1_loss(x * mask, y * mask, reduction="sum")
            / mask.sum()
        )
...
class TacotronLoss(torch.nn.Module):
    ...
    def __init__(self, c, ga_sigma=0.4):
        ...
        if c.postnet_diff_spec_alpha > 0 or c.decoder_diff_spec_alpha > 0:
            self.criterion_diff_spec = DifferentialSpectralLoss(
        ...
        self.criterion_st = (
            BCELossMasked(pos_weight=torch.tensor(self.stopnet_pos_weight))
            if c.stopnet
            else None
        )
    ...
    def forward(
        ...
        postnet_target = (
            linear_input if self.config.model.lower() in ["tacotron"] else mel_input
        )
        ...
        if self.config.loss_masking:
            ...
            if self.postnet_alpha > 0:
                postnet_loss = self.criterion(
                    postnet_output, postnet_target, output_lens
                )
        else:
            if self.decoder_alpha > 0:
        ...
        if self.use_capacitron_vae:
            ...
            return_dict["capaciton_reconstruction_loss"] = (
                self.criterion_capacitron_reconstruction_loss(decoder_output, mel_input)
        ...
        if self.config.bidirectional_decoder:
            if self.config.loss_masking:
                decoder_b_loss = self.criterion(
                    torch.flip(decoder_b_output, dims=(1,)), mel_input, output_lens
                )
            else:
                decoder_b_loss = self.criterion(
        ...
        if self.config.double_decoder_consistency:
            if self.config.loss_masking:
                decoder_b_loss = self.criterion(
                    decoder_b_output, mel_input, output_lens
                )
            else:
        ...
        if self.config.decoder_diff_spec_alpha > 0:
            decoder_diff_spec_loss = self.criterion_diff_spec(
                decoder_output, mel_input, output_lens
            )
        ...
        if self.config.postnet_diff_spec_alpha > 0:
            postnet_diff_spec_loss = self.criterion_diff_spec(
                postnet_output, postnet_target, output_lens
            )
        ...
        if self.config.decoder_ssim_alpha > 0:
            decoder_ssim_loss = self.criterion_ssim(
                decoder_output, mel_input, output_lens
            )
        ...
        if self.config.postnet_ssim_alpha > 0:
            postnet_ssim_loss = self.criterion_ssim(
                postnet_output, postnet_target, output_lens
            )
...
class GlowTTSLoss(torch.nn.Module):
    ...
    def forward(
        self, z, means, scales, log_det, y_lengths, o_dur_log, o_attn_dur, x_lengths
    ):
        return_dict = {}
        ...
        for t in range(1, T_mel):
            prev_step = torch.cat(
                [
                    log_alpha[:, :, t - 1 : t],
                    functional.pad(
                        log_alpha[:, :, t - 1 : t], (0, 0, 1, -1), value=-1e4
                    ),
                ],
                dim=-1,
            )
            log_alpha[:, :, t] = (
                torch.logsumexp(prev_step + 1e-4, dim=-1) + logp[:, :, t]
            )
...
class AlignTTSLoss(nn.Module):
    ...
    def forward(
        ...
        if phase == 0:
            ...
            mdn_loss = self.mdn_loss(logp, input_lens, decoder_output_lens)
            spec_loss = self.spec_lossX(
                decoder_output, decoder_target, decoder_output_lens
            )
            ssim_loss = self.ssim(decoder_output, decoder_target, decoder_output_lens)
        elif phase == 3:
            dur_loss = self.dur_loss(
                dur_output.unsqueeze(2), dur_target.unsqueeze(2), input_lens
            )
        else:
            mdn_loss = self.mdn_loss(logp, input_lens, decoder_output_lens)
            spec_loss = self.spec_loss(
...
class VitsGeneratorLoss(nn.Module):
    ...
    def forward(
        ...
        loss_feat = (
            feature_loss(feats_real=feats_disc_real, feats_generated=feats_disc_fake)
            * self.feat_loss_alpha
        )
        loss_gen = generator_loss(scores_fake=scores_disc_fake)[0] * self.gen_loss_alpha
        loss_mel = (
            torch.nn.functional.l1_loss(mel_slice, mel_slice_hat) * self.mel_loss_alpha
        )
        ...
        if use_speaker_encoder_as_loss:
            loss_se = (
                self.cosine_similarity_loss(gt_spk_emb, syn_spk_emb)
                * self.spk_encoder_loss_alpha
            )
            loss = loss + loss_se
...
class ForwardTTSLoss(nn.Module):
    ...
    def forward(
        ...
        if self.binary_alignment_loss_alpha > 0 and alignment_hard is not None:
            binary_alignment_loss = _binary_alignment_loss(
