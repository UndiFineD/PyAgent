...
class GPTArgs(XttsArgs):
    ...
    tokenizer_file: str = ""
    mel_norm_file: str = (
        "https://github.com/coqui-ai/TTS/releases/download/v0.14.0_models/mel_norms.pth"
    )
    dvae_checkpoint: str = ""
    xtts_checkpoint: str = ""
...
class GPTTrainer(BaseTTS):
    def __init__(self, config: Coqpit):
        ...
        if self.args.xtts_checkpoint:
            self.load_checkpoint(
                self.config, self.args.xtts_checkpoint, eval=False, strict=False
            )
        ...
        if self.args.gpt_checkpoint:
            ...
            if (
                ...
                and gpt_checkpoint["text_embedding.weight"].shape
                != self.xtts.gpt.text_embedding.weight.shape
            ):
                num_new_tokens = (
                    self.xtts.gpt.text_embedding.weight.shape[0]
                    - gpt_checkpoint["text_embedding.weight"].shape[0]
                )
                logger.info(
                    "Loading checkpoint with %d additional tokens.", num_new_tokens
                )
                ...
                new_entry = torch.randn(
                    num_new_tokens, self.xtts.gpt.text_head.weight.shape[1]
                )
    ...
    def test_run(self, assets) -> tuple[dict, dict]:  # pylint: disable=W0613
        ...
        if self.config.test_sentences:
            ...
            self.xtts.gpt.init_gpt_for_inference(
                kv_cache=self.args.kv_cache, use_deepspeed=False
            )
            self.xtts.gpt.eval()
    ...
    def format_batch_on_device(self, batch):
        ...
        paired_conditioning_mel = self.torch_mel_spectrogram_style_encoder(
            conditioning_reshaped
        )
        # transform torch.Size([B * num_cond_samples, n_mel, T_mel]) in torch.Size([B, num_cond_samples, n_mel, T_mel])
        n_mel = (
            self.torch_mel_spectrogram_style_encoder.n_mel_channels
        )  # paired_conditioning_mel.size(1)
        T_mel = paired_conditioning_mel.size(2)
        paired_conditioning_mel = paired_conditioning_mel.view(
            B, num_cond_samples, n_mel, T_mel
        )
    ...
    def get_scheduler(self, optimizer) -> list:
        ...
        return get_scheduler(
            self.config.lr_scheduler, self.config.lr_scheduler_params, optimizer
        )
    ...
    def init_from_config(
        config: "GPTTrainerConfig", samples: list[list] | list[dict] = None
    ):
