# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\xtts\dvae.py
import functools
import logging
...
def dvae_wav_to_mel(
    ...
    if mel_norms is None:
        mel_norms = torch.load(
            mel_norms_file, map_location=device, weights_only=is_pytorch_at_least_2_4()
        )
...
class DiscreteVAE(nn.Module):
    def __init__(
        ...
        if num_layers > 0:
            ...
            enc_chans_io, dec_chans_io = (
                list(zip(t[:-1], t[1:])) for t in (enc_chans, dec_chans)
            )
