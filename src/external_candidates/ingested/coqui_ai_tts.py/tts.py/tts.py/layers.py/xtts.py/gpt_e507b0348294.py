# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\xtts\gpt.py
...
class GPT(nn.Module):
    ...
    def generate(
        ...
        **hf_generate_kwargs,
        ...
        stop_token_tensor = torch.tensor(
            self.stop_audio_token, device=gpt_inputs.device, dtype=torch.long
        )
        attention_mask = _prepare_attention_mask_for_generation(
            gpt_inputs, stop_token_tensor, stop_token_tensor
        )
    ...
    def get_generator(self, fake_inputs, **hf_generate_kwargs):
        stop_token_tensor = torch.tensor(
            self.stop_audio_token, device=fake_inputs.device, dtype=torch.long
        )
        attention_mask = _prepare_attention_mask_for_generation(
            fake_inputs, stop_token_tensor, stop_token_tensor
        )
