# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\xtts\trainer\dataset.py
import logging
import random
import sys
...
def get_prompt_slice(
    gt_path, max_sample_length, min_sample_length, sample_rate, is_eval=False
):
...
class XTTSDataset(torch.utils.data.Dataset):
    def __init__(self, config, samples, tokenizer, sample_rate, is_eval=False):
        ...
        self.use_masking_gt_prompt_approach = (
            model_args.gpt_use_masking_gt_prompt_approach
        )
    ...
    def get_text(self, text, lang):
        ...
        assert not torch.any(
            tokens == 1
        ), f"UNK token found in {text} -> {self.tokenizer.decode(tokens)}"
    ...
    def __getitem__(self, index):
        ...
        if sample_id in self.failed_samples:
            if self.debug_failures:
                logger.info(
                    "Ignoring sample %s because it was already ignored before !!",
                    sample["audio_file"],
                )
        ...
        try:
            ...
            if self.debug_failures:
                logger.warning(
                    "Error loading %s %s", sample["audio_file"], sys.exc_info()
                )
        ...
        res = {
            ...
            "cond_lens": (
                torch.tensor(cond_len, dtype=torch.long)
                if cond_len is not torch.nan
                else torch.tensor([cond_len])
            ),
            "cond_idxs": (
                torch.tensor(cond_idxs)
