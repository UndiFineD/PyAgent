# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\tortoise\autoregressive.py
    ...
    def __init__(
        ...
        *,
        ...
        for a in range(attn_blocks):
            attn.append(
                AttentionBlock(
                    embedding_dim, num_attn_heads, tortoise_norm=tortoise_norm
                )
            )
...
class UnifiedVoice(nn.Module):
    def __init__(
        ...
        self.conditioning_encoder = ConditioningEncoder(
            80, model_dim, num_attn_heads=heads, tortoise_norm=True
        )
    ...
    def get_conditioning(self, speech_conditioning_input):
        ...
        for j in range(speech_conditioning_input.shape[1]):
            conds.append(
                self.conditioning_encoder(speech_conditioning_input[:, j])[:, :, 0]
            )
    ...
    def inference_speech(
        ...
        **hf_generate_kwargs,
        ...
        stop_token_tensor = torch.tensor(
            self.stop_mel_token, device=inputs.device, dtype=torch.long
        )
        attention_mask = _prepare_attention_mask_for_generation(
            inputs, stop_token_tensor, stop_token_tensor
        )
...
def _prepare_attention_mask_for_generation(
    ...
    is_pad_token_in_inputs = (pad_token_id is not None) and (
        isin(elements=inputs, test_elements=pad_token_id).any()
    )
