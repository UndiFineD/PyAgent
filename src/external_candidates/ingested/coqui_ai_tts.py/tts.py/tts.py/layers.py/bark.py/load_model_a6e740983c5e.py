# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\bark\load_model.py
import contextlib
import functools
import hashlib
import logging
...
def load_model(ckpt_path, device, config, model_type="text"):
    ...
    if not os.path.exists(ckpt_path):
        ...
        download_url = config.REMOTE_MODEL_PATHS[model_type]["path"].replace(
            "tree", "resolve"
        )
        logger.info(
            "%s model not found, downloading from %s into %s",
            model_type,
            download_url,
            ckpt_path,
        )
