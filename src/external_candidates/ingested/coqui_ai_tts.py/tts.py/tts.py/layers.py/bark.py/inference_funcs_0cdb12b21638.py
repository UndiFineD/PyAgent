# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\bark\inference_funcs.py
import logging
import re
...
def generate_text_semantic(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    encoded_text = (
        torch.tensor(
            _tokenize(model.tokenizer, text), device=model.device, dtype=torch.long
        )
    ...
    if semantic_history is not None:
        ...
        semantic_history = torch.full(
            (256,), model.config.SEMANTIC_PAD_TOKEN, dtype=torch.int64
        )
    x = torch.cat(
        [
            encoded_text,
            semantic_history,
            torch.tensor([model.config.SEMANTIC_INFER_TOKEN], device=model.device),
        ]
    ).unsqueeze(0)
    ...
    for n in range(n_tot_steps):
        ...
        if allow_early_stop and (
            item_next == model.config.SEMANTIC_VOCAB_SIZE
...
def generate_coarse(
    ...
    semantic_to_coarse_ratio = (
        model.config.COARSE_RATE_HZ
        / model.config.SEMANTIC_RATE_HZ
    ...
    if all(v is not None for v in history_prompt) or base is not None:
        ...
        x_coarse_history = (
            _flatten_codebooks(x_coarse_history, model.config.CODEBOOK_SIZE)
            + model.config.SEMANTIC_VOCAB_SIZE
        )
    ...
    x_semantic = torch.cat([x_semantic_history.to(model.device), x_semantic]).to(
        dtype=torch.int32
    )
    ...
    for _ in tqdm.tqdm(range(n_window_steps), total=n_window_steps, disable=silent):
        ...
        x_in = torch.hstack(
            [
                x_in,
                torch.tensor([model.config.COARSE_INFER_TOKEN])
                .unsqueeze(0)
                .to(model.device),
        ...
        for _ in range(sliding_window_len):
            ...
            logits, kv_cache = model.coarse_model(
                x_input, use_cache=use_kv_caching, past_kv=kv_cache
            )
            logit_start_idx = (
                model.config.SEMANTIC_VOCAB_SIZE
                + (1 - int(is_major_step)) * model.config.CODEBOOK_SIZE
            )
            logit_end_idx = (
                model.config.SEMANTIC_VOCAB_SIZE
                + (2 - int(is_major_step)) * model.config.CODEBOOK_SIZE
            )
    ...
    gen_coarse_audio_arr = (
        gen_coarse_arr.reshape(-1, model.config.N_COARSE_CODEBOOKS).T
        - model.config.SEMANTIC_VOCAB_SIZE
    )
...
def generate_fine(
    ...
    # make input arr
    padding = torch.full(
        (model.config.N_FINE_CODEBOOKS - n_coarse, x_coarse_gen.shape[1]),
    ...
    if in_arr.shape[1] < 1024:
        ...
        padding = torch.full(
            (model.config.N_FINE_CODEBOOKS, n_remove_from_end),
            model.config.CODEBOOK_SIZE,
            dtype=torch.int32,
        ).to(model.device)
        ...
        for nn in range(n_coarse, model.config.N_FINE_CODEBOOKS):
            ...
            if temp is None:
                relevant_logits = logits[
                    0, rel_start_fill_idx:, : model.config.CODEBOOK_SIZE
                ]
