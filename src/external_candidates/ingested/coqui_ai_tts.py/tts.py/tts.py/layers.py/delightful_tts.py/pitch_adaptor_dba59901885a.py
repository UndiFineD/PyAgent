# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\delightful_tts\pitch_adaptor.py
...
class PitchAdaptor(nn.Module):  # pylint: disable=abstract-method
    ...
    def get_pitch_embedding_train(
        ...
        pitch_pred = self.pitch_predictor(
            x, mask
        )  # [B, T_src, C_hidden], [B, T_src] --> [B, T_src]
        pitch_pred.unsqueeze_(1)  # --> [B, 1, T_src]
        avg_pitch_target = average_over_durations(
            target, dr
        )  # [B, 1, T_mel], [B, T_src] --> [B, 1, T_src]
        pitch_emb = self.pitch_emb(
            avg_pitch_target
        )  # [B, 1, T_src] --> [B, C_hidden, T_src]
    ...
    def get_pitch_embedding(
        ...
        if pitch_transform is not None:
            pitch_pred = pitch_transform(
                pitch_pred, (~mask).sum(), pitch_mean, pitch_std
            )
