...
class AcousticModel(torch.nn.Module):
    def __init__(
        ...
        self.energy_scaler = torch.nn.BatchNorm1d(
            1, affine=False, track_running_stats=True, momentum=None
        )
    ...
    def forward(
        ...
        u_prosody_pred = self.u_norm(
            self.average_utterance_prosody(
                u_prosody_pred=self.utterance_prosody_predictor(
                    x=encoder_outputs, mask=src_mask
                ),
                src_mask=src_mask,
            )
        )
        ...
        p_prosody_pred = self.p_norm(
            self.phoneme_prosody_predictor(x=encoder_outputs, mask=src_mask)
        )
        ...
        pitch_pred, avg_pitch_target, pitch_emb = (
            self.pitch_adaptor.get_pitch_embedding_train(
                x=encoder_outputs,
                target=pitches,
                dr=dr,
                mask=src_mask,
        ...
        energy_pred, avg_energy_target, energy_emb = (
            self.energy_adaptor.get_energy_embedding_train(
                x=encoder_outputs,
                target=energies,
                dr=dr,
                mask=src_mask,
        ...
        alignments_dp = generate_attention(
            dr_pred, src_mask.unsqueeze(1), mel_pred_mask
        )  # [B, T_max, T_max2']
    ...
    def inference(
        ...
        src_mask = ~sequence_mask(
            torch.tensor([tokens.shape[1]], dtype=torch.int64, device=tokens.device)
        )
        src_lens = torch.tensor(tokens.shape[1:2]).to(
            tokens.device
        )  # pylint: disable=unused-variable
        sid, g, lid, _ = self._set_cond_input(  # pylint: disable=unused-variable
            {"d_vectors": d_vectors, "speaker_ids": speaker_idx}
        ...
        encoder_outputs = encoder_outputs + self.u_bottle_out(u_prosody_pred).expand_as(
            encoder_outputs
        )
        ...
        encoder_outputs = encoder_outputs + self.p_bottle_out(p_prosody_pred).expand_as(
            encoder_outputs
        )
        ...
        encoder_outputs = (
            encoder_outputs.transpose(1, 2) + pitch_emb_pred + energy_emb_pred
        )
        ...
        if encoder_outputs_ex.shape[1] > pos_encoding.shape[1]:
            encoding = positional_encoding(
                self.emb_dim, encoder_outputs_ex.shape[2], device=tokens.device
            )
