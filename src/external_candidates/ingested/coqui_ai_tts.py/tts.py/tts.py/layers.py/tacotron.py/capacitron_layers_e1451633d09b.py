# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\tacotron\capacitron_layers.py
import torch
...
class CapacitronVAE(nn.Module):
    ...
    def __init__(
        ...
        self.prior_distribution = MVN(
            torch.zeros(capacitron_VAE_embedding_dim),
            torch.eye(capacitron_VAE_embedding_dim),
        )
        ...
        self.beta = torch.nn.Parameter(
            torch.log(torch.exp(torch.Tensor([1.0])) - 1), requires_grad=True
        )
        mlp_input_dimension = reference_encoder_out_dim

        if text_summary_embedding_dim is not None:
            self.text_summary_net = TextSummary(
                text_summary_embedding_dim, encoder_output_dim=encoder_output_dim
            )
        ...
        self.post_encoder_mlp = PostEncoderMLP(
            mlp_input_dimension, capacitron_VAE_embedding_dim
        )
    ...
    def forward(self, reference_mel_info=None, text_info=None, speaker_embedding=None):
        ...
        if reference_mel_info is not None:
            ...
            if text_info is not None:
                text_inputs = text_info[
                    0
                ]  # [batch_size, num_characters, num_embedding]
                input_lengths = text_info[1]
                text_summary_out = self.text_summary_net(text_inputs, input_lengths).to(
                    reference_mels.device
                )
...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self, num_mel, out_dim):
        ...
        self.bns = nn.ModuleList(
            [nn.BatchNorm2d(num_features=filter_size) for filter_size in filters[1:]]
        )
    ...
    def forward(self, inputs, input_lengths):
        ...
        x = inputs.view(
            batch_size, 1, -1, self.num_mel
        )  # [batch_size, num_channels==1, num_frames, num_mel]
        ...
        for conv, bn in zip(self.convs, self.bns):
            ...
            valid_lengths = (
                torch.ceil(valid_lengths).to(dtype=torch.int64) + 1
            )  # 2 is stride -- size: [batch_size]
            ...
            mask = (
                mask.expand(1, 1, -1, -1).transpose(2, 0).transpose(-1, 2)
            )  # [batch_size, 1, post_conv_max_width, 1]
            x = x * mask
