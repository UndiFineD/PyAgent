# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\layers\feed_forward\encoder.py
...
class ResidualConv1dBNEncoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels, params):
        ...
        self.prenet = nn.Sequential(
            nn.Conv1d(in_channels, hidden_channels, 1), nn.ReLU()
        )
        self.res_conv_block = ResidualConv1dBNBlock(
            hidden_channels, hidden_channels, hidden_channels, **params
        )
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        if encoder_type.lower() == "relative_position_transformer":
            ...
            self.encoder = ResidualConv1dBNEncoder(
                in_hidden_channels, out_channels, in_hidden_channels, encoder_params
            )
