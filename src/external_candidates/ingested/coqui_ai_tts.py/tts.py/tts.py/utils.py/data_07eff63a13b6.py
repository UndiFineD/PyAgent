# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\utils\data.py
import bisect
...
def _pad_tensor(x, length):
    ...
    assert x.ndim == 2
    x = np.pad(
        x, [[0, 0], [0, length - x.shape[1]]], mode="constant", constant_values=_pad
    )
    return x
...
def pad_per_step(inputs, pad_len):
    return np.pad(
        inputs, [[0, 0], [0, 0], [0, pad_len]], mode="constant", constant_values=0.0
    )
...
def get_length_balancer_weights(items: list, num_buckets=10):
    ...
    buckets_classes = [
        i + step for i in range(min_length, (max_length - step) + num_buckets + 1, step)
    ]
    # add each sample in their respective length bucket
    buckets_names = np.array(
        [
            buckets_classes[bisect.bisect_left(buckets_classes, item["audio_length"])]
            for item in items
        ]
    )
    ...
    bucket_count = np.array(
        [len(np.where(buckets_names == l)[0]) for l in unique_buckets_names]
    )
