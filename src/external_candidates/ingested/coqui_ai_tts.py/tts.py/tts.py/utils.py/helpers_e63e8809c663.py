# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\utils\helpers.py
import numpy as np
import torch
...
def sequence_mask(
    sequence_length: torch.Tensor, max_len: int | None = None
) -> torch.Tensor:
...
def segment(
    x: torch.tensor, segment_indices: torch.tensor, segment_size=4, pad_short=False
):
