# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\utils\text\phonemizers\multi_phonemizer.py
import logging
...
class MultiPhonemizer:
    ...
    def __init__(
        self, lang_to_phonemizer_name: dict = {}
        ...
        for k, v in lang_to_phonemizer_name.items():
            if v == "" and k in DEF_LANG_TO_PHONEMIZER.keys():
                ...
                raise ValueError(
                    f"Phonemizer wasn't set for language {k} and doesn't have a default."
                )
