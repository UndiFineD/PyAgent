# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\utils\text\tokenizer.py
import logging
...
class TTSTokenizer:
    ...
    def characters(self, new_characters):
        ...
        self.pad_id = (
            self.characters.char_to_id(self.characters.pad)
            if self.characters.pad
            else None
        )
        self.blank_id = (
            self.characters.char_to_id(self.characters.blank)
            if self.characters.blank
            else None
        )
    ...
    def intersperse_blank_char(
        self, char_sequence: list[str], use_blank_char: bool = False
    ):
        ...
        """
        char_to_use = (
            self.characters.blank_id if use_blank_char else self.characters.pad
        )
    ...
    def print_logs(self, level: int = 0):
        ...
        if len(self.not_found_characters) > 0:
            logger.info(
                "%s| %d characters not found:", indent, len(self.not_found_characters)
            )
            ...
            if "phonemizer" in config and config.phonemizer == "multi_phonemizer":
                ...
                for dataset in config.datasets:
                    if dataset.language != "":
                        ...
                        raise ValueError(
                            "Multi phonemizer requires language to be set for each dataset."
                        )
                ...
                if "phonemizer" in config and config.phonemizer:
                    phonemizer = get_phonemizer_by_name(
                        config.phonemizer, **phonemizer_kwargs
                    )
                else:
                    try:
                        phonemizer = get_phonemizer_by_name(
