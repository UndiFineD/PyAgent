# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\utils\fairseq.py
import torch
...
def rehash_fairseq_vits_checkpoint(checkpoint_file):
    ...
    for k, v in chk.items():
        if "enc_p." in k:
            ...
            new_chk[
                k.replace("dp.flows.0.m", "duration_predictor.flows.0.translation")
            ] = v
        elif "dp.flows.0.logs" in k:
            new_chk[
                k.replace("dp.flows.0.logs", "duration_predictor.flows.0.log_scale")
            ] = v
