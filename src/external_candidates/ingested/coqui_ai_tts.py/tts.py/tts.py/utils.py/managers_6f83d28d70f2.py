...
def load_file(path: str | os.PathLike[Any]) -> Any:
    ...
    if path.endswith(".json"):
        ...
        with fsspec.open(path, "rb") as f:
            return torch.load(
                f, map_location="cpu", weights_only=is_pytorch_at_least_2_4()
            )
    else:
...
class BaseIDManager:
    ...
    @staticmethod
    def _save_json(
        json_file_path: str | os.PathLike[Any], data: dict[str, Any]
    ) -> None:
    ...
    @staticmethod
    def parse_ids_from_data(
        items: list[dict[str, Any]], parse_key: str
    ) -> dict[str, int]:
...
class EmbeddingManager(BaseIDManager):
    ...
    def load_embeddings_from_file(self, file_path: str | os.PathLike[Any]) -> None:
        ...
        """
        self.name_to_id, self.clip_ids, self.embeddings, self.embeddings_by_names = (
            self.read_embeddings_from_file(file_path)
        )

    def load_embeddings_from_list_of_files(
        self, file_paths: list[str | os.PathLike[Any]]
    ) -> None:
        ...
        for file_path in file_paths:
            ids, clip_ids, embeddings, embeddings_by_names = (
                self.read_embeddings_from_file(file_path)
            )
            ...
            if duplicates:
                raise ValueError(
                    f" [!] Duplicate embedding names <{duplicates}> in {file_path}"
                )
            # store values
    ...
    def get_mean_embedding(
        self, idx: str, num_samples: int | None = None, randomize: bool = False
    ) -> np.ndarray:
        ...
        if num_samples is None:
            ...
            assert (
                len(embeddings) >= num_samples
            ), f" [!] {idx} has number of samples < {num_samples}"
            if randomize:
    ...
    def get_random_embedding(self) -> Any:
        ...
        """
        if self.embeddings:
            return self.embeddings[random.choices(list(self.embeddings.keys()))[0]][
                "embedding"
            ]
    ...
    def compute_embedding_from_clip(
        ...
        def _compute(wav_file: str | os.PathLike[Any]):
            waveform = self.encoder_ap.load_wav(
                wav_file, sr=self.encoder_ap.sample_rate
            )
