# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\forward_tts.py
import logging
...
class ForwardTTS(BaseTTS):
    ...
    def __init__(
        ...
        self.length_scale = (
            float(self.args.length_scale)
            if isinstance(self.args.length_scale, int)
            else self.args.length_scale
        )
    ...
    def init_multispeaker(self, config: Coqpit):
        ...
        if self.speaker_manager is None and (
            config.use_d_vector_file or config.use_speaker_embedding
        ):
            raise ValueError(
        ...
        if config.use_d_vector_file:
            ...
            if self.args.d_vector_dim != self.args.hidden_channels:
                ...
                self.proj_g = nn.Linear(
                    in_features=self.args.d_vector_dim,
                    out_features=self.args.hidden_channels,
                )
    ...
    def forward(
        ...
        if self.use_aligner:
            o_alignment_dur, alignment_soft, alignment_logprob, alignment_mas = (
                self._forward_aligner(x_emb, y, x_mask, y_mask)
            )
    ...
    def inference(
        ...
        x_mask = (
            torch.unsqueeze(sequence_mask(x_lengths, x.shape[1]), 1).to(x.dtype).float()
        )
        # encoder pass
    ...
    def train_step(self, batch: dict, criterion: nn.Module):
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            loss_dict = criterion(
                ...
                alignment_logprob=(
                    outputs["alignment_logprob"] if self.use_aligner else None
                ),
            ...
            duration_error = (
                torch.abs(durations - durations_pred).sum() / text_lengths.sum()
            )
    ...
    def _create_logs(self, batch, outputs):
        ...
        if self.args.use_pitch:
            ...
            pitch_figures = {
                "pitch_ground_truth": plot_avg_pitch(
                    pitch_avg, chars, output_fig=False
                ),
        ...
        if self.args.use_energy:
            ...
            chars = self.tokenizer.decode(batch["text_input"][0].data.cpu().numpy())
            energy_figures = {
                "energy_ground_truth": plot_avg_energy(
                    energy_avg, chars, output_fig=False
                ),
                "energy_avg_predicted": plot_avg_energy(
        ...
        if "attn_durations" in outputs:
            ...
            figures["alignment_hat"] = plot_alignment(
                alignments_hat.T, output_fig=False
            )
    ...
    def get_criterion(self):
        from TTS.tts.layers.losses import (
            ForwardTTSLoss,  # pylint: disable=import-outside-toplevel
        )
    ...
    def init_from_config(
        config: "ForwardTTSConfig", samples: list[list] | list[dict] = None
    ):
