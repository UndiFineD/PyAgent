# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\vits.py
import logging
import math
import os
...
class VitsArgs(Coqpit):
    ...
    periods_multi_period_discriminator: list[int] = field(
...
class Vits(BaseTTS):
    ...
    def init_multispeaker(self, config: Coqpit):
        ...
        if self.args.use_speaker_encoder_as_loss:
            if self.speaker_manager.encoder is None and (
                not self.args.speaker_encoder_model_path
                or not self.args.speaker_encoder_config_path
            ):
                raise RuntimeError(
    ...
    def on_init_end(self, trainer):  # pylint: disable=W0613
        ...
        if self.args.reinit_DP:
            ...
            for key, value in after_dict.items():
                if value == before_dict[key]:
                    raise RuntimeError(
                        " [!] The weights of Duration Predictor was not reinit check it !"
                    )
        ...
        if self.args.reinit_text_encoder:
            ...
            for key, value in after_dict.items():
                if value == before_dict[key]:
                    raise RuntimeError(
                        " [!] The weights of Text Encoder was not reinit check it !"
                    )
    ...
    def _set_speaker_input(self, aux_input: dict):
        ...
        if speaker_ids is not None and not hasattr(self, "emb_g"):
            raise ValueError(
                "[!] Cannot use speaker-ids without enabling speaker embedding."
            )
    ...
    def forward_mas(self, outputs, z_p, m_p, logs_p, x, x_mask, y_mask, g, lang_emb):
        ...
        if self.args.use_sdp:
            loss_duration = self.duration_predictor(
                ...
                lang_emb=(
                    lang_emb.detach()
                    if self.args.detach_dp_input and lang_emb is not None
                    else lang_emb
                ),
            )
    ...
    def upsampling_z(self, z, slice_ids=None, y_lengths=None, y_mask=None):
        ...
        if self.args.encoder_sample_rate:
            ...
            slice_ids = (
                slice_ids * int(self.interpolate_factor)
            ...
            if self.args.interpolate_z:
                z = torch.nn.functional.interpolate(
                    z, scale_factor=[self.interpolate_factor], mode="linear"
                ).squeeze(0)
                # recompute the mask if needed
                if y_lengths is not None and y_mask is not None:
                    y_mask = (
                        sequence_mask(y_lengths * self.interpolate_factor, None)
                        .to(y_mask.dtype)
                        .unsqueeze(1)
                    )  # [B, 1, T_dec_resampled]
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        if (
            self.args.use_speaker_encoder_as_loss
            and self.speaker_manager.encoder is not None
        ):
    ...
    def inference(
        ...
        y_mask = (
            sequence_mask(y_lengths, None).to(x_mask.dtype).unsqueeze(1)
        )  # [B, 1, T_dec]
    ...
    def voice_conversion(
        ...
        *,
        ...
        speaker_id, d_vector = self._get_speaker_id_or_dvector(
            speaker, target_wav, voice_dir
        )
        source_speaker_id, source_d_vector = self._get_speaker_id_or_dvector(
            source_speaker, source_wav, voice_dir
        )
        y = torch.tensor(
            self.ap.load_wav(
                source_wav,
                sr=(
                    self.args.encoder_sample_rate
                    if self.args.encoder_sample_rate
                    else self.ap.sample_rate
                ),
            ),
            dtype=torch.float,
            device=self.device,
        ...
        speaker_cond_src = (
            source_speaker_id if source_speaker_id is not None else source_d_vector
        )
        speaker_cond_tgt = speaker_id if speaker_id is not None else d_vector
        wav, _, _ = self.inference_voice_conversion(
            y, y_lengths, speaker_cond_src, speaker_cond_tgt
        )
        return wav.squeeze()

    @torch.inference_mode()
    def inference_voice_conversion(
        self, y, y_lengths, speaker_cond_src, speaker_cond_tgt
    ):
        ...
        if self.args.use_speaker_embedding and not self.args.use_d_vector_file:
            g_src = self.emb_g(
                torch.from_numpy(np.array(speaker_cond_src)).unsqueeze(0)
            ).unsqueeze(-1)
            g_tgt = self.emb_g(
                torch.from_numpy(np.array(speaker_cond_tgt)).unsqueeze(0)
            ).unsqueeze(-1)
            ...
            raise RuntimeError(
                " [!] Voice conversion is only supported on multi-speaker models."
            )
    ...
    def train_step(
        ...
        if optimizer_idx == 1:
            ...
            with torch.autocast("cuda", enabled=False):
                if self.args.encoder_sample_rate:
                    spec_segment_size = self.spec_segment_size * int(
                        self.interpolate_factor
                    )
                else:
                    spec_segment_size = self.spec_segment_size
    ...
    def test_run(self, assets) -> dict[str, Any]:
        ...
        for idx, s_info in enumerate(test_sentences):
            ...
            test_figures[f"{idx}-alignment"] = plot_alignment(
                outputs["alignments"].permute(2, 1, 0), output_fig=False
            )
    ...
    def format_batch(self, batch: dict) -> dict:
        ...
        if (
            ...
            language_ids = [
                self.language_manager.name_to_id[ln] for ln in batch["language_names"]
            ]
    ...
    def format_batch_on_device(self, batch):
        ...
        batch["spec"] = wav_to_spec(
            wav, ac.fft_size, ac.hop_length, ac.win_length, center=False
        )
        ...
        if self.args.encoder_sample_rate:
            ...
            if spec_mel.size(2) > int(batch["spec"].size(2) * self.interpolate_factor):
                spec_mel = spec_mel[
                    :, :, : int(batch["spec"].size(2) * self.interpolate_factor)
                ]
            else:
                batch["spec"] = batch["spec"][
                    :, :, : int(spec_mel.size(2) / self.interpolate_factor)
                ]
        else:
            spec_mel = batch["spec"]
        ...
        if self.args.encoder_sample_rate:
            ...
            assert (
                batch["spec"].shape[2] == batch["mel"].shape[2]
            ), f"{batch['spec'].shape[2]}, {batch['mel'].shape[2]}"
        ...
        if self.args.encoder_sample_rate:
            assert (
                batch["spec_lens"] - (batch["mel_lens"] / self.interpolate_factor).int()
            ).sum() == 0
        else:
    ...
    def get_sampler(
        self, config: Coqpit, dataset: TTSDataset, num_gpus=1, is_eval=False
    ):
        weights = None
        ...
        if getattr(config, "use_weighted_sampler", False):
            for attr_name, alpha in config.weighted_sampler_attrs.items():
                logger.info(
                    "Using weighted sampler for attribute '%s' with alpha %.3f",
                    attr_name,
                    alpha,
                )
        ...
        if batch_sampler is None:
            ...
            batch_sampler = (
                DistributedSamplerWrapper(batch_sampler)
                if num_gpus > 1
                else batch_sampler
    ...
    def load_checkpoint(
        ...
        state["model"] = {
            k: v for k, v in state["model"].items() if "speaker_encoder" not in k
        }
        ...
        if (
            hasattr(self, "emb_g")
            and state["model"]["emb_g.weight"].shape != self.emb_g.weight.shape
        ):
            num_new_speakers = (
                self.emb_g.weight.shape[0] - state["model"]["emb_g.weight"].shape[0]
            )
            logger.info(
                "Loading checkpoint with %d additional speakers.", num_new_speakers
            )
    ...
    def load_fairseq_checkpoint(
        ...
        self.text_encoder.emb = nn.Embedding(
            vocab.num_chars, config.model_args.hidden_channels
        )
    ...
    def init_from_config(config: "VitsConfig", samples: list[list] | list[dict] = None):
        ...
        upsample_rate = torch.prod(
            torch.as_tensor(config.model_args.upsample_rates_decoder)
        ).item()
        ...
        if not config.model_args.encoder_sample_rate:
            ...
            encoder_to_vocoder_upsampling_factor = (
                config.audio.sample_rate / config.model_args.encoder_sample_rate
            )
            effective_hop_length = (
                config.audio.hop_length * encoder_to_vocoder_upsampling_factor
            )
            assert (
    ...
    def export_onnx(self, output_path: str = "coqui_vits.onnx", verbose: bool = True):
        ...
        sequences = torch.randint(
            low=0, high=2, size=(1, dummy_input_length), dtype=torch.long
        )
        ...
        if (
            hasattr(self, "num_languages")
            and self.num_languages > 0
            and self.embedded_language_dim > 0
        ):
...
class FairseqVocab(BaseVocabulary):
    ...
    def vocab(self, vocab_file: str | os.PathLike[Any]) -> None:
        ...
        self.pad = " "
        self._char_to_id = {
            s: i for i, s in enumerate(self._vocab)
