# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\overflow.py
import logging
import os
...
class Overflow(BaseTTS):
    ...
    def inference(
        ...
        mels, mel_outputs_len, _ = self.decoder(
            outputs["hmm_outputs"].transpose(1, 2),
            outputs["hmm_outputs_len"],
            reverse=True,
        )
    ...
    def init_from_config(
        config: "OverFlowConfig", samples: list[list] | list[dict] = None
    ):
    ...
    def _create_logs(self, batch, outputs):
        ...
        figures = {
            ...
            "mel_from_most_probable_state": plot_spectrogram(
                get_spec_from_most_probable_state(
                    alignments[0], means[0], self.decoder
                ),
                fig_size=(12, 3),
            ),
