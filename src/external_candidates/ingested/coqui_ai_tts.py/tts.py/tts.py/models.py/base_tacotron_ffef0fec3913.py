# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\base_tacotron.py
import copy
import logging
...
class BaseTacotron(BaseTTS):
    ...
    def __init__(
        ...
        if self.capacitron_vae and self.use_capacitron_vae:
            self.decoder_in_features += (
                self.capacitron_vae.capacitron_VAE_embedding_dim
            )  # add capacitron embedding dim
    ...
    def synthesize(
        ...
        **kwargs,
        ...
        def _compute_style_mel(style_wav):
            return torch.tensor(
                self.ap.melspectrogram(
                    self.ap.load_wav(style_wav, sr=self.ap.sample_rate)
                ),
                dtype=torch.float,
                device=self.device,
            ).unsqueeze(0)
        ...
        if self.config.has("gst") and self.config.gst and style_wav is not None:
            style_mel = (
                ...
                else _compute_style_mel(style_wav)
            )

        if (
            self.config.has("capacitron_vae")
            and self.config.use_capacitron_vae
            and style_wav is not None
        ):
        ...
        if not isinstance(style_mel, dict):
            if style_text is not None:
                style_text = torch.tensor(
                    self.tokenizer.text_to_ids(style_text, language=language),
                    dtype=torch.long,
                    device=self.device,
                ).unsqueeze(0)
        extra_aux_input = {"style_mel": style_mel, "style_text": style_text}
        return super().synthesize(
            *args, language=language, extra_aux_input=extra_aux_input, **kwargs
        )
    ...
    def compute_capacitron_VAE_embedding(
        self, inputs, reference_mel_info, text_info=None, speaker_embedding=None
    ):
    ...
    def on_epoch_start(self, trainer):
        ...
        if self.gradual_training:
            r, trainer.config.batch_size = gradual_training_scheduler(
                trainer.total_steps_done, trainer.config
            )
            trainer.config.r = r
