...
class AlignTTS(BaseTTS):
    ...
    def __init__(
        ...
        self.emb = nn.Embedding(
            self.config.model_args.num_chars, self.config.model_args.hidden_channels
        )
        ...
        self.mod_layer = nn.Conv1d(
            config.model_args.hidden_channels, config.model_args.hidden_channels, 1
        )

        self.mdn_block = MDNBlock(
            config.model_args.hidden_channels, 2 * config.model_args.out_channels
        )

        if (
            self.embedded_speaker_dim > 0
            and self.embedded_speaker_dim != config.model_args.hidden_channels
        ):
            self.proj_g = nn.Conv1d(
                self.embedded_speaker_dim, config.model_args.hidden_channels, 1
            )
    ...
    def compute_log_probs(mu, log_sigma, y):
        ...
        exponential = -0.5 * torch.mean(
            torch._C._nn.mse_loss(expanded_y, expanded_mu, 0)
            / torch.pow(log_sigma.exp(), 2),
            dim=-1,
        )  # B, L, T
    ...
    def forward(
        ...
        if phase == 0:
            ...
            o_de, attn = self._forward_decoder(
                o_en.detach(), o_en_dp.detach(), dr_mas.detach(), x_mask, y_lengths, g=g
            )
        elif phase == 2:
            ...
            dr_mas, mu, log_sigma, logp = self._forward_mdn(o_en, y, y_lengths, x_mask)
            o_de, attn = self._forward_decoder(
                o_en, o_en_dp, dr_mas, x_mask, y_lengths, g=g
            )
        elif phase == 3:
    ...
    def inference(
        self, x, aux_input={"d_vectors": None}
    ):  # pylint: disable=unused-argument
        """
        Shapes:
            - x: :math:`[B, T_max]`
    ...
    def train_step(self, batch: dict, criterion: nn.Module):
        ...
        outputs = self.forward(
            text_input, text_lengths, mel_input, mel_lengths, aux_input, self.phase
        )
        loss_dict = criterion(
            outputs["logp"],
        ...
        if isinstance(config.phase_start_steps, list):
            ...
            if True not in vals:
                ...
                phase = (
                    ...
                    - [i < global_step for i in config.phase_start_steps][::-1].index(
                        True
                    )
                    - 1
                )
    ...
    def init_from_config(
        config: "AlignTTSConfig", samples: list[list] | list[dict] = None
    ):
