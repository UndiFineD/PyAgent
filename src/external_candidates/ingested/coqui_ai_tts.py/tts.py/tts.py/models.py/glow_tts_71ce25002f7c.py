# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\glow_tts.py
import logging
import math
import os
...
class GlowTTS(BaseTTS):
    ...
    def inference_with_MAS(
        ...
        o_scale = torch.exp(-2 * o_log_scale)
        logp1 = torch.sum(-0.5 * math.log(2 * math.pi) - o_log_scale, [1]).unsqueeze(
            -1
        )  # [b, t, 1]
        logp2 = torch.matmul(
            o_scale.transpose(1, 2), -0.5 * (z**2)
    ...
    def train_step(self, batch: dict, criterion: nn.Module):
        ...
        if self.run_data_dep_init and self.training:
            ...
            with torch.autocast(
                "cuda", enabled=False
            ):  # avoid mixed_precision in criterion
                loss_dict = criterion(
                    outputs["z"].float(),
    ...
    def _create_logs(self, batch, outputs):
        ...
        text_input = (
            batch["text_input"][:1] if batch["text_input"] is not None else None
        )
        text_lengths = batch["text_lengths"]
        mel_input = batch["mel_input"]
        d_vectors = batch["d_vectors"][:1] if batch["d_vectors"] is not None else None
        speaker_ids = (
            batch["speaker_ids"][:1] if batch["speaker_ids"] is not None else None
        )
    ...
    def get_criterion():
        from TTS.tts.layers.losses import (
            GlowTTSLoss,  # pylint: disable=import-outside-toplevel
        )
    ...
    def init_from_config(
        config: "GlowTTSConfig", samples: list[list] | list[dict] = None
    ):
