# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\neuralhmm_tts.py
import logging
import os
...
class NeuralhmmTTS(BaseTTS):
    ...
    def __init__(
        ...
        self.encoder = Encoder(
            config.num_chars, config.state_per_phone, config.encoder_in_out_features
        )
    ...
    def preprocess_batch(self, text, text_len, mels, mel_len):
        if self.mean.item() == 0 or self.std.item() == 1:
            statistics_dict = torch.load(
                self.mel_statistics_parameter_path,
    ...
    def forward(self, text, text_len, mels, mel_len):
        ...
        Shapes:
            ...
            mel_len: :math:`[B]`
        """
        text, text_len, mels, mel_len = self.preprocess_batch(
            text, text_len, mels, mel_len
        )
    ...
    def train_step(self, batch: dict, criterion: nn.Module):
        ...
        loss_dict = criterion(
            outputs["log_probs"] / (mel_lengths.sum() + text_lengths.sum())
        )
    ...
    def inference(
        ...
        encoder_outputs, encoder_output_len = self.encoder.inference(
            text, aux_input["x_lengths"]
        )
    ...
    def init_from_config(
        config: "NeuralhmmTTSConfig", samples: list[list] | list[dict] = None
    ):
    ...
    def on_init_start(self, trainer):
        ...
        if (
            not os.path.isfile(trainer.config.mel_statistics_parameter_path)
            or trainer.config.force_generate_statistics
        ):
            ...
            data_mean, data_std, init_transition_prob = (
                OverflowUtils.get_data_parameters_for_flat_start(
                    dataloader,
                    trainer.config.out_channels,
                    trainer.config.state_per_phone,
                )
            )
            logger.info(
            ...
            logger.info(
                "Data parameters loaded with value: %s",
                (data_mean, data_std, init_transition_prob),
            )

        trainer.config.flat_start_params["transition_p"] = (
            init_transition_prob.item()
            if isinstance(init_transition_prob, torch.Tensor)
            else init_transition_prob
        )
    ...
    def _create_logs(self, batch, outputs):
        alignments, transition_vectors = (
            outputs["alignments"],
            outputs["transition_vectors"],
        )
        means = torch.stack(outputs["means"], dim=1)

        figures = {
            "alignment": plot_alignment(
                alignments[0].exp(), title="Forward alignment", fig_size=(20, 20)
            ),
            "log_alignment": plot_alignment(
                alignments[0].exp(),
                title="Forward log alignment",
                plot_log=True,
                fig_size=(20, 20),
            ),
            "transition_vectors": plot_alignment(
                transition_vectors[0], title="Transition vectors", fig_size=(20, 20)
            ),
            "mel_from_most_probable_state": plot_spectrogram(
                get_spec_from_most_probable_state(alignments[0], means[0]),
                fig_size=(12, 3),
            ),
        ...
        inference_output = self.inference(
            batch["text_input"][-1].unsqueeze(0),
            aux_input={"x_lengths": batch["text_lengths"][-1].unsqueeze(0)},
        )
        figures["synthesised"] = plot_spectrogram(
            inference_output["model_outputs"][0], fig_size=(12, 3)
        )

        states = [p[1] for p in inference_output["input_parameters"][0]]
        transition_probability_synthesising = [
            p[2].cpu().numpy() for p in inference_output["output_parameters"][0]
        ]

        for i in range((len(transition_probability_synthesising) // 200) + 1):
            start = i * 200
            end = (i + 1) * 200
            figures[f"synthesised_transition_probabilities/{i}"] = (
                plot_transition_probabilities_to_numpy(
                    states[start:end], transition_probability_synthesising[start:end]
                )
            )

        audio = self.ap.inv_melspectrogram(
            inference_output["model_outputs"][0].T.cpu().numpy()
        )
