# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\delightful_tts.py
import logging
import os
    ...
    def compute_or_load(self, wav_file, audio_name):
        ...
        if not os.path.exists(pitch_file):
            pitch = self._compute_and_save_pitch(
                wav_file=wav_file, pitch_file=pitch_file
            )
        else:
...
class ForwardTTSE2eDataset(TTSDataset):
    ...
    def __getitem__(self, idx):
        ...
        rel_wav_path = (
            Path(item["audio_file"]).relative_to(item["root_path"]).with_suffix("")
        )
    ...
    def load_or_compute_attn_prior(self, token_ids, wav, rel_wav_path):
        ...
        attn_prior_file = os.path.join(
            self.attn_prior_cache_path, f"{rel_wav_path}.npy"
        )
    ...
    def collate_fn(self, batch):
        ...
        return {
            ...
            "attn_priors": (
                batch["attn_prior"] if batch["attn_prior"][0] is not None else None
            ),
        }
...
class VocoderConfig(Coqpit):
    ...
    upsample_kernel_sizes_decoder: list[int] = field(
        default_factory=lambda: [16, 16, 4, 4]
    )
    use_spectral_norm_discriminator: bool = False
    upsampling_rates_discriminator: list[int] = field(
        default_factory=lambda: [4, 4, 4, 4]
    )
...
class DelightfulTTS(BaseTTSE2E):
    ...
    def __init__(
        ...
        super().__init__(
            config=config, ap=ap, tokenizer=tokenizer, speaker_manager=speaker_manager
        )
        ...
        self.acoustic_model = AcousticModel(
            args=self.args, tokenizer=tokenizer, speaker_manager=speaker_manager
        )
    ...
    def _init_d_vector(self):
        ...
        if hasattr(self, "emb_g"):
            raise ValueError(
                "[!] Speaker embedding layer already initialized before d_vector settings."
            )
    ...
    def inference(
        ...
        vocoder_input = encoder_outputs["model_outputs"].transpose(
            1, 2
        )  # [B, T_max2, C_mel] -> [B, C_mel, T_max2]
        if encoder_outputs["spk_emb"] is not None:
            g = encoder_outputs["spk_emb"].unsqueeze(-1)
        else:
            g = None
    ...
    @torch.inference_mode()
    def inference_spec_decoder(
        self, x, aux_input={"d_vectors": None, "speaker_ids": None}
    ):
        encoder_outputs = self.acoustic_model.inference(
            tokens=x,
    ...
    def train_step(self, batch: dict, criterion: nn.Module, optimizer_idx: int):
        if optimizer_idx == 0:
            ...
            self.model_outputs_cache = (
                outputs  # pylint: disable=attribute-defined-outside-init
            )
        ...
        if optimizer_idx == 1:
            ...
            with torch.autocast("cuda", enabled=True):  # use float32 for the criterion
                loss_dict = criterion[optimizer_idx](
                    mel_output=self.model_outputs_cache[
    ...
    def _create_logs(self, batch, outputs):
        ...
        pitch_figures = {
            ...
            "pitch_avg_predicted": plot_avg_pitch(
                pitch_avg_hat, chars, output_fig=False
            ),
        }
        ...
        energy_figures = {
            ...
            "energy_avg_predicted": plot_avg_pitch(
                energy_avg_hat, chars, output_fig=False
            ),
        }
        ...
        encoder_audio = mel_to_wav_numpy(
            mel=db_to_amp_numpy(x=pred_spec.T, gain=1, base=None),
    ...
    def plot_outputs(self, text, wav, alignment, outputs):
        ...
        input_text = self.tokenizer.ids_to_text(
        ...
        figures["alignment"] = plot_alignment(
        ...
        figures["pitch_avg_pred"] = plot_avg_pitch(
            pitch_avg_pred_denorm.squeeze(), input_text
        )
        figures["energy_avg_pred"] = plot_avg_pitch(
            energy_avg_pred.squeeze(), input_text
        )
        return figures
    ...
    def synthesize(
        ...
        **kwargs,
        ...
        text_inputs = torch.as_tensor(
            text_inputs, dtype=torch.long, device=self.device
        ).unsqueeze(0)

        _speaker_id, d_vector = self._get_speaker_id_or_dvector(
            speaker, speaker_wav, voice_dir
        )
    ...
    def test_run(self, assets) -> dict[str, Any]:
        ...
        for idx, s_info in enumerate(test_sentences):
            ...
            speaker_id, d_vector = self._get_speaker_id_or_dvector(
                aux_inputs["speaker"]
            )
    ...
    def format_batch(self, batch: dict) -> dict:
        ...
        if (
            ...
            and self.speaker_manager.speaker_names
            and self.args.use_speaker_embedding
        ):
            speaker_ids = [
                self.speaker_manager.name_to_id[sn] for sn in batch["speaker_names"]
            ]
        ...
        if (
            ...
            and self.args.use_d_vector_file
        ):
            d_vector_mapping = self.speaker_manager.embeddings
            d_vectors = [
                d_vector_mapping[w]["embedding"] for w in batch["audio_unique_names"]
            ]
    ...
    def format_batch_on_device(self, batch):
        ...
        batch["pitch"] = (
            batch["pitch"][:, :, : batch["mel_input"].shape[2]]
            if batch["pitch"] is not None
            else None
        )
        batch["mel_lengths"] = (
            batch["mel_input"].shape[2] * batch["waveform_rel_lens"]
        ).int()

        # zero the padding frames
        batch["mel_input"] = batch["mel_input"] * sequence_mask(
            batch["mel_lengths"]
        ).unsqueeze(1)
        ...
        if self.config.use_attn_priors:
            ...
            for i in range(batch["mel_input"].shape[0]):
                batch["attn_priors"][
                    i, : attn_priors_np[i].shape[0], : attn_priors_np[i].shape[1]
                ] = torch.from_numpy(attn_priors_np[i])
    ...
    def get_sampler(self, config: Coqpit, dataset: TTSDataset, num_gpus=1):
        ...
        if getattr(config, "use_weighted_sampler", False):
            for attr_name, alpha in config.weighted_sampler_attrs.items():
                logger.info(
                    "Using weighted sampler for attribute '%s' with alpha %.2f",
                    attr_name,
                    alpha,
                )
    ...
    def get_data_loader(
        ...
        if is_eval and not config.run_eval:
            ...
            dataset = ForwardTTSE2eDataset(
                ...
                ap=self.ap,
                batch_group_size=(
                    0 if is_eval else config.batch_group_size * config.batch_size
                ),
                ...
                attn_prior_cache_path=(
                    config.attn_prior_cache_path if config.use_attn_priors else None
                ),
            ...
            loader = DataLoader(
                ...
                num_workers=(
                    config.num_eval_loader_workers
                    if is_eval
    ...
    def get_optimizer(self) -> list:
        ...
        gen_parameters = chain(
            params for k, params in self.named_parameters() if not k.startswith("disc.")
        )
    ...
    def get_scheduler(self, optimizer) -> list:
        ...
        scheduler_D = get_scheduler(
            ...
            self.config.lr_scheduler_gen_params,
            optimizer[0],
        )
        scheduler_G = get_scheduler(
            self.config.lr_scheduler_disc,
            self.config.lr_scheduler_disc_params,
            optimizer[1],
        )
    ...
    def on_train_step_start(self, trainer) -> None:
        ...
        self.binary_loss_weight = (
            min(trainer.epochs_done / self.config.binary_loss_warmup_epochs, 1.0) * 1.0
        )
        self.train_disc = (  # pylint: disable=attribute-defined-outside-init
...
class DelightfulTTSLoss(nn.Module):
    def __init__(self, config):
        ...
        self.multi_scale_stft_loss = MultiScaleSTFTLoss(
            **config.multi_scale_stft_loss_params
        )
    ...
    def forward(
        ...
        if not skip_disc:
            loss_feat = (
                feature_loss(feats_real=feats_real, feats_generated=feats_fake)
                * self.feat_loss_alpha
            )
        ...
        loss_mel = (
            torch.nn.functional.l1_loss(spec_slice, spec_slice_hat)
            * self.vocoder_mel_loss_alpha
        )
        loss_stft_mg, loss_stft_sc = self.multi_scale_stft_loss(
            y_hat=waveform_hat, y=waveform
        )
