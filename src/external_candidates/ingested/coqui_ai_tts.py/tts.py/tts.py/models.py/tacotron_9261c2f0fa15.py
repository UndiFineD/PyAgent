# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\tacotron.py
import torch
...
class Tacotron(BaseTacotron):
        ...
        if self.capacitron_vae and self.use_capacitron_vae:
            self.capacitron_vae_layer = CapacitronVAE(
                ...
                speaker_embedding_dim=(
                    ...
                    and self.capacitron_vae.capacitron_use_speaker_embedding
                    else None
                ),
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        if self.capacitron_vae and self.use_capacitron_vae:
            ...
            encoder_outputs, *capacitron_vae_outputs = (
                self.compute_capacitron_VAE_embedding(
                    encoder_outputs,
    ...
    def inference(self, text_input, aux_input=None):
        ...
        if self.capacitron_vae and self.use_capacitron_vae:
            if aux_input["style_text"] is not None:
                ...
                style_text_length = torch.tensor(
                    [style_text_embedding.size(1)], dtype=torch.int64
                ).to(
                    encoder_outputs.device
                )  # pylint: disable=not-callable
            reference_mel_length = (
                torch.tensor([aux_input["style_mel"].size(1)], dtype=torch.int64).to(
                    encoder_outputs.device
                )
            ...
            encoder_outputs, *_ = self.compute_capacitron_VAE_embedding(
                encoder_outputs,
                reference_mel_info=(
                    [aux_input["style_mel"], reference_mel_length]
                    if aux_input["style_mel"] is not None
                    else None
                ),
                text_info=(
                    [style_text_embedding, style_text_length]
                    if aux_input["style_text"] is not None
                    else None
                ),
                speaker_embedding=(
    ...
    def train_step(self, batch: dict, criterion: torch.nn.Module) -> tuple[dict, dict]:
        ...
        with torch.autocast("cuda", enabled=False):  # use float32 for the criterion
            loss_dict = criterion(
                ...
                (
                    None
                    if outputs["decoder_outputs_backward"] is None
                    else outputs["decoder_outputs_backward"].float()
                ),
                outputs["alignments"].float(),
                alignment_lengths,
                (
                    None
                    if outputs["alignments_backward"] is None
                    else outputs["alignments_backward"].float()
                ),
                text_lengths,
            )
    ...
    def get_optimizer(self) -> list:
        ...
        return get_optimizer(
            self.config.optimizer, self.config.optimizer_params, self.config.lr, self
        )

    def get_scheduler(self, optimizer: object):
        opt = optimizer.primary_optimizer if self.use_capacitron_vae else optimizer
        return get_scheduler(
            self.config.lr_scheduler, self.config.lr_scheduler_params, opt
        )
    ...
    def before_gradient_clipping(self):
        if self.use_capacitron_vae:
            ...
            torch.nn.utils.clip_grad_norm_(
                model_params_to_clip, self.capacitron_vae.capacitron_grad_clip
            )
    ...
    def _create_logs(self, batch, outputs):
        ...
        figures = {
            "pred_linear_spec": plot_spectrogram(
                pred_linear_spec, self.ap, output_fig=False
            ),
            "real_linear_spec": plot_spectrogram(
                gt_linear_spec, self.ap, output_fig=False
            ),
    ...
    def init_from_config(
        config: "TacotronConfig", samples: list[list] | list[dict] = None
    ):
