# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\models\base_tts.py
import logging
import os
import random
...
class BaseTTS(CloningMixin, BaseTrainerModel):
    ...
    def _get_speaker_id_or_dvector(
        ...
        speaker_exists = True
        if (
            get_from_config_or_model_args(self.config, "use_d_vector_file")
            and speaker is not None
        ):
            if speaker in self.speaker_manager.embedding_names:
                d_vector = self.speaker_manager.get_mean_embedding(
                    speaker, num_samples=None, randomize=False
                )
                d_vector = torch.tensor(
                    d_vector, dtype=torch.float, device=self.device
                ).unsqueeze(0)
                return None, d_vector  # [1 x embedding_dim]
            speaker_exists = False

        if (
            get_from_config_or_model_args(self.config, "use_speaker_embedding")
            and speaker is not None
        ):
        ...
        if self.speaker_manager.encoder is not None and (
            speaker is not None or speaker_wav is not None
        ):
