...
def load_audio(audiopath, sampling_rate):
    ...
    if torch.any(audio > 10) or not torch.any(audio < 0):
        logger.error(
            "Error with %s. Max=%.2f min=%.2f", audiopath, audio.max(), audio.min()
        )
    ...
    def synthesize(
        ...
        **kwargs,
        ...
        return self.inference(
            text, language, gpt_cond_latent, speaker_embedding, **inference_settings
        )
        ...
        **hf_generate_kwargs,
        ...
        for sent in text:
            ...
            while not is_end:
                ...
                if is_end or (
                    stream_chunk_size > 0 and len(last_tokens) >= stream_chunk_size
                ):
    ...
    def load_checkpoint(
        ...
        if vocab_path is None:
            if (
                ...
                and (Path(checkpoint_dir) / "vocab.json").is_file()
            ):
        ...
        if eval:
            ...
            self.gpt.init_gpt_for_inference(
                kv_cache=self.args.kv_cache, use_deepspeed=use_deepspeed
            )
            self.gpt.eval()
