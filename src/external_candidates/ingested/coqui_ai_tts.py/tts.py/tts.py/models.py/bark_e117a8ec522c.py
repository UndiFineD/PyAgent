...
class Bark(BaseTTS):
    ...
    def _generate_voice(
        self, speaker_wav: str | os.PathLike[Any]
        ...
        hubert_manager.make_sure_tokenizer_installed(
            model_path=self.config.LOCAL_MODEL_PATHS["hubert_tokenizer"]
        )
        ...
        with torch.inference_mode():
            semantic_vectors = hubert_model.forward(
                audio[0], input_sample_hz=self.config.sample_rate
            )
    ...
    def get_voices(self, voice_dir: str | os.PathLike[Any]) -> dict[str, Path]:
        ...
        return {
            path.stem: path
            for path in Path(voice_dir).iterdir()
    ...
    def load_checkpoint(
        ...
        **kwargs,
        ...
        coarse_model_path = coarse_model_path or os.path.join(
            checkpoint_dir, "coarse_2.pt"
        )
        fine_model_path = fine_model_path or os.path.join(checkpoint_dir, "fine_2.pt")
        hubert_tokenizer_path = hubert_tokenizer_path or os.path.join(
            checkpoint_dir, "tokenizer.pth"
        )
