# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\datasets\__init__.py
import logging
import os
import sys
...
def load_tts_samples(
    ...
    for dataset in datasets:
        ...
        logger.info(
            "Found %d files in %s", len(meta_data_train), Path(root_path).resolve()
        )
...
def find_unique_chars(data_samples):
    ...
    logger.info(
        "Unique all forced to lower characters: %s", "".join(sorted(chars_force_lower))
    )
