...
def get_attribute_balancer_weights(
    items: list, attr_name: str, multi_dict: dict | None = None
):
    ...
    if multi_dict is not None:
        ...
        # scale weights
        multiplier_samples = np.array(
            [multi_dict.get(item[attr_name], 1.0) for item in items]
        )
    ...
    def _compute_lengths(samples):
        ...
        for item in samples:
            try:
                ...
                logger.warning(
                    "Failed to compute length, skipping %s", item["audio_file"]
                )
                continue
