# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\TTS\tts\datasets\formatters.py
import csv
import logging
import os
import re
        ...
        items.append(
            {
                ...
                "emotion_name": (
                    emotion_name if emotion_name is not None else row["emotion_name"]
                ),
                "root_path": root_path,
            }
        )
        ...
        items.append(
            {
                ...
                "audio_file": audio_path,
                "speaker_name": (
                    speaker_name if speaker_name is not None else row["speaker_name"]
                ),
                "emotion_name": (
                    emotion_name if emotion_name is not None else row["emotion_name"]
                ),
...
def baker(
    root_path: str, meta_file: str, **kwargs
) -> list[list[str]]:  # pylint: disable=unused-argument
