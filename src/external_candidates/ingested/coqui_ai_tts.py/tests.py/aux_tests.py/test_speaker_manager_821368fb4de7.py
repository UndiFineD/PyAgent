...
d_vectors_file_pth_path = os.path.join(
...
class SpeakerManagerTest(unittest.TestCase):
    ...
    def test_speaker_embedding():
        ...
        d_vector3 = manager.compute_embedding_from_clip(
    ...
    def test_dvector_file_processing(self):
        ...
        d_vector1 = manager.get_mean_embedding(
            manager.speaker_names[0], num_samples=2, randomize=True
        )
        assert len(d_vector1) == 256
        d_vector2 = manager.get_mean_embedding(
            manager.speaker_names[0], num_samples=2, randomize=False
        )
