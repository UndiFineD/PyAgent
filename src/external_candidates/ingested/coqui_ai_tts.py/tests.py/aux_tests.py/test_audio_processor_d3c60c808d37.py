# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\aux_tests\test_audio_processor.py
import os
...
def test_normalize(ap):
    ...
    wav = ap.sound_norm(
        wav
    )  # normalize audio to get abetter normalization range below.
    ...
    assert x_norm.max() <= ap.max_norm + 1, x_norm.max()
    assert (
        x_norm.min() >= -ap.max_norm - 2
    ), x_norm.min()  # pylint: disable=invalid-unary-operand-type
    assert x_norm.min() <= 0, x_norm.min()
    # check denorm.
    x_ = ap.denormalize(x_norm)
    assert (x - x_).sum() < 1e-3, (x - x_).mean()
