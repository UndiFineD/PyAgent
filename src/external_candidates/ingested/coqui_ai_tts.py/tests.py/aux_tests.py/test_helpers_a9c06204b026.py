# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\aux_tests\test_helpers.py
import torch as T
...
def test_average_over_durations():  # pylint: disable=no-self-use
    ...
    for idx, dur in enumerate(durations[0]):
        assert (
            abs(pitch_avg[0, 0, idx] - pitch[0, 0, index : index + dur.item()].mean())
            < 1e-5
        )
        index += dur
...
def test_segment():
    ...
    for idx, start_indx in enumerate(segment_ids):
        assert (
            x[idx, :, start_indx : start_indx + 10].sum() == segments[idx, :, :].sum()
        )
...
def test_rand_segments():
    ...
    segments, seg_idxs = rand_segments(
        x, x_lens.clone(), segment_size=5, pad_short=True, let_short_samples=True
    )
...
def test_generate_path():
    ...
    for b in range(durations.shape[0]):
        ...
        for t in range(durations.shape[1]):
            assert all(
                path[b, t, current_idx : current_idx + durations[b, t].item()] == 1.0
            )
...
def test_expand_encoder_outputs():
    ...
    for b in range(durations.shape[0]):
        ...
        for idx, dur in enumerate(durations[b]):
            ...
            diff = (
                idx_expanded
                - inputs[b, :, idx].repeat(int(dur)).view(idx_expanded.shape)
            ).sum()
