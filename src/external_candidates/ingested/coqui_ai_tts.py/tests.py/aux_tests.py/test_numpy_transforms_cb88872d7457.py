...
class TestNumpyTransforms(unittest.TestCase):
    ...
    def test_build_mel_basis(self):
        ...
        self.assertEqual(
            mel_basis.shape, (self.config.num_mels, self.config.fft_size // 2 + 1)
        )
    ...
    def test_wav_to_mel(self):
        mel_basis = np_transforms.build_mel_basis(**self.config)
        mel = np_transforms.wav_to_mel(
            wav=self.sample_wav, mel_basis=mel_basis, **self.config
        )
        self.assertEqual(
            mel.shape,
            (
    ...
    def test_load_wav(self):
        wav = np_transforms.load_wav(
            filename=WAV_FILE, resample=False, sample_rate=22050
        )
        wav_resample = np_transforms.load_wav(
            filename=WAV_FILE, resample=True, sample_rate=16000
        )
