# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\integration\test_train_vocoder.py
import glob
import os
...
def run_train(tmp_path, config):
    ...
    continue_path = str(
        max(glob.glob(os.path.join(output_path, "*/")), key=os.path.getmtime)
    )
...
def test_train_fullband_melgan(tmp_path):
    config = create_config(
        FullbandMelganConfig, discriminator_model_params=DISCRIMINATOR_MODEL_PARAMS
    )
    run_train(tmp_path, config)
...
def test_train_parallel_wavegan(tmp_path):
    config = create_config(
        ParallelWaveganConfig, batch_size=4, eval_batch_size=4, seq_len=2048
    )
    run_train(tmp_path, config)
...
@pytest.mark.skipif(
    GITHUB_ACTIONS, reason="Takes ~2h on CI (15min/step vs 8sec/step locally)"
)
