# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\integration\test_train_tts.py
import pytest
...
def test_train_delightful_tts(tmp_path, speaker_args):
    config = create_config(
        DelightfulTTSConfig,
        batch_size=2,
        f0_cache_path=tmp_path
        / "f0_cache",  # delightful f0 cache is incompatible with other models
...
def test_train_fast_pitch(tmp_path, speaker_args):
    config = create_config(
        FastPitchConfig, f0_cache_path="tests/data/ljspeech/f0_cache", **speaker_args
    )
...
def test_train_glow_tts(tmp_path, speaker_args):
    config = create_config(
        GlowTTSConfig, batch_size=2, data_dep_init_steps=1, **speaker_args
    )
    run_tts_train(tmp_path, config)
...
def test_train_neuralhmm(tmp_path):
    config = create_config(
        NeuralhmmTTSConfig, batch_size=3, eval_batch_size=3, max_sampling_time=50
    )
    run_tts_train(tmp_path, config)
...
def test_train_overflow(tmp_path):
    config = create_config(
        OverflowConfig, batch_size=3, eval_batch_size=3, max_sampling_time=50
    )
    run_tts_train(tmp_path, config)
...
def test_train_tacotron(tmp_path):
    config = create_config(
        TacotronConfig, use_phonemes=False, r=5, max_decoder_steps=50
    )
    run_tts_train(tmp_path, config)
...
def test_train_tacotron2(tmp_path, speaker_args):
    config = create_config(
        Tacotron2Config, use_phonemes=False, r=5, max_decoder_steps=50, **speaker_args
    )
    run_tts_train(tmp_path, config)
