# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\vc_tests\test_freevc.py
import os
import unittest
...
class TestFreeVC(unittest.TestCase):
    def _create_inputs(self, config, batch_size=2):
        spec = torch.rand(batch_size, 30, config.audio["filter_length"] // 2 + 1).to(
            device
        )
        ...
        waveform = torch.rand(batch_size, spec.size(2) * config.audio["hop_length"]).to(
            device
        )
    ...
    def test_voice_conversion(self):
        ...
        assert (
            output_wav.shape[0]
            == source_wav.shape[0] - source_wav.shape[0] % config.audio.hop_length
