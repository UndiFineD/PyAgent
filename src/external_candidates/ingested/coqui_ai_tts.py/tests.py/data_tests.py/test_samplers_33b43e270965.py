# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\data_tests\test_samplers.py
import functools
import random
import unittest
...
class TestSamplers(unittest.TestCase):
    def test_language_random_sampler(self):  # pylint: disable=no-self-use
        ...
        ids = functools.reduce(
            lambda a, b: a + b, [list(random_sampler) for i in range(100)]
        )
        en, pt = 0, 0
        for index in ids:
    ...
    def test_language_weighted_random_sampler(self):  # pylint: disable=no-self-use
        ...
        ids = functools.reduce(
            lambda a, b: a + b, [list(weighted_sampler) for i in range(100)]
        )
        en, pt = 0, 0
        for index in ids:
            if train_samples[index]["language"] == "en":
        ...
        assert is_balanced(
            en, pt
        ), "Language Weighted sampler is supposed to be balanced"
    ...
    def test_speaker_weighted_random_sampler(self):  # pylint: disable=no-self-use
        ...
        assert is_balanced(
            spk1, spk2
        ), "Speaker Weighted sampler is supposed to be balanced"
    ...
    def test_perfect_sampler(self):  # pylint: disable=no-self-use
        ...
        batchs = functools.reduce(
            lambda a, b: a + b, [list(sampler) for i in range(100)]
        )
        for batch in batchs:
            spk1, spk2 = 0, 0
            # for in each batch
            ...
            assert (
                spk1 == spk2
            ), "PerfectBatchSampler is supposed to be perfectly balanced"
    ...
    def test_length_weighted_random_sampler(self):  # pylint: disable=no-self-use
        for _ in range(1000):
            ...
            weighted_sampler = torch.utils.data.sampler.WeightedRandomSampler(
                get_length_balancer_weights(train_samples, num_buckets=2),
                len(train_samples),
            )
            ids = functools.reduce(
                lambda a, b: a + b, [list(weighted_sampler) for i in range(100)]
            ...
            assert is_balanced(
                len1, len2
            ), "Length Weighted sampler is supposed to be balanced"
