# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\data_tests\test_loader.py
import os
import shutil
...
def _create_dataloader(
    batch_size, r, bgs, dataset_config, start_by_longest=False, preprocess_samples=False
):
    # load dataset
    meta_data_train, meta_data_eval = load_tts_samples(
        dataset_config, eval_split=True, eval_split_size=0.2
    )
...
def test_loader(dataset_config: BaseDatasetConfig):
    batch_size = 1
    dataloader, _ = _create_dataloader(
        batch_size, 1, 0, dataset_config, preprocess_samples=True
    )
    for i, data in enumerate(dataloader):
        ...
        mel_diff = (mel_new[:, : mel_input.shape[1]] - mel_input[0].T.numpy())[
            :, 0:ignore_seg
        ]
...
def test_start_by_longest():
    ...
    """
    dataloader, _ = _create_dataloader(
        2, c.r, 0, dataset_config_wav, start_by_longest=True
    )
    dataloader.dataset.preprocess_samples()
    for i, data in enumerate(dataloader):
        if i == max_loader_iter:
...
def test_padding_and_spectrograms(tmp_path):
    ...
    for i, data in enumerate(dataloader):
        ...
        assert (
            stop_target[1, mel_lengths[1] :].sum()
...
def test_custom_formatted_dataset_with_loader():
    ...
    train_samples, eval_samples = load_tts_samples(
        dataset_configs, eval_split=True, eval_split_size=0.2
    )
