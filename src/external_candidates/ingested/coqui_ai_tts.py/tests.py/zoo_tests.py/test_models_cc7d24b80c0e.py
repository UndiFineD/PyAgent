...
model_names = [
    name
    for name in TTS.list_models()
    if name not in MODELS_WITH_SEP_TESTS and name not in BROKEN_MODELS
]
model_names.extend(["tts_models/deu/fairseq/vits", "tts_models/sqi/fairseq/vits"])
model_names = [
    name for i, name in enumerate(model_names) if i % num_partitions == partition
]
...
def test_models(tmp_path, model_name, manager):
    ...
    if "tts_models" in model_name:
        ...
        run_main(
            main, [*args, "--text", "This is an example.", *speaker_arg, *language_arg]
        )
    elif "voice_conversion_models" in model_name:
        speaker_wav = os.path.join(
            get_tests_data_path(), "ljspeech", "wavs", "LJ001-0001.wav"
        )
        reference_wav1 = os.path.join(
            get_tests_data_path(), "ljspeech", "wavs", "LJ001-0028.wav"
        )
        reference_wav2 = os.path.join(
            get_tests_data_path(), "ljspeech", "wavs", "LJ001-0032.wav"
        )
        run_main(
            main,
            [
