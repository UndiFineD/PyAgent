# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\vocoder_tests\test_wavegrad.py
import numpy as np
import torch
...
def test_train_step():
    ...
    for i, (param, param_ref) in enumerate(
        zip(model.parameters(), model_ref.parameters())
    ):
        ...
        assert (
            ...
            f"param {i} with shape {param.shape} not updated!! \n{param}\n{param_ref}"
        )
