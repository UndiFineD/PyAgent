# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\tts_tests\test_glow_tts.py
import copy
import os
import unittest
...
class TestGlowTTS(unittest.TestCase):
    ...
    def test_init_multispeaker(self):
        ...
        config = GlowTTSConfig(num_chars=32)
        config.use_speaker_embedding = True
        config.speakers_file = os.path.join(
            get_tests_data_path(), "ljspeech", "speakers.json"
        )
        ...
        config.d_vector_file = os.path.join(
            get_tests_data_path(), "dummy_speakers.json"
        )
        speaker_manager = SpeakerManager.init_from_config(config)
        model = GlowTTS(config)
        model.speaker_manager = speaker_manager
    ...
    def _test_forward(self, batch_size):
        input_dummy, input_lengths, mel_spec, mel_lengths, speaker_ids = (
            self._create_inputs(batch_size)
        )
        # create model
        config = GlowTTSConfig(num_chars=32)
        model = GlowTTS(config).to(device)
        ...
        self.assertEqual(
            y["alignments"].shape, mel_spec.shape[:2] + (input_dummy.shape[1],)
        )
    ...
    def _test_forward_with_d_vector(self, batch_size):
        ...
        # inference encoder and decoder with MAS
        y = model.forward(
            input_dummy, input_lengths, mel_spec, mel_lengths, {"d_vectors": d_vector}
        )
        self.assertEqual(y["z"].shape, mel_spec.shape)
        self.assertEqual(y["logdet"].shape, torch.Size([batch_size]))
        self.assertEqual(y["y_mean"].shape, mel_spec.shape)
    ...
    def _assert_inference_outputs(self, outputs, input_dummy, mel_spec):
        ...
        self.assertEqual(
            outputs["alignments"].shape, output_shape[:2] + (input_dummy.shape[1],)
        )
    ...
    def _test_inference_with_d_vector(self, batch_size):
        ...
        outputs = model.inference(
            input_dummy, {"x_lengths": input_lengths, "d_vectors": d_vector}
        )
        self._assert_inference_outputs(outputs, input_dummy, mel_spec)
    ...
    def _test_inference_with_speaker_ids(self, batch_size):
        ...
        model = GlowTTS.init_from_config(config).to(device)
        outputs = model.inference(
            input_dummy, {"x_lengths": input_lengths, "speaker_ids": speaker_ids}
        )
        self._assert_inference_outputs(outputs, input_dummy, mel_spec)
    ...
    def test_train_eval_log(self):
        ...
        logger = TensorboardLogger(
            log_dir=os.path.join(get_tests_output_path(), "dummy_glow_tts_logs"),
    ...
    def test_load_checkpoint(self):
        chkp_path = os.path.join(
            get_tests_output_path(), "dummy_glow_tts_checkpoint.pth"
        )
        config = GlowTTSConfig(num_chars=32)
        model = GlowTTS.init_from_config(config).to(device)
        chkp = {}
