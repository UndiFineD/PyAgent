# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\tts_tests\test_overflow.py
import os
import random
import unittest
...
torch.save(
    {"mean": -5.5138, "std": 2.0636, "init_transition_prob": 0.3212}, parameter_path
)
...
def _create_inputs(batch_size=8):
    ...
    input_lengths = (
        torch.randint(20, max_len_t, (batch_size,))
        .long()
        .to(device)
    ...
    mel_spec = torch.randn(batch_size, max_len_m, config_global.audio["num_mels"]).to(
        device
    )
    mel_lengths = (
        torch.randint(40, max_len_m, (batch_size,))
        .long()
        .to(device)
        .sort(descending=True)[0]
    )
...
class TestOverflow(unittest.TestCase):
    def test_forward(self):
        ...
        self.assertEqual(
            model.state_per_phone * max(input_lengths), outputs["alignments"].shape[2]
        )
    ...
    def test_inference(self):
        ...
        self.assertEqual(
            output_dict["model_outputs"].shape[2], config_global.out_channels
        )
...
class TestOverflowEncoder(unittest.TestCase):
    ...
    def get_encoder(state_per_phone):
        ...
        return Encoder(
            config.num_chars,
            config.state_per_phone,
            config.prenet_dim,
            config.encoder_n_convolutions,
        ).to(device)
...
class TestOverflowUtils(unittest.TestCase):
    def test_logsumexp(self):
        ...
        self.assertTrue(
            torch.eq(torch.logsumexp(a, dim=0), OverflowUtils.logsumexp(a, dim=0)).all()
        )
...
class TestOverflowDecoder(unittest.TestCase):
    @staticmethod
    def _get_decoder(
        num_flow_blocks_dec=None, hidden_channels_dec=None, reset_weights=True
    ):
        config = deepcopy(config_global)
        config.num_flow_blocks_dec = (
            num_flow_blocks_dec
            if num_flow_blocks_dec is not None
            else config.num_flow_blocks_dec
        )
        config.hidden_channels_dec = (
            hidden_channels_dec
            if hidden_channels_dec is not None
            else config.hidden_channels_dec
        )
...
class TestNeuralHMM(unittest.TestCase):
    ...
    def _get_neural_hmm(deterministic_transition=None):
        ...
        neural_hmm = NeuralHMM(
            ...
            (
                ...
                if deterministic_transition is None
                else deterministic_transition
            ),
    ...
    def _get_embedded_input():
        ...
        input_dummy = torch.nn.Embedding(
            config_global.num_chars, config_global.encoder_in_out_features
    ...
    def test_mask_lengths(self):
        ...
        log_c, log_alpha_scaled = (
            neural_hmm._mask_lengths(  # pylint: disable=protected-access
        ...
        for i in range(mel_spec.shape[0]):
            assertions.append(
                log_alpha_scaled[i, mel_lengths[i] :, : input_lengths[i]].sum() == 0.0
            )
        self.assertTrue(all(assertions), "Incorrect masking")
    ...
    def test_process_ar_timestep(self):
        ...
        h_post_prenet, c_post_prenet = (
            model._init_lstm_states(  # pylint: disable=protected-access
                input_dummy.shape[0], config_global.memory_rnn_dim, mel_spec
            )
        )
        h_post_prenet, c_post_prenet = (
            model._process_ar_timestep(  # pylint: disable=protected-access
                1,
                mel_spec,
                h_post_prenet,
                c_post_prenet,
        ...
        self.assertEqual(
            h_post_prenet.shape, (input_dummy.shape[0], config_global.memory_rnn_dim)
        )
        self.assertEqual(
            c_post_prenet.shape, (input_dummy.shape[0], config_global.memory_rnn_dim)
        )
    ...
    def test_add_go_token(self):
        ...
        self.assertTrue(
            (out[:, 1:] == mel_spec[:, :-1]).all(), "Go token not appended properly"
        )
    ...
    def test_get_absorption_state_scaling_factor(self):
        ...
        transition_prob_mask = ~model.get_mask_for_last_item(
            input_lengths, device=input_lengths.device
        )

        outputs = []

        for i in range(input_dummy.shape[0]):
            last_log_alpha_scaled = log_alpha_scaled[i, mel_lengths[i] - 1].masked_fill(
    ...
    def test_inference(self):
        ...
        for temp in [0.334, 0.667, 1.0]:
            outputs = model.inference(
                ...
                input_lengths,
                temp,
                config_global.max_sampling_time,
                config_global.duration_threshold,
            )
            self.assertEqual(
                outputs["hmm_outputs"].shape[-1],
                outputs["input_parameters"][0][0][0].shape[-1],
            )
            self.assertEqual(
                outputs["output_parameters"][0][0][0].shape[-1],
                outputs["input_parameters"][0][0][0].shape[-1],
            )
    ...
    def test_emission_model(self):
        ...
        means = torch.randn(
            input_dummy.shape[0], input_dummy.shape[1], config_global.out_channels
        ).to(device)
    ...
    def test_transition_model(self):
        ...
        input_dummy, input_lengths, mel_spec, mel_lengths = self._get_embedded_input()
        prev_t_log_scaled_alph = torch.randn(
            input_dummy.shape[0], input_lengths.max()
        ).to(device)
...
class TestOverflowOutputNet(unittest.TestCase):
    ...
    def _get_embedded_input():
        ...
        one_timestep_frame = torch.randn(
            input_dummy.shape[0], config_global.memory_rnn_dim
        ).to(device)
    ...
    def test_outputnet_forward_with_flat_start(self):
        ...
        self.assertTrue(
            torch.isclose(
                mean, torch.tensor(model.flat_start_params["mean"] * 1.0)
            ).all()
        )
        self.assertTrue(
            torch.isclose(std, torch.tensor(model.flat_start_params["std"] * 1.0)).all()
        )
        self.assertTrue(
            torch.isclose(
                transition_vector.sigmoid(),
                torch.tensor(model.flat_start_params["transition_p"] * 1.0),
            ).all()
        )
