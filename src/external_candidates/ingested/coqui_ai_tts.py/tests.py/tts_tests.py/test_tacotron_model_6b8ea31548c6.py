# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\tts_tests\test_tacotron_model.py
import copy
import os
import unittest
...
config_global = TacotronConfig(
    num_chars=32, num_speakers=5, out_channels=513, decoder_output_dim=80
)
...
class TacotronTrainTest(unittest.TestCase):
    ...
    def test_train_step():
        ...
        stop_targets = stop_targets.view(
            input_dummy.shape[0], stop_targets.size(1) // config.r, -1
        )
        stop_targets = (stop_targets.sum(2) > 0.0).unsqueeze(2).float().squeeze()

        criterion = L1LossMasked(seq_len_norm=False).to(device)
        criterion_st = nn.BCEWithLogitsLoss().to(device)
        model = Tacotron(config).to(
            device
        ...
        for _ in range(5):
            ...
            loss = (
                loss
                + criterion(outputs["model_outputs"], linear_spec, mel_lengths)
                + stop_loss
            )
            loss.backward()
            optimizer.step()
...
class TacotronGSTTrainTest(unittest.TestCase):
    ...
    def test_train_step():
        ...
        # with file gst style
        mel_spec = (
            torch.FloatTensor(ap.melspectrogram(ap.load_wav(WAV_FILE)))[:, :120]
            .unsqueeze(0)
            .transpose(1, 2)
            .to(device)
        )
        ...
        input_lengths = torch.randint(100, 129, (8,)).long().to(device)
        input_lengths[-1] = 128
        linear_spec = torch.rand(
            8, mel_spec.size(1), config.audio["fft_size"] // 2 + 1
        ).to(device)
...
class TacotronCapacitronTrainTest(unittest.TestCase):
    ...
    def test_train_step():
        ...
        batch["linear_input"] = torch.rand(
            8, 120, config.audio["fft_size"] // 2 + 1
        ).to(device)
        ...
        batch["stop_targets"] = (
            (batch["stop_targets"].sum(2) > 0.0).unsqueeze(2).float().squeeze()
        )
        ...
        model.train()
        print(
            f" > Num parameters for Tacotron with Capacitron VAE model:{count_parameters(model)}"
        )
        model_ref = copy.deepcopy(model)
        count = 0
        for param, param_ref in zip(model.parameters(), model_ref.parameters()):
...
class SCGSTMultiSpeakeTacotronTrainTest(unittest.TestCase):
    ...
    def test_train_step():
        ...
        # check parameter changes
        count = 0
        for name_param, param_ref in zip(
            model.named_parameters(), model_ref.parameters()
        ):
            # ignore pre-higway layer since it works conditional
            # if count not in [145, 59]:
