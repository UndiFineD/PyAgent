...
SPEAKER_ENCODER_CONFIG = os.path.join(
    get_tests_input_path(), "test_speaker_encoder_config.json"
)
...
class TestVits(unittest.TestCase):
    def test_load_audio(self):
        ...
        spec = wav_to_spec(
            wav, n_fft=1024, hop_length=512, win_length=1024, center=False
        )
        mel = wav_to_mel(
            wav,
            n_fft=1024,
        ...
        mel2 = spec_to_mel(
            spec, n_fft=1024, num_mels=80, sample_rate=sr, fmin=0, fmax=8000
        )
    ...
    def _create_inputs(self, config, batch_size=2):
        ...
        waveform = torch.rand(
            batch_size, 1, spec.size(2) * config.audio["hop_length"]
        ).to(device)
        return input_dummy, input_lengths, mel, spec, spec_lengths, waveform

    def _check_forward_outputs(
        self, config, output_dict, encoder_config=None, batch_size=2
    ):
        self.assertEqual(
        ...
        self.assertEqual(
            output_dict["z"].shape, (batch_size, config.model_args.hidden_channels, 30)
        )
        self.assertEqual(
    ...
    def test_forward(self):
        ...
        input_dummy, input_lengths, _, spec, spec_lengths, waveform = (
            self._create_inputs(config)
        )
        model = Vits(config).to(device)
        output_dict = model.forward(
            input_dummy, input_lengths, spec, spec_lengths, waveform
        )
    ...
    def test_d_vector_forward(self):
        ...
        model.train()
        input_dummy, input_lengths, _, spec, spec_lengths, waveform = (
            self._create_inputs(config, batch_size=batch_size)
        )
    ...
    def test_multilingual_forward(self):
        ...
        config = VitsConfig(
            num_speakers=num_speakers, use_speaker_embedding=True, model_args=args
        )
    ...
    def _check_inference_outputs(self, config, outputs, input_dummy, batch_size=1):
        ...
        self.assertEqual(
            outputs["model_outputs"].shape[:2], (batch_size, 1)
        )  # we don't know the channel dimension
        self.assertEqual(
            outputs["alignments"].shape, (batch_size, input_dummy.shape[1], feat_len)
        )
        self.assertEqual(
            outputs["z"].shape,
    ...
    def test_multilingual_inference(self):
        ...
        _ = model.inference(
            input_dummy, {"speaker_ids": speaker_ids, "language_ids": lang_ids}
        )
    ...
    def test_d_vector_inference(self):
        ...
        outputs = model.inference(
            input_dummy, aux_input={"x_lengths": input_lengths, "d_vectors": d_vectors}
        )
    ...
    def _create_batch(self, config, batch_size):
        input_dummy, input_lengths, mel, spec, mel_lengths, _ = self._create_inputs(
            config, batch_size
        )
        batch = {}
    ...
    def test_init_from_config(self):
        ...
        config = VitsConfig(
            model_args=VitsArgs(
                ...
                d_vector_dim=256,
                d_vector_file=[
                    os.path.join(get_tests_data_path(), "dummy_speakers.json")
                ],
            )
        )
        model = Vits.init_from_config(config).to(device)
