# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\tts_tests\test_losses.py
import unittest
...
class BCELossTest(unittest.TestCase):
    def test_in_out(self):  # pylint: disable=no-self-use
        ...
        true_x = (
            target * 200 - 100
        )  # creates logits of [-100, -100, ... 100, 100] corresponding to target
        zero_x = (
            T.zeros(target.shape) - 100.0
        )  # simulate logits if it never stops decoding
        early_x = (
            -200.0 * sequence_mask(length - 3, 100).float() + 100.0
        )  # simulate logits on early stopping
        late_x = (
            -200.0 * sequence_mask(length + 1, 100).float() + 100.0
        )  # simulate logits on late stopping
