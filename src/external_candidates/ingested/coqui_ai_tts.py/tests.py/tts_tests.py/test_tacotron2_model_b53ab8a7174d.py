...
config_global = Tacotron2Config(
    num_chars=32, num_speakers=5, out_channels=80, decoder_output_dim=80
)
...
class TacotronTrainTest(unittest.TestCase):
    ...
    def test_train_step(self):  # pylint: disable=no-self-use
        ...
        for i in range(5):
            ...
            loss = (
                loss
                + criterion(outputs["model_outputs"], mel_postnet_spec, mel_lengths)
                + stop_loss
            )
            loss.backward()
            optimizer.step()
...
class TacotronGSTTrainTest(unittest.TestCase):
    ...
    def test_train_step(self):
        ...
        # with file gst style
        mel_spec = (
            torch.FloatTensor(ap.melspectrogram(ap.load_wav(WAV_FILE)))[:, :30]
            .unsqueeze(0)
            .transpose(1, 2)
            .to(device)
        )
