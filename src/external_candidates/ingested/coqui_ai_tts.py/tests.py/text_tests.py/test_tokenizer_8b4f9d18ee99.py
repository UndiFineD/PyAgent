# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\text_tests\test_tokenizer.py
import unittest
...
class TestTTSTokenizer(unittest.TestCase):
    def setUp(self):
        ...
        self.tokenizer_ph = TTSTokenizer(
            use_phonemes=True, characters=IPAPhonemes(), phonemizer=self.ph
        )
    ...
    def test_text_to_ids_phonemes_with_eos_bos(self):
        ...
        text_ph = (
            IPAPhonemes().bos
            + self.ph.phonemize(text, separator="")
            + IPAPhonemes().eos
        )
        ids = self.tokenizer_ph.text_to_ids(text)
        test_hat = self.tokenizer_ph.ids_to_text(ids)
