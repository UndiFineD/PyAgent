# Extracted from: C:\DEV\PyAgent\.external\coqui-ai-TTS\tests\text_tests\test_phonemizer.py
import unittest
...
class TestGruutPhonemizer(unittest.TestCase):
    def setUp(self):
        self.phonemizer = Gruut(
            language="en-us", use_espeak_phonemes=True, keep_stress=False
        )
...
class TestBN_Phonemizer(unittest.TestCase):
    ...
    def test_phonemize(self):
        self.assertEqual(
            self.phonemizer.phonemize(self._TEST_CASES, separator=""), self._EXPECTED
        )
...
class TestMultiPhonemizer(unittest.TestCase):
    def setUp(self):
        self.phonemizer = MultiPhonemizer(
            {"tr": "espeak", "en-us": "", "de": "gruut", "zh-cn": ""}
        )
    ...
    def test_sub_phonemizers(self):
        for lang in self.phonemizer.lang_to_phonemizer_name.keys():
            ...
            self.assertEqual(
                self.phonemizer.lang_to_phonemizer_name[lang],
                self.phonemizer.lang_to_phonemizer[lang].name(),
            )
