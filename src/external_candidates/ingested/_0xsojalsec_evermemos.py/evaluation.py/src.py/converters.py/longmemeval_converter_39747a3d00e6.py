# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\converters\longmemeval_converter.py
"""
    ...
    for data in lmeval_data:
        ...
        for idx, session in enumerate(data["haystack_sessions"]):
            for i, msg in enumerate(session):
                data["haystack_sessions"][idx][i]["has_answer"] = (
                    idx in evidence_session_idx
                )
...
class LongMemEvalConverter(BaseConverter):
    ...
    def get_input_files(self) -> Dict[str, str]:
        ...
        return {"raw": "longmemeval_s_cleaned.json"}
