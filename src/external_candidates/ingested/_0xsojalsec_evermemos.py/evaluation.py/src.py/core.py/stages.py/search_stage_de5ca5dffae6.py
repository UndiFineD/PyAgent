# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\core\stages\search_stage.py
"""
...
async def run_search_stage(
    ...
    if processed_convs:
        print(
            f"Already processed: {len(processed_convs)} conversations (from checkpoint)"
        )
    ...
    processed_questions = sum(
        len(all_search_results_dict.get(conv_id, [])) for conv_id in processed_convs
    )
            ...
            for attempt in range(max_retries):
                try:
                    result = await asyncio.wait_for(
                        adapter.search(
                            qa.question, conv_id, index, conversation=conversation
                        ),
                    ...
                    if attempt < max_retries - 1:
                        tqdm.write(
                            f"  ⏱️  Search timeout ({timeout_seconds}s) for question in {conv_id}, retry {attempt + 1}/{max_retries}..."
                        )
                        await asyncio.sleep(2)  # Short delay before retry
                    else:
                        tqdm.write(
                            f"  ❌ Search timeout after {max_retries} attempts for question in {conv_id}: {qa.question[:60]}..."
                        )
                    ...
                    if attempt < max_retries - 1:
                        tqdm.write(
                            f"  ⚠️  Search failed for question in {conv_id}: {str(e)}, retry {attempt + 1}/{max_retries}..."
                        )
                        await asyncio.sleep(2)
                    else:
                        tqdm.write(
                            f"  ❌ Search failed after {max_retries} attempts for question in {conv_id}: {str(e)}"
                        )
    ...
    for idx, (conv_id, qa_list) in enumerate(sorted(conv_to_qa.items(), key=sort_key)):
        ...
        tqdm.write(
            f"Processing Conversation ID: {conv_id} ({idx+1}/{total_convs}) - {len(qa_list)} questions"
        )
