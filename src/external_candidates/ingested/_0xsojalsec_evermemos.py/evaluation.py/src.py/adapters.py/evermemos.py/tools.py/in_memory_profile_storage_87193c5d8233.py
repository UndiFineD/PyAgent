# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\adapters\evermemos\tools\in_memory_profile_storage.py
...
class InMemoryProfileStorage:
    ...
    def _persist_to_disk(
        self, user_id: str, profile: Any, metadata: Optional[Dict[str, Any]]
    ) -> None:
    ...
    def _load_from_disk(self) -> None:
        ...
        try:
            for profile_file in self._persist_dir.glob("profile_*.json"):
                if (
                    profile_file.stem.startswith("profile_")
                    and not profile_file.parent.name == "history"
                ):
                    try:
                        ...
                        if user_dir.is_dir():
                            ...
                            for version_file in sorted(user_dir.glob("profile_*.json")):
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to load history from {version_file}: {e}"
                                    )
