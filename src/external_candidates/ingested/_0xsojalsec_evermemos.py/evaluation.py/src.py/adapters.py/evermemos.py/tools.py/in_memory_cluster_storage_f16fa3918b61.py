# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\evaluation\src\adapters\evermemos\tools\in_memory_cluster_storage.py
...
class InMemoryClusterStorage:
    ...
    def __init__(
        self, enable_persistence: bool = False, persist_dir: Optional[Path] = None
    ):
    ...
    def _persist_to_disk(self, group_id: str, state: Dict[str, Any]) -> None:
        ...
        try:
            for state_file in self._persist_dir.glob("cluster_state_*.json"):
                try:
                    ...
                    logger.warning(
                        f"Failed to load cluster state from {state_file}: {e}"
                    )
        except Exception as e:
