# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\tests\test_redis_distributed_lock.py
"""
...
from core.lock.redis_distributed_lock import distributed_lock, with_distributed_lock
...
async def test_basic_lock_operations(redis_distributed_lock_manager):
    ...
    async with lock.acquire() as acquired:
        ...
        assert (
            await lock.is_owned_by_current_coroutine()
        ), "The lock should be held by the current coroutine"

    # Test lock release
    assert not await lock.is_locked(), "The lock should have been released"
    assert (
        not await lock.is_owned_by_current_coroutine()
    ), "The lock should not be held by the current coroutine"
...
async def test_lock_reentrant(redis_distributed_lock_manager):
    ...
    assert (
        await lock.get_reentry_count() == 0
    ), "After full release, reentry count should be 0"
...
async def test_lock_expiration(redis_distributed_lock_manager):
    ...
    async with lock1.acquire(timeout=1) as acquired:  # Expire in 1 second
        ...
        assert (
            await lock1.is_owned_by_current_coroutine()
        ), "The lock should be held by the current coroutine"
        ...
        assert (
            not await lock1.is_locked()
        ), "The lock should have expired and been released"
    ...
    async def try_acquire_expired_lock():
        ...
        async with lock2.acquire(
            timeout=5
        ) as acquired:  # Set a long enough expiration time
            assert (
                acquired
            ), "After the original lock expires, a new coroutine should be able to acquire the lock"
            assert (
                await lock2.is_locked()
            ), "The newly acquired lock should be in locked state"
            assert (
    ...
    assert (
        success
    ), "Should be able to acquire a new lock after the original lock expires"
    ...
    for expire_time in test_times:
        ...
        async with lock.acquire(timeout=expire_time) as acquired:
            assert (
                acquired
            ), f"Should successfully acquire the lock (expiration time: {expire_time} seconds)"
    ...
    async with lock3.acquire(timeout=1) as acquired1:  # Expire in 1 second
        ...
        assert (
            await lock3.is_locked()
        ), "The lock should still exist when close to expiration"
        ...
        async with lock3.acquire(timeout=2) as acquired2:  # Expire in 2 seconds
            ...
            assert (
                await lock3.is_locked()
            ), "After re-entry with a new expiration time, the lock should still exist"
...
async def test_lock_decorator(_redis_distributed_lock_manager):
    ...
    assert (
        result2 == 42
    ), "Decorator with formatted string should not affect function return value"
...
async def test_force_unlock(redis_distributed_lock_manager):
    ...
    async with lock.acquire() as acquired:
        ...
        assert (
            not await lock.is_locked()
        ), "After force unlock, the lock should be released"
...
async def test_blocking_timeout_and_reentry(redis_distributed_lock_manager):
    ...
    async def reentry_test():
        ...
        async with lock.acquire() as acquired1:
            ...
            assert (
                await lock.get_reentry_count() == 1
            ), "Reentry count should be 1 after first acquisition"
            ...
            async with lock.acquire() as acquired2:
                ...
                assert (
                    await lock.get_reentry_count() == 2
                ), "Reentry count should be 2 after re-entry"

                # Re-enter again
                async with lock.acquire() as acquired3:
                    assert (
                        acquired3
                    ), "Third re-entry within the same task should succeed"
                    assert (
    ...
    async def blocking_task():
        ...
        async with lock.acquire() as acquired:
            ...
            assert (
                await lock.get_reentry_count() == 1
            ), "Reentry count for first task should be 1"
    ...
    async def competing_task():
        ...
        async with lock.acquire(blocking_timeout=0.5) as acquired:
            ...
            assert (
                await lock.get_reentry_count() == 0
            ), "Reentry count should be 0 when acquisition fails"
    ...
    async def parent_task():
        ...
        async with lock.acquire() as acquired:
            ...
            assert (
                await lock.get_reentry_count() == 1
            ), "Reentry count for parent task should be 1"
            ...
            async with lock.acquire() as reentry:
                ...
                assert (
                    await lock.get_reentry_count() == 2
                ), "Reentry count should be 2 after parent re-entry"
    ...
    async def child_task():
        ...
        async with lock.acquire(blocking_timeout=0.5) as acquired:
            assert (
                not acquired
            ), "Child task should not be able to acquire the lock held by parent task"
            assert (
                await lock.get_reentry_count() == 0
            ), "Reentry count should be 0 when child task fails to acquire"
    ...
    async def final_task():
        ...
        async with lock.acquire(blocking_timeout=0.5) as acquired:
            assert (
                acquired
            ), "New task should be able to acquire the lock after it is released"
            assert (
                await lock.get_reentry_count() == 1
            ), "Reentry count should be 1 after new task acquires the lock"
...
async def test_context_manager_concurrent(_redis_distributed_lock_manager):
    ...
    print(
        f"✅ Context manager concurrency test passed, successful worker: {results[0]}"
    )
