...
async def test_basic_crud_operations():
    ...
    try:
        ...
        assert (
            created.summary
            == "This is a test memory: discussed the project's technical solution"
        )
        assert created.event_id is not None
        logger.info(
            "✅ Test creating new MemCell succeeded, event_id=%s", created.event_id
        )
        ...
        assert (
            updated.summary
            == "Updated summary: project technical solution has been confirmed"
        )
...
async def test_find_by_time_range():
    ...
    try:
        ...
        logger.info(
            "   Small range query returned %d records (expected 3)", len(small_results)
        )
        ...
        logger.info(
            "   Large range query returned %d records (expected 10)", len(large_results)
        )
...
async def test_find_by_group_id():
    ...
    try:
        ...
        assert len(results) == 3
        logger.info(
            "✅ Test querying by group_id succeeded, found %d records", len(results)
        )
...
async def test_find_by_participants():
    ...
    try:
        ...
        results_all = await repo.find_by_participants(
            ["Zhang San", "Li Si"], match_all=True
        )
...
async def test_search_by_keywords():
    ...
    try:
        ...
        assert len(results_any) == 2
        logger.info(
            "✅ Test matching any keyword succeeded, found %d records", len(results_any)
        )

        # Test matching all keywords (containing both "technology" and "Python")
        results_all = await repo.search_by_keywords(
            ["technology", "Python"], match_all=True
        )
        assert len(results_all) == 1
        logger.info(
...
async def test_statistics_and_aggregation():
    ...
    try:
        ...
        assert (
            range_count == 4
        )  # Records from first 4 days (3 conversation memories + 1 email memory)
        logger.info(
...
async def test_get_by_event_ids():
    ...
    try:
        ...
        assert isinstance(
            results_with_projection, dict
        ), "Return result should be a dictionary"
        assert (
        ...
        for event_id, memcell_projection in results_with_projection.items():
            ...
            assert (
                memcell_projection.timestamp is not None
            ...
            assert not hasattr(
                memcell_projection, "episode"
            ), "episode field should not exist"
        ...
        assert isinstance(
            results_non_existent, dict
        ), "Return result should be a dictionary"
        assert (
            len(results_non_existent) == 0
        ), "Non-existent event_ids should return empty dictionary"
