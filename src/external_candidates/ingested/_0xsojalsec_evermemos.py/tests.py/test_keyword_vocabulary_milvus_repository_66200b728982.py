...
async def test_vector_similarity_search():
    ...
    try:
        ...
        assert (
            len(results) >= 2
        ), f"At least 2 similar keywords should be found, actually found {len(results)}"
...
async def test_type_filtering():
    ...
    try:
        ...
        assert (
            len(pl_results) == 2
        ), f"2 programming languages should be found, actually found {len(pl_results)}"
        ...
        assert (
            len(fw_results) == 2
        ), f"2 frameworks should be found, actually found {len(fw_results)}"
...
async def test_batch_operations():
    ...
    try:
        ...
        logger.info(
            "✅ Data verification successful: found %d keywords", len(all_keywords)
        )
...
async def test_edge_cases():
    ...
    try:
        ...
        )  # Milvus delete returns True even for non-existent ID
        ...
        try:
            ...
            assert "512" in str(e) and "1024" in str(
                e
            ), "Error message should contain dimension information"
        ...
        try:
            ...
            logger.info(
                "⚠️  Duplicate keyword is allowed to be created (same ID will be overwritten)"
            )
        except Exception as e:
...
async def test_real_world_scenario():
    ...
    logger.info(
        "========== Starting real-world scenario test: skill vocabulary =========="
    )
    ...
    try:
        ...
        assert (
            programming_count >= 1
        ), "Top 3 should include at least 1 programming skill"
