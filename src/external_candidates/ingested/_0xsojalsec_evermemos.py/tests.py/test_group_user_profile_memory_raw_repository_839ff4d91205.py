...
async def test_version_management():
    ...
    try:
        ...
        logger.info(
            "✅ Version range query [202502, 202502] successful, returned version=202502"
        )
...
async def test_batch_query_with_only_latest():
    ...
    try:
        ...
        logger.info(
            "✅ get_by_user_ids only_latest=True successful, returned 3 latest versions"
        )
        ...
        logger.info(
            "✅ get_by_user_ids only_latest=False successful, returned 9 versions"
        )
        ...
        logger.info(
            "✅ get_by_group_id only_latest=True successful, returned latest versions for 3 users"
        )
        ...
        logger.info(
            "✅ get_by_group_id only_latest=False successful, returned all 9 versions"
        )
...
async def test_get_profile_method():
    ...
    try:
        ...
        profile_data = {
            ...
            "user_goal": [
                {"value": "Become a technical expert", "evidences": ["conv_005"]}
            ],
        }
...
async def test_batch_get_by_user_groups():
    ...
    logger.info(
        "Starting test of batch retrieval of group user profiles functionality..."
    )
    ...
    try:
        ...
        for user_id, group_id, user_name in test_data:
            ...
            old_data = {
                ...
                "personality": [
                    {"value": "Old personality", "evidences": ["conv_old"]}
                ],
            }
            ...
            new_data = {
                ...
                "personality": [
                    {"value": "New personality", "evidences": ["conv_new"]}
                ],
        ...
        assert (
            len(results) == 5
        ), f"Should return 5 results, actually returned {len(results)}"
        ...
        for (user_id, group_id), profile in results.items():
            ...
            assert profile.user_name.endswith(
                "_v2"
            ), "Should return username from latest version"
            logger.info(
        ...
        assert (
            len(results_with_none) == 7
        ), "Should return 7 results (including non-existent ones)"
        ...
        logger.error(
            "❌ Batch retrieval of group user profiles functionality test failed: %s", e
        )
        # Ensure cleanup
        for user_id, group_id, _ in test_data:
            try:
    ...
    logger.info(
        "✅ Batch retrieval of group user profiles functionality test completed"
    )
