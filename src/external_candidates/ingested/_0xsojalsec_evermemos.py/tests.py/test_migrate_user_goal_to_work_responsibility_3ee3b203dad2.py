...
async def get_core_memory_repository():
    ...
    try:
        ...
        logger.info(
            f"✅ Successfully obtained CoreMemoryRawRepository: {type(core_memory_repo)}"
        )
    ...
    for record in records:
        try:
            ...
            if not record.user_goal:
                logger.warning(
                    f"⚠️  Record {record.user_id} has empty user_goal, skipping"
                )
                continue
...
async def verify_migration_results(repo: CoreMemoryRawRepository):
    ...
    try:
        ...
        print(
            f"   Remaining records where user_goal is not null: {len(remaining_user_goals)}"
        )
        print(
...
async def preview_group_migration_data():
    ...
    try:
        ...
        print(
            f"📈 Found a total of {total_count} group user profile records containing user_goal data"
        )
...
async def migrate_group_user_goal_to_work_responsibility(
    ...
    for record in records:
        try:
            ...
            success_count += 1
            logger.info(
                f"✅ Successfully migrated group record {record.user_id}-{record.group_id}"
            )
...
async def preview_skills_rename_data():
    ...
    try:
        ...
        print(
            f"📈 Found a total of {core_total_count} records in CoreMemory table containing skill fields"
        )
        print(
...
async def rename_skill_to_value_in_core_memory(records: List[CoreMemory]):
    ...
    for record in records:
        try:
            ...
            error_count += 1
            logger.error(
                f"❌ Error renaming skill fields in CoreMemory record {record.user_id}: {e}"
            )
...
async def verify_skills_rename_results():
    ...
    try:
        ...
        print(
            f"   CoreMemory records with remaining skill fields: {len(core_remaining_skill)}"
        )
        print(
            f"   GroupUserProfileMemory records with remaining skill fields: {len(group_remaining_skill)}"
        )
        print(
            f"   CoreMemory records with value fields: {len(core_records_with_value)}"
        )
        print(
        ...
        if len(core_remaining_skill) == 0 and len(group_remaining_skill) == 0:
            print(
                "✅ All skill keys in skill fields have been successfully renamed to value"
            )
        else:
...
async def main():
    """Main function"""
    print(
        "🚀 Core Memories and Group User Profile Memory Data Migration Script Started"
    )
...
async def execute_skills_rename():
    ...
    try:
        ...
        if core_skill_records:
            ...
            print("   This operation will:")
            print(
                "   - Rename the 'skill' key to 'value' key in soft_skills and hard_skills"
            )
            print("   - This operation is irreversible, please confirm to continue")
        ...
        print(
            f"   📈 Total: {len(core_skill_records) if core_skill_records else 0} records"
        )
