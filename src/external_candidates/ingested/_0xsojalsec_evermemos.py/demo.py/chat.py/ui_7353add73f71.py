# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-EverMemOS\demo\chat\ui.py
"""Terminal UI Tools
    ...
    def print_retrieved_memories(
        ...
        if retrieval_metadata and retrieval_metadata.get("retrieval_mode") == "agentic":
            ...
            if is_sufficient is not None:
                ...
                status_text = (
                    texts.get("agentic_sufficient")
                    if is_sufficient
                    else texts.get("agentic_insufficient")
                )
                agentic_info.append(
                    f"{texts.get('agentic_llm_judgment')}: {status_icon} {status_text}"
                )
            ...
            if is_multi_round:
                ...
                if refined_queries:
                    agentic_info.append(
                        f"{texts.get('agentic_generated_queries')}: {len(refined_queries)}"
                    )
            else:
            ...
            if round1_count:
                agentic_info.append(
                    f"{texts.get('agentic_round1_count')}: {round1_count} {items_text}"
                )
            if round2_count:
                agentic_info.append(
                    f"{texts.get('agentic_round2_count')}: {round2_count} {items_text}"
                )
            ...
            if agentic_info:
                ...
                if is_multi_round:
                    ...
                    if refined_queries:
                        print(
                            f"   🔍 {texts.get('agentic_supplementary_queries')} ({len(refined_queries)}):"
                        )
