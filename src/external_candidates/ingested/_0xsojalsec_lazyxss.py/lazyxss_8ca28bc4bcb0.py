# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LazyXss\lazyxss.py
import argparse
    ...
    for test_payload in [payload, encoded_payload]:
        try:
            ...
            if not all([parsed_test_url.scheme, parsed_test_url.netloc]):
                logging.error(
                    f"{RED}[error] Invalid URL with payload: {test_url}{RESET}"
                )
                continue
            ...
            is_vuln, alert_text = check_xss_with_selenium(
                url, f"?payload={test_payload}", timeout
            )
...
def worker(url, payloads, proxies=None, encode_times=0, timeout=10):
    ...
    for i, payload in enumerate(payloads):
        ...
        logging.info(
            f"{CYRILLIC}[info] Payload {i + 1}/{len(payloads)} tested for URL {url}.{RESET}"
        )
...
def test_xss(urls, payloads, proxies=None, encode_times=0, num_threads=5, timeout=10):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [
            executor.submit(worker, url, payloads, proxies, encode_times, timeout)
            for url in urls
        ]
...
def main():
    ...
    parser.add_argument(
        ...
        "--url",
        type=str,
        help="Specify a single URL to test for XSS vulnerabilities.",
    )
    parser.add_argument(
        "-f",
        "--file",
        type=str,
        help="Specify a file containing a list of URLs to test.",
    )
    parser.add_argument(
        "-t",
        "--threads",
        type=int,
        default=5,
        help="Specify the number of threads to use (default: 5).",
    )
    parser.add_argument(
        "-e",
        "--encoding",
        type=int,
        default=0,
        help="Specify the number of times to encode payloads (default: 0).",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="result.txt",
        help="Specify a custom file name for output results (default: result.txt).",
    )
    parser.add_argument(
        "-T",
        "--time-sec",
        type=int,
        default=10,
        help="Specify connection timeout in seconds (default: 10).",
    )
    ...
    if not args.url and not args.file:
        logging.error(
            f"{RED}You must specify either a single URL with -u or a file with URLs using -f.{RESET}"
        )
        sys.exit(1)
    ...
    print_banner()
    logging.info(
        f"{CYRILLIC}[info] Loaded {len(urls)} URLs and {len(payloads)} payloads.{RESET}"
    )

    port = find_free_port()

    # Start the server in a separate thread
    server_thread = concurrent.futures.ThreadPoolExecutor(max_workers=1).submit(
        start_server, port
    )
