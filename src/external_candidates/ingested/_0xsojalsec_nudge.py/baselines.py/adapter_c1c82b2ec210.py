# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nudge\baselines\adapter.py
...
class AdapterFineTuner:
    ...
    def get_train_val_sets(self, train_set, val_set, embeddings):
        ...
        for i in range(len(training_q_embs)):
            for j in range(len(train_anss[i])):
                ...
                plain_text_train.append(
                    (
                        torch.Tensor(training_q_embs[i]).reshape(emb_size),
                        torch.Tensor(embeddings[emb_indx]).reshape(emb_size),
                    )
                )
    ...
    def finetune(
        ...
        plain_text_train, val_q_embs, val_labels = self.get_train_val_sets(
            train_set, val_set, embeddings
        )
        ...
        train_model(
            ...
            val_q_embs=val_q_embs,
            val_labels=val_labels,
            nontrain_embeddings_or_shard=nontrain_embeddings_or_shard,
            data_embs=torch_embeddings,
            val_freq=val_freq,
            loss_scale=loss_scale,
            loss_sim_thresh=loss_sim_thresh,
            warmup_steps=warmup_steps,
            lr=lr,
            val_batch_size=batch_size,
            step_wait=step_wait,
            acc_diff_wait=acc_diff_wait,
            val_k=val_k,
        )
