...
class MultipleNegativesRankingLoss(nn.Module):
    ...
    def forward(self, query_embeds, context_embeds):
        ...
        scores = torch.matmul(
            normalize(query_embeds), torch.t(normalize(context_embeds))
        )
...
def get_val_acc(
    q_embs, data_embs, nontrain_embeddings_or_shard, labels, val_batch_size, k
):
    sim_preds = []
...
def train_model(
    ...
    model.to(device)
    loss_model = MultipleNegativesRankingLoss(
        model=model, scale=loss_scale, sim_thresh=loss_sim_thresh
    )
    ...
    if (
        nontrain_embeddings_or_shard[0] is not None
        and nontrain_embeddings_or_shard[0].shape[0] > 0
    ) or nontrain_embeddings_or_shard[1] is not None:
        val_labels = torch.cat(
            [val_labels, torch.zeros_like(val_labels[:, :val_k])], dim=1
        )
