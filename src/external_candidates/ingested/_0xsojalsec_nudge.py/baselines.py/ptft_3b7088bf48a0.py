# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nudge\baselines\ptft.py
import copy
import gc
import math
import time
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for name, qs, ans in self.val_sets:
            maxs[name] = []
            indxs[name] = []
            tokenized_qs = self.tokenizer(
                qs, padding=True, truncation=True, return_tensors="pt"
            ).to(self.device)
            with torch.no_grad():
                new_q_embs = []
                for i in range(
                    int(math.ceil(len(tokenized_qs["input_ids"]) / self.batch_size))
                ):
                    curr_batch = {}
                    for x in tokenized_qs:
                        curr_batch[x] = tokenized_qs[x][
        ...
        for i in tqdm(range(int(math.ceil(len(self.corpus) / self.batch_size)))):
            tokenized_docs = self.tokenizer(
                self.corpus[i * self.batch_size : (i + 1) * self.batch_size],
                padding=True,
                truncation=True,
                return_tensors="pt",
            ).to(self.device)
            ...
            with torch.no_grad():
                new_doc_embs = normalize(
                    model(tokenized_docs)["sentence_embedding"].detach()
                )

            for name, _, _ in self.val_sets:
                res = torch.matmul(all_new_q_embs[name], torch.t(new_doc_embs)).max(
                    dim=-1, keepdim=True
                )
                maxx = res.values
...
class PTFT:
    ...
    def train_model(
        ...
        for i in pbar:
            ...
            query, doc = data
            tokenized_qs = tokenizer(
                query, padding=True, truncation=True, return_tensors="pt"
            ).to(self.device)
            q_embs = normalize(
                model(tokenized_qs.to(self.device))["sentence_embedding"]
            )
            ...
            pbar.set_description(
                f"loss:{loss.item()/len(query)} {val['eval_recall@1']}"
            )
    ...
    def finetune_model(
        ...
        if dataset_nontrain is not None:
            if nontrain_indx_to_use is not None:
                corpus.extend(
                    list(
                        dataset_nontrain.iloc[nontrain_indx_to_use - len(corpus)][
                            "text"
                        ].values
                    )
                )
        ...
        train_dataloader = DataLoader(
            plain_text_train, shuffle=True, batch_size=batch_size
        )
