...
def embed(q_set, data_path, prefix, emb_model, coco_path, batch_size):
    ...
    np.save(
        f"{data_path}/qs_emb_{emb_model}_{q_set}_{split_type}_{split}.npy", txt_embs
    )
...
def create_splits(q_set, data_path, emb_model):
    ...
    dataset = pd.read_parquet(f"{data_path}/dataset_{dataset_name}.parquet").set_index(
        "doc_id"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "data_path", type=str, help="Directory for to store processed data files"
    )
    parser.add_argument(
        "coco_path", type=str, help="Directory for unprocessed coco files"
    )
