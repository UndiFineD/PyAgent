# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nudge\util\utils.py
import gc
import math
import os
import pickle
...
def compute_topk_sims_given_shard_path(q_embs, shard_path, k, dist):
    ...
    while os.path.isfile(f"{shard_path}{shard_i}.npy"):
        ...
        for i in tqdm(range(int(math.ceil(len(embeddings_nontrain) / batch_size)))):
            ...
            batch_begin_index += embeddings_nontrain[
                i * batch_size : (i + 1) * batch_size
            ].shape[0]
...
def compute_topk_sims(q_embs, nontrain_embeddings_or_shard, k, dist, with_index=False):
    ...
    if nontrain_embs is not None:
        if nontrain_embs.shape[0] == 0:
            return None
        res = compute_topk_sims_given_embs(
            q_embs, nontrain_embs, min(k, nontrain_embs.shape[0]), dist
        )
    else:
        res = compute_topk_sims_given_shard_path(
            q_embs, path_to_nontrain_emb_shards, k, dist
        )
...
def get_data(
    ...
    for split in ["train", "dev", "test"]:
        ...
        if split != "test":
            max_nontest_index = max(
                np.array(
                    [indx for curr_q_ans_indx in q_ans_indx for indx in curr_q_ans_indx]
                ).max()
                + 1,
                max_nontest_index,
            )
...
def calc_metrics(metrics, model_res, true_res, true_res_rel):
    ...
    for i, (met_type, k) in enumerate(metrics):
        if met_type == "recall":
            ...
            ideal_dcg = np.sum(
                [rel / np.log2(loc + 2) for loc, rel in enumerate(ordered_rels[:k])]
            )
            ...
            dcg = np.sum(
                [rel_scores[loc] / np.log2(loc + 2) for loc in range(len(rel_scores))]
            )
...
def calc_metrics_batch(metrics, top_k_preds, test_anss, test_anss_rel=None):
    ...
    for i in range(len(top_k_preds)):
        ...
        met_res = calc_metrics(
            metrics, top_k_preds[i], test_anss[i], curr_test_anss_rel
        )
        all_met_res += met_res
...
def embed_data_and_query_sets(dataset, query_sets, emb_model):
    ...
    data_emb = model.encode(
        list(dataset["text"].values), show_progress_bar=True
    ).astype(np.float32)
    for split in ["train", "dev", "test"]:
        print(f"embedding qs {split}")
        query_sets[split]["q_embs"] = model.encode(
            list(query_sets[split]["q_df"]["input"].values), show_progress_bar=True
        ).astype(np.float32)
