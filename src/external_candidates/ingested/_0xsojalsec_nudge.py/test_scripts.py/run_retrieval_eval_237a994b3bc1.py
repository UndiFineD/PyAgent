...
def run_test(
    ...
    dataset, nontrain_dataset, embeddings, nontrain_embeddings, query_sets = get_data(
        **data_config
    )
    ...
    if finetuner == "NUDGEN":
        ...
        if val_with_topk_only > 0:
            ...
            top_k_indxs_val = ret.retrieve_topk_from_emb_batch(
                val_with_topk_only, query_sets["dev"]["q_embs"]
            )
            indx_nontrain = np.unique(
                top_k_indxs_val[top_k_indxs_val >= len(embeddings)]
            )
            del ret
        else:
            indx_nontrain = None
    ...
    for i in range(len(top_k_indxs)):
        ...
        met_res = calc_metrics(
            metrics, top_k_indxs[i], test_anss[i], curr_test_anss_rel
        )
        all_met_res += met_res
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "retriever", type=str, help="retriever and finetuning method to use"
    )
    ...
    parser.add_argument(
        "data_config_path", type=str, help="path to json file with data config"
    )
