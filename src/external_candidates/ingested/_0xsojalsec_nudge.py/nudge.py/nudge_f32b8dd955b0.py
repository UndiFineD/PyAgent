# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nudge\nudge\nudge.py
import math
        ...
        if dist == "cos":
            ...
            curr_topk = torch.topk(
                torch.matmul(
                    ...
                    torch.t(
                        torch.from_numpy(
                            embeddings_nontrain[i * batch_size : (i + 1) * batch_size]
                        ).to(q_embs.device)
                    ),
                ),
                k=k,
...
class NUDGEM:
    ...
    def finetune_embeddings(
        ...
        if nontrain_embeddings is not None:
            nontrain_sims = compute_topk_sims(
                qs_val, nontrain_embeddings, 1, "cos", val_batch_size
            )
        ...
        for j in range(embeddings.shape[0]):
            gs.append(
                torch.nan_to_num(
                    -1 * qs_train[train_doc_to_q[j]].sum(dim=0, keepdim=True)
                )
            )
        ...
        print("Finding gamma")
        if gamma is None:
            intrval_min_points, intrval_max_points = self._get_all_I_i(
                qs_val,
                val_batch_size,
                embeddings,
                gs,
            ...
            gamma_star = self._get_point_of_most_intersection(
                intrval_min_points, intrval_max_points
            )
        else:
            gamma_star = gamma
    ...
    def _get_I_i(self, curr_qs_val, data_embs, upd, curr_nontrain_sims, curr_val_anss):
        ...
        all_S = torch.repeat_interleave(
            S, torch.tensor(ans_lens).to(self.device), dim=0
        )
        all_G = torch.repeat_interleave(
            G, torch.tensor(ans_lens).to(self.device), dim=0
        )
        S_x = torch.gather(
            all_S, 1, torch.tensor(flattened_ans).to(self.device).reshape(-1, 1)
        )
        G_x = torch.gather(
            all_G, 1, torch.tensor(flattened_ans).to(self.device).reshape(-1, 1)
        )
        ...
        all_high_vals = torch.min(
            neg_mask * alphas + (~neg_mask) * LARGE_NUMBER, dim=1
        ).values
        ...
        all_low_vals = (
            torch.gather(all_low_vals.reshape(-1), 0, mask.reshape(-1)).cpu().numpy()
        )
        all_high_vals = (
            torch.gather(all_high_vals.reshape(-1), 0, mask.reshape(-1)).cpu().numpy()
        )
    ...
    def _get_all_I_i(self, qs_val, val_batch_size, embs, upd, nontrain_sims, val_anss):
        ...
        for val_batch_i in range(int(math.ceil(qs_val.shape[0] / val_batch_size))):
            if nontrain_sims is not None:
                curr_nontrain_sims = nontrain_sims[
                    val_batch_i * val_batch_size : (val_batch_i + 1) * val_batch_size
                ]
            else:
                curr_nontrain_sims = None
            curr_mins, curr_maxs = self._get_I_i(
                qs_val[
                    val_batch_i * val_batch_size : (val_batch_i + 1) * val_batch_size
                ],
                embs,
...
class NUDGEN:
    ...
    def finetune_embeddings(
        ...
        """
        qs_train, train_doc_to_q, qs_val, answer_val = self._get_train_val_sets(
            train_set, val_set, embeddings.shape[0]
        )

        nontrain_sims = None
        if nontrain_embeddings is not None:
            nontrain_sims = compute_topk_sims(
                qs_val, nontrain_embeddings, val_k, "cos", val_batch_size
            )
            if nontrain_sims is not None:
                nontrain_sims = nontrain_sims.reshape((-1, val_k))
                answer_val = torch.cat(
                    [answer_val, torch.zeros_like(answer_val[:, :val_k])], dim=1
                )
        ...
        zero_updates = torch.nonzero(
            torch.logical_or(cTc - vTc**2 == 0, vTc < 0)
        ).reshape(-1)
        ...
        for gamma in gamma_vals:
            ...
            preds = []
            for val_batch_i in range(
                int(math.ceil(embeddings.shape[0] / val_batch_size))
            ):
                preds.append(
                    torch.matmul(
                        qs_val,
    ...
    def _sol_maxm(self, gamma, embeddings, vTc, cTc, gs, zero_updates):
        if gamma != 0:
            ...
            Z = (gs - vTc.reshape(-1, 1) * embeddings) / torch.sqrt(
                cTc - vTc**2
            ).reshape(-1, 1)
            delta = gamma * (gs - mu * embeddings) / (2 * (mu - vTc.reshape(-1, 1)))
            delta[zero_updates] = 0
            new_embs = embeddings + delta
            mask = ((embeddings * normalize(gs)).sum(axis=-1) >= 1 - gamma / 2).reshape(
                -1, 1
            )
