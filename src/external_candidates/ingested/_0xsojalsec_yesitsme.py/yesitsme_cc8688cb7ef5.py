...
def main():
    ...
    parser.add_argument(
        "-t", "--timeout", help="Timeout between requests", required=False
    )
            ...
            if infos["user"] == None:
                ...
                if "public_email" in infos.keys():
                    if infos["public_email"] != "":
                        if (
                            ...
                            and infos["public_email"].split("@")[0][-1]
                            == email.split("@")[0][-1]
                            and infos["public_email"].split("@")[1]
                            == email.split("@")[1]
                        ):
                            print(
                                Fore.GREEN
                ...
                if "public_phone_number" in infos.keys():
                    if str(infos["public_phone_number"]) != "":
                        if (
                            phone != " "
                            and str(infos["public_phone_number"]).split()[0]
                            == phone.split()[0]
                            ...
                            print(
                                "Public phone    : " + str(infos["public_phone_number"])
                            )
                ...
                if other_infos["error"] == "rate limit":
                    ...
                    if other_infos["user"]["message"] == "No users found":
                        ...
                        sys.exit(
                            Fore.RED
                            + "Rate limit! Please, wait a few minutes before running the script again."
                        )
                else:
                    if "obfuscated_email" in other_infos["user"].keys():
                        if other_infos["user"]["obfuscated_email"] != "":
                            if (
                                email != " "
                                and other_infos["user"]["obfuscated_email"][0]
                                == email[0]
                                and other_infos["user"]["obfuscated_email"][8]
                                == email.split("@")[0][-1]
                                and other_infos["user"]["obfuscated_email"].split("@")[
                                    1
                                ]
                                == email.split("@")[1]
                            ):
                    ...
                    if "obfuscated_phone" in other_infos["user"].keys():
                        if str(other_infos["user"]["obfuscated_phone"]) != "":
                            if (
                                phone != " "
                                and str(
                                    other_infos["user"]["obfuscated_phone"].split()[0]
                                )
                                == phone.split()[0]
                                and str(other_infos["user"]["obfuscated_phone"])[-2:]
                                == phone[-2:]
                            ):
                                print(
                                    Fore.GREEN
