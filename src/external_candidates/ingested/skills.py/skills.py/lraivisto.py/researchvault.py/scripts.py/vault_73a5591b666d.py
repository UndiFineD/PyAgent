# Extracted from: C:\DEV\PyAgent\.external\skills\skills\lraivisto\researchvault\scripts\vault.py
import argparse
import json
import os
import sys
...
if __name__ == "__main__":
    ...
    update_parser.add_argument(
        "--status", choices=["active", "paused", "completed", "failed"]
    )
    ...
    log_parser.add_argument(
        "--conf", type=float, default=1.0, help="Confidence score (0.0-1.0)"
    )
    log_parser.add_argument(
        "--source", default="unknown", help="Source of the event (e.g. agent name)"
    )
    log_parser.add_argument(
        "--tags", default="", help="Comma-separated tags for the event"
    )
    ...
    insight_parser.add_argument(
        ...
        "-i",
        action="store_true",
        help="Interactive session to add multiple insights",
    )
    ...
    if args.command == "init":
        ...
        if not projects:
            ...
            for p in projects:
                ...
                status_style = (
                    "green"
                    if p[3] == "active"
                    else "red" if p[3] == "failed" else "blue"
                )
                table.add_row(
                    p[0],
                    str(p[5]),
        ...
        try:
            ...
            if result.success:
                console.print(
                    f"[green]✔ Ingested:[/green] {result.metadata['title']} ({result.metadata['source']})"
                )
            else:
        ...
        if args.set_result:
            ...
            try:
                ...
                console.print(
                    f"[green]✔ Cached provided result for:[/green] {args.query}"
                )
            ...
            if cached:
                console.print(
                    f"[dim]Note: Serving cached result for '{args.query}'[/dim]"
                )
        ...
        console.print(
            f"[green]✔ Logged[/green] [bold cyan]{args.type}[/] for [bold white]{args.id}[/] (conf: {args.conf}, src: {args.source})"
        )
        ...
        if not status:
            ...
            for e in status["recent_events"]:
                ...
                conf_color = (
                    "green" if e[3] > 0.8 else "yellow" if e[3] > 0.5 else "red"
                )
            ...
            console.print(
                Panel(
                    content, title=f"Research Vault Status: {p[1]}", border_style="blue"
                )
            )
        ...
        if args.interactive:
            console.print(
                Panel(
                    f"Interactive Insight Mode for [bold cyan]{args.id}[/bold cyan]\nType [bold red]exit[/] to finish.",
                    border_style="green",
                )
            )
            while True:
                ...
                conf_str = console.input(
                    "[bold yellow]Confidence (0.0-1.0)[/]: "
                ).strip()
                try:
            ...
            if not insights:
                print(
                    "No insights found"
                    + (f" with tag '{args.filter_tag}'" if args.filter_tag else "")
                )
            for i in insights:
                evidence = {}
                try:
