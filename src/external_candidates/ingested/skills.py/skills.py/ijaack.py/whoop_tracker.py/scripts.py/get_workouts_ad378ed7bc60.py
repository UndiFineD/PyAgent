# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ijaack\whoop-tracker\scripts\get_workouts.py
#!/usr/bin/env python3
"""
...
def format_workout(record):
    ...
    total_ms = sum(
        zones.get(f"zone_{z}_milli", 0)
        for z in ["zero", "one", "two", "three", "four", "five"]
    )
    ...
    return {
        ...
        "calories_kcal": (
            round(score.get("kilojoule", 0) * 0.239006)
            if score.get("kilojoule")
            else None
        ),
        "distance_km": (
            round(score.get("distance_meter", 0) / 1000, 2)
            if score.get("distance_meter")
            else None
        ),
        "altitude_gain_m": (
            round(score.get("altitude_gain_meter", 0))
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=25, help="Max records (default 25, max 25)"
    )
    ...
    if args.sport:
        records = [
            r for r in records if r.get("sport_name", "").lower() == args.sport.lower()
        ]
