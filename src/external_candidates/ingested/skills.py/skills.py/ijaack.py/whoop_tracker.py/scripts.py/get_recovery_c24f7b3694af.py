# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ijaack\whoop-tracker\scripts\get_recovery.py
#!/usr/bin/env python3
"""
...
def format_recovery(record):
    ...
    return {
        ...
        "spo2": (
            round(score.get("spo2_percentage", 0), 1)
            if score.get("spo2_percentage")
            else None
        ),
        "skin_temp": (
            round(score.get("skin_temp_celsius", 0), 1)
...
def main():
    ...
    try:
        client = WhoopClient()
        response = client.get_recovery_collection(
