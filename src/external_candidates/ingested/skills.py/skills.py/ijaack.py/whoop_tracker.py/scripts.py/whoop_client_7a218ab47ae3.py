# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ijaack\whoop-tracker\scripts\whoop_client.py
#!/usr/bin/env python3
"""
WHOOP API Client
...
class WhoopClient:
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        if response.status_code == 429:
            ...
            print(
                f"Rate limited. Retrying in {retry_after} seconds...", file=sys.stderr
            )
    ...
    def iter_recovery(
        ...
        next_token = None
        while True:
            resp = self.get_recovery_collection(
                start, end, limit=25, next_token=next_token
            )
    ...
    def iter_sleep(
        ...
        next_token = None
        while True:
            resp = self.get_sleep_collection(
                start, end, limit=25, next_token=next_token
            )
            for record in resp.get("records", []):
                yield record
    ...
    def iter_cycles(
        ...
        next_token = None
        while True:
            resp = self.get_cycle_collection(
                start, end, limit=25, next_token=next_token
            )
            for record in resp.get("records", []):
                yield record
    ...
    def iter_workouts(
        ...
        next_token = None
        while True:
            resp = self.get_workout_collection(
                start, end, limit=25, next_token=next_token
            )
            for record in resp.get("records", []):
                yield record
