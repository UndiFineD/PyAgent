# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alphafactor\tushare\scripts\tushare.py
#!/usr/bin/env python3
"""
Tushare 金融数据接口
...
def print_daily(data: List[Dict]):
    ...
    print(f"\n📊 日线行情 ({len(data)} 条):\n")
    print(
        f"{'日期':<12} {'开盘':<10} {'收盘':<10} {'最高':<10} {'最低':<10} {'成交量':<12}"
    )
    print("-" * 70)
    ...
    for item in data[:20]:
        ...
        print(
            f"{trade_date:<12} {open_price:<10.2f} {close:<10.2f} {high:<10.2f} {low:<10.2f} {vol:<12}"
        )
...
def print_fut_daily(data: List[Dict]):
    ...
    print(f"\n📊 期货日线 ({len(data)} 条):\n")
    print(
        f"{'日期':<12} {'开盘':<10} {'收盘':<10} {'最高':<10} {'最低':<10} {'持仓':<10}"
    )
    print("-" * 70)
    ...
    for item in data[:20]:
        ...
        print(
            f"{trade_date:<12} {open_price:<10.2f} {close:<10.2f} {high:<10.2f} {low:<10.2f} {oi:<10.0f}"
        )
...
def print_moneyflow(data: List[Dict]):
    ...
    for item in data[:20]:
        ...
        print(
            f"{ts_code:<12} {trade_date:<12} {emoji} {net_mf:<10.1f}万 {net_mf_amount:<10.1f}万"
        )
...
def main():
    ...
    if args.command == "stock_basic":
        ...
        for item in data[:20]:
            print(
                f"{item.get('ts_code')}: {item.get('chairman', '-')} - {item.get('main_business', '-')[:30]}..."
            )
        ...
        for item in data[:20]:
            print(
                f"{item.get('trade_date')} {item.get('symbol')} {item.get('broker')}: {item.get('vol')} 手"
            )
        ...
        for item in data[:10]:
            print(
                f"{item.get('year')}年{item.get('quarter')}季度: GDP {item.get('gdp')}亿元, 增速 {item.get('gdp_yoy')}%"
            )
        ...
        for item in data[:10]:
            print(
                f"{item.get('month')}: 全国 {item.get('nt_val')}%, 城市 {item.get('town_val')}%, 农村 {item.get('cnt_val')}%"
            )
        ...
        for item in data[:10]:
            print(
                f"{item.get('month')}: PPI {item.get('ppi')}%, 环比 {item.get('ppi_mp')}%"
            )
