...
def print_projects(projects: List[Dict], show_details: bool = False):
    ...
    for i, project in enumerate(projects, 1):
        ...
        print(
            f"   📝 {info['description'][:100]}{'...' if len(info['description']) > 100 else ''}"
        )
        print(f"   🔗 {info['url']}")
        print(
            f"   📊 Stars: {stars:,} | Forks: {info['forks']:,} | Language: {info['language']}"
        )
...
示例:
  ...
  %(prog)s --topic rust --stars 5000 --limit 50
    ...
    parser.add_argument(
        "--stars", "-s", type=int, default=100, help="最少 Star 数量（默认: 100）"
    )
    parser.add_argument(
        "--days",
        "-d",
    ...
    # 输出选项
    parser.add_argument(
        "--limit", type=int, default=30, help="返回结果数量（默认: 30，最大: 100）"
    )
    parser.add_argument(
        "--sort",
