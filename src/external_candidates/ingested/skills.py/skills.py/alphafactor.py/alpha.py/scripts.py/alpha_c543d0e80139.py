# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alphafactor\alpha\scripts\alpha.py
#!/usr/bin/env python3
"""
...
try:
    ...
    print(
        "⚠️  websocket-client 库未安装，请先运行: pip3 install websocket-client --user"
    )
...
def save_alert_history():
    ...
    try:
        with open(ALERT_HISTORY_FILE, "w", encoding="utf-8") as f:
            json.dump(
                alert_history[-100:], f, indent=2, ensure_ascii=False
            )  # 只保留最近100条
    except Exception as e:
...
def check_symbol_has_price(symbol: str) -> bool:
    ...
    try:
        ...
        req = urllib.request.Request(
            url, headers={"User-Agent": "Alpha-New-Coin-Monitor/1.0"}
        )
...
def alert_new_coin(symbol: str, ticker_data: Dict):
    ...
    if info:
        ...
        print(
            f"📈 24h涨跌: {info.get('change', 'N/A')} ({info.get('change_percent', 'N/A')}%)"
        )
...
示例:
  ...
  %(prog)s reset
    ...
    history_parser.add_argument(
        "--limit", "-l", type=int, default=20, help="显示条数（默认: 20）"
    )
