...
def get_exchange(exchange_id: str = "binance"):
    ...
    try:
        ...
        print(
            f"   支持的交易所: binance, okx, bybit, gateio, kucoin, huobi, coinbase..."
        )
        return None
...
def get_ohlcv(
    exchange, symbol: str, timeframe: str = "1h", limit: int = 24
) -> List[List]:
    """获取K线数据"""
    try:
...
def print_ohlcv(ohlcv: List[List], symbol: str):
    ...
    print(
        f"{'时间':<20} {'开盘':<12} {'最高':<12} {'最低':<12} {'收盘':<12} {'成交量':<12}"
    )
    print("-" * 90)
    ...
    for candle in ohlcv[-10:]:  # 只显示最近10条
        ...
        print(
            f"{dt:<20} {open_p:<12.4f} {high:<12.4f} {low:<12.4f} {close:<12.4f} {volume:<12.4f} {change_emoji}"
        )


def add_alert(
    symbol: str, condition: str, threshold: float, exchange_id: str = "binance"
) -> bool:
    """添加价格预警"""
    alerts = load_alerts()
...
def list_alerts():
    ...
    for alert_id, alert in alerts.items():
        ...
        print(
            f"{alert_id:<25} {symbol:<15} {exchange:<12} {condition_text:<25} {triggered:<8}"
        )
    ...
    for alert_id, alert in alerts.items():
        ...
        if condition == "above" and current_price > threshold:
            ...
            if change_pct >= threshold:
                triggered = True
                message = (
                    f"🚀 {symbol} 涨幅达到 {change_pct:.2f}%，当前价格: {current_price}"
                )
            ...
            if change_pct >= threshold:
                triggered = True
                message = (
                    f"📉 {symbol} 跌幅达到 {change_pct:.2f}%，当前价格: {current_price}"
                )
    ...
    if args.command == "ticker":
        ...
        if orderbook:
            ...
            for i in range(min(5, len(orderbook["bids"]), len(orderbook["asks"]))):
                ...
                print(
                    f"{bid_price:,.4f} x {bid_vol:.4f}    |    {ask_price:,.4f} x {ask_vol:.4f}"
                )
