    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            if "access_token" in data:
                ...
                print(
                    f"❌ 获取 Access Token 失败: {data.get('errmsg', 'Unknown error')}"
                )
                return None
...
def get_drafts(access_token: str, offset: int = 0, count: int = 20) -> List[Dict]:
    ...
    try:
        data = json.dumps({"offset": offset, "count": count, "no_content": 0}).encode(
            "utf-8"
        )
...
def print_drafts(drafts: List[Dict]):
    ...
    for i, draft in enumerate(drafts, 1):
        ...
        if update_time:
            update_time = datetime.fromtimestamp(int(update_time)).strftime(
                "%Y-%m-%d %H:%M"
            )
...
def print_result(result: Dict):
    ...
    if result.get("success"):
        ...
        print(
            f"📋 摘要: {result['digest'][:60]}{'...' if len(result['digest']) > 60 else ''}"
        )
