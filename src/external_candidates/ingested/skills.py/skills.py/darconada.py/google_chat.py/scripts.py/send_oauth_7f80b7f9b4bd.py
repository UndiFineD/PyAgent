# Extracted from: C:\DEV\PyAgent\.external\skills\skills\darconada\google-chat\scripts\send_oauth.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    parser.add_argument(
        "--credentials", required=True, help="Path to OAuth credentials JSON"
    )
    parser.add_argument(
        "--token", required=True, help="Path to token file (will be created if missing)"
    )
    ...
    group.add_argument(
        "--list-spaces", action="store_true", help="List all available spaces"
    )

    parser.add_argument("message", nargs="?", help="Message to send")
    parser.add_argument(
        "--no-emoji", action="store_true", help="Skip robot emoji prefix"
    )
