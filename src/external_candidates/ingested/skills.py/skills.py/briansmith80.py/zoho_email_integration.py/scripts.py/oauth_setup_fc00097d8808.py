# Extracted from: C:\DEV\PyAgent\.external\skills\skills\briansmith80\zoho-email-integration\scripts\oauth-setup.py
#!/usr/bin/env python3
"""
...
def setup_oauth2():
    ...
    try:
        ...
        print(
            f"   python3 scripts/zoho-email.py oauth-status --token-file {token_path}"
        )
        print("3. Use OAuth2 in your scripts:")
        print(
            f"   python3 scripts/zoho-email.py unread --auth oauth2 --token-file {token_path}"
        )
        print()
...
def check_token_status():
    ...
    try:
        ...
        if is_expired:
            print(
                "\n⚠️  Token has expired. Run 'python3 oauth-setup.py refresh' to refresh.",
                file=sys.stderr,
            )
            sys.exit(1)
