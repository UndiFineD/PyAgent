# Extracted from: C:\DEV\PyAgent\.external\skills\skills\briansmith80\zoho-email-integration\scripts\zoho-email.py
#!/usr/bin/env python3
"""
...
class ZohoRestAPIClient:
    ...
    def __init__(self, token_file=DEFAULT_TOKEN_PATH, verbose=False):
        ...
        try:
            ...
            self.session.headers.update({"User-Agent": "ZohoEmailClient/2.0"})
        except ImportError:
            raise Exception(
                "requests library required for REST API mode. Install: pip install requests"
            )
    ...
    def list_messages(self, folder="Inbox", limit=100, search_query=None, status=None):
        ...
        if search_query:
            params = {"searchKey": search_query, "limit": limit, "start": 1}

            self.log(f"Searching messages with query: {search_query} (limit={limit})")
            response = self._make_request(
                "GET", f"/accounts/{account_id}/messages/search", params=params
            )
        else:
            ...
            self.log(
                f"Listing messages in {folder} (limit={limit}, status={status or 'all'})"
            )
            response = self._make_request(
                "GET", f"/accounts/{account_id}/messages/view", params=params
            )
    ...
    def get_message(self, message_id):
        ...
        response = self._make_request(
            "GET", f"/accounts/{account_id}/messages/{message_id}"
        )
    ...
    def send_message(self, to, subject, body, html_body=None, cc=None, bcc=None):
        ...
        response = self._make_request(
            "POST", f"/accounts/{account_id}/messages", json=payload
        )
    ...
    def mark_as_read(self, message_ids):
        ...
        message_id_list = [
            int(mid) if isinstance(mid, str) else mid for mid in message_ids
        ]
        ...
        try:
            ...
            response = self._make_request(
                ...
                json={"mode": "markAsRead", "messageId": message_id_list},
            )
    ...
    def mark_as_unread(self, message_ids):
        ...
        try:
            ...
            response = self._make_request(
                "PUT",
                f"/accounts/{account_id}/updatemessage",
                json={"mode": "markAsUnread", "messageId": message_id_list},
            )
    ...
    def move_messages(self, message_ids, folder_name):
        ...
        try:
            self.log(
                f"Moving {len(message_id_list)} message(s) to folder {folder_name}"
            )
            response = self._make_request(
                "PUT",
                f"/accounts/{account_id}/updatemessage",
    ...
    def list_folders(self):
        ...
        response = self._make_request("GET", f"/accounts/{account_id}/folders")
...
class ZohoEmail:
    def __init__(
        self, verbose=False, auth_method="auto", token_file=None, api_mode="auto"
    ):
        """
        ...
        if api_mode == "auto":
            ...
            if self.auth_method == "oauth2" and has_requests_library():
                self.api_mode = "rest"
                try:
                    self.rest_client = ZohoRestAPIClient(
                        self.token_file, verbose=verbose
                    )
                    self.log("Using REST API mode")
                except Exception as e:
                    self.log(
                        f"REST API initialization failed: {e}, falling back to IMAP"
                    )
                    self.api_mode = "imap"
            else:
                self.api_mode = "imap"
            ...
            if not has_requests_library():
                raise ValueError(
                    "REST API mode requires 'requests' library. Install: pip install requests"
                )
            self.api_mode = "rest"
            ...
            raise ValueError(
                f"Unknown api_mode: {api_mode}. Use 'auto', 'rest', or 'imap'"
            )
    ...
    def _setup_password(self):
        ...
        if not EMAIL or not PASSWORD:
            raise ValueError(
                "ZOHO_EMAIL and ZOHO_PASSWORD environment variables must be set"
            )
    ...
    def _setup_oauth2(self):
        ...
        try:
            ...
            if not self.email:
                raise ValueError(
                    "Email address not found in token file or ZOHO_EMAIL env var"
                )
    ...
    def revoke_token(self):
        ...
        if self.auth_method != "oauth2":
            raise ValueError(
                "Token revocation only available with OAuth2 authentication"
            )
    ...
    def get_token_status(self):
        ...
        if self.auth_method != "oauth2":
            return {"auth_method": self.auth_method, "status": "not_using_oauth2"}
        ...
        try:
            ...
            if self.auth_method == "oauth2":
                ...
                auth_string = generate_oauth2_string(
                    self.email, self.oauth_tokens["access_token"]
                )
    ...
    def search_emails(self, folder="INBOX", query="ALL", limit=10, search_days=None):
        ...
        if self.api_mode == "rest":
            try:
                ...
                messages = self.rest_client.list_messages(
                    folder=folder, limit=limit, search_query=search_query
                )
                    ...
                    results.append(
                        {
                            ...
                            "body": (
                                (msg.get("summary", "") or "")[:500] + "..."
                                if len(msg.get("summary", "") or "") > 500
                                else (msg.get("summary", "") or "")
                            ),
                        }
                    )
        ...
        try:
            ...
            if search_days > 0 and query != "ALL":
                since_date = (datetime.now() - timedelta(days=search_days)).strftime(
                    "%d-%b-%Y"
                )
    ...
    def download_attachment(
        self, folder="INBOX", email_id=None, attachment_index=0, output_path=None
    ):
    ...
    def send_email_with_attachment(
        self, to, subject, body, attachments=None, cc=None, bcc=None, html_body=None
    ):
        ...
        try:
            ...
            self.log(
                f"Sending email with {len(attachments or [])} attachment(s) to {to}..."
            )
            socket.setdefaulttimeout(IMAP_TIMEOUT)
    ...
    def send_email(self, to, subject, body, cc=None, bcc=None, html_body=None):
        ...
        if self.api_mode == "rest":
            try:
                return self.rest_client.send_message(
    ...
    def send_html_email(
        self, to, subject, html_body, text_body=None, cc=None, bcc=None
    ):
        ...
        return self.send_email(
            to, subject, text_body, cc=cc, bcc=bcc, html_body=html_body
        )
    ...
    def get_unread_count(self, folder="INBOX"):
        ...
        if self.api_mode == "rest":
            try:
                ...
                messages = self.rest_client.list_messages(
                    folder=folder, limit=1000, status="unread"
                )
    ...
    def mark_as_read(self, email_ids, folder="INBOX"):
        ...
        try:
            ...
            for email_id in email_ids:
                try:
                    status, _ = self.imap.store(
                        str(email_id).encode(), "+FLAGS", "\\Seen"
                    )
                    if status == "OK":
    ...
    def delete_emails(self, email_ids, folder="INBOX"):
        ...
        try:
            ...
            for email_id in email_ids:
                try:
                    status, _ = self.imap.store(
                        str(email_id).encode(), "+FLAGS", "\\Deleted"
                    )
                    if status == "OK":
    ...
    def bulk_action(
        self, query, action, folder="INBOX", limit=100, search_days=None, dry_run=False
    ):
        ...
        if self.api_mode == "rest":
            try:
                ...
                messages = self.rest_client.list_messages(
                    folder=folder, limit=limit, search_query=rest_query
                )
                ...
                # Extract message IDs
                email_ids = [
                    msg.get("messageId") for msg in messages if msg.get("messageId")
                ]
                ...
                if action == "mark-read":
                    ...
                    raise ValueError(
                        f"Unknown action: {action}. Use 'mark-read', 'mark-unread', or 'delete'"
                    )
                ...
                for email_id in email_ids[:10]:  # Show first 10 as preview
                    try:
                        status, msg_data = self.imap.fetch(
                            email_id, "(BODY[HEADER.FIELDS (SUBJECT FROM DATE)])"
                        )
...
if __name__ == "__main__":
    ...
    if "--api-mode" in sys.argv:
        ...
        if idx + 1 < len(sys.argv):
            ...
            if api_mode not in ("auto", "rest", "imap"):
                print(
                    f"Error: Invalid --api-mode value: {api_mode}. Use 'auto', 'rest', or 'imap'",
                    file=sys.stderr,
                )
                sys.exit(1)
            sys.argv = sys.argv[:idx] + sys.argv[idx + 2 :]
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "  Send:            python3 zoho-email.py send <to> <subject> <body> [--attach file1] [--attach file2]"
        )
        print(
            "  Send HTML:       python3 zoho-email.py send-html <to> <subject> <html_file_or_text>"
        )
        print(
            "  Preview HTML:    python3 zoho-email.py preview-html <html_file_or_text>"
        )
        print("\nAttachments:")
        print(
            "  List attachments: python3 zoho-email.py list-attachments <folder> <email_id>"
        )
        print(
            "  Download:        python3 zoho-email.py download-attachment <folder> <email_id> <index> [output_path]"
        )
        print("\nBatch Operations:")
        print(
            "  Mark as read:    python3 zoho-email.py mark-read <folder> <id1> <id2> ..."
        )
        print(
            "  Mark as unread:  python3 zoho-email.py mark-unread <folder> <id1> <id2> ..."
        )
        print(
            "  Delete emails:   python3 zoho-email.py delete <folder> <id1> <id2> ..."
        )
        print(
            "  Move emails:     python3 zoho-email.py move <source_folder> <target_folder> <id1> <id2> ..."
        )
        print(
            "  Empty Spam:      python3 zoho-email.py empty-spam [--dry-run] [--execute]"
        )
        print(
            "  Empty Trash:     python3 zoho-email.py empty-trash [--dry-run] [--execute]"
        )
        print(
            "  Bulk action:     python3 zoho-email.py bulk-action --folder INBOX --search 'SUBJECT \"spam\"' --action mark-read [--dry-run]"
        )
        print("\nOAuth2 Commands:")
        print("  Setup OAuth2:    python3 scripts/oauth-setup.py")
        print(
            "  Refresh tokens:  python3 zoho-email.py oauth-login [--token-file path]"
        )
        print(
            "  Check status:    python3 zoho-email.py oauth-status [--token-file path]"
        )
        print(
            "  Revoke tokens:   python3 zoho-email.py oauth-revoke [--token-file path]"
        )
        print("\nAuthentication:")
        print(
            "  --auth <method>      Authentication method: 'auto' (default), 'password', or 'oauth2'"
        )
        print(
            "  --token-file <path>  OAuth2 token file path (default: ~/.clawdbot/zoho-mail-tokens.json)"
        )
        print("\nAPI Mode:")
        print("  --api-mode <mode>    API mode: 'auto' (default), 'rest', or 'imap'")
        print(
            "                       'auto': Use REST API if available, fallback to IMAP"
        )
        ...
        print(
            "  --dry-run        Preview bulk action without executing (bulk-action only)"
        )
        print("\nEnvironment:")
        print(
            "  ZOHO_EMAIL       Your Zoho email address (required for all auth methods)"
        )
    ...
    if command == "doctor":
        ...
        report = {
            ...
            "token_file_exists": os.path.exists(
                os.path.expanduser(effective_token_file)
            ),
        ...
        report["rest_base_url"] = os.environ.get(
            "ZOHO_API_BASE_URL", "https://mail.zoho.com/api"
        )
        report["rest_reachable"] = http_ok(
            report["rest_base_url"].rstrip("/") + "/accounts"
        ) or http_ok("https://mail.zoho.com/")

        # Human-friendly output
        print("Zoho Email doctor\n")
        print(f"- ZOHO_EMAIL set: {yn(report['zoho_email_env_set'])}")
        print(
            f"- ZOHO_PASSWORD set (app password): {yn(report['zoho_password_env_set'])}"
        )
        print(
            f"- OAuth token file: {report['token_file']} (exists: {yn(report['token_file_exists'])})"
        )
        if report.get("token_file_exists"):
            if report.get("token_file_readable") is True:
                print(
                    f"  - refresh_token present: {yn(report.get('oauth_has_refresh_token', False))}"
                )
                print(
                    f"  - access_token present: {yn(report.get('oauth_access_token_present', False))}"
                )
                if report.get("oauth_token_expired_or_expiring") is not None:
                    print(
                        f"  - token expired/expiring soon: {yn(bool(report.get('oauth_token_expired_or_expiring')))}"
                    )
            else:
                print(
                    f"  - token file unreadable: yes ({report.get('token_file_error')})"
                )
        print(
            f"- requests installed (needed for REST mode): {yn(report['requests_installed'])}"
        )
        print(
            f"- REST reachable ({report['rest_base_url']}): {yn(report['rest_reachable'])}"
        )
        print(
            f"- IMAP reachable ({IMAP_SERVER}:{IMAP_PORT}): {yn(report['imap_reachable'])}"
        )
        print(
            f"- SMTP reachable ({SMTP_SERVER}:{SMTP_PORT}): {yn(report['smtp_reachable'])}"
        )
        ...
        if (
            report["token_file_exists"]
            and report.get("token_file_readable")
            and report.get("oauth_has_refresh_token")
        ):
            print(
                "- OAuth looks configured. Try: python3 scripts/zoho-email.py unread --api-mode rest"
            )
        else:
            print(
                "- To enable OAuth2 + REST mode (recommended): python3 scripts/oauth-setup.py"
            )
        if report["zoho_password_env_set"]:
            print(
                "- App-password mode is available. Try: python3 scripts/zoho-email.py unread --api-mode imap"
            )
        else:
            print(
                "- For app-password mode: export ZOHO_PASSWORD='<app-specific-password>'"
            )
    ...
    if command == "oauth-status":
        try:
            zoho = ZohoEmail(
                verbose=verbose, auth_method="oauth2", token_file=token_file
            )
            ...
            if status["status"] == "expired":
                print(
                    "\n⚠️  Token has expired. Run 'python3 zoho-email.py oauth-login' to refresh.",
                    file=sys.stderr,
                )
                sys.exit(1)
            elif status["status"] == "valid":
                print(
                    f"\n✓ Token is valid (expires in {status['expires_in_seconds']}s)",
                    file=sys.stderr,
                )
        except Exception as e:
        ...
        try:
            ...
            print(json.dumps(status, indent=2))
            print(
                f"\n✓ Tokens refreshed successfully (expires in {status['expires_in_seconds']}s)",
                file=sys.stderr,
            )
        except Exception as e:
            print(f"✗ Error: {e}", file=sys.stderr)
            print(
                "\nIf tokens are invalid, run 'python3 scripts/oauth-setup.py' to set up OAuth2 again.",
                file=sys.stderr,
            )
            sys.exit(1)
        sys.exit(0)
        ...
        try:
            ...
            print(
                f"\n✓ Token file deleted. Run 'python3 scripts/oauth-setup.py' to set up OAuth2 again.",
                file=sys.stderr,
            )
        except Exception as e:
            print(f"✗ Error: {e}", file=sys.stderr)
    ...
    try:
        ...
        print(
            "  1. OAuth2 (recommended): Run 'python3 scripts/oauth-setup.py' to configure",
            file=sys.stderr,
        )
        print(
            "  2. App Password: Set ZOHO_EMAIL and ZOHO_PASSWORD environment variables:",
            file=sys.stderr,
        )
    ...
    try:
        if command == "search-sent":
            ...
            if not all([to, subject, html_input]):
                print(
                    "Error: to, subject, and html_file_or_text required",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            if len(sys.argv) < 4:
                print(
                    "Error: folder and at least one email_id required", file=sys.stderr
                )
                print(
                    "Usage: python3 zoho-email.py mark-read <folder> <id1> <id2> ...",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                f"Marking {len(email_ids)} emails as read in folder '{folder}'...",
                file=sys.stderr,
            )
            ...
            if result["failed"]:
                print(
                    f"\nWarning: {len(result['failed'])} emails failed to update",
                    file=sys.stderr,
                )
            ...
            if len(sys.argv) < 4:
                ...
                print(
                    "Usage: python3 zoho-email.py mark-unread <folder> <id1> <id2> ...",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                f"Marking {len(email_ids)} emails as unread in folder '{folder}'...",
                file=sys.stderr,
            )
            ...
            if len(sys.argv) < 4:
                ...
                print(
                    "Usage: python3 zoho-email.py delete <folder> <id1> <id2> ...",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                f"⚠️  WARNING: About to delete {len(email_ids)} emails from '{folder}'",
                file=sys.stderr,
            )
            print(
                "Emails will be moved to Trash. Continue? (y/N): ",
                file=sys.stderr,
                end="",
            )
            ...
            if result["failed"]:
                print(
                    f"\nWarning: {len(result['failed'])} emails failed to delete",
                    file=sys.stderr,
                )
            ...
            if len(sys.argv) < 5:
                print(
                    "Error: source_folder, target_folder, and at least one email_id required",
                    file=sys.stderr,
                )
                print(
                    "Usage: python3 zoho-email.py move <source_folder> <target_folder> <id1> <id2> ...",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                f"Moving {len(email_ids)} emails from '{source_folder}' to '{target_folder}'...",
                file=sys.stderr,
            )
            ...
            if result["failed"]:
                print(
                    f"\nWarning: {len(result['failed'])} emails failed to move",
                    file=sys.stderr,
                )
            ...
            if dry_run:
                print(
                    "🔍 DRY RUN: Will delete ALL emails in folder 'Spam'",
                    file=sys.stderr,
                )
                print(
                    "   Tip: run with --execute to perform the deletion.",
                    file=sys.stderr,
                )
            else:
                print(
                    "⚠️  EXECUTING: Deleting ALL emails in folder 'Spam'",
                    file=sys.stderr,
                )
            ...
            if dry_run and result.get("to_process", 0) > 0:
                print(
                    "\n💡 To execute, re-run with: python3 zoho-email.py empty-spam --execute",
                    file=sys.stderr,
                )
            ...
            if dry_run:
                print(
                    "🔍 DRY RUN: Will delete ALL emails in folder 'Trash'",
                    file=sys.stderr,
                )
                print(
                    "   Tip: run with --execute to perform the deletion.",
                ...
                print(
                    "⚠️  EXECUTING: Deleting ALL emails in folder 'Trash'",
                    file=sys.stderr,
                )
            ...
            if dry_run and result.get("to_process", 0) > 0:
                print(
                    "\n💡 To execute, re-run with: python3 zoho-email.py empty-trash --execute",
                    file=sys.stderr,
                )
            elif not dry_run and result.get("failed"):
                print(
            ...
            if not all([folder, search_query, action]):
                print(
                    "Error: --folder, --search, and --action are required",
                    file=sys.stderr,
                )
                print(
                    "Usage: python3 zoho-email.py bulk-action --folder INBOX --search 'SUBJECT \"test\"' --action mark-read [--dry-run]",
                    file=sys.stderr,
                )
            ...
            if dry_run:
                print(
                    f"🔍 DRY RUN: Searching '{folder}' for emails matching: {search_query}",
                    file=sys.stderr,
                )
            else:
                print(
                    f"Executing bulk action '{action}' on '{folder}' for query: {search_query}",
                    file=sys.stderr,
                )

            result = zoho.bulk_action(
                search_query, action, folder=folder, dry_run=dry_run
            )
            print(json.dumps(result, indent=2))
            ...
            if dry_run and result.get("to_process", 0) > 0:
                ...
                print(
                    f"\nWarning: {len(result['failed'])} emails failed to process",
                    file=sys.stderr,
                )
