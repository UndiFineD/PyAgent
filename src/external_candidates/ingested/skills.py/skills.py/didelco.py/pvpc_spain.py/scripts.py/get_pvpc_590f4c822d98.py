...
def get_pvpc_data(date_str=None):
    ...
    url = (
        f"https://api.esios.ree.es/archives/70/download_json?locale=es&date={date_str}"
    )
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Muestra todos los precios del día"
    )
    ...
    if args.json:
        ...
        if current:
            print(
                f"Precio actual ({current['hour']:02d}:00): {current['price']:.4f} €/kWh"
            )
