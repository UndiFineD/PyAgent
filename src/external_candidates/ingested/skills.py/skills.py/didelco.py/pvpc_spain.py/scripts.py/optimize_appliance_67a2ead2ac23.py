...
def find_best_window(prices, duration):
    ...
    for i in range(len(sorted_prices) - duration + 1):
        ...
        if all(
            window[j]["hour"] == window[j - 1]["hour"] + 1
                ...
                alternatives.append(
                    {
                        ...
                        "diff_percent": round(
                            (
                                (total_cost - best_window["total_cost"])
                                / best_window["total_cost"]
                            )
                            * 100,
                            1,
                        ),
...
def main():
    ...
    parser.add_argument(
        "--alternatives", type=int, default=2, help="Número de alternativas a mostrar"
    )
    ...
    if not best:
        print(
            f"No se encontró ventana de {args.duration}h consecutivas", file=sys.stderr
        )
        sys.exit(1)
    ...
    if args.json:
        ...
        if alternatives:
            ...
            for i, alt in enumerate(alternatives, 1):
                ...
                print(
                    f"      Coste: {alt['total_cost']:.4f} € (+{alt['diff_percent']}%)"
                )
