# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ericrosenberg\strava-cycling-coach\scripts\analyze_rides.py
#!/usr/bin/env python3
"""
...
def load_config():
    if not CONFIG_PATH.exists():
        print(
            "Error: Configuration not found. Run scripts/setup.sh first.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def analyze_fitness_trends(rides, estimated_ftp=190):
    ...
    recent_7d = [
        ...
        if r["date"] >= (datetime.now() - timedelta(days=7)).isoformat()
    ]
    recent_28d = [
        r
        for r in rides_sorted
        if r["date"] >= (datetime.now() - timedelta(days=28)).isoformat()
    ]
    ...
    return {
        ...
        "power_trend": (
            "improving"
            if avg_power_recent > avg_power_all
            else "declining" if avg_power_recent < avg_power_all else "stable"
        ),
...
def main():
    ...
    parser.add_argument(
        "--ftp", type=int, default=190, help="Estimated FTP for TSS calculation"
    )
    ...
    if args.json:
        output = {"rides": analyzed_rides, "fitness": fitness}
