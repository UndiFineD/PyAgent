# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ericrosenberg\jellyseerr\scripts\track_requests.py
#!/usr/bin/env python3
"""
...
def add_request(
    media_id, media_type, title, requested_at=None, channel=None, chat_id=None
):
...
def remove_request(media_id, media_type):
    ...
    pending = [
        p
        for p in pending
        if not (p["media_id"] == media_id and p["media_type"] == media_type)
    ]
    save_pending(pending)
