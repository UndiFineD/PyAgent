# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ericrosenberg\jellyseerr\scripts\request_movie.py
#!/usr/bin/env python3
"""
...
def request_movie(config, tmdb_id):
    ...
    data = {"mediaId": tmdb_id, "mediaType": "movie"}
...
def main():
    ...
    for i, movie in enumerate(movies[:5], 1):
        ...
        status = (
            movie.get("mediaInfo", {}).get("status", 0) if movie.get("mediaInfo") else 0
        )
    ...
    status = (
        selected.get("mediaInfo", {}).get("status", 0)
        if selected.get("mediaInfo")
        else 0
    )
    if status >= 4:
