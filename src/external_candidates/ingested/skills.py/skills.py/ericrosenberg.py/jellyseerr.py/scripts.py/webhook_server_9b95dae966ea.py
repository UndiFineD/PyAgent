# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ericrosenberg\jellyseerr\scripts\webhook_server.py
#!/usr/bin/env python3
"""
    ...
    if CONFIG_FILE.exists():
        try:
            with open(CONFIG_FILE) as f:
                config = json.load(f)
                return config.get("telegram_chat_id") or os.environ.get(
                    "TELEGRAM_CHAT_ID"
                )
    ...
    def do_POST(self):
        ...
        try:
            ...
            if notification_type == "MEDIA_AVAILABLE":
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"status": "ok", "queued": success}).encode()
                )
            else:
