# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ricardotrevisan\voice-agent\scripts\client.py
import argparse
import json
import mimetypes
import os
...
def check_health():
    try:
        ...
        print(
            "❌ Could not connect to Voice Agent API (localhost:8000). Is the Docker container running?"
        )
...
def transcribe(filename):
    ...
    try:
        ...
        body.append(
            f'Content-Disposition: form-data; name="file"; filename="{os.path.basename(filename)}"'.encode()
        )
        ...
        req = urllib.request.Request(
            f"{API_URL}/transcribe", data=data, headers=headers
        )
