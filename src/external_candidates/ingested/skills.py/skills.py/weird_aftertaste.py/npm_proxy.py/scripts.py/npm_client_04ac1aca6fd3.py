# Extracted from: C:\DEV\PyAgent\.external\skills\skills\weird-aftertaste\npm-proxy\scripts\npm_client.py
#!/usr/bin/env python3
import json
import os
import sys
...
def cmd_hosts():
    ...
    for h in hosts:
        ...
        print(
            f"{h['id']:<4} {domain[:29]:<30} {forward[:29]:<30} {ssl:<5} {status:<10}"
        )
...
def main():
    if not NPM_URL or not NPM_EMAIL or not NPM_PASSWORD:
        print(
            "Error: NPM_URL, NPM_EMAIL, and NPM_PASSWORD must be set in env.",
            file=sys.stderr,
        )
        sys.exit(1)
