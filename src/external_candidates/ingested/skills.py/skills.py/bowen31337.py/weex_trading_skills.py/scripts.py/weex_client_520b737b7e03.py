...
class WeexClient:
    def __init__(self):
        ...
        if not all([self.api_key, self.api_secret, self.passphrase]):
            print(
                "Warning: WEEX_API_KEY, WEEX_API_SECRET, and WEEX_PASSPHRASE required for authenticated endpoints"
            )
    ...
    def _request(
        ...
        headers = (
            self._get_headers(method, path, body_str)
            if auth
    ...
    def get_ticker(self, symbol: str) -> dict:
        return self._request(
            "GET", "/capi/v2/market/ticker", {"symbol": symbol}, auth=False
        )
    ...
    def get_funding_rate(self, symbol: str | None = None) -> dict:
        ...
        return self._request(
            "GET", "/capi/v2/market/currentFundRate", params, auth=False
        )
    ...
    def cancel_order(self, order_id: str) -> dict:
        return self._request(
            "POST", "/capi/v2/order/cancel_order", body={"orderId": order_id}
        )

    def cancel_all_orders(
        self, symbol: str | None = None, cancel_type: str = "normal"
    ) -> dict:
