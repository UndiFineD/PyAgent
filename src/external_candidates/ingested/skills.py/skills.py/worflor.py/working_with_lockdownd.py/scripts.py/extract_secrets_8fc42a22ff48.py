# Extracted from: C:\DEV\PyAgent\.external\skills\skills\worflor\working-with-lockdownd\scripts\extract_secrets.py
...
def decode_findmy_keys(data):
    ...
    try:
        ...
        return {
            "p": (
                base64.b64encode(keys.get("p", b"")).decode() if keys.get("p") else None
            ),  # Public key
            "s": (
                base64.b64encode(keys.get("s", b"")).decode() if keys.get("s") else None
            ),  # Secret key
            "v": keys.get("v"),  # Version
            "i": keys.get("i"),  # Index
            "l": keys.get("l"),  # Level
            "b": (
                base64.b64encode(keys.get("b", b"")).decode() if keys.get("b") else None
            ),  # Beacon key?
            "1": keys.get("1", {}),  # Slot 1
            "2": keys.get("2", {}),  # Slot 2
        }
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Extract crypto/identity material via WiFi lockdownd (Orchard)"
    )
    ...
    ap.add_argument(
        "--write-files", action="store_true", help="write key files to --outdir"
    )
    ap.add_argument(
        "--yes",
        action="store_true",
    ...
    if not args.yes:
        ...
        print(
            "You must provide the --yes flag to confirm you understand this script extracts PRIVATE KEYS."
        )
        sys.exit(1)
    ...
    resp = send_recv(
        sock, {"Request": "GetValue", "Label": "secrets", "Key": "ActivationPublicKey"}
    )
    ...
    resp = send_recv(
        sock, {"Request": "GetValue", "Label": "secrets", "Key": "ActivationPrivateKey"}
    )
    if resp and resp.get("Value"):
        data = resp["Value"]
    ...
    # Device public key
    resp = send_recv(
        sock, {"Request": "GetValue", "Label": "secrets", "Key": "DevicePublicKey"}
    )
    if resp and resp.get("Value"):
        data = resp["Value"]
    ...
    # Escrow bag
    resp = send_recv(
        sock, {"Request": "GetValue", "Label": "secrets", "Key": "EscrowBag"}
    )
    if resp and resp.get("Value"):
        data = resp["Value"]
        secrets["EscrowBag"] = {"length": len(data), "hex": data.hex()}
    ...
    resp = send_recv(
        sock, {"Request": "GetValue", "Label": "secrets", "Key": "NonVolatileRAM"}
    )
    if resp and resp.get("Value"):
        nvram = resp["Value"]
        secrets["NVRAM"] = {}
            ...
            if isinstance(val, bytes):
                if "fm-" in key:
                    ...
                    if key == "fm-spkeys":
                        ...
                        print(
                            f"    {val.decode('utf-8', errors='replace') if len(val) < 50 else f'{len(val)} bytes'}"
                        )
    ...
    resp = send_recv(
        sock,
        {
            "Request": "GetValue",
            "Label": "secrets",
            "Key": "BasebandKeyHashInformation",
        },
    )
    if resp and resp.get("Value"):
        data = resp["Value"]
    ...
    resp = send_recv(
        sock,
        {"Request": "GetValue", "Label": "secrets", "Key": "BasebandMasterKeyHash"},
    )
    if resp and resp.get("Value"):
    ...
    resp = send_recv(
        sock,
        {"Request": "GetValue", "Label": "secrets", "Domain": "com.apple.fairplay"},
    )
    if resp and resp.get("Value"):
