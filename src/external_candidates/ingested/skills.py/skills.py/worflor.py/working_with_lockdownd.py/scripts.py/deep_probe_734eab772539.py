# Extracted from: C:\DEV\PyAgent\.external\skills\skills\worflor\working-with-lockdownd\scripts\deep_probe.py
#!/usr/bin/env python3
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Deep iOS lockdownd probe (Orchard) — READ ONLY"
    )
    ...
    for key in INTERESTING_KEYS:
        try:
            resp = send_recv(
                sock, {"Request": "GetValue", "Label": "deepprobe", "Key": key}
            )
    ...
    for service in SERVICES:
        try:
            resp = send_recv(
                sock,
                {"Request": "StartService", "Label": "deepprobe", "Service": service},
            )
            ...
            if resp and resp.get("Port"):
                ...
                available_services.append(
                    {"name": service, "port": port, "ssl": ssl_required}
                )
            ...
            try:
                ...
                for notif in notifications:
                    try:
                        send_recv(
                            notif_sock,
                            {"Command": "ObserveNotification", "Name": notif},
                        )
