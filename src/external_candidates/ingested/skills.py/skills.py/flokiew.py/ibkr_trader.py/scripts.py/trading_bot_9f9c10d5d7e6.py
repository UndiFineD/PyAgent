# Extracted from: C:\DEV\PyAgent\.external\skills\skills\flokiew\ibkr-trader\scripts\trading_bot.py
#!/usr/bin/env python3
"""
...
class IBKRClient:
    ...
    def get_quote(self, conid: int) -> Optional[Quote]:
        ...
        for _ in range(3):
            data = self._get(
                "/v1/api/iserver/marketdata/snapshot",
                {"conids": str(conid), "fields": "31,84,86,87,88"},
            )
            if data and len(data) > 0:
                d = data[0]
    ...
    def place_order(
        ...
        # Place order
        result = self._post(
            f"/v1/api/iserver/account/{self.account_id}/orders", {"orders": [order]}
        )
        ...
        if isinstance(result, list) and len(result) > 0:
            if result[0].get("messageIds"):
                # Confirm the order
                confirm = self._post(
                    f"/v1/api/iserver/reply/{result[0]['id']}", {"confirmed": True}
                )
                return confirm
    ...
    def cancel_order(self, order_id: str) -> dict:
        """Cancel an order."""
        return self._post(
            f"/v1/api/iserver/account/{self.account_id}/order/{order_id}",
            {"delete": True},
        )
...
def main():
    ...
    for p in positions:
        print(
            f"   {p.symbol}: {p.quantity} @ ${p.avg_cost:.2f} (P&L: ${p.unrealized_pnl:.2f})"
        )
