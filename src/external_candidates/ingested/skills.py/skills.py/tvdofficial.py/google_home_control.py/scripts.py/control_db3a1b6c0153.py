# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tvdofficial\google-home-control\scripts\control.py
import json
import os
import sys
...
def main():
    ...
    if not os.path.exists(creds_path):
        ...
        print(
            "Please set GOG_HOME_CREDS environment variable or place credentials.json in ~/.config/google-oauthlib-tool/"
        )
        return
    ...
    config = embedded_assistant_pb2.AssistConfig(
        ...
        device_config=embedded_assistant_pb2.DeviceConfig(
            device_id="openclaw_agent", device_model_id="openclaw_agent_model"
        ),
    )
    ...
    responses = assistant.Assist(
        generate_requests(), metadata=[("authorization", "Bearer " + credentials.token)]
    )
