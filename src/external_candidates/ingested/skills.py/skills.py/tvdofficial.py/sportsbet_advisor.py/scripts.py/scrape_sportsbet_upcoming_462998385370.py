...
RECOMMENDED_BETS_FILE = os.path.join(
    os.path.dirname(__file__), "sportsbet_recommended_bets.json"
)
...
def perform_web_search(query: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if search_results and "results" in search_results.get(
            "web_search_response", {}
        ):
...
def analyze_search_results(query_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for result in query_results:
        ...
        if (
            "odds" in snippet.lower()
            or "prediction" in snippet.lower()
            or "vs." in title.lower()
        ):
...
def get_top_bets_via_web_search(
    ...
    all_analyzed_events = []
    print(
        f"Searching Google for top upcoming bets across {', '.join(sports_categories)}..."
    )
    ...
    return all_analyzed_events[
        :num_bets
    ]  # Return top N based on initial discovery order


def recommend_bets(
    analyzed_events: List[Dict[str, Any]], num_recommendations: int = 5
) -> List[Dict[str, Any]]:
    recommendations = []
    ...
    for event in analyzed_events:
        ...
        if event.get("event_name") and event.get("teams"):
            ...
            if len(event["teams"]) > 1:
                recommended_team = event["teams"][
                    0
                ]  # Just pick the first team as an example
                confidence = min(
                    95, 60 + len(event["raw_snippet"]) // 50
                )  # Very simple confidence heuristic
                confidence = max(
                    60, confidence
                )  # Min confidence 60 for any recommendation
...
if __name__ == "__main__":
    ...
    analyzed_events = get_top_bets_via_web_search(
        target_sports_categories, num_bets=5, timeframe_days=1
    )
