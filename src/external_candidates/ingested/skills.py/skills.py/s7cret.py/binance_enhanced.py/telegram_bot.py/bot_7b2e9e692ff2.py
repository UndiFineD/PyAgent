# Extracted from: C:\DEV\PyAgent\.external\skills\skills\s7cret\binance-enhanced\telegram-bot\bot.py
#!/usr/bin/env python3
"""
...
from storage import list_orders, list_webhooks, register_webhook, save_order
...
def webhook():
    ...
    if "message" in data and data["message"].get("text"):
        ...
        msg_id = send_message(
            ...
            confirmation,
            parse_mode="HTML",
            buttons=build_inline_buttons(state, oid),
        )
    ...
    if "callback_query" in data:
        ...
        if data_str.startswith("confirm:"):
            ...
            send_message(
                chat_id, "Ордeр подтверждён. (Прототип — не отправляет на биржу)"
            )
