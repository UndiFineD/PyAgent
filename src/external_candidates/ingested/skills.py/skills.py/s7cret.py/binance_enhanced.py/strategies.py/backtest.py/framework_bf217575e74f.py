# Extracted from: C:\DEV\PyAgent\.external\skills\skills\s7cret\binance-enhanced\strategies\backtest\framework.py
"""
...
class Backtest:
    ...
    def run_strategy(self, strategy, *args, **kwargs):
        result = (
            strategy.run(self.price, *args, **kwargs)
            if hasattr(strategy, "run")
            else strategy(self.price)
        )
        return result
