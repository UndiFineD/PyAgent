# Extracted from: C:\DEV\PyAgent\.external\skills\skills\s7cret\binance-enhanced\ux\autocomplete.py
#!/usr/bin/env python3
"""
...
def suggest_symbols(prefix: str, limit: int = 5):
    ...
    res = [
        x
        for x in POPULAR_PAIRS
        if x["base"].startswith(p)
        or x["symbol"].startswith(p)
        or x["quote"].startswith(p)
    ]
    return res[:limit]
