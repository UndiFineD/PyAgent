# Extracted from: C:\DEV\PyAgent\.external\skills\skills\s7cret\binance-enhanced\ux\interactive.py
#!/usr/bin/env python3
"""
...
from templates import friendly_error, render_confirmation
...
class DialogManager:
    ...
    def handle_user_reply(self, state: Dict, reply: str) -> Dict:
        ...
        if (
            not state.get("symbol")
            and state.get("base_asset")
            and state.get("quote_asset")
        ):
