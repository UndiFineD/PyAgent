# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tanguyvans\bambu-local\bambu.py
#!/usr/bin/env python3
"""
...
def get_status():
    ...
    print(f"   ─────────────────────────")
    print(
        f"   🌡️  Lit: {status_data.get('bed_temper', 'N/A')}°C / {status_data.get('bed_target_temper', 0)}°C"
    )
    print(
        f"   🌡️  Buse: {status_data.get('nozzle_temper', 'N/A')}°C / {status_data.get('nozzle_target_temper', 0)}°C"
    )
    ...
    if status_data.get("gcode_state") == "RUNNING":
        ...
        print(
            f"   🎯 Couche: {status_data.get('layer_num', 0)}/{status_data.get('total_layer_num', 0)}"
        )
