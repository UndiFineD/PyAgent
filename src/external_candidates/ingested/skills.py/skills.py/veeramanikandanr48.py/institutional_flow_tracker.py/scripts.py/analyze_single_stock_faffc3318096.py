# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\institutional-flow-tracker\scripts\analyze_single_stock.py
#!/usr/bin/env python3
"""
...
class SingleStockAnalyzer:
    ...
    def analyze_stock(self, symbol: str, quarters: int = 8) -> Dict:
            ...
            quarterly_metrics.append(
                {
                    ...
                    "top_holders": sorted(
                        holders_q, key=lambda x: x.get("totalShares", 0), reverse=True
                    )[:20],
                }
            )
        ...
        if len(quarterly_metrics) >= 2:
            ...
            current_holders_map = {
                h.get("holder", ""): h for h in current_q["top_holders"]
            }
            previous_holders_map = {
                h.get("holder", ""): h for h in previous_q["top_holders"]
            }
            ...
            for name, holder in current_holders_map.items():
                ...
                if name in previous_holders_map:
                    ...
                    pct_change = (
                        (change / previous_shares * 100) if previous_shares > 0 else 0
                    )
                    ...
                    new_positions.append({"name": name, "shares": current_shares})

            # Check for closed positions
            for name, holder in previous_holders_map.items():
                if name not in current_holders_map:
                    closed_positions.append(
                        {"name": name, "previous_shares": holder.get("totalShares", 0)}
                    )
...
"""
        ...
        if len(metrics[0]["top_holders"]) >= 10:
            top_10_shares = sum(
                h.get("totalShares", 0) for h in metrics[0]["top_holders"][:10]
            )
            concentration = (
...
Examples:
  ...
  python3 analyze_single_stock.py TSLA --output tesla_analysis.md
    ...
    print("=" * 80)
    print(
        f"Trend ({args.quarters} quarters): {analysis['shares_trend']:+.2f}% shares, {analysis['holders_trend']:+d} institutions"
    )
