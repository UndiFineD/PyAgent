# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\pair-trade-screener\scripts\analyze_spread.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    parser.add_argument(
        "--stock-a", type=str, required=True, help="First stock ticker symbol"
    )
    parser.add_argument(
        "--stock-b", type=str, required=True, help="Second stock ticker symbol"
    )
    parser.add_argument(
        "--lookback-days",
        type=int,
    ...
    parser.add_argument(
        "--api-key", type=str, help="FMP API key (or set FMP_API_KEY env variable)"
    )
