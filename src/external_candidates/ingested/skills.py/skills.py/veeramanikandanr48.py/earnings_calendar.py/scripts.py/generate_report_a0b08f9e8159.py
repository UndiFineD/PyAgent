# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\earnings-calendar\scripts\generate_report.py
#!/usr/bin/env python3
"""
    ...
    try:
        with open(filepath, "r") as f:
            ...
            if not isinstance(data, list):
                print(
                    "ERROR: JSON file must contain an array of earnings data",
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        for timing in timings:
            ...
            if not stocks:
                ...
                display_stocks = sorted(
                    stocks, key=lambda x: x.get("marketCap", 0), reverse=True
                )[:30]
...
def print_usage():
    ...
    print(
        "  marketCap, marketCapFormatted, sector, epsEstimated, revenueEstimated",
        file=sys.stderr,
    )
