...
def analyze_html_file(filepath):
    ...
    if not parser.title:
        ...
        warnings.append(
            f"Title too short ({len(parser.title)} chars). Recommended: 50-60 characters"
        )
    elif len(parser.title) > 60:
        warnings.append(
            f"Title too long ({len(parser.title)} chars). May be truncated in search results"
        )
    else:
    ...
    if "description" not in parser.meta_tags:
        ...
        if desc_len < 120:
            warnings.append(
                f"Meta description too short ({desc_len} chars). Recommended: 150-160 characters"
            )
        elif desc_len > 160:
            warnings.append(
                f"Meta description too long ({desc_len} chars). May be truncated in search results"
            )
        else:
    ...
    if h1_count == 0:
        ...
        warnings.append(
            f"Multiple H1 headings ({h1_count}). Recommended: exactly 1 H1 per page"
        )
    else:
    ...
    if any(has_content):
        ...
        if first_used > 0:
            warnings.append(
                f"Heading hierarchy starts at H{first_used + 1} instead of H1"
            )
    ...
    if not parser.schema_markup:
        ...
        good_practices.append(
            f"Schema.org markup found: {', '.join(set(parser.schema_markup))}"
        )
    ...
    if len(content) < 300:
        warnings.append(
            "Page content is very short (< 300 characters). Search engines prefer substantial content"
        )
