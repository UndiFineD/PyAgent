...
class TestValidationModule(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(len(self.valid_data)):
            o, h, l, c = (
                self.valid_data.at[i, col] for col in ["open", "high", "low", "close"]
            )
            h_fixed = max(o, h, l, c)
            l_fixed = min(o, h, l, c)
...
class TestAnalyticsModule(unittest.TestCase):
    ...
    def test_monte_carlo_simulation(self):
        ...
        result = self.analytics.monte_carlo_simulation(
            df=self.valid_data, days_ahead=5, num_simulations=100
        )
...
class TestConfigurationSystem(unittest.TestCase):
    ...
    def test_indicator_config(self):
        ...
        self.assertGreater(
            config.indicators.rsi_overbought, config.indicators.rsi_oversold
        )
    ...
    def test_risk_config(self):
        ...
        print(
            f"✓ Risk config: max_risk={config.risk.max_risk_per_trade:.1%}, min_rr={config.risk.min_risk_reward}"
        )
