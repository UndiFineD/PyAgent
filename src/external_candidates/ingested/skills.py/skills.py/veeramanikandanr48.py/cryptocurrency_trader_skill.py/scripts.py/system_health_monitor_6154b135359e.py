# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\system_health_monitor.py
#!/usr/bin/env python3
"""
...
class SystemHealthMonitor:
    ...
    RESPONSE_TIME_THRESHOLDS = {"good": 1000, "acceptable": 5000, "slow": 10000}

    def __init__(self, check_interval_seconds: int = 60, history_size: int = 100):
        """
        ...
        logger.info(
            f"Initialized SystemHealthMonitor (check interval: {check_interval_seconds}s)"
        )

    def register_component(
        self, component_name: str, health_check_func: Optional[Callable] = None
    ) -> None:
        """
    ...
    def record_component_call(
        ...
        component.status = self._determine_status(
            component.success_rate, response_time_ms
        )

        # Log significant status changes
        if component.status in [HealthStatus.FAILING, HealthStatus.FAILED]:
            logger.warning(
                f"Component {component_name} status: {component.status.value} "
                f"(success rate: {component.success_rate:.1%})"
            )

    def _determine_status(
        self, success_rate: float, response_time_ms: float
    ) -> HealthStatus:
        """
        ...
        if (
            error_rate >= self.ERROR_THRESHOLDS["degraded"]
            or response_time_ms > self.RESPONSE_TIME_THRESHOLDS["slow"]
        ):
    ...
    def get_degradation_strategy(self) -> Dict:
            ...
            if comp_status == HealthStatus.FAILED:
                ...
                if fallback:
                    strategy["fallback_options"].append(
                        {"failed_component": name, "fallback": fallback}
                    )
        ...
        if status == HealthStatus.FAILED:
            strategy["recommendations"].insert(
                0,
                "CRITICAL: System health failed - recommend emergency maintenance mode",
            )
        elif status == HealthStatus.FAILING:
            strategy["recommendations"].insert(
                0, "WARNING: System health failing - enable fallback modes"
            )
        elif status == HealthStatus.DEGRADED:
            strategy["recommendations"].insert(
                0, "CAUTION: System health degraded - increased monitoring recommended"
            )
    ...
    def generate_health_report(self) -> str:
        ...
        if strategy["fallback_options"]:
            report_lines.extend(["", "Fallback Options:", "-" * 60])
