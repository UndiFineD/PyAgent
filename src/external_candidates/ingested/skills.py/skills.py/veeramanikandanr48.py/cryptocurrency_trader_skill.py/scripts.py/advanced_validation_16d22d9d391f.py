# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\advanced_validation.py
#!/usr/bin/env python3
"""
...
class AdvancedValidator:
    ...
    def validate_data_integrity(
        self, df: pd.DataFrame, symbol: str, timeframe: str = None
    ) -> Dict:
        """
    ...
    def _validate_structure(self, df: pd.DataFrame) -> Dict:
        ...
        if len(df) < 20:
            failures.append(
                f"Insufficient data points: {len(df)} (minimum 20 required)"
            )
    ...
    def _validate_price_logic(self, df: pd.DataFrame) -> Dict:
        ...
        for col in price_cols:
            if (df[col] <= 0).any():
                ...
                failures.append(
                    f"Invalid {col} prices: {invalid_count} non-positive values detected"
                )
        ...
        if len(df) > 1:
            ...
            if extreme_moves.any():
                ...
                failures.append(
                    f"Suspicious price jumps: {count} candles with >{max_move:.1f}% moves"
                )
    ...
    def _detect_statistical_anomalies(self, df: pd.DataFrame) -> Dict:
        ...
        if extreme_z.any():
            ...
            max_z = z_scores.max()
            anomalies.append(
                f"Extreme price movements detected: {count} events with Z-score > 5 (max: {max_z:.2f})"
            )
            severity = "WARNING"
        ...
        if outliers.any():
            count = outliers.sum()
            anomalies.append(
                f"Volume anomalies: {count} candles with extreme volume deviations"
            )
        ...
        if len(first_digits) >= 10:
            ...
            if len(benford_observed) >= 5:
                ...
                if obs_sum > 0 and exp_sum > 0:
                    ...
                    if p_value < 0.001:
                        anomalies.append(
                            f"Data may be fabricated (Benford's Law p={p_value:.4f})"
                        )
    ...
    def _validate_freshness(
        self, df: pd.DataFrame, timeframe_minutes: int = None
    ) -> Dict:
        """
        ...
        if timeframe_minutes:
            ...
            max_age = int(
                timeframe_minutes * 60 * multiplier
        ...
        return {
            ...
            "message": (
                f"Data is {age_seconds/60:.1f} minutes old (max: {max_age/60:.1f} minutes)"
    ...
    def _validate_completeness(self, df: pd.DataFrame) -> Dict:
        ...
        if missing_cells > 0:
            failures.append(
                f"Missing data: {missing_cells} cells ({missing_percentage:.2f}%)"
            )
        ...
        for col in ["open", "high", "low", "close"]:
            if df[col].nunique() == 1:
                failures.append(
                    f"Constant {col} values detected - potential data freeze"
                )
    ...
    def validate_indicators(self, indicators: Dict, df: pd.DataFrame) -> Dict:
        ...
        if "rsi" in indicators:
            if not (0 <= indicators["rsi"] <= 100):
                report["critical_failures"].append(
                    f"Invalid RSI: {indicators['rsi']} (must be 0-100)"
                )
                report["passed"] = False
        ...
        if "macd" in indicators and "current_price" in indicators:
            ...
            if (
                abs(indicators["macd"]) > price * 0.1
            ):  # MACD shouldn't exceed 10% of price
        ...
        if "atr" in indicators:
            if indicators["atr"] < 0:
                report["critical_failures"].append(
                    f"Invalid ATR: {indicators['atr']} (must be positive)"
                )
                report["passed"] = False
            elif indicators["atr"] == 0:
                report["warnings"].append(
                    "ATR is zero - indicates no volatility or data issue"
                )
        ...
        if all(k in indicators for k in ["bb_upper", "bb_lower", "current_price"]):
            if indicators["bb_upper"] <= indicators["bb_lower"]:
                report["critical_failures"].append(
                    "Invalid Bollinger Bands: Upper <= Lower"
                )
                report["passed"] = False
        ...
        if "rsi" in indicators and len(df) >= 14:
            ...
            if abs(calculated_rsi - indicators["rsi"]) > 1.0:
                report["warnings"].append(
                    f"RSI calculation mismatch: {calculated_rsi:.1f} vs {indicators['rsi']:.1f}"
                )
    ...
    def validate_trading_signal(self, analysis: Dict) -> Dict:
        ...
        if analysis.get("action") not in ["LONG", "SHORT", "WAIT"]:
            report["critical_failures"].append(
                f"Invalid action: {analysis.get('action')}"
            )
            report["passed"] = False
        ...
        if analysis.get("action") in ["LONG", "SHORT"]:
            if not all([entry, stop_loss, take_profit]):
                report["critical_failures"].append(
                    "Missing price levels for trade signal"
                )
                report["passed"] = False
            elif entry <= 0 or stop_loss <= 0 or take_profit <= 0:
                report["critical_failures"].append(
                    "Invalid price levels (must be positive)"
                )
                report["passed"] = False
            else:
                ...
                if analysis["action"] == "LONG":
                    if stop_loss >= entry:
                        report["critical_failures"].append(
                            "Invalid LONG: Stop loss must be below entry"
                        )
                        report["passed"] = False
                    if take_profit <= entry:
                        report["critical_failures"].append(
                            "Invalid LONG: Take profit must be above entry"
                        )
                        report["passed"] = False
                ...
                # Validate price level logic for SHORT
                    ...
                    if stop_loss <= entry:
                        report["critical_failures"].append(
                            "Invalid SHORT: Stop loss must be above entry"
                        )
                        report["passed"] = False
                    if take_profit >= entry:
                        report["critical_failures"].append(
                            "Invalid SHORT: Take profit must be below entry"
                        )
                        report["passed"] = False
    ...
    def cross_verify_analysis(self, analyses: List[Dict]) -> Dict:
        ...
        if len(set(actions)) > 1:
            report["notes"].append(
                f"Mixed signals detected: {dict(pd.Series(actions).value_counts())}"
            )
        ...
        if len(confidences) > 1:
            ...
            if confidence_std > 20:
                report["notes"].append(
                    f"High confidence variance: ±{confidence_std:.1f}%"
                )
        ...
        if len(entries) > 1:
            ...
            if entry_variance > 0.02:  # More than 2% variance
                report["notes"].append(
                    f"Entry price inconsistency: {entry_variance*100:.2f}% variance"
                )
