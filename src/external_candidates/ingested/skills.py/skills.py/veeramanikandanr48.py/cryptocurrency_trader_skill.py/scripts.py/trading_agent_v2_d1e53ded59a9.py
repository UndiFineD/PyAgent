...
class TradingAgentV2:
    ...
    **12-Stage Enhanced Workflow:**
    ...
    def __init__(
        ...
        self.health_monitor = (
            SystemHealthMonitor() if enable_health_monitoring else None
        )
        ...
        self.data_provider = MarketDataProvider(
            exchange_name=exchange_name, validator=self.validator
        )
    ...
    def _record_component_call(
        ...
        if self.health_monitor:
            self.health_monitor.record_component_call(
                component, success, response_time_ms, error
            )
        ...
        **Enhanced 12-Stage Comprehensive Analysis**
        ...
        try:
            ...
            analysis["stage_9_backtest"] = self._stage_9_backtest_validation(
                symbol, analysis["stage_6_signals"]
            )
            ...
            analysis["execution_ready"] = analysis["stage_11_validation"].get(
                "execution_ready", False
            )
            analysis["confidence"] = analysis["stage_10_recommendation"].get(
                "confidence", 0.0
            )
    ...
    def _stage_2_adaptive_timeframes(
        self, symbol: str, requested_timeframes: Optional[List[str]]
    ) -> Dict:
        """Stage 2: Adaptive Timeframe Selection"""
        ...
        if self.parameter_selector:
            try:
                ...
                recent_data = self.data_provider.fetch_market_data(
                    symbol, "1h", limit=100
                )

                if recent_data is not None:
                    result = self.parameter_selector.select_timeframes(
                        symbol, recent_data
                    )
    ...
    def _stage_3_multi_source_data(self, symbol: str, timeframes: List[str]) -> Dict:
        ...
        if self.multi_source_aggregator:
            for tf in timeframes:
                try:
                    result = self.multi_source_aggregator.fetch_validated_data(
                        symbol, tf
                    )
    ...
    def _stage_5_correlation_analysis(self, symbol: str) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"  BTC Correlation: {correlation.get('correlations', {}).get('BTC/USDT', 0):.2f}"
            )
    ...
    def _stage_6_adaptive_signals(self, data_stage: Dict, patterns_stage: Dict) -> Dict:
        ...
        if self.accuracy_tracker:
            ...
            logger.info(
                f"  Using learned accuracies ({len(learned_accuracies)} indicators)"
            )
        else:
        ...
        for tf, tf_data in data_stage.get("data", {}).items():
            try:
                df = tf_data["data"]
                patterns = patterns_stage.get("multi_timeframe_patterns", {}).get(
                    tf, {}
                )
                ...
                signals[tf] = signal
                logger.info(
                    f"  {tf}: {signal.get('action')} ({signal.get('probability', 0):.1%})"
                )
            except Exception as e:
    ...
    def _stage_7_monte_carlo(self, data_stage: Dict, context_stage: Dict) -> Dict:
        ...
        primary_tf = (
            "1h"
            if "1h" in data_stage.get("data", {})
            else list(data_stage.get("data", {}).keys())[0]
        )
        ...
        try:
            ...
            volatility = context_stage.get("context", {}).get(
            ...
            logger.info(
                f"  Expected Return: {mc_result.get('expected_return_pct', 0):.2f}%"
            )
            logger.info(
                f"  Win Probability: {mc_result.get('profit_probability', 0):.1%}"
            )
    ...
    def _stage_10_recommendation(self, symbol: str, analysis: Dict) -> Dict:
        ...
        try:
            ...
            patterns = analysis.get("stage_4_patterns", {}).get(
                "multi_timeframe_patterns", {}
            )
    ...
    def _stage_11_enhanced_validation(self, analysis: Dict) -> Dict:
        ...
        all_stages_passed = (
            validation.get("execution_ready", False)
            and context_ok
            and backtest_ok
            and health_ok
            and correlation_ok
    ...
    def _stage_12_position_sizing(self, analysis: Dict) -> Dict:
        ...
        try:
            ...
            position = self.position_sizer.calculate_position_size(
                self.balance, recommendation, mc_result
            )
