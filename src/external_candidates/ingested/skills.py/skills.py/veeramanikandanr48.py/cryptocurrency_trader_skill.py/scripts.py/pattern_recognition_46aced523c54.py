# Extracted from: C:\DEV\PyAgent\.external\skills\skills\veeramanikandanr48\cryptocurrency-trader-skill\scripts\pattern_recognition.py
#!/usr/bin/env python3
"""
...
class PatternRecognition:
    ...
    def analyze_comprehensive(self, df: pd.DataFrame) -> Dict:
        ...
        if len(df) < self.min_pattern_length:
            return {
                "error": f"Insufficient data (need {self.min_pattern_length}+ candles)"
            }
    ...
    def _detect_double_top_bottom(self, df: pd.DataFrame) -> List[Dict]:
        ...
        if len(peaks) >= 2:
            for i in range(len(peaks) - 1):
                ...
                if (
                    peak1_price > 0
                    and abs(peak1_price - peak2_price) / peak1_price < 0.02
                ):
                            ...
                            patterns.append(
                                {
                                    ...
                                    "target": round(
                                        neckline - decline_pct / 100 * peak1_price, 2
                                    ),
        ...
        if len(troughs) >= 2:
            for i in range(len(troughs) - 1):
                ...
                if (
                    trough1_price > 0
                    and abs(trough1_price - trough2_price) / trough1_price < 0.02
                ):
    ...
    def _cluster_levels(self, prices: np.ndarray, num_levels: int) -> List[float]:
        ...
        for price in sorted_prices:
            ...
            for cluster_price in clustered:
                ...
                if (
                    cluster_price > 0
                    and abs(price - cluster_price) / cluster_price < 0.02
                ):
                    similar = True
                    break
    ...
    def analyze_trend(self, df: pd.DataFrame) -> Dict:
        ...
        medium_term = (
            self._calculate_trend(prices[-20:]) if len(prices) >= 20 else short_term
        )
    ...
    def analyze_volume(self, df: pd.DataFrame) -> Dict:
        ...
        volume_ratio = (
            avg_volume_recent / avg_volume_historical
        ...
        return {
            ...
            "volume_status": (
                "HIGH"
                if volume_ratio > 1.5
                else "NORMAL" if volume_ratio > 0.7 else "LOW"
            ),
            "obv_trend": obv_trend,
            "vpt_trend": vpt_trend,
    ...
    def detect_market_regime(self, df: pd.DataFrame) -> Dict:
        ...
        # Volatility regime
        hist_vol = (
            returns.rolling(50).std().mean() if len(returns) >= 50 else volatility
        )
        ...
        vol_regime = (
            "HIGH_VOL"
            if vol_ratio > 1.3
            else "LOW_VOL" if vol_ratio < 0.7 else "NORMAL_VOL"
        )
    ...
    def _synthesize_bias(
        self, patterns: List[Dict], trend: Dict, volume: Dict, regime: Dict
    ) -> Dict:
        ...
        if not trend.get("error"):
            ...
            if trend.get("aligned"):
                bullish_score += (
                    20 if trend["short_term"]["direction"] == "UPTREND" else 0
                )
                bearish_score += (
                    20 if trend["short_term"]["direction"] == "DOWNTREND" else 0
                )
