...
requires_unifai = pytest.mark.skipif(
    not UNIFAI_KEY, reason="UNIFAI_AGENT_API_KEY not set"
)
...
class TestTradingAgent:
    ...
    async def test_tool_calling(self):
        ...
        try:
            ...
            assert any(
                word in response.lower() for word in ["bitcoin", "btc", "price", "$"]
            )
        finally:
            await agent.close()
...
if __name__ == "__main__":
    ...
    async def run_quick_tests():
        ...
        try:
            ...
            await client.close()
            results.append(
                ("Kalshi Fed Markets", True, f"Found {len(markets)} markets")
            )
