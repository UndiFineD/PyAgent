# Extracted from: C:\DEV\PyAgent\.external\skills\skills\zbruceli\unifai-trading-suite\skills\polymarket-trader\scripts\polymarket.py
#!/usr/bin/env python3
...
async def get_politics():
    ...
    try:
        ...
        response = await agent.chat(
            "Get the top political prediction markets from Polymarket with current prices"
        )
...
async def get_category(category: str):
    ...
    try:
        agent.clear_history()
        response = await agent.chat(
            f"Get the top prediction markets from Polymarket in the {category} category with current prices and volumes"
        )
...
def main():
    ...
    if cmd == "trending":
        ...
        if not cmd_args:
            ...
            print(
                "Categories: trending, new, politics, crypto, tech, culture, sports, world, economy"
            )
            sys.exit(1)
