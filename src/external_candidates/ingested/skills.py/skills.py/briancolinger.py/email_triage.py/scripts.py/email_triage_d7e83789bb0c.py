# Extracted from: C:\DEV\PyAgent\.external\skills\skills\briancolinger\email-triage\scripts\email-triage.py
#!/usr/bin/env python3
...
def classify_heuristic(sender: str, subject: str, preview: str) -> tuple[str, str]:
    ...
    if any(p in combined for p in spam_patterns) and any(
        s in sender_lower for s in spam_senders
    ):
    ...
    if any(p in combined for p in info_patterns) or any(
        s in sender_lower for s in info_senders
    ):
...
def report(as_json: bool = False) -> list[dict]:
    ...
    important.sort(
        key=lambda e: (priority_order.get(e["category"], 9), e.get("date", ""))
    )
...
def mark_surfaced():
    ...
    count = 0
    for key, entry in state["emails"].items():
        if not entry.get("surfaced") and entry["category"] in (
            "urgent",
            "needs-response",
        ):
            entry["surfaced"] = True
...
def main():
    parser = argparse.ArgumentParser(
        description="Email triage — IMAP scanner with AI classification"
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Scan without saving state"
    )
