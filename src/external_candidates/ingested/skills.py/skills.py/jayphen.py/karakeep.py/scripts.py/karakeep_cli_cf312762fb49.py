# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jayphen\karakeep\scripts\karakeep-cli.py
import argparse
import json
import os
import re
...
def get_config():
    ...
    url = os.environ.get("KARAKEEP_URL") or os.environ.get(
        "HOARDER_URL", "https://hoard.phen.boo"
    )
...
def make_request(method, endpoint, data=None, params=None):
    ...
    if not api_key:
        print(
            "Error: KARAKEEP_API_KEY not set. Run 'login' or set the environment variable."
        )
        sys.exit(1)
    ...
    try:
        response = requests.request(
            method, full_url, headers=headers, json=data, params=params
        )
...
def list_bookmarks(args):
    ...
    for b in bookmarks:
        ...
        title = (
            b.get("title") or content.get("title") or content.get("url") or "(No Title)"
        )
        url = content.get("url")
...
def add_bookmark(args):
    ...
    print(
        f"Successfully hoarded: {result.get('title') or result.get('text') or args.url}"
    )
