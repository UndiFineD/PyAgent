        ...
        for attempt in range(self.max_retries + 1):
            try:
                resp = self._session.get(
                    url, headers=headers, params=params, timeout=self.timeout_s
                )
            ...
            if not resp.ok:
                raise OuraError(
                    f"HTTP {resp.status_code} calling {url}: {_truncate(resp.text)}"
                )
            ...
            try:
                ...
                raise OuraError(
                    f"Non-JSON response from {url}: {_truncate(resp.text)}"
                ) from e
...
def _paginate(
    client: OuraClient, endpoint: str, params: Dict[str, Any]
) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        if not isinstance(batch, list):
            raise OuraError(
                f"Unexpected pagination payload: 'data' is {type(batch).__name__}"
            )
...
def cmd_trends(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    sessions_sorted = sorted(
        sessions, key=lambda x: str(x.get("day") or x.get("date") or "")
    )
    ...
    if t == "daily_readiness":
        ...
        for it in payload.get("items") or []:
            if isinstance(it, dict):
                sys.stdout.write(
                    f"{it.get('day','?')} readiness_score={it.get('score')}\n"
                )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--mock", action="store_true", help="Emit example payloads (no network)"
    )
