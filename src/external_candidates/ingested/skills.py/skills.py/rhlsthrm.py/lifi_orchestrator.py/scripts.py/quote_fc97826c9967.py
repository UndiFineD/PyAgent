# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rhlsthrm\lifi-orchestrator\scripts\quote.py
#!/usr/bin/env python3
...
def get_quote(
    ...
    if from_token_addr == "0x0000000000000000000000000000000000000000":
        ...
        try:
            r = requests.get(
                f"{API_BASE}/token",
                params={"chain": from_chain, "token": from_token_addr},
            )
...
def main():
    ...
    parser.add_argument(
        "--to-chain", type=int, required=True, help="Destination chain ID"
    )
    parser.add_argument(
        "--from-token", required=True, help="Source token (symbol or address)"
    )
    parser.add_argument(
        "--to-token", required=True, help="Destination token (symbol or address)"
    )
