# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mvanhorn\granola-notes\scripts\csv_import.py
#!/usr/bin/env python3
"""
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            note = {
                ...
                "summary": row.get(
                    "summary", row.get("Summary", row.get("note_summary", ""))
                ),
...
def main():
    ...
    parser.add_argument(
        "--file", "-f", required=True, help="Path to Granola CSV export"
    )
    parser.add_argument("--search", "-s", help="Search query")
    parser.add_argument("--json", "-j", action="store_true", help="Output as JSON")
    parser.add_argument(
        "--limit", "-l", type=int, default=10, help="Max results to show"
    )
