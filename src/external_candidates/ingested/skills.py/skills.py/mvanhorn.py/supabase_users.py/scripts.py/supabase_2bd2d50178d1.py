# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mvanhorn\supabase-users\scripts\supabase.py
#!/usr/bin/env python3
"""
...
def list_users(limit: int = 10, time_filter: str = None):
    ...
    if not is_jwt_key(key):
        ...
        print(
            "   Get the JWT key from: Project Settings → API → Legacy tab → service_role"
        )
        return None
...
def run_query(sql: str):
    ...
    try:
        ...
        if resp.status_code == 404:
            ...
            print(
                "   Create one in Supabase SQL Editor, or use list-users/users commands."
            )
            return None
    ...
    if cmd == "auth":
        ...
        if result:
            if args.json:
                ...
                print(
                    f"👥 Users ({result['total']} total, showing {len(result['users'])}):\n"
                )
