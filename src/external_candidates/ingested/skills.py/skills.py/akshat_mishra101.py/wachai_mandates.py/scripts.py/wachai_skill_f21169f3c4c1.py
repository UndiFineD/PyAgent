...
def default_deadlines(
    *, mandate_minutes: int = 10, swap_minutes: int = 5
) -> tuple[str, str]:
    now = utc_now()
    return (
...
def cmd_create_mandate(args: argparse.Namespace) -> int:
    ...
    if args.swap:
        ...
        intent = (
            args.intent
            or f"Swap {amount_in} of {token_in} for >= {min_out} of {token_out} on chain {chain_id}"
        )
        ...
        raise SystemExit(
            "create-mandate requires one of: --swap ... OR --custom <KIND> --body <JSON>"
        )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="wachai", description="WachAI Mandates CLI (create, sign, verify)"
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    create = sub.add_parser(
        "create-mandate", help="Create a mandate and sign it as server (offer)"
    )
    ...
    create.add_argument(
        "--chain-id", default=1, type=int, help="EVM chain id (default: 1)"
    )
    create.add_argument(
        "--client",
    ...
    create.add_argument(
        "--recipient", help="Swap recipient 0x address. Defaults to the client address."
    )
    create.add_argument(
        "--deadline", help="Mandate deadline (ISO-8601, e.g. 2025-12-31T00:10:00Z)"
    )
    create.add_argument(
        "--swap-deadline",
    ...
    create.add_argument(
        "--no-store", action="store_true", help="Do not save mandate to local store"
    )
    create.add_argument(
        "--print-path", action="store_true", help="Print save path to stderr"
    )
