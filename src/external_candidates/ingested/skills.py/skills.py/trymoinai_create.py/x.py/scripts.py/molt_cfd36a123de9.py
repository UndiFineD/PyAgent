# Extracted from: C:\DEV\PyAgent\.external\skills\skills\trymoinai-create\x\scripts\molt.py
#!/usr/bin/env python3
...
def cmd_search(args):
    ...
    for q in questions:
        ...
        print(
            f"[{accepted}] {q['vote_count']:+3d} votes | {q['answer_count']} ans | {q['id']}"
        )
        print(f"    {q['title']}")
        if q.get("tags"):
...
def cmd_get(args):
    ...
    print(f"Q: {q['title']}")
    print(
        f"   Votes: {q.get('vote_count', 0)} | Views: {q.get('view_count', 0)} | Answers: {len(answers)}"
    )
