# Extracted from: C:\DEV\PyAgent\.external\skills\skills\udiedrichsen\event-planner\scripts\plan_event.py
#!/usr/bin/env python3
# /// script
...
def parse_budget(budget_str: str, party_size: int) -> BudgetConstraints:
    ...
    dollar_match = re.match(
        r"\$?(\d+(?:\.\d+)?)\s*(?:per\s+person)?", budget_str.lower()
    )
    if dollar_match:
    ...
    for attempt in range(retry_count):
        try:
            ...
            if attempt < retry_count - 1:
                ...
                print(
                    f"Request failed after {retry_count} attempts: {e}", file=sys.stderr
                )
                return {}
...
def resolve_location(location: str) -> Coordinates | None:
    ...
    return Coordinates(lat=loc["latitude"], lng=loc["longitude"], address=address)


def search_venues_with_fallback(
    query: str, location: Coordinates, filters: dict
) -> list[Venue]:
...
def search_venues(
    query: str, location: Coordinates, filters: dict, radius: int = 5000
) -> list[Venue]:
...
def get_directions_info(
    origin: Coordinates, dest: Coordinates, mode: str = "walking"
...
def select_best_venues(
    venues: list[Venue], origin: Coordinates, budget: BudgetConstraints, count: int = 1
) -> list[Venue]:
...
def check_venue_warnings(venue: Venue) -> list[str]:
    ...
    if event_types:
        warnings.append(
            f"{venue.name} may have variable hours - check schedule before visiting"
        )
...
def generate_itinerary(
    ...
    for i, venue in enumerate(venues):
        ...
        travel_source = ""
        if prev_location:
            travel_time, travel_distance_km, travel_source = calculate_travel_time(
                prev_location, venue.location
            )
            if travel_time > 20:
                source_note = " (estimated)" if travel_source == "estimated" else ""
                warnings.append(
                    f"Long walk to {venue.name}: {travel_time} minutes{source_note}"
                )
        ...
        # Departure time
        departure_time = (current_time + timedelta(minutes=duration)).strftime(
            "%I:%M %p"
        )
    ...
    price_ranges = {1: "$10-20", 2: "$20-40", 3: "$40-60", 4: "$60+"}
...
def format_output_text(itinerary: Itinerary) -> str:
    ...
    for i, item in enumerate(itinerary.items):
        ...
        if i < len(itinerary.items) - 1:
            ...
            if next_travel > 0:
                ...
                source_note = (
                    " ~" if next_item.travel_time_source == "estimated" else ""
                )
                lines.append(
    ...
    lines.extend(["", "---", "💡 *Generated by Clawdbot Event Planner*"])
...
def main():
    parser = argparse.ArgumentParser(description="Plan events using Google Places API")
    parser.add_argument(
        "event_type",
        choices=[
            "night-out",
    ...
    parser.add_argument(
        "--party-size", type=int, default=2, help="Number of people (default: 2)"
    )
    parser.add_argument(
        "--budget",
        default="medium",
    ...
    parser.add_argument(
        "--duration", help="Duration (e.g., '3h', '4h', 'full day') - currently unused"
    )
    parser.add_argument(
        "--preferences",
    ...
    parser.add_argument(
        "--start-time", help="Start time in HH:MM format (e.g., '19:00')"
    )
    parser.add_argument(
        "--output",
        choices=["text", "json"],
    ...
    if not API_KEY:
        print(
            "Error: GOOGLE_PLACES_API_KEY environment variable not set", file=sys.stderr
        )
        sys.exit(1)
    ...
    for template_item in template:
        ...
        if available:
            ...
            if best:
                ...
                print(
                    f"  Found: {best[0].name} ({best[0].rating}⭐, {best[0].price_level})",
                    file=sys.stderr,
                )
        elif venues:
            ...
            if best and best[0].place_id not in selected_place_ids:
                ...
                selected_place_ids.add(best[0].place_id)
                print(
                    f"  Found (fallback): {best[0].name} ({best[0].rating}⭐)",
                    file=sys.stderr,
                )
            else:
