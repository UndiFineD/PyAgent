# Extracted from: C:\DEV\PyAgent\.external\skills\skills\udiedrichsen\harvey\scripts\secret_guide.py
#!/usr/bin/env python3
# /// script
...
def find_nearby_destinations(
    lat: float, lng: float, vibe: str = "explore", radius_m: int = 800
) -> list:
...
def create_secret_plan(
    start_location: str, vibe: str = "explore", max_distance: int = 800
) -> dict:
    """
    ...
    destinations = find_nearby_destinations(
        start["lat"], start["lng"], vibe=vibe, radius_m=max_distance
    )
    ...
    weights = [
        (d.get("rating") or 3.0) * (1 + (d.get("reviews") or 0) / 100)
...
def get_next_step() -> dict:
    ...
    if current == 0:
        ...
        phrase = random.choice(SPONTANEOUS_PHRASES["arrival_hint"]).format(
            direction="links"
        )
...
def reveal_destination() -> dict:
    ...
    result["maps_link"] = (
        f"https://www.google.com/maps/search/?api=1&query={dest['lat']},{dest['lng']}"
    )
...
def get_plan_status() -> dict:
    ...
    return {
        ...
        "destination_type": (
            plan["destination"]["type"] if plan.get("revealed") else "???"
        ),
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["plan", "next", "reveal", "status", "end", "destinations"],
    ...
    parser.add_argument(
        ...
        default="explore",
        help="Vibe/type of destination",
    )
    parser.add_argument(
        "--distance", "-d", type=int, default=800, help="Max distance in meters"
    )
    ...
    if args.action == "plan":
        ...
        if not start:
            ...
            sys.exit(1)
        result = find_nearby_destinations(
            start["lat"], start["lng"], args.vibe, args.distance
        )
