# Extracted from: C:\DEV\PyAgent\.external\skills\skills\udiedrichsen\harvey\scripts\harvey.py
#!/usr/bin/env python3
# /// script
...
def update_session(
    topic: str = None, mention_key: str = None, mention_value: str = None
) -> dict:
...
def get_game_stats() -> dict:
    ...
    if not history:
        return {"games_played": 0, "message": "Noch keine Spiele gespielt! 🎮"}
...
def adjust_difficulty(direction: str = "auto") -> str:
    ...
    current_idx = (
        DIFFICULTY_LEVELS.index(current) if current in DIFFICULTY_LEVELS else 1
    )
...
def main():
    ...
    parser.add_argument(
        "--mode", "-m", choices=list(MODES.keys()), default="bored", help="Session mode"
    )
    ...
    parser.add_argument(
        "--reason", default="user_ended", help="Reason for ending session"
    )
    parser.add_argument("--json", "-j", action="store_true", help="Output as JSON")
    # Game arguments
    parser.add_argument(
        "--game-type", "-g", help="Type of game (trivia, riddle, what_if, etc.)"
    )
    parser.add_argument(
        "--difficulty", "-d", choices=DIFFICULTY_LEVELS, default="medium"
    )
