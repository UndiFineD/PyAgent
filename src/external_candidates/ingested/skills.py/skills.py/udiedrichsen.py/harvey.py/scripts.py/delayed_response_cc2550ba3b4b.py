# Extracted from: C:\DEV\PyAgent\.external\skills\skills\udiedrichsen\harvey\scripts\delayed_response.py
#!/usr/bin/env python3
# /// script
...
def schedule_response(
    message: str, delay_seconds: int = None, mode: str = "restaurant"
) -> dict:
    """
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["schedule", "pending", "sent", "status", "clear"],
    ...
    if args.action == "schedule":
        ...
        if args.json:
            ...
            print(
                f"⏰ Scheduled in {result['delay_seconds']}s: {result['message'][:50]}..."
            )
        ...
        if args.json:
            ...
            print(
                f"📊 Queue: {status['total']} total | {status['pending']} pending | {status['due_now']} due | {status['sent']} sent"
            )
