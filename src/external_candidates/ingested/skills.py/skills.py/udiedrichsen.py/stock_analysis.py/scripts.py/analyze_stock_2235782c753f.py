# Extracted from: C:\DEV\PyAgent\.external\skills\skills\udiedrichsen\stock-analysis\scripts\analyze_stock.py
#!/usr/bin/env python3
# /// script
            ...
            if pd.notna(row.get("Reported EPS")) and pd.notna(row.get("EPS Estimate")):
                ...
                try:
                    ...
                    price_data = data.price_history[
                        data.price_history.index.date == earnings_day
                    ]
...
def analyze_earnings_timing(data: StockData) -> EarningsTiming | None:
    ...
    try:
        ...
        return EarningsTiming(
            ...
            next_earnings_date=(
                next_earnings_date.strftime("%Y-%m-%d") if next_earnings_date else None
            ),
            last_earnings_date=(
                last_earnings_date.strftime("%Y-%m-%d") if last_earnings_date else None
            ),
...
async def analyze_sentiment(
    ...
    try:
        ...
        if skip_insider:
            ...
            if verbose:
                print(
                    "    Skipping insider trading analysis (--no-insider)",
                    file=sys.stderr,
                )
        else:
            tasks.append(
...
def main():
    ...
    for ticker in args.tickers:
        ...
        if args.verbose and is_crypto:
            print(
                f"  Asset type: CRYPTO (using crypto-specific analysis)",
                file=sys.stderr,
            )
        ...
        if is_crypto:
            ...
            sentiment = asyncio.run(
                analyze_sentiment(
                    data, verbose=args.verbose, skip_insider=args.no_insider
                )
            )
        ...
        if args.verbose:
            ...
            if is_crypto:
                ...
                print(
                    f"  (Earnings, Sector, Sentiment: N/A for crypto)\n",
                    file=sys.stderr,
                )
            else:
