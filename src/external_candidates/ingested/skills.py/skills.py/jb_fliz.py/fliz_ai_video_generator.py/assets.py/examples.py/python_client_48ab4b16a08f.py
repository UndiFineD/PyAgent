...
class FlizClient:
    ...
    def list_videos(self, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        response = self._request(
            "GET", "/api/rest/videos", params={"limit": limit, "offset": offset}
        )
    ...
    def wait_for_video(
        ...
        while True:
            ...
            if status["is_error"]:
                raise FlizAPIError(
                    f"Video failed: {status['step']}", response=status["video"]
                )
    ...
    def translate_video(
        ...
        response = self._request(
            "POST", f"/api/rest/videos/{video_id}/translate", params=params
        )
...
if __name__ == "__main__":
    ...
    try:
        video = client.wait_for_video(
            result["video_id"], poll_interval=10, callback=on_progress
        )
