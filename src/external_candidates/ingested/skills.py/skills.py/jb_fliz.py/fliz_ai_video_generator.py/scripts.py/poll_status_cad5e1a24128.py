...
def get_video_status(api_key: str, video_id: str) -> dict:
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/rest/videos/{video_id}", headers=headers, timeout=TIMEOUT
        )
...
def poll_until_complete(
    api_key: str, video_id: str, interval: int = 15, timeout: int = 600
) -> dict:
...
def main():
    ...
    parser.add_argument(
        "--once", action="store_true", help="Check status once without polling"
    )
    ...
    if args.once:
        ...
        result = poll_until_complete(
            api_key, args.video_id, interval=args.interval, timeout=args.timeout
        )
