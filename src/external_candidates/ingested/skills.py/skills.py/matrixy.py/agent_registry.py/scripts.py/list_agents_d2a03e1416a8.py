# Extracted from: C:\DEV\PyAgent\.external\skills\skills\matrixy\agent-registry\scripts\list_agents.py
#!/usr/bin/env python3
"""
List Agents Script
...
def format_table(agents: List[Dict]) -> str:
    ...
    lines.append("-" * 80)
    lines.append(
        f"Total: {len(agents)} agents, {total_tokens:,} tokens (saved from preloading)"
    )
...
def format_detailed(agents: List[Dict]) -> str:
    ...
    lines.append(
        f"Total: {len(agents)} agents | {total_tokens:,} tokens saved by lazy loading"
    )
