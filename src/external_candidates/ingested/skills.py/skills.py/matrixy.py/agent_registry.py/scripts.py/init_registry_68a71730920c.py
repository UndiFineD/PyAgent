# Extracted from: C:\DEV\PyAgent\.external\skills\skills\matrixy\agent-registry\scripts\init_registry.py
#!/usr/bin/env python3
"""
...
def build_page_choices(
    ...
    header = (
        f"Page {page_num + 1}/{page_state.total_pages} | Selected: {selected}/{total}"
    )
    ...
    for category, agent in page_items:
        ...
        choices.append(
            questionary.Choice(
                title=label, value=agent, checked=page_state.is_selected(agent["name"])
            )
        )
...
def display_agent_list(agents: List[Dict], title: str) -> None:
    ...
    for i, agent in enumerate(agents, 1):
        ...
        token_color = (
            Colors.GREEN
            if tokens < 1000
            else Colors.YELLOW if tokens < 3000 else Colors.RED
        )

        print(f"  [{i}] {colorize(agent['name'], Colors.CYAN)}")
        print(
            f"      {colorize(agent['summary'][:60] + '...' if len(agent['summary']) > 60 else agent['summary'], Colors.DIM)}"
        )
        print(
            f"      Tokens: {colorize(str(tokens), token_color)} | Source: {Path(agent['source_dir']).name}"
        )
        print()
...
def interactive_selection_paged(agents: List[Dict]) -> List[Dict]:
    ...
    while True:
        try:
            ...
            selected = questionary.checkbox(
                message="Select agents (Space=toggle, Enter=next)", choices=choices
            ).ask()
        ...
        try:
            ...
            for idx in indices:
                if 1 <= idx <= len(agents):
                    ...
                    print(
                        colorize(
                            f"  Invalid index: {idx}. Valid range: 1-{len(agents)}",
                            Colors.RED,
                        )
                    )
                    continue
...
def migrate_agents(
    agents: List[Dict], target_dir: Path
    ...
    for agent in agents:
        ...
        try:
            ...
            if target.exists():
                print(
                    colorize(
                        f"  Skipping {agent['name']}: already exists in registry",
                        Colors.YELLOW,
                    )
                )
...
def build_registry(agents: List[Dict], registry_path: Path) -> None:
    ...
    for agent in agents:
        ...
        rel_path = (
            Path(agent["path"]).relative_to(get_skill_dir())
            if get_skill_dir() in Path(agent["path"]).parents
            else agent["path"]
        )
    ...
    print(
        colorize(
            f"  Tokens saved by lazy loading: ~{registry['stats']['tokens_saved_vs_preload']:,}",
...
def main():
    ...
    print(
        colorize(
            "║  Reduce token overhead by lazy-loading agents            ║",
            Colors.HEADER,
        )
    )
    print(
    ...
    if existing_registry_agents:
        print(
            colorize(
                f"\nAlready in registry: {len(existing_registry_agents)} agent(s)",
                Colors.CYAN,
            )
        )
    ...
    if all_registry_agents:
        ...
        print(
            "  2. Claude will now search agents on-demand instead of loading all upfront"
        )
        ...
        print(
            colorize(
                "\nNo agents in registry. Run this script again after adding agents.",
                Colors.YELLOW,
            )
        )
