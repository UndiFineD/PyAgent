...
class OAuthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if "code" in params:
            ...
            self.end_headers()
            self.wfile.write(
                "Авторизация прошла успешно! Вы можете закрыть это окно и вернуться в терминал.".encode(
                    "utf-8"
                )
            )
        else:
...
def main():
    ...
    if server.auth_code:
        ...
        print("-------------------\n")
        print(
            "Сохраните эти данные в настройках OpenClaw или используйте их в запросах."
        )
    else:
