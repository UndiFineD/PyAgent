# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ruslanlanket\dzen\scripts\publish.py
import argparse
import json
import os
import sys
...
def upload_media(file_path, cookies, csrf_token):
    ...
    with open(file_path, "rb") as f:
        ...
        response = requests.post(
            UPLOAD_URL, headers=headers, cookies=cookies, files=files
        )
...
def publish_post(title, text, media_paths, cookies, csrf_token):
    ...
    # Text block
    if text:
        blocks.append({"type": "text", "data": {"text": f"<p>{text}</p>"}})
    ...
    for path in media_paths:
        ...
        if media_id:
            if ext in [".jpg", ".jpeg", ".png", ".webp", ".gif"]:
                blocks.append(
                    {"type": "image", "data": {"id": media_id, "caption": ""}}
                )
            elif ext in [".mp4", ".mov", ".avi", ".mkv"]:
                blocks.append(
                    {"type": "video", "data": {"id": media_id, "service": "dzen"}}
                )

    payload = {"title": title, "blocks": blocks, "type": "article"}

    response = requests.post(
        PUBLISH_URL, headers=headers, cookies=cookies, json=payload
    )
...
def main():
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to JSON config with cookies and csrf_token",
    )
