...
def main():
    ...
    reserve_parser.add_argument(
        "--wait-seconds", type=int, default=10, help="재시도 대기 시간 (초, 기본값: 10)"
    )

    # List command
    list_parser = subparsers.add_parser("list", help="예약 목록 조회")
    list_parser.add_argument(
        "--format", choices=["table", "json"], default="table", help="출력 형식"
    )
    ...
    cancel_parser.add_argument(
        "--confirm", action="store_true", help="확인 없이 바로 취소"
    )

    # Check retry log command
    log_parser = subparsers.add_parser("log", help="예약 재시도 로그 확인")
    log_parser.add_argument(
        "--lines", "-n", type=int, default=20, help="표시할 라인 수 (기본값: 20)"
    )
