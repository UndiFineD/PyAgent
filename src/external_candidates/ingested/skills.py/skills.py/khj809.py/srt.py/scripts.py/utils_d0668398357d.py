# Extracted from: C:\DEV\PyAgent\.external\skills\skills\khj809\srt\scripts\utils.py
#!/usr/bin/env python3
"""
...
def format_train_info(train):
    ...
    """
    general_seat = (
        train.general_seat_state if hasattr(train, "general_seat_state") else "N/A"
    )
    special_seat = (
        train.special_seat_state if hasattr(train, "special_seat_state") else "N/A"
    )
    ...
    return {
        ...
        "train_name": (
            train.train_name
            if hasattr(train, "train_name")
            else f"SRT{train.train_number}"
        ),
