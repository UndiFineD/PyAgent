# Extracted from: C:\DEV\PyAgent\.external\skills\skills\abhinavjp\ms-outlook-teams-assistant\scripts\dismiss.py
...
def main() -> int:
    ...
    if idx < 0 or idx >= len(keys):
        raise SystemExit(
            f"Invalid number {args.number}. Last batch had {len(keys)} items."
        )
