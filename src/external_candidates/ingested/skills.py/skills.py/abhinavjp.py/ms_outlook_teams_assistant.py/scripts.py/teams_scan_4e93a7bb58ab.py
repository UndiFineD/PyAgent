...
def _strip_html(s: str) -> str:
    ...
    s = re.sub(
        r"<emoji[^>]*\salt=\"([^\"]+)\"[^>]*></emoji>", r"\1", s, flags=re.IGNORECASE
    )
...
def acquire_token(cfg: dict) -> str:
    ...
    cache_path = tcfg.get("tokenCachePath") or os.path.join(
        "state", "teams_token_cache.bin"
    )
...
def main() -> int:
    ...
    chats = graph_get(
        token, "https://graph.microsoft.com/v1.0/me/chats", params={"$top": max_chats}
    )
    for chat in chats.get("value", []):
        ...
        for m in msgs.get("value", []):
            ...
            mentioned_me = (
                any(
                    (((x.get("mentioned") or {}).get("user") or {}).get("id")) == my_id
                    for x in mentions
                )
                if my_id
                else False
            ...
            if chat_type not in ("group", "meeting"):
                ...
                if keyword_hits:
                    needs_reply = True
                    reason.append(
                        "keywords: " + ", ".join(sorted(set(keyword_hits))[:5])
                    )

                replied_after = bool(
                    latest_my_reply_dt and latest_my_reply_dt > created_dt
                )
            ...
            items.append(
                {
                    ...
                    "myReplyCreated": (
                        latest_my_reply_dt.isoformat(timespec="seconds")
                        if latest_my_reply_dt
                        else None
                    ),
                    "myReplyPreview": (
                        _preview(latest_my_reply_text, 120)
                        if latest_my_reply_text
                        else ""
                    ),
                }
            )
