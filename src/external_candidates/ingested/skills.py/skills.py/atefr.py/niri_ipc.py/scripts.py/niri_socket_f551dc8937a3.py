# Extracted from: C:\DEV\PyAgent\.external\skills\skills\atefr\niri-ipc\scripts\niri_socket.py
#!/usr/bin/env python3
...
def socket_path() -> str:
    ...
    if not p:
        raise SystemExit(
            "NIRI_SOCKET is not set. Run inside your Niri session or export it."
        )
    return p
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="niri_socket.py", description="Direct Niri IPC socket client"
    )
    ...
    p_raw.add_argument(
        "json",
        help="Request JSON. Examples: "
        '"\\"FocusedWindow\\"" or "{\\"Request\\":...}"',
    )

    p_stdin = sub.add_parser(
        "stdin", help="Send one request per stdin line (each line is JSON)"
    )

    sub.add_parser(
        "event-stream",
        help="Request event stream and print JSON lines until interrupted",
    )
