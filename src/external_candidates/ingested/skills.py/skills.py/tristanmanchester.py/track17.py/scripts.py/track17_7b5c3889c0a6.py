# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tristanmanchester\track17\scripts\track17.py
#!/usr/bin/env python3
"""track17.py
...
def resolve_workspace_dir() -> pathlib.Path:
    ...
    env_override = os.environ.get("TRACK17_WORKSPACE_DIR") or os.environ.get(
...
def extract_latest_fields(track_info: Dict[str, Any]) -> Dict[str, Optional[str]]:
    ...
    evt_desc = latest_event.get("description_translation") or latest_event.get(
        "description"
    )
    ...
    if isinstance(evt_loc, dict):
        # best-effort
        evt_loc = (
            evt_loc.get("address") or evt_loc.get("city") or evt_loc.get("country")
        )
    ...
    conn.execute(
        ...
        (
            ...
            (
                ...
                else prev["tracking_status"]
            ),
            (
                str(package_status)
                if package_status is not None
                else prev["package_status"]
            ),
            ...
            conn.execute(
                ...
                (
                    ...
                    (
                        ...
                        else e.get("description_translation") or e.get("description")
                    ),
                    (
                        json.dumps(e.get("location"), ensure_ascii=False)
...
def _parse_gettrackinfo_response_items(
    resp: Dict[str, Any],
...
def cmd_add(args: argparse.Namespace) -> int:
    ...
    if int(row["carrier"]) != new_carrier:
        ...
        if existing:
            ...
            conn.execute(
                "UPDATE packages SET archived=1, updated_at=? WHERE id=?",
                (_utc_now_iso(), row["id"]),
            )
            conn.commit()
            row = existing
    ...
    if args.status:
        return cmd_status(
            argparse.Namespace(key=str(row["id"]), refresh=True, events=10, json=False)
        )
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    print(
        "#    Label                      Number             Carrier Status       Last event time       Last event"
    )
    print("-" * 110)
    for r in rows:
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    for i in range(0, len(items), CHUNK):
        ...
        for acc in accepted:
            ...
            try:
                raw_payload_sha = hashlib.sha256(
                    json.dumps(acc, ensure_ascii=False, sort_keys=True).encode("utf-8")
                ).hexdigest()
            except Exception:
                pass
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if args.refresh:
        ...
        if rejected:
            err = rejected[0].get("error") or {}
            raise Track17Error(
                f"Refresh failed: {err.get('code')} {err.get('message')}"
            )
        if accepted:
            acc = accepted[0]
            raw_payload_sha = hashlib.sha256(
            ...
            _, _ = apply_update_from_trackinfo(
                conn,
                package_row=pkg,
                response_item=acc,
                raw_payload_sha=raw_payload_sha,
...
def cmd_remove(args: argparse.Namespace) -> int:
    ...
    if args.delete_remote:
        ...
        if rejected:
            err = rejected[0].get("error") or {}
            raise Track17Error(
                f"Remote delete failed: {err.get('code')} {err.get('message')}"
            )
...
def cmd_ingest_webhook(args: argparse.Namespace) -> int:
    ...
    changed, summary = ingest_payload(
        conn, raw_body=raw_body, headers={}, source="webhook", secret=secret
    )
...
def cmd_process_inbox(args: argparse.Namespace) -> int:
    ...
    for f in files:
        ...
        try:
            changed, summary = ingest_payload(
                conn, raw_body=raw, headers=headers, source="webhook", secret=secret
            )
            if changed:
                changed_count += 1
    ...
    print(
        f"Processed {len(files)} inbox payload(s). Updated {changed_count} package(s)."
    )
...
class WebhookServer(http.server.ThreadingHTTPServer):
    ...
    def spool(self, raw_body: bytes, headers: Dict[str, str]) -> None:
        ...
        (self.inbox_dir / f"{fpath.stem}.headers.json").write_text(
            json.dumps(headers), "utf-8"
        )
...
def cmd_webhook_server(args: argparse.Namespace) -> int:
    ...
    httpd = WebhookServer(
        (bind, port), _WebhookHandler, inbox_dir=p["inbox"], verbose=args.verbose
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument(
        "--carrier", type=int, help="Carrier code (optional; 0 means auto-detect)"
    )
    ...
    s.add_argument(
        "--lang", help="Translation language code (default: TRACK17_LANG or 'en')"
    )
    s.add_argument(
        "--status", action="store_true", help="Fetch status immediately after adding"
    )
    ...
    s.add_argument(
        "--active-only", action="store_true", help="Only packages not marked as Stopped"
    )
    ...
    s.add_argument(
        "--refresh", action="store_true", help="Fetch latest status from 17TRACK"
    )
    s.add_argument(
        "--events", type=int, default=10, help="How many recent events to show"
    )
    ...
    s.add_argument(
        "--delete-remote",
        action="store_true",
        help="Also delete the tracking number at 17TRACK",
    )
    ...
    s = sub.add_parser(
        "carriers-update",
        help="Download carrier list cache (for looking up carrier codes)",
    )
    ...
    s = sub.add_parser(
        "ingest-webhook", help="Ingest a single webhook payload (stdin or --file)"
    )
    ...
    s = sub.add_parser(
        "process-inbox", help="Process all spooled payloads in the inbox directory"
    )
    s.add_argument("--secret", help="Webhook secret (or set TRACK17_WEBHOOK_SECRET)")
    s.set_defaults(fn=cmd_process_inbox)

    s = sub.add_parser(
        "webhook-server", help="Run a simple HTTP server to receive 17TRACK webhooks"
    )
    s.add_argument(
