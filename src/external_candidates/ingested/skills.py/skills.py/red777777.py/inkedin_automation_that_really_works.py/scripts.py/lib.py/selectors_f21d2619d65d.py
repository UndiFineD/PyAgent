...
def _try_strategies(page, strategies, timeout=10000, label="element"):
    ...
    raise SelectorFailed(
        f"Could not find {label} after trying {len(strategies)} strategies"
    )
...
def _click_typeahead_match(page, full_name):
    ...
    if result.get("clicked"):
        log.debug(
            f"[typeahead] Clicked match: {result.get('text')} via {result.get('strategy')}"
        )
        return True
    return False
...
def insert_mention(page, textbox, full_name, max_retries=2):
    ...
    for attempt in range(max_retries + 1):
        ...
        if not clicked and len(full_name.split()) > 1:
            ...
            log.debug(
                f"[mention] Not found with first name, typing last name progressively: '{last_name}'"
            )
            ...
            for i, char in enumerate(last_name):
                ...
                if not _wait_for_typeahead(page, timeout_ms=1500):
                    log.debug(
                        f"[mention] Dropdown disappeared after typing '{last_name[:i+1]}'"
                    )
                    continue
                ...
                if clicked:
                    log.info(
                        f"[mention] Found after typing '{last_name[:i+1]}' of last name"
                    )
                    break

        if not clicked:
            log.debug(
                f"[mention] No matching item for '{full_name}' in dropdown after progressive typing"
            )
            # Backspace everything and type as plain text
            typed_total = (
                len(trigger_text) + 1 + len(last_name)
                if len(full_name.split()) > 1
                else len(trigger_text)
            )
