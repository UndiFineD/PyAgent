...
STYLE_FILE = os.environ.get(
    "LINKEDIN_STYLE_FILE", os.path.expanduser("~/.linkedin-style.json")
)
    ...
    with browser_session() as (ctx, page):
        ...
        style = {
            ...
            "emoji_usage": (
                ...
                else "moderate" if emoji_density > 0.1 else "minimal"
            ),
            "tone": (
                "casual"
                if casual_markers > 3 or exclamation_count > len(all_content)
                else "professional" if casual_markers == 0 else "professional-friendly"
            ),
