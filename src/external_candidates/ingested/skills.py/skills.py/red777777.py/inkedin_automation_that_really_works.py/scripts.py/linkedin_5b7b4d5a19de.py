# Extracted from: C:\DEV\PyAgent\.external\skills\skills\red777777\inkedin-automation-that-really-works\scripts\linkedin.py
#!/usr/bin/env python3
...
from lib.style_learner import get_style, learn_profile
...
def main():
    ...
    p_del.add_argument(
        "--match", required=True, help="Text fragment to identify the comment"
    )
    ...
    p_analytics = sub.add_parser(
        "analytics", help="Get engagement stats for recent posts"
    )
    ...
    # scan-likes
    p_likes = sub.add_parser(
        "scan-likes", help="Scan recent likes for new ones since last check"
    )
    p_likes.add_argument("--count", type=int, default=15)

    # learn-profile
    p_learn = sub.add_parser(
        "learn-profile", help="Scan your posts/comments to learn your voice and topics"
    )
    ...
    if args.action == "check-session":
        ...
        result = get_style() or {
            "error": "No style learned yet. Run: linkedin.py learn-profile"
        }
    else:
        parser.print_help()
        sys.exit(1)
