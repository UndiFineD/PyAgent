# Extracted from: C:\DEV\PyAgent\.external\skills\skills\justinhartbiz\gedcom-explorer\scripts\build_explorer.py
#!/usr/bin/env python3
...
def parse_date(date_str):
    ...
    if m:
        day, mon, year = (
            int(m.group(1)),
            MONTH_MAP.get(m.group(2).upper(), 0),
            int(m.group(3)),
        )
...
def parse_gedcom(filepath):
    ...
    for indi in individuals.values():
        if indi["occupation"]:
            pres = parse_president_info(
                indi["occupation"], indi.get("occuDate"), indi.get("occuPlace")
            )
            if pres:
                indi["president"] = pres
    ...
    for indi in individuals.values():
        ...
        if indi["familyChild"] and indi["familyChild"] in families:
            ...
            if fam["husband"] and fam["husband"] in individuals:
                parents["father"] = {
                    "id": fam["husband"],
                    "name": individuals[fam["husband"]]["name"],
                }
            if fam["wife"] and fam["wife"] in individuals:
                parents["mother"] = {
                    "id": fam["wife"],
                    "name": individuals[fam["wife"]]["name"],
                }
        ...
        for fam_id in indi["familySpouse"]:
            ...
            for child_id in fam.get("children", []):
                if child_id in individuals:
                    children.append(
                        {"id": child_id, "name": individuals[child_id]["name"]}
                    )
...
def build_families_json(families, individuals):
    ...
    for fam in families.values():
        ...
        f = {
            ...
            "wife": fam["wife"],
            "husbandName": (
                individuals[fam["husband"]]["name"]
                if fam["husband"] and fam["husband"] in individuals
                else ""
            ),
            "wifeName": (
                individuals[fam["wife"]]["name"]
                if fam["wife"] and fam["wife"] in individuals
                else ""
            ),
...
def compute_stats(people, fam_list):
    ...
    presidents = sorted(
        [p for p in people if p["president"]], key=lambda x: x["president"]["number"]
    )
    ...
    return {
        ...
        "presidents": [
            {
                "name": p["name"],
                "number": p["president"]["number"],
                "party": p["president"]["party"],
