...
def run_all_tests(
    test_cases: list, category_filter: str = None, verbose: bool = False
) -> dict:
...
def format_results(results: dict, verbose: bool = False) -> str:
    ...
    lines.append(
        f"Exact Matches:  {results['exact_matches']} ({results['exact_rate']:.1f}%)"
    )
    ...
    for category, stats in sorted(results["by_category"].items()):
        ...
        lines.append(
            f"  {category:25} {stats['passed']:2}/{stats['total']:2} ({pct:5.1f}%)"
        )
    lines.append("")
    ...
    if results["failed_cases"]:
        ...
        for case in results["failed_cases"]:
            ...
            lines.append(
                f"     Expected: {case['expected_risk']}, Got: {case['actual_risk']} (score: {case['score']})"
            )
...
def main():
    ...
    parser.add_argument(
        "--test-file", "-f", default=None, help="Path to test cases JSON file"
    )
    ...
    results = run_all_tests(
        data["test_cases"], category_filter=args.category, verbose=args.verbose
    )
