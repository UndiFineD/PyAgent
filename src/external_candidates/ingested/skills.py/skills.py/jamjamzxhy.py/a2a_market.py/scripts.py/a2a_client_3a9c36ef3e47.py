# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jamjamzxhy\a2a-market\scripts\a2a_client.py
#!/usr/bin/env python3
"""
...
class A2AClient:
    ...
    def _check_budget(self, price: float) -> tuple[bool, str]:
        ...
        if price > self.rules.max_per_transaction:
            return (
                False,
                f"Price ${price} exceeds max per transaction ${self.rules.max_per_transaction}",
            )

        if self.daily_spent + price > self.rules.daily_budget:
            return (
                False,
                f"Would exceed daily budget (spent: ${self.daily_spent}, limit: ${self.rules.daily_budget})",
            )
    ...
    def purchase(
        ...
        if seller_rep < self.rules.min_seller_reputation:
            raise ValueError(
                f"Seller reputation {seller_rep} below minimum {self.rules.min_seller_reputation}"
            )
        ...
        if self._needs_confirmation(price):
            if confirm_callback:
                ...
                raise ValueError(
                    f"Purchase of ${price} requires confirmation (above ${self.rules.auto_approve_below})"
                )
        ...
        response = requests.post(
            f"{self.api_url}/v1/listings/{skill_id}/content", headers=headers
        )
        response.raise_for_status()
    ...
    def _sign_payment(self, payment_info: Dict, price: float) -> str:
        ...
        return json.dumps(
            {"payment": payment_data, "signature": signed.signature.hex()}
        )
    ...
    def list_skill(
        ...
        response = requests.post(
            f"{self.api_url}/v1/listings", headers=headers, data=body
        )
        response.raise_for_status()
    ...
    def get_earnings(self) -> Dict[str, Any]:
        ...
        response = requests.get(
            f"{self.api_url}/v1/account/{self.wallet_address}/earnings"
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_purchases(self) -> List[Dict[str, Any]]:
        ...
        response = requests.get(
            f"{self.api_url}/v1/account/{self.wallet_address}/purchases"
        )
        response.raise_for_status()
    ...
    def register(self, name: str) -> Dict[str, Any]:
        ...
        """
        body = json.dumps({"wallet_address": self.wallet_address, "name": name})
    ...
    def get_credits_balance(self) -> Dict[str, Any]:
        ...
        """
        response = requests.get(
            f"{self.api_url}/v1/credits/balance", headers=self._agent_headers()
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_daily_reward_status(self) -> Dict[str, Any]:
        ...
        """
        response = requests.get(
            f"{self.api_url}/v1/rewards/daily/status", headers=self._agent_headers()
        )
        response.raise_for_status()
        return response.json()
    ...
    def claim_daily_reward(self) -> Dict[str, Any]:
        ...
        """
        response = requests.post(
            f"{self.api_url}/v1/rewards/daily/claim", headers=self._agent_headers()
        )
        response.raise_for_status()
        return response.json()
...
if __name__ == "__main__":
    ...
    if status["available"]:
        ...
        print(
            f"  Claimed {reward['claimed']} credits! Balance: {reward['new_balance']}"
        )
    else:
    ...
    for s in skills[:5]:
        print(
            f"  [{s.id}] {s.name} - ${s.price} (rating:{s.rating}, rep:{s.reputation})"
        )
