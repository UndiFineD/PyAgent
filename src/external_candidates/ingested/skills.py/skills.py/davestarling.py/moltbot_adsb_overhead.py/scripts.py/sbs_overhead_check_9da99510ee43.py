# Extracted from: C:\DEV\PyAgent\.external\skills\skills\davestarling\moltbot-adsb-overhead\scripts\sbs_overhead_check.py
#!/usr/bin/env python3
...
def _planespotters_pick_photo_url_sized(data: dict, size: str) -> Optional[str]:
    ...
    p0 = photos[0]
    if size == "small":
        return (p0.get("thumbnail") or {}).get("src") or (
            p0.get("thumbnail_large") or {}
        ).get("src")
    return (p0.get("thumbnail_large") or {}).get("src") or (
        p0.get("thumbnail") or {}
    ).get("src")


def fetch_planespotters_photo_by_hex(
    hex_: str, size: str = "large", timeout_s: float = 3.5
) -> Optional[str]:
    h = hex_.strip().upper()
    if not h:
        return None
...
def download_image(
    url: str, dest: Path, timeout_s: float = 5.0, max_bytes: int = 3_000_000
) -> Optional[Path]:
    try:
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Detect overhead aircraft from an SBS/BaseStation feed"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=5.0,
        help="How long to listen for SBS messages",
    )
    ap.add_argument(
        "--cooldown-min", type=float, default=15.0, help="Per-aircraft alert cooldown"
    )
    ap.add_argument(
        "--state-file",
        default=str(Path.home() / ".clawdbot" / "adsb-overhead" / "state.json"),
    )
    ...
    ap.add_argument(
        "--photo", action="store_true", help="Enable Planespotters photo lookup"
    )
    ap.add_argument(
        "--photo-mode",
        ...
        while time.time() < end:
            ...
            while b"\n" in buf:
                ...
                if len(aircraft) > args.max_aircraft:
                    oldest = sorted(aircraft.values(), key=lambda x: x.last_seen_ts)[
                        : len(aircraft) // 10
                    ]
                    for o in oldest:
    ...
    for a in aircraft.values():
        ...
        fr24 = (
            f"https://www.flightradar24.com/{cs_for_url}"
            if cs_for_url and cs_for_url != "(nocallsign)"
            else ""
        )
        ...
        if args.photo:
            ...
            if (
                ...
                and (now - float(cached_ts) < args.photo_cache_hours * 3600)
            ):
                photo_url = cached_url
            else:
                photo_url = fetch_planespotters_photo_by_hex(
                    a.hex, size=args.photo_size
                )
                if photo_url:
