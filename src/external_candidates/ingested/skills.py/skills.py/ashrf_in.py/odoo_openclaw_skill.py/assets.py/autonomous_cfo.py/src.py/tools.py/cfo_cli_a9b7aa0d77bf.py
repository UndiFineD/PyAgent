# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ashrf-in\odoo-openclaw-skill\assets\autonomous-cfo\src\tools\cfo_cli.py
import argparse
import json
import os
import sys
...
def main():
    ...
    config_parser.add_argument(
        "--get", action="store_true", help="View current settings"
    )
    config_parser.add_argument(
        "--set",
        nargs=2,
        metavar=("KEY", "VALUE"),
        help="Set a config value (e.g., anomalies.petty_cash_limit 1000)",
    )
    ...
    trends_parser.add_argument(
        "--months", type=int, default=6, help="Months to analyze"
    )
    trends_parser.add_argument(
        "--visualize", action="store_true", help="Generate chart"
    )

    # Anomalies command
    anomalies_parser = subparsers.add_parser(
        "anomalies", help="Detect financial anomalies"
    )
    anomalies_parser.add_argument(
        "--ai", action="store_true", help="Use Gemini AI for advanced analysis"
    )
    ...
    if not all([url, db, user, pwd]):
        print(
            json.dumps({"error": "Missing Odoo credentials in environment variables"})
        )
        return
    ...
    if args.command == "config":
        ...
        date_from = args.date_from or (datetime.now() - timedelta(days=90)).strftime(
            "%Y-%m-%d"
        )
        ...
        if args.visualize:
            ...
            output_file = (
                f"output/revenue_vs_expense_{datetime.now().strftime('%Y%m%d')}.png"
            )
            chart_path = generate_revenue_vs_expense_chart(result, output_file)

        output = {"trends": result, "chart_path": chart_path}
