# Extracted from: C:\DEV\PyAgent\.external\skills\skills\henrik-openclaw\kalshi\scripts\kalshi_portfolio.py
#!/usr/bin/env python3
"""
...
def get_client() -> KalshiClient:
    ...
    key_path = Path(
        creds.get("private_key_path", "~/.kalshi/private_key.pem")
    ).expanduser()
...
def cmd_setup(args):
    ...
    creds = {"api_key_id": api_key_id, "private_key_path": key_path}
...
def main():
    ...
    p_orders.add_argument(
        "-s", "--status", default="resting", choices=["resting", "pending", "all"]
    )
