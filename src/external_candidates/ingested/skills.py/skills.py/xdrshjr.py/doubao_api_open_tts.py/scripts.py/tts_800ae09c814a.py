# Extracted from: C:\DEV\PyAgent\.external\skills\skills\xdrshjr\doubao-api-open-tts\scripts\tts.py
#!/usr/bin/env python3
"""
火山引擎语音合成 (TTS) 工具
...
def setup_api_config(app_id, access_token, secret_key, voice_type=None):
    ...
    with open(env_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"VOLCANO_TTS_ACCESS_TOKEN={existing_content['VOLCANO_TTS_ACCESS_TOKEN']}\n"
        )
        f.write(
            f"VOLCANO_TTS_SECRET_KEY={existing_content['VOLCANO_TTS_SECRET_KEY']}\n"
        )
        if voice_type or "VOLCANO_TTS_VOICE_TYPE" in existing_content:
            f.write(
                f"VOLCANO_TTS_VOICE_TYPE={existing_content.get('VOLCANO_TTS_VOICE_TYPE', voice_type)}\n"
            )
...
def get_voice_info(voice_type):
    ...
    return {
        ...
        "category": category,
        "category_display": (
            CATEGORY_DISPLAY_NAMES.get(category, category) if category else "Unknown"
        ),
    }
...
class VolcanoTTS:
    def __init__(
        self, app_id=None, access_token=None, secret_key=None, voice_type=None
    ):
        """
        初始化火山引擎TTS客户端
        ...
        self.voice_type = (
            voice_type or os.environ.get("VOLCANO_TTS_VOICE_TYPE") or DEFAULT_VOICE_TYPE
        )
    ...
    def synthesize(
        ...
        # 构建请求体 - V3 API格式
        body = {
            "app": {"appid": self.app_id, "token": "access_token", "cluster": cluster},
            "user": {"uid": "user_001"},
            "audio": {
                "voice_type": voice,
                "encoding": encoding,
        ...
        try:
            response = requests.post(
                TTS_ENDPOINT, headers=headers, json=body, timeout=30
            )
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"音色类型 (默认: {DEFAULT_VOICE_TYPE})",
    )
    ...
    parser.add_argument(
        "--category",
        help="按分类筛选音色 (通用场景-多情感/通用场景-普通/角色扮演/视频配音/有声阅读/多语种)",
    )
