# Extracted from: C:\DEV\PyAgent\.external\skills\skills\seojoonkim\prompt-guard\scripts\hivefence.py
#!/usr/bin/env python3
"""
...
class HiveFenceClient:
    ...
    def vote(
        self, pattern_id: str, approve: bool, voter_id: Optional[str] = None
    ) -> bool:
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", choices=["stats", "latest", "pending", "report", "vote"]
    )
    ...
    if args.command == "stats":
        ...
        if args.json:
            ...
            for p in patterns[:10]:
                print(
                    f"   [{p.severity}/5] {p.category}: {p.description or p.pattern_hash[:20]}"
                )
        ...
        if args.json:
            ...
            for p in patterns[:10]:
                print(
                    f"   {p.id[:8]}... [{p.category}] 👍{p.votes_up} 👎{p.votes_down}"
                )
