# Extracted from: C:\DEV\PyAgent\.external\skills\skills\araa47\ez-google\scripts\gmail.py
#!/usr/bin/env python3
# /// script
...
def batch_modify(
    ...
    for batch_num in range(total_batches):
        ...
        click.echo(
            f"Processing batch {batch_num + 1} of {total_batches}... ({len(batch_ids)} messages)"
        )
...
@click.option(
    "--remove", "-r", "remove_labels", multiple=True, help="Label IDs to remove"
)
@click.option("--max", "-n", "max_results", type=int, help="Max messages to process")
def bulk_label(
    query: str, add_labels: tuple, remove_labels: tuple, max_results: int | None
):
    ...
    processed = batch_modify(
        service, message_ids, list(add_labels) or None, list(remove_labels) or None
    )
