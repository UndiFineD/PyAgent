...
def process_inline_formatting(text: str) -> str:
    ...
    text = re.sub(
        r"`([^`]+)`", r'<font face="Courier" size="9" color="#c7254e">\1</font>', text
    )
    ...
    while i < len(lines):
        ...
        while i < len(lines):
            ...
            if current_stripped.startswith(">"):
                break
            # Stop at list
            if re.match(r"^[-*+]\s", current_stripped) or re.match(
                r"^\d+\.\s", current_stripped
            ):
                break
...
def create_pdf(
    markdown_file: str, output_pdf: Optional[str] = None, verbose: bool = False
):
    ...
    for block_type, content in blocks:
        try:
            if block_type in (
                ...
                code_para = Paragraph(
                    f'<font face="Courier" size="8">{code}</font>', styles["CodeBlock"]
                )
                ...
                header_paras = [
                    Paragraph(process_inline_formatting(h), styles["TableHeader"])
                    for h in headers
                ]
                row_paras = [
                    [
                        Paragraph(process_inline_formatting(cell), styles["TableCell"])
                        for cell in row
                    ]
                    for row in rows
                ]
                ...
                if full_img_path.exists():
                    try:
                        ...
                        if verbose:
                            print(
                                f"  Warning: Could not render image {img_path}: {img_e}"
                            )
                else:
                    if verbose:
...
""",
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Print detailed processing info"
    )
