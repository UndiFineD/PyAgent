# Extracted from: C:\DEV\PyAgent\.external\skills\skills\snowshadow\feishu-doc-reader\scripts\read_feishu_doc.py
#!/usr/bin/env python3
"""
飞书文档读取工具 - 优化版本
...
class FeishuDocReader:
        ...
        try:
            ...
            if data.get("code") != 0:
                ...
                if error_code == 94999:
                    ...
                    raise Exception(
                        f"应用权限不足，请检查应用是否已授权相应权限 (错误码: {error_code})"
                    )
                else:
            ...
            if block_type == "text":
                ...
                for element in text_elements:
                    if element.get("type") == "text_run":
                        ...
                        text_parts.append(
                            f"@{user_info.get('name', user_info.get('user_id', 'unknown'))}"
                        )
            elif block_type == "heading1":
                text_parts.append(
                    "# "
                    + self._extract_text_from_elements(
                        block.get("heading1", {}).get("elements", [])
                    )
                )
                ...
                text_parts.append(
                    "## "
                    + self._extract_text_from_elements(
                        block.get("heading2", {}).get("elements", [])
                    )
                )
                ...
                text_parts.append(
                    "### "
                    + self._extract_text_from_elements(
                        block.get("heading3", {}).get("elements", [])
                    )
                )
                ...
                text_parts.append(
                    "1. "
                    + self._extract_text_from_elements(
                        block.get("ordered", {}).get("elements", [])
                    )
                )
    ...
    def get_sheet_values(
        self, sheet_token: str, range_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """获取飞书表格的值"""
        if range_name:
...
def load_config():
    ...
    if not os.path.exists(config_file):
        raise Exception(
            f"配置文件不存在: ./reference/feishu_config.json (尝试了多个位置)"
        )
    ...
    try:
        ...
        if not app_id or not app_secret:
            raise Exception(
                f"配置文件 {config_file} 缺少必要的 app_id 或 app_secret 字段"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "  python read_feishu_doc.py sheet_xxxxxxxxxxxxxxx --type sheet --format simple"
        )
        sys.exit(1)
    ...
    try:
        ...
        if doc_type == "sheet":
            ...
            if output_format == "simple":
                ...
                result = {"type": "doc", "document": doc_info, "blocks": blocks}
