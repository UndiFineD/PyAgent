# Extracted from: C:\DEV\PyAgent\.external\skills\skills\snowshadow\feishu-doc-reader\scripts\get_feishu_doc_blocks.py
#!/usr/bin/env python3
"""
...
class FeishuDocBlocksReader:
    ...
    def get_document_info(self, doc_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("code") != 0:
                raise Exception(
                    f"获取文档信息失败: {data.get('msg', 'Unknown error')} (code: {data.get('code')})"
                )
    ...
    def get_document_blocks(self, doc_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("code") != 0:
                raise Exception(
                    f"获取文档blocks失败: {data.get('msg', 'Unknown error')} (code: {data.get('code')})"
                )

            blocks_data = data["data"]
            logger.info(
                f"成功获取文档blocks，共 {len(blocks_data.get('blocks', {}))} 个块"
            )
            return blocks_data
