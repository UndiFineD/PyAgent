...
API_KEY = os.environ.get(
    "CONTEXT7_API_KEY", "ctx7sk-d6069954-149e-4a74-ae8f-85092cbfcd6f"
)
...
def get_context(
    library_id: str, query: str, output_type: str = "txt", tokens: int = None
) -> str | dict:
...
def main():
    ...
    search_parser.add_argument(
        "library_name", help="Library name to search for (e.g., 'react', 'next.js')"
    )
    search_parser.add_argument(
        "--query", "-q", default="", help="Optional query to filter results"
    )

    # Context command
    context_parser = subparsers.add_parser("context", help="Get documentation context")
    context_parser.add_argument(
        "library_id", help="Library ID from search results (e.g., '/vercel/next.js')"
    )
    context_parser.add_argument(
        "query", help="Query describing what you need (e.g., 'setup ssr')"
    )
    context_parser.add_argument(
        "--type",
        "-t",
