# Extracted from: C:\DEV\PyAgent\.external\skills\skills\am-will\get-you-some-britches\scripts\aggregate_results.py
#!/usr/bin/env python3
"""
...
class PantsProduct:
    ...
    def calculate_discount(self) -> int:
        ...
        if self.discount_percent is None and self.price_original:
            discount = (
                (self.price_original - self.price_current) / self.price_original
            ) * 100
    ...
    def get_value_score(self, user_budget: float, premium_brands: List[str]) -> float:
        ...
        is_premium = any(
            brand.lower() in self.product_name.lower()
            or brand.lower() in self.store_name.lower()
...
def filter_and_rank(
    ...
    if user_size:
        filtered = [
            p for p in filtered if p.in_stock and user_size in p.sizes_available
        ]
...
def generate_recommendation_reason(product: PantsProduct, rank: int) -> str:
    ...
    if rank == 1:
        ...
        if any(
            brand in product.product_name.lower() for brand in ["bonobos", "levi's"]
        ):
        ...
        if (
            "stretch" in product.product_name.lower()
            or "flex" in product.product_name.lower()
        ):
...
def main():
    ...
    parser.add_argument(
        "--budget", "-b", type=float, required=True, help="Maximum budget in USD"
    )
    parser.add_argument(
        "--size", "-s", help="Required size (e.g., 32x30)", default=None
    )
    parser.add_argument(
        "--max-results",
        "-m",
    ...
    # Filter and rank
    ranked = filter_and_rank(products, args.budget, args.size, args.max_results)
