        ...
        if "W" in size_str and "L" in size_str:
            try:
                ...
                if w_idx < l_idx:
                    # 32W30L format
                    waist = int(
                        size_str[:w_idx] if w_idx > 0 else size_str[w_idx + 1 : l_idx]
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert pants sizes between US, EU, and UK"
    )
    parser.add_argument("size", help="Size to convert (e.g., 32, 32x30)")
    parser.add_argument(
        "--to", choices=["eu", "us", "uk"], required=True, help="Target sizing system"
    )
    parser.add_argument(
        "--from",
        dest="from_system",
