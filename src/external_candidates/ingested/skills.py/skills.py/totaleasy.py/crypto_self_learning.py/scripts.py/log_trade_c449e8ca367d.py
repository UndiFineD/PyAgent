...
def log_trade(args):
    ...
    trade = {
        ...
        "market_context": (
            json.loads(args.market_context) if args.market_context else {}
        ),
        "notes": args.notes,
    }
...
def list_trades(args):
    ...
    print(
        f"{'ID':<10} {'Date':<12} {'Symbol':<10} {'Dir':<6} {'PnL %':<8} {'Result':<6}"
    )
    print("-" * 60)
    ...
    for t in recent:
        ...
        print(
            f"{t['id']:<10} {dt:<12} {t['symbol']:<10} {t['direction']:<6} {t['pnl_percent']:+.2f}%   {emoji}"
        )
...
def main():
    ...
    parser.add_argument(
        "--result", choices=["WIN", "LOSS", "win", "loss"], help="Trade result"
    )
