# Extracted from: C:\DEV\PyAgent\.external\skills\skills\totaleasy\crypto-self-learning\scripts\analyze.py
#!/usr/bin/env python3
"""
...
def analyze_by_field(trades, field, min_trades=3):
    ...
    for value, group_trades in groups.items():
        if len(group_trades) >= min_trades:
            ...
            results.append(
                {"value": value, "win_rate": win_rate, "n": n, "avg_pnl": avg_pnl}
            )
...
def main():
    ...
    parser.add_argument(
        "--min-trades", type=int, default=3, help="Minimum trades for analysis"
    )
    ...
    if len(trades) < args.min_trades:
        print(
            f"⚠️  Not enough trades ({len(trades)}) for meaningful analysis. Need at least {args.min_trades}."
        )
        return
...
""")
    ...
    for stat in analyze_by_field(trades, "direction", args.min_trades):
        ...
        print(
            f"   {emoji} {stat['value']}: {stat['win_rate']:.0f}% win rate (n={stat['n']}, avg PnL: {stat['avg_pnl']:+.2f}%)"
        )
    ...
    if day_stats:
        ...
        for stat in day_stats:
            emoji = "✅" if stat["win_rate"] >= 50 else "❌"
            print(
                f"   {emoji} {stat['value'].title()}: {stat['win_rate']:.0f}% (n={stat['n']})"
            )
    ...
    if len(symbol_stats) > 1:
        ...
        for stat in symbol_stats:
            emoji = "✅" if stat["win_rate"] >= 50 else "❌"
            print(
                f"   {emoji} {stat['value']}: {stat['win_rate']:.0f}% (n={stat['n']})"
            )
