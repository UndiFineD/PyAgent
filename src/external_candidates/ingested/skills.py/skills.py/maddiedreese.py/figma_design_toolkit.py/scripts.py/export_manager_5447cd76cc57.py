# Extracted from: C:\DEV\PyAgent\.external\skills\skills\maddiedreese\figma-design-toolkit\scripts\export_manager.py
#!/usr/bin/env python3
"""
...
class ExportManager:
    ...
    def export_frames(
        self, file_key: str, frame_ids: List[str] = None, frame_names: List[str] = None
    ) -> Dict[str, Any]:
    ...
    def export_components(
        self, file_key: str, component_names: List[str] = None
    ) -> Dict[str, Any]:
        ...
        if component_names:
            component_nodes = [
                c for c in component_nodes if c["name"] in component_names
            ]
    ...
    def export_pages(
        self, file_key: str, page_names: List[str] = None
    ) -> Dict[str, Any]:
    ...
    def create_client_package(self, file_key: str, package_name: str = None) -> str:
        ...
        if not package_name:
            file_data = self.client.get_file(file_key)
            package_name = (
                file_data.get("name", "figma-package").replace(" ", "-").lower()
            )
    ...
    def _export_nodes(
        self, file_key: str, nodes: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for node in nodes:
            ...
            for format in self.config.formats:
                for scale in self.config.scales:
                    current_export += 1
                    print(
                        f"Exporting {current_export}/{total_exports}: {node_name} ({format} @ {scale}x)"
                    )
                    ...
                    try:
                        ...
                        if "images" in export_data and node_id in export_data["images"]:
                            ...
                            if image_url:
                                ...
                                print(
                                    f"  Warning: No image URL returned for {node_name}"
                                )
        ...
        return {"exported": len(exported_files), "files": exported_files}
    ...
    def _create_package_documentation(
        self, file_key: str, doc_path: Path, results: Dict[str, Any]
    ):
...
def main():
    ...
    parser.add_argument(
        "node_ids", nargs="?", help="Comma-separated node IDs for export-nodes"
    )
    parser.add_argument(
        "--formats", default="png", help="Export formats (comma-separated)"
    )
    parser.add_argument(
        "--scales", default="1.0", help="Export scales (comma-separated)"
    )
    parser.add_argument(
        "--output-dir", default="./figma-exports", help="Output directory"
    )
    parser.add_argument(
        "--token-format", default="json", choices=["json", "css", "scss", "js"]
    )
    parser.add_argument("--package-name", help="Name for client package")
    parser.add_argument(
        "--frame-names", help="Specific frame names to export (comma-separated)"
    )
    parser.add_argument(
        "--component-names", help="Specific component names to export (comma-separated)"
    )
    ...
    try:
        ...
        if args.command == "export-frames":
            ...
            component_names = (
                args.component_names.split(",") if args.component_names else None
            )
            result = manager.export_components(
                file_key, component_names=component_names
            )
