# Extracted from: C:\DEV\PyAgent\.external\skills\skills\maddiedreese\figma-design-toolkit\scripts\figma_client.py
#!/usr/bin/env python3
"""
...
class FigmaClient:
    ...
    def __init__(self, access_token: str = None):
        ...
        if not self.config.access_token:
            raise ValueError(
                "Figma access token required. Set FIGMA_ACCESS_TOKEN env var or pass token."
            )
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise
                print(
                    f"Request failed, retrying {attempt + 1}/{self.config.max_retries}: {e}"
                )
    ...
    def get_file_nodes(
        self, file_key: str, node_ids: Union[str, List[str]], **params
    ) -> Dict[str, Any]:
    ...
    try:
        ...
        if args.command == "get-file":
            ...
            if not args.file_key or not args.node_ids:
                parser.error(
                    "file_key and --node-ids required for export-images command"
                )

            file_key = client.parse_file_url(args.file_key)
            result = client.export_images(
                file_key, args.node_ids.split(","), format=args.format, scale=args.scale
            )
