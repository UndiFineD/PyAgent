# Extracted from: C:\DEV\PyAgent\.external\skills\skills\maddiedreese\figma-design-toolkit\scripts\style_auditor.py
#!/usr/bin/env python3
"""
...
class StyleAuditor:
    ...
    def audit_multiple_files(self, file_keys: List[str]) -> Dict[str, Any]:
        ...
        for file_key in file_keys:
            try:
                ...
                print(
                    f"✓ Audited {result['file_name']}: {len(result['issues'])} issues"
                )
    ...
    def _audit_accessibility(self, file_data: Dict[str, Any]):
        ...
        def audit_node_accessibility(node):
            ...
            # Check touch targets
            if (
                node_type in ["COMPONENT", "INSTANCE", "FRAME"]
                and "button" in node_name.lower()
            ):
    ...
    def _check_touch_target_size(self, node: Dict[str, Any]):
        ...
        if (
            width < self.config.min_touch_target
            or height < self.config.min_touch_target
        ):
            self.issues.append(
                AuditIssue(
                    severity="warning",
    ...
    def _check_focus_indicators(self, node: Dict[str, Any]):
        ...
        has_focus_effect = any(
            effect.get("type") == "DROP_SHADOW" and "focus" in str(effect).lower()
                    ...
                    if hex_color not in self.config.brand_colors:
                        ...
                        self.issues.append(
                            AuditIssue(
                                ...
                                details={
                                    "used_color": hex_color,
                                    "suggested_color": closest_brand_color,
                                },
                            )
                        )
        ...
        if font_family and font_family not in self.config.brand_fonts:
            self.issues.append(
                AuditIssue(
                    ...
                    details={
                        "used_font": font_family,
                        "brand_fonts": self.config.brand_fonts,
                    },
                )
            )
                ...
                if len(unique_colors) > 1:
                    self.issues.append(
                        AuditIssue(
                            ...
                            details={
                                "similar_colors": list(unique_colors),
    ...
    def _calculate_contrast_ratio(
        self, color1: Dict[str, float], color2: Dict[str, float]
    ) -> float:
    ...
    def _colors_are_similar(
        self, color1: str, color2: str, tolerance: int = 30
    ) -> bool:
    ...
    def _find_closest_brand_color(self, hex_color: str) -> str:
        ...
        for brand_color in self.config.brand_colors:
            if self._colors_are_similar(
                hex_color, brand_color, tolerance=255
            ):  # Use max tolerance for distance calc
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "by_category": {
                "accessibility": len(
                    [i for i in self.issues if i.category == "accessibility"]
                ),
                "brand": len([i for i in self.issues if i.category == "brand"]),
                "consistency": len(
                    [i for i in self.issues if i.category == "consistency"]
                ),
            },
        }
        ...
        penalty = (
            summary["by_severity"]["error"] * error_penalty
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if error_count > 0:
            recommendations.append(
                f"Fix {error_count} critical accessibility issues immediately"
            )
        ...
        if not recommendations:
            recommendations.append(
                "Great work! Consider periodic design system reviews"
            )
    ...
    def _analyze_cross_file_patterns(
        ...
        most_common = sorted(common_issues.items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
        ...
        return {
            ...
            "files_with_errors": len(
                [
                    ...
                    if "error" not in r and r["summary"]["by_severity"]["error"] > 0
                ]
            ),
        }
    ...
    def generate_report(
        self, audit_results: Dict[str, Any], output_path: str = None
    ) -> str:
...
def main():
    ...
    parser.add_argument(
        "command", choices=["audit-file", "audit-multiple", "audit-brand"]
    )
    ...
    parser.add_argument(
        "--brand-colors", help="Comma-separated list of brand hex colors"
    )
    parser.add_argument("--brand-fonts", help="Comma-separated list of brand fonts")
    parser.add_argument(
        "--min-contrast", type=float, default=4.5, help="Minimum contrast ratio"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.generate_html and args.command == "audit-file":
            html_path = (
                args.output.replace(".json", ".html")
                if args.output
                else "audit-report.html"
            )
