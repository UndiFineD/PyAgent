...
<p>{html.escape(context)}</p>"""
    ...
    def format_list(self, text):
        ...
        if len(items) > 1:
            ...
            for item in items[:5]:  # Max 5 items for optimal length
                ...
                html_parts.append(
                    f"<li><strong>{html.escape(clean_item)}</strong></li>"
                )
            html_parts.append("</ul>")
    ...
    def extract_intro_text(self, text):
        ...
        first_item_match = re.search(
            r"(^\d+[.)]|^\*|^\-|First|Here are)", text, re.MULTILINE | re.IGNORECASE
        )
        if first_item_match:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Structures: auto, punchy-wisdom, micro-story, list, contrarian, relatable, poetic"
        )
