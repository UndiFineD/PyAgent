...
def generate_cgv(args):
    ...
    mediateur = (
        args.mediateur
        or "Médiateur de la consommation — consulter la liste officielle sur economie.gouv.fr"
    )
    tribunal = (
        args.tribunal or address.split(",")[-1].strip()
        if address != "Non renseignée"
        else "du siège du prestataire"
    )

    tva_mention = (
        "TVA non applicable, article 293 B du Code général des impôts." if micro else ""
    )
...
def generate_contrat(args):
  ...
  <div style="display: flex; gap: 30px; margin-top: 40px;">
    ...
    return html_wrap(
        f"Contrat — {pname} / {client_name}", body
...
def generate_devis(args):
    ...
    validite = (
        datetime.strptime(devis_date, "%Y-%m-%d") + timedelta(days=30)
    ).strftime("%d/%m/%Y")
...
def cmd_config(args):
    ...
    if not config:
        ...
        print(
            "Créez ~/.freelance/config.json ou utilisez le freelance-toolkit : python3 config.py set ..."
        )
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="Générateur de documents juridiques pour freelances"
    )
    ...
    p_gen.add_argument(
        "--nom", default=None, help="Nom du prestataire (override config)"
    )
