# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hugosbl\freelance-toolkit-fr\scripts\dashboard.py
#!/usr/bin/env python3
...
def cmd_summary(args):
    ...
    relevant_seconds = sum(
        _get_duration(e) for e in entries if e["start"][:7] in months_with_ca
    )
    ...
    if args.json:
        result = {
            ...
            "ca_by_client": {
                k: round(v, 2)
                for k, v in sorted(ca_by_client.items(), key=lambda x: -x[1])
            },
    ...
    print(
        f"📄 Factures : {nb_invoices} ({len(paid_invoices)} payée(s), {len(unpaid_invoices)} en attente)"
    )
    if unpaid_total > 0:
    ...
    if ca_by_month:
        ...
        for month, ca in sorted(ca_by_month.items()):
            bar = (
                "█" * max(1, int(ca / max(ca_by_month.values()) * 20)) if ca > 0 else ""
            )
...
def cmd_monthly(args):
    ...
    if args.json:
        print(
            json.dumps(
                {"year": year, "months": months_data}, ensure_ascii=False, indent=2
            )
        )
        return
    ...
    for m in range(1, 13):
        ...
        if d:
            ...
            rate_str = (
                f"{d['effective_rate']:.0f}€/h" if d["effective_rate"] > 0 else "—"
            )
            print(
                f"{month_key:<10} {d['ca']:>11,.2f}€ {d['invoices']:>10} {d['hours']:>9.1f}h {rate_str:>12}"
            )
        else:
    ...
    print(
        f"{'TOTAL':<10} {total_ca:>11,.2f}€ {total_inv:>10} {total_hours:>9.1f}h {eff_rate:>11.0f}€/h"
    )
...
def main():
    ...
    p_sum.add_argument(
        "--year", type=int, default=None, help="Année (défaut: courante)"
    )
