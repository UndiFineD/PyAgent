# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hugosbl\freelance-toolkit-fr\scripts\invoice.py
#!/usr/bin/env python3
...
def cmd_generate(args):
    ...
    due_date = (
        datetime.strptime(inv_date, "%Y-%m-%d") + timedelta(days=due_days)
    ).strftime("%Y-%m-%d")
...
def cmd_list(args):
    ...
    for inv in invoices:
        ...
        print(
            f"{inv['number']:<12} {inv['date']:<12} {inv['client']:<25} {format_euro(inv['total_ttc']):<14} {statut}"
        )
...
def cmd_paid(args):
    ...
    if args.json:
        print(
            json.dumps(
                {"number": args.number, "paid": True, "paid_date": data["paid_date"]},
                ensure_ascii=False,
                indent=2,
            )
        )
...
def main():
    ...
    p_gen.add_argument(
        "--items", nargs="+", required=True, help="Items (desc:qté:prix)"
    )
    ...
    p_gen.add_argument(
        "--due-days", type=int, default=None, help="Délai de paiement en jours"
    )
    p_gen.add_argument(
        "--no-open", action="store_true", help="Ne pas ouvrir dans le navigateur"
    )
