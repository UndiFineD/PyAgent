# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hugosbl\freelance-toolkit-fr\scripts\timetrack.py
#!/usr/bin/env python3
...
def cmd_start(args):
    ...
    if args.json:
        ...
        print(
            f"▶ Timer démarré pour « {args.project} »{client_info} à {datetime.now().strftime('%H:%M')}"
        )
...
def cmd_log(args):
    ...
    for e in entries:
        ...
        total += dur
        print(
            f"{start.strftime('%Y-%m-%d'):<12} {e['project']:<30} {start.strftime('%H:%M'):<6} {end.strftime('%H:%M'):<6} {format_duration(dur):<8}"
        )
    print("─" * 62)
...
def main():
    ...
    p_log.add_argument(
        "--from", dest="from_date", default=None, help="Date début (YYYY-MM-DD)"
    )
