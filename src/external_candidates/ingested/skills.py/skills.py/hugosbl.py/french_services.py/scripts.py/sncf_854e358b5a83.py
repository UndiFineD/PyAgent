...
def cmd_search(args):
    ...
    for i, j in enumerate(journeys, 1):
        ...
        if nb_transfers > 0:
            print(
                f"     🔄 {nb_transfers} correspondance{'s' if nb_transfers > 1 else ''}"
            )
...
def main():
    ...
    p_search.add_argument(
        "--date", help="Date (YYYY-MM-DD, DD/MM/YYYY, demain, après-demain)"
    )
    p_search.add_argument("--time", help="Heure de départ (HH:MM)")
    p_search.add_argument(
        "--count", type=int, default=5, help="Nombre de résultats (défaut: 5)"
    )
