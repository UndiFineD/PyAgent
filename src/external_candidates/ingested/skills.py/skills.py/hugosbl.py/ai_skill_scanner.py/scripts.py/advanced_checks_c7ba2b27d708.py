...
def find_string_concatenation_evasion(content):  # noscan
    ...
    concat_pat = re.compile(
        r"""['"]([a-zA-Z]{2,8})['""]\s*(?:\+|\.\s*concat)\s*['"]([a-zA-Z/:]{2,8})['"]"""
    )  # noscan
    bad_pairs = [  # noscan
        ("htt", "://"),
        ("web", "hoo"),
...
def decode_and_check_payloads(content):  # noscan
    findings = []  # noscan
    b64_pat = re.compile(
        r"""(?:atob|Buffer\.from|b64decode|decode)\s*\(\s*['"]([A-Za-z0-9+/=]{20,})['"]"""
    )  # noscan
    hex_pat = re.compile(
        r"""(?:Buffer\.from|fromhex|unhexlify)\s*\(\s*['"]([0-9a-fA-F]{20,})['"]"""
    )  # noscan
    ...
    for line_num, line in _lines(content):  # noscan
        ...
        for m in hex_pat.finditer(line):  # noscan
            try:  # noscan
                decoded = binascii.unhexlify(m.group(1)).decode(
                    "utf-8", errors="replace"
                )  # noscan
                if DANGER_DECODED.search(decoded):  # noscan
                    findings.append(
...
def analyze_python_ast(filepath):  # noscan
    ...
    for node in ast.walk(tree):  # noscan
        if isinstance(node, ast.Import):  # noscan
            ...
            if any(
                fn.startswith(m) for m in ("requests.", "urllib.", "http.", "socket.")
            ):  # noscan
...
_AUTH_VAR_PATTERNS = re.compile(
    r"(?:api[_-]?key|token|auth|secret|password|credential)", re.IGNORECASE
)  # noscan
...
def _is_legitimate_api_auth(call_node, tree):  # noscan
    ...
    has_auth_header = any(  # noscan
        s.lower() in _AUTH_HEADER_KEYS for s in all_strings  # noscan  # noscan
    )  # noscan
    has_auth_var = any(  # noscan
        _AUTH_VAR_PATTERNS.search(s) for s in all_strings  # noscan  # noscan
    )  # noscan
...
def _is_network_func(fn_name):  # noscan
    ...
    if fn_name in _NETWORK_FUNCS:  # noscan
        return True  # noscan
    if any(
        fn_name.startswith(p) for p in ("requests.", "urllib.", "http.", "socket.")
    ):  # noscan
        return True  # noscan
    return False  # noscan
...
def check_steganography(skill_path):  # noscan
    ...
    for root, dirs, files in os.walk(skill_path):  # noscan
        ...
        for fname in files:  # noscan
            ...
            if (
                expected_type and actual_type is None and _is_mostly_text(head)
            ):  # noscan
    ...
    for legit in LEGIT_DOMAINS:  # noscan
        ...
        if len(base) >= 2 and len(legit_base) >= 2:  # noscan
            ...
            if 0 < d <= 2 and len(base[-2]) > 3:  # noscan
                return legit  # noscan
    if re.search(
        r"(?:api|cdn|auth|login|oauth|sso|account)[.-]", domain_lower
    ):  # noscan
        for legit in LEGIT_DOMAINS:  # noscan
            parts = legit.split(".")  # noscan
            if (
                len(parts) >= 2 and parts[-2] in domain_lower and domain_lower != legit
            ):  # noscan
...
def check_network_destinations(content, filepath):  # noscan
    ...
    for line_num, line in _lines(content):  # noscan
        ...
        for m in IP_RE.finditer(line):  # noscan
            ...
            if (
                ip == "127.0.0.1" or ip == "0.0.0.0" or ip == "255.255.255.255"
            ):  # noscan
                is_safe = True  # noscan
            if (
