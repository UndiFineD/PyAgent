...
def format_text(result: ScanResult, verbose: bool = False) -> str:
    ...
    # Score
    score = result.score
    emoji = {"DANGEROUS": "🚨", "SUSPICIOUS": "⚠️", "REVIEW": "🔎", "INFO": "ℹ️"}.get(
        score, "❓"
    )
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show matched lines"
    )
    parser.add_argument(
        "--basic", action="store_true", help="Skip advanced checks (faster)"
    )
    parser.add_argument(
        "--exclude", action="append", default=[], help="Exclude file patterns"
    )
