    ...
    if "organic_results" not in data and "shopping_results" not in data:
        ...
        if "products" in data:
            ...
            for r in data.get("products", [])[:10]:
                ...
                if r.get("rating"):
                    lines.append(
                        f"  Rating: {r['rating']} ({r.get('reviews', 0)} reviews)"
                    )
...
def main():
    ...
    parser.add_argument(
        "engine", choices=list(ENGINES.keys()), help="Search engine to use"
    )
    ...
    parser.add_argument(
        "--type", "-t", help="Search type for Google (shop, isch, nws, vid)"
    )
    parser.add_argument(
        "--format",
        "-f",
