...
def parse_run_path(
    path: str, default_project: str = None, default_entity: str = None
) -> tuple[str, str, str]:
...
def print_comparison(run_a, run_b, history_a: list, history_b: list, config_diff: dict):
    ...
    loss_a = [
        get_metric(r, "train/loss", "loss")
        for r in history_a
        if get_metric(r, "train/loss", "loss")
    ]
    loss_b = [
        get_metric(r, "train/loss", "loss")
        for r in history_b
    ...
    if loss_a and loss_b:
        ...
        # Starting loss
        winner = (
            "A" if loss_a[0] < loss_b[0] else "B" if loss_b[0] < loss_a[0] else "Tie"
        )
        print(
            f"   {'Start loss':<20} {loss_a[0]:<20.4f} {loss_b[0]:<20.4f} {winner:<10}"
        )
        ...
        winner = (
            ...
            else "B ✓" if loss_b[-1] < loss_a[-1] else "Tie"
        )
        print(
            f"   {'Current loss':<20} {loss_a[-1]:<20.4f} {loss_b[-1]:<20.4f} {winner:<10}"
        )
...
def main():
    ...
    # Parse run paths
    try:
        entity_a, project_a, run_id_a = parse_run_path(
            args.run_a, args.project, args.entity
        )
        entity_b, project_b, run_id_b = parse_run_path(
            args.run_b, args.project, args.entity
        )
    ...
    # Build paths
    path_a = (
        f"{entity_a}/{project_a}/{run_id_a}" if entity_a else f"{project_a}/{run_id_a}"
    )
    path_b = (
        f"{entity_b}/{project_b}/{run_id_b}" if entity_b else f"{project_b}/{run_id_b}"
    )
