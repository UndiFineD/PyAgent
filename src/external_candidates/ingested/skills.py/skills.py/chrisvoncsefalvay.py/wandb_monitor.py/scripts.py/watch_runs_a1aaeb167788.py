# Extracted from: C:\DEV\PyAgent\.external\skills\skills\chrisvoncsefalvay\wandb-monitor\scripts\watch_runs.py
#!/usr/bin/env python3
"""
...
def quick_health_check(run) -> dict:
    ...
    if grad is not None:
        ...
        if grad > 10:
            ...
            result["status"] = (
                "critical" if result["status"] != "critical" else result["status"]
            )
        elif grad > 5:
...
def get_recent_finished(
    api, entity: str, projects: Optional[list[str]] = None, hours: int = 24
) -> list:
...
def print_report(running: list, recent: list, entity: str):
    ...
    if not running:
        ...
        for run in running:
            ...
            print(
                f"   {emoji} {health['project']}/{health['name']}{progress}{loss_str}"
            )
            print(
                f"      Runtime: {health['runtime_hours']:.1f}h | ID: {health['run_id']}"
            )
...
def main():
    ...
    parser.add_argument(
        "entity",
        nargs="?",
        default="chrisvoncsefalvay",
        help="W&B entity (username/org)",
    )
    parser.add_argument("--projects", "-p", help="Comma-separated project names")
    parser.add_argument(
        "--all-projects", "-a", action="store_true", help="Check all projects"
    )
    parser.add_argument(
        "--hours", type=int, default=24, help="Hours to look back for finished runs"
    )
    ...
    if args.json:
        ...
        output = {
            ...
            "recent_failed": [
                {"id": r.id, "name": r.name, "project": r.project, "state": r.state}
                for r in recent
