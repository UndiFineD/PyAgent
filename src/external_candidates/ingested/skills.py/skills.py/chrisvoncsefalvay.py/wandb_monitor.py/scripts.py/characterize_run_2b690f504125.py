...
def analyze_gradients(history: list[dict]) -> dict:
    ...
    if max(grads) > 10:
        ...
        result["health"] = "spiky"
        result["health_msg"] = (
            f"⚠️ SPIKY - max grad norm {max(grads):.2f}, possible instability"
        )
...
def analyze_evals(history: list[dict]) -> dict:
    ...
    for row in history:
        eval_loss = get_metric(
            row, "eval/loss", "eval_loss", "validation_loss", "val_loss"
        )
        eval_acc = get_metric(
            row, "eval/accuracy", "eval_accuracy", "eval/acc", "accuracy"
        )
...
def print_report(
    run, loss: dict, grads: dict, evals: dict, stall: dict, progress: dict
):
    ...
    if "epoch" in progress:
        ...
        if "total_epochs" in progress:
            epoch_str += (
                f" / {progress['total_epochs']} ({progress['epoch_progress_pct']:.1f}%)"
            )
        print(f"   {epoch_str}")
    if "step" in progress:
        ...
        if "max_steps" in progress:
            step_str += (
                f" / {progress['max_steps']} ({progress['step_progress_pct']:.1f}%)"
            )
        print(f"   {step_str}")
    ...
    if evals["status"] == "no_data":
        ...
        if "loss" in evals:
            el = evals["loss"]
            print(
                f"   Eval Loss: {el['current']:.4f} (best: {el['best']:.4f}, n={el['count']})"
            )
        if "accuracy" in evals:
            ea = evals["accuracy"]
            print(
                f"   Eval Acc: {ea['current']:.4f} (best: {ea['best']:.4f}, n={ea['count']})"
            )
...
def main():
    ...
    parser.add_argument(
        "run_path", help="Run path: ENTITY/PROJECT/RUN_ID or PROJECT/RUN_ID or RUN_ID"
    )
    ...
    if not project:
        print(
            "Project required. Use ENTITY/PROJECT/RUN_ID or --project", file=sys.stderr
        )
        sys.exit(1)
