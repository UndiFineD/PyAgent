# Extracted from: C:\DEV\PyAgent\.external\skills\skills\fontstep\send-email\send_email.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
def send_email(to_email, subject, content, attachment_path=None):
    ...
    if not all([SMTP_SERVER, SMTP_PORT, SENDER_EMAIL, AUTHORIZATION_CODE]):
        print(
            "❌ 错误：请配置环境变量 EMAIL_SMTP_SERVER, EMAIL_SMTP_PORT, EMAIL_SENDER, EMAIL_SMTP_PASSWORD（在 ~/.openclaw/openclaw.json skills.entries.send-email.env）"
        )
        return False
