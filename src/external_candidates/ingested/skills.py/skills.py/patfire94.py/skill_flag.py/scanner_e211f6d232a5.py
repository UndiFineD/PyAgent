...
class SkillReport:
    ...
    def calculate_risk(self):
        ...
        self.risk_score = min(
            100, total_risk // max(1, len(self.findings)) + len(self.findings) * 5
        )
...
class SecurityScanner:
    ...
    def load_patterns(self):
        ...
        for yaml_file in patterns_dir.glob("*.yaml"):
            try:
                ...
                print(
                    f"{Colors.ORANGE}Warning: Could not load {yaml_file}: {e}{Colors.END}"
                )
    ...
    def print_report(self, reports: list):
            ...
            if report.findings:
                ...
                for f in report.findings[:10]:  # Limit to first 10
                    risk_color = (
                        Colors.RED
                        if f.risk >= 80
                        else Colors.ORANGE if f.risk >= 60 else Colors.YELLOW
                    )
                    print(
                        f"    {risk_color}• [{f.category}] {f.pattern_name}{Colors.END}"
                    )
        ...
        if critical > 0:
            ...
            print(
                f"{Colors.RED}   {critical} skill(s) have CRITICAL security issues.{Colors.END}"
            )
            print(
                f"{Colors.RED}   Review immediately and consider removing them.{Colors.END}"
            )
        elif high > 0:
            print(f"\n{Colors.ORANGE}{Colors.BOLD}⚠️  REVIEW RECOMMENDED:{Colors.END}")
            print(
                f"{Colors.ORANGE}   {high} skill(s) have HIGH risk findings.{Colors.END}"
            )
            print(
                f"{Colors.ORANGE}   Manual review recommended before continued use.{Colors.END}"
            )
        else:
            print(
...
def main():
    ...
    parser.add_argument(
        "--all", "-a", action="store_true", help="Scan all installed skills"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--json", "-j", help="Export report as JSON to specified file")
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Only show critical findings"
    )
