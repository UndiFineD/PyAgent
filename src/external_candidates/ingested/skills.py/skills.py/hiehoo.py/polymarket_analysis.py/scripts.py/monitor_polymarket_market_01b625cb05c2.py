# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hiehoo\polymarket-analysis\scripts\monitor-polymarket-market.py
#!/usr/bin/env python3
"""
...
def find_market_by_slug(slug: str) -> dict | None:
    ...
    if data and isinstance(data, list):
        for market in data:
            if (
                slug in str(market.get("slug", ""))
                or slug in str(market.get("question", "")).lower()
            ):
                return market
    return None
...
def main():
    ...
    if args.json:
        ...
        print(
            f"**Prices:** YES ${yes_price:.3f} | NO ${no_price:.3f} | Pair: ${current['pair_cost']:.3f}"
        )
        print(
            f"**Volume:** ${current['volume']:,.0f} | **Liquidity:** ${current['liquidity']:,.0f}"
        )
        if current["end_date"]:
