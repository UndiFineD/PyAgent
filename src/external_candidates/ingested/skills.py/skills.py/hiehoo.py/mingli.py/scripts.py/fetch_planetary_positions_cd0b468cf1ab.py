# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hiehoo\mingli\scripts\fetch-planetary-positions.py
#!/usr/bin/env python3
"""
...
def build_output(today_pos, yesterday_pos):
    ...
    if "moon" in today_pos and "sun" in today_pos:
        ...
        moon_info = {
            "phase": phase,
            "illumination": round(abs(180 - elong) / 180, 2),
            "sign": sign,
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch planetary positions for horoscope"
    )
    ...
    if not auth:
        print(
            json.dumps(
                {
                    "error": "Missing ASTRONOMY_APP_ID or ASTRONOMY_APP_SECRET",
                    "fallback": True,
                }
            )
        )
    ...
    if not today_pos:
        print(
            json.dumps(
                {
                    "error": "No planetary data retrieved",
                    "fallback": True,
                    "date": today,
                }
            )
