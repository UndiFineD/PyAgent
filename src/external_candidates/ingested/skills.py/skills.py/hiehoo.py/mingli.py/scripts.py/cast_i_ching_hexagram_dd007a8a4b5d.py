...
def build_spark(primary, transformed, moving_positions):
    ...
    return {
        ...
        "problem": (
            f"Moving lines at {moving_positions}"
            if moving_positions
            else "No moving lines — stable"
        ),
        "advantage": primary.get("meaning", ""),
        "action": (
            f"Transition toward {transformed['keyword']}"
            if transformed
            else "Maintain current course"
        ),
...
def main():
    ...
    parser.add_argument(
        "--moving", help="Moving line positions, comma-separated (e.g., 2,1)"
    )
    ...
    if args.mode == "manual":
        if not args.upper or not args.lower:
            print(
                json.dumps(
                    {"error": "Manual mode requires --upper and --lower trigrams"}
                )
            )
            sys.exit(1)
        ...
        if upper not in TRIGRAMS or lower not in TRIGRAMS:
            print(
                json.dumps(
                    {"error": f"Unknown trigram. Valid: {list(TRIGRAMS.keys())}"}
                )
            )
            sys.exit(1)
        ...
        moving_positions = (
            [int(x) for x in args.moving.split(",")] if args.moving else []
        )
    else:
