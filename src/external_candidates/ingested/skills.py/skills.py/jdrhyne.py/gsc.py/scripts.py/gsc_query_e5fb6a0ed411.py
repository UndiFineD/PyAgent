# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jdrhyne\gsc\scripts\gsc_query.py
#!/usr/bin/env python3
"""
...
def top_queries(site_url: str, days: int = 28, limit: int = 20):
    ...
    response = search_analytics(
        site_url, days=days, dimensions=["query"], row_limit=limit
    )
    ...
    print(
        f"{'Query':<50} {'Clicks':>8} {'Impressions':>12} {'CTR':>8} {'Position':>10}"
    )
    print("-" * 92)
    ...
    for row in rows:
        ...
        print(
            f"{query:<50} {clicks:>8} {impressions:>12} {ctr:>7.1f}% {position:>10.1f}"
        )
...
def top_pages(site_url: str, days: int = 28, limit: int = 20):
    ...
    response = search_analytics(
        site_url, days=days, dimensions=["page"], row_limit=limit
    )
    rows = response.get("rows", [])
    ...
    for row in rows:
        ...
        position = row.get("position", 0)
        print(
            f"{page:<70} {clicks:>8} {impressions:>12} {ctr:>7.1f}% {position:>10.1f}"
        )
...
def query_page_analysis(site_url: str, days: int = 28, limit: int = 50):
    ...
    response = search_analytics(
        site_url, days=days, dimensions=["query", "page"], row_limit=limit
    )
    rows = response.get("rows", [])
...
def low_ctr_opportunities(site_url: str, days: int = 28, min_impressions: int = 100):
    ...
    response = search_analytics(
        site_url, days=days, dimensions=["query", "page"], row_limit=500
    )
    rows = response.get("rows", [])
...
def main():
    ...
    sa_parser = subparsers.add_parser(
        "search-analytics", help="Raw search analytics query"
    )
    sa_parser.add_argument(
        "--site", required=True, help="Site URL (e.g., https://example.com)"
    )
    sa_parser.add_argument(
        "--days", type=int, default=28, help="Number of days (default: 28)"
    )
    sa_parser.add_argument(
        "--dimensions",
        nargs="+",
    ...
    opp_parser = subparsers.add_parser(
        "opportunities", help="Find low-CTR optimization opportunities"
    )
    ...
    opp_parser.add_argument(
        "--min-impressions", type=int, default=100, help="Minimum impressions"
    )
    ...
    qp_parser = subparsers.add_parser(
        "query-page", help="Query-page combinations (JSON)"
    )
