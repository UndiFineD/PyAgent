# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alopez3006\snipara-mcp\src\snipara_mcp\server.py
#!/usr/bin/env python3
"""
...
def _load_auth() -> tuple[str | None, str, str | None]:
    ...
    ignore_oauth = os.environ.get("SNIPARA_IGNORE_OAUTH", "").lower() in (
        "true",
        "1",
        "yes",
    )
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Complex question to plan for",
                    },
                    "strategy": {
                        "type": "string",
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "summary": {
                        "type": "string",
                        "description": "Summary text to store",
                    },
                    "summary_type": {
                        "type": "string",
                        "enum": [
...
""",
            inputSchema={
                ...
                "properties": {
                    ...
                    "config": {
                        "type": "object",
                        "description": "Optional swarm configuration",
                    },
                },
                "required": ["name"],
            },
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "resource_type": {
                        "type": "string",
                        "description": "Resource type (alternative to claim_id)",
                    },
                    "resource_id": {
                        "type": "string",
                        "description": "Resource ID (alternative to claim_id)",
                    },
                },
                "required": ["swarm_id", "agent_id"],
            },
...
""",
            inputSchema={
                ...
                "properties": {
                    ...
                    "value": {
                        "description": "Value to set (any JSON-serializable type)"
                    },
                    "expected_version": {
                        "type": "integer",
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "event_type": {
                        "type": "string",
                        "description": "Event type (e.g., 'task_completed', 'error')",
                    },
...
""",
            inputSchema={
                ...
                "properties": {
                    ...
                    "task_id": {
                        "type": "string",
                        "description": "Specific task to claim (optional)",
                    },
                    "timeout_seconds": {
                        "type": "integer",
                        "default": 600,
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    try:
        if name == "rlm_context_query":
            ...
            max_tokens = arguments.get("max_tokens") or settings.get(
                "maxTokensPerQuery", 4000
            )
            search_mode = arguments.get("search_mode") or settings.get(
                "searchMode", "hybrid"
            )
            ...
            if result.get("success"):
                ...
                return [
                    TextContent(type="text", text="No relevant documentation found.")
                ]
            return [
                TextContent(
                    type="text",
            ...
            if result.get("success"):
                ...
                return [
                    TextContent(
                        type="text", text=f"No matches for '{arguments['pattern']}'"
                    )
                ]
            return [
                TextContent(
            ...
            if _session_context:
                return [
                    TextContent(
                        type="text", text=f"**Session Context:**\n\n{_session_context}"
                    )
                ]
            return [
                TextContent(
                    type="text", text="No session context. Use rlm_inject to set."
                )
            ]
            ...
            if result.get("success"):
                ...
                for s in sections:
                    lines.append(
                        f"- {s.get('path', '')} ({s.get('chunk_count', 0)} chunks)"
                    )
                return [TextContent(type="text", text="\n".join(lines))]
            return [
                TextContent(
            ...
            settings = await get_project_settings()
            cache_age = (
                int(time.time() - _settings_cache_time) if _settings_cache_time else 0
            )
            lines = [
        ...
        # Phase 7: Shared Context
            ...
            if result.get("success"):
                ...
                return [
                    TextContent(
                        type="text", text="No shared context linked to this project."
                    )
                ]
            return [
                TextContent(
            ...
            if result.get("success"):
                ...
                if template:
                    ...
                    lines.append("\n**Rendered Prompt:**\n```")
                    lines.append(
                        data.get("rendered_prompt", template.get("prompt", ""))
                    )
                    lines.append("```")
                    return [TextContent(type="text", text="\n".join(lines))]
        ...
        # Phase 8.2: Agent Memory Handlers
            ...
            if result.get("success"):
                ...
                if memories:
                    ...
                    for m in memories:
                        ...
                        lines.append(
                            f"  *Relevance: {rel:.2f} | Confidence: {conf:.2f} | Accessed: {m.get('access_count', 0)}x*\n"
                        )
                    return [TextContent(type="text", text="\n".join(lines))]
            ...
            if result.get("success"):
                ...
                if memories:
                    ...
                    for m in memories:
                        lines.append(
                            f"- **[{m.get('type', '')}]** {m.get('content', '')[:100]}..."
                        )
                        lines.append(
                            f"  *ID: {m.get('memory_id', '')} | Category: {m.get('category', 'none')}*"
                        )
        ...
        # Phase 9.1: Multi-Agent Swarm Handlers
            ...
            if result.get("success"):
                ...
                if data.get("found"):
                    ...
                    value_str = (
                        json_mod.dumps(data.get("value"), indent=2)
                        if isinstance(data.get("value"), (dict, list))
                        else str(data.get("value"))
                    )
                    return [
                        TextContent(
                            type="text",
                ...
                return [
                    TextContent(type="text", text=f"Key '{arguments['key']}' not found")
                ]
            return [
                TextContent(
                    type="text",
            ...
            if result.get("success"):
                data = result.get("result", {})
                redis_status = (
                    "✓ real-time" if data.get("redis_published") else "✗ persisted only"
                )
                return [
                    TextContent(
                        type="text",
            ...
            if result.get("success"):
                data = result.get("result", {})
                deps = (
                    f" (depends on: {data.get('depends_on', [])})"
        ...
        return [
            TextContent(
                type="text", text=f"**Connection Error:** Cannot reach {API_URL}"
            )
        ]
    except Exception as e:
        return [
            TextContent(type="text", text=f"**Error:** {type(e).__name__}: {str(e)}")
        ]
...
async def run_server():
    ...
    if not _auth_token and not API_KEY:
        ...
        print(
            "  1. Run 'snipara-mcp-login' to authenticate via browser (recommended)",
            file=sys.stderr,
        )
