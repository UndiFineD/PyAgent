# Extracted from: C:\DEV\PyAgent\.external\skills\skills\owenrao\seedream-image-gen\scripts\generate_image.py
#!/usr/bin/env python3
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate images using Seedream API (doubao-seedream-4.5)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--input-image", "-i", help="Input image URL for image-to-image"
    )
    ...
    if not api_key:
        ...
        print(
            '  skills: { entries: { "seedream-image-gen": { apiKey: "YOUR_KEY" } } }',
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        if "error" in first_image:
            ...
            raise Exception(
                f"Image generation error: {error.get('message', 'Unknown error')}"
            )
