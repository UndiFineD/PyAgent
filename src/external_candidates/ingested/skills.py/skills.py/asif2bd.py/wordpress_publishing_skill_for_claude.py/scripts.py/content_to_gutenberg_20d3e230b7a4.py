...
def parse_list_items(
    lines: List[str], start_idx: int, list_type: str
) -> Tuple[List[str], int]:
    """
...
def markdown_to_gutenberg(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if line.strip().startswith(">"):
            ...
            quote_content = convert_inline_formatting(
                escape_html(" ".join(quote_lines))
            )
            blocks.append(
                f"<!-- wp:quote -->\n"
...
def html_to_gutenberg(content: str) -> str:
    ...
    for part in parts:
        ...
        # Headings
        h_match = re.match(
            r"<h([1-6])[^>]*>(.*?)</h\1>", part, re.DOTALL | re.IGNORECASE
        )
        if h_match:
        ...
        table_match = re.match(
            r"<table[^>]*>(.*?)</table>", part, re.DOTALL | re.IGNORECASE
        )
        if table_match:
            blocks.append(
                f"<!-- wp:table -->\n"
        ...
        # Blockquote
        bq_match = re.match(
            r"<blockquote[^>]*>(.*?)</blockquote>", part, re.DOTALL | re.IGNORECASE
        )
        if bq_match:
            blocks.append(
                f"<!-- wp:quote -->\n"
        ...
        # Figure with image
        fig_match = re.match(
            r"<figure[^>]*>(.*?)</figure>", part, re.DOTALL | re.IGNORECASE
        )
...
def validate_gutenberg_blocks(content: str) -> List[str]:
    ...
    if len(opening_blocks) != len(closing_blocks):
        issues.append(
            f"Unbalanced blocks: {len(opening_blocks)} opening, {len(closing_blocks)} closing"
        )
    ...
    for i, (opening, closing) in enumerate(zip(opening_blocks, closing_blocks)):
        if opening != closing:
            issues.append(
                f"Block mismatch at position {i}: opening '{opening}', closing '{closing}'"
            )
...
def create_table_block(
    ...
    thead = (
        "<thead><tr>"
        + "".join(f"<th>{escape_cell(h)}</th>" for h in headers)
        + "</tr></thead>"
    )
    ...
    caption_html = (
        f'<figcaption class="wp-element-caption">{escape_cell(caption)}</figcaption>'
        if caption
        else ""
    )
...
def create_image_block(
    ...
    figure_classes = " ".join(
        filter(None, ["wp-block-image", f"size-{size}", align_class])
    )

    caption_html = (
        f'<figcaption class="wp-element-caption">{escape_html(caption)}</figcaption>'
        if caption
        else ""
    )
...
def create_button_block(
    text: str, url: str, style: str = "fill", align: str = "center"
) -> str:
    """
...
Examples:
  ...
  Validate existing Gutenberg content:
    ...
    parser.add_argument(
        "--type", "-T", choices=["markdown", "html"], help="Force content type"
    )
    parser.add_argument(
        "--validate",
        "-v",
