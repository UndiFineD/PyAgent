...
def fetch_json(url, headers=None, timeout=20):
    req = urllib.request.Request(
        url, headers=headers or {"User-Agent": "agent-relay-digest/1.1"}
    )
    try:
    ...
    if submolts:
        for s in submolts:
            params = urllib.parse.urlencode(
                {"submolt": s, "sort": sort, "limit": limit}
            )
...
def render_digest(
    ...
    avg_conf = (
        round(sum(confidence_score(p) for p in top_threads) / len(top_threads), 2)
        if top_threads
        else 0
    )
    ...
    lines.append(
        f"- digest_id={now} generated_at={generated_at} items={len(top_threads)} avg_confidence={avg_conf}"
    )
    for p in top_threads:
...
def main():
    ...
    ap.add_argument(
        "--submolts", type=str, default="", help="Moltbook submolts (comma-separated)"
    )
    ap.add_argument(
        "--sources",
        type=str,
    ...
    ap.add_argument(
        "--yclawker-sort", type=str, default="top", help="yclawker sort: top|new|best"
    )
    ...
    if "moltbook" in sources:
        posts.extend(
            fetch_moltbook(limit=args.limit, submolts=submolts, sort=args.moltbook_sort)
        )
