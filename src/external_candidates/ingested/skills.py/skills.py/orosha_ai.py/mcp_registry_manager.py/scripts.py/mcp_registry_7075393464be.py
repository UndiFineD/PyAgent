...
# Configuration
DB_PATH = (
    Path.home() / ".openclaw" / "workspace" / "mcp-registry-manager" / "registry.db"
)
...
class MCPServer:
    ...
    def __init__(
        self, name: str, url: str, description: str = "", category: str = "other"
    ):
        self.name = name
        self.url = url
...
class MCPRegistry:
    ...
    def quality_score(self, server: MCPServer) -> float:
        ...
        if "updated" in server.metadata and server.metadata["updated"]:
            updated = datetime.fromisoformat(
                server.metadata["updated"].replace("Z", "+00:00")
            )
    ...
    def list_servers(
        self, installed_only: bool = False, sort_by: str = "quality"
    ) -> List[Dict]:
...
def main():
    ...
    parser.add_argument(
        "--installed", action="store_true", help="List only installed servers"
    )
    parser.add_argument(
        "--sort",
        type=str,
    ...
    parser.add_argument(
        "--db", type=str, default=str(DB_PATH), help="Registry database path"
    )
    ...
    try:
        ...
        if args.score:
            ...
            cursor.execute(
                "SELECT name, quality_score FROM servers WHERE name=?", (args.score,)
            )
            row = cursor.fetchone()
            if row:
                name, quality = row
        ...
        if args.list:
            servers = registry.list_servers(
                installed_only=args.installed, sort_by=args.sort
            )
            print(f"\n📋 MCP Servers ({len(servers)} total):\n")
            for server in servers:
                status = "[INSTALLED]" if server["installed"] else ""
                print(
                    f"• {server['name']} (Quality: {server['quality_score']}) {status}"
                )
