...
def score_response_relevance(content):
    ...
    confirmations = count_markers(
        content, r"confirmed|correct|yes that works|thanks that helped"
    )
...
def score_initiative(content):
    ...
    proactive = count_markers(
        content, r"i started|i proposed|i initiated|without being asked"
    )
    if proactive == 0:
...
def generate_report(daily_content, long_content):
    ...
    scores[3], details["Memory Consistency"] = score_memory_consistency(
        daily_content, long_content
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Agentic Compass — AI Agent Self-Reflection Tool"
    )
    parser.add_argument(
        "--write", action="store_true", help="Write plan to memory/agentic-compass.md"
    )
    parser.add_argument(
        "--daily",
        type=str,
