# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dennis-da-menace\agent-memory\src\memory.py
"""
...
class AgentMemory:
    ...
    def list_facts(
        self, tags: List[str] = None, limit: int = 50, include_superseded: bool = False
    ) -> List[Fact]:
    ...
    def supersede(self, old_fact_id: str, new_content: str, **kwargs) -> str:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE facts SET superseded_by = ? WHERE id = ?", (new_id, old_fact_id)
        )
        conn.commit()
        conn.close()
    ...
    def get_lessons(
        self, context: str = None, outcome: str = None, limit: int = 10
    ) -> List[Lesson]:
        """
    ...
    def track_entity(
        self, name: str, entity_type: str, attributes: Dict[str, Any] = None
    ) -> str:
        """
    ...
    def link_fact_to_entity(self, entity_name: str, fact_id: str):
        ...
        cursor.execute(
            "SELECT id, fact_ids FROM entities WHERE name = ?", (entity_name,)
        )
        row = cursor.fetchone()
