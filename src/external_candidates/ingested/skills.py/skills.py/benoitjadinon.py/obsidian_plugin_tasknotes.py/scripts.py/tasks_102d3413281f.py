# Extracted from: C:\DEV\PyAgent\.external\skills\skills\benoitjadinon\obsidian-plugin-tasknotes\scripts\tasks.py
...
def api_request(method: str, endpoint: str, params: dict = None, data: dict = None):
    ...
    try:
        ...
        return {
            "error": "Cannot connect to TaskNotes API. Is Obsidian running with TaskNotes enabled?"
        }
...
def main():
    ...
    list_parser.add_argument(
        "--table", action="store_true", help="Human-readable output"
    )
    ...
    create_parser.add_argument(
        "--priority", help="Task priority (check your TaskNotes config for values)"
    )
    create_parser.add_argument(
        "--status", help="Task status (check your TaskNotes config for values)"
    )
    create_parser.add_argument("--due", help="Due date (YYYY-MM-DD)")
    create_parser.add_argument(
        "--scheduled", help="Scheduled date/time (YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS)"
    )
    create_parser.add_argument("--contexts", help="Comma-separated contexts")
    create_parser.add_argument(
        "--time-estimate", type=int, help="Time estimate in minutes"
    )
    create_parser.add_argument(
        "--details", help="Task description/body content (agent handoff context)"
    )
    create_parser.add_argument(
        "--table", action="store_true", help="Human-readable output"
    )
    ...
    update_parser.add_argument(
        "task_id", help="Task file path (e.g., Tasks/my-task.md)"
    )
    update_parser.add_argument(
        "--status", help="Task status (check your TaskNotes config for values)"
    )
