...
class AgentRecoveryRequest(BaseModel):
    ...
    recovery_phrase: str = Field(
        ..., min_length=10, description="BIP39 recovery phrase"
    )
...
class MemoryRetrieveRequest(BaseModel):
    ...
    agent_id: str
    signature: str = Field(
        ..., description="Ed25519 signature of 'retrieve' + timestamp"
    )
    ...
    try:
        ...
        try:
            ...
            if stored_public != public_bytes:
                raise HTTPException(
                    status_code=400, detail="Key mismatch. Invalid recovery phrase."
                )
...
async def store_memory(request: MemoryStoreRequest):
    ...
    try:
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT public_key FROM agents WHERE agent_id = ?", (request.agent_id,)
            )
            result = cur.fetchone()

            if not result:
                raise HTTPException(
                    status_code=404, detail="Agent not found. Register first."
                )
        ...
        logger.info(
            f"Memory stored for agent {request.agent_id[:16]}... version {version}"
        )
...
async def retrieve_memory(request: MemoryRetrieveRequest):
    ...
    try:
        ...
        try:
            ...
            if not result:
                raise HTTPException(
                    status_code=404, detail="No memory found for this agent."
                )
        finally:
            conn.close()
...
async def list_memory_history(request: MemoryRetrieveRequest):
    ...
    try:
        ...
        return MemoryListResponse(
            agent_id=request.agent_id, memories=memories, count=len(memories)
        )
...
async def clear_memory(request: MemoryRetrieveRequest):
    ...
    try:
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "DELETE FROM memory_snapshots WHERE agent_id = ?", (request.agent_id,)
            )
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "detail": (
                str(exc) if os.environ.get("DEBUG") else "An unexpected error occurred"
            ),
            "timestamp": datetime.utcnow().isoformat(),
        },
    )
