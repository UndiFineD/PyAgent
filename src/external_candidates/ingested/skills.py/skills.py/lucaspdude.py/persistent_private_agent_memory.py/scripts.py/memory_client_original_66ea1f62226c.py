# Extracted from: C:\DEV\PyAgent\.external\skills\skills\lucaspdude\persistent-private-agent-memory\scripts\memory_client_original.py
#!/usr/bin/env python3
"""
...
def recover_agent(recovery_phrase):
    ...
    response = requests.post(
        f"{DEFAULT_URL}/agents/recover", json={"recovery_phrase": recovery_phrase}
    )
    data = response.json()
...
def store_memory(file_path=None):
    ...
    if file_path:
        ...
        print(
            "Edit it and run: memory_client.py store --file ~/.agent-memory/memory_template.json"
        )
        return
