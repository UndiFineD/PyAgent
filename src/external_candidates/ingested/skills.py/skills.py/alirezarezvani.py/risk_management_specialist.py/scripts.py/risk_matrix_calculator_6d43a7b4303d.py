...
def display_risk_matrix():
    ...
    print("\n" + "-" * 70)
    print(
        "Risk Levels: Low (Acceptable) | Medium (ALARP) | High (ALARP) | Unacceptable"
    )
    print("=" * 70)
...
def format_result_text(result: dict, analysis_type: str) -> str:
    ...
    if analysis_type == "risk":
        ...
        lines.append(f"\n{'-' * 50}")
        lines.append(
            f"RPN: {result['rpn']} / {result['max_rpn']} ({result['rpn_percentage']}%)"
        )
...
Examples:
  ...
  python risk_matrix_calculator.py -p 4 -s 3 --output json
    ...
    parser.add_argument(
        "-p", "--probability", type=int, help="Probability rating (1-5)"
    )
    parser.add_argument(
        "-s",
        "--severity",
    ...
    parser.add_argument(
        "-o", "--occurrence", type=int, help="FMEA occurrence rating (1-10)"
    )
    parser.add_argument(
        "-d", "--detection", type=int, help="FMEA detection rating (1-10)"
    )
    ...
    parser.add_argument(
        "--show-matrix", action="store_true", help="Display risk matrix"
    )
    parser.add_argument(
        "--list-criteria",
        action="store_true",
