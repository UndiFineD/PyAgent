...
class AuditScheduleOptimizer:
    ...
    def _generate_recommendations(self, prioritized: List) -> List[str]:
        ...
        high_risk_count = sum(
            1 for p, _, _ in prioritized if p.risk_level == RiskLevel.HIGH
        )
        ...
        finding_processes = [
            (p.name, p.previous_findings)
            for p, _, _ in prioritized
            if p.previous_findings >= 3
        ]
        ...
        if not recommendations:
            recommendations.append(
                "Audit program is on track. Maintain scheduled frequency."
            )
...
def main():
    ...
    parser.add_argument(
        "--processes", type=str, help="JSON file with process definitions"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--months", type=int, default=12, help="Planning horizon in months"
    )
