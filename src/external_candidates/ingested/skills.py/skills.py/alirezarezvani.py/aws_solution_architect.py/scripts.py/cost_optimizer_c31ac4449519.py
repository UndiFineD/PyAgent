...
class CostOptimizer:
    ...
    def _analyze_compute(self) -> float:
        ...
        if ec2_instances:
            ...
            idle_count = sum(
                1 for inst in ec2_instances if inst.get("cpu_utilization", 100) < 10
            )
            if idle_count > 0:
            ...
            on_demand_count = sum(
                ...
                if inst.get("pricing", "on-demand") == "on-demand"
            )
        ...
        if lambda_functions:
            oversized = sum(
                ...
                if fn.get("memory_mb", 128) > 512
                and fn.get("avg_memory_used_mb", 0) < 256
            )
            if oversized > 0:
    ...
    def _analyze_storage(self) -> float:
        ...
        for bucket in s3_buckets:
            ...
            if not bucket.get("has_lifecycle_policy", False) and size_gb > 100:
                lifecycle_savings = (
                    size_gb * 0.015
                )  # $0.015/GB savings with IA transition
    ...
    def _analyze_database(self) -> float:
        ...
        for db in rds_instances:
            ...
            if (
                db.get("engine", "").startswith("aurora")
                and db.get("utilization", 100) < 30
            ):
    ...
    def _analyze_networking(self) -> float:
        ...
        if not self.resources.get("vpc_endpoints", []):
            ...
            if s3_data_transfer > 100:
                endpoint_savings = (
                    s3_data_transfer * 0.09 * 0.5
                )  # Save 50% of data transfer costs
    ...
    def _analyze_general_optimizations(self) -> float:
        ...
        for log in log_groups:
            if log.get("retention_days", 1) == -1:  # Never expire
                ...
                )  # 70% savings with 7-day retention
