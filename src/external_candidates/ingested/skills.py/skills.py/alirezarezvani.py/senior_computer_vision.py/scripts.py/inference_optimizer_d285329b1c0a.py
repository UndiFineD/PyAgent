...
class InferenceOptimizer:
    ...
    def benchmark(
        ...
        try:
            if self.model_format == "onnx":
                results.update(
                    self._benchmark_onnx(
                        input_size, batch_sizes, num_iterations, warmup
                    )
                )
            elif self.model_format == "pytorch":
                results.update(
                    self._benchmark_pytorch(
                        input_size, batch_sizes, num_iterations, warmup
                    )
                )
            else:
    ...
    def print_summary(self):
        ...
        if self.benchmark_results and "batch_results" in self.benchmark_results:
            ...
            print()
            print(
                f"{'Batch':<8} {'Latency (ms)':<15} {'Throughput (FPS)':<18} {'P99 (ms)':<12}"
            )
            print("-" * 55)
...
def main():
    ...
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze model structure"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--recommend", action="store_true", help="Show optimization recommendations"
    )
