...
class VisionModelTrainer:
    ...
    def __init__(
        self, data_dir: str, task: str = "detection", framework: str = "ultralytics"
    ):
    ...
    def generate_detectron2_config(
        self, arch: str, epochs: int = 12, batch: int = 16, **kwargs
    ) -> Dict[str, Any]:
    ...
    def generate_mmdetection_config(
        self, arch: str, epochs: int = 12, batch: int = 16, **kwargs
    ) -> Dict[str, Any]:
        ...
        config = {
            ...
            "model": {
                "roi_head": {"bbox_head": {"num_classes": len(self._get_classes())}}
            },
            "data": {
...
def main():
    ...
    trainer = VisionModelTrainer(
        data_dir=args.data_dir, task=args.task, framework=args.framework
    )
    ...
    try:
        if args.framework == "ultralytics":
            ...
            config = trainer.generate_detectron2_config(
                arch=args.arch, epochs=args.epochs, batch=args.batch
            )
