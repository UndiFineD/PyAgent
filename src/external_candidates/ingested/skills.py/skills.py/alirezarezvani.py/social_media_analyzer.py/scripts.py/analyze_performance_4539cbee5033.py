...
class PerformanceAnalyzer:
    ...
    def _identify_weaknesses(self) -> List[str]:
        ...
        if engagement_rate < 0.5:
            weaknesses.append(
                "Low engagement rate - content may not resonate with audience"
            )

        roi = self.roi_metrics.get("roi_percentage", 0)
        if roi < 50:
            weaknesses.append(
                "ROI below target - need to improve conversion or reduce costs"
            )

        cpc = self.roi_metrics.get("cost_per_click", 0)
        if cpc > 2.00:
            weaknesses.append(
                "High cost per click - targeting or bidding needs optimization"
            )
