...
class SocialMediaMetricsCalculator:
    ...
    def calculate_campaign_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_engagement_rate": self.safe_divide(total_engagements, total_reach)
            * 100,
    ...
    def calculate_roi_metrics(self) -> Dict[str, float]:
        ...
        roi_percentage = (
            self.safe_divide(total_value - self.total_spend, self.total_spend) * 100
        )
    ...
    def identify_top_posts(
        self, metric: str = "engagement_rate", limit: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        if metric == "engagement_rate":
            sorted_posts = sorted(
                posts_with_metrics, key=lambda x: x["engagement_rate"], reverse=True
            )
        else:
            sorted_posts = sorted(
                posts_with_metrics, key=lambda x: x.get(metric, 0), reverse=True
            )
