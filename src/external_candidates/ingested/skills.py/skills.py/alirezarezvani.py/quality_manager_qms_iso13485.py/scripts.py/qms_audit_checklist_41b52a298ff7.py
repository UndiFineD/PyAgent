    ...
    for clause, data in sorted(ISO13485_CLAUSES.items()):
        for q in data["questions"]:
            all_questions.append(
                {"clause": clause, "clause_title": data["title"], "question": q}
            )
...
def format_checklist_text(checklist: dict) -> str:
    ...
    lines.append("")
    lines.append(
        "Legend: C=Conforming, NC=Nonconforming, OBS=Observation, N/A=Not Applicable"
    )
    lines.append("=" * 70)
...
Examples:
  ...
  python qms_audit_checklist.py --interactive
    ...
    parser.add_argument(
        "--list-clauses", action="store_true", help="List all ISO 13485 clauses"
    )
    parser.add_argument(
    ...
    if args.list_clauses:
        if args.output == "json":
            ...
            for clause, data in sorted(ISO13485_CLAUSES.items()):
                print(
                    f"  {clause}: {data['title']} ({len(data['questions'])} questions)"
                )
        return
