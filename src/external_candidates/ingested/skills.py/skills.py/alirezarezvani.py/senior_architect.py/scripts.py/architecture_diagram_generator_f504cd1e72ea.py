...
class ProjectScanner:
    ...
    def _analyze_directory(self, dir_path: Path) -> Dict:
        ...
        code_files = [
            ...
            if f.is_file()
            and f.suffix
            in [".py", ".js", ".ts", ".jsx", ".tsx", ".go", ".rs", ".java", ".vue"]
        ]
...
Output formats:
  ...
  ascii       - ASCII art format
    ...
    parser.add_argument(
        "--json", action="store_true", help="Output raw scan results as JSON"
    )
    ...
    if args.verbose:
        ...
        print(
            f"Technologies: {', '.join(scan_result['technologies']) or 'none detected'}"
        )
