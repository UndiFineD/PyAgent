...
class DependencyAnalyzer:
    ...
    def __init__(self, project_path: Path, verbose: bool = False):
        ...
        self.internal_modules: Dict[str, Set[str]] = defaultdict(
            set
        )  # module -> imports
    ...
    def _parse_npm(self):
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"Found {len(self.direct_deps)} direct deps, "
        ...
        for module, imports in self.internal_modules.items():
            for imp in imports:
                ...
                for internal_module in modules:
                    if (
                        internal_module.lower() in imp.lower()
                        and internal_module != module
                    ):
    ...
    def _calculate_coupling_score(self):
        ...
        max_connections = (
            total_modules * (total_modules - 1) if total_modules > 1 else 1
        )
        ...
        self.coupling_score = min(
            100, self.coupling_score + len(self.circular_deps) * 10
        )
