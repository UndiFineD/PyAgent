# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\senior-architect\scripts\project_architect.py
#!/usr/bin/env python3
"""
Project Architect
...
class PatternDetector:
    ...
    def _detect_pattern(self):
        ...
        if best_pattern[1]["weight"] > 3:
            ...
            max_possible = (
                len(best_pattern[1]["structure"]) * 2
                + len(best_pattern[1]["indicators"]) * 2
            )
            self.confidence = min(
...
class LayerViolationDetector:
    ...
    def _extract_imports(self, content: str) -> List[str]:
        ...
        # JS/TS imports
        imports.extend(
            re.findall(r'(?:import|require)\s*\(?[\'"]([^\'"\s]+)[\'"]', content)
        )
...
Detects:
  ...
  - Missing architectural components
    ...
    if args.check == "pattern":
        arch = report["architecture"]
        print(
            f"Pattern: {arch['detected_pattern']} (confidence: {arch['confidence']}%)"
        )
        sys.exit(0)
