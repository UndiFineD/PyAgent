# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\senior-backend\scripts\database_migration_tool.py
#!/usr/bin/env python3
"""
        ...
        for part in parts:
            ...
            # Regular column definition
                ...
                if col_match:
                    ...
                    constraints = (
                        col_match.group(3).upper() if col_match.group(3) else ""
                    )
        ...
        for col_name in table.columns:
            if any(col_name.endswith(pattern) for pattern in self.FK_COLUMN_PATTERNS):
                if col_name not in indexed_columns:
                    ...
                    is_declared_fk = any(
                        fk["column"] == col_name for fk in table.foreign_keys
                    )
    ...
    def _compare_tables(self, old: Table, new: Table) -> Tuple[List[str], List[str]]:
        ...
        for idx in old.indexes:
            if idx.name not in new_index_names:
                ...
                down.append(
                    f"CREATE {unique}INDEX {idx.name} ON {idx.table}({cols}){where};"
                )
...
Examples:
  ...
  %(prog)s schema.sql --suggest-indexes --output migrations/
    ...
    parser.add_argument(
        "--suggest-indexes", action="store_true", help="Generate index suggestions"
    )
