...
class LoadTester:
    ...
    def _aggregate_results(self, elapsed_time: float) -> LoadTestResults:
        ...
        throughput_mbps = (
            (total_bytes * 8) / (elapsed_time * 1_000_000) if elapsed_time > 0 else 0
        )
        ...
        return LoadTestResults(
            ...
            requests_per_second=(
                len(self.results) / elapsed_time if elapsed_time > 0 else 0
            ),
...
def print_results(results: LoadTestResults, verbose: bool = False):
    ...
    print(
        f"  Successful: {results.successful_requests:,} ({results.success_rate():.1f}%)"
    )
    ...
    if results.latency_p95 > 200:
        print(
            f"  Warning: P95 latency ({results.latency_p95:.0f}ms) exceeds 200ms target"
        )
...
def compare_results(results1: LoadTestResults, results2: LoadTestResults):
    ...
    for name, v1, v2, lower_better in metrics:
        print(
            f"{name:<25} {v1:<15.1f} {v2:<15.1f} {diff_str(v1, v2, lower_better):<15}"
        )
