# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\regulatory-affairs-head\scripts\regulatory_tracker.py
#!/usr/bin/env python3
"""
...
class RegulatoryTracker:
                    ...
                    for date_field in [
                        ...
                        if sub_data.get(date_field):
                            sub_data[date_field] = datetime.datetime.strptime(
                                sub_data[date_field], "%Y-%m-%d"
                            ).date()

                    # Convert enums
                    sub_data["submission_type"] = SubmissionType(
                        sub_data["submission_type"]
                    )
                    sub_data["submission_status"] = SubmissionStatus(
                        sub_data["submission_status"]
                    )
    ...
    def update_submission_status(
        self, submission_id: str, new_status: SubmissionStatus, notes: str = ""
    ):
    ...
    def get_submissions_by_status(
        ...
        return [
            sub for sub in self.submissions.values() if sub.submission_status == status
        ]
    ...
    def get_overdue_submissions(self) -> List[RegulatorySubmission]:
        ...
        for submission in self.submissions.values():
            if (
                submission.target_approval_date
                and submission.target_approval_date < today
                and submission.submission_status
                not in [
    ...
    def generate_status_report(self) -> str:
        ...
        if overdue:
            ...
            for submission in overdue:
                days_overdue = (
                    datetime.date.today() - submission.target_approval_date
                ).days
                report.append(
                    f"  {submission.submission_id} - {days_overdue} days overdue"
                )
            report.append("")
        ...
        if active_submissions:
            ...
            for submission in active_submissions:
                report.append(
                    f"  {submission.submission_id} - {submission.product_name}"
                )
