...
class FormatDetector:
    ...
    def _is_python(self, code: str) -> bool:
        ...
        matches = sum(
            1 for pattern in py_patterns if re.search(pattern, code, re.MULTILINE)
        )
        return matches >= 3
    ...
    def identify_test_patterns(self, code: str) -> List[str]:
        ...
        if any(
            comment in code.lower()
            for comment in ["// arrange", "# arrange", "// act", "# act"]
        ):
        ...
        if any(
            keyword in code
            for keyword in ["beforeEach", "afterEach", "setUp", "tearDown"]
        ):
