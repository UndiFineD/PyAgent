...
class CoverageAnalyzer:
    ...
    def parse_coverage_report(
        self, report_content: str, format_type: str
    ) -> Dict[str, Any]:
        """
                    ...
                    for line in cls.findall("lines/line"):
                        ...
                        if branch == "true":
                            condition_coverage = line.get(
                                "condition-coverage", "0% (0/0)"
                            )
                            # Parse "(covered/total)"
                            if "(" in condition_coverage:
                                branch_info = condition_coverage.split("(")[1].split(
                                    ")"
                                )[0]
    ...
    def _analyze_file_gaps(
        ...
        uncovered_branches = [
            branch_id for branch_id, hit in branches.items() if hit == 0
        ]
