...
class TDDWorkflow:
        ...
        validations.append(
            {
                "valid": has_assertion,
                "message": (
                    "Contains assertions" if has_assertion else "Missing assertions"
                ),
            }
        )
    ...
    def validate_green_phase(
        ...
        if not implementation_code or len(implementation_code.strip()) < 5:
            validations.append(
                {"valid": False, "message": "No implementation code provided"}
            )
        else:
            validations.append(
                {"valid": True, "message": "Implementation code provided"}
            )
        ...
        if all_valid:
            ...
            return {
                ...
                "refactoring_suggestions": self._suggest_refactorings(
                    implementation_code
                ),
            }
        else:
    ...
    def validate_refactor_phase(
        ...
        quality_improved = self._check_quality_improvement(
    ...
    def _check_quality_improvement(self, original: str, refactored: str) -> bool:
        ...
        refactored_lines = set(
            line.strip() for line in refactored.split("\n") if line.strip()
        )
