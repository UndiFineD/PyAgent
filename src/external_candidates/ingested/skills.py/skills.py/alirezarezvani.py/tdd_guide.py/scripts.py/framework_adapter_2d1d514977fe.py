# Extracted from: C:\DEV\PyAgent\.external\skills\skills\alirezarezvani\tdd-guide\scripts\framework_adapter.py
"""
...
{self._indent(test_content, 4)}
    ...
    def generate_test_function(
        self, test_name: str, test_body: str, description: str = ""
    ) -> str:
        """
...
{self._indent(test_body, 2)}
    ...
    def generate_assertion(
        self, actual: str, expected: str, assertion_type: str = "equals"
    ) -> str:
        """
    ...
    def generate_setup_teardown(
        self, setup_code: str = "", teardown_code: str = ""
    ) -> str:
...
{self._indent(teardown_code, 4)}""")
            ...
            annotation = (
                "@BeforeEach" if self.framework == Framework.JUNIT else "@BeforeMethod"
            )
            if setup_code:
...
{self._indent(setup_code, 4)}
            ...
            annotation = (
                "@AfterEach" if self.framework == Framework.JUNIT else "@AfterMethod"
            )
            if teardown_code:
                result.append(f"""{annotation}
