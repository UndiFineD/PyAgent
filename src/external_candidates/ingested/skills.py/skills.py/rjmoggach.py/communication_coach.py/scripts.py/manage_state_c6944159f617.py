# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rjmoggach\communication-coach\scripts\manage_state.py
#!/usr/bin/env python3
"""
...
def get_weakest_dimension():
    ...
    return {"weakest_dimension": weakest, "score": lowest_score if weakest else None}
...
def main():
    ...
    parser.add_argument(
        "--load", action="store_true", help="Load and display current state"
    )
    parser.add_argument(
        "--update", action="store_true", help="Update a dimension score"
    )
    parser.add_argument("--dimension", type=str, help="Dimension to update")
    parser.add_argument("--score", type=float, help="New score (0-10)")
    parser.add_argument(
        "--modality", type=str, default="email-formal", help="Communication modality"
    )
    parser.add_argument("--weakest", action="store_true", help="Get weakest dimension")
    parser.add_argument(
        "--archive", action="store_true", help="Archive current state to history"
    )
    parser.add_argument(
        "--reset", action="store_true", help="Reset state (WARNING: destructive)"
    )
