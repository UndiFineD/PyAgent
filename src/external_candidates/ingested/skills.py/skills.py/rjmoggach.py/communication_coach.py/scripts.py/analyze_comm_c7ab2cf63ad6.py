# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rjmoggach\communication-coach\scripts\analyze_comm.py
#!/usr/bin/env python3
"""
...
def detect_passive_voice(text):
    ...
    passive_indicators = re.findall(
        r"\b(is|are|was|were|be|been|being) \w+ed\b", text, re.IGNORECASE
    )
...
def detect_boundary_setting(text):
    ...
    direct_no = len(
        re.findall(
            r"\b(No|I\'m not available|I can\'t|I won\'t)\b", text, re.IGNORECASE
        )
    )
    ...
    justifications = len(
        re.findall(r"\b(because|since|as|due to)\b", text, re.IGNORECASE)
    )
...
def score_clarity(text):
    ...
    if not re.search(
        r"\b(First|Second|Therefore|In summary|Specifically)\b", text, re.IGNORECASE
    ):
        score -= 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze communication text for behavioral dimensions"
    )
    parser.add_argument("--text", type=str, required=True, help="Text to analyze")
    parser.add_argument(
        "--modality",
        type=str,
        default="email-formal",
        choices=[
            "email-formal",
            "email-casual",
            "slack",
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Include diagnostic details"
    )
    ...
    if not args.verbose:
        ...
        output = {"modality": results["modality"], "scores": results["scores"]}
