# Extracted from: C:\DEV\PyAgent\.external\skills\skills\diegopetrucci\odds-checker-api\scripts\odds_api.py
#!/usr/bin/env python3
...
def build_parser():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print request URL without calling the API",
    )
    ...
    bookmakers.add_argument(
        "--all", action="store_true", help="Include inactive bookmakers"
    )
    bookmakers.add_argument(
    ...
    events.add_argument(
        "--status", help="Comma-separated statuses (pending,live,settled)"
    )
    ...
    search = subparsers.add_parser(
        "search", help="Search events by team or league name"
    )
    ...
    odds.add_argument(
        "--bookmakers", required=True, help="Comma-separated bookmaker names"
    )
    odds.add_argument("--summary", action="store_true", help="Print a compact summary")
    odds.set_defaults(func=command_odds)

    matchup = subparsers.add_parser(
        "matchup", help="Search for a matchup and fetch odds"
    )
    ...
    matchup.add_argument(
        "--pick", type=int, help="Pick event by 1-based index when multiple match"
    )
