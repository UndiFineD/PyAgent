...
def _http_get_json(path, params=None):
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "tfl-journey-disruption-skill"}
    )
...
def main():
    ...
    parser.add_argument(
        "--time-is", choices=["Depart", "Arrive"], help="Depart or Arrive"
    )
    ...
    from_location = args.from_location or (
        args.locations[0] if len(args.locations) > 0 else None
    )
    to_location = args.to_location or (
        args.locations[1] if len(args.locations) > 1 else None
    )
