...
def main():
    ...
    parser.add_argument(
        "--register", action="store_true", help="Register webhook with Manus API"
    )
    parser.add_argument(
        "--url", help="Webhook URL for registration (required with --register)"
    )
    ...
    if args.register:
        if not args.url:
            ...
            print(
                "   Example: python3 webhook_server.py 8080 --register --url https://your-domain.com/webhook/manus"
            )
            sys.exit(1)
