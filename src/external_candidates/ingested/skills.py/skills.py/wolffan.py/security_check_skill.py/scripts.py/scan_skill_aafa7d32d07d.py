# Extracted from: C:\DEV\PyAgent\.external\skills\skills\wolffan\security-check-skill\scripts\scan_skill.py
#!/usr/bin/env python3
"""
...
class SecurityScanner:
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        if self.issues:
            results["summary"] = (
                f"SECURITY ISSUES FOUND: {len(self.issues)} issue(s), {len(self.warnings)} warning(s)"
            )
        elif self.warnings:
            results["summary"] = (
                f"WARNINGS: {len(self.warnings)} warning(s) (no critical issues)"
            )
        else:
