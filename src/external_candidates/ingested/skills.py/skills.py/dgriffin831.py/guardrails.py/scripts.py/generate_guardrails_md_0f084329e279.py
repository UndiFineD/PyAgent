# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dgriffin831\guardrails\scripts\generate_guardrails_md.py
#!/usr/bin/env python3
"""
...
def call_llm(system_prompt, user_message):
    ...
    if provider == "none":
        print(
            "Error: No LLM provider available. Set OPENAI_API_KEY or ANTHROPIC_API_KEY.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _extract_user_name(answers):
    if isinstance(answers, dict):
        if (
            isinstance(answers.get("user_name"), str)
            and answers.get("user_name").strip()
        ):
...
def _build_risk_categories(classification):
    risks_by_category = (
        classification.get("risksByCategory", {})
        if isinstance(classification, dict)
        else {}
    )
    descriptions = (
        classification.get("riskCategoryDescriptions", {})
        if isinstance(classification, dict)
        else {}
    )
    lines = []
...
def _build_skill_policies(classification):
    risks_by_skill = (
        classification.get("risksBySkill", {})
        if isinstance(classification, dict)
        else {}
    )
...
def _default_replacements(discovery, classification, answers):
    ...
    sensitive_types = _format_sensitive_types(
        answers.get("sensitive_data_types") if isinstance(answers, dict) else None
    )
    ...
    risk_category_count = (
        len([v for v in risks_by_category.values() if v])
        if isinstance(risks_by_category, dict)
        else 0
    )
    ...
    return {
        ...
        "{{OVERALL_RISK_LEVEL}}": (
            classification.get("overallRiskLevel", "UNKNOWN")
            if isinstance(classification, dict)
            else "UNKNOWN"
        ),
...
def main():
    ...
    if not config_path:
        ...
        print(
            "Usage: python3 generate_guardrails_md.py <config-output-path>",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not input_data.strip():
        print(
            "Error: No input on stdin. Pipe {discovery, classification, answers} JSON.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if needs_fallback:
        print(
            f"Warning: LLM output invalid ({reason}). Falling back to deterministic defaults.",
            file=sys.stderr,
        )
