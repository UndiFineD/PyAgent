...
def format_text_report(report: dict) -> str:
    ...
    lines.append(
        f"  Score:    {report['score']}/100 {risk_icon} {report.get('risk', 'LOW')} RISK"
    )
    lines.append("")
    ...
    if llm_analysis:
        ...
        lines.append(
            f"  Verdict:    {verdict_icon} {verdict} (confidence: {confidence:.2f})"
        )
        lines.append(
            f"  Model:      {llm.get('model', 'unknown')} ({llm.get('provider', 'unknown')})"
        )
    ...
    if alignment:
        ...
        if mismatches:
            ...
            for m in mismatches[:10]:
                ...
                lines.append(
                    f"    {sev_icon} [{m.get('type', '?')}] {m.get('description', '')[:120]}"
                )
                if m.get("evidence"):
...
def format_compact_report(report: dict, skill_name: str | None = None) -> str:
    name = (
        skill_name
        or (report.get("metadata") or {}).get("name")
        or report["path"].split("/")[-1]
    )
    lines: list[str] = []
    ...
    for category, cat_findings in by_category.items():
        worst = max(
            cat_findings, key=lambda f: _severity_rank(f.get("severity", "info"))
        )
        icon = SEVERITY_ICONS.get(worst.get("severity", "info"), "")
        lines.append(f"{icon} **{category}**: {len(cat_findings)} finding(s)")

        unique_matches = list(dict.fromkeys(f.get("match", "") for f in cat_findings))[
            :3
        ]
    ...
    if llm_analysis:
        ...
        if verdict == "SAFE":
            ...
            verdict_icon = "\u26a0\ufe0f"
        lines.append(
            f"{verdict_icon} **LLM**: {verdict} ({llm.get('model', '')}, {llm.get('latencyMs', 0)}ms)"
        )
        if llm.get("overall_assessment"):
    ...
    if alignment:
        ...
        if aligned:
            ...
            mismatches = alignment.get("mismatches", [])
            lines.append(
                f"\u26a0\ufe0f **Alignment**: {len(mismatches)} mismatch(es) ({classification})"
            )
...
def format_moltbook_post(report: dict, skill_name: str | None = None) -> str:
    ...
    if summary.get("critical", 0) > 0:
        ...
        for f in criticals[:5]:
            lines.append(
                f"- [{f.get('ruleId', '')}] {f.get('title', '')} @ {f.get('file', '')}:{f.get('line', 0)}"
            )
        lines.append("")
