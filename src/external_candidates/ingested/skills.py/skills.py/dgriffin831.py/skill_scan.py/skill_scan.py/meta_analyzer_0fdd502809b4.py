    ...
    def _build_user_prompt(
        ...
        if sigs:
            ...
            for sig in sigs:
                ...
                parts.append(
                    f"  - {sig.get('name', '?')}: {sig.get('description', '')[:100]}{suppressed}"
                )
            parts.append("")
    ...
    def apply_to_report(report: dict, meta_result: dict) -> None:
        ...
        for review in meta_result.get("finding_reviews", []):
            ...
            if review.get("verdict") == "false_positive":
                finding["weight"] = 0
                reason = review.get(
                    "reason", "Classified as false positive by meta-analysis"
                )
                ...
                if adj and adj in valid_severities and adj != finding.get("severity"):
                    ...
                    finding["weight"] = severity_to_weight.get(
                        adj, finding.get("weight", 8)
                    )
                    finding["contextNote"] = (
        ...
        report["metaAnalysis"]["overall_risk"] = meta_result.get(
            "overall_risk", "SUSPICIOUS"
        )
        report["metaAnalysis"]["finding_reviews"] = meta_result.get(
            "finding_reviews", []
        )
...
def _validate_meta_result(parsed: dict, num_findings: int) -> dict:
    ...
    for c in correlations:
        ...
        if not isinstance(indices, list):
            continue
        valid_indices = [
            i for i in indices if isinstance(i, int) and 0 <= i < num_findings
        ]
