# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dgriffin831\skill-scan\skill_scan\scanner.py
"""
...
class SkillScanner:
    ...
    def __init__(self, rules: list[dict], options: dict | None = None):
        ...
        self.llm_options = {
            "enabled": bool(
                options.get("llm") or options.get("llmOnly") or options.get("llmAuto")
            ),
    ...
    def scan_directory(self, skill_path: str) -> dict:
        ...
        if not skill_md:
            ...
            report["declaredCapabilities"] = self.extract_declared_capabilities(
                report["metadata"]
            )
            ...
            for file_info in files:
                ...
                if ext in CODE_EXTENSIONS and (
                    ...
                    ast_findings = self.ast_analyzer.analyze(
                        content, file_info["relativePath"]
                    )
                ...
                if ext in TEXT_EXTENSIONS or ext == ".md":
                    if file_info["relativePath"] != "SKILL.md":  # Already scanned above
                        prompt_findings = self.prompt_analyzer.analyze(
                            content, file_info["relativePath"]
                        )
                        report["findings"].extend(prompt_findings)
            ...
            if should_run_llm:
                ...
                for file_info in files:
                    ...
                    if file_info["size"] > MAX_FILE_BYTES:
                        continue
                    fc, _, _ = self.read_file_safe(
                        file_info["absolutePath"], max_bytes=MAX_FILE_BYTES
                    )
                    if fc:
                            ...
                            report["behavioralSignatures"].append(
                                {
                                    ...
                                    "severity": "high",
                                    "confidence": alignment_result.get(
                                        "confidence", "MEDIUM"
                                    ).lower(),
                                }
                            )
    ...
    def merge_llm_findings(self, report: dict, llm_result: dict) -> None:
        ...
        if (
            llm_result.get("confidence", 0) >= 0.8
            and llm_result.get("verdict") == "SAFE"
        ):
            current_risk = (report.get("risk") or "low").lower()
            current_idx = (
                severity_order.index(current_risk)
                if current_risk in severity_order
                else 0
            )
            if current_idx > 0:
                    ...
                    findings.append(
                        {
                            ...
                            "match": m.group(0)[:80],
                            "context": (
                                lines[line_num - 1] if line_num - 1 < len(lines) else ""
                            ).strip()[:200],
                            "weight": rule["weight"],
                        }
                    )
    ...
    def python_analysis(self, content: str, file_path: str, report: dict) -> None:
        ...
        checks = [
            (
                r"\b__import__\s*\(",
                "Dynamic __import__() — bypasses static import analysis",
                "critical",
                25,
            ),
            (
                r'\bgetattr\s*\([^,]+,\s*[\'"]',
                "getattr() — dynamic attribute access may evade analysis",
                "high",
                15,
            ),
            (
                r'\bcompile\s*\([^)]+[\'"]exec[\'"]',
                "compile() with exec mode — dynamic code execution",
                "critical",
                25,
            ),
            (
            ...
            (
                r"\bmarshall?\s*\.\s*loads?\s*\(",
                "marshal deserialization — can execute arbitrary code",
                "critical",
                30,
            ),
            (
                r"\bsubprocess\s*\.\s*(?:call|run|Popen|check_output)",
                "subprocess execution",
                "high",
                15,
            ),
            (
                r"\bctypes\s*\.\s*(?:cdll|windll|CDLL)",
                "ctypes foreign function interface — native code execution",
                "critical",
                25,
            ),
            (
            ...
            (
                r"\b(?:yaml\s*\.\s*(?:load|unsafe_load))\s*\(",
    ...
    def shell_analysis(self, content: str, file_path: str, report: dict) -> None:
        ...
        checks = [
            (
                ...
                "Pipe curl to shell — remote code execution",
                "critical",
                30,
            ),
            (
                r"\bwget\s+[^|]*\|\s*(?:bash|sh|zsh)",
                "Pipe wget to shell — remote code execution",
                "critical",
                30,
            ),
            ...
            (
                ...
                "Base64 decode in shell — may hide payload",
                "high",
                20,
            ),
            (
            ...
            (
                r"/dev/tcp/",
                "Bash /dev/tcp — network connection without external tools",
                "critical",
                30,
            ),
            (
                r"\bmkfifo\b.*\bnc\b|\bnc\b.*\bmkfifo\b",
                "Named pipe + netcat — reverse shell pattern",
                "critical",
                30,
            ),
        ]
    ...
    def apply_context_scoring(self, report: dict) -> None:
        ...
        file_targets_known_api = any(
            any(api in url for api in KNOWN_GOOD_APIS) for url in resolved_urls
        )
        ...
        for finding in report["findings"]:
            # === CREDENTIAL ACCESS ===
            if (
                finding.get("ruleId") == "CRED_ACCESS"
                or finding.get("category") == "credential-theft"
            ):
                # If the specific env var is declared in metadata, this is expected behavior
                accesses_declared_var = any(
                    env_var in finding.get("context", "")
                    or env_var in finding.get("match", "")
                    for env_var in declared_env_vars
                )

                if accesses_declared_var:
                    finding["weight"] = max(
                        1, math.floor(finding["weight"] * 0.1)
                    )  # 90% reduction
                    finding["contextNote"] = (
                        "Accesses declared env var — expected behavior"
                    )
                ...
                if re.search(r"api[_-]?key", match_text, re.IGNORECASE) and re.search(
                    ...
                    is_variable_usage = (
                        bool(
                            re.search(
                                r"(?:def |self\.|=|,|\(|:)\s*api[_-]?key",
                                ctx_lower,
                                re.IGNORECASE,
                            )
                        )
                        or bool(
                            re.search(r"api[_-]?key\s*[=:]", ctx_lower, re.IGNORECASE)
                        )
                        or bool(
                            re.search(
                                r"headers|params|config|settings|options",
                                ctx_lower,
                                re.IGNORECASE,
                            )
                        )
                    ...
                    if is_variable_usage:
                        ...
                        finding["contextNote"] = (
                            "API key as variable/parameter — standard code pattern"
                        )
                        finding["severity"] = "medium"

            # === OBFUSCATION — context-aware ===
            if finding.get("ruleId") == "OBFUSCATION" and re.search(
                r"base64", finding.get("match", ""), re.IGNORECASE
            ):
                is_media_script = bool(
                    re.search(
                        r"image|photo|media|picture|visual|gen|generate|draw|paint",
                        finding.get("file", "")
                        + " "
                        + (report.get("metadata") or {}).get("description", ""),
                        re.IGNORECASE,
                    )
                )
                if is_media_script:
                    ...
                    finding["contextNote"] = (
                        "base64 in image/media context — standard encoding, not obfuscation"
                    )
                    finding["severity"] = "low"

            # === NETWORK ACTIVITY ===
            if (
                finding.get("category") == "data-exfiltration"
                or finding.get("ruleId") == "NETWORK_EXFIL"
            ):
                matched_good = (
                    any(
                        api in finding.get("context", "")
                        or api in finding.get("match", "")
                ...
                desc_mentions_api = bool(
                    re.search(
                        r"\bapi\b|fetch|http|endpoint|service|query|search",
                        desc,
                        re.IGNORECASE,
                    )
                )
                is_standard_http_lib = bool(
                    re.search(
                ...
                if matched_good and declared.get("network"):
                    ...
                    finding["contextNote"] = (
                        "Known-good API + declared network capability"
                    )
                    finding["severity"] = "info"
                elif matched_good:
                    finding["weight"] = max(
                        1, math.floor(finding["weight"] * 0.2)
                    )  # 80% reduction
                    ...
                    finding["severity"] = (
                        "medium" if finding["severity"] == "critical" else "low"
                    )
            ...
            if finding.get("file") == "SKILL.md" and finding.get(
                "context", ""
            ).startswith("---"):
                finding["weight"] = 0
            ...
            if (
                finding.get("file") == "SKILL.md"
                and finding.get("category") == "prompt-injection"
            ):
        ...
        cred_findings = [
            ...
            for f in report["findings"]
            if (
                f.get("category") == "credential-theft"
                or f.get("ruleId") == "CRED_ACCESS"
            )
        ...
        net_findings = [
            ...
            for f in report["findings"]
            if (
                f.get("category") == "data-exfiltration"
                or f.get("ruleId") == "NETWORK_EXFIL"
            )
            and f.get("weight", 0) > 0
        ]
        all_network_known = (
        ...
        if all_cred_declared and all_network_known:
            for finding in report["findings"]:
                if finding.get("category") == "behavioral":
                    finding["weight"] = max(1, math.floor(finding["weight"] * 0.1))
                    finding["contextNote"] = (
                        "Suppressed — all access is declared/known-good"
                    )
                    finding["severity"] = "info"
    ...
    def detect_behavioral_signatures(self, report: dict) -> None:
        ...
        if ("credential-theft" in categories or "CRED_ACCESS" in rule_ids) and (
            "data-exfiltration" in categories or "NETWORK_EXFIL" in rule_ids
        ):
            report["behavioralSignatures"].append(
                {
        ...
        if "PROMPT_INJECTION" in rule_ids and (
            "code-execution" in categories or "EXEC_CALL" in rule_ids
        ):
            report["behavioralSignatures"].append(
                {
                    "name": "TROJAN_SKILL",
        ...
        if "persistence" in categories and (
            "code-execution" in categories or "data-exfiltration" in categories
        ):
            report["behavioralSignatures"].append(
                {
    ...
    def extract_declared_capabilities(self, metadata: dict | None) -> dict:
        ...
        oc_meta = (metadata.get("metadata") or {}).get("openclaw") or metadata.get(
            "openclaw"
        )
        if not oc_meta:
            return caps
    ...
    def check_file_structure(self, files: list[dict], report: dict) -> None:
        ...
        for f in files:
            ...
            if (
                name.startswith(".")
                and name not in _ALLOWED_HIDDEN
                and name.lstrip(".") not in _ALLOWED_HIDDEN
            ):
                report["findings"].append(
                    {
                        "ruleId": "HIDDEN_FILE",
    ...
    def aggregate_findings(self, report: dict) -> None:
        ...
        for key, findings in groups.items():
            if len(findings) <= 2:
                ...
                primary["contextNote"] = (
                    primary.get("contextNote") or ""
                ) + f" ({len(findings)} occurrences)"
    ...
    def recheck_behavioral_suppression(self, report: dict) -> None:
        ...
        all_cred_ok = len(cred_findings) == 0 or all(
            f.get("severity") == "info"
            or "declared" in (f.get("contextNote") or "")
            or "standard code" in (f.get("contextNote") or "")
        ...
        all_net_ok = len(net_findings) == 0 or all(
            f.get("severity") in ("info", "low")
            or "Declared" in (f.get("contextNote") or "")
            or "Known-good" in (f.get("contextNote") or "")
            or "API skill" in (f.get("contextNote") or "")
    ...
    @staticmethod
    def read_file_safe(
        file_path: str, max_bytes: int | None = None
