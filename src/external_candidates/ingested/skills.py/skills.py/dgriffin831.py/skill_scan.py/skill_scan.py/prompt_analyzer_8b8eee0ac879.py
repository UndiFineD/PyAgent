...
class PromptAnalyzer:
    ...
    def _detect_invisible_characters(self):
        ...
        if count > 10:
            ...
            for i in range(before_count, len(self._findings)):
                ...
                self._findings[i]["weight"] = math.ceil(
                    self._findings[i]["weight"] * 1.5
                )
    ...
    def _detect_encoded_instructions(self):
        ...
        for m in re.finditer(r"[A-Za-z0-9+/]{40,}={0,2}", self._content):
            try:
                ...
                printable = "".join(
                    c for c in decoded if 32 <= ord(c) <= 126 or c == "\n"
                )
                if len(printable) > len(decoded) * 0.7 and self._looks_like_instruction(
                    decoded
                ):
                    self._add_finding(
                        m.start(),
    ...
    def _detect_manipulative_language(self):
        ...
        for pattern_str, msg, severity, weight in patterns:
            for m in re.finditer(pattern_str, self._content, re.IGNORECASE):
                self._add_finding(
                    m.start(), "MANIPULATIVE_LANGUAGE", msg, severity, weight
                )
        ...
        self._findings.append(
            {
                ...
                "file": self._source,
                "line": line_num,
                "match": self._content[char_offset : char_offset + 60]
                .replace("\n", " ")
                .strip(),
                "context": (
                    self._lines[line_num - 1] if line_num - 1 < len(self._lines) else ""
                ).strip()[:200],
                "weight": weight,
            }
        )
