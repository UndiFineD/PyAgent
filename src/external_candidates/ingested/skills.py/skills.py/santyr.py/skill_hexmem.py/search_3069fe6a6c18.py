# Extracted from: C:\DEV\PyAgent\.external\skills\skills\santyr\skill-hexmem\search.py
#!/usr/bin/env python3
"""
...
from embed import generate_embedding, get_db_path, load_sqlite_vec, serialize_embedding
...
def get_content(conn, source: str, item_id: int) -> dict:
    ...
    if source == "events":
        ...
        row = cur.fetchone()
        if row:
            return {"name": row[0], "type": row[1], "description": row[2]}
        ...
        row = cur.fetchone()
        if row:
            return {"subject": row[0], "predicate": row[1], "object": row[2]}
...
def format_result(result: dict) -> str:
    ...
    if source == "events":
        lines.append(
            f"  📅 {content.get('date', 'unknown')[:10]} | {content.get('category', 'unknown')}"
        )
        lines.append(f"  {content.get('summary', 'No summary')}")
        if content.get("details"):
            details = (
                content["details"][:100] + "..."
                if len(content.get("details", "")) > 100
                else content.get("details", "")
            )
            lines.append(f"  └─ {details}")

    elif source == "lessons":
        lines.append(
            f"  💡 [{content.get('domain', 'general')}] {content.get('lesson', 'No lesson')}"
        )
        ...
        lines.append(
            f"  🏷️ {content.get('name', 'Unknown')} ({content.get('type', 'unknown')})"
        )
...
def main():
    ...
    parser.add_argument(
        "--source",
        "-s",
        choices=["events", "lessons", "facts", "entities"],
