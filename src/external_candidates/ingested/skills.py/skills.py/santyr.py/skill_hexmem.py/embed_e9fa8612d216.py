# Extracted from: C:\DEV\PyAgent\.external\skills\skills\santyr\skill-hexmem\embed.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    parser.add_argument(
        "--init-vec", action="store_true", help="Initialize vec0 virtual tables"
    )
    parser.add_argument(
        "--process-queue",
        action="store_true",
    ...
    parser.add_argument(
        "--limit", type=int, default=100, help="Max items to process (default: 100)"
    )
    parser.add_argument("--search", type=str, help="Search for similar items")
    parser.add_argument(
        "--source", type=str, help="Limit search to source table (events, lessons, etc)"
    )
    parser.add_argument(
        "--stats", action="store_true", help="Show embedding queue stats"
    )
