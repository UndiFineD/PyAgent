# Extracted from: C:\DEV\PyAgent\.external\skills\skills\santyr\skill-hexmem\scripts\parse-manifest.py
#!/usr/bin/env python3
"""
...
def commit_observation(conn, obs, dry_run=False):
    ...
    """,
        (
            ...
            obj,
            confidence,
            f"event:{source_event_id}" if source_event_id else None,
        ),
    )
...
def commit_insight(conn, insight, dry_run=False):
    ...
    if conflicts and action == "supersede":
        ...
        print(
            f"   ⚠️  Found {len(conflicts)} similar lesson(s) in '{domain}' - review for supersession"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse hex-reflect manifest and commit to HexMem"
    )
    parser.add_argument("manifest", help="Path to YAML manifest file")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be committed without actually doing it",
    )
