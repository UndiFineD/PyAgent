# Extracted from: C:\DEV\PyAgent\.external\skills\skills\spendit-ai\solana-skills\scripts\pumpfun.py
#!/usr/bin/env python3
"""
...
def get_client(network: str = "mainnet") -> Client:
    ...
    rpc_url = os.environ.get("SOLANA_RPC_URL") or NETWORKS.get(
...
def derive_mayhem_state(mint: Pubkey) -> Pubkey:
    ...
    pda, _ = Pubkey.find_program_address(
        [b"mayhem-state", bytes(mint)], MAYHEM_PROGRAM_ID
    )
    return pda
...
def create_create_v2_instruction(
    ...
    accounts = [
        ...
        AccountMeta(
            pubkey=derive_event_authority(), is_signer=False, is_writable=False
        ),
...
def create_buy_instruction(
    user: Keypair, mint: Pubkey, amount: int, max_sol_cost: int
) -> Instruction:
    ...
    accounts = [
        ...
        AccountMeta(pubkey=PUMP_PROGRAM_ID, is_signer=False, is_writable=False),
        AccountMeta(
            pubkey=derive_global_volume_accumulator(), is_signer=False, is_writable=True
        ),
        AccountMeta(
            pubkey=derive_user_volume_accumulator(user.pubkey()),
            is_signer=False,
            is_writable=True,
        ),
...
async def upload_to_ipfs(
    image_path: str, name: str, symbol: str, description: str
) -> str:
    """
    ...
    async with aiohttp.ClientSession() as session:
        ...
        form.add_field(
            "file", image_data, filename=image_path.name, content_type=mime_type
        )
        form.add_field("network", "public")
        form.add_field("name", image_path.name)

        async with session.post(
            image_upload_url, data=form, headers={"source": "sdk/file"}
        ) as resp:
            if resp.status != 200:
        ...
        form = aiohttp.FormData()
        form.add_field(
            "file", metadata_json, filename="data.json", content_type="application/json"
        )
        form.add_field("network", "public")
...
def cmd_launch(args):
    ...
    # Upload to IPFS
    try:
        metadata_uri = asyncio.run(
            upload_to_ipfs(args.image, args.name, args.symbol, args.description or "")
        )
    except Exception as e:
    ...
    if args.buy and args.buy > 0:
        ...
        ata_ix = create_ata_instruction(
            payer=keypair.pubkey(), owner=keypair.pubkey(), mint=mint
        )
...
Environment variables (required):
  ...
  SOLANA_RPC_URL        Custom RPC endpoint (optional, for faster execution)
    ...
    launch_parser.add_argument(
        "--description", "-d", default="", help="Token description"
    )
    launch_parser.add_argument(
        "--buy", "-b", type=float, help="Dev buy amount in SOL (optional)"
    )
    launch_parser.add_argument(
        "--mint-key",
        "-m",
        help="Base58 private key for mint (for vanity addresses ending with 'pump')",
    )
