...
def cmd_spell(args):
    ...
    if args.search:
        data = api_get("/spells")
        spells = [
            s for s in data["results"] if args.search.lower() in s["name"].lower()
        ]
        if not spells:
...
def cmd_monster(args):
    ...
    if args.search:
        data = api_get("/monsters")
        monsters = [
            m for m in data["results"] if args.search.lower() in m["name"].lower()
        ]
        if not monsters:
...
def cmd_encounter(args):
    ...
    for monster in encounter:
        ...
        print(
            f"      AC {monster['armor_class'][0]['value']}, HP {monster['hit_points']}"
        )
