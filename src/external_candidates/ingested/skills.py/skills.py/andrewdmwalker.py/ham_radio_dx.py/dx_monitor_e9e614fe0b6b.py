...
def cmd_watch(args):
    ...
    if args.json:
        ...
        for spot in spots:
            ...
            mode = spot["mode"]
            print(
                f"   {band:5} {mode:6} {spot['frequency']:8.1f} {spot['dx_station']:12} - {spot['comment']}"
            )
            if args.verbose:
...
def cmd_digest(args):
    ...
    for spot in spots:
        ...
        if any(
            prefix in call
            for prefix in ["VP8", "ZL", "VK", "ZS", "P5", "P4", "9G", "S9"]
        ):
            rare_dx.append(spot)
    ...
    for band in sorted(
        by_band.keys(), key=lambda x: float(x.replace("m", "") if "m" in x else "999")
    ):
        count = by_band[band]
        bar = "█" * (count // 2)
    ...
    if rare_dx:
        for spot in rare_dx[:10]:
            print(
                f"   🌍 {spot['dx_station']:12} {spot['band']:5} {spot['frequency']:8.1f} - {spot['comment']}"
            )
    else:
