...
def format_person(person):
    ...
    if person.get("phone_numbers"):
        for phone in person["phone_numbers"][:2]:
            ...
            lines.append(
                f"   📱 {ptype}: {phone.get('sanitized_number', phone.get('number', 'N/A'))}"
            )
...
Examples:
  ...
  apollo.py bulk-enrich --file contacts.json
    ...
    enrich.add_argument(
        "--reveal-email", action="store_true", help="Include personal emails"
    )
    enrich.add_argument(
        "--reveal-phone", action="store_true", help="Include phone numbers"
    )
    ...
    bulk.add_argument(
        "--file", "-f", required=True, help="JSON file with contacts array"
    )
    bulk.add_argument(
        "--reveal-email", action="store_true", help="Include personal emails"
    )
