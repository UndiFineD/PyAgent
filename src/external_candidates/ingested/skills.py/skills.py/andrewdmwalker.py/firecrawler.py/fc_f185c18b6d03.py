# Extracted from: C:\DEV\PyAgent\.external\skills\skills\andrewdmwalker\firecrawler\fc.py
#!/usr/bin/env python3
...
try:
    ...
    print(
        "Error: firecrawl not installed. Run: pip3 install firecrawl", file=sys.stderr
    )
    sys.exit(1)
...
def get_client():
    ...
    if not api_key:
        ...
        print(
            "Get your key at: https://www.firecrawl.dev/app/api-keys", file=sys.stderr
        )
        sys.exit(1)
...
def cmd_markdown(args):
    ...
    app = get_client()
    result = app.scrape(
        args.url, formats=["markdown"], only_main_content=args.main_only
    )
...
def cmd_screenshot(args):
    ...
    app = get_client()
    result = app.scrape(args.url, formats=["screenshot"])
...
def cmd_extract(args):
    ...
    result = app.scrape(
        ...
        formats=["extract"],
        extract=(
            {"schema": schema, "prompt": args.prompt}
            if args.prompt
            else {"schema": schema}
        ),
    )
...
def cmd_crawl(args):
    ...
    if args.output:
        ...
        for i, page in enumerate(pages):
            ...
            # Get content
            content = (
                page.markdown if hasattr(page, "markdown") else page.get("markdown", "")
            )
...
def cmd_map(args):
    ...
    result = app.map(args.url, search=args.search, limit=args.limit)
