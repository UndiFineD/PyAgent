...
def cmd_forecast(args):
    ...
    if args.json:
        print(
            json.dumps(
                {"today": today, "tomorrow": tomorrow, "day_after": day_after}, indent=2
            )
        )
    else:
        ...
        print(
            f"   S: {tomorrow.get('S', {}).get('Text', 'N/A')} (Prob: {tomorrow.get('S', {}).get('Prob', 'N/A')}%)"
        )
        ...
        print(
            f"   S: {day_after.get('S', {}).get('Text', 'N/A')} (Prob: {day_after.get('S', {}).get('Prob', 'N/A')}%)"
        )
...
def cmd_aurora(args):
    ...
    if args.json:
        ...
        if current_scale and int(current_scale) >= 3:
            print(
                f"   🌟 HIGH - Major geomagnetic storm! Aurora visible at mid-latitudes!"
            )
...
def cmd_summary(args):
    ...
    print(
        f"📊 Current Conditions ({current.get('DateStamp')} {current.get('TimeStamp')} UTC)"
    )
