...
def get_api_key():
    ...
    if not api_key:
        print(
            "Error: PHANTOMBUSTER_API_KEY environment variable not set", file=sys.stderr
        )
        print(
...
def cmd_status(args):
    ...
    status_emoji = {"finished": "✅", "error": "❌", "running": "🔄"}.get(
        last_status, "❓"
    )
...
def cmd_fetch_result(args):
    ...
    result_url = (
        f"https://phantombuster.s3.amazonaws.com/{org_folder}/{s3_folder}/result.csv"
    )
...
Examples:
  ...
  pb.py abort 12345                   # Abort running agent
    ...
    fetch_parser = subparsers.add_parser(
        "fetch-result", help="Download result data (CSV)"
    )
