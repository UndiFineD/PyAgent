# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cnyezi\a-stock-analysis\scripts\analyze.py
#!/usr/bin/env python3
# /// script
...
def fetch_realtime_sina(symbols: list[str]) -> dict[str, dict]:
    ...
    try:
        ...
        for line in text.strip().split("\n"):
            ...
            change_pct = (
                (change_amt / pre_close * 100) if pre_close and pre_close > 0 else 0
            )
...
def format_minute_analysis(analysis: dict, name: str = "") -> str:
    ...
    for item in analysis["top_volumes"]:
        lines.append(
            f"    {item['time']} 价格:{item['price']:.2f} 成交:{item['volume']}手 金额:{item['amount']/10000:.1f}万"
        )
...
def analyze_stock(
    code: str, with_minute: bool = False, realtime_cache: dict = None
) -> dict:
    """分析单只股票"""
...
def main():
    ...
    for code in args.codes:
        result = analyze_stock(
            code, with_minute=args.minute, realtime_cache=realtime_cache
        )
