...
def check_frontmatter(skill_path):
    ...
    return CheckResult(
        "SKILL.md has valid frontmatter", CheckResult.PASS, category="structure"
    )
...
def check_name_matches_dir(skill_path):
    ...
    if name == dir_name:
        return CheckResult(
            "Skill name matches directory", CheckResult.PASS, category="structure"
        )
    return CheckResult(
        "Skill name matches directory",
        CheckResult.WARN,
...
def check_resource_dirs(skill_path):
    ...
    return CheckResult(
        "Resource directories are non-empty", CheckResult.PASS, category="structure"
    )
...
def check_no_ext_deps(skill_path):
    ...
    return CheckResult(
        "Scripts use no external dependencies", CheckResult.PASS, category="scripts"
    )
...
def check_no_hardcoded_secrets(skill_path):
    ...
    for root, dirs, files in os.walk(skill_path):
        ...
        for f in files:
            if f.endswith(
                (".py", ".md", ".sh", ".js", ".ts", ".yaml", ".yml", ".json", ".toml")
            ):
    ...
    return CheckResult(
        "No hardcoded credentials or emails", CheckResult.PASS, category="security"
    )
...
def print_report(results, skill_path, verbose=False):
    ...
    print(
        f"  Structural score: {score:.0f}% ({counts[CheckResult.PASS]}/{total} checks passed)"
    )
    print()
