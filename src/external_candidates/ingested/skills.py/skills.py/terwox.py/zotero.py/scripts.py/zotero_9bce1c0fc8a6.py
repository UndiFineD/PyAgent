# Extracted from: C:\DEV\PyAgent\.external\skills\skills\terwox\zotero\scripts\zotero.py
#!/usr/bin/env python3
...
def get_config():
    ...
    if not api_key:
        ...
        print(
            "Create a key at https://www.zotero.org/settings/keys/new", file=sys.stderr
        )
        sys.exit(1)
    ...
    for attempt in range(_MAX_RETRIES + 1):
        ...
        try:
            ...
            if e.code in _RETRY_CODES and attempt < _MAX_RETRIES:
                ...
                print(
                    f"⚠  HTTP {e.code} — retrying in {delay}s (attempt {attempt + 1}/{_MAX_RETRIES})...",
                    file=sys.stderr,
                )
                time.sleep(delay)
                continue
            ...
            if attempt < _MAX_RETRIES:
                delay = (attempt + 1) * 2
                print(
                    f"⚠  Network error — retrying in {delay}s (attempt {attempt + 1}/{_MAX_RETRIES})...",
                    file=sys.stderr,
                )
                time.sleep(delay)
                continue
...
def validate_doi(s):
    ...
    if not re.match(r"^10\.\d{4,}/\S+$", s):
        print(
            f"Invalid DOI format: '{s}'. Expected pattern: 10.xxxx/...", file=sys.stderr
        )
        return False
    return True
...
def validate_item_key(s):
    ...
    if not re.match(r"^[A-Za-z0-9]{8}$", s):
        print(
            f"Invalid item key: '{s}'. Must be 8 alphanumeric characters.",
            file=sys.stderr,
        )
        return False
    return True
...
def cmd_collections(args):
    ...
    for c in cols:
        d = c["data"]
        parent = (
            f" (parent: {d['parentCollection']})" if d.get("parentCollection") else ""
        )
...
def _check_duplicate_by_metadata(api_key, prefix, new_item, identifier, id_type):
    ...
    items, _ = api_get_json(
        f"{prefix}/items/top", api_key, params={"q": search_q, "limit": "25"}
    )
    if not isinstance(items, list):
        return None
    ...
    for item in items:
        ...
        if id_type == "doi" and d.get("DOI"):
            if d["DOI"].lower().strip().rstrip(
                "/"
            ) == identifier.lower().strip().rstrip("/"):
                return item
        # Match by ISBN
...
def cmd_add_identifier(args):
    ...
    translate_data = json.dumps({"url": lookup_url, "sessionid": "zotero-cli"}).encode(
        "utf-8"
    )
    ...
    try:
        ...
        if id_type == "doi":
            print(
                f"Translation server failed ({e.code}). Trying manual DOI metadata...",
                file=sys.stderr,
            )
    ...
    if not getattr(args, "force", False) and new_items:
        existing = _check_duplicate_by_metadata(
            api_key, prefix, new_items[0], identifier, id_type
        )
        if existing:
...
def cmd_check_pdfs(args):
    ...
    for item in all_items:
        ...
        if itype in ("attachment", "note"):
            ...
            if itype == "attachment" and d.get("contentType", "").startswith(
                "application/pdf"
            ):
...
def cmd_crossref(args):
    ...
    items = [
        i for i in items if i["data"].get("itemType") not in ("attachment", "note")
    ]
...
def _match_crossref_result(work, zotero_title, zotero_year, zotero_first_author):
    ...
    if zotero_first_author:
        ...
        for a in cr_authors:
            ...
            if (
                family
                and zotero_first_author in family
                or family in zotero_first_author
            ):
                author_found = True
                break
        if not author_found:
...
def cmd_find_dois(args):
    ...
    if args.collection:
        items = paginate_all(
            f"{prefix}/collections/{args.collection}/items/top", api_key
        )
    else:
        items = paginate_all(f"{prefix}/items/top", api_key)
    ...
    print(
        f"Found {len(candidates)} items missing DOIs "
        ...
        if best_match:
            ...
            if apply_mode:
                try:
                    ...
                    print(
                        f"    ❌ Failed to write DOI: {e.code} {e.reason}",
                        file=sys.stderr,
                    )
                except Exception as e:
...
def _bulk_find_pdf_parents(api_key, prefix, collection_key=None):
    ...
    if collection_key:
        all_items = paginate_all(
            f"{prefix}/collections/{collection_key}/items", api_key
        )
    else:
        all_items = paginate_all(f"{prefix}/items", api_key)
...
def cmd_fetch_pdfs(args):
    ...
    sources = (
        [s.strip() for s in args.sources.split(",")] if args.sources else PDF_SOURCES
    )
    ...
    for s in sources:
        if s not in PDF_SOURCES:
            print(
                f"Unknown source: {s}. Valid: {', '.join(PDF_SOURCES)}", file=sys.stderr
            )
            sys.exit(1)
    ...
    print(
        f"Found {len(candidates)} items to process "
    ...
    for i, item in enumerate(candidates, 1):
        ...
        try:
            ...
            if pdf_downloaded:
                ...
                if upload_mode:
                    if _upload_pdf_to_zotero(
                        api_key, prefix, key, tmp_path, pdf_filename
                    ):
                        print(f"    📎 Uploaded to Zotero storage")
                        downloaded += 1
                    else:
                        print(
                            f"    ⚠  Upload to Zotero failed, creating linked URL instead"
                        )
                        if _create_linked_url_attachment(
                            api_key, prefix, key, f"{title}.pdf", source_url
                        ):
...
def cmd_delete(args):
    ...
    for key in args.keys:
        ...
        if not args.yes:
            ...
            print(f"  {fmt_item_short(item)}")
            confirm = (
                input(f"  {action.capitalize()} '{title}'? [y/N] ").strip().lower()
            )
            if confirm != "y":
                print("  Skipped.")
                continue
        ...
        if args.permanent:
            ...
            try:
                ...
                print(
                    f"❌ Failed to delete {key}: {e.code} {e.reason}", file=sys.stderr
                )
        else:
            ...
            req = urllib.request.Request(
                url, data=body, headers=req_headers, method="PATCH"
            )
            try:
                with urllib.request.urlopen(req, timeout=30) as resp:
                    pass
...
def main():
    ...
    p.add_argument(
        "--top", action="store_true", default=True, help="Top-level items only"
    )
    ...
    p.add_argument(
        "--force", action="store_true", help="Add even if duplicate detected"
    )
    ...
    p = subparsers.add_parser(
        "delete", help="Move items to trash (default) or permanently delete"
    )
    ...
    p.add_argument(
        "--permanent",
        action="store_true",
        help="Permanently delete (default is recoverable trash)",
    )
    p.add_argument(
        "--trash",
        action="store_true",
        help="Move to trash (default, kept for backwards compat)",
    )
    ...
    # batch-add
    p = subparsers.add_parser(
        "batch-add", help="Add multiple items from a file of identifiers"
    )
    p.add_argument("file", help="File with one identifier per line")
    p.add_argument(
        "--type",
        default="doi",
        choices=["doi", "isbn", "pmid"],
    ...
    # crossref
    p = subparsers.add_parser(
        "crossref", help="Cross-reference citations against library"
    )
    p.add_argument(
        "file", help="Text/markdown file with citations (Author, Year format)"
    )

    # find-dois
    p = subparsers.add_parser(
        "find-dois", help="Find and add missing DOIs via CrossRef"
    )
    p.add_argument(
        "--apply", action="store_true", help="Actually write DOIs (default is dry run)"
    )
    ...
    p.add_argument(
        "--upload",
        action="store_true",
        help="Upload to Zotero storage (S3) instead of linked URL",
    )
    p.add_argument(
        "--sources",
        default=",".join(PDF_SOURCES),
