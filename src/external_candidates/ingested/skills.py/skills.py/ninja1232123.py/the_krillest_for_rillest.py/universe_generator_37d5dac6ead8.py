# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ninja1232123\the-krillest-for-rillest\universe_generator.py
#!/usr/bin/env python3
"""
██╗   ██╗███╗   ██╗██╗██╗   ██╗███████╗██████╗ ███████╗███████╗
██║   ██║████╗  ██║██║██║   ██║██╔════╝██╔══██╗██╔════╝██╔════╝
...
class Universe:
    ...
    def _generate_planet(self, star: Star, orbit_index: int) -> Planet:
        ...
        if distance < 0.5:
            ptype = self.rng.choice(
                [PlanetType.LAVA, PlanetType.ROCKY, PlanetType.DESERT]
            )
        elif distance < 2.0:
            ...
            ptype = self.rng.choice(
                [PlanetType.GAS_GIANT, PlanetType.ICE_GIANT, PlanetType.FROZEN]
            )
        else:
            ptype = self.rng.choice(
                [PlanetType.ICE_GIANT, PlanetType.FROZEN, PlanetType.GAS_GIANT]
            )
        ...
        has_water = ptype in [
            ...
            PlanetType.GARDEN,
            PlanetType.FROZEN,
        ] or (ptype == PlanetType.ROCKY and self.rng.random() > 0.6)
        ...
        planet = Planet(
            ...
            has_water=has_water,
            moons=(
                self.rng.randint(0, 20)
                if ptype in [PlanetType.GAS_GIANT, PlanetType.ICE_GIANT]
                else self.rng.randint(0, 3)
            ),
        )
        ...
        if (
            in_habitable_zone
            and planet.habitable_score() > 0.6
            and self.constants.allows_life()
            and star.age > 1.0
        ):  # Need time for life to evolve
    ...
    def _generate_star(self) -> Star:
        ...
        star = Star(
            name=self._generate_star_name(), star_type=star_type, mass=mass, age=age
        )
...
def render_star_system(star: Star) -> str:
    ...
    lines.append(
        f"    {color}{star_symbol} {star.name}{C.END} ({star.star_type.display_name})"
    )
    lines.append(
        f"      {C.DIM}Mass: {star.mass:.2f} M☉ | Age: {star.age:.1f} Gyr{C.END}"
    )

    if star.planets:
        hz_inner, hz_outer = star.habitable_zone()
        lines.append(
            f"      {C.GREEN}Habitable Zone: {hz_inner:.2f} - {hz_outer:.2f} AU{C.END}"
        )
        lines.append("")
        ...
        for planet in star.planets:
            ...
            lines.append(
                f"        {planet.planet_type.display_name} | {planet.distance_au:.2f} AU | {planet.mass_earth:.1f} M⊕{hz_marker}{life_marker}"
            )
...
""")
    ...
    print(
        f"    {'✓' if stable else '✗'} Stable Structure:     {C.GREEN if stable else C.RED}{'YES' if stable else 'NO - Universe cannot exist!'}{C.END}"
    )
    print(
        f"    {'✓' if chem else '✗'} Complex Chemistry:     {C.GREEN if chem else C.RED}{'YES' if chem else 'NO - No molecules possible'}{C.END}"
    )
    print(
        f"    {'✓' if life else '✗'} Life Possible:         {C.GREEN if life else C.RED}{'YES' if life else 'NO - Life cannot emerge'}{C.END}"
    )
    print()
...
""")
    ...
    print(
        f"    {C.CYAN}Age:{C.END}                    {universe.age:.2f} billion years"
    )
    print(
        f"    {C.CYAN}Total Galaxies:{C.END}         ~{format_number(len(universe.galaxies) * 1_000_000_000)}"
    )
    print(
        f"    {C.CYAN}Total Stars:{C.END}            ~{format_number(universe.total_stars)}"
    )
    print(
        f"    {C.CYAN}Total Planets:{C.END}          ~{format_number(universe.total_planets * 1_000_000)}"
    )
    print(
        f"    {C.CYAN}Habitable Planets:{C.END}      ~{format_number(universe.habitable_planets * 100_000)}"
    )
    print(
        f"    {C.CYAN}Planets with Life:{C.END}      ~{format_number(universe.planets_with_life * 10_000)}"
    )
    if universe.intelligent_civilizations > 0:
        print(
            f"    {C.YELLOW}{C.BOLD}Intelligent Civs:{C.END}       ~{format_number(universe.intelligent_civilizations * 1000)}"
        )
    print()

    # Fate
    fate_color = (
        C.RED if "RIP" in universe.fate or "CRUNCH" in universe.fate else C.CYAN
    )
    print(f"    {C.BOLD}Ultimate Fate:{C.END} {fate_color}{universe.fate}{C.END}")
    print(
        f"    {C.DIM}Time remaining: ~{universe.time_remaining:.0f} billion years{C.END}"
    )
    print()
    ...
    for galaxy in universe.galaxies:
        ...
        print(
            f"    {C.DIM}{galaxy.galaxy_type} Galaxy | {format_number(galaxy.star_count)} stars | {format_number(galaxy.diameter_ly)} light-years{C.END}"
        )
        print(
            f"    {C.DIM}Age: {galaxy.age:.1f} Gyr | {'Supermassive Black Hole at center' if galaxy.has_supermassive_black_hole else 'No central black hole'}{C.END}"
        )
        print()
...
def main():
    ...
    for stage in stages:
        print(
            f"    {C.DIM}[{'█' * random.randint(1,3)}{'░' * random.randint(1,2)}]{C.END} {stage}...",
            end="",
        )
        sys.stdout.flush()
        time.sleep(0.3)
