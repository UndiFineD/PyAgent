...
class LiminalCartographer:
    ...
    def _sample_boundary(self, boundary: Boundary):
        ...
        for _ in range(num_samples):
            ...
            point = LiminalPoint(
                x=x, certainty=certainty, phenomena=phenomena, observations=observations
            )
    ...
    def _report_boundary(self, boundary: Boundary):
        ...
        print(
            f"    {Threshold.MIST}Boundary type: {boundary.boundary_type.value}{Threshold.RESET}"
        )
        print(
            f"    {Threshold.MIST}Permeability: {self._bar(boundary.permeability)}{Threshold.RESET}"
        )
        print(
            f"    {Threshold.MIST}Thickness: {self._bar(boundary.thickness)}{Threshold.RESET}"
        )
        print(
            f"    {Threshold.MIST}Stability: {self._bar(boundary.stability)}{Threshold.RESET}"
        )
        print()
        ...
        if boundary.points:
            ...
            if most_liminal.observations:
                print(
                    f"    {Threshold.DREAM}✧ {most_liminal.observations[0]}{Threshold.RESET}"
                )
            if most_liminal.phenomena:
                print(
                    f"    {Threshold.EMBER}⚡ Phenomenon detected: {most_liminal.phenomena[0]}{Threshold.RESET}"
                )
        ...
        for row in grid:
            ...
            for char in line:
                if char in "●◎◍":
                    ...
                    colored_line += char
            print(
                f"    {Threshold.GHOST}│{Threshold.RESET}{colored_line}{Threshold.GHOST}│{Threshold.RESET}"
            )
        print(f"    {Threshold.GHOST}└{'─' * width}┘{Threshold.RESET}")

        # Labels
        print(
            f"    {Threshold.DIM}{'DEFINED':^30}{'LIMINAL':^15}{'DEFINED':^15}{Threshold.RESET}"
        )
        print()
    ...
    def _print_discoveries(self):
        ...
        if self.discovered_phenomena:
            print(
                f"    {Threshold.SILVER}Phenomena observed at boundaries:{Threshold.RESET}"
            )
...
def explore_custom_boundary():
    ...
    if not concept_a:
        ...
        print(
            f"{Threshold.MIST}Randomly selected: {concept_a} ←→ {concept_b}{Threshold.RESET}"
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "--quick", "-q", action="store_true", help="Quick mode: skip animations"
    )
    parser.add_argument(
        "concepts", nargs="*", help="Two concepts to map the boundary between"
    )
    ...
    if args.concepts and len(args.concepts) >= 2:
        ...
        while True:
            ...
            print()
            again = input(
                f"{Threshold.MIST}Explore another boundary? (y/n): {Threshold.RESET}"
            )
            if again.lower() != "y":
                cartographer._closing_meditation()
                break
