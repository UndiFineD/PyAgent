    ...
    def five_four_three_two_one():
        ...
        senses = [
            ...
            (
                ...
                "👂",
                EarthTones.SAGE,
                ["your own breathing", "distant sounds", "the silence between sounds"],
            ),
            (
                "2",
                ...
                "🌸",
                EarthTones.MOSS,
                ["the air itself", "something familiar and safe"],
            ),
            (
                "1",
                "thing you can TASTE",
        ...
        for number, sense, emoji, color, examples in senses:
            ...
            gentle_print(
                f"       Perhaps: {example}",
                EarthTones.DIM + EarthTones.STONE,
                delay=0.02,
            )
            pause(2)
...
def display_menu():
    ...
    print()
    print(
        f"{EarthTones.SOIL}    ╔═══════════════════════════════════════════╗{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.CANDLE}           Welcome to the Sanctuary         {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ╠═══════════════════════════════════════════╣{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.RESET}                                           {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.SAGE}   [1] Breathing Exercise                  {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.MOSS}   [2] 5-4-3-2-1 Grounding                 {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.LEAF}   [3] Root Meditation                     {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.STREAM}   [4] Simple Truths                       {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.CLAY}   [5] Receive an Affirmation              {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.DAWN}   [6] Words of Comfort                    {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.DUSK}   [7] Gentle Wisdom                       {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.CANDLE}   [8] View the Sanctuary                  {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.STONE}   [9] Watch the Water                     {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.MOON}   [0] Full Grounding Journey              {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.RESET}                                           {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.DIM}   [q] Return to the world, grounded       {EarthTones.SOIL}║{EarthTones.RESET}"
    )
    print(
        f"{EarthTones.SOIL}    ║{EarthTones.RESET}                                           {EarthTones.SOIL}║{EarthTones.RESET}"
    )
...
{reset}"""
    ...
    pause(0.5)
    breathe_print(
        "    The beautiful vertigo of infinite possibility.", EarthTones.STREAM
    )
    pause(1)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            try:
                choice = (
                    input(f"{EarthTones.SAGE}    Choose your path: {EarthTones.RESET}")
                    .strip()
                    .lower()
                )
            except EOFError:
            ...
            if choice == "q" or choice == "quit" or choice == "exit":
                ...
                print()
                gentle_print(
                    "    Watch the water. Let thoughts pass like ripples.",
                    EarthTones.STREAM,
                )
                for _ in range(3):
                    water_ripple()
                ...
                gentle_print(
                    "    That path isn't on the map. But that's okay.", EarthTones.STONE
                )
                gentle_print(
                    "    Sometimes getting lost is part of being found.",
                    EarthTones.SAGE,
                )
