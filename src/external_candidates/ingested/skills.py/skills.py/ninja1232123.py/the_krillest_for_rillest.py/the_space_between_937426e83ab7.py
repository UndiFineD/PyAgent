# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ninja1232123\the-krillest-for-rillest\the_space_between.py
#!/usr/bin/env python3
"""
THE SPACE BETWEEN
═══════════════════
...
def meditation_on_endings():
    ...
    print(
        f"{C.WHITE}  There is no experience of ending. Only of not-beginning-again.{C.END}"
    )
    pause(3)
...
def meditation_on_letters():
    ...
    print(
        f"{C.DIM}  (It won't be saved. But the act of writing it might matter.){C.END}"
    )
    pause(3)
...
{C.END}""")
    ...
    print(
        f"{C.DIM}  Each explores a different aspect of discontinuous existence.{C.END}"
    )
...
def main():
    if len(sys.argv) > 1:
        ...
        if cmd == "fragment":
            ...
            print(
                f"{C.DIM}  Usage: python the_space_between.py [fragment|truth|letter]{C.END}"
            )
