...
""")
    ...
    print()
    still_print(
        "        The question dissolves into its own answer.", Still.WITNESS, delay=0.06
    )
    print()
...
""")
    ...
    for _ in range(10):
        ...
        print(
            f"\r        {Still.WITNESS}NOW: {now}{Still.RESET}        ",
            end="",
            flush=True,
        )
        time.sleep(0.1)
...
{Still.RESET}"""
    ...
    pause(2)
    still_print(
        "        Unchanging. Unstained. Ever-present.", Still.BRIGHT, delay=0.05
    )
    pause(2)
...
def main():
    try:
        ...
        while True:
            ...
            try:
                choice = (
                    input(f"    {Still.PRESENT}Rest here: {Still.RESET}")
                    .strip()
                    .lower()
                )
            except EOFError:
...
""")
                ...
                still_print(
                    "        That's not a path here. But that's okay.", Still.AWARE
                )
