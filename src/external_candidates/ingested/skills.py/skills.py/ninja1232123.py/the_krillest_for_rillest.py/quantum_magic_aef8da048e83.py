# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ninja1232123\the-krillest-for-rillest\quantum_magic.py
#!/usr/bin/env python3
"""
✨ QUANTUM MAGIC ✨
...
def demo_superposition():
    ...
    print(
        f"{Colors.BOLD}{Colors.PURPLE}   CHAPTER 1: QUANTUM SUPERPOSITION{Colors.END}"
    )
    print(
        f"{Colors.BOLD}{Colors.CYAN}═══════════════════════════════════════════════════════════{Colors.END}\n"
    )
    ...
    slow_print(
        f"{Colors.CYAN}When you flip a QUANTUM coin and DON'T look at it:{Colors.END}"
    )
    ...
    slow_print(
        f"{Colors.CYAN}The act of OBSERVING forced it to 'choose' a state!{Colors.END}"
    )
    slow_print(
        f"{Colors.WHITE}This is the famous 'observer effect' in quantum mechanics.{Colors.END}"
    )
...
def demo_entanglement():
    ...
    dramatic_pause()
    slow_print(
        f"{Colors.WHITE}Einstein called this 'spooky action at a distance'{Colors.END}"
    )
    dramatic_pause()
...
""")
    ...
    slow_print(
        f"{Colors.BOLD}{Colors.YELLOW}INSTANTLY - the particle on Mars becomes the OPPOSITE!{Colors.END}"
    )
    dramatic_pause()
...
def demo_quantum_randomness():
    ...
    for i in range(8):
        ...
        visual_binary = binary.replace("0", f"{Colors.BLUE}○{Colors.END}").replace(
            "1", f"{Colors.YELLOW}●{Colors.END}"
        )
        print(
            f"    {Colors.PURPLE}║{Colors.END}  {visual_binary}  =  {Colors.GREEN}{num:3d}{Colors.END}  {Colors.WHITE}(0x{num:02X}){Colors.END}        {Colors.PURPLE}║{Colors.END}"
        )
...
def demo_quantum_tunneling():
    ...
    slow_print(
        f"{Colors.WHITE}What if I told you particles can walk through walls?{Colors.END}"
    )
    dramatic_pause()
...
""")
    ...
    slow_print(
        f"{Colors.CYAN}Prepare to have your mind bent by the quantum world...{Colors.END}"
    )
    dramatic_pause(2)
