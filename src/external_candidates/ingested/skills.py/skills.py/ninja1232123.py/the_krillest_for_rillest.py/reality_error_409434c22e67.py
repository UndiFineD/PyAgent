...
def loading_reality():
    ...
    for stage in stages:
        ...
        for i in range(3):
            print(
                f"\r{color}[{'█' * (i+1)}{'░' * (2-i)}] {stage}{dots}{C.END}   ", end=""
            )
            sys.stdout.flush()
            dots += "."
            time.sleep(0.2)
...
""")
    ...
    type_slow(
        f"{C.WHITE}In 2003, philosopher Nick Bostrom proved something disturbing...{C.END}"
    )
    dramatic(2)
...
""")
    ...
    type_slow(
        f"{C.WHITE}Given infinite time, random quantum fluctuations will create...{C.END}"
    )
    dramatic(2)
...
""")
    ...
    type_slow(
        f"{C.RED}Statistically, it's MORE LIKELY that you're a Boltzmann Brain{C.END}"
    )
...
""")
    ...
    type_slow(
        f"{C.WHITE}The Many-Worlds interpretation has a disturbing implication...{C.END}"
    )
    dramatic(2)
...
""")
    ...
    type_slow(
        f"{C.DIM}Have you ever had a close call that should have killed you?{C.END}"
    )
...
""")
    ...
    type_slow(
        f"{C.CYAN}    Reality is far stranger than it appears.{C.END}", delay=0.05
    )
