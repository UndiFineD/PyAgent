        ...
        if not physics.stable:
            ...
            if physics.life_possible:
                ...
                if physics.intelligence_possible and life > 0 and age > 4:
                    ...
                    civs = 0
                    dominant = branch_rng.choice(
                        ["Microbial only", "Simple multicellular", "None yet"]
                    )
                    tech = "N/A - No intelligence"
            else:
...
def render_branch_card(branch: UniverseBranch, index: int = None) -> str:
    ...
    lines.append(
        f"{border_color}║{C.END} Status: {status}".ljust(70) + f"{border_color}║{C.END}"
    )
    lines.append(f"{border_color}╠{'═' * 58}╣{C.END}")

    # Divergence point
    lines.append(
        f"{border_color}║{C.END} {C.DIM}Diverged when:{C.END} {branch.divergence_point[:40]}".ljust(
            70
        )
        + f"{border_color}║{C.END}"
    )
    ...
    lines.append(
        f"{border_color}║{C.END} {physics_bar(branch.physics.gravity, 'Gravity    ')}".ljust(
            70
        )
        + f"{border_color}║{C.END}"
    )
    lines.append(
        f"{border_color}║{C.END} {physics_bar(branch.physics.atomic_binding, 'Chemistry  ')}".ljust(
            70
        )
        + f"{border_color}║{C.END}"
    )
    ...
    if branch.physics.stable:
        ...
        if branch.civilizations > 0:
            ...
            lines.append(
                f"{border_color}║{C.END} {C.YELLOW}Dominant:{C.END} {branch.dominant_life_form[:35]}".ljust(
                    70
                )
                + f"{border_color}║{C.END}"
            )
            lines.append(
                f"{border_color}║{C.END} {C.YELLOW}Tech Level:{C.END} {branch.technology_level[:35]}".ljust(
                    70
                )
                + f"{border_color}║{C.END}"
            )
        ...
        lines.append(
            f"{border_color}║{C.END} {C.RED}Universe collapsed - incompatible physics{C.END}".ljust(
                70
            )
            + f"{border_color}║{C.END}"
        )
    ...
    fate_color = (
        C.RED
        if "Rip" in branch.fate or "Crunch" in branch.fate or "Decay" in branch.fate
        else C.CYAN
    )
    lines.append(
        f"{border_color}║{C.END} {C.DIM}Fate:{C.END} {fate_color}{branch.fate[:45]}{C.END}".ljust(
            70
        )
        + f"{border_color}║{C.END}"
    )
    ...
    if branch.special_note:
        ...
        lines.append(
            f"{border_color}║{C.END} {note_color}✦ {branch.special_note[:50]}{C.END}".ljust(
                70
            )
            + f"{border_color}║{C.END}"
        )
    ...
    if branch.has_contacted_other_branches:
        lines.append(
            f"{border_color}║{C.END} {C.PURPLE}★ Has made contact with other branches!{C.END}".ljust(
                70
            )
            + f"{border_color}║{C.END}"
        )
...
def render_branch_tree(
    branches: List[UniverseBranch], your_branch: UniverseBranch
) -> str:
...
""")
    ...
    time.sleep(0.5)
    slow_print(
        f"{C.DIM}    Every moment, infinite versions of you diverge...{C.END}", 0.03
    )
    time.sleep(1)
...
def main():
    ...
    print(
        f"    {C.WHITE}Total stars across all branches:{C.END} {format_big_number(total_stars)}"
    )
    print(f"    {C.WHITE}Total civilizations:{C.END} {format_big_number(total_civs)}")
    print(
        f"    {C.WHITE}Branches where you exist:{C.END} ~{random.randint(2, 5)} of 10"
    )
    print(
        f"    {C.WHITE}Branches where you're already dead:{C.END} ~{random.randint(1, 3)} of 10"
    )
    print(
        f"    {C.WHITE}Branches where you were never born:{C.END} ~{random.randint(3, 6)} of 10"
    )
