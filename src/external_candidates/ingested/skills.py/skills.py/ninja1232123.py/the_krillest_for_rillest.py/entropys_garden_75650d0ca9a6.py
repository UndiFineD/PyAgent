...
class Garden:
    ...
    def step(self):
        ...
        new_grid = [
            [CellState.VOID for _ in range(self.width)] for _ in range(self.height)
        ]
    ...
    def render(self) -> str:
        ...
        lines = []
        lines.append(
            f"    {GardenColors.SILVER}╔{'═' * (self.width + 2)}╗{GardenColors.RESET}"
        )
        ...
        decay_count = sum(
            1 for row in self.grid for cell in row if cell == CellState.DECAY
        )
...
""")
    ...
    for thought in random.sample(ENTROPY_THOUGHTS, 4):
        print(
            f"        {GardenColors.DIM}─────────────────────────────────────────{GardenColors.RESET}"
        )
...
""")
    ...
    print(
        f"    {GardenColors.ICE}Too much order: frozen, predictable, dead.{GardenColors.RESET}"
    )
    pause(1)
    frozen = "    ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
    ...
    print()
    print(
        f"    {GardenColors.EMBER}Too much chaos: random, meaningless, noise.{GardenColors.RESET}"
    )
    pause(1)
    ...
    print()
    print(
        f"    {GardenColors.BLOOM}The edge: patterns emerging, dancing, alive.{GardenColors.RESET}"
    )
    pause(1)
    ...
    for _ in range(10):
        ...
        for i in range(48):
            ...
            if val > 0.5:
                ...
                edge += " "
        print(
            f"\r    {GardenColors.BLOOM}{edge}{GardenColors.RESET}", end="", flush=True
        )
        time.sleep(0.15)
    ...
    try:
        ...
        while True:
            ...
            try:
                choice = (
                    input(
                        f"    {GardenColors.SPROUT}Plant your intention: {GardenColors.RESET}"
                    )
                    .strip()
                    .lower()
                )
