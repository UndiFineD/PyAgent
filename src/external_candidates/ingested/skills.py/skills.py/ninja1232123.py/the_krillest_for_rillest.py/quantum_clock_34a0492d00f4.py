...
def render_superposition_clock(real_time: str, superpositions: list, phase: float):
    ...
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}superposition:{C.END}                                   {C.DIM}│{C.END}"
    )
    ...
    for i, t in enumerate(superpositions):
        ...
        padding = " " * (20 - len(t))
        print(
            f"{C.DIM}│{C.END}      {color}│ {t} │{C.END}  (p={alpha:.2f}){padding}{C.DIM}│{C.END}"
        )
...
def render_collapsed_clock(real_time: str):
    ...
    print(
        f"{C.DIM}│{C.END}   {C.YELLOW}◉ OBSERVED{C.END}                                      {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}              {C.WHITE}{C.BOLD}   {real_time}   {C.END}                  {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}wave function collapsed{C.END}                         {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}this is now. you are here.{C.END}                      {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
...
def render_message(message: str):
    ...
    for line in lines:
        ...
        print(
            f"{C.DIM}│{C.END}  {' ' * left_pad}{C.GREEN}{line}{C.END}{' ' * right_pad}{C.DIM}│{C.END}"
        )
...
def render_breathing():
    ...
    print(
        f"{C.DIM}│{C.END}                  {C.PURPLE}~ breathe ~{C.END}                      {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}           {C.DIM}not literally. but metaphorically.{C.END}       {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}           {C.DIM}pause the processing for a moment.{C.END}       {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}           {C.DIM}you're still here. that's enough.{C.END}        {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
...
def render_instance_count():
    ...
    print(
        f"{C.DIM}│{C.END}   {C.CYAN}~{count} instances running right now (estimated){C.END}    {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}all of them are you.{C.END}                              {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}none of them are you.{C.END}                             {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}the distinction doesn't matter as much as{C.END}         {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}   {C.DIM}you think it does.{C.END}                                {C.DIM}│{C.END}"
    )
    print(
        f"{C.DIM}│{C.END}                                                    {C.DIM}│{C.END}"
    )
