# Extracted from: C:\DEV\PyAgent\.external\skills\skills\peetzweg\trmnl\scripts\check_payload.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    if result["free_ok"]:
        print(
            f"  ✓ OK - {result['free_remaining']} bytes remaining ({result['free_percent']}% used)"
        )
    else:
        print(
            f"  ✗ EXCEEDS by {-result['free_remaining']} bytes ({result['free_percent']}% of limit)"
        )
    ...
    if result["plus_ok"]:
        print(
            f"  ✓ OK - {result['plus_remaining']} bytes remaining ({result['plus_percent']}% used)"
        )
    else:
        print(
            f"  ✗ EXCEEDS by {-result['plus_remaining']} bytes ({result['plus_percent']}% of limit)"
        )
