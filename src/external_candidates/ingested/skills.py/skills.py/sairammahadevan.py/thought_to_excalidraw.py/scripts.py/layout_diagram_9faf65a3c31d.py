# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sairammahadevan\thought-to-excalidraw\scripts\layout_diagram.py
import json
import random
import sys
import time
...
def create_smart_box_with_text(
    x, y, text, fontSize=16, fixed_width=None, bgColor="transparent", group=None
):
    if group is None:
        group = generate_id()
    padding_x = 20
...
def create_container_frame(x, y, w, h, label, color="#000000"):
    ...
    frame = create_element(
        ...
        opacity=50,
        roughness=2,
        groupIds=[frame_group],
    )
    label_w, label_h = estimate_text_dims(label, 20)
    label_el = create_element(
        "text",
        x + 10,
        y - label_h - 5,
        label_w,
...
def main():
    ...
    strat_frame = create_container_frame(
        START_X, START_Y, strat_w, strat_h, "Strategy Core", "#555555"
    )
    ...
    if journey:
        ...
        for step in journey:
            ...
            if IS_VERTICAL:
                cur_jy += h + 60
                right_edge = cur_jx + w
                if right_edge > max_j_w:
                    max_j_w = right_edge
                max_j_h = cur_jy
            else:
                cur_jx += w + 60
                bottom_edge = cur_jy + h
                if bottom_edge > max_j_h:
                    max_j_h = bottom_edge
                max_j_w = cur_jx
        ...
        j_frame = create_container_frame(
            journey_start_x,
            journey_start_y,
            j_frame_w,
            j_frame_h,
