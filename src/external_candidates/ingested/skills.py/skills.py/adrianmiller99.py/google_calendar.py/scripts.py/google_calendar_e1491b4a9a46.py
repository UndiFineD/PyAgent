# Extracted from: C:\DEV\PyAgent\.external\skills\skills\adrianmiller99\google-calendar\scripts\google_calendar.py
#!/usr/bin/env python3
import argparse
import json
import os
...
def add_event(args):
    ...
    if parsed.attendees:
        event["attendees"] = [
            {"email": e.strip()} for e in parsed.attendees.split(",") if e.strip()
        ]
