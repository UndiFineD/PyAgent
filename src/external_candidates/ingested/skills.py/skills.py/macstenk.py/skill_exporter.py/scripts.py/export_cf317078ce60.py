...
def parse_skill_md(skill_path: Path) -> dict:
    ...
    description = (
        desc_match.group(1).strip() if desc_match else "Exported skill service"
    )
    ...
    if scripts_dir.exists():
        for script in scripts_dir.glob("*.py"):
            ...
            env_matches = re.findall(
                r'os\.(?:environ|getenv)\s*\[?\s*["\']([A-Z_]+)["\']', content
            )
...
def export_skill(skill_path: Path, output_path: Path, target: str, llm: str, port: int):
    ...
    dockerfile = TEMPLATES["Dockerfile"].format(
        system_deps=build_dockerfile_deps(deps["system_deps"]), port=port
    )
    ...
    compose = TEMPLATES["docker-compose.yml"].format(
        skill_name=skill_info["name"].replace("-", "_"), port=port
    )
    ...
    if target == "railway":
        ...
        fly_toml = TEMPLATES["fly.toml"].format(
            skill_name=skill_info["name"], port=port
        )
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", help="Output directory (default: ./<skill-name>-service)"
    )
    parser.add_argument(
        "--target",
        "-t",
    ...
    parser.add_argument(
        "--port", "-p", type=int, default=8000, help="API port (default: 8000)"
    )
