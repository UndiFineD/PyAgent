# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tanchunsiong\zoom-unofficial-community-skill\scripts\zoom.py
#!/usr/bin/env python3
"""
...
try:
    ...
    print(
        "ERROR: requests not installed. Run: pip3 install requests --break-system-packages",
        file=sys.stderr,
    )
    sys.exit(1)
...
def get_token():
    ...
    if not all([account_id, client_id, client_secret]):
        print(
            "ERROR: ZOOM_ACCOUNT_ID, ZOOM_CLIENT_ID, and ZOOM_CLIENT_SECRET must be set",
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        if not resp.ok:
            try:
                ...
                if (
                    ...
                    or "access token" in msg.lower()
                ):
                    print(
                        "HINT: Add the required scope to your Zoom Marketplace S2S app at https://marketplace.zoom.us/",
                        file=sys.stderr,
                    )
                if resp.status_code == 403 and "not been enabled" in msg.lower():
                    print(
                        "HINT: This feature is not enabled on your Zoom account. Check your Zoom plan/settings.",
                        file=sys.stderr,
                    )
            except Exception:
...
def cmd_meetings_live(args):
    ...
    for m in meetings:
        ...
        print(f"    Host: {host}")
        print(
            f"    Participants: {participants} | Started: {start} | Duration: {duration}"
        )
        print(
            f"    Video: {has_video} | Screen share: {has_screen} | Recording: {has_recording}"
        )
        print()
...
def _get_live_meeting_host_user_id(meeting_id):
    ...
    for m in data.get("meetings", []):
        if str(m.get("id")) == str(meeting_id):
            ...
            if not host_email:
                print(
                    "ERROR: Could not find host email for this meeting", file=sys.stderr
                )
                sys.exit(1)
...
def cmd_meetings_rtms_start(args):
    ...
    if not rtms_client_id:
        print(
            "ERROR: ZOOM_RTMS_CLIENT_ID must be set (Client ID of your RTMS Marketplace app)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    api(
        "PATCH",
        f"/live_meetings/{args.meeting_id}/rtms_app/status",
        json={
            "action": "start",
            "settings": {
...
def cmd_recordings_download_transcript(args):
    ...
    transcript_files = [
        f
        for f in files
        if f.get("recording_type")
        in ("audio_transcript", "chat_file", "timeline", "closed_caption")
    ]
    # Also check for separate transcript files
    transcript_files += [
        f for f in files if f.get("file_extension", "").lower() in ("vtt", "txt", "srt")
    ]
...
def cmd_recordings_download_summary(args):
    ...
    lines.append(f"")
    lines.append(
        f"- **Date:** {data.get('meeting_start_time', '?')} — {data.get('meeting_end_time', '?')}"
    )
...
def cmd_users_list(args):
    ...
    for u in data.get("users", []):
        print(
            f"  {u.get('email')} — {u.get('first_name')} {u.get('last_name')} (type={u.get('type')})"
        )
...
def cmd_chat_messages(args):
    ...
    data = api(
        "GET",
        f"/chat/users/{_user_id()}/messages",
        params={**params, "to_channel": args.channel_id},
    )
...
def cmd_chat_contacts(args):
    ...
    for c in data.get("contacts", []):
        print(
            f"  {c.get('email', '?')} — {c.get('first_name', '')} {c.get('last_name', '')}"
        )
...
def cmd_summary_get(args):
    ...
    if isinstance(summary, dict):
        ...
        if not overview and not details and not next_steps:
            print(
                "(Summary metadata exists but no content available — meeting may have been too short)"
            )
...
def main():
    ...
    p.add_argument(
        "--join-before-host",
        dest="join_before_host",
        type=lambda x: x.lower() in ("true", "1", "yes"),
        default=None,
    )
    p.add_argument(
        "--auto-recording", dest="auto_recording", choices=["local", "cloud", "none"]
    )
    p.add_argument(
        "--waiting-room",
        dest="waiting_room",
