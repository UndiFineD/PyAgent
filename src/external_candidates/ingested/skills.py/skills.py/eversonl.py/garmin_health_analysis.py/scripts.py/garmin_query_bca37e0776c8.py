# Extracted from: C:\DEV\PyAgent\.external\skills\skills\eversonl\garmin-health-analysis\scripts\garmin_query.py
#!/usr/bin/env python3
"""
    ...
    def get_timestamp(item):
        if timestamp_key in item:
            ...
            if isinstance(start_gmt, str):
                return int(
                    datetime.fromisoformat(start_gmt.replace("Z", "+00:00")).timestamp()
                )
            else:
...
def query_heart_rate_at_time(client, time_str, date_str=None):
    ...
    try:
        ...
        if hr_array and isinstance(hr_array[0], list):
            hr_array = [
                {"startTimeInSeconds": ts // 1000, "heartRateValue": val}
                for ts, val in hr_array
            ]
...
def query_body_battery_at_time(client, time_str, date_str=None):
    ...
    try:
        ...
        bb_dicts = [
            {"startTimeInSeconds": ts // 1000, "value": val} for ts, val in bb_values
        ]
...
def main():
    ...
    parser.add_argument(
        "metric",
        choices=["heart_rate", "stress", "body_battery", "steps"],
        help="Metric to query",
    )
    parser.add_argument(
        "time", help="Time (e.g., '3:00 PM', '15:00', '2024-01-15 14:30')"
    )
