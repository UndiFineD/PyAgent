# Extracted from: C:\DEV\PyAgent\.external\skills\skills\eversonl\garmin-health-analysis\scripts\garmin_auth.py
#!/usr/bin/env python3
"""
...
def login(email, password):
    ...
    try:
        ...
        try:
            ...
            print(
                f"✅ Login successful! User: {profile.get('displayName', 'Unknown')}",
                file=sys.stderr,
            )
        except Exception as e:
            print(
                f"✅ Login successful! (Unable to fetch profile: {e})", file=sys.stderr
            )
...
def check_status():
    ...
    if client:
        try:
            profile = client.get_user_summary(datetime.now().strftime("%Y-%m-%d"))
            print(
                f"✅ Authentication valid! User: {profile.get('displayName', 'Unknown')}",
                file=sys.stderr,
            )
            return True
        except Exception as e:
...
def main():
    ...
    login_parser.add_argument(
        "--email", help="Garmin account email (or set via env/config)"
    )
    login_parser.add_argument(
        "--password", help="Garmin account password (or set via env/config)"
    )
    ...
    if args.command == "login":
        ...
        if not email or not password:
            ...
            print(
                "  2. Config: create config.json from config.example.json",
                file=sys.stderr,
            )
