# Extracted from: C:\DEV\PyAgent\.external\skills\skills\eversonl\garmin-health-analysis\scripts\garmin_data.py
#!/usr/bin/env python3
"""
...
try:
    ...
    print(
        '{"error": "garminconnect not installed. Run: pip3 install garminconnect"}',
        file=sys.stderr,
    )
    sys.exit(1)
                        ...
                        sleep_data.append(
                            {
                                ...
                                "light_sleep_seconds": sleep_dto.get(
                                    "lightSleepSeconds"
                                ),
                    ...
                    hrv_data.append(
                        {
                            ...
                            "baseline_balanced_high": summary.get(
                                "baselineBalancedHigh"
                            ),
        ...
        while current <= end:
            ...
            try:
                ...
                if data and len(data) > 0:
                    ...
                    values = [
                        v[1] for v in values_array if len(v) > 1
...
def fetch_activities(client, days=7, start=None, end=None):
    ...
    try:
        ...
        activities = client.get_activities_by_date(
            start_date, end_date, activitytype=""
        )
...
def fetch_summary(client, days=7, start=None, end=None):
    ...
    try:
        ...
        sleep_times = [
            s.get("sleep_time_seconds") for s in sleep if s.get("sleep_time_seconds")
        ]
        avg_sleep_hours = (
            (sum(sleep_times) / len(sleep_times) / 3600) if sleep_times else 0
        )

        sleep_scores = [
            s.get("sleep_score") for s in sleep if s.get("sleep_score") is not None
        ]
        avg_sleep_score = (sum(sleep_scores) / len(sleep_scores)) if sleep_scores else 0

        hrv_values = [
            h.get("last_night_avg") for h in hrv if h.get("last_night_avg") is not None
        ]
        avg_hrv = (sum(hrv_values) / len(hrv_values)) if hrv_values else 0

        rhr_values = [
            h.get("resting_hr") for h in hr if h.get("resting_hr") is not None
        ]
        avg_rhr = (sum(rhr_values) / len(rhr_values)) if rhr_values else 0

        bb_charged_values = [
            b.get("charged") for b in bb if b.get("charged") is not None
        ]
        avg_bb_charged = (
            (sum(bb_charged_values) / len(bb_charged_values))
            if bb_charged_values
            else 0
        )
        ...
        return {
            "summary": {
                ...
                "total_calories": sum(
                    a.get("calories", 0) for a in activities if a.get("calories")
                ),
            },
            "sleep": sleep,
            "hrv": hrv,
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=7, help="Number of days to fetch (default: 7)"
    )
    ...
    if not client:
        print(
            '{"error": "Not authenticated. Run: python3 scripts/garmin_auth.py login --email YOUR_EMAIL --password YOUR_PASSWORD"}'
        )
        sys.exit(1)
