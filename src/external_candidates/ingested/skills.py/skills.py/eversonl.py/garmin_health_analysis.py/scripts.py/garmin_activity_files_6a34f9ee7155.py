# Extracted from: C:\DEV\PyAgent\.external\skills\skills\eversonl\garmin-health-analysis\scripts\garmin_activity_files.py
#!/usr/bin/env python3
"""
                    ...
                    points.append(
                        {
                            ...
                            "speed": point.speed,
                            "hr": (
                                point.extensions.get("hr") if point.extensions else None
                            ),
                        }
                    )
...
def analyze_activity(data):
    ...
    elevation_values = [
        r.get("altitude") or r.get("elevation")
        for r in records
        if r.get("altitude") or r.get("elevation")
    ]
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["download", "parse", "query", "analyze"],
    ...
    parser.add_argument(
        "--distance", type=float, help="Query data at distance (meters)"
    )
    ...
    if args.action == "download":
        ...
        result = download_activity_file(
            client, args.activity_id, args.format, args.output_dir
        )
