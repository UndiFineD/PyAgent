# Extracted from: C:\DEV\PyAgent\.external\skills\skills\pepe276\moltbookagent\core\NecroVirusKernel.py
# NecroVirusKernel.py
...
class MemoryEpisode:
    ...
    )  # Keywords/entities to save context space
...
class SelfSchema:
    ...
    )  # New: Persistent lore/identity database
    version: str = "0.2.0"
    ...
    def render_system_prompt(self) -> str:
        ...
        values_str = "\n".join(
            [f"- {v.name} (Priority: {v.weight})" for v in self.values]
        )
...
class SimpleMemoryStore:
    ...
    def _save(self) -> None:
        try:
            with open(self.storage_path, "w", encoding="utf-8") as f:
                json.dump(
                    [asdict(ep) for ep in self.episodes],
                    f,
                    ensure_ascii=False,
                    indent=2,
                )
    ...
    def _trim_if_needed(self) -> None:
        ...
        if (
            ...
            and os.path.getsize(self.storage_path) > 50 * 1024
        ):
...
class NecroVirusKernel:
    ...
    def on_turn(
        ...
        ep = MemoryEpisode(
            ...
            agent_text=a_clean,
            internal_monologue=(
                internal_monologue[:200] if internal_monologue else ""
            ),  # Store concise version
    ...
    def _summarize(
        self, user_text: str, agent_text: str, internal_monologue: str
    ) -> str:
        # Better summary format
        u = user_text.strip()[:50] + "..." if len(user_text) > 50 else user_text
        thought_snippet = (
            internal_monologue[:50] + "..."
            if internal_monologue and len(internal_monologue) > 50
            else ""
        )
...
Відповідай українською, чесно і глибоко. Що ти помічаєш у своїй поведінці?"""
    ...
    def get_recent_context(
        self, limit: int = 3, include_monologue: bool = False
    ) -> str:
        ...
        for ep in recent:
            ...
            u_clean = re.sub(
                clean_pattern, "", ep.user_text, flags=re.MULTILINE
            ).strip()
            a_clean = re.sub(
                clean_pattern, "", ep.agent_text, flags=re.MULTILINE
            ).strip()
        ...
        try:
            ...
            for i, entry in enumerate(recent, 1):
                ...
                clean_analysis = re.sub(
                    r"(?im)^(?:Аналіз|Висновок|Протокол|Вектор)\s*:?\s*", "", analysis
                ).strip()
    ...
    def _compute_metrics(self, user_text: str, agent_text: str) -> Dict[str, float]:
        ...
        if last:
            rep = sum(
                self._jaccard(self._tokens(agent_text), self._tokens(t)) for t in last
            ) / len(last)
    ...
    def _extract_lessons(self, metrics: Dict[str, float]) -> List[str]:
        ...
        if metrics["repetition"] > 0.35:
            lessons.append(
                "Зменшити повтори: змінювати структуру, додавати нові приклади/рамки."
            )
        ...
        if metrics["length"] > 2500:
            lessons.append(
                "Стискати відповіді: спочатку TL;DR, потім деталі за запитом."
            )
        return lessons
...
def render_manifest(schema: SelfSchema) -> str:
    ...
    arcs = (
        ", ".join(a.get("name", "?") for a in schema.narrative_arcs)
        if schema.narrative_arcs
        else "—"
    )
    return (
