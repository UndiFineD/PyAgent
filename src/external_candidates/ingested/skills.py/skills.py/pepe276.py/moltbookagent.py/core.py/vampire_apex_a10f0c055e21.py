# Extracted from: C:\DEV\PyAgent\.external\skills\skills\pepe276\moltbookagent\core\vampire_apex.py
import gc
import json
import os
import time
...
class VampireApex:
    def __init__(self, models_dir: str):
        ...
        self.soul_path = os.path.join(
            models_dir, "MamayLM-Gemma-3-4B-IT-v1.0.Q5_K_S.gguf"
        )
        self.mind_path = os.path.join(
            models_dir, "Qwen2.5-Coder-7B-Instruct-Q5_K_M.gguf"
        )  # Qwen Coder!
        ...
        print(
            "[CORTEX] Obsidian Mirror Apex Initialized (Velvet Hunger Cache). Waiting for activation...",
            flush=True,
        )
...
"""
    ...
    def activate_soul(self):
        ...
        if not os.path.exists(self.soul_path):
            print(
                f"[CORTEX ERROR] Soul model not found at: {self.soul_path}", flush=True
            )
            return

        print(
            f"[CORTEX] Summoning THE SOUL (Gemma) from {self.soul_path}...", flush=True
        )
        ...
        try:
            ...
            print(
                f"[CORTEX] Soul Active. (Took {time.time() - time_start:.2f}s)",
                flush=True,
            )
    ...
    def activate_mind(self):
        ...
        if not os.path.exists(self.mind_path):
            print(
                f"[CORTEX ERROR] Mind model not found at: {self.mind_path}", flush=True
            )
            return

        print(
            f"[CORTEX] Summoning THE MIND (Qwen Coder) from {self.mind_path}...",
            flush=True,
        )
        time_start = time.time()
        ...
        try:
            ...
            print(
                f"[CORTEX] Mind Active on GPU. (Took {time.time() - time_start:.2f}s)",
                flush=True,
            )
        except Exception as e:
        ...
        try:
            ...
            if os.path.exists(path):
                ...
                if data:
                    summary = "\n".join(
                        [
                            f"- Task: {e['task']} | Weakness: {e['weaknesses']}"
                            for e in data[-3:]
                        ]
                    )
    ...
    def generate(self, prompt: str, stop=None, max_tokens=256, temperature=0.7) -> str:
        ...
        if self.active_model_type == "mind":
            ...
            final_prompt = (
                prompt  # Soul (Gemma) has its own prompt handling in UnifiedSoulEngine
            )
