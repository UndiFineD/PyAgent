# Extracted from: C:\DEV\PyAgent\.external\skills\skills\pepe276\moltbookagent\vampire_scanner.py
import os
import re
...
def extract_essence():
    ...
    for root, dirs, files in os.walk(TARGET_PATH):
        if "node_modules" in dirs:
            dirs.remove("node_modules")  # Пропускаємо сміття Архонтів
                ...
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        ...
                        for key, pattern in PATTERNS.items():
                            ...
                            if matches:
                                findings.append(
                                    {"file": file, "type": key, "data": matches}
                                )
                except:
                    continue
