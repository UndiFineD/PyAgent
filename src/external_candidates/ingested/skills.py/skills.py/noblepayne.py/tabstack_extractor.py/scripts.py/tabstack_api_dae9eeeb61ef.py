...
def extract_json(
    url: str, schema: Dict[str, Any], api_key: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if not api_key:
        raise ValueError(
            "Tabstack API key required. Set TABSTACK_API_KEY environment variable."
        )
    ...
    response = requests.post(
        f"{TABSTACK_BASE_URL}/extract/json", headers=headers, json=payload, timeout=30
    )
