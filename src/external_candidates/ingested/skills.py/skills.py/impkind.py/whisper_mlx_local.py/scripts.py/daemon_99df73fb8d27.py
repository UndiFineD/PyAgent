# Extracted from: C:\DEV\PyAgent\.external\skills\skills\impkind\whisper-mlx-local\scripts\daemon.py
#!/usr/bin/env python3
"""
    ...
    def handle_transcribe(self):
        ...
        try:
            ...
            if "multipart/form-data" in content_type:
                self.send_json(
                    {"error": "Use application/octet-stream or send file path as JSON"},
                    400,
                )
                stats["failed"] += 1
                return
            ...
            if translate and not transcriber.translation_mode:
                ...
                try:
                    ...
                    logger.warning(
                        f"Translation mode failed, falling back to transcription: {e}"
                    )
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=int(os.getenv("CLAWD_WHISPER_PORT", "8787"))
    )
    parser.add_argument("--backend", default=os.getenv("CLAWD_WHISPER_BACKEND", "auto"))
    parser.add_argument(
        "--model", default=os.getenv("CLAWD_WHISPER_MODEL", "distil-large-v3")
    )
    parser.add_argument(
        "--translate", action="store_true", help="Translation mode (to English)"
    )
    parser.add_argument(
        "--no-warmup", action="store_true", help="Skip warmup transcription"
    )
