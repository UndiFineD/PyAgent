# Extracted from: C:\DEV\PyAgent\.external\skills\skills\oswalpalash\causal-inference\scripts\estimate_effect.py
#!/usr/bin/env python3
"""
...
def estimate_ate(actions: list, treatment_var: str, outcome_var: str, values: list):
    ...
    for action in actions:
        ...
        if isinstance(outcome, bool):
            ...
            outcome = (
                1.0 if outcome.lower() in ("true", "yes", "1", "positive") else 0.0
            )
    ...
    # Calculate ATE
    if (
        results[values[0]]["mean"] is not None
        and results[values[1]]["mean"] is not None
    ):
...
def main():
    ...
    parser.add_argument(
        "--treatment", "-t", required=True, help="Treatment variable name"
    )
    ...
    parser.add_argument(
        ...
        "-v",
        required=True,
        help="Treatment values to compare (comma-separated)",
    )
    parser.add_argument(
        "--log-path", "-l", default=DEFAULT_LOG_PATH, help="Action log path"
    )
