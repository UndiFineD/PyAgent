...
def find_mail_db():
    ...
    for v in [11, 10, 9]:
        db_path = (
            Path.home() / "Library" / "Mail" / f"V{v}" / "MailData" / "Envelope Index"
        )
        if db_path.exists():
            return str(db_path)
    return None
    ...
    if msg_info["remote_id"] and msg_info["mailbox_url"]:
        ...
        if msg_info["mailbox_url"].startswith("imap://"):
            ...
            if len(parts) >= 1:
                ...
                emlx_file = find_emlx_file(
                    mail_dir, account_id, None, msg_info["remote_id"]
                )
                ...
                if emlx_file:
                    try:
                        ...
                        print(
                            f"Warning: Could not parse emlx file: {e}", file=sys.stderr
                        )
