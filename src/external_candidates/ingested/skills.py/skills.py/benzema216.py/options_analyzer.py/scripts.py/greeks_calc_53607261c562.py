# Extracted from: C:\DEV\PyAgent\.external\skills\skills\benzema216\options-analyzer\scripts\greeks_calc.py
#!/usr/bin/env python3
"""
...
class BlackScholes:
    ...
    def price(self) -> float:
        ...
        if self.option_type == "call":
            return self.S * norm.cdf(self.d1) - self.K * math.exp(
                -self.r * self.T
            ) * norm.cdf(self.d2)
        else:
            return self.K * math.exp(-self.r * self.T) * norm.cdf(
                -self.d2
    ...
    def rho(self) -> float:
        ...
        if self.option_type == "call":
            return (
                self.K * self.T * math.exp(-self.r * self.T) * norm.cdf(self.d2) / 100
            )
        else:
            return (
                -self.K * self.T * math.exp(-self.r * self.T) * norm.cdf(-self.d2) / 100
...
def calculate_greeks_mibian(
    spot: float, strike: float, dte: int, rate: float, iv: float, option_type: str
) -> dict:
...
def get_from_symbol(
    symbol: str, strike: float, expiry: str, option_type: str, rate: float = 5.0
) -> dict:
...
def format_output(greeks: dict, format_type: str = "md") -> str:
    ...
    if "symbol" in greeks:
        ...
        if "market_price" in greeks:
            lines.append(
                f"**市场价格**: ${greeks['market_price']} (Bid: ${greeks['bid']}, Ask: ${greeks['ask']})"
            )
    ...
    lines.append("|-------|-----|------|")
    lines.append(
        f"| **Delta** | {greeks['delta']:.4f} | 标的涨$1，期权涨${abs(greeks['delta']):.2f} |"
    )
    lines.append(f"| **Gamma** | {greeks['gamma']:.4f} | Delta的变化率 |")
    lines.append(
        f"| **Theta** | {greeks['theta']:.4f} | 每天损失${abs(greeks['theta']):.2f} |"
    )
    lines.append(
        f"| **Vega** | {greeks['vega']:.4f} | IV涨1%，期权涨${greeks['vega']:.2f} |"
    )
    lines.append(
        f"| **Rho** | {greeks['rho']:.4f} | 利率涨1%，期权涨${greeks['rho']:.2f} |"
    )
...
def main():
    ...
    parser.add_argument(
        "--expiry", "-e", help="到期日 (YYYY-MM-DD，使用 --symbol 时需要)"
    )
    parser.add_argument("--dte", "-d", type=int, help="距到期天数")
    parser.add_argument(
        "--rate", "-r", type=float, default=5.0, help="无风险利率 (百分比，默认5)"
    )
    parser.add_argument("--iv", type=float, help="隐含波动率 (百分比，如25表示25%%)")
    parser.add_argument(
        "--type", "-t", choices=["call", "put"], default="call", help="期权类型"
    )
    parser.add_argument(
        "--format", "-f", choices=["json", "md"], default="md", help="输出格式"
    )
    ...
    try:
        if args.symbol:
            ...
            greeks = get_from_symbol(
                args.symbol.upper(), args.strike, args.expiry, args.type, args.rate
            )
        else:
            # 手动输入参数
            ...
            if args.use_mibian and USE_MIBIAN:
                greeks = calculate_greeks_mibian(
                    args.spot, args.strike, args.dte, args.rate, args.iv, args.type
                )
            else:
                bs = BlackScholes(
                    args.spot, args.strike, args.dte, args.rate, args.iv, args.type
