...
def format_markdown(data: dict) -> str:
    ...
    if data["iv_rank"] is not None:
        rank_emoji = (
            "🔴" if data["iv_rank"] >= 70 else "🟢" if data["iv_rank"] <= 30 else "🟡"
        )
        lines.append(
            f"| **IV Rank** | {data['iv_rank']:.1f}% {rank_emoji} | 当前IV在52周的位置 |"
        )

    if data["iv_percentile"] is not None:
        lines.append(
            f"| **IV Percentile** | {data['iv_percentile']:.1f}% | 超过多少历史数据 |"
        )
    ...
    if data["iv_hv_premium"] is not None:
        ...
        lines.append(
            f"| **IV-HV Premium** | {premium_sign}{data['iv_hv_premium']:.1f}% |"
        )
    ...
    lines.append(f"\n## 💡 策略建议")
    env_emoji = {"HIGH_IV": "🔴", "LOW_IV": "🟢", "NORMAL_IV": "🟡"}.get(
        data["iv_environment"], "⚪"
    )
