# Extracted from: C:\DEV\PyAgent\.external\skills\skills\benzema216\options-analyzer\scripts\strategy_analyzer.py
#!/usr/bin/env python3
"""
...
class StrategyAnalyzer:
    ...
    def payoff_curve(
        self, price_range: Tuple[float, float] = None, points: int = 100
    ) -> List[dict]:
        """生成盈亏曲线数据"""
        ...
        return [
            {"price": round(p, 2), "pnl": round(self.payoff_at_price(p), 2)}
            for p in prices
        ]
    ...
    def analyze(self) -> dict:
        ...
        return {
            ...
            "max_profit_value": (
                max_profit_val if max_profit_val != float("inf") else None
            ),
            "max_loss": max_loss_str,
            "max_loss_value": max_loss_val if max_loss_val != float("-inf") else None,
            "breakevens": breakevens,
            "risk_reward_ratio": (
                round(rr_ratio, 2) if rr_ratio and rr_ratio != float("inf") else None
            ),
...
def determine_position(
    strategy: str, index: int, opt_type: str, strike: float, all_parts: list
) -> str:
...
def format_markdown(result: dict, strategy_name: str = None) -> str:
    ...
    title = (
        StrategyAnalyzer.STRATEGIES.get(strategy_name, {}).get("desc", "期权策略")
        if strategy_name
        else "期权策略"
    )
    ...
    for i, leg in enumerate(result["legs"], 1):
        ...
        lines.append(
            f"| {i} | ${leg['strike']} | {opt_type} | {direction} | ${leg['premium']:.2f} |"
        )

    lines.append(
        f"\n**净权利金**: ${result['net_premium']:,.2f}"
        + (" (收入)" if result["net_premium"] > 0 else " (支出)")
    )
...
def main():
    ...
    parser.add_argument(
        "--strategy",
        "-s",
        choices=list(StrategyAnalyzer.STRATEGIES.keys()),
        help="策略类型",
    )
