# Extracted from: C:\DEV\PyAgent\.external\skills\skills\benzema216\options-analyzer\scripts\options_chain.py
#!/usr/bin/env python3
"""
...
def get_options_chain(
    symbol: str, expiry: Optional[str] = None, strike_range: float = 10
) -> dict:
    """获取期权链数据"""
    ...
    if expiry:
        if expiry not in expirations:
            ...
            closest = min(
                expirations,
                key=lambda x: abs(datetime.strptime(x, "%Y-%m-%d") - expiry_date),
            )
            print(
                f"⚠️ 指定到期日 {expiry} 不可用，使用最接近的: {closest}",
                file=sys.stderr,
            )
            expiry = closest
    else:
...
def format_markdown(data: dict) -> str:
    ...
    if data["calls"]:
        ...
        for c in data["calls"]:
            itm = "✅" if c["ITM"] else ""
            lines.append(
                f"| {c['Strike']} | {c['Bid']:.2f} | {c['Ask']:.2f} | {c['Last']:.2f} | {c['Volume']} | {c['OI']} | {c['IV']:.1f} | {itm} |"
            )
    ...
    if data["puts"]:
        ...
        for p in data["puts"]:
            itm = "✅" if p["ITM"] else ""
            lines.append(
                f"| {p['Strike']} | {p['Bid']:.2f} | {p['Ask']:.2f} | {p['Last']:.2f} | {p['Volume']} | {p['OI']} | {p['IV']:.1f} | {itm} |"
            )
