# Extracted from: C:\DEV\PyAgent\.external\skills\skills\joe-rlo\memory-pipeline\scripts\ingest-chatgpt.py
#!/usr/bin/env python3
"""
    ...
    while current_id and current_id not in visited:
        ...
        if msg and msg.get("content"):
            ...
            if content.get("content_type") == "text" and content.get("parts"):
                text = "\n".join(
                    str(p) for p in content["parts"] if isinstance(p, str) and p.strip()
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Min user messages to keep (default: 2)",
    )
    parser.add_argument(
        "--min-length",
        type=int,
        default=200,
        help="Min total chars to keep (default: 200)",
    )
    parser.add_argument(
        "--keep-all", action="store_true", help="Keep all conversations"
    )
    parser.add_argument(
    ...
    EXCLUDE_PATTERNS = [
        ...
        r"\bnih\b",
        r"\br01\b",
        r"school.based.health",
        r"\bchc\b",
        r"flovent",
        r"albuterol",
        r"intervention.fidelity",
        r"umass",
        r"glp-1",
        r"cognitive.testing",
        r"research.letter",
    ...
    for conv in conversations:
        ...
        check_text = (
            title + " " + " ".join(m[1][:500] for m in messages[:6] if m[0] == "user")
        )
