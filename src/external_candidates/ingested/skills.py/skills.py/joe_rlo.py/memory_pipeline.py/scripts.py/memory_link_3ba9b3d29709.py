# Extracted from: C:\DEV\PyAgent\.external\skills\skills\joe-rlo\memory-pipeline\scripts\memory-link.py
#!/usr/bin/env python3
"""
...
def generate_summary(graph: Dict) -> str:
    ...
    for subject, nodes in sorted_subjects[:20]:  # Top 20 subjects
        ...
        for node in nodes[:5]:  # Top 5 facts per subject
            ...
            summary.append(
                f"- **[{node['type']}]** {node['content'][:100]}...{valid_status}\n"
            )
            summary.append(
                f"  - Date: {node['date']} | Confidence: {node['confidence']:.2f}\n"
            )
...
def main():
    ...
    avg_links = (
        sum(len(node["links"]) for node in graph["nodes"]) / len(graph["nodes"])
        if graph["nodes"]
        else 0
    )
