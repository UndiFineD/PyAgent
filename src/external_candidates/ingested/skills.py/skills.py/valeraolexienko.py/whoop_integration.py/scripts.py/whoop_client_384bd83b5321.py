# Extracted from: C:\DEV\PyAgent\.external\skills\skills\valeraolexienko\whoop-integration\scripts\whoop_client.py
#!/usr/bin/env python3
"""
WHOOP API Client
...
def get_whoop_credentials():
    ...
    if not client_id or not client_secret:
        try:
            ...
            skill_config = (
                config.get("skills", {}).get("entries", {}).get("whoop-integration", {})
            )

            if not client_id:
                client_id = skill_config.get("clientId")
            if not client_secret:
                client_secret = skill_config.get("clientSecret") or skill_config.get(
                    "apiKey"
                )  # OpenClaw stores as apiKey
...
class WhoopClient:
    ...
    def get_latest_sleep(self) -> Optional[Dict]:
        ...
        params = {"start": start_time, "end": end_time, "limit": 1}
    ...
    def get_sleep_performance_summary(self) -> Dict:
        ...
        if sleep_data and "score" in sleep_data:
            ...
            if "stage_summary" in score:
                ...
                summary["sleep_duration_hours"] = round(
                    total_sleep_ms / (1000 * 60 * 60), 1
                )
