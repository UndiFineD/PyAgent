# Extracted from: C:\DEV\PyAgent\.external\skills\skills\chocomintx\xiaohongshutools\scripts\request\web\encrypt\header\X_S_Common.py
import json
import urllib.parse
...
class XHS_XSC_Encrypt:
    ...
    def __diy_mrc(e):
        ...
        for i in range(255, -1, -1):
            ...
            for _ in range(8, 0, -1):
                j = (
                    unsigned_right_shift(j, 1) ^ 0xEDB88320
                    if j & 1
                    else unsigned_right_shift(j, 1)
                )
    ...
    def encrypt_headers_xsc(self, cookie_a1: str, fp: dict) -> str:
        ...
        p_base64_encoded = b64_encode(
            encode_utf8(
                urllib.parse.quote(
                    json.dumps(source_text, separators=(",", ":"), ensure_ascii=False),
                    safe="-_.!~*'()",
                )
            ),
            self.__base64_table,
