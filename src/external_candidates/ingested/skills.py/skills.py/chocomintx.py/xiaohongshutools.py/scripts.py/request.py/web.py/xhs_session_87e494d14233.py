...
class XHS_Session:
    ...
    async def __init_cookies(self) -> None:
        ...
        self.__fp = self.__xhs_encrypt.get_fingerprint(
            cookies_dict, self._session.headers.get("User-Agent")
        )
    ...
    async def __set_websectiga_and_secPoisonId(self) -> None:
        ...
        self._session.cookie_jar.update_cookies(
            {
                "websectiga": self.__xhs_encrypt.gen_websectiga(
                    res_json.get("data").get("data")
                ),
    ...
    async def __handle_xhs_response_exceptions(
        self, response: aiohttp.ClientResponse, request_info: str
    ):
        """异常处理"""
        try:
        ...
        if "禁言" in msg_lower or "被禁言" in msg_lower:
            ...
            logger.warning(
                f"掉线 | {web_session} | web_session 登录超时 | {logger_info}"
            )
            ...
            logger.warning(
                f"过期 | {web_session} | web_session 没有权限访问 | {logger_info}"
            )
    ...
    async def request(
        ...
        **kwargs,
        ...
        # 日志信息预处理（还原原始格式）
        data_string = (
            f" | data:{json.dumps(request_data)[:200]}" if request_data else ""
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_catch_response_exception:
                    return await self.__handle_xhs_response_exceptions(
                ...
                logger.warning(
                    f"尝试第 {attempt + 1}/{max_retries} 次时出错: {request_info} - {e}"
                )
                ...
                if self._sid is None:
                    logger.error(
                        f"需要扫描二维码, 但由于没有设置app的sid, 无法自动通过: {request_info}"
                    )
                    raise
