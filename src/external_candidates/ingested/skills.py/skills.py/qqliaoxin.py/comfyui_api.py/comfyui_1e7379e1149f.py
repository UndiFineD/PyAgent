...
class ComfyUI:
    ...
    async def _execute_via_api(self, prompt_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{self.server_url}/api/prompt", json=payload
            ) as resp:
                if resp.status != 200:
    ...
    async def _execute_via_ws(self, prompt_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async with websockets.connect(self._get_ws_url()) as ws:
                await ws.send(
                    json.dumps({"prompt": prompt_data, "client_id": self.client_id})
                )
    ...
    async def execute(self, prompt_data: Dict[str, Any]) -> Dict[str, Any]:
        if (
            self.execution_mode == ExecutionMode.WS
            or self.execution_mode == ExecutionMode.AUTO
        ):
    ...
    async def upload_file(
        self, file_path: str, subfolder: str = "", filename: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                data.add_field(
                    "image", f, filename=filename or os.path.basename(file_path)
                )
                data.add_field("subfolder", subfolder)
                data.add_field("type", "input")

                async with session.post(
                    f"{self.server_url}/upload/image", data=data
                ) as resp:
                    return await resp.json()
        except Exception as e:
            return {"error": str(e)}
        ...
        try:
            ...
            if checkpoints and isinstance(checkpoints, list) and len(checkpoints) > 0:
                all_models = checkpoints[
                    0
                ]  # Models are typically in the first element of the list
            else:
                return []
        ...
        if model is None:
            ...
            if any(kw in prompt_lower for kw in ["flux2", "flux 2", "flux"]):
                ...
                if available_flux_models:
                    # Prefer fp8 versions
                    fp8_models = [
                        m for m in available_flux_models if "fp8" in m.lower()
                    ]
                    if fp8_models:
                        model = fp8_models[0]
                    else:
                ...
                available_anime_models = await self._find_available_models(
                    ["anime", "aw", "cartoon"]
                )
                if available_anime_models:
                    # Prefer fp8 versions
                    fp8_models = [
                        m for m in available_anime_models if "fp8" in m.lower()
                    ]
                    model = fp8_models[0] if fp8_models else available_anime_models[0]
                else:
                    model = "awportrait_v14.safetensors"
            elif any(
                kw in prompt_lower for kw in ["realistic", "photo", "真实", "照片"]
            ):
                # Query for realistic models
                available_realistic_models = await self._find_available_models(
                    ["realistic", "vision", "photo"]
                )
                if available_realistic_models:
                    # Prefer fp8 versions
                    fp8_models = [
                        m for m in available_realistic_models if "fp8" in m.lower()
                    ]
                    model = (
                        fp8_models[0] if fp8_models else available_realistic_models[0]
                    )
                else:
                    model = "realisticVisionV60B1_v60B1VAE.safetensors"
            elif (
                "wan" in prompt_lower or "2.1" in prompt_lower or "2.2" in prompt_lower
            ):
                ...
                available_hunyuan_models = await self._find_available_models(
                    ["hunyuan"]
                )
                if available_hunyuan_models:
                    # Prefer fp8 versions
                    fp8_models = [
                        m for m in available_hunyuan_models if "fp8" in m.lower()
                    ]
                ...
                if available_sdxl_models:
                    # Prefer fp8 versions
                    fp8_models = [
                        m for m in available_sdxl_models if "fp8" in m.lower()
                    ]
                ...
                available_common_models = await self._find_available_models(
                    ["realistic", "general", "base"]
                )
                if available_common_models:
                    # Prefer fp8 versions
                    fp8_models = [
                        m for m in available_common_models if "fp8" in m.lower()
                    ]
    ...
    async def monitor_and_retrieve_images(
...
async def handle_command(text: str) -> Dict[str, Any]:
    ...
    if is_setting_url and not comfy.server_url:
        return {
            "error": "请提供完整的服务器地址，如: set_url https://wp08.unicorn.org.cn:40000"
        }
    ...
    if result.get("images"):
        ...
        if image_urls:
            ...
            result["message"] = (
                f"✅ Image generation complete! Generated {len(image_urls)} image(s)."
            )
