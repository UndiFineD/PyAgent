# Extracted from: C:\DEV\PyAgent\.external\skills\skills\xqicxx\memory-baidu-embedding-db\memory_baidu_embedding_db.py
"""
...
class EnhancedMemoryBaiduEmbeddingDB:
    ...
    def __init__(self, db_path: str = None, cache_size: int = 1000):
        ...
        self.db_path = db_path or os.path.join(
            os.path.expanduser("~"), ".clawd", "enhanced_memory_baidu.db"
        )
    ...
    @with_fallback(
        lambda self, content, tags=None, metadata=None: self._add_memory_fallback(
            content, tags, metadata
        )
    )
    def add_memory(
        self, content: str, tags: List[str] = None, metadata: Dict = None
    ) -> bool:
        """
        添加记忆到数据库
        ...
        try:
            ...
            if cached_embedding:
                ...
                if self.client:
                    embedding = self.client.get_embedding_vector(
                        content, model="embedding-v1"
                    )
                    if not embedding:
            ...
            try:
                ...
                if cursor.rowcount > 0:
                    print(
                        f"✅ 已添加记忆: {content[:50]}{'...' if len(content) > 50 else ''}"
                    )
                    return True
                else:
    ...
    def _add_memory_fallback(
        ...
        try:
            ...
            try:
                ...
                if cursor.rowcount > 0:
                    print(
                        f"✅ 降级模式 - 已添加记忆: {content[:50]}{'...' if len(content) > 50 else ''}"
                    )
                    return True
                else:
    ...
    @with_fallback(
        lambda self, query, limit=5, tags=None: self._search_memories_fallback(
            query, limit, tags
        )
    )
    def search_memories(
        self, query: str, limit: int = 5, tags: List[str] = None
    ) -> List[Dict]:
        """
        通过语义搜索相关记忆
        ...
        try:
            ...
            if not query_embedding:
                if self.client:
                    query_embedding = self.client.get_embedding_vector(
                        query, model="embedding-v1"
                    )
                    if query_embedding:
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "memories_without_embeddings": total_memories
...
def main():
    ...
    try:
        ...
        print(
            f"  标签分布: {dict(list(stats['tag_distribution'].items())[:5])}"
        )  # 只显示前5个
        ...
        for example in examples:
            success = mem_db.add_memory(
                example["content"], example["tags"], example["metadata"]
            )
            print(
                f"  添加记忆: {'✅' if success else '❌'} - {example['content'][:30]}..."
            )

        print("\n🔍 语义搜索示例:")
        # 搜索相关记忆
        search_queries = ["用户健身偏好", "读书和学习目标", "今天的活动建议"]
        ...
        for query in search_queries:
            ...
            if results:
                for i, result in enumerate(results, 1):
                    print(
                        f"    {i}. 相似度: {result['similarity']:.3f} - {result['content'][:50]}..."
                    )
            else:
                print("    未找到相关记忆")
