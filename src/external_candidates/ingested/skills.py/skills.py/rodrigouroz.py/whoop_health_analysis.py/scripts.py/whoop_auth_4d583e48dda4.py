...
def do_login(client_id, client_secret):
    ...
    class CallbackHandler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if state != expected_state:
                ...
                self.end_headers()
                self.wfile.write(
                    b"<html><body><h2>Error: invalid state</h2></body></html>"
                )
                done_event.set()
                return
    ...
    try:
        ...
        print(
            f"Error starting local server on port {REDIRECT_PORT}: {e}", file=sys.stderr
        )
        sys.exit(1)
    ...
    if auth_code_holder["error"]:
        print(
            f"Error during authorization: {auth_code_holder['error']}", file=sys.stderr
        )
        sys.exit(1)
...
def do_status():
    ...
    if not expired:
        remaining = (
            tokens.get("saved_at", 0) + tokens.get("expires_in", 0)
        ) - time.time()
