# Extracted from: C:\DEV\PyAgent\.external\skills\skills\xiwan\smtp-send\scripts\send_email.py
#!/usr/bin/env python3
"""
Send email via SMTP.
...
def load_config():
    ...
    if missing:
        print(
            f"Error: Missing required SMTP config: {', '.join(missing)}",
            file=sys.stderr,
        )
...
def main():
    ...
    parser.add_argument(
        "--attachments", help="Comma-separated list of attachment file paths"
    )
