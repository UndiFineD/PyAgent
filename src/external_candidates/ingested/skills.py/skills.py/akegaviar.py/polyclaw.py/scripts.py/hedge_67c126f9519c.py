...
def format_portfolio_row(p: dict) -> str:
    ...
    target_q = (
        p["target_question"][:35] + "..."
        if len(p["target_question"]) > 35
        else p["target_question"]
    )
    cover_q = (
        p["cover_question"][:35] + "..."
        if len(p["cover_question"]) > 35
        else p["cover_question"]
    )
...
async def cmd_scan(args):
    ...
    try:
        for i, target in enumerate(markets):
            if not args.json:
                print(
                    f"[{i+1}/{len(markets)}] {target.question[:60]}...", file=sys.stderr
                )
            ...
            if covers:
                ...
                if not args.json and portfolios:
                    print(
                        f"  Found {len(portfolios)} potential hedges", file=sys.stderr
                    )
    ...
    if args.min_coverage:
        all_portfolios = filter_portfolios_by_coverage(
            all_portfolios, args.min_coverage
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Hedge discovery - find covering portfolios"
    )
    ...
    scan_parser.add_argument(
        "--limit", type=int, default=20, help="Number of markets to scan (default: 20)"
    )
    scan_parser.add_argument(
        "--min-coverage",
        type=float,
    ...
    analyze_parser = subparsers.add_parser(
        "analyze", help="Analyze specific market pair"
    )
    ...
    analyze_parser.add_argument(
        "--min-coverage", type=float, default=0.85, help="Minimum coverage threshold"
    )
