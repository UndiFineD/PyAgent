# Extracted from: C:\DEV\PyAgent\.external\skills\skills\akegaviar\polyclaw\scripts\trade.py
#!/usr/bin/env python3
...
class TradeExecutor:
    ...
    async def buy_position(
        ...
        print(
            f"Will sell: {'NO' if position == 'YES' else 'YES'} @ ~{unwanted_price:.2f}"
        )
    ...
    try:
        ...
        if result.success:
            ...
            if result.clob_filled:
                ...
                print(
                    f"  Note: You have {result.amount:.0f} {unwanted} tokens to sell manually"
                )
...
def main():
    ...
    buy_parser.add_argument(
        "position", choices=["YES", "NO", "yes", "no"], help="YES or NO"
    )
