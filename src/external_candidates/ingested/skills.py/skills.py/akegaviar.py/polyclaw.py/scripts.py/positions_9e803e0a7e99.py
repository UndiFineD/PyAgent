# Extracted from: C:\DEV\PyAgent\.external\skills\skills\akegaviar\polyclaw\scripts\positions.py
#!/usr/bin/env python3
...
from lib.position_storage import PositionEntry, PositionStorage
...
async def calculate_position_pnl(position: dict, gamma: GammaClient) -> dict:
    ...
    try:
        ...
        current_price = (
            market.yes_price if position["position"] == "YES" else market.no_price
        )
...
async def cmd_list(args):
    ...
    if args.json:
        ...
        for r in results:
            print(
                f"{r['position_id']:<10} {r['side']:<4} {r['entry']:>6} {r['current']:>6} {r['pnl']:>10} {r['market'][:35]}"
            )

        print("-" * 80)
        print(
            f"Total: {len(results)} positions | Value: ${total_value:.2f} | P&L: {format_pnl(total_pnl)}"
        )
...
def main():
    ...
    list_parser.add_argument(
        "--all", action="store_true", help="Include closed positions"
    )
