# Extracted from: C:\DEV\PyAgent\.external\skills\skills\akegaviar\polyclaw\lib\clob_client.py
...
class ClobClientWrapper:
    ...
    def _is_cloudflare_block(self, error_msg: str) -> bool:
        ...
        return "403" in error_msg and (
            "cloudflare" in error_msg.lower() or "blocked" in error_msg.lower()
        )
    ...
    def sell_fok(
        ...
        for attempt in range(CLOB_MAX_RETRIES):
            try:
                ...
                if attempt > 0 and proxy:
                    print(
                        f"  Retrying CLOB sell (attempt {attempt + 1}/{CLOB_MAX_RETRIES})..."
                    )
        ...
        if self._is_cloudflare_block(last_error):
            ...
            error_msg = (
                f"No liquidity at ${sell_price:.2f} - tokens kept, sell manually"
            )
        else:
