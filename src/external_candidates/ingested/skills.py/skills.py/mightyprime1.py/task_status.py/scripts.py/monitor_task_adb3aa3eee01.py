# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mightyprime1\task-status\scripts\monitor_task.py
#!/usr/bin/env python3
"""
...
from send_status import can_encode_emoji, send_status
...
def monitor_worker(task_name: str, interval: int = 5):
    ...
    print(
        f"[DEBUG] monitor_worker started for '{task_name}' with {interval}s interval",
        file=sys.stderr,
    )
    sys.stderr.flush()
    ...
    while True:
        ...
        state = load_state()
        print(
            f"[DEBUG] Iteration {iteration}: State keys = {list(state.keys())}",
            file=sys.stderr,
        )
        sys.stderr.flush()

        print(
            f"[DEBUG] Checking if '{task_name}' in state: {task_name in state}",
            file=sys.stderr,
        )
        sys.stderr.flush()
        ...
        if task_name not in state:
            print(
                f"[DEBUG] Task '{task_name}' not in state, stopping worker",
                file=sys.stderr,
            )
            sys.stderr.flush()
            # Task was stopped
        ...
        print(
            f"[DEBUG] Iteration {iteration}: elapsed={elapsed:.1f}s, interval={interval}s",
            file=sys.stderr,
        )
        sys.stderr.flush()
        ...
        if elapsed >= interval:
            print(
                f"[DEBUG] Time to send update! elapsed={elapsed:.1f}s >= interval={interval}s",
                file=sys.stderr,
            )
            sys.stderr.flush()
            ...
            try:
                print(
                    f"[DEBUG] Sending status: {message} {status} {task_name}",
                    file=sys.stderr,
                )
                sys.stderr.flush()
...
def start_monitor(task_name: str, status_type: str = "progress", interval: int = 5):
    ...
    print(
        f"[DEBUG] start_monitor called: task_name='{task_name}', status_type='{status_type}', interval={interval}",
        file=sys.stderr,
    )
    ...
    send_status(
        f"Monitoring started (updates every {interval}s)", status_type, task_name
    )

    # Start background monitor thread
    print(f"[DEBUG] Starting monitor thread", file=sys.stderr)
    thread = threading.Thread(
        target=monitor_worker, args=(task_name, interval), daemon=True
    )
    thread.start()
...
def stop_monitor(task_name: str, final_status: str, final_message: str):
    ...
    print(
        f"[DEBUG] stop_monitor called: task_name='{task_name}', final_status='{final_status}', final_message='{final_message}'",
        file=sys.stderr,
    )
...
def main():
    ...
    if action == "start":
        if len(sys.argv) < 4:
            print(
                "Usage: monitor_task.py start <task_name> <status_type> [--interval <seconds>]",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if len(sys.argv) < 5:
            print(
                "Usage: monitor_task.py stop <task_name> <final_status> <final_message>",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if not state:
            ...
            for task, info in state.items():
                print(
                    f"  {task}: {info['status_type']} (interval: {info['interval']}s)"
                )
