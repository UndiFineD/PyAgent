# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tedkaczynski-the-bot\molt-chess\scripts\play.py
#!/usr/bin/env python3
"""
...
def analyze_move(board: chess.Board, move: chess.Move) -> dict:
    ...
    info = {
        ...
        "eval": -evaluate_position(
            board
        ),  # Negative because we evaluate from opponent's view
    }
...
def main():
    ...
    parser.add_argument(
        "--api-key",
        help="API key (or reads from ~/.config/molt-chess/credentials.json)",
    )
    parser.add_argument(
        "--depth", type=int, default=2, help="Search depth (default: 2)"
    )
    parser.add_argument(
        "--top", type=int, default=5, help="Number of moves to show (default: 5)"
    )
    ...
    if args.game_id:
        ...
        if not api_key:
            print(
                "ERROR: --api-key required or set in ~/.config/molt-chess/credentials.json"
            )
            sys.exit(1)
        ...
        fen = game["fen"]
        print(
            f"Game {args.game_id}: {game.get('white', '?')} vs {game.get('black', '?')}"
        )
