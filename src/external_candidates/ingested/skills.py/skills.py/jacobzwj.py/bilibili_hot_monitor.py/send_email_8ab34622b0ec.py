# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jacobzwj\bilibili-hot-monitor\send_email.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def markdown_to_html(markdown_text: str) -> str:
    ...
    for line in lines:
        ...
        if match:
            ...
            html_parts.append(
                f'<div class="video-title"><span class="num">{num}</span>{title}</div>'
            )
            in_video_block = True
            continue
        ...
        if match:
            ...
            html_parts.append(
                f'<div class="info-row"><span class="label">{label}：</span><span class="value">{content}</span></div>'
            )
            continue
        ...
        if stripped.startswith(">"):
            ...
            if (
                html_parts
                and html_parts[-1].startswith('<div class="quote">')
                and not html_parts[-1].endswith("</div>")
            ):
                html_parts[-1] += f"<br>{content}"
            elif (
                html_parts
                and html_parts[-1].endswith("</div>")
                and '<div class="quote">' in html_parts[-1]
            ):
                # 上一行也是引用，合并
        ...
        if stripped.startswith("🔗"):
            # 格式1: 🔗 [点击观看视频](url)
            match = re.search(
                r"\[([^\]]+)\]\((https://www\.bilibili\.com/video/\S+)\)", stripped
            )
            if match:
                text, url = match.groups()
                html_parts.append(
                    f'<div class="video-link">🔗 <a href="{url}">{text}</a></div>'
                )
                continue
            # 格式2: 🔗 **视频链接**：url
            ...
            if match:
                url = match.group()
                html_parts.append(
                    f'<div class="video-link">🔗 <a href="{url}">点击观看视频</a></div>'
                )
            continue
