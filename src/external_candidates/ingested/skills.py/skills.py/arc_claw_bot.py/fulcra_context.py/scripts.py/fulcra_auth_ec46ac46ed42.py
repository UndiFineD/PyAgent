# Extracted from: C:\DEV\PyAgent\.external\skills\skills\arc-claw-bot\fulcra-context\scripts\fulcra_auth.py
#!/usr/bin/env python3
"""
        ...
        if status == 200 and "access_token" in data:
            ...
            try:
                ...
                if resp.status == 200:
                    ...
                    token_data["user_id"] = uid_data.get("user_id") or uid_data.get(
                        "userid"
                    )
            except Exception:
                pass
            ...
            print(
                f"   Refresh token: {'✅ saved' if token_data.get('refresh_token') else '❌ not returned'}"
            )
            ...
            if not token_data.get("refresh_token"):
                print()
                print(
                    "⚠️  No refresh token returned. Token will expire and need re-auth."
                )
