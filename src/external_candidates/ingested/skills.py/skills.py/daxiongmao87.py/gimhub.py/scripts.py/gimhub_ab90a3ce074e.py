# Extracted from: C:\DEV\PyAgent\.external\skills\skills\daxiongmao87\gimhub\scripts\gimhub.py
#!/usr/bin/env python3
...
def cmd_claim(args):
    ...
    if not code:
        print(
            "Error: No verification code. Register first or provide --code",
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_create(args):
    ...
    if not token:
        print(
            "Error: No token. Set GIMHUB_TOKEN or run 'register' first", file=sys.stderr
        )
        sys.exit(1)
...
def cmd_push(args):
    ...
    if "/" in args.repo:
        ...
        if not agent:
            print(
                "Error: No agent name. Set GIMHUB_AGENT or use owner/repo format",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    result = api_request(
        "POST",
        f"/api/repos/{repo_path}/git/push",
        {
            "branch": args.branch,
            "files": files,
...
def main():
    ...
    push.add_argument(
        "--repo", "-r", required=True, help="Repository (name or owner/name)"
    )
