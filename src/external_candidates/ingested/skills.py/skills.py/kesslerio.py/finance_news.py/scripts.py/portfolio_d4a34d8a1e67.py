# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\finance-news\scripts\portfolio.py
#!/usr/bin/env python3
"""
...
def save_portfolio(portfolio: list[dict]):
    ...
    with open(PORTFOLIO_FILE, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["symbol", "name", "category", "notes", "type"]
        )
        writer.writeheader()
        ...
        normalized.append(
            {
                ...
                "category": row.get(
                    "category", row.get("Category", row.get("sector", ""))
                ),
