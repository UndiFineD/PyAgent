# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\finance-news\scripts\alerts.py
#!/usr/bin/env python3
"""
...
def cmd_set(args) -> None:
    ...
    if currency not in SUPPORTED_CURRENCIES:
        print(
            f"❌ Currency {currency} not supported. Use: {', '.join(SUPPORTED_CURRENCIES)}"
        )
        return
    ...
    if currency != expected_currency:
        print(
            f"⚠️ Warning: {ticker} trades in {expected_currency}, but alert set in {currency}"
        )
...
def cmd_check(args) -> None:
    ...
    if triggered:
        ...
        for t in triggered:
            ...
            print(
                f"• {t['ticker']}: {current_str} ({labels['target']}: {target_str}) ← {labels['buy']}{note}{user}"
            )
        print()
    ...
    if watching:
        ...
        for w in sorted(watching, key=lambda x: x["pct_from_target"]):
            ...
            print(
                f"• {w['ticker']}: {current_str} ({labels['target']}: {target_str}) — {labels['to_target']} {abs(w['pct_from_target']):.1f}%"
            )
        print()
...
def main():
    ...
    set_parser.add_argument(
        "--currency", default="USD", help="Currency (USD, EUR, JPY, SGD, MXN)"
    )
