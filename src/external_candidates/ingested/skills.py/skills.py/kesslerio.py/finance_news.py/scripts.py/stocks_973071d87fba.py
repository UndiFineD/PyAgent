...
def move_to_holdings(
    ticker: str, name: str = "", category: str = "", notes: str = ""
) -> bool:
    ...
    data["holdings"].append(
        {
            ...
            "category": category,
            "notes": notes
            or f"Bought (was on watchlist with target ${watchlist_item.get('target', 'N/A')})",
        }
    )
    data["holdings"].sort(key=lambda x: x.get("ticker", ""))
    save_stocks(data)
...
def main():
    ...
    list_parser.add_argument(
        "--holdings", action="store_true", help="Show only holdings"
    )
    list_parser.add_argument(
        "--watchlist", action="store_true", help="Show only watchlist"
    )
    ...
    set_alert = subparsers.add_parser(
        "set-alert", help="Set buy-more/stop alert on holding"
    )
    ...
    if args.command == "list":
        ...
        for h in data["holdings"]:
            if h.get("ticker") == args.ticker.upper():
                ...
                found = True
                print(
                    f"✅ Set alert on {args.ticker.upper()}: target=${args.target}, stop=${args.stop}"
                )
                break
        if not found:
