# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\finance-news\scripts\ranking.py
#!/usr/bin/env python3
"""
...
def ensure_diversity(
    selected: list[dict], candidates: list[dict], required: list[str]
) -> list[dict]:
    ...
    for req_cat in required:
        if req_cat not in covered:
            ...
            for candidate in candidates:
                if candidate not in result and req_cat in candidate.get(
                    "_categories", []
                ):
...
def rank_headlines(headlines: list[dict], config: dict | None = None) -> dict:
    ...
    must_read_candidates = [
        a for a in capped if a.get("_score", 0) >= cfg["must_read_min_score"]
    ]
    must_read_count = cfg["must_read_count"]
    must_read = must_read_candidates[
        : max(1, must_read_count - 2)
    ]  # Reserve 2 slots for diversity
    must_read = ensure_diversity(
        must_read, capped, ["macro", "equities", "geopolitics"]
    )
    ...
    scan_candidates = [
        a
        for a in capped
        if a not in must_read and a.get("_score", 0) >= cfg["scan_min_score"]
    ]
    ...
    test_headlines = [
        {
            ...
            "source": "WSJ",
            "description": "Federal Reserve hints at policy shift",
        },
        {
        ...
        },
        {
            ...
            "source": "Reuters",
            "description": "Brent crude hits $90",
        },
        {
            "title": "China-US trade tensions escalate",
            "source": "Bloomberg",
            "description": "New tariffs announced",
        },
        {
            "title": "Tech stocks rally on AI optimism",
            "source": "Yahoo Finance",
            "description": "Nvidia leads gains",
        },
        {
            "title": "Fed hints at rate reduction",
            "source": "MarketWatch",
            "description": "Same story as WSJ",
        },  # Dupe
    ]
