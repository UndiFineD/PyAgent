# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\phone-agent\scripts\server.py
...
)  # Daniel - Steady Broadcaster (male)
...
async def handle_incoming_call(request: Request):
    ...
    response.say(
        "Verbinde dich mit dem KI-Assistenten.", voice="alice", language="de-DE"
    )
    connect = Connect()
    ...
    if not ws_base_url:
        logger.error(
            "No valid PUBLIC_URL/Host for Twilio stream (public_url=%s host=%s)",
            PUBLIC_URL,
            host,
        )
        response.say(
            "Konfigurationsfehler. Bitte später erneut versuchen.",
            voice="alice",
            language="de-DE",
        )
        response.hangup()
...
async def make_outbound_call(request: Request):
    ...
    if not to_number:
        return Response(
            content=json.dumps({"error": "Missing 'to' number"}),
            status_code=400,
            media_type="application/json",
        )
    ...
    response = VoiceResponse()
    response.say(
        "Hallo, dies ist ein automatischer Anruf von Niemand.",
        voice="alice",
        language="de-DE",
    )
    connect = Connect()
    ...
    try:
        ...
        logger.info(
            f"Outbound call initiated: {call.sid} to {to_number} (task: {task_name})"
        )
...
async def list_calls():
    ...
    return {"active": active_calls, "completed": call_results}
    ...
    async def process_transcript(transcript: str):
        ...
        try:
            completion = await openai_client.chat.completions.create(
                model="gpt-4o", messages=conversation_history, max_tokens=120
            )
            ai_text = (
                completion.choices[0].message.content
                or "Das System verarbeitet Ihre Anfrage."
            )
                ...
                if data["event"] == "connected":
                    ...
                    if call_sid and call_sid in active_calls:
                        ...
                        if task_name:
                            ...
                            logger.info(
                                f"Loaded task '{task_name}' with config: {task_config}"
                            )
            ...
            async for message in dg_ws:
                ...
                if data.get("type") == "Results":
                    ...
                    if alternatives:
                        ...
                        if transcript:
                            logger.info(
                                f"Deepgram: '{transcript}' (final={is_final}, speech_final={speech_final})"
                            )
