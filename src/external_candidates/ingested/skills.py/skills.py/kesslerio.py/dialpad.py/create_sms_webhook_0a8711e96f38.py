...
def create_sms_subscription(webhook_url, events=None, office_id=None, direction="all"):
    ...
    request = urllib.request.Request(
        webhook_url_api, data=data, headers=headers, method="POST"
    )
    ...
    request = urllib.request.Request(
        subscription_url, data=data, headers=headers, method="POST"
    )
...
def main():
    ...
    create_parser = subparsers.add_parser(
        "create", help="Create a new webhook subscription"
    )
    create_parser.add_argument(
        "--url", required=True, help="Webhook URL to receive events"
    )
    create_parser.add_argument(
        "--events", help="Comma-separated events (default: sms_sent,sms_received)"
    )
    create_parser.add_argument(
        "--office-id", dest="office_id", help="Optional office ID to filter events"
    )
    create_parser.add_argument(
        "--direction",
        default="all",
