...
def store_message(data: dict, is_new: bool = True) -> dict:
    ...
    if other_number == "unknown":
        print(
            f"Warning: Could not determine contact number for message {msg.get('id')}"
        )
        return msg
    ...
    metadata["directions"][msg["direction"]] = (
        metadata["directions"].get(msg["direction"], 0) + 1
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python3 sms_storage.py [migrate|list|thread <number>|search <query>|unread]"
        )
        sys.exit(1)
    ...
    if cmd == "migrate":
        ...
        for t in threads:
            last = (
                datetime.fromtimestamp(t.get("last_message", 0) / 1000).strftime(
                    "%Y-%m-%d %H:%M"
                )
        ...
        for msg in messages:
            ts = (
                datetime.fromtimestamp(msg.get("timestamp", 0) / 1000).strftime(
                    "%H:%M:%S"
                )
        ...
        for t in unread:
            print(
                f"  {t['phone_number']} ({t.get('contact_name', 'Unknown')}): {t['unread_count']} unread"
            )

    else:
        print(
            "Unknown command. Usage: python3 sms_storage.py [migrate|list|thread <number>|search <query>|unread]"
        )
