# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\dialpad\sms_sqlite.py
#!/usr/bin/env python3
"""
...
def init_db() -> sqlite3.Connection:
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_messages_read ON messages(read) WHERE read = 0"
    )
...
def normalize_message(data: dict) -> dict:
    ...
    return {
        ...
        "contact_name": (
            contact.get("name") if contact.get("name") != other_number else None
        ),
        "direction": direction,
...
def get_thread(
    conn: sqlite3.Connection, phone_number: str, limit: int = 100
) -> List[dict]:
            ...
            for line in f:
                ...
                try:
                    ...
                    data = {
                        ...
                        "to_number": (
                            [msg.get("to_number")] if msg.get("to_number") else []
                        ),
                        "text": msg.get("text"),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python3 sms_sqlite.py [list|thread <number>|search <query>|unread|migrate|stats]"
        )
        sys.exit(1)
    ...
    if cmd == "list":
        ...
        for t in threads:
            ...
            if len(name) > 28:
                name = name[:27] + "…"
            print(
                f"{t['phone_number']:<20} {name:<30} {t['message_count']:>6} {t['unread_count']:>7} {last}"
            )
        ...
        # Get contact name
        cursor = conn.execute(
            "SELECT name FROM contacts WHERE phone_number = ?", (number,)
        )
        row = cursor.fetchone()
        ...
        for msg in messages:
            ...
            print(
                f"[{ts}] {direction} {read_status} {text}{'...' if len(msg.get('text', '')) > 100 else ''}"
            )
        ...
        if not threads:
            ...
            for t in threads:
                ...
                print(
                    f"  {t.get('name') or t['phone_number']}: {t['unread_count']} unread"
                )
                print(f"    > {preview}...\n")
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) as unread FROM messages WHERE read = 0 AND direction = 'inbound'"
        )
        unread = cursor.fetchone()["unread"]

        cursor = conn.execute(
            "SELECT COUNT(DISTINCT contact_number) as contacts FROM messages"
        )
        contacts = cursor.fetchone()["contacts"]

        cursor = conn.execute(
            "SELECT COUNT(*) as inbound FROM messages WHERE direction = 'inbound'"
        )
        inbound = cursor.fetchone()["inbound"]

        cursor = conn.execute(
            "SELECT COUNT(*) as outbound FROM messages WHERE direction = 'outbound'"
        )
        ...
        print(
            "Unknown command. Usage: python3 sms_sqlite.py [list|thread <number>|search <query>|unread|migrate|stats|read <number>]"
        )
