# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\camoufox-stealth\scripts\camoufox-session.py
#!/usr/bin/env python3
"""
...
def main() -> None:
    ...
    export_path = (
        Path(args.export_cookies).expanduser() if args.export_cookies else None
    )
    import_path = (
        Path(args.import_cookies).expanduser() if args.import_cookies else None
    )
