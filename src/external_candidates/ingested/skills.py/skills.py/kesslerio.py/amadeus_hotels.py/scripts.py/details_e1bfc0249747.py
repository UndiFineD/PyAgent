...
def format_offer_human(offer: dict) -> str:
    ...
    # Room details
    offers = (
        offer.get("offers", [offer])
        if "offers" not in offer
        else offer.get("offers", [])
    )
    ...
    for o in offers[:1]:  # Usually just one offer in details
        ...
        if cancel:
            ...
            if cancel_type == "NON_REFUNDABLE":
                ...
                if amount:
                    lines.append(
                        f"   Cancellation fee: {amount.get('currency', '')} {amount.get('value', '')}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Get hotel/offer details via Amadeus API"
    )
    ...
    parser.add_argument(
        "--ratings", action="store_true", help="Include ratings/sentiment data"
    )
    parser.add_argument(
        "--format",
        choices=["json", "human"],
