...
def add_hotel(
    ...
    for hotel in data["hotels"]:
        if (
            ...
            and hotel["checkin"] == check_in
            and hotel["checkout"] == check_out
        ):
...
def list_tracked() -> None:
    ...
    for hotel in data["hotels"]:
        ...
        print(
            f"   🕐 Last checked: {last_check[:10] if last_check != 'never' else 'never'}"
        )
        print()
...
def main():
    ...
    parser.add_argument(
        "--remove", action="store_true", help="Remove hotel from tracking"
    )
    ...
    try:
        if args.add:
            if not all([args.hotel, args.checkin, args.checkout, args.target]):
                parser.error(
                    "--add requires --hotel, --checkin, --checkout, and --target"
                )
