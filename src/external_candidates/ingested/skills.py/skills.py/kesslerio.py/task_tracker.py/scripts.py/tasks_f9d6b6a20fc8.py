...
def list_tasks(args):
    ...
    if args.status:
        ...
        filtered = [
            t
            for t in filtered
            if t.get("status", "").lower().replace("-", " ") == normalized_status
        ]
...
def add_task(args):
    ...
    if match:
        ...
        new_content = (
            content[: match.start(2)] + insert_content + content[match.end(2) :]
        )
...
def main():
    ...
    list_parser.add_argument(
        "--status", choices=["todo", "in-progress", "blocked", "waiting", "done"]
    )
