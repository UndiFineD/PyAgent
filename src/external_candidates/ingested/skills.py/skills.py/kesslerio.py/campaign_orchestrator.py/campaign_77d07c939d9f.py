# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\campaign-orchestrator\campaign.py
#!/usr/bin/env python3
"""
...
def cmd_start(args):
    ...
    if (
        lead_name in data["campaigns"]
        and data["campaigns"][lead_name]["status"] == "active"
    ):
        print(f"Error: Active campaign already exists for '{lead_name}'")
        print(
            f"Use '/campaign status {lead_name}' to check, or '/campaign stop {lead_name}' first."
        )
        sys.exit(1)
    ...
    email_delay = (
        delay_override if delay_override is not None else delays.get("email", 24)
    )
    steps.append(
        {
...
def cmd_status(args):
    ...
    for i, step in enumerate(campaign["steps"]):
        status_icon = (
            ...
            else "⏳" if step["status"] == "pending" else "❌"
        )
...
def cmd_list(args):
    ...
    for lead, campaign in active.items():
        ...
        next_step = (
            campaign["steps"][campaign["current_step"]]
            if campaign["current_step"] < total
            else None
        )
...
def cmd_pending(args):
                ...
                pending.append(
                    {
                        ...
                        "scheduled": scheduled,
                        "hours_until": (scheduled - datetime.now()).total_seconds()
                        / 3600,
                    }
                )
    ...
    for p in pending:
        ...
        print(
            f"{p['lead']:<25} {p['step']}/{len(campaign['steps'])}   {p['type']:<6} {when:<20}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Campaign Orchestrator CLI")
    ...
    check_parser = subparsers.add_parser(
        "check", help="Check for responses before next step"
    )
