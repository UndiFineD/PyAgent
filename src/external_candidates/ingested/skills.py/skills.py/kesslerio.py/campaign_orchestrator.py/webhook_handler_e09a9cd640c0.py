# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\campaign-orchestrator\webhook_handler.py
#!/usr/bin/env python3
"""
...
class CampaignWebhookHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        to_number = (
            data.get("to_number", [{}])[0]
            if isinstance(data.get("to_number"), list)
            else data.get("to_number", {})
        )
        ...
        print(
            f"[{timestamp}] Received reply from {lead_name} ({from_number}): {message[:50]}..."
        )
