# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\fitbit-analytics\scripts\fitbit_briefing.py
#!/usr/bin/env python3
"""
...
def _format_text_briefing(
    data, yesterday_activities=None, yesterday_azm=None, baseline=None
):
    ...
    if yesterday_azm and yesterday_azm.get("activeZoneMinutes"):
        ...
        lines.append(
            f"  • Total AZM: {total_azm} min (Daily avg needed: {daily_avg_needed}/day for 150/week)"
        )
        if fat_burn:
...
def main():
    ...
    try:
        ...
        calories_list = (
            calories_data.get("activities-calories", []) if calories_data else []
        )
        ...
        if activity_list and len(activity_list) > 0:
            ...
            today_active_minutes = summary.get("veryActiveMinutes", 0) + summary.get(
                "fairlyActiveMinutes", 0
            )
        if hr_list and len(hr_list) > 0:
            hr_value = hr_list[0].get("value", {})
            resting_hr = (
                hr_value.get("restingHeartRate")
                if isinstance(hr_value, dict)
                else hr_value
            )

            # Extract HR zones and calculate weighted average HR
            heart_rate_zones = (
                hr_value.get("heartRateZones", []) if isinstance(hr_value, dict) else []
            )
            hr_zones = {}
            total_minutes = 0
            weighted_hr_sum = 0
        ...
        week_calories_list = (
            week_calories.get("activities-calories", []) if week_calories else []
        )
        ...
        if valid_days > 0:
            ...
            steps_trend = (
                ((today_steps - avg_steps) / avg_steps * 100) if avg_steps > 0 else 0
            )
            calories_trend = (
                ((today_calories - avg_calories) / avg_calories * 100)
                if avg_calories > 0
                else 0
            )
        else:
        ...
        activity_level = _calculate_activity_level(
            today_steps, today_calories, today_active_minutes
        )
        ...
        yesterday_activities_raw = client._request(
            f"1/user/-/activities/date/{yesterday}.json"
        )
        yesterday_activities = (
            yesterday_activities_raw.get("activities", [])
            if yesterday_activities_raw
            else []
        )

        # Get yesterday's Active Zone Minutes
        try:
            yesterday_azm_raw = client._request(
                f"1/user/-/activities/active-zone-minutes/date/{yesterday}/1d.json"
            )
            azm_list = (
                yesterday_azm_raw.get("activities-active-zone-minutes", [])
                if yesterday_azm_raw
                else []
            )
        ...
        if args.format == "json":
            ...
            print(
                _format_text_briefing(
                    data,
                    yesterday_activities=yesterday_activities,
                    yesterday_azm=yesterday_azm,
                )
            )
