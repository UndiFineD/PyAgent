# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\camoufox-stealth-browser\scripts\camoufox-fetch.py
#!/usr/bin/env python3
"""
...
async def fetch_page(
    ...
    if proxy:
        ...
        if "@" in proxy:
            ...
            auth_part = (
                proxy.split("@")[0].replace("http://", "").replace("https://", "")
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Camoufox stealth browser fetch (max evasion)"
    )
    parser.add_argument("url", help="URL to fetch")
    parser.add_argument(
        "--wait", type=int, default=8, help="Wait time in seconds (default: 8)"
    )
