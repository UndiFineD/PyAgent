# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\weekly_report.py
#!/usr/bin/env python3
"""
...
def analyze_week(sleep_data, readiness_data=None):
    ...
    avg_efficiency = (
        round(sum(efficiencies) / len(efficiencies), 1)
    ...
    for i, d in enumerate(sleep_data[-2:]):
        ...
        r_score = (
            readiness_by_day.get(day, {}).get("score")
            if day in readiness_by_day
            else None
        )
        last_2_days.append(
            {"day": day, "sleep_score": score, "readiness": r_score, "hours": hours}
        )
    ...
    return {
        ...
        "best_day": (
            sleep_data[scores.index(max(scores))].get("day")
            if sleep_data and scores
            else None
        ),
        "worst_day": (
            sleep_data[scores.index(min(scores))].get("day")
            if sleep_data and scores
            else None
        ),
...
def main():
    ...
    try:
        ...
        print(
            f"   Sleep Score: {week_data['avg_sleep_score']} {trend_symbol(sleep_trend)} ({'+' if sleep_trend > 0 else ''}{sleep_trend})"
        )
        print(
            f"   Readiness: {week_data['avg_readiness']} {trend_symbol(readiness_trend)} ({'+' if readiness_trend > 0 else ''}{readiness_trend})"
        )
        ...
        if last_2:
            ...
            for d in last_2:
                ...
                print(
                    f"      {d['day']}: 😴{d['sleep_score']} ⚡{ready}/100 ⏰{d['hours']}h"
                )
        ...
        with open(report_file, "w") as f:
            json.dump(
                {"period": period, "summary": week_data, "raw": sleep}, f, indent=2
            )
