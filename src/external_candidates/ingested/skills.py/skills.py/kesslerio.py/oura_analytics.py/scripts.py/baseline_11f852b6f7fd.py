# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\baseline.py
#!/usr/bin/env python3
"""
...
def calculate_sleep_score(sleep_data: dict) -> float:
    ...
    duration_hours = (
        ...
        if sleep_data.get("total_sleep_duration")
        else 0
    )
...
def build_baseline(
    sleep_data: List[dict], readiness_data: List[dict], period_days: int
) -> Baseline:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="Days for current period comparison (default: 7)",
    )
    parser.add_argument("--token", help="Oura API token")
    parser.add_argument("--json", action="store_true", help="Output JSON format")
    parser.add_argument(
        "--baseline-only", action="store_true", help="Show baseline without comparison"
    )
    ...
    try:
        ...
        baseline = build_baseline(
            baseline_sleep, baseline_readiness, args.baseline_days
        )
                    ...
                    json.dumps(
                        {
                            "baseline": {
                                "sleep_score": (
                                    baseline.sleep_score.__dict__
                                    if baseline.sleep_score
                                    else None
                                ),
                                "readiness": (
                                    baseline.readiness.__dict__
                                    if baseline.readiness
                                    else None
                                ),
                                "sleep_hours": (
                                    baseline.sleep_hours.__dict__
                                    if baseline.sleep_hours
                                    else None
                                ),
                                "efficiency": (
                                    baseline.efficiency.__dict__
        ...
        current_sleep_scores = [
            calculate_sleep_score(s)
            for s in current_sleep
            if calculate_sleep_score(s) > 0
        ]
        current_readiness_scores = [
            r.get("score") for r in current_readiness if r.get("score")
        ]
        current_sleep_hours = [
        ...
        current_efficiencies = [
            s.get("efficiency") for s in current_sleep if s.get("efficiency")
        ]

        avg_sleep_score = (
            statistics.mean(current_sleep_scores) if current_sleep_scores else 0
        )
        avg_readiness = (
            statistics.mean(current_readiness_scores) if current_readiness_scores else 0
        )
        avg_sleep_hours = (
            statistics.mean(current_sleep_hours) if current_sleep_hours else 0
        )
        avg_efficiency = (
            statistics.mean(current_efficiencies) if current_efficiencies else 0
        )
        ...
        if baseline.sleep_score and avg_sleep_score:
            comparisons.append(
                compare_to_baseline(
                    {}, baseline, "Sleep Score", avg_sleep_score, baseline.sleep_score
                )
            )
        ...
        if baseline.readiness and avg_readiness:
            comparisons.append(
                compare_to_baseline(
                    {}, baseline, "Readiness", avg_readiness, baseline.readiness
                )
            )
        ...
        if baseline.efficiency and avg_efficiency:
            comparisons.append(
                compare_to_baseline(
                    {}, baseline, "Efficiency", avg_efficiency, baseline.efficiency
                )
            )
