# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\briefing.py
#!/usr/bin/env python3
"""
...
class Baseline:
    ...
    def from_history(cls, nights: List[NightRecord]) -> "Baseline":
        ...
        hrv_values = sorted(
            [
                n.sleep.average_hrv_ms
                for n in nights
                if n.sleep and n.sleep.average_hrv_ms
            ]
        )
        rhr_values = sorted(
            [
                n.sleep.lowest_heart_rate_bpm
                for n in nights
                if n.sleep and n.sleep.lowest_heart_rate_bpm
            ]
        )
...
def format_brief_briefing(
    night: NightRecord, baseline: Optional[Baseline] = None
) -> str:
    """
...
def format_json_briefing(
    ...
    if night.readiness:
        briefing["readiness"] = {
            "score": night.readiness.score,
            "delta_vs_baseline": (
                round(night.readiness.score - formatter.baseline.avg_readiness, 1)
                if baseline
                else None
            ),
...
def format_hybrid_briefing(
    ...
    if week_data:
        ...
        if last_2_days and len(last_2_days) >= 2:
            ...
            lines.append("")
            lines.append(
                f"*Recent*: {d1_date} → `{d1_sleep}`/`{d1_ready}` • {d2_date} → `{d2_sleep}`/`{d2_ready}`"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format", choices=["brief", "hybrid", "json"], default="hybrid"
    )
