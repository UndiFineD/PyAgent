...
class Signal:
    ...
    def __str__(self) -> str:
        ...
        emoji = (
            ...
            else "🟡" if self.severity == "medium" else "⚪"
        )
...
class EarlyWarningSystem:
    ...
    def calculate_rolling_baseline(
        self, values: List[float], window: int = 14
    ) -> float:
    ...
    def check_temperature(self, temp_history: List[float]) -> Tuple[Signal, bool]:
        ...
        signal = Signal(
            "Temperature deviation", current, baseline, delta, exceeded, severity
        )
    ...
    def analyze(
        self, sleep_data: List[Dict], readiness_data: List[Dict]
    ) -> WarningReport:
        """
        ...
        signals_triggered = sum(
            [temp_triggered, hrv_triggered, rhr_triggered, readiness_triggered]
        )
        ...
        if alarm:
            ...
            if "Temperature" in triggered_names[0]:
                recommendation = (
                    "Consider: Rest day, monitor for illness symptoms, extra sleep"
                )
                ...
                recommendation = (
                    "Consider: Light activity only, prioritize sleep, reduce stressors"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=14, help="Days to analyze (for rolling baseline)"
    )
    parser.add_argument(
        "--temp-threshold",
        type=float,
    ...
    parser.add_argument(
        "--hrv-drop", type=float, default=15.0, help="HRV drop threshold (%)"
    )
    parser.add_argument(
        "--rhr-increase", type=float, default=5.0, help="RHR increase threshold (bpm)"
    )
    parser.add_argument(
        "--readiness-threshold", type=int, default=65, help="Low readiness threshold"
    )
    parser.add_argument(
        "--require-signals",
        type=int,
