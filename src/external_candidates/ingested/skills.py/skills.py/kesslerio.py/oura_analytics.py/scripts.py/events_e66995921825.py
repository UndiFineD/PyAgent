...
class CorrelationAnalyzer:
    ...
    def analyze_tag_correlation(
        ...
        analyzed_tag = tag or (
            events[0]["tags"][0] if events[0].get("tags") else "unknown"
        )
        ...
        for date_str in event_dates:
            ...
            for offset in range(
                -7, lag_days + 1
            ):  # 7 days before, up to lag_days after
    ...
    def _interpret_correlation(
        self, metric: str, diff: float, significance: str
    ) -> str:
        ...
        magnitude = (
            "slightly"
            if abs(diff) < 5
            else "moderately" if abs(diff) < 10 else "significantly"
        )
...
def main():
    ...
    log_parser.add_argument(
        "--tags", required=True, help="Comma-separated tags (e.g., 'alcohol,late-meal')"
    )
    ...
    corr_parser.add_argument(
        "--lag", type=int, default=1, help="Days to look ahead (default: 1 = next-day)"
    )
    ...
    if args.command == "log":
        ...
        result = analyzer.analyze_tag_correlation(
            events, args.metric, args.lag, args.tag
        )
        ...
        print(
            f"   Event days: {result['event_mean']} (n={result['event_sample_size']})"
        )
        print(
            f"   Control days: {result['control_mean']} (n={result['control_sample_size']})"
        )
        print(
            f"   Difference: {'+' if result['difference'] > 0 else ''}{result['difference']}"
        )
