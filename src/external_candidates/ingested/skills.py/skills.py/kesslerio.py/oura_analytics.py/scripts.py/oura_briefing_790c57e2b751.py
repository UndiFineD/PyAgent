...
def _analyze_week(sleep_data, readiness_data=None):
    ...
    # Get durations
    durations = [
        _seconds_to_hours(d.get("total_sleep_duration", 0)) for d in sleep_data
    ]
...
def main():
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Detailed briefing with driver analysis"
    )
    parser.add_argument(
        "--format",
    ...
    try:
        ...
        night = create_night_record(
            ...
            activity=(
                activity_data[0] if (activity_data and len(activity_data) > 0) else None
            ),
        )
        ...
        for sleep_entry in baseline_sleep:
            ...
            baseline_night = create_night_record(
                date=date, sleep=sleep_entry, readiness=readiness_entry
            )
