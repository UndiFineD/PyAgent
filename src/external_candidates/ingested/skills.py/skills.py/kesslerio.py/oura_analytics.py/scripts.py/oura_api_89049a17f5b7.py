# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\oura_api.py
#!/usr/bin/env python3
"""
...
class OuraClient:
    ...
    def __init__(self, token=None, use_cache=True):
        ...
        if not self.token:
            raise ValueError(
                "OURA_API_TOKEN not set. Get it at https://cloud.ouraring.com/personal-access-token"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.code == 429:
                    ...
                    try:
                        ...
                        try:
                            ...
                            wait_time = max(
                                0, int((retry_date - datetime.now()).total_seconds())
                            )
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"Network error. Retrying in {wait_time}s... ({attempt + 1}/{max_retries})",
                        file=sys.stderr,
                    )
                    time.sleep(wait_time)
                    continue
            ...
            for item in fresh_data:
                ...
                if item_date:
                    ...
                    day_data = [
                        i
                        for i in fresh_data
                        if (i.get("day") or i.get("timestamp", "")[:10]) == item_date
                    ]
        ...
        if last_sync:
            ...
            try:
                ...
                if last_sync_date > today:
                    print(
                        f"Warning: last_sync in future ({last_sync}), resetting",
                        file=sys.stderr,
                    )
                    last_sync = None
            except ValueError:
                print(
                    f"Warning: invalid last_sync ({last_sync}), resetting",
                    file=sys.stderr,
                )
                last_sync = None
    ...
    def get_recent_sleep(self, days=2):
        ...
        url_daily = (
            f"{self.BASE_URL}/daily_sleep?start_date={start_date}&end_date={end_date}"
        )
        ...
        with urllib.request.urlopen(req_daily, timeout=10) as resp_daily:
            daily_data = {
                item["day"]: item
                for item in json.loads(resp_daily.read().decode("utf-8")).get(
                    "data", []
                )
            }
...
class OuraAnalyzer:
    ...
    def summary(self):
        """Generate summary"""
        avg_sleep_hours = self.average_metric(
            self.sleep, "total_sleep_duration", convert_to_hours=True
        )
...
def main():
    ...
    parser.add_argument(
        "--timezone",
        help="User timezone (default: USER_TIMEZONE env or America/Los_Angeles)",
    )
    parser.add_argument(
        "--local-time",
        action="store_true",
        help="Show dates in local time instead of UTC",
    )
    parser.add_argument(
        "--endpoint",
        choices=["sleep", "daily_readiness", "daily_activity", "all"],
        help="Endpoint for sync/cache commands",
    )
    parser.add_argument(
        "--clear", action="store_true", help="Clear cache (for cache command)"
    )
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable cache for this request"
    )
    parser.add_argument(
        "--format",
        choices=["json", "brief", "alert", "silent"],
        default="json",
    ...
    try:
        ...
        if args.command == "sleep":
            ...
            start_date_extended = (
                datetime.now() - timedelta(days=doubled_days)
            ).strftime("%Y-%m-%d")
            ...
            previous_sleep = (
                sleep[: -args.days][-args.days :] if len(sleep) > args.days else []
            )

            current_readiness = readiness[-args.days :] if len(readiness) > 0 else []
            previous_readiness = (
                readiness[: -args.days][-args.days :]
                if len(readiness) > args.days
                else []
            )
            ...
            report = reporter.generate_report(
                args.type, args.days, user_tz=args.timezone
            )
            ...
            if args.clear:
                ...
                deleted = client.cache.clear(
                    args.endpoint if args.endpoint != "all" else None
                )
