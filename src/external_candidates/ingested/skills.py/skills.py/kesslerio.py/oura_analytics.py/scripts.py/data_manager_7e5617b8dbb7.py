# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\data_manager.py
#!/usr/bin/env python3
"""
...
class OuraDataManager:
    ...
    def get_storage_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "total_size_bytes": 0,
            "cache": {"size_bytes": 0, "file_count": 0, "endpoints": {}},
            "events": {
                "exists": self.events_file.exists(),
                "size_bytes": 0,
                "count": 0,
            },
            "config": {"exists": self.config_file.exists(), "size_bytes": 0},
            "alert_state": {"exists": self.alert_state_file.exists(), "size_bytes": 0},
        }
    ...
    def clear_cache(
        self, endpoint: Optional[str] = None, confirm: bool = False
    ) -> None:
        """
        Clear cached data.
    ...
    def cleanup_old_cache(self, days: int = 90) -> None:
        ...
        if deleted_count > 0:
            print(
                f"Deleted {deleted_count} files older than {days} days", file=sys.stderr
            )
