# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\alerts.py
#!/usr/bin/env python3
"""
...
def check_thresholds_legacy(sleep_data, readiness_data, thresholds):
    ...
    for day in sleep_data:
        ...
        if readiness_score is not None and readiness_score < thresholds.get(
            "readiness", 60
        ):
...
def send_telegram(message, chat_id=None, bot_token=None):
    ...
    bot_token = (
        bot_token
        or os.environ.get("KESSLER_TELEGRAM_BOT_TOKEN")
...
def main():
    ...
    parser.add_argument(
        "--readiness", type=int, default=60, help="Readiness threshold (legacy)"
    )
    parser.add_argument(
        "--efficiency", type=int, default=80, help="Efficiency threshold (legacy)"
    )
    parser.add_argument(
        "--sleep-hours", type=float, default=7, help="Sleep hours threshold (legacy)"
    )
    ...
    try:
        ...
        if args.config:
            ...
            alerts = check_thresholds_with_quality(sleep, readiness, config, state)
        else:
        ...
        if alerts:
            ...
            for alert in alerts:
                ...
                if consecutive:
                    print(
                        f"  {alert['date']} ({consecutive} bad days): {', '.join(alert['alerts'])}"
                    )
                else:
        ...
        output_dir = os.environ.get(
            "OURA_OUTPUT_DIR", str(Path.home() / ".oura-analytics" / "reports")
        )
        alert_file = f"{output_dir}/oura_alerts_{end_date}.json"
        os.makedirs(output_dir, exist_ok=True)
        with open(alert_file, "w") as f:
            json.dump(
                {"period": f"{start_date} to {end_date}", "alerts": alerts}, f, indent=2
            )
        print(f"\n💾 Saved to {alert_file}")
