...
class Driver:
    ...
    def __str__(self) -> str:
        emoji = (
            ...
            else "⚠️" if self.impact == "negative" else "➡️"
        )
        sign = "+" if self.delta > 0 else ""
        return (
            f"{emoji} {self.metric}: {self.value} ({sign}{self.delta:.1f} vs baseline)"
        )
...
class DriverAnalyzer:
    ...
    def analyze_sleep_drivers(self, sleep_data: Dict) -> List[Driver]:
        ...
        if duration_hours > 0:
            ...
            drivers.append(
                Driver(
                    "Sleep Duration", duration_hours, baseline, delta, impact, severity
                )
            )
        ...
        if efficiency:
            ...
            drivers.append(
                Driver("Efficiency", efficiency, baseline, delta, impact, severity)
            )

        # Deep sleep
        deep_sleep_hours = (
            sleep_data.get("deep_sleep_duration", 0) / 3600
            if sleep_data.get("deep_sleep_duration")
            else 0
        )
        if deep_sleep_hours > 0:
            ...
            drivers.append(
                Driver(
                    "Deep Sleep", deep_sleep_hours, baseline, delta, impact, severity
                )
            )
        ...
        rem_sleep_hours = (
            ...
            if sleep_data.get("rem_sleep_duration")
            else 0
        )
        if rem_sleep_hours > 0:
            ...
            severity = self._classify_severity(abs(delta), thresholds=[0.3, 0.5])
            drivers.append(
                Driver("REM Sleep", rem_sleep_hours, baseline, delta, impact, severity)
            )
    ...
    def analyze_readiness_drivers(
        self, sleep_data: Dict, readiness_data: Dict
    ) -> List[Driver]:
        """
        ...
        if temp_dev is not None:
            ...
            impact = self._classify_impact(
                -abs(delta), thresholds=[0.2, 0.4]
            )  # Any deviation is negative
            severity = self._classify_severity(abs(delta), thresholds=[0.2, 0.4])
            drivers.append(
                Driver("Temperature", temp_dev, baseline, delta, impact, severity)
            )
    ...
    def generate_suggestion(self, readiness_score: float, drivers: List[Driver]) -> str:
        ...
        negative_drivers = [
            d
            for d in drivers
            if d.impact == "negative" and d.severity in ["high", "medium"]
        ]
        ...
        if readiness_score >= 85:
            if not negative_drivers:
                ...
                return "Good recovery, but watch: " + ", ".join(
                    d.metric for d in negative_drivers[:2]
                )
