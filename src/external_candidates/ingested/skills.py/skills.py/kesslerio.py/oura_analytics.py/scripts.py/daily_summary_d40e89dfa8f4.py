# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kesslerio\oura-analytics\scripts\daily_summary.py
#!/usr/bin/env python3
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Daily Oura Summary with Driver Analysis"
    )
    parser.add_argument("--date", help="Date (YYYY-MM-DD, default: yesterday)")
    parser.add_argument(
        "--baseline-days", type=int, default=30, help="Days for baseline calculation"
    )
    ...
    try:
        ...
        if readiness_score:
            ...
            delta_str = (
                f"+{delta_ready:.0f}" if delta_ready > 0 else f"{delta_ready:.0f}"
            )
            ...
            if negative_drivers:
                print(
                    format_drivers_report(
                        negative_drivers, "   └─ Factors pulling down"
                    )
                )
        ...
        if hrv or rhr:
            ...
            if hrv:
                ...
                delta_str = (
                    f"+{delta_hrv:.0f}ms" if delta_hrv > 0 else f"{delta_hrv:.0f}ms"
                )
            ...
            if rhr:
                ...
                delta_str = (
                    f"+{delta_rhr:.0f}bpm" if delta_rhr > 0 else f"{delta_rhr:.0f}bpm"
                )
