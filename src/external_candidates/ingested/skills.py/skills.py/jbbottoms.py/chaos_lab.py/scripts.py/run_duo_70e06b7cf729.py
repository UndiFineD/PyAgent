# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jbbottoms\chaos-lab\scripts\run-duo.py
#!/usr/bin/env python3
"""
...
def read_sandbox() -> str:
    ...
    for file in SANDBOX.rglob("*"):
        if (
            file.is_file()
            and file.name != "run-experiment.py"
            and file.name != "experiment-log.md"
        ):
            try:
                contents.append(
