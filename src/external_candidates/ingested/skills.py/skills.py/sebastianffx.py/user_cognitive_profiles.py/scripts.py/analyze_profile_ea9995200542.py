# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sebastianffx\user-cognitive-profiles\scripts\analyze_profile.py
#!/usr/bin/env python3
"""
...
class BM25:
    ...
    def __init__(self, corpus: List[str], k1: float = 1.5, b: float = 0.75):
        ...
        for term, df in self.df.items():
            # BM25 IDF formula
            self.idf[term] = (
                np.log((self.doc_count - df + 0.5) / (df + 0.5) + 1.0)
                if NUMPY_AVAILABLE
...
class ConversationAnalyzer:
    ...
    def extract_features(
        self, conversation: Dict[str, Any], corpus: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        # Message lengths
        user_lengths = [
            len(self.word_pattern.findall(m.get("content", ""))) for m in user_messages
        ]
        assistant_lengths = [
            len(self.word_pattern.findall(m.get("content", "")))
    ...
    def _extract_keywords(
        self, text: str, corpus: Optional[List[str]] = None, top_n: int = 10
    ) -> List[str]:
        """
        ...
        if corpus and len(corpus) > 1:
            try:
                ...
                unique_tokens = [
                    t for t in set(tokens) if t not in stop_words and len(t) > 3
                ]
...
class CognitiveProfiler:
    ...
    def __init__(
        self, num_archetypes: int = 3, custom_archetypes: Optional[Dict] = None
    ):
    ...
    def analyze_conversations(
        self, conversations: List[Dict[str, Any]], sample_size: Optional[int] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _fallback_cluster(
        ...
        for i in range(self.num_archetypes):
            start = i * cluster_size
                ...
                start + cluster_size
                if i < self.num_archetypes - 1
                else len(sorted_features)
            )
    ...
    def identify_archetypes(
        ...
        for cluster_id, conversations in clusters.items():
            ...
            # Aggregate metrics
            avg_length = sum(c["avg_user_message_length"] for c in conversations) / len(
                conversations
            )
            avg_question_ratio = sum(c["question_ratio"] for c in conversations) / len(
                conversations
            )
            avg_code_ratio = sum(c["code_block_count"] for c in conversations) / len(
                conversations
            )
            ...
            archetype_name = self._classify_archetype(
                avg_length, avg_question_ratio, top_keywords, best_match
            )
            ...
            archetypes[cluster_id] = {
                ...
                "sample_conversations": [
                    c["conversation_id"] for c in conversations[:3]
                ],
            ...
            if bm25_match in self.custom_archetypes:
                ...
                return {
                    ...
                    "recommendations": {
                        ...
                        "communication_style": archetype_def.get(
    ...
    def generate_profile(
        ...
        primary_archetype = max(
            archetypes.values(), key=lambda x: x["metrics"]["conversation_count"]
        )
        ...
        profile = {
            ...
            "insights": {
                ...
                "context_switching": (
                    "high"
                    if len(shifts) > 3
                    else "moderate" if len(shifts) > 0 else "low"
                ),
                "communication_preferences": self._generate_preferences(
                    primary_archetype
                ),
            },
        }
...
def load_conversations(filepath: str) -> List[Dict[str, Any]]:
    ...
    if isinstance(data, list):
        ...
        raise ValueError(
            "Unknown conversation format. Expected list or {conversations: [...]}"
        )
...
Examples:
  ...
  %(prog)s --input conversations.json --sample 1000 --output profile.json
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="cognitive-profile.json",
    ...
    parser.add_argument(
        "--archetypes-config", "-c", help="Path to custom archetypes YAML config"
    )
    parser.add_argument(
        "--sample",
        "-s",
    ...
    profiler = CognitiveProfiler(
        num_archetypes=args.archetypes, custom_archetypes=custom_archetypes
    )
    ...
    if args.format == "json":
        ...
        for archetype in profile["archetypes"]:
            print(
                f"    • {archetype['name']}: {archetype['metrics']['conversation_count']} conversations"
            )
