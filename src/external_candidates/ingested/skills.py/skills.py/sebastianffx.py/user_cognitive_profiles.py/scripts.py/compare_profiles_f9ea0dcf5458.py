# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sebastianffx\user-cognitive-profiles\scripts\compare_profiles.py
#!/usr/bin/env python3
"""
...
def compare_archetypes(old_profile: Dict, new_profile: Dict):
    ...
    for name in sorted(all_names):
        ...
        if old and new:
            ...
            print(f"  {name}:")
            print(
                f"    {old_count} → {new_count} conversations ({change:+d}, {pct_change:+.1f}%)"
            )
        elif old:
...
"""
    ...
    if (
        new_profile["insights"]["primary_mode"]
        != old_profile["insights"]["primary_mode"]
    ):
        report += """
