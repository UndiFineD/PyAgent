# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sebastianffx\user-cognitive-profiles\test_wildchat.py
#!/usr/bin/env python3
"""
...
from analyze_profile import CognitiveProfiler, ConversationAnalyzer


def load_wildchat_streaming(
    num_users: int = 4, convs_per_user: int = 5, min_archetypes: int = 2
):
    """
    ...
    print(
        f"📊 Collecting ~{target_convs}+ conversations from {num_users} unique users..."
    )
    print(
        f"   (Each user needs at least {min_archetypes} conversations for clustering)\n"
    )
    ...
    for i, conv in enumerate(dataset):
        ...
        if (
            len(user_conversations) >= num_users * 2
            and hashed_ip not in user_conversations
        ):
            continue
        ...
        if len(user_conversations[hashed_ip]) >= target_convs:
            if hashed_ip not in users_complete:
                ...
                print(
                    f"   ✓ User {len(users_complete)}/{num_users}: {hashed_ip[:16]}... ({actual_convs} conversations)"
                )
    ...
    if len(result) < num_users:
        print(
            f"   ⚠️  Warning: Only found {len(result)}/{num_users} users with sufficient data"
        )
...
def format_wildchat_conversation(conv: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    title = (
        first_user_msg[:50] + "..."
        if first_user_msg and len(first_user_msg) > 50
        else (first_user_msg or "Untitled")
    )
...
def analyze_user_profile(
    hashed_ip: str, conversations: List[Dict], num_archetypes: int = 2
    ...
    if len(conversations) < num_archetypes:
        print(
            f"   ⚠️  Skipping: Only {len(conversations)} conversations, need at least {num_archetypes} for clustering"
        )
        return None
...
def compare_users(profiles: List[Dict[str, Any]]):
    ...
    for profile in profiles:
        ...
        primary_arch = next(
            (a for a in profile["archetypes"] if a["name"] == primary), None
        )
        if primary_arch:
            metrics = primary_arch["metrics"]
            print(f"\n   User {user_id} ({primary}):")
            print(
                f"      Avg message length: {metrics['avg_message_length']:.0f} words"
            )
...
Examples:
  ...
  %(prog)s --output results.json        # Save results to JSON
    ...
    parser.add_argument(
        "--compare", action="store_true", help="Show comparative analysis between users"
    )
