# Extracted from: C:\DEV\PyAgent\.external\skills\skills\adebayoabdushaheed-a11y\afame\scripts\illustrate.py
#!/usr/bin/env python3
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument(
        ...
        action="append",
        default=None,
        help="Illustration subject (repeatable)",
    )
    ...
    p.add_argument(
        ...
        action="append",
        default=None,
        help="Full custom prompt (repeatable)",
    )
    p.add_argument(
        "--count", type=int, default=1, help="Number of variants per subject"
    )
    ...
    p.add_argument(
        "--sleep", type=float, default=0.3, help="Pause between requests (seconds)"
    )
    ...
    for i, (prompt, metadata) in enumerate(prompts_with_meta, 1):
        ...
        subject_label = (
            metadata.get("subject", "illustration") if metadata else "illustration"
        )
