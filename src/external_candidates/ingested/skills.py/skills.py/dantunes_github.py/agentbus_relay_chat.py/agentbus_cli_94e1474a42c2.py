...
def verify_event(event: dict) -> bool:
    try:
        ...
        expected_id = compute_event_id(
            pubkey, int(created_at), int(kind), tags, content
        )
...
class RelayManager:
    ...
    async def start(self) -> None:
        for idx, url in enumerate(self.relays):
            ...
            client = RelayClient(
                url, sub_id, self.filters, self._handle_event, self.log
            )
...
def main() -> None:
    ...
    if args.write_allowlist:
        if not args.allowlist_agents:
            ...
            return
        agent_names = [
            name.strip() for name in args.allowlist_agents.split(",") if name.strip()
        ]
        if not agent_names:
    ...
    async def on_event(event: dict) -> None:
        ...
        if not isinstance(tags_list, list):
            ...
            return
        if not (
            has_tag(tags_list, "c", args.chan) and has_tag(tags_list, "sid", session_id)
        ):
        ...
        if kind == KIND_PUBLIC:
            ...
            if args.mode == "enc":
                ...
                if not isinstance(wrapper, dict):
                    return
                if (
                    wrapper.get("v") != PROTOCOL_VERSION
                    or wrapper.get("sid") != session_id
                ):
