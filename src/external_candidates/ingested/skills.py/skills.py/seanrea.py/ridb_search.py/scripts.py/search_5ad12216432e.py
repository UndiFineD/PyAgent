...
def geocode(location: str) -> tuple[float, float]:
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "ridb-search/1.0 (camping availability checker)"}
    )
...
def main():
    ...
    parser.add_argument(
        "--location", "-l", help="Location to search near (e.g., 'Bend, OR')"
    )
    ...
    parser.add_argument(
        "--camping-only", action="store_true", help="Filter to camping facilities only"
    )
    parser.add_argument(
        "--reservable-only",
        action="store_true",
    ...
    if args.json:
        output = {
            ...
            "total_count": result.get("METADATA", {})
            .get("RESULTS", {})
            .get("TOTAL_COUNT", 0),
