# Extracted from: C:\DEV\PyAgent\.external\skills\skills\seanrea\recgov-availability\scripts\check.py
#!/usr/bin/env python3
"""
...
def get_campground_name(campground_id: str) -> str:
    ...
    try:
        ...
        return data.get("campground", {}).get(
            "facility_name", f"Campground {campground_id}"
        )
    except Exception:
...
def get_availability(
    campground_id: str, start_date: datetime, end_date: datetime
) -> dict:
    """
    ...
    while current <= end_date:
        ...
        try:
            ...
            print(
                f"Warning: Failed to fetch month {current.strftime('%Y-%m')}: {e}",
                file=sys.stderr,
            )
...
def check_consecutive_availability(
    ...
    while check_date + timedelta(days=nights) <= end_date + timedelta(
        days=30
    ):  # Look ahead
        dates_to_check = [
        ...
        all_available = all(
            availabilities.get(d) == STATUS_AVAILABLE for d in dates_to_check
        )
...
def analyze_campground_status(
    campsites: dict, start_date: datetime, nights: int
) -> dict:
    """
    ...
    for site_data in campsites.values():
        ...
        if (
            STATUS_NYR in all_statuses
            or STATUS_RESERVED in all_statuses
            or STATUS_AVAILABLE in all_statuses
        ):
...
def matches_amenity_filters(details: dict, filters: dict) -> bool:
    ...
    amenities = {
        a.get("attribute_code"): a.get("attribute_value")
        for a in details.get("amenities", [])
    }
    attributes = {
        a.get("attribute_code"): a.get("attribute_value")
    ...
    # Check each filter
    if filters.get("pets") and all_attrs.get("pets_allowed", "").lower() not in [
        "yes",
        "y",
        "true",
    ]:
    ...
    if filters.get("shade") and all_attrs.get("shade", "").lower() not in [
        "full",
        "partial",
    ]:
        return False
    ...
    if filters.get("campfire") and all_attrs.get(
        "campfire_allowed", ""
    ).lower() not in ["yes", "y", "true"]:
        return False
...
def format_results(results: list, json_output: bool = False) -> str:
    ...
    for cg in results:
        ...
        if status.get("nyr", 0) == total and total > 0:
            ...
            lines.append(
                f"   ⏳ NOT YET RELEASED — Reservations not open for these dates"
            )
        elif (nyr + not_yet_bookable) == total and total > 0:
            # Reservable campground but dates not open yet
            lines.append(
                f"   ⏳ NOT YET BOOKABLE — Check back when 6-month window opens"
            )
        elif status.get("fcfs", 0) == total and total > 0:
            # All sites are first-come-first-served
            lines.append(
                f"   🚗 FIRST-COME-FIRST-SERVED — No reservations, show up early"
            )
        elif available > 0:
            ...
            if reserved > 0 and (nyr + not_yet_bookable) > 0:
                lines.append(
                    f"   ❌ SOLD OUT — {reserved} reserved, {nyr + not_yet_bookable} not yet bookable"
                )
            elif (nyr + not_yet_bookable) > 0:
                lines.append(
                    f"   ⏳ {nyr + not_yet_bookable}/{total} sites not yet bookable for these dates"
                )
            elif fcfs > 0:
                lines.append(
                    f"   🚗 {fcfs}/{total} sites are first-come-first-served (not reservable)"
                )
            elif reserved > 0:
                lines.append(
                    f"   ❌ SOLD OUT — All {reserved} reservable sites are booked"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--campground", "-c", nargs="+", required=True, help="Campground ID(s) to check"
    )
    ...
    parser.add_argument(
        "--electric", action="store_true", help="Filter to electric sites only"
    )
    parser.add_argument(
        "--nonelectric", action="store_true", help="Filter to non-electric sites only"
    )
    parser.add_argument(
        "--include-group",
        action="store_true",
    ...
    parser.add_argument(
        "--show-sites", action="store_true", help="Show individual site details"
    )
