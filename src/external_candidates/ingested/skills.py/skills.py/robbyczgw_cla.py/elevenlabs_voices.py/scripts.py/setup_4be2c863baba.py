# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\elevenlabs-voices\scripts\setup.py
#!/usr/bin/env python3
"""
...
""")
    print(
        f"{Colors.DIM}Welcome! Let's configure your ElevenLabs integration.{Colors.RESET}"
    )
    print(
        f"{Colors.DIM}This will create a config.json file in the skill directory.{Colors.RESET}"
    )
    print()
    print(
        f"{Colors.GREEN}🔒 Privacy: Your API key is stored locally only.{Colors.RESET}"
    )
    print(
        f"{Colors.GREEN}   It never leaves your machine and is in .gitignore.{Colors.RESET}"
    )
    print()
...
def get_choice(prompt: str, options: list, default: str = None) -> str:
    ...
    while True:
        choice = input(
            f"\n{Colors.CYAN}>{Colors.RESET} Enter number [1-{len(options)}]: "
        ).strip()
        ...
        try:
            ...
            pass
        print(
            f"{Colors.RED}  Please enter a number between 1 and {len(options)}.{Colors.RESET}"
        )
...
def get_yes_no(prompt: str, default: bool = True) -> bool:
    ...
    while True:
        response = (
            input(f"{Colors.CYAN}>{Colors.RESET} {prompt} [{default_str}]: ")
            .strip()
            .lower()
        )
        if not response:
...
def get_optional_number(prompt: str) -> float | None:
    ...
    while True:
        ...
        try:
            ...
            print(
                f"{Colors.RED}  Please enter a valid number or press Enter to skip.{Colors.RESET}"
            )
...
def run_setup():
    ...
    print(
        f"{Colors.DIM}Get your API key from: https://elevenlabs.io/app/settings/api-keys{Colors.RESET}"
    )
    print()
    ...
    while True:
        ...
        if validate_api_key(api_key):
            ...
            break
        print(
            f"{Colors.RED}  Invalid API key format. Keys are typically 32+ alphanumeric characters.{Colors.RESET}"
        )
    ...
    voice_choice = get_choice(
        "Choose your default voice:",
        popular_voices,
        default="Rachel (warm, friendly - recommended)",
    )
    ...
    lang_choice = get_choice(
        "Default language for synthesis:", languages, default="English (en)"
    )
    # Extract language code from parentheses
    config["defaultLanguage"] = lang_choice.split("(")[1].rstrip(")")
    print(
        f"{Colors.GREEN}  ✓ Default language: {config['defaultLanguage']}{Colors.RESET}"
    )
    ...
    quality_choice = get_choice(
        "Audio quality preference:",
        qualities,
        default="standard (faster, smaller files)",
    )
    ...
    print(
        f"{Colors.DIM}You'll get a warning when approaching this limit.{Colors.RESET}"
    )
...
def main():
    ...
    if config_path.exists():
        ...
        if not get_yes_no("Do you want to reconfigure?", default=False):
            print(
                f"{Colors.DIM}Setup cancelled. Existing config preserved.{Colors.RESET}"
            )
            return 0
        print()
