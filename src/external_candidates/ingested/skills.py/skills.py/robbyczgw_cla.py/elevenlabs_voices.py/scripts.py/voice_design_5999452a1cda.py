# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\elevenlabs-voices\scripts\voice-design.py
#!/usr/bin/env python3
"""
...
VOICE_PREVIEW_URL = (
    "https://api.elevenlabs.io/v1/voice-generation/generate-voice/preview"
)
...
def generate_voice_preview(
    ...
    req = urllib.request.Request(
        VOICE_DESIGN_URL, data=data, headers=headers, method="POST"
    )
...
def save_voice_to_library(
    voice_id: str, name: str, description: str, api_key: str
) -> bool:
    """
...
Examples:
  ...
  python3 voice-design.py --options
    ...
    parser.add_argument(
        "--gender", "-g", default="female", help=f"Gender ({', '.join(VALID_GENDERS)})"
    )
    parser.add_argument(
        "--age", "-a", default="middle_aged", help=f"Age ({', '.join(VALID_AGES)})"
    )
    parser.add_argument(
        "--accent",
        "-c",
    ...
    parser.add_argument(
        "--style",
        choices=list(PREVIEW_TEXTS.keys()),
        default="en",
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="voice_preview.mp3",
        help="Output file (default: voice_preview.mp3)",
    )

    # Save to library
    parser.add_argument(
        "--save", metavar="NAME", help="Save voice to ElevenLabs library with this name"
    )

    # Info
    parser.add_argument("--options", action="store_true", help="List all valid options")
    ...
    if args.save:
        ...
        if voice_id:
            save_voice_to_library(voice_id, args.save, args.description, api_key)
        else:
