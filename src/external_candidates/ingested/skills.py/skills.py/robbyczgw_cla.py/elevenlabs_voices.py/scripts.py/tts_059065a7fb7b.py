# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\elevenlabs-voices\scripts\tts.py
#!/usr/bin/env python3
"""
...
def list_voices(voices_data: dict):
    ...
    for name, v in sorted(voices.items()):
        print(
            f"{name:<15} {v.get('language', 'n/a'):<10} {v.get('gender', 'n/a'):<8} {v.get('persona', 'n/a'):<15} {v.get('description', '')[:40]}..."
        )
            ...
            with urllib.request.urlopen(req, timeout=60) as response:
                with open(output_path, "wb") as f:
                    ...
                    while True:
                        ...
                        print(
                            f"\r⏳ Streaming: {total_bytes / 1024:.1f} KB",
                            end="",
                            flush=True,
                        )
                    print()
...
Examples:
  ...
  python3 tts.py --languages
    ...
    # Voice options
    parser.add_argument(
        "--voice", "-v", default="rachel", help="Voice name or preset (default: rachel)"
    )
    parser.add_argument("--lang", "-L", help="Language code (e.g., de, fr, es)")

    # Output options
    parser.add_argument(
        "--output", "-o", default="output.mp3", help="Output file (default: output.mp3)"
    )
    parser.add_argument(
        "--output-dir",
        "-d",
    ...
    parser.add_argument(
        "--stream", "-s", action="store_true", help="Use streaming mode"
    )

    # Info options
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available voices"
    )
    parser.add_argument(
        "--languages", action="store_true", help="List supported languages"
    )
    ...
    parser.add_argument(
        "--reset-stats", action="store_true", help="Reset usage statistics"
    )
    ...
    parser.add_argument(
        "--no-pronunciations",
        action="store_true",
        help="Disable pronunciation dictionary",
    )
