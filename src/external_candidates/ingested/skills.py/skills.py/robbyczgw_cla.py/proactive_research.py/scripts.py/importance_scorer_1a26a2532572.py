# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\proactive-research\scripts\importance_scorer.py
#!/usr/bin/env python3
"""
    ...
    def _score_conditions(self, content: str, title: str) -> Tuple[float, str]:
        ...
        for condition in alert_on:
            if condition == "price_change_10pct":
                ...
                if re.search(
                    r"(release|launch|announce).*\b(model|gpt|llm)\b", content, re.I
                ):
                    return 0.3, "model_release_detected"

            elif condition == "patch_release":
                if re.search(
                    r"(patch|update|version|release).*\d+\.\d+", content, re.I
                ):
