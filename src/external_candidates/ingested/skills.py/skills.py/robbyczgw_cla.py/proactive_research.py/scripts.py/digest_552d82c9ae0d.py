# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\proactive-research\scripts\digest.py
#!/usr/bin/env python3
"""
...
from config import FINDINGS_DIR, get_topic, load_config
...
def generate_digest(findings_by_topic: dict, start: datetime, end: datetime) -> str:
    ...
    digest += (
        f"📈 **Summary:** {total_findings} findings across {topic_count} topic(s)\n\n"
    )
    digest += "---\n\n"
    ...
    for topic_id, findings in findings_by_topic.items():
        ...
        for finding in findings:
            all_findings.append({"topic": topic, "finding": finding})
    ...
    for topic_id, findings in sorted(findings_by_topic.items()):
        ...
        sorted_findings = sorted(
            findings, key=lambda x: x.get("score", 0), reverse=True
        )
    ...
    digest += (
        "_Feature coming soon: AI-powered topic suggestions based on your findings_\n\n"
    )
...
def main():
    ...
    parser.add_argument(
        "--preview", action="store_true", help="Preview without sending"
    )
    ...
    print(
        f"📊 Generating digest for {start.strftime('%Y-%m-%d')} to {end.strftime('%Y-%m-%d')}"
    )
