...
def load_config() -> dict:
    ...
    try:
        if CONFIG_FILE.exists():
            ...
            print(
                f"⚠️  No config.json found. Copy config.example.json to config.json and customize it."
            )
...
def get_team_by_name(name: str) -> Optional[dict]:
    ...
    for team in get_teams():
        if (
            name_lower in team.get("name", "").lower()
            or name_lower in team.get("short_name", "").lower()
        ):
            return team
    return None
