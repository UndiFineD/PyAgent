# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\sports-ticker\scripts\espn.py
#!/usr/bin/env python3
"""
...
def get_match_details(
    event_id: str, sport: str = "soccer", league: str = "eng.1"
) -> dict:
...
def search_team(team_name: str, sport: str = "soccer", leagues: list = None) -> list:
    ...
    for league in leagues:
        try:
            ...
            for team in teams:
                if (
                    team_lower in team.get("displayName", "").lower()
                    or team_lower in team.get("shortDisplayName", "").lower()
                    or team_lower in team.get("nickname", "").lower()
                ):
                    results.append(
                        {
                            "id": team.get("id"),
...
def find_team_match(
    team_id: str, leagues: list = None, sport: str = "soccer"
) -> Optional[dict]:
...
def format_event(event: dict, sport: str = "soccer") -> str:
    ...
    player = (
        participants[0].get("athlete", {}).get("displayName", "")
        if participants
        else ""
    )
...
def get_fixtures_for_date(
    date_str: str, sport: str = "soccer", league: str = "eng.1"
) -> list:
...
def get_team_fixtures(
    team_id: str, days: int = 30, sport: str = "soccer", leagues: list = None
) -> list:
                        ...
                        fixtures.append(
                            {
                                ...
                                "opponent_short": opponent.get(
                                    "shortDisplayName", opponent.get("displayName")
                                ),
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "  espn.py scoreboard [league] [sport] - Get scoreboard (default: eng.1 soccer)"
        )
        print(
            "  espn.py search <team> [sport]     - Search for a team (default: soccer)"
        )
