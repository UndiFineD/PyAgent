# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\sports-ticker\scripts\schedule.py
#!/usr/bin/env python3
"""
...
from espn import LEAGUES, format_fixture, get_team_fixtures
...
def get_all_fixtures_json(days: int = 14, team_name: str = None) -> list:
    ...
    for team in teams:
        ...
        if team_name:
            ...
            if (
                name_lower not in team.get("name", "").lower()
                and name_lower not in team.get("short_name", "").lower()
            ):
                continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Show upcoming fixtures for your teams"
    )
    parser.add_argument(
        "--days",
        "-d",
