# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\sports-ticker\scripts\setup.py
#!/usr/bin/env python3
"""
...
from espn import SPORT_MAPPING, search_team
...
def step_select_teams(sports: list) -> list:
    ...
    for sport_key in sports:
                ...
                if results:
                    ...
                    try:
                        ...
                        if 0 <= idx < len(unique_results):
                            ...
                            emoji = (
                                input(
                                    f"Emoji for {r['name']} [{sport['emoji']}]: "
                                ).strip()
                                or sport["emoji"]
                            )
        ...
        if not any(t["sport"] == sport["espn_sport"] for t in all_teams):
            ...
            team_count = len(
                [t for t in all_teams if t["sport"] == sport["espn_sport"]]
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        if check_first_run():
            ...
            if choice == "1":
                confirm = get_yes_no(
                    "\n⚠️  This will replace your current config. Continue?",
                    default=False,
                )
                if confirm:
                    run_onboarding()
                ...
                sport = (
                    input(
                        "Sport (soccer/football/basketball/hockey/baseball/racing) [soccer]: "
                    ).strip()
                    or "soccer"
                )
                if name:
        ...
        print("Usage:")
        print(
            "  setup.py                      - Interactive setup (or menu if config exists)"
        )
        print(
            "  setup.py --force              - Run setup wizard (even if config exists)"
        )
