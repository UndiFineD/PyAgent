# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\sports-ticker\scripts\live_monitor.py
#!/usr/bin/env python3
"""
...
from config import get_alert_settings, get_teams
from espn import FOOTBALL_LEAGUES, find_team_match, get_match_events
...
def check_team(team: dict, state: dict, alerts_config: dict) -> tuple[list, dict]:
    ...
    if alerts_config.get("kickoff", True):
        ...
        if is_live and not was_live and prev_status != "Halftime":
            ...
            alerts.append(
                f"{start_emoji} **{start_text}** {team_emoji}\n{home_team} vs {away_team}\n{sport_leagues.get(league, league)}"
            )
            ...
            if sport == "soccer":
                if "Goal" in event_type:
                    ...
                    if "Own Goal" in event_type:
                        alerts.append(
                            f"{emoji} **OWN GOAL!** {clock}\n⚽ {player} ({e_team})\n**{score_line}**"
                        )
                    elif "Penalty" in event_type:
                        alerts.append(
                            f"{emoji} **PENALTY!** {clock}\n⚽ {player} ({e_team})\n**{score_line}**"
                        )
                    else:
                        alerts.append(
                            f"{emoji} **GOAL!** {clock}\n⚽ {player} ({e_team})\n**{score_line}**"
                ...
                if "Touchdown" in event_type:
                    scored = True
                    alerts.append(
                        f"{emoji} **TOUCHDOWN!** {clock}\n🏈 {player} ({e_team})\n**{score_line}**"
                    )
                elif "Field Goal" in event_type:
                    scored = True
                    alerts.append(
                        f"{emoji} **FIELD GOAL!** {clock}\n🎯 {player} ({e_team})\n**{score_line}**"
                    )
    ...
    if alerts_config.get("fulltime", True):
        if status == "Final" and prev_status not in ("Final", ""):
            ...
            alerts.append(
                f"🏁 **{final_text} - {result}** {result_emoji} {team_emoji}\n{home_team} {home_score}-{away_score} {away_team}"
            )
