# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\agent-chronicle\scripts\setup.py
#!/usr/bin/env python3
"""
...
def ask_yes_no(question: str, default: bool = True) -> bool:
    ...
    while True:
        answer = (
            input(f"{Colors.YELLOW}{question}{Colors.END} [{default_str}]: ")
            .strip()
            .lower()
        )
        if not answer:
...
def ask_choice(question: str, options: list, default: int = 0) -> str:
    ...
    while True:
        answer = input(
            f"\nChoice [1-{len(options)}] (default: {default + 1}): "
        ).strip()
        if not answer:
        ...
        try:
            ...
            pass
        print(
            f"{Colors.RED}Please enter a number between 1 and {len(options)}{Colors.END}"
        )
...
def run_setup():
    ...
    config = {
        ...
        "export": {"default_format": "pdf", "include_header": True, "style": "minimal"},
    }
    ...
    quote_enabled = ask_yes_no(
        "Enable Quote Hall of Fame? (memorable user quotes)", True
    )
    ...
    curiosity_enabled = ask_yes_no(
        "Enable Curiosity Backlog? (things to explore later)", True
    )
    ...
    decisions_enabled = ask_yes_no(
        "Enable Decision Archaeology? (log judgment calls)", True
    )
    ...
    relationship_enabled = ask_yes_no(
        "Enable Relationship Evolution? (track dynamic over time)", False
    )
    ...
    memory_enabled = ask_yes_no(
        "Also add diary summary to your daily memory log? (memory/YYYY-MM-DD.md)", True
    )
    ...
    config["auto_generate"] = ask_yes_no(
        "Auto-generate diary entries on heartbeat?", False
    )

    # 6. Export format
    export_format = ask_choice(
        "Default export format?", ["pdf", "html", "markdown"], default=0
    )
