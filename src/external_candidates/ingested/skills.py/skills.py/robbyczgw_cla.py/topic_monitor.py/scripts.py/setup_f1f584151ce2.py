# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\topic-monitor\scripts\setup.py
#!/usr/bin/env python3
"""
...
def build_config(topics: list, settings: dict) -> dict:
    ...
    config = {"topics": topics, "settings": settings, "channels": channels}
...
def print_summary(config: dict):
    ...
    for topic in topics:
        ...
        print(
            f"     Keywords: {', '.join(topic['keywords'][:3])}{'...' if len(topic['keywords']) > 3 else ''}"
        )
        print(
            f"     Frequency: {topic['frequency']}, Threshold: {topic['importance_threshold']}"
        )

    print()
    print(f"⚙️  Settings:")
    print(
        f"   • Weekly digest: {'Enabled' if settings.get('digest_enabled', True) else 'Disabled'}"
    )
    ...
    print(
        f"   • Learning mode: {'Enabled' if settings.get('learning_enabled', True) else 'Disabled'}"
    )
...
def main():
    ...
    if CONFIG_FILE.exists():
        ...
        overwrite = prompt_yes_no(
            "Do you want to start fresh and overwrite it?", default=False
        )
        if not overwrite:
