# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\agent-protocol\scripts\event_bus.py
#!/usr/bin/env python3
"""
...
class EventBus:
    ...
    def _load_config(self, config_path: Path) -> Dict:
        ...
        defaults = {
            ...
            "security": {"require_permissions": False, "audit_log": True},
        }
    ...
    def publish(self, event: Dict) -> tuple[bool, str]:
        ...
        try:
            ...
            self._log(
                f"Published event: {event_id} (type: {event['event_type']}, source: {event['source_agent']})"
            )
        ...
        try:
            for event_file in sorted(QUEUE_DIR.glob("*.json")):
                try:
                    ...
                    if event_types:
                        if not self._matches_event_types(
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "log_size_kb": (
                self.log_file.stat().st_size // 1024 if self.log_file.exists() else 0
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "command", choices=["start", "stop", "status", "tail", "cleanup", "stats"]
    )
    parser.add_argument(
        "--count", type=int, default=20, help="Number of events to tail"
    )
