# Extracted from: C:\DEV\PyAgent\.external\skills\skills\robbyczgw-cla\personal-analytics\scripts\report.py
#!/usr/bin/env python3
"""
...
from config import get_report_settings, is_enabled
...
def generate_markdown_report(analysis: Dict, report_type: str) -> str:
    ...
    if hourly_dist:
        ...
        blocks = {
            ...
            "22-06": sum(
                hourly_dist.get(h, 0) for h in list(range(22, 24)) + list(range(0, 6))
            ),
        }
    ...
    if distribution:
        ...
        emoji_map = {"positive": "😊", "neutral": "😐", "negative": "😟", "mixed": "🤔"}
...
def main():
    ...
    parser.add_argument(
        "type", choices=["weekly", "monthly", "custom"], help="Report type"
    )
    ...
    parser.add_argument(
        "--send", action="store_true", help="Send via configured channel"
    )
