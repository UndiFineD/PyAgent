...
CACHE_DIR = Path(
    os.environ.get("SURFLINE_CACHE_DIR", str(Path.home() / ".cache" / "surfline"))
)
...
def _http_get_json(url: str, timeout: int = 20) -> Any:
    req = urllib.request.Request(
        url, headers={"User-Agent": "clawdbot-surfline-skill/0.1"}
    )
...
def kbyg_wave(spot_id: str, days: int = 2, interval_hours: int = 1) -> Dict[str, Any]:
    params = urllib.parse.urlencode(
        {"spotId": spot_id, "days": str(days), "intervalHours": str(interval_hours)}
    )
