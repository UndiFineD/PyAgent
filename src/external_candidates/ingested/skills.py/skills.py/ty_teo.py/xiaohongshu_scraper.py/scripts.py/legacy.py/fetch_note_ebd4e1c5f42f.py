# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ty-teo\xiaohongshu-scraper\scripts\legacy\fetch_note.py
#!/usr/bin/env python3
"""
小红书单篇笔记爬取脚本
抓取笔记内容和所有图片
        ...
        try:
            ...
            try:
                page.wait_for_selector(
                    '#detail-desc, .note-content, [class*="desc"], .desc', timeout=15000
                )
            except:
            ...
            author_el = page.query_selector(
                '.author-container .username, .username, [class*="author"] .name'
            )
            if author_el:
            ...
            content_el = page.query_selector(
                '#detail-desc, .note-content, [class*="desc"]'
            )
            if content_el:
                result["content"] = content_el.inner_text().strip()

            # 提取点赞数
            like_el = page.query_selector(
                '[class*="like-wrapper"] .count, .like-count, [class*="like"] span.count'
            )
            if like_el:
                result["likes"] = like_el.inner_text().strip()

            # 提取收藏数
            collect_el = page.query_selector(
                '[class*="collect-wrapper"] .count, .collect-count, [class*="collect"] span.count'
            )
            if collect_el:
                result["collects"] = collect_el.inner_text().strip()

            # 提取评论数
            comment_el = page.query_selector(
                '[class*="chat-wrapper"] .count, .comment-count, [class*="comment"] span.count'
            )
            if comment_el:
            ...
            # 提取标签
            tag_els = page.query_selector_all(
                '#detail-desc a[href*="search_result"], .tag, [class*="tag"]'
            )
            ...
            # 提取所有图片
            # 方法1: 从轮播图获取
            carousel_imgs = page.query_selector_all(
                '.swiper-slide img, [class*="carousel"] img, [class*="slider"] img'
            )
            ...
            # 方法2: 从主图获取
            main_imgs = page.query_selector_all(
                '.note-slider img, .media-container img, [class*="note-image"] img'
            )
            ...
            indicators = page.query_selector_all(
                '.swiper-pagination-bullet, [class*="indicator"] span, [class*="dot"]'
            )
            if len(indicators) > 1:
                ...
                for i, indicator in enumerate(indicators):
                    try:
                        indicator.click()
                        time.sleep(1)
                        current_img = page.query_selector(
                            '.swiper-slide-active img, [class*="active"] img'
                        )
                        if current_img:
...
def main():
    ...
    result = fetch_note(
        url=args.url, output_dir=output_dir, user_data_dir=args.user_data
    )
