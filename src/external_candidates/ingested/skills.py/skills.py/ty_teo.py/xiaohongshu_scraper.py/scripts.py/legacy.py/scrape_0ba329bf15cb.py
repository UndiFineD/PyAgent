# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ty-teo\xiaohongshu-scraper\scripts\legacy\scrape.py
#!/usr/bin/env python3
"""
小红书内容爬取脚本
    ...
    for card in cards[:max_count]:
        try:
            ...
            author_el = card.query_selector(
                '.author-wrapper .name, .name, [class*="author"] .name'
            )
            if author_el:
            ...
            like_el = card.query_selector(
                '[class*="like"] span, .like-wrapper span, .count'
            )
            if like_el:
...
def main():
    ...
    parser.add_argument(
        "--count", "-c", type=int, default=10, help="爬取数量 (默认 10)"
    )
    ...
    output_file = (
        args.output
        or f"xiaohongshu_{args.keyword}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
