# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ty-teo\xiaohongshu-scraper\scripts\legacy\fetch_note_v2.py
#!/usr/bin/env python3
"""
小红书笔记抓取脚本 v2
        ...
        try:
            ...
            try:
                page.wait_for_selector(
                    '#detail-desc, .note-content, .desc, [class*="content"]',
                    timeout=20000,
                )
            except:
            ...
            # 尝试多种选择器提取标题
            title_selectors = ["#detail-title", ".title", '[class*="title"]', "h1"]
            ...
            try:
                collect_el = page.query_selector(
                    '[class*="collect-wrapper"] .count, .collect-count'
                )
                if collect_el:
            ...
            if len(indicators) > 1:
                ...
                for i, indicator in enumerate(indicators):
                    try:
                        ...
                        time.sleep(0.8)
                        # 获取当前显示的图片
                        current_imgs = page.query_selector_all(
