# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ty-teo\nvidia-image-gen\scripts\generate.py
#!/usr/bin/env python3
"""
...
NVIDIA_API_KEY = os.environ.get(
    "NVIDIA_API_KEY",
    "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",
)
...
def generate_image(
    ...
    if image_data:
        ...
        print(
            f"⚠️ Unexpected response format: {json.dumps(result, indent=2)}",
            file=sys.stderr,
        )
        return None
...
Examples:
  ...
  %(prog)s "A robot" --seed 12345
    ...
    parser.add_argument(
        "-i", "--input", help="Input image path (for editing with kontext model)"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--seed", type=int, default=0, help="Random seed, 0=random (default: 0)"
    )
    parser.add_argument(
        "--model",
        "-m",
