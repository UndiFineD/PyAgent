# Extracted from: C:\DEV\PyAgent\.external\skills\skills\lucakaufmann\snapmaker\scripts\snapmaker.py
#!/usr/bin/env python3
"""
...
def cmd_temps():
    ...
    for i, ext_name in enumerate(["extruder", "extruder1", "extruder2", "extruder3"]):
        ...
        print(
            f"   {active} T{i}: {temp:6.1f}°C (target: {target:5.0f}°C) power: {power:4.1f}% [{ext_state}]"
        )

    print()
    print(
        f"🛏️  Bed:    {bed.get('temperature', 0):6.1f}°C (target: {bed.get('target', 0):5.0f}°C) power: {bed.get('power', 0)*100:4.1f}%"
    )
...
def cmd_filament():
    ...
    for i, slot in enumerate(slots):
        ...
        print(
            f"   Slot {i}: {color_emoji} {filament_name} ({manufacturer}){official_tag}"
        )
        print(f"           Color: {color_hex}")
        print(f"           Temps: {hotend_min}-{hotend_max}°C nozzle, {bed_temp}°C bed")
        print(f"           Weight: {weight}g")
        print(
            f"           Sensor: {status} {'detected' if detected else 'not detected'}"
        )
        print()
