# Extracted from: C:\DEV\PyAgent\.external\skills\skills\poiley\whatisxlistening-to\skills\whatisxlistening-to\tests\test_server.py
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_from_env_no_display_name(self):
        with patch.dict(
            os.environ,
            {"LASTFM_API_KEY": "key123", "LASTFM_USERNAME": "testuser"},
            clear=True,
        ):
            cfg = server.load_config()
...
class TestGetConfig(unittest.TestCase):
    ...
    def test_caches_config(self):
        with patch.dict(
            os.environ, {"LASTFM_API_KEY": "k", "LASTFM_USERNAME": "u"}, clear=True
        ):
...
class TestGetStats(unittest.TestCase):
    ...
    def test_stats_api_fallback(self, mock_api):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = server.get_stats(
                {"api_key": "k", "username": "u"}, db_path=db_path
            )
    ...
    def test_stats_from_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            now = int(time.time())
            conn.execute(
                "INSERT INTO scrobbles (timestamp, artist, album, track) VALUES (?, ?, ?, ?)",
...
class TestGetRecent(unittest.TestCase):
    ...
    def test_recent_api_fallback(self, mock_api):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = os.path.join(tmpdir, "test.db")
            result = server.get_recent(
                {"api_key": "k", "username": "u"}, limit=10, page=1, db_path=db_path
            )
    ...
    def test_recent_from_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Add album art cache
            conn.execute(
                "INSERT INTO albums (artist, album, image_url) VALUES (?, ?, ?)",
...
class TestDatabaseFunctions(unittest.TestCase):
    ...
    def test_start_sync_thread(self, mock_sync, mock_init, mock_api):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            thread = server.start_sync_thread(
                {"api_key": "k", "username": "u"}, db_path=db_path
            )
    ...
    def test_api_now_endpoint(self, mock_now, mock_webdir):
        ...
        with patch.dict(
            ...
            with patch.object(server, "WEB_DIR", self.web_dir):
                ...
                try:
                    resp = urllib.request.urlopen(
                        f"http://127.0.0.1:{port}/api/now", timeout=5
                    )
    ...
    def test_api_stats_endpoint(self, mock_stats):
        ...
        with patch.dict(
            ...
            with patch.object(server, "WEB_DIR", self.web_dir):
                ...
                try:
                    resp = urllib.request.urlopen(
                        f"http://127.0.0.1:{port}/api/stats", timeout=5
                    )
                    data = json.loads(resp.read().decode())
    ...
    def test_api_recent_endpoint(self, mock_recent):
        ...
        with patch.dict(
            ...
            with patch.object(server, "WEB_DIR", self.web_dir):
                ...
                try:
                    resp = urllib.request.urlopen(
                        f"http://127.0.0.1:{port}/api/recent?limit=5&page=2", timeout=5
                    )
                    data = json.loads(resp.read().decode())
    ...
    def test_history_endpoint(self):
        with patch.dict(
            ...
            with patch.object(server, "WEB_DIR", self.web_dir):
                ...
                try:
                    resp = urllib.request.urlopen(
                        f"http://127.0.0.1:{port}/history", timeout=5
                    )
...
class TestConfigFromFile(unittest.TestCase):
    ...
    def test_load_from_file_no_display_name(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_file = config_dir / "config.json"
            config_file.write_text(
                json.dumps({"api_key": "filekey", "username": "fileuser"})
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(
                ...
                with patch.object(server, "WEB_DIR", web_dir):
                    ...
                    try:
                        resp = urllib.request.urlopen(
                            f"http://127.0.0.1:{port}/test.txt", timeout=5
                        )
                        data = resp.read().decode()
