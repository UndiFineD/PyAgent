# Extracted from: C:\DEV\PyAgent\.external\skills\skills\poiley\whatisxlistening-to\skills\whatisxlistening-to\server.py
#!/usr/bin/env python3
...
def get_now_playing(config):
    ...
    data = api_call(
        {"method": "user.getrecenttracks", "user": config["username"], "limit": 1},
        config,
    )
...
def get_stats(config, db_path=None):
    ...
    try:
        with _db_lock:
            ...
            try:
                ...
                cur.execute(
                    "SELECT COUNT(*) FROM scrobbles WHERE timestamp >= ?",
                    (today_start,),
                )
    ...
    artists_data = api_call(
        {"method": "user.gettopartists", "user": config["username"], "limit": 1}, config
    )
    artists = int(artists_data.get("topartists", {}).get("@attr", {}).get("total", 0))

    recent = api_call(
        {"method": "user.getrecenttracks", "user": config["username"], "limit": 200},
        config,
    )
    ...
    if days_with_scrobbles:
        ...
        check_date = (
            today_date
            if today_date in days_with_scrobbles
            else today_date - timedelta(days=1)
        )
        ...
        tracks.append(
            {
                ...
                "album": t.get("album", {}).get("#text", ""),
                "image": (
                    (t.get("image") or [{}])[1].get("#text", "")
                    if len(t.get("image") or []) > 1
                    else ""
                ),
...
def sync_scrobbles(config, db_path=None, full_backfill=False):
    ...
    while True:
        ...
        for t in tracks:
            ...
            artist = t.get("artist", {}).get("#text") or t.get("artist", {}).get(
                "name", ""
            )
            album = t.get("album", {}).get("#text", "")
        ...
        try:
            ...
            if config is None:
                self.send_error_json(
                    503, "Not configured. Set LASTFM_API_KEY and LASTFM_USERNAME."
                )
                return
