# Extracted from: C:\DEV\PyAgent\.external\skills\skills\poiley\whatisxlistening-to\skills\whatisxlistening-to\lastfm_cli.py
#!/usr/bin/env python3
...
def api_call(method, config, **params):
    ...
    try:
        ...
        if e.code == 403:
            print(
                "❌ Invalid API key. Get one at: https://www.last.fm/api/account/create",
                file=sys.stderr,
            )
        elif e.code == 404:
            print(
                f"❌ User '{config['username']}' not found on Last.fm", file=sys.stderr
            )
        else:
...
def cmd_setup():
    ...
    CONFIG_PATH.write_text(
        json.dumps({"api_key": api_key, "username": username}, indent=2)
    )
    ...
    if reg:
        try:
            reg_date = datetime.fromtimestamp(
                int(user.get("registered", {}).get("unixtime", 0))
            )
...
def cmd_recent(config, limit=10):
    ...
    for t in data.get("recenttracks", {}).get("track", []):
        if t.get("@attr", {}).get("nowplaying"):
            ...
            ts = t.get("date", {}).get("uts")
            prefix = (
                datetime.fromtimestamp(int(ts)).strftime("%H:%M") + " " if ts else ""
            )
        artist = t.get("artist", {}).get("#text", t.get("artist", {}).get("name", ""))
...
def cmd_backfill(config):
    ...
    while True:
        ...
        try:
            data = api_call(
                "user.getrecenttracks", config, limit=1000, page=page, extended=1
            )
        except SystemExit:
            break
...
def cmd_db_stats():
    ...
    cur.execute(
        "SELECT artist, COUNT(*) as c FROM scrobbles GROUP BY artist ORDER BY c DESC LIMIT 5"
    )
    ...
    if first and last:
        print(
            f"   Range:      {datetime.fromtimestamp(first).strftime('%Y-%m-%d')} → {datetime.fromtimestamp(last).strftime('%Y-%m-%d')}"
        )
    print(f"\n🏆 Top Artists:")
