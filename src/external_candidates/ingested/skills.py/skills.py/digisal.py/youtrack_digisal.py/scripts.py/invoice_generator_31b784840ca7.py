# Extracted from: C:\DEV\PyAgent\.external\skills\skills\digisal\youtrack-digisal\scripts\invoice_generator.py
#!/usr/bin/env python3
"""
...
class InvoiceGenerator:
    ...
    def get_project_time_data(
        self, project_id: str, from_date: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if issues:
                project_data["project"] = {
                    "name": issues[0].get("project", {}).get("name", project_id)
                }
        ...
        for issue in issues:
            ...
            total_minutes = sum(
                wi.get("duration", {}).get("minutes", 0) for wi in work_items
            )
    ...
    def generate_invoice_text(
        self, project_data: Dict[str, Any], month: Optional[str] = None
    ) -> str:
        """
        ...
        for issue in project_data["issues"]:
            ...
            lines.append(
                f"  Task total: {issue['total_hours']:.2f} hours (${issue['cost']:.2f})"
            )
...
def main():
    ...
    parser.add_argument(
        "--project", required=True, help="Project ID to generate invoice for"
    )
    parser.add_argument("--from-date", help="Start date (YYYY-MM-DD)")
    parser.add_argument("--month", help='Month label (e.g., "January 2026")')
    parser.add_argument(
        "--rate", type=float, default=100.0, help="Hourly rate (default: 100)"
    )
    parser.add_argument(
