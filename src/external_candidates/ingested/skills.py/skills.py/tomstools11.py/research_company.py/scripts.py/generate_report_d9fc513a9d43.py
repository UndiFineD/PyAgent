# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tomstools11\research-company\scripts\generate_report.py
#!/usr/bin/env python3
"""
...
def create_header_table(data, styles):
    ...
    header_content = [
        [
            Paragraph(
                '<font color="white" size="9">ACCOUNT RESEARCH REPORT</font>',
                styles["Normal"],
            )
        ],
        [
            Paragraph(
                f'<font color="white" size="20"><b>{company}</b></font>',
                styles["Normal"],
            )
        ],
        [Spacer(1, 8)],
        [
            Paragraph(
                f'<font color="#94a3b8" size="9"><b>Date:</b> {date}  |  <b>Source:</b> {url}  |  <b>Analyst:</b> Claude AI</font>',
                styles["Normal"],
            )
        ],
    ]
...
def create_section_heading(title, styles):
    ...
    elements.append(
        HRFlowable(width="100%", thickness=2, color=COLORS["border"], spaceAfter=8)
    )
    return elements
...
def create_tags(keywords, color_map, styles):
    ...
    tags_text = "  ".join(
        [
            f'<font color="{color_map.get(kw, "#334155")}" size="9">[{kw}]</font>'
            for kw in keywords
        ]
    )
...
def generate_pdf(data, output_path):
    ...
    if data.get("products"):
        ...
        if data["products"].get("differentiators"):
            ...
            for diff in data["products"]["differentiators"]:
                block = create_bordered_item(
                    diff, None, COLORS["accent_emerald"], styles
                )
                if block:
                    story.append(block)
        ...
        if data["products"].get("tech_stack"):
            ...
            tech_text = "  |  ".join(
                [f"<b>{k}:</b> {v}" for k, v in data["products"]["tech_stack"].items()]
            )
    ...
    if data.get("target_market"):
        ...
        if data["target_market"].get("business_model"):
            ...
            story.append(
                Paragraph(data["target_market"]["business_model"], styles["ReportBody"])
            )
    ...
    if data.get("use_cases"):
        ...
        for uc in data["use_cases"]:
            block = create_bordered_item(
                uc.get("title"), uc.get("description"), COLORS["accent_amber"], styles
            )
            if block:
                story.append(KeepTogether([block, Spacer(1, 6)]))
        story.append(Spacer(1, 8))
    ...
    if data.get("competitors"):
        ...
        if data.get("competitive_positioning"):
            story.append(Spacer(1, 8))
            story.append(
                Paragraph("<b>Competitive Positioning</b>", styles["SubHeading"])
            )
            story.append(
                Paragraph(data["competitive_positioning"], styles["ReportBody"])
            )
    ...
    if data.get("industry"):
        ...
        if data["industry"].get("trends"):
            ...
            for trend in data["industry"]["trends"]:
                story.append(
                    Paragraph(
                        f'<font color="#3b82f6">•</font>  {trend}', styles["ReportBody"]
                    )
                )
        ...
        if data["industry"].get("opportunities"):
            ...
            for opp in data["industry"]["opportunities"]:
                story.append(
                    Paragraph(
                        f'<font color="#10b981">•</font>  {opp}', styles["ReportBody"]
                    )
                )
        ...
        if data["industry"].get("challenges"):
            ...
            for ch in data["industry"]["challenges"]:
                story.append(
                    Paragraph(
                        f'<font color="#f59e0b">•</font>  {ch}', styles["ReportBody"]
                    )
                )
    ...
    if data.get("developments"):
        ...
        for dev in data["developments"]:
            ...
            block = create_bordered_item(
                f'<font color="#14b8a6">{date_str}</font> — {title}',
                desc,
                COLORS["accent_teal"],
                styles,
            )
    ...
    if data.get("lead_gen"):
        ...
        if kw.get("primary"):
            story.append(
                Paragraph("<b>Primary Service Keywords</b>", styles["SubHeading"])
            )
            story.append(
                Paragraph(
                    "  ".join(
                        [f'<font color="#3b82f6">[{k}]</font>' for k in kw["primary"]]
                    ),
                    styles["TagText"],
                )
            )
        ...
        if kw.get("vertical"):
            ...
            story.append(
                Paragraph(
                    "  ".join(
                        [f'<font color="#10b981">[{k}]</font>' for k in kw["vertical"]]
                    ),
                    styles["TagText"],
                )
            )
        ...
        if kw.get("technology"):
            ...
            story.append(
                Paragraph(
                    "  ".join(
                        [
                            f'<font color="#8b5cf6">[{k}]</font>'
                            for k in kw["technology"]
                        ]
                    ),
                    styles["TagText"],
                )
        ...
        if kw.get("pain_point"):
            ...
            story.append(
                Paragraph(
                    "  ".join(
                        [
                            f'<font color="#f59e0b">[{k}]</font>'
                            for k in kw["pain_point"]
                        ]
                    ),
                    styles["TagText"],
                )
    ...
    if data.get("info_gaps"):
        ...
        story.append(
            Paragraph(
                "The following information was not publicly available:",
                styles["MutedText"],
            )
        )
        for gap in data["info_gaps"]:
            story.append(
                Paragraph(f'<font color="#94a3b8">•</font>  {gap}', styles["MutedText"])
            )
        story.append(Spacer(1, 8))
