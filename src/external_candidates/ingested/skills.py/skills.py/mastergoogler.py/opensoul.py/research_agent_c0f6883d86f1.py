...
class ResearchAgent:
        ...
        for log in history:
            for metric in log.get("metrics", []):
                if metric.get("action") == "web_search":
                    ...
                    if (
                        query.lower() in past_query.lower()
                        or past_query.lower() in query.lower()
                    ):
    ...
    async def analyze_trends(self):
        ...
        if search_topics:
            ...
            for topic, count in sorted(
                search_topics.items(), key=lambda x: x[1], reverse=True
            )[:5]:
        ...
        self.logger.log(
            {
                ...
                "details": {
                    ...
                    "most_common_topic": (
                        max(search_topics, key=search_topics.get)
                        if search_topics
                        else None
                    ),
                },
    ...
    async def generate_summary(self):
        ...
        summary = {
            ...
            "searches_per_minute": (
                total_searches / (duration.total_seconds() / 60)
