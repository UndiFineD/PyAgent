# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mastergoogler\opensoul\agent_template.py
"""
...
class OpenSoulAgent:
    ...
    async def start_session(self, task_description=None):
        ...
        self.logger = AuditLogger(
            priv_wif=os.getenv("BSV_PRIV_WIF"), config=opensoul_config
        )
        ...
        await self._log_action(
            ...
            tokens_in=10,
            tokens_out=10,
            details={"task": task_description, "session_id": session_id},
            status="started",
        )
    ...
    async def _log_action(
        self, action_type, tokens_in, tokens_out, details=None, status="success"
    ):
        """
    ...
    async def check_budget(self):
        ...
        try:
            ...
            percentage_used = (
                (total_spent / self.budget_bsv * 100) if self.budget_bsv > 0 else 0
            )
    ...
    async def reflect_on_performance(self):
        ...
        try:
            ...
            total_tokens = sum(
                m.get("tokens_in", 0) + m.get("tokens_out", 0) for m in all_metrics
            )
