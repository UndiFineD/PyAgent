# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bilalmohamed187-cpu\gcal-pro\scripts\gcal_core.py
#!/usr/bin/env python3
"""
...
def parse_datetime(text: str, reference: datetime = None) -> datetime:
    ...
    if text_lower == "today":
        ...
        return (reference + timedelta(weeks=1)).replace(
            hour=9, minute=0, second=0, microsecond=0
        )
...
def delete_event(
    event_id: str, calendar_id: str = "primary", confirmed: bool = False
) -> bool:
    """
    Delete an event.
