# Extracted from: C:\DEV\PyAgent\.external\skills\skills\kys42\stock-info-explorer\scripts\yf.py
#!/usr/bin/env python3
# /// script
# dependencies = [
#   "yfinance",
...
def show_price(symbol, ticker, info):
    ...
    table.add_row(
        "Change", f"[{color}]{sign}{change:,.2f} ({sign}{pct_change:.2f}%)[/{color}]"
    )
    console.print(table)
...
def save_pro_chart(symbol, ticker, period="3mo", chart_type="candle", indicators=None):
    ...
    if indicators.get("rsi"):
        ...
        addplots.append(
            mpf.make_addplot(
                rsi, panel=rsi_panel, color="orange", width=1.0, ylabel="RSI"
            )
        )
        # guides (30/70)
        addplots.append(
            mpf.make_addplot(
                pd.Series(70, index=hist.index),
                panel=rsi_panel,
                color="gray",
                linestyle="--",
                width=0.7,
        ...
        if indicators.get("macd"):
            ...
            addplots.append(
                mpf.make_addplot(
                    macd, panel=macd_panel, color="blue", width=1.0, ylabel="MACD"
                )
            )
            addplots.append(
                mpf.make_addplot(sig, panel=macd_panel, color="red", width=1.0)
            )
            ...
            addplots.append(
                mpf.make_addplot(
                    histo, panel=macd_panel, type="bar", color=bar_colors, alpha=0.35
                )
            )
    ...
    if indicators.get("atr"):
        ...
        addplots.append(
            mpf.make_addplot(
                atr, panel=atr_panel, color="teal", width=1.0, ylabel="ATR"
            )
        )
...
def main():
    ...
    parser.add_argument(
        "cmd",
        choices=["price", "fundamentals", "history", "pro", "chart", "report"],
        nargs="?",
        default="price",
    )
    ...
    if len(args_list) > 0 and args_list[0] not in [
        "price",
        "fundamentals",
        "history",
        "pro",
    ...
    if cmd == "price":
        ...
        show_fundamentals(symbol, ticker, info)
    elif cmd == "history":
        show_history(symbol, ticker, period=period)
    elif cmd == "report":
        show_report(symbol, ticker, info, period=period)
    elif cmd == "pro":
        path = save_pro_chart(
            symbol, ticker, period=period, chart_type=chart_type, indicators=indicators
        )
        if path:
