# Extracted from: C:\DEV\PyAgent\.external\skills\skills\flyfoxci\twitter-search-skill\scripts\twitter_search.py
#!/usr/bin/env python3
"""
...
def fetch_tweets(
    api_key: str, query: str, query_type: str = "Top", cursor: str = ""
) -> Dict[str, Any]:
    """
    ...
    while len(all_tweets) < max_results:
        try:
            ...
            if page_count >= max_pages + 5:
                print(
                    f"Warning: Reached maximum page limit ({page_count} pages)",
                    file=sys.stderr,
                )
                break
            ...
            print(
                f"Warning: Failed to fetch page {page_count + 1}: {str(e)}",
                file=sys.stderr,
            )
            break
...
def extract_tweet_summary(tweet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "hashtags": [
            h.get("text") for h in tweet.get("entities", {}).get("hashtags", [])
        ],
        "mentions": [
            m.get("screen_name")
...
def main():
    ...
    parser.add_argument(
        "api_key", nargs="?", help="Twitter API key (or set TWITTER_API_KEY env var)"
    )
    parser.add_argument(
        "query", help="Search query (e.g., 'AI' OR 'ChatGPT' from:elonmusk)"
    )
    parser.add_argument(
        "--max-results",
        type=int,
    ...
    try:
        ...
        raw_tweets = fetch_all_tweets(
            api_key, args.query, args.max_results, args.query_type
        )
