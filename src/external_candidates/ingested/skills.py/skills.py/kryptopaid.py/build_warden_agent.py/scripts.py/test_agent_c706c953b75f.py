    ...
    def test_invoke(self, input_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.url}/invoke", json=payload, headers=self.headers, timeout=30
            )
...
def main():
    ...
    parser.add_argument(
        "--health-only", action="store_true", help="Only test health endpoint"
    )
    parser.add_argument(
        "--invoke-only", action="store_true", help="Only test invoke endpoint"
    )
    parser.add_argument(
        "--stream-only", action="store_true", help="Only test stream endpoint"
    )
