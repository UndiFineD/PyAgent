# Extracted from: C:\DEV\PyAgent\.external\skills\skills\digitaladaption\model-router\scripts\setup-wizard.py
#!/usr/bin/env python3
"""
...
def load_config() -> Dict:
    ...
    return {
        ...
        "providers": {},
        "task_mappings": {},
        "preferences": {"cost_optimization": "balanced", "default_provider": None},
    }
...
def prompt_provider_setup() -> Dict:
    ...
    for i, (key, info) in enumerate(providers.items(), 1):
        status = (
            f"{Colors.OKGREEN}✓ Configured{Colors.ENDC}"
            if key in config.get("providers", {})
            else "○ Not configured"
        )
        print(
            f"  {i}. {Colors.BOLD}{info['name']}{Colors.ENDC} - {info['description']}"
        )
    ...
    choice = input(
        f"{Colors.OKCYAN}Select a provider to configure (0-6): {Colors.ENDC}"
    ).strip()
    ...
    try:
        ...
        if 0 <= provider_idx < len(provider_keys):
            ...
            api_key = getpass.getpass(
                f"{Colors.OKCYAN}Enter API key (input hidden): {Colors.ENDC}"
            ).strip()
            ...
            if provider in ["openai", "moonshot", "glm"]:
                ...
                custom_url = input(
                    f"{Colors.OKCYAN}Base URL [default: {default_url}]: {Colors.ENDC}"
                ).strip()
            ...
            another = (
                input(
                    f"\n{Colors.OKCYAN}Configure another provider? (y/N): {Colors.ENDC}"
                )
                .strip()
                .lower()
            )
...
def prompt_preferences(config: Dict) -> Dict:
    ...
    cost_choice = input(
        f"\n{Colors.OKCYAN}Select cost optimization (1-3) [2]: {Colors.ENDC}"
    ).strip()
    cost_levels = {"1": "aggressive", "2": "balanced", "3": "quality"}
    config["preferences"]["cost_optimization"] = cost_levels.get(
        cost_choice, "balanced"
    )
    ...
    if providers:
        ...
        default_choice = input(
            f"\n{Colors.OKCYAN}Default provider [1]: {Colors.ENDC}"
        ).strip()
        try:
...
def main():
    ...
    print()
    print(
        f"{Colors.WARNING}Your API keys will be stored locally with restrictive permissions (600).{Colors.ENDC}"
    )
    print(
        f"{Colors.WARNING}Never commit your API keys to version control.{Colors.ENDC}"
    )
    ...
    another = (
        input(
            f"\n{Colors.OKCYAN}Configure task-to-model mappings? (Y/n): {Colors.ENDC}"
        )
        .strip()
        .lower()
    )
    ...
    # Step 3: Preferences
    another = (
        input(f"\n{Colors.OKCYAN}Configure preferences? (Y/n): {Colors.ENDC}")
        .strip()
        .lower()
    )
    if another != "n":
