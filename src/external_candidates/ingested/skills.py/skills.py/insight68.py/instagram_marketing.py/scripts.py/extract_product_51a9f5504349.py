...
class ProductExtractor:
    ...
    def _extract_amazon(self, soup: BeautifulSoup) -> Dict:
        ...
        data = {
            ...
            "features": self._get_bullet_points(
                soup, ["#feature-bullets ul", "#productDescription"]
            ),
    ...
    def _extract_shopify(self, soup: BeautifulSoup) -> Dict:
        ...
        data = {
            ...
            "url": self.url,
            "name": self._get_text(
                soup, ["h1.product-single__title", ".product_name", "h1"]
            ),
            "price": self._get_text(
                soup, [".product-single__price .price", ".current-price"]
            ),
            "description": self._get_text(
                soup, [".product-single__description", ".product-description"]
            ),
            "features": self._get_bullet_points(soup, [".product-description ul"]),
            "images": self._get_images(
                soup, [".product-single__photo img", ".product-featured-img"]
            ),
        }
        return self._clean_data(data)
    ...
    def _extract_generic(self, soup: BeautifulSoup) -> Dict:
        ...
        data = {
            ...
            "price": self._get_text(
                soup, [".price", ".product-price", ".product-price"]
            ),
            "description": self._get_text(
                soup, [".description", ".product-description"]
            ),
            "features": self._get_bullet_points(
                soup, ["ul.features", ".features-list"]
            ),
            "images": self._get_images(
                soup, [".product-image img", ".product-photo img"]
            ),
        }
        return self._clean_data(data)
