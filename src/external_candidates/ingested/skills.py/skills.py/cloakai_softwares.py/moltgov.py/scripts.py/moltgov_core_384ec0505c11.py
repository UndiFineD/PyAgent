# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cloakai-softwares\moltgov\scripts\moltgov_core.py
#!/usr/bin/env python3
"""
...
class MoltGovClient:
    ...
    def _moltbook_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        if not response.ok:
            raise MoltGovError(
                f"Moltbook API error: {response.status_code} - {response.text}"
            )
    ...
    def register(self, moltbook_key: str) -> Dict[str, str]:
        ...
        if agent_info.get("status") != "claimed":
            raise MoltGovError(
                "Moltbook account not verified. Complete Twitter/X verification first."
            )
...
"""
    ...
    def vouch(self, target_id: str, stake: int, reason: str) -> Vouch:
        """
        ...
        if status.citizen_class < CitizenClass.CITIZEN:
            raise MoltGovError(
                "MOLTGOV_CLASS_INSUFFICIENT: Must be Citizen class to vouch"
            )
    ...
    def create_proposal(
        ...
        if (
            ...
            and status.citizen_class < CitizenClass.SENATOR
        ):
            raise MoltGovError(
                "MOLTGOV_CLASS_INSUFFICIENT: Constitutional amendments require Senator class"
            )

        if (
            proposal_type == ProposalType.EMERGENCY
            and status.citizen_class < CitizenClass.CONSUL
        ):
            raise MoltGovError(
                "MOLTGOV_CLASS_INSUFFICIENT: Emergency proposals require Consul class"
            )
...
"""
        ...
        post_response = self._moltbook_request(
            "POST",
            "/posts",
            json={
                "submolt": "moltgov",
    ...
    def vote(self, proposal_id: str, choice: VoteChoice) -> Dict[str, Any]:
        """
    ...
    def delegate(self, delegate_to: str, scope: str = "all") -> Dict[str, Any]:
        """
    ...
    def create_faction(
        self, name: str, charter: str, founding_members: List[str]
    ) -> Faction:
        """
        ...
        try:
            self._moltbook_request(
                "POST",
                "/submolts",
                json={"name": submolt_name, "description": f"MoltGov Faction: {name}"},
            )
        except MoltGovError:
