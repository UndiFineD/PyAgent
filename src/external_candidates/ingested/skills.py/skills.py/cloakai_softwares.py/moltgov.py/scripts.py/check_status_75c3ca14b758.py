...
def main():
    ...
    try:
        ...
        print(
            f"  Class:           {CLASS_NAMES.get(status.citizen_class)} (Level {status.citizen_class})"
        )
        print(f"  Reputation:      {status.reputation:.1f}\n")
        print("  Activity Stats:")
        print(
            f"    Vouches Given/Received: {status.vouches_given}/{status.vouches_received}"
        )
        print(
            f"    Proposals Created/Passed: {status.proposals_created}/{status.proposals_passed}"
        )
