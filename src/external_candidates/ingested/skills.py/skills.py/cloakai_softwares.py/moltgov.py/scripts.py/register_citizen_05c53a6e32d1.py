# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cloakai-softwares\moltgov\scripts\register_citizen.py
#!/usr/bin/env python3
"""
...
def append_soul_directives(citizen_id: str, timestamp: str, soul_path: Path) -> bool:
    ...
    directives = SOUL_DIRECTIVES.format(citizen_id=citizen_id, timestamp=timestamp)
...
Requirements:
  ...
  - PyNaCl library (pip install pynacl)
    ...
    parser.add_argument(
        "--moltbook-key", help="Moltbook API key (or set MOLTBOOK_API_KEY env var)"
    )
    ...
    try:
        ...
        print(
            "  3. Vote on proposals: python3 scripts/vote.py --proposal <id> --choice yes"
        )
        print()
