...
class RoutingContext:
    ...
    summary: str = (
        "Items tagged for private/draft visibility route to a separate store."
    )
    ...
    private_store_path: Optional[str] = (
        None  # Resolved at init; None = default location
    )
    updated: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
    metadata: dict[str, Any] = field(default_factory=dict)
...
def matches_private_pattern(
    tags: dict[str, str], patterns: list[dict[str, str]]
) -> bool:
    """
