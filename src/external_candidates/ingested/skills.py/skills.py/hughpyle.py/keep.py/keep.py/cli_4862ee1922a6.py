# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hughpyle\keep\keep\cli.py
"""
...
LimitOption = Annotated[
    int, typer.Option("--limit", "-n", help="Maximum results to return")
]
...
def similar(
    ...
    include_self: Annotated[
        bool, typer.Option(help="Include the queried item")
    ] = False,
    output_json: JsonOption = False,
):
    """
...
def update(
    ...
    tags: Annotated[
        Optional[list[str]],
        typer.Option("--tag", "-t", help="Source tag as key=value (can be repeated)"),
    ] = None,
    lazy: Annotated[
        bool,
        typer.Option(
    ...
    if tags:
        for tag in tags:
            if "=" not in tag:
                typer.echo(
                    f"Error: Invalid tag format '{tag}'. Use key=value", err=True
                )
                raise typer.Exit(1)
            k, v = tag.split("=", 1)
            source_tags[k] = v
...
def remember(
    ...
    id: Annotated[
        Optional[str],
        typer.Option("--id", "-i", help="Custom identifier (default: auto-generated)"),
    ] = None,
    tags: Annotated[
        Optional[list[str]],
        typer.Option("--tag", "-t", help="Source tag as key=value (can be repeated)"),
...
def init(
    ...
    actual_path = (
        kp._store_path if hasattr(kp, "_store_path") else Path(store or ".keep")
    )
...
def process_pending(
    ...
    limit: Annotated[
        int,
        typer.Option("--limit", "-n", help="Maximum items to process in this batch"),
    ] = 10,
    all_items: Annotated[
        bool,
        typer.Option("--all", "-a", help="Process all pending items (ignores --limit)"),
    ] = False,
    daemon: Annotated[
        bool,
        typer.Option(
            "--daemon", hidden=True, help="Run as background daemon (used internally)"
        ),
    ] = False,
    output_json: JsonOption = False,
):
    ...
    if all_items:
        ...
        if output_json:
            typer.echo(
                json.dumps({"processed": total_processed, "remaining": remaining})
            )
        else:
        ...
        if output_json:
            typer.echo(json.dumps({"processed": processed, "remaining": remaining}))
        else:
