...
class TestEndToEnd:
    ...
    def test_find_similar(self, memory: Keeper) -> None:
        ...
        assert any(
            "flower" in r.summary.lower() or "bloom" in r.summary.lower()
            for r in similar
        )
...
class TestRecencyDecay:
    ...
    def test_decay_formula_correctness(self, memory: Keeper) -> None:
        ...
        old_item = Item(
            id="old", summary="old item", tags={"_updated": old_timestamp}, score=1.0
        )

        # Item from today should have score × 1.0
        new_timestamp = now.isoformat()
        new_item = Item(
            id="new", summary="new item", tags={"_updated": new_timestamp}, score=1.0
        )
