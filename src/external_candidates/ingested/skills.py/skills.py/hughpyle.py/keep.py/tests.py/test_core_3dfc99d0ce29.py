# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hughpyle\keep\tests\test_core.py
"""
...
class TestDataFiles:
    ...
    def test_all_data_files_exist(self, data_dir: Path):
        ...
        assert (
            len(data_files) >= 4
        ), f"Expected at least 4 data files, found {len(data_files)}"
    ...
    def test_all_text_files_readable(self, data_dir: Path):
        ...
        for txt_file in data_dir.glob("*.txt"):
            ...
            assert (
                len(content) > 100
            ), f"{txt_file.name} should have substantial content"
    ...
    def test_all_json_files_valid(self, data_dir: Path):
        ...
        for json_file in data_dir.glob("*.json"):
            ...
            assert isinstance(
                data, (dict, list)
            ), f"{json_file.name} should parse as dict or list"
...
class TestRoutingPatterns:
    ...
    def test_matches_private_pattern(self):
        ...
        assert matches_private_pattern(
            {"_visibility": "private", "topic": "auth"}, patterns
        )
    ...
    def test_pattern_requires_all_keys(self):
        ...
        # Both must match
        assert matches_private_pattern(
            {"_visibility": "draft", "_for": "self"}, patterns
        )
        assert matches_private_pattern(
            {"_visibility": "draft", "_for": "self", "extra": "ok"}, patterns
        )
