...
def display_search_results(result: dict) -> None:
    ...
    for i, tool in enumerate(tools, 1):
        ...
        if params:
            ...
            if optional:
                print(
                    f"    Optional: {', '.join(optional[:5])}{'...' if len(optional) > 5 else ''}"
                )
...
Examples:
  ...
  %(prog)s execute openweathermap_current_weather --search-id abc123 --params '{"city": "London"}'
    ...
    exec_parser.add_argument(
        "--search-id", required=True, help="Search ID from previous search"
    )
    exec_parser.add_argument(
        "--params", default="{}", help="Tool parameters as JSON string"
    )
    exec_parser.add_argument(
        "--max-size", type=int, default=20480, help="Max response size"
    )
