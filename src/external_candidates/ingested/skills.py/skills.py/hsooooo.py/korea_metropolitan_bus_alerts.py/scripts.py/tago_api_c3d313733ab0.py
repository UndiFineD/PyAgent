# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hsooooo\korea-metropolitan-bus-alerts\scripts\tago_api.py
#!/usr/bin/env python3
...
def _http_get_json(url: str, timeout_sec: int = 10) -> Dict[str, Any]:
    req = urllib.request.Request(
        url, headers={"User-Agent": "korea-metropolitan-bus-alerts/0.1"}
    )
...
def _build_url(base: str, path: str, params: Dict[str, Any]) -> str:
    ...
    for k, v in q.items():
        ...
        if isinstance(v, (list, tuple)):
            for vv in v:
                parts.append(
                    f"{urllib.parse.quote(str(k), safe='')}={urllib.parse.quote(str(vv), safe='')}"
                )
        else:
            parts.append(
...
def stops_nearby(
    gps_lat: float, gps_long: float, *, num_rows: int = 30
) -> List[StopCandidate]:
...
def arrivals_for_stop(
    city_code: str, node_id: str, *, num_rows: int = 200
) -> List[Arrival]:
    ...
    for it in items:
        ...
        out.append(
            Arrival(
                ...
                route_id=(
                    str(it.get("routeid")).strip()
                    if it.get("routeid") is not None
                    else None
                ),
                vehicle_type=(
                    str(it.get("vehicletp")).strip()
                    if it.get("vehicletp") is not None
                    else None
                ),
            )
        )
