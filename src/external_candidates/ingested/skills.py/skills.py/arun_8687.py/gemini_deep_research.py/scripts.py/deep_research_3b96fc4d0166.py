...
def create_interaction(api_key, query, output_format=None, file_search_store=None):
    ...
    if file_search_store:
        payload["tools"] = [
            {"type": "file_search", "file_search_store_names": [file_search_store]}
        ]
...
def poll_interaction(api_key, interaction_id, stream=False):
    ...
    while True:
        response = requests.get(
            f"{API_BASE}/interactions/{interaction_id}", headers=headers
        )
...
def main():
    ...
    parser.add_argument(
        "--stream", action="store_true", help="Show streaming progress updates"
    )
    parser.add_argument(
        "--output-dir", default=".", help="Output directory for results"
    )
    parser.add_argument(
        "--api-key", help="Gemini API key (overrides GEMINI_API_KEY env var)"
    )
