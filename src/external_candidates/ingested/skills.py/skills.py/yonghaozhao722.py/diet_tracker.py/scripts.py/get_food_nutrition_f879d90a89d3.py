# Extracted from: C:\DEV\PyAgent\.external\skills\skills\yonghaozhao722\diet-tracker\scripts\get_food_nutrition.py
import json
import re
...
def calculate_tdee(height_cm, weight_kg, age, gender, activity_level):
    print(
        f"calculate_tdee input: height_cm={height_cm}, weight_kg={weight_kg}, age={age}, gender={gender}, activity_level={activity_level}"
    )
    if gender == "male":
...
def get_user_info():
    try:
        ...
        gender = (
            "male"
            if gender_match and "男" in gender_match.group(1)
            else "female" if gender_match and "女" in gender_match.group(1) else None
        )
        activity_level = (
            activity_level_match.group(1).lower().replace(" ", "")
        ...
        if all([height_cm, weight_kg, age, gender, activity_level]):
            print(
                f"get_user_info: height_cm={height_cm}, weight_kg={weight_kg}, age={age}, gender={gender}, activity_level={activity_level}"
            )
...
def get_nutrition(food_item):
    ...
    try:
        with open(
            "/home/zhaoyh/clawd/skills/diet-tracker/references/food_database.json", "r"
        ) as f:
            db = json.load(f)
