...
def search_comics(query: str, limit: int = 5) -> list[dict]:
    ...
    def check_comic(num: int) -> dict | None:
        ...
        try:
            ...
            if (
                query_lower in comic["title"].lower()
                or query_lower in comic["alt"].lower()
            ):
                return comic
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument(
        "number", nargs="?", type=int, help="Comic number (omit for latest)"
    )
    parser.add_argument(
        "--random", "-r", action="store_true", help="Fetch random comic"
    )
