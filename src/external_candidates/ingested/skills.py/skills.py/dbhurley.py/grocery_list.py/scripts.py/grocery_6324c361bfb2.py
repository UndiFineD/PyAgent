# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\grocery-list\scripts\grocery.py
#!/usr/bin/env python3
# /// script
...
DATA_DIR = Path(
    os.environ.get("GROCERY_DATA_DIR", Path.home() / ".clawdbot" / "grocery-list")
)
...
def cmd_list_view(args: argparse.Namespace) -> int:
    ...
    if list_name not in data.lists:
        print(
            f"❌ List '{list_name}' not found. Available: {', '.join(data.lists)}",
            file=sys.stderr,
        )
        return 1
    ...
    if args.json:
        print(
            json.dumps(
                {"list": list_name, "items": [i.to_dict() for i in items]}, indent=2
            )
        )
        return 0
...
def cmd_remove(args: argparse.Namespace) -> int:
    ...
    data.items = [
        i
        for i in data.items
        if not (i.list_name == args.list_name and i.name.lower() == args.item.lower())
    ]
...
def cmd_clear(args: argparse.Namespace) -> int:
    ...
    before = len(data.items)
    data.items = [
        i for i in data.items if not (i.list_name == args.list_name and i.checked)
    ]
...
def cmd_recipe_add(args: argparse.Namespace) -> int:
    ...
    # Check if exists
    existing = next(
        (r for r in data.recipes if r.name.lower() == args.name.lower()), None
    )
    if existing:
        print(
            f"⚠️  Recipe '{args.name}' already exists. Use a different name or delete first.",
            file=sys.stderr,
        )
        return 1
...
def cmd_recipe_search(args: argparse.Namespace) -> int:
    ...
    matches = [
        ...
        if query in r.name.lower() or query in " ".join(r.ingredients).lower()
    ]
...
def cmd_meals(args: argparse.Namespace) -> int:
    ...
    meals.sort(
        key=lambda m: (
            m.date,
            (
                ["breakfast", "lunch", "dinner", "snack"].index(m.type)
                if m.type in ["breakfast", "lunch", "dinner", "snack"]
                else 4
            ),
        )
    )
    ...
    for m in meals:
        if m.date != current_date:
            ...
            print(f"\n  {day_name}:")
        print(
            f"    {m.type.capitalize()}: {m.recipe_name or '(unplanned)'}{' - ' + m.notes if m.notes else ''}"
        )
    return 0
...
def cmd_meal_add(args: argparse.Namespace) -> int:
    ...
    if meal_type not in ["breakfast", "lunch", "dinner", "snack"]:
        print(
            f"❌ Invalid meal type '{meal_type}'. Use: breakfast, lunch, dinner, snack",
            file=sys.stderr,
        )
        return 1
    ...
    # Check for existing
    existing = next(
        (m for m in data.meals if m.date == date and m.type == meal_type), None
    )
    if existing:
...
def cmd_meal_add_to_list(args: argparse.Namespace) -> int:
    ...
    for meal in meals_today:
        recipe = next(
            (r for r in data.recipes if r.name.lower() == meal.recipe_name.lower()),
            None,
        )
        if not recipe:
...
def cmd_notify(args: argparse.Namespace) -> int:
    ...
    for i in range(7):
        ...
        dinner = next(
            (m for m in data.meals if m.date == check_date and m.type == "dinner"), None
        )
...
def main() -> int:
    ...
    p_meal_add.add_argument(
        "--type", "-t", required=True, help="breakfast/lunch/dinner/snack"
    )
    ...
    # meal-add-to-list
    p_meal_list = subparsers.add_parser(
        "meal-add-to-list", help="Add meal ingredients to grocery list"
    )
