# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\process-watch\scripts\process-watch.py
#!/usr/bin/env python3
# /// script
...
def list_procs(
    sort: str = typer.Option(
        "cpu", "--sort", "-s", help="Sort by: cpu, mem, disk, name, pid"
    ),
...
def top(
    type_: str = typer.Option(
        "cpu", "--type", "-t", help="Resource type: cpu, mem, disk"
    ),
...
def ports(
    port: Optional[int] = typer.Option(
        None, "--port", "-p", help="Filter by specific port"
    ),
    ...
    for c in connections[:50]:
        status_style = (
            "green"
            if c["status"] == "LISTEN"
            else "yellow" if c["status"] == "ESTABLISHED" else ""
        )
        table.add_row(
            str(c["port"]),
            c["ip"],
...
def kill(
    ...
    name: Optional[str] = typer.Option(
        None, "--name", "-n", help="Kill processes by name"
    ),
        ...
        for proc in psutil.process_iter(["pid", "name"]):
            try:
                if name.lower() in proc.info["name"].lower():
                    proc.send_signal(sig)
                    console.print(
                        f"[green]✓ Sent {sig_name} to {proc.info['pid']} ({proc.info['name']})[/green]"
                    )
                    killed += 1
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                pass
...
def watch(
    interval: int = typer.Option(
        2, "--interval", "-i", help="Update interval in seconds"
    ),
    ...
    console.print(
        f"[dim]Watching... (Ctrl+C to stop, alerts at CPU>{alert_cpu}%, Mem>{alert_mem}%)[/dim]\n"
    )
    ...
    try:
        while True:
            ...
            cpu_status = (
                "[red]⚠️ HIGH[/red]" if cpu > alert_cpu else "[green]OK[/green]"
            )
            mem_status = (
                "[red]⚠️ HIGH[/red]" if mem.percent > alert_mem else "[green]OK[/green]"
            )
            ...
            for p in procs[:5]:
                ...
                console.print(
                    f"  {p['pid']:>6}  {p['name']:<25} [{cpu_style}]{p['cpu_percent']:.1f}%[/]"
                )
