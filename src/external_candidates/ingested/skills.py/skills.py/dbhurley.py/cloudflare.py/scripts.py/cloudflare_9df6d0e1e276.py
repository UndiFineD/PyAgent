...
class CloudflareClient:
    ...
    def update_dns_record(
        self, domain: str, record_id: str, content: str = None, proxied: bool = None
    ) -> dict:
        """Update a DNS record"""
        zone_id = self.get_zone_id(domain)
        if not zone_id:
...
def main():
    ...
    dns_add.add_argument(
        "--type", required=True, help="Record type (A, AAAA, CNAME, TXT, MX)"
    )
    ...
    dns_add.add_argument(
        "--proxied", action="store_true", help="Enable Cloudflare proxy"
    )
        ...
        if args.command == "verify":
            ...
            if args.dns_action == "list":
                ...
                if args.json:
                    ...
                    for r in records:
                        ...
                        print(
                            f"{proxy} {r.get('type'):6} {r.get('name'):30} → {r.get('content')}"
                        )
                        print(f"   ID: {r.get('id')}")
                ...
                print(
                    f"✅ Created {args.type} record: {record.get('name')} → {record.get('content')}"
                )
                ...
                record = client.update_dns_record(
                    args.domain, args.record_id, content=args.content, proxied=proxied
                )
                ...
                records = client.list_dns_records(args.domain)
                record = next(
                    (r for r in records if r.get("id") == args.record_id), None
                )
            ...
            if args.cache_action == "purge":
                ...
                result = client.purge_cache(
                    args.domain, purge_all=args.all, urls=urls, prefixes=prefixes
                )
                if args.all:
