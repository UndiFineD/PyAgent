# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\otter\scripts\otter.py
#!/usr/bin/env python3
"""
...
class OtterClient:
    ...
    def search(
        self, query: str, speech_id: Optional[str] = None, size: int = 100
    ) -> list:
        """Search transcripts"""
        if not self._logged_in:
            raise RuntimeError("Not logged in")
    ...
    def upload(self, filepath: str, content_type: str = "audio/mp4") -> dict:
        ...
        fields["success_action_status"] = str(
            fields.get("success_action_status", "201")
        )
        ...
        return (
            resp.json()
            if resp.status_code == 200
            else {"error": "Finish upload failed"}
        )
...
def main():
    ...
    sum_p = subparsers.add_parser(
        "summary", help="Get transcript text for summarization"
    )
    ...
    if not email or not password:
        print(
            "Error: OTTER_EMAIL and OTTER_PASSWORD environment variables required",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.command == "list":
        ...
        if not twenty_url or not twenty_token:
            print(
                "Error: TWENTY_API_URL and TWENTY_API_TOKEN required for CRM sync",
                file=sys.stderr,
            )
            sys.exit(1)
...
"""
        ...
        note_data = {
            ...
            "bodyV2": {"blocknote": "", "markdown": markdown},
        }

        resp = requests.post(
            f"{twenty_url}/rest/notes", headers=headers, json=note_data
        )

        if resp.status_code >= 400:
            print(
                f"Error creating note: {resp.status_code} {resp.text}", file=sys.stderr
            )
            sys.exit(1)
        ...
        if args.company and note_id:
            ...
            eng_resp = requests.get(f"{twenty_url}/rest/engagements", headers=headers)
