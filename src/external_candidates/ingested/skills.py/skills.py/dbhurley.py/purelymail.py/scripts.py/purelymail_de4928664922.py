# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\purelymail\scripts\purelymail.py
#!/usr/bin/env python3
# /// script
...
def test(
    ...
    try:
        ...
        with imaplib.IMAP4_SSL(IMAP_SERVER, IMAP_PORT, ssl_context=context) as imap:
            ...
            console.print(
                f"[green]✓ IMAP connected - {msg_count} messages in inbox[/green]"
            )
...
def send_test(
    ...
    subject: str = typer.Option(
        "Test from Clawdbot Agent", "--subject", "-s", help="Email subject"
    ),
):
    """Send a test email."""
...
def inbox(
    ...
    unread: bool = typer.Option(
        False, "--unread", "-u", help="Show only unread messages"
    ),
):
...
def wizard():
    ...
    has_account = typer.confirm(
        "Do you already have a PurelyMail account?", default=True
    )
...
""")
    ...
    if not (imap_ok and smtp_ok):
        ...
        console.print(
            "[dim]Common issues: wrong password, 2FA enabled (use app password), account not activated[/dim]"
        )
        raise typer.Exit(1)
