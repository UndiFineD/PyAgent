# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\vikunja\scripts\vikunja.py
#!/usr/bin/env python3
# /// script
...
class VikunjaClient:
    ...
    def get_tasks(self, project_id: Optional[int] = None):
        ...
        if project_id:
            ...
            resp = requests.get(
                f"{self.base_url}/api/v1/tasks/all", headers=self._headers()
            )
        return resp.json()
...
def cmd_create_project(args):
    ...
    console.print(
        f"[green]Created project:[/green] {project['title']} (ID: {project['id']})"
    )
...
def cmd_create_task(args):
    ...
    task = client.create_task(
        args.project, args.title, args.description or "", args.due, args.priority or 0
    )
