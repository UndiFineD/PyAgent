...
def get_flightaware_status(flight_number: str, date: str = None) -> dict:
    ...
    try:
        ...
        if dest:
            data["destination"] = dest.get("data-destination") or dest.get_text(
                strip=True
            )
...
def search_flights_flightaware(
    origin: str, dest: str, date: str = None, airline: str = None
) -> list:
    ...
    try:
        ...
        match = re.search(
            r"FA\.findflight\.resultsContent\s*=\s*(\[.*?\]);", resp.text, re.DOTALL
        )
        if match:
            try:
                ...
                for flight in data:
                    ...
                    if airline:
                        ...
                        if not flight_num.upper().startswith(airline_upper):
                            ...
                            if (
                                airline_upper == "MX"
                                and not flight_num.upper().startswith("MXY")
                            ):
                                continue
                    ...
                    # Clean times
                    dep_time = re.sub(
                        r"<[^>]+>", " ", flight.get("flightDepartureTime", "")
                    ).strip()
                    arr_time = re.sub(
                        r"<[^>]+>", " ", flight.get("flightArrivalTime", "")
                    ).strip()
                    dep_day = re.sub(
                        r"<[^>]+>", "", flight.get("flightDepartureDay", "")
                    ).strip()
...
def cmd_search(args):
    """Search for flights."""
    results = search_flights_flightaware(
        args.origin, args.dest, args.date, args.airline
    )
