# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\digital-ocean\scripts\do.py
#!/usr/bin/env python3
# /// script
...
def cmd_droplet(droplet_id: str):
    ...
    console.print(
        f"[bold]Size:[/bold] {d.get('size_slug')} ({d.get('memory')}MB RAM, {d.get('vcpus')} vCPUs)"
    )
...
def cmd_action(droplet_id: str, action: str):
    ...
    act = data.get("action", {})
    console.print(
        f"[green]✓[/green] Action '{action}' initiated (ID: {act.get('id')}, Status: {act.get('status')})"
    )
