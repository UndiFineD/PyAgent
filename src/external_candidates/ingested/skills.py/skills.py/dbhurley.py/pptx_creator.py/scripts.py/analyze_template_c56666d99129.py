# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\pptx-creator\scripts\analyze_template.py
#!/usr/bin/env python3
# /// script
...
def print_summary(analysis: dict):
    ...
    print(
        f"   Dimensions: {analysis['dimensions']['width_inches']}\" x {analysis['dimensions']['height_inches']}\""
    )
    ...
    for slide in analysis["slides"][:10]:
        ...
        for shape in slide["shapes"][:5]:
            text_preview = (
                f" → \"{shape.get('text', '')[:40]}\"" if shape.get("text") else ""
            )
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Analyze all slides"
    )
