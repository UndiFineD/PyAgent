# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\pptx-creator\scripts\create_template.py
#!/usr/bin/env python3
# /// script
...
def create_template(preset_name: str, output_path: Path) -> None:
    ...
    shape = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(0), Inches(6.8), Inches(13.333), Inches(0.2)
    )
    shape.fill.solid()
    ...
    # 2. Section Header
    slide_layout = (
        prs.slide_layouts[2] if len(prs.slide_layouts) > 2 else prs.slide_layouts[0]
    )
    slide = prs.slides.add_slide(slide_layout)
    ...
    for shape in slide.placeholders:
        if shape.placeholder_format.idx == 1:
            ...
            bullets = [
                "First key point",
                "Second key point",
                "Third key point with more detail",
                "Final point",
            ]
    ...
    # 4. Two Column Layout
    slide_layout = (
        prs.slide_layouts[3] if len(prs.slide_layouts) > 3 else prs.slide_layouts[1]
    )
    slide = prs.slides.add_slide(slide_layout)
    ...
    img_placeholder = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(1), Inches(1.8), Inches(7), Inches(5)
    )
    ...
    caption_box = slide.shapes.add_textbox(Inches(8.5), Inches(2), Inches(4), Inches(4))
    ...
    slide = prs.slides.add_slide(
        prs.slide_layouts[6] if len(prs.slide_layouts) > 6 else prs.slide_layouts[1]
    )

    # Add large quote
    quote_box = slide.shapes.add_textbox(Inches(1.5), Inches(2), Inches(10), Inches(3))
...
def main():
    ...
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available presets"
    )
    parser.add_argument(
        "--output", "-o", help="Output path (default: templates/<preset>.pptx)"
    )
    ...
    if args.preset:
        ...
        TEMPLATES_DIR.mkdir(exist_ok=True)
        output_path = (
            Path(args.output) if args.output else TEMPLATES_DIR / f"{args.preset}.pptx"
        )
