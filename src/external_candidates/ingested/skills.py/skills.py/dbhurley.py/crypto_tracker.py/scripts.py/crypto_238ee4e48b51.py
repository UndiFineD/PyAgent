# Extracted from: C:\DEV\PyAgent\.external\skills\skills\dbhurley\crypto-tracker\scripts\crypto.py
# /// script
...
def search(query: str, limit: int):
    ...
    try:
        resp = httpx.get(
            f"{COINGECKO_BASE}/search", params={"query": query}, timeout=10
        )
        resp.raise_for_status()
        data = resp.json()
        ...
        for coin in coins:
            click.echo(
                f"  {coin['symbol'].upper():8} → {coin['id']:30} ({coin['name']})"
            )
...
@click.argument("coin")
@click.argument(
    "alert_type", type=click.Choice(["above", "below", "change", "drop", "rise"])
)
@click.argument("threshold", type=float)
@click.option(
    "--cooldown", "-c", default=1, help="Hours between repeat alerts (default: 1)"
)
...
@click.option(
    "--json-output", "-j", is_flag=True, help="Output triggered alerts as JSON"
)
def check_alerts(json_output: bool):
    ...
    for alert in alerts_list:
        ...
        if alert.get("last_triggered"):
            last = datetime.fromisoformat(
                alert["last_triggered"].replace("Z", "+00:00")
            )
    ...
    for t in triggered:
        ...
        click.echo(
            f"  Coin: {t['coin'].upper()} @ ${t['current_price']:,.2f} ({t['change_24h']:+.2f}%)"
        )
