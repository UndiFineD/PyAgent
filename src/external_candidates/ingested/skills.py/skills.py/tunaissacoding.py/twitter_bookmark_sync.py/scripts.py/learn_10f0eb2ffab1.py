...
def categorize_bookmark(tweet):
    ...
    """
    text = (
        tweet.get("text", "") + " " + tweet.get("user", {}).get("description", "")
    ).lower()
    ...
    if any(
        kw in text for kw in ["investment", "portfolio", "fund", "trading", "market"]
    ):
    ...
    if any(
        kw in text for kw in ["growth", "scale", "gtm", "distribution", "acquisition"]
    ):
    ...
    if any(
        kw in text for kw in ["psychology", "behavior", "mindset", "mental", "therapy"]
    ):
        categories.append(("psychology", "personal_growth", 0.7))

    # Productivity/Systems
    if any(
        kw in text for kw in ["productivity", "system", "habit", "focus", "deep work"]
    ):
    ...
    if any(
        kw in text
        for kw in ["backend", "architecture", "code", "engineering", "system design"]
    ):
...
def update_criteria_from_bookmarks(criteria, bookmarks):
    ...
    for tweet in bookmarks:
        ...
        for category_name, value_type, confidence in categories:
            ...
            category_updates[category_name]["engagement"] += tweet.get(
                "favoriteCount", 0
            )
    ...
    for category_name, update_data in category_updates.items():
        if category_name in criteria["value_categories"]:
            ...
            boost = (
                update_data["count"]
                * (update_data["total_confidence"] / update_data["count"])
                * 5
            )
...
def main():
    ...
    # Show top categories
    sorted_cats = sorted(
        criteria["value_categories"].items(), key=lambda x: x[1]["weight"], reverse=True
    )[:5]
