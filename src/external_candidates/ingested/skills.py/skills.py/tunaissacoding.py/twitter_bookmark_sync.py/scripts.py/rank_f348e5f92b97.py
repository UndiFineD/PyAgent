# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tunaissacoding\twitter-bookmark-sync\scripts\rank.py
#!/usr/bin/env python3
import json
import sys
...
def score_tweet(tweet, criteria):
    ...
    text = (
        tweet.get("text", "") + " " + tweet.get("user", {}).get("screenName", "")
    ).lower()
    score = 50.0  # base
...
def main():
    if len(sys.argv) < 5:
        print(
            "Usage: rank.py <bookmarks.json> <config.json> <criteria.json> <output_dir>"
        )
        sys.exit(1)
    ...
    for i, tweet in enumerate(ranked, 1):
        ...
        tweet["url"] = (
            f"https://x.com/{tweet['author']}/status/{tweet.get('tweetId', '')}"
        )
    ...
    top5_file = (
        Path(output_dir) / f"twitter-top5-{datetime.now().strftime('%Y-%m-%d')}.json"
    )
