...
def format_table(data, columns):
    ...
    for col in columns:
        ...
        for row in data:
            val = col.get("format", lambda x, r: str(x if x is not None else ""))(
                row.get(col["key"]), row
            )
    ...
    for row in data:
        line = " | ".join(
            str(
                col.get("format", lambda x, r: str(x if x is not None else ""))(
                    row.get(col["key"]), row
                )
            ).ljust(widths[col["key"]])
            for col in columns
        )
        print(line)
...
def cmd_unique_domain_backlinks(args):
    ...
    if not args.domain:
        print(
            "Usage: kwe unique-domain-backlinks example.com [--num 10]", file=sys.stderr
        )
        sys.exit(1)
...
""",
    ...
    p = subparsers.add_parser(
        "keywords", help="Get volume, CPC, competition for keywords"
    )
    ...
    p = subparsers.add_parser(
        "unique-domain-backlinks", help="Unique referring domains"
    )
    p.add_argument("domain", help="Domain to analyze")
    p.add_argument("--num", type=int, default=10, help="Number of results")
    p.add_argument("--output", choices=["table", "json", "csv"], default="table")
