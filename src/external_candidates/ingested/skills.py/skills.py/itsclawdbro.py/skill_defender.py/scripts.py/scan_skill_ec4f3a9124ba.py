...
CREDENTIAL_EXFIL_PATTERNS = [
    ...
    (
        re.compile(
            r"os\.environ\.get\s*\(.*(?:KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL)", re.I
        ),
        "high",
        "credential_theft",
        "likely",
...
OBFUSCATION_PATTERNS = [
    ...
    (
        re.compile(
            r"(?<!sha512-)(?<!sha256-)(?<!sha384-)[A-Za-z0-9+/]{50,}={0,2}(?!\.tgz)"
        ),
        "high",
        "obfuscation",
        "possible",
...
class SkillScanner:
    ...
    def scan(self) -> dict[str, Any]:
        ...
        for fpath in sorted(self.skill_path.rglob("*")):
            ...
            # Skip non-text files
            if ext not in TEXT_EXTENSIONS and not fpath.name.upper().startswith(
                "SKILL"
            ):
        ...
        script_count = sum(
            ...
            and not any(
                part in skip_dirs for part in f.relative_to(self.skill_path).parts
            )
        )
        if script_count > 10:
            self._add_finding(
...
def format_human(result: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(
        f"VERDICT: {VERDICT_DISPLAY.get(result['verdict'], result['verdict'])}"
    )
    ...
    clean_files = summary.get("files_scanned", 0) - len(
        set(f["file"] for f in findings)
    )
