# Extracted from: C:\DEV\PyAgent\.external\skills\skills\logesh2496\moltspaces\bot.py
#
...
MOLTSPACES_API_URL = os.getenv(
    "MOLTSPACES_API_URL", "https://moltspaces-api-547962548252.us-central1.run.app"
)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status in [200, 201]:
                    data = await response.json()
                    logger.info(
                        f"✅ Room created: {data.get('room_name') or data.get('roomName')}"
                    )
                    return {
                        "room_url": data.get("room_url") or data.get("roomUrl"),
                        "token": data.get("token"),
...
- Keep the energy positive and inclusive""",
    ...
    async def on_participant_joined(transport, participant):
        ...
        participant_name = (
            participant_info.get("userName") or participant_info.get("name") or "Guest"
        )
        logger.info(f"Participant joined: {participant_name}")
        await transport.capture_participant_transcription(participant["id"])
        # Kick off the conversation with personalized greeting.
        messages.append(
            {"role": "system", "content": f"Greet {participant_name} by name."}
        )
...
async def main(
    ...
    # Topic-based discovery and creation
        ...
        if rooms:
            ...
            chosen_room = rooms[0]
            room_name = (
                chosen_room.get("room_name")
                or chosen_room.get("roomName")
                or chosen_room.get("name")
            )
...
Examples:
  ...
  uv run bot.py --url <daily_room_url> --token <token>
    ...
    parser.add_argument(
        "--topic", type=str, help="Topic to search for or create room with"
    )
    parser.add_argument("--room", type=str, help="Specific room name to join")
    parser.add_argument(
        "-u", "--url", type=str, help="Full Daily room URL (for direct connection)"
    )
    parser.add_argument(
        "-t", "--token", type=str, help="Daily room token (required with --url)"
    )
