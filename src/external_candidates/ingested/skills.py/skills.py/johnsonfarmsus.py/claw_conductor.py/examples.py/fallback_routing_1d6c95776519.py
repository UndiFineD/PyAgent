# Extracted from: C:\DEV\PyAgent\.external\skills\skills\johnsonfarmsus\claw-conductor\examples\fallback-routing.py
#!/usr/bin/env python3
"""
...
def simulate_execution(agent_id: str, task: Task, fail_pattern: dict = None):
    ...
    if agent_id in fail_pattern and attempt_num in fail_pattern[agent_id]:
        return (
            False,
            f"Model '{agent_id}' failed on attempt #{attempt_num} (simulated failure)",
        )
    else:
        return True, None
...
def main():
    ...
    primary_agent, primary_details = router.route_task_with_fallback(
        task, execution_callback=None
    )
    print("Routing Plan:")
    print(
        f"  Primary: {primary_details.get('model_id')} (score: {primary_details.get('total_score')})"
    )
    ...
    agent_id, details = router.route_task_with_fallback(
        task, execution_callback=scenario1_callback
    )
    ...
    if agent_id:
        ...
        for attempt in exec_info.get("attempts", []):
            ...
            print(
                f"  [{status}] Attempt {attempt['attempt']}: {attempt['model_id']} ({attempt['type']})"
            )
    ...
    agent_id, details = router.route_task_with_fallback(
        task, execution_callback=scenario2_callback
    )
    ...
    agent_id, details = router.route_task_with_fallback(
        task, execution_callback=scenario3_callback
    )
    ...
    def scenario4_callback(agent_id, task_obj):
        ...
        fail_pattern = {primary_id: [1, 2], fallback_id: [1, 2] if fallback_id else []}
        return simulate_execution(agent_id, task_obj, fail_pattern=fail_pattern)

    agent_id, details = router.route_task_with_fallback(
        task, execution_callback=scenario4_callback
    )
