# Extracted from: C:\DEV\PyAgent\.external\skills\skills\johnsonfarmsus\claw-conductor\examples\complex-feature.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    print()
    print(
        "Request: 'Build a complete e-commerce checkout flow with payment processing'"
    )
    print()
    ...
    for agent, info in agent_tasks.items():
        ...
        print(
            f"    • Complexity range: {min(info['complexities'])}-{max(info['complexities'])}"
        )
    ...
    for result in results:
        complexity_info = (
            result.get("details", {}).get("breakdown", {}).get("complexity", {})
        )
        ...
        match_type = (
            "Perfect"
            if gap == 0
            else "Slight over-qualification" if gap == 1 else "Over-qualified"
        )

        print(
            f"  • Subtask #{result['subtask_id']} (complexity={task_complexity}): {match_type}"
        )
