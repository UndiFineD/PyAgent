# Extracted from: C:\DEV\PyAgent\.external\skills\skills\johnsonfarmsus\claw-conductor\scripts\update-capability.py
#!/usr/bin/env python3
"""
...
Examples:
  ...
  %(prog)s --list-categories
    ...
    parser.add_argument(
        "--registry",
        default="config/agent-registry.json",
        help="Path to agent registry",
    )
    parser.add_argument("--agent", "--model", help="Agent ID to update")
    parser.add_argument("--category", help="Task category to update")
    parser.add_argument("--rating", type=int, help="Rating (1-5 stars)")
    parser.add_argument("--max-complexity", type=int, help="Maximum complexity (1-5)")
    parser.add_argument("--notes", help="Notes about this capability")
    parser.add_argument(
        "--experience", type=int, help="Experience count (number of tasks)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--show", action="store_true", help="Show capabilities for an agent"
    )
    parser.add_argument(
