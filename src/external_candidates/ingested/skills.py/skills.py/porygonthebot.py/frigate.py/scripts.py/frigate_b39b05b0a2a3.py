# Extracted from: C:\DEV\PyAgent\.external\skills\skills\porygonthebot\frigate\scripts\frigate.py
#!/usr/bin/env python3
"""
...
def get_snapshot(session, camera_name, output_path=None):
    ...
    response = session.get(f"{FRIGATE_URL}/api/{camera_name}/latest.jpg", verify=False)
    if response.status_code != 200:
...
def get_events(session, camera_name=None, limit=10):
    ...
    response = session.get(f"{FRIGATE_URL}/api/events?{params}", verify=False)
