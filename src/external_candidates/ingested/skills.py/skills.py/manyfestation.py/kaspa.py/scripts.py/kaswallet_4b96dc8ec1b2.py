...
def payment_uri(
    address: str, amount: Optional[str], message: Optional[str], label: Optional[str]
) -> str:
    params = {}
...
async def rpc_connect(client: Any, connect_timeout_ms: int) -> None:
    if hasattr(client, "connect"):
        await asyncio.wait_for(
            maybe_await(client.connect()), timeout=connect_timeout_ms / 1000.0
        )
...
async def cmd_send(args: argparse.Namespace) -> int:
    ...
    if not args.dry_run and not args.yes:
        eprint(
            "Refusing to send without explicit confirmation. Re-run with --yes or use --dry-run."
        )
        return 2
    ...
    if not args.from_mnemonic_env and not args.from_private_key_env:
        eprint(
            "Sender is required: use --from-mnemonic-env or --from-private-key-env (secrets via env)."
        )
        return 2
