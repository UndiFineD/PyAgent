# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jjannet\gold-price-mcp\test_api.py
#!/usr/bin/env python3
"""
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if data.get("status") == "success" and "response" in data:
                ...
                print(f"\n  การเปลี่ยนแปลง:")
                print(
                    f"    เทียบครั้งก่อน: {result['price']['change']['compare_previous']}"
                )
                print(
                    f"    เทียบเมื่อวาน: {result['price']['change']['compare_yesterday']}"
                )
