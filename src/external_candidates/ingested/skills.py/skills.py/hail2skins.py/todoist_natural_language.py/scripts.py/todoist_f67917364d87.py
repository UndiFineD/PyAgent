# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hail2skins\todoist-natural-language\scripts\todoist.py
#!/usr/bin/env python3
"""
...
def get_auth_headers():
    ...
    if not token:
        print(
            json.dumps({"error": "TODOIST_API_KEY environment variable not set"}),
            file=sys.stderr,
        )
        sys.exit(1)
...
def make_request(endpoint, method="GET", data=None, params=None):
    ...
    try:
        ...
        try:
            ...
            print(
                json.dumps({"error": f"HTTP {e.code}: {error_body}"}), file=sys.stderr
            )
        sys.exit(1)
...
def create_task_with_duration_sync(
    content,
    due_string=None,
    duration_amount=None,
    duration_unit=None,
    ...
    data = {"commands": json.dumps([command]), "sync_token": "*"}
            ...
            if "sync_status" in result and cmd_uuid in result["sync_status"]:
                ...
                if status == "ok":
                    ...
                    return {
                        ...
                        "due": due_string,
                        "duration": (
                            {"amount": duration_amount, "unit": duration_unit}
                            if duration_amount
                            else None
                        ),
                    }
...
def main():
    ...
    list_parser.add_argument(
        "--filter", "-f", help="Todoist filter string (e.g., 'today', 'overdue', 'p1')"
    )
    list_parser.add_argument(
        "--project", "-p", help="Project name or ID (e.g., 'Personal', 'Work')"
    )
    list_parser.add_argument("--label", "-l", help="Label name")
    list_parser.add_argument(
        "--priority", type=int, choices=[1, 2, 3, 4], help="Priority (1=urgent, 4=low)"
    )
    ...
    add_parser.add_argument(
        "--due", "-d", help="Due date (natural language: 'tomorrow', 'next Monday')"
    )
    add_parser.add_argument(
        "--priority",
        type=int,
    ...
    add_parser.add_argument(
        "--label", "-l", action="append", help="Label (can use multiple)"
    )
    add_parser.add_argument("--description", help="Task description")
    add_parser.add_argument(
        "--duration-amount", type=int, help="Duration amount (e.g., 30, 60)"
    )
    add_parser.add_argument(
        "--duration-unit",
        choices=["minute", "min", "day"],
        help="Duration unit (minute/min or day)",
    )
    ...
    uncomplete_parser = subparsers.add_parser(
        "uncomplete", help="Reopen a completed task"
    )
    ...
    update_parser.add_argument(
        "--priority", type=int, choices=[1, 2, 3, 4], help="Priority"
    )
    update_parser.add_argument(
        "--label", "-l", action="append", help="Labels (replaces existing)"
    )
