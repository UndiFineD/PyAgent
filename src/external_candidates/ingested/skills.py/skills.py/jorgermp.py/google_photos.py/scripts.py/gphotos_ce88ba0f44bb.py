# Extracted from: C:\DEV\PyAgent\.external\skills\skills\jorgermp\google-photos\scripts\gphotos.py
...
def list_albums(creds):
    ...
    response = requests.get(
        "https://photoslibrary.googleapis.com/v1/albums", headers=headers
    )
...
def create_album(creds, title):
    ...
    response = requests.post(
        "https://photoslibrary.googleapis.com/v1/albums", headers=headers, json=payload
    )
    return response.json()
...
def upload_photo(creds, photo_path, album_id=None):
    ...
    with open(photo_path, "rb") as f:
        response = requests.post(
            "https://photoslibrary.googleapis.com/v1/uploads", headers=headers, data=f
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--token", default=TOKEN_FILE, help="Path to store/read auth token"
    )
