# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bgengs\todozi\scripts\todozi.py
#!/usr/bin/env python3
"""
...
class TodoziClient:
        ...
        if not thread_id:
            ...
            for matrix in matrices:
                if matrix.name.lower() in ["default", "general", "do"]:
                    ...
                    break
            if not thread_id:
                default = await self.create_matrix(
                    name="Default", category="do", description="Default matrix"
                )
                thread_id = default.id

        data = await self._request(
            "POST",
            f"matrix/{thread_id}/task",
            {
                "title": title,
                "type": "task",
                "description": description,
    ...
    async def create_note(
        ...
        data = await self._request(
            "POST", endpoint, {"title": title, "description": description}
        )
        return self._parse_task(data)
    ...
    async def update_webhook(
        ...
        return await self._request(
            "PUT", f"webhook/{webhook_id}", {"url": url, "events": events}
        )
    ...
    async def update_user_preferences(
        ...
        learned_context: Optional[List[str]] = None,
    ) -> Dict[str, Any]:
        ...
        if learned_context:
            data["learned_context"] = learned_context
...
def todozi_list_tasks(
    ...
    try:
        ...
        tasks = asyncio.run(
            client.list_tasks(status=status, priority=priority, thread_id=thread_id)
        )[:limit]
        if not tasks:
...
def todozi_complete_task(task_id: str) -> str:
    ...
    try:
        ...
        return (
            ...
            if success
            else f"❌ Could not complete {task_id}"
        )
    except Exception as e:
...
def todozi_search(
    ...
    try:
        ...
        results = asyncio.run(
            client.search(
                query, type_=type_, status=status, priority=priority, limit=limit
            )
        )
        if not results:
