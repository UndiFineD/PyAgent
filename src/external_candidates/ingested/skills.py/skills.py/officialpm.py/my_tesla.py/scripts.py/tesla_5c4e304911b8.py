# Extracted from: C:\DEV\PyAgent\.external\skills\skills\officialpm\my-tesla\scripts\tesla.py
#!/usr/bin/env python3
# /// script
...
def cmd_auth(args):
    ...
    if not email:
        print(
            "❌ Missing Tesla email. Set TESLA_EMAIL or pass --email", file=sys.stderr
        )
        sys.exit(2)
...
def cmd_list(args):
    ...
    for i, v in enumerate(vehicles):
        star = (
            " (default)"
            if default_name and v["display_name"].lower() == default_name.lower()
            else ""
        )
...
def _report(vehicle, data):
    ...
    if (
        dep_enabled is not None
        or dep_time is not None
        or precond is not None
        or off_peak is not None
    ):
        bits = []
...
def _report_json(vehicle, data: dict) -> dict:
    ...
    out = {
        ...
        "scheduled_charging": {
            ...
            "start_time_hhmm": _fmt_minutes_hhmm(
                charge.get("scheduled_charging_start_time")
            ),
        },
        "scheduled_departure": {
...
def cmd_climate(args):
    ...
    if args.action == "status":
        ...
        if driver or passenger:
            print(
                f"Setpoint: driver {driver or '(unknown)'} | passenger {passenger or '(unknown)'}"
            )
        return
    ...
    if args.action == "on":
        ...
        if args.value is None:
            raise ValueError(
                "Missing temperature value (e.g., climate temp 72 or climate temp 22 --celsius)"
            )
        ...
        print(
            f"🌡️ {vehicle['display_name']} temperature set to {value:g}°{'F' if in_f else 'C'}"
        )
        ...
        print(
            f"🧊 {vehicle['display_name']} max defrost {('enabled' if on else 'disabled')}"
        )
    else:
...
def cmd_scheduled_charging(args):
    ...
    if args.action == "set":
        ...
        print(
            f"⏱️ {vehicle['display_name']} scheduled charging set to {_fmt_minutes_hhmm(minutes)}"
        )
        return
...
def _scheduled_departure_status_json(charge: dict) -> dict:
    ...
    return {
        ...
        "scheduled_departure_time_hhmm": _fmt_minutes_hhmm(
            charge.get("scheduled_departure_time")
        ),
        "preconditioning_enabled": charge.get("preconditioning_enabled"),
        "off_peak_charging_enabled": charge.get("off_peak_charging_enabled"),
    }
...
def cmd_scheduled_departure(args):
    ...
    if out.get("scheduled_departure_enabled") is not None:
        print(
            f"Scheduled departure: {_fmt_bool(out.get('scheduled_departure_enabled'), 'On', 'Off')}"
        )
    else:
    ...
    if out.get("preconditioning_enabled") is not None:
        print(
            f"Preconditioning: {_fmt_bool(out.get('preconditioning_enabled'), 'On', 'Off')}"
        )

    if out.get("off_peak_charging_enabled") is not None:
        print(
            f"Off-peak charging: {_fmt_bool(out.get('off_peak_charging_enabled'), 'On', 'Off')}"
        )
...
def resolve_since_ts(
    *, since_ts: int | None = None, since_days: float | None = None
) -> int | None:
...
def cmd_mileage(args):
    ...
    if args.action == "record":
        ...
        if args.json:
            print(
                json.dumps(
                    {"db": str(db_path), "ts_utc": ts, "results": results}, indent=2
                )
            )
            return
    ...
    if args.action == "status":
        ...
        if not latest:
            print(
                "No mileage points recorded yet. Run: " + _invocation("mileage record")
            )
            return
        for v in out["vehicles"]:
            print(
                f"- {v['vehicle']}: {v['odometer_mi']:.1f} mi (last: {_fmt_dt(v['ts_utc'])})"
            )
        return
    ...
    if args.action == "export":
        ...
        if fmt == "json":
            ...
            print(
                json.dumps(
                    {"db": str(db_path), "since_ts_utc": since_ts, "items": items},
                    indent=2,
                )
            )
            return
...
def main():
    ...
    status_parser.add_argument(
        "--summary", action="store_true", help="Also print a one-line summary"
    )
    status_parser.add_argument(
        "--no-wake", action="store_true", help="Do not wake the car (fails if asleep)"
    )
    ...
    default_parser = subparsers.add_parser(
        "default-car", help="Set/show default vehicle name"
    )
    default_parser.add_argument(
        "name", nargs="?", help="Vehicle display name to set as default"
    )

    # Mileage tracking (odometer)
    mileage_parser = subparsers.add_parser(
        "mileage", help="Record odometer mileage to a local SQLite DB"
    )
    mileage_parser.add_argument(
        "action",
        choices=["init", "record", "status", "export"],
        help="init|record|status|export",
    )
    mileage_parser.add_argument(
        "--json", action="store_true", help="Output JSON (record/status only)"
    )
    mileage_parser.add_argument(
        "--db", help="Path to the SQLite DB (default: ~/.my_tesla/mileage.sqlite)"
    )
    mileage_parser.add_argument(
        "--no-wake", action="store_true", help="Do not wake sleeping cars"
    )
    mileage_parser.add_argument(
    ...
    mileage_parser.add_argument(
        "--format", choices=["csv", "json"], default="csv", help="For export: csv|json"
    )
    mileage_parser.add_argument(
        "--since-ts",
        type=int,
    ...
    climate_parser.add_argument(
        "action", choices=["status", "on", "off", "temp", "defrost"]
    )
    climate_parser.add_argument(
        "value",
        nargs="?",
    ...
    temp_units.add_argument(
        "--fahrenheit",
        "-f",
        action="store_true",
        help="Temperature value is in °F (default)",
    )
    temp_units.add_argument(
        "--celsius", action="store_true", help="Temperature value is in °C"
    )

    # Charge
    charge_parser = subparsers.add_parser("charge", help="Charging control")
    charge_parser.add_argument(
        "action", choices=["status", "start", "stop", "limit", "amps"]
    )
    charge_parser.add_argument(
        "value",
        nargs="?",
    ...
    sched_parser = subparsers.add_parser(
        "scheduled-charging", help="Get/set scheduled charging (set/off requires --yes)"
    )
    sched_parser.add_argument(
        "action", choices=["status", "set", "off"], help="status|set|off"
    )
    sched_parser.add_argument(
        "time", nargs="?", help="Start time for 'set' as HH:MM (24-hour)"
    )
    sched_parser.add_argument(
        "--no-wake", action="store_true", help="(status only) Do not wake the car"
    )
    ...
    tires_parser.add_argument(
        "--no-wake", action="store_true", help="Do not wake the car (fails if asleep)"
    )

    # Openings (doors/trunks/windows)
    openings_parser = subparsers.add_parser(
        "openings", help="Show which doors/trunks/windows are open"
    )
    openings_parser.add_argument(
        "--no-wake", action="store_true", help="Do not wake the car (fails if asleep)"
    )

    # Trunk / frunk
    trunk_parser = subparsers.add_parser(
        "trunk", help="Toggle trunk/frunk (requires --yes)"
    )
    trunk_parser.add_argument(
        "which", choices=["trunk", "frunk"], help="Which to actuate"
    )

    # Windows
    windows_parser = subparsers.add_parser(
        "windows", help="Windows status (read-only) or vent/close (requires --yes)"
    )
    windows_parser.add_argument(
        "action", choices=["status", "vent", "close"], help="status|vent|close"
    )
    windows_parser.add_argument(
        "--no-wake", action="store_true", help="(status only) Do not wake the car"
    )
    windows_parser.add_argument(
        "--json", action="store_true", help="(status only) Output JSON"
    )

    # Seat heaters
    seats_parser = subparsers.add_parser(
        "seats", help="Seat heater status (read-only) or set level (requires --yes)"
    )
    ...
    sentry_parser = subparsers.add_parser(
        "sentry", help="Get/set Sentry Mode (on/off requires --yes)"
    )
    sentry_parser.add_argument(
        "action", choices=["status", "on", "off"], help="status|on|off"
    )
    sentry_parser.add_argument(
        "--no-wake", action="store_true", help="(status only) Do not wake the car"
    )
    ...
    charge_port_parser.add_argument(
        "action", choices=["status", "open", "close"], help="status|open|close"
    )
    charge_port_parser.add_argument(
        "--no-wake", action="store_true", help="(status only) Do not wake the car"
    )
    ...
    try:
        ...
        debug = (
            ...
            or os.environ.get("MY_TESLA_DEBUG") == "1"
        )
        if debug:
