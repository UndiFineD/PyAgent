...
def cmd_search(args):
    ...
    status, messages = imap.search(
        None, f'(OR SUBJECT "{args.query}" BODY "{args.query}")'
    )
    msg_ids = messages[0].split()
    ...
    print(
        f"🔍 Search results for '{args.query}' ({len(msg_ids)} found, showing {len(recent_ids)}):\n"
    )
...
def main():
    ...
    inbox_parser.add_argument(
        "--limit", "-l", type=int, default=10, help="Number of emails"
    )

    unread_parser = subparsers.add_parser("unread", help="List unread emails")
    unread_parser.add_argument(
        "--limit", "-l", type=int, default=20, help="Number of emails"
    )
