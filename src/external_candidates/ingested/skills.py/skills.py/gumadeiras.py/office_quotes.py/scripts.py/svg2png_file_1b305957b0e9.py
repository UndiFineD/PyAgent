# Extracted from: C:\DEV\PyAgent\.external\skills\skills\gumadeiras\office-quotes\scripts\svg2png_file.py
#!/usr/bin/env python3
...
def svg_file_to_png(svg_path, output_path, width=500, height=300):
    ...
    quote_match = re.search(
        r'<div[^>]*class="quote-text"[^>]*>(.*?)</div>', svg_content, re.DOTALL
    )
    if quote_match:
    ...
    char_match = re.search(
        r'<text[^>]*class="character-info"[^>]*>(.*?)</text>', svg_content, re.DOTALL
    )
    if char_match:
