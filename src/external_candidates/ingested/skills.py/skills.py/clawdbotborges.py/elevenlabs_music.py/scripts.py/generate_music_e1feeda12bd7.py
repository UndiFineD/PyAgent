# Extracted from: C:\DEV\PyAgent\.external\skills\skills\clawdbotborges\elevenlabs-music\scripts\generate_music.py
#!/usr/bin/env python3
# /// script
...
def generate_music(
    ...
    print(
        f"   Prompt: {prompt[:80]}{'...' if len(prompt) > 80 else ''}", file=sys.stderr
    )
    ...
    try:
        ...
        if "limited_access" in error_str or "402" in error_str:
            print(
                "❌ Error: Music API requires a paid ElevenLabs plan", file=sys.stderr
            )
