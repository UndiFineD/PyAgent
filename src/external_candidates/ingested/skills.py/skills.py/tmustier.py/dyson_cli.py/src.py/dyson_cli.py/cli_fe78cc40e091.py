# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tmustier\dyson-cli\src\dyson_cli\cli.py
...
@click.option(
    "--region",
    type=click.Choice(
        ["US", "CA", "CN", "GB", "AU", "DE", "FR", "IT", "ES", "NL", "IE"]
    ),
    default="GB",
...
def setup(email: str, region: str):
    ...
    try:
        ...
        console.print(
            "[red]Error: libdyson not installed. Run: pip install libdyson[/red]"
        )
        sys.exit(1)
    ...
    try:
        ...
        console.print(
            f"[red]Server error. Try a different region (e.g., GB, US, DE)[/red]"
        )
        sys.exit(1)
...
def list_devices(check: bool):
    ...
    if not devices:
        console.print(
            "[yellow]No devices configured. Run 'dyson setup' first.[/yellow]"
        )
        return
...
def status(device: Optional[str], as_json: bool):
    ...
    if not ip:
        console.print(
            "[yellow]No IP address configured. Trying auto-discovery...[/yellow]"
        )
        try:
    ...
    if not ip:
        console.print(
            "[red]Could not find device IP. Please add 'ip' to config manually.[/red]"
        )
        sys.exit(1)
    ...
    try:
        ...
        raw_state = {
            ...
            "oscillation_angle_low": getattr(
                dyson_device, "oscillation_angle_low", None
            ),
            "oscillation_angle_high": getattr(
                dyson_device, "oscillation_angle_high", None
            ),
        ...
        if as_json:
            ...
            night = (
                "[green]✓[/green]" if raw_state.get("night_mode") else "[dim]Off[/dim]"
            )
...
def _control_power(device_name: Optional[str], power_on: bool):
    ...
    if not ip:
        console.print(
            "[red]No IP configured. Run 'dyson status' first to discover.[/red]"
        )
        sys.exit(1)
...
@click.option(
    "--angle", "-a", type=int, help="Oscillation range in degrees (45, 90, 180, or 350)"
)
@click.option("--device", "-d", help="Device name or serial")
...
def night(state: str, device: Optional[str]):
    ...
    try:
        ...
        enable = state == "on"
        (
            dyson_device.enable_night_mode()
            if enable
            else dyson_device.disable_night_mode()
        )
        console.print(
            f"[green]✓ Night mode {'enabled' if enable else 'disabled'}[/green]"
        )
...
def remove_device(name: str, force: bool):
    ...
    for d in devices:
        if (
            d.get("name", "").lower() == name.lower()
            or d.get("serial", "").lower() == name.lower()
        ):
            device = d
            break
    ...
    if config.get("default_device") == device.get("name"):
        config["default_device"] = (
            config["devices"][0]["name"] if config["devices"] else None
        )
