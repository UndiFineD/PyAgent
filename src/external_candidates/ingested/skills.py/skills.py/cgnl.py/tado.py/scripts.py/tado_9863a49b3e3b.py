# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cgnl\tado\scripts\tado.py
#!/usr/bin/env python3
"""
...
class TadoClient:
        ...
        try:
            if zone_id:
                ...
                return {
                    ...
                    "current_temp": state.get("sensorDataPoints", {})
                    .get("insideTemperature", {})
                    .get("celsius"),
                    "current_humidity": state.get("sensorDataPoints", {})
                    .get("humidity", {})
                    .get("percentage"),
                    "target_temp": state.get("setting", {})
                    .get("temperature", {})
                    .get("celsius"),
                    "heating": state.get("activityDataPoints", {})
                    .get("heatingPower", {})
                    .get("percentage", 0)
                    > 0,
                    "heating_power": state.get("activityDataPoints", {})
                    .get("heatingPower", {})
                    .get("percentage", 0),
    ...
    def set_temperature(
        self, zone_id: int, temperature: float, duration: Optional[int] = None
    ):
        ...
        if isinstance(data, dict):
            if "zones" in data:
                ...
                for zone in data["zones"]:
                    ...
                    print(
                        f"  Current: {zone['current_temp']}°C ({zone['current_humidity']}% humidity)"
                    )
                    if zone["target_temp"]:
                        print(f"  Target:  {zone['target_temp']}°C")
                    print(
                        f"  Heating: {'ON' if zone['heating'] else 'OFF'} ({zone['heating_power']}%)"
                    )
                    if zone["overlay"]:
                ...
                print(
                    f"  Current: {data['current_temp']}°C ({data['current_humidity']}% humidity)"
                )
                if data["target_temp"]:
                    print(f"  Target:  {data['target_temp']}°C")
                print(
                    f"  Heating: {'ON' if data['heating'] else 'OFF'} ({data['heating_power']}%)"
                )
                if data["overlay"]:
                ...
                if "temperature" in data:
                    duration_str = (
                        f" for {data['duration']} min"
                        if data.get("duration")
                        else " (permanent)"
                    )
                    print(
                        f"✅ Set {data['zone_name']} to {data['temperature']}°C{duration_str}"
                    )
...
Examples:
  ...
  %(prog)s presence
    ...
    set_parser.add_argument(
        "--temperature", "-t", type=float, required=True, help="Target temperature (°C)"
    )
    set_parser.add_argument(
        "--duration", "-d", type=int, help="Duration in minutes (optional)"
    )
    ...
    mode_parser.add_argument(
        "mode", choices=["home", "away", "auto"], help="Mode to set"
    )
