# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cgnl\supermarkt-prijzen\get-bonuses-v2.py
#!/usr/bin/env python3
"""
...
def get_user_token(auth_code):
    ...
    response = requests.post(
        AUTH_TOKEN,
        json={"clientId": "appie", "code": auth_code},
        headers={"Content-Type": "application/json", "User-Agent": "Appie/8.22.3"},
        timeout=10,
    )
...
def refresh_token(refresh_token_value):
    ...
    response = requests.post(
        AUTH_REFRESH,
        json={"clientId": "appie", "refreshToken": refresh_token_value},
        headers={"Content-Type": "application/json", "User-Agent": "Appie/8.22.3"},
        timeout=10,
    )
...
def search_bonuses(access_token, filter_set="WEB_BONUS_PAGE"):
    ...
    response = requests_cffi.post(
        ...
        json={
            ...
            "variables": {"periodStart": today, "periodEnd": next_week},
            "query": query,
        },
        headers={
...
def main():
    ...
    parser.add_argument(
        "--setup-anonymous", action="store_true", help="Get anonymous token"
    )
    parser.add_argument(
        "--setup-user", action="store_true", help="Get user token (needs auth code)"
    )
    ...
    if args.setup_user:
        if not args.auth_code:
            ...
            print(
                "2. Login → copy the code from: appie://login-exit?code=CODE\n",
                file=sys.stderr,
            )
            print(
                "3. Run: ./get-bonuses-v2.py --setup-user --auth-code CODE\n",
                file=sys.stderr,
            )
            sys.exit(1)
