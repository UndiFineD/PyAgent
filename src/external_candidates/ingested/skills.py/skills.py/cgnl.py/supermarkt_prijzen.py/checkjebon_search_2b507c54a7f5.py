# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cgnl\supermarkt-prijzen\checkjebon-search.py
#!/usr/bin/env python3
"""
...
def download_data(force=False):
    ...
    print(
        f"✅ Downloaded {DATA_FILE.stat().st_size / 1024 / 1024:.1f}MB", file=sys.stderr
    )
                ...
                matches.append(
                    {
                        ...
                        "size": product["s"],
                        "link": (
                            f"https://www.{store_name}.nl/{product['l']}"
                            if "l" in product
                            else None
                        ),
                        "price_per_unit": calculate_unit_price(
                            product["p"], product["s"]
                        ),
                    }
                )
...
def compare_prices(query, top=5):
    ...
    for store, products in results.items():
        for product in products:
            all_products.append({**product, "store": store})
...
def main():
    parser = argparse.ArgumentParser(
        description="Search Checkjebon.nl supermarket prices"
    )
    parser.add_argument("--query", "-q", help="Search query")
    parser.add_argument("--store", "-s", help="Filter by store (ah, jumbo, plus, etc.)")
    parser.add_argument(
        "--limit", "-l", type=int, default=10, help="Max results per store"
    )
    parser.add_argument("--compare", "-c", help="Compare prices across all stores")
    parser.add_argument(
        "--top", "-t", type=int, default=5, help="Top N cheapest (with --compare)"
    )
    parser.add_argument("--pretty", action="store_true", help="Pretty JSON output")
    parser.add_argument(
        "--refresh", action="store_true", help="Force download latest data"
    )
