# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cgnl\supermarkt-prijzen\test-allerhande-introspection.py
#!/usr/bin/env python3
import json
    ...
    if "errors" in data:
        ...
        for field in fields:
            if "recipe" in field["name"].lower():
                ...
                if field.get("args"):
                    print(
                        f"  Args: {', '.join(a['name'] + ':' + str(a['type'].get('name', a['type'].get('kind'))) for a in field['args'])}"
                    )
    else:
