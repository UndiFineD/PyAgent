# Extracted from: C:\DEV\PyAgent\.external\skills\skills\cgnl\supermarkt-prijzen\ah-mobile-auth.py
#!/usr/bin/env python3
"""
...
def authorize(code):
    ...
    response = requests.post(
        AUTH_URL, headers=headers, json=payload, impersonate="chrome120"
    )
...
def refresh_token():
    ...
    if not TOKEN_FILE.exists():
        print(
            f"❌ No tokens found. Run: ./ah-mobile-auth.py authorize --code YOUR_CODE"
        )
        sys.exit(1)
    ...
    payload = {"clientId": "appie", "refreshToken": tokens["refresh_token"]}

    print("🔄 Refreshing access token...")

    response = requests.post(
        REFRESH_URL, headers=headers, json=payload, impersonate="chrome120"
    )
...
def test_api():
    ...
    response = requests.post(
        GRAPHQL_URL, headers=headers, json={"query": query}, impersonate="chrome120"
    )
