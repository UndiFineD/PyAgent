        ...
        if path.exists():
            try:
                ...
                for key, profile in profiles.items():
                    if profile.get("provider") == "google-antigravity" and profile.get(
                        "refresh"
                    ):
                        email = profile.get(
                            "email", key.split(":")[-1] if ":" in key else "unknown"
                        )
                        all_creds.append(
                            {
                                "email": email,
                ...
                try:
                    ...
                    for candidate in candidates:
                        ...
                        for part in parts:
                            ...
                            if inline_data.get("data") and inline_data.get(
                                "mimeType", ""
                            ).startswith("image/"):
        ...
        try:
            ...
            if not project_id:
                ...
                project_id = extract_project_id(
                    code_assist.get("cloudaicompanionProject")
                )
