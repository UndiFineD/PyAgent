...
def find_context_matches(
    data: bytes, context: bytes, max_occurrences: int = 200
) -> list:
    ...
    for patch in patch_def["patches"]:
        ...
        if context:
            ...
            for ctx_pos in context_positions:
                ...
                if local_pos != -1:
                    ...
                    if data[abs_pos : abs_pos + len(search)] == search:
                        if not dry_run:
                            data = (
                                data[:abs_pos] + replace + data[abs_pos + len(search) :]
                            )
                        total_patches += 1
            ...
            if count > 0:
                ...
                total_patches += count
                patch_details.append(
                    {"count": count, "description": patch.get("description", "Unknown")}
                )
...
def main():
    ...
    parser.add_argument(
        "--variant", "-v", help="Variant to apply (excited, waving, original)"
    )
    parser.add_argument("--binary", "-b", help="Path to Claude binary")
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available variants"
    )
    parser.add_argument(
        "--dry-run", "-n", action="store_true", help="Show what would be changed"
    )
    parser.add_argument("--no-backup", action="store_true", help="Skip creating backup")
    parser.add_argument(
        "--restore", "-r", action="store_true", help="Restore to original"
    )
