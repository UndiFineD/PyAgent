# Extracted from: C:\DEV\PyAgent\.external\skills\skills\masonc15\clawd-modifier\scripts\patch_color.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    if args.restore:
        ...
        if len(parts) != 3:
            print(
                "Error: RGB must be in format R,G,B (e.g., 100,149,237)",
                file=sys.stderr,
            )
            sys.exit(1)
        rgb = f"rgb({args.rgb})"
    ...
    if result["success"]:
        ...
        print(
            f"{action} {result['rgb_replacements']} RGB and {result['ansi_replacements']} ANSI color definitions"
        )
