# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bert-builder\tavily\scripts\tavily_search.py
#!/usr/bin/env python3
"""
...
Environment Variables:
  TAVILY_API_KEY    Your Tavily API key (get one at https://tavily.com)
    ...
    parser.add_argument(
        "--api-key", help="Tavily API key (or set TAVILY_API_KEY env var)"
    )
    ...
    parser.add_argument(
        "--max-results", type=int, default=5, help="Maximum number of results (1-10)"
    )

    parser.add_argument(
        "--no-answer", action="store_true", help="Exclude AI-generated answer summary"
    )

    parser.add_argument(
        "--raw-content", action="store_true", help="Include raw HTML content of sources"
    )

    parser.add_argument(
        "--images", action="store_true", help="Include relevant images in results"
    )

    parser.add_argument(
        "--include-domains", nargs="+", help="List of domains to specifically include"
    )

    parser.add_argument(
        "--exclude-domains", nargs="+", help="List of domains to exclude"
    )
