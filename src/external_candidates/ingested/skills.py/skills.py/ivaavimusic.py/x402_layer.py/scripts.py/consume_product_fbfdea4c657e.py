# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ivaavimusic\x402-layer\scripts\consume_product.py
#!/usr/bin/env python3
"""
...
def resolve_slug_to_api_url(slug: str) -> str:
    ...
    response = requests.get(
        f"{API_BASE}/marketplace", params={"type": "product", "search": slug}
    )
    ...
    if not product:
        ...
        product = next(
            (item for item in items if slug.lower() in item.get("slug", "").lower()),
            None,
        )
...
def consume_product(product_input: str, download_file: bool = False) -> dict:
    ...
    if network != "base":
        raise ValueError(
            f"This script only supports Base network. Product requires: {network}"
        )

    if currency not in ["USDC", "USD"]:
        raise ValueError(
            f"This script only supports USDC. Product requires: {currency}"
        )
