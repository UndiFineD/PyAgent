...
class LongbridgeSkill:
    ...
    async def quote_subscribe(
        self, symbols: List[str], sub_types: List[str]
    ) -> Dict[str, Any]:
        """
        订阅实时行情数据
...
async def submit_order(
    ...
    return await _skill.submit_order(
        symbol, order_type, side, quantity, time_in_force, price, remark
    )
...
async def get_today_orders(
    symbol: Optional[str] = None, status: Optional[List[str]] = None
):
...
async def get_candlesticks(
    symbol: str, period: str, count: int, adjust_type: str = "NoAdjust"
):
