...
def get_api_key() -> str:
    ...
    if len(key) < 10:
        print(
            json.dumps({"error": "Serper API key appears invalid (too short)"}),
            flush=True,
        )
        sys.exit(1)
    return key
...
def serper_news_search(
    query: str, api_key: str, num: int = 3, gl: Optional[str] = None, hl: str = "en"
) -> List[Dict[str, Any]]:
...
def search_current(
    query: str, api_key: str, locale: Dict[str, Optional[str]]
) -> List[Dict[str, Any]]:
    """Current/news mode: past week web + news search, 3 results each."""
    all_results = []
    seen_urls = set()

    for r in serper_web_search(
        query, api_key, num=3, gl=locale["gl"], hl=locale["hl"], tbs="qdr:w"
    ):
        if r["source"] == "knowledge_graph":
    ...
    for r in serper_news_search(
        query, api_key, num=3, gl=locale["gl"], hl=locale["hl"]
    ):
        url = r.get("url", "")
...
def main():
    ...
    parser.add_argument(
        "--gl",
        default="world",
        help="Country code for Google (e.g. de, us, at, ch). Default: world",
    )
    parser.add_argument(
        "--hl", default="en", help="Language code for results (e.g. en, de)"
    )
    ...
    if args.mode == "current":
        ...
        results = serper_web_search(
            args.query, api_key, num=5, gl=locale["gl"], hl=locale["hl"]
        )

    if not results:
        print(
            json.dumps({"error": "No results found", "query": args.query}), flush=True
        )
        sys.exit(1)
    ...
    meta = {
        ...
        "locale": locale,
        "results": [
            {k: r[k] for k in ("title", "url", "source") if k in r} for r in results
        ],
    }
