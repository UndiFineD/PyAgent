...
def get_api_key() -> str:
    ...
    if not key:
        print(
            "❌ Error: SEARCHCH_API_KEY environment variable not set", file=sys.stderr
        )
...
def print_results(results: list[dict], verbose: bool = False):
    ...
    for i, r in enumerate(results, 1):
        ...
        if r.get("canton"):
            addr_parts[-1] = (
                f"{addr_parts[-1]} {r['canton']}" if addr_parts else r["canton"]
            )
        if addr_parts:
...
Examples:
  ...
  %(prog)s search "Restaurant" -l "Bern" -t business -n 5
    ...
    search_parser.add_argument(
        "query", help="Search string (name, category, or phone number)"
    )
    ...
    search_parser.add_argument(
        "-v", "--verbose", action="store_true", help="Show categories and extra details"
    )
