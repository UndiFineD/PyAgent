...
def show_manual_options(origin, dest, date=None):
    ...
    # FlightRadar24 route
    fr24_url = (
        f"https://www.flightradar24.com/data/flights/{origin.lower()}-{dest.lower()}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check flight schedules between airports"
    )
    parser.add_argument(
        "origin", nargs="?", help="Origin airport (IATA code, e.g., HAM)"
    )
    parser.add_argument(
        "destination", nargs="?", help="Destination airport (IATA code, e.g., ZRH)"
    )
    parser.add_argument(
        "--from", dest="from_airport", help="Origin airport (alternative)"
    )
    parser.add_argument(
        "--to", dest="to_airport", help="Destination airport (alternative)"
    )
