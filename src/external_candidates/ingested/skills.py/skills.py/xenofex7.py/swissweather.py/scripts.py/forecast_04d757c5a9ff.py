# Extracted from: C:\DEV\PyAgent\.external\skills\skills\xenofex7\swissweather\scripts\forecast.py
#!/usr/bin/env python3
"""
...
def format_forecast(data, days=3):
    ...
    if "dayForecast" in forecast:
        ...
        print(f"\n{'='*60}")
        print(
            f"Weather Forecast - {data.get('currentWeather', {}).get('location', 'Unknown')}"
        )
        print(f"{'='*60}\n")
...
Common postal codes:
  ...
  6900 - Lugano, 8640 - Rapperswil-Jona, 9000 - St. Gallen
    ...
    parser.add_argument(
        "--days", "-d", type=int, default=3, help="Number of days to show (default: 3)"
    )
