# Extracted from: C:\DEV\PyAgent\.external\skills\skills\hartlco\miniflux-news\scripts\miniflux.py
#!/usr/bin/env python3
"""Miniflux API helper.
...
def cmd_mark_read_category(args: argparse.Namespace) -> int:
    ...
    cid = (
        args.category_id
        if args.category_id is not None
        else _category_id_from_name(args.category)
    )
...
def main(argv: list[str]) -> int:
    ...
    p_cfg.add_argument(
        "--url", help="Miniflux base URL, e.g. https://reader.example.com"
    )
    ...
    p_entries.add_argument(
        "--direction", default="desc", choices=["asc", "desc"], help="Sort direction"
    )
    ...
    p_entry.add_argument(
        "--full", action="store_true", help="Print full content (instead of preview)"
    )
    p_entry.add_argument(
        "--format",
        default="text",
        choices=["text", "html"],
        help="Content format when printing",
    )
    ...
    p_mrc = sub.add_parser(
        "mark-read-category",
        help="Mark ALL unread entries in a category as read (explicit only)",
    )
    ...
    p_mrc.add_argument(
        "--limit", type=int, default=500, help="Safety limit: max entries to mark read"
    )
