# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rm289\hour-meter\scripts\meter.py
#!/usr/bin/env python3
"""
    ...
    for name, meter in data["meters"].items():
        ...
        for m in meter.get("milestones", []):
            ...
            if m["type"] == "hours" and elapsed_hours >= m["value"]:
                ...
                m["type"] == "percent" and percent is not None and percent >= m["value"]
            ):
                should_fire = True
...
def send_email_sendgrid(
    ...
    if not from_email:
        from_email = os.environ.get(
            "SENDGRID_FROM_EMAIL", "hour-meter@noreply.example.com"
        )
...
def generate_verification_email(
    meter_name: str, paper_code: str, full_hash: str, description: str, lock_time: str
) -> tuple[str, str, str]:
...
"""
    ...
    mailto = (
        f"mailto:?subject={urllib.parse.quote(subject)}&body={urllib.parse.quote(body)}"
    )
    return mailto
...
def cmd_lock(args):
    ...
    witness_path = append_witness(
        args.name, paper_code, full_hash, lock_time.isoformat()
    )
    ...
    mailto_link = generate_mailto_link(
        args.name, paper_code, full_hash, meter.get("description", ""), lock_time_str
    )
    ...
    if hasattr(args, "email") and args.email:
        ...
        success, message = send_email_sendgrid(
            args.email, subject, html_content, text_content, from_email
        )
        if success:
            print(f"   ✅ {message}")
        else:
...
def cmd_check(args):
    ...
    if mode == "up":
        ...
        print()
        print(
            f"   ✅ Elapsed:   {format_elapsed(elapsed_seconds)} ({elapsed_seconds/3600:,.0f} hrs)"
        )
        ...
        if remaining_seconds <= 0:
            ...
            print(
                f"   ⏳ Remaining: {format_elapsed(remaining_seconds)} ({remaining_seconds/3600:,.0f} hrs)"
            )
...
def cmd_verify(args):
    ...
    if len(clean_input) <= 20:
        ...
        if expected_hash.startswith(prefix):
            print(f"✅ VERIFIED!")
            print(
                f"   Paper code matches. Meter '{args.name}' has NOT been tampered with."
            )
            print()
            print(f"   Paper code: {meter.get('paper_code')}")
            print(
                f"   Created:    {datetime.fromtimestamp(meter['start_ms']/1000, tz=timezone.utc).strftime('%Y-%m-%d')}"
            )
        else:
            print(f"❌ MISMATCH!")
...
def cmd_career(args):
    ...
    print()
    print(
        f"   ⏱️  Hours worked:     {hours_worked:>12,.0f} hrs ({years_worked:.1f} yrs)"
    )
    print(
        f"   ⏳ Hours remaining:  {hours_remaining:>12,.0f} hrs ({years_remaining:.1f} yrs)"
    )
    print(
        f"   📦 Total inventory:  {total_hours:>12,.0f} hrs ({total_hours/hours_per_year:.0f} yrs)"
    )
    print()
    ...
    while remaining > 0:
        ...
        if year <= 3 or remaining <= hours_per_year or year % 5 == 0:
            print(
                f"   {year:<6} {format_money(current_rate):>10} {format_money(year_earnings):>14} {format_money(total_earnings):>14}"
            )
        elif year == 4:
            print(f"   {'...':^6}")
...
def main():
    ...
    p_milestone.add_argument(
        "--type", "-t", choices=["hours", "percent"], required=True
    )
    ...
    # Check milestones
    p_check_ms = subparsers.add_parser(
        "check-milestones", help="Check all milestones (JSON)"
    )
    ...
    p_lock.add_argument(
        "--email",
        "-e",
        help="Send verification code to this email address (requires SENDGRID_API_KEY)",
    )
    p_lock.add_argument(
        "--from-email",
        help="From email address (default: SENDGRID_FROM_EMAIL or hour-meter@noreply.example.com)",
    )
    ...
    p_verify.add_argument(
        "code", help="Paper code (XXXX-XXXX-XXXX-XXXX-C) or full hash"
    )
    ...
    p_witness.add_argument(
        "--show", action="store_true", help="Show witness file contents"
    )
