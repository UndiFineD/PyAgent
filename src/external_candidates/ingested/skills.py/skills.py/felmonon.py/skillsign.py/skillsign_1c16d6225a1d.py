# Extracted from: C:\DEV\PyAgent\.external\skills\skills\felmonon\skillsign\skillsign.py
#!/usr/bin/env python3
...
def cmd_verify(args):
    ...
    modified = {
        k
        for k in set(current_manifest.keys()) & set(stored_manifest.keys())
    ...
    if rev_info and not revoked:
        print(
            f"   ⚠️  Signer was later revoked ({rev_info['revoked_at']}), but this signature predates revocation."
        )
...
def cmd_revoke(args):
    ...
    if not keyfile or not keyfile.exists():
        print(
            "Error: No private key found. Provide --key or run 'skillsign keygen' first."
        )
        sys.exit(1)
    ...
    rev_bytes = json.dumps(revocation, sort_keys=True, separators=(",", ":")).encode(
        "utf-8"
    )
...
def main():
    ...
    p_revoke.add_argument(
        "--reason", default="Key compromised", help="Reason for revocation"
    )
