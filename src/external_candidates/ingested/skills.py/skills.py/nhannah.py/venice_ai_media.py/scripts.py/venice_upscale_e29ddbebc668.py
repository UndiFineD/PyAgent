...
def main() -> int:
    ...
    ap.add_argument(
        "image", nargs="?", help="Path to image file to upscale (or use --url)"
    )
    ...
    ap.add_argument(
        "--enhance", action="store_true", help="Apply AI enhancement during upscale"
    )
    ap.add_argument(
        "--enhance-prompt", help="Description for enhancement (requires --enhance)"
    )
    ap.add_argument(
        "--enhance-creativity",
        type=float,
    ...
    ap.add_argument(
        "--out-dir",
        help="Output directory (default: same as input or current dir for URLs)",
    )
    ...
    if args.url:
        ...
        if args.output:
            ...
            out_dir = (
                Path(args.out_dir).expanduser() if args.out_dir else image_path.parent
            )
            out_dir.mkdir(parents=True, exist_ok=True)
            timestamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
            ...
            out_path = (
                out_dir
                / f"{image_path.stem}-upscaled-{args.scale}x-{timestamp}{suffix}"
            )
