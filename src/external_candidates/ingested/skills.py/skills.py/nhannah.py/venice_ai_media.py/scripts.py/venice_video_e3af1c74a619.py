...
def main() -> int:
    ...
    ap.add_argument(
        "--video", help="Source video for video-to-video (local path or URL)"
    )
    ...
    ap.add_argument(
        "--audio", action="store_true", default=None, help="Generate audio track"
    )
    ap.add_argument(
        "--no-audio",
        action="store_false",
    ...
    ap.add_argument(
        "--audio-url", help="Background music file (WAV/MP3, max 30s, 15MB)"
    )
    ...
    if args.quote:
        ...
        audio_param = (
            None
            if args.skip_audio_param
            else (args.audio if args.audio is not None else True)
        )
    ...
    out_dir = (
        ...
        if args.out_dir
        else default_out_dir("venice-video")
    )
    ...
    while True:
        ...
        if avg_time and exec_dur:
            ...
            timing_str = (
                f" ({exec_dur // 1000}s / ~{avg_time // 1000}s avg, ~{progress_pct}%)"
            )
        elif exec_dur:
