...
def main() -> int:
    ...
    ap.add_argument(
        "--prompt", help="Image description. If omitted, generates random prompts."
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--width", type=int, default=1024, help="Image width 1-1280 (default: 1024)"
    )
    ap.add_argument(
        "--height", type=int, default=1024, help="Image height 1-1280 (default: 1024)"
    )
    ap.add_argument(
        "--format",
        dest="fmt",
    ...
    ap.add_argument(
        "--cfg-scale", type=float, help="Prompt adherence 0-20 (default: 7.5)"
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        default=False,
        help="Blur adult content (default: false)",
    )
    ap.add_argument(
        "--no-safe-mode",
        action="store_false",
    ...
    ap.add_argument(
        "--hide-watermark", action="store_true", help="Remove Venice watermark"
    )
    ap.add_argument(
        "--embed-exif",
        action="store_true",
    ...
    ap.add_argument(
        "--lora-strength", type=int, help="LoRA strength 0-100 for applicable models"
    )
    ap.add_argument(
        "--enable-web-search",
        action="store_true",
    ...
    if not args.no_validate:
        ...
        if not exists and available:
            print(
                f"Error: Model '{args.model}' not found or unavailable.",
                file=sys.stderr,
            )
    ...
    out_dir = (
        ...
        if args.out_dir
        else default_out_dir("venice-image")
    )
    ...
    if args.prompt:
        ...
        while remaining > 0:
            ...
            print(
                f"[Batch {batch_num}] Generating {variants} image(s): {args.prompt[:60]}{'...' if len(args.prompt) > 60 else ''}"
            )
        ...
        for idx, prompt in enumerate(prompts, start=1):
            print(
                f"[{idx}/{len(prompts)}] {prompt[:80]}{'...' if len(prompt) > 80 else ''}"
            )
