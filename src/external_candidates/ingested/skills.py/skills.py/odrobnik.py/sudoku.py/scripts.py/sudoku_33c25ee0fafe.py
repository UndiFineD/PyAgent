# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\sudoku\scripts\sudoku.py
#!/usr/bin/env python3
    ...
    *,
    ...
    if index is not None:
        ...
        if i < 0 or i >= len(puzzles):
            raise ValueError(
                f"index out of range: {index} (have {len(puzzles)} puzzles)"
            )
        return puzzles[i], i
...
def write_puzzle_json(doc: Dict[str, Any]) -> Path:
    ...
    path = PUZZLES_DIR / puzzle_json_filename(
        str(stamp), str(preset_key), size, puzzle_id
    )
...
def find_puzzle_json_by_short_id(short_id: str) -> Path:
    ...
    ensure_dirs()
    matches = sorted(
        PUZZLES_DIR.glob(f"*_{short_id}.json"), key=lambda p: p.stat().st_mtime
    )
    if not matches:
...
def find_puzzle_json_by_id(puzzle_id: str) -> Path:
    ...
    candidates = sorted(
        PUZZLES_DIR.glob(f"*_{sid}.json"), key=lambda p: p.stat().st_mtime
    )
    if candidates:
...
def load_puzzle_doc(
    *, file: Optional[str] = None, puzzle_id: Optional[str] = None, latest: bool = False
...
def render_paths(doc: Dict[str, Any], *, kind: str, ext: str = "png") -> Path:
    ...
    return (
        RENDERS_DIR
        / f"{stamp}_{preset_key}_{size}x{size}_{puzzle_id_short}_{kind}.{ext}"
    )
...
def render_puzzle_pdf(
    doc: Dict[str, Any], *, printable: bool = True, dpi: int = 300
) -> Path:
    kind = "puzzle_print" if printable else "puzzle"
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    for key in sorted(PRESETS.keys()):
        p = PRESETS[key]
        items.append(
            {"preset": p.key, "desc": p.desc, "letters": p.letters, "url": p.url}
        )
...
def cmd_get(args: argparse.Namespace) -> int:
    ...
    selector_count = sum(
        1
        for x in (args.index is not None, args.id is not None, args.seed is not None)
        if x
    )
    if selector_count > 1:
    ...
    puzzle, picked_idx = pick_puzzle(
        puzzles, index=args.index, puzzle_id=args.id, seed=args.seed
    )
...
def cmd_render(args: argparse.Namespace) -> int:
    doc, json_path = load_puzzle_doc(
        file=args.file, puzzle_id=args.id, latest=args.latest
    )
...
def cmd_reveal(args: argparse.Namespace) -> int:
    ...
    if args.box is not None:
        ...
        if len(vals) == 1:
            ...
            raise ValueError(
                "--box expects either 1 value (index) or 2 values (row col)"
            )

        out_path = render_paths(doc, kind=f"box_{idx}_r{box_r}_c{box_c}")
        crop_box_image(
            reveal_img, size=size, box_r=box_r, box_c=box_c, out_path=out_path
        )

        out = {
            "puzzle_json": str(json_path),
            "box": {"index": idx, "r": box_r, "c": box_c},
            "image": str(out_path),
        }
        if args.json:
            print(json.dumps(out, ensure_ascii=False))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_list.add_argument(
        "--text", dest="json", action="store_false", help="Output text instead of JSON"
    )
    ...
    p_get.add_argument(
        "--index",
        type=int,
        help="Select puzzle by index (1-based; 0 allowed for first)",
    )
    ...
    p_get.add_argument(
        "--render", action="store_true", help="Also render the puzzle image now"
    )
    p_get.add_argument(
        "--text", dest="json", action="store_false", help="Output text instead of JSON"
    )
    ...
    g.add_argument(
        "--latest", action="store_true", help="Use latest stored puzzle (default)"
    )
    g.add_argument("--file", help="Path to a stored puzzle JSON")
    g.add_argument(
        "--id", help="Puzzle ID (full UUID or short 8-char ID from filename)"
    )
    p_ren.add_argument(
        "--printable",
        action="store_true",
        help="Include small header (difficulty + short ID) for printout",
    )
    p_ren.add_argument(
        "--pdf", action="store_true", help="Render as A4 PDF (recommended for printing)"
    )
    p_ren.add_argument(
        "--text", dest="json", action="store_false", help="Output text instead of JSON"
    )
    ...
    p_rev = sub.add_parser(
        "reveal", help="Reveal solution from stored JSON (full/box/cell)"
    )
    ...
    sel.add_argument(
        "--full", action="store_true", help="Full solution image (default)"
    )
    sel.add_argument(
        "--box",
        type=int,
        nargs="+",
        help="Reveal a single box: '--box <idx>' or '--box <row> <col>' (1-based)",
    )
    sel.add_argument(
        "--cell",
        type=int,
        nargs=2,
        metavar=("ROW", "COL"),
        help="Reveal a single cell value: '--cell <row> <col>' (1-based)",
    )

    p_rev.add_argument(
        "--image",
        action="store_true",
        help="With --cell: also write a tiny 1-cell image",
    )
    p_rev.add_argument(
        "--text", dest="json", action="store_false", help="Output text instead of JSON"
    )
