...
def get_valid_token(user_id="default"):
    ...
    if not token_file.exists():
        raise Exception(
            f"No token found for '{user_id}'. Run: python3 withings.py {user_id} auth"
        )
...
def get_body(user_id="default", limit=None):
    ...
    token = get_valid_token(user_id)
    data = post_request(
        "/measure", {"action": "getmeas", "access_token": token, "category": 1}
    )
        ...
        sleep_data.append(
            {
                ...
                "duration": format_duration(
                    d.get("total_sleep_time") or d.get("durationtosleep", 0)
                ),
...
def main():
    ...
    if command == "auth":
        ...
        print(
            "Commands: auth [code], weight, body [limit|all], activity [days], sleep [days]"
        )
        print("\nExamples:")
