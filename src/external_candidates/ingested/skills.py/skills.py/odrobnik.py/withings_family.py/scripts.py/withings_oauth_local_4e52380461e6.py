# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\withings-family\scripts\withings_oauth_local.py
#!/usr/bin/env python3
...
def run_callback_server(
    host: str, port: int, timeout_s: int
...
def main() -> int:
    ...
    ap.add_argument(
        "--timeout", type=int, default=300, help="Seconds to wait for callback"
    )
    ...
    if not CLIENT_ID or not CLIENT_SECRET:
        print(
            "Missing WITHINGS_CLIENT_ID or WITHINGS_CLIENT_SECRET. Set in .env or environment.",
            file=sys.stderr,
        )
        return 2
    ...
    if got_state != state:
        print(
            "State mismatch (possible CSRF / wrong browser session). Aborting.",
            file=sys.stderr,
        )
        return 6
