...
def load_runtime(dir_path: str, *, args: argparse.Namespace) -> TeslaRuntime:
    ...
    base_url = (
        args.base_url or cfg.get("base_url") or _env("TESLA_BASE_URL") or audience
    ).strip()

    return TeslaRuntime(
        client_id=args.client_id or _env("TESLA_CLIENT_ID") or cfg.get("client_id"),
        client_secret=args.client_secret
        or _env("TESLA_CLIENT_SECRET")
...
def cmd_register(rt: TeslaRuntime, dir_path: str) -> int:
    if not rt.domain:
        print(
            "Specify --domain or set TESLA_DOMAIN / config.json:domain", file=sys.stderr
        )
        return 1
    if not rt.client_id or not rt.client_secret:
...
def cmd_config_show(rt: TeslaRuntime, dir_path: str) -> int:
    ...
    print(
        f"🔑 Client ID:     {'(from env)' if _env('TESLA_CLIENT_ID') else '(not set)'}"
    )
    print(
        f"🔒 Client Secret: {'(from env)' if _env('TESLA_CLIENT_SECRET') else '(not set)'}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="auth.py", description="Tesla Fleet API auth/config"
    )
    p.add_argument(
        "--dir",
        default=default_dir(),
    ...
    p_set.add_argument(
        "--clear", action="append", help="Remove a key from config.json (repeatable)"
    )
