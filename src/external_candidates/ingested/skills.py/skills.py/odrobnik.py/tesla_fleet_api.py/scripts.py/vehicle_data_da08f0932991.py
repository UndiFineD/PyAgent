# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\tesla-fleet-api\scripts\vehicle_data.py
#!/usr/bin/env python3
...
def fetch_vehicles(
    base_url: str, token: str, ca_cert: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def get_vehicles_cached(
    ...
    if (
        not force_refresh
        and vehicles
        and (time.time() - cached_at) < VEHICLE_CACHE_MAX_AGE
    ):
        return vehicles
...
def resolve_vehicle(
    ...
    if not identifier:
        if len(vehicles) == 1:
            ...
            for v in vehicles:
                print(
                    f"  - {v.get('display_name', 'Unknown')} ({v.get('vin')})",
                    file=sys.stderr,
                )
            sys.exit(1)
    ...
    matches = [
        v for v in vehicles if (v.get("display_name") or "").lower() == identifier_lower
    ]
    ...
    if len(matches) == 1:
        ...
        partial = [
            v
            for v in vehicles
            if identifier_lower in (v.get("display_name") or "").lower()
        ]
        if partial:
...
def format_charge_state(data: Dict[str, Any]) -> str:
    ...
    if state == "Charging":
        ...
        lines.append(
            f"  Power:      {power} kW ({amps}A × {volts}V × {calculated_phases}φ)"
        )
...
def format_climate_state(data: Dict[str, Any]) -> str:
    ...
    for i, name in enumerate(seat_names):
        ...
        level = data.get(
            f"seat_heater_{['left', 'right', 'rear_left', 'rear_center', 'rear_right'][i]}",
            data.get(
                f"seat_heater_{'driver' if i == 0 else 'passenger' if i == 1 else 'rear_left' if i == 2 else 'rear_center' if i == 3 else 'rear_right'}"
            ),
        )
...
def format_vehicle_state(data: Dict[str, Any]) -> str:
    ...
    doors = []
    if data.get("df"):
        doors.append("Driver front")
    if data.get("pf"):
        doors.append("Pass front")
    if data.get("dr"):
        doors.append("Driver rear")
    if data.get("pr"):
        doors.append("Pass rear")
    if data.get("ft"):
        doors.append("Frunk")
    if data.get("rt"):
        doors.append("Trunk")
    ...
    # Windows
    windows = []
    if data.get("fd_window"):
        windows.append("FD")
    if data.get("fp_window"):
        windows.append("FP")
    if data.get("rd_window"):
        windows.append("RD")
    if data.get("rp_window"):
        windows.append("RP")
    if windows:
...
""",
    ...
    parser.add_argument(
        "--dir",
        default=DEFAULT_DIR,
        help="Config directory (default: ~/.clawdbot/tesla-fleet-api)",
    )
    parser.add_argument(
        "--json", action="store_true", dest="raw_json", help="Output raw JSON"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="vehicle_state",
        help="Vehicle state (locks, doors, odometer)",
    )
    ...
    vin, display_name = resolve_vehicle(
        args.vehicle, dir_path, base_url, token, ca_cert
    )

    # Build endpoints list
    selected = []
    if args.charge:
        selected.append("charge_state")
    if args.climate:
        selected.append("climate_state")
    if args.drive:
        selected.append("drive_state")
    if args.location:
        selected.append("location_data")
    if args.vehicle_state:
        selected.append("vehicle_state")
    if args.gui:
        selected.append("gui_settings")
    if args.vehicle_config:
        selected.append("vehicle_config")
