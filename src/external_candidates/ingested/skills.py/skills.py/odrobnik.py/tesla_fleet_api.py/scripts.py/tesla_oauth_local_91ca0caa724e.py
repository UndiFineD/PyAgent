# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\tesla-fleet-api\scripts\tesla_oauth_local.py
#!/usr/bin/env python3
...
def build_authorize_url(
    *,
    ...
    state: str,
    locale: str,
    prompt_missing_scopes: bool = False,
) -> str:
    q = {
        "response_type": "code",
        "client_id": client_id,
...
def run_callback_server(
    host: str, port: int, expect_path: str, timeout_s: int
    ...
    while True:
        ...
        if getattr(httpd, "_done", False):
            ...
            if expect_path and path != expect_path:
                print(
                    f"Warning: callback path was {path}, expected {expect_path}",
                    file=sys.stderr,
                )
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-exchange",
        action="store_true",
        help="Only capture code; do not exchange for tokens",
    )
    ap.add_argument(
        "--prompt-missing-scopes",
        action="store_true",
        help="Prompt user to grant any missing scopes",
    )
    ...
    client_secret = (
        args.client_secret or _env("TESLA_CLIENT_SECRET") or cfg.get("client_secret")
    )

    redirect_uri = (
        args.redirect_uri
        or cfg.get("redirect_uri")
        or _env("TESLA_REDIRECT_URI")
        or DEFAULT_REDIRECT_URI
    )
    audience = (
        args.audience
        or cfg.get("audience")
        or _env("TESLA_AUDIENCE")
        or DEFAULT_AUDIENCE_EU
    )
    scope = (
        args.scope
        or _env("TESLA_SCOPE")
        or cfg.get("scope")
        or "openid offline_access vehicle_device_data vehicle_cmds vehicle_location"
    )

    if not client_id:
        print(
            "Missing client_id (set TESLA_CLIENT_ID in .env or pass --client-id)",
            file=sys.stderr,
        )
        return 2
    if not args.no_exchange and not client_secret:
        print(
            "Missing client_secret (set TESLA_CLIENT_SECRET in .env or pass --client-secret), or use --no-exchange.",
            file=sys.stderr,
        )
        return 2
