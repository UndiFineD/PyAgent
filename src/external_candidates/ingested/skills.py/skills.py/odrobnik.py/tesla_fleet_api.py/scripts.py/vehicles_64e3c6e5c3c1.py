# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\tesla-fleet-api\scripts\vehicles.py
#!/usr/bin/env python3
...
from store import get_auth, get_config, load_env_file, save_auth, save_vehicles
...
def main() -> int:
    ...
    try:
        ...
        if "401" in str(e) and _refresh_access_token(args.dir):
            ...
            token = auth.get("access_token")
            data = http_json(
                "GET", f"{base_url}/api/1/vehicles", token, ca_cert=ca_cert
            )
        else:
            raise
