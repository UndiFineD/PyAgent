# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\elevenlabs-skill\scripts\quota.py
#!/usr/bin/env python3
"""
...
""",
    ...
    parser.add_argument(
        "--usage", "-u", action="store_true", help="Include usage statistics"
    )
    parser.add_argument(
        "--days", "-d", type=int, default=30, help="Days of usage history (default: 30)"
    )
    parser.add_argument("--json", "-j", action="store_true", help="Output raw JSON")
    parser.add_argument(
        "--warn-pct", type=int, default=80, help="Warn if usage exceeds this percentage"
    )
    ...
    try:
        ...
        if args.json:
            ...
            if args.usage:
                ...
                start_ts_ms = int(
                    (datetime.now() - timedelta(days=args.days)).timestamp() * 1000
                )
                output["usage"] = get_usage_stats(
                    start_unix_ms=start_ts_ms, end_unix_ms=end_ts_ms
                )
        ...
        print(
            f"Characters: {format_characters(used)} / {format_characters(limit)} ({pct:.1f}%)"
        )
        ...
        print(
            f"Voices:    {voice_slots_used} / {voice_limit} (IVC: {'✓' if can_ivc else '✗'})"
        )
        if pro_voice_limit > 0:
            print(
                f"Pro Voice: {pro_voice_used} / {pro_voice_limit} (PVC: {'✓' if can_pvc else '✗'})"
            )
