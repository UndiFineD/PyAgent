# Extracted from: C:\DEV\PyAgent\.external\skills\skills\odrobnik\who-growth-charts-skill\scripts\growth_chart.py
#!/usr/bin/env python3
"""
...
WHO_DATA_FILES = {
    "height": {
        "F": {
            ...
            "5-19": (
                WHO_CDN_5_19,
                "height-for-age-(5-19-years)/hfa-girls-perc-who2007-exp.xlsx",
            ),
        },
        "M": {
            ...
            "5-19": (
                WHO_CDN_5_19,
                "height-for-age-(5-19-years)/hfa-boys-perc-who2007-exp.xlsx",
            ),
        },
    },
    "weight": {
    ...
    "bmi": {
        "F": {
            ...
            "5-19": (
                WHO_CDN_5_19,
                "bmi-for-age-(5-19-years)/bmi-girls-perc-who2007-exp.xlsx",
            ),
        },
        "M": {
            ...
            "5-19": (
                WHO_CDN_5_19,
                "bmi-for-age-(5-19-years)/bmi-boys-perc-who2007-exp.xlsx",
            ),
        },
    },
}
...
WHO_EMBLEM_URL = (
    "https://cdn.who.int/media/images/default-source/infographics/who-emblem.png"
)
...
def ensure_who_data(metric: str, sex: str, age_range: str) -> Path:
    ...
    # Local filename
    local_file = (
        CACHE_DIR / f"{sex.lower()}_{metric}_{age_range.replace('-', '_')}.xlsx"
    )
        ...
        for p in percentiles:
            ...
            for idx, row in df.iterrows():
                ...
                if len(available_cols) >= 2:
                    ...
                    corresponding_z = [
                        z_values[z_cols.index(col)] for col in available_cols
                    ]
    ...
    if len(ages) >= 4:
        try:
            ...
            f = interpolate.UnivariateSpline(
                ages_sorted, values_sorted, s=smoothing, k=min(3, len(ages) - 1)
            )
        except:
...
def plot_growth_chart(
    child_name, birthdate, sex, heights, weights, chart_type="height", output_dir=None
):
    """
    ...
    if chart_type == "height" and heights:
        ...
        for date_str, value in heights:
            ...
            age_months = (
                (date_dt.year - birthdate_dt.year) * 12
                + date_dt.month
                - birthdate_dt.month
            )
        ...
        for date_str, height in heights:
            ...
            day_key = dt.date()
            age_months = (
                (dt.year - birthdate_dt.year) * 12 + dt.month - birthdate_dt.month
            )
    ...
    if child_ages:
        ...
        if np.any(past_mask):
            ax.plot(
                smooth_ages[past_mask],
                smooth_values[past_mask],
                color="#cc0000",
                linewidth=2,
                zorder=4,
                label=f"{child_name} (trend)",
            )

        if np.any(future_mask) and np.any(past_mask):
            transition_idx = np.where(past_mask)[0][-1]
            future_ages = np.concatenate(
                [[smooth_ages[transition_idx]], smooth_ages[future_mask]]
            )
            future_vals = np.concatenate(
                [[smooth_values[transition_idx]], smooth_values[future_mask]]
            )
            ax.plot(
                future_ages,
                future_vals,
    ...
    ax.set_title(
        f"{child_name} - WHO {title_suffix} Chart", fontsize=14, fontweight="bold"
    )

    ax.xaxis.set_major_formatter(
        FuncFormatter(
            lambda x, pos: f"{int(x/12)}" if x / 12 == int(x / 12) else f"{x/12:.1f}"
        )
    )
    ...
    for month in range(0, int(max_age) + 1, 12):
        ax.axvline(
            month, color="gray", linewidth=1.2, alpha=0.4, linestyle="-", zorder=0
        )
...
def main():
    ...
    parser.add_argument(
        "--sex", "-s", choices=["F", "M"], default="F", help="Sex (F/M)"
    )
    parser.add_argument(
        "--type",
        "-t",
        choices=["height", "weight", "bmi", "all"],
        default="all",
        help="Chart type",
    )
    ...
    for ct in chart_types:
        ...
        output = plot_growth_chart(
            args.name, args.birthdate, args.sex, heights, weights, ct, args.output
        )
        print(f"✓ {output}")
