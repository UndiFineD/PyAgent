...
def main() -> int:
    ...
    ap.add_argument(
        "--quarantine-dir", default=str(Path.home() / ".openclaw" / "skills-quarantine")
    )
    ...
    for sp in sdirs:
        ...
        report["summary"]["bySeverity"][sev] = (
            report["summary"]["bySeverity"].get(sev, 0) + 1
        )
    ...
    high_found = any(
        SEVERITY_ORDER.get(r["severity"], 0) >= SEVERITY_ORDER["high"]
