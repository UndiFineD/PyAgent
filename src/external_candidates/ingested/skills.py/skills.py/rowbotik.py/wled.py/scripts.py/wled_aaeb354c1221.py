...
def wled_request(
    host: str, endpoint: str, method: str = "GET", data: Optional[Dict] = None
) -> Any:
...
def cmd_status(host: str) -> None:
    ...
    print(
        f"Palette: {seg.get('pal', 0)} - {info.get('palettes', [''])[seg.get('pal', 0)]}"
    )
...
def main():
    ...
    parser.add_argument(
        "--host", "-H", help="WLED device IP, hostname, or alias from config"
    )
    ...
    p_effect.add_argument(
        "--intensity", "-i", type=int, help="Effect intensity (0-255)"
    )
    ...
    if not host:
        parser.error(
            "--host required or set WLED_HOST env var or configure ~/.wled/config.json"
        )
