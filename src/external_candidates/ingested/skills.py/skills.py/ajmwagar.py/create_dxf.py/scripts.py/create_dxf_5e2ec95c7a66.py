# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ajmwagar\create-dxf\scripts\create_dxf.py
#!/usr/bin/env python3
...
def validate_plate(p: PlateSpec) -> None:
    ...
    def inside(x: float, y: float, margin: float) -> bool:
        return (-half_w + margin) <= x <= (half_w - margin) and (
            -half_h + margin
        ) <= y <= (half_h - margin)
    ...
    for s in p.slots:
        ...
        if not inside(s.x, s.y, r):
            raise ValueError(
                f"slot at ({s.x},{s.y}) may exit profile (conservative check)"
            )
...
def parse_polyline(spec: Dict[str, Any]) -> PolylineSpec:
    ...
    for pt in pts_in:
        ...
        pts.append(
            (float(_req(pt, "x", (int, float))), float(_req(pt, "y", (int, float))))
        )
    ...
    for i in range(len(d.etch_circles)):
        for j in range(i + 1, len(d.etch_circles)):
            ...
            if dist2 < (min_dist * min_dist):
                raise ValueError(
                    f"etch circles overlap: ({a.x},{a.y},d={a.diameter}) vs ({b2.x},{b2.y},d={b2.diameter})"
                )
...
def dxf_lwpolyline(
    layer: str, verts: List[Tuple[float, float, float]], closed: bool = True
) -> List[str]:
    # verts: [(x,y,bulge), ...]
    out = [
        "0",
...
def render_svg(p: PlateSpec, out_path: str) -> None:
    ...
    for hole in p.holes:
        ...
        r = hole.diameter / 2
        parts.append(
            f'<circle cx="{cx:.3f}" cy="{cy:.3f}" r="{r:.3f}" fill="none" stroke="black" stroke-width="0.3"/>'
        )
    ...
    for s in p.slots:
        ...
        parts.append(
            f"<path d=\"{' '.join(d)}\" fill=\"none\" stroke=\"black\" stroke-width=\"0.3\"/>"
        )
...
def render_svg_drawing(d: DrawingSpec, out_path: str) -> None:
    ...
    for c in d.etch_circles:
        ...
        r = c.diameter / 2
        parts.append(
            f'<circle cx="{cx:.3f}" cy="{cy:.3f}" r="{r:.3f}" fill="none" stroke="#111" stroke-width="0.8"/>'
        )
    ...
    for pl in d.etch_polylines:
        ...
        if pl.closed:
            dcmd.append("Z")
        parts.append(
            f"<path d=\"{' '.join(dcmd)}\" fill=\"none\" stroke=\"#111\" stroke-width=\"0.8\" stroke-linejoin=\"round\"/>"
        )
...
def render_dxf_drawing(d: DrawingSpec, out_path: str) -> None:
    ...
    for sp in d.etch_svg_paths:
        ...
        for pts in subpaths:
            verts = [
                ((x * sp.scale) + sp.x, (y * sp.scale) + sp.y, 0.0) for (x, y) in pts
            ]
