# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ajmwagar\render-stl-png\scripts\render_stl_png.py
#!/usr/bin/env python3
...
def render(
    ...
    for ia, ib, ic in faces:
        ...
        if all(x < -2 or x > 2 for x in (ax, bx, cx)) or all(
            y < -2 or y > 2 for y in (ay, by, cy)
        ):
            continue
