# Extracted from: C:\DEV\PyAgent\.external\skills\skills\ajmwagar\image-to-relief-stl\scripts\image_to_relief.py
#!/usr/bin/env python3
"""image_to_relief.py
...
def height_grid_palette(
    img: Image.Image, palette: Dict[Tuple[int, int, int], float]
) -> List[List[float]]:
...
def height_grid_grayscale(
    img: Image.Image, minh: float, maxh: float
) -> List[List[float]]:
    g = img.convert("L")
    px = g.load()
    w, h = g.size
