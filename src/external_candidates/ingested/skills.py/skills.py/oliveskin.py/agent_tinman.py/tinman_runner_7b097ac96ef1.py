# Extracted from: C:\DEV\PyAgent\.external\skills\skills\oliveskin\agent-tinman\tinman_runner.py
#!/usr/bin/env python3
...
# Tinman imports
try:
    from tinman import OperatingMode, Settings, Tinman, create_tinman
...
try:
    ...
    from tinman_openclaw_eval import Severity as EvalSeverity
...
def run_check(
    ...
    return CheckResult(
        ...
        ask_human=(
            _get_human_question(primary_category, tool_name, args_str)
            ...
            tool_span = Span(
                ...
                attributes={
                    ...
                    "tool.result_truncated": (
                        tc.get("result", "")[:500] if tc.get("result") else ""
                    ),
                },
    ...
    if args.command == "init":
        ...
        if args.level:
            ...
            if new_mode == SecurityMode.YOLO:
                ...
                print(
                    "Note: Low-risk actions will be auto-approved. S4 threats still blocked."
                )
        else:
