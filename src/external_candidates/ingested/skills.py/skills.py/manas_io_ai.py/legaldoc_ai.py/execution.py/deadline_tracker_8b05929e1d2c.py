# Extracted from: C:\DEV\PyAgent\.external\skills\skills\manas-io-ai\legaldoc-ai\execution\deadline_tracker.py
#!/usr/bin/env python3
"""
...
DEFAULT_DB_PATH = os.environ.get(
    "LEGALDOC_DEADLINES_DB", os.path.expanduser("~/.legaldoc/deadlines.db")
)
...
STATUTE_OF_LIMITATIONS = {
    ...
    "medical_malpractice": {
        "years": 1,
        "statute": "CCP § 340.5",
        "notes": "Discovery rule may apply",
    },
    ...
    "employment_discrimination": {
        ...
        "notes": "File with DFEH first",
    },
    "title_vii": {
        "days": 300,
        "statute": "42 USC § 2000e-5",
        "notes": "EEOC charge required",
    },
    "wage_claim": {
        "years": 3,
        "statute": "CCP § 338",
        "notes": "Labor Code § 203 penalties: 4 years",
    },
}
...
class DeadlineDatabase:
    ...
    def list_upcoming(
        self, days: int = 30, include_completed: bool = False
    ) -> list[Deadline]:
...
def extract_deadlines_from_document(file_path: str) -> list[Deadline]:
    ...
    date_patterns = [
        (
            r"(?:due|deadline|by|before|no later than)[:\s]+(\w+\s+\d{1,2},?\s+\d{4})",
            "deadline",
        ),
        (
            r"(?:hearing|trial|conference)[:\s]+(\w+\s+\d{1,2},?\s+\d{4})",
            "court_filing",
        ),
        (
            r"(?:response|reply|answer)\s+(?:due|deadline)[:\s]+(\w+\s+\d{1,2},?\s+\d{4})",
            "court_filing",
        ),
        (r"(?:discovery|deposition)[:\s]+.*?(\w+\s+\d{1,2},?\s+\d{4})", "discovery"),
        (
            r"(?:expire|expiration|terminate)[:\s]+.*?(\w+\s+\d{1,2},?\s+\d{4})",
            "contract_milestone",
        ),
...
def calculate_sol_deadline(
    incident_date: str, claim_type: str, jurisdiction: str = "CA"
) -> Optional[Deadline]:
    ...
    deadline_id = hashlib.md5(f"sol:{claim_type}:{incident_date}".encode()).hexdigest()[
        :12
    ]
...
def format_deadlines(
    ...
    upcoming = [
        d for d in deadlines if d.urgency_level() in ["urgent", "soon", "normal"]
    ]
    ...
    if critical:
        lines.extend(["## 🔴 CRITICAL (≤3 days)", ""])
        for d in critical:
            lines.append(
                f"- **{d.date}** ({d.days_until()} days) — {d.description[:80]}"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Track and manage legal deadlines")
    ...
    list_parser.add_argument("--matter", "-m", help="Filter by matter ID")
    list_parser.add_argument(
        "--overdue", action="store_true", help="Show only overdue deadlines"
    )
    list_parser.add_argument(
    ...
    add_parser.add_argument(
        "--date", "-d", required=True, help="Deadline date (YYYY-MM-DD)"
    )
    ...
    extract_parser = subparsers.add_parser(
        "extract", help="Extract deadlines from document"
    )
    extract_parser.add_argument("file_path", help="Path to document")
    extract_parser.add_argument(
        "--save", "-s", action="store_true", help="Save extracted deadlines"
    )
    extract_parser.add_argument(
        "--output", "-o", choices=["markdown", "json"], default="markdown"
    )
            ...
            if args.save:
                ...
                for d in deadlines:
                    if db.add(d):
                        saved += 1
                print(
                    f"✅ Saved {saved} of {len(deadlines)} deadlines", file=sys.stderr
                )
