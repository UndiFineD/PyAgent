    ...
    try:
        ...
        for item in data.get("results", [])[:10]:
            case = CaseResult(
                ...
                citation=(
                    item.get("citation", ["N/A"])[0] if item.get("citation") else "N/A"
                ),
...
def search_statutes(query: str, jurisdiction: str = "federal") -> list[StatuteResult]:
    ...
    if jurisdiction.lower() not in ["federal", "all"]:
        ...
        statutes = [
            s
            for s in statutes
            if jurisdiction_name in s.jurisdiction.upper()
            or s.jurisdiction == "Federal"
        ]
...
def generate_practice_notes(query: str, cases: list, statutes: list) -> list[str]:
    ...
    if "contract" in query_lower:
        ...
        notes.append(
            "💡 Check statute of limitations: typically 4-6 years for written contracts, 2-4 years for oral."
        )
    ...
    if "negligence" in query_lower:
        ...
        notes.append(
            "💡 Consider comparative/contributory negligence rules in your jurisdiction."
        )

    if "employment" in query_lower:
        notes.append(
            "💡 Check administrative exhaustion requirements (EEOC filing for Title VII claims)."
        )
        notes.append(
            "💡 Review arbitration agreements—many employment contracts require arbitration."
        )

    if "liability" in query_lower:
        notes.append(
            "💡 California courts increasingly pierce liability caps for willful/gross negligence breaches."
        )
...
def conduct_research(
    query: str, jurisdiction: str = "federal", research_type: str = "all"
) -> ResearchResult:
...
def main():
    ...
    parser.add_argument(
        "--list-jurisdictions", action="store_true", help="List available jurisdictions"
    )
