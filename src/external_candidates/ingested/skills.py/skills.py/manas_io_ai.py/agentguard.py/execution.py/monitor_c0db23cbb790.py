# Extracted from: C:\DEV\PyAgent\.external\skills\skills\manas-io-ai\agentguard\execution\monitor.py
#!/usr/bin/env python3
"""
...
class FileAccessMonitor:
    ...
    def start(self):
        ...
        if not HAS_WATCHDOG:
            print(
                "File monitoring unavailable - install watchdog: pip install watchdog"
            )
            return
    ...
    def get_hot_files(self, top_n: int = 10) -> List[tuple]:
        ...
        return sorted(self.access_counts.items(), key=lambda x: x[1], reverse=True)[
            :top_n
        ]
...
class APICallMonitor:
    ...
    def get_domain_stats(self) -> Dict[str, Dict]:
        ...
        for domain, count in self.domain_counts.items():
            stats[domain] = {"count": count, "trusted": self.is_trusted(domain)}
        return stats
    ...
    def status(self) -> Dict:
        ...
        return {
            ...
            "file_monitor": {
                ...
                "watched_dirs": [
                    str(d)
                    for d in (self.file_monitor.watch_dirs if self.file_monitor else [])
                ],
            },
            "api_monitor": {
                "active": self.api_monitor is not None,
                "trusted_domains": (
                    list(self.api_monitor.trusted_domains) if self.api_monitor else []
                ),
            },
        }
