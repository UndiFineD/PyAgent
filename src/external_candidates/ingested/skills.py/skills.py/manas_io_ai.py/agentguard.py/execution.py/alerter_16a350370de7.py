# Extracted from: C:\DEV\PyAgent\.external\skills\skills\manas-io-ai\agentguard\execution\alerter.py
#!/usr/bin/env python3
"""
...
""".strip()
            ...
            return (
                f"{icon} [{alert.severity.upper()}] {alert.title}: {alert.description}"
            )
    ...
    def dispatch_webhook(self, alert: Alert, webhook_url: str) -> bool:
        ...
        payload = {"source": "agentguard", "alert": asdict(alert)}
...
def create_alert(
    severity: str, title: str, description: str, details: Dict, recommendation: str
) -> Alert:
...
def main():
    ...
    parser.add_argument(
        "command", choices=["send", "list", "ack", "stats"], help="Command to execute"
    )
    parser.add_argument(
        "--severity",
    ...
    if args.command == "send":
        ...
        alert = create_alert(
            ...
            or "Review this alert and take appropriate action.",
        )

        dispatched = dispatcher.dispatch(alert)
        print(
            f"Alert {alert.id} dispatched to: {', '.join(dispatched) or 'none (filtered)'}"
        )
