# Extracted from: C:\DEV\PyAgent\.external\skills\skills\microck\catbox-upload\upload.py
#!/usr/bin/env python3
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload files to catbox.moe or litterbox.catbox.moe"
    )
    parser.add_argument("file", help="File to upload")
    parser.add_argument(
        "--service",
        choices=["catbox", "litterbox"],
        default="litterbox",
        help="Service to use (default: litterbox)",
    )
    parser.add_argument(
        "--time",
        choices=["1h", "12h", "24h", "72h"],
        default="24h",
        help="Litterbox expiration time (default: 24h)",
    )
