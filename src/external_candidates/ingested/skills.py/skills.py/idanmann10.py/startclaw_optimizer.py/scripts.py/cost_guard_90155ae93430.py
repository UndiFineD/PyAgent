...
class CostGuard:
    def __init__(
        self, state_file="~/.clawdbot/skills/openclaw-optimizer/state/cost-state.json"
    ):
    ...
    def ensure_state(self):
        ...
        if not self.state_file.exists():
            self.save_state(
                {"active_tasks": {}, "daily_total": 0.0, "date": self.today()}
            )
    ...
    def log_spend(
        self, task_id: str, input_tokens: int, output_tokens: int, model: str
    ) -> Tuple[str, Dict]:
        """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "  start <task_id> <budget_soft> <budget_max> <model> <est_input> <est_output>"
        )
