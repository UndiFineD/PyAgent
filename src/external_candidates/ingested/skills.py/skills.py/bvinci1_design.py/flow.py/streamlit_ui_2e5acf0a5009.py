# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bvinci1-design\flow\streamlit_ui.py
#!/usr/bin/env python3
"""
Flow - Streamlit UI
...
try:
    ...
    st.error(
        "Could not import Flow modules. Make sure all files are in the same directory."
    )
    st.stop()
...
page = st.sidebar.radio(
    "Navigation", ["🏠 Build Flow", "📚 Skill Registry", "ℹ️ About"]
)
...
if page == "🏠 Build Flow":
    st.markdown(
        '<div class="main-header">🔄 Flow - Intelligent Skill Orchestrator</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="sub-header">Compose natural language requests into secure, reusable workflows</div>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        build_button = st.button(
            "🚀 Build Flow", type="primary", use_container_width=True
        )
    ...
    if st.session_state.last_result:
        ...
        with col2:
            ...
            st.markdown(
                f'<div class="{security_color}" style="font-size: 0.875rem;">Security: {result.security_status}</div>',
                unsafe_allow_html=True,
            )
    ...
    st.markdown(
        '<div class="main-header">📚 Skill Registry</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div class="sub-header">Browse and search available skills</div>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        search_query = st.text_input(
            "🔍 Search skills", placeholder="Search by name or capability..."
        )
    ...
    try:
        ...
        if search_query:
            skills = [
                ...
                or any(
                    search_query.lower() in cap.lower()
        ...
        if skills:
            for skill in skills:
                with st.expander(
                    f"**{skill['name']}** - Score: {skill.get('reuse_score', 0)}"
                ):
                    st.markdown(
...
st.sidebar.markdown(
    "[GitHub](https://github.com/bvinci1-design/flow) | [ClawdHub](https://clawdhub.com)"
)
