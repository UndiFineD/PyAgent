# Extracted from: C:\DEV\PyAgent\.external\skills\skills\bvinci1-design\flow\flow.py
#!/usr/bin/env python3
"""
    ...
    def process(self, user_request: str) -> FlowResult:
        ...
        try:
            ...
            matching_skills = self.registry.find_skills(
                capabilities=intent.capabilities, tags=intent.tags
            )
            ...
            for skill in matching_skills:
                ...
                if scan_result.risk_level == "CRITICAL":
                    ...
                    if self.config["security_level"] == "strict":
                        errors.append(
                            f"BLOCKED: {skill.name} has high risk (strict mode)"
                        )
                        print(
                            f"      ✗ {skill.name}: BLOCKED (high risk in strict mode)"
                        )
                    else:
            ...
            flow_skill = self.composer.compose(
                skills=scanned_skills, intent=intent, name=intent.suggested_name
            )
...
def main():
    ...
    parser.add_argument(
        "request", nargs="?", help="Natural language request describing what to build"
    )
