...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../google-workspace"))
)
...
LOCAL_EXCEL_DIR = os.path.expanduser(
    _CONFIG.get("local_excel_dir", "~/vehicle_tracker")
)
...
def main():
    ...
    parser.add_argument(
        "--date", help="Date (YYYY-MM-DD)", default=str(datetime.date.today())
    )
    ...
    parser.add_argument(
        "--save-unit", action="store_true", help="Save unit for this category"
    )
    parser.add_argument(
            ...
            json.dumps(
                {
                    ...
                    "target": (
                        spreadsheet_id if mode == "google_sheet" else local_excel_path
                    ),
                    "row_data": row_data,
        ...
        try:
            if not os.path.exists(local_excel_path):
                if os.path.exists(TEMPLATE_PATH):
                    ...
                    print(
                        msg(
                            locale, "success_excel_created_blank", path=local_excel_path
                        )
                    )
