# Extracted from: C:\DEV\PyAgent\.external\skills\skills\qrucio\monkeytype-tracker\scripts\monkeytype_stats.py
#!/usr/bin/env python3
"""
...
WORKSPACE_CONFIG = (
    Path.home() / ".openclaw" / "workspace" / "config" / "monkeytype.json"
)
    ...
    try:
        ...
        if r.status_code == 471:
            ...
            if "inactive" in data.get("message", "").lower():
                print(
                    "ERROR_INACTIVE_KEY: Your API key is inactive. Go to Monkeytype -> Account Settings -> Ape Keys and check the checkbox next to your key to activate it."
                )
                sys.exit(2)
            else:
            ...
            print(
                "ERROR_UNAUTHORIZED: Your API key is invalid. Please set up a new key."
            )
            sys.exit(3)
        ...
        print(
            "ERROR_NETWORK: Couldn't reach Monkeytype servers. Check your connection."
        )
        sys.exit(5)
...
def cmd_stats(config: dict, args):
    ...
    for duration in ["15", "30", "60", "120"]:
        try:
            data = api_get(
                "/users/personalBests", config, {"mode": "time", "mode2": duration}
            ).get("data", [])
            if data:
                best = max(data, key=lambda x: x.get("wpm", 0))
                print(
                    f"  {duration:>3}s: {best.get('wpm', 0):.0f} WPM | {best.get('acc', 0):.1f}% acc"
                )
        except:
...
def cmd_history(config: dict, args):
    ...
    try:
        ...
        if len(wpms) > 1:
            ...
            print(
                f"StdDev: {std:.1f} {'(inconsistent!)' if std > 15 else '(good)' if std < 10 else ''}"
            )
        ...
        print("-" * 45)
        for r in data[:20]:
            ts = datetime.fromtimestamp(r.get("timestamp", 0) / 1000).strftime(
                "%m/%d %H:%M"
            )
            mode = f"{r.get('mode', '?')}-{r.get('mode2', '?')}"
            print(
                f"{ts:12} {mode:10} {r.get('wpm', 0):>6.0f} {r.get('acc', 0):>5.1f}% {r.get('consistency', 0):>5.1f}%"
            )
...
def cmd_compare(config: dict, args):
    ...
    try:
        ...
        print(
            f"{'Avg Accuracy':<15} {tw_acc:>11.1f}% {lw_acc:>11.1f}% {acc_diff:>+9.1f}%"
        )

        print(
            f"{'Tests':<15} {len(this_week):>12} {len(last_week):>12} {len(this_week)-len(last_week):>+10}"
        )
...
def cmd_leaderboard(config: dict, args):
    ...
    try:
        ...
        for i, entry in enumerate(data, 1):
            print(
                f"{i:>4} {entry.get('name', '?'):<20} {entry.get('wpm', 0):>8.1f} {entry.get('acc', 0):>7.1f}%"
            )
...
def main():
    ...
    if not config.get("apeKey"):
        ...
        print(
            "SETUP_HINT: Ask user for their ApeKey from Monkeytype -> Account Settings -> Ape Keys"
        )
        return
