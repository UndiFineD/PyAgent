...
class ProxyPool:
    ...
    def get_proxy(
        ...
        if exclude_failed:
            ...
            candidates = [
                p
                for p in candidates
                if p.success_count + p.fail_count < 3
                or p.fail_count / (p.success_count + p.fail_count) < 0.5
            ]
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "by_type": {
                "residential": len(
                    [p for p in self.proxies if p.type == "residential"]
                ),
...
def test_proxy(
    proxy_url: str, test_url: str = "https://httpbin.org/ip", timeout: int = 10
) -> Dict:
    """
