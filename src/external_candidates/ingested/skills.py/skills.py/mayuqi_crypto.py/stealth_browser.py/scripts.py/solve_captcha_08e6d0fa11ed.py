...
def solve_recaptcha_v2(
    site_key: str, page_url: str, invisible: bool = False, provider: str = None
) -> str:
    """
    Solve reCAPTCHA v2
    ...
    for p in providers:
        if p == "2captcha" and keys.get("2captcha"):
            return _solve_2captcha_recaptcha(
                keys["2captcha"], site_key, page_url, invisible
            )
        elif p == "anticaptcha" and keys.get("anticaptcha"):
            return _solve_anticaptcha_recaptcha(
                keys["anticaptcha"], site_key, page_url, invisible
            )
        elif p == "capsolver" and keys.get("capsolver"):
            return _solve_capsolver_recaptcha(
                keys["capsolver"], site_key, page_url, invisible
            )

    raise ValueError(
        "No CAPTCHA API keys configured. Add keys to ~/.clawdbot/secrets/captcha.json"
    )
...
def _poll_capsolver(api_key, task_id, max_attempts=60):
    for _ in range(max_attempts):
        ...
        if result.get("status") == "ready":
            return result["solution"].get("gRecaptchaResponse") or result[
                "solution"
            ].get("token")
        elif result.get("errorId"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action", "-a", default="verify", help="Action (reCAPTCHA v3)"
    )
    ...
    try:
        if args.type == "recaptcha2":
            ...
            token = solve_recaptcha_v3(
                args.sitekey, args.url, args.action, provider=args.provider
            )
