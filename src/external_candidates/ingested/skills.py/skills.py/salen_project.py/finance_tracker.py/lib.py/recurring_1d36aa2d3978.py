# Extracted from: C:\DEV\PyAgent\.external\skills\skills\salen-project\finance-tracker\lib\recurring.py
"""
...
try:
    from .categories import detect_category, get_emoji
...
class RecurringManager:
    ...
    def _init_file(self):
        if not self.recurring_file.exists():
            self._save({"recurring": [], "created_at": datetime.now().isoformat()})
    ...
    def remove_recurring(self, id_or_name: str) -> bool:
        ...
        for item in data["recurring"]:
            if (
                str(item["id"]) == str(id_or_name)
                or item["description"].lower() == id_or_name.lower()
            ):
                item["active"] = False
                self._save(data)
                return True
    ...
    def log_recurring(self, recurring_id: int) -> Dict[str, Any]:
        ...
        for item in data["recurring"]:
            if item["id"] == recurring_id:
                ...
                item["next_due"] = self._calculate_next_due(
                    item["frequency"], item["day"]
                )
                self._save(data)
    ...
    def get_report(self) -> str:
        ...
        for freq in freq_order:
            ...
            for item in by_freq[freq]:
                ...
                lines.append(
                    f"   {emoji} {item['amount']:,} — {item['description']} (next: {next_due})"
                )
            lines.append("")
