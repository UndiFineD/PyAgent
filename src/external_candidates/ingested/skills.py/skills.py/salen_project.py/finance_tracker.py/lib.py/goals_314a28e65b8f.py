# Extracted from: C:\DEV\PyAgent\.external\skills\skills\salen-project\finance-tracker\lib\goals.py
"""
...
class GoalsManager:
    ...
    def _init_file(self):
        if not self.goals_file.exists():
            self._save({"goals": [], "created_at": datetime.now().isoformat()})
    ...
    def add_goal(
        ...
        # Check for duplicate
        existing = next(
            (g for g in data["goals"] if g["name"].lower() == name.lower()), None
        )
        if existing:
            # Update existing
    ...
    def update_goal(self, id_or_name: str, amount: int) -> Optional[Dict[str, Any]]:
        ...
        for goal in data["goals"]:
            if (
                str(goal["id"]) == str(id_or_name)
                or goal["name"].lower() == id_or_name.lower()
            ):
    ...
    def remove_goal(self, id_or_name: str) -> bool:
        ...
        data["goals"] = [
            g
            for g in data["goals"]
            if str(g["id"]) != str(id_or_name)
            and g["name"].lower() != id_or_name.lower()
        ]
        ...
        if active:
            ...
            for goal in active:
                ...
                if progress.get("overdue"):
                    ...
                    lines.append(
                        f"   📅 {progress['days_left']} days left — need {progress['daily_required']:,}/day"
                    )
