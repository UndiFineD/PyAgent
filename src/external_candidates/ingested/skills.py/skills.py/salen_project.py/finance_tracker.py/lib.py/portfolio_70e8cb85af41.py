# Extracted from: C:\DEV\PyAgent\.external\skills\skills\salen-project\finance-tracker\lib\portfolio.py
"""
...
class Portfolio:
    ...
    def add_asset(
        self, name: str, value: int, asset_type: str = "other"
    ) -> Dict[str, Any]:
        ...
        existing = next(
            (a for a in data["assets"] if a["name"].lower() == name.lower()), None
        )
    ...
    def remove_asset(self, name: str) -> bool:
        ...
        data["assets"] = [
            a for a in data["assets"] if a["name"].lower() != name.lower()
        ]
    ...
    def get_portfolio_report(self) -> str:
        ...
        for asset_type, items in by_type.items():
            ...
            lines.append(
                f"{type_info['emoji']} {type_info['name']}: {type_total:,} {currency} ({pct:.1f}%)"
            )
            for item in items:
    ...
    def add_income(
        self, amount: int, description: str, income_type: str = "other"
    ) -> Dict[str, Any]:
        """Log income."""
    ...
    def get_income_report(self, days: int = 30) -> str:
        ...
        for income_type, data in sorted(
            by_type.items(), key=lambda x: x[1]["amount"], reverse=True
        ):
            type_info = self.INCOME_TYPES.get(income_type, self.INCOME_TYPES["other"])
            pct = (data["amount"] / total * 100) if total > 0 else 0
            lines.append(
                f"{type_info['emoji']} {type_info['name']}: {data['amount']:,} {currency} ({pct:.1f}%)"
            )
