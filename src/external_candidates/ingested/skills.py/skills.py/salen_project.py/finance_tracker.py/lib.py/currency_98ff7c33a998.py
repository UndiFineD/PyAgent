...
class CurrencyConverter:
    ...
    def _fetch_rates(self) -> Dict[str, float]:
        ...
        try:
            ...
            req = urllib.request.Request(
                url, headers={"User-Agent": "FinanceTracker/1.0"}
            )
    ...
    def convert(
        ...
        from_rate = rates.get(
            from_currency, self.FALLBACK_RATES.get(from_currency, 1.0)
        )
    ...
    def parse_amount(
        self, amount_str: str, default_currency: str = "UZS"
    ) -> Tuple[int, str]:
        """
    ...
    def get_rates_report(self) -> str:
        ...
        if self.rates_file.exists():
            try:
                ...
                lines.append("")
                lines.append(
                    f"⏰ Updated: {age.seconds // 3600}h {(age.seconds % 3600) // 60}m ago"
                )
            except:
                pass
