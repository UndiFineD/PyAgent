...
def main():
    ...
    parser.add_argument(
        "stations", nargs="*", help="ICAO station IDs (e.g., KLAX KSMO)"
    )
    parser.add_argument(
        "--metar", "-m", action="store_true", help="Fetch METAR (default if no flags)"
    )
    parser.add_argument("--taf", "-t", action="store_true", help="Fetch TAF")
    parser.add_argument(
        "--pirep", "-p", action="store_true", help="Fetch PIREPs (requires --lat/--lon)"
    )
    ...
    parser.add_argument(
        "--hours", type=int, default=2, help="Hours of METAR history (default: 2)"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show raw observations"
    )
