# Extracted from: C:\DEV\PyAgent\.external\skills\skills\c4chuan\csdn-publisher\scripts\login.py
#!/usr/bin/env python3
"""
CSDN 扫码登录 - 获取 Cookie
...
DEFAULT_CONFIG_FILE = (
    Path.home() / ".openclaw/workspace/credentials/telegram-notify.json"
)
...
def send_telegram_notification(message: str, config_file: str = None):
    ...
    try:
        ...
        if not bot_token or not chat_id:
            print(
                "⚠️ Telegram 配置不完整（需要 bot_token 和 chat_id）", file=sys.stderr
            )
            return False
...
async def csdn_login(
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not qr_captured:
                login_box = await page.query_selector(
                    '.main-login, .login-box, [class*="login"]'
                )
                if login_box:
        ...
        if not logged_in:
            ...
            return {
                "success": False,
                "qr_path": str(qr_path) if qr_captured else None,
                "message": "登录超时",
            }
        ...
        if notify:
            send_telegram_notification(
                "✅ CSDN 登录成功！Cookie 已保存，可以继续发布文章了。", config_file
            )
...
def main():
    ...
    login_parser.add_argument(
        "--no-headless", action="store_true", help="显示浏览器窗口"
    )
    login_parser.add_argument(
        "--timeout", "-t", type=int, default=120, help="登录超时时间（秒）"
    )
    login_parser.add_argument(
        "--notify", "-n", action="store_true", help="登录成功后发送 Telegram 通知"
    )
    ...
    if args.command == "login":
        ...
        if send_telegram_notification(
            "🔔 CSDN Publisher 通知已配置成功！", str(config_path)
        ):
            print("✅ 测试消息发送成功")
        else:
