...
def parse_frontmatter(skill_md: str) -> Tuple[Dict[str, str], List[str]]:
    ...
    if not fm:
        errors.append(
            "SKILL.md frontmatter is empty. It must include name and description."
        )
        return data, errors
    ...
    for idx, line in enumerate(fm.splitlines(), start=1):
        ...
        if ":" not in raw:
            errors.append(
                f"Frontmatter line {idx} is not a valid 'key: value' pair: {line!r}"
            )
            continue
...
def validate_skill_dir(root: Path) -> Tuple[List[str], List[str]]:
    ...
    for bad in DISALLOWED_TOP_LEVEL_FILES:
        if (root / bad).exists():
            warnings.append(
                f"Top-level file '{bad}' is discouraged for skills (remove it)."
            )
    ...
    if "description" in fm_data:
        if len(fm_data["description"].strip()) < 40:
            warnings.append(
                "Frontmatter description seems short; include more trigger context."
            )
