...
def api_request(method: str, endpoint: str, params: Dict = None) -> Dict:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"API error ({e.code}): {error_json.get('detail', error_body)}"
            )
...
def sync_trades(dry_run: bool = False) -> Dict[str, Any]:
    ...
    print(
        f"  API returned {len(api_trades)} trades (total available: {total_available})"
    )
    ...
    if dry_run:
        ...
        for t in new_trades[:5]:
            print(
                f"    - {t['market_question'][:50]}... ({t['side']} @ ${t['cost']:.2f})"
            )
        if len(new_trades) > 5:
...
def sync_outcomes(dry_run: bool = False) -> Dict[str, Any]:
    ...
    for trade in trades:
        ...
        print(
            f"  {result_emoji} {trade['market_question'][:40]}... "
...
def show_history(n: int = 10):
    ...
    for i, t in enumerate(trades, 1):
        ...
        print(
            f"   {shares:.1f} shares @ ${cost:.2f} | {created} | {result}{pnl_str}{source_str}"
        )
        print()
...
def log_trade(
    trade_id: str, source: str, thesis: str = None, confidence: float = None, **extra
):
    """
...
def main():
    ...
    parser.add_argument(
        "--sync", action="store_true", help="Sync trades from Simmer API"
    )
    parser.add_argument(
        "--history",
        type=int,
