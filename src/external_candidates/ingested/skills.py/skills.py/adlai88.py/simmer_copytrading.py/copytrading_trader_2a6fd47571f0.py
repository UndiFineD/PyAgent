...
MAX_TRADES_PER_RUN = int(
    os.environ.get("SIMMER_COPYTRADING_MAX_TRADES", "10")
)  # Max trades per scan cycle
...
def print_config():
    ...
    print(f"\nSettings:")
    print(
        f"  Top N: {config['top_n'] if config['top_n'] else 'auto (based on balance)'}"
    )
...
def remove_risk_monitor(market_id: str, side: str) -> dict:
    ...
    try:
        return api_request(
            "DELETE", f"/api/sdk/positions/{market_id}/monitor?side={side}"
        )
    except Exception as e:
        return {"error": str(e)}
...
def run_copytrading(
    ...
    print(
        f"  Mode: {'Buy only (accumulate)' if buy_only else 'Full rebalance (buy + sell)'}"
    )
    print(
        f"  Whale exits: {'Enabled (sell when whale exits)' if detect_whale_exits else 'Disabled'}"
    )
    ...
    if trades:
        ...
        for t in trades:
            ...
            print(
                f"  {status} {action} {shares:.1f} {side} @ ${price:.3f} (${cost:.2f})"
            )
            print(f"     {title}...")
    ...
    if not result.get("success"):
        ...
        for t in trades:
            if t.get("success") and t.get("action") == "buy":
                ...
                side = t.get("side", "yes")
                if market_id:
                    risk_result = set_risk_monitor(
                        market_id, side, stop_loss_pct=0.25, take_profit_pct=0.50
                    )
        ...
        if risk_monitors_set > 0:
            print(
                f"🛡️  Risk monitors set: {risk_monitors_set} positions (SL -25% / TP +50%)"
            )
        if risk_monitor_errors > 0:
            print(
                f"⚠️  Risk monitor errors: {risk_monitor_errors} (check API key/permissions)"
            )
    else:
        print("\n✅ Scan complete")
...
def show_positions():
    ...
    try:
        ...
        for i, pos in enumerate(poly_positions, 1):
            ...
            pnl = pos.get("pnl", 0)
            pnl_pct = (
                (pnl / pos.get("cost_basis", 1)) * 100 if pos.get("cost_basis") else 0
            )
            ...
            print(
                f"   Value: ${value:.2f} | P&L: {pnl_color}${pnl:.2f} ({pnl_color}{pnl_pct:.1f}%)"
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would trade without executing"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-usd", type=float, help="Max USD per position (overrides env var)"
    )
    parser.add_argument(
        "--rebalance",
        action="store_true",
