# Extracted from: C:\DEV\PyAgent\.external\skills\skills\purch-agent\agentic-commerce\scripts\buy_and_sign.py
#!/usr/bin/env python3
"""
...
def parse_address(address_str: str) -> dict:
    ...
    if len(parts) < 6:
        raise ValueError(
            "Address must have at least: Name,Line1,City,State,PostalCode,Country"
        )
...
def main():
    ...
    parser.add_argument(
        "--private-key", required=True, help="Base58 private key for signing"
    )
    ...
    print(
        f"   Total: {total_price.get('amount', 'N/A')} {total_price.get('currency', 'USDC').upper()}"
    )
    print()
    ...
    if "error" in tx_result:
        ...
        print()
        print(
            f"🌐 You can complete payment via browser: {order_result.get('checkoutUrl')}"
        )
        sys.exit(1)
