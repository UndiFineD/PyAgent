...
def _generate_character_prompt(
    ...
    character_data = {
        ...
        "equipment": (
            equipment.split(", ")
            if equipment
            else (_default_equipment_list(char_class) if char_class else [])
        ),
...
def _default_equipment(char_class: str) -> str:
    ...
    if (
        "warrior" in class_lower
        or "fighter" in class_lower
        or "barbarian" in class_lower
    ):
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument(
        "--race", action="append", default=None, help="Character race (repeatable)"
    )
    p.add_argument(
        "--class",
        action="append",
    ...
    p.add_argument(
        "--count", type=int, default=1, help="Number of variants per character"
    )
    p.add_argument(
        "--sheet", action="store_true", help="Generate character sheet with stats"
    )
    ...
    if character_sheets:
        with open(
            os.path.join(out_dir, "character_sheet.json"), "w", encoding="utf-8"
        ) as f:
