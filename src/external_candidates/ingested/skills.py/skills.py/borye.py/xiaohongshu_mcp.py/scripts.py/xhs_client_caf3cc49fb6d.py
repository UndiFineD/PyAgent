...
def check_status():
    ...
    try:
        ...
        print(
            "❌ Cannot connect to MCP server. Make sure xiaohongshu-mcp is running on localhost:18060"
        )
        sys.exit(1)
...
def search_notes(keyword, sort_by="综合", note_type="不限", publish_time="不限"):
    ...
    try:
        ...
        resp = requests.post(
            f"{BASE_URL}/api/v1/feeds/search", json=payload, timeout=TIMEOUT
        )
        data = resp.json()
...
def get_note_detail(feed_id, xsec_token, load_comments=False):
    ...
    try:
        ...
        resp = requests.post(
            f"{BASE_URL}/api/v1/feeds/detail", json=payload, timeout=TIMEOUT
        )
        data = resp.json()
        ...
        if data.get("success"):
            ...
            if comment_list:
                ...
                for c in comment_list[:5]:
                    ...
                    print(
                        f"  - {user_info.get('nickname', 'Anonymous')}: {c.get('content', '')}"
                    )
        else:
...
def main():
    ...
    search_parser.add_argument(
        "--type", default="不限", choices=["不限", "视频", "图文"], help="Note type"
    )
    search_parser.add_argument(
        "--time",
        default="不限",
    ...
    detail_parser.add_argument(
        "--comments", action="store_true", help="Load all comments"
    )
