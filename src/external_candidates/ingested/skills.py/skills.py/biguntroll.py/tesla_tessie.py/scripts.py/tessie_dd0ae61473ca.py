...
def main():
    ...
    try:
        if action == "vehicles":
            ...
            if not args.value:
                print(
                    "Error: --value required (temperature in Celsius)", file=sys.stderr
                )
                sys.exit(1)
            result = invoke_command(
                api_key, args.vin, "set_temperature", {"temperature": int(args.value)}
            )
            ...
            if not args.value:
                ...
                sys.exit(1)
            result = invoke_command(
                api_key, args.vin, "set_charge_limit", {"percent": int(args.value)}
            )
