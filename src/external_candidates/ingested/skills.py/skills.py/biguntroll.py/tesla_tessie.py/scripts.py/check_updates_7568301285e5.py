...
def main():
    ...
    parser.add_argument(
        "--vin", help="Vehicle VIN (optional, uses first vehicle if not specified)"
    )
    ...
    try:
        ...
        if update and update.get("status"):
            ...
            if status == "available":
                print(
                    f"UPDATE_AVAILABLE: Software update {version} is ready to install!"
                )
            elif status == "downloading":
                download_perc = update.get("download_perc", 0)
                print(
                    f"UPDATE_DOWNLOADING: Downloading update {version} ({download_perc}%)"
                )
            elif status == "installing":
                install_perc = update.get("install_perc", 0)
                print(
                    f"UPDATE_INSTALLING: Installing update {version} ({install_perc}%)"
                )
