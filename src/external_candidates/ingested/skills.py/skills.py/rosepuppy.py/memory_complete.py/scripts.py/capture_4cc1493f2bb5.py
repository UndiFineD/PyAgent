# Extracted from: C:\DEV\PyAgent\.external\skills\skills\rosepuppy\memory-complete\scripts\capture.py
#!/usr/bin/env python3
"""
...
def extract_facts_simple(text):
    ...
    for sentence in sentences:
        ...
        if any(
            kw in lower for kw in ["decided", "agreed", "decision", "chose", "choice"]
        ):
            ...
            kw in lower for kw in ["learned", "realized", "discovered", "found out"]
        ):
...
def main():
    ...
    parser.add_argument(
        "--raw", action="store_true", help="Store raw text without extraction"
    )
