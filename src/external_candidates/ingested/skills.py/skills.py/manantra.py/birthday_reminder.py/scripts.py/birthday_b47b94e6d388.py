...
def add_birthday(name, date_str, year_born=None):
    ...
    print(
        f"✅ Geburtstag gespeichert: {name} - {date.day:02d}.{date.month:02d}."
        + (f"{year_born}" if year_born else "")
    )
    return True
...
def list_birthdays(upcoming_only=False, days=30):
    ...
    for item in items:
        ...
        if item["days_left"] == 0:
            ...
            print(
                f"📅 {item['name']} - {item['date']}{age_str} - in {item['days_left']} Tagen"
            )
...
def main():
    ...
    add_parser.add_argument(
        "--year-born", type=int, help="Year of birth (for age calculation)"
    )

    # List command
    list_parser = subparsers.add_parser("list", help="List birthdays")
    list_parser.add_argument(
        "--upcoming", action="store_true", help="Only show upcoming birthdays"
    )
    ...
    if args.command == "add":
        ...
        for r in reminders:
            ...
            if r["days_left"] == 0:
                ...
                print(
                    f"📅 {r['name']} hat in {r['days_left']} Tagen Geburtstag{age_str}!"
                )
    else:
        parser.print_help()
