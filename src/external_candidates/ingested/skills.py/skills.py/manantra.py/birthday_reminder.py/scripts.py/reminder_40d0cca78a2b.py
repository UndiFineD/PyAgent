# Extracted from: C:\DEV\PyAgent\.external\skills\skills\manantra\birthday-reminder\scripts\reminder.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    for r in reminders:
        ...
        if r["days_left"] == 0:
            print(
                f"🎉 Geburtstagserinnerung: **{r['name']}** hat HEUTE Geburtstag{age_str}!"
            )
        elif r["days_left"] == 1:
            print(
                f"🎈 Geburtstagserinnerung: **{r['name']}** hat morgen Geburtstag{age_str}!"
            )
        elif r["days_left"] == 7:
            print(
                f"📅 Geburtstagserinnerung: **{r['name']}** hat in einer Woche Geburtstag{age_str} ({r['date']})!"
            )
