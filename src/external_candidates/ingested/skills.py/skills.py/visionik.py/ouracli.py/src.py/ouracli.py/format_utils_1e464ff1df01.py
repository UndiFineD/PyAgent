# Extracted from: C:\DEV\PyAgent\.external\skills\skills\visionik\ouracli\src\ouracli\format_utils.py
...
def sort_dict_keys(d: dict[str, Any]) -> list[str]:
    ...
    priority_keys.sort(
        key=lambda k: FIELD_ORDER.index(k) if k in FIELD_ORDER else len(FIELD_ORDER)
    )

    # Sort end keys by their order in FIELD_ORDER_END
    end_keys.sort(
        key=lambda k: (
            FIELD_ORDER_END.index(k) if k in FIELD_ORDER_END else len(FIELD_ORDER_END)
        )
    )
