# Extracted from: C:\DEV\PyAgent\.external\skills\skills\guang384\aliyun-tts\aliyun_tts.py
#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
"""
...
if not ALIYUN_APP_KEY or not ALIYUN_ACCESS_KEY_ID or not ALIYUN_ACCESS_KEY_SECRET:
    print(
        "请设置环境变量: ALIYUN_APP_KEY, ALIYUN_ACCESS_KEY_ID, ALIYUN_ACCESS_KEY_SECRET",
        file=sys.stderr,
    )
    sys.exit(1)
...
def get_token():
    ...
    # 3. 构造待签名字符串
    string_to_sign = (
        "GET" + "&" + percent_encode("/") + "&" + percent_encode(query_string)
    )
    ...
    # 5. 发送请求
    full_url = (
        f"http://nls-meta.{REGION}.aliyuncs.com/?Signature={signature}&{query_string}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", default="/tmp/tts_output.mp3", help="输出文件路径"
    )
