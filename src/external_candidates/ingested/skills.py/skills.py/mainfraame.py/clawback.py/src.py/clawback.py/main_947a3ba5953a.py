...
class TradingBot:
    ...
    def __init__(self, config_path="config/config.json"):
        ...
        self.schedule_timezone = ZoneInfo(
            schedule_config.get("timezone", "America/New_York")
        )
        self.disclosure_check_times = schedule_config.get(
        ...
        self.logger.info(
            f"Disclosure checks scheduled at: {', '.join(self.disclosure_check_times)} ET"
        )
    ...
    def _handle_broker_error(self, operation: str, error: str, details: dict = None):
        ...
        self.notifier.send_broker_error(
            operation, error, details, broker_name=self.broker.BROKER_NAME
        )
    ...
    def _load_saved_tokens(self):
        ...
        if db_tokens and db_tokens.get("access_token"):
            self.broker.set_tokens(
                db_tokens["access_token"], db_tokens.get("access_secret")
            )
        ...
        if os.path.exists(token_file):
            try:
                ...
                if tokens.get("access_token"):
                    self.broker.set_tokens(
                        tokens["access_token"], tokens.get("access_secret")
                    )
                    # Migrate to database
                    self._save_tokens_to_db()
                    self.logger.info(
                        "Loaded and migrated access tokens from file to database"
                    )
    ...
    def check_for_new_disclosures(self):
        ...
        try:
            ...
            self.notifier.send_disclosure_error(
                source="Disclosure Check", error_message=error_msg
            )
            import traceback
    ...
    def execute_pending_trades(self):
        ...
        self.logger.info(
            f"Market open. Executing {len(self.pending_trades)} pending trades..."
        )

        # Process and execute trades
        executed_trades = self.trade_engine.process_congressional_trades(
            self.pending_trades
        )
    ...
    def run_scheduled(self):
        ...
        self.logger.info(
            f"Disclosure checks at: {', '.join(self.disclosure_check_times)} ET"
        )
        self.logger.info(
        ...
        for check_time in self.disclosure_check_times:
            ...
            self.logger.info(
                f"  Scheduled disclosure check at {local_time} local ({check_time} ET)"
            )
        ...
        self.logger.info(
            f"  Scheduled market open execution at {market_open_local} local (09:35 ET)"
        )
    ...
    def interactive_mode(self):
        ...
        while True:
            ...
            print("  ─────────── Automation ───────────")
            print(
                f" 11. Start scheduled trading (checks at {', '.join(self.disclosure_check_times)} ET)"
            )
