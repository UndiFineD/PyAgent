# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\trade_engine.py
"""
    ...
    def update_price(self, current_price: float):
        ...
        if current_price > self.highest_price:
            ...
            if self.trailing_stop_active:
                self.stop_loss_price = self.highest_price * (
...
class TradeEngine:
    ...
    def __init__(self, broker_client, config):
        ...
        self.trade_scale = Decimal(
            str(trading_config.get("tradeScalePercentage", 0.05))
        )
        self.max_position_pct = Decimal(
            str(trading_config.get("maxPositionPercentage", 0.05))
        )
        ...
        self.position_stop_loss = Decimal(
            str(risk_config.get("positionStopLoss", 0.08))
        )
        self.trailing_stop_activation = Decimal(
            str(risk_config.get("trailingStopActivation", 0.10))
        )
        self.trailing_stop_percent = Decimal(
            str(risk_config.get("trailingStopPercent", 0.05))
        )
        ...
        self.market_timezone = ZoneInfo(
            config["trading"].get("marketTimezone", "America/New_York")
        )
        self.market_open = self._parse_time(
            config["trading"].get("marketOpen", "09:30")
        )
        self.market_close = self._parse_time(
            config["trading"].get("marketClose", "16:00")
        )
    ...
    def calculate_scaled_trade(self, congress_trade, account_balance):
        ...
        try:
            ...
            action = (
                "BUY" if congress_trade["transaction_type"] == "purchase" else "SELL"
            )
            ...
            quantity = (target_amount / current_price).quantize(
            ...
            if quantity < 1:
                logger.info(
                    f"Target amount too small for {symbol}: ${target_amount:.2f} at ${current_price:.2f}"
                )
                return None
            ...
            logger.info(
                f"Calculated trade: {action} {quantity} shares of {symbol} at ~${current_price:.2f} (${estimated_cost:.2f})"
            )
    ...
    def check_position_limits(self, symbol, quantity, action, account_value):
        ...
        try:
            ...
            if action == "SELL" and quantity > current_position:
                logger.warning(
                    f"Attempting to sell {quantity} shares but only own {current_position}"
                )
                return False
    ...
    def execute_trade(self, trade_calculation):
        ...
        try:
            ...
            if not self.broker.account_id:
                ...
                return False
            success = self.broker.place_order(self.broker.account_id, order_details)
            ...
            if success:
                ...
                logger.info(
                    f"Trade executed successfully: {trade_calculation['action']} "
                ...
                if trade_calculation["action"] == "BUY":
                    self.add_position(
                        ...
                        congressional_trade_id=trade_calculation.get(
                            "congressional_trade_id"
                        ),
                    )
    ...
    def process_congressional_trades(self, congress_trades):
        ...
        try:
            ...
            logger.info(
                f"Processing trades with account balance: ${account_balance:.2f}"
            )
            ...
            for congress_trade in congress_trades:
                ...
                trade_calc = self.calculate_scaled_trade(
                    congress_trade, account_balance
                )
    ...
    def check_stop_losses(self) -> list:
        ...
        for symbol, pos in self.positions.items():
            try:
                ...
                if (
                    gain_pct >= float(self.trailing_stop_activation)
                    and not pos.trailing_stop_active
                ):
                    pos.trailing_stop_active = True
                    pos.trailing_stop_percent = float(self.trailing_stop_percent)
                    pos.stop_loss_price = pos.highest_price * (
                        1 - pos.trailing_stop_percent
                    )
                    logger.info(
                        f"Trailing stop activated for {symbol} at ${pos.stop_loss_price:.2f}"
                    )
        ...
        for symbol, pos in self.positions.items():
            ...
            if hold_duration >= target_hold:
                try:
                    quote = self.broker.get_quote(symbol)
                    current_price = (
                        quote.get("last_price", pos.entry_price)
                        if quote
                    ...
                    to_sell.append(
                        {
                            ...
                            "pnl_percent": pos.get_unrealized_pnl_percent(current_price)
                            * 100,
                        }
                    )
                    logger.info(
                        f"Position {symbol} exceeded holding period ({hold_duration.days} days)"
                    )
                except Exception as e:
    ...
    def check_portfolio_risk(self) -> dict:
        ...
        try:
            ...
            if (
                self.peak_portfolio_value is None
                or total_value > self.peak_portfolio_value
            ):
