# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\config_loader.py
"""
...
def load_config(config_path: str = "config/config.json") -> Dict[str, Any]:
    ...
    def remove_comments(obj):
        if isinstance(obj, dict):
            return {
                k: remove_comments(v) for k, v in obj.items() if not k.startswith("_")
            }
...
def setup_config_interactive():
    ...
    print(
        f"  Telegram: {'Enabled' if secrets.get('TELEGRAM_BOT_TOKEN') else 'Disabled'}"
    )
    print()
