# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\backtester.py
"""
...
class CongressionalBacktester:
    ...
    def get_historical_price(
        self, ticker: str, date: datetime, days_forward: int = 0
    ) -> Optional[float]:
    ...
    def calculate_return(
        self, ticker: str, entry_date: datetime, exit_days: int, is_purchase: bool
    ) -> Optional[float]:
    ...
    def run_strategy(
        ...
        if len(returns) > 1:
            ...
            sharpe = (
                (avg_return / std_dev) * (252 / holding_period_days) ** 0.5
                if std_dev > 0
                else 0
            )
        else:
    ...
    def compare_strategies(self, trades: List[Trade]) -> List[BacktestResult]:
        ...
        for strat in strategies:
            ...
            if strat.get("pelosi_only"):
                pelosi_trades = [
                    t for t in trades if "pelosi" in t.representative.lower()
                ]
    ...
    def print_results(self, results: List[BacktestResult]):
        ...
        print("=" * 100)
        print(
            f"{'Strategy':<45} {'Trades':>7} {'Win%':>7} {'Avg Ret':>8} {'Total':>9} {'MaxDD':>7} {'Sharpe':>7}"
        )
        print("-" * 100)

        for r in results:
            print(
                f"{r.strategy_name:<45} {r.total_trades:>7} {r.win_rate*100:>6.1f}% "
        ...
        if results:
            ...
            if best.trades_detail:
                print("\n  Top 5 Trades:")
                top_trades = sorted(
                    best.trades_detail, key=lambda t: t["return"], reverse=True
                )[:5]
                for t in top_trades:
                    print(
                        f"    {t['ticker']:6} {t['return']*100:>7.1f}%  ({t['representative'][:25]})"
                    )
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for row in cursor.fetchall():
            try:
                # Parse dates
                tx_date = (
                    datetime.strptime(row["transaction_date"], "%m/%d/%Y")
                    if row["transaction_date"]
                    else datetime.now()
                )
                disc_date = (
                    datetime.strptime(row["disclosure_date"], "%m/%d/%Y")
