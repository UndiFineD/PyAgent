# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\congress_data\broker_integration.py
"""
...
class BrokerIntegration:
    ...
    def __init__(self, config_path=None):
        ...
        self.broker_name = (
            self.config.get("broker", {}).get("adapter", "unknown").upper()
        )
        ...
        try:
            ...
            if state_file.exists():
                ...
                self.processed_alerts = {
                    alert_id
                    for alert_id in self.processed_alerts
    ...
    def process_alert_for_trading(self, alert):
        ...
        try:
            ...
            logger.info(
                f"Processing alert {alert_id}: {trade.get('politician')} - {trade.get('ticker')}"
            )
            ...
            if transaction_type in ["buy", "purchase"]:
                ...
                action = "SELL"
                reason = (
                    f"Congressional sell: {politician} sold ${amount:,.0f} of {ticker}"
                )
            else:
                logger.warning(
                    f"Skipping alert {alert_id}: Unknown transaction type '{transaction_type}'"
                )
                return False

            # Check trade size threshold
            min_trade_size = self.congress_config.get("alerting", {}).get(
    ...
    def get_trading_recommendations(self, ticker=None, limit=10):
        ...
        try:
            ...
            if ticker:
                recommendations = [
                    r for r in recommendations if r.get("ticker") == ticker.upper()
                ]
    ...
    def run_continuous_monitoring(self, interval_seconds=300):
        ...
        logger.info(
            f"Starting continuous monitoring (checking every {interval_seconds} seconds)"
        )
    ...
    def get_integration_stats(self):
        ...
        stats = {
            ...
            "recommendations_directory": str(
                self.project_root / "data" / "trading_recommendations"
            ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Broker Integration for Congressional Trade Data"
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--limit", "-l", type=int, default=10, help="Limit number of recommendations"
    )
    parser.add_argument(
    ...
    if args.command == "process":
        ...
        recommendations = integration.get_trading_recommendations(
