# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\congress_data\data_collector.py
"""
...
class CongressDataCollector:
    ...
    def fetch_senate_data(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(filtered_trades)} Senate trades (min ${min_amount})"
            )
    ...
    def fetch_senate_community_data(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Failed to fetch community Senate data: {response.status_code}"
                )
                return []
    ...
    def fetch_house_data(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(filtered_trades)} House trades (min ${min_amount})"
            )
            return filtered_trades
    ...
    def filter_politicians(self, trades):
        ...
        if not politician_config.get("track_all", False):
            specific_politicians = set(
                politician_config.get("specific_politicians", [])
            )
            ...
            logger.info(
                f"Filtered to {len(filtered_trades)} trades from tracked politicians"
            )
            return filtered_trades
    ...
    def filter_by_trade_size(self, trades):
        ...
        if len(filtered_trades) < len(trades):
            logger.info(
                f"Filtered out {len(trades) - len(filtered_trades)} trades below ${min_size}"
            )
                    ...
                    try:
                        ...
                        for trade in trades:
                            for key in ["transaction_date", "filing_date"]:
                                if trade.get(key):
                                    try:
                                        trade[key] = datetime.fromisoformat(
                                            trade[key].replace("Z", "+00:00")
                                        )
                                    except (ValueError, AttributeError):
                                        pass
    ...
    def collect_all_data(self):
        ...
        for source in sources:
            try:
                ...
                if (
                    last_fetch
                    and (datetime.now() - last_fetch).total_seconds()
                    < check_interval * 3600
                ):
    ...
    def get_collection_stats(self):
        ...
        stats = {
            "last_fetch_times": {
                k: v.isoformat() if v else None for k, v in self.last_fetch_time.items()
            },
