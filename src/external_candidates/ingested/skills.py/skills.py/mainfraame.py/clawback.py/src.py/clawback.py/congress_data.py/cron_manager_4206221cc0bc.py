# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\congress_data\cron_manager.py
"""
...
class CronManager:
    ...
    def job_function(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Job completed: collected {len(trades)} trades, sent {len(alerted_trades)} alerts"
                )
    ...
    def setup_schedule(self):
        ...
        try:
            ...
            _timezone = self.cron_config.get(
            ...
            if day_of_week == "*":
                ...
                for day_num in range(start_day, end_day + 1):
                    ...
                    if 0 <= day_num <= 6:
                        ...
                        getattr(self.scheduler.every(), day_name).at(
                            f"{hour}:{minute}"
                        ).do(self.job_function)
                logger.info(
                    f"Scheduled weekday job at {hour}:{minute} (days {day_of_week})"
                )
            else:
                # Specific day(s)
    ...
    def cleanup_old_data(self, days_to_keep=30):
        ...
        try:
            data_dir = self.data_collector.storage_config.get(
                "data_directory", "data/congress_trades"
            )
            backup_dir = self.data_collector.storage_config.get(
