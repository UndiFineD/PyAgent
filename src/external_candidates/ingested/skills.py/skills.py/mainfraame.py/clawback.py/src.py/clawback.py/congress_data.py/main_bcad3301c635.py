# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\clawback\src\clawback\congress_data\main.py
"""
...
class CongressDataApp:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.scheduler = CongressCronScheduler(
                self.config, self.data_collector, self.alert_manager
            )
    ...
    def run_once(self):
        ...
        try:
            ...
            if result.get("success", False):
                logger.info(
                    f"Collection successful: {result.get('trades_collected', 0)} trades, {result.get('alerts_sent', 0)} alerts"
                )
                return True
            else:
                logger.error(
    ...
    def show_stats(self):
        ...
        try:
            ...
            print("\n📈 COLLECTION STATS:")
            print(
                f"  Total Trades Collected: {scheduler_stats['total_trades_collected']}"
            )
            print(f"  Total Alerts Sent: {scheduler_stats['total_alerts_sent']}")
            print(
                f"  Failed Sources: {', '.join(collection_stats.get('failed_sources', [])) or 'None'}"
            )
            ...
            print(
                f"  Minimum Trade Size: ${alert_stats['thresholds']['minimum_trade_size']:,.0f}"
            )
            print("  Channels:")
            print(
                f"    - Telegram: {'✅' if alert_stats['channels']['telegram'] else '❌'}"
            )
            print(f"    - Email: {'✅' if alert_stats['channels']['email'] else '❌'}")
            print(
                f"    - E*TRADE Integration: {'✅' if alert_stats['channels']['broker_integration'] else '❌'}"
            )
            ...
            data_dir = self.config.get_storage_config().get(
        ...
        try:
            ...
            if self.alert_manager.should_alert(sample_trade):
                ...
                if self.alert_manager.alert_config.get("email_alerts", False):
                    ...
                    if self.alert_manager.send_email_alert(
                        "[TEST] " + message, "[TEST] Congressional Trade Alert"
                    ):
                ...
                logger.warning(
                    "Sample trade would not trigger alert (check thresholds)"
                )
                return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Congressional Trade Data Collection System"
    )
    parser.add_argument(
        "command",
        choices=["run", "once", "stats", "test-alerts", "config"],
        help="Command to execute",
    )
    parser.add_argument(
        "--config",
        "-c",
        default="config/congress_config.json",
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        f"Starting congressional data collection system with command: {args.command}"
    )
