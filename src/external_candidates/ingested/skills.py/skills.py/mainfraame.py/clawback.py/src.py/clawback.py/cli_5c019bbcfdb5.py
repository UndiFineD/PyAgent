...
def setup_wizard():
    ...
    try:
        ...
        if choice == "1":
            ...
            # Get broker type
            broker = (
                input("Broker type (etrade/schwab/fidelity) [etrade]: ").strip()
                or "etrade"
            )

            # Get environment
            env = (
                input("Environment (sandbox/production) [sandbox]: ").strip()
                or "sandbox"
            )
            ...
            if openclaw_config_path.exists():
                try:
                    ...
                    telegram_config = openclaw_config.get("channels", {}).get(
                        "telegram", {}
                    )
                    if telegram_config.get("botToken"):
                        print("✅ OpenClaw Telegram channel is configured")
                        print(
                            f"   Bot token: {telegram_config.get('botToken')[:10]}..."
                        )
                        ...
                        print(
                            "   Run: openclaw config set channels.telegram.botToken YOUR_TOKEN"
                        )
                except Exception as e:
...
def check_status():
    ...
    try:
        ...
        print(f"   Broker: {config.get('broker', {}).get('adapter', 'Not set')}")
        print(
            f"   Environment: {config.get('broker', {}).get('environment', 'Not set')}"
        )
        print(f"   Account ID: {config.get('trading', {}).get('accountId', 'Not set')}")
