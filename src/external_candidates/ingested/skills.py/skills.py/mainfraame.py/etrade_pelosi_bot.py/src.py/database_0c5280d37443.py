# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\etrade-pelosi-bot\src\database.py
"""
...
SEED_DATA_PATH = (
    Path(__file__).parent.parent / "data" / "seed" / "congressional_trades.json"
)
...
class TradingDatabase:
        ...
        with sqlite3.connect(self.db_path) as conn:
            try:
                ...
                if "parsed_date" in trade_copy:
                    trade_copy["parsed_date"] = (
                        trade_copy["parsed_date"].isoformat()
                        if hasattr(trade_copy["parsed_date"], "isoformat")
                        else str(trade_copy["parsed_date"])
                    )
                ...
                conn.commit()
                logger.debug(
                    f"Added trade: {trade.get('ticker')} {trade.get('transaction_type')}"
                )
                return True
    ...
    def trade_exists(self, trade):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT 1 FROM congressional_trades WHERE trade_hash = ?", (trade_hash,)
            )
            return cursor.fetchone() is not None

    # --- Executed Trades ---

    def add_executed_trade(
        self,
    ...
    def get_trade_stats(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            stats["by_chamber"] = {
                row["chamber"]: row["count"] for row in cursor.fetchall()
            }

            # Executed trades
            cursor = conn.execute(
                "SELECT COUNT(*) as count FROM executed_trades WHERE status = 'filled'"
            )
            stats["total_executed"] = cursor.fetchone()["count"]

            # Total value traded
            cursor = conn.execute(
                "SELECT SUM(total_value) as total FROM executed_trades WHERE status = 'filled'"
            )
            ...
            """)
            stats["top_tickers"] = [
                (row["ticker"], row["count"]) for row in cursor.fetchall()
            ]
