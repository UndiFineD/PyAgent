# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\etrade-pelosi-bot\src\etrade_adapter.py
"""
...
class ETradeAdapter(BrokerAdapter):
    ...
    def get_account_balance(
        ...
        try:
            ...
            account_key = self.accounts_map.get(
                account_id, self.account_id_key or account_id
            )
            ...
            if response.status_code == 200:
                ...
                cash_available = float(
                    balance_info.get("cashAvailableForInvestment", 0)
                )
                total_account_value = float(
                    balance_info.get("RealTimeValues", {}).get("totalAccountValue", 0)
                )

                # Fallback to non-realtime if realtime not available
                if total_account_value == 0:
                    total_account_value = float(
                        balance_info.get("totalAccountValue", 0)
                    )

                logger.info(
                    f"Account balance - Cash: ${cash_available:,.2f}, Total: ${total_account_value:,.2f}"
                )
                return {
                ...
                logger.error(
                    f"Failed to get balance: {response.status_code} - {response.text}"
                )
                return None
    ...
    def get_positions(self, account_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                portfolio = data.get("PortfolioResponse", {}).get(
                    "AccountPortfolio", []
                )
    ...
    def place_order(self, account_id: str, order_details: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            oauth = self._get_oauth()
            preview_url = (
                f"{self.BASE_URL}/v1/accounts/{account_key}/orders/preview.json"
            )
            ...
            if (
                order_details.get("price_type") == "LIMIT"
                and "limit_price" in order_details
            ):
                order_request["PreviewOrderRequest"]["Order"][0]["limitPrice"] = float(
            ...
            if response.status_code == 200:
                ...
                if preview_ids:
                    ...
                    # Place the order
                    place_url = (
                        f"{self.BASE_URL}/v1/accounts/{account_key}/orders/place.json"
                    )
                    place_request = {
                        "PlaceOrderRequest": {
                            "orderType": "EQ",
                    ...
                    place_response = requests.post(
                        place_url, auth=oauth, json=place_request
                    )

                    if place_response.status_code == 200:
                        order_response = place_response.json()
                        order_ids = order_response.get("PlaceOrderResponse", {}).get(
                            "OrderIds", []
                        )
                        if order_ids:
                            order_id = order_ids[0].get("orderId")
                            logger.info(
                                f"Order placed: {order_details['action']} {order_details['quantity']} {order_details['symbol']} (Order ID: {order_id})"
                            )
                        else:
                            logger.info(
                                f"Order placed: {order_details['action']} {order_details['quantity']} {order_details['symbol']}"
                            )
                        return True
                    else:
                        logger.error(
                            f"Failed to place order: {place_response.status_code} - {place_response.text}"
                        )
                        return False
                else:
                ...
                logger.error(
                    f"Order preview failed: {response.status_code} - {response.text}"
                )
                return False
    ...
    def get_orders(
        self, account_id: Optional[str] = None, status: str = "OPEN"
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            oauth = self._get_oauth()
            url = (
                f"{self.BASE_URL}/v1/accounts/{account_key}/orders.json?status={status}"
            )
