# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\etrade-pelosi-bot\src\main.py
"""
...
class TradingBot:
    ...
    def _load_saved_tokens(self):
        ...
        if os.path.exists(token_file):
            try:
                ...
                if hasattr(self.broker, "_authenticated") and tokens.get(
                    "access_token"
                ):
    ...
    def check_and_process_trades(self):
        ...
        try:
            ...
            self.congress_tracker.save_trades_to_file(
                new_trades, "recent_congressional_trades.json"
            )
    ...
    def run_scheduled(self, interval_hours=24):
        """Run on a schedule"""
        self.logger.info(
            f"Starting scheduled trading bot (checking every {interval_hours} hours)"
        )
        ...
        while True:
            ...
            if choice == "1":
                ...
                if triggered:
                    ...
                    for t in triggered:
                        print(
                            f"  SOLD {t['symbol']}: ${t['current_price']:.2f} (P/L: {t['pnl_percent']:.1f}%)"
                        )
                else:
                ...
                filepath = (
                    input("Export path (default: data/export.json): ").strip()
                    or "data/export.json"
                )
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "auth":
            ...
            print(
                "Available commands: auth, run, schedule [hours], interactive, status"
            )
    else:
