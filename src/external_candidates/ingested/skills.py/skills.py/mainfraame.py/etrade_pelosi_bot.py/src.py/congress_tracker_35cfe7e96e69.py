# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\etrade-pelosi-bot\src\congress_tracker.py
"""
...
class CongressTracker:
    ...
    def __init__(self, config):
        ...
        self.target_politicians = config.get("congress", {}).get(
            "targetPoliticians", [{"name": "Nancy Pelosi", "chamber": "house"}]
        )
    ...
    def fetch_house_clerk_data(self):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to fetch House Clerk data: {response.status_code}"
                )
                return []
            ...
            for member in root.findall(".//Member"):
                ...
                if (
                    filing_type is None or filing_type.text != "P"
                ...
                rep_name = (
                    f"Hon. {first_name.text} {last_name.text}"
    ...
    def _parse_ptr_pdf(self, pdf_url, filing_date, representative="Hon. Nancy Pelosi"):
        ...
        try:
            ...
            with pdfplumber.open(BytesIO(response.content)) as pdf:
                ...
                table_trades = self._parse_ptr_tables(
                    pdf, filing_date, representative, pdf_url
                )
                if table_trades:
                ...
                if not trades:
                    text_trades = self._parse_ptr_text(
                        pdf, filing_date, representative, pdf_url
                    )
                ...
                for row in table[header_row + 1 :]:
                    ...
                    try:
                        ...
                        if "tx_type" in col_map:
                            ...
                            if (
                                "p" in tx_text
                                or "purchase" in tx_text
                                or "buy" in tx_text
                            ):
                                tx_type = "purchase"
                            elif (
                                "s" in tx_text or "sale" in tx_text or "sell" in tx_text
                            ):
                                tx_type = "sale"
                        else:
                        ...
                        if "amount" in col_map:
                            ...
                            for cell in row:
                                ...
                                if "$" in cell_str:
                                    amount, amount_range = self._parse_amount_range(
                                        cell_str
                                    )
                                    if amount > 0:
                                        break
                        ...
                        if (
                            self._is_option_trade(asset_text)
                            and "exercise" not in asset_text.lower()
                        ):
                            continue
                        ...
                        if tx_type in self.trade_types:
                            ...
                            trades.append(trade)
                            logger.debug(
                                f"Table parsed: {ticker} {tx_type} {amount_range}"
                            )
    ...
    def _parse_ptr_text(self, pdf, filing_date, representative, pdf_url):
        ...
        if len(blocks) <= 1:
            blocks = re.split(
                r"\n(?=.*(?:Common Stock|Stock|Corp|Inc|LLC).*\([A-Z]{1,5}\))",
                full_text,
            )
                ...
                while pages_processed < max_pages:
                    ...
                    if table:
                        ...
                        for row in rows:
                            ...
                            logger.info(
                                f"Found Senate PTR: {full_name} - {filing_date}"
                            )

                            # Fetch and parse the individual report
                            report_trades = self._fetch_senate_report_selenium(
                                driver, report_url, full_name, filing_date
                            )
    ...
    def _fetch_senate_report_selenium(
        self, driver, report_url, senator_name, filing_date
    ):
            ...
            for table in tables:
                ...
                for row in rows:
                    ...
                    try:
                        ...
                        if (
                            tx_type in self.trade_types
                            and amount >= self.min_trade_size
                        ):
                            trade = {
                                "transaction_date": tx_date,
                                "disclosure_date": filing_date,
                            ...
                            trades.append(trade)
                            logger.debug(
                                f"Parsed Senate trade: {ticker} {tx_type} {amount_range}"
                            )
    ...
    def _parse_senate_json_results(
        self, session, base_url, headers, data, target_name=None
    ):
        ...
        try:
            ...
            for record in records:
                ...
                report_trades = self._parse_senate_ptr_report(
                    session, report_url, headers, full_name, filing_date
                )
                trades.extend(report_trades)
    ...
    def _parse_senate_ptr_report(
        self, session, report_url, headers, senator_name, filing_date
    ):
    ...
    def fetch_mock_data(self):
        ...
        for trade in mock_trades:
            if (
                trade["transaction_type"] in self.trade_types
                and trade["amount"] >= self.min_trade_size
            ):
                trades.append(trade)
    ...
    def get_recent_trades(self, days=30):
        ...
        try:
            ...
            if (
                self.data_source in ["senate_efd", "official", "auto"]
                or self.include_senate
            ):
