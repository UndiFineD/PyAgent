# Extracted from: C:\DEV\PyAgent\.external\skills\skills\mainfraame\etrade-pelosi-bot\src\congress_data\alert_manager.py
"""
...
class AlertManager:
    ...
    def load_alert_history(self):
        ...
        try:
            ...
            if os.path.exists(history_file):
                ...
                self.sent_alerts = {
                    alert_id
                    for alert_id in self.sent_alerts
                    if self.parse_alert_date(alert_id) > cutoff_date
                }

                logger.info(
                    f"Loaded {len(self.sent_alerts)} recent alerts from history"
                )
    ...
    def should_alert(self, trade):
        ...
        trans_type = trade.get("transaction_type", "").lower()
        if trans_type in ["buy", "purchase"] and not self.alert_config.get(
            "alert_on_buys", True
        ):
            return False
        if trans_type in ["sell", "sale"] and not self.alert_config.get(
            "alert_on_sells", True
        ):
            return False
...
"""
            ...
            try:
                if self.should_alert(trade):
                    ...
                    if sent:
                        ...
                        logger.info(
                            f"Alert sent for {trade.get('politician')} - {trade.get('ticker')}"
                        )
    ...
    def get_alert_stats(self):
        ...
        stats = {
            ...
            "thresholds": {
                "minimum_trade_size": self.alert_config.get(
