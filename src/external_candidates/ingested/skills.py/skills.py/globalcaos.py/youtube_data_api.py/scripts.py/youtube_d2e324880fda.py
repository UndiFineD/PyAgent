# Extracted from: C:\DEV\PyAgent\.external\skills\skills\globalcaos\youtube-data-api\scripts\youtube.py
#!/usr/bin/env python3
# /// script
...
def cmd_subscriptions(args):
    ...
    request = youtube.subscriptions().list(
        part="snippet", mine=True, maxResults=args.limit
    )
...
def cmd_playlists(args):
    ...
    request = youtube.playlists().list(
        part="snippet,contentDetails", mine=True, maxResults=args.limit
    )
    response = request.execute()
...
def cmd_playlist_items(args):
    ...
    request = youtube.playlistItems().list(
        part="snippet", playlistId=args.playlist_id, maxResults=args.limit
    )
    response = request.execute()
...
def cmd_search(args):
    ...
    request = youtube.search().list(
        part="snippet", q=args.query, type="video", maxResults=args.limit
    )
    response = request.execute()
...
def cmd_video(args):
    ...
    request = youtube.videos().list(
        part="snippet,contentDetails,statistics", id=args.video_id
    )
    response = request.execute()
...
def cmd_captions(args):
    """List video captions."""
    youtube = get_youtube_service()
    request = youtube.captions().list(part="snippet", videoId=args.video_id)
    response = request.execute()
...
def cmd_liked(args):
    ...
    youtube = get_youtube_service()
    request = youtube.videos().list(
        part="snippet", myRating="like", maxResults=args.limit
    )
    response = request.execute()
...
def cmd_channel(args):
    ...
    youtube = get_youtube_service()
    if args.channel_id:
        request = youtube.channels().list(part="snippet,statistics", id=args.channel_id)
    else:
...
def main():
    ...
    ch_parser = subparsers.add_parser(
        "channel", aliases=["ch"], help="Get channel info"
    )
    ch_parser.add_argument(
        "channel_id", nargs="?", help="Channel ID (omit for your channel)"
    )
    ch_parser.set_defaults(func=cmd_channel)

    # subscriptions
    subs_parser = subparsers.add_parser(
        "subscriptions", aliases=["subs"], help="List subscriptions"
    )
    ...
    pl_parser = subparsers.add_parser(
        "playlists", aliases=["pl"], help="List playlists"
    )
    pl_parser.add_argument("-l", "--limit", type=int, default=25, help="Max results")
    pl_parser.set_defaults(func=cmd_playlists)

    # playlist items
    pli_parser = subparsers.add_parser(
        "playlist-items", aliases=["pli"], help="List playlist items"
    )
    ...
    cap_parser = subparsers.add_parser(
        "captions", aliases=["caps"], help="List video captions"
    )
