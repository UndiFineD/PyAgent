...
def get_text(elem, path: str, ns=NS) -> str:
    ...
    for prefix in ["lt4", "lt5", "lt8", "lt"]:
        try:
            full_path = "/".join(
                f"{prefix}:{p}" if ":" not in p else p for p in path.split("/")
            )
...
def parse_location(loc_elem) -> dict:
    ...
    return {
        "name": get_text(loc_elem, "location/locationName")
        or get_text(loc_elem, "locationName"),
...
def parse_service(svc) -> dict:
    ...
    # Get origin(s)
    origins = []
    for orig in svc.findall(".//lt5:origin/lt4:location", NS) or svc.findall(
        ".//lt4:origin/lt4:location", NS
    ):
        origins.append(
            {
...
def get_departures(station: str, rows: int = 10, filter_to: str = None) -> dict:
    ...
    if filter_to:
        filter_xml = (
            f"<filterCrs>{filter_to.upper()}</filterCrs><filterType>to</filterType>"
        )
    ...
    xml_resp = soap_request(
        "http://thalesgroup.com/RTTI/2012-01-13/ldb/GetDepartureBoard", body
    )
...
def get_arrivals(station: str, rows: int = 10, filter_from: str = None) -> dict:
    ...
    if filter_from:
        filter_xml = (
            f"<filterCrs>{filter_from.upper()}</filterCrs><filterType>from</filterType>"
        )
    ...
    xml_resp = soap_request(
        "http://thalesgroup.com/RTTI/2012-01-13/ldb/GetArrivalBoard", body
    )
    return parse_board_response(xml_resp)
...
def parse_board_response(xml_resp: str) -> dict:
    ...
    if result is None:
        result = root.find(
            ".//{http://thalesgroup.com/RTTI/2021-11-01/ldb/}GetStationBoardResult"
        )
