...
def poll_for_result(
    api_key: str, task_uuid: str, max_wait: int = 600, interval: int = 5
) -> dict:
...
def cmd_img2vid(args):
    ...
    task = {
        ...
        "taskUUID": task_uuid,
        "positivePrompt": args.prompt
        or "smooth animation, natural movement, cinematic quality",
        "model": args.model,
        "duration": args.duration,
        "width": args.width,
        "height": args.height,
...
def main():
    parser = argparse.ArgumentParser(
        prog="runware-video", description="Runware Video Generation"
    )
    parser.add_argument("--api-key", help="Runware API key (or set RUNWARE_API_KEY)")
    parser.add_argument(
        "--max-wait", type=int, default=600, help="Max seconds to wait for video"
    )
    ...
    gen = subparsers.add_parser(
        "generate", aliases=["gen", "t2v"], help="Text-to-video generation"
    )
    gen.add_argument("prompt", help="Video description")
    gen.add_argument(
        "--model", default="klingai:5@3", help="Model ID (default: Kling AI 1.6 Pro)"
    )
    ...
    # Image-to-video
    i2v = subparsers.add_parser(
        "img2vid", aliases=["i2v"], help="Image-to-video generation"
    )
    i2v.add_argument(
        "images", nargs="+", help="Input images (paths or URLs, first/last frames)"
    )
