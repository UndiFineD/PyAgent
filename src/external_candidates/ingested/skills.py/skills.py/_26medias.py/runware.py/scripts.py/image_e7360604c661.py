# Extracted from: C:\DEV\PyAgent\.external\skills\skills\26medias\runware\scripts\image.py
#!/usr/bin/env python3
"""
...
def api_request(api_key: str, tasks: list, timeout: int = 180) -> dict:
    ...
    try:
        ...
        try:
            ...
            print(
                f"API Error ({e.code}): {json.dumps(error_data, indent=2)}",
                file=sys.stderr,
            )
        except:
...
def cmd_generate(args):
    ...
    if not api_key:
        print(
            "Error: RUNWARE_API_KEY not set (use --api-key or env var)", file=sys.stderr
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        prog="runware-image", description="Runware Image Generation"
    )
    parser.add_argument("--api-key", help="Runware API key (or set RUNWARE_API_KEY)")
    parser.add_argument(
        "--timeout", type=int, default=180, help="Request timeout in seconds"
    )
    ...
    gen = subparsers.add_parser(
        "generate", aliases=["gen", "t2i"], help="Text-to-image generation"
    )
    gen.add_argument("prompt", help="Image description")
    gen.add_argument(
        "--model", default="runware:101@1", help="Model ID (default: FLUX.1 Dev)"
    )
    ...
    gen.add_argument(
        "--output", "-o", default="./runware-output", help="Output directory"
    )
    gen.set_defaults(func=cmd_generate)

    # Image-to-image
    i2i = subparsers.add_parser(
        "img2img", aliases=["i2i"], help="Image-to-image transformation"
    )
    ...
    i2i.add_argument(
        "--strength", type=float, default=0.7, help="Transformation strength (0-1)"
    )
    ...
    up.add_argument(
        "--factor", type=int, default=2, choices=[2, 4], help="Upscale factor"
    )
