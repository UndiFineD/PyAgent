# Extracted from: C:\DEV\PyAgent\.external\skills\skills\inclinedadarsh\manimce-best-practices\examples\3d_visualization.py
"""
...
class SphereVisualization(ThreeDScene):
    ...
    def construct(self):
        ...
        sphere = Surface(
            lambda u, v: np.array(
                [np.cos(v) * np.sin(u), np.sin(v) * np.sin(u), np.cos(u)]
            ),
            u_range=[0, PI],
            v_range=[0, 2 * PI],
            resolution=(20, 40),
...
class AnimatedSurface(ThreeDScene):
    ...
    def construct(self):
        ...
        surface = always_redraw(
            lambda: Surface(
                lambda u, v: axes.c2p(
                    u, v, np.sin(np.sqrt(u**2 + v**2) - time.get_value())
                ),
                u_range=[-3, 3],
                v_range=[-3, 3],
                resolution=(25, 25),
...
class MultipleObjects3D(ThreeDScene):
    ...
    def construct(self):
        ...
        arrow2 = Arrow3D(
            start=cube.get_center(), end=cylinder.get_center(), color=YELLOW
        )
