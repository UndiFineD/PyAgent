        ...
        **kwargs,
        ...
        for n in layer_sizes:
            layer = VGroup(
                *[
                    Circle(
                        radius=neuron_radius, color=WHITE, fill_opacity=random.random()
                    )
                    for _ in range(n)
                ]
            )
    ...
    def __init__(self, n_tokens=8, token_labels=None, attention_weights=None, **kwargs):
        ...
        if attention_weights is None:
            ...
            attention_weights = softmax(
                np.random.randn(n_tokens, n_tokens), temperature=0.5
            )
...
def show_attention_flow(scene, source_mobs, target_mob, weights=None, run_time=2):
    ...
    scene.play(
        LaggedStart(*[Create(a) for a in arrows], lag_ratio=0.2), run_time=run_time
    )
    return arrows
