# Extracted from: C:\DEV\PyAgent\.external\skills\skills\inclinedadarsh\manimce-best-practices\examples\math_visualization.py
"""
...
class MatrixTransformation(Scene):
    ...
    def construct(self):
        ...
        # Show transformation
        vector = MathTex(
            r"\vec{x} = \begin{bmatrix} 1 \\ 1 \end{bmatrix}", color=YELLOW
        )
...
class SummationNotation(Scene):
    ...
    def construct(self):
        # Summation formula
        formula = MathTex(
            r"\sum_{n=1}^{\infty} \frac{1}{n^2} = \frac{\pi^2}{6}", font_size=64
        )
        ...
        terms = MathTex(
            r"= 1 + \frac{1}{4} + \frac{1}{9} + \frac{1}{16} + \cdots", font_size=48
        )
