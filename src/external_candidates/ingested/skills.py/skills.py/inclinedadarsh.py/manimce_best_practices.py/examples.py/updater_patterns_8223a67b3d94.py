...
class CircleRadiusTracker(Scene):
    ...
    def construct(self):
        ...
        radius_text = always_redraw(
            lambda: MathTex(f"r = {tracker.get_value():.2f}").to_edge(UP)
        )
...
class MultipleTrackers(Scene):
    ...
    def construct(self):
        ...
        self.play(
            x_tracker.animate.set_value(-2), y_tracker.animate.set_value(-1), run_time=2
        )
        self.wait()
...
class SpringMassSimulation(Scene):
    ...
    def construct(self):
        ...
        spring = always_redraw(
            lambda: Line(
                ground.get_center() + UP * 0.1, mass_obj.get_bottom(), color=GREY
            )
        )
