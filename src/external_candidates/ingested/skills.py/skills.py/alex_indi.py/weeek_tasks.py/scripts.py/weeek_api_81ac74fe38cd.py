...
def cmd_list_tasks(args):
    ...
    print(
        json.dumps(
            request("GET", "/tm/tasks", params=params), ensure_ascii=False, indent=2
        )
    )
...
def cmd_create_task(args):
    ...
    body = {
        ...
        "priority": args.priority,
        "customFields": (
            json.loads(args.custom_fields_json) if args.custom_fields_json else None
        ),
    }
    body = {k: v for k, v in body.items() if v is not None}
    print(
        json.dumps(
            request("POST", "/tm/tasks", body=body), ensure_ascii=False, indent=2
        )
    )
...
def cmd_list_boards(args):
    ...
    print(
        json.dumps(
            request("GET", "/tm/boards", params=params), ensure_ascii=False, indent=2
        )
    )
