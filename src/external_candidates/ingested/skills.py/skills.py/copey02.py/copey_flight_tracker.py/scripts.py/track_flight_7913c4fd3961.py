# Extracted from: C:\DEV\PyAgent\.external\skills\skills\copey02\copey-flight-tracker\scripts\track_flight.py
#!/usr/bin/env python3
"""
...
try:
    ...
    print(
        "Error: 'requests' library not installed. Install with: pip3 install requests"
    )
    sys.exit(1)
...
def get_status_emoji(status: Optional[str]) -> str:
    ...
    if (
        ...
        or "airborne" in status_lower
        or "en-route" in status_lower
    ):
        return "🟢"
...
def display_flight(flight_data: dict) -> None:
    ...
    if aircraft_iata or aircraft_icao:
        print(
            f"🛩️  {aircraft_icao or aircraft_iata}{' • ' + aircraft_reg if aircraft_reg else ''}"
        )
    print()
...
Setup:
  ...
  2. Set environment: export AVIATIONSTACK_API_KEY='your-key-here'
    ...
    parser.add_argument("flight_number", help="Flight number (e.g., AA100, UA2402)")
