# Extracted from: C:\DEV\PyAgent\.external\skills\skills\thesethrose\kraken\kraken_api.py
#!/usr/bin/env python3
"""
...
from kraken.spot import Earn, Funding, Market, User
...
def cmd_balance(user: Optional[User]) -> str:
    ...
    try:
        ...
        for asset, balance in sorted(
            result.items(), key=lambda x: float(x[1]), reverse=True
        ):
            if float(balance) > 0:
...
def cmd_open_orders(user: Optional[User]) -> str:
    ...
    try:
        ...
        for refid, order in result.get("open", {}).items():
            ...
            vol = order.get("vol", "?")
            lines.append(
                f"  [{side:4s}] {timestamp.strftime('%Y-%m-%d %H:%M')} {pair:10s} {vol:>12s} @ {price}"
            )
        return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
...
def cmd_closed_orders(user: Optional[User], limit: int = 20) -> str:
    ...
    try:
        ...
        for refid, order in orders:
            ...
            lines.append(
                f"  [{side:4s}] {timestamp.strftime('%Y-%m-%d %H:%M')} {pair:10s} {vol:>12s} [{status}]"
            )
        return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
...
def cmd_query_orders(user: Optional[User], refid: Optional[str] = None) -> str:
    ...
    try:
        ...
        for rid, order in orders.items():
            ...
            lines.append(
                f"  Type: {descr.get('type', '?')} {descr.get('ordertype', '?')}"
            )
...
def cmd_trades(user: Optional[User], limit: int = 20) -> str:
    ...
    try:
        ...
        sorted_trades = sorted(
            trades.items(), key=lambda x: float(x[1].get("time", 0)), reverse=True
        )
        lines = ["=== Trade History ==="]
        for refid, trade in sorted_trades[:limit]:
            ...
            lines.append(
                f"  [{side:4s}] {timestamp.strftime('%Y-%m-%d %H:%M')} "
...
def cmd_query_trades(user: Optional[User], refid: Optional[str] = None) -> str:
    ...
    try:
        ...
        for rid, trade in list(trades.items())[:10]:
            ...
            lines.append(f"  ID: {rid}")
            lines.append(
                f"  [{side:4s}] {timestamp.strftime('%Y-%m-%d %H:%M')} {trade.get('pair', '?')}"
            )
            lines.append(
                f"  Volume: {trade.get('vol', '?')} @ ${trade.get('price', '?')}"
            )
            lines.append(
                f"  Fee: {trade.get('fee', '?')} {trade.get('fee_currency', '')}"
            )
...
def cmd_ledger(
    user: Optional[User], asset: Optional[str] = None, limit: int = 20
) -> str:
    """Get ledger entries."""
    if not user:
        return "Error: API credentials required for this command."
    try:
        result = (
            user.get_ledgers_info(asset=asset) if asset else user.get_ledgers_info()
        )
        if not result or "ledger" not in result or not result.get("ledger"):
            return "No ledger entries found."

        ledger = result.get("ledger", {})
        entries = sorted(ledger.items(), key=lambda x: int(x[0], 36), reverse=True)[
            :limit
        ]
        ...
        for refid, entry in entries:
            ...
            lines.append(
                f"  [{entry.get('type', '?').upper():10s}] {timestamp.strftime('%Y-%m-%d %H:%M')} "
...
def cmd_assets(market: Market) -> str:
    ...
    try:
        ...
        for asset, data in result.items():
            ...
            lines.append(
                f"  {asset:8s}: {altname} (decimals: {decimals}/{display_decimals})"
            )
        return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
...
def cmd_ohlc(market: Market, pair: str, interval: int = 1440) -> str:
    ...
    try:
        ...
        for candle in ohlc_data[:10]:
            ...
            lines.append(
                f"  {timestamp.strftime('%Y-%m-%d %H:%M')} O:{candle[1]} H:{candle[2]} "
...
def cmd_recent_trades(market: Market, pair: str, count: int = 20) -> str:
    ...
    try:
        ...
        for trade in trades:
            ...
            lines.append(
                f"  [{side:4s}] {timestamp.strftime('%Y-%m-%d %H:%M')} ${trade[0]} x {trade[1]}"
            )
        return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
...
def cmd_spreads(market: Market, pair: str, count: int = 20) -> str:
    ...
    try:
        ...
        for spread in spreads[-count:]:
            ...
            lines.append(
                f"  {timestamp.strftime('%Y-%m-%d %H:%M')} ${bid} - ${ask} (spread: ${spread_val:.2f})"
            )
        return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
    ...
    try:
        if positions:
            ...
            for item in result.get("items", []):
                ...
                lines.append(
                    f"  {asset:6s}: ${allocated:>10s} allocated, ${rewarded:>6s} rewarded"
                )
...
def cmd_earn_status(earn: Optional[Earn], strategy_id: Optional[str] = None) -> str:
    ...
    try:
        if not strategy_id:
            ...
            result = earn.list_earn_allocations(
                converted_asset="USD", hide_zero_allocations="false"
            )
...
def cmd_earn_dealloc_status(
    earn: Optional[Earn], strategy_id: Optional[str] = None
) -> str:
...
def cmd_deposits(
    funding: Optional[Funding], asset: Optional[str] = None, address: bool = False
) -> str:
    ...
    try:
        if address:
            ...
            for method in result:
                lines.append(
                    f"  {method.get('method', '?'):20s}: {method.get('description', '?')}"
                )
            return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
...
def main():
    ...
    parser.add_argument(
        "--interval", type=int, default=1440, help="OHLC interval in minutes"
    )
    parser.add_argument(
        "--count", type=int, default=100, help="Order book/trades count"
    )
