# Extracted from: C:\DEV\PyAgent\.external\skills\skills\thesethrose\therapy-mode\therapy-notes.py
#!/usr/bin/env python3
"""
...
def start_session():
    ...
    if session_file.exists():
        print(
            f"Session for {date} already exists. Use 'therapy-notes add' to continue."
        )
        return
...
def main():
    ...
    if command == "new":
        ...
        try:
            ...
            print(
                "Invalid line number. Use: therapy-notes update <line_number> <new_text>"
            )
    elif command == "end":
        end_session()
        ...
        confirm = input(
            f"Permanently delete session {sys.argv[2]}? This cannot be undone. (y/N): "
        )
        if confirm.lower() == "y":
