# Extracted from: C:\DEV\PyAgent\.external\skills\skills\shekohex\miniflux\scripts\miniflux-cli.py
...
def create_client(args) -> miniflux.Client:
    ...
    api_key = (
        args.api_key or os.environ.get("MINIFLUX_API_KEY") or config.get("api_key")
    )
    ...
    if not api_key:
        ...
        print(
            "Set MINIFLUX_API_KEY environment variable or use --api-key",
            file=sys.stderr,
        )
        print(f"Config file: {CONFIG_PATH}", file=sys.stderr)
        sys.exit(1)
...
def truncate_content(
    content: str, limit: Optional[int], offset: int = 0, total_len: Optional[int] = None
) -> str:
    if offset > 0:
...
def cmd_list(args):
    ...
    try:
        ...
        if getattr(args, "json", False):
            ...
            for e in entries:
                print(
                    f"{e['id']}\t{e.get('feed', {}).get('title', 'Unknown')}\t{e['title']}\t{e.get('url', '')}"
                )
        else:
            for e in entries:
        ...
        print(
            "Check MINIFLUX_API_KEY environment variable or --api-key", file=sys.stderr
        )
        sys.exit(1)
...
def cmd_get(args):
    ...
    try:
        ...
        if args.json:
            ...
            print(
                f"{entry['id']}\t{entry.get('feed', {}).get('title', 'Unknown')}\t{entry['title']}\t{entry.get('url', '')}"
            )
        else:
            ...
            if args.limit or args.offset:
                content = truncate_content(
                    content, args.limit, args.offset or 0, total_len
                )
        ...
        print(
            f"Use '{os.path.basename(sys.argv[0])} list' to browse available articles.",
            file=sys.stderr,
        )
        sys.exit(1)
    except miniflux.AccessUnauthorized:
...
For more help on a subcommand:
  miniflux-cli.py <subcommand> --help
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=f"miniflux-cli.py {VERSION}"
    )
    parser.add_argument(
...
Examples:
  ...
  miniflux-cli.py list --search="python" --limit=10
    ...
    list_parser.add_argument(
        "--status", choices=["read", "unread", "removed"], help="filter by status"
    )
    ...
    list_parser.add_argument(
        "--offset", type=int, default=0, help="skip first N chars in content"
    )
    list_parser.add_argument(
        "--content-limit", type=int, help="max characters per article"
    )
    list_parser.add_argument(
        "-b", "--brief", action="store_true", help="brief output (titles only)"
    )
    list_parser.add_argument(
        "-s", "--summary", action="store_true", help="summary output (titles + excerpt)"
    )
    list_parser.add_argument(
        "-f", "--full", action="store_true", help="full article content (default)"
    )
    list_parser.add_argument("--json", action="store_true", help="output as JSON")
    list_parser.add_argument(
        "--plain", action="store_true", help="plain text output (one per line)"
    )

    # get command
    get_parser = subparsers.add_parser(
        "get",
        help="Get single article by ID",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  miniflux-cli.py get 123
...
Examples:
  ...
  miniflux-cli.py mark-read 123 456 789
    ...
    mr_parser.add_argument(
        "entry_ids", nargs="+", type=int, help="article ID(s) to mark as read"
    )

    # mark-unread command
    mu_parser = subparsers.add_parser(
        "mark-unread",
        help="Mark article(s) as unread",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  ...
  miniflux-cli.py mark-unread 123 456
    ...
    mu_parser.add_argument(
        "entry_ids", nargs="+", type=int, help="article ID(s) to mark as unread"
    )
...
Examples:
  ...
  miniflux-cli.py stats
    ...
    stats_parser.add_argument(
        "--entry-id", type=int, help="show stats for specific article"
    )
...
Examples:
  ...
  miniflux-cli.py search "ai" --status=unread --brief
    ...
    search_parser.add_argument(
        "-s", "--summary", action="store_true", help="summary output"
    )
