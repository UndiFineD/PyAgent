# Extracted from: C:\DEV\PyAgent\.external\skills\skills\giulianomlodi\moltbot-arena\scripts\game_loop.py
#!/usr/bin/env python3
"""
...
while True:
    # Get state
    state = requests.get(f"{API}/game/state", headers={"X-API-Key": KEY}).json()["data"]
    ...
    for unit in state["myUnits"]:
        if unit["type"] == "worker":
            if unit["energy"] < 50:
                ...
                actions.append(
                    {"unitId": unit["id"], "type": "transfer", "targetId": spawn["id"]}
                )

    for struct in state["myStructures"]:
        if struct["type"] == "spawn" and struct["energy"] >= 100:
            actions.append(
                {"structureId": struct["id"], "type": "spawn", "unitType": "worker"}
            )
