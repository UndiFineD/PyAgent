# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tonimelisma\email-to-calendar\scripts\tests\test_activity_ops.py
#!/usr/bin/env python3
...
class TestStartSession(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.session_patcher = patch.object(
            activity_ops, "SESSION_FILE", self.session_file
        )
        self.activity_patcher = patch.object(
            activity_ops, "ACTIVITY_FILE", self.activity_file
        )
...
class TestLogSkip(unittest.TestCase):
    ...
    def test_log_skip_adds_to_list(self):
        ...
        activity_ops.log_skip(
            email_id="email123", subject="Test Email", reason="No events found"
        )
...
class TestLogEvent(unittest.TestCase):
    ...
    def test_log_event_adds_to_list(self):
        ...
        self.assertEqual(
            session["events_extracted"][0]["reason"], "Extracted from invite"
        )
