# Extracted from: C:\DEV\PyAgent\.external\skills\skills\tonimelisma\email-to-calendar\scripts\tests\test_changelog_ops.py
#!/usr/bin/env python3
...
class TestLogCreate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = patch.object(
            changelog_ops, "CHANGELOG_FILE", self.changelog_file
        )
        self.patcher.start()
...
class TestLogDelete(unittest.TestCase):
    ...
    def test_log_delete_stores_before_state(self):
        ...
        before = json.dumps(
            {"summary": "Deleted Event", "start": "2026-02-11T10:00:00"}
        )

        change_id = changelog_ops.log_delete(
            event_id="evt123", calendar_id="primary", before_json=before
        )
...
class TestGetChange(unittest.TestCase):
    ...
    def test_get_change_returns_json(self):
        ...
        test_data = {
            "changes": [{"id": "chg_001", "action": "create", "event_id": "evt123"}]
        }
        with open(self.changelog_file, "w") as f:
            json.dump(test_data, f)
...
class TestMaxChangesLimit(unittest.TestCase):
    ...
    def test_limits_to_max_changes(self):
        ...
        changelog_ops.log_create(
            event_id="evt_new", calendar_id="primary", summary="New Event"
        )
