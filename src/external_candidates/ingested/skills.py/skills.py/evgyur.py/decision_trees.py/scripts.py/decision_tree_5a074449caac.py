# Extracted from: C:\DEV\PyAgent\.external\skills\skills\evgyur\decision-trees\scripts\decision_tree.py
#!/usr/bin/env python3
"""
...
def interactive_mode() -> None:
    ...
    while True:
        ...
        while remaining_prob > 0.001:
            outcome_name = input(
                f"    Outcome name (remaining probability: {remaining_prob:.2%}): "
            )

            if remaining_prob < 1.0:
                use_remaining = input(
                    f"    Use remaining {remaining_prob:.2%}? (y/n): "
                ).lower()
                if use_remaining == "y":
                    prob = remaining_prob
                else:
            ...
            outcomes.append({"name": outcome_name, "probability": prob, "value": value})
        ...
        options.append({"name": name, "outcomes": outcomes})
        ...
        try:
            ...
            for option in data["options"]:
                if not validate_probabilities(option["outcomes"]):
                    print(
                        f"Warning: Probabilities for '{option['name']}' don't sum to 1.0"
                    )
