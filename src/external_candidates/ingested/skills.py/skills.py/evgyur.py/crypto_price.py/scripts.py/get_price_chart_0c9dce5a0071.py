# Extracted from: C:\DEV\PyAgent\.external\skills\skills\evgyur\crypto-price\scripts\get_price_chart.py
#!/usr/bin/env python3
import json
import math
import os
...
COINGECKO_OHLC_URL = (
    "https://api.coingecko.com/api/v3/coins/{id}/ohlc?vs_currency={currency}&days=1"
)
...
def _post_json(url, payload):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "clawdbot-crypto-price/1.0",
        },
    )
    retry_codes = {429, 502, 503, 504}
    last_error = None
...
def _get_market_chart(token_id, currency, days):
    ...
    if days == 1:
        ...
        url = COINGECKO_MARKET_CHART_DAYS_URL.format(
            id=token_id, currency=currency, days=days
        )
...
def _search_token_id(symbol):
    ...
    if cached is None:
        data = _fetch_json(
            COINGECKO_SEARCH_URL.format(query=urllib.parse.quote(symbol))
        )
        _write_cache(cache_path, data)
    else:
        data = cached
...
def _build_chart(symbol, ohlc_rows, currency, label, use_gradient=False):
    try:
        ...
        font_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "fonts", "Tomorrow.ttf"
        )
    ...
    for idx, row in enumerate(ohlc_rows):
        ...
        if use_gradient:
            ...
            for seg in range(n_segments):
                ...
                r1, g1, b1 = (
                    int(color_bottom[1:3], 16),
                    int(color_bottom[3:5], 16),
                    int(color_bottom[5:7], 16),
                )
                r2, g2, b2 = (
                    int(color_top[1:3], 16),
                    int(color_top[3:5], 16),
                    int(color_top[5:7], 16),
                )
                r = int(r1 + (r2 - r1) * t)
                g = int(g1 + (g2 - g1) * t)
                b = int(b1 + (b2 - b1) * t)
    ...
    for frac_idx, frac_type, frac_price in fractals:
        ...
        if frac_type == "down":  # bearish fractal - arrow down above high
            ax.plot(
                x,
                frac_price + offset * 0.5,
                marker="v",
                color=frac_down_color,
                markersize=6,
                zorder=5,
            )
            ax.annotate(
                f"{frac_price:.2f}",
                xy=(x, frac_price + offset * 1.5),
                fontsize=8,
                color="white",
                ha="center",
                va="bottom",
    ...
    if highs and lows:
        ...
        ax.plot(
            xh,
            abs_high_price + offset * 0.5,
            marker="v",
            color=abs_high_color,
            markersize=7,
            zorder=6,
        )
        ax.annotate(
            f"{abs_high_price:.2f}",
            xy=(xh, abs_high_price + offset * 1.7),
            fontsize=8,
            color="white",
            ha="center",
            va="bottom",
        ...
        ax.annotate(
            f"{abs_low_price:.2f}",
            xy=(xl, abs_low_price - offset * 1.7),
            fontsize=8,
            color="white",
            ha="center",
            va="top",
    ...
    if has_volume and ax_vol and volumes:
        ...
        ax_vol.grid(
            True, linestyle="-", linewidth=0.6, color="#1f2630", alpha=0.8, zorder=1
        )
    ...
    if lows and highs:
        ...
        max_y = max(highs)
        pad = (
            (max_y - min_y) * 0.08 if max_y > min_y else max_y * 0.01
        )  # More padding for fractals
    ...
    if ohlc_rows:
        ...
        ax.axhline(
            y=last_close, color="white", linestyle="--", linewidth=0.8, alpha=0.6
        )
        # Add price label on left side
        ax.annotate(
            f"{last_close:.2f}",
            xy=(ax.get_xlim()[0], last_close),
            fontsize=9,
            color="white",
            fontweight="bold",
            ha="right",
...
def main():
    ...
    use_gradient = any(
        arg.lower() in ("gradient", "grad", "-g", "--gradient") for arg in sys.argv[2:]
    )
    ...
    if source == "hyperliquid":
        ...
        try:
            candles = _get_hyperliquid_candles(
                hl_symbol, total_minutes, interval_minutes
            )
        except RuntimeError:
            candles = []
    ...
    text = (
        f"{symbol_upper}: ${_format_price(price_usdt)} {currency.upper()} {change_text}"
    )
    text = text.replace("*", "")
    result = {
        "symbol": symbol_upper,
