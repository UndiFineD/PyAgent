# Extracted from: C:\DEV\PyAgent\.external\skills\skills\joneschi\sphero-mini\scripts\sphero_mini_bleak.py
import asyncio
import struct
import sys
import time
...
class SpheroMini:
    ...
    async def connect(self):
        ...
        await self.client.connect(timeout=15.0)  # Increased timeout
        ...
        # services = await self.client.get_services()  # Not needed in bleak 2.x
        ...
        await self.client.write_gatt_char(
            AntiDOS_characteristic, b"usetheforce...band", response=True
        )
        ...
        await self.client.start_notify(
            self.API_V2_characteristic, self.handleNotification
        )
    ...
    async def _send(self, characteristic=None, devID=None, commID=None, payload=[]):
        ...
        self.sequence = (
            self.sequence + 1
        ) % 256  # increment seq number to link response packets with this command
        ...
        esc_index = [
            i
            for i in range(len(sendBytes) - 2, 0, -1)  # loop backwards from EOP to SOP
    ...
    async def roll(self, speed=None, heading=None):
        ...
        """
        print(
            "[SEND {}] Rolling with speed {} and heading {}".format(
        ...
        if speed < 0:
            speed = (
                -1 * speed + 256
            )  # speed values > 256 in the send packet make the spero go in reverse
    ...
    async def setLEDColor(self, red=None, green=None, blue=None):
        ...
        """
        print(
            "[SEND {}] Setting main LED colour to [{}, {}, {}]".format(
    ...
    async def getAcknowledgement(self, ack):
        ...
        while 1:
            ...
            if (
                self.notification_seq == (self.sequence - 1) % 256
            ):  # use one less than sequence, because _send function increments it for next send.
            ...
            if time.time() > start + 10:
                print(
                    "Timeout waiting for acknowledgement: {}/{}".format(
                        ack, self.sequence
                    ),
                    file=sys.stderr,
                )
    ...
    def handleNotification(self, sender, data):
        ...
        self.notificationPacket.extend(
            data
        )  # add new single or multiple bytes to packet list

        # discard if first byte not start-of-packet
        if self.notificationPacket[0] != sendPacketConstants["StartOfPacket"]:
            print(
                "Warning: discarding unexpected data before SOP:",
                self.notificationPacket,
                file=sys.stderr,
            )
            self.notificationPacket = []
        ...
        # parse on EndOfPacket received structure similar to send packets (see docstring in sphero_mini._send())
            ...
            esc_index = [
                i
                for i in range(
                    len(self.notificationPacket) - 2, 0, -1
                )  # descending index between EOP and SOP
            ...
            try:
                ...
                print(
                    "Warning: notification packet unparseable",
                    self.notificationPacket,
                    file=sys.stderr,
                )
            ...
            if (
                checksum != chsum
            ):  # check computed checksum against that recieved in the packet
                print(
                    "Warning: notification packet checksum failed",
                    self.notificationPacket,
                    file=sys.stderr,
                )
                self.notificationPacket = []  # Discard this packet
            ...
            if flags_bits & flags["isResponse"]:  # it is a response
                ...
                if (
                    devid == deviceID["powerInfo"]
                    and commcode == powerCommandIDs["wake"]
                ):
                    self.notification_ack = (
                        "Wake acknowledged"  # Acknowledgement after wake command
                    )

                elif (
                    devid == deviceID["driving"]
                    and commcode == drivingCommands["driveWithHeading"]
                ):
                    self.notification_ack = "Roll command acknowledged"

                elif (
                    devid == deviceID["driving"]
                    and commcode == drivingCommands["stabilization"]
                ):
                    self.notification_ack = "Stabilization command acknowledged"

                elif (
                    devid == deviceID["userIO"]
                    and commcode == userIOCommandIDs["allLEDs"]
                ):
                    self.notification_ack = "LED/backlight color command acknowledged"

                elif (
                    devid == deviceID["driving"]
                    and commcode == drivingCommands["resetHeading"]
                ):
                    self.notification_ack = "Heading reset command acknowledged"

                elif (
                    devid == deviceID["sensor"]
                    and commcode == sensorCommands["configureCollision"]
                ):
                    self.notification_ack = (
                        "Collision detection configuration acknowledged"
                    )

                elif (
                    devid == deviceID["sensor"]
                    and commcode == sensorCommands["configureSensorStream"]
                ):
                    self.notification_ack = "Sensor stream configuration acknowledged"

                elif (
                    devid == deviceID["sensor"]
                    and commcode == sensorCommands["sensorMask"]
                ):
                    self.notification_ack = "Mask configuration acknowledged"

                elif (
                    devid == deviceID["sensor"]
                    and commcode == sensorCommands["sensor1"]
                ):
                    self.notification_ack = "Sensor1 acknowledged"

                elif (
                    devid == deviceID["sensor"]
                    and commcode == sensorCommands["sensor2"]
                ):
                    self.notification_ack = "Sensor2 acknowledged"

                elif (
                    devid == deviceID["powerInfo"]
                    and commcode == powerCommandIDs["batteryVoltage"]
                ):
                    V_batt = (
                        notification_payload[2]
                        + notification_payload[1] * 256
                        + notification_payload[0] * 65536
                    )
                    ...
                    and commcode == SystemInfoCommands["mainApplicationVersion"]
                ):
                    ...
                    self.notification_ack = (
                        "Unknown acknowledgement"  # print(self.notificationPacket)
                    )
                    print(
                        self.notificationPacket,
                        "===================> Unknown ack packet",
                ...
                if (
                    ...
                    if self.collision_detection_callback is not None:
                        self.notificationPacket = (
                            []
                        )  # need to clear packet, in case new notification comes in during callback
                        self.collision_detection_callback()

                # Sensor response:
                elif (
                    devid == deviceID["sensor"]
                    and commcode == sensorCommands["sensorResponse"]
                ):
