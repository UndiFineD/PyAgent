# Extracted from: C:\DEV\PyAgent\.external\skills\skills\aiasisbot61\4chan-reader\scripts\chan_extractor.py
#!/usr/bin/env python3
...
def get_thread(board, thread_id, output_root=None, word_limit=None):
    ...
    for post_id in post_ids:
        ...
        file_match = re.search(
            rf'<div class="fileText" id="fT{post_id}">File: <a [^>]+>([^<]+)</a>',
            html_content,
        )
        ...
        # Message content
        msg_match = re.search(
            rf'<blockquote class="postMessage" id="m{post_id}">(.*?)</blockquote>',
            html_content,
            re.DOTALL,
        )
...
def show_usage():
    ...
    print(
        "  python3 chan_extractor.py thread <board> <thread_id> [output_root_dir] [word_limit]"
    )
    print("\nExamples:")
