...
def detect_event_metadata(
    note, tags, provided_type=None, provided_date=None, source="chat"
):
...
def recall(person, limit):
    ...
    for idx, note in enumerate(reversed(notes[-limit:]), 1):
        ...
        print(
            f"{idx}. {note['note']} (source: {note['source']}, {note['timestamp']}){tags}"
        )
...
def search(query, limit):
    ...
    if person_keys:
        for idx, key in enumerate(person_keys[:limit], 1):
            ...
            print(
                f"{idx}. {person['displayName']}: {latest['note']} ({latest['timestamp']})"
            )
        return
    matches = []
...
def list_people():
    store = load_store()
    people = sorted(
        store.get("people", {}).items(), key=lambda x: x[1]["displayName"].lower()
    )
    if not people:
    ...
    for _, entry in people:
        print(
            f"- {entry['displayName']} ({len(entry['notes'])} notes, updated {entry['notes'][-1]['timestamp']})"
        )
...
def format_reminder_message(events, reference_date, lookahead, fmt="text"):
    ...
    if lookahead == 0:
        header = (
            f"People memories reminders for {reference_date.strftime('%B %d, %Y')}:"
        )
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Store and recall informal notes about people."
    )
    ...
    remind_parser = subparsers.add_parser(
        "reminders", help="List upcoming birthday/anniversary reminders."
    )
