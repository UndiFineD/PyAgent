# Extracted from: C:\DEV\PyAgent\.external\skills\skills\artur-zhdan\humanize-ai\scripts\humanize.py
#!/usr/bin/env python3
...
def humanize(
    text: str, fix_dashes: bool = False, remove_artifacts: bool = True
) -> tuple[str, list[str]]:
    all_changes = []
...
def main():
    ...
    parser.add_argument(
        "--fix-dashes", action="store_true", help="Replace em dashes with commas"
    )
    parser.add_argument(
        "--keep-artifacts", action="store_true", help="Don't remove chatbot artifacts"
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Only output result, no changes log"
    )
    ...
    result, changes = humanize(
        text, fix_dashes=args.fix_dashes, remove_artifacts=not args.keep_artifacts
    )
