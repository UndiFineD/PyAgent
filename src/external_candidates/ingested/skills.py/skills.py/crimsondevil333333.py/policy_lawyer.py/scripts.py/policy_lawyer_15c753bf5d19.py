...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Policy Lawyer: recall workspace policies instantly."
    )
    parser.add_argument(
        "--policy-file",
        type=Path,
    ...
    group.add_argument(
        "--list-topics", action="store_true", help="List every named policy section."
    )
    group.add_argument("--topic", help="Show the section matching this topic name.")
    group.add_argument(
        "--keyword", help="Search for a keyword inside every policy section."
    )
