# Extracted from: C:\DEV\PyAgent\.external\skills\skills\paul-phan\crisp\scripts\crisp.py
#!/usr/bin/env python3
"""
...
if not WEBSITE_ID or not TOKEN_ID or not TOKEN_KEY:
    print(
        "❌ Error: CRISP_WEBSITE_ID, CRISP_TOKEN_ID, and CRISP_TOKEN_KEY environment variables required"
    )
    print("Set them with:")
...
def message_send(session_id: str, text: str):
    ...
    payload = {"type": "text", "content": text, "from": "operator", "origin": "chat"}
...
def conversation_mark_read(session_id: str):
    ...
    payload = {"from": "operator", "read": True}
...
def conversations_search(
    query: str, filter_type: Optional[str] = None, max_results: int = 10
):
