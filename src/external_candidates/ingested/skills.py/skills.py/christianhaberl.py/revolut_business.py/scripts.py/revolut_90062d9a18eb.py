...
# --- Config ---
STATE_DIR = Path(
    os.environ.get("REVOLUT_DIR", os.path.expanduser("~/.clawdbot/revolut"))
)
...
def cmd_transactions(args):
    ...
    if args.account:
        ...
        for acc in accounts:
            if (
                acc.get("name", "").lower() == args.account.lower()
                or acc["id"] == args.account
            ):
    ...
    print(
        f"{'Date':<12} {'Type':<15} {'Description':<30} {'Amount':>10} {'Cur':<5} {'State':<10}"
    )
    print("-" * 90)
    for tx in txns:
        ...
        print(
            f"{date:<12} {tx_type:<15} {desc:<30} {symbol}{amount:>9,.2f} {currency:<5} {state:<10}"
        )
...
def cmd_pay(args):
    ...
    if args.draft:
        ...
        print(
            f"⚠️  About to send {args.currency or 'EUR'} {args.amount:.2f} to {args.counterparty or args.account_id}"
        )
        if not args.yes:
        ...
        print(
            f"✅ Payment sent! ID: {result.get('id', '?')}, State: {result.get('state', '?')}"
        )
...
def cmd_export(args):
    ...
    for tx in txns:
        ...
        for leg in tx.get("legs", []):
            ...
            writer.writerow(
                [date, tx_type, desc, amount, currency, balance, state, merchant, card]
            )
...
def cmd_token_info(args):
    ...
    print(
        f"Obtained:      {datetime.fromtimestamp(obtained).strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print(f"Expires in:    {max(0, int(remaining))}s ({max(0, int(remaining/60))}min)")
    print(
        f"Refresh Token: {'✅ present' if tokens.get('refresh_token') else '❌ missing'}"
    )
...
def main():
    ...
    p_pay.add_argument(
        "--draft", action="store_true", help="Create draft instead of sending"
    )
