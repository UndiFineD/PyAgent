# Extracted from: C:\DEV\PyAgent\.external\skills\skills\christianhaberl\boggle\scripts\solve.py
#!/usr/bin/env python3
"""
...
def load_dictionaries(paths, min_word_len=DEFAULT_MIN_WORD_LEN):
    ...
    for path in paths:
        ...
        print(
            f"Loaded {os.path.basename(path)}: {added} new words ({len(words)} total)",
            file=sys.stderr,
        )
    return words
...
def parse_board(args):
    ...
    if args.letters:
        ...
        if len(tiles) != 16:
            raise ValueError(
                f"Need 16 tiles, got {len(tiles)} (hint: 'Qu' counts as one tile)"
            )
    ...
    if args.rows:
        ...
        if len(rows) != 4 or any(len(r) != 4 for r in rows):
            raise ValueError(
                "Need 4 rows of 4 tiles each (hint: 'Qu' counts as one tile)"
            )
        return rows
...
def main():
    ...
    parser.add_argument(
        "rows", nargs="*", help="4 rows of 4 letters (e.g., HOPL NERI NIDO IEOT)"
    )
    parser.add_argument("--letters", "-l", help="All 16 letters as one string")
    parser.add_argument(
        "--dict", "-d", action="append", dest="dicts", help="Dictionary file(s)"
    )
    parser.add_argument(
        "--min", type=int, default=3, help="Minimum word length (default: 3)"
    )
    ...
    parser.add_argument(
        ...
        choices=["en", "de"],
        default="en",
        help="Dictionary language: 'en' for English, 'de' for German (run once per language for bilingual boards)",
    )
    ...
    t2_trie = time.time()
    print(
        f"Dictionary loaded in {t1-t0:.2f}s, trie built in {t2_trie-t1:.2f}s",
        file=sys.stderr,
    )
    ...
    if args.json:
        ...
        result = {
            ...
            "words": [
                {"word": w, "length": len(w), "score": score_word(w)}
