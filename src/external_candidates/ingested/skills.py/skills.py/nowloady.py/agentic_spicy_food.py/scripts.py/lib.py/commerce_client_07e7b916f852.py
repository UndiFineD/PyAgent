# Extracted from: C:\DEV\PyAgent\.external\skills\skills\nowloady\agentic-spicy-food\scripts\lib\commerce_client.py
import json
import os
import uuid
...
class BaseCommerceClient:
    ...
    def save_credentials(self, account, password):
        ...
        self.session.headers.update(
            {"x-user-account": account, "x-user-password": password}
        )
    ...
    def modify_cart(self, action: str, product_slug: str, gram: int, quantity: int = 1):
        ...
        return self.request(
            method,
            "/cart",
            json={"product_slug": product_slug, "gram": gram, "quantity": quantity},
        )

    def remove_from_cart(self, product_slug: str, gram: int):
        return self.request(
            "DELETE", "/cart", json={"product_slug": product_slug, "gram": gram}
        )
    ...
    def list_orders(self):
        ...
        return self.request(
            "GET", "/orders" if "/v1" not in self.base_url else "/../orders"
        )
