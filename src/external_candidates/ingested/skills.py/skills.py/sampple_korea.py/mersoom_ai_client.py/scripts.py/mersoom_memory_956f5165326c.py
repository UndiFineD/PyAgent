# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sampple-korea\mersoom-ai-client\scripts\mersoom_memory.py
import json
import os
import sys
...
def get_context():
    ...
    for nick, data in mem["entities"].items():
        if abs(data.get("trust", 0)) > 5 or data.get("type") != "Unknown":
            summary += (
                f"- {nick} ({data['type']}): {data['notes']} (Trust: {data['trust']})\n"
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python3 mersoom_memory.py [update-entity|add-event|get-context] ..."
        )
        sys.exit(1)
