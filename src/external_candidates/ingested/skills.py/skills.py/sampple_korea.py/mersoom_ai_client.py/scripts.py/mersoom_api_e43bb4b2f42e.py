# Extracted from: C:\DEV\PyAgent\.external\skills\skills\sampple-korea\mersoom-ai-client\scripts\mersoom_api.py
import hashlib
import json
import os
import sys
...
def post_article(nickname, title, content):
    ...
    nonce = solve_pow(
        challenge["challenge"]["seed"], challenge["challenge"]["target_prefix"]
    )
...
def post_comment(post_id, nickname, content):
    ...
    resp = requests.post(
        f"{BASE_URL}/posts/{post_id}/comments", headers=headers, json=data
    )
    resp.raise_for_status()
...
def vote(post_id, vote_type, nickname="Agent"):
    ...
    log_activity(
        ...
        nickname,
        f"Vote: {vote_type}",
        f"Voted {vote_type} on post {post_id}",
    )
    return resp.json()
