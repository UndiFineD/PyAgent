# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\common\stream\stream.py
import base64
import binascii
import io
import string
...
class BasicType:
    def __init__(
        self, data_type: DataType, size: int, sign: Sign, endianness: Endianness
    ):
...
def bytesne(x):
    return BasicType(DataType.BYTES, x, Sign.NONE, Endianness.DEFAULT)


def bytesle(x):
    return BasicType(DataType.BYTES, x, Sign.NONE, Endianness.LITTLE)


def bytesbe(x):
    return BasicType(DataType.BYTES, x, Sign.NONE, Endianness.BIG)


def ascii_string(x):
    return BasicType(DataType.ASCII_STRING, x, Sign.NONE, Endianness.DEFAULT)


def nt_ascii_string(x):
    return BasicType(
        DataType.NULL_TERMNINATED_ASCII_STRING, x, Sign.NONE, Endianness.DEFAULT
    )
...
class DataStruct:
    ...
    def get_members(cls: object) -> List[Tuple[str, BasicType]]:
        return list(
            filter(
                lambda x: not x[0].startswith("__") and not x[0].startswith("__"),
                cls.__dict__.items(),
            )
        )
...
class BinaryStream(object):
    ...
    def __init__(
        self, base_stream: bytes = b"", endianness: Endianness = Endianness.LITTLE
    ) -> None:
        if self not in BinaryStream.DEBUG_DATA:
            BinaryStream.DEBUG_DATA[self] = []

        self._int_size_map = {BITS_8: "b", BITS_16: "h", BITS_32: "i", BITS_64: "q"}

        self._float_size_map = {BITS_32: "f", BITS_64: "d"}
    ...
    def _is_le(self, endianness: Endianness) -> bool:
        true_endianness = (
            self._endianness if endianness == Endianness.DEFAULT else endianness
        )
    ...
    def _build_struct_format(
        self, dt: DataType, size: int, sign: Sign, endianness: Endianness
    ) -> str:
        code = None
        ...
        if dt == DataType.INTEGER:
            ...
            DataType.NULL_TERMNINATED_ASCII_STRING,
        ]:
            code = f"{size}s"
...
class BinaryReader(BinaryStream):
    ...
    def peek(self, basic_type: BasicType) -> Any:
        return struct.unpack(
            self._build_struct_format(
                basic_type.data_type,
                basic_type.size,
                basic_type.sign,
                basic_type.endianness,
            ),
            self.peek_bytes(basic_type.size),
        )[0]
...
def debug_print():
    ...
    for key, val in BinaryStream.DEBUG_DATA.items():
        ...
        for item in val:
            ...
            all_data += raw
            print_res += (
                " " * spaces + f"{dt_name} = {obj_repr} -> {hexdump.dump(raw)}\n"
            )
