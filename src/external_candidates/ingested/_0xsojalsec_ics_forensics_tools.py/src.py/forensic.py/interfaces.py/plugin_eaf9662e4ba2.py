    ...
    def __init__(self, config: PluginConfig, output_dir: Path, verbose: bool):
        ...
        if verbose:
            self.logger_handler.create_log_file(
                os.path.join(
                    os.getcwd(), constants.LOGS_DIR, f"{self.config.name}Plugin.log"
                )
            )
...
class PluginCLI(ABC):
    ...
    def get_analyzer_choices(self):
        analyzers = modules.list_subclasses(
            AnalyzerInterface, f"{constants.APPLICATION}.analyzers.{self.folder_name}."
        )
...
class PluginConfig(object):
    ...
    def from_json(json: Dict) -> PluginConfig:
        return PluginConfig(
            ...
            analyzers=[
                AnalyzerConfig.from_json(analyzer) for analyzer in json["analyzers"]
            ],
        )
