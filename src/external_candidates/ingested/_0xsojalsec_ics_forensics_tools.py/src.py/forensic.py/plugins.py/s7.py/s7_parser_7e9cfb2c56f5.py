...
class S7Parser:
        ...
        if layer.param_len > 0:
            if layer.rosctr == ROSCTR_ACK_DATA:
                ...
                if function == PARAM_FUNCTION_SETUP_COMM:
                    ...
                    block_size = br.read(bytesbe(param.block_size_length)).replace(
                        b"\x00", b"0"
                    )
    ...
    def parse_user_data(self, layer: S7LayerResponse, br: BinaryReader) -> Any:
        ...
        if layer.param is not None and layer.data_len > 0:
            ...
            if seq_num != 0:
                ...
                if not (
                    layer.param.last_data_unit == NOT_LAST_DATA_UNIT
                    and layer.param.sequence_num != 0
                ):
    ...
    def parse_list_blocks_data(
        self, br: BinaryReader, fragment: S7DataFragment
    ) -> S7ListBlocks:
        res = S7ListBlocks(br)
        res.entries = []
    ...
    def parse_list_blocks_of_type_data(
        self, br: BinaryReader, fragment: S7DataFragment
    ) -> S7ListBlocks:
        res = S7ListBlocks(br)
        res.entries = []
    ...
    def build_user_data_packet(
        ...
        if sequence_num == 0:
            ...
            param = self.build_user_data_fragmented(
                function, sub_function, sequence_num
            )

            data = BinaryWriter()
            data.write_struct(
                S7DataFragment, {"return_code": 0x0A, "transport_size": 0, "length": 0}
            )

        return self.write_header(ROSCTR_USER_DATA, param, data)

    def write_read_szl(
        self, szl_id: int, szl_index: int = 0, sequence_num: int = 0
    ) -> BinaryWriter:
        if szl_index != 0:
            szl_id = szl_id | 0x100
        ...
        return self.build_user_data_packet(
            FUNCTION_CPU, SUBFUNCTION_READ_SZL, data, sequence_num
        )

    def write_list_blocks(self, sequence_num: int = 0) -> BinaryWriter:
        data = BinaryWriter()
        data.write_struct(
            S7EmptyRequest, {"return_code": 0x0A, "transport_size": 0, "length": 0}
        )

        return self.build_user_data_packet(
            FUNCTION_BLOCK, SUBFUNCTION_LIST_BLOCK, data, sequence_num
        )

    def write_list_blocks_of_type(
        self, block_type: str, sequence_num: int = 0
    ) -> BinaryWriter:
        ...
        return self.build_user_data_packet(
            FUNCTION_BLOCK, SUBFUNCTION_LIST_BLOCKS_OF_TYPE, data, sequence_num
        )
