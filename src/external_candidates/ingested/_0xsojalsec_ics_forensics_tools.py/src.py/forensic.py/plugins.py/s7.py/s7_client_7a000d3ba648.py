# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\s7\s7_client.py
import socket
...
from forensic.plugins.s7.cotp import COTP, COTP_CR, COTP_DT, COTPLayer
from forensic.plugins.s7.s7_parser import (
    NO_ERROR,
    NOT_LAST_DATA_UNIT,
    S7Error,
    S7ErrorNotPermitted,
...
class S7Conn(object):
    def __init__(
        self, logger, ip: str, port: int = 102, rack: int = 0, slot: int = 0, timeout=20
    ) -> None:
    ...
    def create_session(self, rack: int = 0, slot: int = 0):
        data = (
            TPKT()
            .write(3, COTP().write_cr(0, 0x14, 0x100, 0x100 + (rack * 0x20) + slot))
            .dump()
        )
    ...
    def parse_cotp(
        self, br: BinaryReader, is_recurse: bool = False
    ) -> [COTPLayer, BinaryReader]:
        cotp = None
        ...
        if br.remaining_data() > TPKTLayer.get_size():
            ...
            if tpkt.real_length > COTPLayer.get_size():
                ...
                # Not last data unit
                if (
                    isinstance(cotp.cotp_data, COTP_DT)
                    and cotp.cotp_data.last_data_unit == 0
                ):
                    if tpkt.length > 7:
    ...
    def parse_s7(self, cotp: COTPLayer, br: BinaryReader) -> S7LayerResponse:
        ...
        if br.remaining_data() > 0 and isinstance(cotp.cotp_data, COTP_DT):
            try:
                ...
                self.logger.warning(
                    f"Stream not enough data for IP: {self._ip}, "
    ...
    def read_szl(self, szl_id: int, szl_index: int = 0) -> List[DataStruct]:
        ...
        while (
            ...
            and response.param.last_data_unit == NOT_LAST_DATA_UNIT
            and response.param.sequence_num != 0
            and n_limit > 0
        ):
            response = self._req_rep(
                self._s7_parser.write_read_szl(
                    szl_id, szl_index, response.param.sequence_num
                )
            )
            n_limit -= 1
    ...
    def list_blocks(self) -> List[S7ListBlocksEntry]:
        ...
        while (
            ...
            response = self._req_rep(
                self._s7_parser.write_list_blocks(response.param.sequence_num)
            )
            n_limit -= 1

        return (
            [] if (response is None or response.data is None) else response.data.entries
        )
    ...
    def list_blocks_of_type(self, block_type: str) -> List[S7ListBlocksOfTypeEntry]:
        ...
        while (
            ...
            response = self._req_rep(
                self._s7_parser.write_list_blocks_of_type(
                    block_type, response.param.sequence_num
                )
            )
            n_limit -= 1
        ...
        try:
            for block_count_entry in self.list_blocks():
                if block_count_entry.block_count > 0:
                    try:
                        for block_entry in self.list_blocks_of_type(
                            block_count_entry.block_type
                        ):
                            res.append(
                                {
                                    "type": block_count_entry.block_type,
                                    "language": block_entry.block_lang,
                                    "num": block_entry.block_number,
                                    "flags": block_entry.block_flags,
                                }
                            )
                    except S7Error:
                        self.logger.warning(
                            f"S7 client blocks error for IP: {self._ip}, "
                            f"Port: {self._port}, Rack: {self._rack}, Slot: {self._slot}"
                        )
        ...
        for block in self.blocks():
            ...
            for _ in range(5):
                try:
                    ...
                    self.logger.warning(
                        f"Upload all blocks exception {e} for IP: {self._ip}, "
                        f"Port: {self._port}, Rack: {self._rack}, Slot: {self._slot}"
                    )
    ...
    def dump_plc(self):
        ...
        if szl_0_res:
            res = {
                "rack": self._rack,
                "slot": self._slot,
                "supported_szl": list(map(lambda x: x.szl_id, szl_0_res)),
                "szl": dict(),
            }
