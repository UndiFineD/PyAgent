...
class S7(PluginInterface):
    ...
    @retry(
        retry=(
            retry_if_exception_type(TimeoutError) | retry_if_exception_type(S7Error)
        ),
        ...
        try:
            ...
            with S7Conn(self.logger, address, self.config.port, rack, slot) as s7:
                ...
                if data:
                    ...
                    self.logger.info(
                        f"Successfully connected to IP: {address}, "
                        f"Port: {self.config.port}, Rack: {rack}, Slot: {slot}"
                    )
        except InvalidS7RackSlot:
            self.logger.info(
                f"Invalid S7 rack slot for IP: {address}, "
                f"Port: {self.config.port}, Rack: {rack}, Slot: {slot}"
            )
            ...
            self.logger.info(
                f"Connection reset error for IP: {address}, "
                f"Port: {self.config.port}, Rack: {rack}, Slot: {slot}"
            )
