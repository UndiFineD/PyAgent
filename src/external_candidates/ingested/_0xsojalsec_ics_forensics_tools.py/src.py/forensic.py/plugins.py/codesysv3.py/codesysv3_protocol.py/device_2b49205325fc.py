# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\CodeSysV3\codesysv3_protocol\device.py
import enum
import logging
import socket
...
class CodeSysV3Device:
    ...
    def recv_pdu(self) -> list:
        if self._datagram_type == DatagramLayerType.TCP:
            ...
            layers = CodeSysV3Protocol.parse_CodeSysTCPBlockDriverLayer(
                pdu + self._socket.recv(tcp_length)
            )
        else:
            layers = CodeSysV3Protocol.parse_CodeSysDatagramLayer(
                self._socket.recvfrom(MAX_PDU_SIZE)[0]
            )
    ...
    def open_channel(self) -> CodeSysV3Channel:
        ...
        self.send_pdu(
            DatagramLayerServices.ChannelManager,
    ...
    def _prepared_datagram_layer(self, service: DatagramLayerServices, payload: bytes):
        if self._datagram_type == DatagramLayerType.TCP:
            return CodeSysV3Protocol.build_DatagramLayerRequestOverTCP(
                self._src_ip,
                self._src_port,
                self._dst_ip,
                self._src_port,
            ...
            return CodeSysV3Protocol.build_DatagramLayerRequestOverUDP(
                self._src_ip,
                self._src_port,
                service,
                payload,
    ...
    def _get_dst_address(self):
        ...
        try:
            ...
            datagram = CodeSysV3Protocol.build_DatagramLayerRequestOverUDP(
                self._src_ip,
                CodeSysV3Device.UDP_PORT,
                DatagramLayerServices.NSServer,
                ns_server,
            )
            ...
            self.logger.error(
                f"Fail to get the target device address: {self._dst_ip}, Error: {ex}"
            )
        finally:
            if sock is not None:
                sock.close()
    ...
    def _determine_datagram_layer_type(self):
        self.logger.info(
            f"Trying to determine the datagram layer type to connect to {self._dst_ip}"
        )
    ...
    def _check_for_datagram_layer_tcp(self):
        ...
        try:
            ...
            datagram = CodeSysV3Protocol.build_DatagramLayerRequestOverTCP(
                self._src_ip,
                src_port,
                self._dst_ip,
    ...
    def read_over_channel(
        self, channel_id: int
    ) -> typing.Tuple[ApplicationBlockFirst, typing.List[Tag], int]:
        if not self.is_connected():
            raise CodeSysProtocolV3Exception("Device is not connected")
        pdu = self.recv_pdu()
            ...
            if channel_layer.channel_id == channel_id:
                ...
                if channel_layer.remaining_data_size > len(service_layer_data):
                    ...
                    while len(service_layer_data) < channel_layer.remaining_data_size:
                        ...
                        if (
                            len(pdu) > 1
                            and isinstance(pdu[-2], ApplicationBlock)
                            and pdu[-2].channel_id == channel_id
                            and pdu[-2].ack_id == channel_layer.ack_id
                            ...
                            continue
                service_layer, tags = CodeSysV3Protocol.parse_CodeSysServiceLayer(
                    service_layer_data
                )
        return service_layer, tags, last_blk_id

    def send_over_channel(
        self,
        channel_id: int,
        cmd_group: CmdGroup,
        smb_cmd: int,
        ...
        if not self.is_connected():
            raise CodeSysProtocolV3Exception("Device is not connected")
        service_layer = CodeSysV3Protocol.build_CodeSysServicesLayer(
            cmd_group, smb_cmd, session_id, tags
        )
        channel_layer_pkts = CodeSysV3Protocol.build_CodeSysChannelLayerAppBlk(
            channel_id, blk_id, ack_id, service_layer
        )
