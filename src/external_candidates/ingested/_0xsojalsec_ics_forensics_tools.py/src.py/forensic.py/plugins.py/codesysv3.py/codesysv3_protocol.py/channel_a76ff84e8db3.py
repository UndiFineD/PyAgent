# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\CodeSysV3\codesysv3_protocol\channel.py
import logging
import pathlib
...
class CodeSysV3Channel:
    ...
    def send(self, cmd_group: CmdGroup, smb_cmd: int, *tags: Tag):
        if tags is None:
            tags = []
        self._blk_id += self._device.send_over_channel(
            self._channel_id,
            cmd_group,
            smb_cmd,
            self._session_id,
    ...
    def login(self, username: str = "", password: str = "") -> bool:
        ...
        try:
            ...
            if len(password) > 0:
                password_hash = CodeSysV3Encryption.hash_password(
                    CodeSysV3Encryption.CHALLENGE, password
                )
                tags.append(
                    Tag(0x23, CodeSysV3Encryption.CHALLENGE.to_bytes(4, "little"))
                )
                username_pass_tag = Tag(0x81)
                tags.append(username_pass_tag)
                username_pass_tag.add_tag(
                    Tag(0x10, bytes(username, "ascii"), align=0x42)
                )
                ...
                tags.append(username_pass_tag)
                username_pass_tag.add_tag(
                    Tag(0x10, bytes(username, "ascii"), align=0x40)
                )
            self.send(CmdGroup.CmpDevice, 2, *tags)
            service_layer, tags = self.read()
            if (
                service_layer is not None
                and len(tags) > 0
                and service_layer.cmd_group == CmdGroup.CmpDevice.value
                and service_layer.subcmd == 2
            ):
                session_id = tags[0x82].get_tag(0x21)
                if session_id is not None:
                    self._session_id = session_id.dword_le
                    self.logger.info(
                        f"Login successfully to the device, session id: {self._session_id: 08X}"
                    )
                    return True

                else:
                    self.logger.error(
                        f"Failed to login to the device, error_code: {tags[0x82][0x20].word: 04X}"
                    )
