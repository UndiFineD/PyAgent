...
class CodeSysV3Protocol:
    ...
    def _build_DatagramLayer(
        ...
        dg_layer = (
            bytes(DatagramLayer(dg_service, len(dst_address), len(src_address)))
            + dst_address
            + src_address
        )
    ...
    def build_DatagramLayerRequestOverUDP(
        ...
        src_ip_format = CodeSysV3Protocol._get_codesys_address_format(
            src_ip, src_port, netmask
        )
        ...
        return CodeSysV3Protocol._build_DatagramLayer(
            0, src_address, dst_address, dg_service, payload
        )
    ...
    def build_CodeSysChannelLayerAppBlk(
        ...
        first_max_pkt_size = (
            MAX_PDU_SIZE - header_size - ctypes.sizeof(ApplicationBlockFirst)
        )
        second_max_pkt_size = (
            MAX_PDU_SIZE - header_size - ctypes.sizeof(ApplicationBlock)
        )
        first_pkt = (
            bytes(
                ApplicationBlockFirst(
                    payload, blk_id=blk_id, ack_id=ack_id, channel_id=channel_id
                )
            )
        ...
        while payload_offset < len(payload):
            ...
            pkt = (
                bytes(
                    ApplicationBlock(
                        blk_id=blk_id, ack_id=ack_id, channel_id=channel_id
                    )
                )
    ...
    @staticmethod
    def build_CodeSysServicesLayer(
        cmd_group: CmdGroup, subcmd: int, session_id: int, tags: typing.List[Tag]
    ) -> bytes:
        tags_layer = b""
        for t in tags:
        ...
        if len(pkt) >= ctypes.sizeof(BlockDriverLayerTcp):
            ...
            if block_layer.tcp_magic != TCP_MAGIC:
                ...
                raise CodeSysProtocolV3Exception(
                    f"Missing pkt bytes, total size:{block_layer.tcp_length}, "
                    f"received: {len(pkt)}"
                )
            layers.append(block_layer)
            layers += CodeSysV3Protocol.parse_CodeSysDatagramLayer(
                pkt, ctypes.sizeof(BlockDriverLayerTcp)
            )
        return layers
    ...
    def parse_CodeSysNSClient(pkt: bytes, offset: int) -> list:
        ...
        if len(pkt) >= ctypes.sizeof(NsHeader) + offset:
            ...
            if (
                nsclient_layer.subcmd == NSSubCmd.DeviceInfo.value
                and nsclient_layer.version in (0x103, 0x400)
            ):
                if len(pkt) >= ctypes.sizeof(NsClientDeviceInfo) + offset:
                    nsclient_device_info_layer = NsClientDeviceInfo.from_buffer_copy(
                        pkt, offset
                    )
                    ...
                    if len(pkt) >= offset + total_strings_name:
                        nsclient_device_info_layer.node_name = pkt[
                            offset : offset
                            + 2 * (nsclient_device_info_layer.node_name_length)
                        ].decode("UTF-16-LE")
                        offset += 2 * (nsclient_device_info_layer.node_name_length + 1)
                        nsclient_device_info_layer.device_name = pkt[
                            offset : offset
                            + 2 * (nsclient_device_info_layer.device_name_length)
                        ].decode("UTF-16-LE")
                        offset += 2 * (
                            nsclient_device_info_layer.device_name_length + 1
                        )
                        nsclient_device_info_layer.vendor_name = pkt[
                            offset : offset
                            + 2 * (nsclient_device_info_layer.vendor_name_length)
                        ].decode("UTF-16-LE")
                        offset += 2 * (
                            nsclient_device_info_layer.vendor_name_length + 1
                        )
    ...
    def parse_CodeSysChannelLayer(pkt: bytes, offset: int) -> list:
        if len(pkt) >= offset + 1:
            ...
            if type == ChannelLayerType.OpenChannelResponse.value:
                ...
                return CodeSysV3Protocol.parse_CodeSysChannelLayerOpenChannelRes(
                    pkt, offset
                )
    ...
    def parse_CodeSysChannelLayerAppBlk(pkt: bytes, offset: int) -> list:
        if len(pkt) >= offset + 2:
            flags = pkt[offset + 1]
            if flags & 0x01 and len(pkt) >= offset + ctypes.sizeof(
                ApplicationBlockFirst
            ):
                return [
