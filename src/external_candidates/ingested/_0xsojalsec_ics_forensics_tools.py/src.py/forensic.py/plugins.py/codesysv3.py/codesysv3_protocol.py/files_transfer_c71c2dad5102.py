# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\plugins\CodeSysV3\codesysv3_protocol\files_transfer.py
import logging
import pathlib
import typing
...
class FilesTransfer:
    ...
    def download(self, src_file: str, dst_file: pathlib.Path):
        ...
        try:
            self.logger.info(
                f"Trying to download the file {src_file} over the channel {self._channel.channel_id: 04X}"
            )
            ...
            self._channel.send(
                CmdGroup.CmpFileTransfer,
                5,
                Tag(0x01, src_file),
                Tag(0x02, bytearray(8)),
            )
            ...
            if error_code == 0:
                ...
                file_size_data[0] = 1
                self.logger.info(
                    f"Successfully open the file {full_file_path} over the channel {self._channel.channel_id: 04X}"
                ...
                with dst_file.open("wb") as dst_fp:
                    ...
                    while written < file_size:
                        self._channel.send(
                            CmdGroup.CmpFileTransfer, 7, Tag(0x05, session_id)
                        )
                        resp, tags = self._channel.read()
                        ...
                        if tag_data is None:
                            self.logger.error(
                                f"Failed to download the file {src_file}, fail to get file's data"
                            )
                ...
                self._channel.send(
                    CmdGroup.CmpFileTransfer,
                    8,
                    Tag(0x07, session_id + src_file),
                    Tag(0x02, file_size_data),
                )
                layer, tags = self._channel.read()
            else:
                self.logger.error(
                    f"Successfully open the file {src_file}, error_code:{error_code}"
                )
    ...
    def dir(self, folder: str):
        ...
        try:
            self.logger.info(
                f"Trying to get folder list: {folder} over the channel {self._channel.channel_id: 04X}"
            )
