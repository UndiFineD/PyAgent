# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\s7\raw_file_parser.py
import json
import os
...
class S7RawFileParser(AnalyzerInterface):
    ...
    def parse_blocks(self, parsed_slot_rack, blocks):
        for block_name, block in blocks.items():
            try:
                parsed_slot_rack["blocks"].append(
                    MC7Parser().parse(b64decode(block["data"].encode()))
                )
