...
class S7BlockLogic(AnalyzerInterface):
    ...
    def get_called_blocks_edges(self, block_id, called_blocks):
        if (not called_blocks) or (called_blocks != called_blocks):  # nan
            return []
        return [
            (block_id, called_block.replace(" ", "_")) for called_block in called_blocks
        ]

    def call_tree_layers(self, layers, edges, nodes):
        ob_calling_edges = list(filter(lambda edge: edge[0].startswith("OB"), edges))
        other_block_calling_edges = list(
            filter(lambda edge: not edge[0].startswith("OB"), edges)
        )
        ...
        layer_added_count = 1
        while True:
            prev_layer_blocks = list(
                filter(lambda block: layers[block] == layer - 1, layers.keys())
            )
            current_edges = list(
                filter(lambda edge: edge[0] in prev_layer_blocks, edges)
            )
            if not current_edges:
                # set layers to the rest of the nodes
                for block_id in list(
                    filter(lambda block_id: layers[block_id] == -1, layers.keys())
                ):
    ...
    def call_tree(self, df, ip_addresses, export_dpath):
        ...
        for ip in ip_addresses:
            ...
            df_ip["called_blocks_edges"] = df_ip.apply(
                lambda row: self.get_called_blocks_edges(
                    row["block_id"], row["used_block"]
                ),
                axis=1,
            )
            ...
            nodes = {
                block_id: block_id
                for block_id in set(chain.from_iterable(edges)).union(
                    df_ip.block_id.tolist()
                )
            }
            ...
            for layer in layers_count.keys():
                layers_count[layer] = len(
                    list(filter(lambda node: layers[node] == layer, layers.keys()))
                )
    ...
    def ob_roles_check(self, df, ip_addresses):
        ...
        with open(
            Path(os.path.dirname(__file__)).joinpath("mapping", "ob_mapping.json"), "rb"
        ) as f:
            ob_mapping = json.loads(f.read())

        df.loc[df["type"] == "OB", "ob_role"] = df["block_num"].apply(
            lambda block_num: (
                ob_mapping[str(block_num)]["start_event"]
                if str(block_num) in ob_mapping.keys()
                else "Unknown"
            )
        )
    ...
    def parse_network_results(self, df, ip_addresses, indicators_exist=True):
        ...
        for ip in ip_addresses:
            ...
            tcon_params = df.loc[
                (df["ip"] == ip)
                & (df["db_ext_header_tcon_params_block_length"].isnull() == False)
            ]
            if not tcon_params.empty:
                ...
                for i, row in tcon_params.iterrows():
                    if row.conn_remote_address != row.conn_remote_address:  # nan
                        ...
                        is_remote_addr_external = not row.conn_remote_address[
                            "is_private"
                        ]
            ...
            uses_network_blocks = df.loc[
                (df["ip"] == ip) & (df["uses_communication_block"] == True)
            ]
            if not uses_network_blocks.empty:
                ip_anomaly_found = True
                block_ids = uses_network_blocks.block_id.unique().tolist()
                tcon_params_msg += (
                    "\n\t\tThe following blocks uses network functionality: {}".format(
                        block_ids
                    )
                )
            ...
            if ip_anomaly_found:
                result_msg += "\n\tip: {}{}{}".format(
                    ip, tcon_params_msg, comm_blocks_used_msg
                )
    ...
    def address_check(self, tcon_remote_address):
        ...
        try:
            conn_remote_address["is_private"] = ipaddress.ip_address(
                tcon_remote_address
            ).is_private
        except:
            pass
    ...
    def network_check(self, df, ip_addresses):
        ...
        if "db_ext_header_tcon_params_block_length" in df.columns:
            ...
            df.loc[
                ...
                lambda row: self.is_use_communication_blocks(
                    df, row["ip"], row["used_block"]
                ),
                axis=1,
            )
    ...
    def process_dates_results(self, df, ip_addresses):
        ...
        for ip in ip_addresses:
            ...
            modified_delta_anomaly = df.loc[
                (df["ip"] == ip)
                & (
                    df["delta_current_time_vs_last_modified"]
                    < datetime.timedelta(days=lower_bound_modified_delta)
                )
            ]
    ...
    def dates_check(self, df, ip_addresses):
        ...
        df["delta_current_time_vs_last_interface_change"] = df[
            "last_interface_change"
        ].apply(lambda t: (current_time - datetime.datetime.fromisoformat(t)))
        ...
        for ip in ip_addresses:
            min_last_modified = datetime.datetime.fromisoformat(
                min(df.loc[df["ip"] == ip]["last_modified"])
            )
            max_last_modified = datetime.datetime.fromisoformat(
                max(df.loc[df["ip"] == ip]["last_modified"])
            )

            df.loc[df["ip"] == ip, "delta_to_min_last_modified"] = df[
                "last_modified"
            ].apply(
                lambda last_modified: datetime.datetime.fromisoformat(last_modified)
                - min_last_modified
            )
            df.loc[df["ip"] == ip, "delta_to_max_last_modified"] = df[
                "last_modified"
            ].apply(
                lambda last_modified: max_last_modified
            ...
            df.loc[df["ip"] == ip, "delta_to_min_last_interface_change"] = df[
                "last_interface_change"
            ].apply(
            ...
            df.loc[df["ip"] == ip, "delta_to_max_last_interface_change"] = df[
                "last_interface_change"
            ].apply(
    ...
    def process_author_results(self, df, ip_addresses):
        ...
        for ip in ip_addresses:
            ...
            lower_bound_msg = ""
            upper_bound_msg = ""
            author_anomaly = df.loc[
                (df["ip"] == ip) & (df["author_blocks_percentage"] < lower_bound)
            ]
            ...
            author_anomaly = df.loc[
                (df["ip"] == ip) & (df["author_blocks_percentage"] > upper_bound)
            ]
            if not author_anomaly.empty:
                ip_anomaly_found = True
                author_names = author_anomaly.author_name.unique().tolist()
            ...
            if ip_anomaly_found:
                result_msg += "\n\tip: {}{}{}".format(
                    ip, lower_bound_msg, upper_bound_msg
                )
    ...
    def author_check(self, df, ip_addresses):
        ...
        for ip in ip_addresses:
            ...
            df.loc[df["ip"] == ip, "author_blocks_percentage"] = df[
                "author_name"
            ].apply(
                lambda author_name: (authors_count[author_name] / ip_blocks_amount)
                * 100
            )
            # how many plcs share this author (percentage)
            df.loc[df["ip"] == ip, "author_plcs_percentage"] = df["author_name"].apply(
                lambda author_name: (
                    len(author_plcs_amount[author_name]) / len(ip_addresses)
                )
                * 100
            )
