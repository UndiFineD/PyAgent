...
class S7OnlineOfflineCompareCLI(AnalyzerCLI):
    def flags(self, parser):
        ...
        parser.add_argument(
            "--project_dir", help="Offline projects directory (optional)"
        )
        parser.add_argument(
            "--project_name", help="Offline project directory name (optional)"
        )
...
class S7OnlineOfflineCompare(AnalyzerInterface):
    ...
    def analyze(self):
        ...
        if not blocks:
            self.logger.debug(
                "No blocks to compare were found for given ip: {}".format(ip)
            )
            return

        if not project_name:
            self.logger.debug(
                "No project directory name was given: compare to all found project directories"
            )
        ...
        for project_path in projects:
            self.logger.debug(
                "Compare to project: {}".format(os.path.join(project_path))
            )
            offline_project_files_path = os.path.join(
                project_path, "ombstx", "offline", "*"
            )
            self.compare_project(
                ip, blocks, project_path, offline_project_files_path, self.output_dir
            )

        if projects:
            self.logger.debug(
                "Comparison results were saved at: {}".format(self.output_dir)
            )
    ...
    def extract_offset(blocks_data, offset):
        if offset:
            ...
            padding = struct.unpack(
                "<H", blocks_data[offset * 512 + 2 : offset * 512 + 4]
            )[0]
            size = struct.unpack(
                "<I", blocks_data[offset * 512 + 4 : offset * 512 + 8]
            )[0]
    ...
    def extract_blocks(self, path):
        res = []
        if os.path.exists(os.path.join(path, "SUBBLK.DBT")) and os.path.join(
            path, "SUBBLK.DBF"
        ):
    ...
    def compare_project(
        ...
        for block in blocks:
            ...
            if block["type"] in ("FB", "FC", "OB"):
                ...
                if (
                    "interface_length" in block.keys()
                ):  # TODO: change only to interface_len
                    block_size = (
                        block["interface_length"]
