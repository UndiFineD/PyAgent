# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\s7\mc7\mc7_parser.py
import binascii
import datetime
import socket
import struct
...
from forensic.analyzers.s7.mc7.mc7_consts import (
    BLOCK_ATTRIBUTE,
    BLOCK_LANGUAGE,
    BLOCK_TYPE,
    CONNECTION_TYPE,
...
class MC7Parser(object):
    ...
    def read_s7_datetime_from_bytes(date_byte):
        ...
        dt = datetime.datetime(1984, 1, 1) + datetime.timedelta(
            microseconds=millis * 1000, days=days
        )
        return dt
    ...
    def parse_block_header_metadata(self, br: BinaryReader):
        ...
        self._aggregated_buffer["attribute"] = BLOCK_ATTRIBUTE.get(
            block_header_metadata.attribute,
            "Attribute ID %s" % hex(block_header_metadata.attribute),
        )
        self._aggregated_buffer["language"] = BLOCK_LANGUAGE.get(
            block_header_metadata.language,
            "Attribute ID %s" % hex(block_header_metadata.language),
        )
        self._aggregated_buffer["type"] = BLOCK_TYPE.get(
            block_header_metadata.type,
            "Attribute ID %s" % hex(block_header_metadata.type),
        )
        ...
        self._aggregated_buffer["last_modified"] = self.read_s7_datetime_from_bytes(
            block_header_metadata.last_modified
        )
        self._aggregated_buffer["last_interface_change"] = (
            self.read_s7_datetime_from_bytes(
                block_header_metadata.last_interface_change
            )
        )
    ...
    def parse_tcon_params(self, br: BinaryReader):
        ...
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "block_length"
        ] = tcon.block_length
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "connection_id"
        ] = tcon.connection_id  # valid range 0x0001 - 0x0fff
        self._aggregated_buffer["db_ext_header"]["tcon_params"]["connection_type"] = (
            CONNECTION_TYPE.get(
                tcon.connection_type, "Attribute ID %s" % hex(tcon.connection_type)
            )
        )
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "active_est"
        ...
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "local_device_id"
        ] = tcon.local_device_id  # allowed values: 0 / 2 / 3 / 5
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "local_tsap_id_length"
        ] = tcon.local_tsap_id_length  # used length of the "local_tsap_id" parameter
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "rem_subnet_id_length"
        ] = tcon.rem_subnet_id_length  # unused
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "rem_staddr_length"
        ] = tcon.rem_staddr_length  # 0 (unspecified) / 4 (valid IP address)
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "rem_tsap_id_length"
        ] = tcon.rem_tsap_id_length  # used length of the "rem_tsap_id" parameter
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "next_staddr_length"
        ] = (
            tcon.next_staddr_length
        )  # Used length of the "next_staddr" parameter. This parameter is not relevant for TCP.
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "local_tsap_id"
        ] = tcon.local_tsap_id  # local port number
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "rem_subnet_id"
        ] = 0  # unused
        self._aggregated_buffer["db_ext_header"]["tcon_params"]["rem_staddr"] = (
            self._to_ip(tcon.rem_staddr)
        )
        self._aggregated_buffer["db_ext_header"]["tcon_params"][
            "rem_tsap_id"
        ] = tcon.rem_tsap_id  # remote port number
    ...
    def parse_db_block_metadata(self, br: BinaryReader):
        ...
        self._aggregated_buffer["local_data_length"] = (
            db_block_metadata.local_data_length
        )
        self._aggregated_buffer["data_length"] = db_block_metadata.data_length
        if self._aggregated_buffer["data_length"]:
            self._aggregated_buffer["data"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["data_length"]))
            ).decode()
        if self._aggregated_buffer["segment_length"]:
            self._aggregated_buffer["segment"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["segment_length"]))
            ).decode()
        if self._aggregated_buffer["local_data_length"]:
            self._aggregated_buffer["local_data"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["local_data_length"]))
            ).decode()
        if self._aggregated_buffer["body_length"]:
            self._aggregated_buffer["body"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["body_length"]))
            ).decode()

    def parse_non_db_block_metadata(self, br: BinaryReader):
        non_db_block_metadata = NonDbBlockMetadata(br)
        self._aggregated_buffer["interface_length"] = (
            non_db_block_metadata.interface_length
        )
        self._aggregated_buffer["segment_length"] = non_db_block_metadata.segment_length
        self._aggregated_buffer["local_data_length"] = (
            non_db_block_metadata.local_data_length
        )
        self._aggregated_buffer["mc7_length"] = non_db_block_metadata.mc7_length
        if self._aggregated_buffer["mc7_length"]:
            self._aggregated_buffer["data"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["mc7_length"]))
            ).decode()
        if self._aggregated_buffer["interface_length"]:
            self._aggregated_buffer["interface"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["interface_length"]))
            ).decode()
        if self._aggregated_buffer["segment_length"]:
            self._aggregated_buffer["segment"] = binascii.hexlify(
                br.read(bytesbe(self._aggregated_buffer["segment_length"]))
    ...
    def parse_non_db_block_segement(self):
        ...
        if self._aggregated_buffer["segment"]:
            segment_br = BinaryReader(
                binascii.unhexlify(self._aggregated_buffer["segment"])
            )
            ...
            for x in range(0, self._aggregated_buffer["segment_num"]):
                ...
                if self._aggregated_buffer[f"network_{x + 1}_mc7"]:
                    self._aggregated_buffer["used_block"] += [
                        " ".join(s.split(" ")[1:]) if s else s
                        for s in self._aggregated_buffer[f"network_{x + 1}_mc7"]
                        if any(xs in s for xs in ["UC", "CC"])
                    ]
                pointer += seg_size

    def parse_block_footer_metadata(self, br: BinaryReader):
        block_footer_metadata = BlockFooterMetadata(br)
        self._aggregated_buffer["author_name"] = self._to_str(
            block_footer_metadata.author_name
        )
        self._aggregated_buffer["block_family"] = self._to_str(
            block_footer_metadata.block_family
        )
        self._aggregated_buffer["block_name"] = self._to_str(
            block_footer_metadata.block_name
        )
        self._aggregated_buffer["block_version"] = self._to_version(
            block_footer_metadata.block_version
        )
        self._aggregated_buffer["check_sum"] = block_footer_metadata.check_sum

        if (
            self._aggregated_buffer["type"] == "DB"
            and self._aggregated_buffer["data_length"]
        ):
            self._aggregated_buffer["db_ext_header"] = dict()
            if self._aggregated_buffer["block_name"] == "TCON_PAR":  # S7-300/400 only
                self.parse_tcon_params(
                    BinaryReader(binascii.unhexlify(self._aggregated_buffer["data"]))
                )

    def parse_db_block_body(self):
        data_struct = []
        actual_values = BinaryReader(
            binascii.unhexlify(self._aggregated_buffer["data"])
        )
        ...
        start_values = BinaryReader(
            binascii.unhexlify(self._aggregated_buffer["body"])[interface_len + 7 :]
        )
        try:
            while (
                db_data.tell() <= (interface_len + 5)
                and start_values.tell() <= value_position
            ):
                interface = self._get_interface_val(
                    db_data, start_values, actual_values
                )
    ...
    def _get_interface_val(self, db_data, start_values, actual_values):
        ...
        if "_Init" in param_type:
            ...
            for i in range(childs):
                value.append(
                    self._get_interface_val(db_data, start_values, actual_values)
                )
        else:
            if (
                data_type in ("INT", "WORD", "DATE", "S5TIME")
                ...
                data_type in ("DWORD", "DINT", "TIME_OF_DAY", "TIME")
