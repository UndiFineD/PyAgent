...
def dump_all(
    output_dir, g_entry_point, symbols, all_code_blocks, taskinfos, project_info
):
...
def parse_app(app_fpath, output_dir):
    ...
    if "@table_start" in cmf.imp_addr:
        ...
        if cmdf.validate_supported_arch(br_memory, cmf.imp_addr):
            cmdf.simulate_to_find_taskinfos(
                cmf.memory_file, cmf.symbols, cmf.imp_addr, cmf.all_code_blocks
            )
