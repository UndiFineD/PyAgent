# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\memory_format.py
from datetime import datetime

from forensic.analyzers.CodeSysV3.armv7.arm_sim import MemoryStream
from forensic.analyzers.CodeSysV3.tag_handlers import (
    CodesysEntryPoint,
    CodesysFooterAddresses,
    CodesysFunctionSymbols,
    CodesysInitApplication,
    CodesysInitJumpTable,
    CodesysInternalTableSymbolEnd,
    CodesysInternalTableSymbolStart,
    CodesysListOfCodeAreas,
    CodesysProjectInfo,
    CodesysSymbol,
    CodesysSymbols,
)
...
class CodesysV3MemoryFormat:
    ...
    def extract_important_addresses(self, all_tags):
        res = {}
        dict_important_addresses_tags = {
            CodesysInitJumpTable: "@init_jump_table",
            ...
            if isinstance(tag, CodesysFooterAddresses):
                for sub_tag in tag.sub_tags:
                    if type(sub_tag) in dict_important_addresses_tags:
                        res[dict_important_addresses_tags[type(sub_tag)]] = (
                            sub_tag.pointer
                        )
        ...
        for tag in all_tags:
            ...
            if isinstance(tag, CodesysFunctionSymbols):
                for pointer in tag.symbols:
                    res[pointer] = tag.symbols[pointer]["name"].decode(
                        "ascii", "replace"
                    )
