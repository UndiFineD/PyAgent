# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\raw_file_parser.py
import os
            ...
            for app in os.listdir(os.path.join(raw_device_dir, device)):
                for app_fname in os.listdir(os.path.join(raw_device_dir, device, app)):
                    if app_fname.endswith(".app"):
                        ...
                        app_analyze_files_dir = Path(
                            os.path.join(self.output_dir, device, app)
                        )
                        app_analyze_files_dir.mkdir(parents=True, exist_ok=True)
                        parse_app(
                            os.path.join(raw_device_dir, device, app, app_fname),
                            app_analyze_files_dir,
                        )
                        self.logger.info(
                            f"App: {app_fname} was analyzed and the results at: {app_analyze_files_dir}"
                        )
