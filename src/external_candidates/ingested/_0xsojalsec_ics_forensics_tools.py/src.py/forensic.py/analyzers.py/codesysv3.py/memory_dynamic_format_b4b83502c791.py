# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\memory_dynamic_format.py
...
def sys_setup_tasks(ms: ArmMachineSimulator):
    ...
    if ms.read_mem(ms.get_reg("sp"), uint32le) != 0xCDE1F2CD:
        offset_stack = -8
    ms.function_storage["task_struct"] = ms.read_mem(
        ms.get_reg("sp") + offset_stack - 4, uint32le
    )
...
class CodesysV3MemoryDynamicFormat:
    ...
    def read_structs(
        self, br: BinaryReader, struct_class: Type[DataStruct], count: int
    ) -> List[DataStruct]:
        res = []
    ...
    def extract_internal_symbols(self, br: BinaryReader, imp_addr: dict):
        ...
        obj_struct = {}
        obj_struct["prog_calls_pous"] = self.read_structs(
            br, CodesysProgCallsPouSymbol, symbol_table_header["prog_calls_pous_count"]
        )
        obj_struct["structs"] = self.read_structs(
            br, CodesysStructSymbol, symbol_table_header["structs_count"]
        )
        obj_struct["vals_structs"] = self.read_structs(
            br, CodesysValStructSymbol, symbol_table_header["vals_structs_count"]
        )
        obj_struct["lib_pous"] = self.read_structs(
            br, CodesysUnknown24Symbol, symbol_table_header["lib_pous_count"]
        )
        obj_struct["methods"] = self.read_structs(
            br, CodesysMethodSymbol, symbol_table_header["methods_count"]
        )
        obj_struct["system_funcs"] = self.read_structs(
            br, CodesysLibSymbol, symbol_table_header["system_funcs_count"]
        )
    ...
    def hook_functions(self, ams, symbols):
        ams.add_patch_function(
            "__SYS__SETUP__TASKS",
            self.get_key_by_value(symbols, "__SYS__SETUP__TASKS"),
            sys_setup_tasks,
        )
        ams.add_patch_function(
            "__MEMCOPY", self.get_key_by_value(symbols, "__MEMCOPY"), memcopy
        )
    ...
    def get_cycle_grandparent_function_address(self, ams, symbols, imp_addr, all_code):
        ...
        for connected_edge, cycle in self.g_entry_point.in_edges(
            "__SYS__RTS__CYCLE__2"
        ):
            for task_create, connected_edge in self.g_entry_point.in_edges(
                connected_edge
            ):
    ...
    def simulate_to_find_taskinfos(self, memory_file, symbols, imp_addr, all_code):
        ...
        ams.simulate_function(
            self.get_cycle_grandparent_function_address(
                ams, symbols, imp_addr, all_code
            )
        )
        ams.simulate_function(
            self.get_task_create_parent_function(ams, symbols, imp_addr, all_code)
        )

        task_struct = ams.read_struct(
            ams.function_storage["task_struct"], CodesysTaskStruct
        )

        task_info_ptrs = []
        for task_no in range(task_struct["taskinfo_count"]):
            task_info_ptrs.append(
                ams.read_mem(
                    task_struct["taskinfo_arr_pointer"] + task_no * 4, uint32le
                )
            )
        ...
        for tinfo_ptr in task_info_ptrs:
            ...
            task_entry_function = ams.read_mem(
                taskinfo["dwTaskEntryFunctionPointer"], uint32le
            )
            symbols[task_entry_function] = name

            g_task_entry_function = ams.xref_graph(
                all_code, symbols, task_entry_function
            )

            self.taskinfos.append(
                {"name": name, "taskinfo": taskinfo, "xrefs": g_task_entry_function}
            )
