# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\armv7\arm_sim.py
import struct
...
class MemoryStream(BinaryReader, BinaryWriter):
    def __init__(
        self, data: bytes = b"", endianness: Endianness = Endianness.LITTLE
    ) -> None:
...
class ArmMachineSimulator:
    ...
    def add_patch_function(self, symbol, address, function):
        self._patched_functions[self._destination_addr] = {
            "func_name": symbol,
    ...
    def iterate_no_emulate(self, start_ea: int):
        ...
        while len(address_to_visit) > 0:
            ...
            for inst in self.md.disasm(self._raw_memory[curr_ea:], curr_ea):
                ...
                if (
                    capstone.arm.ARM_GRP_JUMP in inst.groups
                    and inst.operands[0].type == capstone.arm.ARM_OP_IMM
                ):
            ...
            for inst in self.iterate_no_emulate(curr_ea):
                ...
                if inst.mnemonic == "ldr":
                    if (
                        ...
                        and inst.operands[1].mem.base == capstone.arm.ARM_REG_PC
                    ):
                        address = (
                            inst.operands[1].mem.disp + inst.address + 8
                        ...
                        if jump_target in all_code:
                            ...
                            g.add_edge(
                                f'{hex(curr_ea)} {symbols.get(curr_ea, "")}'.strip(),
    ...
    def simulate_function(self, start_ea: int):
        ...
        while self.get_reg("pc") != end_addr:
            ...
            if inst.mnemonic.startswith("ldr"):
                ...
                if (
                    inst.operands[0].type == capstone.arm.ARM_OP_REG
                    and inst.operands[1].type == capstone.arm.ARM_OP_REG
                    and inst.operands[2].type == capstone.arm.ARM_OP_IMM
                ):
                    target_reg = self.md.reg_name(inst.operands[0].reg)
                    source_reg = self.md.reg_name(inst.operands[1].reg)
                    self.registers[target_reg] = (
                        self.registers[source_reg] - inst.operands[2].imm
                    )
                else:
                    mybreakpoint()
                ...
                if (
                    len(inst.operands) == 3
                    and inst.operands[0].type == capstone.arm.ARM_OP_REG
                    and inst.operands[1].type == capstone.arm.ARM_OP_REG
                ):
                    target_reg = inst.operands[0].reg
                ...
                if inst.mnemonic == "and" or (
                    inst.mnemonic == "andvs" and self.get_reg("v_flag") == 1
                ):
                    if (
                        len(inst.operands) == 3
                        and inst.operands[0].type == capstone.arm.ARM_OP_REG
                        and inst.operands[1].type == capstone.arm.ARM_OP_REG
                        and inst.operands[2].type == capstone.arm.ARM_OP_REG
                    ):
                        target_reg = inst.operands[0].reg
                        ...
                        self.set_reg(
                            target_reg,
                            (self.get_reg(middle_reg) & self.get_reg(source_reg))
                            & 0xFFFFFFFF,
                        )
                    else:
                        mybreakpoint()
                ...
                if (
                    ...
                    self.set_reg(
                        "z_flag", 1 if left_reg_val - right_reg_val == 0 else 0
                    )
                    ...
                    self.set_reg(
                        ...
                        (
                            1
                            if left_reg_val > 0x80000000
                            and ((left_reg_val - right_reg_val) & 0xFFFFFFFF)
                            <= 0x7FFFFFFF
                            else 0
                        ),
                    )
                ...
                if (
                    self.get_reg("n_flag") == self.get_reg("v_flag")
                    and self.get_reg("z_flag") == 0
                ):
                    self.set_reg("pc", inst.operands[0].imm)
