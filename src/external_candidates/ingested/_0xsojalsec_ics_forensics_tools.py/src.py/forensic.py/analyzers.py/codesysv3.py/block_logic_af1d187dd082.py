# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\block_logic.py
import json
import os
...
class CS3BlockLogicCLI(AnalyzerCLI):
    def flags(self, parser):
        parser.add_argument(
            "--logic_all",
            help="Execute all logics for CS3BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_author",
            help="Execute author logic for CS3BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_dates",
            help="Execute date logic for CS3BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_project_info",
            help="Execute project info logic for CS3BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_network",
            help="Execute network logics for CS3BlockLogic analyzer",
            action="store_true",
        )
        parser.add_argument(
            "--logic_tasks",
            help="Execute tasks logic for BlockLogic analyzer",
            action="store_true",
        )
...
class CS3BlockLogic(AnalyzerInterface):
    ...
    def get_parsed_devices_data(self):
        ...
        for device in os.listdir(devices_dir):
            devices_project_info[device.replace("_", ".")] = {}
            devices_tasks[device.replace("_", ".")] = {}
            devices_symbols[device.replace("_", ".")] = {}
            for app in os.listdir(os.path.join(devices_dir, device)):
                for out in os.listdir(os.path.join(devices_dir, device, app)):
                    if out == "project_info.json":
                        with open(
                            os.path.join(devices_dir, device, app, out), "r"
                        ) as f:
                            devices_project_info[device.replace("_", ".")][app] = (
                                json.load(f)
                            )
                    if out == "taskinfo.json":
                        with open(
                            ...
                            devices_tasks[device.replace("_", ".")][app] = json.load(f)
                    if out == "symbols.csv":
                        devices_symbols[device.replace("_", ".")][app] = pd.read_csv(
                            os.path.join(devices_dir, device, app, out),
                            names=["pointer", "name"],
                        )

        return {
            "devices_project_info": devices_project_info,
        ...
        for device in self.parsed_devices_data["devices_tasks"].keys():
            ...
            for app in self.parsed_devices_data["devices_tasks"][device].keys():
                if self.parsed_devices_data["devices_tasks"][device][app]:
                    tasks_amount = len(
                        self.parsed_devices_data["devices_tasks"][device][app]
                    )
                    str += f"\n\t{app} has {tasks_amount} configured tasks"
                    for task in self.parsed_devices_data["devices_tasks"][device][app]:
                        str += "\n\tTask name: {}, parameters: {}".format(
                            task["name"], task["taskinfo"]
                        )
                else:
    ...
    def load_netlibs(self):
        with open(
            Path(os.path.dirname(__file__)).joinpath(
                "mapping", "network_libraries.json"
            ),
            "rb",
        ) as f:
        ...
        for device in self.parsed_devices_data["devices_symbols"].keys():
            ...
            for app in self.parsed_devices_data["devices_symbols"][device].keys():
                ...
                if not used_netlibs.empty:
                    str += "\n\tApp: {} have usage of network libraries: {}".format(
                        app, used_netlibs.name.tolist()
                    )
                else:
    ...
    def dates_check(self):
        ...
        for device in self.parsed_devices_data["devices_project_info"].keys():
            ...
            for app in self.parsed_devices_data["devices_project_info"][device].keys():
                project_info = self.parsed_devices_data["devices_project_info"][device][
                    app
                ]
                if "project_build_datetime" in project_info.keys():
                    str += "\n\tApp: {}, build datetime: {}".format(
                        app, project_info["project_build_datetime"]
                    )
                else:
    ...
    def project_info_check(self):
        ...
        for device in self.parsed_devices_data["devices_project_info"].keys():
            ...
            for app in self.parsed_devices_data["devices_project_info"][device].keys():
                str += "\n\tApp: {}, project information: {}".format(
                    app, self.parsed_devices_data["devices_project_info"][device][app]
                )
                self.logger.info(str)
