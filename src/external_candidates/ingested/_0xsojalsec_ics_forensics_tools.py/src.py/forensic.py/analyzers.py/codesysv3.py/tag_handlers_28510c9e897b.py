# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\analyzers\CodeSysV3\tag_handlers.py
import copy
        ...
        for sub_tag in cff.read_list_of_tags(data):
            if sub_tag.tid in labels:
                if sub_tag.tid == 3:
                    ...
                    self.attributes[labels[sub_tag.tid]] = self._null_ending_bytes(
                        sub_tag.data
                    )
    ...
    def __init__(self, cff: CodesysV3FileFormatAbstract, data: bytes):
        ...
        for sub_tag in cff.read_list_of_tags(data):
            if sub_tag.tid == 1:
                ...
                temp_symbol["raw"] = br_sub_tag.read(
                    bytesbe(br_sub_tag.remaining_data())
                )
            elif sub_tag.tid == 2:
                ...
                self.unknown_tags.append(
                    {"unknown tag": sub_tag.tid, "raw": sub_tag.data}
                )
...
class CodesysSymbol(CodesysBaseTag):
    ...
    def __init__(self, cff: CodesysV3FileFormatAbstract, data: bytes):
        ...
        self.name = self._null_ending_bytes(
            br_symbol.read(bytesbe(br_symbol.remaining_data()))
        )
