# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ics-forensics-tools\src\forensic\client\application.py
import os
...
class Application(object):
    def __init__(self):
        self.logger_handler = LoggerHandler(
            constants.LOG_FILE_NAME, constants.LOGS_FORMATTER
        )
        self.logger = self.logger_handler.get_logger()

    def _exec(self, config):
        forensic.__resources__[config.name](
            config, constants.OUTPUT_DIR, constants.VERBOSE
        ).run()
        ...
        try:
            if verbose:
                ...
                self.logger_handler.create_log_file(
                    os.path.join(
                        os.getcwd(), constants.LOGS_DIR, constants.LOG_FILE_NAME
                    )
                )
            if output_dir:
