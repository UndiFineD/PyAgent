...
class ForensicCLI(Application):
    ...
    def get_parameters(args: Dict[str, Any]):
        return {
            ...
            if key not in constants.ARGUMENTS and value is not None and value
        }
    ...
    def command_arguments(self):
        plugins = modules.list_subclasses(
            PluginCLI, f"{constants.APPLICATION}.plugins."
        )
        analyzers = modules.list_subclasses(
            AnalyzerCLI, f"{constants.APPLICATION}.analyzers."
        )
        ...
        for plugin in plugins:
            ...
            plugin_parser = subparser.add_parser(
                plugin_cli.name, help=plugin_cli.description
            )
    ...
    def scan(
        ...
        if args.plugin:
            ...
            plugin_config = PluginConfig(
                args.plugin, addresses, args.port, args.transport, parameters, analyzers
            )
            if args.save_config:
                plugin_config.create_config_file(constants.CONFIG_FILE)
                self.logger.info("Successfully created config.json file")
            super().scan(
                [plugin_config], args.multiprocess, args.output_dir, args.verbose
            )
