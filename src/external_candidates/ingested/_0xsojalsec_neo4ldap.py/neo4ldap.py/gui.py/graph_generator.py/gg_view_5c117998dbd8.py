...
class GraphView(QGraphicsView):
    ...
    # Node utilities
    def add_node(
        self, x, y, label, node_type, node_id, shadow_relationships
    ) -> GraphNode:
        fill_color = self.choose_node_color(node_type)
        node = GraphNode(
            x, y, label, node_type, node_id, fill_color, shadow_relationships
        )
    ...
    def add_edge(self, src_label, dst_label, acl, offset=10, color=Qt.white) -> None:
        edge = GraphEdge(
            self.nodes[src_label], self.nodes[dst_label], offset, color, acl
        )
    ...
    def update_edge_visibility(self) -> None:
        for edge in self.edges:
            show = (
                edge.src.isVisible
                and edge.dst.isVisible()
                and not edge.src.subgraph_hidden
            )
    ...
    def kahn_sort(self, edges, reachable_nodes=None) -> list:
        ...
        for edge in edges:
            ...
            if reachable_nodes and (
                source_node not in reachable_nodes or target_node not in reachable_nodes
            ):
                continue
    ...
    def calculate_dag_layout(self, root_label) -> None:
        ...
        for depth in sorted(layers):
            ...
            for i, node in enumerate(all_ordered):
                node.setPos(
                    depth * self.COLUMN_WIDTH, start_y + i * self.NODE_VERTICAL_SPACING
                )
