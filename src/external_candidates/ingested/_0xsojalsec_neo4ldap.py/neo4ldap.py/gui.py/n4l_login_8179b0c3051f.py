...
class LoginWindow(ViewerApp):
    def __init__(self, controller):
        ...
        left_span.setStyleSheet(
            "background-color: {background};".format(background=self.PANELS_BG)
        )

        # RIGHT SPAN
        right_span = QWidget(self)
        right_span.setFixedWidth(650)
        right_span.setStyleSheet(
            "background-color: {background};".format(background=self.PANELS_BG)
        )

        # CENTER SPAN
        center_span = QWidget(self)
        center_span.setStyleSheet(
            "background-color: {background};".format(background=self.PANELS_BG)
        )
        ...
        scaled_pixmap = pixmap.scaled(
            250, 1000, Qt.KeepAspectRatio, Qt.SmoothTransformation
        )
