# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadrecon\roadtools\roadrecon\gather.py
import argparse
import asyncio
import json
import os
...
from roadtools.roadlib.metadef.database import (
    ...
    lnk_group_owner_serviceprincipal,
...
def checktoken():
    ...
    if time.time() > expiretime - 300:
        ...
        if "refreshToken" in token:
            ...
            print(
                "Access token is expired, but no access to refresh token! Dumping will fail"
            )
            return False
    return True
...
class DataDumper(object):
    ...
    async def dump_object(self, objecttype, dbtype, method=None):
        ...
        url = "https://graph.windows.net/%s/%s?api-version=1.61-internal" % (
            self.tenantid,
            objecttype,
        )
        cache = []
    ...
    async def dump_l_to_db(self, url, method, mapping, linkname, childtbl, parent):
        ...
        async for obj in dumphelper(url, method=method):
            ...
            if mapping is not None:
                try:
                    ...
                    print(
                        "Unsupported member type: %s for parent %s"
                        % (objclass, parent.__table__)
                    )
                    continue
            ...
            if not child:
                ...
                print(
                    "Non-existing child found on %s %s: %s"
                    % (parent.__table__, parentname, objectid)
                )
                continue
        ...
        if str(parent.__table__) == "Groups":
            groupcounter += 1
            print(
                "Done processing {0}/{1} groups".format(
                    int(groupcounter / 2), totalgroups
                ),
                end="\r",
            )
    ...
    async def dump_l_to_linktable(self, url, method, mapping, parentid, objecttype):
        ...
        if str(objecttype) == "groups":
            groupcounter += 1
            print(
                "Done processing {0}/{1} groups {2}/{3} devices".format(
                    int(groupcounter / 2), totalgroups, devicecounter, totaldevices
                ),
                end="\r",
            )
    ...
    async def dump_links(
        ...
        for parent in parents:
            url = "https://graph.windows.net/%s/%s/%s/$links/%s?api-version=%s" % (
                self.tenantid,
                objecttype,
                parent.objectId,
                linktype,
            ...
            jobs.append(
                self.dump_l_to_db(url, method, mapping, linkname, childtbl, parent)
            )
            i += 1
    ...
    async def dump_links_with_queue(
        ...
        for (parentid,) in parents:
            ...
            # Chunk it to avoid huge memory usage
            await queue.put(
                self.dump_l_to_linktable(url, method, mapping, parentid, objecttype)
            )
        await queue.join()
        self.session.commit()
    ...
    async def dump_mfa_to_db(self, url, method, parentid, cache):
        ...
        cache.append(
            {
                "userid": parentid,
                "strongAuthenticationDetail": obj["strongAuthenticationDetail"],
            }
        )
    ...
    async def dump_mfa(self, objecttype, parenttbl, method=None):
        ...
        for (parentid,) in parents:
            url = (
                "https://graph.windows.net/%s/%s/%s?api-version=%s&$select=strongAuthenticationDetail,objectId"
                % (self.tenantid, objecttype, parentid, self.api_version)
            )
    ...
    async def dump_lo_to_db(
        self, url, method, linkobjecttype, cache, ignore_duplicates=False
    ):
        """
    ...
    async def dump_linked_objects(
        ...
        jobs = []
        for parent in parents:
            url = "https://graph.windows.net/%s/%s/%s/%s?api-version=%s" % (
                self.tenantid,
                objecttype,
                parent.objectId,
                linktype,
    ...
    async def dump_object_expansion(
        self,
        objecttype,
        dbtype,
        expandprop,
        ...
        if method is None:
            method = self.ahsession.get
        url = "https://graph.windows.net/%s/%s?api-version=%s&$expand=%s" % (
            self.tenantid,
            objecttype,
            self.api_version,
            expandprop,
        ...
        async for obj in dumphelper(url, method=method):
            if len(obj[expandprop]) > 0:
                ...
                if not parent:
                    print(
                        "Non-existing parent found during expansion %s %s: %s"
                        % (dbtype.__table__, expandprop, obj["objectId"])
                    )
                    continue
                for epdata in obj[expandprop]:
                    ...
                    if not child:
                        print(
                            "Non-existing child during expansion %s %s: %s"
                            % (dbtype.__table__, expandprop, epdata["objectId"])
                        )
                        continue
                    getattr(parent, linkname).append(child)
                    i += 1
    ...
    async def dump_keycredentials(self, objecttype, dbtype, method=None):
        ...
        cache = []
        url = (
            "https://graph.windows.net/%s/%s?api-version=1.61-internal&$select=keyCredentials,objectId"
            % (self.tenantid, objecttype)
        )
        async for obj in dumphelper(url, method=method):
            cache.append(
                {"userid": obj["objectId"], "keyCredentials": obj["keyCredentials"]}
            )
            if len(cache) > 1000:
    ...
    async def dump_apps_from_list(
        self, parents, endpoint, dbtype, ignore_duplicates=True
    ):
        cache = []
        jobs = []
        for parentid in parents:
            url = "https://graph.windows.net/%s/%s/%s?api-version=%s" % (
                self.tenantid,
                endpoint,
                parentid,
                self.api_version,
    ...
    async def dump_custom_role_members(self, dbtype):
        ...
        for parent in parents:
            url = (
                "https://graph.windows.net/%s/roleAssignments?api-version=%s&$filter=roleDefinitionId eq '%s'"
                % (self.tenantid, self.api_version, parent.objectId)
            )
...
async def run(args):
    ...
    if not args.skip_first_phase:
        async with aiohttp.ClientSession() as ahsession:
            ...
            tasks.append(
                dumper.dump_object("oauth2PermissionGrants", OAuth2PermissionGrant)
            )
    ...
    if totalgroups > MAX_GROUPS:
        print(
            "Gathered {0} groups, switching to 3-phase approach for efficiency".format(
                totalgroups
            )
        )
    async with aiohttp.ClientSession() as ahsession:
        ...
        if totalgroups <= MAX_GROUPS:
            tasks.append(
                dumper.dump_links("groups", "members", Group, mapping=group_mapping)
            )
            tasks.append(
                dumper.dump_links(
                    "groups", "owners", Group, mapping=group_owner_mapping
                )
            )
            tasks.append(
                dumper.dump_object_expansion(
                    "devices", Device, "registeredOwners", "owner", User
                )
            )
        tasks.append(
            dumper.dump_links(
                "directoryRoles", "members", DirectoryRole, mapping=role_mapping
            )
        )
        tasks.append(
            dumper.dump_linked_objects(
                "servicePrincipals",
                "appRoleAssignedTo",
                ServicePrincipal,
                AppRoleAssignment,
        ...
        tasks.append(
            dumper.dump_each(ServicePrincipal, "applicationRefs", ApplicationRef)
        )
    ...
    if totalgroups > MAX_GROUPS:
        ...
        async with aiohttp.ClientSession() as ahsession:
            ...
            tasks.append(
                dumper.dump_links_with_queue(
                    queue, "groups", "members", Group, mapping=group_link_mapping
                )
            )
            tasks.append(
                dumper.dump_links_with_queue(
                    queue, "groups", "owners", Group, mapping=group_owner_link_mapping
                )
            )
...
def main(args=None):
    ...
    if args is None:
        parser = argparse.ArgumentParser(
            add_help=True,
            description="ROADrecon - Gather Azure AD information",
    ...
    print(
        "ROADrecon gather executed in {0:0.2f} seconds and issued {1} HTTP requests.".format(
            elapsed, urlcounter
        )
    )
