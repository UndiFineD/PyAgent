# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadrecon\roadtools\roadrecon\server.py
import argparse
import mimetypes
import os
...
CORS(
    app,
    origins=["http://127.0.0.1:4200", "http://localhost:4200", "http://localhost:5000"],
)
...
class RTModelConverter(ModelConverter):
    SQLA_TYPE_MAPPING = dict(
        list(ModelConverter.SQLA_TYPE_MAPPING.items()) + [(JSON, fields.Raw)]
    )
...
def get_mfa():
    ...
    for user in all_mfa:
        ...
        methods = [
            method["methodType"]
            for method in user.strongAuthenticationDetail["methods"]
        ]
...
def get_approles_by_resource(spid):
    approles = []
    for ar in db.session.query(AppRoleAssignment).filter(
        AppRoleAssignment.resourceId == spid
    ):
        process_approle(approles, ar)
    return jsonify(approles)
...
def get_approles_by_principal(pid):
    approles = []
    for ar in db.session.query(AppRoleAssignment).filter(
        AppRoleAssignment.principalId == pid
    ):
        process_approle(approles, ar)
    return jsonify(approles)
...
def get_stats():
    stats = {
        ...
        "countServicePrincipals": db.session.query(
            func.count(ServicePrincipal.objectId)
        ).scalar(),
        "countApplications": db.session.query(
            func.count(Application.objectId)
        ).scalar(),
...
def main(args=None):
    ...
    if not args:
        ...
        parser.add_argument(
            "--profile", action="store_true", help="Enable flask profiler"
        )
        args = parser.parse_args()
    if not ":/" in args.database:
        if args.database[0] != "/":
            app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + os.path.join(
                os.getcwd(), args.database
            )
        else:
