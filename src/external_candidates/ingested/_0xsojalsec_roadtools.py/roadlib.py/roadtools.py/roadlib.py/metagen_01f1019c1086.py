# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadlib\roadtools\roadlib\metagen.py
import sys
...
for entitytype in root.iter(
    "{http://schemas.microsoft.com/ado/2009/11/edm}EntityType"
):  # root.findall('edm:EntityType', ns):
...
"""
    ...
    for prop in entitytype.iter(
        "{http://schemas.microsoft.com/ado/2009/11/edm}Property"
    ):
        props.append(
            "        '%s': %s," % (prop.get("Name"), trl_prop(prop.get("Type")))
        )
        allprops.append(trl_prop(prop.get("Type")))

    rels = []
    for relout in entitytype.iter(
