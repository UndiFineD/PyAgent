# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ROADtools\roadlib\roadtools\roadlib\auth.py
import argparse
import base64
import binascii
import codecs
...
class Authentication:
    ...
    def authenticate_device_code(self):
        ...
        print(code["message"])
        self.tokendata = context.acquire_token_with_device_code(
            self.resource_uri, code, self.client_id
        )
    ...
    def authenticate_username_password(self):
        ...
        self.tokendata = context.acquire_token_with_username_password(
    ...
    def authenticate_as_app(self):
        ...
        self.tokendata = context.acquire_token_with_client_credentials(
            self.resource_uri, self.client_id, self.password
        )
        return self.tokendata
    ...
    def authenticate_with_code(self, code, redirurl, client_secret=None):
        ...
        self.tokendata = context.acquire_token_with_authorization_code(
            code, redirurl, self.resource_uri, self.client_id, client_secret
        )
        return self.tokendata
    ...
    def authenticate_with_refresh(self, oldtokendata):
        ...
        newtokendata = context.acquire_token_with_refresh_token(
            oldtokendata["refreshToken"], self.client_id, self.resource_uri
        )
        # Overwrite fields
        ...
        inputdata = json.loads(
            base64.b64decode(tokens[1] + ("=" * (len(tokens[1]) % 4)))
        )
    ...
    def authenticate_with_code_native(
        self, code, redirurl, client_secret=None, pkce_secret=None
    ):
        """
        Authenticate with a code plus optional secret in case of a non-public app (authorization grant)
    ...
    def authenticate_with_prt_cookie(
        self, cookie, context=None, derived_key=None, verify_only=False, sessionkey=None
    ):
        """
    ...
    def get_sub_argparse(auth_parser, for_rr=False):
        ...
        auth_parser.add_argument(
            "--refresh-token", action="store", help="Refresh token (JWT)"
        )
        auth_parser.add_argument(
            "--prt-cookie",
            action="store",
