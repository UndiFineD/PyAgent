...
class NetworkXStorage(BaseGraphStorage):
    ...
    def load_nx_graph(self) -> bool:
        ...
        if os.path.exists(self.graphml_xml_file):
            ...
            logger.info(
                "GraphML file does not exist! Need to build the graph from scratch."
            )
            return False
    ...
    async def get_one_path(self, start: str, cand: list[str], cutoff: int = 5):
        pred, dist = nx.dijkstra_predecessor_and_distance(
            self._graph, source=start, cutoff=cutoff, weight=None
        )
        end = None
    ...
    async def get_neighbors_from_sources(self, start_nodes: list[str]):
        ...
        for u in start_nodes:
            neis = [
                (await self.get_edge(e[0], e[1]))["tgt_id"]
                for e in await self.get_node_edges(u)
            ]
            neighbor_list.extend(
                [
                    (await self.get_edge(e[0], e[1]))
                    for e in await self.get_node_edges(u)
                ]
            )
                ...
                if len(inter) != 0:
                    for v in inter:
                        new_neis.extend(
                            [
                                (await self.get_edge(e[0], e[1]))["tgt_id"]
                                for e in await self.get_node_edges(v)
                            ]
                        )
