# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Storage\TreeGraphStorage.py
import os
import pickle
...
class TreeGraphStorage(BaseGraphStorage):
    ...
    async def write_tree_leaves(self):
        TreeGraphStorage.write_tree_graph(
            tree=self.tree, tree_pkl_file=self.tree_leaves_pkl_file
        )

    async def load_tree_graph_from_leaves(self, force=False) -> bool:
        # Attempting to load the graph from the specified pkl file
        logger.info(
            f"Attempting to load the tree leaves from: {self.tree_leaves_pkl_file}"
        )
    ...
    async def get_nodes_data(self):
        return [
            {"content": node.text, "index": node.index} for node in self.tree.all_nodes
        ]
