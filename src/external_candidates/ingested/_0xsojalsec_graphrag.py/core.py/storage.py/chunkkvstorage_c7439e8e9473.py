...
class ChunkKVStorage(BaseKVStorage):
    ...
    async def chunk_datas(self):
        inserting_chunks = {
            key: value for key, value in self._chunk.items() if key in self._chunk
        }
    ...
    async def load_chunk(self):
        ...
        logger.info(
            f"Attempting to load the chunk data from: {self.dat_idx_pkl_file} and {self.dat_key_pkl_file}"
        )
        if os.path.exists(self.dat_idx_pkl_file) and os.path.exists(
            self.dat_key_pkl_file
        ):
            try:
                ...
                self._key_to_index = {
                    key: value.index for key, value in self._chunk.items()
                }
                logger.info(
            ...
            logger.info(
                "Pkl file does not exist! Need to chunk the documents from scratch."
            )
            return False

    async def _persist(self):
        logger.info(
            f"Writing data into {self.dat_idx_pkl_file} and {self.dat_key_pkl_file}"
        )
