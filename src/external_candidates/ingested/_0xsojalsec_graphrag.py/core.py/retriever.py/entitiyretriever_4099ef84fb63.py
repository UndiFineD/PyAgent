...
class EntityRetriever(BaseRetriever):
    ...
    async def _find_relevant_entities_by_ppr(
    ...
    async def _find_relevant_entities_tf_df(self, seed, corpus, top_k, candidates_idx):
        try:
            ...
            corpus = dict(
                {
                    id: (await self.graph.get_node(id))["description"]
    ...
    async def _link_entities(self, query_entities):
        ...
        entities = await asyncio.gather(
            *[
                self.entities_vdb.retrieval_nodes(
    ...
    async def _find_relevant_entities_by_relationships_agent(
        ...
        try:
            ...
            for index, entity in enumerate(total_entity_relation_list):
                candidate_list = total_relations_dict[
            ...
            filtered_list = [
                (rel, ent, top, hea, score)
                for rel, ent, top, hea, score in merged_list
                if score != 0
            ]
