# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Retriever\ChunkRetriever.py
import asyncio
...
class ChunkRetriever(BaseRetriever):
    ...
    async def _find_relevant_chunks_from_entity_occurrence(
        self, node_datas: list[dict]
    ):
    ...
    async def _find_relevant_chunks_by_ppr(
        self, query, seed_entities: list[dict], link_entity=False
    ):
        #
        if link_entity:
        ...
        if len(seed_entities) == 0:
            ...
            node_ppr_matrix = await self._run_personalized_pagerank(
                query, seed_entities
            )
    ...
    async def _find_relevant_chunks_by_ppr(self, query, seed_entities: list[dict]):
        ...
        sorted_relationships = await self.graph.get_edge_by_indices(
            sorted_relationship_ids
        )
        sorted_entities, sorted_relationships, sorted_docs
        return to_str_by_maxtokens(
            max_chars={
                "entities": self.config.entities_max_tokens * TOKEN_TO_CHAR_RATIO,
                "relationships": self.config.relationships_max_tokens
                * TOKEN_TO_CHAR_RATIO,
                "chunks": self.config.local_max_token_for_text_unit
                * TOKEN_TO_CHAR_RATIO,
            },
