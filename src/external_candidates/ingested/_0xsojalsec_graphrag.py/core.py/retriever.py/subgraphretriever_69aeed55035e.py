...
class SubgraphRetriever(BaseRetriever):
    ...
    @register_retriever_method(
        type="subgraph", method_name="concatenate_information_return_list"
    )
    ...
    async def _find_subgraph_by_k_hop(self, seed: list[str], k: int):
        try:
            if seed is None:
                return None
            subgraph_datas = await self.graph.find_k_hop_neighbors_batch(
                start_nodes=seed, k=k
            )  # set
            if not len(subgraph_datas):
                return None
    ...
    @register_retriever_method(
        type="subgraph", method_name="induced_subgraph_return_networkx"
    )
    ...
    async def _find_subgraph_by_paths(self, seed: list[str], cutoff: int = 5):
        try:
            if seed is None:
                return None
            path_datas = await self.graph.get_paths_from_sources(
                start_nodes=seed, cutoff=cutoff
            )
