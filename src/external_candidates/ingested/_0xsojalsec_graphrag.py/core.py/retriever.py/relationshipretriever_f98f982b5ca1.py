...
class RelationshipRetriever(BaseRetriever):
    ...
    async def _find_relevant_relationships_by_ppr(
        self, query, seed_entities: list[dict], node_ppr_matrix=None
    ):
        #
    ...
    async def _find_relevant_relations_vdb(
        self, seed, need_score=False, need_context=True, top_k=None
    ):
        try:
            if seed is None:
                return None
    ...
    async def _find_relevant_relations_by_entity_agent(
        ...
        try:
            ...
            relations_name_super_edge = await self.graph.get_edge_relation_name_batch(
                edges=edges
            )
            relations_name = list(
                map(lambda x: x.split(GRAPH_FIELD_SEP), relations_name_super_edge)
            )  # [[], [], []]
            ...
            prompt = (
                ...
                + "\nTopic Entity: "
                + entity
                + "\nRelations: There are %s relations provided in total, seperated by ;."
                % str(len(total_relations))
            ...
            if len(relations) == 0:
                flag = False
                logger.info(
                    "No relations found by entity: {} and query: {}".format(
                        entity, query
                    )
                )
            else:
