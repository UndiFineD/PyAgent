# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Retriever\MixRetriever.py
...
class MixRetriever:
    ...
    def register_retrievers(self):
        ...
        self.retrievers[Retriever.COMMUNITY] = CommunityRetriever(
            **self.context.as_dict
        )
        self.retrievers[Retriever.CHUNK] = ChunkRetriever(**self.context.as_dict)
        self.retrievers[Retriever.RELATION] = RelationshipRetriever(
            **self.context.as_dict
        )
    ...
    async def retrieve_relevant_content(self, type: Retriever, mode: str, **kwargs):
        return await self.retrievers[type].retrieve_relevant_content(
            mode=mode, **kwargs
        )
