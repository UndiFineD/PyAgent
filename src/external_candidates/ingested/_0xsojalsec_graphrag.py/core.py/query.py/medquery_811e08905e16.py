...
class MedQuery(BaseQuery):
    ...
    async def _concatenate_information(
        self, metagraph_relation: str, metagraph_edge: tuple[str, str]
    ):
    ...
    async def _retrieve_relevant_contexts(self, query: str):
        ...
        while len(metagraph_nodes) < self.config.topk_entity:
            ...
            for node in origin_nodes[(iteration_count - 1) * self.config.topk_entity :]:
                if (
                    ...
                    and node["entity_name"] not in metagraph_nodes
                ):
        ...
        logger.info(
            "Find top {} entities at iteration {}!".format(
                self.config.topk_entity, iteration_count
            )
        )
