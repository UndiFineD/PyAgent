# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\DalkQuery.py
from Core.Common.Constants import Retriever
from Core.Common.Logger import logger
from Core.Prompt import QueryPrompt
from Core.Prompt.QueryPrompt import (
    DALK_CONTEXT_PROMPT,
    DALK_CONVERT_PROMPT,
    DALK_QUERY_PROMPT,
    DALK_RERANK_PROMPT,
...
class DalkQuery(BaseQuery):
    ...
    async def _select_most_relative_paths(self, path_list, query):
        ...
        path_str = [
            (
                path[0]["src_id"] + ("->" + e["content"] + "-> " + e["tgt_id"])
                for edge in path
            )
    ...
    async def _select_most_relative_neis(self, nei_list, query):
        ...
        if nei_list is None:
            return ""
        nei_str_list = [
            "->".join([e["src_id"], e["content"], e["tgt_id"]]) for e in nei_list
        ]
    ...
    async def _process_info(self, query, paths, neis):
        ...
        step = await self.llm.aask(
            DALK_STEP_PROMPT.format(question=query, paths=paths, neis=neis)
        )
    ...
    async def _retrieve_relevant_contexts(self, query):
        ...
        paths = await self._select_most_relative_paths(
            await self._retriever.retrieve_relevant_content(
                ...
                seed=entities,
                cutoff=self.config.k_hop,
            ),
            query,
        )
        neis = await self._select_most_relative_neis(
            await self._retriever.retrieve_relevant_content(
                type=Retriever.SUBGRAPH, mode="neighbors_return_list", seed=entities
            ),
            query,
        )
    ...
    async def query(self, query):
        ...
        response = await self.llm.aask(
            DALK_QUERY_PROMPT.format(question=query, context=context)
        )
        logger.info(query)
    ...
    async def generation_qa(self, query, context):
        ...
        answer = await self.llm.aask(
            DALK_QUERY_PROMPT.format(question=query, context=context_str)
        )
        return answer
