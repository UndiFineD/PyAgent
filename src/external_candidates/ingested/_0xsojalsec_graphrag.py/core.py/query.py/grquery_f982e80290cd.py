# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\GRQuery.py
import asyncio
...
class GRQuery(BaseQuery):
    ...
    async def initialization(self):
        ...
        origin_edges = await self._retriever.retrieve_relevant_content(
            type=Retriever.RELATION, mode="get_all"
        )  # list[dict]
        relations = list(
            map(lambda x: x["relation_name"].split(sep=GRAPH_FIELD_SEP), origin_edges)
        )  # list[list]
        ...
        for tri in document_graph_triplets:
            ...
            raw_edges.append({"src": raw_nodes[h], "edge_attr": r, "dst": raw_nodes[t]})
    ...
    async def retrieval_via_pcst(
        ...
        if topk > 0:
            ...
            retrieve_entity = await self._retriever.retrieve_relevant_content(
                type=Retriever.ENTITY, mode="vdb", seed=query
            )  # list[dict]
            retrieve_entity_id = torch.tensor(
                list(map(lambda x: self.raw_nodes[x["entity_name"]], retrieve_entity))
            )  # [0,1,2,..,node_id]
        ...
        if topk_e > 0:
            ...
            retrieve_relations, topk_e_values = (
            ...
            for i, rel in enumerate(retrieve_relations):
                index = self.edges[
                    ...
                    & (
                        self.edges["edge_attr"]
                        == retrieve_relations[i]["relation_name"]
                    )
    ...
    async def _retrieve_relevant_contexts(self, query: str):
        ...
        desc = truncate_str_by_token_size(
            input_str=desc, max_token_size=self.config.max_txt_len
        )
        return query, desc
