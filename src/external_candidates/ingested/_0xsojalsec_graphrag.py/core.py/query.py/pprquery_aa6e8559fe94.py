# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\PPRQuery.py
...
class PPRQuery(BaseQuery):
    ...
    async def reason_step(
        self, few_shot: list, query: str, passages: list, thoughts: list
    ):
        """
    ...
    async def _retrieve_relevant_contexts(self, query):
        ...
        if not self.config.augmentation_ppr:
            # For HippoRAG
            retrieved_passages, scores = (
                await self._retriever.retrieve_relevant_content(
                    query=query,
        ...
        if self.config.augmentation_ppr:
            ...
            try:
                response = await self.llm.aask(
                    msg=user_prompt, system_msgs=[system_msgs]
                )
            except Exception as e:
