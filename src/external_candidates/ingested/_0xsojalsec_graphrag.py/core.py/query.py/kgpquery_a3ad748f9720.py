# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\KGPQuery.py
...
class KGPQuery(BaseQuery):
    ...
    async def _retrieve_relevant_contexts(self, query):
        corpus, candidates_idx = await self._retriever.retrieve_relevant_content(
            key="description", type=Retriever.ENTITY, mode="all"
        )
        cur_contexts, idxs = await self._retriever.retrieve_relevant_content(
            seed=query,
            corpus=corpus,
        ...
        next_reasons = [
            ...
            + (
                await self.llm.aask(
                    QueryPrompt.KGP_REASON_PROMPT.format(
        ...
        for idx, next_reason in zip(idxs, next_reasons):
            nei_candidates_idx = await self._retriever.retrieve_relevant_content(
                seed=idx, type=Retriever.ENTITY, mode="by_neighbors"
            )
            nei_candidates_idx = [_ for _ in nei_candidates_idx if _ not in visited]
            if nei_candidates_idx == []:
                continue

            next_contexts, next_idxs = await self._retriever.retrieve_relevant_content(
                seed=next_reason,
                corpus=corpus,
    ...
    async def generation_qa(self, query, context):
        ...
        context_str = "\n".join(
            "{index}: {context}".format(index=i, context=c)
            for i, c in enumerate(context, start=1)
        )

        answer = await self.llm.aask(
            QueryPrompt.KGP_QUERY_PROMPT.format(question=query, context=context_str)
        )
        return answer
