# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\ToGQuery.py
...
class ToGQuery(BaseQuery):
    ...
    async def _extend_reasoning_paths_per_depth(
        self, query: str
    ) -> tuple[bool, list[list[tuple]], list, list, list]:
            ...
            if entity != "[FINISH]":
                relations, relations_dict = (
                    await self._retriever.retrieve_relevant_content(
                        type=Retriever.RELATION,  # aim at retrieving relations
    ...
    async def _retrieve_initialization(self, query: str):
        ...
        entities = await self._retriever.retrieve_relevant_content(
            type=Retriever.ENTITY, mode="link_entity", query_entities=entities
        )
    ...
    async def _retrieve_relevant_contexts(self, query: str, mode: str) -> str:
        if mode == "retrieve":
            flag, chains, candidates, rels, heads = (
                await self._extend_reasoning_paths_per_depth(query)
            )
            # update
            ...
            if flag:
                ...
                chain_prompt = "\n".join(
                    [
                        ...
                        for sublist in self.reasoning_paths_list
    ...
    def _is_finish_list(self) -> tuple[bool, list]:
        if all(elem == "[FINISH]" for elem in self.topic_entity_candidates):
            ...
            new_lst = [
                elem for elem in self.topic_entity_candidates if elem != "[FINISH]"
            ]
    ...
    async def query(self, query):
        ...
        for depth in range(1, self.config.depth + 1):
            context = await self._retrieve_relevant_contexts(
                query=query, mode="retrieve"
            )
            if context:
                # If the length of context is not zero, we try to answer the question
                is_stop, response = await self.generation_qa(
                    mode="retrieve", context=context
                )
                if is_stop:
    ...
    async def generation_qa(
        self, mode: str, context: str
