# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Query\BasicQuery.py
...
class BasicQuery(BaseQuery):
    ...
    async def _retrieve_relevant_contexts(self, query):
        ...
        if self.config.tree_search:
            # For RAPTOR
            return await self._retriever.retrieve_relevant_content(
                seed=query, tree_node=True, type=Retriever.ENTITY, mode="vdb"
            )

        entities_context, relations_context, text_units_context, communities_context = (
            None,
            None,
            None,
            None,
        ...
        if self.config.use_keywords and self.config.use_global_query:
            entities_context, relations_context, text_units_context = (
                await self._retrieve_relevant_contexts_global_keywords(query)
            )
        if self.config.enable_local or self.config.enable_hybrid_query:
            (
                ...
                text_units_context,
                communities_context,
            ) = await self._retrieve_relevant_contexts_local(query)
        if self.config.enable_hybrid_query:
            hl_entities_context, hl_relations_context, hl_text_units_context = (
                await self._retrieve_relevant_contexts_global_keywords(query)
            )
            entities_context, relations_context, text_units_context = combine_contexts(
                entities=[entities_context, hl_entities_context],
                relationships=[relations_context, hl_relations_context],
    ...
    async def _retrieve_relevant_contexts_local(self, query):
        ...
        node_datas = await self._retriever.retrieve_relevant_content(
            seed=query, type=Retriever.ENTITY, mode="vdb"
        )

        if self.config.use_communiy_info:
            use_communities = await self._retriever.retrieve_relevant_content(
                seed=node_datas, type=Retriever.COMMUNITY, mode="from_entity"
            )
        use_relations = await self._retriever.retrieve_relevant_content(
            seed=node_datas, type=Retriever.RELATION, mode="from_entity"
        )
        use_text_units = await self._retriever.retrieve_relevant_content(
            node_datas=node_datas, type=Retriever.CHUNK, mode="entity_occurrence"
        )
        logger.info(
        ...
        relations_section_list = (
            ...
            else ["id", "source", "target", "keywords", "description", "weight", "rank"]
        )
    ...
    async def _retrieve_relevant_contexts_global(self, query):
        ...
        community_datas = await self._retriever.retrieve_relevant_content(
            type=Retriever.COMMUNITY, mode="from_level"
        )
    ...
    async def _retrieve_relevant_contexts_global_keywords(self, query):
        ...
        edge_datas = await self._retriever.retrieve_relevant_content(
            seed=query, type=Retriever.RELATION, mode="vdb"
        )
        use_entities = await self._retriever.retrieve_relevant_content(
            seed=edge_datas, type=Retriever.ENTITY, mode="from_relation"
        )
        use_text_units = await self._retriever.retrieve_relevant_content(
            seed=edge_datas, type=Retriever.CHUNK, mode="from_relation"
        )
        logger.info(
    ...
    async def generation_qa(self, query, context):
        ...
        if self.config.community_information and self.config.use_global_query:
            ...
            self.config.community_information
            and not self.config.use_keywords
            and self.config.enable_local
        ):
