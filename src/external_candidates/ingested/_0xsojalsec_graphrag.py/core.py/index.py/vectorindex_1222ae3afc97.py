# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Index\VectorIndex.py
import asyncio
import os
...
from Core.Index.BaseIndex import BaseIndex, VectorIndexEdgeResult, VectorIndexNodeResult
...
class VectorIndex(BaseIndex):
    ...
    async def retrieval(self, query, top_k):
        ...
        retriever = self._index.as_retriever(
            similarity_top_k=top_k, embed_model=self.config.embed_model
        )
    ...
    async def _update_index_from_documents(self, docs: list[Document]):
        ...
        logger.info(
            "refresh index size is {}".format(
                len([True for doc in refreshed_docs if doc])
            )
        )
    ...
    async def retrieval_nodes_with_score_matrix(self, query_list, top_k, graph):
        ...
        reset_prob_matrix[np.arange(len(query_list)).reshape(-1, 1), entity_indices] = (
            scores
        )
