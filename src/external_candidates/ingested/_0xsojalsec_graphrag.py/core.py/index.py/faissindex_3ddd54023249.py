...
class FaissIndex(BaseIndex):
    ...
    async def _update_index(self, datas: list[dict[str:Any]], meta_data: list):
        ...
        vector_store = FaissVectorStore(
            faiss_index=faiss.IndexHNSWFlat(self.embedding_model.dimensions, 32)
        )
        storage_context = StorageContext.from_defaults(vector_store=vector_store)

        self._index = VectorStoreIndex(
            [], storage_context=storage_context, embed_model=self.config.embed_model
        )
