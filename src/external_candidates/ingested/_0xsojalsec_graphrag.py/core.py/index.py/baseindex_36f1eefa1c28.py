# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Index\BaseIndex.py
import os
...
class BaseIndex(ABC):
    ...
    async def build_index(self, elements, meta_data, force=False):
        ...
        if self.exist_index() and not force:
            logger.info(
                "Loading index from the file {}".format(self.config.persist_path)
            )
        ...
        logger.info(
            "✅ Finished starting insert entities of the given graph into vector database"
        )
