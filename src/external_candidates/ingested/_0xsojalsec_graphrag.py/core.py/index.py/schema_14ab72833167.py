# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Index\Schema.py
"""RAG schemas."""
...
class ColBertIndexConfig(BaseIndexConfig):
    ...
    model_name: str = Field(
        default="colbert-ir/colbertv2.0", description="The name of the ColBERT model."
    )
    ...
    ranks: int = Field(
        default=1, description="Number of ranks for distributed indexing."
    )
    ...
    kmeans_niters: int = Field(
        default=4, description="Number of iterations for K-means clustering."
    )
