...
class ERGraph(BaseGraph):
    ...
    async def _kg_agent(self, chunk_info):
        ...
        knowledge_graph_generation_msg = Message(
            role="Graphify", content=knowledge_graph_generation
        )
    ...
    async def _build_graph_from_tuples(entities, triples, chunk_key):
        ...
        for _entity in entities:
            ...
            if entity_name == "":
                logger.warning(
                    f"entity name is not valid, entity is: {_entity}, so skip it"
                )
                continue
        ...
        for triple in triples:
            ...
            if len(triple) != 3:
                logger.warning(
                    f"triples length is not 3, triple is: {triple}, len is {len(triple)}, so skip it"
                )
                continue
            ...
            if src_entity == "" or tgt_entity == "" or relation_name == "":
                ...
                continue
            if (
                isinstance(src_entity, str)
                and isinstance(tgt_entity, str)
