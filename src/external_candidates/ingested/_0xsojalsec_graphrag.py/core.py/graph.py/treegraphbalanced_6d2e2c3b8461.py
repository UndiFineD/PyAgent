# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Graph\TreeGraphBalanced.py
import asyncio
import time
...
class TreeGraphBalanced(BaseGraph):
    ...
    def __init__(self, config, llm, encoder):
        ...
        self.embedding_model = get_rag_embedding(
            config.embedding.api_type, config
        )  # Embedding model
    ...
    async def _perform_clustering(self, embeddings: np.ndarray) -> List[np.ndarray]:

        n_samples = embeddings.shape[0]
        logger.info(
            "Perform Clustering: n_samples = {n_samples}".format(n_samples=n_samples)
        )
        ...
        def _balance_clusters():
            for i in range(n_samples):
                if (
                    cluster_sizes[new_labels[i]]
                    > n_samples / n_clusters + max_size_diff
                ):
                    small_clusters = [
                        j
                        for j in range(n_clusters)
                        if cluster_sizes[j] < n_samples / n_clusters
                    ]
                    distances = np.linalg.norm(
                        embeddings[i] - new_centers[small_clusters], axis=1
                    )
        ...
        for i in range(self.config.max_iter):
            ...
            def _process_clusters(n_clusters, new_labels):
                ...
                cluster_sizes = np.bincount(
                    new_labels, minlength=n_clusters
                )  # 更新簇大小
                new_centers = np.array(
                    [
                        embeddings[np.array(new_labels) == i].mean(axis=0)
                        for i in range(n_clusters)
                    ]
                )
                return n_clusters, new_labels, new_centers, cluster_sizes

            n_clusters, new_labels, new_centers, cluster_sizes = _process_clusters(
                n_clusters, new_labels
            )
            _balance_clusters()
            n_clusters, new_labels, new_centers, cluster_sizes = _process_clusters(
                n_clusters, new_labels
    ...
    async def _clustering(self, nodes: List[TreeNode]) -> List[List[TreeNode]]:
        ...
        clustered_indices = np.split(
            sorted_indices, np.cumsum(np.bincount(inverse_indices))[:-1]
        )
    ...
    async def _create_node(
        self, layer: int, text: str, children_indices: Set[int] = None
    ):
        embedding = self._embed_text(text)
        node_id = self._graph.num_nodes  # Give it an index
        logger.info(
            "Create node_id = {node_id}, children = {children}".format(
                node_id=node_id, children=children_indices
            )
        )
    ...
    async def _extract_cluster_relationship(
        self, layer: int, cluster: List[TreeNode]
    ) -> TreeNode:
        # Build a non-leaf node from a cluster of nodes
        summarized_text = await self._summarize_from_cluster(
            cluster, self.config.summarization_length
        )
        parent_node = await self._create_node(
            layer, summarized_text, {node.index for node in cluster}
        )
        return parent_node

    async def _create_node_without_embedding(
        self, layer: int, text: str, children_indices: Set[int] = None
    ):
        # embedding = self._embed_text(text)
        logger.info(
            "Create node_id = unassigned, children = {children}".format(
                node_id=0, children=children_indices
            )
        )
        return self._graph.upsert_node(
            node_id=0,
    ...
    async def _extract_entity_relationship_without_embedding(
        self, chunk_key_pair: tuple[str, TextChunk]
    ) -> TreeNode:
        # Build a leaf node from a text chunk
        chunk_key, chunk_info = chunk_key_pair
    ...
    async def _extract_cluster_relationship_without_embedding(
        self, layer: int, cluster: List[TreeNode]
    ) -> TreeNode:
        # Build a non-leaf node from a cluster of nodes
        summarized_text = await self._summarize_from_cluster(
        ...
        parent_node = await self._create_node_without_embedding(
            layer, summarized_text, {node.index for node in cluster}
        )
        return parent_node

    async def _summarize_from_cluster(
        self, node_list: List[TreeNode], summarization_length=150
    ) -> str:
        # Give a summarization from a cluster of nodes
        node_texts = f"\n\n".join(
            [" ".join(node.text.splitlines()) for node in node_list]
        )
    ...
    async def _build_tree_from_leaves(self):
        for layer in range(self.config.num_layers):  # build a new layer
            logger.info(
                "length of layer: {length}".format(
                    length=len(self._graph.get_layer(layer))
                )
            )
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                for i in range(0, self.max_workers):
                    cluster_tasks = [
                        pool.submit(
                            self._create_task_for(
                                self._extract_cluster_relationship_without_embedding
                            ),
                            layer=layer + 1,
                            cluster=cluster,
                        )
                        for (j, cluster) in enumerate(clusters)
                        if j % self.max_workers == i
                    ]
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                ...
                for i in range(0, self.max_workers):
                    leaf_tasks = [
                        pool.submit(
                            self._create_task_for(
                                self._extract_entity_relationship_without_embedding
                            ),
                            chunk_key_pair=chunk,
                        )
                        for index, chunk in enumerate(chunks)
                        if index % self.max_workers == i
                    ]
