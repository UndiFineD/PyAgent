# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Graph\PassageGraph.py
import asyncio
import glob
import os
import pickle
...
class PassageGraph(BaseGraph):
    ...
    async def _extract_entity_relationship(
        ...
        logger.info(
            "Linking Entity by WAT system for chunk {chunk_key}".format(
                chunk_key=chunk_key
            )
        )
    ...
    def _run_pool_extract_relationship(self, chunk_per_pair):
        ...
        result = loop.run_until_complete(
            self._extract_entity_relationship(chunk_per_pair)
        )
        loop.close()
        return result
    ...
    async def _build_graph(self, chunk_list: List[Any]):
        ...
        try:
            ...
            cold_start = 169
            save_files = sorted(
                glob.glob("alce_save_*.pkl"), key=os.path.getmtime, reverse=True
            )
            ...
            with ThreadPoolExecutor(max_workers=n) as pool:
                ...
                for i in tqdm(range(num + 1)):
                    ...
                    for j in range(n):
                        if i * n + j < total_num:
                            task = pool.submit(
                                self._run_pool_extract_relationship,
                                chunk_list[i * n + j],
                            )
                            tasks.append(task)
                    import pdb
    ...
    async def __passage_graph__(self, elements, chunk_list: List[Any]):
        ...
        for chunk_pair in chunk_list:
            ...
            node_data = Entity(
                entity_name=chunk_pair[0],
                description=chunk_pair[1].content,
                source_id=chunk_pair[0],
            )
        ...
        with ThreadPoolExecutor(max_workers=n) as pool:
            ...
            for i in tqdm(range(num + 1)):
                ...
                for j in range(n):
                    if i * n + j < total_num:
                        task = pool.submit(
                            self._run_pool_merge_relationship, edge_list[i * n + j]
                        )
                        tasks.append(task)
                for task_1 in as_completed(tasks):
                    result = task_1.result()
