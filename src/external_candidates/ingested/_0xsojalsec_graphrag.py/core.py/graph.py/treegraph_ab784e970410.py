# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\Graph\TreeGraph.py
import asyncio
import time
...
class TreeGraph(BaseGraph):
    ...
    async def _clustering(
        ...
        if len(np.unique(np.concatenate(clusters))) == 1:
            logger.info(
                "Only one cluster length = {len}, return".format(len=len(nodes))
            )
            return [nodes]
        ...
        for label in np.unique(np.concatenate(clusters)):
            ...
            if (
                total_length > max_length_in_cluster
                and len(cluster_nodes) > self.config.reduction_dimension + 1
            ):
                if verbose:
