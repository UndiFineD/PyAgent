...
class BaseGraph(ABC):
    ...
    async def _merge_nodes_then_upsert(
        self, entity_name: str, nodes_data: List[Entity]
    ):
        existing_node = await self._graph.get_node(entity_name)

        existing_data = (
            build_data_for_merge(existing_node) if existing_node else defaultdict(list)
        )
    ...
    async def _merge_edges_then_upsert(
        ...
        description = (
            await self._handle_entity_relation_summary(
                (src_id, tgt_id), merge_description
            )
            if self.config.enable_edge_description
            else ""
        )
    ...
    async def augment_graph_by_similarity_search(self, entity_vdb, duplicate=False):
        ...
        for node in tqdm.tqdm(
            await self._graph.nodes(), total=len(await self._graph.nodes())
        ):
        ...
        for key, rank in ranking.items():
            ...
            for idx, ns_item in enumerate(rank):
                ...
                if is_euclidean_distance:
                    kb_similarity[key].append(
                        (ns_item.metadata["entity_name"], 1 - score / max_score)
                    )
                else:
                    kb_similarity[key].append(
                        (ns_item.metadata["entity_name"], score / max_score)
                    )
        ...
        for src_id, nns in kb_similarity.items():
            ...
            for idx, (nn, score) in enumerate(nns):
                ...
                if (
                    ...
                    or idx >= self.config.similarity_top_k
                ):
                    break
                if nn == src_id:
                    continue
    ...
    async def _handle_entity_relation_summary(
        ...
        use_description = self.ENCODER.decode(
            tokens[: self.config.llm_model_max_token_size]
        )
        ...
        return await self.llm.aask(
            use_prompt, max_tokens=self.config.summary_max_tokens
        )
    ...
    async def stable_largest_cc(self):
        if isinstance(self._graph, NetworkXStorage):
            ...
            logger.exception(
                "**Only NETWORKX is supported for finding the largest connected component.** "
            )
            return None
    ...
    async def cluster_data_to_subgraphs(self, cluster_data: dict):
        if isinstance(self._graph, NetworkXStorage):
            ...
            logger.exception(
                "**Only NETWORKX is supported for constructing the cluster <-> node mapping.** "
            )
            return None
    ...
    async def get_relationships_to_chunks_map(self, doc_chunk):
        raw_relationships_to_chunks = await self.get_relationships_attrs(
            key="source_id"
        )
        ...
        raw_relationships_to_chunks = [
            [
                i
                for i in await doc_chunk.get_index_by_merge_key(chunk_ids)
                if i is not None
            ]
    ...
    async def find_k_hop_neighbors_batch(self, start_nodes: list[str], k: int):
        return await self._graph.find_k_hop_neighbors_batch(
            start_nodes=start_nodes, k=k
        )  # set
