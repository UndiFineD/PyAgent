# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphRAG\Core\GraphRAG.py
...
class GraphRAG(ContextMixin, BaseModel):
    ...
    def _update_context(cls, data):
        ...
        cls.workspace = Workspace(
            data.config.working_dir, data.config.index_name
        )  # register workspace
        cls.graph = get_graph(
            data.config, llm=data.llm, encoder=cls.ENCODER
        )  # register graph
        cls.doc_chunk = DocChunk(
            data.config.chunk, cls.ENCODER, data.workspace.make_for("chunk_storage")
        )
    ...
    def _register_vdbs(cls, data):
        ...
        if data.config.use_entities_vdb:
            cls.entities_vdb = get_index(
                get_index_config(
                    data.config,
                    persist_path=data.entities_vdb_namespace.get_save_path(),
                )
            )
        if data.config.use_relations_vdb:
            cls.relations_vdb = get_index(
                get_index_config(
                    data.config,
                    persist_path=data.relations_vdb_namespace.get_save_path(),
                )
            )
        if data.config.use_subgraphs_vdb:
            cls.subgraphs_vdb = get_index(
                get_index_config(
                    data.config,
                    persist_path=data.subgraphs_vdb_namespace.get_save_path(),
                )
            )
    ...
    def _register_community(cls, data):
        if data.config.graph.use_community:
            cls.community = get_community(
                data.config.graph.graph_cluster_algorithm,
    ...
    def _register_e2r_r2c_matrix(cls, data):
        ...
        if data.config.graph.graph_type == "tree_graph":
            logger.warning(
                "Tree graph is not supported for entity-link-chunk mapping. Skipping entity-link-chunk mapping."
            )
            data.config.use_entity_link_chunk = (
                False  # Disable entity-link-chunk mapping if tree graph is used.
            )
            return data
        if data.config.use_entity_link_chunk:
    ...
    async def _build_retriever_context(self):
        ...
        try:
            ...
            self._querier = get_query(
                self.config.retriever.query_type,
                self.config.query,
                self.retriever_context,
            )
    ...
    async def build_e2r_r2c_maps(self, force=False):
        ...
        logger.info(
            "Starting build two maps: 1️⃣ entity <-> relationship; 2️⃣ relationship <-> chunks "
        )
        if not await self.entities_to_relationships.load(force):
            await self.entities_to_relationships.set(
                await self.graph.get_entities_to_relationships_map(False)
            )
            await self.entities_to_relationships.persist()
        if not await self.relationships_to_chunks.load(force):
            await self.relationships_to_chunks.set(
                await self.graph.get_relationships_to_chunks_map(self.doc_chunk)
            )
    ...
    def _update_costs_info(self, stage_str: str):
        ...
        logger.info(
            f"{stage_str} stage cost: Total prompt token: {last_cost.total_prompt_tokens}, Total completeion token: {last_cost.total_completion_tokens}, Total cost: {last_cost.total_cost}"
        )
    ...
    async def insert(self, docs: Union[str, list[Any]]):
        ...
        await self.graph.build_graph(
            await self.doc_chunk.get_chunks(), self.config.graph.force
        )
        ...
        if self.config.use_entities_vdb:
            ...
            await self.entities_vdb.build_index(
                await self.graph.nodes_data(), node_metadata, False
            )
        ...
        if self.config.use_relations_vdb:
            ...
            await self.relations_vdb.build_index(
                await self.graph.edges_data(), edge_metadata, force=False
            )
        ...
        if self.config.use_subgraphs_vdb:
            ...
            await self.subgraphs_vdb.build_index(
                await self.graph.subgraphs_data(), subgraph_metadata, force=False
            )
