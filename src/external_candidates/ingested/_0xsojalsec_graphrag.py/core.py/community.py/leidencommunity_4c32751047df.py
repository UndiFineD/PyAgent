...
class LeidenCommunity(BaseCommunity):
    def __init__(self, **kwargs):
        ...
        self._community_reports: JsonKVStorage = JsonKVStorage(
            self.namespace, "community_report"
        )
        self._community_node_map: JsonKVStorage = JsonKVStorage(
            self.namespace, "community_node_map"
        )
    ...
    async def _generate_community_report(self, er_graph):
        ...
        if self._communities_schema is None:
            logger.warning(
                "No community schema found, skipping community report generation."
            )
            return None
        community_keys, community_values = list(self._communities_schema.keys()), list(
            self._communities_schema.values()
        )
        ...
        for level in levels:
            ...
            this_level_communities_reports = await asyncio.gather(
                *[
                    self._form_single_community_report(er_graph, c, community_datas)
    ...
    async def _form_single_community_report(
        ...
        describe = await self._pack_single_community_describe(
            er_graph, community, already_reports=already_reports
        )
    ...
    async def _pack_single_community_describe(
        ...
        nodes_data = await asyncio.gather(
            *[er_graph.get_node(n) for n in nodes_in_order]
        )
        ...
        nodes_describe = list_to_quoted_csv_string(
            [node_fields] + nodes_may_truncate_list_data
        )
        edges_describe = list_to_quoted_csv_string(
            [edge_fields] + edges_may_truncate_list_data
        )
    ...
    async def _persist_community(self):
        try:
            ...
            logger.exception(
                "❌ Failed to persist community report: {error}.".format(error=e)
            )
    ...
    async def _persist_cluster_map(self):
        try:
            ...
            logger.exception(
                "❌ Failed to persist community <-> node map: {error}.".format(error=e)
            )
