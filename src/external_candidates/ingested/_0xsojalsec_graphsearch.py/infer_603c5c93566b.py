...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-k", "--top_k", default="5", type=int, help="top retrieved items."
    )
    args = parser.parse_args()

    if args.method in ["graphsearch", "grag"]:
        grag_method = initialize_grag(
            grag_name=args.graphrag, top_k=args.top_k, dataset=args.dataset
        )
    ...
    if args.method == "vanillallm":
        ...
        asyncio.run(
            naive_rag_reasoning(question, documents, index, embed_model, args.top_k)
        )
