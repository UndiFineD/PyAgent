# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GraphSearch\graphrags\base.py
...
class GraphRAGBase(ABC):
    ...
    async def aquery_context(self, question: str):
        return await self.grag.aquery(
            question,
            self.QueryParam(
                mode=self.grag_mode, only_need_context=True, top_k=self.top_k
            ),
        )
    ...
    async def aquery_answer(self, question: str):
        return await self.grag.aquery(
            question,
            self.QueryParam(
                mode=self.grag_mode, only_need_context=False, top_k=self.top_k
            ),
        )
