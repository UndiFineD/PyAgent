...
async def graph_search_reasoning(question: str, grag_method: GraphRAGBase):
    ...
    for i, sub_query in enumerate(sub_queries):
        ...
        if "#" in sub_query:
            sub_query = await query_completer(
                sub_query, decomposition_output + "\n\n" + text_query_history_str
            )
        ...
        # Semantic Filtering
        sub_query_context = grag_method.context_filter(
            context_data=sub_query_context, filter_type="semantic"
        )
        ...
        text_query_history.append(
            (sub_query, sub_query_context_summary, sub_query_answer)
        )
    ...
    text_verification_result = await evidence_verification(
        question, text_query_history_str, text_final_answer
    )
    ...
    if "yes" in normalize(text_verification_result):
        ...
        for expanded_query in expanded_queries:
            expanded_query_context = await grag_method.aquery_context(
            ...
            expanded_query_context_summary = await text_summary(
                expanded_query, expanded_query_context
            )
            logging.info(
                f"Expanded Query Context Summary: {expanded_query_context_summary}"
            )

            text_query_history.append(
                (expanded_query, expanded_query_context_summary, "")
            )
    ...
    for i, sub_kg_query in enumerate(sub_kg_queries):
        ...
        sub_kg_query_context = await grag_method.aquery_context(
            question=sub_kg_query_cleaned
        )
        # Relational Channel
        sub_kg_query_context = grag_method.context_filter(
            context_data=sub_kg_query_context, filter_type="relational"
        )
        logging.info(f"Sub KG Query Context: {sub_kg_query_context}")

        sub_kg_query_context_summary = await kg_summary(
            sub_kg_query, sub_kg_query_context
        )
        logging.info(f"Sub KG Query Context Summary: {sub_kg_query_context_summary}")

        sub_kg_query_context_data = (
            kg_query_history_str + "\n\n" + sub_kg_query_context_summary
        )
        sub_kg_query_answer = await answer_generation(
            sub_kg_query, sub_kg_query_context_data
        )
        logging.info(f"Sub KG Query Answer: {sub_kg_query_answer}")

        kg_query_history.append(
            (sub_kg_query, sub_kg_query_context_summary, sub_kg_query_answer)
        )
    ...
    # Self-Reflection, Optional
    # Evidence verification
    kg_verification_result = await evidence_verification(
        question, kg_query_history_str, kg_final_answer
    )
    ...
    if "yes" in normalize(kg_verification_result):
        ...
        for expanded_query in expanded_queries:
            ...
            expanded_query_context_summary = await kg_summary(
                expanded_query, expanded_query_context
            )
            logging.info(
                f"Expanded KG Query Context Summary: {expanded_query_context_summary}"
            )
