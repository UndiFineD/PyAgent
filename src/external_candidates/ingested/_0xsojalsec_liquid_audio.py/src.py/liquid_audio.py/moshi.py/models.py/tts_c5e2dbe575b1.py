...
def _delayed(codes: torch.Tensor, delays: list[int], fill_value: int) -> torch.Tensor:
    ...
    K, T = codes.shape
    out = torch.full(
        (K, T + max(delays)), fill_value, device=codes.device, dtype=torch.long
    )
...
class TTSModel:
            ...
            if tts_model.n_q < tts_model.lm.n_q:
                ...
                for n in range(mimi_n_q, tts_model.lm.n_q // 2):
                    ...
                    for q in [n, n + tts_model.lm.n_q // 2]:
                        tp.cast(ScaledEmbedding, tts_model.lm.emb[q]).weight.data[
                            :
                        ] = 0.0
                        if q < tts_model.lm.n_q - 1:
                            tp.cast(
                                ScaledEmbedding, tts_model.lm.depformer_emb[q]
                            ).weight.data[:] = 0.0
    ...
    def valid_cfg_conditionings(self) -> set[float]:
        ...
        if (
            ...
            valid_cfg_conditionings = set(
                float(x) for x in cfg_conditioner.tokenizer.possible_values
            )
        ...
        with lm_gen.streaming(len(states)):
            for offset in range(self.max_gen_length):
                ...
                depformer_replace_tokens = (
                    no_depformer_tokens if offset < self.delay_steps else None
                )
        ...
        if voices:
            ...
            for idx in range(5):
                if idx < len(voices):
                    ...
                    if mask is None:
                        mask = torch.zeros(
                            1, self.max_speakers, emb.shape[2], dtype=torch.bool
                        )
    ...
    def get_prefix(self, audio_path: Path) -> torch.Tensor:
        ...
        with torch.no_grad():
            prefix = self.mimi.encode(
                torch.from_numpy(wav).to(device=self.lm.device)[None]
            )[0, :, :-2]
