...
class LMModel(StreamingContainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.text_emb = EmbeddingFactory(
            text_card + 1, dim, demux_second_stream=demux_second_text_stream
        )
        ...
        EmbeddingFactory = partial(
            EmbeddingFactory, low_rank=depformer_low_rank_embeddings
        )
        if dep_q > 0:
            # Only using up to dep_q - 1 because the last codebook is never an input to Depformer.
            self.depformer_emb = nn.ModuleList(
                [
                    EmbeddingFactory(self.card + 1, depformer_dim)
                    for _ in range(dep_q - 1)
                ]
            )
    ...
    def forward_text(
        ...
        transformer_out = self.transformer(
            input_, cross_attention_src=cross_attention_src
        )
        if self.out_norm:
...
class LMGen(StreamingModule[_LMGenState]):
    def __init__(
        ...
        if self.cfg_coef != 1.0:
            if not self.cfg_is_no_text and not self.cfg_is_masked_until:
                assert (
                    self.lm_model.fuser is not None
                ), "Model has no fuser, cannot do CFG."
    ...
    def _init_streaming_state(self, batch_size: int) -> _LMGenState:
        ...
        if self.cfg_is_masked_until is None:
            ...
            cfg_is_masked_until = torch.tensor(
                self.cfg_is_masked_until, dtype=torch.long, device=lm_model.device
            )
        ...
        if self.cfg_coef != 1.0:
            ...
            if state.condition_sum is not None:
                assert (
                    state.condition_sum.shape[0] == batch_size
                ), "cfg requires 2x more conditions."
            if state.condition_cross is not None:
                assert (
                    state.condition_cross.shape[0] == batch_size
                ), "cfg requires 2x more conditions."
    ...
    def _step(
        ...
        assert (
            B == state.batch_size
        ), f"Got a batch size {B}, expected {state.batch_size}"
        ...
        is_init |= ~state.exec_mask[
            :, None, None
        ]  # we also give init tokens if not executing to avoid crashing.
        ...
        zero = torch.full(
            (1,), self.lm_model.zero_token_id, dtype=torch.long, device=input_.device
        )
        if self.cfg_coef != 1.0:
            if state.cfg_is_masked_until is not None:
                limit = self.delays_cuda[:, None] + state.cfg_is_masked_until.view(
                    -1, 1, 1
                )
        ...
        index = (
            state.offsets[:, None, None] - self.max_delay + gen_delays_cuda[:, None]
        ) % CT
