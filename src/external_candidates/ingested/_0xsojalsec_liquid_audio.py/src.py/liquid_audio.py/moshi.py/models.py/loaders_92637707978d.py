# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\moshi\models\loaders.py
...
class CheckpointInfo:
    ...
    def get_mimi(self, device: torch.device | str = "cpu") -> MimiModel:
        if self.lm_config is None:
            ...
            num_codebooks = max(
                self.lm_config["dep_q"], self.lm_config["n_q"] - self.lm_config["dep_q"]
            )
        ...
        if _is_safetensors(filename):
            ...
            for key, value in state.items():
                if value.dtype.is_floating_point:
                    if key.startswith("condition_provider.") or key.startswith(
                        "fuser."
                    ):
                        value = value.float()
                    else:
