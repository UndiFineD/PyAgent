# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\moshi\modules\lora.py
import torch
import torch.nn as nn


def replace_all_linear_with_lora(
    module, rank: int, scaling: float, device=None, dtype=None
):
...
def replace_lora_with_linear(module):
    ...
    for name, child in module.named_children():
        if isinstance(child, LoRALinear):
            ...
            merged_weight = child.frozen_W.weight.data + child.scaling * (
                child.lora_B.weight @ child.lora_A.weight
            )
