# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\moshi\modules\conv_test.py
import functools
...
torch.backends.cudnn.enabled = (
    False  # Disable cuDNN for deterministic behavior and for numerical stability
)
...
@pytest.mark.parametrize(
    "batch_size, in_channels, out_channels, seq_len, kernel_size", CONV1D_DATA
)
...
def test_conv1d_transpose(
    ...
    layer = StreamingConvTranspose1d(
        in_channels, out_channels, kernel_size, stride, causal=True, norm="none"
    )
...
def test_conv1d_transpose_streaming(
    batch_size, in_channels, out_channels, seq_len, kernel_size, stride
):
