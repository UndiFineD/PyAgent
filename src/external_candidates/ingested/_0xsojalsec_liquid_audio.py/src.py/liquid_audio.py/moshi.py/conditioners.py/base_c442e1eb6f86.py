# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\moshi\conditioners\base.py
...
def dropout_tensor(condition: TensorCondition) -> TensorCondition:
    ...
    return TensorCondition(
        tensor=torch.zeros_like(condition.tensor), mask=torch.zeros_like(condition.mask)
    )
...
class ConditionProvider(nn.Module):
    ...
    def tensor_conditions(self):
        return [
            k
            for k, v in self.conditioners.items()
            if isinstance(v, _BaseTensorConditioner)
        ]
    ...
    def prepare(self, inputs: tp.Sequence[ConditionAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        assert set(text.keys() | tensors.keys()).issubset(
        ...
        missing_inputs = set(self.conditioners.keys()) - (
            set(text.keys()) | set(tensors.keys())
        )
        if missing_inputs:
            raise RuntimeError(
        ...
        for fuse_method, conditions in fuse2cond.items():
            for condition in conditions:
                ...
                if fuse_method not in ["cross", "sum"]:
                    raise RuntimeError(
                        "only `sum` and `cross` conditionings are supported "
