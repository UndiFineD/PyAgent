# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\model\transformer.py
import math
...
class SequenceModel(nn.Module, ABC):
    ...
    @abstractmethod
    def forward_cached(
        self, x: torch.Tensor, cache: CacheType = None
...
class BoundedAttention(nn.Module):
    def __init__(
        ...
        if self.head_style == "gqa":
            ...
            assert (
                self.num_heads % self.gqa_dim == 0
            ), f"{self.gqa_dim} % {self.head_dim} != 0"
...
class MHA(SequenceModel):
    def __init__(
        ...
        ), "expected dim to be divisible by num_heads"
    ...
    ) -> TypeGuard[tuple[torch.Tensor, torch.Tensor]]:
        return (
    ...
    def forward_cached(
        ...
        ys, new_cache = self.bounded_attention(
            xq, xk, xv, freqs_cis=freqs_cis, cache=kv_cache
        )
...
class StandardBlock(SequenceModel):
    ...
    def forward_cached(
        self, x: torch.Tensor, cache: CacheType | None = None
    ) -> tuple[torch.Tensor, CacheType]:
...
def apply_rotary_emb(
    ...
    xq_ = torch.view_as_complex(
        rearrange(xq.float(), "... (D two) -> ... D two", two=2)
    )
    xk_ = torch.view_as_complex(
        rearrange(xk.float(), "... (D two) -> ... D two", two=2)
    )
    ...
    def __init__(
        ...
        *,
        ...
        self.embedding_norm = RMSNorm(
            dim, eps=norm_eps
        )  # Note: this is really the norm before output projection
...
class RawLMBackbone(SequenceModel):
    ...
    def __init__(
        ...
        *,
        ...
        ), "expected first layer input dim to be equal to last layer's output dim"
