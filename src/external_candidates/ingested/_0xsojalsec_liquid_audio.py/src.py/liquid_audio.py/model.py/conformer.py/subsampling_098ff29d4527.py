# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\model\conformer\subsampling.py
        ...
        if self.subsampling_conv_chunking_factor != -1 and self.conv2d_subsampling:
            if self.subsampling_conv_chunking_factor == 1:
                ...
                out_size = (
                    x.shape[0]
                    * self._conv_channels
                    * ((x.shape[1] + 1) // self._stride)
                    * ((x.shape[2] + 1) // self._stride)
                )
                if out_size >= x_ceil:
                    need_to_split = True
                else:
