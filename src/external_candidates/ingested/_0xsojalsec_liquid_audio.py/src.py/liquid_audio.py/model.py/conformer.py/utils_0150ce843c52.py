# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\model\conformer\utils.py
...
def compute_stochastic_depth_drop_probs(
    ...
    if (L := num_layers - stochastic_depth_start_layer) > 0:
        if stochastic_depth_mode == "linear":
            ...
            layer_drop_probs += [
                l / L * stochastic_depth_drop_prob for l in range(1, L + 1)
            ]
