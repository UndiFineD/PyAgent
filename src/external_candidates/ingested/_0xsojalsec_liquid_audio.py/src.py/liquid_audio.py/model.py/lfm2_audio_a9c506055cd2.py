# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-liquid-audio\src\liquid_audio\model\lfm2_audio.py
...
from liquid_audio.model.conformer.encoder import (
    ConformerEncoder,
...
class LFM2AudioModel(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "codebook_offsets",
            torch.arange(self.conf.codebooks) * self.audio_vocab_size,
        )

        self.audio_loss_weights: torch.Tensor
        if conf.codebook_weight == "log":
            weights = (
                torch.linspace(1, 0, self.codebooks)
                * math.log(conf.semantic_codebook_factor)
            ).exp()
        else:
        ...
        layers = [
            StandardBlock(
                MHA(self.depthformer_dim, out_init_scale=scale), out_init_scale=scale
            )
        ...
        self.depth_linear = nn.Linear(
            self.lfm.config.hidden_size, self.depthformer_dim * self.codebooks
        )
        ...
        *,
        ...
        for _ in range(max_new_tokens):
            ...
            if current_modality == LFMModality.TEXT:
                text_logits = nn.functional.linear(
                    output_embeddings[0, -1], self.lfm.embed_tokens.weight
                )
                next_token = self._sample_text_token(
                    text_logits, temperature=text_temperature, top_k=text_top_k
                )
                yield next_token
                ...
                yield next_token
                in_emb = self.audio_embedding(next_token + self.codebook_offsets).sum(
                    0
                )[None, None, :]
    ...
    def _prefill(
        ...
        *,
        ...
        assert (modality_flag == LFMModality.AUDIO_IN).sum() == mel2emb_len(
            audio_in_lens
        ).sum()
        ...
        ## Unbatch, unpad
        len_mask = torch.arange(audio_enc.shape[-1], device=audio_enc.device).unsqueeze(
            0
        ) < audio_in_len.unsqueeze(1)
        ...
        offset_audio_tokens = audio_out[
            : self.codebooks
        ] + self.codebook_offsets.unsqueeze(1)
        ...
        *,
        ...
        for i in range(self.codebooks):
            ...
            depthformer_out, cache = self.depthformer.forward_cached(
                cur_depthformer_input[None, None, :], cache
            )
            ...
            if greedy:
                ...
                if top_k is not None:
                    ...
                    depthformer_logits = torch.masked_fill(
                        depthformer_logits, to_remove, -float("inf")
                    )
