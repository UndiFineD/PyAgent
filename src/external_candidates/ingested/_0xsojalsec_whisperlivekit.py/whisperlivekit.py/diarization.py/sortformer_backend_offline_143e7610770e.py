# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\diarization\sortformer_backend_offline.py
import logging
...
def load_model():
    ...
    diar_model.sortformer_modules.chunk_left_context = (
        10  # big so it compensiate the problem with no padding later.
    )
    ...
    )  # pad_to 16 works better than 0. On test audio, we detect a third speaker for 1 second with pad_to=0. To solve that : increase left context to 10.
...
def init_streaming_state(
    ...
    if async_streaming:
        streaming_state.spkcache = torch.zeros(
            (batch_size, self.spkcache_len, self.fc_d_model), device=device
        )
        streaming_state.spkcache_preds = torch.zeros(
            (batch_size, self.spkcache_len, self.n_spk), device=device
        )
        streaming_state.spkcache_lengths = torch.zeros(
            (batch_size,), dtype=torch.long, device=device
        )
        streaming_state.fifo = torch.zeros(
            (batch_size, self.fifo_len, self.fc_d_model), device=device
        )
        ...
        streaming_state.spkcache = torch.zeros(
            (batch_size, 0, self.fc_d_model), device=device
        )
        streaming_state.fifo = torch.zeros(
            (batch_size, 0, self.fc_d_model), device=device
        )
    streaming_state.mean_sil_emb = torch.zeros(
        (batch_size, self.fc_d_model), device=device
    )
...
def process_diarization(chunks):
    ...
    for chunk in chunks:
        ...
        processed_signal_chunk, processed_signal_length_chunk = audio2mel.get_features(
    ...
    total_preds = torch.zeros(
        (batch_size, 0, diar_model.sortformer_modules.n_spk), device=diar_model.device
    )

    chunk_duration_seconds = (
        diar_model.sortformer_modules.chunk_len
        * diar_model.sortformer_modules.subsampling_factor
        * diar_model.preprocessor._cfg.window_stride
    )
    ...
    for i, chunk_feat_seq_t in enumerate(l_chunk_feat_seq_t):
        with torch.inference_mode():
            ...
            if len_prediction is None:
                len_prediction = len(
                    active_speakers
                )  # we want to get the len of 1 prediction
            ...
            for idx, spk in enumerate(active_speakers):
                if spk != l_speakers[-1]["speaker"]:
                    l_speakers.append(
                        {
                            "start_time": (
                                i * chunk_duration_seconds + idx * frame_duration
                            ),
                            "end_time": (
                                i * chunk_duration_seconds + (idx + 1) * frame_duration
                            ),
                            "speaker": spk,
                        }
                    )
                else:
                    l_speakers[-1]["end_time"] = (
                        i * chunk_duration_seconds + (idx + 1) * frame_duration
                    )
    ...
    for speaker in l_speakers:
        print(
            f"Speaker {speaker['speaker']}: {speaker['start_time']:.2f}s - {speaker['end_time']:.2f}s"
        )
