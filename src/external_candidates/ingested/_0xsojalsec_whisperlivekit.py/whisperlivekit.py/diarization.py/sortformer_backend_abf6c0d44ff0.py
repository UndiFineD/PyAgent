# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\diarization\sortformer_backend.py
import logging
import threading
import time
import wave
...
class SortformerDiarizationOnline:
    ...
    def _init_streaming_state(self):
        ...
        self.streaming_state.spkcache = torch.zeros(
            (
                ...
                self.diar_model.sortformer_modules.fc_d_model,
            ),
            device=device,
        )
        self.streaming_state.spkcache_preds = torch.zeros(
            (
                batch_size,
                self.diar_model.sortformer_modules.spkcache_len,
                self.diar_model.sortformer_modules.n_spk,
            ),
            device=device,
        )
        ...
        self.streaming_state.fifo = torch.zeros(
            (
                batch_size,
                self.diar_model.sortformer_modules.fifo_len,
                self.diar_model.sortformer_modules.fc_d_model,
            ),
            device=device,
        )
        ...
        self.streaming_state.mean_sil_emb = torch.zeros(
            (batch_size, self.diar_model.sortformer_modules.fc_d_model), device=device
        )
    ...
    async def diarize(self, pcm_array: np.ndarray):
        ...
        try:
            ...
            audio_signal_chunk = (
                torch.tensor(audio).unsqueeze(0).to(self.diar_model.device)
            )
            audio_signal_length_chunk = torch.tensor([audio_signal_chunk.shape[1]]).to(
                self.diar_model.device
            )

            processed_signal_chunk, processed_signal_length_chunk = (
                self.audio2mel.get_features(
        ...
        try:
            ...
            with self.segment_lock:
                ...
                for idx, spk in enumerate(current_chunk_preds):
                    ...
                    if (
                        self.speaker_segments
                        and self.speaker_segments[-1].speaker == spk
                        and abs(self.speaker_segments[-1].end - start_time)
                        < frame_duration * 0.5
                    ):
                        ...
                        self.speaker_segments.append(
                            SpeakerSegment(speaker=spk, start=start_time, end=end_time)
                        )

                # Update processed time
                self.processed_time = max(
                    self.processed_time, base_time + self.chunk_duration_seconds
                )

                logger.debug(
                    f"Processed chunk {self._chunk_index}, total segments: {len(self.speaker_segments)}"
                )
    ...
    def assign_speakers_to_tokens(
        self, tokens: list, use_punctuation_split: bool = False
    ) -> list:
        """
        ...
        logger.debug(
            f"Assigning speakers to {len(tokens)} tokens using {len(segments)} segments"
        )
    ...
    def _add_speaker_to_tokens_with_punctuation(
        self, segments: List[SpeakerSegment], tokens: list
    ) -> list:
        """
        ...
        punctuation_marks = {".", "!", "?"}
        punctuation_tokens = [
            token for token in tokens if token.text.strip() in punctuation_marks
        ]
            ...
            for i, punctuation_token in enumerate(punctuation_tokens):
                if punctuation_token.start > segment["end"]:
                    ...
                    if before_length > after_length:
                        ...
                        if i < len(punctuation_tokens) - 1 and ind + 1 < len(
                            segments_concatenated
                        ):
                            segments_concatenated[ind + 1][
                                "begin"
                            ] = punctuation_token.start
                    else:
                        segment["end"] = (
                            punctuation_tokens[i - 1].end if i > 0 else segment["end"]
                        )
                        if i < len(punctuation_tokens) - 1 and ind - 1 >= 0:
                            segments_concatenated[ind - 1]["begin"] = (
                                punctuation_tokens[i - 1].end
                            )
                    break
    ...
    def _concatenate_speakers(self, segments: List[SpeakerSegment]) -> List[dict]:
        ...
        for segment in segments[1:]:
            ...
            if segments_concatenated[-1]["speaker"] != speaker:
                segments_concatenated.append(
                    {"speaker": speaker, "begin": segment.start, "end": segment.end}
                )
            else:
    ...
    def clear_old_segments(self, older_than: float = 30.0):
        ...
        with self.segment_lock:
            ...
            logger.debug(
                f"Cleared old segments, remaining: {len(self.speaker_segments)}"
            )
