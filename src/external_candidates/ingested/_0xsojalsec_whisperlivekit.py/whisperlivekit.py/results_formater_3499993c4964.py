# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\results_formater.py
...
def new_line(token, speaker, last_end_diarized, debug_info=""):
    return {
        "speaker": int(speaker),
...
def format_output(state, silence, current_time, diarization, debug):
    ...
    last_end_diarized = 0
    undiarized_text = []
    tokens, buffer_transcription, buffer_diarization = handle_silences(
        tokens, buffer_transcription, buffer_diarization, current_time, silence
    )
    ...
    for i, token in enumerate(tokens):
        ...
        if (
            not diarization and speaker == -1
        ):  # Speaker -1 means no attributed by diarization. In the frontend, it should appear under 'Speaker 1'
            speaker = 1
        ...
        if last_punctuation == i - 1:
            ...
            if speaker_change_pos:
                ...
                lines.append(
                    new_line(token, new_speaker, last_end_diarized, debug_info="")
                )
            else:
                # No speaker change to come
                append_token_to_last_line(
                    lines, sep, token, debug_info, last_end_diarized
                )
            continue
