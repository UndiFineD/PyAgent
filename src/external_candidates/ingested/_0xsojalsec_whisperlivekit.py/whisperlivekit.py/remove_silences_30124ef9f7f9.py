# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WhisperLiveKit\whisperlivekit\remove_silences.py
import re
...
END_SILENCE_DURATION_VAC = (
    3  # VAC is good at detecting silences, but we want to skip the smallest silences
)
...
def blank_to_silence(tokens):
    ...
    patterns = [
        re.compile(r"(?:\s*\[BLANK_AUDIO\]\s*)+"),
        re.compile(r"(?:\s*\[typing\]\s*)+"),
    ]
    matches = []
            ...
            if matches:
                ...
                if start >= matches[0]["start"] and end <= matches[0]["end"]:
                    ...
                    if silence_token:  # there was silence but no more
                        if (
                            silence_token.end - silence_token.start
                            >= MIN_SILENCE_DURATION
                        ):
                            cleaned_tokens.append(silence_token)
                        silence_token = None
                        matches.pop(0)
...
def no_token_to_silence(tokens):
    ...
    for token in tokens:
        if token.speaker == -2:
            if (
                new_tokens and new_tokens[-1].speaker == -2
            ):  # if token is silence and previous one too
        ...
        if (
            token.start - last_end >= MIN_SILENCE_DURATION
        ):  # if token is not silence but important gap
            if new_tokens and new_tokens[-1].speaker == -2:
                new_tokens[-1].end = token.start
            else:
                silence_token = ASRToken(
                    start=last_end, end=token.start, speaker=-2, probability=0.95
                )
...
def ends_with_silence(
    tokens, buffer_transcription, buffer_diarization, current_time, vac_detected_silence
):
    if not tokens:
    ...
    if (
        ...
        and (
            ...
            or (current_time - last_token.end >= 3 and vac_detected_silence)
        )
    ):
        if last_token.speaker == -2:
            ...
            tokens.append(
                ASRToken(
                    start=tokens[-1].end, end=current_time, speaker=-2, probability=0.95
                )
            )
...
def handle_silences(
    ...
    tokens = blank_to_silence(
        tokens
    )  # useful for simulstreaming backend which tends to generate [BLANK_AUDIO] text
    tokens = no_token_to_silence(tokens)
    tokens, buffer_transcription, buffer_diarization = ends_with_silence(
        tokens,
        buffer_transcription,
        buffer_diarization,
        current_time,
        vac_detected_silence,
    )
    return tokens, buffer_transcription, buffer_diarization
