        ...
        async with self.lock:
            ...
            if self.end_buffer > 0:
                remaining_transcription = max(
                    0, round(current_time - self.beg_loop - self.end_buffer, 1)
                )
            ...
            if self.tokens:
                ...
                remaining_diarization = max(
                    0, round(latest_end - self.end_attributed_speaker, 1)
                )
                ...
                if len(self.pcm_buffer) >= self.bytes_per_sec:
                    ...
                    if res is not None:
                        if res.get("end", 0) > res.get("start", 0):
                            ...
                            self.silence = False
                            silence_buffer = Silence(
                                duration=time() - self.start_silence
                            )
            ...
            try:
                ...
                async with self.lock:
                    ...
                    if len(self.tokens) > 0:
                        self.end_attributed_speaker = max(
                            self.tokens[-1].end, self.end_attributed_speaker
                        )
                    if buffer_diarization:
                        self.buffer_diarization = buffer_diarization
    ...
    async def results_formatter(self):
        ...
        while True:
            try:
                ...
                # Format output
                lines, undiarized_text, buffer_transcription, buffer_diarization = (
                    format_output(
                        state,
                        self.silence,
                ...
                response = {
                    ...
                    "remaining_time_diarization": (
                        state["remaining_time_diarization"]
                        if self.args.diarization
                        else 0
                    ),
                }
