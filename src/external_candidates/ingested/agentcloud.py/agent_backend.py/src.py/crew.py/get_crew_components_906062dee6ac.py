# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\crew\get_crew_components.py
...
def construct_crew(session_id: str, socket: Any):
    ...
    if not session:
        raise CrewAIBuilderException(
            f"Session with id '{session_id}' not found. Perhaps terminated by user?"
        )
    ...
    crew_agents_dict: Dict[Set[PyObjectId], Agent] = dict(
        [(keyset(agent.id), agent) for agent in crew_agents]
    )

    # Agent > Model
    agent_models: Dict[Set[PyObjectId], Model] = construct_models(
        crew_agents_dict.items()
    )

    # Put Tasks in a dictionary with their Ids as key
    crew_tasks_dict: Dict[Set[PyObjectId], Task] = dict(
        [(keyset(task.id), task) for task in crew_tasks]
    )

    # Combine agent and task tools
    agents_tasks_tools = construct_tools(crew_agents_dict.items()) | construct_tools(
        crew_tasks_dict.items()
    )
    ...
    agents_tools_datasources_models: Dict[Set[PyObjectId], Model] = construct_models(
        agents_tools_datasources.items()
    )
    ...
    if the_crew.process == Process.Hierarchical:
        if not the_crew.managerModelId:
            raise CrewAIBuilderException(
                "Using hierarchical process but `manager_llm` not set"
            )
