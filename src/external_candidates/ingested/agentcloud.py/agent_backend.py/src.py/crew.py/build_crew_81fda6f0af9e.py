# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\crew\build_crew.py
import json
import logging
import uuid
...
class CrewAIBuilder:
    ...
    def build_tools_and_their_datasources(self):
        for key, tool in self.tools_models.items():
                ...
                case ToolType.BUILTIN_TOOL:
                    ...
                    if tool.linkedToolId:
                        ...
                        if linked_tool:
                            ...
                            logging.warning(
                                f"linked tool ID {tool.linkedToolId} not found for installed tool {tool.id}"
                            )
            ...
            tool_instance = tool_class.factory(
                tool=tool,
        ...
        for key, agent in self.agents_models.items():
            ...
            if model_obj is not None and hasattr(model_obj, "embed_query"):
                ...
                for model_key, model_candidate in self.crew_models.items():
                    if hasattr(model_candidate, "invoke") and not hasattr(
                        model_candidate, "embed_query"
                    ):
                ...
                if language_model is None:
                    print(
                        f"No language model found, creating default OpenAI model for agent {agent.name}"
                    )
    ...
    def make_current_context(self):
        if self.chat_history and len(self.chat_history) > 0:
            return (
                "=== START of Current context: ===\n"
                + "\n".join(
                    map(
                        lambda chat: f"""{chat["role"]}: {chat["content"]}""",
                        filter(
                            lambda chat: "role" in chat
                            and "content" in chat
                            and len(chat["content"]) > 0
                            and len(chat["role"]) > 0,
                            self.chat_history,
                        ),
                    )
    ...
    def run_crew(self):
        crew_output = self.crew.kickoff(
            inputs=self.input_variables if self.input_variables else None
        )
