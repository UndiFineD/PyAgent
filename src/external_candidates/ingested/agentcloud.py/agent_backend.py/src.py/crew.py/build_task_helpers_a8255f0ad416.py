...
def get_task_tools(task: Task, crew_tools: Dict[Set[PyObjectId], Tool]):
    ...
    for task_tool_id in task.toolIds:
        ...
        if len(list(task_tool_set.values())) > 0:
            task_tool = list(task_tool_set.values())[
                0
            ]  # Note: this dict/list always holds 1 item
...
def _upload_task_output(
    ...
    storage_provider.upload_file_buffer(
        buffer, task.taskOutputFileName, session_id, is_public=False
    )
    ...
    signed_url = storage_provider.get_signed_url(
        task.taskOutputFileName, session_id, is_public=False
    )
...
def _assign_structured_output_fields_to_variables(
    ...
    matching_values = extract_matching_values(
        task_output.pydantic.model_dump(), output_variables
    )
    ...
    mongo_client.update_session_variables(
        session_id=session.id, variables=session.variables
    )
...
def _assign_output_to_variable_if_single_variable(
    ...
    if task.taskOutputVariableName:
        ...
        mongo_client.update_session_variables(
            session_id=session.id, variables=session_variables
        )
...
def _update_variables_from_output(
    ...
    if task.isStructuredOutput:
        _assign_structured_output_fields_to_variables(
            task_output, session, mongo_client, output_variables
        )
    else:
        _assign_output_to_variable_if_single_variable(
            task, task_output, session, mongo_client
        )
...
def make_task_callback(
    ...
    def callback(task_output: TaskOutput):
        _update_variables_from_output(
            task, task_output, session, mongo_client, output_variables
        )
        if task.storeTaskOutput:
            _upload_task_output(
                task, session.id, mongo_client, send_to_socket_fn, task_output
            )
