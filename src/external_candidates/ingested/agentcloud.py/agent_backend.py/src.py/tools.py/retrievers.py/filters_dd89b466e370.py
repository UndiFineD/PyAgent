    ...
    def process_condition(condition: Dict, negate=False) -> Dict:
        ...
        if "match" in condition:
            ...
            if not isinstance(range_, dict):
                raise ValueError(
                    f"'range' must be a dictionary in condition: {condition}"
                )
    ...
    def construct_field_condition(condition: Dict) -> models.FieldCondition:
        ...
        if "value" in match:
            return models.FieldCondition(
                key=key, match=models.MatchValue(value=match["value"])
            )

        # Check for 'any' (match any condition)
        elif "any" in match:
            return models.FieldCondition(
                key=key, match=models.MatchAny(any=match["any"])
            )

        # Check for 'except' (match except condition)
        elif "except" in match:
            return models.FieldCondition(
                key=key, match=models.MatchExcept(**{"except": match["except"]})
            )
        ...
        if "range" in condition:
            ...
            is_datetime_range = any(
                value is not None
                and ((isinstance(value, str) and (value.endswith("Z") or "T" in value)))
                for value in [
                    range_.get("gt"),
                    range_.get("gte"),
                    range_.get("lt"),
