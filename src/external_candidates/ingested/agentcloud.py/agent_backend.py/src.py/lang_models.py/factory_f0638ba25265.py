# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\lang_models\factory.py
import json
...
def model_factory(
    agentcloud_model: models.mongo.Model,
) -> BaseLanguageModel | Embeddings:
    """
...
def _build_ollama_model(model: models.mongo.Model) -> BaseLanguageModel:
    return ChatOllama(
        **model.model_dump(exclude_none=True, exclude_unset=True).get("config")
    )
...
def _build_fastembed_model(model: models.mongo.Model) -> Embeddings:
    overwrite_model_name = _fastembed_standard_doc_name_swap(
        model.model_name, from_standard_to_doc=True
    )
    return FastEmbedEmbeddings(model_name=overwrite_model_name)


def _fastembed_standard_doc_name_swap(
    fastembed_model_name: str, from_standard_to_doc: bool
):
    from_enum = (
        models.mongo.FastEmbedModelsStandardFormat
        if from_standard_to_doc
        else models.mongo.FastEmbedModelsDocFormat
    )
    to_enum = (
        models.mongo.FastEmbedModelsDocFormat
        if from_standard_to_doc
        else models.mongo.FastEmbedModelsStandardFormat
    )
    if in_enums(enums=[to_enum], value=fastembed_model_name):
        ...
        return get_enum_value_from_str_key(
            to_enum, get_enum_key_from_value(from_enum, fastembed_model_name)
        )
