# Extracted from: C:\DEV\PyAgent\.external\agentcloud\agent-backend\src\chat\chat_assistant.py
import asyncio
import logging
import re
...
from init.env_variables import AGENT_BACKEND_SOCKET_TOKEN, MONGO_DB_NAME, SOCKET_URL
from init.mongo_session import start_mongo_session
from lang_models import model_factory as language_model_factory
from langchain_core.language_models import BaseLanguageModel
from langchain_core.tools import BaseTool
from messaging.send_message_to_socket import send
from models.mongo import Agent, App, Datasource, Model, Tool, ToolType, VectorDb
from models.sockets import Message, SocketEvents, SocketMessage
from socketio import SimpleClient
from socketio.exceptions import ConnectionError

from tools import GoogleCloudFunctionTool, RagTool
...
class ChatAssistant:
    ...
    def init_app_state(self):
        ...
        agentcloud_agent = self.mongo_conn.get_single_model_by_id(
            "agents", Agent, app_config.agentId
        )
        self.agent_name = agentcloud_agent.name

        agentcloud_tools = self.mongo_conn.get_models_by_ids(
            "tools", Tool, agentcloud_agent.toolIds
        )

        self.system_message = "\n".join(
            [agentcloud_agent.role, agentcloud_agent.goal, agentcloud_agent.backstory]
        )

        if session.variables:
            self.system_message = self.system_message.format(**session.variables)

        model = self.mongo_conn.get_single_model_by_id(
            "models", Model, agentcloud_agent.modelId
        )
        try:
    ...
    def _make_langchain_tool(self, agentcloud_tool: Tool):
        ...
        if agentcloud_tool.type == ToolType.RAG_TOOL:
            datasource = self.mongo_conn.get_single_model_by_id(
                "datasources", Datasource, agentcloud_tool.datasourceId
            )
            embedding_model = self.mongo_conn.get_single_model_by_id(
                "models", Model, datasource.modelId
            )
            ...
            if datasource.byoVectorDb:
                if not datasource.vectorDbId:
                    raise ValueError(
                        f"Vector database ID not found for datasource {datasource.id}"
                    )
                vector_db = self.mongo_conn.get_single_model_by_id(
                    "vectordbs", VectorDb, datasource.vectorDbId
                )
                if not vector_db:
                    raise ValueError(
                        f"Vector database not found for ID {datasource.vectorDbId}"
                    )
                datasource.vector_db = vector_db

            return RagTool.factory(
                tool=agentcloud_tool,
