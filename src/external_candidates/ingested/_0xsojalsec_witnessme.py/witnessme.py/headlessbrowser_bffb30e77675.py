...
class HeadlessChromium:
    ...
    async def start_browser(self, n_urls: int):
        ...
        if os.getuid() == 0:
            log.warning(
                "Running under root privileges, browser will start with --no-sandbox"
            )
        ...
        try:
            ...
            log.info(f"Using {len(worker_threads)} browser tab(s)/thread(s)")
                ...
                worker_threads, timeout=(self.timeout * 2)
            )
            if len(pending):
                log.warning(
                    f"{len(pending)} tasks did not finish, continuing to avoid stalling"
                )
                for task in pending:
                    task.cancel()
