# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\examples\setup_env.py
#!/usr/bin/env python
...
def test_env():
    ...
    if openai_key:
        masked = (
            openai_key[:7] + "..." + openai_key[-4:] if len(openai_key) > 11 else "***"
        )
    ...
    if anthropic_key:
        masked = (
            anthropic_key[:7] + "..." + anthropic_key[-4:]
            if len(anthropic_key) > 11
            else "***"
        )
