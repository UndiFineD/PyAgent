# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\examples\graph_tracking.py
"""
...
def example_basic_tracking():
    ...
    result = rlm.completion(
        query="What are the main topics discussed?", context=LONG_DOCUMENT[:2000]
    )
...
def example_recursive_tracking():
    ...
    if graph:
        ...
        for node_id, node_data in graph.nodes(data=True):
            print(
                f"  Node {node_id[:8]}... - Depth: {node_data.get('depth', 0)}, "
...
def example_with_different_output():
    ...
    result = rlm.completion(
        query="Extract key insights from the document", context=LONG_DOCUMENT
    )
...
def example_without_tracking():
    ...
    result = rlm.completion(
        query="What are the types of machine learning?", context=LONG_DOCUMENT
    )
...
def example_graph_analysis():
    ...
    result = rlm.completion(
        query="Analyze the document structure and content", context=LONG_DOCUMENT
    )
    ...
    if graph:
        ...
        for depth in sorted(depth_stats.keys()):
            ...
            print(
                f"Depth {depth}: {stats['count']} nodes, "
...
def example_with_history_tracking():
    ...
    result = rlm.completion(
        query=query, context=LONG_DOCUMENT  # Use smaller context for debugging
    )
