# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\examples\dspy_usage.py
...
def demo_e2b_sandbox():
    ...
    try:
        rlm = RLM(
            model="gpt-4o-mini", sandbox="e2b", max_iterations=10  # Explicitly use E2B
        )
        ...
        rlm = RLM(model="gpt-4o-mini", sandbox="restricted", max_iterations=10)
        result = rlm.completion(query, context_with_dates)
        print(f"Result: {result}")
