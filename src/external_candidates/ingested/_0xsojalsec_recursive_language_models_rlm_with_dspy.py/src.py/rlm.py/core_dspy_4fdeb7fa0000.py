...
class RLMDSPy:
    ...
    def completion(
        self, context: Any, query: Optional[str] = None, **kwargs: Any
    ) -> str:
        """
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(
                asyncio.run, self.acompletion(context=context, query=query, **kwargs)
            )
    ...
    async def acompletion(
        ...
        if self.logger:
            self.logger.run_start(
                depth=self._current_depth, query=query, context_size=context_size
            )
    ...
    def get_history(self) -> list:
        ...
        if not self.enable_history:
            print(
                "History tracking is not enabled. Set enable_history=True when creating RLM instance."
            )
            return []
                        ...
                        if isinstance(msg, dict):
                            ...
                            if detailed:
                                ...
                                if len(content_str) > max_length:
                                    print(
                                        f"  [{role.upper()}]: {content_str[:max_length]}..."
                                    )
                                    print(
                                        f"  ... (truncated, {len(content_str)} total chars)"
                                    )
                                else:
                    ...
                    for j, output in enumerate(outputs, 1):
                        if detailed:
                            ...
                            if len(output_str) > max_length:
                                ...
                                print(
                                    f"  ... (truncated, {len(output_str)} total chars)"
                                )
                            else:
                    ...
                    for j, choice in enumerate(response.choices, 1):
                        if hasattr(choice, "message"):
                            ...
                            if detailed:
                                ...
                                if len(content_str) > max_length:
                                    print(
                                        f"  Choice #{j}: {content_str[:max_length]}..."
                                    )
                                    print(
                                        f"  ... (truncated, {len(content_str)} total chars)"
                                    )
    ...
    def stats(self) -> Dict[str, int]:
        ...
        stats = {
            ...
            + self.rlm_module.stats.get("llm_calls", 0),
            "iterations": self.rlm_module.stats.get("iterations", 0),
            "depth": self._current_depth,
        }
