# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\src\rlm\modules.py
...
from .signatures import RLMInstructionSignature, RLMSignature
...
class RLMModule(dspy.Module):
    ...
    def set_sandbox_type(
        self, sandbox_type: Literal["e2b", "restricted", "auto"]
    ) -> None:
    ...
    def forward(
        ...
        *,
        ...
        repl_env = self._build_repl_env(
            query=query, context=context_text, recursive_llm_fn=recursive_llm_fn
        )
            ...
            if last_code is not None and sanitized_code.strip() == last_code.strip():
                if self.logger:
                    self.logger.on_duplicate_code(
                        iteration=iteration_index, depth=depth, code=sanitized_code
                    )
            ...
            try:
                ...
                if self.logger:
                    self.logger.execution_exception(
                        iteration=iteration_index, depth=depth, error=str(e)
                    )
                ...
                if answer is not None:
                    ...
                    if self.logger:
                        self.logger.on_final(
                            iteration=iteration_index, depth=depth, answer=answer
                        )
        ...
        if self.logger:
            self.logger.on_max_iterations(
                max_iterations=self.max_iterations, depth=depth
            )
        raise MaxIterationsError(
    ...
    def _build_repl_env(
        self, query: str, context: str, recursive_llm_fn: Optional[Callable]
    ) -> Dict[str, Any]:
        """
        Build REPL environment.
        ...
        env: Dict[str, Any] = {
            ...
            "_current_llm_call_id": {
                "value": None
            },  # Track current LLM call for recursion tracking
            "_current_exec_node_id": {
                "value": None
            },  # Track current execution node for recursion tracking
        }
