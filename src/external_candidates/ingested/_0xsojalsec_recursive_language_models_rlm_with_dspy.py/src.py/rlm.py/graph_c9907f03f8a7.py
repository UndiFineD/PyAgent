# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\src\rlm\graph.py
...
class GraphNode:
    ...
    def total_tokens_computed(self) -> Optional[int]:
        ...
        if (
            self.node_type == "llm_call"
            and self.tokens_prompt is not None
            and self.tokens_completion is not None
        ):
            return self.tokens_prompt + self.tokens_completion
        return None
...
class RLMGraphTracker:
    ...
    def __init__(self):
        ...
        self._current_call_ids: Dict[str, str] = (
            {}
        )  # Track current call ID for each node
        self._operation_sequence_counter: int = (
            0  # Global sequence counter for all operations
        )
    ...
    def add_repl_step(
        self, node_id: str, iteration: int, code: str, output: str
    ) -> None:
        """
    ...
    def mark_call_triggered_recursion(
        self, node_id: str, call_id: str, spawned_node_id: str
    ) -> None:
        """
    ...
    def save_html(
        ...
        for node_id, node_data in self.nodes.items():
            ...
            if node_data.node_type == "llm_call":
                ...
                status_emoji = {"success": "✅", "error": "❌", "pending": "⏳"}.get(
                    node_data.status, "⏳"
                )
            ...
            # Add node
            net.add_node(
                node_id, label=label, title=title, color=color, level=node_data.depth
            )
    ...
    def _build_node_tooltip(self, node: GraphNode) -> str:
        ...
        if node.node_type == "llm_call":
            ...
            if node.prompt:
                prompt_display = (
                    node.prompt[:300] + "..." if len(node.prompt) > 300 else node.prompt
                )
                lines.append("")
                lines.append("  Prompt:")
        ...
        # Code execution node details
            ...
            if node.code:
                code_display = (
                    node.code[:300] + "..." if len(node.code) > 300 else node.code
                )
                lines.append("")
                lines.append("  Code:")
            ...
            if node.output:
                output_display = (
                    node.output[:300] + "..." if len(node.output) > 300 else node.output
                )
                lines.append("")
                lines.append("  Output:")
        ...
        # Legacy rlm_root node details
            ...
            if node.triggering_llm_call_id:
                lines.append(
                    f"Spawned by LLM call: {node.triggering_llm_call_id[:8]}..."
                )

            if node.query:
                query_display = (
                    node.query[:200] + "..." if len(node.query) > 200 else node.query
                )
            ...
            if node.answer:
                answer_display = (
                    node.answer[:200] + "..." if len(node.answer) > 200 else node.answer
                )
            ...
            if cumulative["descendant_count"] > 0:
                ...
                lines.append(
                    f"  Total Iterations: {cumulative['cumulative_iterations']}"
                )
            ...
            if node.llm_calls_list:
                ...
                for i, call in enumerate(node.llm_calls_list[:3], 1):
                    call_info = (
                        f"  #{call.sequence_number if call.sequence_number else i}"
                    )
            ...
            if node.repl_steps:
                ...
                for step in node.repl_steps[:2]:  # Show first 2 steps
                    code_preview = (
                        step.code[:80] + "..." if len(step.code) > 80 else step.code
                    )
                    output_preview = (
                        step.output[:80] + "..."
                        if len(step.output) > 80
                        else step.output
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "success_nodes": sum(
                1 for n in self.nodes.values() if n.status == "success"
            ),
            "pending_nodes": sum(
                1 for n in self.nodes.values() if n.status == "pending"
            ),
        }
        ...
        if self.root_node_id:
            ...
            stats["root_cumulative_iterations"] = root_cumulative[
