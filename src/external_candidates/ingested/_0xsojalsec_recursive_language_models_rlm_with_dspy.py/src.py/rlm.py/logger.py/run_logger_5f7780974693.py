...
class RLMRunLogger:
    ...
    def run_end(self, depth: int, answer: Optional[str]) -> None:
        ...
        self.console.print(
            Panel(text, title=f"RLM depth {depth} completed ✅", border_style="green")
        )
        self._print_rule("")

    def iteration_start(
        self, iteration: int, depth: int, query: str, context_size: int
    ) -> None:
        if not self.enabled:
            return
        text = Text()
    ...
    def after_llm_call(
        self, iteration: int, depth: int, code: str, reasoning: Optional[str]
    ) -> None:
        if not self.enabled:
            return
        ...
        if reasoning:
            ...
            self.console.print(
                Panel(reasoning_text, border_style="yellow", title="Reasoning")
            )

    def before_execution(
        self, iteration: int, depth: int, code: str, blocked_reason: Optional[str]
    ) -> None:
        if not self.enabled:
            return
        if blocked_reason:
            ...
            self.console.print(
                self._code_panel(
                    code, title=f"Executing code (iter {iteration})", style="magenta"
                )
            )
    ...
    def after_execution(
        self, iteration: int, depth: int, output: str, error: Optional[str]
    ) -> None:
        if not self.enabled:
            return
    ...
    def on_duplicate_code(self, iteration: int, depth: int, code: str) -> None:
        ...
        panel = self._code_panel(
            code, title=f"Duplicate code detected (iter {iteration})", style="yellow"
        )
        self.console.print(panel)
    ...
    def on_final(self, iteration: int, depth: int, answer: str) -> None:
        ...
        self.console.print(
            Panel(
                text, border_style="green", title=f"FINAL at iteration {iteration} 🏁"
            )
        )
    ...
    def on_max_iterations(self, max_iterations: int, depth: int) -> None:
        if not self.enabled:
            return
        text = Text(
            f"Max iterations ({max_iterations}) reached without FINAL()", style="red"
        )
        self.console.print(
            Panel(text, border_style="red", title="Max Iterations Reached")
        )
    ...
    def on_error(self, depth: int, error: str) -> None:
        ...
        text = Text(self._truncate(error), style="red")
        self.console.print(
            Panel(text, border_style="red", title=f"Error at depth {depth}")
        )
