# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Recursive-Language-Models-RLM-with-DSpy\tests\test_graph_tracking.py
...
import pytest
from rlm import RLM
from rlm.graph import GraphNode, LLMCall, REPLStep, RLMGraphTracker
...
class TestGraphNode:
    ...
    def test_create_node(self):
        ...
        node = GraphNode(
            node_id="test-123", query="What is this?", context="Some context", depth=0
        )
    ...
    def test_context_preview(self):
        ...
        long_context = "x" * 200
        node = GraphNode(node_id="test-123", context=long_context, depth=0)
    ...
    def test_query_preview(self):
        ...
        node = GraphNode(node_id="test-123", query=long_query, depth=0)
...
class TestREPLStep:
    ...
    def test_create_repl_step(self):
        ...
        step = REPLStep(iteration=1, code="x = 5", output="None")
...
class TestLLMCall:
    ...
    def test_graph_node_stores_llm_calls(self):
        ...
        node = GraphNode(node_id="node-1", query="test query", context="test context")
...
class TestRLMGraphTracker:
    ...
    def test_create_node(self):
        ...
        node_id = tracker.create_node(
            query="Test query", context="Test context", depth=0
        )
    ...
    def test_create_child_node(self):
        ...
        parent_id = tracker.create_node(
            query="Parent query", context="Parent context", depth=0
        )

        child_id = tracker.create_node(
            query="Child query", context="Child context", depth=1, parent_id=parent_id
        )
    ...
    def test_create_code_execution_node(self):
        ...
        exec_node_id = tracker.create_code_execution_node(
            code="result = 2 + 2", output="4", iteration=1, depth=0
        )
    ...
    def test_fine_grained_tracking_chain(self):
        ...
        exec1_id = tracker.create_code_execution_node(
            code="result = 2 + 2", output="4", iteration=1, depth=0, parent_id=llm1_id
        )
    ...
    def test_update_node(self):
        ...
        node_id = tracker.create_node(query="Test", context="Context", depth=0)

        tracker.update_node(
            node_id=node_id, answer="Test answer", iterations=5, llm_calls=3
        )
    ...
    def test_update_node_with_zero_values(self):
        ...
        tracker.update_node(
            node_id=node_id, answer="Quick answer", iterations=0, llm_calls=0
        )
    ...
    def test_update_node_with_error(self):
        ...
        tracker.update_node(node_id=node_id, error="Something went wrong")
    ...
    def test_add_repl_step(self):
        ...
        tracker.add_repl_step(node_id=node_id, iteration=1, code="x = 5", output="None")

        tracker.add_repl_step(node_id=node_id, iteration=2, code="FINAL(x)", output="5")
    ...
    def test_save_html(self):
        ...
        tracker.create_node(query="Test query", context="Test context", depth=0)
    ...
    def test_get_stats(self):
        ...
        child_id = tracker.create_node(
            query="Child", context="", depth=1, parent_id=parent_id
        )
    ...
    def test_get_cumulative_stats(self):
        ...
        child1_id = tracker.create_node(
            query="Child1", context="", depth=1, parent_id=root_id
        )
        tracker.update_node(
            child1_id, answer="Child1 answer", iterations=3, llm_calls=2
        )

        child2_id = tracker.create_node(
            query="Child2", context="", depth=1, parent_id=root_id
        )
        tracker.update_node(
            child2_id, answer="Child2 answer", iterations=2, llm_calls=1
        )

        grandchild_id = tracker.create_node(
            query="Grandchild", context="", depth=2, parent_id=child2_id
        )
        tracker.update_node(
            grandchild_id, answer="Grandchild answer", iterations=1, llm_calls=1
        )
...
class TestRLMWithGraphTracking:
    ...
    def test_enable_graph_tracking(self):
        ...
        rlm = RLM(model="gpt-4o-mini", enable_graph_tracking=True)
