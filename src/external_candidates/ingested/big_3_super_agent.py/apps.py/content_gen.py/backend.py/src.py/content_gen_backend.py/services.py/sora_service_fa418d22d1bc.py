# Extracted from: C:\DEV\PyAgent\.external\big-3-super-agent\apps\content-gen\backend\src\content_gen_backend\services\sora_service.py
...
from ..models.video_response import ErrorDetail, VideoJob
...
class SoraService:
    ...
    async def create_video(
        ...
        try:
            logger.info(
                f"Creating video with prompt: '{prompt[:50]}...', model: {model}, seconds: {seconds}, size: {size}"
            )
            ...
            logger.error(
                f"OpenAI API error during video creation: {str(e)}", exc_info=True
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error during video creation: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_video_status(self, video_id: str) -> VideoJob:
        ...
        try:
            ...
            logger.debug(
                f"Video {video_id} status: {video.status}, progress: {getattr(video, 'progress', 'N/A')}"
            )
            ...
            logger.error(
                f"OpenAI API error fetching video status for {video_id}: {str(e)}",
                exc_info=True,
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error fetching video status for {video_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def poll_until_complete(
        ...
        while True:
            ...
            if elapsed > timeout:
                logger.warning(
                    f"Polling timeout reached for video {video_id} after {elapsed:.1f}s"
                )
    ...
    async def download_video_content(
        ...
        try:
            ...
            content = await self.client.videos.download_content(
                video_id, variant=variant
            )
            ...
            logger.info(
                f"Downloaded {len(data)} bytes of {variant} for video {video_id}"
            )
            return data

        except OpenAIError as e:
            logger.error(
                f"OpenAI API error downloading {variant} for {video_id}: {str(e)}",
                exc_info=True,
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error downloading {variant} for {video_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def delete_video(self, video_id: str) -> dict:
        ...
        try:
            ...
            logger.error(
                f"OpenAI API error deleting video {video_id}: {str(e)}", exc_info=True
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error deleting video {video_id}: {str(e)}", exc_info=True
            )
            raise
    ...
    async def remix_video(self, video_id: str, prompt: str) -> VideoJob:
        ...
        try:
            logger.info(
                f"Creating remix of video {video_id} with prompt: '{prompt[:50]}...'"
            )
            ...
            logger.error(
                f"OpenAI API error remixing video {video_id}: {str(e)}", exc_info=True
            )
            raise
        except Exception as e:
            logger.error(
                f"Unexpected error remixing video {video_id}: {str(e)}", exc_info=True
            )
            raise
