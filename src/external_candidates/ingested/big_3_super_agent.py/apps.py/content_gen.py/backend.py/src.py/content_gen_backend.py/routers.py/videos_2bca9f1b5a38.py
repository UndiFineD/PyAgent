...
async def create_video(
    ...
    try:
        ...
        logger.info(
            f"Received create video request: model={model}, seconds={seconds}, size={size}"
        )
        ...
        if input_reference:
            ...
            # Validate file type
            if (
                not input_reference.content_type
                or not input_reference.content_type.startswith("image/")
            ):
                raise HTTPException(
                    status_code=400,
            ...
            logger.info(
                f"Reference image received: {input_reference.filename}, size: {len(content)} bytes"
            )
...
async def get_video_status(video_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch video status: {str(e)}"
        )
...
async def download_video_content(
    ...
    try:
        ...
        if "not found" in str(e).lower() or "404" in str(e):
            raise HTTPException(status_code=404, detail=f"Video {video_id} not found")
        raise HTTPException(
            status_code=500, detail=f"Failed to download video content: {str(e)}"
        )
...
async def list_videos(
    ...
    try:
        ...
        videos, has_more = await sora_service.list_videos(
            limit=limit, after=after, order=order
        )
