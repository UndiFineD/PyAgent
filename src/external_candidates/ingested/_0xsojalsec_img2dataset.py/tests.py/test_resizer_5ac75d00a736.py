# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-img2dataset\tests\test_resizer.py
import glob
import io
import os
...
def test_resizer(
    ...
    for image_path in image_paths:
        ...
        image_resized_str, width, height, original_width, original_height, err = (
            resizer(image_original_stream)
        )
        assert err is None
        image_original_stream = io.BytesIO(img)
        image_original = cv2.imdecode(
            np.frombuffer(image_original_stream.read(), np.uint8), cv2.IMREAD_UNCHANGED
        )
        image_resized = cv2.imdecode(
            np.frombuffer(image_resized_str, np.uint8), cv2.IMREAD_UNCHANGED
        )
