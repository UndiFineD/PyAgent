# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-img2dataset\tests\test_writer.py
import glob
import os
import tarfile
...
@pytest.mark.parametrize(
    "writer_type", ["files", "webdataset", "parquet", "dummy", "tfrecord"]
)
def test_writer(writer_type, tmp_path):
    ...
    if writer_type == "files":
        ...
        assert (
            len(tarfile.open(output_folder + "/00000.tar").getnames())
            == len(image_paths) * 3
        )
    elif writer_type == "parquet":
