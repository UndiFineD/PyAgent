# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-img2dataset\tests\fixtures.py
import glob
import gzip
import os
import random
    ...
    for i in range(count):
        ...
        test_list.append(
            (
                ...
                image_paths[item].replace(
                    test_folder, f"http://localhost:{port}/allowed"
                ),
            )
        )
    ...
    for i in range(disallowed):
        ...
        test_list.append(
            (
                f"caption {i}" if i != 0 else "",
                image_paths[item].replace(
                    test_folder, f"http://localhost:{port}/disallowed"
                ),
            )
        )
...
def check_one_image_size(
    img, img_unresized, image_size, resize_mode, resize_only_if_bigger
):
    width = img.shape[1]
    height = img.shape[0]
    ...
    if resize_mode == "border":
        if width != image_size or height != image_size:
            raise Exception(
                f"Image size is not 256x256 in border mode found={width}x{height}"
            )
    elif resize_mode == "keep_ratio":
        ratio = float(image_size) / min(width_unresized, height_unresized)
        new_size = tuple(
            [round(x * ratio) for x in [width_unresized, height_unresized]]
        )
