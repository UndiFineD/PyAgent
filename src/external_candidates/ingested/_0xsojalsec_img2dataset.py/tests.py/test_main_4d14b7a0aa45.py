# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-img2dataset\tests\test_main.py
import glob
import os
import shutil
import tarfile
...
def test_download_resize(
    image_size, resize_mode, resize_only_if_bigger, skip_reencode, tmp_path
):
    ...
    if output_format == "files":
        ...
        assert (
            len(
                [
                    x
                    for x in tarfile.open(image_folder_name + "/00000.tar").getnames()
                    if x.endswith(".jpg")
                ]
            )
            == expected_file_count
        ...
        assert (
            len(pd.read_parquet(image_folder_name + "/00000.parquet").index)
            == expected_file_count
        )
...
def test_download_multiple_input_files(input_format, output_format, tmp_path):
    ...
    if not os.path.exists(subfolder):
        os.mkdir(subfolder)
    url_list_names = [
        os.path.join(subfolder, prefix + "url_list1"),
        os.path.join(subfolder, prefix + "url_list2"),
    ]
    image_folder_name = os.path.join(test_folder, prefix + "images")
    ...
    if output_format == "files":
        ...
        assert (
            len(
                [
                    x
                    for x in tarfile.open(image_folder_name + "/00001.tar").getnames()
                    if x.endswith(".jpg")
                ]
            )
            == expected_file_count
...
def test_captions_saving(save_caption, output_format, tmp_path):
    ...
    if output_format == "files":
        ...
        with tarfile.open(image_folder_name + "/00000.tar") as f:
            assert (
                len([x for x in f.getnames() if x.endswith(".jpg")])
                == expected_file_count
            )
...
def test_webdataset(tmp_path):
    ...
    assert (
        len(tarfile.open(image_folder_name + "/00000.tar").getnames())
        == len(test_list) * 2
    )
...
def test_blur_and_resize(resize_mode, resize_only_if_bigger, tmp_path):
    ...
    if resize_only_if_bigger:
        ...
        desired_output_img_path = os.path.join(
            current_folder, "blur_test_files", f"resize_{resize_mode}.jpg"
        )
