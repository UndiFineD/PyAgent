...
def _log_blob(name: str, blob: str | bytes, extra: Dict[str, Any]):
    ...
    if not STRICT_NO_TRUNCATION and len(text) > MAX_LOG_BYTES:
        text = (
            text[:MAX_LOG_BYTES] + f"... [+{len(text) - MAX_LOG_BYTES} bytes truncated]"
        )
...
def _ps_search(base: str, scope: str, filt: str, size: int) -> str:
    ...
    ps_scope = scope_map.get(
        scope.lower(), "[System.DirectoryServices.SearchScope]::Subtree"
    )
    return (
...
def _run_remote_ps(sid: str, ps: str, timeout: float, op_id: str, log) -> str:
    ...
    try:
        if transport in ("http", "https"):
            out = (
                http_exec.run_command_http(sid, ps, op_id=op_id, timeout=timeout) or ""
            )
        else:
            out = (
...
async def ldap_ws(ws: WebSocket):
    ...
    async def _handle_open(req: Dict[str, Any], req_id: Optional[str]):
        ...
        shaped = {
            ...
            or (
                []
                if not info.get("defaultNamingContext")
                else [info.get("defaultNamingContext")]
            ),
        ...
        await _ws_send(
            ws, _ok({"info": shaped}, "ldap.opened", req_id=req_id), _log_wsid=wsid
        )
    ...
    async def _handle_open_current(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if not sid:
            await _ws_send(
                ws, _err("ldap.opened", "missing sid", req_id=req_id), _log_wsid=wsid
            )
            return
        out = _run_remote_ps(
            sid,
        ...
        if not info or not (info.get("host") or info.get("defaultNamingContext")):
            await _ws_send(
                ws, _err("ldap.opened", "discovery returned no DC", req_id=req_id), log
            )
            return
    ...
    async def _handle_rootdse(req: Dict[str, Any], req_id: Optional[str]):
        if use_agent:
            ...
            out = _run_remote_ps(
                sid, ps, timeout=float(req.get("timeout", 10.0)), op_id="ldap", log=log
            )
            data = {}
            with suppress(Exception):
        ...
        if conn is None:
            await _ws_send(
                ws, _err("ldap.rootdse", "not connected", req_id=req_id), log
            )
            return
        ...
        await _ws_send(
            ws, _ok({"rootdse": data}, "ldap.rootdse", req_id=req_id), _log_wsid=wsid
        )
    ...
    async def _handle_read(req: Dict[str, Any], req_id: Optional[str]):
            ...
            if (out or "").strip():
                with suppress(Exception):
                    ...
                    if isinstance(data, dict) and data.get("__err__"):
                        ...
                        return
                    _log_json(
                        "ldap.read.agent.attrs", data, extra={"wsid": wsid, "dn": dn}
                    )
                    await _ws_send(
                        ws,
                        _ok(
                            {"dn": dn, "attrs": data or {}}, "ldap.read", req_id=req_id
                        ),
                        _log_wsid=wsid,
                    )
                    return
        ...
        def _read() -> Tuple[Optional[str], Dict[str, Any]]:
            ...
            e = conn.entries[0] if conn.entries else None
            return (e.entry_dn if e else None), (
                e.entry_attributes_as_dict if e else {}
            )
    ...
    async def _handle_children(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if conn is None:
            await _ws_send(
                ws, _err("ldap.children", "not connected", req_id=req_id), log
            )
            return
    ...
    async def _handle_search(req: Dict[str, Any], req_id: Optional[str]):
        ...
        if use_agent:
            ...
            _log_json(
                "ldap.search.agent.rows", rows, extra={"wsid": wsid, "base": base}
            )
            await _ws_send(
                ws,
                _ok(
    ...
    async def _handle_quick(req: Dict[str, Any], req_id: Optional[str]):
        ...
        base = str(req.get("base") or "") or (
            agent_info.get("defaultNamingContext") if use_agent else ""
        )
    ...
    try:
        while True:
            ...
            if act in ("ping", "__ping__", "health"):
                await _ws_send(
                    ws, _ok({"ts": time.time()}, "pong", req_id=req_id), _log_wsid=wsid
                )
                continue
            ...
            if act == "ldap.open":
                ...
                await _close()
                await _ws_send(
                    ws, _ok({}, "ldap.closed", req_id=req_id), _log_wsid=wsid
                )
