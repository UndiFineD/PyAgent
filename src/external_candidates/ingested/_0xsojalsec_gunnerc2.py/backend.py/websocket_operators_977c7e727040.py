# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\websocket_operators.py
...
async def _cmd_list(ws, req, claims):
    if not claims:
        return await _ws_send(
            ws, {"type": "error", "req_id": req.get("req_id"), "error": "unauthorized"}
        )
    await _ws_send(
        ws,
        {
...
async def _cmd_add(ws, req, claims):
    ...
    if (claims.get("role") or "operator") != "admin":
        return await _ws_send(
            ws, {"type": "error", "req_id": req.get("req_id"), "error": "forbidden"}
        )
    ...
    try:
        ...
        if oid in ("ALREADY EXISTS", "USERNAME REGEX FAIL", "PASSWORD REGEX FAIL"):
            return await _ws_send(
                ws, {"type": "error", "req_id": req.get("req_id"), "error": oid}
            )
        await _ws_send(
            ws, {"type": "error", "req_id": req.get("req_id"), "error": "add failed"}
        )
    except Exception as e:
        await _ws_send(
            ws, {"type": "error", "req_id": req.get("req_id"), "error": str(e)}
        )
...
async def _cmd_delete(ws, req, claims):
    ...
    if ok:
        await _ws_send(
            ws, {"type": "deleted", "req_id": req.get("req_id"), "id": ident}
        )
        await _ws_send(ws, {"type": "snapshot", "operators": _serialize_ops()})
    else:
        await _ws_send(
    ...
    if res is True:
        await _ws_send(
            ws,
            {
                ...
                "id": ident,
                "username": nu,
                "role": (nr if nr is not None else None),
            },
        )
        await _ws_send(ws, {"type": "snapshot", "operators": _serialize_ops()})
    else:
...
async def _cmd_login(ws, req):
    ...
    token = create_access_token(
        {"sub": norm["id"], "username": norm["username"], "role": norm["role"]}
    )
    await _ws_send(
        ws,
        {"type": "login_ok", "req_id": req.get("req_id"), "token": token, "me": norm},
    )
...
async def operators_ws(ws: WebSocket):
    ...
    async def reader():
        ...
        actions = {
            ...
            "ping": lambda w, r: _ws_send(
                w, {"type": "pong", "req_id": r.get("req_id")}
            ),
        }
        while True:
