# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\websocket_console.py
...
async def session_ws(ws: WebSocket, sid: str):
    ...
    await ws.send_text(
        f"** Connected to {meta.get('hostname','?')} as {meta.get('user','?')} ({meta.get('os','?')}/{meta.get('arch','?')}) **"
    )
