# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\websocket_listeners.py
...
from core.listeners.base import _reg_lock
from core.listeners.base import listeners as CORE_REG

from . import config
from .listeners import ALLOWED_TYPES, _serialize_listener, _stop_instance_async
    ...
    try:
        while True:
            ...
            if act in ("listeners.list", "list"):
                ...
                host = (
                    req.get("bind_ip") or req.get("ip") or req.get("host") or "0.0.0.0"
                )
                port = int(req.get("port") or req.get("bind_port") or 0)
                profiles = (
                    req.get("profile") or req.get("profiles") or req.get("base_path")
                )
                ...
                try:
                    asyncio.get_event_loop().run_in_executor(
                        None, _stop_instance_async, inst
                    )
                except Exception:
