# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\files.py
# backend/files.py
import base64
import json
...
def list_dir(sid: str, path: str = Query(default="."), op_id: str = "console"):
    ...
    if "win" in os_type:
        ...
        try:
                ...
                items.append(
                    {
                        "name": d.get("Name", ""),
                        "is_dir": is_dir,
                        "size": (None if is_dir else int(d.get("Length") or 0)),
                    }
                )
            return items
        except Exception:
            ...
            return [
                {"name": l, "is_dir": l.endswith("\\"), "size": None} for l in lines
            ]
    else:
...
def download_file(sid: str, path: str, op_id: str = "console"):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to download/parse remote file"
        )
...
async def upload_file(
    sid: str, path: str, file: UploadFile = File(...), op_id: str = "console"
):
