# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\websocket_files.py
...
async def files_ws(ws: WebSocket):
    ...
    active_upload_remote_archive: Optional[str] = (
        None  # e.g. /tmp/core.zip or /tmp/core.tar.gz
    )
    ...
    async def _do_list(req: Dict[str, Any]):
        ...
        if not sess:
            log.warning(
                "fs.list.session_missing",
                extra={"sid": sid, "path": path, "req_id": req_id},
            )
            return await _ws_send(
                ws,
                {"type": "error", "req_id": req_id, "error": "Session not found"},
                log,
            )
        ...
        if os_type == "windows":
            ...
            log.info(
                "fs.list.ok", extra={"sid": sid, "path": real_path, "count": len(rows)}
            )
            return await _ws_send(
                ws,
                {
    ...
    async def _do_delete(req: Dict[str, Any]):
        ...
        try:
            ...
            if txt.startswith("OK"):
                await _ws_send(
                    ws,
                    {"type": "deleted", "req_id": req_id, "path": target, "ok": True},
                    log,
                )
                ...
                await _ws_send(
                    ws,
                    {
                        ...
                        "path": target,
                        "ok": False,
                        "error": "Refusing to delete directory without folder=True",
                    },
                    log,
                )
            else:
                await _ws_send(
                    ws,
                    {
                        ...
                        "path": target,
                        "ok": False,
                        "error": (txt[:400] or "delete_failed"),
                    },
                    log,
                )
                log.warning(
                    "fs.delete.err",
                    extra={"sid": sid, "path": target, "detail": txt[:400]},
                )
    ...
    async def _do_new_folder(req: Dict[str, Any]):
        ...
        if not full:
            if not (sid and parent and name):
                return await _ws_send(
                    ws,
                    {
                        ...
                        "kind": "folder",
                        "ok": False,
                        "error": "Missing sid/dir/name or path",
                    },
                    log,
                )
            # join safe for OS
            sess = session_manager.sessions.get(sid)
            os_type = (
                (getattr(sess, "metadata", {}) or {}).get("os", "").lower()
                if sess
                else ""
            )
        ...
        try:
            ...
            txt = (out or "").strip()
            ok = ("OK" in txt) or (
                "EXISTS" in txt and bool(req.get("ok_if_exists", True))
            )
    ...
    async def _do_download(req: Dict[str, Any]):
        ...
        if not sess:
            logger.warning(
                "fs.download.session_missing sid=%s path=%s req_id=%s",
                sid,
                path,
                req_id,
            )
        ...
        if active_download_task and not active_download_task.done():
            ...
            if (
                req_path == (active_download_path or "")
                and req_folder == active_download_folder
            ):
                # Duplicate click for the same file while streaming; just acknowledge and return
                logger.info(
                    "fs.download.duplicate sid=%s path=%s (already streaming)",
                    sid,
                    req_path,
                )
                await _ws_send(
                    ws,
                    {
                        "type": "fs.download.already",
                        "path": req_path,
                        "tid": active_download_tid,
                    },
                    log,
                )
                return
            logger.warning(
                "fs.download.reject_busy sid=%s req_id=%s",
                req.get("sid"),
                req.get("req_id"),
            )
            await _ws_send(
                ws,
                {
                    "type": "error",
                    "req_id": req.get("req_id"),
                    "error": "Download already in progress on this socket",
                },
                log,
            )
            return
        ...
        logger.info(
            "fs.download.begin sid=%s peer=%s path=%s folder=%s tmp_dir=%s dest=%s req_id=%s",
            sid,
            ws_peer,
            path,
            folder,
        ...
        tid = tm.start_download(
            ...
            dest,
            folder=folder,
            opts=TransferOpts(quiet=True, defer_extract=True),
        )
        active_download_path = path
        active_download_folder = folder
        active_download_tid = tid
        logger.info(
            "DL start tid=%s sid=%s remote=%s -> %s (folder=%s)",
            tid,
            sid,
            path,
            dest,
        ...
        await _ws_send(
            ws,
            {"type": "fs.download.begin", "req_id": req_id, "tid": tid, "name": fname},
            log,
        )
        ...
        async def _pump():
            ...
            def _hex_head_tail(
                p: str, head_n: int = 16, tail_n: int = 64
            ) -> tuple[str, str]:
                try:
            ...
            try:
                while True:
                    ...
                    if st.status in ("done", "error", "cancelled", "paused"):
                        ...
                        if st.status == "done" and final_src:
                            logger.debug(
                                "DL[%s] final stream start sid=%s file=%s",
                                tid,
                                sid,
                                final_src,
                            )
                            try:
                                with open(final_src, "rb") as f:
                                    ...
                                    while chunk:
                                        ...
                                        if now - last_log_t >= 1.0:
                                            ...
                                            logger.debug(
                                                "fs.download.stream tid=%s sent=%d bps=%d sid=%s src=final",
                                                tid,
                                                bytes_sent,
                                                bps,
                                                sid,
                                ...
                                logger.exception(
                                    "DL[%s] final stream failed sid=%s err=%r",
                                    tid,
                                    sid,
                                    de,
                                )
                        ...
                        size_on_disk = (
                            os.path.getsize(final_src)
                            if final_src and os.path.exists(final_src)
                            else -1
                        )
                        head_hex, tail_hex = (
                            _hex_head_tail(final_src) if final_src else ("", "")
                        )
                        ...
                        try:
                            if final_src and size_on_disk >= 4:
                                ...
                                if head4.startswith(b"PK\x03\x04"):
                                    ...
                                    logger.info(
                                        "zip.eocd.probe tid=%s sid=%s ok=%s meta=%s err=%s",
                                        tid,
                                        sid,
                                        ok,
                                        eocd_meta,
                            ...
                            logger.exception(
                                "zip.eocd.probe_exception tid=%s sid=%s err=%r",
                                tid,
                                sid,
                                _pe,
                            )
                            ...
                            if final_src and size_on_disk >= 4:
                                with open(final_src, "rb") as _ff:
                                    if _ff.read(4).startswith(b"PK\x03\x04"):
                                        ok, eocd_meta, eocd_err = _probe_zip_eocd(
                                            final_src
                                        )
                                        eocd_ok = ok
                                        logger.info(
                ...
                try:
                    ...
                    logger.debug(
                        "fs.download.tmp_cleanup tid=%s tmp_dir=%s sid=%s",
                        tid,
                        tmp_dir,
                        sid,
                    )
                    ...
                    logger.exception(
                        "fs.download.tmp_cleanup_error tid=%s sid=%s err=%r",
                        tid,
                        sid,
                        e,
                    )
    ...
    async def _do_upload_begin(req: Dict[str, Any]):
        ...
        if active_upload_tmp is not None:
            ...
            return await _ws_send(
                ws,
                {
                    "type": "error",
                    "req_id": req.get("req_id"),
                    "error": "Upload already in progress on this socket",
                },
                log,
            )
        ...
        if is_folder:
            ...
            if not remote_path and remote_dir:
                ...
                log.debug(
                    "fs.upload.begin.derive_remote_path",
                    extra={
                        "sid": sid,
                        "os": os_type,
                        "derived_remote_path": remote_path,
                        "from_remote_dir": remote_dir,
                    },
                )
            ...
            if not remote_dir and remote_path:
                ...
                log.debug(
                    "fs.upload.begin.derive_remote_dir",
                    extra={
                        "sid": sid,
                        "derived_remote_dir": remote_dir,
                        "from_remote_path": remote_path,
                    },
                )

        # For plain file upload, remote_path is the target file.
        target_descriptor = (
            remote_path if not is_folder else f"{remote_path} (extract→ {remote_dir})"
        )
        ...
        if (
            not sid
            or size < 0
            or (not remote_path and not is_folder)
            or (is_folder and (not remote_path and not remote_dir))
        ):
            log.warning(
                "fs.upload.bad_request",
                extra={
                    "sid": sid,
                    "folder": is_folder,
        ...
        if (
            not sid
            or size < 0
            or (not is_folder and not remote_path)
            or (is_folder and (not remote_path or not remote_dir))
        ):
            log.warning(
                "fs.upload.bad_request",
                extra={
            ...
            return await _ws_send(
                ws,
                {
                    "type": "error",
                    "req_id": req.get("req_id"),
                    "error": "Missing sid/remote_path/size (and remote_dir for folder)",
                },
                log,
            )
        ...
        if not ok:
            log.warning(
                "fs.upload.preflight_denied",
                extra={"sid": sid, "target": test_target, "detail": detail},
            )
    ...
    async def _do_upload_finish():
        ...
        tid = tm.start_upload(
            active_upload_sid,
            active_upload_tmp,
            (active_upload_remote_archive or ""),
            folder=False,
            opts=TransferOpts(quiet=True),
        )
        ...
        try:
            meta = {
                ...
                "size": (
                    os.path.getsize(active_upload_tmp)
                    if active_upload_tmp and os.path.exists(active_upload_tmp)
                    else 0
                ),
                "magic": file_magic(active_upload_tmp) if active_upload_tmp else "",
                "sha256": (
                    sha256_path(active_upload_tmp)
                    if active_upload_tmp and os.path.exists(active_upload_tmp)
                    else ""
                ),
            }
        ...
        log.info(
            "fs.upload.tm_begin",
            extra={
                "sid": active_upload_sid,
                "remote_path": active_upload_remote_archive,
                "folder": active_upload_is_folder,
                "remote_dir": active_upload_remote_dir,
                "tmp": active_upload_tmp,
                "tid": tid,
            },
        )
        ...
        terminal = {
            ...
            "cancelled",
            "paused",
        }  # include paused so the waiter always ends
        while True:
            ...
            if (st.status or "").lower() in terminal:
                log.info(
                    "fs.upload.tm_end",
                    extra={"tid": tid, "status": st.status, "error": st.error},
                )
                ...
                try:
                    if (
                        final_status == "done"
                        and active_upload_is_folder
                        and active_upload_remote_dir
                        and active_upload_remote_archive
                    ):
                        ...
                        transport = (
                            str(getattr(sess, "transport", "")).lower() if sess else ""
                        )
                        log.info(
                            "fs.upload.extract.begin",
                            extra={
                                "sid": active_upload_sid,
                                "os": os_type,
                                "transport": transport,
                                "archive": active_upload_remote_archive,
                                "dest_dir": active_upload_remote_dir,
                            },
                        )
                        ...
                        if os_type == "windows":
                            ...
                            log.debug(
                                "fs.upload.extract.ps", extra={"cmd_preview": _prev}
                            )
                            out = _run_remote(active_upload_sid, ps, transport, log=log)
                            txt = ((out or "").replace("\r", "")).strip()
                            log.debug(
                                "fs.upload.extract.ps.out",
                                extra={"len": len(txt), "preview": txt[:400]},
                            )
                            if "ERR:" in txt:
                                final_status, final_error = (
                                    "error",
                                    f"expand_archive_failed: {txt!r}",
                                )
                            else:
                                # Avoid reserved LogRecord fields like 'msg'/'message'
                                log.info(
                                    "fs.upload.extract.win.ok",
                                    extra={"detail": txt[:300]},
                                )
                        else:
                            sh = "bash -lc " + _shq(
                            ...
                            log.debug(
                                "fs.upload.extract.sh", extra={"cmd_preview": _prev}
                            )
                            out = _run_remote(active_upload_sid, sh, transport, log=log)
                            txt = ((out or "").replace("\r", "")).strip()
                            log.debug(
                                "fs.upload.extract.sh.out",
                                extra={"len": len(txt), "preview": txt[:400]},
                            )
                            if "ERR:" in txt:
                                final_status, final_error = (
                                    "error",
                                    f"tar_extract_failed: {txt!r}",
                                )
                            else:
                                log.info(
                                    "fs.upload.extract.posix.ok",
                                    extra={"detail": txt[:300]},
                                )
                ...
                log.info(
                    "fs.upload.result.sent",
                    extra={"tid": tid, "status": final_status, "error": final_error},
                )
                break
    ...
    async def _do_drives(req: Dict[str, Any]):
        ...
        transport = str(getattr(sess, "transport", "")).lower()
        log.debug(
            "fs.drives.begin", extra={"sid": sid, "os": os_type, "transport": transport}
        )
        ...
        if os_type == "windows":
            ...
            return await _ws_send(
                ws, {"type": "fs.drives", "drives": rows, "req_id": req_id}, log
            )
    ...
    async def _do_quickpaths(req: Dict[str, Any]):
        ...
        log.debug(
            "fs.quickpaths.begin",
            extra={"sid": sid, "os": os_type, "transport": transport},
        )
        ...
        if os_type == "windows":
            ...
            return await _ws_send(
                ws, {"type": "fs.quickpaths", "paths": obj, "req_id": req_id}, log
            )
            ...
            if msg["type"] == "websocket.receive":
                if "text" in msg:
                    ...
                    try:
                        ...
                        await _ws_send(
                            ws, {"type": "error", "error": "Invalid JSON"}, log
                        )
                        continue
                    ...
                    if act in ("fs.list", "list"):
                        ...
                        await _ws_send(
                            ws, {"type": "pong", "req_id": req.get("req_id")}, log
                        )
                    else:
                        log.warning("ws.unknown_action", extra={"action": act})
                        await _ws_send(
                            ws,
                            {"type": "error", "error": f"Unknown action '{act}'"},
                            log,
                        )
                    ...
                    try:
                        ...
                        await _ws_send(
                            ws,
                            {"type": "error", "error": f"Upload write failed: {e}"},
                            log,
                        )
                        try:
                            if active_upload_tmp:
                    ...
                    if (
                        ...
                        or (recv_written - progress_last_reported)
                        >= (2 * 1024 * 1024)  # every +2 MiB
                    ):
                        log.debug(
                            "fs.upload.progress",
                            extra={
                                "written": recv_written,
                                "expect": active_upload_expect,
                            },
                        )
                        await _ws_send(
                            ws,
                            {
                                "type": "fs.upload.progress",
                                "written": recv_written,
                                "total": active_upload_expect,
                            },
                            log,
                        )
                    ...
                    if active_upload_expect and recv_written >= active_upload_expect:
                        ...
                        try:
                            ...
                            log.exception(
                                "fs.upload.finalize_error", extra={"err": repr(e)}
                            )
                            await _ws_send(
                                ws,
                                {
                                    "type": "error",
                                    "error": f"Upload finalize failed: {e}",
                                },
                                log,
                            )
                            try:
