# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\backend\listeners.py
...
from core.listeners.base import create_listener, load_listeners

from .schemas import ListenerOut, NewListenerRequest
...
def _serialize_listener(inst) -> ListenerOut:
    return {
        ...
        "status": (
            "RUNNING"
            if getattr(inst, "thread", None)
            and getattr(inst.thread, "is_alive", lambda: False)()
            else "STARTED"
        ),
...
def start_listener(req: NewListenerRequest):
    ...
    if friendly and any(getattr(x, "name", "") == friendly for x in _RUNNING.values()):
        raise HTTPException(
            status_code=400, detail=f"Listener name '{friendly}' already in use"
        )
