# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\operators_tab.py
...
class IDElideDelegate(QStyledItemDelegate):
    ...
    def paint(self, p: QPainter, opt, idx):
        ...
        elided = fm.elidedText(
            text, Qt.ElideMiddle, self.max_px if self.max_px > 0 else r.width()
        )
        # softer ink for IDs
...
class RoleChip(QStyledItemDelegate):
    ...
    def paint(self, p, opt, idx):
        ...
        bg = _qcolor(
            (
                theme_color("chip_operator_bg")
                if text == "operator"
                else theme_color("chip_admin_bg")
            ),
            "#34425a" if text == "operator" else "#5a3434",
        )
        fg = _qcolor(
            (
                theme_color("chip_operator_fg")
                if text == "operator"
                else theme_color("chip_admin_fg")
            ),
...
class AvatarNameDelegate(QStyledItemDelegate):
    ...
    def paint(self, p: QPainter, opt, idx):
        ...
        f.setBold(True)
        f.setPointSizeF(max(9.0, p.font().pointSizeF() * 0.95))
        p.setFont(f)
...
class RoleFilter(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, r, parent):
        ...
        if self._needle and self._needle not in blob:
            return False
        if self._role in ("operator", "admin") and role != self._role:
            return False
        return True
...
class AddOperatorDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.uEdit.setPlaceholderText("Username")
        self.pEdit = QLineEdit()
        self.pEdit.setPlaceholderText("Password")
        self.pEdit.setEchoMode(QLineEdit.Password)
        self.roleNew = QComboBox()
        self.roleNew.addItems(["operator", "admin"])
...
class OperatorsTab(QWidget):
    def __init__(self, api):
        ...
        # ===== Header card =====
        header = QFrame(self)
        header.setObjectName("OpsHeader")
        h1 = QHBoxLayout(header)
        h1.setContentsMargins(14, 12, 14, 12)
        h1.setSpacing(10)
        ...
        self.btnAdd.setText("Add Operator")
        self.btnAdd.setObjectName("Primary")
        self.btnAdd.setCursor(Qt.PointingHandCursor)
        self.btnRemove = QToolButton()
        self.btnRemove.setText("Remove")
        self.btnRemove.setEnabled(False)
        self.btnRemove.setProperty("preserveSelection", True)
        ...
        util = QFrame(self)
        util.setObjectName("OpsUtil")
        u = QHBoxLayout(util)
        u.setContentsMargins(12, 10, 12, 10)
        u.setSpacing(10)

        self.search = QLineEdit()
        self.search.setPlaceholderText("Search username, role, or ID…")
        ...
        self.btnAll = QToolButton()
        self.btnAll.setText("all")
        self.btnAll.setCheckable(True)
        self.btnAll.setChecked(True)
        self.btnAll.setProperty("seg", True)

        self.btnOp = QToolButton()
        self.btnOp.setText("operator")
        self.btnOp.setCheckable(True)
        self.btnOp.setProperty("seg", True)
        self.btnAd = QToolButton()
        self.btnAd.setText("admin")
        self.btnAd.setCheckable(True)
        self.btnAd.setProperty("seg", True)
        ...
        # ===== Table =====
        self.model = OpsModel()
        self.proxy = RoleFilter()
        self.proxy.setSourceModel(self.model)

        self.table = QTableView(self)
        self.table.setObjectName("OpsTable")
    ...
    def _open_add_dialog(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            if not u or not p:
                QMessageBox.warning(
                    self, "Add Operator", "Username and password are required."
                )
                return
    ...
    def _menu(self, pos: QPoint):
        r = self._current_row()
        m = QMenu(self)
        act_copy = m.addAction("Copy ID", self._copy_id)
        act_copy.setEnabled(bool(r))
        m.addSeparator()
        a_role_op = m.addAction(
            "Set role: operator", lambda: self._update_role("operator")
        )
        ...
        if not r:
            for a in (a_role_op, a_role_ad, a_ren, a_pwd, a_rm):
                a.setEnabled(False)
    ...
    def _update_role(self, role):
        ident = self._current_id()
        if ident:
            self.ws.update(ident, role_new=role, cb=lambda m: None)
    ...
    def _reset_pw(self):
        ...
        text, ok = QInputDialog.getText(
            self,
            "Reset Password",
            f"New password for {r.get('username')}:",
            QLineEdit.Password,
        )
        if ok and text.strip():
