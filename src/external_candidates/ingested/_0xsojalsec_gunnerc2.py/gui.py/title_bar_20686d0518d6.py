# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\title_bar.py
# gui/title_bar.py
...
def _add_menu_checkbox_row(menu: QMenu, text: str, checked: bool, on_toggled):
    ...
    cb.toggled.connect(
        lambda v, lab=icon: lab.setPixmap(_menu_check_icon(v).pixmap(18, 18))
    )
...
class _SnapAssist:
    ...
    def _half(self, ag: QRect, left=False, right=False) -> QRect:
        ...
        if right:
            return QRect(
                ag.left() + ag.width() // 2, ag.top(), ag.width() // 2, ag.height()
            )
        return ag
    ...
    def _quarter(self, ag: QRect, tl=False, tr=False, bl=False, br=False) -> QRect:
        ...
        if tl:
            return QRect(ag.left(), ag.top(), w2, h2)
        if tr:
            return QRect(ag.left() + w2, ag.top(), w2, h2)
        if bl:
            return QRect(ag.left(), ag.top() + h2, w2, h2)
        if br:
            return QRect(ag.left() + w2, ag.top() + h2, w2, h2)
        return ag
...
class TitleBar(QWidget):
    def __init__(self, owner_window, dashboard=None, *, with_graph_menus: bool = True):
        ...
        if self._menus_enabled:
            self.m_graph = _CloseOnLeaveMenu(
                "Graph", self, leave_delay_ms=CLOSE_DELAY_MS
            )
            self.menubar.addMenu(self.m_graph)

            act_visit = self.m_graph.addAction("Visit C2")
            act_visit.triggered.connect(
                lambda: getattr(self._dash.graph, "visit_c2", lambda: None)()
            )

            # Filter submenu
            self.m_filter = _CloseOnLeaveMenu(
                "Filter", self.m_graph, leave_delay_ms=CLOSE_DELAY_MS
            )
            ...
            self.cb_os_win, row, icon = _add_menu_checkbox_row(
                self.m_filter, "Windows agents", True, self._push_filters
            )
            self._styled_rows.append((row, icon, self.cb_os_win))
            self.cb_os_lin, row, icon = _add_menu_checkbox_row(
                self.m_filter, "Linux agents", True, self._push_filters
            )
            self._styled_rows.append((row, icon, self.cb_os_lin))

            # Transports submenu
            self.m_trans = _CloseOnLeaveMenu(
                "Transports", self.m_filter, leave_delay_ms=CLOSE_DELAY_MS
            )
            ...
            for proto in ("tcp", "tls", "http", "https"):
                cb, row, icon = _add_menu_checkbox_row(
                    self.m_trans, proto, True, self._push_filters
                )
    ...
    def _finish_detach(self, gpos_after: QPoint, to_geo: QRect):
        ...
        self._drag_pos = QPoint(
            gpos_after.x() - to_geo.x(), gpos_after.y() - to_geo.y()
        )
        self._maybe_drag_restore = False
    ...
    def _begin_drag_restore(self, gpos: QPoint, local_pos_in_tb: QPoint):
        ...
        if self._tearoff_anim_ms > 0:
            ...
            QTimer.singleShot(
                self._tearoff_anim_ms,
                lambda: self._finish_detach(QCursor.pos(), to_geo),
            )
        else:
    ...
    def eventFilter(self, obj, ev):
        ...
        if obj is self._win and ev.type() in (
            QEvent.Move,
            QEvent.Resize,
            ...
            if ev.type() == QEvent.MouseButtonPress and ev.button() == Qt.LeftButton:
                if self.menubar.actionAt(ev.pos()) is None:
                    ...
                    if self._win.isMaximized():
                        ...
                        self._drag_pos = (
                            ev.globalPos() - self._win.frameGeometry().topLeft()
                        )
                        return True
