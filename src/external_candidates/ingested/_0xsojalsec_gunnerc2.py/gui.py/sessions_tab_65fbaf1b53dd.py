    ...
    def sort(self, column, order):
        ...
        def _keyfunc(r):
            if key == "last_checkin":
                ...
                rank = {"live": 0, "warm": 1, "cool": 2, "stale": 3, "unknown": 4}.get(
                    cls, 5
                )
                return (rank, label)
...
class SessionsTab(QWidget):
    ...
    def __init__(self, api):
        ...
        self.search.setPlaceholderText(
            "Search sessions (ID, host, user, OS, transport)…"
        )
    ...
    def _rebuild_columns_menu(self):
        ...
        for i, (name, _) in enumerate(COLUMNS):
            act = QAction(
                name, self, checkable=True, checked=not self.table.isColumnHidden(i)
            )
            act.toggled.connect(
                lambda checked, col=i: self.table.setColumnHidden(col, not checked)
            )
    ...
    def kill_selected(self):
        ...
        def _done(resp: dict):
            ...
            if t == "killed":
                ...
                QMessageBox.warning(
                    self, "Kill Failed", str(resp.get("error", "Unknown error"))
                )
