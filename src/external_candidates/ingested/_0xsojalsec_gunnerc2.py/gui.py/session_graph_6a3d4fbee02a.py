# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\session_graph.py
...
from theme_center import ThemeManager, theme_color
...
def _paint_firewall(p: QPainter, rect: QRectF):
    ...
    wall = QRectF(
        rect.left() + 6, rect.top() + 18, rect.width() - 20, rect.height() - 18
    )
    p.setPen(Qt.NoPen)
    ...
    p.drawLine(
        QLineF(wall.left() + 4, wall.center().y(), wall.right() - 4, wall.center().y())
    )
...
def _paint_windows_computer(p: QPainter, rect: QRectF):
    ...
    # base monitor
    screen = QRectF(
        rect.left() + 8, rect.top() + 6, rect.width() - 16, rect.height() - 22
    )
...
class AgentItem(QGraphicsObject):
    ...
    def contextMenuEvent(self, event):
        ...
        act_files = browsers_menu.addAction(
            "Open File Browser"
        )  # <-- item you asked for
        ...
        if chosen == act_console:
            ...
            val = (
                ...
                if chosen == act_c_sid
                    ...
                    self.node.username
                    if chosen == act_c_user
                    else (
                        self.node.hostname
                        if chosen == act_c_host
                        else self.node.os if chosen == act_c_os else self.node.protocol
                    )
                )
            )
...
class C2Item(QGraphicsItem):
    def __init__(self):
        ...
        self.title.setPos(
            -self.title.boundingRect().width() / 2,
            -h / 2 - self.title.boundingRect().height() - 2,
        )
...
class EdgeItem(QGraphicsItem):
    ...
    def refresh(self):
        ...
        if LABEL_EDGE_PAD > 0.0:
            lb = lb.adjusted(
                -LABEL_EDGE_PAD, -LABEL_EDGE_PAD, LABEL_EDGE_PAD, LABEL_EDGE_PAD
            )
    ...
    def paint(self, p: QPainter, opt: QStyleOptionGraphicsItem, widget=None):
        ...
        if (
            not self.label.isVisible()
            or self._seg1_end is None
            or self._seg2_start is None
        ):
            if ARROW_GLOW:
                glow = QPen(NEON_DIM, 5.0)
                glow.setStyle(pen.style())
                p.setPen(glow)
                p.drawLine(QLineF(b, tip))
            p.setPen(pen)
            p.drawLine(QLineF(b, tip))
...
class GraphView(QGraphicsView):
    ...
    def mousePressEvent(self, e):
        ...
        if (e.button() == Qt.MiddleButton) or (
            e.button() == Qt.LeftButton and self._treat_as_background(item)
        ):
            self._panning = True
...
class SessionGraph(QWidget):
    ...
    def set_os_filter(
        self, show_windows: Optional[bool] = None, show_linux: Optional[bool] = None
    ):
    ...
    def _apply_filters_to_items(self):
        ...
        for it in self.agent_items.values():
            ...
            os_ok = (
                (is_win and self._show_windows)
                or (is_lin and self._show_linux)
                or (not is_win and not is_lin)
            )

            proto_ok = (
                (it.node.protocol.lower() in self._allowed_protocols)
                if self._allowed_protocols
                else False
            )
    ...
    def _dict_to_node(self, s: dict) -> SessionNode:
        ...
        return SessionNode(
            sid=sid, hostname=hostname, username=username, os=osname, protocol=protocol
        )
    ...
    def _upsert_node(self, node: SessionNode):
        ...
        def _find_spawn_pos(new_item: AgentItem) -> QPointF:
            ...
            c2_half = (
                max(
                    getattr(self.c2, "_rect", QRectF(-30, -30, 60, 60)).width(),
                    getattr(self.c2, "_rect", QRectF(-30, -30, 60, 60)).height(),
                )
                * 0.5
            )
            ...
            existing_items = [
                it for it in self.agent_items.values() if it is not new_item
            ]
            ...
            for ring in range(max_rings):
                ...
                for s in range(slots):
                    ...
                    cand = QPointF(
                        c2c.x() + math.cos(ang) * r, c2c.y() + math.sin(ang) * r
                    )
                    ...
                    if existing_pos:
                        ...
                        y_deltas = sorted(
                            (cand.y() - p.y() for p in existing_pos),
                            key=lambda v: abs(v),
                        )
                        if y_deltas:
            ...
            return QPointF(
                c2c.x() + (idx % 10) * spacing - 5 * spacing,
        ...
        if item is None:
            ...
            item._label.setPos(
                -item._label.boundingRect().width() / 2, item._rect.height() / 2 + 6
            )
    ...
    def _build_find_panel(self):
        ...
        QShortcut(
            ...
            self._find_panel,
            activated=self._hide_find_panel,
        )
        ...
        QShortcut(
            QKeySequence("Shift+Return"), self._find_panel, activated=self._find_prev
        )
