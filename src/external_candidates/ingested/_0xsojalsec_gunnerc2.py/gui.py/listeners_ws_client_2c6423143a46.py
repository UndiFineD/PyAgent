# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\listeners_ws_client.py
...
class ListenersWSClient(QObject):
    ...
    created = pyqtSignal(
        dict
    )  # {"ok": bool, "row"?: dict, "error"?: str, "req_id"?: int}
    stopped = pyqtSignal(
        dict
    )  # {"ok": bool, "id"?: str, "error"?: str, "req_id"?: int}
    ...
    def stop(self, listener_id: str):
        ...
        self._send(
            {
                "action": "listeners.stop",
                "id": str(listener_id or ""),
