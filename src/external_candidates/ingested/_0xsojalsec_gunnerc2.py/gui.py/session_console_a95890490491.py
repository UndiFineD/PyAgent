# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\session_console.py
...
from PyQt5.QtCore import QPoint, QRect, QRectF, QSize, Qt, QTimer, QUrl, pyqtSignal
...
try:
    from theme_center import ThemeManager, theme_color  # type: ignore
except Exception:
...
def _make_play_icon(sz=16, col="#eaf2ff"):
    ...
    p.setPen(Qt.NoPen)
    path = QPainterPath()
    path.moveTo(int(sz * 0.30), int(sz * 0.20))
    path.lineTo(int(sz * 0.30), int(sz * 0.80))
    path.lineTo(int(sz * 0.80), int(sz * 0.50))
    path.closeSubpath()
    p.drawPath(path)
    p.end()
    return QIcon(pm)
...
class _ConsoleHistoryLineEdit(QLineEdit):
    ...
    def _load(self):
        try:
            if self._history_path and os.path.exists(self._history_path):
                with open(
                    self._history_path, "r", encoding="utf-8", errors="ignore"
                ) as f:
    ...
    def _ris_recompute(self, direction: int = -1):
        ...
        best = (-1, -1)
        rng = (
            range(i - 1, -1, -1) if direction < 0 else range(i + 1, len(self._hist), 1)
        )
        for k in rng:
    ...
    def _ris_update_popup(self):
        ...
        match_txt = (
            self._hist[self._ris_idx]
            if idx_ok
            else ("no history" if not self._hist else "no match")
        )
        self._ris_label.setText(
            f"reverse-i-search: “{_esc(self._ris_query)}”  →  {_esc(match_txt)}"
        )
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_Down:
            if not self._browsing:
                e.accept()
                return  # nothing to do if not browsing
...
class SessionConsole(QWidget):
    ...
    def __init__(self, api, sid: str, hostname: str):
        ...
        self.title.setStyleSheet(
            "QLabel#ConsoleTitle{font-size:16.5pt;font-weight:600;color:#eaf2ff;letter-spacing:0.3px;}"
        )
        ...
        self.status.setStyleSheet(
            "QPushButton#StatusBtn{color:#cbd5e1; border:none; padding:0 4px; font-size:9.5pt;}"
        )

        self.btn_files = QPushButton("Files")
        self._style_pill(self.btn_files)
        self.btn_wrap = QPushButton("Wrap")
        self._style_pill(self.btn_wrap, checkable=True)
        self.btn_time = QPushButton("Timestamps")
        self._style_pill(self.btn_time, checkable=True, checked=True)
        self.btn_lock = QPushButton("Scroll-lock")
        self._style_pill(self.btn_lock, checkable=True, checked=False)
        self.btn_copy = QPushButton("Copy all")
        self._style_pill(self.btn_copy)
        self.btn_clear = QPushButton("Clear")
        self._style_pill(self.btn_clear)
        ...
        # Websocket
        ws_url = (
            self.api.base_url.replace("http", "ws", 1)
            + f"/ws/sessions/{sid}?token={self.api.token}"
        )
        self.ws = QWebSocket()
    ...
    def _append_text(self, text: str, *, key: str = None):
        ...
        if (
            ...
            and (now - self._rep_last_ts) <= self._rep_window
        ):
            self._rep_count += 1
    ...
    def _copy_all(self):
        ...
        QTimer.singleShot(800, lambda: self.status.setText(txt))

    def _on_msg(self, m: str):
        low = m.strip().lower()
        # Collapse and permanently mute "session not found" floods
        if (
            "session not found" in low or "no such session" in low
        ) and not self._snf_seen:
            self._snf_seen = True
            self._append_text(
                "** Session not found — closing this console. **", key="snf_banner"
            )
            try:
                self.ws.close()
            except Exception:
