# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\api_client.py
# gui/api_client.py
import os
import urllib.parse
...
class APIClient:
    ...
    def create_operator(self, username: str, password: str, role: str):
        r = requests.post(
            f"{self.base_url}/auth/operators",
    ...
    def delete_operator(self, operator_id: str):
        r = requests.delete(
            f"{self.base_url}/auth/operators/{operator_id}", headers=self.headers
        )
        if not r.ok:
            try:
                msg = r.json().get("detail")
    ...
    def create_listener(
        self, ltype: str, bind_ip: str, port: int, profile: str | None = None
    ):
        r = requests.post(
    ...
    def create_listener_v2(self, cfg: dict):
        ...
        r = requests.post(
            f"{self.base_url}/listeners", headers=self.headers, json=payload
        )
        if not r.ok:
            try:
                raise Exception(r.json().get("detail"))
    ...
    def exec_once(self, sid: str, cmd: str):
        r = requests.post(
            f"{self.base_url}/sessions/{sid}/exec",
            headers=self.headers,
            params={"cmd": cmd},
        )
        r.raise_for_status()
    ...
    def download_file(self, sid: str, remote_path: str, local_path: str):
        with requests.get(
            ...
            headers=self.headers,
            params={"sid": sid, "path": remote_path},
            stream=True,
        ) as r:
            r.raise_for_status()
    ...
    def upload_file(self, sid: str, local_path: str, remote_path: str):
        with open(local_path, "rb") as fp:
            files = {
                "file": (os.path.basename(local_path), fp, "application/octet-stream")
            }
            r = requests.post(
    ...
    def gen_win_ps1(self, transport: str, host: str, port: int, beacon: int = 5) -> str:
        r = requests.get(
            f"{self.base_url}/payloads/windows/ps1",
            headers=self.headers,
            params={
                "transport": transport,
                "host": host,
    ...
    def gen_linux_bash(self, transport: str, host: str, port: int) -> str:
        r = requests.get(
            f"{self.base_url}/payloads/linux/bash",
            headers=self.headers,
    ...
    def generate_windows_payload(self, cfg: Dict) -> str:
        ...
        """
        r = requests.post(
            f"{self.base_url}/payloads/windows",
            headers=self.headers,
            json=cfg,
            timeout=180,
        )
    ...
    def generate_linux_payload(self, cfg: Dict) -> str:
        ...
        """
        r = requests.post(
            f"{self.base_url}/payloads/linux",
            headers=self.headers,
            json=cfg,
            timeout=180,
        )
