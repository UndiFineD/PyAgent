# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\dashboard.py
# gui/dashboard.py
...
from PyQt5.QtGui import QBrush, QFont, QIcon, QPainter, QPen, QPixmap
...
class Dashboard(QWidget):
    ...
    def __init__(self, api, parent=None):
        ...
        self._tab_meta = (
            {}
        )  # widget -> {"kind": "console"|"gunnershell", "sid":..., "hostname":..., "username":..., "arch":...}
    ...
    def _restore_workspace_from_cache(self):
        ...
        normal = sp.property("heavyNormalSizes") or getattr(
            self, "_ws_normal_sizes", None
        )
    ...
    def _unlock_host_tabbar(self):
        ...
        if isinstance(tb, QTabBar):
            try:
                tb.setMovable(bool(getattr(self, "_host_tabbar_was_movable", True)))
            except Exception:
                pass
    ...
    def _duplicate_tab_from_index(self, idx: int):
        ...
        if kind == "console":
            ...
            base = (
                f"GS — {_strip_host_prefix(user, host)}@{host}"
                if user
                else f"GS — {host}"
            )
    ...
    def _on_tabbar_menu(self, pos):
        ...
        if is_cs:
            ...
            m.addSeparator()
            m.addAction(
                "Copy SID", lambda: self._copy_to_clipboard(meta.get("sid", ""))
            )
            m.addAction(
                "Copy Username",
                lambda: self._copy_to_clipboard(meta.get("username", "")),
            )
            m.addAction(
                "Copy Hostname",
                lambda: self._copy_to_clipboard(meta.get("hostname", "")),
            )
    ...
    def _open_console_tab(self, sid: str, hostname: str):
        ...
        try:
            s_cache = (
                self.sessions_ws.get_cached(sid) if hasattr(self, "sessions_ws") else {}
            )
            arch = (
                (s_cache.get("metadata") or {}).get("arch") or s_cache.get("arch") or ""
            )
        except Exception:
            pass
        ...
        if not username and ws:
            ...
            def _cb(msg):
                ...
                u2 = (
                    s2.get("user")
                    or s2.get("username")
                    or (s2.get("metadata") or {}).get("user")
                    or ""
                )
                if not u2:
                    return
    ...
    def _try_open_gs_from_session(self, sid: str, hostname: str, sess: dict):
        ...
        username = (
            ...
            or (sess.get("metadata") or {}).get("user")
            or ""
        )
        title = (
            f"GS — { _strip_host_prefix(username, hostname) }@{hostname}"
            if username
            else f"GS — {hostname}"
        )
