...
class ListenersTab(QWidget):
    def __init__(self, api):
        ...
        self.search.setPlaceholderText(
            "Search listeners (ID, name, type, IP, port, status)…"
        )
        ...
        self.table.viewport().installEventFilter(
            self
        )  # allow blank-click to clear selection
    ...
    def _sel_changed(self, *_):
        ...
        self.btn_stop.setEnabled(
            row is not None and (row.get("status") or "").upper() == "STARTED"
        )
    ...
    def _copy_ip_port(self):
        ...
        QApplication.clipboard().setText(
            f"{row.get('bind_ip','')}:{row.get('port','')}"
        )
    ...
    def open_create_dialog(self):
        dlg = CreateListenerDialog(
            self, bind_check=self._bind_check, name_check=self._name_check
        )
        ...
        if getattr(self, "_ws_ready", False):
            ...
            return
        try:
            adv = getattr(self.api, "create_listener_v2", None) or getattr(
                self.api, "create_listener_advanced", None
            )
            if callable(adv):
                adv(cfg)
            else:
    ...
    def _on_created(self, msg: dict):
        if not msg.get("ok"):
            QMessageBox.critical(
                self, "Create Listener", str(msg.get("error") or "Unknown error")
            )
            return
    ...
    def _on_stopped(self, msg: dict):
        if not msg.get("ok"):
            QMessageBox.critical(
                self, "Stop Listener", str(msg.get("error") or "Failed to stop")
            )
            return
