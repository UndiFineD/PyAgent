# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\payloads_tab.py
...
from PyQt5.QtGui import QColor, QFont, QIntValidator, QPalette, QTextOption
...
class PayloadsTab(QWidget):
    ...
    def _build_ui(self):
        ...
        tform = QFormLayout(grp_target)
        tform.setLabelAlignment(Qt.AlignRight)
        self.host = QLineEdit()
        self.host.setPlaceholderText("C2 host or IP")
        self.port = QLineEdit()
        ...
        self.beacon = QSpinBox()
        self.beacon.setRange(1, 86400)
        self.beacon.setValue(5)
        self.jitter = QSpinBox()
        self.jitter.setRange(0, 90)
        self.jitter.setValue(0)
        self.useragent = QLineEdit()
        self.useragent.setPlaceholderText("User-Agent (optional)")
        self.accept = QLineEdit()
        self.accept.setPlaceholderText("Accept header (optional)")
        self.byte_range = QLineEdit()
        self.byte_range.setPlaceholderText("Range header (optional)")
        self.byte_range.setValidator(
            QIntValidator(0, 2_147_483_647, self)
        )  # numeric only
        # Profile: full path to a *.profile file (picker)
        self.profile = QLineEdit()
        self.profile.setPlaceholderText("Select .profile file (optional)")
        ...
        _prof_row = QWidget()
        _prof_lyt = QHBoxLayout(_prof_row)
        _prof_lyt.setContentsMargins(0, 0, 0, 0)
        _prof_lyt.setSpacing(6)
        _prof_lyt.addWidget(self.profile, 1)
        _prof_lyt.addWidget(self.btn_profile, 0)

        # Plain text editor behaves better in forms than rich-text QTextEdit
        self.headers = QPlainTextEdit()
        self.headers.setFixedHeight(72)
        ...
        hdr_pal.setColor(
            QPalette.PlaceholderText, QColor("#ffffff")
        )  # <- white placeholder
        ...
        aform = QFormLayout(grp_adv)
        aform.setLabelAlignment(Qt.AlignRight)
        self.obs = QSpinBox()
        self.obs.setRange(0, 5)
        self.obs.setValue(0)
        self.no_child = QCheckBox("Disable child process (PS1)")
        self.stager_ip = QLineEdit()
        self.stager_ip.setPlaceholderText("Stager IP (EXE)")
        self.stager_port = QLineEdit()
        self.stager_port.setPlaceholderText("Stager Port (EXE)")
        ...
        oform = QFormLayout(grp_out)
        oform.setLabelAlignment(Qt.AlignRight)
        self.output_path = QLineEdit()
        self.output_path.setPlaceholderText("Write text payload to file (optional)")
        ...
        _out_row = QWidget()
        _out_lyt = QHBoxLayout(_out_row)
        _out_lyt.setContentsMargins(0, 0, 0, 0)
        _out_lyt.setSpacing(6)
        _out_lyt.addWidget(self.output_path, 1)
        _out_lyt.addWidget(self.btn_output, 0)
        ...
        rlyt.setContentsMargins(10, 10, 10, 10)
        rlyt.setSpacing(8)
    ...
    def _init_smooth_scroll(self, area):
        ...
        metric_key = getattr(
            SM, "OvershootPolicy", int(getattr(SM, "OvershootPolicy", 6))
        )
    ...
    def eventFilter(self, obj, ev):
        ...
        if (
            ...
            if dy:
                ...
                self._smooth_anim.setDuration(
                    110 if target in (sb.minimum(), sb.maximum()) else 160
                )
    ...
    def _refresh_transports(self):
        ...
        if fmt == "gunnerplant":
            ...
            if prev and any(
                prev == self.transport.itemText(i).lower()
    ...
    def _on_transport_changed(self):
        ...
        self.no_child.setVisible(
            self.platform.currentText() == "Windows"
            and self.format.currentText() == "ps1"
        )
        self._toggle_format_specific_fields()
    ...
    def _generate(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Payload", str(e))
            return
        ...
        try:
            if plat == "Windows":
                ...
                if fmt in ("exe", "gunnerplant"):
                    ...
                    if not sip:
                        QMessageBox.warning(
                            self,
                            "Payload",
                            "Stager IP is required for EXE/Gunnerplant.",
                        )
                        return
                    try:
                        sp_int = int(sp)
                        ...
                        QMessageBox.warning(
                            self,
                            "Payload",
                            "Stager Port must be an integer for EXE/Gunnerplant.",
                        )
                        return
                    if not (1 <= sp_int <= 65535):
                        QMessageBox.warning(
                            self, "Payload", "Stager Port out of range (1-65535)."
                        )
                        return
                cfg = {
                    "format": fmt,
                ...
                if fmt == "gunnerplant" and t != "https":
                    QMessageBox.warning(
                        self, "Payload", "Gunnerplant requires HTTPS transport"
                    )
                    return
            ...
            if out_path:
                try:
                    ...
                    self.out.setPlainText(
                        f"[+] Payload written to {out_path}\n\n{txt or ''}"
                    )
                except Exception as e:
                    QMessageBox.critical(
                        self, "Output", f"Failed to write {out_path}:\n{e}"
                    )
                    self.out.setPlainText(txt or "")
            else:
                self.out.setPlainText(txt or "")
    ...
    def _save(self):
        ...
        if not txt:
            return
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Payload", "payload.txt", "All Files (*);;Text (*.txt)"
        )
        if not path:
            return
        try:
