# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\files_ws_client.py
...
class FilesWSClient(QObject):
    ...
    def open(self):
        ws_url = (
            self.base_url.replace("http", "ws", 1) + f"/ws/files?token={self.token}"
        )
    ...
    def start_upload_folder(
        self, sid: str, local_dir: str, remote_dir: str, *, os_type: str = ""
    ):
        """
                ...
                with zipfile.ZipFile(
                    ...
                    for root, _, files in os.walk(local_dir):
                        for name in files:
                            ...
                            arc = os.path.join(
                                base, os.path.relpath(full, local_dir)
                            ).replace("\\", "/")
    ...
    def _on_text(self, s: str):
        ...
        if t == "fs.list":
            ...
            self.dl_end.emit(
                m.get("tid", ""), m.get("status", ""), m.get("error") or ""
            )
            ...
            try:
                if self._tmp_archive_for_upload and os.path.exists(
                    self._tmp_archive_for_upload
                ):
                    try:
