...
class SessionsWSClient(QObject):
    ...
    def exec(
        ...
        return self._send(
            {"action": "exec", "sid": sid, "cmd": cmd, "op_id": op_id}, cb
        )
    ...
    def _send(
        self, payload: Dict[str, Any], cb: Optional[Callable[[dict], None]] = None
    ) -> str:
    ...
    def _send_ping(self):
        try:
            self.ws.sendTextMessage(
                json.dumps({"action": "ping"}, separators=(",", ":"))
            )
        except Exception:
            self._schedule_reconnect()
    ...
    def _on_error(self, err):
        ...
        try:
            name = (
                QAbstractSocket.SocketError(err).name
    ...
    def _on_message(self, txt: str):
        ...
        if t == "snapshot":
            ...
            self.execResult.emit(
                str(rid or ""), str(msg.get("sid", "")), msg.get("output") or ""
            )
        elif t == "error":
...
class OperatorsWSClient(QObject):
    ...
    def login(self, username: str, password: str, cb=None):
        return self._send(
            {"action": "login", "username": username, "password": password}, cb
        )
    ...
    def update(
        self, ident, username_new=None, password_new=None, role_new=None, cb=None
    ):
