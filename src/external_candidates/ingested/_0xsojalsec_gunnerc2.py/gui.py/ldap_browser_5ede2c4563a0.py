# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\gui\ldap_browser.py
# gui/ldap_browser.py
...
class _QuickItem(QStandardItem):
    ...
    def __init__(self, text: str, dn: str = "", kind: str = "", is_group: bool = False):
        ...
        self.setData(
            kind, Qt.UserRole + 2
        )  # 'users' | 'groups' | 'ous' | 'computers' | 'dcs' | ''
...
class _GlassCard(QFrame):
    ...
    def paintEvent(self, ev):
        ...
        top.setColorAt(0.0, QColor(255, 255, 255, 18))
    ...
    def index(self, row, col, parent):
        ...
        if not parent.isValid():
            if 0 <= row < len(self.roots):
                return self.createIndex(row, col, self.roots[row])
            return QModelIndex()
        p = self._node(parent)
        if not p or row >= len(p.children):
            return QModelIndex()
...
class _LdapPane(QWidget):
    titleChanged = pyqtSignal(
        str
    )  # emit when we can show a nice tab title (e.g., host)
    ...
    def __init__(self, api, sid: str = "", hostname: str = "", parent=None):
        ...
        self.title.setStyleSheet(
            "QLabel{font-weight:800;font-size:16pt;color:#eaf2ff;letter-spacing:0.3px;}"
        )
        self.status = QLabel("Disconnected")
        self.status.setStyleSheet("QLabel{color:#cbd5e1;}")
        ...
        self.ed_base.setPlaceholderText("Base DN")
        self.btn_copy = QToolButton()
        self.btn_copy.setText("📋")
        self.btn_copy.setToolTip("Copy Base DN")
        self.ed_filter = QLineEdit()
        self.ed_filter.setPlaceholderText(
            "(type to live search…) e.g. (&(objectCategory=person)(objectClass=user))"
        )
        self.cmb_page = QComboBox()
        self.cmb_page.addItems(["200", "500", "1000"])
        self.cmb_page.setCurrentText("500")
        self.cmb_preset = QComboBox()
        self.cmb_preset.addItems(
            ["Preset: All", "Preset: Users", "Preset: Groups", "Preset: Computers"]
        )
        ...
        bar2.addWidget(QLabel("Base:"))
        bar2.addWidget(self.ed_base, 1)
        bar2.addWidget(self.btn_copy, 0)
        bar2.addWidget(QLabel("Filter:"))
        bar2.addWidget(self.ed_filter, 3)
        # give the small controls a little air before Search
        bar2.addSpacing(6)
        bar2.addWidget(QLabel("Page:"))
        bar2.addWidget(self.cmb_page, 0)
        ...
        qa_v.setContentsMargins(0, 0, 0, 0)
        qa_v.setSpacing(6)
        ...
        dir_wrap = QFrame()
        dir_v = QVBoxLayout(dir_wrap)
        dir_v.setContentsMargins(0, 0, 0, 0)
        dir_v.setSpacing(6)
        ...
        q = QUrlQuery()
        q.addQueryItem("token", self.api.token or "")
        if self.sid:
            q.addQueryItem("sid", self.sid)
        base.setQuery(q)
        ...
        self.ws.connected.connect(
            lambda: (
                self._set_status("Connected — discovering…"),
                self._auto_open_current(),
            )
        )
        self.ws.open(base)
        ...
        self.btn_copy.clicked.connect(
            lambda: QApplication.clipboard().setText(self.ed_base.text() or "")
        )
    ...
    def _set_status(self, s: str):
        ...
        try:
            if (
                ...
                and not sip.isdeleted(self.status)
            ):
    ...
    def _on_quick_expanded(self, idx):
        ...
        if it.hasChildren() and it.child(0).text() == "Loading…":
            ...
            self._send(
                {"action": "ldap.quick", "kind": it.kind, "base": base, "size": 1000}
            )
    ...
    def _update_tab_title(self):
        self.titleChanged.emit(
            self._host_short
            if not self._last_rdn
            else f"{self._host_short}/{self._last_rdn}"
        )
    ...
    def _apply_preset(self):
        ...
        if i == 1:
            self.ed_filter.setText("(&(objectCategory=person)(objectClass=user))")
        elif i == 2:
            self.ed_filter.setText("(&(objectCategory=group)(objectClass=group))")
        elif i == 3:
            self.ed_filter.setText("(objectClass=computer)")
        else:
            self.ed_filter.setText("(objectClass=*)")
        ...
        self._send(
            {
                ...
                "scope": "sub",
                "filter": filt,
                "attrs": ["cn", "name", "distinguishedName", "objectClass"],
                "size": page,
            }
        )
    ...
    def _on_expanded(self, idx: QModelIndex):
        ...
        self._send(
            {
                "action": "ldap.children",
                "dn": node.dn,
                "size": 500,
                "attrs": ["cn", "name", "objectClass"],
            }
        )
    ...
    def _on_msg(self, txt: str):
        ...
        def _install_context_copy():
            ...
            def _on_tbl(pt):
                ...
                if r < 0:
                    return
                k = (
                    self.tbl_attrs.item(r, 0).text()
                    if self.tbl_attrs.item(r, 0)
                    else ""
                )
                v = (
                    self.tbl_attrs.item(r, 1).text()
                    if self.tbl_attrs.item(r, 1)
                    else ""
                )
                m = QMenu(self)
                a1 = m.addAction("Copy Value")
                a2 = m.addAction("Copy Attribute")
                a3 = m.addAction("Copy Row")
                ...
                if act is a1:
                    ...
                    QApplication.clipboard().setText(f"{k}: {v}")

            try:
                self.tbl_attrs.customContextMenuRequested.disconnect()
            except Exception:
                pass
        ...
        if t == "ldap.opened":
            if not msg.get("ok"):
                self._set_status(f'Bind failed: {msg.get("error","")}')
                return
            ...
            for k, v in sorted(attrs.items(), key=lambda kv: kv[0].lower()):
                row = self.tbl_attrs.rowCount()
                self.tbl_attrs.insertRow(row)
                key_item = QTableWidgetItem(k)
                if isinstance(v, list):
                    val_text = ", ".join(str(x) for x in v[:50]) + (
                        " …" if len(v) > 50 else ""
                    )
                else:
                    val_text = str(v)
...
class LdapBrowser(QWidget):
    ...
    def __init__(self, api, sid: str = "", hostname: str = "", parent=None):
        ...
        self.btn_new.setToolTip("New LDAP tab")
        self.btn_new.clicked.connect(self._new_tab)
        tabs_row.addWidget(self.tabbar, 1)
        tabs_row.addWidget(self.btn_new, 0)
    ...
    def _collapse_workspace_header(self, on: bool):
        ...
        if on:
            ...
            try:
                sp.setStyleSheet(
                    (self._old_splitter_css or "")
                    + " QSplitter::handle { image: none; background: transparent; height: 0px; }"
                )
            except Exception:
                pass
        else:
    ...
    def _on_host_tab_changed(self, idx: int):
        ...
        if not self._is_heavy_widget(new_w) and getattr(
            self, "_ldap_mode_active", False
        ):
    ...
    def _make_close_icon_fallback(
        self, size: int = 18, base_hex: str = "#ff3b30"
    ) -> QIcon:
        def mk_pix(
            color_hex: str, shadow_alpha: int = 70, inflate: float = 0.0
        ) -> QPixmap:
    ...
    def eventFilter(self, obj, ev):
        if obj is self.tabbar and ev.type() in (
            QEvent.Show,
            QEvent.Resize,
    ...
    def _skin_tab_scrollers(self):
        ...
        for btn in self.tabbar.findChildren(QToolButton):
            ...
            try:
                ...
                at = Qt.NoArrow
            if at in (Qt.LeftArrow, Qt.RightArrow) or (
                "scroll" in btn.objectName().lower() if btn.objectName() else False
            ):
