# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\windows\system\getsid.py
from colorama import Fore, Style

# Command Execution Imports
from core.command_execution import http_command_execution as http_exec
from core.command_execution import tcp_command_execution as tcp_exec
from core.gunnershell.commands.base import Command, register
from core.session_handlers import session_manager

brightgreen = "\001" + Style.BRIGHT + Fore.GREEN + "\002"
brightyellow = "\001" + Style.BRIGHT + Fore.YELLOW + "\002"
brightred = "\001" + Style.BRIGHT + Fore.RED + "\002"


@register("getsid")
class GetsidCommand(Command):
    """Retrieve Windows SID: getsid"""

    @property
    def help(self):
        return "getsid    Show Windows SID"

    def execute(self, args):
        out = self.logic(self.gs.sid, self.gs.os_type, op_id=self.op_id)
        if out:
            print(brightgreen + out)

    def logic(self, sid, os_type, op_id="console"):
        """
        Retrieve the Windows SID of the current token.
        """
        display = next(
            (a for a, rsid in session_manager.alias_map.items() if rsid == sid), sid
        )
        if "windows" not in os_type:
            return brightyellow + "[*] getsid only supported on Windows"

        cmd = "[System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value"

        sess = session_manager.sessions.get(sid)
        if not sess:
            return brightred + f"[!] No such session: {display}"

        if sess.transport.lower() in ("http", "https"):
            out = http_exec.run_command_http(sid, cmd, op_id=op_id)
        else:
            out = tcp_exec.run_command_tcp(
                sid, cmd, timeout=0.5, portscan_active=True, op_id=op_id
            )

        return out or ""
