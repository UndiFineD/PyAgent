# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\windows\activedirectory\getforests.py
import argparse
import base64
import os
import sys
...
class GetForestsCommand(Command):
    ...
    def execute(self, args):
        ...
        parser.add_argument(
            ...
            dest="name",
            required=False,
            help="Forest DNS name to dump properties for",
        )
        parser.add_argument(
            "-d",
            "--domain",
            dest="domain",
            required=False,
            help="AD domain name (FQDN) or NetBIOS",
        )
        parser.add_argument(
            "--dc-ip",
            dest="dc_ip",
    ...
    def logic(self, sid, name=None, domain=None, dc_ip=None, op_id="console"):
        ...
        if target:
            ...
            if dns_preamble == "ERROR":
                return (
                    brightred
                    + "[!] Failed to resolve DC, use --dc-ip or --domain correctly"
                )

            server_arg = "-Server $nb"
            root_dom = '$ldapPath = "LDAP://$nb"; $root = ([ADSI] $ldapPath).defaultNamingContext'
            root_forest = (
                '$forestRoot = ([ADSI] "LDAP://$nb").configurationNamingContext'
            )
            ...
            root_forest = (
                '$forestRoot = ([ADSI]"LDAP://RootDSE").configurationNamingContext'
            )
...
"""
        ...
        if out:
            if "Failed to resolve DC!" in out:
                return (
                    brightred
                    + "[!] Failed to resolve DC, use --dc‑ip or --domain with the correct IP/domain"
                )
