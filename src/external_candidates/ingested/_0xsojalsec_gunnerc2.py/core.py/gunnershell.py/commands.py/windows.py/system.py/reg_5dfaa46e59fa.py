# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\windows\system\reg.py
...
class RegCommand(Command):
    ...
    def execute(self, args):
        if len(args) < 2:
            print(
                brightyellow
                + "Usage: reg <query|get|set|delete> <hive>\\<path> [name data]"
            )
            return
        action = args[0]
        hive_path = args[1]
    ...
    def logic(
        ...
        if action not in ("query", "get", "set", "delete"):
            return (
                brightyellow
                + "[*] Usage: reg <query|get|set|delete> <hive>\\<path> [<name> <data>] [/s|/f]"
            )
        ...
        if action == "query":
            ...
            cmd = (
                f'reg.exe add "{hive}\\{key_path}" /v {value_name} '
        ...
        if sess.transport.lower() in ("http", "https"):
            ...
            return (
                tcp_exec.run_command_tcp(
                    sid, cmd, timeout=3.0, portscan_active=True, op_id=op_id
                )
                or None
            )
