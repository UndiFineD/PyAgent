...
class ServicesCommand(Command):
    ...
    def logic(self, sid, os_type, action=None, svc_name=None, op_id="console"):
        ...
        if action not in ("list", "start", "stop", "restart"):
            return (
                brightyellow
                + "[*] Usage: services <list|start|stop|restart> [<service_name>]"
            )
        ...
        out = (
            ...
            if sess.transport.lower() in ("http", "https")
                ...
                sid, ps_cmd, timeout=5.0, portscan_active=True, op_id=op_id
            )
        ) or ""
        ...
        for line in out.splitlines():
            ...
            if line == "SUCCESS":
                if action == "stop":
                    return (
                        brightgreen + f"[*] Service '{svc_name}' Stopped successfully."
                    )
                else:
                    return (
                        brightgreen
                        + f"[*] Service '{svc_name}' {action}ed successfully."
                    )

            if line.startswith("FAILED:") or line.startswith("FAILED"):
                return (
                    brightred
                    + f"[!] Insufficient privileges to preform {action} on {svc_name} service"
                )
