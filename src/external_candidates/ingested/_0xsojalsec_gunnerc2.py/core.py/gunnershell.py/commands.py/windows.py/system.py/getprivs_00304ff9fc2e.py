# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\windows\system\getprivs.py
...
class GetprivsCommand(Command):
    ...
    def logic(self, sid, os_type, op_id="console"):
        ...
        """
        display = next(
            (a for a, rsid in session_manager.alias_map.items() if rsid == sid), sid
        )
        ...
        if "windows" in os_type:
            ...
            print(
                brightyellow
                + f"[*] Warning this is an interactive command, run sudo -l on an interactive shell instead!"
            )
            # cmd = "sudo -l"
        ...
        if sess.transport.lower() in ("http", "https"):
            ...
            return (
                tcp_exec.run_command_tcp(
                    sid, cmd, timeout=1.0, portscan_active=True, op_id=op_id
                )
                or None
            )
