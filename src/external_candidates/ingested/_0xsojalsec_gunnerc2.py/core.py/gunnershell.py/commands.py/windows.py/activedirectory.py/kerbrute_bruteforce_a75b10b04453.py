# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\windows\activedirectory\kerbrute_bruteforce.py
import argparse
import base64
import os
import sys
...
from core.gunnershell.commands.base import Command, connection_builder, register
...
class KerbruteBruteforceCommand(Command):
    ...
    def help(self):
        return (
            "kerbrute bruteforce -u <user|userfile> -p <pass|passfile> "
    ...
    def execute(self, args):
        ...
        parser.add_argument(
            ...
            dest="user",
            required=False,
            help="Single user or file of users (comma-list OK)",
        )
        parser.add_argument(
            "-p",
            dest="password",
            required=False,
            help="Single password or file of passwords (comma-list OK)",
        )
        parser.add_argument(
            "-d", dest="domain", required=True, help="Target AD domain (FQDN)"
        )
        parser.add_argument(
            "--dc-ip",
            dest="dc_ip",
        ...
        parser.add_argument(
            ...
            dest="credfile",
            required=False,
            help="File or comma-list of user:pass pairs",
        )
    ...
    def logic(
        ...
        if not credfile:
            if not (user and password):
                return (
                    brightred
                    + "[!] You must supply both -u and -p or -C to bruteforce!"
                )

        elif credfile and (user and password):
            return (
                brightred
                + "[!] You cannot use -C with -u and -p you must chose one mode!"
            )
        ...
        if dns_preamble == "ERROR":
            return (
                brightred + "[!] Failed to resolve DC, use correct --dc-ip and -d flags"
            )
...
"""
        ...
        if transport in ("http", "https"):
            ...
            out = tcp_exec.run_command_tcp(
                sid, one_liner, timeout=0.5, portscan_active=True, op_id=op_id
            )
