...
class BofExecCommand(Command):
    ...
    def help(self):
        return (
            "bofexec <bof_name_or_path> [-z STR ...] [-Z WSTR ...]   "
    ...
    def _parse_args(self, args: List[str]):
        ...
        p.add_argument(
            "bof", help="BOF name (from registry) or filesystem path to .o/.obj"
        )
        p.add_argument(
            "--x86",
            dest="x86",
        ...
        if unknown or ns.bof in CUSTOM_ARG_BOFS:
            ...
            if cls and hasattr(cls, "args_inject"):
                try:
                    ...
                    logger.exception(
                        "adstaleusers argparse (after inject) failed: %s", e
                    )
                    print(brightred + f"[argparse] {e}" + reset)
                    print(brightyellow + p.format_usage().strip() + reset)
                    return
                ...
                print(
                    brightred
                    + f"[!] Unknown arguments for '{ns.bof}': {' '.join(unknown)}"
                    + reset
                )
                return
    ...
    def execute(self, args):
        logger.debug(
            brightyellow + f"Passing args into bofexec argparse: {args}" + reset
        )
    ...
    def logic(
        ...
        if transport in ("http", "https"):
            ...
            out = tcp_exec.run_command_tcp(
                sid, preview, timeout=0.5, portscan_active=True, op_id=self.op_id
            )
