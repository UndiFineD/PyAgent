# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\commands\filesystem\ls.py
    ...
    def logic(self, sid, os_type, path, op_id="console"):
        ...
        if sess.transport.lower() in ("http", "https"):
            ...
            out = tcp_exec.run_command_tcp(
                sid, cmd, timeout=1, portscan_active=True, op_id=op_id
            )
            return out
            ...
            try:
                print(
                    brightred + f"[!] Unsupported shell type: {sess.transport.lower()}"
                )
