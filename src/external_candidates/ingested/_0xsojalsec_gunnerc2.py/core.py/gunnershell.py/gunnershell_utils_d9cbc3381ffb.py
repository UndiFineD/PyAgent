# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\gunnershell\gunnershell_utils.py
import logging
...
def pop_commands(gunnershell_class):
    ...
    try:
        if not instance.gunnerplant and "bofexec" in COMMANDS:
            ...
            logger.debug(
                brightyellow + "Disabled bofexec (gunnerplant is False)" + reset
            )

    except Exception as e:
        logger.debug(
            brightred + f"Failed to conditionally disable bofexec: {e}" + reset
        )
...
def _detect_gunnerplant(gunnershell_class, op_id="console"):
    ...
    try:
        ...
        if not sess:
            ...
            logger.debug(
                brightyellow + "No session found for gunnerplant probe" + reset
            )
            return
        ...
        if sess.transport.lower() in ("http", "https"):
            ...
            out = tcp_exec.run_command_tcp(
                instance.sid, cmd, timeout=1.5, portscan_active=True, op_id=op_id
            )

        text = (out or "").strip().lower()
        instance.gunnerplant = "specialprogram" in text
        logger.debug(
            brightgreen + f"Gunnerplant probe result: {instance.gunnerplant}" + reset
        )
