...
def _emit_header_lines(headers: dict, var: str, is_post: bool = False) -> str:
    ...
    for k, v in (headers or {}).items():
        if is_post and k.lower() == "content-type":
            lines.append(
                f'{var}.Content = "new StringContent(json, Encoding.UTF8, {_cs_escape(v)})";'
            )
        else:
            lines.append(
                f'{var}.Headers.TryAddWithoutValidation("{_cs_escape(k)}", "{_cs_escape(v)}");'
            )
    return "\n".join(lines)
...
def program(ip, port, cfg=None, scheme="https", profile=False):
    ...
    if profile:
        ...
        accept_line = (
            f'getReq.Headers.Accept.ParseAdd("{_cs_escape(cfg.accept)}");'
            if cfg.accept
            else ""
        )
        host_line = (
        ...
        try:
            if (
                getattr(cfg, "byte_range", None) is not None
                and str(cfg.byte_range).strip().isdigit()
            ):
        ...
        accept_post = (
            f'postReq.Headers.Accept.ParseAdd("{_cs_escape(cfg.accept_post)})";'
            if cfg.accept_post
            else ""
        )
        host_post = (
