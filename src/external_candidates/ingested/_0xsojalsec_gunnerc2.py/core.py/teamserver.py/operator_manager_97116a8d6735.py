# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\teamserver\operator_manager.py
import datetime
import fnmatch
import hashlib
import json
        ...
        def persist_state():
            ...
            with lockout_lock:
                serial = {
                    ...
                    "user_blocked_until": {
                        "|".join(k): v for k, v in user_blocked_until.items()
                    },
                }
            ...
            try:
                ...
                if key in user_blocked_until:
                    if now < user_blocked_until[key]:
                        enc_cli.sendall(
                            b"[!] Too many bad password attempts on this account; try again later.\n"
                        )
                        enc_cli.close()
                        continue
                    else:
                ...
                if not creds:
                    ...
                    # prune old
                    ip_failures[ip] = [
                        t for t in ip_failures[ip] if now - t < BF_WINDOW
                    ]
                    ...
                    user_failures[key] = [
                        t for t in user_failures[key] if now - t < BF_WINDOW
                    ]
                ...
                threading.Thread(
                    target=handle_operator, args=(op_id,), daemon=True
                ).start()
