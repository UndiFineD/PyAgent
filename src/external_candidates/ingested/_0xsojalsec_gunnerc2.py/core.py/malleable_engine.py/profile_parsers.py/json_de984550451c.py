# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\malleable_engine\profile_parsers\json.py
    ...
    def parse(self, path: str) -> Optional[MalleableProfile]:
        ...
        for endpoint in ("http-get", "http-post"):
            ...
            if not isinstance(client, dict) or not isinstance(server, dict):
                log.error(
                    "'%s.client' and '%s.server' must be objects", endpoint, endpoint
                )
                return None
