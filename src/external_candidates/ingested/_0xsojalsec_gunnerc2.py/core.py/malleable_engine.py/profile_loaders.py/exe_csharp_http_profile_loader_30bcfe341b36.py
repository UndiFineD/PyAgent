# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\malleable_engine\profile_loaders\exe_csharp_http_profile_loader.py
...
from ..base import EngineProfile, ProfileLoader
...
@register_loader("exe_csharp_http_profile_loader")
class ExeCsharpHttpV1(ProfileLoader):
    NAME = "exe_csharp_http_v1"
    _ABSTRACT = False
    ...
    def load(
        self, prof: MalleableProfile, *, defaults: Dict[str, Any]
    ) -> ExeCsharpHttpConfig:
        ...
        get_map = srv_out.get("mapping") or {
            "cmd": "{{payload}}",
            "DeviceTelemetry": {"Telemetry": "{{payload}}"},
        }
        ...
        return ExeCsharpHttpConfig(
            ...
            get_server_mapping=get_map,
            post_client_mapping=post_map,
        )
