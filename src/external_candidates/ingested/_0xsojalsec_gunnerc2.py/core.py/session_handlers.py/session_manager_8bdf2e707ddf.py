# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\session_handlers\session_manager.py
import base64
import fnmatch
import queue
import signal
    ...
    def detect_os(self, output: str):
        ...
        if "linux" in lower or "darwin" in lower:
            ...
            self.os_metadata_commands = [
                ...
                (
                    "arch",
                    "(Get-WmiObject Win32_OperatingSystem | Select-Object -ExpandProperty OSArchitecture)",
                ),
            ]
...
def resolve_sid(raw: str) -> str | None:
    ...
    try:
        if any(ch in raw for ch in "*?"):
            ...
            matches = [sid for sid in sessions if fnmatch.fnmatch(sid, raw)]
            # also match against alias names, resolving to real SIDs
            matches += [
                alias_map[alias] for alias in alias_map if fnmatch.fnmatch(alias, raw)
            ]
            ...
            if len(matches) == 1:
                ...
                print(
                    brightred
                    + f"[!] Ambiguous session pattern '{raw}' → matches {matches!r}"
                )
