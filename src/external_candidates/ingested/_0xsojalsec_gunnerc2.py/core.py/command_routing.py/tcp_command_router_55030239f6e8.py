...
class TcpCommandRouter:
    ...
    def execute(
        ...
        with self.session.exec_lock:
            ...
            try:
                ...
                logger.warning(
                    "[%s] execute.send() error: %s", time.strftime("%H:%M:%S"), e
                )
                if transfer_use:
            ...
            try:
                ...
                logger.warning(
                    "[%s] execute.receive() error: %s", time.strftime("%H:%M:%S"), e
                )
                if transfer_use:
    ...
    def _drain_socket(self):
        ...
        logger.debug(
            "[%s] Drain complete (%d bytes)", time.strftime("%H:%M:%S"), drained
        )
    ...
    def send(
        ...
        if (
            defender.is_active
            and not defender_bypass
            and os_type in ("windows", "linux")
        ):
        ...
        with self.send_lock:
            try:
                ...
                logger.debug(
                    "[%s] Sent wrapped command to socket", time.strftime("%H:%M:%S")
                )

            except (ConnectionResetError, BrokenPipeError, OSError) as e:
                logger.warning(
                    brightred
                    + f"Connect error ocurred on session {self.session.sid}"
                    + reset
                )
                if transfer_use:
                ...
                logger.warning(
                    "[%s] Failed to send command: %s", time.strftime("%H:%M:%S"), e
                )
                raise
    ...
    def receive(
        ...
        try:
            ...
            if transfer_use:
                raise ConnectionError(
                    "Connection Error while reading data over TCP socket"
                )
        ...
        # Demux other ops
        pattern = re.compile(
            rb"__OP__(?P<op>[^_]+)__\s*(?P<out>.*?)\s*__ENDOP__(?P=op)__", re.DOTALL
        )
