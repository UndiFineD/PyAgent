# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\command_routing\http_command_router.py
import base64
import logging
import queue
import time
...
class CommandRouter:
    ...
    def _ensure_queues(self, op_id: str):
        ...
        logger.debug(
            ...
            op_id,
            before_cmd,
            len(self.session.merge_command_queue),
            before_res,
            len(self.session.merge_response_queue),
        )
    ...
    def send(
        ...
        logger.debug(
            "[%s] Defender active=%r", time.strftime("%H:%M:%S"), defender.is_active
        )
        ...
        try:
            ...
            logger.warning(
                brightred + f"Hit exception while sending in HTTP/HTTPS: {e}" + reset
            )
            if transfer_use:
                raise ConnectionError(
                    "Hit ConnectionError while sending over HTTP/HTTPS"
                ) from e
    ...
    def receive(
        ...
        try:
            ...
            if transfer_use:
                raise ConnectionError(
                    "Hit ConnectionError while reading over HTTP/HTTPS"
                ) from e
