...
def _generate_tls_context(listen_ip: str) -> ssl.SSLContext:
    ...
    san = x509.SubjectAlternativeName(
        [x509.DNSName("GunnerC2"), x509.IPAddress(ipaddress.IPv4Address(listen_ip))]
    )
...
class TcpListener(Listener):
    ...
    def start(
        ...
        if self.is_ssl:
            if cert_path and key_path:
                if not (os.path.isfile(cert_path) and os.path.isfile(key_path)):
                    print(
                        brightred
                        + "[!] Specified cert or key file not found. Exiting listener."
                    )
                    logger.exception(
                        brightred
                        + "[!] Specified cert or key file not found. Exiting listener."
                ...
                print(
                    brightgreen + f"[*] Loaded TLS cert: {cert_path}, key: {key_path}"
                )

            else:
                logger.debug(
                    brightyellow + f"Generating self signed cert for TLS Listener"
                )
        ...
        self._thread = threading.Thread(
            target=self.run_loop, args=(self._stop_event, context), daemon=True
        )
        self._thread.start()
    ...
    def stop(self, timeout: float = None):
        ...
        try:
            ...
            with _reg_lock:
                try:
                    if self.is_ssl:
                        utils.tls_listener_sockets.pop(
                            f"tls-{self.ip}:{self.port}", None
                        )
                    else:
                        utils.tcp_listener_sockets.pop(
                            f"tcp-{self.ip}:{self.port}", None
                        )
                except Exception:
                    pass
            self.server.close()
    ...
    def run_loop(self, stop_event: threading.Event, context):
        ...
        while not stop_event.is_set():
            ...
            if self.is_ssl and ctx:
                ...
                try:
                    client = ctx.wrap_socket(
                        client, server_side=True, do_handshake_on_connect=True
                    )
                except Exception:
            ...
            threading.Thread(
                target=_collect_tcp_metadata, args=(sid,), daemon=True
            ).start()
