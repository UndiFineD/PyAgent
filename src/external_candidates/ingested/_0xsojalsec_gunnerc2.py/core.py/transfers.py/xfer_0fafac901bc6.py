# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\transfers\xfer.py
...
def _find_by_tid(
    tm: TransferManager, tid_or_prefix: str, sid_hint: Optional[str]
...
def _render_list_table(rows: list[dict]) -> str:
    ...
    fixed_w = (
        SID_W + TID_W + DIR_W + TYPE_W + STAT_W + PROG_W + RATE_W + (PADDING * 7)
    )  # + PATH later
...
def cmd_list(
    ...
    *,
    ...
    try:
        ...
        _emit(
            f"[!] Invalid session or alias: {raw_sid}",
            to_console,
            to_op,
            color=brightred,
            override_quiet=True,
...
def cmd_status(
    ...
    *,
    ...
    if not sid or not st0:
        _emit(
            f"[!] Transfer not found for TID/prefix: {tid_or_prefix}",
            to_console,
            to_op,
            override_quiet=True,
        )
    ...
    if not st:
        _emit(
            f"[!] Unable to load status for {sid}:{st0['tid']}",
            to_console,
            to_op,
            override_quiet=True,
        )
    ...
    *,
    ...
    if sid_hint:
        ...
        if session.transport.lower() in ("http", "https") and not timeout:
            _emit(
                f"You must specify a timeout for HTTP/HTTPS transfers (Use 2x your interval, 3x if jitter is big)",
                to_console,
                to_op,
                color=brightyellow,
                override_quiet=True,
    ...
    if err == "ambiguous":
        ...
        _emit(
            f"[!] Ambiguous TID/prefix. Matches: {s}",
            to_console,
            to_op,
            color=brightred,
            override_quiet=True,
    ...
    if sid and st0:
        ...
        ok = tm.resume(
            sid,
            st0["tid"],
            opts=TransferOpts(to_console=to_console, to_op=to_op),
            timeout=timeout,
        )
        if ok:
            _emit(
                f"[*] Resuming TID={st0['tid']} for session {sid}",
                to_console,
                to_op,
                color=brightgreen,
                override_quiet=True,
            ...
            return True
        _emit(
            f"[!] Unable to resume TID={st0['tid']} (status may be final)",
            to_console,
            to_op,
            color=brightred,
            override_quiet=True,
    ...
    if not sid_hint:
        ...
        _emit(
            f"[!] TID {real_tid} exists under session {current_sid}. "
            f"Use -i <live_session_id> to rebind before resuming.",
            to_console,
            to_op,
            color=brightyellow,
            override_quiet=True,
    ...
    if current_sid != sid_hint:
        ...
        if not updated:
            _emit(
                f"[!] Failed to rebind {real_tid} from {current_sid} -> {sid_hint}",
                to_console,
                to_op,
                color=brightred,
                override_quiet=True,
        ...
        if ok:
            _emit(
                f"[*] Rebound and resuming TID={real_tid} for session {sid_hint}",
                to_console,
                to_op,
            )
            return True
        _emit(
            f"[!] Unable to resume TID={real_tid} after rebind (status may be final)",
            to_console,
            to_op,
            override_quiet=True,
        )
    ...
    _emit(
        f"[!] Internal error while resolving resume for {tid_or_prefix}",
        to_console,
        to_op,
        override_quiet=True,
    )
...
def cmd_cancel(
    ...
    *,
    ...
    if ok:
        ...
        return True
    _emit(
        f"[!] Unable to cancel TID={st0['tid']}", to_console, to_op, override_quiet=True
    )
    return False
...
def clear_by_tids(tid_inputs: str, to_console=True, to_op=None) -> int:
    ...
    if not targets:
        echo(
            "[*] No matching transfers for given TID(s)/prefix(es).",
            to_console=to_console,
            to_op=to_op,
            world_wide=False,
        )
...
def clear_by_sid_pattern(sid_pattern: str, to_console=True, to_op=None) -> int:
    ...
    if not sids:
        echo(
            "[*] No sessions matched that pattern in the local store.",
            to_console=to_console,
            to_op=to_op,
            world_wide=False,
        )
    ...
    for sid in sids:
        ...
        try:
            ...
            pass
    echo(
        f"[+] Cleared {removed} transfer(s) across {len(sids)} session(s).",
        to_console=to_console,
        to_op=to_op,
        world_wide=False,
    )
