# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\transfers\protocols\shell.py
...
from ..chunker import chunk_count, ensure_prealloc, index_to_offset, write_at
...
class ShellProtocol(TransferProtocol):
    ...
    def _run_cmd(
        ...
        try:
            out = (
                http_exec.run_command_http(
                    sid, cmd, op_id=op_id, transfer_use=True, timeout=_eff_timeout
                )
                if tr in ("http", "https")
                else tcp_exec.run_command_tcp(
                    sid,
                    cmd,
        ...
        logger.debug(
            "  _run_cmd.ok elapsed=%.4fs out.len=%d out.preview=%s",
            elapsed,
            len(out),
            _preview(out),
        )
    ...
    def _remote_size(self, st: TransferState) -> int:
        ...
        _kv(
            ...
            is_folder=getattr(st, "is_folder", None),
        )
        ...
        if st.os_type == "linux":
            ...
            try:
                ...
                out = (
                    self._run_cmd(
                        st.sid, sh, st.transport, self.op_id, defender_bypass=True
                    )
                    or ""
                ).strip()
                ...
                logger.warning(
                    brightred + f"Connection Error in remote size grabber: {e}" + reset
                )
                raise ConnectionError(
                    "Connection Error in remote size in upload function _remote_size"
                ) from e
            ...
            try:
                ...
                out = (
                    self._run_cmd(st.sid, ps, st.transport, self.op_id) or ""
                ).strip()
    ...
    def _linux_read_chunk(self, st: TransferState, index: int) -> bytes:
        ...
        if not out.strip():
            logger.warning(
                brightred + "No output from agent for linux chunk read!" + reset
            )
    ...
    def _linux_write_chunk(
        self, st: TransferState, offset: int, chunk_b64: str
    ) -> None:
        """
    ...
    def _prepare_remote_archive(self, st: TransferState) -> None:
        ...
        if opt.get("archive_prepared") and opt.get("archive_path"):
            ...
            try:
                if st.os_type == "windows":
                    ...
                    try:
                        exists = "OK" in (
                            self._run_cmd(st.sid, sh, st.transport, self.op_id) or ""
                        )

                    except Exception as e:
                        logger.warning(
                            brightred + f"Connection Error in _run_cmd: {e}" + reset
                        )
                        raise ConnectionError("Connection Error in _run_cmd") from e
        ...
        if st.os_type == "windows":
            ...
            size_ps = f"if (Test-Path {_ps_quote(remote_zip)}) {{ (Get-Item {_ps_quote(remote_zip)}).Length }} else {{ 0 }}"
            logger.debug("  zip.poll.size.cmd=%s", _preview(size_ps))
            for _ in range(30):
                try:
                    sz = (
                        self._run_cmd(st.sid, size_ps, st.transport, self.op_id) or ""
                    ).strip()
            ...
            try:
                mtime = _parse_int(
                    self._run_cmd(st.sid, mtime_ps, st.transport, self.op_id)
                )
                length = _parse_int(
                    self._run_cmd(st.sid, size_ps, st.transport, self.op_id)
                )
            ...
            try:
                self._run_cmd(
                    st.sid, f"rm -f {_linux_shq(remote_tar)}", st.transport, self.op_id
                )
            ...
            try:
                length = _parse_int(
                    self._run_cmd(st.sid, size_sh, st.transport, self.op_id)
                )
                mtime = _parse_int(
                    self._run_cmd(st.sid, mtime_sh, st.transport, self.op_id)
                )
    ...
    def next_download_chunk(self, st: TransferState) -> Optional[int]:
        ...
        try:
            data = (
                self._linux_read_chunk(st, idx)
                if st.os_type == "linux"
                else self._windows_read_chunk(st, idx)
            )
        ...
        if idx < st.total_chunks - 1 and len(data) != st.chunk_size:
            logger.debug(
                "  short_read mid-stream: got=%d expected=%d", len(data), st.chunk_size
            )
        ...
        logger.debug(
            "  write_at: path=%s offset=%d write_len=%d (is_last=%r)",
            st.tmp_local_path,
            offset,
            len(data),
            is_last,
    ...
    def next_upload_chunk(self, st: TransferState) -> Optional[int]:
        ...
        try:
            try:
                ...
                logger.warning(
                    brightred + f"Connection Error in _remote_size: {e}" + reset
                )
                raise ConnectionError("Connection Error in _remote_size") from e
            ...
            if rsz < need:
                ...
                logger.warning(
                    brightred + f"remote short write: {rsz} < {need}" + reset
                )
            ...
            logger.warning(
                brightred + f"Exception in next upload chunk function {str(e)}" + reset
            )
            raise ConnectionError(str(e))
    ...
    def cleanup(self, st: TransferState) -> None:
        ...
        _kv(
            tid=st.tid,
            sid=st.sid,
            cleanup_cmd=st.cleanup_remote_cmd,
            archive=st.archive_remote_path,
        )
