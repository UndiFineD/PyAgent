# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\transfers\manager.py
from core.transfers.logutil import get_logger, setup_once
...
from .chunker import chunk_count, ensure_prealloc, human_bytes
...
class TransferManager:
    ...
    def _mk_state(
        ...
        st = TransferState(
            ...
            local_path=(
                local_path if direction == "download" else local_path
            ),  # local_path is source for upload
            is_folder=is_folder,
            os_type=os_type,
    ...
    def start_download(
        ...
        opts: Optional[TransferOpts] = None,
        timeout: float = None,
    ) -> str:
        ...
        if folder is None:
            try:
                folder = self._probe_remote_is_dir(
                    sid, remote_path, os_type, to_op=getattr(opts, "to_op", None)
                )
            except Exception as e:
                # Fallback heuristic (very conservative): treat as FILE unless the path ends with a slash/backslash
                self._emit(
                    opts,
                    f"[!] Could not probe remote path type ({e}); guessing from path",
                )
        ...
        if st.total_bytes is None or st.total_bytes < 0:
            ...
            self._emit(
                opts,
                f"[{st.tid}] remote path missing/unreadable; aborting",
                color=brightred,
                override_quiet=True,
            )
            return st.tid
        ...
        t = threading.Thread(
            ...
            args=(proto, st, opts, stop, timeout),
            daemon=True,
        )
        t.start()
        self._threads[st.tid] = t
        self._emit(
            opts, f"[*] Transfer started (download) TID={st.tid} → {st.local_path}"
        )
        return st.tid
    ...
    def start_upload(
        ...
        if folder:
            src_for_upload, pack_tmpdir = self._pack_folder_to_archive(
                local_path, os_type
            )
        ...
        # Build state
        st = self._mk_state(
            "upload", sid, remote_path, src_for_upload, bool(folder), opts
        )
        ...
        self._stop_flags[st.tid] = stop
        t = threading.Thread(
            target=self._run_upload, args=(proto, st, opts, stop, timeout), daemon=True
        )
        t.start()
        self._threads[st.tid] = t

        self._emit(
            opts, f"[*] Transfer started (upload) TID={st.tid} → {st.remote_path}"
        )
        return st.tid
    ...
    def _run_download(
        ...
        try:
            ...
            if st.total_bytes is None or st.total_bytes < 0:
                ...
                self._emit(
                    opts,
                    f"[{st.tid}] remote size unavailable; aborting",
                    color=brightred,
                    override_quiet=True,
                )
                return
                ...
                if st.tmp_local_path and os.path.exists(st.tmp_local_path):
                    ...
                    if tail:
                        try:
                            ...
                            logger.debug(
                                f"DL[{st.tid}] align: truncated tail={tail} -> {full_chunks * st.chunk_size} bytes"
                            )
                        except Exception:
                            logger.debug(
                                f"DL[{st.tid}] align: truncate failed (non-fatal)",
                                exc_info=True,
                            )
                    ...
                    self.store.save(st)
                    logger.debug(
                        f"DL[{st.tid}] align -> next_index={st.next_index} bytes_done={st.bytes_done}"
                    )
            except Exception:
                logger.debug(
                    f"DL[{st.tid}] align block raised (ignored)", exc_info=True
                )
            ...
            try:
                if st.next_index > 0 or (st.bytes_done and st.bytes_done > 0):
                    logger.debug(
                        f"DL[{st.tid}] safety: probing remote size; stored total_bytes={st.total_bytes}"
                    )
                    try:
                        current_total = proto._remote_size(st)
                        logger.debug(
                            f"DL[{st.tid}] safety: remote_size={current_total}"
                        )
                    except Exception:
                        st.status = "paused"
                        self.store.save(st)
                        logger.debug(
                            f"DL[{st.tid}] PAUSE: SAFETY_REMOTE_UNREACHABLE at chunk={st.next_index}",
                            exc_info=True,
                        )
                        self._emit(
                            opts,
                            f"[{st.tid}] remote not reachable; paused at chunk {st.next_index}",
                            color=brightred,
                            override_quiet=True,
                        )
                        return
                    ...
                    if st.total_bytes and current_total != st.total_bytes:
                        ...
                        self._emit(
                            opts,
                            f"[{st.tid}] remote file changed (was {st.total_bytes} bytes, now {current_total}); paused",
                            color=brightred,
                            override_quiet=True,
                        )
                        return
                ...
                logger.debug(
                    f"DL[{st.tid}] safety block raised (ignored)", exc_info=True
                )
            ...
            while not stop.is_set():
                ...
                logger.debug(
                    f"DL[{st.tid}] loop: requesting chunk idx={pre_idx}/{st.total_chunks} (bytes_done={st.bytes_done})"
                )
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"DL[{st.tid}] truncate after neterr failed (non-fatal)",
                            exc_info=True,
                        )
                    ...
                    self.store.save(st)
                    logger.debug(
                        f"DL[{st.tid}] PAUSE: NETERR_DURING_CHUNK at idx={st.next_index} ({neterr.__class__.__name__})"
                    )
                    self._emit(
                        opts,
                        f"[{st.tid}] connection lost ({neterr.__class__.__name__}); paused at chunk {st.next_index}",
                        color=brightred,
                        override_quiet=True,
                    )
                    return
                ...
                if idx is None:
                    logger.debug(
                        f"DL[{st.tid}] loop: proto returned None at idx={pre_idx} (total_chunks={st.total_chunks})"
                    )
                    break

                logger.debug(
                    f"DL[{st.tid}] chunk_ok: wrote idx={idx} -> next_index={st.next_index} bytes_done={st.bytes_done}"
                )
                ...
                if time.time() - last >= 0.5:
                    ...
                    last = time.time()
                    logger.debug(
                        f"DL[{st.tid}] progress saved: next_index={st.next_index} bytes_done={st.bytes_done}"
                    )
            ...
            if stop.is_set():
                ...
                self._emit(
                    opts,
                    f"[{st.tid}] paused at chunk {st.next_index}",
                    color=brightred,
                    override_quiet=True,
                )
                return
            ...
            def _have_all_bytes() -> bool:
                try:
                    ...
                    logger.debug(
                        f"DL[{st.tid}] completion_check: have={have} need={need}"
                    )
            ...
            logger.debug(
                f"DL[{st.tid}] finalize: moving {st.tmp_local_path!r} -> {st.local_path!r}"
            )
            ...
            if st.is_folder:
                try:
                    ...
                    if st.os_type == "windows":
                        if head != b"PK\x03\x04":
                            logger.debug(
                                f"DL[{st.tid}] failed: zip header mismatch, head={head}"
                            )
                    ...
                    self.store.save(st)
                    logger.debug(
                        f"DL[{st.tid}] error: archive header check failed: {ex}"
                    )
                    self._emit(
                        opts, f"[!] Downloaded archive invalid; left at {st.local_path}"
                    )
                    return
                ...
                if not st.options.get("defer_extract"):
                    ...
                    try:
                        if st.os_type == "windows" and st.local_path.lower().endswith(
                            ".zip"
                        ):
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"DL[{st.tid}] post-extract: remove zip failed (ignored)",
                                    exc_info=True,
                                )
                            final_msg = extract_to
                        elif st.os_type == "linux" and st.local_path.endswith(
                            ".tar.gz"
                        ):
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"DL[{st.tid}] post-extract: remove tar failed (ignored)",
                                    exc_info=True,
                                )
                            final_msg = extract_to
                    except Exception as ex:
                        logger.debug(
                            f"DL[{st.tid}] local extraction failed: {ex} (archive left)",
                            exc_info=True,
                        )
                        self._emit(
                            opts,
                            f"[!] Local extraction failed ({ex}); archive left at {st.local_path}",
                        )
                ...
                try:
                    ...
                    logger.debug(
                        f"DL[{st.tid}] cleanup: remote cleanup failed (ignored)",
                        exc_info=True,
                    )
            else:
                try:
                    ...
                    logger.debug(
                        f"DL[{st.tid}] cleanup: post-complete cleanup failed (ignored): {ce}"
                    )
            ...
            if completed:
                logger.debug(
                    f"DL[{st.tid}] network error after completion (ignored): {e.__class__.__name__}"
                )
                self._emit(
                    opts,
                    f"[{st.tid}] network error after completion: {e.__class__.__name__} (ignored)",
                )
                return
            st.status = "paused"
            self.store.save(st)
            logger.debug(
                f"DL[{st.tid}] PAUSE: OUTER_NETERR {e.__class__.__name__} at chunk={st.next_index}"
            )
            self._emit(
                opts,
                f"[{st.tid}] connection lost ({e.__class__.__name__}); paused at chunk {st.next_index}",
                color=brightred,
                override_quiet=True,
            )
            ...
            if completed:
                logger.debug(
                    f"DL[{st.tid}] post-complete exception (ignored): {e}",
                    exc_info=True,
                )
    ...
    def _run_upload(
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"UL[{st.tid}] remote_size={rsz} full_chunks={full_chunks} tail={tail}"
                )

                if tail:
                    safe_bytes = full_chunks * st.chunk_size
                    logger.debug(
                        f"UL[{st.tid}] truncating remote to {safe_bytes} to drop partial chunk"
                    )
                    try:
                        if st.os_type == "windows":
                            ps = (
                            ...
                            if tx in ("http", "https"):
                                http_exec.run_command_http(
                                    st.sid,
                                    ps,
                                    op_id=getattr(opts, "to_op", None),
                                    transfer_use=True,
                                    timeout=timeout,
                                )
                            else:
                                tcp_exec.run_command_tcp(
                                    st.sid,
                                    ps,
                                    timeout=0.5,
                        ...
                        logger.debug(
                            f"UL[{st.tid}] remote truncate failed (non-fatal)",
                            exc_info=True,
                        )
                ...
                self.store.save(st)
                logger.debug(
                    f"UL[{st.tid}] aligned: next_index={st.next_index} bytes_done={st.bytes_done}"
                )
            except Exception:
                logger.debug(
                    f"UL[{st.tid}] align block raised (ignored)", exc_info=True
                )
            ...
            while not stop.is_set():
                pre_idx = st.next_index
                logger.debug(
                    f"UL[{st.tid}] loop: sending chunk idx={pre_idx}/{st.total_chunks} (bytes_done={st.bytes_done})"
                )
                try:
                    ...
                    logger.debug(
                        f"UL[{st.tid}] neterr={neterr.__class__.__name__} -> rollback remote to {safe_bytes} bytes"
                    )
                    try:
                        if st.os_type == "windows":
                            ps = (
                        ...
                        logger.debug(
                            f"UL[{st.tid}] remote rollback failed (ignored)",
                            exc_info=True,
                        )
                    finally:
                        st.next_index = pre_idx
                    ...
                    logger.debug(
                        f"UL[{st.tid}] PAUSE: NETERR_DURING_CHUNK at idx={st.next_index}"
                    )
                    self._emit(
                        opts,
                        f"[{st.tid}] connection lost ({neterr.__class__.__name__}); paused at chunk {st.next_index}",
                ...
                if idx is None:
                    logger.debug(
                        f"UL[{st.tid}] loop: proto returned None at idx={pre_idx} (total_chunks={st.total_chunks})"
                    )
                    break

                logger.debug(
                    f"UL[{st.tid}] chunk_ok: sent idx={idx} -> next_index={st.next_index} bytes_done={st.bytes_done}"
                )
                ...
                if time.time() - last >= 0.5:
                    ...
                    last = time.time()
                    logger.debug(
                        f"UL[{st.tid}] progress saved: next_index={st.next_index} bytes_done={st.bytes_done}"
                    )
            ...
            if int(st.total_bytes or 0) == 0:
                try:
                    ...
                    logger.debug(
                        f"UL[{st.tid}] failed to create empty remote file (ignored)",
                        exc_info=True,
                    )
            ...
            if st.next_index < st.total_chunks:
                ...
                self.store.save(st)
                logger.debug(
                    f"UL[{st.tid}] PAUSE: NOT_ALL_CHUNKS next_index={st.next_index}/{st.total_chunks}"
                )
                self._emit(
                    opts,
                    f"[{st.tid}] paused at chunk {st.next_index}",
            ...
            if st.is_folder:
                ...
                dest = st.options.get("extract_dest") or self._remote_dir_of(
                    st.remote_path, st.os_type
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"UL[{st.tid}] local pack cleanup failed (ignored)",
                        exc_info=True,
                    )
            ...
            self.store.save(st)
            logger.debug(
                f"UL[{st.tid}] PAUSE: OUTER_NETERR {e.__class__.__name__} at chunk={st.next_index}"
            )
            self._emit(
                opts,
                f"[{st.tid}] connection lost ({e.__class__.__name__}); paused at chunk {st.next_index}",
    ...
    def _remote_dir_of(self, p: str, os_type: str) -> str:
        return (
            ntpath.dirname(p)
            if os_type == "windows" or ("\\" in p and "/" not in p)
    ...
    # --- Remote probing -------------------------------------------------------
    def _probe_remote_is_dir(
        self, sid: str, remote_path: str, os_type: str, to_op: Optional[str] = None
    ) -> bool:
        """
        ...
        if os_type == "windows":
            ...
            # Linux/Unix
            sh = (
                f"bash -lc 'if [ -d {_linux_shq(remote_path)} ]; then echo DIR; "
    ...
    def _run_remote(
        ...
        if transport in ("http", "https"):
            ...
            return http_exec.run_command_http(
                sid, cmd, op_id=to_op, transfer_use=True, timeout=timeout
            )

        else:
            # TCP/TLS paths use TCP adapter
            return tcp_exec.run_command_tcp(
                sid,
                cmd,
                timeout=0.5,
                portscan_active=True,
