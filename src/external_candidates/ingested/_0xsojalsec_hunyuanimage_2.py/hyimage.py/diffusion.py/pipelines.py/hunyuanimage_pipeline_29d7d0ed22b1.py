# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanImage-2.1\hyimage\diffusion\pipelines\hunyuanimage_pipeline.py
import copy
import os
import re
...
class HunyuanImagePipelineConfig:
    ...
    enable_stage1_offloading: bool = (
        True  # offload models in stage1 pipeline when reprompt or refiner is working
    )
    enable_reprompt_model_offloading: bool = (
        True  # offload reprompt model after finishing
    )
    ...
    enable_full_dit_offloading: bool = (
        True  # offload during text encoding and latent decoding
    )
    ...
    def create_default(
        ...
        reprompt_model="hunyuanimage-reprompt-32b",
        **kwargs,
    ):
        """
        ...
        if version == "v2.1":
            ...
            return cls(
                ...
                reprompt_config=(
                    HUNYUANIMAGE_REPROMPT_32B()
                    if reprompt_model == "hunyuanimage-reprompt-32b"
                    else HUNYUANIMAGE_REPROMPT()
                ),
            ...
            raise ValueError(
                f"Unsupported HunyuanImage version: {version}. Only 'v2.1' is supported"
            )
...
class HunyuanImagePipeline:
    ...
    def __init__(self, config: HunyuanImagePipelineConfig, **kwargs):
        """
        ...
        if self.cfg_mode == "APG_mode_0":
            self.cfg_guider = AdaptiveProjectedGuidance(
                guidance_scale=10.0,
                eta=0.0,
            ...
            self.cfg_guider_ocr = AdaptiveProjectedGuidance(
                guidance_scale=10.0,
                eta=0.0,
                adaptive_projected_guidance_rescale=10.0,
                adaptive_projected_guidance_momentum=-0.5,
            ...
            self.cfg_guider_general = AdaptiveProjectedGuidance(
                guidance_scale=10.0,
                eta=0.0,
                adaptive_projected_guidance_rescale=10.0,
                adaptive_projected_guidance_momentum=-0.5,
    ...
    def _load_dit(self):
        ...
        try:
            ...
            self.dit = instantiate(
                dit_config.model, dtype=self.torch_dtype, device=dit_device
            )
            if self.config.use_fp8:
                ...
                if not Path(dit_config.fp8_scale).exists():
                    raise FileNotFoundError(
                        f"FP8 scale file not found: {dit_config.fp8_scale}. Please download from https://huggingface.co/tencent/HunyuanImage-2.1/"
                    )
                if (
                    dit_config.fp8_load_from is not None
                    and Path(dit_config.fp8_load_from).exists()
                ):
                    convert_fp8_linear(self.dit, dit_config.fp8_scale)
                    load_hunyuan_dit_state_dict(
                        self.dit, dit_config.fp8_load_from, strict=True
                    )
                else:
                    raise FileNotFoundError(
                        f"FP8 ckpt not found: {dit_config.fp8_load_from}. Please download from https://huggingface.co/tencent/HunyuanImage-2.1/"
                    )
                    load_hunyuan_dit_state_dict(
                        self.dit, dit_config.load_from, strict=True
                    )
                    convert_fp8_linear(self.dit, dit_config.fp8_scale)
    ...
    def _load_reprompt_model(self):
        try:
            ...
            self._reprompt_model = instantiate(
                reprompt_config.model,
                models_root_path=reprompt_config.load_from,
                enable_offloading=self.config.enable_reprompt_model_offloading,
            )
    ...
    def refiner_pipeline(self):
        ...
        self._refiner_pipeline = HunYuanImageRefinerPipeline.from_pretrained(
            self.config.refiner_model_name, use_fp8=self.config.use_fp8
        )
        return self._refiner_pipeline
    ...
    def _load_byt5(self):
        ...
        try:
            ...
            glyph_root = os.path.join(
                self.config.text_encoder_config.load_from, "Glyph-SDXL-v2"
            )
    ...
    def _encode_glyph(self, prompt: str):
        ...
        if not prompt:
            return (
                torch.zeros(
                    (1, self.byt5_kwargs["byt5_max_length"], 1472),
                    device=self.execution_device,
                ),
                torch.zeros(
                    (1, self.byt5_kwargs["byt5_max_length"]),
                    device=self.execution_device,
                    dtype=torch.int64,
                ),
            )
        ...
        text_prompt_style_list = [
            {"color": None, "font-family": None} for _ in range(len(text_prompt_texts))
        ]
        glyph_text_formatted = self.prompt_format.format_prompt(
            text_prompt_texts, text_prompt_style_list
        )
    ...
    def _prepare_latents(
        ...
        batch_size: int = 1,
        vae_downsampling_factor: int = 32,
    ):
        """
        ...
        """
        assert (
            width % vae_downsampling_factor == 0
            and height % vae_downsampling_factor == 0
    ...
    def _denoise_step(
        ...
        if byt5_emb is not None and byt5_mask is not None:
            ...
            if self.use_byt5:
                raise ValueError(
                    "Must provide byt5_emb and byt5_mask for HunyuanImage 2.1"
                )
            extra_kwargs = {}
    ...
    def _apply_classifier_free_guidance(
        self, noise_pred, guidance_scale: float, i: int
    ):
        """
        ...
        if self.cfg_mode.startswith("APG_mode_"):
            ...
            noise_pred_text_true = (
                noise_pred_text[true_idx]
                if len(true_idx) > 0
                else torch.empty(
                    (0, noise_pred_text.size(1)),
                    dtype=noise_pred_text.dtype,
                    device=noise_pred_text.device,
                )
            )
            noise_pred_text_false = (
                noise_pred_text[false_idx]
                if len(false_idx) > 0
                else torch.empty(
                    (0, noise_pred_text.size(1)),
                    dtype=noise_pred_text.dtype,
            ...
            noise_pred_uncond_true = (
                noise_pred_uncond[true_idx]
                if len(true_idx) > 0
                else torch.empty(
                    (0, noise_pred_uncond.size(1)),
                    dtype=noise_pred_uncond.dtype,
                    device=noise_pred_uncond.device,
                )
            )
            noise_pred_uncond_false = (
                noise_pred_uncond[false_idx]
                if len(false_idx) > 0
                else torch.empty(
                    (0, noise_pred_uncond.size(1)),
                    dtype=noise_pred_uncond.dtype,
            ...
            if len(noise_pred_text_true) > 0:
                if i <= self.apg_start_step_ocr:
                    ...
                    _ = self.cfg_guider_ocr(
                        noise_pred_text_true, noise_pred_uncond_true, step=i
                    )
                else:
                    noise_pred_true = self.cfg_guider_ocr(
                        noise_pred_text_true, noise_pred_uncond_true, step=i
                    )
            else:
            ...
            if len(noise_pred_text_false) > 0:
                if i <= self.apg_start_step_general:
                    ...
                    _ = self.cfg_guider_general(
                        noise_pred_text_false, noise_pred_uncond_false, step=i
                    )
                else:
                    noise_pred_false = self.cfg_guider_general(
                        noise_pred_text_false, noise_pred_uncond_false, step=i
                    )
            else:
    ...
    def get_timesteps_sigmas(self, sampling_steps: int, shift):
        ...
        timesteps = (sigmas[:-1] * 1000).to(
            dtype=torch.float32, device=self.execution_device
        )
        return timesteps, sigmas

    def step(self, latents, noise_pred, sigmas, step_i):
        return (
            latents.float() - (sigmas[step_i] - sigmas[step_i + 1]) * noise_pred.float()
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not self.cfg_distilled:
            print(
                f"Negative Prompt:  {negative_prompt if negative_prompt else '(none)'}"
            )
        ...
        print(
            f"Before refiner Peak memory requirement: {peak_bytes_requirement / 1024 ** 3:.2f} GB"
        )
    ...
    def to(self, device: str | torch.device):
        ...
        self.device = device
        if (
            not self.config.enable_full_dit_offloading
            and not self.config.enable_stage1_offloading
        ):
    ...
    def from_pretrained(
        cls,
        model_name: str = "hunyuanimage-v2.1",
        use_distilled: bool = False,
        reprompt_model: str = "hunyuanimage-reprompt-32b",
        **kwargs,
    ):
        """
...
def DiffusionPipeline(
    model_name: str = "hunyuanimage-v2.1", use_distilled: bool = False, **kwargs
):
    """
    ...
    Returns:
        HunyuanImagePipeline instance
    """
    return HunyuanImagePipeline.from_pretrained(
        model_name, use_distilled=use_distilled, **kwargs
    )
