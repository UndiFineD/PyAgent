# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanImage-2.1\hyimage\diffusion\cfg_utils.py
import math
...
def normalized_guidance_apg(
    ...
    diff_parallel, diff_orthogonal = v0_parallel.type_as(diff), v0_orthogonal.type_as(
        diff
    )
    ...
    def __call__(
        ...
        if step == 0 and self.adaptive_projected_guidance_momentum is not None:
            self.momentum_buffer = MomentumBuffer(
                self.adaptive_projected_guidance_momentum
            )
