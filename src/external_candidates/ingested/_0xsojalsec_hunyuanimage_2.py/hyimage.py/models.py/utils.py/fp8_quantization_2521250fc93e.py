# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanImage-2.1\hyimage\models\utils\fp8_quantization.py
import copy
import gc
import os
                ...
                if "embed" not in name:
                    if fp8_map_path is None:
                        if linear.weight.dtype != torch.float8_e4m3fn:
                            quant_weight, weight_scale = per_tensor_quantize(
                                linear.weight
                            )
                            fp8_map[name] = weight_scale
                        else:
                            raise ValueError(
                                f"Invalid weight dtype: {linear.weight.dtype}"
                            )
                    ...
                    bias = (
                        copy.deepcopy(linear.bias) if linear.bias is not None else None
                    )
