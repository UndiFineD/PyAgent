# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanImage-2.1\hyimage\models\text_encoder\__init__.py
...
def load_text_encoder(
    ...
    if text_encoder_type == "llm":
        text_encoder = AutoModelForVision2Seq.from_pretrained(
            text_encoder_path, torch_dtype="auto"
        )
    else:
...
class TextEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.prompt_template = (
            dict(prompt_template) if prompt_template is not None else None
        )
        self.prompt_template_video = (
            dict(prompt_template_video) if prompt_template_video is not None else None
        )
        ...
        if infer_mode == "decoder":
            ...
            assert (
                self.prompt_template is not None and hidden_state_skip_layer is not None
            ), (
        ...
        if "t5" in text_encoder_type:
            ...
            self.output_key = output_key or (
                "last_hidden_state" if infer_mode == "encoder" else None
            )
        else:
            raise ValueError(f"Unsupported text encoder type: {text_encoder_type}")
    ...
    def encode(
        ...
        if self.infer_mode == "encoder":
            attention_mask = (
                batch_encoding["attention_mask"].to(self.model.device)
            ...
            input_max_valid_tokens = (
                batch_encoding["attention_mask"].sum(dim=1).max().item()
            )
            ...
            attention_mask = (
                outputs.sequences[:, -output_max_valid_tokens - 1 : -1]
            ...
            if return_texts:
                ...
                return TextEncoderModelOutput(
                    hidden_states, attention_mask, None, text_outputs
                )
            else:
