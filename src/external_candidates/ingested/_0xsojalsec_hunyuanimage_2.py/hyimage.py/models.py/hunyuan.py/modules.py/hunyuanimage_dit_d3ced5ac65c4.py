# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanImage-2.1\hyimage\models\hunyuan\modules\hunyuanimage_dit.py
import os
...
def load_hunyuan_dit_state_dict(
    model, dit_model_name_or_path, strict=True, assign=False
):
    """
    ...
    def forward(
        ...
        *,
        ...
        if len(input_shape) == 5:
            ...
            if freqs_cos is None or freqs_sin is None:
                assert (
                    freqs_cos is None and freqs_sin is None
        ...
        if self.use_meanflow:
            assert (
                self.time_r_in is not None
            ), "use_meanflow is True but time_r_in is None"
        if timesteps_r is not None:
            assert (
                self.time_r_in is not None
            ), "timesteps_r is not None but time_r_in is None"
