...
def archive_old_output_files(
    base_dir_str: str, log_callback: Callable[[str, str], None]
):
    """
    ...
    try:
        ...
        for file_path in output_files:
            try:
                ...
                archive_name = (
                    f"{base_name}_{timestamp_str}_{random_num}{file_path.suffix}"
                )
                ...
                log_callback(
                    f"  Archived {file_path.name} to {archive_name}", "DEBUG"
                )  # Debug level
                archived_count += 1
            except OSError as e:
                ...
                log_callback(
                    f"Unexpected error archiving {file_path.name}: {e}", "ERROR"
                )
...
def load_channels(
    channellist_file: str, log_callback: Callable[[str, str], None]
) -> List[str]:
    """
    ...
    try:
        with file_path.open("r", encoding="utf-8") as infile:
            for line_num, line in enumerate(infile, 1):
                ...
                if channel_name and re.match(
                    r"^[a-zA-Z][a-zA-Z0-9_]{4,}$", channel_name
                ):
        ...
        log_callback(
            f"Error reading channel list file {channellist_file}: {e}", "ERROR"
        )
        raise RuntimeError(f"Error reading channel list file: {e}") from e
    except Exception as e:
        log_callback(
            f"Unexpected error loading channels from {channellist_file}: {e}", "ERROR"
        )
