...
class GuiEventHandlers:
    ...
    def process_log_queue(self):
        ...
        try:
            while not self.app.log_queue.empty():
                ...
                if (
                    self.app.master
                    and hasattr(self.app, "log_text")
                    and self.app.log_text
                ):
    ...
    def open_file_dialog(self):
        ...
        self.log_message(
            "Browse button clicked (feature deprecated, use dropdown).", "WARN"
        )
        messagebox.showinfo(
            "Info", "Channel list selection is now done via the dropdown menu."
        )
    ...
    def validate_date_spinbox(self, prefix: str):
        ...
        try:
            if prefix == "sel":
                ...
                self.log_message(
                    f"Invalid prefix '{prefix}' for date validation.", "WARN"
                )
                return
            ...
            if 1 <= month <= 12:
                ...
                if (
                    day_spinbox
                    and getattr(day_spinbox, "winfo_exists", lambda: False)()
                ):  # Safer check
    ...
    def _parse_date_or_show_error(
        self, year_var, month_var, day_var, date_description: str
    ) -> Optional[date]:
    ...
    def _get_dates_for_mode(
        self, mode: str
    ) -> Optional[Tuple[Optional[date], Optional[date], Optional[date]]]:
    ...
    def start_scraping_base(self, mode: str):
        ...
        if self.app.scraping_thread and self.app.scraping_thread.is_alive():
            messagebox.showwarning(
                "Process Running", "A scraping process is already active."
            )
            return
        ...
        channellist_file = str(
            channelslists_dir / selected_filename
        )  # Convert Path to string for os.path.exists
    ...
    def stop_scraping(self):
        ...
        if self.app.scraping_thread and self.app.scraping_thread.is_alive():
            ...
            if hasattr(self.app, "stop_button") and self.app.stop_button:
                self.app.stop_button.configure(
                    state="disabled"
                )  # Use configure for ctk
        else:
            self.log_message("No active scraping process to stop.", "INFO")

    def scrape_in_thread(
        self, channellist_file, mode, target_date, start_date, end_date
    ):
        ...
        try:
            ...
            if self.app.stop_event.is_set():
                ...
                final_message = self._generate_no_posts_message(
                    mode, final_date_to_show, start_date_for_msg
                )
    ...
    def _generate_no_posts_message(
        self, mode: str, target_date: Optional[date], start_date: Optional[date]
    ) -> str:
        # (Keep this method as is)
        date_info = ""
    ...
    def _set_button_state(
        self, button_name: str, state: str
    ):  # Use string state for ctk
        ...
        if isinstance(button_widget, (ctk.CTkButton)):
            ...
            if button_widget:
                try:
                    ...
                    self.log_message(
                        f"Could not configure button '{button_name}': {e}", "WARN"
                    )
                    pass
        elif isinstance(button_widget, (ttk.Spinbox)):  # Handle spinboxes if needed
            if (
                button_widget
                and getattr(button_widget, "winfo_exists", lambda: False)()
            ):
                try:
