...
class TelegramScraperGUI:
    ...
    def __init__(self, master: ctk.CTk, base_dir: str):
        ...
        self.master.title(
            "Telegram Post Downloader v3.2 (Grid Layout)"
        ...
        self.master.grid_columnconfigure(
            1, weight=0
        )  # Sidebar doesn't expand horizontally
        ...
        self.main_content_frame = ctk.CTkFrame(
            master, corner_radius=0, fg_color="transparent"
        )
        ...
        self.main_content_frame.grid_rowconfigure(
            5, weight=1
        )  # Allow log frame (index 5 if 5 packed above) to expand
    ...
    def _populate_channel_list_dropdown(self):
        ...
        if channelslists_dir.is_dir():
            try:
                ...
                if channel_files:
                    default_selection = channel_files[
                        0
                    ]  # Default to the first file found
                    # Configure the dropdown
                    self.channel_list_dropdown.configure(
                        values=channel_files, state="readonly"
                    )  # Use readonly state
                    self.log_message(
                        f"Found channel lists: {', '.join(channel_files)}", "DEBUG"
                    )
                else:
                    self.log_message(
                        f"No .txt files found in {channelslists_dir}", "WARN"
                    )
                    self.channel_list_dropdown.configure(
                        values=[default_selection], state="disabled"
                    )
                ...
                self.channel_list_dropdown.configure(
                    values=[f"Error reading lists"], state="disabled"
                )
                default_selection = "Error reading lists"
            except Exception as e:
                self.log_message(
                    f"Unexpected error scanning for channel lists: {e}", "ERROR"
                )
                self.channel_list_dropdown.configure(
                    values=[f"Error scanning lists"], state="disabled"
                )
                default_selection = "Error scanning lists"
        else:
            self.log_message(
                f"Channel list directory not found: {channelslists_dir}", "WARN"
            )
            self.channel_list_dropdown.configure(
                values=[default_selection], state="disabled"
            )
