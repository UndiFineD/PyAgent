...
try:
    ...
    raise ImportError(
        "Could not import from 'my_telegram_scrapper'. Is it installed or in PYTHONPATH?"
    ) from e
...
def _determine_date_range(
    ...
    if mode in ["today", "yesterday", "specific_date"]:
        ...
        if start_date < CUTOFF_DATE:
            log_date_info += (
                f" (effective start: {effective_start_date.strftime('%Y-%m-%d')})"
            )
    elif mode == "all":
...
def _write_post_to_file(handle: TextIO, channel: str, post: SimpleTgPost):
    ...
    try:
        ...
        post_time_str = (
            post.timestamp.strftime("%H:%M:%S") if post.timestamp else "[No Time]"
        )
...
def _get_output_file_handle(
    ...
    try:
        ...
        if file_path not in output_files_created:
            ...
            log_callback(
                f"Opened output file: {file_path.name}", "DEBUG"
            )  # Debug level log
        return handle
    except OSError as e:
        log_callback(
            f"Failed to open/write header to output file {file_path}: {e}", "ERROR"
        )
        return None
...
def _process_scraped_post(
    ...
    if mode == "all":
        ...
        is_within_target_range = (
            effective_start_date <= current_post_date <= effective_end_date
        )
    ...
    if mode == "all" or mode == "date_range":
        ...
        all_posts_for_specific_date.append(
            (channel, post)
        )  # Store channel name with post
        processed = True
...
def _scrape_single_channel(
    ...
    while not stop_channel_pagination:
        if stop_event.is_set():
            log_callback(
                f"Stop signal received, interrupting channel {channel}.", "WARN"
            )
        ...
        for post in posts_on_page:
            if post.timestamp:
                ...
                if (
                    oldest_post_date_this_page is None
                    or current_post_date < oldest_post_date_this_page
                ):
        ...
        last_oldest_date_on_page = (
            oldest_post_date_this_page  # Store for pagination logic
        )
        ...
        if (
            pages_checked > 500
        ):  # Arbitrary limit to prevent infinite loops on weird pages
            log_callback(
...
def scrape_channels(
    ...
    if (
        not channels
    ):  # load_channels should raise error if file empty/not found, but double-check
    ...
    try:
        ...
        with SimpleScraperClient() as client:
            ...
            for i, channel in enumerate(channels):
                if stop_event.is_set():
                    log_callback(
                        "Stop signal received. Aborting channel processing.", "WARN"
                    )
                    break  # Break outer loop (channel iteration)

                log_callback(
                    f"--- Channel {i+1}/{len(channels)}: {channel} ---", "INFO"
                )
        ...
        raise RuntimeError(
            f"Scraping failed due to an unexpected error: {client_error}"
        ) from client_error
    finally:
        ...
        if open_files:
            ...
            for date_str, handle in open_files.items():
                try:
                    ...
                    log_callback(
                        f"Error closing file for date {date_str}: {close_e}", "ERROR"
                    )
    ...
    if mode in ["today", "yesterday", "specific_date"] and all_posts_for_specific_date:
        if target_date is None:
            ...
            log_callback(
                "Cannot write single-date file: Target date is missing.", "ERROR"
            )
        else:
            output_file_path = (
                output_dir
                / f"output_{base_list_name}_{target_date.strftime('%Y-%m-%d')}.txt"
            )
            log_callback(
            ...
            try:
                ...
                all_posts_for_specific_date.sort(
                    ...
                    or datetime.min  # Sort by post timestamp
                )
                # Use 'w' mode (write/overwrite) for single-date files
                with open(output_file_path, "w", encoding="utf-8") as outfile:
                    outfile.write(
                        f"### Posts from {target_date.strftime('%Y-%m-%d')} (List: {base_list_name})\n\n"
                    )
    ...
    if stop_event.is_set():
        ...
        total_processed_posts == 0
    ):  # Check if *any* posts matching criteria were found across all channels
        log_callback(
...
def run_scraping(
    ...
    try:
        ...
        archive_old_output_files(
            str(base_dir_path), log_callback
        )  # Pass base_dir as string if util expects it
        ...
        log_callback(
            f"An unexpected critical error occurred in run_scraping: {e}", "ERROR"
        )
