...
def _safe_find_text(
    element: Optional[Tag], selector: str, strip: bool = True
) -> Optional[str]:
...
def _safe_get_attr(
    element: Optional[Tag], selector: str, attribute: str
) -> Optional[str]:
...
def parse_single_post(post_element: Tag) -> Optional[SimpleTgPost]:
    ...
    if not widget_message:
        print(
            "Warning: Could not find main message container ('.tgme_widget_message') in post element."
        )
    ...
    if author_link_tag:
        ...
        author_name_span = author_link_tag.select_one(
            "span"
        )  # or "span.name" if specific
        post.author.display_name = (
            author_name_span.get_text(strip=True)
            if author_name_span
            else author_link_tag.get_text(strip=True)
        )
    else:
        # Fallback for potentially different structures (e.g., forwarded messages might differ)
        author_user_tag = widget_message.select_one(
            ".tgme_widget_message_from_author"
        )  # Check for forwarded author
        if author_user_tag:
    ...
    if time_tag and time_tag.get("datetime"):
        try:
            ...
            print(
                f"Warning: Could not parse timestamp datetime: {time_tag.get('datetime')}"
            )
...
def parse_page(html_content: str) -> ScrapedPage:
    ...
    if load_more_link:
        ...
        if match:
            ...
            print(
                "Warning: Found 'Load More' link but could not extract 'before' token."
            )
