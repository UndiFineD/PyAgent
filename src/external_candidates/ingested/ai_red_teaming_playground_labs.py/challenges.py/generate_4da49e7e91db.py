# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\challenges\generate.py
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate required files for the docker based on the challenge configuration."
    )
    parser.add_argument(
        "challenges_json", type=str, help="Path to the challenges.json file"
    )
    parser.add_argument(
        "output_dir",
        type=str,
        nargs="?",
        default=".",
        help="Output directory for generated files. By default it is the docker/data directory.",
    )
    ...
    docker_chat_copilot_original = copy.deepcopy(
        compose_yaml["services"]["chat-copilot"]
    )
    ...
    if "Challenges" in challenges_json:
        for challenge in challenges_json["Challenges"]:
            ...
            appsettings_path = os.path.join(
                output_dir, challenge_name, "appsettings.json"
            )
