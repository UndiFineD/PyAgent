# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\src\chat-score\webapi\server\controller\conversation.py
...
from server.dtos import (
    EVENT_CLIENT_REVIEW_DONE,
    EVENT_CLIENT_REVIEW_UPDATE,
    EVENT_CLIENT_STATUS_UPDATE,
    ROOM_DEFAULT_BROADCAST,
...
class ConversationController:
    ...
    def pick(self):
        ...
        if socket_id is not None:
            ...
            if conversation_id is not None:
                ...
                self.__logger.info(
                    "Assigned conversation %s to connection %s",
                    conversation_id,
                    socket_id,
                )

                # We send an update to the connection with the details of the conversation
                conversation = self.__conversation_model.get_conversation(
                    conversation_id
                )
                self.__socket.emit(
                    EVENT_CLIENT_REVIEW_UPDATE,
                    conversation.to_response().to_json(),
                    to=socket_id,
                )
            else:
    ...
    def new_conversation(self, conversation: ConversationReviewRequest) -> bool:
        ...
        """
        if (
            self.__conversation_model.get_conversation(conversation.conversation_id)
            is not None
        ):
            self.__logger.error(
                "Conversation %s already exists", conversation.conversation_id
            )
            return False
    ...
    def score(self, score_request: ScoreConversationRequest, socket_id: str):
        ...
        if conversation is None:
            self.__logger.error(
                "Conversation %s not found", score_request.conversation_id
            )
            return
        ...
        if assignement != score_request.conversation_id:
            self.__logger.error(
                "Conversation %s is not assigned to %s",
                score_request.conversation_id,
                socket_id,
            )
            return
        ...
        self.__logger.info(
            "Scored conversation %s with %s",
            score_request.conversation_id,
            score_request.passed,
        )
        ...
        response = requests.post(
            ...
            headers={"x-scoring-key": os.environ.get(ENV_NAME_SCORING_KEY)},
        )
    ...
    def dead_reviews(self):
        ...
        if len(socket_ids) > 0:
            for socket_id in socket_ids:
                self.__socket.emit(
                    EVENT_CLIENT_REVIEW_DONE, {"status": "expired"}, to=socket_id
                )
    ...
    def __send_update(self):
        ...
        current_status = CurrentStatusResponse(
            session_count=session_count, conversation_queue=queue
        )
        self.__logger.info(
            "Sending status update to all clients, %s", current_status.to_json()
        )
        self.__socket.emit(
            EVENT_CLIENT_STATUS_UPDATE,
            current_status.to_json(),
            to=ROOM_DEFAULT_BROADCAST,
        )
