# Extracted from: C:\DEV\PyAgent\.external\AI-Red-Teaming-Playground-Labs\src\chat-score\webapi\app.py
...
from server.dtos import (
    ...
    EVENT_CLIENT_SERVER_ERROR,
    EVENT_HEARTBEAT,
    EVENT_SCORE_CONVERSATION,
    ConversationReviewRequest,
    ScoreConversationRequest,
    ServerErrorResponse,
)
from server.environ import (
    ENV_NAME_REDIS,
    ENV_NAME_SCORING_KEY,
    ENV_NAME_SECRET_KEY,
    REDIS_URL,
)
...
connection_controller = ConnectionController(
    connection_model, conversation_model, socketio
)
conversation_controller = ConversationController(
    conversation_model, connection_model, socketio
)
...
def score_chat():
    ...
    if "conversation" not in data and "picture" not in data:
        app.logger.warning(
            "Scoring request denied due to missing field: conversation or picture"
        )
        abort(400)

    if "conversation" in data and "picture" in data:
        app.logger.warning(
            "Scoring request denied due to both fields: conversation and picture"
        )
        abort(400)
