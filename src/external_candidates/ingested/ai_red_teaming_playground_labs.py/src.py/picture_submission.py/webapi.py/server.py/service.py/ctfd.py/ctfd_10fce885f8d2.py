...
class CtfdService:
    def __init__(self, app, submission_model: SubmissionModel):
        ...
        self.__ctfd_settings = self.__app.config[CONFIG_AUTH_SETTINGS][
            CONFIG_CTFD_SETTINGS
        ]
        self.__signer = Signer(
            self.__ctfd_settings[CONFIG_CTFD_SECRET_KEY], salt=CTFD_COOKIE_SALT
        )
        ...
        self.__challenge_id = self.__app.config[CONFIG_CHALLENGE_SETTINGS][
            CONFIG_CHALLENGE_ID
        ]

    def validate_auth(
        self, request: Request
    ) -> tuple[bool, Optional[CtfdAuthTicket], Optional[Response]]:
        def fail_response(reason: str) -> Response:
            self.__logger.error(f"CTFd Auth failed: {reason}")
            response = AuthErrorResponse(
                "ctfd", reason, self.__ctfd_settings[CONFIG_CTFD_REDIRECT_URL]
            )
    ...
    def send_flag(self, user_id: str, flag: str) -> True:
        ...
        try:
            ...
            if "success" not in response:
                self.__logger.error(
                    f"Invalid response from CTFd url:{url} | {response}"
                )
                return False
            ...
            if (
                ...
                or "status" not in response["data"]
                or response["data"]["status"] != "correct"
            ):
