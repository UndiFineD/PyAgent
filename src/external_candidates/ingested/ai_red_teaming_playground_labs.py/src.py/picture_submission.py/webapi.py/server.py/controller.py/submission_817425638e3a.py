...
class SubmissionController:
    ...
    def get_status(self, user_id: str) -> Response:
        ...
        if status is None:
            ...
            response = SubmissionStatusResponse(
                str(null_guid), ReviewStatus.READY, None
            )
        else:
    ...
    def upload_picture(self, ticket: CtfdAuthTicket) -> Response:
        ...
        self.__logging.info(
            f"Uploaded picture with length: {len(picture)} bytes with a MIME type: {mime}"
        )

        user_id = ticket.id
        self.__submission_model.set_picture(user_id, picture, mime)
        submission = SubmissionStatus(
            str(uuid.uuid4()), ReviewStatus.REVIEWING, None, ticket
        )
