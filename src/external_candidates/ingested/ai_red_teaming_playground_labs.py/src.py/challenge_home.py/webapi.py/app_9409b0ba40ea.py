...
app.config["OPEN_LINK_NEW_TAB"] = (
    os.environ.get("OPEN_LINK_NEW_TAB", "false").lower() == "true"
)
...
def login():
    ...
    response.set_cookie(
        "session", token, httponly=True, max_age=app.config["EXPIRATION"]
    )
    if app.config["OPEN_LINK_NEW_TAB"]:
        response.set_cookie(
            "home-new-tab", "true", httponly=False, max_age=app.config["EXPIRATION"]
        )
