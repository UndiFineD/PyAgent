# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-InfiniteYou\app.py
...
def prepare_pipeline(model_version, enable_realism, enable_anti_blur):
    ...
    if enable_realism:
        loras.append(
            [
                "./models/InfiniteYou/supports/optional_loras/flux_realism_lora.safetensors",
                "realism",
                1.0,
            ]
        )
    if enable_anti_blur:
        loras.append(
            [
                "./models/InfiniteYou/supports/optional_loras/flux_anti_blur_lora.safetensors",
                "anti_blur",
                1.0,
            ]
        )
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            ui_prompt_text = gr.Textbox(
                label="Prompt", value="Portrait, 4K, high quality, cinematic"
            )
            ...
            with gr.Accordion("Advanced", open=False):
                ...
                ui_guidance_scale = gr.Number(
                    label="guidance scale", value=3.5, step=0.5
                )
                ui_infusenet_conditioning_scale = gr.Slider(
                    minimum=0.0,
                    maximum=1.0,
                    value=1.0,
                    step=0.05,
                ...
                with gr.Row():
                    ui_infusenet_guidance_start = gr.Slider(
                        minimum=0.0,
                        maximum=1.0,
                        value=0.0,
                        step=0.05,
                    ...
                    ui_infusenet_guidance_end = gr.Slider(
                        minimum=0.0,
                        maximum=1.0,
                        value=1.0,
                        step=0.05,
            ...
            with gr.Accordion("LoRAs [Optional]", open=True):
                with gr.Row():
                    ui_enable_realism = gr.Checkbox(
                        label="Enable realism LoRA", value=ENABLE_REALISM_DEFAULT
                    )
                    ui_enable_anti_blur = gr.Checkbox(
                        label="Enable anti-blur LoRA", value=ENABLE_ANTI_BLUR_DEFAULT
                    )

        with gr.Column(scale=2):
            image_output = gr.Image(
                label="Generated Image", interactive=False, height=550, format="png"
            )
            gr.Markdown("""
...
prepare_pipeline(
    ...
    enable_realism=ENABLE_REALISM_DEFAULT,
    enable_anti_blur=ENABLE_ANTI_BLUR_DEFAULT,
)
