# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-InfiniteYou\pipelines\pipeline_infu_flux.py
...
def extract_arcface_bgr_embedding(
    in_image, landmark, arcface_model=None, in_settings=None
):
    kps = landmark
    arc_face_image = face_align.norm_crop(
        in_image, landmark=np.array(kps), image_size=112
    )
    arc_face_image = (
...
class InfUFluxPipeline:
    def __init__(
        ...
        try:
            ...
            print(
                "No InfiniteYou model found. Downloading from HuggingFace `ByteDance/InfiniteYou` to `./models/InfiniteYou` ..."
            )
            snapshot_download(
            ...
            infu_model_path = os.path.join(
                "./models/InfiniteYou", f"infu_flux_{infu_flux_version}", model_version
            )
            infusenet_path = os.path.join(infu_model_path, "InfuseNetModel")
            self.infusenet = FluxControlNetModel.from_pretrained(
                infusenet_path, torch_dtype=torch.bfloat16
        ...
        ipm_state_dict = torch.load(
            image_proj_model_path, map_location="cpu", weights_only=True
        )
    ...
    def __call__(
        ...
        id_embed = extract_arcface_bgr_embedding(
            id_image_cv2, landmark, self.arcface_model
        )
        ...
        if control_image is not None:
            ...
            face_info = self._detect_face(
                cv2.cvtColor(np.array(control_image), cv2.COLOR_RGB2BGR)
            )
            if len(face_info) == 0:
