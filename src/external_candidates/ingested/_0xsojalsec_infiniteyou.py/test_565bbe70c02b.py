# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-InfiniteYou\test.py
...
def main():
    ...
    parser.add_argument(
        "--id_image", default="./assets/examples/man.jpg", help="""input ID image"""
    )
    parser.add_argument(
        "--control_image", default=None, help="""control image [optional]"""
    )
    parser.add_argument(
        "--out_results_dir", default="./results", help="""output folder"""
    )
    ...
    # Check arguments
    assert (
        args.infu_flux_version == "v1.0"
    ), "Currently only supports InfiniteYou-FLUX v1.0"
    assert args.model_version in [
        "aes_stage2",
        "sim_stage1",
    ...
    infu_model_path = os.path.join(
        args.model_dir, f"infu_flux_{args.infu_flux_version}", args.model_version
    )
    ...
    if args.enable_anti_blur_lora:
        loras.append(
            [
                os.path.join(lora_dir, "flux_anti_blur_lora.safetensors"),
                "anti_blur",
                1.0,
            ]
        )
