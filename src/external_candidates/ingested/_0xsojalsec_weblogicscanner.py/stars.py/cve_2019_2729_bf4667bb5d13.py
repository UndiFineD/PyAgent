# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-weblogicScanner\stars\cve_2019_2729.py
#!/usr/bin/env python3
...
class CVE_2019_2729(Star):
    ...
    def light_up(self, dip, dport, force_ssl=None, *args, **kwargs) -> (bool, dict):
        ...
        time.sleep(1)
        result2, data = http(
            urljoin(f"http://{dip}:{dport}", path2),
            "POST",
            headers,
            data=payload2,
            timeout=3,
        ...
        if (result1 and result1.status_code == 200 and "uid" in result1.text) or (
            result2
            and result2.status_code == 202
            and result_ico
            and "Vulnerable" in result_ico.text
        ):
