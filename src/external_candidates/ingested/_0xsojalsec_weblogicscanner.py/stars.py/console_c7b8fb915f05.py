# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-weblogicScanner\stars\console.py
#!/usr/bin/env python3
...
class WeblogicConsole(Star):
    info = {"NAME": "Weblogic Console", "CVE": None, "TAG": []}
    type = target_type.MODULE

    def light_up(
        self, dip, dport, force_ssl=None, path="console", *args, **kwargs
    ) -> (bool, dict):
        r, data = http(
            "http://{}:{}/{}/login/LoginForm.jsp".format(dip, dport, path),
            ssl=force_ssl,
        )
