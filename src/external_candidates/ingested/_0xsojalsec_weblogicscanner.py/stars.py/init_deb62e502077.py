# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-weblogicScanner\stars\__init__.py
import logging
import traceback
...
class Star:
    ...
    def light_and_msg(self, dip, dport, force_ssl=None, *arg, **kwargs):
        ...
        try:
            ...
            self.print_msg(
                ...
                {
                    "more_detail": [
                        """The following information output is only used for error tracking, so don't panic""",
                        "以下信息输出仅为错误追踪使用，请勿担心",
    ...
    def light_up(
        self, dip, dport, force_ssl=None, *arg, **kwargs
    ) -> Tuple[Union[bool, None], dict]:
        self.print_msg(f"{dip}:{dport}", result_code.START)
        return None, {}
    ...
    def print_msg(
        ...
        data["target"] = target
        data["call"] = (
            self.get_info("CVE") if self.get_info("CVE") else self.get_info("NAME")
        )
