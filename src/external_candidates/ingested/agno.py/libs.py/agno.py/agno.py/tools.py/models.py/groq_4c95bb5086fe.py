# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\agno\tools\models\groq.py
import base64
import os
...
class GroqTools(Toolkit):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            name="groq_tools",
            tools=[self.transcribe_audio, self.translate_audio, self.generate_speech],
            **kwargs,
        )
    ...
    def transcribe_audio(self, audio_source: str) -> str:
        ...
        """
        log_debug(
            f"Transcribing audio from {audio_source} using Groq model {self.transcription_model}"
        )
        try:
            # Check if the audio source as a local file or a URL
            if not os.path.exists(audio_source):
                log_debug(
                    f"Audio source '{audio_source}' not found locally, attempting as URL."
                )
            ...
            log_error(
                f"Failed to transcribe audio source '{audio_source}' with Groq: {str(e)}"
            )
    ...
    def translate_audio(self, audio_source: str) -> str:
        ...
        """
        log_debug(
            f"Translating audio from {audio_source} to English using Groq model {self.translation_model}"
        )
        try:
            if not os.path.exists(audio_source):
            ...
            log_error(
                f"Failed to translate audio source '{audio_source}' with Groq: {str(e)}"
            )
            return (
                f"Failed to translate audio source '{audio_source}' with Groq: {str(e)}"
            )
