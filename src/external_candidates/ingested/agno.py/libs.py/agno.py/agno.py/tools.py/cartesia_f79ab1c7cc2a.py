...
try:
    ...
    raise ImportError(
        "`cartesia` not installed. Please install using `pip install cartesia`"
    )
...
class CartesiaTools(Toolkit):
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "CARTESIA_API_KEY not set. Please set the CARTESIA_API_KEY environment variable."
            )
    ...
    def list_voices(self) -> str:
        ...
        try:
                ...
                for voice in voice_objects:
                    try:
                        ...
                        voice_data = {
                            ...
                            "language": (
                                voice.language if hasattr(voice, "language") else None
                            ),
                        }
                        ...
                        if voice_data["id"]:  # Only add if we could get an ID
                            ...
                            log_info(
                                f"Could not extract 'id' from voice object: {voice}"
                            )
                    except AttributeError as ae:
                        log_error(
                            f"AttributeError accessing voice data: {ae}. Voice data: {voice}"
                        )
                        continue
                    except Exception as inner_e:
                        log_error(
                            f"Unexpected error processing voice: {inner_e}. Voice data: {voice}"
                        )
                        continue
            ...
            return json.dumps(
                {"error": str(e), "detail": "Error occurred in list_voices function."}
            )
