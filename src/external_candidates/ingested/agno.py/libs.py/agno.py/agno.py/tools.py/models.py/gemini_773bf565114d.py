...
class GeminiTools(Toolkit):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.vertexai and not self.api_key:
            ...
            raise ValueError(
                "GOOGLE_API_KEY not set. Please provide api_key or set the environment variable."
            )
    ...
    def generate_image(
        ...
        try:
            ...
            for generated_image in response.generated_images:
                if (
                    generated_image.image is None
                    or not generated_image.image.image_bytes
                ):
                    continue
                ...
                log_debug(
                    f"Successfully generated image {media_id} with model {self.image_model}"
                )
    ...
    def generate_video(
        ...
        if not self.vertexai:
            log_error(
                "Video generation requires Vertex AI mode. Please enable Vertex AI mode."
            )
            return (
                "Video generation requires Vertex AI mode. "
        ...
        try:
            ...
            if (
                result is None
                or result.generated_videos is None
                or not result.generated_videos
            ):
            ...
            for video in result.generated_videos:
                ...
                media_id = str(uuid4())
                encoded_video = base64.b64encode(generated_video.video_bytes).decode(
                    "utf-8"
                )
                ...
                log_debug(
                    f"Successfully generated video {media_id} with model {self.video_model}"
                )
