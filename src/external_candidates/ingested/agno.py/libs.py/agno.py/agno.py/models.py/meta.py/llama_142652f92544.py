# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\agno\models\meta\llama.py
...
try:
    ...
    raise ImportError(
        "`llama-api-client` not installed. Please install using `pip install llama-api-client`"
    )
...
class Llama(Model):
    ...
    def _get_client_params(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            ...
            if not self.api_key:
                log_error(
                    "LLAMA_API_KEY not set. Please set the LLAMA_API_KEY environment variable."
                )
    ...
    def parse_tool_calls(
        tool_calls_data: List[EventDeltaToolCallDeltaFunction],
    ) -> List[Dict[str, Any]]:
        """
        ...
        for _field, _value in tool_calls_data:
            if _field == "function" and isinstance(
                _value, EventDeltaToolCallDeltaFunction
            ):
                if _value.name and (
                    _tool_call_id or _function_name_parts or _function_arguments_parts
                ):
                    _create_tool_call()
                if _value.name:
    ...
    def parse_provider_response_delta(
        self, response_delta: CreateChatCompletionResponseStreamChunk
    ) -> ModelResponse:
        """
