...
try:
    ...
    raise ImportError(
        "`cerebras-cloud-sdk` not installed. Please install using `pip install cerebras-cloud-sdk`"
    )
...
class Cerebras(Model):
    ...
    def _get_client_params(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            ...
            if not self.api_key:
                log_error(
                    "CEREBRAS_API_KEY not set. Please set the CEREBRAS_API_KEY environment variable."
                )
            ...
            message_dict["tool_calls"] = [
                {
                    ...
                    "function": {
                        ...
                        "arguments": (
                            ...
                            if isinstance(
                                tool_call["function"]["arguments"], (dict, list)
                            )
    ...
    def parse_provider_response_delta(
        self, response_delta: ChatChunkResponse
    ) -> ModelResponse:
        """
