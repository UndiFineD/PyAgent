...
try:
    ...
    raise ImportError(
        "`openai` not installed. Please install using `pip install openai -U`"
    ) from e
    ...
    def parse_provider_response(self, response: Response) -> ModelResponse:
        ...
        for output in response.output:
            if output.type == "message":
                ...
                model_response.extra.setdefault("tool_call_ids", []).append(
                    output.call_id
                )
    ...
    def _process_stream_response(
        ...
        if stream_event.type == "response.created":
            ...
            if stream_event.response.id:
                ...
                stream_data.response_provider_data["response_id"] = (
                    stream_event.response.id
                )
            # Update metrics
            ...
            if stream_data.response_citations is None:
                stream_data.response_citations = Citations(
                    raw=[stream_event.annotation]
                )
            else:
            ...
            stream_data.extra.setdefault("tool_call_ids", []).append(
                tool_use["call_id"]
            )
            tool_use = {}
