# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\agno\models\google\gemini.py
import json
import time
    ...
    def invoke(self, messages: List[Message]):
        ...
        try:
            ...
            raise ModelProviderError(
                ...
                status_code=(
                    e.code if hasattr(e, "code") and e.code is not None else 502
                ),
                    ...
                    try:
                        for audio_snippet in message.audio:
                            if audio_snippet.content is not None and isinstance(
                                ...
                                if (
                                    audio_snippet.content.uri
                                    and audio_snippet.content.mime_type
                                ):
                                    message_parts.insert(
                                        0,
                                        Part.from_uri(
    ...
    def _format_file_for_message(self, file: File) -> Optional[Part]:
        ...
        # Case 3: File is a local file path
            ...
            if file_path.exists() and file_path.is_file():
                if file_path.stat().st_size < 20 * 1024 * 1024:  # 20MB in bytes
                    if file.mime_type:
                        ...
                        if file_content:
                            return Part.from_bytes(
                                mime_type=file.mime_type, data=file_content
                            )
                    else:
                        import mimetypes
                        ...
                        if mime_type_guess is not None:
                            ...
                            if file_content:
                                ...
                                return Part.from_bytes(
                                    mime_type=mime_type_str, data=file_content
                                )
                    return None
                else:
                    ...
                    try:
                        if clean_file_name:
                            remote_file = self.get_client().files.get(
                    ...
                    if (
                        ...
                        return Part.from_uri(
                            file_uri=file_uri, mime_type=file_mime_type
                        )
            else:
        ...
        # Case 4: File is a Gemini File object
            ...
            if file.external.uri and file.external.mime_type:
                return Part.from_uri(
                    file_uri=file.external.uri, mime_type=file.external.mime_type
                )
            return None
        return None
        ...
        if response_message.parts is not None:
            for part in response_message.parts:
                ...
                if hasattr(part, "text") and part.text is not None:
                    ...
                    if isinstance(text_content, str):
                        ...
                        model_response.content = (
                            str(text_content) if text_content is not None else ""
                        )
                ...
                if hasattr(part, "function_call") and part.function_call is not None:
                    call_id = (
                        part.function_call.id if part.function_call.id else str(uuid4())
                    )
                    tool_call = {
                        "id": call_id,
                        "type": "function",
        ...
        if response_delta.candidates and len(response_delta.candidates) > 0:
            ...
            if response_message.parts is not None:
                for part in response_message.parts:
                    ...
                    if hasattr(part, "text") and part.text is not None:
                        model_response.content = (
                            str(part.text) if part.text is not None else ""
                        )
