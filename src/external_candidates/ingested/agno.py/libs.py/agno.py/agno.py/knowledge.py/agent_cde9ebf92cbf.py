# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\agno\knowledge\agent.py
import asyncio
...
class AgentKnowledge(BaseModel):
            ...
            # Insert documents
                ...
                if documents_to_load:
                    self.vector_db.insert(
                        documents=documents_to_load, filters=doc.meta_data
                    )
        ...
        async for document_list in self.async_document_lists:
            ...
            if upsert and self.vector_db.upsert_available():
                await self.vector_db.async_upsert(
                    documents=document_list, filters=doc.meta_data
                )
            # Insert documents
            else:
                # Filter out documents which already exist in the vector db
    ...
    def filter_existing_documents(self, documents: List[Document]) -> List[Document]:
        ...
        if len(filtered_documents) < original_count:
            log_info(
                f"Skipped {original_count - len(filtered_documents)} existing/duplicate documents."
            )
    ...
    def process_documents(
        ...
        log_info(
            f"Loading {len(documents)} documents from {source_info} with metadata: {metadata}"
        )
