# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\agno\embedder\aws_bedrock.py
import json
    ...
    def response(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ModelProviderError(
                ...
                model_name="AwsBedrockEmbedder",
                model_id=self.id,
            ) from e
        except Exception as e:
            log_error(f"Unexpected error calling Bedrock API: {str(e)}")
            raise ModelProviderError(
                message=str(e), model_name="AwsBedrockEmbedder", model_id=self.id
            ) from e
