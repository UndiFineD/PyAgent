# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\tests\unit\vectordb\test_pineconedb.py
import uuid
...
def test_doc_exists(mock_pinecone_db):
    ...
    mock_pinecone_db.index.fetch.assert_called_with(
        ids=[doc.id], namespace=TEST_NAMESPACE
    )
...
async def test_async_exists(mock_pinecone_db):
    ...
    with (
        patch.object(mock_pinecone_db, "exists", return_value=True),
...
async def test_async_create(mock_pinecone_db):
    ...
    with (
        patch.object(mock_pinecone_db, "create"),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
...
async def test_async_doc_exists(mock_pinecone_db):
    ...
    with (
        patch.object(mock_pinecone_db, "doc_exists", return_value=True),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = True
...
async def test_async_name_exists(mock_pinecone_db):
    ...
    with (
        patch.object(mock_pinecone_db, "name_exists", return_value=True),
        patch("asyncio.to_thread") as mock_to_thread,
    ):
        mock_to_thread.return_value = True
