# Extracted from: C:\DEV\PyAgent\.external\agno\libs\agno\tests\unit\tools\test_mcp.py
...
async def test_sse_transport_without_url_nor_sse_client_params():
    ...
    with pytest.raises(
        ValueError, match="One of 'url' or 'server_params' parameters must be provided"
    ):
...
async def test_stdio_transport_without_command_nor_server_params():
    ...
    with pytest.raises(
        ValueError,
        match="One of 'command' or 'server_params' parameters must be provided",
    ):
...
async def test_multimcp_without_endpoints():
    ...
    with pytest.raises(
        ValueError,
        match="Either server_params_list or commands or urls must be provided",
    ):
