...
def test_basic_iron_smelting_chain(game):
    ...
    assert (
        drill_with_coal.fuel.get(Prototype.Coal, 0) > 0
    ...
    assert (
        furnace_with_coal.fuel.get(Prototype.Coal, 0) > 0
...
def test_build_iron_plate_factory(game):
    ...
    for i in range(4):
        furnace_pos = furnaces[
            i
        ].position  # Position(x=miners[i].drop_position.x, y=furnace_line_start.position.y + 1)
