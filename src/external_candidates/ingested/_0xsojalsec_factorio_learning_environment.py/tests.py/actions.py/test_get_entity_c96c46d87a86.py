...
def test_get_iron_chest(game):
    ...
    assert (
        retrieved_chest.inventory.get(Prototype.Coal, 0) == 5
    ), "Failed to insert coal"
    assert (
        retrieved_chest.inventory.get(Prototype.IronPlate, 0) == 5
...
def test_get_lab(game):
    ...
    assert (
        retrieved_lab.lab_input.get(Prototype.AutomationSciencePack, 0) == 1
...
def test_get_turret(game):
    ...
    assert (
        retrieved_turret.turret_ammo.get(Prototype.FirearmMagazine, 0) == 5
