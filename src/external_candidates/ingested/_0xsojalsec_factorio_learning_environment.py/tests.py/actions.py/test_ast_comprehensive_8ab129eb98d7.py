...
""",
    ...
    assert (
        "This should not execute" not in output_str
...
""",
    ...
    output_str = str(result)
    assert (
        "Caught runtime error: New error" in output_str
...
""",
    ...
    assert "Assertion failed: x should be 10" in str(
        result
...
""",
    ...
    assert "Assertion failed without message: AssertionError" in str(
        result
...
""",
    ...
    output_str = str(result)
    assert (
        "Path join: a/b" in output_str or "Path join: a\\b" in output_str
...
""",
    ...
    assert (
        "Inner modified x to 20" in output_str
...
""",
    ...
    assert "List after +=: [1, 2, 3, 4, 5]" in str(
        result4
...
""",
    ...
    assert "Mapped squares: [1, 4, 9, 16, 25]" in str(
        result
...
""",
    ...
    assert "Filtered evens: [2, 4, 6, 8, 10]" in str(
        result
...
""",
    ...
    assert "Sorted by second element: [(3, 1), (1, 2), (5, 4)]" in str(
        result
...
""",
    ...
    assert (
        "Sum found: 0 + 3 = 3" in output_str or "Found 3" in output_str
    ), "Return should work in nested loops"
    assert (
        "Sum found" in output_str or "Found 7" in output_str
...
""",
    ...
    assert "Syntax error caught: SyntaxError" in str(
        result
...
""",
    ...
    assert (
        "Error occurred" not in output_str
