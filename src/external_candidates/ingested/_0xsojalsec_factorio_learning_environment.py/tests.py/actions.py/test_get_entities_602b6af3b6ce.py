...
def test_get_stone_furnace(game):
    ...
    assert (
        retrieved_furnace.furnace_result.get(Prototype.IronPlate, 0) > 0
    ), "Failed to smelt iron plate"
    assert (
        retrieved_furnace.furnace_source.get(Prototype.IronOre, 0) < 5
...
def test_group_prototype_support_belt_group(game):
    ...
    for group in belt_groups:
        assert (
            hasattr(group, "belts") or group.__class__.__name__ == "BeltGroup"
...
def test_group_prototype_support_pipe_group(game):
    ...
    for group in pipe_groups:
        assert (
            hasattr(group, "pipes") or group.__class__.__name__ == "PipeGroup"
...
def test_mixed_entity_grouping_behavior(game):
    ...
    assert (
        found_furnace or found_electricity
...
def test_group_prototype_backwards_compatibility(game):
    ...
    assert len(all_entities_after) >= len(
        all_entities_before
