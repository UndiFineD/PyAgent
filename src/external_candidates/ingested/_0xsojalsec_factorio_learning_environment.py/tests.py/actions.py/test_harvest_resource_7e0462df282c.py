# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\actions\test_harvest_resource.py
...
def test_harvest_bug_2(game):
    ...
    inventory = game.inspect_inventory()
    assert (
        inventory.get(Prototype.Stone) >= stone_needed
    ...
    inventory = game.inspect_inventory()
    assert (
        inventory.get(Prototype.StoneFurnace) >= 1
    ...
    inventory = game.inspect_inventory()
    assert (
        inventory.get(Prototype.Coal) >= coal_needed
    ...
    inventory = game.inspect_inventory()
    assert (
        inventory.get(Prototype.IronOre) >= iron_ore_needed
