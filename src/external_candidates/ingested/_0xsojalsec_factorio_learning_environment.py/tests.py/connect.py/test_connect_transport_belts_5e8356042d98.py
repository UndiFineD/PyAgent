# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\tests\connect\test_connect_transport_belts.py
from time import sleep

import pytest
from fle.env.entities import BeltGroup, Direction, Position, ResourcePatch
...
def test_ensure_final_belt_is_the_correct_orientation(game):
    ...
    assert (
        copper_drill_inserter.direction.value == Direction.RIGHT.value
    ...
    assert (
        beltgroup.belts[-1].direction.value == Direction.DOWN.value
...
def test_no_broken_edges(game):
    ...
    assert (
        copper_ore_drill_with_coal.fuel.get(Prototype.Coal, 0) > 0
    ...
    assert (
        chest_copper_inserter.direction.value == Direction.LEFT.value
    ), "Failed to rotate inserter"
