# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\infra\sweep_manager.py
"""
...
from fle.eval.infra.server_manager import ServerManager, get_server_manager
...
class SweepManager:
    ...
    def _validate_early_stopping_configuration(self):
        ...
        if self.config.num_trials_per_config == 1 and self.config.early_stop_on_success:
            print(
                "⚠️  WARNING: early_stop_on_success=True with num_trials_per_config=1"
            )
            print(
    ...
    async def load_existing_sweep_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_runs = (
                {}
            )  # key: (model, task), value: True if any success exists
            finished_runs = {}  # key: (model, task), value: count of finished runs
            partial_runs = (
                {}
            )  # key: (model, task, trial), value: {version, status} - only for truly incomplete runs
                    ...
                    logger.log_metrics(
                        {
                            ...
                            "job/duration_minutes": (
                                (job.end_time - job.start_time).total_seconds() / 60
                                if job.start_time
                                else 0
                            ),
                    ...
                    logger.log_metrics(
                        {
                            ...
                            "job/completion_status": (
                                "will_retry" if will_retry else "failed_final"
                            ),
                            "job/exit_code": process.exitcode,
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "elapsed_time": (
                (datetime.now() - self.start_time) if self.start_time else None
            ),
        }
