# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-factorio-learning-environment\fle\eval\open\plots\run_results.py
import json
    ...
    def _create_trees_from_db(self) -> Tuple[List[Node], Set[int]]:
        ...
        with self.db_client.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    f"""SELECT id, parent_id, code, response, achievements_json, value, raw_reward, ticks 
