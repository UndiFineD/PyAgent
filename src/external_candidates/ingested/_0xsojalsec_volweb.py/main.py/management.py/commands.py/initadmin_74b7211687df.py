# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VolWeb\main\management\commands\initadmin.py
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        if Account.objects.count() == 0:
            try:
                createSuperUser(
                    "admin", "password", email="", firstName="NAME", lastName="SURNAME"
                )
                createSimpleUser(
                    "user", "password", email="", firstName="NAME", lastName="SURNAME"
                )
            except:
