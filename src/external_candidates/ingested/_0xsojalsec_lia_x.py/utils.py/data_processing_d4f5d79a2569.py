# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LIA-X\utils\data_processing.py
...
def save_vid_edit(save_dir, vid_d, vid_a, w, h, fps):
    ...
    imageio.mimwrite(
        output_vid_all_path, vid_all_np, fps=fps, codec="libx264", quality=8
    )
...
def save_animation(save_dir, img_s, vid_d, vid_a, w, h, fps):
    ...
    vid_a = rearrange(vid_a, "t c h w -> t h w c")
    img_s = repeat(
        rearrange(img_s, "b c h w -> b h w c"),
        "b h w c -> (b t) h w c",
        t=vid_d.size(0),
    )
