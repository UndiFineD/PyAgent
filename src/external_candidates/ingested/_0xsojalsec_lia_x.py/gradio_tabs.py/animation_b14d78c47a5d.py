# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LIA-X\gradio_tabs\animation.py
...
def animation(gen, chunk_size, device):
    ...
    def animate_media(image, video, *selected_s):
        ...
        animated_video = gen.animate(
            image_tensor, video_target_tensor, labels_v, selected_s, chunk_size
        )  # tchw
    ...
    with gr.Tab("Image Animation"):
                ...
                with gr.Row():
                    with gr.Accordion(open=True, label="Source Image"):
                        image_input = gr.Image(
                            type="filepath", elem_id="input_img", width=512
                        )  # , height=550)
                        gr.Examples(
                            examples=[
                ...
                with gr.Row():
                    ...
                    with gr.Accordion(open=True, label="Animated Video"):
                        video_output = gr.Video(
                            label="Output Video", elem_id="output_vid", width=512
                        )
        ...
        clear_btn.click(fn=clear_media, outputs=[image_output, video_output] + inputs_s)
