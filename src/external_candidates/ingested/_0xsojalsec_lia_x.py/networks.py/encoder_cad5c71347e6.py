# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LIA-X\networks\encoder.py
...
from .ops import ConvLayer, EqualConv2d, EqualLinear
...
class Encoder2R(nn.Module):
    def __init__(self, latent_dim=512, scale=1):
        ...
        self.block2 = nn.Sequential(ResBlock(channels[0], channels[1]))  # 64 -> 128
        self.block3 = nn.Sequential(ResBlock(channels[1], channels[2]))  # 128 -> 256
        self.block4 = nn.Sequential(ResBlock(channels[2], channels[3]))  # 256 -> 512
        self.block5 = nn.Sequential(ResBlock(channels[3], channels[3]))  # 512 -> 512
        self.block6 = nn.Sequential(ResBlock(channels[3], channels[3]))  # 512 -> 512
        self.block7 = nn.Sequential(ResBlock(channels[3], channels[3]))  # 512 -> 512
...
class Encoder(nn.Module):
    def __init__(self, dim=512, dim_motion=20, scale=1):
        ...
        # R2T
        self.enc_r2t = nn.Sequential(EqualLinear(dim, dim_motion))
    ...
    def enc_transfer_img(self, z_s2r, d_l, s_l):
        ...
        alpha_r2s[:, d_l] = alpha_r2s[:, d_l] + torch.FloatTensor(s_l).unsqueeze(0).to(
            "cuda"
        )
        alpha = [alpha_r2s]
