...
class Generator(nn.Module):
    ...
    def edit_img(self, img_source, d_l, v_l):
        ...
        alpha_r2s[:, d_l] = alpha_r2s[:, d_l] + torch.FloatTensor(v_l).unsqueeze(0).to(
            "cuda"
        )
    ...
    def animate(self, img_source, vid_target, d_l, v_l, chunk_size):
        ...
        z_s2r, feat_rgb = self.enc.enc_2r(img_source)
        alpha_r2s = self.enc.enc_r2t(z_s2r)
        v_l_tensor = torch.tensor(
            v_l, device=alpha_r2s.device, dtype=alpha_r2s.dtype
        ).unsqueeze(0)
        ...
        for i in range(chunks):
            ...
            alpha = self.enc.enc_transfer_vid(
                alpha_r2s_curr, img_target, alpha_start_curr
            )
