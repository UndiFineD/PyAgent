# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-redis-ai-resources\java-recipes\notebooks\jupyter\install.py
import argparse
import json
import os
import sys
...
class EnvVar(argparse.Action):
    def __init__(
        self, option_strings, dest, aliases=None, name_map=None, list_sep=None, **kwargs
    ):
        ...
        for name in self.option_strings:
            if name.lstrip("-") not in name_map:
                raise ValueError(
                    'Name "%s" is not mapped to an environment variable'
                    % name.lstrip("-")
                )
...
if __name__ == "__main__":
    ...
    install_location.add_argument(
        "--user", help="Install to the per-user kernel registry.", action="store_true"
    )
    install_location.add_argument(
        "--sys-prefix",
    ...
    local_kernel_json_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "java", "kernel.json"
    )
    ...
    with open(local_kernel_json_path, "r") as template_kernel_json_file:
        ...
        with open(installed_kernel_json_path, "w") as installed_kernel_json_file:
            json.dump(
                kernel_json_json_contents,
                installed_kernel_json_file,
                indent=4,
                sort_keys=True,
            )
