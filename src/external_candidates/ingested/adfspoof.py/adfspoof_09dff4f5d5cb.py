...
def parse_args():
    ...
    key_group.add_argument(
        "-b", "--blob", help="Encrypted PFX blob and decryption key", nargs=2
    )
    key_group.add_argument("-c", "--cert", help="AD FS Signing Certificate")
    arg_parser.add_argument(
        "-p", "--password", help="AD FS Signing Certificate Password", default=None
    )
    ...
    arg_parser.add_argument(
        "-a", "--algorithm", help="SAML signing algorithm to use", default="rsa-sha256"
    )
    arg_parser.add_argument(
        "-d", "--digest", help="SAML digest algorithm to use", default="sha256"
    )
    arg_parser.add_argument(
        "-o", "--output", help="Write generated token to the supplied filepath"
    )
    ...
    parser_office365.add_argument(
        "--upn", help="Universal Principal Name of user to spoof", required=True
    )
    parser_office365.add_argument(
        "--objectguid",
    ...
    parser_generic_saml2.add_argument(
        "--nameid", help="The NameIdentifier attribute value", default=None
    )
    parser_generic_saml2.add_argument(
        "--rpidentifier", help="The Identifier for the Relying Party", default=None
    )
    parser_generic_saml2.add_argument(
        "--assertions", help="The XML assertions for the SAML token", default=None
    )
    parser_generic_saml2.add_argument(
        "--config", help="JSON file containing generic args", default=None
    )
    ...
    if command != "dump":
        if not args.server:
            sys.stderr.write(
                "If generating a token you must supply the federation service identifier with --server.\n"
            )
            die()
            ...
            if not args.config and any([arg is None for arg in saml_set]):
                sys.stderr.write(
                    "If not using a config file you must specify all the other SAML 2.0 args. Quitting.\n"
                )
                die()
    ...
    if command == "o365":
        ...
        if args.config:
            ...
            try:
                ...
                sys.stderr.write(
                    "Could not parse JSON config file for SAML2 token creation. Quitting.\n"
                )
                die()
        else:
            saml2_params = {
