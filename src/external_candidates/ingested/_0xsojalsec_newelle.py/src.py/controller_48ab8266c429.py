# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\controller.py
import base64
import json
import os
import pickle
...
class NewelleController:
    ...
    def check_path_integrity(self):
        ...
        if os.path.isdir(self.pip_path):
            ...
            threading.Thread(
                target=self.init_pip_path, args=(self.python_path,)
            ).start()
    ...
    def reload(self, reload_type: ReloadType):
        ...
        if reload_type == ReloadType.EXTENSIONS:
            ...
            restore_handlers()
            self.extensionloader.add_handlers(
                AVAILABLE_LLMS,
                AVAILABLE_TTS,
                AVAILABLE_STT,
                AVAILABLE_MEMORIES,
            ...
            if self.newelle_settings.use_secondary_language_model:
                threading.Thread(
                    target=self.handlers.secondary_llm.load_model, args=(None,)
                ).start()
    ...
    def load_integrations(self):
        """Load integrations"""
        self.integrationsloader = ExtensionLoader(
            self.extension_path,
            pip_path=self.pip_path,
            settings=self.settings,
            extension_cache=self.extensions_cache,
    ...
    def create_profile(
        self, profile_name, picture=None, settings={}, settings_groups=[]
    ):
        """Create a profile
        ...
        """
        self.newelle_settings.profile_settings[profile_name] = {
            "picture": picture,
            "settings": settings,
            "settings_groups": settings_groups,
        }
        self.settings.set_string(
            "profiles", json.dumps(self.newelle_settings.profile_settings)
        )
    ...
    def delete_profile(self, profile_name):
        ...
        """
        if profile_name == "Assistant" or profile_name == self.settings.get_string(
            "current-profile"
        ):
            return
    ...
    def update_current_profile(self):
        ...
        old_settings = get_settings_dict_by_groups(
            self.settings, groups, SETTINGS_GROUPS, ["current-profile", "profiles"]
        )
    ...
    def export_profile(self, profile_name, remove_passwords=False, export_propic=False):
        ...
        if profile is None:
            ...
            if remove_passwords:
                profile["settings"] = self.handlers.remove_passwords(
                    profile["settings"]
                )
...
class NewelleSettings:
    ...
    def load_settings(self, settings):
        ...
        self.current_profile = self.settings.get_string("current-profile")
        if (
            len(self.profile_settings) == 0
            or self.current_profile not in self.profile_settings
        ):
            self.profile_settings[self.current_profile] = {
                "settings": {},
                "picture": None,
                "settings_groups": [],
            }
        ...
        self.stt_silence_detection_threshold = settings.get_double(
            "stt-silence-detection-threshold"
        )
        self.stt_silence_detection_duration = settings.get_int(
            "stt-silence-detection-duration"
        )
        ...
        self.secondary_language_model = self.settings.get_string(
            "secondary-language-model"
        )
        self.secondary_language_model_settings = self.settings.get_string(
            "llm-secondary-settings"
        )
        self.use_secondary_language_model = self.settings.get_boolean(
            "secondary-llm-on"
        )
    ...
    def compare_settings(self, new_settings) -> list[ReloadType]:
        ...
        reloads = []
        if (
            self.language_model != new_settings.language_model
            or self.llm_settings != new_settings.llm_settings
        ):
            reloads.append(ReloadType.LLM)
        if (
            self.secondary_language_model != new_settings.secondary_language_model
            or self.use_secondary_language_model
            != new_settings.use_secondary_language_model
            or self.secondary_language_model_settings
        ...
        if (
            self.embedding_model != new_settings.embedding_model
            or self.embedding_settings != new_settings.embedding_settings
        ):
            reloads.append(ReloadType.EMBEDDINGS)

        if (
            self.memory_on != new_settings.memory_on
            or self.memory_model != new_settings.memory_model
            or self.memory_settings != new_settings.memory_settings
        ):
            reloads.append(ReloadType.MEMORIES)

        if (
            self.rag_on != new_settings.rag_on
            or self.rag_model != new_settings.rag_model
            or self.rag_settings != new_settings.rag_settings
        ):
            reloads.append(ReloadType.RAG)
        if self.extensions_settings != new_settings.extensions_settings:
            reloads += [
                ReloadType.EXTENSIONS,
                ReloadType.LLM,
                ReloadType.SECONDARY_LLM,
                ReloadType.EMBEDDINGS,
                ReloadType.EMBEDDINGS,
                ReloadType.MEMORIES,
                ReloadType.RAG,
        ...
        if (
            self.websearch_on != new_settings.websearch_on
            or self.websearch_model != new_settings.websearch_model
            or self.websearch_settings != new_settings.websearch_settings
        ):
...
class HandlersManager:
    ...
    def select_handlers(self, newelle_settings: NewelleSettings):
        ...
        # Get LLM
        self.llm: LLMHandler = self.get_object(
            AVAILABLE_LLMS, newelle_settings.language_model
        )
        if newelle_settings.use_secondary_language_model:
            self.secondary_llm: LLMHandler = self.get_object(
                AVAILABLE_LLMS, newelle_settings.secondary_language_model, True
            )
        else:
            self.secondary_llm: LLMHandler = self.llm
        self.stt: STTHandler = self.get_object(
            AVAILABLE_STT, newelle_settings.stt_engine
        )
        self.tts: TTSHandler = self.get_object(
            AVAILABLE_TTS, newelle_settings.tts_program
        )
        self.embedding: EmbeddingHandler = self.get_object(
            AVAILABLE_EMBEDDINGS, newelle_settings.embedding_model
        )
        self.memory: MemoryHandler = self.get_object(
            AVAILABLE_MEMORIES, newelle_settings.memory_model
        )
        self.memory.set_memory_size(newelle_settings.memory)
        self.rag: RAGHandler = self.get_object(
            AVAILABLE_RAGS, newelle_settings.rag_model
        )
        self.websearch: WebSearchHandler = self.get_object(
            AVAILABLE_WEBSEARCH, newelle_settings.websearch_model
        )
        # Assign handlers
        self.integrationsloader.set_handlers(
            self.llm,
            self.stt,
            self.tts,
            self.secondary_llm,
    ...
    def cache_handlers(self):
        ...
        for key in AVAILABLE_TTS:
            self.handlers[(key, self.convert_constants(AVAILABLE_TTS), False)] = (
                self.get_object(AVAILABLE_TTS, key)
            )
        for key in AVAILABLE_STT:
            self.handlers[(key, self.convert_constants(AVAILABLE_STT), False)] = (
                self.get_object(AVAILABLE_STT, key)
            )
        for key in AVAILABLE_LLMS:
            self.handlers[(key, self.convert_constants(AVAILABLE_LLMS), False)] = (
                self.get_object(AVAILABLE_LLMS, key)
            )
        # Secondary LLMs
        for key in AVAILABLE_LLMS:
            self.handlers[(key, self.convert_constants(AVAILABLE_LLMS), True)] = (
                self.get_object(AVAILABLE_LLMS, key, True)
            )
        for key in AVAILABLE_MEMORIES:
            self.handlers[(key, self.convert_constants(AVAILABLE_MEMORIES), False)] = (
                self.get_object(AVAILABLE_MEMORIES, key)
            )
        for key in AVAILABLE_RAGS:
            self.handlers[(key, self.convert_constants(AVAILABLE_RAGS), False)] = (
                self.get_object(AVAILABLE_RAGS, key)
            )
        for key in AVAILABLE_EMBEDDINGS:
            self.handlers[
                (key, self.convert_constants(AVAILABLE_EMBEDDINGS), False)
            ] = self.get_object(AVAILABLE_EMBEDDINGS, key)
        for key in AVAILABLE_WEBSEARCH:
            self.handlers[(key, self.convert_constants(AVAILABLE_WEBSEARCH), False)] = (
                self.get_object(AVAILABLE_WEBSEARCH, key)
            )
        self.handlers_cached.release()
    ...
    def get_object(
        self, constants: dict[str, Any], key: str, secondary=False
    ) -> Handler:
        """Get an handler instance for the specified handler key
