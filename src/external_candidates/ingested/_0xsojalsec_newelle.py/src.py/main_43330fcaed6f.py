# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\main.py
import gettext
import os
import sys
...
class MyApp(Adw.Application):
    ...
    def on_about_action(self, *a):
        Adw.AboutWindow(
            ...
            developers=[
                "Yehor Hliebov  https://github.com/qwersyk",
            ...
            designers=[
                "Nokse22 https://github.com/Nokse22",
                "Jared Tweed https://github.com/JaredTweed",
            ],
            translator_credits="\n".join(
                [
    ...
    def close_window(self, *a):
        ...
        if all(element.poll() is not None for element in self.win.streams):
            ...
            dialog = Adw.MessageDialog(
                ...
                body=_(
                    "When you close the window, they will be automatically terminated"
                ),
    ...
    def reload_chat(self, *a):
        self.win.show_chat()
        self.win.notification_block.add_toast(Adw.Toast(title=_("Chat is rebooted")))

    def reload_folder(self, *a):
        self.win.update_folder()
        self.win.notification_block.add_toast(Adw.Toast(title=_("Folder is rebooted")))

    def new_chat(self, *a):
        self.win.new_chat(None)
        self.win.notification_block.add_toast(Adw.Toast(title=_("Chat is created")))
