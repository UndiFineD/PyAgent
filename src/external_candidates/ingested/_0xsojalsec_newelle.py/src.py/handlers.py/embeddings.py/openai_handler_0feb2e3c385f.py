...
class OpenAIEmbeddingHandler(EmbeddingHandler):
    ...
    def build_extra_settings(
        ...
        api_settings = [
            ExtraSettings.EntrySetting(
                "api", _("API Key"), _("API Key for " + provider_name), ""
            ),
        ]
        endpoint_settings = [
            ExtraSettings.EntrySetting(
        ...
        custom_model = [
            ExtraSettings.ToggleSetting(
                "custom_model", _("Use Custom Model"), _("Use a custom model"), False
            )
        ]
        models_settings = [
            ExtraSettings.EntrySetting(
    ...
    def get_embedding(self, text: list[str]) -> np.ndarray:
        ...
        client = Client(
            api_key=self.get_setting("api"), base_url=self.get_setting("endpoint")
        )
        embedding = client.embeddings.create(
            input=text, model=self.get_setting("model")
        )
        res = []
