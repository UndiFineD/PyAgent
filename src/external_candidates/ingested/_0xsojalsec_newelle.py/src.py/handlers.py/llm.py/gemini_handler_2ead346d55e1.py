# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\llm\gemini_handler.py
import base64
import json
import os
import threading
...
from ...utility.media import extract_file, extract_image, extract_video
...
class GeminiHandler(LLMHandler):
    ...
    def __init__(self, settings, path):
        ...
        self.cache = {}
        if (
            self.get_setting("models", False) is None
            or len(self.get_setting("models", False)) == 0
        ):
        ...
        if self.is_installed():
            try:
                ...
                for model in models:
                    ...
                    print(model)
                    if (
                        "embedding" in model.display_name.lower()
                        or "legacy" in model.display_name.lower()
                    ):
                        continue
                    result += (
                        (
                ...
                if manual:
                    self.throw(
                        "Error getting " + self.key + " models: " + str(e),
    ...
    def get_extra_settings(self) -> list:
        r = [
            ExtraSettings.EntrySetting(
                "apikey",
                _("API Key (required)"),
                _("API key got from ai.google.dev"),
                "",
                password=True,
            ),
            ...
            ExtraSettings.ToggleSetting(
                "system_prompt",
                _("Enable System Prompt"),
                _(
                    "Some models don't support system prompt (or developers instructions), disable it if you get errors about it"
                ),
                True,
        ...
        if not (
            self.get_setting("system_prompt", False) is None
            or self.get_setting("system_prompt", False)
        ):
            r += [
                ExtraSettings.ToggleSetting(
                    "force_system_prompt",
                    _("Inject system prompt"),
                    _(
                        "Even if the model doesn't support system prompts, put the prompts on top of the user message"
                    ),
                    True,
                )
            ]
        r += [
            ExtraSettings.NestedSetting(
                "think",
                _("Thinking Settings"),
                _("Settings about thinking models"),
                [
                    ExtraSettings.ToggleSetting(
                        "thinking",
                        _("Enable Thinking"),
                        _(
                            "Show thinking, disable it if your model does not support it"
                        ),
                        True,
                    ),
                    ExtraSettings.ToggleSetting(
                        "enable_thinking_budget",
                        _("Enable Thinking Budget"),
                        _("If to enable thinking budget"),
                        False,
                    ),
                    ExtraSettings.ScaleSetting(
                        "thinking_budget",
                        _("Thinking Budget"),
                        _("How much time to spend thinking"),
                        8000,
                        0,
                        24576,
                        0,
        ...
        r += [
            ExtraSettings.ToggleSetting(
                "img_output",
                _("Image Output"),
                _("Enable image output, only supported by gemini-2.0-flash-exp"),
                False,
            ),
            ExtraSettings.ToggleSetting(
                "streaming",
            ...
            ExtraSettings.ButtonSetting(
                ...
                lambda button: open_website("https://ai.google.dev/gemini-api/terms"),
                None,
                "internet-symbolic",
            ),
            ExtraSettings.ToggleSetting(
        ...
        if self.get_setting("advanced_params", False):
            r += [
                ...
                ExtraSettings.ScaleSetting(
                    "top_p",
                    "Top P",
                    "Probability of the top tokens to keep",
                    1,
                    0,
                    1,
                    2,
                    ...
                    types.Content(
                        ...
                        parts=(
                            ...
                            if img is None
                                ...
                                types.Part.from_text(text=text),
                                types.Part.from_uri(
                                    file_uri=img.uri, mime_type=img.mime_type
                                ),
                            ]
                        ),
                    )
    ...
    def generate_text_stream(
        ...
        if not self.get_setting("system_prompt"):
            ...
            if self.get_setting("force_system_prompt"):
                append_instructions = "\n".join(
                    [p.replace("```", "\\\\\\```") for p in system_prompt]
                )
        if not self.get_setting("advanced_params"):
            generate_content_config = GenerateContentConfig(
                ...
                response_modalities=(
                    ["text"] + ["image"] if self.get_setting("img_output") else ["text"]
                ),
            )
        else:
            generate_content_config = GenerateContentConfig(
                system_instruction=(
                    instructions if not self.get_setting("img_output") else None
                ),
                safety_settings=safety,
                response_modalities=(
                    ["text"] + ["image"] if self.get_setting("img_output") else ["text"]
            ...
            for chunk in response:
                ...
                for part in chunk.candidates[0].content.parts:
                    if part.inline_data:
                        ...
                        self.save_binary_file(file_name, part.inline_data.data)
                        full_message += "\n```image\n" + file_name + "\n```\n"
                    elif not part.text:
                        continue
                    elif part.thought:
