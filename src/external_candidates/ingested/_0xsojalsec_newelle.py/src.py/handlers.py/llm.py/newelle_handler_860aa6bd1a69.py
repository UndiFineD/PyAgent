# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\llm\newelle_handler.py
import json
...
class NewelleAPIHandler(LLMHandler):
        ...
        return [
            {
                ...
                "callback": lambda button: open_website(
    ...
    def generate_text_stream(
        ...
        if prompt.startswith("```image") or any(
            message.get("Message", "").startswith("```image")
        ...
        try:
            response = requests.post(
                url + "/chat/completions", headers=headers, json=data, stream=True
            )
