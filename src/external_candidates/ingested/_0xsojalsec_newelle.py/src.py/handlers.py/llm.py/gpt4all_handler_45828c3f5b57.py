# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\llm\gpt4all_handler.py
import ctypes
import os
import re
import threading
...
class GPT4AllHandler(LLMHandler):
    ...
    def add_library_information(self):
        ...
        for model in self.models_info:
            ...
            subtitle += "\n" + re.sub("<[^<]+?>", "", model["description"]).replace(
                "</ul", ""
            )
    ...
    def get_model_library(self) -> list:
        ...
        for model in self.model_library:
            s = ExtraSettings.DownloadSetting(
                model["key"],
                model["title"],
                model["description"],
                self.model_available(model["key"]),
                self.install_model,
                self.get_percentage,
            )
            res.append(s)
        ...
        if model in self.downloading and self.downloading[model]:
            ...
            if model in self.model_threads:
                ...
                res = ctypes.pythonapi.PyThreadState_SetAsyncExc(
                    ctypes.c_long(thid), ctypes.py_object(SystemExit)
                )
                if res > 1:
            ...
            for file in files:
                if file.endswith(".gguf"):
                    ...
                    relative_path = os.path.relpath(
                        os.path.join(root, file), self.model_folder
                    )
    ...
    def model_available(self, model: str) -> bool:
        ...
        try:
            GPT4All.retrieve_model(
                model, model_path=self.modelspath, allow_download=False, verbose=False
            )
        except Exception as e:
            return False
        return True
    ...
    def download_model(self, model: str) -> bool:
        ...
        try:
            ...
            GPT4All.retrieve_model(
                model, model_path=self.modelspath, allow_download=True, verbose=False
            )
        except Exception as e:
            print(e)
            return False
