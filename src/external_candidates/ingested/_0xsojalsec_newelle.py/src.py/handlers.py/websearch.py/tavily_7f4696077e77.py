# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\websearch\tavily.py
...
class TavilyHandler(WebSearchHandler):
    ...
    def get_extra_settings(self) -> list:
        return [
            ExtraSettings.EntrySetting(
                "token", _("Token"), _("Tavily API key"), "", password=True
            ),
            ExtraSettings.ScaleSetting(
                "results",
                _("Max Results"),
                _("Number of results to consider"),
                2,
                1,
                20,
                0,
            ...
            ExtraSettings.ScaleSetting(
                ...
                _("Chunks per source"),
                _(
                    "The number of content chunks to retrieve from each source. Each chunk's length is maximum 500 characters. Available only when search depth is advanced."
                ),
                3,
                1,
                3,
            ...
            ExtraSettings.ScaleSetting(
                "days",
                _("Number of days back from the current date to include"),
                _("Available only if topic is news."),
                7,
                1,
                365,
                0,
            ...
            ExtraSettings.ToggleSetting(
                ...
                _("Include answer"),
                _(
                    "Include an LLM-generated answer to the provided query. Basic search returns a quick answer. Advanced returns a more detailed answer."
                ),
                True,
            ),
            ExtraSettings.ToggleSetting(
                "include_raw_content",
                _("Include raw content"),
                _("Include the cleaned and parsed HTML content of each search result."),
                False,
            ),
            ExtraSettings.ToggleSetting(
                "include_images",
                _("Include images"),
                _("Perform an image search and include the results in the response."),
                False,
            ),
            ExtraSettings.ToggleSetting(
                "include_image_descriptions",
                _("Include image descriptions"),
                _(
            ...
            ExtraSettings.EntrySetting(
                "include_domains",
                _("Include domains"),
                _("A list of domains to specifically include in the search results."),
                "",
            ),
            ExtraSettings.EntrySetting(
                "exclude_domains",
                _("Exclude domains"),
                _("A list of domains to specifically exclude from the search results."),
                "",
            ),
        ]
    ...
    def query_streaming(self, keywords: str, add_website) -> tuple[str, list]:
        ...
        if not (token := self.get_setting("token")):
            return (
                "Tavily API token not provided. Please enter your token in the settings to continue.",
                [],
            )
        ...
        try:
            results = client.search(
                ...
                include_image_descriptions=self.get_setting(
                    "include_image_descriptions"
                ),
                include_domains=[
                    x
                    for x in re.split(r"[,\s]+", self.get_setting("include_domains"))
                    if x
                ],
                exclude_domains=[
                    x
                    for x in re.split(r"[,\s]+", self.get_setting("exclude_domains"))
                    if x
                ],
            )["results"]
        except Exception as e:
