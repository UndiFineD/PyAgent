# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\memory\memoripy_handler.py
import os

import numpy as np

from ...handlers import ExtraSettings, Handler
...
class MemoripyHandler(MemoryHandler):
    ...
    def install(self):
        pip_path = os.path.join(
            os.path.abspath(os.path.join(self.path, os.pardir)), "pip"
        )
        install_module(
            "git+https://github.com/FrancescoCaracciolo/Memoripy.git", pip_path
        )
    ...
    def load(self, embedding, llm):
        ...
        self.memory_manager = MemoryManager(
            self._create_chat_adapter(llm),
            self._create_embedding_adapter(embedding),
            storage_option,
        )
    ...
    def _create_chat_adapter(self, llm: LLMHandler):
        ...
        class ChatModelAdapter(ChatModel):
            ...
            def invoke(self, messages: list) -> str:
                prompts, history = convert_history_newelle(
                    messages[:-1], llm.supports_vision()
                )
                response = self.llm.generate_text(
                    messages[:-1]["content"]["text"], history, prompts
                )
                return response
    ...
    def get_context(self, prompt, history: list[dict[str, str]]) -> list[str]:
        ...
        if self.memory_manager is not None:
            relevant_interactions = self.memory_manager.retrieve_relevant_interactions(
                prompt, exclude_last_n=min(self.memory_size, len(history))
            )
        else:
            return []
    ...
    def register_response(self, bot_response, history):
        if self.memory_manager is not None:
            ...
            self.memory_manager.add_interaction(
                history[-1]["Message"], bot_response, new_embedding, concepts
            )
