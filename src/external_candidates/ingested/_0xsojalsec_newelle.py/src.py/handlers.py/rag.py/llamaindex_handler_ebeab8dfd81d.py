# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\rag\llamaindex_handler.py
import os
import threading
...
class LlamaIndexHanlder(RAGHandler):
    ...
    def get_extra_settings(self) -> list:
        r = [
            ExtraSettings.ScaleSetting(
                "chunk_size",
                "Chunk Size",
                "Split text in chunks of the given size (in tokens). Requires a reindex",
                512,
                64,
                2048,
                0,
            ...
            ExtraSettings.ScaleSetting(
                ...
                "Similarity of the document to be returned",
                "Set the percentage similarity of a document to get returned",
                0.1,
                0,
                1,
                2,
            ...
            ExtraSettings.ToggleSetting(
                "use_llm",
                "Secondary LLM",
                "Use the secondary LLM to improve retrivial",
                False,
            ),
            ExtraSettings.ToggleSetting(
                "subdirectory_on",
                "Index Only a subdirectory",
                "Choose only a subdirectory to index. If you already have indexed it, you don't need to re-index",
                False,
        ...
        if self.get_setting("subdirectory_on", False, False):
            r += [
                ExtraSettings.ComboSetting(
                    "subdirectory",
                    "Subdirectory",
                    "Subdirectory to index",
                    self.get_subdirectories(),
                    (
                        self.get_subdirectories()[0][1]
                        if len(self.get_subdirectories()) > 0
                        else "."
                    ),
                    update_settings=True,
                )
        ...
        r += [
            ExtraSettings.NestedSetting(
                "documents",
                "Document extensions",
                "List of document extensions to index",
                [
    ...
    def load(self):
        if self.index_exists() and (
            (self.index is None and self.loading_thread is None)
            or self.loaded_index != self.get_paths()[1]
        ):
    ...
    def is_installed(self) -> bool:
        return (
            find_module("llama_index") is not None
            and find_module("tiktoken") is not None
            and find_module("faiss") is not None
            and find_module("llama_index.vector_stores") is not None
        )
    ...
    def load_index(self):
        ...
        storage_context = StorageContext.from_defaults(
            persist_dir=data_path, vector_store=vector_store
        )
    ...
    def index_exists(self):
        documents_path, data_path = self.get_paths()
        return os.path.exists(os.path.join(data_path, "docstore.json")) and (
            not self.indexing
        )
    ...
    def create_index(self, button=None):
        ...
        try:
            ...
            index = VectorStoreIndex.from_documents(
                documents[:1], storage_context=storage_context
            )
            i = 1
    ...
    def build_index(
        self, documents: list[str], chunk_size: int | None = None
    ) -> RAGIndex:
        import faiss
        import tiktoken
        ...
        chunk_size = (
            int(self.get_setting("chunk_size")) if chunk_size is None else chunk_size
        )
        ...
        return LlamaIndexIndex(
            index,
            int(self.get_setting("return_documents")),
            float(self.get_setting("similarity_threshold")),
            counter,
            document_list,
        )
...
class LlamaIndexIndex(RAGIndex):
    ...
    def query(self, query: str) -> list[str]:
        ...
        if self.retriever is None:
            retriever = VectorIndexRetriever(
                index=self.index, similarity_top_k=int(self.return_documents)
            )
            self.retriever = retriever
        r = []
