# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\handlers\rag\rag_handler.py
import os
...
class RAGHandler(Handler):
    ...
    def get_index_row(self):
        ...
        """
        return ExtraSettings.DownloadSetting(
            "index",
            _("Index your documents"),
            _(
                "Index all the documents in your document folder. You have to run this operation every time you edit/create a document, change document analyzer or change embedding model"
            ),
            self.index_exists(),
            self.index_button_pressed,
            lambda _: self.indexing_status,
            download_icon="text-x-generic",
        )
    ...
    def query_document(
        self, prompt: str, documents: list[str], chunk_size: int | None = None
    ) -> list[str]:
        """
        Query the document
