# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\ui\profile.py
...
class ProfileDialog(Adw.PreferencesDialog):
    def __init__(self, parent, profile_settings, profile_name=None):
        ...
        self.settings_row = Adw.ExpanderRow(
            title=_("Copied Settings"),
            subtitle=_("Settings that will be copied to the new profile"),
        )
        ...
        if not editing:
            ...
            self.password_row = Adw.ActionRow(
                ...
                subtitle=_("Also export password-like fields"),
            )
            self.export_password = Gtk.Switch(active=False, valign=Gtk.Align.CENTER)
            self.propic_row = Adw.ActionRow(
                title=_("Export Propic"), subtitle=_("Also export the profile picture")
            )
        ...
        if image is not None:
            ...
            self.avatar.get_last_child().get_last_child().set_icon_size(
                Gtk.IconSize.LARGE
            )
        # Create Button
        self.create_button = Gtk.Button(
            label=_("Create") if not editing else _("Apply")
        )
        ...
        if not editing:
            ...
            warning = Gtk.Label(
                label=_(
                    "The settings of the current profile will be copied into the new one"
                ),
                wrap=True,
            )
            g.add(warning)
    ...
    def export_profile(self, button):
        ...
        dialog = Gtk.FileDialog(
            accept_label=_("Export"), title=_("Export Profile"), default_filter=filter
        )
    ...
    def import_profile(self, button):
        filter = Gtk.FileFilter(name=_("Newelle Profiles"), patterns=["*.np", "*.json"])
        dialog = Gtk.FileDialog(
            accept_label=_("Import"), title=_("Import Profile"), default_filter=filter
        )
    ...
    def export_profile_file(self, dialog, result):
        ...
        if file is not None:
            ...
            with open(path, "w") as f:
                ...
                json.dump(
                    self.parent.controller.export_profile(
                        self.original_name,
                        self.export_password.get_active(),
                        self.export_propic.get_active(),
                    ),
                    f,
                )
    ...
    def build_settings_group(self, edit=False):
        ...
        for setting, group in SETTINGS_GROUPS.items():
            ...
            if edit:
                toggle.set_active(
                    setting
                    in self.profile_settings[self.profile_name].get(
                        "settings_groups", []
                    )
                )
            else:
            ...
            row = Adw.ActionRow(
                title=group["title"], subtitle=group["description"], vexpand=False
            )
    ...
    def on_profile_name_changed(self, entry):
        ...
        if profile_name:
            ...
            self.avatar.set_text("Assistant " + str(len(self.profile_settings) + 1))
    ...
    def on_create_clicked(self, button):
        ...
        copied_settings = [
            setting
            for setting in self.settings_switches
            if self.settings_switches[setting].get_active()
        ]
        if self.editing:
            self.parent.switch_profile(self.original_name)
        self.parent.create_profile(
            self.profile_name, self.pic_path, {}, copied_settings
        )
