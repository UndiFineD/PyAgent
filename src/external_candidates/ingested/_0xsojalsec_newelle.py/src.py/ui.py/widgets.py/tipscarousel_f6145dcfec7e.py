...
class TipsCarousel(Gtk.Box):
    ...
    def _build_ui(self):
        ...
        tips_label.add_css_class(
            "heading"
        )  # Use a suitable style class for a section title
        tips_label.set_halign(Gtk.Align.START)  # Center the title
        tips_label.set_margin_bottom(
            5
        )  # Reduce margin here, box spacing handles the rest
        ...
        self._carousel.set_vexpand(
            False
        )  # Prevent carousel from taking excessive vertical space
        self._carousel.set_halign(
            Gtk.Align.FILL
        )  # Fill available horizontal space allocated to it
        ...
        self._indicator_dots.set_carousel(
            self._carousel
        )  # Link indicators to the carousel
        self._indicator_dots.set_halign(
            Gtk.Align.CENTER
    ...
    def _create_tip_card(self, tip_data: dict) -> Gtk.Widget:
        ...
        title_label = Gtk.Label(
            wrap=True, halign=Gtk.Align.START, css_classes=["heading"]
        )
        ...
        tip_button.set_valign(
            Gtk.Align.FILL
        )  # Align cards to the top within the carousel
