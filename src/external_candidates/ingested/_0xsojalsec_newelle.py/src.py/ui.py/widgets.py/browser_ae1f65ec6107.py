# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\ui\widgets\browser.py
import json
import os
import threading
import urllib.parse
...
from gi.repository import Adw, GdkPixbuf, Gio, GLib, GObject, Gtk, WebKit
...
class BrowserWidget(Gtk.Box):
    ...
    def _is_url(self, text):
        ...
        return ("." in text and " " not in text) or text.startswith(
            ("http://", "https://", "file://", "ftp://")
        )
    ...
    def download_favicon(self):
        ...
        if self.last_favicon != favicon:
            load_image_with_callback(
                favicon, lambda pixbuf_loader: self.on_favicon_loaded(pixbuf_loader)
            )
    ...
    def _emit_page_changed(self):
        ...
        self.emit(
            "page-changed", self.current_url, self.current_title, self.current_favicon
        )
        if self.session_file:
