# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\ui\widgets\comborow.py
...
class ComboRowHelper(GObject.Object):
    ...
    def __init__(
        ...
        if len(options) > 10:
            expression = Gtk.PropertyExpression.new(self.ItemWrapper, None, "name")
            combo.set_expression(expression)
    ...
    def __on_selected(
        self, combo: Adw.ComboRow, selected_item: GObject.ParamSpec
    ) -> None:
        value = self.__combo.get_selected_item().value
        self.emit("changed", value)

    def __on_setup_listitem(
