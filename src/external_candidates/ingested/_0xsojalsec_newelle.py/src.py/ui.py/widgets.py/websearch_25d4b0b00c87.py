# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\ui\widgets\websearch.py
...
class WebSearchWidget(Gtk.Box):
    __gsignals__ = {"website-clicked": (GObject.SignalFlags.RUN_LAST, None, (str,))}
    ...
    def _create_website_row(self, title, link, favicon):
        ...
        button = Gtk.Button()
        button.connect(
            "clicked", lambda x, link=link: self.emit("website-clicked", link)
        )
        row_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        icon = Gtk.Image(icon_name="internet-symbolic")
        load_image_with_callback(
            favicon,
            lambda pixbuf_loader: icon.set_from_pixbuf(pixbuf_loader.get_pixbuf()),
        )
        row_box.append(icon)
        label = Gtk.Label(
            label=title,
    ...
    def add_website(self, title, link, favicon_path=None):
        ...
        row_box, new_spinner, new_spinner_revealer = self._create_website_row(
            title, link, favicon_path
        )
        self._current_spinner = new_spinner
        self._current_spinner_revealer = new_spinner_revealer
        row_revealer = Gtk.Revealer(
            transition_type=Gtk.RevealerTransitionType.SLIDE_UP, transition_duration=300
        )
