# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\utility\media.py
import base64
import fnmatch
import mimetypes
import os
...
def get_image_path(image_str: str):
    ...
    if image_str.startswith("data:image/jpeg;base64,"):
        ...
        saved_image = (
            "/tmp/" + image_str[len("data:image/jpeg;base64,") :][30:] + ".jpg"
        )
...
def extract_supported_files(
    history: list, supported_extensions: list, blacklist_formats: list = []
) -> list[str]:
    """
        ...
        for chunk in chunks:
            if chunk.type == "codeblock" and chunk.lang == "file":
                ...
                for file in files:
                    ...
                    if any(
                        fnmatch.fnmatch(file.lower(), pattern.lower())
                        for pattern in supported_extensions
                        if pattern != "plaintext"
                    ):
                        is_supported = True
                    ...
                    if is_supported:
                        if any(
                            fnmatch.fnmatch(file.lower(), pattern.lower())
                            for pattern in blacklist_formats
                        ):
                            continue
