        ...
        if "|" in potential_header and i + 1 < len(lines):
            ...
            if re.match(r"^\|? *[-:| ]+ *\|?$", potential_separator):
                ...
                if is_markdown_table(potential_table_block):
                    ...
                    chunks.append(
                        MessageChunk(type="table", text=potential_table_block)
                    )
...
def process_text_with_display_latex(text: str, allow_latex: bool) -> List[MessageChunk]:
    ...
    for match in _display_latex_pattern.finditer(text):
        ...
        if start > last_index:
            ...
            processed_intermediate = process_inline_elements(
                intermediate_text, allow_latex
            )
...
def process_text_segment_no_think(text: str, allow_latex: bool) -> List[MessageChunk]:
    ...
    for chunk in table_and_text_parts:
        if chunk.type == "table":
            ...
            latex_processed_parts = process_text_with_display_latex(
                chunk.text, allow_latex
            )
            # Extend the final list. These parts are already Text, Latex, LatexInline
            final_chunks.extend(
                latex_processed_parts
            )  # Extend, don't use append_chunk here
...
def get_message_chunks(message: str, allow_latex: bool = True) -> List[MessageChunk]:
    ...
    for chunk in flat_chunks:
        ...
        if (
            chunk.type == "text"
            and merged_flat_chunks
            and merged_flat_chunks[-1].type == "text"
        ):
            merged_flat_chunks[-1].text += (
                "\n" + chunk.text
            )  # Simple newline merge for now
        elif (
            chunk.type == "text"
            and chunk.text == ""
            and merged_flat_chunks
    ...
    merged_flat_chunks = [
        c for c in merged_flat_chunks if c.type != "text" or c.text != ""
    ]
    ...
    if current_inline_sequence:
        ...
        if is_mixed_or_multiple:
            grouped_chunks.append(
                MessageChunk(
                    type="inline_chunks", text="", subchunks=current_inline_sequence
                )
            )
        else:
