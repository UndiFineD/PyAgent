# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\extensions.py
import importlib
import inspect
import json
import os
...
class NewelleExtension(Handler):
    ...
    def restore_gtk_widget(
        self, codeblock: str, lang: str, msg_uuid=None
    ) -> Gtk.Widget | None:
        """
...
class ExtensionLoader:
    ...
    def __init__(
        ...
        if self.settings is None:
            ...
            self.extensions_settings = json.loads(
                self.settings.get_string("extensions-settings")
            )
    ...
    def load_integrations(self, AVAILABLE_INTEGRATIONS):
        for integration in AVAILABLE_INTEGRATIONS:
            integration_class = integration(
                self.pip, self.extension_cache, self.settings
            )
            ...
            if file.endswith(".py"):
                try:
                    spec = importlib.util.spec_from_file_location(
                        "newelle.name", os.path.join(self.extension_dir, file)
                    )
                    ...
                    for class_name, class_obj in module.__dict__.items():
                        if (
                            ...
                            and issubclass(class_obj, NewelleExtension)
                            and class_obj != NewelleExtension
                        ):
                            extension = class_obj(
                                self.pip, self.extension_cache, self.settings
                            )
                            ...
                            if extension.id in self.extensions_settings and (
                                "disabled" not in self.extensions_settings[extension.id]
                                or not self.extensions_settings[extension.id][
                                    "disabled"
                                ]
                            ):
    ...
    def set_handlers(
        ...
        rag: RAGHandler | None,
        memory: MemoryHandler | None,
        websearch: WebSearchHandler | None,
    ):
        for extension in self.extensions:
            extension.set_handlers(
                llm, stt, tts, secondary_llm, embedding, rag, memory, websearch
            )
    ...
    def add_extension(self, file_path: str):
        ...
        """
        shutil.copyfile(
            file_path, os.path.join(self.extension_dir, os.path.basename(file_path))
        )
    ...
    def save_settings(self):
        ...
        self.settings.set_string(
            "extensions-settings", json.dumps(self.extensions_settings)
        )
    ...
    def check_validity(self, extension: NewelleExtension):
        ...
        """
        if (
            not hasattr(extension, "id")
            or not hasattr(extension, "name")
            or len(extension.id) > 50
            or len(extension.name) > 50
            or extension.id == NewelleExtension.id
            or extension.name == NewelleExtension.name
        ):
    ...
    def check_handler(self, handler: dict, compare):
        if (
            "key" not in handler
            or "title" not in handler
            or "description" not in handler
            or "class" not in handler
        ):
            print(
                "Error: invalid handler, missing key or title or description or class"
            )
            return False
    ...
    def check_prompt(self, prompt):
        if (
            "key" not in prompt
            or "setting_name" not in prompt
            or "title" not in prompt
            or "description" not in prompt
        ):
            print(
                "Error: invalid prompt, missing key or setting_name or title or description"
            )
            return False
        return True
    ...
    def postprocess_history(self, history: list, bot_response: str) -> tuple[list, str]:
        ...
        for extension in self.get_enabled_extensions():
            try:
                history, bot_response = extension.postprocess_history(
