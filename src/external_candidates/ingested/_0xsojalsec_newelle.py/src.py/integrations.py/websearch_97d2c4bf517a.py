# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Newelle\src\integrations\websearch.py
import json
import os
...
class WebsearchIntegration(NewelleExtension):
    ...
    def get_answer(self, codeblock: str, lang: str) -> str | None:
        ...
        if self.websearch.supports_streaming_query():
            text, sources = self.websearch.query_streaming(
                codeblock,
                lambda title, link, favicon, codeblock=codeblock, msgid=msgid: self.add_website(
                    codeblock, title, link, favicon, msgid
                ),
            )
        else:
    ...
    def load_search_widget(self, query, sources, result):
        ...
        widget.connect(
            "website-clicked",
            lambda widget, link: self.ui_controller.open_link(
    ...
    def restore_gtk_widget(self, codeblock: str, lang: str, msgid) -> Gtk.Widget | None:
        ...
        if cache is not None:
            return self.load_search_widget(
                codeblock, cache["websites"], cache["result"]
            )
        else:
