# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-node-images-comfyui\assets\modify-server.py
import asyncio
import glob
import json
import os
...
class PromptServer:
    def __init__(self, loop):
        ...
        self.app = web.Application(
            client_max_size=20971520, middlewares=[cache_control, cors_handler]
        )
        self.sockets = dict()
            ...
            if "filename" in request.rel_url.query:
                ...
                if os.path.isfile(file):
                    return web.FileResponse(
                        file, headers={"Content-Disposition": f'filename="{filename}"'}
                    )
