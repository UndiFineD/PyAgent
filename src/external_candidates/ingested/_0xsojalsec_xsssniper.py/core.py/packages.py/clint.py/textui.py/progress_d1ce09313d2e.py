# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-xsssniper\core\packages\clint\textui\progress.py
# -*- coding: utf-8 -*-
...
def bar(
    ...
    def _show(_i):
        if (time.time() - bar.etadelta) > ETA_INTERVAL:
            ...
            bar.ittimes = bar.ittimes[-ETA_SMA_WINDOW:] + [
                -(bar.start - time.time()) / (_i + 1)
            ]
