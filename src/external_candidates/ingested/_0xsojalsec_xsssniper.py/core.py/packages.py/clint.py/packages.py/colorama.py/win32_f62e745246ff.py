# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-xsssniper\core\packages\clint\packages\colorama\win32.py
...
try:
    ...
    from ctypes import Structure, byref, c_char, c_short, c_uint32, c_ushort
...
if __name__ == "__main__":
    ...
    print(x)
    print(
        "dwSize(height,width)                    = (%d,%d)" % (x.dwSize.Y, x.dwSize.X)
    )
    print(
        "dwCursorPosition(y,x)                   = (%d,%d)"
        % (x.dwCursorPosition.Y, x.dwCursorPosition.X)
    )
    print(
        "wAttributes(color)                      =  %d = 0x%02x"
        % (x.wAttributes, x.wAttributes)
    )
    print(
        "srWindow(Top,Left)-(Bottom,Right)       = (%d,%d)-(%d,%d)"
        % (x.srWindow.Top, x.srWindow.Left, x.srWindow.Bottom, x.srWindow.Right)
    )
    print(
        "dwMaximumWindowSize(maxHeight,maxWidth) = (%d,%d)"
