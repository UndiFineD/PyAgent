# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\tests\conftest.py
# -*- coding: utf-8 -*-
...
def mock_redis_client_sync():
    with patch(
        "app.services.chat_agent.helpers.run_helper.get_redis_client",
...
def llm() -> BaseLanguageModel:
    fake_llm = FakeMessagesListChatModel(
        responses=[HumanMessage(content=f"{i}") for i in range(100)]
    )
    return fake_llm
...
) -> AgentExecutor:  # pylint: disable=redefined-outer-name
    agent_config = get_agent_config()
    return create_meta_agent(
        agent_config=agent_config,
        get_llm_hook=lambda type, key: llm,  # pylint: disable=unused-argument
    )


@pytest.fixture
def test_client(
    meta_agent: AgentExecutor,
) -> TestClient:  # pylint: disable=redefined-outer-name
