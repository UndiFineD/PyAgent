# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\tests\api\test_chat.py
# -*- coding: utf-8 -*-
import json
import time
...
from app.schemas.streaming_schema import (
    StreamingData,
...
@pytest.mark.asyncio
async def test_chat(
    test_client: TestClient, chat_query: dict[str, Any]
    ...
    while not received_end:
        ...
        assert (
            json.loads(response_data[0])["data_type"]
            == StreamingDataTypeEnum.SIGNAL.value
        )
        ...
        if len(response_data) > 1:
            assert (
                json.loads(response_data[1])["data_type"]
                == StreamingDataTypeEnum.ACTION.value
            )
        ...
        if len(response_data) > 2:
            assert (
                json.loads(response_data[2])["data_type"]
                == StreamingDataTypeEnum.SIGNAL.value
            )
            assert (
                json.loads(response_data[2])["data"]
                == StreamingSignalsEnum.LLM_END.value
            )
            assert "run_id" in json.loads(response_data[2])["metadata"]

        if len(response_data) > 3:
            assert (
                json.loads(response_data[3])["data_type"]
                == StreamingDataTypeEnum.SIGNAL.value
            )
            assert (
                json.loads(response_data[3])["data"]
                == StreamingSignalsEnum.TOOL_END.value
            )
            assert "run_id" in json.loads(response_data[3])["metadata"]

        if len(response_data) > 4:
            assert (
                json.loads(response_data[4])["data_type"]
                == StreamingDataTypeEnum.SIGNAL.value
            )
            assert (
                json.loads(response_data[4])["data"] == StreamingSignalsEnum.END.value
            )
