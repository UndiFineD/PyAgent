# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\tests\tools\test_visualizer_tool.py
# -*- coding: utf-8 -*-
...
def mock_llm_call(llm: BaseLanguageModel):  # pylint: disable=redefined-outer-name
    ...
    with patch(
        "app.services.chat_agent.tools.library.visualizer_tool.visualizer_tool.get_llm",
        return_value=llm,
    ):
        yield
...
@pytest.mark.asyncio
async def test_visualizer_tool_arun(
    tool_input: str, run_manager: AsyncCallbackManagerForToolRun
):
    """
    ...
    tool = JsxVisualizerTool.from_config(
        config=agent_config.tools_library.library["visualizer_tool"],
