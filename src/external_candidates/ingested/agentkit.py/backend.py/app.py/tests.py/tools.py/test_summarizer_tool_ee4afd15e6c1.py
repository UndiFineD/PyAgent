# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\tests\tools\test_summarizer_tool.py
# -*- coding: utf-8 -*-
...
from app.services.chat_agent.tools.library.summarizer_tool.summarizer_tool import (
    SummarizerTool,
)
...
def mock_llm_call(llm: BaseLanguageModel):  # pylint: disable=redefined-outer-name
    with patch(
        "app.services.chat_agent.tools.library.summarizer_tool.summarizer_tool.get_llm",
        return_value=llm,
    ):
        yield
...
def summarizer_tool(agent_config: AgentConfig) -> SummarizerTool:
    summarizer_tool = SummarizerTool.from_config(
        config=agent_config.tools_library.library["summarizer_tool"],
...
async def test_summarizer_tool_run_short_input(
    agent_config, summarizer_tool, summarizer_tool_input
):
...
@pytest.mark.skip(
    reason="Need to install transformers package to Github workflows env."
)
@pytest.mark.asyncio
@pytest.mark.parametrize("summarizer_tool_input", [True], indirect=True)
async def test_summarizer_tool_run_long_input(
    agent_config, summarizer_tool, summarizer_tool_input
):
    query = summarizer_tool_input
    output = await summarizer_tool._arun(query)
