# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\app\services\chat_agent\tools\library\sql_tool\sql_tool.py
# -*- coding: utf-8 -*-
...
class SQLTool(ExtendedBaseTool):
    ...
    def from_config(
        ...
        **kwargs: Any,
        ...
        return cls(
            ...
            prompt_message=config.prompt_message.format(
                **{e.name: e.content for e in config.prompt_inputs}
            ),
            system_context=config.system_context.format(
                **{e.name: e.content for e in config.prompt_inputs}
            ),
            prompt_selection=(
                config.prompt_selection.format(
                    **{e.name: e.content for e in config.prompt_inputs}
                )
            ...
            system_context_selection=(
                config.system_context_selection.format(
                    **{e.name: e.content for e in config.prompt_inputs}
                )
            ...
            prompt_validation=(
                config.prompt_validation.format(
                    **{e.name: e.content for e in config.prompt_inputs}
                )
            ...
            system_context_validation=(
                config.system_context_validation.format(
                    **{e.name: e.content for e in config.prompt_inputs}
                )
            ...
            prompt_refinement=(
                config.prompt_refinement.format(
                    **{e.name: e.content for e in config.prompt_inputs}
                )
        ...
        try:
            ...
            if results is None:
                ...
                sample_rows = list(
                    map(
                        lambda ls: [
                            f"{str(i)[:100]}..." if len(str(i)) > 100 else str(i)
                            for i in ls
                        ],
                        sample_rows_result,
                    )
                )
                sample_rows_str = ";".join(
                    [",".join(row) for row in sample_rows]
                ).replace(
                    "\n",
                    "",
                )
                results_str = f"total rows from SQL query: {len(results)}, first {self.nb_example_rows} rows: {sample_rows_str}"
    ...
    async def _alist_sql_tables(
        ...
        table_messages = [
            SystemMessage(
                content=(
                    self.system_context_selection
                    if self.system_context_selection
                    else ""
                )
            ),
            HumanMessage(
                content=(
                    self.prompt_selection.format(question=query)
    ...
    async def _aquery_with_schemas(
        ...
        db_table_infos = (
            sql_tool_db.db_info.tables
            if sql_tool_db and sql_tool_db.db_info is not None
            else []
        )
        table_schemas = "\n".join(
            [
