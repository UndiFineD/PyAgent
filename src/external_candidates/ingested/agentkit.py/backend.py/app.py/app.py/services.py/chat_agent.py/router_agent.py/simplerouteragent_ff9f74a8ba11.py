        ...
        while self.action_plan is None:
            try:
                ...
                action_plan = ActionPlan(
                    **self.action_plans.action_plans[full_output].dict()
                )
                ...
                if retries > 3:
                    raise ValueError(
                        f"Invalid action plan selected ({retries}x)"
                    ) from e

        # Router agent follows action plan
        if len(self.action_plan.actions) > 0:
            logger.info(
                f"Next action plan step ({len(self.action_plan.actions)} remaining)"
            )
            ...
            tool_input = ToolInputSchema(
                ...
                chat_history=[],
                user_settings=(
                    UserSettings(**kwargs["user_settings"].dict())
                    if kwargs["user_settings"]
                    else None
                ),
                intermediate_steps={},
            )
            ...
            SystemMessagePromptTemplate.from_template(
                system_context.format(
                    action_plans="\n".join(
                        [
                            f"{k}: {v.description}"
                            for k, v in action_plans.action_plans.items()
                        ]
                    )
                )
            ),
