# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\app\services\chat_agent\tools\library\pdf_tool\pdf_tool.py
# -*- coding: utf-8 -*-
...
class PDFTool(ExtendedBaseTool):
        ...
        **kwargs: Any,
        ...
        return cls(
            ...
            system_context_refinement=(
                config.system_context_refinement.format(
                    **{e.name: e.content for e in config.prompt_inputs}
                )
    ...
    def _save_to_csv(query: str, docs: list, result: str) -> None:
        ...
        file_name = datetime.datetime.now().strftime(
            f"{settings.PDF_TOOL_LOG_QUERY_PATH}/%Y-%m-%d.csv"
        )
    ...
    async def _aqa_pdf_chunks(
        ...
        response = await self._agenerate_response(
            question_messages, discard_fast_llm=True, run_manager=run_manager
        )
        return response
