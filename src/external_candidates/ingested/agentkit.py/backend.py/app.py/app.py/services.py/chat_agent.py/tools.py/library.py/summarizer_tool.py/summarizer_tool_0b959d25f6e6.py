# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\app\services\chat_agent\tools\library\summarizer_tool\summarizer_tool.py
# -*- coding: utf-8 -*-
...
class SummarizerTool(ExtendedBaseTool):
    ...
    def from_config(
        ...
        **kwargs: Any,
        ...
        prompt_template_message = config.prompt_message.format(
    ...
    async def _arun(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            tool_outputs = [
                f"{k}: {v}" for k, v in tool_input.intermediate_steps.items()
            ]
            assert (
                self.max_token_length is not None
            ), "max_token_length must not be None"
            if get_token_length(query) <= self.max_token_length:
                docs = [
                    Document(page_content=tool_output) for tool_output in tool_outputs
                ]
                ...
                texts = [
                    ...
                    for text in text_splitter.split_text(tool_output)
                ]
