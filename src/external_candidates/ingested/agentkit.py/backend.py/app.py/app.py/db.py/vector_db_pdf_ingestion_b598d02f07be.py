# Extracted from: C:\DEV\PyAgent\.external\agentkit\backend\app\app\db\vector_db_pdf_ingestion.py
# -*- coding: utf-8 -*-
import csv
import logging
import os
    ...
    def run(
        ...
        if folder_path is not None:
            return self._load_documents(
                folder_path=folder_path, collection_name=collection_name
            )
            ...
            for file_name in files:
                ...
                if not self._file_already_loaded(file_path, collection_name):
                    ...
                    # Load Markdown or Plain Text files
                        ...
                        file_type = (
                            "markdown" if file_extension == ".md" else "plain text"
                        )
                        logger.info(
                            f"Loading data from {file_name} as Document ({file_type})..."
                        )
                        try:
                            ...
                            logger.error(
                                f"Could not load {file_type} file {file_name}: {repr(e)}"
                            )
                    ...
                    # Load CSV files
                        ...
                        try:
                            ...
                            logger.error(
                                f"Could not load CSV file {file_name}: {repr(e)}"
                            )
