...
class Pose2VideoLongSVDPipeline(DiffusionPipeline):
    ...
    def _clip_encode_image(
        ...
        if do_classifier_free_guidance:
            ...
            vasa_prompts = torch.stack(vasa_prompts, dim=0).to(
                device=device, dtype=dtype
            )
            ...
            uncond_vasa_prompts = torch.stack(uncond_vasa_prompts, dim=0).to(
                device=device, dtype=dtype
            )
            uncond_vasa_prompts = uncond_vasa_prompts.unsqueeze(0).unsqueeze(2)

            pad_uncond_audio_prompt = uncond_audio_prompts[:, :1].repeat(
                1, frames_per_batch, 1, 1
            )
            pad_uncond_vasa_prompt = uncond_vasa_prompts[:, :1].repeat(
                1, frames_per_batch, 1, 1
            )
            audio_prompts = torch.cat([audio_prompts, pad_uncond_audio_prompt], dim=1)
            uncond_audio_prompts = torch.cat(
                [uncond_audio_prompts, pad_uncond_audio_prompt], dim=1
            )
            vasa_prompts = torch.cat([vasa_prompts, pad_uncond_vasa_prompt], dim=1)
            uncond_vasa_prompts = torch.cat(
                [uncond_vasa_prompts, pad_uncond_vasa_prompt], dim=1
            )
    ...
    def _encode_vae_image(
        ...
        if do_classifier_free_guidance:
            ...
            image_latents = torch.cat(
                [negative_image_latents, image_latents, image_latents, image_latents]
            )
    ...
    def __call__(
        ...
        image_latents = image_latents.unsqueeze(1).repeat(
            1, num_frames + frames_per_batch, 1, 1, 1
        )
        ...
        exp_mask_tensor = exp_mask_tensor.to(
            device=device, dtype=self.pose_guider.dtype
        )
        mouth_mask_tensor = mouth_mask_tensor.to(
            device=device, dtype=self.pose_guider.dtype
        )
        ...
        guidance_scale = torch.linspace(
            min_guidance_scale3, max_guidance_scale3, num_inference_steps
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for batch, index_start in enumerate(
                    ...
                    pose_cond_fea = pose_cond_fea.repeat(
                        4 if do_classifier_free_guidance else 1, 1, 1, 1, 1
                    )
                    ...
                    if do_classifier_free_guidance:
                        ...
                        noise_pred = (
                            ...
                            * (noise_pred_drop_audio_vasa - noise_pred_uncond)
                            + self.guidance_scale2[i]
                            * (noise_pred_drop_vasa - noise_pred_drop_audio_vasa)
                            + self.guidance_scale3[i]
                            * (noise_pred_cond - noise_pred_drop_vasa)
                        )
        ...
        if not output_type == "latent":
            ...
            frames = self.decode_latents(
                latents, num_frames + frames_per_batch, decode_chunk_size
            )[:, :, :num_frames]
        else:
            frames = latents
