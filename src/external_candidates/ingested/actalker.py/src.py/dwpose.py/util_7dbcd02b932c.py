# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\dwpose\util.py
import math
...
def draw_bodypose(canvas, candidate, subset, score, size=2, stickwidth=2):
    H, W, C = canvas.shape
...
def draw_handpose(canvas, all_hand_peaks, all_hand_scores, size=4, thickness=2):
    H, W, C = canvas.shape
    ...
    for peaks, scores in zip(all_hand_peaks, all_hand_scores):
        ...
        for i, keyponit in enumerate(peaks):
            ...
            if x > eps and y > eps:
                cv2.circle(canvas, (x, y), size, (0, 0, score), thickness=-1)
    return canvas


def draw_facepose(canvas, all_lmks, all_scores, size=3):
    H, W, C = canvas.shape
    for lmks, scores in zip(all_lmks, all_scores):
        for lmk, score in zip(lmks, scores):
            ...
            conf = int(score * 255)
            if x > eps and y > eps:
                # cv2.circle(canvas, (x, y), 3, (conf, conf, conf), thickness=-1)  原始
                cv2.circle(canvas, (x, y), size, (conf, conf, conf), thickness=-1)
    return canvas


def draw_facepose_NoOutline(canvas, all_lmks, all_scores, size=3):
    H, W, C = canvas.shape
    for lmks, scores in zip(all_lmks, all_scores):
        # import pdb; pdb.set_trace()
        for lmk, score in zip(lmks[17:, :], scores[17:]):
            x, y = lmk
            x = int(x * W)
            y = int(y * H)
            conf = int(score * 255)
...
def draw_pose(pose, H, W, ref_w=2160):
    ...
    canvas = draw_bodypose(
        canvas, candidate, subset, score=bodies["score"], size=4, stickwidth=4
    )
    ########################################### draw hand pose #####################################################
    canvas = draw_handpose(canvas, hands, pose["hands_score"], size=4, thickness=2)
    ########################################### draw face pose #####################################################
    canvas = draw_facepose(canvas, faces, pose["faces_score"], size=3)
    return cv2.cvtColor(cv2.resize(canvas, (W, H)), cv2.COLOR_BGR2RGB).transpose(
        2, 0, 1
    )


def draw_pose_NoFace(pose, H, W, ref_w=576):
    """vis dwpose outputs
    Args:
        pose (List): DWposeDetector outputs in dwpose_detector.py
        H (int): height
...
def draw_pose_NoOutline(pose, H, W, ref_w=576):
    """vis dwpose outputs
    ...
    canvas = draw_handpose(canvas, hands, pose["hands_score"], size=4, thickness=2)
    ########################################### draw face pose #####################################################
    canvas = draw_facepose_NoOutline(
        canvas, faces, pose["faces_score"], size=3
    )  # 要不要改成 2？
...
def draw_handmasks(pose, H, W, thresh=0.3):
...
def draw_pose_NoFace_OnCanvas(pose, canvas, H, W, ref_w=576):
    """vis dwpose outputs
    Args:
        pose (List): DWposeDetector outputs in dwpose_detector.py
        H (int): height
