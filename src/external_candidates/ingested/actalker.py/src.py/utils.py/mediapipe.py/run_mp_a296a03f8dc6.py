...
def extract_and_draw_lmks(input_file, output_file):
    ...
    for i in trange(200):
        ...
        if tgt_result is not None:
            ...
            IRIS_L_x = (iris_left[0] - LE_kps[:, 0].min()) / (
                LE_kps[:, 0].max() - LE_kps[:, 0].min()
            )
            IRIS_L_y = (iris_left[1] - LE_kps[:, 1].min()) / (
                LE_kps[:, 1].max() - LE_kps[:, 1].min()
            )
            IRIS_R_x = (iris_right[0] - RE_kps[:, 0].min()) / (
                RE_kps[:, 0].max() - RE_kps[:, 0].min()
            )
            IRIS_R_y = (iris_right[1] - RE_kps[:, 1].min()) / (
                RE_kps[:, 1].max() - RE_kps[:, 1].min()
            )

            euler_angles, translation_vector = matrix_to_euler_and_translation(
                tgt_result["trans_mat"]
            )
            ...
            lmks_trans = project_points_with_trans(
                lmks3d, new_pose, [frame.shape[0], frame.shape[1]]
            )
