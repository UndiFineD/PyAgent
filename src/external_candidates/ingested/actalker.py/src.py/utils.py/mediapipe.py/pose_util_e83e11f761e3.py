...
def project_points(points_3d, transformation_matrix, pose_vectors, image_shape):
    ...
    for i in range(L):
        ...
        projected_points_frame[:, 0] = (
            projected_points_frame[:, 0] + 1
        ) * 0.5  # * image_shape[1]
        projected_points_frame[:, 1] = (
            projected_points_frame[:, 1] + 1
        ) * 0.5  # * image_shape[0]
...
def invert_projection(
    ...
    for i in range(L):
        ...
        projected_points_homogeneous = np.hstack(
            [projected_points_frame, np.ones((N, 1))]
        )
        ...
        transformation_inv = np.linalg.inv(
            transformation_matrix
            @ euler_and_translation_to_matrix(pose_vectors[i][:3], pose_vectors[i][3:])
        )
        points_3d_homogeneous = transformed_points @ transformation_inv.T

        # Convert back to non-homogeneous coordinates
        points_3d_frame = (
            points_3d_homogeneous[:, :3] / points_3d_homogeneous[:, 3, np.newaxis]
        )
