...
def align_N(img_im, orgi_landmarks, tar_landmarks, align_h, align_w, rnd_scale=1):
    ...
    pts2 = np.float64(
        np.matrix([[point[0], point[1]] for point in tar_landmarks_scale])
    )
...
def align_N_picasso_aug2(
    img_im, orgi_landmarks, tar_landmarks, align_size, rnd_scale=1.0
):
    pts1 = np.float64(np.matrix([[point[0], point[1]] for point in orgi_landmarks]))
    pts2 = np.float64(np.matrix([[point[0], point[1]] for point in tar_landmarks]))
    M = transformation_from_points(pts1, pts2)
...
def align_N_picasso_aug3(
    ...
    mouth, mouth_M, mouth_info = crop_part_aug3(
        img_im, pts_phase1[54:76], M, align_size, rnd_scale
    )

    return [left_part, right_part, mouth], [left_M, right_M, mouth_M], mouth_info


def align_N_picasso_aug4(
    img_im, orgi_landmarks, tar_landmarks, net_size, rnd_scale=1.0
):
    ...
    # leye
    orgi_eye_landmarks = np.concatenate(
        (orgi_landmarks[0:8, :], orgi_landmarks[16:24, :]), axis=0
    )
    ...
    # reye
    orgi_reye_landmarks = np.concatenate(
        (orgi_landmarks[8:16, :], orgi_landmarks[24:32, :]), axis=0
    )
    tar_reye_landmarks = mean_landmarks_reye
    pts1 = np.float64(
        np.matrix([[point[0], point[1]] for point in orgi_reye_landmarks])
    )
    ...
    pts1 = np.float64(
        np.matrix([[point[0], point[1]] for point in orgi_mouth_landmarks])
    )
    pts2 = np.float64(
        np.matrix([[point[0], point[1]] for point in tar_mouth_landmarks])
    )
    ...
    left_part, left_M = crop_part_aug2(
        img_im, tar_eye_landmarks, M_eye, net_size, rnd_scale
    )
    right_part, right_M = crop_part_aug2(
        img_im, tar_reye_landmarks, M_reye, net_size, rnd_scale, flip=True
    )
    mouth, mouth_M = crop_part_aug2(
        img_im, tar_mouth_landmarks, M_mouth, net_size, rnd_scale
    )

    return [left_part, right_part, mouth], [left_M, right_M, mouth_M]


def align_N_picasso_aug5(
    img_im, orgi_landmarks, tar_landmarks, align_size, rnd_scale=1.0
):
    pts1 = np.float64(np.matrix([[point[0], point[1]] for point in orgi_landmarks]))
    pts2 = np.float64(np.matrix([[point[0], point[1]] for point in tar_landmarks]))
...
def align_N_picassoV2(
    img_im, orgi_landmarks, tar_landmarks, align_size, pred_p1, rnd_scale=1.0
):
    pts1 = np.float64(np.matrix([[point[0], point[1]] for point in orgi_landmarks]))
    pts2 = np.float64(np.matrix([[point[0], point[1]] for point in tar_landmarks]))
    M = transformation_from_points(pts1, pts2)
