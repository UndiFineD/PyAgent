...
def phase1(device, p1_path):
    ...
    in_channel = 3
    meanf = os.path.join(
        BASE_DIR, "meanfiles/mean_pts130_scale112_full_flip_phase1.txt"
    )
...
def phase2(device, p2_path):
    ...
    in_channel = 9
    meanf = os.path.join(
        BASE_DIR, "meanfiles/mean_pts130_scale112_full_flip_phase2.txt"
    )
...
class RefinePts(object):
    def __init__(
        self, device="cuda", p1_path="checkpoints/p1.pt", p2_path="checkpoints/p2.pt"
    ):
        ...
        self.mean_ld0 = read_pts(
            os.path.join(BASE_DIR, "meanfiles/face_mean_5.txt")
        ) * [self.net1_w / 112.0, self.net1_h / 112.0]
    ...
    def __call__(self, im_cv2, pts_list):
        ...
        for pts in pts_list:
            ...
            while dup < 3:
                ...
                pre_face = True
                if (
                    abs(label - pre_conf1) < 0.0001
                    and abs(pre_conf2 - pre_conf1) < 0.0001
                    and label > 0.85
                ):
                    break
                pre_conf2 = pre_conf1
