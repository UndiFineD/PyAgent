# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\models\audio_adapter\audio_proj.py
"""
...
class VasaProjModel(ModelMixin):
    """VasaProjModel"""
    ...
    def __init__(
        ...
        self.proj1 = nn.Linear(self.input_dim, self.output_dim)
        self.norm = nn.LayerNorm(output_dim)
...
class IDProjModel(ModelMixin):
    ...
    def forward(self, audio_embeds):
        ...
        context_tokens = self.proj3(audio_embeds)
        return context_tokens


class ExpProjModel(ModelMixin):
    def __init__(
        self,
        input_dim=512 + 6,
        output_dim=768,
...
class MotionControlProjModel(ModelMixin):
    def __init__(
        self,
        input_dim=1024 + 1024,
        output_dim=768,
        ...
        super().__init__()
        # define multiple linear layers
        self.proj1 = nn.Linear(input_dim, 256)
        self.proj2 = nn.Linear(256, 8)
        self.proj3 = nn.Linear(8 * 32, 2)

    def forward(self, audio_embeds, id_embeds):
        mean_audio = audio_embeds.mean(1)
        mean_id = id_embeds.mean(1)
        embeds = torch.cat([mean_audio, mean_id.expand(1, 32, 1024)], dim=2)
        output = self.proj1(embeds)
        output = torch.relu(output)
        output = torch.relu(self.proj2(output))
        output = rearrange(output, "bz f c -> bz (f c)")
