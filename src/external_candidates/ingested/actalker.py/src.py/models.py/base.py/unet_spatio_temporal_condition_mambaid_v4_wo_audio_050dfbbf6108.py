# Extracted from: C:\DEV\PyAgent\.external\ACTalker\src\models\base\unet_spatio_temporal_condition_mambaID_v4_wo_audio.py
import os
...
from .TransformerSTmodel import (
    TransformerSpatioTemporalModel_mambaID_v4_wo_audio as TransformerSpatioTemporalModel,
)
...
class UNetSpatioTemporalConditionModel(
    ...
    def __init__(
        ...
        num_frames: int = 25,
        attn_cls: nn.Module = TransformerSpatioTemporalModel,
    ):
        super().__init__()
...
def add_ip_adapters(
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None:
            ...
            for i in range(len(num_adapter_embeds)):
                weights.update(
                    {f"to_k_ip.{i}.weight": unet_sd[layer_name + ".to_k.weight"]}
                )
                weights.update(
                    {f"to_v_ip.{i}.weight": unet_sd[layer_name + ".to_v.weight"]}
                )
    ...
    adapter_modules = torch.nn.ModuleList(
        [
            m
            for m in unet.attn_processors.values()
            if isinstance(m, IPAdapterAttnProcessor)
            or isinstance(m, IPAdapterAttnProcessor2_0)
        ]
    )
...
def load_adapter_states(adapter_modules, state_dict_list):
    ...
    info = adapter_modules.load_state_dict(merged_stete_dict, strict=False)
    return info
