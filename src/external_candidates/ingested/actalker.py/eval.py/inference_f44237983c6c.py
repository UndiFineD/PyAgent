...
def driven_video(
    ...
    generate_image_loader = DataLoader(
        video_data, batch_size=opt.batch_size, num_workers=1, shuffle=False
    )
    if source_image_path is not None:
        ...
        faces = DeepFace.analyze(
            source_image[:, :, ::-1], actions=["emotion"], enforce_detection=False
        )
    ...
    source_image = (
        source_image_tensor.clone().permute((1, 2, 0)).numpy() * 255
    ).astype(np.uint8)[:, :, ::-1]
    ...
    vs, global_descriptor, _ = generator.extract_feature(
        source_image_tensor, source_face_image_tensor, source_pose
    )
    ...
    for data in generate_image_loader:
        ...
        driven_images = (
            driven_images_tensor.clone().permute((0, 2, 3, 1)).numpy() * 255
        ).astype(np.uint8)[:, :, :, ::-1]
        ...
        output = generator.drive_feature(
            vs_batch, global_descriptor_batch, driven_face_images_tensor, driven_pose
        )
        # output = generator.drive_feature(vs_batch, global_descriptor_batch, driven_face_images_tensor, source_pose)

        output_images = (
            (output.permute((0, 2, 3, 1)).cpu().detach().numpy() * 255)
            .clip(0, 255)
            .astype(np.uint8)[:, :, :, ::-1]
        )
...
def main():
    ...
    parser.add_argument(
        "--config", default="config/train-stage1.yaml", help="path to config"
    )
    ...
    out_dir = os.path.join(
        opt.save_dir, os.path.splitext(os.path.basename(opt.model_path))[0]
    )
    checkpoint = torch.load(opt.model_path, map_location="cpu")

    pose_model = HeadPose_train().to(device)
    generator = create_instance_by_name(
        config["model_params"]["common_params"]["generator_type"]
    )(config, device)
    ...
    pose_model.eval()
    source_image_paths = [
        line.strip() for line in open(opt.source_image_list, "r").readlines()
    ]
    driven_video_paths = [
        line.strip() for line in open(opt.video_list, "r").readlines()
    ]
    if opt.num > 0:
        driven_video_paths = driven_video_paths[: opt.num]
    for index, driven_video_path in tqdm(enumerate(driven_video_paths[:])):
        video_name = (
            os.path.splitext("/".join(driven_video_path.split("/")[-name_prefix:]))[0]
            + ".mp4"
        )
