# Extracted from: C:\DEV\PyAgent\.external\ACTalker\utils\data_preprocess.py
import glob
import os
import pdb
import random
...
def calculate_global_bbox_with_margin(
    bboxes, frame_width, frame_height, margin_ratio=0.1
):
    """
...
def crop_video(input_video_path, output_video_path, bboxes):
    ...
    # 计算全局裁剪区域
    x1_min, y1_min, x2_max, y2_max = calculate_global_bbox_with_margin(
    ...
    out = cv2.VideoWriter(
        output_video_path, fourcc, fps, (cropped_width, cropped_height)
    )
...
def show_box(box, image):
    x0, y0, x1, y1 = box
    cv2.rectangle(
        image, (int(x0), int(y0)), (int(x1), int(y1)), color=(0, 255, 0), thickness=2
    )
...
def get_largest_bbox(detections):
    ...
    largest_bbox = max(
        detections, key=lambda det: (det[2] - det[0]) * (det[3] - det[1])
    )
    return largest_bbox
...
def step1_scene_detection(video_path, output_folder):
    ...
    split_video_ffmpeg(
        ...
        scene_list,
        output_folder,
        arg_override="-c:v libx264 -crf 23 -preset veryfast",
    )
    # 释放资源
...
def step2_sam_process(input_folder, save_folder):
    ...
    for video in tqdm(videos):
        try:
            ...
            sam_process_video(
                video, bbox, os.path.join(save_folder, os.path.basename(video))
            )
...
if __name__ == "__main__":
    ...
    step2_sam_process(
        "/apdcephfs_cq8/share_1367250/harlanhong/src/fating-novel-view-human/demo2_output_clips",
        "/apdcephfs_cq8/share_1367250/harlanhong/src/fating-novel-view-human/demo2_output_clips_sam",
    )

    step3_crop_video(
        "/apdcephfs_cq8/share_1367250/harlanhong/src/fating-novel-view-human/demo2_output_clips_sam",
        "/apdcephfs_cq8/share_1367250/harlanhong/src/fating-novel-view-human/demo2_output_clips_sam_crop",
    )
