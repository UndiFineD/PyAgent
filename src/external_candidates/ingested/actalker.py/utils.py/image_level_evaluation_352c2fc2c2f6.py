# Extracted from: C:\DEV\PyAgent\.external\ACTalker\utils\image_level_evaluation.py
import os
import time
...
def lpips_metric(video1, video2):
    ...
    for frame1, frame2 in zip(video1_torch, video2_torch):
        lpips_values.append(
            loss_fn_alex(frame1.unsqueeze(0), frame2.unsqueeze(0)).item()
        )
...
def image_level_evaluation(
    input_list, num_pil=None, verbose=False, output_dir="./debug/"
):
        ...
        if verbose:
            for i, (gt_patch, res_patch) in enumerate(zip(gt, res)):
                gt_path = os.path.join(
                    output_dir,
                    f"gt_{os.path.basename(image_path).split('.')[0]}_{i}.png",
                )
                res_path = os.path.join(
                    output_dir,
                    f"res_{os.path.basename(image_path).split('.')[0]}_{i}.png",
                )
