# Extracted from: C:\DEV\PyAgent\.external\agi\uni.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    for domain, (x, y_true, desc) in problems.items():
        ...
        results[domain] = {
            ...
            "mse" if domain != "parity" else "accuracy": (
                mse if domain != "parity" else accuracy
            ),
        }
    ...
    print("=" * 60)
    print(
        f"✓ Routing Accuracy: {correct_routing}/{len(problems)} ({correct_routing/len(problems):.2%})"
    )
