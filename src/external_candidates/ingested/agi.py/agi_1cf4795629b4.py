# Extracted from: C:\DEV\PyAgent\.external\agi\agi.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
WEIGHTS_DIR = (
    Path(__file__).parent / "weights" if "__file__" in globals() else Path("weights")
)
...
def get_parity_dataset(
    n_bits: int = 64, k: int = 3, size: int = 10
...
def generate_wave_data(
    ...
    def step(u_t, u_tm1):
        ...
        u_tp1[1:-1] = (
            2 * u_t[1:-1] - u_tm1[1:-1] + lam**2 * (u_t[2:] - 2 * u_t[1:-1] + u_t[:-2])
        )
        u_tp1[0] = u_tp1[-1] = 0.0
        return u_tp1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.shape[1] != self.input_dim:
            x = (
                x[:, : self.input_dim]
                if x.shape[1] > self.input_dim
                else F.pad(x, (0, self.input_dim - x.shape[1]))
            )
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        return self.out(x)
