...
def initialize_models(
    model_name: str = "Vyvo/VyvoTTS-LFM2-Neuvillette", device: str = "cuda"
):
...
def parse_output_to_audio(
    generated_ids: torch.Tensor, snac_model
...
def process_single_prompt(
    ...
    # Audio processing
    audio_samples, audio_processing_time = parse_output_to_audio(
        generated_ids, snac_model
    )
...
def main():
    ...
    for i, prompt in enumerate(prompts, 1):
        audio_sample, timing = process_single_prompt(
            prompt, snac_model, model, tokenizer, chosen_voice
        )
