# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-withoutbg\examples\custom_backgrounds.py
#!/usr/bin/env python3
...
def gradient_background():
    ...
    for i, grad in enumerate(gradients):
        ...
        background = create_gradient(
            width, height, grad["start"], grad["end"], grad["direction"]
        )
...
def professional_studio_background():
    ...
    if foreground.mode == "RGBA":
        ...
        studio_bg2 = Image.alpha_composite(
            studio_bg2.convert("RGBA"), shadow_img
        ).convert("RGB")
