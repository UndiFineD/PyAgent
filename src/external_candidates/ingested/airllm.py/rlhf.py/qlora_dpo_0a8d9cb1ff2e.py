# Extracted from: C:\DEV\PyAgent\.external\airllm\rlhf\qlora_dpo.py
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == "hh-rlhf":
            ...
            if os.path.exists(dataset_name):
                try:
                    args.dataset_format = (
                        args.dataset_format if args.dataset_format else "hh-rlhf"
                    )
    ...
    def format_dataset(dataset, dataset_format):
        if dataset_format == "hh-rlhf" or (
            dataset_format is None and args.dataset == "hh-rlhf"
        ):
            dataset = dataset.map(
                lambda x: {"rejected": x["rejected"], "chosen": x["chosen"]}
            )
