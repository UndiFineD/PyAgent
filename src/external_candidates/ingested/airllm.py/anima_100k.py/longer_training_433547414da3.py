# Extracted from: C:\DEV\PyAgent\.external\airllm\anima_100k\longer_training.py
...
def print_tensors(where_str=""):
    ...
    new_tensor_sizes = {
        (
            ...
            tuple(x.shape),
            x.dbg_alloc_where,
            x.element_size() * x.nelement() / 1024 / 1024,
        )
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    debug_mode: bool = field(
        default=False,
        metadata={"help": "debug mode sample 200 train/eval samples for validation"},
    )
    training_memory_tracking: bool = field(
        default=False,
        metadata={"help": "dump GPU memory allocation during training for oom debug"},
    )
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        if "model" in kwargs:
            for sample_input in sample_inputs:
                ...
                logger.info(
                    f"sample output: {tokenizer.decode(generation_output[0])[-60:]}"
                )
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    if args.debug_mode:
        dataset["train"] = dataset["train"].filter(
            lambda x, i: i < 200, with_indices=True
        )
