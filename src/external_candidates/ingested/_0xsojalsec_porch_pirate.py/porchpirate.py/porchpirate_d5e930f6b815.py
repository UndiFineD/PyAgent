# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-porch-pirate\porchpirate\porchpirate.py
import warnings
...
class porchpirate:
    ...
    def _show_formatted_workspace(self, workspace_results):
        ...
        for environment in globals:
            try:
                print(
                    f"{BOLD}- Environment: {END}{YELLOW}{environment['data']['name']}{END}"
                )
            except:
    ...
    def _show_formatted_request(self, request, requestid):
        ...
        try:
            if "auth" in request:
                print(
                    f"{BOLD}- Authorization: {END}{YELLOW}{request['auth']['type']}{YELLOW}{END}"
                )
                if request["auth"]["type"] == "basic":
                    for element in request["auth"]["basic"]:
                        print(
                            f"  {BOLD}- {END}{YELLOW}{element['key']}{END}={GREEN}{element['value']}{END}"
                        )
        ...
        for param in request_query_params:
            ...
            request_url = request_url.replace(
                request_value, "{}{}{}{}".format(GREEN, request_value, END, YELLOW)
            )
        ...
        for header in request_headers:
            ...
            if header_value != "":
                print(
                    f"{BOLD}- Header: {END}{YELLOW}{header_key}{END}: {GREEN}{header_value}{END}{END}"
                )
        ...
        if request["dataMode"] == "params":
            for parameter in request_data:
                ...
                print(
                    f"{BOLD}- Request Body: {END}{YELLOW}{param_key}{END}={GREEN}{param_value}{END}{END}"
                )
        elif request["dataMode"] == "raw":
            print(
                f"{BOLD}- Request Body: {END}{GREEN}{request['rawModeData']}{END}{END}"
            )
        for param_name in request_query_params:
            print(
                f"{BOLD}- Parameter: {END}{YELLOW}{param_name['key']}{END}={GREEN}{param_name['value']}{END}"
            )
        if request["preRequestScript"] != None:
            print(
                f"{BOLD}- Pre-Request Script: {END}{GREEN}{request['preRequestScript']}{END}"
            )
    ...
    def build_curl_request(self, request):
        curl_request = (
            f"curl -X {request['method']} '{YELLOW}{request['url']}{END}' \\\n"
        )
        for header in request["headerData"]:
            curl_request += (
                f"-H '{YELLOW}{header['key']}{END}: {GREEN}{header['value']}{END}' \\\n"
            )
        try:
            if request["auth"]["type"] == "basic":
    ...
    def workspace(self, id):
        response = requests.get(
            f"https://www.postman.com/_api/workspace/{id}",
            proxies=self.proxies,
            verify=False,
        )
        return response.text
    ...
    def workspace_globals(self, id):
        response = requests.get(
            f"https://www.postman.com/_api/workspace/{id}/globals",
            proxies=self.proxies,
            verify=False,
        )
        return response.text
    ...
    def collection(self, id):
        response = requests.get(
            f"https://www.postman.com/_api/collection/{id}",
            proxies=self.proxies,
            verify=False,
        )
        return response.text
    ...
    def collections(self, id):
        response = requests.post(
            f"https://www.postman.com/_api/list/collection?workspace={id}",
            proxies=self.proxies,
            verify=False,
        )
        return response.text
    ...
    def request(self, id):
        response = requests.get(
            f"https://www.postman.com/_api/request/{id}",
            proxies=self.proxies,
            verify=False,
        )
        return response.text
    ...
    def environment(self, id):
        response = requests.get(
            f"https://www.postman.com/_api/environment/{id}",
            proxies=self.proxies,
            verify=False,
        )
        return response.text
