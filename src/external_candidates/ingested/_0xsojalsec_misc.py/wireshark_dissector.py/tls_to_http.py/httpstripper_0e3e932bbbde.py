# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-misc\wireshark-dissector\tls-to-http\httpstripper.py
import json
import socket
import struct
import time
...
class HttpStripper:
    ...
    def write_to_pcap(
        ...
        for writes in (
            ...
            (
                ">I",
                int.from_bytes(socket.inet_aton(src_addr), byteorder="big"),
            ),  # Source Address
            (
                ">I",
                int.from_bytes(socket.inet_aton(dst_addr), byteorder="big"),
            ),  # Destination Address
            # TCP header
