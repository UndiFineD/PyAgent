# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\evaluation\get_remote_score.py
import argparse
import json
import os
import traceback
...
def evaluate_method(args):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            data = list(
                tqdm(executor.map(evaluate_one, data), total=len(data), desc=method)
            )
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        "--dir",
        type=str,
        default="../expr_results/Qwen2.5-3B-Instruct_2WikiMultiHopQA_grpo",
    )
