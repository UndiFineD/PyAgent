# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\agent\llm_agent\generation.py
"""
...
class ToolGenerationManager:
    ...
    def _postprocess_responses(self, responses: torch.Tensor) -> torch.Tensor:
        ...
        return (
            cleaned_token_ids,
            responses_str,
            torch.tensor(active_masks, dtype=torch.bool),
        )
    ...
    def _process_tool_responses(self, tool_responses: List[str]) -> torch.Tensor:
        ...
        tool_responses_ids = self.tokenizer(
            tool_responses, padding="longest", return_tensors="pt"
        )["input_ids"]
        ...
        if tool_responses_ids.shape[1] > self.config.max_tool_response_length:
            ...
            tool_responses_ids = tool_responses_ids[
    ...
    def _execute_tool_calls(
        self, response_strs: List[str], envs: List[ToolEnv], active_mask: torch.Tensor
    ) -> List[str]:
        ...
        for i, (resp, env, active) in enumerate(zip(response_strs, envs, active_list)):
            ...
            tool_response = result[
                0
            ]  # Extract observation from (observation, reward, done, info)
            tool_responses[i] = self.config.tool_custom_response_template.format(
    ...
    def _execute_tool_calls_batch(
        ...
        for idx, result in zip(active_indices, batch_results):
            if result is None:
                ...
                tool_responses[idx] = self.config.tool_custom_response_template.format(
                    tool_response=tool_response
                )
        return tool_responses

    def _update_rolling_state(
        self, rollings, cur_responses: torch.Tensor, tool_responses_ids: torch.Tensor
    ) -> Dict:
        """Update rolling state with new responses and observations."""
        # Concatenate and handle padding
        new_input_ids = self.tensor_fn.concatenate_with_padding(
            [rollings.batch["input_ids"], cur_responses, tool_responses_ids]
        )
    ...
    def _update_right_side(
        ...
        responses = self.tensor_fn.concatenate_with_padding(
            [right_side["responses"], cur_responses, tool_responses_ids],
            pad_to_left=False,
        )
    ...
    def run_llm_loop(
        self, gen_batch, envs: List[Any] = None, initial_input_ids: torch.Tensor = None
    ) -> Tuple[Dict, Dict]:
        ...
        for step in range(self.config.max_turns):
            ...
            responses_ids, responses_str, new_active_masks = (
                self._postprocess_responses(gen_output.batch["responses"])
            )
            ...
            if self.config.use_batch_tool_calls:
                ...
                tool_responses = self._execute_tool_calls_batch(
                    responses_str, envs, active_mask
                )
            else:
            ...
            rollings = self._update_rolling_state(
                rollings, responses_ids, tool_responses_ids
            )
            original_right_side = self._update_right_side(
                original_right_side, responses_ids, tool_responses_ids
            )
