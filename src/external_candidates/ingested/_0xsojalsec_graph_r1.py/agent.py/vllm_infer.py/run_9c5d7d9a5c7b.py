# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\agent\vllm_infer\run.py
import argparse
import copy
import json
import re
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run VLLM inference with configurable parameters"
    )
    ...
    parser.add_argument(
        "--model", type=str, default="agent", help="Model name for inference"
    )
    parser.add_argument(
        "--temperature", type=float, default=1.0, help="Temperature for sampling"
    )
    parser.add_argument(
        "--top-p", type=float, default=1.0, help="Top-p for nucleus sampling"
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
    ...
    parser.add_argument(
        "--max-turns", type=int, default=20, help="Maximum turns of search"
    )
    parser.add_argument(
        "--question",
        type=str,
...
def colorprint(mode, r_str, t_str, use_colors):
    ...
    if mode is True:
        ...
        if use_colors:
            ...
            print(
                f"{COLORS['tool_call']}Query:{COLORS['reset']}\n{query}{COLORS['reset']}"
            )
            print(
        ...
        if use_colors:
            ...
            print(
                f"{COLORS['tool_call']}Answer:{COLORS['reset']}\n{answer}{COLORS['reset']}"
            )
        else:
            print(f"\n[Think] {think}")
            print(f"Answer:\n{answer}")
...
def main():
    ...
    if use_colors:
        print(
            f"{COLORS['bg_user']} User {COLORS['reset']} {COLORS['user']}{question_raw}{COLORS['reset']}"
        )
    else:
    ...
    for step in range(MAX_TURNS):
        try:
            ...
            colorprint(
                active_masks[0],
                copy.deepcopy(responses_str[0]),
                copy.deepcopy(tool_responses[0]),
                use_colors,
            )
