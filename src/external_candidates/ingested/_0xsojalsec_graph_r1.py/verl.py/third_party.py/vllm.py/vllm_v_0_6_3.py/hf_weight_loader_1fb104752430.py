# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\third_party\vllm\vllm_v_0_6_3\hf_weight_loader.py
...
def load_hf_weights(actor_weights: Dict, vllm_model: nn.Module):
    ...
    with set_default_torch_dtype(next(vllm_model.parameters()).dtype):  # TODO
        if (
            vllm_model.config.tie_word_embeddings
            and "lm_head.weight" in actor_weights.keys()
        ):
