# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\scripts\model_merger.py
...
parser.add_argument(
    "--tie-word-embedding",
    action="store_true",
    help="Whether to tie word embedding weights",
)
parser.add_argument(
    "--is-value-model",
    action="store_true",
    help="Whether the model loaded as value model",
)
parser.add_argument(
    "--hf_model_path",
    type=str,
...
parser.add_argument(
    ...
    required=True,
    help=(
        "The path for your saved model. For megatron, point to the base dir of model, rng, optimizer checkpoints, commonly be `config.default_local_dir/global_step_\{global_step\}/actor`."
    ),
)
parser.add_argument(
    "--target_dir",
    ...
    default="tmp",
    type=str,
    help="The path for the target model",
)
parser.add_argument(
    "--hf_upload_path",
    default=False,
...
parser.add_argument(
    ...
    required=False,
    default=False,
    help="Whether to upload the model to private repo",
)
...
def patch_model_generation_config(model, hf_model_path):
    ...
    if model.can_generate():
        try:
            ...
            print(
                f"Warning: Generation config file not found in {args.hf_model_path}, using a generation config created from the model config."
            )
            pass
    return model
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    hf_path = (
        os.path.join(local_dir, "huggingface")
        if args.target_dir is None
        else args.target_dir
    )
...
def check_megatron_checkpoint_path(model_path):
    ...
    for sharded_dir in sharded_dirs:
        ...
        assert "model.pt" in os.listdir(
            os.path.join(model_path, sharded_dir)
...
def convert_megatron_checkpoints_to_hfmodels():
    ...
    print(
        f"sharded_dirs: {sharded_dirs}, tp_size: {tp_size}, pp_size: {pp_size}, mp_size: {mp_size}"
    )
    ...
    print("Writing to local disk")
    hf_path = (
        os.path.join(args.local_dir, "huggingface")
        if args.target_dir is None
        else args.target_dir
    )
