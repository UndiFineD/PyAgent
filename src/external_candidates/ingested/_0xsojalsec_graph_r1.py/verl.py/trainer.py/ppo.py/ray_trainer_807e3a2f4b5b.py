# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\verl\trainer\ppo\ray_trainer.py
...
import numpy as np
from agent.llm_agent.generation import ToolGenerationConfig, ToolGenerationManager
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        self.train_dataset = ToolRLDataset(
            ...
            truncation="error",
            tool_env=self.env,
            use_custom_tool_format_func=self.config.data.get(
    ...
    def record(self, metric_dict, global_steps):
        # save the results to a file
        result_save_file = (
            f"expr_results/{self.config.trainer.experiment_name}/evals_training.jsonl"
        )
        os.makedirs(os.path.dirname(result_save_file), exist_ok=True)
        if os.path.exists(result_save_file):
            if global_steps == 1:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw=timing_raw):
                    first_input_ids = (
                        gen_batch.batch["input_ids"][:, -gen_config.max_start_length :]
                        .clone()
                        .long()
                    )
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        reward_tensor, answer_lst_f1, answer_lst_em, format_lst, _ = (
    ...
    def _create_loss_mask(
        ...
        for i, response in enumerate(responses):
            ...
            start_positions = [
                m.start() for m in re.finditer(re.escape(start_tag), response)
            ]
            end_positions = [
                m.start() + len(end_tag)
                for m in re.finditer(re.escape(end_tag), response)
            ]
            ...
            for start, end in zip(start_positions, end_positions):
                ...
                start_tokens = self.tokenizer.encode(
                    prefix_to_start, add_special_tokens=False
                )
                state_tokens = self.tokenizer.encode(
                    state_section, add_special_tokens=False
                )
