...
class HFRollout(BaseRollout):
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        if (
            hasattr(self.config, "custom_stop_words")
            and self.config.custom_stop_words
            and self.tokenizer
        ):
            custom_criteria = CustomStopWordsCriteria(
                self.tokenizer, self.config.custom_stop_words, idx.device
            )
