# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\graphr1\operate.py
import asyncio
import json
import re
import warnings
...
async def _merge_hyperedges_then_upsert(
    ...
    if already_hyperedge is not None:
        ...
        already_source_ids.extend(
            split_string_by_multi_markers(
                already_hyperedge["source_id"], [GRAPH_FIELD_SEP]
            )
        )
...
async def _merge_edges_then_upsert(
    ...
    for node in nodes_data:
        ...
        if await knowledge_graph_inst.has_edge(hyper_relation, entity_name):
            already_edge = await knowledge_graph_inst.get_edge(
                hyper_relation, entity_name
            )
...
async def extract_entities(
    ...
    if (
        not len(all_hyperedges_data)
        and not len(all_entities_data)
        and not len(all_relationships_data)
    ):
        logger.warning(
...
async def _build_query_context(
    ...
    for i, k in enumerate(knowledge_list_2):
        ...
        score = 1 / (i + 1)
        know_score[k] += score
    knowledge_list = sorted(know_score.items(), key=lambda x: x[1], reverse=True)[
        : query_param.top_k
    ]
    knowledge = []
...
async def _get_node_data(
    ...
    knowledge_list = [
        s["description"].replace("<hyperedge>", "") for s in use_relations
    ]
    # s_ids = []
