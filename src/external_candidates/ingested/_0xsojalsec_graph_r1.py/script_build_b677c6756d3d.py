# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\script_build.py
import argparse
import json
import os
import time
...
def extract_knowledge(rag, unique_contexts):
    ...
    for i in range(0, len(unique_contexts), 50):
        print(
            f"This is the {i//50 + 1} round of insertion, remain rounds: {len(unique_contexts)//50 - i//50}"
        )
        retries = 0
        max_retries = 50
        while retries < max_retries:
            try:
                ...
                retries += 1
                print(
                    f"Insertion failed, retrying ({retries}/{max_retries}), error: {e}"
                )
                time.sleep(10)
