# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Graph-R1\script_api.py
import argparse
import asyncio
import json
...
def queries_to_results(queries: List[str]) -> List[str]:
    ...
    entity_match = {
        queries[i]: _format_results(ids[i], corpus_entity) for i in range(len(ids))
    }
    _, ids = index_hyperedge.search(embeddings, 5)  # 每个查询返回 5 个结果
    hyperedge_match = {
        queries[i]: _format_results(ids[i], corpus_hyperedge) for i in range(len(ids))
    }
    ...
    for query_text in tqdm(queries, desc="Processing queries", unit="query"):
        result = loop.run_until_complete(
            process_query(
                query_text, rag, entity_match[query_text], hyperedge_match[query_text]
            )
        )
...
app = FastAPI(
    title="Search API",
    description="An API for document retrieval using FAISS and FlagEmbedding.",
)
