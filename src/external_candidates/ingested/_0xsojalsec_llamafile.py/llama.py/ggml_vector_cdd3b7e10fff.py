# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-llamafile\llama.cpp\ggml-vector.py
import re
...
prototypes = [
    line.replace(";", "")
    for line in prototypes.split("\n")
    if line.endswith(";") and not line.startswith("//") and "(" in line
]
FUNCS = [(re.search(r"(?<= )\w+", proto).group(0), proto) for proto in prototypes]
...
ARCHS = (
    ...
    (
        "amd_avx512",
        "__x86_64__",
        ("X86_HAVE(FMA)", "X86_HAVE(F16C)", "X86_HAVE(AVX2)", "X86_HAVE(AVX512F)"),
    ),
    ("amd_avx2", "__x86_64__", ("X86_HAVE(FMA)", "X86_HAVE(F16C)", "X86_HAVE(AVX2)")),
    ("amd_f16c", "__x86_64__", ("X86_HAVE(AVX)", "X86_HAVE(F16C)")),
    ("amd_fma", "__x86_64__", ("X86_HAVE(AVX)", "X86_HAVE(FMA)")),
    ("amd_avx", "__x86_64__", ("X86_HAVE(AVX)",)),
    ("amd_ssse3", "__x86_64__", ("X86_HAVE(SSSE3)",)),
    ("amd_k8", "__x86_64__", ()),
    (
        "arm82",
        "__aarch64__",
        ("(getauxval(AT_HWCAP) & HWCAP_FPHP)", "(getauxval(AT_HWCAP) & HWCAP_ASIMDHP)"),
    ),
    ("arm80", "__aarch64__", ()),
)
...
with open("llama.cpp/ggml-vector.cpp", "w") as f:
    ...
    for func, proto in FUNCS:
        ...
        args = [
            s.split(" ")[-1]
            for s in re.search(r"(?<=\().*(?=\))", proto).group(0).split(",")
        ]
        f.write(proto + " {\n")
