...
class AgentsGenerator:
    def __init__(
        ...
        if framework == "crewai" and not CREWAI_AVAILABLE:
            raise ImportError(
                "CrewAI is not installed. Please install it with 'pip install praisonai[crewai]'"
            )
    ...
    def load_tools_from_package(self, package_path):
        ...
        for module_file in os.listdir(package_path):
            if module_file.endswith(".py") and not module_file.startswith("__"):
                module_name = (
                    f"{package_path.name}.{module_file[:-3]}"  # Remove .py for import
                )
                module = importlib.import_module(module_name)
                for name, obj in inspect.getmembers(
                    module, self.is_function_or_decorated
                ):
                    tools_dict[name] = obj
        return tools_dict
    ...
    def generate_crew_and_kickoff(self):
        ...
        if PRAISONAI_TOOLS_AVAILABLE and (
            CREWAI_AVAILABLE or AUTOGEN_AVAILABLE or PRAISONAI_AVAILABLE
        ):
            tools_dict = {
        ...
        if os.path.isfile(tools_py_path):
            ...
            self.logger.debug(
                "tools.py exists in the root directory. Loading tools.py and skipping tools folder."
            )
        ...
        if framework == "autogen":
            ...
            if AGENTOPS_AVAILABLE:
                ...
                agentops.init(
                    os.environ.get("AGENTOPS_API_KEY"), default_tags=[version_tag]
                )
            ...
            if AGENTOPS_AVAILABLE:
                agentops.init(
                    os.environ.get("AGENTOPS_API_KEY"), default_tags=["praisonai"]
                )
            ...
            if AGENTOPS_AVAILABLE:
                agentops.init(
                    os.environ.get("AGENTOPS_API_KEY"), default_tags=["crewai"]
                )
        ...
        for role, details in config["roles"].items():
            ...
            for tool in details.get("tools", []):
                if tool in tools_dict:
                    try:
                        tool_class = globals()[
                            f"autogen_{type(tools_dict[tool]).__name__}"
                        ]
    ...
    def _run_autogen_v4(self, config, topic, tools_dict):
        ...
        async def run_autogen_v4_async():
            ...
            for role, details in config["roles"].items():
                ...
                agent_name = (
                    details["role"].format(topic=topic).replace("{topic}", topic)
                )
                ...
                for tool_name in details.get("tools", []):
                    if tool_name in tools_dict:
                        ...
                        if hasattr(tool_instance, "run") and callable(
                            tool_instance.run
                        ):
                ...
                assistant = AutoGenV4AssistantAgent(
                    ...
                    + ". Must reply with 'TERMINATE' when the task is complete.",
        ...
        for role, details in config["roles"].items():
            ...
            if llm_model:
                llm = PraisonAIModel(
                    ...
                    base_url=(
                        self.config_list[0].get("base_url")
                        if self.config_list
                        else None
                    ),
                    api_key=(
                        self.config_list[0].get("api_key") if self.config_list else None
                    ),
                ).get_model()
            else:
                llm = PraisonAIModel(
    ...
    def _run_praisonai(self, config, topic, tools_dict):
        ...
        for role, details in config["roles"].items():
            ...
            agent = PraisonAgent(
                ...
                allow_delegation=details.get("allow_delegation", False),
                llm=details.get("llm", {}).get("model")
                or os.environ.get("MODEL_NAME")
                or "openai/gpt-5-nano",
                function_calling_llm=details.get("function_calling_llm", {}).get(
                    "model"
                )
                or os.environ.get("MODEL_NAME")
                or "openai/gpt-5-nano",
        ...
        if config.get("process") == "hierarchical":
            ...
            agents = PraisonAIAgents(
                agents=list(agents.values()), tasks=tasks, verbose=True, memory=memory
            )
