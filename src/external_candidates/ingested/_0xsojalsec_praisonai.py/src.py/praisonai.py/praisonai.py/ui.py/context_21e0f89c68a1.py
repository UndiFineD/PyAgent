# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\ui\context.py
import fnmatch
import logging
import os
...
class ContextGatherer:
    ...
    def is_relevant_file(self, file_path):
        ...
        return (
            ...
            and any(file_path.endswith(ext) for ext in self.relevant_extensions)
        )
            ...
            if os.path.isdir(path):
                for root, dirs, files in os.walk(path):
                    ...
                    for file in files:
                        ...
                        if not self.should_ignore(file_path) and self.is_relevant_file(
                            file_path
                        ):
