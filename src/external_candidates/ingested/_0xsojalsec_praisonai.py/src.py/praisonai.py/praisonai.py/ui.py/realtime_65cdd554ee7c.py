# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\ui\realtime.py
import asyncio
...
if not CHAINLIT_AUTH_SECRET:
    os.environ["CHAINLIT_AUTH_SECRET"] = (
        "p8BPhQChpg@J>jBz$wGxqLX2V>yTVgP*7Ky9H$aV:axW~ANNX-7_T:o@lnyCBu^U"
    )
    CHAINLIT_AUTH_SECRET = os.getenv("CHAINLIT_AUTH_SECRET")
...
try:
    ...
    if custom_tools_module:
        ...
        if hasattr(custom_tools_module, "tools") and isinstance(
            custom_tools_module.tools, list
        ):
            ...
            for tool in custom_tools_module.tools:
                if isinstance(tool, tuple) and len(tool) == 2:
                    ...
                    if (
                        ...
                        and "type" in tool_def
                        and tool_def["type"] == "function"
                    ):
    ...
    logger.warning(
        f"Error importing custom tools: {str(e)}. Continuing without custom tools."
    )
...
async def start():
    ...
    model_name = os.getenv("OPENAI_MODEL_NAME") or os.getenv(
        "MODEL_NAME", "gpt-5-nano-realtime-preview-2024-12-17"
    )
...
async def setup_openai_realtime():
    ...
    async def handle_item_completed(event):
        ...
        try:
            ...
            message_history = cl.user_session.get("message_history", [])
            content = item.get("formatted", {}).get("text", "") or item.get(
                "formatted", {}
            ).get("transcript", "")
            if content:
    ...
    if valid_tools:
        coros = [
            ...
            for tool_def, tool_handler in valid_tools
        ]
...
async def on_audio_start():
    try:
        ...
        if not openai_realtime.is_connected():
            model_name = (
                cl.user_session.get("model_name")
                or os.getenv("OPENAI_MODEL_NAME")
                or os.getenv("MODEL_NAME", "gpt-5-nano-realtime-preview-2024-12-17")
            )
...
async def on_chat_resume(thread: ThreadDict):
    ...
    model_name = (
        ...
        or "gpt-5-nano-realtime-preview-2024-12-17"
    )
    logger.debug(f"Model name: {model_name}")
    settings = cl.ChatSettings(
        [
    ...
    for message in steps:
        ...
        if msg_type == "user_message":
            message_history.append(
                {"role": "user", "content": message.get("output", "")}
            )
        elif msg_type == "assistant_message":
            message_history.append(
                {"role": "assistant", "content": message.get("output", "")}
            )
        elif msg_type == "run":
            # Handle 'run' type messages
            if message.get("isError"):
                message_history.append(
                    {"role": "system", "content": f"Error: {message.get('output', '')}"}
                )
            else:
