# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\ui\realtimeclient\__init__.py
    ...
    def __init__(self, url=None, api_key=None):
        ...
        if base_url:
            ...
            if base_url.startswith("https://"):
                ws_url = (
                    base_url.replace("https://", "wss://").rstrip("/") + "/realtime"
                )
                ...
                ws_url = (
                    base_url.rstrip("/") + "/realtime"
                    if not base_url.endswith("/realtime")
                    else base_url
                )
    ...
    async def connect(self, model="gpt-5-nano-realtime-preview-2024-12-17"):
        ...
        try:
            self.ws = await websockets.connect(
                f"{self.url}?model={model}", additional_headers=headers
            )
        except TypeError:
            ...
            try:
                ...
                raise Exception(
                    "Websockets library version incompatible. Please update websockets to version 11.0 or higher."
                )
...
class RealtimeClient(RealtimeEventHandler):
    ...
    def _on_session_created(self, event):
        try:
            ...
            logger.info(
                f"OpenAI Realtime session created - ID: {session_id}, Model: {model}"
            )
        except Exception as e:
    ...
    async def connect(self, model=None):
        ...
        if model is None:
            model = os.getenv(
                "OPENAI_MODEL_NAME", "gpt-5-nano-realtime-preview-2024-12-17"
            )
        ...
        if len(array_buffer) > 0:
            try:
                ...
                if "connection" in str(e).lower() or "closed" in str(e).lower():
                    logger.warning(
                        "WebSocket connection appears to be lost. Audio input will be queued until reconnection."
                    )
                return False
        return True
    ...
    async def _send_chainlit_message(self, item):
        ...
        # Debug logging
        logger.debug(
            f"Received item structure: {json.dumps({k: type(v).__name__ for k, v in item.items()}, indent=2)}"
        )
        ...
        logger.debug(
            f"Processed Chainlit message for item: {item.get('id', 'unknown')}"
        )
