# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\ui\colab.py
import asyncio
import importlib
import inspect
import logging
            ...
            if not name.startswith("_") and callable(obj) and not inspect.isclass(obj):
                ...
                tool = {
                    ...
                    "function": {
                        ...
                        or f"Function to {name.replace('_', ' ')}",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "query": {
...
async def step_callback(step_details):
    ...
    logger.info(
        f"[CALLBACK DEBUG] Step callback triggered with details: {step_details}"
    )
    try:
        ...
        logger.error(
            f"[CALLBACK DEBUG] Error in step callback: {str(e)}", exc_info=True
        )
...
async def task_callback(task_output):
    ...
    try:
        ...
        # Queue the message
        message_queue.put({"content": f"Task Output: {content}", "author": "Task"})
        logger.info("[CALLBACK DEBUG] Queued task completion message")

    except Exception as e:
        logger.error(
            f"[CALLBACK DEBUG] Error in task callback: {str(e)}", exc_info=True
        )


async def step_callback_wrapper(step_details):
    logger.info(
        f"[CALLBACK DEBUG] Step callback wrapper triggered with details: {step_details}"
    )
    try:
        ...
        if step_details.get("response"):
            logger.info(
                f"[CALLBACK DEBUG] Sending agent response from wrapper: {step_details.get('response')}"
            )
            try:
                await cl.Message(
                ...
                logger.info(
                    "[CALLBACK DEBUG] Successfully sent agent response message from wrapper"
                )
            except Exception as e:
                logger.error(
                    f"[CALLBACK DEBUG] Error sending agent response message from wrapper: {str(e)}"
                )

        # Create a message for any tool usage
        if step_details.get("tool_name"):
            logger.info(
                f"[CALLBACK DEBUG] Sending tool usage from wrapper: {step_details.get('tool_name')}"
            )
            try:
                await cl.Message(
                ...
                logger.info(
                    "[CALLBACK DEBUG] Successfully sent tool usage message from wrapper"
                )
            except Exception as e:
                logger.error(
                    f"[CALLBACK DEBUG] Error sending tool usage message from wrapper: {str(e)}"
                )

        # Create a message for any thoughts or reasoning
        if step_details.get("thought"):
            logger.info(
                f"[CALLBACK DEBUG] Sending thought from wrapper: {step_details.get('thought')}"
            )
            try:
                await cl.Message(
                ...
                logger.info(
                    "[CALLBACK DEBUG] Successfully sent thought message from wrapper"
                )
            except Exception as e:
                logger.error(
                    f"[CALLBACK DEBUG] Error sending thought message from wrapper: {str(e)}"
                )

    except Exception as e:
        logger.error(
            f"[CALLBACK DEBUG] Error in step callback wrapper: {str(e)}", exc_info=True
        )
        try:
            await cl.Message(
            ...
            logger.error(
                f"[CALLBACK DEBUG] Error sending error message: {str(send_error)}"
            )


async def task_callback_wrapper(task_output):
    logger.info(
        f"[CALLBACK DEBUG] Task callback wrapper triggered with output type: {type(task_output)}"
    )
    try:
        # Check if we have a Chainlit context
        if not cl.context.context_var.get():
            logger.warning(
                "[CALLBACK DEBUG] No Chainlit context available in task wrapper"
            )
            return
        ...
        logger.info(
            f"[CALLBACK DEBUG] Sending task completion message from wrapper: {content[:100]}..."
        )
        try:
            await cl.Message(
            ...
            logger.info(
                "[CALLBACK DEBUG] Successfully sent task completion message from wrapper"
            )
        except Exception as e:
            logger.error(
                f"[CALLBACK DEBUG] Error sending task completion message from wrapper: {str(e)}"
            )
        ...
        if hasattr(task_output, "details"):
            ...
            try:
                ...
                logger.info(
                    "[CALLBACK DEBUG] Successfully sent additional details message"
                )
            except Exception as e:
                logger.error(
                    f"[CALLBACK DEBUG] Error sending additional details message: {str(e)}"
                )

    except Exception as e:
        logger.error(
            f"[CALLBACK DEBUG] Error in task callback wrapper: {str(e)}", exc_info=True
        )
        try:
            await cl.Message(
...
def sync_task_callback_wrapper(task_output):
    ...
    try:
        ...
        if loop.is_running():
            ...
            logger.info(
                "[CALLBACK DEBUG] Loop is not running, running callback directly"
            )
            loop.run_until_complete(task_callback_wrapper(task_output))

    except Exception as e:
        logger.error(
            f"[CALLBACK DEBUG] Error in sync task callback: {str(e)}", exc_info=True
        )
...
def sync_step_callback_wrapper(step_details):
    ...
    try:
        ...
        logger.error(
            f"[CALLBACK DEBUG] Error in sync step callback: {str(e)}", exc_info=True
        )
...
async def ui_run_praisonai(config, topic, tools_dict):
    ...
    try:
        ...
        for role, details in config["roles"].items():
            ...
            await cl.Message(
                content=f"[DEBUG] Creating agent: {role_name}", author="System"
            ).send()
            ...
            def step_callback_sync(step_details):
                try:
                    ...
                    logger.error(
                        f"[CALLBACK DEBUG] Error in step callback: {str(e)}",
                        exc_info=True,
                    )
        ...
        for role, details in config["roles"].items():
            ...
            for task_name, task_details in details.get("tasks", {}).items():
                ...
                expected_output_filled = task_details["expected_output"].format(
                    topic=topic
                )
                ...
                def task_callback_sync(task_output):
                    try:
                        ...
                        logger.error(
                            f"[CALLBACK DEBUG] Error in task callback: {str(e)}",
                            exc_info=True,
                        )
        ...
        await cl.Message(
            content="Starting PraisonAI agents execution...", author="System"
        ).send()
        ...
        if config.get("process") == "hierarchical":
            ...
            crew = PraisonAIAgents(agents=list(agents.values()), tasks=tasks, verbose=2)
        ...
        await cl.Message(
            content="PraisonAI agents execution completed.", author="System"
        ).send()
