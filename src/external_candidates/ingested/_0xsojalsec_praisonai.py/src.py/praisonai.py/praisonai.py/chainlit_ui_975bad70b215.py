# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\chainlit_ui.py
...
async def start_chat():
    ...
    settings = await cl.ChatSettings(
        [
            TextInput(
                id="Model", label="OpenAI - Model", initial=config_list[0]["model"]
            ),
            TextInput(
                id="BaseUrl",
                label="OpenAI - Base URL",
                initial=config_list[0]["base_url"],
            ),
            TextInput(
                id="ApiKey", label="OpenAI - API Key", initial=config_list[0]["api_key"]
            ),
            Select(
                id="Framework",
                label="Framework",
    ...
    if chat_profile == "Manual":
        ...
        settings = await cl.ChatSettings(
            [
                ...
                TextInput(
                    id="agents",
                    label="agents.yaml",
                    initial=yaml_content,
                    multiline=True,
                ),
                TextInput(
                    id="tools", label="tools.py", initial=tools_content, multiline=True
                ),
            ]
        ).send()
        cl.user_session.set("settings", settings)
        ...
        if res and res.get("value") == "modify":
            await cl.Message(
                content="Modify the agents and tools from below settings",
                actions=actions,
            ).send()
