# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\scheduler.py
import logging
import re
import threading
import time
    ...
    def start(self, schedule_expr: str, max_retries: int = 3) -> bool:
        ...
        try:
            ...
            self._thread = threading.Thread(
                target=self._run_schedule, args=(interval, max_retries), daemon=True
            )
            self._thread.start()
...
def create_scheduler(
    provider: str = "gcp", config: Optional[Dict[str, Any]] = None
) -> DeploymentScheduler:
    """
