# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\inbuilt_tools\autogen_tools.py
...
def create_autogen_tool_function(tool_class):
    ...
    def tool_function(assistant, user_proxy):
        ...
        assistant.register_function(
            function_map={tool_class.__name__: wrapped_function},
            name_to_args={tool_class.__name__: {param: "" for param in param_names}},
            description=tool_instance.__doc__
            or f"Use {tool_class.__name__} to perform operations",
        )
...
if TOOLS_AVAILABLE:
    ...
    autogen_ScrapeElementFromWebsiteTool = create_autogen_tool_function(
        ScrapeElementFromWebsiteTool
    )
    ...
    autogen_YoutubeChannelSearchTool = create_autogen_tool_function(
        YoutubeChannelSearchTool
    )
    autogen_YoutubeVideoSearchTool = create_autogen_tool_function(
        YoutubeVideoSearchTool
    )
