...
def run_server(port: int, use_public: bool = False):
    ...
    if not OPENAI_API_KEY:
        raise ValueError(
            "Missing the OpenAI API key. Please set it in the .env file or configure it through the GUI."
        )
...
def main(args=None):
    ...
    parser.add_argument(
        "--public", action="store_true", help="Use ngrok to expose the server publicly"
    )
    parser.add_argument(
