# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\praisonai\auto.py
import json
import logging
import os
...
class AutoGenerator:
    def __init__(
        self,
        topic="Movie Story writing about AI",
...
""")
        ...
        base_url = (
            ...
            or os.environ.get("OLLAMA_API_BASE", "https://api.openai.com/v1")
        )
    ...
    def generate(self, merge=False):
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant designed to output complex team structures.",
                },
    ...
    def merge_with_existing_agents(self, new_json_data):
        ...
        try:
            ...
            logging.warning(
                f"Could not load existing agents file {self.agent_file}: {e}"
            )
...
The final role you create should satisfy the provided task: """
            + self.topic
            + """.
{
"roles": {
