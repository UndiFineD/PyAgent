# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_approval_interactive.py
#!/usr/bin/env python3
"""
...
def test_shell_command_approval():
    ...
    try:
        ...
        result = shell_tools.execute_command(
            "echo 'Hello from approved shell command!'"
        )
    ...
    try:
        ...
        if result and (
            ...
            if delete_result and (
                "success" in str(delete_result).lower()
                or "deleted" in str(delete_result).lower()
            ):
