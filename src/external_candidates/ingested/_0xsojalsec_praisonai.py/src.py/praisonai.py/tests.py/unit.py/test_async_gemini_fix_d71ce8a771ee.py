# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_async_gemini_fix.py
"""
...
async def test_async_gemini_tools():
    ...
    if "mock result" in search_result.lower() or "tool called" in search_result.lower():
        ...
        print(
            "❌ FAILURE: Search agent did NOT use the tool (claimed no internet access)"
        )
