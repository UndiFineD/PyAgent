# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_core_agents.py
import os
import sys
...
class TestAgent:
    ...
    def test_agent_with_llm_dict(self):
        ...
        llm_config = {"model": "gpt-5-nano", "api_key": "test-key", "temperature": 0.7}
    ...
    def test_agent_with_tools(self):
        ...
        agent = Agent(name="Tool Agent", tools=[sample_tool])
...
class TestTask:
    ...
    def test_task_with_callback(self, sample_task_config, sample_agent_config):
        ...
        agent = Agent(**sample_agent_config)
        task = Task(agent=agent, callback=sample_callback, **sample_task_config)
    ...
    def test_async_task_creation(self, sample_task_config, sample_agent_config):
        ...
        agent = Agent(**sample_agent_config)
        task = Task(agent=agent, async_execution=True, **sample_task_config)
...
class TestPraisonAIAgents:
    ...
    def test_agents_creation(self, sample_agent_config, sample_task_config):
        ...
        agents = PraisonAIAgents(agents=[agent], tasks=[task], process="sequential")
    ...
    def test_sequential_execution(
        ...
        with patch.object(
            agents, "start", return_value="Execution completed"
        ) as mock_start:
            result = agents.start()
    ...
    def test_multiple_agents(self, sample_agent_config, sample_task_config):
        ...
        agents = PraisonAIAgents(
            agents=[agent1, agent2], tasks=[task1, task2], process="hierarchical"
        )
