# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_tool_fix_improved.py
"""
...
def test_improved_implementation():
    ...
    try:
        # Create workflow
        workflow = PraisonAIAgents(agents=[test_agent], tasks=[test_task], verbose=True)
        ...
        if isinstance(result, dict) and "task_results" in result:
            ...
            if "do not have access" in result_str:
                ...
                tool_indicator in result_str
                for tool_indicator in ["search", "results", "found", "web"]
            ):
