...
def test_context_management():
    ...
    try:
        ...
        task1 = Task(
            name="task1", description="First task", agent=agent, status="completed"
        )
        task1.result = TaskOutput(
            description="First task", raw="Result from task 1", agent="Test Agent"
        )
        print("✓ Created task1")

        task2 = Task(
            name="task2", description="Second task", agent=agent, status="completed"
        )
        task2.result = TaskOutput(
            description="Second task", raw="Result from task 2", agent="Test Agent"
        )
        ...
        # Verify results
        assert (
            "task2" in limited_context
        ), "Limited context should include most recent task"
        assert (
            "task1" not in limited_context
        ), "Limited context should NOT include earlier tasks"
