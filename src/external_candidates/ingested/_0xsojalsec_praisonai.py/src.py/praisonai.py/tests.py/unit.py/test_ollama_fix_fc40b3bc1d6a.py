# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_ollama_fix.py
#!/usr/bin/env python3
"""
...
def test_tool_call_parsing():
    ...
    tool_call_std = {
        "id": "call_123",
        "function": {"name": "hello_world", "arguments": '{"name": "test"}'},
    }
    ...
    tool_call_alt = {"name": "hello_world", "arguments": '{"name": "test"}'}
    ...
    tool_call_bad = {"function": {"name": "hello_world", "arguments": "invalid json"}}
    name, args, id = llm._parse_tool_call_arguments(tool_call_bad, is_ollama=False)
    assert (
        name == "unknown_function"
    ), f"Expected 'unknown_function' for malformed JSON, got '{name}'"
