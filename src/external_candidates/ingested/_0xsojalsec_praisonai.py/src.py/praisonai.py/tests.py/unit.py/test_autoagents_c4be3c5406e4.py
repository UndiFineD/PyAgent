# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_autoagents.py
"""
...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "..", "..", "praisonai-agents")
)

try:
    from praisonaiagents.agents.autoagents import (
        AgentConfig,
        AutoAgents,
        AutoAgentsConfig,
        TaskConfig,
    )
    from praisonaiagents.llm import LLM, OpenAIClient
...
class TestAutoAgents:
                ...
                AgentConfig(
                    ...
                    tasks=[
                        TaskConfig(
                            ...
                            tools=[
                                "web_search"
                            ],  # Use string tool names, not Mock objects
                        )
                    ],
                ),
                AgentConfig(
    ...
    def test_validate_config_failure_missing_description(
        self, sample_invalid_config_missing_fields
    ):
        ...
        is_valid, error_msg = auto_agents._validate_config(
            sample_invalid_config_missing_fields
        )
    ...
    def test_generate_config_openai_success(
        self, mock_get_client, mock_supports_structured, sample_valid_config, mock_tools
    ):
        ...
        with (
            patch.object(AutoAgents, "_create_agents_and_tasks") as mock_create,
            patch.object(AutoAgents, "_display_agents_and_tasks") as mock_display,
            patch(
                "praisonaiagents.agents.agents.PraisonAIAgents.__init__"
            ) as mock_super_init,
        ):
    ...
    def test_generate_config_llm_success(
        self, mock_llm_class, mock_supports_structured, sample_valid_config, mock_tools
    ):
        ...
        # Mock LLM instance
        mock_llm = Mock()
        mock_llm.get_response.return_value = json.dumps(
            sample_valid_config.model_dump()
        )
    ...
    @patch("praisonaiagents.llm.supports_structured_outputs")
    @patch("praisonaiagents.agents.autoagents.LLM")
    def test_generate_config_with_markdown_response(
        self, mock_llm_class, mock_supports_structured, sample_valid_config, mock_tools
    ):
    ...
    @patch("praisonaiagents.llm.supports_structured_outputs")
    @patch("praisonaiagents.agents.autoagents.get_openai_client")
    def test_generate_config_retry_on_validation_failure(
        self, mock_get_client, mock_supports_structured, sample_valid_config, mock_tools
    ):
        ...
        mock_client.parse_structured_output.side_effect = [
            invalid_config,
            sample_valid_config,
        ]
        mock_get_client.return_value = mock_client
    ...
    @patch("praisonaiagents.llm.supports_structured_outputs")
    @patch("praisonaiagents.agents.autoagents.get_openai_client")
    def test_generate_config_max_retries_exceeded(
        self, mock_get_client, mock_supports_structured, mock_tools
    ):
        ...
        with pytest.raises(
            ValueError, match="Configuration validation failed after 3 attempts"
        ):
            AutoAgents(
                instructions="Create a blog post about AI",
                tools=mock_tools,
    ...
    @patch("praisonaiagents.llm.supports_structured_outputs")
    @patch("praisonaiagents.agents.autoagents.get_openai_client")
    def test_max_agents_truncation(
        self, mock_get_client, mock_supports_structured, mock_tools
    ):
        ...
        with (
            ...
            auto_agents = AutoAgents(
                instructions="Test", tools=mock_tools, max_agents=2, llm="gpt-4"
            )
    ...
    @patch("praisonaiagents.llm.supports_structured_outputs")
    @patch("praisonaiagents.agents.autoagents.get_openai_client")
    def test_insufficient_agents_warning(
        self, mock_get_client, mock_supports_structured, mock_tools
    ):
        ...
        with (
            ...
            auto_agents = AutoAgents(
                instructions="Test", tools=mock_tools, max_agents=3, llm="gpt-4"
            )
    ...
    @patch("praisonaiagents.llm.supports_structured_outputs")
    @patch("praisonaiagents.agents.autoagents.LLM")
    def test_retry_with_previous_response_in_prompt(
        self, mock_llm_class, mock_supports_structured, sample_valid_config, mock_tools
    ):
    ...
    @patch("praisonaiagents.agents.autoagents.get_openai_client")
    @patch("json.dumps")
    def test_custom_api_key_and_base_url(
        self,
        mock_json_dumps,
        mock_get_client,
