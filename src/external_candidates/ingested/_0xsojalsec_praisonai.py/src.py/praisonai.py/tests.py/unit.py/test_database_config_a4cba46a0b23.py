# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_database_config.py
#!/usr/bin/env python3
"""
...
from praisonai.ui.database_config import (
    get_database_config_for_sqlalchemy,
    get_database_url_with_sqlite_override,
    should_force_sqlite,
)
...
def test_database_config():
    ...
    print(
        f"get_database_url_with_sqlite_override(): {get_database_url_with_sqlite_override()}"
    )
    print(
        f"get_database_config_for_sqlalchemy(): {get_database_config_for_sqlalchemy()}"
    )
