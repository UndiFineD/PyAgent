# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_validation_feedback.py
...
class TestValidationFeedback:
    ...
    def test_task_has_validation_feedback_field(self):
        ...
        task = Task(
            name="test_task", description="Test task", expected_output="Test output"
        )
    ...
    def test_validation_feedback_captured_on_invalid_decision(self):
        ...
        collect_task.result = TaskOutput(
            ...
            description="Collected data items",
            agent="agent1",
        )
        ...
        assert (
            collect_task.validation_feedback["validation_details"]
            == "Not enough results, need at least 10"
        )
        assert (
            collect_task.validation_feedback["rejected_output"]
            == "Collected only 3 items"
        )
