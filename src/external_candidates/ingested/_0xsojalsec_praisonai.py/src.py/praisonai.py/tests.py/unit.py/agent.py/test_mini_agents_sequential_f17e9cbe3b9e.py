# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\agent\test_mini_agents_sequential.py
#!/usr/bin/env python3
"""
...
def test_mini_agents_sequential_data_passing(mock_litellm):
    ...
    agent1 = Agent(
        instructions="Generate the number 42 as your output. Only return the number 42, nothing else.",
        llm={"model": "gpt-3.5-turbo"},
    )
    agent2 = Agent(
        instructions="Take the input number and multiply it by 2. Only return the result number, nothing else.",
        llm={"model": "gpt-3.5-turbo"},
    )
    ...
    assert "42" in str(
        task1_result.raw
    ), f"Expected first task to output '42', got: {task1_result.raw}"
    assert "84" in str(
        task2_result.raw
    ), f"Expected second task to output '84', got: {task2_result.raw}"
