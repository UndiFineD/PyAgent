# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_approval_basic.py
#!/usr/bin/env python3
"""
...
def test_approval_configuration():
    ...
    assert is_approval_required(
        "test_tool"
    ), "Tool should require approval after adding"
    ...
    assert not is_approval_required(
        "test_tool"
    ), "Tool should not require approval after removing"
    assert (
        get_risk_level("test_tool") is None
    ), "Risk level should be None after removal"
    print("✅ Remove approval requirement works")

    # Test default dangerous tools are configured
    assert is_approval_required(
        "execute_command"
    ), "execute_command should require approval by default"
    assert (
        get_risk_level("execute_command") == "critical"
    ), "execute_command should be critical risk"
    print("✅ Default dangerous tools are configured")

    print(
        f"✅ Current approval-required tools: {len(APPROVAL_REQUIRED_TOOLS)} configured"
    )
...
def test_approval_decorator():
    ...
    ), "Decorated function should require approval"
    assert (
        get_risk_level("test_dangerous_function") == "high"
    ), "Risk level should match decorator"
...
def test_tool_integration():
    ...
    try:
        ...
        assert is_approval_required(
            "execute_code"
        ), "execute_code should require approval"
...
async def test_approval_callback():
    ...
    from praisonaiagents.approval import ApprovalDecision, request_approval
    ...
    assert (
        "No approval required" in decision.reason
    ), "Should indicate no approval needed"
...
def test_agent_integration():
    ...
    try:
        ...
        print(
            f"✅ Global approval callback configured: {approval_callback is not None}"
        )
