# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\unit\test_approval_agent_integration.py
#!/usr/bin/env python3
"""
...
@pytest.mark.skipif(
    os.getenv("ASK_USER") != "1", reason="interactive approval requires user input"
)
def test_agent_tool_execution_with_approval():
    ...
    try:
        ...
        result = agent.execute_tool(
            "execute_command", {"command": "echo 'Hello from agent-executed command!'"}
        )
...
def test_agent_with_auto_approval(mock_console_callback, mock_confirm):
    ...
    try:
        ...
        try:
            ...
            assert (
                False
            ), "praisonaiagents.approval module not available - check import path"
        ...
        mock_console_callback.return_value = ApprovalDecision(
            approved=True, reason="Auto-approved for testing"
        )
        ...
        result = agent.execute_tool(
            "execute_command", {"command": "echo 'Auto-approved command executed!'"}
        )
...
def test_agent_with_auto_denial():
    ...
    try:
        ...
        result = agent.execute_tool(
            "execute_command", {"command": "echo 'This should be denied'"}
        )

        if result and (
            "denied" in str(result).lower() or "approval" in str(result).lower()
        ):
...
def test_agent_python_code_execution(mock_console_callback, mock_confirm):
    ...
    try:
        ...
        print(
            "⚠️ Skipping Python code test - missing optional packages (black, pylint, autopep8)"
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result and (
                ...
                or "written" in str(result).lower()
            ):
                ...
                if os.path.exists(test_file_path):
                    ...
                    if "This file was created through agent with approval!" in content:
                        ...
                        assert (
                            False
                        ), f"File content mismatch. Expected approval message, got: {content}"
                else:
...
def main():
    ...
    print("=" * 65)
    print(
        "These tests demonstrate the approval system working through agent tool execution."
    )
    print()
    ...
    try:
        ...
        if choice == "1":
            ...
            run_interactive = (
                input("Run interactive approval test? (y/n): ").strip().lower()
            )
        ...
        print("\nKey observations:")
        print(
            "- Approval system works when tools are executed through agent.execute_tool()"
        )
