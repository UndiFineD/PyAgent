# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\integration\test_rag_integration.py
import os
import sys
import tempfile
...
class TestRAGIntegration:
    ...
    def test_ollama_rag_integration(self, sample_agent_config):
        ...
        ollama_config = {
            "vector_store": {
                "provider": "chroma",
                "config": {"collection_name": "ollama_knowledge", "path": ".praison"},
            },
            "llm": {
                "provider": "ollama",
                "config": {
    ...
    def test_rag_context_injection(
        self, mock_chroma_client, sample_agent_config, mock_llm_response
    ):
...
class TestRAGMemoryIntegration:
    ...
    def test_rag_knowledge_update(self, sample_agent_config):
        ...
        update_result = mock_update_knowledge(
            agent, ["new_document.pdf", "updated_info.txt"], mode="append"
        )
