# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\integration\test_mcp_integration.py
import asyncio
import os
import sys
...
class TestMCPIntegration:
    ...
    async def test_mcp_tool_execution(self):
        ...
        with patch("mcp.client.stdio.stdio_client") as mock_stdio_client:
            ...
            with patch("mcp.ClientSession") as mock_session_class:
                ...
                # Mock tool execution
                mock_session.initialize.return_value = None
                mock_session.list_tools.return_value = Mock(
                    tools=[Mock(name="calculator", description="Calculate expressions")]
                )
                mock_session.call_tool.return_value = Mock(
                    content=[Mock(text='{"result": 42}')]
                )
    ...
    async def test_mcp_error_handling(self):
        ...
        with patch("mcp.client.stdio.stdio_client") as mock_stdio_client:
            ...
            mock_stdio_client.side_effect = ConnectionError(
        ...
        with patch("mcp.client.stdio.stdio_client") as mock_stdio_client:
            ...
            with patch("mcp.ClientSession") as mock_session_class:
                ...
                mock_session.call_tool.return_value = Mock(
                    content=[
                        Mock(
                            text='{"results": [{"title": "AI Trend 1", "url": "example.com"}]}'
                        )
                    ]
                )
