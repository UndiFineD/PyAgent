# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai\tests\test_phase2_refactor.py
#!/usr/bin/env python3
...
def test_sync_methods():
    ...
    response = llm.response(prompt="What is 2+2?", verbose=False)
    ...
    response = llm.get_response(
        prompt="What's my name?", chat_history=chat_history, verbose=False
    )
    print(f"   Response: {response}")
...
async def test_async_methods():
    ...
    response = await llm.aresponse(prompt="What is 3+3?", verbose=False)
    print(f"   Response: {response}")
    ...
    response = await llm.get_response_async(
        prompt="What's my favorite color?", chat_history=chat_history, verbose=False
    )
    print(f"   Response: {response}")
...
def test_edge_cases():
    ...
    response = llm.response(prompt=prompt, verbose=False)
    print(f"   Response: {response}")
    ...
    response = llm_o1.response(
        prompt="Hello", system_prompt="This should be skipped", verbose=False
    )
    print(f"   Response: {response[:50]}...")
...
def main():
    ...
    try:
        ...
        print(
            "- All 4 methods (get_response, response, get_response_async, aresponse) updated"
        )
