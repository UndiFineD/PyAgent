# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\fix_notebook.py
#!/usr/bin/env python3
"""
    ...
    try:
        ...
        for cell in notebook.get("cells", []):
            if cell.get("cell_type") == "code":
                ...
                if (
                    "result_obj.dict()" in source_str
                    or 'hasattr(result_obj, "dict")' in source_str
                ):
                    print(f"Found cell with .dict() usage, fixing...")
                    # Replace .dict() with .model_dump() and fix hasattr check for backward compatibility
                    fixed_source = source_str.replace(
                        "result_obj.dict()", "result_obj.model_dump()"
                    ).replace(
                        'hasattr(result_obj, "dict")',
                        'hasattr(result_obj, "model_dump")',
                    )
