# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_duplicate_callback_fix.py
#!/usr/bin/env python3
"""
...
def mock_display_interaction(
    message, response, markdown=True, generation_time=0, **kwargs
):
...
def test_single_display_no_tools():
    ...
    try:
        with patch("litellm.completion") as mock_completion:
            ...
            response = llm.get_response(prompt="Test prompt", verbose=True, stream=True)
            ...
            assert (
                len(display_calls) == 1
            ), f"Expected 1 display call, got {len(display_calls)}"
            ...
            with patch("litellm.completion") as mock_completion:
                ...
                def mock_streaming(*args, **kwargs):
                    ...
                    if call_count == 1:
                        ...
                        return [
                            MagicMock(
                                choices=[MagicMock(delta=MagicMock(content="Initial"))]
                            ),
                            MagicMock(
                                choices=[
                        ...
                        return [
                            MagicMock(
                                choices=[
                                    MagicMock(
                                        delta=MagicMock(content=json.dumps(reflection))
                                    )
                                ]
                            )
                        ]
                        ...
                        return [
                            MagicMock(
                                choices=[MagicMock(delta=MagicMock(content="Better"))]
                            ),
                            MagicMock(
                                choices=[
                                    MagicMock(delta=MagicMock(content=" response"))
    ...
    async def run_test():
        with patch("litellm.acompletion") as mock_acompletion:
            ...
            response = await llm.get_response_async(
                prompt="Test async", verbose=True, stream=True
            )
