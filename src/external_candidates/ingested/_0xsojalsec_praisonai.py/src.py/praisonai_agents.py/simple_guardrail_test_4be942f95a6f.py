# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\simple_guardrail_test.py
#!/usr/bin/env python3
"""
...
def test_function_guardrail():
    ...
    assert (
        guardrail_result.success
    ), f"Good output should pass: {guardrail_result.error}"
    print("✓ Good output passed function guardrail")

    # Test with bad output
    bad_output = TaskOutput(
        description="Test task", raw="Error occurred", agent="Test Agent"
    )
