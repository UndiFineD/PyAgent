# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\reproduce_issue_878_simple.py
#!/usr/bin/env python3
"""
...
def test_verbose_vs_non_verbose():
    ...
    if verbose_count > non_verbose_count and verbose_count > 1:
        ...
        print(
            f"       verbose=True triggers {verbose_count - non_verbose_count} extra callback(s)"
        )
...
def test_multiple_calls():
    ...
    try:
        ...
        for i in range(3):
            print(f"  Call {i+1}:")
            response = llm.get_response(prompt=f"Say exactly: Call {i+1}", verbose=True)
            print(f"    Response: {response}")

        print(
            f"📊 Total callback invocations across 3 calls: {len(callback_invocations)}"
        )
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(
        "This script tests the LLM class directly to isolate where duplicate callbacks occur."
    )
    print()
    ...
    for test_name, count in results.items():
        if count == -1:
            ...
            print(
                f"{test_name}: {count} invocations (this test shows verbose=True duplicates)"
            )
            if count > 1:
                issues_detected.append(
                    f"{test_name}: verbose=True triggered {count} callbacks (duplicate issue)"
                )
        else:
            expected = 1
            print(f"{test_name}: {count} invocations (expected {expected})")
