...
def test_tool_summary_generation():
    ...
    try:
        ...
        if summary_substantial is None:
            ...
            print(
                "❌ Tool summary generation incorrectly generated summary for substantial response"
            )
            return False

        # Test with non-Ollama model (should NOT generate summary)
        non_ollama_llm = LLM(model="gpt-5-nano")
        summary_non_ollama = non_ollama_llm._generate_ollama_tool_summary(
            tool_results, ""
        )
        if summary_non_ollama is None:
            ...
            print(
                "❌ Tool summary generation incorrectly generated summary for non-Ollama model"
            )
            return False
