...
async def async_task_function(task_name, delay):
    ...
    print(
        f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} - {task_name} started (async)"
    )
    await asyncio.sleep(delay)
    end_time = time.time()
    print(
        f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} - {task_name} completed (async)"
    )
...
def sync_task_function(task_name, delay):
    ...
    end_time = time.time()
    print(
        f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} - {task_name} completed (sync)"
    )
...
async def test_sequential_async_execution():
    ...
    try:
        ...
        async_batch1 = [
            ...
            if log["task"] in ["async_task_1", "async_task_2", "async_task_3"]
        ]
        if async_batch1:
        ...
        if sync_logs:
            ...
            print(
                f"  - Started after async batch: {'YES' if sync_log['start'] >= latest_end - 0.1 else 'NO'}"
            )
        ...
        async_batch2 = [
            log
            for log in execution_log
            if log["task"] in ["async_task_4", "async_task_5"]
        ]
        if async_batch2:
        ...
        expected_time = (
            2 + 1 + 1
        )  # First batch (2s parallel) + sync (1s) + second batch (1s parallel)
