...
def test_model_router():
    ...
    selected = router.select_model(
        task_description="Calculate the sum of numbers", budget_conscious=True
    )
...
def test_router_agent():
    ...
    selected = agent._select_model_for_task(
        task_description="Simple math problem", tools=None
    )
...
def test_integration():
    ...
    agents_system = PraisonAIAgents(
        agents=[agent], tasks=[task], process="sequential", verbose=False
    )
