# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_task_name_fix.py
#!/usr/bin/env python3
"""
...
def test_task_structure():
    ...
    try:
        ...
        # Create a basic task
        agent = Agent(
            name="TestAgent", role="Test Role", goal="Test Goal", llm="mock-llm"
        )
        ...
        if has_name and has_description and has_id:
            ...
            print(
                f"  ❌ Task missing attributes - name: {has_name}, description: {has_description}, id: {has_id}"
            )
            return False
    ...
    try:
        ...
        try:
            ...
            if "task_name" in str(e) and "not defined" in str(e):
                ...
                print(
                    f"  ✅ Different error (expected with mock LLM): {type(e).__name__}: {e}"
                )
                return True
