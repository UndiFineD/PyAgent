# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_all_optimizations.py
#!/usr/bin/env python3
"""
...
def test_with_gemini_basic():
    ...
    start = time.time()
    agent = Agent(
        instructions="You are a helpful assistant", llm="gemini/gemini-2.0-flash-exp"
    )
    agent_init = time.time() - start
    print(f"    Agent initialization time: {agent_init:.4f}s")
