# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\async-tavily-agents-gemini.py
import asyncio
import logging
import os
...
async def run_parallel_searches():
    ...
    agents = PraisonAIAgents(
        agents=[search_agent, summary_agent], tasks=search_tasks + [summary_task]
    )
    ...
    return {
        "search_results": [
            str(results["task_results"][i]) for i in range(len(search_tasks))
        ],
