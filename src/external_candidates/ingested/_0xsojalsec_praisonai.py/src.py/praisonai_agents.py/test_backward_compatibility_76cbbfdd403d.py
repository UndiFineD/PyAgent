# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_backward_compatibility.py
#!/usr/bin/env python3
import os
import sys
...
print(
    "✓ The old pattern `from praisonaiagents import Agent` would work if praisonaiagents is available"
)
print(
    "✓ The new pattern `from praisonai import Agent` would work if praisonaiagents is available"
)
...
try:
    ...
    try:
        ...
        print(
            "❌ ERROR: Agent import should have failed when praisonaiagents is not available"
        )
...
try:
    ...
    if hasattr(praisonai.praisonai, "__all__"):
        ...
        has_praisonaiagents_symbols = any(
            item in all_list for item in praisonaiagents_symbols
        )
        if not has_praisonaiagents_symbols:
            print(
                "✓ praisonaiagents symbols correctly excluded from __all__ when not available"
            )
        else:
...
init_file = os.path.join(
    os.path.dirname(__file__), "src", "praisonai", "praisonai", "__init__.py"
)
