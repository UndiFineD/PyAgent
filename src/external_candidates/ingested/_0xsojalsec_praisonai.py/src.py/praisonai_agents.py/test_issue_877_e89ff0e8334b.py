# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_issue_877.py
#!/usr/bin/env python3
"""
...
from praisonaiagents import Agent, PraisonAIAgents, Task, register_display_callback
...
def simple_callback(message=None, response=None, **kwargs):
    callback_log.append({"message": message, "response": response, "kwargs": kwargs})
...
try:
    agents1 = PraisonAIAgents(agents=[agent1], tasks=[task1])
    ...
    if len(callback_log) > 0:
        ...
        print(
            f"   First callback: message='{callback_log[0]['message'][:50]}...', response='{callback_log[0]['response'][:50]}...'"
        )
    else:
...
try:
    agents2 = PraisonAIAgents(agents=[agent2], tasks=[task2])
