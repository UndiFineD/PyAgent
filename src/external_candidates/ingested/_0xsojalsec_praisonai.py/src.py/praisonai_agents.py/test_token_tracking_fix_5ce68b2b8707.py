...
try:
    ...
    if hasattr(agent, "llm_instance") and agent.llm_instance:
        ...
        if (
            hasattr(agent.llm_instance, "metrics")
            and agent.llm_instance.metrics == True
        ):
            ...
            print(f"   Agent.metrics: {agent.metrics}")
            print(
                f"   LLM.metrics: {getattr(agent.llm_instance, 'metrics', 'NOT_FOUND')}"
            )
    else:
    ...
    if hasattr(agent2, "llm_instance") and agent2.llm_instance:
        ...
        if (
            hasattr(agent2.llm_instance, "metrics")
            and agent2.llm_instance.metrics == True
        ):
            print("✅ SUCCESS: metrics=True was properly passed to LLM instance!")
            ...
            print(f"   Agent.metrics: {agent2.metrics}")
            print(
                f"   LLM.metrics: {getattr(agent2.llm_instance, 'metrics', 'NOT_FOUND')}"
            )
    else:
        print("❌ LLM instance not created")
    ...
    if hasattr(agent3, "llm_instance") and agent3.llm_instance:
        ...
        if (
            hasattr(agent3.llm_instance, "metrics")
            and agent3.llm_instance.metrics == True
        ):
            print("✅ SUCCESS: metrics=True was properly passed to LLM instance!")
            ...
            print(f"   Agent.metrics: {agent3.metrics}")
            print(
                f"   LLM.metrics: {getattr(agent3.llm_instance, 'metrics', 'NOT_FOUND')}"
            )
    else:
        print("❌ LLM instance not created")
