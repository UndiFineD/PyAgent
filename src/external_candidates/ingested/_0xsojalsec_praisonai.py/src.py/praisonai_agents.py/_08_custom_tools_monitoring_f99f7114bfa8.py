# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\08_custom_tools_monitoring.py
"""
...
def process_data(data: str, operation: str = "analyze") -> str:
    ...
    processing_times = {"analyze": 0.15, "transform": 0.2, "validate": 0.1}

    time.sleep(processing_times.get(operation, 0.12))

    return (
        f"Data processed with {operation}: {data[:50]}{'...' if len(data) > 50 else ''}"
    )
...
def call_external_api(endpoint: str, method: str = "GET", payload: str = "") -> str:
    ...
    return (
        f"API Response from {endpoint}: {{'status': 'success', 'method': '{method}'}}"
    )
...
def file_operation(operation: str, filename: str, content: str = "") -> str:
    ...
    operation_times = {"read": 0.03, "write": 0.05, "delete": 0.02, "copy": 0.07}
...
def test_tool_performance():
    ...
    for method in ["GET", "POST"]:
        try:
            result = call_external_api(
                "/api/test", method, "test payload" if method == "POST" else ""
            )
    ...
    for op in ["read", "write", "copy"]:
        result = file_operation(
            op, "test_file.txt", "Test content" if op == "write" else ""
        )
...
def main():
    ...
    print(
        f"  Average Time per Tool Call: {total_tool_time/total_tool_calls:.3f}s"
    ...
    reliability = (
        ((total_tool_calls - total_tool_errors) / total_tool_calls * 100)
        if total_tool_calls > 0
        else 100
    )
    ...
    if total_tool_errors > 0:
        print(
            f"  - Investigate {total_tool_errors} tool errors for stability improvements"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Operations: {result['tool_operations']}, Reliability: {result['tool_reliability']:.1f}%"
    )
