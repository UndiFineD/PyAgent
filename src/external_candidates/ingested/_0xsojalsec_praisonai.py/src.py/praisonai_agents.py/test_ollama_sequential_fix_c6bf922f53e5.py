# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_ollama_sequential_fix.py
#!/usr/bin/env python3
"""
...
def test_ollama_fix():
    ...
    sys.path.insert(
        0, os.path.join(os.path.dirname(__file__), "src", "praisonai-agents")
    )
    ...
    try:
        ...
        assert hasattr(
            LLM, "OLLAMA_FINAL_ANSWER_PROMPT"
        ), "Missing OLLAMA_FINAL_ANSWER_PROMPT constant"
        assert hasattr(
            LLM, "OLLAMA_SUMMARY_ITERATION_THRESHOLD"
        ), "Missing OLLAMA_SUMMARY_ITERATION_THRESHOLD constant"
        assert (
            LLM.OLLAMA_SUMMARY_ITERATION_THRESHOLD == 3
        ), "OLLAMA_SUMMARY_ITERATION_THRESHOLD should be 3"

        print("✅ LLM constants properly defined")
        print(f"   OLLAMA_FINAL_ANSWER_PROMPT: {LLM.OLLAMA_FINAL_ANSWER_PROMPT}")
        print(
            f"   OLLAMA_SUMMARY_ITERATION_THRESHOLD: {LLM.OLLAMA_SUMMARY_ITERATION_THRESHOLD}"
        )
    ...
    try:
        ...
        assert hasattr(
            llm, "_generate_ollama_tool_summary"
        ), "Missing _generate_ollama_tool_summary method"
        ...
        assert (
            is_ollama == True
        ), "Ollama provider detection should return True for ollama/ prefix"
    ...
    try:
        ...
        with patch.object(llm, "_client_completion") as mock_completion:
            ...
            messages = [
                {
                    "role": "user",
                    "content": "Get Google's stock price and multiply it by 2",
                }
            ]
    ...
    print(
        "• Execute get_stock_price('Google') → returns 'The stock price of Google is 100'"
    )
