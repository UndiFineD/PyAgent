...
class Session:
    ...
    def memory(self) -> Memory:
        ...
        if self.is_remote:
            raise ValueError(
                "Memory operations are not available for remote agent sessions"
            )
    ...
    def knowledge(self) -> Knowledge:
        ...
        if self.is_remote:
            raise ValueError(
                "Knowledge operations are not available for remote agent sessions"
            )
    ...
    def save_state(self, state_data: Dict[str, Any]) -> None:
        ...
        if self.is_remote:
            raise ValueError(
                "State operations are not available for remote agent sessions"
            )
    ...
    def restore_state(self) -> Dict[str, Any]:
        ...
        results = self.memory.search_short_term(
            query="Session state:", limit=10  # Get more results to filter by session_id
        )
    ...
    def _restore_agent_chat_history(self, agent_key: str) -> List[Dict[str, Any]]:
        ...
        results = self.memory.search_short_term(
            query="Agent chat history for", limit=10
        )
    ...
    def _save_agent_chat_histories(self) -> None:
        ...
        for agent_key, agent_data in self._agents.items():
            ...
            if agent and hasattr(agent, "chat_history"):
                ...
                agent_data["chat_history"] = (
                    chat_history  # Ensure tracked history is up-to-date
                )
            else:
    ...
    def search_memory(
        self, query: str, memory_type: str = "long", limit: int = 5
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_context(self, query: str, max_items: int = 3) -> str:
        ...
        return self.memory.build_context_for_task(
            task_descr=query, user_id=self.user_id, max_items=max_items
        )
    ...
    def _test_remote_connection(self) -> None:
        ...
        try:
            ...
            test_url = (
                self.agent_url.rstrip("/") + "/health"
                if "/health" not in self.agent_url
                else self.agent_url
            )
            ...
            if response.status_code != 200:
                ...
                if response.status_code not in [
                    ...
                    raise ConnectionError(
                        f"Remote agent returned status code: {response.status_code}"
                    )
            print(f"✅ Successfully connected to remote agent at {self.agent_url}")
        except requests.exceptions.Timeout:
            raise ConnectionError(
                f"Timeout connecting to remote agent at {self.agent_url}"
            )
        except requests.exceptions.ConnectionError:
            raise ConnectionError(
                f"Failed to connect to remote agent at {self.agent_url}"
            )
    ...
    def chat(self, message: str, **kwargs) -> str:
        ...
        try:
            ...
            raise ConnectionError(
                f"Timeout communicating with remote agent at {self.agent_url}"
            )
        except requests.exceptions.ConnectionError:
            raise ConnectionError(
                f"Failed to communicate with remote agent at {self.agent_url}"
            )
