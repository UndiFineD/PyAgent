...
class Memory:
    ...
    def __init__(self, config: Dict[str, Any], verbose: int = 0):
        ...
        if isinstance(self.embedder_config, dict):
            ...
            self.embedding_model = embedder_model_config.get(
        ...
        if use_graph:
            ...
            if org_id and proj_id:
                self.mem0_client = MemoryClient(
                    api_key=api_key, org_id=org_id, project_id=proj_id
                )
            else:
    ...
    def _init_chroma(self):
        ...
        try:
            ...
            try:
                self.chroma_col = self.chroma_client.get_collection(
                    name=collection_name
                )
                self._log_verbose("Using existing ChromaDB collection")
            except Exception as e:
                self._log_verbose(
                    f"Collection '{collection_name}' not found. Creating new collection. Error: {e}"
                )
    ...
    def _init_mongodb(self):
        ...
        try:
            ...
            self.connection_string = mongo_cfg.get(
            ...
            self.mongo_client = MongoClient(
                ...
                serverSelectionTimeoutMS=mongo_cfg.get(
                    "server_selection_timeout", 5000
                ),
                retryWrites=True,
                retryReads=True,
    ...
    def _create_mongodb_indexes(self):
        ...
        try:
            ...
            self.mongo_short_term.create_index(
                [("created_at", -1), ("metadata.quality", -1)]
            )
            self.mongo_long_term.create_index(
                [("created_at", -1), ("metadata.quality", -1)]
            )
            ...
            self._log_verbose(
                f"Warning: Could not create MongoDB indexes: {e}", logging.WARNING
            )
            ...
            try:
                ...
                try:
                    ...
                    search_index_model = SearchIndexModel(
                        definition=vector_index_def, name="vector_index"
                    )
                    ...
                    self.mongo_short_term.create_search_index(
                        vector_index_def, "vector_index"
                    )
                    self.mongo_long_term.create_search_index(
                        vector_index_def, "vector_index"
                    )
                self._log_verbose("Vector search indexes created successfully")
            except Exception as e:
                self._log_verbose(
                    f"Could not create vector search indexes: {e}", logging.WARNING
                )

        except Exception as e:
            self._log_verbose(
                f"Error creating vector search indexes: {e}", logging.WARNING
            )
    ...
    def store_short_term(
        ...
        if self.use_mongodb and hasattr(self, "mongo_short_term"):
            try:
                ...
                logger.info(
                    f"Successfully stored in MongoDB short-term memory with ID: {ident}"
                )
            except Exception as e:
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Successfully stored in SQLite short-term memory with ID: {ident}"
            )
        except Exception as e:
            logger.error(f"Failed to store in SQLite short-term memory: {e}")
            if (
                not self.use_mongodb
            ):  # Only raise if we're not using MongoDB as fallback
                raise
        ...
        **kwargs,
        ...
        if self.use_mem0 and hasattr(self, "mem0_client"):
            ...
            try:
                ...
                self._log_verbose(
                    f"Error searching MongoDB short-term memory: {e}", logging.ERROR
                )
                return []
            ...
            try:
                ...
                if resp["ids"]:
                    for i in range(len(resp["ids"][0])):
                        metadata = (
                            resp["metadatas"][0][i] if "metadatas" in resp else {}
                        )
                        quality = metadata.get("quality", 0.0)
                        score = 1.0 - (
                            resp["distances"][0][i] if "distances" in resp else 0.0
                        )
    ...
    def store_long_term(
        ...
        if self.use_mongodb and hasattr(self, "mongo_long_term"):
            try:
                ...
                logger.info(
                    f"Successfully stored in MongoDB long-term memory with ID: {ident}"
                )
            except Exception as e:
        ...
        **kwargs,
        ...
        if self.use_mem0 and hasattr(self, "mem0_client"):
            ...
            try:
                ...
                self._log_verbose(
                    f"Error searching MongoDB long-term memory: {e}", logging.ERROR
                )
        ...
        for row in rows:
            ...
            if not any(f["id"] == row[0] for f in found):
                found.append(
                    {"id": row[0], "text": text, "metadata": meta, "created_at": row[3]}
                )
    ...
    def reset_long_term(self):
        ...
        if self.use_mongodb and hasattr(self, "mongo_long_term"):
            try:
                ...
                self._log_verbose(
                    f"Error clearing MongoDB long-term memory: {e}", logging.ERROR
                )
    ...
    def search_user_memory(
        ...
        if self.use_mem0 and hasattr(self, "mem0_client"):
            ...
            try:
                ...
                self._log_verbose(
                    f"Error searching MongoDB user memory: {e}", logging.ERROR
                )
                return []
        else:
    ...
    def search(
        ...
        **kwargs,
        ...
        if user_id:
            ...
            return self.search_user_memory(
                user_id, query, limit=limit, rerank=rerank, **kwargs
            )
        else:
    ...
    def finalize_task_output(
        ...
        logger.info(
            f"Agent: {agent_name}, Quality: {quality_score}, Threshold: {threshold}"
        )
        ...
        if quality_score >= threshold:
            try:
                logger.info(
                    f"Quality score {quality_score} >= {threshold}, storing in long-term memory..."
                )
            ...
            logger.info(
                f"Quality score {quality_score} < {threshold}, skipping long-term storage"
            )
    ...
    def build_context_for_task(
        ...
        def add_section(title: str, hits: List[Any]) -> None:
            ...
            if formatted_hits:
                ...
                logger.debug(
                    f"Memory section '{brief_title}' ({len(formatted_hits)} items): {formatted_hits}"
                )
        ...
        user_mem = (
            self.search_user_memory(user_id, q, limit=max_items) if user_id else []
        )
        ...
        try:
            if LITELLM_AVAILABLE:
                ...
                response = litellm.completion(
                    model=model_name,
                    messages=[
                        {"role": "user", "content": custom_prompt or default_prompt}
                    ],
                ...
                logger.error(
                    "Neither litellm nor openai available for quality calculation"
                )
                return {
                    "completeness": 0.0,
                    "relevance": 0.0,
    ...
    def search_with_quality(
        ...
        search_func = (
            self.search_short_term if memory_type == "short" else self.search_long_term
        )
    ...
    def get_all_memories(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            c = conn.cursor()
            rows = c.execute(
                "SELECT id, content, meta, created_at FROM short_mem"
            ).fetchall()
            conn.close()
            ...
            c = conn.cursor()
            rows = c.execute(
                "SELECT id, content, meta, created_at FROM long_mem"
            ).fetchall()
            conn.close()
