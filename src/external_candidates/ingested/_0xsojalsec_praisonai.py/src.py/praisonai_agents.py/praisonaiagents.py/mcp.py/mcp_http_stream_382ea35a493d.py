# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\mcp\mcp_http_stream.py
"""
...
try:
    ...
    raise ImportError(
        "aiohttp is required for HTTP Stream transport. Install with: pip install praisonaiagents[mcp]"
    )
...
class HTTPStreamMCPClient:
    ...
    async def _async_initialize(self):
        ...
        logger.debug(
            f"Found {len(tools_data)} tools: {[tool.name for tool in tools_data]}"
        )
        ...
        self._finalizer = weakref.finalize(
            self, self._static_cleanup, self.transport, self._session_context
        )
    ...
    def _force_cleanup(self):
        ...
        try:
            if (
                self.transport
                and hasattr(self.transport, "_session")
                and self.transport._session
            ):
