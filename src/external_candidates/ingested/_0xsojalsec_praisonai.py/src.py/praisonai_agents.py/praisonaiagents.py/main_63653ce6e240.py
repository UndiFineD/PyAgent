...
def _clean_display_content(content: str, max_length: int = 20000) -> str:
    ...
    if not content or not str(content).strip():
        logging.debug(
            f"Empty content received in _clean_display_content: {repr(content)}"
        )
        return ""
...
def display_interaction(
    ...
    if isinstance(message, list):
        text_content = next(
            (item["text"] for item in message if item["type"] == "text"), ""
        )
    ...
    if generation_time:
        console.print(
            Text(f"Response generated in {generation_time:.1f}s", style="dim")
        )

    if markdown:
        console.print(Panel.fit(Markdown(message), title="Task", border_style="cyan"))
        console.print(
            Panel.fit(Markdown(response), title="Response", border_style="cyan")
        )
    else:
        console.print(
            Panel.fit(
                Text(message, style="bold green"), title="Task", border_style="cyan"
            )
        )
        console.print(
            Panel.fit(
                Text(response, style="bold blue"), title="Response", border_style="cyan"
            )
        )
...
def display_tool_call(message: str, console=None):
    ...
    console.print(
        Panel.fit(
            Text(message, style="bold cyan"), title="Tool Call", border_style="green"
        )
    )
...
def display_error(message: str, console=None):
    ...
    console.print(
        Panel.fit(Text(message, style="bold red"), title="Error", border_style="red")
    )
...
def display_generating(content: str = "", start_time: Optional[float] = None):
    ...
    return Panel(
        Markdown(content), title=f"Generating...{elapsed_str}", border_style="green"
    )
