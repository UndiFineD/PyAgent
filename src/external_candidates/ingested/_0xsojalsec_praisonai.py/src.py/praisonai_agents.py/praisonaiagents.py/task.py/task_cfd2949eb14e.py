# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\task\task.py
import asyncio
import inspect
import logging
import os
...
class Task:
    def __init__(
        ...
        callback: Optional[
            Union[
                Callable[[TaskOutput], Any],
                Callable[[TaskOutput], Coroutine[Any, Any, Any]],
            ]
        ] = None,
        ...
        if memory is not None or (config and config.get("memory_config")):
            try:
                ...
                logger.warning(
                    'Some memory features may not work. Install with: pip install "praisonaiagents[memory]"'
                )
        ...
        self.expected_output = (
            ...
            else "Complete the task successfully"
        )
        self.agent = agent
        ...
        self.validation_feedback = (
            None  # Store validation failure feedback for retry attempts
        )
    ...
    def _setup_guardrail(self):
        ...
        if callable(self.guardrail):
            ...
            if len(positional_args) != 1:
                raise ValueError(
                    "Guardrail function must accept exactly one parameter (TaskOutput)"
                )
            ...
            llm = getattr(self.agent, "llm", None) or getattr(
                self.agent, "llm_instance", None
            )
            self._guardrail_fn = LLMGuardrail(description=self.guardrail, llm=llm)
        else:
            raise ValueError(
                "Guardrail must be either a callable or a string description"
            )
    ...
    def initialize_memory(self):
        ...
        if not self.memory and self.config.get("memory_config"):
            try:
                ...
                logger.info(
                    f"Task {self.id}: Initializing memory from config: {self.config['memory_config']}"
                )
                ...
                if os.path.exists(self.config["memory_config"]["storage"]["path"]):
                    logger.info(
                        f"Task {self.id}: Memory database exists after initialization"
                    )
                else:
    ...
    def store_in_memory(
        self, content: str, agent_name: str = None, task_id: str = None
    ):
    ...
    async def execute_callback(self, task_output: TaskOutput) -> None:
        ...
        if self._guardrail_fn:
            try:
                ...
                if not guardrail_result.success:
                    ...
                    self.retry_count += 1
                    logger.warning(
                        f"Task {self.id}: Guardrail validation failed (retry {self.retry_count}/{self.max_retries}): {guardrail_result.error}"
                    )
        ...
        if self.memory:
            ...
            try:
                ...
                logger.error(
                    f"Task {self.id}: Failed to store task output in memory: {e}"
                )
                logger.exception(e)
        ...
        if self.quality_check and self.memory:
            try:
                ...
                logger.info(
                    f"Task {self.id}: Calculating quality metrics for output: {task_output.raw[:100]}..."
                )
                ...
                llm_model = None
                if self.agent:
                    if getattr(self.agent, "_using_custom_llm", False) and hasattr(
                        self.agent, "llm_instance"
                    ):
                ...
                metrics = self.memory.calculate_quality_metrics(
                    task_output.raw, self.expected_output, llm=llm_model
                )
                ...
                if self.next_tasks:
                    ...
                    context = self.memory.build_context_for_task(
                        task_descr=task_output.raw, max_items=5
                    )
                    logger.info(
                        f"Task {self.id}: Built context for next tasks: {len(context)} items"
                    )

                logger.info(f"Task {self.id}: Memory operations complete")
            except Exception as e:
                logger.error(
                    f"Task {self.id}: Failed to process memory operations: {e}"
                )
            ...
            for context_item in self.context:
                if isinstance(context_item, str):
                    ...
                    context_results.append(
                        f"Input Content: {' '.join(str(x) for x in context_item)}"
                    )
                    ...
                    if context_item.result:
                        task_name = (
                            ...
                            else context_item.description
                        )
                        # Log detailed result for debugging
                        logger.debug(
                            f"Previous task '{task_name}' result: {context_item.result.raw}"
                        )
...
"""
    ...
    async def _execute_callback_with_metadata(self, task_output):
        ...
        try:
            ...
            logger.warning(
                f"Task {self.id}: Callback signature inspection failed, falling back to single parameter: {e}"
            )
            if asyncio.iscoroutinefunction(self.callback):
                await self.callback(task_output)
            else:
