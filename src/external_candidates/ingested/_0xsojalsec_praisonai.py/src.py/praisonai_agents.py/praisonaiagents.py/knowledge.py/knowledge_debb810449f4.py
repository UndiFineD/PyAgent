# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\knowledge\knowledge.py
import logging
import os
import time
import uuid
...
class MongoDBMemory:
    ...
    def __init__(self, config):
        ...
        self.connection_string = self.vector_store_config.get(
    ...
    def _init_embedding_model(self):
        ...
        try:
            ...
            if embedder_config.get("provider") == "openai":
                ...
                self.embedding_model_name = embedder_config.get("config", {}).get(
    ...
    def add(self, messages, user_id=None, agent_id=None, run_id=None, metadata=None):
        ...
        try:
            ...
            return [{"id": str(result.inserted_id), "memory": content, "event": "ADD"}]
    ...
    def search(
        self, query, user_id=None, agent_id=None, run_id=None, rerank=False, **kwargs
    ):
        ...
        try:
            ...
            if not results:
                ...
                for doc in self.collection.find(search_filter).limit(
                    kwargs.get("limit", 10)
                ):
                    results.append(
                        {
...
class Knowledge:
    ...
    def _deps(self):
        try:
            ...
            return {"chromadb": chromadb, "markdown": MarkItDown()}
        except ImportError:
            raise ImportError(
    ...
    def config(self):
        ...
        base_config = {
            ...
            "reranker": {"enabled": False, "default_rerank": False},
        }
        ...
        if self._config:
            ...
            if "vector_store" in self._config:
                if "provider" in self._config["vector_store"]:
                    base_config["vector_store"]["provider"] = self._config[
                        "vector_store"
                    ]["provider"]
                    ...
                    if self._config["vector_store"]["provider"] == "mongodb":
                        base_config["vector_store"] = {
                            "provider": "mongodb",
                            "config": {
                                "connection_string": self._config["vector_store"][
                                    "config"
                                ].get(
                                    "connection_string", "mongodb://localhost:27017/"
                                ),
                                "database": self._config["vector_store"]["config"].get(
                                    "database", "praisonai"
                                ),
                                "collection": self._config["vector_store"][
                                    "config"
                                ].get("collection", "knowledge_base"),
                                "use_vector_search": self._config["vector_store"][
                                    "config"
                                ].get("use_vector_search", True),
                            },
                        }

                if (
                    "config" in self._config["vector_store"]
                    and self._config["vector_store"]["provider"] != "mongodb"
                ):
    ...
    def memory(self):
        ...
        try:
            ...
            if "list_collections" in str(e) or "Extra fields not allowed" in str(e):
                ...
                vector_store_config = {
                    "collection_name": self.config["vector_store"]["config"][
                        "collection_name"
                    ],
    ...
    @cached_property
    def chunker(self):
        return Chunking(chunker_type="recursive", chunk_size=512, chunk_overlap=50)
        ...
        try:
            if isinstance(content, str):
                if any(
                    ...
                    self._log(
                        f"Content appears to be a file path, processing file: {content}"
                    )
                    return self.add(
                        content,
                        user_id=user_id,
            ...
            try:
                ...
                if "unexpected keyword argument" in str(
                    e
                ) or "positional argument" in str(e):
    ...
    def search(
        self, query, user_id=None, agent_id=None, run_id=None, rerank=None, **kwargs
    ):
        ...
        if isinstance(file_path, (list, tuple)):
            ...
            for path in file_path:
                result = self._process_single_input(
                    path, user_id, agent_id, run_id, metadata
                )
                results.extend(result.get("results", []))
            return {"results": results, "relations": []}

        return self._process_single_input(
            file_path, user_id, agent_id, run_id, metadata
        )

    def _process_single_input(
        self, input_path, user_id=None, agent_id=None, run_id=None, metadata=None
    ):
        ...
        try:
            ...
            if is_supported_file:
                ...
                file_ext = (
                    "." + input_path.lower().split(".")[-1]
                )  # Get extension reliably
            ...
            with progress:
                store_task = progress.add_task(
                    f"Adding to Knowledge from {os.path.basename(input_path)}",
                    total=len(memories),
                )
                    ...
                    if memory:
                        ...
                        if memory_result:
                            ...
                            if isinstance(memory_result, dict):
                                ...
                                logging.warning(
                                    f"Unexpected memory_result type: {type(memory_result)}, skipping"
                                )
            ...
            logger.error(
                f"Error processing input {input_path}: {str(e)}", exc_info=True
            )
            raise
