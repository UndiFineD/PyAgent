# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\llm\openai_client.py
"""
...
def process_stream_chunks(chunks):
    ...
    try:
        ...
        for chunk in chunks:
            ...
            if hasattr(delta, "tool_calls") and delta.tool_calls:
                for tool_call_delta in delta.tool_calls:
                    ...
                    elif current_tool_call is not None and hasattr(
        ...
        tool_calls = [
            tc for tc in tool_calls if tc and tc["id"] and tc["function"]["name"]
        ]
        ...
        choice = Choice(
            finish_reason=(
                finish_reason or "tool_calls" if processed_tool_calls else None
            ),
            index=0,
            message=message,
        )
...
class OpenAIClient:
    ...
    def format_tools(self, tools: Optional[List[Any]]) -> Optional[List[Dict]]:
        ...
        for tool in tools:
            ...
            if isinstance(tool, dict) and "type" in tool and tool["type"] == "function":
                if (
                    ...
                    logging.debug(
                        "Skipping malformed OpenAI tool: missing function or name"
                    )
                ...
                for subtool in tool:
                    if (
                        ...
                        if (
                            ...
                            logging.debug(
                                "Skipping malformed OpenAI tool in list: missing function or name"
                            )
            elif callable(tool):
        ...
        if (
            result is not None
            and len(self._formatted_tools_cache) < self._max_cache_size
        ):
    ...
    def _generate_tool_definition_from_name(self, function_name: str) -> Optional[Dict]:
        ...
        logging.debug(
            f"Tool definition generation from name '{function_name}' requires function reference"
        )
        return None
            ...
            if stream:
                ...
                if display_fn and console:
                    ...
                    try:
                        with Live(
                            ...
                            for chunk in response_stream:
                                ...
                                if chunk.choices[0].delta.content:
                                    full_response_text += chunk.choices[0].delta.content
                                    live.update(
                                        display_fn(full_response_text, start_time)
                                    )
                        ...
                        self.logger.error(
                            f"Error in Live display for non-streaming: {e}"
                        )
            ...
            if stream:
                ...
                if display_fn and console:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error in Live display for async non-streaming: {e}"
                        )
                        # Fallback to regular completion without display
                ...
                for tool_call in tool_calls:
                    ...
                    if asyncio.iscoroutinefunction(execute_tool_fn):
                        ...
                        tool_result = await loop.run_in_executor(
                            None, lambda: execute_tool_fn(function_name, arguments)
                        )
