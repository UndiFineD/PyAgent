# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\llm\model_router.py
"""
...
class ModelRouter:
    ...
    def select_model(
        ...
        complexity = self.analyze_task_complexity(
            task_description, tools_required, context_size
        )
        ...
        if self.cost_threshold:
            candidates = [
                m for m in candidates if m.cost_per_1k_tokens <= self.cost_threshold
            ]
            ...
            for provider in self.preferred_providers:
                for model in candidates:
                    if model.provider == provider:
                        logger.info(
                            f"Selected model: {model.name} (complexity: {complexity}, cost: ${model.cost_per_1k_tokens}/1k tokens)"
                        )
                        return model.name
        ...
        logger.info(
            f"Selected model: {selected.name} (complexity: {complexity}, cost: ${selected.cost_per_1k_tokens}/1k tokens)"
        )
...
def create_routing_agent(
    ...
    **agent_kwargs,
    ...
    routing_agent = Agent(
        ...
        goal=agent_kwargs.pop(
            "goal", "Select the most appropriate AI model based on task requirements"
        ),
        backstory=agent_kwargs.pop(
            "backstory",
