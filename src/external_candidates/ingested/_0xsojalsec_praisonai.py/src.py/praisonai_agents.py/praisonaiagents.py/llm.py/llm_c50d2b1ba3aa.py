...
class LLM:
    ...
    def _configure_logging(cls):
        ...
        try:
            ...
            if loglevel == "DEBUG":
                logging.getLogger("litellm.llms.custom_httpx.http_handler").setLevel(
                    logging.INFO
                )
            else:
                logging.getLogger("litellm.llms.custom_httpx.http_handler").setLevel(
    ...
    def _log_llm_config(self, method_name: str, **config):
        ...
        if should_log:
            ...
            if "api_key" in safe_config:
                safe_config["api_key"] = (
                    "***" if safe_config["api_key"] is not None else None
                )
            if "extra_settings" in safe_config and isinstance(
                safe_config["extra_settings"], dict
            ):
                safe_config["extra_settings"] = {
                    k: v
                    for k, v in safe_config["extra_settings"].items()
                    if k not in ["api_key"]
                }
            ...
            if "tools" in safe_config:
                ...
                if tools and isinstance(tools, (list, tuple)):
                    ...
                    safe_config["tools"] = (
                        tools.__name__ if hasattr(tools, "__name__") else str(tools)
                    )
                else:
            ...
            if "output_pydantic" in safe_config:
                ...
                safe_config["output_pydantic"] = (
                    str(op.__class__.__name__) if op else None
                )

            # Log based on method name - check more specific conditions first
            if method_name == "__init__":
                logging.debug(
                    f"LLM instance initialized with: {json.dumps(safe_config, indent=2, default=str)}"
                )
            elif "parameters" in method_name:
                logging.debug(
                    f"{method_name}: {json.dumps(safe_config, indent=2, default=str)}"
                )
            elif "_async" in method_name:
                logging.debug(
                    f"LLM async instance configuration: {json.dumps(safe_config, indent=2, default=str)}"
                )
            else:
                logging.debug(
                    f"{method_name} configuration: {json.dumps(safe_config, indent=2, default=str)}"
                )
    ...
    def _is_ollama_provider(self) -> bool:
        ...
        if any(
            url and ("ollama" in url.lower() or ":11434" in url)
    ...
    def _generate_ollama_tool_summary(
        self, tool_results: List[Any], response_text: str
    ) -> Optional[str]:
        """
    ...
    def _format_ollama_tool_result_message(
        self, function_name: str, tool_result: Any
    ) -> Dict[str, str]:
        """
    ...
    def _process_stream_delta(
        ...
        if (
            formatted_tools
            and self._supports_streaming_tools()
    ...
    def _parse_tool_call_arguments(
        self, tool_call: Dict, is_ollama: bool = False
    ) -> tuple:
        """
    ...
    def _validate_and_filter_ollama_arguments(
        ...
        if not target_function:
            logging.debug(
                f"[OLLAMA_FIX] Function {function_name} not found in available tools"
            )
            return arguments
        ...
        try:
                ...
                if param_name in valid_params:
                    ...
                    if param.annotation != inspect.Parameter.empty:
                        try:
                            ...
                            elif param.annotation == bool and isinstance(
                                param_value, str
                            ):
            ...
            if invalid_params:
                logging.debug(
                    f"[OLLAMA_FIX] Function {function_name} received invalid parameters: {invalid_params}"
                )
                logging.debug(
                    f"[OLLAMA_FIX] Valid parameters for {function_name}: {list(valid_params)}"
                )
            ...
            logging.debug(
                f"[OLLAMA_FIX] Error validating arguments for {function_name}: {e}"
            )
            return arguments
    ...
    def _handle_ollama_sequential_logic(
        ...
        if not (
            ...
            and iteration_count >= self.OLLAMA_SUMMARY_ITERATION_THRESHOLD
        ):
        ...
        if (
            ...
            tool_summary = self._generate_ollama_tool_summary(
                accumulated_tool_results, response_text
            )
            if tool_summary:
    ...
    def _build_messages(
        ...
        if output_json or output_pydantic:
            ...
            is_gemini_with_structured_output = (
                self._is_gemini_model() and supports_structured_outputs(self.model)
            )
        ...
        if system_prompt:
            ...
            ) and not is_gemini_with_structured_output:
        ...
        for tool in tools:
            ...
            if isinstance(tool, dict) and "type" in tool and tool["type"] == "function":
                ...
                if (
                    ...
                    logging.debug(
                        f"Using pre-formatted OpenAI tool: {tool['function']['name']}"
                    )
            ...
            # Handle lists of tools (e.g. from MCP.to_openai_tool())
                ...
                for subtool in tool:
                    if (
                        isinstance(subtool, dict)
                        and "type" in subtool
                        and subtool["type"] == "function"
                    ):
                        # Validate nested dictionary structure before accessing
                        if (
                            "function" in subtool
                            ...
                            logging.debug(
                                f"Using pre-formatted OpenAI tool from list: {subtool['function']['name']}"
                            )
                            # Fix array schemas in the tool parameters
                            fixed_tool = subtool.copy()
                            if "parameters" in fixed_tool["function"]:
                                fixed_tool["function"]["parameters"] = (
                                    self._fix_array_schemas(
                                        fixed_tool["function"]["parameters"]
                                    )
                                )
                            formatted_tools.append(fixed_tool)
    ...
    def get_response(
        ...
        **kwargs,
        ...
        try:
            ...
            reflection_count = 0
            callback_executed = (
                False  # Track if callback has been executed for this interaction
            )
            ...
            stored_reasoning_content = (
                None  # Store reasoning content from tool execution
            )
                ...
                try:
                    ...
                    # Otherwise do the existing streaming approach
                        ...
                        if (
                            use_streaming
                            and formatted_tools
                            and self._is_gemini_model()
                        ):
                            logging.debug(
                                "Disabling streaming for Gemini model with tools due to JSON parsing issues"
                            )
                            use_streaming = False
                        ...
                        if use_streaming:
                            ...
                            try:
                                ...
                                if self._is_streaming_error_recoverable(
                                    streaming_error
                                ):
                                    if verbose:
                                        logging.warning(
                                            f"Streaming error (recoverable): {streaming_error}"
                                        )
                                        logging.warning(
                                    ...
                                    try:
                                        ...
                                        logging.warning(
                                            f"Non-streaming fallback also failed: {fallback_error}"
                                        )
                                        logging.warning(
                                            "Using partial streaming response data"
                                        )
                                    ...
                                    logging.error(
                                        f"Non-recoverable streaming error: {streaming_error}"
                                    )
                                    raise streaming_error

                            if streaming_success:
                                response_text = (
                                    response_text.strip() if response_text else ""
                                )
                    ...
                    tool_calls = final_response["choices"][0]["message"].get(
                        "tool_calls"
                    )
                    ...
                    if (
                        ...
                        try:
                            ...
                            if (
                                ...
                                tool_calls = [
                                    {
                                        ...
                                        "function": {
                                            ...
                                            "arguments": json.dumps(
                                                response_json.get("arguments", {})
                                            ),
                                        },
                                    }
                                ]
                                logging.debug(
                                    f"Parsed Ollama tool call from response: {tool_calls}"
                                )
                                ...
                                if tool_calls:
                                    logging.debug(
                                        f"Parsed multiple Ollama tool calls from response: {tool_calls}"
                                    )
                        except (json.JSONDecodeError, KeyError) as e:
                            logging.debug(
                                f"Could not parse Ollama tool call from response: {e}"
                            )
                    ...
                    if not tool_calls and response_text and formatted_tools:
                        ...
                        if should_try_xml:
                            ...
                            try:
                                ...
                                for idx, element in enumerate(tool_call_elements):
                                    if element.text:
                                        try:
                                            ...
                                            logging.debug(
                                                f"Could not parse tool call JSON: {e}"
                                            )
                                            continue
                                ...
                                matches = re.findall(
                                    tool_call_pattern, response_text, re.DOTALL
                                )
                                ...
                                for idx, match in enumerate(matches):
                                    try:
                                        ...
                                        logging.debug(
                                            f"Could not parse XML tool call: {e}"
                                        )
                                        continue

                            if tool_calls:
                                logging.debug(
                                    f"Parsed {len(tool_calls)} tool call(s) from XML format"
                                )
                    ...
                    if (
                        ...
                        messages.append(
                            {"role": "user", "content": self.OLLAMA_TOOL_USAGE_PROMPT}
                        )
                        iteration_count += 1
                        continue
                    ...
                    if tool_calls and execute_tool_fn:
                        ...
                        tool_result_mapping = (
                            {}
                        )  # Store function results by name for Ollama chaining
                        ...
                        for tool_call in tool_calls:
                                ...
                                if is_ollama and tool_result_mapping:
                                    ...
                                    for arg_name, arg_value in list(arguments.items()):
                                        if (
                                            ...
                                            arguments[arg_name] = tool_result_mapping[
                                                arg_value
                                            ]
                                            logging.debug(
                            ...
                            logging.debug(
                                f"[TOOL_EXEC_DEBUG] Tool execution result: {tool_result}"
                            )
                            ...
                            if is_ollama:
                                ...
                                if isinstance(tool_result, (int, float)):
                                    ...
                                    if match:
                                        tool_result_mapping[function_name] = int(
                                            match.group(1)
                                        )
                                    else:
                                        tool_result_mapping[function_name] = tool_result
                            ...
                            if verbose:
                                ...
                                if tool_result:
                                    display_message += (
                                        f"Function returned: {tool_result}"
                                    )
                                    logging.debug(
                                        f"[TOOL_EXEC_DEBUG] Display message with result: {display_message}"
                                    )
                                else:
                                    display_message += "Function returned no output"
                                    logging.debug(
                                        "[TOOL_EXEC_DEBUG] Tool returned no output"
                                    )
                            ...
                            if self._is_ollama_provider():
                                ...
                                messages.append(
                                    self._format_ollama_tool_result_message(
                            ...
                            if function_name == "sequentialthinking" and arguments.get(
                        ...
                        should_break, tool_summary_text, iteration_count = (
                            self._handle_ollama_sequential_logic(
                                iteration_count,
                                accumulated_tool_results,
                                response_text,
                                messages,
                        ...
                        if should_break:
                            ...
                            tool_summary_text is None
                            and iteration_count
                            > self.OLLAMA_SUMMARY_ITERATION_THRESHOLD
                        ):
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
                ...
                if reasoning_steps:
                    ...
                    reflection_text = reflection_resp["choices"][0]["message"][
                        "content"
                    ]
                            ...
                            for chunk in litellm.completion(
                                **self._build_completion_params(
                                ...
                                if (
                                    ...
                                    live.update(
                                        display_generating(reflection_text, start_time)
                                    )
                    else:
                        reflection_text = ""
                        for chunk in litellm.completion(
                ...
                try:
                    ...
                    satisfactory = (
                        reflection_data.get("satisfactory", "no").lower() == "yes"
                    )
                            ...
                            for chunk in litellm.completion(
                                **self._build_completion_params(
                                ...
                                if (
                                    ...
                                    live.update(
                                        display_generating(response_text, time.time())
                                    )
                    else:
                        response_text = ""
                        for chunk in litellm.completion(
            ...
            if use_streaming:
                ...
                try:
                    ...
                    max_consecutive_errors = (
                        3  # Fallback to non-streaming after 3 consecutive errors
                    )
                    ...
                    try:
                        for chunk in stream_iterator:
                            try:
                                ...
                                if self._is_streaming_error_recoverable(chunk_error):
                                    ...
                                    # For non-recoverable errors, re-raise immediately
                                    logging.error(
                                        f"Non-recoverable streaming error: {chunk_error}"
                                    )
                                    raise chunk_error
                        ...
                        if self._is_streaming_error_recoverable(iterator_error):
                            if verbose:
                                logging.warning(
                                    f"Stream iterator error detected (recoverable): {iterator_error}"
                                )
                                logging.warning(
                                    "Falling back to non-streaming mode due to stream iteration failure"
                                )
                            ...
                            # For non-recoverable errors, re-raise immediately
                            logging.error(
                                f"Non-recoverable stream iterator error: {iterator_error}"
                            )
                    ...
                    if tool_calls and execute_tool_fn:
                        ...
                        if self._is_ollama_provider():
                            ...
                            serializable_tool_calls = self._serialize_tool_calls(
                                tool_calls
                            )
                            messages.append(
                                {
                        ...
                        try:
                            ...
                            if follow_up_response and follow_up_response.choices:
                                follow_up_content = follow_up_response.choices[
                                    0
                                ].message.content
                    ...
                    if any(
                        ...
                        logging.warning(
                            f"Streaming failed due to connection issues: {e}"
                        )
                    else:
    ...
    def _is_gemini_model(self) -> bool:
        ...
        return any(
            ...
            for prefix in ["gemini", "gemini/", "google/gemini"]
        )
        ...
        **kwargs,
        ...
        try:
            ...
            while iteration_count < max_iterations:
                ...
                if reasoning_steps and iteration_count == 0:
                    ...
                    if use_streaming:
                                ...
                                **self._build_completion_params(
                                ...
                                if chunk and chunk.choices and chunk.choices[0].delta:
                                    ...
                                    if (
                                        ...
                                        tool_calls = (
                                            self._process_tool_calls_from_stream(
                                                delta, tool_calls
                                            )
                                        )
                        ...
                        # Handle None content from Gemini
                        response_content = tool_response.choices[0].message.get(
                            "content"
                        )
                        response_text = (
                        ...
                        if self._is_gemini_model():
                            logging.debug(
                                f"Gemini response content: {response_content} -> {response_text}"
                            )
                ...
                if tools and execute_tool_fn and tool_calls:
                    ...
                    if self._is_ollama_provider() and iteration_count > 0:
                        ...
                        messages.append(
                            {"role": "user", "content": self.OLLAMA_FINAL_ANSWER_PROMPT}
                        )
            ...
            if not self_reflect:
                ...
                display_text = (
                    final_response_text if final_response_text else response_text
                )
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
            ...
            if LLMContextLengthExceededException(str(error))._is_context_limit_error(
                str(error)
            ):
    ...
    def _build_completion_params(self, **override_params) -> Dict[str, Any]:
        ...
        if self.extra_settings:
            ...
            filtered_extra_settings = {
                k: v for k, v in self.extra_settings.items() if k != "metrics"
            }
        ...
        if output_json or output_pydantic:
            ...
            if self._is_gemini_model():
                ...
                if (
                    ...
                    logging.debug(
                        f"Using Gemini native structured output with schema: {json.dumps(schema, indent=2)}"
                    )
        ...
        if "tools" in params and params["tools"] and "tool_choice" not in params:
            ...
            if self._is_gemini_model():
                try:
                    ...
                    logging.debug(
                        f"Could not verify function calling support: {e}. Setting tool_choice anyway."
                    )
                    params["tool_choice"] = "auto"
    ...
    def _process_tool_calls_from_stream(
        self, delta, tool_calls: List[Dict]
    ) -> List[Dict]:
        ...
        for name, param in parameters_list:
            ...
            parameters["properties"][name] = {
                ...
                "description": param_descriptions.get(
                    name, "Parameter description not available"
                ),
            }
