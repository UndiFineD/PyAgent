...
def process_task_context(context_item, verbose=0, user_id=None):
    ...
    if isinstance(context_item, str):
    ...
    elif isinstance(context_item, dict) and (
        "vector_store" in context_item or "embedding_db_config" in context_item
    ):
        ...
        try:
            ...
            cfg = context_item.get("vector_store") or context_item.get(
                "embedding_db_config"
            )
            ...
            logger.debug(
                f"Knowledge search results ({len(db_results)} items): {str(db_results)}"
            )
...
class PraisonAIAgents:
    def __init__(
        ...
        self.completion_checker = (
            ...
            else self.default_completion_checker
        )
        ...
        if tasks is None:
            ...
            if not tasks:
                raise ValueError(
                    "If tasks are provided, at least one task must be present"
                )
        ...
        if len(tasks) > 1 and (
            process == "sequential" or all(task.next_tasks == [] for task in tasks)
        ):
        ...
        if memory:
            try:
                ...
                if not mem_cfg:
                    mem_cfg = next(
                        (
                            t.config.get("memory_config")
                            for t in tasks
                            if hasattr(t, "config") and t.config
                        ),
                        None,
                    )
    ...
    async def aexecute_task(self, task_id):
        ...
        if task.images and task.status == "not started":
            try:
                ...
                display_error(
                    f"Error: Missing required dependencies for image/video processing: {e}"
                )
        ...
        llm = getattr(executor_agent, "llm", None) or getattr(
            executor_agent, "llm_instance", None
        )
...
"""
        if task.context:
            ...
            for context_item in task.context:
                ...
                context_str = process_task_context(
                    context_item, self.verbose, self.user_id
                )
...
"""
        ...
        if self.verbose >= 2:
            logger.info(
                f"Executing task {task_id}: {task.description} using {executor_agent.name}"
            )
        logger.debug(
            f"Starting execution of task {task_id} with prompt:\n{task_prompt}"
        )
        ...
        if agent_output:
            ...
            if task.output_json:
                ...
                try:
                    ...
                    logger.warning(
                        f"Warning: Could not parse output of task {task_id} as JSON"
                    )
            ...
            if task.status in ["not started", "in progress"]:
                ...
                if task_output and self.completion_checker(task, task_output.raw):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error executing memory callback for task {task_id}: {e}"
                        )
                        logger.exception(e)
                    ...
                    if task.callback:
                        try:
                            ...
                            logger.error(
                                f"Error executing task callback for task {task_id}: {e}"
                            )
                            logger.exception(e)
                ...
                if task.status == "failed":
                    logger.info(
                        "Task is failed, resetting to in-progress for another try..."
                    )
                    task.status = "in progress"
                else:
    ...
    async def astart(self, content=None, return_dict=False, **kwargs):
        ...
        results = {
            ...
            "task_results": {
                task_id: self.get_task_result(task_id) for task_id in self.tasks
            },
        }
    ...
    def execute_task(self, task_id):
        ...
        logger.info(
            f"Task memory status: {'Initialized' if task.memory else 'Not initialized'}"
        )
...
"""
        ...
        if task.memory:
            try:
                ...
                if memory_context:
                    ...
                    logger.debug(
                        f"Memory context for task '{task.description}': {memory_context}"
                    )
    ...
    def get_agent_details(self, agent_name):
        agent = [
            task.agent
            for task in self.tasks.values()
            if task.agent and task.agent.name == agent_name
        ]
        if agent:
            return str(agent[0])
        return None
    ...
    def increment_state(self, key: str, amount: float = 1, default: float = 0) -> float:
        ...
        if not isinstance(current, (int, float)):
            raise TypeError(
                f"Cannot increment non-numeric value at key '{key}': {type(current).__name__}"
            )
    ...
    def restore_session_state(self, session_id: str) -> bool:
        ...
        for result in results:
            ...
            if (
                metadata.get("type") == "session_state"
    ...
    def get_detailed_token_report(self) -> Dict[str, Any]:
        ...
        output_cost = (
            total_metrics.get("output_tokens", 0) / 1000
        ) * cost_per_1k_output
    ...
    def launch(
        ...
        if protocol == "http":
            ...
            if not self.agents:
                logging.warning(
                    "No agents to launch for HTTP mode. Add agents to the Agents instance first."
                )
                return
            ...
            async def handle_query(
                ...
                try:
                    ...
                    for (
                        agent_instance
                    ) in self.agents:  # Corrected variable name to agent_instance
                        try:
                            ...
                            results.append(
                                {"agent": agent_instance.name, "response": response}
                            )
                            ...
                            results.append(
                                {"agent": agent_instance.name, "error": str(e)}
                            )
            ...
            print(
                f"📊 Available agents for this endpoint ({len(self.agents)}): {agent_names}"
            )
            ...
            if not self.agents:
                logging.warning(
                    "No agents to launch for MCP mode. Add agents to the Agents instance first."
                )
                return
            ...
            async def execute_workflow_tool(query: str) -> str:  # Renamed for clarity
                ...
                logging.info(
                    f"MCP tool '{actual_mcp_tool_name}' called with query: {query}"
                )
                current_input = query
                ...
                for agent_instance in self.agents:
                    try:
                        ...
                        if hasattr(
                            agent_instance, "achat"
                        ) and asyncio.iscoroutinefunction(agent_instance.achat):
                            response = await agent_instance.achat(
                                current_input,
                            ...
                            agent_instance, "chat"
                        ):  # Fallback to sync chat if achat not suitable
                            loop = asyncio.get_running_loop()
                            response = await loop.run_in_executor(
                                None,
                                lambda ci=current_input: agent_instance.chat(
                                    ci, tools=agent_instance.tools
                                ),
                            )
                        else:
                            logging.warning(
                                f"Agent {agent_instance.name} has no suitable chat or achat method."
                            )
                        ...
                        final_response = (
                            current_input  # Keep track of the last valid response
                        )
                        logging.debug(
                        ...
                        current_input = (
                            f"Error from agent {agent_instance.name}: {str(e)}"
                        )
                        final_response = (
                            current_input  # Update final response to show error
                        )
                ...
                logging.info(
                    f"MCP tool '{actual_mcp_tool_name}' completed. Final response: {final_response}"
                )
            ...
            async def handle_mcp_sse_connection(request: Request) -> None:
                logging.debug(
                    f"MCP SSE connection request from {request.client} for path {request.url.path}"
                )
            ...
            print(
                f"🚀 PraisonAIAgents MCP Workflow server starting on http://{host}:{port}"
            )
            ...
            mcp_tool_names = [
                actual_mcp_tool_name
            ]  # Use the determined dynamic tool name
            ...
            mcp_server_thread = threading.Thread(
                target=run_praison_mcp_server, daemon=True
            )
