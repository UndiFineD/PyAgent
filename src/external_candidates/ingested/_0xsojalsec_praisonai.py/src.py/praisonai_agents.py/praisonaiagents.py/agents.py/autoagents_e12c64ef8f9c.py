# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\agents\autoagents.py
"""
...
from ..llm import LLM, OpenAIClient, get_openai_client
from ..main import display_instruction, display_interaction, display_tool_call
...
class AutoAgents(PraisonAIAgents):
    def __init__(
        ...
        if self.verbose:
            ...
            if self.tools:
                tool_names = [
                    t.__name__ if hasattr(t, "__name__") else str(t) for t in self.tools
                ]
...
Tools: {', '.join(agent_tools)}""",
            ...
            if agent in agent_tasks:
                for i, task in enumerate(agent_tasks[agent], 1):
                    ...
                    display_instruction(f"""  📋 Task {i}: {task.name}
    ...
    def _get_tool_by_name(self, tool_name: str) -> Optional[Any]:
        ...
        for tool in self.tools:
            if (hasattr(tool, "__name__") and tool.__name__ == tool_name) or str(
                tool
            ) == tool_name:
                return tool
        return None
        ...
        for agent_idx, agent in enumerate(config.agents):
            ...
            for task_idx, task in enumerate(agent.tasks):
                ...
                if not isinstance(task, TaskConfig):
                    return (
                        False,
                        f"Task at index {task_idx} for agent '{agent.name}' is not a proper TaskConfig object",
                    )
                ...
                if not task.name:
                    return (
                        False,
                        f"Task at index {task_idx} for agent '{agent.name}' has no name",
                    )
                if not task.description:
                    return (
                        False,
                        f"Task at index {task_idx} for agent '{agent.name}' has no description",
                    )
                if not task.expected_output:
                    return (
                        False,
                        f"Task at index {task_idx} for agent '{agent.name}' has no expected_output",
                    )
                if task.tools is None:
                    return (
                        False,
                        f"Task at index {task_idx} for agent '{agent.name}' has no tools field",
                    )
            ...
            try:
                if use_openai_structured and client:
                    ...
                    config = client.parse_structured_output(
                        messages=[
                            {
                                "role": "system",
                                "content": "You are a helpful assistant designed to generate AI agent configurations.",
                            },
                    ...
                    llm_instance = LLM(
                        model=self.llm, base_url=self.base_url, api_key=self.api_key
                    )
                ...
                if not is_valid:
                    ...
                    if attempt < max_retries - 1:
                        logging.warning(
                            f"Configuration validation failed (attempt {attempt + 1}/{max_retries}): {error_msg}"
                        )
                        continue
                    else:
                        raise ValueError(
                            f"Configuration validation failed after {max_retries} attempts: {error_msg}"
                        )
                ...
                if len(config.agents) > self.max_agents:
                    ...
                    logging.warning(
                        f"Generated {len(config.agents)} agents, expected {self.max_agents}"
                    )
                ...
                if attempt < max_retries - 1:
                    logging.warning(
                        f"Error generating configuration (attempt {attempt + 1}/{max_retries}): {e}"
                    )
                    continue
                else:
                    logging.error(
                        f"Error generating configuration after {max_retries} attempts: {e}"
                    )
                    raise

        # This should never be reached due to the raise statements above
        raise RuntimeError(
            f"Failed to generate valid configuration after {max_retries} attempts"
        )

    def _create_agents_and_tasks(
        self, config: AutoAgentsConfig
