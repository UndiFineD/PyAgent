# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\agent\agent.py
import asyncio
import copy
import inspect
import json
...
class Agent:
    ...
    def __init__(
        ...
        guardrail: Optional[
            Union[Callable[["TaskOutput"], Tuple[bool, Any]], str]
        ] = None,
...
Your Goal: {self.goal}
    ...
    def _process_knowledge(self, knowledge_item):
        ...
        try:
            ...
            elif knowledge_item.startswith("http://") or knowledge_item.startswith(
                "https://"
            ):
                # It's a URL
                pass
    ...
    def _setup_guardrail(self):
        ...
        if callable(self.guardrail):
            ...
            raise ValueError(
                "Agent guardrail must be either a callable or a string description"
            )
    ...
    def _apply_guardrail_with_retry(
        ...
        while retry_count <= self.max_guardrail_retries:
            ...
            retry_count += 1
            logging.warning(
                f"Agent {self.name}: Guardrail validation failed (retry {retry_count}/{self.max_guardrail_retries}): {guardrail_result.error}"
            )
...
Your Goal: {self.goal}"""
                ...
                try:
                    if callable(tool) and hasattr(tool, "__name__"):
                        ...
                        if isinstance(openai_tools, list):
                            ...
                            isinstance(openai_tools, dict)
                            and "function" in openai_tools
                        ):
        ...
        for tool in tools:
            ...
            if isinstance(tool, dict) and tool.get("type") == "function":
                ...
                if (
                    ...
                    logging.warning(
                        f"Skipping malformed OpenAI tool: missing function or name"
                    )
            # Handle lists of tools
                ...
                for subtool in tool:
                    if isinstance(subtool, dict) and subtool.get("type") == "function":
                        ...
                        if (
                            "function" in subtool
                            and isinstance(subtool["function"], dict)
                            and "name" in subtool["function"]
                        ):
                            formatted_tools.append(subtool)
                        else:
                            logging.warning(
                                f"Skipping malformed OpenAI tool in list: missing function or name"
                            )
    ...
    def generate_task(self) -> "Task":
        ...
        description = (
            ...
            else f"Execute task as {self.role} with goal: {self.goal}"
        )
    ...
    def _cast_arguments(self, func, arguments):
        ...
        try:
            ...
            for param_name, arg_value in arguments.items():
                if param_name in sig.parameters:
                    ...
                    if param.annotation != inspect.Parameter.empty:
                        ...
                        if param.annotation == int and isinstance(
                            arg_value, (str, float)
                        ):
                            try:
                        ...
                        elif param.annotation == float and isinstance(
                            arg_value, (str, int)
                        ):
                            try:
                            ...
                            casted_args[param_name] = arg_value.lower() in (
                                "true",
                                "1",
                                "yes",
                                "on",
            ...
            logging.debug(
                f"Type casting failed for {getattr(func, '__name__', 'unknown function')}: {e}"
            )
            return arguments
    ...
    def execute_tool(self, function_name, arguments):
        ...
        if is_approval_required(function_name):
            ...
            logging.info(
                f"Tool {function_name} requires approval (risk level: {risk_level})"
            )
                ...
                if hasattr(self.tools, "sse_client"):
                    for tool in self.tools.sse_client.tools:
                        if tool.name == function_name:
                            logging.debug(
                                f"Found matching SSE MCP tool: {function_name}"
                            )
        ...
        if func:
            try:
                ...
                if (
                    ...
                    and hasattr(func, "run")
                    and not hasattr(func, "_run")
                ):
                    instance = func()
                    run_params = {
                        k: v
    ...
    def _process_stream_response(
        ...
        if self._openai_client is None:
            raise ValueError(
                "OpenAI client is not initialized. Please provide OPENAI_API_KEY or use a custom LLM provider."
            )
        ...
        try:
            ...
            if self._using_custom_llm and hasattr(self, "llm_instance"):
                if stream:
                    ...
                    if formatted_tools:
                        logging.debug(
                            f"Passing {len(formatted_tools)} formatted tools to LLM instance: {formatted_tools}"
                        )
                    ...
                    final_response = self.llm_instance.get_response(
                        prompt=messages[
                            1:
                        ],  # Skip system message as LLM handles it separately
                        system_prompt=(
                            messages[0]["content"]
                        ...
                        agent_tools=(
                            [t.__name__ for t in self.tools] if self.tools else None
                        ),
                        task_name=task_name,
                        task_description=task_description,
                        task_id=task_id,
                    ...
                    if (
                        False
                    ):  # Don't use display_generating when stream=False to avoid streaming-like behavior
    ...
    def _execute_callback_and_display(
        self,
        prompt: str,
        response: str,
    ...
    def chat(
        ...
        if logging.getLogger().getEffectiveLevel() == logging.DEBUG:
            param_info = {
                "prompt": (
                    str(prompt)[:100] + "..."
                    if isinstance(prompt, str) and len(str(prompt)) > 100
                    else str(prompt)
                ),
                "temperature": temperature,
                "tools": (
                    [t.__name__ if hasattr(t, "__name__") else str(t) for t in tools]
                    if tools
                    else None
                ),
                "output_json": (
                    str(output_json.__class__.__name__) if output_json else None
                ),
                "output_pydantic": (
                    str(output_pydantic.__class__.__name__) if output_pydantic else None
                ),
                "reasoning_steps": reasoning_steps,
                "agent_name": self.name,
                "agent_role": self.role,
        ...
        if self._using_custom_llm:
            try:
                ...
                if tool_param is not None:
                    ...
                    if isinstance(tool_param, MCP) and hasattr(
                        tool_param, "to_openai_tool"
                    ):
                ...
                if isinstance(prompt, list):
                    ...
                    normalized_content = next(
                        (item["text"] for item in prompt if item.get("type") == "text"),
                        "",
                    )
                ...
                if not (
                    ...
                    and self.chat_history[-1].get("content") == normalized_content
                ):
                ...
                try:
                    ...
                    response_text = self.llm_instance.get_response(
                        ...
                        agent_tools=[
                            ...
                            for t in (tools if tools is not None else self.tools)
                        ],
                        task_name=task_name,
                        task_description=task_description,
                        task_id=task_id,
                    ...
                    try:
                        ...
                        logging.error(
                            f"Agent {self.name}: Guardrail validation failed for custom LLM: {e}"
                        )
                    ...
                    try:
                        ...
                        if output_json or output_pydantic:
                            ...
                            try:
                                ...
                                logging.error(
                                    f"Agent {self.name}: Guardrail validation failed for JSON output: {e}"
                                )
                                # Rollback chat history on guardrail failure
                        ...
                        if not self.self_reflect:
                            ...
                            if reasoning_steps and hasattr(
                                response.choices[0].message, "reasoning_content"
                            ):
                                # Apply guardrail to reasoning content
                                try:
                                    validated_reasoning = (
                                        self._apply_guardrail_with_retry(
                                            response.choices[
                                                0
                                    ...
                                    logging.error(
                                        f"Agent {self.name}: Guardrail validation failed for reasoning content: {e}"
                                    )
                                    # Rollback chat history on guardrail failure
                                    self.chat_history = self.chat_history[
                                        :chat_history_length
                            ...
                            try:
                                ...
                                logging.error(
                                    f"Agent {self.name}: Guardrail validation failed: {e}"
                                )
                                # Rollback chat history on guardrail failure
                                self.chat_history = self.chat_history[
                                    :chat_history_length
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
                        ...
                        try:
                            ...
                            if self._using_custom_llm or self._openai_client is None:
                                ...
                                reflection_response = self._chat_completion(
                                    messages,
                                    temperature=temperature,
                                    tools=None,
                                    stream=False,
                                ...
                                if (
                                    not reflection_response
                                    or not reflection_response.choices
                                ):
                                    raise Exception(
                                        "No response from reflection request"
                                    )

                                reflection_text = reflection_response.choices[
                                    0
                                ...
                                class CustomReflectionOutput:
                                    def __init__(self, data):
                                        ...
                                        self.satisfactory = data.get(
                                            "satisfactory", "no"
                                        ).lower()

                                reflection_output = CustomReflectionOutput(
                                    reflection_data
                                )
                            else:
                                ...
                                reflection_output = reflection_response.choices[
                                    0
                                ].message.parsed

                            if self.verbose:
                                display_self_reflection(
                                    f"Agent {self.name} self reflection (using {self.reflect_llm if self.reflect_llm else self.llm}): reflection='{reflection_output.reflection}' satisfactory='{reflection_output.satisfactory}'",
                                    console=self.console,
                                )
                            ...
                            messages.append(
                                {
                                    "role": "assistant",
                                    "content": f"Self Reflection: {reflection_output.reflection} Satisfactory?: {reflection_output.satisfactory}",
                                }
                            )

                            # Only consider satisfactory after minimum reflections
                            if (
                                reflection_output.satisfactory == "yes"
                                and reflection_count >= self.min_reflect - 1
                            ):
                                if self.verbose:
                                    display_self_reflection(
                                        "Agent marked the response as satisfactory after meeting minimum reflections",
                                        console=self.console,
                                    )
                                # User message already added before LLM call via _build_messages
                                self.chat_history.append(
                                ...
                                try:
                                    ...
                                    logging.error(
                                        f"Agent {self.name}: Guardrail validation failed after reflection: {e}"
                                    )
                                    # Rollback chat history on guardrail failure
                                    self.chat_history = self.chat_history[
                                        :chat_history_length
                            ...
                            if reflection_count >= self.max_reflect - 1:
                                if self.verbose:
                                    display_self_reflection(
                                        "Maximum reflection count reached, returning current response",
                                        console=self.console,
                                    )
                                # User message already added before LLM call via _build_messages
                                self.chat_history.append(
                                ...
                                try:
                                    ...
                                    logging.error(
                                        f"Agent {self.name}: Guardrail validation failed after max reflections: {e}"
                                    )
                                    # Rollback chat history on guardrail failure
                                    self.chat_history = self.chat_history[
                                        :chat_history_length
                            ...
                            messages.append(
                                {
                                    "role": "user",
                                    "content": "Now regenerate your response using the reflection you made",
                                }
                            )
                            # For custom LLMs during reflection, always use non-streaming to ensure complete responses
                            use_stream = (
                                self.stream if not self._using_custom_llm else False
                            )
                            response = self._chat_completion(
                                messages,
                                temperature=temperature,
                            ...
                            display_error(
                                f"Error in parsing self-reflection json {e}. Retrying",
                                console=self.console,
                            )
                            ...
                            messages.append(
                                {
                                    "role": "assistant",
                                    "content": "Self Reflection failed.",
                                }
                            )
                            reflection_count += 1
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
                                ...
                                try:
                                    ...
                                    if self._openai_client is None:
                                        ...
                                        if self.verbose:
                                            display_self_reflection(
                                                f"Agent {self.name}: Self-reflection with structured output is not supported for custom LLM providers. Skipping reflection.",
                                                console=self.console,
                                            )
                                    ...
                                    response_text = new_response.choices[
                                        0
                                    ].message.content
                                    reflection_count += 1
                        ...
                        try:
                            ...
                            logging.error(
                                f"Agent {self.name}: Guardrail validation failed for OpenAI client: {e}"
                            )
                            # Rollback chat history on guardrail failure
                            self.chat_history = self.chat_history[:chat_history_length]
                            return None
            ...
            for tool_call in message.tool_calls:
                try:
                    ...
                    try:
                        ...
                        logging.error(
                            f"Failed to parse tool arguments as JSON: {json_error}"
                        )
                        arguments = {}
        ...
        try:
            ...
            if self._using_custom_llm:
                ...
                messages, original_prompt = self._build_messages(
                    ...
                    kwargs.get("output_json"),
                    kwargs.get("output_pydantic"),
                )
                ...
                try:
                    ...
                    completion = (
                        self._openai_client.sync_client.chat.completions.create(
                            **completion_args
                        )
                    )
                        ...
                        if hasattr(delta, "tool_calls") and delta.tool_calls:
                            for tool_call_delta in delta.tool_calls:
                                ...
                                if tool_call_delta.id:
                                    tool_calls_data[tool_call_delta.index][
                                        "id"
                                    ] = tool_call_delta.id
                                if tool_call_delta.function.name:
                                    tool_calls_data[tool_call_delta.index]["function"][
                                        "name"
                        ...
                        for tool_call in tool_calls_data:
                            if tool_call["id"] and tool_call["function"]["name"]:
                                try:
                                    ...
                                    logging.error(
                                        f"Tool execution error in streaming: {tool_error}"
                                    )
    ...
    def launch(
        ...
        if protocol == "http":
            ...
            try:
                ...
                display_error(
                    f"Missing dependency: {missing_module}. Required for launch() method with HTTP mode."
                )
                logging.error(
                    f"Missing dependency: {missing_module}. Required for launch() method with HTTP mode."
                )
            ...
            if path in _registered_agents[port]:
                logging.warning(
                    f"Path '{path}' is already registered on port {port}. Please use a different path."
                )
                print(
                    f"⚠️ Warning: Path '{path}' is already registered on port {port}."
                )
            ...
            async def handle_agent_query(
                request: Request, query_data: Optional[AgentQuery] = None
            ):
                ...
                try:
                    ...
                    if asyncio.iscoroutinefunction(self.chat):
                        response = await self.achat(
                            query, task_name=None, task_description=None, task_id=None
                        )
                    else:
                        # Run sync function in a thread to avoid blocking
                        loop = asyncio.get_event_loop()
                        response = await loop.run_in_executor(
                            None, lambda p=query: self.chat(p)
                        )
            ...
            if not _server_started.get(port, False):
                ...
                def run_server():
                    try:
                        ...
                        print(
                            f"🔌 Available endpoints: {', '.join(list(_registered_agents[port].keys()))}"
                        )
                        uvicorn.run(
                            _shared_apps[port],
                            host=host,
                ...
                time.sleep(0.1)
                print(
                    f"🔌 Available endpoints on port {port}: {', '.join(list(_registered_agents[port].keys()))}"
                )
            ...
            if len(stack) > 1 and stack[1].filename.endswith(".py"):
                ...
                try:
                    ...
                    for line_content in lines[
                        caller_line:
                    ]:  # renamed line to line_content
                        if (
                            ".launch(" in line_content
            ...
            try:
                ...
                missing_module = str(e).split("No module named '")[-1].rstrip("'")
                display_error(
                    f"Missing dependency: {missing_module}. Required for launch() method with MCP mode."
                )
                logging.error(
                    f"Missing dependency: {missing_module}. Required for launch() method with MCP mode."
                )
                ...
                print(
                    f"pip install {missing_module} mcp praison-mcp starlette uvicorn"
                )  # Added mcp, praison-mcp, starlette, uvicorn
                print("\nOr install all MCP dependencies with relevant packages.")
                return None

            mcp_server_instance_name = (
                f"{self.name}_mcp_server" if self.name else "agent_mcp_server"
            )
            mcp = FastMCP(mcp_server_instance_name)

            # Determine the MCP tool name based on self.name
            actual_mcp_tool_name = (
                f"execute_{self.name.lower().replace(' ', '_').replace('-', '_')}_task"
                if self.name
                else "execute_task"
            )
            ...
            async def execute_agent_task(prompt: str) -> str:
                ...
                logging.info(
                    f"MCP tool '{actual_mcp_tool_name}' called with prompt: {prompt}"
                )
                try:
                    # Ensure self.achat is used as it's the async version and pass its tools
                    if hasattr(self, "achat") and asyncio.iscoroutinefunction(
                        self.achat
                    ):
                        response = await self.achat(
                            prompt,
                        ...
                        loop = asyncio.get_event_loop()
                        response = await loop.run_in_executor(
                            None, lambda p=prompt: self.chat(p, tools=self.tools)
                        )
                    else:
                        logging.error(
                            f"Agent {self.name} has no suitable chat or achat method for MCP tool."
                        )
                    ...
                    logging.error(
                        f"Error in MCP tool '{actual_mcp_tool_name}': {e}",
                        exc_info=True,
                    )
            ...
            sse_transport = SseServerTransport(
                messages_path_prefix
            )  # Pass the full prefix

            async def handle_sse_connection(request: Request) -> None:
                logging.debug(
                    f"SSE connection request received from {request.client} for path {request.url.path}"
                )
