# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\agent\router_agent.py
"""
...
class RouterAgent(Agent):
        ...
        **kwargs,
        ...
        if primary_model:
            ...
            cheapest_model = min(
                ...
                key=lambda m: (
                    self.model_router.get_model_info(m).cost_per_1k_tokens
                    if self.model_router.get_model_info(m)
                    else float("inf")
                ),
            )
    ...
    def _process_models_config(
        self, models: Optional[Union[List[str], Dict[str, Any]]]
    ) -> Dict[str, Any]:
    ...
    def _execute_with_model(
        ...
        **kwargs,
        ...
        try:
            ...
            if (
                model_name != self.fallback_model
                and self.fallback_model in self._llm_instances
            ):
    ...
    def execute(
        ...
        **kwargs,
        ...
        selected_model = self._select_model_for_task(
            task_description=task_description, tools=tools, context_size=context_size
        )

        logger.info(
            f"RouterAgent '{self.name}' selected model: {selected_model} for task"
        )
