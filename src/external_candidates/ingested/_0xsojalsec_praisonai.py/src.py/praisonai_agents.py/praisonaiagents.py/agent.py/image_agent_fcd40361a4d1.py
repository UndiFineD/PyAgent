# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\agent\image_agent.py
"""
...
class ImageGenerationConfig(BaseModel):
    ...
    api_version: Optional[str] = Field(
        default=None, description="Optional API version (required for Azure dall-e-3)"
    )
...
class ImageAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        backstory = (
            backstory
            or "I am an AI assistant specialized in generating images from textual descriptions"
        )
    ...
    def generate_image(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        model_name = (
            model_info.model if hasattr(model_info, "model") else str(model_info)
        )
        ...
        elif custom_llm_provider == "gemini" or (
            custom_llm_provider is None and "gemini" in model_name.lower()
        ):
        ...
        with Progress(
            ...
            try:
                ...
                task = progress.add_task(
                    f"[cyan]Generating image with {model_name}...", total=None
                )

                # Use litellm's image generation with parameter dropping enabled as safety net
                response = self.litellm(prompt=prompt, drop_params=True, **config)
    ...
    def chat(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.verbose:
                self.console.print(
                    f"[green]Successfully generated image from prompt[/green]"
                )
            return result
        except Exception as e:
