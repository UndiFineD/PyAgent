...
class Process:
    ...
    def _create_loop_subtasks(self, loop_task: Task):
        ...
        if not loop_task.input_file:
            logging.warning(
                f"_create_loop_subtasks called for {loop_task.name} but no input_file specified"
            )
            return
        ...
        try:
            ...
            if file_ext == ".csv":
                with open(loop_task.input_file, "r", encoding="utf-8") as f:
                    ...
                    for i, row in enumerate(reader):
                        ...
                        task_count += 1
                        logging.debug(
                            f"Creating subtask from CSV row {i+1}: {task_desc}"
                        )

                        row_task = Task(
                            description=(
                                f"{loop_task.description}\n{task_desc}"
                                if loop_task.description
                                else task_desc
                            ),
                            agent=loop_task.agent,
                            name=(
                                f"{loop_task.name}_{task_count}"
                                if loop_task.name
                                else task_desc
                            ),
                    ...
                    logging.info(
                        f"Created {task_count} subtasks from CSV file for {loop_task.name}"
                    )
            else:
                # Handle text files
                with open(loop_task.input_file, "r", encoding="utf-8") as f:
                    ...
                    logging.info(
                        f"Created {len(new_tasks)} subtasks from text file for {loop_task.name}"
                    )
            ...
            logging.error(
                f"Failed to create subtasks for loop task {loop_task.name}: {e}"
            )
            import traceback
    ...
    def _build_task_context(self, current_task: Task) -> str:
        ...
        if current_task.retain_full_context:
            ...
            for prev_name in current_task.previous_tasks:
                prev_task = next(
                    (t for t in self.tasks.values() if t.name == prev_name), None
                )
    ...
    def _find_next_not_started_task(self) -> Optional[Task]:
        ...
        for task in self.tasks.values():
            if (
                ...
                and "Input data from previous tasks:" in task.description
            ):
        ...
        while fallback_attempts < Process.DEFAULT_RETRY_LIMIT and not temp_current_task:
            fallback_attempts += 1
            logging.debug(
                f"Fallback attempt {fallback_attempts}: Trying to find next 'not started' task."
            )
            for task_candidate in self.tasks.values():
                if task_candidate.status == "not started":
                    ...
                    if (
                        self.task_retry_counter.get(task_candidate.id, 0)
                        < Process.DEFAULT_RETRY_LIMIT
                    ):
            ...
            if not temp_current_task:
                logging.debug(
                    f"Fallback attempt {fallback_attempts}: No 'not started' task found within retry limit."
                )
        return None  # Return None if no task found after all attempts

    async def _get_manager_instructions_with_fallback_async(
        self, manager_task, manager_prompt, ManagerInstructions
    ):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    required_desc = (
                        f" (required: {required_props_str})" if required_props else ""
                    )
                ...
                return await self._get_json_response_async(
                    manager_task, enhanced_prompt, ManagerInstructions
                )
            except Exception as fallback_error:
                error_msg = (
                    f"Both structured output and JSON fallback failed: {fallback_error}"
                )
                logging.error(error_msg, exc_info=True)
                raise Exception(error_msg) from fallback_error

    def _get_manager_instructions_with_fallback(
        self, manager_task, manager_prompt, ManagerInstructions
    ):
    ...
    async def _get_json_response_async(
        self, manager_task, enhanced_prompt, ManagerInstructions
    ):
    ...
    async def aworkflow(self) -> AsyncGenerator[str, None]:
            ...
            if task.next_tasks:
                for next_task_name in task.next_tasks:
                    ...
                    if next_task:
                        ...
                        logging.debug(
                            f"Added {task.name} as previous task for {next_task_name}"
                        )
        ...
        while current_task:
            ...
            if current_iter > self.max_iter:
                logging.info(
                    f"Max iteration limit {self.max_iter} reached, ending workflow."
                )
                break
...
- Condition: {st.condition}
                    ...
                    if subtasks and all(st.status == "completed" for st in subtasks):
                        logging.debug(
                            f"=== All {len(subtasks)} subtasks completed for {current_task.name} ==="
                        )
                        ...
                        if current_task.condition:
                            ...
                            if current_task.result:
                                if current_task.result.pydantic and hasattr(
                                    current_task.result.pydantic, "decision"
                                ):
                                    decision_str = (
                                        current_task.result.pydantic.decision.lower()
                                    )
                                elif current_task.result.raw:
                                    decision_str = current_task.result.raw.lower()

                            # For loop tasks, use "done" to follow condition path
                            if current_task.task_type == "loop" and all(
                                t.status == "completed" for t in subtasks
                            ):
                                decision_str = "done"

                            target_tasks = (
                                current_task.condition.get(decision_str, [])
                                if decision_str
                                else []
                            )
                            task_value = (
                                target_tasks[0]
                                if isinstance(target_tasks, list)
                                else target_tasks
                            )
                            next_task = next(
                                (
                                    t
                                    for t in self.tasks.values()
                                    if t.name == task_value
                                ),
                                None,
                            )
                            if next_task:
                                next_task.status = (
                                    "not started"  # Reset status to allow execution
                                )
                                logging.debug(
                                    f"Routing to {next_task.name} based on decision: {decision_str}"
                                )
                                ...
                                logging.info(
                                    f"No valid next task found for decision: {decision_str}"
                                )
                                self.workflow_finished = True
                                current_task = None
                                break
                    ...
                    if current_task.input_file:
                        ...
                        logging.debug(
                            f"Created subtasks from {current_task.input_file}"
                        )
                    else:
                        # No input file, mark as done
                        current_task.status = "completed"
                        logging.debug(
                            f"No input file, marking {current_task.name} as completed"
                        )
                ...
                logging.debug(
                    f"=== Executing non-loop task: {current_task.name} (id: {task_id}) ==="
                )
                ...
                if (
                    ...
                    and not any(
                        t.task_type == "loop"
                        and current_task.name.startswith(t.name + "_")
                        for t in self.tasks.values()
                    )
                ):
                    logging.info(
                        f"Task {current_task.name} has no next tasks, ending workflow"
                    )
                    self.workflow_finished = True
                    current_task = None
                    break
            ...
            if self.tasks[task_id].status == "completed":
                ...
                if (
                    ...
                    and task_to_check.task_type
                    != "loop"  # Removed "decision" from exclusion
                    and not any(
                        t.task_type == "loop" and subtask_name.startswith(t.name + "_")
                        for t in self.tasks.values()
                    )
                    ...
                    logging.debug(
                        f"Task status after reset: {self.tasks[task_id].status}"
                    )
                else:
                    logging.debug(
                ...
                if current_task.task_type in ["decision", "loop"]:
                    ...
                    if current_task.condition:
                        ...
                        if (
                            ...
                            or (
                                isinstance(target_tasks, list)
                                and (not target_tasks or target_tasks[0] == "exit")
                            )
                        ):
                            logging.info(
                                f"Workflow exit condition met on decision: {decision_str}"
                            )
                            self.workflow_finished = True
                            current_task = None
                            break
                                ...
                                if decision_str in Process.VALIDATION_FAILURE_DECISIONS:
                                    if current_task and current_task.result:
                                        ...
                                        if current_task.previous_tasks:
                                            ...
                                            prev_task_name = (
                                                current_task.previous_tasks[-1]
                                            )
                                            validated_task = next(
                                                (
                                                    t
                                                    for t in self.tasks.values()
                                                    if t.name == prev_task_name
                                                ),
                                                None,
                                            )
            ...
            if not next_task and current_task and current_task.next_tasks:
                next_task_name = current_task.next_tasks[0]
                next_task = next(
                    (t for t in self.tasks.values() if t.name == next_task_name), None
                )
                if next_task:
                    ...
                    if (
                        ...
                        and next_task.name
                        in self.tasks[next_task.next_tasks[0]].previous_tasks
                    ):
                        self.workflow_finished = False
            ...
            if not current_task:
                current_task = (
                    self._find_next_not_started_task()
                )  # General fallback if no next task in workflow
...
Workflow Finished: {self.workflow_finished} # ADDED: Workflow Finished Status
    ...
    async def ahierarchical(self) -> AsyncGenerator[str, None]:
        ...
        logging.debug(
            f"Starting hierarchical task execution with {len(self.tasks)} tasks"
        )
        manager_agent = Agent(
            name="Manager",
...
"""
            ...
            try:
                ...
                if manager_task.async_execution:
                    parsed_instructions = (
                        await self._get_manager_instructions_with_fallback_async(
                            manager_task, manager_prompt, ManagerInstructions
                        )
                    )
                else:
            ...
            logging.info(
                f"Manager selected task_id={selected_task_id}, agent={selected_agent_name}, action={action}"
            )
            ...
            original_agent = (
                self.tasks[selected_task_id].agent.name
                if self.tasks[selected_task_id].agent
                else "None"
            )
            ...
            if self.tasks[selected_task_id].status == "completed":
                completed_count += 1
                logging.info(
                    f"Task {selected_task_id} completed. Total completed: {completed_count}/{total_tasks}"
                )
    ...
    def workflow(self):
        ...
        if (
            start_task
            and start_task.task_type == "loop"
            and getattr(start_task, "input_file", None)
        ):
            try:
                ...
                if file_ext == ".csv":
                    with open(start_task.input_file, "r", encoding="utf-8") as f:
                        reader = csv.reader(
                            f, quotechar='"', escapechar="\\"
                        )  # Handle quoted/escaped fields
                        previous_task = None
                        task_count = 0
                        ...
                        for i, row in enumerate(reader):
                            ...
                            inherited_next_tasks = (
                                start_task.next_tasks if start_task.next_tasks else []
                            )
                            ...
                            row_task = Task(
                                ...
                                name=(
                                    f"{start_task.name}_{task_count}"
                                    if start_task.name
                                    else task_desc
                                ),
                                expected_output=getattr(
                                    start_task, "expected_output", None
                                ),
                            ...
                            if previous_task:
                                previous_task.next_tasks = [row_task.name]
                                previous_task.condition["done"] = [
                                    row_task.name
                                ]  # Use "done" consistently
                            previous_task = row_task
                ...
                if new_tasks:
                    ...
                    logging.info(
                        f"Created {len(new_tasks)} tasks from: {start_task.input_file}"
                    )
            except Exception as e:
            ...
            if (
                ...
                if getattr(current_task, "input_file", None):
                    try:
                        ...
                        if file_ext == ".csv":
                            with open(
                                current_task.input_file, "r", encoding="utf-8"
                            ) as f:
                                reader = csv.reader(f)
                                previous_task = None
                                for i, row in enumerate(reader):
                                    if row:  # Skip empty rows
                                        ...
                                        row_task = Task(
                                            ...
                                            name=(
                                                f"{current_task.name}_{i+1}"
                                                if current_task.name
                                                else task_desc
                                            ),
                                            expected_output=getattr(
                                                current_task, "expected_output", None
                                            ),
                                        ...
                                        if previous_task:
                                            previous_task.next_tasks = [row_task.name]
                                            previous_task.condition["complete"] = [
                                                row_task.name
                                            ]
                                        previous_task = row_task
                        else:
                        ...
                        logging.error(
                            f"Failed to read file tasks for loop task {current_task.name}: {e}"
                        )
