# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\__init__.py
...
TOOL_MAPPINGS = {
    ...
    "cot_run_dict": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Orchestrates dict-based solution
    "cot_generate": (
        ".train.data.generatecot",
        "GenerateCOT",
    ...
    "cot_improve_dict": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Improve dict-based solution
    "cot_check": (".train.data.generatecot", "GenerateCOT"),  # Check correctness
    "cot_find_error": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Locate error in solution
    "cot_load_answers": (".train.data.generatecot", "GenerateCOT"),  # Load QA pairs
    # COT Save/Export with QA Pairs
    "cot_save_solutions_with_qa_pairs": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Save with QA pairs
    "cot_append_solutions_with_qa_pairs": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Append with QA pairs
    "cot_export_json_with_qa_pairs": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Export JSON with QA pairs
    "cot_export_csv_with_qa_pairs": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Export CSV with QA pairs
    "cot_append_csv_with_qa_pairs": (
        ".train.data.generatecot",
        "GenerateCOT",
    ),  # Append CSV with QA pairs
