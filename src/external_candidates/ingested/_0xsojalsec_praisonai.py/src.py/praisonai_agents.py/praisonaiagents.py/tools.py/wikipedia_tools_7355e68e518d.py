# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\wikipedia_tools.py
...
class WikipediaTools:
    ...
    def wiki_search(
        self, query: str, results: int = 10, suggestion: bool = True
        ...
        try:
            search_results = wikipedia.search(
                query, results=results, suggestion=suggestion
            )
    ...
    def wiki_summary(
        self, title: str, sentences: int = 5, auto_suggest: bool = True
        ...
        try:
            return wikipedia.summary(
                title, sentences=sentences, auto_suggest=auto_suggest
            )
    ...
    def wiki_page(
        self, title: str, auto_suggest: bool = True
    ...
    def get_sections(self, page: Any) -> List[Dict[str, Any]]:
        ...
        if intro:
            sections.append({"title": "Introduction", "level": 0, "content": intro})
        ...
        for title, content in zip(section_titles, section_contents):
            ...
            sections.append(
                {"title": clean_title, "level": level, "content": content.strip()}
            )
