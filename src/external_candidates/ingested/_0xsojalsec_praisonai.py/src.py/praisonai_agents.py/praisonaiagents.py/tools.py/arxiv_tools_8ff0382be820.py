# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\arxiv_tools.py
...
SORT_ORDER = {"ascending": "Ascending", "descending": "Descending"}
...
class ArxivTools:
    ...
    def _check_arxiv(self):
        ...
        if util.find_spec("arxiv") is None:
            raise ImportError(
                "arxiv package is not available. Please install it using: pip install arxiv"
            )
        global arxiv
        import arxiv
    ...
    def get_paper(
        self, paper_id: str, include_fields: Optional[List[str]] = None
    ...
    def _result_to_dict(
        self, result: Any, include_fields: Optional[List[str]] = None
    ) -> Dict[str, Any]:
