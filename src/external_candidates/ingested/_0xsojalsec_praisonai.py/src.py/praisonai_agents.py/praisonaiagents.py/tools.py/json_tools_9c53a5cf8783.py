...
class JSONTools:
    ...
    def validate_json(
        self, data: Union[Dict[str, Any], str], schema: Dict[str, Any]
    ...
    def analyze_json(
        self, data: Union[Dict[str, Any], str], max_depth: int = 10
    ) -> Dict[str, Any]:
            ...
            def analyze_value(value: Any, depth: int = 0) -> Dict[str, Any]:
                ...
                if isinstance(value, dict):
                    ...
                    if depth < max_depth - 1:
                        result["children"] = {
                            k: analyze_value(v, depth + 1) for k, v in value.items()
                        }

                elif isinstance(value, list):
                    result["length"] = len(value)
                    if value:
                        result["element_types"] = list(
                            set(type(x).__name__ for x in value)
                        )
                        if depth < max_depth - 1:
                            result["sample_elements"] = [
                                analyze_value(x, depth + 1) for x in value[:5]
                            ]

                elif isinstance(value, (int, float)):
                    result.update(
                        {"value": value, "is_integer": isinstance(value, int)}
                    )
    ...
    def transform_json(
        self, data: Union[Dict[str, Any], str], transformations: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _rename_key(
        self, data: Dict[str, Any], old_path: List[str], new_path: List[str]
    ):
