# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\tools\csv_tools.py
    ...
    def write_csv(
        ...
        try:
            ...
            if isinstance(data, str):
                ...
                write_header = (
                    header if mode == "w" else (header and not Path(filepath).exists())
                )
    ...
    def merge_csv(
        ...
        try:
            ...
            for file in files[1:]:
                df = pd.read_csv(file)
                result = pd.merge(
                    result, df, how=how, on=on, suffixes=suffixes or ("_1", "_2")
                )
    ...
    with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as temp:
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as temp2:
            ...
            with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as temp3:
                ...
                result = merge_csv(
                    [temp_file, temp_file2], temp_file3, how="inner", on="name"
                )
