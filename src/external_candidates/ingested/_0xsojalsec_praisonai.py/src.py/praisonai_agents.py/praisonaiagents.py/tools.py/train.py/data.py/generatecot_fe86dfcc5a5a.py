...
class GenerateCOT:
    def __init__(
        ...
        logger.debug(
            f"Initialized GenerateCOT with model={model}, max_attempts={max_attempts}"
        )
    ...
    def cot_find_error(self, question: str, solution: str) -> int:
        ...
        sentences = [
            s.strip() for s in solution.replace("。", ".").split(".") if s.strip()
        ]
    ...
    def _is_qa_pairs(self, qa_pairs: Any) -> bool:
        ...
        if not isinstance(qa_pairs, dict):
            raise ValueError(
                "qa_pairs must be a dictionary with questions as keys and answers as values"
            )
        return True
    ...
    def cot_run_dict(self, question: str) -> dict:
        ...
        if error_pos != -1:
            logger.debug(
                f"Found error at position {error_pos}, generating final solution"
            )
            partial_solution = (
                ". ".join(improved["thought_process"].split(". ")[:error_pos]) + "."
            )
    ...
    def cot_save(
        self, question: str, answer: str, filepath: str = "dataset.csv"
    ) -> Optional[str]:
    ...
    def cot_upload_to_huggingface(
        ...
        logger.debug(
            f"Attempting to upload {filepath} to HuggingFace as {dataset_name}"
        )
        try:
            import pandas as pd
...
if __name__ == "__main__":
    ...
    direct_generator.cot_export_csv_with_qa_pairs(
        filepath="direct_solutions.csv", qa_pairs=direct_qa_data
    )
