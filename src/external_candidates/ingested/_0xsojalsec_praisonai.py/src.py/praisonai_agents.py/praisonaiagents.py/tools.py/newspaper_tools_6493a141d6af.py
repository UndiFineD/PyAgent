...
class NewspaperTools:
    ...
    def _check_newspaper(self):
        ...
        if util.find_spec("newspaper") is None:
            raise ImportError(
                "newspaper3k package is not available. Please install it using: pip install newspaper3k"
            )
        global newspaper
        import newspaper
    ...
    def get_trending_topics(
        ...
        try:
            ...
            for source_url in sources:
                try:
                    articles = self.get_articles_from_source(
                        source_url, limit=5, language=language
                    )
