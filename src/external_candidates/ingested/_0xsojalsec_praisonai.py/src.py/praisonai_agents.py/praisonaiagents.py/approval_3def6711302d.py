...
def console_approval_callback(
    function_name: str, arguments: Dict[str, Any], risk_level: str
) -> ApprovalDecision:
    ...
    tool_info += (
        f"[bold]Risk Level:[/] [{risk_color}]{risk_level.upper()}[/{risk_color}]\n"
    )
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(callback):
            ...
            decision = await loop.run_in_executor(
                None, callback, function_name, arguments, risk_level
            )
