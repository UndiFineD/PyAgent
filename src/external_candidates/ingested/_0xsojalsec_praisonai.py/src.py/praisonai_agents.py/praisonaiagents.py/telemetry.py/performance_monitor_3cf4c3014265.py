# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\telemetry\performance_monitor.py
"""
...
class PerformanceMonitor:
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self._record_function_performance(
                        name, execution_time, success, error
                    )
        ...
        with self._lock:
            ...
            if self._telemetry:
                self._telemetry.track_tool_usage(
                    tool_name=func_name, success=success, execution_time=execution_time
                )
            ...
            if func_name:
                ...
                if stats["call_count"] > 0:
                    ...
                    if stats["recent_times"]:
                        stats["recent_average"] = sum(stats["recent_times"]) / len(
                            stats["recent_times"]
                        )
                ...
                for name, stats in self._function_stats.items():
                    ...
                    processed_stats["recent_times"] = list(
                        processed_stats["recent_times"]
                    )

                    if processed_stats["call_count"] > 0:
                        processed_stats["average_time"] = (
                            processed_stats["total_time"]
                            / processed_stats["call_count"]
                        )
                        processed_stats["success_rate"] = (
                            processed_stats["call_count"]
                            - processed_stats["error_count"]
                        ) / processed_stats["call_count"]

                        if processed_stats["recent_times"]:
                            processed_stats["recent_average"] = sum(
                                processed_stats["recent_times"]
                            ) / len(processed_stats["recent_times"])

                    result[name] = processed_stats

                return result

    def get_api_call_performance(
            ...
            if api_name:
                ...
                for name, stats in self._api_calls.items():
                    ...
                    if processed_stats["call_count"] > 0:
                        ...
                        processed_stats["success_rate"] = (
                            processed_stats["success_count"]
                            / processed_stats["call_count"]
                        )
                        processed_stats["error_rate"] = (
                            processed_stats["error_count"]
                            / processed_stats["call_count"]
                        )
    ...
    def generate_performance_report(self) -> str:
        ...
        if func_stats:
            ...
            report.append(
                f"Overall Success Rate: {((total_calls - total_errors) / total_calls * 100):.1f}%"
                if total_calls > 0
                else "N/A"
            )
            report.append("")
            ...
            if slowest:
                ...
                for i, func in enumerate(slowest, 1):
                    ...
                    report.append(
                        f"   Average: {func['average_time']:.3f}s | Max: {func['max_time']:.3f}s | Calls: {func['call_count']}"
                    )
                report.append("")
        ...
        if api_stats:
            ...
            report.append(
                f"Overall API Success Rate: {((total_api_calls - total_api_errors) / total_api_calls * 100):.1f}%"
            ...
            if slowest_apis:
                ...
                for i, api in enumerate(slowest_apis, 1):
                    ...
                    report.append(
                        f"   Average: {api['average_time']:.3f}s | Max: {api['max_time']:.3f}s | Success Rate: {api['success_rate']*100:.1f}%"
                    )
                report.append("")
    ...
    def export_metrics_for_external_apm(
        self, service_name: str = "praisonai-agents"
    ) -> Dict[str, Any]:
        """
                ...
                metrics.append(
                    {
                        ...
                        "timestamp": timestamp,
                        "tags": {"service": service_name, "function_name": func_name},
                    }
                )
                ...
                metrics.append(
                    {
                        ...
                        "value": stats["call_count"],
                        "timestamp": timestamp,
                        "tags": {"service": service_name, "api_name": api_name},
                    }
                )
...
def export_external_apm_metrics(
    service_name: str = "praisonai-agents",
) -> Dict[str, Any]:
