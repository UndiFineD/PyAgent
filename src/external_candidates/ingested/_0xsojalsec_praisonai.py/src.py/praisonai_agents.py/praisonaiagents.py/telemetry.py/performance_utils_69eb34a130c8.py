# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\telemetry\performance_utils.py
"""
...
_FLOW_ANALYSIS_ENABLED = os.environ.get(
    "PRAISONAI_FLOW_ANALYSIS_ENABLED", ""
).lower() in ("true", "1", "yes")
...
class FunctionFlowAnalyzer:
    ...
    def analyze_execution_flow(
        self, flow_data: Optional[List[Dict]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if flow_data is None:
            if not PERFORMANCE_MONITOR_AVAILABLE:
                return {
                    "error": "Performance monitor not available and no flow data provided"
                }
        ...
        if _FLOW_ANALYSIS_ENABLED:
            ...
            analysis["note"] = (
                "Advanced flow analysis disabled. Set PRAISONAI_FLOW_ANALYSIS_ENABLED=true to enable expensive pattern detection."
            )
    ...
    def _identify_bottlenecks(self, flow_data: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for func_name, durations in function_durations.items():
            if durations:
                ...
                if (
                    avg_duration > BOTTLENECK_THRESHOLD_AVERAGE
                    or max_duration > BOTTLENECK_THRESHOLD_MAX
                ):
                    bottlenecks.append(
                        {
                            "function": func_name,
                            ...
                            "severity": (
                                "high"
                                if avg_duration > HIGH_SEVERITY_THRESHOLD
                                else "medium"
                            ),
                        }
                    )
    ...
    def _analyze_parallelism(self, flow_data: List[Dict]) -> Dict[str, Any]:
        ...
        peak_concurrency = (
            max(timestamp_activities.values()) if timestamp_activities else 0
        )
    ...
    def _build_call_chains(self, flow_data: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for event in flow_data:
            ...
            if event_type == "start":
                ...
                event_type == "end"
                and call_stacks[thread_id]
                and call_stacks[thread_id][-1] == func_name
            ):
    ...
    def _calculate_flow_statistics(self, flow_data: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                len(successful_events) / len(end_events) if end_events else 0
            ),
            "total_execution_time": total_duration,
            "average_execution_time": (
                total_duration / len(end_events) if end_events else 0
            ),
        }

    def visualize_flow(
        self, flow_data: Optional[List[Dict]] = None, format: str = "text"
    ) -> str:
        """
        ...
        for thread_id, events in threads.items():
            ...
            for event in events:
                ...
                if event_type == "start":
                    ...
                    lines.append(
                        f"{indent}{status} END   {func_name} ({duration:.3f}s)"
                    )
...
class PerformanceAnalyzer:
    ...
    def _analyze_function_trends(self, func_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for func_name, stats in func_stats.items():
            ...
            if len(recent_times) >= 10:  # Need sufficient data
                ...
                second_half_avg = sum(recent_times[mid_point:]) / (
                    len(recent_times) - mid_point
                )
    ...
    def _analyze_api_trends(self, api_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        slowest = sorted(
            api_performance, key=lambda x: x["average_time"], reverse=True
        )[:5]
        most_reliable = sorted(
            api_performance, key=lambda x: x["success_rate"], reverse=True
        )[:5]
    ...
    def _generate_recommendations(
        self, func_stats: Dict[str, Any], api_stats: Dict[str, Any]
    ) -> List[str]:
        ...
        if "bottlenecks" in flow_analysis:
            ...
            if bottlenecks:
                ...
                for bottleneck in bottlenecks[:5]:
                    report_lines.append(
                        f"• {bottleneck['function']}: {bottleneck['average_duration']:.2f}s avg, {bottleneck['severity']} severity"
                    )
            else:
