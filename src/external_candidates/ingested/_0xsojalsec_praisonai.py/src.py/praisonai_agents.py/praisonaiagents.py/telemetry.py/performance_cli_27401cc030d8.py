# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\praisonaiagents\telemetry\performance_cli.py
"""
...
try:
    from .performance_monitor import get_performance_report, performance_monitor
...
class PerformanceCLI:
    ...
    def analyze_flow(self) -> None:
        ...
        if stats:
            ...
            print(
                f"   Average Execution Time: {stats.get('average_execution_time', 0):.3f}s"
            )
        ...
        if least_reliable:
            ...
            for api in least_reliable[:3]:
                if api["success_rate"] < 1.0:
                    print(
                        f"  • {api['api']}: {api['success_rate']*100:.1f}% success rate"
                    )
...
Examples:
  ...
  python -m praisonaiagents.telemetry.performance_cli trends
    ...
    report_parser.add_argument(
        "--detailed", action="store_true", help="Show detailed comprehensive report"
    )
    ...
    slowest_func_parser = subparsers.add_parser(
        "slowest-functions", help="Show slowest functions"
    )
    slowest_func_parser.add_argument(
        "limit", type=int, nargs="?", default=10, help="Number of functions to show"
    )

    # Slowest APIs command
    slowest_api_parser = subparsers.add_parser(
        "slowest-apis", help="Show slowest API calls"
    )
    slowest_api_parser.add_argument(
        "limit", type=int, nargs="?", default=10, help="Number of APIs to show"
    )
    ...
    flow_parser.add_argument(
        "--events", type=int, default=50, help="Number of recent events to show"
    )
