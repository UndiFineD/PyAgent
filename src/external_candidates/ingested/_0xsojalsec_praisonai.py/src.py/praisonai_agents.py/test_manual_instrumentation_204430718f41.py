# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_manual_instrumentation.py
#!/usr/bin/env python3
"""
...
task = Task(description="Calculate 3 + 3", expected_output="The sum", agent=agent)
...
if metrics["metrics"]["agent_executions"] > 0:
    ...
    print(
        f"  workflow.execute_task wrapped: {hasattr(workflow.execute_task, '__wrapped__')}"
    )
