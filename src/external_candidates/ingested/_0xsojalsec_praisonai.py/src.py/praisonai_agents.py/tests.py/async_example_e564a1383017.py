# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\async_example.py
import asyncio
import logging
import time
...
def sync_callback(output: TaskOutput):
    display_interaction(
        "Sync Callback",
        f"Processing output: {output.raw[:100]}...",
        markdown=True,
        console=console,
    )
...
def run_sync_example():
    ...
    agents = PraisonAIAgents(
        agents=[sync_agent], tasks=[sync_task], verbose=1, process="sequential"
    )
    result = agents.start()
    display_interaction(
        "Sync Example", f"Result: {result}", markdown=True, console=console
    )
...
async def run_async_example():
    ...
    agents = PraisonAIAgents(
        agents=[async_agent], tasks=[async_task], verbose=1, process="sequential"
    )
    result = await agents.astart()
    display_interaction(
        "Async Example", f"Result: {result}", markdown=True, console=console
    )
...
async def run_mixed_example():
    ...
    result = await agents.astart()
    display_interaction(
        "Mixed Example", f"Result: {result}", markdown=True, console=console
    )
...
async def run_workflow_example():
    ...
    result = await agents.astart()
    display_interaction(
        "Workflow Example", f"Result: {result}", markdown=True, console=console
    )
