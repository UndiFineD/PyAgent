...
def test_ollama_argument_validation():
    ...
    filtered_args = llm._validate_and_filter_ollama_arguments(
        "multiply", valid_args, tools
    )
    ...
    assert (
        filtered_args == expected_filtered
    ), f"Expected {expected_filtered}, got {filtered_args}"
    ...
    filtered_args = llm._validate_and_filter_ollama_arguments(
        "multiply", invalid_args, tools
    )
    expected_empty = {}
    print(f"Original: {invalid_args}")
    print(f"Filtered: {filtered_args}")
    assert (
        filtered_args == expected_empty
    ), "All invalid arguments should be filtered out"
    ...
    filtered_args = llm._validate_and_filter_ollama_arguments(
        "nonexistent_function", some_args, tools
    )
    print(f"Original: {some_args}")
    print(f"Filtered: {filtered_args}")
    assert (
        filtered_args == some_args
    ), "Arguments should pass through if function not found"
    ...
    assert (
        filtered_args == some_args
    ), "Arguments should pass through if no tools provided"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "The Ollama sequential tool calling argument mixing issue has been fixed!"
        )
