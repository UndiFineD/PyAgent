# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\code-analysis-streamlit.py
...
def analyze_code(code_source: str) -> CodeAnalysisReport:
    ...
    agents = PraisonAIAgents(agents=[code_analyzer], tasks=[code_analysis_task])
    ...
    if analyze_button and code_source:
        try:
            with st.spinner("Analyzing code..."):
                ...
                with cols[1]:
                    display_section(
                        "⚠️ Areas for Improvement", result.improvement_areas
                    )
                ...
                color = (
                    "red" if doc_score < 60 else "orange" if doc_score < 75 else "green"
                )
