# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\mcp_wrapper.py
"""
...
class MCPWrapper:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gemini-1.5-pro"):
        ...
        if not self.api_key:
            raise ValueError(
                "API key must be provided or set as GEMINI_API_KEY environment variable"
            )
    ...
    async def connect_to_server(
        ...
        print(
            f"Connected to MCP server with tools: {[tool.name for tool in tools_response.tools]}"
        )
        ...
        try:
            ...
            if mock_response:
                ...
                if "time" in prompt.lower():
                    ...
                    mock_function_calls = [
                        MockFunctionCall("get_current_time", {"timezone": "UTC"})
                    ]
                    ...
                    mock_function_calls = [
                        MockFunctionCall("calculate", {"expression": "5+3"})
                    ]
                else:
