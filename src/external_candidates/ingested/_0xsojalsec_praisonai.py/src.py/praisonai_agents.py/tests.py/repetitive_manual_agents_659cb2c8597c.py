# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\repetitive-manual-agents.py
import pandas as pd
...
def read_csv_smart(csv_path: str) -> pd.DataFrame:
    ...
    if not any(
        col.lower() in ["task", "description", "status", "state"] for col in df.columns
    ):
...
def check_task_status(csv_path: str) -> dict:
    ...
    try:
        ...
        if not pending_tasks.empty:
            ...
            return {"has_pending": True, "next_task": task_data}
...
identify_task = Task(
    ...
    task_type="loop",
    condition={"pending": ["process_task"], "completed": "exit"},
)
