...
memory_config = {
    "config": {"collection_name": "state_memory_demo", "path": "./state_memory_db"}
}
...
def analyze_conversation_patterns() -> Dict[str, Any]:
    ...
    related_convos = shared_memory.search(
        query=topic, n=10, metadata_filter={"type": "conversation_turn"}
    )
    ...
    patterns = {
        "current_conversation": {
            ...
            - workflow.get_state("start_time", time.time()),
        },
        "memory_insights": {
            "related_conversations": len(related_convos),
            "common_speakers": list(
                set(r["metadata"].get("speaker") for r in related_convos)
            ),
            "topics_discussed": list(
                set(r["metadata"].get("topic") for r in related_convos)
            ),
        },
    }
...
def retrieve_user_history(user_name: str) -> Dict[str, Any]:
    ...
    user_history = shared_memory.search(
        query=user_name, n=20, metadata_filter={"user": user_name}
    )
...
def summarize_and_save_state() -> Dict[str, Any]:
    ...
    summary = {
        ...
        "key_points": [
            turn["message"][:50] + "..." for turn in history[-3:]
        ],  # Last 3 messages
        "analysis": analysis,
    }
...
print(
    f"   User history retrieved: {workflow.get_state('user_history_retrieved', False)}"
)
...
test_workflow = PraisonAIAgents(
    agents=[test_agent], tasks=[test_task], memory=memory_config, verbose=0
)
