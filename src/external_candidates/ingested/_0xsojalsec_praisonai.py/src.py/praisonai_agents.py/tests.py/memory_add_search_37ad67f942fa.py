# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\memory-add-search.py
import json
import logging
import os
import time
...
class MemoryManager:
    def __init__(self):
        ...
        config = {
            ...
            "llm": {
                "provider": "openai",
                "config": {"model": "gpt-5-nano", "temperature": 0, "max_tokens": 1000},
            },
        }
    ...
    def add_memory(
        self, text: str, user_id: Optional[str] = None, metadata: Optional[Dict] = None
    ) -> Dict:
        ...
        try:
            ...
            result = self.memory.add(
                messages=messages, user_id=user_id, metadata=clean_metadata
            )
...
def main():
    try:
        ...
        search_results = manager.search_memories(
            query="What are Alice's hobbies?", user_id="alice", limit=5
        )
