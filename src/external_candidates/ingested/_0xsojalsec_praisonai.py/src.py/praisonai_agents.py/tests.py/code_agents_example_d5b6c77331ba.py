# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\code_agents_example.py
import json
...
def code_interpreter(code: str):
    ...
    if exec_result.error:
        ...
        logs = {
            "stdout": list(exec_result.logs.stdout),
            "stderr": list(exec_result.logs.stderr),
        }
...
agents = PraisonAIAgents(
    agents=[code_agent, execution_agent], tasks=[code_agent_task, execution_agent_task]
)
agents.start()
