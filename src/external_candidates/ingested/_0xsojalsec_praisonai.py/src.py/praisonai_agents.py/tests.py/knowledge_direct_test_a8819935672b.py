# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\knowledge-direct-test.py
import logging
import os
...
cricket_memory = knowledge.store(
    "Likes to play cricket on weekends",
    user_id="alice",
    metadata={"category": "hobbies"},
)
logger.info(f"Stored cricket memory: {cricket_memory}")
tennis_memory = knowledge.store(
    "Enjoys playing tennis with friends",
    user_id="alice",
    metadata={"category": "hobbies"},
)
logger.info(f"Stored tennis memory: {tennis_memory}")
work_memory = knowledge.store(
    "Works as a software engineer", user_id="alice", metadata={"category": "work"}
)
...
updated_memory = knowledge.update(
    memory_id=tennis_memory["results"][0]["id"], data="Loves playing tennis on weekends"
)
