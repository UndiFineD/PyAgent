...
def test_memory_graph_config():
    ...
    try:
        ...
        print(
            f"✅ Memory initialized. Graph enabled: {getattr(memory, 'graph_enabled', False)}"
        )

        # Test basic memory operations
        memory.store_short_term("Alice loves hiking", {"category": "preference"})
        memory.store_short_term(
            "Alice's friend John also loves hiking", {"category": "social"}
        )
...
if __name__ == "__main__":
    ...
    if passed == len(tests):
        ...
        print(
            "⚠️  Some tests failed. This is expected if graph dependencies are not installed."
        )
