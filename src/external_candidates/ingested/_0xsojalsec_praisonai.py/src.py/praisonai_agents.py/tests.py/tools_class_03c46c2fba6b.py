# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\tests\tools-class.py
import os
...
class EXASearchTool(BaseModel):
    ...
    def results(
        ...
        if "results" in results:
            return (
                results["results"][:max_results] if max_results else results["results"]
            )
        return []
