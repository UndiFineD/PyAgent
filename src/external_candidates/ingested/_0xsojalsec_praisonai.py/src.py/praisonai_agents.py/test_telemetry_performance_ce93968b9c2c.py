# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_telemetry_performance.py
#!/usr/bin/env python3
"""
...
def test_optimized_telemetry():
    ...
    try:
        ...
        if normal_mode_time > 0:
            improvement = (
                (normal_mode_time - performance_mode_time) / normal_mode_time
            ) * 100
        ...
        from praisonaiagents.telemetry.integration import (
            _get_telemetry_queue,
        ...
        return {
            ...
            "queue_events_per_second": (
                1000 / queue_time if queue_time > 0 else float("inf")
            ),
        }
...
def main():
    ...
    if results:
        ...
        print(
            f"✅ Queue throughput: {results['queue_events_per_second']:.0f} events/sec"
        )
