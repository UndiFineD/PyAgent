# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\src\praisonai-agents\test_guardrail_return_types.py
#!/usr/bin/env python3
"""
...
from praisonaiagents import Agent, GuardrailResult, Task, TaskOutput
...
def test_guardrail_return_types():
    ...
    def validate_with_guardrailresult(task_output: TaskOutput) -> GuardrailResult:
        ...
        if len(task_output.raw) < 10:
            return GuardrailResult(
                success=False, result=None, error="Output is too short"
            )
        return GuardrailResult(success=True, result=task_output, error="")
    ...
    for test_name, guardrail_func in tests:
        ...
        try:
            ...
            ), f"Expected GuardrailResult, got {type(result)}"
...
def test_guardrail_result_handling():
    ...
    test_output = TaskOutput(description="Test", raw="Test output", agent="Test Agent")
    ...
    task1 = Task(description="Test", agent=agent, guardrail=return_guardrailresult)
    ...
    task2 = Task(description="Test", agent=agent, guardrail=return_tuple)
