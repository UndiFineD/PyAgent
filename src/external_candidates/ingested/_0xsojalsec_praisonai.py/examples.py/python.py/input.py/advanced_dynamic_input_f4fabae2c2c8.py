# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\input\advanced_dynamic_input.py
"""
...
class DynamicAgentSystem:
    ...
    def collect_user_inputs(self) -> Dict[str, str]:
        ...
        inputs["output_format"] = (
            input("Output format (summary/report/bullets)? ").lower() or "summary"
        )
    ...
    def create_dynamic_tasks(
        self, agents: List[Agent], inputs: Dict[str, str]
    ) -> List[Task]:
    ...
    def run(self):
        ...
        if inputs["depth"] == "detailed":
            filename = (
                f"{inputs['topic'].replace(' ', '_')}_{inputs['output_format']}.txt"
            )
