# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\input\streamlit_ui_input.py
"""
...
st.set_page_config(page_title="PraisonAI Dynamic Input", page_icon="🤖", layout="wide")
...
with st.sidebar:
    ...
    # Process type
    process_type = st.radio("Process Type", ["sequential", "hierarchical"], index=0)
...
with col1:
    ...
    with st.expander("Advanced Options"):
        max_search_results = st.number_input(
            "Max Search Results", min_value=1, max_value=20, value=5
        )

        output_format = st.selectbox(
            "Output Format", ["Summary", "Detailed Report", "Bullet Points"], index=0
        )
        ...
        with progress_container:
            with st.spinner("🔄 Processing your request..."):
                try:
                    ...
                    task_description = (
                        f"Provide a {output_format.lower()} answer for: {user_query}"
                    )
                    if use_tools:
                        task_description += (
                    ...
                    agents = PraisonAIAgents(
                        agents=[agent], tasks=[task], process=process_type
                    )
                    ...
                    tab1, tab2, tab3 = st.tabs(
                        ["Result", "Configuration", "Raw Output"]
                    )
