# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\general\workflow_example_detailed.py
import json
import random
...
def process_data_of_individuals(data: Union[List[Dict], Dict, str]) -> Dict:
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            raise ValueError(
                "Input must be a dictionary or list with at least one record"
            )
        ...
        print(
            f"[DEBUG] Status check - remaining: {remaining}, current_index: {current_index}, total_records: {total_records}"
        )
        ...
        if remaining <= 0 and current_index >= total_records - 1:
            ...
            processed_person["processing_status"] = (
                f"more records to process ({remaining} remaining)"
            )
...
validate_task = Task(
    ...
    name="validate_data",
    task_type="decision",
    condition={"valid": ["process_data"], "invalid": ["collect_data"]},
...
for task_id, task in workflow.tasks.items():
    ...
    if task.result:
        ...
        try:
            ...
            print(
                task.result.raw[:500] + "..."
                if len(task.result.raw) > 500
                else task.result.raw
            )
