# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\general\advanced-callback-systems.py
"""
...
def interaction_callback(data):
    ...
    print(
        f"📋 Interaction: {data.get('agent_name', 'Unknown')} - {data.get('type', 'unknown')}"
    )
...
def tool_callback(data):
    """Monitor tool usage"""
    print(
        f"🔧 Tool Used: {data.get('agent_name', 'Unknown')} used {data.get('tool_name', 'unknown')}"
    )
...
print(
    "Callbacks successfully tracked agent interactions, tool usage, and content generation."
)
