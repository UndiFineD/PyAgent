# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\usecases\domain-context-solution.py
"""
...
from praisonaiagents import Agent, Memory, PraisonAIAgents, Task
from qdrant_client import QdrantClient

# Load environment variables from .env.local
if not load_dotenv(dotenv_path=".env.local"):
    raise FileNotFoundError(
        ".env.local file not found. Please ensure it exists in the working directory."
    )
...
for key, value in api_keys.items():
    if not value:
        raise ValueError(
            f"Missing API key: {key}. Ensure it is set in the environment variables or .env.local."
        )
...
class DomainContext:
    ...
    def get_context(self):
        return {
            ...
            "constraints": [
                f"Only analyze {self.target_domain}",
                "No generic examples",
                "Domain-specific results only",
            ],
        }
...
def create_domain_aware_tools(target_domain: str):
    ...
    def query_fofa(query: str = None) -> dict:
        ...
        enhanced_query = (
            f"{target_domain} {query}" if target_domain not in query else query
        )
    ...
    def query_networkcalc(domain_param: str = None) -> dict:
        ...
        response = requests.get(
            f"https://networkcalc.com/api/dns/lookup/{domain_param}"
        )
        return response.json()
...
pentester_analysis_task = Task(
    ...
    context=[
        ...
        tool_query_task,
    ],  # Include domain context and previous task
)
...
agents = PraisonAIAgents(
    ...
    memory_config={
        "provider": "rag",
        "config": {"collection_name": f"{domain.replace('.', '_')}_security_analysis"},
    },
)
...
def main():
    ...
    print(
        f"Context properly maintained: {'YES' if all('target_domain' in agent.memory['short_term'] for agent in agents.agents) else 'NO'}"
    )
