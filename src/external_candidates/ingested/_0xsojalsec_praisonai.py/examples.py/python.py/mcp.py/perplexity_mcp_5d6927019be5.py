# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\mcp\perplexity-mcp.py
import os

from praisonaiagents import MCP, Agent

api_key = os.getenv("PERPLEXITY_API_KEY")

agent = Agent(
    instructions="You are a helpful assistant that can search the web for information. Use the available tools when relevant to answer user questions.",
    llm="gpt-5-nano",
    tools=MCP(
        "uvx perplexity-mcp",
        env={"PERPLEXITY_API_KEY": api_key, "PERPLEXITY_MODEL": "sonar"},
    ),
)
result = agent.start(
    "What is the latest news on AI?, Pass only the query parameter to the tool"
)

print(result)
