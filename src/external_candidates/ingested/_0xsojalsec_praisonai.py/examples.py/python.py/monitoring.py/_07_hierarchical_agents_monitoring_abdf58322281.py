# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\monitoring\07_hierarchical_agents_monitoring.py
"""
...
def main():
    ...
    management_functions = [
        f
        for f in stats.keys()
        if "manager" in f or "decision" in f or "delegation" in f or "aggregation" in f
    ]
    worker_functions = [f for f in stats.keys() if "worker" in f or "execution" in f]
    coordination_functions = [
        f for f in stats.keys() if "hierarchical" in f or "workflow" in f
    ]
    ...
    total_coordination_time = sum(
        stats[f]["total_time"] for f in coordination_functions
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Tasks: {result['tasks_completed']}, Efficiency: {result['efficiency_ratio']*100:.1f}%"
    )
