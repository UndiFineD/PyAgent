# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\monitoring\06_memory_agent_monitoring.py
"""
...
def run_stateful_agent(query: str, session_id: str, conversation_turn: int):
    ...
    with track_api_call("memory_postprocessing"):
        ...
        store_memory(
            f"session_{session_id}", f"Turn {conversation_turn} completed", "short_term"
        )
...
def main():
    ...
    print(
        f"  Avg Time per Memory Op: {total_memory_time/total_memory_ops:.3f}s"
        if total_memory_ops > 0
        else "  No memory operations"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Executed {result['memory_operations']} memory operations in {result['total_memory_time']:.3f}s"
    )
