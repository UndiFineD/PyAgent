# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\monitoring\10_comprehensive_dashboard.py
"""
...
def collect_comprehensive_data():
    ...
    test_scenarios = [
        ...
        (
            "Complex analysis",
            "Analyze the impact of artificial intelligence on modern business practices",
        ),
    ...
    dashboard_data = {
        ...
        "summary": {
            ...
            "total_api_requests": sum(
                stats["call_count"] for stats in api_stats.values()
            ),
            "total_execution_time": sum(
                stats["total_time"] for stats in function_stats.values()
            ),
            "total_errors": sum(
                stats["error_count"] for stats in function_stats.values()
            ),
        },
        "performance_data": {
        ...
        "analysis": {
            ...
            "flow_visualization": (
                len(str(flow_visualization)) if flow_visualization else 0
            ),
        },
        "reports": {
            "basic_report": basic_report,
            "comprehensive_report": (
                comprehensive_report[:500] + "..."
                if len(comprehensive_report) > 500
...
def display_dashboard(dashboard_data, benchmark_results):
    ...
    if summary["total_function_calls"] > 0:
        avg_function_time = (
            summary["total_execution_time"] / summary["total_function_calls"]
        )
    ...
    if "error" not in analysis["flow_analysis"]:
        ...
        print(
            f"  Flow Analysis: {len(flow_data.get('functions', []))} functions analyzed"
        )
    else:
    ...
    if analysis["flow_visualization"]:
        print(
            f"  Flow Visualization: Generated ({analysis['flow_visualization']} chars)"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Total Functions Monitored: {result['dashboard_data']['summary']['total_functions']}"
    )
