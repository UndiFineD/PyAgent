# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\monitoring\09_streaming_monitoring.py
"""
...
def collect_live_metrics():
    ...
    while streaming_active:
        ...
        if streaming_stats["start_time"]:
            ...
            messages_per_second = (
                streaming_stats["messages_processed"] / elapsed_time
                if elapsed_time > 0
                else 0
            )
            avg_latency = (
                streaming_stats["total_latency"] / streaming_stats["messages_processed"]
                if streaming_stats["messages_processed"] > 0
                else 0
            )

            print(
                f"📊 Live Metrics - Messages/sec: {messages_per_second:.2f}, Avg Latency: {avg_latency:.3f}s"
            )
...
def compare_batch_vs_streaming():
    ...
    print(
        f"    Avg per Message: {streaming_time/streaming_count:.3f}s"
...
def main():
    ...
    print("\n🌊 Streaming Metrics:")
    elapsed_total = (
        time.time() - streaming_stats["start_time"]
        if streaming_stats["start_time"]
        else 0
    )
    throughput = (
        streaming_stats["messages_processed"] / elapsed_total
        if elapsed_total > 0
        else 0
    )
    avg_latency = (
    ...
    error_rate = (
        streaming_stats["errors"] / streaming_stats["messages_processed"] * 100
        if streaming_stats["messages_processed"] > 0
        else 0
    )
    ...
    streaming_functions = {
        ...
        if "stream" in k.lower() or "real_time" in k.lower()
    }

    for func_name, data in streaming_functions.items():
        avg_time = (
            data["total_time"] / data["call_count"] if data["call_count"] > 0 else 0
        )
        print(f"  {func_name}:")
    ...
    print(
        f"  Average Calls per Function: {total_calls/total_functions:.1f}"
        if total_functions > 0
        else "  No functions monitored"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Throughput: {result['throughput']:.2f} msg/s, Latency: {result['average_latency']:.3f}s"
    )
