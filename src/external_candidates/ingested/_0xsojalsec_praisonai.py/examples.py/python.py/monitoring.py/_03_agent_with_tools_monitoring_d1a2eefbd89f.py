# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\monitoring\03_agent_with_tools_monitoring.py
"""
...
def main():
    ...
    for i, query in enumerate(queries, 1):
        ...
        with track_api_call(f"agent_query_{i}"):
            ...
            print(
                f"📝 Agent Response: {result[:200]}..."
                if len(result) > 200
                else f"📝 Agent Response: {result}"
            )
    ...
    for api_name, stats in api_stats.items():
        ...
        print(
            f"    Success Rate: {stats['success_count']/stats['call_count']*100:.1f}%"
        )
    ...
    print(
        f"  Average Time per Call: {total_execution_time/total_function_calls:.3f}s"
        if total_function_calls > 0
        else "  No calls recorded"
    )
