# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\processes\advanced-workflow-patterns.py
"""
...
for request in sample_requests:
    ...
    if request["type"] == "market_research":
        ...
        specialist_task_desc = (
            f"Conduct comprehensive market research on: {request['description']}"
        )
    elif request["type"] == "technical_solution":
        specialist_agent = technical_agent
        specialist_task_desc = (
            f"Design technical solution for: {request['description']}"
        )
    ...
    workflow_agents = PraisonAIAgents(
        agents=[intake_agent, specialist_agent, review_agent, coordinator_agent],
        tasks=[
            intake_task,
            specialist_task,
            review_task,
    ...
    print(
        f"✅ Request {request['id']} completed with quality score: {quality_score:.2f}"
    )
