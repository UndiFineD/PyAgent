# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\api\secondary-market-research-api.py
"""
...
class MarketResearchRequest(BaseModel):
    ...
    geography: str = Field(
        ..., description="Geographic region", example="North America"
    )
    industry: str = Field(
        ..., description="Industry sector", example="Electric Vehicles"
    )
    ...
    format: Optional[str] = Field(
        default="json", description="Output format: json or pdf", example="json"
    )
...
class MarketResearchConfig:
    ...
    def __init__(
        self, company: str, geography: str, industry: str, sections: List[str]
    ):
...
async def generate_market_research_report(job_id: str, config: MarketResearchConfig):
    ...
    try:
        ...
        if isinstance(results, str):
            ...
            if "task_results" in results:
                ...
                for task_name, result in results["task_results"].items():
                    print(
                        f"DEBUG: Processing task '{task_name}', result type: {type(result)}"
                    )
                    ...
                    if result:
                        ...
                        if content:
                            ...
                            print(
                                f"DEBUG: Added task '{task_name}' with content length: {len(str(content))}"
                            )
                        else:
        ...
        print(
            f"DEBUG: Final research_findings keys: {list(report_data['research_findings'].keys())}"
        )
        print(
            f"DEBUG: Final research_findings count: {len(report_data['research_findings'])}"
        )
        ...
        if "synthesis_report" in report_data["research_findings"]:
            ...
            report_data["executive_summary"] = (
                ". ".join(sentences) + "."
                if sentences
                else "Market research analysis completed."
            )
...
async def generate_research(
    request: MarketResearchRequest, background_tasks: BackgroundTasks
):
    """
...
async def download_report(job_id: str):
    ...
    if job_status[job_id]["status"] != "completed":
        raise HTTPException(
            status_code=400, detail="Report not ready. Check job status first."
        )
        ...
        "jobs": [
            {
                ...
                "company": (
                    status["config"]["company"] if "config" in status else "Unknown"
                ),
            }
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "secondary-market-research-api:app", host="0.0.0.0", port=8000, reload=True
    )
