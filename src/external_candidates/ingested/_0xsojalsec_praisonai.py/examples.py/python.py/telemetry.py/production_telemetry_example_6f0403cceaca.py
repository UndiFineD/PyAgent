# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\telemetry\production-telemetry-example.py
"""
...
telemetry = enable_telemetry(
    ...
    config={
        ...
        "attributes": {"deployment": "cloud", "region": "us-east-1", "cluster": "main"},
    },
)
...
for i, request in enumerate(customer_requests):
    ...
    print(
        f"Complexity: {request['complexity']} | Expected Duration: {request['expected_duration']}s"
    )
    print(f"{'='*60}")
        ...
        try:
                ...
                agent_trace.set_attributes(
                    {
                        ...
                        "agent.tools_used": (
                            len(customer_service_agent.tools)
                            if customer_service_agent.tools
                            else 0
                        ),
                    }
                )
            ...
            with telemetry.trace_agent_execution("QualityAssuranceAgent") as qa_trace:
                ...
                qa_agents = PraisonAIAgents(
                    agents=[quality_assurance_agent], tasks=[qa_task], verbose=False
                )
                ...
                qa_trace.set_attributes(
                    {"qa.score": quality_score, "qa.passed": quality_score >= 0.85}
                )
            ...
            telemetry.record_metric(
                "request_duration",
                duration,
                {"request_type": request["type"], "complexity": request["complexity"]},
            )

            telemetry.record_metric(
                "request_success", 1, {"request_type": request["type"]}
            )
            ...
            print(
                f"✅ Request completed successfully in {duration:.2f}s (Quality: {quality_score:.2f})"
            )
            ...
            telemetry.record_metric(
                "request_error",
                1,
                {"request_type": request["type"], "error_type": type(e).__name__},
            )

            trace.set_attributes({"request.status": "error", "request.error": str(e)})
