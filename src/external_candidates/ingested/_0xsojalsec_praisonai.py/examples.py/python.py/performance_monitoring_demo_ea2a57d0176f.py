# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\performance_monitoring_demo.py
"""
...
if "bottlenecks" in flow_analysis:
    ...
    if bottlenecks:
        ...
        for bottleneck in bottlenecks:
            print(
                f"  • {bottleneck['function']}: {bottleneck['average_duration']:.3f}s average"
            )
    else:
...
for i, api in enumerate(slowest_apis, 1):
    speed_indicator = (
        "🐌"
        if api["average_time"] > 0.3
        else "⚡" if api["average_time"] < 0.1 else "🚀"
    )
    print(
        f"{i}. {speed_indicator} {api['api']}: {api['average_time']:.3f}s avg (Success: {api['success_rate']*100:.1f}%)"
    )
...
def execute_monitored_task(agent, task_description):
    ...
    agents_workflow = PraisonAIAgents(agents=[agent], tasks=[task], verbose=False)
...
result = execute_monitored_task(
    research_agent, "Research the latest trends in AI performance optimization"
)
...
print(
    "python -m praisonaiagents.telemetry.performance_cli export --format json --output performance_data.json"
)
