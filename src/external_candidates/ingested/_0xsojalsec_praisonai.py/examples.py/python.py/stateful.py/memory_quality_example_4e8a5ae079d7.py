# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\stateful\memory-quality-example.py
#!/usr/bin/env python3
"""
...
def main():
    ...
    high_quality = memory.search_long_term(query=query, min_quality=0.8, limit=10)
    ...
    medium_quality = memory.search_long_term(query=query, min_quality=0.6, limit=10)
    ...
    all_results = memory.search_long_term(query=query, min_quality=0.0, limit=10)
    ...
    response1 = agent.chat(
        "What do you know about AI safety? Please prioritize high-quality information."
    )
    ...
    conversation_history = memory.search_short_term(
        query="ai safety interaction", min_quality=0.5, limit=5
    )
