# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\token-metrics\05_advanced_agent_integration.py
"""
...
class TokenBudgetManager:
    ...
    def check_budget(self, agents):
        ...
        if remaining <= 0:
            return (
                False,
                f"Budget exceeded! Used: {current_tokens:,}, Budget: {self.max_tokens:,}",
            )

        return (
            True,
            f"Budget OK. Used: {current_tokens:,}/{self.max_tokens:,} ({remaining:,} remaining)",
        )
...
def compare_model_efficiency(agents):
    ...
    for model, metrics in by_model.items():
        ...
        if input_tokens > 0:
            ...
            print(
                f"  Efficiency Rating: {'⭐⭐⭐' if efficiency < 1.5 else '⭐⭐' if efficiency < 2.5 else '⭐'}"
            )
...
def analyze_task_performance(agents):
    ...
    for agent, perf in sorted(
        agent_performance.items(), key=lambda x: x[1]["total_tokens"], reverse=True
    ):
        print(f"\n👤 {agent}:")
