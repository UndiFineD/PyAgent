# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\tasks\advanced-task-management.py
"""
...
def data_quality_validator(task_output: TaskOutput) -> Tuple[bool, Any]:
    ...
    found_indicators = sum(
        1 for indicator in quality_indicators if indicator in output_text
    )
    ...
    if found_indicators >= 4:  # Require at least 4 quality indicators
        ...
        return (
            False,
            f"Data quality insufficient. Found {found_indicators} quality indicators but need at least 4. Please include more research sources, data analysis, and evidence.",
        )
...
def completeness_validator(task_output: TaskOutput) -> Tuple[bool, Any]:
    ...
    has_introduction = "introduction" in output_text.lower() or output_text.startswith(
        ("In ", "The ", "This ")
    )
    has_conclusion = any(
    ...
    if has_introduction and has_conclusion and sufficient_length:
        ...
        return (
            False,
            f"Analysis incomplete. Missing: {', '.join(missing)}. Please provide a more comprehensive response.",
        )
...
data_analysis = Task(
    ...
    condition={
        ...
        "needs_more_research": [
            "additional_research"
        ],  # If needs more data, do additional research
