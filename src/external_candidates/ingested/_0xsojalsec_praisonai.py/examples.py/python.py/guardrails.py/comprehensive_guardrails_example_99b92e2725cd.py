# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\guardrails\comprehensive-guardrails-example.py
"""
...
def email_format_guardrail(task_output: TaskOutput) -> Tuple[bool, Any]:
    ...
    if emails_found:
        ...
        return (
            False,
            "No valid email addresses found in the output. Please include at least one properly formatted email address.",
        )
...
def word_count_guardrail(task_output: TaskOutput) -> Tuple[bool, Any]:
    ...
    if word_count >= min_words:
        ...
        return (
            False,
            f"Output too short. Found {word_count} words but need at least {min_words} words. Please provide a more detailed response.",
        )
...
def professional_tone_guardrail(task_output: TaskOutput) -> Tuple[bool, Any]:
    ...
    for term in unprofessional_terms:
        if term in output_text:
            return (
                False,
                f"Output contains unprofessional language ('{term}'). Please revise to maintain a professional tone.",
            )
...
def factual_accuracy_guardrail(task_output: TaskOutput) -> Tuple[bool, Any]:
    ...
    has_factual_backing = any(
        indicator in output_text for indicator in factual_indicators
    )
    ...
    if has_factual_backing:
        ...
        return (
            False,
            "Output lacks factual backing. Please include references to research, studies, or data to support your claims.",
        )
