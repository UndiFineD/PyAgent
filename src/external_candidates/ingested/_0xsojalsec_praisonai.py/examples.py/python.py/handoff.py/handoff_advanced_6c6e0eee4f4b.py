# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\handoff\handoff_advanced.py
"""
...
def log_escalation(source_agent: Agent, input_data: EscalationData):
    ...
    logging.info(
        f"ESCALATION from {source_agent.name}: {input_data.reason} (Priority: {input_data.priority})"
    )
...
support_agent.instructions = prompt_with_handoff_instructions(
    support_agent.instructions, support_agent
)
