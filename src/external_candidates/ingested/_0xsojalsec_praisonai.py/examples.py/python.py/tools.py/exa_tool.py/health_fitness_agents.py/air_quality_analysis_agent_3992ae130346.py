# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\tools\exa-tool\Health_Fitness_Agents\Air_Quality_Analysis_Agent.py
#!/usr/bin/env python
# coding: utf-8
...
if __name__ == "__main__":
    ...
    aqi_data = aqi_fetcher.fetch(country=user_country, state=user_state, city=user_city)

    if not aqi_data or not hasattr(aqi_data, "data"):
        print("Mumbai with state failed, trying without state...")
        aqi_data = aqi_fetcher.fetch(country=user_country, state="", city=user_city)

    if not aqi_data or not hasattr(aqi_data, "data"):
        print("Trying Delhi, India...")
        aqi_data = aqi_fetcher.fetch(country="India", state="", city="Delhi")

    if not aqi_data or not hasattr(aqi_data, "data"):
        print(
            "Failed to fetch AQI data for all tried locations. Please check the URLs manually."
        )
    else:
