# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\tools\exa-tool\Finance_Market_Job_Agents\Financial_Coaching_Agent.py
#!/usr/bin/env python
# coding: utf-8
...
if __name__ == "__main__":
    ...
    if not is_valid_key(OPENAI_API_KEY, "sk"):
        print(
            "API key not set or is a placeholder. Using dummy podcast script for CI/testing."
        )
        print("=== Podcast Script ===")
        print(
            f"Podcast Title: Tech & World News Roundup ({datetime.now().strftime('%B %d, %Y')})"
        )
