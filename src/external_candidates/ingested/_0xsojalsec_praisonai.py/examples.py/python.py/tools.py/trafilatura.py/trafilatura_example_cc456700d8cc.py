# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\tools\trafilatura\trafilatura_example.py
#!/usr/bin/env python3
"""
...
async def agent_example():
    ...
    # Run the analysis
    agents = PraisonAIAgents(agents=[research_agent], tasks=[analysis_task])
...
async def format_example():
    ...
    json_content = await asyncio.to_thread(
        tools_instance.extract_content, url, output_format="json"
    )
    print("JSON format - type:", type(json_content))

    # Text format
    text_content = await asyncio.to_thread(
        tools_instance.extract_content, url, output_format="text"
    )
    ...
    xml_content = await asyncio.to_thread(
        tools_instance.extract_content, url, output_format="xml"
    )
