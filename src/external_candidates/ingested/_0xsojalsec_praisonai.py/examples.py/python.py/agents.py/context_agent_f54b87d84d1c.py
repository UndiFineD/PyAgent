# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\python\agents\context-agent.py
#!/usr/bin/env python3
"""
...
def demonstrate_basic_context_generation():
    ...
    context_agent = create_context_agent(
        llm="gpt-5-nano", name="Basic Context Engineer", verbose=True
    )
    ...
    analysis = context_agent.analyze_codebase_patterns(
        project_path=project_path, file_patterns=["*.py"]
    )

    print(f"✅ Analyzed project at: {project_path}")
    print(
        f"   Project structure: {len(analysis.get('project_structure', {}).get('directories', []))} directories"
    )
    print(f"   Code patterns identified: {len(analysis.get('code_patterns', {}))}")
    print(
        f"   Architecture: {analysis.get('architecture_insights', {}).get('primary_pattern', 'Unknown')}"
    )
    ...
    context_doc = context_agent.generate_context_document(
        project_path=project_path, requirements=feature_request, analysis=analysis
    )

    print(f"✅ Generated context document for: {feature_request}")
    print(f"   Document length: {len(context_doc)} characters")
    print(
        f"   Contains architecture patterns: {'Architecture Patterns' in context_doc}"
    )
    ...
    validation_config = context_agent.create_validation_loop(
        implementation_requirements=feature_request, success_criteria=success_criteria
    )

    print(
        f"✅ Created validation loop with {len(validation_config['validation_steps'])} steps"
    )
    ...
    enhanced_prompt = context_agent.enhance_prompt_with_context(
        base_prompt=basic_prompt, context_data=analysis
    )

    print(
        f"✅ Enhanced prompt from {len(basic_prompt)} to {len(enhanced_prompt)} characters"
    )
    print(f"   Original: '{basic_prompt}'")
    print(
        "   Enhanced includes: Context Engineering, Implementation Context, Quality Requirements"
    )
...
def demonstrate_context_agent_as_tool():
    ...
    print(
        f"✅ Created implementation blueprint with {len(blueprint['implementation_steps'])} steps"
    )

    # Step 3: Generate comprehensive PRP
    print("\n3️⃣ Step 3: Generate PRP")
    prp = context_agent.generate_prp(
        feature_request, analysis
    )  # Generated PRP for implementation guidance
    ...
    validation = context_agent.create_validation_loop(
        feature_request, criteria
    )  # Validation framework for QA
