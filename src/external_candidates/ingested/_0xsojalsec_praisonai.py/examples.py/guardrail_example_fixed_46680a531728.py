# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-PraisonAI\examples\guardrail_example_fixed.py
#!/usr/bin/env python3
"""
...
import trafilatura
from praisonaiagents import Agent, GuardrailResult, PraisonAIAgents, Task, TaskOutput
...
def validate_length_guardrailresult(output: TaskOutput) -> GuardrailResult:
    ...
    if 100 <= length <= 500:
        return GuardrailResult(
            success=True, result=output, error=""  # Pass through the original output
        )
    else:
        return GuardrailResult(
            success=False,
...
agents_gr = PraisonAIAgents(agents=[agent], tasks=[task_with_guardrailresult])
...
agents_tuple = PraisonAIAgents(agents=[agent], tasks=[task_with_tuple])
...
agents_llm = PraisonAIAgents(agents=[agent], tasks=[task_with_llm_guardrail])
