# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-make-circuit-boards\src\atopile\front_end.py
"""
...
class Dizzy(HandleStmtsFunctional, HandlesPrimaries):
    ...
    def build_layer(self, cls_def: ClassDef) -> ClassLayer:
        ...
        obj = ClassLayer(
            ...
            assignments={
                ref[0]: v
                for ref, v in strainer.strain(lambda x: isinstance(x.value, Assignment))
            },
            assertions=[
                v for _, v in strainer.strain(lambda x: isinstance(x.value, Assertion))
            ],
        )
    ...
    def visitAssert_stmt(self, ctx: ap.Assert_stmtContext) -> KeyOptMap:
        ...
        def _add_expr_from_context(ctx: ap.Arithmetic_expressionContext):
            expr = Expression.from_numericish(roley.visit(ctx))
