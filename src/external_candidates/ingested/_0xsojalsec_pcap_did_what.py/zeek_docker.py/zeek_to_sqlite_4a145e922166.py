# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pcap-did-what\zeek-docker\zeek-to-sqlite.py
#!/usr/bin/python3
import csv
import os
import sqlite3
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".log"):
            table_name = os.path.splitext(filename)[0].replace(
                "-", "_"
            )  # Ensure table names are valid SQLite identifiers
            ...
            with open(log_path, "r") as file:
                ...
                for row in reader:
                    if row[0].startswith("#fields"):
                        columns = [
                            col.replace(".", "_") for col in row[1:]
                        ]  # Adjusted to handle '#fields'
                ...
                data = [
                    row for row in reader if not row[0].startswith("#")
                ]  # Skip comment lines
