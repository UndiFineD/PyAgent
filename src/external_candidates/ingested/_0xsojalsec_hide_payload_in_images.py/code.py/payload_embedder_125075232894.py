# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-hide-payload-in-images\code\payload-embedder.py
#!/usr/bin/env python3
"""
payload-embedder.py
...
def embed_payload_into_file(target_file_path, payload_file_path, output_file_path):
    ...
    if os.path.exists(output_file_path):
        response = input(
            f"Warning: Output file '{output_file_path}' already exists. Overwrite? (y/n): "
        )
        if response.lower() != "y":
    ...
    try:
        ...
        with open(payload_file_path, "rb") as payload_file:
            ...
            print(
                f"Payload File: Read {payload_size} bytes from '{payload_file_path}'."
            )
        ...
        with open(output_file_path, "wb") as output_file:
            ...
            print(
                f"Embedded File: Wrote {combined_size} bytes to '{output_file_path}'."
            )
