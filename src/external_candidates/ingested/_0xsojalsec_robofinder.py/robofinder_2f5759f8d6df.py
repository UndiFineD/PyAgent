...
def setup_argparse():
    ...
    parser.add_argument(
        "--url", "-u", dest="url", type=str, help="Give me the URL", required=True
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "-c", action="store_true", default=False, help="Concatenate paths with site url"
    )
...
def fetchFiles(url: str):
    ...
    while retry_count < max_retries:
        try:
            ...
            logger(
                args.debug, "ChunkedEncodingError occurred. Retrying in 5 seconds..."
            )
            time.sleep(5)
            logger(args.debug, "Sending request again to {}".format(url))
            retry_count += 1
...
def startProccess(urls, args) -> list:
    ...
    responses = []
    logger(
        args.debug, "Sending a bunch of HTTP requests to fetch all robots.txt files."
    )
    try:
