...
class ArgumentsParser:
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            "--url", type=self.check_arg_url, dest="url", help="Target URL"
        )
        self.parser.add_argument(
            "--target", type=self.check_arg_type, dest="type", help="Target type"
        )
        self.parser.add_argument(
            "-u", "--username", type=str, dest="username", help="Single username"
        )
        self.parser.add_argument(
            "-U",
            "--userlist",
        ...
        self.parser.add_argument(
            "-p", "--password", type=str, dest="password", help="Single password"
        )
        self.parser.add_argument(
            "-P",
            "--passlist",
        ...
        self.parser.add_argument(
            ...
            dest="max_errors",
            default=MAX_ERRORS,
            help="Number of accepted consecutive errors (default: {})".format(
                MAX_ERRORS
            ),
        )
        self.parser.add_argument(
    ...
    def check_arg_threads(self, threads):
        ...
        if threads < 1 or threads > MAX_THREADS:
            raise argparse.ArgumentTypeError(
                "Number of threads must be <= {}".format(MAX_THREADS)
            )
        return threads
        ...
        if self.args.list is False:
            ...
            if self.args.combolist is not None:
                if (
                    ...
                    self.parser.error(
                        "When combolist is used, -u, -U, -p and -P " "are not supported"
                    )
            ...
            if (
                ...
                self.parser.error(
                    "Username/userlist + password/passlist or " "combolist required"
                )
