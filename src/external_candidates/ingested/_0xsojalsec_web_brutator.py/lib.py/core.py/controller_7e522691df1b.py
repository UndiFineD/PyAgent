...
class Controller:
    ...
    def run(self):
        ...
        if meta_refresh_url:
            ...
            try:
                ...
                sys.exit(0)
            logger.success(
                "Connection to redirection OK. HTTP Status {}".format(r.status_code)
            )
        ...
        try:
            ...
            logger.info(
                "Number of creds that will be tested: {}".format(self.wordlist.length)
            )
        ...
        try:
            mod = importlib.import_module(
                "lib.modules.{}".format(self.args.type.capitalize())
            )
        except Exception as e:
            logger.error(
                "Error while importing module lib.modules.{}".format(
        ...
        if self.args.verbose:
            if len(self.creds_found) > 0:
                logger.success(
                    "{} valid credentials found:".format(len(self.creds_found))
                )
    ...
    def run_bruteforcer(self, module):
        ...
        logger.info(
            "Starting bruteforce with {} threads...".format(self.bruteforcer.nb_threads)
        )
    ...
    def request_error_callback(self, message):
        with self.mutex:
            ...
            if self.consec_reqexcept >= self.args.max_errors:
                self.output.fatal_error(
                    "Too many consecutive network requests " "exceptions. Quitting !"
                )
                self.bruteforcer.stop()
                sys.exit(0)
