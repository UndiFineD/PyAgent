# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-web-brutator\lib\modules\Standardform.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import difflib
import re
...
class Standardform:
    ...
    def check(self):
        ...
        logger.warning(
            "This module is based on heuristics and is prone to "
        ...
        for f in forms:
            ...
            if not input_password:
                input_password = f.find(
                    ...
                    attrs={
                        "name": re.compile(
                            ".*(pass|pwd|pswd|pssw|pswrd).*", re.IGNORECASE
                        )
                    },
                )
                if input_password:
        ...
        if target_form:
            ...
            logger.info(
                "Detected password field name = {name}".format(name=self.password_field)
            )
        else:
            logger.error(
        ...
        if target_form.has_attr("action"):
            ...
            # Absolute URL
            if form_action.lower().startswith(
                "http://"
        ...
        if not self.username_field:
            inputs_no_type = target_form.find_all(
                "input", type=False, attrs={"name": True}
            )
            self.username_field = self.__find_username_field_via_name(inputs_no_type)

        if self.username_field:
            logger.info(
                "Detected username field name = {name}".format(name=self.username_field)
            )
        else:
            logger.info(
        ...
        self.has_csrftoken = (
            target_form.find("input", type=lambda x: x and x.lower() == "hidden")
            is not None
        )
    ...
    def try_auth(self, username, password):
        ...
        if self.has_csrftoken:
            ...
            self.parameters = self.__extract_form_fields(target_form)
            if self.password_field not in self.parameters.keys() or (
                self.username_field
                and self.username_field not in self.parameters.keys()
            ):
                raise AuthException(
                    "Problem occured when reloading page. Maybe some WAF/Protection "
                    "is blocking us ?"
        ...
        if self.method == "GET":
            r = Requester.get(
                self.action_url, params=self.parameters, cookies=self.cookies
            )
        else:
            r = Requester.post(
                self.action_url, data=self.parameters, cookies=self.cookies
            )
        if self.verbose:
        ...
        if re.search(
            "(username\s+(or|and)\s+password|cannot\s+log\s*in|unauthorized"
        ...
        if re.search(
            "(log\s*out|log\s*off|deconn?e|disconn?ec)", r.text, re.IGNORECASE
        ):
            return True
