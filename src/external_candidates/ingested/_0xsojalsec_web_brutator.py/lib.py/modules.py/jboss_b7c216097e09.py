...
class Jboss:
    ...
    def check(self):
        ...
        if r.status_code == 200:
            ...
            logger.info(
                "Jboss authentication interface detected: {}".format(self.interface_url)
            )
            return True
        ...
        if auth_type is not AuthMode.UNKNOWN:
            ...
            logger.info(
                "Jboss jmx-console interface detected: {}".format(self.interface_url)
            )
            return True
        ...
        if auth_type is not AuthMode.UNKNOWN:
            ...
            self.http_auth_type = auth_type
            logger.info(
                "Jboss web-console interface detected: {}".format(self.interface_url)
            )
            return True
        ...
        if auth_type is not AuthMode.UNKNOWN:
            ...
            self.http_auth_type = auth_type
            logger.info(
                "Jboss management interface detected: {}".format(self.interface_url)
            )
            return True
        ...
        if self.interface == "admin-console":
            ...
            if not m:
                raise RequestException(
                    "Unable to retrieve ViewState from {}".format(self.interface_url)
                )
            ...
            if not m:
                raise RequestException(
                    "Unable to retrieve JSESSIONID value "
            ...
            status = (
                'name="login_form:password"' not in r.text
