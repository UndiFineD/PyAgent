# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-web-brutator\lib\modules\Glassfish.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import re
...
class Glassfish:
    ...
    def check(self):
        ...
        auth_type = Requester.get_http_auth_type(
            "{}/management/domain".format(self.url)
        )
        if auth_type is not AuthMode.UNKNOWN:
            ...
            logger.info(
                "Glassfish admin interface detected: {}".format(self.interface_url)
            )
            return True
    ...
    def try_auth(self, username, password):
        if self.interface == "glassfish-admin":
            r = Requester.http_auth(
                self.interface_url, self.http_auth_type, username, password
            )
