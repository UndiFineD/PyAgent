...
try:
    ...
    print(
        "Asegúrate de tener los archivos agi.py y unificado.py en el mismo directorio"
    )
    exit(1)
...
class AGIVoiceLayer:
    ...
    def _domain_routing_heuristic(self, question: str) -> str:
        ...
        if max_score == 0:
            if any(
                word in question_lower for word in ["número", "numero", "par", "impar"]
            ):
                return "parity"
            elif any(
                word in question_lower
                for word in ["posición", "posición", "orbita", "órbita"]
            ):
                return "kepler"
            elif any(
                word in question_lower
                for word in ["estado", "angulo", "ángulo", "momento"]
            ):
                return "pendulum"
    ...
    def _prepare_input_for_expert(
        self, expert_name: str, parameters: Dict[str, Any], question: str
    ) -> torch.Tensor:
        ...
        try:
            if expert_name == "parity":
                ...
                if self.debug:
                    print(
                        f"⚠️  Dominio desconocido '{expert_name}', usando parity como fallback"
                    )
            ...
            if expert_name == "parity":
                ...
                return torch.tensor(
                    [[1.0, 0.0, 1.0, 0.5, 1.0]], dtype=torch.float32
                ).to(DEVICE)
            else:
                return torch.tensor([[0.1, 0.2, 0.0, 0.0]], dtype=torch.float32).to(
                    DEVICE
                )
    ...
    def _interpret_technical_result(
        ...
        try:
            ...
            if expert_name == "parity":
                ...
                if self.debug:
                    print(
                        f"📊 Parity - bits: {bits}, k={k}, bits seleccionados: {selected_bits}"
                    )
                    print(
    ...
    def _generate_final_response(
        self, question: str, expert_result: str, expert_name: str
    ) -> str:
        ...
        try:
            ...
            if (
                expert_name == "parity"
                and "El número" not in response_text
                and "es par" in response_text.lower()
            ):
    ...
    def respond_to_question(self, question: str) -> str:
        ...
        with torch.no_grad():
            if self.use_unified:
                ...
                if predicted_domain != expert_name:
                    if self.debug:
                        print(
                            f"⚠️  Router predijo '{predicted_domain}' pero necesitamos '{expert_name}'"
                        )
        ...
        print(
            f"   📈 Salida cruda: {output.shape if hasattr(output, 'shape') else 'escalar'}"
        )

        # Paso 3: Interpretar resultado técnico
        interpretation_start = time.time()
        technical_result = self._interpret_technical_result(
            expert_name, output, parameters, question
        )
        ...
        final_response = self._generate_final_response(
            question, technical_result, expert_name
        )
    ...
    def interactive_mode(self):
        ...
        while True:
            try:
                ...
                print(
                    "💡 Intenta reformular tu pregunta o verifica que Ollama esté ejecutándose."
                )


def demo_voice_layer():
    """Demostración con ejemplos corregidos"""
    voice_layer = AGIVoiceLayer(
        model_name="qwen2.5:0.5b", use_unified=False, debug=True
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "💡 Asegúrate de que Ollama esté ejecutándose y que el modelo 'qwen2.5:0.5b' esté descargado"
        )
