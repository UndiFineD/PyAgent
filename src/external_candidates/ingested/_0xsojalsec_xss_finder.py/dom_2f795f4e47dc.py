# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-XSS-Finder\dom.py
#!/usr/bin/python3
...
NAME, VERSION, AUTHOR, LICENSE = (
    "Damn Small XSS Scanner (DSXS) < 100 LoC (Lines of Code)",
    "0.3a",
...
REGULAR_PATTERNS = (  # each (regular pattern) item consists of (r"context regex", (prerequisite unfiltered characters), "info text", r"content removal regex")
    (
        r"\A[^<>]*%(chars)s[^<>]*\Z",
        ("<", ">"),
        '".xss.", pure text response, %(filtering)s filtering',
        None,
    ),
    (
        r"<!--[^>]*%(chars)s|%(chars)s[^<]*-->",
        ("<", ">"),
        "\"<!--.'.xss.'.-->\", inside the comment, %(filtering)s filtering",
        None,
    ),
    (
    ...
    (
        r">[^<]*%(chars)s[^<]*(<|\Z)",
        ("<", ">"),
        '">.xss.<", outside of tags, %(filtering)s filtering',
        r"(?s)<script.+?</script>|<!--.*?-->",
    ),
    (
        r"<[^>]*=\s*'[^>']*%(chars)s[^>']*'[^>]*>",
    ...
    (
        r"<[^>]*%(chars)s[^>]*>",
        (),
        '"<.xss.>", inside the tag, outside of quotes, %(filtering)s filtering',
...
def _contains(content, chars):
    content = (
        re.sub(r"\\[%s]" % re.escape("".join(chars)), "", content) if chars else content
    )
    ...
    try:
        for phase in (GET, POST):
            ...
            for match in re.finditer(
                r"((\A|[?&])(?P<parameter>[\w\[\]]+)=)(?P<value>[^&#]*)", current
            ):
                ...
                prefix, suffix = (
                    "".join(random.sample(string.ascii_lowercase, PREFIX_SUFFIX_LENGTH))
                    for i in range(2)
                )
                for pool in (LARGER_CHAR_POOL, SMALLER_CHAR_POOL):
                    if not found:
                        ...
                        content = (
                            ...
                            "%s%s" % ("'" if pool == LARGER_CHAR_POOL else "", prefix),
                            prefix,
                        )
                        for (
                            regex,
                            condition,
                            info,
                            content_removal_regex,
                        ) in REGULAR_PATTERNS:
                            filtered = re.sub(content_removal_regex or "", "", content)
                            for sample in re.finditer(
                                "%s([^ ]+?)%s" % (prefix, suffix), filtered, re.I
                            ):
                                context = re.search(
                                    regex % {"chars": re.escape(sample.group(0))},
                                    filtered,
                                    re.I,
                                )
