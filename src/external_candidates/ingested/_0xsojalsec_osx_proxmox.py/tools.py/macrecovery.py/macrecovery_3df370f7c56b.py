# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OSX-PROXMOX\tools\macrecovery\macrecovery.py
#!/usr/bin/env python3
...
def generate_id(id_type, id_value=None):
    ...
    return (
        "".join(random.choice(valid_chars) for i in range(id_type))
        if not id_value
        else id_value
    )
...
def verify_chunklist(cnkpath):
    with open(cnkpath, "rb") as f:
        ...
        if signature_method == 1:
            ...
            plaintext = (
                
...
def verify_image(dmgpath, cnkpath):
    ...
    with open(dmgpath, "rb") as dmgf:
        ...
        for cnksize, cnkhash in verify_chunklist(cnkpath):
            ...
            if len(cnk) != cnksize:
                raise RuntimeError(
                    f"Invalid chunk {cnkcount} size: expected {cnksize}, read {len(cnk)}"
                )
...
def action_download(args):
    ...
    cnkpath = save_image(
        info[INFO_SIGN_LINK], info[INFO_SIGN_SESS], cnkname, args.outdir
    )
    dmgname = "" if args.basename == "" else args.basename + ".dmg"
    dmgpath = save_image(
        info[INFO_IMAGE_LINK], info[INFO_IMAGE_SESS], dmgname, args.outdir
    )
    try:
        ...
        if isinstance(err, AssertionError) and str(err) == "":
            try:
                ...
                while tb.tb_next:
                    tb = tb.tb_next
                err = linecache.getline(
                    tb.tb_frame.f_code.co_filename, tb.tb_lineno, tb.tb_frame.f_globals
                ).strip()
            except Exception:
...
def action_selfcheck(args):
    ...
    session = get_session(args)
    valid_default = get_image_info(
        session, bid=RECENT_MAC, mlb=MLB_VALID, diag=False, os_type="default"
    )
    valid_latest = get_image_info(
        session, bid=RECENT_MAC, mlb=MLB_VALID, diag=False, os_type="latest"
    )
    product_default = get_image_info(
        session, bid=RECENT_MAC, mlb=MLB_PRODUCT, diag=False, os_type="default"
    )
    product_latest = get_image_info(
        session, bid=RECENT_MAC, mlb=MLB_PRODUCT, diag=False, os_type="latest"
    )
    generic_default = get_image_info(
        session, bid=RECENT_MAC, mlb=MLB_ZERO, diag=False, os_type="default"
    )
    generic_latest = get_image_info(
        session, bid=RECENT_MAC, mlb=MLB_ZERO, diag=False, os_type="latest"
    )
    ...
    if product_default[INFO_PRODUCT] != valid_default[INFO_PRODUCT]:
        ...
        print(
            "ERROR: Valid and product MLB give mismatch, got {product_default[INFO_PRODUCT]} and {valid_default[INFO_PRODUCT]}"
        )
        return 1
...
def action_verify(args):
    ...
    uvalid_default = get_image_info(
        session, bid=args.board_id, mlb=args.mlb, diag=False, os_type="default"
    )
    uvalid_latest = get_image_info(
        session, bid=args.board_id, mlb=args.mlb, diag=False, os_type="latest"
    )
    uproduct_default = get_image_info(
        session,
        bid=args.board_id,
        mlb=product_mlb(args.mlb),
        diag=False,
        os_type="default",
    )
    ...
    if uvalid_default[INFO_PRODUCT] != uvalid_latest[INFO_PRODUCT]:
        print(
            f"SUCCESS: {args.mlb} MLB looks valid and supported!"
            if uvalid_latest[INFO_PRODUCT] == generic_latest[INFO_PRODUCT]
            else f"SUCCESS: {args.mlb} MLB looks valid, but probably unsupported!"
        )
        return 0
...
def action_guess(args):
    ...
    for model in db:
        try:
            if anon:
                ...
                model_latest = get_image_info(
                    session, bid=model, mlb=MLB_ZERO, diag=False, os_type="latest"
                )
                ...
                user_default = get_image_info(
                    session, bid=model, mlb=mlb, diag=False, os_type="default"
                )

                if user_default[INFO_PRODUCT] != generic_latest[INFO_PRODUCT]:
                    supported[model] = [
                        db[model],
                        user_default[INFO_PRODUCT],
                        generic_latest[INFO_PRODUCT],
                    ]
            else:
                # For normal lookup check when given model has mismatching normal and latest.
                user_latest = get_image_info(
                    session, bid=model, mlb=mlb, diag=False, os_type="latest"
                )
    ...
    if len(supported) > 0:
        ...
        for model in supported.items():
            print(
                f"- {model}, up to {supported[model][0]}, default: {supported[model][1]}, latest: {supported[model][2]}"
            )
        return 0
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["download", "selfcheck", "verify", "guess"],
    ...
    parser.add_argument(
        ...
        "--outdir",
        type=str,
        default="com.apple.recovery.boot",
    ...
    parser.add_argument(
        "-diag", "--diagnostics", action="store_true", help="download diagnostics image"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--board-db",
        type=str,
        default=os.path.join(SELF_DIR, "boards.json"),
