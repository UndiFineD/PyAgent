# Extracted from: C:\DEV\PyAgent\.external\CraftSelfOptimizer\CraftSelfOptimizerv2.py
    ...
    def step(self, closure=None):
        ...
        if self.hybrid_optimizer and not self.switch_triggered:
            if len(self.loss_history) >= 5:
                ...
                if loss_std < 1e-4 and (
                    self.current_epoch - self.last_switch_epoch >= self.hybrid_cooldown
                ):
                    self.switch_triggered = True
                    self.last_switch_epoch = self.current_epoch
                    if self.debug:
                        print(
                            f"[CraftSelfOptimizerv2] Hybrid switch triggered at epoch {self.current_epoch}"
                        )
        ...
        for group in self.param_groups:
            ...
            if group["auto_tune"] and len(self.loss_history) >= 2:
                ...
                if self.debug:
                    print(
                        f"[CraftSelfOptimizerv2] Epoch {self.current_epoch}: a={group['a']:.4f}, k={group['k']:.6f}, loss_ratio={loss_ratio:.4f}"
                    )
        ...
        if self.switch_triggered and (
            self.current_epoch - self.last_switch_epoch >= self.hybrid_cooldown
        ):
            self.switch_triggered = False
            if self.debug:
                print(
                    f"[CraftSelfOptimizerv2] Hybrid switch reset at epoch {self.current_epoch}"
                )
