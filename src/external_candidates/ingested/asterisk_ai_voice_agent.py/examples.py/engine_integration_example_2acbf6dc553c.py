...
class EngineIntegrationExample:
    ...
    async def handle_stasis_start(self, event):
        ...
        if greeting_audio:
            playback_id = await self.playback_manager.play_audio(
                caller_channel_id, greeting_audio, "greeting"  # Use call_id
            )
    ...
    async def handle_playback_finished(self, event):
        ...
        if success:
            logger.info(
                "PlaybackFinished handled successfully", playback_id=playback_id
            )
        else:
            logger.warning(
                "PlaybackFinished for unknown playback", playback_id=playback_id
            )
...
if __name__ == "__main__":
    # Example usage
    print(
        "This example shows how the Engine would integrate with the new core components."
    )
