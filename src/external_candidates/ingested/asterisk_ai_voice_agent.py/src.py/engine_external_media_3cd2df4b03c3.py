...
class ExternalMediaEngine:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.external_media_channels: Dict[str, str] = (
            {}
        )  # call_id -> external_channel_id
        ...
        if rtp_config:
            base_port = getattr(
                rtp_config, "port", getattr(rtp_config, "rtp_port", 18080)
            )
            ...
            if hasattr(self.config, "providers") and "local" in self.config.providers:
                ...
                if local_config.get("enabled", True):
                    ...
                    self.providers["local"] = LocalProvider(
                        local_config, on_event_wrapper
                    )
                    logger.info("Local provider initialized")

            # Initialize Deepgram provider
            if (
                hasattr(self.config, "providers")
                and "deepgram" in self.config.providers
            ):
                ...
                if deepgram_config.get("api_key"):
                    ...
                    self.providers["deepgram"] = DeepgramProvider(
                        deepgram_config, self.config.llm, on_event_wrapper
                    )
        ...
        try:
            ...
            if event_type == "AgentAudio":
                ...
                if transcript and call_id:
                    logger.info(
                        "Transcript received", call_id=call_id, transcript=transcript
                    )
    ...
    async def _on_stasis_start(self, event: Dict[str, Any]):
        ...
        try:
            ...
            if not external_channel:
                logger.error(
                    "Failed to create External Media channel", channel_id=channel_id
                )
            ...
            external_success = await self.ari_client.add_channel_to_bridge(
    ...
    async def _on_rtp_audio_received(self, call_id: str, pcm_data: bytes):
        ...
        try:
            ...
            if hasattr(provider, "send_audio") and provider.websocket:
                ...
                logger.debug(
                    "Audio sent to LocalProvider", call_id=call_id, size=len(ulaw_data)
                )
    ...
    async def _on_playback_finished(self, event: Dict[str, Any]):
        ...
        try:
            ...
            if playback_id in self.ari_client.active_playbacks:
                ...
                logger.debug(
                    "Playback finished and cleaned up", playback_id=playback_id
                )
