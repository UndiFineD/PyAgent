# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\tools\telephony\unified_transfer.py
"""
...
class UnifiedTransferTool(Tool):
    ...
    async def execute(
        ...
        if not config or not config.get("enabled"):
            logger.warning(
                "Unified transfer tool not configured", call_id=context.call_id
            )
            return {"status": "failed", "message": "Transfer service is not available"}
    ...
    async def _transfer_to_extension(
        self, context: ToolExecutionContext, extension: str, description: str
    ) -> Dict[str, Any]:
        """
        ...
        if result and result.get("status") == 204:
            ...
            logger.error(
                "Extension transfer failed", call_id=context.call_id, result=result
            )
            return {
                "status": "failed",
    ...
    async def _transfer_to_queue(
        self, context: ToolExecutionContext, queue: str, description: str
    ) -> Dict[str, Any]:
        """
        ...
        await context.update_session(
            transfer_active=True, transfer_state="in_queue", transfer_target=description
        )
        ...
        await context.ari_client.send_command(
            ...
            params={"context": "ext-queues", "extension": queue, "priority": 1},
        )

        logger.info("✅ Queue transfer initiated", call_id=context.call_id, queue=queue)
    ...
    async def _transfer_to_ringgroup(
        self, context: ToolExecutionContext, ringgroup: str, description: str
    ) -> Dict[str, Any]:
        """
        ...
        await context.ari_client.send_command(
            method="POST",
            resource=f"channels/{context.caller_channel_id}/continue",
            params={"context": "ext-group", "extension": ringgroup, "priority": 1},
        )

        logger.info(
            "✅ Ring group transfer initiated",
            call_id=context.call_id,
            ringgroup=ringgroup,
        )
