...
class TransferCallTool(Tool):
    ...
    async def execute(
        ...
        try:
            ...
            logger.info(
                f"Resolved {target} → {extension} ({dial_string}), using {mode} mode from config"
            )

            # 2. Execute transfer based on mode
            if mode == "warm":
                result = await self._warm_transfer(
                    extension, dial_string, extension_info, context
                )
            else:
                result = await self._blind_transfer(
                    extension, dial_string, extension_info, context
                )
            ...
            logger.error(
                f"Transfer failed: {e}", exc_info=True, call_id=context.call_id
            )
            return {
                "status": "error",
    ...
    async def _resolve_target(
        self, target: str, context: ToolExecutionContext
    ...
    async def _warm_transfer(
        ...
        transfer_context = self._build_transfer_context(
            session, extension_info, context
        )
        ...
        logger.info(
            f"✅ Warm transfer initiated to {extension}", call_id=context.call_id
        )
    ...
    async def _blind_transfer(
        ...
        if result and result.get("status") == 204:
            logger.info(
                f"✅ Blind transfer completed to {extension}", call_id=context.call_id
            )
            return {
                "status": "success",
    ...
    async def _start_moh(self, channel_id: str, context: ToolExecutionContext):
        ...
        moh_class = context.get_config_value(
            "tools.transfer_call.hold_music_class", "default"
        )
    ...
    async def _stop_moh(self, channel_id: str, context: ToolExecutionContext):
        ...
        try:
            await context.ari_client.send_command(
                method="DELETE", resource=f"channels/{channel_id}/moh"
            )
