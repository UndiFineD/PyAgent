...
class TransferToQueueTool(Tool):
    ...
    async def execute(
        ...
        logger.info(
            "📞 Queue transfer requested", call_id=context.call_id, queue=queue_name
        )
    ...
    def _resolve_queue(
        self, queue_name: str, context: ToolExecutionContext
        ...
        if not config or not config.get("enabled"):
            logger.warning(
                "Queue transfer tool not configured", call_id=context.call_id
            )
            return None
    ...
    async def _get_queue_status(
        self, asterisk_queue: str, context: ToolExecutionContext
    ) -> Dict[str, Any]:
        """
    ...
    def _format_queue_message(
        self, description: str, queue_status: Dict[str, Any], max_wait_time: int
    ) -> str:
        """
