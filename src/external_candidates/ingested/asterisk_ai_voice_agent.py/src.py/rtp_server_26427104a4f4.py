# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\rtp_server.py
"""
...
class RTPServer:
    ...
    async def allocate_session(self, call_id: str) -> int:
        ...
        logger.info(
            "RTP session allocated", call_id=call_id, port=port, codec=self.codec
        )
        return port
    ...
    async def send_audio(
        self, call_id: str, chunk: bytes, *, ssrc: Optional[int] = None
    ) -> bool:
        ...
        if not session.send_sequence_initialized:
            session.sequence_number = session.sequence_number or random.randint(
                0, 0xFFFF
            )
        ...
        try:
            ...
            sent = (
                ...
                if self._socket_is_connected(session.socket)
                else session.socket.sendto(
                    packet, (session.remote_host, session.remote_port)
                )
            )
            if sent != len(packet):
                logger.debug(
                    "Short RTP send", call_id=call_id, expected=len(packet), sent=sent
                )
    ...
    async def _rtp_receiver_loop(self, session: RTPSession) -> None:
        ...
        logger.debug(
            "RTP receiver loop started", call_id=call_id, port=session.local_port
        )
        ...
        while self.running and call_id in self.sessions:
            ...
            await self._handle_inbound_packet(
                session, sequence, timestamp, payload, ssrc
            )

        logger.debug(
            "RTP receiver loop stopped", call_id=call_id, port=session.local_port
        )
    ...
    async def _handle_inbound_packet(
        ...
        try:
            ...
            if self.sample_rate != self.SAMPLE_RATE:
                ...
                pcm_resampled, state = audioop.ratecv(
                    pcm_decoded,
                    2,
                    1,
                    self.SAMPLE_RATE,
                    self.sample_rate,
                    session.resample_state,
                )
    ...
    def get_stats(self) -> Dict[str, Any]:
        now = time.time()
        active_sessions = sum(
            1 for s in self.sessions.values() if now - s.last_packet_at < 30
        )
        return {
            ...
            "packet_loss_total": sum(
                s.packet_loss_count for s in self.sessions.values()
            ),
        }
    ...
    async def _cleanup_session(self, session: RTPSession) -> None:
        ...
        if session.receiver_task:
            ...
            try:
                ...
                logger.debug(
                    "RTP receiver task finalisation error",
                    call_id=call_id,
                    error=str(exc),
                )
