# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\logging_config.py
"""
...
def add_service_context(logger, method_name, event_dict):
    ...
    if not component:
        ...
        try:
            component = getattr(
                getattr(logger, "logger", None), "name", None
...
def sanitize_secrets(logger, method_name, event_dict):
    ...
    def redact_value(value):
        ...
        if isinstance(value, dict):
            return {
                k: redact_value(v) if k.lower() in SENSITIVE_KEYS else v
    ...
    def sanitize_dict(d):
        ...
        for key, value in d.items():
            ...
            for pattern in SENSITIVE_KEYS:
                ...
                if key_normalized == pattern_normalized or key_normalized.endswith(
                    pattern_normalized
                ):
                    is_sensitive = True
                    break
...
def configure_logging(
    ...
    try:
        log_to_file = (
            bool(int(os.getenv("LOG_TO_FILE", "0")))
    ...
    log_level_upper = (
        log_level.upper() if isinstance(log_level, str) else str(log_level)
    )
    tb_mode = (
        os.getenv("LOG_SHOW_TRACEBACKS", "auto").strip().lower()
    )  # auto|always|never
    if tb_mode == "always":
    ...
    try:
        level_value = (
            getattr(logging, log_level_upper, logging.INFO)
    ...
    if log_to_file:
        try:
            ...
            path = log_file_path
            try:
                looks_like_dir = path.endswith(os.sep) or (
