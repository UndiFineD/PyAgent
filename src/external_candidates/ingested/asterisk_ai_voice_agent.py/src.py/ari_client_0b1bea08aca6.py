...
class ARIClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            async with self.http_session.get(
                ...
                if response.status != 200:
                    raise ConnectionError(
                        f"Failed to connect to ARI HTTP endpoint. Status: {response.status}"
                    )
            ...
            logger.error(
                "Failed to connect to ARI, will retry...", error=str(e), exc_info=True
            )
            ...
            async for message in self.websocket:
                try:
                    ...
                    if event_type == "ChannelAudioFrame":
                        ...
                        logger.debug(
                            "ChannelAudioFrame received", channel_id=channel_id
                        )
    ...
    def add_event_handler(self, event_type: str, handler: Callable):
        ...
        logger.debug(
            "Added event handler", event_type=event_type, handler=handler.__name__
        )
    ...
    async def play_media(
        ...
        logger.info(
            "Playing media on channel", channel_id=channel_id, media_uri=media_uri
        )
    ...
    async def create_bridge(self, bridge_type: str = "mixing") -> Optional[str]:
        ...
        try:
            ...
            if response.get("id"):
                logger.info(
                    "Bridge created", bridge_id=response["id"], bridge_type=bridge_type
                )
                return response["id"]
            else:
        ...
        try:
            ...
            if status is not None:
                if 200 <= int(status) < 300:
                    logger.info(
                        "Playback stopped", playback_id=playback_id, status=status
                    )
                    return True
                logger.debug(
            ...
            logger.error(
                "Error stopping playback", playback_id=playback_id, exc_info=True
            )
            return False
    ...
    async def record_channel(
        ...
        try:
            ...
            if status is not None and not (200 <= int(status) < 300):
                ...
                return False
            logger.info(
                "ARI channel recording started", channel_id=channel_id, name=name
            )
            return True
        except Exception:
            logger.error(
    ...
    async def add_channel_to_bridge(self, bridge_id: str, channel_id: str) -> bool:
        ...
        try:
            response = await self.send_command(
                "POST", f"bridges/{bridge_id}/addChannel", data={"channel": channel_id}
            )
    ...
    async def play_audio_response(self, channel_id: str, audio_data: bytes):
        ...
        try:
            ...
            logger.debug(
                "Writing ulaw audio file", path=container_path, size=len(audio_data)
            )
            ...
            try:
                ...
                logger.warning(
                    "Failed to change file ownership", path=container_path, error=str(e)
                )
            ...
            if os.path.exists(container_path):
                ...
                logger.debug(
                    "File created successfully", path=container_path, size=file_size
                )
        ...
        if file_path:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    logger.error(
                        "Error deleting audio file", file_path=file_path, exc_info=True
                    )
            ...
            for file_path in files:
                try:
                    ...
                    if os.path.getmtime(file_path) < (time.time() - 30):
                        os.remove(file_path)
                        logger.debug(
                            "Cleaned up orphaned audio file", file_path=file_path
                        )
                except OSError as e:
                    logger.debug(
                        "Could not clean up file", file_path=file_path, error=str(e)
                    )
    ...
    async def _on_audio_frame(self, channel, event):
        ...
        try:
            ...
            if audio_payload:
                ...
                logger.debug(
                    "Decoded audio data", bytes=len(audio_data), format=frame_format
                )
                ...
                if self.audio_frame_handler:
                    ...
                    logger.debug(
                        f"Received audio frame but no handler set: {len(audio_data)} bytes"
                    )
            else:
    ...
    async def play_audio_file(self, channel_id: str, file_path: str) -> bool:
        ...
        try:
            ...
            for attempt in range(15):  # Try up to 15 times (1.5 seconds total)
                if os.path.exists(file_path):
                    if os.access(file_path, os.R_OK):
                        ...
                        if file_size > 0:
                            logger.debug(
                                f"File verified: {file_path} ({file_size} bytes) - attempt {attempt + 1}"
                            )
                            break
                        else:
                            logger.warning(
                                f"File exists but is empty: {file_path} - attempt {attempt + 1}"
                            )
                    else:
                        logger.warning(
                    ...
                    logger.warning(
                        f"File not found: {file_path} - attempt {attempt + 1}"
                    )
            ...
            if result:
                ...
                logger.error(
                    f"Failed to play audio file {file_path} after {elapsed_time:.1f}ms"
                )
                return False
    ...
    async def create_audio_file_from_ulaw(
        ...
        try:
            ...
            for attempt in range(10):  # Try up to 10 times (1 second total)
                if os.path.exists(temp_file_path) and os.access(
                    temp_file_path, os.R_OK
                ):
                    file_size = os.path.getsize(temp_file_path)
                    if file_size > 0:
                        logger.debug(
                            f"Created WAV file: {temp_file_path} ({file_size} bytes) - attempt {attempt + 1}"
                        )
                        return temp_file_path
                time.sleep(0.1)  # 100ms delay

            logger.error(
                f"Failed to create accessible WAV file after 10 attempts: {temp_file_path}"
            )
            return ""
    ...
    async def is_channel_active(self, channel_id: str) -> bool:
        ...
        try:
            ...
            if result and result.get("id") == channel_id:
                ...
                logger.debug(
                    "Channel not found in ARI", channel_id=channel_id, result=result
                )
                return False
        except Exception as e:
            logger.debug(
                "Error checking channel status", channel_id=channel_id, error=str(e)
            )
            return False
    ...
    async def validate_channel_for_playback(self, channel_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Channel validation successful", channel_id=channel_id, state=state
            )
            return True
    ...
    async def create_external_media_channel(
        ...
        try:
            ...
            if response and response.get("id"):
                ...
                logger.error(
                    "Failed to create External Media channel", response=response
                )
                return None
    ...
    async def play_audio_via_bridge(
        ...
        try:
            ...
            response = await self.send_command(
                "POST", f"bridges/{bridge_id}/play", data=data
            )
    ...
    async def destroy_bridge(self, bridge_id: str) -> bool:
        ...
        try:
            ...
            if status is not None:
                ...
                if int(status) == 404:
                    logger.debug(
                        "Bridge destroy idempotent - already gone", bridge_id=bridge_id
                    )
                    return True
                else:
                    logger.error(
