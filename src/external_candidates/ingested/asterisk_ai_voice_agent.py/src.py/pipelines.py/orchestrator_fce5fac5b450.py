# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\pipelines\orchestrator.py
"""
...
from .base import Component, LLMComponent, STTComponent, TTSComponent
...
from .openai import OpenAILLMAdapter, OpenAISTTAdapter, OpenAITTSAdapter
...
def _extract_role(component_key: str) -> str:
    ...
    if len(parts) != 2:
        raise PipelineOrchestratorError(
            f"Cannot determine component role for '{component_key}'"
        )
    return parts[1]
...
def _make_placeholder_factory(role: str) -> ComponentFactory:
    ...
    if adapter_cls is None:
        raise PipelineOrchestratorError(
            f"No placeholder adapter registered for role '{role}'"
        )
...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self._local_provider_config: Optional[LocalProviderConfig] = (
            self._hydrate_local_config()
        )
        self._deepgram_provider_config: Optional[DeepgramProviderConfig] = (
            self._hydrate_deepgram_config()
        )
        self._openai_provider_config: Optional[OpenAIProviderConfig] = (
            self._hydrate_openai_config()
        )
        self._google_provider_config: Optional[GoogleProviderConfig] = (
            self._hydrate_google_config()
        )
        ...
        self._active_pipeline_name: Optional[str] = getattr(
            config, "active_pipeline", None
        )
    ...
    async def start(self) -> None:
        ...
        validation_results = {}
        for name, entry in pipelines.items():
            validation_results[name] = await self._validate_pipeline_connectivity(
                name, entry
            )
    ...
    async def stop(self) -> None:
        ...
        self._started = False
        logger.info(
            "Pipeline orchestrator stopped",
            remaining_assignments=len(self._assignments),
        )
    ...
    def get_pipeline(
        ...
        if not self._started:
            logger.debug(
                "Pipeline orchestrator requested before start; skipping resolution",
                call_id=call_id,
            )
            return None
    ...
    async def release_pipeline(self, call_id: str) -> None:
        ...
        for adapter in (
            resolution.stt_adapter,
            resolution.llm_adapter,
            resolution.tts_adapter,
        ):
    ...
    def _register_builtin_factories(self) -> None:
        if self._local_provider_config:
            ...
            logger.debug(
                "Local pipeline adapters not registered - provider config unavailable or disabled"
            )

        if self._deepgram_provider_config:
            stt_factory = self._make_deepgram_stt_factory(
                self._deepgram_provider_config
            )
            flux_stt_factory = self._make_deepgram_flux_stt_factory(
                self._deepgram_provider_config
            )
            tts_factory = self._make_deepgram_tts_factory(
                self._deepgram_provider_config
            )
            ...
            logger.debug(
                "Deepgram pipeline adapters not registered - provider config unavailable"
            )
        ...
        if self._openai_provider_config:
            ...
            logger.debug(
                "OpenAI pipeline adapters not registered - provider config unavailable or invalid"
            )
        ...
        if self._google_provider_config:
            ...
            logger.debug(
                "Google pipeline adapters not registered - credentials unavailable or invalid"
            )
    ...
    def _hydrate_openai_config(self) -> Optional[OpenAIProviderConfig]:
        ...
        if not config.api_key:
            logger.warning(
                "OpenAI pipeline adapters require an API key; falling back to placeholder adapters"
            )
            return None
    ...
    def _resolve_factory(self, component_key: str) -> ComponentFactory:
        ...
        raise PipelineOrchestratorError(
            f"No component factory registered for '{component_key}'"
        )

    def _build_component(
        self, component_key: str, options: Dict[str, Any]
    ) -> Component:
