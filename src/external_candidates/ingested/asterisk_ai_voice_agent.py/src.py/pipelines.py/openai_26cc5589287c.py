# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\pipelines\openai.py
"""
...
def _chunk_audio(
    audio_bytes: bytes, encoding: str, sample_rate: int, chunk_ms: int
) -> Iterable[bytes]:
    if not audio_bytes:
        return
...
class OpenAISTTAdapter(STTComponent):
    ...
    def __init__(
        ...
        self._default_timeout = float(
            self._pipeline_defaults.get(
                "response_timeout_sec", provider_config.response_timeout_sec
            )
        )
    ...
    async def open_call(self, call_id: str, options: Dict[str, Any]) -> None:
        ...
        session = _RealtimeSessionState(
            websocket=websocket, options=merged, session_id=session_id
        )
    ...
    async def transcribe(
        ...
        events = [
            ...
            {"type": "input_audio_buffer.commit"},
            {
                ...
                "response": {
                    "modalities": ["text"],
                    "instructions": merged.get("prompt_override"),
                },
            },
        ]
    ...
    async def _await_transcript(
        ...
        while True:
            ...
            try:
                ...
                logger.debug(
                    "OpenAI STT received non-JSON message", message_preview=message[:64]
                )
                continue
    ...
    def _compose_options(
        self, runtime_options: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        merged = {
            ...
            "organization": runtime_options.get(
                "organization",
                self._pipeline_defaults.get(
                    "organization", self._provider_defaults.organization
                ),
            ),
            "project": runtime_options.get(
                "project",
                self._pipeline_defaults.get("project", self._provider_defaults.project),
            ),
            "base_url": runtime_options.get(
                "base_url",
                self._pipeline_defaults.get(
                    "base_url", self._provider_defaults.realtime_base_url
                ),
            ),
            "model": runtime_options.get(
                "model",
                self._pipeline_defaults.get(
                    "model", self._provider_defaults.realtime_model
                ),
            ),
            "modalities": runtime_options.get(
                "modalities",
                self._pipeline_defaults.get(
                    "modalities", self._provider_defaults.default_modalities or ["text"]
                ),
            ),
            ...
            "input_sample_rate_hz": int(
                runtime_options.get(
                    ...
                    self._pipeline_defaults.get(
                        "input_sample_rate_hz",
                        self._provider_defaults.input_sample_rate_hz,
                    ),
                )
            ),
            "response_timeout_sec": runtime_options.get(
                ...
                self._pipeline_defaults.get(
                    "response_timeout_sec", self._provider_defaults.response_timeout_sec
                ),
            ),
        ...
        for attempt in range(retries + 1):
            try:
                async with self._session.post(
                    url, json=payload, headers=headers, timeout=merged["timeout_sec"]
                ) as response:
                    ...
                    if not choices:
                        logger.warning(
                            "OpenAI chat completion returned no choices",
                            call_id=call_id,
                        )
                        return ""
                    ...
                    if tool_calls:
                        ...
                        for tc in tool_calls:
                            try:
                                ...
                                logger.warning(
                                    "Failed to parse tool call", error=str(e)
                                )

                        logger.info(
                            "OpenAI chat completion received with tools", **log_ctx
                        )
                        return LLMResponse(
                            text=content or "",
                ...
                if self._session is None or self._session.closed:
                    logger.info(
                        "OpenAI LLM generation cancelled (session closed)",
                        call_id=call_id,
                    )
                    return ""
                ...
                if attempt == retries:
                    logger.error(
                        "OpenAI LLM connection error", call_id=call_id, error=str(e)
                    )
                    raise

                logger.warning(
                    "OpenAI LLM connection error, retrying",
                    call_id=call_id,
                    error=str(e),
                )
    ...
    async def _generate_realtime(
        ...
        session_payload = {
            ...
            "session": {
                ...
                "modalities": merged.get("modalities"),
                "instructions": merged.get("system_prompt")
        ...
        try:
            while True:
                message = await asyncio.wait_for(
                    websocket.recv(), timeout=merged["timeout_sec"]
                )
                if isinstance(message, bytes):
                    continue
                payload = json.loads(message)
    ...
    def _compose_options(
        ...
        merged = {
            ...
            "realtime_base_url": runtime_options.get(
                "realtime_base_url",
                self._pipeline_defaults.get(
                    "realtime_base_url", self._provider_defaults.realtime_base_url
                ),
            ),
            "chat_model": runtime_options.get(
                "chat_model",
                self._pipeline_defaults.get(
                    "chat_model", self._provider_defaults.chat_model
                ),
            ),
            "realtime_model": runtime_options.get(
                "realtime_model",
                self._pipeline_defaults.get(
                    "realtime_model", self._provider_defaults.realtime_model
                ),
            ),
            "modalities": runtime_options.get(
                "modalities",
            ...
            "system_prompt": runtime_options.get(
                "system_prompt", self._pipeline_defaults.get("system_prompt")
            ),
            "instructions": runtime_options.get(
                "instructions", self._pipeline_defaults.get("instructions")
            ),
            "temperature": runtime_options.get(
                "temperature", self._pipeline_defaults.get("temperature", 0.7)
            ),
            "max_tokens": runtime_options.get(
                "max_tokens", self._pipeline_defaults.get("max_tokens")
            ),
            "timeout_sec": float(
                runtime_options.get(
                    "timeout_sec",
            ...
            "use_realtime": runtime_options.get(
                "use_realtime", self._pipeline_defaults.get("use_realtime", False)
            ),
            "tools": runtime_options.get(
                "tools", self._pipeline_defaults.get("tools", [])
            ),
        }
    ...
    def __init__(
        ...
        *,
        ...
        self._chunk_size_ms = int(
            self._pipeline_defaults.get("chunk_size_ms", provider_config.chunk_size_ms)
        )
    ...
    def _compose_options(
        ...
        merged_source = {
            ...
            "sample_rate": int(
                runtime_options.get("source_format", {}).get(
                    "sample_rate",
                    source_defaults.get(
                        "sample_rate", self._provider_defaults.input_sample_rate_hz
                    ),
                )
            ),
        }
        ...
        merged_target = {
            "encoding": runtime_options.get("format", {}).get(
                "encoding",
                format_defaults.get(
                    "encoding", self._provider_defaults.target_encoding
                ),
            ),
            "sample_rate": int(
                runtime_options.get("format", {}).get(
                    "sample_rate",
                    format_defaults.get(
                        "sample_rate", self._provider_defaults.target_sample_rate_hz
                    ),
                )
            ),
        }
        ...
        merged = {
            ...
            "tts_base_url": runtime_options.get(
                "base_url",
                self._pipeline_defaults.get(
                    "base_url", self._provider_defaults.tts_base_url
                ),
            ),
            ...
            "voice": runtime_options.get(
                "voice",
                self._pipeline_defaults.get("voice", self._provider_defaults.voice),
            ),
            "chunk_size_ms": runtime_options.get(
                "chunk_size_ms",
                self._pipeline_defaults.get(
                    "chunk_size_ms", self._provider_defaults.chunk_size_ms
                ),
            ),
            "timeout_sec": float(
                runtime_options.get(
                    "timeout_sec",
                    self._pipeline_defaults.get(
                        "timeout_sec", self._provider_defaults.response_timeout_sec
                    ),
                )
            ),
