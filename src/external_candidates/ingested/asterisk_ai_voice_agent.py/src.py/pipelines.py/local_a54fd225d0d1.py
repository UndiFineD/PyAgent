# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\pipelines\local.py
"""
...
class _LocalAdapterBase:
    ...
    def _compose_options(
        ...
        merged = _merge_dicts(
            dict(self._provider_defaults or {}), self._pipeline_defaults
        )
        ...
        merged.setdefault(
            "response_timeout_sec", merged.get("response_timeout_sec", 5.0)
        )
        merged.setdefault("mode", merged.get("mode", self._default_mode))
        merged.setdefault(
            "locale", merged.get("locale") or merged.get("language") or "en-US"
        )
        merged.setdefault("chunk_ms", merged.get("chunk_ms", 200))
        return merged

    async def _send_json(
        self, session: _LocalSessionState, payload: Dict[str, Any]
    ) -> None:
        try:
    ...
    async def _await_mode_ready(
        self, session: _LocalSessionState, options: Dict[str, Any]
    ) -> None:
        ...
        try:
            if timeout is None:
                ...
                message = await asyncio.wait_for(
                    session.websocket.recv(), timeout=timeout
                )
    ...
    async def _ensure_session(
        self, call_id: str, options: Dict[str, Any]
    ) -> _LocalSessionState:
        ...
        while True:
            ...
            if remaining <= 0:
                raise asyncio.TimeoutError(
                    "Local STT adapter timed out waiting for transcript"
                )
            ...
            is_final = bool(
                message.get("is_final", not message.get("is_partial", False))
            )
...
class LocalLLMAdapter(_LocalAdapterBase, LLMComponent):
    ...
    async def generate(
        ...
        timeout = float(
            merged.get(
                "llm_response_timeout_sec", merged.get("response_timeout_sec", 5.0)
            )
        )
        started_at = time.perf_counter()
