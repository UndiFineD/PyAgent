# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\pipelines\deepgram.py
"""
...
class DeepgramSTTAdapter(STTComponent):
    ...
    def __init__(
        ...
        self._default_timeout = float(
            self._pipeline_defaults.get("response_timeout_sec", 5.0)
        )
    ...
    async def open_call(self, call_id: str, options: Dict[str, Any]) -> None:
        ...
        http_session = aiohttp.ClientSession()
        self._sessions[call_id] = _STTSessionState(
            options=merged, http_session=http_session
        )
    ...
    async def transcribe(
        ...
        if sample_rate_hz != api_sample_rate:
            ...
            api_audio, _ = audioop.ratecv(
                api_audio, 2, 1, sample_rate_hz, api_sample_rate, None
            )
            logger.debug(
                "STT resampled audio",
                call_id=call_id,
        ...
        if api_encoding in ("mulaw", "g711_ulaw", "mu-law"):
            ...
            logger.debug(
                "STT encoded PCM16 → mulaw", call_id=call_id, bytes=len(api_audio)
            )
            ...
            logger.debug(
                "STT encoded PCM16 → alaw", call_id=call_id, bytes=len(api_audio)
            )
    ...
    def _compose_options(
        ...
        merged = {
            ...
            "model": runtime_options.get(
                "model",
                self._pipeline_defaults.get("model", self._provider_defaults.model),
            ),
            ...
            "encoding": runtime_options.get(
                "encoding",
                self._pipeline_defaults.get(
                    "encoding", self._provider_defaults.input_encoding
                ),
            ),
            ...
            "smart_format": runtime_options.get(
                "smart_format", self._pipeline_defaults.get("smart_format", True)
            ),
    ...
    def _extract_transcript_from_rest(result: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                "Failed to extract transcript from Deepgram response", error=str(exc)
            )
            return None
...
class DeepgramTTSAdapter(TTSComponent):
    ...
    async def synthesize(
        ...
        url, params = self._build_tts_request(
            merged, target_encoding, target_sample_rate
        )
    ...
    def _compose_options(
        ...
        merged_format = {
            "encoding": runtime_options.get("format", {}).get(
                "encoding", format_defaults.get("encoding", "mulaw")
            ),
            "sample_rate": runtime_options.get("format", {}).get(
                "sample_rate", format_defaults.get("sample_rate", 8000)
            ),
        }
        ...
        merged = {
            ...
            "model": runtime_options.get(
                ...
                self._pipeline_defaults.get(
                    "model",
                    self._provider_defaults.tts_model or self._provider_defaults.model,
                ),
            ),
            "voice": runtime_options.get(
                "voice",
                self._pipeline_defaults.get("voice", self._provider_defaults.tts_model),
            ),
            "language": runtime_options.get(
                "language",
                self._pipeline_defaults.get(
        ...
        source_cfg = runtime_options.get(
            "source_format", self._pipeline_defaults.get("source_format", {})
        )
    ...
    def _build_tts_request(
        ...
        params["sample_rate"] = int(
            source_format.get("sample_rate", target_sample_rate)
        )
