# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\pipelines\google.py
...
class _GoogleCredentialManager:
    ...
    async def _get_service_account_token(self, scopes: Sequence[str]) -> str:
        if not self._service_account_path:
            raise RuntimeError(
                "Service account path not configured for Google Cloud requests"
            )
        ...
        async with self._lock:
            ...
            if (
                cached_token
                and expiry
                and expiry - timedelta(minutes=5) > datetime.utcnow()
            ):
                return cached_token

            loop = asyncio.get_running_loop()
            token, new_expiry = await loop.run_in_executor(
                None, self._refresh_token_sync, scope_key
            )
...
class GoogleSTTAdapter(STTComponent):
    ...
    async def transcribe(
        ...
        config_payload["sampleRateHertz"] = (
            target_rate  # Ensure overrides can't remove sample rate.
        )
    ...
    def _compose_options(
        ...
        return {
            ...
            "language_code": merged.get(
                "language_code", self._provider_defaults.stt_language_code
            ),
            "model": merged.get("model"),
            "sample_rate_hz": merged.get("sample_rate_hz"),
            "timeout_sec": float(merged.get("timeout_sec", 12.0)),
            "config_overrides": dict(
                merged.get("config_overrides") or merged.get("config") or {}
            ),
            "request_overrides": dict(
                merged.get("request_overrides") or merged.get("request") or {}
            ),
        }
...
class GoogleLLMAdapter(LLMComponent):
    ...
    def __init__(
        ...
        *,
        ...
        self._session: Optional[aiohttp.ClientSession] = None
        self._credential_manager = _GoogleCredentialManager(provider_config)
        self._auth_scopes = tuple(
            auth_scopes or (_GENERATIVE_SCOPE, _GOOGLE_DEFAULT_SCOPE)
        )
    ...
    def _compose_options(
        ...
        try:
            sys_instr = (
                merged.get("system_instruction") or merged.get("system_prompt") or ""
            ).strip()
        except Exception:
            sys_instr = ""
        if not sys_instr:
    ...
    def _build_payload(
        ...
        if not contents:
            ...
            system_instruction = merged.get("system_instruction") or context.get(
                "system_prompt"
            )
...
class GoogleTTSAdapter(TTSComponent):
    ...
    async def synthesize(
        ...
        chunk_ms = int(
            chunk_value if chunk_value is not None else self._default_chunk_ms
        )
    ...
    def _compose_options(
        ...
        return {
            ...
            "audio_encoding": (
                merged.get("audio_encoding")
                or source_format.get("encoding")
                or self._provider_defaults.tts_audio_encoding
            ).upper(),
