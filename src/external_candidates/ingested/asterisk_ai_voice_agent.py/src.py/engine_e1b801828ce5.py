# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\engine.py
import asyncio
import audioop
import base64
import logging
...
from .core import ConversationCoordinator, PlaybackManager, SessionStore
...
class Engine:
    ...
    def __init__(self, config: AppConfig):
        ...
        if hasattr(config, "streaming") and config.streaming:
            ...
            try:
                if getattr(config, "audiosocket", None) and getattr(
                    config.audiosocket, "format", None
                ):
                    audiosocket_fmt = str(config.audiosocket.format).lower()
            except Exception:
                audiosocket_fmt = "ulaw"
            streaming_sample_rate = int(
                getattr(config.streaming, "sample_rate", 8000) or 8000
            )
            # For PCM transport over AudioSocket, prefer 16 kHz by default unless explicitly set
            if self._canonicalize_encoding(audiosocket_fmt) in {
                "slin16",
                "linear16",
                "pcm16",
            }:
            ...
            streaming_config = {
                ...
                "egress_swap_mode": getattr(
                    config.streaming, "egress_swap_mode", "auto"
                ),
                ...
                "continuous_stream": bool(
                    getattr(config.streaming, "continuous_stream", True)
                ),
                ...
                "normalizer": {
                    "enabled": (
                        bool(
                            getattr(
                                getattr(config, "streaming", {}), "normalizer", {}
                            ).get("enabled", True)
                        )
                ...
                "diag_enable_taps": bool(
                    getattr(config.streaming, "diag_enable_taps", False)
                ),
                "diag_pre_secs": int(
                    getattr(config.streaming, "diag_pre_secs", 0) or 0
                ),
                "diag_post_secs": int(
                    getattr(config.streaming, "diag_post_secs", 0) or 0
                ),
                "diag_out_dir": str(
                    getattr(config.streaming, "diag_out_dir", "") or ""
                ),
            }
        # Debug/diagnostics: allow broadcasting outbound frames to all AudioSocket conns
        try:
            streaming_config["audiosocket_broadcast_debug"] = bool(
                int(os.getenv("AUDIOSOCKET_BROADCAST_DEBUG", "0"))
            )
        except Exception:
        ...
        keep_captures = os.getenv("DIAG_ENABLE_TAPS", "false").lower() in (
            "true",
            "1",
            "yes",
        )
        self.audio_capture = AudioCaptureManager(
            base_dir=capture_dir, keep_files=keep_captures
        )
        logger.info(
        ...
        try:
            initial_as_fmt = None
            if getattr(self.config, "audiosocket", None) and hasattr(
        ...
        try:
            ...
            if lh:
                logger.info(
                    "DEBUG: local_hybrid pipeline config",
                    tools=lh.tools,
                    raw_entry=str(lh),
                )
            else:
        ...
        self.local_channels: Dict[str, str] = (
            {}
        )  # channel_id -> legacy local_channel_id
        self.audiosocket_channels: Dict[str, str] = (
            {}
        )  # call_id -> audiosocket_channel_id
        ...
        try:
            ...
            use_provider_vad = (
                bool(getattr(vad_cfg, "use_provider_vad", False)) if vad_cfg else False
            )
            if use_provider_vad:
                ...
                self.vad_manager = EnhancedVADManager(
                    ...
                    confidence_threshold=float(
                        getattr(vad_cfg, "confidence_threshold", 0.6)
                    ),
                    adaptive_threshold_enabled=bool(
                        getattr(vad_cfg, "adaptive_threshold_enabled", False)
                    ),
                    noise_adaptation_rate=float(
                        getattr(vad_cfg, "noise_adaptation_rate", 0.1)
                    ),
                    webrtc_aggressiveness=int(
                        getattr(vad_cfg, "webrtc_aggressiveness", 1)
                    ),
                    min_speech_frames=int(getattr(vad_cfg, "webrtc_start_frames", 2)),
                    max_silence_frames=int(
                        getattr(vad_cfg, "webrtc_end_silence_frames", 15)
                    ),
                )
                logger.info(
                ...
                if WEBRTC_VAD_AVAILABLE:
                    try:
                        ...
                        logger.info(
                            "🎤 WebRTC VAD initialized", aggressiveness=aggressiveness
                        )
                    except Exception as e:
                        logger.warning(
                            "🎤 WebRTC VAD initialization failed", error=str(e)
                        )
                        self.webrtc_vad = None
        ...
        try:
            ...
            if self.vad_manager:
                ...
                self.audio_gating_manager = AudioGatingManager(
                    vad_manager=self.vad_manager
                )
                logger.info(
                    "🎛️ Audio gating manager initialized (OpenAI echo prevention)"
                )
            else:
        ...
        self.pending_local_channels: Dict[str, str] = (
            {}
        )  # local_channel_id -> caller_channel_id
        self.pending_audiosocket_channels: Dict[str, str] = (
            {}
        )  # audiosocket_channel_id -> caller_channel_id
    ...
    async def on_rtp_packet(self, packet: bytes, addr: tuple):
        ...
        logger.warning(
            "🚨 LEGACY RTP BYPASS - This method bypasses VAD and fragments STT",
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info(
                "✅ Tool calling system initialized",
                tool_count=len(tool_registry.list_tools()),
            )
        except Exception as e:
            logger.warning(
                f"Failed to initialize tool calling system: {e}", exc_info=True
            )
        ...
        if self.config.audio_transport == "audiosocket":
            try:
                ...
                as_format = None
                try:
                    if self.config.audiosocket and hasattr(
                        self.config.audiosocket, "format"
                    ):
                ...
                logger.error(
                    "Failed to start AudioSocket transport",
                    error=str(exc),
                    exc_info=True,
                )
        ...
        if self.config.audio_transport == "externalmedia":
            try:
                ...
                rtp_port = int(
                    getattr(self.config.external_media, "rtp_port", 0) or 18080
                )
                ...
                port_range = self._parse_port_range(
                    getattr(self.config.external_media, "port_range", None), rtp_port
                )
                ...
                logger.info(
                    "RTP server started for ExternalMedia transport",
                    host=rtp_host,
                    port=rtp_port,
                    codec=codec,
                    format=format,
                ...
                try:
                    for prov_name, provider in self.providers.items():
                        if hasattr(provider, "config"):
                            ...
                            provider_input_rate = getattr(
                                cfg, "provider_input_sample_rate_hz", None
                            ) or getattr(cfg, "input_sample_rate_hz", None)
                            if (
                                provider_input_rate
                                and provider_input_rate != sample_rate
                            ):
                                logger.warning(
                ...
                logger.error(
                    "Failed to start ExternalMedia RTP transport",
                    error=str(exc),
                    exc_info=True,
                )
        ...
        self.ari_client.add_event_handler(
    ...
    def _parse_port_range(
        self, value: Optional[Any], fallback_port: int
    ) -> Tuple[int, int]:
    ...
    async def _load_providers(self):
        ...
        logger.info(
            "Loading AI providers...", provider_names=list(self.config.providers.keys())
        )
        for name, provider_config_data in self.config.providers.items():
            if isinstance(provider_config_data, dict) and not provider_config_data.get(
                "enabled", True
            ):
                logger.info(
                    "Provider '%s' disabled in configuration; skipping initialization.",
                    name,
                )
                continue
            try:
                ...
                if name == "local":
                    ...
                    try:
                        if hasattr(provider, "set_initial_greeting"):
                            provider.set_initial_greeting(
                                getattr(self.config.llm, "initial_greeting", None)
                            )
                    except Exception:
                        logger.debug(
                            "Failed to set initial greeting on LocalProvider",
                            exc_info=True,
                        )
                    ...
                    if runtime_issues:
                        self.provider_alignment_issues.setdefault(name, []).extend(
                            runtime_issues
                        )
                elif name == "deepgram":
                    ...
                    if not self.config.llm.api_key:
                        logger.error(
                            "Deepgram provider requires OpenAI API key in LLM config"
                        )
                        continue

                    provider = DeepgramProvider(
                        deepgram_config, self.config.llm, self.on_provider_event
                    )
                    self.providers[name] = provider
                    logger.info(
                        "Provider 'deepgram' loaded successfully with OpenAI LLM dependency."
                    )
                    ...
                    openai_cfg = self._build_openai_realtime_config(
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to build GoogleProviderConfig for google_live: {e}",
                            exc_info=True,
                        )
                        continue
        ...
        if self.config.default_provider != "deepgram":
            ...
            logger.info(
                f"Default provider '{self.config.default_provider}' is available and ready."
            )
            ...
            for provider_name, provider in self.providers.items():
                ...
                try:
                    ready = (
                        provider.is_ready() if hasattr(provider, "is_ready") else True
                    )
                    if not ready:
                        logger.error(
                            "Provider NOT ready",
    ...
    def _is_caller_channel(self, channel: dict) -> bool:
        ...
        return any(
            ...
            for prefix in ["SIP/", "PJSIP/", "DAHDI/", "IAX2/"]
        )
    ...
    async def _handle_stasis_start(self, event: dict):
        ...
        if self._is_caller_channel(channel):
            ...
            logger.info(
                "🎯 HYBRID ARI - Processing caller channel", channel_id=channel_id
            )
            ...
            logger.info(
                "🎯 EXTERNAL MEDIA - ExternalMedia channel entered Stasis",
                channel_id=channel_id,
                channel_name=channel_name,
            )
            ...
            logger.warning(
                "🎯 HYBRID ARI - Unknown channel type in StasisStart",
                channel_id=channel_id,
                channel_name=channel_name,
            )

    async def _start_external_media_channel(
        self, caller_channel_id: str
    ) -> Optional[str]:
        """Allocate RTP resources and originate the ExternalMedia channel via ARI."""
        if not self.config.external_media:
            logger.error(
                "🎯 EXTERNAL MEDIA - Configuration missing; cannot start ExternalMedia channel",
                caller_channel_id=caller_channel_id,
            )
            return None
        if not self.rtp_server:
            logger.error(
                "🎯 EXTERNAL MEDIA - RTP server unavailable; cannot start ExternalMedia channel",
                caller_channel_id=caller_channel_id,
            )
            return None
        ...
        try:
            ...
            logger.error(
                "🎯 EXTERNAL MEDIA - RTP session allocation failed",
                caller_channel_id=caller_channel_id,
                error=str(exc),
                exc_info=True,
            )
        ...
        try:
            ...
            logger.error(
                "🎯 EXTERNAL MEDIA - ARI create_external_media_channel failed",
                caller_channel_id=caller_channel_id,
                external_host=external_host,
                error=str(exc),
                exc_info=True,
            ...
            try:
                ...
                logger.debug(
                    "Failed to reset session after ARI external media failure",
                    caller_channel_id=caller_channel_id,
                    exc_info=True,
                )
            return None
        ...
        if not channel_id:
            logger.error(
                "🎯 EXTERNAL MEDIA - ARI create_external_media_channel returned no channel id",
                caller_channel_id=caller_channel_id,
                response=response,
            )
            await self.rtp_server.cleanup_session(caller_channel_id)
            try:
                ...
                logger.debug(
                    "Failed to reset session after missing ExternalMedia channel id",
                    caller_channel_id=caller_channel_id,
                    exc_info=True,
                )
            return None
        ...
        logger.info(
            "🎯 EXTERNAL MEDIA - ExternalMedia channel originated",
            caller_channel_id=caller_channel_id,
    ...
    async def _handle_external_media_stasis_start(
        self, external_media_id: str, channel: dict
    ):
        ...
        try:
            ...
            if not session:
                logger.warning(
                    "ExternalMedia channel entered Stasis but no caller found",
            ...
            if bridge_id:
                success = await self.ari_client.add_channel_to_bridge(
                    bridge_id, external_media_id
                )
                if success:
                    ...
                    await self._save_session(session)
                    logger.info(
                        "🎯 EXTERNAL MEDIA - ExternalMedia channel added to bridge",
                        external_media_id=external_media_id,
                        bridge_id=bridge_id,
                        caller_channel_id=caller_channel_id,
                    )
                    ...
                    logger.error(
                        "🎯 EXTERNAL MEDIA - Failed to add ExternalMedia channel to bridge",
                        external_media_id=external_media_id,
                        bridge_id=bridge_id,
                    )
            else:
                logger.error(
                    "ExternalMedia channel entered Stasis but no bridge found",
                    external_media_id=external_media_id,
                    caller_channel_id=caller_channel_id,
                )
            ...
            logger.error(
                "Error handling ExternalMedia StasisStart",
                external_media_id=external_media_id,
                error=str(e),
                exc_info=True,
            )

    async def _handle_caller_stasis_start_hybrid(
        self, caller_channel_id: str, channel: dict
    ):
        ...
        if existing_session:
            logger.warning(
                "🎯 HYBRID ARI - Caller already in progress",
                channel_id=caller_channel_id,
            )
            return
        ...
        try:
            # Answer the caller
            logger.info(
                "🎯 HYBRID ARI - Step 1: Answering caller channel",
                channel_id=caller_channel_id,
            )
            await self.ari_client.answer_channel(caller_channel_id)
            logger.info(
                "🎯 HYBRID ARI - Step 1: ✅ Caller channel answered",
                channel_id=caller_channel_id,
            )

            # Create bridge immediately (use default bridge_type to prevent simple_bridge optimization)
            logger.info(
                "🎯 HYBRID ARI - Step 2: Creating bridge immediately",
                channel_id=caller_channel_id,
            )
            bridge_id = (
                await self.ari_client.create_bridge()
            )  # Uses default: mixing,dtmf_events,proxy_media
            if not bridge_id:
                raise RuntimeError("Failed to create mixing bridge")
            logger.info(
                "🎯 HYBRID ARI - Step 2: ✅ Bridge created",
                channel_id=caller_channel_id,
                bridge_id=bridge_id,
            )
            ...
            logger.info(
                "🎯 HYBRID ARI - Step 3: Adding caller to bridge",
                channel_id=caller_channel_id,
                bridge_id=bridge_id,
            )
            ...
            logger.info(
                "🎯 HYBRID ARI - Step 3: ✅ Caller added to bridge",
                channel_id=caller_channel_id,
                bridge_id=bridge_id,
            )
            ...
            try:
                ...
                logger.debug(
                    "Failed to export config metrics for call",
                    call_id=caller_channel_id,
                    exc_info=True,
                )
            logger.info(
                "🎯 HYBRID ARI - Step 4: ✅ Caller session created and stored",
                channel_id=caller_channel_id,
                bridge_id=bridge_id,
            )
            ...
            try:
                ...
                logger.debug(
                    "Transport profile hydration failed",
                    call_id=caller_channel_id,
                    exc_info=True,
                )
            ...
            if resolved_provider and resolved_provider in self.providers:
                ...
                if pipeline_resolution:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Failed to start pipeline runner",
                            call_id=caller_channel_id,
                            exc_info=True,
                        )
                ...
                if session.provider_name and session.provider_name in self.providers:
                    ...
                    if not pipeline_resolution and getattr(
                        self.pipeline_orchestrator, "started", False
                    ):
                        logger.info(
            ...
            if self.config.audio_transport == "externalmedia":
                logger.info(
                    "🎯 EXTERNAL MEDIA - Step 5: Creating ExternalMedia channel",
                    channel_id=caller_channel_id,
                )
                external_media_id = await self._start_external_media_channel(
                    caller_channel_id
                )
                if external_media_id:
                    ...
                    await self._save_session(session)
                    logger.info(
                        "🎯 EXTERNAL MEDIA - ExternalMedia channel created, session updated",
                        channel_id=caller_channel_id,
                        external_media_id=external_media_id,
                    )
                else:
                    logger.error(
                        "🎯 EXTERNAL MEDIA - Failed to create ExternalMedia channel",
                        channel_id=caller_channel_id,
                    )
            else:
                logger.info(
                    "🎯 HYBRID ARI - Step 5: Originating AudioSocket channel",
                    channel_id=caller_channel_id,
                )
            ...
            logger.error(
                "🎯 HYBRID ARI - Failed to handle caller StasisStart",
                caller_channel_id=caller_channel_id,
                error=str(e),
                exc_info=True,
            )
    ...
    async def _handle_local_stasis_start_hybrid(
        self, local_channel_id: str, channel: dict
    ):
        ...
        if not session:
            logger.error(
                "🎯 HYBRID ARI - Caller channel not found for Local channel",
                local_channel_id=local_channel_id,
                caller_channel_id=caller_channel_id,
            )
            await self.ari_client.hangup_channel(local_channel_id)
        ...
        try:
            ...
            local_success = await self.ari_client.add_channel_to_bridge(
                bridge_id, local_channel_id
            )
            if local_success:
                logger.info(
                ...
                logger.error(
                    "🎯 HYBRID ARI - Failed to add Local channel to bridge",
                    local_channel_id=local_channel_id,
                    bridge_id=bridge_id,
                )
                await self.ari_client.hangup_channel(local_channel_id)
            ...
            logger.error(
                "🎯 HYBRID ARI - Failed to handle Local channel StasisStart",
                local_channel_id=local_channel_id,
                error=str(e),
                exc_info=True,
            )
    ...
    async def _handle_audiosocket_channel_stasis_start(
        self, audiosocket_channel_id: str, channel: dict
    ):
        ...
        caller_channel_id = self.pending_audiosocket_channels.pop(
            audiosocket_channel_id, None
        )
        if not caller_channel_id:
            ...
            if not caller_channel_id:
                ...
                for s in sessions:
                    if (
                        getattr(s, "audiosocket_channel_id", None)
        ...
        try:
            added = await self.ari_client.add_channel_to_bridge(
                bridge_id, audiosocket_channel_id
            )
            if not added:
            ...
            try:
                diag_enabled = (
                    bool(getattr(self.config.streaming, "diag_enable_taps", False))
                    if hasattr(self.config, "streaming")
                    else False
                )
            except Exception:
                pass
            ...
            if diag_enabled:
                try:
                    ...
                    logger.debug(
                        "ARI channel recording start failed (diagnostic recording)",
                        exc_info=True,
                    )
            else:
                logger.debug(
    ...
    async def _handle_agent_action_stasis(
        self, channel_id: str, channel: dict, args: list
    ):
        """
        ...
        if len(args) < 2:
            logger.error(
                "🔀 AGENT ACTION - Insufficient args", channel_id=channel_id, args=args
            )
        ...
        logger.info(
            "🔀 AGENT ACTION - Processing action",
            action_type=action_type,
            caller_id=caller_id,
            channel_id=channel_id,
        )
        ...
        if handler:
            ...
            logger.warning(
                f"🔀 AGENT ACTION - Unknown action type: {action_type}",
                channel_id=channel_id,
                args=args,
            )
            await self.ari_client.hangup_channel(channel_id)
    ...
    async def _handle_transfer_answered(self, channel_id: str, args: list):
        ...
        logger.info(
            "🔀 TRANSFER ANSWERED - Direct SIP channel",
            action_type=action_type,
            channel_id=channel_id,
            caller_id=caller_id,
            target=target,
        ...
        if not session:
            logger.error("🔀 TRANSFER - Session not found", caller_id=caller_id)
            await self.ari_client.hangup_channel(channel_id)
            return
        ...
        if session.external_media_id:
            try:
                await self.ari_client.remove_channel_from_bridge(
                    session.bridge_id, session.external_media_id
                )
                logger.info(
                    "✅ UnicastRTP removed from bridge",
        ...
        if provider:
            try:
                ...
                if hasattr(provider, "stop_session"):
                    ...
                    logger.info(
                        "✅ AI provider session stopped", provider=session.provider_name
                    )
            except Exception as e:
        ...
        try:
            await self.ari_client.add_channel_to_bridge(
                session.bridge_id, channel_id  # This is SIP/6000 directly
            )
            logger.info(
                "✅ TRANSFER COMPLETE - Direct SIP channel bridged",
                channel_id=channel_id,
                bridge_id=session.bridge_id,
                target=target,
            )
            ...
            logger.error(f"🔀 TRANSFER - Failed to bridge: {e}", channel_id=channel_id)
            await self.ari_client.hangup_channel(channel_id)
    ...
    async def _originate_audiosocket_channel_hybrid(self, caller_channel_id: str):
        ...
        try:
            response = await self.ari_client.send_command(
                "POST", "channels", params=orig_params
            )
            if response and response.get("id"):
                audiosocket_channel_id = response["id"]
                self.pending_audiosocket_channels[audiosocket_channel_id] = (
                    caller_channel_id
                )
    ...
    async def _handle_channel_destroyed(self, event: dict):
        ...
        try:
            ...
            logger.error(
                "Error handling ChannelDestroyed", error=str(exc), exc_info=True
            )
    ...
    async def _handle_dtmf_received(self, event: dict):
        ...
        try:
            ...
            logger.error(
                "Error handling ChannelDtmfReceived", error=str(exc), exc_info=True
            )
    ...
    async def _cleanup_call(self, channel_or_call_id: str) -> None:
        ...
        try:
            ...
            if not session:
                logger.debug(
                    "No session found during cleanup", identifier=channel_or_call_id
                )
                return
            ...
            try:
                ...
                if call_id in _call_start_times:
                    ...
                    _CALL_DURATION.labels(
                        call_id=call_id, pipeline=pipeline_name, provider=provider_name
                    ).observe(duration)
                ...
                logger.debug(
                    "Failed to record call duration", call_id=call_id, error=str(e)
                )
            ...
            try:
                ...
                logger.debug(
                    "Streaming playback stop failed during cleanup",
                    call_id=call_id,
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.debug(
                    "Provider stop_session failed during cleanup",
                    call_id=call_id,
                    exc_info=True,
                )
            ...
            if bridge_id:
                try:
                    ...
                    logger.info(
                        "Bridge destroyed", call_id=call_id, bridge_id=bridge_id
                    )
                except Exception:
                    logger.debug(
            ...
            if not transfer_active:
                ...
                logger.info(
                    "Skipping caller hangup - transferred to dialplan",
                    call_id=call_id,
                    transfer_target=getattr(session, "transfer_target", "unknown"),
                )
            ...
            if getattr(self, "rtp_server", None):
                try:
                    ...
                    logger.debug(
                        "RTP session cleanup failed during call cleanup",
                        call_id=call_id,
                        exc_info=True,
                    )
            ...
            if session.audiosocket_channel_id:
                self.pending_audiosocket_channels.pop(
            ...
            try:
                to_delete = [
                    ssrc for ssrc, cid in self.ssrc_to_caller.items() if cid == call_id
                ]
            ...
            if (
                getattr(self, "pipeline_orchestrator", None)
                and self.pipeline_orchestrator.enabled
            ):
                try:
                    await self.pipeline_orchestrator.release_pipeline(call_id)
                except Exception:
                    logger.debug(
                        "Milestone7 pipeline release failed during cleanup",
                        call_id=call_id,
                        exc_info=True,
                    )
            ...
            try:
                ...
                if email_tool_config.get("enabled", False):
                    ...
                    if email_tool:
                        ...
                        if not check_session:
                            ...
                            logger.info(
                                "📧 Auto-triggered email summary", call_id=call_id
                            )
                ...
                if "Session not found" in str(e):
                    ...
                    logger.warning(
                        "Failed to auto-trigger email summary",
                        call_id=call_id,
                        error=str(e),
                    )
            except Exception as e:
            ...
            try:
                if hasattr(session, "transcript_emails") and session.transcript_emails:
                    transcript_tool_config = self.config.tools.get(
                        "request_transcript", {}
                    )
                            ...
                            for email_address in session.transcript_emails:
                                try:
                                    ...
                                    if current_session:
                                        ...
                                        email_data = (
                                            transcript_tool._prepare_email_data(
                                                email_address,
                                                current_session,
                                        ...
                                        asyncio.create_task(
                                            transcript_tool._send_transcript_async(
                                                email_data, call_id
                                            )
                                        )
                                        logger.info(
                ...
                logger.warning(
                    "Failed to process transcript emails",
                    call_id=call_id,
                    error=str(e),
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.debug(
                    "Audio capture cleanup failed", call_id=call_id, exc_info=True
                )
            ...
            if self.vad_manager:
                try:
                    ...
                    logger.debug(
                        "VAD cleanup failed during call cleanup",
                        call_id=call_id,
                        exc_info=True,
                    )
            ...
            if self.audio_gating_manager:
                try:
                    ...
                    logger.debug(
                        "Audio gating cleanup failed during call cleanup",
                        call_id=call_id,
                        exc_info=True,
                    )
            ...
            try:
                ...
                if not sess3:
                    sess3 = await self.session_store.get_by_channel_id(
                        channel_or_call_id
                    )
                if (
                    sess3
                    and getattr(sess3, "cleanup_in_progress", False)
                    and not getattr(sess3, "cleanup_completed", False)
                ):
    ...
    async def _resolve_audio_profile(
        ...
        profile_obj = (
            profile_map.get(selected_profile_name) if selected_profile_name else None
        )
        ...
        transport_out = (
            (profile_obj or {}).get("transport_out", {})
            if isinstance(profile_obj, dict)
            else {}
        )
        prov_pref = (
            (profile_obj or {}).get("provider_pref", {})
            if isinstance(profile_obj, dict)
            else {}
        )
        chunk_ms = None
        ...
        enc = (
            self._canonicalize_encoding(transport_out.get("encoding"))
            or session.transport_profile.format
        )
        try:
        ...
        try:
            await self._update_transport_profile(
                session, fmt=enc, sample_rate=rate, source="config"
            )
        except Exception:
            logger.debug(
                "Transport profile update from profile failed",
                call_id=call_id,
                exc_info=True,
            )
        ...
        try:
            ...
            if ctx_provider:
                ...
                if resolved in self.providers and session.provider_name != resolved:
                    ...
                    logger.info(
                        "Context provider override applied",
                        call_id=call_id,
                        context=ai_context,
            ...
            logger.debug(
                "Context provider override failed", call_id=call_id, exc_info=True
            )
        ...
        try:
            self._emit_profile_resolution_card(
                ...
                or ("profile" if ai_profile else ("context" if ai_context else None)),
            )
        except Exception:
            logger.debug(
                "Audio Profile Resolution card logging failed",
                call_id=call_id,
                exc_info=True,
            )
    ...
    def _emit_profile_resolution_card(
        ...
        *,
        ...
        payload = {
            ...
            "provider_origin": provider_origin,
            "transport_encoding": self._canonicalize_encoding(transport_encoding)
            or None,
        ...
        try:
            ...
            logger.debug(
                "Profile resolution card logging failed", call_id=call_id, exc_info=True
            )
    ...
    async def _audiosocket_handle_uuid(self, conn_id: str, uuid_str: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Error binding AudioSocket UUID",
                conn_id=conn_id,
                uuid=uuid_str,
                error=str(exc),
                exc_info=True,
    ...
    async def _audiosocket_handle_audio(self, conn_id: str, audio_bytes: bytes) -> None:
        ...
        try:
            ...
            if not caller_channel_id:
                logger.debug(
                    "AudioSocket audio received for unknown connection",
                    conn_id=conn_id,
            ...
            self._audiosocket_frame_count[caller_channel_id] = (
                self._audiosocket_frame_count.get(caller_channel_id, 0) + 1
            )
            ...
            if not session:
                logger.debug(
                    "No session for caller; dropping AudioSocket audio",
                    conn_id=conn_id,
                    caller_channel_id=caller_channel_id,
                )
                return
            ...
            if "inbound_first_frame" not in diagnostics_flags:
                ...
                await self._update_transport_profile(
                    session, fmt=fmt, sample_rate=rate, source="audiosocket"
                )
            ...
            if not vad_state.get("format_probe_done"):
                try:
                    try:
                        as_fmt = (
                            getattr(self.config, "audiosocket", None).format or "ulaw"
                        ).lower()
                    except Exception:
                        as_fmt = "ulaw"
                    if as_fmt in ("slin16", "linear16", "pcm16"):
                        ...
                        try:
                            ...
                            swap_flag = (
                                frame_bytes >= 640  # 20ms @ 16k PCM
                                and rms_swapped >= 2048
                                and rms_swapped >= 16 * max(1, rms_native)
                            )
            ...
            try:
                swap_needed_flag = bool(
                    session.vad_state.get("pcm16_inbound_swap", False)
                )
            except Exception:
            ...
            try:
                ...
                if self.config.audio_transport == "audiosocket":
                    ...
                    if not profile_rate:
                        ...
                        if canonical_fmt == "slin":
                            ...
                            profile_rate = getattr(
                                self.config.streaming, "sample_rate", 8000
                            )
                else:
                    # For RTP: use transport profile (negotiated codec)
                ...
                if self.config.audio_transport == "audiosocket":
                    ...
                    profile_fmt = "ulaw"
                    profile_rate = 8000
            pcm_bytes, pcm_rate = self._wire_to_pcm16(
                audio_bytes, profile_fmt, swap_needed_flag, profile_rate
            )
            ...
            try:
                ...
                logger.debug(
                    "Inbound DC conditioning failed",
                    call_id=caller_channel_id,
                    exc_info=True,
                )
            try:
                if pcm_bytes:
                    self._update_audio_diagnostics(
                        session, "transport_in", pcm_bytes, "slin16", pcm_rate
                    )
                    self.audio_capture.append_pcm16(
                        session.call_id, "caller_inbound", pcm_bytes, pcm_rate
                    )
            except Exception:
                logger.debug(
                    "Inbound diagnostics update failed",
                    call_id=caller_channel_id,
                    exc_info=True,
                )
                ...
                if q:
                    try:
                        ...
                        if pcm16 and pcm_rate != 16000:
                            try:
                                state = self._resample_state_pipeline16k.get(
                                    caller_channel_id
                                )
                                pcm16, state = audioop.ratecv(
                                    pcm16, 2, 1, pcm_rate, 16000, state
                                )
                                self._resample_state_pipeline16k[caller_channel_id] = (
                                    state
                                )
                            except Exception:
                                pcm16 = pcm_bytes
                        ...
                        logger.debug(
                            "Pipeline queue full; dropping AudioSocket frame",
                            call_id=caller_channel_id,
                        )
                        return
                ...
                if capabilities and capabilities.requires_continuous_audio:
                    ...
                    if isinstance(pcfg, dict):
                        ...
                        continuous_input = bool(
                            getattr(pcfg, "continuous_input", False)
                        )
            except Exception:
                continuous_input = False
            ...
            if continuous_input and provider and hasattr(provider, "send_audio"):
                ...
                try:
                    self._update_audio_diagnostics(
                        session, "provider_in", pcm_bytes, "slin16", pcm_rate
                    )
                except Exception:
                    logger.debug(
                        "Provider input diagnostics update failed (unconditional)",
                        call_id=caller_channel_id,
                        exc_info=True,
                    )
                try:
                ...
                logger.info(
                    ...
                    has_send_audio=(
                        hasattr(provider, "send_audio") if provider else False
                    ),
            ...
            try:
                ...
                post_guard_ms = (
                    int(getattr(cfg, "post_tts_end_protection_ms", 0)) if cfg else 0
                )
            except Exception:
                post_guard_ms = 0
            if (
                post_guard_ms
                and getattr(session, "tts_ended_ts", 0.0)
            ...
            if (
                hasattr(session, "audio_capture_enabled")
                and not session.audio_capture_enabled
            ):
                cfg = getattr(self.config, "barge_in", None)
                ...
                if continuous_input and provider and hasattr(provider, "send_audio"):
                    try:
                        ...
                        logger.debug(
                            "Provider input diagnostics update failed (continuous-input)",
                            call_id=caller_channel_id,
                            exc_info=True,
                        )
                    try:
                        ...
                        try:
                            ...
                            logger.debug(
                                "Provider input capture failed (continuous-input)",
                                call_id=session.call_id,
                                exc_info=True,
                            )
                        ...
                        logger.debug(
                            "Provider continuous-input forward error",
                            call_id=caller_channel_id,
                            exc_info=True,
                        )
                    return
                ...
                try:
                    ...
                    tts_elapsed_ms = 0
                initial_protect = (
                    int(getattr(cfg, "initial_protection_ms", 200)) if cfg else 200
                )
                ...
                try:
                    if (
                        ...
                        and getattr(session, "tts_started_ts", 0.0) > 0.0
                    ):
                        initial_protect = (
                            5000  # 5 seconds to prevent echo feedback loop
                        )
                        logger.debug(
                ...
                try:
                    if (
                        getattr(session, "conversation_state", None) == "greeting"
                        and cfg
                    ):
                ...
                if tts_elapsed_ms < initial_protect:
                    logger.debug(
                        "Dropping inbound during initial TTS protection window",
                        conn_id=conn_id,
                        caller_channel_id=caller_channel_id,
                ...
                if not cfg or not getattr(cfg, "enabled", True):
                    logger.debug(
                        "Dropping inbound AudioSocket audio during TTS playback (barge-in disabled)",
                        conn_id=conn_id,
                        caller_channel_id=caller_channel_id,
                        bytes=len(audio_bytes),
                    )
                ...
                if energy >= threshold:
                    criteria_met += 1
                if vad_result and confidence >= getattr(
                    self.vad_manager, "confidence_threshold", 0.6
                ):
                    criteria_met += 1
                if webrtc_positive:
                    criteria_met += 1
                ...
                if vad_result:
                    if criteria_met >= 2:
                        ...
                        session.barge_in_candidate_ms = (
                            int(getattr(session, "barge_in_candidate_ms", 0)) + frame_ms
                        )
                    else:
                ...
                last_barge_in_ts = float(
                    getattr(session, "last_barge_in_ts", 0.0) or 0.0
                )
                in_cooldown = (
                    (now - last_barge_in_ts) * 1000 < cooldown_ms
                    if last_barge_in_ts
                    else False
                )
                ...
                should_trigger = (
                    not in_cooldown and session.barge_in_candidate_ms >= min_ms
                )
                ...
                if should_trigger:
                    ...
                    try:
                        ...
                        for pid in playback_ids:
                            try:
                                ...
                                logger.debug(
                                    "Playback stop error during barge-in",
                                    playback_id=pid,
                                    exc_info=True,
                                )
                            ...
                            try:
                                if self.conversation_coordinator:
                                    await self.conversation_coordinator.on_tts_end(
                                        caller_channel_id, token, reason="barge-in"
                                    )
                            except Exception:
                                logger.debug(
                                    "Failed to clear gating token during barge-in",
                                    token=token,
                                    exc_info=True,
                                )
                        ...
                        try:
                            if (
                                float(getattr(session, "barge_start_ts", 0.0) or 0.0)
                                > 0.0
                            ):
                                reaction_s = max(
                                    0.0, now - float(session.barge_start_ts)
                                )
                                _BARGE_REACTION_SECONDS.labels(
                                    caller_channel_id
                                ).observe(reaction_s)
                                session.barge_start_ts = 0.0
                        except Exception:
                    ...
                    if energy > 0 and self.conversation_coordinator:
                        try:
                            self.conversation_coordinator.note_audio_during_tts(
                                caller_channel_id
                            )
                        except Exception:
                            pass
            ...
            if vad_result:
                ...
                if not forward_original_audio:
                    ...
                    if getattr(session, "conversation_state", None) == "greeting":
                        ...
                        silence_len = (
                            len(pcm_bytes) if pcm_bytes else len(audio_bytes) * 2
                        )
            ...
            if provider_name == "openai_realtime":
                logger.debug(
                    ...
                    call_id=caller_channel_id,
                    audio_capture_enabled=getattr(
                        session, "audio_capture_enabled", None
                    ),
            ...
            try:
                self._update_audio_diagnostics(
                    session, "provider_in", pcm_payload, "slin16", payload_rate
                )
            except Exception:
                logger.debug(
                    "Provider input diagnostics update failed",
                    call_id=caller_channel_id,
                    exc_info=True,
                )

            provider_payload, provider_encoding, provider_rate = (
                self._encode_for_provider(
                    session.call_id,
                    provider_name,
                    provider,
                    pcm_payload,
            ...
            logger.error(
                "Error handling AudioSocket audio",
                conn_id=conn_id,
                error=str(exc),
                exc_info=True,
            )
    ...
    def _should_use_vad_fallback(self, session: CallSession) -> bool:
        ...
        try:
            ...
            fallback_state = session.vad_state.setdefault(
                "fallback_state",
                {
            ...
            fallback_period_ms = (
                200  # Forward real audio every 200 ms during extended silence
            )
            ...
            logger.debug(
                "VAD fallback logic error", call_id=session.call_id, error=str(e)
            )
    ...
    async def _export_config_metrics(self, call_id: str) -> None:
        ...
        try:
            ...
            if b:
                _CFG_BARGE_MS.labels(call_id, "initial_protection_ms").set(
                    int(getattr(b, "initial_protection_ms", 0))
                )
                _CFG_BARGE_MS.labels(call_id, "min_ms").set(
                    int(getattr(b, "min_ms", 0))
                )
                _CFG_BARGE_MS.labels(call_id, "post_tts_end_protection_ms").set(
                    int(getattr(b, "post_tts_end_protection_ms", 0))
                )
                _CFG_BARGE_MS.labels(call_id, "greeting_protection_ms").set(
                    int(getattr(b, "greeting_protection_ms", 0))
                )
                _CFG_BARGE_THRESHOLD.labels(call_id).set(
                    int(getattr(b, "energy_threshold", 0))
                )
        except Exception:
            pass
        try:
            ...
            if s:
                _CFG_STREAM_MS.labels(call_id, "min_start_ms").set(
                    int(getattr(s, "min_start_ms", 0))
                )
                _CFG_STREAM_MS.labels(call_id, "greeting_min_start_ms").set(
                    int(getattr(s, "greeting_min_start_ms", 0))
                )
                _CFG_STREAM_MS.labels(call_id, "low_watermark_ms").set(
                    int(getattr(s, "low_watermark_ms", 0))
                )
                _CFG_STREAM_MS.labels(call_id, "jitter_buffer_ms").set(
                    int(getattr(s, "jitter_buffer_ms", 0))
                )
                _CFG_STREAM_MS.labels(call_id, "fallback_timeout_ms").set(
                    int(getattr(s, "fallback_timeout_ms", 0))
                )
        except Exception:
            pass
        try:
            pblock = (getattr(self.config, "providers", {}) or {}).get(
                "openai_realtime", {}
            )
            td = (pblock or {}).get("turn_detection") or {}
            if td:
                _CFG_TD_MS.labels(call_id, "silence_duration_ms").set(
                    int(td.get("silence_duration_ms", 0))
                )
                _CFG_TD_MS.labels(call_id, "prefix_padding_ms").set(
                    int(td.get("prefix_padding_ms", 0))
                )
                try:
                    _CFG_TD_THRESHOLD.labels(call_id).set(
                        float(td.get("threshold", 0.0))
                    )
                except Exception:
                    pass
        except Exception:
        ...
        try:
            ...
            if caller_channel_id:
                ...
                if self.audiosocket_primary_conn.get(caller_channel_id) == conn_id:
                    ...
                    if conns:
                        self.audiosocket_primary_conn[caller_channel_id] = next(
                            iter(conns)
                        )
                ...
                try:
                    ...
                    pass
            logger.info(
                "AudioSocket connection disconnected",
                conn_id=conn_id,
                caller_channel_id=caller_channel_id,
            )
        except Exception as exc:
            logger.error(
                "Error during AudioSocket disconnect cleanup",
                conn_id=conn_id,
                error=str(exc),
                exc_info=True,
            )
    ...
    async def _audiosocket_handle_dtmf(self, conn_id: str, digit: str) -> None:
        ...
        try:
            ...
            logger.error(
                "Error handling AudioSocket DTMF",
                conn_id=conn_id,
                error=str(exc),
                exc_info=True,
            )
    ...
    async def _on_rtp_audio(self, ssrc: int, pcm_16k: bytes) -> None:
        ...
        try:
            ...
            if not caller_channel_id:
                ...
                for s in sessions:
                    try:
                        if getattr(s, "external_media_id", None) and not getattr(
                            s, "ssrc", None
                        ):
                            if candidate is None or float(
                                getattr(s, "created_at", 0.0)
                            ) > float(getattr(candidate, "created_at", 0.0)):
                                candidate = s
                    except Exception:
                        continue
                if candidate:
                    ...
                    try:
                        ...
                        pass
                    try:
                        if getattr(self, "rtp_server", None) and hasattr(
                            self.rtp_server, "map_ssrc_to_call_id"
                        ):
            ...
            if not caller_channel_id:
                logger.debug(
                    "RTP audio received for unknown SSRC", ssrc=ssrc, bytes=len(pcm_16k)
                )
                return
            ...
            if not session:
                logger.debug(
                    "No session for caller; dropping RTP audio",
                    ssrc=ssrc,
                    caller_channel_id=caller_channel_id,
                )
                return
            ...
            provider_name = (
                getattr(session, "provider_name", None) or self.config.default_provider
            )
            provider = self.providers.get(provider_name)
            continuous_input = False
            try:
            ...
            if continuous_input:
                if not provider or not hasattr(provider, "send_audio"):
                    logger.debug(
                        "Provider unavailable for continuous RTP audio",
                        provider=provider_name,
                    )
                    return
                ...
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Provider input capture failed (continuous-input RTP)",
                            call_id=session.call_id,
                            exc_info=True,
                        )
                    ...
                    await provider.send_audio(
                        prov_payload, sample_rate=prov_rate, encoding=prov_enc
                    )
                except Exception as exc:
                    logger.debug(
                        "Continuous-input RTP forward error",
                        call_id=caller_channel_id,
                        error=str(exc),
                    )
                return
            ...
            if (
                ...
                if not cfg or not getattr(cfg, "enabled", True):
                    logger.debug(
                        "Dropping inbound RTP during TTS playback (barge-in disabled)",
                        ssrc=ssrc,
                        caller_channel_id=caller_channel_id,
                        bytes=len(pcm_16k),
                    )
                ...
                if tts_elapsed_ms < initial_protect:
                    logger.debug(
                        "Dropping inbound RTP during initial TTS protection window",
                        ssrc=ssrc,
                        caller_channel_id=caller_channel_id,
                        tts_elapsed_ms=tts_elapsed_ms,
                        protect_ms=initial_protect,
                ...
                if not in_cooldown and session.barge_in_candidate_ms >= min_ms:
                    try:
                        ...
                        for pid in playback_ids:
                            try:
                                ...
                                logger.debug(
                                    "Playback stop error during RTP barge-in",
                                    playback_id=pid,
                                    exc_info=True,
                                )
                        ...
                        for token in tokens:
                            try:
                                ...
                                logger.debug(
                                    "Failed to clear gating token during RTP barge-in",
                                    token=token,
                                    exc_info=True,
                                )
                        ...
                        await self._save_session(session)
                        logger.info(
                            "🎧 BARGE-IN (RTP) triggered", call_id=caller_channel_id
                        )
                    except Exception:
                        logger.error(
                            "Error triggering RTP barge-in",
                            call_id=caller_channel_id,
                            exc_info=True,
                        )
                else:
                    ...
                    if energy > 0 and self.conversation_coordinator:
                        try:
                            ...
                            pass
                    logger.debug(
                        "Dropping inbound RTP during TTS (candidate_ms=%d, energy=%d)",
                        session.barge_in_candidate_ms,
                        energy,
                    )
                    return
            ...
            if self._pipeline_forced.get(caller_channel_id):
                ...
                if q:
                    try:
                        ...
                        logger.debug(
                            "Pipeline queue full; dropping RTP frame",
                            call_id=caller_channel_id,
                        )
                        return
            ...
            if not provider or not hasattr(provider, "send_audio"):
                logger.debug(
                    "Provider unavailable for RTP audio", provider=provider_name
                )
                return
            ...
            logger.error(
                "Error handling RTP audio", ssrc=ssrc, error=str(exc), exc_info=True
            )

    def _build_deepgram_config(
        self, provider_cfg: Dict[str, Any]
    ) -> Optional[DeepgramProviderConfig]:
        ...
        try:
            ...
            logger.error(
                "Failed to build DeepgramProviderConfig", error=str(exc), exc_info=True
            )
            return None

    def _build_openai_realtime_config(
        self, provider_cfg: Dict[str, Any]
    ) -> Optional[OpenAIRealtimeProviderConfig]:
        ...
        try:
            ...
            if not cfg.enabled:
                logger.info(
                    "OpenAI Realtime provider disabled in configuration; skipping initialization."
                )
                return None
            if not cfg.api_key:
                logger.error(
                    "OpenAI Realtime provider API key missing (OPENAI_API_KEY)"
                )
                return None
            return cfg
        except Exception as exc:
            logger.error(
                "Failed to build OpenAIRealtimeProviderConfig",
                error=str(exc),
                exc_info=True,
            )
            return None

    def _audit_provider_config(
        self, name: str, provider_cfg: Dict[str, Any]
    ) -> List[str]:
        ...
        try:
            ...
            try:
                if getattr(self.config, "audiosocket", None):
                    audiosocket_format = (
                        self.config.audiosocket.format or "ulaw"
                    ).lower()
            except Exception:
                audiosocket_format = "ulaw"
            ...
            if name == "deepgram":
                ...
                if enc_canon in {
                    ...
                    "linear16",
                    "pcm16",
                } and audiosocket_canon not in {"slin", "slin16"}:
                    issues.append(
                ...
                if enc_canon in {
                    ...
                    "g711_ulaw",
                    "mu-law",
                } and audiosocket_canon not in {"ulaw", "mulaw"}:
            ...
            if name == "openai_realtime":
                provider_rate = int(
                    provider_cfg.get("provider_input_sample_rate_hz") or 0
                )
                ...
                target_encoding = (
                    provider_cfg.get("target_encoding") or "ulaw"
                ).lower()
                ...
                if target_encoding in (
                    ...
                    "linear16",
                    "pcm16",
                ) and audiosocket_format not in ("slin", "slin16", "linear16", "pcm16"):
                    issues.append(
                        f"OpenAI Realtime target_encoding={target_encoding} but audiosocket.format={audiosocket_format}; "
            ...
            logger.debug(
                "Provider configuration audit failed", provider=name, exc_info=True
            )
        return issues

    def _describe_provider_alignment(
        self, name: str, provider: AIProviderInterface
    ) -> List[str]:
        issues: List[str] = []
        try:
            audiosocket_format = "ulaw"
            ...
            streaming_encoding = getattr(
                self.streaming_playback_manager, "audiosocket_format", None
            )
            ...
            streaming_canon = (
                self._canonicalize_encoding(streaming_encoding) or audiosocket_canon
            )

            try:
                streaming_rate = int(
                    getattr(self.streaming_playback_manager, "sample_rate", 8000)
                    or 8000
                )
            except Exception:
                streaming_rate = 8000
            ...
            logger.debug(
                "Provider alignment description failed", provider=name, exc_info=True
            )
        return issues
    ...
    def _audit_transport_alignment(self) -> None:
        ...
        try:
            ...
            # Info summary
            streaming_target_fmt = (
                getattr(self.streaming_playback_manager, "audiosocket_format", None)
                or as_fmt
            ).lower()
            streaming_swap_mode = getattr(
                self.streaming_playback_manager, "egress_swap_mode", "auto"
            )
            streaming_force_mulaw = bool(
                getattr(self.streaming_playback_manager, "egress_force_mulaw", False)
            )
            ...
            summary = {
                ...
                "openai_realtime_input_encoding": _lower_str(
                    oair_cfg, "input_encoding", ""
                ),
                "openai_realtime_input_sample_rate_hz": int(
                    oair_cfg.get("input_sample_rate_hz") or 0
                ),
            ...
            if as_fmt in (
                ...
                "g711_ulaw",
                "mu-law",
            ) and oair_target_enc not in ("ulaw", "mulaw", "g711_ulaw", "mu-law"):
                logger.warning(
            ...
            if as_fmt in ("slin16", "linear16", "pcm16") and oair_target_enc not in (
                "slin16",
                "linear16",
                "pcm16",
            ):
            ...
            if dg_in_enc in ("slin16", "linear16", "pcm16") and as_fmt not in (
                "slin16",
                "linear16",
                "pcm16",
            ):
    ...
    async def on_provider_event(self, event: Dict[str, Any]):
        ...
        try:
            ...
            if not session:
                logger.warning(
                    "Provider event for unknown call", event_type=etype, call_id=call_id
                )
                return
                    ...
                    try:
                        if self.conversation_coordinator:
                            await self.conversation_coordinator.on_tts_end(
                                call_id,
                                token,
                                reason=event.get("reason") or "provider-request",
                            )
                    except Exception:
                        logger.debug(
            ...
            if etype == "ProviderAudioFormat":
                ...
                try:
                    ...
                    logger.debug(
                        "ProviderAudioFormat profile update failed",
                        call_id=call_id,
                        exc_info=True,
                    )

                logger.info(
                    "Provider audio format announced",
                    call_id=call_id,
                    encoding=encoding,
                    sample_rate=sample_rate,
                )
            ...
            if etype == "AgentAudio":
                ...
                try:
                    diag_encoding = (
                        fmt_entry.get("encoding")
                        or encoding
                        or session.transport_profile.format
                    )
                    diag_rate = int(
                        fmt_entry.get("sample_rate")
                        or sample_rate_int
                        or session.transport_profile.sample_rate
                    )
                    self._update_audio_diagnostics(
                        session, "provider_out", chunk, diag_encoding, diag_rate
                    )
                except Exception:
                    logger.debug(
                        "Provider audio diagnostics update failed",
                        call_id=call_id,
                        exc_info=True,
                    )
                try:
                    ...
                    logger.debug(
                        "Provider audio capture failed", call_id=call_id, exc_info=True
                    )
                # Log provider AgentAudio chunk metrics for RCA
                try:
                    rate = (
                        int(sample_rate_int or diag_rate or 0)
                        if (locals().get("diag_rate") is not None)
                        else int(sample_rate_int or 0)
                    )
                except Exception:
                    rate = 0
                try:
                    enc = (
                        (encoding or diag_encoding or "").lower()
                        if (locals().get("diag_encoding") is not None)
                        else (encoding or "")
                    )
                except Exception:
                    enc = encoding or ""
                ...
                try:
                    if rate and bps:
                        duration_ms = round(
                            (len(chunk) / float(bps * rate)) * 1000.0, 3
                        )
                except Exception:
                    duration_ms = 0.0
                ...
                try:
                    transport_encoding = self._canonicalize_encoding(
                        session.transport_profile.format
                    )
                except Exception:
                    transport_encoding = ""
                out_chunk = chunk
                if (
                    enc in ("linear16", "pcm16", "slin", "slin16")
                    and rate
                    and wire_rate
                    and rate != wire_rate
                ):
                    try:
                        out_chunk, _ = audioop.ratecv(
                            chunk, 2, 1, rate, wire_rate, None
                        )
                        seq = self._provider_chunk_seq.get(call_id, 0) + 1
                        self._provider_chunk_seq[call_id] = seq
                        logger.info(
                        ...
                        logger.debug(
                            "Provider chunk resample failed; passing original",
                            call_id=call_id,
                            exc_info=True,
                        )
                ...
                coalesce_enabled = bool(
                    getattr(
                        getattr(self.config, "streaming", {}), "coalesce_enabled", False
                    )
                )
                try:
                    coalesce_min_ms = int(
                        getattr(self.config.streaming, "coalesce_min_ms", 600)
                    )
                except Exception:
                    coalesce_min_ms = 600
                try:
                    micro_fallback_ms = int(
                        getattr(self.config.streaming, "micro_fallback_ms", 300)
                    )
                except Exception:
                ...
                try:
                    if getattr(
                        self.streaming_playback_manager, "continuous_stream", False
                    ):
                        if call_id not in self._segment_tts_active:
                            await self.streaming_playback_manager.start_segment_gating(
                                call_id
                            )
                            self._segment_tts_active.add(call_id)
                except Exception:
                    logger.debug(
                        "Failed to start segment gating", call_id=call_id, exc_info=True
                    )
                if coalesce_enabled and q is None and not isinstance(out_chunk, list):
                    ...
                    try:
                        ...
                        fmt = self._canonicalize_encoding(
                            session.transport_profile.format
                        )
                        bps = 1 if fmt == "mulaw" or fmt == "ulaw" else 2
                        buf_ms = round(
                            (len(buf) / float(max(1, bps * max(1, wire_rate))))
                            * 1000.0,
                            3,
                        )
                    except Exception:
                    ...
                    if buf_ms < coalesce_min_ms:
                        ...
                        try:
                            self._provider_bytes[call_id] = int(
                                self._provider_bytes.get(call_id, 0)
                            ) + (
                                len(chunk)
                    ...
                    try:
                        ...
                        playback_type = (
                            "greeting"
                            if getattr(session, "conversation_state", "") == "greeting"
                        ...
                        alignment_issues = self.provider_alignment_issues.get(
                            provider_name, []
                        )
                        if (
                            alignment_issues
                            and call_id not in self._runtime_alignment_logged
                        ):
                        ...
                        target_encoding, target_sample_rate, remediation = (
                            self._resolve_stream_targets(session, session.provider_name)
                        )
                        ...
                        if use_streaming:
                            ...
                            logger.info(
                                "Using file playback (downstream_mode=file)",
                                call_id=call_id,
                            )
                            try:
                                playback_id = await self.playback_manager.play_audio(
                                    call_id, bytes(buf), "streaming-response"
                                )
                                logger.info(
                                    "File playback started (forced by downstream_mode)",
                                    call_id=call_id,
                                ...
                                logger.error(
                                    "File playback failed (downstream_mode=file)",
                                    call_id=call_id,
                                    exc_info=True,
                                )
                        ...
                        logger.info(
                            ...
                            call_id=call_id,
                            coalesced_ms=buf_ms,
                            coalesced_bytes=len(buf),
                        )
                        try:
                            q.put_nowait(bytes(buf))
                            # Account for the initial coalesced enqueue
                            try:
                                self._enqueued_bytes[call_id] = int(
                                    self._enqueued_bytes.get(call_id, 0)
                                ) + len(buf)
                            except Exception:
                                pass
                        except asyncio.QueueFull:
                            logger.debug(
                                "Coalesced enqueue dropped (queue full)",
                                call_id=call_id,
                            )
                        self._provider_coalesce_buf.pop(call_id, None)
                        return
                        ...
                        logger.error(
                            "File fallback failed after coalesce start error",
                            call_id=call_id,
                            exc_info=True,
                        )
                        self._provider_coalesce_buf.pop(call_id, None)
                    ...
                    if q is None:
                        ...
                        try:
                            playback_type = (
                                "greeting"
                                if getattr(session, "conversation_state", "")
                                == "greeting"
                                else "streaming-response"
                            )
                            fmt_info = self._provider_stream_formats.get(call_id, {})
                            ...
                            if remediation:
                                session.audio_diagnostics["codec_remediation"] = (
                                    remediation
                                )
                            src_encoding = fmt_info.get("encoding") or encoding
                            src_rate = (
                                fmt_info.get("sample_rate")
                                or sample_rate_int
                                or getattr(session.provider, "_dg_output_rate", None)
                            )
                                ...
                                try:
                                    playback_id = (
                                        await self.playback_manager.play_audio(
                                            call_id, out_chunk, "streaming-response"
                                        )
                                    )
                                    if playback_id:
                                        logger.info(
                                    ...
                                    logger.error(
                                        "File playback exception (downstream_mode=file)",
                                        call_id=call_id,
                                        exc_info=True,
                                    )
                                return
                            ...
                            try:
                                ...
                                if not playback_id:
                                    logger.error(
                                        "Fallback file playback failed",
                                        call_id=call_id,
                                        size=len(out_chunk),
                                    )
                            except Exception:
                                logger.error(
                                    "Fallback file playback exception",
                                    call_id=call_id,
                                    exc_info=True,
                                )
                            return
                    try:
                        ...
                        logger.debug(
                            "Provider streaming queue full; dropping chunk",
                            call_id=call_id,
                        )
            elif etype == "AgentAudioDone":
                continuous = bool(
                    getattr(self.streaming_playback_manager, "continuous_stream", False)
                )
                q = self._provider_stream_queues.get(call_id)
                if continuous:
                    # Do NOT end the stream; mark boundary and end per-segment gating
                    try:
                        await self.streaming_playback_manager.mark_segment_boundary(
                            call_id
                        )
                    except Exception:
                        logger.debug(
                            "Failed to mark segment boundary",
                            call_id=call_id,
                            exc_info=True,
                        )
                    try:
                    ...
                    if q is not None:
                        ...
                        logger.debug(
                            "AgentAudioDone with no active stream queue",
                            call_id=call_id,
                        )
                ...
                try:
                    ...
                    try:
                        if (
                            hasattr(self, "streaming_playback_manager")
                            and self.streaming_playback_manager
                        ):
                            self.streaming_playback_manager.record_provider_bytes(
                                call_id, int(prov)
                            )
                    except Exception:
                        logger.debug(
                ...
                if coalesce_enabled and call_id in self._provider_coalesce_buf:
                    ...
                    try:
                        buf_ms = round(
                            (len(buf) / float(2 * max(1, wire_rate))) * 1000.0, 3
                        )
                    except Exception:
                        buf_ms = 0.0
                    micro_fallback_ms = (
                        int(getattr(self.config.streaming, "micro_fallback_ms", 300))
                        if hasattr(self.config, "streaming")
                        else 300
                    )
                    if buf and buf_ms < micro_fallback_ms:
                        try:
                            ...
                            logger.info(
                                "MICRO SEGMENT FILE FALLBACK (end)",
                                call_id=call_id,
                                buf_ms=buf_ms,
                                playback_id=playback_id,
                            )
                            ...
                            logger.error(
                                "File fallback failed at segment end",
                                call_id=call_id,
                                exc_info=True,
                            )
                    elif buf:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.debug(
                                    "Coalesced enqueue dropped at end (queue full)",
                                    call_id=call_id,
                                )
                        except Exception:
                            logger.error(
                                "Coalesced streaming failed at segment end",
                                call_id=call_id,
                                exc_info=True,
                            )
                ...
                if streaming_done:
                    try:
                        ...
                        if session and getattr(session, "cleanup_after_tts", False):
                            logger.info(
                                "🔚 Cleanup after TTS requested - hanging up call",
                                call_id=call_id,
                            )
                        ...
                        logger.debug(
                            "Error checking cleanup_after_tts flag",
                            call_id=call_id,
                            error=str(e),
                        )
                ...
                try:
                    ...
                    if session:
                        ...
                        logger.warning(
                            "No session found for HangupReady", call_id=call_id
                        )
                except Exception as e:
                    logger.error(
    ...
    async def _ensure_pipeline_runner(
        self, session: CallSession, *, forced: bool = False
    ) -> None:
        ...
        if (
            not getattr(self, "pipeline_orchestrator", None)
            or not self.pipeline_orchestrator.enabled
        ):
            return
        ...
        logger.info(
            "Pipeline runner started", call_id=call_id, pipeline=session.pipeline_name
        )
    ...
    async def _pipeline_runner(self, call_id: str) -> None:
        ...
        try:
            ...
            if not session:
                return
            pipeline = self.pipeline_orchestrator.get_pipeline(
                ...
                if context_name:
                    ...
                    if context_config and context_config.prompt:
                        ...
                        logger.info(
                            ...
                            prompt_preview=(
                                context_config.prompt[:80] + "..."
                                if len(context_config.prompt) > 80
                                else context_config.prompt
                            ),
                        )
            ...
            try:
                ...
                if not greeting:
                    global_greeting = (
                        getattr(self.config.llm, "initial_greeting", None) or ""
                    ).strip()
                    if global_greeting:
                ...
                for attempt in range(1, max_attempts + 1):
                    try:
                        ...
                        async for chunk in pipeline.tts_adapter.synthesize(
                            call_id, greeting, pipeline.tts_options
                        ):
                            if chunk:
                        ...
                        if not tts_bytes:
                            ...
                            await self.playback_manager.play_audio(
                                call_id, bytes(tts_bytes), "pipeline-tts-greeting"
                            )

                            # AAVA-85: Persist greeting to session history so it appears in email summary
                            try:
                                logger.info(
                                    "DEBUG: PERSISTING GREETING START",
                                    call_id=call_id,
                                ...
                                await self.session_store.upsert_call(session)
                                logger.info(
                                    "DEBUG: PERSISTING GREETING DONE", call_id=call_id
                                )
                                logger.info(
                                ...
                                logger.error(
                                    "DEBUG: PERSISTING GREETING FAILED",
                                    call_id=call_id,
                                    error=str(e),
                                )
                                logger.warning(
                        ...
                        if attempt < max_attempts and "session" in error_text:
                            ...
                            try:
                                await pipeline.tts_adapter.open_call(
                                    call_id, pipeline.tts_options
                                )
                                continue
                            except Exception:
                                logger.debug(
            ...
            base_commit_ms = 160
            stt_chunk_ms = (
                int(pipeline.stt_options.get("chunk_ms", base_commit_ms))
                if pipeline.stt_options
                else base_commit_ms
            )
            ...
            if use_streaming:
                ...
                if not streaming_supported:
                    logger.warning(
                        ...
                        call_id=call_id,
                        component=getattr(
                            pipeline.stt_adapter, "component_key", "unknown"
                        ),
                    )
                    use_streaming = False
            stream_format = (pipeline.stt_options or {}).get(
            ...
            if use_streaming:
                try:
                    logger.info(
                        ...
                        buffer_max=(
                            getattr(buffer_queue, "_maxsize", 200)
                            if hasattr(buffer_queue, "_maxsize")
                            else 200
                        ),
                    )
                except Exception:
            ...
            if not use_streaming:
                ...
                async def process_audio(audio_chunk: bytes) -> None:
                    ...
                    try:
                        ...
                        logger.debug(
                            "STT transcribe failed", call_id=call_id, exc_info=True
                        )
                        return
            ...
            async def dialog_worker() -> None:
                ...
                async def run_turn(transcript_text: str) -> None:
                    ...
                    provider_label = (
                        getattr(session, "provider_name", None) or "unknown"
                    )
                    ...
                    try:
                        ...
                        logger.debug(
                            "LLM generate failed", call_id=call_id, exc_info=True
                        )
                        return
                    ...
                    logger.info(
                        "DEBUG: LLM Result Type",
                        type=str(type(llm_result)),
                        tool_calls_len=len(getattr(llm_result, "tool_calls", [])),
                        is_llm_response=isinstance(llm_result, LLMResponse),
                        call_id=call_id,
                    )
                    ...
                    if response_text:
                        ...
                        conversation_history.append(
                            {"role": "assistant", "content": "(tool execution)"}
                        )
                    ...
                    logger.info(
                        "DEBUG: Before TTS block",
                        response_text_len=len(response_text),
                        will_skip_tts=not bool(response_text),
                        call_id=call_id,
                    )
                    if response_text:
                        tts_bytes = bytearray()
                        ...
                        try:
                                    ...
                                    if first_tts_ts is None:
                                        ...
                                        try:
                                            if t_start is not None:
                                                _TURN_STT_TO_TTS.labels(
                                                    pipeline_label, provider_label
                                                ).observe(
                            ...
                            logger.debug(
                                "TTS synth failed", call_id=call_id, exc_info=True
                            )
                            ...
                            try:
                                ...
                                try:
                                    if playback_id and t_start is not None:
                                        _TURN_RESPONSE_SECONDS.labels(
                                            pipeline_label, provider_label
                                        ).observe(max(0.0, time.time() - t_start))
                                except Exception:
                                    pass
                                if not playback_id:
                                    logger.error(
                    ...
                    logger.info(
                        "DEBUG: Reached tool execution block",
                        tool_calls_present=bool(tool_calls),
                        tool_calls_count=len(tool_calls),
                        tool_calls_type=str(type(tool_calls)),
                        call_id=call_id,
                    )
                    if tool_calls:
                        logger.info(
                            "DEBUG: Inside tool_calls block",
                            tool_calls_count=len(tool_calls),
                            playback_id=playback_id,
                            call_id=call_id,
                        )
                        ...
                        if playback_id:
                            try:
                                ...
                                await asyncio.sleep(
                                    len(response_text) * 0.08
                                )  # Rough estimate
                            except Exception:
                                pass

                        logger.info(
                            "DEBUG: Before import ToolExecutionContext", call_id=call_id
                        )
                        from src.tools.context import ToolExecutionContext
                        from src.tools.registry import tool_registry

                        logger.info(
                            "DEBUG: After imports, before creating context",
                            call_id=call_id,
                        )
                        ...
                        try:
                            logger.info(
                                "DEBUG: About to create ToolExecutionContext",
                                call_id=call_id,
                            )
                            tool_ctx = ToolExecutionContext(
                                call_id=call_id,
                                caller_channel_id=getattr(
                                    session, "channel_id", call_id
                                ),
                                session_store=self.session_store,
                                ari_client=self.ari_client,
                                config=self.config.dict(),
                            ...
                            logger.info(
                                "DEBUG: ToolExecutionContext created successfully",
                                call_id=call_id,
                            )
                        except Exception as ctx_error:
                            logger.error(
                                "DEBUG: ToolExecutionContext creation FAILED",
                                call_id=call_id,
                                error=str(ctx_error),
                                exc_info=True,
                            )
                        ...
                        for tool_call in tool_calls:
                            try:
                                logger.info(
                                    "DEBUG: Inside for loop iteration",
                                    tool_call=tool_call,
                                    call_id=call_id,
                                )
                                ...
                                if tool:
                                    ...
                                    if result.get("will_hangup"):
                                        ...
                                        if farewell:
                                            ...
                                            await self.session_store.upsert_call(
                                                session
                                            )
                                            logger.info(
                                                "Farewell added to conversation history",
                                                call_id=call_id,
                                            )
                                            ...
                                            try:
                                                ...
                                                async for (
                                                    chunk
                                                ) in pipeline.tts_adapter.synthesize(
                                                    call_id,
                                                    farewell,
                                                    pipeline.tts_options,
                                                ):
                                                ...
                                                logger.error(
                                                    "Farewell TTS failed", error=str(e)
                                                )
                                    ...
                                    if (
                                        ...
                                        logger.info(
                                            "Transfer successful, ending turn loop",
                                            tool=name,
                                        )
                                        return
                                else:
            ...
            if use_streaming:
                ...
                try:
                    await pipeline.stt_adapter.start_stream(
                        call_id, pipeline.stt_options or {}
                    )
                    ...
                    await asyncio.gather(
                        *(
                            task
                            for task in (stt_recv_task, dialog_task)
                            if task is not None
                        ),
                    ...
                    await asyncio.gather(
                        ingest_task, *tasks_to_cancel, return_exceptions=True
                    )
                    if not stop_called:
                        await pipeline.stt_adapter.stop_stream(call_id)
            else:
    ...
    async def _hydrate_transport_from_dialplan(
        self, session: CallSession, channel_id: str
    ) -> None:
    ...
    async def _detect_caller_codec(self, session: CallSession, channel_id: str) -> None:
        ...
        canonical_fmt, sample_rate, reported = self._normalize_audio_format(
            preferred_fmt
        )
    ...
    async def _resolve_audio_profile(
        ...
        try:
            ...
            try:
                self.streaming_playback_manager.audiosocket_format = (
                    transport.wire_encoding
                )
                ...
                if hasattr(self.streaming_playback_manager, "idle_cutoff_ms"):
                    self.streaming_playback_manager.idle_cutoff_ms = (
                        transport.idle_cutoff_ms
                    )
            except Exception as exc:
                logger.warning(
                ...
                if provider and hasattr(provider, "config"):
                    ...
                    try:
                        ...
                        if capabilities and capabilities.requires_continuous_audio:
                            ...
                            pcfg = getattr(provider, "config", None)
                            if isinstance(pcfg, dict):
                                is_continuous_input = bool(
                                    pcfg.get("continuous_input", False)
                                )
                            else:
                                is_continuous_input = bool(
                                    getattr(pcfg, "continuous_input", False)
                    ...
                    if not is_continuous_input:
                        ...
                        provider.config.target_sample_rate_hz = (
                            transport.wire_sample_rate
                        )
                        logger.info(
            ...
            logger.debug(
                ...
                call_id=session.call_id,
                transport_context=(
                    transport.context if hasattr(transport, "context") else None
                ),
            )
                ...
                if context_config:
                    ...
                    try:
                        ...
                        if greeting_to_apply:
                            try:
                                caller_name = (
                                    getattr(session, "caller_name", None) or "there"
                                )
                                caller_number = (
                                    getattr(session, "caller_number", None) or "unknown"
                                )
                        ...
                        if isinstance(provider.config, dict):
                            ...
                            if greeting_to_apply and hasattr(
                                provider.config, "greeting"
                            ):
    ...
    def _infer_transport_from_frame(frame_len: int) -> Tuple[str, int]:
        ...
        fmt, rate = mapping.get(
            frame_len, ("slin16" if frame_len % 2 == 0 else "ulaw", 8000)
        )
        return fmt, rate
    ...
    def _encode_for_provider(
        ...
        try:
            ...
            if provider_cfg is not None:
                ...
                provider_rate = getattr(
                    provider_cfg, "provider_input_sample_rate_hz", None
                )
                ...
                try:
                    gain_target_rms = int(
                        getattr(provider_cfg, "input_gain_target_rms", 0) or 0
                    )
                except Exception:
                    gain_target_rms = 0
                try:
                    gain_max_db = float(
                        getattr(provider_cfg, "input_gain_max_db", 0.0) or 0.0
                    )
                except Exception:
                    gain_max_db = 0.0
        ...
        if expected_enc in ("slin16", "linear16", "pcm16", ""):
            ...
            if pcm_rate != expected_rate and pcm_bytes:
                ...
                try:
                    ...
                    pcm_bytes, _ = audioop.ratecv(
                        pcm_bytes, 2, 1, pcm_rate, expected_rate, None
                    )
                    ...
                    expected_bytes = int(
                        (input_bytes // 2) * (expected_rate / pcm_rate) * 2
                    )
            ...
            if pcm_bytes and gain_target_rms > 0 and gain_max_db > 0.0:
                try:
                    ...
                    logger.error(
                        f"Provider input normalization failed: {e}",
                        call_id=call_id,
                        exc_info=True,
                    )
        ...
        if expected_enc in ("ulaw", "mulaw", "g711_ulaw", "mu-law"):
            ...
            if pcm_rate != expected_rate and working:
                try:
                    ...
                    working, state = audioop.ratecv(
                        working, 2, 1, pcm_rate, expected_rate, state
                    )
    ...
    async def _update_transport_profile(
        self,
        session: CallSession,
        *,
        fmt: Optional[str],
        ...
        if not fmt and not sample_rate:
            return
        canonical_fmt = (
            self._canonicalize_encoding(fmt) or session.transport_profile.format
        )
        ...
        profile.update(
            format=canonical_fmt, sample_rate=final_rate, source=incoming_source
        )
        session.caller_audio_format = canonical_fmt
        ...
        if changed:
            try:
                ...
                logger.debug(
                    "Failed to persist transport profile",
                    call_id=session.call_id,
                    exc_info=True,
                )
            try:
    ...
    def _update_audio_diagnostics(
        ...
        try:
            ...
            if abs(dc_offset) > dc_threshold and not session.audio_diagnostics.get(
                dc_alert_key
            ):
            ...
            logger.debug(
                "Audio diagnostics update failed",
                call_id=session.call_id,
                stage=stage,
                exc_info=True,
            )
    ...
    async def _update_audio_diagnostics_by_call(
        ...
        if not session:
            return
        self._update_audio_diagnostics(
            session, stage, audio_bytes, encoding, sample_rate
        )
    ...
    def _emit_transport_card(
        ...
        *,
        ...
        if session is not None:
            provider_name = (
                getattr(session, "provider_name", None)
                or getattr(session, "provider", None)
                or self.config.default_provider
            )
    ...
    def _resolve_stream_targets(
        ...
        provider_name = (
            provider_name
            or getattr(session, "provider_name", None)
            or self.config.default_provider
        )
        ...
        transport_fmt = (
            self._canonicalize_encoding(
                getattr(session.transport_profile, "wire_encoding", None)
                or getattr(session.transport_profile, "format", None)
            )
            or "ulaw"
        )
        try:
            transport_rate = int(
                getattr(session.transport_profile, "wire_sample_rate", 0)
                or getattr(session.transport_profile, "sample_rate", 0)
                or 0
            )
        except Exception:
            transport_rate = 0
        if transport_rate <= 0:
            transport_rate = (
                8000 if transport_fmt in {"ulaw", "mulaw", "g711_ulaw"} else 16000
            )
        ...
        try:
            ...
            if provider_cfg:
                ...
                provider_input_enc = self._canonicalize_encoding(
                    getattr(provider_cfg, "provider_input_encoding", None)
                    or getattr(provider_cfg, "input_encoding", None)
                )
                raw_input_rate = getattr(
                    provider_cfg, "provider_input_sample_rate_hz", None
                ) or getattr(provider_cfg, "input_sample_rate_hz", None)
                provider_input_rate = int(raw_input_rate) if raw_input_rate else None

                # Also read target for alignment validation
                provider_target_enc = self._canonicalize_encoding(
                    getattr(provider_cfg, "target_encoding", None)
                )
        ...
        try:
            _CODEC_ALIGNMENT.labels(session.call_id, provider_name).set(
                1 if aligned else 0
            )
        except Exception:
            pass
    ...
    async def _start_provider_session(self, call_id: str) -> None:
        ...
        try:
            ...
            if not session:
                logger.error(
                    "Start provider session called for unknown call", call_id=call_id
                )
                return
            ...
            try:
                ...
                logger.debug(
                    "Provider set_input_mode failed or unsupported", exc_info=True
                )
            ...
            try:
                if session.context_name:
                    ...
                    logger.debug(
                        ...
                        config_type=(
                            type(context_config).__name__ if context_config else None
                        ),
                        has_tools_attr=(
                            hasattr(context_config, "tools")
                            if context_config
                            else False
                        ),
                    )
                ...
                logger.warning(
                    f"Failed to build provider context: {e}",
                    call_id=call_id,
                    exc_info=True,
                )
            ...
            if hasattr(provider, "tool_adapter") or hasattr(provider, "_tool_adapter"):
                try:
                    ...
                    logger.warning(
                        f"Failed to inject tool context: {e}", call_id=call_id
                    )

            logger.info(
                "DEBUG: About to call provider.start_session",
                call_id=call_id,
                provider=provider_name,
            )
            await provider.start_session(
                call_id, context=provider_context if provider_context else None
            )
            logger.info(
                "DEBUG: provider.start_session completed",
                call_id=call_id,
                provider=provider_name,
            )
            ...
            try:
                ...
                logger.debug(
                    "Provider initial greeting failed or unsupported", exc_info=True
                )
            ...
            if self.conversation_coordinator:
                try:
                    ...
                    pass
            logger.info(
                "Provider session started", call_id=call_id, provider=provider_name
            )
            ...
            try:
                ...
                logger.debug(
                    "Failed to record call metadata", call_id=call_id, error=str(e)
                )

        except Exception as exc:
            logger.error(
                "Failed to start provider session",
                call_id=call_id,
                error=str(exc),
                exc_info=True,
            )
    ...
    async def _on_playback_finished(self, event: Dict[str, Any]):
        ...
        try:
            ...
            if not playback_id:
                logger.debug(
                    "PlaybackFinished without playback id", playback_event=event
                )
                return
            await self.playback_manager.on_playback_finished(playback_id)
        except Exception as exc:
            logger.error(
                "Error in PlaybackFinished handler", error=str(exc), exc_info=True
            )
        ...
        try:
            ...
            try:
                ...
                if "HEALTH_BIND_HOST" in os.environ:
                    ...
                    health_host = getattr(
                        getattr(self.config, "health", None), "host", "127.0.0.1"
                    )
                ...
                if "HEALTH_BIND_PORT" in os.environ:
                    ...
                    health_port = int(
                        getattr(getattr(self.config, "health", None), "port", 15000)
                    )
            except Exception:
            ...
            logger.error(
                "Failed to start health endpoint", error=str(exc), exc_info=True
            )
    ...
    async def _health_handler(self, request):
        ...
        try:
            ...
            # Compute readiness
            default_ready = False
            if self.config and getattr(self.config, "default_provider", None) in (
                self.providers or {}
            ):
                prov = self.providers[self.config.default_provider]
                try:
                    default_ready = (
                        bool(prov.is_ready()) if hasattr(prov, "is_ready") else True
                    )
                except Exception:
                    default_ready = True
            ari_connected = bool(self.ari_client and self.ari_client.running)
            audiosocket_listening = (
                self.audio_socket_server is not None
                if self.config.audio_transport == "audiosocket"
                else True
            )
            ...
            payload = {
                ...
                "audiosocket": {
                    ...
                    "host": (
                        getattr(self.config.audiosocket, "host", None)
                        if self.config.audiosocket
                        else None
                    ),
                    "port": (
                        getattr(self.config.audiosocket, "port", None)
                        if self.config.audiosocket
                        else None
                    ),
                    "active_connections": (
                        self.audio_socket_server.get_connection_count()
                        if self.audio_socket_server
                        else 0
                    ),
                },
...
async def main():
    ...
    try:
        level_name = str(
            getattr(getattr(config, "logging", None), "level", "info")
        ).upper()
    ...
    if errors:
        logger.error(
            "❌ Configuration validation FAILED", errors=errors, warnings=warnings
        )
