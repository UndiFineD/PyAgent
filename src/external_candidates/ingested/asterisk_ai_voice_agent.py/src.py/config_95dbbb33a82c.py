# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\config.py
"""
...
class ExternalMediaConfig(BaseModel):
    ...
    codec: str = Field(
        default="ulaw"
    )  # Asterisk channel codec: ulaw, alaw, slin, slin16
    ...
    format: str = Field(
        default="slin16"
    )  # Engine internal format: slin (8kHz), slin16 (16kHz), ulaw (8kHz)
    sample_rate: Optional[int] = Field(
        default=None
    )  # Optional: inferred from format if not set (8000 or 16000)
...
class GoogleProviderConfig(BaseModel):
    ...
    llm_max_output_tokens: int = Field(
        default=8192, ge=1, le=8192
    )  # Max output tokens (Gemini supports up to 8192)
    ...
    response_modalities: str = Field(
        default="audio"
    )  # "audio", "text", or "audio_text"

    # Google Live transcription configuration (for email summaries/conversation history)
    enable_input_transcription: bool = Field(
        default=True
    )  # Enable user speech transcription
    enable_output_transcription: bool = Field(
        default=True
    ...
    input_encoding: str = Field(
        default="ulaw"
    )  # Wire format from AudioSocket/RTP (ulaw/slin16)
    input_sample_rate_hz: int = Field(default=8000)  # Wire sample rate
    provider_input_encoding: str = Field(
        default="linear16"
    )  # Gemini Live expects PCM16
...
class OpenAIRealtimeProviderConfig(BaseModel):
    ...
    input_encoding: str = Field(
        default="slin16"
    )  # AudioSocket inbound default (8 kHz PCM16)
    input_sample_rate_hz: int = Field(default=8000)  # AudioSocket source sample rate
    provider_input_encoding: str = Field(
        default="linear16"
    )  # Provider expects PCM16 LE
    provider_input_sample_rate_hz: int = Field(
        default=24000
    )  # OpenAI Realtime input sample rate
    input_gain_target_rms: int = Field(default=0)
    input_gain_max_db: float = Field(default=0.0)
...
class AppConfig(BaseModel):
    ...
    external_media: Optional[ExternalMediaConfig] = Field(
        default_factory=ExternalMediaConfig
    )
    ...
    def _normalize_before(cls, data: Any):  # type: ignore[override]
        try:
            ...
            logger.debug(
                "Pipeline normalization failed (will be caught by Pydantic)",
                error=str(e),
            )
            pass
        return data
    ...
    try:
        ...
        if hasattr(config, "vad") and config.vad:
            if getattr(config.vad, "enhanced_enabled", False):
                if (
                    not hasattr(config.vad, "webrtc_aggressiveness")
                    or config.vad.webrtc_aggressiveness is None
                ):
        ...
        if hasattr(config, "audiosocket") and config.audiosocket:
            ...
            if format_val and format_val not in [
                "slin",
                "slin16",
                "slin24",
                "ulaw",
                ...
                errors.append(
                    f"Invalid audiosocket format: {format_val} (must be slin, slin16, slin24, ulaw, or alaw)"
                )
        ...
        if hasattr(config, "audiosocket") and config.audiosocket:
            ...
            if port and (port < 1024 or port > 65535):
                errors.append(
                    f"AudioSocket port {port} out of valid range (1024-65535)"
                )
        ...
        if log_level == "debug":
            warnings.append(
                "Debug logging enabled (security/performance risk in production)"
            )
        ...
        try:
            ...
            if streaming_log_level == "debug":
                warnings.append(
                    "Streaming log level is DEBUG (increases log volume; set STREAMING_LOG_LEVEL=info for production)"
                )
        except Exception as e:
        ...
        if hasattr(config, "streaming") and config.streaming:
            ...
            if jitter_buffer < 100:
                warnings.append(
                    f"Jitter buffer very small: {jitter_buffer}ms (recommend >= 150ms for production)"
                )
            elif jitter_buffer > 1000:
                warnings.append(
                    f"Jitter buffer very large: {jitter_buffer}ms (adds latency, consider reducing)"
                )
        ...
        try:
            if hasattr(config, "audiosocket") and config.audiosocket:
                if getattr(config.audiosocket, "host", None) == "0.0.0.0":
                    warnings.append(
                        "AudioSocket bound to 0.0.0.0; ensure firewall/segmentation is in place"
                    )
            if hasattr(config, "external_media") and config.external_media:
                if getattr(config.external_media, "rtp_host", None) == "0.0.0.0":
                    warnings.append(
                        "ExternalMedia RTP bound to 0.0.0.0; ensure firewall/segmentation is in place"
                    )
        except Exception as e:
        ...
        if hasattr(config, "streaming") and config.streaming:
            if hasattr(config.streaming, "diag_enable_taps"):
                if getattr(config.streaming, "diag_enable_taps", False):
                    warnings.append(
                        "Diagnostic taps enabled (performance impact, disable in production)"
                    )
        ...
        logger.warning(
            "Configuration validation encountered an error", error=str(e), exc_info=True
        )
