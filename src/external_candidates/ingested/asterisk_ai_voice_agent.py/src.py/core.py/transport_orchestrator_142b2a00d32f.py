# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\transport_orchestrator.py
"""
...
class TransportOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.default_profile_name = config.get("profiles", {}).get(
        ...
        self.audiosocket_format = (
            audiosocket_config.get("format", "slin16")
            if audiosocket_config
            else "slin16"
        )
        self.audiosocket_sample_rate = (
            audiosocket_config.get("sample_rate", None) if audiosocket_config else None
        )
        ...
        if not self.profiles:
            ...
            self.profiles[self.default_profile_name] = self._synthesize_legacy_profile(
                config
            )
    ...
    def _load_profiles(self, config: Dict[str, Any]) -> Dict[str, AudioProfile]:
        ...
        for name, profile_dict in profiles_config.items():
            ...
            try:
                ...
                logger.warning(
                    "Failed to load audio profile", name=name, error=str(exc)
                )
        ...
        for name, context_dict in contexts_config.items():
            ...
            try:
                contexts[name] = ContextConfig(
                    ...
                    tools=context_dict.get(
                        "tools"
                    ),  # Extract tools for function calling
                )
                logger.debug(
                    "Loaded context mapping", name=name, context=contexts[name]
                )
            except Exception as exc:
                logger.warning(
                    "Failed to load context mapping", name=name, error=str(exc)
                )
    ...
    def _negotiate_formats(
        ...
        if provider_config:
            ...
            pref_in_enc = (
                getattr(provider_config, "provider_input_encoding", None)
                or getattr(provider_config, "input_encoding", None)
                or profile.provider_pref.get("input_encoding", "linear16")
            )
            pref_out_enc = (
                getattr(provider_config, "provider_output_encoding", None)
                or getattr(provider_config, "output_encoding", None)
                or profile.provider_pref.get("output_encoding", "linear16")
            )
            try:
                pref_in_rate = (
                    getattr(provider_config, "provider_input_sample_rate_hz", None)
                    or getattr(provider_config, "input_sample_rate_hz", None)
                    or profile.provider_pref.get("input_sample_rate_hz", 16000)
                )
            except Exception:
                pref_in_rate = profile.provider_pref.get("input_sample_rate_hz", 16000)
            try:
                pref_out_rate = (
                    getattr(provider_config, "provider_output_sample_rate_hz", None)
                    or getattr(provider_config, "output_sample_rate_hz", None)
                    or profile.provider_pref.get("output_sample_rate_hz", 16000)
                )
            except Exception:
                pref_out_rate = profile.provider_pref.get(
        ...
        if provider_caps:
            provider_in_enc = self._select_encoding(
                pref_in_enc, provider_caps.input_encodings, "input"
            )
            provider_out_enc = self._select_encoding(
                pref_out_enc, provider_caps.output_encodings, "output"
            )
            provider_in_rate = self._select_sample_rate(
                pref_in_rate, provider_caps.input_sample_rates_hz, "input"
            )
            provider_out_rate = self._select_sample_rate(
                pref_out_rate, provider_caps.output_sample_rates_hz, "output"
            )
        else:
    ...
    def _validate_and_remediate(
        ...
        supported_in_norm = [
            self._normalize_encoding(enc) for enc in provider_caps.input_encodings
        ]
        ...
        supported_out_norm = [
            self._normalize_encoding(enc) for enc in provider_caps.output_encodings
        ]
        ...
        if (
            transport.provider_input_sample_rate
            not in provider_caps.input_sample_rates_hz
        ):
            issues.append(
