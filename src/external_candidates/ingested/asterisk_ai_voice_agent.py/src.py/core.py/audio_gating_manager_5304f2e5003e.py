...
class AudioGatingManager:
    ...
    def __init__(self, vad_manager=None):
        ...
        logger.info(
            ...
            gated_providers=[
                k for k, v in self._provider_configs.items() if v.get("gating_enabled")
            ],
            passthrough_providers=[
                k
                for k, v in self._provider_configs.items()
                if not v.get("gating_enabled")
            ],
        )
        ...
        if state.agent_is_speaking:
            ...
            if self._vad and audio_format == "pcm16":
                try:
                    ...
                    vad_result = await self._vad.process_frame(
                        call_id, audio_chunk, sample_rate
                    )
    ...
    def set_agent_speaking(
        self, call_id: str, is_speaking: bool, provider_name: str = "unknown"
    ) -> None:
