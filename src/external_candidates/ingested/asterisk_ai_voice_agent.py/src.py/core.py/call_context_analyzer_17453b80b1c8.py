# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\call_context_analyzer.py
"""
...
class CallContextAnalyzer:
    ...
    def analyze_call_conditions(
        self, call_id: str, vad_stats: Dict[str, Any]
    ) -> CallConditions:
        hist = self._history.setdefault(
            call_id,
            {
        ...
        interruption_rate = float(hist.get("interruption_count", 0)) / max(
            hist.get("total_turns", 1), 1
        )
        noise_level = float(vad_stats.get("noise_level", 0.5))
        environment = self._classify_environment(
            noise_level, speech_ratio, interruption_rate
        )
        quality_score = (
            1.0
            - noise_level
            + min(speech_ratio * 2, 1.0)
            + (1.0 - min(interruption_rate * 2, 1.0))
        ) / 3.0
    ...
    def _classify_environment(
        noise_level: float, speech_ratio: float, interruption_rate: float
    ) -> CallEnvironment:
