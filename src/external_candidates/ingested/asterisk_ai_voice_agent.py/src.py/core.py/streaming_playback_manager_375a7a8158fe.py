# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\streaming_playback_manager.py
"""
...
from typing import TYPE_CHECKING, Any, Awaitable, Callable, Dict, Optional, Set, Tuple
...
class StreamingPlaybackManager:
    ...
    def __init__(
        ...
        audio_diag_callback: Optional[
            Callable[[str, str, bytes, str, int], Awaitable[None]]
        ] = None,
        ...
        self.audiosocket_broadcast_debug: bool = bool(
            self.streaming_config.get("audiosocket_broadcast_debug", False)
        )
        # Egress endianness override mode: 'auto' | 'force_true' | 'force_false' | 'disabled'
        swap_mode_cfg = str(
            self.streaming_config.get("egress_swap_mode", "disabled") or "disabled"
        ).lower()
        ...
        self.egress_force_mulaw: bool = bool(
            self.streaming_config.get("egress_force_mulaw", False)
        )
        # Output conditioning: limiter and attack envelope
        try:
            self.limiter_enabled: bool = bool(
                self.streaming_config.get("limiter_enabled", True)
            )
        except Exception:
            self.limiter_enabled = True
        try:
            self.limiter_headroom_ratio: float = float(
                self.streaming_config.get("limiter_headroom_ratio", 0.65)
            )
        except Exception:
        ...
        self.keepalive_interval_ms = self.streaming_config.get(
            "keepalive_interval_ms", 5000
        )
        self.connection_timeout_ms = self.streaming_config.get(
            "connection_timeout_ms", 10000
        )
        self.fallback_timeout_ms = self.streaming_config.get(
            "fallback_timeout_ms", 4000
        )
        self.chunk_size_ms = self._resolve_chunk_size_ms(
            self.streaming_config.get("chunk_size_ms")
        )
        self.idle_cutoff_ms = self._resolve_idle_cutoff_ms(
            self.streaming_config.get("idle_cutoff_ms")
        )
        # Continuous streaming across provider segments
        try:
            self.continuous_stream: bool = bool(
                self.streaming_config.get("continuous_stream", True)
            )
        except Exception:
        ...
        self.low_watermark_ms = max(
            0, int(self.streaming_config.get("low_watermark_ms", 80))
        )
        self.provider_grace_ms = max(
            0, int(self.streaming_config.get("provider_grace_ms", 500))
        )
        self.min_start_chunks = max(
            1, int(math.ceil(self.min_start_ms / max(1, self.chunk_size_ms)))
        )
        self.low_watermark_chunks = max(
            0, int(math.ceil(self.low_watermark_ms / max(1, self.chunk_size_ms)))
        )
        # Greeting-specific warm-up (optional)
        try:
            self.greeting_min_start_ms = int(
                self.streaming_config.get("greeting_min_start_ms", 0)
            )
        except Exception:
        ...
        self.greeting_min_start_chunks = (
            max(
                1,
                int(math.ceil(self.greeting_min_start_ms / max(1, self.chunk_size_ms))),
            )
            if self.greeting_min_start_ms > 0
            else self.min_start_chunks
        )
        # Logging verbosity override
        self.logging_level = (
            self.streaming_config.get("logging_level") or "info"
        ).lower()
        ...
        try:
            self.diag_out_dir = str(
                self.streaming_config.get("diag_out_dir", "/tmp/ai-engine-taps")
                or "/tmp/ai-engine-taps"
            )
        except Exception:
        ...
        try:
            self.ulaw_fastpath_guard: bool = bool(
                self.streaming_config.get("ulaw_fastpath_guard", True)
            )
        except Exception:
            self.ulaw_fastpath_guard = True
        # Low-buffer adaptive backoff configuration
        try:
            self.empty_backoff_ticks_max: int = int(
                self.streaming_config.get("empty_backoff_ticks_max", 5)
            )
        except Exception:
            self.empty_backoff_ticks_max = 5
        try:
            self.max_filler_idle_ms: int = int(
                self.streaming_config.get("max_filler_idle_ms", 400)
            )
        except Exception:
    ...
    def _append_call_taps(
        ...
        try:
            ...
            logger.debug(
                "Call-level tap accumulation failed", call_id=call_id, exc_info=True
            )
    ...
    async def start_streaming_playback(
        ...
        try:
            ...
            if not session:
                logger.error(
                    "Cannot start streaming - call session not found", call_id=call_id
                )
                return None
            ...
            provider_name = getattr(session, "provider_name", None) or getattr(
                ...
                try:
                    if hasattr(session.transport_profile, "wire_sample_rate"):
                        wire_sample_rate = int(
                            session.transport_profile.wire_sample_rate
                        )
                    if hasattr(
                        session.transport_profile, "provider_output_sample_rate"
                    ):
            ...
            base_config_ms = max(
                1,
                int(
                    self.greeting_min_start_ms
                    if playback_type == "greeting"
                    else self.min_start_ms
                ),
            )
            ...
            try:
                initial_startup_ready = bool(
                    gap_ms <= int(self.provider_grace_ms or 500)
                )
            except Exception:
            ...
            try:
                logger.info(
                    ...
                    gap_ms=gap_ms,
                    adaptive_min_ms=(
                        adaptive_min_ms if playback_type != "greeting" else base_min_ms
                    ),
                    adaptive_warmup_ms=(
                        adaptive_min_ms if playback_type != "greeting" else base_min_ms
                    ),
                    ...
                    provider_grace_ms=(
                        int(self.provider_grace_ms)
                        if getattr(self, "provider_grace_ms", None) is not None
                        else 0
                    ),
                )
            except Exception:
            ...
            if self.conversation_coordinator:
                gating_success = await self.conversation_coordinator.on_tts_start(
                    call_id, stream_id
                )
            else:
                gating_success = await self.session_store.set_gating_token(
                    call_id, stream_id
                )

            if not gating_success:
                logger.error(
                    "Failed to start streaming gating",
                    call_id=call_id,
                    stream_id=stream_id,
                )
                return None
            ...
            try:
                idle_cutoff_ticks = max(
                    1, int(math.ceil(self.idle_cutoff_ms / max(1, self.chunk_size_ms)))
                )
            except Exception:
            ...
            self.active_streams[call_id] = {
                ...
                "tap_rate": (
                    self.call_tap_rate.get(
                        call_id, resolved_target_rate if self.diag_enable_taps else 0
                    )
                ),
            ...
            try:
                if bool(self.continuous_stream):
                    logger.info(
                        ...
                        stream_id=stream_id,
                        segments_played=int(
                            self.active_streams[call_id].get("segments_played", 0)
                        ),
                        min_start_chunks=int(
                            self.active_streams[call_id].get("min_start_chunks", 0)
                        ),
            ...
            logger.error(
                "Error starting streaming playback",
                call_id=call_id,
                playback_type=playback_type,
                error=str(e),
                exc_info=True,
            ...
            while True:
                try:
                    ...
                    chunk = await asyncio.wait_for(
                        audio_chunks.get(), timeout=fallback_timeout
                    )
                    ...
                    try:
                        ...
                        _STREAMING_JITTER_DEPTH.labels(call_id).set(
                            jitter_buffer.qsize()
                        )
                        ...
                        if info is not None:
                            info["queued_total_bytes"] = int(
                                info.get("queued_total_bytes", 0) or 0
                            ) + len(chunk)
                        ...
                        if not characterizer.characterization_done:
                            ...
                            if characterizer.should_analyze():
                                ...
                                if pattern and call_id in self.adaptive_controllers:
                                    ...
                                    sess = await self.session_store.get_by_call_id(
                                        call_id
                                    )
                                    if sess:
                                        provider_name = getattr(
                                            sess, "provider_name", None
                                        ) or getattr(sess, "provider", "unknown")
                                        wire_rate = 8000
                                        if hasattr(
                                            sess, "transport_profile"
                                        ) and hasattr(
                                            sess.transport_profile, "wire_sample_rate"
                                        ):
                                            wire_rate = int(
                                                sess.transport_profile.wire_sample_rate
                                            )
                                        ...
                                        pattern_cache = get_pattern_cache()
                                        pattern_cache.update_pattern(
                                            provider_name, wire_rate, pattern
                                        )
                    ...
                    try:
                        if call_id in self.active_streams:
                            ...
                            try:
                                src_rate = int(
                                    info.get("source_sample_rate") or 0
                                ) or int(self.sample_rate)
                            except Exception:
                            ...
                            try:
                                tgt_rate = int(
                                    info.get("target_sample_rate") or 0
                                ) or int(self.sample_rate)
                            except Exception:
                            ...
                            try:
                                ratio = (tgt_bps / float(max(1, src_bps))) * (
                                    float(tgt_rate) / float(max(1, src_rate))
                                )
                                egress_bytes = int(
                                    max(1, round(len(chunk) * max(0.5, ratio)))
                                )
                            except Exception:
                                egress_bytes = len(chunk)
                            info["buffered_bytes"] = (
                                int(info.get("buffered_bytes", 0)) + egress_bytes
                            )
                            info["queued_bytes"] = int(
                                info.get("queued_bytes", 0)
                            ) + len(chunk)
                    except Exception:
                        pass
                    ...
                    if not self.continuous_stream:
                        if time.time() - last_send_time > fallback_timeout:
                            logger.warning(
                                "🎵 STREAMING PLAYBACK - Timeout, falling back to file playback",
                                call_id=call_id,
                                stream_id=stream_id,
                                timeout=fallback_timeout,
                            )
                            await self._record_fallback(
                                call_id, f"timeout>{fallback_timeout}s"
                            )
                    ...
                    try:
                        _STREAMING_LAST_CHUNK_AGE.labels(call_id).set(
        ...
        try:
            while True:
                ...
                if call_id not in self.active_streams:
                    break
                status = await self._drain_next_frame(call_id, stream_id, jitter_buffer)
                if status == "error":
                    try:
    ...
    async def _drain_next_frame(
        ...
        if not self._ensure_startup_ready(
            call_id, stream_id, jitter_buffer, stream_info
        ):
            return "wait"
        ...
        if target_rate <= 0:
            target_rate = self._default_sample_rate_for_format(
                target_fmt, int(self.sample_rate)
            )
        ...
        available_frames = self._estimate_available_frames(
            call_id, jitter_buffer, include_remainder=True
        )
        try:
            info = self.active_streams.get(call_id, {})
        ...
        if self._should_wait_for_low_water(
            call_id, stream_info, available_frames, sentinel_seen
        ):
            return "wait"
        ...
        if (
            ...
            if backoff < self.empty_backoff_ticks_max:
                ...
                try:
                    logger.debug(
                        "Low-buffer backoff tick",
                        call_id=call_id,
                        streak=stream_info["empty_backoff_ticks"],
                    )
                except Exception:
                    pass
                return "wait"
        ...
        try:
            ...
            if not isinstance(chunk, (bytes, bytearray)):
                try:
                    ...
                    logger.debug(
                        "Non-bytes audio chunk could not be coerced",
                        call_id=call_id,
                        chunk_type=str(type(chunk)),
                    )
            stream_info = (
                self.active_streams.get(call_id, {})
                if call_id in self.active_streams
                else {}
            )
            ...
            try:
                target_rate = int(
                    stream_info.get("target_sample_rate", self.sample_rate)
                )
            except Exception:
                target_rate = int(self.sample_rate)
            if target_rate <= 0:
            ...
            src_encoding_raw = self._canonicalize_encoding(
                stream_info.get("source_encoding")
            )
            try:
            ...
            if (
                ...
                if guard_ok and not (
                    self.normalizer_enabled and self.normalizer_target_rms > 0
                ):
                    try:
                        logger.info(
                ...
                try:
                    ...
                    logger.debug(
                        "MULAW DECODE SUCCESS", call_id=call_id, pcm_size=len(back_pcm)
                    )
                except Exception as e:
                    logger.error(
                        "MULAW DECODE FAILED",
                ...
                if working_pcm:
                    ...
                    working_pcm = self._trim_leading_silence(
                        working_pcm, threshold_rms=100
                    )
                ...
                try:
                    ...
                    if (
                        working_pcm
                        and self.normalizer_enabled
                        and self.normalizer_target_rms > 0
                    ):
                        logger.debug(
                            "ENTERING NORMALIZER (ulaw guarded)", call_id=call_id
                        )
                        working_pcm = self._apply_normalizer(
                            working_pcm,
                            self.normalizer_target_rms,
                            self.normalizer_max_gain_db,
                        )
                    elif not working_pcm:
                        logger.warning(
                            "EMPTY PCM AFTER DECODE - NORMALIZER SKIPPED",
                            call_id=call_id,
                        )
                except Exception:
                    logger.debug(
                        "Normalizer failed in μ-law guarded path",
                        call_id=call_id,
                        exc_info=True,
                    )
                try:
                    if working_pcm and self.limiter_enabled:
                        working_pcm = self._apply_soft_limiter(
                            working_pcm, self.limiter_headroom_ratio
                        )
                except Exception:
                    pass
                try:
                ...
                try:
                    if (
                        ...
                        and call_id in self.active_streams
                    ):
                        info = self.active_streams.get(call_id, {})
                        try:
                        ...
                        try:
                            if not info.get("tap_first_snapshot_done", False):
                                ...
                                if working_pcm:
                                    fn2 = os.path.join(
                                        self.diag_out_dir,
                                        f"post_compand_pcm16_{call_id}_{stream_id_first}_first.wav",
                                    )
                                    ...
                                    try:
                                        ...
                                        pass
                                    logger.info(
                                        "Wrote post-compand PCM16 tap snapshot",
                                        call_id=call_id,
                                        stream_id=stream_id_first,
                                        path=fn2,
                                        bytes=len(working_pcm),
                                        rate=rate,
                                        snapshot="first",
                                    )
                            ...
                            logger.debug(
                                "Fast-path first-chunk tap snapshot failed",
                                call_id=call_id,
                                exc_info=True,
                            )
                        try:
                        ...
                        if (
                            pre_lim
                            and isinstance(
                                info.get("tap_pre_pcm16"), (bytearray, bytes)
                            )
                            and working_pcm
                        ):
                        ...
                        if (
                            post_lim
                            and isinstance(
                                info.get("tap_post_pcm16"), (bytearray, bytes)
                            )
                            and working_pcm
                        ):
                        ...
                        try:
                            ...
                            win_bytes = 3200
                        if (
                            isinstance(info.get("tap_first_window_post"), bytearray)
                            and working_pcm
                        ):
                            ...
                            if (
                                not info.get("tap_first_window_done")
                                and len(post_w) >= win_bytes
                            ):
                                sid = str(info.get("stream_id", "seg"))
                                try:
                                    fnq200 = os.path.join(
                                        self.diag_out_dir,
                                        f"post_compand_pcm16_{call_id}_{sid}_first200ms.wav",
                                    )
                                    ...
                                    try:
                                        ...
                                        pass
                                    logger.info(
                                        "Wrote post-compand 200ms snapshot",
                                        call_id=call_id,
                                        stream_id=sid,
                                        path=fnq200,
                                        bytes=win_bytes,
                                    ...
                                    logger.warning(
                                        "Failed 200ms post snapshot (guarded)",
                                        call_id=call_id,
                                        stream_id=sid,
                                        rate=win_rate,
                                        exc_info=True,
                    ...
                    logger.debug(
                        "Fast-path tap capture failed", call_id=call_id, exc_info=True
                    )
                return ulaw_bytes
                ...
                try:
                    ...
                    try:
                        if (
                            ...
                            pcm16_bytes = self._apply_normalizer(
                                pcm16_bytes,
                                self.normalizer_target_rms,
                                self.normalizer_max_gain_db,
                            )
                        ...
                        logger.debug(
                            "Normalizer failed in mulaw→pcm fast path",
                            call_id=call_id,
                            exc_info=True,
                        )
                    ...
                    try:
                        if (
                            ...
                            self._append_call_taps(
                                call_id, pcm16_bytes, pcm16_bytes, int(rate)
                            )
                            ...
                            try:
                                if isinstance(
                                    info.get("tap_first_window_pre"), bytearray
                                ):
                                ...
                                if isinstance(
                                    info.get("tap_first_window_post"), bytearray
                                ):
                                    post_w = info["tap_first_window_post"]
                                    if len(post_w) < win_bytes:
                                        needw2 = win_bytes - len(post_w)
                                ...
                                if not info.get("tap_first_window_done"):
                                    pre_w = (
                                        info.get("tap_first_window_pre") or bytearray()
                                    )
                                    post_w = (
                                        info.get("tap_first_window_post") or bytearray()
                                    )
                                    if (
                                        len(pre_w) >= win_bytes
                                        and len(post_w) >= win_bytes
                                        ...
                                        try:
                                            fnp200 = os.path.join(
                                                self.diag_out_dir,
                                                f"pre_compand_pcm16_{call_id}_{sid}_first200ms.wav",
                                            )
                                            ...
                                            try:
                                                ...
                                                pass
                                            logger.info(
                                                "Wrote pre-compand 200ms snapshot",
                                                call_id=call_id,
                                                stream_id=sid,
                                                path=fnp200,
                                                bytes=win_bytes,
                                            ...
                                            logger.warning(
                                                "Failed 200ms pre snapshot (mulaw->pcm fast)",
                                                call_id=call_id,
                                                stream_id=sid,
                                                rate=win_rate,
                                                exc_info=True,
                                        ...
                                        try:
                                            ...
                                            logger.warning(
                                                "Failed 200ms post snapshot (mulaw->pcm fast)",
                                                call_id=call_id,
                                                stream_id=sid,
                                                rate=win_rate,
                                                exc_info=True,
                                ...
                                logger.debug(
                                    "First-window snapshot failed (mulaw->pcm fast path)",
                                    call_id=call_id,
                                    exc_info=True,
                                )
                    except Exception:
                        logger.debug(
                            "Fast-path tap accumulation failed (mulaw->pcm)",
                            call_id=call_id,
                            exc_info=True,
                        )
                    return pcm16_bytes
                    ...
                    if not stream_info.get("src_endian_probe_done", False):
                        ...
                        if swapped is not None:
                            if rms_swapped >= max(1024, 4 * max(1, rms_native)):
                                ...
                                try:
                                    if abs(avg_native) >= 8 * max(
                                        1, abs(avg_swapped)
                                    ) and rms_swapped >= max(256, rms_native // 2):
                                        prefer_swapped = True
                                except Exception:
                                    pass
                        try:
            ...
            if self._is_mulaw(target_fmt):
                ...
                try:
                    ...
                    rate_attack = (
                        int(target_rate)
                        if isinstance(target_rate, int)
                        else int(self.sample_rate)
                    )
                    working = self._apply_attack_envelope(
                        call_id, working, rate_attack, info
                    )
                except Exception:
                    pass
                ...
                try:
                    ...
                    if self.normalizer_enabled and self.normalizer_target_rms > 0:
                        ...
                        working = self._apply_normalizer(
                            working,
                            self.normalizer_target_rms,
                            self.normalizer_max_gain_db,
                        )
                    ...
                    logger.debug(
                        "Normalizer failed; continuing without gain",
                        call_id=call_id,
                        exc_info=True,
                    )
                if self.limiter_enabled:
                    try:
                        working = self._apply_soft_limiter(
                            working, self.limiter_headroom_ratio
                        )
                    except Exception:
                        pass
                if (
                    ...
                    try:
                        if not info.get("tap_first_snapshot_done", False):
                            ...
                            if working:
                                fn = os.path.join(
                                    self.diag_out_dir,
                                    f"pre_compand_pcm16_{call_id}_{stream_id_first}_first.wav",
                                )
                                try:
                                    ...
                                    try:
                                        ...
                                        pass
                                    logger.info(
                                        "Wrote pre-compand PCM16 tap snapshot",
                                        call_id=call_id,
                                        stream_id=stream_id_first,
                                        path=fn,
                                        bytes=len(working),
                                    ...
                                    logger.warning(
                                        "Failed to write pre-compand tap snapshot",
                                        call_id=call_id,
                                        stream_id=stream_id_first,
                                        path=fn,
                                        rate=rate,
                            ...
                            if back_pcm:
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        "Failed to write post-compand tap snapshot",
                                        call_id=call_id,
                                        stream_id=stream_id_first,
                                        path=fn2,
                                        rate=rate,
                        ...
                        logger.debug(
                            "First-chunk tap snapshot failed",
                            call_id=call_id,
                            exc_info=True,
                        )
                    try:
                    ...
                    tap_rate = (
                        int(rate) if isinstance(rate, int) else int(self.sample_rate)
                    )
                    ...
                    try:
                        if (
                            isinstance(info.get("tap_first_window_pre"), bytearray)
                            and working
                        ):
                            pre_w = info["tap_first_window_pre"]
                            if len(pre_w) < win_bytes:
                ...
                try:
                    ...
                    logger.debug(
                        "First-window snapshot failed (ulaw)",
                        call_id=call_id,
                        exc_info=True,
                    )
                    return ulaw_bytes
            ...
            try:
                ...
                if (
                    ...
                    out_pcm = self._apply_normalizer(
                        out_pcm, self.normalizer_target_rms, self.normalizer_max_gain_db
                    )
                    logger.info(
                ...
                if self.limiter_enabled and out_pcm:
                    out_pcm = self._apply_soft_limiter(
                        out_pcm, self.limiter_headroom_ratio
                    )
            except Exception as e:
                logger.error(
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        "First-chunk tap snapshot failed (PCM)",
                        call_id=call_id,
                        exc_info=True,
                    )
                ...
                try:
                    ...
                    logger.debug(
                        "First-window snapshot failed (pcm)",
                        call_id=call_id,
                        exc_info=True,
                    )
            return out_pcm
    ...
    def _trim_leading_silence(
        ...
        try:
            ...
            logger.warning(
                "ENTIRE CHUNK SILENT", chunk_size=len(buf), threshold=threshold_rms
            )
            return pcm_bytes
    ...
    def _apply_normalizer(
        self, pcm_bytes: bytes, target_rms: int, max_gain_db: float
    ) -> bytes:
        ...
        try:
            ...
            if gain <= 1.01:
                ...
                try:
                    logger.debug(
                        "NORMALIZER SKIPPED - gain too small",
                        gain=round(gain, 3),
                        current_rms=int(rms),
                    )
                except Exception:
    ...
    def _apply_attack_envelope(
        ...
        try:
            ...
            total_attack_bytes = int(
                max(0, int(sample_rate * (self.attack_ms / 1000.0)) * 2)
            )
            remaining = int(
                stream_info.get("attack_bytes_remaining", total_attack_bytes)
            )
            if remaining <= 0:
                return pcm_bytes
            buf = array.array("h")
            ...
            for i in range(shape_samples):
                ...
                alpha = max(
                    0.0, min(1.0, consumed_bytes / float(max(1, total_attack_bytes)))
                )
                s = int(buf[i])
    ...
    async def _send_audio_chunk(
        ...
        *,
        ...
        try:
            ...
            if not session:
                logger.warning(
                    "Cannot stream audio - session not found", call_id=call_id
                )
                return False
            stream_info = self.active_streams.get(call_id, {})
            if self.audio_diag_callback:
                try:
                    ...
                    if effective_rate <= 0:
                        effective_rate = self._default_sample_rate_for_format(
                            effective_fmt, self.sample_rate
                        )
                    stage = (
                        f"transport_out:{stream_info.get('playback_type', 'response')}"
                    )
                    await self.audio_diag_callback(
                        call_id, stage, chunk, effective_fmt, effective_rate
                    )
                except Exception:
                    try:
                        ...
                        pass
                    logger.debug(
                        "Streaming diagnostics callback failed",
                        call_id=call_id,
                        exc_info=True,
                    )
            ...
            if self.audio_transport == "externalmedia":
                if not self.rtp_server:
                    logger.warning(
                        "Streaming transport unavailable (no RTP server)",
                        call_id=call_id,
                    )
                    return False
                ...
                if call_id not in self._rtp_codec_cache:
                    codec_str = str(
                        getattr(session, "external_media_codec", "ulaw")
                    ).lower()
                    self._rtp_codec_cache[call_id] = codec_str in (
                        "slin16",
                        "slin",
                        "pcm16",
                        "linear16",
                ...
                if self._rtp_codec_cache.get(call_id, False) and len(chunk) > 0:
                    try:
                        ...
                        logger.warning(
                            "RTP byte-swap failed, sending original",
                            call_id=call_id,
                            error=str(e),
                        )
                        rtp_chunk = chunk
                ...
                success = await self.rtp_server.send_audio(
                    call_id, rtp_chunk, ssrc=ssrc
                )
                if not success:
                    logger.warning(
                        "RTP streaming send failed",
                        call_id=call_id,
                        stream_id=stream_id,
                    )
                else:
                    try:
                        ...
                        if call_id in self.active_streams:
                            info = self.active_streams[call_id]
                            info["tx_bytes"] = int(info.get("tx_bytes", 0)) + len(
                                rtp_chunk
                            )
                            info["tx_total_bytes"] = int(
                                info.get("tx_total_bytes", 0) or 0
                            ) + len(rtp_chunk)
                    except Exception:
                        pass
                return success
            ...
            if self.audio_transport == "audiosocket":
                if not self.audiosocket_server:
                    logger.warning(
                        "Streaming transport unavailable (no AudioSocket server)",
                        call_id=call_id,
                    )
                    return False
                ...
                if not conn_id:
                    logger.warning(
                        "Streaming transport missing AudioSocket connection",
                        call_id=call_id,
                    )
                    return False
                ...
                if call_id not in self._first_send_logged:
                    ...
                    try:
                        sample_rate = int(
                            target_rate if target_rate is not None else self.sample_rate
                        )
                    except Exception:
                        sample_rate = self.sample_rate
                    if sample_rate <= 0:
                        sample_rate = self._default_sample_rate_for_format(
                            fmt, self.sample_rate
                        )
                    logger.info(
                    ...
                    try:
                        ...
                        if info and bool(info.get("diag_enabled")):
                            ...
                            if pre:
                                fn = os.path.join(
                                    self.diag_out_dir,
                                    f"pre_compand_pcm16_{call_id}_{stream_id}_first.wav",
                                )
                                try:
                                    with wave.open(fn, "wb") as wf:
                                        wf.setnchannels(1)
                            ...
                            if post:
                                fn2 = os.path.join(
                                    self.diag_out_dir,
                                    f"post_compand_pcm16_{call_id}_{stream_id}_first.wav",
                                )
                                try:
                                    with wave.open(fn2, "wb") as wf:
                                        wf.setnchannels(1)
                        ...
                        logger.debug(
                            "Per-segment tap snapshot failed",
                            call_id=call_id,
                            stream_id=stream_id,
                            exc_info=True,
                        )
                if self.audiosocket_broadcast_debug:
                    ...
                    if sent == 0:
                        logger.warning(
                            "AudioSocket broadcast send failed (no recipients)",
                            call_id=call_id,
                            stream_id=stream_id,
                        )
                        return False
                    if len(conns) > 1:
                        logger.debug(
                            "AudioSocket broadcast sent",
                            call_id=call_id,
                            stream_id=stream_id,
                            recipients=len(conns),
                        )
                    return True
                ...
                if not success:
                    logger.warning(
                        "AudioSocket streaming send failed",
                        call_id=call_id,
                        stream_id=stream_id,
                    )
                else:
                ...
                try:
                    if (
                        call_id in self.active_streams
                        and not self.active_streams[call_id].get(
                            "first_frame_observed", False
                        )
                        and success
                    ):
                        start_time = float(
                            self.active_streams[call_id].get("start_time", time.time())
                        )
                        pb_type = str(
                            self.active_streams[call_id].get(
                                "playback_type", "response"
                            )
                        )
                        first_s = max(0.0, time.time() - start_time)
                        _STREAM_FIRST_FRAME_SECONDS.labels(call_id, pb_type).observe(
                            first_s
                        )
            ...
            logger.warning(
                "Streaming transport not implemented for audio_transport",
                call_id=call_id,
            ...
            logger.error(
                "Error sending streaming audio chunk",
                call_id=call_id,
                stream_id=stream_id,
                error=str(e),
                exc_info=True,
    ...
    def _should_stop_for_idle(
        self, call_id: str, stream_id: str, jitter_buffer: asyncio.Queue
    ) -> bool:
        info = self.active_streams.get(call_id)
    ...
    def _frame_size_bytes(self, call_id: Optional[str] = None) -> int:
        fmt = self._canonicalize_encoding(self.audiosocket_format) or "ulaw"
        sample_rate = self.sample_rate
        if call_id and call_id in self.active_streams:
            info = self.active_streams.get(call_id, {})
            fmt = self._canonicalize_encoding(info.get("target_format")) or fmt
            try:
    ...
    async def _record_fallback(self, call_id: str, reason: str) -> None:
        ...
        try:
            ...
            logger.debug(
                "Failed to record streaming fallback",
                call_id=call_id,
                reason=reason,
                exc_info=True,
            )

    async def _fallback_to_file_playback(self, call_id: str, stream_id: str) -> None:
        """Fallback to file-based playback when streaming fails."""
        try:
            if not self.fallback_playback_manager:
                logger.error(
                    "No fallback playback manager available",
                    call_id=call_id,
                    stream_id=stream_id,
                )
                return
            ...
            session = await self.session_store.get_by_call_id(call_id)
            if not session:
                logger.error("Cannot fallback - session not found", call_id=call_id)
                return
            ...
            if remaining_audio:
                ...
                try:
                    ...
                    try:
                        src_rate = (
                            int(info.get("source_sample_rate") or 0) or self.sample_rate
                        )
                    except Exception:
                        src_rate = self.sample_rate
                    ...
                    logger.warning(
                        "Fallback conversion failed; passing raw bytes to file playback",
                        call_id=call_id,
                        stream_id=stream_id,
                        exc_info=True,
                    )
                ...
                fallback_playback_id = await self.fallback_playback_manager.play_audio(
                    call_id, mulaw_audio, "streaming-fallback"
                )

                if fallback_playback_id:
                    logger.info(
                        "🎵 STREAMING FALLBACK - Switched to file playback",
                        call_id=call_id,
                        stream_id=stream_id,
                    ...
                    logger.error(
                        "Failed to start fallback file playback",
                        call_id=call_id,
                        stream_id=stream_id,
                    )
            ...
            logger.error(
                "Error in fallback to file playback",
                call_id=call_id,
                stream_id=stream_id,
                error=str(e),
                exc_info=True,
        ...
        try:
            while call_id in self.active_streams:
                ...
                if time_since_last_chunk > (self.connection_timeout_ms / 1000.0):
                    logger.warning(
                        "🎵 STREAMING PLAYBACK - Connection timeout",
                        call_id=call_id,
                        stream_id=stream_id,
                    ...
                    if not self.continuous_stream:
                        ...
                        try:
                            ...
                            if sess:
                                ...
                                sess.last_streaming_error = (
                                    f"keepalive-timeout>{time_since_last_chunk:.2f}s"
                                )
                                await self.session_store.upsert_call(sess)
                        except Exception:
                            pass
                ...
                logger.debug(
                    "🎵 STREAMING KEEPALIVE - Sending keepalive",
                    call_id=call_id,
                    stream_id=stream_id,
                )
            ...
            logger.debug(
                "Keepalive loop cancelled", call_id=call_id, stream_id=stream_id
            )
        except Exception as e:
            logger.error(
                "Error in keepalive loop",
                call_id=call_id,
                stream_id=stream_id,
                error=str(e),
            )
    ...
    async def stop_streaming_playback(self, call_id: str) -> bool:
        ...
        try:
            ...
            await self._cleanup_stream(call_id, stream_id)
            logger.info(
                "🎵 STREAMING PLAYBACK - Stopped", call_id=call_id, stream_id=stream_id
            )
            return True
        except Exception as e:
            logger.error(
                "Error stopping streaming playback",
                call_id=call_id,
                error=str(e),
                exc_info=True,
            )
    ...
    async def mark_segment_boundary(self, call_id: str) -> None:
        ...
        try:
            ...
            logger.debug(
                "Failed to mark segment boundary", call_id=call_id, exc_info=True
            )
    ...
    async def start_segment_gating(self, call_id: str) -> None:
        ...
        try:
            ...
            ok = True
            if self.conversation_coordinator:
                ok = await self.conversation_coordinator.on_tts_start(
                    call_id, stream_id
                )
            else:
            ...
            if not ok:
                logger.warning(
                    "Failed to start segment gating",
                    call_id=call_id,
                    stream_id=stream_id,
                )
        except Exception:
        ...
        try:
            ...
            try:
                ...
                if info and bool(info.get("diag_enabled")):
                    ...
                    if pre:
                        fn = os.path.join(
                            self.diag_out_dir, f"pre_compand_pcm16_{call_id}.wav"
                        )
                        try:
                            with wave.open(fn, "wb") as wf:
                                wf.setnchannels(1)
                            ...
                            logger.warning(
                                "Failed to write pre-compand tap",
                                call_id=call_id,
                                path=fn,
                                rate=rate,
                                exc_info=True,
                        ...
                        try:
                            fn_seg = os.path.join(
                                self.diag_out_dir,
                                f"pre_compand_pcm16_{call_id}_{stream_id}_end.wav",
                            )
                    ...
                    if post:
                        fn2 = os.path.join(
                            self.diag_out_dir, f"post_compand_pcm16_{call_id}.wav"
                        )
                        try:
                            with wave.open(fn2, "wb") as wf:
                                wf.setnchannels(1)
                            ...
                            logger.warning(
                                "Failed to write post-compand tap",
                                call_id=call_id,
                                path=fn2,
                                rate=rate,
                                exc_info=True,
                        ...
                        # Segment-specific snapshot (end of stream)
                        try:
                            fn2_seg = os.path.join(
                                self.diag_out_dir,
                                f"post_compand_pcm16_{call_id}_{stream_id}_end.wav",
                            )
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Accumulated totals
                            provider_total = int(
                                info.get("provider_total_bytes", 0) or 0
                            )
                            ...
                            try:
                                seg_start = float(
                                    info.get("seg_start_ts", 0.0)
                                    or info.get("start_time", 0.0)
                                    or 0.0
                                )
                            except Exception:
                                seg_start = 0.0
                            ...
                            logger.debug(
                                "Streaming segment bytes summary v2 failed",
                                call_id=call_id,
                                exc_info=True,
                            )
                        if cpre:
                            fnc = os.path.join(
                                self.diag_out_dir,
                                f"pre_compand_pcm16_{call_id}_call.wav",
                            )
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                logger.info(
                                    "Wrote call-level pre-compand PCM16 tap",
                                    call_id=call_id,
                                    path=fnc,
                                    bytes=len(cpre),
                                    rate=crate,
                                ...
                                logger.warning(
                                    "Failed to write call-level pre-compand tap",
                                    call_id=call_id,
                                    path=fnc,
                                    rate=crate,
                                    exc_info=True,
                        ...
                        if cpost:
                            fnc2 = os.path.join(
                                self.diag_out_dir,
                                f"post_compand_pcm16_{call_id}_call.wav",
                            )
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                logger.info(
                                    "Wrote call-level post-compand PCM16 tap",
                                    call_id=call_id,
                                    path=fnc2,
                                    bytes=len(cpost),
                                    rate=crate,
                                ...
                                logger.warning(
                                    "Failed to write call-level post-compand tap",
                                    call_id=call_id,
                                    path=fnc2,
                                    rate=crate,
                                    exc_info=True,
                            ...
                            try:
                                if os.path.isdir(self.diag_out_dir):
                                    ...
                                    for name in os.listdir(self.diag_out_dir):
                                        if name.startswith(
                                            prefix_pre
                                        ) or name.startswith(prefix_post):
                                            fpath = os.path.join(
                                                self.diag_out_dir, name
                                            )
                                            try:
                        ...
                        logger.debug(
                            "Call-level tap write failed",
                            call_id=call_id,
                            exc_info=True,
                        )
            except Exception:
                logger.debug(
                    "Diagnostic tap write failed", call_id=call_id, exc_info=True
                )
            finally:
            ...
            try:
                ...
                if rem:
                    ...
                    if self.audio_transport == "audiosocket":
                        ...
                        try:
                            ...
                            sr = self.sample_rate
                        if sr <= 0:
                            sr = self._default_sample_rate_for_format(
                                fmt, self.sample_rate
                            )
                        bytes_per_sample = 1 if self._is_mulaw(fmt) else 2
                        frame_size = int(
                            sr * (self.chunk_size_ms / 1000.0) * bytes_per_sample
                        ) or (160 if bytes_per_sample == 1 else 320)
                ...
                logger.debug(
                    "Remainder flush failed", call_id=call_id, stream_id=stream_id
                )
            ...
            try:
                if call_id in self.active_streams:
                    ...
                    dur = max(
                        0.0, time.time() - float(info.get("start_time", time.time()))
                    )
                    _STREAM_SEGMENT_DURATION_SECONDS.labels(call_id, pb_type).observe(
                        dur
                    )
            ...
            try:
                if call_id in self.active_streams:
                    ...
                    try:
                        ...
                        end_ts = float(
                            info.get("last_real_emit_ts")
                            or info.get("last_frame_ts")
                            or time.time()
                        )
                    ...
                    try:
                        drift_pct = (
                            0.0
                            if wall_seconds <= 0.0
                            else ((eff_seconds - wall_seconds) / wall_seconds) * 100.0
                        )
                    except Exception:
                        drift_pct = 0.0
                    logger.info(
            ...
            logger.debug(
                "Streaming cleanup completed", call_id=call_id, stream_id=stream_id
            )
