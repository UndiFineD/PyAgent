# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\conversation_coordinator.py
...
class ConversationCoordinator:
    ...
    async def register_call(self, session: CallSession) -> None:
        ...
        logger.debug(
            "ConversationCoordinator registering call", call_id=session.call_id
        )
        _AUDIO_CAPTURE_GAUGE.labels(session.call_id).set(
            1 if session.audio_capture_enabled else 0
        )
    ...
    async def on_tts_start(self, call_id: str, playback_id: str) -> bool:
        ...
        logger.info(
            "🔇 ConversationCoordinator gating audio",
            call_id=call_id,
    ...
    async def on_tts_end(
        self, call_id: str, playback_id: str, reason: str = "playback-finished"
    ) -> bool:
    ...
    async def update_conversation_state(self, call_id: str, state: str) -> None:
        ...
        if state not in _CONVERSATION_STATES:
            logger.debug(
                "Unknown conversation state requested", call_id=call_id, state=state
            )
            return
        session = await self._session_store.get_by_call_id(call_id)
        if not session:
    ...
    async def schedule_capture_fallback(self, call_id: str, delay: float) -> None:
        ...
        async def _task():
            try:
                ...
                logger.info(
                    "🎤 ConversationCoordinator fallback re-enabled capture",
                    call_id=call_id,
                )
            except asyncio.CancelledError:
                logger.debug(
                    "ConversationCoordinator capture fallback cancelled",
                    call_id=call_id,
                )
            except Exception:  # pragma: no cover - defensive logging
                logger.exception(
                    "ConversationCoordinator capture fallback failed", call_id=call_id
                )
    ...
    async def get_summary(self) -> Dict[str, Optional[int]]:
        ...
        capture_disabled = sum(
            1 for session in sessions if not session.audio_capture_enabled
        )
