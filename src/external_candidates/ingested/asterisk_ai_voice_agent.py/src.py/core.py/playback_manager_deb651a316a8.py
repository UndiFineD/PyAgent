# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\playback_manager.py
"""
...
class PlaybackManager:
    ...
    def __init__(
        ...
        logger.info("PlaybackManager initialized", media_dir=self.media_dir)

    async def play_audio(
        self, call_id: str, audio_bytes: bytes, playback_type: str = "response"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if not session:
                logger.error(
                    "Cannot play audio - call session not found", call_id=call_id
                )
                return None
    ...
    async def on_playback_finished(self, playback_id: str) -> bool:
        ...
        try:
            ...
            if not playback_ref:
                logger.warning(
                    "🔊 PlaybackFinished for unknown playback ID",
                    playback_id=playback_id,
                )
                return False
            ...
            logger.error(
                "Error handling PlaybackFinished",
                playback_id=playback_id,
                error=str(e),
                exc_info=True,
            )
    ...
    async def _create_audio_file(
        self, audio_bytes: bytes, playback_id: str
    ) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                chowned = True
                logger.debug(
                    "Audio file ownership set for Asterisk", file_path=file_path
                )
            except OSError as e:
                logger.warning(
                    "Failed to set file ownership for Asterisk",
                    file_path=file_path,
                    error=str(e),
                )
            ...
            logger.debug(
                "Audio file created", file_path=file_path, size=len(audio_bytes)
            )
    ...
    async def _play_via_ari(
        self, session: CallSession, audio_file: str, playback_id: str
    ) -> bool:
        ...
        try:
            ...
            play_with_id = getattr(
                self.ari_client, "play_media_on_bridge_with_id", None
            )
    ...
    async def _cleanup_audio_file(self, audio_file: str) -> None:
        ...
        try:
            if os.path.exists(audio_file):
                ...
                logger.debug("Audio file cleaned up", file_path=audio_file)
        except Exception as e:
            logger.warning(
                "Error cleaning up audio file", file_path=audio_file, error=str(e)
            )

    async def _schedule_gating_fallback(
        self, call_id: str, playback_id: str, audio_size: int
    ) -> None:
        """
        ...
        try:
            ...
            fallback_delay = audio_duration + (
                2.5 if is_pipeline else 0.5
            )  # safety margin

            logger.debug(
                "Scheduling gating fallback",
                call_id=call_id,
                playback_id=playback_id,
            ...
            asyncio.create_task(
                self._gating_fallback_task(call_id, playback_id, fallback_delay)
            )

        except Exception as e:
            logger.error(
                "Error scheduling gating fallback",
                call_id=call_id,
                playback_id=playback_id,
                error=str(e),
            )

    async def _gating_fallback_task(
        self, call_id: str, playback_id: str, delay: float
    ) -> None:
        """
        ...
        try:
            ...
            if playback_ref:
                ...
                logger.warning(
                    "🔊 GATING FALLBACK - Cleared gating token due to missing PlaybackFinished",
                    call_id=call_id,
                    playback_id=playback_id,
                    success=success,
                )
                ...
                logger.debug(
                    "Gating fallback skipped - playback already finished",
                    call_id=call_id,
                    playback_id=playback_id,
                )
