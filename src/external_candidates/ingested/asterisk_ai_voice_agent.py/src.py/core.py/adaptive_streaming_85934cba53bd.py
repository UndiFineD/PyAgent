# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\core\adaptive_streaming.py
"""
    ...
    def analyze(self) -> Optional[StreamPattern]:
        ...
        try:
            ...
            coefficient_of_variation = (
                math.sqrt(variance) / mean_interval if mean_interval > 0 else 0
            )
...
class AdaptiveBufferController:
    ...
    def on_stable_period(
        self, call_id: str, avg_buffered_chunks: int, target_chunks: int
    ):
        """
    ...
    def _load_cache(self):
        ...
        try:
            if os.path.exists(self.cache_file):
                ...
                logger.info(
                    "Loaded provider pattern cache", patterns_count=len(self.patterns)
                )
        except Exception as e:
    ...
    def update_pattern(self, provider: str, sample_rate: int, pattern: StreamPattern):
        ...
        if key in self.patterns:
            ...
            merged = StreamPattern(
                ...
                mean_interval_ms=old.mean_interval_ms * 0.7
                + pattern.mean_interval_ms * 0.3,
                max_gap_ms=max(old.max_gap_ms, pattern.max_gap_ms),  # Keep worst-case
                variance=old.variance * 0.7 + pattern.variance * 0.3,
                optimal_buffer_ms=int(
                    old.optimal_buffer_ms * 0.7 + pattern.optimal_buffer_ms * 0.3
                ),
            )
