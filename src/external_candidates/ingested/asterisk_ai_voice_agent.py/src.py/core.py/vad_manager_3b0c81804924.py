...
class AdaptiveThreshold:
    def __init__(
        self, base_threshold: int, adaptation_rate: float = 0.1, max_samples: int = 100
    ):
...
class EnhancedVADManager:
        ...
        *,
        ...
        if WEBRTC_VAD_AVAILABLE:
            try:
                ...
                logger.warning(
                    "Enhanced VAD - WebRTC initialization failed", exc_info=True
                )
                self.webrtc_vad = None
        else:
    ...
    async def process_frame(
        self, call_id: str, audio_frame_pcm16: bytes, sample_rate: int = 8000
    ) -> VADResult:
        ...
        if (
            self.adaptive_threshold_enabled
            and call_state["frame_count"] % self._adaptation_interval == 0
        ):
        ...
        if self.adaptive_threshold_enabled:
            call_state["adaptive_threshold"].update(
                energy, webrtc_result or energy_result
            )

        final_speech = self._smooth_frames_per_call(
            call_id, webrtc_result or energy_result
        )
        confidence = self._calc_confidence(
            webrtc_result, energy_result, energy, threshold
        )
    ...
    def _smooth_frames_per_call(self, call_id: str, raw_speech: bool) -> bool:
        ...
        if raw_speech:
            ...
            if (
                not call_state["is_speaking"]
                and call_state["speech_frames"] >= self.min_speech_frames
            ):
            ...
            if (
                call_state["is_speaking"]
                and call_state["silence_frames"] >= self.max_silence_frames
            ):
    ...
    def _calc_confidence(
        self, webrtc_result: bool, energy_result: bool, energy: int, threshold: int
    ) -> float:
        confidence = 0.0
        if webrtc_result:
            confidence += 0.4
    ...
    async def _adapt_vad_parameters(self, call_id: str) -> None:
        ...
        try:
            ...
            conditions = self.context_analyzer.analyze_call_conditions(
                call_id, call_stats
            )
            ...
            if abs(smoothed_base - current_base) >= 10:
                ...
                adaptive.current_threshold = int(
                    adaptive.current_threshold * 0.8 + adaptive.base_threshold * 0.2
                )
                logger.debug(
            ...
            logger.debug(
                "VAD parameter adaptation error", call_id=call_id, error=str(e)
            )
    ...
    def _update_call_stats(self, call_id: str, result: VADResult) -> None:
        ...
        stats["avg_energy"] = (
            stats["avg_energy"] * (total - 1) + result.energy_level
        ) / total
    ...
    def mu_law_to_pcm16(frame_ulaw: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(
                "Enhanced VAD - ulaw to PCM16 conversion failed", exc_info=True
            )
            return b""
