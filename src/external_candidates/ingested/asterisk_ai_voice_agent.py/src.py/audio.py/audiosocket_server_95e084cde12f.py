...
class AudioSocketServer:
    ...
    async def _handle_client(
        ...
        try:
            ...
            logger.debug(
                "Failed to set TCP_NODELAY on AudioSocket connection", conn_id=conn_id
            )
        ...
        connection_task = asyncio.create_task(
            self._connection_loop(conn_id, reader, writer)
        )
        async with self._lock:
        ...
        try:
            while True:
                ...
                if not handshake_complete:
                    ...
                    logger.info(
                        "AudioSocket UUID bound", conn_id=conn_id, uuid=uuid_str
                    )
                    continue
    ...
    async def send_audio(self, conn_id: str, audio_payload: bytes) -> bool:
        ...
        if not writer:
            logger.debug(
                "Attempted to send audio on closed connection", conn_id=conn_id
            )
            return False

        frame = (
            bytes([TYPE_AUDIO]) + len(audio_payload).to_bytes(2, "big") + audio_payload
        )
        try:
            writer.write(frame)
            await writer.drain()
