# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\providers\google_live.py
"""
...
class GoogleLiveProvider(AIProviderInterface):
    ...
    async def start_session(
        ...
        api_key_preview = (
            f"{api_key[:8]}...{api_key[-4:]}" if len(api_key) > 12 else "<too_short>"
        )
        ...
        try:
            ...
            logger.debug(
                "Waiting for Google Live setupComplete...", call_id=self._call_id
            )
            await asyncio.wait_for(self._setup_ack_event.wait(), timeout=5.0)
            logger.info(
                "Google Live setup complete (ACK received)", call_id=self._call_id
            )
    ...
    async def _send_setup(self, context: Optional[Dict[str, Any]]) -> None:
        ...
        if tool_names and self._tool_adapter:
            try:
                ...
                if tools:
                    tool_count = (
                        len(tools[0].get("functionDeclarations", [])) if tools else 0
                    )
                    logger.debug(
    ...
    async def _send_greeting(self) -> None:
        ...
        logger.info(
            "Sending greeting request to Google Live",
            call_id=self._call_id,
            greeting_preview=greeting[:50],
        )
    ...
    async def send_audio(
        self, audio_chunk: bytes, sample_rate: int = 8000, encoding: str = "ulaw"
    ) -> None:
        """
        ...
        try:
            ...
            chunk_size = int(
                provider_rate * 2 * _COMMIT_INTERVAL_SEC
            )  # 2 bytes per sample
            ...
            while len(self._input_buffer) >= chunk_size:
                ...
                _GOOGLE_LIVE_AUDIO_SENT.labels(call_id=self._call_id).inc(
                    len(chunk_to_send)
                )
    ...
    async def _handle_server_content(self, data: Dict[str, Any]) -> None:
        ...
        if turn_complete:
            ...
            if self._input_transcription_buffer:
                ...
                await self._track_conversation_message(
                    "user", self._input_transcription_buffer
                )
            ...
            if self._output_transcription_buffer:
                ...
                await self._track_conversation_message(
                    "assistant", self._output_transcription_buffer
                )
    ...
    async def _handle_audio_output(self, audio_b64: str) -> None:
        ...
        try:
            ...
            _GOOGLE_LIVE_AUDIO_RECEIVED.labels(call_id=self._call_id).inc(
                len(pcm16_provider)
            )
