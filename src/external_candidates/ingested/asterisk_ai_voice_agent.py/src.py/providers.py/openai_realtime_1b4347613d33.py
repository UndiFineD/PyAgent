# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\providers\openai_realtime.py
"""
...
class OpenAIRealtimeProvider(AIProviderInterface):
    ...
    def __init__(
        ...
        self._current_response_id: Optional[str] = (
            None  # Track active response for cancellation
        )
        self._greeting_response_id: Optional[str] = (
            None  # Track greeting to protect from barge-in
        )
        self._greeting_completed: bool = False  # Track if greeting has finished
        self._farewell_response_id: Optional[str] = (
            None  # Track farewell response for hangup
        )
        self._hangup_after_response: bool = (
            False  # Flag to trigger hangup after next response
        )
        self._farewell_timeout_task: Optional[asyncio.Task] = (
            None  # Timeout fallback for hangup
        )
        ...
        self._active_output_sample_rate_hz: Optional[float] = (
            float(self.config.output_sample_rate_hz)
            if getattr(self.config, "output_sample_rate_hz", None)
            else None
        )
        ...
        self._egress_pacer_enabled: bool = bool(
            getattr(config, "egress_pacer_enabled", True)
        )
        try:
            self._egress_pacer_warmup_ms: int = int(
                getattr(config, "egress_pacer_warmup_ms", 320)
            )
        except Exception:
    ...
    def describe_alignment(
        ...
        *,
        ...
        if (
            inbound_enc in ("slin16", "linear16", "pcm16")
            and _class(audiosocket_format) == "ulaw"
        ):
            issues.append(
            ...
            inbound_enc in ("ulaw", "mulaw", "g711_ulaw", "mu-law")
            and _class(audiosocket_format) == "ulaw"
        ):
            # Perfect alignment: both ulaw
            pass
        elif inbound_enc in ("ulaw", "mulaw", "g711_ulaw", "mu-law") and _class(
            audiosocket_format
        ) in ("pcm16",):
    ...
    async def start_session(
        self, call_id: str, context: Optional[Dict[str, Any]] = None
    ):
        ...
        try:
            ...
            logger.error(
                "Failed to connect to OpenAI Realtime", call_id=call_id, exc_info=True
            )
            raise
        ...
        try:
            first_message = await asyncio.wait_for(self.websocket.recv(), timeout=5.0)
        ...
        try:
            if (self.config.greeting or "").strip():
                logger.info(
                    "Sending explicit greeting (before ACK wait)", call_id=call_id
                )
            ...
            logger.debug(
                "Initial response.create request failed", call_id=call_id, exc_info=True
            )
        ...
        try:
            ...
            await asyncio.wait_for(
                self._session_ack_event.wait(), timeout=3.0
            )  # Increased from 2.0s - ACK arrives at ~2.005s
            logger.info(
    ...
    async def send_audio(
        self, audio_chunk: bytes, sample_rate: int = None, encoding: str = None
    ):
        ...
        if not self.websocket or self.websocket.closed:
            logger.debug(
                "Dropping inbound audio: websocket not ready", call_id=self._call_id
            )
            return
        ...
        try:
            ...
            if encoding and sample_rate:
                ...
                if encoding.lower().strip() in ("linear16", "pcm16", "slin16"):
                    ...
                    provider_rate = int(
                        getattr(self.config, "provider_input_sample_rate_hz", 0)
                        or 24000
                    )
            else:
                ...
                pcm16 = self._convert_inbound_audio(audio_chunk)
                provider_rate = int(
                    getattr(self.config, "provider_input_sample_rate_hz", 0) or 24000
                )
            ...
            logger.warning(
                "OpenAI Realtime socket closed while sending audio",
                call_id=self._call_id,
            )
            await self._reconnect_with_backoff()
        except Exception:
            logger.error(
                "Failed to send audio to OpenAI Realtime",
                call_id=self._call_id,
                exc_info=True,
            )
    ...
    async def cancel_response(self):
        ...
        try:
            ...
            logger.info(
                "Sent response.cancel to OpenAI (barge-in)", call_id=self._call_id
            )
            self._pending_response = False
        except Exception:
            logger.error(
                "Failed to send response.cancel", call_id=self._call_id, exc_info=True
            )
            ...
            try:
                ...
                if call_id_field:
                    ...
                    if self.websocket and not self.websocket.closed:
                        ...
                        logger.info(
                            "Sent error response to OpenAI", call_id=call_id_field
                        )
    ...
    def _build_ws_url(self) -> str:
        ...
        if base.startswith("${") or not base.startswith(("ws://", "wss://")):
            logger.warning(
                "Invalid OpenAI base_url in config; falling back to default",
                base_url=base,
            )
    ...
    async def _send_session_update(self):
        ...
        output_modalities = [
            m for m in (self.config.response_modalities or []) if m in ("audio", "text")
        ]
        ...
        input_enc = (
            getattr(self.config, "provider_input_encoding", None) or "linear16"
        ).lower()
        ...
        if (
            hasattr(self.config, "max_response_output_tokens")
            and self.config.max_response_output_tokens
        ):
        ...
        if getattr(self.config, "turn_detection", None):
            try:
                ...
                logger.debug(
                    "Failed to include turn_detection in session.update",
                    call_id=self._call_id,
                    exc_info=True,
                )
    ...
    async def _send_explicit_greeting(self):
        ...
        logger.info(
            "🔇 Disabling turn_detection for greeting playback", call_id=self._call_id
        )
        ...
        disable_vad_payload: Dict[str, Any] = {
            ...
            "session": {"turn_detection": None},  # Disable automatic VAD
        }
    ...
    async def _re_enable_vad(self):
        ...
        if getattr(self.config, "turn_detection", None):
            try:
                ...
                logger.debug(
                    "Failed to build turn_detection config, using OpenAI defaults",
                    call_id=self._call_id,
                    exc_info=True,
                )
        ...
        logger.info(
            ...
            call_id=self._call_id,
            config=(
                turn_detection_config if turn_detection_config else "OpenAI defaults"
            ),
        )
    ...
    async def _ensure_response_request(self):
        ...
        response_payload: Dict[str, Any] = {
            ...
            "response": {
                ...
                "modalities": [
                    m
                    for m in (self.config.response_modalities or [])
                    if m in ("audio", "text")
                ]
                or ["audio"],
    ...
    def _start_farewell_timeout(self):
        ...
        self._farewell_timeout_task = asyncio.create_task(
            self._farewell_timeout_handler()
        )
        logger.debug(
            "⏱️  Farewell timeout started (5s fallback)", call_id=self._call_id
        )
    ...
    def _cancel_farewell_timeout(self):
        ...
        if self._farewell_timeout_task and not self._farewell_timeout_task.done():
            ...
            logger.debug("⏱️  Farewell timeout cancelled", call_id=self._call_id)
    ...
    async def _send_audio_to_openai(self, pcm16: bytes):
        ...
        if vad_enabled:
            try:
                ...
                logger.error(
                    "Failed to append input audio buffer (VAD)",
                    call_id=self._call_id,
                    exc_info=True,
                )
        else:
            ...
            async with self._audio_lock:
                ...
                if len(self._pending_audio_provider_rate) >= commit_threshold_bytes:
                    ...
                    try:
                        ...
                        logger.error(
                            "Failed to append input audio buffer",
                            call_id=self._call_id,
                            exc_info=True,
                        )
    ...
    def _convert_inbound_audio(self, audio_chunk: bytes) -> Optional[bytes]:
        ...
        if fmt not in valid_encodings:
            logger.warning(
                "Unsupported input encoding for OpenAI Realtime", encoding=fmt_raw
            )
            fmt = "slin16"
            try:
                self.config.input_encoding = fmt
        ...
        if chunk_len == 160:
            ...
            actual_format = (
                "pcm16" if fmt in ("slin16", "linear16", "pcm16") else "ulaw"
            )
            inferred_rate = (
                int(getattr(self.config, "input_sample_rate_hz", 0) or 0) or 8000
            )
        ...
        try:
            if actual_format == "pcm16" and not getattr(
                self, "_endianness_probe_done", False
            ):
        ...
        provider_rate = int(
            getattr(self.config, "provider_input_sample_rate_hz", 0) or 0
        )
        ...
        try:
            async for message in self.websocket:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to decode OpenAI Realtime payload",
                        payload_preview=message[:64],
                    )
                    continue
            ...
            logger.error(
                "OpenAI Realtime receive loop error",
                call_id=self._call_id,
                exc_info=True,
            )
        finally:
            ...
            try:
                ...
                logger.debug(
                    "Failed to schedule OpenAI reconnect",
                    call_id=self._call_id,
                    exc_info=True,
                )
    ...
    async def _handle_event(self, event: Dict[str, Any]):
        ...
        if event_type == "error":
            ...
            # Log other errors
            logger.error(
                "OpenAI Realtime error event", call_id=self._call_id, error_event=event
            )
            return
        ...
        if event_type == "response.output_audio.delta":
            audio_b64 = event.get("audio") or (event.get("delta") or {}).get("audio")
            if audio_b64:
                await self._handle_output_audio(audio_b64)
            else:
                logger.debug(
                    "Missing audio in response.output_audio.delta",
                    call_id=self._call_id,
                )
            return
        ...
        if event_type == "response.audio.delta":
            ...
            if audio_b64:
                ...
                logger.debug(
                    "Missing audio in response.audio.delta", call_id=self._call_id
                )
            return
        ...
        if event_type in (
            ...
            if event_type == "response.error":
                ...
                logger.info(
                    "OpenAI response cancelled (barge-in)",
                    call_id=self._call_id,
                    response_id=self._current_response_id,
                )
            ...
            if (
                ...
                and self._current_response_id == self._farewell_response_id
                and event_type in ("response.completed", "response.done")
            ):
    ...
    async def _handle_output_audio(self, audio_b64: str):
        try:
            ...
            logger.warning(
                "Invalid base64 audio payload from OpenAI", call_id=self._call_id
            )
            return
        ...
        if (
            ...
            and self._provider_output_format
            in ("g711_ulaw", "ulaw", "mulaw", "g711", "mu-law")
                ...
                try:
                    ...
                    if l % 2 == 1:
                        ...
                        try:
                            ...
                            win_mulaw_pcm16 = b""
                        rms_ulaw = (
                            audioop.rms(win_mulaw_pcm16, 2) if win_mulaw_pcm16 else 0
                        )
                    ...
                    inferred = None
                self._inferred_provider_encoding = (
                    inferred or self._inferred_provider_encoding or "pcm16"
                )
            ...
            if effective_fmt in ("g711_ulaw", "ulaw", "mulaw", "g711", "mu-law"):
                try:
                    ...
                    logger.warning(
                        "Failed to convert μ-law provider output to PCM16",
                        call_id=self._call_id,
                        exc_info=True,
                    )
                    return
            ...
            if not self._outfmt_acknowledged and effective_fmt in (
                "g711_ulaw",
                "ulaw",
                "mulaw",
                "g711",
                ...
                source_rate = int(
                    round(
                        self._active_output_sample_rate_hz
                        or self.config.output_sample_rate_hz
                        or 0
                    )
                )
                if not source_rate:
            ...
            outbound = convert_pcm16le_to_target_format(
                pcm_target, self.config.target_encoding
            )
            if not outbound:
                return
        ...
        try:
            ...
            logger.debug(
                "Failed appending to pacer buffer", call_id=self._call_id, exc_info=True
            )
        ...
        if self._egress_pacer_enabled:
            ...
            if self.on_event:
                ...
                try:
                    ...
                    logger.error(
                        "Failed to emit AgentAudio event",
                        call_id=self._call_id,
                        exc_info=True,
                    )
    ...
    async def _emit_audio_done(self):
        ...
        try:
            ...
            logger.error(
                "Failed to emit AgentAudioDone event",
                call_id=self._call_id,
                exc_info=True,
            )
        finally:
            ...
            try:
                ...
                logger.debug(
                    "Failed to pause pacer on AgentAudioDone",
                    call_id=self._call_id,
                    exc_info=True,
                )
            self._output_resample_state = None
    ...
    async def _emit_transcript(self, text: str, *, is_final: bool):
        ...
        try:
            ...
            logger.error(
                "Failed to emit transcript event", call_id=self._call_id, exc_info=True
            )
        ...
        try:
            while self.websocket and not self.websocket.closed:
                ...
                try:
                    ...
                    logger.debug(
                        "OpenAI Realtime keepalive failed",
                        call_id=self._call_id,
                        exc_info=True,
                    )
                    break
        ...
        for attempt in range(1, 6):
            ...
            try:
                ...
                if self.config.organization:
                    headers.append(("OpenAI-Organization", self.config.organization))
                logger.info(
                    "Reconnecting to OpenAI Realtime", call_id=call_id, attempt=attempt
                )
                self.websocket = await websockets.connect(url, extra_headers=headers)
                # Reset minor state
                self._pending_response = False
                ...
                logger.warning(
                    "OpenAI Realtime reconnect failed",
                    call_id=call_id,
                    attempt=attempt,
                    exc_info=True,
                )
    ...
    def _reset_output_meter(self) -> None:
        ...
        try:
            self._active_output_sample_rate_hz = float(
    ...
    def _log_session_assumptions(self) -> None:
        ...
        info_payload = {
            ...
            "input_sample_rate_hz": str(
                getattr(self.config, "input_sample_rate_hz", "") or ""
            ),
            "provider_input_encoding": str(
                getattr(self.config, "provider_input_encoding", "") or ""
            ),
            "provider_input_sample_rate_hz": str(
                getattr(self.config, "provider_input_sample_rate_hz", "") or ""
            ),
            "output_encoding": self._session_output_encoding,
            "output_sample_rate_hz": str(
                int(
                    self._active_output_sample_rate_hz
                    or getattr(self.config, "output_sample_rate_hz", "")
                    or 0
                )
            ),
            "target_encoding": str(getattr(self.config, "target_encoding", "") or ""),
            "target_sample_rate_hz": str(
                getattr(self.config, "target_sample_rate_hz", "") or ""
            ),
        }
        ...
        try:
            logger.info(
                ...
                provider_input_sample_rate_hz=info_payload[
    ...
    def _handle_session_info_event(self, event: Dict[str, Any]) -> None:
        ...
        if (
            enc_norm in ("g711_ulaw", "ulaw", "mulaw", "mu-law")
            and int(provider_rate or 0) == 8000
        ):
            self._outfmt_acknowledged = True
        ...
        info_payload = {
            ...
            "output_sample_rate_hz": str(
                provider_rate
                or self._active_output_sample_rate_hz
                or getattr(self.config, "output_sample_rate_hz", "")
                or ""
            ),
            "target_encoding": str(getattr(self.config, "target_encoding", "") or ""),
    ...
    def _update_output_meter(self, chunk_bytes: int) -> None:
        ...
        try:
            target_is_ulaw = str(
                getattr(self.config, "target_encoding", "") or ""
            ).lower() in (
                "ulaw",
                "mulaw",
                "g711_ulaw",
        ...
        confirmed_pcm = bool(
            self._outfmt_acknowledged and self._provider_output_format == "pcm16"
        )
        ...
        if now - self._output_meter_last_log_ts >= 1.0:
            ...
            if assumed > 0:
                ...
                if drift > 0.10:
                    try:
                        ...
                        logger.debug(
                            "Failed to log OpenAI output rate info", exc_info=True
                        )
            ...
            try:
                if not self._fallback_pcm24k_done:
                    ...
                    window_anchor = (
                        self._pacer_start_ts
                        if self._pacer_start_ts > 0.0
                        else self._output_meter_start_ts
                    )
        ...
        try:
            while self.websocket and not self.websocket.closed and self._pacer_running:
                ...
                try:
                    ...
                    logger.error(
                        "Failed to emit paced AgentAudio",
                        call_id=call_id,
                        exc_info=True,
                    )
    ...
    async def _switch_to_pcm24k_output(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "Failed to switch OpenAI session to PCM16@24k",
                call_id=call_id,
                exc_info=True,
            )
