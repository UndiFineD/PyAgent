# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\providers\deepgram.py
import array
import asyncio
import audioop
import json
...
class DeepgramProvider(AIProviderInterface):
                ...
                if isinstance(obj, dict):
                    for k, v in obj.items():
                        ...
                        if (
                            ...
                            and re.fullmatch(r"[0-9a-fA-F-]{36}", v or "")
                        ):
                            rid = v
                        if (
                            sid is None
                            and (kl == "session_id" or kl == "sessionid")
                            and isinstance(v, str)
                        ):
                            sid = v
        ...
        try:
            ...
            if sample_rate:
                try:
                    ...
                    logger.debug(
                        "Deepgram output sample rate parse failed",
                        value=sample_rate,
                        exc_info=True,
                    )
        except Exception:
            logger.debug(
                "Deepgram output format update failed",
                encoding=encoding,
    ...
    def __init__(
        ...
        try:
            self._dg_input_rate = int(
                self._get_config_value("input_sample_rate_hz", 8000) or 8000
            )
        except Exception:
            self._dg_input_rate = 8000
        # Cache provider output settings for downstream conversion/metadata
        self._original_output_encoding = (
            self._get_config_value("output_encoding", None) or "mulaw"
        )
        self._original_output_rate = (
            self._get_config_value("output_sample_rate_hz", 8000) or 8000
        )
        self._dg_output_encoding = self._canonicalize_encoding(
            self._original_output_encoding
        )
        try:
        ...
        self.allow_output_autodetect = bool(
            self._get_config_value("allow_output_autodetect", False)
        )
    ...
    def parse_ack(self, event_data: Dict[str, Any]) -> Optional[ProviderCapabilities]:
        ...
        try:
            ...
            input_enc = self._canonicalize_encoding(
                input_audio.get("encoding", "mulaw")
            )
            output_enc = self._canonicalize_encoding(
                output_audio.get("encoding", "mulaw")
            )
    ...
    async def start_session(
        ...
        base_url = (
            getattr(self.config, "voice_agent_base_url", None)
            or "wss://agent.deepgram.com/v1/agent/converse"
        )
        ws_url = base_url
        ...
        try:
            ...
            self.websocket = await websockets.connect(
                ws_url, extra_headers=list(headers.items())
            )
            ...
            try:
                rid = None
                if (
                    hasattr(self.websocket, "response_headers")
                    and self.websocket.response_headers
                ):
    ...
    async def _configure_agent(self):
        ...
        if not greeting_val:
            try:
                greeting_val = (
                    getattr(self.llm_config, "initial_greeting", None) or ""
                ).strip()
            except Exception:
                greeting_val = ""
        if not greeting_val:
        ...
        speak_model = (
            self._get_config_value("tts_model", None)
            or getattr(self.llm_config, "tts_model", None)
            or "aura-asteria-en"
        )
        ...
        think_prompt = (
            self._get_config_value(
                "instructions", None
            )  # Context injection uses 'instructions' for Deepgram
            or self._get_config_value("prompt", None)
            or getattr(self.llm_config, "prompt", None)
        ...
        logger.info(
            ...
            prompt_preview=(
                think_prompt[:80] + "..." if len(think_prompt) > 80 else think_prompt
            ),
        )
        ...
        try:
            ...
            if tools_config.get("enabled", False):
                ...
                if tools_schemas:
                    ...
                    logger.warning(
                        "Tools enabled but no tools registered", call_id=self.call_id
                    )
            else:
                logger.debug("Tools disabled in configuration", call_id=self.call_id)
        except Exception as e:
            logger.warning(
                f"Failed to configure tools: {e}", call_id=self.call_id, exc_info=True
            )
        ...
        async def _inject_greeting_immediate():
            try:
                if (
                    ...
                    and greeting_val
                    and self._greeting_injections < 1
                ):
                    logger.info(
                        "Injecting greeting immediately after Settings",
                        call_id=self.call_id,
                    )
                    ...
                    try:
                        ...
                        logger.debug(
                            "Immediate greeting injection failed", exc_info=True
                        )
            except Exception:
                pass
        ...
        try:
            ...
            logger.warning(
                "Deepgram settings ACK not received within timeout; fallback readiness may be active"
            )
        ...
        async def _inject_greeting_if_quiet():
            try:
                ...
                if (
                    ...
                    and greeting_val
                    and self._greeting_injections < 2
                ):
                    logger.info(
                        "Injecting greeting via fallback as no AgentAudio detected",
                        call_id=self.call_id,
                    )
                    try:
                        self._greeting_injections += 1
    ...
    async def send_audio(
        ...
        if self.websocket and audio_chunk:
            try:
                ...
                input_encoding = (
                    (self._get_config_value("input_encoding", None) or "mulaw")
                    .strip()
                    .lower()
                )
                target_rate = int(
                ...
                if encoding and sample_rate:
                    ...
                    if chunk_len == 160:
                        ...
                        try:
                            src_rate = int(
                                self._get_config_value("input_sample_rate_hz", 0) or 0
                            ) or (16000 if actual_format == "pcm16" else 8000)
                        except Exception:
                            src_rate = 8000
                ...
                if input_encoding in ("ulaw", "mulaw", "g711_ulaw", "mu-law"):
                    if actual_format == "pcm16":
                        try:
                            ...
                            logger.warning(
                                "Failed to convert PCM to μ-law for Deepgram",
                                exc_info=True,
                            )
                            payload = audio_chunk
                    else:
                    ...
                    if target_rate and target_rate != 8000:
                        ...
                        try:
                            ...
                            logger.warning(
                                "Failed to convert resampled PCM back to μ-law",
                                exc_info=True,
                            )
                            payload = audio_chunk
                ...
                if pcm_for_rms is not None:
                    try:
                        ...
                        alpha = 0.2
                        self._rms_ma = (alpha * float(rms)) + (1.0 - alpha) * float(
                            self._rms_ma or 0.0
                        )
                        protect_elapsed = 0.0
                        try:
                            if getattr(self, "_settings_ts", 0.0):
                                protect_elapsed = max(
                                    0.0,
                                    time.monotonic() - float(self._settings_ts or 0.0),
                                )
                        except Exception:
                            protect_elapsed = 0.0
                        ...
                        if gate and rms < threshold:
                            ...
                            if self._low_rms_streak % 10 == 0:
                                ...
                                warnings_count = self._low_rms_warnings_logged.get(
                                    call_id, 0
                                )
                                ...
                                if warnings_count < 3:
                                    ...
                                    self._low_rms_warnings_logged[call_id] = (
                                        warnings_count + 1
                                    )
                ...
                if input_encoding in ("slin16", "linear16", "pcm16"):
                    ...
                    if self._prestream_queue and self._settings_acked:
                        try:
                            ...
                            logger.debug(
                                "Deepgram prestream flush failed", exc_info=True
                            )
                        finally:
                ...
                logger.debug(
                    "Could not send audio packet: Connection closed.",
                    code=e.code,
                    reason=e.reason,
                )
            except Exception:
                logger.error(
                    "An unexpected error occurred while sending audio chunk",
                    exc_info=True,
                )
    ...
    async def _handle_function_call(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            if (
                result.get("function_name") == "hangup_call"
            ...
            try:
                ...
                if function_call_id:
                    ...
                    if self.websocket and not self.websocket.closed:
                        ...
                        logger.info(
                            "Sent error response to Deepgram",
    ...
    def describe_alignment(
        ...
        *,
        ...
        if (
            cfg_enc in ("slin16", "linear16", "pcm16")
            and audiosocket_format != "slin16"
        ):
            issues.append(
    ...
    async def _receive_loop(self):
        ...
        try:
                ...
                if isinstance(message, str):
                    try:
                        ...
                        if et == "SettingsApplied":
                            ...
                            try:
                                if self._settings_ts:
                                    latency_ms = max(
                                        0.0,
                                        (time.monotonic() - float(self._settings_ts))
                                        * 1000.0,
                                    )
                                    if self.call_id:
                                        _DEEPGRAM_SETTINGS_ACK_LATENCY_MS.labels(
                                            self.call_id
                                        ).set(latency_ms)
                                        logger.info(
                                            "Deepgram settings ACK latency",
                                            call_id=self.call_id,
                                            latency_ms=round(latency_ms, 1),
                                        )
                            except Exception:
                                logger.debug(
                                    "Failed to record settings ACK latency",
                                    exc_info=True,
                                )
                        # One-time ACK settings log for effective audio configs (log full payload)
                        try:
                            if getattr(self, "_settings_sent", False) and not getattr(
                                self, "_ack_logged", False
                            ):
                                audio_ack = {}
                                if isinstance(event_data, dict):
                                ...
                                settings_accepted = (
                                    True  # SettingsApplied received = accepted
                                )
                                ...
                                if ack_encoding and ack_rate:
                                    settings_match = (
                                        ack_encoding == requested_encoding
                                        and ack_rate == requested_rate
                                    )
                                ...
                                try:
                                    self._update_output_format(
                                        ack_encoding, ack_rate, source="ack"
                                    )
                                except Exception:
                                    logger.debug(
                                        "Deepgram ACK output parsing failed",
                                        exc_info=True,
                                    )
                                try:
                        ...
                        try:
                            ...
                            logger.debug(
                                "ProviderAudioFormat event emission failed",
                                exc_info=True,
                            )
                        ...
                        try:
                            ...
                            if et == "SettingsApplied":
                                ...
                                func_name = (
                                    functions[0].get("name") if functions else None
                                )
                                logger.info(
                                ...
                                asyncio.create_task(
                                    self._handle_function_call(event_data)
                                )
                            ...
                            if (
                                et == "SettingsApplied"
                                and self._ack_event
                                and not self._ack_event.is_set()
                            ):
                                try:
                                    self._ack_event.set()
                                except Exception:
                            ...
                            if et == "Error":
                                ...
                                if (
                                    not self._settings_retry_attempted
                                    and self._last_settings_minimal
                                    and self.websocket
                                    and not self.websocket.closed
                                ):
                                    try:
                                        ...
                                        logger.warning(
                                            "Deepgram Settings error; retrying with minimal Settings",
                                            call_id=self.call_id,
                                        )
                                        await self.websocket.send(
                                            json.dumps(self._last_settings_minimal)
                                        )
                                        # Do not continue here; allow loop to process next server message
                                    except Exception:
                                        logger.debug(
                                            "Failed to send minimal Settings retry",
                                            exc_info=True,
                                        )
                                else:
                                    try:
                            ...
                            if (
                                ...
                                and et == "ConversationText"
                            ):
                                try:
                                    role = event_data.get("role")
                                    text = event_data.get("text") or event_data.get(
                                        "content"
                                    )
                                    logger.info(
                                    ...
                                    if (
                                        self.call_id
                                        and text
                                        and hasattr(self, "_session_store")
                                        and self._session_store
                                    ):
                                        try:
                                            session = await self._session_store.get_by_call_id(
                                                self.call_id
                                            )
                                            if session:
                                                ...
                                                # Update session
                                                await self._session_store.upsert_call(
                                                    session
                                                )
                                                logger.debug(
                                    ...
                                    logger.debug(
                                        "Deepgram conversation text logging failed",
                                        exc_info=True,
                                    )
                        ...
                        try:
                            ...
                            if (
                                ...
                                if self.websocket and not self.websocket.closed:
                                    logger.info(
                                        "Skipping greeting injection - using Deepgram agent greeting",
                                        call_id=self.call_id,
                                        event_type=et,
                                    )
                        ...
                        logger.error(
                            "Failed to parse JSON message from Deepgram",
                            message=message,
                        )
                    ...
                    try:
                        ...
                        if not getattr(self, "_dg_output_inferred", False):
                            ...
                            if l % 2 == 1:
                                ...
                                try:
                                    rms_pcm = (
                                        audioop.rms(message[: min(960, l - (l % 2))], 2)
                                        if l >= 2
                                        else 0
                                    )
                                except Exception:
                                    rms_pcm = 0
                                try:
                                    pcm_from_ulaw = mulaw_to_pcm16le(
                                        message[: min(320, l)]
                                    )
                                    rms_ulaw = (
                                        audioop.rms(pcm_from_ulaw, 2)
                                        if pcm_from_ulaw
                                        else 0
                                    )
                                except Exception:
            ...
            logger.error(
                "Error receiving events from Deepgram Voice Agent", exc_info=True
            )
        finally:
    ...
    async def speak(self, text: str):
        ...
        try:
            ...
            logger.error(
                "Failed to send inject agent message: Connection is closed.",
                exc_info=True,
                code=e.code,
                reason=e.reason,
            )
    ...
    async def _inject_message_dual(self, text: str):
        ...
        try:
            await self.websocket.send(
                json.dumps({"type": "InjectAgentMessage", "content": text})
            )
        except Exception:
