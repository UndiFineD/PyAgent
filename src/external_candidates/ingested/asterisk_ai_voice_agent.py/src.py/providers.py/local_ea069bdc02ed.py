# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\src\providers\local.py
import asyncio
import base64
import json
...
class LocalProvider(AIProviderInterface):
    ...
    def __init__(
        self, config: LocalProviderConfig, on_event: Callable[[Dict[str, Any]], None]
    ):
        ...
        self.response_timeout = float(
            getattr(config, "response_timeout_sec", 5.0) or 5.0
        )
        ...
        self._pending_tts_responses: Dict[str, asyncio.Future] = (
            {}
        )  # Track pending TTS responses
    ...
    async def _reconnect(self):
        ...
        for attempt, delay in enumerate(backoff_schedule, 1):
            try:
                ...
                logger.info(
                    "✅ Connected to Local AI Server", elapsed=f"{total_elapsed}s"
                )
                ...
                logger.warning(
                    ...
                    next_retry=(
                        f"{delay}s" if attempt < len(backoff_schedule) else "none"
                    ),
                )
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                "Initializing connection to Local AI Server...", url=self.ws_url
            )
            ...
            logger.error(
                "Failed to initialize connection to Local AI Server", exc_info=True
            )
            raise
    ...
    async def start_session(
        ...
        try:
            ...
            if self.websocket and not self.websocket.closed:
                logger.debug(
                    "WebSocket already connected, reusing connection", call_id=call_id
                )
    ...
    async def send_audio(self, audio_chunk: bytes):
        ...
        try:
            logger.info(
                "🎵 PROVIDER INPUT - Sending to Local AI Server",
            ...
            logger.error(
                "Failed to enqueue audio for Local AI Server",
                error=str(e),
                bytes=len(audio_chunk),
                exc_info=True,
            )
    ...
    async def _send_loop(self):
        ...
        while True:
            try:
                ...
                logger.info(
                    "🔄 PROVIDER BATCH - Processing for STT",
                    frames=len(batch),
                ...
                try:
                    ...
                    logger.debug(
                        "WebSocket batch send successful",
                        frames=len(batch),
                    ...
                    logger.warning(
                        "WebSocket closed during send, attempting reconnect",
                    ...
                    if ok:
                        try:
                            await self.websocket.send(msg)
                            logger.debug(
                                "WebSocket resend after reconnect successful",
                                frames=len(batch),
                            )
                        except Exception as e:
                            logger.error(
                                "WebSocket resend failed after reconnect",
                                error=str(e),
                                exc_info=True,
                            )
                except Exception as e:
    ...
    async def play_initial_greeting(self, call_id: str):
        ...
        try:
            ...
            if not greeting_text.strip():
                logger.info(
                    "No initial greeting configured; skipping greeting playback",
                    call_id=call_id,
                )
                return
    ...
    async def stop_session(self):
        ...
        logger.info(
            "Provider session stopped, WebSocket connection and listener maintained. Call ID preserved for TTS processing."
        )
    ...
    async def _receive_loop(self):
        ...
        try:
            async for message in self.websocket:
                ...
                if isinstance(message, bytes):
                    ...
                    if self._active_call_id is None:
                        logger.debug(
                            "Dropping AgentAudio - no active call",
                        ...
                        if data.get("type") == "tts_response":
                            ...
                            if text in self._pending_tts_responses:
                                ...
                                if not future.done():
                                    ...
                                    logger.info(
                                        "TTS response received and delivered",
                                        text=text[:50],
                                    )
                                else:
                                    logger.warning(
                                        "TTS response received but future already completed",
                                        text=text[:50],
                                    )
                            else:
                                logger.warning(
                                    "TTS response received but no pending request found",
                                    text=text[:50],
                                )
                            ...
                            if audio_b64:
                                try:
                                    ...
                                    logger.warning(
                                        "Invalid base64 in tts_response from Local AI Server"
                                    )
                                    audio_bytes = b""

                                if audio_bytes and self.on_event:
                                    target_call_id = (
                                        data.get("call_id") or self._active_call_id
                                    )
                                    if target_call_id:
                                        try:
                                            await self.on_event(
                                            ...
                                            logger.error(
                                                "Failed to emit AgentAudio(/Done) for tts_response",
                                                exc_info=True,
                                            )
                                    else:
                                        logger.debug(
                                            "Dropping TTS audio - no active call to attribute",
                                            size=len(audio_bytes),
                                        )
                        else:
                            logger.debug(
                                "Received JSON message from Local AI Server",
                                message=data,
                            )
                    except json.JSONDecodeError:
                        logger.warning(
                            "Received non-JSON string message from Local AI Server",
                            message=message,
                        )
                else:
                    logger.warning(
                        "Received unknown message type from Local AI Server",
            ...
            if success:
                logger.info(
                    "✅ Reconnected to Local AI Server, restarting receive loop"
                )
    ...
    async def speak(self, text: str):
        ...
        logger.warning(
            "Direct 'speak' method not implemented for this provider. Use the streaming pipeline."
        )
    ...
    async def text_to_speech(self, text: str) -> Optional[bytes]:
        ...
        try:
            ...
            await self.websocket.send(json.dumps(tts_message))
            logger.info(
                "Sent TTS request to Local AI Server",
            ...
            try:
                ...
                if response_data.get("type") == "tts_response" and response_data.get(
                    "audio_data"
                ):
                    ...
                    logger.warning(
                        "Unexpected TTS response format", response=response_data
                    )
                    return None
            ...
            logger.error(
                "Failed to generate TTS", text=text, error=str(e), exc_info=True
            )
            return None
