# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\scripts\test_externalmedia_deployment.py
#!/usr/bin/env python3
"""
...
async def test_configuration():
    ...
    try:
        ...
        if config.audio_transport != "externalmedia":
            logger.warning(
                f"⚠️  Audio transport is '{config.audio_transport}', expected 'externalmedia'"
            )
...
async def test_rtp_server():
    ...
    try:
        ...
        rtp_server = RTPServer(
            ...
            engine_callback=lambda call_id, data: logger.info(
                f"RTP audio received: {call_id}, {len(data)} bytes"
            ),
