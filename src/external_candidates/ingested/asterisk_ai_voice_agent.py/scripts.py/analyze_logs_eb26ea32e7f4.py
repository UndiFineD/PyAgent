# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\scripts\analyze_logs.py
#!/usr/bin/env python3
"""
...
class LogAnalyzer:
    ...
    def analyze(self):
        ...
        print(
            f"⏱️  Duration: {self.data.get('capture_session', {}).get('duration_seconds', 'unknown')} seconds"
        )
        print()
        ...
        for log in logs:
            ...
            if level == "error":
                errors.append(
                    {
                        "timestamp": log.get(
                            "timestamp", log.get("capture_timestamp", "")
                        ),
                        "service": log.get("source_service", "unknown"),
                        "event": log.get("event", "unknown"),
                        "message": log.get("message", ""),
    ...
    def _analyze_audio_flow(self):
        ...
        if audio_events:
            ...
            audio_start = any(
                "audiosocket" in event["event"].lower() for event in audio_events
            )
            # Check for speech detection
            speech_detected = any(
                "speech" in event["event"].lower() for event in audio_events
            )
            # Check for TTS generation
            tts_generated = any(
                "tts" in event["event"].lower() or "playback" in event["event"].lower()
    ...
    def _analyze_ari_events(self):
        ...
        for log in logs:
            ...
            if any(
                keyword in event
                for keyword in ["stasis", "channel", "bridge", "playback", "ari"]
            ):
                ari_events.append(
                    {
                        "timestamp": log.get(
...
"""
    ...
    def _extract_call_phases(self, logs):
        ...
        for log in logs:
            ...
            if any(
                keyword in event for keyword in ["stasis", "call", "channel", "answer"]
            ):
                ...
                keyword in event for keyword in ["audiosocket", "originate", "local"]
            ):
                ...
                keyword in event for keyword in ["connection", "accepted", "peer"]
            ):
                ...
                keyword in event
                for keyword in ["audio", "chunk", "frame", "speech", "vad"]
            ):
                ...
                keyword in event for keyword in ["greeting", "tts", "playback", "speak"]
            ):
    ...
    def _generate_phase_analysis(self, phases):
        ...
        for phase_name, phase_data in phases.items():
            ...
            status_icon = (
                ...
                else "❌" if phase_data["status"] == "failure" else "⚠️"
            )
            status_text = (
                "SUCCESS"
                if phase_data["status"] == "success"
                else "FAILURE" if phase_data["status"] == "failure" else "UNKNOWN"
            )
    ...
    def _generate_next_steps(self):
        ...
        for i, issue in enumerate(self.issues, 1):
            steps.append(
                f"{i}. **Fix {issue['title']}** - {self._get_fix_description(issue)}"
            )
    ...
    def _get_fix_code(self, issue):
        ...
        if issue["type"] == "audio_issue":
            if "AudioSocket" in issue["title"]:
                return (
                    "# Fix AudioSocket handler awaiting\nawait self.on_accept(conn_id)"
                )
            elif "Speech Detection" in issue["title"]:
