...
class ExternalMediaCallTester:
    ...
    def test_pre_call_health(self):
        ...
        ready_providers = [
            name for name, info in providers.items() if info.get("ready", False)
        ]
    ...
    async def test_call_flow(self):
        ...
        call_started = await self.wait_for_condition(
            lambda status: status.get("active_calls", 0) > 0, timeout=60
        )
...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:15000/health", help="Health endpoint URL"
    )
