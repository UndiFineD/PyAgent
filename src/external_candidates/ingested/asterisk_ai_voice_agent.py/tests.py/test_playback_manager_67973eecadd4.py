# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\tests\test_playback_manager.py
"""
...
class TestPlaybackManager:
    ...
    async def test_play_audio_success(self, playback_manager, sample_session):
        ...
        with (
            ...
            patch(
                "os.path.basename", return_value="audio-test_call_123-1234567890.ulaw"
            ),
            patch.object(
                playback_manager, "_create_audio_file", return_value="/tmp/test.ulaw"
            ),
        ):
            ...
            session = await playback_manager.session_store.get_by_call_id(
                "test_call_123"
            )
            ...
            playback_ref = await playback_manager.session_store.get_playback(
                playback_id
            )
    ...
    async def test_play_audio_session_not_found(self, playback_manager):
        ...
        playback_id = await playback_manager.play_audio(
            "nonexistent_call", audio_bytes, "response"
        )

        assert playback_id is None

    @pytest.mark.asyncio
    async def test_play_audio_ari_failure(
        self, playback_manager, sample_session, mock_ari_client
    ):
    ...
    async def test_on_playback_finished_success(self, playback_manager, sample_session):
        ...
        await playback_manager.session_store.set_gating_token(
            "test_call_123", "test_playback_123"
        )
    ...
    async def test_playback_id_generation(self, playback_manager):
        ...
        playback_id_1 = playback_manager._generate_playback_id(
            "test_call_123", "response"
        )
        playback_id_2 = playback_manager._generate_playback_id(
            "test_call_123", "greeting"
        )
        playback_id_3 = playback_manager._generate_playback_id(
            "test_call_456", "response"
        )
    ...
    async def test_audio_file_creation(self, playback_manager):
        ...
        with (
            ...
            file_path = await playback_manager._create_audio_file(
                audio_bytes, playback_id
            )
