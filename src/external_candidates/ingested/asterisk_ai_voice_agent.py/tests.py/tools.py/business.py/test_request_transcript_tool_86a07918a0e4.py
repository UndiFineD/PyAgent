# Extracted from: C:\DEV\PyAgent\.external\Asterisk-AI-Voice-Agent\tests\tools\business\test_request_transcript_tool.py
"""
...
class TestRequestTranscriptTool:
    ...
    async def test_empty_email_address(
        ...
        result = await transcript_tool.execute(
            parameters={"caller_email": ""}, context=tool_context
        )

        assert result["status"] == "error"
        assert (
            "didn't catch" in result["message"].lower()
            or "repeat" in result["message"].lower()
        )
    ...
    async def test_invalid_email_format(
        ...
        result = await transcript_tool.execute(
            parameters={"caller_email": "not an email"}, context=tool_context
        )

        assert result["status"] == "error"
        assert (
            "couldn't understand" in result["message"].lower()
            or "spell it again" in result["message"].lower()
        )
    ...
    async def test_invalid_domain_dns_check(
        ...
        with patch("dns.resolver.resolve") as mock_dns:
            ...
            result = await transcript_tool.execute(
                parameters={
                    "caller_email": "test at invalid-domain-that-does-not-exist dot com"
                },
                context=tool_context,
            )
            ...
            assert (
                ...
                or "valid" in result["message"].lower()
            )
    ...
    @pytest.mark.asyncio
    async def test_tool_disabled(self, transcript_tool, tool_context, tool_config):
        """Test behavior when tool is disabled in config."""
        # Ensure disabled
        tool_config["tools"]["request_transcript"]["enabled"] = False

        result = await transcript_tool.execute(
            parameters={"caller_email": "test at gmail dot com"}, context=tool_context
        )
    ...
    async def test_successful_email_send(
        ...
        with patch("dns.resolver.resolve") as mock_dns:
            ...
            with patch("src.tools.business.request_transcript.resend") as mock_resend:
                ...
                assert (
                    "send" in result["message"].lower()
                    or "transcript" in result["message"].lower()
    ...
    async def test_missing_caller_email_parameter(
        ...
        result = await transcript_tool.execute(
            parameters={}, context=tool_context  # No caller_email
        )
