    ...
    for v in reversed(team.users):
        ...
        records.append(
            schemas.team.TeamMember(
                user_id=v.user_id, name=h[v.user_id].name, role=v.role
            )
        )
...
async def remove_member(
    ...
    if (
        (
            user.role == schemas.user.UserType.ADMIN
            or user.role == schemas.user.UserType.SUPER_ADMIN
        )
        or (
        ...
        or (
            me_role == schemas.team.TeamMemberRole.ADMIN
            and kicked_user_role == schemas.team.TeamMemberRole.USER
        )
    ):
        pass
    else:
    ...
    if user_id not in default_users_ids:
        default_users.append(
            schemas.team.TeamMember(
                user_id=user_id, name="", role=schemas.team.TeamMemberRole.USER
            )
        )
