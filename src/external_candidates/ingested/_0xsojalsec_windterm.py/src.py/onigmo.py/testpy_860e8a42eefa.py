...
def xx(pattern, target, s_from, s_to, mem, not_match):
    ...
    if r != 0:
        ...
        nerror += 1
        print_result(
            "ERROR", "%s (/%s/ '%s')" % (msg.value, pattern, target), file=sys.stderr
        )
        return
...
def main():
    ...
    def get_text_writer(fileno, **kwargs):
        ...
        writer.write = lambda s: (
            write(s.decode(enc)) if isinstance(s, bytes) else write(s)
        )  # convert to unistr
        return writer
    ...
    x2(
        "ab(?=c\Kd)", "abcd", 2, 2
    )  # This behaviour is currently not well defined. (see: perlre)
    x2(
        "(?<=a\\Kb|aa)cd", "abcd", 1, 4
    )  # This behaviour is currently not well defined. (see: perlre)
    x2(
