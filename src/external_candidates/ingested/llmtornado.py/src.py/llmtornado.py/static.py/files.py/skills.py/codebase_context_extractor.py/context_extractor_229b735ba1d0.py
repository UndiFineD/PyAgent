# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.Tests\Static\Files\Skills\codebase-context-extractor\context_extractor.py
#!/usr/bin/env python3
"""
    ...
    def extract_imports(self) -> List[str]:
        ...
        try:
            ...
            for match in re.finditer(import_pattern, self.content):
                ...
                imports.extend(
                    [imp.strip().split()[0] for imp in match.group(2).split(",")]
                )
        return imports
...
class CodebaseExtractor:
    ...
    def _detect_entry_points(self, file_contexts: List[FileContext]) -> List[str]:
        ...
        for fc in file_contexts:
            ...
            if any(
                name in fc.path.lower() for name in ["main", "index", "app", "__init__"]
            ):
    ...
    def extract_targeted_context(self, focus: str) -> Dict[str, Any]:
        ...
        files = self._collect_files()
        results = {"focus": focus, "matches": [], "related_files": []}
            ...
            for entity in entities:
                if focus.lower() in entity.name.lower():
                    ...
                    results["related_files"].append(
                        str(file_path.relative_to(self.target_path))
                    )
    ...
    def extract_dependency_graph(self) -> Dict[str, Any]:
        ...
        graph = {"nodes": [], "edges": [], "circular_dependencies": []}
        ...
        for source, targets in context.dependency_graph.items():
            for target in targets:
                graph["edges"].append({"from": source, "to": target})

        # Detect circular dependencies
        graph["circular_dependencies"] = self._find_circular_deps(
    ...
    def format_markdown(context: CodebaseContext) -> str:
        ...
        for dir_name in sorted(files_by_dir.keys()):
            ...
            for fc in files_by_dir[dir_name]:
                lines.append(
                    f"- **{Path(fc.path).name}** ({fc.language}, {fc.lines_of_code} lines)"
                )
                ...
                if fc.entities:
                    ...
                    for entity in fc.entities:
                        entity_summary[entity.type] = (
                            entity_summary.get(entity.type, 0) + 1
                        )
        ...
        for entity_type in ["class", "function", "method"]:
            if entity_type in by_type:
                ...
                for entity in sorted(by_type[entity_type], key=lambda e: e.name)[
                    :20
                ]:  # Top 20
...
def main():
    ...
    parser.add_argument(
        "--target-path", required=True, help="Path to the codebase to analyze"
    )
    parser.add_argument(
        "--mode",
        required=True,
