# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.Tests\Static\Files\Skills\skill-creator\scripts\skill_generator.py
#!/usr/bin/env python3
"""
...
class SkillGenerator:
    ...
    def _generate_skill_md(
        self, skill_dir: Path, name: str, description: str, steps: List[Dict]
    ):
        """Generate the SKILL.md file"""

        # Convert steps to checklist items
        checklist_items = [
            f"- [ ] Step {i+1}: {step['name']}" for i, step in enumerate(steps)
        ]
...
"""
    ...
    def _generate_readme(
        ...
        step_list = "\n".join(
            [
                f"{i+1}. **{step['name']}**: {step['description'][:100]}..."
                for i, step in enumerate(steps)
            ]
        )
