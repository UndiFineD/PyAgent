# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\gui\root_management_dialog.py
"""
...
class RootManagementDialog(QDialog):
    ...
    def _create_status_tab(self) -> QWidget:
        ...
        self.pause_resume_btn = QPushButton(
            "Resume" if self.root_state.paused else "Pause"
        )
    ...
    def _create_ignore_tab(self) -> QWidget:
        ...
        refresh_btn.clicked.connect(
            lambda: asyncio.create_task(self._refresh_file_tree_async())
        )
        ...
        self.file_tree.setHeaderLabels(
            ["File/Folder", "Status", "Chunks", "Reason", "Actions"]
        )
        ...
        self.file_tree.itemExpanded.connect(
            lambda: QTimer.singleShot(0, self._create_visible_buttons)
        )
        ...
        QTimer.singleShot(
            100, lambda: asyncio.create_task(self._refresh_file_tree_async())
        )
    ...
    async def _refresh_file_tree_async(self):
        ...
        try:
            ...
            branch_name = (
                self.root_state.git_tracker.get_current_branch()
                if self.root_state.git_tracker
                else "main"
            )
            file_chunk_counts = (
                await self.indexing_engine.chroma_store.get_file_chunk_counts(
                    self.root_path, branch_name
                )
            )
            ...
            root_item = QTreeWidgetItem(
                self.file_tree, [str(self.root_path.name), "", "", ""]
            )
            ...
            while True:
                ...
                try:
                    batch_files = await loop.run_in_executor(
                        None, lambda: file_queue.get(timeout=0.1)
                    )
                except queue.Empty:
                ...
                for file_path in batch_files:
                    ...
                    file_item = QTreeWidgetItem(
                        parent_item, [file_path.name, status, chunk_str, reason, ""]
                    )

                    # Store file path in item data for later retrieval
                    file_item.setData(0, Qt.ItemDataRole.UserRole, str(file_path))
                    file_item.setData(
                        0, Qt.ItemDataRole.UserRole + 1, is_indexed
                    )  # Store indexed status
    ...
    def _edit_fskbignore(self):
        ...
        if not fskbignore_path.exists():
            try:
                fskbignore_path.write_text(
                    "# Add patterns to ignore (one per line)\n", encoding="utf-8"
                )
            except Exception as e:
    ...
    def _view_gitignore(self):
        ...
        if not gitignore_path.exists():
            QMessageBox.information(
                self, "Not Found", ".gitignore does not exist in this root."
            )
            return
    ...
    def _open_file_editor(self, file_path: Path, title: str, read_only: bool = False):
        ...
        if not read_only:
            ...
            save_btn.clicked.connect(
                lambda: self._save_file(file_path, text_edit.toPlainText(), dialog)
            )
    ...
    async def _show_file_chunks(self, file_path: Path):
        ...
        try:
            ...
            collection = (
                await self.indexing_engine.chroma_store.get_or_create_collection(
                    self.root_path
                )
            )
            ...
            if not results or not results["ids"]:
                QMessageBox.information(
                    self, "No Chunks", f"No chunks found for {file_path.name}"
                )
                return
            ...
            for i, chunk in enumerate(chunks, 1):
                ...
                html_parts.append(
                    f'<span style="color: #ffff00; font-weight: bold;">{divider}</span>\n'
                )
    ...
    def _reindex_file(self, file_path_str: str):
        ...
        try:
            ...
            priority_item = (
                0,
                self.indexing_engine._queue_counter,
                (self.root_path, rel_path, "modified"),
            )
            ...
            QMessageBox.information(
                self, "Queued", f"{file_path.name} queued for reindexing"
            )
            ...
            while iterator.value():
                ...
                if item_rect.isValid() and viewport_rect.intersects(item_rect):
                    ...
                    if file_path_str and id(item) not in self._items_with_buttons:
                        ...
                        reindex_btn.clicked.connect(
                            lambda checked, fp=file_path_str: self._reindex_file(fp)
                        )
    ...
    def _show_context_menu(self, position):
        ...
        if is_indexed:
            ...
            show_chunks_action.triggered.connect(
                lambda: asyncio.create_task(self._show_file_chunks(Path(file_path_str)))
            )
