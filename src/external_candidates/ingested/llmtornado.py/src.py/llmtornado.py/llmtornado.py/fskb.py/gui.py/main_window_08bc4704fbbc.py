# Extracted from: C:\DEV\PyAgent\.external\LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\gui\main_window.py
"""
...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.debug_panel_checkbox = QCheckBox(
            "Show Debug Panel (for testing chunk similarity)"
        )
...
class MainWindow(QMainWindow):
    ...
    def _create_roots_tab(self) -> QWidget:
        ...
        self.roots_table.setEditTriggers(
        ...
        header.setSectionResizeMode(
            6, header.ResizeMode.Interactive
        )  # Status - user can resize
    ...
    def _create_search_tab(self) -> QWidget:
        ...
        self.debug_hash_input.setPlaceholderText(
            "Enter full content hash (from chunk viewer)..."
        )
        ...
        results_header.setSectionResizeMode(
            0, results_header.ResizeMode.Interactive
        )  # File - user can resize
        results_header.setSectionResizeMode(
            1, results_header.ResizeMode.Interactive
        )  # Lines - auto-sized, then user can resize
        results_header.setSectionResizeMode(
            2, results_header.ResizeMode.Interactive
        )  # Score - auto-sized, then user can resize
        results_header.setSectionResizeMode(
            3, results_header.ResizeMode.Interactive
        )  # Preview - auto-sized, then user can resize
    ...
    def _add_root(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "Select Root Directory", str(Path.home())
        )
    ...
    async def _add_root_async(self, root_path: Path):
        ...
        try:
            self.statusBar().showMessage(
                f"Adding root: {root_path} (scanning files...)", 0
            )
            ...
            if success:
                ...
                self.statusBar().showMessage(
                    f"Added root: {root_path} - indexing in background", 5000
                )
    ...
    def _on_search_settings_changed(self):
        ...
        try:
            ...
            logger.debug(
                f"Search settings updated: top_k={self.settings.search.top_k}, min_similarity={self.settings.search.min_similarity}"
            )
        except Exception as e:
    ...
    def _debug_test_similarity(self):
        ...
        asyncio.create_task(
            self._debug_test_similarity_async(query, content_hash, root_path, task)
        )

    async def _debug_test_similarity_async(
        self, query: str, content_hash: str, root_path: Path, task: str = None
    ):
        ...
        try:
            ...
            query_embedding = await self.query_engine.embedding_provider.embed_single(
                query, is_query=True, task=task
            )

            # Get collection for this root
            collection = await self.query_engine.chroma_store.get_or_create_collection(
                root_path
            )
            ...
            if not results["ids"]:
                self.debug_result_label.setText(
                    f"Score: - (hash not found: {content_hash[:16]}...)"
                )
                return
            ...
            if chunk_array is None or len(chunk_array) == 0:
                ...
                logger.warning(
                    f"Debug: No embedding found for hash {content_hash[:16]}"
                )
                return
            ...
            if chunk_norm_value == 0:
                self.debug_result_label.setText(
                    f"Score: - (chunk has zero embedding, dim={len(chunk_array)})"
                )
                logger.warning(
                    f"Debug: Chunk embedding is all zeros (dim={len(chunk_array)}) for hash {content_hash[:16]}"
                )
                return
            ...
            logger.info(
                f"Debug similarity test: query='{query[:50]}...', hash={content_hash[:16]}, score={similarity:.4f}, dims: query={len(query_embedding)} chunk={len(chunk_array)}"
            )
    ...
    def _update_ui_stats(self):
        ...
        try:
            ...
            for root_path, root_state in self.indexing_engine.roots.items():
                ...
                is_initializing = (
                    root_state.stats.files_scanned == 0
                    and root_state.stats.current_file is not None
                ) or (
                    root_state.stats.current_file
                    and any(
                        root_state.stats.current_file.startswith(prefix)
                        for prefix in [
                            "Initializing",
                            "Loading",
                            "Scanning",
                ...
                chunks_loading = (
                    root_state.stats.files_indexed > 0
                    and root_state.stats.chunks_created == 0
                    and root_state.stats.current_file is not None
                )
                ...
                if root_state.paused:
                    ...
                    if root_state.stats.current_file.startswith(
                ...
                eta_text = "-"
                if (
                    not root_state.paused
                    and queued > 0
                    and root_state.stats.indexing_start_time
                ):
                    import time

                    elapsed = time.time() - root_state.stats.indexing_start_time
                    files_indexed_since_start = (
                        root_state.stats.files_indexed
                ...
                current_snapshot[str(root_path)] = {
                    ...
                    "scanned": (
                        "Loading..."
                        if is_initializing
                        else str(root_state.stats.files_scanned)
                    ),
                    "indexed": (
                        "Loading..."
                        if is_initializing
                        else str(root_state.stats.files_indexed)
                    ),
                    "chunks": (
                        "Loading..."
                        if (is_initializing or chunks_loading)
                        else str(root_state.stats.chunks_created)
                    ),
            ...
            for root_path_str, data in current_snapshot.items():
                if root_path_str in root_to_row:
                    ...
                    manage_button.clicked.connect(
                        lambda checked, root=rp: self._open_root_management(root)
                    )
    ...
    def _on_search_completed(self, results: list):
        ...
        for result in results:
            ...
            self.results_table.setItem(
                row, 1, QTableWidgetItem(f"{result['line_start']}-{result['line_end']}")
            )
            self.results_table.setItem(
                row, 2, QTableWidgetItem(f"{result['score']:.3f}")
            )
        ...
        if len(results) > 0:
            ...
            remaining_width = (
                table_width - lines_width - score_width - 20
            )  # -20 for margins
        ...
        if selected_row >= 0 and selected_row < self.results_table.rowCount():
            self.results_table.setCurrentCell(
                selected_row, self.results_table.currentColumn()
            )
