...
skip_if_no_gemini = pytest.mark.skipif(
    not GEMINI_API_KEY,
    reason=(
        "Gemini API key not available (set GEMINI_API_KEY or" " LANGEXTRACT_API_KEY)"
    ),
)
...
def assert_valid_char_intervals(test_case, result):
    ...
    for extraction in result.extractions:
        ...
        test_case.assertIsNotNone(
            ...
            "Missing alignment_status for extraction:" f" {extraction.extraction_text}",
        )
...
class TestLiveAPIGemini(unittest.TestCase):
    ...
    def test_medication_extraction(self):
        ...
        self.assertTrue(
            any(
                re.search(r"\b400\s*mg\b", text, re.IGNORECASE) for text in dosage_texts
            ),
        ...
        self.assertTrue(
            any(
                re.search(r"\b(PO|oral)\b", text, re.IGNORECASE) for text in route_texts
            ),
    ...
    @pytest.mark.xfail(
        reason=(
            "Known tokenizer issue with non-Latin characters - see GitHub" " issue #13"
        ),
        strict=True,
    )
