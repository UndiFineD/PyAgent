# Extracted from: C:\DEV\PyAgent\.external\awesome-agentic-patterns\scripts\build_readme.py
#!/usr/bin/env python3
"""
build_readme.py
    ...
    for category_name in sorted(category_map.keys(), key=lambda c: c.lower()):
        ...
        for p in sorted(category_map[category_name], key=lambda x: x["title"].lower()):
            ...
            if pattern_filename in new_patterns:
                lines.append(
                    f"- [{p['title']}]({p['path']}) <span class='new-badge'>NEW</span>\n"
                )
            elif pattern_filename in updated_patterns:
                lines.append(
                    f"- [{p['title']}]({p['path']}) <span class='updated-badge'>UPDATED</span>\n"
                )
            else:
...
def update_readme_readandreplace(readme_path, new_section_md):
    ...
    if start_marker not in content or end_marker not in content:
        print(
            "Error: Could not find AUTO-GENERATED markers in README.md", file=sys.stderr
        )
        sys.exit(1)

    # Build the replacement block
    replacement = f"{start_marker}\n\n" f"{new_section_md}" f"{end_marker}"
...
def generate_mkdocs_nav(patterns, new_patterns=None, updated_patterns=None):
    ...
    for category_name in sorted_categories:
        ...
        for p in sorted(category_map[category_name], key=lambda x: x["title"].lower()):
            ...
            if pattern_filename in new_patterns:
                nav_lines.append(
                    f"          - \"{p['title']} <span class='new-badge'>NEW</span>\": \"{p['path']}\"\n"
                )
            elif pattern_filename in updated_patterns:
                nav_lines.append(
                    f"          - \"{p['title']} <span class='updated-badge'>UPDATED</span>\": \"{p['path']}\"\n"
                )
            else:
...
def main():
    ...
    if not os.path.isdir(patterns_dir):
        print(
            f"Error: patterns/ directory not found at {patterns_dir}", file=sys.stderr
        )
        sys.exit(1)
    ...
    new_patterns, updated_patterns = get_pattern_labels_git_based(
        repo_root, patterns_dir
    )
    ...
    if new_patterns:
        print(
            f"🆕 {len(new_patterns)} patterns marked as NEW (git-based): {', '.join(sorted(new_patterns))}"
        )

    if updated_patterns:
        print(
            f"🔄 {len(updated_patterns)} patterns marked as UPDATED (git-based): {', '.join(sorted(updated_patterns))}"
        )
