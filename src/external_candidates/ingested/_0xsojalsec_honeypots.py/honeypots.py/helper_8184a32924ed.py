# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-honeypots\honeypots\helper.py
"""
//  -------------------------------------------------------------
//  author        Giga
...
from logging import DEBUG, INFO, Formatter, Handler, Logger, addLevelName, getLogger
from logging.handlers import RotatingFileHandler, SysLogHandler
from os import devnull, getuid, makedirs, path, scandir
        ...
        if custom_filter is not None:
            ...
            if isinstance(record.msg, Mapping):
                ...
                if custom_filter["honeypots"]["contains"]:
                    if not all(
                        k in record.msg for k in custom_filter["honeypots"]["contains"]
                    ):
                        return None
        if isinstance(record.msg, Mapping):
...
def setup_logger(name, temp_name, config, drop=False):
    ...
    if "db_postgres" in logs or "db_sqlite" in logs:
        ret_logs_obj.addHandler(
            CustomHandler(temp_name, logs, custom_filter, config_data, drop)
        )
            ...
            if config_data is not None:
                if "honeypots" in config_data:
                    ...
                    if temp_server_name in config_data["honeypots"]:
                        if (
                            "log_file_name"
                            in config_data["honeypots"][temp_server_name]
                        ):
                            temp_name = config_data["honeypots"][temp_server_name][
                                "log_file_name"
                            ]
                        if "max_bytes" in config_data["honeypots"][temp_server_name]:
                            max_bytes = config_data["honeypots"][temp_server_name][
                                "max_bytes"
                            ]
...
def close_port_wrapper(server_name, ip, port, logs):
    ...
    if sock.connect_ex((ip, port)) != 0 and ret:
        ...
        logs.error(
            {"server": server_name, "error": "port_open.. {} still open..".format(ip)}
        )
        return False
...
class CustomHandler(Handler):
    ...
    def emit(self, record):
        try:
                    ...
                    if isinstance(record.msg, list):
                        if record.msg[0] == "sniffer" or record.msg[0] == "errors":
                            self.db["db_postgres"].insert_into_data_safe(
                                record.msg[0],
                                dumps(
                                    serialize_object(record.msg[1]), cls=ComplexEncoder
                                ),
                            )
                        ...
                        if "server" in record.msg:
                            self.db["db_postgres"].insert_into_data_safe(
                                "servers",
                                dumps(serialize_object(record.msg), cls=ComplexEncoder),
                            )
            ...
            stdout.write(
                dumps(
                    {"error": repr(e), "logger": repr(record)},
                    sort_keys=True,
                    cls=ComplexEncoder,
                )
                + "\n"
    ...
    def drop_db(self):
        with suppress(Exception):
            ...
            if self.check_db_if_exists():
                ...
                sleep(2)
            self.cur.execute(
                sql.SQL("CREATE DATABASE {}").format(sql.Identifier(self.db))
            )
...
def server_arguments():
    ...
    _server_parsergroupdef.add_argument(
        "--uuid", type=str, help="unique id", required=False
    )
