# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Rope\rope\GUIElements.py
...
class Timeline:
    def __init__(self, parent, widget, temp_toggle_swapper, add_action):
        ...
        self.slider.bind(
            "<ButtonPress-1>", lambda e: self.update_timeline_handle(e, True)
        )
        self.slider.bind(
            "<ButtonRelease-1>", lambda e: self.update_timeline_handle(e, True)
        )
        ...
        self.entry_width = 40
        self.entry = tk.Entry(
            self.parent, style.entry_3, textvariable=self.entry_string
        )
    ...
    def draw_timeline(self):
        ...
        self.slider.create_rectangle(
            trough_x1, trough_y1, trough_x2, trough_y2, fill="#43474D", outline=""
        )
        self.handle = self.slider.create_rectangle(
            line_x1, line_y1, line_x2, line_y2, fill="#FFFFFF", outline=""
        )

    def coord2pos(self, coord):
        return (
            float(coord - self.slider_left)
            * self.max_
    ...
    def pos2coord(self, pos):
        return float(
            float(pos) * (self.slider_right - self.slider_left) / self.max_
            + self.slider_left
        )
    ...
    def update_timeline_handle(self, event, also_update_entry=False):
        ...
        if position != self.last_position:
            ...
            self.slider.move(
                self.handle,
                self.pos2coord(position) - self.pos2coord(self.last_position),
                0,
            )
...
class Slider2:
    ...
    def update_handle(self, event, also_update_entry=False, request_frame=True):
        ...
        if position != self.amount:
            # Move handle to coordinate based on position
            self.slider.move(
                self.handle, self.pos2coord(position) - self.pos2coord(self.amount), 0
            )
...
class Text_Entry:
    def __init__(
        ...
        self.entry = tk.Entry(
            self.ts_frame, style.entry_2, textvariable=self.entry_text
        )
        self.entry.place(
            x=self.text_width + 20,
            y=0,
            width=self.width - self.text_width - 50,
            height=15,
        )
