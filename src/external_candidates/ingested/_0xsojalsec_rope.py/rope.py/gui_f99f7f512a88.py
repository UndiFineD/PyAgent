# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Rope\rope\GUI.py
import bisect
import copy
import json
import os
...
class GUI(tk.Tk):
    ...
    def create_gui(self):
        ...
        # Left
        # Label
        self.layer["topleft"] = tk.Frame(
            top_frame, style.canvas_frame_label_1, height=42
        )
        ...
        self.output_videos_text = GE.Text(
            self.layer["topleft"], "", 1, 240, 20, 190, 20
        )
        ...
        ### Videos and Faces
        self.layer["InputVideoFrame"] = tk.Frame(
            middle_frame, style.canvas_frame_label_3
        )
        self.layer["InputVideoFrame"].grid(
        ...
        # Button Frame
        frame = tk.Frame(
            self.layer["InputVideoFrame"], style.canvas_frame_label_2, height=42
        )
        ...
        # Preview Data
        preview_data = tk.Frame(
            self.layer["preview_column"], style.canvas_frame_label_2, height=24
        )
        ...
        # Timeline
        # Slider
        self.layer["slider_frame"] = tk.Frame(
            self.layer["preview_column"], style.canvas_frame_label_2, height=20
        )
        self.layer["slider_frame"].grid(row=2, column=0, sticky="NEWS", pady=0)
        self.video_slider = GE.Timeline(
            self.layer["slider_frame"],
            self.widget,
            self.temp_toggle_swapper,
            self.add_action,
        )

        # Markers
        self.layer["markers_canvas"] = tk.Canvas(
            self.layer["preview_column"], style.canvas_frame_label_2, height=20
        )
        self.layer["markers_canvas"].grid(row=3, column=0, sticky="NEWS")
        self.layer["markers_canvas"].bind(
            "<Configure>", lambda e: self.update_marker(e.width)
        )

        # self.create_ui_button('ToggleStop', marker_frame, 140, 2, width=36, height=36)

        # Controls
        self.layer["preview_frame"] = tk.Frame(
            self.layer["preview_column"], style.canvas_bg, height=40
        )
        ...
        # Left Side
        self.layer["play_controls_left"] = tk.Frame(
            self.layer["preview_frame"],
            style.canvas_frame_label_2,
            height=30,
            width=100,
        )
        self.layer["play_controls_left"].grid(row=0, column=0, sticky="NEWS", pady=0)
        self.widget["SaveImageButton"] = GE.Button(
            self.layer["play_controls_left"],
            "SaveImageButton",
            2,
            self.save_image,
            None,
        ...
        # Images
        self.layer["image_controls"] = tk.Frame(
            self.layer["preview_column"], style.canvas_frame_label_2, height=80
        )
        self.layer["image_controls"].grid(
            row=2, column=0, rowspan=2, sticky="NEWS", pady=0
        )
        self.widget["SaveImageButton"] = GE.Button(
            self.layer["image_controls"],
            "SaveImageButton",
            2,
            self.save_image,
            None,
        ...
        self.widget["AutoSwapButton"] = GE.Button(
            self.layer["image_controls"],
            "AutoSwapButton",
            2,
            self.toggle_auto_swap,
            None,
            "control",
            x=150,
            y=5,
        ...
        # FaceLab
        self.layer["FaceLab_controls"] = tk.Frame(
            self.layer["preview_column"], style.canvas_frame_label_2, height=80
        )
        self.layer["FaceLab_controls"].grid(
            row=2, column=0, rowspan=2, sticky="NEWS", pady=0
        )
        ...
        self.layer["parameter_frame"] = tk.Frame(
            middle_frame, style.canvas_frame_label_3, bd=0, width=width
        )
        self.layer["parameter_frame"].grid(
            row=0, column=2, sticky="NEWS", pady=0, padx=1
        )
        ...
        self.layer["parameters_canvas"] = tk.Canvas(
            self.layer["parameter_frame"], style.canvas_frame_label_3, bd=0, width=width
        )
        self.layer["parameters_canvas"].grid(
            row=1, column=0, sticky="NEWS", pady=0, padx=0
        )

        self.layer["parameters_frame"] = tk.Frame(
        ...
        self.layer["parameters_frame"].grid(
            row=0, column=0, sticky="NEWS", pady=0, padx=0
        )

        self.layer["parameters_canvas"].create_window(
            0, 0, window=self.layer["parameters_frame"], anchor="nw"
        )

        self.layer["parameter_scroll_canvas"] = tk.Canvas(
            self.layer["parameter_frame"],
            style.canvas_frame_label_3,
            bd=0,
        )
        self.layer["parameter_scroll_canvas"].grid(
            row=1, column=1, sticky="NEWS", pady=0
        )
        ...
        self.static_widget["parameters_scrollbar"] = GE.Scrollbar_y(
            self.layer["parameter_scroll_canvas"], self.layer["parameters_canvas"]
        )
        ...
        row += top_border_delta
        self.static_widget["10"] = GE.Separator_x(
            self.layer["parameters_frame"], 0, row
        )
        row += bottom_border_delta
        ...
        row += top_border_delta
        self.static_widget["12"] = GE.Separator_x(
            self.layer["parameters_frame"], 0, row
        )
        row += bottom_border_delta
        ...
        row += row_delta
        self.widget["ColorGammaSlider"] = GE.Slider2(
            self.layer["parameters_frame"],
            "ColorGammaSlider",
            "Gamma",
            3,
        ...
        row += row_delta
        self.widget["MergeTextSel"] = GE.TextSelection(
            self.layer["parameters_frame"],
            "MergeTextSel",
            "Merge Math",
            3,
            self.select_input_faces,
            "merge",
            "",
            398,
            20,
        ...
        row += row_delta
        self.widget["SwapperTypeTextSel"] = GE.TextSelection(
            self.layer["parameters_frame"],
            "SwapperTypeTextSel",
            "Swapper Resolution",
            3,
        ...
        self.layer["tooltip_frame"] = tk.Frame(
            self.layer["parameter_frame"], style.canvas_frame_label_3, height=80
        )
        ...
        self.layer["facelab_canvas"] = tk.Canvas(
            self.layer["parameter_frame"], style.canvas_frame_label_3, bd=0, width=width
        )
        self.layer["facelab_canvas"].grid(
            row=1, column=0, sticky="NEWS", pady=0, padx=0
        )
        #
        self.layer["facelab_frame"] = tk.Frame(
        ...
        #
        self.layer["facelab_canvas"].create_window(
            0, 0, window=self.layer["facelab_frame"], anchor="nw"
        )
        #
        self.layer["facelab_scroll_canvas"] = tk.Canvas(
            self.layer["parameter_frame"],
            style.canvas_frame_label_3,
            bd=0,
        )
        ...
        self.static_widget["facelab_scrollbar"] = GE.Scrollbar_y(
            self.layer["facelab_scroll_canvas"], self.layer["facelab_canvas"]
        )
        # row_delta = 20
        # row = 1
        # for i in range(512):
        ...
        self.status_left_label.bind(
            "<Button-1>", lambda e: self.callback("https://discord.gg/EcdVAFJzqp")
        )
        ...
        self.status_label.bind(
            "<Button-1>", lambda e: self.callback("https://github.com/Hillobar/Rope")
        )

        self.donate_label = tk.Label(
            bottom_frame,
            style.donate_1,
            text="Enjoy Rope? Please Support! (Paypal) ",
            anchor="e",
        )
    ...
    def preview_control(self, event):
        ...
        if (
            self.focus_get() != self.widget["CLIPTextEntry"]
            and self.focus_get() != self.merged_embeddings_text
        ):
                ...
                if file_type == "image":
                    ...
                    if img is not None:
                        ...
                        try:
                            ...
                            self.source_faces[-1]["Image"] = ImageTk.PhotoImage(
                                image=Image.fromarray(crop)
                            )
                            self.source_faces[-1]["Embedding"] = face_emb
                            self.source_faces[-1]["TKButton"] = tk.Button(
                                self.source_faces_canvas,
                                style.media_button_off_3,
                            ...
                            self.source_faces[-1]["TKButton"].bind(
                                "<ButtonRelease-1>",
                                lambda event, arg=len(
                                    self.source_faces
                                ) - 1: self.select_input_faces(event, arg),
                            )
                            self.source_faces[-1]["TKButton"].bind(
                            ...
                            self.static_widget[
                                "input_faces_scrollbar"
                            ].resize_scrollbar(None)
                            i = i + 1
    ...
    def toggle_swapper(self, toggle_value=-1):
        ...
        if (
            self.widget["PreviewModeTextSel"].get() == "Video"
            or self.widget["PreviewModeTextSel"].get() == "Theater"
        ):
    ...
    def set_view(self, load_target_videos, b):
        ...
        if self.widget["PreviewModeTextSel"].get() == "Video":
            ...
            self.layer["facelab_scroll_canvas"].grid(
                row=1, column=1, sticky="NEWS", pady=0
            )
            self.layer["InputVideoFrame"].grid(
                row=0, column=0, sticky="NEWS", padx=1, pady=0
    ...
    def update_marker(self, action):
        ...
        if action == "add":
            ...
            temp = {
                ...
                "icon_ref": self.layer["markers_canvas"].create_line(
        ...
        # resize canvas
            ...
            for marker in self.markers:
                ...
                marker["icon_ref"] = self.layer["markers_canvas"].create_line(
                    position, 0, position, 15, fill="light goldenrod"
                )
    ...
    def save_image(self):
        ...
        cv2.imwrite(
            filename + ".png", cv2.cvtColor(self.video_image, cv2.COLOR_BGR2RGB)
        )
    ...
    def findCosineDistance2(self, vector1, vector2):
        cos_dist = 1.0 - np.dot(vector1, vector2) / (
            np.linalg.norm(vector1) * np.linalg.norm(vector2)
        )  # 2..0
