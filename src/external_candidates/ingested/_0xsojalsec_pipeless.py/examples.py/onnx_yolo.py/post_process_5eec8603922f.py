# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pipeless\examples\onnx-yolo\post-process.py
import cv2
import numpy as np
...
def hook(frame_data, _):
    ...
    if len(model_output) > 0:
        ...
        boxes, scores, class_ids = postprocess_yolo(
            frame.shape, yolo_input_shape, model_output.get("output0", [])
        )
        class_labels = [yolo_classes[id] for id in class_ids]
        for i in range(len(boxes)):
            draw_bbox(
                frame, boxes[i], class_labels[i], scores[i], color_palette[class_ids[i]]
            )
...
def draw_bbox(
    image, box, label="", score=None, color=(255, 0, 255), txt_color=(255, 255, 255)
):
    ...
    if label:
        ...
        w, h = cv2.getTextSize(str(label), 0, fontScale=lw / 3, thickness=tf)[
            0
        ]  # text width, height
        outside = p1[1] - h >= 3
