# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pipeless\examples\yolo-world\pre-process.py
import cv2
import numpy as np
...
def resize_and_pad(frame, target_dim, pad_top, pad_bottom, pad_left, pad_right):
    ...
    if is_cuda_available():
        ...
        resized_frame_gpu = cv2.cuda.resize(
            frame_gpu, (target_width, target_height), interpolation=cv2.INTER_CUBIC
        )
...
def resize_with_padding(frame, target_dim):
    ...
    padded_image = resize_and_pad(
        frame, (new_height, new_width), pad_height, pad_height, pad_width, pad_width
    )
    return padded_image
