...
class SettingsManager:
        ...
        with self._lock:
            try:
                if os.path.exists(self.settings_file):
                    ...
                    # Get current style
                    current_style = settings.get(
                        "current_waveform_style", config.CURRENT_WAVEFORM_STYLE
                    )
                    ...
                    if current_style not in all_configs:
                        logging.warning(
                            f"Invalid current style '{current_style}', falling back to default"
                        )
    ...
    def save_waveform_style_settings(
        self, current_style: str, style_configs: Dict[str, Dict]
    ) -> None:
        ...
        with self._lock:
            ...
            if current_style not in style_configs:
                raise ValueError(
                    f"current_style '{current_style}' not found in style_configs"
                )
            ...
            for style_name, config_dict in style_configs.items():
                if style_name not in valid_styles:
                    raise ValueError(
                        f"Unknown style '{style_name}'. Valid styles: {valid_styles}"
                    )
                if not isinstance(config_dict, dict):
                    raise ValueError(
                        f"Configuration for style '{style_name}' must be a dictionary"
                    )
        ...
        try:
            ...
            if style_name in all_configs:
                ...
                if style_name in config.WAVEFORM_STYLE_CONFIGS:
                    logging.info(
                        f"Style '{style_name}' not found in saved settings, returning default"
                    )
    ...
    def save_model_selection(self, model_value: str) -> None:
        ...
        if model_value not in config.MODEL_VALUE_MAP.values():
            ...
            raise ValueError(
                f"Invalid model '{model_value}'. Valid models: {valid_models}"
            )
    ...
    def load_streaming_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "streaming_paste_enabled": settings.get(
    ...
    def save_streaming_settings(
        self, enabled: bool, chunk_duration: float = None, paste_enabled: bool = None
    ) -> None:
