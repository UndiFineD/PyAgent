...
class LocalWhisperBackend(TranscriptionBackend):
    ...
    def _detect_hardware(self) -> Tuple[str, str, str]:
        ...
        if self._override_compute_type is not None:
            ...
            compute_type = settings.get(
                "whisper_compute_type", config.FASTER_WHISPER_COMPUTE_TYPE
            )
        ...
        if device == "auto" or compute_type == "auto" or model == "auto":
            ...
            if has_cuda:
                ...
                logging.info(
                    "CUDA detected - using GPU acceleration with float16 and turbo model"
                )
            else:
                detected_device = "cpu"
                ...
                detected_model = "base"
                logging.info(
                    "No CUDA available - using CPU with int8 quantization and base model"
                )
    ...
    def transcribe(self, audio_file_path: str) -> str:
        ...
        try:
            ...
            logging.info(
                f"Processing audio with faster-whisper (VAD={config.FASTER_WHISPER_VAD_ENABLED})..."
            )
            ...
            logging.info(
                f"Transcription complete. Length: {len(transcribed_text)} characters"
            )
    ...
    def transcribe_chunks(self, chunk_files: List[str]) -> str:
        ...
        try:
            ...
            for i, chunk_file in enumerate(chunk_files):
                ...
                logging.info(
                    f"Chunk {i+1}/{len(chunk_files)} completed. "
    ...
    def name(self) -> str:
        ...
        device_info = (
            f"{self._device}/{self._compute_type}" if self._device else "not loaded"
        )
