...
class OpenAIBackend(TranscriptionBackend):
        ...
        if not api_key:
            try:
                ...
                env_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "..", config.ENV_FILE
                )
    ...
    def transcribe(self, audio_file_path: str) -> str:
        ...
        if not self.is_available():
            raise Exception(
                "OpenAI API is not available (no API key or client initialization failed)"
            )
        ...
        try:
            ...
            logging.info(
                f"API transcription complete. Length: {len(transcribed_text)} characters"
            )
    ...
    def transcribe_chunks(self, chunk_files: List[str]) -> str:
        ...
        try:
            ...
            for i, chunk_file in enumerate(chunk_files):
                ...
                logging.info(
                    f"Processing chunk {i+1}/{len(chunk_files)} with OpenAI API: {chunk_file}"
                )
                ...
                logging.info(
                    f"Chunk {i+1}/{len(chunk_files)} completed. Length: {len(chunk_text)} characters"
                )
            ...
            logging.info(
                f"OpenAI chunked transcription complete. Total length: {len(combined_text)} characters"
            )
    ...
    def cleanup(self):
        ...
        try:
            if self.client is not None:
                ...
                logging.info(
                    f"OpenAI backend ({self.model_type}) cleaned up successfully"
                )
        except Exception as e:
