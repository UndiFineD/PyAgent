...
def test_chunking():
    ...
    try:
        ...
        logger.info(
            f"File size check: {file_size_mb:.1f} MB, needs_splitting: {needs_splitting}"
        )
        ...
        logger.info(
            f"Total chunk size: {total_size:.1f} MB (original: {file_size_mb:.1f} MB)"
        )
        ...
        mock_transcriptions = [
            f"Mock transcription for chunk {i+1}" for i in range(len(chunk_files))
        ]
        combined_text = audio_processor.combine_transcriptions(mock_transcriptions)

        logger.info(
            f"Combined {len(mock_transcriptions)} transcriptions into {len(combined_text)} characters"
        )
