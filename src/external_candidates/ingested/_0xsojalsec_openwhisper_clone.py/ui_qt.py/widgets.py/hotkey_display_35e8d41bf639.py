...
class HotkeyDisplay(QWidget):
    ...
    def _setup_ui(self):
        ...
        spacer1.setStyleSheet(
            "color: #3a3a3c; font-size: 12px; padding: 0 2px; background: transparent;"
        )
    ...
    def update_hotkeys(
        self, record_key: str, cancel_key: str, enable_disable_key: str = "Ctrl+Alt+*"
    ):
        """
        ...
        if layout.count() >= 8:
            ...
            widgets_to_update = [
                ...
                (
                    enable_widget,
                    enable_disable_key.replace("ctrl", "Ctrl")
                    .replace("alt", "Alt")
                    .replace("shift", "Shift"),
                ),
            ]
