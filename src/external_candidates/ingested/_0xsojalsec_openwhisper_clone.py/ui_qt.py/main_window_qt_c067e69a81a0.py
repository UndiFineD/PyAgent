    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton and self.parent_window:
            ...
            if edge != (0, 0):
                ...
                return
            self._drag_position = (
                global_pos - self.parent_window.frameGeometry().topLeft()
            )
            event.accept()
            return
...
class ModernMainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.history_sidebar.retranscribe_requested.connect(
            self._on_retranscribe_requested
        )
    ...
    def _setup_menu(self):
        ...
        test_overlays_menu.addAction(
            "Recording", lambda: self.test_overlay("recording")
        )
        test_overlays_menu.addAction(
            "Processing", lambda: self.test_overlay("processing")
        )
        test_overlays_menu.addAction(
            "Transcribing", lambda: self.test_overlay("transcribing")
        )
        test_overlays_menu.addAction(
            "Canceling", lambda: self.test_overlay("canceling")
        )
        test_overlays_menu.addSeparator()
        test_overlays_menu.addAction(
            "STT Enable", lambda: self.test_overlay("stt_enable")
        )
        test_overlays_menu.addAction(
            "STT Disable", lambda: self.test_overlay("stt_disable")
        )
            ...
            for display_name, internal_value in config.MODEL_VALUE_MAP.items():
                if internal_value == saved_model:
                    ...
                    if index >= 0:
                        ...
                        self.logger.info(
                            f"Loaded saved model selection: {display_name}"
                        )
                    break
    ...
    def _on_stats_visibility_changed(self, visible: bool):
        ...
        if visible:
            ...
            new_height = max(
                600, current_height - stats_height
            )  # Don't go below min height
    ...
    def test_loading_screen(self):
        ...
        QTimer.singleShot(
            1000,
            lambda: self.test_loading_screen_instance.update_status(
                "Loading resources..."
            ),
        )
        QTimer.singleShot(
            2000,
            lambda: self.test_loading_screen_instance.update_progress(
    ...
    def _apply_resize_delta(self, global_pos) -> None:
        ...
        if (
            not self._resizing
            or not self._resize_edge
            or not self._resize_start_geometry
        ):
            return
        ...
        if h == -1:  # Left edge
            ...
            new_width = min(
                self.maximumWidth(), max(self.minimumWidth(), geo.width() + delta.x())
            )
            ...
            if geo:
                ...
                if screen:
                    ...
                    if screen_geo.intersects(saved_rect):
                        ...
                        width = max(
                            self.minimumWidth(), min(geo["width"], self.maximumWidth())
                        )
