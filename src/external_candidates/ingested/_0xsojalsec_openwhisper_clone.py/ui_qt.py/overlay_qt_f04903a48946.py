...
class ModernWaveformOverlay(QWidget):
    ...
    def __init__(self):
        ...
        try:
            style_config = style_configs.get(
                current_style, config.WAVEFORM_STYLE_CONFIGS.get("particle", {})
            )
            ...
            self.logger.warning(
                f"Failed to load style '{current_style}', using particle"
            )
            self.style = style_factory.create_style(
                "particle", self.overlay_width, self.overlay_height
            )
    ...
    def _draw_recording_state(self, painter: QPainter):
        ...
        painter.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        painter.drawText(
            rect.adjusted(0, h - 25, 0, 0), Qt.AlignmentFlag.AlignCenter, "Recording..."
        )
    ...
    def _draw_canceling_state(self, painter: QPainter):
        ...
        painter.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        painter.drawText(
            rect.adjusted(0, h - 25, 0, 0), Qt.AlignmentFlag.AlignCenter, "Canceling..."
        )
    ...
    def _draw_stt_enable_state(self, painter: QPainter):
        ...
        if self.animation_time > 0.4:
            ...
            painter.drawLine(
                int(w // 2 - 15), int(h // 2), int(w // 2 - 5), int(h // 2 + 10)
            )
            painter.drawLine(
                int(w // 2 - 5), int(h // 2 + 10), int(w // 2 + 15), int(h // 2 - 10)
            )
        ...
        painter.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        painter.drawText(
            rect.adjusted(0, h - 25, 0, 0), Qt.AlignmentFlag.AlignCenter, "Enabled"
        )
    ...
    def _draw_stt_disable_state(self, painter: QPainter):
        ...
        if self.animation_time > 0.1:
            ...
            painter.drawLine(
                w // 2 - x_size, h // 2 - x_size, w // 2 + x_size, h // 2 + x_size
            )
            painter.drawLine(
                w // 2 + x_size, h // 2 - x_size, w // 2 - x_size, h // 2 + x_size
            )
        ...
        painter.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        painter.drawText(
            rect.adjusted(0, h - 25, 0, 0), Qt.AlignmentFlag.AlignCenter, "Disabled"
        )
    ...
    def _draw_copied_state(self, painter: QPainter):
        ...
        painter.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        painter.drawText(
            rect.adjusted(0, h - 25, 0, 0), Qt.AlignmentFlag.AlignCenter, "Copied!"
        )
    ...
    def _draw_large_file_splitting_state(self, painter: QPainter):
        ...
        painter.drawText(
            rect.adjusted(0, h - 25, 0, 0), Qt.AlignmentFlag.AlignCenter, text
        )
    ...
    def set_state(self, state: str):
        ...
        if self.current_state != state:
            ...
            self.last_frame_time = (
                time.time()
            )  # Reset to prevent huge delta on first frame
