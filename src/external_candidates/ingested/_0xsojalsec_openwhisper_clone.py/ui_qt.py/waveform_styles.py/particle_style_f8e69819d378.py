...
class ParticleStyle(BaseWaveformStyle):
    ...
    def draw_recording_state(
        ...
        dt = (
            1 / 30
        )  # Approximate dt since we don't have exact frame delta here easily without passing it
        ...
        audio_energy = (
            sum(self.audio_levels) / len(self.audio_levels)
    ...
    def draw_processing_state(
        ...
        for i in range(vortex_particles):
            ...
            particle.color_hue = (
                angle * 180 / math.pi + self.animation_time * 50
            ) % 360
    ...
    def draw_canceling_state(
        ...
        if (
            progress < self._last_cancel_progress - 0.2
        ) or not self._cancel_initialized:
    ...
    def _emit_audio_particles(self, count: int, audio_energy: float):
        ...
        for _ in range(min(count, self.max_particles - len(self.particles))):
            ...
            particle = Particle(x, y, vx, vy)
            particle.color_hue = (
                self.animation_time * self.color_shift_speed + random.uniform(0, 60)
            ) % 360
            self.particles.append(particle)
    ...
    def _update_particles(
        ...
        for particle in self.particles:
            if vortex_mode:
                ...
                turbulence_x = (
                    math.sin(self.animation_time * 2 + particle.y * 0.1)
                    * self.turbulence_strength
                )
                turbulence_y = (
                    math.cos(self.animation_time * 1.5 + particle.x * 0.1)
                    * self.turbulence_strength
                    * 0.5
                )
    ...
    def _draw_particles(self, painter: QPainter):
        ...
        for particle in self.particles:
            try:
                ...
                painter.drawEllipse(
                    QRectF(particle.x - size, particle.y - size, size * 2, size * 2)
                )
