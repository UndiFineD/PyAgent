# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\tests\unit_tests\test_app_scaffold_design.py
import logging
...
from tests.unit_tests.utils.constants import SCAFFOLD_DESIGN_FILE, TIMEOUT
from tests.unit_tests.utils.mocking import mock_molstar
from tests.unit_tests.utils.rfdiffusion_utils import review_and_submit_tab, settings_tab
...
class TestScaffoldDesign:
    ...
    def test_default_values(
        self, mocker: MockerFixture, test_wrapper, mock_scheduler
    ) -> None:
        """
        ...
        assert workflow.rfdiffusion_params.input_pdb.endswith(
            "1myj.pdb"
        ), "Verify that PDB code was registered."
        ...
        with mock_molstar(
            {
                "sequenceSelections": [
                    {
                        "chainId": "A",
                        "residues": list(range(3, 17 + 1)) + list(range(59, 77 + 1)),
                    }
                ],
            }
        )(mocker):
        ...
        with mock_molstar(
            {
                "sequenceSelections": [
                    {
                        "chainId": "A",
                        "residues": list(range(5, 7 + 1)) + list(range(61, 65 + 1)),
                    }
                ],
            }
        )(mocker):
        ...
        assert (
            num_sequences.value == ProteinMPNNParams().num_sequences
        )  # as per ProteinMPNNParams default
