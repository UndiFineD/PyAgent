# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\tests\unit_tests\utils\rfdiffusion_utils.py
import logging
import time
...
def settings_tab(
    ...
    assert (
        round_ids_by_name[round_name] == at.selectbox("selected_round_id").value
...
def review_and_submit_tab(at: AppTest, mock_scheduler, pool_name: str) -> None:
    ...
    assert (
        len(mock_scheduler.jobs) == num_jobs_before + 1
