# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\tests\integration_tests\test_full_rfdiffusion_fastrelax_binder_design_workflow.py
...
def test_binder_default_end_to_end_logic(project_data):
    ...
    design_job, pool = design_logic.submit_design_workflow(
        ...
        pool_name="5ELI fastrelax binder test",
        pool_description="",
    ...
    assert design_ids[0].endswith("_cycle0")
    ...
    af2_ipae = db.select_descriptor_values(
        descriptors_refolding.AF2_PRIMARY_IPAE.key, design_ids
    )
    assert len(af2_ipae.dropna()) == 2
    assert (af2_ipae < 30).all()

    rosetta_ddg = db.select_descriptor_values(
        descriptors_rfdiffusion.PYROSETTA_DDG.key, design_ids
    )
