# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\tests\integration_tests\test_full_rfdiffusion_scaffold_design_workflow.py
...
def test_scaffold_end_to_end_logic(project_data):
    ...
    rag = db.select_descriptor_values(
        descriptors_rfdiffusion.RADIUS_OF_GYRATION.key, design_ids
    )
    assert len(rag.dropna()) == 2
    assert (rag > 0).all()

    af2_plddt = db.select_descriptor_values(
        descriptors_refolding.AF2_PRIMARY_PLDDT.key, design_ids
    )
    assert len(af2_plddt.dropna()) == 2
    assert (af2_plddt > 10).all()

    design_rmsd = db.select_descriptor_values(
        descriptors_refolding.AF2_PRIMARY_DESIGN_RMSD.key, design_ids
    )
    ...
    assert (
        len(empty.dropna()) == 0
    ), "Refolding descriptors should be empty before refolding is run"
    ...
    design_rmsd = db.select_descriptor_values(
        f"refolding|{test}|design_backbone_rmsd", design_ids
    )
    assert len(design_rmsd.dropna()) == 2
    assert (design_rmsd < 20).all()
