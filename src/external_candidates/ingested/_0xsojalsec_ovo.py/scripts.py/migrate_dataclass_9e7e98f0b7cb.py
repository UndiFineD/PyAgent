...
assert (
    len(sys.argv) == 3
), "Usage: python -i scripts/migrate_dataclass.py MODEL_NAME FIELD_NAME"
...
if field_name not in type_annotations:
    raise ValueError(
        f"No type declared for field {field_name} in class {model_name}, got: {type_annotations}"
    )
...
def run(migrate_func=None, dry=False, all=False):
    ...
    for object_id in object_ids:
        ...
        if all or should_migrate:
            ...
            migrate_func(data)
            value = (
                DataclassType.from_dict(data)
                if hasattr(DataclassType, "from_dict")
                else DataclassType(**data)
            )
            print(data)
            num_updated += 1
            if not dry:
