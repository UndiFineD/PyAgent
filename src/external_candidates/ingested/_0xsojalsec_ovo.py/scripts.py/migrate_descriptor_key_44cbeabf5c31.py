# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\scripts\migrate_descriptor_key.py
import argparse
...
if __name__ == "__main__":
    ...
    result = session.execute(
        text(
            "UPDATE descriptor_value SET descriptor_key = :new_key WHERE descriptor_key = :old_key"
        ),
    ...
    for job in db.DesignJob.select():
        if (
            job.workflow
            and job.workflow.acceptance_thresholds
            and old_key in job.workflow.acceptance_thresholds
        ):
