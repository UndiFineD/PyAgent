...
def home(
    ...
    yes: bool = typer.Option(
        False, "--yes", "-y", help="Confirm all without prompting"
    ),
    no_env: bool = typer.Option(
        False, "--no-env", help="Do not set OVO_HOME in shell .bashrc/.zshrc file"
    ),
    default_profile: str = typer.Option(
        None, "--profile", help="Default nextflow profile to use in default scheduler"
    ),
):
...
""")
        home_dir = yes or Prompt.ask(
            ...
            default=os.getenv(
                "OVO_HOME", DEFAULT_OVO_HOME
            ),  # Use OVO_HOME env var if set already
        )
    ...
    while os.path.exists(home_dir):
        ...
        console.print(
            f"[red]Error:[/red] Home directory already exists: [bold]{home_dir}[/bold]"
        )
        home_dir = Prompt.ask(
            "Please choose a different path for the new OVO Home directory"
        )
        home_dir = os.path.abspath(os.path.expanduser(home_dir))
...
def preview():
    ...
    download_files(
        destination_dir=config.reference_files_dir, file_list=RFDIFFUSION_MODEL_FILES
    )
    ...
    if process.returncode == 0:
        console.print(
            "[bold][green]✔[/green] RFdiffusion preview completed successfully[/bold]"
        )
    ...
    console.print(
        "           Or already explore OVO web app using [bold]ovo app[/bold]"
    )
...
def rfdiffusion(scheduler: str = None):
    ...
    download_files(
        destination_dir=config.reference_files_dir, file_list=ALPHAFOLD_MODEL_FILES
    )
    ...
    process, job_id = scheduler.submit(
        ...
        params={
            ...
            "rfdiffusion_input_pdb": os.path.join(
                ovo_resources_path, "examples/inputs/5ELI_A.pdb"
            ),
    ...
    if process.returncode == 0:
        console.print(
            "[bold][green]✔[/green] RFdiffusion scaffold preview completed successfully[/bold]"
        )
    ...
    if process.returncode == 0:
        console.print(
            "[bold][green]✔[/green] RFdiffusion binder preview completed successfully[/bold]"
        )
    ...
    if config.props.pyrosetta_license:
        ...
        if process.returncode == 0:
            ...
            console.print(
                "RFdiffusion workflow [red]FAILED[/red], please see errors above"
            )
            exit(process.returncode)
...
def proteinqc(
    ...
    if ESM_1V in tools:
        download_files(
            destination_dir=config.reference_files_dir, file_list=ESM1V_MODEL_FILES
        )

    # Download ESM-IF weights
    if ESM_IF in tools:
        download_files(
            destination_dir=config.reference_files_dir, file_list=ESM_IF_MODEL_FILES
        )
