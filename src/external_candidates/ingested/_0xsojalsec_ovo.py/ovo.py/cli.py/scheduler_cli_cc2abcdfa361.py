# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\cli\scheduler_cli.py
import glob
import os
import re
import sys
...
import jsonschema
import typer
from ovo.cli.common import OVOCliError, console
...
def run(
    ...
    link: bool = typer.Option(
        default=None, help="Do not copy output files, create a symlink instead"
    ),
...
def submit(
    ...
    output: str = typer.Option(
        None, "--output", help="Custom output directory (requires --sync)"
    ),
    scheduler_key: str = typer.Option(None, "--scheduler", help="Scheduler key"),
    sync: bool = typer.Option(
        default=None, help="Run nextflow process synchronously, waiting for completion"
    ),
    link: bool = typer.Option(
        default=None,
        help="Do not copy output files, create a symlink instead (when --output is used)",
    ),
    ctx: typer.Context = typer.Option(None),
    help: bool | None = typer.Option(
        None,
    ...
    if not pipeline_name:
        ...
        sep = "\n "
        raise OVOCliError(
            f"Pipeline name is required. Available pipelines: \n{sep}{sep.join(available_paths)}"
        )
    ...
    console.print(
        f"Submitting [bold]{pipeline_name}[/bold] with [bold]{resolved_scheduler_key}[/bold] scheduler:"
    )
    ...
    if sync:
        ...
        if process.returncode != 0:
            console.print(
                f"[bold][red]✘[/red] Pipeline failed with exit code {process.returncode}[/bold]"
            )
            sys.exit(4)
        ...
        if output:
            ...
            console.print(
                f"[green]✔[/green] Output saved to: {os.path.abspath(output)}"
            )
        else:
        ...
        console.print("")
        console.print(
            f"Running in background, use --sync to run synchronously and wait for completion."
        )
        console.print(f"[green]✔[/green] Job submitted: [bold]{job_id}[/bold]")
        scheduler_arg = f"--scheduler {scheduler_key} " if scheduler_key else ""
        console.print(
            f"[green]✔[/green] Output directory: [bold]{scheduler.get_output_dir(job_id)}[/bold]"
        )
        console.print(
            f"[green]✔[/green] Check status with: [bold]ovo scheduler status {scheduler_arg}{job_id}[/bold]"
        )
    return job_id
...
def jupyter(
    dirs: list[str] = typer.Argument(
        help="Directory to run from. Will be mounted inside the container."
    ),
    ...
    run_parameters: str = typer.Option(
        default="", help="Additional commandline parameters to pass to Jupyter server"
    ),
    timeout: int = typer.Option(
        default=3600, help="Timeout in seconds to wait for Jupyter server to start"
    ),
):
    ...
    if not isinstance(scheduler, NextflowScheduler):
        raise NotImplementedError(
            "Only NextflowScheduler currently supports Jupyter submission."
        )
    ...
    if not workdir:
        console.print(
            f"[red][bold]✘[/bold] Failed to find Jupyter workdir: {workdir_glob}[/red]"
        )
        sys.exit(4)
    workdir = workdir[0]
    ...
    for retry in range(timeout):
        ...
        if "Jupyter Lab not available" in log:
            if not installing_jupyter:
                console.print(
                    "Jupyter Lab not available in the environment, installing..."
                )
        ...
        if jupyter_url_lines:
            hostname_lines = [
                line for line in log.splitlines() if "JUPYTER_HOSTNAME:" in line
            ]
            if hostname_lines:
    ...
    if not jupyter_url_lines:
        ...
        console.print(
            f"[red][bold]✘[/bold] Failed to get Jupyter URL for job {job_id}, please check job status[/red]"
        )
        sys.exit(4)
    ...
    console.print("Tips:")
    console.print(
        " - Cmd/Ctrl + Double click the URL to open the link in your browser."
    )
    if hostname:
        console.print(
...
def format_help(schema):
    ...
    for scheduler_key, scheduler_config in config.schedulers.items():
        params_str += f" --scheduler {scheduler_key} ({scheduler_config.name}, args: {scheduler_config.submission_args})\n"
    params_str += "\n"
    if schema:
        sep = "\n  "
        formatted_params = {
            key: (
                ...
                + (f" (default {value['default']})" if value.get("default") else "")
        ...
        required_params = [
            formatted_params.get(key, f"--{key}") for key in schema.get("required", [])
        ]
        if required_params:
...
def status(
    ...
    if job_result is None:
        ...
        console.print(
            f"[bold][green]✔[/green] Job {job_id} completed successfully[/bold]"
        )
...
def cancel(
    ...
    console.print(
        f"Cancelling job [bold]{job_id}[/bold] with scheduler [bold]{resolved_scheduler_key}[/bold]"
    )
...
def worker(
    num_workers: int = typer.Option(
        1, "-n", "--workers", help="Number of worker threads"
    ),
    ...
    host: str = typer.Option(
        None, "--host", help="Run server on custom host (override config)"
    ),
    port: int = typer.Option(
        None, "--port", help="Run server on custom port (override config)"
    ),
):
