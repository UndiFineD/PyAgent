# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\cli\app.py
#!/usr/bin/env python
...
def main(
    ...
    streamlit_script_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "run_app.py"
    )
    ...
    sys.argv.extend(
        ["--server.showEmailPrompt", "0"]
    )  # disable streamlit email prompt (first run)
    sys.argv.extend(
        ["--logger.enableRich", "0"]
    )  # disable rich logging to avoid weird tracebacks
    ...
    if token or os.environ.get("OVO_LOGIN_TOKEN") or config.auth.always_require_token:
        ...
        and not config.auth.streamlit_auth
        and not config.auth.hide_admin_warning
    ):
        console.print(
