# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\utils\cached_db.py
...
def get_cached_project_ids_and_names(
    username: str, extra_project_ids: Collection[str] = None
) -> dict[str, str]:
    ...
    if username in config.auth.admin_users:
        ...
        projects = sorted(
            public_projects + private_projects + extra_projects,
...
def get_cached_descriptor_values(
    descriptor_key: str, design_ids: list[str]
) -> pd.Series():
...
def get_cached_designs(design_ids: Collection[str]) -> list[Design]:
    ...
    return [
        designs_by_id[design_id]
        for design_id in design_ids
        if design_id in designs_by_id
    ]
...
def get_cached_design_ids(pool_ids: list[str], **filters) -> list[str]:
    ...
    return sorted(
        ids, key=lambda design_id: order.get(Design.design_id_to_pool_id(design_id))
    )
...
def get_cached_design_jobs_table(round_ids: list[str]):
    design_job_ids = [
        v for v in db.select_values(Pool, "design_job_id", round_id__in=round_ids) if v
    ]
...
@clear_when_modified(DesignJob)
@st.cache_data(
    show_spinner=False, max_entries=100, ttl="1m"
)  # update every minute since the table contains datetimes
def _get_cached_design_jobs_table(
    project_id: str = None, round_ids: list[str] = None, update=True, **pool_filters
) -> pd.DataFrame:
    return get_design_jobs_table(
        project_id=project_id, round_ids=round_ids, update=update, **pool_filters
    )


@clear_when_modified(Pool, Design)
@st.cache_data(
    max_entries=100, ttl="1m"
)  # update every minute since the table contains datetimes
def get_cached_pools_table(
    project_id: str = None, round_ids: list[str] = None
) -> pd.DataFrame:
...
@clear_when_modified(DescriptorValue)
@st.cache_data(max_entries=100, ttl="1h")
def get_cached_design_descriptors(
    design_id: str, descriptor_keys: list[str]
) -> pd.Series():
    return db.select_design_descriptors(
        design_id=design_id, descriptor_keys=descriptor_keys
    )
...
@st.cache_data(ttl="1h")
def get_cached_workflow_pools_and_jobs(
    project_id: str, workflow_names: Collection[str]
):
    round_ids = db.select_unique_values(Round, "id", project_id=project_id)
    pools = [
        pool for pool in db.select(Pool, round_id__in=round_ids) if pool.design_job_id
    ]
    all_jobs_by_id = {
        j.id: j
        for j in db.select(
            DesignJob,
            id__in=[p.design_job_id for p in pools],
