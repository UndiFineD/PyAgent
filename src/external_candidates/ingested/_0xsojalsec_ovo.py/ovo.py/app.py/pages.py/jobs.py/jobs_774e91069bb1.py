# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\jobs\jobs.py
...
def overview():
    ...
    if not rounds:
        st.write(
            "*No design jobs yet in this project. Submit a workflow in the left panel.*"
        )
        return
    ...
    table.insert(
        ...
        ("Pool", "actions"),
        [
            f"?pool_ids={pool_id}&project_id={project.id}"
            for pool_id in table[("Pool", "id")]
        ],
    )
    ...
    if st.button(
        f"{label} results :material/arrow_forward_ios:", key=f"open_round_{key}"
    ):
        st.query_params["pool_ids"] = ",".join(
            selected_ids if selected_ids else table[("Pool", "id")]
        )
        st.rerun()
...
if "pool_ids" in st.query_params:
    ...
    for round_id in set([p.round_id for p in pools]):
        ...
        assert (
            project_round.project_id == project_id
        ), f"Pool does not belong to project {project_id}"
