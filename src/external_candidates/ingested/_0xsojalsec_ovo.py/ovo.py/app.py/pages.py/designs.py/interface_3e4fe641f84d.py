...
def interface_fragment(pool_ids: List[str], design_ids: List[str] | None = None):
    ...
    st.header(
        f"Interface analyzer | {len(design_ids):,} {'design' if len(design_ids) == 1 else 'designs'}"
    )
    ...
    if not design_jobs:
        st.warning(
            "Selected pools have no design jobs. Please select pools with completed design jobs."
        )
        return
    ...
    interface_descriptors_by_key = {
        desc.key: desc
        for desc in ALL_INTERFACE_DESCRIPTORS
        if desc.key in descriptors_by_key
    }

    if INTERFACE_TARGET_RESIDUES.key not in interface_descriptors_by_key:
        st.warning(
            "Interface analysis metrics are not provided by this design workflow."
        )
        return
    ...
    interface_design_visualization_fragment(
        design_ids, descriptors_df, interface_descriptors_by_key
    )


@st.fragment()
def interface_metrics_summary(
    descriptors_df: pd.DataFrame, descriptors_by_key: Dict[str, Descriptor]
):
    ...
    for col_key in descriptors_df.columns:
        ...
        label = (
            f"Average {descriptor.name}" if not is_single_design else descriptor.name
        )
...
def interface_residues_table(descriptors_df: pd.DataFrame):
    ...
    residue_df = get_residue_presence_df(
        descriptor, residue_values, len(descriptors_df)
    )
...
def interface_residues_table_by_design(descriptors_df: pd.DataFrame):
    ...
    table_data, column_config, caption, format_func = (
        residue_number_descriptor_detail_table(descriptor, descriptor_values)
    )
    # st.caption(caption)
    st.dataframe(
        table_data,
...
def design_interface_detail(
    ...
    interface_residues_str = (
        ...
        and INTERFACE_TARGET_RESIDUES.key in descriptors_df.columns
        else None
    )
    ...
    if interface_residues_str and pd.notna(interface_residues_str):
        interface_residues = [
            r.strip() for r in str(interface_residues_str).split(",") if r.strip()
        ]
    ...
    molstar_custom_component(
        structures=[
            StructureVisualization(
                ...
                highlighted_selections=get_molstar_residue_selections(
                    interface_residues
                ),
            )
        ],
...
def interface_design_visualization_fragment(
    ...
    interface_residues = set(
        x.strip()
        for row in descriptors_df[INTERFACE_TARGET_RESIDUES.key].dropna()
        for x in row.split(",")
    )
    ...
    if target_residue:
        ...
        st.write(
            f"Showing {len(selected_design_ids):,} designs that interact with target residue {target_residue}"
        )
    else:
        st.write(
    ...
    labels_by_design_id = descriptors_df.loc[
        selected_design_ids, INTERFACE_TARGET_RESIDUES.key
    ].to_dict()
