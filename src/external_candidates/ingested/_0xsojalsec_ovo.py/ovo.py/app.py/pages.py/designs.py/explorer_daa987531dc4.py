# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\designs\explorer.py
...
def explorer_fragment(pool_ids: list[str], design_ids: list[str] | None = None):
    ...
    if design_ids is None:
        ...
        design_ids = sorted(
            db.select_values(Design, "id", pool_id__in=pool_ids, accepted=True)
        )
    ...
    st.header(
        f"Explorer | {len(design_ids):,} {'design' if len(design_ids) == 1 else 'designs'}"
    )
...
def design_visualization_fragment(selected_design_ids: list[str]):
    ...
    design_job = (
        get_cached_design_job(pool.design_job_id) if pool.design_job_id else None
    )
    WorkflowType = (
        type(design_job.workflow)
