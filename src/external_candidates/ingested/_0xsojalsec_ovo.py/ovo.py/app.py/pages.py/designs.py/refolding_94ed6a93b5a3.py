# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\designs\refolding.py
...
from ovo.core.database import Design, DesignJob, Pool
...
def refolding_fragment(pool_ids: list[str], design_ids: list[str] | None = None):
    ...
    st.header(
        f"🔁 Refolding | {len(design_ids):,} {'design' if len(design_ids) == 1 else 'designs'}"
    )
    ...
    descriptors = [
        d for d in REFOLDING_DESCRIPTORS if d.key in available_descriptor_keys
    ]
...
def submit_refolding_dialog(pool_ids: list[str], design_ids: list[str]):
    ...
    with content.container():
        ...
        st.write(
            f"""Submit refolding evaluation for {num_designs:,} {"design" if num_designs == 1 else "designs"}"""
        )
        ...
        if any(not p.design_job_id for p in pools):
            ...
            design_type = st.selectbox(
                "Select design type",
                options=REFOLDING_TESTS_BY_TYPE.keys(),
                key="design_type",
            )
            if not design_type:
                return
        ...
        st.write(
            f"Select one or more refolding tests supported for **{design_type}** design:"
        )
    ...
    if submit:
        ...
        for pool in pools:
            index_by_id = db.select_dict(
                Design, "id", "contig_index", id__in=design_ids, pool_id=pool.id
            )
            ...
            for contig_index, ids in ids_by_index.items():
                native_pdb_path = design_workflow.get_refolding_native_pdb_path(
                    contig_index
                )
        ...
        for native_pdb_path, group_design_ids in groups.items():
            ...
            submit_descriptor_workflow(
                workflow, scheduler_key, st.session_state.project.id
            )
        st.rerun()
