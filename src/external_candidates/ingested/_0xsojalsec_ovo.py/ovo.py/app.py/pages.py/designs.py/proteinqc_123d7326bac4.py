# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\designs\proteinqc.py
...
from ovo.core.database import Design, NumericGlobalDescriptor, Pool
from ovo.core.database.descriptors_proteinqc import PROTEINQC_MAIN_DESCRIPTORS
from ovo.core.database.models_proteinqc import PROTEINQC_TOOLS, ProteinQCWorkflow
...
def proteinqc_fragment(pool_ids: list[str], design_ids: list[str] | None = None):
    ...
    st.header(
        f"🔎 ProteinQC | {len(design_ids):,} {'design' if len(design_ids) == 1 else 'designs'}"
    )
    ...
    numeric_descriptors_by_key = {
        d_key: d
        for d_key, d in descriptors_by_key.items()
        if type(d) is NumericGlobalDescriptor
    }
    ...
    with right:
        histogram_source = source_selectbox(
            st.query_params.get("ref"), key_prefix="proteinqc_reference"
        )
...
def submit_proteinqc_dialog(pool_ids: list[str], design_ids: list[str]):
    ...
    st.write(
        f"""Submit ProteinQC for {num_designs:,} {"design" if num_designs == 1 else "designs"}"""
    )
    ...
    if st.button("Submit", key="submit_proteinqc_btn", type="primary"):
        ...
        workflow = ProteinQCWorkflow(
            tools=tool_keys, chains=list(chains), design_ids=design_ids
        )
