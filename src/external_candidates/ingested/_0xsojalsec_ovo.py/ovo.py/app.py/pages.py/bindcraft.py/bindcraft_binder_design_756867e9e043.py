...
def input_step():
    ...
    with right:
        history_dropdown_component(
            __file__, workflow_name=BindCraftBinderDesignWorkflow.name
        )
...
def settings_step():
    ...
    with st.columns([1, 2])[0]:
        if (
            workflow.bindcraft_params.target_chains is None
            and len(workflow.chains) == 1
        ):
        ...
        if (
            workflow.bindcraft_params.num_replicas > 1
            and workflow.bindcraft_params.time_limit_hours
        ):
            st.caption(
