# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\rfdiffusion\binder_design.py
import re
...
from ovo.app.components.trim_components import (
    parameters_trim_structure_component,
    trimmed_structure_visualizer,
)
...
def hotspots_step():
    ...
    sequence_selection_fragment(
        __file__, workflow.input_name, color="hydrophobicity", write_segments=False
    )
...
def trim_step():
    ...
    if (
        workflow.start_res_trimmed_chain is None
        or workflow.end_res_trimmed_chain is None
    ):
        return
    ...
    if st.button(":material/wand_stars: Generate preview"):
        with st.spinner("Submitting RFdiffusion job..."):
            workflow.preview_job_id = submit_rfdiffusion_preview(
                workflow, timesteps=num_timesteps
            )
...
def settings_step():
    ...
    if contig != workflow.rfdiffusion_params.contig and check_contig_parsed(
        contig, True
    ):
        workflow.rfdiffusion_params.contig = contig

        chain_segment, workflow.binder_length = (
            workflow.rfdiffusion_params.contig.split("/0 ")
        )
