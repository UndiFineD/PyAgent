# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\rfdiffusion\binder_diversification.py
import os
import re
...
from ovo.app.components.input_components import (
    initialize_workflow,
...
from ovo.app.pages import designs_page, jobs_page, rf_binder_design_page
...
from ovo.core.database.models import DesignJob, Pool, WorkflowTypes
...
def input_step():
        ...
        if not workflow.rfdiffusion_params.input_pdb_paths:
            ...
            if initialize_workflow_input():
                ...
                for pdb_path in workflow.rfdiffusion_params.input_pdb_paths:
                    chain_contigs = get_chains_and_contigs(
                        storage.read_file_str(pdb_path)
                    )
                    ...
                    try:
                        ...
                        assert (
                            chain_contigs["A"].removeprefix("A").startswith("1-")
                        ...
                        # 100-100/0 B20-130
                        workflow.rfdiffusion_params.contigs.append(
                            f"{binder_len}-{binder_len}/0 {chain_contigs['B']}"
                        )
    ...
    if len(workflow.rfdiffusion_params.input_pdb_paths) > 1:
        indexes = {
            v: i
            for i, v in enumerate(workflow.rfdiffusion_params.input_pdb_paths, start=1)
        }
...
def initialize_workflow_input() -> RFdiffusionBinderDesignWorkflow | None:
    ...
    design_ids = [
        design_id.strip() for design_id in design_ids_raw.replace(",", " ").split()
    ]
    ...
    if st.button("Confirm"):
        ...
        for pool_id in pool_ids:
            ...
            if not design_job.workflow or not design_job.workflow.is_instance(
                RFdiffusionBinderDesignWorkflow
            ):
                continue
        ...
        ), "Designs do not come from RFdiffusion binder design workflow"
        assert (
            len(source_workflow_names) == 1
        ...
        workflow.input_name = (
            design_ids[0] if len(design_ids) == 1 else f"{len(design_ids)} designs"
        )
        ...
        workflow.rfdiffusion_params.input_pdb_paths = [
            design.structure_path for design in designs
        ]
...
def preview_step():
    ...
    result = wait_with_statusbar(
        local_scheduler, workflow.preview_job_id, label="Running RFdiffusion..."
    )
    ...
    visualize_rfdiffusion_preview(
        workflow, output_dir=local_scheduler.get_output_dir(workflow.preview_job_id)
    )
...
def settings_step():
    ...
    with st.columns([1, 2])[0]:
        ...
        workflow.rfdiffusion_params.num_designs = st.number_input(
            ...
            min_value=1,
            max_value=(
                config.props.rfdiffusion_backbones_limit_admin
                if is_admin
    ...
    with st.columns([1, 2])[0]:
        workflow.rfdiffusion_params.model_weights = st.selectbox(
            ...
            index=(
                MODEL_WEIGHTS_BINDER.index(workflow.rfdiffusion_params.model_weights)
