# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\pages\rfdiffusion\scaffold_design.py
...
from ovo.app.components import StructureVisualization, molstar_custom_component
...
from ovo.app.pages import designs_page, jobs_page
...
def input_step():
    ...
    with right:
        history_dropdown_component(
            __file__, workflow_name=RFdiffusionScaffoldDesignWorkflow.name
        )
    ...
    molstar_custom_component(
        structures=[
            StructureVisualization(
                pdb=storage.read_file_str(workflow.rfdiffusion_params.input_pdb),
                color="chain-id",
            )
        ],
        key="input_structure",
    ...
    if st.button(":material/wand_stars: Generate preview"):
        with st.spinner("Submitting RFdiffusion job..."):
            workflow.preview_job_id = submit_rfdiffusion_preview(
                workflow, timesteps=st.session_state.preview_timesteps
            )
...
def inpainting_step():
    ...
    parsed_contig = ContigsParser().parse_contigs_str(
        workflow.rfdiffusion_params.contig
    )
    fixed_segments = [seg for seg in parsed_contig if seg.type == "fixed"]

    sequence_selection_fragment(
        __file__, workflow.input_name, fixed_segments=fixed_segments, inpainting=True
    )
...
def settings_step():
    ...
    if not workflow.rfdiffusion_params.contig:
        st.error(
            "Contig has not been computed. Please fill in all inputs in previous step."
        )
        return
    ...
    with st.columns([1, 2])[0]:
        workflow.rfdiffusion_params.model_weights = st.selectbox(
            ...
            index=(
                MODEL_WEIGHTS_SCAFFOLD.index(workflow.rfdiffusion_params.model_weights)
...
def check_contig_parsed(contig: str | None, verbose: bool = False) -> bool:
    ...
    if not len(contig.split(" ")) == 1:
        if verbose:
            st.warning(
                "Support of multi-chain scaffold design is experimental, proceed with caution"
            )
