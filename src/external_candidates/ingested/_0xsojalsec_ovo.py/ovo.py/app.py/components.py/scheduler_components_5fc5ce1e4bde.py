# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\scheduler_components.py
import time
        ...
        while True:
            ...
            if hasattr(scheduler, "queue_size"):
                ...
                if queue_size is None:
                    ...
                    statusbar.update(
                        state="running", label=f"Waiting in queue{behind_label}..."
                    )

            if time.time() - watching_since > timeout_seconds:
                scheduler.cancel(job_id)
                raise TimeoutError(
                    f"Waiting timed out after {timeout_seconds} seconds, cancelling job {job_id}"
                )
