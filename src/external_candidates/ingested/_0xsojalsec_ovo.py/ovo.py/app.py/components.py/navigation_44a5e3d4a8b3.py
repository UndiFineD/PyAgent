# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\navigation.py
...
from ovo import db
from ovo.core.database import Design, Round
...
def show_buttons(
    ...
    with st.container(key="navigation_buttons_" + key_suffix):
        left, middle, right = st.columns(
            [6, 1, 2] if current_index == 0 else [2, 3, 2], vertical_alignment="top"
        )
        with left:
            if current_index > 0:
...
def project_round_selector(
    ...
    if design_ids_key not in st.session_state:
        ...
        if st.query_params.get(DESIGN_IDS_QUERY_PARAM) and allow_design_input:
            st.session_state[design_ids_key] = st.query_params[
                DESIGN_IDS_QUERY_PARAM
            ].replace(",", "\n")
            single_default = ["designs"]

    if single_key not in st.session_state and st.query_params.get(
        ROUND_IDS_QUERY_PARAM
    ):
    ...
    round_id = st.segmented_control(
        ...
        format_func=lambda round_id: (
            ...
            if round_id == "all"
                ...
                "Specific designs"
                if round_id == "designs"
                else rounds_by_id[round_id].name
            )
        ),
    ...
    if round_id == "designs":
        ...
        if found_design_ids != set(design_ids):
            ...
            st.warning(
                f"{len(missing_ids):,} design IDs were not found: {', '.join(missing_ids)}"
            )
            if not found_design_ids:
                st.stop()
        elif found_design_ids:
            st.success(
                f"Showing {len(found_design_ids):,} "
                + ("design" if len(found_design_ids) == 1 else "designs")
            )
...
def pool_selector_table(pools_table: pd.DataFrame, project_id: str) -> list[str]:
    ...
    default_selection_key = (
        f"pool_default_selection_{get_hash_of_bytes(pools_table.to_csv().encode())}"
    )
    if default_selection_key in st.session_state:
        ...
        default_selection = (
            st.query_params.get("pool_ids").split(",")
            if st.query_params.get("pool_ids")
            else []
        )
        st.session_state[default_selection_key] = default_selection
    pools_table.insert(
        0, "Selected", [pool_id in default_selection for pool_id in pools_table["ID"]]
    )
    pools_table["Job ID"] = [
        (
            f"./jobs?pool_ids={pool_id}&project_id={project_id}"
    ...
    if edited["Selected"].any():
        ...
        st.query_params["pool_ids"] = ",".join(
            selected_pools_table["ID"].astype(str).tolist()
        )
    else:
        selected_pools_table = pools_table
        st.caption(
            "Showing designs from all pools. Use checkboxes above to select specific pools."
        )
...
def design_navigation_selector(
    ...
    if key in st.query_params and st.query_params[key] in design_ids:
        ...
        if (
            element_key in st.session_state
            and st.session_state[element_key] in design_ids
        ):
    ...
    with st.container(horizontal=True, gap="small", vertical_alignment="center"):
        ...
        with count_container:
            ...
            curr = (
                "All" if design_ids[idx] == "ALL" else (idx if allow_all else idx + 1)
            )
            st.html(
        ...
        def format_func(design_id):
            ...
            if fmt:
                label = (
                    fmt.get(design_id, "") if isinstance(fmt, dict) else fmt(design_id)
                )
            else:
                label = ""
