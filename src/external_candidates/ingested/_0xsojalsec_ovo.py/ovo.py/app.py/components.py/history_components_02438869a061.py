# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\history_components.py
...
def history_dropdown_component(
    ...
    pools_by_id, jobs_by_id = get_cached_workflow_pools_and_jobs(
        st.session_state.project.id, workflow_names
    )
    if filter_func:
        pools_by_id = {
            pool_id: pool
    ...
    pool_ids_by_date = [
        p.id
        for p in sorted(
            pools_by_id.values(), key=lambda p: p.created_date_utc, reverse=True
        )
    ]
    pool_id: str = st.selectbox(
        ...
        placeholder=(
            ("1 run available" if num_options == 1 else f"{num_options} runs available")
            if num_options
    ...
    if pool_id != previous_pool_id:
        if pool_id:
            ...
            st.session_state.workflows[page_key] = deepcopy(
                st.session_state.initial_workflows[page_key]
            )
...
def format_label(pool_id: str, pools_by_id: dict[str, Pool]):
    ...
    return "{id} | {name} | {when_created}".format(
        ...
        when_created=(
            timeago.format(delta) if delta.total_seconds() > 3600 else "a moment ago"
        ),
    )
