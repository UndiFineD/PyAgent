...
def create_new_pool():
    ...
    with content.container():
        ...
        name = st.text_input(
            "Pool name *", placeholder="Descriptive name for this collection of designs"
        )

        if name and db.count(Pool, round_id=round_id, name=name):
            st.error(
                f"A pool with this name already exists in this round. Please choose a different name."
            )
            ...
            if file.name.endswith(".pdb"):
                designs.append(
                    Design.from_pdb_file(
                        filename=file.name, pdb_str=file.read().decode(), **shared_args
                    )
                )
