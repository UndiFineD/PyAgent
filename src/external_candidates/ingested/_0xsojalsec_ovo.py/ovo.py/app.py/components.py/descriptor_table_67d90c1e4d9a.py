...
def descriptor_table(
    design_ids: List[str], descriptors_df: pd.DataFrame, descriptors: List[Descriptor]
):
    ...
    assert (
        descriptors_df.columns.nlevels == 2
    ), "Expected nested=True table with 2 levels (tool name, descriptor name)"
    assert len(set(d.key for d in descriptors)) == len(
        descriptors
    ...
    for descriptor in descriptors:
        ...
        styles[col] = selected_df[col].map(
            make_bg_color_func(descriptor, min_val, max_val)
        )
        if isinstance(descriptor, ResidueNumberDescriptor):
            unique_values = set(
                x.strip()
                for row in descriptors_df[col].dropna()
                for x in row.split(",")
            )
            column_config[col_idx] = get_residue_number_column_config(
                descriptor, unique_values
            )
        else:
...
def residue_number_descriptor_detail_table(
    descriptor: Descriptor, descriptor_values: pd.Series
):
    """
    ...
    bool_df = pd.DataFrame(
        bool_data, index=descriptor_values.index, columns=unique_residues
    )
    ...
    formatted_strings = {
        design_id: f"{design_id} | #residues={row['# residues present']}"
        for design_id, row in bool_df.iterrows()
    }
