...
def pool_submission_inputs(page_key: str):
    ...
    with st.columns([2, 1])[0]:
        ...
        previous_pools = db.select(
            Pool, round_id=round_id, limit=3, order_by="-created_date_utc"
        )
        if previous_pools:
            st.write(
                f"*Previous pool names: {', '.join([p.name for p in previous_pools if p.name])}*"
            )
        if db.count(Pool, round_id=round_id, name=pool_name):
            st.error(
                f"Pool '{pool_name}' already exists in this round, please choose a different name."
            )
...
def review_workflow_submission(page_key: str):
    ...
    st.table(
        workflow.get_table_row(name="value").apply(
            lambda v: ", ".join(map(str, v)) if isinstance(v, list) else v
        ),
    )

    if st.button(
        "Continue to submission :material/arrow_forward_ios:", key="continue"
    ) or is_test_dialog_shown("submission_dialog"):
...
def submit_workflow_dialog(
    ...
    with content.container():
        ...
        with st.columns([2, 1])[-1]:
            submitted = st.button(
                "Submit", type="primary", width="stretch", key="submit_run"
            )
...
def create_new_round_dialog():
    ...
    if st.button("Create round", disabled=not round_name, key="create_round"):
        project_round = Round(
            project_id=project_id, name=round_name, author=get_username()
        )
...
def show_rfdiffusion_binder_seq_design_inputs(workflow: RFdiffusionWorkflow):
    ...
    seq_design_method = st.radio(
        ...
        index=(
            options.index(seq_design_method) if seq_design_method in options else None
        ),
    ...
    if seq_design_method == "fastrelax":
        ...
        if workflow.protein_mpnn_params.fastrelax_cycles:
            ...
            workflow.protein_mpnn_params.num_sequences = (
                ProteinMPNNParams().num_sequences
            )
...
def show_rfdiffusion_advanced_settings(workflow: RFdiffusionWorkflow):
    ...
    if not config.props.pyrosetta_license:
        for key, threshold in workflow.acceptance_thresholds.items():
            if "rosetta" in key and threshold.enabled:
                st.warning(
                    f"Disabling PyRosetta acceptance threshold as PyRosetta license is not available: {key}"
                )
    ...
    with st.expander(label="Show advanced settings"):
        ...
        st.caption(
            f"Supported filtering metrics: {', '.join(descriptors_rfdiffusion.BACKBONE_METRIC_FIELD_NAMES)}"
        )
        ...
        st.caption(
            ":material/info: Reference: https://github.com/RosettaCommons/RFdiffusion."
        )
        if workflow.rfdiffusion_params.run_parameters:
            st.warning(
                "Additional parameters can override previously selected parameters."
            )
        ...
        workflow.protein_mpnn_params.sampling_temp = st.number_input(
            ...
            or (
                0.0001 if workflow.is_instance(RFdiffusionBinderDesignWorkflow) else 0.1
            ),
            key="sampling_temp",
        )
        st.caption(
        ...
        supported_refolding_tests = REFOLDING_TESTS_BY_TYPE[
            workflow.get_refolding_design_type()
        ]
...
def customize_json_settings_component(
    diff: dict | None, default: dict, key_suffix: str
):
    """
    ...
    if new_diff:
        st.warning(
            f"You have customized the '{key_suffix}' settings. See changes below:"
        )
...
def show_bindcraft_advanced_settings(workflow: "BindCraftBinderDesignWorkflow"):
    ...
    if advanced_default_dict is None or filter_default_dict is None:
        st.error(
            "Cannot display settings component because the default settings failed to load."
        )
        return
    ...
    with st.expander(label="Advanced settings and filter settings"):
        ...
        if workflow.bindcraft_params.custom_filter_settings and not filter_edit_mode:
            if st.button(
                "Reset Filter Settings to Default", key="reset_filter", type="primary"
            ):
