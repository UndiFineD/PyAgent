# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\trim_components.py
...
def parameters_trim_structure_component(workflow: RFdiffusionBinderDesignWorkflow):
    st.write(
        "Trim the target chain to save computation time, and to avoid running out of GPU memory"
    )

    if workflow.rfdiffusion_params.hotspots:
        st.write(f"Binding hotspots: {workflow.rfdiffusion_params.hotspots}")
        chains_in_hotspot = set(
            [hotspot[0] for hotspot in workflow.rfdiffusion_params.hotspots.split(",")]
        )
    ...
    target_chain_col, start_trimmed_col, end_trimmed_col, _ = st.columns(
        [0.15, 0.15, 0.15, 0.45]
    )
    with target_chain_col:
        try:
            selected_idx = (
                list(workflow.chains.keys()).index(workflow.target_chain)
                if workflow.target_chain
                else None
            )
        except ValueError:
    ...
    residues = list(
        map(int, from_contig_to_residues(workflow.chains.get(target_chain)))
    )
    ...
    with start_trimmed_col:
        ...
        if start_res not in residues:
            start_res = min(
                [res for res in residues if res > start_res],
                key=lambda res: abs(res - start_res),
            )
            st.warning(
    ...
    with end_trimmed_col:
        ...
        if end_res not in residues:
            end_res = min(
                [res for res in residues if res < end_res],
                key=lambda res: abs(res - end_res),
            )
            st.warning(
    ...
    trimmed_residues = [
        res for res in residues if (res <= end_res) and (res >= start_res)
    ]
...
def trimmed_structure_visualizer(
    workflow: RFdiffusionBinderDesignWorkflow, pdb_input_string: str
):
    target_chain = workflow.get_target_chain()
    trim_start, trim_end = workflow.get_target_trim_boundary()
    pdb_input_string_trimmed = trim_pdb_str(
        pdb_input_string, target_chain, trim_start, trim_end
    )
        ...
        molstar_custom_component(
            structures=[
                StructureVisualization(
                    ...
                    contigs=parser.parse_contigs_str(
                        f"{target_chain}{trim_start}-{trim_end}"
                    ),
