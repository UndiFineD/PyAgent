# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\descriptor_dialog.py
...
from ovo.app.components import (
    ChainVisualization,
...
def detail_descriptor_dialog(
    descriptor, descriptor_values: pd.Series, design_id: str = None
):
    st.markdown(
        """
    <style>
    ...
    with col2:
        detail_design(
            descriptor, descriptor_values, table_data, format_func, design_id=design_id
        )
...
def detail_table(descriptor, descriptor_values: pd.Series):
    ...
    if isinstance(descriptor, CategoricalResidueDescriptor):
        ...
        table_data = table_data.sort_values(
            by=["category_abundance", "value"], ascending=[False, True]
        )
        column_config = {
        ...
        format_func = (
            lambda design_id: f"{design_id} | {descriptor.name} = {table_data.value.loc[design_id]}"
        )
        ...
        min_val = (
            ...
            else float(table_data["value"].min())
        )
        max_val = (
            descriptor.max_value
            if descriptor.max_value is not None
            else float(table_data["value"].max())
        )
        ...
        format_func = (
            lambda design_id: f"{design_id} | {descriptor.name} = {table_data.value.loc[design_id]:.2f}"
        )
...
def detail_design(
    ...
    design_id = st.selectbox(
        ...
        index=(
            0
            if design_id is None or design_id not in table_data.index
            else table_data.index.get_loc(design_id)
        ),
    )
    ...
    with right:
        ...
        download_job_designs_component(
            design_ids=[design_id], pools=[pool], key="single", single_line=False
        )
