...
def show_design_metrics(design_id: str, descriptor_keys: list[str]):
    descriptor_values = get_cached_design_descriptors(
        design_id, descriptor_keys=descriptor_keys
    )
...
def select_structure_prediction_descriptor(
    ...
    prediction_descriptors = [
        d
        for d in STRUCTURE_PATH_DESCRIPTORS
        if d.structure_type == "prediction" and d.key in paths
    ]
...
def rfdiffusion_scaffold_design_visualization(design_id: str | None):
    ...
    trb_dict = storage.read_file_pickle(
        paths[descriptors_rfdiffusion.RFDIFFUSION_TRB_PATH.key]
    )
    input_pdb_str = storage.read_file_str(
        workflow.get_input_pdb_path(design.contig_index)
    )
    input_segments = [
        segment
        for contig in trb_dict["sampled_mask"]
        for segment in parser.parse_contigs_str(contig)
    ]
    ...
    with middle:
        ...
        input_motif_pdb = filter_pdb_str(
            input_pdb_str, [s.value for s in input_segments if s.type == "fixed"]
        )
        structures = [
            StructureVisualization(
                pdb=input_motif_pdb,
        ...
        num_fixed = sum(
            segment.out_res_end - segment.out_res_start + 1
            for segment in output_segments
            if segment.type == "fixed"
        )
        ...
        molstar_custom_component(
            ...
            height=350,
            html_filename=os.path.basename(prediction_storage_path).replace(
                ".pdb", "_aligned_motif"
            ),
        )
...
def rfdiffusion_multiple_binder_designs_visualization(
    all_design_ids: list[str], max_examples: int = 20
):
    ...
    # TODO add caching
    backbone_paths = (
        db.select_descriptor_values(
            descriptors_rfdiffusion.RFDIFFUSION_STRUCTURE_PATH.key, example_design_ids
        )
        .dropna()
        .to_dict()
    )
    ...
    with left:
        ...
        aligned_pdb_strs, rmsd = align_multiple_proteins_pdb(
            pdb_strs=[
                storage.read_file_str(backbone_paths[design.id])
                ...
                StructureVisualization(
                    ...
                    chains=[
                        ChainVisualization(
                            chain_id="A",
                            color_params={
                                "value": get_color_from_str(
                                    design.id, "seaborn:tab10_light"
                                )
                            },
                            representation_type="cartoon",
...
def rfdiffusion_binder_design_visualization(design_id: str):
    ...
    hotspot_selections = (
        [
            h.replace(workflow.target_chain, "B")
            for h in workflow.rfdiffusion_params.hotspots.split(",")
        ]
...
def bindcraft_binder_design_visualization(design_id: str):
    ...
    molstar_custom_component(
        structures=[
            StructureVisualization(pdb=storage.read_file_str(design.structure_path))
        ],
        key="bindcraft_1",
    )
...
def visualize_rfdiffusion_design_sequence(design_id: str):
    ...
    paths = (
        get_cached_design_descriptors(
            design_id, [descriptors_rfdiffusion.RFDIFFUSION_TRB_PATH.key]
        )
        .dropna()
        .to_dict()
    )
...
def visualize_scaffold_alignment(
    ...
    for s in parsed_segments:
        ...
        for chunk_offset in range(0, segment_length, 10):
            ...
            generated_seq = designed_sequences[s.out_res_chain][
                output_start - 1 : output_start + chunk_length - 1
            ]
            if s.type == "generated":
                label = s.value if chunk_offset == 0 else "&nbsp;"
                input_seq = " " * chunk_length
                input_seq_formatted = (
                    "Generated&nbsp;" if chunk_offset == 0 else "&nbsp;" * 10
                )
                ...
                label = (
                    f"{s.input_res_chain}{input_start}-{input_start + chunk_length - 1}"
                )
                input_positions = list(range(input_start, input_start + chunk_length))
                input_seq = "".join(
                    [
                        input_seq_by_resno[s.input_res_chain].get(str(pos), "?")
                ...
                input_seq_formatted = "".join(
                    [
                        (
                            f"<b>{aa}</b>"
                            if f"{s.input_res_chain}{pos}" in inpainted_positions
                            else aa
                        )
                ...
                generated_seq_formatted = "".join(
                    [
                        f"<b>{aa}</b>" if aa != bb else aa
                        for aa, bb in zip(generated_seq, input_seq)
                    ]
                )
    ...
    contig_identity = (
        sum(aa == bb for aa, bb in zip(aligned_input, aligned_design))
        / len(aligned_design)
        if aligned_design
        else None
    )
