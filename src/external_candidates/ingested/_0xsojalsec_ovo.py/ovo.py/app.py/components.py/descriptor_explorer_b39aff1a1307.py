# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\descriptor_explorer.py
...
class HistogramSettings:
    ...
    def from_query_params(
        cls, descriptors_by_key: dict[str, Descriptor], key_prefix: str
    ):
        return cls(
            descriptor=(
                descriptors_by_key[st.query_params[f"{key_prefix}_descriptor"]]
                if f"{key_prefix}_descriptor" in st.query_params
                and f"{key_prefix}_descriptor" in descriptors_by_key
                else None
            ),
            source=(
                st.query_params[f"{key_prefix}_source"]
                if f"{key_prefix}_source" in st.query_params
                else None
            ),
        )
...
def descriptor_explorer(
    ...
    with descriptor_col:
        descriptor = st.selectbox(
            ...
            index=(
                descriptor_options.index(settings.descriptor.key)
                if settings.descriptor is not None
                else 0
            ),
    ...
    descriptor_values = descriptors_df[
        (settings.descriptor.tool, settings.descriptor.name)
    ]
    ...
    if not single_design:
        with median_col:
            ...
            median_value = format_threshold_value(
                settings.descriptor.name, median_value
            )
            st.metric(f"Median {settings.descriptor.name}", median_value)
    with warning_col:
        warning_value = format_threshold_value(
            settings.descriptor.name, settings.descriptor.warning_value
        )
        st.metric("Warning threshold", warning_value)
    with error_col:
        error_value = format_threshold_value(
            settings.descriptor.name, settings.descriptor.error_value
        )
