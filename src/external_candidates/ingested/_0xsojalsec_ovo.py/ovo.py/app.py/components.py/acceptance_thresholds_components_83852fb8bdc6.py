# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\acceptance_thresholds_components.py
import pandas as pd
import streamlit as st
from ovo import Threshold, db
from ovo.app.components.custom_elements import wrapped_columns
from ovo.app.utils.cached_db import Descriptor, get_cached_descriptor_values
from ovo.core.database import DesignJob, Pool
...
def thresholds_and_histograms_component(
    ...
    columns = wrapped_columns(
        len(descriptor_keys), wrap=max_items_row, divider=True, gap="large"
    )
    for descriptor_key, column in zip(descriptor_keys, columns):
        with column:
            ...
            descriptor_values = get_cached_descriptor_values(
                descriptor.key, design_ids=all_design_ids
            )
            ...
            if new_thresholds[descriptor.key].enabled and (
                num_missing := descriptor_values.isna().sum()
            ):
                st.warning(
    ...
    for descriptor_key, column in zip(descriptor_keys, columns):
        with column:
            ...
            if descriptor.description:
                st.caption(descriptor.description)
            new_thresholds[descriptor.key] = single_threshold_input_component(
                threshold=selected_thresholds[descriptor_key],
                descriptor=descriptor,
...
def single_threshold_input_component(
    ...
    kwargs = dict(
        min_value=(
            float(descriptor.min_value) if descriptor.min_value is not None else None
        ),
        max_value=(
            float(descriptor.max_value) if descriptor.max_value is not None else None
        ),
...
def descriptor_histogram_component(
    descriptor_values: pd.Series, descriptor: Descriptor, threshold: Threshold
):
    fig = px.histogram(
        x=descriptor_values,
...
def filter_designs_by_thresholds_cached(
    ...
    return filter_designs_by_thresholds(
        ...
        values={
            descriptor_key: get_cached_descriptor_values(
...
def accept_designs_dialog(
    ...
    if len(pools) > 1:
        st.write(
            f"This will update {len(pools):,} pools: **{', '.join(p.id for p in pools)}**"
        )
    ...
    with st.columns(2)[1]:
        if st.button("Save", type="primary", width="stretch"):
            ...
            update_accepted_design_ids(
                pool_ids=updated_pool_ids, accepted_design_ids=new_accepted_design_ids
            )
