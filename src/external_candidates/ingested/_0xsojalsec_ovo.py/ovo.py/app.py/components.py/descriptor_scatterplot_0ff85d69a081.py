# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\descriptor_scatterplot.py
...
from ovo.core.database import Descriptor, NumericGlobalDescriptor, Threshold
...
class PlotSettings:
    ...
    def from_query_params(cls, descriptors_by_key: dict[str, Descriptor]):
        for field in ["x", "y", "color"]:
            ...
            if (
                field in st.query_params
                and st.query_params[field] not in descriptors_by_key
            ):
                st.warning(f"Descriptor not available: {st.query_params[field]}")
                st.query_params.pop(field)
        return cls(
            x=(
                descriptors_by_key[st.query_params["x"]]
                if "x" in st.query_params
                else None
            ),
            y=(
                descriptors_by_key[st.query_params["y"]]
                if "y" in st.query_params
                else None
            ),
            color=(
                descriptors_by_key[st.query_params["color"]]
                if "color" in st.query_params
                else None
            ),
            color_type=(
                st.query_params["color_type"]
                if "color_type" in st.query_params
                else "range"
            ),
        )
...
def descriptor_scatterplot_input_component(
    ...
    descriptors_by_key = {
        ...
        if isinstance(d, NumericGlobalDescriptor)
    }
    ...
    if st.session_state.get("scatterplot_preset") or (
        not settings.x and not settings.y and available_presets
    ):
        ...
        "scatterplot_x"
    ) and settings.get_x_key() != st.session_state.get("scatterplot_x"):
        ...
        "scatterplot_y"
    ) and settings.get_y_key() != st.session_state.get("scatterplot_y"):
    ...
    with x_col:
        st.selectbox(
            "X axis",
            format_func=lambda descriptor_key: format_descriptor_name(
                descriptors_by_key[descriptor_key]
            ),
...
def descriptor_scatterplot_design_explorer_component(
    settings: PlotSettings, design_ids: list[str]
) -> list[str]:
    ...
    color_values = pd.Series(
        [design_id.split("_")[1] for design_id in design_ids], index=design_ids
    )
    ...
    scatterplot_df = pd.DataFrame(
        {"x": x_values, "y": y_values, "color": color_values, "design_id": design_ids}
    )
    fig = px.scatter(
        scatterplot_df,
        x="x",
    ...
    if not event["selection"]["box"] and not event["selection"]["lasso"]:
        ...
        if box := event["selection"]["box"]:
            ...
            selection_label = f"with **{format_range(x_min, x_max, settings.x)}** and **{format_range(y_min, y_max, settings.y)}**"
        else:
...
def descriptor_scatterplot_pool_details_component(
    ...
    if highlight_accepted:
        ...
        layout = go.Layout(
            xaxis=dict(
                title=settings.x.name,
                range=settings.x.get_plot_range(scatterplot_df["x"]),
            ),
            yaxis=dict(
                title=settings.y.name,
                range=settings.y.get_plot_range(scatterplot_df["y"]),
            ),
            width=800,
            height=600,
        )
    ...
    if selected_thresholds and (
        settings.x.key in selected_thresholds or settings.y.key in selected_thresholds
    ):
        ...
        if (
            settings.x.key in selected_thresholds
            and selected_thresholds[settings.x.key].enabled
        ):
            ...
            if is_int and x1 is not None:
                x1 += 0.5
        if (
            settings.y.key in selected_thresholds
            and selected_thresholds[settings.y.key].enabled
        ):
    ...
    if not event["selection"]["box"] and not event["selection"]["lasso"]:
        ...
        if box := event["selection"]["box"]:
            x_min, x_max = get_trimmed_min_max(box[0]["x"], settings.x)
            y_min, y_max = get_trimmed_min_max(box[0]["y"], settings.y)
            selection_label = f"**{format_range(x_min, x_max, settings.x)}** and **{format_range(y_min, y_max, settings.y)}**"
        else:
            selection_label = "Using lasso selection"
...
def print_missing(values_by_name: dict[str, pd.Series]):
    ...
    num_missing = reduce(
        lambda x, y: x | y, [values.isna() for values in values_by_name.values()]
    ).sum()
    if num_missing == num_total:
        ...
        st.warning(
            f"Not showing {num_missing:,}/{num_total:,} designs with missing "
            + " or ".join(
                f"**{name}** ({count:,} missing)"
                for name, count in num_missing_by_name.items()
            )
        )
