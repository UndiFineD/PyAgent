...
def create_project_dialog():
    ...
    if st.button("Create project"):
        ...
        if db.count(Project, name=project_name, public=True):
            st.error(
                f'Public project "{project_name}" already exists. Please choose another name.'
            )
            return
        elif db.count(Project, name=project_name, author=author):
            st.error(
                f'You already have a private project named "{project_name}". Please choose another name.'
            )
            return
...
def get_query_arg_project() -> Project | None:
    if project_id := st.query_params.get("project_id"):
        if (
            not config.auth.allow_private_project_link_access
            and project_id
            not in get_cached_project_ids_and_names(username=get_username())
        ):
            st.error(
                f"Project URL **{project_id}** not accessible, redirecting to last project."
            )
...
def project_sidebar_component():
    ...
    if st.session_state.project is None:
        if project := get_query_arg_project():
            ...
            if not project_ids_and_names:
                st.error(
                    f"No projects available to user {get_username()} in read-only mode."
                )
                st.stop()
            st.session_state.project = db.get(
                Project, id=list(project_ids_and_names.keys())[0]
            )
        else:
            # Restore last selected project or select personal project
            user_settings = get_or_create_user_settings()
            if user_settings.last_project_id and (
                project := db.get(Project, user_settings.last_project_id)
            ):
    ...
    project_ids = sorted(
        project_ids_and_names.keys(), key=lambda x: project_ids_and_names[x].lower()
    )
    ...
    selected_project_id = st.session_state.get(
        dropdown_key, st.session_state.project.id
    )

    if st.session_state.new_project_name:
        # Select project if it was just created
        success_message = (
            f'Project "**{st.session_state.new_project_name.strip()}**" created'
        )
