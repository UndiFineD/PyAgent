...
def refresh_descriptors(
    design_ids: list[str] | set[str], workflow_names: list[str] = None
):
    ...
    with st.spinner("Updating descriptor job status..."):
        ...
        pending_or_failed_jobs = [
            j
            for j in pending_or_failed_jobs
            if j.workflow and set(design_ids).intersection(j.workflow.design_ids)
        ]
        ...
        if workflow_names:
            ...
            pending_or_failed_jobs = [
                j
                for j in pending_or_failed_jobs
                if j.workflow and j.workflow.name in workflow_names
            ]

        pending_jobs = [j for j in pending_or_failed_jobs if j.job_result is None]
        update_and_process_descriptors(
            descriptor_jobs=pending_jobs, error_callback=st.error
        )
        ...
        for job in failed_jobs:
            failed_design_ids = sorted(
                set(design_ids).intersection(job.workflow.design_ids)
            )
            num_failed_designs = len(failed_design_ids)
            design_ids_str = ", ".join(failed_design_ids[:3]) + (
                "..." if len(failed_design_ids) > 3 else ""
            )
            with st.expander(
