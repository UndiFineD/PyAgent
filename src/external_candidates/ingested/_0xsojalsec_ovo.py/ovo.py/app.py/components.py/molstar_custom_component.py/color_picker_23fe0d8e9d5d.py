# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\molstar_custom_component\color_picker.py
import random
...
def mix_colors(first_hex, second_hex):
    return "#" + "".join(
        f"{(int(first_hex[i : i + 2], 16) + int(second_hex[i : i + 2], 16)) // 2:02x}"
        for i in (1, 3, 5)
    )
