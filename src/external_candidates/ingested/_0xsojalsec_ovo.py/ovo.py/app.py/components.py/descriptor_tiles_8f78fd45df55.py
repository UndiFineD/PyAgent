# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\descriptor_tiles.py
...
def descriptor_overview_tiles(
    ...
    af_descriptors = [
        descriptor
        for descriptor in AF2_PRIMARY_DESCRIPTORS
        if descriptor.key in descriptors_by_key
    ]
    if af_descriptors:
    ...
    esmfold_descriptors = [
        descriptor
        for descriptor in ESMFOLD_DESCRIPTORS
        if descriptor.key in descriptors_by_key
    ]
    ...
    backbone_metrics_descriptors = [
        descriptor
        for descriptor in descriptors_rfdiffusion.BACKBONE_METRICS
        if descriptor.key in descriptors_by_key
    ]
...
def get_residue_presence_df(
    descriptor: Descriptor, nonnull_descriptor_values, total_designs: int
):
    """
    ...
    nonnull_values = [
        v for v in nonnull_descriptor_values if isinstance(v, str) and v.strip()
    ]
    all_residues = []
    ...
    residue_df = pd.DataFrame(
        {
            ...
            "% of designs": [
                100 * count / total_designs for count in residue_in_designs.values()
            ],
        }
    )
...
def show_flag_counts(
    descriptor: NumericGlobalDescriptor, values: pd.Series, design_id: str = None
):
    ...
    for flag, ids in warning_ids.items():
        ...
        st.markdown(
            f":{color}-background[{icon} **{len(ids):,}** {label}]",
            help=truncated_list(ids, 10),
        )
            ...
            with st.container(border=True):
                ...
                if isinstance(descriptor, NumericGlobalDescriptor):
                    descriptor_values = pd.to_numeric(
                        descriptor_values, errors="coerce"
                    )
                ...
                if len(descriptor_values) > len(nonnull_descriptor_values):
                    perc_missing = (
                        1 - len(nonnull_descriptor_values) / len(descriptor_values)
                    ) * 100
                    st.markdown(
                ...
                if isinstance(descriptor, CategoricalResidueDescriptor):
                    ...
                    counts_df = (
                        pd.Series(nonnull_descriptor_values)
                        .value_counts()
                        .head(top_n)
                        .reset_index()
                    )
                    ...
                    with st.container(horizontal=True, horizontal_alignment="right"):
                        detail_button(
                            descriptor, descriptor_values, design_id=design_id
                        )
                    ...
                    if design_id:
                        st.metric(
                            descriptor.name,
                            format_threshold_value(
                                descriptor.name, descriptor_values.loc[design_id]
                            ),
                        )
                    else:
                        avg_value = np.mean(nonnull_descriptor_values)
                        st.metric(
                            f"Average {descriptor.name}",
                            format_threshold_value(descriptor.name, avg_value),
                        )
                ...
                # Support for ResidueNumberDescriptor
                    ...
                    residue_df = get_residue_presence_df(
                        descriptor, nonnull_descriptor_values, descriptors_df.shape[0]
                    )
                    if residue_df is None or residue_df.empty:
                        ...
                        st.dataframe(
                            residue_df,
                            column_config={
                                descriptor.name: st.column_config.Column(
                                    descriptor.name
                                ),
