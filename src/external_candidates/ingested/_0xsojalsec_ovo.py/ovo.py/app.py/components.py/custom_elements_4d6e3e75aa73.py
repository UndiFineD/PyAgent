# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\custom_elements.py
import html
...
def heading_with_value(
    heading: str, secondary_text: str, level: int = 2, font_weight: int = 800
):
...
def subheading_with_value(
    heading: str, secondary_text: str, level: int = 3, font_weight: int = 600
):
    return heading_with_value(
        heading, secondary_text, level=level, font_weight=font_weight
    )
...
def confirm_download_button(data, **kwargs):
    ...
    st.download_button(
        "Confirm download" + size_suffix, type="primary", data=data, **kwargs
    )
