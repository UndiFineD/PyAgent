# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\app\components\download_component.py
import os

import streamlit as st
from ovo import Pool, storage
...
def download_job_designs_component(
    ...
    file_descriptors = [
        ...
        if isinstance(d, FileDescriptor) and d.key in available_descriptors
    ]
...
def download_descriptor_table(
    filename, design_ids, descriptor_keys=None, key="default", width="content"
):
    if st.button(
...
def download_design_files(
    ...
    if st.button(f"Download {label}", key=f"prepare_{filename}_{key}", width="stretch"):
        ...
        if not storage_paths:
            ...
            confirm_download_button(
                data=data, file_name=filename, key=f"download_{filename}_{key}"
            )
        else:
            # Multiple files, prepare zip
            with st.spinner(
                f"Preparing zip file with {len(storage_paths):,} results..."
            ):
            ...
            confirm_download_button(
                ...
                key=f"download_{filename}_{key}",
            )
