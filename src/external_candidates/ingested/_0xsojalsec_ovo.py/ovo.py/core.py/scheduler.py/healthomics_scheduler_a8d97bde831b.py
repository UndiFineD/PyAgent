...
class HealthOmicsScheduler(Scheduler):
    def __init__(
        ...
        """
        super().__init__(
            name, workdir, reference_files_dir, allow_submit, submission_args
        )
        self.aws = aws

    def submit(
        self, pipeline_name: str, params: dict = None, submission_args: dict = None
    ) -> str:
        """
        ...
        assert isinstance(
            params, dict
        ), f"params should be a dictionary, got: {type(params).__name__}"
        ...
        if re.fullmatch(r"https?://.*", pipeline_name):
            raise NotImplementedError(
                "GitHub URL pipeline names are not supported yet with HealthOmicsScheduler"
            )
        elif (
            "/" in pipeline_name
            or pipeline_name == "."
            or pipeline_name.endswith(".nf")
        ):
            raise NotImplementedError(
                "Local path pipeline names are not supported with HealthOmicsScheduler"
            )
        elif (
            "." in pipeline_name
        ):  # custom workflow path (python_module_name.my_workflow)
        ...
        omics_workflow_name = (
            workflow_name_prefix + module_name + "_" + workflow_subpath
        )
        ...
        if submission_args:
            ...
            raise ValueError(
                f"Unrecognized {type(self).__name__} submission args: {submission_args.keys()}"
            )
