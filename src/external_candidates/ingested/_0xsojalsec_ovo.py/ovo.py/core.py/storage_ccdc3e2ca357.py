...
class Storage:
    ...
    def _cache_read(self, file_path):
        ...
        if file_path in self._cache_disk:
            ...
            while (
                self.memory_cache_size > self.memory_cache_limit and self._cache_memory
            ):
    ...
    def _cache_store(self, file_path, content: str | bytes):
        ...
        if not scheme:
            ...
            return
        assert relative_path in os.path.abspath(
            relative_path
        ), f"Invalid path {relative_path}"  # make sure path doesn't contain '../'
    ...
    def list_dir(self, abs_path: str, only_dir=False, recursive=False) -> list[str]:
        ...
        if recursive:
            if only_dir:
                ...
                return sorted(
                    os.path.join(root, filename).removeprefix(
                        abs_path.rstrip("/") + "/"
                    )
                    for root, dirs, files in os.walk(abs_path, followlinks=True)
    ...
    def store_file_path(
        self, source_abs_path: str, storage_rel_path: str, overwrite: bool = True
    ) -> str:
        ...
        if source_scheme == "s3":
            ...
            try:
                self.aws.s3.download_file(
                    Bucket=source_bucket, Key=source_path, Filename=storage_abs_path
                )
            except ClientError as e:
                print("S3 ERROR", e, e.response)
                if e.response["Error"]["Code"] in ("NoSuchKey", "404", 404):
                    raise FileNotFoundError(
                        f"File not found: s3://{source_bucket}/{source_path}"
                    ) from e
                raise
            return storage_rel_path
        else:
    ...
    def store_file_bytes(
        self, file_bytes: bytes, storage_rel_path: str, overwrite: bool = True
    ) -> str:
    ...
    def store_file_str(
        self, file_str: str, storage_rel_path: str, overwrite: bool = True
    ) -> str:
        """Store the file string in the source filesystem or source S3 bucket
        ...
        :param overwrite: if True, overwrite the file if it exists
        """
        return self.store_file_bytes(
            file_str.encode(), storage_rel_path, overwrite=overwrite
        )
    ...
    def store_input(
        ...
        if file_path:
            assert (
                file_bytes is None and file_str is None
            ), "Provide only one of file_path, file_bytes, or file_str"
            if filename is None:
            ...
            ), "filename must be provided when loading from bytes"
        elif file_str is not None:
            assert (
                filename is not None
            ), "filename must be provided when loading from string"
            file_bytes = file_str.encode()
        hash_str = get_hashed_path_for_bytes(file_bytes)
        storage_rel_path = os.path.join(
            "project", project_id, "inputs", hash_str, filename
        )
            ...
            with ThreadPoolExecutor() as executor:
                ...
                futures = {
                    (
                        file_path,
                        executor.submit(
                            self.read_file_bytes, file_path, cache_store=False
                        ),
                    ): dir_model
    ...
    def sync_file(
        self, storage_path: str, local_destination_path: str, link: bool = False
    ):
        ...
        try:
            ...
            raise ValueError(
                f"Use sync_directory to sync a directory, got: {storage_path}"
            )
    ...
    def sync_files(
        ...
        if preserve_subdirs:
            for path in storage_paths:
                assert (
                    not os.path.isabs(path) and "://" not in path
    ...
    def prepare_workflow_input(
        ...
        if workdir_scheme == "s3":
            ...
            self.aws.s3.put_object(
                Body=input_bytes, Bucket=workdir_bucket, Key=input_prefix
            )
            return s3_input_path
        else:
    ...
    def prepare_workflow_inputs(
        ...
        if single_directory:
            ...
            if names is not None:
                assert len(set(names)) == len(
                    names
                ), f"Names must be unique when single_directory=True, got: {names}"
            else:
                basenames = [os.path.basename(p) for p in storage_paths]
                assert len(set(basenames)) == len(
                    basenames
        ...
        paths = [
            self.prepare_workflow_input(
                storage_path, workdir=workdir, name=name, custom_hash=custom_hash
            )
        ...
        if single_directory:
            ...
            assert (
                len(set(subdirs)) == 1
            ), f"Expected all files to be in a single directory, got: {paths}"
            return subdirs[0] + "/"
        elif return_paths:
            return paths
        elif len(paths) == 1:
