# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\core\utils\residue_selection.py
import itertools
import json
...
def get_chains_and_contigs(pdb_str: str | None) -> Dict[str, str] | None:
        ...
        for chain in model:
            ...
            for residue in chain:
                ...
                if residue.id[0] == " " and int(residue.id[1]) > -1:
                    ...
                    )  # Get the residue number (not the insertion code)

            segments_by_chain[chain_id] = sorted(residues)

    segments_by_chain_json = {
        chain: parse_selections(
            json.dumps(
                {"sequenceSelections": [{"chainId": chain, "residues": list(residues)}]}
            )
        )
    ...
    return {
        chain: "/".join(segments) for chain, segments in segments_by_chain_json.items()
    }
...
def from_segments_to_hotspots(segments: list[str] | None) -> str:
    ...
    for segment in segments:
        ...
        hotspots.extend(
            [f"{chain}{resnum}" for resnum in range(start_resnum, end_resnum + 1)]
        )
...
def parse_partial_diffusion_binder_contig(binder_contig: str) -> tuple[int, list[str]]:
    ...
    """
    assert (
        " " not in binder_contig
    ), f"Expected single chain in binder_contig, got: {binder_contig}"
    pos = 0
    designed_segments = []
    for segment in binder_contig.split("/"):
        if segment[0].isalpha():
            ...
            assert (
                segment[0] == "A"
            ), f"Expected first contig in partial diffusion to be in chain A, got: {segment}"
            assert (
                "-" in segment
            ), f"Expected fixed segment in partial diffusion to be in format A2-3, got: {segment}"
            ...
            assert (
                "-" in segment
            ), f"Expected designed segment in partial diffusion to be in format 5-5, got: {segment}"
...
def create_partial_diffusion_binder_contig(
    redesigned_segments: list[str], binder_length: int
) -> str:
    ...
    """
    assert all(
        [segment[0] == "A" for segment in redesigned_segments]
    ...
    redesigned_positions = (
        [int(res[1:]) for res in redesigned_str.split(",")] if redesigned_str else []
    )
    all_positions = list(range(1, binder_length + 1))
    new_binder_segments = []
    for is_redesigned, positions in itertools.groupby(
        all_positions, lambda x: x in redesigned_positions
    ):
