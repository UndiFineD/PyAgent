...
def get_pdb(pdb_code: str) -> bytes:
    if len(pdb_code) == 4:
        ...
        base_url = (
            f"https://alphafold.ebi.ac.uk/files/AF-{pdb_code.upper()}-F1-model_v6.pdb"
        )
...
class PDBSegmentSelector(PDB.Select):
    def __init__(self, segments: list[str]):
        ...
        for segment in segments:
            ...
            assert (
                chain_id.isalpha()
            ), f"Invalid chain ID in segment, expected A123-456, got: {segment}"
            if len(segment) == 1:
                # full chain
                ...
                self.parsed_segments.append(
                    (chain_id, int(start_residue), int(end_residue))
                )
            else:
                # single residue
...
def detect_glycosylation_sites(
    ...
    residues = (
        atom_ppdb[["chain_id", "residue_number", "residue_name"]]
        .drop_duplicates()
    ...
    three_mers = pd.concat(
        [residues.chain_id]
        + [residues.residue_name.shift(-i) for i in range(len(glycosylation_pattern))],
        axis=1,
    )
    ...
    if cyclic:
        three_mers.loc[three_mers.index[-1], "resname2"] = three_mers.iloc[0].loc[
            "resname1"
        ]
        three_mers.loc[three_mers.index[-1], "resname3"] = three_mers.iloc[0].loc[
            "resname2"
        ]
        three_mers.loc[three_mers.index[-2], "resname3"] = three_mers.iloc[0].loc[
            "resname1"
        ]
    ...
    for chain in glycosylated_chains:
        ...
        for resnum in glycosylated_residues[
            glycosylated_residues.chain_id == chain
        ].index:
            glycosylation_dict[(chain, resnum)] = {}
            for atom in query_atoms:
                res_coords_df = chain_atom_ppdb.query("residue_number==@resnum")
                glycosylation_dict[(chain, resnum)][atom] = res_coords_df.query(
                    "atom_name==@atom"
                )[["x_coord", "y_coord", "z_coord"]].values.flatten()
...
def calculate_coords_from_transformed_displacements(P1, P2) -> np.ndarray:
    ...
    inverse_translation = (
        -inverse_rotation @ P1
    )  # Inverse translation based on the rotation
    ...
    for displacement in transformed_displacements:
        ...
        displacement_homogeneous = np.array(
            [*displacement, 0]
        )  # The last element is 0 for a displacement
...
def add_glycan_to_pdb(pdb_str: str) -> tuple[str, list[str] | None]:
    ...
    glycosylated_residues_str = [
        f"{chain_resnum[0]}{chain_resnum[1]}" for chain_resnum in glycosylated_residues
    ]
    ...
    for chain_and_resnum, atoms_coords_dict in glycosylated_residues.items():
        ...
        if P1 is not None and P2 is not None:
            coords_to_insert_glycane_per_resnum[chain_and_resnum] = (
                calculate_coords_from_transformed_displacements(P1, P2)
            )
    ...
    element_symbol = []
    for nag_resnum, ((NAG_chain, resnum), glycan_atoms) in enumerate(
        coords_to_insert_glycane_per_resnum.items()
    ):
...
def _align_sequences_and_get_indices(seqs: list[str]):
    ...
    for i, seq in enumerate(seqs):
        ...
        alignment = sorted(
            aligner.align(seqs[0], seq), key=lambda x: x.score, reverse=True
        )[0]
        alignments.append(alignment)

        # Extract aligned indices for current sequence
        aligned_indices = [
            idx for start, end in alignment.aligned[1] for idx in range(start, end)
        ]
        ...
        if not aligned_indexes_ref:
            aligned_indexes_ref = [
                idx for start, end in alignment.aligned[0] for idx in range(start, end)
            ]
...
def align_multiple_proteins_pdb(
    ...
    structures = [
        parser.get_structure(f"Protein{i + 1}", StringIO(pdb_str))
    ...
    for i, (structure, mappings) in enumerate(zip(structures, chain_residue_mappings)):
        ...
        if not mappings:
            coords, res_final = get_atom_coordinates(
                structure, None, None, all_atom=all_atom
            )
            if coords:
        ...
        for chain_id, residues in mappings:
            coords, res_final = get_atom_coordinates(
                structure, chain_id, residues, all_atom=all_atom
            )
            if coords:
    ...
    seqs = [
        "".join([aa3to1.get(residue.resname, "X") for residue in residues])
        for residues in residues_list
    ]

    sequence_alignment = force_sequence_alignment or any(
        len(seqs[i]) != len(seqs[0]) for i in range(1, len(seqs))
    )
    ...
    if aligned_seqs_indices and verbose:
        print(
            f"Found {len(aligned_seqs_indices[1])} overlapping residues between the first and second chains."
        )
    ...
    for i in range(1, len(structures)):
        ref_atoms = []
        mod_atoms = []
        for atom_id, (ref_idx, mod_idx) in enumerate(
            zip(aligned_seqs_indices[0], aligned_seqs_indices[i])
        ):
            ...
            for atom_name in shared_atom_names:
                ...
                ref_atoms.append(
                    PDB.Atom.Atom(
                        "X", ref_coords[atom_name], 1.0, 1.0, " ", "X", atom_id, atom
                    )
                )
                mod_atoms.append(
                    PDB.Atom.Atom(
                        "X", mod_coords[atom_name], 1.0, 1.0, " ", "X", atom_id, atom
                    )
                )
        super_imposer.set_atoms(ref_atoms, mod_atoms)
        super_imposer.apply(structures[i].get_atoms())

    aligned_structures_pdb = [
        get_aligned_structure_as_string(structure) for structure in structures
    ]
...
def pdb_to_mmcif_iter(
    pdb_data: str, structure_id: str, bfactor_to_plddt=False, fractional_plddt=False
):
    """
    ...
    if bfactor_to_plddt:
        ...
        for modelid, model_bfactors in enumerate(models_bfactors, start=1):
            mean_bfactor = np.mean(
                [
                    bfactor
                    for chain_bfactors in model_bfactors.values()
                    for resname, bfactor in chain_bfactors.values()
                ]
            )
            if fractional_plddt:
                mean_bfactor *= 100.0
...
def trim_pdb_str(
    pdb_input_string: str, target_chain: str, start_res: int, end_res: int
) -> str:
