# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\core\utils\export.py
...
def write_sheets(
    ...
    sheet_names = shorten_sheet_names(
        [sanitize_excel_sheet_name(n) for n in df_dict.keys()]
    )
...
def shorten_sheet_names(
    names_orig: list[str], max_length: int = 27, max_iter: int = 25
) -> list[str]:
    ...
    if len(set(names_final)) < len(names_final):
        ...
        raise ValueError(
            f"The colliding sheet names could not be resolved. Collisions: {collisions}"
        )
    return names_final
