...
def prepare_bindcraft_params(
    workflow: BindCraftBinderDesignWorkflow, workdir: str
) -> dict:
    input_dict = {
        "design_path": "output",
    ...
    return {
        ...
        "input_pdb": storage.prepare_workflow_input(
            workflow.bindcraft_params.input_pdb_path, workdir=workdir
        ),
...
def process_workflow_results(job: DesignJob, callback: Callable = None) -> list[Base]:
    ...
    for replica in range(1, num_replicas + 1):
        ...
        accepted_dir = os.path.join(
            source_output_path, f"{batch_dir}/bindcraft/Accepted"
        )
        accepted_filenames = [
            filename
            for filename in storage.list_dir(accepted_dir)
            if filename.endswith(".pdb")
        ]
        accepted_designs = [
            filename.split("_model")[0] for filename in accepted_filenames
        ]
        ...
        rejected_dir = os.path.join(
            source_output_path, f"{batch_dir}/bindcraft/Rejected"
        )
        rejected_filenames = [
            filename
            for filename in storage.list_dir(rejected_dir)
            if filename.endswith(".pdb")
        ]
