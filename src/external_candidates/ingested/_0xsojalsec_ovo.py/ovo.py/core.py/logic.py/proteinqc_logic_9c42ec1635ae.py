...
def tool_supports_scheduler(tool: ProteinQCTool, scheduler: Scheduler) -> bool:
    if (
        "profile" in scheduler.submission_args
        and "conda" in scheduler.submission_args["profile"]
    ):
...
def get_available_tools(
    tools: List[ProteinQCTool], scheduler: Scheduler
) -> List[ProteinQCTool]:
    """
...
def get_thresholds_colormap(
    thresholds: List[float], reverse_colors: bool = False, colors=None
):
    """
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        "threshold_colormap", list(zip(normalized_positions, colors))
    )
...
def get_plddt_color(thresholds: List[float] = [90, 70, 50], lighter: bool = False):
    ...
    assert (
        len(colors) == len(thresholds) + 1
    ), "Number of colors must be one more than number of thresholds"
...
def get_flag_color(
    value: float, descriptor: Descriptor
) -> Literal["green", "yellow", "orange", "missing", None]:
    if not isinstance(descriptor, NumericDescriptor):
        return None
