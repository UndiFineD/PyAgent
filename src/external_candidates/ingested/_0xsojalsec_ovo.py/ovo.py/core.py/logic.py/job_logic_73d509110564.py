# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\core\logic\job_logic.py
...
def update_job_status(job: JobMixin, save_on_success: bool = False) -> bool | None:
    ...
    try:
        ...
        if job.job_started_date_utc is None and (
            start_time := scheduler.get_job_start_time(job.job_id)
        ):
