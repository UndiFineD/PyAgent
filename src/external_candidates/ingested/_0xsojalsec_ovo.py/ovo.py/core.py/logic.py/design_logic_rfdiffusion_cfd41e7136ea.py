...
def submit_rfdiffusion_preview(
    ...
    **submission_args,
    ...
    if not contig:
        raise ValueError(
            "Contig has not been computed. Please check the workflow parameters."
        )

    input_path = storage.prepare_workflow_input(
        workflow.get_input_pdb_path(), workdir=local_scheduler.workdir
    )
...
def process_workflow_results(job: DesignJob, callback: Callable = None) -> list[Base]:
    ...
    assert workflow.is_instance(
        RFdiffusionWorkflow
    ), "This function expects a RFdiffusionWorkflow instance, got: {}".format(
    ...
    destination_dir = os.path.join(
        "project", project_round.project_id, "pools", pool.id, "designs"
    ).rstrip("/")
    ...
    if (
        workflow.refolding_params.primary_test
        and workflow.refolding_params.primary_test.startswith("af2_")
    ):
    ...
    if workflow.refolding_params.primary_test:
        ...
        if workflow.refolding_params.primary_test.startswith("af2_"):
            ...
            descriptor_key_prefix = (
                f"refolding|{workflow.refolding_params.primary_test}"
            )
        # tool_key -> filename
...
def process_rfdiffusion_design(
    ...
    contig_suffix = (
        "_" + str(contig_idx + 1).zfill(max(len(str(num_contigs)), 2))
        if num_contigs > 1
        else ""
    )
    # backbone suffix 01, 001, 0001 based on total number of designs
    backbone_suffix = "_" + str(total_idx_backbone + 1).zfill(
        max(len(str(num_backbone_designs)), 2)
    )
    ...
    for idx_sequence in range(num_seqs_total):
        ...
        mpnn_full_source_path = os.path.join(
            source_output_path, batch_name, mpnn_source_path + ".pdb"
        )
...
def prepare_rfdiffusion_workflow_params(
    workflow: RFdiffusionWorkflow, workdir: str
) -> dict:
    # prepare pdb file or txt file with multiple pdb paths
    workflow_input_path = storage.prepare_workflow_inputs(
        workflow.rfdiffusion_params.input_pdb_paths, workdir=workdir
    )
    ...
    if workflow.protein_mpnn_params.fastrelax_cycles:
        ...
        if not config.props.pyrosetta_license:
            raise ValueError(
                "FastRelax requires a PyRosetta license which is disabled in this instance of OVO."
            )
        ...
        params["mpnn_run_parameters"] = (
            ...
            + (
                f'-bias_AA "{workflow.protein_mpnn_params.bias_aa}"'
                if workflow.protein_mpnn_params.bias_aa
                else ""
            )
    ...
    if workflow.rfdiffusion_params.hotspots:
        hotspots = ",".join(
            workflow.rfdiffusion_params.hotspots.replace(",", " ").split()
        )
