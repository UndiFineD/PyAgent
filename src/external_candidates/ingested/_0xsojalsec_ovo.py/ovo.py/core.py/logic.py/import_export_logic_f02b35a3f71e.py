...
IMPORT_EXPORT_CONFIGS = [
    ...
    },
    {
        ...
        "filter": lambda o: dict(
            id__in=[p.design_job_id for p in o["pool"] if p.design_job_id is not None]
        ),
    },
    {
    ...
    },
    {
        ...
        "filter": lambda o: dict(
            descriptor_job_id__in=[j.id for j in o["descriptor_job"]]
        ),
    },
]
...
def find_storage_path_fields(cls, prefix: str = "") -> list[str]:
    ...
    for f in fields(cls):
        ...
        if is_dataclass(f.default) or (
            callable(f.default_factory) and is_dataclass(f.default_factory())
        ):
...
def export_import_project(
    ...
    projects = source_db.select(
        Project, **(dict(id=project_id) if project_id else dict())
    )
    ...
    if (
        check_conflicts
        and not count_only
        and dest_db.count(Project, id__in=[p.id for p in projects])
    ):
        existing_projects = dest_db.select(Project, id__in=[p.id for p in projects])
        names = ", ".join(
            f"{project.name} ({project.id})" for project in existing_projects
        )
    ...
    for entity_config in IMPORT_EXPORT_CONFIGS:
        ...
        print(
            f" Fetched {len(all_objects[table_name]):,} {model_class.__name__} objects"
        )
        ...
        for column in model_class.__table__.columns:
            ...
            if isinstance(column.type, DataclassType):
                ...
                for path_field in sorted(
                    set(
                        field
                        for cls in subclasses
                        for field in find_storage_path_fields(cls)
                    )
                ):
                    for obj in all_objects[table_name]:
...
def export_project(
    project_id: str, output_zip_path: str = None, accepted_only: bool = False
) -> str:
    """
...
def import_project(
    home_dir: str, project_id: str = None, count_only=False
) -> dict[str, int]:
    """
