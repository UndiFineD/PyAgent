...
def get_design_jobs_table(
    ...
    jobs = db.select(
        DesignJob, order_by="-created_date_utc", id__in=pools_by_job_id.keys()
    )
    ...
    accepted_by_pool = db.count_distinct(
        Design, group_by="pool_id", pool_id__in=[p.id for p in pools], accepted=True
    )
    total_by_pool = db.count_distinct(
        Design, group_by="pool_id", pool_id__in=[p.id for p in pools]
    )
        ...
        rows.append(
            {
                ...
                ("Job", "status"): format_pool_status(
                    job, pool.processed, update_status=update
                ),
                ("Job", "duration"): format_job_duration(job),
                ("Job", "created"): naturaltime(
                    job.created_date_utc, when=datetime.utcnow()
                ),
                ("Designs", "accepted"): (
                    accepted_by_pool.get(pool.id, 0) if pool.processed else None
                ),
                ("Designs", "total"): (
                    total_by_pool.get(pool.id, 0) if pool.processed else None
                ),
            }
        )
        ...
        [
            {
                ...
                "Accepted Designs": (
                    accepted_by_pool.get(pool.id, 0)
                    if pool.processed
...
def submit_design_workflow(
    ...
    if existing_pools := db.select(Pool, name=pool_name, round_id=round_id):
        if not return_existing:
            raise ValueError(
                f"Pool with name '{pool_name}' already exists in this round"
            )
        ...
        print(
            "Pool with same name and params already exists in this round, returning existing pool"
        )
...
def get_log(design_job: DesignJob, tail: int = None) -> str:
    ...
    assert isinstance(
        design_job, DesignJob
    ), f"Expected DesignJob, got {type(design_job).__name__}"
...
def process_results(
    design_job: DesignJob, callback: Callable = None, wait=True
) -> Pool:
...
def update_acceptance_thresholds(
    pools: list[Pool], acceptance_thresholds: dict[str, Threshold]
):
    ...
    for pool in pools:
        assert isinstance(
            pool, Pool
        ), f"Expected collection of Pool objects, got {pool} ({type(pool).__name__})"
        assert (
            pool.design_job_id is not None
        ), f"Pool must have a design job to update acceptance thresholds: {pool.id}"
    ...
    new_accepted_design_ids, num_accepted_by_descriptor = filter_designs_by_thresholds(
        ...
        values={
            descriptor_key: db.select_descriptor_values(
                descriptor_key, design_ids=all_design_ids
            ).to_dict()
    ...
    update_accepted_design_ids(
        pool_ids=[p.id for p in pools], accepted_design_ids=new_accepted_design_ids
    )
...
def set_designs_accepted(
    ...
    for descriptor_value in descriptor_values:
        ...
        values[descriptor_value.descriptor_key][descriptor_value.design_id] = (
            float(descriptor_value.value)
            if descriptor_value.value is not None
            else None
        )
...
def collect_storage_paths(
    download_fields: dict[str, tuple[Base, str]], design_ids: list
) -> list[str]:
    ...
    for label, (Model, field_name) in download_fields.items():
        if "." in field_name:
            ...
            assert (
                field_name.count(".") == 1
            ), f"Only single-level relationships supported, got: {field_name}"
        ...
        if issubclass(Model, Design):
            ...
            design_job_ids = [
                pool.design_job_id for pool in pools if pool.design_job_id
            ]
            field_values = [
                getattr(workflow, field_name)
                for workflow in db.select_values(
                    DesignJob, "workflow", id__in=design_job_ids
                )
                if isinstance(workflow, Model)
            ]
        elif issubclass(Model, DescriptorValue):
            field_values = db.select_unique_values(
                Model, "value", descriptor_key=field_name, design_id__in=design_ids
            )
        else:
            raise ValueError(
        ...
        for field_value in field_values:
            ...
            path = (
                ...
                else getattr(field_value, subfield_name)
            )
            if not path:
                continue
