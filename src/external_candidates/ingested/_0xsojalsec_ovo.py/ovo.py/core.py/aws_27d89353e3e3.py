...
class AWSSessionManager:
    ...
    def get_client(self, service, refresh_minutes_before_expiration=30):
        expired = (
            ...
            and self._AWS_CLIENTS_EXPIRATION[service].timestamp() - time()
            < 60 * refresh_minutes_before_expiration
        )
    ...
    def _create_session(self) -> tuple[boto3.Session, datetime | None]:
        ...
        if self.assume_role is not None:
            assumed_role_object = sts_client.assume_role(
                RoleArn=self.assume_role, RoleSessionName="OVO"
            )
    ...
    def get_latest_workflow_id(self, workflow_name: str):
        limit = 100
        items = self.omics.list_workflows(
            type="PRIVATE", name=workflow_name, maxResults=limit
        )["items"]
        if len(items) >= limit:
            # We could add pagination to solve this, at least make sure we raise an error for now
            raise ValueError(
                f"Too many workflows with name '{workflow_name}' found in AWS (>{limit})"
            )
        if not items:
            raise ValueError(
                f"HealthOmics workflow with name '{workflow_name}' was not found in AWS"
            )
        latest_workflow = sorted(items, key=lambda x: x["creationTime"], reverse=True)[
            0
        ]
