# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\core\configuration.py
import dataclasses
import json
import os
...
import yaml
from ovo.cli.common import OVOCliError, OVONotInitializedError, console
...
class OVOConfig(BaseConfigModel):
    ...
    def resolve_relative_paths(self):
        ...
        if self.db.url.startswith("sqlite:///"):
            self.db.url = "sqlite:///" + make_absolute_if_relative(
                self.db.url.removeprefix("sqlite:///")
            )
        ...
        for scheduler_name, scheduler_config in self.schedulers.items():
            scheduler_config.workdir = make_absolute_if_relative(
                scheduler_config.workdir
            )
    ...
    @classmethod
    def default(
        cls, props: ConfigProps, default_profile=None, admin_users: list[str] = None
    ) -> str:
        if default_profile is None:
            default_profile = "conda"
        props_rows = "\n".join(
            f"  {k}: {json.dumps(v)}" for k, v in props.__dict__.items()
        )
...
def save_global_home_dir(home_dir: str):
    ...
    if not os.access(home_dir_path, os.W_OK):
        raise OVOCliError(
            f"Cannot write to the package directory: {home_dir_path}. Insufficient permissions."
        )
    try:
        ...
        raise OVOCliError(
            f"Failed to write global config flag at {global_config_flag}: {e}"
        )
...
def load_config(home_dir: str) -> OVOConfig:
        ...
        if home_dir != DEFAULT_OVO_HOME:
            ...
            if not os.path.exists(home_dir):
                raise OVONotInitializedError(
                    f"OVO_HOME directory set but is not accessible: {home_dir}"
                )
            else:
                raise OVONotInitializedError(
                    f"OVO config file not found in OVO_HOME: {home_dir}"
                )
        else:
    ...
    if config.props.read_only:
        console.print(
            "[blue][bold]Running in read-only mode[/bold] (configured in config.yml props.read_only)[/blue]"
        )
