...
class RFdiffusionParams(WorkflowParams):
    ...
    input_pdb_paths: list[str] = field(
        default_factory=list, metadata=dict(show_to_user=False, storage_path=True)
    )
    # contig strings
    ...
    def validate(self):
        ...
        if self.hotspots:
            assert isinstance(
                self.hotspots, str
            ), f"Expected str for hotspots, got {type(self.hotspots).__name__}"
            if not all(
                re.fullmatch("[A-Z][0-9]+", hotspot)
                for hotspot in self.hotspots.split(",")
            ):
                raise ValueError(
                    f"Invalid hotspots format, expected 'A123,A124,A131', got: '{self.hotspots}'"
                )
        if self.inpaint_seq:
            assert isinstance(
                self.inpaint_seq, str
            ), f"Expected str for inpaint_seq, got {type(self.inpaint_seq).__name__}"
            if not all(
                re.fullmatch("[A-Z][0-9]+(-[0-9]+)?", segment)
                for segment in self.inpaint_seq.split("/")
            ):
                raise ValueError(
        ...
        if self.backbone_filters:
            ...
            for filter_expr in self.backbone_filters.split(","):
                match = re.fullmatch(
                    "([a-zA-Z0-9_]+)(<=|>=|=|<|>)([-]?[0-9]+(\\.[0-9]+)?)", filter_expr
                )
                if not match:
                    raise ValueError(
    ...
    def validate(self):
        ...
        if self.fastrelax_cycles:
            ...
            if "--" in self.run_parameters:
                raise ValueError(
                    "Please use arguments for dl_interface_design_extended.py (single dash)"
                )
        else:
            # help user use proper run params to avoid failing after RFdiffusion stage
            if (
                self.run_parameters
                and self.run_parameters[0] == "-"
                and self.run_parameters[1] != "-"
            ):
        ...
        if self.bias_aa:
            assert isinstance(
                self.bias_aa, str
            ), f"Expected str for bias_aa, got {type(self.bias_aa).__name__}"
            if not all(
                re.fullmatch("[A-Z]:-?[0-9]+(\\.[0-9]+)?", bias)
                for bias in self.bias_aa.split(",")
            ):
                raise ValueError(
                    f"Invalid bias_aa format, expected format 'F:1.5,K:-1.5', got: '{self.bias_aa}'"
                )
...
class RefoldingParams(WorkflowParams):
    ...
    def validate(self):
        ...
        if "," in self.primary_test:
            raise ValueError(
                f"Only a single refolding test can be selected, got: {self.primary_test}"
            )
...
class RFdiffusionWorkflow(DesignWorkflow, RefoldingSupportedDesignWorkflow):
    ...
    def get_input_pdb_path(self, contig_index=None):
        ...
        if has_multiple_inputs and has_multiple_contigs:
            assert len(self.rfdiffusion_params.input_pdb_paths) == len(
                self.rfdiffusion_params.contigs
            ), (
    ...
    def get_contig(self, contig_index=0):
        return (
            self.rfdiffusion_params.contigs[contig_index]
            if self.rfdiffusion_params.contigs
            else ""
        )
    ...
    def get_selected_segments(self, inpainting=False):
        if inpainting:
            return (
                self.rfdiffusion_params.inpaint_seq.split("/")
                if self.rfdiffusion_params.inpaint_seq
                else []
            )
        else:
    ...
    def set_selected_segments(self, selection: list[str], inpainting=False):
        if inpainting:
            self.rfdiffusion_params.inpaint_seq = (
                "/".join(selection) if selection else None
            )
        else:
            self.selected_segments = selection

    def get_time_estimate(self, scheduler: Scheduler) -> str:
        total_designs = self.rfdiffusion_params.num_designs * len(
            self.rfdiffusion_params.contigs
        )
        STARTUP_TIME_MINUTES = scheduler.get_startup_time_minutes() * 3
        ONE_DESIGN_MINUTES = (
            2  # assuming that one full design takes this many minutes to complete
        )
    ...
    def process_results(
        self, job: "DesignJob", callback: Callable = None
    ) -> list[Base]:
...
class RFdiffusionScaffoldDesignWorkflow(RFdiffusionWorkflow):
    ...
    acceptance_thresholds: dict[str, Threshold] = field(
        default_factory=lambda: {
            ...
            descriptors_refolding.AF2_PRIMARY_NATIVE_MOTIF_RMSD.key: Threshold(
                max_value=2.0
            ),
            descriptors_refolding.AF2_PRIMARY_PLDDT.key: Threshold(min_value=80),
            descriptors_refolding.ESMFOLD_PAE.key: Threshold(
                max_value=10.0, enabled=False
            ),
            descriptors_refolding.ESMFOLD_DESIGN_BACKBONE_RMSD.key: Threshold(
                max_value=5.0, enabled=False
            ),
            descriptors_refolding.ESMFOLD_PLDDT.key: Threshold(
                min_value=80, enabled=False
            ),
...
class RFdiffusionBinderDesignWorkflow(RFdiffusionWorkflow):
    ...
    acceptance_thresholds: dict[str, Threshold] = field(
        default_factory=lambda: {
            ...
            descriptors_refolding.AF2_PRIMARY_TARGET_ALIGNED_BINDER_RMSD.key: Threshold(
                max_value=2.0
            ),
            ...
            descriptors_rfdiffusion.N_CONTACTS_TO_HOTSPOTS.key: Threshold(
                min_value=1, enabled=False
            ),
            descriptors_refolding.AF2_PRIMARY_BINDER_PAE.key: Threshold(
                max_value=5.0, enabled=False
            ),
    ...
    def get_selected_segments(self, contig_index=0, partial_diffusion=False):
        if partial_diffusion:
            ...
            binder_length, designed_segments = parse_partial_diffusion_binder_contig(
                binder_contig
            )
    ...
    def set_selected_segments(
        self, segments: list[str], contig_index=0, partial_diffusion=False
    ):
        if partial_diffusion:
            ...
            binder_length, _ = parse_partial_diffusion_binder_contig(
                self.get_binder_contig(contig_index=contig_index)
            )
            self.set_binder_contig(
                create_partial_diffusion_binder_contig(
                    redesigned_segments=segments, binder_length=binder_length
                )
            )
        else:
            # Hotspots
    ...
    def get_target_contig(self, contig_index=0):
        ...
        assert (
            len(subcontigs) == 2
        ), f"Expected a binder chain and target chain in contig, got: {contig}"
        assert all(
            segment[0].isalpha() for segment in subcontigs[1].split("/")
