...
class JobMixin(MappedAsDataclass):
    ...
    job_started_date_utc: Mapped[datetime] = mapped_column(
        DateTime, default=None, nullable=True
    )
    job_finished_date_utc: Mapped[datetime] = mapped_column(
        DateTime, default=None, nullable=True
    )
    # None = in progress, True = success, False = failed
    job_result: Mapped[bool | None] = mapped_column(
...
class Project(Base, MetadataMixin):
    ...
    __table_args__ = (
        UniqueConstraint("name", "author", name="uq_project_name_author"),
    )
    ...
    def to_dict(self, human_readable=False):
        ...
        if human_readable:
            data = {
                ...
                if self.__dataclass_fields__[k].metadata.get("show_to_user", True)
            }
        return data
...
class Threshold:
    ...
    def __eq__(self, other):
        ...
        if (
            not hasattr(other, "min_value")
            or not hasattr(other, "max_value")
            or not hasattr(other, "enabled")
        ):
            return False
        return (
    ...
    def get_bounds(
        ...
        delta = (
            (descriptor_values.max() - descriptor_values.min()) * 1.5
            if not descriptor_values.empty
            else 1
        )
...
class Workflow:
    ...
    def is_subclass(cls, other_cls):
        ...
        this_class_and_superclass_names = [
            f"{c.__module__}.{c.__name__}" for c in cls.__mro__
        ]
    ...
    def get_param_fields(self, human_readable=False) -> dict[str, WorkflowParams]:
        ...
        for field_name, field_def in self.__dataclass_fields__.items():
            ...
            if tool_name := field_def.metadata.get("tool_name"):
                ...
                ), "Params should be instance of WorkflowParams"
                assert (
    ...
    def validate(self):
        for field_name, params in self.get_param_fields().items():
            try:
                ...
                raise ValueError(
                    f"Validation failed for workflow.{field_name} ({type(params).__name__}: {e}"
                ) from e
    ...
    @abstractmethod
    def process_results(
        self, job: Union["DesignJob", "DescriptorJob"], callback: Callable = None
    ) -> list[Base]:
    ...
    def validate(self):
        ...
        for key, threshold in self.acceptance_thresholds.items():
            ...
            if not isinstance(key, str):
                raise ValueError(
                    f"Acceptance threshold key should be a string, got {key} ({type(key).__name__})"
                )
            if key not in ALL_DESCRIPTORS_BY_KEY:
                raise ValueError(
                    f"Unknown descriptor key '{key}' in acceptance thresholds"
                )
...
class DesignJob(Base, MetadataMixin, JobMixin):
    ...
    workflow: Mapped[DesignWorkflow] = mapped_column(
        DataclassType(Workflow), default=None, nullable=False
    )
...
class Pool(Base, MetadataMixin):
    ...
    design_job_id: Mapped[str] = mapped_column(
...
class Design(Base):
    ...
    structure_path: Mapped[str] = mapped_column(
        String, default=None, nullable=True, info=dict(storage_path=True)
    )
    structure_descriptor_key: Mapped[str] = mapped_column(
        String, default=None, nullable=True
    )
    accepted: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    spec: Mapped[DesignSpec] = mapped_column(
        DataclassType(DesignSpec), default=None, nullable=False
    )
    ...
    def __post_init__(self):
        ...
        assert (
            "_" not in self.pool_id
        ), f"pool_id cannot contain underscores, got: {self.pool_id}"
        assert self.id, "id must be set for Design"
        if not self.id.startswith(f"ovo_{self.pool_id}_"):
            raise ValueError(
                f"Design ID '{self.id}' from pool '{self.pool_id}' must start with 'ovo_{self.pool_id}_'"
            )
    ...
    def from_pdb_file(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ValueError(
                f"File '{filename}' does not contain requested chain '{e}'"
            ) from e
        ...
        return cls(
            ...
            structure_path=storage.store_file_str(
                pdb_str,
                os.path.join(
                    "project", project_id, "pools", pool_id, "designs", f"{id}.pdb"
                ),
            ),
            **kwargs,
        )
...
class DescriptorJob(Base, MetadataMixin, JobMixin):
    ...
    workflow: Mapped[DescriptorWorkflow] = mapped_column(
        DataclassType(Workflow), default=None, nullable=False
    )
        ...
        for length in range(6, 10):
            for i in range(tries):
                ...
                try:
                    ...
                    return new_id
        raise ValueError(
            f"Failed to generate unique id of length {length} after {tries} tries"
        )
...
class NumericDescriptor(Descriptor, ABC):
    ...
    def get_plot_range(
        self, values: pd.Series, padding=0.05
...
class StructureFileDescriptor(FileDescriptor):
    ...
    structure_type: Literal[
        "backbone_design", "sequence_design", "prediction", "experimentally_resolved"
    ] = None
