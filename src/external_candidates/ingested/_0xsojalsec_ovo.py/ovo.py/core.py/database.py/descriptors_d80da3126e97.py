...
for descriptor_path in [
    ...
    assert isinstance(
        _descriptors, list
    ), "Expected DESCRIPTORS to be a list, got {} in {}".format(
        type(_descriptors), descriptor_path
    )
    for descriptor in _descriptors:
        if descriptor.key in ALL_DESCRIPTORS:
            raise ValueError(
                f"Duplicate descriptor key '{descriptor.key}' found in {descriptor_path}"
            )
    ...
    try:
        ...
        assert isinstance(
            _presets, list
        ), "Expected PRESETS to be list, got {} in {}".format(
            type(_presets), descriptor_path
        )
        for preset in _presets:
            assert (
                "label" in preset and "x" in preset and "y" in preset
            ), "Expected preset to have 'label', 'x', and 'y' fields, got {}".format(
                preset
            )
            label = preset["label"]
...
for key in ALL_DESCRIPTOR_KEYS:
    if not key:
        raise ValueError(
            f"Some descriptors are missing a key: {[d for d in ALL_DESCRIPTORS if not d.key]}"
        )
    if key.count("|") != 2:
        raise ValueError(
            f"Descriptor key should contain two | separators (pipeline|tool|descriptor), found: {key}"
        )

STRUCTURE_PATH_DESCRIPTORS = [
    d for d in ALL_DESCRIPTORS if isinstance(d, StructureFileDescriptor)
]
SEQUENCE_DESIGN_PATH_DESCRIPTORS = [
    d for d in STRUCTURE_PATH_DESCRIPTORS if d.structure_type == "sequence_design"
]
