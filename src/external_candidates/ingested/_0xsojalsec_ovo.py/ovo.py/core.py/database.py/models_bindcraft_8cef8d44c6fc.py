# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\core\database\models_bindcraft.py
import os
...
class BindCraftParams(WorkflowParams):
    ...
    input_pdb_path: str = field(
        default=None, metadata=dict(show_to_user=False, storage_path=True)
    )
    ...
    def validate(self):
        ...
        if not self.binder_length.replace(",", "").isdigit():
            raise ValueError(
                f"Binder length should be 'length' or 'min,max', got: '{self.binder_length}'"
            )
        if self.hotspots:
            ...
            for hotspot in self.hotspots.split(","):
                ...
                if not any(
                    hotspot.startswith(chain) for chain in self.target_chains.split(",")
                ):
                    raise ValueError(
                        f"Hotspot {hotspot} does not match selected target chains {self.target_chains}"
                    )
...
class BindCraftBinderDesignWorkflow(DesignWorkflow):
    ...
    def visualize_single_design_structures(cls, design_id: str):
        ...
        from ovo.app.components.workflow_visualization_components import (
            bindcraft_binder_design_visualization,
        )
    ...
    def get_download_fields(cls):
        return {
            "BindCraft Input PDB": (
                ...
                "bindcraft_params.input_pdb_path",
            ),
            "BindCraft design": (Design, "structure_path"),
        }

    def get_settings_paths(self):
        ovo_resources_path = os.path.abspath(
        ...
        if self.bindcraft_params.design_protocol == "Default":
            ...
            raise ValueError(
                f"Unsupported design protocol {self.bindcraft_params.design_protocol}"
            )
        ...
        if self.bindcraft_params.interface_protocol == "AlphaFold2":
            ...
            raise ValueError(
                f"Unsupported interface protocol {self.bindcraft_params.interface_protocol}"
            )
        ...
        if self.bindcraft_params.template_protocol == "Default":
            ...
            raise ValueError(
                f"Unsupported template protocol {self.bindcraft_params.template_protocol}"
            )
        ...
        if self.bindcraft_params.design_protocol == "Peptide":
            ...
            if self.bindcraft_params.prediction_protocol == "Default":
                ...
                raise ValueError(
                    f"Unsupported prediction protocol {self.bindcraft_params.prediction_protocol}"
                )
        ...
        advanced_settings_path = os.path.join(
            bindcraft_resources_path, "settings_advanced", advanced_settings_filename
        )
        ...
        if self.bindcraft_params.filter_type == "Default":
            ...
            raise ValueError(
                f"Unsupported filter type: {self.bindcraft_params.filter_type}"
            )

        filter_settings_path = os.path.join(
            bindcraft_resources_path, "settings_filters", filter_filename
        )
