...
class DBProxy(DCTransformDeclarative):
    ...
    def count_distinct(
        cls, field="id", group_by=None, **kwargs
    ) -> int | dict[Any, int]:
    ...
    def select_dict(
        cls, key_column: str, value_column: str, order_by=None, **kwargs
    ) -> dict:
        return cls._db().select_dict(
            cls, key_column, value_column, order_by=order_by, **kwargs
        )
    ...
    def select_dataframe(
        cls, index_col: str = "id", order_by=None, limit=None, **kwargs
    ) -> pd.DataFrame:
        return cls._db().select_dataframe(
            cls, index_col=index_col, order_by=order_by, limit=limit, **kwargs
        )
