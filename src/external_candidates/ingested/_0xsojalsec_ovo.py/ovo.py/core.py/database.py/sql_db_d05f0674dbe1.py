...
class SqlDBEngine(CacheClearingEngine):
    ...
    def automigrate(self):
        with self._create_session() as session:
            ...
            descriptor_job_columns = [
                c["name"] for c in inspector.get_columns("descriptor_job")
            ]
            if (
                "project_id" not in descriptor_job_columns
                and "round_id" in descriptor_job_columns
            ):
                print(
                ...
                session.execute(
                    text("ALTER TABLE descriptor_job ADD COLUMN project_id VARCHAR")
                )
                # create index on project_id
                session.execute(
                    text(
                        "CREATE INDEX ix_descriptor_job_project_id ON descriptor_job (project_id)"
                    )
                )
    ...
    def check_read_only(self):
        if self._read_only:
            raise RuntimeError(
                "Ovo is running in read-only mode, write operations are not allowed."
            )
    ...
    def remove(self, model: Type[T], *id_args, **kwargs):
        ...
        assert (
            id_args or kwargs
        ), "At least one filter must be provided for remove operation"
        with self._create_session() as session:
    ...
    def save_value(self, model: Type[T], column: str, value, **kwargs):
        ...
        with self._create_session() as session:
            session.query(model).filter(*self._create_filters(model, kwargs)).update(
                {column: value}
            )
            session.commit()

    def select(
        self, model: Type[T], limit: int = None, order_by=None, **kwargs
    ) -> Sequence[T]:
        with self._create_session() as session:
            return self._create_query(
                session, model=model, order_by=order_by, limit=limit, **kwargs
            ).all()
    ...
    def count_distinct(
        self, model: Type[T], field="id", group_by=None, **kwargs
    ) -> int | dict[Any, int]:
        with self._create_session() as session:
            if group_by:
                ...
                query = (
                    session.query(
                        *(
                            group_by_fields
                            + [func.count(distinct(getattr(model, field)))]
                        )
                    )
                ...
                return {
                    (row[0] if len(row) == 2 else tuple(row[:-1])): row[-1]
                    for row in query.all()
                }
            else:
                return (
                    self._create_query(session, model=model, columns=[field], **kwargs)
                    .distinct()
                    .count()
                )
    ...
    def _create_query(
        ...
        **kwargs,
        ...
        if columns:
            ...
            hasattr(model, "__mapper__") and model.__mapper__.polymorphic_on is not None
        ):
    ...
    def _create_order_by_single(self, model: Type[T], order_by):
        return (
            ...
            else getattr(model, order_by[1:]).desc()
        )
    ...
    def get(self, model: Type[T], *id_args, **kwargs) -> T:
        with self._create_session() as session:
            return self._create_query(
                session, model=model, id_args=id_args, **kwargs
            ).one()

    def get_value(self, model: Type[T], column: str, *id_args, raw=False, **kwargs):
        with self._create_session() as session:
            query = self._create_query(
                session, model=model, columns=[column], id_args=id_args, **kwargs
            )
            if raw:
    ...
    def select_values(
        self, model: Type[T], column: str, order_by=None, **kwargs
    ) -> list:
        with self._create_session() as session:
            return [
                row[0]
                for row in self._create_query(
                    session, model=model, columns=[column], order_by=order_by, **kwargs
                ).all()
            ]
    ...
    def select_dataframe(
        ...
        **kwargs,
        ...
        with self._create_session() as session:
            query = self._create_query(
                session, model=model, limit=limit, order_by=order_by, **kwargs
            )
    ...
    def select_descriptor_values(
        self, descriptor_key: str, design_ids: list[str]
    ) -> pd.Series:
                ...
                query = (
                    ...
                    .filter(
                        *self._create_filters(
                            DescriptorValue, dict(design_id__in=batch)
                        )
                    )
                )
    ...
    def select_design_descriptors(
        self, design_id: str, descriptor_keys: list[str]
    ) -> pd.Series:
        ...
        with self._create_session() as session:
            for key in descriptor_keys:
                assert isinstance(
                    key, str
                ), f"Expected collection of descriptor key strings, got {type(key).__name__}"
            query = (
                session.query(DescriptorValue.descriptor_key, DescriptorValue.value)
                .filter(DescriptorValue.design_id == design_id)
                .filter(
                    *self._create_filters(
                        DescriptorValue, {"descriptor_key__in": descriptor_keys}
                    )
                )
            )
            result = query.all()
            series = pd.Series(
                {descriptor_key: value for descriptor_key, value in result}
            )
            try:
                # Try converting whole array to numbers.
                # If any value is not a number, keep it as array of strings.
    ...
    def select_wide_descriptor_table(
        self, design_ids: list[str], descriptor_keys: list[str], **kwargs
    ) -> pd.DataFrame:
        # TODO this does not handle the case when a descriptor was computed multiple times for the same design
        ...
        with self._create_session() as session:
            for key in descriptor_keys:
                assert isinstance(
                    key, str
                ), f"Expected collection of descriptor keys strings, got {type(key).__name__}"
            design_ids = list(design_ids)
            batches = (
                [design_ids]
                if not self._in_clause_items_limit
    ...
    def get_design_accepted_values(self, design_ids: list[str]):
        with self._create_session() as session:
            query = self._create_query(
                session, model=Design, columns=["id", "accepted"], id__in=design_ids
            )
