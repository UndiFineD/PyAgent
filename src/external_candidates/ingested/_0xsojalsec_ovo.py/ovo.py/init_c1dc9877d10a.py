# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ovo\ovo\__init__.py
import importlib.metadata
import os
import sys

from ovo.cli.common import OVONotInitializedError, console
...
try:
    ...
    config = load_config(
        home_dir=GLOBAL_OVO_HOME or os.getenv("OVO_HOME") or DEFAULT_OVO_HOME
    )

    db = SqlDBEngine(
        db_url=config.db.url,
        verbose=config.db.verbose,
        read_only=config.props.read_only,
    )
    db.init()
    ...
    for scheduler_key, scheduler_config in config.schedulers.items():
        ...
        if scheduler_config.aws:
            init_args["aws"] = AWSSessionManager(
                assume_role=scheduler_config.aws.assume_role_arn,
                region_name=scheduler_config.aws.region,
            )
    ...
    if not config.default_scheduler or config.default_scheduler not in schedulers:
        raise ValueError(
            f"default_scheduler '{config.default_scheduler}' not found, please check your config file."
        )

    if not config.local_scheduler or config.local_scheduler not in schedulers:
        raise ValueError(
            f"local_scheduler '{config.local_scheduler}' not found, please check your config file."
        )
    ...
    if (
        ("init" in sys.argv and "home" in sys.argv)
        or "-h" in sys.argv
        or "--help" in sys.argv
    ):
        ...
        console.print(
            Panel.fit(message, title="⚠️  OVO not initialized", border_style="red")
        )
        sys.exit(2)
