# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-nbutools\network-analysis\pynet\NBUEndpoints.py
import socket
import ssl
import struct
...
class NBUService(object):
    ...
    def start_tls(self, b):
        ...
        self.sock = self.ssl_context.wrap_socket(
            self.sock, server_side=self.TLS_SERVER_SIDE
        )
        with self.cond:
            self.cond.notify()
...
class PBX(TCP):
    ...
    def set_cli_arguments(cls, parser):
        ...
        tls.add_argument(
            ...
            "--certificate",
            metavar="PATH",
            help="Certificate file for client authentication",
        )
        tls.add_argument(
            "-k", "--key", metavar="PATH", help="Key file for client authentication"
        )
...
class PBXListen(TCP_LISTEN):
    ...
    def set_cli_arguments(cls, parser):
        ...
        tls = parser.add_argument_group("TLS", "TLS specific options")
        tls.add_argument(
            "-c", "--certificate", metavar="PATH", help="Certificate server"
        )
