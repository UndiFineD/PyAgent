# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_dynamic_weight_shift.py
"""
...
async def run_scenario(scenario, data_dir, embedding_service):
    ...
    if adapter:
        ...
        for doc_id, meta in zip(all_docs["ids"], all_docs["metadatas"]):
            print(
                f"      - {doc_id[:30]}: score={meta.get('score', 'N/A')}, uses={meta.get('uses', 'N/A')}"
            )
    ...
    if history_adapter:
        try:
            ...
            if history_docs["ids"]:
                print(
                    f"    [DEBUG] History collection has {len(history_docs['ids'])} docs (PROMOTED!)"
                )
    ...
    results = await system.search(
        scenario["query"], collections=["working", "history"], limit=5
    )
    ...
    if proven_outcome == "failed":
        ...
        success = (new_rank is not None) and (
            proven_rank is None or new_rank < proven_rank
        )
    ...
    return {
        ...
        "results": [
            (r.get("text", "")[:50], r.get("metadata", {}).get("score"))
            for r in results[:3]
        ],
    }
...
async def main():
    ...
    for i, scenario in enumerate(TEST_SCENARIOS):
        ...
        print(
            f"    Proven outcome: {scenario['proven_memory']['outcome']} x{scenario['proven_memory'].get('outcome_count', 0)}"
        )
        ...
        print(f"    Results:")
        proven_score_str = (
            f"{result['proven_score']:.2f}"
            if result["proven_score"] is not None
            else "N/A"
        )
        new_score_str = (
            f"{result['new_score']:.2f}" if result["new_score"] is not None else "N/A"
        )
        print(
            f"      Proven memory rank: {result['proven_rank']} (score: {proven_score_str})"
        )
    ...
    worked_scenarios = [
        r
        for r in results
        if "worked" in str(TEST_SCENARIOS[results.index(r)]["proven_memory"]["outcome"])
    ]
    failed_scenarios = [
        r
        for r in results
        if "failed" in str(TEST_SCENARIOS[results.index(r)]["proven_memory"]["outcome"])
    ]
    ...
    for r in results:
        ...
        print(
            f"  [{status}] {r['scenario'][:40]}: proven={r['proven_rank']}, new={r['new_rank']}"
        )
