...
class TortureTestHarness:
    ...
    async def run_test(self, name: str, test_func) -> bool:
        ...
        try:
            ...
            self.results.append(
                {"name": name, "status": "ERROR", "time": elapsed, "error": str(e)}
            )
            return False
...
async def test_2_long_term_evolution(harness: TortureTestHarness) -> bool:
    ...
    early_avg = (
        sum(routing_stats["early"]) / len(routing_stats["early"])
        if routing_stats["early"]
        else 5
    )
    late_avg = (
        sum(routing_stats["late"]) / len(routing_stats["late"])
        if routing_stats["late"]
        else 5
    )
    ...
    if late_avg < early_avg:
        harness.log(
            f"SUCCESS: Routing improved ({early_avg:.1f} -> {late_avg:.1f} collections)"
        )
        return True
    else:
...
async def test_3_adversarial_deduplication(harness: TortureTestHarness) -> bool:
    ...
    results = await system.search(
        "Python backend", limit=5, collections=["memory_bank"]
    )
    if results:
        ...
        harness.log(
            f"  Top result quality: {quality:.2f} (importance={importance:.2f}, confidence={confidence:.2f})"
        )
...
async def test_4_score_boundary_stress(harness: TortureTestHarness) -> bool:
    ...
    doc_id = await system.store(
        text="Test memory for score oscillation", collection="working", metadata={}
    )
...
async def test_6_routing_convergence(harness: TortureTestHarness) -> bool:
    ...
    if late < early:
        harness.log(
            f"SUCCESS: Routing converged from {early:.1f} to {late:.1f} collections"
        )
        return True
    else:
...
async def test_7_promotion_cascade(harness: TortureTestHarness) -> bool:
    ...
    harness.log(
        f"  Final: collection={final_collection}, score={final_score:.2f}, uses={final_uses}"
    )
...
async def test_9_knowledge_graph_integrity(harness: TortureTestHarness) -> bool:
    ...
    for i in range(20):
        doc_id = await system.store(
            f"Memory {i}: Information about Python programming", "working", {}
        )
        doc_ids.append(doc_id)
...
async def test_10_concurrent_access(harness: TortureTestHarness) -> bool:
    ...
    if len(unique_ids) == len(all_doc_ids):
        ...
        harness.log(
            f"FAIL: Doc ID collision detected ({len(unique_ids)} unique)", "FAIL"
        )
        return False
...
async def run_torture_suite():
    ...
    await harness.run_test(
        "1. High Volume Stress", lambda: test_1_high_volume_stress(harness)
    )
    await harness.run_test(
        "2. Long-Term Evolution", lambda: test_2_long_term_evolution(harness)
    )
    await harness.run_test(
    ...
    await harness.run_test(
        "4. Score Boundary Stress", lambda: test_4_score_boundary_stress(harness)
    )
    await harness.run_test(
    ...
    await harness.run_test(
        "6. Routing Convergence", lambda: test_6_routing_convergence(harness)
    )
    await harness.run_test(
        "7. Promotion Cascade", lambda: test_7_promotion_cascade(harness)
    )
    await harness.run_test(
        "8. Memory Bank Capacity", lambda: test_8_memory_bank_capacity(harness)
    )
    await harness.run_test(
    ...
    await harness.run_test(
        "10. Concurrent Access", lambda: test_10_concurrent_access(harness)
    )
