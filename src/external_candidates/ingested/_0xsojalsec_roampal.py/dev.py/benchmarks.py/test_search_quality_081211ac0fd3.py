# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_search_quality.py
"""
    ...
    try:
        ...
        for fact, _ in facts:
            await memory.store_memory_bank(
                text=fact, tags=["synonym_test"], importance=0.8, confidence=0.8
            )
...
async def test_typo_tolerance():
    ...
    try:
        ...
        for typo_query, correct_word, description in typo_queries:
            results = await memory.search(
                query=typo_query, collections=["memory_bank"], limit=3
            )
...
async def test_acronym_expansion():
    ...
    try:
        ...
        acronym_tests = [
            (
                "Where does the user live in NYC?",
                "new york city",
                "NYC -> New York City",
            ),
            (
                "Does user work at NASA?",
                "aeronautics",
            ...
            (
                "Did user attend University of California Los Angeles?",
                "ucla",
                "Full name -> UCLA",
            ),
        ]
        ...
        for query, expected_word, description in acronym_tests:
            ...
            found = any(
                expected_word.lower() in r.get("text", "").lower() for r in results
            )
            if found:
                successes += 1
                print(f"  [OK] {description}")
...
async def test_result_diversity():
    ...
    try:
        ...
        for fact in pet_facts:
            await memory.store_memory_bank(
                text=fact, tags=["pet", "max"], importance=0.7, confidence=0.7
            )
            await asyncio.sleep(0.02)
        ...
        # Query for pet info
        results = await memory.search(
            query="Tell me about the user's pet", collections=["memory_bank"], limit=5
        )
...
async def test_recency_vs_relevance():
    ...
    try:
        ...
        print(
            f"  Highly-relevant old fact ranked #1: {'YES' if python_first else 'NO'}"
        )
