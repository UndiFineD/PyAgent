# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_semantic_confusion.py
"""
...
async def test_semantic_confusion():
    ...
    memory = UnifiedMemorySystem(
        data_dir="test_data_semantic_confusion", use_server=False, llm_service=None
    )
    ...
    for test in test_cases:
        ...
        results = await memory.search(
            query=test["query"], collections=["memory_bank"], limit=5
        )
        ...
        print(
            f"         Results: {truth_found}/3 truth in top 5 ({noise_in_top5} noise)"
        )
        ...
        for i, r in enumerate(results[:5]):
            ...
            if marker == "noise" and quality > 0.5:
                print(
                    f"           {i+1}. [{marker}] q={quality:.2f} rank={final_score:.3f} | FULL: {text}"
                )
                print(f"               ID: {doc_id}")
            else:
                print(
                    f"           {i+1}. [{marker}] q={quality:.2f} rank={final_score:.3f} | {text[:70]}..."
                )
    ...
    print(
        f"Truth ranked #1: {truth_at_rank1}/{len(test_cases)} queries ({rank1_rate:.1f}%)"
    )
    print(f"Noise ratio survived: {noise_ratio:.0f}:1")
    print(f"\n[MECHANISM] 3-stage quality enforcement:")
    print(
        f"  1. Distance boost: adjusted_distance = L2_distance * (1.0 - quality * 0.8)"
    )
    ...
    if Path("test_data_semantic_confusion").exists():
        try:
            shutil.rmtree("test_data_semantic_confusion")
        except:
            pass
