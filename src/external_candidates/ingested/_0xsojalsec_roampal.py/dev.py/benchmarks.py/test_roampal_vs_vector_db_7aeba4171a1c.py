...
async def run_control_scenario(
    ...
    collection = client.create_collection(
        name="control_test", metadata={"hnsw:space": "l2"}
    )
...
async def run_treatment_scenario(
    scenario: Dict, data_dir: str, embedding_service
) -> Dict:
    """
...
async def main():
    ...
    for scenario in SCENARIOS:
        ...
        treatment = await run_treatment_scenario(
        ...
        if treatment["good_ranked_first"] and not control["good_ranked_first"]:
            ...
            print(
                f"       Control: {c_status} | Treatment: {t_status} | Vector DB wins"
            )
    ...
    print(f"\n1. SUCCESS RATES:")
    print(
        f"   Control (Plain Vector DB):  {control_success:.0f}/30 ({control_rate*100:.1f}%)"
    )
    print(
        f"   Treatment (Roampal):         {treatment_success:.0f}/30 ({treatment_rate*100:.1f}%)"
    )
    print(
        f"   Improvement:                 +{(treatment_rate - control_rate)*100:.1f} percentage points"
    )
    ...
    for cat in categories:
        cat_control = [
            ...
            if SCENARIOS[r["scenario_id"] - 1]["category"] == cat
        ]
        cat_treatment = [
            r["success"]
        ...
        print(
            f"   {cat:10s}: Control {c_rate:5.1f}% | Roampal {t_rate:5.1f}% | Delta +{t_rate-c_rate:.1f}%"
        )
    ...
    if p_value < 0.05 and treatment_rate > control_rate:
        ...
        print(f"Evidence:")
        print(
            f"  - Treatment success rate {treatment_rate*100:.1f}% vs control {control_rate*100:.1f}%"
        )
        print(f"  - Statistically significant (p = {p_value})")
        print(
            f"  - Effect size: d = {d:.2f} ({['negligible','small','medium','large'][min(3,int(abs(d)/0.3))]})"
        )
        print()
