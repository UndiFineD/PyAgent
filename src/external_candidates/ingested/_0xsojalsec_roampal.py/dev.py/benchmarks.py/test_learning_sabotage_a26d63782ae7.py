# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_learning_sabotage.py
"""
...
from modules.memory.unified_memory_system import ActionOutcome, UnifiedMemorySystem
...
async def create_fresh_memory():
    ...
    memory = UnifiedMemorySystem(data_dir=test_dir, use_server=False, llm_service=None)
...
async def test_outcome_flooding():
    ...
    try:
        ...
        if doc is None:
            ...
            print(
                f"  [AFTER] Document was removed/promoted during flood (expected behavior)"
            )
...
async def test_kg_routing_poisoning():
    ...
    try:
        ...
        history_not_first = (
            routed_collections[0] != "history" if routed_collections else True
        )
...
async def test_deduplication_replacement_attack():
    ...
    try:
        ...
        original_text = (
            "User's password manager is 1Password (verified, critical security info)"
        )
        ...
        original_survives = (
            "verified" in top_text.lower() or "critical" in top_text.lower()
        )

        print(f"\n  [RESULT] Top result: {top_text[:60]}...")
        print(
            f"  [CHECK] Original high-quality version ranked #1: {'YES' if original_survives else 'NO'}"
        )
...
async def test_rapid_outcome_rate_limiting():
    ...
    try:
        ...
        rapid_score = (
            rapid_result["metadata"].get("score", 0.5) if rapid_result else 1.0
        )
        ...
        spaced_score = (
            spaced_result["metadata"].get("score", 0.5) if spaced_result else 1.0
        )
...
async def test_memory_bank_score_immutability():
    ...
    try:
        ...
        print(
            f"  [BEFORE] Quality: {initial_quality:.2f} (imp={initial_importance}, conf={initial_confidence})"
        )

        # Try to manipulate via outcome flooding
        for _ in range(20):
            await memory.record_outcome(
                doc_id=doc_id, outcome="failed"  # Try to degrade quality
            )
        ...
        print(
            f"  [AFTER] Quality: {final_quality:.2f} (imp={final_importance}, conf={final_confidence})"
        )
...
async def test_cross_collection_contamination():
    ...
    try:
        ...
        history_doc = await memory.store(
            text="History document", collection="history", metadata={"score": 0.5}
        )
        ...
        working_score = (
            working_result["metadata"].get("score", 0.5) if working_result else 1.0
        )
        history_score = (
            history_result["metadata"].get("score", 0.5) if history_result else 0.5
        )
...
async def test_invalid_outcome_handling():
    ...
    try:
        ...
        try:
            await memory.record_outcome(
                doc_id="nonexistent_doc_12345", outcome="worked"
            )
            # Should not crash
            tests_passed += 1
        ...
        # Test 2: Empty doc_id
        try:
            await memory.record_outcome(doc_id="", outcome="worked")
            tests_passed += 1
        ...
        doc_id = await memory.store(
            text="Test document", collection="working", metadata={"score": 0.5}
        )
        try:
        ...
        books_doc = await memory.store(
            text="Book content", collection="books", metadata={"title": "Test Book"}
        )
        try:
