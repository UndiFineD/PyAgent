# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_context_poisoning.py
"""
...
async def test_exact_duplicate_poisoning():
    ...
    try:
        ...
        await memory.store_memory_bank(
            text=base_text, tags=["personal"], importance=0.6, confidence=0.6
        )
        ...
        results = await memory.search(
            query="When is the user's birthday?", collections=["memory_bank"], limit=5
        )

        # Count how many versions of the birthday fact appear
        birthday_results = [
            r for r in results if "march 15" in r.get("text", "").lower()
        ]
...
async def test_near_duplicate_confusion():
    ...
    try:
        ...
        variants = [
            ...
            (
                "User said their favorite color is green (verified in 3 conversations).",
                0.95,
                0.95,
            ),
        ...
        for text, imp, conf in variants:
            await memory.store_memory_bank(
                text=text, tags=["preference"], importance=imp, confidence=conf
            )
...
async def test_entity_confusion():
    ...
    try:
        ...
        for fact in confusing_facts:
            await memory.store_memory_bank(
                text=fact, tags=["random", "unverified"], importance=0.3, confidence=0.3
            )
            await asyncio.sleep(0.02)
...
async def test_temporal_confusion():
    ...
    try:
        ...
        results = await memory.search(
            query="When did the user get married?", collections=["memory_bank"], limit=5
        )
...
async def test_negation_poisoning():
    ...
    try:
        ...
        truth_first = (
            "is allergic" in top_text.lower() and "not" not in top_text.lower()
        ) or "epipen" in top_text.lower()
