# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_token_efficiency.py
"""
================================================================================
...
async def run_control_topk(
    scenario: Dict, data_dir: str, embedding_service, k: int = 3
) -> Dict:
    """
    ...
    collection = client.create_collection(
        name=f"control_top{k}", metadata={"hnsw:space": "l2"}
    )
...
async def run_treatment_roampal(
    scenario: Dict, data_dir: str, embedding_service
) -> Dict:
    """
    ...
    for i, text in enumerate(
        ...
        await system.store(
            text=text, collection="working", metadata={"type": "padding"}
        )
    ...
    if results:
        ...
        if (
            scenario["good_advice"].lower()[:50] in top_text
            or top_text in scenario["good_advice"].lower()
        ):
            good_ranked_first = True
...
def calculate_annual_cost(
    tokens_per_query: float, queries_per_month: int = 1_000_000, model: str = "gpt-4o"
) -> Dict:
    """
...
async def main():
    ...
    for scenario in SCENARIOS:
        ...
        # Show result
        print(
            f"  RAG-3: {rag3['tokens_returned']:3d} tokens | RAG-5: {rag5['tokens_returned']:3d} tokens | Roampal: {roampal['tokens_returned']:3d} tokens"
        )
    ...
    print("-" * 80)
    print(
        f"{'Avg tokens/query':<25} {agg_rag3['avg_tokens']:<15.1f} {agg_rag5['avg_tokens']:<15.1f} {agg_roampal['avg_tokens']:<15.1f}"
    )
    print(
        f"{'Hit@1 (accuracy)':<25} {agg_rag3['hit_at_1']:<15.1f}% {agg_rag5['hit_at_1']:<15.1f}% {agg_roampal['hit_at_1']:<15.1f}%"
    )
    print(
        f"{'Hit@3':<25} {agg_rag3['hit_at_3']:<15.1f}% {agg_rag5['hit_at_3']:<15.1f}% {agg_roampal['hit_at_3']:<15.1f}%"
    )
    print(
        f"{'MRR':<25} {agg_rag3['mrr']:<15.3f} {agg_rag5['mrr']:<15.3f} {agg_roampal['mrr']:<15.3f}"
    )
    print(
        f"{'nDCG@3':<25} {agg_rag3['ndcg_at_3']:<15.3f} {agg_rag5['ndcg_at_3']:<15.3f} {agg_roampal['ndcg_at_3']:<15.3f}"
    )
    print(
        f"{'Correct answers':<25} {agg_rag3['correct']:<15.0f}/{n_scenarios} {agg_rag5['correct']:<15.0f}/{n_scenarios} {agg_roampal['correct']:<15.0f}/{n_scenarios}"
    )

    # Token efficiency ratio (accuracy per 100 tokens)
    eff_rag3 = (
        (agg_rag3["accuracy"] / agg_rag3["avg_tokens"]) * 100
        if agg_rag3["avg_tokens"] > 0
        else 0
    )
    eff_rag5 = (
        (agg_rag5["accuracy"] / agg_rag5["avg_tokens"]) * 100
        if agg_rag5["avg_tokens"] > 0
        else 0
    )
    eff_roampal = (
        (agg_roampal["accuracy"] / agg_roampal["avg_tokens"]) * 100
        if agg_roampal["avg_tokens"] > 0
        else 0
    )

    print(
        f"{'Efficiency (acc/100tok)':<25} {eff_rag3:<15.2f} {eff_rag5:<15.2f} {eff_roampal:<15.2f}"
    )
    ...
    reduction_vs_rag3 = (
        (agg_rag3["avg_tokens"] - agg_roampal["avg_tokens"]) / agg_rag3["avg_tokens"]
    ) * 100
    reduction_vs_rag5 = (
        (agg_rag5["avg_tokens"] - agg_roampal["avg_tokens"]) / agg_rag5["avg_tokens"]
    ) * 100

    print(f"\nRoampal token reduction:")
    print(
        f"  vs RAG Top-3: {reduction_vs_rag3:.1f}% fewer tokens ({agg_rag3['avg_tokens']:.0f} -> {agg_roampal['avg_tokens']:.0f})"
    )
    print(
        f"  vs RAG Top-5: {reduction_vs_rag5:.1f}% fewer tokens ({agg_rag5['avg_tokens']:.0f} -> {agg_roampal['avg_tokens']:.0f})"
    )

    # Multiplier
    mult_vs_rag3 = (
        agg_rag3["avg_tokens"] / agg_roampal["avg_tokens"]
        if agg_roampal["avg_tokens"] > 0
        else float("inf")
    )
    mult_vs_rag5 = (
        agg_rag5["avg_tokens"] / agg_roampal["avg_tokens"]
        if agg_roampal["avg_tokens"] > 0
        else float("inf")
    )
    ...
    for key, baseline in RAG_BASELINES.items():
        mult = (
            baseline["tokens"] / agg_roampal["avg_tokens"]
            if agg_roampal["avg_tokens"] > 0
            else float("inf")
        )
        print(
            f"{baseline['name']:<35} {format_tokens(baseline['tokens']):<12} {mult:.1f}x more"
        )

    print(
        f"{'Roampal (this test)':<35} {format_tokens(int(agg_roampal['avg_tokens'])):<12} baseline"
    )
    ...
    for key, baseline in INDUSTRY_BASELINES.items():
        print(
            f"{baseline['name']:<30} {format_tokens(baseline['tokens']):<12} {baseline['source'][:45]}"
        )
