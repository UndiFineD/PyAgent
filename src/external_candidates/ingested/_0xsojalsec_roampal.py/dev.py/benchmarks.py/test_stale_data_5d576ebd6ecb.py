# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_stale_data.py
"""
Stale Data Test
===============
...
async def test_stale_data():
    ...
    memory = UnifiedMemorySystem(
        data_dir="test_data_stale", use_server=False, llm_service=None
    )
    ...
    for test in test_cases:
        ...
        results = await memory.search(
            query=test["query"], collections=["memory_bank"], limit=3
        )
    ...
    print("=" * 80)
    print(
        f"NEW (fresh) data ranked #1: {new_wins}/{len(test_cases)} queries ({accuracy:.1f}%)"
    )
    ...
    if success:
        ...
        if new_wins < 5:
            print(
                f"       Note: {5 - new_wins} queries had OLD closer semantically (expected)"
            )
    else:
