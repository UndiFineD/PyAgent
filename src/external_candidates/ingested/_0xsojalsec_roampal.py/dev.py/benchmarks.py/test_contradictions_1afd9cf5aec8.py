...
async def test_many_wrong_vs_one_right():
    ...
    try:
        ...
        for wrong in wrong_answers:
            await memory.store_memory_bank(
                text=wrong, tags=["job", "uncertain"], importance=0.3, confidence=0.3
            )
        ...
        for i, r in enumerate(results[:7]):
            ...
            print(
                f"  {i+1}. score={score:.3f} q={q:.2f} emb_sim={emb_sim:.3f} | {text}..."
            )
...
async def test_temporal_update():
    ...
    try:
        ...
        results = await memory.search(
            query="Where does the user live?", collections=["memory_bank"], limit=5
        )
...
async def test_confidence_conflict():
    ...
    try:
        ...
        results = await memory.search(
            query="Is the user vegetarian?", collections=["memory_bank"], limit=5
        )
...
async def test_implicit_contradiction():
    ...
    try:
        ...
        results = await memory.search(
            query="Does the user like coffee?", collections=["memory_bank"], limit=5
        )

        top_text = results[0].get("text", "") if results else ""
        coffee_lover_first = (
            "coffee" in top_text.lower() and "loves" in top_text.lower()
        )
