# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_comprehensive_benchmark.py
"""
================================================================================
...
async def test_rag_baseline(
    ...
    collection = client.create_collection(
        name="rag_test", metadata={"hnsw:space": "l2"}
    )
...
async def test_reranker_only(
    ...
    client = chromadb.PersistentClient(path=os.path.join(data_dir, "chromadb"))
    collection = client.create_collection(
        name="reranker_test", metadata={"hnsw:space": "l2"}
    )
    ...
    tokens_used = (
        embedding_service.token_count + reranker.token_count
    ) - initial_tokens
...
async def test_outcomes_only(
    ...
    client = chromadb.PersistentClient(path=os.path.join(data_dir, "chromadb"))
    collection = client.create_collection(
        name="outcomes_test", metadata={"hnsw:space": "l2"}
    )
    ...
    if apply_outcomes and maturity["uses"] > 0:
        ...
        bad_score = max(
            0.1, (maturity["uses"] - maturity["worked"]) / maturity["uses"] * 0.5
        )  # Low score
    else:
        good_score = 0.5
        bad_score = 0.5
...
async def test_full_roampal(
    ...
    system = UnifiedMemorySystem(
        data_dir=data_dir, use_server=False, llm_service=MockLLMService()
    )
    ...
    if (
        apply_outcomes
        and maturity["uses"] > 0
        and scenario["domain"] in TRAIN_DOMAINS
        and adapter
    ):
    ...
    # Search
    results = await system.search(scenario["query"], collections=["working"], limit=5)
...
async def main():
    ...
    print("-" * 80)
    print(
        f"Running {len(SCENARIOS)} scenarios × {len(MATURITY_LEVELS)} levels × {len(conditions)} conditions = {total_tests} tests"
    )
    print("-" * 80)
    ...
    for scenario in SCENARIOS:
        for maturity in MATURITY_LEVELS:
            for condition in conditions:
                test_num += 1
                print(
                    f"\r[{test_num:3d}/{total_tests}] {scenario['domain']:8s} | {maturity['name']:12s} | {condition:15s}",
                    end="",
                    flush=True,
                )

                data_dir = str(
                    test_dir / f"s{scenario['id']}_{maturity['name']}_{condition}"
                )
                ...
                if condition == "rag_baseline":
                    result = await test_rag_baseline(
                        scenario, maturity, data_dir, embedding_service
                    )
                elif condition == "reranker_only":
                    result = await test_reranker_only(
                        scenario, maturity, data_dir, embedding_service, reranker
                    )
                elif condition == "outcomes_only":
                    result = await test_outcomes_only(
                        scenario, maturity, data_dir, embedding_service, apply_outcomes
                    )
                elif condition == "full_roampal":
                    result = await test_full_roampal(
                        scenario, maturity, data_dir, embedding_service, apply_outcomes
                    )
    ...
    for maturity in MATURITY_LEVELS:
        mat_results = [
            ...
            if r.condition == "full_roampal" and r.maturity == maturity["name"]
        ]
        ...
        print(
            f"{maturity['name']:<12} {maturity['uses']:>6} {top1:>7.1f}% {mrr:>8.3f} {ndcg:>8.3f}"
        )
    ...
    if train_results:
        train_top1 = (
            statistics.mean([1 if r.top1_correct else 0 for r in train_results]) * 100
        )
        train_mrr = statistics.mean([r.mrr for r in train_results])
        print(
            f"Train domains ({', '.join(TRAIN_DOMAINS)}): {train_top1:.1f}% Top-1, {train_mrr:.3f} MRR"
        )

    if test_results:
        test_top1 = (
            statistics.mean([1 if r.top1_correct else 0 for r in test_results]) * 100
        )
        test_mrr = statistics.mean([r.mrr for r in test_results])
        print(
            f"Test domains  ({', '.join(TEST_DOMAINS)}): {test_top1:.1f}% Top-1, {test_mrr:.3f} MRR"
        )
    ...
    for condition in conditions:
        cond_results = [
            r
            for r in all_results
            if r.condition == condition and r.maturity == "mature"
        ]
        if cond_results:
            avg_tokens = statistics.mean([r.tokens_used for r in cond_results])
            top1 = (
                statistics.mean([1 if r.top1_correct else 0 for r in cond_results])
                * 100
            )
            efficiency = top1 / avg_tokens * 100 if avg_tokens > 0 else 0
            print(
                f"{condition:<18}: {avg_tokens:>6.0f} tokens/query, {top1:.1f}% accuracy, {efficiency:.2f} acc/token"
            )
    ...
    print(
        f"+ Reranker:        {reranker_acc:>6.1f}% (+{reranker_acc - rag_acc:.1f} pts)"
    )
    print(
        f"+ Outcomes only:   {outcomes_acc:>6.1f}% (+{outcomes_acc - rag_acc:.1f} pts)"
    )
                ...
                "summary": {
                    "by_condition": {
                        c: {
                            ...
                            * 100,
                            "mrr": statistics.mean(
                                [r.mrr for r in all_results if r.condition == c]
                            ),
                            "ndcg5": statistics.mean(
                                [r.ndcg5 for r in all_results if r.condition == c]
                            ),
                        }
                        for c in conditions
                    },
                    ...
                    "cross_domain": {
                        "train_top1": (
                            statistics.mean(
                                [1 if r.top1_correct else 0 for r in train_results]
                            )
                            * 100
                            if train_results
                            else 0
                        ...
                        "test_top1": (
                            statistics.mean(
                                [1 if r.top1_correct else 0 for r in test_results]
                            )
                            * 100
                            if test_results
                            else 0
    ...
    full_mature = [
        r
        for r in all_results
        if r.condition == "full_roampal" and r.maturity == "mature"
    ]
    rag_mature = [
        r
        for r in all_results
        if r.condition == "rag_baseline" and r.maturity == "mature"
    ]
    reranker_mature = [
        r
        for r in all_results
        if r.condition == "reranker_only" and r.maturity == "mature"
    ]
    ...
    def mcnemar_test(a_results, b_results):
        ...
        b = sum(
            ...
            if a.top1_correct and not b.top1_correct
        )
        c = sum(
            1
            for a, b in zip(a_results, b_results)
            if not a.top1_correct and b.top1_correct
        )
        ...
        p_value = (
            stats.binomtest(b, n, 0.5, alternative="two-sided").pvalue if n > 0 else 1.0
        )
        return p_value
    ...
    sig_mcnemar = (
        "***"
        if p_mcnemar < 0.001
        else "**" if p_mcnemar < 0.01 else "*" if p_mcnemar < 0.05 else ""
    )
    sig_ttest = (
        "***"
        if p_ttest < 0.001
        else "**" if p_ttest < 0.01 else "*" if p_ttest < 0.05 else ""
    )
    ...
    cold_results = [
        r
        for r in all_results
        if r.condition == "full_roampal" and r.maturity == "cold_start"
    ]
