# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_learning_speed.py
"""
    ...
    try:
        ...
        for query, expected_collections in test_queries:
            ...
            if hit:
                ...
                print(
                    f"  [MISS] '{query[:30]}...' -> {routed[:2]} (expected {expected_collections})"
                )
...
async def test_learning_curve():
    ...
    try:
        ...
        training_scenarios = [
            ...
            (
                "What Python framework does user like?",
                personal_doc,
                "memory_bank",
                "worked",
            ),
        ...
        for i, (query, doc_id, collection, outcome) in enumerate(training_scenarios):
            ...
            await memory.record_outcome(
                doc_id=doc_id, outcome=outcome, context={"quiz_question": query}
            )
    ...
    try:
        ...
        for i, text in enumerate(
            ...
            doc_id = await memory.store_memory_bank(
                text=text, tags=["fitness", "health"], importance=0.85, confidence=0.9
            )
        ...
        return (
            "Adaptation speed",
            speed_score,
            {"interactions": interactions_to_learn, "learned": learned},
        )
    finally:
        cleanup_memory(test_dir)
...
async def test_knowledge_retention():
    ...
    try:
        ...
        print(
            f"  [AFTER TRAINING] Routes to patterns: {'YES' if trained_has_patterns else 'NO'}"
        )
        ...
        final_has_patterns = "patterns" in final_route[:2]
        print(
            f"  [AFTER NOISE] Routes to patterns: {'YES' if final_has_patterns else 'NO'}"
        )
        ...
        for concept, data in routing_patterns.items():
            if (
                "sql" in concept.lower()
                or "database" in concept.lower()
                or "injection" in concept.lower()
            ):
...
async def test_pattern_recognition():
    ...
    print("-" * 80)
    print(
        "Scenario: System should recognize similar queries and reuse successful routes"
    )
    ...
    try:
        ...
        for query in training_queries:
            await memory.record_outcome(
                doc_id=error_doc, outcome="worked", context={"quiz_question": query}
            )
        ...
        return (
            "Pattern recognition",
            recognition_rate,
            {"recognized": recognized, "total": len(test_queries)},
        )
    finally:
        cleanup_memory(test_dir)
    ...
    try:
        ...
        for context_name, doc_id, expected_collection, queries in contexts:
            for query in queries:
                await memory.record_outcome(
                    doc_id=doc_id, outcome="worked", context={"quiz_question": query}
                )
            print(
                f"  [TRAIN] {context_name} context: {len(queries)} queries -> {expected_collection}"
            )
        ...
        for query, expected, context_name in test_cases:
            ...
            if expected in route[:2]:
                ...
                print(
                    f"  [MISS] {context_name}: '{query[:30]}...' -> {route[:2]} (expected {expected})"
                )
