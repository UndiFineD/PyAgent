# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_comprehensive.py
"""
...
backend_path = str(
    Path(__file__).parent.parent.parent / "ui-implementation" / "src-tauri" / "backend"
)
...
class TestHarness:
    ...
    def end_suite(self):
        ...
        print("=" * 70)
        print(
            f"RESULTS: {self.tests_passed}/{self.tests_run} tests passed ({self.tests_passed/self.tests_run*100:.1f}%)"
        )
...
class TestContext:
    ...
    def __init__(
        self, memory_system: UnifiedMemorySystem, time_manager: MockTimeManager
    ):
    ...
    async def store_test_memory(self, text: str, collection: str, **metadata) -> str:
        ...
        if collection == "memory_bank":
            ...
            doc_id = await self.memory.store_memory_bank(
                text, tags, importance, confidence
            )
        else:
    ...
    async def search_test(
        self, query: str, collections: List[str] = None, limit: int = 5
    ) -> List[Dict]:
    ...
    for collection in collections_to_test:
        ...
        async def test():
            ...
            assert doc_id.startswith(
                f"{collection}_"
            ), f"Doc ID should start with {collection}_"
            assert verify_doc_id_format(
                doc_id, collection
            ), f"Invalid doc ID format: {doc_id}"

            # Verify document is retrievable by returned doc_id
            assert ctx.verify_doc_exists(
                doc_id
            ), f"Document {doc_id} not found after storage"
...
async def test_1_2_deduplication(ctx: TestContext, harness: TestHarness):
    ...
    async def test_memory_bank_dedup():
        ...
        doc_id1 = await ctx.store_test_memory(
            text, "memory_bank", importance=0.9, confidence=0.9
        )
        ...
        ), f"Should not add lower quality duplicate (count: {initial_count} -> {count_after_first} -> {final_count})"
...
async def test_1_3_contextual_retrieval(ctx: TestContext, harness: TestHarness):
    ...
    async def test_prefix_generation():
        ...
        await ctx.store_test_memory(text, "memory_bank", importance=0.9, confidence=0.9)

        # Verify stored (prefix is internal, just verify storage succeeded)
        new_count = ctx.get_collection_count("memory_bank")
        assert (
            new_count == initial_count + 1
        ), "Memory with contextual prefix should be stored"
...
async def test_2_1_basic_search(ctx: TestContext, harness: TestHarness):
    ...
    async def test_single_collection():
        ...
        results = await ctx.search_test(
            "PostgreSQL indexing", collections=["books"], limit=5
        )
    ...
    async def test_empty_collection():
        ...
        results = await ctx.search_test(
            "nonexistent query", collections=["patterns"], limit=5
        )
...
async def test_2_5_quality_ranking(ctx: TestContext, harness: TestHarness):
    ...
    async def test_quality_boost():
        ...
        doc_id1 = await ctx.store_test_memory(
            text1, "memory_bank", importance=0.9, confidence=0.95
        )

        # Store low-quality memory with similar text
        text2 = "Maybe user likes backend work"
        doc_id2 = await ctx.store_test_memory(
            text2, "memory_bank", importance=0.3, confidence=0.4
        )

        # Search - high quality should rank higher
        results = await ctx.search_test(
            "backend engineer", collections=["memory_bank"], limit=10
        )
...
async def test_3_2_collections_with_scoring(ctx: TestContext, harness: TestHarness):
    ...
    async def test_scoreable_collections():
        ...
        await ctx.store_test_memory(
            "Test", "memory_bank", importance=0.8, confidence=0.8
        )

    await harness.run_test(
        "Collections with outcome scoring", test_scoreable_collections
    )
...
async def test_4_1_working_to_history(ctx: TestContext, harness: TestHarness):
    ...
    async def test_promotion_threshold():
        ...
        doc_id = await ctx.store_test_memory(
            "Valuable memory", "working", score=0.5, uses=1
        )
...
async def test_4_2_history_to_patterns(ctx: TestContext, harness: TestHarness):
    ...
    async def test_high_value_promotion():
        ...
        doc_id = await ctx.store_test_memory(
            "Proven solution", "history", score=0.7, uses=1
        )
...
async def test_4_5_deletion(ctx: TestContext, harness: TestHarness):
    ...
    async def test_low_score_deletion():
        # Create history memory
        doc_id = await ctx.store_test_memory("Bad memory", "history", score=0.5)
...
async def test_6_2_working_collection(ctx: TestContext, harness: TestHarness):
    ...
    async def test_working_decay():
        doc_id = await ctx.store_test_memory(
            "Temporary conversation", "working", score=0.5
        )
...
async def test_6_5_memory_bank_collection(ctx: TestContext, harness: TestHarness):
    ...
    async def test_memory_bank_capacity():
        ...
        initial_count = ctx.get_collection_count("memory_bank")
        await ctx.store_test_memory(
            "Important user fact", "memory_bank", importance=0.9, confidence=0.9
        )
        ...
        assert (
            count_after_time == count_after_store
        ), "Memory_bank should be permanent (no time-based decay)"
...
async def main():
    ...
    memory = UnifiedMemorySystem(
        ...
        use_server=False,  # Embedded mode
    ...
    try:
        ...
        print(f"\nKG Statistics:")
        print(
            f"  Routing KG: {len(memory.knowledge_graph.get('routing_patterns', {}))} concepts"
        )
        print(
            f"  Content KG: {len(memory.content_graph.entities)} entities, {len(memory.content_graph.relationships)} relationships"
        )
