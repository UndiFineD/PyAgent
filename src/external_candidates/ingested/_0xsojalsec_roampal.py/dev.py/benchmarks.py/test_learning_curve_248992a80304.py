# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_learning_curve.py
"""
================================================================================
...
async def test_at_maturity_level(
    scenario: Dict, maturity: Dict, data_dir: str, embedding_service
) -> Dict:
    ...
    results = await system.search(
        scenario["query"], collections=["working", "history", "patterns"], limit=10
    )
...
async def main():
    ...
    for m in MATURITY_LEVELS:
        ...
        print(
            f"  {m['name']:12s}: {m['uses']:2d} uses, {m['worked']:2d} worked ({success_rate:.0f}% success)"
        )
    print()
    ...
    print("-" * 80)
    print(
        f"Running {len(SCENARIOS)} scenarios × {len(MATURITY_LEVELS)} maturity levels = {total_tests} tests"
    )
    print("-" * 80)
    ...
    for scenario in SCENARIOS:
        for maturity in MATURITY_LEVELS:
            test_num += 1
            print(
                f"\r[{test_num:3d}/{total_tests}] {scenario['domain']:8s} @ {maturity['name']:12s}",
                end="",
                flush=True,
            )
            ...
            level_accuracies[maturity["name"]].append(
                1.0 if result["top1_correct"] else 0.0
            )
    ...
    for maturity in MATURITY_LEVELS:
        ...
        print(
            f"{maturity['name']:<15} {maturity['uses']:<6} {accuracy:>5.1f}%       {int(correct)}/{total}"
        )
    ...
    print(f"\n{'='*50}")
    print(
        f"IMPROVEMENT: {cold_start_acc:.1f}% -> {mature_acc:.1f}% (+{improvement:.1f} pts)"
    )
    print(f"{'='*50}")
        ...
        json.dump(
            {
                ...
                "per_domain_mature": {
                    d: c["correct"] / c["total"] * 100 for d, c in domains.items()
                },
    ...
    if improvement > 10:
        ...
        print(
            f"    from cold start ({cold_start_acc:.1f}%) to mature ({mature_acc:.1f}%)"
        )
