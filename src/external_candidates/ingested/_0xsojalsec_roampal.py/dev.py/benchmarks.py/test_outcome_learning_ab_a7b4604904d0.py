# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_outcome_learning_ab.py
"""
...
TEST_SCENARIOS = [
    {
        ...
        "memories": [
            {
                "text": "Use connection pooling with a pool size of 10 for database connections",
                "outcome": "worked",
            },
            {
                "text": "Use connection pooling with a pool size of 10 for database connections",
    ...
    },
    {
        ...
        "memories": [
            {
                "text": "Cache API responses in Redis with a 5 minute TTL for performance",
                "outcome": "worked",
            },
            {
                "text": "Cache API responses in Redis with a 5 minute TTL for performance",
    ...
    },
    {
        ...
        "memories": [
            {
                "text": "Use JWT tokens with 1 hour expiration for stateless API authentication",
                "outcome": "worked",
            },
            {
                "text": "Use JWT tokens with 1 hour expiration for stateless API authentication",
    ...
    },
    {
        ...
        "memories": [
            {
                "text": "Implement exponential backoff with max 3 retries for transient failures",
                "outcome": "worked",
            },
            {
                "text": "Implement exponential backoff with max 3 retries for transient failures",
    ...
    },
    {
        ...
        "memories": [
            {
                "text": "Use structured JSON logging with correlation IDs for distributed tracing",
                "outcome": "worked",
            },
            {
                "text": "Use structured JSON logging with correlation IDs for distributed tracing",
...
async def run_scenario_with_outcomes(
    ...
    all_results = await system.search(
        scenario["query"], collections=["working"], limit=4
    )
...
async def main():
    ...
    for i, scenario in enumerate(TEST_SCENARIOS):
        ...
        treatment = await run_scenario_with_outcomes(
            scenario, treatment_dir, embedding_service
        )
        treatment_results.append(treatment)
        marker = "PASS" if treatment["top1_is_worked"] else "FAIL"
        print(
            f"    [WITH outcomes]    Top-1 is worked: {treatment['top1_is_worked']} [{marker}]"
        )

        # Control: WITHOUT outcome scoring
        control_dir = str(test_dir / f"control_{i}")
        os.makedirs(control_dir, exist_ok=True)
        control = await run_scenario_without_outcomes(
            scenario, control_dir, embedding_service
        )
        control_results.append(control)
        marker = "PASS" if control["top1_is_worked"] else "(expected ~25%)"
        print(
            f"    [WITHOUT outcomes] Top-1 is worked: {control['top1_is_worked']} [{marker}]"
        )
