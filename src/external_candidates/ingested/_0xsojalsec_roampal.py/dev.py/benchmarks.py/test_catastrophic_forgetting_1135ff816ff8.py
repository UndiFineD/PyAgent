# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\dev\benchmarks\test_catastrophic_forgetting.py
"""
    ...
    try:
        ...
        for fact in family_facts:
            doc_id = await memory.store_memory_bank(
                text=fact, tags=["family"], importance=0.9, confidence=0.9
            )
        ...
        found_mother_before = any(
            "margaret" in r.get("text", "").lower() for r in results_before
        )
        ...
        for fact in cooking_facts:
            await memory.store_memory_bank(
                text=fact, tags=["cooking", "recipes"], importance=0.7, confidence=0.8
            )
        ...
        found_mother_after = any(
            "margaret" in r.get("text", "").lower() for r in results_after
        )
        ...
        found_brother = any(
            "david" in r.get("text", "").lower()
            or "brother" in r.get("text", "").lower()
        ...
        print(
            f"  Mother query AFTER new facts: {'FOUND' if found_mother_after else 'LOST'}"
        )
        print(
            f"  Brother query AFTER new facts: {'FOUND' if found_brother else 'LOST'}"
        )
...
async def test_same_domain_retention():
    ...
    try:
        ...
        found_jennifer_before = any(
            "jennifer" in r.get("text", "").lower() for r in results_before
        )
        ...
        found_jennifer_after = any(
            "jennifer" in r.get("text", "").lower() for r in results_after
        )
        ...
        found_google = any(
            "google" in r.get("text", "").lower() for r in results_google
        )
        ...
        print(
            f"  Jennifer query AFTER new facts: {'FOUND' if found_jennifer_after else 'LOST'}"
        )
        if jennifer_rank:
            print(f"  Jennifer fact rank: #{jennifer_rank}")
        print(
            f"  Google career start still accessible: {'YES' if found_google else 'NO'}"
        )
...
async def test_bulk_insert_survival():
    ...
    try:
        ...
        found_peanut = any(
            "peanut" in r.get("text", "").lower() for r in results_allergy
        )
        ...
        found_spouse = any(
            "spouse" in r.get("text", "").lower() or "555-1234" in r.get("text", "")
        ...
        success = (
            found_peanut
            and found_spouse
            and (peanut_rank is not None and peanut_rank <= 2)
        )
        print(
...
async def test_quality_preservation():
    ...
    try:
        ...
        results = await memory.search(
            query="Did the user attend MIT?", collections=["memory_bank"], limit=10
        )
...
async def test_cross_domain_isolation():
    ...
    try:
        ...
        for domain, facts in domains.items():
            for fact in facts:
                await memory.store_memory_bank(
                    text=fact, tags=[domain], importance=0.85, confidence=0.85
                )
                await asyncio.sleep(0.01)
        ...
        for domain, (query, keyword) in queries.items():
            results = await memory.search(
                query=query, collections=["memory_bank"], limit=3
            )
