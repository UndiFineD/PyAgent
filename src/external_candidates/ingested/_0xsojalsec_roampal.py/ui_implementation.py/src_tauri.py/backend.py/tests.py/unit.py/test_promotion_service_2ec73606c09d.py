# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\unit\test_promotion_service.py
"""
...
class TestWorkingToHistoryPromotion:
    ...
    async def test_promotes_working_to_history(self, service, mock_collections):
        ...
        mock_collections["working"].delete_vectors.assert_called_with(
            ["working_test123"]
        )
...
class TestHistoryToPatternsPromotion:
    ...
    def mock_collections(self):
        history = MagicMock()
        history.get_fragment = MagicMock(
            return_value={
    ...
    async def test_promotes_history_to_patterns(self, service, mock_collections):
        ...
        mock_collections["history"].delete_vectors.assert_called_with(
            ["history_test123"]
        )
...
class TestDemotion:
    ...
    async def test_demotes_patterns_to_history(self, service, mock_collections):
        ...
        mock_collections["history"].upsert_vectors.assert_called_once()
        mock_collections["patterns"].delete_vectors.assert_called_with(
            ["patterns_test123"]
        )
...
class TestDeletion:
    ...
    def mock_collections(self):
        working = MagicMock()
        working.get_fragment = MagicMock(
            return_value={"content": "test", "metadata": {"text": "test"}}
        )
        working.delete_vectors = MagicMock()
...
class TestBatchPromotion:
    ...
    def mock_collections(self):
        working = MagicMock()
        working.list_all_ids = MagicMock(
            return_value=["working_high_score", "working_low_score", "working_old_item"]
        )
...
class TestPromoteItem:
    ...
    def mock_collections(self):
        source = MagicMock()
        source.get_fragment = MagicMock(return_value={"content": "test content"})
