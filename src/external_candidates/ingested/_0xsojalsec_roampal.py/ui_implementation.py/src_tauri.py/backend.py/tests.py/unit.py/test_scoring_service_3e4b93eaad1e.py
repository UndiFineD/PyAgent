# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\unit\test_scoring_service.py
"""
...
from modules.memory.scoring_service import ScoringService, wilson_score_lower
...
class TestWilsonScoreLower:
    ...
    def test_score_range(self):
        ...
        for successes, total in test_cases:
            ...
            assert (
                0 <= score <= 1
            ), f"Score {score} out of range for {successes}/{total}"
    ...
    def test_matches_expected_values(self):
        ...
        for (successes, total), expected_approx in expected.items():
            ...
            assert (
                abs(new_score - expected_approx) < 0.01
...
class TestScoringService:
    ...
    def test_calculate_final_score_basic(self, service):
        ...
        result = service.calculate_final_score(
            metadata, distance=1.0, collection="history"
        )
    ...
    def test_calculate_final_score_distance_conversion(self, service):
        ...
        result0 = service.calculate_final_score(
            metadata, distance=0.0, collection="history"
        )
        assert result0["embedding_similarity"] == 1.0

        result1 = service.calculate_final_score(
            metadata, distance=1.0, collection="history"
        )
        assert result1["embedding_similarity"] == 0.5

        result10 = service.calculate_final_score(
            metadata, distance=10.0, collection="history"
        )
    ...
    def test_apply_scoring_to_results(self, service):
        ...
        results = [
            {
                "metadata": {"score": 0.3, "uses": 0},
                "distance": 2.0,
                "collection": "history",
            },
            {
                "metadata": {"score": 0.9, "uses": 5},
                "distance": 0.5,
                "collection": "history",
            },
            {
                "metadata": {"score": 0.5, "uses": 2},
                "distance": 1.0,
                "collection": "history",
            },
        ]
...
class TestScoringConsistency:
    ...
    def test_memory_bank_quality_scoring(self, service):
        ...
        result = service.calculate_final_score(
            metadata, distance=0.5, collection="memory_bank"
        )
