...
class TestMemoryBankServiceInit:
    ...
    def test_init_with_custom_config(self):
        ...
        service = MemoryBankService(
            collection=MagicMock(), embed_fn=AsyncMock(), config=config
        )
...
class TestStore:
    ...
    def service(self, mock_collection):
        return MemoryBankService(
            collection=mock_collection, embed_fn=AsyncMock(return_value=[0.1] * 384)
        )
    ...
    async def test_store_with_importance_confidence(self, service, mock_collection):
        ...
        await service.store(
            text="Critical info", tags=["identity"], importance=0.95, confidence=0.9
        )
...
class TestUpdate:
    ...
    async def test_update_archives_old(self, service, mock_collection):
        ...
        await service.update(
            doc_id="memory_bank_test123", new_text="new content", reason="correction"
        )
    ...
    async def test_update_not_found_creates_new(self, service, mock_collection):
        ...
        doc_id = await service.update(
            doc_id="memory_bank_nonexistent", new_text="new memory"
        )
...
class TestArchive:
    ...
    def mock_collection(self):
        coll = MagicMock()
        coll.get_fragment = MagicMock(
            return_value={"content": "test", "metadata": {"status": "active"}}
        )
...
class TestSearch:
    ...
    def mock_collection(self):
        coll = MagicMock()
        coll.list_all_ids = MagicMock(
            return_value=["memory_bank_1", "memory_bank_2", "memory_bank_3"]
        )
...
class TestRestore:
    ...
    def mock_collection(self):
        coll = MagicMock()
        coll.get_fragment = MagicMock(
            return_value={"content": "test", "metadata": {"status": "archived"}}
        )
        coll.update_fragment_metadata = MagicMock()
        return coll
...
class TestIncrementMention:
    ...
    def mock_collection(self):
        coll = MagicMock()
        coll.get_fragment = MagicMock(
            return_value={"content": "test", "metadata": {"mentioned_count": 5}}
        )
        coll.update_fragment_metadata = MagicMock()
        return coll
