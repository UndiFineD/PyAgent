# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\characterization\test_outcome_behavior.py
"""
...
def clear_memory_modules():
    ...
    to_delete = [
        ...
        if key.startswith("modules.memory") or key == "modules"
    ]
...
class TestOutcomeBehavior:
    ...
    async def test_outcome_scoring_logic(self, memory_system):
        ...
        assert (
            memory_system.HIGH_VALUE_THRESHOLD > memory_system.PROMOTION_SCORE_THRESHOLD
        )
        assert (
            memory_system.PROMOTION_SCORE_THRESHOLD
            > memory_system.DEMOTION_SCORE_THRESHOLD
        )
        assert (
            memory_system.DEMOTION_SCORE_THRESHOLD
            > memory_system.DELETION_SCORE_THRESHOLD
        )
...
class TestPromotionBehavior:
    ...
    def test_promotion_is_async(self, memory_system):
        ...
        assert asyncio.iscoroutinefunction(
            memory_system.promote_valuable_working_memory
        )
...
class TestActionOutcome:
    ...
    def test_action_outcome_fields(self):
        ...
        ao = ActionOutcome(
            action_type="search_memory", context_type="coding", outcome="worked"
        )
