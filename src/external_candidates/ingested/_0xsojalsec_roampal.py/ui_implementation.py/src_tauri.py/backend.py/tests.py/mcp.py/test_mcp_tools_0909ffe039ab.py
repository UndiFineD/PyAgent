...
class TestSearchMemoryTool:
    ...
    async def test_search_returns_formatted_results(self, mock_memory):
        ...
        result = await call_search_memory(
            memory=mock_memory, arguments={"query": "test"}, session_id="test_session"
        )
...
class TestUpdateMemoryTool:
    ...
    def mock_memory(self):
        memory = MagicMock()
        memory.search_memory_bank = AsyncMock(
            return_value=[{"id": "memory_bank_123", "content": "old content"}]
        )
...
class TestArchiveMemoryTool:
    ...
    def mock_memory(self):
        memory = MagicMock()
        memory.search_memory_bank = AsyncMock(
            return_value=[{"id": "memory_bank_123", "content": "to archive"}]
        )
...
class TestToolSchemaCompliance:
    ...
    def test_update_memory_required_params(self):
        ...
        assert (
            validate_tool_args(
                "update_memory", {"old_content": "old", "new_content": "new"}
            )
            is True
        )
    ...
    def test_record_response_outcome_enum(self):
        ...
        for outcome in ["worked", "failed", "partial", "unknown"]:
            assert (
                validate_tool_args(
                    "record_response", {"key_takeaway": "test", "outcome": outcome}
                )
                is True
            )
        ...
        assert (
            validate_tool_args(
                "record_response", {"key_takeaway": "test", "outcome": "invalid"}
            )
            is False
        )
