# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\tests\mcp\mcp_tool_harness.py
"""
...
async def call_search_memory(
    ...
    results = await memory.search(
        query=query, collections=collections, limit=limit, metadata_filters=metadata
    )
...
async def call_add_to_memory_bank(
    ...
    doc_id = await memory.store_memory_bank(
        text=content, tags=tags, importance=importance, confidence=confidence
    )
...
async def call_get_context_insights(
    memory, arguments: Dict[str, Any], session_id: str = "default", data_path=None
) -> str:
    """
    ...
    context_type = await memory.detect_context_type(
        system_prompts=[], recent_messages=[]
    )

    # Get conversation context
    org_context = await memory.analyze_conversation_context(
        current_message=query, recent_conversation=[], conversation_id=session_id
    )
...
async def call_record_response(
    memory, arguments: Dict[str, Any], session_id: str = "default"
) -> str:
    """
    ...
    score_map = {"worked": 0.7, "failed": 0.2, "partial": 0.55, "unknown": 0.5}
