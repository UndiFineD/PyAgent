...
class TestRecordOutcome:
    ...
    async def test_record_outcome_with_reason(self, mock_ums):
        ...
        await mock_ums.record_outcome(
            doc_id="working_123", outcome="failed", failure_reason="Test failure"
        )
...
class TestMemoryBankAPI:
    ...
    async def test_store_memory_bank(self, mock_ums):
        ...
        doc_id = await mock_ums.store_memory_bank(
            text="User prefers dark mode", tags=["preference"]
        )
    ...
    async def test_update_memory_bank(self, mock_ums):
        ...
        await mock_ums.update_memory_bank(
            doc_id="memory_bank_123", new_text="Updated text"
        )
...
class TestPromotionAPI:
    ...
    def mock_ums(self, tmp_path):
        ...
        ums._promotion_service.promote_valuable_working_memory = AsyncMock(
            return_value=5
        )
...
class TestKGVisualization:
    ...
    def mock_ums(self, tmp_path):
        ...
        ums._kg_service.get_kg_entities = MagicMock(
            return_value=[{"id": "concept1", "weight": 10}]
        )
        ums._kg_service.get_kg_relationships = MagicMock(
            return_value=[{"source": "c1", "target": "c2", "weight": 1}]
        )
