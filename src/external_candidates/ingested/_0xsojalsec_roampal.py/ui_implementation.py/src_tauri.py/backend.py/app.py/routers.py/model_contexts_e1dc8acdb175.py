...
async def set_model_context(
    model_name: str, request: ContextUpdateRequest
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to save context override"
            )
...
async def reset_model_context(model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to reset context override"
            )
