# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\sessions.py
"""
                ...
                try:
                    ...
                    with open(session_file, "r", encoding="utf-8") as f:
                        ...
                        if lines:
                            ...
                            last_msg = (
                                json.loads(lines[-1]) if len(lines) > 1 else first_msg
                            )
                            ...
                            if isinstance(first_timestamp, str):
                                try:
                                    ...
                                    first_timestamp = datetime.fromisoformat(
                                        first_timestamp.split("+")[0].replace("Z", "")
                                    ).timestamp()
                                except:
                            ...
                            if isinstance(last_timestamp, str):
                                try:
                                    ...
                                    last_timestamp = datetime.fromisoformat(
                                        last_timestamp.split("+")[0].replace("Z", "")
                                    ).timestamp()
                                except:
...
async def delete_session(session_id: str, request: Request):
    ...
    try:
        ...
        if hasattr(request.app.state, "memory") and request.app.state.memory:
            ...
            try:
                deleted_count = await request.app.state.memory.delete_by_conversation(
                    session_id
                )
                logger.info(
                    f"Deleted {deleted_count} memories for conversation {session_id}"
                )
            except Exception as mem_error:
                logger.warning(
                    f"Failed to delete memories for {session_id}: {mem_error}"
                )
