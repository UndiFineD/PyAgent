# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\data_management.py
"""
    ...
    try:
        ...
        if kg_path and kg_path.exists():
            try:
                ...
                stats["knowledge_graph"] = {"nodes": nodes, "edges": edges}
            except:
        ...
        if count_before > 0:
            ...
            if all_docs.get("ids"):
                ...
                for i in range(0, len(all_ids), batch_size):
                    ...
                    adapter.collection.delete(ids=batch)
                    logger.info(
                        f"Deleted batch {i // batch_size + 1}: {len(batch)} items"
                    )
...
async def clear_knowledge_graph(request: Request):
    ...
    try:
        ...
        rel_path = (
            memory.data_dir / "memory_relationships.json"
            if memory
            else Path("data/memory_relationships.json")
        )
        ...
        logger.info(
            f"Cleared knowledge graph ({nodes_count} nodes, {edges_count} edges)"
        )
