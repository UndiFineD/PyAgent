# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\personality_manager.py
...
TEMPLATES_DIR = (
    Path(__file__).parent.parent.parent / "backend" / "templates" / "personality"
)
...
def _validate_template(content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if missing_sections:
            raise ValueError(
                f"Template missing required sections: {', '.join(missing_sections)}"
            )
...
async def get_current_template() -> PersonalityTemplate:
    ...
    try:
        ...
        return PersonalityTemplate(
            template_id=template_id, name=name, content=content, is_preset=is_preset
        )
...
async def get_template(template_id: str) -> PersonalityTemplate:
    ...
    try:
        ...
        if preset_file.exists():
            ...
            name = _get_template_name(data)
            return PersonalityTemplate(
                template_id=template_id, name=name, content=content, is_preset=True
            )
        ...
        if custom_file.exists():
            ...
            name = _get_template_name(data)
            return PersonalityTemplate(
                template_id=template_id, name=name, content=content, is_preset=False
            )
...
async def save_custom_template(request: SaveTemplateRequest) -> Dict[str, str]:
    ...
    try:
        ...
        logger.info(
            f"{'Overwrote' if overwrite else 'Saved'} custom template: {safe_name}"
        )
...
async def upload_template(file: UploadFile = File(...)) -> Dict[str, str]:
    ...
    try:
        ...
        return {"template_id": safe_name, "message": "Template uploaded successfully"}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def delete_custom_template(template_id: str) -> Dict[str, str]:
    ...
    try:
        ...
        if not custom_file.exists():
            raise HTTPException(
                status_code=404, detail=f"Custom template '{template_id}' not found"
            )
        ...
        if ACTIVE_FILE.exists():
            ...
            if active_content == custom_content:
                raise HTTPException(
                    status_code=400, detail="Cannot delete currently active template"
                )
        ...
        return {"template_id": template_id, "message": "Template deleted successfully"}
    except HTTPException:
        raise
    except Exception as e:
