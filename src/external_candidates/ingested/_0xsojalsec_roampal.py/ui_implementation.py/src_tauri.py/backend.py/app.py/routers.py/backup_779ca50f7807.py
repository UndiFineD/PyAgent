# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\backup.py
"""
...
async def create_backup(
    include: Optional[str] = Query(
        None,
        description="Comma-separated list: sessions,memory,books,knowledge (default: all)",
    )
) -> FileResponse:
    """
    ...
    try:
        ...
        logger.info(
            f"Creating backup: {backup_name} (including: {', '.join(include_types)})"
        )
        ...
        with zipfile.ZipFile(backup_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            if "memory" in include_types:
                ...
                logger.info(
                    f"Backed up {backup_stats['chromadb_files']} ChromaDB files"
                )
            ...
            if "knowledge" in include_types:
                ...
                logger.info(
                    f"Backed up {backup_stats['knowledge_files']} knowledge files"
                )
            ...
            metadata = {
                ...
                "roampal_version": "2.0",
                "backup_type": (
                    "selective" if include_types != VALID_EXPORT_TYPES else "full"
                ),
...
async def estimate_export_size(
    ...
    try:
        ...
        if "books" in include_types:
            ...
            if books_dir.exists():
                size_bytes = sum(
                    f.stat().st_size for f in books_dir.rglob("*") if f.is_file()
                )
        ...
        return ExportSizeEstimate(
            total_mb=total_mb, breakdown=breakdown, file_counts=file_counts
        )
...
async def list_backups():
    ...
    try:
        ...
        for backup_file in sorted(
            backups_dir.glob("roampal_backup_*.zip"), reverse=True
        ):
            try:
                ...
                with zipfile.ZipFile(backup_file, "r") as zipf:
                    if "backup_info.json" in zipf.namelist():
                        ...
                        contains = {
                            "sessions": metadata.get("backup_stats", {}).get(
                                "sessions", 0
                            )
                            > 0,
                            "chromadb": metadata.get("backup_stats", {}).get(
                                "chromadb_files", 0
                            )
                            > 0,
                            "books": metadata.get("backup_stats", {}).get("books", 0)
                            > 0,
                            "knowledge_graph": metadata.get("backup_stats", {}).get(
                                "knowledge_files", 0
                            )
                            > 0,
                        }
                ...
                logger.warning(
                    f"Could not read backup metadata for {backup_file.name}: {e}"
                )
...
async def restore_from_backup(file: UploadFile = File(...)) -> RestoreResponse:
    ...
    try:
        ...
        pre_restore_backup = (
            Path(DATA_PATH) / f"backups/pre_restore_{backup_timestamp}.zip"
        )
        ...
        if chromadb_src.exists():
            ...
            restored_items["chromadb_files"] = sum(
                1 for _ in chromadb_dest.rglob("*") if _.is_file()
            )
