# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\memory_bank.py
"""
...
async def get_archived_memories(
    ...
    try:
        results = await memory.search_memory_bank(
            query=None, tags=None, include_archived=True, limit=limit * 2
        )
        ...
        return {"memories": archived[:limit], "total": len(archived)}
...
async def restore_memory(request: Request, doc_id: str):
    """
...
async def delete_memory(request: Request, doc_id: str):
    """
...
async def search_memories(
    request: Request, q: str, tags: Optional[str] = None, limit: int = 20
):
    """
    ...
    try:
        ...
        results = await memory.search_memory_bank(
            query=q, tags=tag_list, include_archived=False, limit=limit
        )
            ...
            formatted.append(
                {
                    ...
                    "tags": json.loads(metadata.get("tags", "[]")),
                    "relevance": 1.0
                    / (1.0 + r.get("distance", 0.5)),  # Convert distance to relevance
                    # Note: confidence removed - memory_bank is authoritative facts, not probabilistic
                }
            )

        return {"query": q, "results": formatted, "total": len(formatted)}
...
async def get_memory_stats(request: Request):
    ...
    try:
        ...
        all_memories = await memory.search_memory_bank(
            query=None, include_archived=True, limit=1000
        )

        # Count by status
        active = sum(
            1 for m in all_memories if m.get("metadata", {}).get("status") == "active"
        )
        archived = sum(
            1 for m in all_memories if m.get("metadata", {}).get("status") == "archived"
        )
...
@router.post("/create")
async def create_memory(request: Request, memory_data: MemoryCreate):
    """
    ...
    try:
        ...
        return {"status": "created", "doc_id": doc_id, "text": memory_data.text}
...
async def update_memory(request: Request, doc_id: str, memory_data: MemoryUpdate):
    """
    ...
    try:
        updated_id = await memory.update_memory_bank(
            doc_id=doc_id, new_text=memory_data.text, reason="api_update"
        )
        ...
        if memory_data.tags is not None or memory_data.importance is not None:
            ...
            if doc:
                ...
                memory.collections["memory_bank"].update_fragment_metadata(
                    updated_id, metadata
                )

        return {"status": "updated", "doc_id": updated_id, "text": memory_data.text}
...
async def archive_memory(request: Request, doc_id: str):
    """
    ...
    try:
        success = await memory.archive_memory_bank(doc_id=doc_id, reason="api_archive")
