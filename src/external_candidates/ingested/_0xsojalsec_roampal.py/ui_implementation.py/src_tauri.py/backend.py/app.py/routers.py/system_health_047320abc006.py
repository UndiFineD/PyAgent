# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\system_health.py
"""
...
async def get_system_health(request: Request) -> SystemHealthResponse:
    ...
    try:
        ...
        if backups_dir.exists():
            ...
            backup_size = sum(f.stat().st_size for f in backups_dir.glob("*.zip")) / (
                1024**2
            )  # MB
            ...
            if backup_size > 500:  # 500MB
                warnings.append(
                    f"⚠️ Large backup folder: {backup_size:.0f}MB (consider cleanup)"
                )
        else:
    ...
    try:
        ...
        for name, dir_path in data_dirs.items():
            if dir_path.exists():
                ...
                sizes[name] = {"size_mb": 0, "size_gb": 0, "file_count": 0}
...
async def get_migration_notice():
    ...
    try:
        ...
        if notice_path.exists():
            ...
            return {"has_notice": True, "notice": notice_data}
