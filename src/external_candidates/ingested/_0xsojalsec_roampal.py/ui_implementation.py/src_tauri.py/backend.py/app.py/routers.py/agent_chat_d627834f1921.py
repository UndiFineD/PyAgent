# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\routers\agent_chat.py
"""
...
class AgentChatService:
        ...
        try:
            ...
            try:
                ...
                collections_to_check = [
                    ...
                    "patterns",
                    "memory_bank",
                ]  # Check all 5 tiers + wildcard

                for action in available_actions:
                    # Check across all collections
                    for collection in collections_to_check:
                        stats = self.memory.get_action_effectiveness(
                            context_type, action, collection
                        )
                ...
                matched_concepts = (
                    org_context.get("matched_concepts", []) if org_context else []
                )
                relevant_facts = []
                if matched_concepts:
                    try:
                        relevant_facts = await self.memory.get_facts_for_entities(
                            matched_concepts[:5], limit=2
                        )
                    except Exception as e:
                        logger.warning(
                            f"[CONTEXTUAL GUIDANCE] Failed to get facts for entities: {e}"
                        )

                # Format and inject combined guidance if any insights exist
                has_content_insights = org_context and (
                    org_context.get("relevant_patterns")
                    or org_context.get("past_outcomes")
                    or org_context.get("proactive_insights")
                )
                ...
                if has_content_insights or action_stats or relevant_facts:
                    ...
                    if relevant_facts:
                        ...
                        for fact in relevant_facts:
                            ...
                            eff_str = (
                                f" ({int(eff['success_rate']*100)}% helpful)"
                    ...
                    self.conversation_histories[conversation_id].append(
                        {"role": "system", "content": guidance_msg}
                    )
                    logger.info(
                        f"[CONTEXTUAL GUIDANCE] Context={context_type}, Memory facts={len(relevant_facts)}, Content insights={len(org_context.get('relevant_patterns', []))}, Action stats={len(action_stats)}"
                    )
            ...
            async for event in self.llm.stream_response_with_tools(
                ...
                history=(
                    history_without_current[-6:] if history_without_current else None
                ),  # Last 3 exchanges (excluding current)
                ...
                if event["type"] == "text":
                    ...
                    if tool_match and not tool_executions:  # Only execute once
                        ...
                        logger.info(
                            f"[FALLBACK] Detected text-based tool call: search_memory(query='{query_text}')"
                        )
                        ...
                        if tool_results:
                            ...
                            continuation_prompt = (
                                "Continue your response using the search results above."
                            )
                            async for (
                                event
                            ) in self.ollama_client.stream_response_with_tools(
            ...
            if self.memory:
                try:
                    ...
                    logger.debug(
                        f"[PERSIST] Stored exchange in memory with doc_id: {exchange_doc_id}"
                    )
                except Exception as e:
                    logger.error(
                        f"[PERSIST] Failed to store in memory: {e}", exc_info=True
                    )
            ...
            if session_file.exists():
                try:
                    ...
                    for line in reversed(
                        lines[:-1]
                    ):  # Skip last line (current user message just written)
                        try:
                            parsed = json.loads(line)
                            if parsed.get("role") == "assistant" and parsed.get(
                                "doc_id"
                            ):
                    ...
                    if prev_assistant:
                        logger.debug(
                            f"[OUTCOME] Found previous assistant message with doc_id {prev_assistant['doc_id']}"
                        )
                        ...
                        logger.info(
                            f"[OUTCOME] Detection result: {outcome_result.get('outcome')} (confidence: {outcome_result.get('confidence', 0):.2f})"
                        )

                        if outcome_result.get("outcome") in [
                            "worked",
                            "failed",
                            "partial",
                        ]:
                            ...
                            await self.memory.record_outcome(
                                doc_id=prev_assistant["doc_id"], outcome=outcome
                            )
                            logger.info(
                                f"[OUTCOME] Recorded '{outcome}' for doc_id {prev_assistant['doc_id']}"
                            )
                            ...
                            if conversation_id in _search_cache:
                                ...
                                logger.info(
                                    f"[OUTCOME] Scoring {len(cached_doc_ids)} cached memories with outcome={outcome}"
                                )

                                for cached_doc_id in cached_doc_ids:
                                    try:
                                        await self.memory.record_outcome(
                                            doc_id=cached_doc_id, outcome=outcome
                                        )
                                    except Exception as e:
                                        logger.warning(
                                            f"[OUTCOME] Failed to score cached memory {cached_doc_id}: {e}"
                                        )

                                # Clear cache after scoring
                                del _search_cache[conversation_id]
                                logger.debug(
                                    f"[OUTCOME] Cleared search cache for conversation {conversation_id}"
                                )
                            ...
                            if conversation_id in _agent_action_cache:
                                ...
                                logger.debug(
                                    f"[ACTION_KG] Cleared action cache for conversation {conversation_id}"
                                )
                        else:
                            logger.debug(
                                f"[OUTCOME] Skipping outcome '{outcome_result.get('outcome')}' (not worked/failed/partial)"
                            )
                    else:
                        logger.debug(
                            f"[OUTCOME] No previous assistant message with doc_id found (first message in conversation)"
                        )
                except Exception as e:
                    logger.error(
                        f"[OUTCOME] Failed to read session file for outcome detection: {e}",
                        exc_info=True,
                    )
            ...
            memory_tool_used = any(
                event.get("tool")
                in ["create_memory", "update_memory", "archive_memory"]
    ...
    def _build_complete_prompt(
        ...
        if personality_prompt:
            ...
            parts.append(
                """You are Roampal - a memory-enhanced AI with persistent knowledge across all sessions.
...
""")
        if (
            self.llm
            and hasattr(self.llm, "api_style")
            and self.llm.api_style == "openai"
        ):
            logger.info(
                f"[SYSTEM PROMPT] Adding OpenAI-style tool calling instructions (api_style={self.llm.api_style})"
            )
            parts.append("""
...
Always wait for function results before responding with detailed information.""")
            ...
            logger.info(
                f"[SYSTEM PROMPT] NOT adding OpenAI instructions. llm={self.llm}, has api_style={hasattr(self.llm, 'api_style') if self.llm else 'N/A'}, api_style value={getattr(self.llm, 'api_style', 'N/A') if self.llm else 'N/A'}"
            )
    ...
    async def _persist_conversation_turn(
        ...
        clean_response = re.sub(
            r"```python\s*.*?search_memory.*?```", "", clean_response, flags=re.DOTALL
        )

        clean_response = re.sub(
            r"```\s*result\s*=\s*search_memory.*?```",
            "",
            clean_response,
            flags=re.DOTALL,
        )
        ...
        clean_response, memory_bank_entries = (
            await self._extract_and_store_memory_bank_tags(
                clean_response, conversation_id
            )
        )
        ...
        formatted_citations = (
            _format_search_results_as_citations(search_results)
            if search_results
            else []
        )
        ...
        try:
            ...
            logger.info(
                f"[PERSIST] Saved conversation turn with {len(formatted_citations)} citations to {session_file.name}"
            )

        except Exception as e:

            logger.error(
                f"[PERSIST] Failed to save to session file: {e}", exc_info=True
            )
...
Respond with ONLY the title, nothing else."""
                            ...
                            try:
                                ...
                                if title_response and title_response.strip():
                                    ...
                                    logger.info(
                                        f"[PERSIST] Auto-generated title for {conversation_id}: {title}"
                                    )
                                ...
                                logger.warning(
                                    f"[PERSIST] Title generation failed: {title_err}"
                                )

        except Exception as e:

            logger.warning(
                f"[PERSIST] Failed to check message count for title generation: {e}"
            )
...
Respond with ONLY the title, nothing else."""
    ...
    async def _execute_tool_and_continue(
        ...
        if tool_name == "search_memory":
            ...
            logger.info(
                f"[TOOL] search_memory called with collections={collections}, query='{query[:50]}...')"
            )
            ...
            if tool_results:
                ...
                if doc_ids:
                    ...
                    logger.debug(
                        f"[SEARCH_CACHE] Cached {len(doc_ids)} doc_ids for conversation {conversation_id}"
                    )
            ...
            if tool_results:
                ...
                for idx, r in enumerate(tool_results[:5], start=1):
                    ...
                    tool_response_content += (
                        f"[{idx}] ({collection}{source_info}): {content}...\n"
                    )
                    citations.append(self._format_citation(r, idx))
            else:
            ...
            if content.strip() and self.memory:
                ...
                logger.info(
                    f"[MEMORY_BANK TOOL] Created: {content[:50]}... with tag={tag} (depth={chain_depth})"
                )
        ...
        if tool_execution_record and tool_execution_record.get("status") != "unknown":
            action = ActionOutcome(
                ...
                collection=(
                    tool_args.get("collections", [None])[0]
                    if tool_name == "search_memory"
                    else None
                ),
                doc_id=(
                    _search_cache.get(conversation_id, [None])[0]
                    if tool_name == "search_memory"
                    else None
                ),
            )
        ...
        if (
            (tool_name == "search_memory" or is_external_tool)
...
async def switch_conversation(req: Request):
    ...
    async with agent_service.conversation_lock:
        ...
        if agent_service.memory and old_id:
            ...
            asyncio.create_task(agent_service.memory._promote_valuable_working_memory())
