# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\app\dependencies_initializers.py
import asyncio
import logging
import os
import re
    ...
    for filename in os.listdir(og_books_dir):
        if filename.endswith(".txt"):
            ...
            keywords = [title.lower()] + (
                ...
                if any(
                    kw in title.lower()
                    for kw in ["sicp", "programming", "mit", "cleanarchitecture"]
                )
                else ["philosophy"]
            )
            BOOKS.append({"title": title, "author": author, "keywords": keywords})
            logger.info(f"Added dynamic book: {title}")


async def initialize_llm_client(config: LLMSettings) -> LLMClientInterface:
    logger.debug(
        f"Initializing LLMClient with provider: {config.provider}, model: {config.ollama_model}"
    )
...
async def initialize_fragment_memory_adapter(
    fragment_id: str, global_settings: Settings, adapter_type: str = "chromadb"
    ...
    if not fragment_id or not re.match(r"^[a-zA-Z0-9_-]+$", fragment_id):
        raise ValueError(
            f"Invalid fragment_id: '{fragment_id}'. Only alphanumeric characters, hyphens and underscores allowed."
        )

    logger.info(
        f"Attempting to initialize MemoryAdapter for fragment: '{fragment_id}' ({adapter_type})"
    )
    ...
    if adapter_type == "chromadb":
        ...
        vector_store_path = os.path.abspath(
            settings.paths.get_vector_db_dir(fragment_id)
        )
        ...
        logger.info(
            f"VECTOR_PATH_FIX: Using vector store path for fragment '{fragment_id}': {vector_store_path}"
        )
        adapter = cast(
            MemoryAdapterInterface,
            ChromaDBAdapter(persistence_directory=vector_store_path),
        )
        collection_name = (
            getattr(fragment_memory_config_obj, "collection_name", None)
            or f"roampal_{fragment_id}_soul_fragments"
        )
        await adapter.initialize(collection_name=collection_name)
        logger.info(
            f"Initialized ChromaDBAdapter for fragment '{fragment_id}' at path '{vector_store_path}' in collection '{collection_name}'."
        )
        return adapter
        ...
        logger.info(
            f"Initialized FileMemoryAdapter for fragment '{fragment_id}' at '{getattr(adapter, 'data_path', None)}'."
        )
        return adapter
...
def _resolve_prompt_dirs_from_config(
    config: Any, default_dir: Optional[str] = None
) -> List[str]:
    prompt_dirs = []
...
async def initialize_og_prompt_engine(
    config: OGPromptSettings,
) -> PromptEngineInterface:
    logger.info(
        f"Initializing PromptEngine for Roampal, default prompt file: {config.default_system_prompt_name}.txt"
    )
    engine = PromptEngine()
...
async def initialize_book_processor_for_fragment(
    ...
    if multi_tier_adapter:
        ...
        logger.info(
            f"Initializing BookProcessor for fragment '{fragment_id}' with MultiTierMemoryAdapter for GLOBAL book seeding"
        )
    else:
        memory_adapter = await initialize_fragment_memory_adapter(
            fragment_id, global_settings, adapter_type="chromadb"
        )
        logger.warning(
            f"Initializing BookProcessor for fragment '{fragment_id}' with regular ChromaDB adapter (books won't be in global memory!)"
        )
...
async def initialize_ingestion_manager(
    settings_instance: Settings, llm_client: LLMClientInterface
