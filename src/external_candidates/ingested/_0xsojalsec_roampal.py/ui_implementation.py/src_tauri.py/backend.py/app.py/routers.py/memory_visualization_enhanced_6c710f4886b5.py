...
async def get_memory_stats(request: Request):
    ...
    try:
        ...
        stats = (
            memory_collections.get_stats()
            if hasattr(memory_collections, "get_stats")
            else {}
        )
        ...
        if outcome_tracker:
            try:
                patterns = await outcome_tracker.get_best_patterns(
                    min_attempts=1, min_success_rate=0.0
                )
                outcome_stats = {
                    "total_patterns": len(patterns),
                    "successful_patterns": len(
                        [p for p in patterns if p.get("success_rate", 0) > 0.7]
                    ),
                    "failed_patterns": len(
                        [p for p in patterns if p.get("success_rate", 0) < 0.3]
                    ),
                }
...
async def get_collection_memories(
    ...
    try:
        ...
        actual_collection = (
            "history" if collection_type == "conversations" else collection_type
        )
            ...
            if pattern:
                ...
                for rel_key, rel_data in relationships.items():
                    ...
                    if concept_id in concepts:
                        other_concept = (
                            concepts[0] if concepts[1] == concept_id else concepts[1]
                        )
    ...
    try:
        ...
        return {
            ...
            "config": {
                ...
                "pattern_failure_threshold": stats["config"][
                    "pattern_failure_threshold"
                ],
        ...
        if isinstance(results, dict):
                ...
                formatted_results.append(
                    {
                        "content": item.get("content", item.get("text", "")),
                        "collection": item.get(
                            "collection", item.get("collection_type", "unknown")
                        ),
            ...
            for item in results:
                if isinstance(item, dict):
                    ...
                    logger.warning(
                        f"Unexpected item type in search results: {type(item)}"
                    )
                    continue
...
async def record_memory_feedback(
    ...
    outcome: str = Query(
        ..., description="Outcome: 'worked', 'failed', 'partial', or 'unknown'"
    ),
    ...
    """
    try:
        memory = getattr(request.app.state, "memory", None) or getattr(
            request.app.state, "memory_collections", None
        )
        if not memory:
        ...
        logger.info(
            f"Recorded feedback for {doc_id}: {outcome} (confidence: {confidence})"
        )
