# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\intelligent_router.py
"""
...
class IntelligentRouter:
    ...
    def __init__(self, knowledge_graph: Dict, llm_service=None, cache_ttl: int = 300):
        ...
        self._routing_cache: Dict[str, tuple] = (
            {}
        )  # query_hash -> (collections, timestamp)
    ...
    async def route_query(self, query: str, concepts: List[str]) -> List[str]:
        ...
        if self.llm_service:
            try:
                ...
                logger.warning(
                    f"[IntelligentRouter] LLM routing failed, using heuristic: {e}"
                )
    ...
    "confidence": 0.0-1.0
        ...
        try:
            ...
            if not all(k in result for k in ["collections", "reasoning", "confidence"]):
                logger.warning(
                    f"[IntelligentRouter] Invalid LLM response structure: {result}"
                )
                return None
            ...
            if confidence <= 0.7:
                logger.debug(
                    f"[IntelligentRouter] Low confidence ({confidence}), using heuristic"
                )
                return None
            ...
            collections = [
                c for c in result.get("collections", []) if c in valid_collections
            ]

            if not collections:
                logger.warning(
                    f"[IntelligentRouter] No valid collections in LLM response: {result}"
                )
                return None

            logger.info(
                f"[IntelligentRouter] LLM decision (conf={confidence:.2f}): {collections} | Reason: {result.get('reasoning', 'N/A')}"
            )
            return collections

        except json.JSONDecodeError as e:
            logger.warning(
                f"[IntelligentRouter] Failed to parse LLM JSON response: {e}"
            )
            return None
        except Exception as e:
