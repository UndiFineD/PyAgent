# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\routing_service.py
"""
...
class RoutingService:
    ...
    def preprocess_query(self, query: str) -> str:
        ...
        for word in words:
            ...
            if word_lower in self.ACRONYM_DICT:
                ...
                if expansion.lower() not in query.lower():
                    ...
                    logger.debug(
                        f"[QUERY_PREPROCESS] Expanded '{word}' -> '{expansion}'"
                    )
        ...
        if expansions_to_add:
            ...
            logger.debug(
                f"[QUERY_PREPROCESS] Enhanced query: '{query}' -> '{enhanced_query}'"
            )
    ...
    def route_query(self, query: str) -> List[str]:
        ...
        if total_score < 0.5:
            ...
            logger.info(
                f"[Routing] Exploration phase (score={total_score:.2f}): searching all collections"
            )

        elif total_score < 2.0:
            # MEDIUM CONFIDENCE: Search top 2-3 collections
            # Take top collections with score > 0.1, up to 3
            selected = [coll for coll, score in sorted_collections[:3] if score > 0.1]
            if not selected:
                selected = [sorted_collections[0][0]]  # At least take top 1
            logger.info(
                f"[Routing] Medium confidence (score={total_score:.2f}): searching {selected}"
            )

        else:
            # HIGH CONFIDENCE: Search top 1-2 collections
            # Take top collections with score > 0.5, up to 2
            selected = [coll for coll, score in sorted_collections[:2] if score > 0.5]
            if not selected:
                selected = [sorted_collections[0][0]]  # At least take top 1
            logger.info(
                f"[Routing] High confidence (score={total_score:.2f}): searching {selected}"
            )
    ...
    def _track_routing_usage(
        self, concepts: List[str], selected_collections: List[str]
    ):
            ...
            if concept in self.knowledge_graph.get("routing_patterns", {}):
                ...
                for collection in selected_collections:
                    if collection in collections_used:
                        collections_used[collection]["total"] = (
                            collections_used[collection].get("total", 0) + 1
                        )
                    else:
    ...
    def get_tier_recommendations(self, concepts: List[str]) -> Dict[str, Any]:
        ...
        if total_score < 0.5:
            ...
            top_collections = [
                coll for coll, score in sorted_collections[:3] if score > 0.1
            ]
            ...
            top_collections = [
                coll for coll, score in sorted_collections[:2] if score > 0.5
            ]
            if not top_collections:
                top_collections = [sorted_collections[0][0]]
