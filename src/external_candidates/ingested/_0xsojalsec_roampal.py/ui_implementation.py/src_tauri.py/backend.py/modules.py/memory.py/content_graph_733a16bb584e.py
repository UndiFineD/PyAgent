...
class ContentGraph:
        ...
        for entity in entities:
            ...
            if quality_score is not None:
                ...
                self.entities[entity]["avg_quality"] = (
                    self.entities[entity]["total_quality"]
                    / self.entities[entity]["mentions"]
                )
        ...
        for i in range(len(unique_entities)):
            for j in range(i + 1, len(unique_entities)):
                ...
                self.relationships[rel_id]["strength"] = round(
                    (co_occur**0.5)
                    * 2.0,  # sqrt for diminishing returns, * 2 for scaling
                    2,
                )
    ...
    def get_entity_relationships(
        self, entity: str, min_strength: float = 0.0
    ) -> List[Dict[str, Any]]:
        """
    ...
    def find_path(
        ...
        logger.debug(
            f"No path found between '{from_entity}' and '{to_entity}' (max_depth={max_depth})"
        )
        return None
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_mentions_per_entity": (
                round(
                    sum(e["mentions"] for e in self.entities.values())
                    / len(self.entities),
                    2,
                )
                if self.entities
            ...
            "avg_relationships_per_entity": (
                round((len(self.relationships) * 2) / len(self.entities), 2)
                if self.entities
                else 0.0
            ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ContentGraph":
        ...
        for name, entity_data in data.get("entities", {}).items():
            ...
            if "collections" in entity_copy:
                entity_copy["collections"] = defaultdict(
                    int, entity_copy["collections"]
                )
    ...
    def load_from_file(cls, filepath: str) -> "ContentGraph":
        ...
        try:
            ...
            logger.warning(
                f"ContentGraph file not found: {filepath}, creating new graph"
            )
            return cls()
        except Exception as e:
