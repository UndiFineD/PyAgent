# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\promotion_service.py
"""
...
class PromotionService:
    ...
    async def handle_promotion(
        ...
        if collection == "working":
            if score >= self.config.promotion_score_threshold and uses >= 2:
                return await self._promote_working_to_history(
                    doc_id, doc, metadata, score, uses
                )

        # Promotion: history -> patterns
        elif collection == "history":
            if score >= self.config.high_value_threshold and uses >= 3:
                return await self._promote_history_to_patterns(
                    doc_id, doc, metadata, score, uses
                )
    ...
    async def _promote_working_to_history(
        self, doc_id: str, doc: Dict, metadata: Dict[str, Any], score: float, uses: int
    ) -> Optional[str]:
        ...
        text_for_embedding = (
            metadata.get("text") or metadata.get("content") or doc.get("content", "")
        )
        ...
        logger.info(
            f"Promoted {doc_id} from working -> history (score: {score:.2f}, uses: {uses})"
        )
        return new_id
    ...
    async def _promote_history_to_patterns(
        ...
        logger.info(
            f"Promoted {doc_id} from history -> patterns (score: {score:.2f}, uses: {uses})"
        )
        return new_id

    async def _demote_patterns_to_history(
        self, doc_id: str, metadata: Dict[str, Any], score: float
    ) -> Optional[str]:
    ...
    async def _handle_deletion(
        self, doc_id: str, collection: str, metadata: Dict[str, Any], score: float
    ):
        ...
        if score < deletion_threshold:
            ...
            logger.info(
                f"Deleted {doc_id} from {collection} (score {score:.2f} < threshold {deletion_threshold})"
            )
        ...
        try:
            ...
            for doc_id in all_ids:
                ...
                if score >= self.config.promotion_score_threshold and uses >= 2:
                    ...
                    promoted_count += 1
                    logger.info(
                        f"Promoted {doc_id} to history (score: {score:.2f}, uses: {uses}, age: {age_hours:.1f}h)"
                    )
                ...
                # Cleanup: Remove items older than 24 hours that weren't promoted
                    ...
                    working_adapter.delete_vectors([doc_id])
                    logger.info(
                        f"Cleaned up old working memory {doc_id} (age: {age_hours:.1f}h, score: {score:.2f})"
                    )

            if promoted_count > 0:
                logger.info(
                    f"Batch promotion: checked {checked_count}, promoted {promoted_count} memories"
                )
    ...
    async def promote_item(
        ...
        try:
            ...
            if not doc:
                logger.warning(
                    f"Cannot promote {doc_id}: not found in {from_collection}"
                )
                return None
            ...
            await self.collections[to_collection].upsert_vectors(
                ids=[new_id], vectors=[await self.embed_fn(text)], metadatas=[metadata]
            )

            logger.info(
                f"Promoted {doc_id} from {from_collection} to {to_collection} as {new_id}"
            )
            return new_id
        ...
        try:
            ...
            for doc_id in all_ids:
                ...
                if age_hours > max_age_hours:
                    ...
                    cleaned_count += 1
                    logger.debug(
                        f"Cleaned up old working memory {doc_id} (age: {age_hours:.1f}h)"
                    )

            if cleaned_count > 0:
                logger.info(
                    f"Working memory cleanup: removed {cleaned_count} old items"
                )
