...
class EmbeddingService(EmbeddingServiceInterface):
    def __init__(self):
        ...
        logger.info(
            f"EmbeddingService initialized (model will load on first use): {self._model_name}"
        )
    ...
    def _load_bundled_model(self):
        ...
        try:
            ...
            if bundled_cache.exists():
                ...
                if ref_file.exists():
                    ...
                    if snapshot_path.exists():
                        logger.info(
                            f"Loading bundled embedding model from snapshot: {snapshot_path}"
                        )
            ...
            logger.warning(
                "Bundled model not found, downloading from HuggingFace (dev mode)"
            )
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        if not isinstance(text, str) or not text.strip():
            logger.warning(
                "Attempted to embed empty or non-string text. Returning zero vector."
            )
        ...
        try:
            ...
            if thread.is_alive():
                logger.error(
                    "Embedding generation timed out after 30 seconds, returning zero vector"
                )
                return [0.0] * self._embedding_dim
            ...
            if len(embedding) != self._embedding_dim:
                ...
                embedding = (
                    embedding + [0.0] * (self._embedding_dim - len(embedding))
                )[: self._embedding_dim]
