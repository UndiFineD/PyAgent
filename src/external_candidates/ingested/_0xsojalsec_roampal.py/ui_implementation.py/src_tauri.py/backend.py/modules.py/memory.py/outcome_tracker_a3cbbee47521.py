...
class OutcomeTracker:
    ...
    async def record_outcome(
        ...
        try:
            ...
            logger.info(
                f"Recorded outcome {outcome_id}: {doc_id} -> {outcome} (confidence: {confidence})"
            )
            return outcome_id
    ...
    async def record_pattern_outcome(
        ...
        if result:
            ...
            pattern_id, success_count, failure_count, partial_count, contexts_json = (
                result
            )
        ...
        logger.info(
            f"Recorded pattern outcome: {problem[:50]} -> {solution[:50]} = {outcome}"
        )
    ...
    async def record_implicit_signal(
        ...
        logger.info(
            f"Recorded implicit signal: {signal_type} (confidence: {confidence})"
        )

    async def infer_outcome_from_signals(
        self, session_id: str, lookback_messages: int = 5
    ) -> Dict[str, Any]:
        """
    ...
    async def get_best_patterns(
        self, min_attempts: int = 3, min_success_rate: float = 0.7
    ) -> List[Dict[str, Any]]:
        """
