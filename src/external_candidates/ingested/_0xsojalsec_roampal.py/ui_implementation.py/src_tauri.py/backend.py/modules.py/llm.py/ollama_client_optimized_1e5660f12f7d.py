...
class OllamaClientOptimized(LLMClientInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                f"OllamaClientOptimized initialized with enhanced pooling: {self.pool_config}"
            )

        except Exception as e:
            raise OllamaException(
                f"Failed to initialize optimized Ollama client: {e}"
            ) from e
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Ollama timeout (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s..."
                    )
    ...
    async def stream_response(
        ...
        try:
            start_time = time.time()
            accumulated_content = (
                ""  # Accumulate content to strip think tags from complete segments
            )
            in_think_tag = False
                                ...
                                if chunk:
                                    ...
                                    while True:
                                        if in_think_tag:
                                            ...
                                            end_idx = accumulated_content.find(
                                                "</think>"
                                            )
                                            if end_idx != -1:
                                                # Skip past the think content
                                                accumulated_content = (
                                                    accumulated_content[end_idx + 8 :]
                                                )
                                                in_think_tag = False
                                            else:
                                            ...
                                            if start_idx != -1:
                                                ...
                                                accumulated_content = (
                                                    accumulated_content[start_idx + 7 :]
                                                )
                                                in_think_tag = True
                                            else:
