# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\smart_book_processor.py
"""
...
class SmartBookProcessor:
    ...
    def __init__(self, data_dir: str, chromadb_adapter=None, embedding_service=None):
        """
    ...
    def _find_source_context(
        self, chunk_start_pos: int, headings: List[Dict]
    ) -> Optional[str]:
        """
    ...
    async def check_duplicate(self, title: str) -> Optional[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(str(self.db_path)) as db:
            ...
            async with db.execute(
                "SELECT book_id, title, author FROM books WHERE title = ?", (title,)
            ) as cursor:
    ...
    async def process_book_with_progress(
        ...
        try:
            ...
            if any(
                pattern in book_content.upper()
                for pattern in [p.upper() for p in suspicious_patterns]
            ):
                logger.warning(
                    f"Book '{title}' contains potential prompt injection patterns - flagging for user awareness"
                )
                # Note: Still process the book, just log the warning

            # Send initial progress
            await self._send_progress(
                task_id, "chunking", "Chunking text with semantic awareness...", 10
            )
            ...
            for i, chunk in enumerate(raw_chunks):
                ...
                chunk_pos = book_content.find(
                    chunk[:100]
                )  # Use first 100 chars to locate
                ...
                chunks_with_metadata.append(
                    {
                        ...
                        or title,  # Fallback to book title
            ...
            # Send progress
            await self._send_progress(
                task_id, "storing", f"Storing {total_chunks} chunks...", 40
            )
            ...
            # Send progress
            await self._send_progress(
                task_id, "embedding", "Generating embeddings...", 70
            )
            ...
            if self.chromadb_adapter and self.embedding_service:
                ...
                logger.warning(
                    "ChromaDB adapter or embedding service not available - skipping embeddings"
                )
    ...
    async def _store_in_sqlite(
        ...
        logger.info(
            f"Stored book '{title}' with {len(chunks_with_metadata)} chunks in SQLite"
        )
    ...
    async def _store_embeddings_with_metadata(
        ...
        try:
            ...
            async with aiosqlite.connect(str(self.db_path)) as db:
                async with db.execute(
                    "SELECT upload_timestamp FROM books WHERE book_id = ?", (book_id,)
                ) as cursor:
                    row = await cursor.fetchone()
                    if row:
            ...
            for batch_start in range(0, len(chunks_with_metadata), batch_size):
                ...
                if self.task_manager and task_id in self.task_manager.cancelled_tasks:
                    logger.info(
                        f"Task {task_id} was cancelled, stopping embedding generation"
                    )
                    return
                ...
                logger.debug(
                    f"[BOOK_PROCESSOR] Embedding {len(batch_texts)} chunks with contextual prefix"
                )
                ...
                await self.chromadb_adapter.upsert_vectors(
                    ids=chunk_ids, vectors=embeddings, metadatas=metadatas
                )
            ...
            logger.info(
                f"Stored {len(chunks_with_metadata)} embeddings with improved metadata in ChromaDB"
            )
    ...
    async def backfill_book_timestamps(self):
        ...
        try:
            if not self.chromadb_adapter:
                logger.warning(
                    "ChromaDB adapter not available, skipping timestamp backfill"
                )
                return
            ...
            async with aiosqlite.connect(str(self.db_path)) as db:
                async with db.execute(
                    "SELECT book_id, upload_timestamp FROM books"
                ) as cursor:
            ...
            for book_id, upload_timestamp in books:
                ...
                try:
                    ...
                    results = collection.get(
                        where={"book_id": book_id}, include=["metadatas"]
                    )
                    ...
                    if results and results["ids"]:
                        ...
                        logger.info(
                            f"Updated {len(chunk_ids)} chunks for book {book_id}"
                        )

                except Exception as e:
                    logger.error(
                        f"Error backfilling timestamps for book {book_id}: {e}"
                    )
                    continue
    ...
    async def _send_progress(
        self, task_id: str, status: str, message: str, progress: int, **kwargs
    ):
        ...
        try:
            ...
            await send_progress_update(
                task_id, status=status, message=message, progress=progress, **kwargs
            )
        except Exception as e:
    ...
    async def get_surrounding_chunks(
        self, chunk_id: str, radius: int = 2
    ) -> Dict[str, Any]:
        """
