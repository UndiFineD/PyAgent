# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\intent\og_intent_router.py
import logging
import re
...
BOOKS = [
    {
        ...
        "keywords": ["stoicism", "suffering", "meaning"],
    },
    {
        "title": "Clean Architecture",
        "author": "Robert C. Martin",
        "keywords": ["uncle bob", "dependency inversion", "layers"],
    },
    {
        "title": "The Art of War",
        "author": "Sun Tzu",
        "keywords": ["strategy", "know thyself", "enemy"],
    },
    {
        "title": "Man's Search for Meaning",
        "author": "Viktor Frankl",
        "keywords": ["logotherapy", "concentration camp"],
    },
    {
        "title": "Tao Te Ching",
        "author": "Lao Tzu",
        "keywords": ["wu wei", "flow", "dao"],
    },
    {
        "title": "Zero to One",
        "author": "Peter Thiel",
        "keywords": ["monopolies", "startups", "innovation"],
    },
]

BOOK_KEYWORDS = (
    [b["title"].lower() for b in BOOKS]
    + [b["author"].lower() for b in BOOKS]
    + [kw.lower() for b in BOOKS for kw in b["keywords"]]
)
...
class OGIntentRouter(IntentRouterInterface):
    ...
    def _parse_save_learning_params(self, input_after_command: str) -> Dict[str, Any]:
        ...
        keywords_match = re.search(
            r'--keywords\s+"?([^"]+)"?', remaining, re.IGNORECASE
        )
        if keywords_match:
            kws = [
    ...
    def _should_trigger_web_search(self, user_input: str) -> bool:
        ...
        for indicator in image_analysis_indicators:
            if indicator in inp_lower:
                logger.info(
                    f"OGIntentRouter: Detected image analysis context ('{indicator}'), skipping web search"
                )
                return False
        ...
        conversation_context_indicators = [
            ...
            "what did we discuss",
            "what were we saying",
            "what was our chat about",
            "what were we discussing",
            "what was our topic",
            "what were we covering",
            "what was our subject",
        ]

        for indicator in conversation_context_indicators:
            if indicator in inp_lower:
                logger.info(
                    f"OGIntentRouter: Detected conversation context query, skipping web search"
                )
                return False
        ...
        for phrase in FACTUAL_TRIGGERS:
            ...
            if phrase in inp_lower:
                logger.info(
                    f"OGIntentRouter: Triggered web search from phrase match: '{phrase}'"
                )
                return True
    ...
    def _detect_book_lookup(self, user_input: str) -> Optional[Dict[str, str]]:
        ...
        is_book_request = any(
            re.search(pattern, inp) for pattern in book_request_patterns
        )
        ...
        if is_book_request:
            ...
            if matched_kw:
                ...
                if "quote" in inp:
                    typ = "quote"
                elif "model" in inp or "concept" in inp:
                    typ = "model"
                elif "lesson" in inp or "learning" in inp or "summary" in inp:
                    typ = "summary"
                else:
                    typ = "summary"  # Default to summary if no specific type
                ...
                book = next(
                    (
                        ...
                        if any(
                            kw
                            in [b["title"].lower(), b["author"].lower()]
    ...
    async def determine_action(
        ...
        if not inp.startswith("Based on this image content:"):
            if (
                ";" in inp
                or (
                    ("1." in inp or "2." in inp or "3." in inp)
                    and len([x for x in ["1.", "2.", "3.", "4.", "5."] if x in inp])
                    >= 2
                )
                or (inp.count("?") >= 2)
            ):  # Multiple questions
                return Action(
                    type=ActionType.MULTI_PART_QUERY, parameters={"original_input": inp}
                )
        ...
        logger.info(
            f"OGIntentRouter: About to check web search triggers for: '{safe_inp}'"
        )
        if self._should_trigger_web_search(inp):
            logger.info(
                f"OGIntentRouter: Web search triggered, returning PERFORM_WEB_SEARCH action"
            )
            return Action(
            ...
            logger.info(
                f"OGIntentRouter: Web search not triggered, continuing to fallback"
            )
    ...
    async def handle_multi_part_query(self, input_text: str) -> List[Dict]:
        ...
        try:
            ...
            if not parts:
                parts = [
                    p.strip() for p in re.split(r"\d+\.", input_text) if p.strip()
                ]  # Split on numbered lists

            sub_queries = []
            for part in parts:
                sub_queries.append(
                    {"sub_query": part, "action": await self.determine_action(part)}
                )
            ...
            return [
                {
                    "sub_query": input_text,
                    "action": Action(type=ActionType.RESPOND_WITH_LLM),
                }
            ]
