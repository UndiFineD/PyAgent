# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\intent\basic_intent_router.py
# backend/modules/intent/basic_intent_router.py
import logging
import re
from typing import Any, Dict, List, Optional

from utils.text_utils import extract_entities_for_action, strip_fluff_phrases
...
class BasicIntentRouter(IntentRouterInterface):
        ...
        if config and "custom_keywords" in config:
            for action_type_str, kw_list in config["custom_keywords"].items():
                try:
                    ...
                    logger.warning(
                        f"Unknown ActionType '{action_type_str}' in custom_keywords config."
                    )

        logger.info(
            f"BasicIntentRouter initialized with {len(self.keywords)} intent patterns."
        )
    ...
    async def determine_action(
        ...
        if not self.initialized:
            logger.warning(
                "BasicIntentRouter not initialized. Defaulting to RESPOND_WITH_LLM."
            )
            return Action(
        ...
        logger.info(
            f"BasicIntentRouter - Original input: '{user_input}', Processed input for intent: '{processed_input}'"
        )
            ...
            for keyword in sorted_kw_list:
                ...
                if " " in keyword:
                    if keyword in processed_input:  # Already lowercased
                        logger.info(
                            f"Intent identified: {action_type.name} (Phrase Keyword: '{keyword}' matched in '{processed_input}')"
                        )
                        params = await extract_entities_for_action(
                            action_type, user_input, processed_input, keyword
                        )
                        if action_type == ActionType.PERFORM_WEB_SEARCH and (
                            not params or not params.get("query")
                        ):
                            params = params or {}
                            params["query"] = (
                                processed_input.split(keyword, 1)[-1].strip()
                                or processed_input
                            )
                        return Action(
                            type=action_type, parameters=params if params else None
                        )
                else:  # Single word keyword, use word boundary
                    pattern = r"\b" + re.escape(keyword) + r"\b"
                    if re.search(pattern, processed_input, re.IGNORECASE):
                        logger.info(
                            f"Intent identified: {action_type.name} (Word Keyword: '{keyword}' matched in '{processed_input}')"
                        )
                        params = await extract_entities_for_action(
                            action_type, user_input, processed_input, keyword
                        )
        ...
        logger.info(
            f"No specific keyword intent found for processed input '{processed_input}'. Defaulting to RESPOND_WITH_LLM."
        )
        return Action(
            type=ActionType.RESPOND_WITH_LLM, parameters={"original_input": user_input}
        )
    ...
    async def get_available_commands(self) -> List[str]:
        ...
        for action_type, kws in self.keywords.items():
            if action_type not in [
                ActionType.PERFORM_WEB_SEARCH,
                ActionType.RESPOND_WITH_LLM,
                ActionType.UNKNOWN_INTENT,
                ActionType.NO_ACTION,
            ]:
                example_trigger = kws[0]
                if "goal" in example_trigger:
                    example = f"{example_trigger} <your goal description>"
                elif "value" in example_trigger:
                    example = f"{example_trigger} <your value description>"
                elif "remember" in example_trigger:
                    example = f"{example_trigger} <information to remember>"
                else:
