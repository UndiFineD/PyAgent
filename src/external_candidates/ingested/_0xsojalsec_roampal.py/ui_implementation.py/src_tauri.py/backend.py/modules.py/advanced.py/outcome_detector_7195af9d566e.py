# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\advanced\outcome_detector.py
"""
...
class OutcomeDetector:
    ...
    def __init__(self, llm_service=None):
        ...
        if not llm_service:
            logger.warning(
                "OutcomeDetector initialized without LLM service - outcome detection will not work"
            )
...
{{
    ...
    "reasoning": "brief why"
            ...
            if all(
                k in result
                for k in ["outcome", "confidence", "indicators", "reasoning"]
            ):
                return result
