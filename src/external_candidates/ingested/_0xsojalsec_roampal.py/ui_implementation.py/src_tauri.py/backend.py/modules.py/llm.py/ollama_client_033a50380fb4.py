# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\llm\ollama_client.py
...
logger.info(
    "[OLLAMA MODULE LOADED] This module has the temporary fix to use /api/generate for all models"
)
...
class OllamaClient(LLMClientInterface):
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        self.request_timeout = config.get(
            "ollama_request_timeout_seconds", self.request_timeout
        )
        self.keep_alive_seconds = config.get(
            "ollama_keep_alive_seconds", self.keep_alive_seconds
        )

        if not self.model_name:
            raise ValueError(
                "Ollama 'ollama_model' name must be provided in the configuration."
            )

        try:
            # Standard httpx client without special limits - deepseek-r1 doesn't work with conservative settings
            timeout_config = httpx.Timeout(
                self.request_timeout, connect=30, read=self.request_timeout, write=30
            )
            ...
            raise OllamaException(
                f"Failed to initialize httpx.AsyncClient for Ollama: {e}"
            ) from e
    ...
    async def generate_response(
        ...
        logger.debug(
            f"Using model: {actual_model} (requested: {model}, default: {self.model_name})"
        )
        ...
        if self.api_style == "openai":
            ...
            try:
                logger.debug(
                    f"Using OpenAI-style API at {self.base_url}/v1/chat/completions"
                )
        ...
        for i, msg in enumerate(messages):
            logger.info(
                f"[DEBUG PAYLOAD] Message {i}: role={msg.get('role')}, content_length={len(str(msg.get('content', '')))}"
            )
            if i == len(messages) - 1:  # Log last message content
                logger.info(
                    f"[DEBUG PAYLOAD] Last message content: {str(msg.get('content', ''))[:200]}"
                )
        ...
        options["num_gpu"] = (
            99  # Force max GPU layers - Ollama is too conservative by default
        )
        ...
        try:
            ...
            if not use_generate:
                ...
                try:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if api_response.status_code == 404:
                                logger.warning(
                                    f"Chat endpoint returned 404 for model: {actual_model}, falling back to generate"
                                )
                                use_generate = True
                                break
                            else:
                                ...
                                logger.debug(
                                    f"Chat endpoint successful for model: {actual_model}"
                                )
                    ...
                    logger.error(
                        f"[CHAT ENDPOINT ERROR] Chat failed for {actual_model}: Type={type(chat_error).__name__}, Error={chat_error}"
                    )
                    logger.error(
                        f"[CHAT ENDPOINT ERROR] Full traceback:", exc_info=True
                    )
                    logger.warning(
                        f"Chat endpoint failed for {actual_model}: {chat_error}, falling back to generate"
                    )
                    use_generate = True
            ...
            if use_generate:
                ...
                if not prompt_text or not prompt_text.strip():
                    logger.warning(
                        f"Empty prompt detected, using fallback: {prompt[:100]}"
                    )
                ...
                logger.critical(
                    f"[GENERATE FALLBACK] Trying /api/generate with model={actual_model}, prompt length={len(prompt_text)}"
                )
                logger.critical(
                    f"[GENERATE PROMPT] First 200 chars: {prompt_text[:200]}"
                )
                ...
                try:
                    ...
                    logger.critical(
                        f"[DEBUG] Raw response keys: {response_json.keys() if response_json else 'None'}"
                    )
                    logger.critical(
                        f"[DEBUG] Response 'response' field: {response_json.get('response', 'MISSING')[:200] if response_json else 'No JSON'}"
                    )
                    ...
                    logger.critical(
                        f"[GENERATE SUCCESS - SYNC] Generate endpoint worked for model: {actual_model}"
                    )
                    ...
                    api_response = await self.client.post(
                        "/api/generate", json=generate_payload
                    )
                    api_response.raise_for_status()
                    logger.critical(
                        f"[GENERATE SUCCESS - ASYNC] Generate endpoint worked for model: {actual_model}"
                    )

            if not api_response:
                raise OllamaException(
                    f"No valid response from either /api/chat or /api/generate for model {actual_model}"
                )
            ...
            if "message" in response_data:
                logger.info(
                    f"[DEBUG RESPONSE] Message keys: {response_data['message'].keys() if response_data['message'] else 'None'}"
                )
                logger.info(
                    f"[DEBUG RESPONSE] Message content type: {type(response_data['message'].get('content')) if response_data['message'] else 'None'}"
                )
                logger.info(
                    f"[DEBUG RESPONSE] Message content: {response_data['message'].get('content', '')[:100] if response_data['message'] else 'None'}"
                )
            ...
            logger.info(
                f"[DEBUG OLLAMA] Raw response from Ollama (length: {len(response_text) if response_text else 0}): {response_text[:500] if response_text else '(empty)'}"
            )
            ...
            response_text = self._clean_model_artifacts(
                response_text, actual_model, strip_thinking=strip_thinking
            )
            logger.info(
                f"[DEBUG OLLAMA] After cleaning (length: {len(response_text) if response_text else 0}): {response_text[:200] if response_text else '(empty)'}"
            )
    ...
    async def generate_response_with_tools(
        ...
        try:
            ...
            strip_thinking = format == "json"
            content = self._clean_model_artifacts(
                content, actual_model, strip_thinking=strip_thinking
            )
    ...
    def _clean_model_artifacts(
        self, text: str, model: str, strip_thinking: bool = False
    ) -> str:
    ...
    async def stream_response(
        ...
        if self.api_style == "openai":
            ...
            try:
                logger.debug(
                    f"Streaming from OpenAI-style API at {self.base_url}/v1/chat/completions"
                )
        ...
        try:
            async with self.client.stream(
                "POST", "/api/chat", json=payload, timeout=self.request_timeout
            ) as response:
                response.raise_for_status()
                async for line in response.aiter_lines():
                    if not line.strip():
                    ...
                    try:
                        ...
                        if "error" in chunk_data:
                            raise OllamaException(
                                f"Ollama API stream error: {chunk_data['error']}"
                            )
    ...
    async def stream_response_with_tools(
        ...
        logger.info(
            f"[DEBUG MESSAGES] Message count: {len(messages)}, Roles: {[m['role'] for m in messages]}"
        )
        if messages:
            logger.info(
                f"[DEBUG MESSAGES] Last 200 chars of last message: {messages[-1]['content'][-200:]}"
            )
        ...
        NATIVE_TOOL_MODELS = [
            ...
            "llama3.2",
            "llama-3.1",
            "llama-3.2",  # Meta Llama variants
            "qwen",
            "qwen2",
            "qwen2.5",  # Alibaba Qwen family - excellent tool support
            "mistral",
            "mixtral",  # Mistral AI family
            "command-r",
            "command-r-plus",  # Cohere models
            "phi-4",
        ...
        supports_native_tools = any(
            native_model in model_lower for native_model in NATIVE_TOOL_MODELS
        )
        ...
        logger.info(
            f"[STREAM WITH TOOLS] Set context window to {num_ctx} for {actual_model} (num_gpu=99)"
        )
        ...
        if tools and supports_native_tools:
            payload["tools"] = tools
            logger.info(
                f"[STREAM WITH TOOLS] {len(tools)} tools passed to {actual_model} (native support)"
            )
            logger.debug(f"[STREAM WITH TOOLS] Tools: {json.dumps(tools, indent=2)}")
        elif tools:
            logger.warning(
                f"[STREAM WITH TOOLS] Model {actual_model} doesn't support native tools - tools disabled for this request"
            )
        ...
        try:
            ...
            if self.api_style == "openai":
                ...
                logger.debug(
                    f"Using OpenAI-style streaming API at {self.base_url}/v1/chat/completions"
                )
                ...
                if tools:
                    ...
                    logger.info(
                        f"[OPENAI PAYLOAD] Added {len(tools)} tools to OpenAI payload"
                    )
                    ...
                    logger.info(
                        f"[OPENAI PAYLOAD] System message length: {sys_msg_len} chars, Total messages: {len(messages)}"
                    )
                    logger.info(
                        f"[OPENAI PAYLOAD] Message roles: {[m['role'] for m in messages]}"
                    )
                    for idx, msg in enumerate(messages):
                        ...
                        logger.info(
                            f"[OPENAI PAYLOAD] Message {idx} ({msg['role']}): {content_preview}..."
                        )
                    # Dump full payload for debugging (excluding large tool definitions)
                    debug_payload = {**openai_payload}
                    debug_payload["tools"] = (
                        f"[{len(tools)} tools]"  # Replace with count
                    )
                    logger.info(
                        f"[OPENAI PAYLOAD] Full request: {json.dumps(debug_payload, indent=2)}"
                    )
                else:
                    logger.warning(
                        f"[OPENAI PAYLOAD] NO TOOLS in payload! tools parameter was: {tools}"
                    )

                try:
                    response_stream = self.client.stream(
                        "POST",
                        "/v1/chat/completions",
                        json=openai_payload,
                        timeout=self.request_timeout,
                    ...
                    response.raise_for_status()
                    logger.info(
                        f"[STREAM DEBUG] OpenAI-style Response status: {response.status_code}"
                    )
                    ...
                    if line.startswith("data: "):
                        ...
                        if data_str == "[DONE]":
                            ...
                            if tool_call_accumulator:
                                logger.info(
                                    f"[OPENAI STREAM] Yielding {len(tool_call_accumulator)} accumulated tool calls"
                                )
                                tool_calls = []
                                for idx, tc in tool_call_accumulator.items():
                                    try:
                                        ...
                                        logger.error(
                                            f"[OPENAI STREAM] Failed to parse tool arguments: {tc['arguments'][:100]}... Error: {e}"
                                        )
                        ...
                        try:
                            ...
                            if "error" in chunk:
                                ...
                                logger.error(
                                    f"[OPENAI STREAM] Server returned error: {error_msg}"
                                )

                                # Provide helpful context-specific error messages
                                if "context" in error_msg.lower() and (
                                    "overflow" in error_msg.lower()
                            ...
                            if line_count <= 3:
                                logger.info(
                                    f"[OPENAI STREAM] Chunk {line_count}: delta keys = {list(delta.keys())}, finish_reason = {finish_reason}"
                                )
                                # Log raw chunk for debugging empty responses
                                logger.info(
                                    f"[OPENAI STREAM] Raw chunk {line_count}: {data_str[:500]}"
                                )
                            ...
                            # LM Studio doesn't support enable_thinking=False, so we filter here
                                ...
                                if not in_reasoning_mode:
                                    ...
                                    logger.debug(
                                        f"[OPENAI STREAM] Discarding reasoning_content (thinking mode)"
                                    )
                            ...
                            # Some models use non-standard field names
                                ...
                                for key in [
                                    ...
                                    if key in delta and delta[key]:
                                        logger.warning(
                                            f"[OPENAI STREAM] Found content in non-standard field '{key}'"
                                        )
                            ...
                            if finish_reason == "tool_calls" and tool_call_accumulator:
                                logger.info(
                                    f"[OPENAI STREAM] finish_reason=tool_calls, yielding {len(tool_call_accumulator)} tool calls"
                                )
                                tool_calls = []
                                for idx, tc in tool_call_accumulator.items():
                                    try:
                                        ...
                                        logger.info(
                                            f"[OPENAI STREAM] Tool call: {tc['name']}({tc['arguments']})"
                                        )
                                    except json.JSONDecodeError as e:
                                        logger.error(
                                            f"[OPENAI STREAM] Failed to parse tool arguments: {tc['arguments'][:100]}... Error: {e}"
                            ...
                            logger.warning(
                                f"[STREAM DEBUG] Failed to parse OpenAI chunk: {data_str[:100]}... Error: {e}"
                            )
                            continue

                logger.info(
                    f"[STREAM DEBUG] OpenAI stream finished: {line_count} lines, {yield_count} yields"
                )
                return
            # ==================== END MULTI-PROVIDER ROUTING ====================
            ...
            try:
                ...
                if e.response.status_code == 500:
                    ...
                    if "terminated" in error_str.lower():
                        logger.warning(
                            f"⚠️ OOM detected (context: {original_ctx}), retrying with 2048"
                        )
                        ...
                        response.raise_for_status()
                        logger.info(
                            f"[STREAM DEBUG] OOM recovery successful, Response status: {response.status_code}"
                        )
            ...
            async for line in response.aiter_lines():
                ...
                try:
                    chunk_data = json.loads(line)
                    logger.debug(
                        f"[STREAM DEBUG] Parsed chunk keys: {chunk_data.keys()}"
                    )
                    ...
                    if "message" in chunk_data:
                        ...
                        if "content" in message:
                            ...
                            if (
                                ...
                                logger.info(
                                    f"[OLLAMA THINK DEBUG] Raw chunk from Ollama: '{content}'"
                                )

                            logger.debug(
                                f"[STREAM DEBUG] Yielding text chunk (length: {len(content)})"
                            )
                            yield {"type": "text", "content": content}
                            yield_count += 1
                        else:
                            logger.debug(
                                f"[STREAM DEBUG] Message has no 'content' field"
                            )
                    ...
                    logger.warning(
                        f"[STREAM DEBUG] Malformed JSON at line {line_count}: {line[:200]}"
                    )
                    logger.warning(f"[STREAM DEBUG] JSON error: {e}")

            logger.info(
                f"[STREAM DEBUG] Stream complete - processed {line_count} lines, yielded {yield_count} chunks"
            )
            yield {"type": "done"}

        except Exception as e:
            raise OllamaException(
                f"Ollama stream_response_with_tools failed: {e}"
            ) from e
