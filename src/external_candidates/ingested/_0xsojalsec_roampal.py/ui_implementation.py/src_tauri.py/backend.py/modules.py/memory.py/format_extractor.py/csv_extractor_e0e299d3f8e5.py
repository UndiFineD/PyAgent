# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\format_extractor\csv_extractor.py
"""
...
class CSVExtractor(BaseExtractor):
    ...
    def extract(self, file_path: Path) -> ExtractedDocument:
        ...
        logger.info(
            f"Extracted {len(df)} rows x {len(df.columns)} cols from {file_path.name}"
        )
