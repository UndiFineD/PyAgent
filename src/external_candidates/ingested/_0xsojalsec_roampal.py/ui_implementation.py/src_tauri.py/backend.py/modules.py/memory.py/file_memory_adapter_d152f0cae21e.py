...
class FileMemoryAdapter(MemoryAdapterInterface):
    ...
    async def initialize(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not base_data_path_str:
            raise ValueError(
                "'base_data_path' must be provided in FileMemoryAdapter config."
            )
        ...
        logger.info(
            f"Session logs will be written to and read from: {self.sessions_dir}"
        )
    ...
    async def update_session_title(self, session_id: str, title: str) -> None:
        ...
        if not session_file.exists():
            logger.warning(
                f"Cannot update title: session file {session_id} does not exist"
            )
            return
        ...
        try:
            ...
            def update_title_sync():
                ...
                if not lines:
                    logger.warning(
                        f"Cannot update title: session file {session_id} is empty"
                    )
                    return
            ...
            logger.error(
                f"Failed to update title for session {session_id}: {e}", exc_info=True
            )
    ...
    async def store_arbitrary_data(self, key: str, data: Any) -> bool:
        ...
        safe_key = (
            "".join(c if c.isalnum() or c in ("_", "-") else "_" for c in key) + ".json"
        )
    ...
    async def add_learning(
        self, learning_content: Union[str, Dict[str, Any]], **kwargs
    ) -> Dict[str, Any]:
        raise NotImplementedError
