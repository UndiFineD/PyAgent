...
class OutcomeService:
    ...
    async def record_outcome(
        ...
        if doc and collection_name and self.kg_service:
            ...
            if context and "quiz_question" in context:
                ...
                problem_text = (
                    metadata.get("query", "") or metadata.get("text", "")[:200]
                )

            if problem_text:
                await self.kg_service.update_kg_routing(
                    problem_text, collection_name, outcome
                )
                logger.info(
                    f"[KG] Updated routing for '{problem_text[:50]}' -> {collection_name} (outcome={outcome})"
                )
        ...
        if outcome == "worked" and context:
            ...
            reasons.append(
                {"reason": failure_reason, "timestamp": datetime.now().isoformat()}
            )
    ...
    def _calculate_score_update(
        self, outcome: str, current_score: float, uses: int, time_weight: float
    ) -> tuple:
        """
    ...
    async def _update_kg_with_outcome(
        ...
        if outcome == "worked" and problem_text and solution_text:
            ...
            self.kg_service.add_solution_pattern(
                doc_id, solution_text, new_score, [problem_key], solution_concepts[:5]
            )
    ...
    async def _track_problem_solution(
        ...
        try:
            ...
            logger.info(
                f"Tracked problem→solution: {problem_signature[:30]}... -> {doc_id}"
            )
        ...
        for coll_name, adapter in self.collections.items():
            if doc_id.startswith(coll_name):
                ...
                if doc:
                    ...
                    worked = sum(
                        1 for o in outcome_history if o.get("outcome") == "worked"
                    )
                    failed = sum(
                        1 for o in outcome_history if o.get("outcome") == "failed"
                    )
                    partial = sum(
                        1 for o in outcome_history if o.get("outcome") == "partial"
                    )
