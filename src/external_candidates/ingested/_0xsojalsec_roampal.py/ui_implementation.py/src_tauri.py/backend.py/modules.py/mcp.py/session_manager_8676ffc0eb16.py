# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\mcp\session_manager.py
"""
MCP Session Manager
...
class MCPSessionManager:
    ...
    def __init__(self, data_path: Path, memory_system):
        ...
        logger.info(
            f"[MCP] Session manager initialized with automatic outcome detection"
        )
    ...
    async def record_exchange(
        ...
        """
        logger.info(
            f"[MCP] DEBUG: record_exchange() called for session {session_id}, outcome={outcome}"
        )
        ...
        prev_outcome_info = {"outcome": None, "score_change": None}
                ...
                try:
                    ...
                    if outcome_result.get("outcome") in ["worked", "failed", "partial"]:
                        ...
                        for collection_name in ["working", "history", "patterns"]:
                            try:
                                prev_doc = self.memory.collections[
                                    collection_name
                                ].get_fragment(prev_doc_id)
                                if prev_doc:
                                    break
                            except:
                                continue
                        ...
                        if prev_doc:
                            ...
                            await self.memory.record_outcome(
                                doc_id=prev_doc_id, outcome=outcome_result["outcome"]
                            )
                            ...
                            for collection_name in ["working", "history", "patterns"]:
                                try:
                                    updated_doc = self.memory.collections[
                                        collection_name
                                    ].get_fragment(prev_doc_id)
                                    if updated_doc:
                            ...
                            if updated_doc:
                                new_score = updated_doc.get("metadata", {}).get(
                                    "score", old_score
                                )
                                outcome_info = {
                            ...
                            logger.warning(
                                f"[MCP] Could not find previous document {prev_doc_id} for scoring"
                            )

                except Exception as e:
                    logger.error(
                        f"[MCP] Error in automatic outcome detection: {e}",
                        exc_info=True,
                    )
                ...
                for line in f:
                    try:
                        ...
                        if entry.get("doc_id") == doc_id:
                            ...
                            logger.info(
                                f"[MCP] Exchange with doc_id {doc_id} already exists, skipping append"
                            )
                            break
        ...
        if not exchange_exists:
            logger.info(
                f"[MCP] DEBUG: Opening file to append new exchange: {session_file}"
            )
    ...
    async def get_last_assistant_with_doc_id(
        self, session_id: str
