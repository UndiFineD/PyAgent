# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\chromadb_adapter.py
import logging
import os
import sys
...
class ChromaDBAdapter:
    ...
    def __init__(
        ...
        )  # Keep for compatibility but not used in server mode
    ...
    def _clean_old_folders(self):
        ...
        for entry in os.scandir(self.db_path):
            if (
                entry.is_dir() and len(entry.name) == 36 and entry.name.count("-") == 4
            ):  # UUID pattern
                ...
                for attempt in range(3):  # Retry 3 times
                    try:
                        ...
                        if attempt < 2:  # Wait and retry
                            ...
                            continue
                        logger.warning(
                            f"Failed to delete old folder {entry.name} after retries: {e}"
                        )
    ...
    async def initialize(
        ...
        if self.client is None:
            if self.use_server:
                ...
                logger.info(
                    f"ChromaDB client initialized for local path: {self.db_path}"
                )
        ...
        if not self.use_server and self.collection.count() == 0:
            logger.info(
                f"Empty collection '{collection_name}' initialized in embedded mode"
            )
    ...
    async def _ensure_initialized(self):
        if self.collection is None and not self._auto_init_lock:
            ...
            await self.initialize(
                collection_name=self.collection_name or DEFAULT_COLLECTION_NAME
            )
            self._auto_init_lock = False
    ...
    async def upsert_vectors(
        ...
        logger.info(
            f"Upserting {len(ids)} vectors into collection '{self.collection.name}'..."
        )
        try:
            ...
            for metadata in metadatas:
                ...
                doc = metadata.get(
                    "content", metadata.get("text", metadata.get("original_text", ""))
                )
    ...
    async def query_vectors(
        ...
        try:
            ...
            if self.client and self.collection_name:
                self.collection = self.client.get_or_create_collection(
                    name=self.collection_name, metadata={"hnsw:space": "l2"}
                )

            if self.collection and self.collection.count() == 0:
                logger.debug(
                    f"[ChromaDB] Collection '{self.collection_name}' is empty, returning empty results"
                )
                return []
            ...
            try:
                ...
                logger.warning(
                    f"[ChromaDB] Failed to convert query vector to floats: {e}"
                )
                return []

            logger.info(
                f"Querying for top {top_k} vectors in collection '{self.collection_name}'..."
            )

            # Perform query with error handling
            try:
                results = self.collection.query(
                    query_embeddings=[query_vector], n_results=top_k, where=filters
                )
            except Exception as e:
                ...
                for i in range(len(ids)):
                    try:
                        ...
                        if result_embedding is None:
                            logger.warning(
                                f"[ChromaDB] Unexpected embeddings type: {type(result_embedding)}"
                            )
                            continue
    ...
    async def hybrid_query(
        ...
        vector_results = await self.query_vectors(
            query_vector, top_k=top_k * 2, filters=filters
        )
        ...
        try:
            ...
            final_results = sorted(
                merged.values(), key=lambda x: x.get("rrf_score", 0.0), reverse=True
            )[:top_k]

            logger.debug(
                f"[HYBRID] Merged {len(vector_results)} vector + {len(bm25_results)} BM25 → {len(final_results)} results"
            )
            return final_results

        except Exception as e:
            logger.warning(
                f"[HYBRID] Hybrid search failed, falling back to vector only: {e}"
            )
            return vector_results[:top_k]
    ...
    async def get_vectors_by_ids(self, ids: List[str]) -> Dict[str, Any]:
        await self._ensure_initialized()
        try:
    ...
    def get_all_vectors(self) -> List[Dict[str, Any]]:
        ...
        for i in range(len(ids)):
            ...
            if isinstance(vectors, (list, tuple)) and len(vectors) > i:
                ...
                and hasattr(vectors, "__getitem__")
                and len(vectors) > i
            ):
                vector = vectors[i]
            metadata = (
                metadatas[i]
                if isinstance(metadatas, (list, tuple)) and len(metadatas) > i
                else {}
            )
            out.append(
                {
    ...
    def get_fragment(self, fragment_id: str) -> Optional[Dict[str, Any]]:
        if self.collection is None:
            raise RuntimeError(
                "ChromaDB collection not initialized. Cannot get fragment."
            )
        result = self.collection.get(
            ids=[fragment_id], include=["embeddings", "metadatas", "documents"]
        )
        ...
        if isinstance(embeddings, (list, tuple)) and len(embeddings) > 0:
            ...
            hasattr(embeddings, "__len__")
            and hasattr(embeddings, "__getitem__")
        ...
        metadata = (
            metadatas[0]
            if isinstance(metadatas, (list, tuple)) and len(metadatas) > 0
            else {}
        )
        documents = result.get("documents", [])
        content = (
            documents[0]
            if isinstance(documents, (list, tuple)) and len(documents) > 0
            else ""
        )
        return {
            "id": result["ids"][0],
    ...
    def update_fragment_metadata(
        self, fragment_id: str, metadata_updates: Dict[str, Any]
    ):
        if self.collection is None:
            raise RuntimeError("ChromaDB collection not initialized")
        ...
        if not frag:
            logger.warning(
                f"update_fragment_metadata: No fragment with id={fragment_id}"
            )
            return
        ...
        self.collection.upsert(
            ids=[fragment_id], embeddings=[frag.get("vector")], metadatas=[metadata]
        )
