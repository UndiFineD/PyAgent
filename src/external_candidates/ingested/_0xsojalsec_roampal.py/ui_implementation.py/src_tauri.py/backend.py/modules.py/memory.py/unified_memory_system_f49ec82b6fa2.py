...
class UnifiedMemorySystem:
    ...
    def _init_services(self):
        ...
        self._routing_service = RoutingService(
            kg_service=self._kg_service, config=self.config
        )
...
Session type (1-2 words only):"""
            ...
            try:
                ...
                for result in results:
                    ...
                    if self._search_service and doc_id:
                        effectiveness = self._search_service.get_doc_effectiveness(
                            doc_id
                        )
    ...
    async def get_cold_start_context(self, limit: int = 5) -> Optional[str]:
        ...
        try:
            history_results = await self.search(
                query="recent conversation context", collections=["history"], limit=1
            )
    ...
    def _format_cold_start_results(self, results: List[Dict]) -> Optional[str]:
        ...
        memory_bank_items = [
            r for r in safe_results if r.get("source") == "memory_bank"
        ]
    ...
    async def _update_kg_routing(
        self, query: str, collection: str, outcome: str
    ) -> None:
