# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\format_extractor\pdf_extractor.py
"""
    ...
    def extract(self, file_path: Path) -> ExtractedDocument:
        ...
        try:
            ...
            if pages_with_text == 0:
                warnings.append(
                    "No text extracted - PDF may be scanned/image-only (OCR not supported)"
                )
            ...
            logger.info(
                f"Extracted {len(content)} chars from {pages_with_text}/{total_pages} pages of {file_path.name}"
            )
