...
class ContextService:
            ...
            for doc_id in past_solutions[:2]:
                ...
                for coll_name in ["patterns", "history"]:
                    ...
                    if doc:
                        ...
                        if (
                            score >= self.config.promotion_score_threshold
                            and last_outcome == "worked"
                        ):
    ...
    def _detect_topic_continuity(
        ...
        last_messages = [
            msg for msg in recent_conversation[-3:] if msg.get("role") == "user"
        ]
    ...
    async def _detect_repetition(
        self, current_message: str, conversation_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def get_context_summary(self, context: Dict[str, Any]) -> str:
        ...
        if patterns:
            parts.append(
                f"Found {len(patterns)} relevant pattern(s) from past conversations"
            )
