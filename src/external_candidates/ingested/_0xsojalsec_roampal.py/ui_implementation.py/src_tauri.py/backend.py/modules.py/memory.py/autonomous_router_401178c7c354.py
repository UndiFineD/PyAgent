...
class AutonomousRouter:
    ...
    def __init__(self, knowledge_graph: Dict, llm_service=None, cache_ttl: int = 300):
        ...
        self._routing_cache: Dict[str, tuple] = (
            {}
        )  # query_hash -> (collections, timestamp, reasoning)
    ...
    async def route_query(self, query: str, concepts: List[str]) -> List[str]:
        ...
        if self.llm_service:
            try:
                ...
                logger.info(
                    f"[AutonomousRouter] LLM: {collections} | {reasoning[:100]}"
                )
                return collections
...
{{
    ...
    def _log_decision(
        self, query: str, collections: List[str], llm_result: Dict
    ) -> str:
    ...
    def record_routing_outcome(
        self, query: str, worked: bool, user_feedback: Optional[str] = None
    ):
        """
        ...
        for decision_id, decision in self.routing_outcomes.items():
            if decision["query"] == query and decision["outcome"] is None:
                ...
                total = (
                    self.llm_accuracy_stats["successful_outcomes"]
                    + self.llm_accuracy_stats["failed_outcomes"]
                )
                if total > 0:
                    self.llm_accuracy_stats["accuracy_rate"] = (
                        self.llm_accuracy_stats["successful_outcomes"] / total
                    )
    ...
    def _analyze_failure(self, decision: Dict):
        ...
        self.knowledge_graph["routing_failures"] = self.knowledge_graph[
            "routing_failures"
        ][-50:]
