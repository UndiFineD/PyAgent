...
class MemoryBankService:
    ...
    async def store(
        ...
        await self.collection.upsert_vectors(
            ids=[doc_id], vectors=[embedding], metadatas=[metadata]
        )
    ...
    async def update(
        ...
        old_text = old_doc.get("metadata", {}).get(
            "content", old_doc.get("metadata", {}).get("text", "")
        )
    ...
    def list_all(
        self, include_archived: bool = False, tags: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
