# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\memory\search_service.py
"""
...
from .routing_service import ALL_COLLECTIONS, RoutingService
...
class SearchService:
    ...
    async def search(
        ...
        known_solutions = await self.kg_service.find_known_solutions(
            query, self._get_fragment
        )

        # Special handling for empty query - return all items
        if not query or query.strip() == "":
            return await self._search_all(collections, limit, offset, return_metadata)
        ...
        if known_solutions:
            ...
            unique_known = [
                s for s in known_solutions if s.get("id") not in existing_ids
            ]
        ...
        if self.reranker and len(all_results) > limit * 2:
            all_results = await self._rerank_with_cross_encoder(
    ...
    async def _search_all(
        self, collections: List[str], limit: int, offset: int, return_metadata: bool
    ) -> Union[List[Dict], Dict]:
        ...
        for coll_name in collections:
            ...
            try:
                ...
                for i in range(len(items["ids"])):
                    metadata = (
                        items["metadatas"][i] if i < len(items["metadatas"]) else {}
                    )
                    result = {
                        "id": items["ids"][i],
                        "content": (
                            items["documents"][i] if i < len(items["documents"]) else ""
                        ),
                        "text": (
                            items["documents"][i] if i < len(items["documents"]) else ""
                        ),
    ...
    def _apply_collection_boost(self, result: Dict, coll_name: str, query: str):
        ...
        # Memory bank: boost by importance * confidence
            ...
            result["distance"] = (
                result.get("distance", 1.0) * metadata_boost / entity_boost
            )
    ...
    def _calculate_entity_boost(self, query: str, doc_id: str) -> float:
        ...
        try:
            ...
            doc_entities = self.kg_service.content_graph._doc_entities.get(
                doc_id, set()
            )
            ...
            for entity in query_entities:
                if (
                    entity in doc_entities
                    and entity in self.kg_service.content_graph.entities
                ):
                    entity_quality = self.kg_service.content_graph.entities[entity].get(
                        "avg_quality", 0.0
                    )
    ...
    def get_doc_effectiveness(self, doc_id: str) -> Optional[Dict[str, Any]]:
        ...
        for key, stats in self.kg_service.knowledge_graph.get(
    ...
    async def _rerank_with_cross_encoder(
        ...
        try:
            ...
            top_candidates = sorted(
                candidates, key=lambda x: x.get("final_rank_score", 0.0), reverse=True
            )[:30]
            ...
            for i, candidate in enumerate(top_candidates):
                ...
                if collection == "memory_bank":
                    ...
                    blended = (
                        (1 - ce_weight) * original_score + ce_weight * ce_norm
                    ) * quality_boost
                else:
                    ce_norm = (ce_score + 1) / 2
                    ce_weight = 0.4
            ...
            top_candidates.sort(
                key=lambda x: x.get("final_rank_score", 0.0), reverse=True
            )
    ...
    def _track_search_results(
        self, query: str, results: List[Dict], transparency_context: Optional[Any]
    ):
        ...
        if transparency_context and hasattr(
            transparency_context, "track_memory_search"
        ):
            confidence_scores = [
                math.exp(-r.get("distance", 0.5) / 100.0) for r in results
            ]
            transparency_context.track_memory_search(
                query=query,
