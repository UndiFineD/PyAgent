# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\modules\prompt\prompt_engine.py
import logging
...
from core.interfaces.prompt_engine_interface import PromptContext, PromptEngineInterface
...
class PromptEngine(PromptEngineInterface):
    ...
    async def initialize(
        ...
        try:
            ...
            logger.info(
                f"PromptEngine initialized. Template directories: {self.template_dirs}, Default system prompt: '{self.default_system_prompt_name}.txt'"
            )
    ...
    async def _load_template_jinja(self, template_name_with_ext: str) -> Template:
        ...
        logger.info(
            f"TEMPLATE DEBUG: Searching for '{template_name_with_ext}' in directories: {self.template_dirs}"
        )
        ...
        logger.info(
            f"TEMPLATE DEBUG: Template '{template_name_with_ext}' found in: {found_in_dirs}"
        )
        ...
        try:
            ...
            logger.info(
                f"TEMPLATE DEBUG: Jinja2 loaded '{template_name_with_ext}' - template object: {template}"
            )
            return template
        except TemplateNotFound:
            logger.error(
                f"Jinja2 template file '{template_name_with_ext}' not found in any of {self.template_dirs}."
            )
            raise FileNotFoundError(
                f"Prompt template file '{template_name_with_ext}' not found."
            )
        except TemplateSyntaxError as e_syntax:
            logger.error(
                f"Syntax error in Jinja2 template '{template_name_with_ext}': {e_syntax.message} (line {e_syntax.lineno})",
                exc_info=True,
            )
            raise
        except Exception as e:
            logger.error(
                f"Error loading Jinja2 template '{template_name_with_ext}': {e}",
                exc_info=True,
            )
            raise
    ...
    async def build_prompt(
        ...
        if not self.jinja_env:
            logger.error(
                "PromptEngine not initialized for build_prompt. Call initialize() first."
            )
            raise RuntimeError("PromptEngine not initialized.")

        effective_system_prompt_name_no_ext = (
            system_prompt_name
            if system_prompt_name
            else self.default_system_prompt_name
        )
        ...
        try:
            system_template = await self._load_template_jinja(
                effective_system_prompt_filename
            )
            ...
            if isinstance(context, dict):
                ...
                render_context_dict = (
                    context.__dict__.copy() if hasattr(context, "__dict__") else {}
                )
            render_context_dict["user_input"] = user_input
            render_context_for_jinja = {
                k: v for k, v in render_context_dict.items() if v is not None
            }
            logger.debug(
                f"Context for Jinja rendering (keys): {list(render_context_for_jinja.keys())}"
            )
            ...
            if "system_prompt" in render_context_for_jinja:
                ...
                logger.debug(
                    f"System prompt passed to template (length {len(system_prompt_content)})"
                )
            else:
                logger.debug("No system_prompt in context - using default template")

            final_prompt_str = await system_template.render_async(
                **render_context_for_jinja
            )
            logger.debug(f"Template output generated (length: {len(final_prompt_str)})")
            logger.info(
                f"Final prompt generated via Jinja2 (length: {len(final_prompt_str)}) using system template '{effective_system_prompt_filename}'."
            )
            return final_prompt_str.strip()
        except FileNotFoundError:
            logger.error(
                f"System prompt template '{effective_system_prompt_filename}' not found in template dirs. Using hardcoded minimal prompt."
            )
            return f"System: You are a helpful assistant. Please assist the user with their query about: {user_input}"
        except Exception as e:
            logger.error(
                f"Error building prompt with Jinja2 template '{effective_system_prompt_filename}': {type(e).__name__} - {e}",
                exc_info=True,
            )
    ...
    async def add_template_directory(self, directory: str) -> None:
        ...
        if str(new_dir) not in [str(d) for d in self.template_dirs]:
            ...
            logger.info(
                f"PRIORITY FIX: Added template directory at HIGHEST priority: {new_dir}. Directory order: {self.template_dirs}"
            )
        else:
