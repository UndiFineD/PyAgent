# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\create_vector_index.py
import argparse
import asyncio
import logging
import sys
...
def is_valid_document(doc: Dict[str, Any], text_key: str) -> bool:
    ...
    if any(phrase in lower_text for phrase in GARBAGE_PHRASES):
        logger.warning(
            f"Skipping noisy document with ID {doc.get('chunk_id') or doc.get('quote_id') or doc.get('model_id')}: '{text[:50]}...'"
        )
        return False
    return True
...
async def main(full_reindex: bool):
    ...
    vector_db = ChromaDBAdapter(
        persistence_directory=settings.paths.get_vector_db_dir("roampal")
    )
    roampal_memory_adapter = await initialize_fragment_memory_adapter(
        "roampal", settings
    )
    soul_manager = SoulLayerManager(
        memory_adapter=roampal_memory_adapter, shard_id="roampal"
    )
    ...
    if full_reindex:
        logger.warning(
            f"FULL REINDEX requested. Deleting existing collection '{COLLECTION_NAME}'..."
        )
    ...
    logger.info(
        f"Found a total of {len(all_docs_to_index)} valid documents to process. Skipped {skipped_docs} noisy or empty docs."
    )

    for i in range(0, len(all_docs_to_index), BATCH_SIZE):
        batch = all_docs_to_index[i : i + BATCH_SIZE]
        logger.info(
            f"Processing batch {i//BATCH_SIZE + 1} of {len(all_docs_to_index)//BATCH_SIZE + 1}..."
        )
        ...
        try:
            ...
            vectors_batch = await asyncio.gather(
                *(embedding_service.embed_text(text) for text in texts_batch)
            )
            await vector_db.upsert_vectors(
                ids=ids_batch, vectors=vectors_batch, metadatas=metadata_batch
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate and store semantic vectors for Roampal OG's soul layer."
    )
    parser.add_argument(
        "--full-reindex",
        action="store_true",
