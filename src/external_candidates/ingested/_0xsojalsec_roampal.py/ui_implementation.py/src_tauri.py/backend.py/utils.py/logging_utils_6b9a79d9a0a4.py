# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\logging_utils.py
...
def setup_logging(log_level: str = "INFO") -> None:  # Removed app_name for simplicity
    ...
    if not isinstance(numeric_level, int):
        print(
            f"Warning: Invalid log level '{log_level}' in settings. Defaulting to INFO.",
            file=sys.stderr,
        )
    ...
    print(
        f"--- Root logging configured by setup_logging: Level={logging.getLevelName(root_logger.getEffectiveLevel())} ---",
        file=sys.stderr,
    )
