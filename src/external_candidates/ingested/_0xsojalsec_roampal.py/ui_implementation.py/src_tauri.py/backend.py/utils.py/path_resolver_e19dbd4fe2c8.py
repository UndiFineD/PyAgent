# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\path_resolver.py
...
def migrate_user_data(username: str, shard: str):
    ...
    if legacy_sessions.exists():
        for session_file in legacy_sessions.glob(f"{username}_*.json"):
            ...
            new_path = Path(
                f"backend/data/users/{user_uuid}/shards/{shard}/sessions/{new_name}"
            )
            new_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if legacy_images.exists():
        for image_file in legacy_images.glob(f"{username}_*"):
            ...
            new_path = Path(
                f"backend/data/users/{user_uuid}/shards/{shard}/images/{new_name}"
            )
            new_path.parent.mkdir(parents=True, exist_ok=True)
...
def list_user_files(username: str, shard: str, path_type: str = "sessions") -> list:
    ...
    for f in files:
        name = (
            f.name.replace(f"{username}_", "") if f"{username}_" in f.name else f.name
        )
        if name not in seen:
            seen.add(name)
