# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\circuit_breaker.py
"""
    ...
    def _record_failure(self):
        ...
        if self.failure_count >= self.failure_threshold:
            if self.state != CircuitState.OPEN:
                logger.warning(
                    f"Circuit breaker '{self.name}' opened - threshold exceeded"
                )
...
circuit_breakers = {
    "embedding_service": CircuitBreaker(
        "embedding_service", failure_threshold=3, recovery_timeout=30
    ),
    ...
    "playwright": CircuitBreaker(
        "playwright", failure_threshold=2, recovery_timeout=30
    ),
    "tools_service": CircuitBreaker(
        "tools_service", failure_threshold=3, recovery_timeout=30
    ),
}
