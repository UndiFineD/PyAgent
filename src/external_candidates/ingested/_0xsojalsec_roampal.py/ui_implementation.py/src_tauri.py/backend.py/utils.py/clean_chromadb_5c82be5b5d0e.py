# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\clean_chromadb.py
import asyncio
import json
import logging
import os
...
BACKEND_DIR = (
    THIS_DIR
    if THIS_DIR.endswith("backend")
    else os.path.abspath(os.path.join(THIS_DIR, ".."))
)
...
async def main():
    ...
    logger.info(
        f"ChromaDB client initialized for path: {getattr(vdb, 'db_path', '[unknown path]')}"
    )
