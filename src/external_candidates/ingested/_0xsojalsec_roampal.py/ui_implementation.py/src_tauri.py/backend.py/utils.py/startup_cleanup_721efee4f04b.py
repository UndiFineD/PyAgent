# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\startup_cleanup.py
"""
...
async def cleanup_soft_deleted_shards() -> Dict[str, any]:
    ...
    for shard_dir in shards_dir.iterdir():
        ...
        if deletion_marker.exists():
            ...
            collections_deleted = await delete_chromadb_collections_for_shard(
                shard_name
            )
            ...
            try:
                ...
                logger.warning(
                    f"Shard {shard_name} still has locked files, skipping cleanup"
                )
            except Exception as e:
                results["failed"].append({"shard": shard_name, "error": str(e)})
            ...
            if hasattr(adapter, "private_dbs"):
                for user_id in list(adapter.private_dbs.keys()):
                    try:
                        ...
                        logger.info(
                            f"Disconnected private ChromaDB for {shard_name} user {user_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Could not disconnect private DB for user {user_id}: {e}"
                        )
...
async def attempt_online_deletion(shard_name: str, app_state) -> Dict[str, any]:
    ...
    result["steps"].append(
        {
            ...
            "success": len(collections_deleted) > 0
            or True,  # Success even if no collections found
    ...
    try:
        if shard_dir.exists():
            ...
            result["steps"].append({"action": "delete_directory", "success": True})
        ...
        try:
            ...
            result["success"] = True
            result["message"] = (
                f"Shard {shard_name} marked for deletion on next restart"
            )
            result["steps"].append({"action": "soft_delete_marker", "success": True})
        ...
        result["steps"].append(
            {"action": "delete_directory", "success": False, "error": str(e)}
        )
    ...
    try:
        ...
        if use_server:
            ...
            try:
                ...
                logger.info(
                    f"Reusing existing ChromaDB client for shard {shard_name} deletion"
                )
            except Exception as e:
            ...
            for collection in all_collections:
                ...
                if (
                    f"global_{shard_name}_" in collection_name
                    or f"_{shard_name}_fragments" in collection_name
                    or f"_{shard_name}_conversations" in collection_name
                    or f"_{shard_name}_knowledge_graph" in collection_name
                ):
            ...
            if deleted_collections:
                logger.info(
                    f"Deleted {len(deleted_collections)} ChromaDB collections for shard {shard_name}"
                )
            else:
