# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\utils\cleanup_chromadb.py
#!/usr/bin/env python3
"""
...
def cleanup_chromadb_folders(
    chromadb_path: str = "./data/chromadb", dry_run: bool = True
):
    """
    ...
    for folder_path in folders_to_remove:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    logger.warning(
                        f"Permission error, retrying in 1 second: {folder_name}"
                    )
                    time.sleep(1)
                else:
                    logger.error(
                        f"Failed to delete {folder_name} after 3 attempts: {e}"
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Clean up orphaned ChromaDB collections"
    )
    parser.add_argument(
        "--path",
    ...
    cleanup_chromadb_folders(chromadb_path=args.path, dry_run=not args.execute)
