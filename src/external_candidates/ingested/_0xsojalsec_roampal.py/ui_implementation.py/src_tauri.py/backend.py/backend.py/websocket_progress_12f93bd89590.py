# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\backend\websocket_progress.py
"""
...
class ConnectionManager:
    ...
    async def send_progress(self, task_id: str, data: dict):
        ...
        if task_id in self.active_connections:
            logger.info(
                f"Sending progress update to {len(self.active_connections[task_id])} connections for task {task_id}: {data.get('status', 'unknown')}"
            )
            disconnected = set()
            ...
            logger.warning(
                f"No active connections for task {task_id} when trying to send: {data.get('status', 'unknown')}"
            )
