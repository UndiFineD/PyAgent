# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\backend\book_upload_api.py
"""
    ...
    try:
        ...
        if check_duplicate:
            ...
            if not hasattr(request.app.state, "book_processor"):
                raise HTTPException(
                    status_code=500, detail="Book processor not initialized"
                )
        ...
        logger.info(
            f"Background task started for book_id '{book_id}' with task_id '{task_id}'"
        )
        ...
        logger.error(
            ...
            exc_info=settings.app.log_level == "DEBUG",
        )
        return BookUploadResponse(
            success=False,
            book_id="",
...
async def process_book(
    book_id: str, request: Request, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        metadata_file = (
            settings.paths.get_book_folder_path() / f"{book_id}.metadata.json"
        )
...
async def process_book_task(
    book_id: str, file_path: str, metadata: Dict, task_id: str, request: Request
):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to read book file: {e}"
            )
        ...
        metadata_file = (
            settings.paths.get_book_folder_path() / "metadata" / f"{book_id}.json"
        )
        ...
        if book_file.exists():
            try:
                ...
                logger.info(
                    f"Archived uploaded book file to {archive_dir / book_file.name}"
                )
            except Exception as e:
...
async def get_processing_progress(book_id: str):
    ...
    raise HTTPException(
        status_code=404, detail="No processing task found for this book"
    )
...
async def get_active_tasks():
    ...
    return {"tasks": list(manager.tasks.values())}
...
async def search_books(
    ...
    try:
        ...
        results = await processor.search_full_text(
            query=query, book_title=book_title, limit=limit, offset=offset
        )
        ...
        if db_path.exists():
            ...
            for book in books:
                ...
                for db_id, db_info in db_books.items():
                    if (
                        db_info["title"] in book.get("title", "")
                        or book.get("title", "") in db_info["title"]
                    ):
    ...
    try:
        ...
        if db_path.exists():
            ...
            if book_record:
                ...
                cursor.execute(
                    "SELECT chunk_id FROM chunks WHERE book_id = ?", (internal_id,)
                )
                ...
                logger.info(
                    f"Deleted book '{title}' (ID: {internal_id}) and {chunks_deleted} chunks from database"
                )
                ...
                if hasattr(request.app.state, "memory_collections"):
                    try:
                        ...
                        if hasattr(request.app.state.memory_collections, "collections"):
                            books_adapter = (
                                request.app.state.memory_collections.collections.get(
                                    "books"
                                )
                            )
                        elif hasattr(
                            request.app.state.memory_collections, "books_collection"
                        ):
                            books_adapter = (
                                request.app.state.memory_collections.books_collection
                            )
                        ...
                        if books_adapter:
                            ...
                            try:
                                ...
                                if book_chunk_ids:
                                    ...
                                    logger.info(
                                        f"Deleted {deleted_count} embeddings from ChromaDB for book '{title}' (by book_id pattern)"
                                    )
                                ...
                                if chunk_ids:
                                    try:
                                        ...
                                        logger.info(
                                            f"Deleted {len(chunk_ids)} additional embeddings from ChromaDB (by database chunk_ids)"
                                        )
                                    except Exception as e:
                                ...
                                if deleted_count != chunks_deleted:
                                    logger.warning(
                                        f"ChromaDB deletion mismatch: deleted {deleted_count} embeddings but expected {chunks_deleted}"
                                    )
                            except Exception as e:
                                logger.warning(f"Error deleting chunks: {e}")
                    except Exception as chroma_err:
                        logger.warning(
                            f"Failed to delete ChromaDB embeddings for book '{title}': {chroma_err}"
                        )
                ...
                if hasattr(request.app.state, "memory") and chunk_ids:
                    try:
                        ...
                        if cleaned > 0:
                            logger.info(
                                f"Cleaned {cleaned} Action KG examples for deleted book '{title}'"
                            )
                    except Exception as kg_err:
                        logger.warning(
                            f"Failed to clean Action KG for book '{title}': {kg_err}"
                        )
