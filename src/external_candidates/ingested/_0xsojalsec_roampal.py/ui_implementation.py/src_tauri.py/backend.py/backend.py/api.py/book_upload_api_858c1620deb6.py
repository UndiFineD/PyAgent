    ...
    try:
        ...
        try:
            ...
            if extracted.extraction_warnings:
                logger.info(
                    f"Extraction warnings for '{file.filename}': {extracted.extraction_warnings}"
                )
        except ExtractionError as e:
            file_path.unlink()
            raise HTTPException(
                status_code=400, detail=f"Could not process file: {str(e)}"
            )
...
async def process_book_task(
    ...
    try:
        ...
        try:
            ...
            if extracted.extraction_warnings:
                logger.warning(
                    f"[BOOK_PROCESSOR] Extraction warnings: {extracted.extraction_warnings}"
                )

        except ExtractionError as e:
            logger.error(f"Failed to extract content from {file_path}: {e}")
            raise HTTPException(
                status_code=500, detail=f"Failed to extract book content: {e}"
            )
        except Exception as e:
