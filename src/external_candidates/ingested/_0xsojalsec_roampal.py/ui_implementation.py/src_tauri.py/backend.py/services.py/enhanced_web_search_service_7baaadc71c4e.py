...
class EnhancedWebSearchService:
    ...
    async def search(
        ...
        try:
            ...
            if not shard_config:
                logger.warning(
                    f"No configuration found for shard {shard_id}, using default"
                )
            ...
            logger.info(
                f"Filtering {len(search_results)} results with threshold {shard_config.relevance_threshold}"
            )
            ...
            filtered_results = self._filter_results_by_relevance(
                search_results, shard_config.relevance_threshold
            )
            ...
            logger.info(
                f"Enhanced search completed for shard {shard_id}: {len(filtered_results)} results"
            )
            return response
    ...
    async def scrape_url(self, url: str, shard_id: str = "og") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"URL scraping completed for shard {shard_id}: {len(content) if content else 0} characters"
            )
            return response
    ...
    async def get_search_history(
        self, shard_id: str = "og", limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            results = await adapter.query_vectors(
                query_vector=None, top_k=limit, filters={"source": "web_search"}
            )
                ...
                history.append(
                    {
                        ...
                        "relevance_score": result.get("meta", {}).get(
                            "relevance_score", 0.0
                        ),
                    }
                )
