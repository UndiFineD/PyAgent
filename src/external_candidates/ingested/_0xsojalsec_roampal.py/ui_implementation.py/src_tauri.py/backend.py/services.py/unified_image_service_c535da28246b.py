# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\services\unified_image_service.py
"""
...
class UnifiedImageService:
    ...
    async def process_image(
        ...
        try:
            ...
            if self.memory_adapter and (
                result.get("extracted_text") or result.get("content_analysis")
            ):
    ...
    async def _store_in_memory(self, result: Dict[str, Any]):
        ...
        try:
            ...
            if result.get("content_analysis"):
                fragment_text += (
                    f"Content: {result['content_analysis'].get('description', '')}\n"
                )
            ...
            await self.memory_adapter.add_memory(text=fragment_text, metadata=metadata)
    ...
    async def search_image_memories(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            results = await self.memory_adapter.search_memories(
                query_text=query, filter_dict={"type": "image"}, top_k=limit
            )
            return results
