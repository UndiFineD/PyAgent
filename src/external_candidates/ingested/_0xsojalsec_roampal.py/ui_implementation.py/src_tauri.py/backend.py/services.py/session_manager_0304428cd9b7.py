...
class SecureSessionManager:
    ...
    def create_secure_session(
        self, user_id: str, shard_id: str, metadata: Optional[Dict] = None
    ) -> Tuple[str, str]:
        """
    ...
    def validate_session_ownership(
        self, session_id: str, user_id: str, shard_id: str
    ) -> bool:
        """
    ...
    def get_user_sessions(self, user_id: str) -> list:
        ...
        valid_sessions = []
        for session_id in self._user_sessions[user_id]:
            if session_id in self._sessions and self._sessions[session_id].get(
                "valid", False
            ):
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self._stats,
            "active_sessions": len(
                [s for s in self._sessions.values() if s.get("valid", False)]
            ),
...
def create_session(
    user_id: str, shard_id: str, metadata: Optional[Dict] = None
) -> Tuple[str, str]:
    """Create a new secure session"""
...
def validate_session(
    session_token: str, user_id: str, shard_id: str
) -> Tuple[bool, Optional[str]]:
    """Validate a session token"""
    is_valid, error_msg, _ = secure_session_manager.validate_session(
        session_token, user_id, shard_id
    )
...
def validate_ownership(session_id: str, user_id: str, shard_id: str) -> bool:
    ...
    return secure_session_manager.validate_session_ownership(
        session_id, user_id, shard_id
    )
