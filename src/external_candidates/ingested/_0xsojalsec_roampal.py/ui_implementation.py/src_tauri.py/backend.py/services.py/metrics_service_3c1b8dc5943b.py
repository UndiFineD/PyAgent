# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\services\metrics_service.py
"""
...
class SimpleMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if self.outcome_agreements["total"] > 0:
            summary["outcome_agreement_rate"] = (
                self.outcome_agreements["agreed"] / self.outcome_agreements["total"]
            )
        else:
            summary["outcome_agreement_rate"] = 0.0

        if self.top1_accepts["total"] > 0:
            summary["top1_accept_rate"] = (
                self.top1_accepts["accepted"] / self.top1_accepts["total"]
            )
        else:
            summary["top1_accept_rate"] = 0.0

        if self.dont_repeat_failures["total"] > 0:
            summary["dont_repeat_failed_fix_rate"] = (
                self.dont_repeat_failures["avoided"]
                / self.dont_repeat_failures["total"]
            )
        else:
        ...
        if self.task_outcomes:
            ...
            summary["p95_latency_ms"] = sorted_latencies[
