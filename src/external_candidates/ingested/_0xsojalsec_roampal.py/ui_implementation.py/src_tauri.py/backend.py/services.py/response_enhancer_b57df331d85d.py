# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\services\response_enhancer.py
"""
...
class ResponseEnhancer:
    ...
    def add_code_change(self, file_path: str, diff: str, description: str) -> str:
    ...
    def track_memory_operation(
        ...
        descriptions = {
            "search": (
                f"Searched memory: {query[:50]}..." if query else "Searched memory"
            ),
            "store": f"Stored in {collection}" if collection else "Stored memory",
            "get_failures": (
                f"Checked failures for: {query}" if query else "Checked failures"
            ),
            "get_recent": "Retrieved recent memories",
            "pattern_match": (
                f"Found pattern match for: {query[:30]}..."
                if query
