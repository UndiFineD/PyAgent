# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\config\feature_flag_validator.py
"""
...
class FeatureFlagValidator:
        ...
        (
            {
                ...
                "MAX_AUTONOMOUS_ACTIONS": lambda x: x > 10,
            },
    ...
    def validate_and_log(config: Dict[str, Any], is_production: bool) -> bool:
        ...
        if warnings:
            logger.warning(
                "⚠️  Dangerous flag combinations detected (non-production mode)"
            )
        else:
            logger.info(
                "✓ Feature flag configuration validated - no dangerous combinations"
            )
    ...
    def sanitize_for_production(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if config.get("MAX_FILE_WRITES_PER_SESSION", 0) > 0:
            logger.warning(
                "⚠️  Production safety: Setting MAX_FILE_WRITES_PER_SESSION to 0"
            )
