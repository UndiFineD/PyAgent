# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\config\model_contexts.py
"""
    ...
    try:
        ...
        with _settings_lock:
            ...
            if "model_overrides" in data and model_name in data["model_overrides"]:
                ...
                logger.info(
                    f"Deleted context override for {model_name}, restored to default"
                )
            else:
...
def get_all_model_contexts() -> Dict[str, Dict[str, Any]]:
    ...
    result = {}
    for prefix, config in MODEL_CONTEXTS.items():
        result[prefix] = {"default": config["default"], "max": config["max"]}
    return result
