...
if ROAMPAL_DATA_DIR:
    ...
    if os.name == "nt":  # Windows
        ...
        DATA_PATH = os.path.expanduser(
            f"~/Library/Application Support/{ROAMPAL_DATA_DIR}/data"
        )
    else:  # Linux
        DATA_PATH = os.path.expanduser(
            f"~/.local/share/{ROAMPAL_DATA_DIR.lower()}/data"
        )
...
class AppSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="ROAMPAL_APP_", extra="ignore", case_sensitive=False
    )
...
class LLMSettings(BaseSettings):
    ...
    ollama_model: str = (
        "mistral-nemo:12b-instruct-2407-q4_0"  # Using Mistral Nemo for better performance
    )
    ollama_request_timeout_seconds: int = 30  # 30 seconds - fail fast for MCP (was 300)
    ollama_keep_alive_seconds: int = 120
    max_keepalive_connections: int = Field(
        5, description="Max keepalive connections for httpx."
    )
...
class FragmentFileMemorySettings(BaseModel):
    ...
    def file_conversation_history_dir(self) -> str:
        return str(
            Path(self.base_data_path).resolve()
            / (self.conversation_history_subdir or "conversations")
        )
    ...
    def file_arbitrary_data_dir(self) -> str:
        return str(
            Path(self.base_data_path).resolve()
            / (self.arbitrary_store_subdir or "arbitrary_store")
        )
...
class WebSearchSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="ROAMPAL_WEB_", extra="ignore", case_sensitive=False
    )
...
class ScoringSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="ROAMPAL_SCORING_", extra="ignore", case_sensitive=False
    )
...
class FeedbackSettings(BaseModel):
    ...
    sentiment_weight: float = Field(
        0.3, description="Weight for sentiment in composite score"
    )
    usefulness_weight: float = Field(
        0.7, description="Weight for LLM evaluation in composite score"
    )

    # Time decay settings
    decay_rate_per_day: float = Field(
        0.01, description="Score decay rate per day unused (1%)"
    )
    min_decay_factor: float = Field(0.5, description="Minimum decay factor (50% floor)")

    # Thresholds
    low_usefulness_threshold: float = Field(
        0.3, description="Threshold for low usefulness"
    )
    high_usefulness_threshold: float = Field(
        0.7, description="Threshold for high usefulness"
    )
    positive_threshold: float = Field(
        0.7, description="Threshold for positive sentiment"
    )
    negative_threshold: float = Field(
        0.3, description="Threshold for negative sentiment"
    )
...
class ChromaDBSettings(BaseModel):
    ...
    use_server: bool = Field(
        True, description="Use ChromaDB server mode instead of embedded mode"
    )
...
class PromptSettings(BaseModel):
    ...
    sentiment_analysis_template_path: Path = Field(
        PROMPTS_DIR / "sentiment_analysis.txt"
    )
    profile_update_detection_template_path: Path = Field(
        PROMPTS_DIR / "profile_update_detection.txt"
    )
...
class MemoryLayerSettings(BaseModel):
    ...
    # Search settings
    hybrid_top_k_initial: int = Field(
        15, description="Number of results for hybrid search"
    )
...
class MemoryInjectorSettings(BaseModel):
    ...
    forgotten_score_threshold: float = Field(
        0.15, description="Threshold for considering a fragment forgotten"
    )
    forgotten_revive_probability: float = Field(
        0.1, description="Probability of reviving a forgotten fragment"
    )
    decay_rate_neutral: float = Field(
        0.02, description="Decay rate for neutral fragments"
    )
    min_score: float = Field(0.0, description="Minimum score for fragments")
    positive_threshold: float = Field(
        0.6, description="Sentiment threshold for positive boost"
    )
    negative_threshold: float = Field(
        -0.6, description="Sentiment threshold for negative decay"
    )
    boost_positive: float = Field(
        0.15, description="Score boost for positive sentiment"
    )
...
class FeatureFlags(BaseModel):
    ...
    disable_seeding: bool = Field(
        False, description="Disable book seeding on startup to improve performance"
    )
...
class Settings(BaseSettings):
    ...
    fragment_memory_configs: Dict[str, FragmentFileMemorySettings] = Field(
    ...
    def __post_init__(self):
        if self.embedding.align_with_llm and self.embedding.provider == "ollama":
            self.embedding.model_name = (
                self.embedding.model_name or self.llm.ollama_model
            )
            logger.info(
                "Aligned embedding model with active LLM: {}".format(
                    self.embedding.model_name
                )
            )
            ...
            with open(config_file_path, "r", encoding="utf-8") as f:
                ...
                for frag_id, config_data in fragment_configs_dict.items():
                    if "base_data_path" in config_data:
                        ...
                        settings_instance.fragment_memory_configs[frag_id] = (
                            FragmentFileMemorySettings(**config_data)
                        )
                        logger.info(
