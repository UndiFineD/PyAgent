...
class FeatureFlags:
    ...
    ENABLE_WEBSOCKET_STREAMING: bool = (
        True  # Token-by-token streaming via WebSocket (REQUIRED for UI functionality)
    )

    # Hybrid features (safe partial enablement)
    ENABLE_OUTCOME_DETECTION: bool = True  # Enable outcome detection system
    ENABLE_LLM_OUTCOME_DETECTION: bool = (
        True  # LLM-only detection (no heuristic fallback as of 2025-10-05)
    )
    ENABLE_LLM_AUTONOMOUS_ROUTING: bool = (
        True  # Full LLM autonomy, zero overrides (2025-10-02)
    )
...
class FeatureFlagManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        self.config_path = (
            Path(config_path) if config_path else Path("data/feature_flags.json")
        )
    ...
    def save_flags(self):
        ...
        try:
            ...
            with open(self.config_path, "w") as f:
                ...
                config = {
                    k: v for k, v in asdict(self.flags).items() if not k.startswith("_")
                }
    ...
    def validate(self) -> Dict[str, str]:
        ...
        if self.flags.ENABLE_AUTONOMY and not self.flags.REQUIRE_CONFIRMATION:
            issues["AUTONOMY_NO_CONFIRM"] = (
                "Autonomous mode without confirmation is dangerous"
            )

        # Auto-apply solutions requires problem-solution index
        if (
            self.flags.ENABLE_AUTO_APPLY_SOLUTIONS
            and not self.flags.ENABLE_PROBLEM_SOLUTION_INDEX
        ):
        ...
        if self.flags.ENABLE_REFLECTION_WRITE and self.flags.REFLECTION_DRY_RUN:
            issues["REFLECTION_CONFLICT"] = (
                "Reflection write enabled but in dry-run mode"
            )
        ...
        if self.flags.ENABLE_GIT_OPERATIONS:
            issues["GIT_WARNING"] = (
                "Git operations enabled - ensure proper access controls"
            )
