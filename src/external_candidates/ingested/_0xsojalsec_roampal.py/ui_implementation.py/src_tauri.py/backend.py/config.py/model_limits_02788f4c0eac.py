# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\config\model_limits.py
"""
...
MODEL_CONFIGURATIONS = {
    ...
    "qwen3-coder:30b": (
        ...
        60000,
        10,
    ),  # MoE 30B, 256K context, 3.3B active
    ...
    "llama4:maverick": (
        ...
        250000,
        12,
    ),  # MoE 401B, 128 experts, 1M context
...
class IterationMetrics:
    ...
    def should_continue(self) -> bool:
        ...
        if self.coherence_score < QUALITY_THRESHOLD:
            logger.warning(
                f"Iteration {self.iteration}: Low coherence {self.coherence_score}"
            )
            return False
...
def get_model_limits(model_name: str, user_message: str) -> ModelLimits:
    ...
    effective_iterations = int(
        base_iterations * complexity_config["iteration_multiplier"]
    )
    ...
    logger.info(
        f"Model limits for {model_name}: context={safe_context}, "
    ...
    if content_type == "list_directory":
        ...
        if isinstance(lines, list) and len(lines) > 30:
            ...
            sample = (
                ...
                else [str(item) for item in lines[:20]]
            )
            sample_text = (
        ...
        for line in lines:
            ...
            if current_size + line_size > limit:
                result_lines.append(
                    f"... [{len(lines) - len(result_lines)} more results truncated]"
                )
                break
...
def calculate_coherence_score(
    response: str, iteration: int, expected_tools: int = 0
) -> float:
    """
...
def should_continue_iterations(
    ...
    if iteration >= model_limits.effective_iterations:
        return (
            False,
            f"Reached maximum iterations ({model_limits.effective_iterations})",
        )
