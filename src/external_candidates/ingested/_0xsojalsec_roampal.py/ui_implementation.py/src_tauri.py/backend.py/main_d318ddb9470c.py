# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roampal\ui-implementation\src-tauri\backend\main.py
#!/usr/bin/env python3
"""
...
from app.routers.mcp_servers import (
    router as mcp_servers_router,  # v0.2.5: External MCP tool servers
)
...
_mcp_search_cache = (
    {}
)  # {session_id: {"doc_ids": [...], "query": "...", "timestamp": ...}}
_mcp_first_tool_call = (
    set()
)  # Track first tool call per session for cold-start injection
_mcp_action_cache = (
    {}
)  # {session_id: {"actions": [...], "last_context": str, "last_activity": datetime}} - Track tool actions for Action-Effectiveness KG
    ...
    if session_id not in _mcp_first_tool_call:
        ...
        try:
            context_summary = await asyncio.wait_for(
                memory_system.get_cold_start_context(limit=5), timeout=10.0
            )

            if context_summary:
                logger.info(
                    f"[MCP] Cold-start injection for {session_id}: {len(context_summary)} chars"
                )
...
async def memory_promotion_task(memory: UnifiedMemorySystem):
    ...
    while True:
        try:
            ...
            if cleaned_refs > 0:
                logger.info(
                    f"Cleaned {cleaned_refs} dead references from knowledge graph"
                )
...
This closes the loop. Without it, the system can't learn what worked OR what didn't.""",
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        ...
        try:
            if name == "search_memory":
                ...
                result_collections = (
                    set()
                )  # Track which collections actually returned results (ALL, for KG routing)
                if results:
                    for r in results:
                ...
                logger.info(
                    f"[MCP] Cached {len(cached_doc_ids)} doc_ids from query '{query[:50]}' (result collections: {result_collections})"
                )
                ...
                for coll in collections_used:
                    action = ActionOutcome(
                        ...
                        doc_id=(
                            cached_doc_ids[0] if cached_doc_ids else None
                        ),  # v0.2.6: Track first result for doc-level insights
                    )
                ...
                try:
                    ...
                    if matched_concepts:
                        try:
                            relevant_facts = await memory.get_facts_for_entities(
                                matched_concepts[:5], limit=2
                            )
                        except Exception as e:
                            logger.warning(
                                f"[MCP] Failed to get facts for entities: {e}"
                            )

                    # Format insights
                    has_actionable_insights = org_context and (
                    ...
                    if (
                        not has_actionable_insights
                        and not action_stats
                        and not relevant_facts
                    ):
                ...
                logger.info(
                    f"[MCP] record_response: session={session_id}, outcome={outcome}"
                )
                ...
                logger.info(
                    f"[MCP] Stored learning with doc_id={doc_id}, initial_score={initial_score}"
                )

                # Score cached memories from last search (architecture.md line 578)
                cached_memories_scored = 0
                if (
                    outcome in ["worked", "failed", "partial"]
                    and session_id in _mcp_search_cache
                ):
                    cached = _mcp_search_cache[session_id]
                    cached_doc_ids = cached.get("doc_ids", [])

                    logger.info(
                        f"[MCP] Scoring {len(cached_doc_ids)} cached memories with outcome={outcome}"
                    )
                    ...
                    for cached_doc_id in cached_doc_ids:
                        try:
                            ...
                            logger.warning(
                                f"[MCP] Failed to score cached memory {cached_doc_id}: {e}"
                            )
                    ...
                    if (
                        cached_query
                        and outcome in ["worked", "failed", "partial"]
                        and cached_collections
                    ):
                        for collection in cached_collections:
                            try:
                                await memory._update_kg_routing(
                                    cached_query, collection, outcome
                                )
                                logger.info(
                                    f"[MCP] KG routing updated: '{cached_query[:50]}' → {collection} ({outcome})"
                                )
                            except Exception as e:
                                logger.warning(
                                    f"[MCP] Failed to update KG routing: {e}"
                                )
                ...
                if (
                    ...
                    logger.info(
                        f"[MCP] Scoring {len(cached_actions)} cached actions for Action-Effectiveness KG with outcome={outcome}"
                    )
                    ...
                    for action in cached_actions:
                        ...
                        try:
                            ...
                            actions_scored += 1
                            logger.info(
                                f"[MCP] Action-Effectiveness KG updated: {action.context_type}|{action.action_type}|{action.collection or '*'} → {outcome}"
                            )
                        except Exception as e:
                            logger.warning(
                ...
                if cached_memories_scored > 0:
                    response_text += (
                        f"Scored {cached_memories_scored} cached memories\n"
                    )

                if actions_scored > 0:
                    response_text += (
                        f"Updated {actions_scored} tool effectiveness stats\n"
                    )
    ...
    logger.info(
        "[MCP] Server initialized with 6 tools: search_memory, add_to_memory_bank, update_memory, archive_memory, get_context_insights, record_response"
    )
