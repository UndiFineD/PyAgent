# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoRAG\Vimo-desktop\python_backend\videorag\_llm.py
# type: ignore
import os
...
class LLMConfig:
    ...
    def __post_init__(self):
        ...
        self.cheap_model_func = (
            lambda prompt, *args, **kwargs: self.cheap_model_func_raw(
                self.cheap_model_name, prompt, *args, **kwargs
            )
        )

        self.caption_model_func = (
            lambda content_list, *args, **kwargs: self.caption_model_func_raw(
...
async def dashscope_caption_complete(model_name, content_list, **kwargs) -> str:
    """
    DashScope vision model completion for video caption
    content_list: list of {"type": "image_url", "image_url": {"url": "..."}} and {"type": "text", "text": "..."}
    """
    dashscope_async_client = get_dashscope_async_client_instance(
