...
class VideoRAG:
    ...
    def __post_init__(self):
        ...
        assert (
            self.ali_dashscope_api_key is not None
        ), "ali_dashscope_api_key is required"
        assert (
            self.ali_dashscope_base_url is not None
        ), "ali_dashscope_base_url is required"
        ...
        self.safe_config["chunk_entity_relation_graph"] = (
        ...
        limited_embedding_func = limit_async_func_call(
            self.llm.embedding_func_max_async
        )(wrapped_embedding_func)

        self.embedding_func = SerializableEmbeddingWrapper(
            limited_embedding_func, self.safe_config
        )
        ...
        self.llm.best_model_func = limit_async_func_call(self.llm.best_model_max_async)(
            SerializableLLMWrapper(
                self.llm.best_model_func, self.safe_config, self.llm_response_cache
            )
        )
        self.llm.cheap_model_func = limit_async_func_call(
            ...
            SerializableLLMWrapper(
                self.llm.cheap_model_func, self.safe_config, self.llm_response_cache
            )
        )
        self.llm.caption_model_func = limit_async_func_call(
            self.llm.caption_model_max_async
        )(
            SerializableLLMWrapper(
                self.llm.caption_model_func, self.safe_config, self.llm_response_cache
            )
        )
    ...
    def insert_video(self, video_path_list=None, progress_callback=None):
        ...
        for video_path in video_path_list:
            ...
            if progress_callback:
                progress_callback(
                    "Video Splitting", f"Splitting video into clips for {video_name}..."
                )
            ...
            if progress_callback:
                progress_callback(
                    "Visual Analyzing", f"Analyzing video content for {video_name}..."
                )
            ...
            if progress_callback:
                progress_callback(
                    "Feature Encoding", f"Encoding video features for {video_name}..."
                )
