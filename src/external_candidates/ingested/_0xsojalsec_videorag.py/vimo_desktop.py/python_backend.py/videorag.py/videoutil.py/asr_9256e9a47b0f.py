# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VideoRAG\Vimo-desktop\python_backend\videorag\_videoutil\asr.py
import asyncio
import os
...
async def process_single_segment(
    semaphore, index, segment_name, audio_file, model, audio_output_format, sample_rate
):
    """
...
async def speech_to_text_online(
    video_name,
    working_dir,
    segment_index2name,
    audio_output_format,
    ...
    logger.info(
        f"🎤 Starting ASR for {total_tasks} audio segments (max {max_concurrent} concurrent)..."
    )
    ...
    for completed_task in asyncio.as_completed(tasks):
        try:
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                completed += 1
                logger.info(
                    f"✅ Completed {completed}/{total_tasks} segments (Progress: {completed/total_tasks*100:.1f}%)"
                )
            else:
            ...
            logger.info(
                f"❌ Failed {completed}/{total_tasks} segments (Progress: {completed/total_tasks*100:.1f}%)"
            )

    logger.info(
        f"🎉 ASR processing completed! Processed {len(transcripts)} segments successfully."
    )
...
async def speech_to_text_async(
    ...
    if not api_key:
        raise ValueError(
            "ali_dashscope_api_key must be provided in global_config for online ASR"
        )
