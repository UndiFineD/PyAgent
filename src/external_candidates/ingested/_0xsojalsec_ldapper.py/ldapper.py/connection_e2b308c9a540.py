# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ldapper\ldapper\connection.py
import logging
import re
...
class BaseConnection(object):
    ...
    def __init__(self, logindn, password, uri=None, certfile=None, basedn=None):
        if basedn:
            self.basedn = basedn
        else:
    ...
    def search(
        ...
        log.debug(
            "Searching with filter %s in %s under scope %s" % (filter, basedn, scope)
        )
        try:
            ...
            result = new_conn.ldaps.search_s(basedn, scope, filter, attrlist=attrlist)
    ...
    def modify_attr(self, dn, attribute, value):
        ...
        try:
            ...
            log.error("mod attr %s on %s failed" % (attribute, dn), exc_info=False)
            raise
            return False
    ...
    def add_attr(self, dn, attribute, value):
        ...
        try:
            ...
            log.error("add attr %s on %s failed" % (attribute, dn), exc_info=False)
            raise
            return False
    ...
    def delete_attr(self, dn, attribute, value=None):
        ...
        try:
            self.ldaps.modify_s(dn, tuple)
            log.debug("del attr %s and value %s on %s" % (attribute, dn, value))
            return True
        except ldap.NO_SUCH_OBJECT:
            raise NoSuchDN(dn=dn)
            ...
            log.error(
                "del attr %s and value %s on %s failed" % (attribute, value, dn),
                exc_info=False,
            )
            raise
            return False
