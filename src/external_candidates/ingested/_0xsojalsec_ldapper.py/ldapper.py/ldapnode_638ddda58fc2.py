...
class LDAPNode(object, metaclass=LDAPNodeBase):
    ...
    def _fetch_entry(cls, primary=None, dnprefix=None, **kwargs):
        ...
        try:
            ...
            logging.warning(
                "Unable to fetch %s %s.  Bad LDAP search filter." % (cls.hrn, primary)
            )
            return None
        ...
        if result is None:
            ...
            logging.warning(
                "Found %d results.  Unable to fetch %s %s."
    ...
    def fetch(cls, primary=None, dnprefix=None, **kwargs):
        ...
        if result is not None:
            ...
            logging.debug(
                "Loaded %s %s Successfully" % (obj.__class__.__name__, obj.dnattr())
            )
            return obj
        else:
            return None
        ...
        if max_results:
            ...
            for dn, entry in results:
                if num_results_processed < max_results:
                    ...
                    logging.debug(
                        "Loaded %s %s Successfully" % (o.__class__.__name__, o.dnattr())
                    )
                    objs.append(o)
    ...
    @classmethod
    def searchable_fields_search_prefix_filter(
        cls, search_prefix, searchable_fields=None
    ):
    ...
    @classmethod
    def searchable_fields_search_string_filter(
        cls, search_string, searchable_fields=None
    ):
    ...
    def log__modify_attr_success(self, attr_name, old_val, new_val):
        ...
        if isinstance(old_val, list) and isinstance(new_val, list):
            ...
            if added:
                ...
                self.logger.info(
                    "Added %s %s to %s %s" % (name, added, self.hrn, self.dnattr())
                )
            if removed:
    ...
    def log__delete_attr_failure(self, attr_name):
        ...
        self.logger.error(
            "Failed to remove %s attribute from %s %s"
            % (attr_name, self.hrn, self.dnattr())
        )
    ...
    def objectclass_filter(cls):
        ...
        if cls._meta.excluded_objectclasses:
            n = build_ldap_filter(
                op="!", attrname="objectClass", items=cls._meta.excluded_objectclasses
            )
            f = "(&%s%s)" % (f, n)
        return f
