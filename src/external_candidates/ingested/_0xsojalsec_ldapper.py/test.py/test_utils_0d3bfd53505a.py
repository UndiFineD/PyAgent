# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ldapper\test\test_utils.py
# -*- coding: utf-8 -*-
...
class TestUtils(object):
    ...
    def test_build_ldap_filter_no_items(self):
        with pytest.raises(ValueError):
            assert (
                build_ldap_filter(op="&", attrname="objectClass", items=None) == "foo"
            )
        with pytest.raises(ValueError):
 ...
 foobarr fourteen"""
    ...
    def test_stringify(self):
        ...
        assert {"name": "foo", "user": p.dnattr()} == stringify(
            {"name": "foo", "user": p}
        )
