# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytune\device\linux.py
import base64
import os
...
from utils.utils import extract_pfx, prtauth, renew_token
...
class Linux(Device):
    def __init__(
        ...
        super().__init__(
            logger, os, device_name, deviceid, uid, tenant, prt, session_key
        )
    ...
    def send_enroll_request(
        self, enrollment_url, csr_pem, csr_token, ztdregistrationid
    ):
        data = {
    ...
    def report_policy_status(self, access_token, policies):
                    ...
                    details.append(
                        {
                            ...
                            "SettingDefinitionItemId": setting[
        ...
        data = {"DeviceId": self.intune_deviceid, "PolicyStatuses": statuses}
    ...
    def checkin(self, mdmpfx):
        ...
        self.checkin_url = self.get_enrollment_info(
            access_token, "LinuxDeviceCheckinService"
        )
