# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytune\device\device.py
import base64
import os
import re
import struct
...
from utils.utils import (
    ...
    prtauth,
    renew_token,
    token_renewal_for_enrollment,
)


class Device:
    def __init__(
        self, logger, os, device_name, deviceid, uid, tenant, prt, session_key
    ):
        self.logger = logger
        self.os = os
    ...
    def enroll_intune(self):
        ...
        try:
            response = self.send_enroll_request(
                enrollment_url, csr_pem, csr_token, None
            )
        except:
    ...
    @abstractmethod
    def send_enroll_request(
        self, enrollment_url, access_token_b64, csr_pem, ztdregistrationid
    ):
        pass
    ...
    def download_msi(self, msi_url, certpath, keypath):
        ...
        response = requests.get(url=msi_url, cert=(certpath, keypath))
    ...
    def check_compliant(self):
        ...
        token_renewal_url = self.get_enrollment_info(
            access_token, "TokenRenewalService"
        )
        ...
        enrollment_token = token_renewal_for_enrollment(
            token_renewal_url, renewal_token
        )

        device_name = self.get_device_info(
            iwservice_url, enrollment_token, "OfficialName"
        )
        ...
        reasons = self.get_device_info(
            iwservice_url, enrollment_token, "NoncompliantRules"
        )
        if reasons == None:
    ...
    def retire_intune(self):
        ...
        retire_info = self.get_device_info(
            iwservice_url, enrollment_token, "#CommonContainer.Retire"
        )
        if retire_info == None:
            retire_info = self.get_device_info(
                iwservice_url, enrollment_token, "#CommonContainer.FullWipe"
            )
        ...
        syncml_template = {
            "SyncML": {
                ...
                "SyncHdr": {
                    ...
                    "MsgID": f"{str(msgid)}",
                    "Target": {"LocURI": self.checkin_url},
    ...
    def create_csr(self, private_key, cname):
        csr_subject = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, cname)])
    ...
    def save_mdm_certs(self, private_key, my_cert, pfxpath):
        cert = x509.load_der_x509_certificate(
            base64.b64decode(my_cert), default_backend()
        )
