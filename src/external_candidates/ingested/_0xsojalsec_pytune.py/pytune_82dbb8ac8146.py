# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pytune\pytune.py
import argparse
import getpass
...
class Pytune:
    ...
    def entra_join(
        self, username, password, access_token, device_name, os, deviceticket
    ):
    ...
    def enroll_intune(
        self, os, device_name, username, password, refresh_token, certpfx
    ):
        device = self.new_device(
            os, device_name, username, password, refresh_token, certpfx
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        add_help=True,
        description=color(description, fore="deepskyblue"),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    subparsers = parser.add_subparsers(dest="command", description="pytune commands")

    entra_join_parser = subparsers.add_parser(
        "entra_join", help="join device to Entra ID"
    )
    ...
    entra_join_parser.add_argument(
        "-a",
        "--access_token",
        action="store",
        help="access token for device registration service",
    )
    entra_join_parser.add_argument(
        "-d", "--device_name", required=True, action="store", help="device name"
    )
    entra_join_parser.add_argument(
        "-o", "--os", required=True, action="store", help="os"
    )
    entra_join_parser.add_argument(
        "-D", "--deviceticket", required=False, action="store", help="device ticket"
    )

    entra_delete_parser = subparsers.add_parser(
        "entra_delete", help="delete device from Entra ID"
    )
    entra_delete_parser.add_argument(
        "-c", "--certpfx", required=True, action="store", help="device cert pfx path"
    )

    enroll_intune_parser = subparsers.add_parser(
        "enroll_intune", help="enroll device to Intune"
    )
    enroll_intune_parser.add_argument(
        "-u", "--username", action="store", help="username"
    )
    enroll_intune_parser.add_argument(
        "-p", "--password", action="store", help="password"
    )
    enroll_intune_parser.add_argument(
        "-r",
        "--refresh_token",
    ...
    checkin_parser.add_argument(
        "-m", "--mdmpfx", required=True, action="store", help="mdm pfx path"
    )
    checkin_parser.add_argument(
        "-d", "--device_name", required=True, action="store", help="device name"
    )
    ...
    checkin_parser.add_argument(
        "-H", "--hwhash", required=False, action="store", help="Autopilot hardware hash"
    )

    retire_intune_parser = subparsers.add_parser(
        "retire_intune", help="retire device from Intune"
    )
    retire_intune_parser.add_argument(
        "-u", "--username", action="store", help="username"
    )
    retire_intune_parser.add_argument(
        "-p", "--password", action="store", help="password"
    ...
    check_compliant_parser = subparsers.add_parser(
        "check_compliant", help="check compliant status"
    )
    check_compliant_parser.add_argument(
        "-u", "--username", action="store", help="username"
    )
    check_compliant_parser.add_argument(
        "-p", "--password", action="store", help="password"
    ...
    download_apps_intune_parser = subparsers.add_parser(
        "download_apps",
        help="download available win32apps and scripts (only Windows supported since I'm lazy)",
    )
    download_apps_intune_parser.add_argument(
        "-m", "--mdmpfx", required=True, action="store", help="mdm pfx path"
    )
    download_apps_intune_parser.add_argument(
        "-d", "--device_name", required=True, action="store", help="device name"
    ...
    if args.command == "retire_intune":
        pytune.retire_intune(
            args.os, args.username, args.password, args.refresh_token, args.certpfx
        )
