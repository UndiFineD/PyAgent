# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-XtremeNmapParser\app\app.py
import confuse
from app.modules import OutputFormat as out
from app.modules.NmapParser import *
from app.utils import banner, cli
from app.utils import functions as func
    ...
    if folder_multiple_xml:
        try:
            ...
            if recursive:
                ...
                xml_files = [
                    folder_multiple_xml + i
                    for i in folder_files
                    if i.endswith(NMAP_FILE_EXTENSION)
                ]
            ...
            if merger:
                ...
                if df is not None:
                    ...
                    out.export_multiple_xml(
                        df, list_output_format, file_output_name, merger
                    )
                else:
                    logger.warning(
                        f" |?| Warning | The file has no scan data, omitting export"
                    )
            else:
            ...
            logger.error(
                f" |-| Are you sure that {folder_multiple_xml} is a directory?"
            )
...
def run():
    ...
    try:
        ...
        folder_multiple_xml = (
            func.add_slash_if_needed(args.directory) if args.directory else None
        )
