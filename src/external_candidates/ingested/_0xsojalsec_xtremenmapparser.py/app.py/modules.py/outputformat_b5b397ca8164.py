# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-XtremeNmapParser\app\modules\OutputFormat.py
import confuse
import pandas as pd
...
def df_to_xlsx(df, filename):
    try:
        ...
        worksheet.add_table(
            ...
            max_row,
            max_col - 1,
            {"name": "NmapScanData", "style": TABLE_STYLE, "columns": table_headers},
        )
        # Close XLSX file
        writer.close()
...
def write_dataframe(
    df, list_output_format, file_output_name=None, merger=None, file_xml=None
):
    if df.empty:
