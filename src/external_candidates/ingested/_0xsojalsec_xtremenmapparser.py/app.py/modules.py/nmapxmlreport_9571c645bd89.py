# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-XtremeNmapParser\app\modules\NmapXMLReport.py
import os
...
class NmapXMLReport:
    ...
    class Host:
        def __init__(self, element):
            ...
            self.addresses = [
                NmapXMLReport.Address(e) for e in element.findall("address")
            ]
            self.hostnames = [
                NmapXMLReport.Hostnames(e) for e in element.findall("hostnames")
            ]
            self.ports = [self.Port(e) for e in element.findall("ports/port")]
            self.extraports = [
                self.Extraports(e) for e in element.findall("ports/extraports")
            ]
            ...
            self.tcpsequence = [
                self.TcpSequence(e) for e in element.findall("tcpsequence")
            ]
            self.ipidsequence = [
                self.IpidSequence(e) for e in element.findall("ipidsequence")
            ]
            self.tcptssequence = [
                self.TcptsSequence(e) for e in element.findall("tcptssequence")
            ]
        ...
        class Extraports:
            def __init__(self, element):
                ...
                self.extrareasons = [
                    self.Extrareasons(e) for e in element.findall("extrareasons")
                ]
        ...
        class OS:
            def __init__(self, element):
                self.portused = [
                    self.PortUsed(portused) for portused in element.findall("portused")
                ]
                self.osmatch = [
                    self.OSMatch(osmatch) for osmatch in element.findall("osmatch")
                ]
                self.osfingerprint = [
                    self.OSFingerprint(osfingerprint)
                    for osfingerprint in element.findall("osfingerprint")
                ]
            ...
            class OSMatch:
                def __init__(self, element):
                    ...
                    self.osclass = [
                        self.OSClass(osclass) for osclass in element.findall("osclass")
                    ]
    ...
    class Table:
        def __init__(self, element):
            ...
            self.nested_tables = [
                self.Table(table) for table in element.findall("table")
            ]
    ...
    def __init__(self, xml_file):
        ...
        if NmapXMLReport.validateNmapDTD(self):
            ...
            self.task_progresses = [
                self.TaskProgress(e) for e in root.findall("taskprogress")
            ]
