# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Pi3MOS-SLAM\dpvo\loop_closure\retrieval\retrieval_dbow.py
import os
import time
...
class RetrievalDBOW:
    ...
    def __init__(self, vocab_path="ORBvoc.txt"):
        ...
        ready = Value("i", 0)
        self.proc = Process(
            target=_dbow_loop, args=(self.in_queue, self.out_queue, vocab_path, ready)
        )
        self.proc.start()
    ...
    def _detect_loop(self, thresh, num_repeat=1):
        ...
        dists_sq = [
            (np.square(i - a) + np.square(j - b)) for a, b in self.prev_loop_closes
        ]
