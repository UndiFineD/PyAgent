        ...
        if centroid_sel_strat == "GRADIENT_BIAS":
            ...
            if static_coords.numel() > 0:
                ...
                if K >= patches_per_image:
                    ...
                    gscore = altcorr.patchify(g[0, :, None], coords, 0).view(
                        n, cand_per_frame
                    )
                    ...
                    idx_fill = torch.randint(
                        0, max(K, 1), (n, patches_per_image), device="cuda"
                    )
                    x = xc[idx_fill]
                    y = yc[idx_fill]
            ...
            raise NotImplementedError(
                f"Patch centroid selection not implemented: {centroid_sel_strat}"
            )
...
class VONet(nn.Module):
    ...
    def forward(
        ...
        kk, jj = flatmeshgrid(
            torch.where(ix < 8)[0], torch.arange(0, 8, device="cuda"), indexing="ij"
        )
        ii = ix[kk]
