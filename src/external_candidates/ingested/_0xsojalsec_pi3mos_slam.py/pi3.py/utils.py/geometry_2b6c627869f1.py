    ...
    if torch.is_tensor(T):
        ...
        T_inv = torch.cat(
            [
                ...
                torch.tensor([0, 0, 0, 1], device=T.device, dtype=T.dtype).repeat(
                    T.shape[0], 1, 1
                ),
            ],
            dim=1,
...
def depthmap_to_absolute_camera_coordinates(
    depthmap, camera_intrinsics, camera_pose, z_far=0, **kw
):
    """
    Args:
        - depthmap (HxW array):
...
def opencv_camera_to_plucker(poses, K, H, W):
    ...
    pixel = (
        torch.from_numpy(get_pixel(H, W).astype(np.float32))
        .to(device)
        .T.reshape(H, W, 3)[None]
        .repeat(B, 1, 1, 1)
    )  # (3, H, W)
