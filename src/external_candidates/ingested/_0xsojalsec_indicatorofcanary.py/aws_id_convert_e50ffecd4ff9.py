# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IndicatorOfCanary\aws_id_convert.py
#!/usr/bin/python3
...
parser = argparse.ArgumentParser(
    description="Process AWS Key ID(s) to AWS Account ID(s)"
)
parser.add_argument("-k", "--keyid", type=str, help="Single AWS Key ID to process")
parser.add_argument(
    "-f", "--file", type=str, help="File with AWS Key IDs, one per line"
)
parser.add_argument(
    "--exportcsv",
    nargs="?",
    const="aws_keyid_accountid.csv",
    help="Export to CSV, optional filename",
)
