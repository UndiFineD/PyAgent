# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\dal\message_library.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from sqlalchemy import JSON, Column, DateTime, Index, Integer, String, asc
...
class MessageORM(Base):
    ...
    __table_args__ = (Index("message_ix_session_id", "session_id"),)
...
class MessageLibrary:
    ...
    def get_db_session():
        ...
        system_sqldb_wrapper = SQLDBWrapperManager().get_instance_obj(
            "__system_db__", new_instance=False
        )
    ...
    def get_messages(self, session_id: str) -> list[MessageDO]:
        ...
        with self.get_db_session() as db_session:
            message_orm_list = (
                ...
                .order_by(asc(MessageORM.gmt_modified))
                .all()
            )
            res = []
            if message_orm_list:
    ...
    def __message_orm_to_do(message_orm: MessageORM) -> MessageDO:
        ...
        for column in message_orm.__table__.columns:
            setattr(message_do, column.name, getattr(message_orm, column.name))
        return message_do
