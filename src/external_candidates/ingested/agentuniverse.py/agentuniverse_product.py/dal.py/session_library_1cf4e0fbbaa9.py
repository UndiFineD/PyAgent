# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\dal\session_library.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from sqlalchemy import JSON, Column, DateTime, Index, Integer, String, desc
...
class SessionLibrary:
        ...
        try:
            with self.get_db_session() as db_session:
                session_orm = (
                    ...
                    .filter(SessionORM.session_id == session_do.session_id)
                    .first()
                )
                if session_orm:
    ...
    def get_session_list(self, agent_id: str) -> list[SessionDO]:
        ...
        with self.get_db_session() as db_session:
            session_orm_list = (
                ...
                .order_by(desc(SessionORM.gmt_modified))
                .all()
            )
            res = []
            if session_orm_list:
    ...
    def __session_orm_to_do(session_orm: SessionORM) -> SessionDO:
        ...
        for column in session_orm.__table__.columns:
            setattr(session_do, column.name, getattr(session_orm, column.name))
        return session_do
