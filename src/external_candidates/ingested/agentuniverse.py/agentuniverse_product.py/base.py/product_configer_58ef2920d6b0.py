# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\base\product_configer.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ProductConfiger(ComponentConfiger):
    ...
    def __set_default_meta_info(self):
        ...
        if (
            not hasattr(self, "_ComponentConfiger__metadata_module")
            or self._ComponentConfiger__metadata_module is None
        ):
            self._ComponentConfiger__metadata_module = (
                "agentuniverse_product." "base.product"
            )
        if (
            not hasattr(self, "_ComponentConfiger__metadata_class")
