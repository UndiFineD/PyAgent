# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\knowledge_service\knowledge_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class KnowledgeService:
    ...
    def create_knowledge(knowledge_dto: KnowledgeDTO) -> str:
        ...
        path = get_core_path()
        product_file_path = (
            path / "product" / "knowledge" / f"{product_file_name}.yaml"
            if path
            else os.path.join(
                "..",
                "..",
        ...
        knowledge_file_path = (
            path / "knowledge" / f"{knowledge_id}.yaml"
            if path
            else os.path.join(
                "..",
                "..",
    ...
    def update_knowledge(knowledge_dto: KnowledgeDTO) -> str:
        ...
        knowledge: Knowledge = KnowledgeManager().get_instance_obj(
            component_instance_name=knowledge_dto.id, new_instance=False
        )
        if knowledge is None:
            raise ValueError(
                "The knowledge instance corresponding to the knowledge_id cannot be found."
            )

        product: Product = ProductManager().get_instance_obj(
            component_instance_name=knowledge_dto.id, new_instance=False
        )
        if product is None:
            raise ValueError(
                "The knowledge product instance corresponding to the knowledge_id cannot be found."
            )
        ...
        knowledge_updated_fields = {
            key: value
            for key, value in knowledge_update_config.items()
            if value is not None
        }
        product_updated_fields = {
            key: value
            for key, value in product_update_config.items()
            if value is not None
        }
        ...
        update_nested_yaml_value(
            knowledge.component_config_path, knowledge_updated_fields
        )
    ...
    def delete_knowledge(knowledge_id: str) -> bool:
        ...
        knowledge: Knowledge = KnowledgeManager().get_instance_obj(
            component_instance_name=knowledge_id, new_instance=False
        )
        if knowledge is None:
            raise ValueError(
                "The knowledge instance corresponding to the knowledge_id cannot be found."
    ...
    def upload_knowledge_file(knowledge_id: str, file) -> bool:
        ...
        knowledge_store_file_path = (
            path / "store" / f"{knowledge_id}_store.yaml"
            if path
            else str(
                os.path.join(
                    "..",
        ...
        knowledge = KnowledgeManager().get_instance_obj(
            component_instance_name=knowledge_id, new_instance=False
        )
        ...
        try:
            ...
            if not os.path.isfile(knowledge_store_file_path):
                ...
                update_nested_yaml_value(
                    knowledge.component_config_path,
                    {"stores": [*knowledge_store, knowledge_store_name]},
                )
        ...
        knowledge.insert_knowledge(
            source_path=file_location, stores=[knowledge_store_name]
        )
