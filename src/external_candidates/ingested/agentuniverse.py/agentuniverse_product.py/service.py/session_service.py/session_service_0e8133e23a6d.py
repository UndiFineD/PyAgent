# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\session_service\session_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class SessionService:
    ...
    def update_session(session_id: str, agent_id: str, update_time: datetime) -> str:
        ...
        return SessionLibrary().update_session(
            SessionDO(
                session_id=session_id, agent_id=agent_id, gmt_modified=update_time
            )
        )
    ...
    def get_session_list(agent_id: str) -> List[SessionDTO]:
        ...
        for session_do in session_do_list:
            message_do_list: List[MessageDO] = MessageLibrary().get_messages(
                session_do.session_id
            )
            session_messages_map[session_do.session_id] = message_do_list
        return SessionService().convert_to_session_dto(
            session_do_list, session_messages_map
        )
    ...
    def get_session_detail(id: str, top_k: int = None) -> SessionDTO | None:
        ...
        return SessionService().convert_to_session_dto(
            [session_do], {session_do.session_id: message_do_list}
        )[0]

    @staticmethod
    def convert_to_session_dto(
        session_do_list: List[SessionDO],
        session_messages_map: dict[str, List[MessageDO]],
    ) -> List[SessionDTO]:
        ...
        for session_do in session_do_list:
            session_dto = SessionDTO(
                id=session_do.session_id,
                agent_id=session_do.agent_id,
            ...
            for message_do in message_do_list:
                message_dto = MessageDTO(
                    id=message_do.id,
                    session_id=session_do.session_id,
