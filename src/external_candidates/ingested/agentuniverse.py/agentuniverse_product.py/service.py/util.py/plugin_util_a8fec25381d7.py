# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\util\plugin_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def validate_create_plugin_parameters(plugin_dto: PluginDTO) -> None:
    ...
    if plugin:
        raise ValueError(
            "Plugin instance corresponding to the plugin id already exists."
        )
    if plugin_dto.openapi_desc is None:
        raise ValueError("The openapi_desc in plugin cannot be None.")


def assemble_plugin_product_config_data(
    plugin_dto: PluginDTO, tool_id_list: List[str]
) -> Dict:
