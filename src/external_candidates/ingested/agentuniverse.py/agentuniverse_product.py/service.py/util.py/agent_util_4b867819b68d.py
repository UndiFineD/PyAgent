...
def assemble_agent_config_data(agent_dto: AgentDTO) -> Dict:
    ...
    if agent_dto.llm:
        ...
        if llm is None:
            raise ValueError(
                "The llm instance corresponding to the llm id cannot be found."
            )
    ...
    if agent_dto.knowledge:
        agent_config_data["action"]["knowledge"] = [
            knowledge.id for knowledge in agent_dto.knowledge
        ]
...
def register_agent(file_path: str):
    ...
    agent_configer: AgentConfiger = AgentConfiger().load_by_configer(
    ...
    component_instance: Agent = component_clz().initialize_by_component_configer(
        agent_configer
    )
...
def register_product(file_path: str):
    ...
    configer = Configer(path=absolute_file_path).load()
    component_configer = ComponentConfiger().load_by_configer(configer)
    product_configer: ProductConfiger = ProductConfiger().load_by_configer(
        component_configer.configer
    )
    component_clz = (
        ComponentConfigerUtil.get_component_object_clz_by_component_configer(
    ...
    component_instance: Product = component_clz().initialize_by_component_configer(
        product_configer
    )
    component_instance.component_config_path = component_configer.configer.path
...
def get_planner_dto(agent: Agent) -> PlannerDTO | None:
    ...
    if not planner_name:
        return None
    workflow_id = (
        planner.get("workflow_id")
        if planner.get("workflow_id")
        else agent_model.profile.get("workflow_id")
    )
    members = None
    if planner_name == "peer_planner":
        members = assemble_peer_planner_members(planner, agent_model)
    return PlannerDTO(
        nickname="", id=planner_name, members=members, workflow_id=workflow_id
    )


def assemble_peer_planner_members(
    planner: dict, agent_model: AgentModel
) -> list[AgentDTO]:
    ...
    try:
        planner_data: dict = (
            agent_model.profile
            if dict_does_not_contain_keys(planner, member_keys)
            else planner
        )
...
def get_knowledge_dto_list(agent_model: AgentModel) -> List[KnowledgeDTO]:
    ...
    for knowledge_name in knowledge_name_list:
        product: Product = ProductManager().get_instance_obj(
            knowledge_name, new_instance=False
        )
        ...
        knowledge_dto = KnowledgeDTO(
            nickname=product.nickname if product else "", id=knowledge_name
        )
...
def get_llm_dto(agent_model: AgentModel) -> LlmDTO | None:
    ...
    if llm is None:
        return None
    llm_model_name = (
        llm_model.get("model_name") if llm_model.get("model_name") else llm.model_name
    )
    llm_temperature = (
        llm_model.get("temperature")
        if llm_model.get("temperature")
...
def update_agent_config(
    agent: Agent, agent_dto: AgentDTO, agent_config_path: str
) -> None:
    ...
    if agent_dto.llm is not None:
        ...
        if llm_dto.temperature:
            ...
            agent.agent_model.profile.get("llm_model")[
                "temperature"
            ] = agent_dto.llm.temperature
        if llm_dto.model_name:
            agent_updates["profile.llm_model.model_name"] = agent_dto.llm.model_name[0]
            agent.agent_model.profile.get("llm_model")["model_name"] = (
                agent_dto.llm.model_name[0]
            )
...
def update_agent_product_config(
    agent_product: Product, agent_dto: AgentDTO, product_config_path: str
) -> None:
    ...
    update_nested_yaml_value(
        product_config_path, {"opening_speech": agent_dto.opening_speech}
    )
...
def validate_and_assemble_agent_input(
    agent_id: str, session_id: str, input: str, chat_history: list = None
) -> dict:
    ...
    if agent is None:
        raise ValueError(
            "The agent instance corresponding to the agent id cannot be found."
        )
