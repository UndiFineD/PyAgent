# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\util\knowledge_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def register_knowledge(file_path: str):
    ...
    knowledge_configer: KnowledgeConfiger = KnowledgeConfiger().load_by_configer(
        component_configer.configer
    )
    component_clz = (
        ComponentConfigerUtil.get_component_object_clz_by_component_configer(
            knowledge_configer
        )
    )
    component_instance: Knowledge = component_clz().initialize_by_component_configer(
        knowledge_configer
    )
    component_instance.component_config_path = component_configer.configer.path
    KnowledgeManager().register(
...
def register_store(file_path: str):
    ...
    component_instance = component_clz().initialize_by_component_configer(
        component_configer
    )
    component_instance.component_config_path = component_configer.configer.path
