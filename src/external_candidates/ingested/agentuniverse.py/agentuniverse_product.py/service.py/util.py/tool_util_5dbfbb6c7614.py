...
def validate_create_api_tool_parameters(tool_dto: ToolDTO) -> None:
    ...
    if tool:
        raise ValueError(
            f"Tool instance corresponding to the tool id already exists. {tool_dto.id}"
        )
...
def register_tool(file_path: str):
    ...
    tool_configer: ToolConfiger = ToolConfiger().load_by_configer(
    ...
    component_instance: Tool = component_clz().initialize_by_component_configer(
        tool_configer
    )
