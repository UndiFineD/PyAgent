# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\plugin_service\plugin_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse_product.service.util.plugin_util import (
    ...
    validate_create_plugin_parameters,
)
    ...
    def create_plugin_with_openapi(plugin_dto: PluginDTO) -> str:
        ...
        for tool in tool_bundles:
            ...
            tool_dto = ToolDTO(
                id=tool_id,
                nickname=(
                    plugin_dto.nickname + "_tool_" + str(index)
                    if plugin_dto.nickname
                    else ""
                ),
        ...
        product_config_data = assemble_plugin_product_config_data(
            plugin_dto, tool_id_list
        )
        # write product YAML file
        product_file_name = f"{plugin_dto.id}_product"
        path = get_core_path()
        product_file_path = (
            path / "product" / "plugin" / f"{product_file_name}.yaml"
            if path
            else os.path.join(
                "..",
                "..",
