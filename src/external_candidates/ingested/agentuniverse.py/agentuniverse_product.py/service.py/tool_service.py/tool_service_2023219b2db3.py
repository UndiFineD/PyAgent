# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\tool_service\tool_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ToolService:
    ...
    def get_tool_list() -> List[ToolDTO]:
        ...
        for product in product_list:
            if product.type == ComponentEnum.TOOL.value:
                tool_dto = ToolDTO(
                    nickname=product.nickname, avatar=product.avatar, id=product.id
                )
    ...
    def create_tool(tool_dto: ToolDTO) -> str:
        ...
        path = get_core_path()
        product_file_path = (
            path / "product" / "tool" / f"{product_file_name}.yaml"
            if path
            else os.path.join(
                "..",
                "..",
        ...
        tool_file_path = (
            path / "tool" / f"{tool_dto.id}.yaml"
            if path
            else os.path.join(
                "..", "..", "intelligence", "agentic", "tool", f"{tool_dto.id}.yaml"
            )
        )
