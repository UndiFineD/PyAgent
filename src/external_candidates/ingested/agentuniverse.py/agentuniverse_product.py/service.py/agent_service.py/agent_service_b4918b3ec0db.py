# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\agent_service\agent_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.agent_serve.web.web_util import (
    agent_run_queue,
    async_agent_run_queue,
)
from agentuniverse.base.component.component_enum import ComponentEnum
from agentuniverse.base.util.monitor.monitor import Monitor
from agentuniverse_product.base.product_manager import Product, ProductManager
...
from agentuniverse_product.service.util.agent_util import (
    assemble_agent_config_data,
    assemble_agent_dto,
    assemble_product_config_data,
    register_agent,
    register_product,
    update_agent_config,
    update_agent_product_config,
    validate_and_assemble_agent_input,
    validate_create_agent_parameters,
)
...
class AgentService:
    ...
    def update_agent(agent_dto: AgentDTO) -> None:
        ...
        if product:
            update_agent_product_config(
                product, agent_dto, product.component_config_path
            )
    ...
    def stream_chat(agent_id: str, session_id: str, input: str) -> Iterator:
        ...
        Returns:
            Iterator: Response.
        """
        agent_input_dict = validate_and_assemble_agent_input(
            agent_id,
            session_id,
            input,
        ...
        # invoke agent
        start_time = time.time()
        task = RequestTask(agent_run_queue, False, **agent_input_dict)
        ...
        for chunk in task.stream_run():
            ...
            if "process" in chunk_dict:
                ...
                if data:
                    ...
                    yield {
                        "output": data.get(
                            "chunk" if yield_type == "token" else "output"
                        ),
                        "type": yield_type,
    ...
    async def async_stream_chat(
        agent_id: str, session_id: str, input: str
    ) -> AsyncIterator:
    ...
    def create_agent(agent_dto: AgentDTO) -> str:
        ...
        path = get_core_path()
        product_file_path = (
            path / "product" / "agent" / f"{product_file_name}.yaml"
            if path
            else os.path.join(
                "..",
                "..",
        ...
        agent_file_path = (
            path / "agent" / f"{agent_dto.id}.yaml"
            if path
            else os.path.join(
                "..", "..", "intelligence", "agentic", "agent", f"{agent_dto.id}.yaml"
            )
        )
    ...
    def add_agent_chat_history(
        agent_id: str, session_id: str, input: str, output: str, cur_time: datetime
    ) -> Tuple[str, int]:
        """Add agent chat history to session and message db tables."""
        content = json.dumps(
            [{"type": "human", "content": input}, {"type": "ai", "content": output}],
            ensure_ascii=False,
        )
