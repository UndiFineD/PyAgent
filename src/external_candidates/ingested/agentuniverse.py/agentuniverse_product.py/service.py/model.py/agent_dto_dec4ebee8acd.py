# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\model\agent_dto.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class AgentDTO(BaseModel):
    ...
    opening_speech: Optional[str] = Field(
        description="agent opening speech", default=""
    )
    ...
    knowledge: Optional[list[KnowledgeDTO]] = Field(
        description="agent knowledge list", default=[]
    )
    mtime: Optional[float] = Field(
        description="product last modification time.", default=None
    )
