# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse_product\service\workflow_service\workflow_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class WorkflowService:
    ...
    def create_workflow(workflow_dto: WorkflowDTO) -> str:
        ...
        path = get_core_path()
        workflow_file_path = (
            path / "workflow" / f"{workflow_id}.yaml"
            if path
            else os.path.join(
                "..", "..", "platform", "difizen", "workflow", f"{workflow_id}.yaml"
            )
        )
    ...
    def update_workflow(workflow_dto: WorkflowDTO) -> str:
        ...
        workflow: Workflow = WorkflowManager().get_instance_obj(
            component_instance_name=workflow_dto.id, new_instance=False
        )
        if workflow is None:
            raise ValueError(
                "The workflow instance corresponding to the workflow_id cannot be found."
            )
        ...
        updated_fields = {
            key: value
            for key, value in workflow_update_config.items()
    ...
    def get_workflow_detail(id: str) -> WorkflowDTO | None:
        ...
        """
        workflow: Workflow = WorkflowManager().get_instance_obj(
            component_instance_name=id, new_instance=False
        )
        if workflow is None:
            raise ValueError(
                "The workflow instance corresponding to the workflow_id cannot be found."
