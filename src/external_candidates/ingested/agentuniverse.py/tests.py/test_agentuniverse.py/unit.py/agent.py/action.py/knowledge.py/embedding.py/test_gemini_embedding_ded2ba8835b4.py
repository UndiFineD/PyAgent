# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\knowledge\embedding\test_gemini_embedding.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.agent.action.knowledge.embedding.gemini_embedding import (
    GeminiEmbedding,
)
...
class GeminiEmbeddingTest(unittest.TestCase):
    ...
    def test_async_get_embeddings(self) -> None:
        res = asyncio.run(
            self.embedding.async_get_embeddings(texts=["hello world", "agentUniverse"])
        )
        print(res)
        self.assertEqual(len(res), 2)
        self.assertEqual(len(res[0]), 768)
    ...
    def test_as_langchain(self) -> None:
        ...
        res = langchain_embedding.embed_documents(
            texts=["hello world", "agentUniverse"]
        )
        print(res)
        self.assertEqual(len(res), 2)
        self.assertEqual(len(res[0]), 768)
