# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\knowledge\reader\file\test_zip_reader.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class TestZipReader(unittest.TestCase):
    ...
    def test_complex_nested_zip_structure(self) -> None:
        ...
        with zipfile.ZipFile(archive_path, "w") as main_zip:
            ...
            main_zip.writestr(
                "web/index.html", "<html><body><h1>欢迎</h1></body></html>"
            )
        ...
        nested_docs = [
            d for d in docs if "level2.zip" in d.metadata.get("archive_path", "")
        ]
        self.assertGreater(len(nested_docs), 0)

        deep_nested = [
            d for d in docs if "level3.zip" in d.metadata.get("archive_path", "")
        ]
    ...
    def test_nested_zip(self) -> None:
        ...
        self.assertEqual(
            doc.metadata["archive_path"], "folder/archive.zip/inner/data.txt"
        )
    ...
    def test_custom_metadata(self) -> None:
        ...
        custom_meta = {"source": "测试来源", "category": "文档类别", "priority": "高"}
    ...
    def test_ultra_complex_nested_structure(self) -> None:
        ...
        level4_docs = [
            d for d in docs if "level4.zip" in d.metadata.get("archive_path", "")
        ]
    ...
    def test_mixed_documents_extraction(self) -> None:
        ...
        with zipfile.ZipFile(archive_path, "w") as archive:
            ...
            archive.writestr(
                "index.html", "<html><body><h1>标题</h1><p>段落</p></body></html>"
            )
            archive.writestr(
    ...
    def test_unicode_content(self) -> None:
        ...
        chinese_doc = [
            d for d in docs if "chinese.txt" in d.metadata.get("file_name", "")
        ][0]
    ...
    def test_json_and_yaml_in_nested_zip(self) -> None:
        ...
        json_docs = [
            d for d in docs if d.metadata.get("file_name", "").endswith(".json")
        ]
    ...
    def test_metadata_propagation_through_nesting(self) -> None:
        ...
        custom_meta = {"project": "测试项目", "version": "1.0", "author": "测试者"}
