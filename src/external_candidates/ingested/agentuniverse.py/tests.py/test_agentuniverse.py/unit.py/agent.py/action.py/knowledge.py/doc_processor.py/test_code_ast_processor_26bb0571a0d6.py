# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\knowledge\doc_processor\test_code_ast_processor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.agent.action.knowledge.doc_processor.code_ast_processor import (
    CodeAstProcessor,
)
...
public:
    ...
    int getValue() const {
        ...
        with open(
            os.path.join(self.temp_dir.name, "test.py"), "w", encoding="utf-8"
        ) as f:
            f.write(self.py_code)
        with open(
            os.path.join(self.temp_dir.name, "test.java"), "w", encoding="utf-8"
        ) as f:
            f.write(self.java_code)
        with open(
            os.path.join(self.temp_dir.name, "test.cpp"), "w", encoding="utf-8"
        ) as f:
    ...
    def test_process_python(self):
        doc = Document(
            text=self.py_code, metadata={"language": "python", "file_name": "test.py"}
        )
    ...
    def test_process_java(self):
        doc = Document(
            text=self.java_code, metadata={"language": "java", "file_name": "test.java"}
        )
        result_docs = self.processor._process_docs([doc])
    ...
    def test_process_cpp(self):
        doc = Document(
            text=self.cpp_code, metadata={"language": "cpp", "file_name": "test.cpp"}
        )
        result_docs = self.processor._process_docs([doc])
