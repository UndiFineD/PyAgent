# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\knowledge\reader\file\test_sevenzip_reader.py
import os
import shutil
import tempfile
import time
...
from agentuniverse.agent.action.knowledge.reader.file.sevenzip_reader import (
    SevenZipReader,
)
...
class TestSevenZipReaderBasic(unittest.TestCase):
    ...
    def create_test_files(self):
        ...
        with open(test_json_path, "w", encoding="utf-8") as f:
            f.write(
                """{"app_name": "7Z Reader Test", "version": "1.0.0", "features": ["compression", "extraction", "metadata"]}"""
            )
        ...
        try:
            if password:
                with py7zr.SevenZipFile(
                    test_7z_path, "w", password=password
                ) as archive:
    ...
    def test_load_data_with_custom_metadata(self):
        ...
        custom_metadata = {
            ...
            "test_scenario": "metadata_validation",
        }
...
class TestSevenZipReaderComplexScenarios(unittest.TestCase):
    ...
    def create_complex_project_structure(self):
        ...
        with open(
            os.path.join(project_dir, "src", "main.py"), "w", encoding="utf-8"
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "src", "models", "processor.py"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "tests", "unit", "test_processor.py"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "tests", "integration", "test_main.py"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "config", "environments", "development.yaml"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "config", "environments", "production.yaml"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "docs", "api", "rest_api.md"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "docs", "tutorials", "getting_started.md"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(project_dir, "data", "raw", "sample_data.json"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(
                """[{"id": 1, "name": "项目A", "value": 100}, {"id": 2, "name": "项目B", "value": 200}, {"id": 3, "name": "项目C", "value": 300}]"""
            )
        with open(
            os.path.join(project_dir, "data", "raw", "users.csv"), "w", encoding="utf-8"
        ) as f:
            f.write(
    ...
    def create_nested_7z_structure(self):
        ...
        with open(
            os.path.join(level3_dir, "deep_config.json"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """{"level": 3, "description": "最深层的配置文件", "security": {"encryption": true, "access_level": "high"}}"""
            )
        with open(
            os.path.join(level3_dir, "secret_data.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(level2_dir, "middle_document.md"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """# 中层文档\n这是第二层的文档文件。\n## 包含内容\n- 业务逻辑说明\n- 下一层的压缩包\n- 配置信息"""
            )
        ...
        with open(
            os.path.join(level2_dir, "business_rules.yaml"), "w", encoding="utf-8"
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(level1_dir, "project_overview.txt"), "w", encoding="utf-8"
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(level1_dir, "system_config.xml"), "w", encoding="utf-8"
        ) as f:
            f.write(
    ...
    def test_nested_7z_archives(self):
        ...
        deep_files = [
            doc for doc in documents if doc.metadata.get("archive_depth", 0) > 1
        ]
    ...
    def test_multiple_file_types_and_encodings(self):
        ...
        chinese_files = [
            doc for doc in documents if "中文" in doc.metadata.get("file_name", "")
        ]
...
class TestSevenZipReaderSizeLimits(unittest.TestCase):
    ...
    def test_max_file_size_limit(self):
        ...
        with open(large_file, "w", encoding="utf-8") as f:
            for i in range(20000):
                f.write(
                    f"Line {i}: 这是用于测试大文件处理的数据行，包含一些变化内容以避免过度压缩。 {i * 123}\n"
                )
    ...
    def test_compression_ratio_detection(self):
        ...
        try:
            documents = self.reader._load_data(
                sevenzip_path, max_compression_ratio=10.0
            )
            self.assertIsInstance(documents, list)
...
class TestSevenZipReaderRealWorldScenarios(unittest.TestCase):
    ...
    def create_software_distribution_package(self):
        ...
        with open(
            os.path.join(dist_dir, "bin", "install.py"), "w", encoding="utf-8"
        ) as f:
            f.write(
        ...
        with open(
            os.path.join(dist_dir, "docs", "manual.html"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """<!DOCTYPE html>\n<html>\n<head>\n    <title>MyApp 用户手册</title>\n</head>\n<body>\n    <h1>MyApp 用户手册</h1>\n    <h2>快速开始</h2>\n    <ol>\n        <li>安装软件</li>\n        <li>配置参数</li>\n        <li>启动应用</li>\n    </ol>\n    <h2>功能特性</h2>\n    <ul>\n        <li>高性能数据处理</li>\n        <li>插件系统</li>\n        <li>多平台支持</li>\n    </ul>\n</body>\n</html>"""
            )
        with open(
            os.path.join(dist_dir, "config", "app_config.yaml"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """# MyApp 配置文件模板\n# 复制此文件为 config.yaml 并修改相应配置\n\napplication:\n  name: "MyApp"\n  version: "2.0.0"\n  debug: false\n  log_level: "INFO"\n\ndatabase:\n  host: "localhost"\n  port: 5432\n  name: "myapp_db"\n  username: "db_user"\n  # password: "请在此设置密码"\n\nserver:\n  host: "0.0.0.0"\n  port: 8080\n  ssl_enabled: false\n\nplugins:\n  enabled: true\n  directory: "./plugins" """
            )
        with open(
            os.path.join(dist_dir, "examples", "basic_usage.py"), "w", encoding="utf-8"
        ) as f:
            f.write(
    ...
    def test_software_distribution_package(self):
        ...
        install_docs = [
            d for d in documents if "INSTALL" in d.metadata.get("file_name", "")
        ]
        self.assertGreater(len(install_docs), 0)
        config_docs = [
            d for d in documents if "config" in d.metadata.get("file_name", "").lower()
        ]
    ...
    def test_documentation_archive_with_metadata(self):
        ...
        with open(
            os.path.join(doc_dir, "api", "rest_api.md"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """# REST API 参考\n## 认证端点\n### POST /auth/login\n用户登录\n### GET /auth/logout\n用户登出\n## 数据端点\n### GET /api/data\n获取数据列表\n### POST /api/data\n创建新数据"""
            )
        with open(
            os.path.join(doc_dir, "tutorials", "quick_start.md"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """# 快速入门教程\n## 第一步：环境准备\n安装 Python 和必要依赖。\n## 第二步：配置应用\n修改配置文件。\n## 第三步：运行测试\n执行测试命令验证安装。"""
            )
        with open(
            os.path.join(doc_dir, "guides", "development.md"), "w", encoding="utf-8"
        ) as f:
            f.write(
                """# 开发指南\n## 代码规范\n- 遵循 PEP 8\n- 编写单元测试\n- 使用类型注解\n## 提交规范\n- 清晰的提交信息\n- 关联问题编号\n- 通过所有测试"""
            )
...
class TestSevenZipReaderEdgeCases(unittest.TestCase):
    ...
    def test_deep_directory_structure(self):
        ...
        mid_file = os.path.join(
            self.temp_dir, "level_00", "level_01", "mid_level_file.yaml"
        )
        ...
        deep_files = [
            doc
            for doc in documents
            if "deep_nested_file" in doc.metadata.get("file_name", "")
        ]
    ...
    def test_path_handling_with_different_types(self):
        ...
        if docs_str and docs_path:
            ...
            self.assertEqual(
                docs_str[0].metadata["file_name"], docs_path[0].metadata["file_name"]
            )
    ...
    def test_mixed_text_encodings_and_formats(self):
        ...
        os.makedirs(files_dir, exist_ok=True)
        with open(
            os.path.join(files_dir, "utf8_bom.txt"), "w", encoding="utf-8-sig"
        ) as f:
            f.write("UTF-8 with BOM: 测试文本")
        with open(
            os.path.join(files_dir, "utf8_nobom.txt"), "w", encoding="utf-8"
        ) as f:
            f.write("UTF-8 without BOM: 测试文本")
        with open(
            os.path.join(files_dir, "unicode_chars.txt"), "w", encoding="utf-8"
        ) as f:
...
class TestSevenZipReaderPerformance(unittest.TestCase):
    ...
    def test_large_number_of_small_files(self):
        ...
        self.assertLess(
            elapsed_time,
            30,
            f"处理 {file_count} 个文件耗时 {elapsed_time:.2f} 秒，超过性能要求",
        )
        print(
