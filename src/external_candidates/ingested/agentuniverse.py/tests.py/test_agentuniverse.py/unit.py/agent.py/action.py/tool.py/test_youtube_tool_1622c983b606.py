# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\tool\test_youtube_tool.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.agent.action.tool.common_tool.youtube_tool import Mode, YouTubeTool
...
class YouTubeToolTest(unittest.TestCase):
    ...
    def test_search_videos(self) -> None:
        tool_input = ToolInput(
            {"mode": Mode.VIDEO_SEARCH.value, "input": "machine learning"}
        )
    ...
    def test_analyze_channel(self) -> None:
        tool_input = ToolInput(
            {"mode": Mode.CHANNEL_INFO.value, "input": "UC_x5XG1OV2P6uZZ5FSM9Ttw"}
        )
        result = self.tool.execute(tool_input.mode, tool_input.input)
        self.assertTrue(result != {})

    def test_get_trending_videos_with_region(self) -> None:
        tool_input = ToolInput({"mode": Mode.TRENDING_VIDEOS.value, "input": "US"})
        result = self.tool.execute(tool_input.mode, tool_input.input)
        self.assertTrue(result != [])

    def test_get_trending_videos(self) -> None:
        tool_input = ToolInput({"mode": Mode.TRENDING_VIDEOS.value})
