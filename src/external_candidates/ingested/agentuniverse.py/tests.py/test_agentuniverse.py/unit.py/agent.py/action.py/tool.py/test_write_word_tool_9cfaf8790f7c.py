# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\tool\test_write_word_tool.py
import json
import os
import tempfile
import unittest

from agentuniverse.agent.action.tool.common_tool.write_word_tool import (
    WriteWordDocumentTool,
)
...
class WriteWordDocumentToolTest(unittest.TestCase):
    ...
    def test_write_new_word_file(self):
        ...
        result_json = self.tool.execute(
            file_path=file_path, content=content, append=False
        )
    ...
    def test_append_to_word_file(self):
        ...
        result_json = self.tool.execute(
            file_path=file_path, content=append_content, append=True
        )
        result = json.loads(result_json)
    ...
    def test_create_directory_structure(self):
        ...
        self.assertTrue(os.path.exists(file_path))
        self.assertTrue(
            os.path.isdir(os.path.join(self.temp_dir, "nested/dir/structure"))
        )
    ...
    def test_missing_dependency(self):
        ...
        try:
            ...
            self.assertEqual(result["status"], "error")
            self.assertIn(
                "python-docx is required to write Word documents", result["error"]
            )
        finally:
