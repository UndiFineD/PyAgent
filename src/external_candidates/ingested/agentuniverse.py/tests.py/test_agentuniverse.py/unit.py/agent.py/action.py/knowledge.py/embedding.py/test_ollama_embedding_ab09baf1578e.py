# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\tests\test_agentuniverse\unit\agent\action\knowledge\embedding\test_ollama_embedding.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
#
...
import httpx
from agentuniverse.agent.action.knowledge.embedding.ollama_embedding import (
    OllamaEmbedding,
)
...
class OllamaEmbeddingTest(unittest.TestCase):
        ...
        try:
            ...
            for model in models_to_test:
                with self.subTest(model=model):
                    ...
                    print(
                        f"Model {model} - Embedding shape: {len(res[0]) if res and res[0] else 'None'}"
                    )
    ...
    def test_async_get_embeddings_real_api(self) -> None:
        ...
        try:
            ...
            print(
                f"Async embedding result shape: {len(res[0]) if res and res[0] else 'None'}"
            )
    ...
    def test_as_langchain(self) -> None:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"LangChain embedding result shape: {len(res[0]) if res and res[0] else 'None'}"
                )
                self.assertIsInstance(res, list)
            except Exception as e:
