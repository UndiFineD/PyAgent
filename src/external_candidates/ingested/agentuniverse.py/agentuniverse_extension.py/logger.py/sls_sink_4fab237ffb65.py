...
class AsyncSlsSender:
    def __init__(
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="sls_uploader"
        )
            ...
            while not self._shutdown.is_set():
                ...
                try:
                    ...
                    await asyncio.wait_for(
                        self._shutdown.wait(), timeout=self._send_interval
                    )
                except asyncio.TimeoutError:
                    pass
            # 程序即将退出，把残余也发送掉
...
class SlsSender:
    ...
    def _send_put_logs_request(
        ...
        """
        try:
            put_request = PutLogsRequest(
                self.project, self.log_store, topic, source, log_item_list
            )
    ...
    def send_single_log(
        self, message: str, topic: str = "", source: str = ""
    ) -> Optional[PutLogsResponse]:
...
class AsyncSlsSink:
    ...
    def __call__(self, message):
        ...
        item = LogItem(
            contents=[("content", message)], timestamp=int(record["time"].timestamp())
        )
