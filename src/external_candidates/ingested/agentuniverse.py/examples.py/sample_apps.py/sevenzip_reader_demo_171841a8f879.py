# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\sevenzip_reader_demo.py
import os
import shutil
import tempfile
...
def create_sample_7z_archive():
    ...
    # 创建文档文件
    with open(
        os.path.join(temp_dir, "docs", "introduction.txt"), "w", encoding="utf-8"
    ) as f:
    ...
    # 创建配置文件
    with open(
        os.path.join(temp_dir, "config", "settings.json"), "w", encoding="utf-8"
    ) as f:
        f.write("""{
            "app_name": "7Z读取器演示",
            "version": "2.0.0",
    ...
    # 创建YAML配置文件
    with open(
        os.path.join(temp_dir, "config", "app_config.yaml"), "w", encoding="utf-8"
    ) as f:
        f.write("""# 应用程序配置
                app:
                name: "7Z Reader Demo"
    ...
    # 创建数据文件
    with open(
        os.path.join(temp_dir, "data", "employees.csv"), "w", encoding="utf-8"
    ) as f:
    ...
    # 创建XML数据文件
    with open(
        os.path.join(temp_dir, "data", "products.xml"), "w", encoding="utf-8"
    ) as f:
...
def demo_statistics(sevenzip_path):
    ...
    try:
        ...
        print(
            f"  平均文档长度: {total_chars // len(documents) if documents else 0} 字符"
        )
        ...
        if documents:
            ...
            print(f"\n文档大小:")
            print(
                f"  最大: {max_doc.metadata.get('file_name')} ({len(max_doc.text)} 字符)"
            )
            print(
                f"  最小: {min_doc.metadata.get('file_name')} ({len(min_doc.text)} 字符)"
            )
...
def demo_nested_7z(temp_dir):
    ...
    try:
        ...
        with open(
            os.path.join(level3_dir, "深层配置.json"), "w", encoding="utf-8"
        ) as f:
            f.write("""{
                "level": 3,
        ...
        with open(
            os.path.join(level2_dir, "业务数据.yaml"), "w", encoding="utf-8"
        ) as f:
            f.write("""departments:
                - name: "技术部"
                    employees: 25
...
def demo_advanced_features(sevenzip_path):
    ...
    try:
        ...
        for keyword, count in sorted(
            common_keywords.items(), key=lambda x: x[1], reverse=True
        ):
            if count > 0:
        ...
        config_docs = [
            doc
            for doc in documents
            if "config" in doc.metadata.get("archive_path", "").lower()
        ]
        code_docs = [
            doc
            for doc in documents
            if any(
                ext in doc.metadata.get("file_suffix", "")
                for ext in [".py", ".json", ".yaml", ".xml"]
            )
        ]
        data_docs = [
            doc
            for doc in documents
            if "data" in doc.metadata.get("archive_path", "").lower()
        ]
...
if __name__ == "__main__":
    ...
    if sevenzip_path and os.path.exists(sevenzip_path):
        try:
            ...
            print("\n使用提示:")
            print(
                "  1. SevenZipReader 自动支持多种文件格式（Python、JSON、XML、YAML等）"
            )
