# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\discussion_group_app\intelligence\agentic\agent\agent_template\discussion_group_template.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class DiscussionGroupTemplate(AgentTemplate):
    ...
    def agents_run(
        self, participant_agents: dict, agent_input: dict, input_object: InputObject
    ) -> dict:
        ...
        for i in range(total_round):
            ...
            for participant_agent_name, participant_agent in participant_agents.items():
                ...
                output_object: OutputObject = participant_agent.run(
                ...
                shared_memory_list = assemble_memory_output(
                    host_agent_memory,
                    agent_input,
                    memory_content,
                ...
                stream_output(
                    input_object.get_data("output_stream"),
                    {
                        "data": {
    ...
    def initialize_by_component_configer(
        ...
        self.prompt_version = self.agent_model.profile.get(
            "prompt_version", "discussion_group_agent.cn"
        )
        return self
