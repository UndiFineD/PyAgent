# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\basic_sop_app\intelligence\agentic\agent\agent_instance\recommend_sop_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class RecommendSopAgent(AgentTemplate):
    ...
    def parse_input(self, input_object: InputObject, agent_input: dict) -> dict:
        ...
        LOGGER.info(
            f"agent, agent input: {agent_input}, input_object: {input_object.to_dict()}"
        )
        return agent_input
    ...
    def customized_execute(
        ...
        **kwargs,
        ...
        choose_product_info_agent: Agent = AgentManager().get_instance_obj(
            "choose_product_info_agent"
        )
        choose_product_info_agent_res: OutputObject = choose_product_info_agent.run(
            **input_object.to_dict()
        )
        LOGGER.info(
            f"choose_product_info_agent_res: {choose_product_info_agent_res.to_dict()}"
        )
        ...
        product_description_dict: dict = (
            ToolManager()
            .get_instance_obj("product_info_tool")
            .run(input=product_info_item_list)
        )
        ...
        choose_product_agent: Agent = AgentManager().get_instance_obj(
            "choose_product_agent"
        )
        if not choose_product_agent:
            raise ValueError("No choose_product_agent found")
        choose_product_agent_res: OutputObject = choose_product_agent.run(
            **input_object.to_dict()
        )
        ...
        for product in product_list:
            ...
            product_description_list += (
                product_info.PRODUCT_NAME_MAP.get(product) + "\n"
            )
            product_description_list += product_description_dict.get(product) + "\n"
            product_recommendation += (
                product_info.PRODUCT_RECOMMENDATION_MAP.get(product) + "\n"
            )
        ...
        process_llm_token(
            llm, prompt.as_langchain(), self.agent_model.profile, agent_input
        )
