# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\basic_sop_app\intelligence\agentic\agent\agent_instance\choose_product_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ChooseProductAgent(AgentTemplate):
    ...
    def process_prompt(self, agent_input: dict, **kwargs) -> ChatPrompt:
        ...
        if "医疗险" in input:
            version_prompt: Prompt = PromptManager().get_instance_obj(
                "choose_product_agent_v2.cn"
            )
        else:
            version_prompt: Prompt = PromptManager().get_instance_obj(
                self.prompt_version
