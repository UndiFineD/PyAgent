...
class DatasetBuildAgent(Agent):
    ...
    executor: Optional[Any] = ThreadPoolExecutor(
        max_workers=1, thread_name_prefix="data_agent"
    )
    ...
    def execute(self, input_object: InputObject, agent_input: dict):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=(
                profile.get("concurrency_level")
                if profile.get("concurrency_level")
                else 1
            ),
    ...
    def build_dataset(self, agent_input: dict) -> List[List[Tuple[str, str]]]:
        ...
        if not candidate_agent:
            raise ValueError(
                "The agent instance corresponding to `candidate` parameter is empty"
            )
        ...
        for i in range(agent_input.get("turn")):
            LOGGER.info("-------------------------------------------")
            LOGGER.info(
                f"Start: build q&a dataset from the candidate agent `{candidate_agent_name}`, turn {i + 1}."
            )
            ...
            for future in done:
                ...
                LOGGER.info(
                    f"Progress: the turn {i + 1} query: `{query}` has generated the answer successfully."
                )
