...
class DatasetEvalAgent(Agent):
    ...
    def execute(self, input_object: InputObject, agent_input: dict):
        ...
        query_answer_list: List[List[Tuple[str, str]]] = input_object.get_data(
            "query_answer_list"
        )
        ...
        eval_dims_json_list: List[List[dict]] = self.eval(
            query_answer_list, max_eval_lines
        )
        LOGGER.info(f"End: evaluate the dataset in multiple dimensions done.")

        LOGGER.info("-------------------------------------------")
        # step2: write the eval results to Excel file.
        self.generate_eval_results_excel(
            query_answer_list, eval_dims_json_list, input_object.get_data("date", "")
        )

        # step3: generate eval report
        eval_report_json_list = self.generate_eval_report(
            eval_dims_json_list, input_object.get_data("date", "")
        )
    ...
    def eval(
        self, query_answer_list: List[List[Tuple[str, str]]], max_eval_lines: int
    ) -> List[List[dict]]:
        ...
        for i in range(len(query_answer_list)):
            ...
            for j in range(len(one_turn_query_answer_list)):
                ...
                try:
                    ...
                    LOGGER.warn(
                        f"except[eval_query_answer_from_jsonl]>>> res: {res}, exception: {e}"
                    )
                    continue
                if len(dimensions) > 0:
                ...
                LOGGER.info(
                    f"Progress: the turn {i + 1} query line {line_num} has been evaluated successfully."
                )
    ...
    def handle_prompt(self) -> ChatPrompt:
        ...
        return ChatPrompt().build_prompt(
    ...
    def generate_eval_report(self, eval_dims_json_list: List[List[dict]], date: str):
        ...
        for i in range(0, len(eval_dims_json_list)):
            ...
            LOGGER.info(
                f"Progress: turn {i + 1} evaluation report has generated successfully."
            )
        ...
        if len(eval_report_json_list) > 1:
            ...
            LOGGER.info(
                f"Progress: total evaluation report has generated successfully."
            )
    ...
    def generate_total_eval_report(eval_report_json_list: List[dict]):
        ...
        if len(eval_report_json_list) > 1:
            ...
            for key in total_dim_avg_score:
                total_dim_avg_score[key] = total_dim_avg_score[key] / len(
                    eval_report_json_list
                )
            report_line_obj = {
    ...
    def generate_eval_results_excel(
        ...
        if len(eval_dims_json_list) > 0 and len(eval_dims_json_list[0]) > 0:
            ...
            LOGGER.info(
                f"Progress: generate evaluation detailed results in excel format successfully."
            )
            LOGGER.info("-------------------------------------------")
