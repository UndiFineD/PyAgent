# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\data_agent_app\intelligence\agentic\agent\agent_instance\data_agent_case\data_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class DataAgent(Agent):
    ...
    def execute(self, input_object: InputObject, agent_input: dict):
        ...
        return {
            ...
            "eval_report_list": eval_report_list,
        }
    ...
    def dataset_build_agent(self, input_object: InputObject):
        ...
        dataset_build_agent: Agent = AgentManager().get_instance_obj(
            profile_config.get("dataset_builder")
        )
        if dataset_build_agent is None:
            raise ValueError(
                "The agent instance corresponding to `dataset_builder` parameter is empty"
            )
    ...
    def dataset_eval_agent(self, input_object: InputObject):
        ...
        profile_config = self.agent_model.profile
        dataset_eval_agent: Agent = AgentManager().get_instance_obj(
            profile_config.get("dataset_evaluator")
        )
        if dataset_eval_agent is None:
            LOGGER.warn(
                "The agent instance corresponding to `dataset_evaluator` parameter is empty"
            )
            return []
