# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_apps\translation_agent_app\intelligence\agentic\agent\agent_instance\translation_agent_case\translation_by_token_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class TranslationAgent(Agent):
    ...
    def execute_agents(self, input_object: InputObject, planner_input: dict) -> dict:
        ...
        output_middle_result(
            input_object, {"init_agent_result": init_agent_result.get_data("output")}
        )
        ...
        output_middle_result(
            input_object,
            {"reflection_agent_result": reflection_result.get_data("output")},
        )
        ...
        output_middle_result(
            input_object, {"improve_agent_result": improve_result.get_data("output")}
        )
    ...
    def execute(self, input_object: InputObject, agent_input: dict) -> dict:
        ...
        source_text_chunks = RecursiveCharacterTextSplitter(
            chunk_size=chunk_size, chunk_overlap=0
        ).split_text(source_text)
