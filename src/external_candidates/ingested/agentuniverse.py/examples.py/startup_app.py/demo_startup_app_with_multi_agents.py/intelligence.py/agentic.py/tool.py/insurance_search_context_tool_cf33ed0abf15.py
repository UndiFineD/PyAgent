# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\startup_app\demo_startup_app_with_multi_agents\intelligence\agentic\tool\insurance_search_context_tool.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class SearchContextTool(Tool):
    ...
    def execute(self, input: str, top_k: int = 2):
        ...
        try:
            ...
            response = MockAPI().post(
                API_URL, headers=headers, data=json.dumps(data, ensure_ascii=False)
            )
            # 解析http response.
            ...
            for recallResult in recallResultTuples:
                ...
                if recallResult.get("content"):
                    context += (
                        f"knowledgeTitle: {recallResult.get('knowledgeTitle')}\n"
