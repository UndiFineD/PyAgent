...
class InsuranceAgentTemplate(AgentTemplate):
    ...
    def _invoke_planning(
        self, input_object: InputObject, agent_input: dict, agents: dict
    ) -> dict:
        planning_agent: Agent = agents.get("planning")
        if not planning_agent:
            planning_result = OutputObject(
    ...
    def _invoke_executing(self, input_object: InputObject, agents: dict) -> dict:
        ...
        input_object.add_data(
            "search_context", executing_result.get_data("search_context")
        )
