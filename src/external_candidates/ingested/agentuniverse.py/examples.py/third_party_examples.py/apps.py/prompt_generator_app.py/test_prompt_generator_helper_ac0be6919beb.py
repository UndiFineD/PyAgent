# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_generator_app\test_prompt_generator_helper.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class TestPromptTemplateHelper(unittest.TestCase):
    ...
    def test_generate_prompt_template_react(self):
        ...
        self.assertTrue(
            any(
                ...
                for keyword in ["客服", "服务", "service", "咨询", "assistant"]
            )
        )
        self.assertTrue(
            any(
                keyword in combined_text
                for keyword in ["电商", "平台", "platform", "商务"]
            )
        )
    ...
    def test_generate_yaml_config(self):
        ...
        prompt_model = PromptTemplateHelper.generate_prompt_template(
            task_description="Test agent", agent_type="react"
        )

        yaml_config = PromptTemplateHelper.generate_yaml_config(
            prompt_model, version_name="test.cn"
        )
    ...
    def test_extract_domain(self):
        ...
        domain = PromptTemplateHelper._extract_domain(
