# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_generator_app\app\intelligence\service\classic_service\template_service.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
    ...
    def search_templates(self, query: str) -> List[Dict[str, Any]]:
        ...
        for name, config in self.preset_templates.items():
            if (
                ...
                or query_lower in config.get("name", "").lower()
                or query_lower in config.get("description", "").lower()
                or query_lower in config.get("scenario", "").lower()
            ):
                results.append({"template_name": name, **config})
