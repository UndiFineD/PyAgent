# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_generator_app\app\intelligence\test\test_prompt_optimizer.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PromptOptimizerTest(unittest.TestCase):
    ...
    def test_basic_prompt_optimization(self):
        ...
        result = optimize_existing_prompt(
            existing_prompt_text=original_prompt, optimization_goal=optimization_goal
        )
    ...
    def test_typed_prompt_optimization(self):
        ...
        combined_content = (
            f"{result.introduction} {result.target} {result.instruction}".lower()
        )
        self.assertTrue(
            any(
                keyword in combined_content
                for keyword in ["工具", "专业", "智能", "助手", "服务"]
            )
        )
    ...
    def test_scenario_prompt_optimization(self):
        ...
        self.assertTrue(
            any(
                keyword in combined_content
                for keyword in ["分析", "数据", "专业", "智能", "助手", "服务", "业务"]
            )
        )
        ...
        print(
            f"Optimized content includes analysis elements: {'分析' in combined_content}"
        )
...
"""
        ...
        result = optimize_existing_prompt(
            existing_prompt_text=yaml_prompt, optimization_goal="完善prompt结构"
        )
    ...
    def test_optimization_error_handling(self):
        ...
        result = optimize_existing_prompt(
            existing_prompt_text="", optimization_goal="优化空prompt"
        )
        self.assertIsNotNone(result)
