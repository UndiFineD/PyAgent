# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_generator_app\prompt_generator_helper.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
Current task: {{input}}""",
    ...
    def generate_prompt_template(
        ...
        if task_description and "assistant" in introduction.lower():
            ...
            if domain:
                introduction = introduction.replace(
                    "AI assistant", f"{domain} AI assistant"
                )
        ...
        return AgentPromptModel(
            introduction=introduction, target=target, instruction=instruction
        )
...
def _analyze_existing_prompt(prompt_text: str) -> Dict[str, Any]:
    ...
    if variables:
        ...
        analysis["strengths"].append(
            f'Contains variable placeholders: {", ".join(variables)}'
        )
    else:
    ...
    if any(
        ...
        for keyword in ["tool", "action", "thought", "observation"]
    ):
        analysis["estimated_type"] = "react"
    elif any(
        keyword in prompt_text.lower()
        for keyword in ["background", "retrieve", "knowledge"]
    ):
        analysis["estimated_type"] = "rag"
    elif any(
        keyword in prompt_text.lower() for keyword in ["plan", "step", "framework"]
    ):
...
def _generate_improvement_suggestions(
    ...
    if not analysis["includes_variables"]:
        suggestions["variable_optimization"].append(
            "Add necessary variable placeholders"
        )
    ...
    if "professional" in optimization_goal.lower():
        ...
        suggestions["instruction_optimization"].append(
            "Add professional terminology and standards"
        )

    if (
        "accuracy" in optimization_goal.lower()
        or "accurate" in optimization_goal.lower()
    ):
        suggestions["instruction_optimization"].append(
            "Add accuracy requirements and verification steps"
        )

    if "efficiency" in optimization_goal.lower():
        suggestions["instruction_optimization"].append(
            "Optimize workflow and procedures"
        )

    # Scenario-based suggestions
    if scenario:
        suggestions["scenario_optimization"].append(
            f"Customize for {scenario} scenario"
        )
...
def _apply_optimizations(
    analysis: Dict[str, Any], suggestions: Dict[str, Any], agent_type: Optional[str]
) -> AgentPromptModel:
...
def generate_prompt_config(
    ...
    yaml_config = PromptTemplateHelper.generate_yaml_config(
        prompt_model=prompt_model, version_name=version_name, agent_type=agent_type
    )
