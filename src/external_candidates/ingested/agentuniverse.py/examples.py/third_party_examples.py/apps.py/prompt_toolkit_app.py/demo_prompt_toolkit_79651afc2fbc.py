...
async def demo_prompt_optimization():
    ...
    sample_prompt = AgentPromptModel(
        introduction="你是一个助手", target="帮助用户", instruction="请回答问题"
    )
...
async def demo_scenario_analysis():
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        if result.analysis_result:
            print(
                f"Recommended Scenario: {result.analysis_result.recommended_scenario.value}"
            )
...
async def demo_batch_generation():
    ...
    requests = [
        PromptGenerationRequest(
            scenario_description="编程助手", domain="技术", target_audience="开发者"
        ),
        PromptGenerationRequest(
            scenario_description="客服助手", domain="商业", target_audience="客户"
        ),
        PromptGenerationRequest(
            scenario_description="教育助手", domain="教育", target_audience="学生"
        ),
    ]
