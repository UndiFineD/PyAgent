# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_toolkit_app\prompt\prompt_generator.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PromptGenerator:
    ...
    def analyze_scenario(
        self, content: str, context: ScenarioContext
    ) -> PromptScenario:
        ...
        if any(
            ...
            for keyword in ["代码", "编程", "开发", "code", "programming"]
        ):
            return PromptScenario.CODE_GENERATION
        elif any(
            keyword in content_lower
            for keyword in ["分析", "数据", "统计", "analysis", "data"]
        ):
            return PromptScenario.ANALYTICAL
        elif any(
            keyword in content_lower
            for keyword in ["创意", "创作", "creative", "design"]
        ):
            return PromptScenario.CREATIVE
        elif any(
            keyword in content_lower
            for keyword in ["推理", "逻辑", "reasoning", "logic"]
        ):
            return PromptScenario.REASONING
        elif any(
            keyword in content_lower
            for keyword in ["客服", "服务", "customer", "service"]
        ):
            return PromptScenario.CUSTOMER_SERVICE
        elif any(
            keyword in content_lower
            for keyword in ["教育", "学习", "education", "learning"]
        ):
            return PromptScenario.EDUCATIONAL
        elif any(
            keyword in content_lower
            for keyword in ["研究", "调查", "research", "investigation"]
        ):
    ...
    def _generate_introduction(
        ...
        return template.format(
            role=role, audience=context.target_audience, domain=context.domain
        )
    ...
    def _generate_instruction(
        ...
        instruction = template.format(
            tone=context.tone, domain=context.domain, audience=context.target_audience
        )
