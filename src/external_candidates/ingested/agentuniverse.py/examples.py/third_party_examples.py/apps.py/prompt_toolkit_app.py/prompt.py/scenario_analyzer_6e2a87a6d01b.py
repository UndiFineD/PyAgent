# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_toolkit_app\prompt\scenario_analyzer.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ScenarioAnalyzer:
    ...
    def analyze_scenario(
        self, content: str, additional_context: Optional[Dict[str, Any]] = None
    ...
    def extract_context_from_content(self, content: str) -> ScenarioContext:
        ...
        domain = self._get_context_value(
            analysis_result.extracted_contexts, ContextType.DOMAIN, "通用"
        )
        user_role = self._get_context_value(
            analysis_result.extracted_contexts, ContextType.USER_ROLE, "用户"
        )
        target_audience = self._get_context_value(
            analysis_result.extracted_contexts, ContextType.TARGET_AUDIENCE, "用户"
        )
        tone = self._get_context_value(
            analysis_result.extracted_contexts, ContextType.TONE, "专业"
        )
    ...
    def _determine_scenario(
        self, content: str, extracted_contexts: List[ExtractedContext]
    ) -> PromptScenario:
        ...
        if any(
            ...
            keyword in content_lower for keyword in ["任务", "工作", "task", "work"]
        ):
    ...
    def _determine_complexity(
        ...
        simple_count = sum(
            1 for indicator in simple_indicators if indicator in content_lower
        )
        complex_count = sum(
            1 for indicator in complex_indicators if indicator in content_lower
        )
    ...
    def _calculate_confidence_score(
        self, extracted_contexts: List[ExtractedContext]
    ) -> float:
    ...
    def _generate_suggestions(
        self, extracted_contexts: List[ExtractedContext], confidence_score: float
    ) -> List[str]:
    ...
    def _get_context_value(
        self, contexts: List[ExtractedContext], context_type: ContextType, default: str
    ) -> str:
