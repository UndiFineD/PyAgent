...
class PromptOptimizer:
    ...
    def _generate_suggestions_for_metric(
        self, metric: PromptQualityMetric, score: float
    ) -> List[str]:
        ...
        if metric == PromptQualityMetric.CLARITY and score < 0.7:
            suggestions.extend(
                ["使用更清晰、具体的语言", "避免模糊或歧义的表达", "提供明确的指导步骤"]
            )

        if metric == PromptQualityMetric.SPECIFICITY and score < 0.7:
            suggestions.extend(
                ["添加具体的示例和说明", "提供详细的执行步骤", "明确约束条件和要求"]
            )
        ...
        if metric == PromptQualityMetric.STRUCTURE and score < 0.7:
            suggestions.extend(
                ["使用有序的步骤和流程", "添加编号或列表格式", "确保逻辑结构清晰"]
            )

        if metric == PromptQualityMetric.TONE and score < 0.7:
            suggestions.extend(
                ["使用更专业、友好的语调", "避免命令式语言", "添加礼貌用语"]
            )
    ...
    def _rule_applies_to_strategies(
        self, rule: OptimizationRule, strategies: List[OptimizationStrategy]
    ) -> bool:
    ...
    def _calculate_confidence_score(
        self, quality_scores: List[QualityScore], improvements: List[str]
    ) -> float:
    ...
    def _format_optimized_prompt(
        self, introduction: str, target: str, instruction: str
    ) -> str:
