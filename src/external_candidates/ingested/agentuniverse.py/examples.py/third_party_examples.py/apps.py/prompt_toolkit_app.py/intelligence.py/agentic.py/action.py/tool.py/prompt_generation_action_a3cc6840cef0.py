# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\prompt_toolkit_app\intelligence\agentic\action\tool\prompt_generation_action.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
            ...
            response = {
                ...
                "analysis_result": {
                    "recommended_scenario": (
                        result.analysis_result.recommended_scenario.value
                        if result.analysis_result
                        else None
                    ),
                    "complexity_level": (
                        result.analysis_result.complexity_level.value
                        if result.analysis_result
                        else None
                    ),
                    "confidence_score": (
                        result.analysis_result.confidence_score
                        if result.analysis_result
                        else 0.0
                    ),
                },
                "optimization_result": {
                    "improvements": (
                        result.optimization_result.improvements
                        if result.optimization_result
                        else []
                    ),
                    "suggestions": (
                        result.optimization_result.suggestions
                        if result.optimization_result
                        else []
                    ),
                    "confidence_score": (
                        result.optimization_result.confidence_score
                        if result.optimization_result
                        else 0.0
                    ),
                },
