# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\app_with_goole_search_tool\intelligence\test\test_tavily_tool.py
import os
import unittest
...
class TavilyToolTest(unittest.TestCase):
    ...
    def test_tavily_tool(self):
        ...
        result = tavily_tool.run(
            input=self.test_query, api_key=self.api_key, mode="search"
        )
        print(f"搜索结果: {result}")
