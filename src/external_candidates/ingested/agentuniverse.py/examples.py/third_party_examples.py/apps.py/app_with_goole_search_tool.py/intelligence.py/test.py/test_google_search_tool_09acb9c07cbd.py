# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\apps\app_with_goole_search_tool\intelligence\test\test_google_search_tool.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class GoogleSearchToolTest(unittest.TestCase):
    ...
    def test_google_search_with_parameters(self):
        ...
        result = self.search_tool.execute(
            query="人工智能", search_type="news", k=5, gl="cn", hl="zh"
        )
        self.assertIsInstance(result, str)
    ...
    def test_scholar_query_building(self):
        ...
        query = self.scholar_tool._build_scholar_query(
            "神经网络", year="2020..2024", author="Yann LeCun", journal="ICML"
        )
    ...
    def test_search_result_formatting(self):
        ...
        formatted = self.search_tool._format_search_result(
            mock_result, "测试查询", "search"
        )
    ...
    def test_scholar_result_formatting(self):
        ...
        formatted = self.scholar_tool._format_scholar_result(
            mock_result, "机器学习", "site:scholar.google.com 机器学习"
        )
