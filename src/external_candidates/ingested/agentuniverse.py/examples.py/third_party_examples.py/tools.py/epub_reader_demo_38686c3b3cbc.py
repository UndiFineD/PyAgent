# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\epub_reader_demo.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
# 乔布斯传记EPUB文件配置
JOBS_BIOGRAPHY_URL = (
    "https://drive.google.com/uc?export=download&id=1_KVCcPFatpe3Pl_4crIwLEWhMAUq3F0o"
)
    ...
    if Path(JOBS_BIOGRAPHY_FILENAME).exists():
        try:
            ...
            if documents:
                ...
                # 计算总字数
                total_words = sum(
                    doc.metadata.get("word_count", 0) for doc in documents
                )
    ...
    if epub_file_path.exists():
        try:
            ...
            if documents:
                print(f"\n章节分析:")
                longest_chapter = max(
                    documents, key=lambda x: x.metadata.get("word_count", 0)
                )
                shortest_chapter = min(
                    documents, key=lambda x: x.metadata.get("word_count", 0)
                )

                print(
                    f"最长章节: {longest_chapter.metadata.get('chapter_file', 'Unknown')} ({longest_chapter.metadata.get('word_count', 0):,} 字)"
                )
                print(
                    f"最短章节: {shortest_chapter.metadata.get('chapter_file', 'Unknown')} ({shortest_chapter.metadata.get('word_count', 0):,} 字)"
                )
...
def demo_with_custom_metadata():
    ...
    if Path(JOBS_BIOGRAPHY_FILENAME).exists():
        try:
            # 加载自定义元数据
            documents = epub_reader.load_data(
                JOBS_BIOGRAPHY_FILENAME, ext_info=custom_metadata
            )
            ...
            if documents:
                ...
                for keyword in keywords:
                    ...
                    if matching_chapters:
                        print(
                            f"关键词 '{keyword}' 出现在章节: {matching_chapters[:5]}{'...' if len(matching_chapters) > 5 else ''} (共{len(matching_chapters)}章)"
                        )
        ...
        for topic, chapters in topic_results.items():
            if chapters:
                ...
                for chapter in chapters[:3]:  # 只显示前3个
                    print(
                        f"   - 第{chapter['chapter_num']}章: {chapter['file']} ({chapter['word_count']:,} 字)"
                    )
                if len(chapters) > 3:
