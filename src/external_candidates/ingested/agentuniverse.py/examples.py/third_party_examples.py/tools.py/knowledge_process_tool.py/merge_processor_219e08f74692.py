# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\knowledge_process_tool\merge_processor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class MergeByMetadata(DocProcessor):
    ...
    def _process_docs(
        self, origin_docs: List[Document], query: Query | None = None
    ) -> List[Document]:
        ...
        for _, docs in grouped.items():
            ...
            if self.prefer_higher_score:
                rep = max(
                    docs, key=lambda d: (d.metadata or {}).get("relevance_score", -1)
                )
