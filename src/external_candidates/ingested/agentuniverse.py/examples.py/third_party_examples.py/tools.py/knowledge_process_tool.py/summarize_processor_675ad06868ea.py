# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\knowledge_process_tool\summarize_processor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class SummarizeDocs(DocProcessor):
    ...
    def _process_docs(
        ...
        if self.mode == "map_reduce":
            combine_prompt = PromptManager().get_instance_obj(
                self.combine_prompt_version
            )
            summary_text = summarize_by_map_reduce(
                texts=texts,
                llm=llm,
            ...
            summary_text = summarize_by_stuff(
                texts=texts, llm=llm, summary_prompt=summary_prompt
            )

        summary_doc = Document(
            text=str(summary_text), metadata={self.summary_metadata_key: True}
        )
    ...
    def _initialize_by_component_configer(
        ...
        if hasattr(doc_processor_configer, "summary_prompt_version"):
            self.summary_prompt_version = str(
                doc_processor_configer.summary_prompt_version
            )
        if hasattr(doc_processor_configer, "combine_prompt_version"):
            self.combine_prompt_version = str(
                doc_processor_configer.combine_prompt_version
            )
