# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\prompt_auto_designer_tool\prompt_auto_designer.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PromptGenerationRequest(BaseModel):
    ...
    inputs: List[str] = Field(
        default_factory=list, description="智能体将接收的关键输入。"
    )
    outputs: List[str] = Field(default_factory=list, description="期望的输出内容形态。")
    constraints: List[str] = Field(
        default_factory=list, description="必须遵循的约束或限制条件。"
    )
    additional_context: Optional[str] = Field(
        default=None, description="可选的背景信息、知识库说明等。"
    )
    examples: Optional[str] = Field(
        default=None, description="示例问答或既有 prompt 片段。"
    )
...
class PromptOptimizationRequest(BaseModel):
    ...
    objective: Optional[str] = Field(
        default=None, description="该 prompt 需要实现的目标。"
    )
    issues: List[str] = Field(
        default_factory=list, description="当前 prompt 遇到的问题或痛点。"
    )
    success_metrics: List[str] = Field(
        default_factory=list, description="衡量优化效果的指标。"
    )
    style: Optional[str] = Field(default=None, description="目标语气或写作风格。")
    language: str = Field(default="中文", description="优化后 prompt 的语言。")
    additional_context: Optional[str] = Field(
        default=None, description="补充的上下文或边界。"
    )
...
class PromptAutoDesigner:
    ...
    def generate_prompt(self, request: PromptGenerationRequest) -> PromptDesignResult:
        ...
        prompt_text = generate_template(
            prompt_model, ["introduction", "target", "instruction"]
        ).strip()
    ...
    def _build_generation_payload(
        ...
        return {
            ...
            "constraints": self._format_bullets(
                request.constraints, fallback="暂无额外约束"
            ),
    ...
    def _build_optimization_payload(
        ...
        current_prompt = generate_template(
            request.prompt, ["introduction", "target", "instruction"]
        ).strip()
        return {
            ...
            "success_metrics": self._format_bullets(
                request.success_metrics, fallback="暂无成功指标"
            ),
    ...
    def _build_prompt_model(
        parsed: Dict[str, Any], fallback: Optional[AgentPromptModel] = None
    ) -> AgentPromptModel:
        data = {
        ...
        try:
            ...
            if match:
                try:
                    ...
                    pass
        raise PromptAutoDesignerError(
            "LLM 响应不是有效的 JSON，请检查提示词模板或模型输出。"
        )
