# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\prompt_auto_designer_tool\test_prompt_auto_designer.py
import json
...
def test_generate_prompt_invalid_json(monkeypatch):
    monkeypatch.setattr(
        PromptAutoDesigner, "_invoke_llm", lambda self, version, payload: "not-json"
    )
    designer = PromptAutoDesigner()
    request = PromptGenerationRequest(
        scenario="安防巡检机器人",
