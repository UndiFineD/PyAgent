...
class TestDocumentClassifier(unittest.TestCase):
    ...
    def test_keyword_classification(self):
        ...
        self.assertGreater(
            classified_docs[0].metadata["classification_confidence"], 0.5
        )
...
class TestChineseDocumentClassifier(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 添加中文关键词规则
        self.classifier.add_chinese_keyword_rule(
            "技术文档", ["代码", "编程", "API", "函数", "技术"]
        )
        self.classifier.add_chinese_keyword_rule(
            "学术论文", ["研究", "实验", "分析", "论文", "学术"]
        )
        ...
        self.chinese_documents = [
            Document(
                text="这是一个Python编程教程，介绍了如何使用API和函数进行技术开发。"
            ),
    ...
    def test_fuzzy_match(self):
        ...
        # 分词匹配
        self.assertTrue(
            self.classifier._fuzzy_match("Python编程", "这是一个Python编程教程")
        )
    ...
    def test_content_type_classification(self):
        ...
        self.assertEqual(
            self.classifier.classify_by_content_type(tech_text), "技术文档"
        )

        # 学术论文
        academic_text = "本研究通过实验分析了机器学习算法的性能。"
        self.assertEqual(
            self.classifier.classify_by_content_type(academic_text), "学术论文"
        )

        # 新闻资讯
        news_text = "据最新消息，某公司今日宣布重大事件。"
        self.assertEqual(
            self.classifier.classify_by_content_type(news_text), "新闻资讯"
        )

    def test_chinese_regex_classification(self):
        """测试中文正则表达式分类"""
        self.classifier.add_chinese_regex_rule("技术文档", r"(代码|编程|API|函数|技术)")
        self.classifier.add_chinese_regex_rule(
            "学术论文", r"(研究|实验|分析|论文|学术)"
        )
        self.classifier.add_chinese_regex_rule(
            "新闻资讯", r"(消息|新闻|事件|报道|记者)"
        )
