        ...
        try:
            ...
            if "{" in response and "}" in response:
                ...
                if (
                    ...
                    and category != self.default_category
                ):
        ...
        for doc in documents:
            if doc.metadata:
                category = doc.metadata.get(
                    self.classification_key, self.default_category
                )
                ...
                if confidence > 0:
                    ...
                    summary["confidence_stats"]["min"] = min(
                        summary["confidence_stats"]["min"], confidence
                    )
                    summary["confidence_stats"]["max"] = max(
                        summary["confidence_stats"]["max"], confidence
                    )
