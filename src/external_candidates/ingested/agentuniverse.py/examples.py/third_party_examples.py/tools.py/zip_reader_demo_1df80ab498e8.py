# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\third_party_examples\tools\zip_reader_demo.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def create_sample_zip():
    ...
    with zipfile.ZipFile(nested_zip, "w") as nested:
        nested.writestr(
            "nested_docs/report.txt", "这是嵌套压缩包中的报告文档\n包含重要数据分析结果"
        )
        nested.writestr(
            "nested_docs/data.json",
            '{"type": "analysis", "status": "completed", "score": 95}',
        )
        nested.writestr(
            "nested_code/script.py",
...
def demo_nested_zip(zip_path):
    ...
    nested_docs = [
        d for d in documents if "nested_data.zip" in d.metadata.get("archive_path", "")
    ]
...
def demo_file_type_filtering(zip_path):
    ...
    json_files = [
        d for d in documents if d.metadata.get("file_name", "").endswith(".json")
    ]
...
def demo_statistics(zip_path):
    ...
    if documents:
        ...
        print(f"\n文档大小:")
        print(
            f"  最大: {largest_doc.metadata.get('file_name', 'Unknown')} ({len(largest_doc.text):,} 字符)"
        )
        print(
            f"  最小: {smallest_doc.metadata.get('file_name', 'Unknown')} ({len(smallest_doc.text):,} 字符)"
        )
...
def main():
    ...
    try:
        ...
        print(
            "  - 测试文件: tests/test_agentuniverse/unit/agent/action/knowledge/reader/file/test_zip_reader.py"
        )
