# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\examples\sample_standard_app\intelligence\utils\log_sink\custom_flask_response_sink.py
...
class CustomFlaskResponseSink(FlaskResponseLogSink):
    def generate_log(self, flask_response, elapsed_time) -> str:
        if isinstance(flask_response, str):
            response_str = (
                f"Response: {flask_response} " f"Duration: {elapsed_time:.3f}s"
            )
        else:
            response_str = (
                f"Response: {flask_response.status_code} {flask_response.content_type} "
