...
class ChatPrompt(Prompt):
    ...
    def build_prompt(
        self, agent_prompt_model: AgentPromptModel, prompt_assemble_order: list[str]
    ) -> "ChatPrompt":
        ...
        """
        self.messages = generate_chat_template(
            agent_prompt_model, prompt_assemble_order
        )
            ...
            for image_url in image_urls:
                if isinstance(image_url, dict) and "url" in image_url:
                    ...
                    self.messages.append(
                        Message(type=ChatMessageEnum.HUMAN.value, content=content)
                    )
                    continue
                ...
                # Check if the URL is a local file.
                    ...
                    if parsed_url.path.lower().endswith(image_extensions):
                        with open(parsed_url.path, "rb") as image_file:
                            ...
                            self.messages.append(
                                Message(
                                    type=ChatMessageEnum.HUMAN.value, content=content
                                )
                            )
