    ...
    def _run(self, workflow_output: WorkflowOutput) -> NodeOutput:
        ...
        for knowledge_param in inputs.knowledge_param:
            if knowledge_param.name in param_map:
                if isinstance(knowledge_param.value, str):
                    ...
                    param_map[knowledge_param.name] = knowledge_param.value.get(
                        "content", None
                    )
        ...
        for knowledge_id in knowledge_id_list:
            ...
            if knowledge is None:
                raise ValueError(
                    "No knowledge with id {} was found.".format(knowledge_id)
                )
            knowledge_list.append(knowledge)

        knowledge_input_params = self._resolve_input_params(
        ...
        for knowledge in knowledge_list:
            document_texts = [
                document.text
                for document in knowledge.query_knowledge(**knowledge_input_params)
            ]
            knowledge_res += "\n".join(document_texts)
