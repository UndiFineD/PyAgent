...
class Node(BaseModel):
    ...
    def _resolve_input_params(
        input_params: List[NodeInputParams], workflow_output: WorkflowOutput
    ) -> Dict[str, Any]:
        ...
        for input_param in input_params:
            ...
            if val.type == "reference":
                ...
                reference_output_params: List[NodeOutputParams] = (
                    workflow_output.workflow_parameters.get(reference_node_id, [])
                )
                node_input_params[input_param.name] = next(
                    (
                        param.value
                        for param in reference_output_params
                        if param.name == val.content[1]
                    ),
                    None,
                )
            else:
