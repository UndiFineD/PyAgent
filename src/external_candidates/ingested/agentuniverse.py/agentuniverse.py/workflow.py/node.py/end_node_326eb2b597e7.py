...
class EndNode(Node):
    ...
    def _run(self, workflow_output: WorkflowOutput) -> NodeOutput:
        ...
        end_node_input_params = self._resolve_input_params(
        ...
        try:
            for var in template_variables:
                if var not in end_node_input_params:
                    raise KeyError(
                        f"The variable '{var}' is not found in the input params."
                    )
                prompt_val = prompt_val.replace(
                    f"{{{{{var}}}}}",
                    (
                        str(end_node_input_params[var])
                        if end_node_input_params[var]
                        else ""
                    ),
                )
