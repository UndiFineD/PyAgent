# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\workflow\graph\graph.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Graph(nx.DiGraph):
    ...
    def _add_graph_node(self, workflow_id: str, node_config: dict) -> None:
        ...
        node_instance = node_cls(
            type=NodeEnum.from_value(node_type), workflow_id=workflow_id, **node_config
        )
    ...
    def run(self, workflow_output: WorkflowOutput) -> None:
        ...
        while True:
            next_node = self._get_next_node(
                workflow_output, sorted_nodes, predecessor_node
            )
            if not next_node:
                break
    ...
    def _get_next_node(
        ...
        nodes: Any,
        predecessor_node: Optional[Node] = None,
    ) -> Optional[Node]:
        ...
        if not predecessor_node:
            ...
            predecessor_node_output: NodeOutput = (
                workflow_output.workflow_node_results.get(predecessor_node.id, None)
            )
            source_handler = (
                predecessor_node_output.edge_source_handler
                if predecessor_node_output
                else None
            )
            ...
            if source_handler:
                for s in successors:
                    if (
                        self.get_edge_data(predecessor_node.id, s).get("source_handler")
                        == source_handler
                    ):
    ...
    @staticmethod
    def _run_node(
        cur_node: Node = None, workflow_output: WorkflowOutput = None
    ) -> None:
