# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\workflow\workflow.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Workflow(ComponentBase):
    ...
    def run(self, input_params: dict) -> WorkflowOutput:
        ...
        workflow_output = WorkflowOutput(
            workflow_id=self.id, workflow_start_params=input_params
        )
        self.graph.run(workflow_output)
        return workflow_output

    def initialize_by_component_configer(
        self, component_configer: WorkflowConfiger
    ) -> "Workflow":
