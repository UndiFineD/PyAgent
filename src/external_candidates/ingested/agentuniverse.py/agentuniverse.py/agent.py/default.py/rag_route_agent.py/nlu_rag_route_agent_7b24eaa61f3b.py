# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\default\rag_route_agent\nlu_rag_route_agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class NluRagRouteAgent(RagAgentTemplate):
    ...
    def process_llm(self, **kwargs) -> LLM:
        llm_name = (
            self.agent_model.profile.get("llm_model", {}).get("name") or self.llm_name
        )
    ...
    def initialize_by_component_configer(
        ...
        self.prompt_version = self.agent_model.profile.get(
            "prompt_version", "nlu_rag_route_prompt.cn"
        )
        return self
