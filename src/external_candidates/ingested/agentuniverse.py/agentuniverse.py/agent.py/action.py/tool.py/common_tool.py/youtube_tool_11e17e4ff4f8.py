# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\common_tool\youtube_tool.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class YouTubeTool(Tool):
    ...
    api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("YOUTUBE_API_KEY")
    )
    ...
    def _initialize_service(self):
        if not self.api_key:
            raise ValueError(
                "YouTube API key not provided, please set the YOUTUBE_API_KEY environment variable."
            )
    ...
    def _search_videos(self, query: str) -> List[Dict]:
        try:
            search_response = (
                self.service.search()
                .list(q=query, part="id", type="video", maxResults=self.max_results)
                .execute()
            )
    ...
    def execute(self, mode: str, input: Optional[str] = None) -> List[Dict] | Dict:
        ...
        if mode == Mode.VIDEO_SEARCH.value:
            ...
            raise ValueError(
                f"Invalid mode: {mode}. Must be one of {[m.value for m in Mode]}"
            )
