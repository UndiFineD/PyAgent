# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\common_tool\search_api_tool.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class SearchAPITool(Tool):
    ...
    search_api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("SEARCHAPI_API_KEY")
    )
    engine: str = "google"
    search_params: dict = {}
    ...
    def _load_api_wapper(self):
        ...
        if not self.search_api_wrapper:
            self.search_api_wrapper = SearchApiAPIWrapper(
                searchapi_api_key=self.search_api_key, engine=self.engine
            )
    ...
    def initialize_by_component_configer(
        self, component_configer: ToolConfiger
    ) -> "Tool":
        ...
        self.search_type = component_configer.configer.value.get(
            "search_type", "common"
        )
        return self
