# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\common_tool\tavily_tool.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class TavilyTool(Tool):
    ...
    api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("TAVILY_API_KEY"),
    ...
    mode: Literal["search", "extract"] = Field(
        default="search", description="操作模式：search或extract"
    )

    # 搜索模式可选参数
    topic: Optional[Literal["general", "news"]] = Field(
        default=None, description="搜索主题：general或news"
    )
    days: Optional[int] = Field(
        default=None, description="当topic为news时，指定搜索的天数"
    )
    time_range: Optional[Literal["day", "week", "month", "year"]] = Field(
        default=None, description="搜索时间范围"
    )
    max_results: int = Field(default=5, description="返回的最大结果数")
    include_domains: Optional[List[str]] = Field(
        default=None, description="包含的域名列表"
    )
    exclude_domains: Optional[List[str]] = Field(
        default=None, description="排除的域名列表"
    )
    ...
    extract_depth: Literal["basic", "advanced"] = Field(
        default="advanced", description="提取深度"
    )
    ...
    def execute(self, input: str | list = None, **kwargs):
        ...
        if not self.api_key:
            return {
                "error": "未提供Tavily API密钥，请设置TAVILY_API_KEY环境变量或在调用时提供api_key参数"
            }
