        ...
        **kwargs,
        ...
        try:
            search = GoogleSerperAPIWrapper(
                serper_api_key=self.serper_api_key, k=k, gl=gl, hl=hl, type=search_type
            )
    ...
    def _format_search_result(self, result: str, query: str, search_type: str) -> str:
        ...
        try:
            ...
            if isinstance(result, str) and result.startswith("["):
                ...
                formatted = (
                    f"🔍 Google {search_type.title()} 搜索结果 (查询: {query}):\n\n"
                )
...
class GoogleScholarSearchTool(Tool):
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            search = GoogleSerperAPIWrapper(
                serper_api_key=self.serper_api_key, k=k, gl="us", hl="en", type="search"
            )
    ...
    def _build_scholar_query(
        self, query: str, year: str = None, author: str = None, journal: str = None
    ) -> str:
        """构建学术搜索查询"""
    ...
    def _format_scholar_result(
        self, result: str, original_query: str, scholar_query: str
    ) -> str:
        """格式化学术搜索结果"""
        try:
            if isinstance(result, str) and result.startswith("["):
