# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\common_tool\github_tool.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class GitHubTool(Tool):
    ...
    api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("GITHUB_API_KEY")
    )
    ...
    def _make_request(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 403:
                return {
                    "error": "API rate limit exceeded. Please check your API key or try again later."
                }
    ...
    @retry(3, 1.0)
    def search_users(
        self, query: str, sort: str = "followers", order: str = "desc"
    ) -> List[Dict[str, Any]]:
        """搜索GitHub用户
        ...
        for item in result.get("items", []):
            issue_info = {
                ...
                "repository": {
                    ...
                    "full_name": item["repository_url"].split("/")[-2]
                    + "/"
    ...
    def execute(
        ...
        if mode == GitHubSearchMode.REPOSITORY.value:
            ...
            if not owner or not repo:
                return {
                    "error": "Owner and repo parameters are required for repository info"
                }
            ...
            return [
                {
                    "error": f"Invalid mode: {mode}. Must be one of {[m.value for m in GitHubSearchMode]} or 'repo_info'"
                }
            ]
