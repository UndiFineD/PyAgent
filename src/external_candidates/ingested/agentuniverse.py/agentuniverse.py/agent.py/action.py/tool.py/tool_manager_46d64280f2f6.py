# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\tool_manager.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ToolManager(ComponentManagerBase):
    ...
    def get_instance_obj(
        ...
        instance_code = (
            f"{appname}.{self._component_type.value.lower()}.{component_instance_name}"
        )
        ...
        if instance_obj is None:
            ...
            tool_configer_map: dict[str, ToolConfiger] = (
                ApplicationConfigManager().app_configer.tool_configer_map
            )
            toolkit_configer_map = (
            ...
            if (
                not instance_obj
                and tool_configer_map
                and component_instance_name in tool_configer_map.keys()
            ):
                ...
                if tool_configer:
                    ...
                    if tool_configer.meta_class:
                        metadata_module = ".".join(
                            tool_configer.meta_class.split(".")[:-1]
                        )
                    ...
                    instance_obj: Tool = (
                        component_clz().initialize_by_component_configer(tool_configer)
                    )
                    if instance_obj:
