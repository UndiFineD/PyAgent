# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\api_tool.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class APITool(Tool):
    ...
    def execute(self, tool_input: ToolInput):
        res = self.do_http_request(
            self.openapi_spec.get("url"),
            self.openapi_spec.get("method"),
            {},
            tool_input.to_dict(),
        )
        return res.text
        ...
        for option in any_of or []:
            try:
                if "type" in option:
                    ...
                    return self.convert_body_property_any_of(
    ...
    def do_http_request(
        ...
        if (
            "requestBody" in self.openapi_spec
            and self.openapi_spec["requestBody"] is not None
        ):
            # handle json request body
            if "content" in self.openapi_spec["requestBody"]:
                for content_type in self.openapi_spec["requestBody"]["content"]:
                    headers["Content-Type"] = content_type
                    body_schema = self.openapi_spec["requestBody"]["content"][
                        content_type
                    ]["schema"]
