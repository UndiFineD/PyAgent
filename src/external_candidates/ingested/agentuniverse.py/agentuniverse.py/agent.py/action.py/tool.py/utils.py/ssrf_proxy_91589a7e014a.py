# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\tool\utils\ssrf_proxy.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def make_request(method, url, **kwargs):
    if SSRF_PROXY_ALL_URL:
        return httpx.request(
            method=method, url=url, proxy=SSRF_PROXY_ALL_URL, timeout=20, **kwargs
        )
    elif proxies:
        return httpx.request(
            method=method, url=url, proxies=proxies, timeout=20, **kwargs
        )
    else:
