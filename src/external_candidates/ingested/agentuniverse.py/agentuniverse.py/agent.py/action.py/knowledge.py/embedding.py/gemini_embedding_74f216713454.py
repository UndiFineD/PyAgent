# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\embedding\gemini_embedding.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class GeminiEmbedding(Embedding):
    ...
    gemini_api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("GOOGLE_API_KEY")
    )
    ...
    def as_langchain(self) -> LCEmbeddings:
        ...
        class GeminiLangchainEmbedding(LCEmbeddings):
            ...
            def __init__(self, gemini_embedding: GeminiEmbedding, **kwargs):
                ...
                self.gemini_embedding = (
                    gemini_embedding  # Store the GeminiEmbedding instance
                )
        ...
        return GeminiLangchainEmbedding(
            gemini_embedding=self
        )  # Pass the instance of GeminiEmbedding

    def _initialize_by_component_configer(
        self, embedding_configer: ComponentConfiger
    ) -> "Embedding":
