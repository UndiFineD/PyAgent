# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\image\scanned_pdf_ocr_reader.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ScannedPdfOCRReader(Reader):
    ...
    def _ocr_pdf_page(self, file: Path, page_index: int) -> (str, str):
        ...
        try:
            ...
            raise ImportError(
                "pdf2image is required: `pip install pdf2image`. Also install poppler."
            )

        print(f"debugging: ScannedPdfOCRReader converting page {page_index} to image")
        images = convert_from_path(
            str(file), first_page=page_index + 1, last_page=page_index + 1
        )
        if not images:
            return "", "none"
