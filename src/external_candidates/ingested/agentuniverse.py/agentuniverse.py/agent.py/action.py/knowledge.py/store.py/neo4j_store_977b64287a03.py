# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\store\neo4j_store.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Neo4jStore(Store):
    ...
    def execute_cypher(self, query_str, param=None, return_data=True):
        df_result = self._execute_cypher(
            self.driver.session(), query_str, param, return_data
        )
        return df_result
    ...
    def _llm_generate_cypher(
        self, natural_language_query: str, schema_info: dict
    ) -> str:
    ...
    def _initialize_by_component_configer(
        self, neo4j_store_configer: ComponentConfiger
    ) -> "Neo4jStore":
