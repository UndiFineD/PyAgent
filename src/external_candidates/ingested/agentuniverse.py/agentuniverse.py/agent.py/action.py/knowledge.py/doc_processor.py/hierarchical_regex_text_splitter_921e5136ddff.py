...
class HierarchicalRegexTextSplitter(DocProcessor):
    ...
    def _hierarchical_split_single_doc(self, doc: Document) -> List[Document]:
        ...
        hierarchy = {}
        levels = [
            (f"item_{i}", re.compile(index["reg_exp"]))
            for i, index in enumerate(self.hierarchical_index)
        ]
    ...
    def _process_docs(
        self, origin_docs: List[Document], query: Query = None
    ) -> List[Document]:
