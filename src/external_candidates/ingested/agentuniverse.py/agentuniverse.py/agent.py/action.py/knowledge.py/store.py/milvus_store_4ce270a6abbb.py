# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\store\milvus_store.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class MilvusStore(Store):
    ...
    def _initialize_by_component_configer(
        self, milvus_store_configer: ComponentConfiger
    ) -> "DocProcessor":
    ...
    def _create_or_load_collection(
        self, dim: int = 128, max_length: int = 65535, index_params: dict = None
    ):
        """
        ...
        if utility.has_collection(self.collection_name, using=self.connection_name):
            ...
            self.collection = Collection(
                self.collection_name, schema, using=self.connection_name
            )
        ...
        for document in documents:
            ...
            if not self.collection:
                self._create_or_load_collection(
                    dim=len(embedding), max_length=max_length, index_params=index_params
                )
            ...
            for res in result:
                documents.append(
                    Document(
                        ...
                        embedding=(
                            res.fields.get("embedding")
                            if res.fields.get("embedding")
                            else []
                        ),
                        metadata=(
                            res.fields.get("metadata")
                            if res.fields.get("metadata")
                            else None
                        ),
                    )
                )
