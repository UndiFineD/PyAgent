# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\file\sevenzip_reader.py
import os
import shutil
import tempfile
...
class SevenZipReader(Reader):
    ...
    def _load_data(
        ...
        try:
            ...
            # 如果py7zr未安装，抛出导入错误
            raise ImportError(
                "py7zr is required to read 7Z files: " "`pip install py7zr`"
            )
            ...
            with py7zr.SevenZipFile(str(sevenzip_path), "r") as archive:
                ...
                for entry_name in entries:
                    ...
                    try:
                        ...
                        # 构建完整路径（包含父路径信息）
                        full_path = (
                            os.path.join(parent_path, entry_name)
                            if parent_path
                            else entry_name
                        )
                        ...
                        if (
                            extracted_path.suffix.lower() == ".7z"
    ...
    def _get_reader_class(self, suffix: str) -> Optional[Type[Reader]]:
        ...
        try:
            ...
            if reader_name == "PdfReader":
                from agentuniverse.agent.action.knowledge.reader.file.pdf_reader import (
                    PdfReader,
                )

                return PdfReader
            elif reader_name == "DocxReader":
                from agentuniverse.agent.action.knowledge.reader.file.docx_reader import (
                    DocxReader,
                )

                return DocxReader
            elif reader_name == "PptxReader":
                from agentuniverse.agent.action.knowledge.reader.file.pptx_reader import (
                    PptxReader,
                )

                return PptxReader
            elif reader_name == "XlsxReader":
                from agentuniverse.agent.action.knowledge.reader.file.xlsx_reader import (
                    XlsxReader,
                )

                return XlsxReader
            elif reader_name == "EpubReader":
                from agentuniverse.agent.action.knowledge.reader.file.epub_reader import (
                    EpubReader,
                )

                return EpubReader
            elif reader_name == "TxtReader":
                from agentuniverse.agent.action.knowledge.reader.file.txt_reader import (
                    TxtReader,
                )
                ...
                from agentuniverse.agent.action.knowledge.reader.file.csv_reader import (
                    CSVReader,
                )

                return CSVReader
            elif reader_name == "CodeReader":
                from agentuniverse.agent.action.knowledge.reader.file.code_reader import (
                    CodeReader,
                )
