# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\store\chroma_store.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ChromaStore(Store):
    ...
    def _new_client(self) -> Any:
        ...
        if self.persist_path.startswith("http") or self.persist_path.startswith(
            "https"
        ):
            # Remote database URL
        ...
        for i in range(len(query_result["ids"][0])):
            documents.append(
                Document(
                    ...
                    embedding=(
                        query_result["embeddings"][0][i]
                        if query_result["embeddings"] is not None
                        else []
                    ),
                    metadata=(
                        query_result["metadatas"][0][i]
                        if query_result["metadatas"] is not None
                        else None
                    ),
                )
            )
    ...
    def _initialize_by_component_configer(
        self, chroma_store_configer: ComponentConfiger
    ) -> "DocProcessor":
