...
class DashscopeEmbedding(Embedding):
    ...
    def get_embeddings(self, texts: List[str], **kwargs) -> List[List[float]]:
        ...
        if self.embedding_model_name == "text-embedding-v3":
            post_params["parameters"]["dimension"] = (
                self.embedding_dims if self.embedding_dims else 1024
            )
            post_params["parameters"]["output_type"] = (
                kwargs["output_type"] if "output_type" in kwargs else "dense"
            )
        post_params["parameters"]["text_type"] = (
            kwargs["text_type"] if "text_type" in kwargs else "document"
        )
        ...
        for batch in batched(texts):
            ...
            if data:
                ...
                raise Exception(
                    f"Failed to call dashscope embedding api, "
