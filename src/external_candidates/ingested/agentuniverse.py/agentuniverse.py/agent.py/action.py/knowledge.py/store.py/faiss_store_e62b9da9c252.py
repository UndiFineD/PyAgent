# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\store\faiss_store.py
# !/usr/bin/env python3
...
class FAISSStore(Store):
    ...
    def _get_embedding(self, text: str, text_type: str = "document") -> List[float]:
        ...
        if not self.embedding_model:
            NO_EMBEDDING_MSG = (
                "No embedding model configured. Please specify an embedding_model."
            )
            raise ValueError(NO_EMBEDDING_MSG)

        try:
            embedding_instance = EmbeddingManager().get_instance_obj(
    ...
    def query(self, query: Query, **kwargs) -> List[Document]:  # noqa: C901
        ...
        if len(embedding) == 0:
            ...
            if self.embedding_model is None:
                logger.warning(
                    "No embeddings provided in query and no embedding model configured"
                )
                return []
    ...
    def _initialize_by_component_configer(
        self, faiss_store_configer: ComponentConfiger
    ) -> "FAISSStore":
