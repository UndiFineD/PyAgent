# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\knowledge.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
import asyncio
...
class Knowledge(ComponentBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.insert_executor = ThreadPoolExecutorWithReturnValue(
            max_workers=5, thread_name_prefix="Knowledge store"
        )
        self.query_executor = ThreadPoolExecutorWithReturnValue(
            max_workers=10, thread_name_prefix="Knowledge query"
        )
    ...
    def _load_data(self, *args: Any, **kwargs: Any) -> List[Document]:
        ...
        if url_pattern.match(source_path):
            ...
            raise Exception(
                f"Knowledge load data error: Unknown source type:{source_path}"
            )
    ...
    def _insert_process(self, origin_docs: List[Document]) -> List[Document]:
        for _processor_code in self.insert_processors:
            doc_processor: DocProcessor = DocProcessorManager().get_instance_obj(
                _processor_code
            )
    ...
    def _paraphrase_query(self, origin_query: Query) -> Query:
        for _paraphraser_code in self.query_paraphrasers:
            query_paraphraser: QueryParaphraser = (
                QueryParaphraserManager().get_instance_obj(_paraphraser_code)
            )
    ...
    def _route_rag(self, query: Query):
        return (
            RagRouterManager()
            .get_instance_obj(self.rag_router)
            .rag_route(query, self.stores)
        )
        ...
        for query_task in query_tasks:
            futures.append(
                self.query_executor.submit(
                    StoreManager().get_instance_obj(query_task[1]).query, query_task[0]
                )
            )
        wait(futures, return_when=ALL_COMPLETED)
        retrieved_docs = {}
    ...
    def _initialize_by_component_configer(
        self, knowledge_configer: ComponentConfiger
    ) -> "Knowledge":
