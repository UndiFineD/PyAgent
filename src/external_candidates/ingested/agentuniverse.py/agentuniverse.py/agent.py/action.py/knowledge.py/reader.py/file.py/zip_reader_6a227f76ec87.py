# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\file\zip_reader.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ZipReader(Reader):
    ...
    def _load_data(
        self, file: Union[str, Path], ext_info: Optional[Dict] = None
    ) -> List[Document]:
    ...
    def _iterate_archive(
        ...
        for info in archive.infolist():
            ...
            metadata = self._build_metadata(
                archive_path, current_stack, depth, ext_meta
            )
            ...
            if suffix == ".zip":
                ...
                documents.extend(self._handle_text_fallback(archive, info, metadata))
            elif suffix in CODE_FILE_EXTENSIONS or suffix in self._reader_classes:
                reader = self._get_reader(suffix)
                if reader:
                    documents.extend(
                        self._handle_reader_with_temp(
                            archive, info, temp_dir, metadata, reader
                        )
                    )
        return documents
    ...
    def _merge_metadata(self, document: Document, metadata: Dict) -> Document:
        ...
        document.metadata.update(
            {k: v for k, v in metadata.items() if k not in document.metadata}
        )
        return document
    ...
    def _build_metadata(
        ...
        metadata = {
            ...
            "archive_depth": depth,
            "file_name": (
                PurePosixPath(path_stack[-1]).name if path_stack else archive_path.name
            ),
    ...
    def _enforce_limits(self, info: zipfile.ZipInfo) -> None:
        ...
        if compressed_size > 0:
            ...
            if compression_ratio > self.max_compression_ratio:
                raise ValueError(
                    f"Zip entry has suspicious compression ratio: {info.filename}"
                )
