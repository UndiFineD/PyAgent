...
class CodeAstProcessor(DocProcessor):
    ...
    def _process_docs(
        ...
        for doc in origin_docs:
            code = doc.text
            language = (
                doc.metadata.get("language", "unknown") if doc.metadata else "unknown"
            )
    ...
    def _calculate_code_metrics(self, code: str, language: str) -> CodeMetrics:
        ...
        metrics: CodeMetrics = {
            ...
            "avg_line_length": sum(len(line) for line in code_lines)
            / max(len(code_lines), 1),
            "max_line_length": (
                max([len(line) for line in code_lines]) if code_lines else 0
            ),
    ...
    def _generate_code_chunks(
        self, code: str, root_node, language: str, metadata: Dict[str, Any]
    ) -> List[Document]:
        chunks = []
        lines = code.splitlines()
        boundaries = []
        ...
        if boundaries:
            for boundary in boundaries:
                ...
                try:
                    ...
                    chunk_doc = Document(
                        text=json.dumps(representation), metadata=chunk_metadata
                    )
