# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\file\file_reader.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class FileReader(Reader):
    ...
    def _load_data(
        self, file_paths: List[Path], ext_info: Optional[Dict] = None
    ) -> List[Document]:
        document_list = []
        for file_path in file_paths:
            ...
            if file_suffix in self.file_readers.keys():
                ...
                document_list.extend(
                    file_reader.load_data(file=file_path, ext_info=ext_info)
                )
