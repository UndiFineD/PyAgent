# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\store\qdrant_store.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class QdrantStore(Store):
    ...
    def _initialize_by_component_configer(
        self, qdrant_store_configer: ComponentConfiger
    ) -> "QdrantStore":
    ...
    def _ensure_collection(self, dim: int):
        ...
        if not self.client.collection_exists(self.collection_name):
            ...
            self.client.create_collection(
                ...
                vectors_config={
                    self.VECTOR_NAME: VectorParams(size=dim, distance=metric)
                },
            )
    ...
    def query(self, query: Query, **kwargs) -> List[Document]:
        ...
        limit = (
            query.similarity_top_k if query.similarity_top_k else self.similarity_top_k
        )
    ...
    def delete_document(self, document_id: str, **kwargs):
        if self.client is None:
            return
        self.client.delete(
            collection_name=self.collection_name, points_selector=[document_id]
        )
