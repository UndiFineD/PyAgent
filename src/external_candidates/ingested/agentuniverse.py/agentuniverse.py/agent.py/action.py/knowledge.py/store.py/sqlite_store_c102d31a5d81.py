# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\store\sqlite_store.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class SQLiteStore(Store):
    ...
    def _initialize_by_component_configer(
        self, sqlite_store_configer: ComponentConfiger
    ) -> "DocProcessor":
    ...
    def compute_bm25(
        self, query_text, doc_text, inverted_index, total_doc_count, total_word_count
    ):
        k1 = self.k1
        b = self.b
        ...
        for term in query_words:
            if term in inverted_index:
                ...
                idf = math.log(
                    (total_doc_count - num_docs_with_term + 0.5)
    ...
    def _get_document_keyword(self, document: Document) -> Set[str]:
        if not self.keyword_extractor:
            ...
            _doc = (
                DocProcessorManager()
                .get_instance_obj(self.keyword_extractor)
    ...
    def delete_document(self, document_id: int):
        with self.conn:
            ...
            self.conn.execute(
                "DELETE FROM inverted_index WHERE doc_id = ?", (document_id,)
            )
    ...
    def upsert_document(self, documents: List[Document], **kwargs):
        with self.conn:
            for document in documents:
                ...
                self.conn.execute(
                    "DELETE FROM inverted_index WHERE doc_id = ?", (document.id,)
                )
                self._get_document_keyword(document)
                for term in set(document.keywords):
                    self.conn.execute(
    ...
    def query(self, query: Query, **kwargs) -> List[Document]:
        ...
        with self.conn:
            for keyword in query_terms:
                cursor = self.conn.cursor()
                cursor.execute(
                    "SELECT doc_id FROM inverted_index WHERE term = ?", (keyword,)
                )
