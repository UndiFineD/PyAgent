# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\reader\image\image_reader.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ImageReader(Reader):
    ...
    def _initialize_by_component_configer(
        self, reader_configer: ComponentConfiger
    ) -> "ImageReader":
    ...
    def _get_description_pipeline(self):
        ...
        if self._description_pipeline is None:
            ...
            self._description_pipeline = pipeline(
                "image-to-text", model=self.image_model
            )
    ...
    def _load_data(
        ...
        try:
            ...
            raise ImportError(
                "PIL is required. Install with: pip install Pillow"
            ) from e
