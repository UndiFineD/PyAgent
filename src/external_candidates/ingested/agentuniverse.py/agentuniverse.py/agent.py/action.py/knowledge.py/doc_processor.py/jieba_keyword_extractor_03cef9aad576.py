# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\action\knowledge\doc_processor\jieba_keyword_extractor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
    ...
    def _process_docs(
        ...
        for _doc in origin_docs:
            ...
            filtered_words = [
                word
                for word in words
                if word not in chinese_stopwords and word.lower() not in stop_words
            ]
            keywords = jieba.analyse.extract_tags(
                " ".join(filtered_words), topK=self.top_k
            )
