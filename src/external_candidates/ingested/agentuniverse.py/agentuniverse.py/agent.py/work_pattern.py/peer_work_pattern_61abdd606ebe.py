# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\work_pattern\peer_work_pattern.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PeerWorkPattern(WorkPattern):
    ...
    def invoke(
        self, input_object: InputObject, work_pattern_input: dict, **kwargs
    ) -> dict:
        ...
        for _ in range(retry_count):
            ...
            if not executing_result or jump_step in ["planning", "executing"]:
                executing_result = self._invoke_executing(
                    input_object, peer_round_results
                )
    ...
    async def async_invoke(
        ...
        for _ in range(retry_count):
            ...
            if not executing_result or jump_step in ["planning", "executing"]:
                executing_result = await self._async_invoke_executing(
                    input_object, peer_round_results
                )
            ...
            if not expressing_result or jump_step in [
                ...
                expressing_result = await self._async_invoke_expressing(
                    input_object, peer_round_results
                )
            ...
            if not reviewing_result or jump_step in [
                ...
                reviewing_result = await self._async_invoke_reviewing(
                    input_object, peer_round_results
                )
    ...
    def _invoke_executing(
        self, input_object: InputObject, peer_round_results: dict
    ) -> dict:
    ...
    async def _async_invoke_executing(
        self, input_object: InputObject, peer_round_results: dict
    ) -> dict:
        if not self.executing:
            executing_result = OutputObject({})
    ...
    async def _async_invoke_expressing(
        self, input_object: InputObject, peer_round_results: dict
    ) -> dict:
        if not self.expressing:
            expressing_result = OutputObject({})
        else:
            expressing_result = await self.expressing.async_run(
    ...
    async def _async_invoke_reviewing(
        self, input_object: InputObject, peer_round_results: dict
    ) -> dict:
        if not self.reviewing:
            reviewing_result = OutputObject({})
    ...
    def _validate_work_pattern_members(self):
        if self.planning and not isinstance(self.planning, PlanningAgentTemplate):
            raise ValueError(
                f"{self.planning} is not of the expected type AgentTemplate."
            )
        if self.executing and not isinstance(self.executing, ExecutingAgentTemplate):
            raise ValueError(
                f"{self.executing} is not of the expected type AgentTemplate."
            )
        if self.expressing and not isinstance(self.expressing, ExpressingAgentTemplate):
            raise ValueError(
                f"{self.expressing} is not of the expected type AgentTemplate."
            )
        if self.reviewing and not isinstance(self.reviewing, ReviewingAgentTemplate):
            raise ValueError(
                f"{self.reviewing} is not not of the expected type ReviewingAgentTemplate."
            )
