...
class ReActPlanner(Planner):
    ...
    @staticmethod
    def get_run_config(
        agent_model: AgentModel, input_object: InputObject
    ) -> RunnableConfig:
        config = RunnableConfig()
        callbacks = []
        output_stream = input_object.get_data("output_stream")
        callbacks.append(
            StreamOutPutCallbackHandler(output_stream, agent_info=agent_model.info)
        )
        callbacks.append(
            InvokeCallbackHandler(
    ...
    def acquire_tools(action) -> list[LangchainTool]:
        ...
        for knowledge_name in knowledge:
            knowledge_tool: Knowledge = KnowledgeManager().get_instance_obj(
                knowledge_name
            )
