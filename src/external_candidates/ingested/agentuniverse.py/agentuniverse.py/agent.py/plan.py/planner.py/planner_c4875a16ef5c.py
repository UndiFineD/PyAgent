# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\plan\planner\planner.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Planner(ComponentBase):
    ...
    def handle_memory(
        self, agent_model: AgentModel, planner_input: dict
    ) -> Memory | None:
        ...
        # get memory instance
        memory: Memory = MemoryManager().get_instance_obj(
            component_instance_name=memory_name
        )
        if memory is None:
            return None
    ...
    def run_all_actions(
        ...
        for agent_name in agents:
            ...
            if agent is None:
                continue
            agent_input = {
                key: input_object.get_data(key) for key in agent.input_keys()
            }
    ...
    def initialize_by_component_configer(
        self, component_configer: PlannerConfiger
    ) -> "Planner":
    ...
    def invoke_chain(
        ...
        if not input_object.get_data("output_stream"):
            res = chain.invoke(
                input=planner_input, config={"configurable": {"session_id": "unused"}}
            )
            return res
        result = []
        for token in chain.stream(
            ...
            self.stream_output(
                ...
                {
                    "type": "token",
                    "data": {"chunk": token, "agent_info": agent_model.info},
                },
            )
            result.append(token)
        return "".join(result)
