# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\plan\planner\peer_planner\peer_planner.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PeerPlanner(Planner):
    ...
    def invoke(
        self, agent_model: AgentModel, planner_input: dict, input_object: InputObject
    ) -> dict:
        """Invoke the planner.
        ...
        return self.agents_run(
            agent_model, sub_agents, planner_config, planner_input, input_object
        )
    ...
    def generate_sub_agents(planner_config: dict) -> dict:
        ...
        for config_key, default_agent in default_sub_agents.items():
            ...
            if config_data == "":
                continue
            agents[config_key] = AgentManager().get_instance_obj(
                config_data if config_data else default_agent
            )
        return agents
    ...
    def build_expert_framework(planner_config: dict, input_object: InputObject):
        ...
        if expert_framework:
            ...
            if selector:
                selector_result = (
                    ToolManager()
                    .get_instance_obj(selector)
                    .run(**input_object.to_dict())
                )
    ...
    def agents_run(
        ...
        for _ in range(retry_count):
            ...
            if not planning_result or jump_step == "planning":
                planning_result = self.planning_agent_run(
                    planningAgent, input_object, agent_input, peer_memory
                )

            if not executing_result or jump_step in ["planning", "executing"]:
                executing_result = self.executing_agent_run(
                    executingAgent, input_object, agent_input, peer_memory
                )

            if not expressing_result or jump_step in [
                "planning",
                "executing",
                "expressing",
            ]:
                expressing_result = self.expressing_agent_run(
                    expressingAgent, input_object, agent_input, peer_memory
                )

            if not reviewing_result or jump_step in [
                "planning",
                "executing",
                "expressing",
                "reviewing",
            ]:
                reviewing_result = self.reviewing_agent_run(
                    reviewingAgent, input_object, agent_input, peer_memory
                )

            if not reviewing_result.to_dict() or (
                reviewing_result.get_data("score")
                and reviewing_result.get_data("score") >= eval_threshold
            ):
                loopResults.append(
                    {
    ...
    def planning_agent_run(
        ...
        """
        planning_agent_name = (
            planning_agent.agent_model.info.get("name") if planning_agent else ""
        )
        ...
        if planning_agent:
            ...
            assemble_memory_output(
                peer_memory, agent_input, content, planning_agent_name
            )
    ...
    def executing_agent_run(
        ...
        executing_agent_name = (
            executing_agent.agent_model.info.get("name") if executing_agent else ""
        )
        ...
        if executing_result.get_data("executing_result"):
            for index, one_exec_res in enumerate(
                executing_result.get_data("executing_result")
            ):
        ...
        if executing_agent:
            ...
            assemble_memory_output(
                peer_memory, agent_input, content, executing_agent_name
            )
        return executing_result

    def expressing_agent_run(
        self,
        expressing_agent: Agent,
        input_object: InputObject,
        agent_input: dict,
        peer_memory: Memory,
    ) -> OutputObject:
        ...
        """
        expressing_agent_name = (
            expressing_agent.agent_model.info.get("name") if expressing_agent else ""
        )
        ...
        if expressing_agent:
            ...
            assemble_memory_output(
                peer_memory, agent_input, content, expressing_agent_name
            )
    ...
    def reviewing_agent_run(
        ...
        """
        reviewing_agent_name = (
            reviewing_agent.agent_model.info.get("name") if reviewing_agent else ""
        )
        ...
        reviewing_info_str = (
            f"review suggestion: {reviewing_result.get_data('suggestion')} \n"
        )
        ...
        if reviewing_agent:
            ...
            assemble_memory_output(
                peer_memory, agent_input, content, reviewing_agent_name
            )
