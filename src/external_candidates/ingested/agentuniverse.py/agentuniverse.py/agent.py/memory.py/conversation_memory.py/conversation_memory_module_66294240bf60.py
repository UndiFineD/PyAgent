...
def generate_relation_str(
    source: str, target: str, source_type: str, target_type: str, type: str
):
...
def sync_to_sub_agent_memory(
    message: ConversationMessage, session_id: str, memory_name: str
):
...
class ConversationMemoryModule:
    ...
    def __init__(self):
        conversation_memory_configer = (
            ApplicationConfigManager().app_configer.conversation_memory_configer
        )
        ...
        self.collection_types = conversation_memory_configer.get(
            "collection_types", ["agent", "user"]
        )
        self.conversation_format = conversation_memory_configer.get(
            "conversation_format", "cn"
        )
        self.max_content_length = conversation_memory_configer.get(
            "max_content_length", 8000
        )
        self.queue = queue.Queue(1000)
        self.thread_pool = ThreadPoolExecutor(
            max_workers=conversation_memory_configer.get("thread_pool", 4)
        )
    ...
    def _add_trace_info(
        ...
        **kwargs,
        ...
        if self.conversation_format == "cn":
            prefix = generate_relation_str(
                source, target, source_type, target_type, type
            )
        else:
            prefix = generate_relation_str_en(
                source, target, source_type, target_type, type
            )
        if not prefix:
            return
    ...
    def add_trace_info(
        self, start_info: dict, target_info: dict, type: str, params: dict, pair_id: str
    ):
        ...
        def add_trace():
            self._add_trace(
                start_info, target_info, type, params, session_id, trace_id, pair_id
            )
    ...
    def add_agent_result_info(
        ...
        def add_trace():
            ...
            self._add_trace(
                target_info, start_info, "output", params, session_id, trace_id, pair_id
            )

        self.queue.put_nowait(add_trace)

    def add_llm_input_info(
        self, start_info: dict, target: str, prompt: str, pair_id: str, auto=True
    ):
        ...
        self.add_trace_info(
            start_info, target_info, "input", {"input": prompt}, pair_id
        )

    def add_llm_output_info(
        self, start_info: dict, target: str, output: str, pair_id: str, auto=True
    ):
        if not self.collection_current_agent_memory(start_info, "llm", auto):
            return
        target_info = {"source": target, "type": "llm"}
        self.add_trace_info(
            start_info, target_info, "output", {"output": output}, pair_id
        )

    def collection_current_agent_memory(
        self, info: dict, collection_type: str, auto: bool
    ):
        if not auto:
            return True
        if not self.activate:
        ...
        if info.get("type") == "agent":
            ...
            if agent_instance:
                collection_types = agent_instance.agent_model.memory.get(
                    "collection_types"
                )
