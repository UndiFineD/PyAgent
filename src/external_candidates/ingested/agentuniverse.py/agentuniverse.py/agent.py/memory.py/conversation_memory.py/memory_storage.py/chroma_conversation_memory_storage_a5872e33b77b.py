...
class ChromaConversationMemoryStorage(MemoryStorage):
    ...
    def add(
        ...
        **kwargs,
        ...
        for message in message_list:
            ...
            metadata["session_id"] = (
                session_id if session_id else message.conversation_id
            )
        ...
        **kwargs,
        ...
        if input:
            ...
            if len(embedding) > 0:
                ...
                results = self._collection.query(
                    query_texts=[input], where=filters, n_results=top_k
                )
    ...
    def to_messages(
        self, result: dict, sort_by_time: bool = False
    ) -> List[ConversationMessage]:
            ...
            if self.is_nested_list(result["ids"]):
                ...
                message_list = [
                    ConversationMessage(
                        id=ids[0][i],
                        conversation_id=(
                            metadatas[0][i].get("session_id", None)
                            if metadatas[0]
                            else None
                        ),
                        ...
                        source=(
                            metadatas[0][i].get("source", None)
                            if metadatas[0]
                            else None
                        ),
                        source_type=metadatas[0][i].get("source_type", ""),
                        target=(
                            metadatas[0][i].get("target", None)
                            if metadatas[0]
                            else None
                        ),
                        target_type=(
                            metadatas[0][i].get("target_type", "")
                            if metadatas[0]
                            else ""
                        ),
                        trace_id=(
                            metadatas[0][i].get("trace_id", "") if metadatas[0] else ""
                        ),
                        type=metadatas[0][i].get("type", "") if metadatas[0] else "",
                        additional_args=json.loads(
                            metadatas[0][i].get("additional_args", "{}")
                        ),
                    )
                ...
                message_list = [
                    ConversationMessage(
                        id=ids[i],
                        conversation_id=(
                            metadatas[i].get("session_id", None)
                            if metadatas[i]
                            else None
                        ),
                        source_type=metadatas[i].get("source_type", ""),
                        target_type=(
                            metadatas[i].get("target_type", "") if metadatas[i] else ""
                        ),
                        target=(
                            metadatas[i].get("target", None) if metadatas[i] else None
                        ),
                        trace_id=(
                            metadatas[i].get("trace_id", "") if metadatas[i] else ""
                        ),
                        source=(
                            metadatas[i].get("source", None) if metadatas[i] else None
                        ),
                        content=documents[i],
                        metadata=metadatas[i] if metadatas[i] else None,
                        type=metadatas[i].get("type", "") if metadatas[i] else "",
                        additional_args=json.loads(
                            metadatas[i].get("additional_args", "{}")
                        ),
                    )
    ...
    def is_nested_list(variable: List) -> bool:
        return (
            ...
            and len(variable) > 0
            and isinstance(variable[0], list)
        )
