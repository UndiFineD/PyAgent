# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\memory\conversation_memory\memory_storage\sqlite_conversation_memory_storage.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def create_memory_model(table_name: str, DynamicBase: Any) -> Any:
    ...
    class MemoryModel(DynamicBase):
        ...
        __table_args__ = (
            Index(
                f"idx_{table_name}_session_id_source",
                "session_id",
                "source",
                "source_type",
            ),
            Index(
                f"idx_{table_name}_session_id_source_type",
                "session_id",
                "target",
                "target_type",
            ),
            Index(
                f"idx_{table_name}_session_id_gmt_created", "session_id", "timestamp"
            ),
...
class DefaultMemoryConverter(BaseMemoryConverter):
    ...
    def to_sql_model(
        self, message: ConversationMessage, session_id: str = None, **kwargs
    ) -> Any:
        """Convert a Message instance to a SQLAlchemy model."""
...
class SqliteMemoryStorage(MemoryStorage):
    ...
    def add(
        self,
        message_list: List[ConversationMessage],
        session_id: str = None,
        agent_id: str = None,
        **kwargs,
    ) -> None:
        ...
        """
        message_list = ConversationMessage.check_and_convert_message(
        ...
        with self.session() as session:
            for message in message_list:
                existing_message = (
                    session.query(self.memory_converter.get_sql_model_class())
