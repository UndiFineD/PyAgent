# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\memory\memory.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Memory(ComponentBase):
    ...
    def prune(self, memories: List[Message]) -> List[Message]:
        ...
        agent_llm_name = (
            self.agent_llm_name if hasattr(self, "agent_llm_name") else None
        )
        ...
        if pruned_memories:
            memory_compressor: MemoryCompressor = (
                MemoryCompressorManager().get_instance_obj(self.memory_compressor)
            )
            if memory_compressor:
                compressed_memory = memory_compressor.compress_memory(
                    pruned_memories, self.max_tokens - tokens
                )
    ...
    def summarize_memory(self, **kwargs) -> str:
        ...
        summarize_content = (
            summarize_messages[-1].content
            if summarize_messages and len(summarize_messages) > 0
            else ""
        )
    ...
    def initialize_by_component_configer(
        self, component_configer: MemoryConfiger
    ) -> "Memory":
        ...
        if component_configer.type:
            self.type = next(
                (
                    ...
                    if member.value == component_configer.type
                )
            )
