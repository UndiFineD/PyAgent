# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\memory\memory_storage\qdrant_memory_storage.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class QdrantMemoryStorage(MemoryStorage):
    ...
    def _build_filter(
        ...
        if type_value:
            must_conditions.append(
                FieldCondition(key="type", match=MatchValue(value=type_value[0]))
            )
        ...
        **kwargs,
        ...
        for message in message_list:
            ...
            if self.embedding_model:
                try:
                    vector = (
                        ...
                        .get_embeddings([str(message.content)])
                    )[0]
                except Exception:
                    vector = []
            ...
            if vector:
                ...
                raise ValueError(
                    "No vectors available for message. Cannot store message without embeddings."
                )
            ...
            try:
                point_id = (
                    str(uuid.UUID(str(message.id))) if message.id else str(uuid.uuid4())
                )
            except Exception:
    ...
    def get(
        ...
        **kwargs,
        ...
        messages = sorted(
            messages, key=lambda msg: (msg.metadata or {}).get("gmt_created", "")
        )
        messages.reverse()
