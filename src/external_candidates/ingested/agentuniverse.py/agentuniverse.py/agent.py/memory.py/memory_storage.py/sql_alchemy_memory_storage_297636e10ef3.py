...
class DefaultMemoryConverter(BaseMemoryConverter):
    ...
    def from_sql_model(self, sql_message: Any) -> Message:
        ...
        return Message.from_dict(
            {"id": sql_message.id, **json.loads(sql_message.message)}
        )
...
class SqlAlchemyMemoryStorage(MemoryStorage):
    ...
    def _init_db(self) -> None:
        ...
        self._sqldb_wrapper = SQLDBWrapperManager().get_instance_obj(
            self.sqldb_wrapper_name
        )
        if self._sqldb_wrapper is None:
            raise Exception(
                "The sqldb_wrapper for the `sqldb_wrapper_name` was not found,"
