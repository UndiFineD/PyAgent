...
class ReActAgentTemplate(AgentTemplate):
    ...
    def build_tools_context(self) -> tuple[str, str]:
        ...
        for lc_tool in lc_tools:
            tools_context += (
                f"tool name: {lc_tool.name}, tool description: {lc_tool.description}\n"
            )
    ...
    def _get_run_config(self, input_object: InputObject) -> RunnableConfig:
        ...
        callbacks.append(
            StreamOutPutCallbackHandler(output_stream, agent_info=self.agent_model.info)
        )
        callbacks.append(
            InvokeCallbackHandler(
    ...
    def initialize_by_component_configer(
        ...
        self.prompt_version = self.agent_model.profile.get(
            "prompt_version", "default_react_agent.cn"
        )
