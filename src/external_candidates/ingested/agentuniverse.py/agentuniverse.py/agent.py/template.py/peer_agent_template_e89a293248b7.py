# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\template\peer_agent_template.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class PeerAgentTemplate(AgentTemplate):
    ...
    def execute(self, input_object: InputObject, agent_input: dict, **kwargs) -> dict:
        ...
        peer_work_pattern: PeerWorkPattern = WorkPatternManager().get_instance_obj(
            "peer_work_pattern"
        )
    ...
    async def async_execute(
        self, input_object: InputObject, agent_input: dict, **kwargs
    ) -> dict:
        memory: Memory = self.process_memory(agent_input, **kwargs)
        agents = self._generate_agents()
        peer_work_pattern: PeerWorkPattern = WorkPatternManager().get_instance_obj(
    ...
    async def customized_async_execute(
        ...
        **kwargs,
        ...
        self.build_expert_framework(input_object)
        work_pattern_result = await peer_work_pattern.async_invoke(
    ...
    def _generate_agents(self) -> dict:
        planning_agent = self._get_and_validate_agent(
            self.planning_agent_name, PlanningAgentTemplate
        )
        executing_agent = self._get_and_validate_agent(
            self.executing_agent_name, ExecutingAgentTemplate
        )
        expressing_agent = self._get_and_validate_agent(
            self.expressing_agent_name, ExpressingAgentTemplate
        )
        reviewing_agent = self._get_and_validate_agent(
            self.reviewing_agent_name, ReviewingAgentTemplate
        )
        return {
    ...
    def add_peer_memory(
        self, peer_memory: Memory, agent_input: dict, work_pattern_result: dict
    ):
        if not peer_memory:
            return
    ...
    def initialize_by_component_configer(
        ...
        if (
            self.agent_model.profile.get("planning") is not None
            or planner_config.get("planning") is not None
        ):
            self.planning_agent_name = (
        ...
        if (
            self.agent_model.profile.get("executing") is not None
            or planner_config.get("executing") is not None
        ):
            self.executing_agent_name = (
        ...
        if (
            self.agent_model.profile.get("expressing") is not None
            or planner_config.get("expressing") is not None
        ):
            self.expressing_agent_name = (
        ...
        if (
            self.agent_model.profile.get("reviewing") is not None
            or planner_config.get("reviewing") is not None
        ):
            self.reviewing_agent_name = (
        ...
        if self.agent_model.profile.get("eval_threshold") or planner_config.get(
            "eval_threshold"
        ):
            self.eval_threshold = self.agent_model.profile.get(
                "eval_threshold"
            ) or planner_config.get("eval_threshold")
        if self.agent_model.profile.get("retry_count") or planner_config.get(
            "retry_count"
        ):
            self.retry_count = self.agent_model.profile.get(
                "retry_count"
            ) or planner_config.get("retry_count")
        if self.agent_model.profile.get("jump_step") or planner_config.get("jump_step"):
            self.jump_step = self.agent_model.profile.get(
                "jump_step"
            ) or planner_config.get("jump_step")
        if self.agent_model.profile.get("expert_framework") or planner_config.get(
            "expert_framework"
        ):
            self.expert_framework = self.agent_model.profile.get(
                "expert_framework"
            ) or planner_config.get("expert_framework")
