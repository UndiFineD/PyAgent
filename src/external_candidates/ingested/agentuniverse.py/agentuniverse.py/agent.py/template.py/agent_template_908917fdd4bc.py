# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\template\agent_template.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class AgentTemplate(Agent, ABC):
    ...
    def execute(self, input_object: InputObject, agent_input: dict, **kwargs) -> dict:
        ...
        return self.customized_execute(
            input_object, agent_input, memory, llm, prompt, **kwargs
        )
    ...
    async def async_execute(
        ...
        llm: LLM = self.process_llm(**kwargs)
        prompt: Prompt = self.process_prompt(agent_input, **kwargs)
        return await self.customized_async_execute(
            input_object, agent_input, memory, llm, prompt, **kwargs
        )
    ...
    def customized_execute(
        ...
        **kwargs,
        ...
        self.add_memory(
            memory,
            f"Human: {agent_input.get('input')}, AI: {res}",
    ...
    def initialize_by_component_configer(
        ...
        self.conversation_memory_name = self.agent_model.memory.get(
            "conversation_memory", ""
        )
        return self
    ...
    def invoke_tools(self, input_object: InputObject, **kwargs) -> str:
        return super().invoke_tools(
            input_object=input_object, tool_names=self.tool_names
        )

    async def async_invoke_tools(self, input_object: InputObject, **kwargs) -> str:
        return await super().async_invoke_tools(
            input_object=input_object, tool_names=self.tool_names
        )
    ...
    def process_prompt(self, agent_input: dict, **kwargs) -> ChatPrompt:
        return super().process_prompt(
            agent_input=agent_input, prompt_version=self.prompt_version
        )
    ...
    def create_copy(self) -> "AgentTemplate":
        ...
        copied.knowledge_names = (
            self.knowledge_names.copy() if self.knowledge_names is not None else None
        )
