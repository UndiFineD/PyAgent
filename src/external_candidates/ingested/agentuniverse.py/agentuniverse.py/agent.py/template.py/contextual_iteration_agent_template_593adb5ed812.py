...
class ContextualIterationAgentTemplate(AgentTemplate):
    ...
    def customized_execute(
        ...
        **kwargs,
        ...
        conversation_history.append(
            {"user": lc_prompt_template.format(**prompt_input_dict), "assistant": res}
        )
        agent_input["chat_history"] = json.dumps(
        ...
        for i in range(self.iteration):
            continue_res = self.invoke_chain(
                continue_chain, agent_input, input_object, **kwargs
            )
            res = res + "\n" + continue_res
            ...
            if not self.if_loop_prompt_version:
                continue
            loop_res = self.invoke_chain(
                if_loop_prompt_chain, agent_input, input_object, **kwargs
            )
    ...
    async def customized_async_execute(
        ...
        **kwargs,
        ...
        for i in range(self.iteration):
            continue_res = await self.async_invoke_chain(
                continue_chain, agent_input, input_object, **kwargs
            )
            res = res + "\n" + continue_res
            ...
            if not self.if_loop_prompt_version:
                continue
            loop_res = await self.async_invoke_chain(
                if_loop_prompt_chain, agent_input, input_object, **kwargs
            )
            if not self.if_loop(loop_res):
                break
    ...
    def initialize_by_component_configer(
        ...
        if self.iteration > 0:
            try:
                self.continue_prompt_version = (
                    component_configer.continue_prompt_version
                )
            except AttributeError as e:
                LOGGER.error(
                    "Contextual iteration agent need continue_prompt_version while iteration > 0"
                )
