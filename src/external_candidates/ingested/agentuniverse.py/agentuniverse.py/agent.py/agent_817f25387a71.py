# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent\agent.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Agent(ComponentBase, ABC):
    ...
    def execute(self, input_object: InputObject, agent_input: dict) -> dict:
        ...
        planner_result = planner_base.invoke(
            self.agent_model, agent_input, input_object
        )
    ...
    def get_instance_code(self) -> str:
        ...
        return f"{appname}." f"{self.component_type.value.lower()}.{name}"
    ...
    def initialize_by_component_configer(
        ...
        profile = process_dict_with_funcs(
            profile, component_configer.yaml_func_instance
        )
        profile = process_agent_llm_config(
            info.get("name"), profile, component_configer.default_llm_configer
        )
    ...
    def langchain_run(self, input: str, callbacks=None, **kwargs):
        ...
        try:
            ...
            LOGGER.error(
                f"langchain run parse_json_markdown error,input(parse_result) error({str(e)})"
            )
    ...
    async def async_langchain_run(self, input: str, callbacks=None, **kwargs):
        ...
        try:
            ...
            return "Error , Your Action Input is not a valid JSON string"
        output_object = await self.async_run(
            **parse_result, callbacks=callbacks, **kwargs
        )
        result_dict = {}
        for key in self.output_keys():
            result_dict[key] = output_object.get_data(key)
    ...
    def process_llm(self, **kwargs) -> LLM:
        llm_name = kwargs.get("llm_name") or self.agent_model.profile.get(
            "llm_model", {}
        ).get("name")
    ...
    def process_memory(self, agent_input: dict, **kwargs) -> Memory | None:
        ...
        conversation_memory_name = kwargs.get(
            "conversation_memory"
        ) or self.agent_model.memory.get("conversation_memory")
        ...
        params: dict = dict()
        params["agent_llm_name"] = kwargs.get(
            "llm_name"
        ) or self.agent_model.profile.get("llm_model", {}).get("name")
        ...
        **kwargs,
        ...
        for token in chain.stream(input=agent_input, config=self.get_run_config()):
            stream_output(
                ...
                {
                    "type": "token",
                    "data": {"chunk": token, "agent_info": self.agent_model.info},
                },
            )
            result.append(token)
    ...
    async def async_invoke_chain(
        ...
        **kwargs,
        ...
        async for token in chain.astream(
            input=agent_input, config=self.get_run_config()
        ):
            stream_output(
                input_object.get_data("output_stream", None),
                {
    ...
    def invoke_tools(self, input_object: InputObject, **kwargs) -> str:
        ...
        for tool_name in tool_names:
            ...
            try:
                tool_input = {
                    key: input_object.get_data(key) for key in tool.input_keys
                }
                tool_results.append(str(tool.run(**tool_input)))
            except:
                LOGGER.warn(
                    f"Tool {tool_name} call failed, maybe invalid or lack arguments"
                )
        return "\n\n".join(tool_results)
    ...
    def invoke_knowledge(
        self, query_str: str, input_object: InputObject, **kwargs
    ) -> str:
        knowledge_names = kwargs.get("knowledge_names") or self.agent_model.action.get(
            "knowledge", []
        )
        ...
        for knowledge_name in knowledge_names:
            ...
            knowledge_res: List[Document] = knowledge.query_knowledge(
                query_str=query_str, **input_object.to_dict()
            )
    ...
    def process_prompt(self, agent_input: dict, **kwargs) -> ChatPrompt:
        ...
        prompt_version = kwargs.get("prompt_version") or self.agent_model.profile.get(
            "prompt_version"
        )
        ...
        if version_prompt:
            version_prompt_model: AgentPromptModel = AgentPromptModel(
                ...
                instruction=expert_framework
                + getattr(version_prompt, "instruction", ""),
            )
            profile_prompt_model = profile_prompt_model + version_prompt_model

        chat_prompt = ChatPrompt().build_prompt(
            profile_prompt_model, ["introduction", "target", "instruction"]
        )
    ...
    def get_memory_params(self, agent_input: dict) -> dict:
        ...
        if not self.agent_model.memory.get("name") and self.agent_model.memory.get(
            "conversation_memory"
        ):
    ...
    def get_run_config(self, **kwargs) -> dict:
        ...
        callbacks = [
            InvokeCallbackHandler(
                source=self.agent_model.info.get("name"), llm_name=llm_name
            )
        ]
    ...
    def load_summarize_memory(
        self, memory: Memory, agent_input: dict[str, Any] = {}
    ) -> str:
        if memory:
            params = self.get_memory_params(agent_input)
