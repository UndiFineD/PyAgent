# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\default\default_ollama_llm.py
import json
...
class OllamaLLM(LLM):
    ...
    def _call(
        ...
        if should_stream:
            ...
            return LLMOutput(
                text=res.get("message").get("content"), raw=json.dumps(res)
            )
    ...
    def generate_result(self, data):
        for line in data:
            yield LLMOutput(
                text=line.get("message").get("content"), raw=json.dumps(line)
            )
    ...
    def initialize_by_component_configer(
        ...
        if "base_url" in component_configer.configer.value:
            ...
            self.base_url = process_yaml_func(
                base_url, component_configer.yaml_func_instance
            )
