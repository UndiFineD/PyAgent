# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\default\kimi_openai_style_llm.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class KIMIOpenAIStyleLLM(OpenAIStyleLLM):
    ...
    api_base: Optional[str] = Field(
        default_factory=lambda: get_from_env("KIMI_API_BASE")
        or "https://api.moonshot.cn/v1"
    )
    proxy: Optional[str] = Field(default_factory=lambda: get_from_env("KIMI_PROXY"))
    organization: Optional[str] = Field(
        default_factory=lambda: get_from_env("KIMI_ORGANIZATION")
    )
