# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\default\claude_llm.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class ClaudeLLM(LLM):
    ...
    def initialize_by_component_configer(
        ...
        if "api_key" in component_configer.configer.value:
            ...
            self.api_key = process_yaml_func(
                api_key, component_configer.yaml_func_instance
            )
        if "api_url" in component_configer.configer.value:
            api_url = component_configer.configer.value.get("api_url")
            self.api_url = process_yaml_func(
                api_url, component_configer.yaml_func_instance
            )
        ...
        if "connection_pool_limits" in component_configer.configer.value:
            self.connection_pool_limits = component_configer.configer.value.get(
