# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\default\baichuan_openai_style_llm.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class BAICHUANOpenAIStyleLLM(OpenAIStyleLLM):
    ...
    api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("BAICHUAN_API_KEY")
    )
    api_base: Optional[str] = Field(
        default_factory=lambda: get_from_env("BAICHUAN_API_BASE")
        or "https://api.baichuan-ai.com/v1"
    )
    proxy: Optional[str] = Field(default_factory=lambda: get_from_env("BAICHUAN_PROXY"))
    organization: Optional[str] = Field(
        default_factory=lambda: get_from_env("BAICHUAN_ORGANIZATION")
    )
