# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\default\deep_seek_openai_style_llm.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class DefaultDeepSeekLLM(OpenAIStyleLLM):
    ...
    api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("DEEPSEEK_API_KEY")
    )
    organization: Optional[str] = Field(
        default_factory=lambda: get_from_env("DEEPSEEK_ORGANIZATION")
    )
    api_base: Optional[str] = Field(
        default_factory=lambda: get_from_env("DEEPSEEK_API_BASE")
    )
