# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\llm_channel\llm_channel.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class LLMChannel(ComponentBase):
    ...
    def _initialize_by_component_configer(
        ...
        if hasattr(component_configer, "model_support_max_context_length"):
            self.model_support_max_context_length = (
                component_configer.model_support_max_context_length
            )
        ...
        if hasattr(component_configer, "model_is_openai_protocol_compatible"):
            self.model_is_openai_protocol_compatible = (
                component_configer.model_is_openai_protocol_compatible
            )
        if component_configer.configer.value.get("extra_headers"):
            self.ext_headers = component_configer.configer.value.get(
                "extra_headers", {}
            )
            ...
            for key, value in config.items():
                ...
                if key == "max_tokens":
                    if self.model_support_max_tokens:
                        value = (
                            min(self.model_support_max_tokens, value)
                            if value
                            else self.model_support_max_tokens
                        )
    ...
    def _call(
        ...
        streaming = (
            ...
            else self.channel_model_config.get("streaming")
        )
        if "stream" in kwargs:
        ...
        max_tokens = (
            ...
            or self.channel_model_config.get("max_tokens", None)
            or support_max_tokens
        )
        ...
        self.client.base_url = (
            kwargs.pop("api_base") if kwargs.get("api_base") else self.channel_api_base
        )
    ...
    def _new_client(self):
        ...
        return OpenAI(
            ...
            http_client=(
                httpx.Client(proxy=self.channel_proxy) if self.channel_proxy else None
            ),
    ...
    def _new_async_client(self):
        ...
        return AsyncOpenAI(
            ...
            max_retries=self.channel_model_config.get("max_retries"),
            http_client=(
                httpx.AsyncClient(proxy=self.channel_proxy)
    ...
    def parse_result(chunk):
        ...
        if len(chunk["choices"]) == 0:
            return LLMOutput(
                text="", raw=chunk, usage=TokenUsage.from_openai(chunk.get("usage", {}))
            )
