# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\llm_manager.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class LLMManager(ComponentManagerBase):
    ...
    def get_instance_obj(
        ...
        if instance_obj is None:
            ...
            llm_configer_map: dict[str, LLMConfiger] = (
                ApplicationConfigManager().app_configer.llm_configer_map
            )
                ...
                if llm_configer:
                    ...
                    if llm_configer.meta_class:
                        metadata_module = ".".join(
                            llm_configer.meta_class.split(".")[:-1]
                        )
                    ...
                    instance_obj: LLM = (
                        component_clz().initialize_by_component_configer(llm_configer)
                    )
                    if instance_obj:
