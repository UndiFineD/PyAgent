# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\llm\openai_llm.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class OpenAILLM(LLM):
    ...
    openai_api_key: Optional[str] = Field(
        default_factory=lambda: get_from_env("OPENAI_API_KEY")
    )
    openai_organization: Optional[str] = Field(
        default_factory=lambda: get_from_env("OPENAI_ORGANIZATION")
    )
    openai_api_base: Optional[str] = Field(
        default_factory=lambda: get_from_env("OPENAI_API_BASE")
    )
    openai_proxy: Optional[str] = Field(
        default_factory=lambda: get_from_env("OPENAI_PROXY")
    )
    ...
    def _new_client(self):
        ...
        return OpenAI(
            ...
            http_client=(
                httpx.Client(proxy=self.openai_proxy) if self.openai_proxy else None
            ),
    ...
    @classmethod
    async def agenerate_stream_result(
        cls, stream: AsyncIterator
    ) -> AsyncIterator[LLMOutput]:
        """Generate the result of the stream."""
