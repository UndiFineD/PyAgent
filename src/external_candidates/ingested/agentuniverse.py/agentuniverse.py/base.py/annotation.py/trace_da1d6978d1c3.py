# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\annotation\trace.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def _process_knowledge(source, knowledge_input, start_info, pair_id):
    ...
    """
    ConversationMemoryModule().add_knowledge_input_info(
        start_info, source, knowledge_input, pair_id
    )
...
def _handle_knowledge_result(start_info, source, result, pair_id):
    ...
    Returns:
        The execution result
    """
    ConversationMemoryModule().add_knowledge_output_info(
        start_info, source, params=result, pair_id=pair_id
    )
    return result
...
async def _default_knowledge_wrapper_async(func, *args, **kwargs):
    ...
    knowledge_instance, source, knowledge_input, start_info, pair_id = (
        _get_knowledge_info(func, *args, **kwargs)
    )
