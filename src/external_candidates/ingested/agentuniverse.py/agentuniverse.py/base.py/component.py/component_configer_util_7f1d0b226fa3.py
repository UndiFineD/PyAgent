...
class ComponentConfigerUtil(object):
    ...
    def get_component_config_clz_by_type(
        ...
        if component_config_clz is None:
            raise Exception(
                f"Failed to get the ComponentConfiger class by the component type: {component_type_enum}"
            )
        return component_config_clz

    @classmethod
    def get_component_object_clz_by_component_configer(
        cls, component_configer: ComponentConfiger
    ) -> Callable:
        ...
        if component_configer.meta_class:
            try:
                metadata_module = ".".join(
                    component_configer.meta_class.split(".")[:-1]
                )
    ...
    @classmethod
    def get_component_manager_clz_by_type(
        cls, component_type_enum: ComponentEnum
    ) -> Callable:
