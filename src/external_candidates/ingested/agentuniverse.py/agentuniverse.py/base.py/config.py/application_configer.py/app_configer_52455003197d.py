...
class AppConfiger(object):
    ...
    def load_by_configer(self, configer: Configer) -> "AppConfiger":
        ...
        self.__root_package_name = configer.value.get("PACKAGE_PATH_INFO", {}).get(
            "ROOT_PACKAGE"
        )
        self.__core_default_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "default"
        )
        self.__core_agent_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "agent"
        )
        self.__core_knowledge_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "knowledge"
        )
        self.__core_llm_package_list = configer.value.get("CORE_PACKAGE", {}).get("llm")
        self.__core_planner_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "planner"
        )
        self.__core_tool_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "tool"
        )
        self.__core_toolkit_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "toolkit"
        )
        self.__core_memory_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "memory"
        )
        self.__core_service_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "service"
        )
        self.__core_sqldb_wrapper_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("sqldb_wrapper")
        self.__core_prompt_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "prompt"
        )
        self.__core_product_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "product"
        )
        self.__core_workflow_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "workflow"
        )
        self.__core_embedding_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "embedding"
        )
        self.__core_doc_processor_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("doc_processor")
        self.__core_reader_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "reader"
        )
        self.__core_store_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "store"
        )
        self.__core_rag_router_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("rag_router")
        self.__core_query_paraphraser_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("query_paraphraser")
        self.__core_memory_compressor_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("memory_compressor")
        self.__core_memory_storage_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("memory_storage")
        self.__core_work_pattern_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("work_pattern")
        self.__core_log_sink_package_list = configer.value.get("CORE_PACKAGE", {}).get(
            "log_sink"
        )
        self.__core_llm_channel_package_list = configer.value.get(
            "CORE_PACKAGE", {}
        ).get("llm_channel")
        self.__conversation_memory_configer = configer.value.get(
            "CONVERSATION_MEMORY", {}
        )
        self.__llm_plugins = self.load_llm_plugins(
            configer.value.get("PLUGINS", {}).get("llm_plugins", [])
        )
        return self
