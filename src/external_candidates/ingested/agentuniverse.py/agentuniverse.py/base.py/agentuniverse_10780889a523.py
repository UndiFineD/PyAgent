...
class AgentUniverse(object):
    ...
    def __init__(self):
        ...
        self.__system_default_embedding_package = [
            "agentuniverse.agent.action.knowledge.embedding"
        ]
    ...
    def start(self, config_path: str = None, core_mode: bool = False):
        ...
        sync_service_timeout = configer.value.get("HTTP_SERVER", {}).get(
            "sync_service_timeout"
        )
    ...
    def __scan_and_register(self, app_configer: AppConfiger):
        ...
        core_agent_package_list = (
            app_configer.core_agent_package_list
        ...
        core_planner_package_list = (
            app_configer.core_planner_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_planner_package
        core_tool_package_list = (
            app_configer.core_tool_package_list
            or app_configer.core_default_package_list
        ...
        core_service_package_list = (
            app_configer.core_service_package_list
            or app_configer.core_default_package_list
        )
        core_sqldb_wrapper_package_list = (
            app_configer.core_sqldb_wrapper_package_list
            or app_configer.core_default_package_list
        )
        core_memory_package_list = (
            app_configer.core_memory_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_memory_package
        core_prompt_package_list = (
            app_configer.core_prompt_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_prompt_package
        core_workflow_package_list = (
            app_configer.core_workflow_package_list
            or app_configer.core_default_package_list
        )
        core_embedding_package_list = (
            app_configer.core_embedding_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_embedding_package
        core_doc_processor_package_list = (
            app_configer.core_doc_processor_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_doc_processor_package
        core_reader_package_list = (
            app_configer.core_reader_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_reader_package
        core_store_package_list = (
            app_configer.core_store_package_list
            or app_configer.core_default_package_list
        )
        core_rag_router_package_list = (
            app_configer.core_rag_router_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_rag_router_package
        core_query_paraphraser_package_list = (
            app_configer.core_query_paraphraser_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_query_paraphraser_package
        core_memory_compressor_package_list = (
            app_configer.core_memory_compressor_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_memory_compressor_package
        ...
        core_work_pattern_package_list = (
            app_configer.core_work_pattern_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_work_pattern_package
        core_log_sink_package_list = (
            app_configer.core_log_sink_package_list
            or app_configer.core_default_package_list
        ) + self.__system_default_log_sink_package

        core_llm_channel_package_list = (
            app_configer.core_llm_channel_package_list
            or app_configer.core_default_package_list
        )
    ...
    def scan(
        ...
        if component_enum.value == ComponentEnum.LLM.value:
            ...
            if default_llm_config_path:
                ...
                self.__config_container.app_configer.default_llm_configer = (
                    default_llm_configer
                )
                ...
                if default_llm_configer and default_llm_configer.default_llm:
                    self.__config_container.app_configer.agent_llm_set.add(
                        default_llm_configer.default_llm
                    )
    ...
    def __register(
        ...
        default_llm_configer: DefaultLLMConfiger = (
            self.__config_container.app_configer.default_llm_configer
        )
        ...
        for component_configer in component_configer_list:
            ...
            configer_instance: ComponentConfiger = configer_clz().load_by_configer(
            ...
            if component_enum.value == ComponentEnum.AGENT.value:
                ...
                if (
                    ...
                    if tool_name_list and isinstance(tool_name_list, list):
                        self.__config_container.app_configer.agent_tool_set.update(
                            tool_name_list
                        )
                ...
                if hasattr(configer_instance, "name") and configer_instance.name:
                    if (
                        configer_instance.name
                        not in self.__config_container.app_configer.agent_llm_set
                    ):
                ...
                if hasattr(configer_instance, "name") and configer_instance.name:
                    if hasattr(configer_instance, "as_mcp_tool"):
                        MCPServerManager().register_mcp_tool(
                            configer_instance, component_enum.value
                        )
                    if (
                        configer_instance.name
                        not in self.__config_container.app_configer.agent_tool_set
                    ):
                ...
                if hasattr(configer_instance, "name") and configer_instance.name:
                    ...
                    if (
                        configer_instance.name
                        not in self.__config_container.app_configer.agent_toolkit_set
                    ):
            ...
            component_instance: ComponentBase = (
                component_clz().initialize_by_component_configer(configer_instance)
            )
    ...
    def __parse_sub_config_path(
        self, input_path: str, reference_file_path: str
    ) -> str | None:
