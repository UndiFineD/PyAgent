# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\util\memory_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
        ...
        [
            {
                ...
                "type": "ai" if message.type == "AIMessageChunk" else message.type,
            }
...
def get_memory_string(messages: List[Message], agent_id=None) -> str:
    ...
    for m in messages:
        if m.type == ChatMessageEnum.SYSTEM.value:
            ...
            m.type == ChatMessageEnum.INPUT.value
            or m.type == ChatMessageEnum.OUTPUT.value
        ):
