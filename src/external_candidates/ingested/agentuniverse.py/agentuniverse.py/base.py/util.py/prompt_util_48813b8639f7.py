# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\util\prompt_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def split_text_on_tokens(
    text: str, text_token: int, chunk_size=800, chunk_overlap=100
) -> List[str]:
...
def split_texts(
    texts: list[str], llm: LLM, chunk_size=800, chunk_overlap=100, retry=True
) -> list[str]:
    """
...
def generate_template(
    agent_prompt_model: AgentPromptModel, prompt_assemble_order: list[str]
) -> str:
...
def generate_chat_template(
    ...
    if message_list:
        ...
        system_messages = "\n".join(
            ...
            if msg.type == ChatMessageEnum.SYSTEM.value
        )
        if system_messages:
            message_list = list(
                filter(
                    lambda msg: msg.type != ChatMessageEnum.SYSTEM.value, message_list
                )
            )
            message_list.insert(
                0, Message(type=ChatMessageEnum.SYSTEM.value, content=system_messages)
            )
    return message_list
...
def process_llm_token(
    ...
    prompt_processor_type: str = (
        prompt_processor.get("type") or PromptProcessEnum.TRUNCATE.value
    )
    prompt_processor_llm: str = prompt_processor.get("llm")

    # get the summary and combine prompt versions
    summary_prompt_version: str = (
        prompt_processor.get("summary_prompt_version") or "prompt_processor.summary_cn"
    )
    combine_prompt_version: str = (
        prompt_processor.get("combine_prompt_version") or "prompt_processor.combine_cn"
    )

    prompt_input_dict = {
        key: planner_input[key]
        for key in lc_prompt_template.input_variables
    ...
    input_tokens = agent_llm.max_context_length() - llm_model.get(
    ...
    if content:
        if process_prompt_type_enum == PromptProcessEnum.TRUNCATE:
            planner_input[var_to_process] = truncate_content(
                content, input_tokens, agent_llm
            )
