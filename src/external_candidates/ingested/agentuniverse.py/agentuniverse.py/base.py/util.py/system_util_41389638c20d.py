# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\util\system_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def get_module_path(yaml_path: str, root_name: str) -> str:
    ...
    if not os.path.isfile(py_file):
        raise FileNotFoundError(
            f"Corresponding Python file not found for YAML file: {yaml_path}"
        )
...
def process_yaml_func(func_expr: str, yaml_func_instance: Any) -> str:
    ...
    if func_expr.startswith("@FUNC(") and yaml_func_instance is None:
        raise ValueError(
            f"yaml_func_extension.py is required to resolve @FUNC expression {func_expr}."
        )
    ...
    if func_expr.startswith("@FUNC(") and func_expr.endswith(")"):
        ...
        kwargs = {
            kw.arg: ast.literal_eval(kw.value) for kw in parsed_expr.body.keywords
        }
        ...
        if hasattr(yaml_func_instance, func_name):
            ...
            try:
                sig.bind(
                    *args, **kwargs
                )  # Ensure arguments match the function signature
            ...
            raise AttributeError(
                f"Method {func_name} not found in yaml_func_extension.py"
            )
...
def is_system_builtin(component_instance: ComponentBase) -> bool:
    ...
    try:
        ...
        normalized_config_path = os.path.normpath(
            component_instance.component_config_path
        )
        ...
        print(
            f"An error occurred while checking if the component is system-built-in: {str(e)}"
        )
        return False
...
def is_api_key_missing(component_instance: ComponentBase, api_key_name: str) -> bool:
    ...
    """
    return hasattr(component_instance, api_key_name) and not getattr(
        component_instance, api_key_name
    )
            ...
            if "agentic.llm" in package_path:
                ...
                try:
                    ...
                    if spec and spec.origin:
                        ...
                        default_llm_toml_path = os.path.join(
                            module_dir, "default_llm.toml"
                        )
