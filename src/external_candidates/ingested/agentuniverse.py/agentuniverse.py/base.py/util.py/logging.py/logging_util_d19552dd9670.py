# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\util\logging\logging_util.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from .general_logger import LOG_LEVEL, GeneralLogger
...
def _add_standard_logger():
    ...
    loguru.logger.add(
        ...
        compression=(
            LoggingConfig.log_compression if LoggingConfig.log_compression else None
        ),
        encoding="utf-8",
        enqueue=True,
    )
...
def _add_sls_log_handler():
    ...
    def _sls_filter(record):
        return (
            ...
            or record["extra"].get("log_type") == LogTypeEnum.sls
        )
...
def _add_sls_log_async_handler():
    """Add a handler to record all"""
    from agentuniverse_extension.logger.sls_sink import AsyncSlsSender, AsyncSlsSink
...
def get_module_logger(
    module_name: str, log_level: Optional[LOG_LEVEL] = None
) -> GeneralLogger:
...
def add_sink(sink, log_level: Optional[LOG_LEVEL] = None) -> bool:
    ...
    print(
        f"The provided sink type {type(sink)} is not supported " f"or caused an error."
    )
    return False
