# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\util\logging\general_logger.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
def _get_source_filter(source: str) -> callable:
    ...
    def source_filter(record) -> bool:
        return (
            record["extra"].get("log_type") == LogTypeEnum.default
            and record["extra"].get("source") == source
        )
        ...
        for key, value in kwargs.items():
            if hasattr(self, key):
                ...
                raise AttributeError(
                    f"{self.__class__.__name__} " f"has no attribute '{key}'"
                )
