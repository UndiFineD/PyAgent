# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\util\monitor\monitor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class Monitor(BaseModel):
    ...
    def trace_llm_token_usage(
        self, llm_obj: object, llm_input: dict, output: LLMOutput
    ) -> None:
        ...
        if trace_id:
            ...
            old_token_usage: dict = FrameworkContextManager().get_context(
                trace_id + "_token_usage"
            )
    ...
    def trace_agent_invocation(
        ...
        if self.activate:
            ...
            path_save = os.path.join(
                str(self._get_or_create_subdir(AGENT_INVOCATION_SUBDIR)), filename
            )
    ...
    def init_invocation_chain():
        ...
        trace_id = AuTraceManager().get_trace_id()
        if (
            FrameworkContextManager().get_context(trace_id + "_invocation_chain")
            is None
        ):
    ...
    def add_invocation_chain(source: dict):
        ...
        if trace_id is not None:
            ...
            invocation_chain_bak = FrameworkContextManager().get_context(
                trace_id + "_invocation_chain_bak"
            )
        ...
        if trace_id is not None:
            ...
            if old_token_usage is not None:
                ...
                for key, value in cur_token_usage.items():
                    try:
                        result_usage[key] = (
                            old_token_usage[key] + value
                            if key in old_token_usage
                            else value
                        )
                    except:
                        # not addable value
                        pass
                FrameworkContextManager().set_context(
                    trace_id + "_token_usage", result_usage
                )
    ...
    def get_token_usage():
        ...
        trace_id = AuTraceManager().get_trace_id()
        return (
            FrameworkContextManager().get_context(trace_id + "_token_usage", {})
            if trace_id is not None
            else {}
        )
    ...
    def get_invocation_chain_str() -> str:
        ...
        if len(invocation_chain) > 0:
            invocation_chain_str = " -> ".join(
                [
                    f"source: {d.get('source', '')}, type: {d.get('type', '')}"
                    for d in invocation_chain
                ]
            )
            invocation_chain_str += " | "
    ...
    def get_llm_token_usage(
        llm_obj: object, llm_input: dict, output: LLMOutput
    ) -> dict:
        ...
        try:
            if (
                output.usage
                and output.usage.completion_tokens > 0
                and output.usage.prompt_tokens > 0
            ):
