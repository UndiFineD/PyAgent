# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\tracing\otel\instrumentation\agent\agent_instrumentor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.base.annotation.trace import InvocationChainContext, _get_agent_info
...
def safe_json_dumps(obj, *, indent=None, ensure_ascii=False) -> str:
    ...
    try:
        return json.dumps(
            obj, default=_fallback, indent=indent, ensure_ascii=ensure_ascii
        )
    except Exception:
        return str(obj)
...
class AgentMetricsRecorder:
    ...
    def record_error(
        self, error: Exception, duration: float, labels: Dict[str, str]
    ) -> None:
...
class AgentSpanAttributesSetter:
    ...
    def set_input_attributes(
        ...
        span.set_attribute(
            SpanAttributes.AGENT_INPUT,
            safe_json_dumps(input_params, ensure_ascii=False),
        )
        span.set_attribute(
            SpanAttributes.TRACE_CALLER_NAME, caller_info.get("source", "unknown")
        )
        span.set_attribute(
            SpanAttributes.TRACE_CALLER_TYPE, caller_info.get("type", "user")
        )
        span.set_attribute(SpanAttributes.AGENT_PAIR_ID, pair_id)

    @staticmethod
    def set_success_attributes(
        span: Span, duration: float, result: OutputObject
    ) -> None:
        ...
        span.set_attribute(
            SpanAttributes.AGENT_OUTPUT,
            safe_json_dumps(result.to_dict(), ensure_ascii=False),
        )
        span.set_attribute(
            SpanAttributes.AGENT_USAGE_TOTAL_TOKENS,
            get_current_token_usage().total_tokens,
        )
        span.set_attribute(
            SpanAttributes.AGENT_USAGE_COMPLETION_TOKENS,
            get_current_token_usage().completion_tokens,
        )
        span.set_attribute(
            SpanAttributes.AGENT_USAGE_PROMPT_TOKENS,
            get_current_token_usage().prompt_tokens,
        )
        span.set_attribute(
            SpanAttributes.AGENT_USAGE_DETAIL_TOKENS,
...
class AgentInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs):
        ...
        self._tracer = trace.get_tracer(
            INSTRUMENTOR_NAME, INSTRUMENTOR_VERSION, tracer_provider
        )
        self._meter = metrics.get_meter(
            INSTRUMENTOR_NAME, INSTRUMENTOR_VERSION, meter_provider
        )
    ...
    def _uninstrument(self, **kwargs):
        ...
        try:
            ...
            setattr(
                trace_module, "_agent_wrapper_async", self._original_agent_wrapper_async
            )
            setattr(
                trace_module, "_agent_wrapper_sync", self._original_agent_wrapper_sync
            )
        except ImportError:
            pass
    ...
    def _patch_trace_agent(self):
        ...
        try:
            ...
            self._original_agent_wrapper_async = getattr(
                trace_module, "_agent_wrapper_async", None
            )
            self._original_agent_wrapper_sync = getattr(
                trace_module, "_agent_wrapper_sync", None
            )
            setattr(
                trace_module, "_agent_wrapper_async", self._wrap_async_agent_function
            )
            setattr(trace_module, "_agent_wrapper_sync", self._wrap_sync_agent_function)
        except ImportError:
            pass

    def _wrap_output_stream(
        self, output_stream: Any, start_time: float, span: Span, labels: Dict[str, str]
    ) -> Any:
