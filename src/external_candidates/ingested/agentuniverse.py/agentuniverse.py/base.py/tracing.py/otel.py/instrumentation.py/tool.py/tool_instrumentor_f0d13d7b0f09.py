# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\base\tracing\otel\instrumentation\tool\tool_instrumentor.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
from agentuniverse.base.annotation.trace import InvocationChainContext, _get_tool_info
...
class ToolMetricsRecorder:
    ...
    def record_total_token_usage(self, labels: Dict[str, str]) -> None:
        ...
        self.metrics[MetricNames.TOOL_TOTAL_TOKENS].record(
            get_current_token_usage().total_tokens, labels
        )
        self.metrics[MetricNames.TOOL_PROMPT_TOKENS].record(
...
class ToolSpanAttributesSetter:
    ...
    def set_input_attributes(
        ...
        span.set_attribute(
            SpanAttributes.TOOL_INPUT, safe_json_dumps(input_params, ensure_ascii=False)
        )
        span.set_attribute(
    ...
    def set_success_attributes(span: Span, duration: float, result: Any) -> None:
        ...
        span.set_attribute(
            SpanAttributes.TOOL_OUTPUT, safe_json_dumps(result, ensure_ascii=False)
        )
        span.set_attribute(
            SpanAttributes.TOOL_USAGE_TOTAL_TOKENS,
            get_current_token_usage().total_tokens,
        )
        span.set_attribute(
            SpanAttributes.TOOL_USAGE_COMPLETION_TOKENS,
            get_current_token_usage().completion_tokens,
        )
        span.set_attribute(
            SpanAttributes.TOOL_USAGE_PROMPT_TOKENS,
            get_current_token_usage().prompt_tokens,
        )
        span.set_attribute(
            SpanAttributes.TOOL_USAGE_DETAIL_TOKENS,
...
class ToolInstrumentor(BaseInstrumentor):
    ...
    def _uninstrument(self, **kwargs):
        ...
        try:
            ...
            setattr(
                trace_module, "_tool_wrapper_async", self._original_tool_wrapper_async
            )
            setattr(
                trace_module, "_tool_wrapper_sync", self._original_tool_wrapper_sync
            )
        except ImportError:
            pass
    ...
    def _patch_trace_tool(self):
        ...
        try:
            ...
            self._original_tool_wrapper_async = getattr(
                trace_module, "_tool_wrapper_async", None
            )
            self._original_tool_wrapper_sync = getattr(
                trace_module, "_tool_wrapper_sync", None
            )
