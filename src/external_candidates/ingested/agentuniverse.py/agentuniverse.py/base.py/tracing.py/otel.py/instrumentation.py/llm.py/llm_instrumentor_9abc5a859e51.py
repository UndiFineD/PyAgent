...
class LLMMetricsRecorder:
    ...
    def record_first_token(
        self, duration: float, labels: Dict[str, str], streaming: bool = True
    ) -> None:
...
class LLMSpanAttributesSetter:
    ...
    def set_input_attributes(
        ...
        span.set_attribute(
            SpanAttributes.AU_TRACE_CALLER_NAME, caller_info.get("source", "unknown")
        )
        span.set_attribute(
            SpanAttributes.AU_TRACE_CALLER_TYPE, caller_info.get("type", "user")
        )
    ...
    def set_success_attributes(span: Span, duration: float, result: LLMOutput) -> None:
        ...
        if result.usage:
            span.set_attribute(
                SpanAttributes.AU_LLM_USAGE_PROMPT_TOKENS, result.usage.prompt_tokens
            )
            span.set_attribute(
            ...
            span.set_attribute(
                SpanAttributes.AU_LLM_USAGE_TOTAL_TOKENS, result.usage.total_tokens
            )
            span.set_attribute(
...
class StreamingResultProcessor:
    ...
    def _finalize_streaming_result(
        self, llm_output: list, usage: Optional[TokenUsage]
    ) -> None:
        ...
        Monitor().trace_llm_token_usage(
            self.llm_instance, self.llm_input, pseudo_result
        )
        ...
        LLMSpanAttributesSetter.set_success_attributes(
            self.span, duration, pseudo_result
        )
...
class LLMInstrumentor(BaseInstrumentor):
    ...
    def _uninstrument(self, **kwargs):
        ...
        try:
            ...
            setattr(
                trace_module, "_llm_wrapper_async", self._original_llm_wrapper_async
            )
    ...
    def _patch_trace_llm(self):
        ...
        try:
            ...
            self._original_llm_wrapper_async = getattr(
                trace_module, "_llm_wrapper_async", None
            )
            self._original_llm_wrapper_sync = getattr(
                trace_module, "_llm_wrapper_sync", None
            )
    ...
    def _prepare_llm_context(self, func, *args, **kwargs) -> tuple:
        ...
        llm_instance, source, channel_name, llm_input, params, caller_info = (
        ...
        try:
            ...
            if isinstance(result, LLMOutput):
                ...
                self._handle_non_streaming_result(
                    result, llm_instance, source, llm_input, duration, span, labels
                )
