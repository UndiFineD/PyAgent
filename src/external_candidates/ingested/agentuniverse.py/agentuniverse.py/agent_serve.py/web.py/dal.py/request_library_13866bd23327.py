...
class RequestLibrary:
    def __init__(self, configer: Configer = None):
        ...
        if system_db_uri and system_db_uri.strip():
            ...
            db_path = (
                get_project_root_path() / "intelligence" / "db" / "agent_universe.db"
            )
        ...
        request_table_name = configer.get("DB", {}).get(
        ...
        self.sqldb_wrapper = SQLDBWrapper(
            name="__system_db__", db_wrapper_configer=_configer
        )
        SQLDBWrapperManager().register(
            self.sqldb_wrapper.get_instance_code(), self.sqldb_wrapper
        )
    ...
    def query_request_by_request_id(self, request_id: str) -> RequestDO | None:
        ...
        try:
            result = (
                session.execute(
                    select(self.request_orm).where(
                        self.request_orm.request_id == request_id
                    )
                )
                .scalars()
                .first()
    ...
    def update_request(self, request_do: RequestDO):
        ...
        try:
            db_request_do = (
                session.query(self.request_orm)
                .filter(self.request_orm.request_id == request_do.request_id)
                .first()
            )
            if db_request_do:
    ...
    def update_gmt_modified(self, request_id: str):
        ...
        try:
            db_request_do = (
                session.query(self.request_orm)
                .filter(self.request_orm.request_id == request_id)
                .first()
            )
            if db_request_do:
