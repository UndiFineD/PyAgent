# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent_serve\web\flask_server.py
import time
import traceback
...
def service_run(service_id: str, params: dict, saved: bool = False):
    ...
    try:
        ...
        return make_standard_response(
            success=False, message="AU sync service timeout", status_code=504
        )

    return make_standard_response(
        success=True, result=result, request_id=request_task.request_id
    )
...
def service_run_stream(service_id: str, params: dict, saved: bool = False):
    ...
    context_prefix = get_context_prefix()
    response = Response(
        timed_generator(task.stream_run(), g.start_time, context_prefix),
...
def service_run_result(request_id: str):
    ...
    if data is None:
        return make_standard_response(
            success=False, message=f"request {request_id} not found"
        )
...
def openai_protocol_chat(model: str, messages: list):
    ...
    if stream:
        ...
        context_prefix = get_context_prefix()
        response = Response(
            timed_generator(task.user_stream_run(), g.start_time, context_prefix),
            mimetype="text/event-stream",
        )
        response.headers["X-Request-ID"] = task.request_id
        return response
    try:
        ...
        request_task = RequestTask(
            ServiceInstance(params.get("service_id")).run, False, **params
        )
        with ThreadPoolExecutorWithReturnValue() as executor:
