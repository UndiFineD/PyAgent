...
class RequestTask:
    ...
    def update_request_do(self, force: bool = False):
        ...
        if_update = (
            current_time - self.last_update_time >= RequestLibrary().update_interval
        )
    ...
    def receive_steps(self):
        ...
        try:
            ...
            LOGGER.error(
                "Request task execute failed: " + str(e) + traceback.format_exc()
            )
            if self.saved:
        ...
        while True:
            try:
                output: str = await asyncio.wait_for(
                    self.async_queue.get(), timeout=0.5
                )
    ...
    def async_run(self):
        ...
        self.thread = ThreadWithReturnValue(
            target=copy_current_request_context(self.func), kwargs=self.kwargs
        )
        self.thread.start()
