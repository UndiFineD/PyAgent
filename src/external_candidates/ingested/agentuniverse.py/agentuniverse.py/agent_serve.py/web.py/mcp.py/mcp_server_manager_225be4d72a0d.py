# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent_serve\web\mcp\mcp_server_manager.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
import functools
import inspect
...
def create_exposed_tool_method_wrapper(tool_name):
    ...
    original_method = getattr(
        ToolManager().get_instance_obj(tool_name).__class__, method_name
    )
...
class MCPServerManager:
    ...
    def register_mcp_tool(
        self, configer_instance: ComponentConfiger, configer_type: str
    ):
        if isinstance(configer_instance.as_mcp_tool, dict):
            ...
            isinstance(configer_instance.as_mcp_tool, bool)
            and configer_instance.as_mcp_tool
        ):
    ...
    def start_server(
        ...
        for server_name, tool_dict in self.server_tool_map.items():
            ...
            if transport == "sse":
                mcp_server_dict["mount"] = Mount(
                    f"/{server_name}", app=mcp_server.sse_app()
                )
        ...
        if transport == "sse":
            app = Starlette(
                routes=[_mcp_server["mount"] for _mcp_server in mcp_server_list]
            )
                ...
                async with contextlib.AsyncExitStack() as stack:
                    for _mcp_server in mcp_server_list:
                        await stack.enter_async_context(
                            _mcp_server["server"].session_manager.run()
                        )
                    yield
            ...
            for _mcp_server in mcp_server_list:
                app.mount(
                    f"/{_mcp_server['server_name']}",
                    _mcp_server["server"].streamable_http_app(),
                )
        else:
