# Extracted from: C:\DEV\PyAgent\.external\agentUniverse\agentuniverse\agent_serve\web\rpc\grpc\grpc_server_booster.py
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
...
class AgentUniverseService(agentuniverse_service_pb2_grpc.AgentUniverseService):
    ...
    def service_run(self, request, context):
        ...
        service_result = service_run(
            saved=request.saved, params=request.params, service_id=request.service_id
        )
...
def set_grpc_config(configer):
    GRPC_CONFIG["server_port"] = configer.value.get("GRPC", {}).get(
        "server_port", 50051
    )
