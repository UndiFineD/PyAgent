# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class FractalCascade(Strategy):
    ...
    def next(self):
        if (
            np.isnan(self.adx[-1]) or self.adx[-1] < 30
        ):  # 🌙 Moon Dev: Tightened ADX to 30 for stronger trend filter
            return
        ...
        if not self.position:
            ...
            if (
                ...
                * self.volume_ma[
                    -1
                ]  # 🌙 Moon Dev: Stricter volume filter (1.2x) to avoid low-quality signals
                and self.rsi[-1]
                > 50  # 🌙 Moon Dev: RSI >50 for bullish momentum confirmation
            ...
            # Short entry
                ...
                * self.volume_ma[
                ...
                and self.rsi[-1]
                < 50  # 🌙 Moon Dev: RSI <50 for bearish momentum confirmation
