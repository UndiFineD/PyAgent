# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v8.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.data.Volume[-1] > 1.5 * self.volume_ma[-1]
                and self.rsi[-1] > 50
                ...
                if not np.isnan(sl) and sl < entry_price:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev: Long entry at {entry_price:.2f}, Size: {size}, SL: {sl:.2f}, TP: {self.current_tp:.2f} 🚀"
                        )
            ...
            # Short entry
                ...
                if not np.isnan(sl) and sl > entry_price:
                    ...
                    if size > 0:
                        ...
                        self.current_tp = entry_price - 2 * risk_dist
                        print(
                            f"🌙 Moon Dev: Short entry at {entry_price:.2f}, Size: {size}, SL: {sl:.2f}, TP: {self.current_tp:.2f} 📉"
                        )
        ...
        if self.position:
            if self.position.is_long:
                ...
                if (
                    not np.isnan(self.current_tp)
                    and self.data.Close[-1] >= self.current_tp
                ):
                    self.position.close()
                ...
                if (
                    potential_sl > self.current_sl
                    and potential_sl < self.data.Close[-1]
                ):
                    ...
                    self.current_tp = self.data.Close[-1] + 2 * (
                        self.data.Close[-1] - potential_sl
                    )
                    print(
                        f"🌙 Moon Dev: ATR Trailing SL long to {potential_sl:.2f}, TP: {self.current_tp:.2f} ✨"
                    )
                ...
                if not np.isnan(self.down_fractal[-1]):
                    ...
                    if new_sl > current_sl:
                        ...
                        self.current_tp = self.data.Close[-1] + 2 * (
                            self.data.Close[-1] - new_sl
                        )
                        print(
                            f"🌙 Moon Dev: Fractal Trailing SL long to {new_sl:.2f}, TP: {self.current_tp:.2f} ✨"
                        )
                ...
                if (
                    ...
                    print(
                        f"🌙 Moon Dev: ATR Trailing SL short to {potential_sl:.2f}, TP: {self.current_tp:.2f} ✨"
                    )
                ...
                if not np.isnan(self.up_fractal[-1]):
                    ...
                    if new_sl < current_sl:
                        ...
                        self.current_sl = new_sl
                        self.current_tp = self.data.Close[-1] - 2 * (
                            new_sl - self.data.Close[-1]
                        )
                        print(
                            f"🌙 Moon Dev: Fractal Trailing SL short to {new_sl:.2f}, TP: {self.current_tp:.2f} ✨"
                        )
