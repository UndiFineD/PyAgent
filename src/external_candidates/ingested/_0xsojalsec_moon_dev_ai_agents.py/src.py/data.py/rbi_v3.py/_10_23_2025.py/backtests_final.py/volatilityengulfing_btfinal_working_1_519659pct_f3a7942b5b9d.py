# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_final\VolatilityEngulfing_BTFinal_WORKING_-1.519659pct.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityEngulfing(Strategy):
    ...
    def init(self):
        ...
        # Debug print
        print(
            "🌙 Moon Dev: VolatilityEngulfing Strategy Initialized! 🚀 Bollinger Bands, Volume SMA, SMA50, and ATR ready. ✨"
        )
    ...
    def next(self):
        ...
        bearish_curr_exit = (
            curr_c < curr_o
        )  # Hypothetical for current, but use prev for signal
        ...
        if breakout and vol_confirm and pattern_confirm and trend_filter:
            print(
                f"🌙 Moon Dev: Volatility Breakout Detected! Close: {curr_c:.2f} > BB Upper: {self.bb_upper[-1]:.2f} | Vol: {curr_v:.2f} > {self.vol_multiplier}*{self.vol_sma[-1]:.2f} | Engulfing: True | Above SMA50: {self.sma50[-1]:.2f} 🚀"
            )
        ...
        if (
            ...
            if risk_dist > 0:
                ...
                print(
                    f"🚀 Moon Dev: Long Entry! Size: {size}, Entry≈{entry_price:.2f}, SL: {sl_price:.2f}, TP: {tp_price:.2f} | Risk: {risk_amount:.2f} (1%) ✨"
                )
        ...
        if self.position:
            ...
            if curr_c < self.bb_middle[-1]:
                ...
                self.current_entry = None
                print(
                    f"🌙 Moon Dev: Trailing Exit - Close below Middle BB: {curr_c:.2f} < {self.bb_middle[-1]:.2f} 💫"
                )
            ...
            if is_bearish_engulfing:
                ...
                self.current_entry = None
                print(
                    f"🌙 Moon Dev: Bearish Engulfing Exit! 🚨 Pattern confirmed, closing position."
                )

        # Debug position info
        if self.position and self.current_entry is not None:
            print(
                f"🌙 Moon Dev Position Update: PL: {self.position.pl:.2f}, Size: {self.position.size}, Entry: {self.current_entry:.2f} 📊"
            )


# Run backtest
bt = Backtest(
    data, VolatilityEngulfing, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
