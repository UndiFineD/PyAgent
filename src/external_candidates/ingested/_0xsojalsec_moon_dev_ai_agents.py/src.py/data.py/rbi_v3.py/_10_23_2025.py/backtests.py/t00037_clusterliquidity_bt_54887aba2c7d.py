...
class ClusterLiquidity(Strategy):
    ...
    def init(self):
        ...
        bb_upper, bb_mid, bb_lower = self.I(
            talib.BBANDS, cl, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.entry_bar is not None and (
                current_bar - self.entry_bar > self.max_bars
            ):
                self.position.close()
            ...
            if self.position.is_long and self.data.Close[-1] < ch:
                self.position.close()
                print(
                    f"🌙❌ Invalidate long: price re-entered cluster at {self.data.Close[-1]}"
                )
                self.entry_bar = None
                return
            if not self.position.is_long and self.data.Close[-1] > clu:
                self.position.close()
                print(
                    f"🌙❌ Invalidate short: price re-entered cluster at {self.data.Close[-1]}"
                )
                self.entry_bar = None
                return
            # Partial exit at 1:1
            ...
            if self.position.is_long:
                ...
                if unreal >= risk:
                    ...
                    if half >= 1:
                        ...
                        print(
                            f"🌙✨ Partial close long at 1:1 RR, remaining size {abs(self.position.size)}"
                        )
            else:
                risk = sl_p - entry_p
                ...
                if unreal >= risk:
                    ...
                    if half >= 1:
                        self.position.close(size=half)
                        print(
                            f"🌙✨ Partial close short at 1:1 RR, remaining size {abs(self.position.size)}"
                        )
            # Trailing stop
            if self.position.is_long:
                unreal = self.data.Close[-1] - entry_p
...
bt = Backtest(
    data, ClusterLiquidity, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
