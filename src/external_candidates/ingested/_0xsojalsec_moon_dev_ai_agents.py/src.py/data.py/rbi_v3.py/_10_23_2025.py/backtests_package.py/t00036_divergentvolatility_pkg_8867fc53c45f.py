# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_package\T00036_DivergentVolatility_PKG.py
import numpy as np
import pandas as pd
import talib
...
class DivergentVolatility(Strategy):
    ...
    def next(self):
        ...
        if self.death_cross_bar == -1 and i >= 1:
            ...
            if (
                not np.isnan(prev_sma50)
                and not np.isnan(prev_sma200)
                and not np.isnan(curr_sma50)
                and not np.isnan(curr_sma200)
                and prev_sma50 > prev_sma200
                and curr_sma50 <= curr_sma200
                and close < curr_sma200
                and volume < 0.8 * self.vol_avg[-1]
            ):
                ...
                print(
                    f"🌙 Death Cross detected at bar {i} on low volume! Waiting for divergence... ✨"
                )
        ...
        if self.state == "post_cross" and self.death_cross_bar != -1:
            ...
            if low < self.lowest_since_cross:
                if self.RSI[-1] > self.rsi_at_lowest:
                    ...
                    self.div_bar = i
                    print(
                        f"🚀 Bullish Divergence confirmed at bar {i}! Price: {low}, RSI: {self.RSI[-1]:.2f} > prev {self.rsi_at_lowest:.2f} 🌙"
                    )
                self.lowest_since_cross = low
                self.rsi_at_lowest = self.RSI[-1]
            ...
            if self.divergence_confirmed and 5 <= bars_since_cross <= 10:
                ...
                print(
                    f"✨ Divergence confirmed, now waiting for volatility breakout! 🌙"
                )
        ...
        # Waiting for breakout after divergence
            ...
            consol_high = (
                max(self.data.High[-10:])
                if i >= 9
                else max(self.data.High[self.death_cross_bar : i + 1])
            )
            # BB squeeze breakout
            ...
            if (
                (breakout1 or breakout2)
                and close < self.SMA200[-1]
                ...
                if risk_per_unit > 0:
                    ...
                    )  # Fixed: Use self._broker.getcash() for equity approximation
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Long Entry at {close:.2f}, size {size}, stop {self.stop_price:.2f}, risk 1% 🚀"
                        )
        ...
        if self.position and self.position.is_long:
            ...
            if close < self.stop_price:
                self.position.close()
                print(
                    f"💥 Stop loss hit at {close:.2f} (stop: {self.stop_price:.2f}) 🌙"
                )
                self.reset_state()
                return
            ...
            if self.entry_price > 0 and self.stop_price > 0:
                ...
                if unrealized_profit > 2 * unrealized_risk:
                    ...
                    if new_trail > self.stop_price:
                        ...
                        print(
                            f"🔄 Trailing stop updated to middle BB: {self.stop_price:.2f} ✨"
                        )
...
# Run backtest
bt = Backtest(
    data, DivergentVolatility, cash=1000000, commission=0.001, exclusive_orders=True
)
stats = bt.run()
print(stats)
