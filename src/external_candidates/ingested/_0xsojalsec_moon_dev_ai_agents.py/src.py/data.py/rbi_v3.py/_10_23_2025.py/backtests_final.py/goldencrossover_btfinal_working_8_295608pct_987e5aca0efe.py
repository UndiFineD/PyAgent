# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_final\GoldenCrossover_BTFinal_WORKING_-8.295608pct.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, SMA20 {sma20:.2f}, SMA200 {sma200:.2f}, RSI {rsi:.2f}, ATR {atr:.2f} ✨"
        )
        ...
        if crossover and touch_fib and volume_confirm and uptrend and not self.position:
            ...
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, Fib Touch Confirmed! ✨🚀"
                )
        ...
        if self.position:
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Bearish RSI Divergence detected in overbought zone, EXITING! 🚀"
                )
                self.position.close()
                return
