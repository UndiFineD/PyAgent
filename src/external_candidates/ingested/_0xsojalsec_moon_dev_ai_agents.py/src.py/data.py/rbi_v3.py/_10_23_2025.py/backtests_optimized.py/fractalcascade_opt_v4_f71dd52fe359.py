# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v4.py
import numpy as np
import pandas as pd
import talib
...
class FractalCascade(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Added RSI for entry momentum filter
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                > self.lips[
                    -2
                ]  # 🌙 Moon Dev: Added lips momentum for better trend confirmation
                and self.ao[-1] > 0
                and self.ao[-1] > self.ao[-2]
                and self.data.Volume[-1] > self.volume_ma[-1] * vol_mult
                and not np.isnan(self.rsi[-1])
                and self.rsi[-1] > 50  # 🌙 Moon Dev: RSI filter to avoid weak momentum
                ...
                if not np.isnan(sl) and sl < entry_price:
                    ...
                    tp = (
                        entry_price + reward_risk * risk_dist
                    )  # 🌙 Moon Dev: Calculate fixed TP based on RR
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev: Long entry at {entry_price:.2f}, Size: {size}, SL: {sl:.2f}, TP: {tp:.2f} 🚀"
                        )
            ...
            # Short entry - 🌙 Moon Dev: Symmetric tightening with RSI <50, lips momentum, volume *1.2
                ...
                if not np.isnan(sl) and sl > entry_price:
                    ...
                    if size > 0:
                        ...
                        self.tp = tp  # 🌙 Moon Dev: Track TP for trailing
                        print(
                            f"🌙 Moon Dev: Short entry at {entry_price:.2f}, Size: {size}, SL: {sl:.2f}, TP: {tp:.2f} 📉"
                        )
        ...
        if self.position:
            if self.position.is_long:
                ...
                if (
                    not np.isnan(potential_sl)
                    and potential_sl > self.current_sl
