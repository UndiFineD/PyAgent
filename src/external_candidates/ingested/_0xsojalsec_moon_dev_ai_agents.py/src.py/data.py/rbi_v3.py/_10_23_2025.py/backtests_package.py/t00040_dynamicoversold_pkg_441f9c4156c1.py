# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_package\T00040_DynamicOversold_PKG.py
import numpy as np
import pandas as pd
import talib
...
class DynamicOversold(Strategy):
    ...
    def next(self):
        ...
        current_ratio = (
            ...
            / (self.bb_upper[-1] - self.bb_lower[-1])
        ) * 100
        ...
        if self.bb_width[-1] > 2 * self.avg_width[-1]:
            vol_adjust = 0.5
            print(
                f"🌙 Moon Dev: High volatility detected, adjusting size by {vol_adjust} 🛡️"
            )
        ...
        if self.position:
            if self.sma5[-1] < self.ema50[-1]:
                self.sell()
                print(
                    f"🌙 Moon Dev: Trend invalidation exit at {self.data.Close[-1]} ❌"
                )
                return
            ...
            if bars_in_trade > self.time_exit_bars:
                self.sell()
                print(
                    f"🌙 Moon Dev: Time-based exit after {bars_in_trade} bars at {self.data.Close[-1]} ⏰"
                )
                return

            # Profit target: partial exit 50%
            if (
                self.rsi[-1] >= 70 or self.data.Close[-1] >= self.bb_upper[-1]
            ) and self.position.size > 0:
                partial_size = self.position.size / 2
                self.sell(size=partial_size)
                print(
                    f"🌙 Moon Dev: Partial profit exit (50%) at {self.data.Close[-1]}, ratio {current_ratio:.1f}% 💰"
                )
                return
            ...
            if current_ratio > 80:
                ...
                print(
                    f"🌙 Moon Dev: Tightened stop to middle BB {new_sl} at ratio {current_ratio:.1f}% 🔒"
                )

        # Entry logic
        if (
            self.rsi[-1] < 30
            and self.sma5[-1] > self.ema50[-1]
            and current_ratio <= 40
            and len(self.trades) < self.max_trades
        ):
            ...
            if size > 0:
                ...
                print(
                    f"🌙 Moon Dev: Long entry at {entry_price}, size {size}, stop {stop_price}, ratio {current_ratio:.1f}%, risk {risk_pct*100}% 🚀"
                )


bt = Backtest(
    data, DynamicOversold, cash=1000000, commission=0.001, exclusive_orders=False
)
stats = bt.run()
print(stats)
