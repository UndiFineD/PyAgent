# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v7.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, EMA12 {ema20:.2f}, EMA26 {ema50:.2f}, EMA200 {ema200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f}, MACD {macd:.2f} ✨"
        )
        ...
        ):  # 🌙 Moon Dev Optimization: Reduced min lookback to 20 bars to enable earlier swing detection in shorter trends
            return
        ...
        )  # 🌙 Moon Dev Optimization: Changed to actual EMA12/EMA26 crossover for reliable golden cross signals, reducing false entries from price-only crosses
        # Check Fib touch in recent bars
        recent_lows = self.data.Low[
            -10:
        ]  # 🌙 Moon Dev Optimization: Extended to 10 bars for capturing deeper pullbacks
        touch_fib = any(
            recent_low <= fib618 + (0.005 * close) for recent_low in recent_lows
        )  # 🌙 Moon Dev Optimization: Increased tolerance to 0.5% to account for BTC volatility and noise
        volume_confirm = (
            volume > 1.0 * avg_vol
        )  # 🌙 Moon Dev Optimization: Relaxed to 1.0x average for higher trade frequency, as volume spikes are less reliable in crypto
        momentum_rsi = (
            rsi > 40
        )  # 🌙 Moon Dev Optimization: Lowered lower bound to 40 to enter on milder oversold bounces, expanding opportunities
        trend_strength = (
            adx > 15
        )  # 🌙 Moon Dev Optimization: Reduced threshold to 15 for more entries in emerging trends
        macd_confirm = (
            self.macd_hist[-1] > 0
        )  # 🌙 Moon Dev Optimization: Switched to MACD histogram >0 for positive momentum acceleration confirmation
        uptrend = (
            close > ema50 and ema20 > ema50
        )  # 🌙 Moon Dev Optimization: Loosened uptrend filter to above EMA26 instead of EMA200, allowing trades in intermediate uptrends for more frequency while maintaining directionality
        ...
        if (
            ...
            swing_low_recent = min(
                self.data.Low[-20:]
            )  # 🌙 Moon Dev Optimization: Extended lookback to 20 bars for stronger support identification
            ...
            )  # 🌙 Moon Dev Optimization: Adjusted cap to 15% for aggressive growth targeting 50% return, with min 0.01 BTC
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, True Golden Cross + Loosened Filters! ✨🚀"
                )
        ...
        if self.position:
            ...
            if unrealized_pnl >= 0.75 * risk:
                ...
                if trail_sl > current_sl:
                    ...
                    print(
                        f"🌙 Moon Dev: Trailing SL updated to {trail_sl:.2f} after 0.75:1 RR ✨"
                    )
            ...
            if not self.partial_closed_2 and unrealized_pnl >= 3 * risk:
                partial_size_2 = (
                    self.position.size * 0.3
                )  # Note: size adjusted after first partial
                self.sell(size=partial_size_2)
                self.partial_closed_2 = True
                print(
                    f"🌙 Moon Dev: Partial close 30% at 3:1 RR, more profits locked! 🚀"
                )

            if unrealized_pnl >= 4 * risk:
                print(
                    f"🌙 Moon Dev: Full TP at 4:1 RR for optimized gains in BTC trends! 🚀"
                )
                self.position.close()
                return
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Bearish RSI + MACD Hist Divergence in overbought, EXITING! 🚀"
                )
                self.position.close()
                return
