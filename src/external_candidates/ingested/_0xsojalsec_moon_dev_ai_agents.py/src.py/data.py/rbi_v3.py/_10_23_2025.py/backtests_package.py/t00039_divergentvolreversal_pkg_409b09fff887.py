# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_package\T00039_DivergentVolReversal_PKG.py
import numpy as np
import pandas as pd
import talib
...
class DivergentVolReversal(Strategy):
    ...
    rsi_os = 30
    vix_proxy_threshold = (
        300  # Arbitrary threshold for BTC ATR as VIX proxy to avoid entry
    )
    ...
    def next(self):
        ...
        if self.position:
            if self.atr[-1] > self.vol_mult * self.atr_sma[-1]:
                self.position.close()
                print(
                    f"🌙 Moon Dev: Volatility spike exit at {self.data.Close[-1]}! ⚠️"
                )
                return
            bars_held = (
            ...
            if bars_held > self.max_bars:
                self.position.close()
                print(
                    f"🌙 Moon Dev: Time-based exit at {self.data.Close[-1]} after {bars_held} bars! ⏰"
                )
                return
            return
                ...
                if (
                    ...
                    if risk_per_unit > 0:
                        ...
                        if pos_size > 0:
                            ...
                            print(
                                f"🌙 Moon Dev: Bullish divergence LONG entry at {entry_price:.2f}, size {pos_size}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀"
                            )
                ...
                if (
                    ...
                    if risk_per_unit > 0:
                        ...
                        if pos_size > 0:
                            ...
                            self.entry_bar = len(self.data)
                            print(
                                f"🌙 Moon Dev: Bearish divergence SHORT entry at {entry_price:.2f}, size {pos_size}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀"
                            )


bt = Backtest(
    data, DivergentVolReversal, cash=1000000, commission=0.001, exclusive_orders=True
)
stats = bt.run()
print(stats)
