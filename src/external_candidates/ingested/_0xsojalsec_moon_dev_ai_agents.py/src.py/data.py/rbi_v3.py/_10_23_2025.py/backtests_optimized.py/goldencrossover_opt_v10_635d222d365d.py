# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v10.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def init(self):
        ...
        self.macd, self.macd_signal, _ = self.I(
            talib.MACD, self.data.Close, fastperiod=8, slowperiod=21, signalperiod=5
        )
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, EMA12 {ema12:.2f}, EMA50 {ema50:.2f}, EMA200 {ema200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f}, MACD {macd:.2f} ✨"
        )
        ...
        ):  # 🌙 Moon Dev Optimization: Kept min lookback at 30 for robust swing detection
            return
        ...
        crossover = (
            len(self.data) > 1 and self.ema12[-2] <= ema50[-2] and ema12 > ema50
        )  # 🌙 Moon Dev Optimization: Switched to actual EMA12/EMA50 crossover for classic golden cross signal, more reliable trend starts
        # Check Fib touch in recent bars
        recent_lows = self.data.Low[
            -10:
        ]  # 🌙 Moon Dev Optimization: Extended to 10 bars to capture more valid pullback entries without missing setups
        touch_fib = any(
            recent_low <= fib618 + (0.005 * close) for recent_low in recent_lows
        )  # 🌙 Moon Dev Optimization: Increased tolerance to 0.5% for BTC volatility, allowing slight overshoots
        volume_confirm = (
            volume > 1.0 * avg_vol
        )  # 🌙 Moon Dev Optimization: Relaxed to 1.0x average to increase trade opportunities while still filtering extremes
        momentum_rsi = (
            rsi > 40 and rsi < 70
        )  # 🌙 Moon Dev Optimization: Widened RSI range to 40-70 for earlier entries and avoiding overbought traps prematurely
        trend_strength = (
            adx > 18
        )  # 🌙 Moon Dev Optimization: Lowered to 18 for more moderate trend entries, boosting overall return potential
        macd_confirm = (
            macd - macd_signal > 0
        )  # 🌙 Moon Dev Optimization: Use MACD histogram >0 for positive momentum divergence, more sensitive than line cross
        uptrend = (
            close > ema200 and ema12 > ema50 > ema200
        )  # 🌙 Moon Dev Optimization: Updated to use EMA12 in stack for consistency with faster signal
        entry_confirm = (
            close > self.data.Open[-1] or ema12 > ema50
        )  # 🌙 Moon Dev Optimization: OR condition - bullish candle OR EMA alignment to reduce false negatives in entries
        ...
        if (
            ...
            )  # 🌙 Moon Dev Optimization: Raised cap to 15% for aggressive BTC plays, but with min 0.01 for liquidity
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, Optimized Golden Cross + Relaxed Filters! ✨🚀"
                )
        ...
        if self.position:
            ...
            if not self.partial_closed and unrealized_pnl >= 1.5 * risk:
                ...
                print(
                    f"🌙 Moon Dev: Partial close 40% at 1.5:1 RR, securing early profits! 🚀"
                )
