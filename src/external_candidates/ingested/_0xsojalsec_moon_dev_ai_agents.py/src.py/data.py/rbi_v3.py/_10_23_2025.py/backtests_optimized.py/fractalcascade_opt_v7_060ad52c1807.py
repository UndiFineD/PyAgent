# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\FractalCascade_OPT_v7.py
import numpy as np
import pandas as pd
import talib
...
class FractalCascade(Strategy):
    ...
    volume_mult = (
        1.5  # 🌙 Moon Dev: Increased volume threshold for higher conviction entries
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.data.Volume[-1] > self.volume_mult * self.volume_ma[-1]
                and self.data.Close[-1] > self.ffill_up[-1]
                and self.data.Close[-2] <= self.ffill_up[-2]
                and self.data.Close[-1] > self.sma200[-1]
            ):  # 🌙 Moon Dev: Trend filter for longs only in uptrend
                ...
                if sl < entry_price:
                    ...
                    if size > 0:
                        ...
                        self.current_sl = sl
                        print(
                            f"🌙 Moon Dev: Long entry at {entry_price:.2f}, Size: {size}, SL: {sl:.2f} 🚀"
                        )
            ...
            # Short entry with added trend filter
                ...
                and self.data.Close[-1] < self.sma200[-1]
            ):  # 🌙 Moon Dev: Trend filter for shorts only in downtrend
                ...
                if sl > entry_price:
                    ...
                    if size > 0:
                        ...
                        self.current_sl = sl
                        print(
                            f"🌙 Moon Dev: Short entry at {entry_price:.2f}, Size: {size}, SL: {sl:.2f} 📉"
                        )
