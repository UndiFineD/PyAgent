# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v4.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, EMA20 {ema20:.2f}, EMA50 {ema50:.2f}, EMA200 {ema200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f}, MACD Hist {macd_hist:.2f} ✨"
        )
        ...
        if (
            len(recent_high_values) < 30
        ):  # 🌙 Moon Dev Optimization: Extended minimum lookback to 30 for more robust swing detection
            return
        ...
        print(
            f"🌙 Moon Dev Fib Calc: 50% Level {fib50:.2f}, High {swing_high:.2f}, Low {swing_low:.2f} 🚀"
        )
        ...
        crossover = (
            len(self.data) > 1
            and self.data.Close[-2] <= self.ema50[-2]
            and close > ema50
        )  # 🌙 Moon Dev: Using EMA50 crossover for entry signal
        touch_fib = low <= fib50 + (
            0.003 * close
        )  # 🌙 Moon Dev Optimization: Tighter 0.3% tolerance for precise Fib interaction
        volume_confirm = (
            volume > 1.2 * avg_vol
        )  # 🌙 Moon Dev Optimization: Reduced to 1.2x to increase trade frequency without sacrificing quality
        momentum_rsi = (
            rsi > 45 and rsi < 65
        )  # 🌙 Moon Dev Optimization: Widened RSI range to capture more pullback entries
        trend_strength = adx > 30  # 🌙 Moon Dev: Higher ADX for stronger trends
        macd_bullish = (
            macd_hist > 0
        )  # 🌙 Moon Dev: Added MACD histogram filter for positive momentum
        uptrend = (
            close > ema200 and ema20 > ema200
        )  # 🌙 Moon Dev Optimization: EMA20 > EMA200 for shorter-term uptrend confirmation
        ...
        if (
            ...
            risk_amount = (
                capital * 0.015
            )  # 🌙 Moon Dev Optimization: Increased risk per trade to 1.5% for higher returns potential
            ...
            position_size = max(
                0.01, position_size
            )  # 🌙 Moon Dev: Minimum fraction size to avoid zero positions
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size:.4f} BTC, Optimized Filters Confirmed! ✨🚀"
                )
        ...
        if self.position:
            ...
            if unrealized_pnl >= 0.75 * risk:
                ...
                if breakeven_sl > current_sl:
                    ...
                    print(
                        f"🌙 Moon Dev: Moved SL to breakeven {breakeven_sl:.2f} after 0.75:1 RR ✨"
                    )

            if unrealized_pnl >= 1.5 * risk:
                trail_sl = (
                    ema20 - 0.5 * atr
                )  # 🌙 Moon Dev Optimization: Tighter trail using EMA20 for faster adjustment
                if trail_sl > current_sl:
                    self.current_sl = trail_sl
                    print(
                        f"🌙 Moon Dev: Trailing SL updated to {trail_sl:.2f} after 1.5:1 RR ✨"
                    )
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Bearish RSI & MACD Divergence in overbought with strong trend, EXITING! 🚀"
                )
                self.position.close()
                return
            ...
            if bars_since_entry > 200:
                print(
                    f"🌙 Moon Dev: Max hold time exit after {bars_since_entry} bars 🚀"
                )
                self.position.close()
                return
