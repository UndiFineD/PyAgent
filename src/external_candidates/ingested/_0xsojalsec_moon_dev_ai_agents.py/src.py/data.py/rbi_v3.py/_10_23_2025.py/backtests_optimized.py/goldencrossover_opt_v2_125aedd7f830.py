# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v2.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        if (
            len(self.data) < 100
        ):  # 🌙 Moon Dev Optimization: Reduced min bars to match shorter SMA100
            return
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, SMA20 {sma20:.2f}, SMA100 {sma100:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f} ✨"
        )
        ...
        if (
            len(recent_high_values) < 10
        ):  # 🌙 Moon Dev Optimization: Shortened min lookback to 10 for quicker Fib calculations
            return
        ...
        print(
            f"🌙 Moon Dev Fib Calc: 61.8% Level {fib618:.2f}, High {swing_high:.2f}, Low {swing_low:.2f} 🚀"
        )
        ...
        volume_confirm = (
            volume > 1.2 * avg_vol
        )  # 🌙 Moon Dev Optimization: Loosened to 1.2x for higher trade frequency while still confirming interest
        momentum_rsi = (
            rsi > 45 and rsi < 80
        )  # 🌙 Moon Dev Optimization: Widened RSI range to capture more pullback opportunities without missing momentum
        ...
        if (
            ...
            swing_low_recent = min(
                self.data.Low[-5:]
            )  # 🌙 Moon Dev Optimization: Tighter recent low (5 bars) for better risk in short-term trades
            ...
            risk_amount = (
                capital * 0.015
            )  # 🌙 Moon Dev Optimization: Increased risk to 1.5% per trade to amplify returns while staying managed
            ...
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, Enhanced Filters Confirmed! ✨🚀"
                )
        ...
        if self.position:
            ...
            if unrealized_pnl >= risk:
                trail_sl = (
                    sma20 - 0.5 * atr
                )  # 🌙 Moon Dev Optimization: Tighter trail using SMA20 and reduced ATR multiplier for better lock-in
            ...
            if rr >= 2 and not self.partial_taken:
                ...
                print(
                    f"🌙 Moon Dev: Partial exit (50%) at 2:1 RR! Remaining size {self.position.size} 🚀"
                )
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Bearish RSI Divergence detected in overbought zone with trend, EXITING! 🚀"
                )
                self.position.close()
                return
