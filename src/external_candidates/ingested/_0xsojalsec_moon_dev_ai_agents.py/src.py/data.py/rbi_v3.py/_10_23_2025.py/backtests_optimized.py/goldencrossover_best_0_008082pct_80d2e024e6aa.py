# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_BEST_-0.008082pct.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        ):  # 🌙 Moon Dev Optimization: Increased min lookback to 30 for robust swing detection
            return
        ...
        crossover = (
            len(self.data) > 1
            and self.data.Close[-2] <= self.ema20[-2]
            and close > ema20
        )  # 🌙 Note: Using EMA20 crossover for quicker entries
        # Check Fib touch in recent bars
        recent_lows = self.data.Low[-5:]
        touch_fib = any(
            recent_low <= fib618 + (0.003 * close) for recent_low in recent_lows
        )  # 🌙 Moon Dev Optimization: Widened tolerance to 0.3% and checked last 5 bars for pullback entries
        volume_confirm = (
            volume > 1.2 * avg_vol
        )  # 🌙 Moon Dev Optimization: Reduced to 1.2x to increase trade frequency without sacrificing too much quality
        momentum_rsi = (
            rsi > 45 and rsi < 65
        )  # 🌙 Moon Dev Optimization: Adjusted RSI range lower for catching pullbacks earlier in uptrends
        trend_strength = adx > 20
        macd_confirm = (
            macd > macd_signal
        )  # 🌙 Moon Dev Optimization: Added MACD > signal line for additional momentum filter
        uptrend = (
            close > ema200 and ema20 > ema50 > ema200
        )  # 🌙 Moon Dev Optimization: Strengthened uptrend with EMA20 > EMA50 > EMA200
        bullish_candle = (
            close > self.data.Open[-1]
        )  # 🌙 Moon Dev Optimization: Require bullish candle for entry confirmation
        ...
        if (
            ...
            swing_low_recent = min(
                self.data.Low[-15:]
            )  # 🌙 Moon Dev Optimization: Extended recent low lookback to 15 bars for better support levels
            ...
            position_size = max(
                0.01, min(int(round(position_size)), int(capital * 0.1 / entry_price))
            )  # 🌙 Moon Dev Optimization: Ensure fractional sizing minimum 0.01 BTC, cap at 10% for safety
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, Refined Filters + MACD Confirmed! ✨🚀"
                )
        ...
        if self.position:
            ...
            if unrealized_pnl >= 0.3 * risk:
                ...
                if breakeven_sl > current_sl:
                    ...
                    print(
                        f"🌙 Moon Dev: Moved SL to breakeven {breakeven_sl:.2f} after 0.3:1 RR ✨"
                    )
