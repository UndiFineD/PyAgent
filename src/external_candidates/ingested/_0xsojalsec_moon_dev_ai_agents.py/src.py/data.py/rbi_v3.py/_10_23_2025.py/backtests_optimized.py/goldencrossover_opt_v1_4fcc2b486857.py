...
class GoldenCrossover(Strategy):
    ...
    def next(self):
        ...
        crossover = (
            len(self.data) > 1
            and self.data.Close[-2] <= self.sma50[-2]
            and close > sma50
        )
        ...
        if self.position:
            ...
            if unrealized_pnl >= risk:
                trail_sl = (
                    sma50 - atr
                )  # 🌙 Moon Dev Optimization: Use SMA50 for trailing in uptrend
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Bearish RSI Divergence detected in overbought zone with strong trend, EXITING! 🚀"
                )
                self.position.close()
                return
