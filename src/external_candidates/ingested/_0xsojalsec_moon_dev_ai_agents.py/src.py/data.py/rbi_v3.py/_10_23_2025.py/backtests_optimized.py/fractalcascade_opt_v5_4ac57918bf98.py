    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                * self.volume_ma[
                    -1
                ]  # 🌙 Moon Dev: Tightened volume filter for better setups
                and self.data.Close[-1] > self.ffill_up[-1]
                and self.data.Close[-2] <= self.ffill_up[-2]
                and self.data.Close[-1]
                > self.sma200[-1]  # 🌙 Moon Dev: Trend filter - only long in uptrend
                and self.rsi[-1] > 50
                ...
                if sl < entry_price:
                    ...
                    if pos_size > 1:
                        pos_size = 1
                    tp = (
                        entry_price + 2 * risk_dist
                    )  # 🌙 Moon Dev: Added 2:1 RR take profit
            ...
            # Short entry - tightened with new filters
                ...
                and self.ao[-1] < self.ao[-2]
                and self.data.Volume[-1]
                > 1.5 * self.volume_ma[-1]  # 🌙 Moon Dev: Tightened volume filter
                and self.data.Close[-1] < self.ffill_down[-1]
                and self.data.Close[-2] >= self.ffill_down[-2]
                and self.data.Close[-1]
                < self.sma200[-1]  # 🌙 Moon Dev: Trend filter - only short in downtrend
                and self.rsi[-1] < 50
            ):  # 🌙 Moon Dev: RSI momentum confirmation
