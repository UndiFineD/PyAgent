# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev Optimization: Increased volume SMA to 20 for smoother average in volatile sessions
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, EMA12 {ema12:.2f}, EMA26 {ema26:.2f}, EMA200 {ema200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f}, MACD {macd:.2f} ✨"
        )
        ...
        ):  # 🌙 Moon Dev Optimization: Reduced min lookback to 20 bars for quicker adaptation
            return
        ...
        if (swing_high - swing_low) < 2 * self.atr[
            -(len(self.data) - from_idx) + rel_high_idx
        ]:
            return
        ...
        crossover = (
            len(self.data) > 1 and ema12[-2] <= ema26[-2] and ema12 > ema26
        )  # 🌙 Moon Dev Optimization: Changed to actual EMA12 crossing above EMA26 for timely golden cross signals
        # Check Fib touch in recent bars
        recent_lows = self.data.Low[
            -8:
        ]  # 🌙 Moon Dev Optimization: Extended to 8 bars for capturing deeper pullbacks
        touch_fib = any(
            recent_low <= fib618 + (0.005 * close) for recent_low in recent_lows
        )  # 🌙 Moon Dev Optimization: Increased tolerance to 0.5% for BTC volatility
        volume_confirm = (
            volume > 1.1 * avg_vol
        )  # 🌙 Moon Dev Optimization: Further relaxed to 1.1x to boost trade frequency in moderate volume uptrends
        momentum_rsi = (
            rsi > 40 and rsi < 70
        )  # 🌙 Moon Dev Optimization: Widened RSI range to 40-70 for more entries on mild pullbacks and avoiding extreme oversold
        trend_strength = (
            adx > 25
        )  # 🌙 Moon Dev Optimization: Increased to 25 for filtering into stronger trends only
        macd_confirm = (
            macd > macd_signal and macd_hist > 0
        )  # 🌙 Moon Dev Optimization: Added MACD histogram >0 for accelerating momentum confirmation
        uptrend = (
            close > ema200 and ema12 > ema26 > ema200
        )  # 🌙 Moon Dev Optimization: Updated to use EMA12/26 in uptrend check for consistency
        ...
        adx_rising = (
            len(self.data) > 3 and adx > self.adx[-3]
        )  # Simple trend strengthening filter
        ...
        if (
            ...
            swing_low_recent = min(
                self.data.Low[-20:]
            )  # 🌙 Moon Dev Optimization: Increased to 20 bars for stronger support identification
            ...
            capital = (
                self.equity
            )  # 🌙 Moon Dev Optimization: Use current equity instead of fixed 1M for compounding
            ...
            position_size = max(
                0.01, min(position_size, capital * 0.15 / entry_price)
            )  # 🌙 Moon Dev Optimization: Cap at 15% equity for aggressive but safe scaling
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, Golden Cross + Enhanced Filters Confirmed! ✨🚀"
                )
        ...
        if self.position:
            ...
            if unrealized_pnl >= risk:
                ...
                trail_sl = (
                    ema26 - 1.0 * atr
                )  # 🌙 Moon Dev Optimization: Switched to EMA26 for smoother trail in medium trends
            ...
            if self.scale_level == 0 and unrealized_pnl >= 2 * risk:
                ...
                self.scale_level = 2
                print(
                    f"🌙 Moon Dev: Scaled out another 25% at 3:1 RR, trailing the rest! 🚀"
                )

            if (
                unrealized_pnl >= 7 * risk
            ):  # 🌙 Moon Dev Optimization: Increased full TP to 7:1 for higher average win size in strong trends
            ...
            if (
                ...
                and macd_hist < 0
                and adx > 25
            ):  # 🌙 Moon Dev Optimization: Use MACD hist <0 and higher ADX for reliable exits
                print(
                    f"🌙 Moon Dev: Bearish RSI + MACD Histogram Divergence in overbought zone, EXITING! 🚀"
                )
                self.position.close()
                return
            ...
            if (
                close < ema26
            ):  # 🌙 Moon Dev Optimization: Changed to below EMA26 to align with entry signals
