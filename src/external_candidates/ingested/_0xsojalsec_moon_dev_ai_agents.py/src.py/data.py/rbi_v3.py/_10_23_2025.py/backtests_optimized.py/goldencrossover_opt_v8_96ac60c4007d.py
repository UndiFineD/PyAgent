# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v8.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def init(self):
        ...
        self.macd, self.macd_signal, _ = self.I(
            talib.MACD, self.data.Close, fastperiod=8, slowperiod=17, signalperiod=9
        )
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Backtest Bar {len(self.data)}: Close {close:.2f}, EMA20 {ema20:.2f}, EMA50 {ema50:.2f}, EMA200 {ema200:.2f}, RSI {rsi:.2f}, ADX {adx:.2f}, ATR {atr:.2f}, MACD {macd:.2f} ✨"
        )
        ...
        if (
            len(recent_high_values) < 20
        ):  # 🌙 Moon Dev Optimization: Reduced min lookback to 20 bars for faster adaptation to recent swings
            return
        ...
        print(
            f"🌙 Moon Dev Fib Calc: {fib_level*100}% Level {fib618:.2f}, High {swing_high:.2f}, Low {swing_low:.2f} 🚀"
        )
        ...
        crossover = (
            len(self.data) > 1 and self.ema20[-2] <= self.ema50[-2] and ema20 > ema50
        )  # 🌙 Moon Dev Optimization: Changed to actual EMA20/EMA50 crossover for classic Golden Cross, reducing false signals from price-only crosses
        # Check Fib touch in recent bars
        recent_lows = self.data.Low[
            -10:
        ]  # 🌙 Moon Dev Optimization: Extended to 10 bars for capturing recent pullbacks more effectively
        touch_fib = any(
            recent_low <= fib618 + (0.005 * close) for recent_low in recent_lows
        )  # 🌙 Moon Dev Optimization: Increased tolerance to 0.5% to account for BTC volatility, allowing near-touches
        volume_confirm = (
            volume > 1.0 * avg_vol
        )  # 🌙 Moon Dev Optimization: Reduced threshold to 1.0x to increase trade frequency in varying volume conditions
        momentum_rsi = (
            rsi > 40 and rsi < 70
        )  # 🌙 Moon Dev Optimization: Widened RSI range to include more neutral momentum, enabling entries on dips without overbought restriction
        trend_strength = (
            adx > 25
        )  # 🌙 Moon Dev Optimization: Raised to 25 for stronger trend filter, avoiding weak markets
        macd_confirm = (
            macd > macd_signal and (macd - macd_signal) > 0
        )  # 🌙 Moon Dev Optimization: Added positive histogram for accelerating momentum confirmation
        uptrend = (
            close > ema200 and ema20 > ema50 > ema200
        )  # Retained strengthened uptrend filter
        bullish_candle = (
            close > self.data.Open[-1]
        )  # Retained bullish candle confirmation
        ...
        if (
            ...
            swing_low_recent = min(
                self.data.Low[-20:]
            )  # 🌙 Moon Dev Optimization: Increased recent low lookback to 20 bars for more conservative support
            ...
            position_size = max(
                0.01, min(int(round(position_size)), int(capital * 0.15 / entry_price))
            )  # 🌙 Moon Dev Optimization: Increased cap to 15% for aggressive compounding in bull runs
            if position_size > 0:
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry_price:.2f}, SL {stop_price:.2f}, Size {position_size} BTC, True Golden Cross + Enhanced Filters! ✨🚀"
                )
        ...
        if self.position:
            ...
            print(
                f"🚀 Moon Dev Position: Bars {bars_since_entry}, RR {rr:.2f}, Current SL {current_sl:.2f} 🌙"
            )
            ...
            if unrealized_pnl >= 0.5 * risk:
                ...
                if breakeven_sl > current_sl:
                    ...
                    print(
                        f"🌙 Moon Dev: Moved SL to breakeven {breakeven_sl:.2f} after 0.5:1 RR ✨"
                    )
            ...
            if unrealized_pnl >= risk:
                ...
                if trail_sl > current_sl:
                    ...
                    print(
                        f"🌙 Moon Dev: Trailing SL updated to {trail_sl:.2f} after 1:1 RR ✨"
                    )
            ...
            if not self.partial_closed_1 and unrealized_pnl >= 1.5 * risk:
                ...
                print(
                    f"🌙 Moon Dev: Partial close 30% at 1.5:1 RR, securing early gains! 🚀"
                )
            ...
            if unrealized_pnl >= 8 * risk:
                print(
                    f"🌙 Moon Dev: Full TP at 8:1 RR for outsized BTC trend gains! 🚀"
                )
                self.position.close()
                return
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Bearish RSI + MACD Divergence in overbought zone, EXITING! 🚀"
                )
                self.position.close()
                return
