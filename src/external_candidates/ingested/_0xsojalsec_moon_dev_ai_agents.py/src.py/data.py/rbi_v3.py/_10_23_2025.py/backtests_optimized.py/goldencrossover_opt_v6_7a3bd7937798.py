# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_23_2025\backtests_optimized\GoldenCrossover_OPT_v6.py
import numpy as np
import pandas as pd
import talib
...
class GoldenCrossover(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 New: EMA100 for intermediate trend filter
        ...
        self.macd, self.macd_signal, _ = self.I(
            talib.MACD, self.data.Close, fastperiod=8, slowperiod=17, signalperiod=6
        )
    ...
    def next(self):
        ...
        )  # 🌙 Fixed: Actual EMA20/50 golden cross for timely trend starts
        # Check Fib touch in recent bars
        recent_lows = self.data.Low[
            -3:
        ]  # 🌙 Tightened to last 3 bars for fresher pullbacks
        touch_fib = any(
            recent_low <= fib618 + (0.002 * close) for recent_low in recent_lows
        )  # 🌙 Reduced tolerance to 0.2% for stricter Fib bounce
        volume_confirm = (
            volume > 1.5 * avg_vol
        )  # 🌙 Increased to 1.5x for higher quality volume spikes, reducing noise entries
        momentum_rsi = (
            rsi > 50 and rsi < 70
        )  # 🌙 Adjusted RSI to 50-70 to capture building strength in uptrends, avoiding early exhaustion
        trend_strength = (
            adx > 25
        )  # 🌙 Raised to 25 for stronger trend filter, improving signal quality
        macd_hist = (
            macd - macd_signal > 0
        )  # 🌙 Changed to positive MACD histogram for accelerating bullish momentum
        uptrend = (
            close > ema200 and ema20 > ema50 > ema100 > ema200
        )  # 🌙 Added EMA100 to uptrend for multi-EMA confirmation, ensuring robust bull bias
        volatility_filter = (
            atr > avg_atr_val
        )  # 🌙 New: Only enter in higher volatility regimes for trend-following edge
        bullish_candle = close > self.data.Open[-1] + (
            0.001 * close
        )  # 🌙 Strengthened to > open + 0.1% for decisive bullish action
        ...
        if (
            ...
            sl_distance = (
                1.0 * atr
            )  # 🌙 Reduced multiplier for tighter risk control, allowing higher RR potential
            stop_price = entry_price - sl_distance
            swing_low_recent = min(
                self.data.Low[-10:]
            )  # 🌙 Reduced lookback to 10 bars for more relevant support
            if fib618 < entry_price:
                stop_price = min(
                    stop_price, fib618 - 0.2 * atr, swing_low_recent
                )  # 🌙 Tighter buffer below Fib
            ...
            )  # 🌙 Increased cap to 15% for BTC leverage potential, min 0.01
            if position_size > 0:
                print(
        ...
        if self.position:
            ...
            if unrealized_pnl >= 0.5 * risk:
                breakeven_sl = entry_price + (
                    0.3 * atr
                )  # 🌙 Increased buffer slightly for volatility cushion
            ...
            if unrealized_pnl >= risk:
                ...
                trail_sl = (
                    ema50 - 0.5 * atr
                )  # 🌙 Switched to EMA50 for more conservative trailing, reduced multiplier
            ...
            if not self.partial_closed and unrealized_pnl >= 1.5 * risk:
                ...
                print(
                    f"🌙 Moon Dev: Partial close 30% at 1.5:1 RR, early profit lock! 🚀"
                )

            if (
                self.partial_closed
                and not self.second_partial
                and unrealized_pnl >= 3 * risk
            ):
            ...
            if (
                unrealized_pnl >= 4 * risk
            ):  # 🌙 Reduced full TP from 5:1 to 4:1 for more frequent larger wins, realistic for BTC trends
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Enhanced Bearish RSI/MACD/Volume Divergence, EXITING! 🚀"
                )
                self.position.close()
                return
            ...
            if (
                close < ema50
            ):  # 🌙 Changed to EMA50 for proactive exits on trend weakness
