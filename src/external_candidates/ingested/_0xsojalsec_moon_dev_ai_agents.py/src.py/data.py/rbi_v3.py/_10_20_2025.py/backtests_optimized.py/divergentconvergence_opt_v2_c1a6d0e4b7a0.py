...
class DivergentConvergence(Strategy):
    ...
    lookback = (
        25  # Increased lookback for better divergence detection in volatile markets 🌙
    )
    ...
    buffer_mult = (
        0.75  # Adjusted buffer for SL to balance tightness and avoiding whipsaws
    )
    ...
    def next(self):
        ...
        if (
            ...
            if risk_per_unit <= 0:
                ...
                )  # Fallback to pure ATR SL for validity
                risk_per_unit = entry_price - sl_price
            if risk_per_unit <= 0:
                print(
