# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\ExtremeDivergence_OPT_v4.py
import numpy as np
import pandas as pd
import talib
...
class ExtremeDivergence(Strategy):
    def init(self):
        ...
        self.entry_price = 0
        self.risk_dist = (
            0  # 🌙 Moon Dev Optimization: Store risk distance for RR-based take profit
        )
        self.trail_sl = 0
        self.highest = 0
        self.lowest = 0
            ...
            if (
                ...
                if risk_dist > 0:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev Long Entry at {entry:.2f}! Size: {size}, SL: {sl:.2f} ✨🚀"
                        )
            elif (
                downtrend
                and bearish_div
                ...
                if risk_dist > 0:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev Short Entry at {entry:.2f}! Size: {size}, SL: {sl:.2f} ✨📉"
                        )
        ...
        if self.position and self.risk_dist > 0:
            ...
            if self.position.is_long:
                ...
                if profit_dist >= 3 * self.risk_dist:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev Long Exit at {self.data.Close[-1]:.2f} - Reason: Take Profit 3:1 💰✨"
                    )
                ...
                if profit_dist >= 3 * self.risk_dist:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev Short Exit at {self.data.Close[-1]:.2f} - Reason: Take Profit 3:1 💰✨"
                    )
                    return  # Exit early after TP
