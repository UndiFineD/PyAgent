# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v17.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def init(self):
        ...
        self.slowk = self.I(
            lambda h, l, c: talib.STOCH(
                h, l, c, fastk_period=14, slowk_period=3, slowd_period=3
            )[0],
            self.data.High,
            self.data.Low,
            self.data.Close,
        ...
        self.plus_di = self.I(
            talib.PLUS_DI, self.data.High, self.data.Low, self.data.Close, timeperiod=14
        )  # 🌙 Moon Dev Optimization: Added +DI to confirm bullish directional momentum
        self.minus_di = self.I(
            ...
            self.data.Close,
            timeperiod=14,
        )  # 🌙 Moon Dev Optimization: Added -DI to ensure uptrend dominance, filtering out weak entries
        self.entry_bar = None
        ...
        self.div_bar = None
        self.current_sl = (
            None  # 🌙 Moon Dev Optimization: Track current trailing SL level
        )
    ...
    def next(self):
        ...
        bull_div = (
            (low2 < low1) and (rsi2 > rsi1) and (rsi2 < 50) and (distance <= 5)
        )  # 🌙 Moon Dev Optimization: Loosened RSI threshold to <50 and distance to <=5 for more frequent, realistic divergence signals
        if bull_div:
            self.div_low = low2
            self.div_bar = current_bar
            print(
                f"🌙 Moon Dev: Bullish RSI Divergence detected! Price low: {low2}, RSI: {rsi2} ✨ Distance: {distance} bars"
            )
        ...
        stoch_converge = (
            ...
            and (k_now < d_now)
        )  # 🌙 Moon Dev Optimization: Relaxed %K threshold to <30 to capture more oversold convergence opportunities in BTC
        if stoch_converge:
            print(
                f"🚀 Moon Dev: Stochastic Convergence! %K: {k_now}, %D: {d_now}, Gap narrowed! 🌙"
            )
        ...
        if (
            ...
            and stoch_converge
            and current_price > self.sma200[-1]
            and self.data.Volume[-1] > 1.2 * self.vol_sma[-1]
            and self.adx[-1] > 18
            and self.plus_di[-1] > self.minus_di[-1]
        ):  # 🌙 Moon Dev Optimization: Refined filters - volume >1.2x SMA, ADX>18 for more trades, +DI > -DI for bullish regime confirmation, ensuring quality trending entries
            ...
            if size > 0:
                ...
                self.buy(
                    size=size, tp=tp_price
                )  # 🌙 Moon Dev Optimization: Retained fixed TP with higher RR, complemented by improved trailing for balanced exits
                ...
                self.div_low = None
                print(
                    f"🌙 Moon Dev: LONG ENTRY! Size: {size} (fraction), Entry: {entry_price}, Initial SL: {sl_price}, TP: {tp_price} 🚀✨"
                )
            else:
        ...
        if self.position:
            ...
            print(
                f"🌙 Moon Dev: Position held. Bars: {bars_held}, Price: {current_price}, RSI: {rsi_now}, %K: {self.slowk[-1]}, Trail SL: {self.current_sl}, High: {self.highest_since_entry} ✨"
            )
