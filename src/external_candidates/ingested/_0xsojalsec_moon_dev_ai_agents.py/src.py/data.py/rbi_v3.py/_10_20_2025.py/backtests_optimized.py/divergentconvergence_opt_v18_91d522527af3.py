# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v18.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def init(self):
        ...
        macd_line, macd_signal, _ = self.I(
    ...
    def next(self):
        ...
        )  # 🌙 Moon Dev Optimization: Tightened RSI oversold threshold to <40 for stronger bearish-to-bullish reversals; extended distance to <=5 bars for more flexible but timely signals
        if bull_div:
            self.div_low = low2
        ...
        stoch_converge = (
            ...
            and (k_now < d_now)
        )  # 🌙 Moon Dev Optimization: Lowered %K threshold to <20 for deeper oversold entries, capturing stronger momentum shifts in BTC
        if stoch_converge:
            print(
        ...
        if (
            ...
            and self.data.Volume[-1] > 0.8 * self.vol_sma[-1]
            and self.adx[-1] > 15
            and self.macd[-1] > self.macd_signal[-1]
        ):  # 🌙 Moon Dev Optimization: Loosened volume filter to >80% of SMA and ADX to >15 for more entry opportunities; added MACD bullish cross for momentum confirmation to improve win rate
            ...
            if size > 0:
                ...
                )  # 🌙 Moon Dev Optimization: Retained TP for automatic profit taking; manual trailing for dynamic protection
