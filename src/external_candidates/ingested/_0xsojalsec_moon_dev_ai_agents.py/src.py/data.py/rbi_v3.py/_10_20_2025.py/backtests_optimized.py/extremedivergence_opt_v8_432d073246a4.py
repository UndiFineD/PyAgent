    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and bullish_div
                and self.adx[-1] > 20
                and self.data.Volume[-1] > self.vol_ma[-1] * 1.2
            ):
                ...
                if risk_dist > 0:
                    size = (
                        self.equity * 0.01
                    ) / risk_dist  # 🌙 Moon Dev Optimization: Allow float size for precise risk management (no int rounding)
                    if size > 0:
                        self.buy(size=size)
                ...
                if risk_dist > 0:
                    size = (
                        self.equity * 0.01
                    ) / risk_dist  # 🌙 Moon Dev Optimization: Float size for precision
                    if size > 0:
                        self.sell(size=size)
                        self.entry_price = entry
                        self.trail_sl = sl
                        self.tp = (
                            entry - 3 * risk_dist
                        )  # 🌙 Moon Dev Optimization: Set 3R take profit
