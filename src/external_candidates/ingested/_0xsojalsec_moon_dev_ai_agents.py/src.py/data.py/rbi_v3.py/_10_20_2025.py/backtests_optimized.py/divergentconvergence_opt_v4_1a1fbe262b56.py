...
class DivergentConvergence(Strategy):
    ...
    atr_mult_sl = 1.5
    buffer_mult = (
        1.0  # Increased buffer for SL to account for volatility, reducing whipsaws ⚠️
    )
    ...
    def init(self):
        ...
        )  # New: EMA50 for shorter-term trend filter 📊
        self.atr = self.I(
    ...
    def next(self):
        ...
        )  # New: ADX >25 to avoid choppy markets 🌙
        vol_confirm = self.data.Volume[-1] > (
            self.vol_sma[-1] * self.vol_mult
        ...
        if (
            ...
            print(
                f"🌙 Moon Dev: All entry conditions met including filters! Attempting LONG entry... 🚀"
            )
            # Calculate SL: below div low with increased buffer
            div_low = self.div_low
            atr_val = self.atr[-1]
            sl_price = div_low - (
                atr_val * self.buffer_mult
            )  # Wider buffer to reduce false stops ⚠️
            ...
            if risk_per_unit <= 0:
                ...
                return
            size_frac = (
                self.risk_pct * entry_price / risk_per_unit
            )  # Now 2% risk for more exposure
            size = min(1.0, size_frac)
            if size > 0:
                tp_price = (
                    entry_price + self.rr * risk_per_unit
                )  # Higher RR=3 for better returns 📈
                self.buy(
                    size=size
                )  # Buy without sl/tp - manage manually for trailing 🌟
        ...
        if self.position:
            ...
            if (
                current_price > self.entry_price + atr_val
            ):  # Once in profit by 1 ATR, move to breakeven
                if not self.breakeven_hit:
                    ...
                    new_trail = current_price - (
                        atr_val * self.trail_mult
                    )  # Trail by 2 ATR
