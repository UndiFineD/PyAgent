...
class InverseMomentum(Strategy):
    fast_period = (
        12  # 🌙 Optimized: Kept at 12 for balanced responsiveness on daily BTC
    )
    ...
    trend_period = (
        50  # 🌙 Optimized: Kept at 50 for robust long-term trend confirmation
    )
    ...
    trail_start_mult = (
        2.0  # New: Start trailing after 2x ATR profit to avoid premature trailing
    )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long and bearish_cross:
                self.position.close()
                print(
                    f"🌙 Moon Dev Long Exit: Bearish crossover detected! Closing at {current_price:.2f} ✨"
                )
            elif self.position.is_short and bullish_cross:
                self.position.close()
                print(
                    f"🌙 Moon Dev Short Exit: Bullish crossover detected! Closing at {current_price:.2f} ✨"
                )
            ...
            if hasattr(self, "entry_price") and hasattr(self, "entry_atr"):
                if self.position.is_long:
                    if not hasattr(self, "trail_start_long"):
                        self.trail_start_long = self.entry_price + (
                            self.trail_start_mult * self.entry_atr
                        )
                    ...
                    if not hasattr(self, "trail_start_short"):
                        self.trail_start_short = self.entry_price - (
                            self.trail_start_mult * self.entry_atr
                        )
            ...
            if (
                ...
                self.buy(
                    size=size, sl=sl_price
                )  # No TP - rely on trailing and signals for extended winners
            ...
            # Short entry conditions (symmetric to long for inverse momentum on downtrends)
                ...
                self.sell(
                    size=size, sl=sl_price
                )  # No TP - rely on trailing and signals
                self.entry_price = entry_price
                self.entry_atr = self.atr[-1]
                # Reset trailing flag
