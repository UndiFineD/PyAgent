# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\InverseMomentum_BEST_2.246898pct.py
import numpy as np
import pandas as pd
import talib
...
class InverseMomentum(Strategy):
    fast_period = (
        12  # 🌙 Optimized: Lengthened from 3 to 12 for fewer whipsaws on daily BTC data
    )
    ...
    trend_period = (
        50  # 🌙 Optimized: Increased from 20 to 50 for stronger long-term trend filter
    )
    ...
    def init(self):
        ...
        self.vol_sma = self.I(
            talib.SMA, volume, timeperiod=self.vol_period
        )  # SMA fine for volume
        self.trend_sma = self.I(
            talib.SMA, close, timeperiod=self.trend_period
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev Entry: Bullish momentum on BTC! EMA crossover confirmed with volume ({self.vol_mult}x), "
