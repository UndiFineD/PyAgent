...
class DivergentReversion(Strategy):
    ...
    atr_multiplier_sl = 2
    atr_multiplier_tp = (
        5  # Increased to 1:2.5 RR for better profit potential while maintaining risk 🌙
    )
    ...
    def init(self):
        ...
        )  # Matched to RSI period for consistency
        ...
        print(
            "🌙 Moon Dev: Indicators initialized with optimizations: standard params, volume & trend filters ✨"
        )
        ...
        if self.position:
            ...
            if self.position.is_long:
                if (self.rsi[-1] > self.rsi_sma[-1]) or (
                    ...
                    self.position.close()
                    print(
                        f"🌙 Moon Dev: Long Exit on Reversion Signal at {current:.2f} 🚀"
                    )
                    return
            elif self.position.is_short:
                if (self.rsi[-1] < self.rsi_sma[-1]) or (
                    ...
                    self.position.close()
                    print(
                        f"🌙 Moon Dev: Short Exit on Reversion Signal at {current:.2f} 🚀"
                    )
                    return
