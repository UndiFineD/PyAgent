# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_final\DivergentConvergence_BTFinal_v6.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            if size > 0:
                self.buy(
                    size=size, sl=sl_price, tp=entry_price + self.rr * risk_per_unit
                )
                ...
                print(
                    f"🌙 Moon Dev: LONG ENTRY! Size: {size}, Entry: {entry_price}, SL: {sl_price}, TP: {self.tp_price} 🚀✨"
                )
            else:
