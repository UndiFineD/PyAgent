# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    stoch_window = (
        8  # Slightly increased for smoother stochastic convergence signals 🚀
    )
    ...
    def init(self):
        ...
        self.sma50 = self.I(
            talib.SMA, self.data.Close, timeperiod=50
        )  # Added SMA50 for shorter-term trend filter 📈
        self.atr = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, timeperiod=14
        )
        self.avg_vol = self.I(
            talib.SMA, self.data.Volume, timeperiod=20
        )  # Added average volume for entry filter 📊
        self.adx = self.I(
        ...
        self.div_bar = None
        print(
            "🌙 Moon Dev Backtest Initialized! ✨ Initializing indicators with optimizations... 🚀"
        )
    ...
    def next(self):
        ...
        )  # Tightened to RSI <40 for stronger signals 🎯
        if bull_div:
            self.div_low = low2
        ...
        stoch_converge = (
            ...
            and (k_now < d_now)
        )  # Loosened to <25 for more opportunities 🚀
        if stoch_converge:
            print(
        ...
        if (
            ...
            and self.sma50[-1] > self.sma200[-1]
            and self.data.Volume[-1] > (self.vol_mult * self.avg_vol[-1])
            and self.adx[-1] > self.adx_threshold
        ):
            print(
                f"🌙 Moon Dev: All entry conditions met with filters! Attempting LONG entry... 🚀"
            )
        ...
        if self.position:
            ...
            if self.trail_sl is not None:
                ...
                if new_trail > self.trail_sl:
                    ...
                    self.position.sl = (
                        self.trail_sl
                    )  # Update the position's SL in backtesting.py
                    print(
                        f"🌙 Moon Dev: Trailing SL updated to {self.trail_sl} (ATR-based) 🔒"
                    )
