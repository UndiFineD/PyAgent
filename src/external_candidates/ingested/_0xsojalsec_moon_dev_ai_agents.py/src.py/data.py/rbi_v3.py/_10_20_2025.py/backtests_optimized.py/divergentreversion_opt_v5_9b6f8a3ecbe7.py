# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentReversion_OPT_v5.py
import numpy as np
import pandas as pd
import talib
...
class DivergentReversion(Strategy):
    ...
    risk_per_trade = (
        0.015  # Increased from 1% to 1.5% for higher exposure while maintaining risk
    )
    atr_multiplier_sl = 1.5  # Tightened SL from 2 to 1.5 ATR for better risk control
    atr_multiplier_tp = (
        4.5  # Increased TP from 4 to 4.5 ATR for improved 1:3 RR to boost returns
    )
    ...
    def next(self):
        ...
        if (
            ...
            if (self.rsi[-1] < self.rsi_sma[-1]) and (
                ...
                equity = (
                    self._broker.get_cash() + self.position.pl
                )  # Use current equity
