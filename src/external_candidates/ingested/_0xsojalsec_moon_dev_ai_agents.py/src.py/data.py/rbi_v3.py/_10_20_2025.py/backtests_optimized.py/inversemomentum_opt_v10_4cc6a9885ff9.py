# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\InverseMomentum_OPT_v10.py
import numpy as np
import pandas as pd
import talib
...
class InverseMomentum(Strategy):
    ...
    rsi_threshold = (
        45  # New: Lowered RSI entry from 50 to 45 for earlier momentum capture
    )
    ...
    def init(self):
        ...
        # Debug print on init
        print(
            "🌙 Moon Dev Backtest Initialized: Further Optimized InverseMomentum Strategy Loaded! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.rsi[-1] > 80:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: Overbought RSI >80 detected! Closing position at {self.data.Close[-1]:.2f} to lock profits ✨"
                )
        else:
            ...
            if (
                ...
                and self.data.Close[-1] > self.long_trend_sma[-1]
