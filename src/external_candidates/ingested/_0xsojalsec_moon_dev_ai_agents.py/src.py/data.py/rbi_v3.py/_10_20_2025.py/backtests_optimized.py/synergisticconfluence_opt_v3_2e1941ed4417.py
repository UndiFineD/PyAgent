# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\SynergisticConfluence_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class SynergisticConfluence(Strategy):
    ...
    def init(self):
        ...
        self.ema50 = self.I(
            talib.EMA, close, timeperiod=self.ema_period
        )  # 🌙 Moon Dev: EMA50 for directional bias
        self.vol_sma = self.I(
            talib.SMA, volume, timeperiod=self.vol_sma_period
        )  # 🌙 Moon Dev: Volume SMA for entry confirmation
    ...
    def next(self):
        ...
        if self.position:
            ...
            risk_distance = (
                abs(self.entry_price - self.stop_price)
                if self.entry_price != 0
                else current_atr
            )  # Fallback for initial risk calc

            # Update trailing stop with tighter multiplier
            if self.is_long:
                trail_sl = (
                    current_close - 1.2 * current_atr
                )  # 🌙 Moon Dev: Tighter trailing for better profit capture
                self.stop_price = max(self.stop_price, trail_sl)
                if current_low <= self.stop_price:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev Long SL Hit at {current_low}, Trail SL {self.stop_price} ✨"
                    )
                    return
                ...
                if partial_cond and not self.partial_taken:
                    partial_size = (
                        abs(self.position.size) * 0.5
                    )  # 🌙 Moon Dev: Fixed to 50% of current position size
                    self.sell(size=partial_size)
                    self.partial_taken = True
                    # Move stop to breakeven + small buffer
                    self.stop_price = max(
                        self.stop_price, self.entry_price + 0.1 * current_atr
                    )
                    print(
                        f"🌙 Moon Dev Partial Profit Long at {current_close}, Size Closed: {partial_size} ✨"
                    )
                ...
                if current_high >= self.stop_price:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev Short SL Hit at {current_high}, Trail SL {self.stop_price} ✨"
                    )
                    return
                # 🌙 Moon Dev: Improved partial at 1R, then move to breakeven + buffer
                partial_cond = current_close <= (self.entry_price - risk_distance)
                if partial_cond and not self.partial_taken:
                    ...
                    print(
                        f"🌙 Moon Dev Partial Profit Short at {current_close}, Size Covered: {partial_size} ✨"
                    )
                # 🌙 Moon Dev: Full TP at 3R after partial
                if self.partial_taken and current_close <= self.tp_price:
                    self.position.close()
        ...
        long_rsi_cross = (
            current_rsi > 30 and prev_rsi <= 30 and current_rsi < 50
        )  # 🌙 Moon Dev: Kept <50 to avoid overbought entries
        ...
        )  # 🌙 Moon Dev: EMA50 trend filter for longs
        if (
            long_rsi_cross
            and long_bb_touch
            and long_macd_cross
            and long_hist_expand
            and long_trend
            and volume_filter
        ):
            if self.position and not self.is_long:
                self.position.close()
            stop_price = (
                current_swing_low - 1.5 * current_atr
            )  # 🌙 Moon Dev: Tighter initial stop for better R:R
            ...
            if risk_distance > 0:
                ...
                self.tp_price = (
                    current_close + 3 * risk_distance
                )  # 🌙 Moon Dev: Increased to 3R for higher returns
                self.partial_taken = False
                self.entry_bar = len(self.data)
                self.is_long = True
                print(
                    f"🌙 Moon Dev LONG Entry at {current_close}, Size: {size}, SL: {stop_price}, TP: {self.tp_price} 🚀"
                )

        # Short entry with added EMA and volume filters, tightened RSI
        short_rsi_cross = (
            current_rsi < 70 and prev_rsi >= 70 and current_rsi > 50
        )  # 🌙 Moon Dev: Kept >50 to avoid oversold entries
        ...
        short_trend = (
            current_close < current_ema50
        )  # 🌙 Moon Dev: EMA50 trend filter for shorts
        if (
            short_rsi_cross
            and short_bb_touch
            and short_macd_cross
            and short_hist_expand
            and short_trend
            and volume_filter
        ):
            if self.position and self.is_long:
                self.position.close()
            stop_price = (
                current_swing_high + 1.5 * current_atr
            )  # 🌙 Moon Dev: Tighter initial stop for better R:R
            ...
            if risk_distance > 0:
                ...
                self.is_long = False
                print(
                    f"🌙 Moon Dev SHORT Entry at {current_close}, Size: {size}, SL: {stop_price}, TP: {self.tp_price} 🚀"
                )
