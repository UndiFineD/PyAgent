...
class DivergentConvergence(Strategy):
    ...
    max_bars = (
        30  # Optimized: Extended max hold time to 30 bars to allow more room for trends
    )
    ...
    min_div_strength = (
        3  # Optimized: Added minimum RSI divergence strength to filter weak signals
    )
    adx_min = (
        20  # Optimized: ADX threshold for trend strength filter to avoid choppy markets
    )
    ...
    def next(self):
        ...
        if (
            ...
            if size > 0:
                ...
                self.div_low = None
                print(
                    f"🌙 Moon Dev: LONG ENTRY! Size: {size} (fraction), Entry: {entry_price}, SL: {sl_price} 🚀✨"
                )
            else:
