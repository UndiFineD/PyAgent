# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_final\DifferentialCrossover_BTFinal_v4.py
import pandas as pd
import talib
...
class DifferentialCrossover(Strategy):
    ...
    def init(self):
        ...
        self.ema_diff = self.I(
            lambda: talib.EMA(self.diff, timeperiod=self.ema20_period)
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.current_sl is not None and self.data.Low[-1] <= self.current_sl:
                self.position.close()
                print(
                    f"🌙 Moon Dev: SL hit! Price low: {self.data.Low[-1]}, SL: {self.current_sl:.2f} ✨"
                )
                self.trailing_active = False
                self.initial_risk = 0
                self.current_sl = None
            ...
            if self.diff[-1] < self.ema_diff[-1] and self.diff[-2] >= self.ema_diff[-2]:
                self.position.close()
                print(
                    f"🚀 Moon Dev: Exit on momentum reversal at {self.data.Close[-1]} 🌙"
                )
                self.trailing_active = False
                self.current_sl = None
                return
        ...
        if not self.position:
            if (
                ...
                and self.diff[-1] > self.ema_diff[-1]
                and self.diff[-2] <= self.ema_diff[-2]
            ):
                ...
                self.trailing_active = False
                print(
                    f"🌙 Moon Dev: LONG Entry! Price: {entry_price}, Size: {size}, SL: {sl:.2f}, TP: {tp:.2f}, Risk: {risk:.2f} ✨🚀"
                )


bt = Backtest(data, DifferentialCrossover, cash=1000000, commission=0.001)
stats = bt.run()
print(stats)
