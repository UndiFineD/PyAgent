# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v14.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    lookback = (
        15  # Tightened lookback for more recent divergences to catch fresher setups 🌙
    )
    ...
    buffer_mult = (
        0.3  # Reduced buffer for more precise SL placement near divergence low ⚡
    )
    ...
    def next(self):
        ...
        )  # Stricter RSI <40 for better quality ✨
        if bull_div:
            self.div_low = low2
        ...
        if self.position:
            ...
            if (
                ...
                and risk_per_unit > 0
                and current_price >= self.entry_price + risk_per_unit
            ):  # Activate at 1:1
