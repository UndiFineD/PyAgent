# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\ExtremeDivergence_OPT_v6.py
import numpy as np
import pandas as pd
import talib
...
class ExtremeDivergence(Strategy):
    ...
    def next(self):
        ...
        if (
            current_len < 21
        ):  # 🌙 Moon Dev Optimization: Increased from 11 to 21 for more stable EMA slope calculation
            return
            ...
            if uptrend and bullish_div and self.rsi[-1] < 40 and self.macd_line[-1] > 0:
                ...
                if risk_dist > 0:
                    ...
                    if size > 0:
                        ...
                        self.tp = (
                            entry + 3 * risk_dist
                        )  # 🌙 Moon Dev Optimization: Set 3:1 RR take profit
        ...
        if self.position:
            if self.position.is_long:
                ...
                if self.data.Low[-1] <= self.trail_sl:
                    ...
                    exit_reason = (
                        "Take Profit"  # 🌙 Moon Dev Optimization: Added TP exit
                    )
                elif self.rsi[-1] > 70:
