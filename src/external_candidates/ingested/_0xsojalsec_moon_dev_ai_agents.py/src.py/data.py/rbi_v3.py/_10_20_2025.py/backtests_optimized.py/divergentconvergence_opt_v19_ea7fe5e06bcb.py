# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v19.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def init(self):
        ...
        self.partial_taken = (
            False  # 🌙 Moon Dev Optimization: Flag for partial profit taking
        )
    ...
    def next(self):
        ...
        )  # 🌙 Moon Dev Optimization: Tightened RSI oversold to <40 for deeper, higher-quality signals; extended distance to <=5 for slightly more flexible timing
        if bull_div:
            self.div_low = low2
        ...
        if (
            ...
            and current_price > self.ema21[-1]
            and self.data.Volume[-1] > self.vol_mult * self.vol_sma[-1]
            and self.adx[-1] > self.adx_threshold
        ):  # 🌙 Moon Dev Optimization: Added EMA21 uptrend filter and volume multiplier for enhanced entry quality in short-term bullish regimes; lowered ADX for more trades
            ...
            if size > 0:
                ...
                self.buy(
                    size=size
                )  # 🌙 Moon Dev Optimization: Removed TP from order to enable manual partial and full profit taking
                ...
                self.partial_taken = (
                    False  # 🌙 Moon Dev Optimization: Reset partial flag on new entry
                )
        ...
        if self.position:
            ...
            if self.entry_price is not None and self.risk_per_unit is not None:
                ...
                if (
                    profit >= partial_rr * self.risk_per_unit
                    ...
                    print(
                        f"🌙 Moon Dev: Partial TP at {partial_rr}:1 RR! Locked {half_size} size. 🔒"
                    )
