# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_final\AdaptiveDivergence_BTFinal_v2.py
    ...
    def next(self):
        ...
        if self.position.size != 0:
            ...
            if self.is_long:
                ...
                # Reversal check
                if (
                    self.detect_bearish_div()
                    or self.current_mom < self.mom_short_thresh
                ):
                    self.position.close()
                    print(
        ...
        # Entry logic if no position
            ...
            if (
                ...
                or self.atr[-1] <= self.atr_avg[-1]
            ):
                print(
                    f"🌙 Low volatility filter: ATR {self.atr[-1]:.2f} <= Avg {self.atr_avg[-1]:.2f} 😴"
                )
                return
            ...
            if (
                ...
                self.highest_since_entry = self.data.High[-1]
                print(
                    f"🌙 Bullish divergence detected! Long entry at {self.entry_price:.2f}, size: {size}, Mom: {self.current_mom:.2f} 🚀 ATR: {self.atr[-1]:.2f}"
                )
            ...
            # Short entry
                ...
                self.lowest_since_entry = self.data.Low[-1]
                print(
                    f"🌙 Bearish divergence detected! Short entry at {self.entry_price:.2f}, size: {size}, Mom: {self.current_mom:.2f} ✨ ATR: {self.atr[-1]:.2f}"
                )
    ...
    def update_swings(self):
        ...
        for potential in range(self.last_checked + 1, current_bar - pl + 1):
            ...
            for k in range(
                ...
                if (
                    k != potential
                    and self.data.Low.iloc[k] < self.data.Low.iloc[potential]
                ):
                    is_low_pivot = False
                    break
            if is_low_pivot:
                ...
                if not np.isnan(rsi_val):
                    self.confirmed_swing_lows.append(
                        (potential, self.data.Low.iloc[potential], rsi_val)
                    )
                    print(
                        f"🌙 New swing low confirmed at bar {potential}, price {self.data.Low.iloc[potential]:.2f}, RSI {rsi_val:.2f} 📉"
                    )
            ...
            for k in range(
                ...
                if (
                    k != potential
                    and self.data.High.iloc[k] > self.data.High.iloc[potential]
                ):
                    is_high_pivot = False
                    break
            if is_high_pivot:
                ...
                if not np.isnan(rsi_val):
                    self.confirmed_swing_highs.append(
                        (potential, self.data.High.iloc[potential], rsi_val)
                    )
                    print(
                        f"🌙 New swing high confirmed at bar {potential}, price {self.data.High.iloc[potential]:.2f}, RSI {rsi_val:.2f} 📈"
                    )
