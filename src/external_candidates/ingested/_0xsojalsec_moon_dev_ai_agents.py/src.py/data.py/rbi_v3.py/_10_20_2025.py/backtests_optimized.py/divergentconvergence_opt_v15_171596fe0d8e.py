# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v15.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def next(self):
        ...
        bull_div = (
            (low2 < low1) and (rsi2 > rsi1) and (rsi2 < 45) and (distance <= 4)
        )  # 🌙 Moon Dev Optimization: Tightened RSI threshold to <45 for deeper oversold and distance to <=4 for fresher signals
        if bull_div:
            self.div_low = low2
        ...
        stoch_converge = (
            ...
            and (k_now < d_now)
        )  # 🌙 Moon Dev Optimization: Relaxed %K threshold to <25 for more opportunities in BTC volatility
        if stoch_converge:
            print(
        ...
        if (
            ...
            and self.adx[-1] > 20
        ):  # 🌙 Moon Dev Optimization: Added volume and ADX filters for quality entries in trending, high-volume conditions
