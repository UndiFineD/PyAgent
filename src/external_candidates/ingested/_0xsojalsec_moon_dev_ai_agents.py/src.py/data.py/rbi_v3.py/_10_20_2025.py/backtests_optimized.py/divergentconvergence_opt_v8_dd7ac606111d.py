# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v8.py
import numpy as np
import pandas as pd
import talib
...
print(
    "🌙 Moon Dev: Data loaded and cleaned successfully! Columns: Open, High, Low, Close, Volume. Index: Datetime. ✨"
)
...
class DivergentConvergence(Strategy):
    ...
    buffer_mult = 0.5
    trail_atr_mult = (
        2.0  # New: ATR multiplier for trailing stop activation and distance 🌙
    )
    trail_start_rr = (
        1.0  # New: Start trailing after 1:1 RR profit to lock in gains early 🚀
    )

    def init(self):
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=10
        )  # Shortened from 14 to 10 for more sensitive oversold signals in short-term BTC moves ✨
        self.slowk = self.I(
            lambda h, l, c: talib.STOCH(
                h, l, c, fastk_period=8, slowk_period=3, slowd_period=3
            )[0],
            self.data.High,
            self.data.Low,
            self.data.Close,
        )  # Faster Stoch from 14 to 8 for quicker convergence detection 🌙
        self.slowd = self.I(
            lambda h, l, c: talib.STOCH(
                h, l, c, fastk_period=8, slowk_period=3, slowd_period=3
            )[1],
        ...
        self.ema50 = self.I(
            talib.EMA, self.data.Close, timeperiod=50
        )  # Changed from SMA200 to EMA50 for more responsive trend filter in 15m timeframe, catching uptrends sooner 🚀
        self.atr = self.I(
        ...
        )  # New: Volume SMA for confirmation filter to avoid low-volume fakeouts ✨
        self.entry_bar = None
    ...
    def next(self):
        if (
            len(self.data) < 100
        ):  # Reduced from 220 since using EMA50 instead of SMA200, faster initialization ✨
            return
        ...
        vol_at_recent_low = volumes_recent[
            argmin_low_recent
        ]  # New: Volume at recent low
        ...
        bull_div = (
            ...
            and (rsi2 < 50)
            and (distance <= 5)
            and (vol_at_recent_low > vol_sma)
        )
        if bull_div:
            self.div_low = low2
            self.div_bar = current_bar
            print(
                f"🌙 Moon Dev: Bullish RSI Divergence detected! Price low: {low2}, RSI: {rsi2} ✨ Distance: {distance} bars, Volume confirmed!"
            )
        ...
        k_momentum = (
            k_now > self.slowk[-self.stoch_window * 2]
            if len(self.slowk) >= self.stoch_window * 2
            else True
        )
        stoch_converge = (
            (k_now < 20)
        ...
        if stoch_converge:
            print(
                f"🚀 Moon Dev: Stochastic Convergence! %K: {k_now}, %D: {d_now}, Gap narrowed! Momentum: {k_momentum} 🌙"
            )

        # Entry Logic - Updated trend filter to EMA50, added volatility-based size adjustment
        if (
            not self.position
            and self.div_bar is not None
            and stoch_converge
            and current_price > self.ema50[-1]
        ):
            ...
            atr_val = self.atr[-1]
            sl_price = div_low - (
                atr_val * self.atr_mult_sl
            )  # Improved SL placement with explicit ATR mult for better risk management ✨
            entry_price = current_price
            risk_per_unit = entry_price - sl_price
            if risk_per_unit <= 0:
                print(
                    f"🌙 Moon Dev: Invalid risk per unit: {risk_per_unit}, skipping entry. ⚠️"
                )
                return
            # Dynamic sizing: Scale risk_pct down if ATR high (volatility filter) to maintain good risk management 🌙
            vol_adjust = min(
                1.0, 0.02 / (atr_val / entry_price)
            )  # Cap exposure in high vol
            ...
            if size > 0:
                ...
                self.div_low = None
                print(
                    f"🌙 Moon Dev: LONG ENTRY! Size: {size} (fraction, vol-adjusted), Entry: {entry_price}, SL: {sl_price}, TP: {tp_price} 🚀✨"
                )
            else:
        ...
        if self.position:
            bars_held = (
                current_bar - self.entry_bar if self.entry_bar is not None else 0
            )
            rsi_now = self.rsi[-1]
            k_cross_below_d = (self.slowk[-2] >= self.slowd[-2]) and (
                self.slowk[-1] < self.slowd[-1]
            )

            # New: Trailing Stop - Activate after trail_start_rr profit, trail with ATR
            profit_rr = (
                (current_price - self.entry_price) / (self.entry_price - self.sl_price)
                if (self.entry_price - self.sl_price) > 0
                else 0
            )
            if profit_rr >= self.trail_start_rr:
                ...
                print(
                    f"🌙 Moon Dev: Trailing SL updated to {self.trail_sl} (RR: {profit_rr:.2f}) ✨"
                )
            ...
            print(
                f"🌙 Moon Dev: Position held. Bars: {bars_held}, Price: {current_price}, RSI: {rsi_now}, %K: {self.slowk[-1]} ✨"
            )


# Run backtest
bt = Backtest(
    data, DivergentConvergence, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
