# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_final\DifferentialCrossover_BTFinal_v7.py
import pandas as pd
import talib
...
class DifferentialCrossover(Strategy):
    ...
    def init(self):
        ...
        self.ema_diff = self.I(
            lambda: talib.EMA(self.diff.array, timeperiod=self.ema20_period)
        )
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                if not valid_sls:
                    print(
                        "⚠️ Moon Dev: No valid SL candidates below entry, skipping 🌙"
                    )
                    return
                sl = max(valid_sls)
