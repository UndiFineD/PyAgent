# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v6.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    recent_period = 10
    stoch_window = (
        3  # Reduced to focus on more recent stochastic behavior for faster signals ✨
    )
    ...
    trail_atr_mult = (
        2.0  # Trail distance as 2 * ATR for better risk-reward in trends ✨
    )
    ...
    rsi_older_thresh = (
        30  # Tighter RSI threshold for older low to avoid weak signals ✨
    )
    rsi_recent_thresh = (
        40  # Tighter RSI threshold for recent low for more selective entries 🚀
    )
    ...
    def next(self):
        ...
        bull_div = (
            (low2 < low1)
            and (rsi2 > rsi1)
            and (rsi2 < self.rsi_recent_thresh)
            and (rsi1 < self.rsi_older_thresh)
            and (distance <= 5)
        )
        if bull_div:
            self.div_low = low2
        ...
        if (
            ...
            and self.data.Volume[-1] > self.vol_mult * self.vol_avg[-1]
