...
class SynergisticConfluence(Strategy):
    ...
    avg_atr_period = 20
    ema_period = (
        50  # 🌙 Moon Dev: Added EMA for trend filter to avoid counter-trend trades
    )
    ...
    def init(self):
        ...
        )  # Volume filter to avoid low-activity setups
        self.ema50 = self.I(
            talib.EMA, close, timeperiod=self.ema_period
        )  # Trend filter for directional bias
    ...
    def next(self):
        ...
        trend_filter = (
            current_adx > 25
        )  # Tightened from 20 to 25 for higher quality trends
        ...
        if (
            ...
            if risk_distance > 0:
                ...
                )  # 🌙 Moon Dev: Allow float size for precise fractional positions
                self.buy(size=size)
        ...
        )  # 🌙 Moon Dev: Only short in downtrend
        if (
            short_rsi_cross
            and short_bb_touch
