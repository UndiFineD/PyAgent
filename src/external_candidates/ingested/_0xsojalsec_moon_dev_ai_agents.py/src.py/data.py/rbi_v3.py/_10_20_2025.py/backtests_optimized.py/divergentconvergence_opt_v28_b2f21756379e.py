# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v3\10_20_2025\backtests_optimized\DivergentConvergence_OPT_v28.py
import numpy as np
import pandas as pd
import talib
...
class DivergentConvergence(Strategy):
    ...
    def next(self):
        ...
        )  # 🌙 Moon Dev Optimization: Tightened RSI threshold to <40 for stronger oversold divergences and increased distance tolerance to <=5 for more flexible signal timing
        if bull_div:
            self.div_low = low2
        ...
        if (
            ...
            and self.data.Volume[-1] > self.vol_sma[-1]
            and self.adx[-1] > 18
            and self.macd[-1] > self.macd[-2]
            and self.data.Close[-1] > self.data.Open[-1]
        ):  # 🌙 Moon Dev Optimization: Lowered ADX to >18 for more trending opportunities, added MACD momentum and bullish candle confirmation for higher-quality, lower-false-positive entries
            ...
            if size > 0:
                ...
                )  # 🌙 Moon Dev Optimization: Removed fixed SL from order to enable manual trailing management
        ...
        if self.position:
            ...
            if (
                rsi_now > 80
            ):  # 🌙 Moon Dev Optimization: Raised RSI exit threshold to >80 to allow more room for overbought conditions in strong trends, letting winners run longer
                self.position.close()
