# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\BandedStochastic_BTFinal_v8.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            if (
                price > upper
                and (
                    self.slowk[-1] < self.slowd[-1] and self.slowk[-2] > self.slowd[-2]
                )
                and slowk < 20
                and slowd < 20
                ...
                if position_size > 0:
                    print(
                        f"🌙 MOON DEV LONG SIGNAL DETECTED! ✨ Price: {price:.2f}, Upper Band: {upper:.2f}, Stoch K: {slowk:.2f}, Stoch D: {slowd:.2f}"
                    )
                ...
                if position_size > 0:
                    print(
                        f"🌙 MOON DEV SHORT SIGNAL DETECTED! ✨ Price: {price:.2f}, Lower Band: {lower:.2f}, Stoch K: {slowk:.2f}, Stoch D: {slowd:.2f}"
                    )
        ...
        if self.position:
            if self.position.is_long:
                if slowk > 90 or price <= self.lower_band[-1]:
                    print(
                        f"🌙 MOON DEV EXIT LONG POSITION! 🚀 Price: {price:.2f}, Stoch K: {slowk:.2f}"
                    )
                    self.position.close()
            else:
                if slowk < 10 or price >= self.upper_band[-1]:
                    print(
                        f"🌙 MOON DEV EXIT SHORT POSITION! 🚀 Price: {price:.2f}, Stoch K: {slowk:.2f}"
                    )
                    self.position.close()
