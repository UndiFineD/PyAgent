# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\VolatilityBreakout_BTFinal_v6.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if low_volatility and breakout_signal and rsi_confirmation:
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙 MOON DEV SHORT SIGNAL! ✨ Price: {price:.2f}, Size: {position_size}, RSI: {self.rsi[-1]:.2f}"
                        )
            ...
            if price >= self.upper[-1]:
                self.position.close()
                print(
                    f"🌙 MOON DEV EXIT SIGNAL! 🚀 Price: {price:.2f}, Profit: {self.position.pl:.2f}"
                )
...
# Run backtest
bt = Backtest(
    data, VolatilityBreakout, commission=0.002, margin=1.0, trade_on_close=True
)
stats = bt.run()
print(stats)
