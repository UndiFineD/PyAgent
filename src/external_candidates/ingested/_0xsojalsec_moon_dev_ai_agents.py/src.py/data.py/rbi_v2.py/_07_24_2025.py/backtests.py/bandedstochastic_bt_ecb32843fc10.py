# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests\BandedStochastic_BT.py
import pandas as pd
import talib
...
class BandedStochastic(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            if (
                price > upper
                and crossover(self.slowk, self.slowd)
                and slowk < 20
                and slowd < 20
                ...
                price < lower
                and crossover(self.slowd, self.slowk)
                and slowk > 80
                and slowd > 80
                and volume > avg_volume
            ):
        ...
        if self.position:
            if self.position.is_long:
                ...
                if slowk < 10 or price >= self.upper_band[-1]:
                    print(
                        f"🌙 MOON DEV EXIT SHORT! 🚀 Price: {price}, Stoch K: {slowk}"
                    )
                    self.position.close()
