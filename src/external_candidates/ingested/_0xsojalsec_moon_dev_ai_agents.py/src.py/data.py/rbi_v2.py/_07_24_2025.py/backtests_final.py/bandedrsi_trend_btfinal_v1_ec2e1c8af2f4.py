# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\BandedRSI_Trend_BTFinal_v1.py
import pandas as pd
import talib
...
class BandedRSITrend(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            current_risk = (
                abs(self.trades[-1].entry_price - self.trades[-1].sl)
                / self.trades[-1].entry_price
            )
            position_size = int(
                round((self.equity * self.risk_per_trade) / (current_risk * price))
            )
        else:
            position_size = int(
                round(self.equity * 0.1 / price)
            )  # Default size if no position
        ...
        if not self.position:
            if uptrend and (
                self.rsi[-2] < self.rsi_lower[-2] and self.rsi[-1] > self.rsi_lower[-1]
            ):
                ...
                self.rsi[-2] > self.rsi_upper[-2] and self.rsi[-1] < self.rsi_upper[-1]
            ):
        ...
        # Exit conditions
            ...
            if self.position.is_long:
                if (
                    self.rsi[-2] > self.rsi_lower[-2]
                    and self.rsi[-1] < self.rsi_lower[-1]
                ) or price < ma:
                ...
                if (
                    ...
                    and self.rsi[-1] > self.rsi_upper[-1]
                ) or price > ma:
                    print(
                        f"🌖 Moon Dev Closing SHORT: RSI={rsi:.2f}, Price={price:.2f}"
                    )
                    self.position.close()
...
if __name__ == "__main__":
    ...
    # Run backtest
    bt = Backtest(data, BandedRSITrend, commission=0.002, exclusive_orders=True)
    stats = bt.run()
    print(stats)
