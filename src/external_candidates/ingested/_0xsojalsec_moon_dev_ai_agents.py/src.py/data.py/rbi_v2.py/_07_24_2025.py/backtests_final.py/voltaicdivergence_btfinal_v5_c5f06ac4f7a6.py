# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\VoltaicDivergence_BTFinal_v5.py
import pandas as pd
import talib
...
class VoltaicDivergence(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and price_high > self.prev_price_high
            and current_atr < self.prev_atr_high
        ):
            if current_close > upper_bb and current_rsi > 50 and not self.position:
                ...
                if position_size > 0:
                    self.buy(
                        ...
                        sl=stop_loss,
                        tp=current_close
                        + (self.rr_ratio * (current_close - stop_loss)),
                    )
                    print(
                        f"🌙 MOON DEV LONG SIGNAL! 🚀 Price: {current_close}, RSI: {current_rsi}, Size: {position_size}"
                    )

        # Short setup: bearish divergence (lower price low but higher volatility)
        elif (
            self.prev_price_low is not None
            and price_low < self.prev_price_low
            and current_atr > self.prev_atr_low
        ):
            if current_close < lower_bb and current_rsi < 50 and not self.position:
                ...
                if position_size > 0:
                    # Fixed: Ensure TP < current price < SL for short orders
                    tp_price = max(
                        0.01,
                        current_close - (self.rr_ratio * (stop_loss - current_close)),
                    )
                    if tp_price < current_close < stop_loss:
                        self.sell(size=position_size, sl=stop_loss, tp=tp_price)
                        print(
                            f"🌙 MOON DEV SHORT SIGNAL! 🌑 Price: {current_close}, RSI: {current_rsi}, Size: {position_size}"
                        )
                    else:
                        print(
                            f"🌙 MOON DEV SHORT ORDER REJECTED! Invalid TP/SL levels (TP: {tp_price}, Current: {current_close}, SL: {stop_loss})"
                        )
        ...
        self.prev_atr_high = (
            current_atr if price_high == self.data.High[-1] else self.prev_atr_high
        )
        self.prev_atr_low = (
            current_atr if price_low == self.data.Low[-1] else self.prev_atr_low
        )
