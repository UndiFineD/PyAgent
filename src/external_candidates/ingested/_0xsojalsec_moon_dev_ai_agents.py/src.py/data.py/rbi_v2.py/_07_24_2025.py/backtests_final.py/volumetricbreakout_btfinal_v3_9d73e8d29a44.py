# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\VolumetricBreakout_BTFinal_v3.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        # Trailing stop for open positions
            ...
            if self.position.is_long:
                ...
                self.position.close(sl=max(new_sl, self.position.sl or 0))
            else:
                new_sl = self.data.Close[-1] + (self.atr[-1] * 1.5)
                self.position.close(sl=min(new_sl, self.position.sl or float("inf")))
