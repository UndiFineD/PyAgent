# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_package\VoltaicBreakout_PKG.py
import pandas as pd
import talib
...
class VoltaicBreakout(Strategy):
    ...
    def next(self):
        if not self.position:
            if (
                self.data.Low[-1] < self.low_previous
                and self.rsi[-1] > self.rsi_previous
            ):
                if (
                    ...
                    if self.bb_upper[-1] > self.bb_upper[-2]:
                        ...
                        print(
                            f"🌙 VoltaicBreakout Entry Signal! Bought {position_size:.2f} units. 🚀"
                        )
        else:
            if (
