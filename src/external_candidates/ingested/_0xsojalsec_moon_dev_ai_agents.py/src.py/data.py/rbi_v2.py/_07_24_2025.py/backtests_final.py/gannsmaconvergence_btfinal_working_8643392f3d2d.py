# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\GannSMAConvergence_BTFinal_WORKING.py
import numpy as np
import pandas as pd
import talib
...
class GannSMAConvergence(Strategy):
    def init(self):
        # Calculate indicators directly from self.data
        ...
        print(
            "🌙 Moon Dev Strategy Initialized: GannSMAConvergence Ready for Launch! 🚀"
        )
    ...
    def next(self):
        ...
        if price > sma and gann_up_angle and not self.position.is_long:
        ...
        elif price < sma and gann_down_angle and not self.position.is_short:
        ...
        if self.position.is_long:
            if (
                self.data.Close[-2] < self.sma5[-2]
                and self.data.Close[-1] > self.sma5[-1]
            ):
        ...
        if self.position.is_short:
            if (
                self.sma5[-2] < self.data.Close[-2]
                and self.sma5[-1] > self.data.Close[-1]
            ):
...
print(
    "🌙 Moon Dev Data Preparation Complete! ✨ Cleaned and formatted data ready for backtesting"
)
