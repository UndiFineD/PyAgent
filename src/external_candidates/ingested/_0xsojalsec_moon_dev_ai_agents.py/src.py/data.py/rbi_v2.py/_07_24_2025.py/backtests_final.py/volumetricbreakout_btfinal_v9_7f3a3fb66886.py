# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\VolumetricBreakout_BTFinal_v9.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        # Trailing stop for open positions
            ...
            if self.position.is_long:
                new_sl = max(
                    self.trades[-1].sl or 0, current_close - (self.atr[-1] * 1.5)
                )
                self.orders.set_sl(new_sl)
            else:
                new_sl = min(
                    self.trades[-1].sl or float("inf"),
