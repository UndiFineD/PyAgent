# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\VolumetricBreakout_BTFinal_v6.py
import pandas as pd
import talib
...
class VolumetricBreakout(Strategy):
    ...
    def next(self):
        ...
        position_size = min(
            risk_amount / stop_distance, 0.99
        )  # Cap at 99% of equity to avoid margin issues
        ...
        if (
            ...
            print(
                f"📈 Price: {entry_price:.2f}, SL: {stop_loss:.2f}, TP: {take_profit_price:.2f}"
            )
        ...
        # Trailing stop for open positions
            ...
            if self.position.is_long:
                new_sl = max(
                    self.position.sl or 0, current_close - (self.atr[-1] * 1.5)
                )
                self.position.update(sl=new_sl)
            else:
                new_sl = min(
                    self.position.sl or float("inf"),
                    current_close + (self.atr[-1] * 1.5),
                )
                self.position.update(sl=new_sl)
...
bt = Backtest(data, VolumetricBreakout, commission=0.002, margin=1.0, cash=100000)
stats = bt.run()
print(stats)
