# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_final\FractalFibonacci_BTFinal_v4.py
import numpy as np
import pandas as pd
import talib
...
class FractalFibonacci(Strategy):
    ...
    def next(self):
        ...
        if self.fib_levels is not None and len(self.data.Close) > 2:
            ...
            if (
                price <= self.fib_levels["38.2"] * 1.005
                and price >= self.fib_levels["38.2"] * 0.995
                and self.rsi[-1] > 30
                ...
                if position_size > 0 and price > stop_loss:
                    print(
                        f"🚀 Moon Dev BUY Signal at {price} | TP: {self.fib_levels['61.8']} | SL: {stop_loss}"
                    )
                    self.buy(
                        size=position_size, sl=stop_loss, tp=self.fib_levels["61.8"]
                    )

            # Check for bearish setup (price at resistance with bearish divergence)
            elif (
                price <= self.fib_levels["61.8"] * 1.005
                and price >= self.fib_levels["61.8"] * 0.995
                and self.rsi[-1] < 70
                and self.rsi[-1] < self.rsi[-2]
                and self.data.Close[-1] < self.data.Open[-1]
            ):
                ...
                if position_size > 0 and price < stop_loss:
                    print(
                        f"🌑 Moon Dev SELL Signal at {price} | TP: {self.fib_levels['38.2']} | SL: {stop_loss}"
                    )
                    self.sell(
                        size=position_size, sl=stop_loss, tp=self.fib_levels["38.2"]
                    )
...
print("✨ Preparing Fractal Fibonacci Strategy...")
print("🚀 Increasing initial cash to handle BTC prices...")
bt = Backtest(
    data, FractalFibonacci, commission=0.002, exclusive_orders=True, cash=1000000
)
stats = bt.run()
print("🌑 Backtest Complete! Moon Dev Results:")
print(stats)
