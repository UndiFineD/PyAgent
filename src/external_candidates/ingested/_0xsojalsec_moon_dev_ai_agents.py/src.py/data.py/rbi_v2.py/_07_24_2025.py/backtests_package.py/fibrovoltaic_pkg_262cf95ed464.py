# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\07_24_2025\backtests_package\FibroVoltaic_PKG.py
import numpy as np
import pandas as pd
import talib
...
class FibroVoltaic(Strategy):
    ...
    def init(self):
        ...
        self.vol_contraction = self.atr < self.atr_sma
    ...
    def next(self):
        ...
        last_swing_low = self.swing_low[-5:].min()
        last_swing_high = self.swing_high[-5:].max()
        ...
        uptrend = (
            current_close > self.ema[-1]
            and self.swing_high[-1] > self.swing_high[-2]
            and self.swing_low[-1] > self.swing_low[-2]
        )
        ...
        if (
            ...
            and in_golden_zone
            and volatility_ok
            and current_close > self.data.Open[-1]
        ):  # Bullish candle
            ...
            if position_size > 0:
                ...
                print(
                    f"✨ Price: {current_close:.2f} | Fib Zone: {fib_50:.2f}-{fib_618:.2f}"
                )
                print(
                    f"🚀 ATR: {self.atr[-1]:.2f} (contracting) | Stop: {self.stop_loss:.2f}"
                )
        ...
        # Exit conditions
            ...
            if current_high >= self.take_profit:
                ...
                print(
                    f"🎯 Target hit at {self.take_profit:.2f} | ATR: {self.atr[-1]:.2f}"
                )
            ...
            # Emergency volatility exit
                ...
                print(
                    f"⚡ ATR expanded >20% | Current: {self.atr[-1]:.2f} vs Entry: {self.entry_atr:.2f}"
                )
            ...
            # Trail stop with 3x ATR
                ...
                if new_stop > self.stop_loss:
                    ...
                    print(
                        f"📈 New stop: {self.stop_loss:.2f} | ATR: {self.atr[-1]:.2f}"
                    )
...
# Run backtest
bt = Backtest(data, FibroVoltaic, commission=0.002, exclusive_orders=True)
stats = bt.run()
print(stats)
