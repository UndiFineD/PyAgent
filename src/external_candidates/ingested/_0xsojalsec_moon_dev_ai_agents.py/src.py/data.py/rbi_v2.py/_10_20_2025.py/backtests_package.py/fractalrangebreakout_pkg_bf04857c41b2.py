# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\10_20_2025\backtests_package\FractalRangeBreakout_PKG.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if len(self.data) >= 5:
            ...
            if middle_idx - 2 >= -len(self.data):
                ...
                if (
                    h_middle > h_left1
                    and h_middle > h_left2
                    and h_middle > h_right1
                    and h_middle > h_right2
                ):
                    self.last_upper = h_middle
                    self.last_upper_bar = len(self.data) - 3
                    print(
                        f"🌙 New Upper Fractal: {h_middle} at bar {self.last_upper_bar} ✨"
                    )
            # Lower fractal check
            if middle_idx - 2 >= -len(self.data):
                ...
                if (
                    l_middle < l_left1
                    and l_middle < l_left2
                    and l_middle < l_right1
                    and l_middle < l_right2
                ):
                    self.last_lower = l_middle
                    self.last_lower_bar = len(self.data) - 3
                    print(
                        f"🌙 New Lower Fractal: {l_middle} at bar {self.last_lower_bar} ✨"
                    )
        ...
        if (
            current_close > self.last_upper
            and current_close >= breakout_long_threshold
            and self.rsi[-1] > 50
            and self.rsi[-1] > self.rsi[-2]
            and current_close > self.ema[-1]
            and current_vol > self.vol_ma[-1]
        ):
            ...
            if sl_distance > 0:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG Entry: {current_close}, Size: {size}, SL: {sl_price}, TP: {tp_price} ✨"
                    )

        # Short entry
        breakout_short_threshold = self.last_lower - 0.5 * self.atr[-1]
        if (
            current_close < self.last_lower
            and current_close <= breakout_short_threshold
            and self.rsi[-1] < 50
            and self.rsi[-1] < self.rsi[-2]
            and current_close < self.ema[-1]
            and current_vol > self.vol_ma[-1]
            ...
            if sl_distance > 0:
                ...
                if size > 0:
                    ...
                    self.entry_bar = current_bar
                    print(
                        f"🌙🚀 SHORT Entry: {current_close}, Size: {size}, SL: {sl_price}, TP: {tp_price} ✨"
                    )
