# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\10_20_2025\backtests\FibonacciDivergence_BT.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciDivergence(Strategy):
    ...
    def is_bullish_divergence(self):
        if len(self.data) < self.prev_lookback:
            return False
        recent_lows = self.data.Low[-self.lookback_div :]
        recent_hist = self.hist[-self.lookback_div :]
        recent_low_idx = np.argmin(recent_lows)
        recent_price_low = recent_lows.iloc[recent_low_idx]
        recent_hist_low = recent_hist.iloc[recent_low_idx]
        ...
        prev_price_low = prev_lows.iloc[prev_low_idx]
        prev_hist_low = prev_hist.iloc[prev_low_idx]
    ...
    def is_bearish_divergence(self):
        ...
        recent_price_high = recent_highs.iloc[recent_high_idx]
        recent_hist_high = recent_hist.iloc[recent_high_idx]

        prev_highs = self.data.High[-self.prev_lookback : -self.lookback_div]
        prev_hist = self.hist[-self.prev_lookback : -self.lookback_div]
        prev_high_idx = np.argmax(prev_highs)
        prev_price_high = prev_highs.iloc[prev_high_idx]
        prev_hist_high = prev_hist.iloc[prev_high_idx]

        return (recent_price_high > prev_price_high) and (
            recent_hist_high < prev_hist_high
        )

    def next(self):
        if len(self.data) < 200 + self.piv_length * 2:
            return
        ...
        if pivot_high_bar >= self.piv_length:
            ...
            if len(right_highs) >= self.piv_length:
                ...
                is_pivot_high = (
                    pivot_high_price >= left_highs.max()
                    and pivot_high_price >= right_highs.max()
                )
                if is_pivot_high and pivot_high_bar > self.last_swing_high_bar:
                    ...
                    if (
                        ...
                        and self.last_swing_low < self.last_swing_high
                    ):
                        self.fib_long_levels = self.calculate_fib_retracement_long(
                            self.last_swing_low, self.last_swing_high
                        )
                    print(
                        f"🌙 Moon Dev: Confirmed Swing High at {pivot_high_price:.2f} on bar {pivot_high_bar} 🚀"
                    )
        ...
        if pivot_low_bar >= self.piv_length:
            ...
            if len(right_lows) >= self.piv_length:
                ...
                is_pivot_low = (
                    pivot_low_price <= left_lows.min()
                    and pivot_low_price <= right_lows.min()
                )
                if is_pivot_low and pivot_low_bar > self.last_swing_low_bar:
                    ...
                    if (
                        ...
                        and self.last_swing_high > self.last_swing_low
                    ):
                        self.fib_short_levels = self.calculate_fib_retracement_short(
                            self.last_swing_high, self.last_swing_low
                        )
                    print(
                        f"🌙 Moon Dev: Confirmed Swing Low at {pivot_low_price:.2f} on bar {pivot_low_bar} ✨"
                    )
        ...
        if (
            not self.position
            and uptrend
            and len(self.fib_long_levels) > 0
            and self.last_swing_high_bar > self.last_swing_low_bar
        ):
            ...
            if close > fib_786_long:  # Avoid deep pullback
                return
            if (
                self.is_near_fib(self.fib_long_levels, close)
                and self.is_bullish_divergence()
            ):
                sl_long = min(fib_786_long, self.last_swing_low) * (
                    1 - 0.01
                )  # 1% beyond
                ...
                if price_risk_long > 0:
                    ...
                    if size_long > 0:
                        ...
                        print(
                            f"🌙 Moon Dev: LONG ENTRY at {close:.2f}, Size: {size_long}, SL: {sl_long:.2f}, TP: {tp_long:.2f} 🚀✨"
                        )
        ...
        if (
            not self.position
            and downtrend
            and len(self.fib_short_levels) > 0
            and self.last_swing_low_bar > self.last_swing_high_bar
        ):
            ...
            if close < fib_786_short:  # Avoid deep rally
                return
            if (
                self.is_near_fib(self.fib_short_levels, close)
                and self.is_bearish_divergence()
            ):
                sl_short = max(fib_786_short, self.last_swing_high) * (
                    1 + 0.01
                )  # 1% beyond
                ...
                if price_risk_short > 0:
                    ...
                    if size_short > 0:
                        ...
                        print(
                            f"🌙 Moon Dev: SHORT ENTRY at {close:.2f}, Size: {size_short}, SL: {sl_short:.2f}, TP: {tp_short:.2f} 🚀✨"
                        )
...
# Run backtest
bt = Backtest(
    data, FibonacciDivergence, cash=1000000, commission=0.001, exclusive_orders=True
)
stats = bt.run()
print(stats)
