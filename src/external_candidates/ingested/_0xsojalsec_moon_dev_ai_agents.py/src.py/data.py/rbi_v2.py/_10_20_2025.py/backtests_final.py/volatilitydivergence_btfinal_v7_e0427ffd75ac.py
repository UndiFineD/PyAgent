# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\10_20_2025\backtests_final\VolatilityDivergence_BTFinal_v7.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityDivergence(Strategy):
    ...
    def init(self):
        ...
        self.squeeze = self.I(
            lambda w, aw: w < 0.5 * aw, self.bb_width, self.avg_bb_width
        )
        ...
        print(
            f"🌙 Moon Dev: Initialized VolatilityDivergence Strategy with talib indicators ✨"
        )

    def next(self):
        if (
            len(self.data)
            < self.sma200_period + self.div_lookback + self.prev_lookback + 1
        ):
            return
        ...
        if self.position:
            ...
            if self.bars_in_trade > self.max_bars_in_trade:
                self.position.close()
                print(
                    f"🌙 Moon Dev: Time-based exit after {self.bars_in_trade} bars 📅"
                )
        ...
        if (
            self.consol_count >= self.consol_bars_min
            and len(self.data) > total_lookback
        ):
            print(
                f"🌙 Moon Dev: Checking divergence after {self.consol_count} consolidation bars 🔍"
            )
            ...
            bull_div = (
                (curr_min_p < prev_min_p)
                and (curr_min_r > prev_min_r)
                and (curr_min_r < 50)
            )
            ...
            bear_div = (
                (curr_max_p > prev_max_p)
                and (curr_max_r < prev_max_r)
                and (curr_max_r > 50)
            )

            if bull_div:
                print(
                    f"🌙 Moon Dev: Bullish divergence detected! curr_min_p={curr_min_p:.2f} < prev_min_p={prev_min_p:.2f}, curr_min_r={curr_min_r:.2f} > prev_min_r={prev_min_r:.2f} 📈"
                )
            if bear_div:
                print(
                    f"🌙 Moon Dev: Bearish divergence detected! curr_max_p={curr_max_p:.2f} > prev_max_p={prev_max_p:.2f}, curr_max_r={curr_max_r:.2f} < prev_max_r={prev_max_r:.2f} 📉"
                )

        between_emas = (
            min(self.ema9[-1], self.ema21[-1])
            <= self.data.Close[-1]
            <= max(self.ema9[-1], self.ema21[-1])
        )
        ...
        if squeeze_ok and flat_ok and between_emas:
            ...
            if self.consol_count == self.consol_bars_min:
                print(
                    f"🌙 Moon Dev: Consolidation threshold reached: {self.consol_count} bars! Ready for breakout 🎯"
                )
        else:
            if self.consol_count >= self.consol_bars_min:
                print(
                    f"🌙 Moon Dev: Consolidation broken after {self.consol_count} bars 💥"
                )
            self.consol_count = 0
        ...
        if (
            ...
            print(
                f"🌙 Moon Dev: All LONG entry conditions met! Vol OK, Above SMA200, Breakout above BB & EMAs ✅"
            )
            atr_val = self.atr[-1]
            entry_price = price
            ...
            if sl_distance > 0:
                ...
                if size > 0:
                    max_affordable = (
                        int(self.cash // entry_price) if entry_price > 0 else 0
                    )
                    ...
                    if size > 0:
                        ...
                        self.bars_in_trade = 0
                        print(
                            f"🌙 Moon Dev: Entering LONG at {entry_price:.2f}, size {size}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀"
                        )
                    else:
                        print(
                            f"🌙 Moon Dev: Size {size} after affordability cap <=0, no LONG trade 😔"
                        )
                else:
                    print(f"🌙 Moon Dev: Calculated size {size} <=0, no LONG trade ❌")
            else:
                print(
                    f"🌙 Moon Dev: Invalid SL distance {sl_distance} <=0 for LONG, no trade ⚠️"
                )
            ...
            print(
                f"🌙 Moon Dev: All SHORT entry conditions met! Vol OK, Below SMA200, Breakdown below BB & EMAs ✅"
            )
            atr_val = self.atr[-1]
            entry_price = price
            sl_price = entry_price + self.sl_atr_mult * atr_val
            ...
            if sl_distance > 0:
                ...
                if size > 0:
                    ...
                    if size > 0:
                        ...
                        self.bars_in_trade = 0
                        print(
                            f"🌙 Moon Dev: Entering SHORT at {entry_price:.2f}, size {size}, SL {sl_price:.2f}, TP {tp_price:.2f} ✨"
                        )
                    else:
                        print(
                            f"🌙 Moon Dev: Size {size} after affordability cap <=0, no SHORT trade 😔"
                        )
                else:
                    print(f"🌙 Moon Dev: Calculated size {size} <=0, no SHORT trade ❌")
            else:
                print(
                    f"🌙 Moon Dev: Invalid SL distance {sl_distance} <=0 for SHORT, no trade ⚠️"
                )
...
print(stats)
print(
    f"🌙 Moon Dev: Backtest completed with talib-powered indicators! Final stats: {stats['Return [%]']:.2f}% 📊✨"
)
