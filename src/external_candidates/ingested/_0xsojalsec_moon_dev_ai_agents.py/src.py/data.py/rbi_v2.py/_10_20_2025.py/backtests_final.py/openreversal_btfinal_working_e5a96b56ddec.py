# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_v2\10_20_2025\backtests_final\OpenReversal_BTFinal_WORKING.py
import pandas as pd
import talib
...
class OpenReversal(Strategy):
    ...
    def init(self):
        ...
        self.sma_vol20 = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.vol_ma_period
        )
        # Removed unused prev_vol as talib.SHIFT does not exist; using direct indexing in next() instead
    ...
    def next(self):
        ...
        high_vol_trigger = self.data.Volume[-1] > (
            self.vol_threshold * self.sma_vol20[-1]
        )
        ...
        if in_decline and high_vol_trigger and not self.position:
            ...
            print(
                f"🌙 Moon Dev Entry: OpenReversal Long at {entry_price:.2f} BTC | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f} 🚀✨"
            )

        # Exit on "OI reversal": Volume increase >5% (proxy for OI rise signaling renewed pressure)
        if self.position and self.data.Volume[-1] > (
            self.oi_reversal_threshold * self.data.Volume[-2]
        ):
            self.position.close()
...
bt = Backtest(
    data, OpenReversal, cash=10000000, commission=0.001, exclusive_orders=True
)
