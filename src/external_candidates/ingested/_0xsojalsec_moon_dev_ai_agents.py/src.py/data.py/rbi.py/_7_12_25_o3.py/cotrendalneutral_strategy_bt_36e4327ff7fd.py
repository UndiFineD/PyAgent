# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_o3\CoTrendalNeutral_strategy_bt.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long and (
                price - self.position.entry_price >= rr_target or atr_high
            ):
                print("🌙 Moon Dev CoTrendalNeutral closing LONG ✨")
                self.position.close()
            elif self.position.is_short and (
                self.position.entry_price - price >= rr_target or atr_low
            ):
