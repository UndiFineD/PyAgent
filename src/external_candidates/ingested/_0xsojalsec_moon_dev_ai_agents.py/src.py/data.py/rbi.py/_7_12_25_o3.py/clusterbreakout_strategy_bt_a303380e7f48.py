# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_o3\ClusterBreakout_strategy_bt.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long:
                if (
                    price - self.position.entry_price >= 2 * atr_val
                    or price < low_level
                ):
                ...
                if (
                    self.position.entry_price - price >= 2 * atr_val
                    or price > high_level
                ):
