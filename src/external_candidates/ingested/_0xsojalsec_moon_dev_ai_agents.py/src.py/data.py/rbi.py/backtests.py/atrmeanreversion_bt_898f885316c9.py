# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\ATRMeanReversion_BT.py
#!/usr/bin/env python3
"""
...
def load_and_clean_data(file_path):
    ...
    if not all(col in df.columns for col in required_columns):
        raise ValueError(
            f"Missing required columns. Data must have columns: {required_columns}"
        )
...
class ATRMeanReversion(Strategy):
    ...
    def init(self):
        ...
        self.kc_middle = self.I(
            lambda: pd.Series(self.data.Close).rolling(self.kelter_period).mean()
        )
    ...
    def next(self):
        ...
        print(
            f"🌙✨ [BAR] Current Candle >> Open: {curr_open:.2f}, High: {self.data.High[-1]:.2f}, "
            f"Low: {self.data.Low[-1]:.2f}, Close: {curr_close:.2f}"
        )

        # Print Keltner Channel levels
        print(
            f"🌙✨ [LEVELS] Upper: {self.kc_upper[-1]:.2f}, Lower: {self.kc_lower[-1]:.2f}, "
        ...
        try:
            ...
            if (
                ...
                and curr_close < self.kc_lower[-1]
            ):
                ...
                position_size = max(
                    int(risk_amount / risk_per_unit), self.min_position_size
                )

                print(
                    f"🌙🚀 [LONG ENTRY] Moon Dev spotted a Bullish Signal! Entry: {entry_price:.2f}"
                )
                print(
                ...
                print(
                    f"   ➡ Risk Amount: ${risk_amount:.2f}, Risk per unit: ${risk_per_unit:.2f}"
                )
            ...
            # Short Entry Conditions
                ...
                not self.position
                and curr_close < curr_open
                and curr_close > self.kc_upper[-1]
            ):
                # Calculate entry parameters
                entry_price = curr_close
                stop_loss = entry_price + atr
                ...
                print(
                    f"🌙🚀 [SHORT ENTRY] Moon Dev spotted a Bearish Signal! Entry: {entry_price:.2f}"
                )
                print(
                    f"   ➡ Stop Loss: {stop_loss:.2f}, Take Profit: {take_profit:.2f}"
                )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, ATRMeanReversion, cash=1_000_000, commission=0.002)
    ...
    try:
        ...
        chart_file = os.path.join(
            charts_dir, f"ATRMeanReversion_chart_{timestamp}.html"
        )
