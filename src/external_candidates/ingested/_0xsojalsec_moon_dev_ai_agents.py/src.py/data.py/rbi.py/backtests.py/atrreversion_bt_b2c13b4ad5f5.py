# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\AtrReversion_BT.py
#!/usr/bin/env python3
"""
...
class AtrReversion(Strategy):
    ...
    def init(self):
        ...
        self.atr = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, self.atr_period
        )
        ...
        self.keltner_upper = self.I(
            lambda ema, atr: ema + self.keltner_mult * atr, self.ema, self.atr
        )
        self.keltner_lower = self.I(
            lambda ema, atr: ema - self.keltner_mult * atr, self.ema, self.atr
        )
        print(
            "🌙 [INIT] Indicators set up: ATR period =",
            self.atr_period,
            ", EMA period =",
            self.ema_period,
            ", Keltner Multiplier =",
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_short and C < self.ema[-1]:
                print(
                    "🌙💥 [EXIT] Short position closed! Price has reverted near the EMA midline."
                )
                self.position.close()
            # For long positions, exit when price rises above the EMA.
            elif self.position.is_long and C > self.ema[-1]:
                print(
                    "🌙💥 [EXIT] Long position closed! Price has reverted near the EMA midline."
                )
                self.position.close()
        ...
        risk_amount = (
            account_size * self.risk_percent * self.trade_fraction
        )  # e.g., 1% risk half-sized
        ...
        if prev_close > prev_keltner_upper and C < O:
            ...
            print(
                f"🌙🔥 [SIGNAL] SHORT candidate detected! Prev Close {prev_close:.2f} > Prev KC Upper {prev_keltner_upper:.2f} and current bearish candle (O {O:.2f} > C {C:.2f})"
            )
            if risk_per_unit > 0:
                position_size = int(round(risk_amount / risk_per_unit))
                if position_size > 0:
                    print(
                        f"🌙💰 [ORDER] Enter SHORT: entry={entry_price:.2f}, stop_loss={stop_loss:.2f}, risk_per_unit={risk_per_unit:.2f}, size={position_size}"
                    )
                    # Place a short order with a protective stop loss.
                    self.sell(size=position_size, sl=stop_loss)
                else:
                    print(
                        "🌙⚠️ [SKIP] Calculated position size is 0. Check risk parameters!"
                    )
            else:
        ...
        # We then place a buy stop order above the reversal candle.
            ...
            print(
                f"🌙🔥 [SIGNAL] LONG candidate detected! Prev Close {prev_close:.2f} < Prev KC Lower {prev_keltner_lower:.2f} and current bullish candle (C {C:.2f} > O {O:.2f})"
            )
            if risk_per_unit > 0:
                position_size = int(round(risk_amount / risk_per_unit))
                if position_size > 0:
                    print(
                        f"🌙💰 [ORDER] Enter LONG: entry={entry_price:.2f}, stop_loss={stop_loss:.2f}, risk_per_unit={risk_per_unit:.2f}, size={position_size}"
                    )
