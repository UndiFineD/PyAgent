# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\DynamicRetest_BT.py
#!/usr/bin/env python3
"""
    ...
    def next(self):
        ...
        if trend == "up" and current_close > current_open:
            ...
            if curr_zone_bottom <= current_close <= curr_zone_top:
                ...
                recent_range = (
                    self.data.High[-self.consolidation_span :].max()
                    - self.data.Low[-self.consolidation_span :].min()
                )
                if take_profit > current_close + (2 * recent_range):
                    print(
                        f"🚀 Moon Dev: TP target ({take_profit:.2f}) too far from current price. LONG trade skipped!"
                    )
                    return
                ...
                print(
                    f"🌙🚀 Moon Dev: LONG signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size}"
                )
        ...
        if trend == "down" and current_close < current_open:
            ...
            if curr_zone_bottom <= current_close <= curr_zone_top:
                ...
                if take_profit < current_close - (2 * recent_range):
                    print(
                        f"🚀 Moon Dev: TP target ({take_profit:.2f}) too far from current price. SHORT trade skipped!"
                    )
                    return
                ...
                print(
                    f"🌙🚀 Moon Dev: SHORT signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size}"
                )
