# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\RSIDivergence_BT.py
#!/usr/bin/env python3
"""
...
class RSIDivergence(Strategy):
    ...
    def init(self):
        ...
        self.atr = self.I(
            lambda: talib.ATR(
                high_values, low_values, close_values, timeperiod=self.atr_period
            )
        )

        # Find divergences
        self.bullish_div, self.bearish_div = self.I(
            lambda: find_divergences(close_values, self.rsi, self.divergence_window)
        )
    ...
    def next(self):
        ...
        print(
            f"🌙✨ [BAR] Current Candle >> Open: {self.data.Open[-1]:.2f}, "
        ...
        try:
            ...
            if (
                ...
                print(
                    f"🌙🚀 [LONG ENTRY] Moon Dev spotted a Bullish Divergence! Entry: {entry_price:.2f}"
                )
                print(
            ...
            # Short Entry on Bearish Divergence
                ...
                print(
                    f"🌙🚀 [SHORT ENTRY] Moon Dev spotted a Bearish Divergence! Entry: {entry_price:.2f}"
                )
                print(
                    f"   ➡ Stop Loss: {stop_loss:.2f}, Take Profit: {take_profit:.2f}"
                )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, RSIDivergence, cash=1_000_000, commission=0.002)
