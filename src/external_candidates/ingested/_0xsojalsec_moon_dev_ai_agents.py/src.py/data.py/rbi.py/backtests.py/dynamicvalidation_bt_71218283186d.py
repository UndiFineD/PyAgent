# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\DynamicValidation_BT.py
#!/usr/bin/env python3
"""
...
class DynamicValidation(Strategy):
    ...
    def init(self):
        ...
        self.supply_zone = self.I(
            talib.MAX, self.data.High, timeperiod=self.swing_period
        )
        ...
        if not self.position:
            ...
            if self.data.Close[-1] <= self.demand_zone[-1] * 1.01:
                ...
                if risk > 0:
                    ...
                    position_size = max(
                        1, int(risk_amount / risk)
                    )  # Ensure minimum size of 1
                    take_profit = entry_price + (risk * self.risk_reward_ratio)

                    self.buy(size=position_size, sl=stop_loss, tp=take_profit)
                    print(
                        f"🌙🚀 LONG Entry: Price={entry_price:.2f}, SL={stop_loss:.2f}, TP={take_profit:.2f}, Size={position_size}"
                    )
            ...
            # SHORT setup
                ...
                if risk > 0:
                    ...
                    print(
                        f"🌙🚀 SHORT Entry: Price={entry_price:.2f}, SL={stop_loss:.2f}, TP={take_profit:.2f}, Size={position_size}"
                    )
...
if __name__ == "__main__":
    ...
    initial_chart_file = os.path.join(
