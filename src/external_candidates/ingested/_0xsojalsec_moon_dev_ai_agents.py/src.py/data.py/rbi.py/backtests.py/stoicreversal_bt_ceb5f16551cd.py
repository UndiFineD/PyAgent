# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\StoicReversal_BT.py
#!/usr/bin/env python3
"""
...
class StoicReversal(Strategy):
    ...
    stoch_oversold = (
        20  # When the STOCH RSI %K drops below this, a long entry is considered.
    )
    stoch_overbought = (
        80  # When the STOCH RSI %K goes above this with a confirmation crossover, sell.
    )
    ...
    def init(self):
        ...
        self.stoch_k = self.I(
            lambda: talib.STOCHRSI(
                ...
                fastd_matype=self.stoch_fastd_matype,
            )[0],
            name="stoch_k",
        )
        self.stoch_d = self.I(
        ...
        if not self.position:
            ...
            if self.stoch_k[-1] < self.stoch_oversold:
                ...
                take_profit_price = price * (
                    1 + self.risk_reward_ratio * self.stop_loss_pct
                )
                ...
                print(
                    f"   ➤ Stop Loss set at {stop_loss_price:.2f}, Take Profit at {take_profit_price:.2f}"
                )
                print(
                    f"   ➤ Calculated position size: {position_size:.4f} (risk_amount: {risk_amount:.2f})"
                )
            ...
            if len(self.stoch_k) >= 2:
                ...
                if (
                    ...
                    if self.stoch_k[-1] > self.stoch_overbought:
                        print(
                            f"🌙✨ [EXIT] SELL signal triggered at price {price:.2f}!"
                        )
                        print(
                            "   ➤ STOCH RSI crossover and overbought condition confirmed!"
                        )
                        self.position.close()
...
if __name__ == "__main__":
    ...
    print(
        "🌙✨ [DATA] Data preprocessing complete. Required columns: 'Open', 'High', 'Low', 'Close', 'Volume'."
    )
