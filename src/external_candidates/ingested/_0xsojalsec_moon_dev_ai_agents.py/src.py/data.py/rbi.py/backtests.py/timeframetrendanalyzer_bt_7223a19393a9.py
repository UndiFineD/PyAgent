# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\TimeframeTrendAnalyzer_BT.py
#!/usr/bin/env python3
"""
...
class TimeframeTrendAnalyzer(Strategy):
    ...
    def init(self):
        ...
        self.fourhour_data = self.data.resample(
            "4H", closed="right", label="right"
        ).agg(
            {
                "Open": "first",
                "High": "max",
        ...
        self.fiftymin_data = self.data.resample(
            "50T", closed="right", label="right"
        ).agg(
            {
                "Open": "first",
                "High": "max",
        ...
        print(
            "🌙✨ [INIT] Aggregated weekly, daily, 4H, 1H, and 50min data computed! 🚀"
        )
    ...
    def determine_trend(self, df, current_time):
        ...
        if prev_bar is None or last_bar is None:
            return None
        if (last_bar["High"] > prev_bar["High"]) and (
            last_bar["Low"] > prev_bar["Low"]
        ):
            trend = "bullish"
        elif (last_bar["High"] < prev_bar["High"]) and (
            last_bar["Low"] < prev_bar["Low"]
        ):
            trend = "bearish"
        else:
            trend = "sideways"
        print(
            "🌙✨ [DEBUG] Trend determined: {0} | Prev High: {1}, Last High: {2}".format(
    ...
    def get_fiftymin_breakout(self, current_time, direction):
        ...
        if direction == "bullish":
            if last_bar["Close"] > prev_bar["High"]:
                print(
                    "🌙🚀 [DEBUG] 50min breakout detected for LONG! Prev High: {0}, Last Close: {1}".format(
            ...
            if last_bar["Close"] < prev_bar["Low"]:
                print(
                    "🌙🚀 [DEBUG] 50min breakout detected for SHORT! Prev Low: {0}, Last Close: {1}".format(
    ...
    def next(self):
        ...
        if fourhr_trend != "sideways" and fourhr_trend is not None:
            ...
            if onehr_trend != "sideways" and onehr_trend is not None:
                ...
                print(
                    "🌙✨ [DEBUG] 4H trend sideways. Using 1H trend:", trend_direction
                )
            else:
        ...
        print(
            "🌙🚀 [SIGNAL] {0} entry signal! Entry: {1:.2f}, Stop: {2:.2f}, TP: {3:.2f}, Risk: {4:.2f}, Position Size: {5}".format(
...
if __name__ == "__main__":
    ...
    # ---------------------------
    optimized_chart_file = os.path.join(
        chart_dir, f"{strategy_name}_optimized_chart.html"
    )
