...
class ValidatedBreakthrough(Strategy):
    ...
    lookback = (
        20  # Lookback period for swing highs/lows (optimize e.g., range(15,30,5))
    )
    ...
    def next(self):
        ...
        print(
            "🚀 [Moon Dev] New bar processed – Current Price: {:.2f}".format(
                current_price
            )
        )
        ...
        if not self.position:
            ...
            if current_price > self.sma[-1]:
                print(
                    "✨ [Moon Dev] Uptrend detected – Close {:.2f} > SMA {:.2f}".format(
                ...
                if current_price <= self.recent_low[-1] * (1 + self.zone_buffer):
                    ...
                    print(
                        "     • Calculated Risk: {:.2f} | Reward: {:.2f}".format(
                            risk, reward
                        )
                    )
                    ...
                    if risk <= 0:
                        print(
                            "🚀 [Moon Dev] ERROR: Non‐positive risk detected. Skipping LONG trade!"
                        )
                        ...
                        if position_size <= 0:
                            print(
                                "🚀 [Moon Dev] Calculated position size is 0. Trade skipped!"
                            )
                        else:
                            print(
                                "🌙 [Moon Dev] Placing LONG order with size: {}".format(
                                    position_size
                                )
                            )
                    ...
                    print(
                        "🌙 [Moon Dev] Price not within demand zone threshold for LONG entry."
                    )
            ...
            # -------------------
                ...
                print(
                    "✨ [Moon Dev] Downtrend detected – Close {:.2f} < SMA {:.2f}".format(
                        current_price, self.sma[-1]
                    )
                )
                ...
                if current_price >= self.recent_high[-1] * (1 - self.zone_buffer):
                    ...
                    if risk <= 0:
                        print(
                            "🚀 [Moon Dev] ERROR: Non‐positive risk detected. Skipping SHORT trade!"
                        )
                    elif reward / risk < self.min_rr:
                        print(
                        ...
                        if position_size <= 0:
                            ...
                            print(
                                "🌙 [Moon Dev] Placing SHORT order with size: {}".format(
                                    position_size
                                )
                            )
                    ...
                    print(
                        "🌙 [Moon Dev] Price not within supply zone threshold for SHORT entry."
                    )
        else:
            print(
                "🌙 [Moon Dev] Active position exists – Monitoring trade exit conditions..."
            )
...
if __name__ == "__main__":
    ...
    print(
        "🌙 [Moon Dev] Data cleaning complete – Columns: {}".format(list(data.columns))
    )
    ...
    print(
        "🚀 [Moon Dev] Saving initial performance chart to: {}".format(
            initial_chart_file
        )
    )
    ...
    optimized_chart_file = os.path.join(
        charts_dir, f"{strategy_name}_optimized_chart.html"
    )
    print(
        "🚀 [Moon Dev] Saving optimized performance chart to: {}".format(
            optimized_chart_file
        )
    )
    optimized_bt.plot(filename=optimized_chart_file, open_browser=False)

    print(
        "✨🌙 [Moon Dev] Backtest execution complete – Keep reaching for the moon! 🚀"
    )
