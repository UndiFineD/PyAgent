...
class GapAdvantage(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙✨ [INIT] Indicators loaded: SMA9, SMA50, and VWAP calculated via custom_vwap()!"
        )
    ...
    def next(self):
        ...
        print(
            f"🌙🚀 [NEXT] Price: {price:.2f}, VWAP: {current_vwap:.2f}, SMA9: {current_sma9:.2f}"
        )
        ...
        if not self.position:
            ...
            if (
                ...
                print(
                    f"🌙✨ [ENTRY SIGNAL] Price crossed above VWAP! Entry Price: {self.entry_price:.2f}"
                )
                print(
            ...
            if price <= self.sl:
                print(
                    f"🌙🚀 [EXIT SIGNAL] Price hit stop loss! Current Price: {price:.2f}, SL: {self.sl:.2f}"
                )
                self.position.close()
            elif price >= self.tp:
                print(
                    f"🌙🚀 [EXIT SIGNAL] Price hit take profit! Current Price: {price:.2f}, TP: {self.tp:.2f}"
                )
                self.position.close()
...
if __name__ == "__main__":
    ...
    bt = Backtest(
        data, GapAdvantage, cash=1000000, commission=0.0005, exclusive_orders=True
    )
