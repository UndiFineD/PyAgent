# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\MACDCrossover_BT.py
#!/usr/bin/env python
"""
    ...
    def next(self):
        ...
        if (
            ...
            if pos_size > 0 and stop_loss < price < take_profit:
                print(
                    f"🌙🚀 [LONG ENTRY] Moon Dev spotted a Bullish MACD Cross! Entry: {price:.2f}"
                )
                print(
        ...
        # Short Entry: MACD crosses below Signal
            ...
            if pos_size > 0 and take_profit < price < stop_loss:
                print(
                    f"🌙🚀 [SHORT ENTRY] Moon Dev spotted a Bearish MACD Cross! Entry: {price:.2f}"
                )
                print(
                    f"   ➡ Stop Loss: {stop_loss:.2f}, Take Profit: {take_profit:.2f}"
                )
...
if __name__ == "__main__":
    ...
    # Run backtest
    bt = Backtest(df, MACDCrossover, cash=100000, commission=0.002)
    stats = bt.run()
    ...
    stats_file = os.path.join(
        STATS_DIR,
        f"MACDCrossover_stats_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.txt",
    )
    ...
    chart_file = os.path.join(
        CHARTS_DIR,
        f"MACDCrossover_chart_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.html",
    )
