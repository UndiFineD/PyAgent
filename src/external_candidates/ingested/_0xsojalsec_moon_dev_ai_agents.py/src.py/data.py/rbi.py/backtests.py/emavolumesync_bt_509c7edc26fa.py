# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\EMAVolumeSync_BT.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if is_uptrend and volume_confirmation:
            if not self.position:
                ...
                stop_loss = min(
                    self.red_ema[-1], current_close * 0.98
                )  # At least 2% below entry
                risk_amount = self.risk_per_trade * self.equity
                position_size = risk_amount / abs(current_close - stop_loss)
                position_size = max(
                    1, round(position_size / current_close)
                )  # Convert to whole units

                # For longs, take profit must be above entry price
                take_profit = current_close * (
                    1 + (self.risk_reward_ratio * 0.02)
                )  # 2% * RR above entry

                # Enter long trade
                self.buy(size=position_size, sl=stop_loss, tp=take_profit)
                print(
                    f"🚀 Long Entry at {current_close} | SL: {stop_loss} | TP: {take_profit}"
                )
        ...
        # Entry logic for short trades
            ...
            if not self.position:
                ...
                stop_loss = max(
                    self.green_ema[-1], current_close * 1.02
                )  # At least 2% above entry
                risk_amount = self.risk_per_trade * self.equity
                ...
                print(
                    f"📉 Short Entry at {current_close} | SL: {stop_loss} | TP: {take_profit}"
                )
