# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\AccumulationManipulation_BT.py
#!/usr/bin/env python3
"""
...
class AccumulationManipulation(Strategy):
    ...
    def next(self):
        ...
        if not (time(10, 0) <= current_time <= time(11, 30)):
            ...
            print(
                f"✨ {current_dt} - Outside trading window (10:00-11:30). No action taken."
            )
            return
        ...
        if self.position:
            ...
            print(
                f"🚀 {current_dt} - Position open. Monitoring... Current PnL: {self.position.pl}"
            )
            return
        ...
        if all(
            earlier < later for earlier, later in zip(recent_closes, recent_closes[1:])
        ):
            market_bias = "up"
        elif all(
            earlier > later for earlier, later in zip(recent_closes, recent_closes[1:])
            ...
            market_bias = "neutral"
        print(
            f"🚀 {current_dt} - Market bias determined as: {market_bias.upper()} (from last 1H data)"
        )
        ...
        print(
            f"✨ {current_dt} - Accumulation check: range={accumulation_range:.2f}, threshold={accum_threshold:.2f} -> {is_accumulation}"
        )
        ...
        if not manipulation:
            ...
            print(
                f"🌙 {current_dt} - No valid manipulation detected. Waiting for signal..."
            )
            return
        ...
        if risk <= 0:
            print(
                f"🚀 {current_dt} - Invalid risk calculation (risk <= 0). Skipping trade."
            )
            return
        ...
        print(
            ...
            + (
                ...
                if direction == "long"
                else "🚀 SHORT SIGNAL detected!"
            )
        )
        print(f"✨ {current_dt} - Entry Price: {entry_price:.2f}")
        print(
            f"✨ {current_dt} - Stop Loss: {stop_loss:.2f} | Risk per Unit: {risk:.2f}"
        )
        ...
        if direction == "long":
            ...
            print(
                f"🌙 🚀 Placed LONG order: Size={position_size} @ {entry_price:.2f} (SL: {stop_loss:.2f}, TP: {take_profit:.2f})"
            )
        else:
            self.sell(size=position_size, sl=stop_loss, tp=take_profit)
            print(
                f"🌙 🚀 Placed SHORT order: Size={position_size} @ {entry_price:.2f} (SL: {stop_loss:.2f}, TP: {take_profit:.2f})"
            )
    ...
    def notify_trade(self, trade):
        ...
        if trade.isclosed:
            print(
                f"🌙 🚀 Trade closed on {self.data.index[-1]}. PnL: {trade.pl:.2f} | Net Profit: {trade.pnl:.2f}"
            )
...
bt = Backtest(
    ...
    commission=0.000,  # No commission for simplicity
