# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests\BollingerBands_BT.py
#!/usr/bin/env python
"""
...
class BollingerBandsStrategy(Strategy):
    ...
    def next(self):
        ...
        required_bars = max(
            self.n_sma, self.n_rsi, self.n_atr, 3
        )  # Need at least 3 bars for momentum
        if len(self.data) < required_bars:
            return

        # Skip if any indicator is NaN
        if (
            np.isnan(self.upper[-1])
            or np.isnan(self.lower[-1])
        ...
        if (
            ...
            if (
                pos_size > 0 and not self.position
            ):  # Check we're not already in a position
                print(
                    f"🌙🚀 [LONG ENTRY] Moon Dev spotted a Bullish BB Signal! Entry: {entry:.2f}"
                )
                print(
        ...
        # For short positions - price above upper band + RSI overbought + momentum confirmation
            ...
            if (
                ...
                print(
                    f"🌙🚀 [SHORT ENTRY] Moon Dev spotted a Bearish BB Signal! Entry: {entry:.2f}"
                )
                print(
                    f"   ➡ Stop Loss: {stop_loss:.2f}, Take Profit: {take_profit:.2f}"
                )
...
if __name__ == "__main__":
    ...
    # Run backtest
    bt = Backtest(data, BollingerBandsStrategy, cash=1_000_000, commission=0.002)
    stats = bt.run()
