# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_05_2025\backtests_package\VolatilityBackwardation_PKG.py
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.data.High[-1] >= take_profit:
                self.position.close()
                print(
                    f"🌕 Moon Profit Achieved! Closed at {self.data.Close[-1]} (▲ {2*entry_atr:.2f}) 🌙"
                )
            return
        ...
        if current_atr < atr_threshold and vix_confirm and current_atr != 0:
            ...
            if position_size > 0:
                ...
                print(
                    f"🚀 Lunar Launch! Long {position_size} units ▲▲▲ "
