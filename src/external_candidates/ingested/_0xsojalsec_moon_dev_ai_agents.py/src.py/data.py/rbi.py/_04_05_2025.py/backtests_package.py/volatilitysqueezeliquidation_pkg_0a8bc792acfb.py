# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_05_2025\backtests_package\VolatilitySqueezeLiquidation_PKG.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if current_width > self.exit_threshold[-1]:
                self.position.close()
                print(
                    f"🌙✨ Moon Dev Alert! Closing position - Volatility expanded to {current_width:.4f}"
                )
            ...
            if in_squeeze and high_funding and price > self.upper[-1]:
                ...
                self.buy(size=position_size, sl=sl, tp=price + 2 * (price - sl))
                print(
                    f"🌕🚀 Moon Dev Launch! LONG position activated | Size: {position_size} | Entry: {price:.2f}"
                )
            ...
            # Short entry logic
                ...
                self.sell(size=position_size, sl=sl, tp=price - 2 * (sl - price))
                print(
                    f"🌑🚀 Moon Dev Dark Launch! SHORT position activated | Size: {position_size} | Entry: {price:.2f}"
                )


# Run backtest with Moon Dev parameters
bt = Backtest(data, VolatilitySqueezeLiquidation, cash=1_000_000, commission=0.002)
stats = bt.run()
