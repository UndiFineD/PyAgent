# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_05_2025\backtests_package\VolatilitySqueezeTrigger_PKG.py
import numpy as np
import pandas as pd
...
class VolatilitySqueezeTrigger(Strategy):
    def init(self):
        ...
        self.bb_low = self.I(
            talib.MIN, self.bb_width, timeperiod=126, name="BB_LOW_126"
        )
    ...
    def next(self):
        ...
        kvo_cross = (self.kvo[-2] > self.kvo_sma[-2]) and (
            self.kvo[-3] <= self.kvo_sma[-3]
        )
