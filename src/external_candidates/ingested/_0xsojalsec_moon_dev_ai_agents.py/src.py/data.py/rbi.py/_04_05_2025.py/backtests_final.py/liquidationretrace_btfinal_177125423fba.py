# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_05_2025\backtests_final\LiquidationRetrace_BTFinal.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
...
class LiquidationRetrace(Strategy):
    ...
    def init(self):
        ...
        self.data.df = self.data.df.drop(
            columns=[col for col in self.data.df.columns if "unnamed" in col]
        )
        ...
        self.swing_low = self.I(
            talib.MIN, self.data.Low, timeperiod=self.swing_period, name="SWING LOW 🚀"
        )
        self.oi_pct_change = self.I(
            lambda x: x.pct_change(4), self.data.df["oi"], name="OI %Δ ⚡"
        )  # 4-period OI change
    ...
    def next(self):
        ...
        entry_conditions = (
            oi_change <= self.oi_drop_threshold
            and funding_rate <= self.funding_threshold
            and self.data.Close[-1] > self.data.Open[-1]  # Bullish candle
        ...
        for trade in self.trades:
            if trade.is_long:
                if self.data.Close[-1] >= trade.tp:
                    trade.close()
                    print(
                        f"🌕 PROFIT CAPTURED AT MIDLINE! Closing @ {self.data.Close[-1]:.2f}"
                    )
                elif self.data.Close[-1] <= trade.sl:
                    print(
                        f"🌑 LIQUIDATION AVOIDANCE! Stop Loss @ {self.data.Close[-1]:.2f}"
                    )
