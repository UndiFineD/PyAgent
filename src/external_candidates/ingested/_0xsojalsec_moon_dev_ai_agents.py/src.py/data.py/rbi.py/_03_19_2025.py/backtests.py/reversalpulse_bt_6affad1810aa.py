# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_19_2025\backtests\ReversalPulse_BT.py
...
class ReversalPulse(Strategy):
    ...
    def init(self):
        ...
        self.ema200 = self.I(
            talib.EMA, self.data.Close, timeperiod=200, name="EMA 200 🚀"
        )
        self.adx = self.I(
            talib.ADX,
            self.data.High,
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if crossover(self.ema50, self.ema200) and current_adx < 20:
                sl = self.swing_low[-1]
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 ENTRY LONG: {position_size} units at {current_close:.2f}"
                        )
                        print(
                            f"   SL: {sl:.2f} | Risk: {risk_per_share:.2f} | ADX: {current_adx:.2f} ✨"
                        )
            ...
            if current_adx > 40:
                position.close()
                print(
                    f"💎🌙 EXIT (ADX >40): {current_close:.2f} | ADX: {current_adx:.2f} 🚀"
                )
            elif crossover(self.ema200, self.ema50):
                position.close()
                print(
                    f"💎🌙 EXIT (Death Cross): {current_close:.2f} | EMA50: {self.ema50[-1]:.2f} 🚀"
                )
...
bt = Backtest(data, ReversalPulse, cash=1_000_000, commission=0.002)
stats = bt.run()
