# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_19_2025\backtests\VolumeSurgeEMA_BT.py
...
class VolumeSurgeEMA(Strategy):
    ...
    def init(self):
        ...
        self.ema50 = self.I(
            talib.EMA, self.data.Close, self.ema_period_short, name="EMA50"
        )
        self.ema200 = self.I(
            talib.EMA, self.data.Close, self.ema_period_long, name="EMA200"
        )
        self.volume_sma = self.I(
            talib.SMA, self.data.Volume, self.volume_ma_period, name="Volume SMA"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if len(self.rsi) >= 2 and self.rsi[-2] >= 70 and self.rsi[-1] < 70:
                ...
                print(
                    f"🌙✨ Moon Dev Exit Signal! RSI {self.rsi[-1]:.1f} dropping below 70"
                )
            ...
            if len(self.ema50) >= 2 and len(self.ema200) >= 2:
                ...
                if ema_cross and volume_spike and len(self.swing_low) > 0:
                    ...
                    if entry_price > stop_price:
                        ...
                        position_size = int(
                            round((self.equity * self.risk_pct) / risk_per_share)
                        )
...
bt = Backtest(data, VolumeSurgeEMA, cash=1_000_000, commission=0.002)
stats = bt.run()
