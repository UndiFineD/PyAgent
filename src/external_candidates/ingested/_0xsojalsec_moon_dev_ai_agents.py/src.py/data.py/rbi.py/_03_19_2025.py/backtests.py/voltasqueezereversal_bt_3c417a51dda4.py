# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_19_2025\backtests\VoltaSqueezeReversal_BT.py
    ...
    def init(self):
        ...
        def bb_width(close):
            upper, _, lower = talib.BBANDS(
                close, timeperiod=self.bb_period, nbdevup=2, nbdevdn=2
            )
            return upper - lower
        ...
        if not self.position:
            ...
            if squeeze_condition and rsi_above:
                ...
                if position_size > 0:
                    ...
                    self.buy(
                        size=position_size, sl=sl, tp=tp, tag="🌙 VOLTA SQUEEZE ENTRY"
                    )

                    print(
                        f"🌕 MOON DEV SIGNAL 🌕 | Entry: {entry_price:.2f} | "
...
# 🚀 RUN BACKTEST
bt = Backtest(data, VoltaSqueezeReversal, cash=1_000_000, commission=0.002)
stats = bt.run()
