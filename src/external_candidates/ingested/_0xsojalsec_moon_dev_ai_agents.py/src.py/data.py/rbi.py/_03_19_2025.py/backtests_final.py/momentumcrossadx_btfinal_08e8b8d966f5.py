# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_19_2025\backtests_final\MomentumCrossADX_BTFinal.py
import pandas as pd
import talib
...
class MomentumCrossADX(Strategy):
    ...
    def next(self):
        # Skip if not enough data (waiting for moon cycle completion) 🌑
                ...
                if adx_current > 25 and adx_current > adx_prev:
                    ...
                    if risk_per_share > 0:
                        ...
                        if position_size > 0:
                            ...
                            print(
                                f"🌙✨ EMA Crossover CONFIRMED! 🚀 Long entry at {entry_price:.2f} (Size: {position_size})"
                            )
        ...
        # Exit supernova conditions 💫
            ...
            if self.rsi[-1] > 70:
                self.position.close()
                print(
                    f"🚨 RSI OVERHEATING! {self.rsi[-1]:.1f} - Exiting at {self.data.Close[-1]:.2f} 🌡️"
                )
            elif self.adx[-1] < 20:
                self.position.close()
                print(
                    f"🌑 ADX COLLAPSE! {self.adx[-1]:.1f} - Closing at {self.data.Close[-1]:.2f} ⚠️"
                )
    ...
    def notify_trade(self, trade):
        if trade.is_closed:
            ...
            print(
                f"{profit_emoji} Trade closed: {trade.entry_price:.2f} → {trade.exit_price:.2f} | PNL: {trade.pnl:.2f}"
            )


# Launch moon mission 🚀🌙
bt = Backtest(data, MomentumCrossADX, cash=1_000_000, commission=0.002)
stats = bt.run()
print("\n🌕✨ Lunar Backtest Results 🌙✨")
print(stats)
