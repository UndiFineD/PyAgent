# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_18_2025\backtests_final\VolatilityFade_BTFinal.py
import pandas as pd
...
class VolatilityFade(Strategy):
    ...
    def next(self):
        ...
        print(
            f"🌙 Price: {price:.2f} | ADX: {self.adx[-1]:.2f} | Chaikin: {self.chaikin[-1]:.2f} | ATR: {self.atr[-1]:.2f}"
        )

        if not self.position:
            # 🚀 Entry Conditions 🚀
            if (self.adx[-1] < 20) and (self.chaikin[-2] > 0 and self.chaikin[-1] < 0):
                ...
                if size > 0:
                    ...
                    print(
                        f"🔒 SL: {price + stop_distance:.2f} | Risk: {self.risk_pct*100}%"
                    )
