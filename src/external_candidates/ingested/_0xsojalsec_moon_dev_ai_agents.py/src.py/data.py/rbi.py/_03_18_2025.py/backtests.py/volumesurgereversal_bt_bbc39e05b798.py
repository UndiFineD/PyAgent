# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_18_2025\backtests\VolumeSurgeReversal_BT.py
    ...
    def next(self):
        ...
        # 🔄 Trailing Stop Logic
            ...
            if self.data.Close[-1] < self.trailing_stop:
                self.position.close()
                print(
                    f"🌑💫 EXIT | Price: {current_close:.2f} | Stop: {self.trailing_stop:.2f}"
                )

    def notify_trade(self, trade):
        if trade.is_closed:
            print(
                f"💰🌙 Trade Closed | Profit: {trade.pnl:.2f} | Return: {trade.return_pct:.2%}"
            )
