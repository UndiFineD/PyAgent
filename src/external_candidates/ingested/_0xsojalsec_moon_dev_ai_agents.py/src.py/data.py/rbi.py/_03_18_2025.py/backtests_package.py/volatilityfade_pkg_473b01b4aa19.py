# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_18_2025\backtests_package\VolatilityFade_PKG.py
import numpy as np
import pandas as pd
...
class VolatilityFade(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (self.adx[-1] < 20) and (
                -self.chaikin[-2] < 0 and -self.chaikin[-1] > 0
            ):
                ...
                if size > 0:
                    ...
                    self.sell(
                        size=size,
                        sl=high + 1.5 * atr_value,
                        tag="🌙 Moon Dev Short Entry",
                    )
                    print(f"🚀🚀 MOON DEV SHORT! {size} units @ {price:.2f}")
                    print(
                        f"🔒 SL: {high + 1.5*atr_value:.2f} | Risk: {self.risk_pct*100}%"
                    )
