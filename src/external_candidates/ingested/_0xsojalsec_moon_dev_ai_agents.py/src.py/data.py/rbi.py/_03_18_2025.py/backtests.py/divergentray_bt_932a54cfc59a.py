# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_18_2025\backtests\DivergentRay_BT.py
import numpy as np
import pandas as pd
import talib
...
class DivergentRay(Strategy):
    ...
    def init(self):
        self.bull_power = self.I(
            calculate_bull_power, self.data.Close, self.bull_power_ema
        )
        self.adx = self.I(
            talib.ADX, self.data.High, self.data.Low, self.data.Close, self.adx_period
        )
    ...
    def next(self):
        ...
        price_divergence = (
            self.price_highs[current_idx] > self.price_highs[prev_idx]
        ) and (self.bull_highs[current_idx] < self.bull_highs[prev_idx])
        ...
        if not self.position and price_divergence and volume_declining and strong_trend:
            ...
            position_size = int(
                round((self.equity * self.risk_percent) / risk_distance)
            )
            if position_size > 0:
                self.sell(size=position_size, sl=stop_loss)
                print(
                    f"🌙🚀 BEARISH DIVERGENCE DETECTED! Short {position_size} units at {self.data.Close[-1]:.2f} 🌌"
                )
            ...
            if bull_cross or trend_weak:
                self.position.close()
                reason = (
                    "Bull Power breakout 🌤️" if bull_cross else "Trend weakening 📉"
                )
                print(
                    f"🌙✨ Closing position: {reason} at {self.data.Close[-1]:.2f} 🏁"
                )
...
# Run backtest
bt = Backtest(data, DivergentRay, cash=1_000_000, commission=0.002)
stats = bt.run()
