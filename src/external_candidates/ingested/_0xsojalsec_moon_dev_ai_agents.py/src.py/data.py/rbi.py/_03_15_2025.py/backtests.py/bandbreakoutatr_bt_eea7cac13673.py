# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests\BandBreakoutATR_BT.py
import numpy as np
import pandas as pd
import talib
...
class BandBreakoutATR(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if crossover(self.data.BB_UPPER, self.data.BB_MIDDLE):
                ...
                if risk_per_share > 0:
                    ...
                    if moon_size > 0:
                        ...
                        print(
                            f"🌕🚀 MOON DEV BLASTOFF! LONG {moon_size} @ {price:.2f} | ATR Shield: {sl_price:.2f}"
                        )
        else:
