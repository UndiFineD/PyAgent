# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_final\ATRRatioReversal_BTFinal.py
import numpy as np
import pandas as pd
import talib
...
class ATRRatioReversal(Strategy):
    ...
    def init(self):
        ...
        self.ratio = self.I(
            lambda c, a: c / a, self.data.Close, self.atr, name="Close/ATR"
        )
    ...
    def next(self):
        ...
        if not self.position and self.ratio[-1] < 0:
            ...
            risk_per_share = abs(
                price - sl_price
            )  # Added abs() to ensure positive value
