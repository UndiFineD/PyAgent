...
class BandedMeanTarget(Strategy):
    ...
    def init(self):
        ...
        self.hist_high = self.I(
            lambda: self.data.High.expanding().max(), name="Historical High"
        )
    ...
    def next(self):
        ...
        if not self.position:
            entry_condition = (
                current_close <= 0.75 * hist_high and current_close > bb_lower
            )
            ...
            if entry_condition:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙✨ MOON DEV ENTRY! Buying {position_size} @ {entry_price:.2f} 🚀"
                        )
