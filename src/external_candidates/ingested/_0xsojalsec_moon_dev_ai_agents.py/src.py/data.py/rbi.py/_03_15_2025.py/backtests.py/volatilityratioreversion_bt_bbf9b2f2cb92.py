# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests\VolatilityRatioReversion_BT.py
...
class VolatilityRatioReversion(Strategy):
    ...
    def init(self):
        ...
        self.atr3 = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, 3, name="ATR3"
        )
    ...
    def next(self):
        ...
        if not self.position and current_ratio > self.upper_threshold:
            ...
            if position_size > 0:
                ...
                print(
                    f"🌙✨ MOON DEV LONG SIGNAL! Ratio: {current_ratio:.2f} | Size: {position_size} ✨🚀"
                )
