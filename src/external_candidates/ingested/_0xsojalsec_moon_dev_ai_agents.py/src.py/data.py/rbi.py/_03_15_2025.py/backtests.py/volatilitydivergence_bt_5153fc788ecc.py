    ...
    def next(self):
        ...
        if self.position:
            if current_spread < self.exit_threshold:
                self.position.close()
                print(
                    f"🌕 Moon Dev Alert: Cosmic spread normalized! ({current_spread:.2f}%) Closing position 🛸"
                )
            ...
            for trade in self.trades:
                if trade.is_short and trade.sl:
                    ...
                    if self.data.High[-1] >= lunar_sl:
                        print(
                            f"🚨 Moon Dev Warning: Stellar stop loss triggered! ({self.data.High[-1]:.2f}) 🌪️"
                        )
            ...
            if current_spread > self.entry_threshold:
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"   SL: {sl_price:.2f} | Risk: {self.risk_pct*100:.0f}% of equity 🌌"
                        )
