# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_package\MonthlyHighLeverage_PKG.py
import pandas as pd
import talib
...
class MonthlyHighLeverage(Strategy):
    ...
    def init(self):
        ...
        self.weekly_high = self.I(talib.MAX, self.data.High, timeperiod=self.week_bars)
    ...
    def next(self):
        ...
        if (
            self.month_end_mask[-1]
            and current_dt.time() == pd.Timestamp("00:00:00").time()
        ):
            if not self.position:
                ...
                if position_size > 0:
                    self.sell(
                        size=position_size, sl=strike_price, tag="Moon Put Strike 🎯"
                    )
                    print(
                        f"🚀 LAUNCHING SHORT POSITION 🌑\n"
