# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests\VolatilityContraction_BT.py
...
class VolatilityContraction(Strategy):
    ...
    def next(self):
        ...
        atr_ratio = (
            self.atr[-1] / self.hist_high_atr[-1] if self.hist_high_atr[-1] != 0 else 0
        )
            ...
            if (atr_ratio >= 0.95) and (bandwidth < 2):
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙✨ MOON DEV ALERT: Short initiated {position_size} units "
        ...
        # 🌓 Exit Logic
            ...
            if current_high >= upper_band or current_low <= lower_band:
                self.position.close()
                print(
                    f"✨🌑 MOON DEV EXIT: Price breakout at {self.data.Close[-1]:.2f}"
                )
            ...
            if self.atr[-1] < 0.8 * self.peak_atr_since_entry:
                self.position.close()
                print(
                    f"🌗🚀 MOON DEV EXIT: Volatility collapse at {self.data.Close[-1]:.2f}"
                )
