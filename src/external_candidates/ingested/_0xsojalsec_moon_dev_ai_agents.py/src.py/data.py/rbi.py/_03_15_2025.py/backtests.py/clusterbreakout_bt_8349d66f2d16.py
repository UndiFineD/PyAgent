# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests\ClusterBreakout_BT.py
import pandas as pd
import talib
...
class ClusterBreakout(Strategy):
    ...
    def init(self):
        ...
        self.cluster_high = self.I(
            talib.MAX, self.data.High, timeperiod=20, name="🌙 Cluster High"
        )
        self.cluster_low = self.I(
            talib.MIN, self.data.Low, timeperiod=20, name="🌙 Cluster Low"
        )
        self.trend_low = self.I(
            talib.MIN, self.data.Low, timeperiod=50, name="✨ Trend Low"
        )
    ...
    def next(self):
            ...
            if self.trend_low[-1] > self.trend_low[-2]:
                ...
                if self.data.Close[-1] > self.cluster_high[-1]:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌕 **LIFTOFF!** Buying {position_size} shares at {self.entry_price:.2f}"
                        )
                        print(
                            f"   🌙 Stop Loss: {stop_loss_price:.2f} | Cluster High: {self.cluster_high[-1]:.2f}"
                        )
        ...
        # ✨ Exit Logic
            ...
            if current_price < self.entry_price * 0.99:
                self.sell()
                print(
                    f"🌑✨ **GRAVITY PULL** Selling at {current_price:.2f} (1% Retracement)"
                )
            ...
            # Trend Line Break Exit
                ...
                self.sell()
                print(
                    f"🌑🌪️ **BLACK HOLE EXIT** Selling at {current_price:.2f} (Trend Break)"
                )
