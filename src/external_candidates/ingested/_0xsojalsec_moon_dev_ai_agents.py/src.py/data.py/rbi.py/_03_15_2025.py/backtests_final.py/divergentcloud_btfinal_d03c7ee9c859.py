# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_final\DivergentCloud_BTFinal.py
import numpy as np
import pandas as pd
...
class DivergentCloud(Strategy):
    ...
    def next(self):
        ...
        bearish_entry = (
            ...
            and price < min(self.senkou_a[-1], self.senkou_b[-1])
            and self.chikou[-26] < price  # Chikou Span comparison (26-period lag)
        )

        # Exit conditions
        bullish_exit = (
            price <= self.lower_band[-1]
            or price > min(self.senkou_a[-1], self.senkou_b[-1])
            or self.chikou[-26] > price
        )
        ...
        if bearish_entry and not self.position:
            ...
            if position_size > 0:
                ...
                print(
                    f"🚨 BEARISH CLOUD BREAK! Shorting {position_size} units at {price:.2f} 🌧️"
                )
