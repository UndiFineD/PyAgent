# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_final\VolatilityMomentum_BTFinal.py
...
class VolatilityMomentum(Strategy):
    ...
    def next(self):
        ...
        if current_bar % 100 == 0:
            ...
            print(
                f"   ATR: {self.atr[-1]:.2f} | 20-period MIN ATR: {self.min_atr[-1]:.2f}"
            )

        if not self.position:
            # 🌠 Entry Conditions: Low Volatility + Momentum Cross
            if (
                self.atr[-1] <= self.min_atr[-1]
                and self.data.Close[-2] < self.ema10[-2]
                and self.data.Close[-1] > self.ema10[-1]
            ):
...
# 🚀 Launch Backtest
bt = Backtest(data, VolatilityMomentum, cash=1_000_000, commission=0.002)
stats = bt.run()
