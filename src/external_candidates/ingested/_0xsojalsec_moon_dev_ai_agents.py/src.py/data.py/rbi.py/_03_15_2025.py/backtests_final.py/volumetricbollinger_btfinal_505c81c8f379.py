# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_15_2025\backtests_final\VolumetricBollinger_BTFinal.py
import pandas as pd
import talib
...
class VolumetricBollinger(Strategy):
    def init(self):
        ...
        self.std20 = self.I(
            talib.STDDEV, self.data.Close, timeperiod=20, nbdev=2, name="STD20"
        )
        self.avg_volume = self.I(
            talib.SMA, self.data.Volume, timeperiod=96, name="AvgVolume"
        )
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Processing bar {len(self.data)} | Price: {self.data.Close[-1]:.2f} | Equity: {self.equity:.2f}"
            )
        ...
        if not self.position:
            ...
            if bb_width < 2.0 and vol_condition:
                print(
                    f"✨🌟 SIGNAL DETECTED! | BB Width: {bb_width:.4f} | Volume: {self.data.Volume[-1]:.2f} (2x {self.avg_volume[-1]:.2f})"
                )
            ...
            if hasattr(self, "entry_window") and self.entry_window > 0:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌕 ENTRY! {position_size} units @ {price:.2f} | SL: {sl:.2f} | TP: {tp:.2f} | Equity: {self.equity:.2f}"
                    )
