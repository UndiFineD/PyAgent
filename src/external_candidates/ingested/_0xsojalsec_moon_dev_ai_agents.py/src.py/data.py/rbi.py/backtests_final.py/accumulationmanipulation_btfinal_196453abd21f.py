...
class AccumulationManipulation(Strategy):
    ...
    def init(self):
        ...
        self.ema = self.I(
            lambda x: pd.Series(x).ewm(span=self.n1).mean(), self.data.Close
        )
        self.volume_ma = self.I(
            lambda x: pd.Series(x).rolling(self.n2).mean(), self.data.Volume
        )
    ...
    def next(self):
        ...
        if (
            ...
            print(
                f"🎯 Entry Signal! Price: {price:.2f}, SL: {sl_price:.2f}, TP: {tp_price:.2f}"
            )
