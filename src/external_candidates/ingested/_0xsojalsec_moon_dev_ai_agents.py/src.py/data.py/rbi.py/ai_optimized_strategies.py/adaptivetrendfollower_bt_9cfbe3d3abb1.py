# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_OPTIMIZED_STRATEGIES\AdaptiveTrendFollower_BT.py
...
class AdaptiveTrendFollower(Strategy):
    ...
    def init(self):
        ...
        self.close_series = pd.Series(
            self.data.Close, index=range(len(self.data.Close))
        )

        # Volatility measurement for adaptation
        self.atr = self.I(
            ta.atr, self.data.High, self.data.Low, self.data.Close, self.atr_period
        )

        # Price change volatility
        self.price_change = self.I(
            lambda: self.close_series.pct_change()
            .abs()
        ...
        macd_data = ta.macd(
            self.data.Close,
            fast=self.macd_fast_base,
            slow=self.macd_slow_base,
            signal=self.macd_signal_base,
        )
        ...
        supertrend_data = ta.supertrend(
            self.data.High, self.data.Low, self.data.Close, length=10, multiplier=3.0
        )
        self.supertrend = self.I(lambda: supertrend_data[f"SUPERT_10_3.0"].values)
        self.supertrend_direction = self.I(
            lambda: supertrend_data[f"SUPERTd_10_3.0"].values
        )
        ...
        print(
            "🌙✨ Adaptive Trend Follower Initialized! Market regime detection active! ✨"
        )
    ...
    def calculate_trend_strength(self):
        ...
        if self.ema_fast[-1] > self.ema_slow[-1] > self.ema_trend[-1]:
            ...
            or self.ema_fast[-1] < self.ema_slow[-1]
        ):
            ema_alignment = 0.5  # Partial alignment

        # Price vs trend EMA
        price_trend_strength = (
            abs(self.data.Close[-1] - self.ema_trend[-1]) / self.ema_trend[-1]
        )
    ...
    def check_trend_signals(self):
        ...
        ema_momentum = (
            (self.ema_fast[-1] - self.ema_fast[-2]) / self.ema_fast[-2]
            if self.ema_fast[-2] != 0
            else 0
        )
        ...
        bb_position = (current_close - self.bb_lower[-1]) / (
        ...
        required_signals = max(
            2, int(3 * entry_threshold)
        )  # Adaptive signal requirement
        required_confirmations = max(2, int(3 * entry_threshold))

        if (
            sum(long_signals) >= required_signals
            and sum(long_confirmations) >= required_confirmations
        ):
            self.enter_long_trend()

        elif (
            sum(short_signals) >= required_signals
            and sum(short_confirmations) >= required_confirmations
        ):
    ...
    def enter_long_trend(self):
        ...
        # Adaptive stop loss
        atr_multiplier = self.adaptive_params.get(
            "atr_multiplier", self.atr_multiplier_base
        )

        atr_stop = entry_price - (self.atr[-1] * atr_multiplier)
        supertrend_stop = (
            self.supertrend[-1] if self.supertrend_direction[-1] == 1 else atr_stop
        )
        ...
        if position_size > 0:
            ...
            print(
                f"🚀📈 LONG TREND ({self.current_regime}): Size={position_size} @ {entry_price:.2f}"
            )
            print(
                f"   SL: {stop_loss:.2f} | TP: {take_profit:.2f} | Trend Strength: {self.calculate_trend_strength():.2f}"
            )
    ...
    def enter_short_trend(self):
        ...
        if position_size > 0:
            ...
            print(
                f"📉📈 SHORT TREND ({self.current_regime}): Size={position_size} @ {entry_price:.2f}"
            )
            print(
                f"   SL: {stop_loss:.2f} | TP: {take_profit:.2f} | Trend Strength: {self.calculate_trend_strength():.2f}"
            )
    ...
    def manage_trend_position(self):
        ...
        if self.position.is_long:
            ...
            if (
                trend_strength < 0.2
                or self.supertrend_direction[-1] == -1
                or (self.ema_fast[-1] < self.ema_slow[-1] and bars_held > 5)
            ):
                self.position.close()
                print(
                    f"🔄📈 LONG TREND REVERSAL @ {current_price:.2f} ({bars_held} bars)"
                )
                self.reset_trade_params()
                return
            ...
            if (
                ...
                self.position.close()
                print(
                    f"🔄📉 SHORT TREND REVERSAL @ {current_price:.2f} ({bars_held} bars)"
                )
                self.reset_trade_params()
                return
        ...
        if bars_held > 10:  # Only after position is established
            atr_trail_multiplier = (
                self.adaptive_params.get("atr_multiplier", self.atr_multiplier_base)
                * 0.75
            )
...
bt = Backtest(data, AdaptiveTrendFollower, cash=1000000, commission=0.002)
stats = bt.run()
