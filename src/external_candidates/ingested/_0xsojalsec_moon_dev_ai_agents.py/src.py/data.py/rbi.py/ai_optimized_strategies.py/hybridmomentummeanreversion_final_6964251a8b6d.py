# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_OPTIMIZED_STRATEGIES\HybridMomentumMeanReversion_FINAL.py
...
class HybridMomentumMeanReversionFinal(Strategy):
    ...
    def next(self):
        ...
        long_conditions = [
            ...
            and self.rsi[-1] < 50,  # Price momentum + not overbought
            ema_bullish
            and current_close > self.bb_lower[-1] * 1.001,  # Above BB lower with trend
        ]
        ...
        short_conditions = [
            ...
            price_momentum < -0.001
            and self.rsi[-1] > 50,  # Negative momentum + not oversold
            not ema_bullish
            and current_close
            < self.bb_upper[-1] * 0.999,  # Below BB upper with downtrend
        ]
...
bt = Backtest(data, HybridMomentumMeanReversionFinal, cash=1000000, commission=0.002)
stats = bt.run()
