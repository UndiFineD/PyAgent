# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_OPTIMIZED_STRATEGIES\HybridMomentumMeanReversion_BT.py
...
class HybridMomentumMeanReversion(Strategy):
    ...
    def init(self):
        ...
        self.macd_hist = self.I(
            macd, self.data.Close, self.macd_fast, self.macd_slow, self.macd_signal
        )
    ...
    def check_entry_signals(self):
        ...
        # Volume confirmation
        volume_above_avg = (
            current_volume > self.volume_sma[-1] * 0.8
        )  # Relaxed volume filter

        # Volatility filter
        current_atr = self.atr[-1]
        atr_suitable = (
            current_atr > np.nanmean(self.atr[-10:]) * 0.7
        )  # Ensure some volatility
        ...
        momentum_bullish = (ema_bullish and ema_trend_strong) or (
            macd_bullish and macd_improving
        )
        mean_reversion_long = (
            (rsi_oversold and rsi_recovering)
            or (bb_oversold and bb_bounce)
            or (stoch_oversold and stoch_recovering)
        )

        # Short entries: Bearish momentum trend + overbought mean reversion signal
        momentum_bearish = (not ema_bullish and not ema_trend_strong) or (
        ...
        mean_reversion_short = (
            (rsi_overbought and rsi_declining)
            or (bb_overbought and bb_rejection)
            or (stoch_overbought and stoch_declining)
        )

        # Execute trades
        if (
            momentum_bullish
            and mean_reversion_long
    ...
    def enter_long(self):
        ...
        if position_size > 0:
            ...
            print(
                f"   SL: {stop_loss:.2f} | TP: {take_profit:.2f} | R:R: {reward_ratio:.1f}"
            )
    ...
    def manage_position(self):
        ...
        if self.position.is_long:
            ...
            if (
                self.ema_fast_line[-1] < self.ema_slow_line[-1]
                or self.macd_hist[-1] < self.macd_hist[-2] < self.macd_hist[-3]
            ...
            if (
                self.ema_fast_line[-1] > self.ema_slow_line[-1]
                or self.macd_hist[-1] > self.macd_hist[-2] > self.macd_hist[-3]
                or self.rsi[-1] < self.rsi_oversold
            ):
                self.position.close()
        ...
        if (
            self.position.is_long and current_price > self.entry_price * 1.005
        ):  # 0.5% profit
            ...
            self.position.is_short and current_price < self.entry_price * 0.995
        ):  # 0.5% profit
...
bt = Backtest(data, HybridMomentumMeanReversion, cash=1000000, commission=0.002)
stats = bt.run()
