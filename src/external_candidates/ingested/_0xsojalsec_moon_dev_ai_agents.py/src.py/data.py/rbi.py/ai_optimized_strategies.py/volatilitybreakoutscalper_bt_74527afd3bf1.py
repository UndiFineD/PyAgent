# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_OPTIMIZED_STRATEGIES\VolatilityBreakoutScalper_BT.py
...
class VolatilityBreakoutScalper(Strategy):
    ...
    def init(self):
        ...
        self.bb_upper = self.I(
            lambda: bb_data[f"BBU_{self.bb_period}_{self.bb_std}"].values
        )
        self.bb_middle = self.I(
            lambda: bb_data[f"BBM_{self.bb_period}_{self.bb_std}"].values
        )
        self.bb_lower = self.I(
            lambda: bb_data[f"BBL_{self.bb_period}_{self.bb_std}"].values
        )
        self.bb_width = self.I(
            lambda: (
                bb_data[f"BBU_{self.bb_period}_{self.bb_std}"]
                - bb_data[f"BBL_{self.bb_period}_{self.bb_std}"]
            ).values
        )
        ...
        self.roc = self.I(
            ta.roc, self.data.Close, self.momentum_period
        )  # Rate of change
        self.rsi = self.I(ta.rsi, self.data.Close, 7)  # Short RSI for quick signals

        # Price range indicators
        self.true_range = self.I(
            lambda: ta.true_range(self.data.High, self.data.Low, self.data.Close).values
        )
        ...
        self.bb_width_sma = self.I(
            ta.sma, pd.Series(self.bb_width), self.squeeze_period
        )
        ...
        print(
            "🌙✨ Volatility Breakout Scalper Initialized! Ready for high-frequency action! ✨"
        )
    ...
    def next(self):
        ...
        if (
            len(self.data)
            < max(self.bb_period, self.atr_period, self.squeeze_period) + 1
        ):
            return
    ...
    def check_breakout_signals(self):
        ...
        range_contraction = np.mean(self.price_range[-3:]) < np.mean(
            self.price_range[-10:-3]
        )
        ...
        price_breaking_range = (
            current_high > max(self.data.High[-5:-1]) * self.breakout_multiplier
            or current_low < min(self.data.Low[-5:-1]) / self.breakout_multiplier
        )
        ...
        # LONG BREAKOUT SCALP
        if price_above_bb_upper or (
            price_breaking_range and current_close > self.bb_middle[-1]
        ):
            if (
                volume_confirmation
                and momentum_strong
                and rsi_not_extreme
                and (volatility_squeeze or momentum_accelerating)
            ):
                self.enter_long_scalp()

        # SHORT BREAKOUT SCALP
        elif price_below_bb_lower or (
            price_breaking_range and current_close < self.bb_middle[-1]
        ):
            if (
                volume_confirmation
        ...
        # VOLATILITY EXPANSION SCALP (trade in direction of breakout)
            ...
            not volatility_squeeze and atr_current > atr_avg * 1.2
    ...
    def enter_long_scalp(self):
        ...
        if position_size > 0:
            ...
            print(
                f"   SL: {stop_loss:.2f} | TP: {take_profit:.2f} | Hold Max: {self.max_hold_bars} bars"
            )
    ...
    def enter_short_scalp(self):
        ...
        if position_size > 0:
            ...
            print(
                f"📉⚡ SHORT BREAKOUT SCALP: Size={position_size} @ {entry_price:.2f}"
            )
            print(
                f"   SL: {stop_loss:.2f} | TP: {take_profit:.2f} | Hold Max: {self.max_hold_bars} bars"
            )
    ...
    def manage_scalp_position(self):
        ...
        if self.position.is_long:
            ...
            if (
                momentum_weakening
                or self.rsi[-1] > 70
                or current_price < self.bb_middle[-1]
            ):  # Price back below middle BB
                self.position.close()
                print(
                    f"🔄⚡ LONG MOMENTUM EXIT @ {current_price:.2f} ({bars_held} bars)"
                )
                self.reset_trade_params()
                return
            ...
            if (
                ...
                or self.rsi[-1] < 30
                or current_price > self.bb_middle[-1]
            ):  # Price back above middle BB
                self.position.close()
                print(
                    f"🔄⚡ SHORT MOMENTUM EXIT @ {current_price:.2f} ({bars_held} bars)"
                )
                self.reset_trade_params()
                return
...
bt = Backtest(data, VolatilityBreakoutScalper, cash=1000000, commission=0.002)
stats = bt.run()
