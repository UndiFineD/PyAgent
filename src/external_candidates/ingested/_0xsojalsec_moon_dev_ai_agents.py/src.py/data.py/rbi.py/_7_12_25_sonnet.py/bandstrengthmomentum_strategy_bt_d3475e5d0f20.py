# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_sonnet\BandStrengthMomentum_strategy_bt.py
import pandas as pd
import talib
...
class BandStrengthMomentumStrategy(Strategy):
    ...
    def next(self):
        if self.position:
            ...
            if self.position.is_long:
                if (
                    ...
                    or self.relative_strength[-1] < 40
                ):
                    print("🌙 Moon Dev long exit signal! Closing position. ✨")
                    self.position.close()
            elif self.position.is_short:
                if (
                    self.data.Close[-1] <= self.lower_bb[-1]
                    or self.relative_strength[-1] > 60
                ):
        ...
        price_above_lower_bb = (
            self.data.Close[-1] > self.lower_bb[-1]
            and self.data.Close[-2] <= self.lower_bb[-2]
        )
        ...
        price_below_upper_bb = (
            self.data.Close[-1] < self.upper_bb[-1]
            and self.data.Close[-2] >= self.upper_bb[-2]
        )
