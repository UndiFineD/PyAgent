# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_sonnet\ClusterBreakout_strategy_bt.py
import pandas as pd
import talib
...
class ClusterBreakoutStrategy(Strategy):
    ...
    def next(self):
        ...
        breakout_above = (
            self.data.Close[-1] > self.high_cluster[-1]
            and self.data.Close[-1] > self.sma_50[-1]
        )
        ...
        breakout_below = (
            self.data.Close[-1] < self.low_cluster[-1]
            and self.data.Close[-1] < self.sma_50[-1]
        )
