# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_sonnet\ConfluencePattern_strategy_bt.py
import pandas as pd
import talib
    ...
    def next(self):
        if self.position:
            ...
            if (
                ...
                and (self.rsi[-1] > 70 or crossover(self.signal, self.macd))
            ) or (
                self.position.is_short
                and (self.rsi[-1] < 30 or crossover(self.macd, self.signal))
            ):
        ...
        # Bullish confluence
        trend_up = (
            self.data.Close[-1] > self.ema_50[-1] and self.ema_50[-1] > self.ema_200[-1]
        )
