# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_sonnet\CoTrendalNeutral_strategy_bt.py
import pandas as pd
import talib
...
class CoTrendalNeutralStrategy(Strategy):
    ...
    def next(self):
        ...
        trend_up = (
            self.data.Close[-1] > self.sma_50[-1] and self.sma_50[-1] > self.sma_200[-1]
        )
        vix_declining = (
            self.vix_proxy[-1] < self.vix_proxy[-2] and self.vix_proxy[-1] < 50
        )
