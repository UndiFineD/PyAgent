# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_16_2025\backtests_final\RSIReversalPut_BTFinal.py
...
class RSIReversalPut(Strategy):
    ...
    def next(self):
        ...
        if not self.position and self.min_rsi[-1] <= 30:
            ...
            if risk_per_share > 0:
                position_size = int(
                    round((self.equity * self.risk_percentage) / risk_per_share)
                )
                if position_size > 0:
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            ...
            print(
                f"{emoji} Cosmic {direction} Order Executed at {order.executed_price:.2f} "
