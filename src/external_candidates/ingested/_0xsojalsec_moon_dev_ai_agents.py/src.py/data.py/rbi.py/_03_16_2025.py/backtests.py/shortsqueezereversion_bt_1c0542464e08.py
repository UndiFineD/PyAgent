# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_16_2025\backtests\ShortSqueezeReversion_BT.py
import numpy as np
import pandas as pd
import talib
...
class ShortSqueezeReversion(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            # 🚀 ENTRY LOGIC
            three_green = all(
                self.data.Close[-i] > self.data.Open[-i] for i in range(2, 5)
            )
            vol_spike = self.data.Volume[-1] > 1.5 * self.vol_sma[-1]

            if (
                self.short_interest_ratio[-1] > 0.2
                and self.rsi[-1] < 30
                and (three_green or vol_spike)
            ):
