# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_16_2025\backtests\FractalConvergence_BT.py
import pandas as pd
import talib
...
class FractalConvergenceStrategy(Strategy):
    ...
    def init(self):
        ...
        self.willr = self.I(
            talib.WILLR, self.data.High, self.data.Low, self.data.Close, 14
        )
    ...
    def calculate_position_size(self, entry_price, sl_price, is_long):
        ...
        if position_size > 0:
            if is_long:
                ...
                print(
                    f"🌙✨🚀 BULLISH Convergence! LONG {position_size} @ {entry_price:.2f}"
                )
            else:
                self.sell(
                    size=position_size,
                ...
                print(
                    f"🌙✨🚀 BEARISH Divergence! SHORT {position_size} @ {entry_price:.2f}"
                )
