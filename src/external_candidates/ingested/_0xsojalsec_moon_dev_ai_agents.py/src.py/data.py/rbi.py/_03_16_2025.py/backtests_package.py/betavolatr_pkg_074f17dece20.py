# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_16_2025\backtests_package\BetaVolATR_PKG.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
...
class BetaVolATR(Strategy):
    ...
    def init(self):
        ...
        self.data = self.data.drop(
            columns=[col for col in self.data.columns if "unnamed" in col]
        )

        # Core indicators ✨
        self.high_3d = self.I(
            talib.MAX, self.data.High, timeperiod=288
        )  # 3-day high (288x15m)
        self.low_3d = self.I(talib.MIN, self.data.Low, timeperiod=288)  # 3-day low

        # Volume analysis 🌊
        long_volume = self.I(
            lambda close, open: (close > open) * self.data.Volume,
            self.data.Close,
            self.data.Open,
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            vol_ratio = (
                self.sum_long_vol[-1] / self.sum_total_vol[-1]
                if self.sum_total_vol[-1] > 0
                else 0
            )
            ...
            if new_high and vol_ratio >= 0.8 and self.ema_atr[-1] > 0:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀 MOON DEV ALERT: Long {position_size} units at {self.data.Close[-1]:.2f} | SL: {self.data.Close[-1] - stop_distance:.2f} ✨"
                    )
        ...
        # Exit logic 💸
            ...
            if self.data.Close[-1] < self.low_3d[-1]:
                self.position.close()
                print(
                    f"🌙 MOON DEV EXIT: Momentum reversal at {self.data.Close[-1]:.2f} 🌗"
                )
            ...
            # Time-based exit (5 days = 480x15m)
                ...
                self.position.close()
                print(
                    f"🕒 MOON DEV EXIT: Time exit after 5 days at {self.data.Close[-1]:.2f} ⏳"
                )
...
# Run backtest ⚡
bt = Backtest(data, BetaVolATR, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
