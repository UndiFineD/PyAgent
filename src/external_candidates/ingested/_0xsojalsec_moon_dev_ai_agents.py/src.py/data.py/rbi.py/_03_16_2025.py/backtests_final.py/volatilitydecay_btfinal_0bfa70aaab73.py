# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_16_2025\backtests_final\VolatilityDecay_BTFinal.py
    ...
    def next(self):
        ...
        if not self.position and entry_condition:
            ...
            if position_size > 0:
                ...
                print(
                    f"🌙✨ Moon Dev Alert: Shorting {position_size} units at {entry_price:.2f}! SL at {stop_loss:.2f} 🚀"
                )
        ...
        if self.position:
            ...
            if bars_held >= self.hold_period:
                self.position.close()
                print(
                    f"🌕🕒 Moon Dev Exit: Hold period expired after {self.hold_period} bars!"
                )
...
bt = Backtest(data, VolatilityDecay, cash=1_000_000, commission=0.002)
stats = bt.run()
