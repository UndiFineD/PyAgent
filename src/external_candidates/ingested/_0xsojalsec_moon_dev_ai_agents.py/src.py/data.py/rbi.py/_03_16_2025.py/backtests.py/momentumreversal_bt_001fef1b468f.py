# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_16_2025\backtests\MomentumReversal_BT.py
...
class MomentumReversal(Strategy):
    ...
    def init(self):
        ...
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=5, name="Volume_SMA"
        )
    ...
    def next(self):
        ...
        # 🌙 Lunar Debug Console 🌙
        print(
            f"🌕 Close: {current_close:.2f} | RSI: {current_rsi:.1f} | EMA5/10: {ema5:.2f}/{ema10:.2f} | Vol: {current_vol:.2f} vs SMA: {vol_sma:.2f}"
        )
        ...
        if not self.position:
            ...
            if current_rsi > 70 and ema5 > ema10:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌙 LIFTOFF! Long {position_size} @ {entry_price:.2f} | SL: {sl_price:.2f}"
                    )
                else:
            ...
            if ema_cross or vol_decline:
                self.position.close()
                print(
                    f"🌙💫 REENTRY! Closing @ {current_close:.2f} | P/L: {self.position.pl:.2f}"
                )
