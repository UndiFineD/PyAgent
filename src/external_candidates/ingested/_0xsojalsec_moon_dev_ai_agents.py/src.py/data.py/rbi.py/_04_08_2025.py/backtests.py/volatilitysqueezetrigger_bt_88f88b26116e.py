...
class VolatilitySqueezeTrigger(Strategy):
    ...
    def init(self):
        ...
        self.upper = self.I(
            lambda sma, std: sma + std * self.bb_dev, self.sma, self.std
        )
        self.lower = self.I(
            lambda sma, std: sma - std * self.bb_dev, self.sma, self.std
        )
        self.band_width = self.I(
            lambda u, l, m: (u - l) / m, self.upper, self.lower, self.sma
        )
        ...
        self.hv = self.I(
            talib.STDDEV, self.I(log_returns, self.data.Close), self.hv_period
        )

        print(
            "🌙 MOON DEV INIT: Strategy armed with Bollinger Bands & Volatility Squeeze Detector! ✨"
        )
    ...
    def next(self):
        ...
        if not self.position and in_squeeze and price_in_bands:
            ...
            if position_size > 0:
                ...
                print(
                    f"🚀 MOON DEV LAUNCH: Long {position_size} units at {current_close:.2f}! ✨"
                )
                print(
                    f"   SL: {self.lower[-1]:.2f} | TP: {self.upper[-1]:.2f} | Risk: {self.risk_pct*100}%"
                )
            ...
            if current_close >= self.upper[-1]:
                ...
                self.position.close()
                print(
                    f"🌧️ MOON DEV STORM: Stop loss triggered at {current_close:.2f}! ☔"
                )
            elif len(self.data) - self.position.entry_bar >= self.time_exit_bars:
                self.position.close()
                print(
                    f"⌛ MOON DEV TIMEOUT: Position closed after {self.time_exit_bars} bars! ⏳"
                )
