...
class VolatilitySqueeze(Strategy):
    ...
    def init(self):
        ...
        self.bb_upper = self.I(
            lambda c: talib.BBANDS(c, 20, 2, 2, 0)[0], self.data.Close, name="bb_upper"
        )
        self.bb_lower = self.I(
            lambda c: talib.BBANDS(c, 20, 2, 2, 0)[2], self.data.Close, name="bb_lower"
        )
        self.bb_width = self.I(
            lambda u, l: u - l, self.bb_upper, self.bb_lower, name="bb_width"
        )
    ...
    def next(self):
        ...
        historical_percentile = (
            sum(1 for x in self.bb_width_history if x <= current_width)
            / len(self.bb_width_history)
        ) * 100
        ...
        if not self.position and historical_percentile <= 10:
            ...
            if risk_per_unit > 0:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ MOON DEV ALERT: Squeeze Detected! "
