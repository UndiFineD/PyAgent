# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_18_2025\backtests_final\LiquidationDivergence_BTFinal.py
import pandas as pd
import talib
...
class LiquidationDivergence(Strategy):
    ...
    def next(self):
        ...
        liq_condition = (
            self.long_liq[-1] > self.liq_spike[-2] * 1.5
            and self.data.Close[-1] < self.data.Close[-96]
        )

        # RSI Divergence Detection 🌗
        rsi_bull_div = (
            self.swing_low[-1] < self.swing_low[-2]
            and self.rsi_4h[-1] > self.rsi_4h[-2]
            and self.rsi_4h[-1] < 30
        )

        # Moon Dev Entry Logic 🚀
        if not self.position and liq_condition and rsi_bull_div:
            # Fibonacci Calculations 🌙
            fib_high = max(self.swing_high[-96:-1])
            fib_low = min(self.swing_low[-96:-1])
