# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_09_2025\backtests_final\VolumetricBreakout_BTFinal.py
...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        ...
        self.upper_band = self.I(
            lambda: self.mid_band + 2 * self.std, name="Upper Band"
        )
    ...
    def next(self):
        ...
        if not self.position and vol > 2 * avg_vol and price > upper:
            ...
            if price <= sl_price:
                print(
                    f"🚨 Moon Crash Alert! Aborting launch (Price {price:.2f} <= SL {sl_price:.2f})"
                )
                return
        ...
        # 🌑 Re-entry Conditions
            ...
            self.position.close()
            print(
                f"🌘🌍 Re-entry Signal! Closing @ {price:.2f} (Below SMA {self.mid_band[-1]:.2f})"
            )
