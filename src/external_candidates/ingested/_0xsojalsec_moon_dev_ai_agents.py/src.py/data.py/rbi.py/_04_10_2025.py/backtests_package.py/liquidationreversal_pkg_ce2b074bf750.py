# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests_package\LiquidationReversal_PKG.py
import pandas as pd
import talib
...
class LiquidationReversal(Strategy):
    ...
    def next(self):
        ...
        if all([funding_negative, in_liquidation_zone, rsi_oversold, bullish_signal]):
            ...
            if position_size <= 0:
                print(
                    f"🌙 Stardust shortage: {position_size} units available. Needs more cosmic dust! 🌌"
                )
                return

            # Execute trade with celestial stops 🌌
            tp_price = entry_price + 1.5 * self.atr[-1]
            self.buy(size=position_size, sl=sl_price, tp=tp_price, tag="LunarReversal")

            print(
                f"🚀🌕 MOONSHOT INITIATED! Entry: {entry_price:.2f} | "
