# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests\LiquidationPivot_BT.py
import pandas as pd
import talib
...
class LiquidationPivot(Strategy):
    ...
    def init(self):
        ...
        self.swing_high = self.I(
            talib.MAX, self.data.High, self.swing_period, name="SWING_HIGH"
        )
        self.swing_low = self.I(
            talib.MIN, self.data.Low, self.swing_period, name="SWING_LOW"
        )
    ...
    def next(self):
        ...
        if current_dt.weekday() >= 5 or (
            current_dt.weekday() == 4 and current_dt.hour >= 20
        ):
            print(f"🌙 Skipping low liquidity period: {current_dt} ✨")
            return

        print(f"🌙 Price: {self.data.Close[-1]:.2f} | Pivot: {self.pivot[-1]:.2f} ✨")
...
# Execute backtest
bt = Backtest(data, LiquidationPivot, cash=1_000_000, commission=0.002)
stats = bt.run()
