# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests_final\VolSurgeBreakout_BTFinal.py
import pandas as pd
import talib
...
class VolSurgeBreakout(Strategy):
    ...
    def init(self):
        ...
        self.lower_band = self.I(
            lambda x: talib.BBANDS(x, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)[2],
            self.data.Close,
        )
        self.volume_sma = self.I(talib.SMA, self.data.Volume, timeperiod=20)
        self.sar = self.I(
            talib.SAR, self.data.High, self.data.Low, acceleration=0.02, maximum=0.2
        )
    ...
    def next(self):
        ...
        if (
            not self.position and len(self.data.Close) > 3
        ):  # Changed >= to > for proper indexing
            # Volatility compression check
            in_compression = all(
                c <= lb for c, lb in zip(self.data.Close[-3:], self.lower_band[-3:])
            )
            ...
            if in_compression and volume_spike and breakout_trigger:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙🚀 MOON BLASTOFF! Long {position_size} units @ ${entry_price:.2f} | Stop: ${stop_loss:.2f}"
                    )
...
# LAUNCH MOON MISSION 🌙🚀
bt = Backtest(
    data, VolSurgeBreakout, cash=1_000_000, commission=0.002
