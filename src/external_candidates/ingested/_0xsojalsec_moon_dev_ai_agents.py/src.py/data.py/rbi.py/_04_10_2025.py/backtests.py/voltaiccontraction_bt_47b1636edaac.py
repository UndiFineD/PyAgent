# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests\VoltaicContraction_BT.py
# 🌙 Moon Dev's VoltaicContraction Backtest 🌙
import numpy as np
import pandas as pd
from backtesting import Backtest, Strategy
from talib import ATR, MAX, MIN, SMA
...
class VoltaicContraction(Strategy):
    ...
    def init(self):
        ...
        self.atr14 = self.I(
            ATR, self.data.High, self.data.Low, self.data.Close, timeperiod=14
        )
    ...
    def next(self):
        ...
        if not self.position and len(self.atr14) > 20:
            ...
            if atr_expanding and volume_declining:
                ...
                if position_size > 0:
                    print(
                        f"🌋 ENTRY: Short at {current_price:.2f} | Size: {position_size} | Moon Dev Risk Active! 🌋"
                    )
        ...
        # ✨ Moon Dev Exit Logic ✨
            ...
            if (exit_condition1 and exit_condition2) or time_exit:
                print(
                    f"🌕 EXIT: Closing at {current_price:.2f} | Moon Dev Profit: {self.position.pl:.2f} 🌕"
                )
                self.position.close()
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, VoltaicContraction, cash=1_000_000, commission=0.002)
    stats = bt.run()
