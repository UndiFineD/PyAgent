# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests\VelocityDivergence_BT.py
import numpy as np
import pandas as pd
import talib
...
class VelocityDivergence(Strategy):
    ...
    def init(self):
        ...
        mean = self.I(talib.SMA, self.price_velocity, self.zscore_window)
        std = self.I(talib.STDDEV, self.price_velocity, self.zscore_window)
        self.zscore = self.I(
            lambda pv, m, s: (pv - m) / s,
            self.price_velocity,
            mean,
            std,
            name="Z-Score",
    ...
    def next(self):
        ...
        if not self.position:
            if current_z > 2 and liquidity > self.liquidity_threshold:
                ...
                print(
                    f"🚀 LONG ACTIVATED! Size: {position_size} | Cosmic Momentum Detected 🌠"
                )
                ...
                print(
                    f"🌑 SHORT ENGAGED! Size: {position_size} | Black Hole Liquidation Detected 🕳️"
                )
...
bt = Backtest(data, VelocityDivergence, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
