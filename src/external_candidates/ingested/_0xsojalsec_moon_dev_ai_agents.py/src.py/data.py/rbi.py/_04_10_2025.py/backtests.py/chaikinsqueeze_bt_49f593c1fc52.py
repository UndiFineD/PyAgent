# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests\ChaikinSqueeze_BT.py
import pandas as pd
...
class ChaikinSqueeze(Strategy):
    ...
    def next(self):
        ...
        if len(self.data) % 1000 == 0:
            print(
                f"🌙 MOON DEV PROGRESS: Processing bar {len(self.data)}/{len(self.data.df)}"
            )
        ...
        if not self.position:
            ...
            if crossover(current_cmf, 0) and current_bb < 0.5 * current_atr:
                ...
                if position_size > 0:
                    print(
                        f"🌙✨🚀 MOON BLASTOFF! LONG {position_size} @ {self.data.Close[-1]:.2f}"
                    )
                    print(
                        f"   CMF: {current_cmf:.2f} | BB%: {current_bb:.4f} | ATR: {current_atr:.2f}"
                    )
            ...
            reversal_exit = (
                current_cmf < 0 and days_in_trade <= 1.25
            )  # 5 bars * 15min = 75min
