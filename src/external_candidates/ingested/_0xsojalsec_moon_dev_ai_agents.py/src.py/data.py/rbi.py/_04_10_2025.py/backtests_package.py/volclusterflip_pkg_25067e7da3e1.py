# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_10_2025\backtests_package\VolClusterFlip_PKG.py
...
class VolClusterFlip(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            print(
                f"   📈 RSI: {self.rsi[-1]:.2f} | 📊 Volume: {self.data.Volume[-1]:.2f}"
            )
        ...
        if self.position:
            ...
            if (
                price >= entry_price + 1.5 * self.atr[-1]
                or price <= entry_price - 1 * self.atr[-1]
            ):
...
# Launch Backtest 🌙🚀
bt = Backtest(data, VolClusterFlip, cash=1_000_000, commission=0.002)
stats = bt.run()
