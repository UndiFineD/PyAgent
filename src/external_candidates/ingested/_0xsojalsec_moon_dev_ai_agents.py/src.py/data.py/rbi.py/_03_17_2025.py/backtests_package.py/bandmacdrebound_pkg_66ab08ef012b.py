# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_17_2025\backtests_package\BandMACDRebound_PKG.py
...
class BandMACDRebound(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                current_close <= current_lower * 1.02
                and macd_hist > 0
                ...
                if price_diff > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"BB Lower: {current_lower:.2f} | MACD Hist: {macd_hist:.2f}"
                        )
...
# 🌙 Launch Backtest
bt = Backtest(data, BandMACDRebound, cash=1_000_000, commission=0.002)
stats = bt.run()
