        ...
        if not self.position:
            ...
            if self.data.High[-1] > prev_high and self.data.Close[-1] < prev_high:
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🚨 SHORT SIGNAL 🌑 | Price: {price:.2f} | SL: {prev_high:.2f} | TP: {tp:.2f}"
                    )
            ...
            # Long Entry: False breakout below support 🌠
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🚀 LONG SIGNAL 🌕 | Price: {price:.2f} | SL: {prev_low:.2f} | TP: {tp:.2f}"
                    )
            ...
            if self.position.is_short and crossover(self.data.Close, self.swing_high):
                print(
                    f"🌑 Closing SHORT Position at {price:.2f} | Moon Cycle Complete 🌗"
                )
                self.position.close()
            elif self.position.is_long and crossunder(self.data.Close, self.swing_low):
                print(
                    f"🌕 Closing LONG Position at {price:.2f} | Lunar Profit Achieved 🌓"
                )
                self.position.close()
