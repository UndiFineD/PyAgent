# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_final\VolumetricBreakout_BTFinal.py
...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        ...
        self.volume_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=20, name="Volume SMA"
        )
        self.swing_high = self.I(
            talib.MAX, self.data.High, timeperiod=20, name="Swing High"
        )
    ...
    def calculate_position(self, entry_price, stop_loss, take_profit, direction):
        ...
        if position_size > 0:
            if direction == "LONG":
                ...
                print(
                    f"🚀🌙 MOON DEV LONG! Entry: {entry_price:.2f}, Size: {position_size}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}"
                )
            else:
                self.sell(size=position_size, sl=stop_loss, tp=take_profit)
                print(
                    f"🌙🚀 MOON DEV SHORT! Entry: {entry_price:.2f}, Size: {position_size}, SL: {stop_loss:.2f}, TP: {take_profit:.2f}"
                )
