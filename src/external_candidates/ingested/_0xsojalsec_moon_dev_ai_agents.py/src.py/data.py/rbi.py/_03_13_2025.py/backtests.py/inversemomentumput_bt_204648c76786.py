# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\InverseMomentumPut_BT.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
...
class InverseMomentumPut(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            bearish_crossover = (
                self.sma3[-1] < self.sma9[-1] and self.sma3[-2] >= self.sma9[-2]
            )
            ...
            if bearish_crossover and rsi_overbought:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ SHORT ENTERED! Size: {position_size} @ {current_close}"
                    )
                    print(
                        f"   🛑 Stop Loss: {self.stop_loss_level:.2f} | 📉 Risk: {self.risk_per_trade*100}%"
                    )
        ...
        # Exit conditions
            ...
            if bullish_crossover or rsi_undersold or stop_hit:
                ...
                print(
                    f"   Reason: {'Bullish crossover' if bullish_crossover else ''} "
