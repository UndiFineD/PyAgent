...
class BandwidthMomentum(Strategy):
    ...
    def init(self):
        ...
        self.middle_band = self.I(
            talib.SMA, self.data.Close, timeperiod=50, name="MIDDLE"
        )
        self.std_dev = self.I(
            talib.STDDEV, self.data.Close, timeperiod=50, nbdev=1, name="STDDEV"
        )
        self.upper_band = self.I(
            lambda m, s: m + 2 * s, self.middle_band, self.std_dev, name="UPPER"
        )
        self.lower_band = self.I(
            lambda m, s: m - 2 * s, self.middle_band, self.std_dev, name="LOWER"
        )
        ...
        self.rsi_upper_ratio = self.I(
            lambda r, u: r / u, self.rsi, self.upper_band, name="RSI_UPPER_RATIO"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.bb_width[-2] == self.bb_width_min[-2]
            ):
        ...
        # 🚪 Exit Logic: Momentum threshold
            ...
            if self.rsi_upper_ratio[-1] > 1.5:
                self.position.close()
                print(
                    f"🌙💸 MOON DEV EXIT: Closing {price:.2f} | "
