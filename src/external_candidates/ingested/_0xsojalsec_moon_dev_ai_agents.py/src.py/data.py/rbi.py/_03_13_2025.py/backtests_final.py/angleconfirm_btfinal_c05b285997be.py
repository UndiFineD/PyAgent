...
class AngleConfirm:
    ...
    def on_bar(self):
        ...
        if (
            len(data) < 60
        ):  # Not enough historical bars for confirmation (adjust as needed)
            return
        ...
        if (
            last_ma_ewma[0] > last_ma_ewma[1]
            and ((data["high"][-2] + data["low"][-2]) / 2) > last_gann_angle[-1]
        ):
            ...
            if last_ma_ewma[0] > last_gann_angle[-1]:
                ...
                if not position:
                    size = (self.risk_percent * 2) / (
                        (last_ma_ewma[0] - last_gann_angle[-1])
                ...
                if not position:
                    size = (self.risk_percent * 2) / (
                        (last_ma_ewma[1] - last_gann_angle[-1])
