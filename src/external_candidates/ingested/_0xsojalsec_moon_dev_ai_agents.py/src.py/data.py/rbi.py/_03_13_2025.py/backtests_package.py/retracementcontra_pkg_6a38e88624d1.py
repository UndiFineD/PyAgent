# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\RetracementContra_PKG.py
...
class RetracementContra(Strategy):
    ...
    def init(self):
        ...
        self.swing_high = self.I(
            talib.MAX, self.data.High, self.swing_period, name="SWING HIGH"
        )
    ...
    def next(self):
        ...
        if current_drawdown >= 0.2:
            self.position.close()
            print(
                f"🚨 MOON DEV ALERT: 20% Max Drawdown Triggered! Equity: {self.equity:,.2f}"
            )  # DEBUG: BACKTESTING.LIB NOT USED HERE
            return
        ...
        if self.position:
            ...
            if (
                ...
                or self.data.Close[-1] < self.lower_band[-1]
            ):
                self.position.close()
                print(
                    f"🌙✨ Exit Signal: Price broke ATR bands! Closing at {self.data.Close[-1]:.2f}"
                )  # DEBUG: BACKTESTING.LIB NOT USED HERE
        else:
            # 🎯 ENTRY CONDITIONS
            ...
            atr_contract = current_atr < np.nanmean(
                self.atr[-self.atr_contract_lookback :]
            )  # DEBUG: BACKTESTING.LIB NOT USED HERE

            # 💡 ENTRY LOGIC
            if (
                self.data.Close[-1] > self.sma[-1]  # Uptrend check
                and any(
                    abs(self.data.Low[-1] - lvl) < 0.005 * self.data.Close[-1]
                    for lvl in fib_levels.values()
                )  # Fib touch
                and self.rsi[-1] < 30  # Oversold
                and atr_contract
            ):  # Vol contraction
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙🚀 ENTRY: {position_size} units at {self.data.Close[-1]:.2f}"
                    )  # DEBUG: BACKTESTING.LIB NOT USED HERE
