# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\SymmetricVolatility_PKG.py
...
class SymmetricVolatility(Strategy):
    ...
    def init(self):
        ...
        self.bearish_engulf = self.I(
            lambda O, H, L, C: talib.CDLENGULFING(O, H, L, C) * -1,
            self.data.Open,
            self.data.High,
            self.data.Low,
            self.data.Close,
            ...
            for level, price in fib_levels.items():
                if abs(current_close - price) <= self.atr[-1] * 0.5:
                    if self.bullish_engulf[-1] > 0:
                        ...
                        print(
                            f"🌙✨ LONG ENTRY! Size: {size} | SL: {sl:.2f} | TP: {tp:.2f}"
                        )
                        break
