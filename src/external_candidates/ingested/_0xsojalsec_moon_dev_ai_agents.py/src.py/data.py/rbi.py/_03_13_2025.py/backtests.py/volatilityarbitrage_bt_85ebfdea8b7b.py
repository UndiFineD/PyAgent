# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\VolatilityArbitrage_BT.py
...
class VolatilityArbitrage(Strategy):
    ...
    def init(self):
        ...
        self.atr_sma = self.I(talib.SMA, self.atr, self.sma_period)

        # ✨ MEAN REVERSION RATIO
        self.vol_ratio = self.I(
            lambda: self.atr / self.atr_sma if self.atr_sma != 0 else 1
        )
        ...
        if not self.position:
            ...
            if current_ratio > self.entry_ratio:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌘 MOONSHOT SHORT! {position_size} units @ {price:.2f} | SL: {stop_price:.2f} 🌗"
                    )
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌖 LUNAR LONG! {position_size} units @ {price:.2f} | SL: {stop_price:.2f} 🌔"
                    )
        else:
