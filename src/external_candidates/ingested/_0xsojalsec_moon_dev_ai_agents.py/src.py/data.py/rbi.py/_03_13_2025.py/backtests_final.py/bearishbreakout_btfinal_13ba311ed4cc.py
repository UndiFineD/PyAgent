...
class BearishBreakout(Strategy):
    ...
    def next(self):
        ...
        if self.current_engulfing_low and not self.position:
            if (
                self.data.Low[-1] < self.current_engulfing_low
                ...
                )  # Fixed: Added abs() for proper risk calculation

                if risk_per_unit > 0:
                    position_size = risk_amount / risk_per_unit
                    position_size = int(
                        round(position_size)
                    )  # Fixed: Ensured whole number position size
        ...
        if self.position and self.data.Close[-1] > self.current_engulfing_low:
            self.position.close()
            print(
                f"🌙✨ EXIT SIGNAL | Price closed above pattern low: {self.current_engulfing_low:.2f}"
            )
