# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\TrendBandRSI_PKG.py
        ...
        if not self.position:
            ...
            if (
                ...
                if position_size > 0:
                    ...
                    print(
                        f" 🌙✨ Moon Rocket LONG! 🚀 Size: {position_size} @ {current_close:.2f}"
                    )

            # Short entry logic
            elif (
                (previous_rsi >= previous_lower)
                and (current_rsi < current_lower)
                and (current_close < self.sma50[-1])
            ):
                ...
                if position_size > 0:
                    ...
                    print(
                        f" 🌙✨ Moon Dive SHORT! 🌑 Size: {position_size} @ {current_close:.2f}"
                    )


# Launch Moon Dev Backtest
bt = Backtest(data, TrendBandRSI, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
