# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\AdaptiveSynergy_BT.py
import pandas as pd
...
class AdaptiveSynergy(Strategy):
    def init(self):
        ...
        self.macd_line, self.macd_signal_line, _ = self.I(
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev Pulse | Close: {current_close:.2f} | MACD: {self.macd_line[-1]:.2f} | RSI: {self.rsi_series[-1]:.2f}"
        )
        ...
        if not self.position:
            ...
            if all(entry_conditions):
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🚀🌙 Moon Dev LONG Launch | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_loss:.2f} | TP: {take_profit:.2f}"
                    )
            ...
            if any(exit_conditions):
                self.position.close()
                print(
                    f"🌑🌙 Moon Dev Exit Signal | Price: {current_close:.2f} | Equity: {self.broker.equity:.2f}"
                )
