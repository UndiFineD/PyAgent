# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\ContrarianEarnings_BT.py
...
class ContrarianEarnings(Strategy):
    ...
    def init(self):
        ...
        self.earnings_dates = self.I(
            talib.MAX, self.data.Earnings_Release, timeperiod=1
        )
    ...
    def next(self):
        ...
        if self.earnings_dates[-1] == 1:
            ...
            print(
                f"🌕 NEW EARNINGS DETECTED! Index: {current_idx} | Price: {self.data.Close[-1]:.2f}"
            )

        # Entry Logic: 4th day after earnings
        if (
            self.last_earnings_idx != -1
            and (current_idx - self.last_earnings_idx) == 384
        ):  # 4 days in 15min bars
            ...
            if current_price < earnings_price:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀 ENTRY SIGNAL | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_loss:.2f} | TP: {take_profit:.2f}"
                    )
        ...
        for trade in self.trades:
            if (current_idx - trade.entry_bar) >= self.holding_period:
                self.position.close()
                print(
                    f"🕒 TIME EXIT | Closed position after {self.holding_period} bars"
                )
