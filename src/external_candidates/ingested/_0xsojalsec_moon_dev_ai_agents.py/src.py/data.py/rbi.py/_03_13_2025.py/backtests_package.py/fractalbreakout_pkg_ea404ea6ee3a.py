# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\FractalBreakout_PKG.py
...
class FractalBreakout(Strategy):
    ...
    def init(self):
        ...
        self.fractal_high = self.I(
            ta.MAX, self.data.High, timeperiod=self.fractal_period, name="FRACTAL HIGH"
        )
        self.fractal_low = self.I(
            ta.MIN, self.data.Low, timeperiod=self.fractal_period, name="FRACTAL LOW"
        )

        # 🚀 MOMENTUM INDICATORS
        self.ema_fast = self.I(
            talib.EMA, self.data.Close, self.ema_period_fast, name="EMA20"
        )
        self.ema_slow = self.I(
            talib.EMA, self.data.Close, self.ema_period_slow, name="EMA50"
        )
    ...
    def next(self):
        ...
        momentum_bullish = (self.ema_fast[-2] < self.ema_slow[-2]) and (
            self.ema_fast[-1] > self.ema_slow[-1]
        )
        momentum_bearish = (self.ema_fast[-2] > self.ema_slow[-2]) and (
            self.ema_fast[-1] < self.ema_slow[-1]
        )
        ...
        if not self.position:
            ...
            if price > prev_fractal_high and momentum_bullish:
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🚀🌙 LONG SIGNAL: {price:.2f} > {prev_fractal_high:.2f} | Size: {size} | SL: {sl:.2f} | TP: {upper_band:.2f}"
                    )
            ...
            # 🌑 SHORT ENTRY: Breakout below fractal low with bearish momentum
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🌑🌙 SHORT SIGNAL: {price:.2f} < {prev_fractal_low:.2f} | Size: {size} | SL: {sl:.2f} | TP: {lower_band:.2f}"
                    )
