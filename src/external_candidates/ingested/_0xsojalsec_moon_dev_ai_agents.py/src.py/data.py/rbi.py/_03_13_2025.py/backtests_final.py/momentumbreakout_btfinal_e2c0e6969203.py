# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_final\MomentumBreakout_BTFinal.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
...
class MomentumBreakout(Strategy):
    ...
    def init(self):
        ...
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=self.rsi_period, name="🌙 RSI 50"
        )
        self.ma = self.I(
            talib.SMA, self.data.Close, timeperiod=self.ma_period, name="🌙 MA 20"
        )
        self.atr = self.I(
            talib.ATR,
            self.data.High,
        ...
        self.resistance = self.I(
            talib.MAX, self.data.High, timeperiod=20, name="🌙 Resistance"
        )
    ...
    def next(self):
        ...
        print(
            (
                f"🌙 Moon Dev System Status | RSI: {self.rsi[-1]:.1f} | "
        ...
        if not self.position:
            ...
            if (
                self.rsi[-1] < 30
                and price > self.resistance[-1]
                and self.data.Close[-2] <= self.resistance[-2]
            ):
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        print(
                            (
                                f"🚀🌕 MOON SHOT! Entering {position_size} units at {entry_price:.1f} | "
