# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\GapMomentumReversal_BT.py
...
class GapMomentumReversal(Strategy):
    ...
    def init(self):
        ...
        self.top_band = self.I(
            ta.percentile, self.daily_range, length=self.lookback_period, q=80
        )
        self.bottom_band = self.I(
            ta.percentile, self.daily_range, length=self.lookback_period, q=20
        )
    ...
    def next(self):
        ...
        print(
            f"🌙 Range: {current_range:.2f} | Top: {current_top:.2f} | Bottom: {current_bottom:.2f}"
        )
    ...
    def enter_trade(self, direction):
        ...
        if position_size > 0:
            if direction == "long":
                ...
                print(
                    f"🚀 MOON DEV LONG ENTRY 🌕 | Size: {position_size} @ {price:.2f}"
                )
            else:
                self.sell(size=position_size, sl=sl_price, tp=tp_price)
                print(
                    f"🚀 MOON DEV SHORT ENTRY 🌑 | Size: {position_size} @ {price:.2f}"
                )
