# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests\DivergentReversion_BT.py
...
class DivergentReversion(Strategy):
    ...
    def next(self):
        ...
        if not self.position and current_rsi > rsi_ma and current_stoch < stoch_ma:
            self.sell(size=position_size, sl=price * 1.03, tp=price - 10)
        ...
        # Long Entry: Oversold Divergence 🌕
            ...
            self.buy(size=position_size, sl=price * 0.97, tp=price + 50)
        ...
        if self.position:
            ...
            if (
                ...
                and (
                    crossover(self.rsi, self.rsi_ma)
                    or crossover(self.stoch_k, self.stoch_ma)
                )
            ) or (
                self.position.is_long
                and (
                    crossover(self.rsi_ma, self.rsi)
                    or crossover(self.stoch_ma, self.stoch_k)
                )
            ):

                self.position.close()
                print(f"🌗 CLOSING POSITION @ {self.data.Close[-1]:.2f}")
