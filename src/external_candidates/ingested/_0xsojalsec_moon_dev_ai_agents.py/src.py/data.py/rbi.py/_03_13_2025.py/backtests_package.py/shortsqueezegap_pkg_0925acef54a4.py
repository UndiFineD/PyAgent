...
class ShortSqueezeGap(Strategy):
    ...
    def init(self):
        ...
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=20, name="Vol SMA 20"
        )
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=self.rsi_period, name="RSI 14"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # 🌟 ENTRY CONDITIONS 🌟
            gap_down = self.data.Open[-1] < self.prev_close[-1] * (
                1 - self.gap_threshold
            )
            volume_spike = (
                self.data.Volume[-1] > self.vol_sma[-1] * self.volume_multiplier
            )
