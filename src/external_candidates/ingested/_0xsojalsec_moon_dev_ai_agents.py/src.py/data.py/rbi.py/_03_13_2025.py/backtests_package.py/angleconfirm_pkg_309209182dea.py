# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\AngleConfirm_PKG.py
...
class AngleConfirm(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                self.short_gann[-2] < self.long_gann[-2]
                and self.short_gann[-1] > self.long_gann[-1]
            ) and sma5_up:
    ...
    def enter_trade(self, direction):
        ...
        if direction == "long":
            ...
            print(
                f"🌙🚀 BULLISH ANGLE CONFIRMATION 🚀🌙 | Entry: {entry_price:.2f} | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
            )
        else:
            self.sell(size=position_size, sl=sl_price, tp=tp_price)
            print(
                f"🌙🌧️ BEARISH ANGLE CONFIRMATION 🌧️🌙 | Entry: {entry_price:.2f} | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
            )
    ...
    def notify_trade(self, trade):
        if trade.is_closed:
            profit = trade.pl_pct
            emoji = (
                "🚀🌙 PROFIT MOONSHOT 🌙🚀" if profit > 0 else "🌧️🌙 RAIN CHECK 🌙🌧️"
            )
