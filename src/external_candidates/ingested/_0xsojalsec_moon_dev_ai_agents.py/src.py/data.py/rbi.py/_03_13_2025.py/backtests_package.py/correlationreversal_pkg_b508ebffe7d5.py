# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_package\CorrelationReversal_PKG.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
...
class CorrelationReversal(Strategy):
    ...
    def init(self):
        ...
        def correlate(vix_series, close_series, timeperiod):
            ...
            correlated_values = np.corrcoef(
                close_series.values[-timeperiod:], values.values
            )[-1, 0]
            return correlated_values

        self.correlation = self.I(
            correlate, self.vix, self.data.Close, 20, name="CORREL"
        )
        ...
        self.daily_high = self.I(daily_high, self.data.High, 20, name="Daily High")
        ...
        self.next_day_low = self.I(
            next_day_extreme, self.data.Low, 20, talib.MIN, name="Next Day Low"
        )

        self.next_day_high = self.I(
            next_day_extreme, self.data.High, 20, talib.MAX, name="Next Day High"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                abs(self.correlation[-1]) <= 0.05
                and self.data.High[-1] >= self.daily_high[-1]
            ):
                ...
                if stop_loss > current_close:
                    ...
                    print(
                        f"   Entry: {current_close:.2f} ✨ SL: {stop_loss:.2f} 🌙 TP: {take_profit:.2f}"
                    )
                    self.sell(
                        size=position_size,
                        sl=stop_loss,
            ...
            if self.position.pl <= -self.position.size * (
