# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_13_2025\backtests_final\VolatilityBreakout_BTFinal.py
...
class VolatilityBreakout(Strategy):
    def init(self):
        ...
        self.stddev = self.I(
            talib.STDDEV, self.data.Close, timeperiod=20, nbdev=1, name="STDDEV"
        )
        ...
        self.band_width = self.I(
            lambda: self.bb_upper - self.bb_lower, name="BAND_WIDTH"
        )
        self.avg_band_width = self.I(
            talib.SMA, self.band_width, timeperiod=20, name="AVG_BAND_WIDTH"
        )

        print(
            "🌙✨ Moon Dev Indicators Initialized! SMA20, BBANDS, Volatility Ready! ✨"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if current_volatility < avg_volatility and close_below_lower:
                ...
                if risk_per_share <= 0:
                    print(
                        "🌙⚠️ Moon Dev Warning: Negative/Zero Risk Per Share - Trade Skipped!"
                    )
                    return
                ...
                position_size = int(
                    round(position_size)
                )  # 🌙 Ensure whole number units

                if position_size <= 0:
                    print(
                        "🌙⚠️ Moon Dev Warning: Invalid Position Size - Trade Skipped!"
                    )
                    return
                ...
                print(
                    f"🌙🚀 MOON DEV SHORT ENTRY! Size: {position_size} @ {entry_price:.2f} | SL: {entry_bb_upper:.2f} 🚀"
                )
        ...
        # 🌙 Check for exit conditions on existing short position
            ...
            if self.data.High[-1] >= self.bb_upper[-1]:
                self.position.close()
                print(
                    f"🌙🎯 MOON DEV EXIT! Price Touched Upper Band @ {self.data.Close[-1]:.2f} 🎯"
                )
