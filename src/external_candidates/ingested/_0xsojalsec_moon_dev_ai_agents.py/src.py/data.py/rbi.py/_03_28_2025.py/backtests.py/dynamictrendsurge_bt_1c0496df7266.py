...
class DynamicTrendSurge(Strategy):
    ...
    def init(self):
        ...
        self.ema200 = self.I(
            talib.EMA, self.data.Close, timeperiod=self.ema_period_slow
        )
        self.adx = self.I(
            talib.ADX,
            self.data.High,
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if ema_crossover and adx_strong:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ BUY SIGNAL! Size: {position_size} | Entry: {self.data.Open[-1]:.2f} 🚀"
                    )
        ...
        # 🌑 EXIT LOGIC: RSI Overbought or Trend Weakness
            ...
            if current_rsi > 70:
                self.position.close()
                print(
                    f"🌑✨ SELL SIGNAL! RSI {current_rsi:.1f} >70 | Exit: {self.data.Open[-1]:.2f}"
                )
            elif current_adx < 20:
                self.position.close()
                print(
                    f"🌑✨ SELL SIGNAL! ADX {current_adx:.1f} <20 | Exit: {self.data.Open[-1]:.2f}"
                )
