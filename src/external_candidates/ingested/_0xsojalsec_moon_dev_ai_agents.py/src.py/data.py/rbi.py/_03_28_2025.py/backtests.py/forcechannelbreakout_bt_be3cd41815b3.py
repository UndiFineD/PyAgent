# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests\ForceChannelBreakout_BT.py
...
class ForceChannelBreakout(Strategy):
    def init(self):
        ...
        self.fi = self.I(
            talib.FORCE, self.data.Close, self.data.Volume, 13, name="ForceIndex"
        )
        self.ema20 = self.I(talib.EMA, self.data.Close, 20, name="EMA20")
        self.atr20 = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, 20, name="ATR20"
        )
        self.upper_kc = self.I(
            lambda ema, atr: ema + 1.5 * atr, self.ema20, self.atr20, name="UpperKC"
        )
        self.lower_kc = self.I(
            lambda ema, atr: ema - 1.5 * atr, self.ema20, self.atr20, name="LowerKC"
        )
    ...
    def next(self):
        ...
        if not self.position and all(
            ...
            if (entry_price - final_stop) > 0:
                ...
                self.buy(
                    size=position_size, sl=final_stop, tp=tp_level, tag="MoonRocket"
                )
                print(
                    f"🚀 LIFTOFF! Long {position_size} @ {entry_price:.2f} | SL: {final_stop:.2f} | TP: {tp_level:.2f}"
                )

        # 🌒 Exit Black Hole 🌒
        elif self.position and fi_now < 0 and fi_prev >= 0:
            self.position.close()
