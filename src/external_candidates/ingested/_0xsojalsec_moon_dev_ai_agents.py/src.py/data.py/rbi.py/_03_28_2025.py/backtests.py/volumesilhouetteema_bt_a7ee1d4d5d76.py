# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests\VolumeSilhouetteEMA_BT.py
...
class VolumeSilhouetteEMA(Strategy):
    ...
    def init(self):
        ...
        self.swing_low = self.I(
            talib.MIN, self.data.Low, timeperiod=self.swing_low_period
        )

        # Volume analysis using 5-period MA
        self.vol_ma5 = self.I(
            talib.MA, self.data.Volume, timeperiod=5, name="Volume MA5"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            vol_decline = (
                self.data.Volume[-3] > self.data.Volume[-2]
                and self.data.Volume[-2] > self.data.Volume[-1]
            )
...
bt = Backtest(data, VolumeSilhouetteEMA, cash=1_000_000, commission=0.002)
stats = bt.run()
