# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests_package\VolumetricBreakout_PKG.py
...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        ...
        self.volume_pct = self.I(
            ta.percentile, self.data.Volume, length=20, percentile=90
        )
    ...
    def next(self):
        ...
        if not self.position and price > upper and volume > vol_pct:
            ...
            if risk_per_share <= 0:
                print(
                    f"🌙 Invalid stop {final_stop:.2f} for entry {entry_price:.2f} ✖️"
                )
                return
            ...
            if position_size > 0:
                ...
                print(
                    f"🚀 BLASTOFF! Long {position_size} @ {entry_price:.2f} | Stop: {final_stop:.2f} 🌕"
                )
        ...
        if self.position:
            ...
            if (self.rsi[-2] < 70 and self.rsi[-1] >= 70) or price < self.upper_band[
                -1
            ]:
                self.position.close()
                print(
                    f"🌙 RETURN TO BASE | Exit @ {price:.2f} | RSI: {rsi_value:.1f} 🎯"
                )
