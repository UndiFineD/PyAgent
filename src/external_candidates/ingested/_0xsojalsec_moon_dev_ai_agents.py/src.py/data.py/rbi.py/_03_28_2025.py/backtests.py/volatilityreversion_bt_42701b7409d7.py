# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests\VolatilityReversion_BT.py
import pandas as pd
import talib
...
class VolatilityReversion(Strategy):
    ...
    def next(self):
            ...
            if vix_backwardation:
                ...
                if current_close > self.upper_band[-1]:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙 MOON DEV SHORT SIGNAL! Sold {position_size} units at {current_close} ✨"
                        )
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀 MOON DEV LONG SIGNAL! Bought {position_size} units at {current_close} 🌕"
                        )
                        self.entry_bar = len(self.data)
        ...
        # Exit logic ⏳
            ...
            if self.position.is_long and current_close >= (
                current_sma + exit_threshold
            ):
                exit_condition = True
...
# Run backtest 💻
bt = Backtest(data, VolatilityReversion, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
