# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests_package\VolSqueezeSurge_PKG.py
import numpy as np
import pandas as pd
import talib
...
class VolSqueezeSurge(Strategy):
    ...
    def init(self):
        ...
        self.oi_ma = self.I(
            talib.SMA, self.data.OpenInterest, timeperiod=960
        )  # 10-day MA (960 periods)
        self.atr = self.I(
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if volatility_exit or bb_exit:
                self.position.close()
                reason = (
                    "Volatility Expansion" if volatility_exit else "BB Width Expansion"
                )
                print(
                    f"🌕 MOON DEV EXIT 🛑 | Price: {current_close:.2f} | Reason: {reason}"
                )
