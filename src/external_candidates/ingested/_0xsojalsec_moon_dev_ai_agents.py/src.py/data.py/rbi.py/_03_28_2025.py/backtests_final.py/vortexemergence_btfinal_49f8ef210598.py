# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests_final\VortexEmergence_BTFinal.py
import pandas as pd
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                if risk_per_share > 0:
                    position_size = int(
                        round((self.equity * self.risk_per_trade) / risk_per_share)
                    )
                    if position_size > 0:
        ...
        # Galactic Exit Protocol 🌌
            ...
            if self.choppiness[-1] > 65:
                self.position.close()
                print(
                    f"✨🌙 STARLIGHT RETREAT! Exit: {price:.2f} | Profit: {self.position.pl_pct:.2%}"
                )
