# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests\BandwidthSpike_BT.py
...
class BandwidthSpike(Strategy):
    ...
    def next(self):
        ...
        if len(self.data) % 500 == 0:
            ...
            print(
                f"🌙 Moon Pulse | Bar {len(self.data)} | BBW: {current_bbw:.4f} | ATR: {current_atr:.2f} | Equity: {self.equity:,.2f} ✨"
            )
            ...
            if (self.bbw[-1] == self.bbw_low[-1]) and (
                ...
                if atr_value > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀 Lunar Launch! Entry @ {self.data.Close[-1]} | Size: {position_size} ✨"
                        )
        ...
        # 🌙 Exit Logic
            ...
            if self.position.is_long:
                ...
                if self.data.Low[-1] < stop_price:
                    ...
                    print(
                        f"🌑 Moon Retreat! Exit @ {self.data.Close[-1]} | Trailing Stop: {stop_price:.2f} ✨"
                    )
...
# =====================
bt = Backtest(
    data, BandwidthSpike, cash=1_000_000, commission=0.002, exclusive_orders=True
)
