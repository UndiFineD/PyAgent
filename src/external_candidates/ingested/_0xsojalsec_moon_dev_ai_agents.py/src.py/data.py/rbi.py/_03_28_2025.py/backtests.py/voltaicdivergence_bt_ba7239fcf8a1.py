...
class VoltaicDivergence(Strategy):
    ...
    def init(self):
        ...
        self.atr = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, self.keltner_atr
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.adx[i - 1] > 30:
                ...
                if (
                    self.data.Low[i - 1] < self.data.Low[i - 2]
                    and self.obv[i - 1] > self.obv[i - 2]
                ):
                    if current_close > self.data.High[i - 1]:
                        ...
                        if risk_per_share > 0:
                            ...
                            print(
                                f"🌙🚀 BULLISH DIVERGENCE! Buying {size} @ {current_close:.2f}"
                            )
                            self.buy(size=size, sl=sl_price)

                # Bearish divergence detection
                elif (
                    self.data.High[i - 1] > self.data.High[i - 2]
                    and self.obv[i - 1] < self.obv[i - 2]
                ):
                    if current_close < self.data.Low[i - 1]:
                        ...
                        if risk_per_share > 0:
                            size = int(round(risk_amount / risk_per_share))
                            print(
                                f"🌙🚀 BEARISH DIVERGENCE! Shorting {size} @ {current_close:.2f}"
                            )
