# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests\ChaikinSqueeze_BT.py
import pandas as pd
import talib
...
class ChaikinSqueeze(Strategy):
    ...
    def init(self):
        ...
        self.upper_band = self.I(
            lambda c: talib.BBANDS(c, 20, 2, 2, 0)[0], self.data.Close
        )
        self.middle_band = self.I(
            lambda c: talib.BBANDS(c, 20, 2, 2, 0)[1], self.data.Close
        )
        self.lower_band = self.I(
            lambda c: talib.BBANDS(c, 20, 2, 2, 0)[2], self.data.Close
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if cmf_cross and squeeze_condition:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ BUY SIGNAL! Size: {position_size} @ {entry_price} | Equity: {self.equity:,.2f} 🌌"
                    )
        ...
        if self.position:
            ...
            if self.data.High[-1] >= self.upper_band[-1]:
                self.sell()
                print(
                    f"🎯 PROFIT TAKEN @ {self.upper_band[-1]:.2f} | PNL: {self.position.pl:.2f} 🌕"
                )
            ...
            if self.cmf[-2] > 0 and self.cmf[-1] < 0:
                self.sell()
                print(
                    f"🛑 CMF STOP TRIGGERED! Exiting @ {self.data.Close[-1]:.2f} | PNL: {self.position.pl:.2f} 🌘"
                )
...
# Launch backtest 🚀
bt = Backtest(data, ChaikinSqueeze, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
