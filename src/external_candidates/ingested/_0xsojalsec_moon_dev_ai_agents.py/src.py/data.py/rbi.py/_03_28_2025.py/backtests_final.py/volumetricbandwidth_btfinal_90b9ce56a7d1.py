# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_28_2025\backtests_final\VolumetricBandwidth_BTFinal.py
import numpy as np
import pandas as pd
import talib
...
class VolumetricBandwidth(Strategy):
    def init(self):
        ...
        print(
            "🌙 VolumetricBandwidth initialized! Moon Dev engines ready for launch! 🚀"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                self.vw_macd[-2] < self.vw_signal[-2]
                and self.vw_macd[-1] > self.vw_signal[-1]
            ) and bandwidth < 1:
                ...
                if stop_loss >= entry_price:
                    print(
                        f"🌙 ABORT MISSION! Invalid SL {stop_loss:.2f} ≥ entry {entry_price:.2f}"
                    )
                    return
                ...
                if position_size > 0:  # Ensure valid position size
                    ...
                    print(
                        f"🚀 LIFTOFF! Long {position_size} units @ {entry_price:.2f} | SL: {stop_loss:.2f} 🌕"
                    )
        else:
            # Exit conditions 🌑
            ...
            if (
                (
                    self.vw_signal[-2] < self.vw_macd[-2]
                    and self.vw_signal[-1] > self.vw_macd[-1]
                )
                or bandwidth > 3
                or time_in_trade >= 5
            ):
                self.position.close()
                print(
                    f"🌑 EXIT SIGNAL! {'Timeout' if time_in_trade >=5 else 'Signal/Bandwidth'} | Price: {self.data.Close[-1]:.2f} 🌘"
                )
