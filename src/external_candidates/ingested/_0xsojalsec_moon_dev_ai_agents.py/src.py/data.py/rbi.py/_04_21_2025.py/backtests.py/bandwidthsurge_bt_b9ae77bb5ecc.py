# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_21_2025\backtests\BandwidthSurge_BT.py
import pandas as pd
import talib
...
class BandwidthSurge(Strategy):
    def init(self):
        ...
        self.bb_width = self.I(
            lambda u, l: (u - l) / self.I(talib.SMA, self.data.Close, 20),
            self.bb_upper,
            self.bb_lower,
        )
        self.bb_low = self.I(talib.MIN, self.bb_width, 17280)

        # Volume and ATR indicators
        self.vol_sma = self.I(talib.SMA, self.data.Volume, 1920)
