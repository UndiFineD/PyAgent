# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_21_2025\backtests_final\VolatilityDivergence_BTFinal.py
import numpy as np
import pandas as pd
import talib as TA
...
class VolatilityDivergence(Strategy):
    def init(self):
        ...
        self.bb_upper, self.bb_middle, self.bb_lower = self.I(
            TA.BBANDS, self.data.Close, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0
        )
    ...
    def next(self):
        if self.position:
            ...
            if current_profit >= 2 * abs(risk):
                ...
                print(f"🌕 Trailing SL to breakeven! {entry:.2f}")
            return
        ...
        div_condition = (self.data.High[-1] > max(price_highs)) and (
            self.cmf[-1] < max(cmf_values)
        )
        ...
        if (
            ...
            print(
                f"🌑 SHORT ENTRY! Size: {position_size} ✨ Entry: {entry_price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
            )


bt = Backtest(data, VolatilityDivergence, cash=1000000, commission=0.002)
stats = bt.run()
print(stats)
