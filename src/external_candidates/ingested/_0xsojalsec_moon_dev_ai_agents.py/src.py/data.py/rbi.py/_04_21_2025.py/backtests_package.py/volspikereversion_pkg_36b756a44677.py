# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_21_2025\backtests_package\VolSpikeReversion_PKG.py
import pandas as pd
import talib
...
class VolSpikeReversion(Strategy):
    def init(self):
        ...
        def bb_upper(data):
            upper, _, _ = talib.BBANDS(
                data, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0
            )
            return upper
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                (vvix_below_sma and term_contango and vol_exit)
                or vvix_5d_high
                or time_exit
            ):
                self.position.close()
                print(
                    f"🌙 MOON DEV EXIT | Reason: {'Signal' if not vvix_5d_high else 'Stop'} 🌟"
                )


# Run backtest
bt = Backtest(data, VolSpikeReversion, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
