# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_06_2025\backtests\SigmaSqueeze_BT.py
import pandas as pd
...
class SigmaSqueeze(Strategy):
    def init(self):
        ...
        self.lower_band = self.I(
            lambda: self.mean_close - 3 * self.std_dev, name="3σ Lower"
        )
        ...
        self.bb_upper, self.bb_middle, self.bb_lower = self.I(
            talib.BBANDS, self.data.Close, 20, 2, 2, name="BBANDS"
        )
        self.bb_width = self.I(
            lambda: (self.bb_upper - self.bb_lower) / self.bb_middle, name="BB Width"
        )
        ...
        self.vwap = self.I(
            lambda: tpv.cumsum() / self.data.Volume.cumsum(), name="VWAP"
        )
    ...
    def next(self):
            ...
            if sigma_trigger and volume_spike and bb_squeeze:
                ...
                if stop_distance > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 MOON SHOT! Long {position_size} units at {price:.2f} | SL: {price-stop_distance:.2f} ✨"
                        )
        ...
        # Exit conditions
            ...
            if vwap_cross or self.rsi[-1] > 65:
                self.position.close()
                print(
                    f"🌙💎 Profit Capture! Closing at {price:.2f} ({'VWAP' if vwap_cross else 'RSI'}) 🛑"
                )


# Execute backtest
bt = Backtest(data, SigmaSqueeze, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
