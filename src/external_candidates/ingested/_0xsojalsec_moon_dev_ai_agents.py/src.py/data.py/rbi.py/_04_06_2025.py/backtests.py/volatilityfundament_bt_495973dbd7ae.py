# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_06_2025\backtests\VolatilityFundament_BT.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        if self.position:
            ...
            if current_atr < median_atr:
                print(
                    f"🌙🌌 Low Volatility Alert: ATR {current_atr:.2f} < Median {median_atr:.2f}"
                )
                return
            ...
            if self.data.Close[-1] > vsma and funding <= -0.0001:
                ...
                print(
                    f"🚀🌙 LIFTOFF! Long {position_size} units | Entry: {entry_price:.2f} | SL: {stop_loss:.2f}"
                )
