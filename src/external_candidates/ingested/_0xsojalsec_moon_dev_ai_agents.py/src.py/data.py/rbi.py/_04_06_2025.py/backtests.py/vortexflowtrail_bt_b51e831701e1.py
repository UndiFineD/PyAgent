# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_06_2025\backtests\VortexFlowTrail_BT.py
import pandas as pd
...
class VortexFlowTrail(Strategy):
    ...
    def init(self):
        ...
        print(
            f"VI Period: {self.vi_period}, CMF Period: {self.cmf_period}, ATR Multiplier: {self.atr_multiplier}"
        )
        ...
        if not self.position:
            ...
            if vi_cross and cmf_cross:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"Entry: {self.data.Close[-1]:.2f}, Initial Stop: {self.trailing_stop:.2f}"
                    )
...
bt = Backtest(data, VortexFlowTrail, cash=1_000_000, commission=0.002)
stats = bt.run()
