# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_06_2025\backtests\VolSyncBreakout_BT.py
    ...
    def next(self):
            ...
            if bbw_contraction and obv_bullish and price_breakout:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙 MOON DEV ENTRY 🌙 | Price: {self.data.Close[-1]:.2f} | Size: {position_size} | SL: {stop_loss:.2f} | BBW: {self.entry_bbw:.4f}"
                        )
        ...
        # ✨ Exit Conditions ✨
            ...
            if self.bbw[-1] >= 2 * self.entry_bbw:
                self.position.close()
                print(
                    f"✨ MOON DEV VOL EXPANSION EXIT ✨ | Price: {self.data.Close[-1]:.2f} | BBW: {self.bbw[-1]:.4f}"
                )
...
# 🌖 Launch Backtest 🌖
bt = Backtest(data, VolSyncBreakout, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
