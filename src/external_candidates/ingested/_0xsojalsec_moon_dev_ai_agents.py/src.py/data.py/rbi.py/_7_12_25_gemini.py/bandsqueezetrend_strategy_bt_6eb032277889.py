# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_12_25_gemini\BandSqueezeTrend_strategy_bt.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.data.Close[-1] < self.middle[-1]:
                print(
                    "🌙 Moon Dev price fell below middle band! Closing long position. ✨"
                )
                self.position.close()
            return
        ...
        is_breakout = (
            ...
            and self.data.Close[-2] <= self.upper[-2]
        )
...
if __name__ == "__main__":
    ...
    bt = Backtest(
        data, BandSqueezeTrendStrategy, cash=1000000, margin=1, commission=0.001
    )
    stats = bt.run()
    print(stats)
