# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_package\VortexFisherDivergence_PKG.py
import pandas as pd
...
class VortexFisherDivergence(Strategy):
    ...
    def next(self):
        if not self.position:
            ...
            # Bullish entry
            if (
                (self.fisher[-1] > 0 and self.fisher[-2] <= 0)
                and (self.vi_minus[-1] > self.vi_plus[-1])
                and vol_cond
                and adx_cond
            ):
                sl = self.swing_low[-1]
                ...
                size = (
                    int(round(risk / (self.data.Open[-1] - sl)))
                    if self.data.Open[-1] > sl
                    else 0
                )
                if size > 0:
                    self.buy(size=size, sl=sl)
                    print(
                        f"🚀 Moon Dev LONG: {self.data.Close[-1]} | Size: {size} | SL: {sl}"
                    )

            # Bearish entry
            elif (
                (self.fisher[-1] < 0 and self.fisher[-2] >= 0)
                and (self.vi_plus[-1] > self.vi_minus[-1])
                and vol_cond
                and adx_cond
            ):
                ...
                risk = self.equity * 0.01
                size = (
                    int(round(risk / (sl - self.data.Open[-1])))
                    if sl > self.data.Open[-1]
                    else 0
                )
                if size > 0:
                    self.sell(size=size, sl=sl)
                    print(
                        f"🌑 Moon Dev SHORT: {self.data.Close[-1]} | Size: {size} | SL: {sl}"
                    )
        else:
            ...
            if self.position.is_long:
                if (
                    current_adx < 25
                    or current_adx < prev_adx
                    or (self.fisher[-1] < 0 and self.fisher[-2] >= 0)
                ):
                    self.position.close()
