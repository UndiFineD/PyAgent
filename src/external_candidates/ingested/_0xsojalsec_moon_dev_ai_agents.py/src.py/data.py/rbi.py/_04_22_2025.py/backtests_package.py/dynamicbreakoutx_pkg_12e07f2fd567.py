# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_package\DynamicBreakoutX_PKG.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        if not self.position:
            if (
                ...
                if atr_value != 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 LONG ENTRY @ {self.data.Close[-1]} | Size: {position_size} | Moon Power Activated!"
                        )
                ...
                if atr_value != 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"📉🌙 SHORT ENTRY @ {self.data.Close[-1]} | Size: {position_size} | Dark Side Engaged!"
                        )
            ...
            if self.data.Low[-1] <= stop_price:
                self.position.close()
                print(
                    f"✨🚀 EXIT LONG @ {self.data.Close[-1]} | Cosmic Profits Captured!"
                )
            ...
            if self.data.High[-1] >= stop_price:
                self.position.close()
                print(
                    f"✨📉 EXIT SHORT @ {self.data.Close[-1]} | Stellar Losses Avoided!"
                )
