# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests\MomentumBreakout_BT.py
import pandas as pd
import talib
        ...
        if not self.position:
            if (self.data.Close[-1] > self.donchian_upper[-1]) and (
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀 Moon Boost! LONG {position_size} @ {self.data.Close[-1]:.2f} | MACD Rising 🌛"
                    )
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌒 Moon Dip! SHORT {position_size} @ {self.data.Close[-1]:.2f} | MACD Falling 🌜"
                    )
