# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests\LiquidationReversal_BT.py
import pandas as pd
import talib
...
class LiquidationReversal(Strategy):
    def init(self):
        ...
        self.swing_high = self.I(
            talib.MAX, data.High, timeperiod=self.swing_period, name="Swing High"
        )
        self.swing_low = self.I(
            talib.MIN, data.Low, timeperiod=self.swing_period, name="Swing Low"
        )
    ...
    def next(self):
        ...
        if not self.position:
            # Long entry logic
            if (
                self.data.Low[-1] < swing_low
                and price > swing_low
                and bb_width < bb_percentile
            ):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ MOON DEV LONG SIGNAL! Entry: {price:.2f} | Size: {position_size} | SL: {sl:.2f} 🚀"
                    )

            # Short entry logic
            elif (
                self.data.High[-1] > swing_high
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙✨ MOON DEV SHORT SIGNAL! Entry: {price:.2f} | Size: {position_size} | SL: {sl:.2f} 🚀"
                    )
...
# Run backtest
bt = Backtest(data, LiquidationReversal, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
