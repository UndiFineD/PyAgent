# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_final\DynamicBreakoutX_BTFinal.py
import numpy as np
import pandas as pd
import talib
...
class DynamicBreakoutX(Strategy):
    ...
    def init(self):
        self.donchian_high = self.I(
            talib.MAX, self.data.High, timeperiod=self.donchian_period
        )
        self.donchian_low = self.I(
            talib.MIN, self.data.Low, timeperiod=self.donchian_period
        )
        self.adx = self.I(
            talib.ADX,
            self.data.High,
    ...
    def next(self):
        if not self.position:
            if (
                ...
                if atr_value != 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 LONG ENTRY @ {self.data.Close[-1]:.2f} | Size: {position_size} | Moon Power Activated!"
                        )
                ...
                if atr_value != 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"📉🌙 SHORT ENTRY @ {self.data.Close[-1]:.2f} | Size: {position_size} | Dark Side Engaged!"
                        )
            ...
            if self.data.Low[-1] <= stop_price:
                self.position.close()
                print(
                    f"✨🚀 EXIT LONG @ {self.data.Close[-1]:.2f} | Cosmic Profits Captured!"
                )
            ...
            if self.data.High[-1] >= stop_price:
                self.position.close()
                print(
                    f"✨📉 EXIT SHORT @ {self.data.Close[-1]:.2f} | Stellar Losses Avoided!"
                )
...
bt = Backtest(data, DynamicBreakoutX, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
