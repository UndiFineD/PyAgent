# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_final\MomentumBreakout_BTFinal.py
import pandas as pd
import talib
...
class MomentumBreakout(Strategy):
    ...
    def init(self):
        ...
        self.peak = None
        self.trough = None  # Added missing variable initialization
    ...
    def next(self):
        ...
        if self.position.is_long:
            ...
            if self.data.Low[-1] <= self.trailing_stop:
                self.position.close()
                print(
                    f"🌙 Moon Exit! Closed LONG at {self.data.Close[-1]:.2f} | ATR Trail: {self.trailing_stop:.2f} ✨"
                )
            ...
            if self.data.High[-1] >= self.trailing_stop:
                self.position.close()
                print(
                    f"🌙 Moon Exit! Closed SHORT at {self.data.Close[-1]:.2f} | ATR Trail: {self.trailing_stop:.2f} ✨"
                )

        # Entry logic
        if not self.position:
            if (self.data.Close[-1] > self.donchian_upper[-1]) and (
                macd_current > macd_prev
            ):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀 Moon Boost! LONG {position_size} units @ {self.data.Close[-1]:.2f} | MACD Rising 🌛"
                    )

            elif (self.data.Close[-1] < self.donchian_lower[-1]) and (
                macd_current < macd_prev
            ):
                stop_distance = 2 * atr_value
                position_size = int(round(risk_amount / stop_distance))
                if position_size > 0:
                    ...
                    print(
                        f"🌒 Moon Dip! SHORT {position_size} units @ {self.data.Close[-1]:.2f} | MACD Falling 🌜"
                    )
