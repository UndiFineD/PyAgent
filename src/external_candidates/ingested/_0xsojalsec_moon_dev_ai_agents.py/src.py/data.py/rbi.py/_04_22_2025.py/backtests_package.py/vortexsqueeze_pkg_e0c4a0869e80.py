# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_package\VortexSqueeze_PKG.py
import numpy as np
import pandas as pd
...
class VortexSqueeze(Strategy):
    def init(self):
        ...
        self.vi_plus, self.vi_minus = self.I(
            calculate_vortex, self.data.High, self.data.Low, self.data.Close
        )
            ...
            if (
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙 MOON DEV LONG ENTRY 🌙 Size: {position_size} @ {entry_price:.2f} ✨"
                        )
...
# Execute backtest
bt = Backtest(data, VortexSqueeze, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
