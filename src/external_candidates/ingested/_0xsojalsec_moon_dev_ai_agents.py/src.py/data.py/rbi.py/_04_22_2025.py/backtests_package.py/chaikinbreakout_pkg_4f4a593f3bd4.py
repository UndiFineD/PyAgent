# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests_package\ChaikinBreakout_PKG.py
import pandas as pd
...
class ChaikinBreakout(Strategy):
    ...
    def next(self):
        ...
        has_cmf_confirmation = all(v > 0 for v in cmf_values) or all(
            v < 0 for v in cmf_values
        )

        if not self.position:
            # Long entry
            if self.data.High[-1] > self.high_20[-2] and all(v > 0 for v in cmf_values):
                ...
                if units > 0:
                    ...
                    print(
                        f"🌙🚀 BULLISH BREAKOUT! Long {units} units at {self.data.Close[-1]:.2f}"
                    )

            # Short entry
            elif self.data.Low[-1] < self.low_20[-2] and all(v < 0 for v in cmf_values):
                equity_risk = self.equity * self.risk_pct
                risk_per_unit = self.data.Close[-1] * self.stop_loss
                units = int(round(equity_risk / risk_per_unit))
                if units > 0:
                    ...
                    print(
                        f"🌙✨ BEARISH BREAKOUT! Short {units} units at {self.data.Close[-1]:.2f}"
                    )

        else:
            # Exit logic
            if self.position.is_long and (
                self.sma_5[-2] < self.data.Close[-2]
                and self.sma_5[-1] > self.data.Close[-1]
            ):
                self.position.close()
                print(f"🌙💎 CLOSING LONG at {self.data.Close[-1]:.2f} (SMA5 reversal)")
            elif self.position.is_short and (
                self.data.Close[-2] < self.sma_5[-2]
                and self.data.Close[-1] > self.sma_5[-1]
            ):
                self.position.close()
                print(
                    f"🌙💎 CLOSING SHORT at {self.data.Close[-1]:.2f} (SMA5 reversal)"
                )
