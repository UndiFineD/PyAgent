# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests\VolatilityConvergence_BT.py
import pandas as pd
import talib
...
class VolatilityConvergence(Strategy):
    def init(self):
        ...
        self.bandwidth = self.I(
            calc_bandwidth, self.bb_upper, self.bb_middle, self.bb_lower
        )
        ...
        self.daily_atr = self.I(
            daily_atr, self.data.High, self.data.Low, self.data.Close
        )
    ...
    def next(self):
        ...
        if (
            ...
            if position_size > 0:
                ...
                print(
                    f"🌙✨ MOON DEV LONG! Entry: {self.data.Close[-1]:.2f}, Size: {position_size}, TP: {tp:.2f}, SL: {sl:.2f} 🚀"
                )


bt = Backtest(data, VolatilityConvergence, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
