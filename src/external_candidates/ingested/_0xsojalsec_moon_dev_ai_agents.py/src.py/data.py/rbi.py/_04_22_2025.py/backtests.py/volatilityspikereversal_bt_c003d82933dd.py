# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_22_2025\backtests\VolatilitySpikeReversal_BT.py
import pandas as pd
import talib
...
class VolatilitySpikeReversal(Strategy):
    def init(self):
        ...
        self.std_dev20 = self.I(talib.STDDEV, self.data.Close, 20)
        self.ema20 = self.I(talib.EMA, self.data.Close, 20)
        self.atr20 = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, 20
        )
    ...
    def next(self):
        ...
        current_drawdown = (
            (self.daily_high_equity - self.equity) / self.daily_high_equity
        ...
        if not self.position:
            if (self.data.Close[-1] <= (self.sma20[-1] - 3 * self.std_dev20[-1])) and (
                self.data.funding_rate[-1] >= 0.001
            ):
                ...
                print(
                    f"🌙 MOON DEV ENTRY: Long {position_size} units at {self.data.Close[-1]} ✨"
                )
        else:
            ...
            if self.data.High[-1] >= current_upper_keltner:
                self.position.close()
                print(
                    f"🌙 MOON DEV PROFIT TAKEN: Keltner target {current_upper_keltner:.2f} 🎯"
                )
            elif self.data.Low[-1] <= self.stop_loss_price:
                self.position.close()
                print(
                    f"🌙 MOON DEV EMERGENCY EXIT: 4σ stop {self.stop_loss_price:.2f} 🚨"
                )
...
bt = Backtest(data, VolatilitySpikeReversal, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
