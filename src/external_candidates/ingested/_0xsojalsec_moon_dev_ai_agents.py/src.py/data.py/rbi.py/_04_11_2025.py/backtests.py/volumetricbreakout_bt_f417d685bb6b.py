# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_11_2025\backtests\VolumetricBreakout_BT.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
...
class VolumetricBreakout(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and current_volume > 2 * self.volume_ma[-1]
        ):
            ...
            )  # Never exceed available equity

            print(f"🌙 MOON SIGNAL: BUY TRIGGERED! Size: {position_size} @ {price:.2f}")
            print(
                f"🚀 RISK MANAGEMENT: SL={stop_loss_price:.2f}, RSI={self.rsi[-1]:.2f}"
            )
