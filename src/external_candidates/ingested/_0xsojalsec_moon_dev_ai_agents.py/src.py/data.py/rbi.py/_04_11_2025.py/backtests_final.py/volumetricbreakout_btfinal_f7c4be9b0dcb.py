...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        ...
        print(
            "✨ MOON INDICATORS INITIALIZED: BBANDS(20,2), SMA(20), RSI(14), ATR(14) ✨"
        )
    ...
    def next(self):
        ...
        if (
            ...
            if risk_per_unit > 0:  # Prevent division by zero
                ...
                position_size = int(
                    min(position_size, self.equity // price)
                )  # Convert to whole units

                print(
                    f"🌙 MOON SIGNAL: BUY TRIGGERED! Size: {position_size} units @ {price:.2f}"
                )
                print(
