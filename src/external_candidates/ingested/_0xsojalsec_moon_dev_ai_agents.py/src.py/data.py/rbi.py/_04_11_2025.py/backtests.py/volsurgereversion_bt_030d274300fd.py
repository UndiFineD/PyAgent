# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_11_2025\backtests\VolSurgeReversion_BT.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
...
class VolSurgeReversion(Strategy):
    ...
    def init(self):
        ...
        self.log_returns = self.I(
            lambda close: np.log(close / close.shift(1)),
            self.data.Close,
            name="LogReturns",
        )
        self.realized_vol = self.I(
            talib.STDDEV, self.log_returns, timeperiod=20, name="RealizedVol20"
        )
        self.volume_sma20 = self.I(
            talib.SMA, self.data.Volume, timeperiod=20, name="VolumeSMA20"
        )
        self.spread = self.I(
            lambda vix, rv: vix - rv, self.data.Vix, self.realized_vol, name="Spread"
        )
        self.spread_mean = self.I(
            talib.SMA, self.spread, timeperiod=252, name="SpreadMean1yr"
        )
        self.atr = self.I(
