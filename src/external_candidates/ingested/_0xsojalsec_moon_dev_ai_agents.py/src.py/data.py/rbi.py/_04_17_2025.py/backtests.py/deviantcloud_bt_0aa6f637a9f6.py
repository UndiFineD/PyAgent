# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_17_2025\backtests\DeviantCloud_BT.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
...
class DeviantCloud(Strategy):
    ...
    def init(self):
        ...
        self.funding_mean = self.I(
            lambda x: funding_series.rolling(2880).mean(), name="Fund30MA"
        )  # 30-day (2880*15m)
        self.funding_std = self.I(
            lambda x: funding_series.rolling(2880).std(), name="Fund30Std"
        )
        self.funding_upper = self.I(
            lambda x: self.funding_mean + 2 * self.funding_std, name="FundUpper"
        )
        ...
        if not self.position:
            ...
            if fund_condition and cloud_condition:
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"| Size: {position_size} | Risk: {self.risk_per_trade*100}% Equity"
                    )
