...
class LiquidationPulse(Strategy):
    ...
    def init(self):
        ...
        self.low_cluster = self.I(
            talib.MIN, self.data.Low, timeperiod=self.cluster_period
        )
        self.high_cluster = self.I(
            talib.MAX, self.data.High, timeperiod=self.cluster_period
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.funding_rate[-1] >= 0:
                self.position.close()
                print(
                    f"🌑 MOON DEV FUNDING RATE EXIT: Turned positive ({self.funding_rate[-1]:.6f})"
                )
                ...
                self.position.close()
                print(
                    f"🌓 MOON DEV RESISTANCE EXIT: Reached high cluster {self.high_cluster[-1]:.2f}"
                )
        else:
