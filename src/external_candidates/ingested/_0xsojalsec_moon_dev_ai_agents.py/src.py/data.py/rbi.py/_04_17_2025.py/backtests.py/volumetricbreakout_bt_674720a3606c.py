# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_17_2025\backtests\VolumetricBreakout_BT.py
...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        # 🌙 CORE INDICATORS
        self.ema50 = self.I(
            talib.EMA, self.data.Close, timeperiod=self.ema50_period, name="EMA50"
        )
        self.ema200 = self.I(
            talib.EMA, self.data.Close, timeperiod=self.ema200_period, name="EMA200"
        )
    ...
    def next(self):
        ...
        # 🛑 EXIT LOGIC
            ...
            if self.data.Low[-1] < current_stop:
                self.position.close()
                print(
                    f"🌙🛑 MOON DEV EXIT: Trail stop triggered at {current_stop:.2f}!"
                )
