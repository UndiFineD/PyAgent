# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests\DynamicTrendSync_BT.py
import numpy as np
import pandas as pd
import talib
...
class DynamicTrendSync(Strategy):
    ...
    def next(self):
        if not self.position:
            ...
            adx_strong = (self.adx[-1] > self.adx_threshold) and (
            ...
            # RSI exit
            if (
                current_rsi > self.rsi_exit_levels[0]
                or current_rsi < self.rsi_exit_levels[1]
            ):
                self.position.close()
                print(
                    f"🚨🌙 RSI EXIT at {self.data.Close[-1]:.2f} | RSI: {current_rsi:.2f}"
                )
