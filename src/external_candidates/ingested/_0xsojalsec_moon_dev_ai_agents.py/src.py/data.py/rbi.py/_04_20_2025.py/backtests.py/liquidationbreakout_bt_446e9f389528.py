# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests\LiquidationBreakout_BT.py
import pandas as pd
import talib
...
class LiquidationBreakout(Strategy):
    ...
    def init(self):
        self.ema = self.I(talib.EMA, self.data.Close, 20, name="EMA 20")
        self.atr = self.I(
            talib.ATR, self.data.High, self.data.Low, self.data.Close, 20, name="ATR 20"
        )
        ...
        if not self.position:
            ...
            if current_close > upper_band and self.data.High[-1] >= self.swing_high[-1]:
                ...
                if stop_distance > 0:
                    ...
                    print(
                        f"🌙🚀 MOON DEV LONG ENTRY @ {current_close:.2f}! Cluster Breakout Confirmed! TP: {self.swing_high[-1]:.2f} 🌕"
                    )
            ...
            # Short entry logic
                ...
                if stop_distance > 0:
                    ...
                    print(
                        f"🌙🌌 MOON DEV SHORT ENTRY @ {current_close:.2f}! Liquidation Zone Activated! TP: {self.swing_low[-1]:.2f} 🌠"
                    )


bt = Backtest(data, LiquidationBreakout, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
