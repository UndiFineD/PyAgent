# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests_package\VoltaicSurge_PKG.py
import pandas as pd
import talib
...
daily_df["daily_atr"] = talib.ATR(
    daily_df["High"], daily_df["Low"], daily_df["Close"], timeperiod=14
)
    ...
    def next(self):
        for trade in self.trades:
            if trade.is_long:
                if trade.sl is None:
                    ...
                    print(f"🌕 Moon Dev: Initial SL {sl:.2f} ✨")
                else:
                    ...
                    if current_high > trade.tag["high_water"]:
                        ...
                        if new_sl > trade.sl:
                            trade.sl = new_sl
                            print(f"🚀 Moon Dev: Trail SL to {new_sl:.2f} 🌙")
        ...
        if not self.position:
            ...
            if atr_15 > daily_atr * 1.5 and vol > vol_sma:
                ...
                if size > 0:
                    self.buy(
                        size=size, tag={"breakout_atr": atr_value, "high_water": None}
                    )
                    print(
                        f"🌙✨ Moon Dev Entry: {size} units @ {self.data.Open[-1]:.2f} 🚀"
                    )


bt = Backtest(data, VoltaicSurge, cash=1_000_000)
stats = bt.run()
print(stats)
