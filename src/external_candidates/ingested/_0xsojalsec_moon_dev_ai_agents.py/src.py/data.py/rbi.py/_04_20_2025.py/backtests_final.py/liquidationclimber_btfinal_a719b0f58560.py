# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests_final\LiquidationClimber_BTFinal.py
import numpy as np
import pandas as pd
import talib
        ...
        if len(self.trades) < self.max_positions and not self.position:
            if current_atr < atr_sma and liq > (liq_mean + 2 * liq_std):
                if self.data.Close[-1] > self.data.Close[-2]:
                    ...
                    print(
                        f"🚀 Moon Dev Long Entry: {position_size} units @ {self.data.Close[-1]:.2f}"
                    )
                    ...
                    self.take_profit = self.data.Close[-1] - 2 * current_atr
                    print(
                        f"🌒 Moon Dev Short Entry: {position_size} units @ {self.data.Close[-1]:.2f}"
                    )
