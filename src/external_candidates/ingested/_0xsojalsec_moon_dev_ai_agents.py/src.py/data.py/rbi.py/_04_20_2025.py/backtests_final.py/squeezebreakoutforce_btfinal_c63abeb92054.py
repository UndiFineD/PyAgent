# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_20_2025\backtests_final\SqueezeBreakoutForce_BTFinal.py
import numpy as np
import pandas as pd
import talib
...
class SqueezeBreakoutForce(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            efi_cross = (self.efi_13[-2] < self.efi_50[-2]) and (
                self.efi_13[-1] > self.efi_50[-1]
            )
            ...
            if squeeze and efi_cross and breakout:
                ...
                if size > 0:
                    print(
                        f"🌙✨ MOON DEATH STAR ACTIVATED! Buying {size} units at {price:.2f} 🚀"
                    )
            ...
            if self.data.Close[-1] < trail_stop:
                print(
                    f"🌙🚨 COSMIC STOP TRIGGERED! Selling at {self.data.Close[-1]:.2f} ✨"
                )
                self.position.close()
...
bt = Backtest(
    data, SqueezeBreakoutForce, cash=1_000_000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
