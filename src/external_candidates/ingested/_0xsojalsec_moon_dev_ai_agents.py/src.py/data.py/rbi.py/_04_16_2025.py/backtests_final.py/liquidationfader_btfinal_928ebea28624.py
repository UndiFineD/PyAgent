# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_16_2025\backtests_final\LiquidationFader_BTFinal.py
import numpy as np
import pandas as pd
import talib
            ...
            if self.atr[-1] > self.atr_ma[-1]:
                ...
                if entry_price < (self.swing_low[-1] - 2 * self.atr[-1]):
                    ...
                    if size > 0:
                        self.buy(size=size)
                        print(
                            f"🌙✨ MOON ALERT: LONG {size} units @ {entry_price:.2f} 🌛 ATR: {self.atr[-1]:.2f} 🚀"
                        )
                ...
                # Short setup 🌜
                    ...
                    if size > 0:
                        self.sell(size=size)
                        print(
                            f"🌙✨ MOON ALERT: SHORT {size} units @ {entry_price:.2f} 🌛 ATR: {self.atr[-1]:.2f} 🚀"
                        )
        else:
