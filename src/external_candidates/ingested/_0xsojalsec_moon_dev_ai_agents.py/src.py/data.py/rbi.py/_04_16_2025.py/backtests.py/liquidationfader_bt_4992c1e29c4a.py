# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_16_2025\backtests\LiquidationFader_BT.py
import numpy as np
import pandas as pd
import talib
...
class LiquidationFader(Strategy):
    ...
    def next(self):
        if not self.position:
            ...
            if np.isnan(self.atr[-1]) or np.isnan(self.swing_high[-1]):
                return
            ...
            if self.atr[-1] > self.atr_ma[-1]:
                ...
                if entry_price < (self.swing_low[-1] - 2 * self.atr[-1]):
                    ...
                    if size > 0:
                        self.buy(size=size)
                        print(
                            f"🌙✨ MOON ALERT: LONG {size} @ {entry_price:.2f} 🌛 ATR: {self.atr[-1]:.2f} 🚀"
                        )
                ...
                # Short setup 🌜
                    ...
                    if size > 0:
                        self.sell(size=size)
                        print(
                            f"🌙✨ MOON ALERT: SHORT {size} @ {entry_price:.2f} 🌛 ATR: {self.atr[-1]:.2f} 🚀"
                        )
        else:
...
# Launch backtest 🚀
bt = Backtest(data, LiquidationFader, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
