...
class VortexCompression(Strategy):
    ...
    def init(self):
        ...
        self.volume_ma = self.I(
            lambda: talib.SMA(self.data.Volume, 20), name="Volume_MA"
        )
    ...
    def next(self):
        if not self.position:
            ...
            if vi_crossover and volume_surge and volatility_contraction:
                ...
                if risk_per_share <= 0:
                    print(
                        f"🌑 Moon Dev Alert: Negative risk detected! Aborting launch!"
                    )
                    return
...
# Execute backtest 🌕
bt = Backtest(
    data, VortexCompression, cash=1_000_000, commission=0.002, trade_on_close=False
)
stats = bt.run()
