# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_27_2025\backtests\VolatilityReversal_BT.py
import pandas as pd
import talib
...
class VolatilityReversal(Strategy):
    ...
    def init(self):
        ...
        self.adx = self.I(
            talib.ADX, self.data.High, self.data.Low, self.data.Close, 14, name="ADX"
        )
    ...
    def next(self):
        ...
        if not self.position:
            if crossover(self.ema50, self.ema200):
                ...
                if current_adx < 20:
                    ...
                    if risk_per_share <= 0:
                        print(
                            f"🌙🚫 Invalid Risk: {risk_per_share:.2f}. Trade skipped."
                        )
                        return

                    position_size = int(
                        round((self.risk_pct * self.equity) / risk_per_share)
                    )

                    if position_size > 0:
                        self.buy(size=position_size, sl=sl_price)
                        print(
                            f"🚀🌙 MOON ROCKET LAUNCH! 🚀 | Entry: {entry_price:.2f} | Size: {position_size} | SL: {sl_price:.2f} | ADX: {current_adx:.2f}"
                        )
        ...
        # ✨ Exit Logic: High Volatility or Death Cross 🌙
            ...
            if current_adx > 30:
                self.position.close()
                print(
                    f"🌙💫 MOON LANDING! 💫 | Exit: {self.data.Close[-1]:.2f} | ADX: {current_adx:.2f}"
                )

            elif crossover(self.ema200, self.ema50):
                self.position.close()
                print(
                    f"🌙☄️ METEOR SHOWER EXIT! ☄️ | Price: {self.data.Close[-1]:.2f} | EMA50/200 Cross"
                )
