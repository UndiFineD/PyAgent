# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_27_2025\backtests\VolumetricBreakout_BT.py
import pandas as pd
import talib
...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        ...
        # Volume SMA
        self.volume_sma = self.I(talib.SMA, self.data.Volume, 20, name="Volume SMA")
        ...
        if not self.position:
            ...
            if price_break and volume_spike:
                ...
                if entry_price <= stop_price:
                    self._print(
                        f"🌑 Aborted: Stop {stop_price:.2f} ≥ Entry {entry_price:.2f}"
                    )
                    return
                ...
                if position_size > 0:
                    ...
                    self._print(
                        f"🚀 LIFTOFF! {position_size} units @ {entry_price:.2f}"
                    )
                else:
