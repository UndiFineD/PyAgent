# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\LiquidityArbitrage_BTFinal.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
...
class LiquidityArbitrage(Strategy):
    ...
    def init(self):
        ...
        self.avg_spread = self.I(
            talib.SMA, self.spread, timeperiod=20, name="Avg Spread"
        )
        self.avg_volume = self.I(
            talib.SMA, self.data.Volume, timeperiod=20, name="Avg Volume"
        )
        self.price_sma = self.I(
            talib.SMA, self.data.Close, timeperiod=20, name="Price SMA"
        )
    ...
    def next(self):
        ...
        if not self.position:
            if (
                current_spread > self.spread_multiplier * avg_spread
                and current_vol > avg_vol
                and current_close < price_sma
            ):
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙✨ MOON DEV LONG ENTRY: {position_size} units at {current_close:.2f} | SL: {stop_price:.2f} 🚀"
                        )
        ...
        # 🛑 EXIT LOGIC
            ...
            # Time-based exit
                ...
                self.position.close()
                print(
                    f"🌙⏰ TIME EXIT at {current_close:.2f} | Held {self.max_hold_bars} bars ⏳"
                )
