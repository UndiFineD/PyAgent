# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\SuperSimple_BuyHoldCrusher.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class SuperSimpleCrusher(Strategy):
    ...
    def next(self):
        ...
        if (
            self.buys_executed < len(self.buy_times)
            and current_bar >= self.buy_times[self.buys_executed]
        ):
    ...
    def execute_scheduled_buy(self):
        ...
        if position_size > 0:
            ...
            print(
                f"🚀 SCHEDULED BUY #{self.buys_executed}: Size={position_size} @ {current_price:.2f}"
            )
...
bt = Backtest(data, SuperSimpleCrusher, cash=1000000, commission=0.002)
stats = bt.run()
