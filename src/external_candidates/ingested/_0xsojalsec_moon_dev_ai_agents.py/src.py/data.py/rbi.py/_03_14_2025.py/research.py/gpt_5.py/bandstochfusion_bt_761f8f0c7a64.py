# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\research\GPT-5\BandStochFusion_BT.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.data.close[-1] <= self.bb_lower[-1]:
            if self.stoch_k[-1] < 20 and (
                ...
                print(
                    f"🌙 Moon Dev Alert: LONG activated at {self.data.close[-1]:.2f} | Size: {position_size} units 🚀"
                )

        # Short Entry
        if self.data.close[-1] >= self.bb_upper[-1]:
            if self.stoch_k[-1] > 80 and (
                self.stoch_d[-2] < self.stoch_k[-2]
                and self.stoch_d[-1] > self.stoch_k[-1]
            ):
                position_size = int(round(1000000 / self.data.close[-1]))
                self.sell(size=position_size)
                print(
                    f"🌙 Moon Dev Alert: SHORT activated at {self.data.close[-1]:.2f} | Size: {position_size} units 🌒"
                )
        ...
        if self.position.is_long:
            if self.data.close[-1] >= self.bb_middle[-1] or self.stoch_k[-1] > 80:
                self.position.close()
                print(
                    f"✨ Moon Dev Exit: LONG closed at {self.data.close[-1]:.2f} | Profit: {self.position.pl:.2f} 🌟"
                )
        ...
        if self.position.is_short:
            if self.data.close[-1] <= self.bb_middle[-1] or self.stoch_k[-1] < 20:
                self.position.close()
                print(
                    f"✨ Moon Dev Exit: SHORT closed at {self.data.close[-1]:.2f} | Profit: {self.position.pl:.2f} 🌟"
                )
