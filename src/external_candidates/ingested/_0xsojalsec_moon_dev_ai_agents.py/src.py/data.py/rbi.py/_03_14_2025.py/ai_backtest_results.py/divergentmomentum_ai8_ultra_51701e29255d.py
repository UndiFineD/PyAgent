# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DivergentMomentum_AI8_Ultra.py
#!/usr/bin/env python3
...
class DivergentMomentumUltra(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            volume_confirmation = (
                current_volume > self.volume_sma[-1] * 1.2
            )  # 20% above average
            price_above_bb_middle = current_price > self.bb_middle[-1]

            # Breakout condition
            bb_squeeze = (self.bb_upper[-1] - self.bb_lower[-1]) / self.bb_middle[
                -1
            ] < 0.04
            recent_breakout = (
                current_price > self.bb_upper[-2] and current_price > self.data.High[-2]
            )
            ...
            if (
                ...
                and price_above_bb_middle
                and (bb_squeeze or recent_breakout)
            ):
                ...
                momentum_score = (
                    ...
                    + min(
                        (current_volume / self.volume_sma[-1] - 1), 1
                    )  # Volume momentum (0-1)
                    + min(
                        ((self.ema_fast[-1] / self.ema_slow[-1] - 1) * 100), 0.5
                    )  # EMA momentum
                ) / 3
                ...
                if position_size > 0:
                    ...
                    atr_value = (
                        self.atr[-1] if len(self.atr) > 0 else current_price * 0.03
                    )
                    ...
                    print(
                        f"🚀🌙 ULTRA MOMENTUM ENTRY! Size: {position_size} @ {current_price:.2f}"
                    )
                    print(
                        f"   📊 Momentum Score: {momentum_score:.2f}, Volume: {current_volume/self.volume_sma[-1]:.1f}x avg"
                    )
        ...
        # Dynamic exit conditions 🌊
            ...
            momentum_weakening = (
                self.rsi_fast[-1] > 75  # Overbought
                or self.macd[-1] < self.macd_signal[-1]  # MACD bearish
                or current_price < self.ema_fast[-1]
            )  # Below fast EMA
            ...
            if (
                (momentum_weakening and current_return > 0.02)
                or profit_target_hit
                or big_profit_hit
            ):
                self.position.close()
                print(
                    f"💰 ULTRA EXIT! Return: {current_return*100:.1f}% @ {current_price:.2f}"
                )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, DivergentMomentumUltra, cash=1_000_000, commission=0.002)
    stats = bt.run()
