# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MoonDev_AI8_CHAMPION.py
#!/usr/bin/env python3
...
class MoonDevChampion(Strategy):
    ...
    def next(self):
        ...
        if (
            not self.position and self.trade_count < 6
        ):  # Only 6 strategic entries maximum
            ...
            price_momentum = (
                current_price > self.data.Close[-10] * 1.01
            )  # 1% gain in 10 bars
        ...
        # CHAMPION HOLDING STRATEGY - Ride the ENTIRE trend 🌊
            ...
            if self.position:
                ...
                clear_bear = (
                    current_price < self.sma20[-1] * 0.90  # 10% below SMA20
                    and self.rsi[-1] < 30  # RSI oversold
                ...
                time_exit_losing = (
                    bars_held > 8000 and profit_pct < 0
                )  # Exit if losing after long time
                ...
                if mega_profit or clear_bear or time_exit_losing:
                    ...
                    exit_reason = (
                        "MEGA PROFIT"
                        if mega_profit
                        else ("CLEAR BEAR" if clear_bear else "TIME EXIT LOSING")
                    )
                    print(f"💰👑 CHAMPION EXIT! {exit_reason}")
                    print(
                        f"   🎯 EPIC P&L: {profit_pct*100:.1f}% @ {current_price:.2f}"
                    )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, MoonDevChampion, cash=1_000_000, commission=0.002)
    stats = bt.run()
    ...
    print(
        f"   🎯 Performance Ratio: {stats['Return [%]'] / stats['Buy & Hold Return [%]'] * 100:.1f}%"
    )
