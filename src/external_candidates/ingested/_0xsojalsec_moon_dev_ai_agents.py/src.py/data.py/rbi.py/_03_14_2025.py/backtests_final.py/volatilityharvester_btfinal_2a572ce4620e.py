# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\VolatilityHarvester_BTFinal.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if time_condition and vol_condition and liquidity_condition:
            if not self.position:
                ...
                if position_size > 0:
                    self.sell(
                        size=position_size, sl=self.stop_loss, tp=self.take_profit
                    )

                    print(
                        f"🌙 MOON DEV ENTRY 🌙 | SHORT {position_size} units | "
            ...
            if self.position:
                if (
                    ...
                    self.position.close()
                    print(
                        f"🌖 MOON DEV EXIT 🌖 | CLOSING POSITION | "
...
if __name__ == "__main__":
    ...
    stats = bt.run()
    print("🌌 COSMIC BACKTEST COMPLETE 🌌")
    print(stats)
