# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\BandReversalEdge_AI2.py
import numpy as np
import pandas as pd
...
class BandReversalEdge(Strategy):
    ...
    def next(self):
        if not self.position:
            ...
            bb_l20 = (
                self.bb_lower_20[-1] if not np.isnan(self.bb_lower_20[-1]) else price
            )
            bb_l50 = (
                self.bb_lower_50[-1] if not np.isnan(self.bb_lower_50[-1]) else price
            )
            atr_value = self.atr[-1] if not np.isnan(self.atr[-1]) else 100

            if (
                (price <= bb_l20 * 1.005)
                and (price <= bb_l50 * 1.005)
                and atr_value > 0
            ):
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev Entry Signal 🚀 | Price: {price:.2f} | Size: {position_size} | SL: {self.stop_loss_price:.2f} | Equity: {equity:.2f}"
                        )
        else:
            # Exit conditions
            ...
            bb_u20 = (
                self.bb_upper_20[-1]
                if not np.isnan(self.bb_upper_20[-1])
            ...
            if self.data.High[-1] >= bb_u20:
                self.position.close()
                print(
                    f"🎯 Moon Dev Profit Target Hit! | Close: {self.data.Close[-1]:.2f}"
                )
                return
            ...
            if self.data.Low[-1] <= self.stop_loss_price:
                self.position.close()
                print(
                    f"🛑 Moon Dev Stop Loss Triggered! | Close: {self.data.Close[-1]:.2f}"
                )
                return
            ...
            if current_bar - self.entry_bar >= self.exit_bars:
                self.position.close()
                print(
                    f"⌛ Moon Dev Time Exit | Bars Held: {current_bar - self.entry_bar} | Close: {self.data.Close[-1]:.2f}"
                )
