# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DivergenceVolatility_AI6.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class DivergenceVolatility(Strategy):
    ...
    def init(self):
        ...
        self.volume_sma = self.I(
            calc_sma, self.data.Volume, 15
        )  # Shorter period for responsiveness
        self.volume_sma_long = self.I(
            calc_sma, self.data.Volume, 30
        )  # Longer period for context
        ...
        self.atr = self.I(
            calc_atr, self.data.High, self.data.Low, self.data.Close, self.atr_period
        )
        ...
        def calc_rolling_min(data, period):
            return (
                pd.Series(data)
                .rolling(window=period, center=True)
                .min()
                .fillna(method="ffill")
                .values
            )
    ...
    def next(self):
        ...
        if (
            not np.isnan(current_swing_low)
            and abs(current_low - current_swing_low) < 0.01
        ):
            ...
            for i in range(len(self.swing_history) - 1):
                ...
                if current_index - prev_idx > self.min_separation:
                    ...
                    if current_low < prev_low and current_macd > prev_macd:
                        print(
                            f"🌙 BULLISH DIVERGENCE: Price {prev_low:.2f} -> {current_low:.2f} | MACD {prev_macd:.4f} -> {current_macd:.4f}"
                        )
                        ...
                        if (
                            volume_conditions
                            and volatility_conditions
                            and momentum_conditions
                        ):
                            print(
                                f"✅ ALL CONDITIONS MET: Volume={volume_conditions} | Vol={volatility_conditions} | Mom={momentum_conditions}"
                            )
    ...
    def check_volume_conditions(self):
        ...
        long_avg = (
            self.volume_sma_long[-1]
            if len(self.volume_sma_long) > 0 and not np.isnan(self.volume_sma_long[-1])
            else short_avg
        )
        ...
        volume_trend = (
            len(self.volume_sma) > 2 and self.volume_sma[-1] > self.volume_sma[-3]
        )
    ...
    def check_volatility_conditions(self):
        ...
        bb_width_avg = np.nanmean(
            [self.bb_upper[i] - self.bb_lower[i] for i in range(-10, 0) if i < 0]
        )
    ...
    def check_momentum_conditions(self):
        ...
        price_trend = (
            len(self.data.Close) > 3 and self.data.Close[-1] > self.data.Close[-3]
        )
    ...
    def execute_long_entry(self):
        ...
        if position_size > 0:
            ...
            print(
                f"   ATR: {current_atr:.1f} (Avg: {atr_avg:.1f}) | Multiplier: {multiplier:.1f}"
            )
    ...
    def check_exit_conditions(self):
        ...
        # MACD reversal exit
        if (
            len(self.macd_hist) > 3
            and self.macd_hist[-1] < self.macd_hist[-2] < self.macd_hist[-3]
            and self.entry_price
            and current_price > self.entry_price
        ):  # Only if profitable
            self.position.close()
        ...
        if (
            self.atr_at_entry
            and len(self.atr) > 5
            and self.atr[-1] < self.atr_at_entry * 0.6
            and current_price > self.entry_price
        ):  # Only if profitable
            self.position.close()
...
bt = Backtest(data, DivergenceVolatility, cash=1000000, commission=0.002)
stats = bt.run()
...
print(
    f"   💰 Average Trade: {(stats['Return [%]'] / max(stats['# Trades'], 1)):.2f}% per trade"
)
