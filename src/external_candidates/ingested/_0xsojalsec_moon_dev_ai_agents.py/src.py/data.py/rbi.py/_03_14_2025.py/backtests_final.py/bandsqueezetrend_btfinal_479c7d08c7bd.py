# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\BandSqueezeTrend_BTFinal.py
import pandas as pd
import talib
...
class BandSqueezeTrend(Strategy):
    ...
    def init(self):
        ...
        self.upper_band, self.middle_band, _ = [
    ...
    def next(self):
        ...
        if not self.position:
            # Entry conditions
            if current_close > self.upper_band[-1] and current_close < self.atr_ma[-1]:
                ...
                # Ensure whole number position sizing
                position_size = max(1, int(round(risk_amount / risk_per_share)))
