# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MomentumBreakout_AI7.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class MomentumBreakout(Strategy):
    ...
    def init(self):
        ...
        self.ma_fast_line = self.I(
            _sma_fast, self.data.Close, name=f"SMA{self.ma_fast}"
        )
        self.ma_slow_line = self.I(
            _sma_slow, self.data.Close, name=f"SMA{self.ma_slow}"
        )
        ...
        self.volume_ma_line = self.I(
            _volume_sma, self.data.Volume, name=f"Volume_SMA{self.volume_ma}"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                (bullish_ma_cross or (uptrend and price > self.ma_fast_line[-1]))
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀 LONG: {entry_price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | Size: {position_size}"
                        )
            ...
            # 4. High volume confirmation
                ...
                (bearish_ma_cross or (downtrend and price < self.ma_fast_line[-1]))
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🐻 SHORT: {entry_price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | Size: {position_size}"
                        )
