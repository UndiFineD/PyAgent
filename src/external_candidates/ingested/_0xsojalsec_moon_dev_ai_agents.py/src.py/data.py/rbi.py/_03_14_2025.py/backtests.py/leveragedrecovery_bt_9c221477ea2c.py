# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests\LeveragedRecovery_BT.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position and len(self.data) == 1:
            ...
            if size > 0:
                ...
                print(
                    f"🚀🌕 MOON DEV INITIAL ENTRY: Bought {size:,} units at ${self.entry_price:.2f}!"
                )
        ...
        if self.position.is_long:
            if current_rsi < 30:
                ...
                if sell_size > 0:
                    ...
                    print(
                        f"🌙✨ SELL SIGNAL: RSI {current_rsi:.2f} <30! Sold {sell_size:,} units (70%). Remaining: {remaining:,}"
                    )
        ...
        if self.pending_reentry and not self.position.is_short:
            if current_rsi > 60:
                ...
                if buy_size > 0:
                    ...
                    print(
                        f"🚀🌓 RE-ENTRY: Added {buy_size:,} units (120% leverage) at ${self.data.Close[-1]:.2f}. New position: {new_position:,}"
                    )
                    ...
                    if take_size > 0:
                        ...
                        print(
                            f"🌙💸 PROFIT TAKING: Sold {take_size:,} units (50% of position)"
                        )
