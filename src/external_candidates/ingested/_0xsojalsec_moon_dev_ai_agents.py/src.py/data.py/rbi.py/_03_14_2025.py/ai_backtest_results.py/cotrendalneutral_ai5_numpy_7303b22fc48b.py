# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\CoTrendalNeutral_AI5_numpy.py
...
def atr_func(high, low, close, period=14):
    ...
    return true_range.rolling(period).mean().values
...
class CoTrendalNeutral(Strategy):
    ...
    def init(self):
        ...
        print(
            "✨ Moon Dev CoTrendalNeutral Indicators Initialized! Ready for Launch 🚀"
        )
    ...
    def next(self):
        ...
        if (
            np.isnan(self.sma50[-1])
            or np.isnan(self.sma200[-1])
            or np.isnan(self.atr[-1])
            or np.isnan(self.atr_ma[-1])
            or np.isnan(self.swing_high[-1])
            or np.isnan(self.swing_low[-1])
        ):
            return
            ...
            if uptrend and vol_decreasing:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙🚀 MOON SHOT LONG! Entry: {price:.2f}, Size: {position_size}, SL: {sl:.2f}, TP: {tp:.2f}"
                        )
            ...
            # Short Entry Logic - Downtrend with increasing volatility
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙📉 BLACK HOLE SHORT! Entry: {price:.2f}, Size: {position_size}, SL: {sl:.2f}, TP: {tp:.2f}"
                        )
