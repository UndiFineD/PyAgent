# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DivergentVolume_AI8_Ultra.py
#!/usr/bin/env python3
    ...
    def next(self):
        ...
        if not self.position and self.trade_count < 10:
            ...
            bull_market = (
                ...
                and current_price > self.ema200[-1]
            )

            # Momentum confirmation
            macd_momentum = (
                self.macd[-1] > self.macd_signal[-1] and self.macd[-1] > self.macd[-5]
            )  # Accelerating

            # Volume surge indicating institutional interest
            volume_surge = (
                self.data.Volume[-1] > self.volume_sma20[-1] * 1.5
                and self.volume_sma20[-1] > self.volume_sma50[-1] * 1.2
            )
            ...
            breakout = (
                current_price > recent_high * 1.002
            )  # Breaking above 20-period high

            pullback_buy = (
                current_price > self.ema20[-1]
                and self.data.Low[-1] <= self.ema20[-1] * 1.01
            )  # Touch EMA20
            ...
            if (
                ...
                if position_size > 0:
                    ...
                    print(
                        f"   📊 Volume: {self.data.Volume[-1]/self.volume_sma20[-1]:.1f}x avg"
                    )
                    print(
                        f"   📈 EMA Stack: {self.ema20[-1]:.0f} > {self.ema50[-1]:.0f} > {self.ema200[-1]:.0f}"
                    )
        ...
        # Trend-following exits with profit maximization 🌊
            ...
            if self.position:
                ...
                bear_market = (
                    self.ema20[-1] < self.ema50[-1] or current_price < self.ema50[-1]
                )

                macd_bearish = (
                    self.macd[-1] < self.macd_signal[-1]
                    and self.macd[-1] < self.macd[-5]
                )  # Decelerating
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, DivergentVolumeUltra, cash=1_000_000, commission=0.002)
    stats = bt.run()
