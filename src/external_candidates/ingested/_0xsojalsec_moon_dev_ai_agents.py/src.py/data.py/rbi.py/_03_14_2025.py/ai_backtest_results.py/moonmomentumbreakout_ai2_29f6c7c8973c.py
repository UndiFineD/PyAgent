...
class MoonMomentumBreakout(Strategy):
    ...
    def next(self):
        ...
        high_break = (
            self.high_breakout[-1] if not np.isnan(self.high_breakout[-1]) else price
        )
        ...
        volume_sma = (
            self.volume_sma[-1] if not np.isnan(self.volume_sma[-1]) else volume
        )
        ...
        # 📈 Exit conditions - trend reversal
            ...
            if momentum_val < -0.02 or sma_fast < sma_slow * 0.995:
                self.position.close()
                print(
                    f"🌑 MOON RETREAT! Closing long @ {price:.2f} | Momentum: {momentum_val:.3f}"
                )
