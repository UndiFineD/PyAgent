# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\LeveragedBuyHold_AI5.py
...
class LeveragedBuyHold(Strategy):
    ...
    def next(self):
        ...
        if not self.position and self.trades_made < 10:  # Limit trades
            ...
            if (
                current_close < sma20_val * 0.90  # 10% below SMA20
                and rsi_val < 30  # Oversold
                and self.trades_made < 3
            ):  # First 3 major opportunities
                ...
                if position_size > 0:
                    ...
                    self.trades_made += 1
                    print(
                        f"🚀🌙 LEVERAGED DIP BUY #{self.trades_made}! Entry: {current_close:.2f}, Size: {position_size}"
                    )

            # Regular accumulation during uptrends
            elif (
                current_close > sma20_val
                and rsi_val < 70
                and self.trades_made >= 3
                and self.trades_made < 8
            ):
                ...
                if position_size > 0:
                    ...
                    self.trades_made += 1
                    print(
                        f"🚀🌙 ACCUMULATION BUY #{self.trades_made}! Entry: {current_close:.2f}, Size: {position_size}"
                    )

        else:
            # Only sell near the end for final profit taking
            if (
                current_close > self.data.Close[0] * 2.0  # Doubled from start
                and rsi_val > 85  # Extremely overbought
                and self.trades_made >= 8
            ):
                ...
                if self.position.size > 0:
                    ...
                    if sell_size > 0:
                        ...
                        self.trades_made += 1
                        print(
                            f"🌕 PARTIAL PROFIT TAKE #{self.trades_made} at {current_close:.2f}"
                        )
