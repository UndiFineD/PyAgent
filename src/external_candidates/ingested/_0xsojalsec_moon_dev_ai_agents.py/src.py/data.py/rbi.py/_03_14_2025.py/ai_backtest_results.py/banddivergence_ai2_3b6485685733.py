...
class BandDivergence(Strategy):
    ...
    def next(self):
        ...
        bb_mid_prev = (
            self.bb_mid[-2]
            if len(self.data) > 1 and not np.isnan(self.bb_mid[-2])
            else bb_mid_current
        )
        ...
        if not self.position and atr_val > 0:
            ...
            if (
                len(self.data) >= 22
            ):  # Ensure we have enough data for swing calculations
        ...
        if not self.position and atr_val > 0:
            ...
            bb_width_prev = (
                ...
                if len(self.data) > 1
                and not np.isnan(self.bb_upper[-2])
                and not np.isnan(self.bb_lower[-2])
                else bb_width
            )
        ...
        if self.position:
            ...
            if (
                ...
                and not np.isnan(self.bb_upper[-1])
                ...
                and not np.isnan(self.bb_lower[-1])
