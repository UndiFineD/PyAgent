...
class TrendSurfer(Strategy):
    ...
    def init(self):
        ...
        self.ema_fast_line = self.I(
            _ema_fast, self.data.Close, name=f"EMA{self.ema_fast}"
        )
        self.ema_slow_line = self.I(
            _ema_slow, self.data.Close, name=f"EMA{self.ema_slow}"
        )
        ...
        self.highest = self.I(
            _rolling_high, self.data.High, self.lookback, name=f"High{self.lookback}"
        )
        self.lowest = self.I(
            _rolling_low, self.data.Low, self.lookback, name=f"Low{self.lookback}"
        )
    ...
    def next(self):
        ...
        strong_uptrend = (
            price > self.ema_fast_line[-1] > self.ema_slow_line[-1]
            and self.ema_fast_line[-1] > self.ema_fast_line[-2]
        )
        strong_downtrend = (
            price < self.ema_fast_line[-1] < self.ema_slow_line[-1]
            and self.ema_fast_line[-1] < self.ema_fast_line[-2]
        ...
        if not self.position:
            ...
            if (strong_uptrend and breakout_high) or (ema_bullish and breakout_high):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀🌊 LONG TREND: {price:.2f} | SL: {sl_price:.2f} | Size: {position_size}"
                    )
            ...
            # SHORT Entry: Strong downtrend + breakdown
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🐻🌊 SHORT TREND: {price:.2f} | SL: {sl_price:.2f} | Size: {position_size}"
                    )
            ...
            if self.position.is_long:
                ...
                if (
                    ...
                    or price < self.ema_slow_line[-1] * 0.95
                ):
                    self.position.close()
                ...
                if (
                    self.ema_fast_line[-1] > self.ema_slow_line[-1]
                    or price > self.ema_slow_line[-1] * 1.05
                ):
                    self.position.close()
