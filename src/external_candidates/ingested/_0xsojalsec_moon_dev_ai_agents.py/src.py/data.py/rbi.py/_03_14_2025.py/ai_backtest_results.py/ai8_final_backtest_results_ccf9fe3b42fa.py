# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\AI8_Final_Backtest_Results.py
#!/usr/bin/env python3
...
class DivergentMomentumEnhanced(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                self.rsi[-1] < 40  # Less strict than 30
                and self.data.Close[-1] > self.ma[-1]
                and self.data.Close[-1] > self.data.Close[-2]
            ):  # Price momentum
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        print(
                            f"🌙✨ DivergentMomentum Entry! Long {position_size} @ {entry_price:.2f}, SL: {stop_loss:.2f}"
                        )
        ...
        # Exit conditions
            ...
            if len(self.cci) > 0 and self.cci[-1] > 150:  # CCI overbought
                print(
                    f"🚀🌙 DivergentMomentum Exit! Closing at {self.data.Close[-1]:.2f}"
                )
                self.position.close()
...
class DivergentPulseEnhanced(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                len(self.macd) > 1
                and len(self.macd_signal) > 1
                and self.macd[-1] > self.macd_signal[-1]
                and self.macd[-2] <= self.macd_signal[-2]  # Crossover
                and self.rsi[-1] < 60  # Not overbought
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        print(
                            f"🚀🌙 DivergentPulse Entry! Long {position_size} @ {entry_price:.2f}, SL: {stop_loss:.2f}"
                        )
                        self.buy(size=position_size, sl=stop_loss)
...
class DivergentVolumeEnhanced(Strategy):
    def init(self):
        ...
        self.price_highs = self.I(
            talib.MAX, self.data.High, timeperiod=10
        )  # Shorter window
    ...
    def next(self):
        ...
        prev_price_high = (
            self.price_highs[-2] if len(self.price_highs) > 1 else current_high
        )
        ...
        price_higher_high = (
            current_high >= prev_price_high * 0.999
        ...
        if bearish_divergence and volume_declining and not self.position:
            ...
            max_size = (
                int((self.equity * 0.05) // current_close) if current_close > 0 else 0
            )
            ...
            if position_size > 0:
                ...
                self.trade_count += 1
                print(
                    f"🚀 DivergentVolume Entry! Long {position_size} @ {current_close:.2f}"
                )
        ...
        if self.position:
            ...
            if (
                current_close > entry_price * 1.02  # 2% profit
                or current_close < entry_price * 0.98  # 2% loss
                or len(self.data) % 20 == 0
            ):  # Time-based exit
                self.position.close()
...
if __name__ == "__main__":
    ...
    results["DivergentMomentum"] = run_strategy_backtest(
        DivergentMomentumEnhanced, "DivergentMomentum"
    )

    # Strategy 2: DivergentPulse
    results["DivergentPulse"] = run_strategy_backtest(
        DivergentPulseEnhanced, "DivergentPulse"
    )

    # Strategy 3: DivergentVolume
    results["DivergentVolume"] = run_strategy_backtest(
        DivergentVolumeEnhanced, "DivergentVolume"
    )
