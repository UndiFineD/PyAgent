# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MoonDev_AI8_WINNER.py
#!/usr/bin/env python3
...
class MoonDevWinner(Strategy):
    ...
    def next(self):
        ...
        if not self.position and self.trade_count < 6:  # Max 6 strategic trades
            ...
            pullback_buy = (
                current_price > self.ema50[-1] * 0.98  # Within 2% of EMA50
                and current_price < self.ema50[-1] * 1.02  # Not too far above
                and self.data.Close[-5] > self.ema50[-5]
            )  # Was above EMA50 recently

            if (
                above_long_term
                and medium_term_bull
                and macd_bullish
                and rsi_healthy
                and pullback_buy
            ):
                ...
                if position_size > 0:
                    ...
                    print(
                        f"   📊 EMA50: {self.ema50[-1]:.0f}, EMA200: {self.ema200[-1]:.0f}"
                    )
        ...
        # Let winners run - exit logic 🌊
            ...
            if self.position:
                ...
                trend_broken = (
                    self.ema50[-1] < self.ema200[-1] and current_price < self.ema50[-1]
                )

                # 2. MACD turns bearish
                macd_bearish = (
                    self.macd[-1] < self.macd_signal[-1] and self.macd[-1] < 0
                )
                ...
                if trend_broken or macd_bearish or huge_profit or big_loss:
                    ...
                    exit_reason = (
                        ...
                        if trend_broken
                            ...
                            "MACD BEARISH"
                            if macd_bearish
                            else ("HUGE PROFIT" if huge_profit else "STOP LOSS")
                        )
                    )
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, MoonDevWinner, cash=1_000_000, commission=0.002)
    stats = bt.run()
