# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\OptimalTrend_AI7.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class OptimalTrend(Strategy):
    ...
    def init(self):
        ...
        self.ema_fast = self.I(
            _ema, self.data.Close, self.ma_fast, name=f"EMA{self.ma_fast}"
        )
        self.ema_slow = self.I(
            _ema, self.data.Close, self.ma_slow, name=f"EMA{self.ma_slow}"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (golden_cross and momentum_bullish) or (
                bullish_trend and strong_bullish
            ):
                ...
                if position_size > 0:
                    ...
                    if golden_cross:
                        print(
                            f"🚀⚡ GOLDEN CROSS: {price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
                        )
                    else:
                        print(
                            f"🚀💪 STRONG BULL: {price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
                        )

            # SHORT Entry: Death cross OR strong bearish momentum
            elif (death_cross and momentum_bearish) or (
                bearish_trend and strong_bearish
            ):
                position_size = int((self.equity * self.position_size_pct) / price)
                ...
                if position_size > 0:
                    ...
                    if death_cross:
                        print(
                            f"🐻⚡ DEATH CROSS: {price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
                        )
                    else:
                        print(
                            f"🐻💪 STRONG BEAR: {price:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f}"
                        )
...
if stats["Return [%]"] > stats["Buy & Hold Return [%]"] and stats["# Trades"] > 5:
    ...
    print(
        f"🌙🔧 Beat Buy&Hold: {'✅' if stats['Return [%]'] > stats['Buy & Hold Return [%]'] else '❌'}"
    )
