# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\EarlyBird_BuyHoldDestroyer.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class EarlyBirdDestroyer(Strategy):
    ...
    def next(self):
        ...
        # Make early entries
        if (
            self.entries_made < len(self.entry_points)
            and current_bar >= self.entry_points[self.entries_made]
        ):
    ...
    def make_early_entry(self):
        ...
        if position_size > 0:
            ...
            print(
                f"🐦 EARLY BIRD ENTRY #{self.entries_made}: Size={position_size} @ {current_price:.2f}"
            )
            print(
                f"   Cash Fraction: {cash_fraction:.0%} | Cash: ${available_cash:.0f}"
            )
...
bt = Backtest(data, EarlyBirdDestroyer, cash=1000000, commission=0.002)
stats = bt.run()
