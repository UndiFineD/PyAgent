...
class ClusterBreakout(Strategy):
    ...
    def next(self):
        ...
        breakout_long = (
            self.data.Close[-1] > cluster_high[-1]
            and self.data.Volume[-1] > volume_clusters[-1]
        )
        breakout_short = (
            self.data.Close[-1] < cluster_low[-1]
            and self.data.Volume[-1] > volume_clusters[-1]
        )
        ...
        if (
            self.sma50[-2] > self.data.Close[-2]
            and self.sma50[-1] < self.data.Close[-1]
        ):
