# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\EuroSpreadScalper_BTFinal.py
...
class EuroSpreadScalper(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            sl_price = price * (
                1 - self.sl_factor
            )  # Fixed: Corrected SL calculation for short position
            ...
            if position_size > 0:
                ...
                self.sell(
                    size=position_size, sl=sl_price, tp=price * (1 - self.tp_factor)
                )
        ...
        if self.position and (len(self.data) - self.entry_bar) >= 2:
            self.position.close()
            print(
                f"⏰🌙 MOON DEV TIME EXIT! Bars held: {len(self.data)-self.entry_bar}"
            )
