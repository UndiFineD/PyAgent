# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\BandedMACD_BTFinal.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
...
class BandedMACD(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                macd > signal
                and self.data.High[-1] >= upper_band
                and len(self.data.Close) > 26
            ):
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🌙 Moon Dev Entry: Buying {position_size} units at {entry_price:.2f} 🚀"
                    )
        ...
        # 💎 Exit Logic
            ...
            if close_below_swing_high or current_low < trailing_stop:
                print(
                    f"🌙 Moon Dev Exit: Closing {self.position.size} units at {price:.2f} ✨"
                )
                self.position.close()
