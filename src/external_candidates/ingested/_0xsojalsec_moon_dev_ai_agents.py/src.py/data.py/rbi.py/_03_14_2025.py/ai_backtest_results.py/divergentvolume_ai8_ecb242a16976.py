...
class DivergentVolume(Strategy):
    ...
    def next(self):
        ...
        prev_volume_peak = (
            self.volume_peaks[-2] if len(self.volume_peaks) > 1 else current_volume
        )
        ...
        if (
            ...
            position_size = (
                int(round(risk_amount / stop_loss_distance))
            ...
            if position_size > 0:
                ...
                print(
                    f"   🔐 Stop Loss: {stop_price:.2f} | 🎯 Take Profit: {profit_price:.2f}"
                )
