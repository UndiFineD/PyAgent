# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_package\DivergentMomentum_PKG.py
import numpy as np
import pandas as pd
import talib
...
class DivergentMomentum(Strategy):
    ...
    def init(self):
        ...
        self.cci = self.I(
            talib.CCI, self.data.High, self.data.Low, self.data.Close, self.cci_period
        )
    ...
    def next(self):
        ...
        if not self.position:
            if self.rsi[-1] < 30 and divergence and self.data.Close[-1] > self.ma50[-1]:
                ...
                if risk_per_share > 0:
                    position_size = (
                        self.broker.equity * self.risk_percent
                    ) / risk_per_share
                    position_size = int(round(position_size))

                    # Moon-themed debug print
                    print(
                        f"🌙✨ Lunar Entry! Price: {entry_price:.2f}, Size: {position_size} contracts, SL: {stop_loss:.2f}"
                    )
        ...
        # Exit conditions
            ...
            if len(self.cci) > 2:
                ...
                if current_cci > 100 and current_cci < prev_cci:
                    print(
                        f"🚀🌙 CCI Reversal! Closing position at {self.data.Close[-1]:.2f}"
                    )
                    self.position.close()
