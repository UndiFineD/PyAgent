# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\DCAplus_BuyHoldSlayer.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class DCAPlusSlayer(Strategy):
    ...
    def next(self):
        ...
        if (
            self.dca_made < len(self.dca_intervals)
            and current_bar >= self.dca_intervals[self.dca_made]
        ):
        ...
        # Also buy dips (price > 5% below MA50)
            ...
            len(self.ma50) > 0
            and not np.isnan(self.ma50[-1])
            and self.data.Close[-1] < self.ma50[-1] * 0.95
            and current_bar % 500 == 0
        ):  # Every 500 bars if dip
    ...
    def execute_dca_buy(self):
        ...
        if position_size > 0:
            ...
            print(
                f"💰 DCA BUY #{self.dca_made}: Size={position_size} @ {current_price:.2f}"
            )
...
bt = Backtest(data, DCAPlusSlayer, cash=1000000, commission=0.002)
stats = bt.run()
