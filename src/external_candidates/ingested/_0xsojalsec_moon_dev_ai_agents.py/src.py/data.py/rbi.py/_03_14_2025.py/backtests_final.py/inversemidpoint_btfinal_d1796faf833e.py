# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\backtests_final\InverseMidpoint_BTFinal.py
# -*- coding: utf-8 -*-
import numpy as np
import pandas as pd
import talib as ta
...
print("🌕 Loading data from lunar database...")
data = pd.read_csv(data_path, parse_dates=["datetime"], index_col="datetime")

# Clean and process data ✨
print("🌖 Cleaning cosmic data streams...")
data.columns = data.columns.str.strip().str.lower()
data = data.drop(columns=[col for col in data.columns if "unnamed" in col.lower()])

# Resample to daily timeframe 🌙
print("🌗 Resampling to daily lunar cycles...")
...
class InverseMidpoint(Strategy):
    ...
    def init(self):
        ...
        self.prev_open = self.I(ta.SMA, self.data.Open, timeperiod=1, name="PrevOpen")
        self.prev_high = self.I(ta.SMA, self.data.High, timeperiod=1, name="PrevHigh")

        # Midpoint calculation (previous day's (Open+High)/2)
        self.midpoint = self.I(
            lambda: (self.prev_open + self.prev_high) / 2, name="Midpoint"
        )

        # Volume indicators 📊
        self.avg_volume = self.I(
            ta.SMA, self.data.Volume, timeperiod=20, name="AvgVolume"
        )
    ...
    def next(self):
        ...
        if not self.position:
            if current_close < midpoint and current_volume > 0.1 * avg_vol:
                ...
                risk_per_share = abs(stop_loss - entry_price)  # Ensure positive value
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 Moon Dev SHORT Signal! Entry: {entry_price:.2f}, Size: {position_size} ✨"
                        )
        ...
        # Exit logic: Price above midpoint or volume drops 📈
            ...
            if current_close > midpoint or current_volume < 0.1 * avg_vol:
                self.position.close()
                print(
                    f"🌙💫 Closing SHORT at {current_close:.2f} - Moon Profit Activated! 🎯"
                )
