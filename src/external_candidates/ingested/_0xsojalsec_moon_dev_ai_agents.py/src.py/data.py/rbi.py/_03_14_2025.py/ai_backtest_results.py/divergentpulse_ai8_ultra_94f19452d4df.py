...
class DivergentPulseUltra(Strategy):
    ...
    def next(self):
        ...
        if not self.position and self.trade_count < 20:  # Allow more trades
            ...
            ema_stack = (
                self.ema8[-1] > self.ema21[-1] > self.ema55[-1]
            )  # Bullish EMA stack
            ...
            price_distance = (
                abs(current_price - self.last_entry_price) / current_price > 0.05
            )
            ...
            if (
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀💥 PULSE ULTRA ENTRY #{self.trade_count}! Size: {position_size} @ {current_price:.2f}"
                    )
                    print(
                        f"   📈 EMA Stack: {self.ema8[-1]:.0f} > {self.ema21[-1]:.0f} > {self.ema55[-1]:.0f}"
                    )
        ...
        # Trend-following exits 🌊
            ...
            if self.position:
                ...
                if (
                    ...
                    self.position.close()
                    print(
                        f"💰 PULSE EXIT! P&L: {profit_pct*100:.1f}% @ {current_price:.2f}"
                    )
