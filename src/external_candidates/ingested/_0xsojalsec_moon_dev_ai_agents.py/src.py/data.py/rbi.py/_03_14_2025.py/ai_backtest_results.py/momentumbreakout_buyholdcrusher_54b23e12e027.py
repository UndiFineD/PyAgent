# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\MomentumBreakout_BuyHoldCrusher.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class MomentumBreakoutCrusher(Strategy):
    ...
    def init(self):
        ...
        def calc_roc(close, period=10):
            ...
            return (
                (close_series - close_series.shift(period))
                / close_series.shift(period)
                * 100
            ).values
    ...
    def check_momentum_breakout(self):
        ...
        trend_alignment = (
            current_price > self.sma_short[-1] > self.sma_medium[-1] > self.sma_long[-1]
        )

        # Momentum surge
        momentum_surge = (
            self.roc_short[-1] > 2 and self.roc_medium[-1] > 1
        )  # Strong momentum
        ...
        if position_size > 0:
            ...
            if self.sma_medium[-1] > 0:
                self.trailing_stop = max(
                    self.sma_medium[-1], current_price * 0.92
                )  # 8% or medium MA
            else:
            ...
            self.pyramid_levels.append(
                {"price": current_price, "size": position_size, "time": len(self.data)}
            )

            self.buy(size=position_size)

            print(
                f"🚀 MOMENTUM ENTRY #{self.positions_taken}: Size={position_size} @ {current_price:.2f}"
            )
            print(
                f"   ROC Short: {self.roc_short[-1]:.1f}% | ROC Medium: {self.roc_medium[-1]:.1f}%"
            )
            print(
                f"   Position Fraction: {position_fraction:.0%} | Trailing Stop: {self.trailing_stop:.2f}"
            )
    ...
    def manage_exits(self):
        ...
        if current_price <= self.trailing_stop:
            profit_pct = (
                (current_price - self.last_entry_price) / self.last_entry_price
            ) * 100
            self.position.close()
            print(
                f"🛑 TRAILING STOP EXIT @ {current_price:.2f} | Profit: {profit_pct:.1f}%"
            )
            self.reset_position()
            return
        ...
        if (
            len(self.roc_short) > 5
            and self.roc_short[-1] < -2  # Negative momentum
            and current_price < self.sma_short[-1]  # Below short MA
            and current_price > self.last_entry_price
        ):  # Only if profitable
            ...
            self.position.close()
            print(
                f"📉 MOMENTUM FAILURE EXIT @ {current_price:.2f} | Profit: {profit_pct:.1f}%"
            )
            self.reset_position()
            return
...
bt = Backtest(data, MomentumBreakoutCrusher, cash=1000000, commission=0.002)
stats = bt.run()
