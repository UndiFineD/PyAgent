# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\BandMACDDivergence_AI2.py
import numpy as np
import pandas as pd
...
class BandMACDDivergence(Strategy):
    ...
    def next(self):
        ...
        macd_signal_val = (
            self.macd_signal[-1] if not np.isnan(self.macd_signal[-1]) else 0
        )
        ...
        if not self.position and atr_value > 10:
            if ub_slope > self.slope_threshold and macd_diff > 0.5 * atr_value:
                ...
                if risk_per_share > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 LIFT OFF! Long {position_size} @ {price:.2f} | "
