# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\03_14_2025\AI_BACKTEST_RESULTS\SmartBuyHold_AI7.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
class SmartBuyHold(Strategy):
    ...
    def init(self):
        ...
        self.ma_long_line = self.I(
            _sma, self.data.Close, self.ma_long, name=f"SMA{self.ma_long}"
        )
        ...
        self.highest_recent = self.I(
            _rolling_max, self.data.Close, 20, name="HighestRecent"
        )
    ...
    def next(self):
        ...
        dip_detected = (
            price < self.highest_recent[-1] * (1 - self.dip_threshold)
            and self.rsi[-1] < 40
        )  # Oversold condition
        ...
        if not self.position:
            ...
            if in_uptrend and (dip_detected or strong_momentum):
                ...
                if position_size > 0:
                    ...
                    if dip_detected:
                        print(
                            f"🚀📉 BUY DIP: {price:.2f} | MA200: {self.ma_long_line[-1]:.2f} | RSI: {self.rsi[-1]:.1f}"
                        )
                    else:
                        print(
                            f"🚀📈 BUY MOMENTUM: {price:.2f} | MA200: {self.ma_long_line[-1]:.2f} | RSI: {self.rsi[-1]:.1f}"
                        )
            ...
            major_downtrend = (
                price < self.ma_long_line[-1] * 0.90  # 10% below MA200
                and self.rsi[-1] < 30
            )  # Very oversold
            ...
            if major_downtrend:
                self.position.close()
                print(
                    f"💰🔻 EXIT: {price:.2f} | MA200: {self.ma_long_line[-1]:.2f} | Major downtrend detected"
                )
