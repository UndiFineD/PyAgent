...
class EarlyBuyLateHold(Strategy):
    ...
    def next(self):
        current_bar = len(self.data.Close) - len(
            self.data.Close[self.data.Close.index <= self.data.index[-1]]
        )
        ...
        if not self.initial_buy_done and current_bar < self.total_bars * 0.05:
            ...
            if position_size > 0:
                ...
                print(
                    f"🚀🌙 EARLY BUY! Entry: {current_close:.2f}, Size: {position_size}"
                )
        ...
        # Add more positions during specific dips
            ...
            and current_close < self.data.Close[0] * 1.5
            ...
            if self.equity > current_close * 10:  # If we have cash left
                ...
                if add_size > 0:
                    ...
                    self.trade_count += 1
                    print(
                        f"🚀🌙 ADD POSITION #{self.trade_count}! Entry: {current_close:.2f}, Size: {add_size}"
                    )
