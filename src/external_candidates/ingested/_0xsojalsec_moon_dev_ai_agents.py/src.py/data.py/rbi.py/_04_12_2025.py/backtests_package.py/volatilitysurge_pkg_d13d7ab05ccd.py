# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_12_2025\backtests_package\VolatilitySurge_PKG.py
...
class VolatilitySurge(Strategy):
    ...
    def init(self):
        ...
        self.bb_lower = self.I(
            lambda x: talib.BBANDS(x, self.bb_period, 2, 2)[2], self.data.Close
        )
    ...
    def next(self):
        ...
        atr_cross = (self.atr[-2] < self.atr_sma[-2]) and (
        ...
        if not self.position and atr_cross and close_above_bb:
            ...
            if position_size > 0:
                ...
                print(
                    f"🌙🚀 MOON DEV LONG ENTRY! 🌙 | Price: {entry_price:.2f} | Size: {position_size}"
                )
                print(
                    f"   🛡️ SL: {stop_loss:.2f} | 🎯 TP: {tp_price:.2f} | Risk/Reward: 1:2"
                )
...
bt = Backtest(
    data, VolatilitySurge, cash=1_000_000, commission=0.002, exclusive_orders=True
)
