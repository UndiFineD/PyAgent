# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_12_2025\backtests\DynamicCrossfire_BT.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
...
class DynamicCrossfire(Strategy):
    ...
    def init(self):
        ...
        self.ema_fast = self.I(
            talib.EMA, self.data.Close, timeperiod=self.ema_period_fast, name="EMA50"
        )
        self.ema_slow = self.I(
            talib.EMA, self.data.Close, timeperiod=self.ema_period_slow, name="EMA200"
        )
        self.adx = self.I(
            talib.ADX,
            self.data.High,
        ...
        self.swing_low = self.I(
            talib.MIN, self.data.Low, timeperiod=self.swing_period, name="SWING_LOW"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if crossover(70, self.rsi):
                self.position.close()
                print(
                    f"🌙✨ RSI EXIT | Price: {self.data.Close[-1]} | RSI: {self.rsi[-1]:.1f}"
                )
        ...
        # Entry Conditions 🚀
            ...
            if ema_cross and adx_rising:
                ...
                print(
                    f"🌙🔥 SIGNAL CONFIRMED | EMA Cross: ✅ | ADX Strength: {self.adx[-1]:.1f} ✅"
                )
            ...
            if self.entry_signal:
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🚀🌙 ENTRY | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_price:.2f}"
                        )
