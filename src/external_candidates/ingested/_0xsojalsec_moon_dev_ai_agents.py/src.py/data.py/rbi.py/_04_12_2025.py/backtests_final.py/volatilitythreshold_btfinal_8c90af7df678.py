# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_12_2025\backtests_final\VolatilityThreshold_BTFinal.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityThreshold(Strategy):
    ...
    def init(self):
        ...
        self.atr_percentile = self.I(lambda s: s.expanding().quantile(0.10), self.atr)
        print("🌙 Indicators initialized! Ready for lunar trading cycles...")

    def next(self):
        # Wait for indicators to warm up 🕒
        if (
            len(self.data) < self.sma_window
            or pd.isna(self.atr[-1])
            or pd.isna(self.atr_percentile[-1])
        ):
            return
        ...
        if not self.position:
            if current_close > sma_value and current_atr < atr_threshold:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"   ATR({self.atr_window}): {current_atr:.2f} < Percentile: {atr_threshold:.2f}"
                    )
                    print(f"   Position size: {position_size} units")
                    self.buy(
                        size=position_size, sl=current_close - stop_distance
...
bt = Backtest(
    data, VolatilityThreshold, cash=1_000_000, commission=0.00075, trade_on_close=False
)
stats = bt.run()
