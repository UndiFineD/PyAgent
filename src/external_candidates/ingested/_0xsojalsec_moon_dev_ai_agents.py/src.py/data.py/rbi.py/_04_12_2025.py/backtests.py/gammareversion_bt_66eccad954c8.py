# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_12_2025\backtests\GammaReversion_BT.py
...
class GammaReversion(Strategy):
    ...
    def init(self):
        ...
        self.gamma_high = self.I(
            talib.MAX, self.data.High, self.gamma_period, name="Gamma High"
        )

        # ✨ Funding Rate Proxy (Close vs SMA)
        self.funding_sma = self.I(
            talib.SMA, self.data.Close, self.funding_period, name="Funding SMA"
        )

        # 🚀 Liquidation Clusters (Volume-Weighted)
        typical_price = (self.data.High + self.data.Low + self.data.Close) / 3
        vwap = (typical_price * self.data.Volume).cumsum() / self.data.Volume.cumsum()
        self.liquidation_cluster = self.I(
            lambda: vwap.rolling(self.cluster_period).mean(), name="Liquidation Cluster"
        )
    ...
    def next(self):
        ...
        # 🌙 Entry Conditions
        if current_high > gamma_level and funding_value > 0.001 and not self.position:
