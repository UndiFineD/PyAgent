...
class VolatilityCompression(Strategy):
    ...
    def init(self):
        ...
        self.trend_filter = self.I(
            SMA, self.data.Close, timeperiod=self.trend_filter_period
        )
    ...
    def next(self):
        ...
        # 🌑 Exit Logic
            ...
            if (
                self.data.Low[-1] <= self.initial_stop
                or self.data.Low[-1] <= trailing_stop
            ):
                self.position.close()
                exit_type = (
                    "HARD STOP"
                    if self.data.Low[-1] <= self.initial_stop
                    else "TRAILING STOP"
                )
                print(f"\n🌑🌠 MOON EXIT: {exit_type} Triggered!")
                print(
                    f"💰 Profit: {(self.data.Close[-1]/self.position.entry_price - 1)*100:.2f}%"
                )
