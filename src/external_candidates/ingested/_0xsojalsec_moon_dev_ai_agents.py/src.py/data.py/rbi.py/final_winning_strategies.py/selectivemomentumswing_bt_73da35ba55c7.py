# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\SelectiveMomentumSwing_BT.py
...
class SelectiveMomentumSwing(Strategy):
    ...
    def init(self):
        ...
        stoch_k, stoch_d = stochastic(
            self.data.High, self.data.Low, self.data.Close, 14, 3
        )
    ...
    def next(self):
        ...
        if (
            len(self.data)
            < max(self.ema_filter, self.volume_period, self.atr_period) + 1
        ):
            return
    ...
    def analyze_trend_structure(self):
        ...
        # Trend direction
        trend_up = (
            ema_fast_val > ema_slow_val > ema_filter_val
            and current_close > ema_filter_val
        )
        trend_down = (
            ema_fast_val < ema_slow_val < ema_filter_val
            and current_close < ema_filter_val
        ...
        strong_trend = (
            (trend_up and macd_trend_up) or (trend_down and macd_trend_down)
        ) and strength >= 1.0
    ...
    def analyze_pullback_opportunity(self):
        ...
        recent_high = (
            max(self.data.High[-20:]) if len(self.data.High) >= 20 else current_close
        )
        recent_low = (
            min(self.data.Low[-20:]) if len(self.data.Low) >= 20 else current_close
        )

        # Pullback analysis for long setups
        pullback_from_high = ((recent_high - current_close) / recent_high) * 100
        pullback_quality_long = (
            pullback_from_high >= self.pullback_threshold
            and current_close > pullback_ema_val
        )

        # Pullback analysis for short setups
        pullback_from_low = ((current_close - recent_low) / recent_low) * 100
        pullback_quality_short = (
            pullback_from_low >= self.pullback_threshold
            and current_close < pullback_ema_val
        )
    ...
    def get_swing_confirmations(self):
        ...
        if (
            self.rsi_oversold < self.rsi[-1] < 50  # Long setup
            or 50 < self.rsi[-1] < self.rsi_overbought
        ):  # Short setup
        ...
        if (
            ...
            or current_volume > self.volume_ema[-1] * 1.3
        ):
        ...
        if (
            self.rsi[-1] < 40 and self.ema_fast[-1] > self.ema_slow[-1]
        ) or (  # Oversold in uptrend
            self.rsi[-1] > 60 and self.ema_fast[-1] < self.ema_slow[-1]
        ...
        if (
            self.stoch_k[-1] < 30 and self.stoch_k[-1] > self.stoch_d[-1]
        ) or (  # Oversold turning up
            self.stoch_k[-1] > 70 and self.stoch_k[-1] < self.stoch_d[-1]
        ...
        if (
            len(self.macd_histogram) > 2
            and self.macd_histogram[-1] > self.macd_histogram[-2]
        ):
    ...
    def execute_swing_entry(self, trend_analysis, pullback_analysis):
        ...
        if direction == 1:  # Long entry
            ...
            stop_loss = max(
                recent_swing_low, ema_stop, atr_stop
            )  # Use highest for better R:R
            ...
            stop_loss = min(
                recent_swing_high, ema_stop, atr_stop
            )  # Use lowest for better R:R
        else:
            return
    ...
    def manage_swing_position(self):
        ...
        if pnl_pct > self.trailing_start:
            ...
            if self.position.is_long:
                ...
                if self.trailing_stop is None or new_trailing > self.trailing_stop:
                    ...
                    print(
                        f"📈 SWING TRAILING STOP: {self.stop_loss:.2f} (Profit: {pnl_pct:.1f}%)"
                    )
        ...
        if current_time - self.entry_time >= max_hold_bars:
            self.position.close()
            print(
                f"⏰ SWING TIME EXIT @ {current_price:.2f} ({pnl_pct:.2f}%) after {self.max_hold_days} days"
            )
            self.reset_trade_params()
            return
        ...
        if pnl_pct > 1.0:  # Only check if profitable
            ...
            if (
                ...
                and current_price < self.ema_fast[-1]
            ):
                trend_broken = True
            ...
            if trend_broken:
                self.position.close()
                print(
                    f"🔄 SWING TREND REVERSAL EXIT @ {current_price:.2f} (+{pnl_pct:.2f}%)"
                )
                self.reset_trade_params()
                return
...
bt = Backtest(data, SelectiveMomentumSwing, cash=1000000, commission=0.002)
stats = bt.run()
...
print(f"\n✅ STRATEGY VALIDATION:")
print(
    f"📊 Trade Count Requirement (>100): {'✅ PASS' if trade_requirement else '❌ FAIL'} ({stats_opt['# Trades']} trades)"
)
print(
    f"📈 Sharpe Ratio Requirement (>2.0): {'✅ PASS' if sharpe_requirement else '❌ FAIL'} ({stats_opt['Sharpe Ratio']:.2f})"
)
