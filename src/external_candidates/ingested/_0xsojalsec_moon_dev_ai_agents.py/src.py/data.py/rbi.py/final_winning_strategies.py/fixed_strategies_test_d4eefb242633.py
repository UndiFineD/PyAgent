# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\fixed_strategies_test.py
...
print("=" * 80)
print(
    f"{'Strategy':<25} {'Trades':<8} {'Return%':<10} {'Sharpe':<8} {'Status':<15} {'Changes'}"
)
print("=" * 80)
...
for strategy_config in strategies:
    try:
        ...
        print(
            f"{strategy_name:<25} {trades:<8} {returns:<10.2f} {sharpe:<8.2f} {status:<15} {changes}"
        )

    except Exception as e:
        print(
            f"{strategy_name:<25} ERROR    -         -        ❌ FAILED      {str(e)[:30]}..."
        )
        results.append(
            {
                "strategy": strategy_name,
...
failing_strategies = sum(
    1 for r in results if r["status"] in ["❌ TOO FEW", "⚠️ TOO MANY"]
)
