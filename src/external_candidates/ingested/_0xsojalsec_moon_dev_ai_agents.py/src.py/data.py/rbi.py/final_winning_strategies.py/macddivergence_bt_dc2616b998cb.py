# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\MACDDivergence_BT.py
...
class MACDDivergence(Strategy):
    ...
    def next(self):
        ...
        if (
            len(self.data)
            < max(self.macd_slow + self.macd_signal, self.divergence_lookback, 50) + 5
        ):
            return
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if (
                    ...
                    and self.macd_line[-2] >= self.macd_signal[-2]
                ):
                    self.position.close()
                    pnl = self.calculate_pnl(current_price)
        ...
        # Look for divergence signals
            ...
            if bullish_div["detected"]:
                ...
                macd_bullish = (
                    self.macd_line[-1] > self.macd_signal[-1]
                    or self.macd_histogram[-1] > self.macd_histogram[-2]
                )
                ...
                if macd_bullish:
                    ...
                    self.take_profit = current_price * (
                        1 + self.profit_target_pct / 100
                    )
                    self.entry_time = current_time

                    print(
                        f'🚀 LONG BULLISH DIV @ {current_price:.2f} | Strength: {bullish_div["strength"]:.3f}'
                    )
            ...
            if bearish_div["detected"]:
                ...
                if macd_bearish:
                    ...
                    print(
                        f'📉 SHORT BEARISH DIV @ {current_price:.2f} | Strength: {bearish_div["strength"]:.3f}'
                    )
                    print(f"   SL: {self.stop_loss:.2f} | TP: {self.take_profit:.2f}")
    ...
    def detect_bullish_divergence(self):
        ...
        for i in range(2, len(price_data) - 2):
            ...
            # MACD low detection
            if (
                macd_data[i] < macd_data[i - 1]
                and macd_data[i] < macd_data[i - 2]
                and macd_data[i] < macd_data[i + 1]
                and macd_data[i] < macd_data[i + 2]
            ):
        ...
        if price_lower and macd_higher:
            ...
            macd_change = (
                (recent_macd_low[1] - prev_macd_low[1]) / abs(prev_macd_low[1])
                if prev_macd_low[1] != 0
                else 0
            )
    ...
    def detect_bearish_divergence(self):
        ...
        # Find previous highs
        prev_price_highs = [
            high for high in price_highs if high[0] < recent_price_high[0]
        ]
        ...
        if price_higher and macd_lower:
            # Calculate divergence strength
            price_change = (
                recent_price_high[1] - prev_price_high[1]
            ) / prev_price_high[1]
            macd_change = (
                (prev_macd_high[1] - recent_macd_high[1]) / abs(prev_macd_high[1])
                if prev_macd_high[1] != 0
                else 0
            )
            strength = price_change + macd_change
...
bt = Backtest(data, MACDDivergence, cash=1000000, commission=0.002)
stats = bt.run()
...
print(f"\n✅ STRATEGY VALIDATION:")
print(
    f"📊 Trade Count Requirement (>30): {'✅ PASS' if trade_requirement else '❌ FAIL'} ({stats_opt['# Trades']} trades)"
)
print(
