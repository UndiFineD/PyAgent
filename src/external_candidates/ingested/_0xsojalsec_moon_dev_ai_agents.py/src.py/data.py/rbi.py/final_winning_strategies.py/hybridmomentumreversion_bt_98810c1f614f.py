# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\HybridMomentumReversion_BT.py
...
class HybridMomentumReversion(Strategy):
    ...
    def init(self):
        ...
        self.adx = self.I(
            calc_adx, self.data.High, self.data.Low, self.data.Close, self.adx_period
        )
        self.atr = self.I(
        ...
        self.volatility = self.I(
            calc_volatility, self.data.Close, self.volatility_period
        )
    ...
    def next(self):
        ...
        if (
            len(self.data)
            < max(
                self.ema_slow, self.bb_period, self.adx_period, self.volatility_period
            )
            + 5
        ):
            return
    ...
    def detect_market_regime(self):
        ...
        current_volatility = (
            self.volatility[-1] if not np.isnan(self.volatility[-1]) else 0
        )

        # Trending market indicators
        strong_trend = current_adx > self.adx_trend_threshold
        high_volatility = (
            current_volatility > np.nanmean(self.volatility[-50:]) * 1.2
            if len(self.volatility) >= 50
            else False
        )
    ...
    def momentum_entry_logic(self, current_price, current_volume, current_time):
        ...
        volume_surge = (
            current_volume > self.volume_sma[-1] * self.momentum_volume_threshold
        )
    ...
    def reversion_entry_logic(self, current_price, current_volume, current_time):
        ...
        volume_ok = (
            current_volume > self.volume_sma[-1] * self.reversion_volume_threshold
        )
        ...
        if (
            ...
            and current_price <= self.bb_lower[-1] * 1.01
            and volume_ok
        ):
        ...
        # Short reversion: RSI overbought + price near upper BB
            ...
            and current_price >= self.bb_upper[-1] * 0.99
            and volume_ok
        ):
    ...
    def execute_entry(self, direction, current_price, current_time, mode):
        ...
        print(
            f"   SL: {self.stop_loss:.2f} | TP: {self.take_profit:.2f} | Mode: {mode.upper()}"
        )
...
bt = Backtest(data, HybridMomentumReversion, cash=1000000, commission=0.002)
stats = bt.run()
