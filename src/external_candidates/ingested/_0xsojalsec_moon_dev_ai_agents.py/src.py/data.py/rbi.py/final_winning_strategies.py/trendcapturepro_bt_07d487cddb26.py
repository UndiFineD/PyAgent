# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\TrendCapturePro_BT.py
...
class TrendCapturePro(Strategy):
    ...
    def next(self):
        ...
        if (
            len(self.data)
            < max(self.trend_filter, self.volume_period, self.atr_period) + 1
        ):
            return
    ...
    def assess_trend_quality(self):
        ...
        uptrend_aligned = (
            ...
            and current_close > ema_fast_val
        )
        downtrend_aligned = (
            ema_fast_val < ema_slow_val < ema_filter_val
            and current_close < ema_fast_val
        ...
        adx_increasing = (
            len(self.adx) > 2 and self.adx[-1] > self.adx[-2] > self.adx[-3]
        )
    ...
    def analyze_entry_timing(self):
        ...
        near_pullback_ema = (
            abs(current_close - pullback_ema_val) / pullback_ema_val < 0.005
        )
        ...
        macd_bullish = (
            ...
            and self.macd_line[-1] > self.macd_line[-2]
        )
        macd_bearish = (
            self.macd_line[-1] < self.macd_signal[-1]
            and self.macd_line[-1] < self.macd_line[-2]
    ...
    def get_trend_confirmations(self):
        ...
        if (
            ...
            or current_volume > self.volume_ema[-1] * 1.6
        ):
        ...
        if (ema_fast_rising and ema_slow_rising) or (
            not ema_fast_rising and not ema_slow_rising
        ):
        ...
        if (
            self.di_plus[-1] > self.di_minus[-1] * 1.2
            or self.di_minus[-1] > self.di_plus[-1] * 1.2
        ):
    ...
    def execute_trend_entry(self, trend_assessment, entry_timing):
        ...
        if position_size > 0:
            ...
            print(
                f'   Trend Strength: {trend_assessment["strength"]}/10 | ADX: {trend_assessment["adx_value"]:.1f}'
            )
    ...
    def manage_trend_position(self):
        ...
        if current_time - self.entry_time >= self.max_hold_bars:
            self.position.close()
            print(
                f"⏰ TREND TIME EXIT @ {current_price:.2f} ({pnl_pct:.2f}%) after {self.max_hold_bars} bars"
            )
            self.reset_trade_params()
            return
        ...
        if (
            pnl_pct > 0.5
            and len(self.adx) > 5
            and self.adx[-1] < self.adx_threshold * 0.8
            and self.adx[-1] < self.adx[-3]
        ):
            self.position.close()
            ...
            if pnl_pct >= level and i >= len(self.trailing_levels):
                ...
                if self.position.is_long:
                    ...
                    if new_trail_stop > self.current_stop:
                        ...
                        print(
                            f"📈 TREND TRAIL LEVEL {i+1}: {self.current_stop:.2f} (Profit: {pnl_pct:.1f}%)"
                        )
                else:
                    new_trail_stop = current_price + (current_atr * distance)
                    if new_trail_stop < self.current_stop:
...
bt = Backtest(data, TrendCapturePro, cash=1000000, commission=0.002)
stats = bt.run()
