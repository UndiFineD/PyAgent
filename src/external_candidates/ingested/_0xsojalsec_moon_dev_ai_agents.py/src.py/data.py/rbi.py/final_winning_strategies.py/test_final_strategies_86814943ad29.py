# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\test_final_strategies.py
...
def test_single_strategy(strategy_class, strategy_name, data, run_optimization=True):
    ...
    try:
        ...
        default_results = {
            ...
            "overall_success": stats_default["# Trades"] > 100
            and stats_default["Sharpe Ratio"] > 2.0,
        }
        ...
        if run_optimization:
            try:
                ...
                optimized_results = {
                    ...
                    "overall_success": stats_opt["# Trades"] > 100
                    and stats_opt["Sharpe Ratio"] > 2.0,
                }
        ...
        print(f"\n✅ VALIDATION SUMMARY:")
        print(
            f"📊 Trade Count (>100): {'✅ PASS' if best_results['trade_requirement'] else '❌ FAIL'} ({best_results['trades']} trades)"
        )
        print(
            f"📈 Sharpe Ratio (>2.0): {'✅ PASS' if best_results['sharpe_requirement'] else '❌ FAIL'} ({best_results['sharpe_ratio']:.2f})"
        )
        print(
            f"🏆 Overall Success: {'✅ SUCCESS' if best_results['overall_success'] else '❌ NEEDS WORK'}"
        )
...
def run_comprehensive_test_suite():
    ...
    for strategy_config in strategies:
        ...
        strategy_class = load_strategy_class(
            strategy_config["file"], strategy_config["class"]
        )
        ...
        default_result, optimized_result = test_single_strategy(
            strategy_class, strategy_config["name"], data, run_optimization=True
        )
    ...
    if successful_strategies:
        ...
        print("-" * 80)
        print(
            f"{'Strategy':<25} {'Trades':<8} {'Return%':<9} {'Sharpe':<8} {'DrawDn%':<9} {'WinRate%':<9}"
        )
        print("-" * 80)
    ...
    print("-" * 90)
    print(
        f"{'Strategy':<25} {'Type':<10} {'Trades':<8} {'Return%':<9} {'Sharpe':<8} {'MaxDD%':<8} {'Success':<8}"
    )
    print("-" * 90)
    ...
    print(
        f"📊 Trade Count Requirement (>100): {len(trade_passers)}/{len(all_results)} strategies"
    )
    print(
        f"📈 Sharpe Ratio Requirement (>2.0): {len(sharpe_passers)}/{len(all_results)} strategies"
    )
    print(
        f"🏆 Both Requirements Met: {len(successful_strategies)}/{len(all_results)} strategies"
    )
    ...
    if all_results:
        ...
        avg_sharpe = np.mean(
            [r["sharpe_ratio"] for r in all_results if not np.isnan(r["sharpe_ratio"])]
        )
    ...
    success_rate = (
        len(successful_strategies) / len(strategies) * 100 if strategies else 0
    )
        ...
        with open(summary_file, "w") as f:
            ...
            if winners:
                ...
                for winner in winners:
                    f.write(
                        f"- {winner['strategy']}: {winner['trades']} trades, {winner['sharpe_ratio']:.2f} Sharpe\n"
                    )
