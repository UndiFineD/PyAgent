...
class DivergenceVolatilityEnhanced(Strategy):
            ...
            if current_index - prev_idx >= self.min_separation:
                ...
                if (
                    ...
                    if confirmations["total"] >= 4:  # Require at least 4 confirmations
                        print(
                            f'✅ HIGH CONFIDENCE ENTRY: {confirmations["total"]}/6 confirmations'
                        )
    ...
    def get_entry_confirmations(self):
        ...
        if self.price_ema_fast[-1] > self.price_ema_slow[
            -1
        ] or current_close > np.nanmean(  # Uptrend structure
            self.data.Close[-5:]
        ...
        if (
            risk_per_unit > 0 and potential_reward / risk_per_unit >= 2.0
        ):  # At least 2:1 R:R
    ...
    def manage_position(self):
        ...
        if profit_pct > self.trailing_start:  # Start trailing after profit threshold
            ...
            if new_stop > self.stop_loss:
                ...
                print(
                    f"📈 TRAILING STOP: {self.stop_loss:.2f} (Profit: {profit_pct:.1f}%)"
                )
        ...
        if current_time - self.entry_time >= self.max_hold_bars:
            ...
            print(
                f"⏰ TIME EXIT @ {current_price:.2f} ({final_pct:.2f}%) after {self.max_hold_bars} bars"
            )
            self.reset_trade_params()
            return
        ...
        if (
            ...
            self.position.close()
            print(
                f"🔄 MOMENTUM REVERSAL EXIT @ {current_price:.2f} (+{profit_pct:.2f}%)"
            )
            self.reset_trade_params()
            return
