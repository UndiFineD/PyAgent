# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\quick_strategy_test.py
...
def check_strategy_parameters():
    ...
    print("=" * 80)
    print(
        f"{'Strategy':<30} {'Parameter Updates':<25} {'Constraint Removed':<20} {'Status'}"
    )
    print("=" * 80)
    ...
    for strategy in strategies_to_check:
        ...
        try:
            ...
            params_found = all(
                change in content for change in strategy["expected_changes"]
            )

            # Check that constraint was removed
            constraint_removed = not any(
                removed in content for removed in strategy["removed"]
            )
            ...
            print(
                f"{strategy['file'][:-3]:<30} {param_status:<25} {constraint_status:<20} {status}"
            )
            ...
            print(
                f"{strategy['file'][:-3]:<30} {'❌ FILE NOT FOUND':<25} {'❌':<20} {'❌ ERROR'}"
            )
            all_good = False
        except Exception as e:
            print(
                f"{strategy['file'][:-3]:<30} {'❌ ERROR':<25} {'❌':<20} {'❌ ERROR'}"
            )
            all_good = False
