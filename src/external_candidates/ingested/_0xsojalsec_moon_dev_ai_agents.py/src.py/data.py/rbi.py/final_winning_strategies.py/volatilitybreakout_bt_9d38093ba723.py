# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\FINAL_WINNING_STRATEGIES\VolatilityBreakout_BT.py
...
class VolatilityBreakout(Strategy):
    ...
    def next(self):
        ...
        # Look for breakout signals
            ...
            if (
                current_high > upper_breakout
                and current_price > upper_breakout
                and volume_surge
            ):
                ...
                breakout_strength = (
                    (current_price - upper_breakout) / upper_breakout * 100
                )
                print(
                    f"🚀 LONG BREAKOUT @ {current_price:.2f} | SL: {self.stop_loss:.2f} | TP: {self.take_profit:.2f}"
                )
                print(
                    f"   Breakout: {breakout_strength:.2f}% | ATR: {current_atr:.2f} | Vol: {volume_surge}"
                )

            # Short breakdown: price breaks below lower level with volume
            elif (
                current_low < lower_breakout
                and current_price < lower_breakout
                and volume_surge
            ):
                ...
                breakdown_strength = (
                    (lower_breakout - current_price) / lower_breakout * 100
                )
                print(
                    f"📉 SHORT BREAKDOWN @ {current_price:.2f} | SL: {self.stop_loss:.2f} | TP: {self.take_profit:.2f}"
                )
                print(
                    f"   Breakdown: {breakdown_strength:.2f}% | ATR: {current_atr:.2f} | Vol: {volume_surge}"
                )
...
bt = Backtest(data, VolatilityBreakout, cash=1000000, commission=0.002)
stats = bt.run()
...
print(f"\n✅ STRATEGY VALIDATION:")
print(
    f"📊 Trade Count Requirement (>40): {'✅ PASS' if trade_requirement else '❌ FAIL'} ({stats_opt['# Trades']} trades)"
)
print(
