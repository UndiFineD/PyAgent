# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_07_2025\backtests_package\VolTerminus_PKG.py
...
class VolTerminus(Strategy):
    ...
    def init(self):
        ...
        self.bbw_percentile = self.I(
            lambda: self.bbw.rolling(self.bbw_percentile_window).quantile(
                self.bbw_threshold
            )
        )
    ...
    def next(self):
        ...
        if not self.position:
            if self.vix_inversion[-1] and self.bbw[-1] < self.bbw_percentile[-1]:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🚀 MOON SHOT! Long {position_size} units @ {price:.2f} | Stop {stop_price:.2f} 🌙"
                    )
        ...
        # Exit Logic 🚀➡️🌑
            ...
            if self.z_score[-1] >= 2:
                self.position.close()
                print(
                    f"🌑 VOL EXPANSION COMPLETE! Closing @ {price:.2f} | Profit: {self.position.pl_usd:.2f} ✨"
                )
...
# Launch Backtest 🚀
bt = Backtest(data, VolTerminus, cash=1_000_000, exclusive_orders=True)
stats = bt.run()
