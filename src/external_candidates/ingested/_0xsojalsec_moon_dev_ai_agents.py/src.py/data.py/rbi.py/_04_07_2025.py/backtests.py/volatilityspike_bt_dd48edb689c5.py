# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_07_2025\backtests\VolatilitySpike_BT.py
import pandas as pd
import talib
...
class VolatilitySpike(Strategy):
    ...
    def init(self):
        ...
        self.intraday_atr = self.I(
            talib.ATR,
            self.data.High,
            self.data.Low,
            self.data.Close,
            self.intraday_atr_period,
            name="Intraday_ATR",
        )
        self.daily_atr = self.I(
            talib.ATR,
            self.data.High,
            self.data.Low,
            self.data.Close,
            self.daily_atr_period,
            name="Daily_ATR",
        )
        self.vol_ma = self.I(
            talib.SMA, self.data.Volume, self.vol_ma_period, name="Volume_MA"
        )
        ...
        if not self.position:
            ...
            if atr_spike and volume_confirm:
                ...
                stop_loss = entry_price - (
                    self.daily_atr[-1] * self.atr_stop_multiplier
                )
                ...
                print(
                    f"🚀 Moon Dev LONG Entry! 🌕 | Size: {position_size} | Entry: {entry_price:.2f} | SL: {stop_loss:.2f} | TP: {take_profit:.2f}"
                )
        else:
...
bt = Backtest(data, VolatilitySpike, cash=1_000_000, commission=0.002)
stats = bt.run()
