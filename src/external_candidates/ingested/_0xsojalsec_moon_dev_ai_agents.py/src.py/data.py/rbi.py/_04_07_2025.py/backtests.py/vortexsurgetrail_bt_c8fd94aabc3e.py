# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_07_2025\backtests\VortexSurgeTrail_BT.py
import numpy as np
import pandas as pd
...
class VortexSurgeTrail(Strategy):
    ...
    def init(self):
        ...
        self.vi_plus, self.vi_minus = self.I(
            talib.VORTEX, h, l, c, self.vi_period, name=["VI+", "VI-"], plot=True
        )
            ...
            if crossover(self.vi_plus, self.vi_minus):
                if self.data.Volume[-1] > self.vol_ma[-1]:
                    ...
                    if position_size > 0:
                        ...
                        print(
                            f"🌙✨🚀 LUNAR LIFT-OFF! Buying {position_size} units at {price:.2f}"
                        )
...
# Launch Backtest 🚀
bt = Backtest(data, VortexSurgeTrail, cash=1_000_000, commission=0.002)
stats = bt.run()
print(stats)
