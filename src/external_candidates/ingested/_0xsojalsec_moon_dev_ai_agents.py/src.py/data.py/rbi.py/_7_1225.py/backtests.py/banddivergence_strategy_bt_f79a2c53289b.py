# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_1225\backtests\BandDivergence_strategy_bt.py
import pandas as pd
import talib
    ...
    def next(self):
        if self.position:
            if self.position.is_long:
                if (
                    (len(self.macd) >= 2 and self.macd[-1] < 0 and self.macd[-2] > 0)
                    or self.data.Close[-1] < self.swing_low[-1]
                    or (len(self.middle) >= 2 and self.middle[-1] <= self.middle[-2])
                ):
                    print(
                        "🌙 Moon Dev exit condition met for long! Closing position. ✨"
                    )
                    self.position.close()
            elif self.position.is_short:
                if (
                    (len(self.macd) >= 2 and self.macd[-1] > 0 and self.macd[-2] < 0)
                    or self.data.Close[-1] > self.swing_high[-1]
                    or (len(self.middle) >= 2 and self.middle[-1] >= self.middle[-2])
                ):
                    print(
                        "🌙 Moon Dev exit condition met for short! Closing position. ✨"
                    )
                    self.position.close()
            return
        ...
        if uptrend and middle_higher and macd_above:
            ...
            if position_size > 0:
                ...
                print(
                    f"🌙 Moon Dev entered long with size {position_size}! SL: {sl}, TP: {tp} ✨"
                )
            else:
        ...
        if downtrend and width_tight and macd_below:
            ...
            if position_size > 0:
                ...
                print(
                    f"🌙 Moon Dev entered short with size {position_size}! SL: {sl}, TP: {tp} ✨"
                )
            else:
