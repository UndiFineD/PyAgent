# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_1225\backtests\VolatilityReversal_strategy_bt.py
import pandas as pd
import talib
...
class VolatilityReversalStrategy(Strategy):
    ...
    def next(self):
        if self.position:
            if (
                self.data.Close[-1] > self.channel_upper[-1]
                or self.atr[-1] > 1.5 * self.avg_atr[-1]
            ):
        ...
        if (
            ...
            and bb_expanding
            and self.rsi[-1] < 30
            and self.data.Close[-1] <= self.lower[-1]
        ):
...
bt = Backtest(
    data, VolatilityReversalStrategy, cash=1000000, margin=1, commission=0.001
)
stats = bt.run()
print(stats)
