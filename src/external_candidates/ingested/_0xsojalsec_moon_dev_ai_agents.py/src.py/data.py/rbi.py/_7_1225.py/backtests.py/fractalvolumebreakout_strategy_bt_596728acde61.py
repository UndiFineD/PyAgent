# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\7_1225\backtests\FractalVolumeBreakout_strategy_bt.py
import pandas as pd
import talib
...
class FractalVolumeBreakoutStrategy(Strategy):
    ...
    def next(self):
        ...
        if (
            self.data.Close[-1] > self.fractal_high[-3]
            and self.data.Volume[-1] > 1.5 * self.vma[-1]
        ):
        ...
        if (
            self.data.Close[-1] < self.fractal_low[-3]
            and self.data.Volume[-1] > 1.5 * self.vma[-1]
        ):
...
bt = Backtest(
    data, FractalVolumeBreakoutStrategy, cash=1000000, margin=1, commission=0.001
)
stats = bt.run()
print(stats)
