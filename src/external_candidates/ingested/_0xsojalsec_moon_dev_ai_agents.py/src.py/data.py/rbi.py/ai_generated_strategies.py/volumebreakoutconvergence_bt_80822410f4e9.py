# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_GENERATED_STRATEGIES\VolumeBreakoutConvergence_BT.py
# -*- coding: utf-8 -*-
...
class VolumeBreakoutConvergence(Strategy):
    ...
    def init(self):
        ...
        self.resistance = self.I(
            talib.MAX, self.data.High, timeperiod=self.resistance_period
        )
        self.support = self.I(talib.MIN, self.data.Low, timeperiod=self.support_period)

        # Volume analysis
        self.volume_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.volume_period
        )
        self.volume_ema = self.I(
            talib.EMA, self.data.Volume, timeperiod=self.volume_period
        )
        ...
        self.cci = self.I(
            talib.CCI, self.data.High, self.data.Low, self.data.Close, timeperiod=20
        )
        self.williams_r = self.I(
            talib.WILLR, self.data.High, self.data.Low, self.data.Close, timeperiod=14
        )
    ...
    def ema_convergence(self):
        ...
        past_emas = [
            self.ema_8[-self.convergence_lookback],
            self.ema_13[-self.convergence_lookback],
            self.ema_21[-self.convergence_lookback],
            self.ema_34[-self.convergence_lookback],
        ]
    ...
    def volume_surge(self):
        ...
        return (
            ...
            and self.data.Volume[-1] > self.volume_ema[-1] * self.volume_multiplier
        )
    ...
    def next(self):
        ...
        # Breakout detection
        resistance_breakout = (
            prev_price <= resistance_level
            and current_price > resistance_level * (1 + self.breakout_threshold)
        )
        support_breakout = (
            prev_price >= support_level
            and current_price < support_level * (1 - self.breakout_threshold)
        )
        ...
        if not self.position:
            ...
            if (
                ...
                and self.ema_convergence()
                and self.rsi[-1] >= self.rsi_neutral_low
                and self.rsi[-1] <= self.rsi_neutral_high
                and self.adx[-1] > 25  # Strong trend
                and self.macd[-1] > self.macd_signal[-1]  # MACD bullish
                and self.cci[-1] < 100  # Not extremely overbought
                and self.williams_r[-1] > -80
            ):  # Not oversold

                # Calculate position size
                stop_loss = current_price - (self.atr[-1] * self.atr_stop_multiplier)
                profit_target = current_price + (
                    self.atr[-1] * self.atr_target_multiplier
                )
                ...
                if risk_per_share > 0:
                    ...
                    position_size = min(
                        int(self.equity * 0.95 / current_price),
            ...
            # Short entry: Support breakdown with volume and convergence
                ...
                and self.rsi[-1] <= self.rsi_neutral_high
                and self.adx[-1] > 25  # Strong trend
                and self.macd[-1] < self.macd_signal[-1]  # MACD bearish
                and self.cci[-1] > -100  # Not extremely oversold
                and self.williams_r[-1] < -20
            ):  # Not overbought
