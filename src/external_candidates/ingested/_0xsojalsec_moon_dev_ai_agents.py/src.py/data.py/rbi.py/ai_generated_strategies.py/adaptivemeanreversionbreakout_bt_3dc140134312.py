# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_GENERATED_STRATEGIES\AdaptiveMeanReversionBreakout_BT.py
# -*- coding: utf-8 -*-
...
class AdaptiveMeanReversionBreakout(Strategy):
    ...
    def init(self):
        ...
        self.trend_sma = self.I(
            talib.SMA, self.data.Close, timeperiod=self.trend_period
        )
    ...
    def next(self):
        ...
        volume_ratio = (
            self.data.Volume[-1] / self.volume_sma[-1] if self.volume_sma[-1] > 0 else 1
        )
        ...
        if not self.position:
            ...
            if (
                ...
                and self.rsi[-1] <= self.rsi_extreme_low
                and self.stoch_k[-1] <= 20
                and trend_up  # Only trade in direction of main trend
                and ema_trend_up  # EMA trend confirmation
                ...
                profit_target = current_price + (
                    self.atr[-1] * self.profit_target_multiplier
                )
            ...
            # Short entry: Overbought rejection with trend confirmation
                ...
                and self.rsi[-1] >= self.rsi_extreme_high
                and self.stoch_k[-1] >= 80
                and not trend_up  # Only short in bearish trend
                and not ema_trend_up  # EMA trend confirmation
            ...
            if self.position.is_long:
                ...
                if (
                    current_price >= self.bb_middle[-1]
                    or self.rsi[-1] >= 70
                    or not ema_trend_up
...
bt = Backtest(
    data, AdaptiveMeanReversionBreakout, cash=1_000_000, exclusive_orders=True
)
stats = bt.run()
