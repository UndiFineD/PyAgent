# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\AI_GENERATED_STRATEGIES\test_strategies_simple.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def simple_backtest(data, signals, initial_capital=1000000, risk_pct=0.02):
    ...
    portfolio["strategy_returns"] = (
        portfolio["positions"].shift(1) * portfolio["returns"]
    )
    ...
    annual_return = (
        (1 + portfolio["strategy_returns"].mean()) ** (252 * 96) - 1
    ) * 100  # 15min = 96 periods/day
...
for name, result in strategies:
    print(
        f"{name:<30} {result['Sharpe Ratio']:<8.2f} {result['Total Trades']:<8.0f} {result['Total Return [%]']:<10.2f} {result['Win Rate [%]']:<8.1f}"
    )
