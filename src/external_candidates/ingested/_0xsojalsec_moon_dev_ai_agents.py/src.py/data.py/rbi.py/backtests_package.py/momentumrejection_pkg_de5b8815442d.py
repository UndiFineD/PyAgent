# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests_package\MomentumRejection_PKG.py
import pandas as pd
import talib
...
class MomentumRejection(Strategy):
    ...
    def init(self):
        ...
        self.uptrend_line = self.I(
            talib.MIN, self.data.Low, timeperiod=20
        )  # Higher lows for uptrend
        self.downtrend_line = self.I(
            talib.MAX, self.data.High, timeperiod=20
    ...
    def next(self):
        ...
        if (
            ...
            and stoch_d < self.stoch_oversold
        ):
            if (
                stoch_k[-2] < stoch_d[-2] and stoch_k[-1] > stoch_d[-1]
            ):  # Stochastic crossover confirmation
                self.buy(size=position_size)
                print(
                    f"🌙 Moon Dev Buy Signal: Continuation Uptrend | Price: {price} | Stochastic: {stoch_k}, {stoch_d}"
                )

        # Entry logic for continuation pattern (downtrend)
        elif (
            price < downtrend_line
            and stoch_k > self.stoch_overbought
            and stoch_d > self.stoch_overbought
        ):
            if (
                stoch_d[-2] < stoch_k[-2] and stoch_d[-1] > stoch_k[-1]
            ):  # Stochastic crossover confirmation
                self.sell(size=position_size)
                print(
                    f"🌙 Moon Dev Sell Signal: Continuation Downtrend | Price: {price} | Stochastic: {stoch_k}, {stoch_d}"
                )
        ...
        if (
            ...
            if (
                ...
                self.sell(size=position_size)
                print(
                    f"🌙 Moon Dev Sell Signal: Breakout Uptrend Reversal | Price: {price} | Stochastic: {stoch_k}, {stoch_d}"
                )
        ...
        # Entry logic for breakout pattern (downtrend reversal)
            ...
            if (
                ...
                self.buy(size=position_size)
                print(
                    f"🌙 Moon Dev Buy Signal: Breakout Downtrend Reversal | Price: {price} | Stochastic: {stoch_k}, {stoch_d}"
                )
