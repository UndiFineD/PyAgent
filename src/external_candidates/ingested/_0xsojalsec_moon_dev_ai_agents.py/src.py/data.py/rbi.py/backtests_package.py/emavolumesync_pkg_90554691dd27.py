# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests_package\EMAVolumeSync_PKG.py
import pandas as pd
import talib
...
class EMAVolumeSync(Strategy):
    ...
    def next(self):
        ...
        # Trend direction
        is_uptrend = (
            current_close > self.green_ema[-1] and current_close > self.red_ema[-1]
        )
        is_downtrend = (
            current_close < self.green_ema[-1] and current_close < self.red_ema[-1]
        )
        ...
        if is_uptrend and volume_confirmation:
            if not self.position:
                ...
                self.buy(
                    ...
                    sl=stop_loss,
                    tp=current_close
                    + (current_close - stop_loss) * self.risk_reward_ratio,
                )
                print(
                    f"🚀 Long Entry at {current_close} | SL: {stop_loss} | TP: {current_close + (current_close - stop_loss) * self.risk_reward_ratio}"
                )
        ...
        # Entry logic for short trades
            ...
            if not self.position:
                ...
                self.sell(
                    ...
                    sl=stop_loss,
                    tp=current_close
                    - (stop_loss - current_close) * self.risk_reward_ratio,
                )
                print(
                    f"📉 Short Entry at {current_close} | SL: {stop_loss} | TP: {current_close - (stop_loss - current_close) * self.risk_reward_ratio}"
                )
        ...
        if self.position:
            ...
            if is_uptrend and (
                self.data.Close[-2] > self.red_ema[-2]
                and self.data.Close[-1] < self.red_ema[-1]
            ):
                self.position.close()
            ...
            # Bearish crossover detection (replacing crossover)
                ...
                self.data.Close[-2] < self.green_ema[-2]
                and self.data.Close[-1] > self.green_ema[-1]
            ):
                self.position.close()
