# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests_package\DynamicRetest_PKG.py
#!/usr/bin/env python3
"""
...
class DynamicRetest(Strategy):
    ...
    def init(self):
        ...
        self.zone_top = self.I(
            talib.MAX, self.data.High, timeperiod=self.consolidation_span
        )
        self.zone_bottom = self.I(
            talib.MIN, self.data.Low, timeperiod=self.consolidation_span
        )
        # Although our strategy is pure price action, we initialize a 20‐period SMA for optional context.
        self.sma20 = self.I(talib.SMA, self.data.Close, timeperiod=20)
        print(
            "🌙✨ Moon Dev: INIT complete – indicators (zone_top, zone_bottom, sma20) are online! 🚀"
        )
    ...
    def next(self):
        ...
        )  # e.g., 1 becomes 1% risk per trade

        # Debug print – current bar info with Moon Dev flair
        print(
            f"🌙✨ Moon Dev: Processing bar {self.data.index[-1]} → Open: {current_open:.2f}, High: {current_high:.2f}, Low: {current_low:.2f}, Close: {current_close:.2f}"
        )
        print(
            f"🌙 Debug: Consolidation zone [Top: {curr_zone_top:.2f} | Bottom: {curr_zone_bottom:.2f}] over last {self.consolidation_span} bars."
        )
        ...
        if self.position:
            print(
                "🌙✨ Moon Dev: Currently in a trade – holding position with lunar patience!"
            )
            return
        ...
        if trend == "up" and current_close > current_open:
            ...
            if curr_zone_bottom < current_close <= curr_zone_top:
                risk = (
                    current_close - curr_zone_bottom
                )  # Risk per unit for a long trade
                if risk <= 0:
                    print(
                        "🚀 Moon Dev: Calculated risk for LONG trade is non-positive. Skipping... "
                    )
                    return
                if eff_risk_reward < 2.5:
                    print(
                        "✨ Moon Dev: Effective risk/reward ratio below 2.5:1 for LONG. Aborting trade!"
                    )
                    return
                ...
                if recent_high < take_profit:
                    print(
                        f"🚀 Moon Dev: Recent high ({recent_high:.2f}) is below desired TP ({take_profit:.2f}). LONG trade skipped!"
                    )
                    return
                ...
                print(
                    f"🌙🚀 Moon Dev: LONG signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size:.4f}"
                )
        ...
        if trend == "down" and current_close < current_open:
            ...
            if curr_zone_bottom <= current_close < curr_zone_top:
                ...
                if risk <= 0:
                    print(
                        "🚀 Moon Dev: Calculated risk for SHORT trade is non-positive. Skipping... "
                    )
                    return
                if eff_risk_reward < 2.5:
                    print(
                        "✨ Moon Dev: Effective risk/reward ratio below 2.5:1 for SHORT. Aborting trade!"
                    )
                    return
                potential_reward = risk * eff_risk_reward
                take_profit = current_close - potential_reward
                ...
                if recent_low > take_profit:
                    print(
                        f"🚀 Moon Dev: Recent low ({recent_low:.2f}) is above desired TP ({take_profit:.2f}). SHORT trade skipped!"
                    )
                    return
                ...
                print(
                    f"🌙🚀 Moon Dev: SHORT signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size:.4f}"
                )
        ...
        print(
            "🌙🔍 Moon Dev: No valid trade signal on this bar. Scanning the cosmos for the perfect setup…"
        )
    ...
    def onexit(self, trade):
        ...
        print(
            f"✨ Trade Details → Entry: {trade.entry:.2f}, Exit: {trade.exit:.2f}, PnL: {trade.pl:.2f}, Bars held: {trade.barssince}"
        )
...
optimized_stats = bt.optimize(
    risk_reward=range(25, 31, 1),  # Effective risk_reward: 2.5:1 to 3.0:1
