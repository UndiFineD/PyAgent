# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\backtests_package\VengeanceTrend_PKG.py
import numpy as np
import pandas as pd
import talib
...
class VengeanceTrend(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙 Initialized VengeanceTrend Strategy with ATR-based trailing stops! 🚀"
        )
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                if self.data.Close[-1] < self.data.Close[-2]:  # Pullback
                    self.buy(
                        size=position_size,
                        sl=self.data.Close[-1]
                        - self.trailing_stop_multiplier * current_atr,
                    )
                    print(
                        f"🚀 Entered Long at {self.data.Close[-1]} with trailing stop! 🌙"
                    )
        ...
        if self.position:
            if self.position.is_long:
                ...
                new_sl = (
                    self.data.Close[-1] - self.trailing_stop_multiplier * current_atr
                )
