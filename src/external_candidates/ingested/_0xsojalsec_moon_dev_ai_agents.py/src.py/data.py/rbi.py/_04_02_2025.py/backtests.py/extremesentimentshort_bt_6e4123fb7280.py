# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_02_2025\backtests\ExtremeSentimentShort_BT.py
import pandas as pd
...
assert (
    "long_short_ratio" in data.columns
), "🌙 CRITICAL: Missing long_short_ratio column!"
...
class ExtremeSentimentShort(Strategy):
    ...
    def init(self):
        ...
        self.fr_threshold = self.I(
            lambda ma, std: ma + 2 * std, self.fr_ma, self.fr_std
        )
        ...
        self.lower_bb = self.I(
            talib.BBANDS, self.data.Close, self.bb_length, self.bb_dev, self.bb_dev
        )[2]
    ...
    def next(self):
        ...
        if not self.position:
            if current_fr > fr_threshold and current_ls >= ls_pct:
                print(
                    f"🚀🚀🚀 MOON DEV SHORT ACTIVATED! Selling at {self.data.Close[-1]:.2f} 🚀🚀🚀"
                )
