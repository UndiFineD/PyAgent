# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_02_2025\backtests_final\LiquidityDivergence_BTFinal.py
...
class LiquidityDivergence(Strategy):
    ...
    def init(self):
        ...
        print(
            "✨ All systems nominal - backtesting.lib dependencies purged from orbit! 🚀"
        )
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌕 Moon Phase Update | Bar: {len(self.data)} | OBI: {current_obi:.1f}% | Funding: {current_funding:.4f}"
            )
        ...
        if not self.position:
            ...
            exit_condition = (
                current_obi < self.obi_exit
                or current_funding > 0
                or (len(self.data) - self.entry_bar) >= self.max_bars_held
            )
