# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi\04_02_2025\backtests\VoltaSqueeze_BT.py
# -*- coding: utf-8 -*-
import pandas as pd
import talib
        ...
        if not self.position:
            ...
            if squeeze_on and rsi_cross:
                ...
                if position_size > 0:
                    ...
                    print(
                        f"🌙 MOON DEV LONG ENTRY 🚀 | Size: {position_size} | Entry: {self.entry_price:.2f} | ATR: {atr_value:.2f}"
                    )
        else:
            # Dynamic Exit Logic ✨
            ...
            if self.data.High[-1] >= target_price:
                self.position.close()
                print(
                    f"✨ MOON DEV PROFIT TAKEOFF 🌕 | Exit: {self.data.Close[-1]:.2f} | Profit: {self.position.pl:.2f}"
                )
