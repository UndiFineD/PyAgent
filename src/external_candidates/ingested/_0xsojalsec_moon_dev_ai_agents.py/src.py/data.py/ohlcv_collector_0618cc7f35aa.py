# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\ohlcv_collector.py
"""
...
def collect_token_data(
    token, days_back=DAYSBACK_4_DATA, timeframe=DATA_TIMEFRAME, exchange="SOLANA"
):
    ...
    try:
        ...
        bars_per_day = {
            ...
            "3d": 1 / 3,
            "1w": 1 / 7,
            "1month": 1 / 30,  # lowercase versions
        }

        bars_needed = int(
            days_back * bars_per_day.get(timeframe, 24)
        )  # Default to hourly if unknown
        cprint(
            f"📊 Calculating bars: {days_back} days × {timeframe} = {bars_needed} bars",
            "cyan",
        )
        ...
        if hl_timeframe != timeframe:
            cprint(
                f"🔄 Converting timeframe: {timeframe} → {hl_timeframe} (for HyperLiquid)",
                "yellow",
            )
        ...
        if data is None or data.empty:
            cprint(
                f"❌ Moon Dev's AI Agent couldn't fetch data for {token}",
                "white",
                "on_red",
            )
            return None
        ...
        cprint(
            f"💾 Moon Dev's AI Agent cached data for {token[:4]}", "white", "on_green"
        )
