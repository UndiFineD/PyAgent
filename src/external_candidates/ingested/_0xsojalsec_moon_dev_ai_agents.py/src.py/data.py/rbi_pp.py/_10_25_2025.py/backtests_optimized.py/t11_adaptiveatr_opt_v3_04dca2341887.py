# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙✨ AdaptiveAtr indicators initialized with EMA, RSI, and ADX optimizations! 🚀"
        )
    ...
    def calculate_size(self):
        ...
        return (
            max(size, 0.01) if size > 0 else 0
        )  # Minimum fractional size to avoid zero
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙📊 Bar {len(self.data)}: Close={current_price:.2f}, EMA50={self.ema50[-1]:.2f}, EMA200={self.ema200[-1]:.2f}, ATR={atr:.2f}, RSI={self.rsi[-1]:.2f}, ADX={self.adx[-1]:.2f}"
            )
        ...
        if not self.position:
            ...
            if (
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size:.2f}, ATR: {atr:.2f}, Vol OK: {vol_ok}, RSI: {self.rsi[-1]:.2f}, ADX: {self.adx[-1]:.2f} ✨"
                    )
                else:
                ...
                if size > 0:
                    ...
                    self.entry_step = len(self.data) - 1
                    print(
                        f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size:.2f}, ATR: {atr:.2f}, Vol OK: {vol_ok}, RSI: {self.rsi[-1]:.2f}, ADX: {self.adx[-1]:.2f} ✨"
                    )
                else:
                    print("🌙❌ Short signal but size=0, skipped.")
            elif long_cross or short_cross:
                print(
                    f"🌙⚠️ Crossover detected but filters failed: ATR_OK={atr_ok}, Vol_OK={vol_ok}, Trend_L={trend_ok_long}, Trend_S={trend_ok_short}, RSI_L={rsi_long_ok}, RSI_S={rsi_short_ok}, ADX={adx_ok}"
                )
            else:
                pass  # No signal
