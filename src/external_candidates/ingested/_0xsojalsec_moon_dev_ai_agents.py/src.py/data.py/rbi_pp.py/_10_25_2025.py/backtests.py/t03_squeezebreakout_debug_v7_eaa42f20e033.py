# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T03_SqueezeBreakout_DEBUG_v7.py
import numpy as np
import pandas as pd
import talib
...
class SqueezeBreakout(Strategy):
    ...
    def next(self):
        ...
        prev_squeeze = (
            bool(prev_squeeze_val) if not pd.isna(prev_squeeze_val) else False
        )
        ...
        is_lowest_width = (
            not pd.isna(current_bb_width)
            and not pd.isna(min_width)
            and np.isclose(current_bb_width, min_width, atol=1e-10)
        )
        ...
        if squeeze_ended:
            print(
                f"🌙 Moon Dev: Squeeze ended detected at {current_price:.2f}, vol_osc {current_vol_osc:.2f}, bb_width {current_bb_width:.4f} 🔍✨"
            )
