# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v7.py
import numpy as np
import pandas as pd
import talib
...
class VigorConvergence(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            if self.position.is_long:
                ...
                if trail_sl > self.position.sl:
                    ...
                    print(
                        f"🌙 Moon Dev: Trailing SL for long to {trail_sl} at {self.data.index[-1]} 🚀"
                    )
                ...
                if trail_sl < self.position.sl:
                    self.position.sl = trail_sl
                    print(
                        f"🌙 Moon Dev: Trailing SL for short to {trail_sl} at {self.data.index[-1]} 🚀"
                    )
        ...
        if self.position and current_adx < self.adx_exit_threshold:
            self.position.close()
            print(
                f"🌙 Moon Dev: Emergency exit due to weak trend (ADX < {self.adx_exit_threshold}) at {self.data.index[-1]} 🚀"
            )
        ...
        if self.position.is_long and macd_cross_down:
            self.position.close()
            print(
                f"🌙 Moon Dev: Long exit on bearish MACD crossover at {self.data.index[-1]} ✨"
            )
        elif self.position.is_short and macd_cross_up:
            self.position.close()
            print(
                f"🌙 Moon Dev: Short exit on bullish MACD crossover at {self.data.index[-1]} ✨"
            )
        ...
        if not self.position:
            if (
                ...
                and current_rsi > 45
                and current_close > current_ema_fast
                and current_ema_fast > current_ema_slow
                and current_vol > current_avg_vol * 1.2
            ):  # 🌙 Optimization: Added RSI >45, EMA trend alignment, and volume >1.2x avg for high-quality long setups
                # Long entry
                ...
                )  # 🌙 Optimization: Simplified to direct units, allowing fractional sizes for precision

                self.buy(sl=sl_price, size=size)  # No TP, use trailing
                print(
                    f"🌙 Moon Dev: Long entry at {current_close}, SL: {sl_price}, Size: {size} 🚀"
                )
                ...
                and current_ema_fast < current_ema_slow
                and current_vol > current_avg_vol * 1.2
            ):  # 🌙 Optimization: Added RSI <55, EMA trend alignment, and volume >1.2x avg for high-quality short setups
                # Short entry
                sl_price = current_close + self.atr_multiplier_sl * current_atr
                ...
                print(
                    f"🌙 Moon Dev: Short entry at {current_close}, SL: {sl_price}, Size: {size} 🚀"
                )

        # Debug print every 100 bars or so
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Backtest Progress: {self.data.index[-1]}, ADX: {current_adx:.2f}, MACD: {current_macd:.2f}, RSI: {current_rsi:.2f} ✨"
            )  # 🌙 Optimization: Added RSI to debug for monitoring


# Run backtest
bt = Backtest(
    data, VigorConvergence, cash=1000000, commission=0.001, exclusive_orders=True
)
stats = bt.run()
print(stats)
