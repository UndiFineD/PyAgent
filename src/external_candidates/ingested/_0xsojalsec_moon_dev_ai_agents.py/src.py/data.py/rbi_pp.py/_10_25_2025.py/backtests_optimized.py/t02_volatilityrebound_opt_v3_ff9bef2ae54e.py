# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v3.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                tp_price = (
                    entry_price + 2 * sl_distance
                )  # 1:2 RR for improved expectancy
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {atr_val:.2f}, ADX {self.adx[-1]:.2f}, Vol Filter OK ✨"
                    )
            ...
            # Short entry
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {atr_val:.2f}, ADX {self.adx[-1]:.2f}, Vol Filter OK ✨"
                    )
