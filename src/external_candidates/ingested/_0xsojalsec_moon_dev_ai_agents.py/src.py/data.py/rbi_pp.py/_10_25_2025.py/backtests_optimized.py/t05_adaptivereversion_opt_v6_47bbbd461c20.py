# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v6.py
...
class AdaptiveReversion(Strategy):
    ...
    rsi_period = 14
    rsi_overbought = (
        75  # 🌙 Moon Dev: Tightened RSI thresholds for higher quality reversion signals
    )
    rsi_oversold = 25
    sma_period = 200
    atr_period = 14
    ...
    risk_percent = 0.01
    max_bars_in_trade = (
        15  # 🌙 Moon Dev: Extended time limit to allow more room for mean reversion
    )
    ...
    vol_period = 20
    vol_mult = (
        1.2  # 🌙 Moon Dev: Volume filter to confirm setups with above-average volume
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol Ratio: {vol/avg_vol:.2f} 🚀"
                )
            ...
            # Short entry
                ...
                self.entry_time = current_time  # 🌙 Moon Dev: Store entry time on self for reliable calculation
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol Ratio: {vol/avg_vol:.2f} ✨"
                )
