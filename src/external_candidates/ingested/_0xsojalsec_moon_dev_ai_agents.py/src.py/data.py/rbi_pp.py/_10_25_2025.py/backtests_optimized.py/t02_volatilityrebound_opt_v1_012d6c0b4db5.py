# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityRebound(Strategy):
    ...
    def next(self):
        if (
            len(self.data) < 200
        ):  # 🌙 OPTIMIZATION: Increased buffer to 200 for EMA initialization
            return
        ...
        if self.position:
            if self.position.is_long and self.data.Close[-1] >= self.bb_middle[-1]:
                self.position.close()
                print(
                    f"🌙 LONG EXIT at {self.data.Close[-1]:.2f} ✨ TP Middle: {self.bb_middle[-1]:.2f} 🚀"
                )
            elif self.position.is_short and self.data.Close[-1] <= self.bb_middle[-1]:
                self.position.close()
                print(
                    f"🌙 SHORT EXIT at {self.data.Close[-1]:.2f} ✨ TP Middle: {self.bb_middle[-1]:.2f} 📉"
                )
        ...
        # Entry logic - no position
            ...
            if (
                ...
                and self.rsi[-1] < 25
                and self.data.Close[-1] > self.ema200[-1]  # Trend filter
                ...
                sl_distance = (
                    2 * atr_value
                )  # 🌙 OPTIMIZATION: ATR-based SL distance (2x ATR for reasonable risk)
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {atr_value:.2f}, Vol Conf {vol_confirmation} ✨"
                    )
            ...
            # Short entry - tightened RSI to >75 for stronger overbought, added filters
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {atr_value:.2f}, Vol Conf {vol_confirmation} ✨"
                    )
