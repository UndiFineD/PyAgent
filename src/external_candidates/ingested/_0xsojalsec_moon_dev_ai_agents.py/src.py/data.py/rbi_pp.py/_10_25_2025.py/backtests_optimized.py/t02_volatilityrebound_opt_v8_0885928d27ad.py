# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v8.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        # Entry logic - no position
            ...
            if (
                ...
                and self.rsi[-1] < 25
                and self.adx[-1] < 25
                and current_volume > self.vol_sma[-1]
            ):
                ...
                if sl_distance > 0:
                    ...
                    )  # 🌙 Allow float size for precise risking (no int rounding)
                    self.buy(size=size, sl=sl_price)
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {self.atr[-1]:.2f}, ADX {self.adx[-1]:.2f}, Vol > SMA ✨"
                    )
            ...
            # 🌙 OPTIMIZATION: Short entry - symmetric changes: RSI >75, ADX <25, volume filter
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {self.atr[-1]:.2f}, ADX {self.adx[-1]:.2f}, Vol > SMA ✨"
                    )
