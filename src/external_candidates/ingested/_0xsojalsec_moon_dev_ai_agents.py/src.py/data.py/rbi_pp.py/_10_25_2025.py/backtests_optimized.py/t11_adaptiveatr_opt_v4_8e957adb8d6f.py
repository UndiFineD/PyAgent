# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v4.py
import numpy as np
import pandas as pd
import talib
            ...
            if long_cross and atr_ok and vol_ok and adx_ok:
                ...
                if rsi_ok and trend_ok:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, Vol OK: {vol_ok}, RSI: {self.rsi[-1]:.1f}, ADX: {self.adx[-1]:.1f} ✨"
                        )
                    else:
                        print("🌙❌ Long signal but size=0, skipped.")
                else:
                    print(
                        f"🌙⚠️ Long crossover but momentum/trend failed: RSI_OK={rsi_ok}, Trend_OK={trend_ok}"
                    )
                ...
                if rsi_ok and trend_ok:
                    ...
                    if size > 0:
                        ...
                        self.entry_step = len(self.data) - 1
                        print(
                            f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, Vol OK: {vol_ok}, RSI: {self.rsi[-1]:.1f}, ADX: {self.adx[-1]:.1f} ✨"
                        )
                    else:
                        print("🌙❌ Short signal but size=0, skipped.")
                else:
                    print(
                        f"🌙⚠️ Short crossover but momentum/trend failed: RSI_OK={rsi_ok}, Trend_OK={trend_ok}"
                    )
            elif long_cross or short_cross:
                print(
                    f"🌙⚠️ Crossover detected but filters failed: ATR_OK={atr_ok}, Vol_OK={vol_ok}, ADX_OK={adx_ok}"
                )
            else:
                pass  # No signal
