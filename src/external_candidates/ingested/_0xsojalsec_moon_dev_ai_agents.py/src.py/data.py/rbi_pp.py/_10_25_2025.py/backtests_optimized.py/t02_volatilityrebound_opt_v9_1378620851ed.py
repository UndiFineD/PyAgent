# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityRebound(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            atr_val = self.atr[-1]
            vol_condition = (
                self.data.Volume[-1] > self.vol_sma[-1] * 1.2
            )  # 🌙 Volume filter for higher quality entries
            ...
            if (
                ...
                and entry_price > self.sma200[-1]  # 🌙 Trend filter: uptrend only
                and vol_condition
            ):
                ...
                tp_price = entry_price + (
                    2 * sl_distance
                )  # 🌙 1:2 RR for better returns
                risk_amount = self.equity * 0.01  # 1% risk
                size = risk_amount / sl_distance  # 🌙 Float size for precision
                if size > 0:
                    self.buy(size=size, sl=sl_price, tp=tp_price)
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {atr_val:.2f} ✨"
                    )
            ...
            # Short entry - symmetric optimizations
                ...
                and entry_price < self.sma200[-1]  # 🌙 Trend filter: downtrend only
                and vol_condition
            ):
                ...
                if size > 0:
                    ...
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {atr_val:.2f} ✨"
                    )
