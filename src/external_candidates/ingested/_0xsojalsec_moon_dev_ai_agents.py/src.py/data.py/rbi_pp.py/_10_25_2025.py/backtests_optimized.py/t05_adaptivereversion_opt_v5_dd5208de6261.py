# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v5.py
...
class AdaptiveReversion(Strategy):
    ...
    max_bars_in_trade = (
        20  # 🌙 Moon Dev: Extended time limit to let winners breathe longer
    )
    ...
    def next(self):
        ...
        if self.position:
            bars_in_trade = (
                (current_time - self.position.entry_time).total_seconds() / (15 * 60)
            ...
            if (is_long and rsi > 60) or (not is_long and rsi < 40):
                self.position.close()
                print(
                    f"🌙 Moon Dev: RSI Reversal Exit {'Long' if is_long else 'Short'} at {close} 🚀 RSI: {rsi:.2f}"
                )
                return
        ...
        if not self.position:
            ...
            if (
                ...
                price_risk = (
                    sl_dist / close
                )  # 🌙 Moon Dev: Calculate % price risk for proper fractional sizing
                size = (
                    self.risk_percent / price_risk
                )  # 🌙 Moon Dev: Fraction of equity to risk exact % - ensures consistent risk regardless of price level
                tp_dist = (
                    2 * sl_dist
                )  # 🌙 Moon Dev: Dynamic 2:1 RR TP instead of fixed middle for better reward potential
                tp = close + tp_dist

                self.buy(size=size, sl=sl, tp=tp)
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol Confirm: {vol_confirm} 🚀"
                )
            ...
            # Short entry
                ...
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol Confirm: {vol_confirm} ✨"
                )
