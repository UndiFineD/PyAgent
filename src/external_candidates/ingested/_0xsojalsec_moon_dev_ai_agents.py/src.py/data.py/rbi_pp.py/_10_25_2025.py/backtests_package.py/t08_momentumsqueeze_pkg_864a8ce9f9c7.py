# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_package\T08_MomentumSqueeze_PKG.py
import numpy as np
import pandas as pd
import talib
...
class MomentumSqueeze(Strategy):
    def init(self):
        ...
        self.initial_risk_long = 0
        self.initial_risk_short = 0
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if favorable_move >= self.initial_risk_long:
                ...
                if favorable_move >= self.initial_risk_short:
                    new_sl = min(self.position.sl, self.bb_middle[-1])
                    if new_sl < self.position.sl:
                        self.position.sl = new_sl
