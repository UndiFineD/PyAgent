# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_package\T10_SwiftStrength_PKG.py
import numpy as np
import pandas as pd
import talib
...
class SwiftStrength(Strategy):
    ...
    def init(self):
        ...
        self.macd, self.macdsignal, self.macdhist = self.I(
            talib.MACD, close, fastperiod=8, slowperiod=17, signalperiod=9
        )
    ...
    def next(self):
        ...
        if self.position.is_long and (
            self.macdsignal[-2] < self.macd[-2] and self.macdsignal[-1] > self.macd[-1]
        ):
            self.position.close()
            print(f"🌙 Moon Dev: Exit long on MACD bearish reversal 🔄")
            return
        if self.position.is_short and (
            self.macd[-2] < self.macdsignal[-2] and self.macd[-1] > self.macdsignal[-1]
        ):
            self.position.close()
        ...
        if len(self.trades) == 0 and not self.position:
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Long entry at {current_price:.2f}, size {position_size}, SL {sl:.2f}, TP {tp:.2f} 🚀"
                )
            ...
            # Short entry
                ...
                print(
                    f"🌙 Moon Dev: Short entry at {current_price:.2f}, size {position_size}, SL {sl:.2f}, TP {tp:.2f} 📉"
                )
