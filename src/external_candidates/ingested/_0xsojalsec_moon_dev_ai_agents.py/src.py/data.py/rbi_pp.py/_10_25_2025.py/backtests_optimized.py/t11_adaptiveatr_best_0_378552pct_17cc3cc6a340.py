# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_BEST_0.378552pct.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def next(self):
        ...
        long_cross = (
            self.sma50[-2] < self.sma200[-2] and self.sma50[-1] > self.sma200[-1]
        )
        short_cross = (
            self.sma50[-2] > self.sma200[-2] and self.sma50[-1] < self.sma200[-1]
        )
