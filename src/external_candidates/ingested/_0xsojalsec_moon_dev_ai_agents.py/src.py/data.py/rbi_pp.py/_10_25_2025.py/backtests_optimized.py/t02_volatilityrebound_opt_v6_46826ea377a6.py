# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v6.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityRebound(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            risk_amount = (
                self.equity * 0.015
            )  # 🌙 Increased to 1.5% risk for higher returns
            ...
            if (
                ...
                > self.ema200[-1]  # 🌙 Trend filter: bullish regime
                ...
                )  # 🌙 1:2 RR for improved profitability
                size = (
                    risk_amount / sl_distance
                )  # 🌙 Float size for precision, no rounding to int
                if size > 0:
            ...
            # Short entry with tightened conditions and filters
                ...
                and self.data.Close[-1]
                < self.ema200[-1]  # 🌙 Trend filter: bearish regime
                and self.data.Volume[-1] > self.vol_sma[-1]
            ):  # 🌙 Volume confirmation
