# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v9.py
...
class AdaptiveReversion(Strategy):
    ...
    ema_period = (
        200  # 🌙 Moon Dev: Changed from SMA 200 to EMA for faster trend response
    )
    atr_period = 14
    atr_mult = (
        2.0  # 🌙 Moon Dev: Increased from 1.5 to 2.0 for wider stops, reducing whipsaws
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Switched to EMA for improved trend detection
        self.atr = self.I(talib.ATR, high, low, close, timeperiod=self.atr_period)
        self.vol_ma = self.I(
            talib.SMA, volume, timeperiod=self.vol_ma_period
        )  # 🌙 Moon Dev: Added volume MA filter for entry quality
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (is_long and rsi >= self.rsi_overbought) or (
                not is_long and rsi <= self.rsi_oversold
            ):
                self.position.close()
                print(
                    f"🌙 Moon Dev: RSI Extreme Exit {'Long' if is_long else 'Short'} at {close} 🚀 RSI: {rsi:.2f}"
                )
                return
        ...
        if not self.position and vol > vol_ma * self.vol_mult:
            ...
            if close <= lower and rsi < self.rsi_oversold and close > ema200:
                ...
                size = max(
                    0.01, min(100.0, size)
                )  # 🌙 Moon Dev: Allow fractional sizes (0.01 min, 100 max BTC cap for realism), no int rounding to enable precise sizing
                ...
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol Mult: {vol/vol_ma:.2f} 🚀"
                )
            ...
            # Short entry with tightened conditions and dynamic TP
                ...
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol Mult: {vol/vol_ma:.2f} ✨"
                )
