# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T01_ClimaxReversal_DEBUG_v3.py
import numpy as np
import pandas as pd
import talib
...
class ClimaxReversal(Strategy):
    ...
    def get_consec_down(self):
        ...
        while (
            prev_idx >= 0
            and self.data.Close.iloc[check_idx] < self.data.Close.iloc[prev_idx]
        ):
            count += 1
            check_idx = prev_idx
            prev_idx -= 1
    ...
    def next(self):
        max_period = max(
            self.sma_period, self.atr_period, self.vol_ma_period, self.stoch_k
        )
        ...
        if (
            ...
            and (i - self.entry_bar) > self.max_hold_bars
        ):
            self.position.close()
        ...
        prev_to_gap_close = self.data.Close.iloc[-3]
        gap_open = self.data.Open.iloc[-2]
        gap_close = self.data.Close.iloc[-2]
        gap_high = self.data.High.iloc[-2]
        gap_low = self.data.Low.iloc[-2]
        gap_volume = self.data.Volume.iloc[-2]
        curr_open = self.data.Open.iloc[-1]
        curr_close = self.data.Close.iloc[-1]
        curr_volume = self.data.Volume.iloc[-1]
        ...
        if (
            is_gap_down
            and gap_size_pct > self.gap_threshold
            and self.get_consec_down() >= self.consec_bars
            and gap_close < self.sma[-2]  # Downtrend
            and self.rsi[-2] < 30  # Oversold at gap
            and curr_close > gap_open  # Confirmation close above gap open
            and curr_close > curr_open  # Bullish candle
            and vol_high_gap
            and vol_low_confirm
            and self.stoch_k[-1] > self.stoch_d[-1]
            and self.stoch_k[-2] <= self.stoch_d[-2]
        ):  # Stoch crossover up
            ...
            if risk_dist > 0:
                ...
                if size > 0:
                    ...
                    self.entry_bar = i
                    print(
                        f"🌙 Moon Dev: LONG Entry at {entry_price:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size} 🚀💹"
                    )
        ...
        # Short Entry: Bearish Reversal in Uptrend
            ...
            is_gap_up
            and gap_size_pct > self.gap_threshold
            and self.get_consec_up() >= self.consec_bars
            and gap_close > self.sma[-2]  # Uptrend
            and self.rsi[-2] > 70  # Overbought at gap
            and curr_close < gap_open  # Confirmation close below gap open
            and curr_close < curr_open  # Bearish candle
            and vol_high_gap
            and vol_low_confirm
            and self.stoch_k[-1] < self.stoch_d[-1]
            and self.stoch_k[-2] >= self.stoch_d[-2]
        ):  # Stoch crossover down
            ...
            if risk_dist > 0:
                ...
                if size > 0:
                    ...
                    self.entry_bar = i
                    print(
                        f"🌙 Moon Dev: SHORT Entry at {entry_price:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size} 📉🔻"
                    )


bt = Backtest(
    data, ClimaxReversal, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
