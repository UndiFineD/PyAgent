# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T13_VolatilityReversion_OPT_v2.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityReversion(Strategy):
    ...
    rsi_high = (
        75  # Optimized: Tightened RSI high threshold for stronger overbought signals
    )
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev: Indicators initialized for optimized VolatilityReversion strategy (ATR, Volume, EMA added) ✨"
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if stop_distance > 0:
                ...
                )  # Optimized: Float size for precise fractional BTC positioning
                ...
                if (
                    ...
                    and current_volume > self.vol_sma[-1]  # New: Volume confirmation
                    and current_close > self.ema200[-1]
                ):  # New: Uptrend filter

                    entry_price = current_close
                    stop_price = entry_price - stop_distance
                    tp_price = (
                        entry_price + self.rr * stop_distance
                    )  # New: Fixed RR take profit for better returns

                    if size > 0:
                        self.buy(size=size, sl=stop_price, tp=tp_price)
                        print(
                            f"🚀 Moon Dev: Long entry at {entry_price:.2f}, size {size:.6f} BTC, stop {stop_price:.2f}, tp {tp_price:.2f}, risk {risk_amount:.2f} USD 🌙"
                        )
                ...
                # Short entry - Optimized: Added volume and EMA filters for higher quality setups
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🚀 Moon Dev: Short entry at {entry_price:.2f}, size {size:.6f} BTC, stop {stop_price:.2f}, tp {tp_price:.2f}, risk {risk_amount:.2f} USD 🌙"
                        )
...
bt = Backtest(
    df, VolatilityReversion, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
