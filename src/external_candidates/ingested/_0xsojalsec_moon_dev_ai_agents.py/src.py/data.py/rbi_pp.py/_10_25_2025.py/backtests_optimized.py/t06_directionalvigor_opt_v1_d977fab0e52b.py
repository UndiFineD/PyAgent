# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v1.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev: Indicators initialized for optimized DirectionalVigor strategy (ADX, DI, ATR, EMA200, RSI, VolMA) ✨"
        )
    ...
    def next(self):
        ...
        if self.position.is_long and short_cross:
            self.position.close()
            print(
                f"🌙 Moon Dev: Exiting LONG on reversal at {current_close:.2f} | ADX: {current_adx:.2f} 📉💫"
            )
        if self.position.is_short and long_cross:
            self.position.close()
            print(
                f"🌙 Moon Dev: Exiting SHORT on reversal at {current_close:.2f} | ADX: {current_adx:.2f} 📉💫"
            )
        ...
        if self.position and current_adx < self.adx_weak:
            if self.position.is_long:
                print(
                    f"🌙 Moon Dev: Exiting LONG on weakening trend at {current_close:.2f} | ADX: {current_adx:.2f} 📉💫"
                )
            else:
                print(
                    f"🌙 Moon Dev: Exiting SHORT on weakening trend at {current_close:.2f} | ADX: {current_adx:.2f} 📉💫"
                )
            self.position.close()
        ...
        if entry_long and not self.position:
            ...
            tp_dist = (
                self.tp_rr * sl_dist
            )  # Dynamic TP based on RR for improved returns
            ...
            print(
                f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {fraction:.4f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
            )
        ...
        if entry_short and not self.position:
            ...
            print(
                f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {fraction:.4f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
            )
