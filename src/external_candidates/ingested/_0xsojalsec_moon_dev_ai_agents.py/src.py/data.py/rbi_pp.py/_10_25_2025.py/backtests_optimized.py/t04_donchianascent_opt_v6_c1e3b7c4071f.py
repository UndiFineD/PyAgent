# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v6.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            )  # Tighter multiplier for improved equilibrium
            ...
            )  # 🌙 New: Only enter in ascending Donchian channels to align with uptrend momentum
            ...
            if (
                ...
                if risk_per_share > 0:
                    ...
                    )  # Higher TP for amplified RR in optimized setups

                    self.buy(size=position_size, sl=sl_price, tp=tp_price)
                    self.entry_bar = len(self.data)
                    self.initial_sl = sl_price
                    print(
                        f"🌙 Moon Dev Long Entry: Optimized Breakout at {current_close}, size={position_size}, SL={sl_price}, TP={tp_price} (RSI={current_rsi:.1f}, ADX={current_adx:.1f}, Trend=Up, Ascending=Yes) ✨🚀"
                    )
