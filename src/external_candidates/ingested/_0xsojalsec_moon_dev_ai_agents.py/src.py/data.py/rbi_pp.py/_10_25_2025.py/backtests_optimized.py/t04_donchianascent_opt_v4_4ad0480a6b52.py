...
class DonchianAscent(Strategy):
    ...
    tp_multiplier = (
        3  # 🌙 Improved reward:risk ratio to 1:3 for better profitability on winners
    )
    ...
    atr_period = 14
    atr_sl_mult = (
        1.5  # 🌙 ATR-based SL multiplier for dynamic, volatility-adjusted stop losses
    )
    vol_multiplier = (
        1.2  # 🌙 Added volume confirmation multiplier to require stronger volume surge
    )
    rsi_period = 14
    rsi_lower = 50
        ...
        if self.position:
            ...
            if (
                ...
                if risk_per_share > 0:
                    ...
                    )  # Higher RR for improved returns
