# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_working\T04_DonchianAscent_OPT_v10_3.0pct.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    def init(self):
        ...
        )  # Renamed for new shorter period
        ...
        print(
            "🌙 Moon Dev Backtest Initialized: Optimized DonchianAscent with Regime Filter & Tighter Quality Controls for 50% Target! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            trail_candidate = current_high - (
                2.0 * current_atr
            )  # Loosened to 2x ATR for longer runs in volatile BTC uptrends
            ...
            if should_exit:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: {exit_reason} at {current_close} after {bars_in_trade} bars 🚀"
                )
                self.entry_bar = None
                self.initial_sl = None
                self.trailing_sl = None
            ...
            )  # New: Only in bull regime to avoid downtrend traps

            if (
                breakout
                and vol_confirm
                and not channel_too_narrow
                and not pct_too_narrow
                and not extended
                and trend_filter
                and momentum_filter
                and adx_filter
                and ascending_channel
                and regime_filter
            ):
                sl_price = (
                    current_lower  # Channel-based SL with enhanced trailing protection
                )
                ...
                if risk_per_share > 0:
                    ...
                    adjusted_risk = self.risk_percent * (
                        1 / (1 + atr_factor)
                    )  # Reduce size in high vol to manage risk
                    position_size = int(
                        round((self.equity * adjusted_risk) / risk_per_share)
                    )
                    tp_price = current_close + (
                        self.tp_multiplier * risk_per_share
                    )  # Balanced TP for achievable targets
                    ...
                    print(
                        f"🌙 Moon Dev Long Entry: Quality Breakout at {current_close}, size={position_size}, SL={sl_price}, TP={tp_price} (RSI={current_rsi:.1f}, ADX={current_adx:.1f}, Trend=Up, Regime=Bull, Ascending=Yes, Width={pct_width:.3f}) ✨🚀"
                    )
