# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v3.py
...
class AdaptiveReversion(Strategy):
    ...
    sma_period = (
        100  # 🌙 Moon Dev: Reduced from 200 to 100 for more responsive trend filter
    )
    atr_period = 14
    ...
    max_bars_in_trade = (
        15  # 🌙 Moon Dev: Extended to 15 bars to allow more time for reversion
    )
    ...
    max_position_fraction = (
        0.1  # 🌙 Moon Dev: Cap position size to 10% of equity for risk control
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol Ratio: {vol/vol_sma:.2f} 🚀"
                )
            ...
            # Short entry - 🌙 Moon Dev: Same filters applied to shorts for consistency
                ...
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f}, Vol Ratio: {vol/vol_sma:.2f} ✨"
                )
