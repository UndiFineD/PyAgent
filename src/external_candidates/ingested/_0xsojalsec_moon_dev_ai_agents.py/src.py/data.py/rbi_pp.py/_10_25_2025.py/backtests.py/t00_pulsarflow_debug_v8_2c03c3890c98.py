# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T00_PulsarFlow_DEBUG_v8.py
import numpy as np
import pandas as pd
import talib
...
class PulsarFlow(Strategy):
    ...
    def get_bullish_div_info(self, current_i):
        ...
        recent_low_price = recent_lows.iloc[idx_rel]
        ...
        prev_low_price = prev_lows.iloc[idx_rel_prev]
    ...
    def get_bearish_div_info(self, current_i):
        ...
        recent_high_price = recent_highs.iloc[idx_rel]
        ...
        prev_high_price = prev_highs.iloc[idx_rel_prev]
    ...
    def next(self):
        ...
        print(
            f"🌙 Moon Dev: Bar update - Close: {close:.2f}, MFI: {self.mfi[current_i]:.2f}, ADX: {self.adx[current_i]:.2f} 📊"
        )
        ...
        if not self.position:
            ...
            if self.adx[current_i] < 20:
                print(
                    f"🌙 Moon Dev: ADX too low ({self.adx[current_i]:.2f}), choppy market, skipping 🚫"
                )
                return
            ...
            if close > self.ema[current_i] and self.mfi[current_i] <= self.oversold:
                print(
                    f"🌙 Moon Dev: Potential LONG: MFI {self.mfi[current_i]:.2f} <= {self.oversold}, Close {close:.2f} > EMA {self.ema[current_i]:.2f} 🔍"
                )
            ...
            if close < self.ema[current_i] and self.mfi[current_i] >= self.overbought:
                print(
                    f"🌙 Moon Dev: Potential SHORT: MFI {self.mfi[current_i]:.2f} >= {self.overbought}, Close {close:.2f} < EMA {self.ema[current_i]:.2f} 🔍"
                )
