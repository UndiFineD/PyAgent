# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙✨ AdaptiveAtr indicators initialized with RSI & ADX for enhanced optimization! 🚀"
        )
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙📊 Bar {len(self.data)}: Close={current_price:.2f}, SMA20={self.sma20[-1]:.2f}, SMA50={self.sma50[-1]:.2f}, ATR={atr:.2f}, RSI={rsi_val:.2f}, ADX={adx_val:.2f}"
            )
        ...
        short_cross = (
            self.sma20[-2] > self.sma50[-2] and self.sma20[-1] < self.sma50[-1]
        )
        ...
        if not self.position:
            ...
            if long_cross and atr_ok and vol_ok and rsi_long_ok and adx_ok:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size:.4f}, ATR: {atr:.2f}, RSI: {rsi_val:.2f}, ADX: {adx_val:.2f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                ...
                if size > 0:
                    ...
                    self.entry_step = len(self.data) - 1
                    print(
                        f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size:.4f}, ATR: {atr:.2f}, RSI: {rsi_val:.2f}, ADX: {adx_val:.2f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                    print("🌙❌ Short signal but size=0, skipped.")
            elif long_cross or short_cross:
                print(
                    f"🌙⚠️ Crossover detected but filters failed: ATR_OK={atr_ok}, Vol_OK={vol_ok}, RSI_L={rsi_long_ok}, RSI_S={rsi_short_ok}, ADX_OK={adx_ok}"
                )
            else:
                pass  # No signal
