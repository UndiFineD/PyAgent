# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v2.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    def next(self):
        ...
        prev_close = self.data.Close[
            prev_idx
        ]  # 🌙 Added: Use previous close for consistent channel width calculation
        ...
        if self.position:
            ...
            if bars_in_trade > self.time_exit_bars:
                ...
                self.rsi[-1] > 75
            ):  # 🌙 New: RSI overbought exit to lock in profits before reversal
                should_exit = True
            ...
            vol_confirm = (
                current_vol > 1.2 * prev_avg_vol
            )  # 🌙 Optimized: Tightened volume filter from 1x to 1.2x for stronger confirmation
            channel_too_narrow = (
                prev_width / prev_close
            ) < self.min_channel_width_pct  # 🌙 Optimized: Use prev_close for accurate width percentage
            extended = (current_close - prev_middle) > (
                self.extended_multiplier * prev_width
            )
            trend_up = (
                current_close > self.sma200[-1]
            )  # 🌙 New: Trend filter - only enter if above 200 SMA for favorable market regime
            momentum_ok = (
                self.rsi[-1] > 50
            )  # 🌙 New: RSI >50 for bullish momentum, avoiding weak breakouts
            ...
            if (
                ...
                if risk_per_share > 0:
                    ...
                    print(
                        f"🌙 Moon Dev Long Entry: Breakout at {current_close}, size={position_size}, SL={sl_price}, TP={tp_price} ✨🚀"
                    )
