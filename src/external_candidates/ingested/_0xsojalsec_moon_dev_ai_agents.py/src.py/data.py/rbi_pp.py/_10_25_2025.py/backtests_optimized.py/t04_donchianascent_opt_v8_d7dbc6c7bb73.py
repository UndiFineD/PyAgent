# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v8.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    def init(self):
        ...
        )  # New long-term filter for regime awareness
        ...
        self.trail_sl = (
            None  # 🌙 New: Track trailing stop level for dynamic risk management
        )
        print(
            "🌙 Moon Dev Backtest Initialized: Super-Optimized DonchianAscent Strategy Loaded with Trailing Stops & Dual Trends! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if bars_in_trade > 1 and self.trail_sl is not None:
                ...
                self.position.sl = (
                    self.trail_sl
                )  # Dynamically update SL in backtesting.py
            ...
            if should_exit:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: {exit_reason} at {current_close} after {bars_in_trade} bars (Trail SL={self.trail_sl}) 🚀"
                )
                self.entry_bar = None
                self.initial_sl = None
                self.trail_sl = None
            ...
            )  # Balanced volume for more entries
            ...
            long_trend_filter = (
                current_close > current_sma_long
            )  # New: Ensures overall uptrend, avoiding bearish regimes
            ...
            if (
                ...
                and not extended
                and short_trend_filter
                and long_trend_filter
                and momentum_filter
                and adx_filter
                ...
                if risk_per_share > 0:
                    ...
                    position_size = (
                        self.equity * self.risk_percent
                    ) / risk_per_share  # Use fraction for precision in crypto
                    tp_price = current_close + (
                        self.tp_multiplier * risk_per_share
                    )  # Amplified TP for 4:1 RR
                    ...
                    print(
                        f"🌙 Moon Dev Long Entry: Super Breakout at {current_close}, size={position_size:.4f}, SL={sl_price}, TP={tp_price} (RSI={current_rsi:.1f}, ADX={current_adx:.1f}, ShortTrend=Up, LongTrend=Up, Ascending=Yes) ✨🚀"
                    )
