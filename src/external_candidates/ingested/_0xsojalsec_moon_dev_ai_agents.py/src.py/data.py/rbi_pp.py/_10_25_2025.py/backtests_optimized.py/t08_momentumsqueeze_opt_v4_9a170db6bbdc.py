# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T08_MomentumSqueeze_OPT_v4.py
import numpy as np
import pandas as pd
import talib
...
class MomentumSqueeze(Strategy):
    ...
    def next(self):
        if (
            len(self.data) < 300
        ):  # 🌙 Moon Dev: Increased to 300 to accommodate EMA200 initialization safely
            return
        ...
        trend_down = (
            self.ema20[-1] < self.ema50[-1] and self.ema20[-1] < self.ema200[-1]
        )
