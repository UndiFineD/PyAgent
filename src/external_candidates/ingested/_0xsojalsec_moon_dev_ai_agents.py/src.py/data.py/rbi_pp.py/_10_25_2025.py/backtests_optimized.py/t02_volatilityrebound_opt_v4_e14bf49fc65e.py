    ...
    def next(self):
            ...
            if self.position_is_long is True:
                ...
                if sl_hit or tp_hit:
                    ...
                    exit_price = self.data.Close[
                        -1
                    ]  # Approximate exit at close; in live, would be at SL/TP level
                    if tp_hit:
                        print(
                            f"🌙 LONG TP EXIT at {exit_price:.2f} ✨ TP: {self.tp:.2f} 🚀"
                        )
                    else:
                        print(
                            f"🌙 LONG SL EXIT at {exit_price:.2f} ✨ SL: {self.sl:.2f} 🚨"
                        )
                    self.position_is_long = None
                    exit_triggered = True
                ...
                if sl_hit or tp_hit:
                    ...
                    if tp_hit:
                        print(
                            f"🌙 SHORT TP EXIT at {exit_price:.2f} ✨ TP: {self.tp:.2f} 📉"
                        )
                    else:
                        print(
                            f"🌙 SHORT SL EXIT at {exit_price:.2f} ✨ SL: {self.sl:.2f} 🚨"
                        )
                    self.position_is_long = None
                    exit_triggered = True
        ...
        if not self.position:
            ...
            if (
                ...
                atr_val = self.atr[-1]
                self.sl = (
                    entry_price - 1.5 * atr_val
                )  # 🌙 ATR-based SL for better volatility adjustment
                sl_distance = entry_price - self.sl
                self.tp = (
                    entry_price + 2 * sl_distance
                )  # 🌙 1:2 RR for improved profit capture
                risk_amount = self.equity * risk_per_trade
                size = (
                    risk_amount / sl_distance
                )  # 🌙 Allow fractional size for precise risk (no int rounding)
