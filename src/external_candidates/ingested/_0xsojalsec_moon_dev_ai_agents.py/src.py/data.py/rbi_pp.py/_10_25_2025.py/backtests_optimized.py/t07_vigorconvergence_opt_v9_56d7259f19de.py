# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v9.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                and current_close
                > current_ema20  # 🌙 Moon Dev: Trend filter - only long above EMA20
                and current_rsi > 50  # 🌙 Moon Dev: RSI filter for bullish momentum
                and current_vol > self.sma_vol[-1]
            ):  # 🌙 Moon Dev: Volume filter for confirmation
                # Long entry
                ...
                )  # 🌙 Moon Dev: Simplified and allow fractional sizes for precise risk control (no int rounding)
                self.buy(sl=sl_price, tp=tp_price, size=size)
                print(
                    f"🌙 Moon Dev: Long entry at {current_close}, SL: {sl_price}, TP: {tp_price}, Size: {size} 🚀"
                )
                ...
                and current_close
                < current_ema20  # 🌙 Moon Dev: Trend filter - only short below EMA20
                and current_rsi < 50  # 🌙 Moon Dev: RSI filter for bearish momentum
                and current_vol > self.sma_vol[-1]
            ):  # 🌙 Moon Dev: Volume filter for confirmation
                # Short entry
                sl_price = current_close + self.atr_multiplier_sl * current_atr
                ...
                size = (
                    risk_amount / risk_dist
                )  # 🌙 Moon Dev: Simplified and allow fractional sizes for precise risk control
                self.sell(sl=sl_price, tp=tp_price, size=size)
                print(
                    f"🌙 Moon Dev: Short entry at {current_close}, SL: {sl_price}, TP: {tp_price}, Size: {size} 🚀"
                )

        # Debug print every 100 bars or so
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Backtest Progress: {self.data.index[-1]}, ADX: {current_adx:.2f}, MACD: {current_macd:.2f} ✨"
            )
