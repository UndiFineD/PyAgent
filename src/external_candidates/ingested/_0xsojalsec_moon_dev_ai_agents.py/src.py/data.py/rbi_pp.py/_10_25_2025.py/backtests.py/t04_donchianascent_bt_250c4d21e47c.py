# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T04_DonchianAscent_BT.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            channel_too_narrow = (
                current_width / current_close
            ) < self.min_channel_width_pct
            extended = (current_close - current_middle) > (
                self.extended_multiplier * current_width
            )
