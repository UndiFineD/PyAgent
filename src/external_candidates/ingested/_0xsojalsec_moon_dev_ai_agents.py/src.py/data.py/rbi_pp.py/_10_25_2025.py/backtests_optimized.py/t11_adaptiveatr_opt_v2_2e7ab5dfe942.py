# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v2.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if pos.is_long:
                ...
                trail_sl = (
                    current_price - 1 * atr
                )  # Tighter trailing for better lock-in
                sl = max(sl, trail_sl)
        ...
        if not self.position:
            ...
            if long_cross and atr_ok and vol_ok and rsi_long_ok and trend_long_ok:
                ...
                print(
                    f"🌙⚠️ Crossover detected but filters failed: ATR_OK={atr_ok}, Vol_OK={vol_ok}, Trend OK (L/S)={trend_long_ok}/{trend_short_ok}, RSI={rsi:.2f}"
                )
            else:
                pass  # No signal
