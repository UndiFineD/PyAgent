# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v5.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityRebound(Strategy):
    ...
    def next(self):
        if (
            len(self.data) < 200
        ):  # 🌙 OPTIMIZATION: Wait for EMA200 to initialize properly
            return
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if not self.has_moved_to_be and current_price >= self.bb_middle[-1]:
                    ...
                    self.initial_sl = (
                        current_price  # Move remaining SL to breakeven (entry approx)
                    )
                    print(
                        f"🌙 LONG PARTIAL EXIT at {current_price:.2f} ✨ Scale-out Middle: {self.bb_middle[-1]:.2f}, Trailing to BE 🚀"
                    )
                ...
                if self.has_moved_to_be and self.initial_sl is not None:
                    self.position.sl = max(
                        self.position.sl or 0, self.entry_price
                    )  # Breakeven trail
                ...
                if current_price >= self.bb_upper[-1] or self.rsi[-1] > 75:
                    self.position.close()
                    print(
                        f"🌙 LONG FULL EXIT at {current_price:.2f} ✨ TP Upper/RSI: {self.bb_upper[-1]:.2f}/{self.rsi[-1]:.2f} 🚀"
                    )
                ...
                if not self.has_moved_to_be and current_price <= self.bb_middle[-1]:
                    self.position.close(
                        size=self.position.size * 0.5
                    )  # Close half (note: size positive for short)
                    self.has_moved_to_be = True
                    self.initial_sl = current_price  # Move remaining SL to breakeven
                    print(
                        f"🌙 SHORT PARTIAL EXIT at {current_price:.2f} ✨ Scale-out Middle: {self.bb_middle[-1]:.2f}, Trailing to BE 📉"
                    )

                # Trail remaining SL to breakeven or better
                if self.has_moved_to_be and self.initial_sl is not None:
                    self.position.sl = min(
                        self.position.sl or float("inf"), self.entry_price
                    )  # Breakeven trail for short

                # Full exit if hits opposite band or strong RSI
                if current_price <= self.bb_lower[-1] or self.rsi[-1] < 25:
                    self.position.close()
                    print(
                        f"🌙 SHORT FULL EXIT at {current_price:.2f} ✨ TP Lower/RSI: {self.bb_lower[-1]:.2f}/{self.rsi[-1]:.2f} 📉"
                    )
        ...
        # Entry logic - no position
            ...
            if (
                ...
                and self.rsi[-1] < 25
                and current_price > self.ema200[-1]  # Uptrend filter
                and vol_confirm
            ):  # Volume filter
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {current_price:.2f}, SL {sl_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {atr_value:.2f}, Vol Confirm ✨"
                    )
            ...
            # Short entry with optimizations
                ...
                and self.rsi[-1] > 75
                and current_price < self.ema200[-1]  # Downtrend filter
                and vol_confirm
            ):  # Volume filter
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {current_price:.2f}, SL {sl_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {atr_value:.2f}, Vol Confirm ✨"
                    )


if __name__ == "__main__":
    bt = Backtest(
        data, VolatilityRebound, cash=1000000, commission=0.001, exclusive_orders=True
    )
    stats = bt.run()
    print(stats)
