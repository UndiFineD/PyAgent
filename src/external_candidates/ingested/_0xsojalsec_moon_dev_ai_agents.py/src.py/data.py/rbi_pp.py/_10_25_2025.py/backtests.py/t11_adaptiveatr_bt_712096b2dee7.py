# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T11_AdaptiveAtr_BT.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙📊 Bar {len(self.data)}: Close={current_price:.2f}, SMA50={self.sma50[-1]:.2f}, SMA200={self.sma200[-1]:.2f}, ATR={atr:.2f}"
            )
        ...
        if not self.position:
            ...
            if long_cross and atr_ok and vol_ok:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                ...
                if size > 0:
                    ...
                    self.entry_price = current_price
                    print(
                        f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                    print("🌙❌ Short signal but size=0, skipped.")
            elif long_cross or short_cross:
                print(
                    f"🌙⚠️ Crossover detected but filters failed: ATR_OK={atr_ok}, Vol_OK={vol_ok}"
                )
            else:
                pass  # No signal
