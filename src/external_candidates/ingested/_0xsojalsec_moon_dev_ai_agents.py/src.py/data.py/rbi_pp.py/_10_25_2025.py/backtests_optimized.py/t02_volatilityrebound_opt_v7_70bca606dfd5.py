# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v7.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and self.rsi[-1]
                < 30  # Kept original RSI threshold; could tighten to 25 if needed
                and self.data.Close[-1]
                > self.ema200[-1]  # Trend filter: only long in uptrend
                and self.data.Volume[-1] > 1.2 * self.avg_vol[-1]
            ):  # Volume confirmation

                sl_price = (
                    entry_price - 1.5 * self.atr[-1]
                )  # ATR-based SL for better risk management
                tp_price = (
                    entry_price + 3.0 * self.atr[-1]
                )  # 2:1 RR TP for higher returns
                ...
                if sl_distance > 0:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, ATR {self.atr[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f} ✨"
                        )
            ...
            # Short entry with tightened filters
                ...
                and self.rsi[-1]
                > 70  # Kept original RSI threshold; could tighten to 75 if needed
                and self.data.Close[-1]
                < self.ema200[-1]  # Trend filter: only short in downtrend
                and self.data.Volume[-1] > 1.2 * self.avg_vol[-1]
            ):  # Volume confirmation
                ...
                if sl_distance > 0:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size}, RSI {self.rsi[-1]:.2f}, ATR {self.atr[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f} ✨"
                        )
