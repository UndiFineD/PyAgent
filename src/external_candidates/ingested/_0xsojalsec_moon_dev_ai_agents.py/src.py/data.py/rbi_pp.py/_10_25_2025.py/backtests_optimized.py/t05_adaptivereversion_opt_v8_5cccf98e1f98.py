# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v8.py
...
class AdaptiveReversion(Strategy):
    ...
    vol_mult = (
        1.1  # 🌙 Moon Dev: Require 10% above average volume for entry confirmation
    )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long:
                if close > entry_price:
                    ...
                    if new_sl > self.position.sl:  # Only update and print if improved
                        ...
                        print(
                            f"🌙 Moon Dev: Breakeven SL Adjustment Long from {old_sl:.2f} to {new_sl:.2f} at {close} 🚀"
                        )
            else:
                if close < entry_price:
                    ...
                    if new_sl < self.position.sl:  # Only update and print if improved
                        ...
                        self.position.sl = new_sl
                        print(
                            f"🌙 Moon Dev: Breakeven SL Adjustment Short from {old_sl:.2f} to {new_sl:.2f} at {close} ✨"
                        )
            ...
            if hasattr(self.position, "entry_time"):
                ...
                if bars_in_trade > self.max_bars_in_trade:
                    ...
                    self.position.close()
                    print(
                        f"🌙 Moon Dev: Time-Based Exit {'Long' if is_long else 'Short'} at {close} after {bars_in_trade:.1f} bars ✨"
                    )
                    return
        ...
        if not self.position:
            ...
            if (
                ...
                size = (
                    risk / sl_dist
                )  # 🌙 Moon Dev: Use float size for precise position sizing (allows fractions 0-1)
                tp = (
                    close + 2 * sl_dist
                )  # 🌙 Moon Dev: Set TP at 1:2 risk-reward for better profit potential
                entry_price = close
                self.buy(size=size, sl=sl, tp=tp)
                self.position.entry_price = entry_price
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol Mult: {vol/vol_avg:.2f} 🚀"
                )
            ...
            # Short entry
                ...
                size = (
                    risk / sl_dist
                )  # 🌙 Moon Dev: Use float size for precise position sizing
                tp = (
                    close - 2 * sl_dist
                )  # 🌙 Moon Dev: Set TP at 1:2 risk-reward for better profit potential
                entry_price = close
                ...
                self.position.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, TP: {tp:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, Vol Mult: {vol/vol_avg:.2f} ✨"
                )
