# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v2.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if exit_reason:
                ...
                print(
                    f"🌙 {pos_type} EXIT at {current_price:.2f} ✨ {exit_reason}: {exit_value:.2f} {emoji}"
                )
                # 🌙 Reset trackers
                self.entry_price = None
                self.sl_price = None
        ...
        # 🌙 Entry logic - no position
            ...
            if (
                ...
                and self.rsi[-1] < 25  # 🌙 Tightened from 30
                and self.data.Close[-1]
                > self.ema50[-1]  # 🌙 Trend filter: uptrend only
                and self.data.Volume[-1] > self.avg_vol[-1]
            ):  # 🌙 Volume confirmation
                ...
                if sl_distance > 0:
                    ...
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {self.sl_price:.2f}, TP {self.tp_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {atr_val:.2f}, EMA50 {self.ema50[-1]:.2f}, Vol {self.data.Volume[-1]:.0f} > {self.avg_vol[-1]:.0f} ✨"
                    )

            # Short entry with optimized filters
            elif (
                self.data.Close[-1] >= self.bb_upper[-1]
                and self.rsi[-1] > 75  # 🌙 Tightened from 70
                and self.data.Close[-1]
                < self.ema50[-1]  # 🌙 Trend filter: downtrend only
                and self.data.Volume[-1] > self.avg_vol[-1]
            ):  # 🌙 Volume confirmation
                ...
                if sl_distance > 0:
                    ...
                    self.entry_price = entry_price
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {self.sl_price:.2f}, TP {self.tp_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {atr_val:.2f}, EMA50 {self.ema50[-1]:.2f}, Vol {self.data.Volume[-1]:.0f} > {self.avg_vol[-1]:.0f} ✨"
                    )
