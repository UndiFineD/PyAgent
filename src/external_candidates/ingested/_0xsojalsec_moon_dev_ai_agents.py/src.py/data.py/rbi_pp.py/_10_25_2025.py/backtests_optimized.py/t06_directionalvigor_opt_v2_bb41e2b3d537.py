...
class DirectionalVigor(Strategy):
    adx_period = 14
    adx_threshold = (
        25  # Kept at 25 for quality trends, but added filters for better selectivity
    )
    adx_weak = 20
    risk_per_trade = (
        0.015  # Increased to 1.5% to amplify returns while maintaining risk control
    )
    atr_multiplier = 2.0
    ...
    def next(self):
        ...
        if short_entry and not self.position:
            ...
            if risk_per_unit > 0:
                position_size = (
                    risk_amount / risk_per_unit
                )  # Same sizing logic for shorts
                sl_price = current_close + risk_per_unit
                tp_price = (
                    current_close - self.tp_atr_multiplier * current_atr
                )  # TP at 4 ATR below for shorts
