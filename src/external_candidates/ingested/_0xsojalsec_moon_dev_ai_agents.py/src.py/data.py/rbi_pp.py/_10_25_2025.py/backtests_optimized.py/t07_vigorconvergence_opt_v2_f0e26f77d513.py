# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v2.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                )  # 🌙 Moon Dev: Simplified and corrected to fractional size (no int rounding) for better risk management and scalability
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Backtest Progress: {self.data.index[-1]}, ADX: {current_adx:.2f}, MACD: {current_macd:.2f}, RSI: {current_rsi:.2f}, Trend: {'Bull' if current_close > current_ema200 else 'Bear'} ✨"
            )
