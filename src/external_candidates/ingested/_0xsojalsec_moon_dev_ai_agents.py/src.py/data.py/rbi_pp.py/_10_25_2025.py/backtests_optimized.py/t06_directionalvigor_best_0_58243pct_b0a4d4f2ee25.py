# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_BEST_0.58243pct.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    atr_multiplier = 2.0
    tp_multiplier = (
        4.0  # 🌙 Moon Dev: Added 2:1 reward-to-risk ratio for better profitability ✨
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Added 200 EMA for trend filter to avoid counter-trend trades ✨
        print(
            "🌙 Moon Dev: Indicators initialized for DirectionalVigor strategy with EMA200 and TP optimization ✨"
        )

    def next(self):
        if (
            len(self.data) < 201
        ):  # 🌙 Moon Dev: Adjusted for EMA200 initialization to ensure data availability ✨
            return
        ...
        if long_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                position_size = int(
                    round(position_size)
                )  # 🌙 Moon Dev: Kept integer sizing for consistency, but risk-based for optimization ✨
                sl_price = current_close - risk_per_unit
                tp_price = current_close + (self.tp_multiplier * current_atr)
                self.buy(size=position_size, sl=sl_price, tp=tp_price)
                print(
                    f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} 🚀✨"
                )
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                print(
                    f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {position_size} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} 🚀✨"
                )
