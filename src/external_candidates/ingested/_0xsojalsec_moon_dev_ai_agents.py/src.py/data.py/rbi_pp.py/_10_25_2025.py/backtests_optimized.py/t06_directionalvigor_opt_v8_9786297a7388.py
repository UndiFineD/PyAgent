# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v8.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Switched to EMA50 for quicker trend confirmation ✨
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=self.rsi_period
        )  # 🌙 Moon Dev: Added RSI for momentum filter to avoid weak entries ✨
        self.vol_ma = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.vol_period
        )  # 🌙 Moon Dev: Added Volume MA for confirming conviction in signals ✨
        self.entry_price = 0
        self.position_type = None
        print(
            "🌙 Moon Dev: Indicators initialized with EMA50, RSI, Volume MA, and trailing stop setup for optimized returns ✨"
        )

    def next(self):
        if (
            len(self.data) < 51
        ):  # 🌙 Moon Dev: Adjusted for EMA50 and other indicators to ensure sufficient data ✨
            return
        ...
        if long_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                )  # 🌙 Moon Dev: Allow fractional sizing for precise risk allocation and better capital efficiency ✨
                ...
                print(
                    f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {position_size:.4f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                position_size = (
                    risk_amount / risk_per_unit
                )  # 🌙 Moon Dev: Fractional sizing for shorts as well ✨
                sl_price = current_close + risk_per_unit
                tp_price = current_close - (self.tp_multiplier * current_atr)
                ...
                print(
                    f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {position_size:.4f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
            ...
            if self.position_type == "long":
                ...
                if profit_dist > self.atr_multiplier * current_atr:
                    ...
                    if self.position.sl is None or trail_sl > self.position.sl:
                        ...
                        print(
                            f"🌙 Moon Dev: Trailing LONG SL to {trail_sl:.2f} | Profit Dist: {profit_dist:.2f} ✨"
                        )
                ...
                if profit_dist > self.atr_multiplier * current_atr:
                    ...
                    if self.position.sl is None or trail_sl < self.position.sl:
                        self.position.sl = trail_sl
                        print(
                            f"🌙 Moon Dev: Trailing SHORT SL to {trail_sl:.2f} | Profit Dist: {profit_dist:.2f} ✨"
                        )
