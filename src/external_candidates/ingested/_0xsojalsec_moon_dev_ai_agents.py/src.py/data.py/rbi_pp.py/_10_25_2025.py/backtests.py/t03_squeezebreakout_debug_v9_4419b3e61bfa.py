# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T03_SqueezeBreakout_DEBUG_v9.py
import numpy as np
import pandas as pd
import talib
...
class SqueezeBreakout(Strategy):
    ...
    def init(self):
        ...
        self.squeeze = self.I(
            squeeze_func, bb_upper, bb_lower, self.kc_upper, self.kc_lower
        )
    ...
    def next(self):
        ...
        prev_squeeze = (
            bool(prev_squeeze_val) if not np.isnan(prev_squeeze_val) else False
        )

        current_vol_osc = self.vol_osc[current_index]
        current_bb_width = self.bb_width[current_index]
        prev_bb_width = self.bb_width[prev_index]
        start_idx = max(0, prev_index - self.lookback_squeeze + 1)
        bb_widths = self.bb_width[start_idx : prev_index + 1]
        min_width = np.nanmin(bb_widths)
        is_lowest_width = (
            not np.isnan(prev_bb_width)
            and not np.isnan(min_width)
            and np.isclose(prev_bb_width, min_width, atol=1e-10)
        )
        ...
        if squeeze_ended:
            print(
                f"🌙 Moon Dev: Squeeze ended detected at {current_price:.2f}, vol_osc {current_vol_osc:.2f}, prev_bb_width {prev_bb_width:.4f}, min_width {min_width:.4f} 🔍✨"
            )
        ...
        if not self.position:
            ...
            if (
                squeeze_ended
                and current_price > self.bb_upper[current_index]
                and current_vol_osc > 0
                and current_price > self.bb_middle[current_index]
            ):
                ...
                if risk_distance > 0:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev: Squeeze Breakout LONG entry at {entry_price:.2f}, size {size}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀✨"
                        )
                    else:
                        print(
                            f"🌙 Moon Dev: LONG signal but size {size} <=0, skipping 📉"
                        )

            # Short entry
            elif (
                squeeze_ended
                and current_price < self.bb_lower[current_index]
                and current_vol_osc < 0
                and current_price < self.bb_middle[current_index]
            ):
                entry_price = current_price
                ...
                if risk_distance > 0:
                    ...
                    if size > 0:
                        ...
                        print(
                            f"🌙 Moon Dev: Squeeze Breakout SHORT entry at {entry_price:.2f}, size {size}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀✨"
                        )
                    else:
                        print(
                            f"🌙 Moon Dev: SHORT signal but size {size} <=0, skipping 📉"
                        )
        ...
        # Early exit if re-squeeze
            ...
            self.position.close()
            print(
                f"🌙 Moon Dev: Early exit due to re-squeeze at {current_price:.2f} 😔"
            )


# Run backtest
bt = Backtest(data, SqueezeBreakout, cash=1000000, commission=0.002)
stats = bt.run()
print(stats)
