# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T10_SwiftStrength_OPT_v2.py
import numpy as np
import pandas as pd
import talib
...
class SwiftStrength(Strategy):
    ...
    rr_ratio = (
        3  # 🌙 Moon Dev: Increased from 2 to 3 for better reward potential on winners
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Changed to standard 12/26/9 for less noise and more reliable signals on 15m
        ...
        )  # 🌙 Moon Dev: Added RSI for momentum confirmation to filter overbought/oversold entries
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=20
        )  # 🌙 Moon Dev: Added Volume SMA for volume confirmation to avoid low-conviction setups
        print(
            "🌙 Moon Dev: Indicators including MACD(12/26/9), ADX, RSI, and Volume SMA initialized successfully! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if current_adx < self.weak_trend_threshold:
                self.position.close()
                print(
                    f"🌙 Moon Dev: Emergency exit due to weak trend (ADX < {self.weak_trend_threshold}) 📉"
                )
            ...
            if (self.is_long and current_low <= self.trailing_sl) or (
                not self.is_long and current_high >= self.trailing_sl
            ):
                self.position.close()
            ...
            if not self.partial_closed:
                if (self.is_long and current_high >= self.partial_tp) or (
                    not self.is_long and current_low <= self.partial_tp
                ):
                    half_size = abs(self.position.size) / 2
                    self.position.close(size=half_size)
                    self.partial_closed = True
                    print(
                        f"🌙 Moon Dev: Partial TP (50%) hit at {self.partial_tp:.2f} 🟡"
                    )
                    # Continue to check full TP on remaining position

            # Check full TP (close remaining at 3R)
            if (self.is_long and current_high >= self.full_tp) or (
                not self.is_long and current_low <= self.full_tp
            ):
                self.position.close()
            ...
            if self.position.is_long and (
                self.macdsignal[-2] < self.macd[-2]
                and self.macdsignal[-1] > self.macd[-1]
            ):
                self.position.close()
            ...
            if self.position.is_short and (
                self.macd[-2] < self.macdsignal[-2]
                and self.macd[-1] > self.macdsignal[-1]
            ):
                self.position.close()
        ...
        if not self.position:
            ...
            )  # 🌙 Moon Dev: Removed int(round()) to allow fractional sizes for precise risk management
            if position_size <= 0:
                print(
                    "🌙 Moon Dev: Position size too small or invalid, skipping entry 🚫"
                )
                return
            ...
            if (
                ...
                print(
                    f"🌙 Moon Dev: Long entry at {current_price:.2f}, size {position_size:.4f}, SL {sl_price:.2f}, partial TP {self.partial_tp:.2f}, full TP {self.full_tp:.2f} 🚀"
                )
            ...
            # Short entry with tightened filters (RSI < 50 for bearish momentum)
                ...
                sl_price = current_price + sl_distance
                print(
                    f"🌙 Moon Dev: Short entry at {current_price:.2f}, size {position_size:.4f}, SL {sl_price:.2f}, partial TP {self.partial_tp:.2f}, full TP {self.full_tp:.2f} 📉"
                )


# Run backtest
bt = Backtest(
    data, SwiftStrength, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
