# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T11_AdaptiveAtr_OPT_v8.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveAtr(Strategy):
    ...
    def init(self):
        ...
        # 🌙✨ OPTIMIZATION: Switched to faster SMAs (20/100) for more responsive signals on 15m BTC data to capture more trends and increase trade frequency realistically
        self.sma20 = self.I(talib.SMA, close, timeperiod=20)
        self.sma100 = self.I(talib.SMA, close, timeperiod=100)
        # 🌙✨ OPTIMIZATION: Added longer SMA200 as trend filter (only long above it, short below) to avoid counter-trend trades and improve win rate
        self.sma200 = self.I(talib.SMA, close, timeperiod=200)
        self.atr = self.I(talib.ATR, high, low, close, timeperiod=14)
        # 🌙✨ OPTIMIZATION: Shortened ATR SMA to 14 for quicker volatility regime detection
        self.sma_atr = self.I(talib.SMA, self.atr, timeperiod=14)
        self.sma_atr50 = self.I(talib.SMA, self.atr, timeperiod=50)
        self.avg_vol = self.I(talib.SMA, volume, timeperiod=20)
        # 🌙✨ OPTIMIZATION: Added RSI(14) as momentum filter to confirm crossovers (RSI>50 for long, <50 for short) reducing false signals in ranging markets
    ...
    def calculate_size(self):
        equity = self.equity
        # 🌙✨ OPTIMIZATION: Increased base risk to 1.5% in normal regime for higher returns, keep 0.75% in low vol to balance risk; uses ATR for dynamic sizing
        risk_pct = 0.0075 if self.low_vol_regime else 0.015
        risk_amount = equity * risk_pct
        atr_val = self.atr[-1]
        # 🌙✨ OPTIMIZATION: Tightened stop distance to 1.5*ATR for better risk-reward, allowing larger positions
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙📊 Bar {len(self.data)}: Close={current_price:.2f}, SMA20={self.sma20[-1]:.2f}, SMA100={self.sma100[-1]:.2f}, RSI={rsi:.2f}, ATR={atr:.2f}"
            )

        # 🌙✨ OPTIMIZATION: Updated crossovers to use faster SMAs for more timely entries
        long_cross = (
            self.sma20[-2] < self.sma100[-2] and self.sma20[-1] > self.sma100[-1]
        )
        short_cross = (
            self.sma20[-2] > self.sma100[-2] and self.sma20[-1] < self.sma100[-1]
        )
        ...
        if self.position:
            ...
            # Reversal exit using faster SMAs
            ...
            # 🌙✨ OPTIMIZATION: Reduced time-based exit to 8 bars for quicker exits in non-trending conditions, preventing stagnation
            if bars_in_trade > 8 and profit_distance < atr:
                self.position.close()
                print("🌙⏰ Time-based exit: No progress in 8 bars!")
                return
            ...
            # SL/TP/Trailing
            if pos.is_long:
                # 🌙✨ OPTIMIZATION: Tightened initial SL to 1.5*ATR, improved trailing to start after 1*ATR profit and trail at 1*ATR for better profit capture
                sl = self.entry_price - 1.5 * self.entry_atr
                profit = current_price - self.entry_price
                if profit > self.entry_atr:
                    sl = max(
                        sl, self.entry_price + 1 * self.entry_atr - 1 * atr
                    )  # Dynamic trail
                trail_sl = current_price - 1 * atr
                sl = max(sl, trail_sl)

                # 🌙✨ OPTIMIZATION: Increased TP to 4*ATR for higher reward:risk ratio (targeting 2.67:1 RR) to boost returns
                ...
                if profit > self.entry_atr:
                    sl = min(sl, self.entry_price - 1 * self.entry_atr + 1 * atr)
                trail_sl = current_price + 1 * atr
                sl = min(sl, trail_sl)
        ...
        if not self.position:
            ...
            # 🌙✨ OPTIMIZATION: Added RSI momentum filter and trend filter (price vs SMA200) to enter only high-quality setups, reducing whipsaws
            rsi_long_ok = rsi > 50
            rsi_short_ok = rsi < 50
            ...
            if long_cross and atr_ok and vol_ok and rsi_long_ok and trend_long_ok:
                ...
                if size > 0:
                    ...
                    print(
                        f"🌙🚀 LONG ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, RSI: {rsi:.2f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                ...
                if size > 0:
                    ...
                    self.entry_step = len(self.data) - 1
                    print(
                        f"🌙📉 SHORT ENTRY! Price: {current_price:.2f}, Size: {size}, ATR: {atr:.2f}, RSI: {rsi:.2f}, Vol OK: {vol_ok} ✨"
                    )
                else:
                    print("🌙❌ Short signal but size=0, skipped.")
            elif long_cross or short_cross:
                print(
                    f"🌙⚠️ Crossover detected but filters failed: ATR_OK={atr_ok}, Vol_OK={vol_ok}, RSI={rsi:.2f}, Trend OK={trend_long_ok if long_cross else trend_short_ok}"
                )
            else:
                pass  # No signal
