# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T01_ClimaxReversal_DEBUG_v1.py
import numpy as np
import pandas as pd
import talib
...
class ClimaxReversal(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and (self._i - self.entry_bar) > self.max_hold_bars
        ):
            self.position.close()
        ...
        if (
            ...
            and gap_close < self.sma[self._i - 1]  # Downtrend
            and self.rsi[self._i - 1] < 30  # Oversold at gap
            and curr_close > gap_open  # Confirmation close above gap open
            and curr_close > curr_open  # Bullish candle
            and vol_high_gap
            and vol_low_confirm
            and self.stoch_k[self._i] > self.stoch_d[self._i]
            and self.stoch_k[self._i - 1] <= self.stoch_d[self._i - 1]
        ):  # Stoch crossover up
        ...
        # Short Entry: Bearish Reversal in Uptrend
            ...
            and gap_close > self.sma[self._i - 1]  # Uptrend
            and self.rsi[self._i - 1] > 70  # Overbought at gap
