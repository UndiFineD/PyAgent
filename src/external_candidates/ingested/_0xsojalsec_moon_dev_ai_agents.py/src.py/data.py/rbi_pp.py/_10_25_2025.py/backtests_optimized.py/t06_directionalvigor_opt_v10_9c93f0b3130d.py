# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v10.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    def init(self):
        ...
        self.ema100 = self.I(
            talib.EMA, self.data.Close, timeperiod=100
        )  # 🌙 Moon Dev: Switched to 100 EMA for faster trend confirmation on 15m timeframe, reducing lag in bull/bear detection ✨
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=14
        )  # 🌙 Moon Dev: Added RSI for momentum filter to ensure entries align with market strength and avoid weak signals ✨
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=20
        )  # 🌙 Moon Dev: Added volume SMA to filter for high-conviction moves, only trading on above-average volume spikes ✨
        print(
            "🌙 Moon Dev: Indicators initialized for optimized DirectionalVigor with RSI, Volume filter, faster ADX/EMA, and enhanced RR/trailing logic ✨"
        )

    def next(self):
        if (
            len(self.data) < 101
        ):  # 🌙 Moon Dev: Adjusted for EMA100 and other indicators to ensure sufficient data history ✨
            return
        ...
        long_signal = (
            ...
            and (current_rsi > 50)
            and (current_vol > current_vol_sma * 1.2)
        )
        ...
        if long_signal and not self.position:
            ...
            )  # 🌙 Moon Dev: Fallback for equity calculation ✨
            risk_per_unit = self.atr_multiplier * current_atr
            if risk_per_unit > 0:
                position_size = (
                    risk_amount / risk_per_unit
                )  # 🌙 Moon Dev: Use float sizing for precise risk allocation, allowing fractional positions for better capital efficiency ✨
                ...
                print(
                    f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {position_size:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                self.entry_price = current_close  # 🌙 Moon Dev: Track entry price for trailing stop logic ✨
                print(
                    f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {position_size:.2f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} | RSI: {current_rsi:.2f} 🚀✨"
                )
        ...
        if self.position:
            ...
            )  # Actual entry price from backtesting.py
            ...
            if self.position.is_long:
                ...
                if profit > trail_threshold:
                    ...
                    if new_sl > self.position.sl:
                        ...
                        print(
                            f"🌙 Moon Dev: Trailing LONG SL to {new_sl:.2f} | Profit: {profit:.2f} 📈🔒"
                        )
                ...
                if reversal or weakening:
                    ...
                    print(
                        f"🌙 Moon Dev: Exiting LONG at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} 📉💫"
                    )
                ...
                if profit > trail_threshold:
                    ...
                    if new_sl < self.position.sl:
                        self.position.sl = new_sl
                        print(
                            f"🌙 Moon Dev: Trailing SHORT SL to {new_sl:.2f} | Profit: {profit:.2f} 📉🔒"
                        )
                ...
                if reversal or weakening:
                    ...
                    exit_reason = "Reversal" if reversal else "Weakening Trend"
                    print(
                        f"🌙 Moon Dev: Exiting SHORT at {current_close:.2f} | Reason: {exit_reason} | ADX: {current_adx:.2f} 📉💫"
                    )
...
# Run backtest
bt = Backtest(
    data, DirectionalVigor, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
