# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T10_SwiftStrength_OPT_v4.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if len(self.trades) == 0 and not self.position:
            ...
            if (
                ...
                and current_rsi > 50
            ):  # 🌙 Moon Dev: Added RSI >50 filter to ensure sufficient bullish momentum, filtering out weak setups
                ...
                print(
                    f"🌙 Moon Dev: Long entry at {current_price:.2f}, size {position_size:.4f}, SL {sl:.2f}, TP {tp:.2f} 🚀"
                )
            ...
            # Short entry with added RSI filter (<50 for bearish momentum)
                ...
                and current_price < current_sma
                and current_rsi < 50
            ):  # 🌙 Moon Dev: Added RSI <50 filter to ensure sufficient bearish momentum, filtering out weak setups
                ...
                print(
                    f"🌙 Moon Dev: Short entry at {current_price:.2f}, size {position_size:.4f}, SL {sl:.2f}, TP {tp:.2f} 📉"
                )
