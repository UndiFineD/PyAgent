# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_package\T03_SqueezeBreakout_PKG.py
import numpy as np
import pandas as pd
import talib
...
data.columns = [
    col.capitalize() if col in ["open", "high", "low", "close", "volume"] else col
...
class SqueezeBreakout(Strategy):
    ...
    def next(self):
        ...
        is_lowest_width = (
            current_bb_width == np.min(bb_widths)
            if len(bb_widths) == self.lookback_squeeze
            else False
        )
        ...
        elif (self.position.is_long and current_squeeze) or (
            self.position.is_short and current_squeeze
        ):
            self.position.close()
            print(
