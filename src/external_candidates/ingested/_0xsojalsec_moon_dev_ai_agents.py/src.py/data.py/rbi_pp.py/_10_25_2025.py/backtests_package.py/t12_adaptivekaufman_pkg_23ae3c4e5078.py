# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_package\T12_AdaptiveKaufman_PKG.py
import numpy as np
import pandas as pd
import talib
...
class AdaptiveKaufman(Strategy):
    ...
    def init(self):
        ...
        self.vol_short_ma = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.vol_short
        )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if close > kama and vol_osc > 0 and kama_slope_up and not atr_expanding:
                ...
                size = (risk_amount / stop_distance) * vol_adjust
                sl_price = close - stop_distance
                print(
                    f"🌙 Moon Dev: Long signal! Price: {close:.2f}, KAMA: {kama:.2f}, Vol Osc: {vol_osc:.3f}, ATR: {atr:.2f}, Size: {size:.4f}"
                )
                self.buy(size=size, sl=sl_price)
                print(
                    f"🚀 Entering long at {close:.2f}, SL {sl_price:.2f}, Size {size:.4f} BTC ✨"
                )
            ...
            # Short entry
                ...
                sl_price = close + stop_distance
                print(
                    f"🌙 Moon Dev: Short signal! Price: {close:.2f}, KAMA: {kama:.2f}, Vol Osc: {vol_osc:.3f}, ATR: {atr:.2f}, Size: {size:.4f}"
                )
                self.sell(size=size, sl=sl_price)
                print(
                    f"🔻 Entering short at {close:.2f}, SL {sl_price:.2f}, Size {size:.4f} BTC ✨"
                )

        else:
            if self.position.is_long:
                # Trailing stop
                profit = close - self.position.entry_price
                if profit > self.trail_start * atr:
                    new_sl = max(
                        self.position.sl, self.kama[-1] - self.trail_offset * atr
                    )
                ...
                # Trailing stop
                profit = self.position.entry_price - close
                if profit > self.trail_start * atr:
                    new_sl = min(
                        self.position.sl, self.kama[-1] + self.trail_offset * atr
                    )
...
bt = Backtest(
    data, AdaptiveKaufman, cash=1000000, commission=0.001, exclusive_orders=True
)
stats = bt.run()
print(stats)
