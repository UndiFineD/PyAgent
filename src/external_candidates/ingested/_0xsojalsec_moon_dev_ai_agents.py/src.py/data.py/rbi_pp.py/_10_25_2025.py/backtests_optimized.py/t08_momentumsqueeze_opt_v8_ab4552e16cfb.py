# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T08_MomentumSqueeze_OPT_v8.py
import numpy as np
import pandas as pd
import talib
...
class MomentumSqueeze(Strategy):
    def init(self):
        ...
        self.tp = 0.0
        self.partial_closed = (
            False  # 🌙 Moon Dev: Flag for scaling out once (Exit Optimization)
        )
        self.entry_bar = (
            0  # 🌙 Moon Dev: Track entry bar for time-based exit (Exit Optimization)
        )
    ...
    def next(self):
        ...
        if (
            ...
            and current_bar - self.last_short_breakout > 10
        ):
            self.last_short_breakout = -100
            self.short_breakout_high = None
        ...
        if (
            ...
            if risk > 0:
                ...
                print(
                    f"🌙 Moon Dev: LONG ENTRY at {entry}, SL: {sl}, TP: {tp}, Size: {pos_size:.2f}, Risk: {risk_amount:.2f} 🚀"
                )
                if bull_cross:
        ...
        if (
            ...
            if risk > 0:
                ...
                self.entry_bar = current_bar  # 🌙 Moon Dev: Record entry bar
                print(
                    f"🌙 Moon Dev: SHORT ENTRY at {entry}, SL: {sl}, TP: {tp}, Size: {pos_size:.2f}, Risk: {risk_amount:.2f} 📉"
                )
                if bear_cross:
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if favorable_move >= self.initial_risk and not self.partial_closed:
                    ...
                    print(
                        f"🌙 Moon Dev: Scaled out 30% of long at 1R, remaining size: {self.position.size:.2f} 💰"
                    )
                if favorable_move >= 3 * self.initial_risk and self.partial_closed:
                    partial_size = self.position.size * 0.5  # 50% of remaining
                    self.sell(size=partial_size)
                    print(
                        f"🌙 Moon Dev: Scaled out additional 50% of remaining long at 3R 💰"
                    )
                ...
                if (
                    self.hist[-1] <= 0 and self.macd_line[-1] < self.macd_line[-5]
                ):  # Added simple divergence filter
                    self.position.close()
                    print(
                        f"🌙 MACD histogram turned negative with bearish divergence, closing long 📊"
                    )
                    self._reset_pos()
                    return
            else:  # short
                ...
                if favorable_move >= self.initial_risk and not self.partial_closed:
                    ...
                    self.partial_closed = True
                    print(
                        f"🌙 Moon Dev: Scaled out 30% of short at 1R, remaining size: {self.position.size:.2f} 💰"
                    )
                if favorable_move >= 3 * self.initial_risk and self.partial_closed:
                    partial_size = abs(self.position.size) * 0.5  # 50% of remaining
                    self.buy(size=partial_size)
                    print(
                        f"🌙 Moon Dev: Scaled out additional 50% of remaining short at 3R 💰"
                    )
                # Trailing stop after 1:1, tightened to EMA20 + 0.5 ATR for better protection after initial move (Exit Optimization)
                if favorable_move >= self.initial_risk:
                ...
                if (
                    ...
                    self.position.close()
                    print(
                        f"🌙 MACD histogram turned positive with bullish divergence, closing short 📊"
                    )
                    self._reset_pos()
                    return
