# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v3.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Initialized SMA for market regime filter
        print("🌙 Moon Dev: Indicators initialized for DirectionalVigor strategy ✨")

    def next(self):
        if (
            len(self.data) < max(self.adx_period, self.sma_period) + 1
        ):  # 🌙 Moon Dev: Updated length check to account for SMA period, ensuring all indicators are valid
            return
        ...
        tp_distance = (
            self.tp_multiplier * current_atr
        )  # 🌙 Moon Dev: Dynamic TP based on ATR for consistent RR across volatility regimes
        risk_amount = self.risk_per_trade * self.equity
        if risk_per_unit > 0:
            position_size = (
                risk_amount / risk_per_unit
            )  # 🌙 Moon Dev: Kept volatility-based sizing but now applied to both long/short for balanced exposure
        ...
        if long_entry and not self.position:
            ...
            print(
                f"🌙 Moon Dev: Entering LONG at {current_close:.2f} | Size: {position_size:.4f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} 🚀✨"
            )
        ...
        if short_entry and not self.position:
            ...
            print(
                f"🌙 Moon Dev: Entering SHORT at {current_close:.2f} | Size: {position_size:.4f} | SL: {sl_price:.2f} | TP: {tp_price:.2f} | ADX: {current_adx:.2f} 📉✨"
            )
