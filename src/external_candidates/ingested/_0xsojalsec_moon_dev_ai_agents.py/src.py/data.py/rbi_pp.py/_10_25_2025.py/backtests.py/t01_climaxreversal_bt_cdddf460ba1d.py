# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T01_ClimaxReversal_BT.py
import numpy as np
import pandas as pd
import talib
...
class ClimaxReversal(Strategy):
    ...
    def next(self):
        if (
            len(self.data)
            < max(self.sma_period, self.atr_period, self.vol_ma_period) + 3
        ):
            return
