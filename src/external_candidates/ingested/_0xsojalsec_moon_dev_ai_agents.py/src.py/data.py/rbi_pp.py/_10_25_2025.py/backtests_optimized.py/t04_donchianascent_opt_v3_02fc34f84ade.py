# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    tp_multiplier = (
        3  # 🌙 Increased from 2 to 3 to capture larger profits on winning breakouts
    )
        ...
        if self.position:
            ...
            if current_close > entry_price:
                ...
                if new_sl > self.position.sl:
                    ...
                    print(
                        f"🌙 Moon Dev Trailing SL updated to {new_sl} from {old_sl} 🌙"
                    )
            ...
            if bars_in_trade >= self.time_exit_bars:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: Time-based exit at {current_close} after {bars_in_trade} bars 🚀"
                )
                self.entry_bar = None
        else:
