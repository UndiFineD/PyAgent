# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T08_MomentumSqueeze_BEST_1.059768pct.py
import numpy as np
import pandas as pd
import talib
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if favorable_move >= 2 * self.initial_risk and not self.partial_closed:
                    ...
                    print(
                        f"🌙 Moon Dev: Scaled out 50% of long at 2R, remaining size: {self.position.size} 💰"
                    )
                ...
                if favorable_move >= 2 * self.initial_risk and not self.partial_closed:
                    ...
                    self.partial_closed = True
                    print(
                        f"🌙 Moon Dev: Scaled out 50% of short at 2R, remaining size: {self.position.size} 💰"
                    )
                # Trailing stop after 1:1
                if favorable_move >= self.initial_risk:
                    # 🌙 Moon Dev: Loosened trailing to EMA20 + 1 ATR for more room to run while protecting gains (Exit Optimization)
