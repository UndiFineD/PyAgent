# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T12_AdaptiveKaufman_DEBUG_v4.py
import numpy as np
import pandas as pd
import talib
        ...
        if self.position:
            if self.position.is_long:
                ...
                if profit > self.trail_start * atr:
                    new_sl = max(
                        self.stop_loss, self.kama[-1] - self.trail_offset * atr
                    )
                ...
                if close < self.stop_loss:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev: Long SL hit at {close:.2f}, SL was {self.stop_loss:.2f} 💥"
                    )
                    self.stop_loss = None
                    return
                ...
                if close > self.stop_loss:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev: Short SL hit at {close:.2f}, SL was {self.stop_loss:.2f} 💥"
                    )
                    self.stop_loss = None
                    return
