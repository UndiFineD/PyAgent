# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T08_MomentumSqueeze_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class MomentumSqueeze(Strategy):
    ...
    def next(self):
        ...
        trend_up = (
            self.ema20[-1] > self.ema50[-1] > self.ema200[-1]
            and price > self.ema200[-1]
        )
        trend_down = (
            self.ema20[-1] < self.ema50[-1] < self.ema200[-1]
            and price < self.ema200[-1]
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if (
                    favorable_move >= 2.5 * self.initial_risk
                    ...
                    print(
                        f"🌙 Moon Dev: Scaled out 50% of long at 2.5R, remaining size: {self.position.size} 💰"
                    )
                ...
                if (
                    ...
                    self.partial_closed = True
                    print(
                        f"🌙 Moon Dev: Scaled out 50% of short at 2.5R, remaining size: {self.position.size} 💰"
                    )
                # Trailing stop after 1:1
                if favorable_move >= self.initial_risk:
                    # 🌙 Moon Dev: Tightened trailing to EMA20 + 0.5 ATR for better protection without cutting winners short (Exit Optimization)
