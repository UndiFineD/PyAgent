# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T07_VigorConvergence_OPT_v3.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                )  # Float size in BTC units for realistic fractional positioning

                self.buy(sl=sl_price, tp=tp_price, size=size)
                print(
                    f"🌙 Moon Dev: Long entry at {current_close}, SL: {sl_price}, TP: {tp_price}, Size: {size} (filters: ADX={current_adx:.1f}, RSI={current_rsi:.1f}, Vol>{current_vol_sma:.0f}) 🚀"
                )
            ...
            # 🌙 Moon Dev Optimization: Enhanced short entry - symmetric filters for bearish setups
                ...
                print(
                    f"🌙 Moon Dev: Short entry at {current_close}, SL: {sl_price}, TP: {tp_price}, Size: {size} (filters: ADX={current_adx:.1f}, RSI={current_rsi:.1f}, Vol>{current_vol_sma:.0f}) 🚀"
                )

        # Debug print every 100 bars or so (enhanced with new indicators)
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Backtest Progress: {self.data.index[-1]}, ADX: {current_adx:.2f}, MACD: {current_macd:.2f}, RSI: {current_rsi:.1f}, Close vs EMA200: {current_close > current_ema200} ✨"
            )
