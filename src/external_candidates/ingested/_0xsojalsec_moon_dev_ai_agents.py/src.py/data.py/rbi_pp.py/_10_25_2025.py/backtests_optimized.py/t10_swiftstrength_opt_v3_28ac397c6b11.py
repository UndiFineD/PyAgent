...
class SwiftStrength(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev: Indicators initialized successfully with volume filter and trailing prep! ✨"
        )
    ...
    def next(self):
            ...
            if self.position.is_long:
                ...
                if unrealized_pnl > trail_threshold:
                    ...
                    if new_sl > self.position.sl:
                        ...
                        print(
                            f"🌙 Moon Dev: Trailing SL updated to {new_sl:.2f} for long 🛡️"
                        )
                ...
                if unrealized_pnl > trail_threshold:
                    ...
                    if new_sl < self.position.sl:
                        self.position.sl = new_sl
                        print(
                            f"🌙 Moon Dev: Trailing SL updated to {new_sl:.2f} for short 🛡️"
                        )
        ...
        if not self.position:
            ...
            )  # 🌙 Keep as float for precise position sizing
            if position_size <= 0:
            ...
            if (
                ...
                if sl >= current_price or tp <= current_price:
                    ...
                    return
                self.buy(
                    size=position_size, sl=sl, tp=tp
                )  # 🌙 Removed limit for market entry at next open
