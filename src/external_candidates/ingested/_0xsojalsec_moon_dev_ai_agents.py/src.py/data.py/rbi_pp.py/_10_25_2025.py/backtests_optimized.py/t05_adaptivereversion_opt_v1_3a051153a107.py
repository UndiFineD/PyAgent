# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T05_AdaptiveReversion_OPT_v1.py
...
class AdaptiveReversion(Strategy):
    ...
    adx_period = 14
    adx_threshold = (
        25  # 🌙 Moon Dev: ADX < 25 filters for ranging markets ideal for mean reversion
    )
    vol_sma_period = 20
    atr_sma_period = 20
    ...
    def next(self):
        ...
        if self.position:
            ...
            if exit_reason:
                self.position.close()
                print(
                    f"🌙 Moon Dev: {exit_reason} Exit {'Long' if is_long else 'Short'} at {close} 🚀 RSI: {rsi:.2f}"
                )
        ...
        if not self.position:
            ...
            if (
                ...
                size = (
                    risk / sl_dist
                )  # 🌙 Moon Dev: Use float for precise fractional sizing, no int rounding
                ...
                self.buy(
                    size=size, sl=sl
                )  # 🌙 Moon Dev: No fixed TP - use dynamic exit for adaptability
                self.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Long at {close}, Size: {size:.4f}, SL: {sl:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f} 🚀"
                )
            ...
            # Short entry - tightened with filters for better downtrend reversion
                ...
                size = (
                    risk / sl_dist
                )  # 🌙 Moon Dev: Use float for precise fractional sizing
                ...
                self.sell(
                    size=size, sl=sl
                )  # 🌙 Moon Dev: No fixed TP - use dynamic exit
                self.entry_time = current_time
                print(
                    f"🌙 Moon Dev: Entering Short at {close}, Size: {size:.4f}, SL: {sl:.2f}, RSI: {rsi:.2f}, ATR: {atr:.2f}, ADX: {adx:.2f} ✨"
                )
