# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests\T01_ClimaxReversal_DEBUG_v9.py
import numpy as np
import pandas as pd
import talib
...
class ClimaxReversal(Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            and (len(self.data) - 1 - self.entry_bar) > self.max_hold_bars
        ):
            self.position.close()
        ...
        if gap_size_pct > 0.0005:  # 0.05% threshold for debug
            ...
            print(
                f"🌙 Moon Dev Debug: Bar {len(self.data)-1}, Gap%: {gap_size_pct*100:.3f}, Down?: {is_gap_down}, Consec down: {self.get_consec_down()}, up: {self.get_consec_up()}, "
