# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T04_DonchianAscent_OPT_v5.py
import numpy as np
import pandas as pd
import talib
...
class DonchianAscent(Strategy):
    ...
    risk_percent = (
        0.01  # 🌙 Maintained 1% risk per trade for consistent risk management
    )
    ...
    sma_period = (
        200  # New: Longer SMA for overall trend filter to only trade in uptrends
    )
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev Backtest Initialized: Optimized DonchianAscent Strategy Loaded! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            )  # ✨ Stricter volume confirmation for higher conviction breakouts
            # 🌙 Switched to ATR-based width filter for volatility-adjusted quality (avoids % bias in high-price BTC)
            channel_too_narrow = prev_width < (
                1.5 * prev_atr
            )  # New: Use 1.5x ATR for min width to filter low-vol chop
            extended = (current_close - prev_middle) > (
                self.extended_multiplier * prev_width
            )  # Tuned multiplier for better equilibrium check
            ...
            if (
                ...
                if risk_per_share > 0:
                    ...
                    )  # Higher TP for improved RR

                    self.buy(size=position_size, sl=sl_price, tp=tp_price)
                    self.entry_bar = len(self.data)
                    self.initial_sl = sl_price
                    print(
                        f"🌙 Moon Dev Long Entry: Optimized Breakout at {current_close}, size={position_size}, SL={sl_price}, TP={tp_price} (RSI={current_rsi:.1f}, Trend=Up) ✨🚀"
                    )
