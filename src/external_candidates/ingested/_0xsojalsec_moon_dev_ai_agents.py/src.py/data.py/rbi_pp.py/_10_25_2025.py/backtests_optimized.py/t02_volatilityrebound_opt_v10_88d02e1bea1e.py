# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T02_VolatilityRebound_OPT_v10.py
import numpy as np
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            if self.position.is_long:
                if self.data.Close[-1] <= self.sl:
                    self.position.close()
                    print(
                        f"🌙 LONG EXIT SL at {self.data.Close[-1]:.2f} 💥 SL: {self.sl:.2f} 🚀"
                    )
                elif self.data.Close[-1] >= self.tp:
                    self.position.close()
                    print(
                        f"🌙 LONG EXIT TP at {self.data.Close[-1]:.2f} 🎯 TP: {self.tp:.2f} 🚀"
                    )
            elif self.position.is_short:
                if self.data.Close[-1] >= self.sl:
                    self.position.close()
                    print(
                        f"🌙 SHORT EXIT SL at {self.data.Close[-1]:.2f} 💥 SL: {self.sl:.2f} 📉"
                    )
                elif self.data.Close[-1] <= self.tp:
                    self.position.close()
                    print(
                        f"🌙 SHORT EXIT TP at {self.data.Close[-1]:.2f} 🎯 TP: {self.tp:.2f} 📉"
                    )
        ...
        # Entry logic - no position
            ...
            if (
                ...
                and self.data.Volume[-1] > self.vol_ma[-1]
            ):
                ...
                if sl_distance > 0:
                    risk_amount = (
                        self.equity * 0.02
                    )  # Increased to 2% risk for higher returns
                    size = (
                        risk_amount / sl_distance
                    )  # Float size for precise sizing (no int rounding)
                    self.buy(size=size)
                    self.sl = sl_price
                    self.tp = tp_price
                    print(
                        f"🚀 🌙 LONG ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Lower BB {self.bb_lower[-1]:.2f}, ATR {self.atr[-1]:.2f} ✨"
                    )
            ...
            # 🌙 Short entry: Tightened RSI>75, added trend (below EMA200) and volume (>avg) filters, ATR-based SL/TP
                ...
                if sl_distance > 0:
                    ...
                    self.tp = tp_price
                    print(
                        f"📉 🌙 SHORT ENTRY: Price {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size:.4f}, RSI {self.rsi[-1]:.2f}, Upper BB {self.bb_upper[-1]:.2f}, ATR {self.atr[-1]:.2f} ✨"
                    )
