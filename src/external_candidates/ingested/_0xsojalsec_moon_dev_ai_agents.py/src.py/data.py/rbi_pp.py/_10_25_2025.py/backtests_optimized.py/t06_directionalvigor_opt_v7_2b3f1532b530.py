# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_25_2025\backtests_optimized\T06_DirectionalVigor_OPT_v7.py
import pandas as pd
import talib
...
class DirectionalVigor(Strategy):
    ...
    vol_period = 20
    vol_multiplier = (
        1.5  # 🌙 Moon Dev: Volume filter to confirm conviction in signals ✨
    )
    trail_activation = (
        2.0  # 🌙 Moon Dev: Activate trailing after 2x ATR profit to let winners run ✨
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Retained 200 EMA for trend filter ✨
        self.rsi = self.I(
            talib.RSI, self.data.Close, timeperiod=self.rsi_period
        )  # 🌙 Moon Dev: Added RSI for momentum confirmation to filter weak entries ✨
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=self.vol_period
        )  # 🌙 Moon Dev: Added volume SMA for liquidity/conviction filter ✨
        self.entry_price = 0
        self.entry_atr = 0
        self.trail_sl = 0
        self.trail_active = False
        print(
            "🌙 Moon Dev: Indicators initialized with RSI, Volume SMA, and trailing stop support for enhanced performance ✨"
        )

    def next(self):
        if (
            len(self.data) < 201
        ):  # 🌙 Moon Dev: Ensures all indicators (EMA200, RSI, Vol SMA) are ready ✨
            return
        ...
        vol_condition = current_vol > (
            self.vol_multiplier * self.vol_sma[-1]
        )  # 🌙 Moon Dev: Volume surge filter for higher-quality setups ✨
        ...
        long_signal = (
            ...
            and vol_condition
            and (self.rsi_long_min < current_rsi < self.rsi_long_max)
        )
        ...
        if long_signal and not self.position:
            ...
            if risk_per_unit > 0:
                ...
                )  # 🌙 Moon Dev: Switched to float sizing for precise risk control and better capital efficiency ✨
        ...
        short_signal = (
            ...
            and (current_close < current_ema)
            and vol_condition
            and (self.rsi_short_min < current_rsi < self.rsi_short_max)
        )
        ...
        if short_signal and not self.position:
            ...
            if risk_per_unit > 0:
                position_size = (
                    risk_amount / risk_per_unit
                )  # 🌙 Moon Dev: Float sizing for precision in shorts as well ✨
                sl_price = current_close + risk_per_unit
                tp_price = current_close - (self.tp_multiplier * current_atr)
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if not self.trail_active and profit > (
                    self.trail_activation * self.entry_atr
                ):
                ...
                if self.trail_active:
                    ...
                    if new_sl > self.trail_sl:
                        ...
                        print(
                            f"🌙 Moon Dev: Trailing SL updated for LONG to {self.trail_sl:.2f} 📈✨"
                        )
                ...
                if self.trail_active:
                    ...
                    if new_sl < self.trail_sl:
                        ...
                        self.position.sl = self.trail_sl
                        print(
                            f"🌙 Moon Dev: Trailing SL updated for SHORT to {self.trail_sl:.2f} 📉✨"
                        )
