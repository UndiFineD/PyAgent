# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v9.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    volume_ma_period = 20
    adx_period = (
        14  # 🌙 Moon Dev: ADX for trend strength filter to avoid choppy markets
    )
    ...
    rr_ratio = (
        5  # 🌙 Moon Dev: Increased from 4 to 5 for larger reward capture in BTC trends
    )
    ...
    trail_profit_threshold_rr = (
        2  # 🌙 Moon Dev: Start trailing after 2R profit to lock in gains
    )
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Initialized ADX

        print(
            "🌙 Moon Dev: Initialized Enhanced FibonacciCrossover with Trailing Stops, Looser Filters & Rising ADX for 50% Target 🚀"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (
                ...
                and hasattr(self, "sl_distance")
            ):
                profit_threshold = self.entry_price + (
                    self.sl_distance * self.trail_profit_threshold_rr
                )
                if current_close > profit_threshold:
                    if not np.isnan(current_atr) and current_atr > 0:
                        ...
                        if new_sl > self.sl:
                            ...
                            self.sl = new_sl
                            print(
                                f"🌙 Moon Dev: LONG Trailing SL updated from {old_sl:.2f} to {self.sl:.2f} (Close: {current_close:.2f}) 🔄"
                            )
            elif (
                ...
                if current_close < profit_threshold:
                    if not np.isnan(current_atr) and current_atr > 0:
                        ...
                        if new_sl < self.sl:
                            ...
                            self.sl = new_sl
                            print(
                                f"🌙 Moon Dev: SHORT Trailing SL updated from {old_sl:.2f} to {self.sl:.2f} (Close: {current_close:.2f}) 🔄"
                            )
            ...
            if hasattr(self, "sl") and hasattr(self, "tp"):
                if self.position.is_long:
                    if current_low <= self.sl:
                        ...
                        if hasattr(self, "sl_distance"):
                            del self.sl_distance
                        return
        ...
        if (
            ...
            > self.sma200[
                -1
            ]  # 🌙 Moon Dev: Retained EMA55 > SMA200 for uptrend confirmation
            and current_rsi
            > 50  # 🌙 Moon Dev: Loosened from >55 to >50 for more bullish momentum opportunities
            and current_close > self.sma200[-1]
            and current_volume
            > self.volume_multiplier
            * self.volume_ma[
                -1
            ]  # 🌙 Moon Dev: Loosened to 1.5x for increased trade frequency
            and current_adx > self.adx_threshold  # 🌙 Moon Dev: ADX trend filter
            and adx_rising  # 🌙 Moon Dev: Added rising ADX for better entry in accelerating trends
            and harmonic_long
        ):
            ...
            print(
                f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} | ADX: {current_adx:.2f} (Rising: {adx_rising}) 🚀 Trailing-Ready Enhanced Entry ✨"
            )
        ...
        # Short Entry - Symmetric enhancements: Loosened RSI to <50, volume to 1.5x, added rising ADX
            ...
            < self.sma200[
                -1
            ]  # 🌙 Moon Dev: Retained EMA55 < SMA200 for downtrend confirmation
            and current_rsi
            < 50  # 🌙 Moon Dev: Loosened from <45 to <50 for more bearish momentum opportunities
            and current_close < self.sma200[-1]
            and current_volume
            > self.volume_multiplier
            * self.volume_ma[
            ...
            self.sl_distance = sl_distance
            print(
                f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} | ADX: {current_adx:.2f} (Rising: {adx_rising}) 📉 Trailing-Ready Enhanced Entry ✨"
            )
