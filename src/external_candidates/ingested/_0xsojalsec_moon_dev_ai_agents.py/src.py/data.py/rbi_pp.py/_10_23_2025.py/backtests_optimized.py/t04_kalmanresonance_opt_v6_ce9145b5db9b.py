# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_OPT_v6.py
import warnings
...
def fib_support(
    high, low, period=20
):  # Shortened period to 20 for more dynamic Fib levels in 15m timeframe 🌙
...
class KalmanResonance(Strategy):
    ...
    max_bars_in_trade = (
        8  # Reduced from 10 to exit non-performers quicker, improving capital turnover
    )
    ...
    def next(self):
        ...
        equity = (
            self._broker._cash
        )  # Use broker cash for accurate equity in position sizing
        risk_per_trade = (
            0.02 * equity
        )  # Increased to 2% risk per trade to amplify returns while staying managed 🚀
        ...
        if is_ranging and self.adx[-1] < 25:
            vel_positive_accel = (
                self.velocity[-1] > 0 and self.velocity[-1] > self.velocity[-2]
            )  # Changed to positive and increasing for broader momentum capture ✨
            stoch_oversold_cross = (
                self.stoch_k[-2] <= self.stoch_d[-2]
                and self.stoch_k[-1] > self.stoch_d[-1]
                and self.stoch_k[-1] < 30
                and self.stoch_k[-2] < 30
            )  # Relaxed to 30 for more oversold opportunities
            rsi_oversold = (
                self.rsi[-1] < 35
            )  # Added RSI <35 for stronger oversold confirmation without over-filtering
            near_support = (
            ...
            if (
                ...
                if stop_distance > 0:
                    ...
                    size = (
                        risk_per_trade
                        / (self.data.Close[-1] * stop_pct)
                        / self.data.Close[-1]
                    )  # Corrected sizing: risk / (price * stop_pct) for shares, but normalized
