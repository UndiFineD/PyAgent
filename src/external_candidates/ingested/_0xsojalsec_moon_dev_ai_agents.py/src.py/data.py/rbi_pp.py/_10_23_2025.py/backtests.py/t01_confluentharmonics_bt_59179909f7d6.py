# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests\T01_ConfluentHarmonics_BT.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if not self.position and len(self.data) > 1:
            ...
            if (
                ...
                print(
                    f"🌙 Bullish ConfluentHarmonics Entry at {self.data.index[-1]} 🚀 Size: {size}, Entry: {entry}, SL: {sl}, TP: {tp}"
                )
            ...
            # Bearish Reversal Proxy: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze
                ...
                print(
                    f"🌙 Bearish ConfluentHarmonics Entry at {self.data.index[-1]} ✨ Size: {size}, Entry: {entry}, SL: {sl}, TP: {tp}"
                )
