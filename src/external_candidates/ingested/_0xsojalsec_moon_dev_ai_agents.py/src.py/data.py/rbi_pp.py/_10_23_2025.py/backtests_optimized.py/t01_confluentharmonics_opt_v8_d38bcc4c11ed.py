# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v8.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    rsi_period = 14
    rsi_oversold = (
        40  # 🌙 NEW: RSI threshold for oversold confirmation in bullish setups
    )
    rsi_overbought = (
        60  # 🌙 NEW: RSI threshold for overbought confirmation in bearish setups
    )
    ...
    def init(self):
        ...
        self.ema_trend = self.I(
            talib.EMA, self.data.Close, timeperiod=self.ema_trend_period
        )
    ...
    def next(self):
        ...
        vol_confirm = (
            vol_val > vol_ma_val
        )  # 🌙 NEW: Volume filter for stronger entry conviction
        ...
        if not self.position:
            ...
            if (
                ...
                and squeeze
                and rsi_val
                < self.rsi_oversold  # 🌙 NEW: RSI filter for oversold confirmation
                and vol_confirm  # 🌙 NEW: Volume filter
                and trend_long
            ...
            # Bearish Reversal Proxy: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze, RSI overbought, volume confirm, trend filter
                ...
                and squeeze
                and rsi_val
                > self.rsi_overbought  # 🌙 NEW: RSI filter for overbought confirmation
                and vol_confirm  # 🌙 NEW: Volume filter
                and trend_short
            ):  # 🌙 NEW: Trend alignment filter
                entry = price
