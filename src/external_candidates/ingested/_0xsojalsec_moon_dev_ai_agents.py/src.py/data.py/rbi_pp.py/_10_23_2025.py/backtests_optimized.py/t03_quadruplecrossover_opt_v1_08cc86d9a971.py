# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_QuadrupleCrossover_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class QuadrupleCrossover(Strategy):
    ...
    atr_sma_period = 20
    time_exit_bars = (
        20  # Increased to 20 bars to allow more room for trends to develop ✨
    )
    volatility_multiplier = (
        2.0  # Tightened to 2.0 for higher volatility filter to avoid choppy markets 🚀
    )
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev: Initialized Optimized QuadrupleCrossover Strategy with EMAs, Volume, ADX, and Enhanced Exits! ✨"
        )
    ...
    def next(self):
        ...
        if self.position.size != 0:
            ...
            if (
                self.entry_bar is not None
                and current_bar - self.entry_bar > self.time_exit_bars
            ):
                self.position.close()
                print(
                    f"⏰ Moon Dev: Time-based exit after {self.time_exit_bars} bars 🌙"
                )
                self.entry_bar = None
                self.breakeven_set = False
        ...
        if not self.position.size and volatility_ok:
            ...
            long_crossover = (
                self.ema1[-1] > self.ema2[-1] and self.ema1[-2] <= self.ema2[-2]
            )
            long_alignment = (
                self.ema1[-1] > self.ema2[-1] > self.ema3[-1] > self.ema4[-1]
            )
            long_rsi = (
                rsi_val > 55
            )  # Tightened from 50 to 55 for better momentum confirmation ✨
            ...
            if (
                ...
                if position_size > 0:
                    ...
                    self.breakeven_set = False
                    print(
                        f"🌙 Moon Dev: LONG ENTRY 🚀 Price: {entry_price}, Size: {position_size}, SL: {sl_price}, TP: {tp_price} ✨"
                    )
            ...
            short_rsi = (
                rsi_val < 45
            )  # Tightened from 50 to 45 for better momentum confirmation ✨
            ...
            if (
                ...
                if position_size > 0:
                    ...
                    self.breakeven_set = False
                    print(
                        f"🌙 Moon Dev: SHORT ENTRY 🚀 Price: {entry_price}, Size: {position_size}, SL: {sl_price}, TP: {tp_price} ✨"
                    )
...
stats = bt.run()
print(
    "🌙 Moon Dev: Optimized Backtest Completed! Targeting 50%+ returns with enhanced filters and RR! ✨"
)
print(stats)
