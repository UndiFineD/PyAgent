...
class ConfluentHarmonics(Strategy):
    ...
    vol_sma_period = (
        20  # 🌙 New: Added volume SMA filter to confirm momentum on entries
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (
                ...
                and squeeze
                and self.rsi[-2]
                < self.rsi_oversold  # 🌙 New: Confirm oversold at BB touch
                and price
                > self.sma_trend[-1]  # 🌙 New: Trend filter - only long in uptrend
            ...
            # 🌙 Optimized: Added RSI overbought filter, trend filter (below SMA200), and volume confirmation for better entry quality
                ...
                and squeeze
                and self.rsi[-2]
                > self.rsi_overbought  # 🌙 New: Confirm overbought at BB touch
                and price
                < self.sma_trend[-1]  # 🌙 New: Trend filter - only short in downtrend
                and self.data.Volume[-1] > self.vol_sma[-1]
            ):  # 🌙 New: Volume surge for momentum
                entry = price
                risk_dist = self.atr_sl_mult * atr_val
