# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T02_QuadrupleHarmonic_OPT_v4.py
import pandas as pd
import talib
...
class QuadrupleHarmonic(Strategy):
    def init(self):
        ...
        )  # 🌙 Added EMA50 for stronger uptrend confirmation in pullbacks
        self.adx = self.I(
        ...
        )  # 🌙 Kept RSI for oversold filter in pullbacks
        self.vol_sma = self.I(
            talib.SMA, self.data.Volume, timeperiod=20
        )  # 🌙 Kept volume SMA for confirmation filter
    ...
    def next(self):
        ...
        if len(self.data) % 100 == 0:
            print(
                f"🌙 Moon Dev Debug: Current Close {current_close:.2f}, EMA {self.ema[-1]:.2f}, EMA50 {self.ema50[-1]:.2f}, ADX {self.adx[-1]:.2f} ✨"
            )
            ...
            if self.entry_price and self.stop_distance:
                ...
                if profit > self.stop_distance:
                    ...
                    if trail_sl > self.position.sl:
                        ...
                        print(
                            f"🌙 Moon Dev Trail: Updated SL to {trail_sl:.2f} after 1R profit 🚀"
                        )
        ...
        )  # 🌙 Tightened from 0.5 to 0.6 for stronger confirmation
        ...
        if (
            ...
            and current_close > self.ema50[-1]
            and self.ema50[-1]
            > self.ema[
                -1
            ]  # 🌙 Enhanced uptrend filter: above EMA50 and EMA50 > EMA200 for quality trends
            and self.adx[-1]
            > 22  # 🌙 Slightly raised ADX from 20 to 22 for stronger trend quality
            and self.rsi[-1]
            < 45  # 🌙 Loosened RSI from <40 to <45 for more pullback signals without being too oversold
            and self.data.Volume[-1] > 1.2 * self.vol_sma[-1]
        ):  # 🌙 Loosened volume filter from 1.5x to 1.2x for more opportunities while keeping conviction

            print(
                f"🌙 Moon Dev Signal: 3 down bars pullback in strong uptrend, strong bullish confirmation at {current_close:.2f} 🚀"
            )
            ...
            )  # 🌙 Increased RR from 2 to 3 for better average wins
