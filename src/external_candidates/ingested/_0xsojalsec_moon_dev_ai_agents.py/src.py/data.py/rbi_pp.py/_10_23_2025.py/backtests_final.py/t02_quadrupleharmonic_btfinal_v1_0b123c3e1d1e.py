# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_final\T02_QuadrupleHarmonic_BTFinal_v1.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.entry_bar and len(self.data) - self.entry_bar > 10:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: Time-based close after 10 bars at {current_close:.2f} ✨"
                )
            return
        ...
        if (
            ...
            and current_close > current_open  # Bullish confirmation candle
            and current_close < self.ema[-1]  # Downtrend filter
            and self.adx[-1] > 25
        ):  # Trending market

            print(
                f"🌙 Moon Dev Signal: 4 down bars detected, bullish confirmation at {current_close:.2f} 🚀"
            )
            ...
            if tp_price <= entry_price or sl_price >= entry_price or xa_length <= 0:
                print(
                    f"🌙 Moon Dev Skip: Invalid TP/SL - TP {tp_price:.2f}, Entry {entry_price:.2f}, SL {sl_price:.2f} 😔"
                )
                return
            ...
            if size > 0:
                ...
                print(
                    f"🌙 Moon Dev Debug: Order params - Size {size}, SL {sl_price:.5f}, TP {tp_price:.5f}, Entry/Close {entry_price:.5f} ✨"
                )
                self.buy(size=size, sl=sl_price, tp=tp_price)
                self.entry_bar = len(self.data)
                print(
                    f"🌙 Moon Dev Entry: Long {size} units at {entry_price:.2f}, SL {sl_price:.2f}, TP {tp_price:.2f} (XA High {swing_high:.2f}, Min Low {min_low:.2f}) 🚀✨"
                )
