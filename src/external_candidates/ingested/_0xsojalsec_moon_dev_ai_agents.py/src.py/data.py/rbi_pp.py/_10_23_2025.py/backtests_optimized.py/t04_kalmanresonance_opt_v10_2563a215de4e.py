# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_OPT_v10.py
import warnings
...
class KalmanResonance(Strategy):
    ...
    stoch_overbought = (
        70  # Loosened from 80 to 70 for more overbought entry opportunities ✨
    )
    atr_period = 20
    ema_period = 200
    fib_period = 50
    ...
    def next(self):
        ...
        if is_ranging and self.adx[-1] < 25:
            ...
            stoch_oversold_cross = (
                ...
                and self.stoch_k[-1] < self.stoch_oversold
                and self.stoch_k[-2] < self.stoch_oversold
            )  # Loosened to 30 for more signals
            near_support = (
            ...
            if vel_cross_up and stoch_oversold_cross and near_support and vol_confirm:
                ...
                if stop_distance > 0:
                    ...
                    size = min(size, 0.95)
                    tp = (
                        self.data.Close[-1] + self.rr_mult * stop_distance
                    )  # Increased to 1:3 RR for higher reward potential
        ...
        if is_ranging and self.adx[-1] < 25:
            ...
            stoch_overbought_cross = (
                self.stoch_k[-2] >= self.stoch_d[-2]
                and self.stoch_k[-1] < self.stoch_d[-1]
                and self.stoch_k[-1] > self.stoch_overbought
                and self.stoch_k[-2] > self.stoch_overbought
            )  # Loosened to 70 for more signals
            near_resistance = (
