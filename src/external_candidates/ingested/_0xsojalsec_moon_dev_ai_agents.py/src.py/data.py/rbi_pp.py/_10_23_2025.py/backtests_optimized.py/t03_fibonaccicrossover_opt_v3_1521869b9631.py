# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    adx_threshold = (
        20  # 🌙 Moon Dev: ADX > 20 ensures sufficient trend strength for entries
    )
    sma200_period = 200
    volume_ma_period = 20
    ...
    trail_trigger_mult = (
        1.5  # 🌙 Moon Dev: Multiplier of initial ATR to trigger trailing stop
    )
    ...
    def init(self):
        ...
        self.plus_di = self.I(
            talib.PLUS_DI, high, low, close, timeperiod=self.adx_period
        )
        self.minus_di = self.I(
            talib.MINUS_DI, high, low, close, timeperiod=self.adx_period
        )

        print(
            "🌙 Moon Dev: Initialized Super Optimized FibonacciCrossover with ADX, DI, Directional Confluence & Trailing Stops ✨"
        )
    ...
    def next(self):
            ...
            if hasattr(self, "sl") and hasattr(self, "tp"):
                ...
                if (
                    ...
                    if unrealized_profit > trail_trigger:
                        new_sl = current_close - (
                            self.initial_atr * self.trail_offset_mult
                        )
        ...
        harmonic_short = current_close <= current_ema21 * (
            1 + self.confluence_threshold
        )
        ...
        if (
            ...
            and current_rsi
            > 45  # 🌙 Moon Dev: Loosened RSI from >50 to >45 to capture more bullish momentum setups
            and current_close > self.sma200[-1]
            and current_volume
            > 1.2
            * self.volume_ma[
                -1
            ]  # 🌙 Moon Dev: Loosened volume filter from 1.5x to 1.2x MA for more trade opportunities in active markets
            and self.adx[-1] > self.adx_threshold
            and self.plus_di[-1]
            > self.minus_di[
                -1
            ]  # 🌙 Moon Dev: Added ADX >20 and +DI > -DI for strong uptrend confirmation
            and harmonic_long
        ):
            ...
            print(
                f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} 🚀 Enhanced Trend & Confluence Confirmed with Trailing Potential ✨"
            )
        ...
        # Short Entry - Enhanced with ADX/DI trend confirmation, loosened RSI (<55), volume (1.2x MA), directional confluence
            ...
            and self.ema13[-1] < self.ema21[-1]
            and current_rsi
            < 55  # 🌙 Moon Dev: Loosened RSI from <50 to <55 to capture more bearish momentum setups
            and current_close < self.sma200[-1]
            and current_volume
            > 1.2
            * self.volume_ma[
            ...
            > self.plus_di[
                -1
            ]  # 🌙 Moon Dev: Added ADX >20 and -DI > +DI for strong downtrend confirmation
            and harmonic_short
        ):
            ...
            self.initial_atr = current_atr
            print(
                f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} 📉 Enhanced Trend & Confluence Confirmed with Trailing Potential ✨"
            )
