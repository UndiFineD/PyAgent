# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_final\T03_QuadrupleCrossover_BTFinal_v1.py
import numpy as np
import pandas as pd
import talib
...
class QuadrupleCrossover(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            if self.position.is_long and current_price < self.sma3[-1]:
                self.position.close()
                self.entry_bar = None
                self.breakeven_set = False
                print(f"🚀 Moon Dev: Long trailing exit on SMA3 at {current_price} 🌙")
            elif self.position.is_short and current_price > self.sma3[-1]:
                self.position.close()
                self.entry_bar = None
                self.breakeven_set = False
                print(f"🚀 Moon Dev: Short trailing exit on SMA3 at {current_price} 🌙")
        ...
        if not self.position and volatility_ok:
            # Long entry
            long_crossover = (
                self.sma1[-1] > self.sma2[-1] and self.sma1[-2] <= self.sma2[-2]
            )
            long_alignment = (
                self.sma1[-1] > self.sma2[-1] > self.sma3[-1] > self.sma4[-1]
            )
            long_rsi = rsi_val > 50
...
stats = bt.run()
print("🌙 Moon Dev: Backtest Completed! Here's the stats: ✨")
print(stats)
