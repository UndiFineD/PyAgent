# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_BEST_-0.724755pct.py
import warnings
...
class KalmanResonance(Strategy):
    ...
    ema_period = 200
    fib_period = 50
    prz_tolerance = 0.003  # Tightened to 0.3% for better precision near Fib levels 🌙
    ranging_threshold = (
        0.05  # Tightened to 5% around EMA for stricter ranging detection ✨
    )
    max_bars_in_trade = 10  # Reduced from 15 to exit faster in non-trending moves 🚀
    trail_atr_mult = 2.0  # Loosened to 2.0x ATR for better profit letting in trails
    ...
    def next(self):
        ...
        # Enhanced Long Entry: Require velocity cross up (removed weak decel), Stoch <20, near support 0.3%, ADX<25, volume > avg
        if is_ranging and self.adx[-1] < 25:
            ...
            stoch_oversold_cross = (
                ...
                and self.stoch_k[-2] < 20
            )  # Tightened to 20 for deeper oversold
            near_support = (
        ...
        # Enhanced Short Entry: Similar strict conditions for shorts
        if is_ranging and self.adx[-1] < 25:
            vel_cross_down = (
                self.velocity[-2] > 0 and self.velocity[-1] < 0
            ...
            stoch_overbought_cross = (
                ...
                and self.stoch_k[-2] > 80
            )  # Tightened to 80 for deeper overbought
            near_resistance = (
