# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v4.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: Added ADX indicator for market regime filtering to trade only in strong trends
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long:
                ...
                if (
                    not hasattr(self, "partial_closed")
                    and hasattr(self, "entry_price")
                    and hasattr(self, "initial_risk")
                    and hasattr(self, "initial_size")
                ):
                    if current_high >= self.entry_price + 2 * self.initial_risk:
                        ...
                        print(
                            f"🌙 Moon Dev: Partial LONG close at 2R (high: {current_high}), size {half_size} 🎂 Locking profits!"
                        )
                ...
                if hasattr(self, "entry_price") and hasattr(self, "initial_risk"):
                    ...
                    if profit > self.initial_risk and self.sl < self.entry_price:
                        ...
                        print(
                            f"🌙 Moon Dev: Moved LONG SL to breakeven from {old_sl} at 1R profit 🔒"
                        )
                ...
                if (
                    ...
                    if current_low <= self.entry_price - 2 * self.initial_risk:
                        ...
                        self.partial_closed = True
                        print(
                            f"🌙 Moon Dev: Partial SHORT close at 2R (low: {current_low}), size {half_size} 🎂 Locking profits!"
                        )
                ...
                if hasattr(self, "entry_price") and hasattr(self, "initial_risk"):
                    ...
                    if profit > self.initial_risk and self.sl > self.entry_price:
                        ...
                        self.sl = self.entry_price
                        print(
                            f"🌙 Moon Dev: Moved SHORT SL to breakeven from {old_sl} at 1R profit 🔒"
                        )
        ...
        if (
            ...
            > self.sma200[
                -1
            ]  # 🌙 Moon Dev: Added EMA55 > SMA200 filter for stronger long-term uptrend confirmation, filtering out counter-trend trades
            and current_rsi
            > 50  # 🌙 Moon Dev: Tightened RSI from >40 to >50 to avoid weaker bullish signals
            ...
            * self.volume_ma[
            ...
            and self.adx[-1]
            > self.adx_threshold  # 🌙 Moon Dev: Added ADX > 25 filter to ensure strong trend, avoiding ranging markets for higher win rate and returns
            and harmonic_long
        ):
            ...
            )  # 🌙 Moon Dev: Use float for precise position sizing without rounding loss
            if position_size <= 0:
            ...
            )  # 🌙 Moon Dev: Float max size for precision
        ...
        # Short Entry - Optimized with tighter RSI (<50), EMA13 < EMA21 filter for stronger downtrend, higher volume threshold (1.5x MA), ADX trend filter, and EMA55 < SMA200 for long-term alignment
            ...
            < self.sma200[
                -1
            ]  # 🌙 Moon Dev: Added EMA55 < SMA200 filter for stronger long-term downtrend confirmation, filtering out counter-trend trades
            and current_rsi
            < 50  # 🌙 Moon Dev: Tightened RSI from <60 to <50 to avoid weaker bearish signals
            and current_close < self.sma200[-1]
            and current_volume
            > 1.5
            * self.volume_ma[
