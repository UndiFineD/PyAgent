# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v7.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    def init(self):
        ...
        print(
            "🌙 Moon Dev: Initialized Further Optimized FibonacciCrossover Strategy with Trailing Stops and Loosened Filters for Higher Returns ✨"
        )
    ...
    def next(self):
            ...
            if (
                ...
                if self.position.is_long:
                    ...
                    if favorable_move > self.trailing_atr_multiplier * current_atr:
                        ...
                        self.sl = new_sl
                        print(
                            f"🌙 Moon Dev: Trailing SL updated for LONG to {self.sl} (favorable move: {favorable_move:.2f}) 🔒"
                        )
                    ...
                    if favorable_move > self.trailing_atr_multiplier * current_atr:
                        ...
                        self.sl = new_sl
                        print(
                            f"🌙 Moon Dev: Trailing SL updated for SHORT to {self.sl} (favorable move: {favorable_move:.2f}) 🔒"
                        )
        ...
        if (
            ...
            and current_rsi
            > 50  # 🌙 Moon Dev: Loosened RSI from >55 to >50 for broader bullish momentum capture, increasing trade opportunities
            and current_close > self.sma200[-1]
            and current_volume
            > 1.5
            * self.volume_ma[
                -1
            ]  # 🌙 Moon Dev: Reduced volume filter from 2x to 1.5x MA to include more conviction entries without sacrificing quality
            and current_adx
            > self.adx_threshold  # 🌙 Moon Dev: Retained ADX filter with lowered threshold for more trending setups
            and harmonic_long
        ):
            ...
            )  # 🌙 Moon Dev: Changed to float for fractional sizing, allowing precise risk management in BTC trades
            ...
            self.tp = tp_price
            self.entry_price = (
                current_close  # 🌙 Moon Dev: Track entry price for trailing stop logic
            )
            print(
                f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size:.4f} | ADX: {current_adx:.2f} 🚀 Optimized Entry with Trailing Potential Confirmed ✨"
            )
        ...
        # Short Entry - Retained core conditions but loosened RSI (<50 from <45), volume (1.5x from 2x), and ADX (20 from 25) for more frequent high-quality entries
            ...
            and current_rsi
            < 50  # 🌙 Moon Dev: Loosened RSI from <45 to <50 for broader bearish momentum capture, increasing trade opportunities
            and current_close < self.sma200[-1]
            and current_volume
            > 1.5
            * self.volume_ma[
            ...
            print(
                f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size:.4f} | ADX: {current_adx:.2f} 📉 Optimized Entry with Trailing Potential Confirmed ✨"
            )
