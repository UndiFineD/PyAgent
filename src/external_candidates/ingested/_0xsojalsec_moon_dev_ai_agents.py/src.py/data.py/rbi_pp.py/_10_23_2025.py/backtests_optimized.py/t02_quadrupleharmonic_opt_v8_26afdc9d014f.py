# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T02_QuadrupleHarmonic_OPT_v8.py
import pandas as pd
import talib
...
class QuadrupleHarmonic(Strategy):
    def init(self):
        ...
        self.ema_short = self.I(
            talib.EMA, self.data.Close, timeperiod=20
        )  # 🌙 NEW: Short EMA for pullback confirmation (close above EMA20 after pullback)
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.entry_price and current_close > self.entry_price + (
                self.entry_price - self.position.sl
            ):
                # Update highest
                if (
                    self.highest_since_entry is None
                    or current_high > self.highest_since_entry
                ):
                ...
                if trail_sl > self.position.sl:
                    ...
                    print(
                        f"🌙 Moon Dev Trail: Updated SL to {trail_sl:.2f} at high {self.highest_since_entry:.2f} 🚀"
                    )
            ...
            if self.entry_bar and len(self.data) - self.entry_bar > 30:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: Time-based close after 30 bars at {current_close:.2f} ✨"
                )
            return
        ...
        )  # 🌙 TIGHTENED: Increased to >60% body for stronger reversal conviction
        ...
        prev_lows_decreasing = (
            len(self.data) >= 5
            and self.data.Low[-4] < self.data.Low[-5]
            and self.data.Low[-3] < self.data.Low[-4]
            and self.data.Low[-2] < self.data.Low[-3]
        )
        prev_closes_decreasing = (
            self.data.Close[-5] < self.data.Open[-5]
        ...
        if (
            prev_closes_decreasing
            and prev_lows_decreasing
            and strong_bullish  # Bullish confirmation candle with strength filter
            and current_close > self.ema[-1]  # Uptrend filter
            and current_close
            > self.ema_short[
                -1
            ]  # 🌙 NEW: Above short EMA for pullback end confirmation
            and self.adx[-1]
            > 22  # 🌙 ADJUSTED: Slightly raised from 20 to 22 for stronger trends, balancing signal quality
            and self.rsi[-1]
            < 35  # 🌙 TIGHTENED: Lowered from 40 to 35 for deeper, higher-quality oversold pullbacks
            and self.data.Volume[-1]
            > 2.0
            * self.vol_sma[
                -1
            ]  # 🌙 TIGHTENED: Increased volume multiplier to 2x for higher conviction reversals
            and valid_depth
        ):  # 🌙 NEW: Depth filter for optimal pullback size

            print(
                f"🌙 Moon Dev Signal: 4 down bars (lower lows/closes) pullback in uptrend, strong bullish at {current_close:.2f}, depth {pullback_depth:.2%} 🚀"
            )
            ...
            size = min(
                size_frac, 0.4
            )  # 🌙 TIGHTENED: Cap at 40% from 50% for conservative exposure in volatile markets
