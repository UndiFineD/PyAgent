# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v6.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    def next(self):
        ...
        squeeze = (
            bb_width < self.bb_squeeze_threshold
            or prev_bb_width < self.bb_squeeze_threshold
        )
        ...
        if self.position:
            if self.position.is_long:
                if (
                    adx_val < 18 or rsi_val > 70
                ):  # Exit if overbought or ADX weakens more aggressively
                    self.position.close()
                    print(
                        f"🌙 Early Long Exit: ADX/RSI Signal at {self.data.index[-1]} 🚀 Price: {price}"
                    )
                # 🌙 NEW: Trailing stop logic for longs
                elif self.position.pl_pnl > (
                    self.atr_sl_mult * atr_val * self.position.size
                ):  # After 1:1 RR approx
                    trail_sl = price - (self.trail_atr_mult * atr_val)
                    if (
                        hasattr(self, "current_trail_sl_long")
                        and trail_sl > self.current_trail_sl_long
                    ):
                    ...
                    if hasattr(self, "current_trail_sl_long"):
                        self.position.sl = max(
                            self.position.sl, self.current_trail_sl_long
                        )
            elif self.position.is_short:
                if adx_val < 18 or rsi_val < 30:  # Exit if oversold or ADX weakens
                    self.position.close()
                    print(
                        f"🌙 Early Short Exit: ADX/RSI Signal at {self.data.index[-1]} ✨ Price: {price}"
                    )
                # 🌙 NEW: Trailing stop logic for shorts
                elif self.position.pl_pnl < -(
                    self.atr_sl_mult * atr_val * self.position.size
                ):  # After 1:1 RR approx (negative for short pnl)
                    trail_sl = price + (self.trail_atr_mult * atr_val)
                    if (
                        hasattr(self, "current_trail_sl_short")
                        and trail_sl < self.current_trail_sl_short
                    ):
                        self.current_trail_sl_short = trail_sl
                    elif not hasattr(self, "current_trail_sl_short"):
                        self.current_trail_sl_short = trail_sl
                    if hasattr(self, "current_trail_sl_short"):
                        self.position.sl = min(
                            self.position.sl, self.current_trail_sl_short
                        )
        ...
        if not self.position:
            ...
            if (
                ...
                and pdi_val > mdi_val
                and squeeze
                and rsi_val < self.rsi_oversold + 5  # <35
                and price > sma_trend  # Trend filter
                and volume_confirm
            ):  # Volume filter
                entry = price
                ...
                atr_sma = self.I(talib.SMA, self.atr, timeperiod=20)[
                    -1
                ]  # But since init, need to compute in next? Wait, add in init
                ...
                print(
                    f"🌙 Bullish ConfluentHarmonics Entry at {self.data.index[-1]} 🚀 Size Frac: {size_frac}, Entry: {entry}, SL: {sl}, TP: {tp}, RSI: {rsi_val}"
                )
            ...
            # 🌙 OPTIMIZATION: Tightened bearish entry - similar additions for symmetry
                ...
                and mdi_val > pdi_val
                and squeeze
                and rsi_val > self.rsi_overbought - 5  # >65
                and price < sma_trend  # Trend filter
                and volume_confirm
            ):
                entry = price
                ...
                print(
                    f"🌙 Bearish ConfluentHarmonics Entry at {self.data.index[-1]} ✨ Size Frac: {size_frac}, Entry: {entry}, SL: {sl}, TP: {tp}, RSI: {rsi_val}"
                )
