# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T02_QuadrupleHarmonic_OPT_v7.py
import pandas as pd
import talib
...
class QuadrupleHarmonic(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.entry_price is not None and self.stop_distance is not None:
                ...
                if current_close > profit_threshold:
                    trail_distance = (
                        1.5 * self.atr[-1]
                    )  # Trail by 1.5 ATR for balanced risk
                    new_trailing_sl = current_close - trail_distance
                    if new_trailing_sl > self.position.sl:
                        self.position.sl = new_trailing_sl
                        print(
                            f"🌙 Moon Dev Trailing: SL updated to {new_trailing_sl:.2f} at close {current_close:.2f} 🚀"
                        )
        ...
        if (
            ...
            and strong_bullish  # Bullish confirmation candle with tightened strength filter
            and current_close
            > self.ema[
                -1
            ]  # 🌙 Changed to uptrend filter (from below to above EMA) for pullback buys in trending up markets
            and ema_uptrend  # 🌙 New: EMA must be in short-term uptrend
            and self.adx[-1]
            > 25  # 🌙 Tightened ADX threshold from 20 to 25 for stronger trends, fewer false signals
            and self.rsi[-1]
            < 35  # 🌙 Tightened RSI oversold filter from 40 to 35 for deeper, higher-probability pullbacks
            and self.data.Volume[-1] > 2.0 * self.vol_sma[-1]
        ):  # 🌙 Tightened volume filter to >2x average for stronger conviction
