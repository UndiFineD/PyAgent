# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T02_QuadrupleHarmonic_OPT_v6.py
import pandas as pd
import talib
    ...
    def next(self):
        ...
        if self.position:
            if current_close < self.ema[-1]:
                self.position.close()
                print(
                    f"🌙 Moon Dev Exit: Early close below EMA at {current_close:.2f} 🚀"
                )
            ...
            if self.entry_price is not None and self.initial_sl is not None:
                ...
                if current_high > trail_trigger:
                    ...
                    if new_sl > self.position.sl:
                        ...
                        print(
                            f"🌙 Moon Dev Trail: SL trailed to breakeven at {new_sl:.2f} 🚀"
                        )
        ...
        strong_bullish = (
            body > 0.5 * candle_range and body > 0
        )  # 🌙 Tightened entry: require strong bullish candle body > 50% of range
        ...
        if (
            ...
            and self.adx[-1]
            > 25  # 🌙 Tightened ADX threshold from 20 to 25 for stronger trend quality to improve win rate
            and self.rsi[-1]
            < 35  # 🌙 Tightened RSI oversold filter from 40 to 35 for deeper, higher-quality pullbacks
            and self.data.Volume[-1] > 2.0 * self.vol_sma[-1]
        ):  # 🌙 Tightened volume filter from 1.5x to 2x for stronger conviction on entries
            ...
            size = min(
                size_frac, 0.3
            )  # 🌙 Reduced cap from 0.5 to 0.3 for better risk control in volatile markets
