# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_OPT_v5.py
import warnings
...
def kalman_velocity_position(
    close, q=0.05, r=1.0
):  # Reduced q for smoother velocity estimation to reduce noise in signals 🌙
...
def fib_support(
    high, low, period=21
):  # Shortened fib period to 21 (Fibonacci number) for more responsive levels on 15m timeframe ✨
...
class KalmanResonance(Strategy):
    kalman_q = (
        0.05  # Smoother Kalman for better velocity crosses without excessive noise 🌙
    )
    kalman_r = 1.0
    stoch_k_period = 14
    stoch_d_period = 3
    ...
    ema_period = 200
    fib_period = (
        21  # Optimized shorter period for faster adaptation to recent swings ✨
    )
    ...
    rr_ratio = (
        2.5  # Improved RR to 1:2.5 for higher reward potential on winning trades 🌙
    )
    ...
    def init(self):
        ...
        self.stop_distance = (
            0.0  # Added to track stop distance for conditional trailing logic ✨
        )
    ...
    def next(self):
        ...
        equity = (
            self._broker._cash + self._broker._value
        )  # Use internal equity for accurate risk calc (fixed potential issue) 🌙
        ...
        if self.position:
            ...
            if is_long:
                ...
                if (
                    current_profit > self.stop_distance
                ):  # Start trailing only after breaking even + 1x risk
                    new_trail = self.data.Close[-1] - self.trail_atr_mult * self.atr[-1]
                    self.trail_stop = max(self.trail_stop, new_trail)
                else:
                    self.trail_stop = (
                        self.sl_price
                    )  # Hold at initial SL until profit threshold
        ...
        if (
        ...
        ):  # Stricter ADX for purer ranging to avoid weak trends ✨
            ...
            stoch_oversold_cross = (
                ...
                and self.stoch_k[-1] < 15
                and self.stoch_k[-2] < 15
            )  # Tightened to 15 for deeper oversold confirmation 🌙
            near_support = (
                abs(self.data.Close[-1] - self.fib_support[-1]) / self.data.Close[-1]
                < self.prz_tolerance
            )
            vol_confirm = (
                self.data.Volume[-1] > 1.2 * self.avg_vol[-1]
            )  # Stricter volume for stronger conviction ✨
            rsi_oversold = (
                self.rsi[-1] < 25
            )  # Added RSI filter for better oversold alignment 🚀
            bullish_candle = (
                self.data.Close[-1] > self.data.Open[-1]
            )  # Added candle direction for entry confirmation 🌙
            ...
            if (
                ...
                if stop_distance > 0:
                    ...
                    size = min(size, 0.95)
                    tp = (
                        self.data.Close[-1] + self.rr_ratio * stop_distance
                    )  # Updated to dynamic RR for higher targets ✨
        ...
        if is_ranging and self.adx[-1] < 20:
            ...
            stoch_overbought_cross = (
                self.stoch_k[-2] >= self.stoch_d[-2]
                and self.stoch_k[-1] < self.stoch_d[-1]
                and self.stoch_k[-1] > 85
                and self.stoch_k[-2] > 85
            )  # Tightened to 85 for deeper overbought 🌙
            near_resistance = (
            ...
            rsi_overbought = (
                self.rsi[-1] > 75
            )  # Added RSI filter for overbought alignment ✨
            bearish_candle = (
                self.data.Close[-1] < self.data.Open[-1]
            )  # Added candle direction confirmation 🚀
