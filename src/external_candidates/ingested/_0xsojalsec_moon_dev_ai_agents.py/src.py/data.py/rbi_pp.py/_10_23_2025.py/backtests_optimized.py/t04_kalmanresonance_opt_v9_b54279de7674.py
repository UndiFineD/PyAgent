# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_OPT_v9.py
import warnings
...
class KalmanResonance(Strategy):
    kalman_q = (
        0.05  # Smoothed Kalman for better velocity detection in ranging markets 🌙
    )
    kalman_r = 1.0
    stoch_k_period = 14
    stoch_d_period = 3
    ...
    atr_period = (
        14  # Standard ATR period for more responsive volatility on 15m timeframe 🚀
    )
    ema_period = 200
    ...
    adx_threshold = (
        22  # Tightened ADX for stricter ranging confirmation (lower trend strength) 🌙
    )
    vol_mult = (
        1.5  # Require stronger volume spike for momentum confirmation on entries ✨
    )
    rsi_oversold = 35
    rsi_overbought = 65
    ...
    trail_atr_mult = (
        2.5  # Loosened trailing to let profits run further towards higher RR goals
    )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if is_long:
                ...
                if (
                    not self.be_moved
                    and self.data.Close[-1] >= self.entry_price + self.initial_risk
                ):
                    self.sl_price = self.entry_price
                    self.be_moved = True
                    print(
                        f"🌙 Moon Dev Moved SL to Breakeven Long at {self.data.Close[-1]:.2f} 🚀"
                    )
                ...
                if self.data.Low[-1] <= effective_stop:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev SL/Trail Exit Long at {self.data.Close[-1]:.2f} (stop {effective_stop:.2f}) 🚀"
                    )
                ...
                if (
                    ...
                    self.be_moved = True
                    print(
                        f"🌙 Moon Dev Moved SL to Breakeven Short at {self.data.Close[-1]:.2f} 🚀"
                    )
                # Update trailing stop
                new_trail = self.data.Close[-1] + self.trail_atr_mult * self.atr[-1]
                ...
                if self.data.High[-1] >= effective_stop:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev SL/Trail Exit Short at {self.data.Close[-1]:.2f} (stop {effective_stop:.2f}) 🚀"
                    )
                    self.reset_trade_vars()
                    return
                # Check TP hit
        ...
        if is_ranging and self.adx[-1] < self.adx_threshold:
            ...
            stoch_oversold_cross = (
                ...
                and self.stoch_k[-1] < self.stoch_oversold_level
                and self.stoch_k[-2] < self.stoch_oversold_level
            )
            near_support = (
            ...
            vol_confirm = (
                self.data.Volume[-1] > self.vol_mult * self.avg_vol[-1]
            )  # Stronger volume requirement
            rsi_confirm = (
                self.rsi[-1] < self.rsi_oversold
            )  # Complementary RSI oversold filter
            ...
            if (
                ...
                if stop_distance > 0:
                    ...
                    )  # Fixed: Proper equity-based position sizing for 1% risk 🚀
                    size = min(size, (self.equity / self.data.Close[-1]) * 0.95)
                    tp = (
                        self.data.Close[-1] + 3 * stop_distance
                    )  # Improved to 1:3 RR for higher returns ✨
        ...
        if is_ranging and self.adx[-1] < self.adx_threshold:
            ...
            stoch_overbought_cross = (
                self.stoch_k[-2] >= self.stoch_d[-2]
                and self.stoch_k[-1] < self.stoch_d[-1]
                and self.stoch_k[-1] > self.stoch_overbought_level
                and self.stoch_k[-2] > self.stoch_overbought_level
            )
            near_resistance = (
