...
def fib_support(
    high, low, period=30
):  # 🌙 Shortened period for more responsive Fibonacci levels in 15m timeframe
...
class KalmanResonance(Strategy):
    ...
    max_bars_in_trade = (
        20  # 🌙 Increased: Allow more time for trades to develop in 15m BTC volatility
    )
    ...
    def next(self):
        ...
        if (
            ...
            or np.isnan(self.vol_ma[-1])
        ):
            return

        equity = (
            self._broker._cash + self.position.pl
        )  # 🌙 Use accurate equity for dynamic risk
        risk_per_trade = (
            0.015 * equity
        ...
        if is_ranging:
            ...
            vel_turn_up = (
                self.velocity[-1] > self.velocity[-2] and self.velocity[-1] > 0
            )
            ...
            if vel_turn_up and stoch_oversold_cross and near_support and high_volume:
                ...
                if stop_distance > 0:
                    ...
                    size = min(size, 0.95)
                    tp = (
                        self.data.Close[-1] + 2 * stop_distance
                    )  # Maintain 1:2 RR for positive expectancy
