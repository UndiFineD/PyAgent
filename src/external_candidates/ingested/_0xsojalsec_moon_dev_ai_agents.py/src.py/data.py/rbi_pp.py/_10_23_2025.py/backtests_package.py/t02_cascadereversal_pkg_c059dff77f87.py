# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_package\T02_CascadeReversal_PKG.py
import warnings
    ...
    def next(self):
        ...
        if current_close < prev_close:
            ...
            if self.down_streak == 1:
                ...
                print(
                    "🌙 Moon Dev: Four consecutive down bars detected! Checking filters... 🔍"
                )
        else:
            self.down_streak = 0
        ...
        if self.position and self.fourth_close is not None:
            if current_close < self.fourth_close:
                self.sell()
                print(
                    f"🌙 Moon Dev: Pattern invalidated - fifth bar closed down at {current_close}! Exiting. ❌"
                )
        ...
        if self.position and hasattr(self, "post_down_streak"):
            if current_close < prev_close:
                ...
                if self.post_down_streak >= 2:
                    self.sell()
                    print(
                        "🌙 Moon Dev: New down streak post-entry (2+ bars)! Exiting. ⚠️"
                    )
        ...
        if (
            self.position
            and current_close < self.sma[-1]
            and prev_close >= self.sma[-2]
        ):
            self.sell()
            print(
                f"🌙 Moon Dev: Price crossed below SMA({self.sma_period}) at {current_close}! Trailing exit. 📉"
            )

        # Time-based exit
        if (
            self.position
            and hasattr(self, "entry_bar")
            and len(self.data) - self.entry_bar >= self.max_bars_in_trade
        ):
            self.sell()
            print(
                f"🌙 Moon Dev: Time-based exit after {self.max_bars_in_trade} bars. ⏰"
            )
            del self.entry_bar
        ...
        if self.down_streak == 4 and not self.position:
            ...
            if len(self.data) >= 4:
                avg_prior_vol = (
                    self.data.Volume[-4] + self.data.Volume[-3] + self.data.Volume[-2]
                ) / 3
                vol_filter = self.data.Volume[-1] < avg_prior_vol
                if not vol_filter:
                    print(
                        "🌙 Moon Dev: Volume filter failed - fourth bar volume too high! 🚫"
                    )
                    return
                print(
                    "🌙 Moon Dev: Volume filter passed - waning seller interest. 📊✅"
                )
            else:
                vol_filter = True
            ...
            if len(self.data) >= 4:
                ...
                if not body_filter:
                    print(
                        f"🌙 Moon Dev: Body filter failed - bodies not progressively smaller (b1:{body1:.2f}, b2:{body2:.2f}, b3:{body3:.2f}, b4:{body4:.2f})! 🚫"
                    )
                    return
            ...
            if len(self.data) >= 5:
                ...
                if not trend_filter:
                    print(
                        f"🌙 Moon Dev: Trend filter failed - pre-streak close {pre_streak_close} <= SMA {pre_streak_sma}! 🚫"
                    )
                    return
                print(
                    f"🌙 Moon Dev: Trend filter passed - bullish bias pre-streak! 📈✅"
                )
            else:
                trend_filter = True
            ...
            if vol_filter and body_filter and trend_filter:
                ...
                print(
                    f"🌙 Moon Dev: Entering LONG CascadeReversal! Entry ~{approx_entry}, SL {sl_price:.2f}, TP {tp_price:.2f}, Size {size}, Risk {risk_amount:.2f} USD 🚀💰"
                )
