# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T00_SynergisticOscillator_OPT_v3.py
import numpy as np
import pandas as pd
import talib
...
class SynergisticOscillator(Strategy):
    ...
    atr_multiplier = (
        2.0  # 🌙 Widened initial stop to 2x ATR for better risk-reward ratio
    )
    partialed = False
    pos_bars = 0
    ...
    def init(self):
        ...
        )  # 🌙 Shortened SMA to 20 periods for faster trend response on 15m timeframe
        self.stoch_k, self.stoch_d = self.I(
            ...
            fastk_period=5,
            fastd_period=3,
        )  # 🌙 Faster StochRSI with fastk=5 for quicker signal detection
        self.atr = self.I(
        ...
        )  # 🌙 Added ADX for trend strength filter to avoid choppy markets
    ...
    def next(self):
        ...
        if self.position:
            if (
                self.pos_bars >= 20
            ):  # 🌙 Extended time-based exit to 20 bars to allow more room for profitable trades
                self.position.close()
            ...
            if self.position.is_long:
                ...
                if not self.partialed:
                    ...
                    if profit_dist >= 1.5 * atr_val[-1]:
                        ...
                        )  # 🌙 Use fractional sizing for precision
                        if size_to_close > 0:
        ...
        if not self.position:
            ...
            )  # 🌙 Unified and tightened volume filter: require increasing volume for both directions for momentum confirmation
            trend_strength = (
                adx_val[-1] > 25
            )  # 🌙 Added ADX filter to only trade in trending markets, avoiding ranging conditions
            # Long entry
            cross_up = k[-1] > d[-1] and k[-2] <= d[-2]
            oversold = (
                k[-1] < 30 and d[-1] < 30
            )  # 🌙 Relaxed oversold to 30 for more quality setups without too many false signals
            ...
            if cross_up and oversold and uptrend and vol_ok and trend_strength:
                ...
                )  # 🌙 Use exact float sizing for precise risk management
                if size > 0:
                    self.buy(size=size, sl=entry_price - stop_dist)
                    print(
                        f"🚀 Long entry at {entry_price:.2f}, size {size}, SL {entry_price - stop_dist:.2f}, ATR {atr_val[-1]:.2f} 🌙"
                    )
                    self.partialed = False
                    self.pos_bars = 0
            ...
            overbought = (
                k[-1] > 70 and d[-1] > 70
            )  # 🌙 Relaxed overbought to 70 for more quality setups
            ...
            if cross_down and overbought and downtrend and vol_ok and trend_strength:
                ...
                if size > 0:
                    ...
                    print(
                        f"📉 Short entry at {entry_price:.2f}, size {size}, SL {entry_price + stop_dist:.2f}, ATR {atr_val[-1]:.2f} 🌙"
                    )
                    self.partialed = False
                    self.pos_bars = 0
