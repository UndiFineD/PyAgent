# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T03_FibonacciCrossover_OPT_v1.py
import numpy as np
import pandas as pd
import talib
...
class FibonacciCrossover(Strategy):
    ...
    def init(self):
        ...
        self.ema200 = self.I(
            talib.EMA, close, timeperiod=self.ema200_period
        )  # 🌙 Moon Dev: Switched to EMA200 for better responsiveness in trends
        ...
        )  # 🌙 Moon Dev: Added ADX for trend strength filter
    ...
    def next(self):
        ...
        harmonic_long = current_close > current_ema21 * (
            1 - self.confluence_threshold
        )  # 🌙 Moon Dev: Directional for long: close not too far below EMA21 (pullback)
        harmonic_short = current_close < current_ema21 * (
            1 + self.confluence_threshold
        )  # 🌙 Moon Dev: Directional for short: close not too far above EMA21 (pullback)
        ...
        if (
            ...
            and current_rsi
            > 50  # 🌙 Moon Dev: Tightened from >40 to >50 for stronger bullish momentum
            and current_close
            > self.ema200[-1]  # 🌙 Moon Dev: Using EMA200 instead of SMA200
            and current_volume
            > self.volume_ma[-1]
            * self.volume_multiplier  # 🌙 Moon Dev: Stronger volume requirement
            and harmonic_long
            and current_adx > self.adx_threshold
        ):  # 🌙 Moon Dev: ADX filter for trending market
            ...
            )  # 🌙 Moon Dev: Changed to float for fractional BTC sizing (more precise)
            ...
            self.tp = tp_price
            print(
                f"🌙 Moon Dev: LONG Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} 🚀 Harmonic Confluence Confirmed ✨"
            )
        ...
        # Short Entry - added ADX filter, tightened RSI to <50, volume *1.5, EMA200
            ...
            and self.ema21[-1] < self.ema55[-1]
            and current_rsi
            < 50  # 🌙 Moon Dev: Tightened from <60 to <50 for stronger bearish momentum
            and current_close
            < self.ema200[-1]  # 🌙 Moon Dev: Using EMA200 instead of SMA200
            and current_volume
            > self.volume_ma[-1]
            * self.volume_multiplier  # 🌙 Moon Dev: Stronger volume requirement
            ...
            self.tp = tp_price
            print(
                f"🌙 Moon Dev: SHORT Entry at {current_close} | SL: {sl_price} | TP: {tp_price} | Size: {position_size} 📉 Harmonic Confluence Confirmed ✨"
            )
