...
class ConfluentHarmonics(Strategy):
    ...
    rr_ratio = (
        3  # 🌙 Optimized: Increased from 2 to 3 for higher reward potential on winners
    )
    risk_per_trade = 0.015  # 🌙 Optimized: Increased from 0.01 to 0.015 for slightly more aggressive sizing to boost returns
    rsi_period = 14
    rsi_long_threshold = (
        45  # 🌙 New: RSI filter for longs to ensure oversold reversal conditions
    )
    rsi_short_threshold = (
        55  # 🌙 New: RSI filter for shorts to ensure overbought reversal conditions
    )
    vol_sma_period = 20
    vol_mult = (
        1.1  # 🌙 New: Volume confirmation multiplier to filter for higher volume setups
    )
        ...
        if self.position and not self.position.sl is None:  # Ensure SL exists
            if self.position.is_long:
                ...
                if new_sl > self.position.sl:
                    ...
                    print(
                        f"🌙 Trailing SL updated to {new_sl} for long at {self.data.index[-1]} 🚀"
                    )
                ...
                if new_sl < self.position.sl:
                    self.position.sl = new_sl
                    print(
                        f"🌙 Trailing SL updated to {new_sl} for short at {self.data.index[-1]} ✨"
                    )
