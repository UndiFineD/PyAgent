# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_final\T00_StochasticHarmony_BTFinal_v8.py
import pandas as pd
import talib
...
class StochasticHarmony(Strategy):
    ...
    def init(self):
        ...
        self.k = self.I(
            lambda close: talib.STOCHRSI(
                close, timeperiod=14, fastk_period=14, fastd_period=3
            )[0],
            self.data.Close,
        )
        self.d = self.I(
        ...
        if len(high_window) == 2 * self.swing_length + 1:
            ...
            if pivot_high_price == high_window.max():
                if not self.pivots or self.pivots[-1][0] != pivot_n:
                    ...
                    print(
                        f"🌙 High Pivot confirmed at bar {pivot_n}, price {pivot_high_price:.2f} 🚀"
                    )
        ...
        if len(low_window) == 2 * self.swing_length + 1:
            ...
            if pivot_low_price == low_window.min():
                if not self.pivots or self.pivots[-1][0] != pivot_n:
                    ...
                    print(
                        f"🌙 Low Pivot confirmed at bar {pivot_n}, price {pivot_low_price:.2f} ✨"
                    )
            ...
            if not p4[2] and p3[2] and not p2[2] and p1[2] and not p0[2]:
                ...
                if X < A and B < A and B > X and C > B and D < C:
                    ...
                    if XA > 0 and AB > 0 and BC > 0 and CD > 0:
                        ...
                        retr_xd = (A - D) / XA
                        if (
                            abs(retr_ab - 0.618) < self.tolerance
                            and 0.382 < retr_bc < 0.886
                            and abs(retr_xd - 0.786) < self.tolerance
                        ):
                            print(
                                f"🌙 Moon Dev: Bullish Gartley Pattern Detected at bar {n}! Checking Stoch RSI... 📈"
                            )
                            ...
                            if bull_cross and oversold and not self.position:
                                ...
                                if risk > 0:
                                    ...
                                    print(
                                        f"🌙 Moon Dev: Bullish Gartley Entry! LONG at {entry:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size:.4f} 🚀✨"
                                    )
                ...
                if X > A and B > A and B < X and C < B and D > C:
                    ...
                    if XA > 0 and AB > 0 and BC > 0 and CD > 0:
                        ...
                        if (
                            ...
                            print(
                                f"🌙 Moon Dev: Bearish Gartley Pattern Detected at bar {n}! Checking Stoch RSI... 📉"
                            )
                            # Stochastic RSI confirmation
                            curr_k = self.k[-1]
                            curr_d = self.d[-1]
                            ...
                            if bear_cross and overbought and not self.position:
                                ...
                                if risk > 0:
                                    ...
                                    print(
                                        f"🌙 Moon Dev: Bearish Gartley Entry! SHORT at {entry:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size:.4f} 🚀✨"
                                    )

        # Debug print for Stochastic RSI occasionally
        if n % 100 == 0:
            print(
                f"🌙 Debug: Bar {n}, K: {self.k[-1]:.2f}, D: {self.d[-1]:.2f}, ATR: {self.atr[-1]:.2f}, Pivots: {len(self.pivots)} ✨"
            )


# Run backtest
bt = Backtest(
    data, StochasticHarmony, cash=1000000, commission=0.001, exclusive_orders=True
)
stats = bt.run()
print(stats)
