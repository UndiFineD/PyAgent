# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T02_QuadrupleHarmonic_OPT_v5.py
import pandas as pd
import talib
...
class QuadrupleHarmonic(Strategy):
    def init(self):
        ...
        )  # 🌙 New: Added MACD for momentum confirmation to filter better entries
    ...
    def next(self):
        ...
        if self.position:
            ...
            if current_high > entry_price + stop_distance:  # If price reached 1:1
                ...
                if trail_sl > self.position.sl:
                    ...
                    print(
                        f"🌙 Moon Dev Update: Trailing SL to {trail_sl:.2f} after 1:1 hit 🚀"
                    )
        ...
        )  # 🌙 Tightened from 0.5 to 0.6 for better entry setups
        ...
        if (
            ...
            and strong_bullish  # Bullish confirmation candle with enhanced strength filter
            and current_close > self.ema[-1]  # Uptrend filter for pullback buys
            and self.adx[-1]
            > 25  # 🌙 Raised ADX from 20 to 25 for stronger trend filter (reduces choppy entries)
            and self.rsi[-1]
            < 35  # 🌙 Tightened RSI from <40 to <35 for deeper oversold pullbacks (better timing)
            and self.data.Volume[-1]
            > 2.0
            * self.vol_sma[
                -1
            ]  # 🌙 Increased volume filter from 1.5x to 2x for higher conviction signals
            and self.macd[-1]
            > self.macdsignal[
                -1
            ]  # 🌙 New: MACD bullish crossover for momentum confirmation
            and min(self.data.Low[-5:-1]) > 0.95 * self.ema[-1]
        ):  # 🌙 New: Pullback depth filter to stay in shallow retracements

            print(
                f"🌙 Moon Dev Signal: Enhanced 4 down bars pullback in strong uptrend at {current_close:.2f} 🚀"
            )
            ...
            size = min(
                size_frac, 0.3
            )  # 🌙 Lowered cap from 0.5 to 0.3 for better risk in volatile markets
