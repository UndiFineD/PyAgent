# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_OPT_v7.py
import warnings
...
def fib_support(
    high, low, period=34
):  # Changed to 34 for golden ratio alignment, better Fib levels 🌙
...
class KalmanResonance(Strategy):
    ...
    max_bars_in_trade = (
        20  # Increased to 20 bars to allow more room for profitable moves in ranges
    )
    ...
    def next(self):
        ...
        risk_per_trade = (
            0.015 * equity
        )  # Increased to 1.5% risk per trade for higher return potential while managing drawdown 🚀
        ...
        if (
            is_ranging and self.adx[-1] < 22
        ):  # Tightened ADX for purer ranging conditions ✨
            vel_cross_up = (
            ...
            )  # Deepened to 15 for stronger oversold signals 🌙
            near_support = (
                abs(self.data.Close[-1] - self.fib_support[-1]) / self.data.Close[-1]
                < self.prz_tolerance
            )
            vol_confirm = (
                self.data.Volume[-1] > 1.1 * self.avg_vol[-1]
            )  # Slightly higher volume threshold for better momentum filter
            rsi_oversold = (
                self.rsi[-1] < 25
            )  # Added RSI confirmation for deeper oversold to reduce false signals 🚀
            ...
            if (
                ...
                if stop_distance > 0:
                    ...
                    size = (
                        ...
                        / self.data.Close[-1]
                    )  # Corrected size calc for % risk
                    size = min(size, 0.95)
                    tp = (
                        self.data.Close[-1] + 2.5 * stop_distance
                    )  # Improved to 1:2.5 RR for higher reward potential without overreaching
        ...
        if is_ranging and self.adx[-1] < 22:
            ...
            )  # Deepened to 85 for stronger overbought signals 🌙
            near_resistance = (
            ...
            vol_confirm = (
                self.data.Volume[-1] > 1.1 * self.avg_vol[-1]
            )  # Matching volume filter
            rsi_overbought = (
                self.rsi[-1] > 75
            )  # Added RSI confirmation for deeper overbought 🚀
