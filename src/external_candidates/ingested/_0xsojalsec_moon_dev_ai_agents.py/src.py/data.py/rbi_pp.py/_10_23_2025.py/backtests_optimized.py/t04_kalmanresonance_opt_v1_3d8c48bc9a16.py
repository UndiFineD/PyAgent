# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T04_KalmanResonance_OPT_v1.py
import warnings
...
class KalmanResonance(Strategy):
    ...
    def init(self):
        ...
        )  # 🌙 Moon Dev: New ADX indicator for regime filter
        self.trade_bars = 0
        self.entry_price = 0.0
        self.sl_price = 0.0
        self.tp_price = 0.0
        print(
            "🌙 Moon Dev KalmanResonance Optimized: Enhanced filters, proper sizing, and trailing for 50% target! ✨"
        )
    ...
    def next(self):
        ...
        if self.position:
            ...
            if is_long:
                if self.data.Low[-1] <= self.sl_price:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev SL Exit Long at {self.data.Close[-1]:.2f} (SL {self.sl_price:.2f}) 💥"
                    )
                    self.trade_bars = 0
                ...
                if self.data.High[-1] >= self.sl_price:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev SL Exit Short at {self.data.Close[-1]:.2f} (SL {self.sl_price:.2f}) 💥"
                    )
                    self.trade_bars = 0
            ...
            if is_long:
                ...
                if self.data.Low[-1] <= trail_stop:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev Trailing Exit Long at {self.data.Close[-1]:.2f} (trail {trail_stop:.2f}) 🌙"
                    )
                    self.trade_bars = 0
                    delattr(self, "highest")
                    return
                ...
                if self.data.High[-1] >= trail_stop:
                    self.position.close()
                    print(
                        f"🌙 Moon Dev Trailing Exit Short at {self.data.Close[-1]:.2f} (trail {trail_stop:.2f}) 🌙"
                    )
                    self.trade_bars = 0
                    delattr(self, "lowest")
                    return
        ...
        equity = (
            self._broker._cash + self.position.pl
        )  # Approximate current equity for sizing
        ...
        )  # Strict cross up for momentum confirmation
        ...
        near_support = (
            abs(close_price - self.fib_support[-1]) / close_price < self.prz_tolerance
        )
        ...
        if vel_cross_up and stoch_oversold_cross and near_support:
            ...
            if stop_distance > 0:
                ...
                tp = (
                    close_price + self.rr_ratio * stop_distance
                )  # 1:3 RR for better returns
                self.buy(size=size, sl=None, tp=None)  # Manual exits for trailing
                self.entry_price = close_price
                self.sl_price = sl
                self.tp_price = tp
                print(
                    f"🌙 Moon Dev Long Entry: Price={close_price:.2f}, Size={size:.4f}, SL={sl:.2f}, TP={tp:.2f} ✨"
                )
        ...
        near_resistance = (
            abs(close_price - self.fib_resistance[-1]) / close_price
            < self.prz_tolerance
        )
        ...
        if vel_cross_down and stoch_overbought_cross and near_resistance:
            ...
            if stop_distance > 0:
                ...
                self.tp_price = tp
                print(
                    f"🌙 Moon Dev Short Entry: Price={close_price:.2f}, Size={size:.4f}, SL={sl:.2f}, TP={tp:.2f} ✨"
                )
