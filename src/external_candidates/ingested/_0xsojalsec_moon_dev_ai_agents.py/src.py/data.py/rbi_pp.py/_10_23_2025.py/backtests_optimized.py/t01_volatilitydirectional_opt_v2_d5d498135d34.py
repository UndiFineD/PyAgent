...
class VolatilityDirectional(Strategy):
    ...
    def next(self):
        ...
        )  # New: Volume filter for entry quality
        ...
        if (
            ...
            )  # New: ATR-based SL for better risk management
            ...
            if risk_dist > 0:
                ...
                tp_price = (
                    entry_price + 3 * risk_dist
                )  # Improved: 3:1 R:R for higher returns
                self.buy(
                    size=pos_size
                )  # No built-in SL/TP; manage manually for proper trailing
        ...
        if self.position:
            ...
            current_profit = (
                (close - entry_price_pos)
                if self.position.is_long
                else (entry_price_pos - close)
            )
            atr_val = self.atr[-1]
            ...
            if current_profit > 1.5 * initial_risk:
                if self.position.is_long:
                    ...
                    if new_sl > self.current_sl:
                        ...
                        print(
                            f"🌙 Moon Dev: Long trailing SL updated to {new_sl:.2f} 🚀"
                        )
                else:
                    new_sl = close + 2 * atr_val
                    if new_sl < self.current_sl:
                        self.current_sl = new_sl
                        print(
                            f"🌙 Moon Dev: Short trailing SL updated to {new_sl:.2f} ✨"
                        )
