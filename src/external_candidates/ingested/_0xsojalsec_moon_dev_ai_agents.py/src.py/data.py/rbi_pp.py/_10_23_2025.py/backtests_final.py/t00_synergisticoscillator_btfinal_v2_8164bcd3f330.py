# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_final\T00_SynergisticOscillator_BTFinal_v2.py
import numpy as np
import pandas as pd
import talib
        ...
        if not self.position:
            ...
            if cross_up and oversold and uptrend and vol_ok_long:
                ...
                if size > 0:
                    ...
                    print(
                        f"🚀 Long entry at {entry_price:.2f}, size {size}, SL {sl_price:.2f}, ATR {atr_val[-1]:.2f} 🌙"
                    )
                    self.partialed = False
                    self.pos_bars = 0
            ...
            if cross_down and overbought and downtrend and vol_ok_short:
                ...
                if size > 0:
                    ...
                    print(
                        f"📉 Short entry at {entry_price:.2f}, size {size}, SL {sl_price:.2f}, ATR {atr_val[-1]:.2f} 🌙"
                    )
                    self.partialed = False
                    self.pos_bars = 0
