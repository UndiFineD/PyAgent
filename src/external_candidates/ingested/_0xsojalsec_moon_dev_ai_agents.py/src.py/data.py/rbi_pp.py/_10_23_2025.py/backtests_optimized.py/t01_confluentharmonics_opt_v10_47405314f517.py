# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v10.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    def next(self):
        ...
        vol_confirm = (
            self.data.Volume[-1] > vol_sma_val * 1.2
        )  # 🌙 Volume filter: require 20% above 20-period average for conviction
        trend_long = (
            price > ema200_val
        )  # 🌙 Trend filter for longs: only trade above 200 EMA
        trend_short = (
            price < ema200_val
        )  # 🌙 Trend filter for shorts: only trade below 200 EMA
        ...
        if self.position:
            if self.position.is_long:
                ...
                if trail_sl > self.position.sl:
                    ...
                    print(
                        f"🌙 Trailing Long SL updated to {trail_sl} at {self.data.index[-1]} 🚀"
                    )
                ...
                if trail_sl < self.position.sl:
                    self.position.sl = trail_sl
                    print(
                        f"🌙 Trailing Short SL updated to {trail_sl} at {self.data.index[-1]} ✨"
                    )

            # Early exits for weakening trend - adjusted threshold
            if (
                self.position.is_long and adx_val < 18
            ):  # 🌙 Raised early exit threshold from 20 to 18 to allow more trend breathing room
                self.position.close()
                print(
        ...
        if not self.position:
            ...
            if (
                ...
                and pdi_val > mdi_val
                and squeeze
                and rsi_val < 35  # 🌙 RSI filter: oversold for better reversal setups
                and vol_confirm
                and trend_long
            ):
                entry = price
            ...
            # Bearish Reversal Proxy: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze, plus new filters
                ...
                and mdi_val > pdi_val
                and squeeze
                and rsi_val > 65  # 🌙 RSI filter: overbought for better reversal setups
                and vol_confirm
                and trend_short
            ):
                entry = price
