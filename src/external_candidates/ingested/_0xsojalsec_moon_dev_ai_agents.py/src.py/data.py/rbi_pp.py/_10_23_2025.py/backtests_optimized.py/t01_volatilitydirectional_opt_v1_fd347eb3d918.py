...
class VolatilityDirectional(Strategy):
    ...
    def next(self):
        ...
        long_momentum = (pullback_long and self.rsi[-1] < 35) or (
            squeeze_long and self.rsi[-1] > 55
        )
        if (
            not self.position
            and long_momentum
            ...
            sl_dist = (
                1.5 * atr_val
            )  # 🌙 Moon Dev: ATR-based SL for dynamic risk (avoids tight fixed % SL)
            sl_price = entry_price - sl_dist
            risk_dist = sl_dist
            if risk_dist > 0:
                risk_amount = (
                    self.equity * 0.015
                )  # 🌙 Moon Dev: Increased risk to 1.5% for higher returns (balanced)
                pos_size = (
                    risk_amount / risk_dist
                )  # 🌙 Moon Dev: Allow fractional size, removed vol_adjust (ATR handles vol)
                tp_dist = (
                    3 * risk_dist
                )  # 🌙 Moon Dev: Improved RR to 1:3 for higher reward potential
        ...
        short_momentum = (pullback_short and self.rsi[-1] > 65) or (
            squeeze_short and self.rsi[-1] < 45
        )
        if (
            not self.position
            and short_momentum
