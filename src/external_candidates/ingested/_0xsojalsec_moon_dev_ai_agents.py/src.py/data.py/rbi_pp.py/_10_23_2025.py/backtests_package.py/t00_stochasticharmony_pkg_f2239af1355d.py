# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_package\T00_StochasticHarmony_PKG.py
import pandas as pd
import talib
...
class StochasticHarmony(Strategy):
    ...
    def init(self):
        ...
        self.stochrsi, self.k, self.d = self.I(
            talib.STOCHRSI,
            self.data.Close,
            timeperiod=14,
            fastk_period=14,
    ...
    def next(self):
        ...
        pivot_offset = self.swing_length
        ...
        high_window = self.data.High.iloc[start_idx:n]
        if len(high_window) == 2 * self.swing_length + 1:
            pivot_high_price = self.data.High.iloc[pivot_n]
        ...
        low_window = self.data.Low.iloc[start_idx:n]
        if len(low_window) == 2 * self.swing_length + 1:
            pivot_low_price = self.data.Low.iloc[pivot_n]
                            ...
                            if bull_cross and oversold and not self.position:
                                ...
                                if risk > 0:
                                    ...
                                    if size > 0:
                                        ...
                                        print(
                                            f"🌙 Moon Dev: Bullish Gartley Detected! LONG entry at {entry:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size} 🚀✨"
                                        )
                            ...
                            if bear_cross and overbought and not self.position:
                                ...
                                if risk > 0:
                                    ...
                                    if size > 0:
                                        ...
                                        print(
                                            f"🌙 Moon Dev: Bearish Gartley Detected! SHORT entry at {entry:.2f}, SL: {sl:.2f}, TP: {tp:.2f}, Size: {size} 🚀✨"
                                        )

        # Debug print for Stochastic RSI occasionally
        if n % 100 == 0:
            print(
                f"🌙 Debug: Bar {n}, K: {self.k[-1]:.2f}, D: {self.d[-1]:.2f}, ATR: {self.atr[-1]:.2f} ✨"
            )
