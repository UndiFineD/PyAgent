# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_final\T01_VolatilityDirectional_BTFinal_v1.py
import numpy as np
import pandas as pd
import talib
...
class VolatilityDirectional(Strategy):
    ...
    def next(self):
        ...
        plus_cross = (
            ...
            and self.plus_di[-2] <= self.minus_di[-2]
        )
        minus_cross = (
            self.minus_di[-1] > self.plus_di[-1]
            and self.minus_di[-2] <= self.plus_di[-2]
        )
        ...
        if (
            ...
            if risk_dist > 0:
                ...
                print(
                    f"🌙 Moon Dev: Long entry at {entry_price:.2f}, size {pos_size}, SL {sl_price:.2f}, TP {tp_price:.2f} 🚀"
                )
        ...
        if (
            ...
            if risk_dist > 0:
                ...
                self.initial_risk = risk_dist
                print(
                    f"🌙 Moon Dev: Short entry at {entry_price:.2f}, size {pos_size}, SL {sl_price:.2f}, TP {tp_price:.2f} ✨"
                )
        ...
        if self.position:
            ...
            current_profit = (
                abs(close - entry_price_pos)
                if self.position.is_long
                else abs(entry_price_pos - close)
            )
