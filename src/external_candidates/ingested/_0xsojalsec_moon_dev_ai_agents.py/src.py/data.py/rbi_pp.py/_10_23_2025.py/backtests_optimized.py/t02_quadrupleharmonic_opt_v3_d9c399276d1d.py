# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T02_QuadrupleHarmonic_OPT_v3.py
import pandas as pd
import talib
...
class QuadrupleHarmonic(Strategy):
    def init(self):
        ...
        )  # 🌙 New: +DI for uptrend strength filter
        ...
        self.initial_sl = (
            None  # 🌙 New: Track initial SL for RR calculations in trailing
        )
    ...
    def next(self):
            ...
            if self.entry_price and self.initial_sl:
                ...
                if profit > risk:  # Reached 1R
                    ...
                    if trail_sl > self.position.sl:
                        ...
                        print(
                            f"🌙 Moon Dev Trail: Updated SL to {trail_sl:.2f} at {current_close:.2f} 🚀"
                        )
            return
        ...
        strong_bullish = (
            body > 0.7 * candle_range and body > 0
        )  # 🌙 Tightened to 70% of range for stronger conviction entries
        ...
        if (
            len(self.data) >= 4
            and self.data.Close[-4] < self.data.Open[-4]  # First down bar
            and self.data.Close[-3] < self.data.Open[-3]  # Second
            and self.data.Close[-2] < self.data.Open[-2]  # Third
            and self.data.Close[-3] < self.data.Close[-4]  # Strictly decreasing closes
            and self.data.Close[-2] < self.data.Close[-3]
            and strong_bullish  # Strong bullish reversal
            and current_close > self.ema[-1]  # Uptrend filter
            and self.adx[-1]
            > 22  # 🌙 Adjusted ADX to 22 for balanced trend strength (more than 20, less restrictive than 25)
            and self.rsi[-1]
            < 45  # 🌙 Loosened RSI to <45 for capturing shallower pullbacks with potential
            and self.data.Volume[-1]
            > 1.3
            * self.vol_sma[
                -1
            ]  # 🌙 Adjusted volume to 1.3x for more signals while keeping conviction
            and self.plus_di[-1] > self.minus_di[-1]
        ):  # 🌙 New: +DI > -DI for confirmed uptrend regime filter

            print(
                f"🌙 Moon Dev Signal: 3 down bars pullback in uptrend, strong bullish at {current_close:.2f} 🚀"
            )
            ...
            size = min(
                size_frac, 0.4
            )  # 🌙 Reduced cap to 40% for tighter risk in volatile markets
