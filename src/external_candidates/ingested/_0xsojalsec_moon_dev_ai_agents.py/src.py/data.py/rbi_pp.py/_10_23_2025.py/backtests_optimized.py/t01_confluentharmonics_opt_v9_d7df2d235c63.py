# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_23_2025\backtests_optimized\T01_ConfluentHarmonics_OPT_v9.py
import pandas as pd
import talib
...
class ConfluentHarmonics(Strategy):
    ...
    def next(self):
        ...
        vol_confirmation = (
            vol_current > 1.5 * avg_vol_current
        )  # 🌙 Volume spike filter to avoid low-conviction entries
        ...
        if not self.position:
            ...
            if (
                ...
                and squeeze
                and rsi_prev
                < 30  # 🌙 RSI filter for oversold condition on previous bar
            ...
            # Bearish Reversal Proxy: Previous close at/touching upper BB, current close below middle, ADX strong, -DI > +DI, squeeze, RSI overbought prev, volume spike
                ...
                and squeeze
                and rsi_prev
                > 70  # 🌙 RSI filter for overbought condition on previous bar
                and vol_confirmation
            ):  # 🌙 Volume confirmation
                entry = price
                risk_dist = self.atr_sl_mult * atr_val
