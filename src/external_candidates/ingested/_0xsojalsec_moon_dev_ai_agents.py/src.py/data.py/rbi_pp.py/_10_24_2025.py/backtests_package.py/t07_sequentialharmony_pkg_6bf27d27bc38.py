# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\data\rbi_pp\10_24_2025\backtests_package\T07_SequentialHarmony_PKG.py
import pandas as pd
import talib
...
class SequentialHarmony(Strategy):
    ...
    def next(self):
        ...
        is_down_sequence = (
            (prev_closes[0] < prev_opens[0])
            and (prev_closes[1] < prev_opens[1])
            and (prev_closes[2] < prev_opens[2])
            and (prev_closes[3] < prev_opens[3])
            and (prev_closes[1] < prev_closes[0])
            and (prev_closes[2] < prev_closes[1])
            and (prev_closes[3] < prev_closes[2])
        )
        ...
        if is_down_sequence and is_reversal and in_downtrend and oversold:
            ...
            if risk > 0:
                ...
                print(
                    f"🌙 Moon Dev's SequentialHarmony: 4 consecutive down bars exhausted! Reversal confirmed with RSI oversold. Entering LONG at {entry_price:.2f}, SL: {stop_price:.2f}, TP: {tp_price:.2f}, Size: {size} units 🚀✨"
                )
