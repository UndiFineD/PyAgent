...
class DynamicHarmonics(Strategy):
    ...
    def next(self):
        ...
        for i in range(max(0, current_idx - self.lookback), current_idx):
            if high == self.swing_high[i] and (
                i == 0 or self.data.High[i - 1] < self.data.High[i]
            ):
                recent_swing_high_idx = i
                break
        for i in range(max(0, current_idx - self.lookback), current_idx):
            if low == self.swing_low[i] and (
                i == 0 or self.data.Low[i - 1] > self.data.Low[i]
            ):
        ...
        if (
            ...
            xa_range = (
                self.data.High[recent_swing_high_idx]
                - self.data.Low[recent_swing_low_idx]
            )
        ...
        bb_expanding_up = (
            self.bb_upper[-1] > self.bb_upper[-2] and close > self.bb_middle[-1]
        )
        bb_expanding_down = (
            self.bb_lower[-1] < self.bb_lower[-2] and close < self.bb_middle[-1]
        )
        ...
        if all(long_condition):
            ...
            sl = (
                min(
                    ...
                    (
                        self.data.Low[recent_swing_low_idx]
                        if recent_swing_low_idx
                        else self.bb_lower[-1]
                    ),
                )
                - self.atr[-1]
            )
            ...
            self.buy(
                size=position_size, sl=sl, tp=entry_price + self.rr_ratio * sl_distance
            )
        ...
        if all(short_condition):
            ...
            sl = (
                max(
                    ...
                    (
                        self.data.High[recent_swing_high_idx]
                        if recent_swing_high_idx
                        else self.bb_upper[-1]
                    ),
                )
                + self.atr[-1]
            )
...
bt = Backtest(
    data, DynamicHarmonics, cash=1000000, commission=0.002, exclusive_orders=True
)
stats = bt.run()
print(stats)
