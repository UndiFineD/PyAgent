# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\strategies\example_strategy.py
"""
        ...
        try:
            for token in MONITORED_TOKENS:
                ...
                if prev_fast <= prev_slow and current_fast > current_slow:
                    signal.update({"signal": 1.0, "direction": "BUY"})

                # Bearish crossover (fast crosses below slow)
                elif prev_fast >= prev_slow and current_fast < current_slow:
                    signal.update({"signal": 1.0, "direction": "SELL"})
