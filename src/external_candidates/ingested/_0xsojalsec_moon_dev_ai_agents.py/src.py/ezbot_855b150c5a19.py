# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\ezbot.py
"""
...
action = input(
    "0 to close, 1 to buy, 2 stop loss, 3 breakout, 5 market maker  |||| 6 funding buy, 7 liquidation amount:"
)
...
def bot():
    ...
    while action == 1:
        ...
        while pos_usd < (0.97 * usd_size):
            ...
            print(
                f"position: {round(pos,2)} price: {round(price,8)} buy_under: {buy_under} pos_usd: ${round(pos_usd,2)}"
            )
        ...
        cprint(
            f"position filled of {symbol[-4:]} total: ${pos_usd}", "white", "on_green"
        )
        break
    ...
    while action == 2:
        ...
        print(
            f"stop loss: close if price under {STOPLOSS_PRICE} current price is {price}"
        )

        if price < STOPLOSS_PRICE and pos > 0:
            print(
                f"selling {symbol[-4:]} bc price is {price}  is under {STOPLOSS_PRICE}"
            )
            n.chunk_kill(symbol, max_usd_order_size, slippage)
    ...
    while action == 5:
        ...
        if price > sell_over:
            print(
                f"selling {symbol[-4:]} bc price is {price} and sell over is {sell_over}"
            )
            n.chunk_kill(symbol, max_usd_order_size, slippage)
            print(f"chunk kill complete... thank you moon dev you are my savior 777")
            time.sleep(15)
            ...
            if (pos_usd < usd_size) and (price < buy_under):
                print(
                    f"buying {symbol[-4:]} bc price is {price} and buy under is {buy_under}"
                )
