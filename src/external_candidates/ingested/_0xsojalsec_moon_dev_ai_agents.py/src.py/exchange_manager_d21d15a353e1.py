# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\exchange_manager.py
"""
...
class ExchangeManager:
    ...
    def __init__(self, exchange=None):
        ...
        if self.exchange.lower() == "hyperliquid":
            try:
                ...
                cprint(
                    f"   Account: {self.account.address[:6]}...{self.account.address[-4:]}",
                    "cyan",
                )
    ...
    def market_sell(self, symbol_or_token, usd_amount_or_percent):
        ...
        if self.exchange.lower() == "hyperliquid":
            ...
            return self.hl.market_sell(
                symbol_or_token, usd_amount_or_percent, self.account
            )
        else:
    ...
    def chunk_kill(self, symbol_or_token, max_order_size=None, slippage=None):
        ...
        if self.exchange.lower() == "hyperliquid":
            ...
            from src.config import max_usd_order_size
            from src.config import slippage as default_slippage
    ...
    def get_all_positions(self):
        ...
        if self.exchange.lower() == "hyperliquid":
            ...
            from src.config import EXCLUDED_TOKENS, address
