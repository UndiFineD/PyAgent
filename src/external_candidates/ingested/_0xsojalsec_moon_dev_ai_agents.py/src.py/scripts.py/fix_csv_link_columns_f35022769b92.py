# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\fix_csv_link_columns.py
"""
...
CONSENSUS_PICKS_CSV = os.path.join(
    PROJECT_ROOT, "src/data/polymarket/consensus_picks.csv"
)
...
def fix_csv_link_column(csv_path, link_column_name):
    ...
    try:
        ...
        cprint(
            f"   ├─ Current position of '{link_column_name}': {current_position}",
            "yellow",
        )
        ...
        cprint(
            f"   ├─ '{link_column_name}' is now at position: {len(cols) - 1} (last)",
            "green",
        )
...
def main():
    ...
    cprint("\n" + "=" * 80, "green")
    cprint(
        "✅ Done! Your CSV link columns are now at the end.", "green", attrs=["bold"]
    )
    cprint("=" * 80 + "\n", "green")
