# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\codefolder_totxt.py
import glob
import os
...
OUTPUT_FOLDER = (
    "/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data/codetotext"
)
...
def process_folder_to_txt():
    ...
    output_file = os.path.join(
        OUTPUT_FOLDER, f"{folder_name}_code_dump_{timestamp}.txt"
    )
    ...
    with open(output_file, "w", encoding="utf-8") as outfile:
        outfile.write(
            f"// Moon Dev's Code Collection - Generated on {datetime.now()}\n"
        )
