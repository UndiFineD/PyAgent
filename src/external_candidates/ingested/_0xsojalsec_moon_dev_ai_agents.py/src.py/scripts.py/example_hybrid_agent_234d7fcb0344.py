# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\example_hybrid_agent.py
#!/usr/bin/env python3
"""
...
def trade_on_hyperliquid():
    ...
    try:
        ...
        # Check position
        positions, im_in_pos, size, sym, entry, pnl, is_long = hl.get_position(
            "BTC", account
        )
        if im_in_pos:
            side = "LONG" if is_long else "SHORT"
            cprint(
                f"Current position: {side} {size} BTC @ ${entry:.2f} (PnL: {pnl:.2f}%)",
                "green",
            )
        else:
...
def main():
    ...
    cprint(
        f"  HyperLiquid: {'✅' if has_hyperliquid else '❌'} HYPER_LIQUID_KEY", "white"
    )
