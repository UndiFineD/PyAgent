# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\coingecko_exchangeless_tokens.py
"""
...
class CoinGeckoTokenFinder:
    ...
    def _make_request(self, endpoint: str, params: Optional[Dict] = None) -> Dict:
        ...
        try:
            self.api_calls += 1
            cprint(
                f"\n🔄 API Call #{self.api_calls} - Endpoint: {endpoint}",
                "white",
                "on_blue",
            )
    ...
    def filter_tokens(self, tokens: List[Dict]) -> List[Dict]:
        ...
        existing_token_ids = (
            set(existing_tokens_df["token_id"].tolist())
            if not existing_tokens_df.empty
            else set()
        )
        ...
        for token in tokens:
            try:
                ...
                if token_id in existing_token_ids:
                    ...
                    matching_token = existing_tokens_df[
                        existing_tokens_df["token_id"] == token_id
                    ].iloc[0]
...
def main():
    ...
    try:
        while True:
            ...
            next_run_str = datetime.fromtimestamp(next_run).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
