# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\arvix_download.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
SEARCH_TOPIC = (
    "market making"  # Try: "mean reversion", "momentum", "market making", etc.
)
...
DOWNLOAD_DIRECTORY = os.path.join(
    "/Users/md/Dropbox/dev/github/moon-dev-ai-agents-for-trading/src/data", "Arxiv"
)
...
def print_banner():
    ...
    print(
        f"{Colors.BOLD}Current search topic:{Colors.ENDC} {Colors.YELLOW}{SEARCH_TOPIC}{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}Category:{Colors.ENDC} {Colors.YELLOW}{SEARCH_CATEGORY}{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}Max papers:{Colors.ENDC} {Colors.YELLOW}{MAX_PAPERS_TO_DOWNLOAD}{Colors.ENDC}"
    )
    print(
        f"{Colors.BOLD}Download directory:{Colors.ENDC} {Colors.YELLOW}{DOWNLOAD_DIRECTORY}{Colors.ENDC}"
    )
    topic_dir = os.path.join(
        DOWNLOAD_DIRECTORY, f"{SEARCH_CATEGORY}_{SEARCH_TOPIC.replace(' ', '_')}"
    )
    print(
        f"{Colors.BOLD}Papers will be saved to:{Colors.ENDC} {Colors.YELLOW}{topic_dir}{Colors.ENDC}\n"
    )
...
def search_arxiv(
    ...
    print(
        f"🔍 Searching arXiv for: {Colors.YELLOW}{query}{Colors.ENDC} in category {Colors.YELLOW}{category if category else 'all'}{Colors.ENDC}"
    )
    print(f"🌐 API URL: {url}")
...
def display_papers(papers):
    ...
    for i, paper in enumerate(papers):
        ...
        if already_downloaded:
            print(
                f"{Colors.BOLD}{i+1}. {Colors.RED}[ALREADY DOWNLOADED] {Colors.YELLOW}{paper['title']}{Colors.ENDC}"
            )
        else:
        ...
        print(
            f"   {Colors.BLUE}Categories:{Colors.ENDC} {', '.join(paper['categories'])}"
        )
...
def download_paper(paper, download_dir, show_progress=True):
    ...
    if "pdf_url" not in paper:
        print(
            f"{Colors.RED}Error: No PDF URL found for paper {paper['arxiv_id']}{Colors.ENDC}"
        )
        return False
    ...
    if is_paper_downloaded(paper["arxiv_id"]):
        print(
            f"{Colors.RED}{Colors.BOLD}⚠️ DUPLICATE DETECTED: {paper['arxiv_id']} - {paper['title']}{Colors.ENDC}"
        )
        print(
            f"{Colors.RED}⏩ Skipping download - Paper already in tracking database{Colors.ENDC}"
        )
        return True
    ...
    if os.path.exists(filepath):
        print(
            f"{Colors.YELLOW}⚠️ File exists on disk but not in tracking database: {filepath}{Colors.ENDC}"
        )
        # Add it to our tracking system
        paper["filepath"] = filepath
        save_downloaded_paper(paper["arxiv_id"], paper)
        print(
            f"{Colors.GREEN}✅ Added to tracking database - No download needed{Colors.ENDC}"
        )
        return True
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}✅ Successfully downloaded paper to {filepath}{Colors.ENDC}"
        )
...
def batch_download(papers, download_dir, max_papers=5, delay=1.0):
    ...
    for i, paper in enumerate(papers_to_download):
        ...
        if is_paper_downloaded(paper["arxiv_id"]):
            print(
                f"{Colors.RED}{Colors.BOLD}⚠️ DUPLICATE DETECTED: {paper['arxiv_id']}{Colors.ENDC}"
            )
            print(
                f"{Colors.RED}⏩ Skipping download - Paper already in tracking database{Colors.ENDC}"
            )
    ...
    print(
        f"{Colors.GREEN}✅ Successfully downloaded: {success_count} papers{Colors.ENDC}"
    )
    print(
        f"{Colors.YELLOW}⏩ Skipped (already downloaded): {skipped_count} papers{Colors.ENDC}"
    )
    print(
        f"{Colors.BLUE}📚 Total processed: {success_count + skipped_count} papers{Colors.ENDC}"
    )

    return success_count


def auto_download_by_topic(
    topic,
    category=None,
    max_papers=5,
    download_dir=None,
    ...
    if download_dir is None:
        download_dir = os.path.join(
            os.path.expanduser("~"), "Downloads", "arxiv_papers"
        )

    # Create topic-specific directory
    topic_dir = os.path.join(
        download_dir, f"{category or 'all'}_{topic.replace(' ', '_')}"
    )
    topic_dir = create_download_dir(topic_dir)

    # Search for papers with the specified category
    print(
        f"{Colors.BOLD}🔍 Searching for '{topic}' in {category or 'all'} category...{Colors.ENDC}"
    )
    papers = search_arxiv(
        topic,
        category=category,
    ...
    if not papers and category:
        print(
            f"{Colors.YELLOW}⚠️ No results found in {category}. Trying without category constraint...{Colors.ENDC}"
        )
        papers = search_arxiv(
            topic,
            category=None,
    ...
    if not papers:
        print(
            f"{Colors.YELLOW}⚠️ No results found. Trying with a more specific search...{Colors.ENDC}"
        )
    ...
    if not papers:
        print(
            f"{Colors.YELLOW}⚠️ No results found. Trying with a broader search...{Colors.ENDC}"
        )
    ...
    if not papers:
        print(
            f"{Colors.RED}No papers found for topic: {topic} after multiple search attempts{Colors.ENDC}"
        )
        return 0
...
def display_downloaded_papers():
    ...
    for i, (paper_id, paper_info) in enumerate(downloaded_papers.items()):
        ...
        print(
            f"   {Colors.BLUE}Authors:{Colors.ENDC} {', '.join(paper_info['authors'])}"
        )
        print(
            f"   {Colors.BLUE}Categories:{Colors.ENDC} {', '.join(paper_info['categories'])}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "--num-papers",
        type=int,
        default=MAX_PAPERS_TO_DOWNLOAD,
    ...
    parser.add_argument(
        "--list", action="store_true", help="List all previously downloaded papers"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"{Colors.CYAN}👋 Thanks for using Moon Dev's arXiv Downloader! Goodbye!{Colors.ENDC}"
        )
    except Exception as e:
