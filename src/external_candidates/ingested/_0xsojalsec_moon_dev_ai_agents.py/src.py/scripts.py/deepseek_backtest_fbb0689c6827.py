# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\deepseek_backtest.py
import numpy as np
import pandas as pd
...
class VWAPVolumeStrategy(Strategy):
    ...
    def init(self):
        ...
        rolling_sum_price_volume = np.convolve(
            typical_price * volume, np.ones(self.vwap_window), "valid"
        )
        ...
        self.vwap = np.pad(
            rolling_sum_price_volume / rolling_sum_volume,
            (pad_length, 0),
            "constant",
        ...
        self.avg_volume = (
            np.convolve(volume, np.ones(self.vwap_window), "valid") / self.vwap_window
        )
        self.avg_volume = np.pad(
            self.avg_volume, (pad_length, 0), "constant", constant_values=np.nan
        )
    ...
    def next(self):
        ...
        if (
            ...
            and self.data.Volume[-1] > self.volume_threshold * self.avg_volume[-1]
        ):
...
# Run the backtest
bt = Backtest(data, VWAPVolumeStrategy, cash=10000, commission=0.002)
stats = bt.run()
