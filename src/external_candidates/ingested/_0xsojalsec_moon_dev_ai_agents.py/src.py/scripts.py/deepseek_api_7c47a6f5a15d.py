# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\scripts\deepseek_api.py
import json
import time
    ...
    try:
        ...
        async def check_health():
            ...
            if response.status_code == 200:
                return {
                    "status": "healthy",
                    "message": "✨ Ollama is healthy and responding!",
                }
            return {
                "status": "unhealthy",
...
async def create_chat_completion(request: dict):
    ...
    try:
        ...
        if not ollama_model:
            raise HTTPException(
                status_code=400,
                detail=f"❌ Unsupported model: {requested_model}. Use 'deepseek-chat' or 'deepseek-reasoner'",
            )
        ...
        raise HTTPException(
            status_code=504, detail="Request timed out after all retries"
        )
    except requests.ConnectionError:
        print("🔌 Connection error reaching Ollama")
        raise HTTPException(
            status_code=502, detail="Cannot connect to Ollama after all retries"
        )
    except Exception as e:
