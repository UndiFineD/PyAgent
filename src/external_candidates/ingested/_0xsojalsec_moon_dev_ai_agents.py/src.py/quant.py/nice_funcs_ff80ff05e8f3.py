...
def token_overview(address):
    ...
    if response.status_code == 200:
        ...
        result["minimum_trades_met"] = (
            True if trade1h >= MIN_TRADES_LAST_HOUR else False
        )
        ...
        rug_pull = any(
            value < -80 for key, value in price_changes.items() if value is not None
        )
...
def market_buy(token, amount, slippage):
    ...
    quote = requests.get(
        f"https://lite-api.jup.ag/swap/v1/quote?inputMint={QUOTE_TOKEN}&outputMint={token}&amount={amount_in_units}&slippageBps={SLIPPAGE}"
    ).json()
...
def market_sell(QUOTE_TOKEN, amount, slippage):
    ...
    # Use Jupiter Lite API (faster and more efficient)
    quote = requests.get(
        f"https://lite-api.jup.ag/swap/v1/quote?inputMint={QUOTE_TOKEN}&outputMint={token}&amount={amount}&slippageBps={SLIPPAGE}"
    ).json()
    # print(quote)
    txRes = requests.post(
        "https://lite-api.jup.ag/swap/v1/swap",
...
def fetch_wallet_holdings_og(address):
    ...
    API_KEY = (
        BIRDEYE_API_KEY  # Assume this is your API key; replace it with the actual one
    )
...
def get_position(token_mint_address):
    ...
    if dataframe["Mint Address"].isin([token_mint_address]).any():
        ...
        balance = dataframe.loc[
            dataframe["Mint Address"] == token_mint_address, "Amount"
        ].iloc[0]
...
def pnl_close(token_mint_address):
    ...
    print(
        f"checking pnl close to see if its time to exit for {token_mint_address[:4]}..."
    )
    ...
    if usd_value != 0:
        ...
        while usd_value < sl and usd_value > 0:
            ...
            cprint(
                f"for {token_mint_address[:4]} value is {usd_value} and sl is {sl} so closing as a loss...",
                "white",
                "on_blue",
            )
            ...
            sell_size = int(sell_size * 10**decimals)
            print(
                f"balance is {balance} and price is {price} and usd_value is {usd_value} and tp is {tp} and sell_size is {sell_size} decimals is {decimals}"
            )

            # break the loop if usd_value is 0
            if usd_value == 0:
                print(
                    f"successfully closed {token_mint_address[:4]} usd_value is {usd_value} so breaking loop AFTER putting it on my dont_overtrade.txt..."
                )
...
def close_all_positions():
    ...
    for index, row in open_positions.iterrows():
        ...
        cprint(
            f"this is the token mint address {token_mint_address} this is don not trade list {dont_trade_list}",
            "white",
            "on_magenta",
        )
...
def get_token_balance_usd(token_mint_address):
    ...
    try:
        ...
        df = fetch_wallet_token_single(
            address, token_mint_address
        )  # Using address from config
