            ...
            if sym_info["symbol"] == symbol:
                ...
                for filter_info in sym_info.get("filters", []):
                    ...
                    if filter_info["filterType"] == "LOT_SIZE":
                        ...
                        quantity_precision = (
                            len(step_size.rstrip("0").split(".")[-1])
                            if "." in step_size
                            else 0
                        )
...
def place_limit_order_with_chase(
    symbol, side, quantity, leverage, max_attempts=20, check_interval=0.5
):
    ...
    try:
        ...
        while attempts < max_attempts:
            ...
            if current_order_id and target_price == last_price:
                ...
                if status == "FILLED":
                    cprint(
                        f"✅ Order FILLED! Order ID: {current_order_id}",
                        "green",
                        attrs=["bold"],
                    )
                    return order_status
            ...
            if current_order_id:
                try:
                    cprint(
                        f"🔄 Best {side} price changed: ${last_price:.2f} → ${target_price:.2f}",
                        "yellow",
                    )
                    ...
                    cprint(
                        f"   Could not cancel order {current_order_id}: {e}", "yellow"
                    )

            # Place new limit order at best bid/ask
            cprint(
                f"📝 Placing LIMIT {side}: {quantity} {symbol} @ ${target_price:.2f}",
                "cyan",
            )
        ...
        if current_order_id:
            cprint(
                f"⚠️  Max attempts reached, cancelling order {current_order_id}",
                "yellow",
            )
            try:
                api.cancel_order(symbol, order_id=current_order_id)
...
def market_buy(token, amount, slippage, leverage=DEFAULT_LEVERAGE):
    ...
    try:
        ...
        if quantity <= 0 or actual_notional < min_notional:
            ...
            cprint(
                f"   Actual Notional: ${actual_notional:.2f} (minimum: ${min_notional:.2f})",
                "red",
            )
...
def market_sell(token, amount, slippage, leverage=DEFAULT_LEVERAGE):
    ...
    try:
        ...
        if position and position["position_amount"] > 0:
            ...
            cprint(
                f"✅ Market sell order placed! Order ID: {order.get('orderId')}",
                "green",
            )
            return order
        else:
            ...
            if quantity <= 0 or actual_notional < min_notional:
                ...
                cprint(
                    f"   💡 Need at least ${min_notional:.2f} position size", "yellow"
                )
                return None
            ...
            required_margin = amount / leverage
            cprint(
                f"📉 MARKET SELL (SHORT): {quantity} {symbol} @ ~${current_price:.2f}",
                "red",
            )
            cprint(
                f"💰 Notional Position: ${amount:.2f} | Margin Required: ${required_margin:.2f} ({leverage}x)",
...
def limit_sell(token, amount, slippage, leverage=DEFAULT_LEVERAGE):
    ...
    try:
        ...
        if position and position["position_amount"] > 0:
            ...
            cprint(
                f"📉 Closing LONG: {quantity} {symbol} @ MARKET (immediate exit)", "red"
            )
            cprint(f"💰 Closing ${amount:.2f} notional position", "cyan")
            ...
            required_margin = amount / leverage
            cprint(
                f"🚀 LIMIT SELL (SHORT): {quantity} {symbol} (chasing best ask)", "red"
            )
            cprint(
                f"💰 Notional Position: ${amount:.2f} | Margin Required: ${required_margin:.2f} ({leverage}x)",
                "cyan",
...
def chunk_kill(token_mint_address, max_usd_order_size, slippage):
    ...
    try:
        ...
        cprint(
            f"🔄 Closing position: {position_amt} {symbol} ({'LONG' if is_long else 'SHORT'})",
            "cyan",
        )
        ...
        cprint(
            f"📊 Closing in {num_chunks} chunks of ~{chunk_size_tokens} tokens",
            "yellow",
        )
        ...
        for i in range(num_chunks):
            ...
            if (
                ...
                or abs(current_position["position_amount"]) < 0.0001
            ):
        ...
        if not final_position or abs(final_position["position_amount"]) < 0.0001:
            ...
            cprint(
                f"⚠️  Position still has {final_position['position_amount']} remaining",
                "yellow",
            )
            return False
...
def ai_entry(
    symbol, amount, max_chunk_size=None, leverage=DEFAULT_LEVERAGE, use_limit=True
):
    ...
    try:
        ...
        cprint(
            f"🎯 AI Entry: ${amount} in {num_chunks} {order_type_str} chunks of ${chunk_size:.2f}",
            "cyan",
        )
        ...
        cprint(
            f"✅ AI Entry complete! ${amount} deployed across {num_chunks} {order_type_str} orders",
            "green",
        )
        return True
