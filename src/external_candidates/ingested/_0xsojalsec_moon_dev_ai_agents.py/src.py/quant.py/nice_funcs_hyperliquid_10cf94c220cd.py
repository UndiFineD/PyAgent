...
def get_sz_px_decimals(symbol):
    ...
    print(
        f"{symbol} price: {ask} | sz decimals: {sz_decimals} | px decimals: {px_decimals}"
    )
...
def get_position(symbol, account):
    ...
    for position in user_state["assetPositions"]:
        if (
            position["position"]["coin"] == symbol
            and float(position["position"]["szi"]) != 0
        ):
            ...
            print(
                f'{colored(f"{coin} position:", "green")} Size: {pos_size} | Entry: ${entry_px} | PnL: {pnl_perc:.2f}%'
            )
...
def cancel_all_orders(account):
    ...
    for order in open_orders:
        try:
            ...
            print(
                colored(
                    f'   ⚠️ Could not cancel {order["coin"]} order: {str(e)}', "yellow"
                )
            )
...
def limit_order(coin, is_buy, sz, limit_px, reduce_only, account):
    ...
    if isinstance(order_result, dict) and "response" in order_result:
        print(
            f"✅ Order placed with status: {order_result['response']['data']['statuses'][0]}"
        )
    else:
...
def kill_switch(symbol, account):
    ...
    order_result = exchange.order(
        symbol, side, abs_size, close_price, {"limit": {"tif": "Ioc"}}, reduce_only=True
    )
...
def pnl_close(symbol, target, max_loss, account):
    ...
    positions, im_in_pos, pos_size, pos_sym, entry_px, pnl_perc, is_long = get_position(
        symbol, account
    )
    ...
    print(
        f'Current PnL: {colored(f"{pnl_perc:.2f}%", "green" if pnl_perc > 0 else "red")}'
    )
    ...
    if pnl_perc >= target:
        print(
            colored(
                f"✅ Target reached! Closing position WIN at {pnl_perc:.2f}%",
                "green",
                attrs=["bold"],
            )
        )
        ...
        print(
            colored(
                f"🛑 Stop loss hit! Closing position LOSS at {pnl_perc:.2f}%",
                "red",
                attrs=["bold"],
            )
        )
...
def market_buy(symbol, usd_size, account):
    ...
    exchange = Exchange(account, constants.MAINNET_API_URL)
    order_result = exchange.order(
        symbol, True, pos_size, buy_price, {"limit": {"tif": "Ioc"}}, reduce_only=False
    )

    print(
        colored(f"✅ Market buy executed: {pos_size} {symbol} at ${buy_price}", "green")
    )
    return order_result
...
def market_sell(symbol, usd_size, account):
    ...
    print(
        f"   Position size: {pos_size} {symbol} (value: ${pos_size * sell_price:.2f})"
    )
    ...
    print(
        colored(f"✅ Market sell executed: {pos_size} {symbol} at ${sell_price}", "red")
    )
    return order_result
...
def close_position(symbol, account):
    ...
    positions, im_in_pos, pos_size, _, _, pnl_perc, is_long = get_position(
        symbol, account
    )
    ...
    print(
        f'Closing {"LONG" if is_long else "SHORT"} position with PnL: {pnl_perc:.2f}%'
    )
            ...
            if response.status_code == 200:
                ...
                if snapshot_data:
                    ...
                    if timestamp_offset is None:
                        latest_api_timestamp = datetime.datetime.utcfromtimestamp(
                            snapshot_data[-1]["t"] / 1000
                        )
                        system_current_date = datetime.datetime.utcnow()
                        expected_latest_timestamp = system_current_date
                        timestamp_offset = (
                            latest_api_timestamp - expected_latest_timestamp
                        )
                    ...
                    first_time = datetime.datetime.utcfromtimestamp(
                        snapshot_data[0]["t"] / 1000
                    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if (
                ...
                universe = {
                    coin["name"]: i for i, coin in enumerate(data[0]["universe"])
                }
    ...
    try:
        ...
        for symbol in test_symbols:
            ...
            if data:
                ...
                print(
                    f"Symbol: {symbol:8} | Hourly: {hourly_rate:7.4f}% | Annual: {annual_rate:7.2f}% | OI: {data['open_interest']:10.2f}"
                )
            else:
...
def get_token_balance_usd(token_mint_address, account):
    ...
    """
    try:
        positions, im_in_pos, pos_size, _, _, _, _ = get_position(
...
def ai_entry(
    symbol, amount, max_chunk_size=None, leverage=DEFAULT_LEVERAGE, account=None
):
...
def open_short(token, amount, slippage=None, leverage=DEFAULT_LEVERAGE, account=None):
    ...
    try:
        ...
        print(
            colored(
                f"💰 Notional Position: ${amount:.2f} | Margin Required: ${required_margin:.2f} ({leverage}x)",
                "cyan",
            )
        )
