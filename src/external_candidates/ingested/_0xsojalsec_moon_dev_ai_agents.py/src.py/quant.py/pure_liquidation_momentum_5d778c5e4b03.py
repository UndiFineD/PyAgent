...
if "long_liquidations" in data.columns and "short_liquidations" in data.columns:
    ...
    data["total_liq_count"] = (
        data["liquidation_count"] if "liquidation_count" in data.columns else 0
    )
...
class PureLiquidationMomentumStrategy(Strategy):
    ...
    def init(self):
        ...
        self.avg_long_liq = self.I(
            lambda x: pd.Series(x).rolling(24).mean(), self.long_liq_volume
        )
        self.avg_short_liq = self.I(
            lambda x: pd.Series(x).rolling(24).mean(), self.short_liq_volume
        )
...
if len(data) > 100:
    ...
    print(
        f"Liquidation Threshold: ${PureLiquidationMomentumStrategy.liquidation_threshold:,}"
    )
    ...
    try:
        optimization_results = bt.optimize(
            liquidation_threshold=range(
                100000, 1000000, 25_000
            ),  # $100k to $2M in $100k steps
        ...
        print("-" * 50)
        print(
            f"Liquidation Threshold: ${optimization_results._strategy.liquidation_threshold:,}"
        )
        ...
        print(
            "This may happen if there aren't enough trades with the current parameter ranges."
        )
