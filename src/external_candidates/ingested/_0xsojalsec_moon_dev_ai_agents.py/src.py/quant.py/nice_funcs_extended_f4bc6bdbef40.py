# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\quant\nice_funcs_extended.py
"""
...
try:
    ...
    cprint(
        "⚠️  Extended Exchange SDK not installed. Install with: pip install x10-perpetual-sdk",
        "yellow",
    )
    raise
...
DEFAULT_LEVERAGE = (
    20  # Default leverage for Extended Exchange (matches trading_agent.py)
)
...
class ExtendedExchangeAPI:
    ...
    def __init__(
        ...
        self.base_url = (
            ...
            if not USE_TESTNET
            else "https://api-testnet.extended.exchange"
        )
    ...
    def get_position(self, symbol: str) -> Dict:
        ...
        try:
            ...
            for pos in positions:
                # Get market name
                market_attr = (
                    getattr(pos, "market_name", None)
                    or getattr(pos, "market", None)
                    or getattr(pos, "symbol", None)
                )

                if market_attr == symbol:
                    mypos_size = float(pos.size)
                    entry_px = float(
                        getattr(pos, "entry_price", 0) or getattr(pos, "open_price", 0)
                    )
                    mark_px = float(getattr(pos, "mark_price", 0) or entry_px)

                    # Get PnL from Extended API
                    unrealized_pnl = float(
                        getattr(pos, "unrealised_pnl", 0)
                        or getattr(pos, "unrealized_pnl", 0)
                    )
                    ...
                    position_leverage = float(
                        getattr(pos, "leverage", DEFAULT_LEVERAGE)
                    )
                    actual_margin = (
                        position_value / position_leverage
                        if position_leverage > 0
                        else position_value
                    )
                    pnl_perc = (
                        (unrealized_pnl / actual_margin) * 100
                        if actual_margin > 0
                        else 0
                    )
        ...
        async def set_lev():
            try:
                return await self.trading_client.account.update_leverage(
                    market_name=symbol, leverage=leverage
                )
            except Exception as e:
    ...
    def buy_limit(
        self, symbol: str, quantity: float, price: float, leverage: int = None
    ) -> Dict:
        ...
        async def place_order():
            rounded_price = int(
                round(price)
            )  # Extended requires integer prices for BTC-USD
    ...
    def buy_market(self, symbol: str, quantity: float, leverage: int = None) -> Dict:
        ...
        async def place_order():
            ...
            orderbook = await self.trading_client.markets_info.get_orderbook_snapshot(
                market_name=symbol
            )
            ask_price = float(orderbook.data.ask[0].price) if orderbook.data.ask else 0
            aggressive_price = round(
                ask_price * 1.01
            )  # 1% above ask, rounded to integer
    ...
    def sell_market(self, symbol: str, quantity: float, leverage: int = None) -> Dict:
        ...
        async def place_order():
            ...
            aggressive_price = round(
                bid_price * 0.99
            )  # 1% below bid, rounded to integer
    ...
    def get_open_orders(self, symbol: str) -> List[Dict]:
        ...
        try:
            ...
            response = self._request(
                "GET", f"/api/v1/user/orders?market={symbol}&status=OPEN"
            )
                ...
                symbol_orders.append(
                    {
                        ...
                        "quantity": float(
                            order.get("quantity", 0) or order.get("amount", 0)
                        ),
    ...
    def usd_to_asset_size(self, symbol: str, usd_amount: float) -> float:
        ...
        try:
            ...
            async def get_prices():
                ...
                return {
                    "bid": (
                        float(orderbook.data.bid[0].price)
                        if orderbook.data.bid
                        else 0.0
                    ),
                    "ask": (
                        float(orderbook.data.ask[0].price)
                        if orderbook.data.ask
                        else 0.0
                    ),
                }
            ...
            cprint(
                f"💱 USD to Asset: ${usd_amount} → {asset_size} {symbol} @ ${mid_price:,.2f}",
                "cyan",
            )
            return asset_size
...
def market_buy(
    symbol: str, usd_amount: float, slippage=None, leverage: int = DEFAULT_LEVERAGE
):
...
def chunk_kill(
    symbol: str, max_chunk_size: float = 999999, slippage: int = None
) -> bool:
    """
    ...
    cprint(
        f"\n🔄 Moon Dev's Chunk Kill: Closing {symbol} position (MAKER orders)...",
        "yellow",
    )
    ...
    while True:  # No max attempts - MUST close position
        ...
        if not position or position["position_amount"] == 0:
            ...
            if not double_check or double_check["position_amount"] == 0:
                ...
                cprint(
                    f"⚠️  Position reappeared: {abs(double_check['position_amount'])} - continuing close",
                    "yellow",
                )
        ...
        if last_size is not None and remaining_size < last_size:
            cprint(
                f"📉 Partial fill detected: {last_size} → {remaining_size}", "yellow"
            )
        ...
        if close_price != last_price or remaining_size != last_size:
            ...
            cprint(
                f"📝 Close attempt {attempt}: {close_side} {remaining_size} @ ${close_price:,.2f} (maker)",
                "cyan",
            )
            ...
            try:
                if is_long:
                    ...
                    order_result = limit_sell(
                        symbol, usd_amount, close_price, slippage=slippage
                    )
                else:
                    # Closing short = buy limit
                    order_result = limit_buy(
                        symbol, usd_amount, close_price, slippage=slippage
                    )
...
def get_ticker(symbol: str) -> Dict:
    ...
    async def fetch_ticker():
        orderbook = await api.trading_client.markets_info.get_orderbook_snapshot(
            market_name=symbol
        )
        ...
        return {"bid": bid, "ask": ask, "mark_price": mark}
...
if __name__ == "__main__":
    ...
    if api:
        ...
        if in_pos:
            cprint(
                f"📊 Position: {'LONG' if is_long else 'SHORT'} {abs(size)} {test_symbol} @ ${entry:,.2f} | PnL: {pnl:.2f}%",
                "cyan",
            )
        else:
