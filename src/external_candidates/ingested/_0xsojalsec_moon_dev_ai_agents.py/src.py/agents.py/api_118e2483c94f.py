# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\api.py
"""
...
class MoonDevAPI:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"💥 Error fetching {filename} after {max_retries} attempts: {str(e)}"
                    )
                ...
                if df is not None:
                    ...
                    for col in df.columns:
                        try:
                            ...
                            if (
                                str(col).isdigit() and len(str(col)) == 13
                            ):  # Column name itself is 13-digit timestamp
                                timestamp_col = col
                                break
                            elif pd.api.types.is_numeric_dtype(df[col]):
                                sample_val = df[col].iloc[0]
                                if (
                                    isinstance(sample_val, (int, float))
                                    and sample_val > 1000000000000
                                ):  # Epoch milliseconds
                                    timestamp_col = col
                                    break
                        except:
                    ...
                    if timestamp_col:
                        ...
                        print(
                            f"🎯 Data sorted chronologically (oldest→newest like OHLCV)"
                        )
                    else:
                        print(
                            f"✅ Moon Dev: Got {len(df):,} records (timestamp column not identified)"
                        )
    ...
    def _parse_liquidation_chunk(self, chunk_bytes):
        ...
        try:
            ...
            lines = [
                line.strip() for line in chunk_str.strip().split("\n") if line.strip()
            ]
    ...
    def _download_complete_chunked_dataset(self):
        ...
        try:
            ...
            response = self.session.get(
                url, headers=self.headers, stream=True, timeout=(30, None)
            )
            response.raise_for_status()

            print(f"📡 Moon Dev: Connected! Status: {response.status_code}")
            print(
                f"📋 Transfer-Encoding: {response.headers.get('Transfer-Encoding', 'not chunked')}"
            )
                ...
                for chunk in response.iter_content(chunk_size=chunk_size):
                    if chunk:
                        ...
                        if chunk_count % 1000 == 0:
                            ...
                            print(
                                f"📡 Moon Dev: Downloaded {chunk_count} chunks ({mb_downloaded:.1f} MB so far)... 🌙"
                            )

            print(
                f"✅ Moon Dev: Download complete! {chunk_count} chunks, {total_bytes:,} total bytes"
            )
            ...
            if full_data_path.exists():
                ...
                print(
                    f"📊 Moon Dev: File saved successfully! Size: {file_size_mb:.1f} MB"
                )
            ...
            print(
                "⏰ Moon Dev: Request timed out - server may be processing large file"
            )
            ...
            print(
                f"📦 Moon Dev: Chunk encoding error - partial download may have occurred"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"💥 Error fetching oi.csv after {max_retries} attempts: {str(e)}"
                    )
                    print(f"📋 Stack trace:\n{traceback.format_exc()}")
                    return None
    ...
    def get_copybot_follow_list(self):
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/copybot/data/follow_list", headers=self.headers
            )
            ...
            if "403" in str(e):
                print(
                    "❗ Make sure your API key is set in the .env file and has the correct permissions"
                )
            return None
    ...
    def get_copybot_recent_transactions(self):
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/copybot/data/recent_txs", headers=self.headers
            )
