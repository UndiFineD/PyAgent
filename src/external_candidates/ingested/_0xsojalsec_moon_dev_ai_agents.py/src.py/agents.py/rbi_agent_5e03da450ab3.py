...
PROCESSED_IDEAS_LOG = (
    DATA_DIR / "processed_ideas.log"
)  # New file to track processed ideas
    ...
    if output:
        ...
        if "STRATEGY_NAME:" in output:
            try:
                ...
                cprint(
                    f"✅ Successfully extracted strategy name: {strategy_name}", "green"
                )
            except Exception as e:
...
def main():
    ...
    max_ideas = (
        int(max_ideas_env) if max_ideas_env and max_ideas_env.isdigit() else None
    )
    processed_count = 0
...
if __name__ == "__main__":
    try:
        ...
        cprint(
            f"📦 Package: {PACKAGE_CONFIG['type']} - {PACKAGE_CONFIG['name']}", "cyan"
        )
        main()
