...
def read_strategy(file_path: Path) -> str:
    with open(file_path, "r") as f:
        content = f.read()
    print(
        f"📝🌙 MOON DEV: Loaded strategy text from {file_path.name} ({len(content)} chars) 📚"
    )
    return content
...
def generate_backtest_code(strategy_text: str) -> str:
    ...
    for cfg in candidate_configs:
        print(
            f"🛰️🌙 MOON DEV: Attempting model {cfg['type']} :: {cfg['name']} for codegen ✨"
        )
        try:
...
def package_fix_code(code: str) -> str:
    print(
        "📦🌙 MOON DEV: Running package-check to remove forbidden backtesting.lib usage... 🧹"
    )
    ...
    for cfg in candidate_configs:
        print(
            f"🛰️🌙 MOON DEV: Package-fix model attempt {cfg['type']} :: {cfg['name']}"
        )
        try:
            raw = rbi_agent.chat_with_model(
...
def save_text_outputs(
    output_dir: Path, strategy_name: str, stdout: str, stderr: str
) -> None:
    ...
    with open(stderr_file, "w") as f:
        f.write(stderr or "")
    print(
        f"💾🌙 MOON DEV: Saved stdout/stderr to {stdout_file.name} / {stderr_file.name}"
    )
...
def run_one_strategy(research_dir: Path, file_path: Path, conda_env: str) -> None:
    ...
    if result.get("success"):
        ...
        print(
            "💥🌙 MOON DEV: Backtest still failing after auto-fixes. Needs manual review."
        )
