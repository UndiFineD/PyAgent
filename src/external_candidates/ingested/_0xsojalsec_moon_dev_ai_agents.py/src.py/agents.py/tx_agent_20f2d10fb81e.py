# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\tx_agent.py
"""
...
SOUND_ENABLED = True
AUTO_OPEN_BROWSER = (
    True  # Set to True to automatically open new transactions in browser
)
USE_DEXSCREENER = True  # Set to True to use DexScreener instead of Birdeye
DATA_FOLDER = (
    Path(__file__).parent.parent / "data" / "tx_agent"
)  # Folder for transaction data
...
class TxScanner:
    ...
    def display_past_transaction(self, row):
        ...
        print(
            f"{colored(f'{random_emoji} NEW TRANSACTION {time_str}', 'white', random_bg)} {display_link}"
        )
    ...
    def show_past_transactions(self):
        ...
        self.last_check_time = pd.to_datetime(
            recent_txs.iloc[-1]["blockTime"], unit="s"
        )
        ...
        while True:
            try:
                ...
                if self.last_check_time and current_time > self.last_check_time:
                    ...
                    new_df = df[
                        pd.to_datetime(df["blockTime"], unit="s") > self.last_check_time
                    ]
