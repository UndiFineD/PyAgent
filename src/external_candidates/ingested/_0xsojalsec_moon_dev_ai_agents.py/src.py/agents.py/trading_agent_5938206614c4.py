# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\trading_agent.py
"""
...
AI_MODEL_TYPE = (
    "xai"  # Options: 'groq', 'openai', 'claude', 'deepseek', 'xai', 'ollama'
)
...
USE_PORTFOLIO_ALLOCATION = (
    False  # True = Use AI for portfolio allocation across multiple tokens
)
# False = Simple mode - trade single token at MAX_POSITION_PERCENTAGE

MAX_POSITION_PERCENTAGE = (
    90  # % of account balance to use as MARGIN per position (0-100)
)
...
def monitor_position_pnl(token, check_interval=PNL_CHECK_INTERVAL):
    ...
    try:
        cprint(
            f"\n👁️  Monitoring {token} position for P&L targets...",
            "cyan",
            attrs=["bold"],
        )
        cprint(
        ...
        while True:
            ...
            if not position or (
                EXCHANGE in ["ASTER", "HYPERLIQUID"]
                and position.get("position_amount", 0) == 0
            ):
            ...
            if EXCHANGE in ["ASTER", "HYPERLIQUID"]:
                ...
                if pnl_pct <= -STOP_LOSS_PERCENTAGE:
                    ...
                    if position["position_amount"] > 0:
                        ...
                        n.limit_sell(
                            token, position_size, slippage=0, leverage=LEVERAGE
                        )
                    else:
    ...
    try:
        if EXCHANGE in ["ASTER", "HYPERLIQUID"]:
            ...
            if EXCHANGE == "ASTER":
                ...
                balance = balance_dict.get(
                    "total_equity", 0
                )  # Use total equity for trading
...
def calculate_position_size(account_balance):
    ...
    if EXCHANGE in ["ASTER", "HYPERLIQUID"]:
        ...
        cprint(
            f"   💎 Notional Position: ${notional_position:,.2f}",
            "cyan",
            attrs=["bold"],
        )
...
"""
                ...
                cprint(
                    f"⚠️ Market data is not a DataFrame: {type(market_data)}", "yellow"
                )
    ...
    def handle_exits(self):
        ...
        for _, row in self.recommendations_df.iterrows():
            ...
            if current_position > 0:
                ...
                if action == "SELL":
                    cprint(
                        f"🚨 SELL signal with position - CLOSING POSITION",
                        "white",
                        "on_red",
                    )
                    try:
                        cprint(
                            f"📉 Executing chunk_kill (${max_usd_order_size} chunks)...",
                            "yellow",
                        )
                        ...
                        cprint(
                            f"❌ Error closing position: {str(e)}", "white", "on_red"
                        )
                elif action == "NOTHING":
                    cprint(
                        f"⏸️  DO NOTHING signal - HOLDING POSITION", "white", "on_blue"
                    )
                ...
                if action == "SELL":
                    if LONG_ONLY:
                        cprint(
                            f"⏭️  SELL signal but NO POSITION to close",
                            "white",
                            "on_blue",
                        )
                        cprint(
                            f"📊 LONG ONLY mode: Can't open short, doing nothing",
                            "cyan",
                        )
                    else:
                        ...
                        cprint(
                            f"📉 SELL signal with no position - OPENING SHORT",
                            "white",
                            "on_red",
                        )
                        cprint(
                        ...
                        try:
                            ...
                            if hasattr(n, "open_short"):
                                cprint(
                                    f"📉 Executing open_short (${position_size:,.2f})...",
                                    "yellow",
                                )
                                n.open_short(
                                    token, position_size, slippage, leverage=LEVERAGE
                                )
                            else:
                            ...
                            cprint(
                                f"✅ Short position opened successfully!",
                                "white",
                                "on_green",
                            )
                        except Exception as e:
                            cprint(
                                f"❌ Error opening short position: {str(e)}",
                                "white",
                                "on_red",
                            )
                elif action == "NOTHING":
                    cprint(
                        f"⏸️  DO NOTHING signal with no position", "white", "on_blue"
                    )
                    ...
                    if USE_PORTFOLIO_ALLOCATION:
                        cprint(
                            f"📊 Portfolio allocation will handle entry",
                            "white",
                            "on_cyan",
                        )
                    else:
                        ...
                        cprint(
                            f"💰 Opening position at MAX_POSITION_PERCENTAGE",
                            "white",
                            "on_green",
                        )
                        try:
                            if EXCHANGE in ["ASTER", "HYPERLIQUID"]:
                                success = n.ai_entry(
                                    token, position_size, leverage=LEVERAGE
                                )
                            else:
    ...
    def run_trading_cycle(self, strategy_signals=None):
        ...
        try:
            ...
            if EXCHANGE in ["ASTER", "HYPERLIQUID"]:
                ...
                cprint(
                    f"🏦 Using SOLANA - Trading tokens: {MONITORED_TOKENS}", "yellow"
                )
            ...
            if USE_PORTFOLIO_ALLOCATION and len(buy_recommendations) > 0:
                ...
                cprint(
                    f"\n💰 Found {len(buy_recommendations)} BUY signal(s)",
                    "white",
                    "on_green",
                )
                cprint(
                    "📊 Portfolio allocation is DISABLED - positions opened in handle_exits",
                    "cyan",
                )
            else:
...
def main():
    ...
    while True:
        try:
            ...
            for token in (
                SYMBOLS if EXCHANGE in ["ASTER", "HYPERLIQUID"] else MONITORED_TOKENS
            ):
                if EXCHANGE in ["ASTER", "HYPERLIQUID"]:
                    position = n.get_position(token)
            ...
            if has_position and monitored_token:
                ...
                cprint(
                    f"\n🔍 Open position detected for {monitored_token}",
                    "yellow",
                    attrs=["bold"],
                )
