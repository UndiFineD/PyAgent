...
# 📁 File Paths
DISCOVERED_TOKENS_FILE = Path(
    "src/data/discovered_tokens.csv"
)  # Input from token discovery script
...
class AIAgent:
    ...
    def __init__(self, name: str, model: str):
        ...
        self.memory_file = Path(
            f"src/data/agent_memory/{name.lower().replace(' ', '_')}.json"
        )
        self.memory = {
            "analyzed_tokens": [],
            "promising_tokens": [],
        ...
        try:
            ...
            for field in required_fields:
                if field not in token_data:
                    token_data[field] = (
                        "Unknown" if field in ["name", "symbol", "token_id"] else 0
                    )
...
class ListingArbSystem:
    ...
    def get_ohlcv_data(self, token_id: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"⚠️ API returned status code {response.status_code} for {token_id}"
                )
            ...
            try:
                ...
                for entry in ohlcv_data[-10:]:  # Show last 10 entries for readability
                    timestamp = datetime.fromtimestamp(entry[0] / 1000).strftime(
                        "%Y-%m-%d %H:%M"
                    )
                    ...
                    formatted_data += (
                        f"{timestamp} | {open_price} | {high} | {low} | {close}\n"
                    )

                # Add some basic statistics
                prices = np.array(
                    [float(entry[4]) for entry in ohlcv_data]
                )  # Close prices
        ...
        if not self.analysis_log.empty:
            ...
            if not last_analysis.empty:
                ...
                if hours_since < HOURS_BETWEEN_RUNS:
                    print(
                        f"⏭️ Skipping {token_id} - Analyzed {hours_since:.1f} hours ago"
                    )
                    return False
        return True
    ...
    def analyze_tokens_parallel(self, tokens_batch):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            if tokens_to_analyze:
                ...
                futures = [
                    executor.submit(self.analyze_token, token)
    ...
    def analyze_token(self, token_data: Dict):
        ...
        try:
            ...
            if market_cap > MAX_MARKET_CAP:
                print(
                    f"⏭️ Skipping - Market cap above maximum (${MAX_MARKET_CAP:,.0f})"
                )
                return
            ...
            # Agent Two responds
            agent_two_analysis = self.agent_two.analyze(
                analysis_data, agent_one_analysis
            )
            ...
            try:
                ...
                print(
                    f"🔗 CoinGecko URL: https://www.coingecko.com/en/coins/{token_id}"
                )
    ...
    def run_analysis_cycle(self):
        ...
        try:
            ...
            discovered_tokens_df = discovered_tokens_df.sort_values(
            ...
            if AI_ANALYSIS_FILE.exists():
                ...
                if "coingecko_url" not in full_analysis.columns:
                    ...
                    if missing_urls.any():
                        print(
                            f"🔗 Adding missing URLs for {missing_urls.sum()} entries"
                        )
                        full_analysis.loc[missing_urls, "coingecko_url"] = (
                            full_analysis.loc[missing_urls, "token_id"].apply(
                                lambda x: f"https://www.coingecko.com/en/coins/{x}"
                            )
                        )
