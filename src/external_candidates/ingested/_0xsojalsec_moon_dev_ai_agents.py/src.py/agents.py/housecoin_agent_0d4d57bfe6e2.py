# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\housecoin_agent.py
#!/usr/bin/env python3
"""
...
STATE_FILE = os.path.join(
    project_root, "src", "data", "housecoin_agent", "housecoin_agent_state.json"
)
...
class HousecoinAgent:
    def __init__(self):
        ...
        cprint(
            "\n🏠 Initializing Housecoin DCA Agent with AI Decision Layer 🏠",
            "cyan",
            attrs=["bold"],
        )
        ...
        self.last_buy_time = (
            datetime.fromisoformat(self.state["last_buy_time"])
            if self.state["last_buy_time"]
            else None
        )

        # API keys
        self.birdeye_api_key = os.getenv("birdeye_api_key") or os.getenv(
            "BIRDEYE_API_KEY"
        )
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                if close_col:
                    ...
                    cprint(
                        f"✅ Price: ${current_price:.8f}, SMA: ${sma_20:.8f}", "green"
                    )
                    return current_price, sma_20
                else:
                    cprint(
                        f"❌ No close column found. Available columns: {df.columns.tolist()}",
                        "red",
                    )
...
"""
            ...
            if df_5min is not None:
                ...
                if close_col:
                    ...
                    technical_analysis = (
                        f"- 5-min SMA (20): ${df_5min.tail(20)[close_col].mean():.8f}\n"
                    )
            ...
            if response:
                ...
                if (
                    "BUY" in response_text
                    and "DONT" not in response_text
                    and "DON'T" not in response_text
                ):
    ...
    def execute_buy(self, amount, reason):
        ...
        try:
            cprint(
                f"\n💰 Executing buy for ${amount} worth of Housecoin",
                "green",
                attrs=["bold"],
            )
    ...
    def run(self):
        ...
        print("=" * 60)
        cprint(
            "⚠️ NOT FINANCIAL ADVICE - This may go to zero!", "yellow", attrs=["bold"]
        )
        cprint(f"Thesis: 1 House = 1 Housecoin", "magenta")
        cprint(f"Exit Target: ${EXIT_PRICE}", "yellow")
        cprint(
            f"Trading Hours: {TRADING_START_HOUR}AM-{TRADING_END_HOUR-12}PM ET", "cyan"
        )
        ...
        while True:
            try:
                ...
                if current_price >= EXIT_PRICE:
                    cprint(
                        f"\n🚀 EXIT PRICE REACHED! Current: ${current_price:.8f} Target: ${EXIT_PRICE}",
                        "yellow",
                        attrs=["bold", "blink"],
                    )
                ...
                if not self.is_trading_hours():
                    cprint(
                        f"\n🌙 Outside trading hours ({TRADING_START_HOUR}:00 AM - {TRADING_END_HOUR-12}:00 PM ET)",
                        "blue",
                    )
                ...
                print(f"20D-SMA: ", end="")
                cprint(
                    f"{sma_status} {((current_price/sma_20 - 1) * 100):+.1f}%",
                    sma_color,
                    end=" ",
                )
                ...
                if below_sma:
                    ...
                    if self.state["daily_spent"] >= BELOW_SMA_DAILY_CAP:
                        cprint(
                            f"⚠️ Daily cap reached (${BELOW_SMA_DAILY_CAP})", "yellow"
                        )
                    elif (
                        self.last_buy_time
                        and (datetime.now() - self.last_buy_time).seconds
                        < BELOW_SMA_BUY_MINUTES * 60
                    ):
                        seconds_until_next = (
                            BELOW_SMA_BUY_MINUTES * 60
                            - (datetime.now() - self.last_buy_time).seconds
                        )
                        cprint(
                            f"⏳ Next buy check in {seconds_until_next//60}m {seconds_until_next%60}s",
                            "yellow",
                        )
                    ...
                    if daily_low:
                        ...
                        if percent_from_low <= DAILY_LOW_THRESHOLD * 100:
                            if self.state["daily_spent"] >= ABOVE_SMA_DAILY_CAP:
                                cprint(
                                    f"⚠️ Daily cap reached (${ABOVE_SMA_DAILY_CAP})",
                                    "yellow",
                                )
                            else:
                                should_check_buy = True
                                ...
                                cprint(
                                    "📊 Strategy: NEAR DAILY LOW",
                                    "green",
                                    attrs=["bold"],
                                )
                        else:
                            cprint(
                                f"⏸️ Waiting ({percent_from_low:.1f}% from low, need < {DAILY_LOW_THRESHOLD*100}%)",
                                "yellow",
                            )
                ...
                if should_check_buy:
                    ...
                    if ai_approved:
                        ...
                        cprint(
                            f"\n🚀 Buying ~{estimated_housecoin:,} Housecoin for ${buy_amount}",
                            "green",
                            attrs=["bold"],
                        )
                        ...
                        cprint(
                            "\n🤖 AI rejected the buy signal. Waiting for better conditions...",
                            "yellow",
                        )
                ...
                if below_sma:
                    ...
                    time.sleep(
                        ABOVE_SMA_CHECK_MINUTES * 60
                    )  # Check every X minutes when above SMA
