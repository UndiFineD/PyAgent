...
ANALYSIS_CHECK_INTERVAL_SECONDS = (
    300  # How often to check for new markets to analyze (5 minutes)
)
...
REANALYSIS_HOURS = (
    8  # Re-analyze markets after this many hours (even if previously analyzed)
)
...
CONSENSUS_PICKS_CSV = os.path.join(
    DATA_FOLDER, "consensus_picks.csv"
)  # 🌙 Moon Dev - Top consensus picks only
...
class PolymarketAgent:
    ...
    def __init__(self):
        ...
        if USE_SWARM_MODE:
            ...
            cprint(
                f"🤖 Using single model: {AI_MODEL_PROVIDER}/{AI_MODEL_NAME}", "green"
            )
            self.model = ModelFactory().get_model(AI_MODEL_PROVIDER, AI_MODEL_NAME)
            self.swarm = None
    ...
    def _load_predictions(self):
        ...
        if os.path.exists(PREDICTIONS_CSV):
            try:
                ...
                cprint(
                    f"✅ Loaded existing predictions from {PREDICTIONS_CSV}", "green"
                )
                return df
            except Exception as e:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if topic == "activity" and msg_type == "orders_matched":
                    ...
                    if (
                        usd_amount >= MIN_TRADE_SIZE_USD
                        and not self.is_near_resolution(price)
                    ):
                        ...
                        trade_data = {
                            ...
                            "trader": payload.get(
                                "name", payload.get("pseudonym", "Unknown")
                            ),
                        }
    ...
    def fetch_historical_trades(self, hours_back=None):
        ...
        try:
            ...
            for trade in trades:
                ...
                # Filter by minimum amount and near-resolution prices
                if usd_amount >= MIN_TRADE_SIZE_USD and not self.is_near_resolution(
                    price
                ):
        ...
        for market_id, trade in unique_markets.items():
            try:
                ...
                if market_id in self.markets_df["market_id"].values:
                    ...
                    self.markets_df.loc[mask, "last_trade_timestamp"] = (
    ...
    def display_recent_markets(self):
        ...
        cprint("\n" + "=" * 80, "cyan")
        cprint(
            f"Total markets tracked: {len(self.markets_df)}", "green", attrs=["bold"]
        )
        cprint("=" * 80 + "\n", "cyan")
...
Provide predictions for each market in the specified format."""
        ...
        if USE_SWARM_MODE and self.swarm:
            ...
            if not swarm_result or not swarm_result.get("responses"):
                cprint(
                    "❌ No responses from swarm - all models failed or timed out", "red"
                )
                return
            ...
            consensus_text = self._calculate_polymarket_consensus(
                swarm_result, markets_to_analyze
            )
            ...
            try:
                ...
                prediction_record = {
                    ...
                    "xai_prediction": (
                        prediction_summary if AI_MODEL_PROVIDER == "xai" else "N/A"
                    ),
    ...
    def _save_swarm_predictions(
        self, analysis_run_id, analysis_timestamp, markets, swarm_result
    ):
        ...
        try:
                ...
                for line in lines:
                    ...
                    if "MARKET" in line_upper and ":" in line:
                        try:
                            ...
                            market_part = (
                                line_upper.split("MARKET")[1].split(":")[0].strip()
                            )
            ...
            for market_num, predictions in market_predictions.items():
                ...
                if 1 <= market_num <= len(markets_list):
                    ...
                    confidence = (
                        int((votes[majority] / total) * 100) if total > 0 else 0
                    )
            ...
            if new_records:
                # Add all new records
                self.predictions_df = pd.concat(
                    [self.predictions_df, pd.DataFrame(new_records)], ignore_index=True
                )
        ...
        try:
            ...
            if failed_models:
                ...
                for model_name in failed_models:
                    error = swarm_result["responses"][model_name].get(
                        "error", "Unknown"
                    )
    ...
    def _save_consensus_picks_to_csv(self, consensus_response, markets_df):
        ...
        try:
            ...
            for line in lines:
                ...
                # Extract Consensus
                    ...
                    consensus_match = re.search(
                        r"(\d+)\s+out\s+of\s+(\d+)", consensus_text
                    )
                    if consensus_match:
            ...
            consensus_df = pd.concat(
                [consensus_df, pd.DataFrame(records)], ignore_index=True
            )
            ...
            cprint(
                f"📁 Consensus picks CSV: {CONSENSUS_PICKS_CSV}", "cyan", attrs=["bold"]
            )
            ...
            try:
                ...
                for idx, row in self.markets_df.iterrows():
                    ...
                    if self.last_analysis_run_timestamp is None:
                        has_fresh_trade = (
                            not pd.isna(last_trade) and last_trade is not None
                        )
                    else:
                        try:
                            if not pd.isna(last_trade) and last_trade is not None:
                                trade_time = pd.to_datetime(last_trade)
                                last_run_time = pd.to_datetime(
                                    self.last_analysis_run_timestamp
                                )
                ...
                cprint(
                    f"   Total trades received: {self.total_trades_received}", "white"
                )
    ...
    def analysis_cycle(self):
        ...
        if is_first_run:
            ...
            cprint(
                f"   Need: {NEW_MARKETS_FOR_ANALYSIS} fresh eligible markets", "white"
            )
        ...
        should_analyze = (is_first_run and total_markets > 0) or (
            fresh_eligible_count >= NEW_MARKETS_FOR_ANALYSIS
        )
        ...
        while True:
            try:
                ...
                next_check = datetime.now() + timedelta(
                    seconds=ANALYSIS_CHECK_INTERVAL_SECONDS
                )
                cprint(
...
def main():
    ...
    cprint("\n" + "=" * 80, "cyan")
    cprint(
        "🌙 Moon Dev's Polymarket Agent - WebSocket Edition!", "cyan", attrs=["bold"]
    )
    cprint("=" * 80, "cyan")
    cprint(f"💰 Tracking trades over ${MIN_TRADE_SIZE_USD}", "yellow")
    cprint(
        f"🚫 Ignoring prices within {IGNORE_PRICE_THRESHOLD:.2f} of $0 or $1", "yellow"
    )
    cprint(
    ...
    if historical_trades:
        ...
        cprint(
            "⚠️ No historical trades found - will start fresh from WebSocket", "yellow"
        )
    ...
    status_thread = threading.Thread(
        target=agent.status_display_loop, daemon=True, name="Status"
    )
    analysis_thread = threading.Thread(
        target=agent.analysis_loop, daemon=True, name="Analysis"
    )
