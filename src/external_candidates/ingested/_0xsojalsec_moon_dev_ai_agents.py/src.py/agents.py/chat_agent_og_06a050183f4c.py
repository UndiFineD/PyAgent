...
RESTREAM_WEBSOCKET_URL = (
    "wss://chat.restream.io/embed/ws"  # Updated to match embed URL format
)
            ...
            try:
                chat_present = WebDriverWait(self.driver, 10).until(
                    EC.presence_of_element_located(
                        (By.CSS_SELECTOR, "yt-live-chat-app, #content")
                    )
                )
    ...
    def get_chat_messages(self):
        ...
        try:
            ...
            chat_container = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "yt-live-chat-item-list-renderer")
                )
            )
...
class YouTubeChatMonitor:
    ...
    def _init_fallback(self):
        ...
        if not self.scraper and USE_FALLBACK:
            cprint(
                "🔄 API quota exceeded - switching to Selenium fallback...", "yellow"
            )
        ...
        try:
            ...
            if response.get("items"):
                ...
                if response.get("items"):
                    ...
                    if chat_id:
                        cprint(
                            f"🎯 Found active live chat! ID: {chat_id[:20]}...", "green"
                        )
                        return chat_id
        ...
        while self.connected:
            try:
                ...
                messages = self.driver.find_elements(
                    By.CLASS_NAME, "message-info-container"
                )

                # Process only messages we haven't seen
                for msg in messages[-10:]:
                    try:
                        username = msg.find_element(
                            By.CLASS_NAME, "message-sender"
                        ).text.strip()
                        text = msg.find_element(
                            By.CLASS_NAME, "chat-text-normal"
                        ).text.strip()
                        ...
                        ai_response = None
                        if self.chat_agent:
                            ai_response = self.chat_agent.process_question(
                                username, text
                            )
...
class ChatAgent:
    def __init__(self):
        ...
        self.leaderboard_chat_interval = (
            LEADERBOARD_INTERVAL  # Use the constant we defined (10)
        )
...
"""
    ...
    def run(self):
        ...
        cprint(
            f"📝 Will process last {DEFAULT_INITIAL_CHATS} messages on startup", "cyan"
        )
            ...
            while True:
                try:
                    ...
                    if not self.youtube_monitor.live_chat_id:
                        chat_id = self.youtube_monitor.get_live_chat_id(
                            YOUTUBE_CHANNEL_ID
                        )
                        if chat_id:
                    ...
                    for msg in messages:
                        ...
                        if (
                            self.chat_count_since_last_leaderboard
...
def evaluate_chat_sentiment(message):
    ...
    if positive_score > negative_score:
        ...
        print(
            "🌙 ayo fam lets keep it positive, spam the 777s to increase the vibes in here"
        )
        return -1
    return 0
