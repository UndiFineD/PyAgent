# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\research_agent.py
"""
...
def animate_loading(duration=3, message="Generating idea", emoji="🌙"):
    ...
    while time.time() < end_time:
        ...
        bg_color = bg_colors[
            (i // 6) % len(bg_colors)
        ]  # Change background even less frequently
...
def generate_idea(model_config):
    ...
    try:
        ...
        emoji = random.choice(EMOJIS)
        cprint(
            f"🔮 Asking {model_config['name']} for trading ideas...",
            "yellow",
            "on_blue",
        )
        time.sleep(0.5)  # Pause for readability
        ...
        if not model:
            cprint(
                f"❌ Could not initialize {model_config['type']} model!",
                "white",
                "on_red",
            )
            return None
...
def log_idea(idea, model_config):
    ...
    for phase in MOON_PHASES:
        clear_line()
        print(
            f"\r{colored(' ' + phase + ' Saving to Moon Dev database... ', 'white', 'on_magenta')}",
            end="",
            flush=True,
        )
        time.sleep(0.3)  # Slower animation
...
def run_idea_generation_loop(interval=10):
    ...
    try:
        while True:
            ...
            cprint(
                f"📚 Loaded {len(existing_ideas)} existing ideas for duplicate checking",
                "white",
                "on_blue",
            )
            time.sleep(1)  # Pause for readability
            ...
            for i in range(10):  # Always exactly 10 seconds
                ...
                clear_line()
                print(
                    f"\r{colored(f' {emoji} Next idea in: {remaining} seconds ', 'white', bg)}",
                    end="",
                    flush=True,
                )
                time.sleep(1)
        ...
        cprint(
            "\n🌙 Thank you for using Moon Dev's Research Agent! 🌙",
            "white",
            "on_magenta",
        )
    except Exception as e:
...
def test_run(num_ideas=1, interval=10):
    ...
    time.sleep(0.5)  # Pause for readability
    cprint(
        f"🔄 Will generate {num_ideas} ideas with {interval} seconds interval", "cyan"
    )
    time.sleep(1)  # Pause for readability
