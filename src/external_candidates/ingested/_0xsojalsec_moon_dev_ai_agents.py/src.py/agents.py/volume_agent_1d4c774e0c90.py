# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\volume_agent.py
#!/usr/bin/env python3
"""
        ...
        with open(VOLUME_LOG, "r") as f:
            ...
            for row in rows:
                if row["timestamp"] == target_timestamp:
                    symbol = row["symbol"]
                    snapshot_24h[symbol] = {"volume_24h": float(row["volume_24h"])}
...
def calculate_changes(current_tokens, previous_data, data_24h=None):
    ...
    for i, token in enumerate(current_tokens):
        ...
        if data_24h and symbol in data_24h:
            ...
            if vol_24h_ago > 0:
                change_info["volume_change_24h"] = (
                    (current_volume - vol_24h_ago) / vol_24h_ago
                ) * 100
...
def display_changes(changes):
    ...
    for change in changes:
        ...
        price = (
            f"${change['current_price']:.4f}"
            if change["current_price"] < 10
            else f"${change['current_price']:.2f}"
        )
        ...
        signal_color = (
            "yellow" if "🆕NEW" in signals else "green" if signals else "white"
        )
    ...
    vol_accelerators = [
        c for c in changes if c["volume_change_4h"] and c["volume_change_4h"] > 50
    ]
    ...
    if big_movers:
        symbols = ", ".join(
            [f"{c['symbol']} ({c['change_24h']:+.1f}%)" for c in big_movers]
        )
        cprint(f"   🚀 24H Big Movers (>20%): {symbols}", "green")
    if vol_accelerators:
        symbols = ", ".join(
            [f"{c['symbol']} ({c['volume_change_4h']:+.1f}%)" for c in vol_accelerators]
        )
        cprint(f"   🔥 Volume Accelerators (>50%): {symbols}", "green")
    if climbers:
        symbols = ", ".join(
            [f"{c['symbol']} (↑{c['rank_change_4h']})" for c in climbers]
        )
...
"""
    ...
    for change in changes:
        ...
        if vol_chg_4h is not None:
            ...
            prompt += (
                f"   - 4H Volume Change: NEW ENTRY (wasn't in top 15 last check)\n"
            )
...
def display_swarm_results(result):
    ...
    for i, (provider, data) in enumerate(sorted_responses, 1):
        if data["success"]:
            ...
            if ai_label:
                cprint(
                    f"💬 {ai_label}: {provider_name} (Response Time: {data['response_time']:.2f}s)",
                    "yellow",
                    attrs=["bold"],
                )
            else:
                cprint(
                    f"💬 {provider_name} (Response Time: {data['response_time']:.2f}s)",
                    "yellow",
                    attrs=["bold"],
                )
            cprint(f"{'─' * 170}", "yellow")
    ...
    if "metadata" in result:
        ...
        cprint(
            f"   ✅ Successful Responses: {meta.get('successful_responses', 0)}/{meta.get('total_models', 0)}",
            "green",
        )
...
def display_data_table(changes):
    ...
    for change in changes:
        ...
        symbol = change["symbol"]
        price = (
            f"${change['current_price']:.6f}"
            if change["current_price"] < 1
            else f"${change['current_price']:.2f}"
        )
        volume = format_volume(change["current_volume"])
...
def run_check():
    ...
    cprint("\n" + "=" * 120, "magenta")
    cprint(
        f"🔄 VOLUME AGENT CHECK - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "magenta",
        attrs=["bold"],
    )
    cprint("=" * 120 + "\n", "magenta")
...
def run_continuous():
    ...
    try:
        while True:
            ...
            cprint(
                f"⏳ Sleeping for 4 hours... (Check #{iteration} complete)\n", "yellow"
            )
