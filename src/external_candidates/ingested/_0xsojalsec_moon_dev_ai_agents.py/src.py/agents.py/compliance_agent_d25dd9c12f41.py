# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\compliance_agent.py
"""
...
VIDEOS_DIR = Path(
    "/Users/md/Dropbox/dev/github/search-arbitrage/bots/compliance/tiktok_ads"
)
...
class ComplianceAgent:
    ...
    def _init_model(self):
        ...
        try:
            cprint(
                f"🤖 Initializing {MODEL_CONFIG['type']} model: {MODEL_CONFIG['name']}...",
                "cyan",
            )
    ...
    def extract_frames(self, video_path: Path, output_folder: Path) -> List[Path]:
        ...
        try:
            ...
            for frame_number in range(0, frame_count, frame_interval):
                ...
                if saved_count % 10 == 0:
                    cprint(
                        f"💫 Extracted {saved_count} frames... Moon Dev would be proud!",
                        "green",
                    )
    ...
    def analyze_compliance(
        self, ad_name: str, frames_dir: Path, transcript: str
    ) -> Dict:
        ...
        try:
            ...
            user_content = [
                {
                    "type": "text",
                    "text": "Please analyze this ad for compliance with Facebook's advertising guidelines.",
                },
                {"type": "text", "text": f"Ad Name: {ad_name}"},
                {
                    "type": "text",
                    "text": f"Analyzing {len(image_contents)} frames from the video:",
                },
                *image_contents,
            ...
            try:
                ...
                cprint(
                    f"Overall Assessment: {analysis.get('overall_assessment', 'No assessment provided.')}",
                    "white",
                    "on_blue",
                )
                cprint(
                    f"Moon Dev Message: {analysis.get('moon_dev_message', 'Moon Dev says: Keep those ads compliant! 🌙')}",
                    "white",
                    "on_blue",
                )
                ...
                cprint(
                    f"❌ Error parsing JSON from model response: {str(json_error)}",
                    "red",
                )
    ...
    def process_all_videos(self) -> None:
        ...
        try:
            ...
            if not video_files:
                ...
                cprint(
                    f"🌙 Moon Dev says: Add some videos and try again! 🌙", "magenta"
                )
                return
            ...
            for i, video_path in enumerate(video_files, 1):
                ...
                results.append({"video": video_path.name, "result": result})

                end_time = time.time()
                cprint(
                    f"⏱️ Processing time: {end_time - start_time:.2f} seconds", "cyan"
                )
            ...
            cprint(
                f"🌟 Moon Dev's Compliance Agent has completed all tasks! 🌟", "magenta"
            )
        except Exception as e:
    ...
    def _generate_summary_report(self, results: List[Dict]) -> None:
        ...
        try:
            ...
            summary_data = {
                ...
                "compliant_videos": sum(
                    1 for r in results if r.get("result", {}).get("is_compliant", False)
                ),
                "non_compliant_videos": sum(
                    1
                    for r in results
                    if not r.get("result", {}).get("is_compliant", False)
                ),
                "video_results": [],
            }
            ...
            for i, result in enumerate(results, 1):
                ...
                if "error" in analysis:
                    ...
                    if all_issues:
                        ...
                        if sorted_issues:
                            video_summary["top_issue"] = sorted_issues[0].get(
                                "issue", "Unknown issue"
                            )
            ...
            cprint(
                f"✅ Summary JSON report generated and saved to {summary_path}", "green"
            )
        except Exception as e:
