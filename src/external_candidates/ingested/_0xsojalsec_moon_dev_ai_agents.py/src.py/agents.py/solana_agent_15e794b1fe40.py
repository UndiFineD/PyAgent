# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\solana_agent.py
"""
    ...
    def analyze_token(self, token_address):
        ...
        try:
            ...
            if market_cap < MIN_MARKET_CAP:
                print(
                    f"💰 Market cap too low (${market_cap:,.2f}) for {token_address[:8]}"
                )
                return None

            if market_cap > MAX_MARKET_CAP:
                print(
                    f"💰 Market cap too high (${market_cap:,.2f}) for {token_address[:8]}"
                )
                return None
            ...
            if liquidity > MAX_LIQUIDITY:
                print(
                    f"💧 Liquidity too high (${liquidity:,.2f}) for {token_address[:8]}"
                )
                return None
            ...
            if buy_percentage < MIN_BUY_TX_PCT:
                print(
                    f"📈 Low buy ratio ({buy_percentage:.1f}%) for {token_address[:8]}"
                )
                return None
            ...
            try:
                ...
                print(
                    f"⚠️ Couldn't get security info for {token_address[:8]}: {str(e)}"
                )
                return None
