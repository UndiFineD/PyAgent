# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\websearch_agent.py
"""
...
def animate_loading(duration=2, message="Processing", emoji="🌙"):
    ...
    while time.time() < end_time:
        ...
        clear_line()
        print(
            f"\r{colored(f' {frame} {message} {emoji} ', color, bg_color)}",
            end="",
            flush=True,
        )
...
def setup_files():
    ...
    if not SEARCH_QUERIES_CSV.exists():
        cprint(
            f"📝 Creating search queries CSV at {SEARCH_QUERIES_CSV}",
            "yellow",
            "on_blue",
        )
    ...
    if not SEARCH_RESULTS_CSV.exists():
        cprint(
            f"📊 Creating search results CSV at {SEARCH_RESULTS_CSV}",
            "white",
            "on_magenta",
        )
        with open(SEARCH_RESULTS_CSV, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["timestamp", "search_query", "title", "url", "snippet", "content"]
            )

    if not STRATEGIES_INDEX_CSV.exists():
        cprint(
            f"📚 Creating strategies index CSV at {STRATEGIES_INDEX_CSV}",
            "cyan",
            "on_blue",
        )
    ...
    if not FINAL_STRATEGIES_INDEX_CSV.exists():
        cprint(
            f"✨ Creating final strategies index CSV at {FINAL_STRATEGIES_INDEX_CSV}",
            "white",
            "on_green",
        )
...
def check_duplicate_query(new_query: str, max_recent_checks: int = 10) -> bool:
    ...
    try:
        ...
        for i, old_query in enumerate(recent_queries, 1):
            ...
            if similarity > 0.70:
                cprint(
                    f"\n⚠️ DUPLICATE DETECTED! (Similarity: {similarity:.2%})",
                    "white",
                    "on_red",
                )
...
def generate_search_query_with_glm():
    ...
    try:
        ...
        cprint(
            f"  API Key: {'✓ Configured' if OPENROUTER_API_KEY else '✗ Missing'}",
        ...
        wrapped_prompt = textwrap.fill(
            SEARCH_QUERY_GENERATION_PROMPT, width=min(70, TERM_WIDTH)
        )
        ...
        cprint(
            "📥 API RESPONSE STATUS:",
            "white",
            "on_green" if response.status_code == 200 else "on_red",
        )
        cprint(
        ...
        if not search_query or len(search_query) < 5:
            reasoning = (
                response_json["choices"][0]["message"].get("reasoning", "").strip()
            )
            if reasoning:
                cprint(
                    "\n🔄 Content field empty, extracting from reasoning field...",
                    "yellow",
                )
        ...
        if not search_query or len(search_query) < 5:
            ...
            cprint(
                "This can happen if the model responds in non-English or with reasoning only.",
                "yellow",
            )
...
def search_web_with_openai(search_query: str):
    ...
    try:
        ...
        cprint(
            f"  API Key: {'✓ Configured' if OPENAI_API_KEY else '✗ Missing'}",
            "green" if OPENAI_API_KEY else "red",
        )
        print("─" * min(70, TERM_WIDTH))
        ...
        cprint(
            "\n⏳ Executing web search (this may take 10-30 seconds)...",
            "white",
            "on_blue",
        )
...
def extract_strategies_with_glm(
    md_filepath: Path, source_url: str = ""
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        if not raw_content or len(raw_content) < 10:
            ...
            if reasoning:
                cprint(
                    "\n🔄 Content field empty, using reasoning field instead...",
                    "yellow",
                )
...
def process_raw_file_for_extraction(raw_filepath: Path, source_url: str = ""):
    ...
    for i, strategy in enumerate(strategies, 1):
        filename = save_extracted_strategy(strategy, raw_filepath.name, i, source_url)
...
def run_search_cycle():
    ...
    try:
        ...
        if not search_query:
            cprint(
                "\n❌ Failed to generate search query. Aborting cycle.",
                "white",
                "on_red",
            )
            return False
        ...
        if is_duplicate:
            cprint(
                "\n⚠️ Duplicate query detected - trying to regenerate...",
                "white",
                "on_yellow",
            )
            time.sleep(2)
            ...
            if not search_query:
                cprint(
                    "\n❌ Failed to generate alternate search query. Aborting cycle.",
                    "white",
                    "on_red",
                )
                return False
            ...
            if is_duplicate_again:
                cprint(
                    "\n⚠️ Still a duplicate after retry. Skipping this cycle.",
                    "white",
                    "on_yellow",
                )
                return False
        ...
        if saved_files:
            print("\n" + "=" * min(70, TERM_WIDTH))
            cprint(
                f" 🧠 EXTRACTING STRATEGIES FROM {len(saved_files)} FILES 🧠 ",
                "white",
                "on_cyan",
            )
            print("=" * min(70, TERM_WIDTH))
            ...
            for file_data in saved_files:
                process_raw_file_for_extraction(file_data["filepath"], file_data["url"])
...
def run_continuous_search():
    ...
    time.sleep(0.5)
    cprint(
        "\n🌟 MOON DEV'S WEB SEARCH RESEARCH AGENT - CONTINUOUS MODE 🌟",
        "white",
        "on_magenta",
    )
    time.sleep(0.5)
    ...
    try:
        while True:
            ...
            for i in range(SLEEP_BETWEEN_SEARCHES):
                ...
                clear_line()
                print(
                    f"\r{colored(f' {emoji} Next search in: {remaining} seconds ', 'white', bg)}",
                    end="",
                    flush=True,
                )
                time.sleep(1)
        ...
        cprint(
            "\n👋 MOON DEV'S WEB SEARCH RESEARCH AGENT SHUTTING DOWN...",
            "white",
            "on_yellow",
        )
        cprint(f"Completed {cycle_count} search cycles", "cyan")
        cprint(
            "\n🌙 Thank you for using Moon Dev's Web Search Research Agent! 🌙",
            "white",
            "on_magenta",
        )
    except Exception as e:
