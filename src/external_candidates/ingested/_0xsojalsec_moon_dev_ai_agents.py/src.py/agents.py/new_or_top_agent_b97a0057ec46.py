# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\new_or_top_agent.py
"""
...
class NewOrTopAgent:
    ...
    def get_top_gainers(self) -> pd.DataFrame:
        ...
        try:
            print_spinner(
                "🚀 Fetching top gainers...", ROCKET_SEQUENCE, "cyan", "on_blue"
            )
            response = requests.get(
                f"{BASE_URL}/coins/top_gainers_losers",
                headers=self.headers,
                params={"vs_currency": "usd", "sparkline": "false"},
            )
            ...
            if response.status_code == 200:
                ...
                if not gainers.empty:
                    ...
                    # Add CoinGecko URL
                    gainers["coingecko_url"] = gainers["id"].apply(
                    ...
                    for _, coin in gainers.iterrows():
                        gain_str = (
                            f"+{coin['usd_24h_change']:,.2f}%"
                            if "usd_24h_change" in coin
                            else "N/A"
                        )
                        price_str = f"${coin['usd']:,.8f}" if "usd" in coin else "N/A"
                        print_fancy(
                            f"{coin['name']} ({coin['symbol'].upper()}): {gain_str} @ {price_str}",
                            "green",
                            "on_grey",
                            ["💰", "🚀", "📈"],
                        )
                ...
                print_fancy(
                    f"Error fetching top gainers: {response.text}",
                    "white",
                    "on_red",
                    ERROR_EMOJIS,
                )
    ...
    def get_new_coins(self) -> pd.DataFrame:
        ...
        try:
            ...
            response = requests.get(f"{BASE_URL}/coins/list/new", headers=self.headers)
            ...
            if response.status_code == 200:
                ...
                print_fancy(
                    f"Discovered {len(df)} new cosmic tokens! 🪐",
                    "cyan",
                    "on_grey",
                    SUCCESS_EMOJIS,
                )
                ...
                print_fancy(
                    f"Error fetching new coins: {response.text}",
                    "white",
                    "on_red",
                    ERROR_EMOJIS,
                )
    ...
    def analyze_coin(self, coin_data: Dict, source_type: str) -> str:
        ...
        try:
            ...
            if market_df is None or market_df.empty:
                print_fancy(
                    "No market data available!", "white", "on_red", ERROR_EMOJIS
                )
    ...
    def save_analysis(self, result: Dict):
        ...
        if result["recommendation"] == "BUY":
            ...
            print_fancy(
                f"💰 Added {result['name']} to AI Buys!",
                "white",
                "on_green",
                ["💰", "🚀", "📈"],
            )
            ...
            print_fancy(
                f"💾 Saved analysis for {result['name']}",
                "white",
                "on_green",
                SUCCESS_EMOJIS,
            )
            ...
            for _, coin in new_coins_df.iterrows():
                ...
                if coin_data:
                    ...
                    result = {
                        ...
                        "price_usd": coin_data.get("market_data", {})
                        .get("current_price", {})
                        .get("usd", 0),
