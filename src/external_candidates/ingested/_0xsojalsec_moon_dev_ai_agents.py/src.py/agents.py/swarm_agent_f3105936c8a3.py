# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-moon-dev-ai-agents\src\agents\swarm_agent.py
#!/usr/bin/env python3
"""
...
SWARM_MODELS = {
    ...
    "deepseek": (
        ...
        "deepseek",
        "deepseek-chat",
    ),  # DeepSeek Chat - Fast chat model (API)
    "xai": (
        True,
        "xai",
        "grok-4-fast-reasoning",
    ),  # Grok-4 fast reasoning ($0.20-$0.50/1M tokens)
    "openrouter_qwen": (
        True,
        "openrouter",
        "qwen/qwen3-max",
    ),  # Qwen 3 Max - Powerful reasoning ($1.00/$1.00 per 1M tokens)
    # 🔇 Disabled Models (uncomment to enable)
    "claude": (
        True,
        "claude",
        "claude-sonnet-4-5",
    ),  # Claude 4.5 Sonnet - Latest & Greatest!
    ...
    "openrouter_glm": (
        ...
        "openrouter",
        "z-ai/glm-4.6",
    ),  # GLM 4.6 - Zhipu AI reasoning ($0.50/$0.50 per 1M tokens)
...
DEFAULT_MAX_TOKENS = (
    2048  # Increased for model compatibility (Gemini/Groq/Qwen need 2048+ minimum)
)
...
CONSENSUS_REVIEWER_MODEL = (
    "deepseek",
    "deepseek-chat",
)  # Using DeepSeek Chat API (fast)
        ...
        for provider, (enabled, model_type, model_name) in self.models_config.items():
            ...
            try:
                ...
                if model:
                    ...
                    cprint(
                        f"⚠️ Could not initialize {provider}: {model_name}", "yellow"
                    )
    ...
    def _query_single_model(
        ...
        try:
            ...
            if system_prompt is None:
                system_prompt = (
                    "You are a helpful AI assistant providing thoughtful analysis."
                )
    ...
    def query(self, prompt: str, system_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=len(self.active_models)) as executor:
            ...
            try:
                for future in as_completed(futures, timeout=MODEL_TIMEOUT + 10):
                    ...
                    try:
                        ...
                        if response["success"]:
                            ...
                            cprint(
                                f"   ❌ {provider} failed: {response['error']}", "red"
                            )

                    except TimeoutError:
                        cprint(
                            f"   ⏰ {provider} timed out (>{MODEL_TIMEOUT}s) - skipping",
                            "yellow",
                        )
                ...
                cprint(
                    f"\n⏰ Overall timeout reached - some models didn't respond",
                    "yellow",
                )
                ...
                for future, provider in futures.items():
                    if provider not in all_responses:
                        cprint(
                            f"   ⏰ {provider} never responded - marking as timeout",
                            "red",
                        )
                        all_responses[provider] = {
                            "provider": provider,
                ...
                cprint(
                    f"✅ Continuing with {len([r for r in all_responses.values() if r['success']])} successful responses",
                    "green",
                )

        # Generate consensus review summary (with model mapping)
        consensus_summary, model_mapping = self._generate_consensus_review(
        ...
        result = {
            ...
            "metadata": {
                ...
                "successful_responses": sum(
                    1 for r in all_responses.values() if r["success"]
                ),
                "failed_responses": sum(
                    1 for r in all_responses.values() if not r["success"]
                ),
