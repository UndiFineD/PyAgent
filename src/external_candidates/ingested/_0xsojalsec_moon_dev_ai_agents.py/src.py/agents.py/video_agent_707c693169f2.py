...
def create_video_job(
    ...
    try:
        ...
        # Create job object
        job = VideoJob(
            job_id=video.id, prompt=prompt, status="queued", created_at=datetime.now()
        )
...
def save_video_bytes(video_bytes: bytes, job: VideoJob) -> str:
    ...
    try:
        ...
        safe_prompt = "".join(
            c if c.isalnum() or c in (" ", "-", "_") else "" for c in job.prompt[:30]
        )
            ...
            while attempts < max_attempts:
                ...
                if completed:
                    if job.status == "completed":
                        ...
                        cprint(
                            f"❌ Worker {worker_id} failed: {job.job_id[:16]}...", "red"
                        )
...
""",
    ...
    try:
        ...
        while True:
            ...
            if first_video:
                cprint(
                    "TYPE YOUR VIDEO IDEA BELOW:", "white", "on_green", attrs=["bold"]
                )
            else:
            ...
            if job:
                ...
                with jobs_lock:
                    queued = sum(1 for j in jobs.values() if j.status == "queued")
                    generating = sum(
                        1 for j in jobs.values() if j.status == "generating"
                    )
                    completed = sum(1 for j in jobs.values() if j.status == "completed")
