# Extracted from: C:\DEV\PyAgent\.external\AdaptixC2\Extenders\agent_beacon\src_beacon\files\hashes.py
#!/usr/bin/env python3
# -*- coding:utf-8 -*-
...
for f in libs.split("\n"):
    if len(f) == 0:
        ...
        print(
            "#define HASH_LIB_%s%s0x%x"
            % (f.upper().split(".")[0], (35 - len(f)) * " ", djb2w(f))
        )
