...
class WhisperSecrets:
    ...
    def detect_secrets(
        self, key: str, value: str, filepath: Path, breadcrumbs: list = []
    ) -> Optional[Secret]:
        if not key:
            key = ""
        else:
        ...
        yield from self.rules.check(
            key, value, filepath, self.foundlines[filepath.as_posix()]
        )
    ...
    def scan(self, filename: str) -> Optional[Secret]:
        ...
        yield from self.detect_secrets(
            "file", plugin.filepath.as_posix(), plugin.filepath
        )
        for ret in plugin.pairs():
            if len(ret) == 2:
                ...
                yield from self.detect_secrets(
                    key, value, plugin.filepath, breadcrumbs=breadcrumbs
                )
