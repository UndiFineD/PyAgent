# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-whispers\whispers\cli.py
...
def cli_parser() -> ArgumentParser:
    args_parser = ArgumentParser(
        "whispers", description=("Identify secrets and dangerous behaviours")
    )
    args_parser.add_argument(
        "-v", "--version", action="version", version=f"whispers {__version__}"
    )
    args_parser.add_argument(
        "-i",
        "--info",
    ...
    args_parser.add_argument(
        "-r", "--rules", default="all", help="comma-separated rule ID list"
    )
    args_parser.add_argument(
        "-s",
        "--severity",
        default="BLOCKER,CRITICAL,MAJOR,MINOR",
        help="severity levels to report",
    )
    args_parser.add_argument("-o", "--output", help="output file (.yml)")
    args_parser.add_argument(
        "-e", "--exitcode", default=0, type=int, help="exit code on success"
    )
