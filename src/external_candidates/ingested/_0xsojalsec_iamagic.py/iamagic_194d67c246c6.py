# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-IAMagic\IAMagic.py
import argparse
...
def list_buckets():
    ...
    try:
        ...
        if count == 0:
            ...
            print(
                "Looks like there are some buckets here :D be sure to check out https://github.com/Pyr0sec/S3scan to scan these buckets."
            )
    except botocore.exceptions.ClientError:
        ...
        for region in regions:
            ...
            for r in response["Reservations"]:
                ...
                if status == "running":
                    ...
                    print(
                        "\33[94m"
                        + f"id: {instance_id}, type: {instance_type}, az: {az}"
                        + "\33[0m"
                    )
                    cnt += 1
        if cnt == 0:
...
group.add_argument(
    "-id", "--access-key-id", help="Accepts AWS access key ID as an argument", type=str
)
parser.add_argument(
    "-key",
    "--secret-access-key",
    help="Accepts AWS Secret access key as an argument",
    type=str,
)
group.add_argument(
    "--profile",
    help="Used to specify an AWS profile on your system (like awscli), Uses default credentials if not specified any.",
    type=str,
)
parser.add_argument(
    "--enumerate",
    help="Further enumerates the credentials by Checking ",
    action="store_true",
    default=False,
)
