# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\definition\feature_derivations.py
...
class DerivedFeature(FeatureBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.input_features = (
            input_features if isinstance(input_features, List) else [input_features]
        )
    ...
    def validate_feature(self):
        ...
        for key_alias in self.key_alias:
            assert key_alias in input_feature_key_alias, (
                "key alias {} in derived feature {} must come from "
                "its input features key alias list {}".format(
