# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\definition\source.py
import copy
import json
...
class HdfsSource(Source):
    ...
    def __init__(
        ...
        super().__init__(
            name, event_timestamp_column, timestamp_format, registry_tags=registry_tags
        )
        self.path = path
...
class SnowflakeSource(Source):
    ...
    def __init__(
        ...
        if dbtable is not None and query is not None:
            raise RuntimeError(
                "Both dbtable and query are specified. Can only specify one.."
            )
    ...
    def _get_snowflake_path(
        self, dbtable: Optional[str] = None, query: Optional[str] = None
    ) -> str:
        """
