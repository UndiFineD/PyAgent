...
class _PreprocessingPyudfManager(object):
    ...
    @staticmethod
    def build_anchor_preprocessing_metadata(
        anchor_list: List[FeatureAnchor], local_workspace_dir
    ):
        ...
        client_udf_repo_path = os.path.join(
            local_workspace_dir, FEATHR_CLIENT_UDF_FILE_NAME
        )
        metadata_path = os.path.join(local_workspace_dir, FEATHR_PYSPARK_METADATA)
        pyspark_driver_path = os.path.join(
            local_workspace_dir, FEATHR_PYSPARK_DRIVER_FILE_NAME
        )
        ...
        for anchor in anchor_list:
            ...
            if preprocessing_func:
                _PreprocessingPyudfManager.persist_pyspark_udf_to_file(
                    preprocessing_func, local_workspace_dir
                )
                feature_names = [feature.name for feature in anchor.features]
                features_with_preprocessing = (
                    features_with_preprocessing + feature_names
                )
                feature_names.sort()
                ...
                if isinstance(anchor.source.preprocessing, str):
                    feature_names_to_func_mapping[string_feature_list] = (
                        _PreprocessingPyudfManager._parse_function_str_for_name(
                            anchor.source.preprocessing
                        )
                    )
                else:
                    # it's a callable function
                    feature_names_to_func_mapping[string_feature_list] = (
                        anchor.source.preprocessing.__name__
                    )
        ...
        feathr_pyspark_metadata_abs_path = os.path.join(
            local_workspace_dir, FEATHR_PYSPARK_METADATA
        )
    ...
    @staticmethod
    def write_feature_names_to_udf_name_file(
        feature_names_to_func_mapping, local_workspace_dir
    ):
    ...
    def prepare_pyspark_udf_files(feature_names: List[str], local_workspace_dir):
        ...
        if has_py_udf_preprocessing:
            ...
            pyspark_driver_template_abs_path = str(
                Path(
                    Path(__file__).parent / FEATHR_PYSPARK_DRIVER_TEMPLATE_FILE_NAME
                ).absolute()
            )
            client_udf_repo_path = os.path.join(
                local_workspace_dir, FEATHR_CLIENT_UDF_FILE_NAME
