# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\utils\config.py
...
def generate_config(
    ...
    **kwargs,
    ...
    _maybe_update_config_with_env_var(
        config, "SPARK_CONFIG__AZURE_SYNAPSE__WORKSPACE_DIR"
    )
    _maybe_update_config_with_env_var(config, "SPARK_CONFIG__DATABRICKS__WORK_DIR")
    _maybe_update_config_with_env_var(
        config, "SPARK_CONFIG__DATABRICKS__WORKSPACE_INSTANCE_URL"
    )
    _maybe_update_config_with_env_var(
        config, "SPARK_CONFIG__DATABRICKS__CONFIG_TEMPLATE"
    )

    config["project_config"]["project_name"] = project_name
    config["feature_registry"][
        "api_endpoint"
    ] = f"https://{resource_prefix}webapp.azurewebsites.net/api/v1"
    config["online_store"]["redis"][
        "host"
    ] = f"{resource_prefix}redis.redis.cache.windows.net"
...
def _set_azure_synapse_config(
    ...
    config["spark_config"]["azure_synapse"] = config["spark_config"].get(
        "azure_synapse", {}
    )
    ...
    if not config["spark_config"]["azure_synapse"].get("dev_url"):
        config["spark_config"]["azure_synapse"][
            "dev_url"
        ] = f"https://{resource_prefix}syws.dev.azuresynapse.net"
...
def _set_databricks_config(
    ...
    if not config["spark_config"]["databricks"].get("config_template"):
        ...
        config["spark_config"]["databricks"]["config_template"] = json.dumps(
            databricks_config
        )
...
def _verify_config(config: Dict):
    ...
    if config["spark_config"]["spark_cluster"] == "azure_synapse":
        ...
        if not os.environ.get("DATABRICKS_WORKSPACE_TOKEN_VALUE"):
            ...
            and config["spark_config"]["databricks"].get("workspace_instance_url")
            is None
        ):
