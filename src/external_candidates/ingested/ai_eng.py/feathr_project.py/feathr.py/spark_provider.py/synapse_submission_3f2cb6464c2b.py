# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\spark_provider\_synapse_submission.py
import json
import os
import pathlib
import re
...
class _FeathrSynapseJobLauncher(SparkJobLauncher):
    ...
    def upload_or_get_cloud_path(
        ...
        """
        if local_path_or_cloud_src_path.startswith(
            "abfs"
        ) or local_path_or_cloud_src_path.startswith("wasb"):
            if tar_dir_path is None or not (
                tar_dir_path.startswith("abfs") or tar_dir_path.startswith("wasb")
            ):
                raise RuntimeError(
            ...
            self._datalake.upload_file_to_workdir(
                tem_dir_obj.name, tar_dir_path, dir_client
            )
            logger.info(
                "{} is uploaded to location: {}",
        ...
        logger.info(
            "{} is uploaded to location: {}", local_path_or_cloud_src_path, res_path
        )
        return res_path
    ...
    def submit_feathr_job(
        ...
        if main_jar_path:
            ...
            if main_jar_path.startswith("abfs"):
                ...
                logger.info(
                    "Cloud path {} is used for running the job: {}",
                    main_jar_path,
                    job_name,
                )
            else:
                logger.info(
                    "Uploading jar from {} to cloud for running job: {}",
                    main_jar_path,
                    job_name,
                )
                main_jar_cloud_path = self._datalake.upload_file_to_workdir(
                    main_jar_path
                )
                logger.info(
                    "{} is uploaded to {} for running job: {}",
                    main_jar_path,
                    main_jar_cloud_path,
                    job_name,
                )
        ...
        logger.info(
            "See submitted job here: https://web.azuresynapse.net/en-us/monitoring/sparkapplication"
        )
    ...
    def wait_for_completion(self, timeout_seconds: Optional[float]) -> bool:
        ...
        while (timeout_seconds is None) or (time.time() - start_time < timeout_seconds):
            ...
            if status in {LivyStates.SUCCESS.value}:
                ...
                LivyStates.ERROR.value,
                LivyStates.DEAD.value,
                LivyStates.KILLED.value,
            }:
                logger.error("Feathr job has failed.")
                error_msg = self._api.get_driver_log(self.current_job_info.id).decode(
                    "utf-8"
                )
...
class _SynapseJobRunner(object):
    ...
    def __init__(
        ...
        if credential is None:
            logger.warning(
                "No valid Azure credential detected. Using DefaultAzureCredential"
            )
    ...
    def create_spark_batch_job(
        ...
        return self.client.spark_batch.create_spark_batch_job(
            spark_batch_job_options, detailed=True
        )
    ...
    def get_driver_log(self, job_id) -> str:
        ...
        url = (
            "%s/sparkhistory/api/v1/sparkpools/%s/livyid/%s/applications/%s/driverlog/stderr/?isDownload=true"
            % (
        ...
        req = urllib.request.Request(
            url=url, headers={"authorization": "Bearer %s" % token}
        )
...
class _DataLakeFiler(object):
    ...
    def __init__(self, datalake_dir, credential=None):
        ...
        self.datalake_dir = (
            datalake_dir + "/" if datalake_dir[-1] != "/" else datalake_dir
        )
    ...
    def upload_file_to_workdir(
        ...
        elif src_parse_result.scheme.startswith(
            "abfs"
        ) or src_parse_result.scheme.startswith("wasb"):
            # passed a cloud path
            logger.info(
                "Skip uploading file {} as it's already in the cloud", src_file_path
            )
            ...
            if os.path.isdir(src_file_path):
                ...
                if tar_dir_client is not None:
                    ...
                    for item in Path(src_file_path).iterdir():
                        returned_path = self.upload_file(
                            item.resolve(), tar_dir_path, tar_dir_client
                        )
    ...
    def upload_file(
        ...
        file_client = (
            self.dir_client.create_file(file_name)
            if tar_dir_client is None
            else tar_dir_client.create_file(file_name)
        )
        returned_path = (
            self.datalake_dir + file_name
            if tar_dir_path == ""
            else tar_dir_path + file_name
        )
    ...
    def download_file(
        self, target_adls_directory: str, local_dir_cache: str, file_name: str = None
    ):
        """
        ...
        if file_name is not None:
            ...
            logger.info(
                "Finish downloading file {} from {} to {}.",
                file_name,
        ...
        result_paths = [
            ...
            for file_path in self.file_system_client.get_paths(
        ...
        for folder in result_folders:
            ...
            local_paths = [
                os.path.join(local_dir_cache, file_name) for file_name in file_in_folder
            ]
            self._download_file_list(local_paths, file_in_folder, directory_client)

        # download files that are in the result folder
        local_paths = [
            os.path.join(local_dir_cache, file_name) for file_name in result_paths
        ]
        self._download_file_list(local_paths, result_paths, directory_client)

        logger.info(
            "Finish downloading files from {} to {}.",
            target_adls_directory,
            local_dir_cache,
        )

    def _download_file_list(
        self, local_paths: List[str], result_paths, directory_client
    ):
        """
        Download filelist to local
        """
        for idx, file_to_write in enumerate(
            tqdm(result_paths, desc="Downloading result files: ")
        ):
            try:
