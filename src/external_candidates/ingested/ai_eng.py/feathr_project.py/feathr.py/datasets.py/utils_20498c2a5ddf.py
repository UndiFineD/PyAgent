# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\datasets\utils.py
"""Dataset utilities"""
...
def maybe_download(src_url: str, dst_filepath: str, expected_bytes=None) -> bool:
    ...
    if response.status_code == 200:
        ...
        if expected_bytes is not None and expected_bytes != dst_filepath.stat().st_size:
            ...
            dst_filepath.unlink()
            raise IOError(
                f"Failed to verify {str(dst_filepath)}. Maybe interrupted while downloading?"
            )
        else:
            return True
