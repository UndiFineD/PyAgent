# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\registry\registry_utils.py
import inspect
from re import sub
...
def _correct_function_indentation(user_func: str) -> str:
    ...
    leading_space_num = len(temp_udf_source_code[0]) - len(
        temp_udf_source_code[0].lstrip()
    )
    # strip the lines to make sure the function has the correct indentation
    udf_source_code_striped = [
        line[leading_space_num:] for line in temp_udf_source_code
    ]
