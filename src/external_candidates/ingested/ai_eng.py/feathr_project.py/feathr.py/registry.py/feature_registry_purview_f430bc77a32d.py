# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\registry\_feature_registry_purview.py
import glob
import inspect
import itertools
import os
...
def _to_snake(d, level: int = 0):
    ...
    if isinstance(d, list):
        d = d[:100]
        return [
            _to_snake(i, level + 1) if isinstance(i, (dict, list)) else i for i in d
        ]
    if len(d) > 100:
        raise ValueError("Dict has too many keys")
    return {
        _to_snake(a, level + 1): (
            _to_snake(b, level + 1) if isinstance(b, (dict, list)) else b
        )
        for a, b in d.items()
    }
...
class _PurviewRegistry(FeathrRegistry):
    ...
    def _parse_anchors(self, anchor_list: List[FeatureAnchor]) -> List[AtlasEntity]:
        ...
        for anchor in anchor_list:
            ...
            anchor_fully_qualified_name = (
                self.project_name + self.registry_delimiter + anchor.name
            )
            original_id = self.get_feature_id(
                anchor_fully_qualified_name, type=TYPEDEF_ANCHOR
            )
            original_anchor = (
                self.get_feature_by_guid(original_id) if original_id else None
            )
            merged_elements = self._merge_anchor(
                original_anchor, anchor_feature_entities
            )
    ...
    def _merge_anchor(
        self, original_anchor: Dict, new_anchor: Dict
    ) -> List[Dict[str, any]]:
        """
        ...
        if not original_anchor:
            ...
            original_anchor_elements = [
                x for x in original_anchor["entity"]["attributes"]["features"]
            ]
    ...
    def _parse_source(
        self, source: Union[Source, HdfsSource, JdbcSource, SnowflakeSource]
    ) -> AtlasEntity:
        """
        ...
        if isinstance(source, JdbcSource):
            attrs = {
                "type": (
                    INPUT_CONTEXT if input_context else urlparse(source.path).scheme
                ),
    ...
    def _add_all_derived_features(
        self, derived_features: List[DerivedFeature], ts: TopologicalSorter
    ) -> None:
    ...
    def _parse_derived_features(
        self, derived_features: List[DerivedFeature]
    ) -> List[AtlasEntity]:
        ...
        for derived_feature in toposorted_derived_feature_list:
            ...
            input_feature_entity_list: List[AtlasEntity] = [
                self.global_feature_entity_dict[f.name]
                for f in derived_feature.input_features
            ]
            key_list = []
            ...
            self.global_feature_entity_dict[derived_feature.name] = (
    ...
    def _parse_features_from_context(
        self, workspace_path: str, anchor_list, derived_feature_list
    ):
        """
        ...
        try:
            ...
            if j["typeName"] == response["typeName"]:
                if j["typeName"] == "Process":
                    ...
                    keys = set(
                        [_to_snake(key) for key in j["attributes"].keys()]
                    ) - set(["qualified_name"])
    ...
    def _generate_relation_pairs(
        self, from_entity: dict, to_entity: dict, relation_type
    ):
        type_lookup = {
    ...
    def _create_source(self, s: Source) -> UUID:
        ...
        qualified_name = self.registry_delimiter.join(
            [project_entity["attributes"]["qualifiedName"], attrs["name"]]
        )
        source_entity = AtlasEntity(
            name=attrs["name"],
        ...
        [
            ...
            for x in project_contains_source_relation
        ]
        return source_id
    ...
    def _create_anchor(self, s: FeatureAnchor) -> UUID:
        ...
        source_entity = self.purview_client.get_entity(
            qualifiedName=self.registry_delimiter.join(
                [self.project_name, s.source.name]
            ),
            typeName=TYPEDEF_SOURCE,
        )["entities"][0]
        attrs = anchor_to_def(s)
        qualified_name = self.registry_delimiter.join(
            [self.project_name, attrs["name"]]
        )
        anchor_entity = AtlasEntity(
            name=s.name,
            qualified_name=qualified_name,
    ...
    def _create_anchor_feature(
        self, anchor_id: str, source: Source, s: Feature
    ) -> UUID:
        """
        ...
        # change from AtlasEntity to Entity
        anchor_feature_entity = self.purview_client.get_entity(anchor_feature_id)[
            "entities"
        ][0]
    ...
    def _create_derived_feature(self, s: DerivedFeature) -> UUID:
        ...
        """
        input_features = [
            self.purview_client.get_entity(x._registry_id)["entities"][0]
            for x in s.input_features
        ]
        ...
        derived_feature_id = self.upload_single_entity_to_purview(
            derived_feature_entity
        )

        # change from AtlasEntity to Entity
        derived_feature_entity = self.purview_client.get_entity(derived_feature_id)[
            "entities"
        ][0]
        ...
        [
            self.upload_single_entity_to_purview(x)
            for x in feature_project_contain_belong_pairs + consume_produce_pairs
        ]
    ...
    def list_registered_features(
        self, project_name: str, limit=1000, starting_offset=0
    ) -> List[Dict[str, str]]:
        """
    ...
    def get_feature_by_fqdn_type(self, qualifiedName, typeName):
        ...
        for entity in entities:
            if (
                entity.get("typeName") == typeName
                and entity.get("attributes").get("qualifiedName") == qualifiedName
            ):
                return entity
    ...
    def get_features_from_registry(
        ...
        """
        project_entity = self.purview_client.get_entity(
            qualifiedName=project_name, typeName=TYPEDEF_FEATHR_PROJECT
        )["entities"][0]

        single_direction_process = [
            entity
            for _, entity in self.purview_client.get_entity_lineage(
                project_entity["guid"]
            )["guidEntityMap"].items()
        ...
        for derived_feature_entity in [
            x
            for x in entities_under_project
            if x["typeName"] == TYPEDEF_DERIVED_FEATURE
        ]:
            ...
            def search_for_input_feature(elem, full_relations, full_entities):
                ...
                input_features = [
                    ...
                    if x["typeName"] == TYPEDEF_ANCHOR_FEATURE
            ...
            all_input_features = search_for_input_feature(
                derived_feature_entity, consume_relations, entities_dict
            )
            derived_feature_list.append(
                DerivedFeature(
                    name=derived_feature_entity["attributes"]["name"],
                    feature_type=self._get_feature_type_from_hocon(
                        derived_feature_entity["attributes"]["type"]
                    ),
                    transform=self._get_transformation_from_dict(
        ...
        for anchor_entity in [
            x for x in entities_under_project if x["typeName"] == TYPEDEF_ANCHOR
        ]:
            consume_items_under_anchor = [
                entities_dict[x[1]]
                for x in consume_relations
                if x[0] == anchor_entity["guid"]
            ]
            source_entity = [
                x for x in consume_items_under_anchor if x["typeName"] == TYPEDEF_SOURCE
            ][0]
            ...
            anchor_features_guid = [
                x["guid"]
                for x in contain_items_under_anchor
                if x["typeName"] == TYPEDEF_ANCHOR_FEATURE
            ]
            ...
            anchor_list.append(
                FeatureAnchor(
                    ...
                    source=HdfsSource(
                        ...
                        event_timestamp_column=source_entity["attributes"][
                            "event_timestamp_column"
                        ],
                        timestamp_format=source_entity["attributes"][
                            "timestamp_format"
                        ],
                        preprocessing=self._correct_function_indentation(
                            source_entity["attributes"]["preprocessing"]
                        ),
                    ...
                    features=self._get_features_by_guid_or_entities(
                        guid_list=anchor_features_guid,
                        entity_list=entities_under_project,
                    ),
    ...
    def search_input_anchor_features(
        self, derived_guids, feature_entity_guid_mapping
    ) -> List[str]:
        """
        ...
        while len(stack) > 0:
            ...
            new_derived_features = [
                x["guid"] for x in current_input["attributes"]["input_derived_features"]
            ]
            new_anchor_features = [
                x["guid"] for x in current_input["attributes"]["input_anchor_features"]
            ]
    ...
    def _get_feature_type_from_hocon(self, input_str: str) -> FeatureType:
        ...
        valType = (
            conf.get_string("valType")
            if "valType" in conf
            else conf.get_string("type.valType")
        )
        dimensionType = (
            conf.get_string("dimensionType")
            if "dimensionType" in conf
            else conf.get_string("type.dimensionType")
        )
    ...
    def _get_transformation_from_dict(self, input: Dict) -> FeatureType:
        if "transformExpr" in input:
            ...
            agg_expr = (
                input["def_expr"]
                if "def_expr" in input
                else (input["defExpr"] if "defExpr" in input else None)
            )
            agg_func = (
                input["agg_func"]
                if "agg_func" in input
                else (input["aggFunc"] if "aggFunc" in input else None)
            )
            window = input["window"] if "window" in input else None
            group_by = (
                input["group_by"]
                if "group_by" in input
                else (input["groupBy"] if "groupBy" in input else None)
            )
            ...
            return WindowAggTransformation(
                agg_expr, agg_func, window, group_by, filter, limit
            )
        else:
    ...
    def _get_features_by_guid_or_entities(
        ...
        """
        if not entity_list:
            feature_entities = self.purview_client.get_entity(guid=guid_list)[
                "entities"
            ]
        else:
