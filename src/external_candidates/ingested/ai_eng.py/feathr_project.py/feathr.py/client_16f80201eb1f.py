# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\feathr\client.py
import base64
import copy
...
from feathr.definition.settings import ConflictsAutoCorrection, ObservationSettings
...
class FeathrClient(object):
    ...
    def __init__(
        ...
        if not (
            self.s3_enabled
            or self.adls_enabled
            or self.wasb_enabled
            or self.jdbc_enabled
            or self.snowflake_enabled
        ):
        ...
        if self.spark_runtime not in {"azure_synapse", "databricks", "local"}:
            ...
            # their local env.
            self._FEATHR_JOB_JAR_PATH = self.env_config.get(
        ...
        azure_purview_name = self.env_config.get(
        ...
        if registry_endpoint:
            self.registry = _FeatureRegistry(
                ...
                project_tags=project_registry_tag,
                credential=credential,
            )
        elif azure_purview_name:
            registry_delimiter = self.env_config.get(
    ...
    def register_features(self, from_context: bool = True):
        ...
        if from_context:
            ...
            if "anchor_list" in dir(self) and "derived_feature_list" in dir(self):
                ...
                raise RuntimeError(
                    "Please call FeathrClient.build_features() first in order to register features"
                )
        else:
            self.registry.register_features(
                self.local_workspace_dir, from_context=from_context
            )
    ...
    def build_features(
        ...
        for anchor in anchor_list:
            ...
            if anchor.source.name in source_names and (
                anchor.source is not source_names[anchor.source.name]
            ):
                raise RuntimeError(
        ...
        _PreprocessingPyudfManager.build_anchor_preprocessing_metadata(
            anchor_list, self.local_workspace_dir
        )
        self.config_helper.save_to_feature_config_from_context(
    ...
    def get_snowflake_path(
        self, database: str, schema: str, dbtable: str = None, query: str = None
    ) -> str:
        """
    ...
    def get_online_features(
        self, feature_table: str, key: Any, feature_names: List[str]
    ):
    ...
    def multi_get_online_features(
        self, feature_table: str, keys: List[Any], feature_names: List[str]
    ):
        ...
        for raw_feature in feature_list:
            if raw_feature:
                ...
                if feature_value.WhichOneof("FeatureValueOneOf") == "boolean_value":
                    ...
                    feature_value.WhichOneof("FeatureValueOneOf")
                    == "sparse_string_array"
                ):
                    typed_result.append(
                        (
                    ...
                    feature_value.WhichOneof("FeatureValueOneOf") == "sparse_bool_array"
                ):
                    typed_result.append(
                        (
                            feature_value.sparse_bool_array.index_integers,
                    ...
                    typed_result.append(
                        (
                            feature_value.sparse_float_array.index_integers,
                    ...
                    feature_value.WhichOneof("FeatureValueOneOf") == "sparse_long_array"
                ):
                    typed_result.append(
                        (
                            feature_value.sparse_long_array.index_integers,
    ...
    def get_offline_features(
        ...
        execution_configurations: Union[
            SparkExecutionConfiguration, Dict[str, str]
        ] = {},
        ...
        """
        feature_queries = (
            feature_query if isinstance(feature_query, List) else [feature_query]
        )
        feature_names = []
        ...
        if (
            len(feature_names) > 0
            and observation_settings.conflicts_auto_correction is None
        ):
            ...
            if (
                dataset_column_names_from_path is None
                or len(dataset_column_names_from_path) == 0
                ...
                if (
                    dataset_column_names_from_path is not None
                    and len(dataset_column_names_from_path) > 0
                ):
                ...
                if len(conflict_names) != 0:
                    ...
                    raise RuntimeError(
                        f"Feature names exist conflicts with dataset column names: {conflict_names}"
                    )

        udf_files = _PreprocessingPyudfManager.prepare_pyspark_udf_files(
            feature_names, self.local_workspace_dir
        )
        ...
        if "anchor_list" in dir(self) and "derived_feature_list" in dir(self):
            ...
            raise RuntimeError(
                "Please call FeathrClient.build_features() first in order to get offline features"
            )
    ...
    def _get_offline_features_with_config(
        ...
        """
        cloud_udf_paths = [
            self.feathr_spark_launcher.upload_or_get_cloud_path(udf_local_path)
            for udf_local_path in udf_files
        ]
        ...
        if (
            execution_configurations is not None
            and OUTPUT_FORMAT in execution_configurations
        ):
        ...
        return self.feathr_spark_launcher.submit_feathr_job(
            ...
            arguments=[
                "--join-config",
                self.feathr_spark_launcher.upload_or_get_cloud_path(
    ...
    def _valid_materialize_keys(self, features: List[str], allow_empty_key=False):
        ...
        for feature in features:
            ...
            if ",".join(new_keys) == "NOT_NEEDED" and not allow_empty_key:
                self.logger.error(
                    f"Empty feature key is not allowed for features: {features}"
                )
                return False
            if keys is None:
                ...
                if len(keys) != len(new_keys):
                    self.logger.error(
                        f"Inconsistent feature keys. Current keys are {str(keys)}"
                    )
                    return False
    ...
    def materialize_features(
        ...
        if len(feature_list) > 0:
            if "anchor_list" in dir(self):
                anchors = [
                    anchor
                    for anchor in self.anchor_list
                    if isinstance(anchor.source, InputContext)
                ]
                anchor_feature_names = set(
                    feature.name for anchor in anchors for feature in anchor.features
                )
            ...
            for fn in feature_list:
                ...
                for anchor in self.anchor_list:
                    for feature in anchor.features:
                        if feature.name == fn and not isinstance(
                            feature.transform, WindowAggTransformation
                        ):
                            raise RuntimeError(
        ...
        for end in settings.get_backfill_cutoff_time():
            ...
            config_file_name = "feature_gen_conf/auto_gen_config_{}.conf".format(
                end.timestamp()
            )
            config_file_path = os.path.join(self.local_workspace_dir, config_file_name)
            write_to_file(content=config, full_file_name=config_file_path)
    ...
    def _getRedisConfigStr(self):
        ...
        """.format(
            ...
            REDIS_HOST=host,
            REDIS_PORT=port,
            REDIS_SSL_ENABLED=str(ssl_enabled),
        )
    ...
    def _get_monitoring_config_str(self):
        ...
        password = self.env_config.get_from_env_or_akv(
            "MONITORING_DATABASE_SQL_PASSWORD"
        )
        if url:
            # HOCON format will be parsed by the Feathr job
            config_str = """
    ...
    def get_features_from_registry(
        ...
        Dict[str, FeatureBase],
        Tuple[Dict[str, FeatureBase], Dict[str, Union[TypedKey, List[TypedKey]]]],
    ]:
        """
        Get feature from registry by project name. The features got from registry are automatically built.
        """
        registry_anchor_list, registry_derived_feature_list = (
            self.registry.get_features_from_registry(project_name)
        )
