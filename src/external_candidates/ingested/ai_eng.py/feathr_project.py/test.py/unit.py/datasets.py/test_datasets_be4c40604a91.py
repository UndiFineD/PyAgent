# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\test\unit\datasets\test_datasets.py
...
NYC_TAXI_FILE_PATH = str(
    TEST_DATASET_DIR.joinpath("green_tripdata_2020-04_with_index.csv").resolve()
)
...
def test__nyc_taxi__get_pandas_df(
    ...
    mocked_TemporaryDirectory = mocker.patch(
        "feathr.datasets.nyc_taxi.TemporaryDirectory", return_value=mocked_tmpdir
    )
...
def test__nyc_taxi__get_spark_df__with_databricks(
    ...
    mocked_maybe_download = mocker.patch("feathr.datasets.nyc_taxi.maybe_download")
    mocked_is_databricks = mocker.patch(
        "feathr.datasets.nyc_taxi.is_databricks", return_value=True
    )
