# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\test\test_registry_client.py
import datetime
import json
import os
import time
...
def test_parse_feature():
    ...
    assert (
        f._qualified_name
        == "feathr_getting_started__request_features__f_is_long_trip_distance"
    )
...
def test_registry_client_list_features():
    c = _FeatureRegistry(
        project_name="p",
        endpoint="https://feathr-sql-registry.azurewebsites.net/api/v1",
    )
    f = [
        e["qualifiedName"]
        for e in c.list_registered_features("feathr_ci_registry_getting_started")
    ]
    print(f)
    assert len(f) == 9
    for i in f:
...
def test_registry_client_load():
    ...
    anchors, derived_features = c.get_features_from_registry(
        "feathr_ci_registry_getting_started"
    )
    assert len(anchors) == 2
    request_features = [a for a in anchors if a.name == "request_features"][0]
    assert isinstance(request_features.source, InputContext)
