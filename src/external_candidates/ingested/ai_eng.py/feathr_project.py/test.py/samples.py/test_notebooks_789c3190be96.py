# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\test\samples\test_notebooks.py
...
NOTEBOOK_PATHS = {
    ...
    "product_recommendation_demo_advanced": str(
        SAMPLES_DIR.joinpath("product_recommendation_demo_advanced.ipynb")
    ),
}
...
def test__nyc_taxi_demo(config_path, tmp_path):
    ...
    assert outputs["materialized_feature_values"].data["239"] == pytest.approx(
        [1480.0, 5707.0], abs=1.0
    )
    assert outputs["materialized_feature_values"].data["265"] == pytest.approx(
        [4160.0, 10000.0], abs=1.0
    )
...
def test__fraud_detection_demo(config_path, tmp_path):
    ...
    assert outputs["materialized_feature_values"].data == pytest.approx(
        [False, 0, 9, 239.0, 1, 1, 239.0, 0.0], abs=1.0
    )
