...
def test_feathr_materialize_to_offline():
    ...
    res_df = get_result_df(
        client, data_format="avro", res_url=output_path + "/df0/daily/2020/05/20"
    )
...
def test_feathr_online_store_agg_features():
    ...
    client: FeathrClient = composite_keys_test_setup(
        os.path.join(test_workspace_dir, "feathr_config.yaml")
    )
    ...
    res = client.get_online_features(
        online_test_table, ["81", "254"], ["f_location_avg_fare", "f_location_max_fare"]
    )
    ...
    if client.spark_runtime.casefold() == "databricks":
        ...
        with pytest.raises(RuntimeError):
            client.feathr_spark_launcher.download_result(
                result_path="wasb://res_url", local_folder="/tmp"
            )
...
@pytest.mark.skip(
    reason="Marked as skipped as we need to setup token and enable SQL AAD login for this test"
)
def test_feathr_get_offline_features_to_sql_with_token():
    ...
    os.environ["SQL1_TOKEN"] = client.credential.get_token(
...
# )
@pytest.mark.skip(
    reason="Marked as skipped as we need to setup resources for this test"
)
...
def test_feathr_materialize_to_es():
    ...
    sink = ElasticSearchSink(
        name="es1", host="somenode:9200", index=index, ssl=True, auth=True
    )
    settings = MaterializationSettings(
        "nycTaxiTable",
        sinks=[sink],
