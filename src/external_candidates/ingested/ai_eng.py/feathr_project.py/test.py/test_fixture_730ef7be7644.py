# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\test\test_fixture.py
import os
import random
...
def basic_test_setup(config_path: str):
    ...
    f_trip_distance = Feature(
        name="f_trip_distance", feature_type=FLOAT, transform="trip_distance"
    )
    ...
    agg_features = [
        ...
        Feature(
            ...
            feature_type=FLOAT,
            transform=WindowAggTransformation(
                agg_expr="cast_float(fare_amount)", agg_func="MAX", window="90d"
            ),
        ),
    ]

    agg_anchor = FeatureAnchor(
        name="aggregationFeatures", source=batch_source, features=agg_features
    )

    client.build_features(
        anchor_list=[agg_anchor, request_anchor],
...
def conflicts_auto_correction_setup(config_path: str):
    ...
    agg_features = [
        Feature(
            ...
            feature_type=FLOAT,
            transform=WindowAggTransformation(
                agg_expr="cast_float(fare_amount)", agg_func="AVG", window="3d"
            ),
        ),
        Feature(
            name="total_amount",
            ...
            feature_type=FLOAT,
            transform=WindowAggTransformation(
                agg_expr="cast_float(fare_amount)", agg_func="MAX", window="3d"
            ),
        ),
    ]
...
def snowflake_test_setup(config_path: str):
    ...
    f_snowflake_call_center_division_name = Feature(
        name="f_snowflake_call_center_division_name",
        feature_type=STRING,
        transform="CC_DIVISION_NAME",
        key=call_sk_id,
    )

    f_snowflake_call_center_zipcode = Feature(
        name="f_snowflake_call_center_zipcode",
        feature_type=STRING,
        transform="CC_ZIP",
        key=call_sk_id,
    )
    ...
    snowflakeFeatures = FeatureAnchor(
        name="snowflakeFeatures", source=batch_source, features=features
    )
...
def kafka_test_setup(config_path: str):
    ...
    kafkaAnchor = FeatureAnchor(
        ...
        features=[
            Feature(
                name="f_modified_streaming_count",
                feature_type=INT32,
...
def registry_test_setup(config_path: str):
    ...
    client = FeathrClient(
        config_path=config_path, project_registry_tag={"for_test_purpose": "true"}
    )
    request_anchor, agg_anchor, derived_feature_list = generate_entities()

    client.build_features(
        anchor_list=[agg_anchor, request_anchor],
        derived_feature_list=derived_feature_list,
    )
    return client
...
def generate_entities():
    ...
    agg_features = [
        Feature(
            ...
            feature_type=FLOAT,
            transform=WindowAggTransformation(
                agg_expr="cast_float(fare_amount)", agg_func="AVG", window="90d"
            ),
        )
    ]
...
def time_partition_pattern_feature_gen_test_setup(
    ...
    client = FeathrClient(
        config_path=config_path, local_workspace_dir=local_workspace_dir
    )
    ...
    agg_features = [
        Feature(
            ...
            feature_type=FLOAT,
            transform=WindowAggTransformation(
                agg_expr="f_location_avg_fare", agg_func="AVG", window="3d"
            ),
        ),
        Feature(
            ...
            key=[key],
            transform=WindowAggTransformation(
                agg_expr="f_location_max_fare", agg_func="MAX", window="3d"
            ),
        ),
    ]

    agg_anchor = FeatureAnchor(
        name="testTimePartitionFeatures", source=batch_source, features=agg_features
    )
    client.build_features(anchor_list=[agg_anchor])
    return client
...
def time_partition_pattern_feature_join_test_setup(
    ...
    tpp_key = TypedKey(
        key_column="f_location_max_fare", key_column_type=ValueType.FLOAT
    )
    tpp_features = [
        Feature(
            name="key0",
            ...
            feature_type=FLOAT,
            transform=WindowAggTransformation(
                agg_expr="key0", agg_func="LATEST", window="3d"
            ),
        )
    ]
    tpp_anchor = FeatureAnchor(
        name="tppFeatures", source=batch_source_tpp, features=tpp_features
    )
