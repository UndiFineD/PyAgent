# Extracted from: C:\DEV\PyAgent\.external\ai-eng\feathr_project\test\test_time_partition_pattern_e2e.py
import os
from datetime import datetime, timedelta
from pathlib import Path

from feathr import BackfillTime, FeathrClient, HdfsSink, MaterializationSettings
from feathr.utils.job_utils import cloud_dir_exists, copy_cloud_dir, get_result_df
...
def test_feathr_materialize_with_time_partition_pattern():
    ...
    client: FeathrClient = time_partition_pattern_feature_gen_test_setup(
        ...
        source_path,
        local_workspace_dir="test_materialize_tpp",
    )

    backfill_time_tpp = BackfillTime(
        start=datetime(2020, 5, 2), end=datetime(2020, 5, 2), step=timedelta(days=1)
    )
    now = datetime.now()
    if client.spark_runtime == "databricks":
        output_path_tpp = "".join(
            [
                "dbfs:/feathrazure_cijob_materialize_offline_",
                "_",
                str(now.minute),
                "_",
                str(now.second),
    ...
    res_df = get_result_df(
        client, data_format="avro", res_url=output_path_tpp + "/df0/daily/2020/05/02"
    )
    assert res_df.shape[0] > 0
...
def test_feathr_materialize_with_time_partition_pattern_postfix_path():
    ...
    res_df = get_result_df(
        client, data_format="avro", res_url=output_path_pf + "/df0/daily/2020/05/02"
    )
    assert res_df.shape[0] > 0
...
def test_feathr_materialize_with_time_partition_pattern_hourly():
    ...
    res_df = get_result_df(
        client, data_format="avro", res_url=output_path_tpp + "/df0/daily/2020/05/02/00"
    )
    assert res_df.shape[0] > 0
