# Extracted from: C:\DEV\PyAgent\.external\ai-eng\docs\samples\feature_naming_conflicts_samples.py
...
assert (
    str(e.value)
    == "Feature names exist conflicts with dataset column names: trip_distance,fare_amount"
)
...
client = FeathrClient(
    config_path="feathr_config.yaml", local_workspace_dir="conflicts_test"
)
...
output_path = "".join(
    ["dbfs:/feathrazure_cijob", "_", str(now.minute), "_", str(now.second), ".avro"]
)
