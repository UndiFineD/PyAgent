# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\access_control\rbac\db_rbac.py
import logging
import os
...
from rbac.models import SUPER_ADMIN_SCOPE, AccessType, RoleType, UserRole, _to_uuid
...
class DbRBAC(RBAC):
    ...
    def _get_userroles(self) -> list[UserRole]:
        ...
        rows = self.conn.query(
            rf"""select record_id, project_name, user_name, role_name, create_by, create_reason, create_time, delete_by, delete_reason, delete_time
            from userroles
    ...
    def get_global_admin_users(self) -> list[str]:
        ...
        return [
            ...
            if (
                u.project_name == SUPER_ADMIN_SCOPE
                and u.role_name == RoleType.ADMIN.value
            )
        ]

    def validate_project_access_users(
        self, project: str, user: str, access: str = AccessType.READ
    ) -> bool:
        self.get_userroles()
        for u in self.userroles:
            if (
                u.user_name == user.lower()
                and u.project_name in [project.lower(), SUPER_ADMIN_SCOPE]
                and (access in u.access)
            ):
                return True
        return False

    def get_userroles_by_user(
        self, user_name: str, role_name: str = None
    ) -> list[UserRole]:
        """query the active user role of certain user"""
        query = rf"""select record_id, project_name, user_name, role_name, create_by, create_reason, create_time, delete_by, delete_reason, delete_time
            from userroles
            where delete_reason is null and user_name ='%s'"""
        if role_name:
            query += rf"and role_name = '%s'"
    ...
    def get_userroles_by_project(
        self, project_name: str, role_name: str = None
    ) -> list[UserRole]:
        """query the active user role of certain project."""
        query = rf"""select record_id, project_name, user_name, role_name, create_by, create_reason, create_time, delete_reason, delete_time
            from userroles
    ...
    def list_userroles(self, user_name: str) -> list[UserRole]:
        ...
        if user_name in self.get_global_admin_users():
            ...
            admin_roles = self.get_userroles_by_user(user_name, RoleType.ADMIN.value)
            ret = []
    ...
    def add_userrole(
        ...
        for u in self.userroles:
            if (
                u.project_name == project_name.lower()
                and u.user_name == user_name.lower()
                and u.role_name == role_name
            ):
                logging.warning(
        ...
        # insert new record
        query = rf"""insert into userroles (project_name, user_name, role_name, create_by, create_reason, create_time)
    ...
    def init_userrole(self, creator_name: str, project_name: str):
        ...
        if project_name.casefold() == SUPER_ADMIN_SCOPE.casefold():
            raise BadRequest(
                f"{SUPER_ADMIN_SCOPE} is keyword for Global Admin (admin of all projects), please try other project name."
            )
        else:
            # check if project already exist (have valid rbac records)
            # no 400 exception to align the registry api behaviors
            query = rf"""select project_name, user_name, role_name, create_by, create_reason, create_time, delete_reason, delete_time
                from userroles
                where delete_reason is null and project_name ='%s'"""
            rows = self.conn.query(query % (project_name.lower()))
            if len(rows) > 0:
                logging.warning(
                    f"{project_name} already exist, please pick another name."
                )
                return
            else:
