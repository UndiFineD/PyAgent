# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\access_control\rbac\auth.py
import base64
import logging
...
class AzureADAuth(OAuth2AuthorizationCodeBearer):
    ...
    def __init__(
        self,
        aad_instance: str = config.RBAC_AAD_INSTANCE,
        aad_tenant: str = config.RBAC_AAD_TENANT_ID,
    ):
    ...
    def _decode_token(self, token: str) -> Mapping:
        ...
        try:
            decode = jwt.decode(
                ...
                key=key,
                algorithms=["RS256"],
                audience=["https://management.azure.com", config.RBAC_API_AUDIENCE],
            )
            return decode
