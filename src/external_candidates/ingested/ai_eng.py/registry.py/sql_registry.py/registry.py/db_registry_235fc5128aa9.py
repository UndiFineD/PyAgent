# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\sql-registry\registry\db_registry.py
import json
import os
...
from registry import Registry, connect
...
class DbRegistry(Registry):
    def __init__(self):
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            ...
            if sandbox_registry_url:
                print(
                    f"FEATHR_SANDBOX_REGISTRY_URL is set to {sandbox_registry_url}. Please refer to https://docs.sqlalchemy.org/en/20/core/engines.html#database-urls for how to construct the URLs."
                )
                engine = db.create_engine(sandbox_registry_url)
            else:
                engine = db.create_engine(
                    "sqlite:////tmp/feathr_registry.sqlite?check_same_thread=False"
                )  # Create test.sqlite automatically
            self.sql_session = Session(engine)
            self.connection = engine.connect()
            metadata = db.MetaData()

            self.entities_table = db.Table(
                "entities",
                metadata,
                ...
                db.Column(
                    "attributes", db.String(2000), nullable=False
                ),  # TODO: sqlite doesn't enforce length but others might
            )
            self.edges_table = db.Table(
                "edges",
                metadata,
    ...
    def get_projects(self) -> List[str]:
        if os.environ.get("FEATHR_SANDBOX"):
            ...
            ret = self.conn.query(
                f"select qualified_name from entities where entity_type=%s",
    ...
    def get_projects_ids(self) -> Dict:
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            query = db.select(
                self.entities_table.c.entity_id, self.entities_table.c.qualified_name
            ).where((self.entities_table.c.entity_type == str(EntityType.Project)))
            ret = self._fetch_helper(query)
        else:
            ret = self.conn.query(
                f"select entity_id, qualified_name from entities where entity_type=%s",
                str(EntityType.Project),
            )
        for r in ret:
    ...
    def get_entity_id(self, id_or_name: Union[str, UUID]) -> UUID:
        ...
        # It is a name
        if os.environ.get("FEATHR_SANDBOX"):
            query = db.select(self.entities_table.c.entity_id).where(
                (self.entities_table.c.qualified_name == str(id_or_name))
            )
            ret = self._fetch_helper(query)
        else:
            ret = self.conn.query(
                f"select entity_id from entities where qualified_name=%s",
                str(id_or_name),
            )
        if len(ret) == 0:
    ...
    def get_neighbors(
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            query = db.select(
                self.edges_table.c.edge_id,
                self.edges_table.c.from_id,
                self.edges_table.c.to_id,
                self.edges_table.c.conn_type,
            ).where(
    ...
    def get_lineage(self, id_or_name: Union[str, UUID]) -> EntitiesAndRelations:
        ...
        upstream_entities, upstream_edges = self._bfs(id, RelationshipType.Consumes)
        downstream_entities, downstream_edges = self._bfs(id, RelationshipType.Produces)
        return EntitiesAndRelations(
            upstream_entities + downstream_entities, upstream_edges + downstream_edges
        )
    ...
    def get_project(self, id_or_name: Union[str, UUID]) -> EntitiesAndRelations:
        ...
        for anchor in project.attributes.anchors:
            ...
            source_id = self.get_neighbors(anchor.id, RelationshipType.Consumes)[
                0
            ].to_id
    ...
    def search_entity(
        ...
        if project:
            ...
            if os.environ.get("FEATHR_SANDBOX"):
                query = (
                    self.sql_session.query(
                        Entities.entity_id.label("id"),
                        Entities.qualified_name,
                        Entities.entity_type.label("type"),
                    )
                    .join(
                        Edges,
                        and_(
                            Entities.entity_id == Edges.from_id,
                            Edges.conn_type == "BelongsTo",
                        ),
                    )
                    .filter(
                        Edges.to_id == str(project_id),
                        Entities.qualified_name.ilike("%" + keyword + "%"),
                        Entities.entity_type.in_(tuple([str(t) for t in type])),
                    )
                    .order_by(Entities.qualified_name)
                    .slice(int(start), int(start + size))
                )
                rows = self._fetch_helper(query)
            else:
                sql = rf"""select {top_clause} entity_id as id, qualified_name, entity_type as type
                    from entities
            ...
            if os.environ.get("FEATHR_SANDBOX"):
                ...
                sql = rf"""select {top_clause} entity_id as id, qualified_name, entity_type as type from entities where qualified_name like %s and entity_type in %s order by qualified_name"""
                rows = self.conn.query(
                    sql, ("%" + keyword + "%", tuple([str(t) for t in type]))
                )
        if size:
            rows = rows[-size:]
    ...
    def create_project(self, definition: ProjectDef) -> UUID:
        ...
        with self.conn.transaction() as c:
            ...
            if os.environ.get("FEATHR_SANDBOX"):
                query = db.select(
                    self.entities_table.c.entity_id,
                    self.entities_table.c.entity_type,
                    self.entities_table.c.attributes,
                ).where(
                ...
                c.execute(
                    f"""select entity_id, entity_type, attributes from entities where qualified_name = %s""",
            ...
            if r:
                if len(r) > 1:
                    assert False, (
                        "Data inconsistency detected, %d entities have same qualified_name %s"
                ...
                if _to_type(r[0]["entity_type"], EntityType) != EntityType.Project:
                    raise ConflictError(
                        "Entity %s already exists" % definition.qualified_name
                    )
                # Just return the existing project id
                return _to_uuid(r[0]["entity_id"])
            id = uuid4()
            if os.environ.get("FEATHR_SANDBOX"):
                query = db.insert(self.entities_table).values(
                    entity_id=str(id),
    ...
    def create_project_anchor(self, project_id: UUID, definition: AnchorDef) -> UUID:
        ...
        with self.conn.transaction() as c:
            ...
            if os.environ.get("FEATHR_SANDBOX"):
                query = db.select(
                    self.entities_table.c.entity_id,
                    self.entities_table.c.qualified_name,
                ).where(
                ...
                c.execute(
                    "select entity_id, qualified_name from entities where entity_id = %s and entity_type = %s",
                    (str(definition.source_id), str(EntityType.Source)),
                )
                r = c.fetchall()
            if not r:
                raise ValueError("Source %s does not exist" % definition.source_id)
            ref = EntityRef(
                r[0]["entity_id"], EntityType.Source, r[0]["qualified_name"]
            )
            id = uuid4()
            if os.environ.get("FEATHR_SANDBOX"):
                query = db.insert(self.entities_table).values(
            ...
            self._create_edge(c, id, definition.source_id, RelationshipType.Consumes)
            self._create_edge(c, definition.source_id, id, RelationshipType.Produces)
            return id
    ...
    def create_project_anchor_feature(
        ...
        with self.conn.transaction() as c:
            ...
            if r:
                ...
                # The entity with same name already exists but with different type
                if (
                    _to_type(r[0]["entity_type"], EntityType)
        ...
        with self.conn.transaction() as c:
            ...
            if definition.input_anchor_features:
                if os.environ.get("FEATHR_SANDBOX"):
                    ...
                    c.execute(
                        rf"""select entity_id, entity_type, qualified_name from entities where entity_id in %s and entity_type = %s """,
                        (
                            tuple([str(id) for id in definition.input_anchor_features]),
                            str(EntityType.AnchorFeature),
                        ),
            ...
            if definition.input_derived_features:
                if os.environ.get("FEATHR_SANDBOX"):
                    ...
                    c.execute(
                        ...
                        (
                            tuple(
                                [str(id) for id in definition.input_derived_features]
                            ),
            ...
            refs = list(
                [
                    EntityRef(r["entity_id"], r["entity_type"], r["qualified_name"])
                    for r in r1 + r2
                ]
            )
            id = uuid4()
            ...
            for r in r1 + r2:
                ...
                self._create_edge(c, id, input_feature_id, RelationshipType.Consumes)
                self._create_edge(c, input_feature_id, id, RelationshipType.Produces)
            return id
    ...
    def _create_edge(self, cursor, from_id: UUID, to_id: UUID, type: RelationshipType):
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            ...
            query = db.insert(self.edges_table).values(
                edge_id=str(uuid4()),
                from_id=str(from_id),
                to_id=str(to_id),
                conn_type=type.name,
    ...
    def _delete_all_entity_edges(self, cursor, entity_id: UUID):
        ...
        """
        if os.environ.get("FEATHR_SANDBOX"):
            row_to_delete = self.sql_session.query(Edges).filter(
                (Edges.from_id == str(entity_id)) | Edges.to_id == str(entity_id)
            )
            row_to_delete.delete()
            self.sql_session.commit()
        else:
            sql = rf"""DELETE FROM edges WHERE from_id = %s OR to_id = %s"""
    ...
    def _delete_entity(self, cursor, entity_id: UUID):
        ...
        """
        if os.environ.get("FEATHR_SANDBOX"):
            row_to_delete = self.sql_session.query(Entities).filter(
            ...
            sql = rf"""DELETE FROM entities WHERE entity_id = %s"""
    ...
    def _get_edges(
        self, ids: List[UUID], types: List[RelationshipType] = []
    ) -> List[Edge]:
        if not ids:
            return []
        if os.environ.get("FEATHR_SANDBOX"):
            if len(types) > 0:
                query = self.sql_session.query(
                    Edges.edge_id, Edges.from_id, Edges.to_id, Edges.conn_type
                ).filter(
            ...
            sql = rf"""select edge_id, from_id, to_id, conn_type from edges
    ...
    def _get_entities(self, ids: List[UUID]) -> List[Entity]:
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            ...
            rows = self.conn.query(
                rf"""select entity_id, qualified_name, entity_type, attributes
                from entities
    ...
    def _bfs(
        self, id: UUID, conn_type: RelationshipType
    ...
    def _bfs_step(self, ids: List[UUID], conn_type: RelationshipType) -> Set[Dict]:
        ...
        if os.environ.get("FEATHR_SANDBOX"):
            query = self.sql_session.query(
                ...
                Edges.conn_type == conn_type.name,
                Edges.from_id.in_(tuple([str(id) for id in ids])),
            )
            r = self._fetch_helper(query)
            return r
        else:
            sql = rf"""select edge_id, from_id, to_id, conn_type from edges where conn_type = %s and from_id in %s"""
            return self.conn.query(
                sql, (conn_type.name, tuple([str(id) for id in ids]))
            )
