# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\sql-registry\main.py
import os
import traceback
...
def get_datasource(project: str, datasource: str) -> Dict:
    ...
    raise HTTPException(status_code=404, detail=f"Data Source {datasource} not found")
...
def new_project_datasource(project: str, definition: Dict) -> Dict:
    ...
    id = registry.create_project_datasource(
        project_id, SourceDef(**to_snake(definition))
    )
    return {"guid": str(id)}
...
def new_project_anchor_feature(project: str, anchor: str, definition: Dict) -> Dict:
    ...
    id = registry.create_project_anchor_feature(
        project_id, anchor_id, AnchorFeatureDef(**to_snake(definition))
    )
    return {"guid": str(id)}
...
def new_project_derived_feature(project: str, definition: Dict) -> Dict:
    project_id = registry.get_entity_id(project)
    id = registry.create_project_derived_feature(
        project_id, DerivedFeatureDef(**to_snake(definition))
    )
    return {"guid": str(id)}
