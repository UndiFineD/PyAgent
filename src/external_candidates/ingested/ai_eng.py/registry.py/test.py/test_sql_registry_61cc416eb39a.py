# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\test\test_sql_registry.py
import os
import sys
...
import pytest
from registry.db_registry import ConflictError, DbRegistry, quote
...
class SqlRegistryTest(unittest.TestCase):
    ...
    def setup(self):
        if os.getenv("CONNECTION_STR") is None:
            raise RuntimeError(
                "Failed to run SQL registry test case. Cannot get environment variable: 'CONNECTION_STR'"
            )
    ...
    def cleanup(self, ids):
        with self.registry.conn.transaction() as c:
            ids = quote(ids)
            c.execute(f"delete from edges where from_id in ({ids}) or to_id in ({ids})")
            c.execute(f"delete from entities where entity_id in ({ids})")
    ...
    def create_and_get_data_source(self, project_id, qualified_name, name, path, type):
        source_id = self.registry.create_project_datasource(
            project_id,
            SourceDef(qualified_name=qualified_name, name=name, path=path, type=type),
        )
    ...
    def create_and_get_anchor(self, project_id, source_id, qualified_name, name):
        anchor_id = self.registry.create_project_anchor(
            project_id,
            AnchorDef(qualified_name=qualified_name, name=name, source_id=source_id),
        )
    ...
    def create_and_get_anchor_feature(
        self,
        project_id,
        anchor_id,
        qualified_name,
        ...
        af_id = self.registry.create_project_anchor_feature(
            project_id,
            anchor_id,
            AnchorFeatureDef(
    ...
    def create_and_get_derived_feature(
        self,
        project_id,
        qualified_name,
        name,
        ...
        inpur_derived_features,
    ):
        df_id = self.registry.create_project_derived_feature(
            project_id,
            DerivedFeatureDef(
                qualified_name=qualified_name,
                name=name,
                ...
                transformation=transformation,
                key=keys,
                input_anchor_features=input_anchor_features,
                input_derived_features=inpur_derived_features,
            ),
        )
    ...
    def test_registry(self):
        ...
        source_id = self.create_and_get_data_source(
            project_id, project_name + "__source", "source", "hdfs://somewhere", "hdfs"
        )

        # test create anchor
        anchor_id = self.create_and_get_anchor(
            project_id, source_id, project_name + "__anchor", "anchor"
        )

        # test create anchor feature
        ft1 = FeatureType(
            type=VectorType.TENSOR,
            tensor_category=TensorCategory.DENSE,
            dimension_type=[],
            val_type=ValueType.INT,
        )
        t1 = ExpressionTransformation("af")
        t2 = WindowAggregationTransformation(
            "def_expr", "agg_func", "window", "group_by", "filter", limit=10
        )
        t3 = UdfTransformation(name="udf_trans")
        k = TypedKey(
            key_column="c1",
            key_column_type=ValueType.INT,
            full_name="tk",
            description="description",
            key_column_alias="alias",
        )
        af_id = self.create_and_get_anchor_feature(
            project_id,
            anchor_id,
            project_name + "__anchor__af",
            name="af",
            feature_type=ft1,
            transformation=t1,
            keys=[k],
        )
        af_id2 = self.create_and_get_anchor_feature(
            project_id,
            anchor_id,
            project_name + "__anchor__af2",
            name="af2",
            feature_type=ft1,
            transformation=t2,
            keys=[k],
        ...
        df_id = self.create_and_get_derived_feature(
            project_id, project_name + "__df", "df", ft1, t1, [k], [af_id], []
        )
        df_id2 = self.create_and_get_derived_feature(
            project_id, project_name + "__df2", "df2", ft1, t1, [k], [af_id], [df_id]
        )
        df_id3 = self.create_and_get_derived_feature(
            project_id,
            project_name + "__df3",
            "df3",
            ft1,
        ...
        entities = self.registry.search_entity(
            project_name + "__anchor", [EntityType.Anchor], project_id
        )
        ...
        with pytest.raises(ConflictError):
            project_id3 = self.registry.create_project(
                ProjectDef(project_name + "__anchor")
            )
            assert project_id3 == anchor_id
        with pytest.raises(ConflictError):
            source_id2 = self.registry.create_project_datasource(
                project_id,
                SourceDef(
                    qualified_name=project_name + "__anchor",
                    name="anchor",
                    path="somepath",
                    type="hdfs",
                ),
        ...
        with pytest.raises(ConflictError):
            anchor_id2 = self.registry.create_project_anchor(
                project_id,
                AnchorDef(
                    qualified_name=project_name + "__source",
                    name="source",
                    source_id=source_id,
                ),
            )
            assert anchor_id2 == source_id
        af_id3 = self.registry.create_project_anchor_feature(
            project_id,
            anchor_id,
            AnchorFeatureDef(
                qualified_name=project_name + "__anchor__af",
                name="af",
                feature_type=ft1,
                transformation=t1,
                key=[k],
        ...
        assert af_id3 == af_id
        df_id4 = self.registry.create_project_derived_feature(
            project_id,
            DerivedFeatureDef(
                qualified_name=project_name + "__df",
                name="df",
                feature_type=ft1,
                transformation=t1,
                key=[k],
        ...
        self.cleanup(
            [project_id, source_id, anchor_id, af_id, af_id2, df_id, df_id2, df_id3]
        )
