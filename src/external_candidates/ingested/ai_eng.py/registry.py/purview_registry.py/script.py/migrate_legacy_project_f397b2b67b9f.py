# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\purview-registry\script\migrate_legacy_project.py
...
from pyapacheatlas.core import AtlasEntity, AtlasException, AtlasProcess, PurviewClient
...
purview_client = PurviewClient(account_name=default_purview_name, authentication=oauth)
...
def _generate_relation_pairs(from_entity, to_entity, relation_type):
    ...
    global guid
    type_lookup = {
        RELATION_CONTAINS: RELATION_BELONGSTO,
    ...
    backward_relation = AtlasProcess(
        ...
        qualified_name=registry_delimeter.join(
            [
                type_lookup[relation_type],
                str(to_entity["guid"]),
                str(from_entity["guid"]),
            ]
        ),
        inputs=[to_entity],
        outputs=[from_entity],
...
def upload_single_entity_to_purview(
    purview_client, entity: Union[AtlasEntity, AtlasProcess]
):
    """
...
for entity in project_entities:
    ...
    for anchor in anchors:
        ...
        if anchor_entity["attributes"]["source"]:
            source_entity = purview_client.get_entity(
                anchor_entity["attributes"]["source"]["guid"]
            )["entities"][0]
            relations += _generate_relation_pairs(
        ...
        containing_anchor_features = [
            x
            for x in anchor_features
            if x["qualifiedName"].startswith(
    ...
    for derived_feature in derived_features:
        ...
        if (
            "input_derived_features" in derived_feature_entity["attributes"]
            or "input_anchor_features" in derived_feature_entity["attributes"]
        ):
            input_features = [
                x["guid"]
                for x in derived_feature_entity["attributes"]["input_derived_features"]
                + derived_feature_entity["attributes"]["input_anchor_features"]
            ]
            input_feature_entities = [
                purview_client.get_entity(x)["entities"][0] for x in input_features
            ]
