# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\purview-registry\registry\purview_registry.py
import copy
import itertools
...
class PurviewRegistry(Registry):
    def __init__(
        ...
        self.purview_client = PurviewClient(
            account_name=self.azure_purview_name, authentication=self.oauth
        )
        ...
        if recursive:
            ...
            if base_entity.entity_type == EntityType.Anchor:
                ...
                source_id = self.get_neighbors(
                    base_entity.id, RelationshipType.Consumes
                )[0].to_id
    ...
    def get_all_neighbours(self, id_or_name: Union[str, UUID]) -> List[Edge]:
        ...
        related_entities = self.purview_client.get_entity_lineage(
            str(entity.id), direction="BOTH"
        )["guidEntityMap"]
        process_entities = [
            v for _, v in related_entities.items() if v["typeName"] == "Process"
        ]
        ...
        for process_entity in process_entities:
            qualified_name_segs = process_entity["attributes"]["qualifiedName"].split(
        ...
        result_edges.extend(
            [
                Edge(
                    ...
                    str(entity.id),
                    relation_lookup[
                        x["attributes"]["qualifiedName"].split(self.registry_delimiter)[
                            0
                        ]
                    ],
                )
    ...
    def get_neighbors(
        ...
        project_contain_process = [
            x
            for x in process_entities
            if x["attributes"]["qualifiedName"].startswith(
                str(relationship.name).upper()
                + self.registry_delimiter
                + str(entity.id)
            )
        ]
        ...
        edge_end_object = [
            related_entities[x["displayText"].split(" to ")[1]]
            for x in project_contain_process
            if x["displayText"].split(" to ")[1] in related_entities
        ]

        result_edges = [
            Edge(x["guid"], str(entity.id), x["guid"], relationship)
            for x in edge_end_object
        ]
        return result_edges
    ...
    def _get_edges(self, ids: List[UUID]) -> List[Edge]:
        ...
        for id in ids:
            ...
            for neighbour in neighbours:
                if neighbour.from_id in ids and neighbour.to_id in ids:
    ...
    def get_project_features(
        self, project: str, keywords: Optional[str] = None
    ) -> List[Entity]:
        project_id = self.get_entity_id(project)
        if not project_id:
            return None
        guidAtlasEntityMap = self.purview_client.get_entity_lineage(
            project_id, depth=1, width=1, direction="OUTPUT"
        )["guidEntityMap"]
        atlasEntities = []
        ...
        for entity in guidAtlasEntityMap.values():
            ...
            if type != "Process":
                ...
                if (
                    type == EntityType.AnchorFeature
                    or type == EntityType.DerivedFeature
                ):
                    attributes = entity["attributes"]
                    if not keywords or (
                        attributes and keywords in attributes["qualifiedName"]
                    ):
    ...
    def get_project(self, id_or_name: Union[str, UUID]) -> EntitiesAndRelations:
        ...
        targetsRelationships = {
            ...
            EntityType.Anchor: RelationshipType.Contains.value
            | RelationshipType.Consumes.value,
        ...
        for id, entity in guidAtlasEntityMap.items():
            if entity["typeName"] == "Process":
                ...
                if not (
                    name.startswith("BELONGSTO") and name.endswith(str(project_id))
                ):
        ...
        for edge in edges:
            ...
            if (
                fromEntityType in targetsRelationships
                and targetsRelationships[fromEntityType] & relationship != 0
            ):
                ...
                if fromEntityType == EntityType.Project:
                    ...
                    curr_input_features = finalGuidEntityMap[
                        fromId
    ...
    def search_entity(
        ...
        for entity in query_result:
            ...
            if type and entity_type in [str(x) for x in type]:
                if project:
                    if not (
                        qualified_name.startswith(str(project))
                        or entity_id == str(project)
                    ):
                        continue
    ...
    def create_project_datasource(
        ...
        qualified_name = self.registry_delimiter.join(
            [project_entity.qualified_name, attrs["name"]]
        )
        source_entity = AtlasEntity(
            name=attrs["name"],
            qualified_name=qualified_name,
            attributes={
                k: v for k, v in attrs.items() if k not in ["name", "qualifiedName"]
            },
            typeName=str(EntityType.Source),
            guid=self.guid.get_guid(),
        )
        self._upload_entity_batch([source_entity])
    ...
    def create_project_anchor(self, project_id: UUID, definition: AnchorDef) -> UUID:
        ...
        self._upload_entity_batch([anchor_entity])
        ...
        self._upload_entity_batch(
            project_contains_anchor_relation + anchor_consumes_source_relation
        )
    ...
    def create_project_anchor_feature(
        ...
        self._upload_entity_batch([anchor_feature_entity])
    ...
    def create_project_derived_feature(
        ...
        input_features = self.get_entities(
            definition.input_anchor_features + definition.input_derived_features
        )
        ...
        self._upload_entity_batch([derived_feature_entity])
        ...
        self._upload_entity_batch(
            feature_project_contain_belong_pairs + consume_produce_pairs
        )
    ...
    def _register_feathr_feature_types(self):
        ...
        type_feathr_project = EntityTypeDef(
            ...
            attributeDefs=[
                ...
                AtlasAttributeDef(
                    ...
                    typeName=TYPEDEF_ARRAY_ANCHOR,
                    cardinality=Cardinality.SET,
                ),
                AtlasAttributeDef(
                    name="derived_features",
                    typeName=TYPEDEF_ARRAY_DERIVED_FEATURE,
                    cardinality=Cardinality.SET,
                ),
                AtlasAttributeDef(
                    name="tags",
        ...
        type_feathr_sources = EntityTypeDef(
            ...
            attributeDefs=[
                ...
                AtlasAttributeDef(
                    name="schemaStr", typeName="string", cardinality=Cardinality.SINGLE
                ),
                AtlasAttributeDef(
                ...
                AtlasAttributeDef(
                    name="type", typeName="string", cardinality=Cardinality.SINGLE
                ),
                AtlasAttributeDef(
                    name="preprocessing",
                    typeName="string",
        ...
        type_feathr_derived_features = EntityTypeDef(
            ...
            attributeDefs=[
                ...
                AtlasAttributeDef(
                    name="input_anchor_features",
                    typeName=TYPEDEF_ARRAY_ANCHOR_FEATURE,
                    cardinality=Cardinality.SET,
                ),
                AtlasAttributeDef(
                    name="input_derived_features",
                    typeName=TYPEDEF_ARRAY_DERIVED_FEATURE,
                    cardinality=Cardinality.SET,
                ),
                AtlasAttributeDef(
        ...
        existing_entitydefs_names = (
            [entity["name"] for entity in existing_defs["entityDefs"]]
            if "entityDefs" in existing_defs
            else []
        )
        if existing_entitydefs_names and any(
            name in new_entitydefs_names for name in existing_entitydefs_names
        ):
            logger.warning(
                f"EntityTypeDefs Exists: {new_entitydefs_names}. Registration Skipped."
            )
            logger.info("Feathr Feature Type System Initialized.")
            return

        def_result = self.purview_client.upload_typedefs(
            entityDefs=new_entitydefs, force_update=True
        )
        logger.info("Feathr Feature Type System Initialized.")
    ...
    def _upload_single_entity(self, entity: AtlasEntity):
        try:
            ...
            """
            response = self.purview_client.get_entity(
            ...
            if j["typeName"] == response["typeName"]:
                if j["typeName"] == "Process":
                    if (
                        response["attributes"]["qualifiedName"]
                    ...
                    keys = set([to_snake(key) for key in j["attributes"].keys()]) - set(
                        ["qualified_name"]
                    )
        ...
        results = None
        try:
            results = self.purview_client.upload_entities(batch=entity)
        except AtlasException as e:
    ...
    def _generate_relation_pairs(
        self, from_entity: Entity, to_entity: Entity, relation_type
    ):
