# Extracted from: C:\DEV\PyAgent\.external\ai-eng\registry\purview-registry\registry\models.py
import json
import re
...
def to_snake(d, level: int = 0):
    ...
    return {
        to_snake(a, level + 1): (
            to_snake(b, level + 1) if isinstance(b, (dict, list)) else b
        )
        for a, b in d.items()
    }
...
class ExpressionTransformation(Transformation):
    ...
    def to_dict(self) -> Dict:
        return {"transform_expr": self.transform_expr}
...
class Entity(ToDict):
    ...
    def get_ref(self) -> EntityRef:
        return EntityRef(self.id, self.attributes.entity_type, self.qualified_name)
...
class ProjectAttributes(Attributes):
    ...
    @property
    def sources(self):
        return [e for e in self.children if e.entity_type == EntityType.Source]

    @property
    def anchors(self):
        return [e for e in self.children if e.entity_type == EntityType.Anchor]

    @property
    def anchor_features(self):
        return [e for e in self.children if e.entity_type == EntityType.AnchorFeature]

    @property
    def derived_features(self):
        return [e for e in self.children if e.entity_type == EntityType.DerivedFeature]
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "anchor_features": list(
                [e.get_ref().to_dict() for e in self.anchor_features]
            ),
            "derived_features": list(
                [e.get_ref().to_dict() for e in self.derived_features]
            ),
            "tags": self.tags,
        }
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "input_derived_features": [
                e.to_dict() for e in self.input_derived_features
            ],
            "tags": self.tags,
        }
...
class Edge(ToDict):
    ...
    def __eq__(self, o: object) -> bool:
        ...
        return (
            self.from_id == o.from_id
            and self.to_id == o.to_id
            and self.conn_type == o.conn_type
        )
...
class EntitiesAndRelations(ToDict):
    ...
    def to_dict(self) -> Dict:
        return {
            "guidEntityMap": dict(
                [(str(id), self.entities[id].to_dict()) for id in self.entities]
            ),
...
class AnchorDef:
    ...
    def to_attr(self, source: EntityRef) -> AnchorAttributes:
        attr = AnchorAttributes(
            qualified_name=self.qualified_name, name=self.name, tags=self.tags
        )
        attr.source = source
        return attr
...
class AnchorFeatureDef:
    ...
    def to_attr(self) -> AnchorFeatureAttributes:
        return AnchorFeatureAttributes(
            qualified_name=self.qualified_name,
            name=self.name,
