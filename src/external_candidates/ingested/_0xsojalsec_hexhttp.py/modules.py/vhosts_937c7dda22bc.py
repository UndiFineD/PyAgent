# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\vhosts.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
        ...
        for vh in vhosts:
            ...
            try:
                ...
                if req_vh.status_code not in [404, 403, 425, 503, 500, 400] and len(
                    req_vh.content
                ) not in range(len_index - 100, len_index + 100):
                    retrieve_vh = True
                    print(
                        f" └── \033[32m\u251c\033[0m {url} [{len_index}b] <> {vh} [{len(req_vh.content)}b] "
                    )
            except (requests.RequestException, socket.gaierror) as e:
                logger.exception(
                    "The host IP has a problem, check it manually please: %s. Exception: %s",
                    vh,
                    e,
                )
    except (requests.RequestException, socket.gaierror) as e:
