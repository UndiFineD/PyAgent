# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\cache_poisoning_files.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def get_hit(url, matching_forward, custom_header, authent):
    ...
    for i in range(10):
        ...
        for cs in res.headers:
            if (
                "Cache-Status" in cs
                or "X-Cache" in cs
                or "x-drupal-cache" in cs
                or "X-Proxy-Cache" in cs
                or "X-HS-CF-Cache-Status" in cs
                or "X-Vercel-Cache" in cs
                or "X-nananana" in cs
                or "x-vercel-cache" in cs
                or "X-TZLA-EDGE-Cache-Hit" in cs
                or "x-spip-cache" in cs
                or "x-nextjs-cache" in cs
                or "x-pangle-cache-from" in cs
                or "X-Deploy-Web-Server-Cache-Hit" in cs
                or "CDN-Cache" in cs
            ):
    ...
    if word_in_text:
        print(
            f"\033[33m └── INTERESTING BEHAVIOR\033[0m | HEADER REFLECTION | \033[34m{url}?cb=1337\033[0m | PAYLOAD: X-Forwarded-Host"
        )
...
def wcp_import(url, matching_forward, custom_header, req_status, authent):
    ...
    req_verify_redirect = requests.get(
        url,
        params=params,
    ...
    if req_verify_redirect.status_code in [301, 302] or req_verify_url.status_code in [
        301,
        302,
    ]:
        if (
            matching_forward in req_verify_redirect.url
            or matching_forward in req_verify_url.url
        ):
            print(
                f"  \033[31m └── VULNERABILITY CONFIRMED\033[0m | DIFFERENT STATUS-CODE | \033[34m{url_param}\033[0m | PAYLOAD: X-Forwarded-Host"
            )
            # vuln_found_notify(url_param, "X-Forwarded-Host")
    elif (
        matching_forward in req_verify_redirect.text
        or matching_forward in req_verify_url.text
    ):
        print(
            f"  \033[31m └── VULNERABILITY CONFIRMED\033[0m | HEADER REFLECTION | \033[34m{url_param}\033[0m | PAYLOAD: X-Forwarded-Host"
        )
        # vuln_found_notify(url_param, "X-Forwarded-Host")
    elif req_verify_url.status_code != req_status:
        print(
            f"  \033[31m └── VULNERABILITY CONFIRMED\033[0m | DIFFERENT STATUS-CODE:\033[36m {req_status} → {req_verify_url.status_code}\033[0m | \033[34m{url_param}\033[0m | PAYLOAD: X-Forwarded-Host"
        )
        # vuln_found_notify(url_param, "X-Forwarded-Host")
