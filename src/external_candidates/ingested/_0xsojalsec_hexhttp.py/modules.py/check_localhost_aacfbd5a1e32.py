# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\modules\check_localhost.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
        ...
        try:
            ...
            if req.status_code in [301, 302]:
                try:
                    ...
                    print(
                        f" └── Host: {lt:<13}{'→':^3} {req.status_code:>3}{'→':^3}{req.headers['location']}"
                    )
                except:
                    print(
                        f" └── Host: {lt:<13}{'→':^3} {req.status_code:>3}{'→':^3}{req.headers['location']}"
                ...
                print(
                    f" └── Host: {lt:<13}{'→':^3} {req.status_code:>3} [{len(req.content)} bytes]"
                )
        except:
            # traceback.print_exc()
            pass
