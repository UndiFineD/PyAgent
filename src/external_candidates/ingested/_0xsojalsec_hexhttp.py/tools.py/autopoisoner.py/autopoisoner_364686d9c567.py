# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HExHTTP\tools\autopoisoner\autopoisoner.py
import argparse
import os
import random
import re
...
def vulnerability_confirmed(
    responseCandidate: requests.Response, url, randNum, buster, custom_header
):
    try:
...
def port_poisoning_check(url, initialResponse, custom_header, human):
    ...
    try:
        ...
        if (
            ...
            if behavior:
                behavior_or_confirmed_message(
                    uri, "BEHAVIOR", "LENGTH", explicitCache, url, header=custom_head
                )
    ...
    for header in headersToFuzz.keys():
        ...
        if canary_in_response(response):
            ...
            if vulnerability_confirmed(response, url, randNum, buster, custom_header):
                ...
                if behavior:
                    behavior_or_confirmed_message(
                        uri, "BEHAVIOR", "LENGTH", explicitCache, url, header=payload
                    )
