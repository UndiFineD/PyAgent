# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\limits.py
...
def _apply_exchanges_limits(dict_config, logger, limit):
    if len(trading_api.get_enabled_exchanges_names(dict_config)) > limit:
        enabled_exchanges = []
        for exchange, config in dict_config.get(
        ...
        return (
            f"Reached maximum allowed simultaneous exchanges for this plan, maximum is {limit}. "
...
def _apply_symbols_limits(dict_config, logger, limit):
    ...
    message = ""
    for currency, crypto_currency_data in dict_config.get(
        ...
        if crypto_currency_data.get(common_constants.CONFIG_ENABLED_OPTION, True):
            if len(enabled_symbols) >= limit:
                ...
                continue
            updated_symbols = []
            for symbol in crypto_currency_data.get(
                common_constants.CONFIG_CRYPTO_PAIRS, []
            ):
                if (
                    symbol == common_constants.CONFIG_SYMBOLS_WILDCARD[0]
    ...
    if has_disabled_symbols:
        return (
            f"{message}Reached maximum allowed simultaneous trading pairs for this plan, maximum is {limit}. "
...
def _apply_time_frames_limits(full_config, logger, limit):
    tentacles_setup_config = tentacles_manager_api.get_tentacles_setup_config(
        full_config.get_tentacles_config_path()
    )
    has_disabled_time_frames = False
    all_enabled_time_frames = []
    # patch time frames config
    for strategy_class in evaluators_api.get_activated_strategies_classes(
        ...
        if has_disabled_time_frames:
            ...
            strategy_enabled_time_frames = [
                tf for tf in config_time_frames if tf in all_enabled_time_frames
            ]
            for time_frame in config_time_frames:
                if time_frame not in strategy_enabled_time_frames:
                    should_update_config = True
                    logger.error(
                        f"Disabled {time_frame.value} time frame for {strategy_class.get_name()}"
                    )
    ...
    if has_disabled_time_frames:
        return (
            f"Reached maximum allowed simultaneous time frames for this plan, maximum is {limit}. "
...
def apply_config_limits(configuration) -> list:
    ...
    try:
        if constants.MAX_ALLOWED_EXCHANGES != constants.UNLIMITED_ALLOWED:
            if message := _apply_exchanges_limits(
                configuration.config, logger, constants.MAX_ALLOWED_EXCHANGES
            ):
                limit_warning_messages.append(message)
        if constants.MAX_ALLOWED_SYMBOLS != constants.UNLIMITED_ALLOWED:
            if message := _apply_symbols_limits(
                configuration.config, logger, constants.MAX_ALLOWED_SYMBOLS
            ):
                limit_warning_messages.append(message)
        if constants.MAX_ALLOWED_TIME_FRAMES != constants.UNLIMITED_ALLOWED:
            if message := _apply_time_frames_limits(
                configuration, logger, constants.MAX_ALLOWED_TIME_FRAMES
            ):
                limit_warning_messages.append(message)
...
def _check_max_backtesting_candles_count(time_frames, start_timestamp, end_timestamp):
    ...
    _check_max_backtesting_setting(
        "time frames using custom duration",
        constants.MAX_ALLOWED_TIME_FRAMES,
        time_frames,
    )
    if (
        start_timestamp is None
        or constants.MAX_ALLOWED_BACKTESTING_CANDLES_HISTORY
        == constants.UNLIMITED_ALLOWED
    ):
        return
    ...
    time_frame_seconds = (
        common_enums.TimeFramesMinutes[shortest_time_frame]
...
def ensure_backtesting_limits(
    exchanges, symbols, time_frames, start_timestamp, end_timestamp
) -> None:
    _check_max_backtesting_setting(
        "exchanges", constants.MAX_ALLOWED_EXCHANGES, exchanges
    )
    _check_max_backtesting_setting(
        "trading pairs", constants.MAX_ALLOWED_SYMBOLS, symbols
    )
