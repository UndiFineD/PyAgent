# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\producers\evaluator_producer.py
...
class EvaluatorProducer(octobot_channel.OctoBotChannelProducer):
    ...
    async def start(self):
        ...
        await evaluator_api.create_evaluator_channels(
            self.matrix_id,
            is_backtesting=backtesting_api.is_backtesting_enabled(self.octobot.config),
        )
    ...
    async def create_evaluators(self, exchange_configuration):
        await self.send(
            ...
            data={
                evaluator_channel_consumer.OctoBotChannelEvaluatorDataKeys.TENTACLES_SETUP_CONFIG.value: self.octobot.tentacles_setup_config,
                evaluator_channel_consumer.OctoBotChannelEvaluatorDataKeys.MATRIX_ID.value: self.octobot.evaluator_producer.matrix_id,
                evaluator_channel_consumer.OctoBotChannelEvaluatorDataKeys.EXCHANGE_CONFIGURATION.value: exchange_configuration,
            },
        )
