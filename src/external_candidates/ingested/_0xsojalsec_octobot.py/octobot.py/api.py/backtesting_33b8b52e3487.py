# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\api\backtesting.py
...
async def initialize_and_run_independent_backtesting(
    independent_backtesting, log_errors=True
) -> None:
    await independent_backtesting.initialize_and_run(log_errors=log_errors)


async def join_independent_backtesting(
    independent_backtesting, timeout=constants.BACKTESTING_DEFAULT_JOIN_TIMEOUT
) -> None:
...
async def stop_independent_backtesting(
    independent_backtesting, memory_check=False, should_raise=False
) -> None:
...
def get_independent_backtesting_symbols_by_exchanges(independent_backtesting) -> dict:
    return (
        independent_backtesting.octobot_backtesting.symbols_to_create_exchange_classes
    )
