# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\api\strategy_optimizer.py
...
def create_strategy_optimizer(
    config, tentacles_setup_config, strategy_name
) -> strategy_optimizer.StrategyOptimizer:
    return strategy_optimizer.StrategyOptimizer(
        config, tentacles_setup_config, strategy_name
    )
...
async def initialize_design_strategy_optimizer(
    strategy_optimizer, is_resuming, is_computing=False
):
...
async def generate_and_save_strategy_optimizer_runs(
    trading_mode, tentacles_setup_config, optimizer_settings
) -> list:
    optimizer = create_design_strategy_optimizer(
        trading_mode, optimizer_settings, None, tentacles_setup_config
    )
    return await optimizer.generate_and_save_run()


def create_strategy_optimizer_settings(
    settings_dict: dict,
) -> optimizer_settings_import.OptimizerSettings:
    return optimizer_settings_import.OptimizerSettings(settings_dict)


async def resume_design_strategy_optimizer(
    optimizer, optimizer_settings: optimizer_settings_import.OptimizerSettings
):
    ...
    optimizer_settings.optimizer_ids = (
        optimizer_settings.optimizer_ids or await optimizer.get_queued_optimizer_ids()
    )
    return await optimizer.resume(optimizer_settings)


def find_optimal_configuration(
    strategy_optimizer, TAs=None, time_frames=None, risks=None
) -> None:
    strategy_optimizer.find_optimal_configuration(
        TAs=TAs, time_frames=time_frames, risks=risks
    )
...
async def get_design_strategy_optimizer_queue(trading_mode) -> list:
    return await strategy_design_optimizer.StrategyDesignOptimizer.get_run_queue(
        trading_mode
    )


async def update_design_strategy_optimizer_queue(trading_mode, updated_queue) -> list:
    return await strategy_design_optimizer.StrategyDesignOptimizer.update_run_queue(
        trading_mode, updated_queue
    )
