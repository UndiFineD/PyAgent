# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\task_manager.py
...
class TaskManager:
    ...
    def stop_tasks(self, stop_octobot=True):
        ...
        if stop_coroutines:
            try:
                asyncio_tools.run_coroutine_in_asyncio_loop(
                    _await_timeouted_gather(stop_coroutines), self.async_loop
                )
            except asyncio.exceptions.TimeoutError:
        ...
        asyncio.run_coroutine_threadsafe(
            asyncio_tools.wait_asyncio_next_cycle(), self.async_loop
        )
    ...
    def _loop_exception_handler(self, loop, context):
        ...
        function = (
            self.logger.debug
            if self._should_use_debug_for_message(message)
    ...
    def _create_new_asyncio_main_loop(self):
        ...
        self.current_loop_thread = threading.Thread(
    ...
    def run_in_main_asyncio_loop(
        ...
        return asyncio_tools.run_coroutine_in_asyncio_loop(
            coroutine, self.async_loop, log_exceptions=log_exceptions, timeout=timeout
        )
