# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\octobot.py
...
class OctoBot:
    ...
    def __init__(
        ...
        self.configuration_manager.add_element(
            constants.CONFIG_KEY, config, has_dict=True
        )
        ...
        self.community_auth = (
            community_authenticator
            or community.CommunityAuthentication.create(community_config)
        )
        ...
        self.global_consumer = octobot_channel_consumer.OctoBotChannelGlobalConsumer(
            self
        )
    ...
    async def initialize(self):
        ...
        if not (
            self.community_auth.is_initialized()
            and self.community_auth.is_using_the_current_loop()
        ):
    ...
    async def create_producers(self):
        self.exchange_producer = producers.ExchangeProducer(
            self.global_consumer.octobot_channel, self, None, self.ignore_config
        )
        self.evaluator_producer = producers.EvaluatorProducer(
            self.global_consumer.octobot_channel, self
        )
        self.interface_producer = producers.InterfaceProducer(
            self.global_consumer.octobot_channel, self
        )
        self.service_feed_producer = producers.ServiceFeedProducer(
            self.global_consumer.octobot_channel, self
        )
    ...
    async def _post_initialize(self):
        ...
        self.configuration_manager.add_element(
            constants.TENTACLES_SETUP_CONFIG_KEY, self.tentacles_setup_config
        )
        await service_api.send_notification(
            service_api.create_notification(
                f"{constants.PROJECT_NAME} {constants.LONG_VERSION} is starting ...",
        ...
        self.automation = automation.Automation(
            self.bot_id, self.tentacles_setup_config
        )
        self._init_metadata_run_task = asyncio.create_task(
            self._store_run_metadata_when_available()
        )
            ...
            for topic in constants.REQUIRED_TOPIC_FOR_DATA_INIT:
                await commons_tree.EventProvider.instance().wait_for_event(
                    ...
                    commons_tree.get_exchange_path(
                        trading_api.get_exchange_name(exchange_manager), topic.value
                    ),
                    timeout,
                )
    ...
    async def _store_run_metadata_when_available(self):
        ...
        try:
            await asyncio.wait_for(
                self.exchange_producer.created_all_exchanges.wait(),
                run_metadata_init_timeout,
            )
        ...
        try:
            await self._wait_for_run_data_init(
                exchange_managers, run_metadata_init_timeout
            )
        except asyncio.TimeoutError:
            pass
        try:
            if exchange_managers:
                ...
                await storage.store_run_metadata(
                    self.bot_id, exchange_managers, self.start_time, flush=True
                )
            else:
                self.logger.debug(
                    "Skipping run metadata update: no available exchange manager"
                )
    ...
    async def stop(self):
        try:
            ...
            if (
                self._init_metadata_run_task is not None
                and not self._init_metadata_run_task.done()
            ):
    ...
    async def _ensure_clock(self):
        if (
            trading_api.is_trader_enabled_in_config(self.config)
            and constants.ENABLE_CLOCK_SYNCH
        ):
    ...
    async def _on_profile_update(self, profile_name: str):
        ...
        asyncio.create_task(
            self.delayed_restart(
                constants.PROFILE_UPDATE_RESTART_MIN
    ...
    def _log_config(self):
        exchanges = [
            ...
            for exchange, config in self.config.get(
        ...
        has_simulated_trader = trading_api.is_trader_simulator_enabled_in_config(
            self.config
        )
        trader_str = (
            "real trader"
            if has_real_trader
            else "simulated trader" if has_simulated_trader else "no trader"
        )
        ...
        trading_mode_str = (
            trading_mode.get_name() if trading_mode else "no trading mode"
        )
        self.logger.info(
            f"Starting OctoBot with {trader_str} on "
    ...
    def run_in_main_asyncio_loop(
        ...
        return self.task_manager.run_in_main_asyncio_loop(
    ...
    async def _init_aiohttp_session(self):
        if self._aiohttp_session is None:
            self._aiohttp_session = (
                await aiohttp_util.get_ssl_fallback_aiohttp_client_session(
