# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\octobot_backtesting_factory.py
...
class OctoBotBacktestingFactory(octobot_class.OctoBot):
    def __init__(
        ...
        super().__init__(
            config, community_authenticator=_BacktestingCommunityAuthenticator()
        )
    ...
    async def initialize(self):
        try:
            ...
            join_backtesting_timeout = (
                constants.BACKTESTING_DEFAULT_JOIN_TIMEOUT
                if self.enable_join_timeout
                else None
            )
            ...
            self.logger.exception(
                e, True, f"Error when starting backtesting: {e.__class__.__name__}"
            )
        finally:
