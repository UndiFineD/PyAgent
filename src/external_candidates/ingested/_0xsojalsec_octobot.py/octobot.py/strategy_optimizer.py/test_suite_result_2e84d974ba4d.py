# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\test_suite_result.py
...
class TestSuiteResult:
    ...
    def get_result_string(self, details=True):
        details_str = (
            f" details: (profitabilities (bot, market):{self.run_profitabilities}, trades: "
            f"{self.trades_counts})"
            if details
            else ""
        )
        return (
            f"{self.get_evaluators_without_strategy()} on {self.time_frames} at risk: {self.risk} "
    ...
    def get_result_dict(self, index=0):
        return self.convert_result_into_dict(
            index,
            self.get_evaluators_without_strategy(),
            self.time_frames,
            self.risk,
            round(self.get_average_score(), 5),
