# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\fitness_parameter.py
...
class FitnessParameter:
    ...
    def _get_value_from_ratio(self, raw_value):
        return (
            raw_value * self._get_parameter_normalizer()
            if self.max_ratio_value is None
            else (raw_value - self.min_ratio_value)
            / (self.max_ratio_value - self.min_ratio_value)
        )
    ...
    def update_ratio(self, full_result):
        try:
            if (
                self.max_ratio_value is None
                or full_result[self.name] > self.max_ratio_value
            ):
                self.max_ratio_value = full_result[self.name]
            if (
                self.min_ratio_value is None
                or full_result[self.name] < self.min_ratio_value
            ):
