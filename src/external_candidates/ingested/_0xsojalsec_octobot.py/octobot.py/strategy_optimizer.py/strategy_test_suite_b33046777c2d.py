# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\strategy_test_suite.py
...
class StrategyTestSuite(octobot_backtesting.AbstractBacktestingTest):
    ...
    def get_test_suite_result(self):
        return octobot_strategy_optimizer.TestSuiteResult(
            self._profitability_results,
            self._trades_counts,
    ...
    async def run_test_suite(self, strategy_tester):
        self.exceptions = []
        tests = [
            self.test_slow_downtrend,
            self.test_sharp_downtrend,
            self.test_flat_markets,
            self.test_slow_uptrend,
            self.test_sharp_uptrend,
            self.test_up_then_down,
        ]
        print("| ", end="")
        nb_tests = len(tests)
        for i, test in enumerate(tests):
            try:
                ...
                self.logger.exception(
                    e, True, f"Exception when running test {test.__name__}: {e}"
                )
    ...
    async def test_slow_downtrend(self, strategy_tester):
        await strategy_tester.run_test_slow_downtrend(
            None, None, None, None, StrategyTestSuite.SKIP_LONG_STEPS
        )

    async def test_sharp_downtrend(self, strategy_tester):
        await strategy_tester.run_test_sharp_downtrend(
            None, None, StrategyTestSuite.SKIP_LONG_STEPS
        )

    async def test_flat_markets(self, strategy_tester):
        await strategy_tester.run_test_flat_markets(
            None, None, None, None, StrategyTestSuite.SKIP_LONG_STEPS
        )
    ...
    async def test_up_then_down(self, strategy_tester):
        await strategy_tester.run_test_up_then_down(
            None, StrategyTestSuite.SKIP_LONG_STEPS
        )
    ...
    def _handle_results(self, independent_backtesting, profitability):
        ...
        if independent_backtesting is not None:
            ...
            try:
                for (
                    exchange_manager
                ) in trading_api.get_exchange_managers_from_exchange_ids(
