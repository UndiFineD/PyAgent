# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\scored_run_result.py
...
class ScoredRunResult:
    ...
    def _compute_score(self, fitness_parameter):
        try:
            self.values[fitness_parameter.name] = self.full_result[
                fitness_parameter.name
            ]
            score = fitness_parameter.get_normalized_value(
                self.values[fitness_parameter.name]
            )
    ...
    def result_str(self):
        ...
        user_inputs = {
            ui[strategy_design_optimizer.StrategyDesignOptimizer.CONFIG_USER_INPUT]: ui[
                strategy_design_optimizer.StrategyDesignOptimizer.CONFIG_VALUE
            ]
