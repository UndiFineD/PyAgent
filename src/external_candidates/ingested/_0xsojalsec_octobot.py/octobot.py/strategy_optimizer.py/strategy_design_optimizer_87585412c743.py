...
class StrategyDesignOptimizer:
    ...
    LAST_CREATED_QUEUE_CONFIG = (
        f"last_created_queue_config{commons_constants.CONFIG_FILE_EXT}"
    )

    def __init__(
        self, trading_mode, config, tentacles_setup_config, optimizer_settings=None
    ):
    ...
    async def multi_processed_optimize(
        self, optimizer_settings, run_data_by_optimizer_id=None
    ):
        success = True
        optimizer_ids = optimizer_settings.optimizer_ids or [
            optimizer_settings.optimizer_id
        ]
        run_data_by_optimizer_id = run_data_by_optimizer_id or {}
        self.is_computing = True
        self.is_finished = False
        self.average_run_time = 0
        self.active_processes_count = multiprocessing.cpu_count() - abs(
            optimizer_settings.required_idle_cores
        )
        ...
        shared_run_time = multiprocessing.Array(
            ctypes.c_float, [0.0 for _ in range(self.active_processes_count)]
        )
        try:
            async for selected_optimizer_ids in self._all_optimizer_ids(
                ...
                try:
                    ...
                    for optimizer_id, run_queues in run_queues_by_optimizer_id.items():
                        try:
                            await self._update_runs_from_done_queue(
                                run_queues, optimizer_id
                            )
                        except Exception as e:
                            self.logger.exception(
                                e, True, f"Error on final db queue update: {e}"
                            )
            ...
            self.logger.exception(
                e, True, f"Error when running optimizer processes: {e}"
            )
            success = False
        finally:
        ...
        self.logger.info(
            f"Optimizer runs complete in {time.time() - global_t0} seconds."
        )
        return success
    ...
    def find_optimal_configuration_wrapper(
        ...
        run_queues_by_optimizer_id = multiprocessing_util.get_shared_element(
            self.SHARED_RUNS_QUEUES_KEY
        )
        asyncio.run(
    ...
    async def _read_optimizer_runs_details_and_hashes(self, optimizer_id):
        ...
        try:
            return self._get_optimizer_runs_details_and_hashes(
                run_data[0][self.CONFIG_RUNS]
            )
        except IndexError:
    ...
    async def _execute_optimizer_run(
        ...
        if update_database and optimizer_id is None:
            ...
            return
        try:
            run_data_by_hash = await self._read_optimizer_runs_details_and_hashes(
                optimizer_id
            )
            t_start = time.time()
            ...
            while self._should_keep_running():
                ...
                if (
                    update_database
                    and time.time() - last_db_update_time
                    > self.optimizer_settings.db_update_period
                ):
                    await self._update_runs_from_done_queue(run_queues, optimizer_id)
    ...
    def _should_keep_running(self):
        try:
            return multiprocessing_util.get_shared_element(
                self.SHARED_KEEP_RUNNING_KEY
            ).value
        except KeyError:
    ...
    def _register_run_time(self, run_time):
        try:
            shared_run_times = multiprocessing_util.get_shared_element(
                self.SHARED_RUN_TIMES_KEY
            )
    ...
    def get_average_run_time(self):
        if self.average_run_time == 0:
            try:
                run_times = [
                    v
                    for v in multiprocessing_util.get_shared_element(
                        self.SHARED_RUN_TIMES_KEY
                    )
                ]
            except KeyError:
    ...
    def cancel(self):
        ...
        try:
            multiprocessing_util.get_shared_element(
                self.SHARED_KEEP_RUNNING_KEY
            ).value = False
        except KeyError:
            pass
    ...
    async def _get_remaining_runs_count_from_multi_process_queue(self, optimizer_ids):
        try:
            ...
            return sum(
                ...
                or self.optimizer_settings.empty_the_queue
            )
        except KeyError:
            return 0
    ...
    async def get_overall_progress(self):
        if self.optimizer_settings.optimizer_id is None:
            remaining_runs = (
                await self._get_remaining_runs_count_from_multi_process_queue(
        ...
        return (
            int(done_runs / self.total_nb_runs * 100) if self.total_nb_runs else 0
        ), remaining_time
    ...
    async def get_queued_optimizer_ids(self):
        return [
            ...
            for run in (
                await self.get_run_queue(
                    self.trading_mode, self.optimization_campaign_name
                )
            )
        ]

    def _get_optimization_func(
        self, optimizer_settings: optimizer_settings_import.OptimizerSettings
    ):
    ...
    async def resume(
        ...
        self.total_nb_runs = await self._get_total_nb_runs(
        ...
        if optimizer_func := self._get_optimization_func(optimizer_settings):
            ...
            raise NotImplementedError(
                f"Unknown optimizer mode: {optimizer_settings.optimizer_mode}"
            )
            ...
            for run_data_from_runs in reversed(all_run_results):
                for to_find_run_data in run_data_elements.values():
                    if self._is_using_these_user_inputs(
                        run_data_from_runs, to_find_run_data
                    ):
    ...
    def _is_using_these_user_inputs(self, full_run_data, to_find_run_data):
        full_run_data_user_inputs = full_run_data[
    ...
    async def _get_total_nb_runs(self, optimizer_ids):
        ...
        return sum(
            len(run[self.CONFIG_RUNS])
            for run in full_queue
            if run[self.CONFIG_ID] in optimizer_ids
        )
            ...
            try:
                ...
                if updated_queue[cls.CONFIG_DELETE_EVERY_RUN]:
                    ...
                    await writer_reader.delete(
                        cls.RUN_SCHEDULE_TABLE, query.id == updated_queue["id"]
                    )
    ...
    async def _synchronized_pick_run(self, run_queue, run_details_by_hash):
        try:
            ...
            with multiprocessing_util.get_lock(
                commons_enums.MultiprocessingLocks.DBLock.value
            ):
    ...
    async def _run_with_config(
        ...
        self.logger.debug(
            f"Running optimizer with id {optimizer_id} "
            f"on backtesting {run_id} with config {run_config}"
        )
        self._update_config_for_optimizer(optimizer_id, run_id)
        tentacles_setup_config = self._get_custom_tentacles_setup_config(
            optimizer_id, run_id, run_config
        )
        ...
        try:
            ...
            await octobot_backtesting_api.join_independent_backtesting(
                independent_backtesting, timeout=None
            )
    ...
    def _updated_nested_tentacle_config(
        self, nested_tentacles, user_input, config_value, local_tentacle_config
    ):
    ...
    def _get_custom_tentacles_setup_config(self, optimizer_id, run_id, run_config):
        ...
        tentacles_setup_config_path = os.path.join(
            run_folder, commons_constants.CONFIG_TENTACLES_FILE
        )
        ...
        tentacles_updates = {
            tentacle: {}
            for tentacle in tentacles_manager_api.get_activated_tentacles(
                self.base_tentacles_setup_config
            )
            if tentacles_manager_api.has_profile_local_configuration(
                self.base_tentacles_setup_config, tentacle
            )
        }
        ...
        for tentacle, updated_values in tentacles_updates.items():
            ...
            current_config = tentacles_manager_api.get_tentacle_config(
                self.base_tentacles_setup_config, tentacle_class
            )
            dict_util.nested_update_dict(current_config, updated_values)
            tentacles_manager_api.update_tentacle_config(
                local_tentacles_setup_config, tentacle_class, current_config
            )
    ...
    def shuffle_and_select_runs(runs, select_size=None) -> dict:
        ...
        selected_runs = (
            shuffled_runs if select_size is None else shuffled_runs[:select_size]
        )
    ...
    def _generate_runs(self):
        ...
        if runs:
            shuffled_runs = self.shuffle_and_select_runs(
                runs, select_size=self.optimizer_settings.queue_size
            )
    ...
    def _is_run_allowed(self, run):
        for run_filter_config in self.optimizer_settings.optimizer_config[
    ...
    def _parse_filter_entry(self, run, run_filter_config):
        user_input_left_operand_key = run_filter_config["user_input_left_operand"][
            self.CONFIG_VALUE
        ]
        user_input_right_operand_key = run_filter_config["user_input_right_operand"][
            self.CONFIG_VALUE
        ]
        ...
        right_operand = (
            ...
            if text_right_operand in ("null", "")
            else text_right_operand
        )
        ...
        try:
            if config_element[self.CONFIG_ENABLED]:
                if config_element[self.CONFIG_TYPE] is ConfigTypes.OPTIONS:
                    values = [
                        [value]
                        for value in config_element[self.CONFIG_VALUE][
                            self.CONFIG_VALUE
                        ]
                    ]
            ...
            return [
                {
                    ...
                    self.CONFIG_TENTACLE: config_element[self.CONFIG_VALUE][
                        self.CONFIG_TENTACLE
    ...
    def _get_config_elements(self):
        for key, val in self.optimizer_settings.optimizer_config[
            self.CONFIG_USER_INPUTS
        ].items():
            yield {
                self.CONFIG_KEY: key,
    ...
    async def _create_run_schedule_and_config_snapshot(self):
        ...
        optimizer_target_identifier = (
            self.run_dbs_identifier.get_backtesting_run_folder()
        )
        try:
            shutil.copy(
                self.run_dbs_identifier.get_optimizer_runs_schedule_identifier(),
            ...
            with open(
                os.path.join(
                    optimizer_target_identifier, self.LAST_CREATED_QUEUE_CONFIG
                ),
                "w",
            ) as fw:
    ...
    async def _save_run_schedule(self, runs):
        ...
        async with databases.DBWriterReader.database(
            ...
            try:
                existing_runs = await self._get_run_data_from_db(
                    self.optimizer_settings.optimizer_id, writer_reader
                )
                if existing_runs:
                    merged_runs = {
