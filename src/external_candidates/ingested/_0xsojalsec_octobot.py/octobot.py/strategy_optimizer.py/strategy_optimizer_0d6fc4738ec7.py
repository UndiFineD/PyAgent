# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\strategy_optimizer\strategy_optimizer.py
...
class StrategyOptimizer:
    ...
    def find_optimal_configuration(self, TAs=None, time_frames=None, risks=None):
        if not self.is_computing:
            ...
            previous_log_level_per_handler = (
                common_logging.get_logger_level_per_handler()
            )
            ...
            try:
                ...
                self.all_time_frames = (
                    self.strategy_class.get_required_time_frames(
                        self.config, self.tentacles_setup_config
                    )
                ...
                self.logger.info(
                    f"Trying to find an optimized configuration for {self.strategy_class.get_name()} "
                ...
                self.total_nb_runs = int(
                    len(self.risks)
                    * ((math.pow(2, nb_TFs) - 1) * (math.pow(2, nb_TAs) - 1))
                )

                self.logger.info(
                    "Setting logging level to logging.ERROR to limit messages."
                )
            ...
            raise RuntimeError(
                f"{self.get_name()} is already computing: processed "
            ...
            for evaluator_conf_iteration in range(nb_TAs):
                ...
                for nb_evaluators in range(1, nb_TAs + 1):
                    ...
                    for _ in range(nb_TAs):
                        activated_evaluators = self._get_activated_element(
                            self.all_TAs,
                        ...
                        if activated_evaluators is not None:
                            ...
                            for time_frame_conf_iteration in range(nb_TFs):
                                current_forced_time_frame = self.all_time_frames[
                                    time_frame_conf_iteration
                                ]
    ...
    def _run_on_config(
        ...
        activated_time_frames = self._get_activated_element(
            self.all_time_frames,
            current_forced_time_frame,
            nb_time_frames,
            time_frames_conf_history,
        ...
        if activated_time_frames is not None:
            ...
            print(
                f"{self.run_id}/{self.total_nb_runs} Run with: evaluators: {activated_evaluators}, "
    ...
    def _run_test_suite(self, config, evaluators):
        ...
        self.current_test_suite.initialize_with_strategy(
            self.strategy_class, self.tentacles_setup_config, copy.deepcopy(config)
        )
        no_error = asyncio.run(
        ...
        if not no_error:
            self.errors = self.errors.union(
                set([str(e) for e in self.current_test_suite.exceptions])
            )
    ...
    def _adapt_tentacles_config(self, activated_evaluators):
        ...
        for tentacle_class_name in tentacles_activation[
            ...
            if tentacle_class_name in activated_evaluators:
                ...
                tentacles_management.get_class_from_string(
                    tentacle_class_name,
                    evaluators.StrategyEvaluator,
                    tentacles_Evaluator.Strategies,
                    tentacles_management.evaluator_parent_inspection,
        ...
        tentacles_manager_api.update_activation_configuration(
            self.tentacles_setup_config, to_update_config, False
        )
    ...
    def _find_optimal_configuration_using_results(self):
        for time_frame in self.all_time_frames:
            ...
            self.sorted_results_by_time_frame[time_frame.value] = (
                time_frame_sorted_results
            )
        ...
        for results in self.sorted_results_by_time_frame.values():
            for rank, result in enumerate(results):
                ...
                results_through_all_time_frame[result_summary][
                    TRADES
                ] += result.trades_counts

        result_list = [
            (result, trades_and_rank[RANK], data_util.mean(trades_and_rank[TRADES]))
        ...
        self.sorted_results_through_all_time_frame = sorted(
            result_list, key=_sorting_key
        )

    def print_report(self):
        self.logger.info(
            "Full execution sorted results: Minimum time frames are defining the range of the run "
        ...
        for rank, result in enumerate(self.sorted_results_through_all_time_frame[0:25]):
            self.logger.info(
                f"{rank}: {result[CONFIG].get_result_string()} (time frame rank sum: {result[RANK]}) "
                f"average trades count: {result[TRADES_IN_RESULT]:f}"
            )
        self.logger.info(
            f" *** Overall best configuration for {self.strategy_class.get_name()} using "
            f"{self.trading_mode.get_name()} trading mode *** "
        )
        best_result = self.sorted_results_through_all_time_frame[0]
        self.logger.info(
            f"{best_result[CONFIG].get_result_string()} "
            f"average trades count: {best_result[TRADES_IN_RESULT]:f}"
        )

    async def get_overall_progress(self):
        return (
            int((self.run_id - 1) / self.total_nb_runs * 100)
            if self.total_nb_runs
            else 0
        ), 0
    ...
    def get_current_test_suite_progress(self):
        return (
            self.current_test_suite.current_progress if self.current_test_suite else 0
        )
        ...
        if self.sorted_results_through_all_time_frame:
            results = [
                ...
                for rank, result in enumerate(
                    self.sorted_results_through_all_time_frame[0:100]
                )
            ]
        else:
            results = []
    ...
    @staticmethod
    def _get_activated_element(
        all_elements,
        current_forced_element,
        nb_elements_to_consider,
        elem_conf_history,
        ...
        if nb_elements_to_consider > 1:
            ...
            while (
                i < len(all_elements)
                and len(eval_conf) < nb_elements_to_consider + additional_elements_count
            ):
                ...
                if current_elem not in eval_conf:
                    ...
                    if len(
                        eval_conf
                    ) == nb_elements_to_consider + additional_elements_count and (
                        (
                            eval_conf
                            if dict_shaped
                            else sorted([key.value for key in eval_conf])
                        )
    ...
    def _get_filtered_results(results, time_frame=None):
        return [
            ...
            if time_frame is None or result.min_time_frame == time_frame
        ]

    @staticmethod
    def _get_sorted_results(results, time_frame=None):
        return sorted(
            StrategyOptimizer._get_filtered_results(results, time_frame),
            key=lambda result: result.get_average_score(),
            reverse=True,
        )
    ...
    def _is_relevant_evaluation_config(evaluator):
        ...
        return (
            tentacles_management.get_class_from_string(
                evaluator,
                evaluators.TAEvaluator,
                tentacles_Evaluator.TA,
                tentacles_management.evaluator_parent_inspection,
            )
            is not None
        )
    ...
    def _get_all_TA(self):
        return [
            evaluator
            for evaluator, activated in tentacles_manager_api.get_tentacles_activation(
                self.tentacles_setup_config
