...
async def store_run_metadata(
    bot_id, exchange_managers, start_time, user_inputs=None, flush=False
) -> dict:
    run_db = commons_databases.RunDatabasesProvider.instance().get_run_db(bot_id)
    ...
    metadata = await _get_trading_metadata(
        exchange_managers, start_time, user_inputs, run_dbs_identifier, False, None
    )
...
async def store_backtesting_run_metadata(
    exchange_managers, start_time, user_inputs, run_dbs_identifier, name
) -> dict:
...
async def _get_trading_metadata(
    ...
    try:
        ...
        trading_mode = tentacles.Trading.Mode.BlankTradingMode(
            exchange_managers[0].config, exchange_managers[0]
        )
    multi_exchanges_data = await _get_multi_exchange_data(
        exchange_managers, is_backtesting
    )
...
async def _get_multi_exchange_data(exchange_managers, is_backtesting):
    ...
    data_files = (
        list(
            set(
                data_file
                for exchange_manager in exchange_managers
                for data_file in trading_api.get_backtesting_data_files(
                    exchange_manager
                )
            )
        )
    ...
    markets_profitability = (
        _get_markets_profitability(exchange_managers, time_frames)
        if is_backtesting
        else {}
    )
    exchange_names = [
    ...
    trades = [
        ...
        for trade in trading_api.get_trade_history(
            exchange_manager, include_cancelled=False
        )
    ]
    entries = [
        trade
        ...
        and trade.side is trading_enums.TradeOrderSide.BUY
    ]
    win_rate = round(
        numpy.average(
            tuple(
    ...
    duration = (
        round(
            max(
                backtesting_api.get_backtesting_duration(
    ...
    if trading_api.get_is_backtesting(exchange_managers[0]):
        start_time = min(
            backtesting_api.get_backtesting_starting_time(
                exchange_manager.exchange.backtesting
            )
            for exchange_manager in exchange_managers
        )
    ...
    return {
        **backtesting_only_metadata,
        **{
            common_enums.BacktestingMetadata.GAINS.value: round(
                float(profitability), 8
            ),
            common_enums.BacktestingMetadata.PERCENT_GAINS.value: round(
                float(profitability_percent), 3
            ),
            common_enums.BacktestingMetadata.MARKETS_PROFITABILITY.value: {
                c: f"{round(float(v) * 100, 2)}%"
                for c, v in markets_profitability.items()
            },
            common_enums.BacktestingMetadata.END_PORTFOLIO.value: str(end_portfolio),
            common_enums.BacktestingMetadata.START_PORTFOLIO.value: str(
                origin_portfolio
            ),
            common_enums.BacktestingMetadata.WIN_RATE.value: win_rate,
            common_enums.BacktestingMetadata.DRAW_DOWN.value: draw_down or 0,
            common_enums.BacktestingMetadata.COEFFICIENT_OF_DETERMINATION_MAX_BALANCE.value: r_sq_max_balance
            or 0,
            common_enums.BacktestingMetadata.COEFFICIENT_OF_DETERMINATION_END_BALANCE.value: r_sq_end_balance
            or 0,
...
async def _get_coefficient_of_determination(
    exchange_managers, use_high_instead_of_end_balance
):
    try:
        ...
        for symbol in trading_api.get_trading_pairs(exchange_manager):
            ...
            try:
                markets_profitability[symbol] = (
                    backtesting.IndependentBacktesting.get_market_delta(
...
def _get_portfolio(exchange_managers):
    ...
    for exchange_manager in exchange_managers:
        try:
            exchange_origin_portfolio = trading_api.get_origin_portfolio(
                exchange_manager, as_decimal=False
            )
        except AttributeError:
            # no initialized portfolio on this exchange
            continue
        exchange_end_portfolio = trading_api.get_portfolio(
            exchange_manager, as_decimal=False
        )
        ...
        if exchange_manager.is_future:
            for position in trading_api.get_positions(exchange_manager):
                try:
                    exchange_end_portfolio[position.get_currency()]["position"] = float(
                        position.quantity
                    )
                except KeyError:
...
def _get_future_contracts_by_exchange(exchange_managers):
    return {
        trading_api.get_exchange_name(exchange_manager): {
            ...
            for symbol, contract in trading_api.get_pair_contracts(
                exchange_manager
            ).items()
        ...
        for exchange_manager in exchange_managers
        if exchange_manager.is_future
        and hasattr(exchange_manager.exchange, "pair_contracts")
    }
...
async def _get_single_exchange_data(
    ...
    for user_input in user_inputs:
        if not user_input["is_nested_config"]:
            try:
                formatted_user_inputs[user_input["tentacle"]][user_input["name"]] = (
                    user_input["value"]
                )
            except KeyError:
    ...
    leverage = 0
    if exchange_manager.is_future and hasattr(
        exchange_manager.exchange, "get_pair_future_contract"
    ):
        leverage = float(
            next(
                iter(trading_api.get_pair_contracts(exchange_manager).values())
            ).current_leverage
        )
    ...
    return {
        ...
        **{
            ...
            common_enums.BacktestingMetadata.NAME.value: name
            or trading_mode.get_name(),
            common_enums.BacktestingMetadata.LEVERAGE.value: leverage,
            common_enums.DBRows.TRADING_TYPE.value: exchange_type,
            common_enums.DBRows.REFERENCE_MARKET.value: trading_api.get_reference_market(
                exchange_manager.config
            ),
        },
    }
