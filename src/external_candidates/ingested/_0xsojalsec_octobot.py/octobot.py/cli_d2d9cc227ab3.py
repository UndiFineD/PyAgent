# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\cli.py
...
def update_config_with_args(starting_args, config: configuration.Configuration, logger):
    ...
    if starting_args.backtesting:
        if starting_args.backtesting_files:
            ...
            ] = starting_args.backtesting_files
        config.config[backtesting_constants.CONFIG_BACKTESTING][
    ...
    if starting_args.risk is not None and 0 < starting_args.risk <= 1:
        config.config[common_constants.CONFIG_TRADING][
...
def _log_environment(logger):
    try:
        ...
        logger.info(
            f"Running {bot_type} OctoBot on {os_util.get_current_platform()} "
...
async def _apply_db_bot_config(logger, config, community_auth) -> bool:
    try:
        ...
        profile_data, auth_data = (
            await community_auth.fetch_bot_tentacles_data_based_config(
        ...
        for auth_data_element in auth_data:
            logger.info(
                f"Applying {auth_data_element.internal_name} exchange auth details"
            )
...
def _apply_env_variables_to_config(logger, config):
    commands.download_and_select_profile(
        ...
        config,
        (
            [url.strip() for url in constants.TO_DOWNLOAD_PROFILES.split(",")]
        ...
        if not community_auth.is_initialized():
            if constants.IS_CLOUD_ENV:
                ...
                try:
                    ...
                    logger.info(
                        f"Auth key auth failure ({err}). Trying other methods if available."
                    )
                if (
                    not authenticated
                    and constants.USER_ACCOUNT_EMAIL
                    and constants.USER_PASSWORD_TOKEN
                ):
                    try:
                        logger.debug("Attempting password token authentication")
                        await community_auth.login(
                            constants.USER_ACCOUNT_EMAIL,
                            None,
                            password_token=constants.USER_PASSWORD_TOKEN,
                        )
                    except authentication.AuthenticationError as err:
                        logger.info(
                            f"Password token auth failure ({err}). Trying with saved session."
                        )
            if not community_auth.is_initialized():
                ...
                await community_auth.async_init_account(
                    fetch_private_data=fetch_private_data
                )
        if not community_auth.is_logged_in():
            logger.info("No authenticated community account")
    except authentication.FailedAuthentication as err:
        logger.error(
            f"Failed authentication when initializing community authenticator: {err}"
        )
    except Exception as err:
...
async def _async_load_community_data(community_auth, config, logger, is_first_startup):
    if constants.IS_CLOUD_ENV and is_first_startup:
        ...
        if constants.USE_FETCHED_BOT_CONFIG:
            ...
            await _apply_community_startup_info_to_config(
                logger, config, community_auth
            )


def _apply_forced_configs(community_auth, logger, config, is_first_startup):
    asyncio.run(
        _async_load_community_data(community_auth, config, logger, is_first_startup)
    )
...
def _repair_with_default_profile(config, logger):
    logger.error(
        "OctoBot can't start without a valid profile configuration. Selecting default profile ..."
    )
...
def _load_or_create_tentacles(community_auth, config, logger):
    ...
    if os.path.isfile(
        tentacles_manager_constants.USER_REFERENCE_TENTACLE_CONFIG_FILE_PATH
    ):
...
def start_octobot(args, default_config_file=None):
    ...
    try:
        ...
        community_auth = (
            None
            if args.backtesting
            else asyncio.run(_get_authenticated_community_if_possible(config, logger))
        )
        ...
        if args.backtesting:
            ...
            bot = octobot_class.OctoBot(
                config,
                community_authenticator=community_auth,
...
def octobot_parser(parser, default_config_file=None):
    parser.add_argument(
        "-v", "--version", help="Show OctoBot current version.", action="store_true"
    )
    parser.add_argument(
        "-s",
        "--simulate",
    ...
    parser.add_argument(
        "-u", "--update", help="Update OctoBot to latest version.", action="store_true"
    )
    parser.add_argument(
        "-rts",
    ...
    parser.add_argument(
        "-nl",
        "--no_logs",
        help="Disable OctoBot logs in backtesting.",
        action="store_true",
    )
    parser.add_argument(
        "-nt",
    ...
    parser.add_argument(
        "--encrypter",
        help="Start the exchange api keys encrypter. This tool is useful to manually add"
    ...
    parser.add_argument(
        "--identifier", help="OctoBot community identifier.", type=str, nargs=1
    )
    parser.add_argument(
        "-o",
        "--strategy_optimizer",
...
def start_background_octobot_with_args(
    ...
    if in_subprocess:
        bot_process = multiprocessing.Process(
            target=start_octobot, args=(args, default_config_file)
        )
        bot_process.start()
        return bot_process
    else:
...
def main(args=None, default_config_file=None):
    ...
    try:
        ...
        if packaging_version.Version(VERSION) < packaging_version.Version(
            MIN_TENTACLE_MANAGER_VERSION
        ):
            print(
                "OctoBot requires OctoBot-Tentacles-Manager in a minimum version of "
                + MIN_TENTACLE_MANAGER_VERSION
        ...
        print(
            "OctoBot requires OctoBot-Tentacles-Manager, you can install it using "
            "python3 -m pip install -U OctoBot-Tentacles-Manager",
            file=sys.stderr,
        )
        sys.exit(-1)
