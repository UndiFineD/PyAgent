# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\configuration_manager.py
...
def config_health_check(
    config: configuration.Configuration, in_backtesting: bool
    ...
    if common_constants.CONFIG_EXCHANGES in config.config:
        for exchange, exchange_config in config.config[
            ...
            for key in common_constants.CONFIG_EXCHANGE_ENCRYPTED_VALUES:
                try:
                    if not configuration.handle_encrypted_value(
    ...
    try:
        ...
        if trader_enabled:
            simulator_enabled = trading_api.is_trader_simulator_enabled_in_config(
                config.config
            )
            if simulator_enabled:
                logger.error(
                    f"Impossible to activate a trader simulator additionally to a "
    ...
    if not (
        in_backtesting
        or trading_api.is_trader_enabled_in_config(config.config)
        or trading_api.is_trader_simulator_enabled_in_config(config.config)
    ):
        logger.error(
            f"Real trader and trader simulator are deactivated in configuration. This will prevent OctoBot "
    ...
    if should_replace_config:
        try:
            ...
            logger.error(
                f"Save of the health checked config failed : {e}, "
...
def get_default_tentacles_url(version=None):
    beta_tentacle_bundle = os.getenv(
        constants.ENV_BETA_TENTACLES_PACKAGE_NAME, constants.BETA_TENTACLE_PACKAGE_NAME
    )
    # use beta tentacles repository for beta tentacles
    tentacles_repository = (
        os.getenv(
            constants.ENV_BETA_TENTACLES_REPOSITORY, constants.BETA_TENTACLES_REPOSITORY
        )
        if version == beta_tentacle_bundle
        else os.getenv(
            constants.ENV_TENTACLES_REPOSITORY, constants.TENTACLES_REPOSITORY
        )
    )
    if version is None:
        version = (
            constants.TENTACLES_REQUIRED_VERSION
            if constants.TENTACLES_REQUIRED_VERSION
            else constants.LONG_VERSION
        )
    return os.getenv(
...
def load_default_tentacles_config(profile_folder):
    if os.path.isdir(tentacles_manager_constants.USER_REFERENCE_TENTACLE_CONFIG_PATH):
        shutil.copyfile(
            ...
            os.path.join(
                profile_folder,
                tentacles_manager_constants.constants.CONFIG_TENTACLES_FILE,
            ),
        )
        shutil.copytree(
            tentacles_manager_constants.USER_REFERENCE_TENTACLE_SPECIFIC_CONFIG_PATH,
            os.path.join(
                profile_folder,
...
def migrate_from_previous_config(config):
    ...
    previous_tentacles_config = os.path.join(
        common_constants.USER_FOLDER, "tentacles_config"
    )
    previous_tentacles_config_save = os.path.join(
        common_constants.USER_FOLDER, "tentacles_config.back"
    )
    if os.path.isdir(previous_tentacles_config) and not os.path.isdir(
        ...
        load_default_tentacles_config(
            os.path.join(
                common_constants.USER_PROFILES_FOLDER, common_constants.DEFAULT_PROFILE
            )
        )
