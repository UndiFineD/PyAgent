...
async def get_current_octobots_stats():
    bot_metrics_url = (
        f"{identifiers_provider.IdentifiersProvider.COMMUNITY_API_URL}/stats"
    )
...
async def _get_stats(endpoint_by_key):
        ...
        try:
            async with aiohttp_util.ssl_fallback_aiohttp_client_session(url) as session:
                async with session.get(url) as resp:
                    if resp.status > 299:
                        logger.error(
                            f"Error when getting community status : error code={resp.status}"
                        )
                    else:
    ...
    await asyncio.gather(
        *(get_stats(endpoint, key) for key, endpoint in endpoint_by_key.items())
    )
