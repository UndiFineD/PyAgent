# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\identifiers_provider.py
...
class IdentifiersProvider:
    ...
    def use_production():
        ...
        IdentifiersProvider.FRONTEND_PASSWORD_RECOVER_URL = (
            constants.OCTOBOT_COMMUNITY_RECOVER_PASSWORD_URL
        )
        ...
        IdentifiersProvider._register_environment(
            enums.CommunityEnvironments.Production
        )

    @staticmethod
    def use_staging():
        IdentifiersProvider.COMMUNITY_URL = constants.STAGING_OCTOBOT_COMMUNITY_URL
        IdentifiersProvider.COMMUNITY_API_URL = (
            constants.STAGING_OCTOBOT_COMMUNITY_API_URL
        )
        IdentifiersProvider.FRONTEND_PASSWORD_RECOVER_URL = (
    ...
    def _register_environment(env):
        if IdentifiersProvider.ENABLED_ENVIRONMENT != env:
            logging.get_logger(IdentifiersProvider.__name__).debug(
                f"Using {env.value} Community environment."
            )
    ...
    def is_staging_environment_enabled(config: dict):
        try:
            env = config[constants.CONFIG_COMMUNITY][
                constants.CONFIG_COMMUNITY_ENVIRONMENT
            ]
            return (
                enums.CommunityEnvironments(env) is enums.CommunityEnvironments.Staging
            )
