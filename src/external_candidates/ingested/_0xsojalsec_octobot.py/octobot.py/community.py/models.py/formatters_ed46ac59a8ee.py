...
def _format_trade(trade: dict, exchange_name: str, bot_id: str):
    ...
    return {
        ...
        backend_enums.TradeKeys.TRADE_ID.value: trade[
        ...
        backend_enums.TradeKeys.PRICE.value: float(
            trade[trading_enums.ExchangeConstantsOrderColumns.PRICE.value]
        ),
        backend_enums.TradeKeys.QUANTITY.value: float(
            trade[trading_enums.ExchangeConstantsOrderColumns.AMOUNT.value]
        ),
        backend_enums.TradeKeys.SYMBOL.value: trade[
    ...
    return [
        {
            ...
            backend_enums.PositionKeys.SIDE.value: position[
            ...
            backend_enums.PositionKeys.MAINTENANCE_MARGIN_RATE.value: (
                float(
                    position[
                        trading_enums.ExchangeConstantsPositionColumns.MAINTENANCE_MARGIN_RATE.value
                    ]
                )
                if position[
                    trading_enums.ExchangeConstantsPositionColumns.MAINTENANCE_MARGIN_RATE.value
    ...
    return [
        {
            ...
            backend_enums.OrderKeys.SYMBOL.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ][trading_enums.ExchangeConstantsOrderColumns.SYMBOL.value],
            backend_enums.OrderKeys.PRICE.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ][trading_enums.ExchangeConstantsOrderColumns.PRICE.value],
            backend_enums.OrderKeys.TIME.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ...
            backend_enums.OrderKeys.QUANTITY.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ][trading_enums.ExchangeConstantsOrderColumns.AMOUNT.value],
            backend_enums.OrderKeys.SIDE.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ][trading_enums.ExchangeConstantsOrderColumns.SIDE.value],
            backend_enums.OrderKeys.TRIGGER_ABOVE.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ].get(trading_enums.ExchangeConstantsOrderColumns.TRIGGER_ABOVE.value),
            backend_enums.OrderKeys.EXCHANGE_ID.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
            ][trading_enums.ExchangeConstantsOrderColumns.EXCHANGE_ID.value],
            backend_enums.OrderKeys.REDUCE_ONLY.value: storage_order[
                trading_constants.STORAGE_ORIGIN_VALUE
...
def to_community_exchange_internal_name(
    bot_exchange_internal_name: str, exchange_type: str
) -> str:
...
def create_profile_name(
    bot_strategy_slug: str, nested_strategy_slug: typing.Optional[str]
) -> str:
...
def get_master_and_nested_product_slug_from_profile_name(
    ...
    if "[" in profile_name and "]" in profile_name:
        ...
        nested_product_slug = profile_name[
            profile_name.index("[") + 1 : profile_name.index("]")
        ]
...
def ensure_profile_data_exchanges_internal_name_and_type(
    ...
    if profile_data.exchanges:
        for exchange_data in profile_data.exchanges:
            exchange_type = get_exchange_type_from_internal_name(
                exchange_data.internal_name
            )
            exchange_data.internal_name = to_bot_exchange_internal_name(
                exchange_data.internal_name
            )
...
def get_exchange_type_from_availability(exchange_availability: dict) -> str:
    ...
    # 1. try futures
    if (
        exchange_availability.get("futures")
        == backend_enums.ExchangeSupportValues.SUPPORTED.value
    ):
        return commons_constants.CONFIG_EXCHANGE_FUTURE
    # 2. try spot
    if (
        exchange_availability.get("spot")
        == backend_enums.ExchangeSupportValues.SUPPORTED.value
    ):
        return commons_constants.CONFIG_EXCHANGE_SPOT
    # 3. try market_making
    if (
        exchange_availability.get("market_making")
        == backend_enums.ExchangeSupportValues.SUPPORTED.value
    ):
...
def format_portfolio(
    ...
    return {
        backend_enums.PortfolioKeys.CONTENT.value: format_portfolio_content(
            content, price_by_asset
        ),
    ...
    return [
        {
            ...
            backend_enums.PortfolioAssetKeys.QUANTITY.value: float(
                quantity[commons_constants.PORTFOLIO_TOTAL]
            ),
            backend_enums.PortfolioAssetKeys.VALUE.value: float(
                quantity[commons_constants.PORTFOLIO_TOTAL]
            )
