# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\models\strategy_data.py
...
CATEGORY_NAME_TRANSLATIONS_BY_SLUG = {"coingecko-index": {"en": "Crypto Basket"}}
...
class CategoryData(commons_dataclasses.FlexibleDataclass):
    ...
    def get_name(self, locale, default_locale=constants.DEFAULT_LOCALE):
        return CATEGORY_NAME_TRANSLATIONS_BY_SLUG.get(
            self.slug, self.name_translations
...
def is_custom_strategy_profile(profile: profiles.ProfileData) -> bool:
    return profile.profile_details.name and profile.profile_details.name.startswith(
