# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\models\community_user_account.py
...
class CommunityUserAccount:
    ...
    NO_SELECTED_BOT_DESC = (
        "No selected bot. Please select a bot to enable your community features."
    )
    ...
    def is_self_hosted(self, bot):
        return (
            self._get_bot_deployment(bot).get(
                backend_enums.BotDeploymentKeys.TYPE.value,
    ...
    def get_selected_bot_current_portfolio_id(self):
        return self._selected_bot_raw_data[
            backend_enums.BotKeys.CURRENT_PORTFOLIO_ID.value
        ]

    def get_selected_bot_current_config_id(self):
        return self._selected_bot_raw_data[
            backend_enums.BotKeys.CURRENT_CONFIG_ID.value
        ]
    ...
    def _get_user_data_metadata(self):
        return self._profile_raw_data.get(
            backend_enums.UserKeys.USER_METADATA.value, {}
        )
