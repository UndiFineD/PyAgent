...
class CommunityMQTTFeed(abstract_feed.AbstractFeed):
    ...
    def __init__(self, feed_url, authenticator):
        ...
        self._default_callbacks_by_subscription_topic = (
            self._build_default_callbacks_by_subscription_topic()
        )
        self._stop_on_cfg_action: typing.Optional[
    ...
    async def start(
        ...
        try:
            ...
            self.logger.exception(
                err, True, f"Timeout when connecting to mqtt device: {err}"
            )
        except Exception as err:
            self.logger.exception(
                err, True, f"Unexpected error when connecting to mqtt device: {err}"
            )
    ...
    async def _config_feed_callback(self, data: dict):
        ...
        if action == enums.CommunityConfigurationActions.EMAIL_CONFIRM_CODE.value:
            ...
            self.authenticator.user_account.last_email_address_confirm_code_email_content = (
                email_body
            )
        ...
        if (
            action
            and self._stop_on_cfg_action
            and self._stop_on_cfg_action.value == action
        ):
    ...
    async def register_feed_callback(self, channel_type, callback, identifier=None):
        ...
        if topic not in self._subscription_topics:
            ...
            if self._valid_auth:
                ...
                self.logger.error(
                    f"Can't subscribe to {channel_type.name} feed, invalid authentication"
                )
    ...
    async def _on_message(self, client, topic, payload, qos, properties):
        try:
            ...
            self.logger.exception(
                err, True, f"Unexpected error when reading message: {err}"
            )
            return
    ...
    def _get_feed_callbacks(self, topic) -> list:
        return self._default_callbacks_by_subscription_topic.get(
            topic, []
        ) + self.feed_callbacks.get(topic, [])
    ...
    def _ensure_supported(self, parsed_message):
        if not (
            ...
            < packaging_version.Version(
                constants.COMMUNITY_FEED_CURRENT_EXCLUDED_MAXIMUM_VERSION
            )
        ):
    ...
    def _on_connect(self, client, flags, rc, properties):
        ...
        self._subscribe(
            self._subscription_topics.union(self._get_default_subscription_topics())
        )
    ...
    async def _reconnect(self, client):
        try:
            ...
            while not self.should_stop:
                ...
                try:
                    self.logger.info(
                        f"Reconnecting, client_id: {self._get_username(client)} (attempt {attempt})"
                    )
                    await self._connect()
                    await asyncio.sleep(self.RECONNECT_ENSURE_DELAY)
                    if self.is_connected():
                        self.logger.info(
                            f"Reconnected, client_id: {self._get_username(client)}"
                        )
                        return
                    ...
                    self.logger.debug(
                        f"Reconnect attempt {attempt} {'succeeded' if error is None else 'failed'}."
                    )
                    attempt += 1
                self.logger.debug(
                    f"Error while reconnecting: {error}. Trying again in {delay} seconds."
                )
    ...
    def _on_disconnect(self, client, packet, exc=None):
        ...
        if self.should_stop:
            ...
            if self._connected_at_least_once:
                self.logger.info(
                    f"Disconnected, client_id: {self._get_username(client)}"
                )
    ...
    def _on_subscribe(self, client, mid, qos, properties):
        ...
        for subscription, granted_qos in zip(subscriptions, qos):
            ...
            if granted_qos >= gmqtt.constants.SubAckReasonCode.UNSPECIFIED_ERROR.value:
                ...
                if self._subscription_attempts < self.MAX_SUBSCRIPTION_ATTEMPTS * len(
                    subscriptions
                ):
    ...
    async def _connect(self):
        ...
        if device_uuid is None:
            ...
            raise errors.BotError(
                "mqtt device uuid is None, impossible to connect client"
            )
        ...
        self._connect_task = asyncio.create_task(
            self._mqtt_client.connect(
                self.feed_url, self.mqtt_broker_port, version=self.MQTT_VERSION
            )
        )
        try:
            ...
            self.logger.error(
                f"Can't connect to server, your device uuid might be invalid. "
    ...
    def _subscribe(self, topics):
        if not topics:
            ...
            return
        subscriptions = [
            gmqtt.Subscription(topic, qos=self.default_QOS) for topic in topics
        ]
