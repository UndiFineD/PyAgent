# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\feeds\community_ws_feed.py
...
class CommunityWSFeed(abstract_feed.AbstractFeed):
    ...
    async def start(
        self, stop_on_cfg_action: typing.Optional[enums.CommunityConfigurationActions]
    ):
    ...
    async def start_consumer(self):
        while not self.should_stop:
            try:
                ...
                async for message in self.websocket_connection:
                    try:
                        ...
                        self.logger.exception(
                            e, True, f"Error while consuming feed: {e}"
                        )
            except (websockets.ConnectionClosedError, ConnectionRefusedError):
                if self._reconnect_attempts == 0:
                    self.logger.warning(
                        f"Disconnected from community, reconnecting now."
                    )
                    # first reconnect instantly
                else:
                    # use self.RECONNECT_DELAY + or - 0-10 %
                    reconnect_delay = self.RECONNECT_DELAY * (
                        1 + (random.random() * 2 - 1) / 10
                    )
                    self.logger.warning(
                        f"Disconnected from community, retrying to connect in {reconnect_delay}s "
                ...
                self.logger.exception(
                    e, True, f"Unexpected exception when receiving community feed: {e}"
                )
    ...
    async def connection_watcher(self):
        while not self.should_stop:
            ...
            if (
                self._last_ping_time is not None
                and time.time() - self._last_ping_time > self.STALE_CONNECTION_TIMEOUT
            ):
                self.logger.error(
                    "Stale community connection detected, resetting the connection"
                )
    ...
    def _ensure_supported(self, parsed_message):
        if packaging_version.Version(
            parsed_message[commons_enums.CommunityFeedAttrs.VERSION.value]
    ...
    async def send(
        ...
        channel_type,
        identifier,
        command=COMMANDS.MESSAGE.value,
        ...
        await self.websocket_connection.send(
            self._build_ws_message(message, channel_type, command, identifier)
        )
    ...
    def _get_channel_type(self, message):
        return commons_enums.CommunityChannelTypes(
            message[commons_enums.CommunityFeedAttrs.CHANNEL_TYPE.value]
        )

    def _get_identifier(self, message):
        return self._identifier_by_stream_id[
            message[commons_enums.CommunityFeedAttrs.STREAM_ID.value]
        ]

    def _build_channel_identifier(self):
        return json.dumps({"channel": CHANNELS.MESSAGE.value})
    ...
    async def _connect(self):
        ...
        if self.authenticator.initialized_event is not None:
            await asyncio.wait_for(
                self.authenticator.initialized_event.wait(), self.INIT_TIMEOUT
            )
        if not self.authenticator.is_logged_in():
            raise authentication.AuthenticationRequired(
                "OctoBot Community authentication is required to "
