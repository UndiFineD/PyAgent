# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\feeds\community_supabase_feed.py
...
class CommunitySupabaseFeed(abstract_feed.AbstractFeed):
    ...
    def _should_process(self, parsed_message):
        if (
            parsed_message[commons_enums.CommunityFeedAttrs.ID.value]
            in self._processed_messages
        ):
            self.logger.debug(
                f"Ignored already processed message with id: "
        ...
        if len(self._processed_messages) > self.MAX_MESSAGE_ID_CACHE_SIZE:
            self._processed_messages = [
                message_id
                for message_id in self._processed_messages[
                    self.MAX_MESSAGE_ID_CACHE_SIZE // 2 :
                ]
            ]
        return True
    ...
    async def _process_message(self, table: str, message: dict):
        try:
            ...
            self.logger.exception(
                err, True, f"Unexpected error when processing message: {err}"
            )

    async def start(
        self,
        stop_on_cfg_action: typing.Optional[
            octobot.enums.CommunityConfigurationActions
        ],
    ):
    ...
    async def _subscribe_to_table_if_necessary(self, table):
        if (
            table
            not in self.authenticator.supabase_client.get_subscribed_channel_tables()
        ):
            if self.authenticator.is_logged_in():
                await self._subscribe_to_table(table)
            else:
                raise authentication.AuthenticationRequired(
                    "You need to be authenticated to be able to " "connect to signals"
                )
    ...
    async def _subscribe_to_table(self, table):
        ...
        await channel.on(self.INSERT_EVENT, self._message_callback).subscribe(
            self._subscribed_callback
        )
    ...
    async def _subscribed_callback(self, state: str, message: realtime.Message):
        self.logger.debug(
            f"Subscribe to {self._get_table_from_message(message)}: {state}"
        )
    ...
    async def send(self, message, channel_type, identifier, **kwargs):
        ...
        if not self.authenticator.is_logged_in():
            self.logger.warning(
                f"Can't send {channel_type.name}, invalid feed authentication."
            )
            return
        self.logger.info(
            f"Sending signal on identifier: {identifier}, message: {message}"
        )
