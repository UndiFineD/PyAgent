# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\errors_upload\sentry_aiohttp_transport.py
...
class SentryAiohttpTransport(sentry_sdk.HttpTransport):
    ...
    async def _async_send_request(
        ...
        try:
            ...
            logging.getLogger(self.__class__.__name__).warning(
                f"Sentry post error: {err} {err.__class__.__name__}"
            )
    ...
    async def _async_send_envelope(
        ...
        envelope = sentry_sdk.envelope.Envelope(
            headers=envelope.headers, items=new_items
        )
...
class AiohttpWorker:
    ...
    def flush(self, timeout: float, callback=None) -> None:
        sentry_sdk.utils.logger.debug(
            "Custom background worker got flush request, ignored"
        )
