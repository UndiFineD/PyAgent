# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\authentication.py
    ...
    async def bot_data_update_wrapper(*args, raise_errors=False, **kwargs):
        ...
        try:
            ...
            self.logger.exception(
                err, True, f"Error when calling {func.__name__} {err}"
            )
        finally:
...
class CommunityAuthentication(authentication.Authenticator):
    ...
    def __init__(
        self, config=None, backend_url=None, backend_key=None, use_as_singleton=True
    ):
        super().__init__(use_as_singleton=use_as_singleton)
        self.config = config
        self.backend_url = (
            backend_url or identifiers_provider.IdentifiersProvider.BACKEND_URL
        )
        self.backend_key = (
            backend_key or identifiers_provider.IdentifiersProvider.BACKEND_KEY
        )
        self.configuration_storage = supabase_backend.ASyncConfigurationStorage(
            self.config
        )
    ...
    async def get_strategy_profile_data(
        ...
        profile_data = await self.supabase_client.fetch_product_config(
            strategy_id, product_slug=product_slug
        )
    ...
    def is_feed_connected(self):
        return (
            self._community_feed is not None
            and self._community_feed.is_connected_to_remote_feed()
        )
    ...
    async def get_gpt_signal(
        ...
        return await self.supabase_client.fetch_gpt_signal(
            exchange, symbol, time_frame, candle_open_time, version
        )

    async def get_gpt_signals_history(
        self,
        exchange: typing.Union[str, None],
        symbol: str,
        time_frame: commons_enums.TimeFrames,
    ...
    async def update_supports(self):
        def _supports_mock():
            return {
                "data": {
                    "attributes": {"support_role": self.user_account.get_support_role()}
                }
            }
    ...
    def _create_client(self):
        return supabase_backend.CommunitySupabaseClient(
            self.backend_url, self.backend_key, self.configuration_storage
        )
    ...
    def is_using_the_current_loop(self):
        return (
            self.supabase_client.event_loop is None
    ...
    def init_account(self, fetch_private_data):
        if fetch_private_data and self._fetched_private_data is None:
            self._fetched_private_data = asyncio.Event()
        self._fetch_account_task = asyncio.create_task(
            self._initialize_account(fetch_private_data=fetch_private_data)
        )
    ...
    async def _create_community_feed_if_necessary(self) -> bool:
        if self._community_feed is None:
            ...
            self._community_feed = community_feeds.community_feed_factory(
                self, constants.COMMUNITY_FEED_DEFAULT_TYPE
            )
            return True
        return False
    ...
    async def _ensure_init_community_feed(
        ...
        if (
            not self._community_feed.is_connected()
            and self._community_feed.can_connect()
        ):
            if (
                self.initialized_event is not None
                and not self.initialized_event.is_set()
            ):
                await asyncio.wait_for(
                    self.initialized_event.wait(), self.LOGIN_TIMEOUT
                )
        await self._community_feed.start(stop_on_cfg_action)

    async def register_feed_callback(
        self,
        channel_type: commons_enums.CommunityChannelTypes,
        callback,
        identifier=None,
    ):
        try:
            await self._ensure_init_community_feed()
            await self._community_feed.register_feed_callback(
                channel_type, callback, identifier=identifier
            )
    ...
    async def trigger_wait_for_email_address_confirm_code_email(self):
        ...
        await self._ensure_init_community_feed(
            enums.CommunityConfigurationActions.EMAIL_CONFIRM_CODE
        )
    ...
    async def wait_for_private_data_fetch_if_processing(self):
        ...
        if (
            self.is_logged_in()
            and self._fetched_private_data is not None
            and not self._fetched_private_data.is_set()
        ):
            # ensure login details have been fetched
            await asyncio.wait_for(
    ...
    async def login(
        ...
        with self._login_process():
            if auth_key and not password_token:
                password_token = await self.supabase_client.get_otp_with_auth_key(
                    email, auth_key
                )
            if password_token:
    ...
    async def auto_reauthenticate(self) -> bool:
        if (
            constants.IS_CLOUD_ENV
            and constants.USER_ACCOUNT_EMAIL
            and constants.USER_AUTH_KEY
        ):
    ...
    async def register(self, email, password):
        if self.must_be_authenticated_through_authenticator():
            raise authentication.AuthenticationError(
                "Creating a new account is not authorized on this environment."
            )
    ...
    async def _load_bot_if_selected(self):
        ...
        if len(self.user_account.get_all_user_bots_raw_data()) == 0:
            await self.select_bot(
                self.user_account.get_bot_id(await self.create_new_bot())
            )
        # more than one possible bot, can't auto-select one

    async def create_new_bot(self):
        deployment_type = (
            backend_enums.DeploymentTypes.CLOUD
    ...
    async def select_bot(self, bot_id):
        ...
        bot_name = self.user_account.get_bot_name_or_id(
            self.user_account.get_selected_bot_raw_data()
        )
    ...
    async def load_user_bots(self):
        self.user_account.set_all_user_bots_raw_data(
            self._get_self_hosted_bots(await self.supabase_client.fetch_bots())
        )
    ...
    async def get_startup_info(self):
        if self._startup_info is None:
            ...
            self._startup_info = startup_info.StartupInfo.from_dict(
                await self.supabase_client.fetch_startup_info(self.user_account.bot_id)
            )
    ...
    def has_open_source_package(self) -> bool:
        return bool(self.get_owned_packages()) or (
            not self.is_logged_in() and self.was_connected_with_remote_packages()
        )
    ...
    def is_logged_in_and_has_selected_bot(self):
        return (
            self.supabase_client.is_admin or self.is_logged_in()
        ) and self.user_account.bot_id is not None
    ...
    def _get_self_hosted_bots(self, bots):
        return [
            ...
            and not self.user_account.is_archived(bot)
        ]
    ...
    def is_logged_in(self):
        return bool(
            self.supabase_client.is_signed_in() and self.user_account.has_user_data()
        )
    ...
    def _update_supports(self, resp_status, json_data):
        if resp_status == 200:
            self.user_account.supports = (
                community_supports.CommunitySupports.from_community_dict(json_data)
            )
            self.logger.debug(f"Fetched supports data.")
        else:
            self.logger.error(
                f"Error when fetching community support, " f"error code: {resp_status}"
            )
    ...
    async def _initialize_account(self, minimal=False, fetch_private_data=True):
        try:
            ...
            self.logger.exception(
                e,
                True,
                f"Error when fetching community data, "
            ...
            self.logger.exception(
                e,
                True,
                f"Error when fetching community supports: {e}({e.__class__.__name__})",
            )
        finally:
    ...
    async def fetch_private_data(self, reset=False):
        try:
            ...
            if constants.DISABLE_COMMUNITY_EXTENSIONS_CHECK:
                ...
                not self.user_account.community_package_urls or not mqtt_uuid
            ):
                self.successfully_fetched_tentacles_package_urls = False
                ...
                self.logger.debug(
                    f"Account extension packages: {', '.join(packages) if packages else packages}"
                )
                self.save_installed_package_urls(package_urls)
                has_tentacles_to_install = await community_tentacles_packages.has_tentacles_to_install_and_uninstall_tentacles_if_necessary(
                    self
                )
                ...
                if (
                    tradingview_email
                    and tradingview_email != self.get_saved_tradingview_email()
                ):
                    self.save_tradingview_email(tradingview_email)
        except Exception as err:
            self.logger.exception(
                err, True, f"Unexpected error when fetching package urls: {err}"
            )
        finally:
    ...
    async def _fetch_extensions_details(
        self, mqtt_uuid: typing.Optional[str]
        ...
        packages = [
            package for package in extensions_details["paid_package_slugs"] if package
        ]
        urls = [url for url in extensions_details["package_urls"] if url]
    ...
    async def fetch_checkout_url(self, payment_method: str, redirect_url: str):
        try:
            ...
            url_details = await self.supabase_client.fetch_checkout_url(
    ...
    def save_installed_package_urls(self, package_urls: list[str]):
        self._save_value_in_config(
            constants.CONFIG_COMMUNITY_PACKAGE_URLS, package_urls
        )

    def save_tradingview_email(self, tradingview_email: str):
        self._save_value_in_config(
            constants.CONFIG_COMMUNITY_TRADINGVIEW_EMAIL, tradingview_email
        )

    def save_tradingview_email_confirmed(self, confirmed: bool):
        self._save_value_in_config(
            constants.CONFIG_COMMUNITY_TRADINGVIEW_EMAIL_CONFIRMED, confirmed
        )
    ...
    def _save_bot_scoped_data_identifier(self, identifier: str):
        self._save_value_in_config(
            constants.CONFIG_COMMUNITY_LOCAL_DATA_IDENTIFIER, identifier
        )
    ...
    def _get_saved_bot_scoped_data_identifier(self) -> str:
        return self._get_value_in_config(
            constants.CONFIG_COMMUNITY_LOCAL_DATA_IDENTIFIER
        )
    ...
    def is_tradingview_email_confirmed(self) -> bool:
        return (
            self._get_value_in_config(
                constants.CONFIG_COMMUNITY_TRADINGVIEW_EMAIL_CONFIRMED
            )
            is True
        )
    ...
    def _get_saved_bot_id(self) -> str:
        return constants.COMMUNITY_BOT_ID or self._get_value_in_config(
            constants.CONFIG_COMMUNITY_BOT_ID
        )
    ...
    async def _auth_handler(self):
        ...
        try:
            ...
            if should_warn:
                self.logger.warning(
                    f"Invalid authentication details, please re-authenticate. {e}"
                )
            if await self.has_login_info():
                await self.logout()
            ...
            self.logger.exception(
                e, True, f"Error when trying to refresh community login: {e}"
            )

    def _ensure_email(self, email):
        if constants.USER_ACCOUNT_EMAIL and email != constants.USER_ACCOUNT_EMAIL:
            raise authentication.AuthenticationError(
                "The given email doesn't match the expected user email."
            )
    ...
    async def update_trades(self, trades: list, exchange_name: str, reset: bool):
        ...
        trades_to_upload = (
            trades
            if len(trades) <= self.MAX_UPLOADED_TRADES_COUNT
            else (
                sorted(
                    trades,
                    key=lambda x: x[
        ...
        if formatted_trades := formatters.format_trades(
            trades_to_upload, exchange_name, self.user_account.bot_id
        ):
    ...
    async def update_orders(self, orders_by_exchange: dict[str, list]):
        ...
        await self.supabase_client.update_bot_orders(
            self.user_account.bot_id, formatted_orders
        )
    ...
    async def update_positions(self, positions_by_exchange: dict[str, list]):
        ...
        await self.supabase_client.update_bot_positions(
            self.user_account.bot_id, formatted_positions
        )
        self.logger.info(
            f"Bot positions updated: using {len(formatted_positions)} positions"
        )
    ...
    async def update_portfolio(
        ...
        try:
            ...
            if (
                reset
                or self.user_account.get_selected_bot_current_portfolio_id() is None
            ):
            ...
            self.logger.debug(
                f"Error when updating community portfolio {err} (missing reference market value)"
            )

    @_bot_data_update
    async def update_bot_config_and_stats(self, profitability):
        formatted_portfolio = formatters.format_portfolio_with_profitability(
            profitability
        )
    ...
    async def _update_deployment_activity(self):
        try:
            ...
            if not deployment_id:
                self.logger.debug(
                    f"Missing deployment id to update last deployment activity time."
                )
                return
            ...
            await self.supabase_client.update_deployment(
                ...
                self.supabase_client.get_deployment_activity_update(
                    current_time,
                    current_time
                    + commons_constants.TIMER_BETWEEN_METRICS_UPTIME_UPDATE,
                ),
            )
        except KeyError:
            self.logger.debug(
