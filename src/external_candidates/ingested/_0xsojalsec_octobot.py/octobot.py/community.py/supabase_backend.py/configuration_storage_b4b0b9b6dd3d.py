...
class SyncConfigurationStorage(gotrue.SyncSupportedStorage):
    ...
    def set_configuration(
        self, configuration: octobot_commons.configuration.Configuration
    ):
    ...
    def _get_value_in_config(self, key):
        if self._configuration is not None:
            try:
                return self._configuration.config[octobot.constants.CONFIG_COMMUNITY][
                    key
                ]
            except KeyError:
                return ""
...
class ASyncConfigurationStorage(gotrue.AsyncSupportedStorage):
    def __init__(self, configuration: octobot_commons.configuration.Configuration):
        self.sync_storage: SyncConfigurationStorage = SyncConfigurationStorage(
            configuration
        )
