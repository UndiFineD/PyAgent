# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\supabase_backend\community_supabase_client.py
...
class CommunitySupabaseClient(supabase_client.AuthenticatedAsyncSupabaseClient):
        ...
        try:
            ...
            if self._is_email_already_in_use(resp.user):
                raise authentication.AuthenticationError(
                    error_translator.translate_error_code("email_exists")
                )
            ...
            raise authentication.AuthenticationError(
                error_translator.translate_error_code(err.code)
            ) from err
    ...
    def _requires_email_validation(self, user: gotrue.types.User) -> bool:
        return (
            user.app_metadata.get("provider") == "email" and user.confirmed_at is None
        )
    ...
    async def restore_session(self):
        ...
        if not self.is_signed_in():
            raise authentication.FailedAuthentication(
                f"Community auth error: restoring session failed"
            )
    ...
    async def update_metadata(self, metadata_update) -> dict:
        return (
            await self.auth.update_user({"data": metadata_update})
        ).user.model_dump()
    ...
    async def get_otp_with_auth_key(self, user_email: str, auth_key: str) -> str:
        try:
            resp = await self.functions.invoke(
                "create-auth-token",
                {
                    "headers": {"User-Auth-Token": auth_key},
            ...
            raise authentication.AuthenticationError(
                f"Community auth error: invalid auth key authentication details"
            )
    ...
    async def fetch_extensions(self, mqtt_uuid: typing.Optional[str]) -> dict:
        resp = await self.functions.invoke(
            "os-paid-package-api",
            {
                "body": {"action": "get_extension_details", "mqtt_id": mqtt_uuid},
            },
        )
    ...
    async def fetch_bot(self, bot_id) -> dict:
        with jwt_expired_auth_raiser():
            try:
                ...
                return (
                    await self.table("bots")
                    .select(
                        "*,bot_deployment:bot_deployments!bots_current_deployment_id_fkey(*)"
                    )
                    .eq(enums.BotKeys.ID.value, bot_id)
                    .execute()
                ).data[0]
            except IndexError:
    ...
    async def create_bot(self, deployment_type: enums.DeploymentTypes) -> dict:
        created_bot = (
            await self.table("bots")
            .insert({enums.BotKeys.USER_ID.value: (await self._get_user()).id})
            .execute()
        ).data[0]
        ...
        await self.table("bots").update(
            {
                enums.BotKeys.CURRENT_DEPLOYMENT_ID.value: created_deployment[
                    enums.BotDeploymentKeys.ID.value
                ],
            }
        ).eq(enums.BotDeploymentKeys.ID.value, bot_id).execute()
    ...
    async def _create_deployment(self, deployment_type, bot_id, version):
        ...
        return (
            await self.table("bot_deployments")
            .insert(
                {
    ...
    async def update_bot(self, bot_id, bot_update) -> dict:
        await self.table("bots").update(bot_update).eq(
    ...
    async def update_deployment(self, deployment_id, deployment_update: dict) -> dict:
        return (
            ...
            .eq(enums.BotDeploymentKeys.ID.value, deployment_id)
            .execute()
        ).data[0]

    def get_deployment_activity_update(
        self, last_activity: float, next_activity: float
    ) -> dict:
        return {
            enums.BotDeploymentKeys.ACTIVITIES.value: self._get_activities_content(
    ...
    def _get_activities_content(self, last_activity: float, next_activity: float):
        return {
            enums.BotDeploymentActivitiesKeys.LAST_ACTIVITY.value: self.get_formatted_time(
                last_activity
            ),
            enums.BotDeploymentActivitiesKeys.NEXT_ACTIVITY.value: self.get_formatted_time(
                next_activity
            ),
        }
    ...
    async def fetch_deployment_url(self, deployment_url_id) -> dict:
        try:
            return (
                await self.table("bot_deployment_urls")
                .select("*")
                .eq(enums.BotDeploymentURLKeys.ID.value, deployment_url_id)
                .execute()
            ).data[0]
        except IndexError:
        ...
        try:
            ...
            if sanitized_authors:
                query = query.or_(
                    ...
                    enums.ProductKeys.VISIBILITY.value,
                    "deleted",
                )
            else:
                query = query.eq(enums.ProductKeys.VISIBILITY.value, "public").is_(
                    enums.ProductKeys.AUTHOR_ID.value, "NULL"
                )
            return (await query.execute()).data
        except postgrest.exceptions.APIError as err:
            commons_logging.get_logger(__name__).error(
                f"Error when fetching products: {err}"
            )
            return []
    ...
    async def fetch_bot_products_subscription(self, bot_deployment_id: str) -> dict:
        return (
            ...
            .eq(enums.BotDeploymentKeys.ID.value, bot_deployment_id)
            .execute()
        ).data[0]["products_subscription"]

    async def fetch_trades(self, bot_id) -> list:
        # should be paginated to fetch all trades, will fetch the 1000 first ones only
        return (
            await self.table("bot_trades")
            .select("*")
            .eq(enums.TradeKeys.BOT_ID.value, bot_id)
            .execute()
        ).data
    ...
    async def upsert_trades(self, formatted_trades) -> list:
        return (
            ...
            .upsert(
                formatted_trades,
                on_conflict=f"{enums.TradeKeys.TRADE_ID.value},{enums.TradeKeys.TIME.value}",
            )
            .execute()
        ).data

    async def update_bot_orders(self, bot_id, formatted_orders) -> dict:
        bot_update = {enums.BotKeys.ORDERS.value: formatted_orders}
        return await self.update_bot(bot_id, bot_update)

    async def update_bot_positions(self, bot_id, formatted_positions) -> dict:
        bot_update = {enums.BotKeys.POSITIONS.value: formatted_positions}
        return await self.update_bot(bot_id, bot_update)
    ...
    async def fetch_bot_tentacles_data_based_config(
        ...
        try:
            ...
            raise errors.MissingBotConfigError(
                f"No bot config for bot with bot_id: '{bot_id}'"
            )
    ...
    async def _apply_options_based_authenticated_tentacles_config(
        self,
        profile_data: commons_profiles.ProfileData,
        auth_data: list[commons_profiles.ExchangeAuthData],
        bot_config: dict,
        authenticator,
        ...
        if tentacles_data := self.get_tentacles_data_options(bot_config):
            await commons_profiles.TentaclesProfileDataTranslator(
                profile_data, auth_data
            ).translate(tentacles_data, bot_config, authenticator, auth_key)

    def _apply_options_based_tentacles_config(
        self, profile_data: commons_profiles.ProfileData, bot_config: dict
    ):
    ...
    def get_tentacles_data_options(
        self, bot_config: dict
    ) -> list[commons_profile_data.TentaclesData]:
        return [
            commons_profile_data.TentaclesData.from_dict(td)
            for td in bot_config[enums.BotConfigKeys.OPTIONS.value].get(
                enums.BotConfigOptionsKeys.TENTACLES.value, []
            )
        ]
    ...
    async def fetch_bot_nested_config_profile_data_if_any(
        ...
        if bot_config_options:
            ...
            to_fetch_nested_config = bot_config_options.get(
        ...
        if master_profile_config and not to_fetch_nested_config:
            ...
            to_fetch_nested_config = master_profile_config.get(
                enums.NestedProductConfigKeys.NESTED_CONFIG_ROOT.value, {}
            )
        try:
            if to_fetch_nested_config:
                if nested_config_slug := to_fetch_nested_config.get(
                    enums.NestedProductConfigKeys.SLUG.value
                ):
                    nested_product_details = (
                        await self.table("products")
                        .select(
                        ...
                        .eq(enums.ProductKeys.SLUG.value, nested_config_slug)
                        .execute()
                    ).data[0]
                    ...
                    nested_product_config["product"] = {
                        enums.ProductKeys.ID.value: nested_product_details[
                            enums.ProductKeys.ID.value
                        ],
                        enums.ProductKeys.SLUG.value: nested_product_details[
                            enums.ProductKeys.SLUG.value
                        ],
                        enums.ProductKeys.ATTRIBUTES.value: nested_product_details[
                ...
                key = (
                    f"bot_config.options.{enums.NestedProductConfigKeys.NESTED_CONFIG_ROOT.value}"
                    if bot_config_options
                    else f"master product_config.config.{enums.NestedProductConfigKeys.NESTED_CONFIG_ROOT.value}"
                )
                raise TypeError(
                    f"Invalid product nested config: {key} is '{to_fetch_nested_config}'"
                )
            if nested_config_id:
                nested_product_config = (
                    await self.table("product_configs")
                    .select(
                        "id, "
                        "config, "
                        "version, "
                    ...
                    .eq(enums.ProfileConfigKeys.ID.value, nested_config_id)
                    .execute()
                ).data[0]
                return nested_product_config
    ...
    async def fetch_bot_profile_data(
        ...
        bot_config = (
            ...
            .eq(enums.BotConfigKeys.ID.value, bot_config_id)
            .execute()
        ).data[0]
        ...
        try:
            ...
            master_profile_config = master_product_config[
                enums.ProfileConfigKeys.CONFIG.value
            ]
            master_product_details = master_product_config.get("product", {})
            if not master_profile_config:
                raise TypeError(
                    f"master product_config.config is '{master_profile_config}'"
                )
            if nested_product_config := await self.fetch_bot_nested_config_profile_data_if_any(
                master_profile_config=master_profile_config,
                bot_config_options=bot_config_options,
            ):
                # nested strategy: use nested strategy product config and details
                nested_product_details = nested_product_config.get("product", {})
                nested_strategy_slug = nested_product_details.get(
                    enums.ProductKeys.SLUG.value, ""
                )
                nested_strategy_config_id = nested_product_config.get(
                    enums.ProfileConfigKeys.ID.value, ""
                )
                # the nested strategy profile config will be executed
                executed_product_config = nested_product_config
                executed_profile_config = nested_product_config[
                    enums.ProfileConfigKeys.CONFIG.value
                ]
                # for nested strategy, use bot_config created_at: this is the time this product was selected
                executed_config_created_at = self.get_parsed_time(
                    bot_config[enums.BotConfigKeys.CREATED_AT.value]
                )
                ...
                executed_config_created_at = self.get_parsed_time(
                    bot_details[enums.BotKeys.CREATED_AT.value]
                )
            ...
            profile_data = commons_profiles.ProfileData.from_dict(
                executed_profile_config
            )
        except (TypeError, KeyError) as err:
            raise errors.InvalidBotConfigError(
                f"Missing bot product config: {err} ({err.__class__.__name__})"
            ) from err
        ...
        executed_product_details = (
            executed_product_details_import.ExecutedProductDetails(
                executed_product_details.get(enums.ProductKeys.ID.value, ""),
                executed_config_created_at.timestamp(),
            )
        )
        profile_data.profile_details.nested_strategy_config_id = (
            nested_strategy_config_id
        )
        profile_data.profile_details.user_id = bot_details[enums.BotKeys.USER_ID.value]
        profile_data.profile_details.version = executed_product_config[
            enums.ProfileConfigKeys.VERSION.value
        ]
        ...
        profile_data.trader_simulator.enabled = bot_config.get(
            enums.BotConfigKeys.IS_SIMULATED.value, False
        )
        ...
        master_product_exchanges_configs = master_product_config[
            enums.ProfileConfigKeys.CONFIG.value
        ].get("exchanges")
        profile_data.exchanges, exchange_tentacles_data = (
            await self._fetch_full_exchange_configs(
        ...
        if bot_config_options:
            profile_data.options = commons_profiles.OptionsData.from_dict(
                bot_config_options
            )
        ...
        if portfolio:
            ...
            if trading_api.is_usd_like_coin(profile_data.trading.reference_market):
                ...
                usd_like_asset = usd_like_per_exchange.get(
                    exchange, commons_constants.USD_LIKE_COINS[0]
                )
            try:
    ...
    async def _fetch_full_exchange_configs(
        ...
        if incomplete_exchange_config_by_id:
            fetched_exchanges = await self.fetch_exchanges(
                list(incomplete_exchange_config_by_id)
            )
            exchanges_configs += [
                {
                    **incomplete_exchange_config_by_id[
                        exchange[enums.ExchangeKeys.ID.value]
                    ],
                    **{
                        enums.ExchangeKeys.INTERNAL_NAME.value: exchange[
                            enums.ExchangeKeys.INTERNAL_NAME.value
                        ],
                        enums.ExchangeKeys.AVAILABILITY.value: exchange[
                            enums.ExchangeKeys.AVAILABILITY.value
                        ],
                        enums.ExchangeKeys.URL.value: exchange[
                            enums.ExchangeKeys.URL.value
                        ],
                    },
                }
        ...
        if incomplete_exchange_config_by_credentials_id:
            ...
            if len(incomplete_exchange_config_by_credentials_id) != len(
                exchanges_by_credential_ids
            ):
                missing = [
                    cred
                    for cred in incomplete_exchange_config_by_credentials_id
            ...
            exchanges_configs += [
                {
                    ...
                    **{
                        enums.ExchangeKeys.EXCHANGE_ID.value: exchange[
                            enums.ExchangeKeys.ID.value
                        ],
                        enums.ExchangeKeys.INTERNAL_NAME.value: exchange[
                            enums.ExchangeKeys.INTERNAL_NAME.value
                        ],
        ...
        if exchanges_configs:
            ...
            exchange_type = formatters.get_exchange_type_from_availability(
                exchanges_configs[0].get(enums.ExchangeKeys.AVAILABILITY.value)
            )
        ...
        for exchange_data in exchanges_configs:
            ...
            exchange_data[enums.ExchangeKeys.INTERNAL_NAME.value] = (
                formatters.to_bot_exchange_internal_name(
                    exchange_data[enums.ExchangeKeys.INTERNAL_NAME.value]
                )
            )
    ...
    async def fetch_exchanges(
        self, exchange_ids: list, internal_names: typing.Optional[list] = None
    ) -> list:
        ...
        if internal_names:
            return (
                await select.in_(
                    enums.ExchangeKeys.INTERNAL_NAME.value, internal_names
                ).execute()
            ).data
        return (
            await select.in_(enums.ExchangeKeys.ID.value, exchange_ids).execute()
        ).data

    async def fetch_exchanges_by_credential_ids(
        self, exchange_credential_ids: list
    ) -> dict:
        exchanges = (
            await self.table("exchange_credentials")
            .select(
                "id,"
            ...
            .in_("id", exchange_credential_ids)
            .execute()
        ).data
        return {exchange["id"]: exchange["exchange"] for exchange in exchanges}

    async def fetch_product_config(
        self, product_id: str, product_slug: str = None
    ) -> commons_profiles.ProfileData:
        ...
        try:
            ...
            query = (
                query.eq(enums.ProductKeys.SLUG.value, product_slug)
                if product_slug
                else query.eq(enums.ProductKeys.ID.value, product_id)
            )
            product = (await query.execute()).data[0]
        except IndexError:
            raise errors.MissingProductConfigError(
                f"Missing product with id '{product_id}'"
            )
        profile_data = commons_profiles.ProfileData.from_dict(
        ...
        profile_data.profile_details.version = product["product_config"][
            enums.ProfileConfigKeys.VERSION.value
        ]
        return profile_data
    ...
    async def fetch_configs(self, bot_id: str) -> list:
        ...
        return (
            await self.table("bot_configs")
            .select("*")
            .eq(enums.BotConfigKeys.BOT_ID.value, bot_id)
            .execute()
        ).data

    async def fetch_portfolios(self, bot_id: str) -> list:
        return (
            await self.table("bot_portfolios")
            .select("*")
            .eq(enums.PortfolioKeys.BOT_ID.value, bot_id)
            .execute()
        ).data
    ...
    async def fetch_portfolio_from_id(self, portfolio_id: str) -> list:
        return (
            await self.table("bot_portfolios")
            .select("*")
            .eq(enums.PortfolioKeys.ID.value, portfolio_id)
            .execute()
        ).data
    ...
    async def update_portfolio(self, portfolio_update: dict) -> list:
        ...
        return (
            ...
            .eq(
                enums.PortfolioKeys.ID.value,
                portfolio_update[enums.PortfolioKeys.ID.value],
            )
            .execute()
        ).data
    ...
    async def switch_portfolio(self, new_portfolio) -> dict:
        ...
        inserted_portfolio = (
            await self.table("bot_portfolios").insert(new_portfolio).execute()
        ).data[0]
        await self.update_bot(
            bot_id,
            {
                enums.BotKeys.CURRENT_PORTFOLIO_ID.value: inserted_portfolio[
                    enums.PortfolioKeys.ID.value
                ]
            },
        )
    ...
    async def fetch_portfolio_history(self, portfolio_id) -> list:
        return (
            await self.table("bot_portfolio_histories")
            .select("*")
            .eq(enums.PortfolioHistoryKeys.PORTFOLIO_ID.value, portfolio_id)
            .execute()
        ).data
    ...
    async def fetch_gpt_signal(
        ...
        signals = (
            await (await self.get_production_anon_client())
            .table("temp_chatgpt_signals")
            .select("signal")
            .match(
                {
    ...
    async def _paginated_fetch_historical_data(
        ...
        def request_factory(
            table: postgrest.AsyncRequestBuilder, select_count, last_fetched_row
        ):
        ...
        return await self.cursor_paginated_fetch(client, table_name, request_factory)
    ...
    async def paginated_fetch(
        ...
        request_factory: typing.Callable[
            [postgrest.AsyncRequestBuilder, postgrest.types.CountMethod],
        ...
        while request_count < self.MAX_PAGINATED_REQUESTS_COUNT:
            ...
            total_elements_count = (
                total_elements_count or result.count
            )  # don't change total count within iteration
            total_elements += fetched_elements
            if (
                len(fetched_elements) == 0  # nothing to fetch
                or len(fetched_elements)
                < max_size_per_fetch  # fetched the last elements
                or len(fetched_elements) == total_elements_count  # finished fetching
            ):
                # fetched everything
                break
    ...
    def _format_gpt_signals(self, signals: list):
        return {
            self.get_parsed_time(signal["timestamp"]).timestamp(): signal["signal"][
                "content"
            ]
    ...
    def get_subscribed_channel_tables(self) -> set:
        return set(channel.table_name for channel in self.realtime.channels)
            ...
            try:
                ...
                try:
                    ...
                    if "." in str_time and "+" in str_time:
                        without_ms_time = (
                            str_time[0 : str_time.rindex(".")]
                            + str_time[str_time.rindex("+") :]
                        )
...
def jwt_expired_auth_raiser():
    try:
        ...
        if _is_jwt_expired_error(err):
            raise errors.JWTExpiredError(
                f"Please re-login to your OctoBot account: {err}"
            ) from err
        raise
