# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\community\supabase_backend\supabase_client.py
...
class AuthenticatedAsyncSupabaseClient(supabase.AClient):
    async def init_other_postgrest_client(
        ...
        if supabase_key != self.supabase_key:
            auth_headers = self._get_auth_headers(
                authorization=self._create_auth_header(supabase_key)
            )
    ...
    def _listen_to_auth_events(
        self,
        event: gotrue.types.AuthChangeEvent,
        session: typing.Union[gotrue.types.Session, None],
    ):
