...
class CommunityManager:
    ...
    def __init__(self, octobot_api):
        ...
        self.enabled = (
            constants.IS_CLOUD_ENV or self.edited_config.get_metrics_enabled()
        )
            ...
            try:
                ...
                while self.keep_running:
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Exception when handling community data : {e}"
                        )
            except asyncio.CancelledError:
                pass
            except Exception as e:
                self.logger.debug(
                    f"Exception when handling community registration: {e}"
                )
    ...
    def _blocking_get_id_and_register(self):
        try:
            ...
            if resp.status_code != 200:
                self.logger.debug(
                    f"Impossible to get bot id: status code: {resp.status_code}, text: {text}"
                )
            else:
    ...
    async def register_session(self, retry_on_error=True):
        ...
        await self._post_community_data(
            common_constants.METRICS_ROUTE_REGISTER, self.current_config, retry_on_error
        )
    ...
    async def _update_session(self, retry_on_error=True):
        ...
        await self._post_community_data(
            common_constants.METRICS_ROUTE_UPTIME, self.current_config, retry_on_error
        )
    ...
    def _get_bot_community(self):
        return {
            ...
            community_fields.CommunityFields.CURRENT_SESSION.value: {
                community_fields.CommunityFields.STARTED_AT.value: int(
                    self.octobot_api.get_start_time()
                ),
                community_fields.CommunityFields.UP_TIME.value: int(
                    time.time() - self.octobot_api.get_start_time()
                ),
                ...
                community_fields.CommunityFields.EXCHANGES.value: list(
                    trading_api.get_exchange_names()
                ),
        ...
        for exchange_manager in self._get_exchange_managers():
            if trading_api.is_exchange_trading(exchange_manager):
                ...
                total_origin_values += float(
                    trading_api.get_origin_portfolio_value(exchange_manager)
                )

        return (
            (total_profitability * 100 / total_origin_values)
            if total_origin_values > 0
            else 0
        )
    ...
    async def _init_bot_id(self):
        try:
            async with self.session.get(
                ...
                if resp.status != 200:
                    self.logger.debug(
                        f"Impossible to get bot id: status code: {resp.status}, text: {text}"
                    )
                else:
                    self.bot_id = json.loads(text)
                    self._save_bot_id()
    ...
    def _save_bot_id(self):
        if (
            ...
            self.edited_config.config[common_constants.CONFIG_METRICS] = {
                common_constants.CONFIG_ENABLED_OPTION: True
            }
        self.edited_config.config[common_constants.CONFIG_METRICS][
            common_constants.CONFIG_METRICS_BOT_ID
        ] = self.bot_id
    ...
    async def _handle_post_error(self, resp, retry_on_error):
        if resp.status != 200:
            if resp.status == 404:
                ...
                self.logger.debug(
                    f"Impossible to send community data : "
