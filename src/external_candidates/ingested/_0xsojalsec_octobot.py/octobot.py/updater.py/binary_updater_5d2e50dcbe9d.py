# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\updater\binary_updater.py
...
class BinaryUpdater(updater_class.Updater):
    ...
    def _get_latest_release_url(self, use_api_url):
        base = (
            f"{commons_constants.GITHUB_API_CONTENT_URL}/repos"
            if use_api_url
            else commons_constants.GITHUB_BASE_URL
        )
    ...
    def _parse_latest_version(self, latest_release_data):
        try:
            if latest_release_data.get("draft", False) or latest_release_data.get(
                "prerelease", False
            ):
                return None
            else:
    ...
    def _get_asset_from_name(self, release_data, expected_asset_name):
        try:
            ...
            self.logger.debug(
                f"Error when searching for {expected_asset_name} in latest release data : {e}"
            )
        return None
    ...
    def _create_release_asset_name(self, platform):
        if platform is commons_enums.PlatformsName.WINDOWS:
            if os_util.is_machine_64bit():
                return (
                    f"{constants.PROJECT_NAME}{self.BINARY_DELIVERY_SEPARATOR}"
    ...
    async def _download_binary(self):
        release_asset_name, matching_asset_binary = (
            await self._get_asset_from_release_data()
        )
        ...
        if matching_asset_binary is None:
            self.logger.error(
                f"Error when downloading latest version binary : Release not found on server"
            )
            return None
