# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\backtesting\octobot_backtesting.py
...
class OctoBotBacktesting:
    ...
    async def initialize_and_run(self):
        ...
        self.logger.info(
            f"Starting on {self.backtesting_files} with {symbols_by_exchange}"
        )
        ...
        start_date = (
            timestamp_util.convert_timestamp_to_datetime(min_timestamp)
            if min_timestamp
            else None
        )
        end_date = (
            timestamp_util.convert_timestamp_to_datetime(max_timestamp)
            if max_timestamp
            else None
        )
        self.logger.info(
            f"Configured backtesting from the {start_date} to the {end_date}"
        )
        await self._init_exchanges()
        self._ensure_limits()
        await self._create_evaluators()
        await self._fetch_backtesting_extra_data_if_any(min_timestamp, max_timestamp)
    ...
    async def stop(self, memory_check=False, should_raise=False):
        ...
        self.logger.debug(
            f"Stopping for {self.backtesting_files} with {symbols_by_exchange}"
        )
        exchange_managers = []
        try:
            if self.backtesting is None:
                self.logger.warning(
                    "No backtesting to stop, there was probably an issue when starting the backtesting"
                )
            else:
                ...
                if exchange_managers and self.enable_storage and self._has_started:
                    try:
                        for exchange_manager in exchange_managers:
                            await trading_api.store_history_in_run_storage(
                                exchange_manager
                            )
                    except Exception as e:
                        self.logger.exception(
                            e, True, f"Error when saving exchange historical data: {e}"
                        )
                    try:
                        ...
                        self.logger.exception(
                            e, True, f"Error when saving run metadata: {e}"
                        )
            ...
            await commons_databases.CacheManager().close_cache(
            ...
            if memory_check:
                ...
                asyncio.get_event_loop().call_soon(
                    self.memory_leak_checkup, to_reference_check
                )
            self.backtesting = None
    ...
    def memory_leak_checkup(self, to_check_elements):
        gc.collect()
        self.logger.debug(
            f"Memory leak checking {[e.__class__.__name__ for e in to_check_elements]}"
        )
        memory_leak_errors = []
        for i in range(len(to_check_elements)):
            if sys.getrefcount(to_check_elements[i]) > 2:
                ...
                memory_leak_errors.append(
                    f" Too many remaining references on the {element.__class__.__name__} element "
    ...
    def check_remaining_objects(self):
        ...
        to_watch_objects = (
            exchange_data.ExchangeSymbolData,
            exchanges.ExchangeManager,
            exchanges.ExchangeSimulator,
            exchange_data.OHLCVUpdaterSimulator,
        )
        ...
        for obj in gc.get_objects():
            if isinstance(obj, to_watch_objects):
                if (
                    isinstance(obj, exchanges.ExchangeManager)
                    and not obj.is_initialized
                ):
    ...
    async def _store_metadata(self, exchange_managers):
        run_db = commons_databases.RunDatabasesProvider.instance().get_run_db(
            self.bot_id
        )
        await run_db.flush()
        ...
        metadata = await storage.store_backtesting_run_metadata(
            ...
            self.start_time,
            user_inputs,
            commons_databases.RunDatabasesProvider.instance().get_run_databases_identifier(
                self.bot_id
            ),
            self.name,
        )
    ...
    async def _init_evaluators(self):
        ...
        if (
            (not self.backtesting_config[commons_constants.CONFIG_TIME_FRAME])
            and evaluator_constants.CONFIG_FORCED_TIME_FRAME in self.backtesting_config
        ):
            self.backtesting_config[commons_constants.CONFIG_TIME_FRAME] = (
                self.backtesting_config[evaluator_constants.CONFIG_FORCED_TIME_FRAME]
            )
        await evaluator_api.create_evaluator_channels(
            self.matrix_id, is_backtesting=True
        )

    async def _init_service_feeds(self):
        service_feed_factory = service_api.create_service_feed_factory(
            self.backtesting_config, asyncio.get_event_loop(), self.bot_id
        )
        self.service_feeds = [
            service_feed_factory.create_service_feed(feed)
    ...
    def _ensure_limits(self):
        for exchange_id in self.exchange_manager_ids:
            exchange_configuration = (
                trading_api.get_exchange_configuration_from_exchange_id(exchange_id)
            )
            ...
            limits.ensure_backtesting_limits(
                self.symbols_to_create_exchange_classes.keys(),
                symbols,
                time_frames,
                start_time,
                end_time,
    ...
    async def _create_service_feeds(self):
        for feed in self.service_feeds:
            if not await service_api.start_service_feed(feed, False, {}):
                self.logger.error(
                    f"Failed to start {feed.get_name()}. Evaluators requiring this service feed "
    ...
    async def _init_backtesting(self):
        ...
        self.backtesting = await backtesting_api.initialize_backtesting(
            ...
            importers_by_data_file=(
                self.backtesting_data.importers_by_data_file
                if self.backtesting_data
                else None
            ),
        ...
        if self.run_on_all_available_time_frames:
            self.backtesting_config[evaluator_constants.CONFIG_FORCED_TIME_FRAME] = [
                tf for tf in self.backtesting.importers[0].time_frames
            ]
    ...
    async def _fetch_backtesting_extra_data_if_any(
        ...
        for evaluator in list_util.flatten_list(self.evaluators):
            if evaluator and evaluator.get_name() not in handled_classes:
                if (
                    evaluator.get_signals_history_type()
                    == commons_enums.SignalHistoryTypes.GPT
                ):
                    coros.append(
                        self._fetch_gpt_history(evaluator, min_timestamp, max_timestamp)
                    )
    ...
    async def _fetch_gpt_history(
        self, evaluator, min_timestamp: float, max_timestamp: float
    ):
        # prevent circular import
        import tentacles.Services.Services_bases.gpt_service as gpt_service

        service = await service_api.get_service(
            gpt_service.GPTService, True, self.services_config
        )
        ...
        for exchange_id in self.exchange_manager_ids:
            ...
            await service.fetch_gpt_history(
                ...
                [
                    str(symbol)
                    for symbol in self.symbols_to_create_exchange_classes.get(
                        exchange_name, []
                    )
                ],
    ...
    async def _init_exchanges(self):
        for exchange_class_string in self.symbols_to_create_exchange_classes.keys():
            is_future = (
                self.exchange_type_by_exchange[exchange_class_string]
                == commons_constants.CONFIG_EXCHANGE_FUTURE
            )
            exchange_builder = (
                trading_api.create_exchange_builder(
                    self.backtesting_config, exchange_class_string
                )
                ...
                .use_cached_markets(
                    self.backtesting_data.use_cached_markets
                    if self.backtesting_data
                    else False
                )
...
def _get_remaining_object_error(obj, expected, actual):
    ...
    for i in range(len(actual[1])):
        ...
        error += (
            f"{sys.getrefcount(actual[1][i])} references on {actual[1][i]} {debug_info}"
        )
        return error
