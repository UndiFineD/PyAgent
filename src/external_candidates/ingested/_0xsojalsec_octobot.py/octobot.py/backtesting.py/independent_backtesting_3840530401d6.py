# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\backtesting\independent_backtesting.py
...
class IndependentBacktesting:
    def __init__(
        ...
        self.previous_handlers_log_level = (
            commons_logging.get_logger_level_per_handler()
        )
        self.enforce_total_databases_max_size_after_run = (
            enforce_total_databases_max_size_after_run
        )
        self.backtesting_data = backtesting_data
        self.required_extra_timeframes = config.get(
            common_constants.CONFIG_REQUIRED_EXTRA_TIMEFRAMES, []
        )
    ...
    async def join_backtesting_updater(self, timeout=None):
        if self.octobot_backtesting.backtesting is not None:
            await asyncio.wait_for(
                self.octobot_backtesting.backtesting.time_updater.finished_event.wait(),
                timeout,
            )
        ...
        try:
            if not self.stopped:
                await self.octobot_backtesting.stop(
                    memory_check=memory_check, should_raise=should_raise
                )
        finally:
            self.stopped = True
    ...
    def _post_backtesting_start(self):
        ...
        self.post_backtesting_task = asyncio.create_task(
            self._register_post_backtesting_end_callback()
        )
        ...
        if self.enforce_total_databases_max_size_after_run:
            try:
                ...
                self.logger.exception(
                    e, True, f"Error when enforcing max run databases size: {e}"
                )
    ...
    async def _register_available_data(self):
        if not self.symbols_to_create_exchange_classes:
            for data_file in self.backtesting_files:
                ...
                description = await backtesting_data.get_file_description(
                    data_file_path
                )
                if description is None:
                    raise RuntimeError(
                        f"Impossible to start backtesting: missing or invalid data file: {data_file}"
                    )
                exchange_name = description[
                    backtesting_enums.DataFormatKeys.EXCHANGE.value
                ]
                ...
                for symbol in description[
                    backtesting_enums.DataFormatKeys.SYMBOLS.value
                ]:
                    self.symbols_to_create_exchange_classes[exchange_name].append(
                        symbol_util.parse_symbol(symbol)
                    )

    def _init_default_config_values(self):
        self.risk = copy.deepcopy(
            self.octobot_origin_config[common_constants.CONFIG_TRADING][
                common_constants.CONFIG_TRADER_RISK
            ]
        )
        self.starting_portfolio = copy.deepcopy(
            self.octobot_origin_config[common_constants.CONFIG_SIMULATOR][
                common_constants.CONFIG_STARTING_PORTFOLIO
            ]
        )
        self.fees_config = copy.deepcopy(
            self.octobot_origin_config[common_constants.CONFIG_SIMULATOR][
                common_constants.CONFIG_SIMULATOR_FEES
            ]
        )
        if evaluator_constants.CONFIG_FORCED_TIME_FRAME in self.octobot_origin_config:
            self.forced_time_frames = copy.deepcopy(
                self.octobot_origin_config[evaluator_constants.CONFIG_FORCED_TIME_FRAME]
            )
        self.optimizer_id = self.octobot_origin_config.get(
            common_constants.CONFIG_OPTIMIZER_ID
        )
        self.backtesting_id = self.octobot_origin_config.get(
            common_constants.CONFIG_BACKTESTING_ID
        )
        self.backtesting_config = {
    ...
    async def get_dict_formatted_report(self):
        ...
        try:
            trading_mode = trading_api.get_activated_trading_mode(
                self.tentacles_setup_config
            ).get_name()
    ...
    def _get_exchanges_report(self, reference_market, trading_mode):
        ...
        for exchange_id in self.octobot_backtesting.exchange_manager_ids:
            ...
            min_timeframe = time_frame_manager.find_min_time_frame(
                trading_api.get_trading_timeframes(exchange_manager)
            )
            ...
            for symbol in self.symbols_to_create_exchange_classes[exchange_name]:
                try:
                    market_delta = self.get_market_delta(
                        symbol, exchange_manager, min_timeframe
                    )
                    report[SYMBOL_REPORT].append(
                        {symbol.symbol_str: market_delta * 100}
                    )
                    report[CHART_IDENTIFIERS].append(
                        {
                            ...
                            "time_frames": list(
                                reversed(
                                    [
                                        tf.value
                                        for tf in trading_api.get_watched_timeframes(
                                            exchange_manager
                                        )
                                    ]
                                )
            ...
            market_average_profitabilities[exchange_name] = float(
                market_average_profitability
            )
            starting_portfolios[exchange_name] = trading_api.get_origin_portfolio(
                exchange_manager, as_decimal=False
            )
    ...
    def log_report(self):
        ...
        for exchange_id in self.octobot_backtesting.exchange_manager_ids:
            ...
            self.logger.info(
                f" ========= Prices evolution on {exchange_name} ========="
            )
            min_timeframe = time_frame_manager.find_min_time_frame(
                trading_api.get_trading_timeframes(exchange_manager)
            )
    ...
    def _log_trades_history(self, exchange_manager, exchange_name):
        trades_history_string = "\n".join(
            [
                pretty_printer.trade_pretty_printer(exchange_name, trade)
    ...
    def _log_global_report(self, exchange_manager):
        ...
        starting_portfolio = trading_api.get_origin_portfolio(
            exchange_manager, as_decimal=False
        )
    ...
    def _adapt_config(self):
        ...
        for exchange_details in self.backtesting_config[
            common_constants.CONFIG_EXCHANGES
        ].values():
        ...
        self.backtesting_config[common_constants.CONFIG_TRADING][
            common_constants.CONFIG_TRADER_REFERENCE_MARKET
        ] = self._find_reference_market_and_update_contract_type()
        ...
        self.backtesting_config[common_constants.CONFIG_OPTIMIZER_ID] = (
            self.optimizer_id
        )
        self.backtesting_config[common_constants.CONFIG_BACKTESTING_ID] = (
            self.backtesting_id
        )
        ...
        self.backtesting_config[common_constants.CONFIG_REQUIRED_EXTRA_TIMEFRAMES] = (
            self.required_extra_timeframes
        )
        self._add_config_default_backtesting_values()

    def _init_exchange_type(self):
        forced_exchange_type = self.octobot_origin_config.get(
            common_constants.CONFIG_EXCHANGE_TYPE, common_constants.USE_CURRENT_PROFILE
        )
        try:
            for exchange_name in self.symbols_to_create_exchange_classes:
                if forced_exchange_type == common_constants.USE_CURRENT_PROFILE:
                    ...
                    self.octobot_backtesting.exchange_type_by_exchange[
                        exchange_name
                    ] = (
                        self.octobot_origin_config[common_constants.CONFIG_EXCHANGES]
                        .get(exchange_name, {})
                        .get(
                            common_constants.CONFIG_EXCHANGE_TYPE,
                            common_constants.DEFAULT_EXCHANGE_TYPE,
                        )
                    )
                else:
                    self.octobot_backtesting.exchange_type_by_exchange[
                        exchange_name
                    ] = forced_exchange_type
    ...
    async def _generate_backtesting_id_if_missing(self):
        if self.backtesting_config[common_constants.CONFIG_BACKTESTING_ID] is None:
            ...
            run_dbs_identifier.backtesting_id = (
                await run_dbs_identifier.generate_new_backtesting_id()
            )
    ...
    def _find_reference_market_and_update_contract_type(self):
        ...
        forced_contract_type = self.octobot_origin_config.get(
            common_constants.CONFIG_CONTRACT_TYPE, common_constants.USE_CURRENT_PROFILE
        )
        for symbols in self.symbols_to_create_exchange_classes.values():
            symbol = symbols[0]
            if (
                next(iter(self.octobot_backtesting.exchange_type_by_exchange.values()))
                ...
                if forced_contract_type == common_constants.USE_CURRENT_PROFILE:
                    if symbol.is_inverse():
                        if not all([symbol.is_inverse() for symbol in symbols]):
                            self.logger.error(
                                f"Mixed inverse and linear contracts backtesting are not supported yet"
                            )
                    ...
                    self.octobot_backtesting.futures_contract_type = (
                        forced_contract_type
                    )
                    return (
                        symbol.base
                        if forced_contract_type
                        is trading_enums.FutureContractType.INVERSE_PERPETUAL
                        else symbol.quote
                    )
            ...
            for symbol in symbols:
                ...
                if (
                    ref_market_candidate != quote
                    and ref_market_candidates[ref_market_candidate]
                    < ref_market_candidates[quote]
                ):
                    ref_market_candidate = quote
    ...
    def _add_config_default_backtesting_values(self):
        ...
        self.backtesting_config[backtesting_constants.CONFIG_BACKTESTING][
            common_constants.CONFIG_ENABLED_OPTION
        ] = True
        self.backtesting_config[common_constants.CONFIG_TRADER][
            common_constants.CONFIG_ENABLED_OPTION
        ] = False
        self.backtesting_config[common_constants.CONFIG_SIMULATOR][
            common_constants.CONFIG_ENABLED_OPTION
    ...
    def _add_crypto_currencies_config(self):
        for symbols in self.symbols_to_create_exchange_classes.values():
            for symbol in symbols:
                ...
                if (
                    symbol_id
                    not in self.backtesting_config[
                        common_constants.CONFIG_CRYPTO_CURRENCIES
                    ]
                ):
                    self.backtesting_config[common_constants.CONFIG_CRYPTO_CURRENCIES][
                        symbol_id
