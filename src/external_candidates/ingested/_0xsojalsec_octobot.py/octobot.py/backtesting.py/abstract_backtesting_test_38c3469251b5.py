...
DATA_FILES = {
    ...
    "2020ADA/USDT": path.join(
        TEST_DATA_FILES_FOLDER, "ExchangeHistoryDataCollector_1587769859.9278197.data"
    ),
    "2020ADA/BTC": path.join(
        TEST_DATA_FILES_FOLDER, "ExchangeHistoryDataCollector_1588110698.1060486.data"
    ),
}
...
class AbstractBacktestingTest:
    ...
    def initialize_with_strategy(
        self, strategy_evaluator_class, tentacles_setup_config, config
    ):
    ...
    async def _run_backtesting_with_current_config(self, data_file_to_use):
        raise NotImplementedError(
            "_run_backtesting_with_current_config not implemented"
        )

    async def _run_and_handle_results(self, data_file, expected_profitability):
        independent_backtesting = None
        try:
            independent_backtesting = await self._run_backtesting_with_current_config(
                data_file
            )
    ...
    async def run_test_slow_downtrend(
        ...
        if not skip_extended:
            await self._run_and_handle_results(
                EXTENDED_DATA_FILES["NEO/BTC"], profitability_3
            )
            await self._run_and_handle_results(
                EXTENDED_DATA_FILES["XVG/BTC"], profitability_4
            )

    async def run_test_sharp_downtrend(
        self, profitability_1, profitability_2, skip_extended=False
    ):
        await self._run_and_handle_results(DATA_FILES["VEN/BTC"], profitability_1)
        if not skip_extended:
            await self._run_and_handle_results(
                EXTENDED_DATA_FILES["XRP/BTC"], profitability_2
            )
    ...
    async def run_test_flat_markets(
        ...
        if not skip_extended:
            await self._run_and_handle_results(
                EXTENDED_DATA_FILES["ADA/BTC"], profitability_3
            )
            await self._run_and_handle_results(
                EXTENDED_DATA_FILES["WAX/BTC"], profitability_4
            )
    ...
    async def run_test_up_then_down(self, profitability_1, skip_extended=False):
        if not skip_extended:
            await self._run_and_handle_results(
                EXTENDED_DATA_FILES["ETC/BTC"], profitability_1
            )
    ...
    def _register_only_strategy(self, strategy_evaluator_class):
        try:
            ...
            self.logger.error(
                f"Backtesting tests requires OctoBot-Evaluator package installed {e}"
            )
