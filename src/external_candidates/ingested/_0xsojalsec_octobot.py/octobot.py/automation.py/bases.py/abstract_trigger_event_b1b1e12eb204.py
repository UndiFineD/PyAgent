# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\automation\bases\abstract_trigger_event.py
...
class AbstractTriggerEvent(automation_step.AutomationStep, abc.ABC):
    ...
    async def next_event(self):
        ...
        while not self.should_stop and not (
            self.trigger_only_once and self._last_trigger_time != 0
        ):
            new_event = await self._get_next_event()
            trigger_time = time.time()
            if not self.max_trigger_frequency or (
                trigger_time - self._last_trigger_time > self.max_trigger_frequency
            ):
                yield new_event
