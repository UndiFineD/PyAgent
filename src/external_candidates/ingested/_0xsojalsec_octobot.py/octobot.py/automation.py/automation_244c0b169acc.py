# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\automation\automation.py
...
class AutomationDetails:
    ...
    def __str__(self):
        return (
            f"Automation with {self.trigger_event.get_name()} trigger, "
...
class Automation(tentacles_management.AbstractTentacle):
    ...
    async def restart(self):
        ...
        await self.stop()
        self.automations_config = tentacles_manager_api.get_tentacle_config(
    ...
    def reset_config(self):
        tentacles_manager_api.update_tentacle_config(
            ...
            self.__class__,
            {self.AUTOMATIONS_COUNT: 0, self.AUTOMATIONS: {}},
        )
    ...
    def get_all_steps(self):
        all_events = {
            ...
            for event in self._all_possible_steps(
                abstract_trigger_event.AbstractTriggerEvent
            )
        }
        all_conditions = {
            condition.get_name(): condition
            for condition in self._all_possible_steps(
                abstract_condition.AbstractCondition
            )
        }
        all_actions = {
    ...
    def _get_default_steps(self):
        ...
        return (
            trigger_events_impl.PeriodicCheck.get_name(),
    ...
    def init_user_inputs(self, inputs: dict) -> None:
        ...
        automations_count = self.UI.user_input(
            self.AUTOMATIONS_COUNT,
            common_enums.UserInputTypes.INT,
            self.automations_config.get(self.AUTOMATIONS_COUNT, 0),
            inputs,
            min_val=0,
        ...
        if not automations_count:
            return
        automations = self.UI.user_input(
            self.AUTOMATIONS,
            common_enums.UserInputTypes.OBJECT,
            self.automations_config.get(self.AUTOMATIONS, {}),
            inputs,
            title="Automations",
        )
    ...
    def _apply_user_inputs(
        self, step_names, step_classes_by_name: dict, inputs, automation_id
    ):
        for step_name in step_names:
            try:
                self._apply_step_user_inputs(
                    step_name, step_classes_by_name[step_name], inputs, automation_id
                )
            except KeyError:
    ...
    def _create_automation_details(self):
        ...
        for automation_id, automation_config in self.automations_config.get(
            self.AUTOMATIONS, {}
        ).items():
            ...
            if not self._is_valid_automation_config(automation_config):
                continue
            event = self._create_step(
                automation_config, automation_config[self.TRIGGER_EVENT], all_events
            )
            conditions = [
            ...
            self.automation_details.append(
                AutomationDetails(event, conditions, actions)
            )
    ...
    async def _run_automation(self, automation_detail):
        ...
        async for _ in automation_detail.trigger_event.next_event():
            self.logger.debug(
                f"{automation_detail.trigger_event.get_name()} event triggered"
            )
    ...
    async def _check_conditions(self, automation_detail):
        for condition in automation_detail.conditions:
            if not await condition.evaluate():
                ...
                self.logger.debug(
                    f"{condition.get_name()} is not valid: skipping "
                    f"{automation_detail.trigger_event.get_name()} event"
                )
                return False
            self.logger.debug(
                f"All conditions are valid for "
