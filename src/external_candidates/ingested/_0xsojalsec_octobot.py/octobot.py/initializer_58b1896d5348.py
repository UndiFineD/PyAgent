# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\octobot\initializer.py
...
class Initializer:
    ...
    async def create(self, init_bot_storage):
        ...
        tentacles_config_path = self.octobot.get_startup_config(
            constants.CONFIG_KEY, dict_only=False
        ).get_tentacles_config_path()
        self.octobot.tentacles_setup_config = (
            tentacles_manager_api.get_tentacles_setup_config(tentacles_config_path)
        )
            ...
            try:
                ...
                await databases.init_bot_storage(
                    ...
                    databases_util.get_run_databases_identifier(
                        self.octobot.config, self.octobot.tentacles_setup_config
                    ),
                    True,
                )
                ...
                logging.get_logger(self.__class__.__name__).warning(
                    f"Error when initializing bot storage: {err}"
                )
