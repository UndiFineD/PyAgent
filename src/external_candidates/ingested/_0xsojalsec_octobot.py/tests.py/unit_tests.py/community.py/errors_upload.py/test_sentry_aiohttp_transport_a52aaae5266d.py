# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\community\errors_upload\test_sentry_aiohttp_transport.py
...
def _mocked_resp():
    return mock.Mock(status=200, headers={}, text=mock.AsyncMock(return_value="plop"))
...
async def test_upload_error(sentry_init):
    ...
    with mock.patch.object(transport._worker.session, "post", patched_post):
        ...
        # if post_mock is called, then the whole transport works as the aiohttp session post is called by sentry
        assert (
            len(post_mock.mock_calls) == 2
        )  # called twice to account for error and exception log
