# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\community\test_community_mqtt_feed.py
...
async def connected_community_feed(authenticator):
    ...
    try:
        ...
        with (
            ...
            mock.patch.object(
                gmqtt.Client, "connect", mock.AsyncMock()
            ) as _connect_mock,
        ):
            await feed.register_feed_callback(
                commons_enums.CommunityChannelTypes.SIGNAL, mock.AsyncMock()
            )
            _subscribe_mock.assert_called_once_with(
                (f"{commons_enums.CommunityChannelTypes.SIGNAL.value}/None",)
            )
            await feed.start(None)
            assert get_selected_bot_device_uuid_mock.call_count == 2
            _connect_mock.assert_called_once_with(
                FEED_URL, feed.mqtt_broker_port, version=feed.MQTT_VERSION
            )
            yield feed
    finally:
        if feed is not None:
...
async def test_start_and_connect(connected_community_feed):
    ...
    assert (
        connected_community_feed._mqtt_client._config["reconnect_retries"]
        == connected_community_feed.DISABLE_RECONNECT_VALUE
    )
    assert (
        connected_community_feed._mqtt_client.on_connect
        == connected_community_feed._on_connect
    )
    assert (
        connected_community_feed._mqtt_client.on_message
        == connected_community_feed._on_message
    )
    assert (
        connected_community_feed._mqtt_client.on_disconnect
        == connected_community_feed._on_disconnect
    )
    assert (
        connected_community_feed._mqtt_client.on_subscribe
        == connected_community_feed._on_subscribe
    )
...
async def test_stop(connected_community_feed):
    ...
    with mock.patch.object(
        connected_community_feed, "_stop_mqtt_client", mock.AsyncMock()
    ) as _stop_mqtt_client_mock:
        await connected_community_feed.stop()
        _stop_mqtt_client_mock.assert_called_once()

        _stop_mqtt_client_mock.reset_mock()
        connected_community_feed._reconnect_task = mock.Mock(
            done=mock.Mock(return_value=True), cancel=mock.Mock()
        )
        await connected_community_feed.stop()
        _stop_mqtt_client_mock.assert_called_once()
        ...
        _stop_mqtt_client_mock.reset_mock()
        connected_community_feed._reconnect_task = mock.Mock(
            done=mock.Mock(return_value=False), cancel=mock.Mock()
        )
        await connected_community_feed.stop()
        _stop_mqtt_client_mock.assert_called_once()
        connected_community_feed._reconnect_task.done.assert_called_once()
...
async def test_on_message(connected_community_feed):
    ...
    topic = "topic"
    connected_community_feed.feed_callbacks["topic"] = [
        mock.AsyncMock(),
        mock.AsyncMock(),
    ]
