...
async def authenticated_feed():
    ...
    try:
        ...
        def _community_feed_factory(self, *args):
            return origin_community_feed_factory(
                self, enums.CommunityFeedType.SupabaseFeed
            )
...
async def test_start_and_connect(authenticated_feed):
    ...
    with (
        ...
        mock.patch.object(
            authenticated_feed, "_subscribe_to_table", mock.AsyncMock()
...
async def test_register_feed_callback(authenticated_feed):
    ...
    target_table = authenticated_feed._get_table(
    ...
    with mock.patch.object(
        ...
        assert authenticated_feed.feed_callbacks == {
            target_table: {"identifier": ["plop_cb"]}
        }
...
async def test_message_callback(authenticated_feed):
    ...
    with mock.patch.object(
        ...
        await authenticated_feed.register_feed_callback(
            channel_type, callback_2, identifier=identifier
        )
        await authenticated_feed.register_feed_callback(
            channel_type, callback_3, identifier="other"
        )
