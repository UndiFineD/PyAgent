# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\community\test_community_ws_feed.py
...
DATA_DICT = {"data": "yes", "content": {"hi": 1, "a": ["@@@", 1]}}
...
def _mocked_signal():
    return commons_signals.Signal("identifier", {"key1": 11, "key2": "random value"})
    ...
    async for message in websocket:
        ...
        if parsed_message.get("command") == "subscribe":
            ...
            base_message = _build_message(
                _mocked_signal().to_dict() if value == "signal" else value
            )
        ...
        async for message in websocket:
            ...
            if parsed_message.get("command") == "subscribe":
                ...
                await websocket.send(
                    json.dumps({"message": _build_message(mock_consumer.call_count)})
                )
...
async def connected_community_feed(authenticator):
    ...
    try:
        ...
        with mock.patch.object(
            feed, "_fetch_stream_identifier", mock.AsyncMock(return_value=1)
        ) as _fetch_stream_identifier_mock:
            await feed.register_feed_callback(
                commons_enums.CommunityChannelTypes.SIGNAL, mock.AsyncMock()
            )
...
async def test_consume_base_message(community_echo_server, connected_community_feed):
    consume_mock = connected_community_feed.feed_callbacks[
        commons_enums.CommunityChannelTypes.SIGNAL
    ][None][0]
    consume_mock.assert_not_called()
    await connected_community_feed.send(
        "hiiiii", commons_enums.CommunityChannelTypes.SIGNAL, None
    )
    await _wait_for_receive()
    consume_mock.assert_called_once_with(
        {"v": "1.0.0", "t": "t", "i": 1, "s": "hiiiii"}
    )
    consume_mock.reset_mock()

    await connected_community_feed.send(
        json.dumps(DATA_DICT), commons_enums.CommunityChannelTypes.SIGNAL, None
    )
    await _wait_for_receive()
    consume_mock.assert_called_once_with(
        {"v": "1.0.0", "t": "t", "i": 1, "s": json.dumps(DATA_DICT)}
    )
...
async def test_consume_signal_message(community_echo_server, connected_community_feed):
    ...
    connected_community_feed.feed_callbacks = {
        commons_enums.CommunityChannelTypes.SIGNAL: {"strat1": [consume_mock]}
    }
    connected_community_feed._identifier_by_stream_id[1] = "strat1"
    consume_mock.assert_not_called()
    await connected_community_feed.send(
        "signal", commons_enums.CommunityChannelTypes.SIGNAL, "strat1"
    )
    await _wait_for_receive()
    consume_mock.assert_called_once_with(
        {"v": "1.0.0", "t": "t", "i": 1, "s": _mocked_signal().to_dict()}
    )


async def test_send_base_message(
    community_mocked_consumer_server, connected_community_feed
):
    ...
    consume_mock.assert_not_called()
    await connected_community_feed.send(
        "signal", commons_enums.CommunityChannelTypes.SIGNAL, None
    )
    await _wait_for_receive()
    community_mocked_consumer_server.assert_called_once_with(
    ...
    await connected_community_feed.send(
        DATA_DICT, commons_enums.CommunityChannelTypes.SIGNAL, None
    )
    await _wait_for_receive()
    ...
    data_str = json.dumps(
        {"topic": "t", "feed_id": 1, "version": "1.0.0", "value": sub_data_str}
    )
    assert community_mocked_consumer_server.mock_calls[1].args == (
        json.dumps(
            {"command": "message", "identifier": identifier_str, "data": data_str}
        ),
    )
...
async def test_send_signal_message(
    ...
    consume_mock.assert_not_called()
    await connected_community_feed.send(
        _mocked_signal().to_dict(), commons_enums.CommunityChannelTypes.SIGNAL, None
    )
    await _wait_for_receive()
...
async def test_reconnect(authenticator):
    ...
    try:
        ...
        await client.register_feed_callback(
            commons_enums.CommunityChannelTypes.SIGNAL, client_handler
        )
        ...
        await next(iter(server.websockets)).send(
            json.dumps({"message": _build_message("greetings")})
        )
        await _wait_for_receive()
        assert client.is_connected()
        # client_handler is called as the consume task did reconnect the client
        client_handler.assert_called_once_with(
            {"v": "1.0.0", "t": "t", "i": 1, "s": "greetings"}
        )
