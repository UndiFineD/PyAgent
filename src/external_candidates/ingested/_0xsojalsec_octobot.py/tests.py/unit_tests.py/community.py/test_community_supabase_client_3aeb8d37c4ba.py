# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\community\test_community_supabase_client.py
...
@pytest.mark.asyncio
async def test_fetch_bot_nested_config_profile_data_if_any_invalid_inputs(
    mock_supabase_client,
):
    mock_supabase_client.table = mock.Mock()
    assert (
        await mock_supabase_client.fetch_bot_nested_config_profile_data_if_any() is None
    )
    ...
    assert (
        await mock_supabase_client.fetch_bot_nested_config_profile_data_if_any(
            master_profile_config={
                enums.NestedProductConfigKeys.NESTED_CONFIG_ROOT.value: {}
            }
        )
        is None
    )
...
@pytest.mark.asyncio
async def test_fetch_bot_nested_config_profile_data_if_any_valid_inputs(
    mock_supabase_client,
):
            ...
            select=mock.Mock(
                return_value=mock.Mock(
                    eq=mock.Mock(
                        return_value=mock.Mock(
                            execute=(
                                _execute_products_mock
                                if table == "products"
                                else _execute_product_configs_mock
                            )
                        )
                    )
                )
...
async def test_fetch_bot_profile_data_invalid_inputs(mock_supabase_client):
    ...
    with mock.patch.object(
        mock_supabase_client,
        "fetch_bot_nested_config_profile_data_if_any",
        mock.AsyncMock(side_effect=KeyError),
