# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\unit_tests\community\test_authentication.py
...
def test_constructor():
    with mock.patch.object(
        community.CommunityAuthentication, "login", mock.Mock()
    ) as login_mock:
        community.IdentifiersProvider.use_production()
...
async def test_login(auth):
    ...
    with (
        mock.patch.object(
            community.CommunityAuthentication, "_reset_tokens", mock.Mock()
        ) as reset_mock,
        mock.patch.object(
            community.CommunityAuthentication, "_ensure_community_url", mock.Mock()
        ) as _ensure_community_url_mock,
        mock.patch.object(
            community.CommunityAuthentication, "_ensure_email", mock.Mock()
        ) as _ensure_email_mock,
        mock.patch.object(
            community.CommunityAuthentication, "_on_account_updated", mock.AsyncMock()
        ) as _on_account_updated_mock,
        mock.patch.object(
            community.CommunityAuthentication, "is_logged_in", mock.Mock()
        ) as is_logged_in_mock,
        mock.patch.object(
            community.CommunityAuthentication, "on_signed_in", mock.AsyncMock()
        ...
        auth.supabase_client.sign_in_with_otp_token.assert_awaited_once_with(
            "password_t"
        )
...
async def test_fetch_bot_profile_data_without_tentacles_options(auth):
    FETCHED_PROFILE_USD_LIKE = {
        ...
        "bot": {
            ...
            "created_at": "2024-08-14T22:13:22.1111+04:00",
        },
        "exchanges": [
            {"exchange_credential_id": "30ee7b12-3415-4ce4-b050-80d8bf4548be"}
        ],
        "is_simulated": True,
        "created_at": "2023-08-14T22:13:22.466399+04:00",
        "options": {"portfolio": [{"asset": "USD-like", "value": 1000}]},
        "product_config": {
            "config": {
                "backtesting_context": {
                    "exchanges": ["mexc"],
                    "start_time_delta": 15552000,
                    "starting_portfolio": {"USDT": 3000},
                },
                "crypto_currencies": [
                    {"name": "Bitcoin", "trading_pairs": ["BTC/USDT"]}
                ],
                "exchanges": [{"internal_name": "mexc"}],
                "options": {},
                "profile_details": {"name": "serverless"},
                "tentacles": [
                    {
                        "config": {
                            "buy_order_amount": "4%t",
                            "default_config": [None],
                            "enable_health_check": True,
                            "entry_limit_orders_price_percent": 0.6,
                            "exit_limit_orders_price_percent": 0.5,
                            "minutes_before_next_buy": 10080,
                            "required_strategies": ["123"],
                            "secondary_entry_orders_amount": "3%t",
                ...
                "trader": {"enabled": True},
                "trader_simulator": {},
                "trading": {"reference_market": "USD-like", "risk": 0.5},
            },
            "product": {
                "attributes": {
                    "coins": ["BTC", "USDT"],
                    "ease": "Easy",
                    "exchanges": ["mexc"],
                    "minimal_funds": [{"asset": "USD-like", "value": 50}],
                    "risk": "Moderate",
                    "subcategories": ["classic-dca", "popular"],
                    "trading": ["Spot"],
                },
                "slug": "bitcoin-vision",
                "id": "product_id_123",
    ...
    exchange_data = octobot_commons.profiles.profile_data.ExchangeData(
        internal_name="mexc"
    )
    with (
        mock.patch.object(
            ...
            "execute",
            mock.AsyncMock(return_value=mock.Mock(data=[FETCHED_PROFILE_USD_LIKE])),
        ) as execute_mock,
        mock.patch.object(
            auth.supabase_client,
            "_fetch_full_exchange_configs",
            mock.AsyncMock(return_value=([exchange_data], [])),
        ) as _fetch_full_exchange_configs_mock,
    ):
        parsed_data = octobot_commons.profiles.profile_data.ProfileData.from_dict(
            {
                ...
                "crypto_currencies": [
                    {"enabled": True, "name": "Bitcoin", "trading_pairs": ["BTC/USDT"]}
                ],
                "exchanges": [{"internal_name": "mexc"}],
                "future_exchange_data": {"default_leverage": None, "symbol_data": []},
                "options": {"values": {}},
                "profile_details": {
                    "bot_id": None,
                    "id": "bot_id",
                ...
                "trader_simulator": {
                    "enabled": True,
                    "maker_fees": 0.1,
                    "starting_portfolio": {"USDC": 1000},
                    "taker_fees": 0.1,
                },
                "trading": {
                    "minimal_funds": [
                        {"asset": "USD-like", "available": 50, "total": 50}
                    ],
                    "reference_market": "USDC",
                    "risk": 0.5,
                    "sub_portfolio": {"USDC": 1000},
        ...
        assert await auth.supabase_client.fetch_bot_profile_data(
            "bot_id", {"mexc": "USDC"}
...
async def test_fetch_bot_profile_data_with_tentacles_options(auth):
    FETCHED_PROFILE = {
        ...
        "bot": {
            "user_id": "3330dc3e-3cbe-476d-9bda-b30bc4941fb4",
            "created_at": "2024-08-14T22:13:22.1111+08:00",
        },
        "exchanges": [
            {"exchange_credential_id": "30ee7b12-3415-4ce4-b050-80d8bf4548be"}
        ],
        ...
        "product_config": {
            "config": {
                ...
                "trader": {"enabled": True},
                "trader_simulator": {},
                "trading": {"reference_market": "USDT", "risk": 0.5},
            },
            "product": {
                "attributes": {
                    "coins": ["BTC", "USDT"],
    ...
    with (
        mock.patch.object(
            postgrest.AsyncQueryRequestBuilder,
            "execute",
            mock.AsyncMock(return_value=mock.Mock(data=[FETCHED_PROFILE])),
        ) as execute_mock,
        mock.patch.object(
            auth.supabase_client,
            "_fetch_full_exchange_configs",
...
async def test_logout(auth):
    with (
        ...
        mock.patch.object(
            community.CommunityAuthentication, "remove_login_detail", mock.Mock()
        ) as remove_mock,
    ):
        await auth.logout()
        reset_mock.assert_called_once()
...
def test_ensure_community_url(auth):
    with mock.patch.object(
        auth, "can_authenticate", mock.Mock(return_value=False)
    ) as can_authenticate_mock:
        with pytest.raises(authentication.UnavailableError):
            auth._ensure_community_url()
        can_authenticate_mock.assert_called_once()
    with mock.patch.object(
        auth, "can_authenticate", mock.Mock(return_value=True)
    ) as can_authenticate_mock:
        auth._ensure_community_url()
        can_authenticate_mock.assert_called_once()
...
def test_remove_login_detail(auth):
    with (
        mock.patch.object(
            auth, "_reset_login_token", mock.Mock()
        ) as _reset_login_token_mock,
...
def test_reset_login_token(auth):
    with mock.patch.object(
        ...
        auth.configuration_storage.sync_storage._configuration.config = {
            constants.CONFIG_COMMUNITY: {"_storage_key": "plop"}
        }
...
def test_get_saved_bot_id(auth):
    ...
    auth.configuration_storage.sync_storage._configuration.config = {
        constants.CONFIG_COMMUNITY: {constants.CONFIG_COMMUNITY_BOT_ID: "bid"}
    }
...
def test_authenticated(auth):
    ...
    with mock.patch.object(
        auth, "ensure_token_validity", mock.Mock()
    ) as ensure_token_validity_mock:
        mock_func(auth)
...
def test_update_supports(auth):
    with mock.patch.object(
        community.CommunitySupports, "from_community_dict", mock.Mock()
    ) as from_community_dict_mock:
...
def test_init_account(auth):
    with (
        mock.patch.object(
            asyncio, "create_task", mock.Mock(return_value="task")
        ) as create_task_mock,
        mock.patch.object(
            auth, "_initialize_account", mock.Mock(return_value="coro")
