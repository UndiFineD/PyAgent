...
def check_order_limits(order, market_status):
    symbol_market_limits = market_status[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS.value
    ]
    limit_amount = symbol_market_limits[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_AMOUNT.value
    ]
    limit_cost = symbol_market_limits[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_COST.value
    ]
    limit_price = symbol_market_limits[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_PRICE.value
    ]

    min_quantity = limit_amount[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_AMOUNT_MIN.value
    ]
    max_quantity = limit_amount[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_AMOUNT_MAX.value
    ]
    min_cost = limit_cost[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_COST_MIN.value
    ]
    max_cost = limit_cost[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_COST_MAX.value
    ]
    min_price = limit_price[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_PRICE_MIN.value
    ]
    max_price = limit_price[
        trading_enum.ExchangeConstantsMarketStatusColumns.LIMITS_PRICE_MAX.value
    ]
    maximal_price_digits = market_status[
        trading_enum.ExchangeConstantsMarketStatusColumns.PRECISION.value
    ][trading_enum.ExchangeConstantsMarketStatusColumns.PRECISION_PRICE.value]
    maximal_volume_digits = market_status[
        trading_enum.ExchangeConstantsMarketStatusColumns.PRECISION.value
            ...
            for symbol in [market, order_symbol]:
                ...
                if not only_positivity:
                    if order.order_type in (
                        ...
                        # order is filled
                        assert initial_portfolio.get_currency_portfolio(
                            symbol
                        ...
                        if order_symbol == symbol:
                            ...
                            assert (
                                initial_portfolio.get_currency_portfolio(
                                    symbol
                                ).available
                                - orders_currency_amount
                                == portfolio.get_currency_portfolio(symbol).available
                            )
                        elif market == symbol:
                            assert (
