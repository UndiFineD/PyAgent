...
async def test_generate_and_save_strategy_optimizer_runs(optimizer_inputs):
    ...
    with (
        mock.patch.object(
            databases.TinyDBAdaptor, "create_identifier", mock.AsyncMock()
        ) as create_identifier_mock,
        mock.patch.object(
...
async def test_resume_unknown_mode(optimizer_inputs):
    ...
    with mock.patch.object(
        optimizer, "_get_total_nb_runs", mock.AsyncMock(return_value=5)
    ) as _get_total_nb_runs_mock:
        with pytest.raises(NotImplementedError):
            await optimizer.resume(optimizer_settings)
        _get_total_nb_runs_mock.assert_awaited_once_with(
...
async def test_resume_normal_mode(optimizer_inputs):
    ...
    with (
        ...
        _get_total_nb_runs_mock.assert_called_once_with(
            optimizer_settings.optimizer_ids
        )
