...
def _get_filter_setting(left_value, right_value, flat_value, operator):
    return {
        "user_input_left_operand": _get_filter_setting_content(
            "user_input_left_operand", left_value
        ),
        "user_input_right_operand": _get_filter_setting_content(
            "user_input_right_operand", right_value
        ),
        "text_right_operand": _get_filter_setting_content(
            "text_right_operand", flat_value
        ),
        "operator": _get_filter_setting_content(
            strategy_optimizer.OptimizerFilter.OPERATOR_KEY, operator.value
        ),
    }
...
def _get_mocked_run_ui(ui_name, tentacle, value):
    return {
        ...
        strategy_optimizer.StrategyDesignOptimizer.CONFIG_TENTACLE: [
            tentacle.get_name()
        ],
        strategy_optimizer.StrategyDesignOptimizer.CONFIG_VALUE: value,
    }
...
def _get_mocked_run(period_length, constrained_risk, time_frame):
    return (
        _get_mocked_run_ui(
            "period_length", Evaluator.RSIMomentumEvaluator, period_length
        ),
        _get_mocked_run_ui(
            "constrained_risk", Evaluator.RSIMomentumEvaluator, constrained_risk
        ),
        _get_mocked_run_ui(
            evaluators_constants.STRATEGIES_REQUIRED_TIME_FRAME,
            Strategies.SimpleStrategyEvaluator,
