# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\test_utils\data_bank.py
...
class DataBank(Initializable):
    ...
    def __init__(self, data_file=None):
        ...
        self.data_file = (
            data_file
            if data_file
            else "tests/static/AbstractExchangeHistoryCollector_1586017993.616272.data"
        )
    ...
    def get_last_candle_for_default_symbol(self, time_frame):
        return [
            val[0]
            for val in self.candles_managers_by_time_frame[time_frame]
            .get_symbol_prices(1)
            .values()
        ]
    ...
    def increment_data_for_default_symbol(self, time_frame):
        ...
        candle = self.origin_ohlcv_by_symbol[self.default_symbol][time_frame][
            current_index
        ]
    ...
    def rise_after_over_sold_mode(self):
        self.current_data = self._merge_candles(
            self.origin_ohlcv_by_symbol[self.default_symbol][TimeFrames.FOUR_HOURS][
                35:61
            ],
            self.origin_ohlcv_by_symbol[self.default_symbol][TimeFrames.FOUR_HOURS][
                0:49
    ...
    def dip_after_over_bought_mode(self):
        self.current_data = self.origin_ohlcv_by_symbol[self.default_symbol][
            TimeFrames.ONE_DAY
        ]
    ...
    def overall_flat_trend_mode(self):
        cheap_prices = [
            ...
            for candle in self.origin_ohlcv_by_symbol[self.default_symbol][
                TimeFrames.FIFTEEN_MINUTES
            ][6:72]
        ]
        self.current_data = self._merge_candles(
            cheap_prices,
            self.origin_ohlcv_by_symbol[self.default_symbol][TimeFrames.HEIGHT_HOURS][
                25:43
            ],
    ...
    def _set_candle_manager(
        self, time_frame, initial_candles_max_index, use_current_data=False
    ):
        self.candles_managers_by_time_frame[time_frame] = create_new_candles_manager(
            self.current_data[:initial_candles_max_index]
            if use_current_data
            else self.origin_ohlcv_by_symbol[self.default_symbol][time_frame][
                :initial_candles_max_index
            ]
        )
    ...
    def _merge_candles(*candles_list):
        ...
        timestamp_list = [
            candle[PriceIndexes.IND_PRICE_TIME.value] for candle in result_list
        ]
        for candle_list in candles_list[1:]:
            # ensure no timestamp is present in 2 candles
            while any(
                candle[PriceIndexes.IND_PRICE_TIME.value] in timestamp_list
                for candle in candle_list
            ):
                for candle in candle_list:
                    candle[PriceIndexes.IND_PRICE_TIME.value] = (
                        candle[PriceIndexes.IND_PRICE_TIME.value] + 1
                    )
