# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\test_utils\memory_check_util.py
...
async def run_independent_backtestings_with_memory_check(
    ...
    backtesting_count=3,
    use_multiple_asset_data_file=False,
):
    """
    ...
    try:
        ...
        for _ in range(backtesting_count):
            ...
            # init_logger()
            backtesting = await _run_backtesting(
...
async def _run_backtesting(
    ...
    data_file = (
        MULTI_ASSETS_DATA_FILE
        if use_multiple_asset_data_file
        else DATA_FILES["ETH/USDT"]
    )
