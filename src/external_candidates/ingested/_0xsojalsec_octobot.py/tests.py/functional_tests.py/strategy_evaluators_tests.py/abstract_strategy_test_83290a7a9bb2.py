# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\functional_tests\strategy_evaluators_tests\abstract_strategy_test.py
...
class AbstractStrategyTest(octobot_backtesting.AbstractBacktestingTest, abc.ABC):
    ...
    def _handle_results(self, independent_backtesting, profitability):
        exchange_manager_ids = (
            octobot_api.get_independent_backtesting_exchange_manager_ids(
        ...
        for exchange_manager in trading_api.get_exchange_managers_from_exchange_ids(
            ...
            _, run_profitability, _, market_average_profitability, _ = (
    ...
    async def _run_backtesting_with_current_config(self, data_file_to_use):
        ...
        await octobot_api.initialize_and_run_independent_backtesting(
    ...
    def _update_tentacles_config(self, strategy_evaluator_class, trading_mode_class):
        ...
        for tentacle_class_name in tentacles_activation[
            ...
            if (
                commons_constants.CONFIG_WILDCARD not in default_evaluators
                and tentacle_class_name in default_evaluators
            ):
