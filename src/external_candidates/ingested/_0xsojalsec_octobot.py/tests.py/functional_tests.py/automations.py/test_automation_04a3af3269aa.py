# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\functional_tests\automations\test_automation.py
...
async def test_automation_workflow(automation):
    ...
    with mock.patch.object(
        ...
        "get_tentacle_config",
        mock.Mock(return_value=test_config),
        ...
        assert isinstance(
            automation_detail.trigger_event, test_automations.TestTriggerEvent
        )
        assert len(automation_detail.conditions) == 1
        assert len(automation_detail.actions) == 1
        assert isinstance(
            automation_detail.conditions[0], test_automations.TestCondition
        )
