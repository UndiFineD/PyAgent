...
class AbstractTATest:
            ...
            with (
                ...
                for symbol in self.data_bank.data_importer.symbols:
                    ...
                    for (
                        ...
                        if (
                            TimeFramesMinutes[time_frame]
                            > TimeFramesMinutes[TimeFrames.THREE_DAYS]
                        ...
                        if total_candles_count > start_point:
                            ...
                            assert (
                                not_neutral_evaluation_count
                                / (total_candles_count - start_point)
        ...
        try:
            ...
            with (
                ...
                await self._set_data_and_check_eval(
                    start_dump, slight_dump_started_eval, True
                )

                # real dumping
                await self._set_data_and_check_eval(
                    heavy_dump, heavy_dump_started_eval, True
                )
    ...
    async def run_test_reactions_to_pump(
        ...
        try:
            ...
            with (
                ...
                # starts pumping
                await self._set_data_and_check_eval(
                    start_dump, start_pump_started_eval, False
                )

                # real pumping
                await self._set_data_and_check_eval(
                    heavy_pump, heavy_pump_started_eval, False
                )
    ...
    async def _move_and_set_data_and_check_eval(
        self, eval_index, expected_eval_note, check_inferior
    ):
        ...
        await self._set_data_and_check_eval(
            eval_index, expected_eval_note, check_inferior
        )
        self.previous_move_stop = eval_index

    async def _set_data_and_check_eval(
        self, end_index, expected_eval_note, check_inferior
    ):
        await self._set_bank_data_and_call_evaluator(end_index)
        if expected_eval_note != -2:
            if check_inferior:
                assert (
                    self.evaluator.eval_note == expected_eval_note
