# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\tests\functional_tests\backtesting_tests\backtesting_test.py
...
async def test_single_data_file_backtesting():
    await _check_double_backtesting(
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]]], enable_storage=True
        ),
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]]], enable_storage=True
        ),
...
async def test_single_data_file_no_logs_backtesting():
    await _check_double_backtesting(
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]]], use_loggers=False
        ),
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]]], use_loggers=False
        ),
...
async def test_single_data_file_mixed_logs_backtesting():
    await _check_double_backtesting(
        ...
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]]], use_loggers=True
        ),
    )
...
async def test_double_synchronized_data_file_backtesting():
    await _check_double_backtesting(
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]], DATA_FILES[BACKTESTING_SYMBOLS[1]]],
            timeout=40,
        ),
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[0]], DATA_FILES[BACKTESTING_SYMBOLS[1]]],
...
async def test_double_partially_synchronized_data_file_backtesting_common_only():
    await _check_double_backtesting(
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[3]], DATA_FILES[BACKTESTING_SYMBOLS[4]]],
            timeout=40,
        ),
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[3]], DATA_FILES[BACKTESTING_SYMBOLS[4]]],
...
async def test_double_data_file_3_months_gap_backtesting_all_data_files_range():
    await _check_double_backtesting(
        run_independent_backtesting(
            [DATA_FILES[BACKTESTING_SYMBOLS[5]], DATA_FILES[BACKTESTING_SYMBOLS[0]]],
            run_on_common_part_only=False,
            timeout=80,
        ),
        run_independent_backtesting(
...
async def _check_backtesting_results(backtesting1, backtesting2):
    if backtesting1 is not None and backtesting2 is not None:
        ...
        _, previous_profitability, previous_market_profitability, _, _ = (
            get_profitability_stats(exchange_manager_1)
        )
        _, current_profitability, current_market_profitability, _, _ = (
            get_profitability_stats(exchange_manager_2)
        )
        ...
        trades = open_orders = exchange_manager_1 = exchange_manager_2 = (
            None  # prevent memory leak
        )
    ...
    for backtesting in backtestings:
        await stop_independent_backtesting(
            backtesting, memory_check=True, should_raise=True
        )
        await asyncio.wait_for(backtesting.post_backtesting_task, 5)
    for backtesting in backtestings:
        asyncio.get_event_loop().call_soon(
            check_independent_backtesting_remaining_objects, backtesting
        )
