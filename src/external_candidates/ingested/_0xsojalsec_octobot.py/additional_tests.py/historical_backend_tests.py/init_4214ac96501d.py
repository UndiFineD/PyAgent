# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\historical_backend_tests\__init__.py
...
def _load_historical_backend_creds_env_variables_if_necessary():
    ...
    if not LOADED_BACKEND_CREDS_ENV_VARIABLES:
        ...
        dotenv_path = os.getenv(
            "HISTORICAL_BACKEND_TESTS_DOTENV_PATH",
...
async def clickhouse_client():
    async with octobot.community.history_backend_client(
        octobot.enums.CommunityHistoricalBackendType.Clickhouse
    ) as client:
        yield client
