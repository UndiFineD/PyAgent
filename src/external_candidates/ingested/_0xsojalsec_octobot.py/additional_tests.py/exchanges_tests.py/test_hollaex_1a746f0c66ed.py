# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\exchanges_tests\test_hollaex.py
...
class TestHollaexAuthenticatedExchange(
    ...
    EXPECT_BALANCE_FILTER_BY_MARKET_STATUS = (
        True  # set true when using filtered market status also filters
    )
    ...
    EXPECT_MISSING_ORDER_FEES_DUE_TO_ORDERS_TOO_OLD_FOR_RECENT_TRADES = (
        True  # when recent trades are limited and
    )
