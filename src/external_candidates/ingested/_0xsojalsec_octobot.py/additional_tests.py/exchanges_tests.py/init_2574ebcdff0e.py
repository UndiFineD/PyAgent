# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\exchanges_tests\__init__.py
        ...
        self.get_internal_producer = mock.Mock(
            return_value=mock.Mock(
                update_order_from_exchange=mock.AsyncMock(
                    side_effect=_clear_pending_state
                ),
                send=mock.AsyncMock(),
            )
        )
...
async def get_authenticated_exchange_manager(
    ...
    config = (
        ...
        if config
        else test_config.load_test_config()
    )
    if exchange_name not in config[commons_constants.CONFIG_EXCHANGES]:
        config[commons_constants.CONFIG_EXCHANGES][exchange_name] = {}
    config[commons_constants.CONFIG_EXCHANGES][exchange_name].update(
        _get_exchange_auth_details(credentials_exchange_name, use_invalid_creds)
    )
    exchange_type = config[commons_constants.CONFIG_EXCHANGES][exchange_name].get(
        commons_constants.CONFIG_EXCHANGE_TYPE,
        exchanges.get_default_exchange_type(exchange_name),
    )
    exchange_builder = (
        trading_api.create_exchange_builder(config, exchange_name)
        .has_matrix("")
        .use_tentacles_setup_config(
            get_tentacles_setup_config_with_exchange(exchange_tentacle_name)
        )
        .set_bot_id("")
        .is_real()
    ...
    if http_proxy_callback_factory:
        ...
        exchange_builder.set_proxy_config(
            exchanges.ProxyConfig(http_proxy_callback=proxy_callback)
        )
    ...
    exchange_manager_instance.exchange_backend = (
        trading_backend.exchange_factory.create_exchange_backend(
    ...
    try:
        ...
        raise errors.UnreachableExchange(
            f"{exchange_name} can't be reached, it is either offline or you are not connected "
...
def set_mocked_required_channels(exchange_manager):
    ...
    for channel in (
        ...
        exchange_channel.set_chan(
            ExchangeChannelMock(exchange_manager, channel), channel
        )
...
def _load_exchange_creds_env_variables_if_necessary():
    ...
    if not LOADED_EXCHANGE_CREDS_ENV_VARIABLES:
        ...
        dotenv_path = os.getenv(
            "EXCHANGE_TESTS_DOTENV_PATH", os.path.dirname(os.path.abspath(__file__))
        )
...
def _get_exchange_auth_details(exchange_name, use_invalid_creds):
    config = {
        commons_constants.CONFIG_EXCHANGE_KEY: _get_exchange_credential_from_env(
            exchange_name, commons_constants.CONFIG_EXCHANGE_KEY
        ),
        ...
        commons_constants.CONFIG_EXCHANGE_PASSWORD: _get_exchange_credential_from_env(
            exchange_name, commons_constants.CONFIG_EXCHANGE_PASSWORD
        ),
    }
    if use_invalid_creds:
        _invalidate(config)
    if not config[commons_constants.CONFIG_EXCHANGE_KEY]:
        raise NoProvidedCredentialsError(
            f"No {exchange_name} credentials in environment variables"
        )
    return config
    ...
    for numb in range(9):
        if str(numb) in decoded:
            ...
            if number_rindex != len(decoded) - 1:
                updated_decoded = (
                    f"{decoded[:number_rindex]}{numb + 1}{decoded[number_rindex+1:]}"
                )
                break
    if updated_decoded == decoded:
        raise ValueError("No number to invalid api key")
    exchange_config[commons_constants.CONFIG_EXCHANGE_KEY] = configuration.encrypt(
        updated_decoded
    ).decode()
...
def _get_exchange_is_sandboxed(exchange_name):
    ...
    return os_util.parse_boolean_environment_var(
        f"{exchange_name}_SANDBOXED".upper(), "false"
    )
