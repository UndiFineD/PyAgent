# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\exchanges_tests\abstract_authenticated_exchange_tester.py
...
from additional_tests.exchanges_tests import (
    NoProvidedCredentialsError,
...
class AbstractAuthenticatedExchangeTester:
    ...
    UNCANCELLABLE_ORDER_ID_SYMBOL_TYPE: tuple[
        str, str, trading_enums.TraderOrderType
    ] = None
    ...
    ORDER_PRICE_DIFF = (
        20  # % of price difference compared to current price for limit and stop orders
    )
    EXPECT_MISSING_ORDER_FEES_DUE_TO_ORDERS_TOO_OLD_FOR_RECENT_TRADES = (
        False  # when recent trades are limited and
    )
    # closed orders fees are taken from recent trades
    EXPECT_MISSING_FEE_IN_CANCELLED_ORDERS = (
        True  # when get_cancelled_orders returns None in fee
    )
    ...
    EXPECTED_QUOTE_MIN_ORDER_SIZE = (
        1  # min quote value of orders to create (used to check market status parsing)
    )
    EXPECT_BALANCE_FILTER_BY_MARKET_STATUS = (
        False  # set true when using filtered market status also filters
    )
    ...
    IGNORE_EXCHANGE_TRADE_ID = (
        False  # set True when trade.exchange_trade_id can't be set
    )
    MAX_TRADE_USD_VALUE = decimal.Decimal(8000)
    MIN_TRADE_USD_VALUE = decimal.Decimal("0.1")
    IS_ACCOUNT_ID_AVAILABLE = (
        True  # set False when get_account_id is not available and should be checked
    )
    IS_AUTHENTICATED_REQUEST_CHECK_AVAILABLE = (
        False  # set True when is_authenticated_request is implemented
    )
    ...
    USE_ORDER_OPERATION_TO_CHECK_API_KEY_RIGHTS = (
        False  # set True when api key rights can't be checked using a
    )
    ...
    IS_BROKER_ENABLED_ACCOUNT = (
        True  # set False when this test account can't generate broker fees
    )
    ...
    SUPPORTS_GET_MAX_ORDERS_COUNT = (
        False  # when True, will ensure that default values are not used
    )
    DEFAULT_MAX_DEFAULT_ORDERS_COUNT = (
        trading_constants.DEFAULT_MAX_DEFAULT_ORDERS_COUNT
    )
    ...
    async def test_get_portfolio_with_market_filter(self):
        ...
        async with self.local_exchange_manager(market_filter=self._get_market_filter()):
            ...
            if self.EXPECT_BALANCE_FILTER_BY_MARKET_STATUS:
                ...
                assert (
                    filtered_markets_portfolio == filtered
                ), f"{filtered_markets_portfolio=} != {filtered=}"
            else:
                assert (
                    filtered_markets_portfolio == all_markets_portfolio
                ), f"{filtered_markets_portfolio=} != {all_markets_portfolio=}"
    ...
    def check_portfolio_content(self, portfolio):
        ...
        for asset, values in portfolio.items():
            ...
            if (
                values[trading_constants.CONFIG_PORTFOLIO_TOTAL]
    ...
    async def inner_test_untradable_symbols(self):
        ...
        async with self.local_exchange_manager():
            ...
            all_symbols_including_disabled = (
                self.exchange_manager.exchange.get_all_available_symbols(
                    active_only=False
                )
            )
            disabled = [
            ...
            tradable_symbols = (
                await self.exchange_manager.exchange.get_all_tradable_symbols()
            )
            ...
            if disabled:
                print(
                    f"Including {disabled[0]} disabled coin (from {len(disabled)} coins)"
                )
            ...
            for untradable_symbol in untradable:
                ...
                assert (
                    ...
                    in self.exchange_manager.exchange.get_market_status(
                        untradable_symbol
                    )
                )
                ...
                ticker = await self.exchange_manager.exchange.get_price_ticker(
                    untradable_symbol
                )
                assert ticker
                price = ticker[
                ...
                recent_trades = await self.exchange_manager.exchange.get_recent_trades(
                    untradable_symbol
                )
                assert len(recent_trades) > 1
                # fetching order book is ok
                order_book = await self.exchange_manager.exchange.get_order_book(
                    untradable_symbol
                )
                assert (
                    len(
                        order_book[
                            trading_enums.ExchangeConstantsOrderBookInfoColumns.ASKS.value
                        ]
                    )
                    > 0
                )
                # is in all tickers
                all_tickers = (
                    await self.exchange_manager.exchange.get_all_currencies_price_ticker()
                )
                assert (
                    all_tickers[untradable_symbol][
                        trading_enums.ExchangeConstantsTickersColumns.CLOSE.value
                    ]
                    > 0
                )
                ...
                tradable_indexes = [
                    tradable_stepper * i for i in range(0, symbols_to_test)
                ]
                untradable_stepper = random.randint(1, len(untradable_symbols) - 2)
                untradable_indexes = [
                    untradable_stepper * i for i in range(0, symbols_to_test)
                ]
                to_test_symbols = [
                    tradable_symbols[i % (len(tradable_symbols) - 1)]
                    for i in tradable_indexes
                ] + [
                    untradable_symbols[i % (len(untradable_symbols) - 1)]
                    for i in untradable_indexes
                ]
            for i, symbol in enumerate(to_test_symbols):
                ...
                price = self.get_order_price(
                    decimal.Decimal(str(price)), False, symbol=symbol
                )
                ...
                buy_limit = await self.create_limit_order(
                    price, size, trading_enums.TradeOrderSide.BUY, symbol=symbol
                )
                await self.cancel_order(buy_limit)
                print(
                    f"{i+1}/{len(to_test_symbols)} : {symbol} Create & cancel order OK"
                )
        ...
        if self.IS_ACCOUNT_ID_AVAILABLE:
            ...
            if self.EXPECTED_GENERATED_ACCOUNT_ID:
                assert account_id in (
                    trading_constants.DEFAULT_ACCOUNT_ID,
                    trading_constants.DEFAULT_SUBACCOUNT_ID,
                )
            else:
    ...
    async def test_is_authenticated_request(self):
        ...
        def _http_proxy_callback_factory(_exchange_manager):
            ...
            def proxy_callback(
                url: str, method: str, headers: dict, body
            ) -> typing.Optional[str]:
                if not self.IS_AUTHENTICATED_REQUEST_CHECK_AVAILABLE:
                    return None
                if "rest_exchange" not in rest_exchange_data:
                    rest_exchange_data["rest_exchange"] = rest_exchange_data[
                        "exchange_manager"
                    ].exchange
                rest_exchange = rest_exchange_data["rest_exchange"]
                exchange_return_value = rest_exchange.is_authenticated_request(
    ...
    async def inner_is_authenticated_request(self, rest_exchange_data):
        if self.IS_AUTHENTICATED_REQUEST_CHECK_AVAILABLE:
            ...
            def assert_has_at_least_one_authenticated_call(calls):
                ...
                assert (
                    has_authenticated_call
                ), f"{calls} should contain at last 1 authenticated call"  # authenticated request

            # 1. test using different values
            assert (
                self.exchange_manager.exchange.is_authenticated_request(
                    "", "", {}, None
                )
                is False
            )
            ...
            assert (
                self.exchange_manager.exchange.is_authenticated_request("", "", {}, b"")
                is False
            )
            assert (
                self.exchange_manager.exchange.is_authenticated_request(
            ...
            for latest_call in latest_calls:
                assert (
                    latest_call[1] is False
                ), f"{latest_call} should be NOT authenticated"  # authenticated request
            ...
            assert (
                rest_exchange_data["calls"][-1][0][0] != latest_calls[-1][0][0]
            )  # assert latest call's url changed
            latest_calls = get_latest_calls()
            for latest_call in latest_calls:
                assert (
                    latest_call[1] is False
            ...
            with pytest.raises(ccxt.BaseError):
                ...
                await self.exchange_manager.exchange.connector.client.cancel_order(
                    self.VALID_ORDER_ID, self.SYMBOL
                )
            assert (
                rest_exchange_data["calls"][-1][0][0] != latest_calls[-1][0][0]
            )  # assert latest call's url changed
            ...
            await asyncio.sleep(
                1.5
            )  # let initial requests finish to be able to stop exchange manager
        ...
        with pytest.raises(trading_errors.AuthenticationError):
            ...
            async with self.local_exchange_manager(use_invalid_creds=True):
                ...
                raise AssertionError(
                    f"Did not raise on invalid api keys, fetched portfolio: {portfolio}"
                )
    ...
    async def inner_test_get_api_key_permissions(self):
        origin_get_api_key_rights_using_order = (
            self.exchange_manager.exchange_backend._get_api_key_rights_using_order
        )
        with mock.patch.object(
            self.exchange_manager.exchange_backend,
            "_get_api_key_rights_using_order",
            mock.AsyncMock(side_effect=origin_get_api_key_rights_using_order),
        ) as _get_api_key_rights_using_order_mock:
            permissions = (
                await self.exchange_manager.exchange_backend._get_api_key_rights()
            )
    ...
    async def inner_test_missing_trading_api_key_permissions(self):
        ...
        assert "inner_test_create_and_cancel_limit_orders#create_limit_order" in str(
            err
    ...
    async def test_api_key_ip_whitelist_error(self):
        ...
        try:
            with pytest.raises(trading_errors.InvalidAPIKeyIPWhitelistError):
                created_exchange = mock.Mock()
                async with self.local_exchange_manager(
                    identifiers_suffix="_INVALID_IP_WHITELIST"
                ):
                    created_exchange()
                    # should fail
                    portfolio = await self.get_portfolio()
                    raise AssertionError(
                        f"Did not raise on invalid IP whitelist error, fetched portfolio: {portfolio}"
                    )
                # ensure self.local_exchange_manager did not raise
                created_exchange.assert_called_once()
    ...
    async def inner_test_get_not_found_order(self):
        ...
        non_existing_order = await self.exchange_manager.exchange.get_order(
            self.VALID_ORDER_ID, self.SYMBOL
        )
    ...
    async def inner_test_is_valid_account(self):
        is_compatible, error = (
            await self.exchange_manager.exchange_backend.is_valid_account(
    ...
    async def inner_test_get_special_orders(self):
        ...
        for (
            exchange_id,
            order_details,
        ) in self.SPECIAL_ORDER_TYPES_BY_EXCHANGE_ID.items():
            (
                symbol,
                info_key,
                info_type,
                expected_type,
                expected_side,
                expected_trigger_above,
            ) = order_details
            print(order_details)
            fetched_order = await self.exchange_manager.exchange.get_order(
                exchange_id, symbol=symbol
            )
            assert fetched_order is not None
            self._check_fetched_order_dicts([fetched_order])
            # ensure parsing order doesn't crash
            parsed = personal_data.create_order_instance_from_raw(
                self.exchange_manager.trader, fetched_order
            )
            assert isinstance(parsed, personal_data.Order)

            assert (
                fetched_order[trading_enums.ExchangeConstantsOrderColumns.SYMBOL.value]
                == symbol
            )
            found_type = fetched_order[ccxt_constants.CCXT_INFO][info_key]
            assert (
                found_type == info_type
            ), f"[{exchange_id}]: {found_type} != {info_type}"
            parsed_type = fetched_order[
                trading_enums.ExchangeConstantsOrderColumns.TYPE.value
            ]
            assert (
                parsed_type == expected_type
            ), f"[{exchange_id}]: {parsed_type} != {expected_type}"
            assert (
                fetched_order[trading_enums.ExchangeConstantsOrderColumns.SIDE.value]
                == expected_side
            )
            if expected_trigger_above is None:
                assert (
                    trading_enums.ExchangeConstantsOrderColumns.TRIGGER_ABOVE.value
                    not in fetched_order
                )
            else:
                parsed_trigger_above = fetched_order[
                    trading_enums.ExchangeConstantsOrderColumns.TRIGGER_ABOVE.value
                ]
                assert (
                    parsed_trigger_above == expected_trigger_above
            ...
            if expected_type == trading_enums.TradeOrderType.LIMIT.value:
                ...
                assert (
                    fetched_order[
                        trading_enums.ExchangeConstantsOrderColumns.STOP_PRICE.value
                    ]
                    > 0
                )
    ...
    async def inner_test_create_and_cancel_limit_orders(
        ...
        price = self.get_order_price(
            await self.get_price(symbol=symbol), False, symbol=symbol
        )
        ...
        if enable_min_size_check:
            ...
            min_size = personal_data.decimal_adapt_quantity(
                ...
                * (
                    ...
                    if symbols.parse_symbol(symbol).is_spot()
                    else decimal.Decimal("1")
                )
                / (
                    decimal.Decimal("1")
                    if symbols.parse_symbol(symbol).is_inverse()
                    else price
                ),
            )
        ...
        if self.CHECK_EMPTY_ACCOUNT:
            assert (
                size >= trading_constants.ZERO
                if enable_min_size_check
                else size == trading_constants.ZERO
            )
            assert open_orders == []
        ...
        try:
            self.check_created_limit_order(
                buy_limit, price, size, trading_enums.TradeOrderSide.BUY
            )
            assert await self.order_in_open_orders(
                open_orders, buy_limit, symbol=symbol
            )
            ...
            assert (
                balance[settlement_currency][trading_constants.CONFIG_PORTFOLIO_FREE]
        ...
        assert await self.order_in_cancelled_orders(
            cancelled_orders, buy_limit, symbol=symbol
        )
    ...
    async def inner_test_create_and_fill_market_orders(self):
        ...
        if self.CHECK_EMPTY_ACCOUNT:
            ...
            return
        buy_market = await self.create_market_order(
            current_price, size, trading_enums.TradeOrderSide.BUY
        )
        post_buy_portfolio = {}
        try:
            self.check_created_market_order(
                buy_market, size, trading_enums.TradeOrderSide.BUY
            )
            filled_order = await self.wait_for_fill(buy_market)
            parsed_filled_order = personal_data.create_order_instance_from_raw(
                self.exchange_manager.trader, filled_order
            )
            self._check_order(
                parsed_filled_order, size, trading_enums.TradeOrderSide.BUY
            )
            await self.wait_for_order_exchange_id_in_trades(
                parsed_filled_order.exchange_order_id
            )
            ...
            sell_market = await self.create_market_order(
                current_price, sell_size, trading_enums.TradeOrderSide.SELL
            )
            self.check_created_market_order(
                sell_market, sell_size, trading_enums.TradeOrderSide.SELL
            )
            await self.wait_for_fill(sell_market)
            post_sell_portfolio = await self.get_portfolio()
            if post_buy_portfolio:
                self.check_portfolio_changed(
                    post_buy_portfolio, post_sell_portfolio, True
                )

    async def check_require_order_fees_from_trades(
        self, filled_exchange_order_id, symbol=None
    ):
        symbol = symbol or self.SYMBOL
        order_with_fees = await self.exchange_manager.exchange.get_order(
            filled_exchange_order_id, symbol
        )
        assert not trading_exchanges.is_missing_trading_fees(order_with_fees)
        order_maybe_without_fees = (
            await self.exchange_manager.exchange.connector.get_order(
                filled_exchange_order_id, symbol=symbol
            )
        )
        if self.exchange_manager.exchange.REQUIRE_ORDER_FEES_FROM_TRADES:
            ...
            assert not trading_exchanges.is_missing_trading_fees(
                order_maybe_without_fees
            )
    ...
    async def inner_test_create_and_cancel_stop_orders(self):
        ...
        size = self.get_order_size(
            portfolio,
            price,
            settlement_currency=self._get_edit_order_settlement_currency(),
        )
        open_orders = await self.get_open_orders()
        assert (
            self.exchange_manager.exchange.is_supported_order_type(
        ...
        try:
            self.check_created_stop_order(
                stop_loss, price, size, trading_enums.TradeOrderSide.SELL
            )
            stop_loss_from_get_order = await self.get_order(
                stop_loss.exchange_order_id, stop_loss.symbol
            )
            self.check_created_stop_order(
                stop_loss_from_get_order, price, size, trading_enums.TradeOrderSide.SELL
            )
    ...
    async def inner_test_get_cancelled_orders(self):
        if not self.exchange_manager.exchange.SUPPORT_FETCHING_CANCELLED_ORDERS:
            assert not self.exchange_manager.exchange.connector.client.has[
                "fetchCanceledOrders"
            ]
            # use get_closed order, no cancelled order is returned
            assert (
                await self.exchange_manager.exchange.connector.get_cancelled_orders(
                    self.SYMBOL
                )
                == []
            )
    ...
    async def inner_test_edit_limit_order(self):
        ...
        sell_limit = None
        try:
            sell_limit = await self.create_limit_order(
                price, size, trading_enums.TradeOrderSide.SELL
            )
            self.check_created_limit_order(
                sell_limit, price, size, trading_enums.TradeOrderSide.SELL
            )
            assert await self.order_in_open_orders(open_orders, sell_limit)
            edited_price = self.get_order_price(
                current_price, True, price_diff=2 * self.ORDER_PRICE_DIFF
            )
            edited_size = self.get_order_size(
                portfolio,
                price,
                order_size=2 * self.ORDER_SIZE,
                settlement_currency=self._get_edit_order_settlement_currency(),
            )
            sell_limit = await self.edit_order(
                sell_limit, edited_price=edited_price, edited_quantity=edited_size
            )
            await self.wait_for_edit(sell_limit, edited_size)
            sell_limit = await self.get_order(
                sell_limit.exchange_order_id, sell_limit.symbol
            )
            self.check_created_limit_order(
                sell_limit, edited_price, edited_size, trading_enums.TradeOrderSide.SELL
            )
        finally:
    ...
    async def inner_test_edit_stop_order(self):
        ...
        try:
            ...
            assert await self.order_in_open_orders(open_orders, stop_loss)
            edited_price = self.get_order_price(
                current_price, False, price_diff=2 * self.ORDER_PRICE_DIFF
            )
            edited_size = self.get_order_size(
                portfolio,
                price,
            ...
            stop_loss = await self.edit_order(
                stop_loss, edited_price=edited_price, edited_quantity=edited_size
            )
            await self.wait_for_edit(stop_loss, edited_size)
            stop_loss = await self.get_order(
                stop_loss.exchange_order_id, stop_loss.symbol
            )
            self.check_created_stop_order(
                stop_loss, edited_price, edited_size, trading_enums.TradeOrderSide.SELL
            )
        finally:
    ...
    async def _test_untriggered_single_bundled_orders(self):
        ...
        price = personal_data.decimal_adapt_price(
            ...
            stop_loss.origin_price
            * decimal.Decimal(f"{1 + self.ORDER_PRICE_DIFF/2/100}"),
        )
        limit_order = await self.create_limit_order(
            price, size, trading_enums.TradeOrderSide.BUY, push_on_exchange=False
        )
        ...
        self.check_created_limit_order(
            limit_order, price, size, trading_enums.TradeOrderSide.BUY
        )
        # stop and take profit are bundled but not created as long as the initial order is not filled
        additionally_fetched_orders = await self.get_similar_orders_in_open_orders(
    ...
    async def _test_untriggered_double_bundled_orders(self):
        # tests uncreated bundled orders (remain bound to initial order but initial order does not get filled)
        current_price = await self.get_price()
        stop_loss, take_profit = await self._get_bundled_orders_stop_take_profit(
            current_price
        )
        size = stop_loss.origin_quantity
        open_orders = await self.get_open_orders()
        ...
        params = await self.bundle_orders(
            limit_order, stop_loss, take_profit=take_profit
        )
        if not self.SUPPORTS_DOUBLE_BUNDLED_ORDERS:
            await self._create_order_on_exchange(
                limit_order, params=params, expected_creation_error=True
            )
            return
        limit_order = await self._create_order_on_exchange(limit_order, params=params)
        self.check_created_limit_order(
    ...
    async def _test_triggered_simple_bundled_orders(self):
        ...
        market_order = await self.create_market_order(
            current_price,
            size,
        ...
        try:
            ...
            fetched_conditional_orders = await self.get_similar_orders_in_open_orders(
                open_orders, created_orders
            )
            ...
            for fetched_conditional_order in fetched_conditional_orders:
                assert await self.order_not_in_open_orders(
                    open_orders, fetched_conditional_order
                )
        finally:
            # close position
            sell_market = await self.create_market_order(
                current_price, size, trading_enums.TradeOrderSide.SELL
            )
            self.check_created_market_order(
                sell_market, size, trading_enums.TradeOrderSide.SELL
            )
            await self.wait_for_fill(sell_market)
    ...
    async def _test_triggered_double_bundled_orders(self):
        ...
        params = await self.bundle_orders(
            market_order, stop_loss, take_profit=take_profit
        )
        if not self.SUPPORTS_DOUBLE_BUNDLED_ORDERS:
            await self._create_order_on_exchange(
                market_order, params=params, expected_creation_error=True
            )
            return
        buy_market = await self._create_order_on_exchange(market_order, params=params)
        self.check_created_market_order(
        ...
        try:
            ...
            orders_to_cancel = (
                fetched_conditional_orders[0:1]
                if self.CANCEL_DOUBLE_BUNDLED_ORDERS_TOGETHER
                else fetched_conditional_orders
            )
    ...
    async def _get_bundled_orders_stop_take_profit(self, current_price):
        ...
        take_profit = await self.create_order(
            take_profit_price,
            current_price,
            size,
            trading_enums.TradeOrderSide.SELL,
    ...
    async def get_portfolio(self):
        return await exchanges_test_tools.get_portfolio(
            self.exchange_manager, as_float=False, clear_empty=False
        )

    async def get_my_recent_trades(self, exchange_data=None):
        exchange_data = exchange_data or self.get_exchange_data()
        return await exchanges_test_tools.get_trades(
            self.exchange_manager, exchange_data
        )
    ...
    async def get_cancelled_orders(
        self, exchange_data=None, force_fetch=False, _symbols=None
    ):
        if (
            not force_fetch
            and not self.exchange_manager.exchange.SUPPORT_FETCHING_CANCELLED_ORDERS
        ):
            # skipped
            return []
        exchange_data = exchange_data or self.get_exchange_data()
        return await exchanges_test_tools.get_cancelled_orders(
            self.exchange_manager, exchange_data, symbols=_symbols
        )
    ...
    async def check_require_closed_orders_from_recent_trades(self, symbol=None):
        if self.exchange_manager.exchange.REQUIRE_CLOSED_ORDERS_FROM_RECENT_TRADES:
            with pytest.raises(trading_errors.NotSupported):
                await self.exchange_manager.exchange.connector.get_closed_orders(
                    symbol=symbol or self.SYMBOL
                )

    def check_duplicate(self, orders_or_trades):
        assert (
            len(orders_or_trades) * (1 - self.DUPLICATE_TRADES_RATIO)
            <= len(
                {
        ...
        for closed_order in closed_orders:
            self.check_parsed_closed_order(
                personal_data.create_order_instance_from_raw(
                    self.exchange_manager.trader, closed_order
                ),
                incomplete_fees_orders,
    ...
    def check_raw_cancelled_orders(self, cancelled_orders):
        ...
        for cancelled_order in cancelled_orders:
            ...
            self.check_parsed_closed_order(
                personal_data.create_order_instance_from_raw(
                    self.exchange_manager.trader, cancelled_order
                ),
                incomplete_fees_orders,
                allow_incomplete_fees,
            )
    ...
    def check_parsed_closed_order(
        ...
        assert order.status
        if (
            order.status == trading_enums.OrderStatus.CANCELED
            and self.EXPECT_MISSING_FEE_IN_CANCELLED_ORDERS
        ):
            ...
            try:
                ...
                assert isinstance(
                    order.fee[trading_enums.FeePropertyColumns.COST.value],
                    decimal.Decimal,
                )
                has_paid_fees = (
                    order.fee[trading_enums.FeePropertyColumns.COST.value]
                ...
                if has_paid_fees:
                    assert (
                        order.fee[trading_enums.FeePropertyColumns.CURRENCY.value]
                        is not None
                    )
                else:
                ...
                if self.CANCELLED_ORDERS_IN_CLOSED_ORDERS:
                    ...
                    assert order.fee[
                        trading_enums.FeePropertyColumns.IS_FROM_EXCHANGE.value
                    ] in (True, False)
                else:
                    assert (
                        order.fee[
                ...
                if (
                    not order.fee
                    and self.ALLOW_0_MAKER_FEES
                    and order.taker_or_maker
                    == trading_enums.ExchangeConstantsOrderColumns.MAKER.value
                ):
                    incomplete_fee_orders.append(order)
                elif (
                    allow_incomplete_fees
                    and self.EXPECT_MISSING_ORDER_FEES_DUE_TO_ORDERS_TOO_OLD_FOR_RECENT_TRADES
                ):
                    incomplete_fee_orders.append(order)
                else:
                    raise
        ...
        if order.status not in (
            trading_enums.OrderStatus.REJECTED,
            trading_enums.OrderStatus.CANCELED,
        ):
            assert order.origin_quantity
            if (
                self.OPEN_ORDERS_IN_CLOSED_ORDERS
                and order.status is trading_enums.OrderStatus.OPEN
            ):
            ...
            if order.exchange_order_id in self.EXPECTED_INVALID_ORDERS_QUANTITY:
                with pytest.raises(AssertionError):
                    self.check_theoretical_cost(
                        symbols.parse_symbol(order.symbol),
            ...
            if "USD" in order.market:
                assert (
                    self.MIN_TRADE_USD_VALUE
                    < order.total_cost
                    < self.MAX_TRADE_USD_VALUE
                )
    ...
    def check_raw_trades(self, trades):
        ...
        for trade in trades:
            self.check_parsed_trade(
                personal_data.create_trade_instance_from_raw(
                    self.exchange_manager.trader, trade
                )
            )
    ...
    def check_parsed_trade(self, trade: personal_data.Trade):
        ...
        if trade.status is not trading_enums.OrderStatus.CANCELED:
            ...
            self.check_theoretical_cost(
                symbols.parse_symbol(trade.symbol),
            ...
            if "USD" in trade.market:
                assert (
                    self.MIN_TRADE_USD_VALUE * decimal.Decimal("0.9")
                    < trade.total_cost
                    < self.MAX_TRADE_USD_VALUE
                ), (
    ...
    def check_theoretical_cost(self, symbol, quantity, price, cost):
        ...
        assert (
            theoretical_cost * decimal.Decimal("0.8")
            <= cost
            <= theoretical_cost * decimal.Decimal("1.2")
        )
    ...
    async def get_price(self, symbol=None):
        return decimal.Decimal(
            str(
                (
                    await self.exchange_manager.exchange.get_price_ticker(
                        symbol or self.SYMBOL
                    )
                )[trading_enums.ExchangeConstantsTickersColumns.CLOSE.value]
            )
    ...
    async def get_order(self, exchange_order_id, symbol=None):
        assert (
            self.exchange_manager.exchange.connector.client.has["fetchOrder"]
            is self.EXPECT_FETCH_ORDER_TO_BE_AVAILABLE
        )
        order = await self.exchange_manager.exchange.get_order(
            exchange_order_id, symbol or self.SYMBOL
        )
        self._check_fetched_order_dicts([order])
        return personal_data.create_order_instance_from_raw(
            self.exchange_manager.trader, order
        )
    ...
    async def edit_order(
        ...
        print(
            f"editing order: edited_quantity {edited_quantity} edited_price {edited_price} (current order: {order})"
        )
        edited_order = await self.exchange_manager.exchange.edit_order(
            ...
            order.symbol,
            quantity=(
                order.origin_quantity if edited_quantity is None else edited_quantity
            ),
        ...
        return personal_data_orders.order_factory.create_order_instance_from_raw(
            self.exchange_manager.trader, edited_order
        )

    async def create_limit_order(
        self, price, size, side, symbol=None, push_on_exchange=True
    ):
        order_type = (
    ...
    async def create_market_stop_loss_order(
        self, current_price, stop_price, size, side, symbol=None, push_on_exchange=True
    ):
    ...
    async def create_market_order(
        self, current_price, size, side, symbol=None, push_on_exchange=True
    ):
        order_type = (
    ...
    async def create_order(
        ...
        assert (
            current_order.is_self_managed() is False
        )  # will be a real order: can't be self-managed
        if push_on_exchange:
            current_order = await self._create_order_on_exchange(current_order)
            assert (
                current_order.is_self_managed() is False
            )  # is a real order: can't be self-managed
        if current_order is None:
            raise AssertionError("Error when creating order")
        return current_order

    async def _create_order_on_exchange(
        self, order, params=None, expected_creation_error=False
    ):
        ...
        created_order = await self.exchange_manager.trader.create_order(
            order, params=params, wait_for_creation=False
        )
        ...
        if created_order is None:
            raise AssertionError(
                f"Created order is None. input order: {order}, params: {params}"
            )
    ...
    def get_order_size(
        ...
        settlement_currency = settlement_currency or self.SETTLEMENT_CURRENCY
        currency_quantity = (
            portfolio[settlement_currency][self.PORTFOLIO_TYPE_FOR_SIZE]
            * decimal.Decimal(order_size)
    ...
    def check_order_size_and_price(
        self, size, price, symbol=None, allow_empty_size=False
    ):
        market_status = self.exchange_manager.exchange.get_market_status(
        ...
        precision_amount = market_status[
            ...
            trading_enums.ExchangeConstantsMarketStatusColumns.PRECISION_AMOUNT.value, 0
        )
        ...
        precision_price = market_status[
            ...
            trading_enums.ExchangeConstantsMarketStatusColumns.PRECISION_PRICE.value, 0
        )
        assert 0 < precision_price < 10  # is really the number of digits
        assert int(precision_price) == precision_price  # is an int

        assert (
            personal_data_orders.decimal_trunc_with_n_decimal_digits(
                size, precision_amount
            )
            == size
        )
        assert (
        ...
        adapted_details = (
            personal_data.decimal_check_and_adapt_order_details_if_necessary(
                size, price, market_status
            )
        )
        if size == trading_constants.ZERO:
            if allow_empty_size:
            ...
            assert (
                adapted_details
            ), f"Given size ({size}, cost={cost}) is too small according to parsed exchange rules"
    ...
    def get_sell_size_from_buy_order(self, buy_order):
        ...
        if (
            buy_order.fee
            and buy_order.fee[trading_enums.FeePropertyColumns.CURRENCY.value]
            == self.ORDER_CURRENCY
        ):
            sell_size = (
                sell_size - buy_order.fee[trading_enums.FeePropertyColumns.COST.value]
            )
        return personal_data.decimal_adapt_quantity(
    ...
    async def get_open_orders(self, exchange_data=None, _symbols=None):
        exchange_data = exchange_data or self.get_exchange_data()
        orders = await exchanges_test_tools.get_open_orders(
            self.exchange_manager, exchange_data, symbols=_symbols
        )
    ...
    def check_created_limit_order(self, order, price, size, side):
        ...
        expected_type = (
            personal_data.BuyLimitOrder
            if side is trading_enums.TradeOrderSide.BUY
            else personal_data.SellLimitOrder
        )
    ...
    def check_created_market_order(self, order, size, side):
        ...
        if self.CONVERTS_MARKET_INTO_LIMIT_ORDERS:
            ...
            expected_type = (
                personal_data.BuyMarketOrder
                if side is trading_enums.TradeOrderSide.BUY
                else personal_data.SellMarketOrder
            )
        assert isinstance(order, expected_type)
    ...
    def check_portfolio_changed(
        self, previous_portfolio, updated_portfolio, has_increased, symbol=None
    ):
    ...
    def _check_order(self, order, size, side):
        if self.CONVERTS_ORDER_SIZE_BEFORE_PUSHING_TO_EXCHANGES:
            ...
            assert (
                size * decimal.Decimal("0.8")
                <= order.origin_quantity
                <= size * decimal.Decimal("1.2")
            ), f"FALSE: {size * decimal.Decimal('0.8')} <= {order.origin_quantity} <= {size * decimal.Decimal('1.2')}"
        else:
            assert round(order.origin_quantity, 8) == round(
                size, 8
    ...
    async def wait_for_order_exchange_id_in_trades(self, order_exchange_id):
        ...
        def check_order_exchange_id_in_trades(trades):
            ...
            return (
                False,
                f"{order_exchange_id} is not in recent trades exchange ids ({trades_exchange_ids})",
            )

        return await self._get_recent_trades_until(
            check_order_exchange_id_in_trades, self.RECENT_TRADES_UPDATE_TIMEOUT
        )
    ...
    async def _get_recent_trades_until(self, validation_func, timeout):
        ...
        while time.time() - t0 < timeout:
            ...
            if recent_trades:
                ...
                if found:
                    print(
                        f"{self.exchange_manager.exchange_name} {validation_func.__name__} "
    ...
    async def wait_for_fill(self, order):
        def parse_is_filled(raw_order):
            return personal_data.parse_order_status(raw_order) in {
                trading_enums.OrderStatus.FILLED,
                trading_enums.OrderStatus.CLOSED,
            }

        return await self._get_order_until(
            order, parse_is_filled, self.MARKET_FILL_TIMEOUT, True
        )

    def parse_order_is_not_pending(self, raw_order):
        return personal_data.parse_order_status(raw_order) not in (
            trading_enums.OrderStatus.UNKNOWN,
            None,
        )

    async def wait_for_open(self, order):
        await self._get_order_until(
            order, self.parse_order_is_not_pending, self.OPEN_TIMEOUT, True
        )
    ...
    async def wait_for_cancel(self, order):
        return personal_data.create_order_instance_from_raw(
            self.exchange_manager.trader,
            await self._get_order_until(
                order, personal_data.parse_is_cancelled, self.CANCEL_TIMEOUT, False
            ),
        )
    ...
    async def _get_order_until(
        self, order, validation_func, timeout, can_order_be_not_found_on_exchange
    ):
        ...
        while time.time() - t0 < timeout:
            ...
            if raw_order is None:
                print(
                    f"{self.exchange_manager.exchange_name} {order.order_type} {validation_func.__name__} "
            ...
            await asyncio.sleep(1)
        raise TimeoutError(
            f"Order not filled/cancelled within {timeout}s: {order} ({validation_func.__name__})"
        )

    async def check_can_get_order(self, order):
        fetched_order = await self.get_order(order.exchange_order_id, order.symbol)
        self.check_created_limit_order(
            fetched_order, order.origin_price, order.origin_quantity, order.side
        )

    async def order_in_fetched_orders(
        self, method, previous_orders, order, symbol=None, check_presence=True
    ):
        t0 = time.time()
        iterations = 0
        while (
            time.time() - t0 < self.ORDER_IN_OPEN_AND_CANCELLED_ORDERS_TIMEOUT
            or iterations == 0
        ):
            iterations += 1
            delay_allowed = (
                self.exchange_manager.exchange.CAN_HAVE_DELAYED_OPEN_ORDERS
                if method is self.get_open_orders
                else self.exchange_manager.exchange.CAN_HAVE_DELAYED_CANCELLED_ORDERS
            )
            ...
            for fetched_order in fetched_orders:
                if check_presence:
                    if (
                        ...
                        print(
                            f"=> {self.exchange_manager.exchange_name} {order.order_type} Order found in {len(fetched_orders)} "
                        ...
                        assert len(fetched_orders) in (
                            len(previous_orders),
                            len(previous_orders) + 1,
                    ...
                    if (
                        ...
                        print(
                            f"{self.exchange_manager.exchange_name} {order.order_type} "
            ...
            if check_presence:
                ...
                print(
                    f"=> {self.exchange_manager.exchange_name} {order.order_type} Order not found"
                    f" in {len(fetched_orders)} {method.__name__} after after {time.time() - t0} seconds "
                    f"and {iterations} iterations. "
                    f"Order: [{order}]."
                )
                assert len(fetched_orders) <= len(
                    previous_orders
            ...
            await asyncio.sleep(1)
        print(
            f"Order {'not' if check_presence else 'still'} "
    ...
    async def get_similar_orders_in_open_orders(self, previous_open_orders, orders):
        ...
        for order in orders:
            for open_order in open_orders:
                fetched_order = (
                    personal_data_orders.order_factory.create_order_instance_from_raw(
                        self.exchange_manager.trader, open_order
                    )
                )
        ...
        if len(found_orders) == len(orders):
            return found_orders
        raise AssertionError(
            f"Can't find any order similar to {orders}. Found: {found_orders}"
        )
    ...
    async def order_in_cancelled_orders(
        self, previous_cancelled_orders, order, symbol=None
    ):
        if not self.exchange_manager.exchange.SUPPORT_FETCHING_CANCELLED_ORDERS:
            # skipped
            return True
        return await self.order_in_fetched_orders(
            self.get_cancelled_orders,
    ...
    async def cancel_order(self, order):
        ...
        if not await self.exchange_manager.trader.cancel_order(
            order, wait_for_cancelling=False
        ):
    ...
    def get_config(self):
        return {
            constants.CONFIG_EXCHANGES: {
                self.EXCHANGE_NAME: {constants.CONFIG_EXCHANGE_TYPE: self.EXCHANGE_TYPE}
            },
            constants.CONFIG_CRYPTO_CURRENCIES: {
                "test-crypto": {constants.CONFIG_CRYPTO_PAIRS: self._get_all_symbols()}
            },
        }
    ...
    def get_exchange_data(
        self, symbol=None, all_symbols=None
    ...
    def _get_market_filter(self):
        def market_filter(market):
            return market[
                trading_enums.ExchangeConstantsMarketStatusColumns.SYMBOL.value
            ] in (self.SYMBOL,)
    ...
    async def local_exchange_manager(
        self,
        market_filter=None,
        identifiers_suffix=None,
        use_invalid_creds=False,
        http_proxy_callback_factory=None,
    ):
        try:
            credentials_exchange_name = (
                self.CREDENTIALS_EXCHANGE_NAME or self.EXCHANGE_NAME
            )
            if identifiers_suffix:
                credentials_exchange_name = (
                    f"{credentials_exchange_name}{identifiers_suffix}"
                )
    ...
    def _get_exchange_tentacle_class(self):
        return tentacles_manager_api.get_tentacle_class_from_string(
            self._get_exchange_tentacle_name()
        )
