# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\exchanges_tests\test_binance_futures.py
...
class TestBinanceFuturesAuthenticatedExchange(
    ...
    DUPLICATE_TRADES_RATIO = (
        0.1  # allow 10% duplicate in trades (due to trade id set to order id)
    )
    VALID_ORDER_ID = "26408108410"
    EXPECTED_QUOTE_MIN_ORDER_SIZE = (
        200  # min quote value of orders to create (used to check market status parsing)
    )
    ...
    SPECIAL_ORDER_TYPES_BY_EXCHANGE_ID: dict[
        ...
        "4075521283": (
            ...
            "type",
            "TAKE_PROFIT_MARKET",
            octobot_trading.enums.TradeOrderType.LIMIT.value,
            octobot_trading.enums.TradeOrderSide.SELL.value,
            True,
        ),
        "622529": (
            "BTC/USDC:USDC",
            "type",
            "STOP_MARKET",
            octobot_trading.enums.TradeOrderType.STOP_LOSS.value,
            octobot_trading.enums.TradeOrderSide.SELL.value,
            False,
        ),
        "4076521927": (
            ...
            "TAKE_PROFIT",
            octobot_trading.enums.TradeOrderType.LIMIT.value,
            octobot_trading.enums.TradeOrderSide.BUY.value,
            False,
        ),
        "4076521976": (
            "BTC/USDT:USDT",
    ...
    UNCANCELLABLE_ORDER_ID_SYMBOL_TYPE: tuple[
        str, str, octobot_trading.enums.TraderOrderType
    ] = (
        "4076521927",
        "BTC/USDT:USDT",
        octobot_trading.enums.TraderOrderType.BUY_LIMIT.value,
    )
    ...
    async def _inner_test_create_and_cancel_limit_orders_for_margin_type(
        ...
        await self._set_account_types([self.exchange_manager.exchange.LINEAR_TYPE])
        await abstract_authenticated_exchange_tester.AbstractAuthenticatedExchangeTester.inner_test_create_and_cancel_limit_orders(
            self, margin_type=margin_type
        )
        # test with inverse symbol
        await self._set_account_types([self.exchange_manager.exchange.INVERSE_TYPE])
        await abstract_authenticated_exchange_tester.AbstractAuthenticatedExchangeTester.inner_test_create_and_cancel_limit_orders(
            self,
