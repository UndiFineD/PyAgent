...
class AbstractAuthenticatedFutureExchangeTester(
    ...
    async def _inner_test_get_empty_linear_and_inverse_positions_for_margin_type(
        ...
        for contract_type in (
            ...
            if not self.has_empty_position(
                self.get_filtered_positions(positions, contract_type)
            ):
    ...
    async def inner_test_get_and_set_leverage(self):
        ...
        if not self.exchange_manager.exchange.supports_api_leverage_update(self.SYMBOL):
            ...
            with pytest.raises(trading_errors.NotSupported):
                await self.exchange_manager.exchange.connector.set_symbol_leverage(
                    self.SYMBOL, float(new_leverage)
                )
            return
        await self.set_leverage(new_leverage)
        await self._check_margin_type_and_leverage(
            origin_margin_type, new_leverage
        )  # did not change margin type
        # change leverage back to origin value
        await self.set_leverage(origin_leverage)
        await self._check_margin_type_and_leverage(
            origin_margin_type, origin_leverage
        )  # did not change margin type
    ...
    async def inner_test_get_and_set_margin_type(
        ...
        if not self.exchange_manager.exchange.SUPPORTS_SET_MARGIN_TYPE:
            ...
            with pytest.raises(trading_errors.NotSupported):
                await self.exchange_manager.exchange.connector.set_symbol_margin_type(
                    symbol, True
                )
            with pytest.raises(trading_errors.NotSupported):
        ...
        if not has_open_position or (
            has_open_position
            and self.exchange_manager.exchange.SUPPORTS_SET_MARGIN_TYPE_ON_OPEN_POSITIONS
        ):
            await self.set_margin_type(new_margin_type, symbol=symbol)
            position = await self.get_position(symbol=symbol)
            if allow_empty_position and (
                position[trading_enums.ExchangeConstantsPositionColumns.SIZE.value]
                != trading_constants.ZERO
                or self.SUPPORTS_EMPTY_POSITION_SET_MARGIN_TYPE
            ):
                # did not change leverage
                await self._check_margin_type_and_leverage(
                    new_margin_type, origin_leverage, symbol=symbol
                )
            # restore margin type
        ...
        # did not change leverage
        await self._check_margin_type_and_leverage(
            origin_margin_type, origin_leverage, symbol=symbol
        )
    ...
    async def _check_margin_type_and_leverage(
        self, expected_margin_type, expected_leverage, symbol=None
    ):
        margin_type, leverage = await self.get_margin_type_and_leverage_from_position(
            symbol=symbol
        )
    ...
    def _check_position_content(
        self, position, symbol, position_mode=None, margin_type=None
    ):
        if symbol:
            assert (
    ...
    async def inner_test_create_and_cancel_limit_orders(
        self, symbol=None, settlement_currency=None
    ):
        if self.exchange_manager.exchange.SUPPORTS_SET_MARGIN_TYPE:
    ...
    async def inner_test_create_and_fill_market_orders(self):
        ...
        try:
            ...
            if post_buy_position is not None:
                self.check_position_changed(
                    post_buy_position, post_sell_position, False
                )
    ...
    def get_order_size(
        ...
        contract_size = self.exchange_manager.exchange.connector.get_contract_size(
            symbol
        )
        if contract_size > 1:
    ...
    async def get_positions(self, symbols=None):
        ...
        if (
            symbols is None
            and self.exchange_manager.exchange.REQUIRES_SYMBOL_FOR_EMPTY_POSITION
        ):
    ...
    async def get_leverage(self, symbol=None):
        leverage = await self.exchange_manager.exchange.get_symbol_leverage(
            symbol or self.SYMBOL
        )
        return leverage[
            trading_enums.ExchangeConstantsLeveragePropertyColumns.LEVERAGE.value
        ]

    async def set_leverage(self, leverage, symbol=None):
        return await self.exchange_manager.exchange.set_symbol_leverage(
            symbol or self.SYMBOL, float(leverage)
        )
    ...
    async def required_empty_position(self):
        ...
        if not self.is_position_empty(position):
            raise AssertionError(
                f"Empty {self.SYMBOL} position required for bundle orders tests"
            )
        try:
            yield
        finally:
    ...
    async def load_contract(self, symbol=None):
        symbol = symbol or self.SYMBOL
        if (
            self.exchange_manager.is_future
            and symbol not in self.exchange_manager.exchange.pair_contracts
        ):
            await self.exchange_manager.exchange.load_pair_future_contract(symbol)
    ...
    async def create_order(
        ...
        if not size:
            raise AssertionError(
                f"Size in required to create an order, provided size is {size}"
            )
    ...
    def check_position_changed(
        self, previous_position, updated_position, has_increased, symbol=None
    ):
        # use unified enums as it the ccxt position should have been parsed already
        previous_size = previous_position[
            trading_enums.ExchangeConstantsPositionColumns.SIZE.value
        ]
        updated_size = updated_position[
            trading_enums.ExchangeConstantsPositionColumns.SIZE.value
        ]
        if has_increased:
    ...
    def get_other_position_symbol(self, positions_blacklist, contract_type):
        ...
        for symbol in self.exchange_manager.exchange.connector.client.markets:
            if (
                symbol in ignored_symbols
                or self.exchange_manager.exchange.is_expirable_symbol(symbol)
            ):
                continue
            if (
