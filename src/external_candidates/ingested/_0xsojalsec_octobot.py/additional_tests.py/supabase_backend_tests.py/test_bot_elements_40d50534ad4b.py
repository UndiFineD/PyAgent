...
async def test_create_update_and_fetch_bot(
    authenticated_client_1, authenticated_client_2
):
    ...
    try:
        ...
        assert all(
            attribute.value in bot for attribute in supabase_backend_enums.BotKeys
        )
        ...
        updated_bot = await authenticated_client_1.update_bot(
            created_bot_id, bot_update
        )
        assert updated_bot == bot

        fetched_bots = await authenticated_client_1.fetch_bots()
        assert created_bot_id in [
            b[supabase_backend_enums.BotKeys.ID.value] for b in fetched_bots
        ]
        assert len(fetched_bots) == len(existing_bots) + 1

        client_2_fetched_bots = await authenticated_client_2.fetch_bots()
        assert created_bot_id not in [
            b[supabase_backend_enums.BotKeys.ID.value] for b in client_2_fetched_bots
        ]
...
async def test_fetch_deployment_and_deployment_data(
    authenticated_client_1, authenticated_client_2, admin_client
):
    created_bot_id = None
    try:
        bot = await authenticated_client_1.create_bot(
        ...
        try:
            deployment_url = await authenticated_client_1.fetch_deployment_url(
                deployment_url_id
            )
            assert all(
            ...
            async def _delete_deployment_url(client):
                ...
                assert (
                    res2.data[0][supabase_backend_enums.BotDeploymentURLKeys.ID.value]
                    == deployment_url_id
                )
...
async def test_fetch_startup_info(authenticated_client_1_with_temp_bot):
    ...
    assert (
        isinstance(parsed_info.forced_profile_url, str)
        or parsed_info.forced_profile_url is None
    )
    assert all(isinstance(val, str) for val in parsed_info.subscribed_products_urls)


async def test_upsert_and_reset_trades(
    authenticated_client_1_with_temp_bot, authenticated_client_2
):  # todo
    authenticated_client_1, bot_id = authenticated_client_1_with_temp_bot
    ...
    assert len(
        set(
            [
                trade[supabase_backend_enums.TradeKeys.TRADE_ID.value]
...
async def test_switch_and_update_portfolio(
    authenticated_client_1_with_temp_bot, authenticated_client_2
):
    authenticated_client_1, bot_id = authenticated_client_1_with_temp_bot
...
async def test_upsert_portfolio_history(
    authenticated_client_1_with_temp_bot, authenticated_client_2
):
    authenticated_client_1, bot_id = authenticated_client_1_with_temp_bot
    ...
    upserted_history = await authenticated_client_1.upsert_portfolio_history(
        full_history
    )
    assert_are_same_elements(full_history, upserted_history)

    assert upserted_history == await authenticated_client_1.fetch_portfolio_history(
        portfolio_id
    )
...
def _equal(d_1, d_2, key):
    ...
    if isinstance(d_1[key], list):
        return all(
            _equal(d_1[key], d_2[key], sub_key) for sub_key in range(len(d_1[key]))
        )
    if key == "time":
...
def trades_mock(seed, bot_id):
    trades = [
        {
            octobot_trading.enums.ExchangeConstantsOrderColumns.EXCHANGE_ID.value: str(
                round((seed + i) % 10) * 101
            ),
            octobot_trading.enums.ExchangeConstantsOrderColumns.TIMESTAMP.value: seed
            + i,
            octobot_trading.enums.ExchangeConstantsOrderColumns.PRICE.value: round(
                (seed + i) % 10
            )
            * 100,
            octobot_trading.enums.ExchangeConstantsOrderColumns.AMOUNT.value: round(
                (seed + i) % 3
            )
            * 100,
            ...
            octobot_trading.enums.ExchangeConstantsOrderColumns.ENTRIES.value: [
                "entry_id"
            ],
