    ...
    async with _authenticated_client(*get_backend_client_creds(1)) as client:
        ...
        try:
            bot = await client.create_bot(
                supabase_backend_enums.DeploymentTypes.SELF_HOSTED
            )
            bot_id = bot[supabase_backend_enums.BotKeys.ID.value]
            _print(
                f"created bot {bot[supabase_backend_enums.BotKeys.NAME.value]} (id: {bot_id})"
            )
            yield client, bot_id
        finally:
...
def skip_if_no_service_key(request):
    if _get_backend_service_key() is None:
        pytest.skip(
            reason=f"Disabled {request.node.name} [SUPABASE_BACKEND_SERVICE_KEY is not set]"
        )
...
async def delete_bot(client, bot_id):
    ...
    if deployment_id := bot[supabase_backend_enums.BotKeys.CURRENT_DEPLOYMENT_ID.value]:
        ...
        await client.update_bot(
            bot_id, {supabase_backend_enums.BotKeys.CURRENT_DEPLOYMENT_ID.value: None}
        )
    ...
    if portfolios:
        ...
        await client.update_bot(
            bot_id, {supabase_backend_enums.BotKeys.CURRENT_PORTFOLIO_ID.value: None}
        )
        for portfolio in portfolios:
            to_del_portfolio_id = portfolio[
    ...
    if configs:
        ...
        await client.update_bot(
            bot_id, {supabase_backend_enums.BotKeys.CURRENT_CONFIG_ID.value: None}
        )
...
async def sandboxed_insert():
    ...
    async with _authenticated_client(
        None, None, _get_backend_service_key()
    ) as admin_client:
        ...
        async def _sandboxed_insert(_table, _content):
            if _table not in to_delete:
                to_delete[_table] = []
            inserted = (
                await admin_client.table(_table).insert(_content).execute()
            ).data[0]
