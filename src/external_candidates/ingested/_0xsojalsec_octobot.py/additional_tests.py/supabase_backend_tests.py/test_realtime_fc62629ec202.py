# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\supabase_backend_tests\_test_realtime.py
...
SECONDARY_TABLE = (
    "todos2"  # any random table that all authenticated users can listen to
)
...
async def test_listen_one_chan(
    authenticated_client_1,
    authenticated_client_2,
    authenticated_client_3,
    sandboxed_insert,
    ...
    await asyncio.gather(
        *(
            _start_client(
                client, identifier, insert_received, subscribed, system_received
            )
...
async def test_listen_two_chans(
    authenticated_client_1,
    authenticated_client_2,
    authenticated_client_3,
    sandboxed_insert,
    ...
    async def start_client_dual_cb(client, identifier):
        verbose = True
        await _start_client(
            client, identifier, insert_received_cb_1, subscribed_cb_1, system_received
        )
    ...
    await asyncio.gather(
        *(
            asyncio.wait_for(subscribed_event.event.wait(), timeout)
            for subscribed_event in list(subscribed_cb_1.values())
            + list(subscribed_cb_2.values())
            if not subscribed_event.event.is_set()
        )
    )
    await asyncio.gather(
...
async def test_listen_late_signin_then_signout(
    authenticated_client_1, sandboxed_insert
):
    sign_out_timeout = 5
    _print("signing out")
...
async def test_reconnect(authenticated_client_1, sandboxed_insert):
    ...
    with mock.patch.object(
        authenticated_client_1.realtime.socket,
        "_on_message",
...
async def _start_client(
    client,
    identifier,
    on_callback_by_identifier,
    subscribed_callback_by_identifier,
    system_received_by_identifier,
    schema="public",
    table=SIGNALS_TABLE,
    event="INSERT",
):
    assert (
        client.realtime.socket.ws_connection is None
    )  # ensure connection has not yet been created
    ...
    await channel.on(event, insert_cb).subscribe(
        subscribe_cb, system_callback=system_cb
    )
...
async def _full_realtime_sign_out(client, timeout):
    ...
    async def wait_for_fully_signed_out():
        while True:
            if (
                not client.is_signed_in()
                and client.realtime.socket.closed
                and not client.realtime.socket.connected
            ):
                return
            await asyncio.sleep(0.1)
...
async def _full_realtime_sign_in(client, cred_id, timeout):
    ...
    async def wait_for_fully_signed_in():
        while True:
            if (
                client.is_signed_in()
                and not client.realtime.socket.closed
                and client.realtime.socket.connected
                and _all_chan_joined()
            ):
                return True
            await asyncio.sleep(0.1)
...
async def _insert_in_other_table(
    sandboxed_insert, table=SECONDARY_TABLE, content=SECONDARY_TABLE_INSERT_CONTENT
):
    return await sandboxed_insert.insert(table, content)


async def _send_signal(sandboxed_insert, product_id: str, signal: str):
    return await sandboxed_insert.insert(
        "signals",
        {
            enums.SignalKeys.TIME.value: community_supabase_client.CommunitySupabaseClient.get_formatted_time(
                time.time()
            ),
