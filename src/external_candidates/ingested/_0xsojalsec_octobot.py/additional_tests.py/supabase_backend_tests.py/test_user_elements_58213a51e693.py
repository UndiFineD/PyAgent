# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-OctoBot\additional_tests\supabase_backend_tests\test_user_elements.py
...
from additional_tests.supabase_backend_tests import (
    ...
    get_backend_api_creds,
    get_backend_client_auth_key,
    get_backend_client_creds,
    get_backend_client_expired_jwt_token,
    skip_if_no_service_key,
)
...
async def test_get_user(authenticated_client_1):
    ...
    assert all(attribute.value in user for attribute in supabase_backend_enums.UserKeys)
...
async def test_update_metadata(authenticated_client_1):
    ...
    assert all(updated_metadata[key] == metadata_update[key] for key in metadata_update)


async def test_fetch_subscribed_products_urls(authenticated_client_1):
    subscribed_products_urls = (
        await authenticated_client_1.fetch_subscribed_products_urls()
    )
    assert all(
        (isinstance(val, str) or val is None) for val in subscribed_products_urls
    )


async def test_sign_in_with_otp_token(
    authenticated_client_1, skip_if_no_service_key, admin_client
):
    # generate OTP token
    user = await authenticated_client_1.get_user()
    res = await admin_client.auth.admin.generate_link(
        {"email": user["email"], "type": "magiclink"}
    )
    ...
    try:
        supabase_client = community.CommunitySupabaseClient(
            backend_url, backend_key, community.ASyncConfigurationStorage(config)
        )
        saved_session = "saved_session"
        await supabase_client.auth._storage.set_item(
            supabase_client.auth._storage_key, saved_session
        )
        # wrong token
        ...
        assert (
            await supabase_client.auth._storage.get_item(
                supabase_client.auth._storage_key
            )
            == saved_session
        )
        ...
        updated_session = await supabase_client.auth._storage.get_item(
            supabase_client.auth._storage_key
        )
...
async def test_sign_in_with_auth_token():
    ...
    try:
        ...
        token = await supabase_client.get_otp_with_auth_key(
            email, get_backend_client_auth_key(1)
        )
...
async def test_expired_jwt_token(authenticated_client_1):
    initial_email = (await authenticated_client_1.get_user())[
        supabase_backend_enums.UserKeys.EMAIL.value
    ]
    ...
    assert (await authenticated_client_1.get_user())[
        supabase_backend_enums.UserKeys.EMAIL.value
    ] == initial_email
    ...
    authenticated_client_1._listen_to_auth_events("SIGNED_IN", session)
