# Extracted from: C:\DEV\PyAgent\.external\Chain-of-Recursive-Thoughts\recursive_thinking_ai.py
import json
import os
import sys
import time
...
class EnhancedRecursiveThinkingChat:
    ...
    def _call_api(
        self, messages: List[Dict], temperature: float = 0.7, stream: bool = True
    ) -> str:
        ...
        try:
            response = requests.post(
                self.base_url, headers=self.headers, json=payload, stream=stream
            )
...
Alternative response:"""
            ...
            alternative = self._call_api(
                messages, temperature=0.7 + i * 0.1, stream=True
            )
...
Then on a new line, explain your choice in one sentence."""
    ...
    def save_full_log(self, filename: str = None):
        ...
        if filename is None:
            filename = (
                f"full_thinking_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
def main():
    ...
    # Get API key
    api_key = input(
        "Enter your OpenRouter API key (or press Enter to use env variable): "
    ).strip()
    if not api_key:
        api_key = os.getenv("OPENROUTER_API_KEY")
        if not api_key:
            print(
                "Error: No API key provided and OPENROUTER_API_KEY not found in environment"
            )
            return
