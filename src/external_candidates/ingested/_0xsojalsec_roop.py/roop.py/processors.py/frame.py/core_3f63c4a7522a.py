# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-roop\roop\processors\frame\core.py
import importlib
import os
import sys
...
def multi_process_frame(
    ...
    with ThreadPoolExecutor(max_workers=roop.globals.execution_threads) as executor:
        ...
        queue_per_future = max(
            len(temp_frame_paths) // roop.globals.execution_threads, 1
        )
        while not queue.empty():
            future = executor.submit(
                process_frames, source_path, pick_queue(queue, queue_per_future), update
            )
...
def process_video(
    ...
    with tqdm(
        ...
        multi_process_frame(
            source_path, frame_paths, process_frames, lambda: update_progress(progress)
        )
