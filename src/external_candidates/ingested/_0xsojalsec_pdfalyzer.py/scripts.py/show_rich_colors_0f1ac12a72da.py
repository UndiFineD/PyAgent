# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\scripts\show_rich_colors.py
#!/usr/bin/env python
...
for i in range(256):
    texts = [
        Text(
            "{0: >{width}}".format(
                f"{STATEMENT} {i} {style}", width=len(STATEMENT) + 5 + len(style)
            ),
            style=f"color({i}) {style}",
        )  # noqa: E501
    ...
    for bg in BACKGROUNDS:
        txt = Text(
            "{0}".format(f" {BG_STATEMENT} {bg} "), style=f"color({i}) on color({bg})"
        )
        texts.append(txt)
