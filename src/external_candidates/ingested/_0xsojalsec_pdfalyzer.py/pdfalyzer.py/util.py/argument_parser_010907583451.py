# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\util\argument_parser.py
"""
...
from pdfalyzer.helpers.filesystem_helper import (
    do_all_files_exist,
    extract_page_number,
    file_exists,
    is_pdf,
...
from yaralyzer.util.argument_parser import export
from yaralyzer.util.argument_parser import parse_arguments as parse_yaralyzer_args
from yaralyzer.util.argument_parser import parser, source
...
EPILOG = (
    "Values for various config options can be set permanently by a .pdfalyzer file in your home directory; "
...
parser._action_groups = (
    parser._action_groups[:2]
    + [parser._action_groups[-1]]
    + parser._action_groups[2:-1]
)
...
def parse_arguments():
    ...
    if not args.streams:
        if args.extract_quoteds:
            exit_with_error(
                "--extract-quoted does nothing if --streams is not selected"
            )
        if args.suppress_boms:
            log.warning(
                "--suppress-boms has nothing to suppress if --streams is not selected"
            )

    if args.no_default_yara_rules and not args.yara_rules_files:
        exit_with_error(
            "--no-default-yara-rules requires at least one --yara-file argument"
        )
...
def output_sections(args, pdfalyzer) -> List[OutputSection]:
    ...
    output_sections = [
        section for section in possible_output_sections if vars(args)[section.argument]
    ]

    if len(output_sections) == 0:
        log_and_print(
            "No output section specified so outputting all sections except --streams..."
        )
        return [
            section
            for section in possible_output_sections
            if section.argument != STREAMS
        ]
    else:
...
combine_pdfs_parser.add_argument(
    "pdfs", help="two or more PDFs to combine", metavar="PDF_PATH", nargs="+"
)
...
combine_pdfs_parser.add_argument(
    "-o", "--output-file", help="path to write the combined PDFs to", required=True
)
...
def parse_combine_pdfs_args() -> Namespace:
    ...
    confirm_overwrite_txt = (
        Text("Overwrite '").append(args.output_file, style="cyan").append("'?")
    )
    ...
    if all(is_pdf(pdf) for pdf in args.pdfs):
        if all(extract_page_number(pdf) for pdf in args.pdfs):
            print_highlighted(
                "PDFs appear to have page number suffixes so sorting numerically..."
            )
            args.pdfs.sort(key=lambda pdf: extract_page_number(pdf))
        else:
            print_highlighted(
                "PDFs don't seem to end in page numbers so using provided order...",
                style="yellow",
            )
    else:
        print_highlighted(
            "WARNING: At least one of the PDF args doesn't end in '.pdf'",
            style="bright_yellow",
        )
        ask_to_proceed()
...
def exit_with_error(error_message: Optional[str] = None) -> None:
    ...
    if error_message:
        print_highlighted(
            Text("").append("ERROR", style="bold red").append(f": {error_message}")
        )
