...
def build_decoding_stats_table(scanner: BinaryScanner) -> Table:
    ...
    stats_table = _new_decoding_stats_table(
        scanner.label.plain if scanner.label else ""
    )
    ...
    for pattern, stats in scanner.regex_extraction_stats.items():
        ...
        for i, (encoding, encoding_stats) in enumerate(
            stats.per_encoding_stats.items()
        ):
            decodes_subtable.add_row(
                Text(
                    encoding, style=f"color({CHAR_ENCODING_1ST_COLOR_NUMBER + 2 * i})"
                ),
