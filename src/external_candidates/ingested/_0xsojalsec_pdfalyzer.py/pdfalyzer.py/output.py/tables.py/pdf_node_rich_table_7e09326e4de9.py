...
def get_symlink_representation(from_node, to_node) -> SymlinkRepresentation:
    ...
    pdf_instruction = root_address(
        reference_key
    )  # In case we ended up with a [0] or similar
...
def generate_rich_tree(
    node: "PdfTreeNode", tree: Optional[Tree] = None, depth: int = 0
) -> Tree:
...
def build_pdf_node_table(node: "PdfTreeNode") -> Table:
    ...
    if isinstance(node.obj, dict):
        for k, v in node.obj.items():
            ...
            if (k in DANGEROUS_PDF_KEYS) or (
                node.label == FONT and k == SUBTYPE and v == TYPE1_FONT
            ):
...
def _get_stream_preview_rows(node: "PdfTreeNode") -> List[List[Text]]:
    ...
    if node.stream_data is None or len(node.stream_data) == 0:
        log.warning(
            node.__rich__().append(" is a stream object but had no stream data")
        )
        return return_rows
    ...
    if isinstance(node.stream_data, bytes):
        ...
        stream_preview_lines = [
            clean_byte_string(line) for line in stream_preview.split(NEWLINE_BYTE)
        ]
        stream_preview_string = "\n".join(stream_preview_lines)
    else:
        stream_preview_hex = (
            f"N/A (Stream data is type '{type(node.stream_data).__name__}', not bytes)"
        )
    ...
    return_rows.append(
        [Text("StreamLength", style="grey"), size_text(len(node.stream_data))]
    )
    return return_rows
