# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\output\character_mapping.py
"""
...
def print_character_mapping(font: "FontInfo") -> None:  # noqa: F821
    ...
    charmap_entries = [
        _format_charmap_entry(k, v) for k, v in font.character_mapping.items()
    ]
    ...
    console.print(
        Padding(charmap_columns, CHARMAP_TITLE_PADDING), width=subheading_width()
    )
    console.line()
...
def _format_charmap_entry(k: str, v: str) -> Text:
    ...
    return (
        quoted_text(key, "charmap.byte")
        + Text(" => ")
        + quoted_text(str(v), "charmap.char")
    )
