# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\output\pdfalyzer_presenter.py
"""
...
from pdfalyzer.output.layout import (
...
class PdfalyzerPresenter:
    ...
    def print_document_info(self) -> None:
        ...
        console.line()
        console.print(
            bytes_hashes_table(self.pdfalyzer.pdf_bytes, self.pdfalyzer.pdf_basename)
        )
        console.line()
    ...
    def print_tree(self) -> None:
        ...
        for pre, _fill, node in RenderTree(self.pdfalyzer.pdf_tree, style=DoubleStyle):
            if isinstance(node, SymlinkNode):
                ...
                console.print(
                    pre
                    + f"[{symlink_rep.style}]{symlink_rep.text}[/{symlink_rep.style}]"
                )
            else:
    ...
    def print_font_info(self, font_idnum=None) -> None:
        ...
        print_section_header(
            f"{len(self.pdfalyzer.font_infos)} fonts found in {self.pdfalyzer.pdf_basename}"
        )

        for font_info in [
            fi
            for fi in self.pdfalyzer.font_infos
            if font_idnum is None or font_idnum == fi.idnum
        ]:
    ...
    def print_streams_analysis(self, idnum: Optional[int] = None) -> None:
        ...
        for node in [
            n
            for n in self.pdfalyzer.stream_nodes()
            if idnum is None or idnum == n.idnum
        ]:
            ...
            if not isinstance(node_stream_bytes, bytes):
                msg = (
                    f"Stream in {node} is not bytes, it's {type(node.stream_data)}. Will reencode for YARA "
            ...
            console.line()
            print_section_subheader(
                f"{escape(str(node))} Summary and Analysis",
                style=f"{BYTES_HIGHLIGHT} reverse",
            )
            ...
            if not YaralyzerConfig.args.suppress_decodes_table:
                ...
                console.line(2)
                console.print(
                    build_decoding_stats_table(binary_scanner), justify="center"
                )

    def print_yara_results(self) -> None:
        """Scan the main PDF and each individual binary stream in it with yara_rules/*.yara files"""
        print_section_header(
            f"YARA Scan of PDF rules for '{self.pdfalyzer.pdf_basename}'"
        )
        YaralyzerConfig.args.standalone_mode = (
            True  # TODO: using 'standalone mode' like this kind of sucks
        )
