# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\__init__.py
import code
import sys
...
if not environ.get("INVOKED_BY_PYTEST", False):
    for dotenv_file in [
        path.join(dir, ".pdfalyzer") for dir in [getcwd(), path.expanduser("~")]
    ]:
...
def pdfalyze():
    ...
    if args.extract_binary_streams:
        log_and_print(
            f"Extracting binary streams in '{args.file_to_scan_path}' to files in '{args.output_dir}'..."
        )
        PdfParserManager(args.file_to_scan_path).extract_all_streams(args.output_dir)
        log_and_print(
            f"Binary stream extraction complete, files written to '{args.output_dir}'.\nExiting.\n"
        )
        sys.exit()
...
def combine_pdfs():
    ...
    txt.append(str(file_size_in_mb(args.output_file)), style="cyan").append(
        " megabytes\n"
    )
