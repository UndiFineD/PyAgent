...
class PdfalyzerConfig:
    ...
    if is_env_var_set_and_not_false(PDF_PARSER_EXECUTABLE_ENV_VAR):
        PDF_PARSER_EXECUTABLE = path.join(
            environ[PDF_PARSER_EXECUTABLE_ENV_VAR], "pdf-parser.py"
        )
    ...
    def get_output_basepath(cls, export_method: str) -> str:
        ...
        if export_type == "streams_analysis":
            ...
            if cls._args.extract_quoteds:
                output_basename += (
                    f"_extractquoteds-{','.join(cls._args.extract_quoteds)}"
                )

        return path.join(
            cls._args.output_dir,
            output_basename
            + cls._args.file_suffix
            + f"___pdfalyzed_{cls._args.invoked_at_str}",
        )
