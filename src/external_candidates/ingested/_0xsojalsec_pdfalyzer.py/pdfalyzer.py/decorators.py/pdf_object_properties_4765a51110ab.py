...
class PdfObjectProperties:
    ...
    @classmethod
    def from_reference(
        cls, reference: IndirectObject, address: str
    ) -> "PdfObjectProperties":
    ...
    def to_table_row(
        ...
        return [
            ...
            # they are in.
            Text("", style=get_class_style(obj)).append_text(
                cls._obj_to_rich_text(with_resolved_refs)
            ),
            # 3rd col (AKA type(value)) is redundant if it's a TextString/Number/etc. node so we make it empty
            (
                ""
                if is_single_row_table
                else Text(pypdf_class_name(obj), style=get_class_style_dim(obj))
            ),
        ]
    ...
    def _obj_to_rich_text(cls, obj: Any) -> Text:
        ...
        if isinstance(obj, dict):
            key_value_pairs = [
                Text(f"{k}: ").append_text(cls._obj_to_rich_text(v))
