# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\decorators\indeterminate_node.py
"""
...
from pdfalyzer.helpers.string_helper import (
    all_strings_are_same_ignoring_numbers,
    has_a_common_substring,
)
...
class IndeterminateNode:
    ...
    def place_node(self) -> None:
        ...
        if self._has_only_similar_relationships():
            log.info(
                f"  Fuzzy match addresses or labels; placing under node w/most descendants: {parent_str}"
            )
        elif self._make_parent_if_one_remains(
            lambda r: r.from_node.type in PAGE_AND_PAGES
        ):
            log.info("  Found a single /Page or /Pages relationship to make parent")
            return
        elif self.node.type == COLOR_SPACE:
            log.info(
                f"  Color space node found; placing under node w/most descendants: {parent_str}"
            )
            ...
            # in a particular way.
            log.warning(
                f"  {self.node} seems to be a loose {PAGE}. Linking to first {PAGES}"
            )
            pages_nodes = [
                n
                for n in self.node.nodes_with_here_references()
                if self.node.type == PAGES
            ]
            self.node.set_parent(self.find_node_with_most_descendants(pages_nodes))
            return
        else:
            log.warning(
                f"  {self.node} parent {parent_str} chosen based on descendant count only"
            )
            self.node.log_non_tree_relationships()

        self.node.set_parent(parent)

    def find_node_with_most_descendants(
        self, list_of_nodes: List[PdfTreeNode] = None
    ) -> PdfTreeNode:
        """Find node with a reference to this one that has the most descendants"""
        list_of_nodes = list_of_nodes or [
            r.from_node for r in self.node.non_tree_relationships
        ]
        max_descendants = max([node.descendants_count() for node in list_of_nodes])
        return find_node_with_lowest_id(
            [n for n in list_of_nodes if n.descendants_count() == max_descendants]
        )
    ...
    def _has_only_similar_relationships(self) -> bool:
        ...
        reference_keys_or_nodes_are_same = any(
            [
                all_strings_are_same_ignoring_numbers(_list)
                or has_a_common_substring(_list)
    ...
    def _check_for_common_ancestor(self) -> bool:
        common_ancestor = self._find_common_ancestor_among_nodes(
            self.node.nodes_with_here_references()
        )
    ...
    def _find_common_ancestor_among_nodes(
        self, nodes: List[PdfTreeNode]
        ...
        for possible_ancestor in nodes:
            ...
            if all(possible_ancestor in node.ancestors for node in other_nodes):
                ...
                log.info(
                    f"{possible_ancestor} is the common ancestor of {other_nodes_str}"
                )
    ...
    def _check_single_relation_rules(self) -> bool:
        ...
        if self._make_parent_if_one_remains(lambda r: r.reference_key in [K, KIDS]):
            ...
            lambda r: r.from_node.type not in NON_TREE_KEYS
        ):
    ...
    def _make_parent_if_one_remains(self, is_possible_parent: Callable) -> bool:
        ...
        remaining_relationships = [
            r for r in self.node.non_tree_relationships if is_possible_parent(r)
        ]

        if len(remaining_relationships) == 1:
            log.info(
                f"  Single remaining relationship {remaining_relationships[0]}; making it the parent"
            )
