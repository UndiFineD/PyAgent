# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\decorators\document_model_printer.py
"""
...
from pdfalyzer.helpers.string_helper import INDENT_DEPTH, pp
...
def print_all_props(pdf_obj, console, verbose=False, indent=""):
    ...
    for k, v in pdf_obj.items():
        ...
        console.print(
            indent + " {0: <66}{1: >20}".format(pretty_str, pypdf_class_name(v))
        )
    ...
    for k, v in large_print_jobs.items():
        ...
        console.print(
            indent + f" {k}  ({pypdf_class_name(v)})    {indent_join}{pretty_str}"
        )
