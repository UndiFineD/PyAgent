# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\decorators\pdf_tree_verifier.py
"""
...
class PdfTreeVerifier:
    ...
    def verify_all_nodes_encountered_are_in_tree(self) -> None:
        ...
        missing_nodes = [
            node
            for idnum, node in self.pdfalyzer.nodes_encountered.items()
            if self.pdfalyzer.find_node_by_idnum(idnum) is None
        ]

        if len(missing_nodes) > 0:
            msg = (
                f"Nodes were traversed but never placed: {escape(str(missing_nodes))}\n"
    ...
    def verify_unencountered_are_untraversable(self) -> None:
        ...
        if self.pdfalyzer.pdf_size is None:
            log.warning(
                f"{SIZE} not found in PDF trailer; cannot verify all nodes are in tree"
            )
            return
        if self.pdfalyzer.max_generation > 0:
            log.warning(
                f"Methodd doesn't check revisions but this doc is generation {self.pdfalyzer.max_generation}"
            )

        # We expect to see all ordinals up to the number of nodes /Trailer claims exist as obj. IDs.
        missing_node_ids = [
            i
            for i in range(1, self.pdfalyzer.pdf_size)
            if self.pdfalyzer.find_node_by_idnum(i) is None
        ]

        for idnum in missing_node_ids:
            ref = IndirectObject(
                idnum, self.pdfalyzer.max_generation, self.pdfalyzer.pdf_reader
            )
            ...
            try:
                ...
                if "Invalid Elementary Object" in str(e):
                    log.warning(
                        f"Couldn't verify elementary obj with id {idnum} is properly in tree"
                    )
                    continue
                log.error(str(e))
            ...
            if obj is None:
                ...
                log.info(
                    f"Obj {idnum} is a {type(obj)} w/value {obj}; if relationshipd by /Length etc. this is a nonissue but maybe worth doublechecking"
                )  # noqa: E501
                continue
            elif not isinstance(obj, dict):
                log.error(
                    f"Obj {idnum} ({obj}) of type {type(obj)} isn't dict, cannot determine if it should be in tree"
                )  # noqa: E501
                continue
            ...
            if obj_type == OBJECT_STREAM:
                log.debug(
                    f"Object with id {idnum} not found in tree because it's an {OBJECT_STREAM}"
                )
            elif obj[TYPE] == XREF:
                ...
                for k, v in self.pdfalyzer.pdf_reader.trailer.items():
                    ...
                    if k in [XREF_STREAM, PREV]:
                        ...
                        if xref_val_for_key is None or v != (xref_val_for_key + 1):
                            log.info(
                                f"{XREF} has {SIZE} of {xref_val_for_key}, trailer has {SIZE} of {v}"
                            )
                            placeable = False
                        ...
                        log.info(
                            f"Trailer has {k} -> {v} but {XREF} obj has {obj.get(k)} at that key"
                        )
                        placeable = False

                if placeable:
                    self.pdfalyzer.pdf_tree.add_child(
                        self.pdfalyzer._build_or_find_node(ref, XREF_STREAM)
                    )
            else:
