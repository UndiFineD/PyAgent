# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\decorators\pdf_tree_node.py
"""
...
class PdfTreeNode(NodeMixin, PdfObjectProperties):
    def __init__(self, obj: PdfObject, address: str, idnum: int):
        ...
        if isinstance(obj, StreamObject):
            try:
                ...
                msg = (
                    f"PyPDF failed to decode stream in {self}: {e}.\n"
    ...
    def from_reference(cls, ref: IndirectObject, address: str) -> "PdfTreeNode":
        ...
        try:
            ...
            console.print_exception()
            msg = (
                f"Failed to build node properly because of above exception ({e}). "
    ...
    def set_parent(self, parent: "PdfTreeNode") -> None:
        ...
        if self.parent is not None and self.parent != parent:
            raise PdfWalkError(
                f"Cannot set {parent} as parent of {self}, parent is already {self.parent}"
            )

        self.parent = parent
        self.remove_non_tree_relationship(parent)
        self.known_to_parent_as = (
            self.address_of_this_node_in_other(parent) or self.first_address
        )
    ...
    def remove_non_tree_relationship(self, from_node: "PdfTreeNode") -> None:
        ...
        relationships_to_remove = [
            r for r in self.non_tree_relationships if r.from_node == from_node
        ]

        if len(relationships_to_remove) == 0:
            return
        elif len(relationships_to_remove) > 1 and not all(
            r.reference_key in [FIRST, LAST] for r in relationships_to_remove
        ):
            log.warning(
                f"> 1 relationships to remove from {from_node} to {self}: {relationships_to_remove}"
            )
    ...
    def tree_address(
        self, max_length: Optional[int] = DEFAULT_MAX_ADDRESS_LENGTH
    ) -> str:
        ...
        if self.label == TRAILER:
            ...
            raise PdfWalkError(
                f"{self} does not have a parent; cannot get accurate node address."
            )
    ...
    def address_of_this_node_in_other(self, from_node: "PdfTreeNode") -> Optional[str]:
        ...
        refs_to_this_node = [
            ref
            for ref in from_node.references_to_other_nodes()
        ...
        if len(refs_to_this_node) == 1:
            ...
            if (
                self.parent
                and self.parent.label == TRAILER
                and self.type == XREF
                and XREF_STREAM in self.parent.obj
            ):
                return XREF_STREAM
            elif self.label not in NON_STANDARD_ADDRESS_NODES:
                log.info(
                    f"Could not find expected reference from {from_node} to {self}"
                )
            else:
                return None
        else:
            ...
            if not (
                is_prefixed_by_any(from_node.label, NON_STANDARD_ADDRESS_NODES)
                or all(
                    ref.address in NON_STANDARD_ADDRESS_NODES
                    for ref in refs_to_this_node
                )
            ):
                refs_to_this_node_str = "\n   ".join(
                    [f"{i + 1}. {r}" for i, r in enumerate(refs_to_this_node)]
                )
    ...
    def symlink_non_tree_relationships(self):
        ...
        log.info(
            f"Symlinking {self}'s {self.non_tree_relationship_count()} other relationships..."
        )

        for relationship in self.non_tree_relationships:
            if relationship.from_node in self.tree_relationships():
                log.warning(
                    f"  {relationship} is still 'non-tree' but is a parent or child of {self}"
                )
            else:
    ...
    def descendants_count(self) -> int:
        ...
        return len(self.children) + sum(
            [child.descendants_count() for child in self.children]
        )
    ...
    def _write_non_tree_relationships(self, write_method: Callable) -> None:
        ...
        for i, r in enumerate(self.non_tree_relationships):
            write_method(
                f"  {i + 1}. {escape(str(r))}, Descendant Count: {r.from_node.descendants_count()}"
            )
    ...
    def __rich__(self) -> Text:
        return (
            PdfObjectProperties.__rich__(self)[:-1]
            + self._colored_address()
            + Text(">")
        )
