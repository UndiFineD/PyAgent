# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\helpers\rich_text_helper.py
"""
...
from pdfalyzer.output.styles.node_colors import get_class_style_italic, get_label_style
...
def node_label(
    idnum: int, label: str, pdf_object: PdfObject, underline: bool = True
) -> Text:
    ...
    text.append(
        label[1:],
        style=f"{get_label_style(label)} {'underline' if underline else ''} bold",
    )
    text.append("(", style="white")
...
def number_and_pct(_number: int, total: int, digits: int = 1) -> Text:
    """Return e.g. '8 (80%)'."""
    return Text(str(_number), style="bright_white").append_text(
        pct_txt(_number, total, digits)
    )
