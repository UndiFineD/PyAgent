# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\helpers\filesystem_helper.py
"""
...
def set_max_open_files(
    num_filehandles: int = DEFAULT_MAX_OPEN_FILES,
    ...
    if resource is None:
        print_highlighted(
            f"No resource module; cannot set max open files on this platform...",
            style="yellow",
        )
        return (None, None)
    ...
    if soft < num_filehandles:
        ...
        hard = max(soft, hard)
        print_highlighted(
            f"Increasing max open files soft & hard 'ulimit -n {soft} {hard}'..."
        )
        ...
        try:
            ...
            try:
                hard = soft
                print_highlighted(
                    f"Retrying setting max open files (soft, hard)=({soft}, {hard})",
                    style="yellow",
                )
                resource.setrlimit(resource.RLIMIT_NOFILE, (soft, hard))
            except Exception:
                print_highlighted(
                    "Failed to set max open files / ulimit, giving up!", style="error"
                )
                soft, hard = resource.getrlimit(resource.RLIMIT_NOFILE)
