# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\detection\yaralyzer_helper.py
"""
...
def _build_yaralyzer(
    scannable: Union[bytes, str], label: Optional[str] = None
) -> Yaralyzer:
                ...
                with as_file(YARA_RULES_DIR.joinpath(YARA_RULES_FILES[3])) as yara3:
                    with as_file(YARA_RULES_DIR.joinpath(YARA_RULES_FILES[4])) as yara4:
                        ...
                        if not YaralyzerConfig.args.no_default_yara_rules:
                            rules_paths += [
                                str(y) for y in [yara0, yara1, yara2, yara3, yara4]
                            ]

                        try:
                            return Yaralyzer.for_rules_files(
                                rules_paths, scannable, label
                            )
