# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\pdf_object_relationship.py
"""
...
class PdfObjectRelationship:
    def __init__(
        ...
        if (
            has_indeterminate_prefix(from_node.type)
            and not isinstance(self.from_node.obj, dict)
        ...
        self.is_link = reference_key in NON_TREE_KEYS or is_prefixed_by_any(
            from_node.label, LINK_NODE_KEYS
        )
        self.is_parent = reference_key == PARENT or (
            from_node.type == STRUCT_ELEM and reference_key == P
        )
        ...
        if from_node.type == OBJR and reference_key == OBJ:
            log.info(
                f"Explicit (theoretically) child reference found for {OBJ} in {from_node}"
            )
            self.is_child = True
        elif reference_key == KIDS or (
            from_node.type == STRUCT_ELEM and reference_key == K
        ):
            self.is_child = True
        else:
            self.is_child = False
    ...
    def build_node_references(
        ...
        if from_node is None and from_obj is None:
            raise ValueError(
                "Either :from_node or :from_obj must be provided to get references"
            )
        ...
        if isinstance(from_obj, IndirectObject):
            ...
            for i, item in enumerate(from_obj):
                references += cls.build_node_references(
                    from_node, item, ref_key or i, _build_address(i, address)
                )
        elif isinstance(from_obj, dict):
            for key, val in from_obj.items():
                references += cls.build_node_references(
                    from_node, val, ref_key or key, _build_address(key, address)
                )
        else:
            log.debug(
                f"Adding no references for PdfObject reference '{ref_key}' -> '{from_obj}'"
            )
...
def _build_address(ref_key: Union[str, int], base_address: Optional[str] = None) -> str:
    ...
    if isinstance(ref_key, int):
        return (
            ...
            else f"{base_address}{bracketed_ref_key}"
        )
