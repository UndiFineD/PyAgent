# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\font_info.py
"""
...
from pdfalyzer.output.character_mapping import (
    print_character_mapping,
    print_prepared_charmap,
)
...
class FontInfo:
    ...
    def extract_font_infos(cls, obj_with_resources: PdfObject) -> ["FontInfo"]:
        ...
        return [
            cls.build(label, font, obj_with_resources) for label, font in fonts.items()
        ]

    @classmethod
    def build(
        cls, label: str, font_ref: IndirectObject, obj_with_resources
    ) -> "FontInfo":
        ...
        if FONT_DESCRIPTOR in font_obj:
            ...
            if len(font_file_keys) > 1:
                raise RuntimeError(
                    f"Too many /FontFile keys in {font_descriptor}: {font_file_keys}"
                )
    ...
    def __init__(
        self, label, idnum, font, font_descriptor, font_file, obj_with_resources
    ):
        self.label = label
        self.idnum = idnum
