# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\pdfalyzer.py
"""
...
class Pdfalyzer:
    def __init__(self, pdf_path: str):
        ...
        pdf_file = open(
            pdf_path, "rb"
        )  # Filehandle must be left open for PyPDF to perform seeks
        ...
        self.max_generation = (
            0  # PDF revisions are "generations"; this is the max generation encountered
        )
    ...
    def walk_node(self, node: PdfTreeNode) -> None:
        ...
        log.info(
            f"walk_node() called with {node}. Object dump:\n{print_with_header(node.obj, node.label)}"
        )
        nodes_to_walk_next = [
            self._add_relationship_to_pdf_tree(r)
            for r in node.references_to_other_nodes()
        ]
        node.all_references_processed = True

        for next_node in [
            n
            for n in nodes_to_walk_next
            if not (n is None or n.all_references_processed)
        ]:
    ...
    def find_node_by_idnum(self, idnum) -> Optional[PdfTreeNode]:
        ...
        nodes = [
            node
            for node in findall_by_attr(self.pdf_tree, name="idnum", value=idnum)
            if not isinstance(node, SymlinkNode)
        ]
    ...
    def stream_nodes(self) -> List[PdfTreeNode]:
        ...
        stream_filter = lambda node: node.contains_stream() and not isinstance(
            node, SymlinkNode
        )  # noqa: E731
        return sorted(findall(self.pdf_tree, stream_filter), key=lambda r: r.idnum)

    def _add_relationship_to_pdf_tree(
        self, relationship: PdfObjectRelationship
    ) -> Optional[PdfTreeNode]:
        """
        ...
        was_seen_before = (
            relationship.to_obj.idnum in self.nodes_encountered
        )  # Must come before _build_or_find()
        ...
        self.max_generation = max(
            [self.max_generation, relationship.to_obj.generation or 0]
        )
        ...
        if relationship.is_parent or relationship.is_child:
            ...
            if relationship.is_parent:
                ...
                if relationship.from_node.type.startswith(STRUCT_ELEM):
                    log.info(
                        f"{relationship} fail: {to_node} parent is already {to_node.parent}"
                    )
                else:
                    log.warning(
                        f"{relationship} fail: {to_node} parent is already {to_node.parent}"
            ...
            if relationship.to_obj.idnum in self.indeterminate_ids:
                log.info(
                    f"  Found {relationship} => {to_node} was marked indeterminate but now placed"
                )
        ...
        # a non-tree relationship for now. An attempt to place the node will be made at the end.
            ...
            if was_seen_before:
                if (
                    relationship.to_obj.idnum not in self.indeterminate_ids
                    and to_node.parent is None
                ):
                    raise PdfWalkError(
                        f"{relationship} - ref has no parent and is not indeterminate"
                    )
                else:
            ...
            elif relationship.is_indeterminate or (
                relationship.is_link and not self.is_in_tree(to_node)
            ):
    ...
    def _resolve_indeterminate_nodes(self) -> None:
        ...
        indeterminate_nodes = [
            self.nodes_encountered[idnum] for idnum in self.indeterminate_ids
        ]
        indeterminate_nodes_string = "\n   ".join(
            [f"{node}" for node in indeterminate_nodes]
        )
        log.info(
            f"Resolving {len(indeterminate_nodes)} indeterminate nodes: {indeterminate_nodes_string}"
        )
    ...
    def _extract_font_infos(self) -> None:
        ...
        for node in self.node_iterator():
            if isinstance(node.obj, dict) and RESOURCES in node.obj:
                log.debug(
                    f"Extracting fonts from node with '{RESOURCES}' key: {node}..."
                )
                known_font_ids = [fi.idnum for fi in self.font_infos]

                self.font_infos += [
                    fi
                    for fi in FontInfo.extract_font_infos(node.obj)
                    if fi.idnum not in known_font_ids
                ]

    def _build_or_find_node(
        self, relationship: IndirectObject, relationship_key: str
    ) -> PdfTreeNode:
