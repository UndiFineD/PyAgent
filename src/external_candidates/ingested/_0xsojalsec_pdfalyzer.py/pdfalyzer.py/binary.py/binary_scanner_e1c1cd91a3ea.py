# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\pdfalyzer\binary\binary_scanner.py
"""
...
from pdfalyzer.detection.constants.binary_regexes import (
    BACKTICK,
    DANGEROUS_PDF_KEYS_TO_HUNT_ONLY_IN_FONTS,
    DANGEROUS_STRINGS,
    FRONTSLASH,
    GUILLEMET,
    QUOTE_PATTERNS,
...
class BinaryScanner:
    ...
    def check_for_dangerous_instructions(self) -> None:
        ...
        for instruction in DANGEROUS_STRINGS:
            yaralyzer = self._pattern_yaralyzer(
                instruction, REGEX
            )  # TODO maybe change REGEX const string?
        ...
        if self.owner.type in FONT_FILE_KEYS:
            log.info(
                f"{self.owner} is a /FontFile. Scanning for short but dangerous PDF keys..."
            )
    ...
    def force_decode_quoted_bytes(self) -> None:
        ...
        if len(quote_selections) == 0:
            headline = (
                "Skipping extract/decode of quoted bytes (--extract-quoted is empty)"
            )
        ...
        for quote_type in quote_selections:
            if (
                self.owner
                and self.owner.type == CONTENTS
                and quote_type in [FRONTSLASH, GUILLEMET]
            ):
    ...
    def print_stream_preview(self, num_bytes=None, title_suffix=None) -> None:
        ...
        num_bytes = (
            num_bytes or PdfalyzerConfig._args.preview_stream_length or console_width()
        )
        ...
        if snipped_byte_count < 0:
            ...
            title = (
                f"FIRST AND LAST {num_bytes} BYTES OF {self.stream_length} BYTE STREAM"
            )
    ...
    def process_yara_matches(
        self, yaralyzer: Yaralyzer, pattern: str, force: bool = False
    ) -> None:
        ...
        for bytes_match, decoder in yaralyzer.match_iterator():
            log.debug(
                f"Trackings match stats for {pattern}, bytes_match: {bytes_match}, is_decodable: {bytes_match.is_decodable()}"
            )  # noqa: E501
            ...
            self.regex_extraction_stats[pattern].tally_match(
                decoder
            )  # TODO: This call must come after print(decoder)
    ...
    def bytes_after_eexec_statement(self) -> bytes:
        ...
        return (
            self.bytes.split(CURRENTFILE_EEXEC)[1]
    ...
    def _pattern_yaralyzer(
        ...
        return Yaralyzer.for_patterns(
            ...
            rules_label=safe_label(
                rules_label or pattern
            ),  # TODO: maybe safe_label() belongs in yaralyzer
    ...
    def _print_suppression_notices(self) -> None:
        ...
        suppression_notices_txt = Text("\n").join(
            [notice for notice in self.suppression_notice_queue]
        )
    ...
    def _eexec_idx(self) -> int:
        ...
        return (
            self.bytes.find(CURRENTFILE_EEXEC) if CURRENTFILE_EEXEC in self.bytes else 0
        )
