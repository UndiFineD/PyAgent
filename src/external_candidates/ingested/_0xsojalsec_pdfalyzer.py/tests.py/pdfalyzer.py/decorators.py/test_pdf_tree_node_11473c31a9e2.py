...
def test_pdf_node_address(analyzing_malicious_pdfalyzer):
    ...
    assert (
        node41.tree_address()
        == "/Root/StructTreeRoot/K[0]/K[24]/K[1]/K[3]/K[0]/K[0]/K[1]/K[0]/Obj"
    )
    node6 = analyzing_malicious_pdfalyzer.find_node_by_idnum(6)
    assert (
        node6.tree_address()
        == "/Root/Pages/Kids[0]/Resources[/Font][/F1]/FontDescriptor"
    )
    node17 = analyzing_malicious_pdfalyzer.find_node_by_idnum(17)
    assert (
        node17.tree_address()
        == "/Root/Pages/Kids[0]/Resources[/Font][/F4]/DescendantFonts[0]/CIDSystemInfo"
    )


def test_address_of_this_node_in_other(
    analyzing_malicious_pdfalyzer, page_node, pages_node
):
    ...
    assert (
        node7.address_of_this_node_in_other(page_node) == "/Resources[/ExtGState][/GS7]"
    )
