# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\tests\pdfalyzer\helpers\test_string_helper.py
from pdfalyzer.helpers.string_helper import (
    all_strings_are_same_ignoring_numbers,
    has_a_common_substring,
    is_prefixed_by_any,
    is_substring_of_longer_strings_in_list,
    replace_digits,
)
...
def test_all_strings_are_same_ignoring_numbers():
    ...
    assert all_strings_are_same_ignoring_numbers(
        ["ny_state_of_9_mind", "ny_state_of_5_mind"]
    )


def test_is_substring_of_longer_strings_in_list():
    assert not is_substring_of_longer_strings_in_list("blah", ["blimp", "blew", "ruby"])
    assert is_substring_of_longer_strings_in_list(
        "goforit", ["goforit", "agoforitnow", "goforitnowdww"]
    )
