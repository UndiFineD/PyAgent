# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdfalyzer\tests\pdfalyzer\test_pdf_object_relationship.py
import pytest
...
def page_obj_direct_refs(page_obj, page_node):
    yield [
        PdfObjectRelationship(
            page_node, IndirectObject(2, 0, pdf_reader), PARENT, PARENT
        ),
        PdfObjectRelationship(
            page_node, IndirectObject(4, 0, pdf_reader), CONTENTS, CONTENTS
        ),
    ]
...
def test_build_node_references(pdf_reader, page_obj, page_node, page_obj_direct_refs):
    ...
    annots_refs = [
        PdfObjectRelationship(
            page_node, IndirectObject(id, 0, pdf_reader), ANNOTS, ANNOTS + f"[{i}]"
        )
        for i, id in enumerate(ANNOTS_IDS)
    ]

    expected_refs = _sort_pdf_object_refs(
        page_obj_direct_refs + ext_g_state_refs + font_refs + annots_refs
    )
    actual_refs = _sort_pdf_object_refs(
        PdfObjectRelationship.build_node_references(page_node)
    )
