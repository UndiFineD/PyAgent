# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\storages\object_storages\google_cloud.py
    ...
    def _prepare_and_check(self) -> None:
        ...
        try:
            ...
            if not exists and self.create_if_not_exists:
                self._client.create()
                warn(
                    f"Bucket {self._client.name} not found. Automatically " f"created."
                )
            elif not exists:
