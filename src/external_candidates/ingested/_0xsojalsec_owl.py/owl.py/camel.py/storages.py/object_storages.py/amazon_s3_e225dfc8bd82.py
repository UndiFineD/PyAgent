# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\storages\object_storages\amazon_s3.py
        ...
        try:
            ...
            if error_code == "403":
                raise PermissionError(
                    f"Failed to access bucket {self._bucket_name}: " f"No permission."
                )
            elif error_code == "404":
                if self._create_if_not_exists:
                    ...
                    raise FileNotFoundError(
                        f"Failed to access bucket {self._bucket_name}: Not " f"found."
                    )
            else:
                raise e
