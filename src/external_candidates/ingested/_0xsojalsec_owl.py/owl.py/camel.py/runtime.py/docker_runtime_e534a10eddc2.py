# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-owl\owl\camel\runtime\docker_runtime.py
        ...
        for func in funcs:
            ...
            def wrapper(*args, func=func, redirect_stdout=redirect_stdout, **kwargs):
                ...
                if resp.status_code != 200:
                    logger.error(f"""ailed to execute function: 
                        {func.get_function_name()}, 
                        status code: {resp.status_code}, 
                        response: {resp.text}""")
                    return {"error": f"""Failed to execute function:
                        {func.get_function_name()}, 
                        response: {resp.text}"""}
                data = resp.json()
                if redirect_stdout:
