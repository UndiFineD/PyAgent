# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-redpill\lib\Sign-Executables\sigthief.py
#!/usr/bin/env python3
# LICENSE: BSD-3
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option(
        "-i", "--file", dest="inputfile", help="input file", metavar="FILE"
    )
    parser.add_option(
        "-r",
        "--rip",
    ...
    parser.add_option(
        "-t", "--target", dest="targetfile", help="file to append signature to"
    )
    parser.add_option(
        "-c",
        "--checksig",
