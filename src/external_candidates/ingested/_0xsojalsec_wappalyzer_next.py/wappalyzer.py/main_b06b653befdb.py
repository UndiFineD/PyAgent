# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wappalyzer-next\wappalyzer\__main__.py
import argparse
import queue
import re
import sys
...
def main():
    ...
    def process_urls(
        urls, num_threads=3, cookie=None, scan_type="full", should_print=False
    ):
        ...
        try:
            driver_pool = (
                DriverPool(size=min(num_threads, 3)) if scan_type == "full" else None
            )  # Limit max concurrent drivers
    ...
    try:
        if re.search(r"^https?://", args.input_file.lower()):
            should_print = not (
                args.json_output_file or args.csv_output_file or args.html_output_file
            )
            result = analyze(
                args.input_file, args.scan_type, args.thread_num, args.cookie
            )
            if should_print:
                pretty_print(result)
        else:
            try:
                ...
                should_print = not (
                    ...
                    or args.csv_output_file
                    or args.html_output_file
                )
                result = process_urls(
                    urls,
                    args.thread_num,
