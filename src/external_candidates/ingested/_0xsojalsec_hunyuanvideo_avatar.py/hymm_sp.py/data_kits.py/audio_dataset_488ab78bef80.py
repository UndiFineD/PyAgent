# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\data_kits\audio_dataset.py
import json
import math
import os
import random
...
class VideoAudioTextLoaderVal(Dataset):
    ...
    def get_batch_data(self, idx):
        ...
        prompt = (
            "Authentic, Realistic, Natural, High-quality, Lens-Fixed, "
            + meta_file["prompt"]
        )
        fps = meta_file["fps"]
        ...
        pixel_value_ref_llava = [
            self.llava_transform(to_pil(image)) for image in pixel_value_ref
        ]
        ...
        pixel_value_ref_clip = self.clip_image_processor(
            images=Image.fromarray(
                (pixel_value_ref[0].permute(1, 2, 0))
                .data.cpu()
                .numpy()
                .astype(np.uint8)
            ),
        ...
        batch = {
            "text_prompt": prompt,  #
            "videoid": videoid,
            "pixel_value_ref": pixel_value_ref.to(
                dtype=torch.float16
            ),  # 参考图，用于vae提特征 (1, 3, h, w), 取值范围(0, 255)
            "pixel_value_ref_llava": pixel_value_ref_llava.to(
                dtype=torch.float16
            ),  # 参考图，用于llava提特征 (1, 3, 336, 336), 取值范围 = CLIP取值范围
            "pixel_value_ref_clip": pixel_value_ref_clip.to(
                dtype=torch.float16
            ),  # 参考图，用于clip_image_encoder提特征 (1, 3, 244, 244), 取值范围 = CLIP取值范围
