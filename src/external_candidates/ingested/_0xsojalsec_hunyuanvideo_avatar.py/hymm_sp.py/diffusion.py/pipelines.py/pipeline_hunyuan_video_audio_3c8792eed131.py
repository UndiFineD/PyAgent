...
class HunyuanVideoAudioPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            text_inputs = text_encoder.text2tokens(
                prompt, data_type=data_type, name=name
            )
            ...
            if pixel_value_llava is not None:
                ...
                text_inputs["attention_mask"] = torch.cat(
                    [
                        ...
                        torch.ones((1, 575 * len(pixel_value_llava))).to(
                            text_inputs["attention_mask"]
                        ),
                    ],
                    dim=1,
            ...
            if uncond_pixel_value_llava is not None:
                ...
                uncond_input["attention_mask"] = torch.cat(
                    [
                        ...
                        torch.ones((1, 575 * len(uncond_pixel_value_llava))).to(
                            uncond_input["attention_mask"]
                        ),
                    ],
                    dim=1,
    ...
    def encode_prompt_audio_text_base(
        ...
        if prompt_embeds is None:
            ...
            if attention_mask is not None:
                ...
                attention_mask = attention_mask.view(
                    bs_embed * num_images_per_prompt, seq_len
                )
        ...
        if frame is not None:
            if "884" in vae_ver:
                if frame != 1 and (frame - 1) % 4 != 0:
                    raise ValueError(
                        f"`frame` has to be 1 or a multiple of 4 but is {frame}."
                    )
            elif "888" in vae_ver:
                if frame != 1 and (frame - 1) % 8 != 0:
                    raise ValueError(
                        f"`frame` has to be 1 or a multiple of 8 but is {frame}."
                    )
    ...
    def prepare_latents(
        ...
        if timestep is not None:
            init_latents = (
                ref_latents.clone().repeat(1, 1, frame, 1, 1).to(device).to(dtype)
            )
            latents = latents
        ...
        **kwargs,
        ...
        if self.do_classifier_free_guidance:
            ...
            if prompt_embeds_2 is not None:
                prompt_embeds_2_input = torch.cat(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for index_start in range(0, infer_length, frames_per_batch):
                    ...
                    if self.do_classifier_free_guidance:
                        if i < 10:
                            self._guidance_scale = (1 - i / len(timesteps)) * (
                                self.start_cfg_scale - 2
                            ) + 2
                    ...
                    with torch.autocast(
                        ...
                        if i in no_cache_steps:
                            ...
                            if (
                                ...
                                * latent_model_input.shape[-2]
                                ...
                                noise_pred = torch.cat(
                                    [noise_pred_uncond, noise_pred_text], dim=0
                                )
                                torch.cuda.empty_cache()
                            else:
                                additional_kwargs = {
                            ...
                            self.transformer.cache_out[:, : img_ref_len - img_len] = (
                                self.cache_tensor["ref"][:, idx_list][:, 0].clone()
                            )
                            self.transformer.cache_out[
                                :, img_ref_len - img_len : img_ref_len
                            ] = (
                                self.cache_tensor["img"][:, idx_list]
                                .reshape(-1, img_len, 3072)
                                .clone()
                            )
                            self.transformer.cache_out[:, img_ref_len:] = (
                                self.cache_tensor["txt"][:, idx_list][:, 0].clone()
                            )
