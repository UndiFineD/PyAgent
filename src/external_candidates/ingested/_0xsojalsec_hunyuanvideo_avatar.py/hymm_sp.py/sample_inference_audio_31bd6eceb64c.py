...
class HunyuanVideoSampler(Inference):
    ...
    @torch.no_grad()
    def predict(
        self, args, batch, wav2vec, feature_extractor, align_instance, **kwargs
    ):
        """
        ...
        audio_prompts = torch.cat(audio_prompts, dim=0).to(
        ...
        pixel_value_ref = batch["pixel_value_ref"].to(
            self.device
        ...
        with torch.autocast(
            ...
            uncond_ref_latents = self.vae.encode(
            ...
            if (
                ...
                ref_latents.sub_(self.vae.config.shift_factor).mul_(
                    self.vae.config.scaling_factor
                )
                uncond_ref_latents.sub_(self.vae.config.shift_factor).mul_(
                    self.vae.config.scaling_factor
                )
            else:
        ...
        freqs_cos, freqs_sin = self.get_rotary_pos_embed(
            target_length, target_height, target_width, concat_dict
        )
