...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.motion_exp = TimestepEmbedder(
            self.hidden_size // 4, get_activation_layer("silu"), **factory_kwargs
        )
    ...
    def forward(
        ...
        **additional_kwargs,
        ...
        motion_exp_vec = self.motion_exp(additional_kwargs["motion_exp"].view(-1)).view(
            x.shape[0], -1
        )  # (b, 3072)
        vec = vec + motion_exp_vec
        motion_pose_vec = self.motion_pose(
            additional_kwargs["motion_pose"].view(-1)
        ).view(
            x.shape[0], -1
        )  # (b, 3072)
        ...
        if additional_kwargs["face_mask"].shape[2] == 1:
            face_mask = additional_kwargs["face_mask"].repeat(
                1, 1, ot, 1, 1
            )  # repeat if number of mask frame is 1
        else:
        ...
        face_mask = (
            face_mask.view(-1, mask_len, 1).repeat(1, 1, img.shape[-1]).type_as(img)
        )
        ...
        if not is_cache:
            for layer_num, block in enumerate(self.double_blocks):
                ...
                if layer_num in self.double_stream_list:
                    ...
                    audio_feature_all_insert = torch.cat(
                        [audio_feature_pad, audio_feature_all], dim=1
                    ).view(bsz, ot, 16, 3072)

                    double_idx = self.double_stream_map[str(layer_num)]
                    real_img = self.audio_adapter_blocks[double_idx](
                        audio_feature_all_insert, real_img
                    ).view(bsz, -1, 3072)
            ...
            if len(self.single_blocks) > 0:
                for layer_num, block in enumerate(self.single_blocks):
                    if layer_num == (len(self.single_blocks) - 1):
                        ...
                        self.cache_out = torch.cat(
                            [tmp, x[:, -txt_seq_len:, ...]], dim=1
                        )
