# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\vae\autoencoder_kl_causal_3d.py
import math
import os
...
class AutoencoderKLCausal3D(ModelMixin, ConfigMixin, FromOriginalVAEMixin):
    ...
    def _decode(
        ...
        if self.use_trt_decoder:
            ...
            dec = self.trt_decoder_runner.infer(
                {"input": z.to(RECOMMENDED_DTYPE).contiguous()},
    ...
    def blend_v(
        ...
        weights = (
            torch.arange(blend_extent, device=a.device, dtype=a.dtype) / blend_extent
        )
        ...
        if self.parallel_decode:
            ...
            for i in range(0, z.shape[-2], overlap_size):
                for j in range(0, z.shape[-1], overlap_size):
                    ...
                    if self.use_padding and (
                        tile.shape[-2] < self.tile_latent_min_size
                        or tile.shape[-1] < self.tile_latent_min_size
                    ):
                        ...
                        padding = (
                            0,
                            self.tile_latent_min_size - tile.shape[-1],
                            0,
                            self.tile_latent_min_size - tile.shape[-2],
                            0,
                            0,
                        )
                        tile = F.pad(tile, padding, "replicate").to(
            ...
            tiles_curr_rank = tiles[
                rank * ratio : None if rank == world_size - 1 else (rank + 1) * ratio
            ]

            decoded_results = []

            total = len(tiles)
            n_task = [ratio] * (total // ratio) + (
                [total % ratio] if total % ratio else []
            )
            ...
            for i, tile in enumerate(tiles_curr_rank):
                ...
                def find(n):
                    return next(
                        (i for i, task_n in enumerate(n_task) if task_n < n),
                        len(n_task),
                    )
            ...
            if not self.nccl_gather:
                ...
                if self.gather_to_rank0:
                    ...
                    raise NotImplementedError(
                        "The old `allgather` implementation is deprecated for nccl plan."
                    )
                ...
                decoded_results = [
                    col[i]
                    for i in range(max([len(k) for k in gather_results]))
    ...
    def temporal_tiled_encode(
        ...
        assert (
            not self.disable_causal_conv
        ), "Temporal tiling is only compatible with causal convolutions."
    ...
    def temporal_tiled_decode(
        ...
        assert (
            not self.disable_causal_conv
        ), "Temporal tiling is only supported with causal convolutions."
        ...
        for i in range(0, T, overlap_size):
            ...
            if self.use_spatial_tiling and (
                ...
                decoded = self.decoder(tile)
            if i > 0 and (
                not (self.parallel_decode and self.gather_to_rank0) or rank == 0
            ):
                decoded = decoded[:, :, 1:, :, :]
            row.append(decoded)
        if (
            not CPU_OFFLOAD
            and not DISABLE_SP
            and self.parallel_decode
            and self.gather_to_rank0
            and rank != 0
        ):
            return DecoderOutput(sample=None)
        result_row = []
