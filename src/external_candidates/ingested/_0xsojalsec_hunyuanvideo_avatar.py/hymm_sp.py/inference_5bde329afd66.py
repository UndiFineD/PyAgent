# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\inference.py
from pathlib import Path

import torch
import torch.distributed
from hymm_sp.constants import PRECISION_TO_TYPE, PROMPT_TEMPLATE
...
class Inference(object):
    ...
    def from_pretrained(cls, pretrained_model_path, args, device=None, **kwargs):
        ...
        if args.text_encoder_2 is not None:
            text_encoder_2 = TextEncoder(
                ...
                logger=logger,
                device=(
                    "cpu" if args.cpu_offload else device
    ...
    def load_state_dict(args, model, ckpt_path):
        ...
        if load_key in state_dict:
            ...
            raise KeyError(
                f"Key '{load_key}' not found in the checkpoint. Existed keys: {state_dict.keys()}"
            )
    ...
    def get_exp_dir_and_ckpt_id(self):
        ...
        if ckpt.parents[1].name == "checkpoints":
            ...
            raise ValueError(
                f"We cannot infer the experiment directory from the checkpoint path: {ckpt}. "
