# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-HunyuanVideo-Avatar\hymm_sp\config.py
import argparse
...
def parse_args(namespace=None):
    parser = argparse.ArgumentParser(
        description="Hunyuan Multimodal training/inference script"
    )
...
def add_extra_models_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group(
        title="Extra Models (VAE, Text Encoder, Tokenizer)"
    )

    # VAE
    group.add_argument(
        "--vae", type=str, default="884-16c-hy0801", help="Name of the VAE model."
    )
    group.add_argument(
        "--vae-precision",
        type=str,
    ...
    group.add_argument(
        ...
        action="store_true",
        default=True,
        help="Enable tiling for the VAE model.",
    )
    group.add_argument(
        "--text-encoder",
        type=str,
...
def add_denoise_schedule_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        type=float,
        default=None,
        help="Shift factor for flow matching schedulers when using video data.",
    )
    group.add_argument(
        "--flow-reverse",
        action="store_true",
...
def add_evaluation_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--cpu-offload", action="store_true", help="Use CPU offload for the model load."
    )
    ...
    group.add_argument(
        ...
        type=str,
        default="",
        help="Negative prompt for sampling during evaluation.",
    )
    group.add_argument("--image-size", type=int, default=704)
    group.add_argument(
        "--pad-face-size", type=float, default=0.7, help="Pad bbox for face align."
    )
    ...
    group.add_argument(
        "--ip-cfg-scale", type=float, default=0, help="Classifier free guidance scale."
    )
