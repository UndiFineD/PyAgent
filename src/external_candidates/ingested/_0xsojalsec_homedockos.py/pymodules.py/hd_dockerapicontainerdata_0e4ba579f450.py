...
def get_docker_containers():
    ...
    try:
        ...
        config_lines = []
    ports_config = {
        sanitize_container_name(line.strip().split("*")[0]): line.strip()
        .split("*")[1]
        .split(":")
    ...
    for container in containers:
        ...
        compose_file_path = os.path.join(
            compose_upload_folder, f"{sanitize_container_name(container.name)}.yml"
        )
        ...
        if not is_valid_hostname(base_url_without_scheme_or_www):
            ...
            if container.name in ports_config:
                ...
                if ports_list and ports_list[0] not in ["hostmode", ""]:
                    ...
                    if sanitized_port:
                        service_url = (
                            f"//{base_url_without_scheme_or_www}/app/{sanitized_port}"
                        )
                    else:
                        service_url = None
                else:
