...
def get_download_data(interface_name):
    try:
        ...
        bytes_received_gb = round(
            interface_data.bytes_recv / (1024.0 * 1024.0 * 1024.0), 2
        )
...
def start_log_sample_network_usage():
    ...
    while True:
        ...
        if download_data["received"] is not None and upload_data["sent"] is not None:
            network_data_samples.append(
                (download_data["received"], upload_data["sent"])
            )
            sample_count += 1

        if sample_count == 100:
            if network_data_samples:
                avg_download_data = sum(x[0] for x in network_data_samples) / len(
                    network_data_samples
                )
                avg_upload_data = sum(x[1] for x in network_data_samples) / len(
                    network_data_samples
                )
