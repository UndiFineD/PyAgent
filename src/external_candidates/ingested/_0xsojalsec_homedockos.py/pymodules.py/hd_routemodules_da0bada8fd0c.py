...
def RouteAllModules(homedock_www, send_public_key):
    ...
    homedock_www.add_url_rule(
        "/login", "api_login", CSRF_Protect(api_login), methods=["POST"]
    )
    homedock_www.add_url_rule(
        "/api/pksend", "send_public_key", CSRF_Protect(send_public_key), methods=["GET"]
    )
    homedock_www.add_url_rule(
        "/api/pcrypt",
        "login_pwd_encrypt",
    ...
    homedock_www.add_url_rule(
        "/logout", "logout", CSRF_Protect(logout), methods=["POST"]
    )
    ...
    homedock_www.add_url_rule(
        "/api/get_files", "list_files", CSRF_Protect(list_files), methods=["GET"]
    )
    homedock_www.add_url_rule(
        "/api/upload_file", "upload_file", CSRF_Protect(upload_file), methods=["POST"]
    )
    homedock_www.add_url_rule(
        "/api/download_file",
        "download_file",
    ...
    homedock_www.add_url_rule(
        "/api/delete_file", "delete_file", CSRF_Protect(delete_file), methods=["POST"]
    )

    from pymodules.hd_HMRUpdate import check_update, update_now

    homedock_www.add_url_rule(
        "/api/check_update", "check_update", CSRF_Protect(check_update), methods=["GET"]
    )
    homedock_www.add_url_rule(
        "/api/update_now", "update_now", CSRF_Protect(update_now), methods=["POST"]
    )
    ...
    homedock_www.add_url_rule(
        "/app/<int:port>/<path:subpath>", "app_loader_subpath", app_loader
    )
    homedock_www.add_url_rule(
        "/api/check-port", "check_port", CSRF_Protect(check_port), methods=["POST"]
    )
    ...
    homedock_www.add_url_rule(
        "/images/<path:path>", "send_static_images", send_static_images
    )
    homedock_www.add_url_rule(
        "/favicon/<path:path>", "send_static_favicon", send_static_favicon
    )
    homedock_www.add_url_rule(
        "/audio/<path:path>", "send_static_audio", send_static_audio
    )
    homedock_www.add_url_rule(
        "/homedock-ui/vue3/static/<path:path>", "send_src_static", send_src_static
    )
    homedock_www.add_url_rule(
        "/homedock-ui/vue3/dist/<path:path>", "send_src_dist", send_src_dist
    )
    ...
    homedock_www.add_url_rule(
        "/thread/update_cpu_temp", "homedock_cpu_temp", CSRF_Protect(homedock_cpu_temp)
    )
    homedock_www.add_url_rule(
    ...
    homedock_www.add_url_rule(
        "/thread/downloaded_data", "downloaded_data", CSRF_Protect(downloaded_data)
    )
    homedock_www.add_url_rule(
        "/thread/uploaded_data", "uploaded_data", CSRF_Protect(uploaded_data)
    )

    from pymodules.hd_UIDashboardThreads import active_containers, get_containers

    homedock_www.add_url_rule(
        "/thread/installed_containers", "get_containers", CSRF_Protect(get_containers)
    )
    homedock_www.add_url_rule(
    ...
    homedock_www.add_url_rule(
        "/thread/system_uptime", "get_uptime", CSRF_Protect(get_uptime)
    )
    homedock_www.add_url_rule(
        "/thread/homedock_uptime", "homedock_uptime", CSRF_Protect(homedock_uptime)
    )
    ...
    homedock_www.add_url_rule(
        "/api/containers", "get_docker_containers", CSRF_Protect(get_docker_containers)
    )
    homedock_www.add_url_rule(
