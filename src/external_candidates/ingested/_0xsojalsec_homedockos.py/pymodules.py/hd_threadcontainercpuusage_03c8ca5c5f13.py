...
def update_cpu_usage():
    ...
    while True:
        ...
        current_container_names = set(
            container.name for container in current_containers
        )

        inactive_containers = set(cpu_usage.keys()) - current_container_names
        for container_name in inactive_containers:
            print(
                f" + THREAD: Container {container_name} was stopped, cleaning up CPU usage entry"
            )
        ...
        for container in current_containers:
            try:
                ...
                ) - float(stats["precpu_stats"]["cpu_usage"].get("total_usage", 0))
                system_delta = float(stats["cpu_stats"]["system_cpu_usage"]) - float(
                    stats["precpu_stats"].get("system_cpu_usage", 0)
                )

                cpu_usage[container.name] = (
                    round(cpu_delta / system_delta * 100.0, 1)
                    if system_delta > 0.0
                    else 0.0
                )
