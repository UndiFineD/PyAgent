...
def serve_logins():
    ...
    try:
        with open(
            os.path.join(current_directory, "logs", "loginattempts.log"), "r"
        ) as file:
...
def serve_network_usage():
    ...
    try:
        with open(
            os.path.join(current_directory, "logs", "networkusage.log"), "r"
        ) as file:
            for line in file.readlines():
                parts = line.strip().split("*")
                if len(parts) == 3:
...
def serve_disk_usage():
    ...
    try:
        with open(
            os.path.join(current_directory, "logs", "diskusage.log"), "r"
        ) as file:
            for line in file.readlines():
                parts = line.strip().split("*")
                if len(parts) == 2:
                    ...
                    disk_usages.append(
                        {"timestamp": timestamp, "avg": float(avg_usage)}
                    )
                else:
                    print(f"Unexpected line format: {line}")
...
def serve_external_disk_usage():
    ...
    try:
        ...
        with open(
            os.path.join(current_directory, "logs", "externaldiskusage.log"), "r"
        ) as file:
            for line in file.readlines():
