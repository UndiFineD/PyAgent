# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-malwoverview\malwoverview\modules\malshare.py
import json
import os
import sys
...
class MalshareExtractor:
    ...
    def requestMALSHAREAPI(self):
        if self.MALSHAREAPI == "":
            print(
                ...
                + "\nTo be able to get/submit information from/to Malshare, you must create the .malwapi.conf file under your user home directory (on Linux is $HOME\\.malwapi.conf and on Windows is in C:\\Users\\[username]\\.malwapi.conf) and insert the Malshare API according to the format shown on the Github website."
                + mycolors.reset
                + "\n"
            )
            exit(1)
    ...
    def malsharedown(self, filehash):
        ...
        try:
            ...
            finalurl3 = "".join(
                [urlmalshare, self.MALSHAREAPI, "&action=getfile&hash=", resource]
            )
            ...
            if cv.bkg == 1:
                print(
                    (
                        mycolors.foreground.lightred
                        + "Error while connecting to Malshare.com!\n"
                    )
                )
            else:
                print(
    ...
    def malsharelastlist(self, typex):
        ...
        try:
            if filetype != "all":
                ...
                finalurl = "".join(
                    [urlmalshare, self.MALSHAREAPI, "&action=type&type=", filetype]
                )
