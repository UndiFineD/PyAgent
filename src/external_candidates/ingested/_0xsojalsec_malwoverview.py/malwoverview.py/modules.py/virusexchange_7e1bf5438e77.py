# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-malwoverview\malwoverview\modules\virusexchange.py
import os
...
class VirusExchangeExtractor:
    ...
    def _requireVXAPI(self):
        if self.VXAPI == "":
            printc(
                "\nTo be able to get/submit information from/to Virus Exchange, you must create the .malwapi.conf file under your user home directory (on Linux is $HOME\\.malwapi.conf and on Windows is in C:\\Users\\[username]\\.malwapi.conf) and insert the Virus Exchange API according to the format shown on the Github website.",
    ...
    def check_hash(self, sha256):
        ...
        metadata_to_show = [
            {
                "key": "md5",
                "name": "MD5",
                "colors": (mycolors.foreground.cyan, mycolors.foreground.lightcyan),
                "pad": 12,
            },
            {
                "key": "sha1",
            ...
            },
            {
                "key": "size",
                "name": "Size",
                "colors": (mycolors.foreground.purple, mycolors.foreground.yellow),
                "pad": 12,
            },
            {
                "key": "tags",
        ...
        try:
            ...
            printc(
                f"[-] Error checking hash metadata: {str(e)}",
                mycolors.foreground.error(cv.bkg),
            )
            return
            ...
            if response.status_code == 200:
                ...
                if download_link:
                    ...
                    if sample_response.status_code == 200:
                        ...
                        printc(
                            f"Sample downloaded to: {output_path}",
                            mycolors.foreground.success(cv.bkg),
                        )
                    else:
                        printc(
                            f"Failed to download sample: {sample_response.status_code}",
                            mycolors.foreground.error(cv.bkg),
                        )
                else:
                    printc(
                        "No download link available in metadata",
                        mycolors.foreground.error(cv.bkg),
                    )
            else:
                error_detail = metadata.get("errors", {}).get("detail", "Unknown error")
                printc(
                    f"Failed to fetch sample metadata: {error_detail}",
                    mycolors.foreground.error(cv.bkg),
                )
        except Exception as e:
            printc(
                f"Error downloading sample: {str(e)}", mycolors.foreground.error(cv.bkg)
            )
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                response = requests.post(
                    url, headers={"Authorization": f"Bearer {self.VXAPI}"}, files=files
                )

            if response.status_code == 200:
                printc(
                    "Sample uploaded successfully", mycolors.foreground.success(cv.bkg)
                )
            else:
                printc(
                    f"Failed to upload sample: {response.status_code}",
                    mycolors.foreground.error(cv.bkg),
                )
        except Exception as e:
            printc(
                f"Error uploading sample: {str(e)}", mycolors.foreground.error(cv.bkg)
            )
