# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-malwoverview\malwoverview\modules\multipleip.py
import malwoverview.modules.configvars as cv
from malwoverview.utils.colors import mycolors, printc, printr
...
class MultipleIPExtractor:
    ...
    def _get_info_alienvault(self, data):
        try:
            ...
            print(
                infocolor
                + f"Continent:".ljust(COLSIZE)
                + "\t"
                + mycolors.reset
                + str(data.get("continent_code"))
            )
            print(
                infocolor
                + f"Latitude:".ljust(COLSIZE)
                + "\t"
                + mycolors.reset
            ...
            print(
                mycolors.foreground.error(cv.bkg)
                + f"Pulses Found:".ljust(COLSIZE)
                + "\t"
                + mycolors.reset
                + str(data.get("pulse_info", {}).get("count"))
            )
