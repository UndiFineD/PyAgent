# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-VeriFree\utils\data.py
    ...
    def __init__(
        ...
        **_,
        ...
        for i in prograss_bar:
            ...
            gt_trajectory_ids = (
                list(buffer[i].prompt_ids)
                + list(buffer[i].think_ids)
                + list(buffer[i].answer_ids)
            )
            traj_dict = {
            ...
            traj_dict["gt_action_logprobs"] = (
                torch.ones_like(torch.tensor(traj_dict["action_logprobs"])) * 99999.0
            )  # placeholder
