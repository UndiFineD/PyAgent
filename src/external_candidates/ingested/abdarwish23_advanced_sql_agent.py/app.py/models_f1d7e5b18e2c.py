# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\models.py
# app/models.py
...
class SQLValidationResult(BaseModel):
    is_sql_valid: bool = Field(
        description="Whether the SQL query is valid and safe to execute"
    )
    issues: List[str] = Field(
        default_factory=list, description="List of identified issues with the SQL query"
    )
    suggested_fix: str = Field(
        default="", description="Suggested fix for the SQL query if issues are found"
    )
...
class EvaluationResult(BaseModel):
    is_result_relevant: bool = Field(
        description="Whether the results are relevant to the original query"
    )
    explanation: str = Field(description="Explanation of the evaluation")
    requires_visualization: bool = Field(
        description="Whether the results would benefit from visualization"
    )
...
class SQLCorrectionResult(BaseModel):
    ...
    identified_issues: str = Field(
        default="No issues identified",
        description="List of specific issues identified in the current SQL query",
    )
