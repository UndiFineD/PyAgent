...
def create_visualization(
    ...
    if visualization_type == "bar":
        ...
        plt.plot(
            df[x_column], df[y_column], alpha=0
        )  # This creates anchor points for the spline
        if is_time_series:
    ...
    plt.title(
        title or f"Visualization for: {query}", fontsize=16, fontweight="bold", pad=20
    )
    plt.tight_layout()
...
def data_visualizer(state: AgentState) -> AgentState:
    ...
    if (
        not state["execution_result"].success
        or not state["evaluation_result"].requires_visualization
    ):
