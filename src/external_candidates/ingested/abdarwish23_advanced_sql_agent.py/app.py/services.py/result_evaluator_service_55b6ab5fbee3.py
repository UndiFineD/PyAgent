# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\result_evaluator_service.py
...
def result_evaluator(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not state["is_result_relevant"]:
            state["reflection"] = {
                ...
                "suggestion": parsed_response.get(
    ...
    logger.info(
        f"Requires visualization: {state['evaluation_result'].requires_visualization}"
    )
