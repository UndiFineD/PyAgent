# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\sql_generator_service.py
# app/services/sql_generator_service.py

import json
import logging

from app.models import AgentState, GeneratedSQL, ReflectedGeneratedSQL
...
def sql_generator_optimizer(state: AgentState) -> AgentState:
    ...
    memories_text = "\n".join(
        [
            f"Memory {i+1}: {memory.page_content}"
    ...
    selected_table_info = {
        name: info.table_schema.dict()
        for name, info in state["db_info"].items()
    ...
    try:
        ...
        state["generated_sql"] = GeneratedSQL(
            sql_query=sql_query, explanation=parsed_response.get("explanation", "")
        )
...
def sql_generator_optimizer_reflection(state: AgentState) -> AgentState:
    ...
    try:
        ...
        state["reflected_generated_sql"] = ReflectedGeneratedSQL(
            reflected_sql_query=parsed_response.get(
                "sql_query", "SELECT 'Error: Failed to generate SQL query'"
            ),
            reflected_explanation=parsed_response.get(
    ...
    logger.info(
        f"Reflected Generated SQL: {state['reflected_generated_sql'].reflected_sql_query}"
    )
    logger.info(
        f"Reflected explanation: {state['reflected_generated_sql'].reflected_explanation}"
    )
