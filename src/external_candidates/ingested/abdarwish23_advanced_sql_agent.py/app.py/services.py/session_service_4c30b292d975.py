# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\session_service.py
# app/services/session_service.py

import logging
import uuid

from app.models import SessionHistory, db
...
class SessionService:
    ...
    def get_session_history(session_id, limit=10):
        ...
        try:
            return (
                db.session.query(SessionHistory)
                .filter(SessionHistory.session_id == session_id)
    ...
    def add_to_session_history(session_id, run_id, query, response):
        ...
        try:
            new_history = SessionHistory(
                session_id=session_id, run_id=run_id, query=query, response=response
            )
    ...
    def get_recent_history(session_id, limit=5):
        if not session_id:
            ...
            return []
        try:
            recent_history = SessionService.get_session_history(
