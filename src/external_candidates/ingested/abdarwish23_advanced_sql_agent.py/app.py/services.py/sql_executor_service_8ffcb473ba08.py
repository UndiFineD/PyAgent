# Extracted from: C:\DEV\PyAgent\.external\abdarwish23-Advanced_SQL_Agent\app\services\sql_executor_service.py
# app/services/sql_executor_service.py

import pandas as pd
from app.config import Config
from app.models import AgentState, SQLCorrectionResult, SQLExecutionResult
...
def execute_sql(state: AgentState) -> AgentState:
    ...
    try:
        ...
        state["execution_result"] = SQLExecutionResult(
    ...
    try:
        with engine.connect() as connection:
            result = connection.execute(
                text(state["reflected_generated_sql"].reflected_sql_query)
            )
            df = pd.DataFrame(result.fetchall(), columns=result.keys())
        state["execution_result"] = SQLExecutionResult(
            success=True, data=df.to_dict(orient="records")
...
def execute_sql_corrected(state: AgentState) -> AgentState:
    ...
    try:
        with engine.connect() as connection:
            result = connection.execute(
                text(state["sql_correction"].corrected_sql_query)
            )
            df = pd.DataFrame(result.fetchall(), columns=result.keys())
        state["execution_result"] = SQLExecutionResult(
            success=True, data=df.to_dict(orient="records")
