...
def should_reflect_result(state: AgentState) -> str:
    return (
        "visualization_check" if state["is_result_relevant"] else "sql_corrected"
    )  # "sql_reflector"
...
def should_regenerate_sql(state: AgentState) -> str:
    if (
        ...
        or not state["validation_result"].is_sql_valid
    ) and len(state.get("regenerate_list", [])) < 3:
        append_to_list(state, "regenerate_list")
        logger.info(
            f"Regenerating SQL. New attempt count: {len(state['regenerate_list'])}"
        )
        return "generator"
...
def save_graph_visualization(graph: StateGraph):
    try:
        ...
        response = requests.post(
            "https://mermaid.ink/img", json={"mermaid": mermaid_string}
        )
        ...
        logger.info(
            f"Graph visualization completed in {end_time - start_time:.2f} seconds"
        )
