# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\client\commands\get_s3_buckets.py
import json
...
def list_aws_s3_buckets(apihost, jwt_token):
    try:
        buckets = json.loads(
            requests.get(
                "{}/api/latest/awsbuckets".format(apihost),
...
def get_aws_s3_bucket(apihost, bucket, jwt_token):
    try:
        bucket_dict = json.loads(
            requests.post(
                "{}/api/latest/awsbuckets".format(apihost),
                json={"aws_s3_bucket_name": bucket},
                headers={"Authorization": "Bearer {}".format(jwt_token)},
            ).text
        )
