# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\teamserver.py
import argparse
import json
...
from core.database.models import Cosmonaut, WebsocketListener
...
parser.add_argument(
    ...
    "--apiPort",
    type=int,
    help="The API Server Port. (Default: 5000)",
    default=5000,
)
parser.add_argument(
    "-dH",
    "--databaseHost",
    type=str,
    help="The MongoDB Database Server Host. (Default: localhost)",
    default="localhost",
)
parser.add_argument(
    "-dP",
    "--databasePort",
    type=int,
    help="The MongoDB Database Server Port. (Default: 27017)",
    default=27017,
)
parser.add_argument(
    "-dn", "--databaseName", type=str, help="The MongoDB Database Name. (Required)"
)
parser.add_argument(
    "-du", "--databaseUser", type=str, help="The MongoDB Database User Name."
)
parser.add_argument(
    "-dp", "--databasePassword", type=str, help="The MongoDB Database Password."
)
parser.add_argument(
    "-p", "--password", type=str, help="The password for user 'cosmonaut'. (Required)"
)
parser.add_argument(
    "-y",
    "--yes",
    action="store_true",
    help="If this is set, the check for the database port will be skipped.",
)
...
for cloud in clouds:
    ...
    for module in show:
        arr = os.listdir(
            f"{pathlib.Path(__file__).parent.resolve()}/core/module/{module}"
        )
        for x in arr:
            if "__" in x:
                continue
            ...
            for bline in botocoresessionfilelines:
                if "return base" in bline:
                    ...
                    botocoresessiontemp.write(
                        f'\t\tif os.path.exists(f"{useragentfile}"):\n'.expandtabs(4)
                    )
                    botocoresessiontemp.write(
                        f'\t\t\twith open(f"{useragentfile}") as uafile:\n'.expandtabs(
                            4
                        )
                    )
                    botocoresessiontemp.write(
                        '\t\t\t\tbase = uafile.read().replace("\\n", "").strip()\n'.expandtabs(
                            4
                        )
                    )
                    botocoresessiontemp.write("\n")
...
try:
    ...
    if result_of_check == 0 and not args.yes:
        mongo_instance_verify = input(
            colored(
                "[*] Port is busy. Is a MongoDB instance running there? [y/N] ", "red"
            )
        )
        if (
            not mongo_instance_verify.strip().replace("\n", "") == "y"
            and not mongo_instance_verify.strip().replace("\n", "") == "Y"
        ):
    ...
    print(
        "{} '{}'".format(
            colored("[*] JWT Secret Key set to:", "green"), colored(jwt_token, "blue")
        )
    )
    ...
    print(
        "{} '{}'".format(
            colored("[*] Database set to:", "green"), colored(database, "blue")
        )
    )
    ...
    try:
        body = {"cosmonaut_name": "cosmonaut", "cosmonaut_pass": password}
            ...
            for wslistener in websocket_listeners:
                if wslistener["listener_status"] == "running":
                    ...
                    try:
                        ...
                        print(
                            colored(
                                "[*] Listener '{}' started on host '{}' and port '{}'".format(
                        ...
                        try:
                            WebsocketListener.objects.get_or_404(
                                listener_name=wslistener["listener_name"]
                            ).update(**wsbody)
                            ...
                            print(
                                colored(
                                    "[*] Listener '{}' on host '{}' and port '{}' did not start".format(
                                        wslistener["listener_name"],
                                        wslistener["listener_host"],
                                        str(wslistener["listener_port"]),
                                    ),
