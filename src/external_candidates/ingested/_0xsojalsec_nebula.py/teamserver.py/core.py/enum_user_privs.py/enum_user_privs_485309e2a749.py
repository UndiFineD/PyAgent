# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\enum_user_privs\enum_user_privs.py
import json
import sys
...
def enum_privs(profile_dict, workspace):
    full_perm = {}
    list_of_perms = {}
    read_perm = {"List": {}, "Describe": {}}
    read_perm2 = {"List": [], "Describe": []}
    list_perm = []
    desc_perm = []
    ...
    for service in SERVICES:
        ...
        if len(list_perm) > 0:
            ...
            for lmeth in list_perm:
                try:
                    ...
                    print(
                        colored(
                            "[*] Stopping. It might take a while. Please wait.", "green"
                        )
                    )
                    now = datetime.now()
                    ...
                    print(
                        colored(
                            "[*] Output of the allowed functions is saved to '{}'".format(
                                filename
                            ),
                            "green",
                        )
                    ...
                    print(
                        colored(
                            "[*] The list of the allowed functions is saved to '{}'".format(
                                filename2
                            ),
                            "green",
                        )
        ...
        if len(desc_perm) > 0:
            print(
                colored(
                    "[*] Trying the 'Describe' functions:", "yellow", attrs=["bold"]
                )
            )
