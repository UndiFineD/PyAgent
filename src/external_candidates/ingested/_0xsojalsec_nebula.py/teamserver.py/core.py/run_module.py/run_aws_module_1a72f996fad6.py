# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\run_module\run_aws_module.py
import os
import sys
...
def enter_credentials(
    service, access_key_id, secret_key, region, ua, proxy_definitions
):
    args = {
...
def enter_credentials_with_session_token(
    service, access_key_id, secret_key, region, session_token, ua, proxy_definitions
):
    args = {
        "service_name": service,
        "region_name": region,
...
def run_aws_module(
    imported_module, all_sessions, cred_prof, useragent, web_proxies, workspace
):
    sig = signature(imported_module.exploit)

    if len(sig.parameters) == 1 or len(sig.parameters) == 2:
        return run_aws_module_one_service(
            imported_module, all_sessions, cred_prof, useragent, web_proxies, workspace
        )
    elif len(sig.parameters) == 5:
        return imported_module.exploit(
            all_sessions, cred_prof, useragent, web_proxies, workspace
        )


def run_aws_module_one_service(
    imported_module, all_sessions, cred_prof, useragent, web_proxies, workspace
):
    ...
    if useragent != "":
        if os.path.exists(
            f"{os.environ.get('VIRTUAL_ENV')}/lib/python3.10/site-packages/botocore/session.py"
        ):
            useragentfile = f"{os.environ.get('VIRTUAL_ENV')}/lib/python3.10/site-packages/botocore/.user-agent"
        elif os.path.exists(
            f"/usr/lib/python{sys.version.split('.')[0]}.{sys.version.split('.')[1]}/dist-packages/botocore/session.py"
        ):
            useragentfile = f"/usr/lib/python{sys.version.split('.')[0]}.{sys.version.split('.')[1]}/dist-packages/botocore/.user-agent"
        elif os.path.exists(
            f"/usr/local/lib/python3.10/site-packages/botocore/session.py"
        ):
            useragentfile = (
                f"/usr/local/lib/python3.10/site-packages/botocore/.user-agent"
            )
        else:
            useragentfile = None
        ...
        if cred_prof == "":
            ...
            if c == 0:
                ...
                if not "session_token" in sess:
                    ...
                    profile_v = enter_credentials_with_session_token(
                        service,
                        sess["access_key_id"],
                        sess["secret_key"],
                        sess["region"],
