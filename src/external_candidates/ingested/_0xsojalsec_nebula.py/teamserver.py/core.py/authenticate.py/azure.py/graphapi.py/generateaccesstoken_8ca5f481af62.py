# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\Authenticate\Azure\GraphAPI\GenerateAccessToken.py
import os
import sys
import webbrowser

import requests
from simulib.Azure.Authentication.Authentication import (
...
def generateGraphClientSecret(client_secret, client_id, tenant):
    ...
    if "error" in graphTokenData:
        printOutput(
            "Lucr3 AzureAD Authentication",
            f"Error authenticating: {graphTokenData['error_description']}",
            "error",
        )
        return None
    else:
