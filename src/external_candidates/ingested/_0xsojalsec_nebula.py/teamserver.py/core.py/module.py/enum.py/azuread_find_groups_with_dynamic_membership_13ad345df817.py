# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\azuread_find_groups_with_dynamic_membership.py
import datetime
import json
import random
import string
    ...
    try:
        ...
        for group in groupInfo["value"]:
            if group["membershipRule"] is not None:
                ...
                groupUsersReq = json.loads(
                    requests.get(
                        f"https://graph.microsoft.com/beta/groups/{groupID}/members",
                        headers={
                            "Content-Type": "application/json",
                            "Authorization": "Bearer {}".format(access_token),
                        },
                ...
                groupAttachedRolesReq = json.loads(
                    requests.get(
                        f"https://graph.microsoft.com/beta/rolemanagement/directory/transitiveRoleAssignments?$filter=principalId eq '{groupID}'",
                        headers={
                            "Content-Type": "application/json",
                            "Authorization": "Bearer {}".format(access_token),
                ...
                while "@odata.nextLink" in groupAttachedRoles:
                    ...
                    if "error" in groupAttachedRolesReq:
                        ...
                        groupAttachedRoles["value"].extend(
                            groupAttachedRolesReq["value"]
                        )
