# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\azuread_enum_admin_users.py
import datetime
import json
import random
import string
        ...
        if "error" in userInfo:
            ...
            for user in userInfo["value"]:
                if (
                    ...
                    or "admin." in user["userPrincipalName"]
                    or "adm-" in user["userPrincipalName"]
                    or ".admin" in user["userPrincipalName"]
                    or ".adm" in user["userPrincipalName"]
                    or "-admin" in user["userPrincipalName"]
                    or "-adm" in user["userPrincipalName"]
                    ...
                    userGroups = json.loads(
                        requests.get(
                            f"https://graph.microsoft.com/beta/users/{userID}/memberOf",
                            headers={
                                "Content-Type": "application/json",
                                "Authorization": "Bearer {}".format(access_token),
                            },
                    ...
                    userAttachedRoles = json.loads(
                        requests.get(
                            f"https://graph.microsoft.com/beta/rolemanagement/directory/transitiveRoleAssignments?$filter=principalId eq '{userID}'",
                            headers={
                                "Content-Type": "application/json",
                                "Authorization": "Bearer {}".format(access_token),
