    ...
    if infoRetrieved.status_code == 200:
        if "registry" in infoRetrieved.json():
            if infoRetrieved.json()["registry"] is not None:
                ...
                repoinfoRetrieved = runDOAPICalls(
                    token, "GET", f"registry/{registryInfo['name']}/repositoriesV2"
                )
                if repoinfoRetrieved.status_code == 200:
                    if "repositories" in repoinfoRetrieved.json():
                        registryInfo["repos"] = repoinfoRetrieved.json()

                dockerCreds = runDOAPICalls(
                    token, "GET", f"registry/docker-credentials"
                )
                ...
                registrysubscription = runDOAPICalls(
                    token, "GET", f"registry/subscription"
                )
            ...
            if domainInfo is not None:
                if len(domainInfo) > 0:
                    for domain in domainInfo:
                        ...
                        domain["records"] = []
                        recordinforetrieved = runDOAPICalls(
                            token, "GET", f"domains/{domainname}/records"
                        )
                        if recordinforetrieved.status_code == 200:
                            domain["records"] = recordinforetrieved.json()[
                                "domain_records"
                            ]
                return domainInfo
            return []
                ...
                if len(functionInfo) > 0:
                    for function in functionInfo:
                        ...
                        if triggerinfoRetrieved.status_code == 200:
                            function["triggers"] = triggerinfoRetrieved.json()[
                                "triggers"
                            ]
