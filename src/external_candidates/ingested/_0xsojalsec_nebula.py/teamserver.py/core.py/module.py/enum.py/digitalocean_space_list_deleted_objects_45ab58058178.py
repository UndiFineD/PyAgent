# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\digitalocean_space_list_deleted_objects.py
import json
import sys
...
def exploit(profile):
    ...
    try:
        ...
        if "DeleteMarkers" in all_versions:
            ...
            try:
                do_user = DigitalOceanSpace.objects.get(
                    digitalocean_s3_space_name=bucket
                )
            ...
            return {"Bucket": return_dict}, 200
        else:
            return {
                "Bucket": {"Bucket": bucket, "Message": "No deleted files in space"}
            }
    except Exception as e:
        return {"error": "Error from module: {}".format(str(e))}, 500
