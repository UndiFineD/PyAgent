# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\aws_iam_enum_iam.py
import sys

from mongoengine import DoesNotExist

from core.database.models import AWSGroups, AWSPolicies, AWSRoles, AWSUsers
...
aws_command = (
    "aws iam get-account-authorization-details  --region <region> --profile <profile>"
)
...
def exploit(profile, workspace):
    ...
    try:
        ...
        if filter is None:
            ...
            result = profile.get_account_authorization_details(**args)
            iamData[iamDataFilter].extend(result[iamDataFilter])
            while result["IsTruncated"]:
                args = {"Filter": [filter], "Marker": result["Marker"]}
                result = profile.get_account_authorization_details(**args)
        ...
        if resourceType == "ALL":
            ...
            return {
                "ResourceType": {
                    ...
                    iamDataFilter: iamData[iamDataFilter],
                }
            }
    ...
    if len(iamData["UserDetailList"]) > 0:
        for user in iamData["UserDetailList"]:
            ...
            if "AttachedManagedPolicies" in user:
                database_data["aws_user_attached_policies"] = user[
    ...
    if len(iamData["GroupDetailList"]) > 0:
        for group in iamData["GroupDetailList"]:
            ...
            if "AttachedManagedPolicies" in group:
                database_data["aws_group_attached_policies"] = group[
                    "AttachedManagedPolicies"
                ]
    ...
    if len(iamData["RoleDetailList"]) > 0:
        for role in iamData["RoleDetailList"]:
            ...
            if "AttachedManagedPolicies" in role:
                database_data["aws_role_attached_policies"] = role[
                    "AttachedManagedPolicies"
                ]
            ...
            if "AssumeRolePolicyDocument" in role:
                database_data["aws_role_assume_role_policy"] = role[
    ...
    if len(iamData["Policies"]) > 0:
        for policy in iamData["Policies"]:
            ...
            if "PermissionsBoundaryUsageCount" in policy:
                database_data["aws_policy_permission_boundary_usage_count"] = policy[
            ...
            try:
                aws_policy = AWSPolicies.objects.get(
                    aws_policy_name=policy["PolicyName"]
                )
...
def otherGetUsers(profile, resourceType):
    ...
    for user in userlist:
        try:
            userinfo = profile.get_user(User=user["UserName"])["User"]

        except Exception as e:
            pass
        try:
            userGroups = profile.list_groups_for_user(User=user["UserName"])["Groups"]
        ...
        try:
            userInlinePolicies = profile.list_user_policies(User=user["UserName"])[
                "PolicyNames"
            ]
...
def otherGetGroups(profile, resourceType):
    ...
    for group in groupList:
        try:
            groupInfo = profile.get_group(GroupName=group["GroupName"])

        except Exception as e:
            pass

        try:
            groupUsers = profile.list_group_users(GroupName=group["GroupName"])
        ...
        try:
            groupPolicies = profile.list_group_policies(GroupName=group["GroupName"])[
                "PolicyNames"
            ]
...
def otherGetRoles(profile, resourceType):
    ...
    for role in iamData:
        try:
            roleInlinePolicies = profile.list_user_policies(User=role["RoleName"])[
                "PolicyNames"
            ]
...
def otherGetLocalPolicies(profile, resourceType):
    ...
    try:
        ...
        for policy in policyList:
            ...
            try:
                policyInfo = profile.get_policy_version(
        ...
        return {
            "ResourceType": {"ResourceType": resourceType, "LocalPolicies": iamData}
        }
...
def otherGetIAM(profile, resourceType):
    if resourceType == "USERS":
        ...
        iamData = {
            "UserDataList": otherGetUsers(profile=profile, resourceType="USERS")[
                "ResourceType"
            ]["UserDetailList"],
            "GroupDataList": otherGetUsers(profile=profile, resourceType="GROUPS")[
                "ResourceType"
            ]["GroupDetailList"],
            "RoleDataList": otherGetUsers(profile=profile, resourceType="ROLES")[
                "ResourceType"
            ]["RoleDetailList"],
            "LocalPolicies": otherGetUsers(
                profile=profile, resourceType="LOCALPOLICIES"
            )["ResourceType"]["LocalPolicies"],
            "AWSPolicies": otherGetUsers(profile=profile, resourceType="AWSPOLICIES")[
                "ResourceType"
            ]["AWSPolicies"],
        }
