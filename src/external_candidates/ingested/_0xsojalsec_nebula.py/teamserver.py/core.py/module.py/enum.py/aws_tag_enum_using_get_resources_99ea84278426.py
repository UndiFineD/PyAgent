# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\enum\aws_tag_enum_using_get_resources.py
import json
import re
import sys
import time
...
calls = ["tag:GetResources"]

aws_command = (
    "aws resourcegroupstaggingapi get_resources --region <region> --profile <profile>"
)
...
def filterResourcesByServiceAccountAndRegion(resource):
    ...
    return [service, region, accountID, resourceType]
...
def filterTagsAndResources(resourceArn, tags):
    ...
    for tag in tags:
        if tag["Key"] == "aws:ssmmessages:target-id":
            ...
            tagFilter.append(
                f"Other tag attached to resource: {tag['Key']}:{tag['value']}"
            )
    return {"ResourceARN": resourceArn, "TagsFilter": tagFilter}
    ...
    try:
        ...
        try:
            ...
            for resource in resources:
                resourceInfo = filterResourcesByServiceAccountAndRegion(
                ...
                if len(returnList) == 0:
                    ...
                    if resourceInfo[2] in returnList:
                        if resourceInfo[1] in returnList[resourceInfo[2]]:
                            if (
                                resourceInfo[0]
                                in returnList[resourceInfo[2]][resourceInfo[1]]
                            ):
                                returnList[resourceInfo[2]][resourceInfo[1]][
                                    resourceInfo[0]
                                ].append({resource["ResourceARN"]: resource["Tags"]})
                            else:
                                returnList[resourceInfo[2]][resourceInfo[1]][
                                    resourceInfo[0]
                                ] = [{resource["ResourceARN"]: resource["Tags"]}]
