...
def exploit(profile):
    ...
    for bucket in all_buckets:
        ...
        try:
            bucketLifeCycleConfig = profile.get_bucket_lifecycle_configuration(
                Bucket=bucket["Name"]
            )
        ...
        try:
            bucketNotificationConfiguration = (
                profile.list_bucket_metrics_configurations(Bucket=bucket["Name"])
            )
        ...
        try:
            bucketOwnership = profile.get_bucket_ownership_controls(
                Bucket=bucket["Name"]
            )
            ...
            if bucketOwnership["OwnershipControls"] == {}:
                ...
                bucket["Ownership"] = bucketOwnership["OwnershipControls"]["Rules"][
                    "ObjectOwnership"
                ]
        except botocore.exceptions.ClientError:
            bucket["Ownership"] = None
        ...
        try:
            bucketObjectLockConfig = profile.get_object_lock_configuration(
                Bucket=bucket["Name"]
            )
        ...
        try:
            bucketPublicAccessBlock = profile.get_public_access_block(
                Bucket=bucket["Name"]
            )
        ...
        try:
            ...
            if bucketObjects["KeyCount"] > 0:
                for bucketObject in bucketObjects["Contents"]:
                    try:
                        bucketObjectACL = profile.get_object_acl(
                    ...
                    try:
                        ...
                        bucketObject["ObjectLegalHold"] = bucketObjectLegalHold[
                            "LegalHold"
                        ]["Status"]
                    except botocore.exceptions.ClientError:
                    ...
                    try:
                        bucketObjectTagging = profile.get_object_tagging(
                            Bucket=bucket["Name"], Key=bucketObject["Key"]
                        )
