...
def exploit(profile, workspace):
    ...
    if not stager_type in stagers:
        return {
            "error": f"{stager_type} is not a valid stager type for this module. Look at stagers list."
        }
    ...
    if not checkBucket(bucket):
        ...
        return {
            "BucketName": {"BucketName": bucket, "Status": "Bucket Exists. C2 Setup."}
        }
...
def setupBucket(
    profile, bucketname, commandkey, outputkey, stageruser, kmskey, serveruser
):
    try:
        access_key = os.environ.get("AWS_ACCESS_KEY")
        secret_key = os.environ.get("AWS_SECRET_KEY")
