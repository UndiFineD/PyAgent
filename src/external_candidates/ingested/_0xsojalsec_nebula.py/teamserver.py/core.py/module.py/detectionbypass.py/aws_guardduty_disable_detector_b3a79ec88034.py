# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\detectionbypass\aws_guardduty_disable_detector.py
import sys
...
def exploit(profile, workspace):
    ...
    try:
        profile.update_detector(DetectorId=detectorID, Enable=False)
        status = f"Detector {detectorID} was disabled"
    except Exception as e:
        status = f"Detector {detectorID} was not disabled with error code: {str(e)}."
