# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\privesc\aws_iam_create_policy_version_terraform.py
import json
import os.path
import sys
...
def exploit(all_sessions, cred_prof, useragent, web_proxies, workspace):
    ...
    try:
        if os.path.exists(
            f"{sys.prefix}/lib/python3.10/site-packages/botocore/.user-agent"
        ):
            with open(
                f"{sys.prefix}/lib/python3.10/site-packages/botocore/.user-agent", "r"
            ) as uafile:
                currentua = uafile.read().replace("\n", "").strip()

        with open(
            f"{sys.prefix}/lib/python3.10/site-packages/botocore/.user-agent", "w"
        ) as uafile:
            uafile.write(
        ...
        return {"error": f"Error Creating Version for Policy {policy_arn}: {str(e)}"}
