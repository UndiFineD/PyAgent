# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\azure_check_azure_usage.py
import json
import sys
...
def exploit(workspace):
    ...
    try:
        ...
        if json_data["NameSpaceType"] == "Unknown":
            return {
                "DomainName": {"DomainName": domain, "Usage": "No Azure Usage"}
            }, 404
        elif json_data["NameSpaceType"] == "Managed":
            url_tenantId = "https://login.microsoftonline.com/{}/v2.0/.well-known/openid-configuration".format(
                domain
            )
            tenantId = (
                json.loads(requests.get(url_tenantId).text)["token_endpoint"]
            ).split("/")[3]
        ...
        try:
            ...
            models.Domains.objects().get(dn_name=azure_json["DomainName"]).update(
                **database_data
            )
