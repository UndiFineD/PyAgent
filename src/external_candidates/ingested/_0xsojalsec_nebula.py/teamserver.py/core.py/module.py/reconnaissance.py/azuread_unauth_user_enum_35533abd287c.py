# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\azuread_unauth_user_enum.py
import json
import os.path
...
def exploit(workspace):
    ...
    if not email == "" and not userfile == "":
        return {
            "error": colored("[*] Only add a username or a user file. Not both.", "red")
        }, 500

    if email == "" and userfile == "":
        return {
            "error": colored(
                "[*] Add at least a username or a user file. Not both though.", "red"
            )
        }, 500
        ...
        return {
            "azure_user_email": {
                "azure_user_email": "User '{0}' does not exist".format(email)
            }
        }, 200
