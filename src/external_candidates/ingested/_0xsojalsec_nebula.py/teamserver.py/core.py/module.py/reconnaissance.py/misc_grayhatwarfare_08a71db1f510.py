# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\misc_grayhatwarfare.py
import sys
...
def exploit(workspace):
    ...
    if not variables["FILE_KEYWORDS"]["value"] == "":
        ...
        file_keywords = []
    bucket_keywords = (
        (variables["BUCKET_KEYWORDS"]["value"])
        .replace(" ", "")
        .replace("\n", "")
        .strip()
        .split(",")
    ...
    try:
        nr_of_results_to_be_returned = int(
            variables["NUMBER_OF_RESULTS_TO_BE_RETURNED"]["value"]
        )
    except ValueError:
        return {
            "error": "NUMBER_OF_RESULTS_TO_BE_RETURNED can only be an Integer."
        }, 500

    for bucket_keyword in bucket_keywords:
        returned_output = bucket_keyword_func(
            bucket_keyword, nr_of_results_to_be_returned, api_token, cloud_vendor
        )
            ...
            if list_objects_bool.replace("\n", "").replace(" ", "").strip() == "true":
                if len(file_keywords) == 0:
                    bucket_files = object_keyword_func(
                        [], nr_of_results_to_be_returned, api_token, bucket["id"]
                    )
...
def bucket_keyword_func(keyword, nr_of_results_to_be_returned, api_token, cloud_vendor):
    ...
    params = {"access_token": api_token, "keywords": keyword, "type": cloud_vendor}

    # bucket_url += "&keyword={}".format(keyword)

    try:
        buckets_found = requests.get(bucket_url, params=params).json()
...
def object_keyword_func(keyword, nr_of_results_to_be_returned, api_token, bucket_id):
    bucket_url = (
        "https://buckets.grayhatwarfare.com/api/v1/bucket/{}/files/0/{}".format(
            str(bucket_id), nr_of_results_to_be_returned
        )
    )
    ...
    try:
        files_found = requests.get(bucket_url, params=params).json()
