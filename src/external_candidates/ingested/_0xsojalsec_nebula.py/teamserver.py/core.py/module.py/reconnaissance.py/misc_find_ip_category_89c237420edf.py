# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\misc_find_ip_category.py
import ipaddress
import re
import sys
...
def exploit(workspace):
    ...
    try:
        ...
        for d in all_domain:
            singlehost = {"Host": d, "Resolved": "", "Region": "", "Service": ""}
            if "*." in d:
                continue
            try:
        ...
        for i in all_ips:
            singlehost = {"Host": i, "Resolved": i, "Region": "", "Service": ""}
            try:
        ...
        return {"IP-FILE": split_by_services}, 200
    ...
    for azIPJSON in AZURE_IP_RANGE:
        for azIP in azIPJSON["properties"]["addressPrefixes"]:
            if an_address in ipaddress.ip_network(azIP):
                ...
                return [
                    "AZURE",
                    azIPJSON["properties"]["region"],
                    azIPJSON["name"].split(",")[0],
                ]
    return None
