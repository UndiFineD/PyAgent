# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\azure_fuzz_aks_api_server.py
import os
import sys
...
def exploit(workspace):
    ...
    try:
        if (
            variables["BASENAME"]["value"] == ""
            and variables["WORDLIST"]["value"] != ""
        ):
            try:
                ...
                for index in range(0, 8):
                    for a in range(0, 35):
                        ...
                        if region == "":
                            for r in regions:
                                try:
                                    url = "{0}-dns-{1}.hcp.{2}.azmk8s.io".format(
                                        basename, random_part, r
                                    )
            ...
            return {
                "error": "[*] Add either a basenameet or a wordlist of basenameets."
            }, 404
