# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\azure_service_dns_fuzzer.py
import os
import sys
...
def exploit(workspace):
    ...
    try:
        if (
            ...
            and variables["BASENAME"]["value"] == ""
            and variables["COUNTRY_CODE"]["value"] == ""
        ):
            return {
                "error": "[*] Either enter Basenames or Targets Domain and Country!"
            }

        elif (
            variables["DOMAIN"]["value"] != "" and variables["BASENAME"]["value"] == ""
        ):
            if variables["COUNTRY_CODE"]["value"] == "":
                ...
                all_basenameets = generate_basename(domain, country, country_code)[
                    "Domain"
                ]["Basenames"]
                ...
                for basename in all_basenameets:
                    ...
                    for dns_suffix, explanation in dns_suffixes.items():
                        try:
                            ...
                            basenames["{0}.{1}".format(basename, dns_suffix)] = (
                                explanation
                            )
                    ...
                    if basenames == {}:
                        ...
                        try:
                            models.AzureServices.objects().get(
                                azure_services_base_name=basename
                            ).update(**all_services)
            ...
            return {
                "error": "[*] Please enter at least Basenames or Targets Domain and Country!"
            }, 404
