# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\module\reconnaissance\digitalocean_space_bucket_name_fuzzer.py
import os
import sys
...
DO_REGIONS = ["nyc3", "fra1", "sfo2", "sfo3", "ams3", "sgp1", "blr1", "syd1"]
    ...
    try:
        for buck in variables["WORDLIST"]["wordlistvalue"]:
            for region in DO_REGIONS:
                buck = buck.replace("\n", "")
                try:
                    do_url = "https://{0}.{1}.digitaloceanspaces.com".format(
                        buck, region
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            do_website_url = (
                                "https://{0}.{1}.cdn.digitaloceanspaces.com/".format(
                                    buck, region
                                )
                            )
        ...
        for s3object in SPACES:
            try:
                models.DigitalOceanSpace.objects().get(
                    digitalocean_s3_space_name=s3object["digitalocean_s3_space_name"]
                ).update(**s3object)
