# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\auth\auth_azuread.py
import adal
...
def authenticate_device_code(proxies, verify, client_id, tenant):
    ...
    context = adal.AuthenticationContext(
        authority_host_uri, api_version=None, proxies=proxies, verify_ssl=verify
    )
...
def authenticate_username_password(username, password, client_id):
    ...
    context = adal.AuthenticationContext(
        authority_uri, api_version=None, proxies=proxies, verify_ssl=verify
    )
    tokendata = context.acquire_token_with_username_password(
...
def authenticate_clientid_clientsecret(client_id, client_secret, tenant, resource):
    ...
    token = context.acquire_token_with_client_credentials(
        GRAPH_RESOURCE, client_id, client_secret
    )
...
def acquire_token_with_client_certificate(
    resource_uri, client_id, certificate, thumbprint
):
    print()
