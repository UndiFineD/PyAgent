# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\createSession\giveMeClient.py
import base64
import os
import random
...
def enter_credentials(
    ...
    if not len(proxy_definitions) == 0:
        ...
        for key, value in web_proxy.items():
            ...
            proxy_configurations = {"proxy_client_cert": "/tmp/cert.pem"}
...
def enter_credentials_with_session_token(
    ...
    if not len(proxy_definitions) == 0:
        ...
        session_config_args["proxies"] = {web_proxy.split(":")[0]: web_proxy}
        # session_config_args["proxies"] = proxy_definitions
    ...
    if cred_prof == "":
        ...
        if c == 0:
            ...
            if not "session_token" in sess:
                ...
                return {
                    "error": (
                        colored("[*] Check if the session key is empty.", "yellow")
                    )
                }
