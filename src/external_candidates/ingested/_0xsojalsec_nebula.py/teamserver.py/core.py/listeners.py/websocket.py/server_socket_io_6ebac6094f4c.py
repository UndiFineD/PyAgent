...
def connect(sid, socket):
    ...
    print(
        colored("[*] New client connected and was given the id {}".format(sid), "green")
    )
    # print(sid, 'connected')
...
def get_task_output(sid, listener_name, listener_task_id, task_output):
    try:
        listener_object = WebsocketListener.objects.get(
        ...
        if len(listener_tasks) == 0:
            ...
            try:
                WebsocketListener.objects.get(listener_name=listener_name).update(
                    **listener_object
                )
...
def get_task(sid, listener_name):
    tasks_to_send = []
    try:
        listener_tasks = WebsocketListener.objects.get(
            listener_name=listener_name
        ).json()["listener_tasks"]
        if len(listener_tasks) == 0:
            pass
