# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\set_credentials\set_azuread_credentials_2.py
import json
import sys
...
def set_azuread_creds(command, all_sessions, comms):
    ...
    if len(command.split(" ")) < 3:
        print(
            colored(
                "[*] The right command is: set azure-credentials <profile name>", "red"
            )
        )
        ...
        if len(all_sessions) > 0:
            for credentials in all_sessions:
                if credentials["profile"] == command.split(" ")[2]:
                    print(
                        colored(
                            "[*] Those credentials exist. Try a new Profile Name", "red"
                        )
                    )
                    a = 1
...
>>> """)
            ...
            if (
                not int(connectiontype.strip()) == 1
                and not int(connectiontype.strip()) == 2
                and not int(connectiontype.strip()) == 3
            ):
                print(
                    colored(
            ...
            try:
                ...
                if int(connectiontype.strip()) == 1:
                    creds = auth_device_code(
                        sess_test["client_id"], sess_test["scopes"]
                    )

                elif int(connectiontype.strip()) == 2:
                    creds = auth_user_pass(
                        sess_test["username"],
                        sess_test["password"],
                        sess_test["client_id"],
                        sess_test["scopes"],
                    ...
                    creds = auth_refresh_token(
                        refresh_token, sess_test["client_id"], sess_test["scopes"]
                    )
