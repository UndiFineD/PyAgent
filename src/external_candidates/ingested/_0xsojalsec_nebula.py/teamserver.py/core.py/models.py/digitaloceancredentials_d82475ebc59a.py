# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\models\DigitalOceanCredentials.py
import datetime
import sys
...
@digitaloceancredentials_blueprint.route(
    "/api/latest/digitaloceancredentials", methods=["GET"]
)
@jwt_required()
...
@digitaloceancredentials_blueprint.route(
    "/api/latest/digitaloceancredentials/getuid", methods=["POST"]
)
@jwt_required()
def getuid_digitalocean_credentials():
    ...
    try:
        ...
        docredentials = DigitalOceanCredentials.objects.get(
            digitalocean_profile_name=body.get("digitalocean_profile_name")
        )
...
@digitaloceancredentials_blueprint.route(
    "/api/latest/digitaloceancredentials", methods=["POST"]
)
@jwt_required()
def get_digitaloceancredentials():
    body = request.get_json()
    try:
        digitaloceancreds = DigitalOceanCredentials.objects.get(
            digitalocean_profile_name=body.get("digitalocean_profile_name")
        )
...
@digitaloceancredentials_blueprint.route(
    "/api/latest/digitaloceancredentials", methods=["PUT"]
)
@jwt_required()
    ...
    try:
        ...
        return {
            "message": "Credentials of '{}' was created!".format(
                body["digitalocean_profile_name"]
            )
        }, 200
...
@digitaloceancredentials_blueprint.route(
    "/api/latest/digitaloceancredentials", methods=["DELETE"]
)
@jwt_required()
def delete_digitaloceancredentials():
    try:
        ...
        DigitalOceanCredentials.objects.get_or_404(
            digitalocean_profile_name=digitalocean_profile_name
        ).delete()
        return {
            "message": "Credentials of '{}' were deleted!".format(
                body["digitalocean_profile_name"]
            )
        }, 200
    except Exception as e:
