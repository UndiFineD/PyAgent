# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\models\Cosmonaut.py
import datetime
import json
...
def get_cosmonaut():
    ...
    access_token = create_access_token(
        identity=str(cosmonaut.cosmonaut_name), expires_delta=expires
    )
    print(
        colored(
            f"[*] User {cosmonaut['cosmonaut_name']} authenticated successfully at {str(datetime.datetime.now())}",
            "blue",
        )
    )
...
def set_cosmonaut():
    ...
    try:
        ...
        print(
            colored(
                f"[*] User '{body['cosmonaut_name']}' was created at {str(datetime.datetime.now())}!",
                "green",
            )
        )
...
def update_cosmonaut():
    try:
        ...
        print(
            colored(
                f"[*] User '{cosmonaut_name}' was updated at {datetime.datetime.now()}!",
                "light_blue",
            )
        )
        return {"message": "User '{}' was updated!".format(cosmonaut_name)}, 200
...
def delete_cosmonaut():
    try:
        ...
        print(
            colored(
                f"[*] User '{cosmonaut_name}' was deleted at {datetime.datetime.now()}!",
                "magenta",
            )
        )
