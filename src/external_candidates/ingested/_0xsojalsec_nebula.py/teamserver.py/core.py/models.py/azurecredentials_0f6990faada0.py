# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Nebula\teamserver\core\models\AZURECredentials.py
import datetime
import sys
...
@azurecredentials_blueprint.route(
    "/api/latest/azurecredentials/getuid", methods=["POST"]
)
@jwt_required()
...
def get_azurecredentials():
    body = request.get_json()
    try:
        azurecredentials = AZURECredentials.objects.get(
            aws_profile_name=body.get("aws_profile_name")
        )
...
@azurecredentials_blueprint.route(
    "/api/latest/azurecredentials/devicecode", methods=["POST"]
)
@jwt_required()
...
@azurecredentials_blueprint.route(
    "/api/latest/azurecredentials/generate", methods=["POST"]
)
@jwt_required()
...
@azurecredentials_blueprint.route(
    "/api/latest/azurecredentials/regenerate", methods=["POST"]
)
@jwt_required()
...
def set_azurecredentials():
    ...
    try:
        ...
        return {
            "message": "Credentials of '{}' was created!".format(
                body["aws_profile_name"]
            )
        }, 200
...
def delete_azurecredentials():
    try:
        ...
        AZURECredentials.objects.get_or_404(
            azurecredentials_name=azurecredentials_name
        ).delete()
        return {
            "message": "Credentials of '{}' was deleted!".format(
                body["aws_profile_name"]
            )
        }, 200
    except Exception as e:
