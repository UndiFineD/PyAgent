# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-Uscrapper\Uscrapper-vanta.py
import argparse
import atexit
import random
import re
...
def selenium_wd(url):
    ...
    if counter_Tor == 0 and is_tor_link(url):
        ...
        options_tor.set_preference(
            "plugin.state.torlauncher", 0
...
def web_crawler(start_url, max_pages=10, num_threads=4, key=0):
    ...
    if crwl == 0:
        print(
            colored("[!] Preparing Crawler \x1b[3m(Utilizing", "yellow"),
            colored(num_threads, "yellow"),
            colored("\x1b[3mthreads)", "yellow"),
        )
    if num_threads == None:
        num_threads = 4
    visited_links = set()
...
def extract_details(url, generate_report, non_strict):
    ...
    if args.keywords or args.file:
        ...
        if non_strict:
            usernames = set(
                username.string
                for username in soup.find_all(
                    "a", href=True, string=re.compile(r"^[^\s]+$")
                )
            )

        email_addresses = set(
            email["href"].replace("mailto:", "")
            for email in soup.find_all(
        ...
        geolocations = set(
            location["content"]
            for location in soup.find_all("meta", attrs={"name": "geo.position"})
        )
        ...
        phone_regex_combined = "|".join(
            "(?:{0})".format(x) for x in (phone_regex, phone_regex2, phone_regex3)
        )
...
def printlist():
    ...
    concl = (
        "Email Addresses:" + str(len(email_addresses1) + len(extracted_emails1)),
        "Social Links:" + str(len(social_links1)),
        "Phone Numbers:" + str(len(extracted_phone_numbers1)),
        "Geolocations:" + str(len(geolocations1)),
    )
    print("\n")
...
def generate_report():
    ...
    report_content += (
        "\n{Email Addresses:"
        + str(len(email_addresses1) + len(extracted_emails1))
        + ", Social Links:"
        + str(len(social_links1))
        + ", Phone Numbers:"
        + str(len(extracted_phone_numbers1))
        + ", Geolocations:"
        + str(len(geolocations1))
        + "}\n"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-O", "--generate-report", action="store_true", help="Generate a report"
    )
    parser.add_argument(
        "-ns",
        "--nonstrict",
        action="store_true",
        help="Display non-strict usernames (may show inaccurate results)",
    )
    parser.add_argument(
        "-c",
        "--crawl",
        type=int,
        help="specify max number of links to  Crawl and scrape within the same scope",
    )
    parser.add_argument(
        "-t",
        "--threads",
        type=int,
        help="Number of threads to utilize while crawling (default=4)",
    )
    parser.add_argument(
        "-k",
        "--keywords",
        nargs="+",
        help="Keywords to search for (as space-separated arguments)",
    )
        ...
        try:
            ...
            if response.status_code != 200 and response.status_code != 404:
                print(
                    ...
                    colored("Website might be using anti webscrapping methods.", "red"),
                )
