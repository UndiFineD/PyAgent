# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\src\repello_agent_wiz\frameworks\pydantic\mapper.py
# -*- coding: utf-8 -*-
import argparse
import ast
import inspect
...
def node_to_string(node: ast.AST) -> str:
    ...
    if isinstance(node, ast.Name):
        return node.id
    if isinstance(node, ast.Attribute):
        return get_potential_fqn(node) or "<Attribute>"
    if isinstance(node, ast.Constant):
        return repr(node.value)
    if isinstance(node, ast.Subscript):
        value_str = node_to_string(node.value)
        slice_node = node.slice
        if hasattr(slice_node, "value") and not isinstance(slice_node, ast.Tuple):
            slice_node = slice_node.value
        if isinstance(slice_node, ast.Tuple):
            slice_str = ", ".join(node_to_string(elt) for elt in slice_node.elts)
        else:
            slice_str = node_to_string(slice_node)
    ...
    if isinstance(node, ast.Compare):
        ...
        op_str = (
            ...
            else "!=" if isinstance(op, ast.NotEq) else str(op)
        )
...
def get_definition_location(node: ast.AST, filepath: Optional[str]) -> str:
    ...
    filepath_str = str(filepath) if filepath else "unknown_file"
    return f"{filepath_str}:{line}:{col}"
...
class AgentCallFinder(ast.NodeVisitor):
    ...
    def visit_Await(self, node: ast.Await):
        if isinstance(node.value, ast.Call):
            ...
            if (
                ...
                if agent_var_name:
                    self.called_agents.append((agent_var_name, call_node))
    ...
    def visit_Call(self, node: ast.Call):
        if isinstance(node.func, ast.Attribute) and node.func.attr == "run":
            ...
            if agent_var_name:
                self.called_agents.append((agent_var_name, node))
        self.generic_visit(node)
...
class PydanticAIStructureExtractor(ast.NodeVisitor):
    def __init__(self):
        ...
        self.pydantic_ai_fqns = {
            "Agent": "pydantic_ai.agent.Agent",
            "RunContext": "pydantic_ai.context.RunContext",
        }
        ...
        return "".join(
            (
                ...
                if isinstance(part, ast.Constant)
                    ...
                    f"{{{node_to_string(part.value)}}}"
                    if isinstance(part, ast.FormattedValue)
                    else f"<complex_part:{type(part).__name__}>"
                )
            )
        ...
        if name not in self.output_node_ids:
            node_data = {
                ...
                "function_name": (
                    name
                    if node_type
                    in [PydanticAINodeType.TOOL, PydanticAINodeType.ORCHESTRATOR]
                    else None
                ),
                "docstring": docstring,
            ...
            if node_type == PydanticAINodeType.AGENT:
        ...
        if source in self.output_node_ids and target in self.output_node_ids:
            ...
            if not any(
                ...
                if definition_location:
                    edge_metadata["definition_location"] = definition_location
        ...
        if isinstance(node, ast.List):
            ...
            for elt in node.elts:
                ...
                if name:
                    ...
                    if call_name:
                        elements.append(f"{call_name}()")
                else:
                    elements.append(node_to_string(elt))
            return elements
        return None
        ...
        for keyword in node.keywords:
            ...
            if arg_name:
                ...
                if isinstance(value_node, ast.Constant):
                    ...
                    value_node, (ast.Name, ast.Attribute, ast.BinOp, ast.Subscript)
                ):
                    kwargs[arg_name] = node_to_string(value_node)
                elif isinstance(value_node, ast.List):
                    kwargs[arg_name] = (
                        self._get_list_of_identifiers(value_node) or "<complex_list>"
                    )
                elif isinstance(value_node, ast.JoinedStr):
                    kwargs[arg_name] = self._get_joined_string_value(value_node)
                else:
                    kwargs[arg_name] = f"<ast.{type(value_node).__name__}>"
        return kwargs
    ...
    def visit_Import(self, node: ast.Import):
        for alias in node.names:
            ...
            if module_name == "pydantic_ai":
                self.pydantic_ai_fqns["Agent"] = "pydantic_ai.Agent"
                self.pydantic_ai_fqns["RunContext"] = "pydantic_ai.RunContext"
        self.generic_visit(node)
            ...
            for alias in node.names:
                ...
                for key, default_fqn in self.pydantic_ai_fqns.items():
                    ...
                    if node.level > 0 and self.current_filepath:
                        ...
                        potential_fqn_match = (
                            f"{relative_prefix}{base_module}.{imported_name}"
                        )
                ...
                for sub_node in ast.walk(node):
                    if isinstance(sub_node, ast.Call):
                        if (
                            ...
                            and sub_node.func.attr == "run"
                            and get_potential_fqn(sub_node.func.value)
                            in ["asyncio", self.imports.get("asyncio")]
                        ):
                            if len(sub_node.args) == 1 and isinstance(
                                sub_node.args[0], ast.Call
                            ):
                                potential_entry_call = sub_node.args[0]
                                func_name = get_potential_fqn(potential_entry_call.func)
                                if (
                                    func_name
                                    and func_name not in self.potential_entry_points
                                ):
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        is_pydantic_model = any(
            self._resolve_fqn_from_node(b)
            in ["pydantic.main.BaseModel", "pydantic.BaseModel"]
            or get_potential_fqn(b) == "BaseModel"
            for b in node.bases
        )
        is_dataclass = any(
            (isinstance(d, ast.Name) and d.id == "dataclass")
            or (isinstance(d, ast.Attribute) and d.attr == "dataclass")
            or (isinstance(d, ast.Call) and get_potential_fqn(d.func) == "dataclass")
        ...
        if is_pydantic_model or is_dataclass:
            ...
            for body_node in node.body:
                if isinstance(body_node, ast.AnnAssign) and isinstance(
                    ...
                    field_type = (
                        node_to_string(body_node.annotation)
                        if body_node.annotation
                        else "Any"
                    )
            ...
            self.deps_classes[class_name] = {
                "name": class_name,
                "fields": fields,
                "definition_location": definition_loc,
                "is_pydantic": is_pydantic_model,
                "is_dataclass": is_dataclass,
                "class_node": class_node_ref,
            }
        self.generic_visit(node)
                ...
                if base_func_node:
                    ...
                    is_agent_instantiation = (
                        (
                            resolved_func_fqn
                            and agent_fqn
                            and resolved_func_fqn == agent_fqn
                        )
                        or (func_name_simple == "Agent")
                        or (resolved_func_fqn == "pydantic_ai.agent.Agent")
                    )
                    ...
                    if is_agent_instantiation:
                        ...
                        if (
                            value_node.args
                            and len(value_node.args) > 0
                            and isinstance(
                                value_node.args[0], (ast.Constant, ast.JoinedStr)
                            )
                        ):
                            model_name = (
                                self._get_string_value(value_node.args[0])
                                or self._get_joined_string_value(value_node.args[0])
                                or model_name
                            )
                        ...
                        for kw in value_node.keywords:
                            if kw.arg == "system_prompt":
                                if isinstance(kw.value, ast.JoinedStr):
                                    system_prompt = self._get_joined_string_value(
                                        kw.value
                                    )
                                else:
                                    system_prompt = self._get_string_value(kw.value)
                                if system_prompt is None:
                                    system_prompt = (
                                        f"<complex_prompt:{type(kw.value).__name__}>"
                                    )
                                break
                        if (
                            ...
                            and isinstance(
                                value_node.args[1], (ast.Constant, ast.JoinedStr)
                            )
                        ):
                            if isinstance(value_node.args[1], ast.JoinedStr):
                                system_prompt = self._get_joined_string_value(
                                    value_node.args[1]
                                )
                            else:
                                system_prompt = self._get_string_value(
                                    value_node.args[1]
                                )
                        ...
                        definition_loc = get_definition_location(
                            node, self.current_filepath
                        )
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        for decorator in node.decorator_list:
            if isinstance(decorator, ast.Attribute) and decorator.attr == "tool":
                ...
                if potential_agent_var and potential_agent_var in self.agents_info:
                    is_tool = True
                    tool_agent_var = potential_agent_var
        ...
        if is_tool and tool_agent_var:
            ...
            self.tools_info[func_name] = {
                "name": func_name,
                "agent_var": tool_agent_var,
                "func_def_node": node,
                "docstring": docstring,
                "definition_location": definition_loc,
            }
            if tool_agent_var in self.agents_info:
                self.agents_info[tool_agent_var]["tools"].append(func_name)
        else:
            finder = AgentCallFinder()
            finder.visit(node)
            if finder.called_agents:
                if func_name not in self.tools_info:
                    called_agent_names = list(
                        set(agent_name for agent_name, _ in finder.called_agents)
                    )
                    call_locations = {}
                    processed_agents = set()
                    for agent_name, call_node in finder.called_agents:
                        if agent_name not in processed_agents:
                            call_locations[agent_name] = get_definition_location(
                                call_node, self.current_filepath
                            )
    ...
    def finalize_graph(self):
        ...
        self._add_output_node(
            name="Start",
            node_type=PydanticAINodeType.START,
            description="Application Entry Point",
            location="system",
        )
        self._add_output_node(
            name="End",
            node_type=PydanticAINodeType.END,
        ...
        for agent_id, agent_data in self.agents_info.items():
            ...
            metadata = {
                **metadata_kwargs,
                ...
                "result_type": agent_data.get("result_type_str"),
                "deps_type": agent_data.get("deps_type_str"),
                "tools": agent_data.get("tools"),
            }
            description = (
                agent_data.get("system_prompt")
                or f"Agent using model {agent_data.get('model', 'N/A')}"
            )
            self._add_output_node(
                name=agent_id,
                node_type=PydanticAINodeType.AGENT,
                description=description,
                docstring=None,
                location=agent_data.get("definition_location"),
                metadata=metadata,
            )
        ...
        for tool_id, tool_data in self.tools_info.items():
            ...
            description = (
                tool_data.get("docstring")
                or f"Tool function used by agent '{tool_data.get('agent_var')}'"
            )
            self._add_output_node(
                name=tool_id,
                node_type=PydanticAINodeType.TOOL,
                description=description,
                docstring=tool_data.get("docstring"),
                location=tool_data.get("definition_location"),
                metadata=metadata,
            )
        ...
        for func_id, func_data in self.orchestrator_funcs.items():
            ...
            self._add_output_node(
                name=func_id,
                node_type=PydanticAINodeType.ORCHESTRATOR,
                description=description,
                docstring=docstring,
                location=func_data.get("definition_location"),
                metadata=metadata,
            )
            ...
            for tool_name in agent_data.get("tools", []):
                if tool_name in self.tools_info:
                    ...
                    self._add_output_edge(
                        ...
                        definition_location=tool_def_loc,
                        metadata={"agent_definition": agent_def_loc},
                    )
        ...
        for func_id, func_data in self.orchestrator_funcs.items():
            ...
            for called_agent_id in func_data.get("calls", []):
                if called_agent_id in self.agents_info:
                    ...
                    self._add_output_edge(
                        source=func_id,
                        target=called_agent_id,
                        condition="calls agent",
                        definition_location=edge_def_loc,
                        metadata={},
                    )

        for tool_id, tool_data in self.tools_info.items():
            calling_agent_id = tool_data.get("agent_var")
            func_def_node = tool_data.get("func_def_node")
            if not calling_agent_id or not func_def_node:
                continue
            finder = AgentCallFinder()
            finder.visit(func_def_node)
            ...
            for called_agent_id_in_tool, call_node_in_tool in finder.called_agents:
                if called_agent_id_in_tool not in processed_agents_in_tool:
                    tool_filepath = (
                        self.tools_info[tool_id]
                        .get("definition_location", "unknown")
                        .split(":")[0]
                        if self.current_filepath is None
                        else self.current_filepath
                    )
                    edge_def_loc_in_tool = get_definition_location(
                        call_node_in_tool, tool_filepath
                    )
                    ...
                    if called_agent_id_in_tool in self.agents_info:
                        ...
                        self._add_output_edge(
                            source=calling_agent_id,
                            target=called_agent_id_in_tool,
                            condition="calls agent",
                            definition_location=edge_def_loc_in_tool,
                            metadata={"via_tool": tool_id},
                        )
        ...
        if self.potential_entry_points:
            ...
            print(
                "No specific __main__ entry points found. Using fallback: connecting Start to root orchestrators."
            )
            ...
            for node in self.output_nodes:
                ...
                if (
                    node_type == PydanticAINodeType.ORCHESTRATOR.value
                    and node_id not in all_target_nodes
                ):
                    self._add_output_edge(
                        source="Start",
                        target=node_id,
                        condition="initiates",
                        definition_location="fallback_heuristic",
                    )
        ...
        if entry_points_connected_to_start:
            ...
            print(
                "Warning: Could not identify any entry points to connect Start/End nodes."
            )
...
def extract_pydantic_ai_graph(directory_path: str, output_filename: str):
    ...
    for root, dirs, files in os.walk(directory_path):
        dirs[:] = [
            ...
            for d in dirs
            if not d.startswith(".")
            and d not in ["venv", "env", "__pycache__", "node_modules", ".git"]
        ]
        for filename in files:
            if filename.endswith(".py"):
                ...
                try:
                    ...
                    print(
                        f"Warning: Skipping file {filepath} due to SyntaxError: {e}",
                        file=sys.stderr,
                    )
                    filepaths_processed.pop()
                except Exception as e:
                    print(
                        f"Warning: Skipping file {filepath} due to error: {e}",
                        file=sys.stderr,
                    )
                    import traceback
    ...
    print(
        f"AST parsing complete ({parsed_files} files processed). Finalizing graph structure..."
    )
    extractor.finalize_graph()
    print(
        f"Extraction finished. Found {len(extractor.output_nodes)} nodes and {len(extractor.output_edges)} edges."
    )
    ...
    if graph_structure["nodes"] or graph_structure["edges"]:
        try:
            ...
            graph_structure["edges"].sort(
                key=lambda x: (x["source"], x["target"], x["condition"])
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directory", "-d", type=str, default=".", help="Target directory"
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="pydantic_ai_graph_output.json",
        help="Output file",
    )
