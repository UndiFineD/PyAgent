    ...
    def visit_Assign(self, node):
        if isinstance(node.value, ast.Call) and hasattr(node.value.func, "id"):
            ...
            if call_type == "FunctionTool":
                ...
                if isinstance(name_value, ast.Constant) and isinstance(
            ...
            # RoundRobinGroupChat
                ...
                if args and isinstance(args[0], ast.List):
                    agent_vars = [
                        elt.id for elt in args[0].elts if isinstance(elt, ast.Name)
                    ]
                    agent_ids = [
                        self.agent_names_to_ids.get(var)
                        for var in agent_vars
                        if var in self.agent_names_to_ids
                    ]
            ...
            # SelectorGroupChat
                ...
                if args and isinstance(args[0], ast.List):
                    ...
                    for agent_id in agent_ids:
                        self.agent_edges.append(
                            (group_var, agent_id, "SelectorGroupChat")
                        )
...
def build_graph_data(visitor):
    ...
    for agent_name, agent_data in visitor.agents.items():
        ...
        for tool_var in agent_data["tools"]:
            ...
            if not tool_funcs:
                print(
                    f"[WARN] Tool variable '{tool_var}' declared in agent '{agent_name}' not found in visitor.tools"
                )
                continue
    ...
    for source, target, edge_type in getattr(visitor, "agent_edges", []):
        edge = {
            ...
            "condition": {
                "type": (
                    "group_sequence"
                    if edge_type == "RoundRobinGroupChat"
                    else "member_of_team"
                )
            },
        ...
        if edge_type == "SelectorGroupChat" and not any(
            n["name"] == source for n in nodes
        ):
            nodes.append(
                {
                    "name": source,
    ...
    selector_group_names = set(
        source for source, _ in getattr(visitor, "selector_memberships", [])
    )
    ...
    for visitor in visitors:
        ...
        for source, targets in visitor.tool_calls.items():
            if source not in merged.tool_calls:
                ...
                merged.tool_calls[source].extend(
                    t for t in targets if t not in merged.tool_calls[source]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", "-o", type=str, default="agentchat_graph.json", help="Output file"
    )
