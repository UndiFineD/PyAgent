# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\src\repello_agent_wiz\frameworks\llama_index\mapper.py
import ast
import json
import os
import sys
            ...
            if func_name in {"FunctionAgent", "ReActAgent"}:
                ...
                if func_name == "FunctionAgent":
                    ...
                    agent_name = (
                        ...
                        else f"ReActAgent_{node.lineno}"
                    )
                        ...
                        if isinstance(handoffs, ast.List):
                            for elt in handoffs.elts:
                                if isinstance(elt, ast.Constant):
                                    self.agents[agent_name]["handoffs"].append(
                                        elt.value
                                    )
...
def build_graph_data(visitor):
        ...
        nodes.append(
            {
                ...
                "node_type": "Agent",
                "source_location": None,
                "metadata": {"inherits": agent_data.get("inherits", [])},
            }
        )
    ...
    for source_func, targets in visitor.tool_calls.items():
        for target_func in targets:
            for agent_name, agent_data in visitor.agents.items():
                if (
                    source_func in agent_data["tools"]
                    and target_func in agent_data["tools"]
                ):
                    edges.append(
                        {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--directory", "-d", type=str, default=".", help="Source directory"
    )
    parser.add_argument(
        "--output", "-o", type=str, default="llamaindex_graph.json", help="Output file"
    )
