# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\src\repello_agent_wiz\frameworks\google_adk\mapper.py
import argparse
import ast
import json
import logging
...
def node_to_string(node: Optional[ast.AST]) -> str:
    ...
    if isinstance(node, ast.Call):
        ...
        kwargs_str = ", ".join(
            f"{kw.arg}={node_to_string(kw.value)}" for kw in node.keywords
        )
    ...
    if isinstance(node, ast.List):
        return "[" + ", ".join(node_to_string(elt) for elt in node.elts) + "]"
    ...
    if isinstance(node, ast.Dict):
        return (
            "{"
            + ", ".join(
                f"{node_to_string(k)}: {node_to_string(v)}"
...
class AdkStructureExtractor(ast.NodeVisitor):
    ...
    def _resolve_import(self, name: str, filepath: Optional[str]) -> Optional[str]:
        if not filepath or filepath not in self.imports_map:
            return name
        ...
        if len(parts) > 1 and parts[0] in file_imports:
            ...
            if not base_import.endswith(parts[0]):
                return f"{base_import}.{parts[1]}"
            else:
                return f"{base_import}.{parts[1]}"
        return name
    ...
    def _get_value_repr(self, node: ast.AST, resolve_imports: bool = True) -> str:
        ...
        if isinstance(node, ast.Attribute):
            base_repr = self._get_value_repr(
                node.value, resolve_imports=resolve_imports
            )
        ...
        for keyword in node.keywords:
            ...
            if arg_name == "tools" and isinstance(value_node, ast.List):
                ...
                for elt in value_node.elts:
                    if isinstance(elt, ast.Call):
                        ...
                        if call_func_name == function_tool_local_name:
                            ...
                            if func_arg_value:
                                tool_reprs.append(
                                    self._get_value_repr(
                                        func_arg_value, resolve_imports=False
                                    )
                                )
                                logger.debug(
                                    f"Extracted tool '{tool_reprs[-1]}' from FunctionTool wrapper."
                                )
                            else:
                                logger.warning(
                                    f"Found FunctionTool call without 'func' kwarg: {node_to_string(elt)}"
                                )
                                tool_reprs.append(
                                    f"<Invalid FunctionTool Call: {node_to_string(elt)}>"
                                )
                        else:
                            tool_reprs.append(
                                self._get_value_repr(elt, resolve_imports=False)
                            )
                    else:
                        tool_reprs.append(
                            self._get_value_repr(elt, resolve_imports=False)
                ...
                if isinstance(value_node, ast.Constant):
                    ...
                    kwargs[arg_name] = self._get_value_repr(
                        value_node, resolve_imports=False
                    )
        ...
        for local_name, fqn_or_module in imports_to_check.items():
            for key, adk_fqn in self.adk_classes.items():
                if fqn_or_module == adk_fqn:
                    ...
                    if (
                        key == "LlmAgent"
                        and "Agent" in imports_to_check
                        and imports_to_check["Agent"] == adk_fqn
                    ):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        if not self.current_filepath or not node.module:
            return
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        base_classes_repr = [
            self._get_value_repr(b, resolve_imports=True) for b in node.bases
        ]
        ...
        if base_agent_fqn in base_classes_repr:
            ...
            self.custom_agent_classes[class_name] = {
                "name": class_name,
                "bases": base_classes_repr,
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        file_rel_path = (
            os.path.relpath(self.current_filepath, ".")
            if self.current_filepath
            else "unknown"
        )
        full_func_key = (
            f"{file_rel_path}::{current_class}.{func_name}"
            if current_class
            else f"{file_rel_path}::{func_name}"
        )
        simple_name = f"{current_class}.{func_name}" if current_class else func_name
        if full_func_key not in self.all_functions:
            logger.debug(
                f"Collected function/method definition: {simple_name} (key: {full_func_key}) at {definition_loc}"
            )
        ...
        if len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
            ...
            if isinstance(value_node, ast.Call):
                ...
                if agent_type_key and agent_type_key != "AgentTool":
                    ...
                    logger.debug(
                        f"Found Agent Instance: {instance_var_name} (Type: {agent_type_key}) at {definition_loc}"
                    )
                    self.agent_instances[instance_var_name] = {
                        "instance_var_name": instance_var_name,
                        "agent_type_key": agent_type_key,
                        "class_name_repr": self._get_value_repr(
                            call_func_node, resolve_imports=False
                        ),
                        "name_param_repr": kwargs.get("name"),
                        "description_param_repr": kwargs.get("description"),
                        "instruction_param_repr": kwargs.get("instruction"),
                        "global_instruction_param_repr": kwargs.get(
                            "global_instruction"
                        ),
                    ...
                    if wrapped_agent_var_repr and isinstance(
                        wrapped_agent_var_repr, str
                    ):
                        logger.debug(
                            f"Found AgentTool Instance: {tool_instance_name} wrapping '{wrapped_agent_var_repr}' at {definition_loc}"
                        )
                        self._agent_tool_instance_map[tool_instance_name] = (
                            wrapped_agent_var_repr
                        )
                    else:
                        logger.warning(
                            f"AgentTool instance {tool_instance_name} needs 'agent' kwarg."
                        )
        self.generic_visit(node)
    ...
    def _add_output_edge(
        ...
        if not (
            source_id in self.output_node_ids and target_id in self.output_node_ids
        ):
            logger.warning(
                f"Skipping edge '{label}' from '{source_id}' to '{target_id}'. Source or target node ID not found."
            )
            return
        if any(
            e["source"] == source_id
    ...
    def _find_function_def(
        self, func_name_repr: str, agent_context_filepath: Optional[str]
    ) -> Optional[Dict[str, Any]]:
        potential_match = next(
            (
                data
                for data in self.all_functions.values()
                if data["simple_name"] == func_name_repr
            ),
            None,
        )
        if potential_match:
            ...
            if resolved_fqn and resolved_fqn != func_name_repr:
                potential_match = next(
                    (
                        data
                        for data in self.all_functions.values()
                        if data["simple_name"] == resolved_fqn
                        or data["key"] == resolved_fqn
                    ),
                    None,
                )
                if potential_match:
                ...
                potential_match = next(
                    (
                        data
                        for data in self.all_functions.values()
                        if data["key"].endswith(f"::{resolved_fqn}")
                    ),
                    None,
                )
                if potential_match:
        ...
        if potential_match:
            return potential_match
        logger.warning(
            f"Could not find definition for function/tool '{func_name_repr}' referenced in context '{agent_context_filepath}'."
        )
        return None
    ...
    def finalize_graph(self):
        ...
        for agent_var, data in self.agent_instances.items():
            ...
            node_type = (
                AdkNodeType.LLMAGENT
                if agent_type_key in ["LlmAgent", "Agent"]
                else (
                    AdkNodeType[agent_type_key.upper()]
                    if agent_type_key != "CustomAgent"
                    else AdkNodeType.CUSTOM_AGENT
                )
            )
            metadata = {
                "original_variable_name": agent_var,
                "class_called_repr": data["class_name_repr"],
                "name_parameter": data.get("name_param_repr"),
                "description": data.get("description_param_repr"),
                "instruction": data.get("instruction_param_repr"),
                "global_instruction": data.get("global_instruction_param_repr"),
        ...
        for agent_var, data in self.agent_instances.items():
            ...
            agent_type_key = data["agent_type_key"]
            if agent_type_key not in ["LlmAgent", "Agent"]:
                continue

            for tool_var_repr in data.get("tool_vars_repr", []):
                target_node_id = None
                edge_label = "uses_tool"
                tool_metadata = {}
                ...
                if tool_var_repr in self._agent_tool_instance_map:
                    ...
                    if target_node_id:
                        edge_label = "uses_tool (Agent)"
                        tool_metadata["via_agent_tool_instance"] = tool_var_repr
                    else:
                        logger.warning(
                            f"AgentTool '{tool_var_repr}' wraps unknown agent var '{wrapped_agent_var}'."
                        )
                else:
                    ...
                    if func_data:
                        ...
                        if simple_name in created_tool_node_ids:
                            ...
                            tool_node_metadata = {
                                "original_function_key": func_data["key"],
                                "simple_name": simple_name,
                                "docstring": func_data.get("docstring"),
                                "is_method": func_data.get("is_method"),
                                "filepath": func_data.get("filepath"),
                            }
                            tool_node_metadata = {
                                k: v
                                for k, v in tool_node_metadata.items()
                                if v is not None
                            }
                            self._add_output_node(
                                target_node_id,
                                simple_name,
                                AdkNodeType.TOOL_FUNCTION,
                                func_data["definition_location"],
                                tool_node_metadata,
                            )
        ...
        for agent_var, data in self.agent_instances.items():
            ...
            for sub_agent_var_repr in data.get("sub_agent_vars_repr", []):
                ...
                if target_node_id:
                    ...
                    self._add_output_edge(
                        source_node_id, target_node_id, edge_label, agent_definition_loc
                    )
                else:
                    logger.warning(
                        f"Sub-agent variable '{sub_agent_var_repr}' for agent '{agent_var}' not found."
                    )

        all_sub_agent_vars = set()
        for data in self.agent_instances.values():
            all_sub_agent_vars.update(data.get("sub_agent_vars_repr", []))
        ...
        if self.output_nodes:
            ...
            if root_agent_vars:
                ...
                for root_var in root_agent_vars:
                    ...
                    if root_node_id:
                        self._add_output_edge(
                            start_node_id, root_node_id, "initiates", "graph_analysis"
                        )
                        self._add_output_edge(
                            root_node_id,
                            end_node_id,
                ...
                logger.warning("No root agents identified to link Start/End.")
...
def extract_adk_graph(directory_path: str, output_filename: str):
    ...
    if not os.path.isdir(directory_path):
        logger.error(f"Provided path '{directory_path}' is not a valid directory.")
        return {"nodes": [], "edges": []}
    filepaths_to_parse = []
    for root, dirs, files in os.walk(directory_path):
        ...
        for filename in files:
            if filename.endswith(".py"):
                filepaths_to_parse.append(os.path.join(root, filename))
    parsed_files = 0
    logger.info(
        f"Found {len(filepaths_to_parse)} Python files. Starting parsing pass..."
    )
    for filepath in filepaths_to_parse:
        ...
        try:
            ...
            logger.warning(f"Skipping file {filepath} due to SyntaxError: {e}")
        except Exception as e:
            logger.error(f"Error parsing file {filepath}: {e}", exc_info=True)
    logger.info(
    ...
    if graph_structure["nodes"] or graph_structure["edges"]:
        try:
            ...
            graph_structure["edges"].sort(
                key=lambda x: (x["source"], x["target"], x["label"])
            )
        ...
        logger.info("No graph structure extracted. Output file not written.")
    return graph_structure


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse Google ADK Python code to extract agent graph structure."
    )
    parser.add_argument(
        "--directory",
        "-d",
        type=str,
        default=".",
        help="Directory containing the ADK Python code.",
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="google_adk_graph_output.json",
