# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\src\repello_agent_wiz\frameworks\n8n\mapper.py
import argparse
import json
import os
import re
...
def load_categories(filepath="nodes_categorized.json"):
    ...
    if not category_path.exists():
        print(
            f"Warning: Category file not found at {category_path}. Categories will not be added.",
            file=sys.stderr,
        )
        return {}
    try:
        with open(category_path, "r", encoding="utf-8") as f:
            ...
            if not isinstance(data, dict):
                print(
                    f"Error: Category file {category_path} is not a JSON object (key-value pairs) as expected. Found type: {type(data)}",
                    file=sys.stderr,
                )
                return {}
            print(
                f"Successfully loaded {len(data)} category definitions from {filepath}."
            )
            return data
    except json.JSONDecodeError as e:
        print(
            f"Error: Invalid JSON in category file {category_path}: {e}",
            file=sys.stderr,
        )
        return {}
    except Exception as e:
        print(
            f"Error: Could not read category file {category_path}: {e}", file=sys.stderr
        )
        return {}
...
class N8nWorkflowParser:
    ...
    def _parse_nodes(self):
        ...
        if "nodes" not in self.n8n_data or not isinstance(self.n8n_data["nodes"], list):
            print(
                f"Warning ({self.filename}): 'nodes' array not found or invalid.",
                file=sys.stderr,
            )
            return
        ...
        for node_data in self.n8n_data["nodes"]:
            ...
            if not node_id:
                print(
                    f"Warning ({self.filename}): Skipping node without an 'id': {node_data.get('name', 'Unnamed')}",
                    file=sys.stderr,
                )
                continue
            ...
            if primary_category:
                if isinstance(subcategories_data, list):
                    primary_subcategory = (
                        subcategories_data[0] if subcategories_data else None
                    )
            ...
            if n8n_node_name in self.node_map_by_name:
                print(
                    f"Warning ({self.filename}): Duplicate node name '{n8n_node_name}'. Connection mapping might use the first encountered ID.",
                    file=sys.stderr,
                )
    ...
    def _parse_edges(self):
        ...
        if not isinstance(connections, dict):
            print(
                f"Warning ({self.filename}): 'connections' is not a dictionary. Found type: {type(connections)}. Skipping edge parsing.",
                file=sys.stderr,
            )
            return
        ...
        for source_node_name, outputs in connections.items():
            ...
            if not source_node_id:
                if source_node_name in self.node_map_by_id:
                    ...
                    print(
                        f"Warning ({self.filename}): Could not find source node ID for name '{source_node_name}' in connections. Skipping connections from this node.",
                        file=sys.stderr,
                    )
                    continue
            ...
            if not isinstance(outputs, dict):
                print(
                    f"Warning ({self.filename}): Expected dictionary for outputs of node '{source_node_name}', found {type(outputs)}. Skipping.",
                    file=sys.stderr,
                )
                continue
            ...
            for output_handle, targets_array in outputs.items():
                if not isinstance(targets_array, list):
                    print(
                        f"Warning ({self.filename}): Expected list for targets from '{source_node_name}' output '{output_handle}', found {type(targets_array)}. Skipping.",
                        file=sys.stderr,
                    )
                    continue
                ...
                for target_list in targets_array:
                    if not isinstance(target_list, list):
                        print(
                            f"Warning ({self.filename}): Expected inner list for targets from '{source_node_name}' output '{output_handle}', found {type(target_list)}. Skipping.",
                            file=sys.stderr,
                        )
                        continue
                    ...
                    for target_info in target_list:
                        if not isinstance(target_info, dict):
                            print(
                                f"Warning ({self.filename}): Expected dictionary for target info from '{source_node_name}' output '{output_handle}', found {type(target_info)}. Skipping.",
                                file=sys.stderr,
                            )
                            continue
                        ...
                        if not target_node_name:
                            print(
                                f"Warning ({self.filename}): Target connection from '{source_node_name}' (ID: {source_node_id}) via handle '{output_handle}' is missing target node name. Skipping.",
                                file=sys.stderr,
                            )
                            continue
                        ...
                        if not target_node_id:
                            if target_node_name in self.node_map_by_id:
                                ...
                                print(
                                    f"Warning ({self.filename}): Could not find target node ID for name '{target_node_name}' connected from '{source_node_name}' (ID: {source_node_id}). Skipping this edge.",
                                    file=sys.stderr,
                                )
                                continue
                        ...
                        if target_node_id not in self.parsed_node_ids:
                            print(
                                f"Info ({self.filename}): Skipping edge to filtered node '{target_node_name}' (ID: {target_node_id}). It might be a StickyNote.",
                                file=sys.stderr,
                            )
                            continue
    ...
    def _add_start_end_nodes(self):
        ...
        if not self.parsed_node_ids:
            print(
                f"Info ({self.filename}): No functional nodes found, skipping _START_/_END_ node addition."
            )
            return
...
def extract_n8n_graph(input_path_str, output_arg_str):
    ...
    if input_path.is_dir():
        ...
        if not json_files:
            print(
                f"Warning: No .json files found in directory: {input_dir}",
                file=sys.stderr,
            )
            ...
            try:
                ...
                print(
                    f"Error: Could not write empty output file {output_file}: {e}",
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        for file_path in json_files:
            ...
            try:
                ...
                parser = N8nWorkflowParser(
                    n8n_data, input_filename_str_loop, categories_data
                )
                graph_data = parser.parse()
                ...
                if graph_data:
                    if graph_data.get("nodes"):
                        ...
                        print(
                            f"Successfully parsed and added graph data from: {file_path.name}"
                        )
                    else:
                        print(
                            f"Info: No functional nodes found in {file_path.name}. Skipping addition to combined graph."
                        )
                    processed_files_count += 1
                else:
                    print(
                        f"Warning: Parsing {file_path.name} returned None or empty data."
                    )
                    failed_files_count += 1

            except json.JSONDecodeError as e:
                print(
                    f"Error: Invalid JSON in file {file_path.name}: {e}",
                    file=sys.stderr,
                )
                failed_files_count += 1
            except Exception as e:
                print(
                    f"Error: An unexpected error occurred while processing {file_path.name}: {e}",
                    file=sys.stderr,
                )
                failed_files_count += 1
        ...
        if failed_files_count > 0:
            print(
                f"Warning: {failed_files_count} file(s) failed to process. The combined graph may be incomplete.",
                file=sys.stderr,
            )
        ...
        try:
            ...
            print(
                f"Error: Could not write combined output file {output_file}: {e}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if input_path.suffix.lower() != ".json":
            print(
                f"Error: Input file is not a .json file: {input_path}", file=sys.stderr
            )
            sys.exit(1)

        if output_arg.suffix.lower() == ".json":
            print(
                f"Warning: Output argument '{output_arg}' looks like a file for single file input. Using its parent directory '{output_arg.parent}' for output.",
                file=sys.stderr,
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse n8n workflow JSON files into a standardized graph format."
    )
    parser.add_argument(
        "input_path",
        help="Path to a single n8n .json file or a directory containing .json files.",
    )
    parser.add_argument(
        "-o",
        "--output",
        default=".",
        help="Directory to save the parsed output JSON files (default: current directory).",
    )
