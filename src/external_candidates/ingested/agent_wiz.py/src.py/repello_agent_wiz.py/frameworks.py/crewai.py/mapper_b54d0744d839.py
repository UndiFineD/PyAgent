...
def _safe_load_yaml(filepath: str) -> Optional[Dict]:
    ...
    try:
        ...
        print(
            f"Warning: Unexpected error loading YAML {filepath}: {e}", file=sys.stderr
        )
    return None
...
def _stringify_ast_node(node: ast.AST) -> str:
    if isinstance(node, ast.Constant):
        ...
        pairs = [
            f"{_stringify_ast_node(k)}: {_stringify_ast_node(v)}"
        ...
        kwargs = [
            f"{kw.arg}={_stringify_ast_node(kw.value)}"
...
class CrewAIStructureExtractor(ast.NodeVisitor):
    ...
    def _add_output_edge(
        self, source: str, target: str, condition: Dict, metadata: Optional[Dict]
    ):
            ...
            if arg_name:
                ...
                if isinstance(value_node, ast.Constant):
                    ...
                    kwargs[arg_name] = (
                        resolved_name
                        if resolved_name
                        else _stringify_ast_node(value_node)
                    )
                elif isinstance(value_node, ast.List):
                    kwargs[arg_name] = self._get_list_of_identifiers(
                        value_node
                    ) or _stringify_ast_node(value_node)
                else:
                    kwargs[arg_name] = _stringify_ast_node(value_node)
        return kwargs

    def _parse_config_arg(
        self, node: ast.AST, base_dir: str
    ...
    def _find_return_call(
        self, body_nodes: List[ast.AST], target_class_fqn: str
    ) -> Optional[ast.Call]:
            ...
            if isinstance(stmt, ast.Return) and isinstance(stmt.value, ast.Call):
                ...
                if (
                    ...
                    or (
                        ...
                        and target_class_fqn.endswith(f".{call_node.func.id}")
                    )
                ):
                    return call_node
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        if node.module:
            ...
            for alias in node.names:
                ...
                if base_module == "crewai_tools" or base_module.startswith(
                    "crewai_tools."
                ):
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        for decorator in node.decorator_list:
            ...
            if (
                dec_name == "CrewBase"
                or self._resolve_fqn_from_node(decorator) == cb_decorator_name
            ):
                is_crew_base = True
                break
        ...
        for base in node.bases:
            ...
            if resolved_base_fqn == bt_class_name or (
        ...
        if (
            self.current_class_name == node.name
        ):  # Check if we're back from visiting this class
            init_method = next(
                (
                    n
                    for n in node.body
                    ...
                    if isinstance(stmt, ast.Assign) and len(stmt.targets) == 1:
                        ...
                        if (
                            ...
                            if tool_class_name and (
                                tool_class_name in self.tool_imports
                                or tool_class_name in self.tools_info
                            ):
                                self.tool_instance_map[node.name][
                                    attr_name
                                ] = value_node
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        base_dir = (
            os.path.dirname(self.current_filepath) if self.current_filepath else "."
        )
        ...
        for decorator in node.decorator_list:
            ...
            if self.current_crew_base_info and (
                decorator_fqn == agent_decorator or decorator_name == "agent"
            ):
                is_crewai_decorated = True
                agent_call = self._find_return_call(
                    node.body, self.crewai_fqns["Agent"]
                )
                if agent_call:
                    agent_id = func_name
                    ...
                    if config_kw_node:
                        config_ref, config_data = self._parse_config_arg(
                            config_kw_node, base_dir
                        )
            ...
            elif self.current_crew_base_info and (
                decorator_fqn == task_decorator or decorator_name == "task"
            ):
                is_crewai_decorated = True
                ...
                if task_call:
                    ...
                    agent_var = (
                        None  # Explicitly set to None for decorator tasks initially
                    )
                    agent_kw_node = next(
                    ...
                    self.tasks_info[task_id] = {
                        ...
                        "description": kwargs.get(
                            "description",
                            (
                                config_data.get("description", task_id)
                                if config_data
                                else task_id
                            ),
                        ),
                        "context_vars": (
                            context_vars if isinstance(context_vars, list) else []
                        ),
                ...
                decorator_fqn == crew_decorator or decorator_name == "crew"
            ):
                is_crewai_decorated = True
                ...
                or (
                    ...
                    and get_potential_fqn(decorator.func) == "tool"
                )
            ):
                is_crewai_decorated = True
                tool_name = func_name
                tool_description = (
                    func_docstring or f"Custom tool function: {tool_name}"
                )
                self.tools_info[tool_name] = {
                    "name": tool_name,
                    "function_name": tool_name,
        ...
        if len(node.targets) == 1:
            ...
            if (
                ...
                if tool_class_name and (
                    ...
                    self.tool_instance_map[self.current_class_name][
                        attr_name
                    ] = value_node
                ...
                if isinstance(value_node, ast.List):
                    ...
                    if func_fqn == agent_fqn or func_name_simple == "Agent":
                        ...
                        self.agents_info[target_var] = {
                            ...
                            "config_data": config_data,
                            "tools_vars": (
                                tool_refs if isinstance(tool_refs, list) else []
                            ),
                            "metadata_kwargs": kwargs,
                            "is_decorator": False,
                            "class_context": self.current_class_name,
                        ...
                        agent_var = (
                            agent_var_raw if isinstance(agent_var_raw, str) else None
                        )
                        ...
                        if tool_class_name and tool_class_name in self.tool_imports:
                            if tool_class_name in self.tools_info:
                                self.tools_info[tool_class_name][
                                    "var_name"
                                ] = target_var
                                self.tools_info[tool_class_name][
                                    "assign_location"
                                ] = assign_location
                                self.tools_info[target_var] = self.tools_info[
                                    tool_class_name
                                ]
                            else:
    ...
    def finalize_graph(self):
        ...
        for agent_id, agent_data in self.agents_info.items():
            ...
            if agent_data.get("config_data"):
                metadata["role"] = agent_data["config_data"].get(
                    "role", metadata.get("role")
                )
                metadata["goal"] = agent_data["config_data"].get(
                    "goal", metadata.get("goal")
                )
                metadata["backstory"] = agent_data["config_data"].get(
        ...
        for task_id, task_data in self.tasks_info.items():
            ...
            if task_data.get("config_data"):
                metadata["description"] = task_data["config_data"].get(
                    "description", metadata.get("description")
                )
            metadata["config_ref"] = task_data.get("config_ref")
            metadata["config_resolved"] = bool(task_data.get("config_data"))
            metadata["agent"] = task_data.get(
                "agent_var"
            )  # Keep the potentially null agent_var here
            ...
            metadata["dependencies"] = task_data.get(
                "dependencies"
            )  # Add dependencies to metadata
            node_description = metadata.get("description") or task_data.get(
                "description"
            )
        ...
        for tool_ref, tool_data in self.tools_info.items():
            ...
            if node_name not in final_tool_nodes:
                ...
                final_tool_nodes[node_name] = {
                    ...
                    "source_location": tool_data.get("location")
                    or tool_data.get("assign_location"),
                    "metadata": metadata,
                }
        ...
        for tool_ref in all_tool_refs_found:
            ...
            if isinstance(tool_ref, str) and tool_ref.startswith("self."):
                ...
                if class_context and attr_name in self.tool_instance_map.get(
                    class_context, {}
                ):
                    ...
                    if isinstance(value_node, ast.Call):
                        ...
                        value_node, ast.Name
                    ):  # Assigned from another var/tool func
            ...
            if (
                not resolved_node_name
                and tool_ref in self.tool_imports
                and tool_ref not in tool_node_map
            ):
        ...
        for task_id, task_data in self.tasks_info.items():
            ...
            if task_data.get("is_decorator") and not agent_node_name:
                ...
                if (
                    ...
                    and self.agents_info[task_name].get("class_context")
                    == class_context
                ):
        ...
        for crew_id, crew_data in self.crews_info.items():
            ...
            if isinstance(task_sequence_refs, list):
                ...
                if task_sequence_refs in self.task_lists:
                    actual_task_sequence = self.task_lists[task_sequence_refs].get(
                        "tasks", []
                    )
                    if not crew_def_location:
                        crew_def_location = self.task_lists[task_sequence_refs].get(
                            "location"
                        )
            ...
            valid_task_node_names = [
                t for t in actual_task_sequence if t in self.tasks_info
            ]
            ...
            if process_type == CrewProcess.SEQUENTIAL:
                ...
                self._add_output_edge(
                    "Start", valid_task_node_names[0], start_condition, crew_metadata
                )
                ...
                self._add_output_edge(
                    valid_task_node_names[-1], "End", end_condition, crew_metadata
                )
                ...
                for task_node_name in valid_task_node_names:
                    self._add_output_edge(
                        "Start", task_node_name, start_condition, crew_metadata
                    )
                    self._add_output_edge(
                        task_node_name, "End", end_condition, crew_metadata
                    )
            ...
            if isinstance(context_vars, list):
                for ctx_task_var in context_vars:
                    if ctx_task_var in self.tasks_info:
                        ...
                        self._add_output_edge(
                            ctx_task_var, task_id, condition, metadata
                        )
            # Dependency Edges
            ...
            if isinstance(dependencies, list):
                for dep_task_name in dependencies:
                    if dep_task_name in self.tasks_info:
                        ...
                        metadata = {
                            "definition_location": task_def_location
                        }  # Dependency defined in target task
                        # Edge direction: Dependency -> Dependent Task
                        self._add_output_edge(
                            dep_task_name, task_id, condition, metadata
                        )
...
def extract_crewai_graph(directory_path: str, output_file: str):
    ...
    for root, _, files in os.walk(directory_path):
        ...
        if any(
            part.startswith(".")
            or part in ["venv", "env", "__pycache__", "node_modules", ".git"]
    ...
    if graph_structure["nodes"] or graph_structure["edges"]:
        try:
            graph_structure["nodes"].sort(
                key=lambda x: (x.get("node_type", ""), x.get("name", ""))
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract CrewAI structures from Python code into a graph JSON."
    )
    parser.add_argument(
        "directory",
        type=str,
