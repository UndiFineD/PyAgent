# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\crewai\echo_synth\flows\app_flow.py
import os
import traceback
...
class EchoSynthFlow(Flow[EchoSynthState]):
    ...
    def process_audio(self) -> None:
        ...
        try:
            ...
            initial_context = {"audio_file_path": os.path.abspath(self.audio_file_path)}
            ...
            self.state.transcribed_text = self._get_task_output_by_name(
                results, "convert_audio_to_text"
            )
            self.state.speech_text = self._get_task_output_by_name(
                results, "write_speech"
            )
            self.state.summary_text = self._get_task_output_by_name(
                results, "write_summary"
            )
            self.state.summary_for_image = self._get_task_output_by_name(
                results, "write_image_summary"
            )
            self.state.image_file = self._get_task_output_by_name(
                results, "create_image"
            )
    ...
    def save_output(self) -> None:
        ...
        try:
            ...
            results = json_crew.kickoff(
                inputs={"state_data": state_dict, "file_id": self.state.id}
            ).tasks_output

            # Get the json file path from the results
            self.state.json_file_path = self._get_task_output_by_name(
