# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\autogen\autogen.py
import asyncio
import json
import logging
...
class BookingManager(RoutedAgent):
    ...
    def __init__(self, model_client: ChatCompletionClient):
        super().__init__(
            "Travel booking manager that routes requests to specialized agents"
        )
        self._system_messages: List[LLMMessage] = [
            SystemMessage(
                content="You are a helpful travel booking manager. "
    ...
    async def handle_user_message(
        ...
        create_result = await self._model_client.create(
            messages=session, cancellation_token=ctx.cancellation_token
        )
...
class FlightAgent(RoutedAgent):
    ...
    def __init__(self, model_client: ChatCompletionClient, tool_schema: List[Tool]):
        ...
        self._system_messages: List[LLMMessage] = [
            SystemMessage(
                content="You are a flight booking specialist. Help users find flights."
            )
        ]
        self._model_client = model_client
...
class HotelAgent(RoutedAgent):
    ...
    def __init__(self, model_client: ChatCompletionClient, tool_schema: List[Tool]):
        ...
        self._system_messages: List[LLMMessage] = [
            SystemMessage(
                content="You are a hotel booking specialist. Help users find accommodations."
            )
        ]
        self._model_client = model_client
        self._tools = tool_schema
...
class CarRentalAgent(RoutedAgent):
    ...
    def __init__(self, model_client: ChatCompletionClient, tool_schema: List[Tool]):
        ...
        self._system_messages: List[LLMMessage] = [
            SystemMessage(
                content="You are a car rental specialist. Help users find rental cars."
            )
        ]
        self._model_client = model_client
        self._tools = tool_schema
    ...
    async def handle_user_message(
        ...
        # Simple implementation for brevity
        return Message(
            content="I can help you book a rental car. Here are some options..."
        )
...
class CostCalculatorAgent(RoutedAgent):
    ...
    def __init__(self, model_client: ChatCompletionClient, tool_schema: List[Tool]):
        ...
        self._system_messages: List[LLMMessage] = [
            SystemMessage(
                content="You are a travel cost calculator. Help users compare prices."
            )
        ]
        self._model_client = model_client
        self._tools = tool_schema
...
async def search_rental_cars(
    location: str, pickup_date: str, return_date: str
) -> Dict[str, Any]:
...
async def calculate_trip_cost(
    flights: List[Dict], hotels: List[Dict], cars: List[Dict]
) -> Dict[str, Any]:
...
async def main():
    ...
    hotel_tools = [
        FunctionTool(
            get_hotel_prices,
            description="Get hotel prices for a specific location and dates.",
        )
    ]
    car_tools = [
        FunctionTool(
            search_rental_cars,
            description="Search for rental cars based on location and dates.",
        )
    ]
    cost_tools = [
        FunctionTool(
            calculate_trip_cost,
            description="Calculate the total cost of a trip based on selected options.",
        )
    ]

    # Register agents
    await BookingManager.register(
        runtime, "booking_manager", lambda: BookingManager(model_client)
    )
    await FlightAgent.register(
        runtime, "flight_agent", lambda: FlightAgent(model_client, flight_tools)
    )
    await HotelAgent.register(
        runtime, "hotel_agent", lambda: HotelAgent(model_client, hotel_tools)
    )
    await CarRentalAgent.register(
        runtime, "car_rental_agent", lambda: CarRentalAgent(model_client, car_tools)
    )
    await CostCalculatorAgent.register(
        runtime,
        "cost_calculator_agent",
        lambda: CostCalculatorAgent(model_client, cost_tools),
    )
    ...
    await runtime.send_message(
        Message(
            content="I need to book a flight from New York to Los Angeles on June 15th."
        ),
        manager_id,
    )
