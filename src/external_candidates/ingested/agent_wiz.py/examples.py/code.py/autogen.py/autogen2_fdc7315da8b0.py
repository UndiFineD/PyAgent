# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\autogen\autogen2.py
import asyncio
import json
import logging
...
class MathGreetAgent(RoutedAgent):
    ...
    def __init__(self, model_client: ChatCompletionClient, tool_schema: List[Tool]):
        ...
        self._system_messages: List[LLMMessage] = [
            SystemMessage(
                content="You are a helpful assistant that can perform math calculations and provide greetings."
            )
        ]
...
async def main():
    ...
    tools = [
        ...
        FunctionTool(
            greet_and_calculate,
            description="Greets a person and performs a calculation for them.",
        ),
    ]

    # Register the single agent
    await MathGreetAgent.register(
        runtime, "math_greet_agent", lambda: MathGreetAgent(model_client, tools)
    )
