# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\openai_agents\agent.py
...
async def get_product_info(product_sku: str) -> str:
    ...
    products = {
        "SKU-TECH-001": {
            ...
            "price": 25.99,
            "stock": 150,
            "desc": "A reliable ergonomic wireless mouse.",
        },
        "SKU-TECH-002": {
            "name": "Mechanical Keyboard",
            "price": 79.99,
            "stock": 50,
            "desc": "A backlit mechanical keyboard with blue switches.",
        },
        "SKU-HOME-001": {
            "name": "Coffee Mug",
            "price": 12.50,
            "stock": 0,
            "desc": "A ceramic coffee mug with our logo.",
        },
    }
    ...
    if info:
        stock_status = (
            f"In Stock ({info['stock']} available)"
            if info["stock"] > 0
            else "Out of Stock"
        )
        return (
        ...
        return (
            f"Sorry, I could not find any information for product SKU: {product_sku}."
        )


async def on_order_status_handoff(
        ...
        with trace("E-commerce Support Turn", group_id=conversation_id):
            ...
            for new_item in result.new_items:
                ...
                if isinstance(new_item, MessageOutputItem):
                    ...
                    print(
                        f"--- System: {agent_name} calling tool `{tool_name}` with args: {args} ---"
                    )
                elif isinstance(new_item, ToolCallOutputItem):
                    pass  # Often the agent summarizes this in its next message
                else:
                    print(
                        f"--- System: {agent_name} produced item: {new_item.__class__.__name__} ---"
                    )
