# Extracted from: C:\DEV\PyAgent\.external\agent_wiz\examples\code\langgraph\lg.py
#!/usr/bin/env python3
...
def update_dates(file: str) -> str:
    ...
    if not os.path.exists(backup_file):
        raise FileNotFoundError(
            f"Backup file {backup_file} not found. Cannot update dates."
        )
    ...
    if pd.isna(example_time_str):
        print(
            "Warning: No valid 'actual_departure' times found to calculate time difference."
        )
        conn.close()
        return file
    ...
    try:
        ...
        if example_time.tz is None:
            ...
            example_time = pd.to_datetime(example_time_str).tz_localize(
                "UTC"
    ...
    current_time = pd.to_datetime("now").tz_convert(
        example_time.tz
    )  # Convert current time to example time's timezone
...
def update_ticket_to_new_flight(
    ...
    try:
            ...
            try:
                ...
                if departure_time.tzinfo is None:
                    ...
                    try:
                        ...
                        departure_time = departure_time.tz_localize(
        ...
        try:
            ...
            pass  # Ignore rollback error if connection is already bad
    finally:
        cursor.close()
        conn.close()
...
def book_car_rental(rental_id: int) -> str:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            result_message = (
                f"No car rental found with ID {rental_id} or it was already booked."
            )
    except sqlite3.Error as e:
...
def cancel_car_rental(rental_id: int) -> str:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            result_message = (
                f"No car rental found with ID {rental_id} or it was already cancelled."
            )
    except sqlite3.Error as e:
        result_message = f"Database error: {e}"
        conn.rollback()
...
def book_hotel(hotel_id: int) -> str:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            result_message = (
                f"No hotel found with ID {hotel_id} or it was already booked."
            )
    except sqlite3.Error as e:
        result_message = f"Database error: {e}"
        conn.rollback()
...
def cancel_hotel(hotel_id: int) -> str:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            result_message = (
                f"No hotel found with ID {hotel_id} or it was already cancelled."
            )
    except sqlite3.Error as e:
        result_message = f"Database error: {e}"
        conn.rollback()
...
def update_excursion(recommendation_id: int, details: str) -> str:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            result_message = (
                f"No trip recommendation found with ID {recommendation_id}."
            )
    except sqlite3.Error as e:
        result_message = f"Database error: {e}"
        conn.rollback()
...
def cancel_excursion(recommendation_id: int) -> str:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            result_message = (
                f"Trip recommendation {recommendation_id} successfully cancelled."
            )
        else:
...
class Assistant:
    ...
    def __call__(self, state: Dict, config: RunnableConfig):
        ...
        while True:
            ...
            current_state = {
                k: v for k, v in state.items() if k != "error"
            ...
            if not result.tool_calls:
                if not result.content:
                    empty_response = True
                elif isinstance(result.content, list) and not result.content[0].get(
                    "text"
                ):
                    empty_response = True
...
part_1_assistant_runnable = primary_assistant_prompt_part1 | llm.bind_tools(
    part_1_tools
)
...
print("\n=== Part 1 Review ===")
print(
    "Part 1 agent responded but made decisions without confirmation and struggled with some searches."
)
    ...
    while snapshot and snapshot.next:  # Check if snapshot is not None
        ...
        if user_input.strip().lower() == "y":
            ...
            if current_step_event and current_step_event["messages"][-1].tool_calls:
                ...
                events = part_2_graph.stream(
                    {"messages": [tool_message]}, config_part2, stream_mode="values"
                )
            else:
                print(
                    "Error: Could not get tool_call_id for feedback. Continuing without feedback."
                )
                events = part_2_graph.stream(None, config_part2, stream_mode="values")
...
print("\n=== Part 2 Review ===")
print(
    "Part 2 agent fetched user info automatically and allowed user confirmation before tool use."
)
...
def route_tools_part3(
    state: StatePart3,
) -> Literal["safe_tools", "sensitive_tools", END]:
    ...
    if not ai_message.tool_calls:
        return (
            END  # Should not happen if tools_condition didn't return END, but safeguard
        )
...
for i, question in enumerate(tutorial_questions):
    ...
    while snapshot and snapshot.next:  # Check if snapshot is not None
        ...
        if user_input.strip().lower() == "y":
            ...
            print("User disapproved or requested changes. Providing feedback...")
            if (
                current_step_event_part3
                and current_step_event_part3["messages"][-1].tool_calls
            ):
                tool_call_id = current_step_event_part3["messages"][-1].tool_calls[0][
                    "id"
                ]
                tool_message = ToolMessage(
                    tool_call_id=tool_call_id,
                ...
                events = part_3_graph.stream(
                    {"messages": [tool_message]}, config_part3, stream_mode="values"
                )
            else:
                print(
                    "Error: Could not get tool_call_id for feedback. Continuing without feedback."
...
def wrapped_user_info_node_part4(state: StatePart4, config: RunnableConfig):
    ...
    initial_state = (
        {"dialog_state": "assistant"} if not state.get("dialog_state") else {}
    )
...
builder_part4.add_node(
    "book_hotel_safe_tools", create_tool_node_with_fallback(book_hotel_safe_tools)
)
builder_part4.add_node(
...
def route_book_hotel(
    state: StatePart4,
) -> Literal["book_hotel_sensitive_tools", "book_hotel_safe_tools", "leave_skill", END]:
    route = tools_condition(state)
    if route == END:
        return END
    tool_calls = state["messages"][-1].tool_calls
...
def route_book_excursion(
    ...
    "book_excursion_sensitive_tools", "book_excursion_safe_tools", "leave_skill", END
]:
    route = tools_condition(state)
    if route == END:
        return END
...
def route_primary_assistant(
    ...
    if first_tool_name == ToFlightBookingAssistant.__name__:
        return "enter_update_flight"
    if first_tool_name == ToBookCarRental.__name__:
        return "enter_book_car_rental"
    if first_tool_name == ToHotelBookingAssistant.__name__:
        return "enter_book_hotel"
    if first_tool_name == ToBookExcursion.__name__:
    ...
    print(
        f"Warning: Unhandled tool call '{first_tool_name}' in primary assistant routing."
    )
...
def route_to_workflow(
    ...
    if not dialog_state:
        print(
            "Warning: dialog_state is empty in route_to_workflow. Defaulting to primary_assistant."
        )
...
if __name__ == "__main__":
    ...
    for i, question in enumerate(tutorial_questions):
        ...
        while snapshot and snapshot.next:  # Check if snapshot is not None
            ...
            if user_input.strip().lower() == "y":
                ...
                print("User disapproved or requested changes. Providing feedback...")
                if (
                    current_step_event_part4
                    and current_step_event_part4["messages"][-1].tool_calls
                ):
                    tool_call_id = current_step_event_part4["messages"][-1].tool_calls[
                        0
                    ]["id"]
                    tool_message = ToolMessage(
                        tool_call_id=tool_call_id,
                    ...
                    events = part_4_graph.stream(
                        {"messages": [tool_message]}, config_part4, stream_mode="values"
                    )
                else:
                    print(
                        "Error: Could not get tool_call_id for feedback. Continuing without feedback."
                    ...
                    events = part_4_graph.stream(
                        None, config_part4, stream_mode="values"
                    )
    ...
    print(
        "Further improvements would involve adding evaluations (e.g., using LangSmith)."
    )
