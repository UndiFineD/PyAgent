        ...
        try:
            ...
            if assets:
                for asset in assets:
                    ...
                    validated_asset_dict["created_timestamp"] = (
                        CommonUtils.get_ikaros_std_timestamp()
                    )
        ...
        try:
            if findings:
                for finding in findings:
                    ...
                    finding["created_timestamp"] = (
                        CommonUtils.get_ikaros_std_timestamp()
                    )
                    if asset is not None:
                        finding["app"] = CrudUtils.assign_app_context(asset)
                    else:
                        finding["app"] = CrudUtils.assign_app_context(
                            finding[app_context_param]
                        )
    ...
    async def update_asset(asset: str, org: str, tool_output_dict: dict):
        ...
        for key in tool_output_dict:
            if key in Assets.__fields__:
                ...
                logging.warning(
                    f"{key} does not exist in database model. This key will be ignored"
                )
