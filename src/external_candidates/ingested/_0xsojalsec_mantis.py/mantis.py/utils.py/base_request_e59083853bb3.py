        ...
        try:
            if method == "POST":
                if headers is not None:
                    ...
                    response = session.post(
                        url, data=body, timeout=BaseRequestExecutor.TIMEOUT, verify=True
                    )

                logging.debug(
                    f"Response code for {url} : {response.status_code}, {response.request}"
                )
                ...
                if headers is not None:
                    ...
                    response = session.get(
                        url, verify=True, timeout=BaseRequestExecutor.TIMEOUT
                    )
