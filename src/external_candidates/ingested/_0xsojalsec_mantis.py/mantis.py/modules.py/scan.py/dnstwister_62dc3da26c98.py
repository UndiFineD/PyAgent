# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\scan\DNSTwister.py
import json
import logging
...
from mantis.utils.tool_utils import (
    get_assets_by_field_value,
    get_assets_with_non_empty_fields,
)
...
class DNSTwister(ToolScanner):
    ...
    async def get_commands(self, args: ArgsModel):
        ...
        self.assets = await get_assets_by_field_value(
            self, args, "stale", False, constants.ASSET_TYPE_TLD
        )
        ...
        try:
            if report_dict:
                ...
                for phishing_domain in report_dict:
                    ...
                    if (
                        "fuzzer" in phishing_domain
                        and phishing_domain["fuzzer"]
                        != constants.DNS_TWISTER_FUZZER_ORIGINAL
                    ):
                        finding_dict = {}
                        ...
                        try:
                            ...
                            if finding_dict["url"] not in self.db_assets:
                                phishing_domain_response = session.get(
                                    "http://" + phishing_domain["domain"]
                                )
                                # print("response", phishing_domain_response.status_code)
                                if phishing_domain_response.status_code in [
                                    200,
                                    302,
                                    301,
                                ]:
                                    ...
                                    finding_dict["others"][
                                        "status_code"
                                    ] = phishing_domain_response.status_code
                            else:
                                finding_dict["falsepositive"] = True
                ...
                logging.warning(
                    "DNSTwister output file found, but no vulnerabilities were reported"
                )
        except Exception as e:
            print(e)
