# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\scan\Nuclei.py
import json
import logging
...
"""
Nuclei Module is running the nuclei scans excluding technology scan
Output: JSON file
...
class Nuclei(ToolScanner):
    ...
    async def get_commands(self, args: ArgsModel):
        ...
        self.base_command = "nuclei -u {input_domain} -jle {output_file_path} -v -et http/technologies/ -exclude-severity info -hm"
        ...
        self.assets.extend(
            await get_assets_with_empty_fields(self, args, "active_hosts")
        )
        ...
        for every_asset in self.assets:
            if "_id" in every_asset:
                ...
                outfile = CommonUtils.generate_unique_output_file_name(
                    every_asset, self.outfile_extension
                )
                command = self.base_command.format(
                    input_domain=every_asset, output_file_path=outfile
                )
    ...
    def parse_report(self, outfile):
        ...
        if report_dict:
            ...
            logging.debug(
                "Nuclei output file found, but no vulnerabilities were reported"
            )
