# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\alerter.py
import copy
import logging
import sys
...
from mantis.utils.notifications import Notifications, NotificationsUtils
...
class Alerter:
    ...
    async def send_alerts(log_dict, args):
        try:
            ...
            if log_dict is not None:
                scan_efficiency_blocks, scan_stats, module_scan_stats = (
                    Alerter.get_stats_slack_message(log_dict)
                )

            for team in notify_config:
                asset_type_list, asset_tag_list = (
                    NotificationsUtils.get_assets_to_notify_list(team.teamName)
                )
                finding__type_list, finding_tag_list = (
                    NotificationsUtils.get_findings_to_notify_list(team.teamName)
                )
                assets, findings = await Alerter.fetch_results(
                    asset_types=asset_type_list,
                    finding_types=finding__type_list,
                    args=args,
                    app=team.app,
                )
                slack_blocks = await Alerter.get_inventory_slack_message(
                    assets, findings, asset_tag_list, finding_tag_list
                )
                        ...
                        if isinstance(team.channel[channel_type], list):
                            for webhook in team.channel[channel_type]:
                                if team.scanEfficiency == True:
                                    Notifications.send_slack_notifications(
                                        scan_efficiency_blocks, webhook
                                    )
                                # for block in slack_blocks:

                                Notifications.send_slack_notifications(
                                    slack_blocks, webhook
                                )
                        else:
                            logging.error(
                                "Slack must provide list of webhooks, check local.yml"
                            )
    ...
    @staticmethod
    async def get_inventory_slack_message(
        assets, findings, asset_tag_list, finding_tag_list
    ):
        blocks = []
                ...
                if "asset_info" in asset_type:
                    ...
                    for asset in asset_type["asset_info"]:
                        ...
                        if "friendly_name" in asset:
                            asset_section["text"][
                                "text"
                            ] += f"| {asset['friendly_name']}"
                        asset_section["text"]["text"] += "\n"
                    blocks.append(asset_section)
            total_new_assets_count = copy.deepcopy(section)
            total_new_assets_count["text"][
                "text"
            ] = f"Total New Assets Discovered: {total_new_assets}"
        ...
        if len(findings):
            ...
            total_new_findings_count["text"][
                "text"
            ] = f"Total New Findings Discovered: {total_new_findings}"
    ...
    async def fetch_results(asset_types, finding_types, args, app):
        assets = await Alerter.get_assets_by_type_discovered(
            asset_types=asset_types, args=args, app=app
        )
        findings = await Alerter.get_findings_by_type_discovered(
            finding_types=finding_types, args=args, app=app
        )
    ...
    async def get_assets_by_type_discovered(asset_types, args, app):
        ...
        if len(app):
            pipeline_type_discovered.append({"$match": {"app": {"$in": app}}})
        if args is not None:
            pipeline_type_discovered.append({"$match": {"org": args.org}})
        pipeline_type_discovered.extend(
            [
                {"$match": {"asset_type": {"$in": asset_types}}},
                {
                    "$match": {
                        "created_timestamp": {
    ...
    def get_stats_slack_message(logs):
        ...
        for modules in logs.scan_modules_logs:
            ...
            if success + failure != 0:
                module_scan_stats["module_efficiency"] = (
        ...
        scan_stats["scan_percentage"] = scan_stats["scan_percentage"] / (
            len(logs.scan_modules_logs) - no_scan_efficiency_modules
        )
        ...
        command_executed_section["text"][
            "text"
        ] = f"*Command Executed*: `python3 {' '.join(sys.argv)}`"
        blocks.append(command_executed_section)
        scan_stats_section = copy.deepcopy(section)
        scan_stats_section["text"][
            "text"
        ] = f"•Scan Efficiency: {scan_stats['scan_percentage']}%\n"
        scan_stats_section["text"][
            "text"
        ] += f"•Scan Started: {scan_stats['scan_start']}\n"
        scan_stats_section["text"]["text"] += f"•Scan Ended: {scan_stats['scan_end']}\n"
        scan_stats_section["text"][
            "text"
        ] += f"•Scan Time Taken: {scan_stats['scan_time_taken']}\n"
        blocks.append(scan_stats_section)
