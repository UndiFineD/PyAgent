# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\prerecon\IPinfo.py
import json
import logging

from mantis.constants import ASSET_TYPE_IP, ASSET_TYPE_SUBDOMAIN, ASSET_TYPE_TLD
...
class IPinfo(ToolScanner):
    ...
    async def get_commands(self, args: ArgsModel):
        ...
        if self.token is None:
            logging.warning(
                "IPinfo token not provided, IPinfo will not run successfully"
            )
            raise Exception("IPinfo token not provided!!")
        self.org = args.org
        self.std = "PIPE"
        self.base_command = (
            "dig +short {input_domain} | ipinfo bulk  --token "
            + self.token
        ...
        self.assets.extend(
            await get_assets_grouped_by_type(self, args, ASSET_TYPE_SUBDOMAIN)
        )
