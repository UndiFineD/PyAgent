# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\dns\Route53.py
import logging
import os
...
if ConfigProvider.get_config().aws.credentials_path is not None:
    os.environ["AWS_SHARED_CREDENTIALS_FILE"] = (
        ConfigProvider.get_config().aws.credentials_path
    )
else:
...
class Route53(BaseScanner):
    ...
    async def execute(self, tooltuple):
        logging.info(
            f"Using credentials from {os.environ['AWS_SHARED_CREDENTIALS_FILE']}"
        )
    ...
    async def main(self):
        ...
        try:
            for profile in self.args.aws_profiles:
                try:
                    logging.info(
                        "[+] Enumerating subdomains for profiles - {}".format(profile)
                    )
    ...
    def get_hosted_zone_ids(self, client, profile):
        logging.info(
            "[+] Getting all the hosted zones on Route53 instance for {}".format(
                profile
            )
        )
        hosted_zones = []
        ...
        if ("HostedZones" in response.keys() and len(response["HostedZones"])) > 0:
            ...
            logging.error(
                "[+] Error in accessing hosted zones on Route53 instance for {}".format(
                    profile
                )
            )
    ...
    def get_resource_records(self, hosted_zones, client, profile):
        ...
        for hosted_zone in hosted_zones:
            logging.error(
                "[+] Getting all the DNS resource records for {} - {}".format(
                    hosted_zone, profile
                )
            )
            paginated_resource_record_sets = paginator.paginate(
    ...
    def get_subdomains(self, resource_records, subdomains, profile):
        logging.info(
            "[+] Parsing the subdomains and extracting subdomains - {}".format(profile)
        )
