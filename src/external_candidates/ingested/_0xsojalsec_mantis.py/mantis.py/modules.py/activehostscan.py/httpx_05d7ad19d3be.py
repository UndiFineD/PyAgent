# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\activehostscan\HTTPX.py
import json
import logging
...
class HTTPX(ToolScanner):
    ...
    async def get_commands(self, args: ArgsModel):
        self.org = args.org
        self.base_command = (
            "httpx -u {input_domain} -asn -json -o {output_file_path} -cname -td"
        )
            ...
            if "domains" in report_dict["csp"]:
                for csp_domain in report_dict["csp"]["domains"]:
                    ...
                    for tld in self.db_assets:
                        if (
                            domain_tld != tld
                            and tld.split(".")[0] in third_party_domain
                        ):
                            third_party_integration = {}
                            third_party_integration["asset"] = third_party_domain
                            third_party_integration["asset_type"] = (
                                ASSET_TYPE_THIRD_PARTY
                            )
                            third_party_integration["org"] = self.org
                            self.third_party_integrations.append(
                                third_party_integration
                            )
