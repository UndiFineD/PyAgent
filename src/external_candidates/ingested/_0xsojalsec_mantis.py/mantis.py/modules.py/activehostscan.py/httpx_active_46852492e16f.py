# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mantis\mantis\modules\activehostscan\HTTPX_Active.py
import json
import logging
...
class HTTPX_Active(ToolScanner):
    ...
    async def get_commands(self, args: ArgsModel):
        ...
        for every_asset in self.assets:
            ...
            outfile = CommonUtils.generate_unique_output_file_name(
                domain, self.outfile_extension
            )
            command = self.base_command.format(
                subdomain=domain, output_file_path=outfile, ports_comma_seperated=ports
            )
    ...
    async def db_operations(self, tool_output_dict, asset):
        await CrudUtils.update_asset(
            asset=asset, org=self.org, tool_output_dict=tool_output_dict
        )
