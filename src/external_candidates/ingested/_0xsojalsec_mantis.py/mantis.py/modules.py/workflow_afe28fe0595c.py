...
class Workflow:
    ...
    def get_all_classes(module):
        ...
        try:
            for tool in module.tools:
                ...
                curr_package = (
                    sys.modules[__name__].__package__ + "." + module.moduleName
                )
    ...
    async def workflow_executor(args: ArgsModel):
        ...
        if args.input:
            ...
            assets_with_type = AssetType.assign_asset_type(
                assets=list_assets, args=args
            )
            asset_dict_list = CrudUtils.create_assets_dict(
                args, assets_with_type=assets_with_type
            )
        ...
        subdomain_assets = await get_assets_grouped_by_type(
            None, args, ASSET_TYPE_SUBDOMAIN
        )
        ...
        for module in ordered_modules:
            ...
            module_classes = Workflow.get_all_classes(
                ConfigUtils.get_module(module_name=module, workflow=workflow)
            )
            ...
            try:
                for _class in module_classes:
                    try:
                        ...
                        logging.debug(
                            f"Command list for {_class.__name__} : {tool_command_list}"
                        )
                    except Exception as e:
                        logging.error(
                            f"Something went wrong in {_class.__name__} : {e}"
                        )
                ...
                if args.use_ray:
                    ...
                    futures_res = []
                    with tqdm(
                        total=len(commands_list), desc=module.upper(), colour="green"
                    ) as pbar:
                        futures = []
                        while len(commands_list):
                            while len(futures) < num_actors + 1 and len(commands_list):
                                ...
                                futures.append(
                                    executeScanActor.execute_and_store.remote(
                                        commands_list.pop()
                                    )
                                )
                    ...
                    with tqdm(total=len(commands_list), colour="green") as pbar:
                        ...
                        for tool_tuple in commands_list:
                            ...
                            task = loop.create_task(
                                run_with_semaphore(
                                    execute_threadpool_obj.execute_and_store(tool_tuple)
                                )
                            )
                            tasks.append(task)
        ...
        scan_stat["scan_time_taken"] = str(
            timedelta(seconds=round(scan_end_time - scan_start_time, 0))
        )
        ...
        scan_stats, module_scan_stats = await Alerter.send_alerts(
            log_dict=scan_stat_validated, args=args
        )
        module_stats = ""
