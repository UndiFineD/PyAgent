# Extracted from: C:\DEV\PyAgent\.external\APT-Hunter\APT-Hunter_old.py
import argparse
import glob
import logging
import os
...
def evtxdetect():
    ...
    try:
        ...
        logging.error(traceback.format_exc())
    try:
        EvtxDetection.detect_events_powershell_operational_log(
            powershellop_path, input_timezone
        )
    except IOError:
    ...
    try:
        ...
        logging.error(traceback.format_exc())
    try:
        EvtxDetection.detect_events_TerminalServices_LocalSessionManager_log(
            terminal_path, input_timezone
        )
    except IOError:
        ...
        logging.error(traceback.format_exc())
    try:
        EvtxDetection.detect_events_scheduled_task_log(
            scheduledtask_path, input_timezone
        )
    except IOError:
...
def csvdetect(winevent):
    ...
    try:
        ...
        logging.error(traceback.format_exc())
    try:
        CSVDetection.detect_events_powershell_operational_log(
            powershellop_path, winevent
        )
    except IOError:
        print("Error Analyzing Powershell Operational logs : ", end="")
        print("File Path Does Not Exist")
    ...
    try:
        ...
        logging.error(traceback.format_exc())
    try:
        CSVDetection.detect_events_TerminalServices_LocalSessionManager_log(
            terminal_path, winevent
        )
    except IOError:
        print("Error Analyzing TerminalServices LocalSessionManager logs: ", end="")
        print("File Path Does Not Exist")
...
def evtxdetect_auto():
    ...
    try:
        EvtxDetection.detect_events_powershell_operational_log(
            powershellop_path_list, input_timezone
        )
    except IOError:
        print("Error Analyzing Powershell Operational logs : ", end="")
        print("File Path Does Not Exist")
    ...
    try:
        EvtxDetection.detect_events_TerminalServices_LocalSessionManager_log(
            terminal_path_list, input_timezone
        )
    except IOError:
        print("Error Analyzing TerminalServices LocalSessionManager logs: ", end="")
        print("File Path Does Not Exist")
    ...
    try:
        EvtxDetection.detect_events_scheduled_task_log(
            scheduledtask_path_list, input_timezone
        )
    except IOError:
        print("Error Analyzing Scheduled Task logs : ", end="")
        print("File Path Does Not Exist")
    ...
    try:
        EvtxDetection.detect_events_windows_defender_log(
            defender_path_list, input_timezone
        )
    except IOError:
        print("Error Analyzing Windows Defender logs : ", end="")
        print("File Path Does Not Exist")
        ...
        logging.error(traceback.format_exc())
    try:
        EvtxDetection.detect_events_Microsoft_Windows_WinRM(
            winrm_path_list, input_timezone
        )
    except IOError:
        print("Error Analyzing WinRM logs : ", end="")
        print("File Path Does Not Exist")
...
def report():
    ...
    allresults = allresults.rename(
        columns={"Date and Time": "datetime", "Detection Rule": "message"}
    )
    ...
    Powershell_Operational.to_excel(
        writer, sheet_name="Powershell_Operational Events", index=False
    )
    ...
    Terminal_Services_Summary.to_excel(
        writer, sheet_name="Terminal Services Logon Summary", index=False
    )
    Authentication_Summary.to_excel(
        writer, sheet_name="Security Authentication Summary", index=False
    )
    ExecutedProcess_Summary.to_excel(
        writer, sheet_name="Executed Process Summary", index=False
    )
    writer.save()
...
def main():
    ...
    parser.add_argument(
        "-p",
        "--path",
        help="path to folder containing windows event logs generated by the powershell log collector",
    )
    parser.add_argument("-o", "--out", help="output file name")
    parser.add_argument(
        "-t",
        "--type",
        help="csv ( logs from get-eventlog or windows event log GUI or logs from Get-WinEvent ) , evtx ( EVTX extension windows event log )",
        choices=["csv", "evtx"],
    )
    ...
    parser.add_argument(
        "--terminal", help="Path to TerminalServices LocalSessionManager Logs"
    )
    ...
    parser.add_argument(
        "-tz",
        "--timezone",
        help="default Timezone is UTC , you can enter ( 'local' : for local timzone , <Country time zone> : like (Asia/Dubai) )",
    )
    parser.add_argument(
        "-hunt", "--hunt", help="String or regex to be searched in evtx log path"
    )
    ...
    if (
        args.path is None
        and args.security is None
        and args.system is None
        and args.scheduledtask is None
        and args.defender is None
        and args.powershell is None
        and args.powershellop is None
        and args.terminal is None
        and args.winrm is None
        and args.sysmon is None
    ):
        print(
            "You didn't specify a path for any log \nuse --help to print help message"
        )
        exit()
    if args.type is None and args.hunt is None:
        print(
            "log type must be defined using -t \ncsv( logs from get-eventlog or windows event log GUI or logs from Get-WinEvent ) , evtx ( EVTX extension windows event log )\nuse --help to print help message"
        )
        exit()
    else:
