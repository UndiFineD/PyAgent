# Extracted from: C:\DEV\PyAgent\.external\APT-Hunter\lib\CSVDetection.py
import csv
import re
...
Security_Authentication_Summary = [
    {"User": [], "Number of Failed Logins": [], "Number of Successful Logins": []}
]
...
Source_Network_Address_rex = re.compile(
    "Source Network Address:\t{1,15}(.*)", re.IGNORECASE
)
...
Process_Command_Line_rex = re.compile(
    "Process Command Line:\t{1,15}(.*)", re.IGNORECASE
)
...
Service_and_state_rex = re.compile(
    "The (.*) service entered the (.*) state\.", re.IGNORECASE
)
StartType_rex = re.compile(
    "The start type of the (.*) service was changed", re.IGNORECASE
)
...
task_register_rex = re.compile(
    'User "(.*)"  registered Task Scheduler task "(.*)"', re.IGNORECASE
)
task_update_rex = re.compile(
    'User "(.*)"  updated Task Scheduler task "(.*)"', re.IGNORECASE
)
task_delete_rex = re.compile(
    'User "(.*)"  deleted Task Scheduler task "(.*)"', re.IGNORECASE
)
...
Source_Network_Address_Terminal_rex = re.compile(
    "Source Network Address: ((\d{1,3}\.){3}\d{1,3})"
)
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list2:
            ...
            if row["Event ID"] == "4688":
                try:
                    if len(re.findall(".*user.*/add.*", row["Details"])) > 0:
                        ...
                        Event_desc = (
                            "User Name : ( %s ) " % Account_Name[0].strip()
                            + "with Command Line : ( "
                            + Process_Command_Line[0].strip()
                            + " )"
                        )
                        Security_events[0]["Date and Time"].append(
                            datetime.strptime(
                                row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
                            ).isoformat()
                        )
                        Security_events[0]["timestamp"].append(
                            datetime.timestamp(
                                datetime.strptime(
                                    row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
                                )
                    ...
                    if (
                        len(
                            re.findall(
                                r"cmd.exe /c echo [a-z]{6} > \\\.\\pipe\\\w{1,10}",
                        ...
                        Event_desc = (
                            "User Name : ( %s ) " % user
                            + "conducting NAMED PIPE privilege escalation with Command Line : ( "
                            + process_command_line
                            + " ) "
                        )
                        Security_events[0]["Date and Time"].append(
                    ...
                    if (
                        Process_Command_Line[0].strip().lower().find("\\temp\\") > -1
                        or Process_Command_Line[0].strip().lower().find("\\tmp\\") > -1
                        or Process_Command_Line[0]
                        .strip()
                        .lower()
                        .find("\\program data\\")
                        > -1
                    ...
                    for i in Suspicious_executables:
                        ...
                        if Process_Command_Line[0].strip().lower().find(i.lower()) > -1:
                            ...
                            Event_desc = (
                                ...
                                + "with Command Line : ( "
                                + Process_Command_Line[0].strip()
                                + " ) contain suspicious command ( %s)" % i
                            )
                            Security_events[0]["Date and Time"].append(
                                datetime.strptime(
                                    row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
            ...
            if row["Event ID"] == "4720":
                ...
                Security_events[0]["timestamp"].append(
                    datetime.timestamp(
                        datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p")
                    )
                )
                Security_events[0]["Detection Rule"].append(
                    "User Created through management interface"
            ...
            if row["Event ID"] == "4730":
                print("##### " + row["Date and Time"] + " ####  ", end="")
                print(
                    "User ( " + Account_Name[0].strip() + " ) removed Group ( ", end=""
                )
            ...
            if row["Event ID"] == "4688":
                try:
                    ...
                    if (
                        ...
                        not in Executed_Process_Summary[0]["Process Name"]
                    ):
                        Executed_Process_Summary[0]["Process Name"].append(
                            Process_Command_Line[0].strip()
                        )
            ...
            if row["Event ID"] == "4625":
                try:
                    if (
                        Account_Name[1].strip()
                        not in Security_Authentication_Summary[0]["User"]
                    ):
                        Security_Authentication_Summary[0]["User"].append(
                            Account_Name[1].strip()
                        )
            ...
            if row["Event ID"] == "4648":
                try:
                    ...
                    if Account_Name[0].strip() not in PasswordSpray:
                        ...
                        PasswordSpray[Account_Name[0].strip()].append(
                            Account_Name[1].strip()
                        )
                    # else:
                    ...
                    if (
                        Account_Name[1].strip()
                        not in PasswordSpray[Account_Name[0].strip()]
                    ):
                        PasswordSpray[Account_Name[0].strip()].append(
                            Account_Name[1].strip()
                        )
            ...
            if row["Event ID"] == "4625" or row["Event ID"] == "4624":
                if (
                    Logon_Type[0].strip() == "3"
                    and Account_Name[1].strip() != "ANONYMOUS LOGON"
                    and Account_Name[1].strip().find("$") == -1
                    and Logon_Process[0].strip() == "NtLmSsp"
                    and Key_Length[0].strip() == "0"
                ):
                    # print("##### " + row['Date and Time'] + " ####  ", end='')
                    # print(
                    ...
                    Event_desc = (
                        ...
                        % (
                            Account_Name[1].strip(),
                            Account_Domain[1].strip(),
                            Source_IP[0].strip(),
                            Workstation_Name[0].strip(),
                        )
                    )
                    Security_events[0]["Date and Time"].append(
                        datetime.strptime(
            ...
            if (
                row["Event ID"] == "4798"
                or row["Event ID"] == "4799"
                and row["Details"].find("System32\\svchost.exe") == -1
            ):
                """print("##### " + row['Date and Time'] + " ####  ", end='')
                print(
            ...
            if (
                row["Event ID"] == "4719"
                and len(Security_ID) > 0
                and Security_ID[0].strip() != "S-1-5-18"
                and Security_ID[0].strip() != "SYSTEM"
            ):
                """print("##### " + row['Date and Time'] + " ####  ", end='')
                print(
            ...
            if row["Event ID"] == "4698":
                ...
                try:
                    Event_desc = (
                        "schedule task created by user ( %s ) with task name ( %s ) , Command ( %s ) and Argument ( %s )  "
                        % (
                            Account_Name[0].strip(),
                            Task_Name[0].strip(),
                            Task_Command[0],
            ...
            if row["Event ID"] == "1699":
                ...
                try:
                    Event_desc = (
                        "schedule task deleted by user ( %s ) with task name ( %s ) , Command ( %s ) and Argument ( %s )  "
                        % (
                            Account_Name[0].strip(),
                            Task_Name[0].strip(),
                            Task_Command[0],
            ...
            if row["Event ID"] == "4702":
                ...
                try:
                    Event_desc = (
                        "schedule task updated by user ( %s ) with task name ( %s ) , Command ( %s ) and Argument ( %s )  "
                        % (
                            Account_Name[0].strip(),
                            Task_Name[0].strip(),
                            Task_Command[0],
            ...
            if row["Event ID"] == "4700":
                ...
                try:
                    Event_desc = (
                        "schedule task enabled by user ( %s ) with task name ( %s )  "
                        % (
                            Account_Name[0].strip(),
                            Task_Name[0].strip(),
                            Task_Command[0],
            ...
            if row["Event ID"] == "4701":
                ...
                try:
                    Event_desc = (
                        "schedule task disabled by user ( %s ) with task name ( %s ) "
                        % (
                            Account_Name[0].strip(),
                            Task_Name[0].strip(),
                            Task_Command[0],
    ...
    for user in PasswordSpray:
        if len(PasswordSpray[user]) > 3:
            ...
            Security_events[0]["Original Event Log"].append(
                "User ( "
                + user
                + " ) did password sparay attack using usernames ( "
                + ",".join(PasswordSpray[user])
                + " )"
            )
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Event ID"] == "1117" or row["Event ID"] == "1007":
                ...
                Event_desc = (
                    "Windows Defender took action against Malware - details : Severity ( %s ) , Name ( %s ) , Action ( %s ) , Catgeory ( %s ) , Path ( %s ) , Process Name ( %s ) , User ( %s ) "
                    % (
                        Severity[0].strip(),
                        Name[0].strip(),
                        Action[0].strip(),
                ...
                Windows_Defender_events[0]["timestamp"].append(
                    datetime.timestamp(
                        datetime.strptime(row["Date and Time"], "%m/%d/%Y %I:%M:%S %p")
                    )
                )
            ...
            if (
                row["Event ID"] == "1118"
                or row["Event ID"] == "1008"
                or row["Event ID"] == "1119"
            ):
                # print("##### " + row['Date and Time'] + " ####  ", end='')
                ...
                Event_desc = (
                    "Windows Defender failed to take action against Malware - details : Severity ( %s ) , Name ( %s ) , Action ( %s ) , Catgeory ( %s ) , Path ( %s ) , Process Name ( %s ) , User ( %s ) "
                    % (
                        Severity[0].strip(),
                        Name[0].strip(),
                        Action[0].strip(),
            ...
            if row["Event ID"] == "1116" or row["Event ID"] == "1006":
                ...
                Event_desc = (
                    "Windows Defender Found Malware - details : Severity ( %s ) , Name ( %s ) , Catgeory ( %s ) , Path ( %s ) , Process Name ( %s ) , User ( %s ) "
                    % (
                        Severity[0].strip(),
                        Name[0].strip(),
                        Category[0].strip(),
            ...
            if row["Event ID"] == "1013":
                ...
                Event_desc = (
                    " Windows Defender deleted history of malwares - details : User ( %s ) "
                    % (User[0])
                )
                Windows_Defender_events[0]["Date and Time"].append(
                    datetime.strptime(
            ...
            if row["Event ID"] == "1015":
                ...
                Event_desc = (
                    "Windows Defender detected suspicious behavior Malware - details : Severity ( %s ) , Name ( %s ) , Catgeory ( %s ) , Path ( %s ) , Process Name ( %s ) , User ( %s ) "
                    % (
                        Severity[0].strip(),
                        Name[0].strip(),
                        Category[0].strip(),
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Event ID"] == "106":
                # print("##### " + row['Date and Time'] + " ####  ", end='')
                if (
                    task_register.group(1).strip() == "S-1-5-18"
                    and task_register.group(2).find(
                    ...
                    Event_desc = (
                        "schedule task registered with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) "
                        % (task_register.group(2))
                    )
                else:
                    # print("schedule task registered with Name ( %s ) by user ( %s ) " % (
                    #    task_register.group(2), task_register.group(1)))
                    Event_desc = (
                        "schedule task registered with Name ( %s ) by user ( %s ) "
                        % (task_register.group(2), task_register.group(1))
                    )
                ...
                ScheduledTask_events[0]["Schedule Task Name"].append(
                    task_register.group(2)
                )
                ScheduledTask_events[0]["Event ID"].append(row["Event ID"])
                ScheduledTask_events[0]["Original Event Log"].append(
                    str(row["Details"]).replace("\r", " ")
                )
            ...
            if row["Event ID"] == "140":
                # print("##### " + row['Date and Time'] + " ####  ", end='')
                if (
                    task_update.group(1).strip() == "S-1-5-18"
                    and task_update.group(2).find("\\Microsoft\\Windows\\WindowsUpdate")
                    != 0
                ):
                    # print("schedule task updated with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (task_update.group(2)))
                    Event_desc = (
                        "schedule task updated with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) "
                        % (task_update.group(2))
                    )
                else:
                    ...
                    Event_desc = (
                        "schedule task updated with Name ( %s ) by user ( %s ) "
                        % (task_update.group(2), task_update.group(1))
                    )
            ...
            if row["Event ID"] == "141":
                # print("##### " + row['Date and Time'] + " ####  ", end='')
                if (
                    task_delete.group(1).strip() == "S-1-5-18"
                    and task_delete.group(2).find("\\Microsoft\\Windows\\WindowsUpdate")
                    != 0
                ):
                    # print("schedule task deleted with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) " % (task_delete.group(2)))
                    Event_desc = (
                        "schedule task deleted with Name ( %s ) by user ( NT AUTHORITY\SYSTEM ) "
                        % (task_delete.group(2))
                    )
                else:
                    ...
                    Event_desc = (
                        "schedule task deleted with Name ( %s ) by user ( %s ) "
                        % (task_delete.group(2), task_delete.group(1))
                    )
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Event ID"] == "104":
                ...
                System_events[0]["Detection Rule"].append("System Logs Cleared")
                ...
                System_events[0]["Original Event Log"].append(
                    str(row["Details"]).replace("\r", " ")
                )

            if (row["Event ID"] == "7045" or row["Event ID"] == "601") and (
                row["Details"].strip().find("\\temp\\") > -1
                or row["Details"].strip().find("\\tmp\\") > -1
            ):
            ...
            if row["Event ID"] == "7045" or row["Event ID"] == "601":
                ...
                Event_desc = (
                    "Service installed in the system with Name ( %s ) , File Name ( %s ) , Service Type ( %s ) , Service Start Type ( %s ) , Service Account ( %s )"
                    % (
                        Service_Name[0].strip(),
                        Service_File_Name[0].strip(),
                        Service_Type[0].strip(),
                        Service_Start_Type[0].strip(),
                        Service_Account[0],
                    )
                )
                System_events[0]["Date and Time"].append(
            ...
            if (
                row["Event ID"] == "7036"
                and Service_and_state[0][0].strip() in critical_services
                and (
                    Service_and_state[0][1].strip() == "stopped"
                    or Service_and_state[0][1].strip() == "disabled"
                )
            ):
                # print("##### " + row['Date and Time'] + " ####  ", end='')
            ...
            if row["Event ID"] == "7040":
                ...
                # print("Service with Name ( %s ) entered ( %s ) state "%(Service_and_state.group(1),Service_and_state.group(2)))
                # print(str(row['Details']).replace("\r"," "))
                Event_desc = "Service with Name ( %s ) changed start type" % (
                    Start_Type_Service_Name[0].strip()
                )
                System_events[0]["Date and Time"].append(
                    datetime.strptime(
                        row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
                ...
                System_events[0]["Detection Domain"].append("Audit")
                System_events[0]["Severity"].append("Medium")
                System_events[0]["Service Name"].append(
                    Start_Type_Service_Name[0].strip()
                )
                System_events[0]["Event Description"].append(Event_desc)
                System_events[0]["Event ID"].append(row["Event ID"])
...
def detect_events_powershell_operational_log(
    file_name="powershell-logs.csv", winevent=False
):
        ...
        for row in list:
            ...
            if row["Event ID"] == "4103":
                ...
                if len(Suspicious) > 0:
                    ...
                    Event_desc = (
                        "Found User ("
                        + User[0].strip()
                        + ") run Suspicious PowerShell commands that include ("
                        + ",".join(Suspicious)
                        + ") in event with Command Name ("
                        + Command_Name[0].strip()
                        + ") and full command ("
                        + host_app
                        + ") "
                    )
                    ...
                    Powershell_Operational_events[0]["Detection Domain"].append(
                        "Threat"
                    )
                    Powershell_Operational_events[0]["Severity"].append("Critical")
                    Powershell_Operational_events[0]["Event Description"].append(
                        Event_desc
                    )
                    Powershell_Operational_events[0]["Event ID"].append(row["Event ID"])
                    Powershell_Operational_events[0]["Original Event Log"].append(
            ...
            if row["Event ID"] == "4104" or row["Event ID"] == "24577":
                ...
                if len(Suspicious) > 0:
                    ...
                    Event_desc = (
                        "Found Suspicious PowerShell commands that include ("
                        + ",".join(Suspicious)
                        + ") , check event details "
                        + row["Details"]
                    )
            ...
            if row["Event ID"] == "4100":
                ...
                if len(Suspicious) > 0:
                    ...
                    Event_desc = (
                        "Found User ("
                        + User[0].strip()
                        + ") run PowerShell with Command Name ("
                        + Command_Name[0].strip()
                        + ") and full command ("
                        + host_app
                        + ") "
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if (
                ...
                Powershell_events[0]["Original Event Log"].append(
                    str(row["Details"]).replace("\r", " ")
                )
...
def detect_events_TerminalServices_LocalSessionManager_log(
    file_name="powershell-logs.csv", winevent=False
):
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            Source_Network_Address = Source_Network_Address_Terminal_rex.findall(
                row["Details"]
            )
                ...
                if len(Source_Network_Address) > 0:
                    ...
                    if Source_Network_Address[0][0].strip() == "127.0.0.1":
                        ...
                        Event_desc = (
                            "Found User ("
                            + User[0].strip()
                            + ") connecting from Local Host ( 127.0.0.1 ) which means attacker is using tunnel to connect RDP "
                        )
                        ...
                        TerminalServices_events[0]["timestamp"].append(
                            datetime.timestamp(
                                datetime.strptime(
                                    row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
                                )
                        ...
                        TerminalServices_events[0]["Event Description"].append(
                            Event_desc
                        )
                    ...
                    try:
                        if (
                            Source_Network_Address[0][0].strip() != "127.0.0.1"
                            and not IPAddress(
                            ...
                            TerminalServices_events[0]["Detection Domain"].append(
                                "Audit"
                            )
                            TerminalServices_events[0]["Severity"].append("Critical")
                            TerminalServices_events[0]["Event Description"].append(
                            ...
                            TerminalServices_events[0]["Event ID"].append(
                                row["Event ID"]
                            )
                            TerminalServices_events[0]["Original Event Log"].append(
                                str(row["Details"]).replace("\r", " ")
...
def detect_events_Microsoft_Windows_WinRM_CSV_log(
    file_name="powershell-logs.csv", winevent=False
):
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Event ID"] == "6":
                ...
                WinRM_events[0]["Original Event Log"].append(
                    str(row["Details"]).replace("\r", " ")
                )
            if row["Event ID"] == "91":
            ...
            if (
                ...
                try:
                    print(
                        ...
                        + "  ### connection is initiated using WinRM - Powershell remoting  ##### User with ID ("
                        + d.find(
                            "{http://schemas.microsoft.com/win/2004/08/events/event}Security"
                        ).attrib["UserID"]
                        + ") is connecting from current machine ("
                        + d.find(
                    ...
                    WinRM_events[0]["Severity"].append("High")
                    WinRM_events[0]["Event Description"].append(Event_desc)
                    WinRM_events[0]["Event ID"].append(
                        d.find(
                            "{http://schemas.microsoft.com/win/2004/08/events/event}EventID"
                        ).text
                    )
...
def detect_events_Sysmon_log(file_name="sysmon-logs.csv", winevent=False):
    ...
    with open(file_name, newline="") as csvfile:
        ...
        for row in list:
            ...
            if row["Event ID"] == "1" and Image[0].strip().find("powershell.exe") > -1:
                ...
                if len(Suspicious) > 0:
                    ...
                    Event_desc = (
                        ...
                        + ParentImage[0].strip()
                        + " , Parent CommandLine ("
                        + ParentCommandLine[0].strip()
                        + ") "
                        + "in directory : ( "
                        + CurrentDirectory[0].strip()
                        + " )"
                    )
                    ...
                    Sysmon_events[0]["Original Event Log"].append(
                        str(row["Details"]).replace("\r", " ")
                    )
            ...
            if (
                row["Event ID"] == "1"
                and Image[0].strip().find("\\sc.exe") > -1
                and (
                    CommandLine[0].find("create") > -1
                    or CommandLine[0].find("start") > -1
                    or CommandLine[0].find("config") > -1
                    or OriginalFileName[0].find("create") > -1
                    or OriginalFileName[0].find("start") > -1
                    or OriginalFileName[0].find("config") > -1
                )
            ):
                ...
                Event_desc = (
                    "Found User ("
                    + User[0].strip()
                    + ") Trying to manipulate windows services usign Sc.exe with Command Line ("
                    + CommandLine[0].strip()
                    + ") and Parent Image :"
                    + ParentImage[0].strip()
                    + " , Parent CommandLine ("
            ...
            if row["Event ID"] == "1" and (
                Image[0].strip().find("\\wscript.exe") > -1
                or Image[0].strip().find("\\cscript.exe") > -1
            ):
                ...
                Event_desc = (
                    "Found User ("
                    + User[0].strip()
                    + ") Trying to run wscript or cscript with Command Line ("
                    + CommandLine[0].strip()
                    + ") and Parent Image :"
                    + ParentImage[0].strip()
                    + " , Parent CommandLine ("
            ...
            if row["Event ID"] == "1" and (Image[0].strip().find("\\mshta.exe") > -1):
                ...
                Event_desc = (
                    "Found User ("
                    + User[0].strip()
                    + ") Trying to run mshta with Command Line ("
                    + CommandLine[0].strip()
                    + ") and Parent Image :"
                    + ParentImage[0].strip()
                    + " , Parent CommandLine ("
            ...
            if row["Event ID"] == "13" and (
                ...
                Event_desc = (
                    "Found User ("
                    + User[0].strip()
                    + ") Trying to run psexec with process Image :"
                    + Image[0].strip()
                )
                Sysmon_events[0]["Date and Time"].append(
                    datetime.strptime(
            ...
            if (
                row["Event ID"] == "1"
                and (
                    Image[0].strip().find("\\taskeng.exe") > -1
                    or Image[0].strip().find("\\svchost.exe") > -1
                )
                and ParentImage[0].strip().find("services.exe") == -1
                and ParentImage[0].strip().find("?") == -1
            ):
                ...
                Event_desc = (
                    "Found User ("
                    + User[0].strip()
                    + ") Trying to run taskeng.exe or svchost.exe with Command Line ("
                    + CommandLine[0].strip()
                    + ") and Parent Image :"
                    + ParentImage[0].strip()
                    + " , Parent CommandLine ("
            ...
            if row["Event ID"] == "3" and (
                Image[0].strip().find("powershell.exe") > -1
                or Image[0].strip().find("mshta.exe") > -1
                or Image[0].strip().find("cscript.exe") > -1
                or Image[0].strip().find("regsvr32.exe") > -1
                or Image[0].strip().find("certutil.exe") > -1
            ):
                # temp.append()
                # print("##### " + row[
                ...
                Event_desc = (
                    ...
                    + ") run process "
                    + Image[0].strip()
                    + " and initiated network connection from hostname ( "
                    + SourceHostname[0].strip()
                    + " and IP ( "
                    + SourceIp[0].strip()
                    + " ) to hostname ( "
                    + DestinationHostname[0].strip()
                    + " ) , IP ( "
                    + DestinationIp[0].strip()
                    + " ) and port ( "
                    + DestinationPort[0].strip()
                    + " )"
                )
                Sysmon_events[0]["Date and Time"].append(
                    datetime.strptime(
            ...
            if row["Event ID"] == "1" and (
                ParentCommandLine[0].strip().find("WmiPrvSE.exe") > -1
                or Image[0].strip().find("WmiPrvSE.exe") > -1
            ):

                Event_desc = (
                    "User ("
                    + User[0].strip()
                    + ") run command through WMI with process ("
                    + Image[0].strip()
                    + ") and commandline ( "
                    + CommandLine[0].strip()
                    + " )"
            ...
            if row["Event ID"] == "1" and (
                ParentCommandLine[0].strip().find("w3wp.exe") > -1
                or Image[0].strip().find("w3wp.exe") > -1
            ):

                Event_desc = (
                    "IIS run command with user ("
                    + User[0].strip()
                    + ") and process name ("
                    + Image[0].strip()
                    + ") and commandline ( "
            ...
            if row["Event ID"] == "8" and (
                StartFunction[0].strip().lower().find("loadlibrary") > -1
            ):

                Event_desc = (
                    "Process ( %s) attempted process injection on process ( %s)"
                    % (SourceImage[0], TargetImage[0])
                )
                Sysmon_events[0]["Date and Time"].append(
                    datetime.strptime(
                        row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
            ...
            if row["Event ID"] == "1" and (
                CommandLine[0].strip().find("sysinfo.exe") > -1
                or Image[0].strip().find("sysinfo.exe") > -1
                or CommandLine[0].strip().find("whoami.exe") > -1
                or Image[0].strip().find("whoami.exe") > -1
            ):

                Event_desc = (
                    "System Information Discovery Process ( %s) ith commandline ( %s) "
                    % (Image[0].strip(), CommandLine[0].strip())
                )
                Sysmon_events[0]["Date and Time"].append(
                    datetime.strptime(
                        row["Date and Time"], "%m/%d/%Y %I:%M:%S %p"
            ...
            if row["Event ID"] == "1" and (
                Image[0].strip().find("regsvr32.exe") > -1
                or Image[0].strip().find("rundll32.exe") > -1
                or Image[0].strip().find("certutil.exe") > -1
            ):

                Event_desc = (
                    "[T1117] Bypassing Application Whitelisting with Regsvr32 , Process ( %s) with commandline ( %s)"
                    % (Image[0].strip(), CommandLine[0].strip())
                )
                Sysmon_events[0]["Date and Time"].append(
                    datetime.strptime(
