...
def recommend_prompt(
    ...
    embedding_fn=get_embedding_func(
        "local", model_id="ibm-granite/granite-embedding-278m-multilingual"
    ),
    ...
    for v in prompt_json["positive_values"]:
        ...
        if len(v["centroid"]) == len(input_embedding):
            if (
                get_similarity(
                    pd.DataFrame(input_embedding), pd.DataFrame(v["centroid"])
                )
                > add_lower_threshold
            ):
                closer_prompt = -1
                for p in v["prompts"]:
                    d_prompt = get_similarity(
                        pd.DataFrame(input_embedding), pd.DataFrame(p["embedding"])
                    )
                    ...
                    if (
                        d_prompt > closer_prompt
                        and d_prompt > add_lower_threshold
                        and d_prompt < add_upper_threshold
                    ):
    ...
    for sentence in input_sentences:
        ...
        if (
            len(prompt_json["negative_values"][0]["centroid"]) == len(input_embedding)
            and sentence != ""
        ):
            input_items.append(
                {
            ...
            if len(v["centroid"]) == len(input_embedding):
                if (
                    ...
                    for p in v["prompts"]:
                        ...
                        if (
                            d_prompt > closer_prompt
                            and d_prompt > remove_upper_threshold
                        ):
                            closer_prompt = d_prompt
...
def get_thresholds(
    ...
    for p_id, p in enumerate(prompts):
        out = recommend_prompt(
            p, prompt_json, embedding_fn, 0, 1, 0, 0, model_id
    ...
    thresholds = {}
    thresholds["add_lower_threshold"] = round(
        add_similarities_df.describe([0.1]).loc["10%", "similarity"], 1
    )
    thresholds["add_higher_threshold"] = round(
        add_similarities_df.describe([0.9]).loc["90%", "similarity"], 1
    )
    thresholds["remove_lower_threshold"] = round(
        remove_similarities_df.describe([0.1]).loc["10%", "similarity"], 1
    )
    thresholds["remove_higher_threshold"] = round(
        remove_similarities_df.describe([0.9]).loc["90%", "similarity"], 1
    )
