...
def find_data_offset(
    header: bytes, iv_offset: int, key: bytes, starting_data_offset: int
) -> int:
    ...
    for i in oscillate(
        n=starting_data_offset, n_min=iv_offset + len(iv), n_max=C.HEADER_SIZE - 128
    ):
...
def guess_offsets(
    key: bytes, encrypted: io.BufferedReader, def_iv_offset: int, def_data_offset: int
):
...
def main():
    ...
    if not (0 < args.data_offset < C.HEADER_SIZE - 128):
        log.fatal(
            "The data offset must be between 1 and {}".format(C.HEADER_SIZE - 129)
        )
