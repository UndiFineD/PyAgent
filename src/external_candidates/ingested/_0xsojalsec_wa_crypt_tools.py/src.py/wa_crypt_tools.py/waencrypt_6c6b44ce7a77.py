# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wa-crypt-tools\src\wa_crypt_tools\waencrypt.py
import argparse
import hashlib
import logging
import os
...
def parsecmdline() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Encrypts a file in Crypt14 or Crypt15 format."
    )
    parser.add_argument(
        "keyfile",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="*",
        default=C.DEFAULT_FEATURE_LIST,
    ...
    parser.add_argument(
        "--iv", type=str, help="The IV to use for crypt15 encryption. Default: random"
    )
    parser.add_argument(
        "--reference",
        type=argparse.FileType("rb"),
