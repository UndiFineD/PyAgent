# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wa-crypt-tools\src\wa_crypt_tools\lib\key\key14.py
import logging
        ...
        if keyarray is None:
            ...
            if serversalt is None:
                ...
                if len(serversalt) != 32:
                    raise ValueError(
                        "Invalid server salt length: {}".format(serversalt.hex())
                    )
            ...
            if googleid is None:
                ...
                if len(googleid) != 16:
                    raise ValueError(
                        "Invalid google id length: {}".format(googleid.hex())
                    )
            ...
            if hashedgoogleid is None:
                ...
                if len(hashedgoogleid) != 32:
                    log.error(
                        "Invalid hashed google id length: {}".format(
                            hashedgoogleid.hex()
                        )
                    )
        ...
        if self.__SUPPORTED_CIPHER_VERSION != self.__cipher_version:
            log.error(
                "Invalid keyfile: Unsupported cipher version {}".format(
                    keyarray[: len(self.__SUPPORTED_CIPHER_VERSION)].hex()
                )
            )
        ...
        if not version_supported:
            log.error(
                "Invalid keyfile: Unsupported key version {}".format(
                    keyarray[
                        index : index + len(self.__SUPPORTED_KEY_VERSIONS[0])
                    ].hex()
                )
            )
