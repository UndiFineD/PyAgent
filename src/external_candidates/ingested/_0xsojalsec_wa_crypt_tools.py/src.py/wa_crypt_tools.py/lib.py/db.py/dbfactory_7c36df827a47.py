# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wa-crypt-tools\src\wa_crypt_tools\lib\db\dbfactory.py
import logging
        ...
        try:
            ...
            try:
                ...
                if header.ParseFromString(protobuf_raw) != protobuf_size:
                    ...
                    log.debug(
                        "Your phone number ends with {}".format(header.info.jidSuffix)
                    )
                ...
                log.debug(
                    "Could not parse the protobuf message as a crypt14/15. Trying as a crypt12..."
                )
                try:
                    encrypted.seek(0)
                except OSError as e:
