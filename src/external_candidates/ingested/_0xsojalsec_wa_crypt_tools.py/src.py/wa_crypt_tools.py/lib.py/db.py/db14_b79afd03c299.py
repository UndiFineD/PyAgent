# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wa-crypt-tools\src\wa_crypt_tools\lib\db\db14.py
import logging
...
class Database14(Database):
            ...
            try:
                ...
                try:
                    ...
                    if self.header.ParseFromString(protobuf_raw) != protobuf_size:
                        ...
                        self.__version = findall(
                            r"\d(?:\.\d{1,3}){3}", self.header.info.app_version
                        )
                        ...
                        l.debug(
                            "Your phone number ends with {}".format(
                                self.header.info.jidSuffix
                            )
                        )
    ...
    def decrypt(self, key: Key14, encrypted: bytes) -> bytes:
        ...
        if self.file_hash.digest() != checksum:
            ...
            l.debug(
                "Checksum OK ({}). Decrypting...".format(self.file_hash.hexdigest())
            )
