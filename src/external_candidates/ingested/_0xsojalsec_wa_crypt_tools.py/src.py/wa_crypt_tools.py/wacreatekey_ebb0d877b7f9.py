# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-wa-crypt-tools\src\wa_crypt_tools\wacreatekey.py
#!/usr/bin/env python
"""
...
def parsecmdline() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Create a traditional key file.",
    )
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Prints all messages"
    )
    parser.add_argument(
        "--hex", type=str, nargs="?", help="The hex string to convert to a key"
    )
    ...
    parser.add_argument(
        ...
        "--cipher-version",
        type=int,
        help="The cipher version to use. Default: 1",
    )
    parser.add_argument(
        "-kv", "--key-version", type=int, help="The key version to use. Default: 3"
    )
    parser.add_argument(
        "-ss", "--server-salt", type=str, help="The server salt to use. Default: random"
    )
    parser.add_argument(
        "-gi", "--googleid", type=str, help="The google id salt to use. Default: random"
    )
...
def main():
    ...
    logging.getLogger("wa_crypt_tools.lib").setLevel(
    ...
    if args.crypt14:
        ...
        try:
            key: Key14 = Key14(
                ...
                serversalt=(
                    bytes.fromhex(args.server_salt)
                    if args.server_salt is not None
                    else None
                ),
                googleid=(
                    bytes.fromhex(args.googleid) if args.googleid is not None else None
                ),
                iv=None,
                key=hex_key,
            )
        ...
        if args.cipher_version is not None:
            lo.warning(
                "Cipher version specified, but it is not used for crypt15 keys, ignoring."
            )
        if args.key_version is not None:
            lo.warning(
                "Key version specified, but it is not used for crypt15 keys, ignoring."
            )
        if args.server_salt is not None:
            lo.warning(
                "Server salt specified, but it is not used for crypt15 keys, ignoring."
            )
        if args.googleid is not None:
            lo.warning(
                "Google id specified, but it is not used for crypt15 keys, ignoring."
            )
        try:
