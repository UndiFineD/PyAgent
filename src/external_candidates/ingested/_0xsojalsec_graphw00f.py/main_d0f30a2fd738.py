# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-graphw00f\main.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import sys
...
def main():
    ...
    parser.add_option(
        "-T", "--timeout", dest="timeout", default=10, help="Request timeout in seconds"
    )
    parser.add_option(
        "-o",
        "--output-file",
        dest="output_file",
        help="Output results to a file (CSV)",
        default=None,
    )
    parser.add_option(
        "-l",
    ...
    parser.add_option(
        ...
        dest="useragent",
        default=None,
        help="Custom user-agent to use (overrides the one from headers configuration)",
    )
    parser.add_option(
        "-w",
        "--wordlist",
        dest="wordlist",
        default=False,
        help="Path to a list of custom GraphQL endpoints",
    )
    parser.add_option(
        "--version",
        "-v",
    ...
    if options.list:
        ...
        for k, v in get_engines().items():
            count += 1
            print(
                "{index}. {name} ({technology})".format(
                    index=count, name=v["name"], technology=", ".join(v["technology"])
                )
            )
        sys.exit(0)
    ...
    g = GRAPHW00F(
        ...
        headers=(
            ...
            else {**conf.HEADERS, **{"User-Agent": options.useragent}}
        ),
    ...
    if options.detect:
        ...
        try:
            ...
            print(
                bcolors.FAIL
                + "[x] Could not determine the existence of GraphQL (Error: GraphQLDetectionFailed)"
                + bcolors.ENDC
            )
