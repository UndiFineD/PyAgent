# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-graphw00f\graphw00f\lib.py
import requests
...
class GRAPHW00F:
    def __init__(self, headers, cookies, timeout, proxies, follow_redirects=False):
    ...
    """
        ...
        if response.get("data"):
            if response.get("data", {}).get("__typename", "") in (
                "Query",
                "QueryRoot",
                "query_root",
            ):
                return True
        elif response.get("errors") and (
            any("locations" in i for i in response["errors"])
            or (any("extensions" in i for i in response))
        ):
            return True
    ...
    """
        ...
        ) or error_contains(response, "Unexpected error."):
            return True
    ...
    """
        response = self.graph_query(self.url, payload=query)
        if (
            error_contains(response, "Unknown directive '@abc'.")
    ...
    """
        response = self.graph_query(self.url, payload=query)
        if error_contains(
            response, "No DirectiveResolver resolves directive with name 'doesnotexist'"
        ):
            return True
    ...
    """
        ...
        try:
            ...
            if (
                debug_msg["type"] == "DEBUG_LOGS_INACTIVE"
    ...
    """
        ...
        try:
            if (
                response["data"] != None
                and response["data"]["__typename"] == "RootQuery"
            ):
                return True
        except KeyError:
            pass
    ...
    """
        response = self.graph_query(self.url, payload=query)
        if error_contains(
            response, "Directive < @deprecated > is not used in a valid location."
        ):
            return True
    ...
    def engine_directus(self):
        ...
        if response.get("errors", []):
            if (
                errors
                and errors[0].get("extensions", {}).get("code" "") == "INVALID_PAYLOAD"
            ):
                return True
    ...
    """
        response = self.graph_query(self.url, payload=query)
        if error_contains(response, "Internal server error") or error_contains(
            response, "internal", part="category"
        ):
            return True
    ...
    """
        ...
        if error_contains(response, "expecting white space") or error_contains(
            response, "offset"
        ):
            return True
    ...
    def engine_jaal(self):
        ...
        if error_contains(response, "must have a single query") or error_contains(
            response, "offset"
        ):
            return True
