# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\core\registry.py
import importlib
import pkgutil
import time
...
class ModelRegistry:
    ...
    def __init__(self, config_dir: Path, models_config_path: Optional[Path] = None):
        ...
        self.models_config_path = models_config_path or (config_dir / "models.yaml")
            ...
            if not ispkg:
                if not modname.startswith("app.models._"):
                    try:
                        ...
                        logger.warning(f"Failed to import model module {modname}: {e}")
    ...
    def _read_models_config(self) -> Dict:
        ...
        if not self.models_config_path.exists():
            logger.warning(f"models.yaml not found at {self.models_config_path}")
    ...
    def _load_single_model(self, model_id: str):
        ...
        logger.info(f"Model [{model_id}] loaded successfully (took {elapsed:.2f}s)")
    ...
    def get_all_models_info(self) -> Dict[str, Dict]:
        ...
        """
        return {
            model_id: model.get_metadata() for model_id, model in self.models.items()
        }
