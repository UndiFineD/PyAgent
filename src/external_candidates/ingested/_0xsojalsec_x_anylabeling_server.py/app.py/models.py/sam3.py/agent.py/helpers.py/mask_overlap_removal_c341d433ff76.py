# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-X-AnyLabeling-Server\app\models\sam3\agent\helpers\mask_overlap_removal.py
...
def _decode_single_mask(mask_repr, h: int, w: int) -> np.ndarray:
    ...
    rle = {
        "counts": (
            mask_repr if isinstance(mask_repr, (str, bytes)) else str(mask_repr)
        ),
        "size": [h, w],
    }
