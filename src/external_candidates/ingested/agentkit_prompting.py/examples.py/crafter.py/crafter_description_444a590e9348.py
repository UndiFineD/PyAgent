# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\examples\crafter\crafter_description.py
import crafter
import numpy as np
...
def describe_env(info):
    ...
    counts = sorted(
        [
            (id_to_item[i], counts[i])
            for i in range(len(counts))
            if id_to_item[i] not in {"player", "None"} and counts[i] > 0
        ],
        key=lambda x: x[1],
        reverse=True,
    )
    ...
    general_obs = "* Further to the {}: {}.".format(
        describe_dir(np.array([0, 0]), facing)[0], general_desc
    )
...
def describe_act(info, repeats):
    ...
    if "move" in action_str:
        action_str = action_str.replace(
            "move_up", "move_north {} step(s)".format(repeats)
        )
        action_str = action_str.replace(
            "move_down", "move_south {} step(s)".format(repeats)
        )
        action_str = action_str.replace(
            "move_left", "move_west {} step(s)".format(repeats)
        )
        action_str = action_str.replace(
            "move_right", "move_east {} step(s)".format(repeats)
        )
    else:
...
def match_act(string):
    ...
    if matches:
        ...
        return (
            None,
            None,
            "'{}' does not seem to match any elemet in the list: {}".format(
                string, action_list
            ),
        )
