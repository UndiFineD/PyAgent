        ...
        messages.append(
            {
                ...
                "content": "Player's in-game observation:\n\n{}".format(
                    db["environment"]["observation_current"]
                ),
            }
        )
        return messages
...
class ComposeObservationReasoningPrompt(ComposeBasePrompt):
    ...
    def __init__(self):
        super().__init__()
        self.system_prompt = (
            "Provide accuracte answers based on the observation, inventory, and vitals."
        )
...
class ComposeActionReflectionPrompt(ComposeBasePrompt):
    ...
    def after_dependencies(self, messages, db):
        if "unknowns" in db["kb"] and len(db["kb"]["unknowns"]) > 0:
            kb_desc = "\n\n".join(
                [
                    "{}:\n\n{}".format(d, db["kb"]["unknowns"][q])
                    for q, d in db["prompts"]["kb_questions_desc"].items()
                ]
            )
...
Finally, make sure that each planned items is both quantifiable and achievable with the game actions, and the requirements are well-addressed."""
    ...
    def before_dependencies(self, messages, db):
        ...
        qa_history = db["history"]["qa_history_stream"][
            -db["history"]["qa_history_planner_length"] :
        ]
            ...
            messages.append(
                {
                    ...
                    "content": "Knowledge base:\n\n{}".format(
                        json.dumps(db["kb"]["knowledge_base"], indent=0)
                    ),
                }
            )

        text_desc = self.compose_gamestep_prompt(
            qa_history, db, step_offset, skip_last=False
        )
...
Be vigilant for information or details missing from the manual."""
    ...
    def before_dependencies(self, messages, db):
        qa_history = db["history"]["qa_history_stream"][
            -db["history"]["qa_history_planner_reflection_length"] :
        ]
        step_offset = db["environment"]["step"] - len(qa_history)
        ...
        messages.append(
            {
                ...
                "content": "Skill library\n\n```{}```".format(
                    print_skill_library(db["skills"]["skill_library"])
                ),
            }
        )
        return messages
...
class ComposeActorPlannerPrompt(ComposeBasePrompt):
            ...
            messages.append(
                {
                    "role": "system",
                    "content": "Knowledge base:\n\n{}".format(
                        json.dumps(knowledge_base, indent=0)
                    ),
                }
            )
    ...
    def after_dependencies(self, messages, db):
        if (
            db["skills"]["skill"] is not None
            and db["skills"]["skill"] == db["skills"]["skill_old"]
        ):
            messages.append(
                {
                    "role": "system",
                    "content": "Previous plan:\n\n{}".format(
                        json.dumps(db["action_summary"], indent=0)
                    ),
                }
            )
            messages.append(
            ...
            messages.append(
                {"role": "system", "content": "Notes from the last step: NA"}
            )
            db["action_summary"] = {
                "plan-sketch": "NA",
                "details": "NA",
...
The game does not contain bugs or glitches, and does not offer additional cues or patterns."""
    ...
    def before_dependencies(self, messages, db):
        qa_history = db["history"]["qa_history_stream"][
            -db["history"]["qa_history_actor_length"] :
        ]
        step_offset = db["environment"]["step"] - len(qa_history)
...
def compose_feedback_prompt(CTXT_dict, qa_history, current_objective, questions):
    ...
    messages_feedback.append(
        {"role": "system", "content": "Instruction manual:\n\n{}".format(CTXT)}
    )
    ...
    messages_feedback.append(
        {"role": "system", "content": "Current skill:\n{}".format(current_objective)}
    )
...
def compose_gameplay_prompt(CTXT_dict, qa_history, Q_CTXT, question):
    ...
    messages.append(
        {"role": "system", "content": "You already know:\n\n{}".format(CTXT)}
    )
