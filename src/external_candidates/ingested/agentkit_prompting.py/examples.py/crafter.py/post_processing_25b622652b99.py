# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\examples\crafter\post_processing.py
import json
import traceback
...
class SubgoalAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        self.node.db["subgoals"]["completion_criteria"] = parsed_answer[-1][
            "completion_criteria"
        ]
...
class SkillAfterQuery(aq.BaseAfterQuery):
    ...
    def post_process(self):
        ...
        if parsed_answer is None:
            ...
            error = ex.AfterQueryError(
                "No answer",
                "Invalid Json: It seems that the last Json object in the output above is either invalid or empty.",
            )
        elif type(parsed_answer[-1]) != dict:
            error = ex.AfterQueryError(
                "Invalid answer",
                "Invalid Type: Expecting the last Json object to be dictionary, got length {} instead.".format(
                    type(parsed_answer[-1])
                ),
            )
        elif len(parsed_answer[-1]) != 1:
            error = ex.AfterQueryError(
                "Invalid answer",
                "Invalid Length: Expecting only one identified skill in the dictionary, got {} instead.".format(
                    len(parsed_answer[-1])
                ),
            )
        elif (
            list(parsed_answer[-1].values())[0] is None
            or len(list(parsed_answer[-1].values())[0]) != 3
        ):
            error = ex.AfterQueryError(
                "Invalid answer",
                "Invalid Value: Expecting the value in the last Json dictionary to be `[description, supported parameters tuple, usage_guide]`, got length {} instead.".format(
                    list(parsed_answer[-1].values())[0]
                ),
            )
        ...
        self.node.result = "[{},{},{}]".format(
            skill_type, skill_desc, skill_param, skill_guide
        )
...
class KBAddAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        try:
            ...
            raise ex.AfterQueryError(
                "Invalid answer", "{}: {}".format(e, traceback.format_exc())
            )
        self.node.result_raw = self.node.result
...
class KBReasonAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        self.node.db["kb"]["unknowns"] = {
            self.node.prompt: json.dumps(unknowns, sort_keys=True, indent=0)
        }
...
class ReflectionAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        if "yes" in json_dict["unexpected_encounters"].lower():
            self.node.db["reflection"]["unexpected"].append(
                self.node.db["environment"]["step"]
            )
        if "yes" in json_dict["mistake"].lower():
            self.node.db["reflection"]["mistake"].append(
                self.node.db["environment"]["step"]
            )
        if "yes" in json_dict["correction_planned"].lower():
            self.node.db["reflection"]["correction"].append(
                self.node.db["environment"]["step"]
            )
        if "yes" in json_dict["confused"].lower():
            self.node.db["reflection"]["confusion"].append(
                self.node.db["environment"]["step"]
            )
        if True in [
            "yes" in json_dict[k].lower()
            for k in [
            ...
            self.node.db["reflection"]["all"].append(
                self.node.db["environment"]["step"]
            )
        if (
            True not in ["yes" in v.lower() for v in json_dict.values()]
            and len(self.node.db["history"]["qa_history"]) > 0
        ):
            print(
                Fore.BLUE + "Skipping a bunch of reflection questions..." + Fore.RESET
            )
            self.node.graph.skip_nodes_temporary(
    ...
    def post_process(self):
        ...
        try:
            ...
            for action, v in parsed_answer[-1].items():
                if (
                    action.strip().lower() == "noop"
                ):  # Skip noop. This doesn't change the behavior of the LLM experimentally but saves quite a bit of tokens.
                    continue
        ...
        self.node.result = json.dumps(
            {k: str(i) for i, k in enumerate(filtered_result.keys())}, indent=0
        ).strip()
        ...
        # Adaptive Questions
        if (
            "adaptive_questions" not in self.node.db
            or self.node.db["adaptive_questions"] is None
        ):
            return
...
class ActionAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        parsed_answer = self.parse_json()
        act, action_name, error_msg = match_act(
            parsed_answer[-1]["action"].replace("(", "").replace("_", " ")
        )
        if act is None:
            raise ex.AfterQueryError(
                "Invalid answer",
                "Invalid action: {}".format(parsed_answer[-1]["action"], error_msg),
            )
        if (
            type(parsed_answer[-1]["repeats"]) == str
            and not (parsed_answer[-1]["repeats"]).isnumeric()
        ):
            raise ex.AfterQueryError(
                "Invalid answer",
                "Invalid repeats: '{}'. Expecting an integer.".format(
                    parsed_answer[-1]["repeats"]
                ),
            )
        self.node.db["action"] = act
        if (
            "move" in action_name.lower()
            and "yes" not in parsed_answer[-1]["hazards"].lower()
            and "yes" not in parsed_answer[-1]["obstacles"].lower()
        ):
        ...
        self.node.result = json.dumps(
            {"action": action_name, "repeats": self.node.db["action_repeats"]}, indent=2
        )
...
class SummaryAfterQuery(aq.JsonAfterQuery):
    ...
    def post_process(self):
        ...
        if "move" in parsed_answer[-1]["action"].lower():
            action_desc += "{}, {} steps".format(
                parsed_answer[-1]["action"], parsed_answer[-1]["repeats"]
            )
        else:
            action_desc += "{}, {} steps, target: {}".format(
                parsed_answer[-1]["action"],
                parsed_answer[-1]["repeats"],
                parsed_answer[-1]["target"],
            )
        if "no" in parsed_answer[-1]["success"].lower():
            action_desc += " (failed, causes of failure: {})".format(
                parsed_answer[-1]["causes_of_failure"]
            )
        else:
