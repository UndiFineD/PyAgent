# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\examples\crafter\main.py
import argparse
import copy
import datetime
import json
...
parser.add_argument(
    "--outdir", type=str, default="logs/test_env", help="output directory"
)
parser.add_argument(
    "--Eps", type=int, default=1, help="Total number of episodes to run"
)
parser.add_argument("--temperature", type=float, default=0.0, help="LLM Temperature")
parser.add_argument("--wandb_log_interval", type=int, default=25, help="some integer")
parser.add_argument(
    "--granularities", type=int, nargs="+", default=[500], help="list of integers"
)
parser.add_argument("--feedback_granularity", type=int, default=3, help="some integer")
parser.add_argument(
    "--planner_reflection_granularity", type=int, default=25, help="some integer"
)
parser.add_argument(
    "--actor_reflection_granularity", type=int, default=5, help="some integer"
)
parser.add_argument(
    "--kb_refine_granularity", type=int, default=50, help="some integer"
)
parser.add_argument(
    "--llm_plan_accurate", type=str, default="gpt-4-turbo-2024-04-09", help="LLM"
)
parser.add_argument(
    "--llm_plan", type=str, default="gpt-4-turbo-2024-04-09", help="LLM"
)
...
parser.add_argument(
    "--llm_fast_accurate", type=str, default="gpt-3.5-turbo-0125", help="LLM"
)
...
from crafter_description import action_list, describe_frame
...
if args.resume_id is not None:
    wandb.init(
        id=args.resume_id, resume="allow", project="AgentKit", config=args.__dict__
    )
else:
...
llm_functions = {
    ...
    "query_fast_accurate": {
        "query_model": query_fast_accurate,
        "token_counter": query_fast_accurate.count_tokens,
    },
    "query_reason": {
        "query_model": partial(query_reason, max_gen=2048, temp=args.temperature),
        "token_counter": query_reason.count_tokens,
    },
    "query_plan_accurate": {
        "query_model": partial(
            query_plan_accurate, max_gen=2048, temp=args.temperature
        ),
        "token_counter": query_plan_accurate.count_tokens,
    },
    "query_spatial": {
        "query_model": partial(query_spatial, max_gen=1500, temp=args.temperature),
        "token_counter": query_spatial.count_tokens,
    },
}
...
columns = (
    ["Step", "OBS", "Reward", "Return"]
    + ["Action", "Repeats", "Skills", "Knowledge Base", "Env step"]
    + table_questions
)
...
while eps < args.Eps:
    ...
    while not done:
        ...
        qprint(
            ...
            + Style.BRIGHT
            + "==" * 15
            + " Step: {}, Env step: {}, Reward: {} ".format(step, env_step, R)
            + "==" * 15
            + Style.RESET_ALL
        )
        # qprint(desc)
        ...
        wandb.log(
            {
                "eps-{}-metric/reward".format(eps): reward,
                "eps-{}-metric/return".format(eps): R,
                "eps-{}-metric/step".format(eps): step,
                "eps-{}-metric/env_step".format(eps): env_step,
            },
            commit=False,
        )
        ...
        for node in graph.nodes.values():
            if node.token_counter is not None:
                ...
                tokens_table.add_data(
                    *[node.key, counts["calls"], counts["prompt"], counts["completion"]]
                )
        wandb.log(
            {
                "token_counts": tokens_table,
            },
            commit=False,
        )
        ...
        database["history"] = {
            ...
            "qa_history_actor_length": min(
                args.actor_reflection_granularity, max(3, skill_length)
            ),
        ...
        if "action_summary" in database.keys():
            qprint(
                Style.DIM
                + json.dumps(database["action_summary"], indent=2)
                + Style.RESET_ALL
            )
        qprint(
            "Skill: {} -> {}".format(
        ...
        if len(qa_history) > 0:
            strategy_desc = "\n\n".join(
                [
                    "## {}\n{}".format(d, qa_history[-1][q])
                    for q, d in database["prompts"]["strategy_questions_desc"].items()
                ]
            )
        ...
        qprint(
            Style.DIM
            + json.dumps(database["kb"]["knowledge_base"], indent=2)
            + Style.RESET_ALL
        )
        if "unknowns_json" in database["kb"].keys():
            ...
            qprint(
                Style.DIM
                + json.dumps(
                    list(database["kb"]["unknowns_json"].values())[0], indent=2
                )
                + Style.RESET_ALL
            )
        qprint(
        ...
        qprint(
            ...
            + json.dumps(
                {
                    k: v["skill_desc"]
                    for k, v in database["skills"]["skill_library"].items()
                },
                indent=2,
            )
            + Style.RESET_ALL
        ...
        qprint(
            Style.DIM
            + json.dumps(database["feedback"]["skill_feedback"], indent=2)
            + Style.RESET_ALL
        )
        ...
        if skill is not None:
            ...
            if (
                len(
                    set(skill_history[skill])
                    & set(attention_rounds["mistake"]).union(
                        set(attention_rounds["confusion"])
                    )
                )
                + 1
            ) % args.feedback_granularity == 0:
                CTXT_dict = {
                    "CTXT": describe_achievements(info, MANUAL),
                    "attention_rounds": list(
                        set(skill_history[skill])
                        & set(attention_rounds["mistake"]).union(
                            set(attention_rounds["confusion"])
                        )
                ...
                messages, shrink_idx = compose_feedback_prompt(
                    CTXT_dict,
                    qa_history,
                    "{}:{}".format(
                        skill, database["skills"]["skill_library"][skill]["skill_guide"]
                    ),
                    database["prompts"]["feedback_questions"],
                )
                database["feedback"]["skill_feedback"][skill] = ""
                for shorthand, msg in messages.items():
                    answer, _ = llm_functions["query_reason"]["query_model"](
                        msg, shrink_idx
                    )
                    # database['feedback']['skill_feedback'][skill] += "{}:\n{}\n\n".format(shorthand, answer)
                    database["feedback"]["skill_feedback"][skill] = "{}".format(answer)
                database["feedback"]["skill_feedback"][skill] = database["feedback"][
                    "skill_feedback"
                ][skill].strip()
                qprint(
                    Fore.MAGENTA + "Feedback for {}:".format(skill) + Style.RESET_ALL
                )
                qprint(
                    Style.DIM
                    + database["feedback"]["skill_feedback"][skill]
        ...
        if info["sleeping"]:
            qprint(
                Fore.RED
                + "Player is sleeping. We manually take noop until the player's awake to save LLM calls:"
                + Style.RESET_ALL
            )
            a = action_list.index("noop")
            database["action"] = a
            ...
            while info["sleeping"]:
                ...
                qprint(
                    Style.DIM
                    + "====Sleeping: {} Reward: {}====".format(rep + 1, rr)
                    + Style.RESET_ALL
                )
        ...
        if (
            step % args.kb_refine_granularity == 0
            and len(database["kb"]["knowledge_base"]) > 0
        ):
            messages = [
                {
                    "role": "system",
                    "content": "Improve the knowledge base. Note that items in the knowledge base should augment the instruction manual, not duplicate it or contradict it. In addition, the knowledge base should not contain duplicate items.",
                }
            ]
            messages.append(
                {
            ...
            messages.append(
                {
                    ...
                    "content": "Knowledge base:\n\n{}".format(
                        json.dumps(database["kb"]["knowledge_base"], indent=0)
                    ),
                }
            )
...
""".strip(),
            ...
            for _ in range(10):
                ...
                if parsed_answer is None or type(parsed_answer[-1]) != dict:
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": "Invalid Type: Expecting the last Json object to be dictionary",
                        }
                    )
                    continue
                problem = False
                for k, v in parsed_answer[-1].items():
                    if len(v) != 5 or type(v) != dict:
                        ...
                        messages.append(
                            {
                                "role": "user",
                                "content": "Invalid Type: Expecting each value to be a dictionary with 5 keys",
                            }
                        )
                        problem = True
                        break
                ...
                qprint(
                    Style.DIM
                    + json.dumps(parsed_answer[-1], indent=2)
                    + Style.RESET_ALL
                )
                for k, v in parsed_answer[-1].items():
                    if "no" in [
                        v["addition"].strip(),
                    ] or "yes" in [v["duplicate"].strip(), v["contradiction"].strip()]:
        ...
        if step % args.wandb_log_interval == 0 or done:
            if root_span is not None:
                root_span._span.end_time_ms = round(
            ...
            for skill in skill_history.keys():
                feedback_table.add_data(
                    *(
                        ...
                        if skill not in database["feedback"]["skill_feedback"].keys()
                        else [
                            skill,
                            database["feedback"]["skill_feedback"][skill],
                            len(
                                set(skill_history[skill])
                                & set(attention_rounds["mistake"])
                            ),
                        ]
                    )
                )
            ...
            wandb.log(
                {
                    "eps-{}-rollout/rollout {}~{}".format(
                        eps, last_log, step - 1
                    ): rollouts,
                    "eps-{}-achievements/achievements {}~{}".format(
                        eps, last_log, step - 1
                    ): achievement_table,
                    "eps-{}-feedback/feedback {}~{}".format(
                        eps, last_log, step - 1
                    ): feedback_table,
                    "eps-{}-feedback/feedback-current".format(eps): feedback_table,
                    "eps-{}-current/rollout-current".format(eps): rollouts,
                    "eps-{}-current/achievements-current".format(
                        eps
                    ): achievement_table,
                }
            )
            ...
            feedback_table = wandb.Table(
                columns=["Skill", "feedback", "attention_rounds"]
            )
            last_log = step
    ...
    wandb.log(
        {
            ...
            "eps-{}-ALL/rollout-ALL".format(eps): wandb.Table(
                columns=columns, data=rollout_history
            ),
