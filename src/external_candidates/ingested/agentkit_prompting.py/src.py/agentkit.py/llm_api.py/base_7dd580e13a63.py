...
class BaseModel:
    ...
    def decode(self, txt):
        print(
            "Warning: decode is not implemented for this model, returning concatenation of input."
        )
        return " ".join(txt)
    ...
    def count_tokens(self, text):
        if type(text) == list:
            return min(
                sum([len(self.encode(x["content"])) for x in text]), self.model_max
            )
        elif type(text) == str:
    ...
    def shrink_msg_by(self, msg, shrink_idx, L):
        ...
        while L > 0:
            ...
            msg[shrink_idx]["content"] = self.decode(
                self.encode(msg[shrink_idx]["content"])[L:]
            )
