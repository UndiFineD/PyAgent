# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\src\agentkit\llm_api\claude.py
import os
...
if os.environ.get("ANTHROPIC_KEY") is None:
    print(
        "Environment variable for Anthropic key not found, using Anthropic API key from ~/.openai/openai.key."
    )
    if not os.path.exists(os.path.join(os.path.expanduser("~"), ".openai/openai.key")):
        raise FileNotFoundError(
            "Please create a file at ~/.openai/openai.key with your OpenAI API key and organization ID. The third line should be your anthropic API key."
        )
    else:
        with open(
            os.path.join(os.path.expanduser("~"), ".openai/openai.key"), "r"
        ) as f:
            org_key = f.readlines()
