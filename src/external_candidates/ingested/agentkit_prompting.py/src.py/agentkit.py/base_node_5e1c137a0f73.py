...
class BaseNode:
    ...
    def compose_prompt(self):
        return self._compose_prompt(
            dependencies=self.get_dependencies(), prompt=self.prompt
        )
    ...
    def skip_turn(self):
        ...
        ), "Attempting to skip a node ({}) that has never been evaluated".format(
            self.key
        )
        ...
        if status_code == "error":
            if ignore_errors:
                print(
                    Fore.RED
                    + "WARNING: ({}, {}) for {}".format(
                        error, status_message, self.result
                    )
                    + Fore.RESET
                )
                self.result = "N/A"
    ...
    def commit_trace(self):
        ...
        self.chain_span.add_inputs_and_outputs(
            inputs={
                "dependencies": [(node.key, node.result) for node in self.adjacent_from]
            },
            outputs={"response": self.result},
        )
    ...
    def evaluate(self):
        ...
        for node in self.adjacent_from:
            ...
            ), "Dependency {} of {} has been not evaluated".format(node.key, self.key)
            ...
            for i in range(3):
                try:
                    ...
                    if error is not None:
                        temp_prompt = self._add_error_msg(
                            temp_prompt, self.result, error
                        )
