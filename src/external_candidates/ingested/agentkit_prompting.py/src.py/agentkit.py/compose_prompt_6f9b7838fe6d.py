# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\src\agentkit\compose_prompt.py
import re
    ...
    def add_dependencies(self, messages, dependencies):
        ...
        if len(dependencies) > 0:
            for node in dependencies:
                ...
                assert type(messages[-1]["content"]) == str, "Invalid type: {}".format(
        ...
        if len(dependencies) > 0:
            for node in dependencies:
                if node.key in db["shorthands"].keys():
                    messages.append(
                        {
                            "role": "system",
                            "content": "{}:\n\n{}".format(
                                db["shorthands"][node.key], node.result
                            ),
                        }
                    )
                else:
                    ...
                    ), "Rendered prompt is not set for node {}".format(node.key)
