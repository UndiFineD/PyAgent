# Extracted from: C:\DEV\PyAgent\.external\agentkit_prompting\src\agentkit\after_query.py
...
class JsonAfterQuery(BaseAfterQuery):
    ...
    def parse_json(self):
        ...
        if parsed_answer is None:
            ...
            raise AfterQueryError(
                "No answer",
                "Invalid Json: It seems that the last Json object in the output above is invalid.",
            )
        elif type(parsed_answer[-1]) != self.type:
            raise AfterQueryError(
                "Invalid answer",
                "Invalid Type: Expecting the last Json object to be {}, got {} instead.".format(
                    self.type, type(parsed_answer[-1])
                ),
            )
        if self.length is not None and len(parsed_answer[-1]) != self.length:
            raise AfterQueryError(
                "Invalid answer",
                "Expecting length {}, got {} instead.".format(
                    self.length, len(parsed_answer[-1])
                ),
            )
        ...
        for k in self.required_keys:
            if k not in parsed_answer[-1].keys():
                raise AfterQueryError(
                    "Invalid answer", "Expecting '{}' in the keys.".format(k)
                )
