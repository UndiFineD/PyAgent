...
class Graph:
    ...
    def add_node(self, node):
        ...
        """
        assert node.key not in self.nodes.keys(), "Node ({}) already exists".format(
            node.key
        )
    ...
    def add_temporary_node(self, node):
        ...
        """
        assert (
            node.key not in self.nodes.keys()
        ), "Node ({}) already exists in permanent graph".format(node.key)
        assert (
            node.key not in self.temporary_nodes.keys()
        ), "Node ({}) already exists in temporary graph".format(node.key)
    ...
    def has_edge_with_temporary(self, from_key, to_key):
        ...
        assert (
            node_from is not None and node_to is not None
        ), "Node ({}) not found in graph".format(from_key)
    ...
    def add_edge(self, from_key, to_key, prepend=False):
        ...
        if from_key in self.nodes.keys() and to_key in self.nodes.keys():
            assert not self.has_edge_with_temporary(
                from_key, to_key
            ), "Edge ({}) already exists".format((from_key, to_key))
            ...
            raise ValueError(
                "Node ({}, {}) not found in graph".format(from_key, to_key)
            )
    ...
    def add_order(self, from_key, to_key):
        ...
        """
        assert (
            from_key in self.nodes.keys() and to_key in self.nodes.keys()
    ...
    def add_edge_temporary(self, from_key, to_key, prepend=False):
        ...
        assert (
            to_key not in self.history.keys()
        ), "Cannot add edge to a node ({}) that has already been evaluated".format(
            to_key
        )
        if (
            from_key in self.history.keys() and to_key not in self.queue
        ):  # if from_key has been evaluated, add to queue
    ...
    def remove_edge_temporary(self, from_key, to_key):
        ...
        assert (
            to_key not in self.history.keys()
        ), "Cannot remove edge to a node ({}) that has already been evaluated".format(
            to_key
        )
        assert (
            from_key not in self.history.keys()
        ), "Cannot remove edge from a node ({}) that has already been evaluated".format(
            from_key
        )
    ...
    def skip_nodes_temporary(self, keys):
        ...
        for key in keys:
            ...
            assert (
                key not in self.temporary_nodes.keys()
            ), "Cannot skip temporary node: {}".format(key)
            assert (
                key not in self.history.keys()
            ), "Cannot skip node {}. It has already been evaluated.".format(key)
            node.skip_turn()
    ...
    def commit_trace(self):
        if self.chain_span is None:
            return
        self.chain_span._span.end_time_ms = round(
            datetime.datetime.now().timestamp() * 1000
        )
        self.chain_span.add_inputs_and_outputs(
            inputs={}, outputs={"response": self.history}
        )
    ...
    def get_streaming_history(self):
        assert (
            len(self.history) == 0
        ), "Error: This function should only be called before evaluate() is called."
        return self.history
    ...
    def evaluate(self):
        ...
        def recalculate_in_degree(key):
            ...
            return len(
                [
                    n
                    for n in node.get_dependencies_inc_order()
                    if n.key not in self.history.keys()
                ]
            )
        ...
        assert (
            len(self.temporary_nodes) == 0
        ), "Temporary nodes must be cleared before calling evaluate()"
        in_degree = {
            key: len(self.nodes[key].adjacent_from) for key in self.nodes.keys()
        }
        ...
        while True:
            ...
            for adjacent in node.adjacent_to:
                if (
                    adjacent.key not in self.history.keys()
                    and adjacent.key not in self.queue
                ):
