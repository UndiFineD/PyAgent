# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\tinychart\train\llava_trainer.py
import os
...
class LLaVATrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if getattr(self.args, "tune_vision_tower", False) or getattr(
            self.args, "tune_entire_model", False
        ):
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ...
                try:
                    if getattr(self.args, "lora_enable", False):
                        self.model.model.model.vision_tower.vision_tower.config.save_pretrained(
                            output_dir
                        )
                    else:
                        self.model.model.vision_tower.vision_tower.config.save_pretrained(
                            output_dir
                        )
                except:
