# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\tinychart\eval\eval_model.py
import argparse
import json
import math
import os
...
from tinychart.conversation import SeparatorStyle, conv_templates
...
class EvalDataset(Dataset):
    def __init__(
        self, data_items, image_folder, tokenizer, image_processor, model_config
    ):
...
def eval_model(args):
    ...
    data_loader = create_data_loader(
        all_data, args.image_folder, tokenizer, image_processor, model.config
    )
    for (input_ids, image_tensor, image_sizes), line in tqdm(
        zip(data_loader, all_data), total=len(all_data)
    ):
        idx = line["id"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", type=str, default="./data/test_chartqa+cot_shuffle.json"
    )
