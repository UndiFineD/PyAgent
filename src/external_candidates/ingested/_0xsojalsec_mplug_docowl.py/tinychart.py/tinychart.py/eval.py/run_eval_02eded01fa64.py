# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\tinychart\eval\run_eval.py
import argparse
import json
import os
...
from tinychart.eval.eval_metric import (
    chartqa_evaluator,
    chartqa_oracle_merger_evaluator,
    chartqa_rule_merger_evaluator,
    chartqapot_evaluator,
)
...
if __name__ == "__main__":
    ...
    for result_file in result_files:
        ...
        if "chartqa-" in dataset_name:
            direct_result, direct_acc = chartqa_evaluator(
                result_data, key="model_answer"
            )
    ...
    if direct_result is not None and pot_result is not None:
        ...
        oracle_results, oracle_acc = chartqa_oracle_merger_evaluator(
            direct_result, pot_result
        )
        ...
        rule_results, rule_acc = chartqa_rule_merger_evaluator(
            direct_result, pot_result
        )
