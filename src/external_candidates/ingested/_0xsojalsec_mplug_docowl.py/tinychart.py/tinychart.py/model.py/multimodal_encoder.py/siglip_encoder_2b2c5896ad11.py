    ...
    def forward(
        ...
        if self.r > 0:
            merge, unmerge = bipartite_soft_matching(
                metric, r=self.r, class_token=False, distill_token=False
            )
            if trace_source:
                source = merge_source(merge, hidden_states, source)
            hidden_states, attention_size = merge_wavg(
                merge, hidden_states, attention_size
            )
        residual = hidden_states
...
class SigLipEncoderToMe(nn.Module):
    ...
    def __init__(self, config: SigLipVisionConfig):
        ...
        self.layers = nn.ModuleList(
            [
                SigLipEncoderLayerToMe(config, layer_id=layer_id)
