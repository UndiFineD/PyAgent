# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\TinyChart\tinychart\data\dataset.py
import copy
import json
...
def make_supervised_data_module_with_eval(
    tokenizer: transformers.PreTrainedTokenizer, data_args
) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedDataset(
    ...
    if data_args.eval_data_path is None or data_args.eval_data_path == "":
        ...
        eval_dataset = LazySupervisedDataset(
            tokenizer=tokenizer, data_path=data_args.eval_data_path, data_args=data_args
        )
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer)
    return dict(
