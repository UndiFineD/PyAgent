...
def evaluate_method(gtJson, submJson, evaluationParams):
    ...
    for gtObject in gtJson["data"]:
        ...
        if q_id in question_ids_to_exclude:
            ...
            if (
                gtObject["answer_type"] == "not-answerable"
            ):  # gracefully deal with not-answerable
                if gtObject["answers"] == []:
                    gtObject["answers"] = [""]
    ...
    if show_scores_per_question_type:
        for answer_type, answer_type_str in answer_types.items():
            answer_types_ANLS[answer_type_str] = (
                0
                if len(gtJson["data"]) == 0
                else answerTypeTotalANLS[answer_type]
                / (answerTypeNumQuestions[answer_type])
            )
...
def postprocess_llm_vqa(
    ...
    if not os.path.exists(gt_path):
        ...
        for s in src_data["data"]:
            if (
                s["data_split"] == split
                and s["questionId"] not in question_ids_to_exclude
            ):
                split_data.append(s)
