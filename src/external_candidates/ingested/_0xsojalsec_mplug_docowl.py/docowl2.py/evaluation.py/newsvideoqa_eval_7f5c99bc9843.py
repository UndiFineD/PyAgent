# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\DocOwl2\evaluation\newsvideoqa_eval.py
"""
...
def get_NLSL(gt_list, pred_list):
    ...
    match_dict, cost_matrix = get_best_matches_hungarian_munkers(
        anchor_list, matching_list
    )
...
def postprocess_llm_vqa(
    dataset_name, split, llm_pred_path, meta_dir, save_results=True
):
    """
    ...
    if split == "test":
        ...
        for x in reformat_preds:
            submissions.append(
                {"questionId": x["questionId"], "answer": x["answers"][0]}
            )
        submission_path = os.path.join(
            "/".join(reformat_pred_path.split("/")[:-1]),
            dataset_name + "_submission.json",
        )
        ...
        if save_results:
            ...
            print(
                "All results including per-sample result has been correctly saved to ",
                save_path,
            )
...
if __name__ == "__main__":
    postprocess_llm_vqa(
        dataset_name="NewsVideoQA",
        split="val",
        llm_pred_path="/nas-alinlp/anwenhu/DocSFTv2/NewsVideoQA/val.jsonl",
        save_results=True,
    )
