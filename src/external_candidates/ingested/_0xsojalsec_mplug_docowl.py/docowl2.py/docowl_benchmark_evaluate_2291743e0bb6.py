# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\DocOwl2\docowl_benchmark_evaluate.py
import argparse
import json
import os
...
class DocOwl2Infer:
    def __init__(self, ckpt_path):
        ...
        self.model.init_processor(
            tokenizer=self.tokenizer, basic_image_size=504, crop_anchors="grid_12"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--downstream_dir",
        type=str,
        help="the directory path of DocDownstream-1.0 or DocDownstream-2.0",
    )
    parser.add_argument(
    ...
    if dataset not in ["MP-DocVQA", "DUDE", "NewsVideoQA"]:
        try:
            ...
            print(
                "For single-image datasets of DocDownstream 1.0 ('DocVQA', 'InfographicsVQA', 'WikiTableQuestions', 'DeepForm', 'KleisterCharity', 'TabFact',\
    ...
    if dataset in [
        ...
        llm_mpdocvqa_eval(
            ...
            split=split,
            llm_pred_path=pred_path,
            meta_dir=os.path.join(meta_dir, dataset),
        )
