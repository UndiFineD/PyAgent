# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\UReader\pipeline\data_utils\xgpt3_dataset.py
import json
import logging
import os
import random
    ...
    def _load_img(self, images, image_root=None):
        ...
        for image_url in images:
            ...
            if image_url.startswith("oss://"):
                ...
                image = Image.open(requests.get(image_url, stream=True).raw).convert(
                    "RGB"
                )
...
class MultiModalDataset(Dataset):
    ...
    def process_data(self, data, processor=None):
        ...
        if "image" in data and len(data["image"]) > 0:
            ...
            if num_img_token != len(images):
                ...
                warnings.warn(
                    "The number of <image> is different from the number of input images"
                )
            ...
            if processor:
                ...
                process_results = [
                    processor(image=image, text=None) for image in images
                ]
                if len(process_results) > 0 and len(process_results[0][0].shape) == 4:
                    ...
                    patch_positions = []
                    text = text_list[0]
                    for ri, (image_input, text_input, patch_position) in enumerate(
                        process_results
                    ):
        ...
        # Process Text
        text = {"prompt": data.get("prompt", ""), "text": text}
        if processor:
    ...
    def __getitem__(self, index):
        ...
        task_type = data.get("task_type", "dummy_default").split("_")[
            -1
        ]  # Get processor type
        while True:
            try:
