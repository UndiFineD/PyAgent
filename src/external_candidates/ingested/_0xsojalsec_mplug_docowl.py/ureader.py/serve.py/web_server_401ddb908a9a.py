# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\UReader\serve\web_server.py
import argparse
import datetime
import json
import os
...
def drop_image(images, prompt):
    ...
    if len(images) > 1:
        ...
        prompt = (
            prompt[:last_image_pos].replace("<image>", "") + prompt[last_image_pos:]
        )
    ...
    with gr.Blocks(title="mPLUG-DocOwl", css=css) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Accordion(
                    "Parameters", open=True, visible=False
                ) as parameter_row:
