# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\PaperOwl\serve\io_utils.py
# -*- coding: utf-8 -*-
"""
...
class DefaultIO(IO):
    ...
    def listdir(self, path, recursive=False, full_path=False, contains=None):
        ...
        if recursive:
            files = (
                os.path.join(dp, f) if full_path else f
...
class OSS(DefaultIO):
    ...
    def __init__(
        self, access_key_id: str, access_key_secret: str, region_bucket: List[List[str]]
    ):
        """
        ...
        self.buckets = {
            bucket_name: Bucket(
                self.auth, f"http://oss-{region}.aliyuncs.com", bucket_name
            )
