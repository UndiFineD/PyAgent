# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\PaperOwl\pipeline\utils.py
import math
import random
...
def batchify(batch):
    ...
    patch_positions = [
        data["patch_positions"] if data["patch_positions"] is not None else None
        for data in batch
    ]
    if all([img is None for img in image]):
        ...
        patch_positions = torch.cat(
            [patch for patch in patch_positions if patch is not None], dim=0
        )
    ...
    prompt_length = torch.from_numpy(
        np.stack([data["text"]["prompt_length"] for data in batch])
    )
    seq_length = torch.from_numpy(
        np.stack([data["text"]["seq_length"] for data in batch])
    )
...
def get_cosine_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            return (1 - delta_min_lr) + delta_min_lr * float(current_step) / float(
                max(1, num_warmup_steps)
            )
        progress = float(current_step - num_warmup_steps) / float(
            max(1, num_training_steps - num_warmup_steps)
        )
        return delta_min_lr + (1 - delta_min_lr) * max(
