...
def postprocess_llm_vqa(dataset_name, split, llm_pred_path, eval_flag=True):
    ...
    for i in range(len(donut_data)):
        ...
        if dataset_name in ["DeepFormQA", "KleisterCharityQA"]:
            cate_question = json.loads(donut_data[i]["ground_truth"])["gt_parses"][0][
                "question"
            ]
        ...
        # keep quesiton consistent with gt file is necessary for due eveluation
            ...
            preds[i]["question"] = json.loads(donut_data[i]["ground_truth"])[
                "gt_parses"
            ][0]["question"]
    ...
    for i in range(len(preds)):
        ...
        if (
            dataset_name not in ["DeepFormQA", "KleisterCharityQA"]
            or pred["answer"] != "None"
        ):
            img["annotations"].append(
    ...
    if eval_flag:
        ...
        if split == "validation":
            ...
            gt_path = (
                DUE_BENCHMARK + eval_dataset_name + "/" + split + "/document.jsonl"
            )
...
if __name__ == "__main__":
    ...
    for metric in ["BLEU1", "BLEU2", "BLEU3", "BLEU4", "RougeL", "Meteor", "CIDEr"]:
        score, scores = doc_evaluate(
            metric=metric, targets=targets, predictions=predictions
        )
        ic(metric, score, scores)
