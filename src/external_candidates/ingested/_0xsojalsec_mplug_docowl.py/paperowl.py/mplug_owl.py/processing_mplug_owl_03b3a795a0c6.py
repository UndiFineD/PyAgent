# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-mPLUG-DocOwl\PaperOwl\mplug_owl\processing_mplug_owl.py
import re
...
class MplugOwlProcessor(ProcessorMixin):
    ...
    def __call__(self, text=None, images=None, return_tensors=None, **kwargs):
        ...
        if images is not None:
            ...
            process_results = [
                self.image_processor(image=image, text=None) for image in images
            ]
        ...
        if text is not None and images is not None:
            ...
            return BatchEncoding(
                data=dict(pixel_values=images, patch_position=patch_position),
...
def detokenize_generations(
    tokens_gpu_tensor, lengths_gpu_tensor, return_segments, tokenizer
):
...
def _tokenize_prompt(
    ...
    if all([media_token not in prompt for media_token in media_tokens.keys()]):
        enc_chunk = (
            prompt_chunk
            + tokenizer(prompt, add_special_tokens=False, **kwargs)["input_ids"]
        )
