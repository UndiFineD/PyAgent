# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pennywiseai-tracker\scripts\verify_anon_smsxml.py
import html
import re
...
def extract_sensitive_data(text):
    ...
    for line in lines:
        if any(
            loc in line
            for loc in ["DUBAI", "SHARJAH", "BANGKOK", "AE", "US", "TH", "SG"]
        ):
            if len(line) > 20:
...
def compare_messages(original_file, anonymized_file):
    ...
    for i in range(sample_size):
        ...
        if orig_data["account_numbers"]:
            ...
            print(
                f"      ✓ Changed: {set(orig_data['account_numbers']) != set(anon_data['account_numbers'])}"
            )
        ...
        if orig_data["amounts_aed"]:
            ...
            print(
                f"      ✓ Changed: {set(orig_data['amounts_aed']) != set(anon_data['amounts_aed'])}"
            )
        ...
        if orig_data["phone_numbers"]:
            ...
            print(
                f"      ✓ Changed: {set(orig_data['phone_numbers']) != set(anon_data['phone_numbers'])}"
            )
        ...
        if orig_data["otp_codes"]:
            ...
            print(
                f"      ✓ Changed: {set(orig_data['otp_codes']) != set(anon_data['otp_codes'])}"
            )
        ...
        if orig_data["merchant_names"]:
            ...
            print(
                f"      ✓ Changed: {set(orig_data['merchant_names']) != set(anon_data['merchant_names'])}"
            )
    ...
    print(
        f"      ✓ Status: {'PASS - No overlap' if len(all_orig_accounts & all_anon_accounts) == 0 else '✗ FAIL - Data leaked!'}"
    )
    ...
    print(
        f"      ✓ Status: {'PASS - No overlap' if len(all_orig_amounts & all_anon_amounts) == 0 else '✗ FAIL - Data leaked!'}"
    )
    ...
    print(
        f"      ✓ Status: {'PASS - No overlap' if len(all_orig_phones & all_anon_phones) == 0 else '✗ FAIL - Data leaked!'}"
    )
...
if __name__ == "__main__":
    ...
    anonymized_file = (
        "../parser-core/src/test/resources/fab_sms_test_data_anonymized.xml"
    )
    try:
