# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pennywiseai-tracker\scripts\randomziedata.py
import html
import random
import string
...
def anonymize_body_text(body):
    ...
    for line in lines:
        ...
        if len(line) > 25 and any(
            loc in line
            for loc in ["DUBAI", "SHARJAH", "AE", "US", "TH", "SG", "BANGKOK"]
        ):
        ...
        if "OTP" in line or "Dear Customer" in line or "DO NOT DISCLOSE" in line:
            if "DO NOT DISCLOSE" in line:
                ...
                merchant = random.choice(
                    ["Online Store", "Retail Shop", "Food Delivery"]
                )
                new_line = line
                # Replace OTP codes
                import re
                ...
                new_line = re.sub(
                    r"\d{2}-\d{2}-\d{4}",
                    f"{random.randint(1,28):02d}-{random.randint(1,12):02d}-{random.randint(2023,2025)}",
                    new_line,
                )
                # Replace times
                new_line = re.sub(
                    r"\d{2}:\d{2} [AP]M",
                    f'{random.randint(1,12):02d}:{random.randint(0,59):02d} {"AM" if random.random() > 0.5 else "PM"}',
                    new_line,
                )
                ...
                for merchant_name in ["Deliveroo", "DELIVEROO", "Samsung Pay"]:
                    new_line = new_line.replace(
                        merchant_name,
                        random.choice(
                            ["Online Store", "Digital Service", "Payment App"]
                        ),
                    )
        ...
        if "unsuccessful transaction" in line.lower():
            ...
            new_line = re.sub(
                r"\d{2}/\d{2}/\d{2}",
                f"{random.randint(1,28):02d}/{random.randint(1,12):02d}/{random.randint(23,25):02d}",
                new_line,
            )
            new_line = re.sub(
                r"\d{2}:\d{2}",
...
def anonymize_xml(input_file, output_file):
    ...
    for mms in root.findall(".//mms"):
        ...
        if mms.get("tr_id"):
            mms.set(
                "tr_id", f"T{''.join(random.choices(string.hexdigits.lower(), k=8))}"
            )
    ...
    print(
        f"✓ Processed {len(root.findall('.//sms'))} SMS and {len(root.findall('.//mms'))} MMS messages"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "Please make sure the input file exists in the same directory as this script."
        )
