...
def clipeval(image_paths, prompts, model, device):
    ...
    return np.mean([s["CLIPScore"] for s in scores.values()]), np.std(
        [s["CLIPScore"] for s in scores.values()]
    )
...
def clipeval_image(image_paths, image_dir_ref, model, device):
    ...
    image_feats_ref = image_feats_ref / np.sqrt(
        np.sum(image_feats_ref**2, axis=1, keepdims=True)
    )
...
for i, (comb_idx, comb) in enumerate(comb_idx2prompt.items()):
    ...
    for i, comb_target_path in enumerate(comb_target_paths):
        cur_clipi_score = clipeval_image(
            image_paths, comb_target_path, clip_model, device=device
        )
        cur_dino_score = dinoeval_image(
            image_paths, comb_target_path, dino_model, device
        )
    ...
    with open(
        os.path.join(save_dir, f"all_score_img_rank_{args.rank}.json"), "w"
    ) as file:
