# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-UNO\uno\dataset\uno.py
...
class FluxPairedDatasetV2(Dataset):
    def __init__(
        ...
        self.resolution_ref = (
            resolution_ref if resolution_ref is not None else resolution
        )
    ...
    def __getitem__(self, idx):
        ...
        if image_tgt_path is not None:
            img = Image.open(os.path.join(self.image_root, image_tgt_path)).convert(
                "RGB"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json_file", type=str, default="datasets/fake_train_data.json"
    )
