# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\main.py
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--mcp", action="store_true", help="Run as MCP server (stdio mode)"
    )
    parser.add_argument(
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        cleanup_components(
                            resource_manager, indexing_engine, chroma_store
                        ),
...
async def cleanup_components(
    ...
    try:
        ...
        try:
            await asyncio.wait_for(
                recovery.save_state(indexing_engine.roots), timeout=2.0
            )
        except asyncio.TimeoutError:
...
async def async_main():
    ...
    logger.info(
        f"Embedding provider: {settings.embedding.provider}/{settings.embedding.model}"
    )
