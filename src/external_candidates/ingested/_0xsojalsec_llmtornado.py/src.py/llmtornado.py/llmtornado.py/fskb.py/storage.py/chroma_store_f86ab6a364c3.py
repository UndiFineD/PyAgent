# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\storage\chroma_store.py
"""
...
class ChromaStore:
    ...
    async def get_cached_embeddings(
        ...
        try:
            ...
            results = await asyncio.to_thread(
                cache.get, ids=unique_hashes, include=["embeddings"]
            )
            ...
            if cache_hits > 0:
                logger.debug(
                    f"Embedding cache: {cache_hits}/{len(unique_hashes)} unique hits (from {len(content_hashes)} total chunks)"
                )
    ...
    async def cache_embeddings(
        self, content_hashes: List[str], embeddings: List[np.ndarray]
    ):
        """
        ...
        if len(content_hashes) != len(embeddings):
            logger.error(
                f"Hash/embedding count mismatch: {len(content_hashes)} vs {len(embeddings)}"
            )
            return
        ...
        try:
            ...
            await asyncio.to_thread(
                ...
                documents=[""]
                * len(content_hashes),  # Empty documents (not needed for cache)
    ...
    async def add_chunks(
        ...
        try:
            ...
            logger.debug(
                f"Added {len(chunks)} chunks from {file_path} to {branch_name}"
            )
    ...
    async def delete_file_chunks(
        ...
        try:
            ...
            if chunk_count > 0:
                ...
                logger.debug(
                    f"Deleted {chunk_count} chunks for {file_path} in {branch_name}"
                )
    ...
    async def cleanup_orphaned_files(
        ...
        try:
            ...
            logger.info(
                f"Cleaning up {len(orphaned_files)} orphaned files from {branch_name}"
            )
            ...
            for i in range(0, len(orphaned_list), batch_size):
                ...
                if progress_callback:
                    progress_callback(
                        f"Cleaning up orphaned files: {total_deleted}/{len(orphaned_files)}"
                    )
                ...
                await asyncio.to_thread(
                    collection.delete,
                    where={
                        "$and": [{"branch": branch_name}, {"file_path": {"$in": batch}}]
                    },
                )
                ...
                if i % 500 == 0 and i > 0:
                    logger.info(
                        f"Cleaned up {total_deleted}/{len(orphaned_files)} orphaned files..."
                    )

            logger.info(
                f"✓ Cleaned up {len(orphaned_files)} orphaned files from ChromaDB"
            )
    ...
    async def get_branch_stats(
        self, root_path: Path, branch_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            results = await asyncio.to_thread(
                collection.get, where={"branch": branch_name}, include=["metadatas"]
            )
    ...
    async def get_file_chunk_counts(
        self, root_path: Path, branch_name: str
    ) -> Dict[str, int]:
        """
        ...
        try:
            ...
            if results and results["metadatas"]:
                for metadata in results["metadatas"]:
                    ...
                    file_mtime = metadata.get(
                        "file_mtime", "0"
                    )  # Default to 0 if missing (old data)
