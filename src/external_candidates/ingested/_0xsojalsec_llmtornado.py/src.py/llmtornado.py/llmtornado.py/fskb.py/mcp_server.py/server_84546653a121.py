# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\mcp_server\server.py
"""
...
class MCPServer:
    ...
    async def _handle_remove_root(self, arguments: dict) -> list[TextContent]:
        ...
        if success:
            return [
                TextContent(type="text", text=f"Successfully removed root: {root_path}")
            ]
        else:
            return [
                TextContent(type="text", text=f"Failed to remove root: {root_path}")
            ]
    ...
    async def _handle_search(self, arguments: dict) -> list[TextContent]:
        ...
        if not branch:
            ...
            if not root_state:
                return [
                    TextContent(type="text", text=f"Error: root not found: {root_path}")
                ]
