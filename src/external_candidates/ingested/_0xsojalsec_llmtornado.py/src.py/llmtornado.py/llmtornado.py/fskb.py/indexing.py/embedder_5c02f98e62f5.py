# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\indexing\embedder.py
"""
...
def detect_task_from_query(query: str) -> str:
    ...
    if any(
        c in query for c in ["{", "}", "()", "=>", ";", "def ", "class ", "function "]
    ):
        return "code2code"
...
class EmbeddingProvider:
    ...
    def _initialize_local(self):
        ...
        try:
            ...
            self._tokenizer = AutoTokenizer.from_pretrained(
                self.model, padding_side="left", trust_remote_code=True
            )
            ...
            if device == "cuda":
                ...
                gpu_cc = float(
                    f"{gpu_compute_capability[0]}.{gpu_compute_capability[1]}"
                )
                supports_flash_attn = gpu_cc >= 8.0  # Ampere (RTX 3000+) or newer

                if not supports_flash_attn:
                    logger.info(
                        f"GPU compute capability {gpu_cc} < 8.0 (Flash Attention 2 requires Ampere/RTX 3000+ or newer)"
                    )
                ...
                try:
                    ...
                    if supports_flash_attn:
                        try:
                            ...
                            logger.info(
                                "Model loaded with INT8 quantization + Flash Attention 2 (3-4x faster)"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Flash Attention 2 failed ({e}), using standard attention"
                            )
                            self._local_model = AutoModel.from_pretrained(
                                self.model,
                                quantization_config=quantization_config,
                            ...
                            logger.info(
                                "Model loaded with INT8 quantization (2-3x faster)"
                            )
                    else:
                    ...
                    if "bitsandbytes" in str(e).lower() or isinstance(e, ImportError):
                        logger.warning(
                            "bitsandbytes not available, falling back to BF16"
                        )
                    else:
                        logger.warning(
                            f"INT8 quantization failed: {e}, falling back to BF16"
                        )
                    ...
                    try:
                        ...
                        if "flash_attn" in str(e).lower():
                            logger.warning(
                                "Flash Attention 2 not available, using eager attention"
                            )
                            self._local_model = AutoModel.from_pretrained(
                                self.model, dtype=torch.bfloat16, trust_remote_code=True
                            ).to(device)
    ...
    async def embed_texts(
        self, texts: List[str], is_query: bool = False, task: str = None
    ) -> List[np.ndarray]:
        """
    ...
    async def _embed_local(
        ...
        all_embeddings = []
        total_batches = (
            len(texts_with_instruction) + self.batch_size - 1
        ) // self.batch_size
        logger.debug(
        ...
        for i in range(0, len(texts_with_instruction), self.batch_size):
            ...
            try:
                ...
                embeddings = await asyncio.wait_for(
                    loop.run_in_executor(None, encode_batch, batch), timeout=timeout
                )

                elapsed = time.time() - start_time
                logger.debug(
                    f"Batch {batch_num}/{total_batches} completed in {elapsed:.1f}s"
                )
    ...
    async def embed_single(
        self, text: str, is_query: bool = False, task: str = None
    ) -> np.ndarray:
        """Embed a single text string."""
        embeddings = await self.embed_texts([text], is_query=is_query, task=task)
        return (
            embeddings[0] if embeddings else np.zeros(self.dimension, dtype=np.float32)
        )
