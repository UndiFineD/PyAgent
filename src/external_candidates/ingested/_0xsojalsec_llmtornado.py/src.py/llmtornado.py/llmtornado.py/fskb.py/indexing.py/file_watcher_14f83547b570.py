# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\indexing\file_watcher.py
"""
...
class FileWatcher:
    ...
    def __init__(
        ...
        self._ignore_file_stats: Dict[Path, tuple[float, int]] = (
            {}
        )  # path -> (mtime, size)
    ...
    async def check_ignore_files_on_startup(
        self, branch: str, progress_callback=None
        ...
        logger.debug(
            f"Loaded ignore file cache in {load_time:.1f}ms ({len(cached_ignore_files)} files)"
        )
    ...
    def save_metadata_cache(
        ...
        try:
            ...
            logger.debug(
                f"Saved metadata cache: {len(indexed_files)} indexed files, {len(ignore_files)} ignore files"
            )
        except Exception as e:
                ...
                if self._on_ignore_change_callback:
                    ...
                    try:
                        ...
                        asyncio.run_coroutine_threadsafe(
                            self._check_ignore_file_change(file_path), loop
                        )
                    except RuntimeError:
                        # Event loop not available, log and skip
                        logger.debug(
                            f"Event loop not available for ignore file change: {file_path}"
                        )
            return
    ...
    async def _process_pending_events(self):
        ...
        while self._running:
            try:
                ...
                to_process = []
                for file_path, (event_type, timestamp) in list(
                ...
                for file_path, event_type in to_process:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error in file watcher callback for {file_path}: {e}"
                        )
    ...
    async def scan_existing_files(self, progress_callback=None) -> Set[Path]:
                        ...
                        with os.scandir(directory) as entries:
                            for entry in entries:
                                ...
                                if entry.is_dir(follow_symlinks=False):
                                    ...
                                    if (
                                        self.ignore_matcher
                                        and self.ignore_matcher.should_ignore(
                                            entry_path
                                        )
                                    ):
        ...
        logger.info(
            f"Found {len(text_files)} text files in {self.root_path} (scanned {total_scanned} items)"
        )
        return text_files
