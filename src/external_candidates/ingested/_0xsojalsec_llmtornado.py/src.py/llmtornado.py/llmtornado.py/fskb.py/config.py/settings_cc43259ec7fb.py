# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\config\settings.py
"""
...
class EmbeddingConfig(BaseModel):
    ...
    provider: Literal["local", "openai", "voyage", "cohere", "google", "anthropic"] = (
        "local"
    )
    model: str = (
        "jinaai/jina-code-embeddings-0.5b"  # Default: Jina Code Embeddings 0.5B (494M params, 896 dim, 32768 context, 15+ languages, task-specific)
    )
    api_key: Optional[str] = None
    batch_size: int = (
        8  # Larger batch with INT8 quantization (uses ~2-3GB VRAM on RTX 2080)
    )
...
class ChunkingConfig(BaseModel):
    ...
    chunk_size: int = Field(
        default=3000, ge=100, le=8000
    )  # Characters per chunk (Jina supports 8K tokens ~6-7K chars)
    chunk_overlap: int = Field(
        default=500, ge=0, le=2000
    )  # Overlap for context (increased for larger chunks)
    separators: List[str] = ["\n\n", "\n", ". ", " ", ""]  # Split priority
...
class ResourceConfig(BaseModel):
    ...
    idle_timeout_seconds: int = Field(
        default=300, ge=60
    )  # Unload inactive indices after 5 min
...
class SearchConfig(BaseModel):
    ...
    )  # Increased from 0.0 to filter low-quality results
