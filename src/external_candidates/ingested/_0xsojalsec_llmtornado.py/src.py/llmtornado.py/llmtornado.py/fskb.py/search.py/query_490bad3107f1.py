# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.FsKb\LlmTornado.FsKb\fskb\search\query.py
"""
...
class QueryEngine:
    ...
    async def search(
        ...
        try:
            ...
            logger.debug(
                f"Generating embedding for query: {query[:50]}... (task: {task or 'auto'})"
            )
            query_embedding = await self.embedding_provider.embed_single(
    ...
    async def _get_context(
        ...
        try:
            ...
            context_before = (
                "".join(lines[start_idx:before_idx]) if before_count > 0 else None
            )
            context_after = (
                "".join(lines[after_idx:end_idx]) if after_count > 0 else None
            )
