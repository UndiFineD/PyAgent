# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LLMTornado\src\LlmTornado.Tests\Static\Files\Skills\skill-creator\scripts\validate_skill.py
#!/usr/bin/env python3
"""
...
class SkillValidator:
    ...
    def check_workflow_steps(self):
        ...
        if len(steps) >= 3:
            ...
            self.warnings.append(
                f"Only {len(steps)} workflow steps found (recommend 3+)"
            )
    ...
    def check_markdown_formatting(self):
        ...
        long_lines = [
            ...
            if len(line) > 500 and not line.strip().startswith("http")
        ]
