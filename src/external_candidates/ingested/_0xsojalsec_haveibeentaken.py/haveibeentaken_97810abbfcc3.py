# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-haveibeentaken\haveibeentaken.py
import argparse
import os
...
def check_username(username):
    ...
    print(
        f"{yellow}[+] {blue}Checking if {bblue}@{username}{blue} is taken or not{reset}"
    )
    resp = req.get(f"https://infosec.exchange/@{username}")
    if resp.status_code == 404:
        print(
            f"{bgreen}[✓] {bblue}@{username}{bgreen} is available for registration!{reset}"
        )
...
def check_usernames(filename):
    if os.path.isfile(filename):
        ...
        for i in f.readlines():
            ...
            if resp.status_code == 404:
                print(
                    f"{bgreen}[✓] {bblue}@{i.strip()}{bgreen} is available for registration!{reset}"
                )
            elif resp.status_code == 200:
