# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-extract-api\app\tasks.py
import os
import time
...
OCR_STRATEGIES = {"marker": MarkerOCRStrategy(), "tesseract": TesseractOCRStrategy()}
...
def ocr_task(self, pdf_bytes, strategy_name, pdf_hash, ocr_cache, prompt, model):
    ...
    if strategy_name not in OCR_STRATEGIES:
        raise ValueError(
            f"Unknown strategy '{strategy_name}'. Available: marker, tesseract"
        )
