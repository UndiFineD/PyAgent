# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-extract-api\app\main.py
import os
import time
...
from tasks import OCR_STRATEGIES, ocr_task
...
async def ocr_endpoint(
    ...
    if strategy not in OCR_STRATEGIES:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown strategy '{strategy}'. Available: marker, tesseract",
        )

    print(
        f"Processing PDF {file.filename} with strategy: {strategy}, ocr_cache: {ocr_cache}, model: {model}"
    )

    # Asynchronous processing using Celery
    task = ocr_task.apply_async(
        args=[pdf_bytes, strategy, pdf_hash, ocr_cache, prompt, model]
    )
