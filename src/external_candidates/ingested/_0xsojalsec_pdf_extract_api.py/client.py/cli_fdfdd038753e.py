# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-pdf-extract-api\client\cli.py
import argparse
import os
import time
...
def upload_file(
    file_path, ocr_cache, prompt, prompt_file=None, model="llama3.1", strategy="marker"
):
    ...
    if response.status_code == 200:
        ...
        if respObject.get("task_id"):
            ...
            return {"text": respObject.get("text")}  # sync mode support
    else:
...
def main():
    ...
    clear_cache_parser = subparsers.add_parser(
        "clear_cache", help="Clear the OCR result cache"
    )
    ...
    if args.command == "ocr":
        print(args)
        result = upload_file(
            args.file, args.ocr_cache, args.prompt, args.prompt_file, args.model
        )
        if result is None:
