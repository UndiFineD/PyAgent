# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\LHM\models\rendering\utils\ray_marcher.py
...
class MipRayMarcher2(nn.Module):
    ...
    def run_forward(self, colors, densities, depths, rendering_options, bg_colors=None):
        ...
        composite_depth = torch.clamp(
            composite_depth, torch.min(depths), torch.max(depths)
        )
        ...
        if rendering_options.get("white_back", False):
            ...
            assert (
                bg_colors is not None
            ), "Must provide bg_colors if white_back is False"
