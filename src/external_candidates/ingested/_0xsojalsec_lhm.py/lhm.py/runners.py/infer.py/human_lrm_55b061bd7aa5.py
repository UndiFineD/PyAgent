...
class HumanLRMInferrer(Inferrer):
    ...
    def infer_mesh(
        ...
        gs_app_model_list, query_points, transform_mat_neutral_pose = (
            self.model.infer_single_view(
        ...
        output_gs = self.model.animation_infer_gs(
            gs_app_model_list, query_points, smplx_params
        )
    ...
    def infer(self):
        ...
        for image_path in tqdm(
            ...
            try:
                assert (
                    shape_pose.ratio > 0.4
                ), f"body ratio is too small: {shape_pose.ratio}"
            except:
                continue
