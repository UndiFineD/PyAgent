# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\blocks\cross_attn_transformer.py
# Multi-HMR
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        self, inp: torch.Tensor, *args, context=None, context_list=None, mask=None
    ):
        ...
        x += self.pos_embedding[:, 0][
            :, None, :
        ]  # For now, we don't wish to embed a position. We might in future versions though.
        x = self.transformer(
            x, *args, context=context, context_list=context_list, mask=mask
        )
        return x
