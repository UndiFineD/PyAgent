# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-LHM\engine\pose_estimation\third-party\ViTPose\mmpose\models\misc\discriminator.py
# ------------------------------------------------------------------------------
...
class BaseDiscriminator(nn.Module):
    ...
    def _check(self):
        ...
        if not isinstance(self.fc_layers, tuple):
            raise TypeError(f"fc_layers require tuple, " f"get {type(self.fc_layers)}")

        if not isinstance(self.use_dropout, tuple):
            raise TypeError(
                f"use_dropout require tuple, " f"get {type(self.use_dropout)}"
            )

        if not isinstance(self.drop_prob, tuple):
            raise TypeError(f"drop_prob require tuple, " f"get {type(self.drop_prob)}")

        if not isinstance(self.use_activation, tuple):
            raise TypeError(
                f"use_activation require tuple, " f"get {type(self.use_activation)}"
            )
...
class ShapeDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, fc_layers, use_dropout, drop_prob, use_activation):
        if fc_layers[-1] != 1:
            msg = (
                f"the neuron count of the last layer "
...
class FullPoseDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, fc_layers, use_dropout, drop_prob, use_activation):
        if fc_layers[-1] != 1:
            msg = (
                f"the neuron count of the last layer must be 1,"
