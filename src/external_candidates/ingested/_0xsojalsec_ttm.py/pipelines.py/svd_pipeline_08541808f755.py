# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-TTM\pipelines\svd_pipeline.py
...
class StableVideoDiffusionTTMPipeline(StableVideoDiffusionPipeline):
    ...
    def encode_frames(
        self, frames: torch.Tensor, num_frames: int, encode_chunk_size: int = 14
    ):
        """
        Args:
        ...
        if frames.dim() != 5:
            raise ValueError(
                f"Expected frames with shape [B, C, T, H, W], got {list(frames.shape)}"
            )
    ...
    def convert_rgb_mask_to_latent_mask(
        self, mask: torch.Tensor, first_different=True
    ) -> torch.Tensor:
        """
        Args:
    ...
    def __call__(
        ...
        if not (0 <= tstrong_index < num_inference_steps):
            raise ValueError(
                f"tstrong_index must be in [0, {num_inference_steps-1}], got {tstrong_index}."
            )
        if not (0 <= tweak_index < num_inference_steps):
            raise ValueError(
                f"tweak_index must be in [0, {num_inference_steps-1}], got {tweak_index}."
            )
        if not (tstrong_index > tweak_index):
            raise ValueError(
                f"Require tweak_index < tstrong_index, got {tweak_index} >= {tstrong_index}."
            )
        ...
        if num_frames < refT:
            logger.warning(
                f"num_frames ({num_frames}) < input frames ({refT}); trimming reference video."
            )
            ref_vid = ref_vid[:, :, :num_frames]
        elif num_frames > refT:
            raise ValueError(
                f"num_frames ({num_frames}) is greater than input frames ({refT}). This is not supported."
            )

        ref_latents = self.encode_frames(
            ref_vid, num_frames, decode_chunk_size
        ).detach()
        ref_latents = ref_latents.to(dtype=latents.dtype, device=device)

        if not hasattr(
            self, "vae_scale_factor_temporal"
        ):  # encode ref video to latents
            if hasattr(self.vae, "scale_factor_temporal"):
                ...
                if ref_latents.shape[1] == num_frames:
                    ...
                    raise ValueError(
                        "Please configure the temporal scale factor of the VAE."
                    )
        ...
        mB, mC, mT, mH, mW = (
            ref_mask.shape
        )  # do resizing with nearest neighbor to avoid interpolation artifacts
        ref_mask = (
            F.interpolate(
        ...
        if ref_mask.shape[0] > num_frames:
            print(
                f"Warning: num_frames ({num_frames}) is less than input mask frames ({mT}). Trimming to {num_frames}."
            )
            ref_mask = ref_mask[:num_frames]
        elif ref_mask.shape[0] < num_frames:
            raise ValueError(
                f"num_frames ({num_frames}) is greater than input mask frames ({mT}). This is not supported."
            )
        ref_mask = (ref_mask > 0.5).float().max(dim=1, keepdim=True)[0]  # [T, 1, H, W]
        motion_mask = self.convert_rgb_mask_to_latent_mask(
            ref_mask, False
        )  # [1, T, 1, H, W]
            ...
            for i, t in enumerate(timesteps[tweak_index:]):
                ...
                if in_between_tweak_tstrong:
                    if i + tweak_index + 1 < len(timesteps):
                        prev_t = torch.tensor(
                            [timesteps[i + tweak_index + 1]], device=device
                        )
                        noisy_latents = self.scheduler.add_noise(
                            ref_latents, fixed_noise, prev_t
                        ...
                        raise ValueError(
                            f"Unexpected timestep index {i+tweak_index+1} >= {len(timesteps)}"
                        )
