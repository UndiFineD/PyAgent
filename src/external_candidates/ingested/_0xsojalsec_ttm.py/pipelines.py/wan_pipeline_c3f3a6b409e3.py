# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-TTM\pipelines\wan_pipeline.py
...
class WanImageToVideoTTMPipeline(WanImageToVideoPipeline):
    ...
    model_cpu_offload_seq = (
        "text_encoder->image_encoder->transformer->transformer_2->vae"
    )
    ...
    def __call__(
        ...
        if (
            ...
            and self.transformer.config.image_dim is not None
        ):
        ...
        if tweak_index >= len(timesteps):
            raise ValueError(
                f"`tweak_index` ({tweak_index}) must be < {len(timesteps)}."
            )
        ...
        if tstrong_index >= len(timesteps):
            raise ValueError(
                f"`tstrong_index` ({tstrong_index}) must be < {len(timesteps)}."
            )
        if tstrong_index < max(0, tweak_index):
            raise ValueError(
                f"`tstrong_index` ({tstrong_index}) must be >= `tweak_index` ({tweak_index})."
            )
        ...
        if last_image is not None:
            last_image = self.video_processor.preprocess(
        ...
        ref_latents = retrieve_latents(
            self.vae.encode(ref_vid), sample_mode="argmax"
        )  # [1, z, T', H', W']
        latents_mean = (
        ...
        if mask_tc_hw.shape[0] > num_frames:  # Align time dimension to num_frames
            logger.warning(
                "Mask has %d frames but num_frames=%d; trimming.",
                mask_tc_hw.shape[0],
                num_frames,
            )
            ...
            raise ValueError(
                f"num_frames ({num_frames}) is greater than mask frames ({mask_tc_hw.shape[0]}). "
                "Please pad/extend your mask or lower num_frames."
            )

        if (
            mask_tc_hw.shape[1] > 1
        ):  # Reduce channels if needed -> [T,1,H,W], binarize once
            mask_t1_hw = (mask_tc_hw > 0.5).any(dim=1, keepdim=True).float()
        else:
            mask_t1_hw = (mask_tc_hw > 0.5).float()

        motion_mask = (
            self.convert_rgb_mask_to_latent_mask(mask_t1_hw)
            .permute(0, 2, 1, 3, 4)
            .contiguous()
        )
        ...
        if tweak_index >= 0:
            ...
            tweak = torch.as_tensor(
                tweak, device=ref_latents.device, dtype=torch.long
            ).view(1)
        ...
        with self.progress_bar(total=len(timesteps) - tweak_index) as progress_bar:
            for i, t in enumerate(timesteps[tweak_index:]):
                ...
                if in_between_tweak_tstrong:
                    if i + tweak_index + 1 < len(timesteps):
                        ...
                        prev_t = torch.as_tensor(
                            prev_t, device=ref_latents.device, dtype=torch.long
                        ).view(1)
                        noisy_latents = self.scheduler.add_noise(
                        ...
                        latents = (
                            latents * background_mask
                            + ref_latents.to(dtype=latents.dtype, device=latents.device)
                            * motion_mask
                        )
                ...
                if i == len(timesteps) - tweak_index - 1 or (
