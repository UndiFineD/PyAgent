# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-TTM\pipelines\cog_pipeline.py
...
class CogVideoXImageToVideoTTMPipeline(CogVideoXImageToVideoPipeline):
    ...
    @torch.no_grad()
    def encode_frames(
        self, frames: torch.Tensor, vae_scale_factor: float = None
    ) -> torch.Tensor:
        ...
        return latents.permute(
            0, 2, 1, 3, 4
        ).contiguous()  # shape (B, C, F, H, W) -> (B, F, C, H, W)
    ...
    def convert_rgb_mask_to_latent_mask(self, mask: torch.Tensor) -> torch.Tensor:
        ...
        pooled = F.interpolate(
            pooled, size=(pooled.shape[2], H_latent, W_latent), mode="nearest"
        )
    ...
    def __call__(
        ...
        ref_vid = load_video_to_tensor(motion_signal_video_path).to(
            device=device
        )  # shape [1, C, T, H, W]
        ...
        ref_vid = (
            ...
            .reshape(refB, refT, refC, height, width)
            .permute(0, 2, 1, 3, 4)
        )

        ref_vid = self.video_processor.normalize(
            ref_vid.to(dtype=self.vae.dtype)
        )  # Normalize and convert dtype for VAE encoding
        ref_latents = (
            self.encode_frames(ref_vid).float().detach()
        )  # shape [1, T, C, H, W]

        ref_mask = load_video_to_tensor(motion_signal_mask_path).to(
            device=device
        )  # shape [1, C, T, H, W]
        ...
        ref_mask = (
            ...
            .reshape(mB, mT, mC, height, width)
            .permute(0, 2, 1, 3, 4)
        )
        ref_mask = (
            ref_mask[0].permute(1, 0, 2, 3).contiguous()
        )  # (1, C, T, H, W) -> (T, H, W, 1)
            ...
            for i, t in enumerate(timesteps[tweak_index:]):
                ...
                if in_between_tweak_tstrong:
                    if i + tweak_index + 1 < len(timesteps):
                        ...
                        noisy_latents = self.scheduler.add_noise(
                            ref_latents, fixed_noise, prev_t.long()
                        ).to(dtype=latents.dtype, device=latents.device)
                        latents = (
                            latents * background_mask + noisy_latents * motion_mask
                        )
                    else:
