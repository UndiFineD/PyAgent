# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-TTM\run_wan.py
try:
    import argparse
    import os
...
def parse_args():
    ...
    parser.add_argument(
        "--input-path",
        type=str,
        default="./examples/wan_monkey",
        help="Path to input image",
    )
    parser.add_argument(
        "--output-path",
        type=str,
        default="./outputs/output_wan_monkey.mp4",
    ...
    parser.add_argument(
        "--max-area", type=int, default=480 * 832, help="Maximum area for resizing"
    )
    parser.add_argument(
        "--num-frames", type=int, default=81, help="Number of frames to generate"
    )
    parser.add_argument(
        "--guidance-scale",
        type=float,
