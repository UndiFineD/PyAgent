...
class UrlChecker:
    ...
    def __init__(self, tenant_name, userdata, enum_method):
        ...
        self.sanitycount = 0
        self.sanitylimit = (
            100  # this is how many users to try before performing a sanitycheck
        )
    ...
    def control_fireprox_endpoint(self, method, command):
        ...
        if command == "create":
            ...
            if result:
                print(
                    "Hold up - we have an old one in here. Removing first. Waiting 30s"
                )
    ...
    def sql_insert_user(
        self, email_address, source_domain, target_tenant, scrape_date_unix, hostname
    ):
        if not halt_logging:
            try:
                ...
                print(
                    "Some SQLite error in sql_insert_user! Maybe write some better logging next time."
                )
    ...
    def nestoriLookup(self, username):
        ...
        guest_username = (
            f"{username.replace('@','_')}#EXT#@{self.tenant_name}.onmicrosoft.com"
        )
        ...
        try:
            ...
            if num_0_responses >= 3:
                ...
                isPaused = True
                print(
                    "Stopping because 3 or more of the last 5 responses were showing valid (false positives)."
                )
            ...
            if if_exists_result == 1:
                ...
                try:
                    self.sql_insert_user(
                        username, source_domain, self.tenant_name, currenttime, hostname
                    )
                except:
            ...
            if verbose:
                if username != fakeuser:
                    print(
                        f"{username}:{guest_username}:{self.tenant_name}.onmicrosoft.com:{english_status}"
                    )
    ...
    def check_url(self, username):
        ...
        if debug:
            print(
                f"Our CURRENT COUNT IS {self.currentcount} and the sanitylimit is {self.sanitylimit}"
            )
...
def lookup_tenant(domain):
    ...
    try:
        ...
        domain_extract = re.findall(
            "<Domain>(.*?)<\/Domain>", r.content.decode("utf-8")
        )
        tenant_extract = [
            i
            for i, x in enumerate(domain_extract)
            if ".onmicrosoft.com" in x and ".mail.onmicrosoft.com" not in x
        ]  # this line gets the matching list item numbers only
        if len(tenant_extract) > 0:
            print(f"\nTenants Identified:\n---------------------")
            for found_tenant in tenant_extract:
                cleaned_tenant = (
                    (domain_extract[found_tenant])
...
def export_sql_users():
    if enable_db:
        ...
        try:
            ...
            with open(
                output_filename, "w"
            ) as f:  # 'w' means write mode which overwrites existing contents
                for user in export_results:
                    f.write(user[0] + "\n")  # write each email on a new line
            conn.close()
            print(
                f"{resultcount} guestlist lines have been written to {output_filename}"
            )
...
def main():
    ...
    parser.add_argument(
        "-D", "--debug", help="enable debug output", action="store_true", default=False
    )
    ...
    parser.add_argument(
        "-c", "--command", help="fireprox command (create, delete, list)", metavar=""
    )
    ...
    if args.access and args.secret:
        ...
        print(
            "You have not specified all required values for fireprox: --access, --secret, and --region"
        )
        print(
            " **** WARNING: FIREPROX IS NOT ENABLED. Waiting for 10 seconds before continuing ...  ****"
        )
        time.sleep(10)
    ...
    if isUser:
        ...
        try:
            ...
            if fireprox_enabled:
                ...
                try:
                    result = url_checker.control_fireprox_endpoint(
                        enum_method, "delete"
                    )
                except:
                    pass
