# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-guestlist\fire.py
#!/usr/bin/env python3
import argparse
import configparser
import datetime
...
class FireProx(object):
    ...
    def load_creds(self) -> bool:
        ...
        if self.profile_name in credentials:
            if config_profile_section not in config:
                print(
                    f"Please create a section for {self.profile_name} in your ~/.aws/config file"
                )
                return False
            ...
            try:
                ...
                if self.profile_name:
                    ...
                    if self.session_token:
                        ...
                        credentials.remove_option(
                            self.profile_name, "aws_session_token"
                        )
    ...
    def create_api(self, url):
        ...
        response = self.client.import_rest_api(
            parameters={"endpointConfigurationTypes": "REGIONAL"}, body=template
        )
    ...
    def store_api(
        self, api_id, name, created_dt, version_dt, url, resource_id, proxy_url
    ):
        print(f"[{created_dt}] ({api_id}) {name} => {proxy_url} ({url})")
...
def parse_arguments() -> Tuple[argparse.Namespace, str]:
    ...
    parser.add_argument(
        "--secret_access_key", help="AWS Secret Access Key", type=str, default=None
    )
    parser.add_argument(
        "--session_token", help="AWS Session Token", type=str, default=None
    )
    parser.add_argument("--region", help="AWS Region", type=str, default=None)
    parser.add_argument(
        "--command",
