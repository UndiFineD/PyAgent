# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-WebSecProbe\WebSecProbe\main.py
...
class WebSecProbe:
    ...
    def send_request(self, payload):
        ...
        response = requests.get(
            full_url
        )  # Remove verify=False to enable SSL certificate verification
    ...
    def run(self):
        ...
        if not self.is_valid_url():
            print(
                "Invalid URL. It should start with http:// or https:// and should not end with /"
            )
            exit(1)
