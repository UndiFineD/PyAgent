    ...
    def start(self):
        ...
        def send_payload(json_path):
            try:
                ...
                if (
                    json_path.split(pdl + "payload" + pdl, 1)[1].split(pdl)[0].upper()
                    in self.exclude_dir
                ):
                    return
                ...
                if not payload:
                    ...
                    if self.wb_result_json:
                        self.wb_result["FAILED"].append(
                            {
                                json_path.split(pdl + "payload" + pdl, 1)[
                                    1
                                ]: "No payloads found"
                            }
                        )
                ...
                for z in payload:
                    ...
                    for encode in encode_list:
                        ...
                        if z == "URL":
                            ...
                            v = self.test_url(
                                json_path, z, payload, method, headers, encode
                            )

                            if self.wb_result_json:
                                self.test_resp_status_processing(
                                    k.split(pdl + "payload" + pdl, 1)[1], v
                                )
                            else:
                                self.wb_result[k] = v
                            ...
                            if not encode:
                                ...
                                k = ":".join(
                                    [str(json_path), str(z).upper(), encode.upper()]
                                )
                            ...
                            v = self.test_body(
                                json_path, z, payload, method, body, headers, encode
                            )
                ...
                if self.wb_result_json:
                    ...
                    self.wb_result["FAILED"].append(
                        {relative_path.split(pdl + "payload" + pdl, 1)[1]: err}
                    )
                else:
    ...
    def test_err_resp_code_processing(self, json_path, z, encode, result):
        try:
            ...
            if self.wb_result_json:
                ...
                err = "An incorrect response was received while processing request: {}".format(
                    result[1]
                )
    ...
    def test_fail_response_processing(self, json_path, z, encode, error):
        try:
            if self.wb_result_json:
                ...
                self.wb_result["FAILED"].append(
                    {
                        json_path.split(pdl + "payload" + pdl, 1)[1]
                        + " in "
                        + z: str(error)
                    }
                )
            else:
                err = "An error occurred while processing file {} in {}: {}".format(
                    json_path, z, error
                )
                print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))
        except Exception as e:
    ...
    def test_noblocked(self, method, headers):
        try:
            ...
            if self.wb_result_json:
                ...
                err = (
                    "An incorrect response was received while test request: {}".format(
                        error
                    )
                )
                print("{}{}{}".format(Fore.YELLOW, err, Style.RESET_ALL))
    ...
    def test_url(self, json_path, z, payload, method, headers, encode):
        try:
            ...
            result = processing_result(
                payload["BLOCKED"], self.block_code, result.status_code
            )
            v = result[0]
