# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-waf-bypass\payloads.py
#!/usr/bin/env python3
...
def get_payload(json_path):
    ...
    with open(json_path) as f:
        try:
            ...
            print(
                "An error occurred while loading file {}: file not in JSON format ({})".format(
                    json_path, e
                )
            )
            return {}
    ...
    ret["COOKIE"] = (
        None if not cookie else cookie.replace("%RND%", secrets.token_hex(3))
    )
    ...
    ret["REFERER"] = (
        None if not referer else referer.replace("%RND%", secrets.token_hex(3))
    )

    # header
    header = jdata.get("HEADER", None)
    ret["HEADER"] = (
        None if not header else header.replace("%RND%", secrets.token_hex(3))
    )
