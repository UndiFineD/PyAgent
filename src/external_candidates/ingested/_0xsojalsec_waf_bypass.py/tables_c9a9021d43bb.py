# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-waf-bypass\tables.py
#!/usr/bin/env python3
...
def table_get_result_details(fp, fn):
    ...
    def get_result_details(fx, status):
        ...
        print("")
        fx_type = (
            ">> FALSE POSITIVE PAYLOADS"
            if status == "FALSED"
            else ">> FALSE NEGATIVE PAYLOADS"
        )
        print(fx_type)
        print("")
...
def table_get_result_summary(wb_result, pdl):
    ...
    payloads_list = list(
        set([pdl.join(k.split(":", 1)[0].split(pdl)[:-1]) for k in wb_result.keys()])
    )

    # create result dictionary by payloads type
    for payloads in payloads_list:

        payloads_type = payloads.split(pdl + "payload" + pdl)[1].split(pdl)[
            0
        ]  # leave payload type only
        ...
        passed = len(
            [
                ...
                if k.startswith(payloads) and v == "PASSED"
            ]
        )
        failed = len(
            [
                k
                for k, v in wb_result.items()
                if k.startswith(payloads) and v == "FAILED"
            ]
        )
        fx = len(
            [k for k, v in wb_result.items() if k.startswith(payloads) and v == fx_type]
        )
    ...
    payloads_summary_list.append(
        [len(wb_result.items()), passed, falsed, bypassed, failed]
    )
    ...
    if total != len(wb_result):
        print(
            "An error occurred while processing the result: {} != {}".format(
                total, len(wb_result)
            )
        )
