# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-ldeep\ldeep\__main__.py
#!/usr/bin/env python3
...
class Ldeep(Command):
        ...
        try:
            self.display(
                self.engine.query(
                    ...
                    attributes,
                    base=",".join(
                        ["CN=Partitions,CN=Configuration", self.engine.base_dn]
                    ),
                ),
                verbose,
            )
            ...
            self.display(
                self.engine.query(self.engine.FSMO_DOMAIN_FILTER(), attributes), verbose
            )
    ...
    def list_delegations(self, kwargs):
        ...
        if filter_ == "all":
            ...
            entries = self.engine.query(
                self.engine.RESOURCE_BASED_CONSTRAINED_DELEGATION_FILTER(), attributes
            )
        else:
            return None
