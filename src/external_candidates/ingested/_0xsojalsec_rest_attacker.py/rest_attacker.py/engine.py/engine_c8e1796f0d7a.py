# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\engine\engine.py
...
class Engine:
    ...
    def __init__(
        self, config: EngineConfig, checks: list[TestCase], handlers=[]
    ) -> None:
        """
    ...
    def current_check(self) -> None:
        ...
        """
        logging.debug(
            f"Starting: Check {self.checks[self.index].check_id} "
        ...
        try:
            ...
            logging.warning(
                (
                    f"Check {current_check.check_id} did not execute: "
        ...
        logging.debug(
            f"Finished: Check {self.checks[self.index].check_id} ({current_check.get_test_case_id()}) "
            " with status: "
    ...
    def export(self, output_dir) -> None:
        ...
        for check in self.checks:
            try:
                ...
                logging.exception(
                    f"Report for check {check.check_id} could not be generated.",
                    exc_info=exc,
                )
    ...
    def update_handlers(self):
        ...
        if current_check.live_type is LiveType.ONLINE:
            if self.state.rate_limit:
                rl_limit_reached = self.state.rate_limit.update(
                    current_check.result.last_response
                )
            ...
            if self.state.access_limit:
                # Check interval
                if (
                    self.state.access_limit.current_pos
                    >= self.state.access_limit.interval
                ):
                    ...
                    if acc_limit_reached:
                        ...
                        while (
                            self.checks[self.index].check_id
                            != self.state.access_limit.last_check_id
                            and self.index > -1
                        ):
                            # Clear test result
                            self.checks[self.index].result = TestResult(
                                self.checks[self.index]
                            )
                            self.index -= 1
                        ...
                        self.state.access_limit.last_check_id = self.checks[
                            self.index
                        ].check_id
