# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\headers.py
...
class FindCustomHeaders(TestCase):
    ...
    def run(self):
        ...
        for header_id, header in response_headers.items():
            if (
                header_id.lower() in STANDARD_HEADERS
                or header_id.lower() in COMMON_HEADERS
            ):
                continue

            unique_headers.update({header_id: header})
        ...
        for descr in config.descriptions.values():
            for server in descr["servers"]:
                nonauth_request = RequestInfo(
                    server["url"], "/", "get"  # TODO: better default path
                )
                ...
                if config.auth:
                    ...
                    auth_info = AuthRequestInfo(
                        config.auth, policy=AccessLevelPolicy.MAX
                    )
                    test_cases.append(
                        FindCustomHeaders(cur_check_id, auth_request, auth_info)
                    )
                    cur_check_id += 1
    ...
    def run(self):
        ...
        for header_id, header in response_headers.items():
            if header_id.lower() in SECURITY_HEADERS:
                security_headers.update({header_id: header})
        ...
        for descr in config.descriptions.values():
            for server in descr["servers"]:
                ...
                if config.auth:
                    ...
                    test_cases.append(
                        FindSecurityHeaders(cur_check_id, auth_request, auth_info)
                    )
                    cur_check_id += 1
...
class MetaCompareHeaders(TestCase):
    ...
    def run(self):
        ...
        if not (
            self.check_left.result.status is CheckStatus.FINISHED
            and self.check_right.result.status is CheckStatus.FINISHED
        ):
            raise Exception(
                f"Cannot run meta check {self}. Dependent checks are not finished."
            )
    ...
    def generate(cls, config, check_id_start=0):
        ...
        cur_check_id = check_id_start
        test_cases = []
        new_checks = FindCustomHeaders.generate(config, check_id_start)
        ...
        if len(new_checks) >= 2:
            ...
            test_cases.append(
                MetaCompareHeaders(cur_check_id, new_checks[0], new_checks[1])
            )
            cur_check_id += 1

        new_checks = FindSecurityHeaders.generate(config, check_id_start)
        test_cases.extend(new_checks)
        cur_check_id += len(new_checks)
