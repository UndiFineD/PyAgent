# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\resources.py
...
class TestObjectIDInvalidUserAccess(TestCase):
                ...
                for path_id, path in descr.endpoints.items():
                    for op_id, op in path.items():
                        ...
                        if descr.requires_parameters(path_id, op_id):
                            ...
                            req_parameters = descr.get_required_param_defs(
                                path_id, op_id
                            )

                            replacments = replace_params(
                                path, default_user, req_parameters
                            )
                            if not replacments:
                        ...
                        if replacments:
                            ...
                            request_info = RequestInfo(server["url"], path_id, op_id)
                        ...
                        if config.auth:
                            ...
                            test_cases.append(
                                TestObjectIDInvalidUserAccess(
                                    cur_check_id, request_info, auth_info
                                )
                            )
                            cur_check_id += 1
    ...
    def deserialize(cls, serialized, config, check_id: int = 0):
        ...
        auth_info = AuthRequestInfo.deserialize(
            serialized.pop("auth_info"), config.auth
        )

        return TestObjectIDInvalidUserAccess(
            check_id, request_info, auth_info, **serialized
        )
...
class CountParameterRequiredRefs(TestCase):
            ...
            for op_id, op in path.items():
                for param_id in self.description.get_required_param_ids(path_id, op_id):
                    if param_id not in unique_parameter_count.keys():
                        unique_parameter_count.update({param_id: 0})
    ...
    def propose(self, config, check_id_start):
        ...
        new_checks.append(
            FindParameterReturns(
                check_id_start, self.description, list(self.result.value.keys())
            )
        )
...
class FindIDParameters(TestCase):
    ...
    def run(self):
        ...
        for path_id, path in paths.items():
            for op_id, op in path.items():
                parameters = self.description.get_required_param_defs(
                    path_id, op_id
                ).values()
                    ...
                    for candidate in candidate_strings:
                        if candidate in param_name.lower():
                            if param_name not in unique_parameters:
                                unique_parameter_count.update({param_name: 0})
                                unique_parameters.add(param_name)

                            unique_parameter_count[param_name] += 1

                            endpoints.update({path_id: (op_id, param_loc, param_name)})
...
class FindParameterReturns(TestCase):
    ...
    def __init__(
        self, check_id: int, description: OpenAPI, parameters: list[str]
    ) -> None:
        """
                ...
                for status_code, response in responses.items():
                    ...
                    for cty in response_content.values():
                        ...
                        if "$ref" in schema_def.keys():
                            schema_def = self.description.resolve_ref(
                                schema_def["$ref"]
                            )
                        ...
                        for schema in schema_defs:
                            ...
                            for param_id in found_params:
                                ...
                                if param_name not in param_locations.keys():
                                    param_locations[param_name] = {"endpoints": []}
                                ...
                                param_locations[param_name]["endpoints"].append(
                                    {
                                        ...
                                        "required_param_count": len(
                                            self.description.get_required_param_ids(
                                                path_id, op_id
                                            )
                                        ),
                                    }
    ...
    def _recursive_search(self, schema_object: dict) -> list[str]:
        ...
        for param_name, param in schema_object.items():
            ...
            for candidate in self.search_parameters:
                if candidate in param_name.lower() or candidate in param_descr.lower():
    ...
    def propose(self, config, check_id_start):
        ...
        for path_name, path_item in self.result.value["endpoints"].items():
            new_request = RequestInfo(
                self.description["servers"][0][
                    "url"
                ],  # TODO: What if there are multiple servers?
                path_name,
                path_item["method"],
            )
            ...
            new_checks.append(
                GetParameters(
                    check_id_start, new_request, new_auth_info, parameters=params
                )
            )
    ...
    def run(self):
                        ...
                        for schema in schema_defs:
                            ...
                            for param_id, param_meta in found_params.items():
                                ...
                                if param_id not in params.keys():
                                    params.update({param_id: param_meta})

                                else:
                                    params[param_id]["endpoints"].append(
                                        param_meta["endpoints"]
                                    )
        ...
        for param_name, param_data in params.items():
            if param_data["count"] > 1:
                candidate_params.update({param_name: param_data})
        ...
        for component_ref, component_data in components.items():
            if component_data["count"] > 1:
                candidate_components.update({component_ref: component_data})
        ...
        self.result.value = {
            ...
            "components": dict(
                sorted(candidate_components.items(), key=lambda dic: dic[0])
            ),
        }
    ...
    def _recursive_search(self, schema_object: dict) -> dict[str, dict]:
        ...
        for param_name, param in schema_object.items():
            if param_name not in found_params.keys():
                found_params.update({param_name: {"count": 1}})
