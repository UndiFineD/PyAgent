...
class TestOptionsHTTPMethod(TestCase):
    ...
    def run(self):
        ...
        if "allow" in response.headers.keys():
            ...
            allowed_methods = (
                response.headers["access-control-allow-methods"].lower().split(", ")
            )
                ...
                for server_url in server_urls:
                    ...
                    test_cases.append(
                        TestOptionsHTTPMethod(
                            cur_check_id, auth_request, auth_info, claims=claims
                        )
                    )
                    cur_check_id += 1
    ...
    def run(self):
        ...
        for check in checks:
            if check.result.issue_type == IssueType.CANDIDATE:
                ...
                self.result.value["total_missing_claims"] += len(
            ...
            for path_id, path in descr.endpoints.items():
                ...
                for op in HTTP_REQUEST_METHODS:
                    ...
                    for server_url in server_urls:
                        ...
                        test_cases.append(
                            TestAllowedHTTPMethod(cur_check_id, auth_request, auth_info)
                        )
                        cur_check_id += 1
...
class MetaTestAllowedHTTPMethod(TestCase):
    ...
    def run(self):
        ...
        if not all(
            check.result.status == CheckStatus.FINISHED for check in self.checks
        ):
            raise Exception(
