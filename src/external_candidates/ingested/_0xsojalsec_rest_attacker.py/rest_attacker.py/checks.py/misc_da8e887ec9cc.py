# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\misc.py
...
class GetHeaders(TestCase):
    ...
    def run(self):
        ...
        for header_id in self.headers:
            self.result.value.update({header_id: response.headers[header_id]})
...
class GetParameters(TestCase):
    ...
    def run(self):
        ...
        for param in self.parameters:
            ...
            self.result.value.update({"/".join(param): current_item})
