# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\scopes.py
...
class CheckScopesEndpoint(TestCase):
    ...
    def __init__(
        self, check_id: int, request_info: RequestInfo, auth_info: AuthRequestInfo
    ) -> None:
        """
    ...
    def run(self):
        ...
        if 200 < response.status_code < 300:
            ...
            self.result.value = {"accepted": True}

        else:
            self.result.issue_type = IssueType.NO_CANDIDATE
            self.result.value = {"accepted": False}
            ...
            for path_id, path_item in descr["paths"]:
                ...
                for op_id, op in path_item.items():
                    ...
                    for server in descr["servers"]:
                        nonauth_request = RequestInfo(server["url"], path_id, op_id)
                        test_cases.append(
                            CheckScopesEndpoint(cur_check_id, nonauth_request)
                        )
                        cur_check_id += 1

                        auth_request = RequestInfo(server["url"], path_id, op_id)
                        auth_info = AuthRequestInfo(config.auth)
                        test_cases.append(
                            CheckScopesEndpoint(cur_check_id, auth_request, auth_info)
                        )
                        cur_check_id += 1
...
class ScopeMappingDescription(TestCase):
    ...
    def run(self):
        ...
        scoped_schemes = set()
        scopemap = {}
        for scheme_id, scheme in self.description["components"][
            "securitySchemes"
        ].items():
            if scheme["type"] != "oauth2":
                # TODO: Other scoped schemes?
        ...
        if len(scoped_schemes) == 0:
            logging.debug(
                "API description has defined no scoped schemes (e.g. OAuth2)."
            )
            self.result.status = CheckStatus.SKIPPED
            return
            ...
            for op_id, op in path.items():
                if "security" not in op.keys():
                    ...
                    if len(op["security"]) == 0:
                        ...
                        # No security --> Empty security requirement object
                        logging.debug(
                            f"{op_id} / {path_id} has empty security requirements."
                        )
...
class CompareTokenScopesToClientScopes(TestCase):
    ...
    def __init__(
        self, check_id: int, token: OAuth2Token, client_info: ClientInfo
    ) -> None:
        """
    ...
    def run(self) -> None:
        ...
        extra_scopes = received_scopes.difference(
            set(self.client_info.supported_scopes)
        )
        self.result.value = {}
    ...
    def generate(cls, config, check_id_start=0) -> list:
        ...
        for cred in config.credentials.values():
            if isinstance(cred, OAuth2TokenGenerator):
                ...
                test_cases.append(
                    CompareTokenScopesToClientScopes(
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.token = self.token_gen.request_new_token(
                scopes=None, grant_type=self.grant_type, policy=AccessLevelPolicy.NOPE
            )
    ...
    def generate(cls, config, check_id_start=0) -> list:
        ...
        for cred in config.credentials.values():
            ...
            if "token" in cred.client_info.supported_grants:
                test_cases.append(
                    TestTokenRequestScopeOmit(cur_check_id, cred, "token")
                )
                cur_check_id += 1
...
class TestTokenRequestScopeEmpty(TestCase):
    ...
    def run(self) -> None:
        ...
        try:
            self.token = self.token_gen.request_new_token(
                scopes=[], grant_type=self.grant_type, policy=AccessLevelPolicy.NOPE
            )
    ...
    def generate(cls, config, check_id_start=0) -> list:
        ...
        for cred in config.credentials.values():
            ...
            if "code" in cred.client_info.supported_grants:
                test_cases.append(
                    TestTokenRequestScopeEmpty(cur_check_id, cred, "code")
                )
                cur_check_id += 1

            if "token" in cred.client_info.supported_grants:
                test_cases.append(
                    TestTokenRequestScopeEmpty(cur_check_id, cred, "token")
                )
                cur_check_id += 1
...
class TestTokenRequestScopeInvalid(TestCase):
    ...
    def run(self) -> None:
        ...
        try:
            self.token = self.token_gen.request_new_token(
                scopes=[scope], grant_type=self.grant_type
            )
    ...
    def generate(cls, config, check_id_start=0) -> list:
        ...
        for cred in config.credentials.values():
            ...
            if "code" in cred.client_info.supported_grants:
                test_cases.append(
                    TestTokenRequestScopeInvalid(cur_check_id, cred, "code")
                )
                cur_check_id += 1

            if "token" in cred.client_info.supported_grants:
                test_cases.append(
                    TestTokenRequestScopeInvalid(cur_check_id, cred, "token")
                )
                cur_check_id += 1
    ...
    def run(self) -> None:
        ...
        if not self.first_token:
            self.first_token = self.token_gen.request_new_token(
                scopes=None, grant_type="code", policy=AccessLevelPolicy.NOPE
            )

        try:
            self.refreshed_token = self.token_gen.refresh_token(
                self.first_token, scopes=None
            )
