                ...
                for path_id, path in descr.endpoints.items():
                    for op_id, op in path.items():
                        ...
                        for status_code, response in op["responses"].items():
                            ...
                            if (
                                not "schema"
                                in response["content"]["application/json"].keys()
                            ):
                                continue

                            schema_def = response["content"]["application/json"][
                                "schema"
                            ]
                            schemas = [schema_def]
...
class CompareHTTPBodyAuthNonauth(TestCase):
    ...
    def run(self):
        ...
        common_values, unique_values_left, unique_values_right = _recursive_diff(
            response1_json, response2_json
        )
    ...
    def deserialize(cls, serialized, config, check_id: int = 0):
        ...
        return CompareHTTPBodyAuthNonauth(
