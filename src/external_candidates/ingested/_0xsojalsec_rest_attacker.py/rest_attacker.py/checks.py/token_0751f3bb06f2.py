# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\checks\token.py
...
from rest_attacker.util.auth.token_generator import (
    AccessLevelPolicy,
...
class TestReadOAuth2Expiration(TestCase):
    ...
    def run(self):
        ...
        if "expires_in" in self.token:
            ...
            self.result.value = {"vailidity_length": self.token["expires_in"]}

        if "expires_at" in self.token:
            self.result.issue_type = IssueType.CANDIDATE
            self.result.value = {"expires_at": self.token["expires_at"]}
...
class TestOAuth2Expiration(TestCase):
    ...
    def run(self):
        ...
        logging.debug(
            "Testing token expiration validation: -30 seconds after expiration time."
        )
        ...
        if not 200 <= response.status_code < 300:
            logging.warning(
                "Token could not be used before expiration time was reached."
            )
            self.result.status = CheckStatus.ERROR
            return
        ...
        logging.debug(
            "Testing token expiration validation: 1 seconds after expiration time."
        )
        time.sleep(31)
        response = self.request_info.send(auth_data)
        self.result.last_response = response
        ...
        logging.debug(
            "Testing token expiration validation: 60 seconds after expiration time."
        )
        time.sleep(60)
        response = self.request_info.send(auth_data)
        self.result.last_response = response
        ...
        logging.debug(
            "Testing token expiration validation: 300 seconds after expiration time."
        )
        time.sleep(240)
        response = self.request_info.send(auth_data)
        self.result.last_response = response
        ...
        self.result.value = {"min_validity_time": validity_time}
        self.result.status = CheckStatus.FINISHED
    ...
    def run(self) -> None:
        ...
        if not self.token:
            self.token = self.token_gen.request_new_token(
                grant_type="code", policy=AccessLevelPolicy.MAX
            )

        if not "refresh_token" in self.token:
            # Token is not refreshable
            logging.warning(
                f"No refresh token available received for checking {repr(self)}."
            )
            self.result.status = CheckStatus.ERROR
            return
...
class TestRefreshTokenClientBinding(TestCase):
    ...
    def run(self) -> None:
        ...
        if (
            self.token_gen0.client_info.client_id
            == self.token_gen1.client_info.client_id
        ):
        ...
        try:
            ...
            self.result.value["refresh_token"] = self.token["refresh_token"]
            self.result.value["refresher_client"] = (
                self.token_gen1.client_info.client_id
            )
