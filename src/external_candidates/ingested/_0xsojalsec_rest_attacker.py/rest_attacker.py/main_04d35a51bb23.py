# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\__main__.py
#!/usr/bin/env python3
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        "REST-Attacker", description=("Pentesting tool for analyzing REST APIs")
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to the service configuration. " "Can be a directory or OpenAPI file."
        ),
    )

    parser.add_argument(
        "--version", nargs=0, action=GetVersion, help="Print version number."
    )
    parser.add_argument(
        "--list",
        "-l",
    ...
    parser.add_argument(
        "--output-dir", default=None, help="Export path for logs and reports."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Generate fake input parameter values when generating checks. "
    ...
    run_cfg.add_argument(
        "--run", default=None, help="Start test run from a run configuration file."
    )
    run_cfg.add_argument(
        "--continue",
        default=None,
    ...
    filters.add_argument(
        ...
        nargs="+",
        type=str,
        choices={"NOPE", "OPTIONAL", "RECOMMENDED", "REQUIRED"},
    ...
    proxy.add_argument(
        "--cacert", default=None, type=str, help="Path to a custom CA certificate."
    )
...
def setup_outputs(args: argparse.Namespace) -> None:
    ...
    if args.output_dir:
        ...
        out_folder = datetime.utcfromtimestamp(time.time()).strftime(
            "%Y-%m-%dT%H-%M-%SZ"
        )
...
def setup_config(args: argparse.Namespace) -> EngineConfig:
    ...
    if not args.config_path.exists():
        ...
        args.config_path = (
            pathlib.Path().resolve() / "rest_attacker" / "cfg" / args.config
        )
...
def setup_run(cfg: EngineConfig, args: argparse.Namespace) -> list[TestCase]:
    ...
    test_filters = {}
    if args.test_cases:
        test_filters.update({"test_cases": args.test_cases})
    ...
    if args.run:
        ...
        checks = config_run.load_config(
            get_test_cases(), cfg, args.run_path, continue_run=True
        )
        ...
        logging.warning(
            "Use --run to specify run configuration or "
...
def setup_request_backend(args: argparse.Namespace) -> None:
    ...
    if args.proxy:
        RequestInfo.global_kwargs["proxies"] = {"http": args.proxy, "https": args.proxy}
    ...
    if args.handle_limits:
        ...
        if cfg.current_user_id:
            ...
            if default_user.userinfo_endpoint:
                ...
                handlers.append(
                    AccessLimitHandler(request_info, auth_info, interval=20)
                )
...
def main():
    ...
    try:
        ...
        logging.exception("Execution failed with the following error:", exc_info=e)
