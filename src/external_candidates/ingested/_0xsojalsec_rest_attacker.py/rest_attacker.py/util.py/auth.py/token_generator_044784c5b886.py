...
class OAuth2TokenGenerator:
    ...
    def get_token(
        ...
        if self._is_expired(self.active_token) and self._is_refreshable(
            self.active_token
        ):
            # TODO: Use requests_oauthlib's token refresher for automatic updates
            return self.request_new_token(
                scopes=scopes, grant_type="refresh_token", policy=policy
            )
    ...
    def request_new_token(
        ...
        if scopes is None:
            if policy is AccessLevelPolicy.MAX:
                ...
                "scope_required" in self.client_info.flags
                and policy is AccessLevelPolicy.DEFAULT
            ):
        ...
        logging.debug(
            f"Requesting new token from client '{self.client_info.client_id}'"
        )
        ...
        if grant_type == "code":
            ...
            if not self.active_token:
                raise Exception(
                    f"Action 'refresh_token' not available: No currently active token"
                )

            self.active_token = self.refresh_token(
                token=self.active_token, scopes=scopes
            )
            self._req_scopes = scopes
    ...
    def _request_impl_grant(self, scopes: list[str] = None) -> OAuth2Token:
        ...
        redirect_result = self._get_redirect_result(
            authorization_url, fragment_expected=True
        )
    ...
    def refresh_token(
        self, token: OAuth2Token, scopes: list[str] = None
    ) -> OAuth2Token:
        """
        ...
        session = OAuth2Session(
            client_id=self.client_info.client_id, scope=scopes, token=token
        )
    ...
    def _get_redirect_result(
        self, auth_url: str, fragment_expected: bool = False
    ) -> str:
        """
        ...
        if user_session:
            logging.debug(
                f"Selected user agent via established session: {user_session}"
            )
            if isinstance(user_session, ROBrowserSession):
                ...
                if RedirectHandler.called:
                    ...
                    logging.warning(
                        f"No redirect received after {server.timeout} seconds. "
                        "Token could not be generated"
                    )
                    return  # TODO: Custom error type

            else:
                redirect_response = user_session.session.get(
            ...
            cli_manual_request = (
                "Authorization required for OAuth2 " f"Authorization Grant:\n{auth_url}"
            )
