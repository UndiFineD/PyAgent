...
class AuthGenerator:
    ...
    def get_auth(
            ...
            for scheme_id in scheme_ids:
                auth_infos.append(
                    self.get_auth_scheme(
                        scheme_id=scheme_id, scopes=scopes, policy=policy
                    )
                )
        ...
        logging.debug(
            f"Generating auth infos from required schemes {self.required_auth}"
        )
    ...
    def get_min_auth(self) -> list[tuple[AuthType, dict]]:
        ...
        auth_infos = []
        logging.debug(
            f"Generating auth infos from required schemes {self.required_min}"
        )
    ...
    def get_auth_scheme(
        ...
        if scheme_id:
            ...
            if credentials_map and not scheme.supports_credentials(
                credentials_map.keys()
            ):
        ...
        auth_type, auth_info = scheme.get_auth(
            credentials_map=credentials_map, scopes=scopes, access_policy=policy
        )
