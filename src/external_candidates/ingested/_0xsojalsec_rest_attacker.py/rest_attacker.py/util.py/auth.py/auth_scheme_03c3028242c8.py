# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\auth\auth_scheme.py
...
class KeyValueAuthScheme(AuthScheme):
    ...
    def __init__(
        ...
        """
        super().__init__(
            scheme_id, auth_type, credentials=credentials, default_creds=default_creds
        )
    ...
    def get_auth(
        ...
        for p_param in params:
            ...
            try:
                ...
                raise Exception(
                    f"Could not generate auth info for scheme '{self.scheme_id}': "
                    f"Could not find credentials '{cred_src_id}' in dict "
                    f"of credentials for the scheme"
                ) from err

            try:
                if isinstance(cred, OAuth2TokenGenerator):
                    param_val = cred.get_token(scopes, policy=access_policy)[
                        cred_value_id
                    ]
...
class BasicAuthScheme(AuthScheme):
    ...
    def get_auth(
        ...
        header = {self.key_id: payload_str}
