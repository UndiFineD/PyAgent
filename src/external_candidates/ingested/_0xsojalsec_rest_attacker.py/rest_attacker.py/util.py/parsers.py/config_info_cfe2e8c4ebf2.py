# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\parsers\config_info.py
...
def load_config(path: Path) -> EngineConfig:
    ...
    if "credentials" in info.keys():
        ...
        logging.debug(
            "No service credentials file specified: Skipping credential info load."
        )
        users = None
        credentials = {}
        auth_gen = None
...
def create_config_from_openapi(path: Path) -> EngineConfig:
    ...
    return EngineConfig({}, {}, {}, descriptions=descriptions)
