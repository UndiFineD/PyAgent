# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\parsers\config_auth.py
...
from rest_attacker.util.auth.auth_scheme import (
    AuthType,
    BasicAuthScheme,
    KeyValueAuthScheme,
)
...
def load_auth_file(
    path: Path,
) -> tuple[typing.Optional[dict[str, UserInfo]], dict, AuthGenerator]:
    """
    ...
    if "users" in auth_data.keys():
        ...
        for user_internal_id, user_data in users_info.items():
            ...
            if "sessions" in user_data.keys():
                ...
                for session_id, session_data in sessions_info.items():
                    ...
                    if session_data["type"] == "weblogin":
                        ...
                        session = ROWebSession(
                            session_id, login_url, login_data, test_url=test_url
                        )
                        ...
                        session = ROCookieSession(
                            session_id, cookies, test_url=test_url
                        )
                        ...
                        session = ROBrowserSession(
                            session_id, executable, port, test_url=test_url
                        )
                        ...
                        raise ValueError(
                            f"Unrecognized session type: '{session_data['type']}'"
                        )
            ...
            users.update({user_internal_id: UserInfo(**user_info_data)})
    ...
    for cred_id, cred in auth_data["creds"].items():
        if cred["type"] == "oauth2_client":
            ...
            if "revocation_endpoint" in cred.keys():
                client_info_data.update({"revoke_url": cred["revocation_endpoint"]})

            if "scopes" in cred.keys():
                client_info_data.update({"scopes": cred["scopes"]})

            if "grants" in cred.keys():
                client_info_data.update({"grants": cred["grants"]})

            if "description" in cred.keys():
                client_info_data.update({"description": cred["description"]})

            if "flags" in cred.keys():
                client_info_data.update({"flags": cred["flags"]})
            ...
            credentials.update({cred_id: token_gen})
    ...
    for scheme_id, scheme in scheme_info.items():
        ...
        for _, param in params_cfg.items():
            ...
            for param_src in param_srcs:
                scheme_creds.update({param_src: credentials[param_src]})
