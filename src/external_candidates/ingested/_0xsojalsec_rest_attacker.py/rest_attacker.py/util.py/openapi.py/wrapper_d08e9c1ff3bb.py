# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\openapi\wrapper.py
        ...
        for path in self.definition["paths"].values():
            for method in path.values():
                    ...
                    for param in input_parameters:
                        if param["in"] == "body":
                            response_body = {
                                ...
                                "content": {local_consumes: param["schema"]},
                            }
                ...
                for response in method["responses"].values():
                    ...
                    if response_schema:
                        response.update(
                            {"content": {global_produces[0]: response_schema}}
                        )
        ...
        if ref[0] == "#":
            ...
            for part in parts:
                ...
                if isinstance(current_item, dict):
                    ...
                    return Exception(
                        f"Item at {part} in {new_ref} must be a JSON object or array."
                    )
    ...
    def endpoints(self) -> dict[str, dict]:
        ...
        for path_id, path in self.paths.items():
            ...
            for op_id, operation in path.items():
                ...
                endpoints[path_id].update({op_id: operation})
