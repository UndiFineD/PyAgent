# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-REST-Attacker\rest_attacker\util\request\request_info.py
...
class RequestInfo:
    ...
    def __init__(self, url: str, path: str, operation: str, **kwargs) -> None:
        ...
        for idx in range(3, 6):
            ...
            if len(self._url[idx]) > 0:
                logging.warning(
                    "URL for request contains more than scheme, netloc, path."
                )
    ...
    def send(self, auth_data: list[tuple[AuthType, dict]] = None) -> Response:
        ...
        if self.operation.lower() not in self.allowed_ops:
            raise RestrictedOperationError(
                f"HTTP Method {self.operation} is not allowed."
            )
