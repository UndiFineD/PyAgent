# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-whatsapp-osint\whatsappbeacon.py
import argparse
import datetime
import math
import os
...
def remove_idle(driver):
    while True:
        time.sleep(10)
        try:
            smiley = driver.find_element(
                by=By.XPATH, value='//span[@data-testid="smiley"]'
            )
            smiley.click()
...
def study_user(driver, user, language, excel):
    ...
    try:
        ...
        chat_icon = driver.find_element(
            by=By.CSS_SELECTOR, value='span[data-testid="chat"]'
        )
        chat_icon.click()
        ...
        print(
            "{} is not found. Returning...(Maybe your contact is in the archive. Check it)".format(
                user
            )
        )
        return
    ...
    while True:
        try:
            ...
            if previous_state == "OFFLINE":
                input = "[{}][ONLINE] {}".format(
                    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), user
                )
                ...
                Database.insert_connection_data(
                    user, date, hour, minute, second, type_connection
                )
                first_online = time.time()
            ...
            if previous_state == "ONLINE":
                ...
                total_online_time = (
                    time.time() - first_online
                )  # approximately what it takes onPause to send signal
                if (
                    total_online_time < 0
                ):  # This means that the user was typing instead of going offline.
                ...
                Database.insert_disconnection_data(
                    user,
                    date,
                    hour,
                    minute,
    ...
    while True:
        try:
            loading = driver.find_element(
                by=By.XPATH, value='//div[@data-testid="wa-web-loading-screen"]'
            )
            loading.click()
            print("\n Loaded")
            break
