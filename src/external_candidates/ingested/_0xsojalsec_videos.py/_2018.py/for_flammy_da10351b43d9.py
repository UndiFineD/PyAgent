...
class SphereRings(SecondProof):
    ...
    def grow_rings(self):
        ...
        southern_mesh = VGroup(
            *[face.copy() for face in sphere if face.get_center()[2] < 0]
        )
    ...
    def show_radial_line(self):
        ...
        theta = np.arccos(
            np.dot(normalize(R_line.get_vector()), normalize(xy_line.get_vector()))
        )
...
class ShamelessPlug(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "erm", "maybe", "happy", added_anims=[self.teacher.change, "happy"]
        )
        self.wait(3)
