# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\cramer.py
...
class LinearSystem(VGroup):
    ...
    def __init__(self, matrix=None, output_vect=None, **kwargs):
        ...
        if matrix is None:
            dim = self.dimensions
            matrix = np.random.randint(self.min_int, self.max_int, size=(dim, dim))
        else:
            dim = len(matrix)
    ...
    def construct(self):
        ...
        for i in range(3):
            numer_matrix = get_cramer_matrix(
                system.matrix_mobject, system.output_vect_mob, index=i
            )
            # color = colors[i]
            color = YELLOW
            ...
            numer = VGroup(
                get_det_text(numer_matrix, initial_scale_factor=3), numer_matrix
            )
            numer.to_corner(UP)
...
class PrerequisiteKnowledge(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        images = Group(*[ImageMobject("eola%d_thumbnail" % d) for d in [5, 7, 6]])
...
class SetupSimpleSystemOfEquations(LinearTransformationScene):
    ...
    def introduce_system(self):
        ...
        unknown_circles = VGroup(
            *[Circle(color=YELLOW).replace(term).scale(1.5) for term in system.unknowns]
        )
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, unknown_circles, lambda m: (m.restore,), lag_ratio=0.7
            )
        )
        self.play(FadeOut(unknown_circles))
        self.play(LaggedStartMap(ShowCreation, row_rects, run_time=1, lag_ratio=0.8))
    ...
    def show_geometry(self):
        ...
        columns = VGroup(*[VGroup(*matrix_mobject.mob_matrix[:, i]) for i in (0, 1)])
        ...
        for column, column_mob, m in zip(columns, column_mobs, matrices):
            ...
            self.apply_matrix(
                m, added_anims=[ApplyMethod(column_mob.restore, path_arc=90 * DEGREES)]
            )
        self.wait()
...
class ShowZeroDeterminantCase(LinearTransformationScene):
    ...
    def show_det_equation(self):
        ...
        det_equation = OldTex("\\det(", "A", ")", self.det_eq_symbol, "0")
...
class NonZeroDeterminantCase(ShowZeroDeterminantCase, SetupSimpleSystemOfEquations):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(GrowArrow(input_vect_mob), Write(input_vect_label), run_time=1)
        self.wait()
...
class ThinkOfPuzzleAsLinearCombination(SetupSimpleSystemOfEquations):
    ...
    def rearrange_equation_as_linear_combination(self):
        ...
        columns = VGroup(*[VGroup(*matrix.mob_matrix[:, i].flatten()) for i in (0, 1)])
        ...
        corner_rect.target.stretch_to_fit_width(
            new_system.get_width() + MED_LARGE_BUFF, about_edge=LEFT
        )
    ...
    def show_dot_products(self):
        ...
        for equation in moving_equations:
            ...
            transformed_equation = VGroup(
                T1, lp1, xy_vect, rp1, dot, T2, lp2, basis, rp2, equals, coord
            )
            ...
            transformed_input_rects.add(
                SurroundingRectangle(transformed_equation[:4], color=OUTPUT_COLOR)
            )
            transformed_basis_rects.add(
                SurroundingRectangle(
                    transformed_equation[5 : 5 + 4], color=basis.elements.get_color()
                )
            )
            ...
            transformed_equation.parts_to_write = VGroup(T1, lp1, rp1, T2, lp2, rp2)
...
class OrthonormalWords(Scene):
    def construct(self):
        ...
        bottom_words = OldTexText("$T$", "is", "``Orthonormal''")
...
class SolvingASystemWithOrthonormalMatrix(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for i, vect in zip(list(range(2)), [DR, DL]):
            ...
            column_mob.next_to(
                self.get_vector(matrix[:, i]).get_end(), vect, buff=SMALL_BUFF
            )
            column_mobs.add(column_mob)
        ...
        for i, equation in enumerate(equations):
            anims = [
                ...
                LaggedStartMap(MoveToTarget, equation.movers, path_arc=60 * DEGREES),
            ]
            if i == 0:
            ...
            it.chain(
                *[
                    [
                        ...
                        ReplacementTransform(
                            matrix_mobject.brackets.copy(), cm.brackets, **get_kwargs(i)
                        ),
                        ReplacementTransform(
...
class TransitionToParallelogramIdea(TeacherStudentsScene):
    def construct(self):
        teacher_words = OldTexText(
            "Now ask about \\\\ other geometric \\\\ views of", "$x$", "and", "$y$"
        )
        ...
        self.wait()
        self.teacher_says(
            teacher_words, added_anims=[self.change_students(*["pondering"] * 3)]
        )
        self.wait()
...
class TransformingAreasYCoord(LinearTransformationScene):
    ...
    def show_coord_parallelogram(self):
        ...
        for basis, char, direction in zip(basis_vectors, chars, directions):
            label = self.get_vector_label(
                basis, "\\mathbf{\\hat{%s}}" % char, direction=direction
            )
        ...
        area_words = OldTex("\\text{Area}", "=", "1", "\\times", ["x", "y"][index])
        area_words.next_to(
            area_arrow.get_start(), UL, SMALL_BUFF, submobject_to_align=area_words[0]
        )
        ...
        self.play(
            UpdateFromFunc(
                ip,
                lambda m: Transform(
                    m, self.get_input_parallelogram(input_vect_mob)
                ).update(1),
            ),
    ...
    def transform_space(self):
        ...
        # Show many areas
        self.play(LaggedStartMap(DrawBorderThenFill, blobs), Write(area_scale_words))
        ...
        self.play(
            ...
            ReplacementTransform(matrix_mobject.brackets.copy(), column_mob.brackets),
            ReplacementTransform(
    ...
    def solve_for_coord(self):
        ...
        system = VGroup(
            matrix_mobject.target, system_input, system_eq, system_output.target
        )
...
class CramersYEvaluation(Scene):
    def construct(self):
        ...
        group = VGroup(OldTex("="), frac, OldTex("= \\frac{4}{2}"), OldTex("=2"))
        group.arrange(RIGHT)
...
class CramersXEvaluation(Scene):
    def construct(self):
        ...
        group = VGroup(OldTex("="), frac, OldTex("= \\frac{6}{2}"), OldTex("=3"))
        group.arrange(RIGHT)
...
class ThinkItThroughYourself(TeacherStudentsScene):
    def construct(self):
        self.teacher_says("Try thinking \\\\ it through!", target_mode="hooray")
