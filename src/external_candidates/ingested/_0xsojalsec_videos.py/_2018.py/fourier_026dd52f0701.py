    ...
    def fourier_transform(f):
        z = (
            ...
            * scipy.integrate.quad(
                lambda t: func(t) * np.exp(complex(0, -TAU * f * t)), t_min, t_max
            )[0]
        )
...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        fourier_graph = FunctionGraph(
            get_fourier_transform(func, 0, 5), x_min=0, x_max=5
        )
...
class AddingPureFrequencies(PiCreatureScene):
    ...
    def play_A440(self):
        ...
        self.broadcast(
            FadeIn(A_label),
            Succession(
                ApplyMethod, randy.change, "pondering", Animation, randy, Blink, randy
            ),
        )
...
class MachineThatTreatsOneFrequencyDifferently(Scene):
    ...
    def change_graph_frequency(self, frequency, run_time=2):
        ...
        self.play(
            ...
            ChangingDecimal(
                frequency_mob, lambda a: 440 * interpolate(curr_frequency, frequency, a)
            ),
            run_time=run_time,
        )
    ...
    def get_cosine_graph(self, frequency, num_steps=200, color=YELLOW):
        result = FunctionGraph(
            lambda x: 0.5 * np.cos(2 * np.pi * frequency * x), num_steps=num_steps
        )
...
class WrapCosineGraphAroundCircle(FourierMachineScene):
    ...
    def show_initial_signal(self):
        ...
        v_lines = VGroup(
            *[
                DashedLine(ORIGIN, 2 * UP, color=RED).move_to(
    ...
    def wrap_around_circle(self):
        ...
        self.play(
            ReplacementTransform(
                moving_graph, polarized_graph, run_time=3, path_arc=-TAU / 2
            )
        )
    ...
    def show_time_sweeps(self):
        ...
        self.animate_time_sweep(
            freq=freq, t_max=4, run_time=6, added_anims=[FadeIn(v_lines)]
        )
    ...
    def get_winding_frequency_label(self):
        ...
        winding_freq_label = VGroup(
            DecimalNumber(freq, num_decimal_places=2), OldTexText("cycles/second")
        )
    ...
    def get_peak_braces(self, **kwargs):
        peak_points = [
            ...
            for x in np.arange(0, 3.5, 1.0 / self.signal_frequency)
        ]
        return VGroup(
            *[
...
class DrawFrequencyPlot(WrapCosineGraphAroundCircle, PiCreatureScene):
    ...
    def introduce_frequency_plot(self):
        ...
        x_coord_label.next_to(
            frequency_axes.y_axis.get_top(), RIGHT, aligned_edge=UP, buff=LARGE_BUFF
        )
    ...
    def change_frequency(self, new_freq, **kwargs):
        ...
        if hasattr(self, "winding_freq_label"):
            freq_label = [
                sm for sm in self.winding_freq_label if isinstance(sm, DecimalNumber)
            ][0]
            self.add(freq_label)
...
class ShowLowerFrequency(DrawFrequencyPlot):
    ...
    def show_lower_frequency_signal(self):
        ...
        v_lines = VGroup(
            *[
                DashedLine(ORIGIN, 1.5 * UP).move_to(axes.coords_to_point(x, 0), DOWN)
                for x in (1, 2)
            ]
        )
...
class ShowCommutativeDiagram(ShowLinearity):
    ...
    def show_diagram(self):
        ...
        for func, color, label, ta, fa in zip(
            funcs, colors, labels, ta_group, fa_group
        ):
...
class WriteComplexExponentialExpression(DrawFrequencyPlot):
    ...
    def show_plane_as_complex_plane(self):
        ...
        self.play(
            MoveAlongPath(dot, flower_path, run_time=10, rate_func=bezier([0, 0, 1, 1]))
        )
        self.wait()
        self.play(
            ShowCreation(
    ...
    def show_eulers_formula(self):
        ...
        self.set_variables_as_attrs(
            ambient_ghost_dot_movement, ghost_dot, vector, vector_update, exp_expression
        )
...
class ScaleUpCenterOfMass(WriteComplexExponentialExpression):
    ...
    def scale_up_center_of_mass(self):
        ...
        com_vector_update = UpdateFromFunc(
            com_vector, lambda v: v.put_start_and_end_on(origin, com_dot.get_center())
        )
        ...
        axes = self.time_axes
        time_span.replace(
            Line(axes.coords_to_point(0, 0), axes.coords_to_point(3, 1.5)), stretch=True
        )
        ...
        to_fade = VGroup(
            self.integral_brace, self.integral_label, self.frac, self.frac_cross
        )
        self.play(
            to_shift.shift,
            2 * DOWN,
    ...
    def comment_on_current_signal(self):
        ...
        new_com_vector_update = UpdateFromFunc(
            com_vector,
            lambda v: v.put_start_and_end_on(origin, com_dot.get_center()).scale(
                12, about_point=origin
            ),
        )
...
class SummarizeTheFullTransform(DrawFrequencyPlot):
    ...
    def show_winding(self):
        ...
        update_pol_graph = UpdateFromFunc(
            graph.polarized_mobject, lambda m: m.set_stroke(width=2)
        )
...
class BoundsAtInfinity(SummarizeFormula):
    def construct(self):
        ...
        self.play(
            Transform(
                time_interval, wide_interval, run_time=4, rate_func=there_and_back
            )
        )
        self.play(
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                axes.x_axis.numbers, lambda m, a: m.set_fill(opacity=1 - a)
            ),
            *decimal_updates,
            run_time=12,
            rate_func=bezier([0, 0, 1, 1]),
...
class Thumbnail(Scene):
    def construct(self):
        ...
        words = VGroup(
            OldTexText("Signal"), OldTexText("Winding"), OldTexText("Transform")
        )
