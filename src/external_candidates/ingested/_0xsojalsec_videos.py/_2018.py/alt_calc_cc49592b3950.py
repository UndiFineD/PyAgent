...
class NumberlineTransformationScene(ZoomedScene):
    ...
    def setup_titles(self):
        input_title, output_title = self.titles = VGroup(
            *[OldTexText(word) for word in ("Inputs", "Outputs")]
        )
        vects = [UP, DOWN]
    ...
    def get_sample_input_points(self, x_min=None, x_max=None, delta_x=None):
        ...
        return [
            self.get_input_point(x) for x in np.arange(x_min, x_max + delta_x, delta_x)
        ]

    def get_sample_dots(
        self, x_min=None, x_max=None, delta_x=None, dot_radius=None, colors=None
    ):
    ...
    # Mapping animations
    def get_mapping_animation(
        self, func, mobject, how_to_apply_func=apply_function_to_center, **kwargs
    ):
    ...
    def get_line_mapping_animation(self, func, **kwargs):
        ...
        return AnimationGroup(
            ...
            self.get_mapping_animation(
                func, input_line_copy.tick_marks, apply_function_to_submobjects
            ),
        )
    ...
    def apply_function(
        ...
        **kwargs,
        ...
        if hasattr(self, "mini_line"):  # Test for if mini_line is in self?
            anims.append(
                self.get_mapping_animation(
                    func, self.mini_line, how_to_apply_func=apply_function_to_center
                )
            )
        if sample_dots:
        ...
        if target_coordinate_values:
            coordinates = self.get_local_coordinates(
                self.output_line, *target_coordinate_values
            )
...
class WriteOpeningWords(Scene):
    def construct(self):
        ...
        words1, words2 = [
            OldTexText("\\Large", *rs.split(" ")) for rs in (raw_string1, raw_string2)
        ]
...
class StartingCalc101(PiCreatureScene):
    ...
    def show_mystery_topic(self):
        ...
        for image in images:
            ...
            self.play(
                image.shift, DOWN, image.fade, 1, randy.change, "erm", run_time=1.5
            )
            self.remove(image)
        self.wait()
    ...
    def adjust_size(self, group):
        group.set_width(
            min(group.get_width(), self.image_frame_width - 2 * MED_SMALL_BUFF)
        )
        group.set_height(
            min(group.get_height(), self.image_frame_height - 2 * MED_SMALL_BUFF)
        )
        return group
    ...
    def get_physics_image(self):
        ...
        group.continual_animations = [
            t_tracker.add_udpater(
                lambda tracker, dt: tracker.set_value(tracker.get_value() + dt)
            ),
            spring.add_updater(
    ...
    def get_graphical_intuition_image(self):
        ...
        rects = gs.get_riemann_rectangles(
            graph, x_min=2, x_max=8, stroke_width=0.5, dx=0.25
        )
...
class ChangingVectorField(Scene):
    ...
    def func(self, point, time):
        x, y, z = point
        return np.array(
            [np.sin(time + 0.5 * x + y), np.cos(time + 0.2 * x * y + 0.7), 0]
        )
...
class SetTheStage(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_holds_up(
            ordinary, added_anims=[self.change_students(*3 * ["sassy"])]
        )
        self.wait()
        self.play(
            ordinary.shift,
...
class StandardDerivativeVisual(GraphScene):
    ...
    def show_function_graph(self):
        ...
        output_label_update = Mobject.add_updater(
            output_label, lambda m: m.next_to(output_triangle, LEFT, SMALL_BUFF)
        )
    ...
    def show_slope_of_graph(self):
        ...
        slope_decimal.add_upater(
            lambda d: d.next_to(deriv_label, RIGHT, SMALL_BUFF).shift(
                0.2 * SMALL_BUFF * DOWN
            )
        )
        ...
        for x in 9, 2, 4:
            self.play(
                input_tracker.set_value, x, deriv_input_tracker.set_value, x, run_time=3
            )
            self.wait()
    ...
    def encourage_not_to_think_of_slope_as_definition(self):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(
                morty, lambda m, a: m.set_fill(opacity=1 - a), remover=True
            ),
        )
...
class IntroduceTransformationView(NumberlineTransformationScene):
    ...
    def show_animation_preview(self):
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, sample_dots, lambda sd: (sd.restore,), run_time=2
            )
        )
        self.play(
            LaggedStartMap(
...
class TalkThroughXSquaredExample(IntroduceTransformationView):
    ...
    def get_stretch_words(self, factor, color=RED, less_than_one=False):
        ...
        result = OldTexText(
            "Scale \\\\ by", factor_str, tex_to_color_map={factor_str: color}
        )
        result.scale(0.7)
...
class ZoomInOnXSquaredNearOne(TalkThroughXSquaredExample):
    ...
    def construct(self):
        ...
        zcbr_group.remove(
            self.local_coordinates, self.local_target_coordinates, local_sample_dots
        )
        ...
        self.apply_function(
            ...
            added_anims=[
                ...
                MaintainPositionRelativeTo(
                    self.local_coordinates, self.zoomed_camera.frame
                ),
            ],
        )
        self.wait(2)
...
class IntroduceContinuedFractionPuzzle(PiCreatureScene):
    ...
    def set_equal_to_x(self):
        ...
        self.play(
            Write(fixed_point_words), morty.change, "hooray", friend.change, "happy"
        )
        self.wait(3)
...
class GraphOnePlusOneOverX(GraphScene):
    ...
    def draw_graphs(self, animate=True):
        ...
        identity_graph.label = self.get_graph_label(
            identity_graph, "y = x", x_val=3, direction=UL, buff=SMALL_BUFF
        )
    ...
    def show_solutions(self):
        ...
        for num in phi, phi_bro:
            ...
            line_anim = ShowCreationThenDestruction(
                line_copy, lag_ratio=0.5, run_time=2
            )
        ...
        phi_arrow = Arrow(
            phi_decimal[:4].get_top(), phi_line.get_bottom(), **arrow_kwargs
        )
        ...
        phi_bro_arrow = Arrow(
            phi_bro_decimal[:6].get_bottom(), phi_bro_line.get_top(), **arrow_kwargs
        )
        ...
        brother_words.next_to(
            phi_bro_decimal[-2], UP, buff=MED_SMALL_BUFF, aligned_edge=RIGHT
        )
...
class RepeatedApplicationWithNegativeSeed(
    RepeatedApplicationWithPhiBro, MovingCameraScene
):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            FadeOut(question), FadeOut(rect), self.camera.frame.set_height, FRAME_HEIGHT
        )
...
class ShowRepeatedApplication(Scene):
    ...
    def add_func_title(self):
        title = self.title = VGroup(
            OldTex("f(", "x", ")"), OldTex("="), get_nested_one_plus_one_over_x(1)
        )
        title.arrange(RIGHT)
        title.to_corner(UL)
    ...
    def show_repeated_iteration(self):
        ...
        rects = VGroup(
            *[SurroundingRectangle(mob) for mob in (line[0], line[1:-1], line[-1])]
        )
...
class NumericalPlayOnNumberLineFromOne(Scene):
    ...
    def bounce_around(self):
        ...
        self.play(
            FadeIn(start_here), GrowArrow(arrow), GrowFromPoint(dot, arrow.get_start())
        )
    ...
    def draw_spider_web(self):
        ...
        for n in range(self.n_jumps):
            ...
            if n < self.n_times_to_show_identity_property:
                ...
                lines = VGroup(
                    *[Line(dot.get_center(), point) for point in (x_point, y_point)]
                )
...
class AnalyzeFunctionWithTransformations(NumberlineTransformationScene):
    ...
    def repeatedly_apply_function(self):
        ...
        for x in range(self.num_initial_applications):
            self.apply_function(
                self.func, apply_function_to_number_line=False, sample_dots=sample_dots
            )
            self.wait()
...
class StabilityAndInstability(AnalyzeFunctionWithTransformations):
    ...
    def label_stability(self):
        ...
        for point in phi_point, phi_bro_point:
            arrows = VGroup(
                *[a for a in self.all_arrows if get_norm(a.get_start() - point) < 0.75]
            ).copy()
...
class NotBetterThanGraphs(TeacherStudentsScene):
    def construct(self):
        ...
        self.teacher_says(
            ...
            added_anims=[
                self.change_students(*3 * ["thinking"], look_at=self.teacher.eyes)
            ],
        )
        self.wait(3)
