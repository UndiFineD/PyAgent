...
class IntroSceneWrapper(PiCreatureScene):
    ...
    def describe_main_topic(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, xy_equation, run_time=1), morty.change, "pondering"
        )
        self.wait(2)
...
class Introduce1DFunctionCase(Scene):
    ...
    def show_two_graphs(self):
        ...
        f_graph = axes.get_graph(
            lambda x: 2 * x * (x - 0.75) * (x - 1.5) + 1, color=BLUE
        )
        ...
        equation_arrow = Arrow(
            equation.get_bottom(), cross_point, buff=SMALL_BUFF, color=WHITE
        )
        ...
        self.set_variables_as_attrs(
            f_graph, f_label, g_graph, g_label, equation=equation.target
        )
    ...
    def show_example_binary_search(self):
        self.binary_search(
            self.squared_graph, self.two_graph, x0=1, x1=2, n_iterations=8
        )
    ...
    def binary_search(
        ...
        for x in range(n_iterations - 1):
            ...
            if zoom:
                ...
                if factor > 1:
                    self.play(
                        everything.scale, factor, {"about_point": rect.get_center()}
                    )
            else:
                self.wait()
...
class PiCreaturesAreIntrigued(AltTeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "You can extend \\\\ this to 2d", bubble_config={"width": 4, "height": 3}
        )
...
class RewriteEquationWithTeacher(AltTeacherStudentsScene):
    def construct(self):
        ...
        z_def = OldTexText(
            "(", "$z$", " is complex, ", "$a + bi$", ")", arg_separator=""
        )
        ...
        for x in np.arange(x_min, x_max + step, step):
            for y in np.arange(y_max, y_min - step, -step):
                ...
                dot.target_color = rgba_to_color(
                    point_to_rgba(tuple(self.output_scalar * np.array(out_coords)))
                )
                dots.add(dot)
        return dots
...
class IntroduceInputOutputScene(InputOutputScene):
    ...
    def map_single_point_to_point(self):
        ...
        self.play(
            GrowFromCenter(in_dot), GrowArrow(input_label_arrow), FadeIn(input_label)
        )
        self.wait()
        self.play(
        ...
        for vect in UP, RIGHT:
            self.play(
                in_dot.shift, 0.25 * vect, rate_func=lambda t: wiggle(t, 8), run_time=2
            )
        for vect in compass_directions(4, UP + RIGHT):
            self.play(
                Rotating(
                    in_dot, about_point=in_dot.get_corner(vect), radians=TAU, run_time=1
                )
            )
        self.wait()
...
class IntroduceVectorField(IntroduceInputOutputScene):
    ...
    def construct(self):
        ...
        self.remove()
        self.play(
            *list(map(FadeOut, [out_dot, out_vector, in_vectors, in_dot, in_vector]))
        )
        self.wait()
        target_length = 0.4
    ...
    def get_graph_part(self, *interim_values):
        ...
        result.set_points_smoothly(
            [
                ...
                for x, fx in zip(np.linspace(self.x0, self.x1, len(values)), values)
            ]
        )
        return result
...
class HypothesisAboutFullyColoredBoundary(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, hypothesis[1]), ShowCreation(square, run_time=8)
        )
...
class TransitionFromPathsToBoundaries(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        # Add regions
        self.play(
            FadeOut(paths), FadeOut(joint_path.brace), dot.move_to, path1.get_start()
        )
        for square in squares:
    ...
    def get_dot_and_add_continual_animations(self):
        ...
        update_dot_color = Mobject.add_updater(
            dot, lambda d: d.set_fill(get_output_color(), self.dot_fill_opacity)
        )
    ...
    def show_winding(self):
        ...
        def generate_label_update(label, point_func, origin):
            return UpdateFromFunc(
                label,
                lambda m: m.move_to(
                    (1 + buff) * point_func() - buff * origin, self.label_move_to_corner
                ),
            )
...
class PolynomialTerms(MonomialTerm):
    ...
    def sweep_through_loop_interior(self):
        ...
        update_out_line = UpdateFromFunc(
            out_line,
            lambda m: m.set_points(line.get_points()).apply_function(
                self.point_function
            ),
        )
    ...
    def cinch_loop(self):
        ...
        update_out_loop = UpdateFromFunc(
            out_loop,
            lambda m: m.set_points(loop.get_points()).apply_function(
                self.point_function
            ),
        )
...
class PiCreatureInAwe(Scene):
    def construct(self):
        ...
        self.play(
            randy.look, RIGHT, run_time=4, rate_func=there_and_back, path_arc=-TAU / 4
        )
        self.wait()
...
class WindingNumbersInInputOutputContext(PathContainingZero):
    ...
    def construct(self):
        ...
        update_out_loop = Mobject.add_updater(
            out_loop,
            lambda m: m.set_points(in_loop.get_points()).apply_function(
                self.point_function
            ),
        )
...
class EndingCredits(Scene):
    def construct(self):
        text = OldTexText(
            "Written and animated by: \\\\", "Sridhar Ramesh \\\\", "Grant Sanderson"
        )
...
class TickingClock(Scene):
    ...
    def construct(self):
        ...
        run_time = self.run_time
        self.play(
            ClockPassesTime(clock, run_time=run_time, hours_passed=0.1 * run_time)
        )
...
class MentionFree(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            items[:2].shift, UP, FadeInFromDown(items[2]), morty.change, "surprised"
        )
        self.wait(4)
