...
class SourcesOfOriginality(TeacherStudentsScene):
    ...
    def break_down_value_of_math_presentations(self):
        ...
        big_rect = self.big_rect = Rectangle(
            width=title.get_width() + 2 * MED_LARGE_BUFF, height=3.5, color=WHITE
        )
        ...
        based_on_wastlund = OldTexText(
            "Previous video based on\\\\", 'a paper by Johan W\\"{a}stlund'
        )
        ...
        self.play(
            FadeIn(communication_parts), FadeIn(lighthouse), SwitchOn(ambient_light)
        )
...
class ShowProduct(Scene):
    ...
    def show_larger_terms(self):
        ...
        lines = VGroup(
            *[Line(d1.get_center(), d2.get_center()) for d1, d2 in zip(dots, dots[1:])]
        )
        ...
        for part, line, dot, prod, new_brace in tuples:
            self.play(
                ...
                ChangeDecimalToValue(
                    decimal, prod, position_update_func=lambda m: m.next_to(brace, DOWN)
                ),
                ShowCreation(line),
    ...
    def show_smaller_terms(self):
        ...
        self.play(
            ...
            -vect,
            UpdateFromAlphaFunc(
                larger_parts_mover, lambda m, a: m.set_fill(opacity=1 - a), remover=True
            ),
...
class DistanceProductScene(MovingCameraScene):
    ...
    def get_observer_dot(self):
        self.observer_dot = Dot(
            self.get_observer_point(), color=self.observer_config["color"]
        )
    ...
    def get_lights(self):
        ...
        for point in self.get_lh_points():
            light = AmbientLight(
                source_point=VectorizedPoint(point), **self.ambient_light_config
            )
    ...
    def get_numeric_distance_labels(
    ...
    def get_distance_product(self, fraction=None):
        ...
        distances = [
            get_norm(point - observer_point) / radius for point in self.get_lh_points()
        ]
...
class IntroduceDistanceProduct(DistanceProductScene):
    ...
    def show_sum_of_inverse_squares(self):
        ...
        non_d_terms = VGroup(
            *[
                m
                for m in sum_of_inverse_squares
        ...
        self.play(
            LaggedStartMap(
                ...
                light_rings,
                lambda mob: (
                    lambda m: m.shift(MED_SMALL_BUFF * UP).set_fill(
...
class Lemma1(DistanceProductScene):
    ...
    def add_circle_group(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, VGroup(*circle_group.family_members_with_points()))
        )
    ...
    def wiggle_observer(self):
        ...
        self.play(
            ...
            UpdateFromFunc(
                self.numeric_distance_labels, update_numeric_distance_labels
            ),
            UpdateFromFunc(
                self.distance_product_column, update_distance_product_column
            ),
            run_time=5,
        )
...
class FromGeometryToAlgebra(DistanceProductScene):
    ...
    def transition_to_complex_plane(self):
        ...
        self.play(
            ChangeDecimalToValue(
                complex_decimal, values[1], tracked_mobject=complex_decimal_rect
            ),
    ...
    def discuss_powers(self):
        ...
        for i, arrow in enumerate(outer_arrows[:-1]):
            label = OldTex("x^%d" % i)
            label.next_to(
                arrow.get_start(), -arrow.get_vector(), submobject_to_align=label[0]
            )
            label.add_background_rectangle()
...
class PlugObserverIntoPolynomial(DistanceProductScene):
    ...
    def raise_observer_to_the_N(self):
        ...
        question = OldTexText(
            "What fraction \\\\", "between $L_0$ and $L_1$", "?", arg_separator=""
        )
...
class DefineChordF(Scene):
    def construct(self):
        ...
        self.play(
            MoveToTarget(chord_f), FadeOut(VGroup(full_chord_f[0], full_chord_f[-1]))
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(formula[0].copy(), formula[1], path_arc=45 * DEGREES)
        )
        self.wait()
...
class DistanceProductIsChordF(PlugObserverIntoPolynomial):
    ...
    def show_all_animations(self):
        ...
        O_dot.move_to(
            self.get_circle_point_at_proportion(fraction / self.num_lighthouses)
        )
        ...
        fraction_words = VGroup(
            OldTexText("$f$", "of the way"), OldTexText("between lighthouses")
        )
...
class ProveLemma2(PlugObserverIntoPolynomial):
    ...
    def rearrange_polynomial(self):
        ...
        self.play(
            LaggedStartMap(ApplyMethod, roots_of_unity_circle, lambda m: (m.restore,))
        )
        self.wait()
    ...
    def plug_in_one(self):
        ...
        product = DecimalNumber(
            self.num_lighthouses, num_decimal_places=3, show_ellipsis=True
        )
...
class KeeperAndSailor(DistanceProductScene, PiCreatureScene):
    ...
    def write_distance_product_fraction(self):
        ...
        keeper_lines = self.get_distance_lines(
            self.get_keeper_point(), line_class=DashedLine
        )
        sailor_lines = self.get_distance_lines(
            self.get_sailor_point(), line_class=DashedLine
        )
        ...
        self.wait()
        self.play(
            ShowCreation(keeper_dp_decimal_rect), ShowCreation(keeper_top_zero_rect)
        )
        self.wait(2)
    ...
    def break_down_distance_product_by_parts(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeIn, sailor_lines, rate_func=there_and_back, remover=True
            ),
            LaggedStartMap(
                FadeIn, keeper_lines, rate_func=there_and_back, remover=True
            ),
        )
    ...
    def show_limit_for_each_fraction(self):
        ...
        for group, color in (braces[:4], self.keeper_color), (
            braces[1:4],
            self.sailor_color,
        ):
            self.play(
        ...
        def show_braces(n):
            ...
            for group in cw_group, VGroup(braces[0], *cw_group):
                self.play(
                    group.scale, 0.95, {"about_point": center}, rate_func=there_and_back
                )
                self.wait(0.5)
    ...
    def show_limit_of_lhs(self):
        ...
        pi_halves_arrow = Arrow(
            result_rect.get_bottom(), pi_halves.get_top(), color=WHITE, buff=SMALL_BUFF
        )
        ...
        final_rect = SurroundingRectangle(
            VGroup(wallis_product, pi_halves), buff=MED_SMALL_BUFF
        )
    ...
    def let_N_approach_infinity(
        self, braces=None, factor=4, run_time=5, zoom_in_after=False
    ):
        lights = self.lights
        labels = self.lighthouse_labels
        ...
        self.play(
            fraction_tracker.set_value, target_fraction, *all_updates, run_time=run_time
        )
        if zoom_in_after:
            self.play(
                circle.scale,
    ...
    def get_all_circle_braces(self):
        ...
        return VGroup(
            *[self.get_circle_brace(f1, f2) for f1, f2 in zip(fractions, fractions[1:])]
        )
...
class HowThisArgumentRequiresCommunitingLimits(PiCreatureScene):
    def construct(self):
        ...
        fraction_limit_arrows = VGroup(
            *[Vector(0.5 * DOWN).next_to(fraction, DOWN) for fraction in fractions]
        )
...
class NonCommunitingLimitsExample(Scene):
    ...
    def construct(self):
        ...
        columns = VGroup(
            *[VGroup(*[row[0][i] for row in rows]) for i in range(len(rows[0][0]))]
        )
        column_rects = VGroup(*list(map(SurroundingRectangle, columns)))
        column_rects.set_color(BLUE)

        row_arrows = VGroup(
            *[OldTex("\\rightarrow").next_to(row, RIGHT) for row in rows[:-1]]
        )
...
class KeeperAndSailorForSineProduct(KeeperAndSailor):
    ...
    def show_new_distance_product(self):
        ...
        self.wait()
        self.play(
            LaggedStartMap(FadeOut, VGroup(sailor_lines, sailor_dp_rect, two_group))
        )
    ...
    def show_new_limit_of_ratio(self):
        ...
        frac1 = OldTex(
            "{N", "(", "f", " \\cdot 2\\pi / N)", "\\over", "\\text{Chord}(", "f", ")}"
        )
        frac2 = OldTex(
            "{f", "\\cdot", "2", "\\pi", "\\over", "\\text{Chord}(", "f", ")}"
        )
