...
inverse_quadratic = lambda maxint, scale, cutoff: inverse_power_law(
...
class LightIndicator(Mobject):
    ...
    def init_points(self):
        ...
        self.foreground.set_stroke(
            color=INDICATOR_STROKE_COLOR, width=INDICATOR_STROKE_WIDTH
        )
    ...
    def measured_intensity(self):
        distance = get_norm(
            self.get_measurement_point() - self.light_source.get_source_point()
        )
        intensity = (
            self.light_source.opacity_function(distance)
...
class IntroScene(PiCreatureScene):
    ...
    def build_up_euler_sum(self):
        ...
        partial_sum_decimal = self.partial_sum_decimal = DecimalNumber(
            series_terms[1], num_decimal_places=2
        )
    ...
    def refocus_on_euler_sum(self):
        ...
        self.play(
            ...
            ApplyMethod(
                self.euler_sum.shift, ORIGIN + 2 * UP - self.euler_sum.get_center()
            ),
        )
        ...
        self.pi_creature_thinks(
            thought, target_mode="confused", bubble_config={"height": 2, "width": 3}
        )
...
class MathematicalWebOfConnections(PiCreatureScene):
    ...
    def complain_that_pi_is_not_about_circles(self):
        ...
        words = self.words = OldTexText(
            "I am not", "fundamentally \\\\", "about circles"
        )
    ...
    def draw_circle(self):
        ...
        self.play(
            ...
            ChangeDecimalToValue(
                decimal, np.pi, position_update_func=decimal_position_update_func
            ),
    ...
    def show_web_of_connections(self):
        ...
        path_edges = VGroup(
            *[
                Line(d1.get_center(), d2.get_center(), color=RED)
...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        ...
        lighthouses = self.lighthouses = VGroup(
            *[ls.lighthouse for ls in light_sources[: NUM_VISIBLE_CONES + 1]]
        )
        ...
        self.play(
            morty.change, "pondering", LaggedStartMap(FadeIn, lighthouses, run_time=1)
        )
        self.play(
            LaggedStartMap(
                SwitchOn,
    ...
    def describe_brightness_of_each(self):
        ...
        triangle_anim = GrowFromPoint(
            triangle, triangle.get_right(), point_color=YELLOW
        )
...
class IntroduceScreen(Scene):
    ...
    def setup_angle(self):
        ...
        self.angle_indicator = DecimalNumber(
            arc_angle / DEGREES, num_decimal_places=0, unit="^\\circ"
        )
        self.angle_indicator.next_to(self.angle_arc, RIGHT)

        angle_update_func = (
            lambda x: self.light_source.spotlight.opening_angle() / DEGREES
        )
    ...
    def shoot_rays(self, show_creation_kwargs=None):
        ...
        rays = VGroup(
            *[
                ...
                for angle in np.arange(
                    self.min_ray_angle, self.max_ray_angle, step_size
                )
            ]
        )
        ...
        max_angle = np.max(
            [angle_of_vector(point - source_point) for point in screen.get_points()]
        )
        min_angle = np.min(
            [angle_of_vector(point - source_point) for point in screen.get_points()]
        )
        ...
        self.play(
            *[ShowCreation(ray, run_time=3, **show_creation_kwargs) for ray in rays]
        )
...
class EarthScene(IntroduceScreen):
    ...
    def construct(self):
        ...
        pole_arrow.shift(
            screen.target.get_center() + SMALL_BUFF * LEFT - pole_arrow.get_end()
        )
...
class ShowLightInThreeDimensions(IntroduceScreen, ThreeDScene):
    ...
    def construct(self):
        ...
        cone = ThreeDSpotlight(
            screen, light_source.ambient_light, light_source.get_source_point
        )
...
class InverseSquareLaw(ThreeDScene):
    ...
    def move_screen_farther_away(self):
        ...
        screen.shift(
            source_point + unit_distance * RIGHT - screen.get_reference_point()
        )
    ...
    def morph_into_3d(self):
        ...
        cone = ThreeDSpotlight(
            new_screen, ambient_light, source_point_func=lambda: source_point
        )
        cone_update_anim = ContinualThreeDLightConeUpdate(cone)
        ...
        self.move_camera(
            ...
            added_anims=[
                ...
                # ),
                ApplyFunction(
                    lambda m: m.fade(1).shift(1.5 * DOWN), light_indicator, remover=True
                ),
                FadeOut(morty),
            ],
            run_time=2,
        ...
        self.play(
            UpdateFromAlphaFunc(
                ...
                lambda ls, a: ls.move_to(
                    observer_point
                    + rotate_vector(unit_distance * RIGHT, (1 + 1.0 / 8) * a * TAU)
                ),
                run_time=6,
...
class TwoLightSourcesScene(ManipulateLightsourceSetups):
    ...
    def construct(self):
        ...
        morty.target.next_to(
            origin_point, LEFT, buff=0, submobject_to_align=morty.target.eyes[1]
        )
        ...
        def update_indicator(indicator):
            ...
            for ls in lsA, lsB, lsC:
                if ls in self.mobjects:
                    ...
                    d_indensity = fdiv(
                        3.0 / (distance**2), indicator.opacity_for_unit_intensity
                    )
        ...
        intensity = (
            lsA.ambient_light.opacity_function(distance1)
            / indicator.opacity_for_unit_intensity
        )
        distance2 = get_norm(origin_point - lsB.get_source_point())
        intensity += (
            lsB.ambient_light.opacity_function(distance2)
            / indicator.opacity_for_unit_intensity
        )

        # IPT Theorem
        theorem = OldTex(
            "{1 \over ", "a^2}", "+", "{1 \over", "b^2}", "=", "{1 \over", "h^2}"
        )
...
class MathologerVideoWrapper(Scene):
    def construct(self):
        ...
        self.play(
            logo.shift, FRAME_WIDTH * LEFT, LaggedStartMap(FadeIn, title), run_time=2
        )
        self.play(ShowCreation(screen))
        self.wait(5)
...
class SimpleIPTProof(Scene):
    def construct(self):
        ...
        for arrow, line in zip(argument_lines[1::2], argument_lines[2::2]):
            line.save_state()
            line.shift(
                prev_line.alignment_mob.get_center() - line.alignment_mob.get_center()
            )
            line.fade(1)
...
class IPTScene(TwoLightSourcesScene, ZoomedScene):
    ...
    def construct(self):
        ...
        def show_key_point(spotlight, new_point):
            ...
            self.wait()
            self.play(
                lsC.move_source_to, H, screen.restore, update_spotlight_anim, run_time=2
            )
            self.wait()
        ...
        self.play(
            *list(map(FadeOut, [spotlight_a, lsA.ambient_light, lsB.ambient_light]))
        )
    ...
    def construct(self):
        ...
        for angle in 0.2 * TAU, -0.4 * TAU, 0.3 * TAU:
            ...
            perp_mark.target.shift(
                point.target.get_center() - perp_mark.target[1].get_center()
            )
    ...
    def construct(self):
        ...
        for angle in 0.25 * TAU, -0.4 * TAU, 0.3 * TAU, -0.35 * TAU:
            ...
            half_angle_mark.target.shift(
                point.target.get_center() - half_angle_mark.target[1].get_center()
            )
...
class PondScene(ThreeDScene):
    def construct(self):
        ...
        indicator = LightIndicator(
            precision=2, radius=INDICATOR_RADIUS, show_reading=False, color=LIGHT_COLOR
        )
        ...
        ls0 = LightSource(
            opacity_function=original_op_func, radius=15.0, num_levels=150
        )
        ...
        self.play(morty.restore)
        self.play(
            GrowFromCenter(obs_dot), GrowFromCenter(ls0_dot), FadeIn(ls0.lighthouse)
        )
        ...
        self.play(
            ...
            ReplacementTransform(
                new_diameter_text_copy, new_reading, parth_arc=30 * DEGREES
            ),
        )
        ...
        def indicator_wiggle():
            ...
            self.play(
                ...
                ScaleInPlace(
                    indicator_reading, INDICATOR_WIGGLE_FACTOR, rate_func=wiggle
                ),
            )
        ...
        def construction_step(
            n, show_steps=True, run_time=1, simultaneous_splitting=False
        ):
            ...
            if show_steps == True:
                self.play(
                    FadeIn(new_outer_lake, run_time=run_time), FadeIn(self.ls0_dot)
                )
            else:
                self.play(
            ...
            if simultaneous_splitting == False:
                ...
                for i in range(2**n):
                    ...
                    split_light_source(
                        i, step=n, show_steps=show_steps, run_time=run_time
                    )
        ...
        self.play(
            FadeOut(self.altitudes), FadeOut(self.hypotenuses), FadeOut(self.legs)
        )
        ...
        for i, submob in zip(list(range(nb_symbols)), two_sided_sum.submobjects):
            submob.next_to(
                self.number_line.number_to_point(i - 13), DOWN, buff=2 * scale
            )
        ...
        randy = (
            Randolph(color=MAROON_E)
            .scale(scale)
            .move_to(2 * scale * DOWN + 5 * scale * LEFT)
        )
...
class FinalSumManipulationScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        result1 = OldTex(
            "{\pi^2\over 8} =", fill_color=LIGHT_COLOR, stroke_color=LIGHT_COLOR
        )
        ...
        for i in even_range:
            ...
            term.next_to(
                self.number_line1.number_to_point(i), DOWN, buff=sum_vertical_spacing
            )
            even_terms.add(term)
...
class ArcHighlightOverlaySceneCircumferenceEight(Scene):
    ...
    def construct(self):
        ...
        def flash_arcs(n):
            ...
            arcs.append(
                LabeledArc(
                    angle / 2, start_angle=-TAU / 4, radius=LAKE0_RADIUS, length=1
                )
            )
...
class ArcHighlightOverlaySceneCircumferenceSixteen(
    ArcHighlightOverlaySceneCircumferenceEight
):
    CONFIG = {
        "n": 3,
    }
...
class InfiniteCircleScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        right_arc = DashedLine(
            TAU / 4 * UP, ORIGIN, stroke_color=YELLOW, stroke_width=8
        ).apply_complex_function(np.exp)
        ...
        right_line = DashedLine(
            B + 0.2 * DOWN, C + 0.2 * UP, stroke_color=YELLOW, stroke_width=8
        )
...
class Thumbnail(Scene):
    ...
    def construct(self):
        ...
        for i in range(16):
            ...
            pos = lake_center + lake_radius * np.array(
