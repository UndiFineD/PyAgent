# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\basel\basel.py
#!/usr/bin/env python
...
class IntroScene(PiCreatureScene):
    ...
    def build_up_euler_sum(self):
        ...
        self.partial_sum_decimal = DecimalNumber(
            partial_results_values[1], num_decimal_places=2
        )
    ...
    def build_up_sum_on_number_line(self):
        ...
        for i in range(1, 7):
            if i == 1:
                ...
                term = OldTex(
                    "{1\over " + str(i**2) + "}", fill_color=slab_colors[i - 1]
                )
            term.scale(0.4)
        ...
        for i in range(5):
            self.play(
                GrowFromPoint(
                    self.rects[i], self.euler_sum[2 * i].get_center(), run_time=1
                )
            )
        ...
        for i in range(5, max_n1):
            ...
            if i == 5:
                ...
                self.play(
                    ...
                    GrowFromPoint(
                        self.rects[i], self.euler_sum[10].get_center(), run_time=0.5
                    ),
                )
            else:
                self.play(
        ...
        for i in range(max_n1, max_n2):
            self.play(
                GrowFromPoint(
                    self.rects[i], self.euler_sum[10].get_center(), run_time=0.01
                )
            )
    ...
    def show_pi_answer(self):
        ...
        self.pi_answer.next_to(
            self.euler_sum[-1], RIGHT, buff=1, submobject_to_align=self.pi_answer[-2]
        )
    ...
    def refocus_on_euler_sum(self):
        ...
        self.play(
            WiggleOutThenIn(pi_squared, scale_value=4, angle=0.003 * TAU, run_time=2)
        )
        ...
        self.pi_creature_thinks(
            thought, target_mode="confused", bubble_config={"height": 2.5, "width": 5}
        )
        self.look_at(pi_squared)
...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        ...
        for i, light_source in zip(
            list(range(NUM_VISIBLE_CONES)), light_sources[:NUM_VISIBLE_CONES]
        ):
        ...
        for i, light_source in zip(
            ...
            self.play(
                ...
                ApplyMethod(
                    light_indicator.foreground.set_fill, None, opacities[i - 1]
                ),
            )
...
class EarthScene(Scene):
    ...
    def construct(self):
        ...
        self.screen.set_points_as_corners(
            [[0, -self.screen_height / 2, 0], [0, self.screen_height / 2, 0]]
        )
        ...
        self.play(
            Transform(self.screen, screen_on_earth), Transform(morty, polar_morty)
        )
...
class ScreenShapingScene(ThreeDScene):
    ...
    def create_brightness_rect(self):
        ...
        brightness_rect0 = Rectangle(width=0, height=self.screen_height).move_to(
    ...
    def left_shift_screen_while_showing_light_indicator(self):
        ...
        self.left_shift = (
            self.screen.get_center()[0] - self.spotlight.get_source_point()[0]
        ) / 2
...
class BackToEulerSumScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        bubble = ThoughtBubble(
            direction=RIGHT, width=4, height=3, file_name="Bubbles_thought.svg"
        )
        bubble.next_to(randy, LEFT + UP)
        ...
        for i in range(2, NUM_VISIBLE_CONES + 1):
            ...
            self.play(
                SwitchOn(new_light.ambient_light),
            )
...
class TwoLightSourcesScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        intensity = (
            ls1.ambient_light.opacity_function(distance1)
            / indicator.opacity_for_unit_intensity
        )
        distance2 = get_norm(C - ls2.get_source_point())
        intensity += (
            ls2.ambient_light.opacity_function(distance2)
            / indicator.opacity_for_unit_intensity
        )
        ...
        theorem_box = Rectangle(
            width=buffer * theorem.get_width(), height=buffer * theorem.get_height()
        )
...
class IPTScene1(PiCreatureScene):
    ...
    def construct(self):
        ...
        screen1p.move_to(
            C
            + (screen_width_b - screen_width_bp / 2) * RIGHT
        ...
        screen2p.move_to(
            C
            + (screen_width_a - screen_width_ap / 2) * UP
...
class IPTScene2(Scene):
    ...
    def construct(self):
        ...
        buffer = 1.5
        box = Rectangle(
            width=formula.get_width() * buffer, height=formula.get_height() * buffer
        )
        box.move_to(formula)
...
class InscribedAngleScene(ThreeDScene):
    ...
    def construct(self):
        ...
        inner_label = (
            OldTex("\\theta", fill_color=ANGLE_COLOR1)
            .scale(3)
            .next_to(inner_angle_arc, LEFT, buff=-0.1)
        )
        ...
        outer_label = (
            OldTex("{\\theta \over 2}", color=ANGLE_COLOR2)
            .scale(2.5)
            .move_to(outer_angle_arc)
        )
...
class PondScene(ThreeDScene):
    ...
    def construct(self):
        ...
        for n in range(3, max_it + 1):
            ...
            for i in range(2**n):
                ...
                if onscreen_old or onscreen_1:
                    ...
                    print(
                        "------------------ moving from", ls_old_loc[:2], "to", pos1[:2]
                    )
                    shift_list.append(
                        ApplyMethod(ls1.move_source_to, pos1, run_time=STEP_RUN_TIME)
                    )
                else:
                ...
                if onscreen_old or onscreen_2:
                    ...
                    print(
                        "------------------ moving from", ls_old_loc[:2], "to", pos2[:2]
                    )
                    shift_list.append(
                        ApplyMethod(ls2.move_source_to, pos2, run_time=STEP_RUN_TIME)
                    )
                else:
        ...
        open_sea = (
            ...
            .flip()
            .next_to(self.obs_dot.get_center(), UP, buff=0)
        )
