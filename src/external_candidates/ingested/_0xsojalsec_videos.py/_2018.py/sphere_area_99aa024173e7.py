...
class AskAboutShadowRelation(SpecialThreeDScene):
    ...
    def show_surface_area(self):
        ...
        sa_equation = OldTex(
            "\\text{Surface area} = 4\\pi R^2", tex_to_color_map={"R": BLUE}
        )
        ...
        for thumbnail in thumbnails:
            ...
            thumbnail.add_to_back(
                SurroundingRectangle(
                    thumbnail, buff=0, stroke_color=WHITE, stroke_width=5
                )
            )
...
class UnfoldCircles(Scene):
    ...
    def add_four_circles(self):
        ...
        R_labels = VGroup(
            *[OldTex("R").next_to(line, LEFT, SMALL_BUFF) for line in radii_lines]
        )
...
class ShowProjection(SphereCylinderScene):
    ...
    def show_length_stretch_of_rect(self, example_group):
        ...
        line1, line2 = lines = VGroup(
            *[Line(m.get_anchors()[1], m.get_anchors()[2]) for m in rects]
        )
    ...
    def get_example_group(self, vect):
        pieces = self.pieces
        rect = pieces[
            np.argmax([np.dot(r.saved_state.get_center(), vect) for r in pieces])
        ].saved_state.copy()
...
class JustifyLengthStretch(ShowProjection):
    ...
    def show_similar_triangles(self):
        ...
        self.play(
            ReplacementTransform(point, triangle), Animation(self.camera.phi_tracker)
        )
...
class JustifyHeightSquish(MovingCameraScene):
        ...
        for new_phi in alt_phis:
            ...
            self.play(
                ...
                UpdateFromFunc(
                    tri_group,
                    lambda tg: tg.become(
                        self.get_double_triangle_group(phi_tracker.get_value(), d_phi)
                    ),
                ),
                Rotate(
                    rad_tangent,
    ...
    def label_angles(self):
        ...
        self.wait()
        self.play(
            ShowPassingFlash(self.tangent.copy().scale(0.1).set_stroke(PINK, 0.5))
        )
        self.wait()
...
class WhyAreWeDoingThis(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "maybe", "pondering", "hesitant", added_anims=[self.teacher.change, "tease"]
        )
        self.wait(3)
        self.play(
...
class SameEffectAsRotating(Scene):
    ...
    def construct(self):
        ...
        self.play(
            TransformFromCopy(rect1, rotated_rect1, path_arc=-90 * DEGREES, run_time=2)
        )
...
class RotateAllPiecesWithExpansion(ShowProjection):
    ...
    def rotate_all_pieces(self):
        ...
        self.wait()
        self.play(
            *[Rotate(piece, 90 * DEGREES, axis=piece.get_center()) for piece in sphere]
        )
        self.wait()
...
class UnwrappedCircleLogic(UnfoldCircles):
    ...
    def construct(self):
        ...
        tri_R_line = always_redraw(
            lambda: Line(ORIGIN, radius * DOWN).shift(center_tracker.get_center())
        )
...
class NobodyLikesHomework(TeacherStudentsScene):
    def construct(self):
        self.play_student_changes(
            "angry", "pleading", "angry", added_anims=[self.teacher.change, "guilty"]
        )
        self.wait()
...
class SecondProof(SpecialThreeDScene):
    ...
    def cut_cross_section(self):
        ...
        self.set_variables_as_attrs(
            back_half, front_half, front_half_ghost, slice_circle=circle
        )
    ...
    def show_theta(self):
        ...
        lit_ring = always_redraw(
            lambda: self.get_ring_from_theta(self.rings, get_theta())
            .copy()
            .set_color(YELLOW)
        )
    ...
    def ask_about_ring_circumference(self):
        ...
        self.move_camera(
            phi=0 * DEGREES, theta=-90 * DEGREES, added_anims=[FadeOut(to_fade)]
        )
        self.play(ShowCreation(circle))
        self.wait()
        self.move_camera(
...
class YouCouldIntegrate(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.teacher_says(
            "We'll be a bit \\\\ more Archimedean", target_mode="speaking"
        )
...
class Question3(Scene):
    def construct(self):
        ...
        equation = OldTexText(
            "(Shadow area)", "=", "$\\frac{1}{2}$", "(Area of one of the rings)"
        )
