# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\pi_day.py
# -*- coding: utf-8 -*-
...
class CircleConstants(Scene):
    ...
    def setup_circle(self):
        ...
        self.play(
            ShowCreation(self.circle_arc), Write(self.pi_equals), Write(self.decimal)
        )
...
class AnalysisQuote(Scene):
    ...
    def construct(self):
        ...
        text = OldTexText(
            "``We therefore set the radius of \\\\"
...
class BernoulliQuote(Scene):
    ...
    def construct(self):
        ...
        text = OldTexText(
            "``Your most profound investigation of the series \\\\"
...
class EulerSignature(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(sig, run_time=5))
...
def get_circle_drawing_terms(radius=1, positioning_func=lambda m: m.center()):
    ...
    one_update = UpdateFromFunc(
        one,
        lambda m: m.move_to(
            radius.get_center() + 0.25 * rotate_vector(radius.get_vector(), TAU / 4)
        ),
    )
    ...
    changing_decimal = ChangingDecimal(
        decimal, lambda a: a * TAU, position_update_func=reposition_decimal, **kwargs
    )
...
class PiTauDebate(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            RemovePiCreatureBubble(pi), RemovePiCreatureBubble(tau), *generate_anims1()
        )
        self.play(tau.change, "hooray", pi.change, "sassy", *generate_anims2())
        self.wait()
        ...
        self.play(ReplacementTransform(circle, circum_line, run_time=2))
...
class HistoryOfOurPeople(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            *[ApplyMethod(pi.change, "happy", self.screen) for pi in self.pi_creatures]
        )
        self.wait(4)
...
class TauFalls(Scene):
    def construct(self):
        ...
        self.play(
            Rotate(tau, angle, rate_func=rush_into, path_arc=angle, about_point=bottom)
        )
...
class EulerWrites628(Scene):
    ...
    def construct(self):
        ...
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(
            radius=1, positioning_func=lambda circ: circ.next_to(words, DOWN, buff=1.25)
        )
        circle, radius, one, decimal = terms
        ...
        self.play(Write(words), GrowFromPoint(rect, rect.point_from_proportion(0.9)))
        self.wait()
        self.play(*generate_anims1())
        self.play(*generate_anims2())
        self.play(terms.shift, UP)
        self.play(pi.scale, 2, pi.shift, DOWN, pi.set_color, GREEN)
        self.wait()
        self.play(
...
class HeroAndVillain(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeInFromDown(good_euler), Write(good_euler_label))
...
class AnalysisQuote(Scene):
    def construct(self):
        ...
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(
            radius=1, positioning_func=lambda circ: circ.next_to(text, DOWN, LARGE_BUFF)
        )
...
class QuarterTurn(Scene):
    def construct(self):
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(
            radius=2, positioning_func=lambda circ: circ.move_to(4 * RIGHT)
        )
        circle, radius, one, decimal = terms
        ...
        self.play(*generate_anims2(), rate_func=lambda t: 0.25 * smooth(t))
        self.wait()
        ...
        self.wait()
        self.play(pi_halves.shift, 0.75 * UP, FadeInFromDown(tau_fourths))
        self.wait()
...
class UsingTheta(Scene):
    def construct(self):
        ...
        last_frac = 0
        for frac in 1.0 / 4, 1.0 / 12, 3.0 / 8, 1.0 / 6, 5.0 / 6:
            self.play(
...
class EulerThinking(Scene):
    def construct(self):
        ...
        pi_vs_tau = OldTexText(
            "Should $\\pi$ represent \\\\", "3.1415...", "or", "6.2831...", "?"
        )
...
class WhatIsRight(PiCreatureScene):
    ...
    def construct(self):
        ...
        literal_derivative = OldTex("f'(x) = \\frac{f(x+dx) - f(x)}{dx}")
        limit_derivative = OldTex("f'(x) = \\lim_{h \\to 0}\\frac{f(x+h) - f(x)}{h}")

        divide_by_zero_definable = OldTex("\\frac{1}{0}", "\\text{ has meaning}")
        divide_by_zero_undefinable = OldTex("\\frac{1}{0}", "\\text{ is undefined}")

        left_mobs = VGroup(
            sum_over_N_converges, literal_derivative, divide_by_zero_definable
        )
        right_mobs = VGroup(
            sum_over_N_diverges, limit_derivative, divide_by_zero_undefinable
        )
        ...
        for left, right in zip(left_mobs, right_mobs)[:-1]:
            ...
            self.wait()
            self.play(randy.change, "raise_left_hand", FadeInFromDown(right))
...
class AskPuzzle(TeacherStudentsScene):
    def construct(self):
        ...
        response = OldTexText(
            "What do you mean by ", "$\\displaystyle \\sum_{n = 1}^{\\infty}$", "?"
        )
        ...
        self.play(
            ...
            series.to_corner,
            UP + LEFT,
            PiCreatureSays(self.students[0], question, target_mode="raise_left_hand"),
        )
...
class ChangeTopic(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                tau,
                "We should \\emph{really} celebrate \\\\ on 6/28!",
                target_mode="angry",
            ),
            pi.change,
            "guilty",
...
class EndScene(PatreonEndScreen):
    ...
    def construct(self):
        ...
        instagram_line = OldTexText("randy\\_the\\_pi")
...
class Thumbnail(Scene):
    def construct(self):
        pi, eq, num = formula = OldTex("\\pi", "=", "6.283185\\dots")
        formula.scale(2)
