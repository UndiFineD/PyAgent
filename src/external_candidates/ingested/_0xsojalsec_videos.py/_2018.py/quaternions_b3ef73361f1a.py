# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\quaternions.py
...
class IntroduceHamilton(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, quote, lag_ratio=0.2, run_time=4),
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeOutAndShiftDown, VGroup(*it.chain(quote, quote_rect, quote_label))
            ),
        )
        self.wait()
...
class QuaternionHistory(Scene):
    ...
    def show_dot_product_and_cross_product(self):
        ...
        dot_product = VGroup(
            v1.copy(), OldTex("\\cdot").scale(2), v2.copy(), OldTex("="), dot_rhs
        )
        cross_product = VGroup(
            v1.copy(), OldTex("\\times"), v2.copy(), OldTex("="), cross_rhs
        )
    ...
    def teaching_students_quaternions(self):
        ...
        self.play(
            ...
            LaggedStartMap(FadeOutAndShift, images_with_labels, lambda m: (m, UP)),
    ...
    def show_anti_quaternion_quote(self):
        ...
        for quote in quotes:
            self.play(
                LaggedStartMap(
                    FadeIn, VGroup(*quote.family_members_with_points()), lag_ratio=0.3
                )
            )
            self.wait()
...
class HereWeTackle4d(TeacherStudentsScene):
    def construct(self):
        titles = VGroup(
            OldTexText("This video:\\\\", "Quaternions in 4d"),
            OldTexText("Next video:\\\\", "Quaternions acting on 3d"),
        )
        for title in titles:
        ...
        self.play(
            ...
            FadeIn(titles[1]),
            ApplyMethod(
                titles[0].shift, 2 * UP, rate_func=squish_rate_func(smooth, 0.2, 1)
            ),
        )
...
class TableOfContents(Scene):
    def construct(self):
        chapters = VGroup(
            OldTexText("\\underline{Chapter 1}\\\\", "Linus the Linelander"),
            OldTexText("\\underline{Chapter 2}\\\\", "Felix the Flatlander"),
            OldTexText("\\underline{Chapter 3}\\\\", " You, the 3d-lander"),
        )
...
class IntroduceLinusTheLinelander(Scene):
    ...
    def look_at_complex_plane(self):
        ...
        q_marks_anim = LaggedStartMap(
            FadeIn, q_marks, run_time=15, rate_func=there_and_back, lag_ratio=0.1
        )
...
class DefineComplexNumbersPurelyAlgebraically(Scene):
    ...
    def show_example_number(self):
        ...
        self.wait()
        self.play(
            GrowFromCenter(imag_brace), LaggedStartMap(FadeIn, imag_label), run_time=1
        )
        self.play(Blink(linus))
        self.play(
            linus.change,
...
class TextbookQuaternionDefinition(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students("horrified", "pleading", "sick", equation),
            self.teacher.change,
            "erm",
            equation,
        )
...
class ProblemsWhereComplexNumbersArise(Scene):
    def construct(self):
        ...
        problems = VGroup(
            OldTexText("Integer solutions to\\\\ $a^2 + b^2 = c^2$", alignment=""),
            OldTexText(
                "Understanding\\\\",
...
class WalkThroughComplexMultiplication(ShowComplexMultiplicationExamples):
    ...
    def show_action_on_all_complex_numbers(self):
        ...
        product_label = VGroup(
            OldTex("z \\cdot w ="), DecimalNumber(product, num_decimal_places=0)
        )
        ...
        self.play(
            LaggedStartMap(
                ShowCreationThenDestruction, sparkly_plane, lag_ratio=0.5, run_time=2
            )
        )
        self.play(
            plane.apply_complex_function,
...
class ShowUnitCircleActions(ShowComplexMultiplicationExamples):
    ...
    def show_unit_circle_actions(self):
        ...
        title = OldTexText(
            "Numbers on the unit circle", "$\\rightarrow$", "pure rotation."
        )
...
class IntroduceStereographicProjection(MovingCameraScene):
    ...
    def describe_individual_points(self):
        ...
        one_point, zero_point, i_point, neg_i_point, neg_one_point = [
            plane.number_to_point(n) for n in [1, 0, complex(0, 1), complex(0, -1), -1]
        ]
        i_pin = PushPin()
        ...
        for arc, point, path_arc in triplets:
            self.play(
                ShowCreation(arc), dot.move_to, point, path_arc=path_arc, run_time=2
            )
            self.wait()
...
class ShowRotationUnderStereographicProjection(IntroduceStereographicProjection):
    ...
    def talk_through_90_degree_rotation(self):
        plane = self.plane
        points = [
            plane.number_to_point(z) for z in [1, complex(0, 1), -1, complex(0, -1)]
        ]
        arrows = VGroup()
    ...
    def show_example_angles(self):
        ...
        for angle in self.example_angles:
            ...
            self.play(
                ...
                ChangingDecimal(
                    angle_label, lambda a: (self.get_angle() % TAU) / DEGREES
                ),
                run_time=4,
            )
            self.wait()
...
class IntroduceThreeDNumbers(SpecialThreeDScene):
    ...
    def show_example_number(self):
        ...
        self.play(
            ShowCreation(point_line), FadeIn(dot, -coords), FadeInFromDown(number_label)
        )
        self.wait()
...
class TwoDStereographicProjection(IntroduceFelix):
    ...
    def talk_through_sphere(self):
        ...
        def get_dot():
            ...
            dot.apply_matrix(
                z_to_vector(normalize(point.get_location())), about_point=ORIGIN
            )
            return dot
    ...
    def draw_projection_lines(self):
        ...
        self.play(
            point_mob.move_to, radius * normalize(np.array([1, -1, -1])), run_time=3
        )
    ...
    def show_point_at_infinity(self):
        points = list(
            compass_directions(12, start_vect=rotate_vector(RIGHT, 3.25 * DEGREES))
        )
        points.pop(7)
        points.pop(2)
...
class FelixViewOfProjection(TwoDStereographicProjection):
    ...
    def show_a_few_rotations(self):
        ...
        dot = Dot(color=PINK)
        dot.add_updater(
            lambda d: d.move_to(self.project_point(np.dot(2 * OUT, get_rot_matrix().T)))
        )
        hand = Hand()
...
class IntroduceQuaternions(Scene):
    ...
    def show_components_of_quaternion(self):
        ...
        self.play(
            LaggedStartMap(GrowFromCenter, braces),
            LaggedStartMap(FadeInFrom, VGroup(real_word, imag_word), lambda m: (m, UP)),
        )
        self.wait()
        self.play(
    ...
    def get_example_numbers(self):
        number_2d = VGroup(
            DecimalNumber(3.14), OldTex("+"), DecimalNumber(1.59), OldTex("i")
        )
        number_3d = VGroup(
            DecimalNumber(2.65),
            OldTex("+"),
...
class ShowComplexMagnitude(ShowComplexMultiplicationExamples):
    def construct(self):
        ...
        z_label = OldTex(
            "z", "=", "a", "+", "b", "i", tex_to_color_map=tex_to_color_map
        )
        z_label.add_background_rectangle()
...
class BreakUpQuaternionMultiplicationInParts(Scene):
    def construct(self):
        ...
        self.play(
            Rotate(line2, -line1.get_angle(), about_point=line2.get_start(), run_time=3)
        )
        self.wait()
...
class SphereProjectionsWrapper(Scene):
    def construct(self):
        rect_rows = VGroup(
            *[
                VGroup(*[ScreenRectangle(height=3) for x in range(3)]).arrange(
...
class HypersphereStereographicProjection(SpecialThreeDScene):
    ...
    def show_one(self):
        ...
        self.play(
            ApplyMethod(q_tracker.set_value, [1, 0, 0, 0], run_time=2),
    ...
    def show_unit_sphere(self):
        sphere = self.sphere = self.get_projected_sphere(
            quaternion=[1, 0, 0, 0], null_axis=0, solid=False, stroke_width=0.5
        )
    ...
    def show_quaternions_with_nonzero_real_part(self):
        ...
        self.set_quat(
            [1, 1, 2, 0],
            added_anims=[ApplyMethod(self.sphere.copy().scale, 0, remover=True)],
        )
        self.wait(2)
        ...
        self.set_quat(
            ...
            added_anims=[
                ApplyFunction(
                    lambda s: s.scale(10).fade(1), self.sphere.copy(), remover=True
                )
            ],
        )
        self.wait(2)
    ...
    def get_projected_sphere(self, null_axis, quaternion=None, solid=True, **kwargs):
        ...
        sphere = StereoProjectedSphereFromHypersphere(
            quaternion=quaternion, null_axis=null_axis, **config
        )
...
class RuleOfQuaternionMultiplication(HypersphereStereographicProjection):
    ...
    def setup_multiplier_tracker(self):
        ...
        self.multiplier_tracker.add_updater(
            lambda m: m.set_value(normalize(m.get_value(), fall_back=[1, 0, 0, 0]))
        )
...
class ShowDistributionOfI(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            "What does it do \\\\ to other quaternions?", target_mode="raise_left_hand"
        )
    ...
    def show_multiplication(self):
        ...
        for q in [quat, [1, 0, 0, 0], quat]:
            self.play(
                ...
                q,
                UpdateFromFunc(
                    m_tracker, lambda m: m.set_value(normalize(m.get_value()))
                ),
                run_time=5,
            )
            self.wait()
...
class ShowArbitraryMultiplication(ShowMultiplicationBy135Example):
    ...
    def add_circles(self):
        ...
        self.circle2 = self.add_auto_updating_circle(
            basis_vectors=[bv1, bv2], colors=[WHITE, GREY]
        )
...
class RotationsOfCube(SpecialThreeDScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            cube.shift, 5 * RIGHT, label1.shift, 5 * RIGHT, Write(cube2, run_time=1)
        )
        self.play(
