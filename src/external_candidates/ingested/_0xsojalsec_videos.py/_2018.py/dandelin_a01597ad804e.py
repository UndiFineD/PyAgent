...
class SumOfIntegersProof(Scene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                MoveToTarget, numbers, path_arc=-90 * DEGREES, lag_ratio=1, run_time=1
            )
        )
        ...
        self.play(
            ReplacementTransform(
                rows.copy().set_fill(opacity=0), flipped_rows, path_arc=-PI, run_time=2
            )
        )
        self.wait()
...
class MultipleDefinitionsOfAnEllipse(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeInFrom, definitions, lambda m: (m, RIGHT), run_time=4)
        )
        self.wait()
        for definition in definitions:
...
class ShowArrayOfEccentricities(Scene):
    def construct(self):
        ...
        group = VGroup(
            *it.chain(
                eccentricity_labels, ellipses[1:], arrows, name, alt_name, [morty]
            )
        )
        self.play(
...
class ShowOrbits(ShowArrayOfEccentricities):
    ...
    def construct(self):
        ...
        self.play(
            ...
            ChangingDecimal(
                eccentricity_label, lambda a: self.get_eccentricity(comet_orbit)
            ),
...
class AskWhyAreTheyTheSame(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            morty.change, "awe", self.change_students("confused", "confused", "sassy")
        )
        ...
        self.wait(2)
        self.play(
            baby_morty.change, "thinking", LaggedStartMap(DrawBorderThenFill, bubble)
        )
...
class WriteConjecture(Scene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeInFromDown, VGroup(title, equation), lag_ratio=0.7)
        )
        self.wait()
        for word, line in zip(words, lines):
...
class QuickGeometryProof(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(GrowFromCenter, everything, lag_ratio=0.25, run_time=4)
        )
...
class AskWhyYouWouldChooseThisProof(PiCreatureScene):
    def construct(self):
        ...
        for pi, vect, word in (randy, UP, "You"), (
            other,
            LEFT,
            "Non-math \\\\ enthusiast",
        ):
...
class CreativeConstruction(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            MoveToTarget(lightbulb, run_time=3, rate_func=there_and_back, lag_ratio=0.5)
        )
    ...
    def get_light_shine(self, lightbulb, max_radius=15.0, delta_r=0.025):
        ...
        return LaggedStartMap(
            FadeIn, rings, rate_func=there_and_back, run_time=2, lag_ratio=0.5
        )
