# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\WindingNumber.py
import warnings
...
def rev_to_rgba(alpha):
    alpha = (
        0.5 - alpha
    ) % 1  # For convenience, to go CW from red on left instead of CCW from right
...
class EquationSolver1d(GraphScene, ZoomedScene):
    ...
    def drawGraph(self):
        ...
        if self.graph_label != None:
            curve_label = self.get_graph_label(
                self.graph, self.graph_label, x_val=4, direction=LEFT
            )
    ...
    def solveEquation(self):
        ...
        startBrace = OldTex(
            "|", stroke_width=10
        )  # Tex("[") # Not using [ and ] because they end up crossing over
        ...
        leftBrace.move_to(
            self.coords_to_point(lowerX, self.base_line_y)
        )  # , aligned_edge = RIGHT)
        ...
        rightBrace.move_to(
            self.coords_to_point(upperX, self.base_line_y)
        )  # , aligned_edge = LEFT)
        ...
        x_guess_line = Line(
            lowerDotXPoint, upperDotXPoint, color=WHITE, stroke_width=10
        )

        lowerGroup = Group(
            lowerDot, leftBrace, downBrace, lowerXLine, lowerYLine, x_guess_line
        )

        upperGroup = Group(
            upperDot, rightBrace, upBrace, upperXLine, upperYLine, x_guess_line
        )
        ...
        for i in range(self.num_iterations):
            if i == self.iteration_at_which_to_start_zoom:
                ...
                self.play(
                    lowerDot.scale, inverseZoomFactor, upperDot.scale, inverseZoomFactor
                )
            ...
            def makeUpdater(xAtStart, fixed_guess_x):
                def updater(group, alpha):
                    ...
                    fixed_guess_point = self.coords_to_point(
                        fixed_guess_x, self.base_line_y
                    )
            ...
            midXBrace = (
                startBrace.copy()
            )  # Had start and endBrace been asymmetric, we'd do something different here
            ...
            midXPoint = Dot(
                self.coords_to_point(midX, self.base_line_y) + OUT, color=sign_color
            )
            ...
            midXLine = DashedLine(
                self.coords_to_point(midX, self.base_line_y), midCoords, color=midColor
            )
            self.play(ShowCreation(midXLine))
            midDot = Dot(midCoords, color=sign_color)
            if (
                self.iteration_at_which_to_start_zoom != None
                and i >= self.iteration_at_which_to_start_zoom
            ):
                midDot.scale(inverseZoomFactor)
            self.add(midDot)
            midYLine = DashedLine(
                midCoords, self.coords_to_point(0, midY), color=sign_color
            )
            self.play(
            ...
            if in_negative_branch:
                self.play(
                    UpdateFromAlphaFunc(
                        lowerGroup, makeUpdater(lowerX, fixed_guess_x=upperX)
                    ),
                ...
                self.play(
                    UpdateFromAlphaFunc(
                        upperGroup, makeUpdater(upperX, fixed_guess_x=lowerX)
                    ),
                    FadeOut(guess_labels),
                )
                upperX = midX
...
def make_alpha_winder(func, start, end, num_checkpoints, cheap=False):
    ...
    for i in range(num_checkpoints - 1):
        check_points[i + 1] = resit_near(
            func(start + (i + 1) * step_size), check_points[i]
        )
    ...
    def return_func(alpha):
        ...
        if cheap:
            return check_points[
                index
            ]  # A more principled test that at least returns a reasonable answer
        else:
...
class WalkerAnimation(Animation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        base_walker = (
            Dot().scale(5 * 0.35).set_stroke(walker_stroke_color, 2)
        )  # PiCreature().scale(0.8 * 0.35)
...
class ColorMappedByFuncScene(Scene):
    ...
    def setup(self):
        ...
        if self.in_background_pass:
            print(
                "The background file does not exist yet; this will be a background creation + video pass"
            )
        else:
        ...
        if self.in_background_pass:
            self.camera.set_background_from_func(
                lambda x_y: point_to_rgba(
                    self.pixel_pos_to_color_func((x_y[0], x_y[1]))
                )
            )
...
class PiWalker(ColorMappedByFuncScene):
    ...
    def construct(self):
        ...
        for i in range(len(walk_coords)):
            ...
            val_alpha_func = (
                lambda a, start_coords=start_coords, end_coords=end_coords: self.func(
                    interpolate(start_coords, end_coords, a)
                )
            )

            if self.display_wind:
                clockwise_val_func = lambda p: -point_to_rev(self.func(p))
                alpha_winder = make_alpha_winder(
                    clockwise_val_func, start_coords, end_coords, self.num_checkpoints
                )
                number_update_func = (
                    lambda alpha, alpha_winder=alpha_winder, start_wind=start_wind: alpha_winder(
                        alpha
                    )
                    - alpha_winder(0)
                    + start_wind
                ...
                start_wind = (
                    0 if i + 1 in self.wind_reset_indices else number_update_func(1)
                )
            elif self.display_size:
                # We need to do this roundabout default argument thing to get the closure we want,
                # so the next iteration changing val_alpha_func doesn't change this closure
                number_update_func = (
                    lambda a, val_alpha_func=val_alpha_func: point_to_rescaled_size(
                        val_alpha_func(a)
                    )
            ...
            if self.display_odometer:
                ...
                # We need to do this roundabout default argument thing to get the closure we want,
                # so the next iteration changing val_alpha_func doesn't change this closure
                rev_func = lambda a, val_alpha_func=val_alpha_func: point_to_rev(
                    val_alpha_func(a)
                )
                base_arrow = Arrow(ORIGIN, RIGHT, buff=0)
                new_anim = FuncRotater(
                    base_arrow,
                    rev_func=rev_func,
                    run_time=self.step_run_time,
                    rate_func=linear,
        ...
        if self.display_odometer:
            ...
            color_wheel.color_using_background_image(
                self.short_path_to_long_path("pure_color_map.png")
            )  # Manually inserted here; this is unclean
...
class PiWalkerCircle(PiWalker):
    ...
    def setup(self):
        ...
        N = self.num_steps
        self.walk_coords = [
            r * np.array((np.cos(i * TAU / N), np.sin(i * TAU / N))) for i in range(N)
        ]
        PiWalker.setup(self)
...
class RectangleData:
    ...
    def splits_on_dim(self, dim):
        ...
        if dim == 0:
            return_data = [
                RectangleData(new_interval, y_interval)
                for new_interval in split_interval(x_interval)
            ]
        elif dim == 1:
            return_data = [
                RectangleData(x_interval, new_interval)
                for new_interval in split_interval(y_interval)[::-1]
            ]
        else:
...
class EquationSolver2dNode(object):
    ...
    def display_in_series(self):
        return Succession(
            self.first_anim, *[n.display_in_series() for n in self.children]
        )

    def display_in_parallel(self):
        return Succession(
            self.first_anim,
            AnimationGroup(*[n.display_in_parallel() for n in self.children]),
        )
...
class EquationSolver2d(ColorMappedObjectsScene):
    ...
    def construct(self):
        if self.num_iterations == 0:
            print(
                "You forgot to set num_iterations (maybe you meant to subclass something other than EquationSolver2d directly?)"
            )
            return
        ...
        base_line = Line(
            UP, RIGHT, stroke_width=border_stroke_width, color=self.line_color
        )
        ...
        linger_rate = squish_rate_func(
            lambda t: t, 0, fdiv(run_time_base, run_time_with_lingering)
        )
        ...
        def Animate2dSolver(
            ...
            def draw_line_return_wind(
                start, end, start_wind, should_linger=False, draw_line=True
            ):
                alpha_winder = make_alpha_winder(
                    clockwise_val_func,
                    start,
                    ...
                    cheap=self.use_cheap_winding_numbers,
                )
                a0 = alpha_winder(0)
                rebased_winder = lambda alpha: alpha_winder(alpha) - a0 + start_wind
                colored_line = Line(
                    num_plane.coords_to_point(*start) + IN,
                    num_plane.coords_to_point(*end) + IN,
                )
                ...
                walker_anim = LinearWalker(
                    ...
                    number_update_func=(
                        rebased_winder if self.show_winding_numbers else None
                    ),
                    remover=True,
                ...
                if (
                    should_linger
                ):  # Do we need an "and not self.display_in_parallel" here?
                ...
                opt_line_anim = (
                    ShowCreation(colored_line) if draw_line else empty_animation
                )

                line_draw_anim = AnimationGroup(
                    opt_line_anim, walker_anim, run_time=run_time, rate_func=rate_func
                )
            ...
            for i, (start, end) in enumerate(sides):
                next_anim, wind_so_far = draw_line_return_wind(
                    start,
                    end,
                    wind_so_far,
            ...
            if total_wind == 0:
                ...
                points = (
                    np.array([num_plane.coords_to_point(x, y) for (x, y) in coords])
                    + 3 * IN
                )
                ...
                fill_rect = polygonObject = Polygon(
                    *points, fill_opacity=0.8, color=GREY_D
                )
                ...
                children = [
                    ...
                    for (index, (sub_rect, side_to_draw)) in enumerate(
                        sub_rect_and_sides
                    )
                ]
                mid_line_coords = rect.split_line_on_dim(dim_to_split)
                mid_line_points = [
                    num_plane.coords_to_point(x, y) + 2 * IN
                    for (x, y) in mid_line_coords
                ]
                mid_line = DashedLine(*mid_line_points)

                return EquationSolver2dNode(
                    Succession(anim, ShowCreation(mid_line)), children
                )
        ...
        border_anim = ShowCreation(
            border, run_time=rect_time_with_linger, rate_func=rect_rate
        )

        print(
            "About to do the big Play; for reference, the current time is ",
            time.strftime("%H:%M:%S"),
        )
        ...
        print(
            "All done; for reference, the current time is ", time.strftime("%H:%M:%S")
        )
...
class LinePulser(ContinualAnimation):
    def __init__(
        self, line, bullet_template, num_bullets, pulse_time, output_func=None, **kwargs
    ):
        self.line = line
...
class ArrowCircleTest(Scene):
    def construct(self):
        ...
        arrows = [
            rev_rotate(base_arrow.copy(), 0.5 - (fdiv(i, num_arrows)))
...
class OdometerScene(ColorMappedObjectsScene):
    CONFIG = {
        ...
        "rotate_func": lambda x: 2
        * np.sin(2 * x * TAU),  # This is given in terms of CW revs
        "run_time": 40,
...
class VectorField(Scene):
    ...
    def construct(self):
        ...
        x_min, y_min = num_plane.point_to_coords(
            FRAME_X_RADIUS * LEFT + FRAME_Y_RADIUS * UP
        )
        x_max, y_max = num_plane.point_to_coords(
        ...
        for x, y in points:
            ...
            normalized_output = output * fdiv(
                self.normalized_arrow_scale_factor, output_size
            )  # Assume output has nonzero size here
...
class HasItsLimitations(Scene):
    ...
    def construct(self):
        ...
        new_curved_arrow.shift(
            (new_input_dot.get_center() - new_curved_arrow.get_points()[0]) - input_diff
        )

        new_output_dot = output_dot.copy().move_to(
            new_curved_arrow.get_points()[-1] + output_diff
        )
        new_output_label = output_label.copy().next_to(new_output_dot, UP + RIGHT)

        dot_objects = Group(
            input_dot, input_label, output_dot, output_label, curved_arrow
        )
        new_dot_objects = Group(
            new_input_dot,
            new_input_label,
            new_output_dot,
            new_output_label,
...
class NumberLineScene(Scene):
    def construct(self):
        ...
        interval_1d = Line(
            left_point, right_point, stroke_color=inner_color, stroke_width=stroke_width
        )
        ...
        border_2d = Polygon(
            *random_points, stroke_color=border_color, stroke_width=stroke_width
        )
        ...
        self.play(
            FadeOut(num_line), FadeIn(num_plane), ReplacementTransform(full_1d, full_2d)
        )
...
class Initial2dFuncSceneWithoutMorphing(Initial2dFuncSceneBase):
    ...
    def setup(self):
        ...
        split_screen_camera = SplitScreenCamera(
            left_camera, right_camera, **self.camera_config
        )
    ...
    def construct(self):
        ...
        if self.show_full_color_map:
            bright_background = Rectangle(
                width=2 * FRAME_X_RADIUS + 1,
                height=2 * FRAME_Y_RADIUS + 1,
                fill_opacity=1,
            )
...
class LoopSplitScene(ColorMappedObjectsScene):
    ...
    def PulsedLine(
        self, start, end, bullet_template, num_bullets=4, pulse_time=1, **kwargs
    ):
    ...
    def construct(self):
        ...
        def bullet_fade(start, end, mob):
            return UpdateFromAlphaFunc(
                mob, lambda m, a: m.set_fill(opacity=interpolate(start, end, a))
            )
        ...
        def line_fade(start, end, mob):
            return UpdateFromAlphaFunc(
                ...
                lambda m, a: m.set_stroke(
                    width=interpolate(start, end, a) * stroke_width
                ),
            )
        ...
        def play_fade_left(start, end):
            play_combined_fade(
                start, end, left_square_lines_vmobject, left_square_bullets
            )

        def play_fade_right(start, end):
            play_combined_fade(
                start, end, right_square_lines_vmobject, right_square_bullets
            )
        ...
        self.play(
            line_fade(1, faded, right_square_lines_vmobject),
            *bullet_list_fade(0, 1, left_square_bullets),
        )
        self.wait()
        flash_circles([indicate_circle(l) for l in left_square_lines])
        self.play(
            line_fade(faded, 1, right_square_lines_vmobject),
            *bullet_list_fade(1, 0, left_square_bullets),
        )
        self.wait()

        self.add(*right_square_anims)
        self.play(
            line_fade(1, faded, left_square_lines_vmobject),
            *bullet_list_fade(0, 1, right_square_bullets),
        )
        self.wait()
        flash_circles([indicate_circle(l) for l in right_square_lines])
        self.play(
            line_fade(faded, 1, left_square_lines_vmobject),
            *bullet_list_fade(1, 0, right_square_bullets),
        )
        self.wait()
        ...
        self.play(
            ...
            line_fade(1, 0, midline_lines_vmobject),
            *bullet_list_fade(1, 0, midline_bullets),
        )
        self.wait()
        ...
        self.play(
            FadeIn(inner_circle),
            line_fade(0, 1, midline_lines_vmobject),
            *bullet_list_fade(0, 1, midline_bullets),
        )
        self.wait()
        self.play(
            FadeOut(inner_circle),
    ...
    CONFIG = {
        "func": plane_func_by_wind_spec(
            ...
            (
                ...
                -0.2,
                1,
            ),  # Not an exact match, because our breakdown function has a zero along midlines...
        ),
        "num_iterations": 5,
        "display_in_parallel": True,
        "show_cursor": True,
...
class BorderOf2dRegionScene(Scene):
    def construct(self):
        ...
        interior = Polygon(
            *points, fill_color=neutral_color, fill_opacity=1, stroke_width=0
        )
        self.play(FadeIn(interior))

        border = Polygon(
            *points, color=negative_color, stroke_width=border_stroke_width
        )
        self.play(ShowCreation(border))


big_loop_no_zeros_func = lambda x_y5: complex_to_pair(
    np.exp(complex(10, x_y5[1] * np.pi))
)
...
class ExamplePlaneFunc(ColorMappedByFuncScene):
    ...
    def construct(self):
        ...
        nonzero_point = (
            ORIGIN  # Manually chosen, not auto-synced with example_plane_func
        )
...
class UhOhScene(EquationSolver2d):
    CONFIG = {
        "func": uhOhFunc,
        "manual_wind_override": (
            1,
            None,
            (1, None, (1, None, None)),
        ),  # Tailored to UhOhFunc above
        "show_winding_numbers": False,
        "num_iterations": 5,
    }
...
class UhOhSalientStill(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        midline = Line(
            new_up + 0.5 * new_left, 0.5 * new_left, stroke_width=border_stroke_width
        )
...
criticalStrip = Axes(
    x_min=-0.5,
    x_max=1.5,
    x_axis_config={"unit_size": FRAME_X_RADIUS, "number_at_center": 0.5},
    y_min=-criticalStripYScale,
    y_max=criticalStripYScale,
