...
class ShowWritingTrajectory(TeacherStudentsScene):
    ...
    def show_meandering_path(self):
        ...
        dashed_path = VGroup(
            *[
                VMobject().pointwise_become_partial(solid_path, x, x + step / 2)
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, dashed_path, lambda m: (m.scale, 0), remover=True
            ),
            self.teacher.change,
            "tease",
    ...
    def show_previous_video(self):
        ...
        complex_words.next_to(
            screen.target, LEFT, buff=2 * SMALL_BUFF + arrow.get_length()
        )
...
class CylinderModel(Scene):
    ...
    def show_numbers(self):
        ...
        exp_decimal = DecimalNumber(
            0, num_decimal_places=2, include_background_rectangle=True, color=YELLOW
        )
...
class ElectricField(CylinderModel, MovingCameraScene):
    ...
    def show_uniform_field(self):
        vector_field = self.vector_field = VectorField(
            lambda p: UP, colors=[BLUE_E, WHITE, RED]
        )
    ...
    def show_moving_charges(self):
        ...
        electrons = VGroup(
            *[
                get_electron().move_to(
                    proton.get_center() + proton.radius * rotate_vector(RIGHT, angle)
                )
        ...
        new_vector_field = VectorField(
            new_electric_field, colors=self.vector_field.colors
        )
...
class ScopeMeiosis(PiCreatureScene):
    ...
    def construct(self):
        ...
        sections = VGroup(
            *[VGroup(title, self.get_lines(title, 3)) for title in section_titles]
        )
    ...
    def get_lines(self, title, n_lines):
        lines = VGroup(
            *[Line(3 * LEFT, 3 * RIGHT, color=GREY_B) for x in range(n_lines)]
        )
...
class TopicsAndConnections(Scene):
    ...
    def construct(self):
        dots = VGroup(
            *[
                Dot(8 * np.array([random.random(), random.random(), 0]))
                for n in range(5)
            ]
        )
        topics = VGroup(
...
class IntroduceVectorField(Scene):
    ...
    def points_to_vectors(self):
        vector_field = self.vector_field = VectorField(
            four_swirls_function, **self.vector_field_config
        )
        dots = VGroup()
    ...
    def show_fluid_flow(self):
        ...
        stream_line_animation = AnimatedStreamLines(
            stream_lines, **self.stream_line_animation_config
        )
...
class ShorteningLongVectors(IntroduceVectorField):
    ...
    def contrast_adjusted_and_non_adjusted(self):
        ...
        self.wait()
        self.play(
            LaggedStartMap(ApplyMethod, adjusted, lambda m: (m.restore,), run_time=3)
        )
        self.wait()
...
class GravityFluidFlow(IntroduceVectorField):
    def construct(self):
        self.vector_field = VectorField(
            lambda p: np.array(ORIGIN), **self.vector_field_config
        )
        self.show_gravitational_force()
        self.show_fluid_flow()
...
class DefineDivergence(ChangingElectricField):
    ...
    def point_out_sources_and_sinks(self):
        ...
        self.positive_points, self.negative_points = [
            [particle.get_center() for particle in particles if u * particle.charge > 0]
            for u in (+1, -1)
        ]
...
class DefineDivergenceSymbols(Scene):
    def construct(self):
        ...
        output_label = output_brace.get_text(
            "Measure of how much \\\\ " "$(x, y)$ ``generates'' fluid"
        )
        brace_label_pairs = [
            (F_brace, F_label),
            (xy_brace, xy_label),
...
class DivergenceAsNewFunction(Scene):
    ...
    def show_vector_field_function(self):
        ...
        in_x, in_y, out_x, out_y = [
            DecimalNumber(0, include_sign=True) for x in range(4)
        ]
        ...
        self.out_vect_update = Mobject.add_updater(
            out_vect, lambda ov: Transform(ov, get_out_vect()).update(1)
        )
        ...
        self.add(
            Mobject.add_updater(VGroup(out_x, out_y), lambda m: m.match_style(out_vect))
        )
        self.wait()
    ...
    def show_divergence_function(self):
        ...
        vector_field.remove(
            *[v for v in vector_field if v.get_start()[0] < 0 and v.get_start()[1] > 2]
        )
        ...
        def get_vector_ring():
            return VGroup(
                *[vector_field.get_vector(point) for point in get_neighboring_points()]
            )
        ...
        def show_flow():
            ...
            self.play(
                LaggedStartMap(ShowCreationThenDestruction, stream_lines, remover=True)
            )

        vector_ring = get_vector_ring()
        vector_ring_update = Mobject.add_updater(
            vector_ring, lambda vr: Transform(vr, get_vector_ring()).update(1)
        )
        ...
        self.add(
            *[
                Mobject.add_updater(mob, lambda m: m.set_fill(None, 0))
...
class IntroduceCurl(IntroduceVectorField):
    ...
    def show_rotation(self):
        ...
        for group, u in (counterclockwise_arrows, +1), (clockwise_arrows, -1):
            for arrows in group:
                label = OldTex(
                    "\\text{curl} \\, \\textbf{F}", ">" if u > 0 else "<", "0"
                )
        ...
        for group in counterclockwise_arrows, clockwise_arrows:
            ...
            self.play(
                UpdateFromAlphaFunc(
                    ...
                    ).set_stroke(width=interpolate(5, 10, alpha)),
                    rate_func=there_and_back,
                    run_time=2,
                )
            )
    ...
    def get_rotation_arrows(self, clockwise=True, width=1):
        result = VGroup(
            *[
                Arrow(*points, buff=2 * SMALL_BUFF, path_arc=90 * DEGREES).set_stroke(
                    width=5
                )
...
class ShowCurlAtVariousPoints(IntroduceCurl):
    ...
    def show_curl_at_points(self):
        ...
        curl_tex_update = Mobject.add_updater(
            curl_tex, lambda m: m.next_to(circle, UP, SMALL_BUFF)
        )
...
class IllustrateGaussMagnetic(IllustrateGaussLaw):
    ...
    def add_wires(self):
        top, bottom = [
            Circle(radius=0.275, stroke_color=WHITE, fill_color=BLACK, fill_opacity=1)
            for x in range(2)
        ]
...
class ShowTwoPopulations(Scene):
    ...
    def introduce_animals(self):
        foxes = self.foxes = VGroup(
            *[self.get_fox() for n in range(self.total_num_animals)]
        )
        rabbits = self.rabbits = VGroup(
            *[self.get_rabbit() for n in range(self.total_num_animals)]
        )
    ...
    def evolve_system(self):
        ...
        phase_point = VectorizedPoint(
            self.start_num_rabbits * RIGHT + self.start_num_foxes * UP
        )
        self.add(
    ...
    def get_animal(self, name, color):
        ...
        x_shift, y_shift = [
            ...
            for max_val in [FRAME_WIDTH / 2 - 2, FRAME_HEIGHT / 2 - 2]
        ]
...
class PhaseSpaceOfPopulationModel(
    ShowTwoPopulations, PiCreatureScene, MovingCameraScene
):
    CONFIG = {
...
class AskAboutComputation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "confused", "sassy", "angry", added_anims=[self.teacher.change, "guilty"]
        )
        self.wait()
        self.teacher_says(
...
class DivCurlDotCross(Scene):
    def construct(self):
        ...
        titles = VGroup(
            *list(map(TexText, ["Divergence", "Curl", "Dot product", "Cross product"]))
        )
...
class ShowDotProduct(MovingCameraScene):
    ...
    def construct(self):
        ...
        dot_product = OldTex(
            "\\vec{\\textbf{v}}", self.prod_tex, "\\vec{\\textbf{w}}", "="
        )
...
class DivergenceTinyNudgesView(MovingCameraScene):
    ...
    def take_tiny_step(self):
        ...
        step_vector = Arrow(
            point, nudged_point, buff=0, color=TEAL, **vector_field.vector_config
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(vector.copy(), shifted_vector, path_arc=-TAU / 4)
        )
        self.wait()
        self.play(
        ...
        self.play(
            ...
            ReplacementTransform(
                new_vector, diff_vector.copy().shift(-vector.get_vector()), remover=True
            ),
            diff_vector.shift,
            -vector.get_vector(),
    ...
    def show_dot_product(self):
        ...
        self.moving_diff_vector_update = Mobject.add_updater(
            moving_diff_vector, update_moving_diff_vector
        )
        ...
        brace = Brace(
            dot_product.copy().scale(1 / sf, about_point=ORIGIN), DOWN, buff=SMALL_BUFF
        ...
        self.play(
            ShowPassingFlashAround(
                div_text[1:3], surrounding_rectangle_config={"buff": sf * SMALL_BUFF}
            )
        )
        ...
        for s in 0.6, -0.6:
            for step, diff in zip(all_step_vectors, all_diff_vectors):
                ...
                diff.target.put_start_and_end_on(
                    step.get_end(), step.get_end() + s * step.get_vector()
                )
            self.play(
...
class ZToHalfFlowNearWall(ComplexTransformationScene, MovingCameraScene):
    ...
    def construct(self):
        ...
        horizontal_lines = VGroup(
            *[
                l
                for l in list(planes) + [plane.axes[0]]
                if np.abs(l.get_center()[0]) < 0.1
            ]
        )
        ...
        self.apply_complex_function(
            lambda x: x ** (1.0 / 2), added_anims=[Write(equation)]
        )
...
class BroughtToYouBy(PiCreatureScene):
    ...
    def just_you_and_me(self):
        ...
        spiral = VGroup(
            *[
                VMobject().pointwise_become_partial(spiral, a, a + step)
...
class ThoughtsOnAds(Scene):
    def construct(self):
        ...
        left_text = OldTexText(
            "Any website presented \\\\", "as a click-maximizing \\\\ slideshow"
        )
        left_text.scale(0.8)
        ...
        right_rect_label = Group(
            ImageMobject("3b1b_logo", height=1), OldTexText("(hopefully)").scale(0.8)
        )
        ...
        self.play(
            Write(misaligned), Write(aligned), *list(map(GrowArrow, arrows)), run_time=1
        )
    ...
    def change_curve(self):
        new_graph = self.get_graph(
            lambda x: interpolate(
                7 * np.exp(-0.01 * x), 7 * np.exp(-3 * x), smooth(np.clip(x / 5, 0, 1))
            )
        )
