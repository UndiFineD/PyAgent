# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\reusables\coin_flipping_pi_creature.py
...
class PiCreatureCoin(VMobject):
    ...
    def init_points(self):
        ...
        ridge_positions = (
            0.5 * self.diameter * np.array([np.cos(theta) for theta in ridge_angles])
        )
...
class FlipCoin(AnimationGroup):
    CONFIG = {
        ...
        "pi_rate_func": lambda t: there_and_back_with_pause(t, 1.0 / 4),
    }

    def __init__(self, pi_creature, **kwargs):
        digest_config(self, kwargs)
        pi_creature_motion = ApplyMethod(
            pi_creature.flip_coin_up, rate_func=self.pi_rate_func, **kwargs
        )
