# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\independence.py
import scipy
...
def get_quiz(*questions):
    q_mobs = VGroup(
        *list(
            map(
                TexText,
                ["%d. %s" % (i + 1, question) for i, question in enumerate(questions)],
            )
        )
    )
    q_mobs.arrange(
...
def get_slot_group(
    ...
    lines = VGroup(*[Line(ORIGIN, MED_LARGE_BUFF * RIGHT) for x in range(n)])
    lines.arrange(RIGHT, buff=buff)
    if include_qs:
        labels = VGroup(*[OldTexText("Q%d" % d) for d in range(1, n + 1)])
    else:
...
class MeaningOfIndependence(SampleSpaceScene):
    ...
    def add_labeled_space(self):
        ...
        self.sample_space[0].divide_vertically(0.9, colors=[BLUE_D, GREEN_C])
        self.sample_space[1].divide_vertically(0.5, colors=[BLUE_E, GREEN_E])
    ...
    def align_conditionals(self):
        ...
        self.play(ShowCreation(line), Write(word, run_time=1))
        self.wait()
    ...
    def relabel(self):
        ...
        self.play(
            self.independence_word.next_to,
            equation,
            UP,
            MED_LARGE_BUFF,
            Write(equation),
    ...
    def assume_independence(self):
        ...
        self.play(morty.change, "hooray", everything, ShowCreation(bubble))
        self.wait()
...
class IntroduceBinomial(Scene):
    ...
    def add_bar_chart(self):
        ...
        self.play(LaggedStartMap(FadeIn, VGroup(*it.chain(*chart)), run_time=2))

    def add_p_slider(self):
        interval = UnitInterval(color=GREY_B)
        interval.set_width(4)
        interval.next_to(
            VGroup(self.bar_chart.x_axis, self.bar_chart.y_axis), UP, MED_LARGE_BUFF
        )
    ...
    def shift_weight_to_tails(self):
        ...
        self.play(
            ...
            ReplacementTransform(old_bars.copy().set_fill(opacity=0), new_bars),
        )
        self.play(chart_copy.change_bar_values, values)
        self.wait(2)
...
class IntroduceQuiz(PiCreatureScene):
    ...
    def add_quiz(self):
        ...
        self.play(Write(quiz), self.randy.change, "pondering", quiz)
        self.wait()
    ...
    def ask_about_probabilities(self):
        probabilities, abbreviated_probabilities = [
            ...
            for s_tex, rhs in [("\\text{Score}", "= \\, ???"), ("\\text{S}", "")]
        ]
        ...
        self.play(
            ...
            self.quiz.set_height,
            0.7 * self.randy.get_height(),
            self.quiz.next_to,
            self.randy,
            RIGHT,
            self.randy.change,
    ...
    def show_distribution(self):
        ...
        chart = BarChart(values, width=7, bar_names=list(range(4)))
        chart.to_edge(RIGHT)
        ...
        self.play(
            LaggedStartMap(Write, VGroup(*[m for m in chart if m is not chart.bars])),
        )
        self.play(
            *[
                ReplacementTransform(
                    bar.copy().stretch_to_fit_height(0).move_to(bar.get_bottom()), bar
                )
    ...
    def show_single_question_probability(self):
        prob = OldTex("P(", "\\text{Can answer a given question}", ")", "= 0.8")
        prob.to_corner(UP + RIGHT)
        prob.set_color_by_tex("text", GREEN)
        rect = SurroundingRectangle(prob, buff=MED_SMALL_BUFF)

        self.play(Write(prob), self.randy.change, "happy", prob)
        self.play(ShowCreation(rect))
        self.wait()

        self.single_question_probability = VGroup(prob, rect)
...
class BreakDownQuestionPatterns(IntroduceQuiz):
    ...
    def add_parts_from_last_scene(self):
        ...
        chart_group = VGroup(self.bar_chart, self.abbreviated_probabilities)
        self.play(
            self.single_question_probability.scale,
            0.8,
            self.single_question_probability.to_corner,
            UP + LEFT,
            chart_group.scale,
            0.7,
            chart_group.get_top(),
...
class AssociatePatternsWithScores(BreakDownQuestionPatterns):
    ...
    def add_slot_groups(self):
        self.slot_groups = VGroup(
            *list(map(self.get_slot_group, it.product(*[[True, False]] * 3)))
        )
    ...
    def show_score_groups(self):
        ...
        full_score_groups.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT)
        ...
        for score, score_group in zip(scores, score_groups):
            ...
            self.wait()
            self.play(
                ReplacementTransform(score_group.copy(), score_group.organized),
    ...
    def think_about_binomial_patterns(self):
        ...
        row = VGroup(*[triangle.coords_to_mobs[3][k] for k in range(4)])
        ...
        self.play(FadeOut(self.slot_groups), FadeIn(self.randy), FadeIn(bubble))
        self.play(
            self.randy.change,
            "pondering",
...
class BeforeCounting(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(GrowFromCenter(brace), LaggedStartMap(FadeIn, q_marks))
        self.wait(2)
...
class TemptingButWrongCalculation(BreakDownQuestionPatterns):
    ...
    def write_simple_product(self):
        ...
        for part, mob in zip(rhs, slot_group.content):
            self.play(
                *[
                    ReplacementTransform(mob.copy(), subpart, path_arc=np.pi / 6)
                    for subpart, mob in zip(part, [p_of, mob, rp])
                ]
            )
            ...
            self.wait()
        self.wait()
        self.play(Write(question), self.randy.change, "confused")
        self.wait(3)
...
class ThousandPossibleQuizzes(Scene):
    ...
    def draw_all_quizzes(self):
        ...
        self.play(Transform(full_quizzes, target_quizzes), FadeIn(title))
        self.play(
            LaggedStartMap(
                FadeIn,
                quizzes,
    ...
    def show_division_by_first_question(self):
        ...
        for num, b, split in (800, True, top_split), (200, False, bottom_split):
            label = VGroup(
                OldTex(str(num)), get_slot_group([b], buff=SMALL_BUFF, include_qs=False)
            )
            label.arrange(DOWN)
    ...
    def ask_about_second_question(self):
        ...
        sg1, sg2 = slot_groups = VGroup(
            *[
                get_slot_group([True, b], include_qs=False, buff=SMALL_BUFF)
                for b in (True, False)
            ]
        )
        question = VGroup(
            OldTexText("Where are"),
            sg1,
            OldTexText("and"),
            sg2,
            OldTexText("?"),
        ...
        little_rects = VGroup(
            *[
                SurroundingRectangle(VGroup(sg.lines[1], sg.content[1]))
    ...
    def show_that_second_is_still_80(self):
        second_right = VGroup(self.bottom_left_split, self.top_left_split)
        second_wrong = VGroup(self.bottom_right_split, self.top_right_split)
        rects = VGroup(
            *[SurroundingRectangle(mob, buff=SMALL_BUFF) for mob in second_right]
        )
        ...
        self.play(FadeOut(self.q1_split_labels), ShowCreation(rects))
        self.play(
            FadeIn(slot_group),
            Transform(
                num1,
        ...
        self.play(Write(equation), *list(map(Animation, [num1, num2])))
    ...
    def get_thousand_quizzes(self):
        ...
        for x in range(self.n_quiz_rows):
            quiz = VGroup(
                *[
                    Rectangle(height=SMALL_BUFF, width=0.5 * SMALL_BUFF)
                    for x in range(3)
                ]
            )
...
class ExampleConditional(Scene):
    def construct(self):
        prob = get_probability_of_slot_group([True, True], [True])
...
class HarderQuizzes(Scene):
    def construct(self):
        ...
        crosses = VGroup(*[Cross(quiz.questions[0]) for quiz in quizzes])
...
class SubmitToTemptation(TemptingButWrongCalculation):
    def construct(self):
        ...
        self.play(MoveToTarget(title), FadeOut(question))
        self.play(Write(new_words, run_time=2), self.randy.change, "hooray")
...
class AccurateProductRule(SampleSpaceScene, ThreeDScene):
    ...
    def show_first_division(self):
        ...
        space.divide_horizontally([0.8], colors=[GREEN_E, RED_E])
        ...
        braces_and_labels = space.get_side_braces_and_labels([top_label, bottom_label])

        self.play(FadeIn(space.horizontal_parts), FadeIn(braces_and_labels))
        self.play(ReplacementTransform(top_label.copy(), self.terms[1]))
        self.wait()
    ...
    def show_second_division(self):
        ...
        top_part.divide_vertically([0.9], colors=[GREEN_E, green_red_mix])
        label = self.terms[2].deepcopy()
        braces_and_labels = top_part.get_top_braces_and_labels(labels=[label])

        self.play(FadeIn(top_part.vertical_parts), FadeIn(braces_and_labels))
        self.play(ReplacementTransform(label.copy(), self.terms[2]))
        self.wait()
    ...
    def show_final_probability(self):
        ...
        self.wait()
        self.play(ReplacementTransform(label.copy(), self.terms[3]))
        self.wait()
...
class ShowAllEightConditionals(Scene):
    ...
    def show_all_conditionals(self):
        ...
        for bool_list in it.product(*[[True, False]] * 3):
            ...
            sub_bool_lists = [bool_list[:n] for n in (3, 1, 2, 1, 3, 2)]
            parts = equation.get_parts_by_tex(filler_tex)
            for part, sub_list in zip(parts, sub_bool_lists):
                slot_group = get_slot_group(sub_list, buff=SMALL_BUFF, include_qs=False)
                slot_group.replace(part, dim_to_match=0)
        ...
        rect = SurroundingRectangle(
            VGroup(*equations[0][7:], *equations[-1][7:]), buff=SMALL_BUFF
        )
        rect.shift(0.5 * SMALL_BUFF * RIGHT)

        self.play(LaggedStartMap(FadeIn, equations, run_time=5, lag_ratio=0.3))
        self.wait()
    ...
    def suggest_independence(self):
        ...
        self.play(FadeIn(full_screen_rect), FadeIn(randy))
        self.play(
            PiCreatureSays(
                randy, "Let's just assume \\\\ independence.", target_mode="shruggie"
            )
        )
        self.play(Blink(randy))
        self.wait()
...
class ComputeProbabilityOfOneWrong(Scene):
    ...
    def show_all_three_patterns(self):
        ...
        for i in range(2):
            self.play(
                ReplacementTransform(probabilities[i].copy(), probabilities[i + 1])
            )
        self.wait()
        for group in point_8s, point_2s:
            self.play(
                LaggedStartMap(Indicate, group, rate_func=there_and_back, lag_ratio=0.7)
            )
            self.wait()
...
class ShowFullDistribution(Scene):
    ...
    def add_scores_one_and_two(self):
        ...
        scores.arrange(DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT)
    ...
    def show_bar_chart(self):
        ...
        new_p_terms = VGroup(*[OldTex("P(", "S=%d" % k, ")") for k in range(4)])
        ...
        self.play(
            ReplacementTransform(value_mobs, chart.bars, lag_ratio=0.5, run_time=2)
        )
        self.play(
            LaggedStartMap(
                FadeIn,
                VGroup(
                    *it.chain(*[submob for submob in chart if submob is not chart.bars])
                ),
            ),
    ...
    def compare_to_binomial_pattern(self):
        ...
        self.play(ReplacementTransform(bars_copy, self.bar_chart.bars))
        self.wait(2)
        self.play(VGroup(self.bar_chart, bars, nums).to_edge, LEFT)

        self.alt_bars = bars
        self.alt_bars_labels = nums

    def show_alternate_values_of_p(self):
        new_prob = OldTex("P(", "\\text{Correct}", ")", "=", "0.8")
...
class ProbablyWrong(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(*["angry"] * 3, run_time=1)
        self.wait()
...
class ShowTrueDistribution(PiCreatureScene):
    ...
    def show_distributions(self):
        ...
        for bar, old_bar in zip(bars, old_bars):
            for mob, vect in (bar, RIGHT), (old_bar, LEFT):
                ...
                mob.target.do_about_point(
                    mob.get_corner(DOWN + vect), mob.target.stretch, 0.5, 0
                )
        ...
        self.play(chart.change_bar_values, alt_values, *list(map(ShowCreation, arrows)))
        self.wait(2)
    ...
    def imagine_score_0(self):
        ...
        bar_rect = SurroundingRectangle(
            VGroup(
                self.bar_chart.bars[0],
                self.old_bars[0],
        ...
        self.wait()
        self.play(ReplacementTransform(prob_rect, bar_rect))
        self.wait()
...
class TeacherAssessingLiklihoodOfZero(TeacherStudentsScene):
    ...
    def show_independence_probability(self):
        ...
        self.play(self.teacher.change, "raise_right_hand", FadeIn(words), Write(prob))
        self.wait()
...
class CorrelationsWith35Percent(ThousandPossibleQuizzes):
    ...
    def show_first_split(self):
        ...
        self.add(quizzes)
        self.wait()
        self.play(GrowFromCenter(brace), FadeIn(prop), *list(map(MoveToTarget, parts)))
        self.wait()
        self.play(
            top_part.fade,
            0.8,
    ...
    def show_second_split(self):
        ...
        self.play(GrowFromCenter(brace), FadeIn(prop), *list(map(MoveToTarget, parts)))
        self.wait()
        self.play(Transform(brace, term_brace), prop.next_to, term_brace, DOWN)
    ...
    def comment_on_final_size(self):
        ...
        words = OldTexText("Much more than ", "$(0.35)^3 \\approx 4.3\\%$")
...
class WeighingIndependenceAssumption(PiCreatureScene):
    def construct(self):
        ...
        self.play(randy.change, "raise_left_hand", clean, clean.restore)
...
class NameBinomial(Scene):
    ...
    def name_distribution(self):
        ...
        probability = OldTex("P(", "\\checkmark", ")", "=", str(p))
    ...
    def add_quiz_questions(self):
        n = 10
        checkmarks = VGroup(*[OldTex("\\checkmark").set_color(GREEN) for x in range(n)])
        ...
        for checkmark in checkmarks:
            ...
            crosses.add(cross)
            arrow = Arrow(checkmark, cross, tip_length=0.15, color=WHITE)
            arrows.add(arrow)
        ...
        self.play(
            *[
                LaggedStartMap(Write, mob, run_time=3, lag_ratio=0.3)
        ...
        self.play(
            ...
            faded_crosses.set_fill,
            None,
            0.5,
            faded_checkmarks.set_fill,
            None,
            0.5,
        )
        self.wait()
    ...
    def change_to_gender(self):
        ...
        girls, boys = [
            VGroup(*[template.copy().move_to(mob) for mob in group])
            for template, group in [(female, self.checkmarks), (male, self.crosses)]
        ]
        ...
        self.play(GrowFromCenter(brace), FadeIn(words))
        for m1, m2 in (self.crosses, boys), (self.checkmarks, girls):
            self.play(ReplacementTransform(m1, m2, lag_ratio=0.5, run_time=3))
        self.wait()
    ...
    def point_out_example_input(self):
        boy_girl_groups = VGroup(
            *[VGroup(boy, girl) for boy, girl in zip(self.boys, self.girls)]
        )
        girl_rects = VGroup(
            *[
        ...
        chart_rect = SurroundingRectangle(
            VGroup(chart.bars[n_girls], chart.bar_labels[n_girls]), buff=SMALL_BUFF
        )

        prob = OldTex("P(", "\\# \\text{Girls}", "=", "6", ")")
        ...
        self.play(
            LaggedStartMap(
                Indicate, girls, run_time=3, lag_ratio=0.3, rate_func=there_and_back
            )
        )
    ...
    def write_probability_of_girl(self):
        ...
        probability_copies = VGroup(
            *[
                probability.copy().scale(0.7).next_to(girl, LEFT, MED_LARGE_BUFF)
        ...
        self.wait()
        self.play(
            ReplacementTransform(VGroup(probability.copy()), probability_copies),
...
class CycleThroughPatterns(NameBinomial):
    ...
    def construct(self):
        ...
        patterns = VGroup(
            *[self.get_pattern(indicies) for indicies in shown_combinations]
        )
...
class Compute6of10GirlsProbability(CycleThroughPatterns):
    ...
    def write_n_choose_k(self):
        ...
        self.play(FadeIn(see_chapter_one), GrowFromCenter(brace))
        self.play(Write(ten_choose_six))
        self.wait(2)
        self.play(
            ten_choose_six.next_to,
            computation.copy(),
            LEFT,
...
class ProbabilityOfAGivenBoyGirlPattern(CycleThroughPatterns):
    ...
    def write_total_count(self):
        count = OldTexText("${10 \\choose 6}$", " $= 210$", "total patterns.")
        count.to_edge(UP)
        self.add(count)
    ...
    def write_example_probability(self):
        ...
        final_probability = OldTex("(0.49)^6", "(0.51)^4")
        ...
        self.wait(2)
        self.play(
            ReplacementTransform(factored, factored_in_nums, run_time=2, lag_ratio=0.5)
        )
        self.wait(2)
        for group, tex in (gp_nums, "0.49"), (bp_nums, "0.51"):
            ...
            self.play(
                ReplacementTransform(group.copy(), VGroup(VGroup(*part[:-1]))),
                Write(part[-1]),
            )
            self.wait()
...
class GeneralBinomialDistributionValues(Scene):
    ...
    def add_chart(self):
        ...
        chart = BarChart(values, bar_names=list(range(self.n + 1)))
        chart.to_edge(LEFT)
    ...
    def show_a_few_values(self):
        ...
        for i, bar in enumerate(chart.bars):
            prob = self.get_probability_expression(
                "10", str(i), "(0.49)", "(0.51)", full=False
            )
            arrow = Arrow(UP, DOWN, color=WHITE, tip_length=0.15)
    ...
    def compare_to_pascal_row(self):
        ...
        last_row = VGroup(*[triangle.coords_to_mobs[10][k] for k in range(11)])
        ...
        self.play(LaggedStartMap(FadeIn, triangle), FadeOut(self.shown_prob))
        self.play(
            last_row.set_color_by_gradient,
            BLUE,
            YELLOW,
    ...
    def mention_center_concentration(self):
        ...
        bars.target.move_to(self.chart.x_axis.point_from_proportion(0.05), DOWN)
        ...
        self.play(
            bars.restore,
            *list(
                map(FadeOut, [brace, words, self.pascals_triangle, self.ten_choose_ks])
            ),
        )
    ...
    def generalize(self):
        ...
        alt_chart = BarChart(values, bar_names=list(range(alt_n + 1)))
        ...
        self.play(FocusOn(self.full_probability.get_part_by_tex("choose")))
        self.play(
    ...
    def play_with_p_value(self):
        ...
        self.play(ShowCreation(interval), Write(triangle, run_time=1))
        self.wait()
    ...
    def get_probability_expression(self, n="n", k="k", p="p", q="(1-p)", full=True):
        args = []
        if full:
            args += ["P(", "\\# \\text{Girls}", "=", k, ")", "="]
        args += [
...
class PointOutSimplicityOfFormula(
    TeacherStudentsScene, GeneralBinomialDistributionValues
):
    def construct(self):
        ...
        self.play(prob.restore, self.teacher.change_mode, "raise_right_hand")
        self.play_student_changes(*["pondering"] * 3, look_at=prob)
        self.wait()
        self.student_says(
...
class CorrectForDependence(NameBinomial):
    ...
    def show_tendency_to_align(self):
        ...
        groups = [VGroup(*trip) for trip in zip(checkmarks, arrows, crosses)]
    ...
    def adjust_chart(self):
        ...
        alt_values /= sum(alt_values)
        arrows = VGroup()
        arrow_template = Arrow(0.5 * UP, ORIGIN, buff=0, tip_length=0.15, color=WHITE)
    ...
    def get_arrow_flip_anims(self, indices):
        checkmarks, arrows, crosses = movers = [
            VGroup(*[group[i] for i in range(len(group)) if i in indices])
        ...
        return [LaggedStartMap(MoveToTarget, mover, **kwargs) for mover in movers]


class ButWhatsTheAnswer(TeacherStudentsScene):
    def construct(self):
        self.student_says("But what's the \\\\ actual answer?", target_mode="confused")
        self.play_student_changes(*["confused"] * 3)
        self.wait()
...
class PermuteQuizQuestions(Scene):
    def construct(self):
        ...
        questions = [VGroup(*q[2:]) for q in quiz.questions]
        ...
        for m1, m2 in it.combinations(questions, 2):
            self.play(m1.move_to, m2, LEFT, m2.move_to, m1, LEFT, path_arc=np.pi)
            self.wait()
...
class AssumeOrderDoesntMatter(Scene):
    ...
    def add_title(self):
        title = OldTexText("Softer simplifying assumption: " + "Order doesn't matter")
        title.to_edge(UP)
    ...
    def show_equality(self):
                ...
                VGroup(
                    *list(
                        map(
                            ...
                            [
                                t
                                for t in it.product(*[[True, False]] * n)
                                if sum(t) == k
                            ],
                        )
                    )
        ...
        self.play(FadeIn(prob_groups[1], run_time=2, lag_ratio=0.5))
        self.wait(2)
        self.play(
            FadeIn(VGroup(prob_groups[0], *prob_groups[2:]), run_time=3, lag_ratio=0.5)
        )
        self.wait()
    ...
    def mention_correlation(self):
        ...
        question = OldTexText("But what is ", "``correlation''", "?", arg_separator="")
        question.set_color(BLUE)
        question.to_edge(UP)
        bottom = question.get_bottom()

        self.play(Write(question), assumption_group.next_to, bottom, DOWN, LARGE_BUFF)
        self.wait()
    ...
    def coming_soon(self):
        ...
        self.play(GrowFromCenter(brace), part.set_color, YELLOW)
...
class FormulaCanBeRediscovered(PointOutSimplicityOfFormula):
    def construct(self):
        ...
        self.play(Write(prob), self.teacher.change, "hesitant", prob)
        self.wait()
        self.play(GrowFromCenter(brace), Write(rediscover, run_time=1))
...
class CompareTwoSituations(PiCreatureScene):
    def construct(self):
        ...
        for screen, s in zip(screens, ["left", "right"]):
            self.play(randy.change, "raise_%s_hand" % s, screen, ShowCreation(screen))
            self.wait(3)
        self.play(randy.change, "pondering", arrow, ShowCreation(arrow))
        self.wait(2)
...
class SkepticalOfDistributions(TeacherStudentsScene):
    ...
    def show_binomial(self):
        ...
        for values in binomial.values_list:
            ...
            self.wait()
        self.student_says("Is that valid?", target_mode="sassy", index=0, run_time=1)
    ...
    def show_alternate_distributions(self):
        ...
        VGroup(poisson, poisson.title).next_to(self.students, UP, LARGE_BUFF).shift(
            RIGHT
        )
        gaussian = self.get_gaussian()
        VGroup(gaussian, gaussian.title).next_to(poisson, RIGHT, LARGE_BUFF)
        ...
        self.play_student_changes(
            *["sassy"] * 3, added_anims=[self.teacher.change, "plain"]
        )
        self.wait(2)
    ...
    def emphasize_underweighted_tails(self):
        ...
        arrow_template = Arrow(ORIGIN, UP, color=GREEN, tip_length=0.15)
        ...
        for arrows in poisson_arrows, gaussian_arrows:
            self.play(
                ShowCreation(arrows, lag_ratio=0.5, run_time=2),
                *[
    ...
    def get_binomial(self):
        ...
        dists = [get_binomial_distribution(10, p) for p in (0.2, 0.8, 0.5)]
        values_list = [list(map(dist, k_range)) for dist in dists]
        chart = BarChart(values=values_list[-1], bar_names=k_range)
    ...
    def get_poisson(self):
        k_range = list(range(11))
        L = 2
        values = [np.exp(-L) * (L**k) / (scipy.special.gamma(k + 1)) for k in k_range]
        chart = BarChart(values=values, bar_names=k_range, bar_colors=[RED, YELLOW])
        chart.set_height(self.chart_height)
        title = OldTexText(
