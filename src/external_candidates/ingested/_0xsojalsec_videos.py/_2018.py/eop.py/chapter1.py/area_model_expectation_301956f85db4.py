# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\chapter1\area_model_expectation.py
...
class IllustrateAreaModelExpectation(Scene):
    ...
    def construct(self):
        ...
        probabilities = np.array([1.0 / 8, 3.0 / 8, 3.0 / 8, 1.0 / 8])
        ...
        self.play(ReplacementTransform(flat_hist, hist))
        ...
        self.play(LaggedStartMap(FadeIn, braces), LaggedStartMap(FadeIn, p_labels))

        y_average = np.mean(y_values)
        averaged_y_values = y_average * np.ones(np.shape(y_values))

        averaged_hist = flat_hist = Histogram(
            probabilities,
            averaged_y_values,
            mode="widths",
            x_scale=x_scale,
        ...
        one_p_label = (
            OldTex(str(1)).next_to(one_brace, DOWN, buff=SMALL_BUFF).scale(0.7)
        )
