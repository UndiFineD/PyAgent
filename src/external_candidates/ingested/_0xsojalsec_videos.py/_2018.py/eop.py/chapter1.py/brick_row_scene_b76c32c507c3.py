# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\chapter1\brick_row_scene.py
...
class BrickRowScene(PiCreatureScene):
    ...
    def split_tallies(self, row, direction=DOWN):
        ...
        tally_targets_left = [
            rect.get_center() + 0.25 * rect.get_width() * LEFT for rect in row.rects
        ]

        tally_targets_right = [
            rect.get_center() + 0.25 * rect.get_width() * RIGHT for rect in row.rects
        ]
    ...
    def merge_tallies(self, row, target_pos=UP):
        ...
        if np.all(target_pos == DOWN):
            tally_targets = [rect.get_center() for rect in row.get_rects_for_level(r)]
    ...
    def move_tallies_on_top(self, row):
        self.play(self.tallies.shift, 1.2 * 0.5 * row.height * UP)
    ...
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                Succession,
                coin_seqs,
                lambda m: (FadeIn(m, run_time=0.1), MoveToTarget(m)),
                run_time=5,
                lag_ratio=0.5,
            )
        )
        ...
        v = (
            self.row.get_outcome_centers_for_level(2)[0]
            - single_flip_labels[0].get_center()
        )
        self.play(single_flip_labels.shift, v)
        ...
        v = (
            self.row.get_outcome_centers_for_level(2)[-1]
            - single_flip_labels_copy[-1].get_center()
        )
        self.play(single_flip_labels_copy.shift, v)
        ...
        outcomes = self.row.get_outcome_rects_for_level(
            2, with_labels=False, inset=True
        )
        ...
        self.play(FadeIn(tally), FadeIn(grouped_outcomes[0]))
        self.wait()
        ...
        decimal_tallies.add(tally)
        self.play(
            FadeIn(tally), FadeOut(grouped_outcomes[0]), FadeIn(grouped_outcomes[1])
        )
        self.wait()
        ...
        decimal_tallies.add(tally)
        self.play(
            FadeIn(tally), FadeOut(grouped_outcomes[1]), FadeIn(grouped_outcomes[2])
        )
        self.wait()

        self.play(FadeOut(grouped_outcomes[2]))
        self.wait()
        ...
        for decimal_tally, tally_stack in zip(decimal_tallies, self.tallies):
            anims.append(
                ApplyFunction(tally_stack.position_decimal_tally, decimal_tally)
            )
        ...
        outcomes = self.row.get_outcome_rects_for_level(2, with_labels=True, inset=True)
        subdivs = self.row.get_sequence_subdivs_for_level(2)
        self.play(FadeIn(outcomes), FadeIn(subdivs), FadeOut(self.tallies))

        self.wait()

        rect_to_dice = self.row.get_outcome_rects_for_level(
            2, with_labels=False, inset=False
        )[1]
        N = 10
        ...
        self.play(LaggedStartMap(FadeIn, all_dice), FadeOut(outcomes[1]))
        self.wait()
        ...
        right_target_pos = 0.5 * (
            self.row.get_outcome_centers_for_level(3)[2]
            + self.row.get_outcome_centers_for_level(3)[3]
        )
        right_v = right_target_pos - rect_right.get_center()
        rect_right.target.move_to(right_target_pos)

        self.play(MoveToTarget(rect_left), tally_left.move_to, left_target_pos)
        ...
        self.play(Transform(tally_left, new_tally_left))

        self.play(MoveToTarget(rect_right), tally_right.move_to, right_target_pos)
        ...
        self.play(Transform(tally_right, new_tally_right))
        ...
        outcomes = self.row.get_outcome_rects_for_level(3, with_labels=True, inset=True)
        ...
        self.play(LaggedStartMap(ShowCreation, braces), LaggedStartMap(Write, probs))
        self.wait()
        self.play(
            LaggedStartMap(
                FadeOut,
        ...
        self.play(
            ...
            FadeOut(self.tallies, run_time=run_time),
        )
        self.wait()
        self.play(
            LaggedStartMap(
            ...
            FadeIn(self.tallies, run_time=run_time),
        )
        ...
        # show individual outcomes
        outcomes = previous_row.get_outcome_rects_for_level(
            n, with_labels=False, inset=True
        )
        ...
        target_outcomes = self.row.get_outcome_rects_for_level(
            n + 1, with_labels=False, inset=True
        )
        ...
        new_tallies = VGroup(
            *[
                TallyStack(n + 1 - i, i).move_to(rect)
        ...
        self.play(Transform(grouped_outcomes[k], original_grouped_outcomes[k]))

        new_rects = self.row.get_rects_for_level(n + 1)

        self.play(
            Transform(
                grouped_outcomes[k][0],
                grouped_target_outcomes[k][0][old_tally_sizes[k - 1] :],
            ),
            Transform(
                grouped_outcomes_copy[k - 1][0],
                grouped_target_outcomes[k][0][: old_tally_sizes[k - 1]],
            ),
        )
...
class ShowOutcomesInBrickRow4(BrickRowScene):
    ...
    def construct(self):
        ...
        self.wait()
        previous_outcomes = previous_row.get_outcome_rects_for_level(
            3, with_labels=True, inset=True
        )
        ...
        new_outcomes = self.row.get_outcome_rects_for_level(
            4, with_labels=True, inset=True
        )
        ...
        run_time = 0.5
        self.play(
            Transform(previous_outcomes[0], new_outcomes_left[0], run_time=run_time)
        )
        self.play(
            Transform(previous_outcomes[1:4], new_outcomes_left[1:4], run_time=run_time)
        )
        self.play(
            Transform(previous_outcomes[4:7], new_outcomes_left[4:7], run_time=run_time)
        )
        self.play(
            Transform(previous_outcomes[7], new_outcomes_left[7], run_time=run_time)
        )
        ...
        self.play(
            Transform(
                previous_outcomes_copy[0], new_outcomes_right[0], run_time=run_time
            )
        )
        self.play(
            Transform(
                previous_outcomes_copy[1:4], new_outcomes_right[1:4], run_time=run_time
            )
        )
        self.play(
            Transform(
                previous_outcomes_copy[4:7], new_outcomes_right[4:7], run_time=run_time
            )
        )
        self.play(
            Transform(
                previous_outcomes_copy[7], new_outcomes_right[7], run_time=run_time
            )
        )
...
class SplitTalliesIntoBrickRow4(BrickRowScene):
    ...
    def construct(self):
        ...
        anims = []
        for tally, rect in zip(
            moving_tallies_left[1:], hypothetical_new_row.rects[1:-1]
        ):
            anims.append(tally.move_to)
            anims.append(rect)
        for tally, rect in zip(
            moving_tallies_right[:-1], hypothetical_new_row.rects[1:-1]
        ):
            anims.append(tally.move_to)
            anims.append(rect)
        self.play(*anims)
