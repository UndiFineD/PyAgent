# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\chapter1\morph_brick_row_into_histogram.py
...
class GenericMorphBrickRowIntoHistogram(Scene):
    ...
    def construct(self):
        ...
        self.bar_anchors = [
            self.bar_anchor_height * UP
            + self.row.height * (i - 0.5 * self.level) * RIGHT
        ...
        self.wait()
        self.play(
            self.row.rects.space_out_submobjects, {"factor": 1.3}, FadeOut(tallies)
        )
        self.wait()
        anims = []
        ...
        anims = [ApplyMethod(rect.set_stroke, {"width": 0}) for rect in self.bars]
...
class MorphBrickRowIntoHistogram3(GenericMorphBrickRowIntoHistogram):
    ...
    def construct(self):
        ...
        for i in range(0, self.prob_denominator + 1):
            ...
            y_guide.move_to(
                self.bar_anchor_height * UP
                + i * float(self.row.width) / self.prob_denominator * UP
            )
            y_guide_label = OldTex(
                "{" + str(i) + "\over " + str(self.prob_denominator) + "}", color=GREY
            )
        ...
        self.play(FadeIn(y_guides), Animation(self.bars.outline), Animation(self.bars))
        self.wait()

        self.play(FadeIn(x_axis), FadeIn(x_labels), FadeIn(nb_tails_label))
        ...
        for i, rect in enumerate(covering_rects):
            ...
            self.play(
                ...
                ChangeDecimalToValue(
                    area_decimal, cumulative_areas[i], rate_func=linear
                ),
            )
            self.wait(0.2)
        ...
        total_area_rect = SurroundingRectangle(
            total_area_group, buff=MED_SMALL_BUFF, stroke_color=area_color
        )

        self.play(FadeOut(covering_rects), ShowCreation(total_area_rect))
        self.wait()
...
class MorphBrickRowIntoHistogram20(GenericMorphBrickRowIntoHistogram):
    ...
    def construct(self):
        ...
        for guide, i, h in zip(y_guides, prob_grid, y_guide_heights):
            new_y_guide_height = self.bar_anchor_height + i * self.scale_y * float(
                self.row.width
            )
...
class MorphBrickRowIntoHistogram500(MorphBrickRowIntoHistogram20):
    CONFIG = {"level": 500, "x_ticks": 100, "prob_ticks": 0.01}
