...
class QuizResult(PiCreatureScene):
    ...
    def construct(self):
        ...
        all_students = PiCreatureClass(
            width=nb_students_x, height=nb_students_y
        )  # Defunct()
        student_points = []
        grades = []
        grades_count = []
        ...
        self.play(
            ReplacementTransform(
                VGroup(master_quiz), all_quizzes, run_time=2, lag_ratio=0.5
            )
        )
        self.wait(2)
        ...
        for label, percentage in zip(grade_hist.y_labels_group, percentages):
            new_label = DecimalNumber(
                percentage, num_decimal_places=1, unit="\%", color=highlight_color
            )
            new_label.scale(0.7)
