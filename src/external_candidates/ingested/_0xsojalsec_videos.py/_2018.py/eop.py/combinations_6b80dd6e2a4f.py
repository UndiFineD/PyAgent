# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\combinations.py
...
class ExperienceProblemSolver(PiCreatureScene):
    ...
    def no_genius(self):
        ...
        self.play(
            ShowCreation(cross), jenny.change, "sassy", cross, randy.change, "happy"
        )
        self.wait(2)
...
class InitialFiveChooseThreeExample(Scene):
    ...
    def show_all_stacks(self):
        stacks = get_stacks(
            self.get_obj1(), self.get_obj2(), self.n, vertical_buff=SMALL_BUFF
        )
...
class SixChooseThreeExample(InitialFiveChooseThreeExample):
    ...
    def show_stack(self):
        ...
        equation = OldTex(
            "{%d \\choose %d}" % (self.n, self.k), "=", str(choose(self.n, self.k))
        )
        equation.set_color(YELLOW)
    ...
    def talk_through_one_line(self):
        ...
        arrows = VGroup(
            *[Vector(0.5 * UP).next_to(one, DOWN, SMALL_BUFF) for one in line.ones]
        )
        ...
        self.wait(2)
        self.play(
            line.restore, *list(map(FadeOut, [brace, n_options, arrows, choose_k]))
        )
...
class ProbabilityOfKWomenInGroupOfFive(Scene):
    ...
    def ask_question(self):
        ...
        prob_words = VGroup(
            *[
                OldTexText("Probability of", str(n), "women?").set_color_by_tex(
                    str(n), YELLOW
                )
    ...
    def show_all_possibilities(self):
        ...
        two_anims = [
            ReplacementTransform(VectorizedPoint(twos[0].get_center()), twos[0])
        ] + [ReplacementTransform(t1.copy(), t2) for t1, t2 in zip(twos, twos[1:])]
    ...
    def show_answer_to_question(self):
        ...
        n_possibilities_part_to_fade = VGroup(
            self.n_possibilities[-2], *self.n_possibilities[:-3]
        )
    ...
    def ask_about_pattern(self):
        ...
        numbers = self.numbers
        circles = VGroup(
            *[Circle().replace(num, dim_to_match=1).scale(1.5) for num in numbers]
        )
...
class BuildFiveFromFour(ProbabilityOfKWomenInGroupOfFive):
    ...
    def show_all_configurations_of_four(self):
        ...
        n_possibilities = OldTex(
            "2 \\cdot", "2 \\cdot", "2 \\cdot", "2", "\\text{ Possibilities}"
        )
    ...
    def combine_stacks(self):
        ...
        self.play(
            VGroup(top_stacks[-1], top_stacks.numbers[-1]).align_to, bottom_stacks, DOWN
        )
        self.wait()
...
class IntroducePascalsTriangle(Scene):
    ...
    def show_9_choose_4_pattern(self):
        ...
        show_random_lines(6)
        self.play(
            self.nine_choose_four_term.scale,
            1.0 / 1.2,
            self.nine_choose_four_term.set_color,
            WHITE,
            *list(
                map(
                    FadeOut,
                    [expr, nine_choose_four_term, over_512, eq_result, self.curr_line],
                )
            ),
        )
...
class StacksApproachBellCurve(Scene):
    ...
    def construct(self):
        ...
        for x in range(self.n_iterations):
            ...
            self.play(
                *list(map(MoveToTarget, [bars, bars_copy, numbers, numbers_copy]))
            )
            self.play(
                bars.shift,
                distance * LEFT / 2,
            ...
            for i in range(len(bars) - 1):
                ...
                if new_num.get_width() > top_bar.get_width():
                    min_scale_val = min(
                        min_scale_val, top_bar.get_width() / new_num.get_width()
                    )
...
class ChooseThreeFromFive(InitialFiveChooseThreeExample, PiCreatureScene):
    ...
    def show_binary_strings(self):
        ...
        for i, line in enumerate(stack):
            ...
            self.remove(num)
        self.play(
            Write(VGroup(*equation[:-1])), ReplacementTransform(num, equation[-1])
        )
        self.wait()
    ...
    def add_people(self):
        ...
        self.play(
            ...
            VGroup(self.stack, self.equation).to_edge,
            RIGHT,
            LARGE_BUFF,
        )
    ...
    def show_association_with_binary(self, index):
        ...
        self.wait()
        self.play(
            ReplacementTransform(line_rect, full_line_rect), triplet.set_color, YELLOW
        )
        self.wait(2)
        self.play(
            people.restore,
    ...
    def pattern_is_unambiguous(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, all_ones, lag_ratio=0.2, run_time=3, rate_func=there_and_back
            ),
            LaggedStartMap(
                ApplyMethod,
                self.people,
    ...
    def get_names(self, people):
        names = VGroup(
            *[OldTexText(name + ",") for name in ("Ali", "Ben", "Cam", "Denis", "Evan")]
        )
...
class SubsetProbabilityExample(ChooseThreeFromFive):
    ...
    def ask_question(self):
        ...
        words = OldTexText(
            "Choose 3 people randomly.\\\\", "Probability", "Ali", "is one of them?"
        )
...
class StudentsGetConfused(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            self.get_shuffle_anim(line),
            *[ApplyMethod(pi.change, "confused", line) for pi in self.pi_creatures],
        )
        for x in range(4):
...
class HowToComputeNChooseK(ChooseThreeFromFive):
    ...
    def show_permutations_of_ABC(self):
        ...
        for indices in list(it.permutations(list(range(3))))[1:n_perms]:
            self.play(
                *[
                    ApplyMethod(name.next_to, lines[i], UP, SMALL_BUFF, path_arc=np.pi)
    ...
    def count_permutations_of_ABC(self):
        ...
        for line, name, count, arrow in zip(
            lines, shuffled_names, choice_counts, arrows
        ):
    ...
    def get_choice_counts(self, n, k):
        ...
        choice_counts = VGroup(
            *[
                OldTexText("(", str(n0), " choices", ")", arg_separator="")
...
class NineChooseFourExample(HowToComputeNChooseK):
    ...
    def show_n_choose_k_pattern(self):
        n, k = self.n, self.k
        stack = get_stack(
            OldTex("1").set_color(PINK), OldTex("0").set_color(BLUE), n, k
        )
        l = len(stack)
        n_stacks = 6
        columns = VGroup(
    ...
    def choose_k_people(self):
        ...
        self.play(
            FadeIn(choose_k),
            LaggedStartMap(
                ApplyMethod, chosen_subset, lambda m: (m.shift, MED_LARGE_BUFF * DOWN)
            ),
        )
        self.wait()
        self.play(
...
class SumsToPowerOf2(Scene):
    ...
    def count_32(self):
        ...
        numbers.target = VGroup(
            *[OldTex("{%d \\choose %d}" % (self.n, k)) for k in range(self.n + 1)]
        )
    ...
    def show_alternate_sum(self):
        ...
        sum_group.next_to(
            self.n_choose_k_terms, DOWN, aligned_edge=LEFT, buff=LARGE_BUFF
        )
