# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\pascal.py
...
def graded_square(n, k):
    return Square(
        side_length=1, fill_color=graded_color(n, k), fill_opacity=1, stroke_width=1
    )


def graded_binomial(n, k):
    return Integer(choose(n, k), color=graded_color(n, k))
...
class BuildNewPascalRow(Transform):
    ...
    def __init__(self, mobject, duplicate_row=None, **kwargs):
        if (
            mobject.__class__ != GeneralizedPascalsTriangle
            and mobject.__class__ != PascalsTriangle
        ):
            raise (
        ...
        new_row_left_copy = VGroup(
            *[new_pt.coords_to_mobs[n + 1][k] for k in range(0, n + 1)]
        )

        new_row_right_copy = VGroup(
            *[new_pt.coords_to_mobs[n + 1][k] for k in range(1, n + 2)]
        ).copy()
