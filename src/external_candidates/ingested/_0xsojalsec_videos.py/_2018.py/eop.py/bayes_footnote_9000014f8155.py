# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\bayes_footnote.py
...
class Introduction(TeacherStudentsScene):
    ...
    def put_it_first(self):
        ...
        bayes_to_intuition = VGroup(
            *list(map(TexText, ["Bayes", "$\\leftrightarrow$", "Intuition"]))
        )
    ...
    def swap_example_order(self):
        ...
        self.play(
            ...
            disease_group.shift,
            LEFT,
            intuitive_examples.scale,
            0.7,
            intuitive_examples.to_corner,
            UP + LEFT,
...
class OneInOneThousandHaveDisease(Scene):
    def construct(self):
        ...
        all_creatures = VGroup(
            *[
                VGroup(*[creature.copy() for y in range(25)]).arrange(DOWN, SMALL_BUFF)
                for x in range(40)
            ]
...
class RephraseQuestion(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(FadeIn, posterior), ShowCreation(posterior_arrow), run_time=1
        )
        self.wait(2)
...
class ShowRestrictedSpace(Scene):
    ...
    def show_accurate_positive_result(self):
        equation = OldTex(
            "P(", "\\text{Test positive }", "|", "\\text{ sick}", ")", "=", "100\\%"
        )
    ...
    def show_false_positive_conditional(self):
        equation = OldTex(
            "P(", "\\text{Test positive }", "|", "\\text{ healthy}", ")", "=", "1\\%"
        )
        equation.set_color_by_tex("positive", YELLOW)
    ...
    def fade_out_negative_result_individuals(self):
        ...
        words.to_edge(UP)
        arrows = [
            Arrow(words.get_bottom(), movers.target[i].get_top()) for i in (0, -1)
        ]
        ...
        self.play(
            ...
            self.false_positives_words.next_to,
            brace,
            DOWN,
        )
    ...
    def get_all_creatures(self):
        ...
        all_creatures = VGroup(
            *[
                VGroup(*[creature.copy() for y in range(self.n_rows)]).arrange(
                    DOWN, SMALL_BUFF
                )
...
class DepressingForMedicalTestDesigners(TestScene):
    ...
    def show_99_percent_accuracy(self):
        ...
        checks = VGroup(
            *[
                VGroup(
                    *[OldTex("\\checkmark").set_color(GREEN) for y in range(10)]
                ).arrange(DOWN)
                for x in range(10)
            ]
        ).arrange(RIGHT)
...
class HowMuchCanYouChangeThisPrior(ShowRestrictedSpace, PiCreatureScene):
    ...
    def show_subgroups(self):
        ...
        rects = VGroup(
            *[SurroundingRectangle(group, buff=0, color=GREEN) for group in subgroups]
        )
...
class ShowTheFormula(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play(
            initial_formula.move_to,
            initial_formula.saved_state,
            FadeIn(VGroup(*number_fraction[:3])),
        )
        self.wait(2)
        self.play(
            LaggedStartMap(
                FadeIn, VGroup(*number_fraction[3:]), run_time=3, lag_ratio=0.7
            )
        )
        self.wait(2)
...
class SourceOfConfusion(Scene):
    ...
    def ask_question(self):
        ...
        question_arrow = Arrow(
            question.get_bottom(), self.bayes_rule_words.get_top(), color=WHITE
        )
    ...
    def shift_arrow(self):
        new_arrow = Arrow(
            self.question.get_bottom(), self.progression[0].get_top(), color=WHITE
        )
...
class StatisticsVsEmpathy(PiCreatureScene):
    def construct(self):
        ...
        self.play(
            PiCreatureSays(
                morty, "1 in 1{,}000 people \\\\ have this disease.", look_at=randy.eyes
            )
        )
...
class IntroduceTelepathyExample(StatisticsVsEmpathy):
    ...
    def claim_mind_reading_powers(self):
        ...
        self.wait()
        self.play(
            RemovePiCreatureBubble(randy, target_mode="happy", look_at=morty.eyes)
        )
    ...
    def guess_number_correctly(self):
        ...
        self.read_mind(randy, morty)
        self.play(
            PiCreatureSays(randy, number_copy, target_mode="hooray", look_at=morty.eyes)
        )
        self.wait()
