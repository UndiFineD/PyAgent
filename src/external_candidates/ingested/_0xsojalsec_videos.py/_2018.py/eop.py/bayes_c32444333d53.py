# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\eop\bayes.py
...
class IntroducePokerHand(PiCreatureScene, SampleSpaceScene):
    ...
    def indicate_straight(self):
        ...
        straight_cards = VGroup(
            *it.chain(you.hand.target, [c.target for c in selected_community_cards])
        )
        ...
        self.wait(2)
        self.play(
            selected_community_cards.restore, you.hand.restore, you.change_mode, "happy"
        )
        self.wait()
    ...
    def compute_flush_probability(self):
        ...
        num_hearts_arrow = Arrow(
            num_hearts.get_bottom(), ten.get_right(), color=RED, buff=SMALL_BUFF
        )
    ...
    def place_high_bet(self):
        ...
        pre_money = VGroup(
            *[
                VGroup(*[OldTex("\\$") for x in range(10)]).arrange(
    ...
    def name_bayes_rule(self):
        ...
        words = OldTexText(
            "Maybe she really \\\\ does have a flush $\\dots$", alignment=""
        )
        words.scale(0.7)
...
class HowDoesPokerWork(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "Wait, how does \\\\ poker work again?", target_mode="confused", run_time=1
        )
...
class UpdatePokerPrior(SampleSpaceScene):
    ...
    def ask_where_conditionals_come_from(self):
        ...
        arrows = VGroup(
            *[
                Arrow(numbers_word.get_left(), num.get_right(), buff=2 * SMALL_BUFF)
                for num in numbers
            ]
        )
    ...
    def write_P_flush_given_bet(self):
        posterior_tex = OldTex(
            "P(", self.double_heart_template, "|", self.cash_string, ")"
        )
        ...
        self.play(
            *it.chain(
                list(map(Write, [frac_line, plus])), list(map(MoveToTarget, rects[1:]))
            )
        )
        self.wait(3)
    ...
    def compare_prior_to_posterior(self):
        ...
        prior_rect, post_rect = [
            SurroundingRectangle(tex, stroke_width=2) for tex in [prior_tex, post_tex]
        ]
    ...
    def get_prior_labels(self, value):
        ...
        q_str = "%0.3f" % (1 - value)
        labels = [
            OldTex("P(", s, self.double_heart_template, ")", "= ", num)
    ...
    def show_propogation(self, node):
        ...
        while curr_nodes:
            ...
            for node in curr_nodes:
                ...
                connected_nodes = [
                    n
                    for n in it.chain(node.children, node.parents)
                ...
                for next_node in connected_nodes:
                    ...
                    anims.append(
                        Transform(node.fill.copy(), next_node.fill, remover=True)
                    )
...
class GeneralizeBayesRule(SampleSpaceScene):
    ...
    def add_sample_space(self):
        sample_space = SampleSpace(
            full_space_config={"height": 3, "width": 3, "fill_opacity": 0}
        )
    ...
    def talk_through_terms(self):
        ...
        posterior_arrow = Arrow(
            posterior_word.get_top(), posterior.get_bottom(), tip_length=0.15
        )
...
class MusicExample(SampleSpaceScene, PiCreatureScene):
    ...
    def friend_gives_compliment(self):
        ...
        self.play(
            ...
            LaggedStartMap(
                ApplyMethod, VGroup(*friends[1:]), lambda pi: (pi.change_mode, "happy")
            ),
        )
        self.wait(2)
    ...
    def restrict_space(self):
        positive_space, negative_space = [
            VGroup(*[self.sample_space[i][j] for i in range(2)]) for j in range(2)
        ]
    ...
    def intuition_of_positive_feedback(self):
        ...
        self.play(
            *list(
                map(FadeOut, [prior_rect, post_rect, self.ratio_group, self.post_rhs])
            )
        )
    ...
    def make_friends_honest(self):
        ...
        for value in 0.5, 0.1, 0.9:
            ...
            self.play(
                *self.get_top_conditional_change_anims(
                    value,
                    post_rects,
                    new_label_kwargs={"labels": [label]},
                ),
