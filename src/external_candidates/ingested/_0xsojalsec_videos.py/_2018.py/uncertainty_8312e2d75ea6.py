# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\uncertainty.py
# -*- coding: utf-8 -*-
...
class MentionUncertaintyPrinciple(TeacherStudentsScene):
    def construct(self):
        ...
        dot_brace_anim = get_brace_text_group_update(
            dot_cloud.gaussian_distribution_wrapper, DOWN, "position", dot_cloud.color
        )
...
class FourierTradeoff(Scene):
    ...
    def construct(self):
        ...
        def get_wave_packet_function():
            ...
            return (
                lambda t: (factor**0.25)
                * np.cos(4 * TAU * t)
                * np.exp(-factor * (t - time_mean) ** 2)
            )
...
class ShowPlan(PiCreatureScene):
    ...
    def play_doppler_anims(self, word):
        ...
        self.play(
            ...
            UpdateFromAlphaFunc(target, lambda m, a: m.set_fill(opacity=a)),
            morty.change,
            "pondering",
            run_time=1,
        )
        ...
        self.play(
            Write(checkmark),
            UpdateFromAlphaFunc(target, lambda m, a: m.set_fill(opacity=1 - a)),
            FadeOut(radar_dish),
            morty.change,
            "happy",
        )
...
class TwoCarsAtRedLight(Scene):
    ...
    def show_low_confidence(self):
        ...
        frequency_axes.add(
            frequency_label, VectorizedPoint(frequency_axes.y_axis.get_top())
        )
    ...
    def flash_in_sync_long_time(self):
        ...
        self.play(
            ...
            *[self.get_flashes(car, num_flashes=n_spikes) for car in self.cars],
        )
    ...
    def show_high_confidence(self):
        ...
        self.play(
            FadeIn(label), GrowArrow(arrow), *list(map(self.get_flashes, self.cars))
        )
        self.play(
            GrowFromCenter(brace),
...
class VariousMusicalNotes(Scene):
    def construct(self):
        ...
        short_signal_words = OldTexText(
            "Short", "signal", "correlates", "with", "wide range", "of frequencies"
        )
        ...
        self.play(
            ShowCreation(long_graph), *list(map(FadeOut, [last_brace, last_phrase]))
        )
...
class FourierRecapScene(DrawFrequencyPlot):
    ...
    def match_winding_to_beat_frequency(self):
        ...
        self.play(
            *self.get_vector_animations(self.time_graph, draw_polarized_graph=False),
            run_time=10,
        )
        self.wait()
...
class LongAndShortSignalsInWindingMachine(FourierRecapScene):
    ...
    def note_fourier_peak(self, fourier_graph, freq1, freq2):
        ...
        self.wait()
        self.change_frequency(
            freq2, added_anims=[MaintainPositionRelativeTo(arrow, dot)], run_time=3
        )
        self.wait()
...
class MentionDopplerRadar(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                VGroup(plane, dish), lambda m, a: m.set_fill(opacity=1 - a)
            )
        )
...
class IntroduceDopplerRadar(Scene):
    ...
    def measure_distance_with_time(self):
        ...
        pulse_graph, echo_graph, sum_graph = self.get_pulse_and_echo_graphs(
            self.get_single_pulse_graph, (1,), (1 + time_diff,)
        )
        ...
        double_arrow = DoubleArrow(
            pulse_graph.peak_point, echo_graph.peak_point, color=WHITE
        )
        ...
        graph_draw = turn_animation_into_updater(
            ShowCreation(sum_graph, rate_func=linear, run_time=0.97 * axes.x_max)
        )
    ...
    def show_frequency_shift(self):
        ...
        pulse_graph, echo_graph, sum_graph = graphs = self.get_pulse_and_echo_graphs(
            self.get_frequency_pulse_graph, (1, 25), (1 + time_diff, 50)
        )
        for graph in graphs:
    ...
    def show_frequency_shift_in_fourier(self):
        ...
        self.play(
            ReplacementTransform(pulse_label[0].copy(), original_fourier_brace),
        ...
        self.play(
            ...
            FadeIn(sum_graph),
            ReplacementTransform(
                pulse_graph.fourier_transform, sum_graph.fourier_transform
            ),
        )
    ...
    def get_frequency_pulse_graph(self, x, freq=50, **kwargs):
        return self.get_graph(
            self.get_frequency_pulse_function(x, freq), num_graph_points=700, **kwargs
        )
...
class MentionPRFNuance(TeacherStudentsScene):
    def construct(self):
        ...
        displayed_doppler_shift = OldTexText(
        ...
        self.play(
            Animation(pulse_graph), self.teacher.change, "raise_right_hand", run_time=1
        )
        self.play(
        ...
        self.play(
            ...
            UpdateFromFunc(
                brace,
                lambda b: b.match_width(peak_points, stretch=True).next_to(
                    peak_points, UP, SMALL_BUFF
                ),
            ),
            Transform(
        ...
        everything = VGroup(
            title, axes, pulse_graph, shift_graph, brace, displayed_doppler_shift
        )
...
class AmbiguityInLongEchos(IntroduceDopplerRadar, PiCreatureScene):
    ...
    def introduce_multiple_objects(self):
        ...
        self.play(
            Transform(
                sum_graph, noisy_graph, rate_func=lambda t: wiggle(t, 4), run_time=3
            )
        )
        self.wait(2)
    ...
    def fourier_transform_of_one_pulse(self):
        frequency_axes = Axes(
            ...
            y_min=-0.01,
            y_max=0.01,
            y_axis_config={"unit_size": 110, "tick_frequency": 0.006},
        )
        frequency_label = OldTexText("Frequency")
        ...
        self.play(
            ApplyMethod(
                VGroup(self.axes, first_echo_graph).to_edge,
                UP,
                {"buff": SMALL_BUFF},
                rate_func=squish_rate_func(smooth, 0.5, 1),
            ),
            ...
            LaggedStartMap(
                FadeOut, VGroup(self.curr_graph, self.dish, self.pi_creature)
            ),
            run_time=2,
        )
    ...
    def overlapping_frequenies_of_various_objects(self):
        ...
        self.play(
            ReplacementTransform(
                VGroup(fourier_graph), shifted_graphs, lag_ratio=0.5, run_time=2
            )
        )
        self.wait()
        self.play(
    ...
    def get_pulses(self):
        return [
            self.get_pulse(self.dish.copy().shift(0.01 * obj.get_center()[0]), obj)
...
class SummarizeFourierTradeoffForDoppler(Scene):
    def construct(self):
        ...
        update_bottom_graphs = Mobject.add_updater(
            bottom_graphs, lambda g: Transform(g, get_bottom_graphs()).update(1)
        )
...
class IntroduceDeBroglie(Scene):
    ...
    def construct(self):
        ...
        update_words(words)
        self.play(
            FadeIn(time_line), GrowArrow(arrow), Write(words), Write(date), run_time=1
        )
        self.wait()
        self.play(
            arrow.next_to,
        ...
        self.play(
            ...
            VGroup(arrow, words, date).shift,
            MED_LARGE_BUFF * UP,
            *[
                ApplyFunction(
                    lambda m: m.rotate(TAU / 4).set_stroke(width=0), mob, remover=True
                )
        ...
        for x in range(3):
            ...
            self.play(
                ApplyMethod(
                    particle.move_to, axes.coords_to_point(22, 0), rate_func=linear
                ),
    ...
    def get_wave(
        self, axes, particle, complex_to_real_func=lambda z: z.real, freq=None, **kwargs
    ):
        ...
        def func(x):
            dispersion = fdiv(1.0, self.dispersion_factor) * (
                np.sqrt(1.0 / (1 + t0**2))
            )
    ...
    def get_wave_pair(self, axes, particle, colors=None, **kwargs):
        ...
        return VGroup(
            *[
                self.get_wave(axes, particle, C_to_R, color=color, **kwargs)
                for C_to_R, color in zip([lambda z: z.imag, lambda z: z.real], colors)
            ]
        )
...
class ShowMomentumFormula(IntroduceDeBroglie, TeacherStudentsScene):
    ...
    def introduce_formula(self):
        ...
        xi_arrow = Arrow(
            xi_words.get_bottom(), xi.get_corner(UP + RIGHT), buff=SMALL_BUFF
        )
        ...
        v_lines.arrange(
            RIGHT, buff=float(axes.x_axis.unit_size) / self.default_wave_frequency
        )
...
class SortOfDopplerEffect(PiCreatureScene):
    ...
    def construct(self):
        ...
        def get_wave():
            ...
            return VGroup(
                ...
                *[
                    ...
                    for x in np.arange(
                        -4 * FRAME_X_RADIUS, FRAME_X_RADIUS, self.arrow_spacing
                    )
                ],
            )
            return
        ...
        self.play(
            UpdateFromAlphaFunc(
                screen_rect,
                lambda m, a: m.move_to(interpolate(start_center, rect.get_center(), a)),
            ),
            k_tracker.shift,
            2 * RIGHT,
        )
        self.play(
            MaintainPositionRelativeTo(screen_rect, rect, run_time=4),
        )
        self.play(
            screen_rect.move_to,
...
class HangingWeightsScene(MovingCameraScene):
    ...
    def show_analogy_with_electron(self):
        words = OldTexText(
            "Analogous to the energy of a particle \\\\", "(in the sense of $E=mc^2$)"
        )
        words.move_to(DOWN)
    ...
    def metaphor_for_something(self):
        ...
        self.wait(7)
        self.play(
            FadeOut(words), Animation(einstein.backdrop, remover=True), FadeIn(einstein)
        )
        self.wait(2)
...
class FourierTransformOfWaveFunction(Scene):
    ...
    def show_wave_packet(self):
        axes = Axes(
            x_min=0, x_max=12, y_min=-1, y_max=1, y_axis_config={"tick_frequency": 0.5}
        )
    ...
    def take_fourier_transform(self):
        ...
        self.fourier_graph_update_animation = UpdateFromFunc(
            fourier_graph,
            lambda m: Transform(m, get_wave_function_fourier_graph()).update(1),
        )
    ...
    def show_tradeoff(self):
        for a in 5, 0.1, 0.01, 10, 0.5:
            self.play(
                ApplyMethod(self.a_tracker.set_value, a, run_time=2),
                self.wave_update_animation,
                self.fourier_graph_update_animation,
            )
            self.wait()
...
class MusicalNote(AddingPureFrequencies):
    def construct(self):
        ...
        v_line_update = UpdateFromFunc(
            v_line,
            lambda l: l.put_start_and_end_on_with_projection(
                graph.get_points()[-1],
...
class ProbabalisticDetection(FourierTransformOfWaveFunction):
    ...
    def detect_only_single_points(self):
        ...
        gdw_anim = Mobject.add_updater(
            gdw,
            lambda m: m.set_width(2.0 / (self.a_tracker.get_value() ** (0.5))).move_to(
                rect
            ),
        )
...
class OneLevelDeeper(Scene):
    def construct(self):
        ...
        randomness = ProbabalisticMobjectCloud(
            OldTexText("Randomness"), n_copies=5, time_per_change=0.05
        )
...
class BetterTranslation(TeacherStudentsScene):
    def construct(self):
        ...
        translation_group.next_to(
            german_word, UP, submobject_to_align=translation_arrow
        )
        self.play(
            group.next_to,
            translation_arrow,
...
class ThinkOfHeisenbergUncertainty(PiCreatureScene):
    def construct(self):
        ...
        position, momentum, time, frequency = list(
            map(TexText, ["Position", "Momentum", "Time", "Frequency"])
        )
