# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2018\borsuk_addition.py
...
class Introduction(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.play_student_changes(
            "pondering", "sad", "concerned_musician", look_at=10 * LEFT + 2 * DOWN
        )
...
class StudentsWatching(TeacherStudentsScene):
    def construct(self):
        self.play(
            self.teacher.change,
            "raise_right_hand",
            self.change_students(*3 * ["thinking"], look_at=self.screen),
...
class MinimizeSharding(Scene):
    def construct(self):
        piece_groups = VGroup(
            *[
                VGroup(*[self.get_piece() for x in range(3)]).arrange(
...
class TopologyWordBreak(Scene):
    def construct(self):
        ...
        genus_labels = VGroup(
            *[
                OldTexText("Genus %d:" % d)
                .scale(1.5)
                .next_to(classes[d], LEFT, MED_LARGE_BUFF)
                for d in range(3)
            ]
        )
        ...
        self.play(
            LaggedStartMap(
                ApplyMethod, word, lambda m: (m.set_color, m.target_color), run_time=1
            )
        )
        self.play(
...
class FunctionGInSymbols(Scene):
    def construct(self):
        ...
        self.play(
            ...
            FadeIn(eq2),
            FadeIn(neg_g_of_p),
            VGroup(seeking_text, g_equals_zero).shift,
            1.5 * DOWN,
        )
        self.wait()
...
class FunctionGInputSpace(SpecialThreeDScene):
    ...
    def show_antipodal_point(self):
        ...
        end_dot = always_redraw(
            lambda: self.get_dot(path[-1].point_from_proportion(1)).set_color(RED)
        )
    ...
    def get_start_path(self):
        path = ParametricCurve(
            lambda t: np.array(
                [-np.sin(TAU * t + TAU / 4), np.cos(2 * TAU * t + TAU / 4), 0]
            ),
            color=RED,
        )
        path.scale(0.5)
    ...
    def draw_path(
        self, path, run_time=4, dot_follow=True, uncreate=False, added_anims=None
    ):
        ...
        if dot_follow:
            anims.append(
                UpdateFromFunc(
                    point_mob, lambda p: p.move_to(path.point_from_proportion(1))
                )
            )
...
class FunctionGOutputSpace(FunctionGInputSpace):
    ...
    def setup(self):
        axes = self.axes = Axes(
            x_min=-2.5, x_max=2.5, y_min=-2.5, y_max=2.5, axis_config={"unit_size": 1.5}
        )
        for axis in axes:
    ...
    def draw_path(
        ...
        if dot_follow:
            anims.append(
                UpdateFromFunc(
                    point_mob, lambda p: p.move_to(shadow_path.point_from_proportion(1))
                )
            )
    ...
    def show_antipodal_point(self):
        ...
        pre_path = VMobject().set_points_smoothly(
            [ORIGIN, DOWN, DOWN + 2 * RIGHT, 3 * RIGHT + 0.5 * UP, 0.5 * RIGHT, ORIGIN]
        )
        ...
        self.play(
            ...
            ReplacementTransform(gp_label.deepcopy().clear_updaters(), g_neg_p),
        )
        self.wait()
    ...
    def show_equator(self):
        ...
        # To address
        self.play(
            point_mob.move_to, equator_start, dot.move_to, self.odd_func(equator_start)
        )
    ...
    def deform_towards_north_pole(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                equator,
                lambda m, a: m.become(self.get_lat_line(a * PI / 2)).set_stroke(
                    width=0
                ),
                run_time=16,
            ),
            UpdateFromFunc(
                ...
                lambda m: m.become(
                    equator.deepcopy()
                    .apply_function(self.odd_func)
                    .set_stroke(color=RED, width=3)
                ),
            ),
        )
        self.wait()
