...
class SunAnimation(Group):
    ...
    def update(self, dt):
        ...
        self.rotated_sun.pixel_array = np.array(
            a * self.sun.pixel_array, dtype=self.sun.pixel_array.dtype
        )
...
class ShowEmergingEllipse(Scene):
    ...
    def get_circle(self):
        circle = self.circle = Circle(
            radius=self.circle_radius, color=self.circle_color
        )
        return circle
    ...
    def get_ghost_lines(self, lines):
        return lines.copy().set_stroke(
            color=self.ghost_lines_stroke_color, width=self.ghost_lines_stroke_width
        )
...
class ShowFullStory(Scene):
    def construct(self):
        directory = os.path.join(
            MEDIA_DIR, "animations/active_projects/lost_lecture/images"
        )
        scene_names = [
            "ShowEmergingEllipse",
            "ShowFullStory",
...
class TheMotionOfPlanets(Scene):
    ...
    def get_planets_ellipses_and_orbits(self, sun):
        ...
        ellipses = VGroup(
            *[Circle(radius=r, color=WHITE, stroke_width=1) for r in orbit_radii]
        )
...
class AskAboutEllipses(TheMotionOfPlanets):
    ...
    def add_force_labels(self):
        ...
        def update_radial_measurement(measurement):
            ...
            new_decimal = DecimalNumber(
                radial_line.get_length(), **radius_measurement_kwargs
            )
        ...
        def v_fade_in(mobject):
            return UpdateFromAlphaFunc(
                mobject, lambda mob, alpha: mob.set_fill(opacity=alpha)
            )
    ...
    def comment_on_imperfections(self):
        ...
        self.wait(7)
        self.play(
            ellipses.scale, scale_factor, {"about_point": center}, Animation(planets)
        )
        self.remove(*orbits)
    ...
    def construct(self):
        ...
        for image, cross in zip(images, crosses):
            image.rect = SurroundingRectangle(
                image, stroke_width=3, stroke_color=WHITE, buff=0
            )
            cross.scale(1.1)
...
class DrawEllipseOverlay(Scene):
    def construct(self):
        ...
        image = ImageMobject(
            os.path.join(
                get_image_output_directory(self.__class__), "HeldUpEllipse.jpg"
            )
        )
...
class ShowEllipseDefiningProperty(Scene):
    ...
    def add_focal_lines(self):
        ...
        dot_update = Mobject.add_updater(
            dot,
            lambda d: d.move_to(
                self.ellipse.point_from_proportion(position_tracker.get_value() % 1)
            ),
        )
    ...
    def label_foci(self):
        foci = self.get_foci()
        focus_words = VGroup(
            *[OldTexText("Focus").next_to(focus, DOWN) for focus in foci]
        )
...
class ProveEllipse(ShowEmergingEllipse, ShowEllipseDefiningProperty):
    ...
    def hypothesize_foci(self):
        ...
        for label, dot, vect in zip(labels, dots, vects):
            ...
            arrow = Arrow(
                label.get_corner(-vect), dot.get_corner(vect), buff=SMALL_BUFF
            )
    ...
    def setup_and_show_focal_sum(self):
        ...
        focal_lines, focal_lines_update_animation = self.get_focal_lines_and_update(
            get_foci, focal_sum_point
        )
        ...
        self.remove(*to_add)
        self.play(
            *list(map(FadeOut, [focal_lines, distance_labels, sum_expression, numbers]))
        )
    ...
    def look_at_perpendicular_bisector(self):
        ...
        focal_sum_point_animation = turn_animation_into_updater(
            MaintainPositionRelativeTo(self.focal_sum_point, Q_dot)
        )
        ...
        QE_line_update = Mobject.add_updater(
            QE_line,
            lambda l: l.put_start_and_end_on(
                Q_dot.get_center(), self.get_eccentricity_point()
            ),
        )
        ...
        def put_dot_at_intersection(dot):
            point = line_intersection(
                line.get_start_and_end(), radial_line.get_start_and_end()
            )
            dot.move_to(point)
            return dot
        ...
        ghost_line_update_animation = UpdateFromFunc(
            ghost_line,
            lambda l: l.put_start_and_end_on(
                self.get_eccentricity_point(), P_dot.get_center()
            ),
        )
        ...
        elbow_update_animation = UpdateFromFunc(
            elbow, lambda e: Transform(e, self.get_elbow(ghost_line)).update(1)
        )
        ...
        self.play(
            ApplyFunction(
                distance_label_rotate_update, distance_label_copy, path_arc=angle
            ),
        ...
        def update_lines(lines):
            for gl, line in zip(self.ghost_lines, lines):
                intersection = line_intersection(
                    [self.circle.get_center(), gl.get_end()], line.get_start_and_end()
                )
...
class KeplersSecondLaw(AskAboutEllipses):
    ...
    def show_area_sweep(self, time=1.0):
        ...
        area_update = UpdateFromFunc(
            area,
            lambda a: Transform(a, self.get_area(start_prop, orbit.proportion)).update(
                1
            ),
        )
    ...
    def get_arc(self, prop1, prop2):
        ...
        if prop2 > prop1:
            ...
            arc, arc_extension = [
                VMobject().pointwise_become_partial(ellipse, p1, p2)
...
class NonEllipticalKeplersLaw(KeplersSecondLaw):
    ...
    def get_ellipse(self):
        orbit_shape = ParametricCurve(
            lambda t: (1 + 0.2 * np.sin(5 * TAU * t))
            * np.array([np.cos(TAU * t), np.sin(TAU * t), 0])
        )
...
class AngularMomentumArgument(KeplersSecondLaw):
    ...
    def show_small_sweep(self):
        ...
        alt_triangle = Polygon(
            sun_center, interpolate(comet_start, comet_end, 0.9), comet_end
        )
        ...
        self.play(
            ShowCreation(
                triangle, rate_func=lambda t: interpolate(1.0 / 3, 2.0 / 3, t)
            ),
    ...
    def show_sweep_dimensions(self):
        ...
        self.play(
            ReplacementTransform(v_perp_delta_t.copy(), VGroup(*area_expression[-2:]))
        )
        self.wait()
...
class FeynmanRecountingNewton(Scene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            *[FadeIn(mob, direction=3 * LEFT) for mob in (principia, principia.rect)]
        )
        self.wait()
...
class IntroduceShapeOfVelocities(AskAboutEllipses, MovingCameraScene):
    ...
    def get_velocity_vector(self, alpha, d_alpha=0.01, scalar=3.0):
        ...
        curr_v_perp = norm(
            np.cross(vector.get_vector(), radius_vector / norm(radius_vector))
        )
        vector.scale(
...
class ShowEqualAngleSlices(IntroduceShapeOfVelocities):
    ...
    def show_equal_angle_slices(self):
        ...
        self.play(
            *[
                LaggedStartMap(
                    ApplyMethod, fader, lambda m: (m.set_fill, {"opacity": 1}), **kwargs
                )
                for fader in faders
            ]
            + [Animation(lines)]
    ...
    def areas_are_proportional_to_radius_squared(self):
        ...
        expression = OldTexText(
            "Time", "$\\propto$", "Area", "$\\propto$", "$(\\text{Radius})^2$"
        )
    ...
    def show_inverse_square_law(self):
        ...
        force_expression = OldTex(
            "ma", "=", "\\text{Force}", "\\propto", "\\frac{1}{(\\text{Radius})^2}"
        )
        ...
        acceleration_expression.next_to(
            force_expression, DOWN, buff=0.75, aligned_edge=LEFT
        )

        delta_v_expression = OldTex(
            "\\Delta v}", "\\propto", "{\\Delta t", "\\over", "(\\text{Radius})^2}"
        )
        delta_v_expression.next_to(
            acceleration_expression, DOWN, buff=0.75, aligned_edge=LEFT
        )
    ...
    def directly_compare_velocity_vectors(self):
        ...
        for vector in vectors:
            vector.save_state()
            vector.target = Arrow(
                *vector.get_start_and_end(), color=vector.get_color(), buff=0
            )
        ...
        self.play(
            ReplacementTransform(
                v1.saved_state.copy(), v2.saved_state, path_arc=self.theta
            )
        )
...
class UseVelocityDiagramToDeduceCurve(ShowEqualAngleSlices):
    ...
    def show_theta_degrees(self):
        ...
        circle_wedge_start = self.get_wedge(
            Line(circle.get_top(), circle.get_top()), circle.get_center()
    ...
    def replace_vectors_with_lines(self):
        ...
        highlighted_line = Line(
            *highlighted_vector.get_start_and_end(), stroke_color=YELLOW, stroke_width=4
        )
    ...
    def show_90_degree_rotation(self):
        ...
        self.play(
            *[
                Restore(
                    mob, path_arc=angle, run_time=4, rate_func=there_and_back_with_pause
                )
                for mob in list(lines) + [highlighted_line]
            ]
