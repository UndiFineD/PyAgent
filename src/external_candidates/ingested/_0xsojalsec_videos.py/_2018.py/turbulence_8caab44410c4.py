...
class Eddy(VMobject):
    ...
    def __init__(self, **kwargs):
        ...
        # self.add(lines)
        self.add(
            *[CreationDestructionMobject(line, **self.cd_mob_config) for line in lines]
        )
...
class Chaos(Eddy):
    ...
    def get_lines(self):
        return VGroup(
            *[self.get_line(y) for y in np.linspace(0, self.height, self.n_lines)]
        )
...
class CarefulWithLasers(TeacherStudentsScene):
    def construct(self):
        ...
        def update_laser(laser, dt):
            laser.time += dt
            laser.rotate(
                0.5 * dt * np.sin(laser.time), about_point=laser[0].get_center()
            )
...
class AskAboutTurbulence(TeacherStudentsScene):
    ...
    def pi_creatures_ask(self):
        ...
        self.play(
            ShowCreation(h_line),
            LaggedStartMap(
                FadeOutAndShiftDown, self.pi_creatures, run_time=1, lag_ratio=0.8
            ),
        )
        self.wait()
...
class NewtonsSecond(Scene):
    def construct(self):
        square = Square(
            stroke_color=WHITE, fill_color=GREY_B, fill_opacity=0.5, side_length=1
        )
        label = OldTex("m")
        label.scale(1.5)
        ...
        self.play(
            square.shift, 4 * RIGHT + 2 * UP, rate_func=lambda t: t**2, run_time=2
        )
        self.wait()
        square.restore()
...
class FiguresOfFluidDynamics(Scene):
    def construct(self):
        ...
        images = Group(
            *[ImageMobject(name.replace(" ", "_"), height=3) for name in names]
        )
        ...
        self.play(
            LaggedStartMap(FadeInFromDown, image_groups, lag_ratio=0.5, run_time=3)
        )
        self.wait()
