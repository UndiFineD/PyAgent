...
class ComputationalNetwork(MovingCameraScene):
    ...
    def walk_through_parts(self):
        ...
        for formula in f_formula, g_formula:
            ...
            self.play(
                ReplacementTransform(
                    x.copy(), formula.get_parts_by_tex("x"), path_arc=PI / 3
                )
            )
            self.wait()
        ...
        self.play(
            ReplacementTransform(
                f_formula[0].copy(), h_formula.get_part_by_tex("f"), path_arc=PI / 3
            )
        )
        self.play(
            ReplacementTransform(
                g_formula[0].copy(), h_formula.get_part_by_tex("g"), path_arc=PI / 3
            )
        )
        self.wait()
    ...
    def expand_out_h_as_function_of_x(self):
        ...
        expanded_formula = OldTex(
            "h = x^4 \\cos(\\pi x)", tex_to_color_map=self.tex_to_color_map
        )
    ...
    def plug_in_values(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(full_derivative.copy(), rhs, lag_ratio=0.5, run_time=2)
        )
        self.wait()
