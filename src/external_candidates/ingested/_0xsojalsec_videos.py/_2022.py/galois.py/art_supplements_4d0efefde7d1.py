# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\galois\art_supplements.py
    ...
    def construct(self):
        ...
        for x in range(n_swaps):
            ...
            for i, letter in enumerate(text):
                target = text[perm[i]]
                letter_anims.append(
                    letter.animate.move_to(target, DOWN).set_anim_args(
                        path_arc=90 * DEGREES
                    )
                )
                if i < perm[i]:
                    arrow = Arrow(
                        letter.get_bottom(), target.get_bottom(), path_arc=90 * DEGREES
                    )
                elif i > perm[i]:
                    arrow = Arrow(
                        letter.get_top(), target.get_top(), path_arc=90 * DEGREES
                    )
                else:
                    arrow = VMobject()
...
class WriteName(InteractiveScene):
    def construct(self):
        self.play(
            Write(Text("Évariste Galois", font_size=90).to_corner(UL), run_time=3)
        )
        self.wait()
...
class NightSkyOutpaintingTransition(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            MoveToTarget(image), run_time=12, rate_func=bezier([0, 0, 0, 1, 1, 1])
        )
        self.wait()
...
class NightBeforeQuote(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            .set_anim_args(lag_ratio=0.5, run_time=3)
        )
        self.wait(2)
...
class CauchyFourierPoisson(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeIn(image, 0.5 * UP, scale=1.1) for image in images), **kw
            ),
            LaggedStart(
                *(
                    Write(name, rate_func=squish_rate_func(smooth, 0, 0.85))
                    for name in names
                ),
                **kw,
            ),
