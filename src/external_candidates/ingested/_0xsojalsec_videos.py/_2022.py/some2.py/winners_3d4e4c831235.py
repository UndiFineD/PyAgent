# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\some2\winners.py
import pandas as pd
import requests
...
class ExampleChannels(InteractiveScene):
    def construct(self):
        ...
        tiles = Group(*(Group(tile.copy()) for x in range(9))).arrange_in_grid(
            3, 3, buff=0
        )
        ...
        self.play(
            ...
            LaggedStart(
                *(CountInFrom(tile[-1][0], 0, run_time=2.5) for tile in tiles),
                lag_ratio=0.2,
            ),
        )
...
class IntroScene(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(FadeTransform(init.copy(), two) for init in inits), lag_ratio=0.1
            ),
            MoveToTarget(inits),
        )
        inits.add(two)
...
class PeerReview(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(
                *(
                    AnimationGroup(
                        pi.change("raise_right_hand"), FadeIn(video, 0.5 * UP)
                    )
        ...
        self.play(
            LaggedStart(
                *(
                    AnimationGroup(
                        pi.watchlist[0]
                        .animate.next_to(pi.get_top(), UL, buff=0.1)
                        .shift(0.025 * RIGHT),
                        pi.watchlist[1]
                        .animate.next_to(pi.get_top(), UR, buff=0.1)
                        .shift(0.025 * LEFT),
        ...
        for x in range(10):
            ...
            self.play(
                LaggedStart(
                    *(
                        AnimationGroup(
                            pi.change("sassy", pi.get_top() + UP).set_anim_args(
                                run_time=0.5
                            ),
                            FadeOut(pi.watchlist[x], scale=0.5),
                            pi.watchlist[x + 1]
                            .animate.set_height(0.25)
                            .next_to(pi.get_top(), UL, buff=0.1)
                            .shift(0.025 * RIGHT),
                            pi.watchlist[x + 2]
                            .animate.next_to(pi.get_top(), UR, buff=0.1)
                            .shift(0.025 * LEFT),
    ...
    def get_judgement_animation(self, pi, videos):
        ...
        for i in range(8):
            anims.append(
                pi.animate.change(
                    random.choice(["raise_right_hand", "raise_left_hand"])
                )
            )
            anims.append(
                AnimationGroup(
...
class OneHundredEntries(InteractiveScene):
    def construct(self):
        ...
        title.add(
            ...
            .scale(1.5)
            .insert_n_curves(20)
            .set_stroke(width=(0, 3, 3, 3, 0))
        )
        title.to_edge(UP, buff=MED_SMALL_BUFF)
        title.set_color(GREY_A)

        winners = VGroup(
            VideoIcon(), self.written_icon(), *(VideoIcon() for x in range(3))
        )
        ...
        self.add(title)
        self.play(
            LaggedStartMap(FadeIn, winners, shift=0.5 * DOWN, lag_ratio=0.1, run_time=1)
        )
        self.play(
            LaggedStartMap(
                FadeIn, prizes, shift=0.25 * RIGHT, lag_ratio=0.1, run_time=1
            )
        )
        self.wait()
        ...
        article_title = Text("25 top non-video entries").next_to(
            articles, UP, buff=MED_LARGE_BUFF
        )
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeIn, judges, shift=0.5 * RIGHT, lag_ratio=0.5, run_time=6
            ),
        )
...
class FourCategories(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStart(*(FadeIn(tile, scale=0.8) for tile in self.tiles)),
            run_time=5,
            lag_ratio=0.1,
        )
        self.wait()
        ...
        # Accentuate titles
        icons = VGroup(
            *(SVGMobject(file) for file in ["teaching", "gem", "lightbulb", "memory"])
        )
        ...
        self.play(
            ...
            .animate.scale(0.9)
            .arrange_in_grid(3, 3)
            .move_to(subregions[0])
            .align_to(ORIGIN, UP),
            regions[0]
            .tiles[7:]
            .animate.scale(0.9)
            .arrange_in_grid(2, 2)
            .move_to(subregions[1])
            .align_to(ORIGIN, UP),
        ...
        regions[1].tiles.scale(
            regions[0].tiles[0].get_height() / regions[1].tiles[0].get_height()
        )
        ...
        focus_dots = Group(
            *(
                GlowDot(
                    [
                        interpolate(
                            bucket.get_x(LEFT) + 0.1,
                            bucket.get_x(RIGHT) - 0.1,
                            random.random(),
                        ),
                        interpolate(
                            bucket.get_y(DOWN) + 0.1,
                            bucket.get_y(TOP) - 0.1,
                            random.random(),
                        ),
                        0,
                    ]
        ...
        self.play(
            ...
            c_region.animate.become(
                FullScreenRectangle(fill_color=BLACK, fill_opacity=0)
            ),
        ...
        self.play(
            regions[2]
            .animate.replace(FullScreenRectangle(), stretch=True)
            .set_opacity(0),
            titles[2].animate.center().to_edge(UP),
            *map(
                FadeOut, [*titles[:2], titles[3], *regions[:2], regions[3], *self.tiles]
            ),
        )
        ...
        for st, rect in zip(subtitles, rects):
            ...
            st.add_updater(
                lambda m: m.set_max_width(min(m.start_width, 0.8 * m.rect.get_width()))
            )
            st.add_updater(lambda m: m.move_to(m.rect))

        lines = VGroup(
            *(Line(titles[2].get_bottom(), st.get_top(), buff=0.25) for st in subtitles)
        )
        ...
        self.play(
            ...
            LaggedStartMap(
                FadeInFromPoint, subtitles, point=titles[2].get_bottom(), lag_ratio=0.5
            ),
        )
        ...
        self.play(
            rects[0].animate.set_width(
                rects[0].get_width() - 5, stretch=True, about_edge=LEFT
            ),
            rects[1].animate.set_width(
                rects[1].get_width() + 5, stretch=True, about_edge=RIGHT
            ),
        )
        self.wait()
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "pondering", "pondering", look_at=equation
            ),
        )
        self.wait()
...
class Overphilosophizing(TeacherStudentsScene):
    def construct(self):
        ...
        words = VGroup(
            *map(Text, ["Motivation", "Core lesson", "Broader\nconnections"])
        )
        words.scale(0.7)
        for word, rect in zip(words, rects):
            ...
            word.add_updater(
                lambda m: m.set_max_width(min(m.rect.get_width() * 0.8, m.start_width))
            )
        ...
        def left_shift(x, rects=rects, **kw):
            return (
                rects[0]
                .animate.set_width(
                    rects[0].get_width() + x, about_edge=LEFT, stretch=True
                )
                .set_anim_args(**kw),
                rects[1]
                .animate.set_width(
                    rects[1].get_width() - x, about_edge=RIGHT, stretch=True
                )
                .set_anim_args(**kw),
            )
        ...
        def right_shift(x, rects=rects, **kw):
            return (
                rects[1]
                .animate.set_width(
                    rects[1].get_width() + x, about_edge=LEFT, stretch=True
                )
                .set_anim_args(**kw),
                rects[2]
                .animate.set_width(
                    rects[2].get_width() - x, about_edge=RIGHT, stretch=True
                )
                .set_anim_args(**kw),
            )
...
class MusicSidenote(InteractiveScene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*(FadeIn(note, 0.2 * LEFT) for note in notes), lag_ratio=0.3),
...
class Winners(InteractiveScene):
    def construct(self):
        ...
        mystery_tiles = Group(
            *(get_entry_tile().replace(winner[0]) for winner in winners)
        )
        ...
        to_fade = VGroup(
            title, first_outline, group_label, *(winner[1] for winner in winners)
        )
...
class Sponsors(InteractiveScene):
    def construct(self):
        # Images
        self.add(
            FullScreenFadeRectangle(fill_color=interpolate_color(GREY_A, WHITE, 0.5))
        )
        ...
        contributions = VGroup(
            ...
            Text("$500 for each\nhonorable mention").next_to(
                logos[1:3], RIGHT, buff=2.0
            ),
        ...
        for logo, j in zip(logos, [0, 1, 1, 2]):
            arrows.add(
                Arrow(logo.get_right(), contributions[j].get_left(), color=BLACK)
            )
