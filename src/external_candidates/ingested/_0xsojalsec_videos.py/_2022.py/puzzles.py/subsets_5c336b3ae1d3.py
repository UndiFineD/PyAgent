...
def get_part_by_value(set_tex, value):
    try:
        return next(
            sm for sm in set_tex if isinstance(sm, Integer) and sm.get_value() == value
        )
...
def get_sum_wrapper(set_tex):
    wrapper = VGroup(
        OldTex("\\text{sum}\\big(\\big) = ")[0], Integer(sum(set_tex.values))
    )
...
def get_subsets(full_set):
    return list(
        it.chain(*(it.combinations(full_set, k) for k in range(len(full_set) + 1)))
    )
...
def massive_int(num, n_cols=42, width=7):
    ...
    for n in range(len(total) - 3, -3, -3):
        ...
        triplet.arrange_to_fit_width(
            triplet.get_width() - 2 * comma.get_width(), about_edge=LEFT
        )
...
class UnreasonableUsefulness(InteractiveScene):
    def construct(self):
        ...
        self.play(
            Write(
                plane, run_time=1, lag_ratio=0.01, stroke_color=WHITE, stroke_width=1
            ),
            Write(plane_title),
            quote_anim,
        )
...
class AnswerGuess(InteractiveScene):
    def construct(self):
        ...
        randy.get_bubble(
            Text("Why?", font_size=24), direction=LEFT, height=1.0, width=1.0
        )
        ...
        self.play(
            TransformMatchingShapes(count[1].copy(), guess[1], path_arc=30 * DEGREES)
        )
        self.wait()
            ...
            [
                int(
                    sum(
                        (
                            int(bit) * n
                            for n, bit in zip(it.count(1), reversed(bin(n)[2:]))
                        )
                    )
                    % 5
                    == 0
...
class TwoThousandBinaryChoices(InteractiveScene):
    def construct(self):
        ...
        elem_rects = VGroup(
            *(SurroundingRectangle(elem, buff=0.05).round_corners() for elem in elems)
        )
        elem_rects.set_stroke(BLUE, 2)

        twos = VGroup(
            *(Integer(2).match_width(elems[1]).next_to(er, UP) for er in elem_rects)
        )
        dots = VGroup(
            *(
...
class AskAboutGuess(InteractiveScene):
    def construct(self):
        ...
        left_label = OldTexText(
            "Guess: ", "$\\frac{1}{5} \\cdot 2^{2{,}000}$", font_size=36
        )
...
class ExampleWith5(InteractiveScene):
        ...
        for n in range(n_selections):
            ...
            for stack, sum_stack in zip(stacks, sum_stacks):
                for subset, sum_mob in zip(stack, sum_stack):
                    if set(subset.get_family()).intersection(
                        self.selection.get_family()
                    ):
    ...
    def construct_all_subsets(self, set_tex):
        ...
        rects = VGroup(
            *(SurroundingRectangle(vp, buff=0.1).round_corners() for vp in value_parts)
        )
        ...
        lines = VGroup(
            *(Line(words.get_top(), rect.get_bottom(), buff=0.15) for rect in rects)
        )
        ...
        for value in range(1, 6):
            ...
            added_subsets = VGroup(
                *(get_set_tex([*ss.values, value]).move_to(ss) for ss in subsets)
            )
    ...
    def show_generating_function(self, set_tex):
        ...
        rects = VGroup(
            *(SurroundingRectangle(part, buff=0.05).round_corners() for part in parts)
        )
        rects.set_stroke(BLUE, 2)
    ...
    def transition_to_full_generating_function(self, set_tex):
        ...
        for n, i, j in [(1, 5, 11), (2, 11, 17), (3, 17, 23), (2000, 36, 46)]:
            ...
            self.play(
                FadeTransform(
                    value_part.copy(), factored[i + 4 : j - 1].copy(), remover=True
                ),
    ...
    def get_subset_stacks(self, full_set, buff=3.5):
        stacks = VGroup(
            *(
                VGroup(
                    *(get_set_tex(subset) for subset in it.combinations(full_set, k))
                )
    ...
    def get_subset_sums(self, stacks):
        return VGroup(
            *(
                VGroup(*(get_sum_group(set_tex) for set_tex in stack))
                for stack in stacks
            )
        )
...
class ShowHypercubeConstruction(InteractiveScene):
    ...
    def styling(self):
        ...
        everything = VGroup(
            *curr_sets.family_members_with_points(), *lines.family_members_with_points()
        )
        everything.sort(
            lambda p: -get_norm(p - self.frame.get_implied_camera_location())
        )
        self.add(*everything)
    ...
    def construct(self):
        ...
        poly = Tex(
            ...
            tex_to_color_map=dict(
                zip((f"{{{n}}}" for n in n_range), color_gradient([BLUE_B, BLUE_D], 5))
            ),
            isolate=["x"],
        )
        ...
        questions = VGroup(
            ...
            OldTexText("What is $x$?", font_size=60, color=TEAL).set_backstroke(
                width=8
            ),
        )
        ...
        anims = []
        for pi, mode, question in zip(
            pis, ["maybe", "confused", "pleading"], questions
        ):
            anim = PiCreatureSays(
                pi,
                question,
        ...
        rects = VGroup(
            *(SurroundingRectangle(x, buff=0.05).round_corners() for x in xs)
        )
        rects.set_stroke(TEAL, 2)
        ...
        self.play(
            TransformMatchingShapes(
                poly[5:].copy(), expansion, run_time=3, lag_ratio=0.01
            ),
...
class PolynomialConstruction(InteractiveScene):
    def construct(self):
        ...
        for n in range(2, 8):
            ...
            new_term = Tex(f"x^{n}")
            new_poly = Tex(
                "".join([f"\\left(1 + x^{k}\\right)" for k in range(1, n + 1)])
            )
...
class GeneratingFunctions(InteractiveScene):
    def construct(self):
        ...
        func_prop2.target.next_to(details_title, DOWN, MED_LARGE_BUFF).align_to(
            details_box, LEFT
        ).shift(SMALL_BUFF * RIGHT)
...
class EvaluationTricks(InteractiveScene):
    def construct(self):
        ...
        factored = Tex(
            "f(x) = (1 + x)(1 + x^2)(1 + x^3) \\cdots (1 + x^{2{,}000})", **tex_kw
        )
        ...
        line.add_updater(
            lambda l: l.put_start_and_end_on(
                plane.c2p(get_x(), 0), plane.i2gp(get_x(), graph)
            )
        )
        ...
        desire = VGroup(
            VGroup(parens, words), Tex("= c_0 + c_5 + c_{10} + \\cdots + c_{N}")
        )
...
class MotivateRootsOfUnity(InteractiveScene):
    def construct(self):
        ...
        # Add f(-1)
        m1_pieces = [
            p.replace("(x)", "x").replace("x", "\\left(-1\\right)") for p in pieces
        ]
        ...
        points = [
            plane.n2p(np.exp(complex(0, angle))) for angle in np.arange(0, TAU, TAU / 5)
        ]
...
class FifthRootsOfUnity(InteractiveScene):
    def construct(self):
        ...
        y_line = Line(
            plane.n2p(math.cos(TAU / 5)), plane.n2p(np.exp(complex(0, TAU / 5)))
        )
        ...
        self.play(
            LaggedStartMap(FadeOut, VGroup(trig_rhs, x_line, low_cos, y_line, low_sin))
        )
        ...
        example_sums = [
            Tex(
                "+".join([f"\\zeta^{{{k * n}}}" for n in range(5)])
                + ("=5" if k == 5 else "=0"),
                **root_kw,
            )
            for k in range(1, 6)
        ...
        for k, ex_text, ex_sum in zip(it.count(1), example_texts, example_sums):
            ...
            if k == 1:
                ...
                self.wait(
                    note="Move root vectors tip to tail (next animation they restore)"
                )
                ...
                arrows = VGroup(
                    *(
                        Arrow(
                            root_dots[n], root_dots[(k * n) % 5], buff=0, stroke_width=3
                        )
                        for n in range(1, 5)
                    )
                )
        ...
        root_groups = Group(
            *(Group(*parts) for parts in zip(root_dots, root_lines, zp1_labels))
        )
        ...
        rect_groups = VGroup(
            *(
                VGroup(
                    *(
                        SurroundingRectangle(bp[int(n + 1 in ss)], buff=SMALL_BUFF)
                        .round_corners()
                        .set_stroke(TEAL, 2)
        ...
        arrows = VGroup(
            *(Vector(0.5 * UP).next_to(part, UP, SMALL_BUFF) for part in parts)
        )
        values = VGroup(
        ...
        # Final answer
        final_answer = Tex(
            "= \\frac{1}{5}\\Big(" "2^{2{,}000} + 4 \\cdot 2^{400}\\Big)"
        )
        ...
        # Smaller case
        box = Rectangle(
            width=(RIGHT_SIDE[0] - plane.get_right()[0]) - 1, height=plane.get_height()
        )
        box.to_corner(DR)
    ...
    def animate_kth_power(self, plane, dots, k):
        ...
        paths = [
            ParametricCurve(
                lambda t: plane.n2p(
                    np.exp(complex(0, interpolate(angle, k * angle, t)))
                ),
                t_range=(0, 1, 0.01),
            )
            for angle in angles
        ...
        self.play(
            *(MoveAlongPath(dot, path) for dot, path in zip(dots, paths)), run_time=4
        )
        if k == 5:
            self.wait()
...
class JustifyLinearity(InteractiveScene):
    ...
    def construct(self):
        ...
        zeta_polys = VGroup(
            *(self.get_polynomial("\\zeta", YELLOW, exp_multiple=n) for n in range(5))
        )
        ...
        for plus, lhs1, lhs2 in zip(
            plusses.target, lhss.saved_state, lhss.saved_state[1:]
        ):
        ...
        for mob in x_poly, group:
            self.play(
                FlashAround(
                    mob, time_width=2, run_time=2, color=mob[2].get_fill_color()
                ),
        ...
        for j in range(len(x_poly.sep_indices) - 1):
            ...
            self.wait()
            self.play(
                Write(plus), *(FadeTransform(piece.copy(), term) for piece in col[1:])
            )
            self.wait()
...
class Recap(InteractiveScene):
    def construct(self):
        ...
        rhs = (
            "\\Big("
            + "".join(f"(1 + \\zeta^{n})" for n in range(1, 6))
            + "\\Big)^{400}"
        )
        fz = Tex(
            f"f(\\zeta) = {rhs}",
...
class ReflectOnGeneratingFunctions(InteractiveScene):
    def construct(self):
        ...
        coefs = VGroup(
            *[poly[i + 1 : i + 1 + len(str(n))] for i, n in zip(plus_indices, seq)]
        )
        self.play(
            LaggedStart(
                *(
        ...
        arrow = Arrow(
            in_plane.get_right(), out_plane.get_left(), path_arc=-45 * DEGREES
        )
        arrow.shift(UP)
        ...
        arc = ParametricCurve(
            lambda t: in_plane.n2p(np.exp(complex(0, PI - t))), t_range=(0, TAU)
        )
        ...
        paths = [
            TracingTail(
                in_dot, stroke_color=YELLOW, stroke_width=(0, 3), time_traced=16
            ),
            TracingTail(
                out_dot, stroke_color=RED, stroke_width=(0, 1.5), time_traced=16
            ),
        ]
        self.add(*paths)
        self.wait(16)
...
class GeneratingFunctionForPrimes(InteractiveScene):
    def construct(self):
        ...
        piece_groups = [
            ...
            [
                "{ " + str(int(sympy.isprime(n))) + f" \\over  {{{n}}}^s }}"
                for n in n_range
            ],
...
class DirichletSeries(InteractiveScene):
    def construct(self):
        ...
        series_terms = [
            f"{{ {coef} \\over  {denom} }}"
            for coef, denom in zip(coef_texs, denom_texs)
        ]
        series = Tex(
        ...
        v_line = ParametricCurve(
            lambda t: in_plane.n2p(complex(2, t)), t_range=(-4, 4, 0.01)
        )
        ...
        out_line.add_updater(
            lambda m: m.set_points(
                [out_plane.n2p(func(in_plane.p2n(p))) for p in v_line.get_points()]
            )
        )
        ...
        self.play(
            FadeIn(in_plane), FadeIn(out_plane), FadeIn(plane_labels), FadeIn(in_dot)
        )
        self.play(
            ShowCreation(arrow),
...
class AskAboutComplex(TeacherStudentsScene):
    def construct(self):
        ...
        # Blank check
        new_line = Tex(
            "f(\\dots) = \\text{(more information)}", tex_to_color_map={"\\dots": BLUE}
        )
...
class PracticeQuestions(InteractiveScene):
    def construct(self):
        # Blah
        division = VGroup(
            Line().set_width(FRAME_WIDTH), Line(UP, DOWN).set_height(FRAME_HEIGHT)
        )
...
class ProblemStatement(TeacherStudentsScene):
    def construct(self):
        ...
        words = VGroup(
            *(title.get_part_by_text(text) for text in title.get_text().split(" "))
        )
        ...
        self.play(
            LaggedStart(
                *(FadeInFromPoint(word, morty.get_corner(UL)) for word in words)
            ),
        ...
        sum_groups = VGroup()
        for subset, sum_wrapper, mode_triple in zip(
            subsets, sum_wrappers, mode_triples
        ):
            self.play(
            ...
            elem_rects = VGroup(
                *(
                    SurroundingRectangle(get_part_by_value(set_tex, value))
                    .set_stroke(TEAL, 2)
                    .round_corners()
            ...
            if subset is subsets[0]:
                ...
                self.play(
                    sum_group.animate.scale(0.5).next_to(
                        sum_groups, DOWN, MED_LARGE_BUFF, LEFT
                    ),
                    *anims,
                )
...
class BookQuestionHighlight(InteractiveScene):
    def construct(self):
        # Temp
        img = ImageMobject(
            "/Users/grant/Dropbox/3Blue1Brown/videos/2022/puzzles/subsets/images/BookQuestion.jpg"
        )
...
class QuestionMorph(InteractiveScene):
    def construct(self):
        question = OldTexText(
            "How many subsets are there\\\\", "with a sum divisible by 5", "?"
        )
...
class SquaringZeta(InteractiveScene):
    def construct(self):
        ...
        # Explicit
        exp = Tex(
            "\\zeta^2 =" "\\big(e^{2\\pi i / 5}\\big)^2 =" "e^{4\\pi i / 5}", **kw
        )
...
class ConjugatePairFact(InteractiveScene):
    def construct(self):
        ...
        lines.add_updater(
            lambda ls: [
                line.put_start_and_end_on(plane.n2p(0), dot.get_center())
                for line, dot in zip(ls, dots)
            ]
        )
        labels = VGroup(
...
class HangInThereRecapPrep(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            "confused", "thinking", added_anims=[self.teacher.change("happy")]
        )
        self.wait(3)
...
class RotationalSum(InteractiveScene):
    ...
    def construct(self):
        ...
        values = VGroup(
            *(Integer(0, edge_to_fix=ORIGIN).move_to(point) for point in points)
        )
        ...
        # Add labels
        labels = VGroup(
            *(OldTexText(f"sum $\\equiv$ {n}\\\\mod 5", font_size=30) for n in range(5))
        )
...
class HighlightAnswerParts(InteractiveScene):
    def construct(self):
        ...
        e2 = "4 \\cdot 2^{400}"
        answer = Tex(
            f"{frac} \\Big({e1} + {e2} \\Big)", isolate=[frac, e1, e2], font_size=60
        )
        answer.move_to(UP)
...
class ReferenceZetaPromise(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says(
            OldTexText(
                "Didn't you once promise\\\\a video about $\\zeta(s)$ and primes?"
            ),
            target_mode="sassy",
            index=0,
            added_anims=[
...
class AltThumbnail(InteractiveScene):
    def construct(self):
        # Organize sets
        subsets = [
            subset for subset in get_subsets(range(1, 10)) if sum(subset) % 5 == 0
        ]
        n_shown = 28
        shown_sets = VGroup(*(get_set_tex(ss) for ss in subsets[:n_shown]))
        shown_sets.arrange(DOWN, aligned_edge=LEFT)
        shown_sets[n_shown // 2 :].next_to(
            shown_sets[: n_shown // 2], RIGHT, MED_LARGE_BUFF
        )
