...
class DecomposeRoar_SlowFPS(DecomposeAudioSegment):
    ...
    def show_all_waves(self):
        ...
        self.play(
            ...
            LaggedStart(
                *(
                    ReplacementTransform(graph.copy().set_opacity(0), wave)
...
class CombineWavesToImage_SlowFPS(FourierCirclesScene):
    ...
    def show_sine_waves(self):
        ...
        for vect, axes, color in zip(shown_vectors, all_axes, colors):
            ...
            line.add_updater(
                lambda l: l.put_start_and_end_on(l.vect.get_end(), l.graph.get_start())
            )
            v2g_lines.add(line)
    ...
    def get_terms(self, n_terms, max_terms=100, max_width=12):
        ...
        for k in ks:
            terms.add(
                Tex("c_{k} e^{k \\cdot 2 \\pi i \\cdot t} +".replace("k", str(k)))
            )
    ...
    def add_draw_dot(self, vectors):
        dot = GlowDot()
        dot.add_updater(
            lambda d: d.move_to(vectors[-1].get_end() if len(vectors) > 0 else ORIGIN)
        )
        self.add(dot)
        return dot
