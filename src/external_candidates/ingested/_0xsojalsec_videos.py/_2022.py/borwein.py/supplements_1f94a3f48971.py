# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\borwein\supplements.py
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "confused", "thinking", look_at=3 * RIGHT + UP
            ),
        )
        self.wait(2)
        self.play(
...
class XInMovingAverageGraphs(MovingAverages):
    def construct(self):
        ...
        self.play(Write(x_label), ShowCreation(arrow))
        self.wait()
...
class ThisConstant(InteractiveScene):
    def construct(self):
        ...
        arrow = Arrow(
            words1.get_bottom() + 0.1 * DOWN, words1.get_bottom() + DOWN + 1.5 * LEFT
        )
...
class HeavyMachinery(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.play(FadeIn(conv, UP), ft.animate.next_to(conv, UP, MED_LARGE_BUFF))
        self.wait(2)
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "thinking", "thinking", look_at=self.screen
            ),
        )
        self.wait(4)
        ...
        self.play(
            self.change_students("thinking", "hesitant", "pondering", look_at=conv_thm)
        )
        self.wait(3)
...
class EngineersSinc(InteractiveScene):
    def construct(self):
        # Blah
        sinc = OldTex(
            R"{\text{sin}(\pi x) \over \pi x}", tex_to_color_map={R"\pi": TEAL}
        )
        sinc.shift(UP)
...
class HowDoYouCompute(TeacherStudentsScene):
    def construct(self):
        ...
        stds = self.students
        self.student_says(
            "But how do you\ncompute this\nFourier Transform?", mode="angry", run_time=2
        )
        self.play(
            morty.change("guilty"),
...
class TranslateToFourierLand(InteractiveScene):
    def construct(self):
        ...
        left_axes = VGroup(
            *(Axes((0, 12, 1), (-2, 2), **axes_kw).scale(0.5) for x in range(3))
        )
        ...
        right_axes = VGroup(
            *(Axes((0, 5), (0, 3), **axes_kw).scale(0.5) for x in range(3))
        )
        ...
        property_pairs = [
            ...
            VGroup(
                ...
                OldTex(
                    R"\int_{-\infty}^\infty \hat f(\xi) \hat g(\omega - \xi) d\xi",
...
class TranslatedByFourier(InteractiveScene):
    def construct(self):
        ...
        equals = OldTex("=").move_to(
            midpoint(left_expr.get_right(), right_expr.get_left())
        )
