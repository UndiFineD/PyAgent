# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\borwein\main.py
...
class ShowIntegrals(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(GrowArrow(arrow), FadeIn(sinc_label, UR))
        self.wait()
        ...
        for n in range(2, 9):
            ...
            self.play(
                Write(new_area, stroke_color=WHITE, stroke_width=1, lag_ratio=0.01)
            )
            self.wait(0.25)
            ...
            if n < 8:
                ...
                new_low_label = OldTex(
                    Rf"{{\sin(x / {2 * n + 1}) \over x / {2 * n + 1}}}"
                )
    ...
    def get_integral(self, n):
        return OldTex(
            R"\int_{-\infty}^\infty",
            R"{\sin(x) \over x}",
            *(Rf"{{\sin(x/{k}) \over x / {k} }}" for k in range(3, 2 * n + 1, 2)),
            "dx",
        ).to_corner(UL)
...
class SineLimit(InteractiveScene):
    def construct(self):
        ...
        sine_label.add_updater(
            lambda m: m.next_to(h_line, LEFT, buff=0.25 * m.get_width())
        )

        x_label = OldTex("x", font_size=24)
        x_label.add_updater(lambda m: m.match_width(sine_label[0][4]))
        x_label.add_updater(
            lambda m: m.next_to(arc.pfp(0.5), RIGHT, buff=m.get_height())
        )
        ...
        value_label = VGroup(
            OldTex(R"\frac{\sin(x)}{x} = "), DecimalNumber(1, num_decimal_places=4)
        )
        ...
        target_frame.add_updater(
            lambda m: m.set_height(4 * h_line.get_height()).move_to(h_line.get_bottom())
        )
        ...
        self.play(
            UpdateFromFunc(
                frame,
                lambda m: m.interpolate(m, target_frame, get_alpha()),
                run_time=15,
            ),
...
class WriteFullIntegrals(InteractiveScene):
    def construct(self):
        ...
        ds = ValueTracker(0)
        frame.add_updater(
            lambda m, dt: m.scale(1 + 0.02 * dt, about_edge=UL).shift(
                ds.get_value() * dt * DOWN
            )
        )
        ...
        self.play(
            Write(q_marks),
            frame.animate.scale(1.1, about_edge=UL).shift(2 * DOWN),
            run_time=2,
        )
        self.wait()
...
class WriteMoreFullIntegrals(InteractiveScene):
    def construct(self):
        ...
        ints.add(
            get_multi_sinc_integral(range(1, 17, 2), rhs=R"= (0.99999999998529)\pi")
        )
        ints.add(
            get_multi_sinc_integral(range(1, 19, 2), rhs=R"= (0.99999998807962)\pi")
        )
        ints.add(
            get_multi_sinc_integral(range(1, 21, 2), rhs=R"= (0.99999990662610)\pi")
        )
        ints.add(
            get_multi_sinc_integral(range(1, 23, 2), rhs=R"= (0.99999972286332)\pi")
        )
        ...
        ds = ValueTracker(0)
        frame.add_updater(
            lambda m, dt: m.scale(1 + 0.04 * dt, about_edge=UL).shift(
                ds.get_value() * dt * DOWN
            )
        )

        key_map = dict(
            (ints[n][-1].get_tex(), ints[n + 1][-1].get_tex()) for n in range(-4, -1, 1)
        )
...
class WriteOutIntegrals(InteractiveScene):
    ...
    def get_integrals(self):
        return VGroup(
            ...
            get_multi_sinc_integral(
                [1, 3, 5, 7, 13, 15], dots_at=7, rhs=Rf"= ({SUB_ONE_FACTOR}\dots)\pi"
            ),
        )
...
class WriteOutIntegralsWithPi(WriteOutIntegrals):
    def get_integrals(self):
        ...
        result = VGroup(
            OldTex(
                R"\int_{-\infty}^\infty", R"\frac{\sin(\pi x)}{\pi x}", R"dx = ", "1.0"
            ),
            OldTex(
                R"\int_{-\infty}^\infty",
...
class InsertTwoCos(InteractiveScene):
    ...
    def construct(self):
        ...
        integrals = VGroup(
            ...
            *(
                get_multi_sinc_integral(
                    range(1, n, 2), insertion=R"2\cos(x)", rhs=R"= \pi"
                )
                for n in n_range
            ),
        )
        ...
        areas = VGroup(
            *(
                axes.get_riemann_rectangles(
                    graph, dx=self.dx, colors=(BLUE_D, BLUE_D), fill_opacity=1
                )
                for graph in graphs
            )
        )
    ...
    def construct(self):
        ...
        integrals = VGroup(
            *(
            ...
            OldTex(R"\vdots"),
            get_multi_sinc_integral(
                [1, 3, 5, 111], dots_at=5, insertion=R"2\cos(x)", rhs=R"= \pi"
            ),
            get_multi_sinc_integral(
                [1, 3, 5, 111, 113], dots_at=5, insertion=R"2\cos(x)", rhs=R"= \pi"
            ),
        )
        integrals.scale(0.75)
        for inter in integrals:
...
class MovingAverages(InteractiveScene):
    ...
    def construct(self):
        ...
        rect_graph = axes.get_graph(
            lambda x: rect_func(x / rs), discontinuities=[-0.5 * rs, 0.5 * rs]
        )
        ...
        graphs = [
            rect_graph,
            *self.get_all_convolution_graphs(xs, rect_func(xs / rs), low_axes, k_range),
        ]
        for graph in graphs:
        ...
        for n in range(1, self.n_iterations):
            ...
            window_group = self.show_moving_average(
                axes, low_axes, graphs[n], k_range[n - 1]
            )
            ...
            fn0_label = OldTex(
                f"f_{{{n + 1}}}(0) = "
                + ("1.0" if n < self.n_iterations - 1 else "0.9999999999852937..."),
                font_size=36,
            )
        ...
        graph_groups = VGroup(
            *(VGroup(graph.axes, graph, graph.func_labels[1]) for graph in graphs[:8])
        )
        graph_groups.save_state()
        graph_groups.generate_target()
        graph_groups[:-2].set_opacity(0)
        graph_groups.target.arrange_in_grid(
            4, 2, v_buff=2.0, h_buff=1.0, aligned_edge=LEFT, fill_rows_first=False
        )
    ...
    def get_all_convolution_graphs(self, xs, ys, axes, k_range):
        ...
        for k in k_range:
            ...
            kernel /= sum(kernel)
            func_samples.append(
                np.convolve(func_samples[-1], kernel, mode="same")  # Last function
            )
            new_graph = VMobject()
    ...
    def show_moving_average(self, top_axes, low_axes, low_graph, k):
        ...
        arrow = Arrow(
            window.get_left(), window.get_right(), stroke_width=4, buff=arrow_buff
        )
        ...
        v_line = DashedLine(
            window.get_top(), low_axes.c2p(low_axes.x_axis.p2n(window.get_bottom()), 0)
        )
        ...
        drawing_dot.add_updater(
            lambda m, w=window, g=low_graph.copy(): m.move_to(
    ...
    def get_axes(
        self, x_range=(-1, 1, 0.25), y_range=(0, 1, 0.25), width=13.0, height=2.5
    ):
...
class ShowReciprocalSums(InteractiveScene):
    ...
    def construct(self):
        # Equations
        equations = VGroup(*(self.get_sum(n) for n in range(1, 8)))
        ...
        for eq1, eq2 in zip(equations, equations[1:]):
            self.play(
                TransformMatchingTex(eq1.copy(), eq2, fade_transform_mismatches=True)
            )
            self.wait()
        self.wait()
...
class LongerReciprocalSums(ShowReciprocalSums):
    ...
    def construct(self):
        # Test
        equations = VGroup(*(self.get_sum(n) for n in [1, 2, 3, 4, 54, 55, 56]))
        dots = OldTex(R"\vdots")
...
class MoreGeneralFact(InteractiveScene):
    ...
    def construct(self):
        ...
        # Graph
        axes = Axes(
            (-4 * PI, 4 * PI, PI), (-0.5, 1, 0.5), width=FRAME_WIDTH + 1, height=4
        )
        axes.x_axis.add_numbers(unit=PI, unit_tex=R"\pi")
        axes.to_edge(DOWN)

        graphs = VGroup(
            *(axes.get_graph(lambda x, n=n: multi_sinc(x, n)) for n in range(1, 5))
        )
        graphs.set_stroke(WHITE, 2)
        areas = VGroup(
            *(
        ...
        self.play(
            ...
            rect.animate.match_points(
                SurroundingRectangle(general_group, buff=SMALL_BUFF)
            ),
        ...
        for graph, label, word in zip(graphs, labels, words):
            self.play(
                ...
                TransformMatchingShapes(
                    VGroup(*f_label[:4], *g_label[:4]).copy(), label
                ),
                FadeIn(word),
            )
            self.wait()
...
class ReplaceXWithPiX(InteractiveScene):
    def construct(self):
        ...
        sinc_pi_graph = sinc_graph.copy().stretch(
            1 / PI, 0, about_point=axes.get_origin()
        )
        ...
        sinc_pi_area = sinc_area.copy().stretch(
            1 / PI, 0, about_point=axes.get_origin()
        )
        ...
        sinc_pi_label = Tex(
            R"\int_{-\infty}^\infty \frac{\sin(\pi x)}{\pi x} dx = 1.0", **kw
        )
        ...
        self.wait()
        self.play(ShowCreation(pi_rect), FadeIn(want_to_show, 0.5 * DOWN))
        self.wait()
        ...
        self.play(
            ...
            rect.animate.become(
                SurroundingRectangle(VGroup(sinc_label, want_to_show), **rect_kw)
            ),
        )
        self.wait()
...
class WhatWeNeedToShow(InteractiveScene):
    def construct(self):
        ...
        expressions = VGroup(
            Tex(
                R"\mathcal{F}\left[\frac{\sin(\pi {t})}{\pi {t}} \right]({\omega}) = \text{rect}({\omega})",
                **kw,
            ),
            Tex(
                R"\mathcal{F}\left[\frac{\sin(\pi {t} / {k})}{\pi {t} / {k}} \right]({\omega}) = {k} \cdot \text{rect}({k}{\omega})",
                **kw,
            ),
            Tex(
                R"\int_{-\infty}^\infty f({t}) dt = \mathcal{F}\left[ f({t}) \right](0)",
                **kw,
            ),
            Tex(
                R"\int_{-\infty}^\infty \frac{\sin(\pi {t})}{\pi {t}} dt = \text{rect}(0) = 1",
                **kw,
            ),
            Tex(
                R"\mathcal{F}\left[ f({t}) \cdot g({t}) \right] = \mathcal{F}[f({t})] * \mathcal{F}[g({t})]",
                **kw,
            ),
            Tex(
        ...
        axes1.add(
            OldTex("t", color=BLUE, font_size=24).next_to(
                axes1.x_axis.get_right(), UP, 0.2
            )
        )
        axes2.add(
            OldTex(R"\omega", color=YELLOW, font_size=24).next_to(
                axes2.x_axis.get_right(), UP, 0.2
            )
        )
        axes1.add_coordinate_labels(font_size=20)
        axes2.add_coordinate_labels(
            x_values=np.arange(-1, 1.5, 0.5), font_size=20, num_decimal_places=1
        )
        ...
        self.play(FadeIn(axes1), ShowCreation(graph1), FadeIn(graph1_label, UP))
        self.wait()
        self.play(
            ShowCreation(arrow),
        ...
        self.play(Write(graph2_label), ShowCreation(graph2))
        self.wait()
        self.play(
        ...
        self.play(FadeTransform(graph2_label.copy(), ft_sinc[17:]))
        self.add(ft_sinc)
        self.wait()
        ...
        self.play(FadeOut(ft_sinc, UP), FadeIn(ft_sinck, UP))
        self.wait()
        ...
        area = axes1.get_riemann_rectangles(
            axes1.get_graph(np.sinc), colors=(BLUE, BLUE), dx=0.01
        )
        ...
        self.wait()
        self.play(
            int_to_eval.animate.set_height(0.5).next_to(
                ft_sinck, DOWN, MED_LARGE_BUFF, aligned_edge=LEFT
            )
        )
        ...
        dots = Group(
            *(
                GlowDot(axes2.c2p(x, rect_func(x)), color=TEAL)
        ...
        bullets = VGroup(*(Dot().next_to(fact, LEFT) for fact in facts.target))
...
class ConvolutionTheoremDiagram(InteractiveScene):
    def construct(self):
        ...
        left_plots = VGroup(
            *(VGroup(axes, graph) for axes, graph in zip(left_axes, left_graphs))
        )
        right_plots = VGroup(
            *(VGroup(axes, graph) for axes, graph in zip(right_axes, right_graphs))
        )
        ...
        left_labels = VGroup(
            ...
            OldTex(
                R"\frac{\sin(\pi x)}{\pi x} \cdot \frac{\sin(\pi x / 2)}{\pi x / 2}"
            )[0],
        )
        right_labels = VGroup(
            *(
        ...
        for label_group, axes_group, x in (left_labels, left_axes, -2), (
            right_labels,
            right_axes,
            -0.85,
        ):
        ...
        self.play(
            MoveAlongPath(
                dot, right_graphs[2], rate_func=lambda t: smooth(1 - 0.5 * t)
            ),
            MoveAlongPath(
                dot.copy(), right_graphs[2], rate_func=lambda t: smooth(0.5 * t)
            ),
            # UpdateFromFunc(rect, lambda m: m.match_x(dot)),
            run_time=1,
        )
...
class MultiplyBigNumbers(InteractiveScene):
    def construct(self):
        ...
        d2_label = OldTex(
            R"\mathcal{O}(N^2)", font_size=60, tex_to_color_map={"N": YELLOW}
        )
        dlogd_label = OldTex(
            R"\mathcal{O}({N} \cdot \text{log}({N}))",
            font_size=60,
        ...
        for num in numbers:
            ...
            num.digit_highlights = VGroup(
                *(
                    VHighlight(
                        digit, color_bounds=(YELLOW, YELLOW_E), max_stroke_addition=8
                    )
        ...
        for dh2 in numbers[1].digit_highlights:
            ...
            self.play(
                ShowSubmobjectsOneByOne(
                    numbers[0].digit_highlights.copy(), remover=True
                ),
                run_time=1.0,
            )
            self.remove(dh2)
