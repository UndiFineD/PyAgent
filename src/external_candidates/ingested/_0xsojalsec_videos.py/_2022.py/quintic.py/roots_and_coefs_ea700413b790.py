# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\quintic\roots_and_coefs.py
...
def expanded_poly_tex(
    roots, vertical=True, root_colors=[YELLOW, YELLOW], abbreviate=False
):
    roots = list(roots)
    root_colors = color_gradient(root_colors, len(roots))
    n = len(roots)
    kw = dict(
        tex_to_color_map=dict(
            ((str(r), root_color) for r, root_color in zip(roots, root_colors))
        ),
        arg_separator=" ",
    )
    result = VGroup()
...
class RootCoefScene(Scene):
    ...
    def get_coef_poly(self):
        ...
        return OldTex(
            f"x^{degree}",
            *(f" + c_{n} x^{n}" for n in range(degree - 1, 1, -1)),
            " + c_{1} x",
            " + c_{0}",
        )
    ...
    def get_roots(self):
        return [
            self.root_plane.p2n(root_dot.get_center()) for root_dot in self.root_dots
        ]

    def get_coefs(self):
        return [
            self.coef_plane.p2n(coef_dot.get_center()) for coef_dot in self.coef_dots
        ] + [1.0]
    ...
    def add_root_lines(self, **kwargs):
        self.root_lines = self.get_tracking_lines(
            self.root_dots, self.get_r_symbols(self.root_poly), **kwargs
        )
        self.add(self.root_lines)

    def add_coef_lines(self, **kwargs):
        self.coef_lines = self.get_tracking_lines(
            self.coef_dots, self.get_c_symbols(self.coef_poly), **kwargs
        )
    ...
    def add_r_labels(self):
        self.r_dot_labels = self.add_dot_labels(
            VGroup(*(OldTex(f"r_{i}") for i in range(self.get_degree()))),
            self.root_dots,
        )

    def add_c_labels(self):
        self.c_dot_labels = self.add_dot_labels(
            VGroup(*(OldTex(f"c_{i}") for i in range(self.get_degree()))),
            self.coef_dots,
        )
    ...
    def swap_roots(self, *indices, run_time=2, wait_time=1, **kwargs):
        self.play(
            CyclicReplace(
                *(self.root_dots[i] for i in indices), run_time=run_time, **kwargs
            )
        )
        self.wait(wait_time)
    ...
    def carry_out_cycle(self):
        if self.dots_awaiting_cycle:
            self.tie_coefs_to_roots()
            self.play(
                CyclicReplace(*self.dots_awaiting_cycle, run_time=self.cycle_run_time)
            )
    ...
    def on_mouse_release(self, point, button, mods):
        ...
        dot.add_updater(
            lambda d: d.move_to(self.mouse_point.get_center() + d.mouse_point_diff)
        )
        if self.lock_coef_imag or self.window.is_key_pressed(ord("r")):
            ...
            self.lock_coef_norm or self.window.is_key_pressed(ord("a"))
        ) and dot in self.coef_dots:
            # Fix the norm
...
class RadicalScene(RootCoefScene):
    ...
    def get_angle_label(self, dot, plane, sym, get_theta):
        ...
        line.add_updater(
            lambda m: m.put_start_and_end_on(plane.get_origin(), dot.get_center())
        )
...
class QuadraticFormula(RootCoefScene):
    ...
    def add_c_labels(self):
        self.c_dot_labels = self.add_dot_labels(
            VGroup(OldTex("c"), OldTex("b")), self.coef_dots
        )
