# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\quintic\polynomial_baisics.py
...
class ConstructPolynomialWithGivenRoots(Scene):
    ...
    def construct(self):
        ...
        rects = VGroup(
            *(
                SurroundingRectangle(
                    factored[i : i + 5], stroke_width=1, stroke_color=BLUE, buff=0.05
                )
        ...
        zeros_eqs = VGroup(
            *(
                OldTex(f"P({r}) = 0", font_size=24).next_to(rect, UP, SMALL_BUFF)
        ...
        for rect, arrow, eq in zip(rects, arrows, zeros_eqs):
            self.play(
                ShowCreation(rect), FadeIn(eq), ShowCreation(arrow), FadeOut(to_fade)
            )
            self.wait(2)
        ...
        root_terms = VGroup(
            *(VGroup(m1, m2) for m1, m2 in zip(factored[3::5], factored[4::5]))
        )
        ...
        cleaner_expanded.shift(
            expanded[0][0].get_center() - cleaner_expanded[0][0][0].get_center()
        )
        ...
        for alt_roots in [(2, 4, 1), (4, 2, 1), (1, 4, 2), (1, 2, 4)]:
            ...
            movers, targets = [
                ...
                for groups in [
                    (factored, *cleaner_expanded),
                    (alt_factored, *alt_expanded),
                ]
            ]
        ...
        full_factored.target.next_to(
            coef_poly.target, DOWN, buff=0.75, aligned_edge=LEFT
        )
        ...
        gen_expanded = expanded_poly_tex(
            rs, vertical=False, root_colors=[YELLOW, GREEN]
        )
        ...
        for i in range(1, 4):
            self.play(
                ...
                cleaner_expanded[i + 1 :].animate.next_to(
                    gen_expanded[i], RIGHT, SMALL_BUFF
                ),
            )
            self.wait()
        ...
        q_poly_factored = factored_poly_tex(
            [f"r_{i}" for i in range(5)], root_colors=[YELLOW, GREEN]
        )
        ...
        new_graph = axes.get_graph(
            lambda x: y_scale_factor * poly(x, q_coefs), x_range=(-1.2, 6.2)
        )
        ...
        root_tuple_groups = VGroup(
            *(
                VGroup(*(VGroup(*tup) for tup in it.combinations(root_terms, k)))
                for k in range(1, 6)
            )
        )
        ...
        self.play(
            FadeIn(question), ShowCreation(arrow), FlashAround(gen_factored, run_time=3)
        )
        self.wait()
        ...
        for dec, mob, vect in (const_dec, system[2][0], RIGHT), (
            top_const_dec,
            answer[-1][1],
            LEFT,
        ):
        ...
        root_eqs = VGroup(
            *(
                VGroup(
                    OldTex(f"r_{i} ", "="), DecimalNumber(root, num_decimal_places=3)
                ).arrange(RIGHT)
...
class FactsAboutRootsToCoefficients(RootCoefScene):
    ...
    def construct(self):
        ...
        t2c[f_name] = GREY_A
        mvf = OldTex(
            f"{f_name}(c_0, c_1, c_2)\\\\",
            "=\\\\",
            "\\left\\{r_0, r_1, r_2\\right\\}",
    ...
    def add_graph(self):
        ...
        graph = always_redraw(
            lambda: axes.get_graph(lambda x: poly(x, self.get_coefs()).real).set_stroke(
                BLUE, 2
            )
        )

        root_dots = GlowDot()
        root_dots.add_updater(
            lambda d: d.set_points(
                [axes.c2p(r.real, 0) for r in self.get_roots() if abs(r.imag) < 1e-5]
            )
        )
    ...
    def add_system(self):
        ...
        self.play(
            LaggedStart(
                *(TransformFromCopy(c, line[0]) for c, line in zip(c_parts, system)),
                **kw,
            ),
            LaggedStart(*(FadeIn(line[1:], lag_ratio=0.1) for line in system), **kw),
        )
        self.add(system)
    ...
    def add_solver_functions(self):
        ...
        lines = VGroup(
            *(OldTex(f"r_{i} = {func_name}_{i}(c_0, c_1, c_2)", **kw) for i in range(3))
        )
        lines.scale(0.8)
        ...
        self.play(
            ...
            LaggedStart(*(FadeIn(line[1:], lag_ratio=0.1) for line in lines), **kw),
        )
        self.add(lines)
            ...
            self.play(
                *(
                    TransformMatchingTex(
                        l1, l2, path_arc=PI / 2, lag_ratio=0.01, run_time=2
                    )
...
class SolvabilityChart(Scene):
    ...
    def get_lines(self, rows, cols, color=GREY_A, width=2):
        ...
        col_lines[0].match_height(
            Group(row_lines, Point(col_lines.get_bottom())), about_edge=DOWN
        )
    ...
    def get_marks(self, equations, solvability_words):
        ...
        marks = VGroup(
            *(
                VGroup(*(Checkmark() if pm == "c" else Exmark() for pm in pm_list))
    ...
    def get_axes(self, frame):
        ...
        axes.add(
            OldTex("x", font_size=24).next_to(axes.x_axis.get_right(), DOWN, SMALL_BUFF)
        )
        axes.add(
            OldTex("y", font_size=24).next_to(axes.y_axis.get_top(), LEFT, SMALL_BUFF)
        )
        return axes
...
class StudySqrt(RadicalScene):
    ...
    def construct(self):
        ...
        option = VGroup(
            ...
            OldTexText(
                "\\\\Make",
                " $\\sqrt{\\quad}$",
                " single-valued, but discontinuous",
                font_size=36,
            ),
        )
        ...
        sqrt_dot.add_updater(
            lambda d: d.move_to(self.root_plane.n2p(np.sqrt(self.get_c())))
        )
    ...
    def show_angle_variation(self, c_label, r_label, arrow):
        ...
        left_exp_label, right_exp_label = (
            self.get_exp_label(get_theta=func, color=angle_color).move_to(label[-1], DL)
            for label, func in [
                (c_label, get_theta),
        ...
        angle_labels = VGroup(
            self.get_angle_label(
                self.coef_dots[0], self.coef_plane, "\\theta", get_theta
            ),
            self.get_angle_label(
                self.root_dots[0],
                self.root_plane,
...
class CubeRootBehavior(StudySqrt):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            right_arrow_group.animate.to_edge(UP), *map(FadeIn, (c_label, r_label))
        )
