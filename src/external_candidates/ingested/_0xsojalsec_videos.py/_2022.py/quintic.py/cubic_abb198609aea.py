...
class CubicFormula(RootCoefScene):
    ...
    def add_lower_planes(self):
        ...
        sqrt_label = OldTex(
            "\\delta_1, \\delta_2 = \\sqrt{ \\frac{q^2}{4} + \\frac{p^3}{27}}", **kw
        )
    ...
    def add_c_labels(self):
        self.c_dot_labels = self.add_dot_labels(
            VGroup(*map(Tex, ["q", "p", "0"])), self.coef_dots
        )
    ...
    def add_sqrt_dots(self):
        ...
        self.delta_labels = self.add_dot_labels(
            VGroup(OldTex("\\delta_1"), OldTex("\\delta_2")), sqrt_dots
        )
    ...
    def get_cube_root_values(self):
        return [
            [self.crt_plane.p2n(d.get_center()) for d in triple]
    ...
    def add_cf_dots(self):
        ...
        alt_root_dots.add_updater(
            lambda m: m.set_points(list(map(self.cf_plane.n2p, self.get_roots())))
        )
