# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2022\zeta\part1.py
import sympy
...
class ShowPrimeDensity(InteractiveScene):
    def construct(self):
        ...
        for n in range(x_max):
            if sympy.isprime(n):
                t = smooth()
                all_prime_animations.append(
                    self.get_prime_animation(
                        numberline, n, run_time=11, time_span=(t, t + 1)
                    )
                )
