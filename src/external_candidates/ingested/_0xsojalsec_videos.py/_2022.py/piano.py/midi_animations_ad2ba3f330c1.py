...
class AnimatedMidi(Scene):
    ...
    def add_note_rects(self):
        ...
        for msg in track:
            ...
            if msg.type == "note_on" and msg.velocity > 0:
                ...
                rect.set_fill(
                    self.note_color, opacity=clip(0.25 + velocity / 100, 0, 1)
                )
                note_rects.add(rect)
                notes_to_time_spans[msg.note].append(
                    (start_time_in_ticks * sec_per_tick, time_in_ticks * sec_per_tick)
                )
    ...
    def scroll(self):
        ...
        def update_piano(piano, dt):
            ...
            for note, key in zip(piano_midi_range, piano):
                ...
                if hit:
                    ...
                    key.set_fill(
                        interpolate_color(key[0].get_fill_color(), self.hit_color, 0.5)
                    )
                else:
        ...
        note_rects.add_updater(
            lambda m: m.move_to(note_rects_start + self.dist_per_sec * self.time * DOWN)
        )
