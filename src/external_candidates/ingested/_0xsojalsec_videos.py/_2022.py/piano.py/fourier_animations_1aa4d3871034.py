...
class SumOfWaves(Scene):
    def construct(self):
        ...
        axes_labels = VGroup(
            *(Text(word, font_size=30) for word in ["Air pressure", "Time"])
        )
        ...
        low_axes_group = VGroup(
            *(Axes((0, 12), (-1, 1), height=0.65) for freq in freqs)
        )
        ...
        waves = VGroup(
            *(get_wave_sum(la, [freq]) for la, freq in zip(low_axes_group, freqs))
        )
        waves.set_submobject_colors_by_gradient(BLUE, YELLOW)
        waves.set_stroke(width=2)

        axes_labels = VGroup(
            *(Text(f"{int(mult * 220)} Hz", font_size=24) for mult in freq_multiples)
        )
        ...
        for index in range(len(waves)):
            ...
            wave.add_updater(
                lambda w: w.set_height(
                    amp_tracker.get_value()[w.index] * w.max_height, stretch=True
                )
            )
...
class DecomposeAudioSegment(Scene):
    ...
    def construct(self):
        ...
        self.zoom_in_on_segment(
            self.axes, self.graph, self.graph_point, self.zoom_rect_dims
        )
        self.prepare_for_3d()
    ...
    def zoom_in_on_segment(
        ...
        for mover in movers:
            ...
            mover.target.stretch(
                frame.get_width() / zoom_rect.get_width(), 0, about_point=point
            )
            mover.target.stretch(
                frame.get_height() / zoom_rect.get_height(), 1, about_point=point
            )
    ...
    def break_down_into_fourier_components(self):
        ...
        fft_graph.set_points_as_corners(
            [f_axes.c2p(freq, 2 * value) for freq, value in zip(freqs, signal_fft_abs)]
        )
        ...
        glow_keys = VGroup(
            *(
                piano[
                    np.argmin(
                        [get_norm(k.get_center() - wave.get_left()) for k in piano]
                    )
                ]
        ...
        peak_dots = GlowDots(
            [
                lines[
                    np.argmin(
                        [get_norm(line.get_start() - wave.get_left()) for line in lines]
                    )
                ].get_end()
                for wave in top_waves
            ]
        ...
        self.play(
            ...
            ApplyMethod(
                glow_keys.set_fill, RED, 1, rate_func=squish_rate_func(smooth, 0, 0.2)
            ),
            run_time=6,
        )
        self.wait()
        ...
        self.play(
            ...
            *(ReplacementTransform(wave, approx_wave) for wave in top_waves),
            LaggedStartMap(
                FadeOut, VGroup(fft_graph, lines, fft_label, freq_label, f_axes)
            ),
            FadeOut(peak_dots),
...
class WaveformDescription(DecomposeAudioSegment):
    def construct(self):
        ...
        dot = GlowDot()
        dot.add_updater(
            lambda d: d.move_to(axes.i2gp(axes.x_axis.p2n(line.get_x()), graph))
        )
        self.add(line, dot)
...
class SignalsAsVectors(DecomposeAudioSegment):
    audio_file = os.path.join(
        DATA_DIR, "audio_clips", "SignalFromSpeech.wav"
    )  # Change?
    sample_density = 1.0
    ...
    def describe_fourier_basis(self):
        ...
        self.play(
            ...
            LaggedStart(
                ...
                *(
                    ...
                    for point, mob in zip(
                        dots.get_points()[:3], vector.get_entries()[:3]
                    )
                ),
                ...
                *(
                    GrowFromPoint(mob, point)
                    for point, mob in zip(
                        dots.get_points()[-2:], vector.get_entries()[-2:]
                    )
                ),
            ),
            run_time=3,
        ...
        fourier_basis = np.array(
            [np.fft.ifft(basis) for basis in np.identity(values.size)]
        )
...
class SampleRateOverlay(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            GrowFromPoint(text[1], text[0].get_bottom()), FadeIn(text[2][0], DOWN)
        )
        self.wait()
