...
class SimpleSphereQuestion(InteractiveScene):
    def construct(self):
        # Intro statement
        text = OldTexText(
            "A ``proof'' of a formula for\\\\the surface area of a sphere"
        )
...
class SphereExample(InteractiveScene):
    ...
    def construct(self):
        ...
        slices.scale(radius)
        slice_highlights = (
            slices[0][len(slices[0]) // 4 : 3 * len(slices[0]) // 4]
            .copy()
            .set_color(YELLOW)
        )
...
class Proof2Slide(InteractiveScene):
    def construct(self):
        ...
        radial_line.add_updater(
            lambda m: m.set_angle(TAU * (0.75 - get_t())).shift(
                circle.get_center() - m.get_start()
            )
        )
...
class CircleExample(InteractiveScene):
    ...
    def construct(self):
        ...
        self.play(
            TransformFromCopy(width_line, semi_circ, path_arc=-PI / 2, run_time=2)
        )
        self.wait()
    ...
    def get_sectors(self, circle, n_slices=20, fill_colors=[BLUE_D, BLUE_E]):
        ...
        sectors = VGroup(
            *(
                Sector(
                    angle=angle, start_angle=i * angle, fill_color=color, fill_opacity=1
                )
...
class SphereSectorAnalysis(Scene):
    ...
    def construct(self):
        ...
        width_label.add_updater(
            lambda m: m.next_to(width_line, OUT, buff=SMALL_BUFF).set_width(
                1.5 * width_line.get_width() + 1e-2
            )
        )
        ...
        def get_angle_label():
            arc = Arc(
                start_angle=90 * DEGREES, angle=-get_phi(), radius=0.5, n_components=8
            )
        ...
        angle_label[0].add_updater(
            lambda m: m.match_points(get_angle_label()[0])
            .set_stroke(WHITE, 2)
            .set_fill(opacity=0)
        )
        ...
        lat_line_label.add_updater(
            lambda m: m.next_to(lat_line_radius, OUT, SMALL_BUFF)
        )
        ...
        # Exact formula
        formula1 = OldTex(
            "2\\pi", " R \\sin(\\phi)", "\\cdot", "{\\Delta \\theta", " \\over 2\\pi}"
        )
        ...
        axes.y_axis.add(
            Text("Wedge width", font_size=24, color=RED).next_to(axes.c2p(0, 1), LEFT)
        )
        axes.x_axis.add(
            OldTex("\\pi / 2", font_size=24).next_to(axes.c2p(PI / 2, 0), DOWN)
        )
        axes.x_axis.add(OldTex("\\pi", font_size=24).next_to(axes.c2p(PI, 0), DOWN))
        axes.x_axis.add(
            OldTex("\\phi", font_size=24).next_to(axes.c2p(PI, 0), UR, buff=SMALL_BUFF)
        )
        axes.to_corner(UL)
        axes.fix_in_frame()
...
class ContrastSphericalGeometry(InteractiveScene):
    def construct(self):
        ...
        sph_tri = VGroup(
            ...
            VMobject()
            .pointwise_become_partial(mesh[26], 16 / 20, 19 / 20)
            .reverse_points(),
        )
        ...
        area_formulas = VGroup(
            Tex(
                "\\text{Area}(\\Delta) = (\\alpha + \\beta + \\gamma - \\pi) R^2", **kw
            ),
...
class SquareCircleExample(InteractiveScene):
    def construct(self):
        ...
        circle.set_stroke(WHITE, 1)
        approx_curves = [
            self.get_square_approx(rich_circle, 4 * 2**n) for n in range(10)
        ]
        ...
        points = [
            square.get_edge_center(np.round(vect)) for vect in compass_directions(8)
        ]
        new_radii = VGroup(
            *(
        ...
        self.play(
            TransformFromCopy(
                VGroup(radial_line), new_radii, lag_ratio=0.2, run_time=2
            ),
        ...
        for i, curve in enumerate(approx_curves[1:]):
            ...
            self.play(
                square.animate.set_stroke(width=1), TransformFromCopy(square, curve)
            )
            self.wait()
            if i == 0:
...
class UpshotOfLimitExample(InteractiveScene):
    def construct(self):
        ...
        folder = (
            "/Users/grant/Dropbox/3Blue1Brown/videos/2022/visual_proofs/lies/images/"
        )
        rational_example = VGroup(numbers, properties)
        circle_example = Group(
            Group(
                *(
                    ImageMobject(os.path.join(folder, "SquareApprox" + end)).set_width(
                        1.25
                    )
        ...
        all_rects = VGroup(
            *(
                axes.get_riemann_rectangles(graph, (0, 3), dx).set_stroke(
                    BLACK, np.round(4 * dx, 1), behind=False
                )
...
class FalseEuclidProofAnnotation(InteractiveScene):
    def construct(self):
        ...
        self.play(
            AB.animate.set_angle(-90 * DEGREES).next_to(
                A, RIGHT, aligned_edge=UP, buff=2
            ),
            AC.animate.set_angle(-90 * DEGREES).next_to(
                A, RIGHT, aligned_edge=UP, buff=2.5
            ),
        )
        self.wait()
...
class FalseEuclidFollowup(InteractiveScene):
    def construct(self):
        ...
        tri3 = tri2.copy().set_height(
            math.sqrt(3) * tri2.get_width() / 2, stretch=True, about_edge=DOWN
        )
        ...
        def get_side(i, j, tri=tri):
            return Line(tri.get_vertices()[i], tri.get_vertices()[j]).set_stroke(
                YELLOW, 4
            )
...
class SideSumTruthiness(InteractiveScene):
    def construct(self):
        ...
        E.add_updater(
            lambda m: m.move_to(
                interpolate(A.get_center(), C.get_center(), alpha_tracker.get_value())
            )
        )
        ...
        labels[0].add_updater(
            lambda m: m.set_opacity(1 if 0 < alpha_tracker.get_value() < 1 else 0)
        )
        labels[1].add_updater(
            lambda m: m.set_opacity(0 if 0 < alpha_tracker.get_value() < 1 else 1)
        )
    ...
    def construct(self):
        ...
        self.play(
            ...
            side_labels.animate.scale(
                scale_factor, about_point=pre_tri.get_center()
            ).shift(shift_vect),
        )
        ...
        self.play(
            Transform(
                tris, tris.saved_state, lag_ratio=0.2, run_time=3, path_arc=45 * DEGREES
            ),
