...
class NormalFunctionPreview(InteractiveScene):
    def construct(self):
        ...
        VGroup(axes2d, curve, label, label3d).rotate(
            PI / 2, RIGHT, about_point=axes3d.get_origin()
        )
        frame.reorient(0, 90)
...
class HoldUpImageProcessing(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait(2)
        self.play(
            self.change_students("pondering", "thinking", "pondering", look_at=2 * UP)
        )
        self.wait(6)
...
class OtherVisualizations(TeacherStudentsScene):
    def construct(self):
        self.play(
            self.change_students("happy", "thinking", "tease", look_at=self.screen)
        )
        self.play(
...
class AskForExample(TeacherStudentsScene):
    def construct(self):
        self.play(
            ...
            self.change_students(
                "pondering", "thinking", "confused", look_at=self.screen
            ),
        )
        self.wait(3)
        self.play(
...
class SobelKernelLabel(MarioConvolutionLabel, SobelFilter2):
    ...
    def construct(self):
        for square in self.kernel_array:
            ...
            square.set_fill(
                rgb_to_color([max(-value, 0), max(value, 0), max(value, 0)]), 1
            )
...
class MakeAPrediction(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.change_students(
                "pondering", "confused", "thinking", look_at=self.screen
            )
        )
        self.wait()
...
class ThinkDifferently(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(self.teacher.change("tease"))
        self.play(
            self.change_students("thinking", "pondering", "tease", look_at=self.screen)
        )
        self.wait(4)
...
class DumbIdea(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            self.change_students(
                "pondering", "pondering", "frustrated", look_at=self.screen
            ),
        )
        self.wait()
        self.student_says(
            "Uh...", target_mode="hesitant", bubble_direction=LEFT, index=2
        )
