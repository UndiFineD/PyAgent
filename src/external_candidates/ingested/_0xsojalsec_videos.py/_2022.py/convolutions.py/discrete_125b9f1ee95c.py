...
class WaysToCombine(InteractiveScene):
    def construct(self):
        ...
        # Operation labels
        op_labels = VGroup()
        left_op_labels = VGroup(
            *map(Text, ["Addition", "Multiplication", "Convolution"])
        )
        left_op_labels.set_color(TEAL)
        for left_op_label, seq, graph_label in zip(
            left_op_labels, seqs[2:], graph_labels[2:]
        ):
        ...
        self.play(
            LaggedStartMap(
                FadeOut, VGroup(*boxes, *box_arrows, mystery_box, q_marks, comb_graph)
            )
        )
        ...
        for seq, label, axes, graph, op_label in tuples:
            self.play(
                LaggedStart(
                    TransformMatchingShapes(
                        VGroup(*seq1_tex[:2], *seq2_tex[:2]).copy(), seq[:4]
                    ),
            ...
            self.play(
                ...
                *(
                    UpdateFromAlphaFunc(
                        dot, lambda d, a: d.set_opacity(min(10 * a * (1 - a), 1))
                    )
                    for dot in dots
                ),
                *(
        ...
        self.wait()
        self.play(
            MoveAlongPath(GlowDot(color=WHITE), conv_graph, run_time=5, remover=True)
        )
        self.wait()
...
class DiceExample(InteractiveScene):
    def construct(self):
        ...
        self.play(
            ...
            *(
                die.animate.set_width(0.6 * square.get_width()).next_to(
                    square, UP, SMALL_BUFF
                )
                for die, square in zip(blue_dice, grid[:6])
            ),
            *(
                die.animate.set_width(0.6 * square.get_width()).next_to(
                    square, LEFT, SMALL_BUFF
                )
        ...
        for n in range(2, 13):
            ...
            pairs = VGroup(
                *(
                    VGroup(m1, m2)
                    for m1, m2 in zip(to_highlight[::2], to_highlight[1::2])
                )
            )
            num = len(pairs)
            ...
            if n <= 4:
                self.play(
                    LaggedStart(
                        *(
                            FlashAround(
                                pair.copy(), remover=True, time_width=1, run_time=1.5
                            )
                            for pair in pairs
                        ),
                        lag_ratio=0.2,
        ...
        while n < 13:
            ...
            if n == 4 and isinstance(blue_dice[0].prob_label, DecimalNumber):
                # Make more general
                blue_labels = VGroup(
                    *(OldTex(f"a_{{{i}}}", font_size=42) for i in range(1, 7))
                )
                red_labels = VGroup(
                    *(OldTex(f"b_{{{i}}}", font_size=42) for i in range(1, 7))
                )
        ...
        pairss = VGroup(
            *(self.get_aligned_pairs(blue_dice, red_dice, n) for n in n_range)
        )
        rhss = VGroup(
            *(self.get_conv_rhs(pairs, lhs) for pairs, lhs in zip(pairss, lhss))
        )
        prob_labels = VGroup()
        ...
        self.play(
            LaggedStartMap(
                ShowCreationThenFadeOut, rhs_rects, lag_ratio=0.1, run_time=5
            )
        )
        self.wait()
        ...
        for dice, squares, vect in (blue_dice, grid[:6], UP), (
            red_dice,
            grid[::6],
            LEFT,
        ):
        ...
        self.wait()
        self.play(
            Restore(blue_dice), Restore(red_dice), diagonals.animate.set_opacity(1)
        )
        ...
        self.play(
            ShowSubmobjectsOneByOne(
                pair_rects, remover=True, rate_func=linear, run_time=3
            )
        )
        self.wait()
    ...
    def get_p_sum_expr(self, n, rhs=" "):
        raw_expr = Tex(
            Rf"P\big(O + O = {n}\big) = \, {rhs}", isolate=("O", str(n), "=", rhs)
        )
...
class SimpleExample(InteractiveScene):
    def construct(self):
        ...
        for n in range(len(conv_result)):
            ...
            c_label = OldTex(f"c_{n}", font_size=30, color=YELLOW).next_to(
                rhs[2 * n + 2], UP
            )

            rects = VGroup(
                *(
                    SurroundingRectangle(lp, buff=0.2)
                    .set_stroke(YELLOW, 1)
                    .round_corners()
        ...
        ovals[0].become(
            Circle(radius=radius).match_style(ovals[0]).move_to(products[0])
        )
        ...
        self.play(
            FadeIn(polynomial_eq, DOWN), grid_group.animate.center().to_edge(DOWN)
        )
        self.wait()
        ...
        eq_values2 = VGroup(
            polynomial_eq[11], polynomial_eq[13:15], polynomial_eq[16:19]
        )
        ...
        for values, eq_values in [(values1, eq_values1), (values2, eq_values2)]:
            self.play(
                LaggedStart(
                    *(
                        TransformMatchingShapes(ev.copy(), v.target)
                        for ev, v in zip(eq_values, values)
                    )
                ),
        ...
        arrows = VGroup(
            *(
                Vector(0.5 * UP).next_to(
                    polynomial_eq.select_part(f"{{{n}}}"), DOWN, buff=SMALL_BUFF
                )
...
class MovingAverageExample(InteractiveScene):
    ...
    def construct(self):
        ...
        for n, conv_bar in enumerate(conv_bars):
            ...
            rects = VGroup(
                *(
                    SurroundingRectangle(lp, buff=0.05)
                    .set_stroke(YELLOW, 2)
                    .round_corners()
                    for lp in label_pairs
                )
...
class CompareSizes(InteractiveScene):
    def construct(self):
        ...
        braces = VGroup(
            *(
                Brace(arr, vect, buff=SMALL_BUFF)
                for arr, vect in zip(arrays, [UP, DOWN, DOWN])
            )
        )
        brace_labels = VGroup(
            *(
                brace.get_tex(tex, buff=SMALL_BUFF)
                for brace, tex in zip(braces, ["n", "m", "n + m - 1"])
            )
        )
...
class BigPolynomials(InteractiveScene):
    def construct(self):
        ...
        a_terms[-2].become(
            OldTex(R"\cdots", font_size=24).move_to(a_terms[-2]).shift(0.02 * DOWN)
        )
        ...
        for i, row in enumerate(grid):
            ...
            for j, square in enumerate(row):
                ...
                if j == N - 1:
                    j = 99
                term = OldTex(
                    f"a_{{{j}}}", f"b_{{{i}}}", f"x^{{{i + j}}}", font_size=20
                )
                if i + j == 0:
        ...
        product_count = OldTexText(
            R"$100 \times 100 = 10{,}000$ \\ products", font_size=60
        )
...
class FunctionToCoefficientCommutativeDiagram(InteractiveScene):
    def construct(self):
        # Axes
        axes1, axes2, axes3 = all_axes = VGroup(
            *(Axes((-3, 3), (-2, 4), width=6, height=4) for x in range(3))
        )
...
class DataPointsToPolynomial(InteractiveScene):
    def construct(self):
        ...
        dots = Group(
            *(GlowDot(axes.c2p(x, y), glow_factor=0.8, radius=0.1) for x, y in data)
        )
        dots.set_color(TEAL)
        circles = VGroup(
            *(Circle(radius=0.075).set_stroke(YELLOW, 2).move_to(dot) for dot in dots)
        )
        ...
        self.play(
            ...
            LaggedStartMap(
                ShowCreationThenFadeOut, circles[:2], lag_ratio=0.2, run_time=1
            ),
        )
        ...
        for n in range(2, len(data) - 1):
            self.play(
                ...
                FadeIn(
                    poly_tex[2 * n - 1 : 2 * n + 1], time_span=(0, 1), lag_ratio=0.1
                ),
                run_time=2,
            )
            self.wait()
...
class PolynomialSystem(InteractiveScene):
    ...
    def construct(self):
        ...
        boxes = VGroup(
            *(VGroup(SurroundingRectangle(const, buff=0)) for const in consts)
        )
        boxes.set_stroke(RED, 1)

        self.play(
            FadeOut(words2, LEFT, rate_func=running_start),
            inputs.animate.set_color(RED)
            .shift(2 * LEFT)
            .set_anim_args(run_time=1.5, lag_ratio=0.2, path_arc=PI / 2),
        ...
        all_omega_powers = [
            VGroup(*(Tex(Rf"\omega^{{{(k * n) % N}}}", **kw) for n in range(N)))
            for k in range(0, N)
        ]
        new_rhss = VGroup(
            *(
        ...
        short_label.next_to(h_rect, UP, buff=0.5).shift(
            sub_label.get_width() * LEFT / 2
        )
    ...
    def construct(self):
        ...
        for k in range(max_power - 1):
            ...
            self.play(
                ...
                Transform(
                    root_dots[k % N].copy(), root_dots[(k + 1) % N], remover=True, **kw
                ),
...
class AlgorithmOutline(InteractiveScene):
    def construct(self):
        ...
        t2c.update(
            {tex: TEAL for tex in [R"\textbf{b}", "b_0", "b_1", "b_2", "b_{m - 1}"]}
        )
        ...
        polys = VGroup(
            Tex(
                R"f(x) = a_0 + a_1 x + a_2 x^2 + \cdots + a_{n - 1}x^{n - 1}", **tex_kw
            ),
            Tex(
                R"g(x) = b_0 + b_1 x + b_2 x^2 + \cdots + b_{m - 1}x^{m - 1}", **tex_kw
            ),
        )
        ...
        f_dots, g_dots, fg_dots = all_dots = Group(
            *(
                Group(
                    *(
                        GlowDot(point, color=WHITE, glow_factor=0.8, radius=0.07)
                        for point in points
                    )
                )
        ...
        self.play(
            FadeIn(f_dots, lag_ratio=0.7), FadeIn(g_dots, lag_ratio=0.7), run_time=5
        )
        self.wait()
        self.play(
        ...
        mult_arrow = Vector(2 * DOWN).move_to(
            VGroup(fft_lists[1], fft_lists[2]).get_center()
        )
