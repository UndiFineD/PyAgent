# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\dominos\domino_play.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class SimpleVelocityGraph(GraphScene):
    ...
    def init_data(self, data_file):
        file_name = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "dominos", data_file
        )
...
class ContrastTwoGraphs(SimpleVelocityGraph):
    ...
    def construct(self):
        ...
        self.play(ShowCreation(graph1, run_time=3, rate_func=linear), Write(felt))
        self.play(ShowCreation(graph2, run_time=4, rate_func=linear), Write(hardwood))
        self.wait()
        ...
        self.play(
            dot2.shift, MED_SMALL_BUFF * UP, dot2.set_color, PINK, rate_func=wiggle
        )
        self.wait()
        self.play(
            FadeOut(arrow),
        ...
        self.wait()
        self.play(Transform(graph1, smoothed_graph1, run_time=2), Animation(rect))
        ...
        self.play(
            LaggedStartMap(FadeIn, felt), LaggedStartMap(FadeIn, hardwood), run_time=1
        )
        self.wait()

        # Compare regions
        dot_group1 = VGroup(*graph1.dots[35:75] + graph2.dots[35:75])
        dot_group2 = VGroup(*graph1.dots[75:] + graph2.dots[75:])
        dot_group3 = VGroup(*graph1.dots[35:] + graph2.dots[35:])
    ...
    def get_smoothed_velocity_graph(self, velocities):
        ...
        smoothed_vs = np.array(
            [np.mean(velocities[max(0, i - n) : i]) for i in range(len(velocities))]
        )
    ...
    def get_dot(self, x, y):
        dot = Dot(radius=0.05)
        dot.move_to(self.coords_to_point(x * float(self.x_max) / self.n_data_points, y))
        return dot
...
class ShowAllSteadyStateVelocities(SimpleVelocityGraph):
    ...
    def construct(self):
        ...
        for index in range(1, 20):
            ...
            self.init_data(data_file)
            color = WHITE if self.friction == "low" else BLUE
            label = OldTexText(index_str, color=color)
            label.scale(0.5)
            label.set_color(color)
            ...
            dot.scale(0.5)
            dot.move_to(
                self.coords_to_point(self.domino_spacing, self.velocities[-1] - 400)
            )
            label.next_to(dot, random.choice([LEFT, RIGHT]), SMALL_BUFF)
            self.add(dot)
            self.add(label)
...
class Test(Scene):
    def construct(self):
        ...
        domino1 = Rectangle(
            width=0.5, height=3, stroke_width=0, fill_color=GREEN, fill_opacity=1
        )
        domino1.shift(LEFT)
