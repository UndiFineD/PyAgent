# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\leibniz.py
...
class LatticePointScene(Scene):
    ...
    def get_radial_line_with_label(self, radius=None, color=None):
        ...
        radial_line = Line(
            self.plane_center, self.plane.coords_to_point(radius, 0), color=color
        )
    ...
    def add_axis_labels(self, spacing=2):
        ...
        x_ticks = VGroup(
            *[tick.copy().move_to(self.plane.coords_to_point(x, 0)) for x in x_range]
        )
        tick.rotate(-np.pi / 2)
        y_ticks = VGroup(
            *[tick.copy().move_to(self.plane.coords_to_point(0, y)) for y in y_range]
        )
    ...
    def introduce_three_objects(self):
        ...
        self.play(
            ReplacementTransform(
                morty.body, pi_group.get_part_by_tex("pi"), run_time=1
            ),
...
class ShowSum(TeacherStudentsScene):
    ...
    def show_sum(self):
        ...
        numbers = [0] + [
            ((-1) ** n) / (2.0 * n + 1) for n in range(self.num_terms_to_add)
        ]
        ...
        fading_terms = [
            ...
            for n, sign in zip(list(range(self.num_terms_to_add)), it.cycle("+-"))
        ]
...
class ShowCalculus(PiCreatureScene):
    def construct(self):
        ...
        rhs_group.shift(
            frac_sum.get_right() + MED_SMALL_BUFF * RIGHT - int1[0].get_left()
        )
...
class Outline(PiCreatureScene):
    ...
    def wonder_at_pi(self):
        ...
        self.wait(2)
        self.play(
            RemovePiCreatureBubble(self.pi_creature), question.to_corner, UP + RIGHT
        )
    ...
    def count_lattice_points(self):
        ...
        circle = Circle(
            color=YELLOW,
            radius=get_norm(plane.coords_to_point(10, 0) - plane.coords_to_point(0, 0)),
        )
        ...
        self.play(
            *[
                ...
                for dot, a in zip(
                    lattice_points, np.linspace(0, 0.75, len(lattice_points))
                )
            ]
        )
        ...
        self.wait()
        self.play(
            lattice_group.set_height,
            2.5,
            lattice_group.next_to,
            self.question,
            DOWN,
            lattice_group.to_edge,
            RIGHT,
        )
...
class CountLatticePoints(LatticePointScene):
    ...
    def draw_lattice_points_in_circle(self):
        ...
        radius_label = brace.get_text(
            str(self.max_lattice_point_radius), buff=SMALL_BUFF
        )
    ...
    def write_pi_R_squared(self):
        equations = VGroup(
            *[
                VGroup(
                    OldTexText(
                        "\\# Lattice points\\\\", "within radius ", R, alignment=""
                    ),
                    OldTex("\\approx \\pi", "(", R, ")^2"),
                ).arrange(RIGHT)
...
class CountThroughRings(LatticePointScene):
    ...
    def count_through_rings(self):
        ...
        left_list = VGroup(
            *[
                OldTex("\\sqrt{%d} \\Rightarrow" % n, str(count))
                for n, count in zip(
                    list(range(self.num_rings_to_show_explicitly)), counts
                )
            ]
        )
        left_counts = VGroup()
    ...
    def show_ring_count(self, radius_squared, target, added_anims=None, run_time=1):
        ...
        radial_line = Line(
            self.plane_center, self.plane.coords_to_point(radius, 0), color=RED
        )
...
class LookAtExampleRing(LatticePointScene):
    ...
    def analyze_25(self):
        ...
        sums_of_squares = [
            OldTex(special_str(x), "^2", "+", special_str(y), "^2", "= 25")
        ...
        for label, sum_of_squares in zip(labels, sums_of_squares)[1:]:
            self.play(
                ...
                label.point.set_color,
                PINK,
                curr_label.point.set_color,
                self.dot_color,
            )
            curr_label = label
...
class IntroduceComplexConjugate(LatticePointScene):
    ...
    def write_points_with_complex_coords(self):
        ...
        ticks = VGroup(
            *[
                Line(ORIGIN, MED_SMALL_BUFF * RIGHT).move_to(
    ...
    def expand_algebraically(self):
        ...
        self.wait()
        self.play(
            *list(map(FadeOut, [expansion[0].rect, expansion[1].rect, expansion]))
        )
    ...
    def show_geometrically(self):
        ...
        for dot in dots:
            ...
            dot.magnitude_word.next_to(
                ORIGIN, np.sign(dot.get_center()[1]) * UP, buff=SMALL_BUFF
            )
        ...
        self.play(
            ReplacementTransform(top_dot.arc, low_dot.arc), FadeIn(low_dot.rotate_word)
        )
        self.play(
            Rotate(mover, low_dot.angle, about_point=self.plane_center), run_time=2
        )
        self.play(
            FadeOut(low_dot.arc),
...
class NameGaussianIntegers(LatticePointScene):
    ...
    def restrict_to_one_circle(self):
        ...
        self.play(
            ...
            ShowCreation(circle),
            Rotating(
                radius, run_time=1, rate_func=smooth, about_point=self.plane_center
            ),
    ...
    def construct(self):
        ...
        for alt_rhs in alt_rhs_list:
            ...
            if alt_rhs is alt_rhs_list[-2]:
                self.play_student_changes(
                    *["sassy"] * 3, look_at=alt_rhs, added_anims=[Write(title[0])]
                )
            self.wait(2)
...
class IntroduceGaussianPrimes(LatticePointScene, PiCreatureScene):
    ...
    def construct(self):
        ...
        times_neg_i_arc = Arrow(
            p2_dot.get_bottom(), p4_dot.get_bottom(), path_arc=-arc_angle
        )
        ...
        for alt_factorization in neg_alt_factorization, i_alt_factorization:
            alt_factorization.next_to(
                factorization.get_part_by_tex("="), DOWN, aligned_edge=LEFT
            )
...
class FromIntegerFactorsToGaussianFactors(TeacherStudentsScene):
    def construct(self):
        ...
        for mob in two, five:
            self.play(
                ReplacementTransform(mob.copy(), mob.factors),
        ...
        self.teacher_says(
            "Now for a \\\\ surprising fact...", added_anims=[FadeOut(group)]
        )
        self.wait(2)
...
class FactorizationPattern(Scene):
    ...
    def ask_why_this_is_true(self):
        ...
        self.wait(2)
        self.play(
            *list(map(FadeOut, [randy, randy.bubble, randy.bubble.content, links_text]))
        )
    ...
    def show_two(self):
        ...
        time_i_arrow = Arrow(
            factors[1].get_bottom(), factors[0].get_bottom(), path_arc=-np.pi
        )
        times_i = OldTex("\\times i")
        # times_i.scale(1.5)
...
class FactorTwo(LatticePointScene):
    ...
    def construct(self):
        ...
        self.play(
            *[
                ReplacementTransform(VGroup(mob1.copy()), mob2)
                for mob1, mob2 in [
...
class IntroduceRecipe(Scene):
    ...
    def subfactor_ordinary_factorization(self):
        ...
        self.play(
            LaggedStartMap(
                MoveToTarget, movers, replace_mobject_with_target_in_scene=True
            )
        )
        self.wait()
    ...
    def mention_conjugate_rule(self):
        ...
        for lf, rf in zip(left_factors.target, right_factors.target):
            arrow = DoubleArrow(
                lf, rf, buff=SMALL_BUFF, tip_length=SMALL_BUFF, color=GREEN
            )
...
class ThreeOutputsAsLatticePointsContinued(ThreeOutputsAsLatticePoints):
    ...
    def show_multiplication_by_units(self):
        ...
        for dot in original_dots:
            ...
            dot.add(line)
        words_group = VGroup(
            *[OldTexText("Multiply by $%s$" % s) for s in ("1", "i", "-1", "-i")]
        )
        ...
        for words, angle, color in zip(words_group[1:], angles, self.colors[1:]):
            ...
            self.play(
                dots_copy.rotate, angle, dots_copy.set_color, color, path_arc=angle
            )
            self.wait()
        self.wait(2)
...
class Show125Circle(ThreeOutputsAsLatticePointsContinued):
    ...
    def draw_circle(self):
        ...
        self.add_foreground_mobject(root_label)
        self.play(
            Rotating(radial_line, rate_func=smooth, about_point=self.plane_center),
            ShowCreation(circle),
            run_time=2,
        )
...
class SummarizeCountingRule(Show125Circle):
    ...
    def draw_circle(self):
        ...
        self.play(
            LaggedStartMap(DrawBorderThenFill, dots, stroke_width=4, stroke_color=PINK)
        )
        self.wait(2)
    ...
    def talk_through_rules(self):
        factorization = OldTex(
            "N =", "3", "^4", "\\cdot", "5", "^3", "\\cdot", "13", "^2"
        )
        factorization.next_to(ORIGIN, RIGHT)
        factorization.to_edge(UP)

        three, five, thirteen = [
            factorization.get_part_by_tex(str(n), substring=False) for n in (3, 5, 13)
        ]
        ...
        self.wait()
        for choices, power in (five_choices, five_power), (
            thirteen_choices,
            thirteen_power,
        ):
            self.play(
        ...
        for mob in movers:
            mob.save_state()
        self.play(
            Transform(three_power, alt_three_power, path_arc=np.pi),
        ...
        final_choice_arrow = Arrow(
            final_choice_words[0].get_top(), four.get_bottom(), buff=SMALL_BUFF
        )
...
class RecipeFor10(IntroduceRecipe):
    ...
    def swap_two_factors(self):
        ...
        self.play(
            Transform(arrow, new_arrow), MaintainPositionRelativeTo(times_i, arrow)
        )
        self.wait(2)
...
class EffectOfPowersOfTwo(LatticePointScene):
    ...
    def construct(self):
        ...
        dots_list = list(
            map(self.get_lattice_points_on_r_squared_circle, self.square_radii)
        )
        groups = [
            VGroup(*mobs) for mobs in zip(radial_lines, circles, root_labels, dots_list)
        ]
        group = groups[0]
        ...
        self.play(
            LaggedStartMap(
                DrawBorderThenFill, dots_list[0], stroke_width=4, stroke_color=PINK
            )
        )
        self.wait()
...
class IntroduceChi(FactorizationPattern):
    CONFIG = {
        "numbers_list": [list(range(i, 36, d)) for i, d in [(1, 4), (3, 4), (2, 2)]],
    ...
    def cyclic_pattern(self):
        ...
        self.play(
            *[FadeIn(mob, run_time=3, lag_ratio=0.5) for mob in [chis, arrows, numbers]]
        )
    ...
    def show_multiplicative(self):
        ...
        for x, y in pairs:
            expression = OldTex(
                "\\chi(%d)" % x, "\\cdot", "\\chi(%d)" % y, "=", "\\chi(%d)" % (x * y)
            )
    ...
    def introduce_dots_arrows_and_labels(self, dots, arrows, labels):
        if hasattr(self, "last_dots"):
            ...
            self.play(
                ...
                LaggedStartMap(
                    DrawBorderThenFill, dots, stroke_width=4, stroke_color=YELLOW
                ),
                run_time=2,
            )
        self.last_dots = dots
    ...
    def get_chi_expression(self, numbers, color, num_terms=4):
        ...
        result = OldTex(
            *it.chain(
                *[["\\chi(", str(n), ")", "="] for n in truncated_numbers]
                + [["\\cdots =", output]]
            )
        )
...
class WriteCountingRuleWithChi(SummarizeCountingRule):
    ...
    def add_factorization_and_rule(self):
        ...
        count.next_to(
            factorization.get_part_by_tex("="), DOWN, buff=LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        self.play(
            *[
                ReplacementTransform(
                    VGroup(factorization.get_part_by_tex(tex, substring=False)).copy(),
                    part,
                )
    ...
    def write_chi_expression(self):
        ...
        expression.shift(
            self.count[0].get_center() + LARGE_BUFF * DOWN - equals_four.get_center()
        )
    ...
    def walk_through_expression_terms(self):
        ...
        groups = [
            ...
            for index, tex1, tex2 in [(-1, "5", "^3"), (-2, "3", "^4"), (-3, "2", "^2")]
        ]
        evaluation_strings = [
            "(1+1+1+1)",
            "(1-1+1-1+1)",
        ...
        for group, tex in zip(groups, evaluation_strings):
            ...
            if base.get_tex() is "3":
                ...
                self.play(
                    ...
                    Transform(VGroup(*evaluation[-3:-1]), evaluation_point),
                    evaluation[-1].next_to,
                    evaluation_point,
                    RIGHT,
                    SMALL_BUFF,
...
class CountLatticePointsInBigCircle(LatticePointScene):
    ...
    def ignore_center_dot(self):
        ...
        self.play(
            ApplyMethod(
                VGroup(self.circle, self.radius).scale,
                2,
                rate_func=linear,
            ),
            LaggedStartMap(
...
class AddUpGrid(Scene):
    ...
    def add_row_lines(self):
        ...
        row_lines = VGroup(
            *[
                h_line.copy().next_to(radical, DOWN, buff=SMALL_BUFF, aligned_edge=LEFT)
                for radical in self.radicals
            ]
        )
        row_lines[-2].shift(
            row_lines[-1].get_left()[0] * RIGHT - row_lines[-2].get_left()[0] * RIGHT
        )
    ...
    def talk_through_rows(self):
        ...
        composite_rects, prime_rects = [
            VGroup(*[rect.copy().move_to(self.radicals[N - 1], LEFT) for N in numbers])
    ...
    def collapse_columns(self):
        ...
        for column, term in zip(chi_mob_columns, full_sum):
            ...
            self.wait()
            self.play(
                ReplacementTransform(column.copy(), VGroup(term[1]), run_time=2),
                Write(term[0]),
                Write(term[2]),
            )
            self.wait()
    ...
    def factor_out_R(self):
        ...
        self.wait()
        self.play(
            MoveToTarget(self.corner_four, run_time=2), FadeOut(self.corner_four.rect)
        )
        self.wait(2)
    ...
    def compare_to_pi_R_squared(self):
        ...
        pi.set_color(YELLOW)
        pi_sum.shift(
            self.alt_rhs[-1].get_bottom(), MED_SMALL_BUFF * DOWN, -pi_sum[1].get_top()
        )
...
class Thumbnail(Scene):
    def construct(self):
        ...
        primes = [
            n for n in range(2, 1000) if all(n % k != 0 for k in list(range(2, n)))
        ]
        prime_mobs = VGroup()
        x_spacing = 1.7
        y_spacing = 1.5
