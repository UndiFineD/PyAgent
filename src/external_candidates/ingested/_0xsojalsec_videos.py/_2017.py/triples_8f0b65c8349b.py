    ...
    def construct(self):
        ...
        for a, b, c in triples:
            ...
            if c in [5, 13, 25]:
                ...
                i_list = [
                    i for i in range(c**2) if (i % c) in keys and (i // c) in keys
                ]
            else:
            ...
            for start, target in zip([a_square, b_square], c_square_parts):
                ...
                self.play(
                    ReplacementTransform(mover, target, run_time=2, path_arc=np.pi / 2)
                )
...
class ShowManyTriples(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                FadeIn, VGroup(*[title.target for title in titles[17:]]), run_time=5
            )
        )
...
class AskAboutFavoriteProof(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "What's you're \\\\ favorite proof?", target_mode="raise_right_hand"
        )
...
class PythagoreanProof(Scene):
    ...
    def show_proof(self):
        ...
        self.add_new_triangles(
            triangle, self.get_added_triangles_to_c_square(triangle, c_square)
        )
        self.wait()
        self.add_new_triangles(
            triangle_copy,
...
class ReframeOnLattice(PiCreatureScene):
    ...
    def resize_plane(self):
        ...
        self.plane.target.shift(
            new_plane.coords_to_point(0, 0) - self.plane.target.coords_to_point(0, 0)
        )
    ...
    def mention_squaring_it(self):
        ...
        self.play(
            RemovePiCreatureBubble(
                morty, target_mode="pondering", look_at=self.example_label
            )
        )

    def work_out_square_algebraically(self):
        rect = Rectangle(
            height=3.5, width=6.5, stroke_width=0, fill_color=BLACK, fill_opacity=0.8
        )
        ...
        result_dot = Dot(
            self.plane.coords_to_point(3, 4), color=MAROON_B, radius=self.dot_radius
        )
...
class OneMoreExample(Scene):
    ...
    def square_algebraically(self):
        ...
        second_line = OldTex(
            "\\big( 3^2 + (2i)^2 \\big) + " + "\\big(3 \\cdot 2 + 2 \\cdot 3 \\big)i"
        )
        for i in 1, 12, 18:
    ...
    def show_triangle(self):
        triangle = Polygon(
            *[self.plane.coords_to_point(x, y) for x, y in [(0, 0), (5, 0), (5, 12)]]
        )
        ...
        self.play(
            ...
            Animation(
                VGroup(
                    self.line, self.dot, self.number_label[1], *self.distance_label[1:]
                )
            ),
            run_time=2,
        )
...
class WriteGeneralFormula(GeneralExample):
    ...
    def show_uv_to_triples(self):
        ...
        self.play(
            *[
                LaggedStartMap(FadeIn, mob, run_time=5, lag_ratio=0.2)
...
class VisualizeZSquared(Scene):
    ...
    def add_plane(self):
        ...
        coordinate_labels = VGroup()
        z_list = np.append(
            self.x_label_range, complex(0, 1) * np.array(self.y_label_range)
        )
        for z in z_list:
            if z == 0:
                continue
    ...
    def draw_arrows(self):
        ...
        for z in z_list:
            z_point, square_point, mid_point = [
                self.background_plane.number_to_point(z**p) for p in (1, 2, 1.5)
            ]
...
class AskAboutHittingAllPoints(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "Does this hit \\\\ all pythagorean triples?", target_mode="raise_left_hand"
        )
        self.wait()
...
class PointsWeMiss(VisualizeZSquared):
    ...
    def add_dots(self):
        z_list = [
            complex(x, y) ** 2 for x in self.dot_x_range for y in self.dot_y_range
        ]
        dots = VGroup(
            *[
                Dot(
    ...
    def mention_one_half_rule(self):
        ...
        self.play(
            PiCreatureSays(
                morty, "Never need to scale \\\\ by less than $\\frac{1}{2}$"
            )
        )
        self.play(Blink(morty))
        self.wait(2)
...
class PointsWeMissAreMultiplesOfOnesWeHit(TeacherStudentsScene):
    def construct(self):
        words = OldTexText(
            "Every point we", "miss", "is \\\\ a multiple of one we", "hit"
        )
...
class DrawRadialLines(PointsWeMiss):
    ...
    def resize_plane(self):
        everything = VGroup(*self.get_top_level_mobjects())
        everything.scale(
            self.final_unit_size / self.initial_unit_size, about_point=self.plane_center
        )
        self.background_plane.set_stroke(width=1)
    ...
    def show_all_lines(self):
        ...
        self.play(
            ReplacementTransform(seed_dots, self.lines, run_time=3, lag_ratio=0.5)
        )
        self.play(
            LaggedStartMap(
                DrawBorderThenFill,
...
class RationalPointsOnUnitCircle(DrawRadialLines):
    ...
    def from_rational_point_to_triple(self):
        ...
        self.play(
            ReplacementTransform(self.curr_example_point_group, rational_point_group)
        )
        self.wait(2)
        self.play(
            *[
    ...
    def get_unit_circle(self):
        template_line = Line(
            *[self.background_plane.number_to_point(z) for z in (-1, 1)]
        )
...
class ProjectPointsOntoUnitCircle(DrawRadialLines):
    ...
    def add_unit_circle(self):
        template_line = Line(
            *[self.background_plane.number_to_point(n) for n in (-1, 1)]
        )
    ...
    def zoom_in(self):
        ...
        self.play(
            group.shift, -self.plane_center, group.scale, scale_factor, run_time=2
        )
        self.wait(2)

    def draw_infinitely_many_lines(self):
        lines = VGroup(
            *[Line(ORIGIN, FRAME_WIDTH * vect) for vect in compass_directions(1000)]
        )
...
class FinalProof(RationalPointsOnUnitCircle):
    ...
    def square_example_point(self):
        ...
        z_to_z_squared_arrow = Arrow(
            point, square_point, path_arc=np.pi / 3, color=WHITE
        )
        z_to_z_squared = OldTex("z", "\\to", "z^2")
        z_to_z_squared.set_color_by_tex("z", YELLOW)
        z_to_z_squared.set_color_by_tex("z^2", MAROON_B)
        ...
        z_to_z_squared.next_to(
            z_to_z_squared_arrow.point_from_proportion(0.5), RIGHT, SMALL_BUFF
        )
