...
class ProductRuleDiagram(Scene):
    def construct(self):
        ...
        for mob in df_brace, df_label, dg_brace, dg_label:
            mob.save_state()
            mob.scale(
                0.01, about_point=rect.get_corner(mob.get_center() - rect.get_center())
            )
    ...
    def show_calculus_symbols(self):
        ...
        self.play(
            ReplacementTransform(
                self.area.get_part_by_tex("R").copy(), ftc.get_part_by_tex("int")
            ),
            self.pi_creature.change_mode,
            "plain",
        )
        ...
        self.play(
            ReplacementTransform(
                self.area.get_part_by_tex("Area").copy(), ftc.get_part_by_tex("frac")
            ),
            ReplacementTransform(
                self.area.get_part_by_tex("R").copy(), ftc.get_part_by_tex("\\,dr")
            ),
        )
        self.wait()
...
class ApproximateOneRing(CircleScene, ReconfigurableScene):
    ...
    def approximate_as_rectangle(self):
        ...
        self.play(
            ...
            ReplacementTransform(
                self.ring_radius_group[1].copy(), width_label.get_part_by_tex("r")
            ),
        )
        self.wait()
...
class GraphRectangles(CircleScene, GraphScene):
    ...
    def unwrap_rings_onto_graph(self):
        ...
        self.play(
            *[
                ...
                for ring, alpha in zip(
                    transformed_rings, np.linspace(0, 0.75, len(transformed_rings))
                )
            ]
        ...
        self.play(
            *[
                ApplyMethod(ring.replace, ring.rect, method_kwargs={"stretch": True})
    ...
    def draw_graph(self):
        graph_label = self.get_graph_label(
            self.graph, "2\\pi r", direction=UP + LEFT, x_val=2.5, buff=SMALL_BUFF
        )
            ...
            *[
                Transform(
                    ...
                    rate_func=squish_rate_func(
                        lambda t: 0.1 * there_and_back(t), alpha, alpha + 0.5
                    ),
                    lag_ratio=0.5,
                )
                for rect, flat_rect, alpha in zip(
                    self.rects, self.flat_rects, np.linspace(0, 0.5, len(self.rects))
                )
            ]
    ...
    def compute_area_under_graph(self):
        ...
        base_line = Line(
            self.coords_to_point(0, 0), self.coords_to_point(3, 0), color=YELLOW
        )
    ...
    def get_ring_sum(self, rings):
        ...
        for ring, alpha in zip(middle_rings, alphas):
            ...
            ring.target.move_to(
                interpolate(dots.get_left(), last_ring.target.get_center(), alpha)
            )
...
class RecapCircleSolution(GraphRectangles, ReconfigurableScene):
    ...
    def show_sum(self):
        ...
        for ring, radius in zip(visible_rings, radii):
            label = OldTex(
                "\\approx", "2\\pi", "(%s)" % str(radius), "(%s)" % str(self.dR)
            )
        ...
        arrows = VGroup(
            *[
                Arrow(area_label.get_left(), approximation.get_right(), color=WHITE)
        ...
        self.play(
            ...
            FadeIn(radial_lines),
            *[
                ReplacementTransform(area_label.copy(), VGroup(*approximation[1:]))
                for approximation in approximations
            ],
        )
        self.wait()
        self.play(
            Write(VGroup(*[approximation[0] for approximation in approximations]))
        )
        self.wait()
    ...
    def show_riemann_sum(self):
        graph = self.get_graph(lambda r: 2 * np.pi * r)
        graph_label = self.get_graph_label(
            graph, "2\\pi r", x_val=2.5, direction=UP + LEFT
        )
...
class ExampleIntegralProblems(PiCreatureScene, GraphScene):
    ...
    def show_graph(self):
        ...
        rects = self.get_riemann_rectangles(
            v_graph, x_min=0, x_max=self.t_max, dx=self.dt
        )
...
class MathematicianPonderingAreaUnderDifferentCurves(PiCreatureScene):
    def construct(self):
        ...
        self.wait(4)
        self.play(
            self.pi_creature.change_mode, "pondering", self.pi_creature.look, UP + LEFT
        )
        self.wait(2)
...
class WhoCaresAboutArea(TeacherStudentsScene):
    def construct(self):
        ...
        self.play_student_changes(
            *["pondering"] * 3, look_at=point, added_anims=[self.teacher.look_at, point]
        )
        self.wait(3)
...
class PlayingTowardsDADX(AreaUnderParabola, ReconfigurableScene):
    ...
    def describe_sliver(self):
        ...
        difference_in_area = OldTexText(
            "d", "ifference in ", "A", "rea", arg_separator=""
        )
    ...
    def shrink_dx(self, **kwargs):
        self.transition_to_alt_config(
            deriv_dx=0.05, transformation_kwargs={"run_time": 2}, **kwargs
        )
    ...
    def write_example_inputs(self):
        ...
        for label, line, vect in labels_lines_vects:
            ...
            label.arrow = Arrow(
                label, point, buff=SMALL_BUFF, color=WHITE, tip_length=0.15
            )
            line_copy = line.copy()
    ...
    def show_dA_dx_in_detail(self):
        ...
        self.play(
            ...
            ReplacementTransform(
                self.graph_label_copy.copy(), VGroup(*expression[-2:]), run_time=2
            ),
        )
        self.wait()
    ...
    def show_smaller_x(self):
        self.transition_to_alt_config(
            default_right_x=2, deriv_dx=0.04, transformation_kwargs={"run_time": 2}
        )
...
class AlternateAreaUnderCurve(PlayingTowardsDADX):
    ...
    def approximation_improves_for_smaller_dx(self):
        ...
        self.wait()
        self.transition_to_alt_config(
            deriv_dx=self.deriv_dx / 4.0, transformation_kwargs={"run_time": 2}
        )
