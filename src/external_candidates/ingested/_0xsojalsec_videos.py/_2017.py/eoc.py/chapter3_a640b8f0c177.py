...
class ContrastAbstractAndConcrete(Scene):
    ...
    def get_spring_anim(self, alignement_mob):
        ...
        for spring in compact_spring, extended_spring:
            ...
            spring.shift(
                alignement_mob.get_center()[1] * UP
                + SMALL_BUFF * RIGHT
                - spring.get_points()[0]
            )
            weight = Square(
...
class DerivativeOfXSquaredAsGraph(GraphScene, ZoomedScene, PiCreatureScene):
    ...
    def ask_about_df_dx(self):
        ...
        v_line, nudged_v_line = [
            self.get_vertical_line_to_graph(
                x, self.graph, line_class=DashedLine, color=RED, dash_length=0.025
            )
        ...
        self.play(
            Write(derivative_q, run_time=1), self.pi_creature.change_mode, "speaking"
        )
        self.wait()
    ...
    def show_differing_slopes(self):
        ...
        self.play(
            ShowCreation(ss_group.secant_line), self.pi_creature.change_mode, "thinking"
        )
...
class NudgeSideLengthOfSquare(PiCreatureScene):
    ...
    def write_df_equation(self):
        ...
        df_equation.arrange()
        df_equation.next_to(
            self.function_label, DOWN, aligned_edge=LEFT, buff=SMALL_BUFF
        )
        ...
        self.wait()
        self.play(
            *[mob.restore for mob in (r1, r2, s)]
            + [Write(symbol) for symbol in (equals, plus1, plus2)],
            run_time=2,
        )
    ...
    def examine_thin_rectangles(self):
        ...
        for group, tex, color in (xs, "3", self.three_color), (
            dxs,
            "0.01",
            self.dx_color,
        ):
    ...
    def examine_tiny_square(self):
        ...
        self.play(
            Transform(text, alt_text), self.pi_creature.change_mode, "raise_right_hand"
        )
        self.wait(2)
        self.play(
            *[mob.restore for mob in (self.dxs, text)]
    ...
    def show_smaller_dx(self):
        ...
        alt_scene = self.__class__(
            skip_animations=True, dx=self.alt_dx, is_recursing_on_dx=True
        )
...
class NudgeSideLengthOfCube(Scene):
    ...
    def introduce_cube(self):
        ...
        self.play(
            *[
                ApplyMethod(piece.shift, 0.5 * (piece.get_center() - cube.get_center()))
                for piece in dv_pieces
            ]
    ...
    def write_df_equation(self):
        ...
        for submob, sides in zip(x_squared_dx, [face[0], VGroup(*face[1:])]):
            self.play(
                submob.set_color, RED, sides.set_color, RED, rate_func=there_and_back
            )
            self.wait()
        self.play(
            face.restore,
            Transform(
                x_squared_dx, derivative, replace_mobject_with_target_in_scene=True
            ),
        ...
        self.play(
            *[
                ...
                for mob in [plus1, bars, plus2, corner_cube, extras_brace, ignore_text]
            ]
        )
        self.wait()
    ...
    def shrink_dx(self, state_name, restore=True):
        ...
        self.play(
            *[
                Transform(*pair)
                for pair in zip(movers, self.alt_scene.states[state_name])
            ]
        )
        self.wait()
        if restore:
...
class GraphOfXCubed(GraphScene):
    ...
    def construct(self):
        ...
        deriv_label = self.get_graph_label(
            deriv_graph, "\\frac{df}{dx}(x) = 3x^2", x_val=-3, direction=LEFT
        )
        ...
        for x_val in -2, -self.dx / 2, 2:
            ...
            if x_val != -self.dx / 2:
                v_line = self.get_vertical_line_to_graph(
                    x_val, deriv_graph, line_class=DashedLine
                )
                self.play(ShowCreation(v_line))
...
class PatternForPowerRule(PiCreatureScene):
    ...
    def introduce_pattern(self):
        ...
        for d1, d2 in zip(derivatives, derivatives[1:]):
            self.play(
                Transform(d1.copy(), d2, replace_mobject_with_target_in_scene=True)
            )
        self.change_mode("thinking")
        self.wait()
...
class PowerRuleAlgebra(Scene):
    ...
    def workout_product(self, equals, full_product):
        ...
        xs, dxs = [
            VGroup(
                *[submob for submob, tex in product_part_tex_pairs if tex == target_tex]
            )
    ...
    def pull_out_linear_terms(self, x_to_n, product_part_tex_pairs, xs, dxs):
        ...
        for dx_index, dx in enumerate(dxs):
            if dx is dxs[-1]:
                ...
                h_dots_list = [
                    submob for submob, tex in product_part_tex_pairs if tex == "\\cdots"
                ]
...
class OneOverX(PiCreatureScene, GraphScene):
    ...
    def perform_nudge(self):
        ...
        negative_arrow = Arrow(
            negative.get_left(), df_label.get_corner(UP + RIGHT), color=RED
        )
        ...
        for color, point_pair in zip([self.dx_color, self.df_color], point_pairs):
            ...
            area_change_rect.replace(
                VGroup(*list(map(VectorizedPoint, point_pair))), stretch=True
            )
        ...
        area_gained_label.next_to(
            rect_copy.get_corner(DOWN + RIGHT), UP + LEFT, buff=SMALL_BUFF
        )
        area_gained_arrow = Arrow(
            area_gained_label.get_top(), area_gained.get_center(), buff=0, color=WHITE
        )
        ...
        area_lost_arrow = Arrow(
            area_lost_label.get_top(), area_lost.get_center(), buff=0, color=WHITE
        )
    ...
    def get_rectangle_group(self, x, x_label="x", one_over_x_label="\\frac{1}{x}"):
        ...
        result.x_brace, result.recip_brace = braces = [
            Brace(result.rectangle, vect) for vect in (UP, RIGHT)
        ]
...
class SquareRootOfX(Scene):
    ...
    def introduce_square(self):
        ...
        bottom_brace, right_brace = braces = VGroup(
            *[Brace(square, vect) for vect in (DOWN, RIGHT)]
        )
        for brace in braces:
    ...
    def nudge_square(self):
        ...
        self.play(
            *it.chain(list(map(ShowCreation, lines)), list(map(FadeIn, little_braces)))
        )
...
class DerivativeOfSineIsSlope(Scene):
    def construct(self):
        tex = OldTex(
            "\\frac{d(\\sin(\\theta))}{d\\theta} = ", "\\text{Slope of this graph}"
        )
...
class IntroduceUnitCircleWithSine(GraphScene):
    ...
    def draw_example_radians(self):
        ...
        theta_copy.target.next_to(
            arc_copy.target, RIGHT, aligned_edge=DOWN, buff=SMALL_BUFF
        )
    ...
    def draw_derivative_from_slopes(self):
        ...
        while last_theta < self.x_max:
                ...
                added_anims=[
                    ShowCreation(
                        ...
                        rate_func=lambda t: interpolate(
                            last_theta / self.x_max, next_theta / self.x_max, smooth(t)
                        ),
                        run_time=3,
                    ),
                    UpdateFromFunc(
                        v_line, lambda v: self.v_line_update(v, deriv_copy), run_time=3
                    ),
                ],
            )
            self.wait()
            if next_theta == 2 * np.pi:
                ...
                words.to_edge(UP)
                arrow = Arrow(
                    words.get_bottom(), deriv_graph.point_from_proportion(0.45)
                )
...
class DerivativeFromZoomingInOnSine(IntroduceUnitCircleWithSine, ZoomedScene):
    ...
    def perform_nudge(self):
        ...
        d_theta_label.next_to(
            d_theta_brace.get_center(), d_theta_brace.direction, MED_LARGE_BUFF
        )
