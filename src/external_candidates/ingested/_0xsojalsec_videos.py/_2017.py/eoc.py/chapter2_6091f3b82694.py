...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.teacher_says(
            "It's actually a \\\\", "very subtle idea", target_mode="well"
        )
...
class GraphCarTrajectory(GraphScene):
    ...
    def ask_critically_about_velocity(self):
        ...
        self.play(
            PiCreatureSays(morty, "Think critically about \\\\", "what velocity means.")
        )
...
class ShowSpeedometer(IntroduceCar):
    ...
    def setup(self):
        ...
        needle = Polygon(
            LEFT, UP, RIGHT, stroke_width=0, fill_opacity=1, fill_color=YELLOW
        )
...
class CompareTwoTimes(Scene):
    ...
    def show_equation(self):
        ...
        VGroup(
            VGroup(
                *formula[-2 - seconds_len - et_len - st_len : -2 - seconds_len - st_len]
            ),
...
class VelocityAtIndividualPointsVsPairs(GraphCarTrajectory):
    ...
    def construct(self):
        ...
        distance_label = self.label_graph(
            distance_graph, label="s(t)", proportion=1, direction=RIGHT, buff=SMALL_BUFF
        )
    ...
    def show_individual_times_to_velocity(self, velocity_graph):
        ...
        self.wait()
        self.play(
            UpdateFromAlphaFunc(line, line_update, run_time=4, rate_func=there_and_back)
        )
        self.wait()
    ...
    def show_two_times_on_distance(self):
        ...
        for line, brace, text in (dt_line, dt_brace, dt_text), (
            ds_line,
            ds_brace,
            ds_text,
        ):
...
class DsOverDtGraphically(GraphCarTrajectory, ZoomedScene):
    ...
    def construct(self):
        ...
        self.play(
            ApplyFunction(align_little_rectangle_on_ds_dt_group, self.little_rectangle)
        )
        ...
        v_line_at_t, v_line_at_t_plus_dt = [
            self.get_vertical_line_to_graph(
                time, line_class=Line, line_kwargs={"color": MAROON_B}
            )
        ...
        self.play(
            ...
            UpdateFromFunc(
                self.little_rectangle, align_little_rectangle_on_ds_dt_group
            ),
            run_time=6,
        )
        ...
        def sin_wiggle(t):
            return (10 / (2 * np.pi / 10.0)) * (
                np.sin(2 * np.pi * t / 10.0) + 2 * np.pi * t / 10.0
            )
        ...
        for graph_func in graph_funcs:
            new_graph = self.graph_function(
                graph_func, color=DISTANCE_COLOR, is_main_graph=False
            )
            self.remove(new_graph)
            new_velocity_graph = self.get_derivative_graph(
                graph=new_graph,
...
class DefineTrueDerivative(Scene):
    ...
    def get_fraction(self, dt_string):
        ...
        result = VGroup(
            *[VGroup(*tex_mob[i1:i2]) for i1, i2 in zip(pl_cumsum, pl_cumsum[1:])]
        )
...
class SecantLineToTangentLine(GraphCarTrajectory, DefineTrueDerivative):
    ...
    def what_this_is_not_saying(self):
        phrases = [
            OldTexText("$dt$", "is", "not", s) for s in ("``infinitely small''", "0")
        ]
    ...
    def best_constant_approximation_around_a_point(self):
        ...
        circle = Circle(radius=0.25, color=WHITE).shift(
...
class TCubedExample(SecantLineToTangentLine):
    ...
    def draw_graph(self):
        ...
        self.label_graph(
            graph, label="s(t) = t^3", proportion=0.62, direction=LEFT, buff=SMALL_BUFF
        )
        self.wait()
    ...
    def show_expansion(self):
        ...
        terms = [
            ...
            for i1, i2 in zip([0] + list(np.cumsum(term_lens)), np.cumsum(term_lens))
        ]
        ...
        self.play(
            *it.chain(
                *[
                    ...
                    for pi, mode in zip(
                        self.get_students(), ["pondering", "happy", "hesitant"]
                    )
                ]
            )
        )
...
class ParadoxAtTEquals0(TCubedExample):
    ...
    def draw_graph(self):
        ...
        self.label_graph(
            graph, label="s(t) = t^3", proportion=0.8, direction=RIGHT, buff=SMALL_BUFF
        )
        self.wait()
...
class NextVideos(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            GrowFromCenter(brace), self.get_teacher().change_mode, "raise_right_hand"
        )
        self.play(
            Write(brace_text),
