# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter4.py
    ...
    def construct(self):
        ...
        for part, mode in zip(parts, modes):
            self.play(
                ...
                self.get_teacher().change_mode,
                "raise_right_hand",
                *[ApplyMethod(pi.change_mode, mode) for pi in self.get_students()],
            )
        self.wait()
        self.play_student_changes(*["happy"] * 3)
        words = list(
            map(
                TexText, ["composition", "product", "composition", "sum", "composition"]
            )
        )
...
class DampenedSpring(Scene):
    def construct(self):
        ...
        self.play(
            UpdateFromAlphaFunc(
                moving_spring, update_spring, run_time=10, rate_func=linear
            )
        )
        self.wait()
...
class PreSumRuleDiscussion(Scene):
    def construct(self):
        ...
        for indices in [(1, 2, 3), (6,), (7, 8)]:
            self.play(
                *[ReplacementTransform(specific[i].copy(), general[i]) for i in indices]
            )
            self.wait()
        ...
        for i in 1, 3, -1, 6, 8:
            if i < 0:
                ...
                part = specific[i]
                self.play(
                    part.set_color, YELLOW, part.scale, 1.2, rate_func=there_and_back
                )
        self.wait()
...
class SumRule(GraphScene):
    ...
    def zoom_in_on_graph(self):
        graph_parts = VGroup(
            self.axes, self.sine_graph, self.parabola, self.sum_graph, *self.v_line_sets
        )
    ...
    def show_example_stacking(self):
        ...
        self.wait()
        self.play(
            *it.chain(*[[line.restore, line.brace_anim] for line in example_v_lines])
        )
        ...
        self.play(
            ...
            *it.chain(
                *[[line.restore, line.set_stroke, None, 4] for line in nudged_v_lines]
            ),
        )
        self.wait()
        for line in nudged_v_lines:
    ...
    def show_df(self):
        ...
        self.play(
            ...
            (parabola_lines[1].get_bottom()[1] - parabola_lines[0].get_bottom()[1])
            * UP,
    ...
    def expand_derivative(self):
        ...
        expanded_deriv.next_to(
            self.deriv_mob, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        ...
        self.play(
            FadeIn(background_rect, rate_func=squish_rate_func(smooth, 0.6, 1)),
        ...
        self.play(
            *[
                Transform(expanded_deriv[i], rearranged_deriv[j], path_arc=-np.pi / 2)
...
class DiscussProducts(TeacherStudentsScene):
    def construct(self):
        ...
        wrong_product_rule.next_to(
            self.get_teacher().get_corner(UP + LEFT), UP, buff=MED_LARGE_BUFF
        ).shift_onto_screen()

        self.teacher_says("Products are a bit different", target_mode="sassy")
        self.wait(2)
        self.play(
            RemovePiCreatureBubble(self.get_teacher(), target_mode="raise_right_hand")
        )
        ...
        self.play_student_changes(
            ...
            "confused",
            "erm",
            added_anims=[not_equals.scale, 1.3, not_equals.set_color, RED],
        )
        self.wait()
        self.teacher_says(
            ...
            added_anims=[
                wrong_product_rule.scale,
                0.7,
                wrong_product_rule.to_corner,
                UP + LEFT,
            ],
        )
...
class NotGraphsForProducts(GraphScene):
    ...
    def construct(self):
        ...
        sine_graph.label = self.get_graph_label(
            sine_graph, "\\sin(x)", x_val=3 * np.pi / 2, direction=DOWN
        )
        parabola = self.get_graph(lambda x: x**2, color=X_SQUARED_COLOR)
        ...
        product_graph.label = self.get_graph_label(
            product_graph, "\\sin(x)x^2", x_val=2.8, direction=UP + RIGHT, buff=0
        )
...
class IntroduceProductAsArea(ReconfigurableScene):
    ...
    def talk_though_sine(self):
        ...
        self.play(
            Rotate(y_axis, np.pi / 2, about_point=y_axis.get_left()), Animation(x_axis)
        )
        ...
        for x, rt in zip([0.25, np.pi / 2, 3, self.default_x], [2, 4, 4, 2]):
            self.animate_x_change(
                x, run_time=rt, added_anims=[UpdateFromFunc(v_line, v_line_update)]
            )
            self.wait()
        self.play(
            *it.chain(
                list(map(FadeOut, [y_axis, graph, label, v_line])), [Animation(x_axis)]
            )
        )
        self.wait()
    ...
    def define_f_of_x(self):
        f_def = OldTex(
            "f(x)", "=", self.top_func_label, self.side_func_label, "=", "\\text{Area}"
        )
    ...
    def get_df_boxes(self):
        ...
        df_boxes = VGroup(
            *[
                Rectangle(height=height, width=width, **self.df_box_kwargs).next_to(
                    box, vect, buff=0
                )
    ...
    def write_df(self):
        ...
        self.play(
            ...
            df_boxes_copy.set_fill,
            None,
            self.df_box_kwargs["fill_opacity"],
            df_boxes_copy.next_to,
            deriv[1],
        )
    ...
    def set_color_right_boxes(self):
        self.set_color_boxes_and_label(
            VGroup(*self.df_boxes[1:]), self.df_box_labels[0]
        )

    def set_color_bottom_boxes(self):
        self.set_color_boxes_and_label(
            VGroup(*self.df_boxes[:-1]), self.df_box_labels[1]
        )
    ...
    def write_mneumonic(self):
        ...
        morty.shift(2 * LEFT)
        words = OldTexText(
            "``Left ", "d(Right) ", "+", " Right ", "d(Left)", "''", arg_separator=""
        )
    ...
    def animate_x_change(self, target_x, box_label_group=None, x_slider=None, **kwargs):
        ...
        self.play(
            UpdateFromAlphaFunc(box_label_group, update_box_label_group),
            UpdateFromAlphaFunc(x_slider, update_x_slider),
            *added_anims,
            **kwargs,
        )
...
class ConstantMultiplication(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        self.student_says(
            answer, target_mode="hooray", added_anims=[question.copy().to_edge, UP]
        )
...
class ThreeLinesChainRule(ReconfigurableScene):
    ...
    def nudge_x(self):
        ...
        dx_squared_value.shift(
            dx_squared_brace.text.get_right()
        ...
        self.animate_x_change(
            ...
            added_anims=it.chain(
                [GrowFromCenter(dx_brace)], list(map(ShowCreation, nudge_lines))
            ),
        )
    ...
    def replace_x_squared_with_h(self):
        ...
        self.transition_to_alt_config(
            return_to_original_configuration=False, **new_config
        )
    ...
    def get_line_label(
        self, number_line, x, func, func_label, triangle_color, **spillover_kwargs
    ):
        triangle = RegularPolygon(
            n=3,
            start_angle=-np.pi / 2,
...
class GeneralizeChainRule(Scene):
    def construct(self):
        ...
        self.play(
            Transform(
                example_inner.copy(), d_example_inner, path_arc=-np.pi / 2, remover=True
            )
        )
        self.wait()
        self.play(
            GrowFromCenter(d_example_d_inner_brace), Write(d_example_d_inner_brace.text)
        )
        self.play(
            Transform(
