    ...
    def construct(self):
        ...
        self.play(
            ...
            *self.get_added_movement_anims(
                rate_func=v_rate_func, radians=-(16.0 / 70) * 4 * np.pi / 3
            ),
            run_time=8,
        )
        eight = OldTex("8").move_to(time_label[1])
        self.play(
            Transform(time_label[1], eight, rate_func=squish_rate_func(smooth, 0, 0.5))
        )
        self.wait()
...
class ConstantVelocityPlot(PlotVelocity):
    ...
    def show_product(self):
        ...
        rect.replace(
            VGroup(self.graph, VectorizedPoint(self.graph_origin)), stretch=True
        )
    ...
    def note_units(self):
        x_line, y_line = lines = VGroup(
            *[axis.copy() for axis in (self.x_axis, self.y_axis)]
        )
        ...
        units_of_area = VGroup(
            *[
                square.copy().move_to(self.coords_to_point(x, y), DOWN + LEFT)
                for x in range(8)
                for y in range(10)
            ]
        )
...
class PiecewiseConstantPlot(PlotVelocity):
    ...
    def always_changing(self):
        ...
        def update_group(group, alpha):
            ...
            prop = interpolate(
                self.min_graph_proportion, self.max_graph_proportion, alpha
            )
        ...
        self.play(
            Write(words), ShowCreation(arrow), DrawBorderThenFill(dot), run_time=1
        )
        self.play(
            UpdateFromAlphaFunc(
                group, update_group, rate_func=there_and_back, run_time=5
            )
        )
        self.wait()
    ...
    def compute_distance_on_each_interval(self):
        ...
        flat_rects = self.get_riemann_rectangles(
            self.get_graph(lambda t: 0), x_min=0, x_max=8, dx=1
        )
        rects = rect_list[0]
        rect = rects[1]
    ...
    def approximate_original_curve(self):
        ...
        self.play(
            ...
            *[m.restore for m in (self.v_graph, self.v_graph_label)]
    ...
    def show_specific_rectangle(self):
        ...
        self.play(
            *list(
                map(FadeOut, [group[1] for group in (v_lines, h_lines, height_labels)])
            )
        )
        self.play(
            v_lines[0].set_color,
        ...
        area = OldTexText(
            "7$\\frac{\\text{m}}{\\text{s}}$", "$\\times$", "0.25s", "=", "1.75m"
        )
    ...
    def write_integral_symbol(self):
        ...
        for bound, num in zip(bounds, [0, 8]):
            ...
            self.play(
                bound_copy.scale, 1.5, bound_copy.next_to, point, DOWN, MED_LARGE_BUFF
            )
...
class DontKnowHowToHandleNonConstant(TeacherStudentsScene):
    def construct(self):
        self.play(
            *[ApplyMethod(pi.change, "maybe", UP) for pi in self.get_pi_creatures()]
        )
        self.wait(3)
...
class TellMeThatsNotSurprising(TeacherStudentsScene):
    def construct(self):
        self.teacher_says(
            "Tell me that's \\\\ not surprising!", target_mode="hooray", run_time=1
        )
        self.wait(3)
...
class HowDoesThisHelp(TeacherStudentsScene):
    def construct(self):
        self.student_says(
            "How does this help\\textinterrobang", target_mode="angry", run_time=1
        )
...
class AreaIsDerivative(PlotVelocity, ReconfigurableScene):
    ...
    def write_integral(self):
        ...
        int_arrow, s_arrow = [
            Arrow(mob.get_left(), self.area.get_center(), color=WHITE)
    ...
    def nudge_input(self):
        ...
        self.play(
            ReplacementTransform(
                self.v_graph_label.get_part_by_tex("v").copy(), v_T_label, run_time=2
            )
        )
        self.wait()
...
class FindAntiderivative(Antiderivative):
    ...
    def introduce(self):
        ...
        self.play(
            *[
                ReplacementTransform(
                    rhs[i], expanded_rhs[j], run_time=2, path_arc=np.pi
                )
    ...
    def first_part(self):
        ...
        self.play(
            *[
                ReplacementTransform(
                    t_squared[i].copy(), two_t[1 - i], run_time=2, path_arc=-np.pi / 6.0
                )
                for i in (0, 1)
            ]
        )
    ...
    def second_part(self):
        ...
        part1_group = VGroup(
            self.s_part1, self.v_part1, self.arcs_copy, self.words_copy
        )
        ...
        self.play(
            FadeOut(self.q_marks), FadeIn(t_cubed), self.v_part2.shift, DOWN + RIGHT
        )
        self.play(
            *[
                ReplacementTransform(
...
class LowerBound(AreaIsDerivative):
    ...
    def drag_right_endpoint_to_zero(self):
        ...
        self.play(
            *[
                ReplacementTransform(bound.copy(), VGroup(equals_zero[1]))
                for bound in zero_int_bounds
            ]
        )
        self.wait(2)
    ...
    def show_alternate_antiderivative_difference(self):
        ...
        numbers = [
            OldTex("%d" % d).next_to(self.coords_to_point(d, 0), DOWN, MED_LARGE_BUFF)
            for d in (1, 7)
        ]
    ...
    def get_integral(self, lower_bound, upper_bound):
        result = OldTex(
            "\\int", "^" + upper_bound, "_" + lower_bound, "t(8-t)", "\\,dt"
        )
...
class FundamentalTheorem(GraphScene):
    ...
    def show_f_dx_sum(self):
        ...
        self.play(
            *[
                ApplyMethod(
                    rect.set_fill, None, 1 if rect is start_rect else low_opacity
                )
                for rect in rects
            ]
        )
    ...
    def write_antiderivative(self):
        ...
        for tex in "b", "a":
            self.play(
                ReplacementTransform(
                    self.integral.get_part_by_tex(tex).copy(), rhs.get_part_by_tex(tex)
                )
            )
            self.wait()
        self.wait(2)
    ...
    def show_integral_considering_continuum(self):
        self.play(
            *[ApplyMethod(mob.set_fill, None, 0.2) for mob in (self.deriv, self.rhs)]
        )
            ...
            self.play(
                *[
                    ...
                    for rect, alpha in zip(
                        self.rects, np.linspace(0, 0.8, len(self.rects))
                    )
                ]
            )
        self.wait()
...
class NegativeArea(GraphScene):
    ...
    def show_negative_point(self):
        v_line = self.get_vertical_line_to_graph(
            self.sample_input, self.graph, color=RED
        )
        self.play(ShowCreation(v_line))
        self.wait()
        self.v_line = v_line
