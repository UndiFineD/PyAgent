# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter6.py
...
class SlopeOfCircleExample(ZoomedScene):
    ...
    def talk_through_pythagorean_theorem(self):
        ...
        for line, tex in zip(lines, "5xy"):
            ...
            label.next_to(
                line.get_center(), rotate_vector(UP, line.get_angle()), buff=SMALL_BUFF
            )
        ...
        movers = labels.copy()
        pairs = list(
            zip([movers[1], movers[2], movers[0]], self.circle_equation[1][0:-1:3])
        )
    ...
    def show_dx_and_dy(self):
        ...
        self.wait()
        self.play(
            self.little_rectangle.restore, dot.scale, 1.0 / self.zoom_factor, run_time=2
        )
        self.wait()
...
class NameImplicitDifferentation(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            self.get_teacher().change_mode, "raise_right_hand", ShowCreation(arrow)
        )
        self.play_student_changes(
            *["confused"] * 3,
            look_at=derivative,
...
class RelatedRatesExample(ThreeDScene):
        ...
        self.play(
            ...
            *[
                ReplacementTransform(
                    ...
                    path_arc=np.pi / 2,
                    run_time=3,
                    rate_func=squish_rate_func(smooth, j / 12.0, (j + 6) / 12.0),
                )
    ...
    def let_dt_pass(self):
        ...
        for line, brace, label in (dy_line, dy_brace, dy_label), (
            dx_line,
            dx_brace,
            dx_label,
        ):
            self.play(
                ShowCreation(line), GrowFromCenter(brace), Write(label), run_time=1
            )
        self.wait()
    ...
    def take_derivative_of_lhs(self):
        ...
        lhs_derivative.next_to(
            derivative_scaffold, DOWN, aligned_edge=RIGHT, buff=MED_LARGE_BUFF
        )
        ...
        self.play(
            Indicate(
                VGroup(*list(lhs_derivative[:2]) + list(lhs_derivative[2][:2])),
                run_time=2,
            )
        )
    ...
    def replace_terms_in_final_form(self):
        ...
        new_lhs_derivative.next_to(
            self.lhs_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT
        )
        ...
        def fill_in_equation_part(*indices):
            self.play(
                *[
                    ReplacementTransform(
                        self.lhs_derivative[i].copy(), new_lhs_derivative[i], run_time=2
                    )
                    for i in indices
                ]
            )
    ...
    def write_final_solution(self):
        ...
        solution.next_to(
            self.new_lhs_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=RIGHT
        )
...
class CompareLadderAndCircle(PiCreatureScene, ThreeDScene):
    ...
    def introduce_both_scenes(self):
        ...
        for mobs, vect in (ladder_mobs, LEFT), (circle_mobs, RIGHT):
            ...
            mobs.next_to(
                self.pi_creature.get_corner(UP + vect),
                UP,
                buff=SMALL_BUFF,
                aligned_edge=-vect,
            )
    ...
    def comment_on_ladder_derivative(self):
        ...
        time_derivative = OldTex(
            "2", "x(t)", "\\frac{dx}{dt}", "+", "2", "y(t)", "\\frac{dy}{dt}", "=0"
        )
...
class TwoVariableFunctionAndDerivative(SlopeOfCircleExample):
    ...
    def shift_example_point(self, coords):
        ...
        self.play(
            self.example_point_dot.move_to,
            point,
        ...
        self.play(
            *[
                ReplacementTransform(
                    ...
                    path_arc=np.pi / 2,
                    run_time=2,
                    rate_func=squish_rate_func(smooth, (j - 1) / 12.0, (j + 6) / 12.0),
                )
    ...
    def show_dx_dy_step(self):
        ...
        new_s_label.next_to(
            new_dot, DOWN, buff=SMALL_BUFF / self.zoom_factor, aligned_edge=LEFT
        )
    ...
    def plug_in_example_values(self):
        ...
        def add_example_parts(*indices):
            self.play(
                *[
                    ReplacementTransform(
                        self.derivative[1][i].copy(), deriv_example[1][i], run_time=2
                    )
                    for i in indices
                ]
            )
        ...
        self.play(FadeIn(randy))
        self.play(
            randy.change_mode, "pondering", randy.look_at, deriv_example.get_left()
        )
        self.play(Blink(randy))
...
class AlternateExample(ZoomedScene):
    ...
    def draw_graph(self):
        ...
        self.play(
            ShowCreation(graphs, run_time=3, lag_ratio=0), Animation(self.formula)
        )
        self.wait()
    ...
    def emphasize_meaning_of_points(self):
        ...
        self.play(
            *[
                ApplyMethod(
                    label[1][i].copy().move_to,
                    self.formula[1][j],
                    run_time=3,
                    rate_func=squish_rate_func(
                        smooth, count / 6.0, count / 6.0 + 2.0 / 3
                    ),
                )
        ...
        self.wait()
        self.play(
            *[ApplyMethod(mover.set_fill, None, 0, remover=True) for mover in movers]
        )
    ...
    def show_tiny_step(self):
        ...
        arrow = Arrow(
            point, new_point, buff=0, tip_length=0.15 / self.zoom_factor, color=WHITE
        )
    ...
    def ask_about_derivatives(self):
        ...
        word.add_background_rectangle()
        word.next_to(
            Line(lhs.get_center(), rhs.get_center()), DOWN, buff=1.5 * LARGE_BUFF
        )
    ...
    def put_step_on_curve(self):
        ...
        graph_points = [
            graph.point_from_proportion(alpha) for alpha in np.linspace(0, 1, 1000)
        ]
        distances = np.apply_along_axis(
            lambda p: np.abs(p[0] - arrow_end[0]), 1, graph_points
        )
...
class DerivativeOfNaturalLog(ZoomedScene):
    ...
    def show_slope_above_x(self):
        ...
        self.play(
            UpdateFromAlphaFunc(line, update_line, rate_func=there_and_back, run_time=6)
        )
        self.wait()
