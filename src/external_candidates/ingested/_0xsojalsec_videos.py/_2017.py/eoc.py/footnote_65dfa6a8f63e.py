...
class Introduce(TeacherStudentsScene):
    def construct(self):
        ...
        derivs = VGroup(
            *[
                OldTex("{d", "^%d" % n, "f \\over dx", "^%d}" % n).set_color_by_tex(
                    str(n), YELLOW
                )
                for n in range(2, 5)
            ]
...
class SecondDerivativeGraphically(GraphScene):
    ...
    def show_curvature(self):
        ...
        self.animate_secant_slope_group_change(
            self.ss_group, target_x=self.x1, run_time=0
        )
    ...
    def contrast_big_and_small_concavity(self):
        ...
        self.play(
            ReplacementTransform(
                input_label.copy(), graph.arg_rhs.get_part_by_tex(str(x0))
            ),
...
class HowToReadNotation(GraphScene, ReconfigurableScene):
    ...
    def take_two_steps(self):
        v_lines = [
            self.get_vertical_line_to_graph(
                self.x + i * self.dx, self.graph, line_class=DashedLine, color=WHITE
            )
            for i in range(3)
        ]
    ...
    def show_ddf(self):
        ...
        ddf.scale(
            df_labels[0].get_height() / ddf.get_height(), about_point=ddf.get_right()
        )
...
class SecondDerivativeAsAcceleration(Scene):
    ...
    def introduce_acceleration(self):
        a_words = OldTex(
            "{d^2 s \\over dt^2}(t)", "\\Leftrightarrow", "\\text{Acceleration}"
        )
