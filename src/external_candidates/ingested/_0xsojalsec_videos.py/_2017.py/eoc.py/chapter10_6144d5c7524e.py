...
class ExampleApproximation(GraphScene):
    ...
    def construct(self):
        ...
        approx_graphs = [
            self.get_graph(
                taylor_approximation(self.function, n), self.approximation_color
            )
            for n in self.order_sequence
        ]

        near_text = OldTexText(
            "Near %s $= %d$" % (self.x_axis_label, self.center_point)
        )
...
class Pendulum(ReconfigurableScene):
    ...
    def show_oscillation(self):
        ...
        self.play(
            ShowCreation(
                trajectory_dots, rate_func=linear, run_time=kwargs["run_time"]
            ),
    ...
    def get_pendulum(self):
        line = Line(
            self.anchor_point, self.anchor_point + self.radius * DOWN, color=WHITE
        )
        weight = Circle(
...
class ConstructQuadraticApproximation(ExampleApproximationWithCos):
    ...
    def add_cosine_graph(self):
        ...
        self.play(
            ShowCreation(cosine_graph), UpdateFromFunc(dot, update_dot), run_time=4
        )
    ...
    def add_quadratic_graph(self):
        ...
        self.play(
            ReplacementTransform(self.cosine_graph.copy(), quadratic_graph, run_time=3)
        )
    ...
    def show_value_at_zero(self):
        ...
        v_line = self.get_vertical_line_to_graph(
            0, self.cosine_graph, line_class=DashedLine, color=YELLOW
        )
        ...
        for x, run_time in (-1, 2), (1, 4):
            self.play(
                self.get_tangent_line_change_anim(
                    self.tangent_line, x, self.cosine_graph, run_time=run_time
                )
            )
            self.wait()
    ...
    def compute_polynomial_second_derivative(self):
        ...
        second_derivs = [
            OldTex("{d^2 P \\over dx^2}", "(x)", "=", "2", c2) for c2 in c2s
        ]
        ...
        poly_group_target = VGroup(
            second_derivs[1], derivs[1], self.get_quadratic_tex("1", "0", c2s[1])
        )
    ...
    def get_quadratic_derivative(self, c1, c2, arg="x"):
        ...
        if hasattr(self, "quadratic_tex"):
            result.next_to(
                self.quadratic_tex, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
            )
        return result
    ...
    def get_cosine_derivative(self):
        ...
        derivative.next_to(
            self.cosine_label, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        rhs = OldTex("=", "-\\sin(0)", "=", "0")
    ...
    def get_cosine_second_derivative(self):
        ...
        second_deriv.next_to(
            self.cosine_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        rhs = OldTex("=", "-\\cos(0)", "=", "-1")
...
class ReflectOnQuadraticApproximation(TeacherStudentsScene):
    ...
    def show_example_approximation(self):
        ...
        self.play(
            Write(approx_at_x, run_time=2), self.teacher.change_mode, "raise_right_hand"
        )
        self.wait(2)
        self.play(
...
class CubicAndQuarticApproximations(ConstructQuadraticApproximation):
    ...
    def take_third_derivative_of_cubic(self):
        ...
        self.play(
            ReplacementTransform(
                polynomial.quadratic_part.copy(), VGroup(third_derivative[1])
            )
        )
        self.wait(2)
    ...
    def show_third_derivative_of_cosine(self):
        ...
        self.play(
            ReplacementTransform(
                self.cosine_second_derivative.copy(), cosine_third_derivative
            )
        )
        self.wait(2)
    ...
    def show_fourth_derivative_of_cosine(self):
        ...
        self.play(
            ReplacementTransform(
                self.cosine_third_derivative.copy(), cosine_fourth_derivative
            )
        )
        self.wait(3)
    ...
    def take_fourth_derivative_of_quartic(self):
        ...
        fourth_deriv_lhs.next_to(
            self.polynomial, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
    ...
    def get_cosine_third_derivative(self):
        ...
        third_deriv.next_to(
            self.cosine_second_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        rhs = OldTex("=", "\\sin(0)", "=", "0")
    ...
    def get_cosine_fourth_derivative(self):
        ...
        fourth_deriv.next_to(
            self.cosine_third_derivative, DOWN, buff=MED_LARGE_BUFF, aligned_edge=LEFT
        )
        rhs = OldTex("=", "\\cos(0)", "=", "1")
...
class FactorialTerms(CubicAndQuarticApproximations):
    def construct(self):
        ...
        added_anims = [
            ReplacementTransform(
                start_lhs, target_lhs, rate_func=squish_rate_func(smooth, 0, 0.5)
            )
...
class ApproximateNearNewPoint(CubicAndQuarticApproximations):
    ...
    def get_quartic_approximation(self, a):
        ...
        func = lambda x: sum(
            [(c / math.factorial(n)) * (x - a) ** n for n, c in enumerate(coefficients)]
        )
...
class TranslationOfInformation(CubicAndQuarticApproximations):
                ...
                UpdateFromAlphaFunc(
                    ...
                    lambda d, a: d.move_to(
                        self.input_to_graph_point(
                            interpolate(start_x, x, a), self.cosine_graph
                        )
                    ),
                    run_time=2,
                )
    ...
    def show_derivative_pattern(self):
        ...
        for start_d, arrow, target_d in zip(group[::2], group[1::2], group[2::2]):
            self.play(
                ReplacementTransform(start_d.copy(), target_d), ShowCreation(arrow)
            )
            self.wait()
        self.wait()
    ...
    def name_taylor_polynomial(self):
        brace = Brace(
            VGroup(self.polynomial.coefficients, self.polynomial.factorials), DOWN
        )
    ...
    def write_general_function_derivative(self):
        ...
        self.play(
            ReplacementTransform(zeros.copy(), self.dot, run_time=2, lag_ratio=0.5)
        )
    ...
    def replace_coefficients_in_generality(self):
        new_polynomial = self.get_polynomial(
            "x", *[tex_mob.get_tex() for tex_mob in self.derivs_at_zero[:-1]]
        )
        ...
        self.play(
            ReplacementTransform(
                self.polynomial, new_polynomial, run_time=2, lag_ratio=0.5
            ),
    ...
    def walk_through_terms(self):
        ...
        approx_graphs = [
            self.get_graph(taylor_approximation(func, n), color=WHITE) for n in range(7)
        ]
    ...
    def show_polynomial_around_a(self):
        new_polynomial = self.get_polynomial(
            "(x-a)", *[tex_mob.get_tex() for tex_mob in self.derivs_at_a[:-2]]
        )
        new_polynomial.to_corner(UP + RIGHT)
        ...
        self.play(
            Transform(self.polynomial, new_polynomial), FadeOut(polynomial_third_term)
        )
        self.wait()
        for x in -1, np.pi / 6:
            self.play(
...
class ExpPolynomial(TranslationOfInformation, ExampleApproximationWithExp):
    ...
    def show_derivatives(self):
        ...
        derivs_at_x, derivs_at_zero = [
            VGroup(*[OldTex("e^%s" % s).set_color(c) for c in self.colors])
            for s in ("x", "0")
        ]
        ...
        for start_d, arrow, target_d in zip(group[::2], group[1::2], group[2::2]):
            self.play(
                ReplacementTransform(start_d.copy(), target_d), Write(arrow, run_time=1)
            )
            self.wait()
        self.wait()
        self.play(ReplacementTransform(derivs_at_x, derivs_at_zero))
...
class SecondTermIntuition(AreaIsDerivative):
    ...
    def compute_triangle_area(self):
        ...
        group.target[-1].next_to(
            group.target[-2], RIGHT, buff=SMALL_BUFF, align_using_submobjects=True
        )
        group.target[1].shift(0.02 * DOWN)
        group.target.to_corner(UP + RIGHT)
        exp_2 = OldTex("2").scale(0.8 * tex_scale_factor)
        exp_2.next_to(
            group.target[-2], UP + RIGHT, buff=0, align_using_submobjects=True
        )
...
class ExpGraphConvergence(ExpPolynomial, ExpConvergenceExample):
    ...
    def construct(self):
        ...
        v_line = self.get_vertical_line_to_graph(
            self.example_input, graph, line_class=DashedLine, color=YELLOW
        )
        dot = Dot(color=YELLOW)
        ...
        self.play(
            dot.move_to, self.input_to_graph_point(x, graph), ShowCreation(v_line)
        )
        self.wait()
    ...
    def get_approx_graphs(self):
        ...
        approx_graphs = [
            self.get_graph(get_nth_approximation(n)) for n in range(self.n_iterations)
        ]
...
class BoundedRadiusOfConvergence(CubicAndQuarticApproximations):
    ...
    def construct(self):
        ...
        v_line = self.get_vertical_line_to_graph(
            0, graph, line_class=DashedLine, color=YELLOW
        )
        ...
        outer_v_lines = VGroup(
            *[
                DashedLine(
                    self.coords_to_point(x, -2), self.coords_to_point(x, 2), color=WHITE
                )
                for x in (-1, 1)
            ]
        )
