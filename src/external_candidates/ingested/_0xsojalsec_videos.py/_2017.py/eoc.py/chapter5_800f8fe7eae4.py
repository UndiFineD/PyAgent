# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter5.py
...
class ExpFootnoteOpeningQuote(OpeningQuote):
    CONFIG = {
        "quote": [
            ...
            "$y = e^x$,",
            "like a phoenix rising again from its own",
            "ashes, is its own derivative?",
        ],
        "highlighted_quote_terms": {"$y = e^x$": MAROON_B},
...
class DoublingPopulation(PiCreatureScene):
    ...
    def introduce_expression(self):
        ...
        pop_brace, mass_brace = [Brace(function[0], DOWN) for function in (P_t, M_t)]
        ...
        self.play(Write(f_x))
        self.wait()
        self.play(Transform(f_x, f_t), FadeIn(t_expression, run_time=2, lag_ratio=0.5))
        self.play(Transform(f_x, P_t))
        self.play(GrowFromCenter(pop_brace), Write(pop_brace.text, run_time=2))
        self.wait(2)
    ...
    def ask_about_dM_dt(self):
        ...
        self.play(
            FadeOut(self.pop_brace), FadeOut(self.pop_brace.text), Write(dM_dt_question)
        )
        self.wait(3)
        for mob in dM, dt:
    ...
    def let_day_pass_and_highlight_new_creatures(self, frac):
        ...
        self.let_one_day_pass()
        new_creatures = VGroup(*self.get_on_screen_pi_creatures()[-num_new_creatures:])
        rect = Rectangle(
            color=GREEN,
            fill_color=BLUE,
            fill_opacity=0.3,
    ...
    def relate_growth_rate_to_pop_size(self):
        false_deriv = OldTex("{d(2^t) ", "\\over dt}", "= 2^t")
        difference_eq = OldTex("{ {2^{t+1} - 2^t} \\over", "1}", "= 2^t")
        real_deriv = OldTex("{ {2^{t+dt} - 2^t} \\over", "dt}", "= \\, ???")
        VGroup(
            false_deriv[0][3],
            false_deriv[2][-1],
            difference_eq[0][1],
        ...
        self.play(
            *list(map(FadeIn, [false_deriv, false_deriv.brace, false_deriv.brace_text]))
        )
        self.wait()
        ...
        rect.stretch_to_fit_width(rect.get_width() + MED_SMALL_BUFF)
        rect.stretch_to_fit_height(rect.get_height() + MED_SMALL_BUFF)
        self.play(*list(map(FadeOut, [false_deriv.brace, false_deriv.brace_text])))
    ...
    def create_pi_creatures(self):
        ...
        creature_array = VGroup(
            *[
                VGroup(*[PiCreature(mode="plain") for y in range(height)]).arrange(
    ...
    def get_num_pi_creatures_on_screen(self):
        ...
        return sum([pi in mobjects for pi in self.get_pi_creatures()])
    ...
    def get_from_day_to_day_label(self):
        ...
        top_words = OldTexText("From day", str(curr_day), "to", str(curr_day + 1), ":")
        ...
        bottom_words = OldTex(
            str(2**curr_day), "\\text{ creatures}", "\\over {1 \\text{ day}}"
        )
...
class GraphOfTwoToT(GraphScene):
    ...
    def construct(self):
        ...
        self.animate_secant_slope_group_change(ss_group, target_dx=1, run_time=0)
        ...
        for target_dx in 0.01, 1, 0.01:
            self.animate_secant_slope_group_change(ss_group, target_dx=target_dx)
            self.wait()

        # Mark up with values

        self.play(ShowCreation(v_line))
        self.play(GrowFromCenter(brace), Write(height_label, run_time=1))
        self.wait()
        self.play(
            FadeIn(slope_label, run_time=4, lag_ratio=0.5),
            ReplacementTransform(
                height_label.copy(), slope_label.get_part_by_tex("2^")
            ),
        )
        self.wait()
    ...
    def get_label_group(self, t):
        ...
        slope_label = OldTex(
            "\\text{Slope}", "=", "2^%d" % t, "(%.7f\\dots)" % np.log(2)
        )
        slope_label.next_to(
            ss_group.secant_line.point_from_proportion(0.65), DOWN + RIGHT, buff=0
        )
...
class FakeDiagram(TeacherStudentsScene):
    def construct(self):
        ...
        background_graph, foreground_graph = graphs = VGroup(
            *[
                gs.get_graph(lambda t: np.log(2) * 2**t, x_min=-8, x_max=2 + dx)
                for dx in (0.25, 0)
            ]
        )
        for graph in graphs:
        ...
        two_to_t = OldTex("2^t")
        two_to_t.next_to(foreground_graph.get_corner(DOWN + RIGHT), UP + LEFT)
        corner_line = Line(*[graph.get_corner(DOWN + RIGHT) for graph in graphs])
        ...
        diagram = VGroup(
            graphs, two_to_t, dt_brace, dt, side_brace, deriv, circle, words, arrow
        )
        ...
        self.play(ShowCreation(circle), self.teacher.change_mode, "hooray")
        self.play_student_changes(*["confused"] * 3)
        self.play(
            Write(words), ShowCreation(arrow), self.teacher.change_mode, "shruggie"
        )
        self.wait(3)
        self.play(
            FadeOut(diagram),
            *[ApplyMethod(pi.change_mode, "plain") for pi in self.get_pi_creatures()],
        )
        self.teacher_says("More numerical \\\\ than visual...")
        self.wait(2)
...
class AnalyzeExponentRatio(PiCreatureScene):
    ...
    def construct(self):
        ...
        two_to_t_two_to_dt = OldTex("%s^" % base_str, "t", "%s^" % base_str, "{dt}")
        ...
        expressions = [ratio, two_to_t_two_to_dt, extracted_two_to_t, lhs, func_def]
        ...
        self.play(
            Write(ratio), Write(lhs), self.pi_creature.change_mode, "raise_right_hand"
        )
        self.wait(2)
        self.play(
            two_to_t_plus_dt.next_to,
            exp_prop_brace,
            UP,
        ...
        words = VGroup(
            *[OldTexText(s, " ideas") for s in ("Additive", "Multiplicative")]
        )
        ...
        self.play(shifter.next_to, ratio[7], UP, stretcher.stretch_in_place, 0.9, 0)
        self.wait(2)
        ...
        self.play(Transform(brace, alt_brace), dt_to_zero.next_to, alt_brace, DOWN)
        self.wait()
    ...
    def try_specific_dt_values(self):
        ...
        self.play(Write(curr_expression), self.pi_creature.change_mode, "pondering")
        self.wait(2)
...
class CompareTwoConstantToEightConstant(PiCreatureScene):
    def construct(self):
        two_deriv, eight_deriv = derivs = VGroup(
            *[self.get_derivative_expression(base) for base in (2, 8)]
        )

        derivs.arrange(DOWN, buff=1.5, aligned_edge=LEFT)
        ...
        self.wait()
        self.play(ReplacementTransform(eight_deriv.copy(), two_deriv))
        self.wait()
        self.play(ShowCreation(arrow))
        self.play(Write(times_three), self.pi_creature.change_mode, "thinking")
        self.wait(3)

        self.play(
            Animation(derivs), Write(why), self.pi_creature.change, "confused", derivs
        )
        self.wait()
        for deriv in derivs:
            for index in -5, -2:
    ...
    def get_derivative_expression(self, base):
        ...
        tex_color_paris = [("t", YELLOW), ("dt", GREEN), (const_str, BLUE)]
...
class AskAboutConstantOne(TeacherStudentsScene):
    def construct(self):
        ...
        self.student_says("Is there a base where\\\\", "that constant is 1?")
        ...
        self.teacher_says("There is!\\\\", "$e = 2.71828\\dots$", target_mode="hooray")
        self.play_student_changes(*["confused"] * 3)
        self.wait(3)
...
class GraphOfExp(GraphScene):
    ...
    def show_slopes(self):
        ...
        graph_label = self.get_graph_label(graph, "e^t", direction=LEFT)
        ...
        slope_labels = [
            OldTexText("Slope = $e^%d$" % x)
            .next_to(vl.get_top(), UP + RIGHT)
            .shift(0.7 * RIGHT / x)
            for vl, x in zip(v_lines, self.example_inputs)
        ]
        ...
        self.animate_secant_slope_group_change(
            ...
            run_time=2,
            added_anims=[
                Transform(*pair, path_arc=np.pi / 6, run_time=2)
                for pair in [
                    slope_labels,
                    v_lines,
                    height_labels,
...
class ApplyChainRule(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            *[
                Transform(
                    *deriv_equation.get_parts_by_tex(tex, substring=False).copy()[:2],
                    path_arc=-np.pi,
                    run_time=2,
                )
                for tex in ("e^", "{3", "t}")
        ...
        self.play(Transform(*deriv_equation.get_parts_by_tex("3").copy()[-1:-3:-1]))
...
class ChainRuleIntuition(ThreeLinesChainRule):
    ...
    def nudge_x(self):
        ...
        def get_value_points():
            return [label[0].get_bottom() for label in labels]
        ...
        braces = VGroup()
        numbers = VGroup()
        for start, end, line, label, config in zip(
            starts, ends, lines, labels, self.line_configs
        ):
        ...
        for count in range(3):
            for dx in self.dx, 0:
                self.animate_x_change(self.example_x + dx, run_time=2)
        self.wait()
...
class WhyNaturalLogOf2ShowsUp(TeacherStudentsScene):
    ...
    def add_e_to_the_three_t(self):
        ...
        self.play(
            FadeIn(exp_c, run_time=2, lag_ratio=0.5),
            self.teacher.change,
            "raise_right_hand",
        )
        self.wait()
    ...
    def show_e_to_log_2(self):
        ...
        derivative_arrow.next_to(derivative.get_parts_by_tex("="), UP)
        ...
        self.play(
            Write(log_words),
            ShowCreation(
                log_words_arrow, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)
            ),
        )
        self.play_student_changes(*["pondering"] * 3, look_at=log_words)
        self.wait(2)
        ...
        self.play(
            ShowCreation(derivative_arrow), Write(derivative_symbol), Write(derivative)
        )
        self.wait(3)
        self.play(self.teacher.change_mode, "happy")
        self.wait(2)
    ...
    def get_a_to_t(self, a):
        ...
        result = OldTex(
            "{d(", a_str, "^t", ")", "\\over", "dt}", "=", log_str, a_str, "^t"
        )
...
class CompareWaysToWriteExponentials(GraphScene):
    ...
    def construct(self):
        ...
        graphs = [self.get_graph(lambda t: base**t, color=GREEN) for base in bases]
        graph = graphs[0]
        ...
        self.play(
            ShowCreation(graph),
            Write(a_to_t, rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=2,
        )
        ...
        for new_graph, new_equation in zip(graphs, equations)[1:]:
            self.play(Transform(graph, new_graph), Transform(equation, new_equation))
            self.wait(2)
        self.wait()
    ...
    def construct(self):
        ...
        for part in group:
            ...
            if const:
                ...
                log = brace.get_text("$\\log_{%s}(2)$" % part[1].get_tex()[:-1])
                log.set_color(BLUE)
                part.add(brace, log)
        ...
        self.play_student_changes(*["pondering"] * 3, look_at=exp)
        self.wait(3)


class TooManySymbols(TeacherStudentsScene):
    def construct(self):
        self.student_says("Too symbol heavy!", target_mode="pleading")
...
class TemperatureOverTimeOfWarmWater(GraphScene):
    ...
    def construct(self):
        self.setup_axes()
        graph = self.get_graph(lambda t: 3 * np.exp(-0.3 * t) + self.T_room, color=RED)
        h_line = DashedLine(
            *[self.coords_to_point(x, self.T_room) for x in (self.x_min, self.x_max)]
        )
        T_room_label = OldTex("T_{\\text{room}}")
        T_room_label.next_to(h_line, LEFT)

        ode = OldTex("\\frac{d\\Delta T}{dt} = -k \\Delta T")
        ode.to_corner(UP + RIGHT)

        solution = OldTex("\\Delta T(", "t", ") = e", "^{-k", "t}")
        ...
        def update_delta_T_group(group):
            ...
            label.set_height(min(label.get_height(), brace.get_height()))
            label.next_to(brace, RIGHT, SMALL_BUFF)

        self.add(ode)
        self.play(Write(T_room_label), ShowCreation(h_line, run_time=2))
        if self.include_solution:
            self.play(Write(solution))
        graph_growth = ShowCreation(graph, rate_func=linear)
        delta_T_group_update = UpdateFromFunc(delta_T_group, update_delta_T_group)
        self.play(
...
class InvestedMoney(Scene):
    def construct(self):
        # cash_str = "\\$\\$\\$"
        cash_str = "M"
        equation = OldTex("{d", cash_str, "\\over", "dt}", "=", "(1 + r)", cash_str)
        ...
        solution = OldTex(cash_str, "(", "t", ")", "=", "e^", "{(1+r)", "t}")
        solution.set_color_by_tex("t", YELLOW)
...
class NaturalLog(Scene):
    def construct(self):
        ...
        self.play(FadeIn(expressions, run_time=3, lag_ratio=0.5))
        self.wait()
        self.play(expressions.set_fill, None, 1, run_time=2, lag_ratio=0.5)
        self.wait()
        for i in 0, 2, 1:
    ...
    def show_natural_loggedness(self, expression):
        ...
        VGroup(exp_base, exp_equals).next_to(
            VGroup(e, exp_constant), RIGHT, aligned_edge=DOWN
        )
...
class NextVideo(TeacherStudentsScene):
    def construct(self):
        ...
        this_tex = OldTex("{d(", "a^t", ") \\over dt} = ", "a^t", "\\ln(a)")
        this_tex[1][1].set_color(YELLOW)
        this_tex[3][1].set_color(YELLOW)
        this_tex.next_to(brace, DOWN)

        next_tex = VGroup(
            *list(map(TexText, ["Chain rule", "Product rule", "$\\vdots$"]))
        )
        next_tex.arrange(DOWN)
        next_tex.next_to(brace, DOWN)
        next_tex.shift(
            next_video.get_center()[0] * RIGHT - next_tex.get_center()[0] * RIGHT
        )

        self.add(series, brace, *this_tex[:3])
        self.play_student_changes("confused", "pondering", "erm", look_at=this_tex)
        self.play(ReplacementTransform(this_tex[1].copy(), this_tex[3]))
        self.wait()
        self.play(
            Write(this_tex[4]),
            ReplacementTransform(
                this_tex[3][0].copy(), this_tex[4][3], path_arc=np.pi, remover=True
            ),
        )
        self.wait(2)
        ...
        self.play_student_changes(*["pondering"] * 3, look_at=next_tex)
        self.wait(3)
...
class Thumbnail(GraphOfTwoToT):
    ...
    def construct(self):
        derivative = OldTex("\\frac{d(a^t)}{dt} =", "a^t \\ln(a)")
