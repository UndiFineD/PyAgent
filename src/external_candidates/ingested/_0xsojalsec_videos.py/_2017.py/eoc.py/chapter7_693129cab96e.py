# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter7.py
# -*- coding: utf-8 -*-
    ...
    def construct(self):
        ...
        start_x_v_line, nudged_x_v_line = [
            self.get_vertical_line_to_graph(
                self.start_x + nudge, graph, line_class=DashedLine, color=RED
            )
        ...
        self.animate_secant_slope_group_change(
            ss_group, target_dx=self.start_dx, run_time=2
        )
        ...
        self.animate_secant_slope_group_change(
            ...
            target_dx=0.01,
            run_time=5,
            added_anims=[Transform(nudged_x_v_line, start_x_v_line, run_time=5)],
        )
        self.wait(2)
...
class RantOpenAndClose(Scene):
    def construct(self):
        opening, closing = [
            OldTexText(start, "Rant on infinitesimals", "$>$", arg_separator="")
            for start in ("$<$", "$<$/")
        ]
...
class DiscussLowercaseDs(RefreshOnDerivativeDefinition, PiCreatureScene, ZoomedScene):
    ...
    def skip_superclass_anims(self):
        ...
        self.play(
            *[ApplyMethod(mob.shift, 2.5 * LEFT) for mob in everything]
    ...
    def replace_dx_terms(self):
        ...
        for tex in "\\Delta x", "h":
            dx_list_replacement = [
                OldTex(tex).set_color(self.dx_color).move_to(dx, DOWN) for dx in dx_list
            ]
            self.play(
                Transform(
                    VGroup(*dx_list),
    ...
    def compare_rhs_and_lhs(self):
        ...
        self.animate_secant_slope_group_change(
            ...
            target_dx=0.01,
            added_anims=[
                Transform(self.nudged_x_v_line, self.start_x_v_line, run_time=3)
            ],
        )
        ...
        self.wait(2)
        self.play(
            ReplacementTransform(df.copy(), df_target, path_arc=-np.pi / 2, run_time=2)
        )
        self.wait(2)
    ...
    def h_is_finite(self):
        self.play(
            ...
            self.definition_group.to_corner,
            UP + RIGHT,
            self.pi_creature.change_mode,
            "sassy",
            self.pi_creature.look_at,
        ...
        h_group = VGroup(
            self.rhs[1].get_part_by_tex("dx"), *self.rhs[2].get_parts_by_tex("dx")
        )
        moving_h = h_group[0]
    ...
    def construct(self):
        ...
        for statement in statements:
            statement.h, statement.dx = [
                VGroup(*statement.get_parts_by_tex(tex, substring=False)).set_color(
                    GREEN
                )
                for tex in ("$h$", "$dx$")
            ]
...
class GraphLimitExpression(GraphScene):
    ...
    def show_limit(self):
        ...
        self.play(
            ReplacementTransform(
                ed_group.input_range.copy(), ed_group.output_range, run_time=2
            ),
        )
    ...
    def skeptic_asks(self):
        ...
        self.play(
            RemovePiCreatureBubble(
                randy, target_mode="pondering", look_at=self.limit_point_hole
            )
        )
        self.play(
    ...
    def show_epsilon_delta_intuition(self):
        self.play(
            FadeOut(self.ed_group.epsilon_lines), FadeIn(self.input_range_brace_group)
        )
        self.ed_group.epsilon_lines.restore()
        self.wait()
        self.play(
        ...
        self.play(
            ReplacementTransform(
                self.ed_group.input_range.copy(), self.ed_group.output_range, run_time=2
            )
        )
        self.wait()
        ...
        self.animate_epsilon_delta_group_change(
            ...
            added_anims=[ApplyMethod(hole.scale, 0.5, run_time=8) for hole in holes],
        )
        self.wait()
    ...
    def get_epsilon_delta_group(
        ...
        dashed_line = DashedLine(
            ORIGIN, dashed_line_length * RIGHT, stroke_width=dashed_line_stroke_width
        )
    ...
    def animate_epsilon_delta_group_change(
        self, epsilon_delta_group, target_delta, **kwargs
    ):
        ...
        def update_ed_group(ed_group, alpha):
            ...
            new_group = self.get_epsilon_delta_group(
                delta, limit_x=limit_x, **ed_group_kwargs
            )
            Transform(ed_group, new_group).update(1)
            return ed_group

        self.play(
            UpdateFromAlphaFunc(epsilon_delta_group, update_ed_group, **kwargs),
            *added_anims,
        )
...
class LimitCounterExample(GraphLimitExpression):
    ...
    def show_epsilon_delta_intuition(self):
        ...
        self.wait()
        self.animate_epsilon_delta_group_change(
            ed_group, target_delta=self.small_delta, run_time=6
        )
...
class EpsilonDeltaExample(GraphLimitExpression, ZoomedScene):
    ...
    def match_epsilon(self):
        self.animate_epsilon_delta_group_change(
            ...
            run_time=2,
            added_anims=[
                ApplyMethod(hole.scale, 0.25, run_time=2) for hole in self.holes
            ],
        )
    ...
    def introduce_delta(self):
        ...
        self.play(
            ApplyWave(
                VGroup(self.ed_group.output_range, self.holes[1]), direction=RIGHT
            )
        )
        self.wait(2)
    ...
    def smaller_epsilon(self):
        ...
        self.animate_epsilon_delta_group_change(
            ...
            added_anims=[Transform(self.delta_group, self.get_delta_group(new_delta))]
            + [ApplyMethod(hole.scale, 0.5) for hole in self.holes],
        )
        self.ed_group.input_range.make_jagged()
        self.wait(2)
    ...
    def get_delta_group(self, delta):
        ...
        result.labels = VGroup(
            *[
                OldTex("\\delta").scale(1.0 / self.zoom_factor)
                for brace in result.braces
            ]
        )
        for label, brace in zip(result.labels, result.braces):
...
class EpsilonDeltaCounterExample(LimitCounterExample, EpsilonDeltaExample):
    ...
    def introduce_epsilon_delta_group(self):
        ...
        self.wait(2)
        self.animate_epsilon_delta_group_change(
            ed_group, target_delta=self.small_delta, run_time=3
        )
...
class TheoryHeavy(TeacherStudentsScene):
    def construct(self):
        ...
        Scene.play(
            ...
            *[
                ApplyFunction(
                    lambda pi: pi.change_mode("pondering").look_at(epsilon_delta), pi
                )
        ...
        self.student_says(
            ...
            index=2,
            added_anims=[derivative.scale, 0.8, derivative.to_corner, UP + LEFT],
        )
        self.play(self.get_teacher().change_mode, "happy")
        self.wait(2)


class LHopitalExample(
    LimitCounterExample, PiCreatureScene, ZoomedScene, ReconfigurableScene
):
    CONFIG = {
        "graph_origin": ORIGIN,
    ...
    def introduce_function(self):
        ...
        self.play(
            Write(func_label),
            Transform(x_copy, VGroup(*func_label.get_parts_by_tex("x")), remover=True),
        )
    ...
    def show_non_definedness_at_one(self):
        ...
        v_line, alt_v_line = [
            self.get_vertical_line_to_graph(
                x, self.graph, line_class=DashedLine, color=self.x_color
            )
            for x in (1, -1)
        ]
        ...
        self.play(
            RemovePiCreatureBubble(morty, target_mode="pondering", look_at=func_1),
        ...
        self.play(
            ReplacementTransform(ed_group.input_range.copy(), ed_group.output_range),
            *list(map(ShowCreation, ed_group.delta_lines)),
        )
        self.play(*list(map(GrowFromCenter, ed_group.epsilon_lines)))
        self.play(morty.change_mode, "thinking")
        self.animate_epsilon_delta_group_change(
            ed_group, target_delta=self.small_delta, run_time=4
        )
        self.wait()
        self.play(
            Write(lim),
        ...
        self.wait(2)
        self.play(
            func_1_group.to_corner, UP + LEFT, *list(map(FadeOut, [morty, ed_group]))
        )
        self.wait()
    ...
    def ask_about_systematic_process(self):
        ...
        self.play(
            RemovePiCreatureBubble(
                morty, target_mode="raise_left_hand", look_at=self.func_1_group
            ),
    ...
    def show_graph_of_numerator_and_denominator(self):
        ...
        sine_label = self.get_graph_label(
            sine_graph, "\\sin(\\pi x)", x_val=4.5, direction=UP
        )
    ...
    def talk_through_sizes_of_nudges(self):
        ...
        d_sine_arrow, d_parabola_arrow = [
            Arrow(
                ...
                self.coords_to_point(
                    1 + self.dx, graph.underlying_function(1 + self.dx)
                ),
                tip_length=arrow_tip_length,
        ...
        self.wait()
        self.play(
            ReplacementTransform(VGroup(dx_label[1].copy()), d_sine_label, run_time=2)
        )
        self.wait(2)
        ...
        self.play(
            ReplacementTransform(
                VGroup(dx_label[1].copy()), d_parabola_label, run_time=2
            )
        )
        self.wait(2)
    ...
    def substitute_x_equals_1(self, tex_mob, zoom_tex_scale_factor):
        ...
        equation.next_to(
            x, UP, buff=MED_SMALL_BUFF / self.zoom_factor, aligned_edge=LEFT
        )
        ...
        self.wait()
        self.play(
            ReplacementTransform(x.copy(), eq_x), Transform(x, one), Write(equals)
        )
        self.wait()
    ...
    def show_final_ratio(self):
        ...
        self.transition_to_alt_config(
            transformation_kwargs={"run_time": 2}, dx=self.dx / 10
        )
        self.wait()
...
class GeneralLHoptial(LHopitalExample):
    ...
    def show_limit_in_symbols(self):
        ...
        self.play(
            ...
            ReplacementTransform(
                VGroup(*self.g_label).copy(), VGroup(frac_a.denominator)
            ),
            Write(frac_a.over),
            run_time=2,
        )
        ...
        self.play(
            ReplacementTransform(
                VGroup(*frac_a.get_parts_by_tex("a")), VGroup(lim.get_part_by_tex("a"))
            )
        )
...
class CannotUseLHopital(TeacherStudentsScene):
    def construct(self):
        ...
        self.wait()
        answer = OldTex(
            "\\text{That requires knowing }", "{d(e^x)", "\\over \\,", "dx}"
        )
