# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\chapter9.py
import fractions
    ...
    def get_v_line_change_anim(self, v_line, graph, target_x, **kwargs):
        ...
        def update(v_line, alpha):
            ...
            v_line.put_start_and_end_on(
                self.coords_to_point(new_x, 0), self.input_to_graph_point(new_x, graph)
            )
            return v_line
...
class AverageOfSineStart(AverageOfContinuousVariable):
    ...
    def ask_about_average(self):
        ...
        v_line = self.get_vertical_line_to_graph(
            midpoint, self.graph, line_class=DashedLine, color=WHITE
        )
        ...
        for bound in self.bounds + [midpoint]:
            self.play(
                self.get_v_line_change_anim(v_line, self.graph, bound, run_time=3)
            )
    ...
    def get_graph_portion_between_bounds(self):
        self.graph_portion_between_bounds = self.get_graph(
            self.func, x_min=self.bounds[0], x_max=self.bounds[1], color=YELLOW
        )
...
class LengthOfDayGraph(GraphScene):
    ...
    def add_graph(self):
        ...
        self.play(
            ...
            FadeIn(
                graph_label, rate_func=squish_rate_func(smooth, 0.5, 1), lag_ratio=0.5
            ),
            run_time=3,
        )
        self.wait()
    ...
    def set_color_summer_months(self):
        ...
        summer_rect.replace(
            Line(self.graph_origin, self.coords_to_point(365 / 2, 15.5)), stretch=True
        )
    ...
    def mention_constants(self):
        ...
        self.play(
            *[
                ...
                for constant, a in zip(constants, np.linspace(0, 0.3, len(constants)))
            ]
        )
        self.wait()
    ...
    def construct(self):
        ...
        dividing_lines = VGroup(
            *[
                ...
                for p in [
                    interpolate(
                        lines.target.get_left(), lines.target.get_right(), alpha
                    )
                ]
            ]
        )
        ...
        self.play(
            *it.chain(
                list(map(Write, averages)), list(map(GrowFromCenter, lower_braces))
            )
        )
...
class TryToAddInfinitelyManyPoints(AverageOfSineStart):
        ...
        v_lines = VGroup(
            *[
                self.get_vertical_line_to_graph(
                    numerator * np.pi / denominator,
                    self.graph,
                    color=YELLOW,
                    stroke_width=6.0 / denominator,
                )
    ...
    def show_continuum(self):
        ...
        self.play(
            arrow.next_to, input_range.get_start(), DOWN + RIGHT, SMALL_BUFF, run_time=3
        )
    ...
    def show_finite_sample(self):
        ...
        self.play(
            ReplacementTransform(
                v_lines.copy(), summed_v_lines, run_time=3, lag_ratio=0.5
            ),
...
class IntegralOfSine(FiniteSample):
    ...
    def show_riemann_rectangles(self):
        ...
        for i in range(start_index + 1, end_index):
            ...
            self.wait()
        self.play(
            rects.restore, *list(map(FadeOut, [sin_x, dx, side_brace, bottom_brace]))
        )
    ...
    def bring_back_average(self):
        ...
        self.play(
            *[
                ...
                for line, a in zip(self.v_lines, np.linspace(0, 0.7, len(self.v_lines)))
            ]
        ...
        self.play(
            ReplacementTransform(
                self.x_axis_labels[1].copy(), rhs.get_part_by_tex("pi")
            )
        )
        self.play(FadeOut(input_range))
        self.play(
            ReplacementTransform(example_dx.copy(), rhs.get_part_by_tex("0.1")),
    ...
    def show_moving_v_line(self):
        ...
        for count in range(2):
            for x in self.bounds + [mean]:
                self.play(
                    self.get_v_line_change_anim(v_line, self.graph, x, run_time=3)
                )
...
class Antiderivative(AverageOfSineStart):
    ...
    def negate_derivative_of_cosine(self):
        ...
        for graph, label, x_val, vect in zip(graphs, labels, x_vals, vects):
            graph.label = self.get_graph_label(
                graph, label, x_val=x_val, direction=vect, buff=SMALL_BUFF
            )
        ...
        for graph in cos, neg_sin:
            self.play(
                ...
                Write(graph.label, rate_func=squish_rate_func(smooth, 0.3, 1)),
                run_time=2,
            )
            self.wait()
        self.wait()
    ...
    def walk_through_slopes(self):
        ...
        v_line = self.get_vertical_line_to_graph(
            0, neg_cos, line_class=DashedLine, color=WHITE
        )

        ss_group = self.get_secant_slope_group(
            0, neg_cos, dx=0.001, secant_line_color=YELLOW
        )
    ...
    def apply_ftoc(self):
        ...
        (start_pi, end_pi), (start_zero, end_zero) = start_end_pairs = [
            [m.get_part_by_tex(tex) for m in (integral, rhs)] for tex in ("\\pi", "0")
        ]
        ...
        for start, end in start_end_pairs:
            self.play(
                ReplacementTransform(start.copy(), end, path_arc=np.pi / 6, run_time=2)
            )
            self.wait()
    ...
    def show_difference_in_antiderivative(self):
        pi_point, zero_point = points = [
            self.input_to_graph_point(x, self.neg_cos) for x in reversed(self.bounds)
        ]
    ...
    def divide_by_pi(self):
        ...
        frac_lines = VGroup(
            *[
                OldTex("\\over\\,").stretch_to_fit_width(mob.get_width()).move_to(mob)
                for mob in (integral, rhs_without_eq)
            ]
        )
        frac_lines.shift((integral.get_height() / 2 + SMALL_BUFF) * DOWN)
        pi_minus_zeros = VGroup(
            *[OldTex("\\pi", "-", "0").next_to(line, DOWN) for line in frac_lines]
        )
        ...
        self.antiderivative_fraction = VGroup(
            rhs_without_eq, frac_lines[1], pi_minus_zeros[1]
        )
        self.integral_fraction = VGroup(
            integral, frac_lines[0], pi_minus_zeros[0], equals
        )
...
class GeneralAverage(AverageOfContinuousVariable):
    ...
    def add_graph(self):
        ...
        self.play(
            ShowCreation(graph),
            Write(graph_label, rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=2,
        )
        for line, label in zip(v_lines, labels):
    ...
    def ask_about_average(self):
        v_line = self.get_vertical_line_to_graph(
            self.bounds[0], self.graph, line_class=DashedLine, color=WHITE
        )
    ...
    def show_finite_sample(self):
        v_lines = self.get_vertical_lines_to_graph(
            self.graph, x_min=self.bounds[0], x_max=self.bounds[1], color=GREEN
        )
        for line in v_lines:
        ...
        self.play(
            *[
                ApplyFunction(
                    ...
                    v_line,
                    run_time=3,
                    rate_func=squish_rate_func(there_and_back, a, a + 0.2),
                )
        ...
        self.play(
            ...
            VGroup(add_up_f_over, rhs_copy).shift,
            0.4 * DOWN,
        )
...
class GeneralAntiderivative(GeneralAverage):
    ...
    def add_antiderivative_graph(self):
        ...
        antideriv_graph = self.get_graph(
            lambda x: scipy.integrate.quad(self.graph.underlying_function, 1, x)[0],
            color=YELLOW,
            x_max=x_max,
        )
        antideriv_graph_label = self.get_graph_label(
            antideriv_graph, "F(x)", x_val=x_max
        )
        ...
        self.play(
            ...
            Write(antideriv_graph_label, rate_func=squish_rate_func(smooth, 0.5, 1)),
            run_time=2,
        )
        self.wait()
    ...
    def show_average_in_terms_of_F(self):
        ...
        new_fraction.next_to(
            self.fraction.get_part_by_tex("over"), RIGHT, align_using_submobjects=True
        )

        to_write = VGroup(
            *it.chain(*[new_fraction.get_parts_by_tex(tex) for tex in "=F()-"])
        )
    ...
    def show_change_in_height(self, numerator):
        ...
        a_point, b_point = points = [
            self.input_to_graph_point(x, self.antideriv_graph) for x in self.bounds
        ]
