# Extracted from: C:\DEV\PyAgent\.external\0xSojalSec-videos\_2017\eoc\old_chapter1.py
...
class CircleScene(PiCreatureScene):
    ...
    def get_outer_ring(self):
        return self.get_ring(
            radius=self.radius, dR=self.dR, color=self.outer_ring_color
        )
    ...
    def get_unwrapped(self, ring, to_edge=LEFT, **kwargs):
        ...
        result.set_points_as_corners(
            ...
            + [
                interpolate(
                    np.pi * R * RIGHT + ring.dR * UP, np.pi * R * LEFT + ring.dR * UP, a
                )
                for a in np.linspace(0, 1, n_anchors / 2)
            ]
        )
...
class Introduction(TeacherStudentsScene):
    ...
    def show_series(self):
        ...
        self.play(
            ...
            *[
                ApplyFunction(lambda p: p.change_mode("hooray").look_at(series[1]), pi)
        ...
        self.play(
            ...
            *[ApplyMethod(pi.look_at, series[-1]) for pi in self.get_pi_creatures()],
            run_time=5,
        )
        self.play(
            FadeOut(self.teacher.bubble),
            FadeOut(self.teacher.bubble.content),
            *[ApplyMethod(pi.change_mode, "happy") for pi in self.get_pi_creatures()],
        )
...
class IntroduceCircle(Scene):
    def construct(self):
        ...
        area, circumference = words = VGroup(
            *list(map(TexText, ["Area =", "Circumference ="]))
        )
        area.set_color(BLUE)
        ...
        self.play(
            Transform(area_form.copy(), circum_form, path_arc=-np.pi / 2, run_time=3)
        )
        self.wait()
        self.play(
            area_form.copy().replace,
            derivative[1],
    ...
    def construct(self):
        ...
        rect_sets = [
            self.get_riemann_rectangles(
                0, self.x_max, 1.0 / (2**n), stroke_width=1.0 / (n + 1)
            )
            for n in range(6)
        ]
...
class PragmatismToArt(Scene):
    def construct(self):
        ...
        self.play(
            ...
            morty.look_at,
            art,
            Transform(VectorizedPoint(morty.get_corner(UP + RIGHT)), art),
            pragmatism.fade,
            0.7,
            pragmatism.rotate,
            np.pi / 4,
...
class IntroduceTinyChangeInArea(CircleScene):
    ...
    def show_unwrapping(self, outer_ring):
        ...
        self.unwrap_ring(
            almost_rect, added_anims=[self.pi_creature.change_mode, "pondering"]
        )
        ...
        final_area = VGroup(
            change_in_area, two_pi_R.target, lp, dR.target.scale(1.0 / 0.7), rp
        )
        ...
        self.add(final_area)
        self.play(
            self.pi_creature.change_mode, "happy", self.pi_creature.look_at, final_area
        )
        self.wait()
    ...
    def work_out_expression(self, expression_group):
        exp, exp_brace, title, area_brace, area_word, new_area_brace, new_area_word = (
            expression_group
        )
        ...
        pi_R_squared, plus, two_pi_R_dR, plus2, pi_dR_squared, minus2, pi_R_squared2 = (
            expanded
        )
        ...
        self.wait(2)
        self.play(
            *[ApplyMethod(mob.set_color, RED) for mob in (pi_R_squared, pi_R_squared2)]
        )
        self.wait()
        self.play(
            *[
    ...
    def second_unwrapping(self, outer_ring):
        ...
        self.play(
            almost_rect.set_color, YELLOW, self.pi_creature.change_mode, "pondering"
        )
...
class BuildToDADR(CircleScene):
    ...
    def show_fractions(self):
        ...
        self.play(
            self.circum_term[0].move_to,
            self.circum_term.frac_line,
            LEFT,
            self.circum_term[0].shift,
            0.1 * UP,
            FadeOut(self.circum_term.frac_line),
            MaintainPositionRelativeTo(self.circum_term.label, self.circum_term[0]),
        )
        ...
        self.play(
            self.error_term.move_to,
            self.error_term.frac_line,
            self.error_term.shift,
            0.3 * LEFT + 0.15 * UP,
    ...
    def transition_to_dR(self):
        ...
        difference_text = OldTexText(
            "``Tiny ", "d", "ifference in ", "$R$", "''", arg_separator=""
        )
    ...
    def not_infinitely_small(self):
        ...
        for n in range(7):
            ...
            self.play(
                ...
                self.nudge_line.stretch_to_fit_width,
                0.1 / (n + 1),
            )
        self.wait()
        bubble.write("Wrong!")
...
class NameDerivative(IntroduceTinyChangeInArea):
    ...
    def interpret_geometrically(self):
        ...
        self.play(
            ...
            MoveToTarget(
                circum_circle, run_time=2, rate_func=squish_rate_func(smooth, 0.5, 1)
            ),
            self.pi_creature.change_mode,
            "hooray",
        )
    ...
    def show_limiting_process(self):
        ...
        self.play(
            Transform(ring_group, big_group), self.pi_creature.change_mode, "sassy"
        )
            ...
            if n < num_fracs:
                ...
                anims.append(
                    Transform(
                        ring_group,
                        small_group,
                        rate_func=lambda t: t * (1.0 / (num_fracs - n)),
                        run_time=2,
                    )
                )
            elif n > num_fracs:
    ...
    def emphasize_equality(self):
        ...
        self.play(
            self.two_pi_R.set_stroke, YELLOW, 3, rate_func=there_and_back, run_time=2
        )
        self.wait()
...
class DerivativeAsTangentLine(ZoomedScene):
    ...
    def show_tangent_lines(self):
        ...
        self.last_alpha = alphas[
            np.argmin([get_norm(point - curr_graph_point) for point in graph_points])
        ]
    ...
    def state_commonality(self):
        ...
        self.play(
            ShowCreation(bubble), Write(bubble.content), morty.change_mode, "surprised"
        )
        self.play(Blink(morty))
        self.wait()
...
class IntroduceConcentricRings(CircleScene):
    ...
    def build_up_rings(self):
        self.circle.set_fill(opacity=0)
        rings = VGroup(
            *[self.get_ring(r, self.dR) for r in np.arange(0, self.radius, self.dR)]
        )
        ...
        self.wait()
        self.play(
            FadeIn(VGroup(*rings[:-1]), lag_ratio=1, run_time=5),
    ...
    def add_up_areas(self):
        ...
        area_sum = VGroup(
            *it.chain(*list(zip([ring for ring in moving_rings.target], plusses)))
        )
    ...
    def unwrap_special_ring(self):
        ...
        self.to_fade = VGroup(
            brace, left_brace, two_pi_r, dr, think_concrete, arrow, less_wrong
        )
    ...
    def write_integral(self):
        ...
        integral_expression = VGroup(
            def_int, self.two_pi_r.target, self.dr.target, equals_pi_R_squared
        )
    ...
    def grow_and_shrink_r_line(self, zero_target, R_target):
        ...
        equals_0.target.next_to(
            self.radial_line.target.get_center_of_mass(), UP + LEFT, buff=SMALL_BUFF
        )
    ...
    def ask_about_approx(self):
        ...
        self.play(
            *[
            ...
            + [FadeOut(self.dr_group), Animation(self.foreground_group)]
        )
        self.wait()
        self.play(
        ...
        self.wait(2)
        self.play(
            self.integral_expression.set_color_by_gradient,
            BLUE,
            GREEN,
...
class GraphIntegral(GraphScene):
    ...
    def add_rectangles(self):
        ...
        arrows = VGroup(
            *[
                Arrow(values_words.get_bottom(), tick.get_center(), tip_length=0.15)
                for tick in ticks
            ]
        )

        dr_brace = Brace(
            VGroup(*ticks[special_tick_index : special_tick_index + 2]), buff=SMALL_BUFF
        )
        ...
        graph_label = self.label_graph(
            self.graph, "f(r) = 2\\pi r", proportion=0.5, direction=LEFT, animate=False
        )
        self.play(
            rectangles.restore,
            Animation(ticks),
...
class FundamentalTheorem(CircleScene):
    ...
    def setup(self):
        ...
        group = VGroup(
            self.circle, self.radius_line, self.radius_brace, self.radius_label
        )
        self.remove(*group)
        group.shift(DOWN)
    ...
    def add_derivative_terms(self):
        ...
        area_circle = Circle(
            stroke_width=0, fill_color=self.area_color, fill_opacity=0.5, radius=radius
        )
    ...
    def add_integral_terms(self):
        ...
        circum_circle = Circle(
            radius=geometric[1].get_width() / 2, color=self.circum_color
        )
    ...
    def bring_in_circle(self):
        self.play(
            ...
            self.derivative_terms[1].to_corner,
            UP + LEFT,
            MED_LARGE_BUFF,
            self.integral_terms[1].to_corner,
            UP + RIGHT,
            MED_LARGE_BUFF,
            self.pi_creature.change_mode,
            "speaking",
    ...
    def show_outer_ring(self):
        ...
        self.play(
            GrowFromCenter(brace), Write(deriv), self.pi_creature.change_mode, "happy"
        )
...
class NameTheFundamentalTheorem(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            GrowFromCenter(brace),
            Write(abstract),
            *[ApplyMethod(pi.look_at, symbols) for pi in self.get_pi_creatures()],
        )
...
class CalculusInANutshell(CircleScene):
    ...
    def morph_word(self):
        ...
        rings = VGroup(
            *[self.get_ring(rad, 0.95 * dR) for rad in np.arange(0, self.radius, dR)]
        )
        for ring in rings:
